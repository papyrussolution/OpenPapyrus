...
typedef int yy_state_t; /** State numbers. */
typedef int yyRuleNum; /** Rule numbers. */
typedef short yyItemNum; /** Item references. */
...
_Noreturn static void yyFail (yyGLRStack* yystackp]b4_pure_formals[, const char* yymsg)
{
	if(yymsg != YY_NULLPTR)
...
[[static const char * yysymbol_name(yysymbol_kind_t yysymbol)
{
  ...
  /* YYTRANSLATABLE[SYMBOL-NUM] -- Whether YY_SNAME[SYMBOL-NUM] is internationalizable.  */
  static ]b4_int_type_for([b4_translatable])[ yytranslatable[] =
  {
  ]b4_translatable[
  };
  return (yysymbol < YYNTOKENS && yytranslatable[yysymbol] ? _(yy_sname[yysymbol]) : yy_sname[yysymbol]);]], [[return yy_sname[yysymbol];]])[
}]])[
#endif
