...
// 
// Test harness will assert: argon2_hash() returns ARGON2_OK
// HEX output matches expected encoded output matches expected argon2_verify() correctly verifies value
// 
static void hashtest(uint32 version, uint32 t, uint32 m, uint32 p, const char * pwd, const char * salt, const char * hexref, const char * mcfref, argon2_type type) 
{
	uchar  out[OUT_LEN];
	uchar  hex_out[OUT_LEN * 2 + 4];
	char   encoded[ENCODED_LEN];
	int    ret;
	int    i;
	printf("Hash test: $v=%d t=%d, m=%d, p=%d, pass=%s, salt=%s: ", version, t, m, p, pwd, salt);
	ret = argon2_hash(t, 1 << m, p, pwd, strlen(pwd), salt, strlen(salt), out, OUT_LEN, encoded, ENCODED_LEN, type, version);
...
static int main_Argon2_Test() 
{
	...
	ret = argon2_hash(2, 1, 1, "password", strlen("password"), "diffsalt", strlen("diffsalt"), out, OUT_LEN, NULL, 0, Argon2_id, version);
	assert(ret == ARGON2_MEMORY_TOO_LITTLE);
	printf("Fail on invalid memory: PASS\n");
	ret = argon2_hash(2, 1 << 12, 1, NULL, strlen("password"), "diffsalt", strlen("diffsalt"), out, OUT_LEN, NULL, 0, Argon2_id, version);
	assert(ret == ARGON2_PWD_PTR_MISMATCH);
	printf("Fail on invalid null pointer: PASS\n");
	ret = argon2_hash(2, 1 << 12, 1, "password", strlen("password"), "s", 1, out, OUT_LEN, NULL, 0, Argon2_id, version);
	assert(ret == ARGON2_SALT_TOO_SHORT);
