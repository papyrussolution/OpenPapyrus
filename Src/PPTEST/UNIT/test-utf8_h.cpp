...
const char _Cmp[] = {'\xce', '\xbc', '\xcf', '\x85', '\0'};
const char lt[] = {'\xce', '\x93', '\xce', '\xb1', '\xce', '\xb6', '\xce', '\xac', '\0'};
const char gt[] = {'\xce', '\x93', '\xce', '\xb1', '\xce', '\xb6', '\xce', '\xae', '\0'};
const char spn[] = {'\xce', '\x93', '\xce', '\xb1', '\xce', '\xb6',
		    '\xce', '\xad', '\xce', '\xb5', '\xcf', '\x82', '\x20', '\xce', '\xba', '\0'};
...
SLTEST_R(utf8_h)
{
	...
	{
		char cat[512] = {'\0'};
		SLCHECK_EQ(0U, utf8len(cat));
		SLCHECK_EQ(53U, utf8len(utf8cat(cat, data)));
	}
	{
		char cat[512];
		cat[0] = 'a';
		cat[1] = '\0';
		SLCHECK_EQ(1U, utf8len(cat));
		SLCHECK_EQ(54U, utf8len(utf8cat(cat, data)));
	}
	{
		char cat[512];
		cat[0] = '\xce';
	...
	{
		char cpy[512] = {'\0'};
		SLCHECK_EQ(53U, utf8len(utf8cpy(cpy, data)));
	}
	// Matches \xce\x93 \xce\xb1 \xce\xb6 \xce\xad \xce\xb5 \xcf\x82 \x20 \xce\xba
	// \xce\xb1
	SLCHECK_EQ(9U, utf8spn(data, spn));
	SLCHECK_EQ(53U, utf8spn(data, data));
	SLCHECK_EQ(0U, utf8spn(data, "ab"));
	SLCHECK_EQ(2U, utf8spn(spnasciisearch, spnasciidelims));
	SLCHECK_EQ(0U, utf8cspn(data, spn));
	SLCHECK_EQ(0U, utf8cspn(data, data));
	SLCHECK_EQ(53U, utf8cspn(data, "ab"));
	SLCHECK_EQ(12U, utf8cspn(cspnmultisearch, cspnmultidelims));
	SLCHECK_EQ(data + 21, utf8rchr(data, 0x3bc));
	SLCHECK_EQ(NULL, utf8rchr(data, 0x20ac));
	SLCHECK_EQ(data + 104, utf8rchr(data, '\0'));
	...
	}
	{
		...
		SLCHECK_EQ(5U, static_cast<uint>(utf8rchr(ascii, 'o') - ascii));
	}
	{
		utf8_int8_t * dup = utf8dup(data);
		SLCHECK_NZ(dup);
		SLCHECK_EQ(53U, utf8len(dup));
		SAlloc::F(dup);
	}
	{
		utf8_int8_t * dup = utf8dup("ab");
		SLCHECK_NZ(dup);
		SLCHECK_EQ(2U, utf8len(dup));
		SAlloc::F(dup);
	}
	{
		utf8_int8_t * dup = utf8dup("");
		SLCHECK_NZ(dup);
		SLCHECK_EQ(0U, utf8len(dup));
		SAlloc::F(dup);
	}
	{
		utf8_int8_t * dup = utf8ndup("1234567890", 4);
		SLCHECK_NZ(dup);
		SLCHECK_EQ(4U, utf8len(dup));
		SAlloc::F(dup);
	}
	{
		utf8_int8_t * dup = utf8ndup("1234567890", 100);
		SLCHECK_NZ(dup);
		SLCHECK_EQ(10U, utf8len(dup));
		SAlloc::F(dup);
	}
	SLCHECK_LT(0, utf8cmp(data, lt));
	SLCHECK_EQ(0, utf8cmp(data, data));
	SLCHECK_LE(utf8cmp(data, gt), 0);
	//UTEST(utf8dup_ex, ascii) 
	{
		char user_data[1024];
		utf8_int8_t * dup = utf8dup_ex("1234567890", allocate_from_buffer, user_data);
		SLCHECK_NZ(dup);
		SLCHECK_EQ(dup, user_data);
		SLCHECK_EQ(10U, utf8len(dup));
	}
	//UTEST(utf8ndup_ex, ascii) 
	{
		char user_data[1024];
		utf8_int8_t * dup = utf8ndup_ex("1234567890", 4, allocate_from_buffer, user_data);
		SLCHECK_NZ(dup);
		SLCHECK_EQ(dup, user_data);
		SLCHECK_EQ(4U, utf8len(dup));
	}
	SLCHECK_EQ(105U, utf8size(data));
	SLCHECK_EQ(3U, utf8size("ab"));
	SLCHECK_EQ(1U, utf8size(""));
	SLCHECK_EQ(104U, utf8size_lazy(data));
	SLCHECK_EQ(2U, utf8size_lazy("ab"));
	SLCHECK_EQ(0U, utf8size_lazy(""));
	SLCHECK_EQ(50U, utf8nsize_lazy(data, 50));
	SLCHECK_EQ(2U, utf8nsize_lazy("ab", 50));
	SLCHECK_EQ(0U, utf8nsize_lazy("", 50));
	{
		invalid[0] = '\xf0';
		invalid[1] = '\x8f';
		invalid[2] = '\xbf';
		invalid[3] = '\xbf';
		invalid[4] = '\0';
		SLCHECK_EQ(invalid, utf8valid(invalid));
	}
	{
		invalid[0] = '\xf1';
		invalid[1] = '\x3f';
	...
	SLCHECK_EQ(NULL, utf8valid(data));
	SLCHECK_EQ(NULL, utf8valid("ab"));
	SLCHECK_EQ(NULL, utf8valid(""));
	{
		char valid[3];
		...
		valid[1] = '\x80';
		valid[2] = '\0';
		SLCHECK_EQ(invalid, utf8nvalid(valid, invalid_size));
	}
	{
		char valid[4];
		const char * invalid = valid;
	...
	SLCHECK_EQ(NULL, utf8nvalid(data, 105));
	SLCHECK_EQ(NULL, utf8nvalid("ab", 3));
	SLCHECK_EQ(NULL, utf8nvalid("", 1));
	{
		char cat[512] = {'\0'};
		cat[0] = 'a';
		cat[1] = '\0';
		SLCHECK_EQ(2U, utf8len(utf8ncat(cat, data, 2)));
	}
	{
		char cat[512] = {'\0'};
		SLCHECK_EQ(53U, utf8len(utf8ncat(cat, data, 40000)));
	}
	{
		char cat[512] = {'\0'};
		SLCHECK_EQ(cat, utf8valid(utf8ncat(cat, data, 1)));
	}
	{
		char cat[512] = {'\0'};
		SLCHECK_EQ(NULL, utf8valid(utf8ncat(cat, data, 0)));
	}
	{
		SLCHECK_LT(0, utf8ncmp(data, lt, 4000));
	}
	{
		SLCHECK_EQ(0, utf8ncmp(data, lt, 7));
	}
	{
		SLCHECK_EQ(0, utf8ncmp(data, data, 4000));
	}
	{
		SLCHECK_EQ(0, utf8ncmp(data, data, 7));
	}
	{
		SLCHECK_LE(utf8ncmp(data, gt, 4000), 0);
	}
	//UTEST(utf8ncmp, gt_small) 
	{
		SLCHECK_EQ(0, utf8ncmp(data, gt, 7));
	}
	//UTEST(utf8ncpy, data_null_terminated) 
	{
		char cpy[512] = {'\0'};
		SLCHECK_EQ('\0', *((char *)utf8ncpy(cpy, data, 106) + 105));
	}
	//UTEST(utf8ncpy, data) 
	{
		char cpy[512] = {'\0'};
		SLCHECK_EQ(53U, utf8len(utf8ncpy(cpy, data, 105)));
	}
	//UTEST(utf8ncpy, check_no_buffer_overflow) 
	{
		...
		SLCHECK_EQ('o', buffer[2]);
		for(i = 3; 10 != i; i++) {
			SLCHECK_EQ(0, buffer[i]);
		}
		SLCHECK_EQ((char)0xdd, buffer[10]);
	}
	//UTEST(utf8ncpy, check_no_n_overflow) 
	{
		char buffer[4] = {1, 2, 3, 4};
		SLCHECK_EQ(buffer, utf8ncpy(buffer, "foo", 2));

		SLCHECK_EQ('f', buffer[0]);
		SLCHECK_EQ('o', buffer[1]);
		SLCHECK_EQ(3, buffer[2]);
		SLCHECK_EQ(4, buffer[3]);
	}
	//UTEST(utf8ncpy, truncated_copy_valid) 
	{
		...
		SLCHECK_EQ(NULL, utf8valid(cpy1));

		utf8ncpy(cpy2, data, 3);
		SLCHECK_EQ(NULL, utf8valid(cpy2));

		utf8ncpy(cpy3, data, 1);
		SLCHECK_EQ(NULL, utf8valid(cpy3));
	}
	...
	//UTEST(utf8pbrk, pbrk) 
	{
		SLCHECK_EQ(data + 8, utf8pbrk(data, pbrk));
	}
	//UTEST(utf8pbrk, data)
	{
		SLCHECK_EQ(data, utf8pbrk(data, data));
	}
	//UTEST(utf8casecmp, ascii) 
	{
		SLCHECK_EQ(0, utf8casecmp(ascii1, ascii2));
	}
	//UTEST(utf8casecmp, latin_upvslow) 
	{
		SLCHECK_EQ(0, utf8casecmp(lowersStr, uppersStr));
	}
	//UTEST(utf8casecmp, latin_lowvsup) 
	{
		SLCHECK_EQ(0, utf8casecmp(uppersStr, lowersStr));
	}
	//UTEST(utf8casecmp, allascii) 
	{
		SLCHECK_EQ(0, utf8casecmp(allascii1, allascii2));
	}
	//UTEST(utf8casecmp, data_lt) 
	{
		SLCHECK_LT(0, utf8casecmp(data, lt));
	}
	//UTEST(utf8casecmp, data_eq) 
	{
		SLCHECK_EQ(0, utf8casecmp(data, data));
	}
	//UTEST(utf8casecmp, data_gt) 
	{
		SLCHECK_LE(utf8casecmp(data, gt), 0);
	}
	//UTEST(utf8ncasecmp, lt_large) 
	{
		SLCHECK_LT(0, utf8ncasecmp(data, lt, 4000));
	}
	//UTEST(utf8ncasecmp, lt_small)
	{
		SLCHECK_EQ(0, utf8ncasecmp(data, lt, 7));
	}
	//UTEST(utf8ncasecmp, eq_large) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(data, data, 4000));
	}
	//UTEST(utf8ncasecmp, eq_small) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(data, data, 7));
	}
	//UTEST(utf8ncasecmp, gt_large) 
	{
		SLCHECK_LE(utf8ncasecmp(data, gt, 4000), 0);
	}
	//UTEST(utf8ncasecmp, gt_small) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(data, gt, 7));
	}
	//UTEST(utf8ncasecmp, ascii) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(ascii1, ascii2, 4));
	}
	//UTEST(utf8ncasecmp, latin_upvslow) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(lowersStr, uppersStr, 120));
	}
	//UTEST(utf8ncasecmp, latin_lowvsup) 
	{
		SLCHECK_EQ(0, utf8ncasecmp(uppersStr, lowersStr, 120));
	}
	//UTEST(utf8ncasecmp, basic_ascii) 
	{
		SLCHECK_EQ(-15, utf8ncasecmp(".gdoc", ".GSHeeT", 5));
		SLCHECK_EQ(-4, utf8ncasecmp(".gsheet", ".gSLiDe", 7));
	#ifndef _MSC_VER
		SLCHECK_EQ(strcasecmp(".gdoc", ".GSHeeT"), utf8ncasecmp(".gdoc", ".GSHeeT", 5));
		SLCHECK_EQ(strcasecmp(".gsheet", ".gSLiDe"), utf8ncasecmp(".gsheet", ".gSLiDe", 7));
	#endif
	}
	//UTEST(utf8ncasecmp, latin_extended_a) 
	{
		SLCHECK_EQ(96, utf8ncasecmp("Camón Romasan", "camu", 4));
	}
	//UTEST(utf8codepoint, data) 
	{
		utf8_int32_t codepoint;
		size_t expected_length = utf8len(data) - 1;
		for(utf8_int8_t * v = utf8codepoint(data, &codepoint); codepoint; v = utf8codepoint(v, &codepoint)) {
			SLCHECK_EQ(expected_length, utf8len(v));
			expected_length -= 1;
		}
	}
	//UTEST(utf8codepointcalcsize, data) 
	{
		...
		for(const char * v = data; *v; v += utf8codepointcalcsize(v)) {
			SLCHECK_EQ(expected_length, utf8len(v));
			expected_length -= 1;
		}
	}
	//UTEST(utf8codepointsize, size_1) 
	{
		SLCHECK_EQ(1U, utf8codepointsize('A'));
	}
	//UTEST(utf8codepointsize, size_4) 
	{
		SLCHECK_EQ(4U, utf8codepointsize(0x20C78));
	}
	//UTEST(utf8catcodepoint, data) 
	{
		...
		SLCHECK_EQ(51U, utf8len(buffer));
	}
	//UTEST(utf8islower, upper) 
	{
		for(utf8_int32_t i = 0; 0 != lowupPairs[i].lower; i++) {
			SLCHECK_EQ(0, utf8islower(lowupPairs[i].upper));
		}
	}
	//UTEST(utf8islower, lower) 
	{
		for(utf8_int32_t i = 0; 0 != lowupPairs[i].lower; i++) {
			SLCHECK_EQ(1, utf8islower(lowupPairs[i].lower));
		}
	}
	//UTEST(utf8isupper, upper) 
	{
		for(utf8_int32_t i = 0; 0 != lowupPairs[i].lower; i++) {
			SLCHECK_EQ(1, utf8isupper(lowupPairs[i].upper));
		}
	}
	//UTEST(utf8isupper, lower) 
	{
		for(utf8_int32_t i = 0; 0 != lowupPairs[i].lower; i++) {
			SLCHECK_EQ(0, utf8isupper(lowupPairs[i].lower));
		}
	}
	//UTEST(utf8lwr, ascii) 
	{
		size_t sz = strlen(ascii1);
		char * str = (char *)SAlloc::M(sz + 1);
		memcpy(str, ascii1, sz + 1);
		utf8lwr(str);
		SLCHECK_EQ(0, strcmp(str, "i like goats yarhar."));
		SAlloc::F(str);
	}
	//UTEST(utf8lwr, latin_lower) 
	{
		size_t sz = utf8size(lowersStr);
		utf8_int8_t * str = (utf8_int8_t *)SAlloc::M(sz);
		memcpy(str, lowersStr, sz);
		utf8lwr(str);
		SLCHECK_EQ(0, utf8cmp(str, lowersStr));
		SAlloc::F(str);
	}
	...
	//UTEST(utf8upr, latin_lower) 
	{
		...
		memcpy(str, lowersStr, sz);
		utf8upr(str);
		SLCHECK_EQ(0, utf8cmp(str, uppersStr));
		SAlloc::F(str);
	}
	//UTEST(utf8upr, latin_upper) 
	{
		size_t sz = utf8size(uppersStr);
		utf8_int8_t * str = (utf8_int8_t *)SAlloc::M(sz);
		memcpy(str, uppersStr, sz);
		utf8upr(str);
		SLCHECK_EQ(0, utf8cmp(str, uppersStr));
	...
	{
		SLCHECK_EQ(-15, utf8casecmp(".gdoc", ".GSHeeT"));
		SLCHECK_EQ(-4, utf8casecmp(".gsheet", ".gSLiDe"));
	#ifndef _MSC_VER
		SLCHECK_EQ(strcasecmp(".gdoc", ".GSHeeT"), utf8casecmp(".gdoc", ".GSHeeT"));
		SLCHECK_EQ(strcasecmp(".gsheet", ".gSLiDe"), utf8casecmp(".gsheet", ".gSLiDe"));
	#endif
	}
	//UTEST(utf8lwr, greek_capital_theta) 
	{
		const char ref[] = {'\xce', '\xb8', '\xce', '\xb8', '\xce', '\xb8', '\xcf', '\x91', '\0'};
		char str[] = {'\xcf', '\xb4', '\xce', '\xb8', '\xce', '\x98', '\xcf', '\x91', '\0'};
		utf8lwr(str);
		SLCHECK_EQ(0, utf8cmp(str, ref));
	}
	...
	//UTEST(utf8casecmp, greek_capital_theta) 
	{
		const char ref[] = {'\xcf', '\xb4', '\xce', '\x98', '\xce', '\x98', '\xce', '\x98', '\0'};
		char str[] = {'\xcf', '\xb4', '\xce', '\xb8', '\xce', '\x98', '\xcf', '\x91', '\0'};
		SLCHECK_EQ(0, utf8casecmp(ref, str));
	}
	//UTEST(utf8ncasecmp, greek_capital_theta) 
	{
		const char ref[] = {'\xcf', '\xb4', '\xce', '\x98', '\xce', '\x98', '\xce', '\x98', '\0'};
		char str[] = {'\xcf', '\xb4', '\xce', '\xb8', '\xce', '\x98', '\xcf', '\x91', '\0'};
		SLCHECK_EQ(0, utf8ncasecmp(ref, str, 8));
	}
	{
		utf8_int32_t codepoint;
		SLCHECK_EQ(ascii1, utf8rcodepoint(ascii1 + 1, &codepoint));
		SLCHECK_EQ(ascii1[1], codepoint);
	}
	{
		utf8_int32_t codepoint;
		SLCHECK_EQ(data, utf8rcodepoint(data + 2, &codepoint));
		SLCHECK_EQ(0x3B1, codepoint);
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xf0';
		invalid[1] = '\x8f';
		invalid[2] = '\xbf';
		invalid[3] = '\xbf';
		SLCHECK_EQ(0, utf8makevalid(invalid, '?'));
		SLCHECK_EQ(invalid[0], '\xef');
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xf1';
		...
		invalid[3] = '\xbf';
		SLCHECK_EQ(0, utf8makevalid(invalid, '?'));
		SLCHECK_EQ(invalid[0], '?');
		SLCHECK_EQ(invalid[1], '?');
		SLCHECK_EQ(invalid[2], '?');
		SLCHECK_EQ(invalid[3], '?');
		SLCHECK_EQ(invalid[4], '\0');
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xf1';
	...
	}
	{
		...
		invalid[2] = '\xbf';
		SLCHECK_EQ(0, utf8makevalid(invalid, '?'));
		SLCHECK_EQ(invalid[0], '\xdf');
		SLCHECK_EQ(invalid[1], '\xbf');
		SLCHECK_EQ(invalid[2], '\0');
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xef';
	...
	}
	{
		...
		invalid[1] = '\xbf';
		SLCHECK_EQ(0, utf8makevalid(invalid, '?'));
		SLCHECK_EQ(invalid[0], '\x7f');
		SLCHECK_EQ(invalid[1], '\0');
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xdf';
	...
	}
	{
		...
		invalid[4] = '\xbf';
		SLCHECK_EQ(0, utf8makevalid(invalid, '?'));
		SLCHECK_EQ(invalid[0], '\xf1');
		SLCHECK_EQ(invalid[1], '\xbf');
		SLCHECK_EQ(invalid[2], '\xbf');
		SLCHECK_EQ(invalid[3], '\xbf');
		SLCHECK_EQ(invalid[4], '?');
		SLCHECK_EQ(invalid[5], '\0');
	}
	{
		memzero(invalid, sizeof(invalid));
		invalid[0] = '\xef';
	...
	}
	{
		...
		invalid[1] = '\xbf';
		invalid[2] = '\xbf';
		SLCHECK_NZ(utf8makevalid(invalid, 0x80));
	}
	{
		const char terminated[] = "\xc2\xa3";
		SLCHECK_EQ(utf8nvalid(terminated, 2), NULL);
	}
	{
		const char terminated[] = "\xe1\xbd\xb6";
		SLCHECK_EQ(utf8nvalid(terminated, 3), NULL);
	}
	{
		const char terminated[] = "\xf0\x90\x8d\x88";
		SLCHECK_EQ(utf8nvalid(terminated, 4), NULL);
	}
