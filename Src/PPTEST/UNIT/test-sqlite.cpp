...
SLTEST_R(SQLite)
{
	...
	{
		//
		// √енераци€ записей таблиц TestRef01, TestRef02, TestTa01
		//
	...
	}
	{
		...
		if(dbp.StartTransaction()) {
			{
				//
				// ƒл€ таблицы TestRef01 примен€ем BExtInsert
				//
	...
	{
		// ”далить каждую сотую запись и убедитьс€, что ее больше нет в таблице
		if(dbp.StartTransaction()) {
	...
	{
		//
		// ”далить записи из таблицы TestTa01 по запросу RVal1 в диапазоне [2.5..2.6]
		// 
#if 0 // @construction {
		RealRange range;
		range.Set(2.5, 2.6);
		LongArray idx_list_to_remove;
		{
			...
			if(idx_list_to_remove.getCount()) {
				TestTa01Tbl ta_tbl; // ¬ отличии от p_tbl здесь есть заданные дескрипторы полей
