...
SLTEST_R(LocalStateBinderyCore) // @v12.5.9
{
	...
	constexpr uint max_serial_sub_iter = 100;
	constexpr uint serial_delay_ms = 15;
	...
	{
		long   path_seq = 0;
		(path = GetSuiteEntry()->OutPath).SetLastSlash().Cat("LocalStateBindery");
		do {
			(temp_buf = path).SetLastSlash().CatLongZ(++path_seq, 4);
		} while(SFile::IsDir(temp_buf));
		path = temp_buf;
		LocalStateBinderyCore instance(SConstructorTest, path);
		SLCHECK_NZ(instance.IsValid());
		if(instance.IsValid()) {
			uint8  buf_to_put[4096];
			SBuffer buf_to_work;
			SBuffer buf_to_read;
			SBuffer buf_to_fetch;
			SRandGenerator & r_rg = SLS.GetTLA().Rg;
			{
				...
				for(uint i = 0; i < max_iter; i++) {
					...
					const uint buf_to_put_size = r_rg.GetUniformIntPos(4011);
					r_rg.ObfuscateBuffer(buf_to_put, buf_to_put_size);
					ident.Kind = LocalStateBinderyCore::kViewPosition;
					ident.Subj = (i % 5) + 1;
					PPID   new_id = 0;
					PPID   test_id = 0;
					PPID   test_id2 = 0;
					...
					if(rr) {
						...
						if(gr > 0) {
							if(test_id == new_id && buf_to_read.IsEq(buf_to_work))
								vr = 1;
						}
						//
						buf_to_fetch.Z();
			...
			if(!err_count) {
				...
				// Serial State
				//
				for(uint si = 0; si < max_serial_iter; si++) {
					const  int64  subj = (si + 1);
					TSCollection <LocalStateBinderyCore::SerialEntry> pattern_list;
					uint   local_err_count = 0;
					for(uint i = 0; i < max_serial_sub_iter; i++) {
						LocalStateBinderyCore::StateIdent ident;
						const uint buf_to_put_size = r_rg.GetUniformIntPos(4011);
						r_rg.ObfuscateBuffer(buf_to_put, buf_to_put_size);
						...
						PPID   test_id = 0;
						buf_to_work.Z().Write(buf_to_put, buf_to_put_size);
						const  int rr = instance.RegisterState(&new_id, ident, buf_to_work, 1);
						if(!rr) {
							local_err_count++;
						}
						else {
							LocalStateBinderyCore::SerialEntry * p_new_entry = pattern_list.CreateNewItem();
							p_new_entry->ID = new_id;
							p_new_entry->Buf = buf_to_work;
						}
						SDelay(serial_delay_ms);
					}
					err_count += local_err_count;
					SLCHECK_Z(local_err_count);
					if(!local_err_count) {
						TSCollection <LocalStateBinderyCore::SerialEntry> test_list;
						TSCollection <LocalStateBinderyCore::SerialEntry> test_list2;
						LocalStateBinderyCore::StateIdent ident;
						ident.Kind = LocalStateBinderyCore::kInput;
						ident.Subj = subj;
						const  int gr = instance.GetStateSerial(ident, &test_list);
						if(gr > 0) {
							SLCHECK_EQ(test_list.getCount(), pattern_list.getCount());
							if(test_list.getCount() == pattern_list.getCount()) {
								for(uint tli = 0; tli < test_list.getCount(); tli++) {
									const LocalStateBinderyCore::SerialEntry * p_test_entry = test_list.at(tli);
									assert(p_test_entry);
									SLCHECK_NZ(p_test_entry);
									if(p_test_entry) {
										uint   pli = 0;
										const  bool sr = pattern_list.lsearch(&p_test_entry->ID, &pli, CMPF_LONG);
										SLCHECK_NZ(sr);
										if(sr) {
											const LocalStateBinderyCore::SerialEntry * p_pattern_entry = pattern_list.at(pli);
											assert(p_pattern_entry);
											SLCHECK_NZ(p_pattern_entry);
											if(p_pattern_entry) {
												SLCHECK_NZ(p_pattern_entry->Buf.IsEq(p_test_entry->Buf));
											}
										}
									}
								}
							...
							SLCHECK_EQ(gr2, 2); // Так как RegisterState внес данные в inmem-список, то этот вызов должен быть строго из памяти
							{
								if(test_list2.getCount() == pattern_list.getCount()) {
									for(uint tli = 0; tli < test_list2.getCount(); tli++) {
										const LocalStateBinderyCore::SerialEntry * p_test_entry = test_list2.at(tli);
										assert(p_test_entry);
										SLCHECK_NZ(p_test_entry);
										if(p_test_entry) {
											uint   pli = 0;
