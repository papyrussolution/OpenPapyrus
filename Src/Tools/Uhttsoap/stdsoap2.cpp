		...
		for(;; ) {
				...
				for(;;
					...
					if(r < 0) {
						...
						if(r != SOAP_EINTR) {
							soap_closesock(soap);
							soap_set_sender_error(soap, tcp_error(
									soap), "accept failed in soap_accept()",
								SOAP_TCP_ERROR);
							return SOAP_INVALID_SOCKET;
						}
					}
			...
			if(soap->accept_timeout)
				SOAP_SOCKNONBLOCK(soap->master)
					...
					soap->socket = soap->faccept(soap, soap->master, (struct sockaddr *)&soap->peer,
						&n);
			...
			if(soap_valid_socket(soap->socket)) {
				...
				if(soap->accept_flags == SO_LINGER) {
					...
					if(setsockopt(soap->socket, SOL_SOCKET, SO_LINGER, (char *)&linger,
				        ...
				        setsockopt(soap->socket, SOL_SOCKET, soap->accept_flags, (char *)&set,
						sizeof(int))) {
					soap->errnum = soap_socket_errno(soap->socket);
					soap_set_receiver_error(soap, tcp_error(
				...
				if(setsockopt(soap->socket, SOL_SOCKET, SO_SNDBUF, (char *)&len, sizeof(int))) {
					soap->errnum = soap_socket_errno(soap->socket);
					soap_set_receiver_error(soap, tcp_error(
							soap), "setsockopt SO_SNDBUF failed in soap_accept()",
						SOAP_TCP_ERROR);
					soap_closesock(soap);
					return SOAP_INVALID_SOCKET;
				}
				if(setsockopt(soap->socket, SOL_SOCKET, SO_RCVBUF, (char *)&len, sizeof(int))) {
					soap->errnum = soap_socket_errno(soap->socket);
					soap_set_receiver_error(soap, tcp_error(
							soap), "setsockopt SO_RCVBUF failed in soap_accept()",
						SOAP_TCP_ERROR);
					soap_closesock(soap);
					return SOAP_INVALID_SOCKET;
				}
				...
				if(!(soap->omode&SOAP_IO_UDP) &&
				   setsockopt(soap->socket, IPPROTO_TCP, TCP_NODELAY, (char *)&set, sizeof(int))) {
					soap->errnum = soap_socket_errno(soap->socket);
					soap_set_receiver_error(soap, tcp_error(
							soap), "setsockopt TCP_NODELAY failed in soap_accept()",
						SOAP_TCP_ERROR);
					soap_closesock(soap);
					return SOAP_INVALID_SOCKET;
				}
...
SOAP_FMAC1
struct soap *
SOAP_FMAC2 soap_new1(soap_mode mode)
{
...
SOAP_FMAC1
struct soap *
SOAP_FMAC2 soap_new2(soap_mode imode, soap_mode omode)
{
...
SOAP_FMAC2 soap_element(struct soap * soap, const char * tag, int id, const char * type)
{
 ...
 #endif
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "Element begin tag='%s' id='%d' type='%s'\n", tag, id, type ? type : SOAP_STR_EOS));
 #ifdef WITH_DOM
  #ifndef WITH_LEAN
...
SOAP_FMAC2 soap_array_begin_out(struct soap * soap, const char * tag, int id, const char * type, const char * offset)
{
	...
	else {if(offset && soap_attribute(soap, "SOAP-ENC:offset", offset))
		      return soap->error;
	...
	for(tp = soap->attributes; tp; tp = tp->next) {
		if(tp->visible && tp->value)                                                {
			...
			if(!soap_match_tag(soap, tp->name, "xsi:type")) {
				...
				if(!soap_match_tag(soap, tp->name,
					   "SOAP-ENC:arrayType"))                              {
...
SOAP_FMAC2 soap_recv_fault(struct soap * soap, int check)
{
	...
	else {register const char * s = *soap_faultcode(soap);
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_set_sender_error(struct soap * soap, const char * faultstring, const char * faultdetailXML,
	int soaperror)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_set_receiver_error(struct soap * soap, const char * faultstring, const char * faultdetailXML,
	int soaperror)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_sender_fault_subcode(struct soap * soap, const char * faultsubcodeQName, const char * faultstring,
	const char * faultdetailXML)
{
