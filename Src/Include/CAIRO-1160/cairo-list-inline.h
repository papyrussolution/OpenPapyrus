...
#define cairo_list_foreach_entry(pos, type, head, member)               \
	for(pos = cairo_list_entry((head)->next, type, member); &pos->member != (head); \
	    pos = cairo_list_entry(pos->member.next, type, member))

#define cairo_list_foreach_entry_safe(pos, n, type, head, member)       \
	for(pos = cairo_list_entry((head)->next, type, member), n = cairo_list_entry(pos->member.next, type, member); &pos->member != (head); \
...
#define cairo_list_foreach_entry_reverse_safe(pos, n, type, head, member)       \
	for(pos = cairo_list_entry((head)->prev, type, member), n = cairo_list_entry(pos->member.prev, type, member); &pos->member != (head); \
