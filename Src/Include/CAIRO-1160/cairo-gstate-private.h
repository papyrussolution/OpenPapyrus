...
/* cairo-gstate.c */
cairo_private cairo_status_t _cairo_gstate_init(cairo_gstate_t * gstate,
...
cairo_private void FASTCALL _do_cairo_gstate_backend_to_user(const cairo_gstate_t * gstate, double * x, double * y);

static inline void _cairo_gstate_backend_to_user(const cairo_gstate_t * gstate, double * x, double * y)
{
	if(!gstate->is_identity)
		_do_cairo_gstate_backend_to_user(gstate, x, y);
}

cairo_private void _do_cairo_gstate_backend_to_user_distance(const cairo_gstate_t * gstate, double * x, double * y);

static inline void _cairo_gstate_backend_to_user_distance(const cairo_gstate_t * gstate, double * x, double * y)
{
	if(!gstate->is_identity)
		_do_cairo_gstate_backend_to_user_distance(gstate, x, y);
}

cairo_private void _cairo_gstate_backend_to_user_rectangle(cairo_gstate_t * gstate,
    double * x1, double * y1,
    double * x2, double * y2,
    cairo_bool_t * is_tight);

cairo_private void _cairo_gstate_path_extents(cairo_gstate_t * gstate,
...
cairo_private cairo_bool_t _cairo_gstate_in_fill(cairo_gstate_t * gstate,
    cairo_path_fixed_t * path,
    double x,
    double y);
cairo_private cairo_bool_t _cairo_gstate_in_clip(cairo_gstate_t * gstate,
    double x,
    double y);
...
cairo_private cairo_bool_t _cairo_gstate_clip_extents(cairo_gstate_t * gstate,
    double * x1,
    double * y1,
    double * x2,
    double * y2);
...
cairo_private cairo_status_t _cairo_gstate_tag_begin(cairo_gstate_t * gstate,
    const char * tag_name,
    const char * attributes);

cairo_private cairo_status_t _cairo_gstate_tag_end(cairo_gstate_t * gstate,
    const char * tag_name);
...
cairo_private void _cairo_gstate_set_font_options(cairo_gstate_t * gstate,
    const cairo_font_options_t * options);

cairo_private cairo_status_t _cairo_gstate_get_font_face(cairo_gstate_t * gstate,
    cairo_font_face_t ** font_face);

cairo_private cairo_status_t _cairo_gstate_get_scaled_font(cairo_gstate_t * gstate,
    cairo_scaled_font_t ** scaled_font);

cairo_private cairo_status_t _cairo_gstate_get_font_extents(cairo_gstate_t * gstate,
...
cairo_private cairo_status_t _cairo_gstate_show_text_glyphs(cairo_gstate_t * gstate,
    const cairo_glyph_t * glyphs,
    int num_glyphs,
