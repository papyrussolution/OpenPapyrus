 ...
 * compliance with the License. You may obtain a copy of the License at http://www.mozilla.og/MPL/
 *
...
typedef struct _cairo_cogl_device {
	...
	CoglMatrix identity;
	cairo_cache_t linear_cache; /* Caches 1d linear gradient textures */
...
cairo_status_t _cairo_cogl_path_fixed_rectangle(cairo_path_fixed_t * path, cairo_fixed_t x, cairo_fixed_t y,
    cairo_fixed_t width, cairo_fixed_t height);
cairo_int_status_t _cairo_cogl_surface_fill_rectangle(void * abstract_surface, cairo_operator_t op,
    const cairo_pattern_t    * source, double x, double y, double width, double height, cairo_matrix_t * ctm, const cairo_clip_t * clip);
