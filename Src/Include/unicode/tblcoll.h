...
public:
	 ...
	 * @internal
	 */
	void internalGetContractionsAndExpansions(UnicodeSet * contractions, UnicodeSet * expansions,
...
private:
	...
	void setAttributeDefault(int32_t attribute) { explicitlySetAttributes &= ~((uint32_t)1 << attribute); }
	void setAttributeExplicitly(int32_t attribute) { explicitlySetAttributes |= (uint32_t)1 << attribute; }

	bool attributeHasBeenSetExplicitly(int32_t attribute) const {
		// assert(0 <= attribute < ATTR_LIMIT);
		return (bool)((explicitlySetAttributes & ((uint32_t)1 << attribute)) != 0);
	}
	/**
