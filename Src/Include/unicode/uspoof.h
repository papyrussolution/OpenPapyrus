 ...
 * bool result = u_strcmp(skel1, skel2) == 0;
 * // areConfusable: 1 (status: U_ZERO_ERROR)
 * printf("areConfusable: %d (status: %s)\n", result, u_errorName(status));
 * uspoof_close(sc);
 * free(skel1);
...
typedef enum USpoofChecks {
	...
	/**
	 * When performing the two-string {@link uspoof_areConfusable} test, this flag in the return value indicates
	 * that the two strings are visually confusable and that they are <b>not</b> from the same script, according to
	 *UTS
	 * 39 section 4.
	 *
	 * @see uspoof_areConfusable
	 ...
	 * @stable ICU 58
	 */
	USPOOF_CONFUSABLE               =   USPOOF_SINGLE_SCRIPT_CONFUSABLE | USPOOF_MIXED_SCRIPT_CONFUSABLE |
 ...
 * @stable ICU 4.2
 */
U_CAPI int32_t U_EXPORT2 uspoof_serialize(USpoofChecker * sc,
