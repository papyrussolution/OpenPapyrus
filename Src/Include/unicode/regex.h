...
private:
	 ...
	 * `UnicodeString(bool isTerminated, const char16_t *text, int32_t textLength);`
	 * or in a UText, using
...
public:
	 ...
	 * @stable ICU 2.4
	 */
	static bool U_EXPORT2 matches(const UnicodeString   &regex,
	 ...
	 * @stable ICU 4.6
	 */
	static bool U_EXPORT2 matches(UText * regex,
	    UText           * input,
	    UParseError     &pe,
	    UErrorCode      &status);
...
public:
	 ...
	 *    @stable ICU 2.4
	 */
	virtual bool matches(UErrorCode & status);
	/**
	 ...
	 *    @stable ICU 2.8
	 */
	virtual bool matches(int64_t startIndex, UErrorCode & status);
	 ...
	 *    @stable ICU 2.4
	 */
	virtual bool lookingAt(UErrorCode & status);
	 ...
	 *    @stable ICU 2.8
	 */
	virtual bool lookingAt(int64_t startIndex, UErrorCode & status);
	 ...
	 * @stable ICU 55
	 */
	virtual bool find(UErrorCode & status);
	/**
	 ...
	 *   @stable ICU 2.4
	 */
	virtual bool find(int64_t start, UErrorCode & status);
...
public:
	 ...
	 * @stable ICU 4.0
	 */
	virtual bool hasTransparentBounds() const;
	 ...
	 * @stable ICU 4.0
	 **/
	virtual RegexMatcher &useTransparentBounds(bool b);
	 ...
	 * @stable ICU 4.0
	 */
	virtual bool hasAnchoringBounds() const;
	 ...
	 * @stable ICU 4.0
	 */
	virtual RegexMatcher &useAnchoringBounds(bool b);
...
private:
	...
	//
	void                 MatchAt(int64_t startIdx, bool toEnd, UErrorCode & status);
	inline void          backTrack(int64_t &inputIdx, int32_t &patIdx);
	bool                isWordBoundary(int64_t pos);     // perform Perl-like  \b test
	bool                isUWordBoundary(int64_t pos, UErrorCode & status);// perform RBBI based \b test
	...
	inline bool         findProgressInterrupt(int64_t matchIndex, UErrorCode & status);
	int64_t      appendGroup(int32_t groupNum, UText * dest, UErrorCode & status) const;
	bool                findUsingChunk(UErrorCode & status);
	void                 MatchChunkAt(int32_t startIdx, bool toEnd, UErrorCode & status);
	bool                isChunkWordBoundary(int32_t pos);
	const RegexPattern  * fPattern;
	RegexPattern        * fPatternOwned;// Non-NULL if this matcher owns the pattern, and should delete it when through.
	...
	int64_t fLookLimit;                //   and other boundary tests.  See useTransparentBounds
	int64_t fActiveStart;              // Currently active bounds for matching.
	int64_t fActiveLimit;              //   Usually is the same as region, but is changed to fLookStart/Limit when entering look around regions.
	bool fTransparentBounds;             // True if using transparent bounds.
	bool fAnchoringBounds;            // True if using anchoring bounds.
	bool fMatch;                      // True if the last attempted match was successful.
	...
	int64_t fMatchEnd;                 // First position after the end of the most recent match
	                                   //   Zero if no previous match, even when a region is active.
	int64_t fLastMatchEnd;             // First position after the end of the previous match, or -1 if there was no previous match.
	int64_t fAppendPosition;           // First position after the end of the previous
	                                   ...
	                                   //   "append position"
	bool fHitEnd;                     // True if the last match touched the end of input.
	bool fRequireEnd;                 // True if the last match required end-of-input (matched $ or Z)
	UVector64           * fStack;
	...
	int32_t fTimeLimit;                // Max time (in arbitrary steps) to let the match engine run.  Zero for unlimited.
	...
	int32_t fTickCounter;              // Low bits counter for time.  Counts down StateSaves.
	                                   ...
	                                   //   StateSave function.
	int32_t fStackLimit;               // Maximum memory size to use for the backtrack stack, in bytes.  Zero for unlimited.
	URegexMatchCallback * fCallbackFn;  // Pointer to match progress callback funct. NULL if there is no callback.
	const void         * fCallbackContext;// User Context ptr for callback function.
	URegexFindProgressCallback  * fFindProgressCallbackFn;// Pointer to match progress callback funct. NULL if there is no callback.
	const void         * fFindProgressCallbackContext; // User Context ptr for callback function.
	bool fInputUniStrMaybeMutable; // Set when fInputText wraps a UnicodeString that may be mutable - compatibility.
	bool fTraceDebug;                 // Set true for debug tracing of match engine.
	UErrorCode fDeferredStatus; // Save error state that cannot be immediately reported, or that permanently disables this matcher.
