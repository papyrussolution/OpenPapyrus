...
public:
     ...
     * @stable ICU 4.8
     */
    bool hasNumberedArguments() const {
     ...
     * @stable ICU 4.8
     */
    bool partSubstringMatches(const Part &part, const UnicodeString & s) const {
    ...
    public:
         ...
         * @stable ICU 4.8
         */
        static bool hasNumericValue(UMessagePatternPartType type) {
...
private:
     ...
     * @param errorCode
     */
    void parseDouble(int32_t start, int32_t limit, bool allowInfinity,
                     UParseError *parseError, UErrorCode & errorCode);
    ...
    static bool isArgTypeChar(UChar32 c);

    bool isChoice(int32_t index);

    bool isPlural(int32_t index);

    bool isSelect(int32_t index);

    bool isOrdinal(int32_t index);

    /**
     * @return true if we are inside a MessageFormat (sub-)pattern,
     *         as opposed to inside a top-level choice/plural/select pattern.
     */
    bool inMessageFormatPattern(int32_t nestingLevel);

    /**
     * @return true if we are in a MessageFormat sub-pattern
     *         of a top-level ChoiceFormat pattern.
     */
    bool inTopLevelChoiceMessage(int32_t nestingLevel, UMessagePatternArgType parentType);
    ...
    bool init(UErrorCode & errorCode);
    bool copyStorage(const MessagePattern &other, UErrorCode & errorCode);
