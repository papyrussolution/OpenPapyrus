 ...
 * <pre>
 * \code
 *    char16_t myString[20];
 *    double myNumber = 7.0;
 *    UErrorCode status = U_ZERO_ERROR;
 *    UNumberFormat* nf = unum_open(UNUM_DEFAULT, NULL, -1, NULL, NULL, &status);
 *    unum_formatDouble(nf, myNumber, myString, 20, NULL, &status);
 *    printf(" Example 1: %s\n", austrdup(myString)); //austrdup( a function used to convert char16_t* to char *)
 * \endcode
 * </pre>
 ...
 * @stable ICU 2.0
 */
U_CAPI UNumberFormat* U_EXPORT2 unum_open(UNumberFormatStyle style, const char16_t * pattern, int32_t patternLength, const char * locale, UParseError * parseErr, UErrorCode * status);
/**
 ...
 * @stable ICU 2.0
 */
U_CAPI UNumberFormat* U_EXPORT2 unum_clone(const UNumberFormat * fmt,
    UErrorCode * status);
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_format(const UNumberFormat*    fmt, int32_t number, char16_t* result, int32_t resultLength,
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_formatInt64(const UNumberFormat * fmt, int64_t number,
    char16_t * result, int32_t resultLength, UFieldPosition * pos, UErrorCode*     status);
/**
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_formatDouble(const UNumberFormat*  fmt, double number,
    char16_t * result, int32_t resultLength, UFieldPosition  * pos/* 0 if ignore */, UErrorCode*     status);
/**
 ...
 * @stable ICU 59
 */
U_CAPI int32_t U_EXPORT2 unum_formatDoubleForFields(const UNumberFormat* format, double number, char16_t* result,
 ...
 * @stable ICU 4.4
 */
U_CAPI int32_t U_EXPORT2 unum_formatDecimal(const UNumberFormat*  fmt, const char *    number, int32_t length,
    char16_t * result, int32_t resultLength, UFieldPosition  * pos/* 0 if ignore */, UErrorCode*     status);
/**
 ...
 * @stable ICU 52
 */
U_CAPI int32_t U_EXPORT2 unum_formatUFormattable(const UNumberFormat* fmt,
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_parse(const UNumberFormat*  fmt,
    const char16_t*          text,
    int32_t textLength,
...
/**
 * Parse a char16_t string into a UFormattable.
 * Example code:
 * \snippet test/cintltst/cnumtst.c unum_parseToUFormattable
 * Note: parsing is not supported for styles UNUM_DECIMAL_COMPACT_SHORT
 * and UNUM_DECIMAL_COMPACT_LONG.
 * @param fmt the formatter to use
 * @param result the UFormattable to hold the result. If NULL, a new UFormattable will be allocated (which the caller
 *must close with ufmt_close).
 * @param text the text to parse
 * @param textLength the length of text, or -1 if null-terminated
 * @param parsePos a pointer to an offset index into text at which to
 * begin parsing. On output, *parsePos will point after the last
 ...
 * begins at offset 0.
 * @param status a pointer to an input-output UErrorCode
 * @return the UFormattable.  Will be ==result unless NULL was passed in for result, in which case it will be the newly
 *opened UFormattable.
 * @see ufmt_getType
 * @see ufmt_close
 * @stable ICU 52
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 unum_applyPattern(UNumberFormat  * format,
    bool localized,
 ...
 * @stable ICU 2.0
 */
U_CAPI const char * U_EXPORT2 unum_getAvailable(int32_t localeIndex);
...
typedef enum UNumberFormatAttribute {
	...
	UNUM_LENIENT_PARSE,
#if UCONFIG_HAVE_PARSEALLINPUT
	/** Consume all input. (may use fastpath). Set to UNUM_YES (require fastpath), UNUM_NO (skip fastpath), or
	   UNUM_MAYBE (heuristic).
	...
	/** If 1, specifies that if setting the "max integer digits" attribute would truncate a value, set an error
	 ...
	 *There is no effect on parsing.
	 * Default: 0 (not set)
	 * @stable ICU 50
	 */
	UNUM_FORMAT_FAIL_IF_MORE_THAN_MAX_DIGITS = 0x1000,
	/**
	 * if this attribute is set to 1, specifies that, if the pattern doesn't contain an exponent, the exponent will
	 *not be parsed. If the pattern does contain an exponent, this attribute has no effect.
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_getAttribute(const UNumberFormat*          fmt,
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 unum_setAttribute(UNumberFormat*          fmt,
    UNumberFormatAttribute attr,
    int32_t newValue);
 ...
 * @stable ICU 2.0
 */
U_CAPI double U_EXPORT2 unum_getDoubleAttribute(const UNumberFormat*          fmt,
    UNumberFormatAttribute attr);
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 unum_setDoubleAttribute(UNumberFormat*          fmt,
    UNumberFormatAttribute attr,
    double newValue);
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_getTextAttribute(const UNumberFormat*                    fmt,
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 unum_setTextAttribute(UNumberFormat * fmt, UNumberFormatTextAttribute tag, const char16_t * newValue, int32_t newValueLength, UErrorCode * status);
/**
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_toPattern(const UNumberFormat*          fmt,
    bool isPatternLocalized,
    char16_t*                  result,
    int32_t resultLength,
    UErrorCode*             status);
 ...
 * @stable ICU 2.0
 */
U_CAPI int32_t U_EXPORT2 unum_getSymbol(const UNumberFormat * fmt,
 ...
 * @stable ICU 2.0
 */
U_CAPI void U_EXPORT2 unum_setSymbol(UNumberFormat * fmt,
    UNumberFormatSymbol symbol,
 ...
 * @stable ICU 53
 */
U_CAPI void U_EXPORT2 unum_setContext(UNumberFormat* fmt, UDisplayContext value, UErrorCode* status);
 ...
 * @stable ICU 53
 */
U_CAPI UDisplayContext U_EXPORT2 unum_getContext(const UNumberFormat * fmt, UDisplayContextType type, UErrorCode* status);
