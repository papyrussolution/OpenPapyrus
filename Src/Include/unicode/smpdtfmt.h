 ...
 *         name, four for the full (wide) name, or five for the narrow name (currently the data only provides
 *abbreviated names,
 ...
 *         <td>02</td>
 *         <td rowspan="3">Quarter - Use one or two for the numerical quarter, three for the abbreviation, or four for
 *the
 ...
 *         <td style="text-align: center">1..2</td>
 *         <td>02</td>
 *         <td rowspan="3"><b>Stand-Alone</b> Quarter - Use one or two for the numerical quarter, three for the
 *abbreviation,
 ...
 *         <td style="text-align: center">1..2</td>
 *         <td>09</td>
 *         <td rowspan="4"><b>Stand-Alone</b> Month - Use one or two for the numerical month, three for the
 *abbreviation,
 ...
 *         starting day of the week, using one or two letters. For this example, Monday is the first day of the
 *week.</td>
 *     </tr>
 *     <tr>
 *         <td style="text-align: center">3</td>
...
public:
	 ...
	 * If the field is not numeric, then override has no effect (like "MMM" will use abbreviation, not numerical
	 *field)
	 * Per field NumberFormat can also be cleared in {@see DateFormat::setNumberFormat(const NumberFormat&
	 *newNumberFormat)}
	 *
	 ...
	 * @internal ICU 4.0
	 */
	bool isFieldUnitIgnored(UCalendarDateFields field) const;
	 ...
	 * @internal ICU 4.0
	 */
	static bool isFieldUnitIgnored(const UnicodeString & pattern,
...
private:
	 ...
	 */
	static bool isNumeric(char16_t formatChar, int32_t count);

	/**
	 * Returns true if the patternOffset is at the start of a numeric field.
	 */
	static bool isAtNumericField(const UnicodeString &pattern, int32_t patternOffset);

	/**
	 * Returns true if the patternOffset is right after a non-numeric field.
	 */
	static bool isAfterNonNumericField(const UnicodeString &pattern, int32_t patternOffset);
	 ...
	 */
	static bool matchLiterals(const UnicodeString &pattern, int32_t &patternOffset,
	    const UnicodeString &text, int32_t &textOffset,
	    bool whitespaceLenient, bool partialMatchLenient, bool oldLeniency);
	 ...
	 */
	static bool isSyntaxChar(char16_t ch);
	/**
	...
	bool fHasMinute;
	bool fHasSecond;
	bool fHasHanYearChar;            // pattern contains the Han year character \u5E74
