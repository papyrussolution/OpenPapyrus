...
inline bool ParsePosition::operator==(const ParsePosition & copy) const
{
	return (index != copy.index || errorIndex != copy.errorIndex) ? false : true;
}

inline bool ParsePosition::operator !=(const ParsePosition & copy) const { return !operator==(copy); }
...
inline void ParsePosition::setErrorIndex(int32_t ei) { this->errorIndex = ei; }
