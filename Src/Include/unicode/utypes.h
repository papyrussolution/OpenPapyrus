...
// 02/27/97    aliu        Added typedefs for UClassID, int8, int16, int32, uint8, uint16, and uint32.
// 04/01/97    aliu        Added XP_CPLUSPLUS and modified to work under C as well as C++. Modified to use memcpy() for uprv_arrayCopy() fns.
// 04/14/97    aliu        Added TPlatformUtilities.
// 05/07/97    aliu        Added import/export specifiers (replacing the old broken EXT_CLASS).  Added version number for our code.  Cleaned up header.
...
#ifndef U_HIDE_DEPRECATED_API
	U_REGEX_OCTAL_TOO_BIG,            /**< Octal character constants must be <= 0377. @deprecated ICU 54. This error
	                                     cannot occur. */
#endif  /* U_HIDE_DEPRECATED_API */
	U_REGEX_MISSING_CLOSE_BRACKET = U_REGEX_SET_CONTAINS_STRING+2, /**< Missing closing bracket on a bracket
	                                                                  expression. */
 ...
 * @stable ICU 2.0
 */
static inline bool U_FAILURE(UErrorCode code) { return (bool)(code>U_ZERO_ERROR); }
