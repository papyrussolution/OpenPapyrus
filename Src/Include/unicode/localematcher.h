...
private:
		Result(const Locale * desired, const Locale * supported, int32_t desIndex, int32_t suppIndex, bool owned) :
			desiredLocale(desired), supportedLocale(supported), desiredIndex(desIndex), supportedIndex(suppIndex), desiredIsOwned(owned) 
		{
		}
...
public:
		 ...
		 * @stable ICU 65
		 */
		template <typename Iter> Builder &setSupportedLocales(Iter begin, Iter end) 
		{
		 ...
		 * @stable ICU 65
		 */
		template <typename Iter, typename Conv> Builder &setSupportedLocalesViaConverter(Iter begin, Iter end, Conv converter) 
		{
			if(U_FAILURE(errorCode_)) {
				return *this;
			}
...
private:
	 ...
	 * @stable ICU 68
	 */
	bool isMatch(const Locale &desired, const Locale &supported, UErrorCode & errorCode) const;
