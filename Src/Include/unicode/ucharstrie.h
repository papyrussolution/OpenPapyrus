...
private:
		...
		bool skipValue_; // Skip intermediate value which was already delivered.
		UnicodeString str_;
		int32_t maxLength_;
		int32_t value_;
...
private:
	 ...
	 */
	UCharsTrie(char16_t * adoptUChars, const char16_t * trieUChars) : ownedArray_(adoptUChars), uchars_(trieUChars), pos_(uchars_), remainingMatchLength_(-1) 
	{
	}
	...
	static inline UStringTrieResult valueResult(int32_t node) { return (UStringTrieResult)(USTRINGTRIE_INTERMEDIATE_VALUE-(node>>15)); }
	...
	// from a branch.
	static const char16_t * findUniqueValueFromBranch(const char16_t * pos, int32_t length, bool haveUniqueValue, int32_t &uniqueValue);
	// Recursively finds a unique value (or whether there is not a unique one)
	// starting from a position on a node lead unit.
	static bool findUniqueValue(const char16_t * pos, bool haveUniqueValue, int32_t &uniqueValue);
	...
	// For a branch sub-node with at most this many entries, we drop down to a linear search.
	...
	static const int32_t kValueIsFinal = 0x8000; // A final-value node has bit 15 set.
	static const int32_t kMaxOneUnitValue = 0x3fff; // Compact value: After testing and masking off bit 15, use the following thresholds.
	...
	const char16_t * uchars_; // Fixed value referencing the UCharsTrie words.
	// Iterator variables.
