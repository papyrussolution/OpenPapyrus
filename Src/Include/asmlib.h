...
static inline const char * A_strstr(const char * haystack, const char * needle) { return A_strstr(const_cast<char *>(haystack), needle); } // Overload A_strstr with const char * version
...
//
// Descr: Signed 32 bit integer division
//
class div_i32 {                                                
public:
...
//
// Descr: Unsigned 32 bit integer division
//
class div_u32 {                                                
public:
	div_u32() 
	...
	div_u32(uint32_t d) { setdivisor(d); }
	// Set divisor
	void setdivisor(uint32_t d) { setdivisoru32(buffer, d); }
protected:
	uint32_t buffer[2];                                        // Internal memory
	friend uint32_t operator / (uint32_t x, div_u32 const & D);
};
//
// Descr: Overloaded operator '/'
//
static inline uint32_t operator / (uint32_t x, div_u32 const & D) {  return dividefixedu32(D.buffer, x); }
//
// Descr: Overloaded operator '/='
//
static inline uint32_t operator /= (uint32_t &x, div_u32 const &D) { return x = x / D; }
