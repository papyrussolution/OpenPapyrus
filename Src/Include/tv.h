...
public:
	struct Param {
		...
		uint   Quant;          // Квант временной решетки (секунд)
			// Для vHourDay - Квант одного дня (секунд)
		uint   PixQuant;       // Квант временной решетки (в пикселах)
			// Для vHourDay - Квант одного дня (в пикселах)
		...
		long   ViewType;       // @v7.5.0 Param::vXXX Тип отображения //
		DateRange ScrPeriod;   // @v7.5.0 Период, который должен отображаться в окне (по оси X).
		long   DaysCount;      // @v7.6.10 (Рекомендованное) количество дней, отображаемых по оси X в режиме vHourDay.
			// default=7
		long   SingleRowIdx;   // @v7.6.12 Просмотр в режиме vHourDay единственной строки P_Data с индексом SingleRowIdx. -1 - undefined
		long   Flags;
		SString RegSaveParam;  // Имя параметра, по которому должны быть сохранены установки
			// таблицы в системном реестре
	};
	//
