...
// These were added in libpng-1.0.2 
	...
	png_bytep prev_row; // buffer to save previous (unfiltered) row. While reading this is a pointer into
		// big_prev_row; while writing it is separately allocated if needed.
	png_bytep row_buf;  // buffer to save current (unfiltered) row. While reading, this is a pointer into
		// big_row_buf; while writing it is separately allocated.
...
// Added at libpng-1.5.10
	...
	uint8 maximum_pixel_depth; // pixel depth used for the row buffers
	uint8 transformed_pixel_depth; // pixel depth after read/write transforms 
...
/* For the Borland special 64K segment handler */
	...
	uint32 options; // On/off state (up to 16 options) // @libpng-1629 uint8-->uint32
#endif
	...
	uint32 mng_features_permitted; // Changed from uint8 to uint32 at version 1.2.0 
#endif
#ifdef PNG_MNG_FEATURES_SUPPORTED
	uint8 filter_type; // New member added in libpng-1.0.9, ifdef'ed out in 1.0.12, enabled in 1.2.0 
#endif
	//
	...
	// Temporary storage for unknown chunk that the library doesn't recognize, used while reading the chunk.
	png_unknown_chunk unknown_chunk;
#endif
	size_t old_big_row_buf_size; // New member added in libpng-1.2.26 
	...
	uint32 io_state; // New member added in libpng-1.4.0 
#endif
	png_bytep big_prev_row; // New member added in libpng-1.5.6 
