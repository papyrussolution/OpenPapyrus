 ...
 */
	...
	#else
		// Not building a DLL, or the DLL doesn't require specific export definitions. 
		#define PNG_IMPEXP
	#endif
#endif
...
#define PNG_COLOR_DIST(c1, c2) (abs((int)((c1).R) - (int)((c2).R)) + abs((int)((c1).G) - (int)((c2).G)) + abs((int)((c1).B) - (int)((c2).B)))
...
#define PNG_ROWBYTES(pixel_bits, width)	((pixel_bits) >= 8 ? ((size_t)(width) * (((size_t)(pixel_bits)) >> 3)) : (( ((size_t)(width) * ((size_t)(pixel_bits))) + 7) >> 3))
 ...
 */
int png_user_version_check(png_structrp png_ptr, const char * user_png_ver);

/* Internal base allocator - no messages, NULL on failure to allocate.  This
 * does, however, call the application provided allocator and that could call
 * png_error (although that would be a bug in the application implementation.)
 */
void * png_malloc_base(png_const_structrp png_ptr, png_alloc_size_t size)/*PNG_ALLOCATED*/;

#if defined(PNG_TEXT_SUPPORTED) || defined(PNG_sPLT_SUPPORTED) || defined(PNG_STORE_UNKNOWN_CHUNKS_SUPPORTED)
// Internal array allocator, outputs no error or warning messages on failure, just returns NULL.
void * png_malloc_array(png_const_structrp png_ptr, int nelements, size_t element_size)/*PNG_ALLOCATED*/;

/* The same but an existing array is extended by add_elements.  This function
 * also memsets the new elements to 0 and copies the old elements.  The old
 * array is not freed or altered.
 */
void * png_realloc_array(png_const_structrp png_ptr, const void * array, int old_elements, int add_elements, size_t element_size)/*PNG_ALLOCATED*/;
 ...
 * call png_error.
 */
png_structp png_create_png_struct(const char * user_png_ver, void * error_ptr, png_error_ptr error_fn, png_error_ptr warn_fn, void * mem_ptr, png_malloc_ptr malloc_fn, png_free_ptr free_fn)/*PNG_ALLOCATED*/;
/* Free memory from internal libpng struct */
void png_destroy_png_struct(png_structrp png_ptr);
/* Free an allocated jmp_buf (always succeeds) */
void png_free_jmpbuf(png_structrp png_ptr);
/* Function to allocate memory for zlib.  PNGAPI is disallowed. */
voidpf png_zalloc(void * png_ptr, uInt items, uInt size)/*PNG_ALLOCATED*/;
...
uint32 png_read_chunk_header(png_structrp png_ptr);
/* Read data from whatever input you are using into the "data" buffer */
void png_read_data(png_structrp png_ptr, png_bytep data, size_t length);
/* Read bytes into buf, and update png_ptr->crc */
void png_crc_read(png_structrp png_ptr, png_bytep buf, uint32 length);
/* Read "skip" bytes, read the file crc, and (optionally) verify png_ptr->crc */
int png_crc_finish(png_structrp png_ptr, uint32 skip);
 ...
 * information.
 */
void png_write_IHDR(png_structrp png_ptr, uint32 width, uint32 height, int bit_depth, int color_type, int compression_method, int filter_method, int interlace_method);
void png_write_PLTE(png_structrp png_ptr, png_const_colorp palette, uint32 num_pal);
...
void png_write_IEND(png_structrp png_ptr);
	...
	void png_write_iCCP(png_structrp png_ptr, const char * name, png_const_bytep profile);
	// The profile must have been previously validated for correctness, the
	// length comes from the first four bytes.  Only the base, deflate, compression is supported.
#endif
...
void png_write_tEXt(png_structrp png_ptr, const char * key, const char * text, size_t text_len);
#endif
#ifdef PNG_WRITE_zTXt_SUPPORTED
void png_write_zTXt(png_structrp png_ptr, const char * key, const char * text, int compression);
#endif
#ifdef PNG_WRITE_iTXt_SUPPORTED
void png_write_iTXt(png_structrp png_ptr, int compression, const char * key, const char * lang, const char * lang_key, const char * text);
#endif
...
void png_write_sCAL_s(png_structrp png_ptr, int unit, const char * width, const char * height);
#endif
...
void png_combine_row(png_const_structrp png_ptr, png_bytep row, int display);
	 ...
	 */
	void png_do_read_interlace(png_row_infop row_info, png_bytep row, int pass, uint32 transformations);
#endif
...
void png_handle_IHDR(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
void png_handle_PLTE(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
void png_handle_IEND(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#ifdef PNG_READ_bKGD_SUPPORTED
void png_handle_bKGD(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_cHRM_SUPPORTED
void png_handle_cHRM(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_gAMA_SUPPORTED
void png_handle_gAMA(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_hIST_SUPPORTED
void png_handle_hIST(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_iCCP_SUPPORTED
void png_handle_iCCP(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif /* READ_iCCP */
#ifdef PNG_READ_iTXt_SUPPORTED
void png_handle_iTXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_oFFs_SUPPORTED
void png_handle_oFFs(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_pCAL_SUPPORTED
void png_handle_pCAL(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_pHYs_SUPPORTED
void png_handle_pHYs(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_sBIT_SUPPORTED
void png_handle_sBIT(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_sCAL_SUPPORTED
void png_handle_sCAL(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_sPLT_SUPPORTED
void png_handle_sPLT(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif /* READ_sPLT */
#ifdef PNG_READ_sRGB_SUPPORTED
void png_handle_sRGB(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_tEXt_SUPPORTED
void png_handle_tEXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_tIME_SUPPORTED
void png_handle_tIME(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_tRNS_SUPPORTED
void png_handle_tRNS(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
#ifdef PNG_READ_zTXt_SUPPORTED
void png_handle_zTXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
#endif
void png_check_chunk_name(png_structrp png_ptr, uint32 chunk_name);
void png_handle_unknown(png_structrp png_ptr, png_inforp info_ptr, uint32 length, int keep);
...
void   png_push_handle_unknown(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
...
void   FASTCALL png_push_have_row(png_structrp png_ptr, png_bytep row);
...
void   FASTCALL png_read_push_finish_row(png_structrp png_ptr);
#ifdef PNG_READ_tEXt_SUPPORTED
void   png_push_handle_tEXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
void   png_push_read_tEXt(png_structrp png_ptr, png_inforp info_ptr);
#endif
#ifdef PNG_READ_zTXt_SUPPORTED
void   png_push_handle_zTXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
void   png_push_read_zTXt(png_structrp png_ptr, png_inforp info_ptr);
#endif
#ifdef PNG_READ_iTXt_SUPPORTED
void   png_push_handle_iTXt(png_structrp png_ptr, png_inforp info_ptr, uint32 length);
 ...
 */
	...
	PNG_INTERNAL_FUNCTION(int, png_colorspace_set_ICC, (png_const_structrp png_ptr, png_colorspacerp colorspace, 
		const char * name, uint32 profile_length, png_const_bytep profile, int color_type), PNG_EMPTY);
	...
	PNG_INTERNAL_FUNCTION(int, png_icc_check_header, (png_const_structrp png_ptr, png_colorspacerp colorspace, const char * name, uint32 profile_length,
		png_const_bytep profile /* first 132 bytes only */, int color_type), PNG_EMPTY);
...
/* Added at libpng version 1.4.0 */
PNG_INTERNAL_FUNCTION(void, png_check_IHDR, (png_const_structrp png_ptr, uint32 width, uint32 height, int bit_depth,
	int color_type, int interlace_type, int compression_type, int filter_type), PNG_EMPTY);
/* Added at libpng version 1.5.10 */
	...
	PNG_INTERNAL_FUNCTION(void, png_fixed_error, (png_const_structrp png_ptr, const char * name), PNG_NORETURN);
#endif
...
//
	...
	// Returns the pointer to the start of the formatted string.  This utility only does unsigned values.
	// 
	PNG_INTERNAL_FUNCTION(char *, png_format_number, (const char * start, char * end, int format, png_alloc_size_t number), PNG_EMPTY);
	// Convenience macro that takes an array:
	#define PNG_FORMAT_NUMBER(buffer, format, number) png_format_number(buffer, buffer + (sizeof buffer), format, number)
	#define PNG_NUMBER_BUFFER_SIZE 24 // Suggested size for a number buffer (enough for 64 bits and a sign!) 
	//
 ...
 */
	...
	#ifdef PNG_FLOATING_POINT_SUPPORTED
		PNG_INTERNAL_FUNCTION(void, png_ascii_from_fp, (png_const_structrp png_ptr, char * ascii, size_t size, double fp, uint precision), PNG_EMPTY);
	#endif /* FLOATING_POINT */
	#ifdef PNG_FIXED_POINT_SUPPORTED
		PNG_INTERNAL_FUNCTION(void, png_ascii_from_fixed, (png_const_structrp png_ptr, char * ascii, size_t size, png_fixed_point fp), PNG_EMPTY);
	#endif /* FIXED_POINT */
#endif /* sCAL */
...
typedef struct png_control {
	...
	uint for_write       : 1; /* Otherwise it is a read structure */
	uint owned_file      : 1; /* We own the file in io_ptr */
} png_control;
// 
...
// 
// Utility to log an error; this also cleans up the png_image; the function always returns 0 (false).
// 
int FASTCALL png_image_error(png_imagep image, const char * error_message);
...
// time using the macro PNG_FILTER_OPTIMIZATIONS.  If the macro is not defined the generic code is used.
// 
#ifdef PNG_FILTER_OPTIMIZATIONS
	// Just declare the optimization that will be used
	PNG_INTERNAL_FUNCTION(void, PNG_FILTER_OPTIMIZATIONS, (png_structp png_ptr, uint bpp), PNG_EMPTY);
#else
	// List *all* the possible optimizations here - this branch is required if
	// the builder of libpng passes the definition of PNG_FILTER_OPTIMIZATIONS in
	// CFLAGS in place of CPPFLAGS *and* uses symbol prefixing.
	#if PNG_ARM_NEON_OPT > 0
		PNG_INTERNAL_FUNCTION(void, png_init_filter_functions_neon, (png_structp png_ptr, uint bpp), PNG_EMPTY);
	#endif
	#if PNG_MIPS_MSA_OPT > 0
		PNG_INTERNAL_FUNCTION(void, png_init_filter_functions_msa, (png_structp png_ptr, uint bpp), PNG_EMPTY);
	#endif
	#if PNG_INTEL_SSE_IMPLEMENTATION > 0
