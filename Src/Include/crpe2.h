...
typedef struct UXFXlsOptions {
	WORD structSize;
	BOOL bColumnHeadings;   //TRUE -- has column headings, which come from. "Page Header" and "Report Header" ares. FALSE -- no clolumn headings.
	                        //The default value is FALSE.
	BOOL bUseConstColWidth; //TRUE -- use constant column width. FALSE -- set column width based on an area. The default value is FALSE.
	double fConstColWidth;  //Column width, when bUseConstColWidth is TRUE. The default value is 9.
	BOOL bTabularFormat;    //TRUE -- tabular format (flatten an area into a row). FALSE -- non-tabular format. The default value is FALSE.
	WORD baseAreaType;      //One of the 7 Section types defined in "Crpe.h". The default value is PE_SECT_DETAIL.
