...
//#ifdef __has_attribute
	//#if __has_attribute(always_inline)
		//#define LIBDIVIDE_INLINE_Removed __attribute__((always_inline)) inline
	//#endif
//#endif
//#ifndef FORCEINLINE
	//#define LIBDIVIDE_INLINE_Removed inline
//#endif
#if defined(__AVR__)
	...
	#define LIBDIVIDE_ERROR(msg)                                                                     \
		do {                                                                                         \
			slfprintf_stderr("libdivide.h:%d: %s(): Error: %s\n", __LINE__, LIBDIVIDE_FUNCTION, msg); \
			abort();                                                                                 \
		} while(0)
#endif
...
static FORCEINLINE struct libdivide_s16_t libdivide_s16_gen(int16 d);
static FORCEINLINE struct libdivide_u16_t libdivide_u16_gen(uint16 d);
static FORCEINLINE struct libdivide_s32_t libdivide_s32_gen(int32_t d);
static FORCEINLINE struct libdivide_u32_t libdivide_u32_gen(uint32 d);
static FORCEINLINE struct libdivide_s64_t libdivide_s64_gen(int64 d);
static FORCEINLINE struct libdivide_u64_t libdivide_u64_gen(uint64 d);

static FORCEINLINE struct libdivide_s16_branchfree_t libdivide_s16_branchfree_gen(int16 d);
static FORCEINLINE struct libdivide_u16_branchfree_t libdivide_u16_branchfree_gen(uint16 d);
static FORCEINLINE struct libdivide_s32_branchfree_t libdivide_s32_branchfree_gen(int32_t d);
static FORCEINLINE struct libdivide_u32_branchfree_t libdivide_u32_branchfree_gen(uint32 d);
static FORCEINLINE struct libdivide_s64_branchfree_t libdivide_s64_branchfree_gen(int64 d);
static FORCEINLINE struct libdivide_u64_branchfree_t libdivide_u64_branchfree_gen(uint64 d);

static FORCEINLINE int16 libdivide_s16_do_raw(int16 numer, int16 magic, uint8 more);
static FORCEINLINE int16 libdivide_s16_do(int16 numer, const struct libdivide_s16_t * denom);
static FORCEINLINE uint16 libdivide_u16_do_raw(uint16 numer, uint16 magic, uint8 more);
static FORCEINLINE uint16 libdivide_u16_do(uint16 numer, const struct libdivide_u16_t * denom);
static FORCEINLINE int32_t libdivide_s32_do(int32_t numer, const struct libdivide_s32_t * denom);
static FORCEINLINE uint32 libdivide_u32_do(uint32 numer, const struct libdivide_u32_t * denom);
static FORCEINLINE int64 libdivide_s64_do(int64 numer, const struct libdivide_s64_t * denom);
static FORCEINLINE uint64 libdivide_u64_do(uint64 numer, const struct libdivide_u64_t * denom);

static FORCEINLINE int16 libdivide_s16_branchfree_do(int16 numer, const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE uint16 libdivide_u16_branchfree_do(uint16 numer, const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE int32_t libdivide_s32_branchfree_do(int32_t numer, const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE uint32 libdivide_u32_branchfree_do(uint32 numer, const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE int64 libdivide_s64_branchfree_do(int64 numer, const struct libdivide_s64_branchfree_t * denom);
static FORCEINLINE uint64 libdivide_u64_branchfree_do(uint64 numer, const struct libdivide_u64_branchfree_t * denom);

static FORCEINLINE int16 libdivide_s16_recover(const struct libdivide_s16_t * denom);
static FORCEINLINE uint16 libdivide_u16_recover(const struct libdivide_u16_t * denom);
static FORCEINLINE int32_t libdivide_s32_recover(const struct libdivide_s32_t * denom);
static FORCEINLINE uint32 libdivide_u32_recover(const struct libdivide_u32_t * denom);
static FORCEINLINE int64 libdivide_s64_recover(const struct libdivide_s64_t * denom);
static FORCEINLINE uint64 libdivide_u64_recover(const struct libdivide_u64_t * denom);

static FORCEINLINE int16 libdivide_s16_branchfree_recover(const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE uint16 libdivide_u16_branchfree_recover(const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE int32_t libdivide_s32_branchfree_recover(const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE uint32 libdivide_u32_branchfree_recover(const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE int64 libdivide_s64_branchfree_recover(const struct libdivide_s64_branchfree_t * denom);
static FORCEINLINE uint64 libdivide_u64_branchfree_recover(const struct libdivide_u64_branchfree_t * denom);
//
// Internal Utility Functions
//
static FORCEINLINE uint16 libdivide_mullhi_u16(uint16 x, uint16 y) 
{
	uint32 xl = x, yl = y;
	uint32 rl = xl * yl;
	return (uint16)(rl >> 16);
...
static FORCEINLINE int16 libdivide_mullhi_s16(int16 x, int16 y) 
{
	int32_t xl = x, yl = y;
	int32_t rl = xl * yl;
...
static FORCEINLINE uint32 libdivide_mullhi_u32(uint32 x, uint32 y) 
{
	uint64 xl = x, yl = y;
	uint64 rl = xl * yl;
	return (uint32)(rl >> 32);
...
static FORCEINLINE int32_t libdivide_mullhi_s32(int32_t x, int32_t y) 
{
	int64 xl = x, yl = y;
	int64 rl = xl * yl;
	// needs to be arithmetic shift
...
static FORCEINLINE uint64 libdivide_mullhi_u64(uint64 x, uint64 y) 
{
	...
	uint32 x0 = (uint32)(x & mask);
	uint32 x1 = (uint32)(x >> 32);
	uint32 y0 = (uint32)(y & mask);
	uint32 y1 = (uint32)(y >> 32);
	uint32 x0y0_hi = libdivide_mullhi_u32(x0, y0);
	uint64 x0y1 = x0 * (uint64)y1;
	uint64 x1y0 = x1 * (uint64)y0;
	uint64 x1y1 = x1 * (uint64)y1;
	uint64 temp = x1y0 + x0y0_hi;
	uint64 temp_lo = temp & mask;
	uint64 temp_hi = temp >> 32;
	return x1y1 + temp_hi + ((temp_lo + x0y1) >> 32);
#endif
}

static FORCEINLINE int64 libdivide_mullhi_s64(int64 x, int64 y) 
{
#if defined(LIBDIVIDE_VC) && defined(LIBDIVIDE_X86_64)
	return __mulh(x, y);
#elif defined(HAS_INT128_T)
	...
	uint32 x0y0_hi = libdivide_mullhi_u32(x0, y0);
	int64 t = x1 * (int64)y0 + x0y0_hi;
	int64 w1 = x0 * (int64)y1 + (t & mask);

	return x1 * (int64)y1 + (t >> 32) + (w1 >> 32);
#endif
}
...
static FORCEINLINE int16 libdivide_count_leading_zeros16(uint16 val) 
{
	...
	// On the AVR 8-bit architecture __builtin_clz() works on a int16.
	...
	ulong result;
	if(_BitScanReverse(&result, (ulong)val)) {
		return (int16)(15 - result);
	}
	return 0;
#else
	if(val == 0)  return 16;
...
static FORCEINLINE int32_t libdivide_count_leading_zeros32(uint32 val) 
{
#if defined(__AVR__)
	// Fast way to count leading zeros
...
static FORCEINLINE int32_t libdivide_count_leading_zeros64(uint64 val) 
{
...
static FORCEINLINE uint16 libdivide_32_div_16_to_16(uint16 u1, uint16 u0, uint16 v, uint16 * r) 
{
	uint32 n = ((uint32)u1 << 16) | u0;
	uint16 result = (uint16)(n / v);
	*r = (uint16)(n - result * (uint32)v);
	return result;
}
...
// Returns the quotient directly and the remainder in *r
static FORCEINLINE uint32 libdivide_64_div_32_to_32(uint32 u1, uint32 u0, uint32 v, uint32 * r) 
{
	...
	return result;
#else
	uint64 n = ((uint64)u1 << 32) | u0;
	uint32 result = (uint32)(n / v);
	*r = (uint32)(n - result * (uint64)v);
	return result;
#endif
}
...
static FORCEINLINE uint64 libdivide_128_div_64_to_64(uint64 numhi, uint64 numlo, uint64 den, uint64 * r) 
{
	...
	num1 = (uint32)(numlo >> 32);
	num0 = (uint32)(numlo & 0xFFFFFFFFu);
	den1 = (uint32)(den >> 32);
	den0 = (uint32)(den & 0xFFFFFFFFu);
	...
	return ((uint64)q1 << 32) | q0;
#endif
}

#if !(defined(HAS_INT128_T) && defined(HAS_INT128_DIV))
	// Bitshift a u128 in place, left (signed_shift > 0) or right (signed_shift < 0)
	static FORCEINLINE void libdivide_u128_shift(uint64 * u1, uint64 * u0, int32_t signed_shift) 
	{
		if(signed_shift > 0) {
			uint32 shift = signed_shift;
			*u1 <<= shift;
			*u1 |= *u0 >> (64 - shift);
			*u0 <<= shift;
		}
...
static FORCEINLINE uint64 libdivide_128_div_128_to_64(uint64 u_hi, uint64 u_lo, uint64 v_hi, uint64 v_lo, uint64 * r_hi, uint64 * r_lo) 
{
#if defined(HAS_INT128_T) && defined(HAS_INT128_DIV)
	...
	ufull = (ufull << 64) | u_lo;
	vfull = (vfull << 64) | v_lo;
	uint64 res = (uint64)(ufull / vfull);
	__uint128_t remainder = ufull - (vfull * res);
	*r_lo = (uint64)remainder;
	*r_hi = (uint64)(remainder >> 64);
	return res;
#else
	...
	uint64 v1 = v1t.hi; // i.e. v1 = v1t >> 64
	...
	uint64 rem_ignored;
	uint64 q1 = libdivide_128_div_64_to_64(u1.hi, u1.lo, v1, &rem_ignored);
...
// UINT16
//
static FORCEINLINE struct libdivide_u16_t libdivide_internal_u16_gen(uint16 d, int branchfree)                                                                       
{
	if(d == 0) {
	...
	if((d & (d - 1)) == 0) {
		...
		if(!branchfree && (e < ((uint16)1 << floor_log_2_d))) {
			// This power works
			more = floor_log_2_d;
		}
		else {
...
struct libdivide_u16_t libdivide_u16_gen(uint16 d) { return libdivide_internal_u16_gen(d, 0); }

struct libdivide_u16_branchfree_t libdivide_u16_branchfree_gen(uint16 d) 
{
	if(d == 1) {
		LIBDIVIDE_ERROR("branchfree divider must be != 1");
	}
	struct libdivide_u16_t tmp = libdivide_internal_u16_gen(d, 1);
	struct libdivide_u16_branchfree_t ret = { tmp.magic, (uint8)(tmp.more & LIBDIVIDE_16_SHIFT_MASK)};
	return ret;
}
...
uint16 libdivide_u16_do_raw(uint16 numer, uint16 magic, uint8 more) 
{
	if(!magic) {
		return numer >> more;
	}
	else {
		uint16 q = libdivide_mullhi_u16(magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			uint16 t = ((numer - q) >> 1) + q;
...
uint16 libdivide_u16_do(uint16 numer, const struct libdivide_u16_t * denom) { return libdivide_u16_do_raw(numer, denom->magic, denom->more); }

uint16 libdivide_u16_branchfree_do(uint16 numer, const struct libdivide_u16_branchfree_t * denom) 
{
	uint16 q = libdivide_mullhi_u16(denom->magic, numer);
	uint16 t = ((numer - q) >> 1) + q;
	return t >> denom->more;
}

uint16 libdivide_u16_recover(const struct libdivide_u16_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_16_SHIFT_MASK;
	if(!denom->magic) {
		...
		uint16 hi_dividend = (uint16)1 << shift;
		uint16 rem_ignored;
		...
		uint32 half_n = (uint32)1 << (16 + shift);
		uint32 d = ((uint32)1 << 16) | denom->magic;
		// Note that the quotient is guaranteed <= 16 bits, but the remainder
		// may need 17!
		uint16 half_q = (uint16)(half_n / d);
		uint32 rem = half_n % d;
		// We computed 2^(16+shift)/(m+2^16)
		// Need to double it, and then add 1 to the quotient if doubling th
		// remainder would increase the quotient.
		// Note that rem<<1 cannot overflow, since rem < d and d is 17 bits
		uint16 full_q = half_q + half_q + ((rem << 1) >= d);
		// We rounded down in gen (hence +1)
		return full_q + 1;
	}
}

uint16 libdivide_u16_branchfree_recover(const struct libdivide_u16_branchfree_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_16_SHIFT_MASK;
	if(!denom->magic) {
		return (uint16)1 << (shift + 1);
	}
	else {
		// Here we wish to compute d = 2^(16+shift+1)/(m+2^16).
		// Notice (m + 2^16) is a 17 bit number. Use 32 bit division for now
...
// UINT32
//
static FORCEINLINE struct libdivide_u32_t libdivide_internal_u32_gen(uint32 d, int branchfree)                                                                       
{
	if(d == 0) {
		LIBDIVIDE_ERROR("divider must be != 0");
	}
	...
	if((d & (d - 1)) == 0) {
		...
		// This power works if e < 2**floor_log_2_d.
		if(!branchfree && (e < ((uint32)1 << floor_log_2_d))) {
			// This power works
			more = (uint8)floor_log_2_d;
		}
		else {
...
struct libdivide_u32_t libdivide_u32_gen(uint32 d) { return libdivide_internal_u32_gen(d, 0); }

struct libdivide_u32_branchfree_t libdivide_u32_branchfree_gen(uint32 d) 
{
	if(d == 1) {
		LIBDIVIDE_ERROR("branchfree divider must be != 1");
	}
...
uint32 libdivide_u32_do(uint32 numer, const struct libdivide_u32_t * denom) 
{
	uint8 more = denom->more;
	if(!denom->magic) {
		return numer >> more;
		...
		uint32 q = libdivide_mullhi_u32(denom->magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			uint32 t = ((numer - q) >> 1) + q;
...
uint32 libdivide_u32_branchfree_do(uint32 numer, const struct libdivide_u32_branchfree_t * denom) 
{
	uint32 q = libdivide_mullhi_u32(denom->magic, numer);
	uint32 t = ((numer - q) >> 1) + q;
	return t >> denom->more;
...
uint32 libdivide_u32_recover(const struct libdivide_u32_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_32_SHIFT_MASK;
	if(!denom->magic) {
		return (uint32)1 << shift;
	}
	else if(!(more & LIBDIVIDE_ADD_MARKER)) {
		...
		// We know d is not a power of 2, so m is not a power of 2,
		// so we can just add 1 to the floor
		uint32 hi_dividend = (uint32)1 << shift;
		uint32 rem_ignored;
		...
		// then double the quotient and remainder.
		uint64 half_n = (uint64)1 << (32 + shift);
		uint64 d = ((uint64)1 << 32) | denom->magic;
		// Note that the quotient is guaranteed <= 32 bits, but the remainder
		// may need 33!
		uint32 half_q = (uint32)(half_n / d);
		uint64 rem = half_n % d;
...
uint32 libdivide_u32_branchfree_recover(const struct libdivide_u32_branchfree_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_32_SHIFT_MASK;
	if(!denom->magic) {
...
// UINT64
//
static FORCEINLINE struct libdivide_u64_t libdivide_internal_u64_gen(uint64 d, int branchfree)                                                                       
{
	if(d == 0) {
		LIBDIVIDE_ERROR("divider must be != 0");
	}
	...
	if((d & (d - 1)) == 0) {
		...
		// This power works if e < 2**floor_log_2_d.
		if(!branchfree && e < ((uint64)1 << floor_log_2_d)) {
			// This power works
			more = (uint8)floor_log_2_d;
		}
		else {
...
struct libdivide_u64_t libdivide_u64_gen(uint64 d) { return libdivide_internal_u64_gen(d, 0); }

struct libdivide_u64_branchfree_t libdivide_u64_branchfree_gen(uint64 d) 
{
	if(d == 1) {
		LIBDIVIDE_ERROR("branchfree divider must be != 1");
	}
...
uint64 libdivide_u64_do(uint64 numer, const struct libdivide_u64_t * denom) 
{
	uint8 more = denom->more;
	if(!denom->magic) {
		return numer >> more;
		...
		uint64 q = libdivide_mullhi_u64(denom->magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			uint64 t = ((numer - q) >> 1) + q;
...
uint64 libdivide_u64_branchfree_do(uint64 numer, const struct libdivide_u64_branchfree_t * denom) 
{
	uint64 q = libdivide_mullhi_u64(denom->magic, numer);
	uint64 t = ((numer - q) >> 1) + q;
	return t >> denom->more;
...
uint64 libdivide_u64_recover(const struct libdivide_u64_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_64_SHIFT_MASK;
	if(!denom->magic) {
		...
		// We know d is not a power of 2, so m is not a power of 2,
		// so we can just add 1 to the floor
		uint64 hi_dividend = (uint64)1 << shift;
		uint64 rem_ignored;
		...
		uint64 half_n_hi = (uint64)1 << shift, half_n_lo = 0;
		// d is a 65 bit value. The high bit is always set to 1.
		const uint64 d_hi = 1, d_lo = denom->magic;
		...
		// cannot overflow
		uint64 dr_lo = r_lo + r_lo;
		uint64 dr_hi = r_hi + r_hi + (dr_lo < r_lo); // last term is carry
		int dr_exceeds_d = (dr_hi > d_hi) || (dr_hi == d_hi && dr_lo >= d_lo);
		uint64 full_q = half_q + half_q + (dr_exceeds_d ? 1 : 0);
		return full_q + 1;
	}
}

uint64 libdivide_u64_branchfree_recover(const struct libdivide_u64_branchfree_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_64_SHIFT_MASK;
	if(!denom->magic) {
...
// SINT16
//
static FORCEINLINE struct libdivide_s16_t libdivide_internal_s16_gen(int16 d, int branchfree)                                                                       
{
	if(d == 0) {
		LIBDIVIDE_ERROR("divider must be != 0");
	}
	...
	if((absD & (absD - 1)) == 0) {
		...
		result.magic = 0;
		result.more = (uint8)(floor_log_2_d | (d < 0 ? LIBDIVIDE_NEGATIVE_DIVISOR : 0));
	}
	else {
		...
		uint16 rem, proposed_m;
		proposed_m = libdivide_32_div_16_to_16((uint16)1 << (floor_log_2_d - 1), 0, absD, &rem);
		const uint16 e = absD - rem;

		// We are going to start with a power of floor_log_2_d - 1.
		// This works if works if e < 2**floor_log_2_d.
		if(!branchfree && e < ((uint16)1 << floor_log_2_d)) {
			// This power works
			more = (uint8)(floor_log_2_d - 1);
		}
		else {
...
struct libdivide_s16_t libdivide_s16_gen(int16 d) { return libdivide_internal_s16_gen(d, 0); }

struct libdivide_s16_branchfree_t libdivide_s16_branchfree_gen(int16 d) 
{
...
// taking the address of a temporary. Hence this overload.
int16 libdivide_s16_do_raw(int16 numer, int16 magic, uint8 more) 
{
	uint8 shift = more & LIBDIVIDE_16_SHIFT_MASK;
	if(!magic) {
		uint16 sign = (int8_t)more >> 7;
		uint16 mask = ((uint16)1 << shift) - 1;
		uint16 uq = numer + ((numer >> 15) & mask);
		int16 q = (int16)uq;
		q >>= shift;
		q = (q ^ sign) - sign;
		return q;
		...
		uint16 uq = (uint16)libdivide_mullhi_s16(magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			// must be arithmetic shift and then sign extend
			int16 sign = (int8_t)more >> 7;
			// q += (more < 0 ? -numer : numer)
			// cast required to avoid UB
			uq += ((uint16)numer ^ sign) - sign;
		}
		int16 q = (int16)uq;
		q >>= shift;
		q += (q < 0);
...
int16 libdivide_s16_do(int16 numer, const struct libdivide_s16_t * denom) { return libdivide_s16_do_raw(numer, denom->magic, denom->more); }

int16 libdivide_s16_branchfree_do(int16 numer, const struct libdivide_s16_branchfree_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_16_SHIFT_MASK;
	// must be arithmetic shift and then sign extend
	int16 sign = (int8_t)more >> 7;
	int16 magic = denom->magic;
	int16 q = libdivide_mullhi_s16(magic, numer);
	q += numer;

	// If q is non-negative, we have nothing to do
	// If q is negative, we want to add either (2**shift)-1 if d is a power of
	// 2, or (2**shift) if it is not a power of 2
	uint16 is_power_of_2 = (magic == 0);
	uint16 q_sign = (uint16)(q >> 15);
	q += q_sign & (((uint16)1 << shift) - is_power_of_2);
...
int16 libdivide_s16_recover(const struct libdivide_s16_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_16_SHIFT_MASK;
	if(!denom->magic) {
		uint16 absD = (uint16)1 << shift;
		...
		if(denom->magic == 0) {
			int16 result = (uint16)1 << shift;
			return negative_divisor ? -result : result;
		}

		uint16 d = (uint16)(magic_was_negated ? -denom->magic : denom->magic);
		uint32 n = (uint32)1 << (16 + shift); // this shift cannot exceed 30
		uint16 q = (uint16)(n / d);
		int16 result = (int16)q;
		result += 1;
		return negative_divisor ? -result : result;
	}
}

int16 libdivide_s16_branchfree_recover(const struct libdivide_s16_branchfree_t * denom) { return libdivide_s16_recover((const struct libdivide_s16_t *)denom); }
//
// SINT32
//
static FORCEINLINE struct libdivide_s32_t libdivide_internal_s32_gen(int32_t d, int branchfree)                                                                       
{
	if(d == 0) {
		LIBDIVIDE_ERROR("divider must be != 0");
	}
	...
	if((absD & (absD - 1)) == 0) {
		...
		// is 0 and the high word is floor_log_2_d - 1
		uint32 rem, proposed_m;
		proposed_m = libdivide_64_div_32_to_32((uint32)1 << (floor_log_2_d - 1), 0, absD, &rem);
		const uint32 e = absD - rem;
...
int32_t libdivide_s32_do(int32_t numer, const struct libdivide_s32_t * denom) 
{
	...
	uint8 shift = more & LIBDIVIDE_32_SHIFT_MASK;
	if(!denom->magic) {
		uint32 sign = (int8_t)more >> 7;
		uint32 mask = ((uint32)1 << shift) - 1;
		uint32 uq = numer + ((numer >> 31) & mask);
		...
		uint32 uq = (uint32)libdivide_mullhi_s32(denom->magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			// must be arithmetic shift and then sign extend
			int32_t sign = (int8_t)more >> 7;
			// q += (more < 0 ? -numer : numer)
			// cast required to avoid UB
			uq += ((uint32)numer ^ sign) - sign;
		}
		int32_t q = (int32_t)uq;
		q >>= shift;
		q += (q < 0);
...
int32_t libdivide_s32_branchfree_do(int32_t numer, const struct libdivide_s32_branchfree_t * denom) 
{
	...
	// If q is negative, we want to add either (2**shift)-1 if d is a power of
	// 2, or (2**shift) if it is not a power of 2
	uint32 is_power_of_2 = (magic == 0);
	uint32 q_sign = (uint32)(q >> 31);
	q += q_sign & (((uint32)1 << shift) - is_power_of_2);
...
int32_t libdivide_s32_recover(const struct libdivide_s32_t * denom) 
{
	...
	if(!denom->magic) {
		...
		// Handle the power of 2 case (including branchfree)
		if(denom->magic == 0) {
			int32_t result = (uint32)1 << shift;
			return negative_divisor ? -result : result;
		}
		uint32 d = (uint32)(magic_was_negated ? -denom->magic : denom->magic);
		uint64 n = (uint64)1 << (32 + shift); // this shift cannot exceed 30
		uint32 q = (uint32)(n / d);
...
static FORCEINLINE struct libdivide_s64_t libdivide_internal_s64_gen(int64 d, int branchfree)                                                                       {
	...
	if((absD & (absD - 1)) == 0) {
		...
		// is 0 and the high word is floor_log_2_d - 1
		uint8 more;
		uint64 rem, proposed_m;
		proposed_m = libdivide_128_div_64_to_64((uint64)1 << (floor_log_2_d - 1), 0, absD, &rem);
		const uint64 e = absD - rem;
...
struct libdivide_s64_t libdivide_s64_gen(int64 d) { return libdivide_internal_s64_gen(d, 0); }

struct libdivide_s64_branchfree_t libdivide_s64_branchfree_gen(int64 d) 
{
...
int64 libdivide_s64_do(int64 numer, const struct libdivide_s64_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_64_SHIFT_MASK;
	if(!denom->magic) { // shift path
		uint64 mask = ((uint64)1 << shift) - 1;
		uint64 uq = numer + ((numer >> 63) & mask);
		int64 q = (int64)uq;
		q >>= shift;
		// must be arithmetic shift and then sign-extend
		int64 sign = (int8_t)more >> 7;
		q = (q ^ sign) - sign;
		return q;
	}
	else {
		uint64 uq = (uint64)libdivide_mullhi_s64(denom->magic, numer);
		if(more & LIBDIVIDE_ADD_MARKER) {
			// must be arithmetic shift and then sign extend
			int64 sign = (int8_t)more >> 7;
			// q += (more < 0 ? -numer : numer)
			// cast required to avoid UB
			uq += ((uint64)numer ^ sign) - sign;
		}
		int64 q = (int64)uq;
		q >>= shift;
		q += (q < 0);
...
int64 libdivide_s64_branchfree_do(int64 numer, const struct libdivide_s64_branchfree_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_64_SHIFT_MASK;
	// must be arithmetic shift and then sign extend
	int64 sign = (int8_t)more >> 7;
	int64 magic = denom->magic;
	int64 q = libdivide_mullhi_s64(magic, numer);
	q += numer;

	// If q is non-negative, we have nothing to do.
	// If q is negative, we want to add either (2**shift)-1 if d is a power of
	// 2, or (2**shift) if it is not a power of 2.
	uint64 is_power_of_2 = (magic == 0);
	uint64 q_sign = (uint64)(q >> 63);
	q += q_sign & (((uint64)1 << shift) - is_power_of_2);
...
int64 libdivide_s64_recover(const struct libdivide_s64_t * denom) 
{
	uint8 more = denom->more;
	uint8 shift = more & LIBDIVIDE_64_SHIFT_MASK;
	if(denom->magic == 0) { // shift path
		uint64 absD = (uint64)1 << shift;
		if(more & LIBDIVIDE_NEGATIVE_DIVISOR) {
			absD = -absD;
		}
		return (int64)absD;
		...
		int negative_divisor = (more & LIBDIVIDE_NEGATIVE_DIVISOR);
		int magic_was_negated = (more & LIBDIVIDE_ADD_MARKER) ? denom->magic > 0 : denom->magic < 0;
		uint64 d = (uint64)(magic_was_negated ? -denom->magic : denom->magic);
		uint64 n_hi = (uint64)1 << shift, n_lo = 0;
		uint64 rem_ignored;
		uint64 q = libdivide_128_div_64_to_64(n_hi, n_lo, d, &rem_ignored);
		int64 result = (int64)(q + 1);
		if(negative_divisor) {
			result = -result;
		}
		return result;
...
int64 libdivide_s64_branchfree_recover(const struct libdivide_s64_branchfree_t * denom) {
...
static FORCEINLINE uint16x8_t libdivide_u16_do_vec128(uint16x8_t numers, const struct libdivide_u16_t * denom);
static FORCEINLINE int16x8_t libdivide_s16_do_vec128(int16x8_t numers, const struct libdivide_s16_t * denom);
static FORCEINLINE uint32x4_t libdivide_u32_do_vec128(uint32x4_t numers, const struct libdivide_u32_t * denom);
static FORCEINLINE int32x4_t libdivide_s32_do_vec128(int32x4_t numers, const struct libdivide_s32_t * denom);
static FORCEINLINE uint64x2_t libdivide_u64_do_vec128(uint64x2_t numers, const struct libdivide_u64_t * denom);
static FORCEINLINE int64x2_t libdivide_s64_do_vec128(int64x2_t numers, const struct libdivide_s64_t * denom);

static FORCEINLINE uint16x8_t libdivide_u16_branchfree_do_vec128(uint16x8_t numers, const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE int16x8_t libdivide_s16_branchfree_do_vec128(int16x8_t numers, const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE uint32x4_t libdivide_u32_branchfree_do_vec128(uint32x4_t numers, const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE int32x4_t libdivide_s32_branchfree_do_vec128(int32x4_t numers, const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE uint64x2_t libdivide_u64_branchfree_do_vec128(uint64x2_t numers, const struct libdivide_u64_branchfree_t * denom);
static FORCEINLINE int64x2_t libdivide_s64_branchfree_do_vec128(int64x2_t numers, const struct libdivide_s64_branchfree_t * denom);

//////// Internal Utility Functions

// Logical right shift by runtime value.
// NEON implements right shift as left shits by negative values.
static FORCEINLINE uint32x4_t libdivide_u32_neon_srl(uint32x4_t v, uint8 amt) {
	int32_t wamt = (int32_t)(amt);
	return vshlq_u32(v, vdupq_n_s32(-wamt));
}

static FORCEINLINE uint64x2_t libdivide_u64_neon_srl(uint64x2_t v, uint8 amt) {
	int64 wamt = (int64)(amt);
	return vshlq_u64(v, vdupq_n_s64(-wamt));
}

// Arithmetic right shift by runtime value.
static FORCEINLINE int32x4_t libdivide_s32_neon_sra(int32x4_t v, uint8 amt) {
	int32_t wamt = (int32_t)(amt);
	return vshlq_s32(v, vdupq_n_s32(-wamt));
}

static FORCEINLINE int64x2_t libdivide_s64_neon_sra(int64x2_t v, uint8 amt) {
	int64 wamt = (int64)(amt);
	return vshlq_s64(v, vdupq_n_s64(-wamt));
}

static FORCEINLINE int64x2_t libdivide_s64_signbits(int64x2_t v) { return vshrq_n_s64(v, 63); }

static FORCEINLINE uint32x4_t libdivide_mullhi_u32_vec128(uint32x4_t a, uint32 b) {
...
static FORCEINLINE int32x4_t libdivide_mullhi_s32_vec128(int32x4_t a, int32_t b) {
...
static FORCEINLINE uint64x2_t libdivide_mullhi_u64_vec128(uint64x2_t x, uint64 sy) {
...
static FORCEINLINE int64x2_t libdivide_mullhi_s64_vec128(int64x2_t x, int64 sy) {
	int64x2_t p = vreinterpretq_s64_u64(
		libdivide_mullhi_u64_vec128(vreinterpretq_u64_s64(x), (uint64)(sy)));
...
int32x4_t libdivide_s32_branchfree_do_vec128(int32x4_t numers, const struct libdivide_s32_branchfree_t * denom) {
	...
	int32x4_t mask = vdupq_n_s32(((uint32)1 << shift) - is_power_of_2);
...
int64x2_t libdivide_s64_branchfree_do_vec128(int64x2_t numers, const struct libdivide_s64_branchfree_t * denom) {
	...
	int64x2_t mask = vdupq_n_s64(((uint64)1 << shift) - is_power_of_2);
...
static FORCEINLINE __m512i libdivide_u16_do_vec512(__m512i numers, const struct libdivide_u16_t * denom);
static FORCEINLINE __m512i libdivide_s16_do_vec512(__m512i numers, const struct libdivide_s16_t * denom);
static FORCEINLINE __m512i libdivide_u32_do_vec512(__m512i numers, const struct libdivide_u32_t * denom);
static FORCEINLINE __m512i libdivide_s32_do_vec512(__m512i numers, const struct libdivide_s32_t * denom);
static FORCEINLINE __m512i libdivide_u64_do_vec512(__m512i numers, const struct libdivide_u64_t * denom);
static FORCEINLINE __m512i libdivide_s64_do_vec512(__m512i numers, const struct libdivide_s64_t * denom);

static FORCEINLINE __m512i libdivide_u16_branchfree_do_vec512(__m512i numers, const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE __m512i libdivide_s16_branchfree_do_vec512(__m512i numers, const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE __m512i libdivide_u32_branchfree_do_vec512(__m512i numers, const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE __m512i libdivide_s32_branchfree_do_vec512(__m512i numers, const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE __m512i libdivide_u64_branchfree_do_vec512(__m512i numers, const struct libdivide_u64_branchfree_t * denom);
static FORCEINLINE __m512i libdivide_s64_branchfree_do_vec512(__m512i numers, const struct libdivide_s64_branchfree_t * denom);

//////// Internal Utility Functions

static FORCEINLINE __m512i libdivide_s64_signbits_vec512(__m512i v) {
	;
	return _mm512_srai_epi64(v, 63);
}

static FORCEINLINE __m512i libdivide_s64_shift_right_vec512(__m512i v, int amt) {
	return _mm512_srai_epi64(v, amt);
}

// Here, b is assumed to contain one 32-bit value repeated.
static FORCEINLINE __m512i libdivide_mullhi_u32_vec512(__m512i a, __m512i b) {
...
static FORCEINLINE __m512i libdivide_mullhi_s32_vec512(__m512i a, __m512i b) {
...
static FORCEINLINE __m512i libdivide_mullhi_u64_vec512(__m512i x, __m512i y) {
...
static FORCEINLINE __m512i libdivide_mullhi_s64_vec512(__m512i x, __m512i y) {
...
__m512i libdivide_u32_do_vec512(__m512i numers, const struct libdivide_u32_t * denom) {
	...
	if(!denom->magic) {
		...
		if(more & LIBDIVIDE_ADD_MARKER) {
			// uint32 t = ((numer - q) >> 1) + q;
			// return t >> denom->shift;
			uint32 shift = more & LIBDIVIDE_32_SHIFT_MASK;
...
__m512i libdivide_u64_do_vec512(__m512i numers, const struct libdivide_u64_t * denom) {
	...
	if(!denom->magic) {
		...
		if(more & LIBDIVIDE_ADD_MARKER) {
			// uint32 t = ((numer - q) >> 1) + q;
			// return t >> denom->shift;
			uint32 shift = more & LIBDIVIDE_64_SHIFT_MASK;
...
__m512i libdivide_s32_branchfree_do_vec512(__m512i numers, const struct libdivide_s32_branchfree_t * denom) {
	...
	__m512i mask = _mm512_set1_epi32(((uint32)1 << shift) - is_power_of_2);
...
__m512i libdivide_s64_branchfree_do_vec512(__m512i numers, const struct libdivide_s64_branchfree_t * denom) {
	...
	__m512i mask = _mm512_set1_epi64(((uint64)1 << shift) - is_power_of_2);
...
static FORCEINLINE __m256i libdivide_u16_do_vec256(__m256i numers, const struct libdivide_u16_t * denom);
static FORCEINLINE __m256i libdivide_s16_do_vec256(__m256i numers, const struct libdivide_s16_t * denom);
static FORCEINLINE __m256i libdivide_u32_do_vec256(__m256i numers, const struct libdivide_u32_t * denom);
static FORCEINLINE __m256i libdivide_s32_do_vec256(__m256i numers, const struct libdivide_s32_t * denom);
static FORCEINLINE __m256i libdivide_u64_do_vec256(__m256i numers, const struct libdivide_u64_t * denom);
static FORCEINLINE __m256i libdivide_s64_do_vec256(__m256i numers, const struct libdivide_s64_t * denom);

static FORCEINLINE __m256i libdivide_u16_branchfree_do_vec256(__m256i numers, const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE __m256i libdivide_s16_branchfree_do_vec256(__m256i numers, const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE __m256i libdivide_u32_branchfree_do_vec256(__m256i numers, const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE __m256i libdivide_s32_branchfree_do_vec256(__m256i numers, const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE __m256i libdivide_u64_branchfree_do_vec256(__m256i numers, const struct libdivide_u64_branchfree_t * denom);
static FORCEINLINE __m256i libdivide_s64_branchfree_do_vec256(__m256i numers, const struct libdivide_s64_branchfree_t * denom);

//////// Internal Utility Functions

// Implementation of _mm256_srai_epi64(v, 63) (from AVX512).
static FORCEINLINE __m256i libdivide_s64_signbits_vec256(__m256i v) {
...
static FORCEINLINE __m256i libdivide_s64_shift_right_vec256(__m256i v, int amt) {
	const int b = 64 - amt;
	__m256i m = _mm256_set1_epi64x((uint64)1 << (b - 1));
...
// Here, b is assumed to contain one 32-bit value repeated.
static FORCEINLINE __m256i libdivide_mullhi_u32_vec256(__m256i a, __m256i b) {
...
// b is one 32-bit value repeated.
static FORCEINLINE __m256i libdivide_mullhi_s32_vec256(__m256i a, __m256i b) {
...
// Here, y is assumed to contain one 64-bit value repeated.
static FORCEINLINE __m256i libdivide_mullhi_u64_vec256(__m256i x, __m256i y) {
	// see m128i variant for comments.
...
// y is one 64-bit value repeated.
static FORCEINLINE __m256i libdivide_mullhi_s64_vec256(__m256i x, __m256i y) {
...
__m256i libdivide_s16_do_vec256(__m256i numers, const struct libdivide_s16_t * denom) {
	uint8 more = denom->more;
	if(!denom->magic) {
		uint16 shift = more & LIBDIVIDE_16_SHIFT_MASK;
		uint16 mask = ((uint16)1 << shift) - 1;
...
__m256i libdivide_s16_branchfree_do_vec256(__m256i numers, const struct libdivide_s16_branchfree_t * denom) {
	...
	__m256i mask = _mm256_set1_epi16(((uint16)1 << shift) - is_power_of_2);
...
__m256i libdivide_s32_branchfree_do_vec256(__m256i numers, const struct libdivide_s32_branchfree_t * denom) {
	...
	__m256i mask = _mm256_set1_epi32(((uint32)1 << shift) - is_power_of_2);
...
__m256i libdivide_s64_branchfree_do_vec256(__m256i numers, const struct libdivide_s64_branchfree_t * denom) {
	...
	__m256i mask = _mm256_set1_epi64x(((uint64)1 << shift) - is_power_of_2);
...
static FORCEINLINE __m128i libdivide_u16_do_vec128(__m128i numers, const struct libdivide_u16_t * denom);
static FORCEINLINE __m128i libdivide_s16_do_vec128(__m128i numers, const struct libdivide_s16_t * denom);
static FORCEINLINE __m128i libdivide_u32_do_vec128(__m128i numers, const struct libdivide_u32_t * denom);
static FORCEINLINE __m128i libdivide_s32_do_vec128(__m128i numers, const struct libdivide_s32_t * denom);
static FORCEINLINE __m128i libdivide_u64_do_vec128(__m128i numers, const struct libdivide_u64_t * denom);
static FORCEINLINE __m128i libdivide_s64_do_vec128(__m128i numers, const struct libdivide_s64_t * denom);

static FORCEINLINE __m128i libdivide_u16_branchfree_do_vec128(__m128i numers, const struct libdivide_u16_branchfree_t * denom);
static FORCEINLINE __m128i libdivide_s16_branchfree_do_vec128(__m128i numers, const struct libdivide_s16_branchfree_t * denom);
static FORCEINLINE __m128i libdivide_u32_branchfree_do_vec128(__m128i numers, const struct libdivide_u32_branchfree_t * denom);
static FORCEINLINE __m128i libdivide_s32_branchfree_do_vec128(__m128i numers, const struct libdivide_s32_branchfree_t * denom);
static FORCEINLINE __m128i libdivide_u64_branchfree_do_vec128(__m128i numers, const struct libdivide_u64_branchfree_t * denom);
static FORCEINLINE __m128i libdivide_s64_branchfree_do_vec128(__m128i numers, const struct libdivide_s64_branchfree_t * denom);

//////// Internal Utility Functions

// Implementation of _mm_srai_epi64(v, 63) (from AVX512).
static FORCEINLINE __m128i libdivide_s64_signbits_vec128(__m128i v) {
...
static FORCEINLINE __m128i libdivide_s64_shift_right_vec128(__m128i v, int amt) {
	const int b = 64 - amt;
	__m128i m = _mm_set1_epi64x((uint64)1 << (b - 1));
...
// Here, b is assumed to contain one 32-bit value repeated.
static FORCEINLINE __m128i libdivide_mullhi_u32_vec128(__m128i a, __m128i b) {
...
// repeated four times.
static FORCEINLINE __m128i libdivide_mullhi_s32_vec128(__m128i a, __m128i b) {
...
// Here, y is assumed to contain one 64-bit value repeated.
static FORCEINLINE __m128i libdivide_mullhi_u64_vec128(__m128i x, __m128i y) {
	// full 128 bits product is:
	// x0*y0 + (x0*y1 << 32) + (x1*y0 << 32) + (x1*y1 << 64)
	// Note x0,y0,x1,y1 are all conceptually uint32, products are 32x32->64.
...
// y is one 64-bit value repeated.
static FORCEINLINE __m128i libdivide_mullhi_s64_vec128(__m128i x, __m128i y) {
...
__m128i libdivide_s16_branchfree_do_vec128(__m128i numers, const struct libdivide_s16_branchfree_t * denom) {
	...
	__m128i mask = _mm_set1_epi16(((uint16)1 << shift) - is_power_of_2);
...
__m128i libdivide_s32_branchfree_do_vec128(__m128i numers, const struct libdivide_s32_branchfree_t * denom) {
	...
	__m128i mask = _mm_set1_epi32(((uint32)1 << shift) - is_power_of_2);
...
__m128i libdivide_s64_branchfree_do_vec128(__m128i numers, const struct libdivide_s64_branchfree_t * denom) {
	...
	__m128i mask = _mm_set1_epi64x(((uint64)1 << shift) - is_power_of_2);
...
#define LIBDIVIDE_DIVIDE_NEON(ALGO, INT_TYPE)                    \
	FORCEINLINE typename NeonVecFor<INT_TYPE>::type divide( \
...
#define LIBDIVIDE_DIVIDE_SSE2(ALGO)                     \
	FORCEINLINE __m128i divide(__m128i n) const {  \
		return libdivide_ ## ALGO ## _do_vec128(n, &denom); \
	}
#else
#define LIBDIVIDE_DIVIDE_SSE2(ALGO)
...
#define LIBDIVIDE_DIVIDE_AVX2(ALGO)                     \
	FORCEINLINE __m256i divide(__m256i n) const {  \
...
#define LIBDIVIDE_DIVIDE_AVX512(ALGO)                   \
	FORCEINLINE __m512i divide(__m512i n) const {  \
...
#define DISPATCHER_GEN(T, ALGO)                                                       \
	...
	FORCEINLINE dispatcher() {}                                                  \
	FORCEINLINE dispatcher(T d) : denom(libdivide_ ## ALGO ## _gen(d)) {}            \
	FORCEINLINE T divide(T n) const { return libdivide_ ## ALGO ## _do(n, &denom); } \
	FORCEINLINE T recover() const { return libdivide_ ## ALGO ## _recover(&denom); } \
...
template <> struct dispatcher<16, SIGNED, BRANCHFULL> { DISPATCHER_GEN(int16, s16) };
template <> struct dispatcher<16, SIGNED, BRANCHFREE> { DISPATCHER_GEN(int16, s16_branchfree) };
template <> struct dispatcher<16, UNSIGNED, BRANCHFULL> { DISPATCHER_GEN(uint16, u16) };
template <> struct dispatcher<16, UNSIGNED, BRANCHFREE> { DISPATCHER_GEN(uint16, u16_branchfree) };
template <> struct dispatcher<32, SIGNED, BRANCHFULL> { DISPATCHER_GEN(int32_t, s32) };
template <> struct dispatcher<32, SIGNED, BRANCHFREE> { DISPATCHER_GEN(int32_t, s32_branchfree) };
template <> struct dispatcher<32, UNSIGNED, BRANCHFULL> { DISPATCHER_GEN(uint32, u32) };
template <> struct dispatcher<32, UNSIGNED, BRANCHFREE> { DISPATCHER_GEN(uint32, u32_branchfree) };
template <> struct dispatcher<64, SIGNED, BRANCHFULL> { DISPATCHER_GEN(int64, s64) };
template <> struct dispatcher<64, SIGNED, BRANCHFREE> { DISPATCHER_GEN(int64, s64_branchfree) };
template <> struct dispatcher<64, UNSIGNED, BRANCHFULL> { DISPATCHER_GEN(uint64, u64) };
template <> struct dispatcher<64, UNSIGNED, BRANCHFREE> { DISPATCHER_GEN(uint64, u64_branchfree) };
}  // namespace detail

#if defined(LIBDIVIDE_NEON)
// Allow NeonVecFor outside of detail namespace.
template <typename T> struct NeonVecFor { typedef typename detail::NeonVecFor<T>::type type; };
...
public:
	...
	FORCEINLINE divider(T d) : div(d) {}
	// Divides n by the divisor
	FORCEINLINE T divide(T n) const { return div.divide(n); }
	// Recovers the divisor, returns the value that was
	// used to initialize this divider object.
	T recover() const { return div.recover(); }
	bool operator==(const divider<T, ALGO> &other) const { return div.denom.magic == other.denom.magic && div.denom.more == other.denom.more; }
	...
	// quotients.
#if defined(LIBDIVIDE_SSE2)
	FORCEINLINE __m128i divide(__m128i n) const { return div.divide(n); }
#endif
#if defined(LIBDIVIDE_AVX2)
	FORCEINLINE __m256i divide(__m256i n) const { return div.divide(n); }
#endif
#if defined(LIBDIVIDE_AVX512)
	FORCEINLINE __m512i divide(__m512i n) const { return div.divide(n); }
#endif
#if defined(LIBDIVIDE_NEON)
	FORCEINLINE typename NeonVecFor<T>::type divide(typename NeonVecFor<T>::type n) const { return div.divide(n); }
#endif
private:
...
FORCEINLINE T operator/(T n, const divider<T, ALGO> &div) {
	return div.divide(n);
}
...
#if defined(LIBDIVIDE_SSE2)
template <typename T, Branching ALGO>
FORCEINLINE __m128i operator/(__m128i n, const divider<T, ALGO> &div) {
	return div.divide(n);
}
...
#if defined(LIBDIVIDE_AVX2)
template <typename T, Branching ALGO>
FORCEINLINE __m256i operator/(__m256i n, const divider<T, ALGO> &div) {
	return div.divide(n);
}
...
#if defined(LIBDIVIDE_AVX512)
template <typename T, Branching ALGO>
FORCEINLINE __m512i operator/(__m512i n, const divider<T, ALGO> &div) {
	return div.divide(n);
}
...
#endif
#if defined(LIBDIVIDE_NEON)
	template <typename T, Branching ALGO> FORCEINLINE typename NeonVecFor<T>::type operator/(typename NeonVecFor<T>::type n, const divider<T, ALGO> &div) 
	{
		return div.divide(n);
	}
	template <typename T, Branching ALGO> FORCEINLINE typename NeonVecFor<T>::type operator/=(typename NeonVecFor<T>::type &n, const divider<T, ALGO> &div) 
