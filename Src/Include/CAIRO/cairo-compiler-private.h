...
#define _CAIRO_BOOLEAN_EXPR(expr) __extension__ ({ int _cairo_boolean_var_ = expr ? 1 : 0; _cairo_boolean_var_; })
...
#if 0 // { @sobolev #include <intrin.h>
#pragma intrinsic(_BitScanForward)
static __forceinline int ffs(int x)
{
    unsigned long i;
	return (_BitScanForward(&i, x) != 0) ? (i + 1) : 0;
}
#endif // } 0 @sobolev
