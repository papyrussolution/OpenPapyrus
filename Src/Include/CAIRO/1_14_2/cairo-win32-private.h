...
enum {
	CAIRO_WIN32_SURFACE_FOR_PRINTING = (1<<0), /* If this is a surface created for printing or not */
	CAIRO_WIN32_SURFACE_IS_DISPLAY = (1<<1), /* Whether the DC is a display DC or not */
	CAIRO_WIN32_SURFACE_CAN_BITBLT = (1<<2), /* Whether we can use BitBlt with this surface */
	CAIRO_WIN32_SURFACE_CAN_ALPHABLEND = (1<<3), /* Whether we can use AlphaBlend with this surface */
	CAIRO_WIN32_SURFACE_CAN_STRETCHBLT = (1<<4), /* Whether we can use StretchBlt with this surface */
	CAIRO_WIN32_SURFACE_CAN_STRETCHDIB = (1<<5), /* Whether we can use StretchDIBits with this surface */
	CAIRO_WIN32_SURFACE_CAN_RECT_GRADIENT = (1<<6), // Whether we can use GradientFill rectangles with this surface
	CAIRO_WIN32_SURFACE_CAN_CHECK_JPEG = (1<<7), // Whether we can use the CHECKJPEGFORMAT escape function 
	CAIRO_WIN32_SURFACE_CAN_CHECK_PNG = (1<<8), // Whether we can use the CHECKJPEGFORMAT escape function 
};
...
typedef struct _cairo_win32_display_surface {
	...
	// We create off-screen surfaces as DIBs or DDBs, based on what we created originally
	HBITMAP bitmap;
	cairo_bool_t is_dib;
...
cairo_bool_t _cairo_win32_surface_get_extents(void * abstract_surface, CairoIRect   * rectangle);
uint32_t _cairo_win32_flags_for_dc(HDC dc);

cairo_int_status_t _cairo_win32_surface_emit_glyphs(cairo_win32_surface_t * dst, const cairo_pattern_t * source,
