...
static inline cairo_fixed_t _cairo_fixed_from_int(int i)
{
...
static inline cairo_fixed_16_16_t _cairo_fixed_16_16_from_double(double d)
{
	union {
		double d;
		int32_t i[2];
...
//
static inline cairo_fixed_t _cairo_fixed_mul_div_floor(cairo_fixed_t a, cairo_fixed_t b, cairo_fixed_t c)
{
...
//
static inline cairo_fixed_t _cairo_edge_compute_intersection_y_for_x(const cairo_point_t * p1, const cairo_point_t * p2, cairo_fixed_t x)
{
	cairo_fixed_t y, dx;
	if(x == p1->x)
		return p1->y;
...
// http://paulbourke.net/geometry/pointlineplane/. This implementation uses floating point math.
// 
static inline cairo_bool_t _slow_segment_intersection(const cairo_point_t * seg1_p1,
    const cairo_point_t * seg1_p2, const cairo_point_t * seg2_p1, const cairo_point_t * seg2_p2, cairo_point_t * intersection)
{
	double seg1_dx = _cairo_fixed_to_double(seg1_p2->x - seg1_p1->x);
	double seg1_dy = _cairo_fixed_to_double(seg1_p2->y - seg1_p1->y);
	double seg2_dx = _cairo_fixed_to_double(seg2_p2->x - seg2_p1->x);
	double seg2_dy = _cairo_fixed_to_double(seg2_p2->y - seg2_p1->y);
	double denominator = (seg2_dy * seg1_dx) - (seg2_dx * seg1_dy);
	if(denominator == 0)
		return FALSE;
	else {
		double seg_start_dx = _cairo_fixed_to_double(seg1_p1->x - seg2_p1->x);
		double seg_start_dy = _cairo_fixed_to_double(seg1_p1->y - seg2_p1->y);
		double u_a = ((seg2_dx * seg_start_dy) - (seg2_dy * seg_start_dx)) / denominator;
		double u_b = ((seg1_dx * seg_start_dy) - (seg1_dy * seg_start_dx)) / denominator;
