...
static inline double cairo_test_NaN(void)
{
#ifdef _MSC_VER
...
typedef cairo_test_status_t (cairo_test_preamble_function_t)(cairo_test_context_t * ctx);
...
void cairo_test_register(cairo_test_t * test);

struct cairo_boilerplate_target_t;
...
//void cairo_test_log(const cairo_test_context_t * ctx, const char * fmt, ...) CAIRO_BOILERPLATE_PRINTF_FORMAT(2, 3);
//void cairo_test_logv(const cairo_test_context_t * ctx, const char * fmt, va_list ap) CAIRO_BOILERPLATE_PRINTF_FORMAT(2, 0);
