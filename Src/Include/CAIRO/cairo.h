 ...
 * @CAIRO_STATUS_INVALID_SIZE: invalid value (typically too big) for the size of the input (surface, pattern, etc.)
 **(Since 1.10)
...
cairo_public void cairo_set_fill_rule(cairo_t * cr, CairoFillRule fill_rule);
...
typedef enum _cairo_text_cluster_flags {
	CAIRO_TEXT_CLUSTER_FLAG_NONE     = 0x00000000, // @sobolev
...
cairo_public void cairo_show_text_glyphs(cairo_t * cr, const char * utf8, int utf8_len, const cairo_glyph_t * glyphs,
 ...
 * Since: 1.8
 **/
typedef cairo_status_t (*cairo_user_scaled_font_render_glyph_func_t)(cairo_scaled_font_t * scaled_font, ulong glyph, cairo_t * cr, cairo_text_extents_t * extents);
...
cairo_public CairoFillRule cairo_get_fill_rule(cairo_t * cr);
...
cairo_public cairo_status_t cairo_device_set_user_data(cairo_device_t * device, const cairo_user_data_key_t * key,
	void * user_data, cairo_destroy_func_t destroy);

/* Surface manipulation */
cairo_public cairo_surface_t * cairo_surface_create_similar(cairo_surface_t * other, cairo_content_t content,
	int width, int height);
cairo_public cairo_surface_t * cairo_surface_create_similar_image(cairo_surface_t * other, cairo_format_t format,
	int width, int height);
...
cairo_public cairo_status_t FASTCALL cairo_surface_status(cairo_surface_t * surface);
/**
...
cairo_public cairo_status_t cairo_surface_set_user_data(cairo_surface_t * surface, const cairo_user_data_key_t * key,
    void * user_data, cairo_destroy_func_t destroy);
 ...
 * Since: 1.12
 **/
typedef cairo_surface_t * (*cairo_raster_source_acquire_func_t)(cairo_pattern_t * pattern,
    void * callback_data, cairo_surface_t * target, const CairoIRect * extents);
/**
...
cairo_public void cairo_raster_source_pattern_set_acquire(cairo_pattern_t * pattern, cairo_raster_source_acquire_func_t acquire,
    cairo_raster_source_release_func_t release);
cairo_public void cairo_raster_source_pattern_get_acquire(cairo_pattern_t * pattern,
    cairo_raster_source_acquire_func_t * acquire, cairo_raster_source_release_func_t * release);
...
cairo_public void cairo_region_get_rectangle(const cairo_region_t  * region, int nth, CairoIRect * rectangle);
cairo_public cairo_bool_t cairo_region_is_empty(const cairo_region_t * region);
cairo_public cairo_region_overlap_t FASTCALL cairo_region_contains_rectangle(const cairo_region_t * region, const CairoIRect * rectangle);
...
cairo_public cairo_status_t cairo_region_subtract_rectangle(cairo_region_t * dst, const CairoIRect * rectangle);
cairo_public cairo_status_t cairo_region_intersect(cairo_region_t * dst, const cairo_region_t * other);
cairo_public cairo_status_t cairo_region_intersect_rectangle(cairo_region_t * dst, const CairoIRect * rectangle);
cairo_public cairo_status_t cairo_region_union(cairo_region_t * dst, const cairo_region_t * other);
cairo_public cairo_status_t cairo_region_union_rectangle(cairo_region_t * dst, const CairoIRect * rectangle);
cairo_public cairo_status_t cairo_region_xor(cairo_region_t * dst, const cairo_region_t * other);
cairo_public cairo_status_t cairo_region_xor_rectangle(cairo_region_t * dst, const CairoIRect * rectangle);
