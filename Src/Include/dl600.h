...
public:
	...
	enum COption { // @persistent
		...
		cuifLayoutSymbol        = 47, // @v12.7.2 string Символ элемента лейаута. Может понадобиться если в runtime'е с этим элементом надо что-то делать.
			// Неприменимо для элементов лейаута, которым символ присваивается автоматом (например, лейауты верхнего уровня, типа диалога)
		//
...
#define DBTABLE_CONSTRUCTOR(tbl, firstField) \
	tbl##Tbl::tbl##Tbl(const char * pFileName, DbProvider * pDbP) : DBTable(#tbl, pFileName, &firstField, &data, omNormal, pDbP) {}\
