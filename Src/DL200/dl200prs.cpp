	...
	if(pItem->CiType == DL2CIT_ACC) {
		...
		if(r > 0) {
			...
			if(dl2ac.Flags & DL2_Acc::fTurnover && dl2ac.CorrAcc.ac) {
				//
				// ќбороты в корреспонденции с заданным счетом
				//
				AccAnlzFilt flt;
				flt.Period = period;
				...
				//
				// –азвернутый остаток.
				// –ассчитываетс€ тогда, когда запрашиваетс€ вход€щий либо исход€щий остаток
				// конкретно по дебету либо по кредиту счета (субсчета). ¬ остальных случа€х
				// опци€ fSpread игнорируетс€.
				//
