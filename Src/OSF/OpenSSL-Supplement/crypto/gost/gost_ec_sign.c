...
#ifdef DEBUG_SIGN
	extern void dump_signature(const char * message, const unsigned char * buffer, size_t len);
...
static R3410_ec_params * gost_nid2params(int nid)
{
	...
	switch(nid) {
		case NID_id_tc26_gost_3410_2012_256_paramSetB: nid = NID_id_GostR3410_2001_CryptoPro_A_ParamSet; break;
		case NID_id_tc26_gost_3410_2012_256_paramSetC: nid = NID_id_GostR3410_2001_CryptoPro_B_ParamSet; break;
		case NID_id_tc26_gost_3410_2012_256_paramSetD: nid = NID_id_GostR3410_2001_CryptoPro_C_ParamSet; break;
	}
...
int fill_GOST_EC_params(EC_KEY * eckey, int nid)
{
	...
	if(!BN_hex2bn(&p, params->p) || !BN_hex2bn(&a, params->a) || !BN_hex2bn(&b, params->b) || !BN_hex2bn(&cofactor, params->cofactor)) {
	...
	if(!BN_hex2bn(&x, params->x) || !BN_hex2bn(&y, params->y) || !EC_POINT_set_affine_coordinates(grp, P, x, y, ctx) || !BN_hex2bn(&q, params->q)) {
		GOSTerr(GOST_F_FILL_GOST_EC_PARAMS, ERR_R_INTERNAL_ERROR);
		goto end;
	}
...
ECDSA_SIG * gost_ec_sign(const unsigned char * dgst, int dlen, EC_KEY * eckey)
{
	...
	do {
		do {
			...
			if(!BN_add(k, k, order) || (BN_num_bits(k) <= BN_num_bits(order) && !BN_add(k, k, order))) {
				goto err;
			}
			...
			SETIFZ(X, BN_CTX_get(ctx));
			SETIFZ(r, BN_CTX_get(ctx));
			if(!X || !r) {
				GOSTerr(GOST_F_GOST_EC_SIGN, ERR_R_MALLOC_FAILURE);
				goto err;
			}
		...
		SETIFZ(tmp, BN_CTX_get(ctx));
		SETIFZ(tmp2, BN_CTX_get(ctx));
		SETIFZ(s, BN_CTX_get(ctx));
		if(!tmp || !tmp2 || !s) {
			GOSTerr(GOST_F_GOST_EC_SIGN, ERR_R_MALLOC_FAILURE);
			goto err;
		}
 ...
 *
 */
int gost_ec_verify(const unsigned char * dgst, int dgst_len, ECDSA_SIG * sig, EC_KEY * ec)
{
	BN_CTX * ctx;
	...
	if(BN_is_zero(sig_s) || BN_is_zero(sig_r) || (BN_cmp(sig_s, order) >= 1) || (BN_cmp(sig_r, order) >= 1)) {
	...
	if(!v || !BN_mod_mul(z1, sig_s, v, order, ctx) || !BN_sub(tmp, order, sig_r) || !BN_mod_mul(z2, tmp, v, order, ctx)) {
		GOSTerr(GOST_F_GOST_EC_VERIFY, ERR_R_INTERNAL_ERROR);
		goto err;
	}
#ifdef DEBUG_SIGN
