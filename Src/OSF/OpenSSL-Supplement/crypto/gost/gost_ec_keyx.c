...
int VKO_compute_key(unsigned char * shared_key, const EC_POINT * pub_key, const EC_KEY * priv_key,
    const unsigned char * ukm, const size_t ukm_size, const int vko_dgst_nid)
{
	...
	if(!EC_POINT_get_affine_coordinates(EC_KEY_get0_group(priv_key), pnt, X, Y, ctx)) {
	...
	buf_len = 2 * half_len;
	databuf = static_cast<uchar *>(OPENSSL_zalloc(buf_len));
	if(!databuf) {
		GOSTerr(GOST_F_VKO_COMPUTE_KEY, ERR_R_MALLOC_FAILURE);
		goto err;
	}
 ...
 * */
static int gost_keg(const unsigned char * ukm_source, int pkey_nid, const EC_POINT * pub_key, const EC_KEY * priv_key, unsigned char * keyout)
{
/* Adjust UKM */
	...
	switch(pkey_nid) {
		case NID_id_GostR3410_2012_512:
		    keylen =
			VKO_compute_key(keyout, pub_key, priv_key, real_ukm, 16, NID_id_GostR3411_2012_512);
		    return (keylen) ? keylen : 0;
		    break;
		case NID_id_GostR3410_2012_256:
	    {
		    unsigned char tmpkey[32];
		    keylen = VKO_compute_key(tmpkey, pub_key, priv_key, real_ukm, 16, NID_id_GostR3411_2012_256);
		    if(keylen == 0)
			    return 0;
		    if(gost_kdftree2012_256(keyout, 64, tmpkey, 32, (const unsigned char*)"kdf tree", 8, ukm_source + 16, 8, 1) > 0)
			    keylen = 64;
		    else
			    keylen = 0;
...
int pkey_gost_ec_derive(EVP_PKEY_CTX * ctx, unsigned char * key, size_t * keylen)
{
	...
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(ctx));
	...
	switch(data->shared_ukm_size) {
		case 8:
	    {
		    ...
		    *keylen = VKO_compute_key(key, EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(peer_key))), (EC_KEY*)EVP_PKEY_get0(my_key), data->shared_ukm, 8, dgst_nid);
		    return (*keylen) ? 1 : 0;
	    }
	    break;
		case 32:
	    {
		    ...
		    *keylen = gost_keg(data->shared_ukm, EVP_PKEY_id(my_key), EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(peer_key))), (EC_KEY*)EVP_PKEY_get0(my_key), key);
		    return (*keylen) ? 1 : 0;
	    }
	    break;
		default:
...
static int pkey_GOST_ECcp_encrypt(EVP_PKEY_CTX * pctx, unsigned char * out, size_t * out_len, const unsigned char * key, size_t key_len)
{
	GOST_KEY_TRANSPORT * gkt = NULL;
	EVP_PKEY * pubk = EVP_PKEY_CTX_get0_pkey(pctx);
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(pctx));
	...
	if(sec_key) {
		...
		if(!gost_get0_priv_key(sec_key)) {
			GOSTerr(GOST_F_PKEY_GOST_ECCP_ENCRYPT, GOST_R_NO_PRIVATE_PART_OF_NON_EPHEMERAL_KEYPAIR);
			goto err;
		}
	}
	else {
		...
		if(out) {
			...
			if(!EVP_PKEY_assign(sec_key, EVP_PKEY_base_id(pubk), EC_KEY_new()) || !EVP_PKEY_copy_parameters(sec_key, pubk)
			    || !gost_ec_keygen(static_cast<EC_KEY *>(EVP_PKEY_get0(sec_key)))) {
	...
	if(out) {
		...
		if(dgst_nid == NID_id_GostR3411_2012_512)
			dgst_nid = NID_id_GostR3411_2012_256;
		if(!VKO_compute_key(shared_key, EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(pubk))), static_cast<EC_KEY *>(EVP_PKEY_get0(sec_key)), ukm, 8, dgst_nid)) {
			GOSTerr(GOST_F_PKEY_GOST_ECCP_ENCRYPT, GOST_R_ERROR_COMPUTING_SHARED_KEY);
			goto err;
		}
 ...
 */
static int pkey_gost2018_encrypt(EVP_PKEY_CTX * pctx, unsigned char * out, size_t * out_len, const unsigned char * key, size_t key_len)
{
	...
	exp_buf = static_cast<uchar *>(OPENSSL_malloc(exp_len));
	if(!exp_buf) {
	...
	if(!sec_key) {
		...
		if(!EVP_PKEY_assign(sec_key, EVP_PKEY_base_id(pubk), EC_KEY_new()) || !EVP_PKEY_copy_parameters(sec_key, pubk) || !gost_ec_keygen(static_cast<EC_KEY *>(EVP_PKEY_get0(sec_key)))) {
			GOSTerr(GOST_F_PKEY_GOST2018_ENCRYPT, GOST_R_ERROR_COMPUTING_SHARED_KEY);
			goto err;
		}
		key_is_ephemeral = 1;
	}
	if(gost_keg(data->shared_ukm, pkey_nid, EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(pubk))), static_cast<const EC_KEY *>(EVP_PKEY_get0(sec_key)), expkeys) <= 0) {
		GOSTerr(GOST_F_PKEY_GOST2018_ENCRYPT, GOST_R_ERROR_COMPUTING_EXPORT_KEYS);
		goto err;
	}
	if(gost_kexp15(key, key_len, data->cipher_nid, expkeys + 32, mac_nid, expkeys + 0, data->shared_ukm + 24, iv_len, exp_buf, &exp_len) <= 0) {
...
int pkey_gost_encrypt(EVP_PKEY_CTX * pctx, unsigned char * out, size_t * out_len, const unsigned char * key, size_t key_len)
{
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(pctx));
 ...
 * Implementation of GOST2001/12 key transport, cryptopro variation
 */
static int pkey_GOST_ECcp_decrypt(EVP_PKEY_CTX * pctx, unsigned char * key, size_t * key_len, const unsigned char * in, size_t in_len)
{
	...
	if(dgst_nid == NID_id_GostR3411_2012_512)
		dgst_nid = NID_id_GostR3411_2012_256;
	if(!VKO_compute_key(sharedKey, EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(peerkey))),
	    static_cast<const EC_KEY *>(EVP_PKEY_get0(priv)), wrappedKey, 8, dgst_nid)) {
		GOSTerr(GOST_F_PKEY_GOST_ECCP_DECRYPT, GOST_R_ERROR_COMPUTING_SHARED_KEY);
		goto err;
	}
 ...
 * Implementation of GOST2018 key transport
 */
static int pkey_gost2018_decrypt(EVP_PKEY_CTX * pctx, unsigned char * key, size_t * key_len, const unsigned char * in, size_t in_len)
{
	const unsigned char * p = in;
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(pctx));
	EVP_PKEY * priv = EVP_PKEY_CTX_get0_pkey(pctx);
 ...
 */
	if(gost_keg(data->shared_ukm, pkey_nid,
	    EC_KEY_get0_public_key(static_cast<const EC_KEY *>(EVP_PKEY_get0(eph_key))), static_cast<const EC_KEY *>(EVP_PKEY_get0(priv)), expkeys) <= 0) {
		GOSTerr(GOST_F_PKEY_GOST2018_DECRYPT, GOST_R_ERROR_COMPUTING_EXPORT_KEYS);
		goto err;
	}
	if(gost_kimp15(ASN1_STRING_get0_data(pst->psexp), ASN1_STRING_length(pst->psexp), data->cipher_nid, expkeys + 32, mac_nid, expkeys + 0, data->shared_ukm + 24, iv_len, key) <= 0) {
...
int pkey_gost_decrypt(EVP_PKEY_CTX * pctx, unsigned char * key, size_t * key_len, const unsigned char * in, size_t in_len)
{
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(pctx));
	if(data->shared_ukm == NULL || data->shared_ukm_size == 8)
