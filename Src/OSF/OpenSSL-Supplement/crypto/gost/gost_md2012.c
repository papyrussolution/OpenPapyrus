...
EVP_MD * digest_gost2012_256(void)
{
	if(_hidden_GostR3411_2012_256_md == NULL) {
		EVP_MD * md = EVP_MD_meth_new(NID_id_GostR3411_2012_256, NID_undef);
		if(!md || !EVP_MD_meth_set_result_size(md, 32)
...
static int gost_digest_update(EVP_MD_CTX * ctx, const void * data, size_t count)
{
	gost2012_hash_block((gost2012_hash_ctx*)EVP_MD_CTX_md_data(ctx), static_cast<const uchar *>(data), count);
	return 1;
}
...
static int gost_digest_cleanup(EVP_MD_CTX * ctx)
{
	if(EVP_MD_CTX_md_data(ctx))
		memzero(EVP_MD_CTX_md_data(ctx), sizeof(gost2012_hash_ctx));
	return 1;
}
...
static int gost_digest_ctrl_256(EVP_MD_CTX * ctx, int type, int arg, void * ptr)
{
	switch(type) {
	    ...
	    {
		    *((char**)ptr) = static_cast<char *>(OPENSSL_malloc(strlen(micalg_256) + 1));
		    if(*((char**)ptr) != NULL) {
...
static int gost_digest_ctrl_512(EVP_MD_CTX * ctx, int type, int arg, void * ptr)
{
	switch(type) {
		case EVP_MD_CTRL_MICALG:
	    {
		    *((char**)ptr) = static_cast<char *>(OPENSSL_malloc(strlen(micalg_512) + 1));
		    if(*((char**)ptr) != NULL) {
