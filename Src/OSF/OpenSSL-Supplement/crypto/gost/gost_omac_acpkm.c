...
{
	CMAC_ACPKM_CTX * ctx = static_cast<CMAC_ACPKM_CTX *>(OPENSSL_malloc(sizeof(CMAC_ACPKM_CTX)));
	if(!ctx)
		return NULL;
    ...
    const EVP_CIPHER * cipher, ENGINE * impl)
{
	...
	if(key) {
		...
		memzero(acpkm_iv + block_size / 2, block_size / 2);
		if(!EVP_EncryptInit_ex(ctx->actx, NULL, NULL, static_cast<const uchar *>(key), acpkm_iv))
			return 0;
...
{
	OMAC_ACPKM_CTX * c = static_cast<OMAC_ACPKM_CTX *>(EVP_MD_CTX_md_data(ctx));
	memzero(c, sizeof(OMAC_ACPKM_CTX));
...
static int omac_acpkm_imit_update(EVP_MD_CTX * ctx, const void * data, size_t count)
{
	OMAC_ACPKM_CTX * c = static_cast<OMAC_ACPKM_CTX *>(EVP_MD_CTX_md_data(ctx));
	if(!c->key_set) {
...
{
	OMAC_ACPKM_CTX * c_to = static_cast<OMAC_ACPKM_CTX *>(EVP_MD_CTX_md_data(to));
	const OMAC_ACPKM_CTX * c_from = static_cast<const OMAC_ACPKM_CTX *>(EVP_MD_CTX_md_data(from));
	if(c_from && c_to) {
...
int omac_acpkm_imit_cleanup(EVP_MD_CTX * ctx)
{
	...
	if(c) {
		...
		memzero(EVP_MD_CTX_md_data(ctx), sizeof(OMAC_ACPKM_CTX));
	}
	return 1;
}

static int omac_acpkm_key(OMAC_ACPKM_CTX * c, const EVP_CIPHER * cipher, const unsigned char * key, size_t key_size)
{
	int ret = 0;
	...
	switch(type) {
		...
		case EVP_MD_CTRL_SET_KEY:
	    {
		    ...
		    if(arg == 0) {
			    ...
			    return omac_acpkm_key(c, cipher, static_cast<const uchar *>(ptr), 32);
		    }
