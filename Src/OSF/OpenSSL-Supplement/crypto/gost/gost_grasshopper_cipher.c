...
static EVP_CIPHER * gost_grasshopper_ciphers[6] = { NULL, NULL, NULL, NULL, NULL, NULL, };
...
static void acpkm_next(gost_grasshopper_cipher_ctx * c)
{
	...
	for(n = 0; n < J; n++) {
		...
		grasshopper_encrypt_block(&c->encrypt_round_keys, (grasshopper_w128_t*)D_n, (grasshopper_w128_t*)&newkey[n * GRASSHOPPER_BLOCK_SIZE], &c->buffer);
	}
...
GRASSHOPPER_INLINE void gost_grasshopper_master_key(gost_grasshopper_cipher_ctx * c, const uint8_t * k)
{
	for(int i = 0; i < 2; i++) {
		grasshopper_copy128(&c->master_key.k.k[i], (const grasshopper_w128_t*)(k + i * 16));
	}
}
...
{
	gost_grasshopper_cipher_ctx * c = static_cast<gost_grasshopper_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
	...
	if(iv != NULL) {
		memcpy((uchar *)EVP_CIPHER_CTX_original_iv(ctx), iv, EVP_CIPHER_CTX_iv_length(ctx));
	}
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_ecb(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx * c = static_cast<gost_grasshopper_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_cbc(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx * c = static_cast<gost_grasshopper_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_ofb(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx_ofb * c = static_cast<gost_grasshopper_cipher_ctx_ofb *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
	c->c.type = GRASSHOPPER_CIPHER_OFB;
	grasshopper_zero128(&c->buffer1);
	return gost_grasshopper_cipher_init(ctx, key, iv, enc);
}

GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_cfb(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx * c = static_cast<gost_grasshopper_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_ctr(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx_ctr * c = static_cast<gost_grasshopper_cipher_ctx_ctr *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_init_ctracpkm(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	gost_grasshopper_cipher_ctx_ctr * c = static_cast<gost_grasshopper_cipher_ctx_ctr *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
GRASSHOPPER_INLINE int gost_grasshopper_cipher_do(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
...
{
	gost_grasshopper_cipher_ctx * c = (gost_grasshopper_cipher_ctx*)EVP_CIPHER_CTX_get_cipher_data(ctx);
	bool encrypting = LOGIC(EVP_CIPHER_CTX_encrypting(ctx));
	...
	for(i = 0; i < blocks; i++, current_in += GRASSHOPPER_BLOCK_SIZE, current_out += GRASSHOPPER_BLOCK_SIZE) {
		if(encrypting) {
			grasshopper_encrypt_block(&c->encrypt_round_keys, (grasshopper_w128_t*)current_in, (grasshopper_w128_t*)current_out, &c->buffer);
		}
		else {
			grasshopper_decrypt_block(&c->decrypt_round_keys, (grasshopper_w128_t*)current_in, (grasshopper_w128_t*)current_out, &c->buffer);
		}
	}
	return 1;
}
...
int gost_grasshopper_cipher_do_cbc(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	...
	size_t blocks = inl / GRASSHOPPER_BLOCK_SIZE;
	size_t i;
	grasshopper_w128_t * currentBlock = (grasshopper_w128_t*)iv;
	for(i = 0; i < blocks;
...
int gost_grasshopper_cipher_do_ctracpkm(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	...
	for(i = 0; i < blocks; i++) {
		...
		grasshopper_encrypt_block(&c->c.encrypt_round_keys, (grasshopper_w128_t*)iv, (grasshopper_w128_t*)&c->partial_buffer, &c->c.buffer);
	...
	if(lasted > 0) {
		apply_acpkm_grasshopper(c, &num);
		grasshopper_encrypt_block(&c->c.encrypt_round_keys, (grasshopper_w128_t*)iv, &c->partial_buffer, &c->c.buffer);
		for(i = 0; i < lasted; i++)
 ...
 */
static void gost_grasshopper_cnt_next(gost_grasshopper_cipher_ctx_ofb * ctx, grasshopper_w128_t * iv, grasshopper_w128_t * buf)
{
...
int gost_grasshopper_cipher_do_ofb(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	...
	for(; i + GRASSHOPPER_BLOCK_SIZE < inl; i += GRASSHOPPER_BLOCK_SIZE, in_ptr += GRASSHOPPER_BLOCK_SIZE, out_ptr += GRASSHOPPER_BLOCK_SIZE) {
		/*
...
int gost_grasshopper_cipher_do_cfb(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	...
	for(; i + GRASSHOPPER_BLOCK_SIZE < inl; i += GRASSHOPPER_BLOCK_SIZE, in_ptr += GRASSHOPPER_BLOCK_SIZE, out_ptr += GRASSHOPPER_BLOCK_SIZE) {
		 ...
		 * block cipher current iv
		 */
		grasshopper_encrypt_block(&c->encrypt_round_keys, (grasshopper_w128_t*)iv, (grasshopper_w128_t*)buf, &c->buffer);
		/*
		 * xor next block of input text with it and output it
		 */
		/*
...
int gost_grasshopper_cipher_ctl(EVP_CIPHER_CTX * ctx, int type, int arg, void * ptr)
{
	switch(type) {
		case EVP_CTRL_RAND_KEY: {
		    if(RAND_bytes
				((uchar *)ptr, EVP_CIPHER_CTX_key_length(ctx)) <= 0) {
		...
		case EVP_CTRL_KEY_MESH: {
		    gost_grasshopper_cipher_ctx_ctr * c = static_cast<gost_grasshopper_cipher_ctx_ctr *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
		    if(c->c.type != GRASSHOPPER_CIPHER_CTRACPKM || !arg || (arg % GRASSHOPPER_BLOCK_SIZE))
			    return -1;
		    c->section_size = arg;
		    break;
	    }
...
{
	return EVP_CIPHER_meth_new(cipher_type, block_size /* block_size */, GRASSHOPPER_KEY_SIZE /* key_size */);
}

const int cipher_gost_grasshopper_setup(EVP_CIPHER * cipher, uint8_t mode, int iv_size, bool padding)
{
	return EVP_CIPHER_meth_set_iv_length(cipher, iv_size) && EVP_CIPHER_meth_set_flags(cipher,
		   (unsigned long)(mode | ((!padding) ? EVP_CIPH_NO_PADDING : 0) | ((iv_size > 0) ? EVP_CIPH_CUSTOM_IV : 0) |
...
{
	struct GRASSHOPPER_CIPHER_PARAMS * params;
	EVP_CIPHER ** cipher = &gost_grasshopper_ciphers[num];
	if(*cipher == NULL) {
		grasshopper_init_cipher_func init_cipher;
		...
		*cipher = cipher_gost_grasshopper_create(nid, block_size);
		...
		if(!cipher_gost_grasshopper_setup(*cipher, mode, iv_size, padding) || !EVP_CIPHER_meth_set_init(*cipher, init_cipher) || !EVP_CIPHER_meth_set_impl_ctx_size(*cipher, ctx_size)) {
