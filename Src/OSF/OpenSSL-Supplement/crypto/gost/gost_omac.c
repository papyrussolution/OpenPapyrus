...
{
	OMAC_CTX * c = static_cast<OMAC_CTX *>(EVP_MD_CTX_md_data(ctx));
	memzero(c, sizeof(OMAC_CTX));
...
{
	OMAC_CTX * c_to = static_cast<OMAC_CTX *>(EVP_MD_CTX_md_data(to));
	const OMAC_CTX * c_from = static_cast<const OMAC_CTX *>(EVP_MD_CTX_md_data(from));
	if(c_from && c_to) {
...
int omac_imit_cleanup(EVP_MD_CTX * ctx)
{
	...
	if(c) {
		...
		memzero(EVP_MD_CTX_md_data(ctx), sizeof(OMAC_CTX));
	}
	return 1;
}
	...
	switch(type) {
		...
		case EVP_MD_CTRL_SET_KEY:
	    {
		    ...
		    if(arg == 0) {
			    ...
			    ret = omac_key(c, cipher, static_cast<const uchar *>(ptr), 32);
			    if(ret > 0)
				    memcpy(c->key, ptr, 32);
			    return ret;
		    }
		...
		case EVP_MD_CTRL_TLSTREE:
	    {
		    ...
		    if(c->key_set) {
			    ...
			    return gost_tlstree(c->cipher_nid, c->key, diversed_key, (const unsigned char*)ptr) ? omac_key(c, EVP_get_cipherbynid(c->cipher_nid), diversed_key, 32) : 0;
		    }
		    GOSTerr(GOST_F_OMAC_IMIT_CTRL, GOST_R_BAD_ORDER);
		    return 0;
	    }
