...
int hash_file(gost_hash_ctx * ctx, char * filename, char * sum, int mode, int hashsize);
...
int main(int argc, char ** argv)
{
	...
	if(check_file) {
		char inhash[MAX_HASH_TXT_BYTES + 1], calcsum[MAX_HASH_TXT_BYTES + 1], filename[MAXPATH];
		...
		while(get_line(check_file, inhash, filename, verbose, &expected_hash_size)) {
			int error = 0;
			...
			if(!hash_file(&ctx, filename, calcsum, open_mode, expected_hash_size)) {
				errors++;
				error = 1;
			}
			count++;
		...
		if(errors) {
			fprintf(stderr, "%s: WARNING %d of %d file(s) cannot be processed\n", argv[0], errors, count);
		}
		if(failcount) {
			fprintf(stderr, "%s: WARNING %d of %d processed file(s) failed GOST hash sum check\n", argv[0], failcount, count - errors);
		}
		exit((failcount || errors) ? 1 : 0);
	}
	else if(filenames_from_stdin) {
		char sum[MAX_HASH_TXT_BYTES + 1];
		char filename[MAXPATH + 1], * end;
		while(!feof(stdin)) {
			if(!fgets(filename, MAXPATH, stdin))
				break;
