...
static int pkey_gost_init(EVP_PKEY_CTX * ctx)
{
	...
	struct gost_pmeth_data * data = static_cast<struct gost_pmeth_data *>(OPENSSL_malloc(sizeof(*data)));
	if(!data)
		return 0;
	...
	if(pkey && EVP_PKEY_get0(pkey)) {
		switch(EVP_PKEY_base_id(pkey)) {
		    ...
		    {
			    const EC_GROUP * group = EC_KEY_get0_group(static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)pkey)));
			    if(group != NULL) {
...
static int pkey_gost_copy(EVP_PKEY_CTX * dst, ossl3_const EVP_PKEY_CTX * src)
{
	...
	src_data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(src));
	dst_data = static_cast<struct gost_pmeth_data *>(EVP_PKEY_CTX_get_data(dst));
...
static int pkey_gost_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_SET_IV:
		    OPENSSL_assert(p2 != NULL);
		    pctx->shared_ukm = static_cast<uchar *>(OPENSSL_malloc((int)p1));
...
static int pkey_gost_ec_ctrl_str_256(EVP_PKEY_CTX * ctx, const char * type, const char * value)
{
	int param_nid = 0;
	if(strcmp(type, param_ctrl_string) == 0) {
		...
		if(strlen(value) == 1) {
			switch(toupper((uchar)value[0])) {
				case 'A': param_nid = NID_id_GostR3410_2001_CryptoPro_A_ParamSet; break;
				case 'B': param_nid = NID_id_GostR3410_2001_CryptoPro_B_ParamSet; break;
				case 'C': param_nid = NID_id_GostR3410_2001_CryptoPro_C_ParamSet; break;
				case '0': param_nid = NID_id_GostR3410_2001_TestParamSet; break;
				default: return 0;
			}
		}
		else if((strlen(value) == 2) && (toupper((uchar)value[0]) == 'X')) {
			switch(toupper((uchar)value[1])) {
				case 'A': param_nid = NID_id_GostR3410_2001_CryptoPro_XchA_ParamSet; break;
				case 'B': param_nid = NID_id_GostR3410_2001_CryptoPro_XchB_ParamSet; break;
				default: return 0;
			}
		}
		else if((strlen(value) == 3) && (toupper((uchar)value[0]) == 'T') && (toupper((uchar)value[1]) == 'C')) {
			switch(toupper((uchar)value[2])) {
				case 'A': param_nid = NID_id_tc26_gost_3410_2012_256_paramSetA; break;
				case 'B': param_nid = NID_id_tc26_gost_3410_2012_256_paramSetB; break;
				case 'C': param_nid = NID_id_tc26_gost_3410_2012_256_paramSetC; break;
				case 'D': param_nid = NID_id_tc26_gost_3410_2012_256_paramSetD; break;
				default: return 0;
			}
		}
		else {
    ...
    const char * type, const char * value)
{
	...
	if(strlen(value) == 1) {
		switch(toupper((uchar)value[0])) {
			case 'A': param_nid = NID_id_tc26_gost_3410_2012_512_paramSetA; break;
			case 'B': param_nid = NID_id_tc26_gost_3410_2012_512_paramSetB; break;
			case 'C': param_nid = NID_id_tc26_gost_3410_2012_512_paramSetC; break;
			default: return 0;
		}
	}
	else {
...
static int pkey_gost2001cp_keygen(EVP_PKEY_CTX * ctx, EVP_PKEY * pkey)
{
	if(!pkey_gost2001_paramgen(ctx, pkey))
		...
		EC_KEY * ec = static_cast<EC_KEY *>(EVP_PKEY_get0(pkey));
		gost_ec_keygen(ec);
		return 1;
	}
}
...
static int pkey_gost2012cp_keygen(EVP_PKEY_CTX * ctx, EVP_PKEY * pkey)
{
	if(!pkey_gost2012_paramgen(ctx, pkey))
		...
		gost_ec_keygen(static_cast<EC_KEY *>(EVP_PKEY_get0(pkey)));
		return 1;
	}
}
/* ----------- sign callbacks --------------------------------------*/
...
static int pkey_gost_ec_cp_sign(EVP_PKEY_CTX * ctx, unsigned char * sig, size_t * siglen, const unsigned char * tbs, size_t tbs_len)
{
	...
	switch(EVP_PKEY_base_id(pkey)) {
		case NID_id_GostR3410_2001:
		case NID_id_GostR3410_2012_256: order = 64; break;
		case NID_id_GostR3410_2012_512: order = 128; break;
		default: return 0;
	}
	if(!sig) {
		*siglen = order;
	...
	unpacked_sig = gost_ec_sign(tbs, tbs_len, static_cast<EC_KEY *>(EVP_PKEY_get0(pkey)));
	if(!unpacked_sig) {
		return 0;
	}
...
ECDSA_SIG * unpack_cp_signature(const unsigned char * sigbuf, size_t siglen)
{
	...
	if(!sig) {
		...
		BIGNUM * s = BN_bin2bn(sigbuf, siglen / 2, NULL);
		BIGNUM * r = BN_bin2bn(sigbuf + siglen / 2, siglen / 2, NULL);
		ECDSA_SIG_set0(sig, r, s);
	}
	return sig;
}

static int pkey_gost_ec_cp_verify(EVP_PKEY_CTX * ctx, const unsigned char * sig, size_t siglen, const unsigned char * tbs, size_t tbs_len)
{
	int ok = 0;
...
#endif
	if(pub_key)
		ok = gost_ec_verify(tbs, tbs_len, s, static_cast<EC_KEY *>(EVP_PKEY_get0(pub_key)));
	ECDSA_SIG_free(s);
	return ok;
}
...
{
	struct gost_mac_pmeth_data * data = static_cast<struct gost_mac_pmeth_data *>(OPENSSL_malloc(sizeof(*data)));
	EVP_PKEY * pkey = EVP_PKEY_CTX_get0_pkey(ctx);
	if(!data)
		return 0;
	memzero(data, sizeof(*data));
	...
	if(pkey) {
		struct gost_mac_key * key = static_cast<struct gost_mac_key *>(EVP_PKEY_get0(pkey));
		if(key) {
...
static int pkey_gost_omac_init(EVP_PKEY_CTX * ctx, size_t mac_size)
{
	...
	if(!data)
		return 0;
	memzero(data, sizeof(*data));
	data->mac_size = static_cast<short>(mac_size);
	data->mac_param_nid = NID_undef;
	if(pkey) {
		struct gost_mac_key * key = static_cast<struct gost_mac_key *>(EVP_PKEY_get0(pkey));
		if(key) {
...
{
	struct gost_mac_pmeth_data * data = static_cast<struct gost_mac_pmeth_data *>(EVP_PKEY_CTX_get_data(ctx));
	if(data)
		OPENSSL_free(data);
}
...
static int pkey_gost_mac_copy(EVP_PKEY_CTX * dst, ossl3_const EVP_PKEY_CTX * src)
{
	...
	src_data = static_cast<struct gost_mac_pmeth_data *>(EVP_PKEY_CTX_get_data(src));
	dst_data = static_cast<struct gost_mac_pmeth_data *>(EVP_PKEY_CTX_get_data(dst));
	if(!src_data || !dst_data)
		return 0;
	*dst_data = *src_data;
	return 1;
...
static int pkey_gost_mac_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_GOST_PARAMSET:
	    {
		    struct gost_cipher_info * param = static_cast<struct gost_cipher_info *>(p2);
		    data->mac_param_nid = param->nid;
		    return 1;
	    }
		case EVP_PKEY_CTRL_DIGESTINIT:
	    {
		    EVP_MD_CTX * mctx = static_cast<EVP_MD_CTX *>(p2);
		    ...
		    if(!data->key_set) {
			    ...
			    key = static_cast<struct gost_mac_key *>(EVP_PKEY_get0(pkey));
			    if(!key) {
				    GOSTerr(GOST_F_PKEY_GOST_MAC_CTRL, GOST_R_MAC_KEY_NOT_SET);
				    return 0;
			    }
			    return EVP_MD_meth_get_ctrl(EVP_MD_CTX_md(mctx))(mctx, EVP_MD_CTRL_SET_KEY, 0, key);
		    }
		    else {
			    return EVP_MD_meth_get_ctrl(EVP_MD_CTX_md(mctx))(mctx, EVP_MD_CTRL_SET_KEY, 32, &(data->key));
		    }
	    }
...
static int pkey_gost_omac_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2, size_t max_size)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_MAC_LEN:
	    {
		    if(p1 < 1 || p1 > static_cast<int>(max_size)) {
...
static int pkey_gost_magma_mac_ctrl_str(EVP_PKEY_CTX * ctx, const char * type, const char * value)
{
	return pkey_gost_omac_ctrl_str(ctx, type, value, 8);
}

static int pkey_gost_grasshopper_mac_ctrl_str(EVP_PKEY_CTX * ctx, const char * type, const char * value)
{
	return pkey_gost_omac_ctrl_str(ctx, type, value, 8);
}

static int pkey_gost_mac_keygen_base(EVP_PKEY_CTX * ctx, EVP_PKEY * pkey, int mac_nid)
{
	struct gost_mac_pmeth_data * data = static_cast<struct gost_mac_pmeth_data *>(EVP_PKEY_CTX_get_data(ctx));
	...
	keydata = static_cast<struct gost_mac_key *>(OPENSSL_malloc(sizeof(struct gost_mac_key)));
	if(keydata == NULL)
		return 0;
...
static int pkey_gost_mac_signctx_init(EVP_PKEY_CTX * ctx, EVP_MD_CTX * mctx)
{
	...
	data = static_cast<struct gost_mac_pmeth_data *>(EVP_PKEY_CTX_get_data(ctx));
	if(!data) {
...
static int pkey_gost_mac_signctx(EVP_PKEY_CTX * ctx, unsigned char * sig, size_t * siglen, EVP_MD_CTX * mctx)
{
	...
	if(!siglen)
		return 0;
	tmpsiglen = *siglen;    /* for platforms where sizeof(int) != sizeof(size_t) */
	if(!sig) {
		*siglen = data->mac_size;
		return 1;
	}
	EVP_MD_meth_get_ctrl(EVP_MD_CTX_md(mctx))(mctx, EVP_MD_CTRL_XOF_LEN, data->mac_size, NULL);
...
{
	return EC_KEY_check_key(static_cast<EC_KEY *>(EVP_PKEY_get0(pkey)));
}
	...
	*pmeth = EVP_PKEY_meth_new(id, flags);
	...
	switch(id) {
		case NID_id_GostR3410_2001:
		    ...
		    EVP_PKEY_meth_set_encrypt(*pmeth, pkey_gost_encrypt_init, pkey_gost_encrypt);
