...
static int gost_cipher_init_cbc(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc);
static int gost_cipher_init_cpa(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc);
static int gost_cipher_init_cp_12(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc);
...
static int gost_cipher_do_cbc(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl);
...
static int magma_cipher_init(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc);
/* Handles block of data in CBC mode */
static int magma_cipher_do_cbc(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl);
static int magma_cipher_do_ctr(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl);

static EVP_CIPHER * _hidden_Gost28147_89_cipher = NULL;
const EVP_CIPHER * cipher_gost(void)
{
	if(_hidden_Gost28147_89_cipher == NULL && ((_hidden_Gost28147_89_cipher = EVP_CIPHER_meth_new(NID_id_Gost28147_89, 1 /* block_size */,
	    32 /* key_size */)) == NULL
	    ...
	    gost_cipher_cleanup)
	    || !EVP_CIPHER_meth_set_impl_ctx_size(_hidden_Gost28147_89_cipher, sizeof(struct ossl_gost_cipher_ctx)) ||
	    !EVP_CIPHER_meth_set_set_asn1_params(_hidden_Gost28147_89_cipher, gost89_set_asn1_parameters)
	    ||
	    !EVP_CIPHER_meth_set_get_asn1_params(_hidden_Gost28147_89_cipher, gost89_get_asn1_parameters)
	    || !EVP_CIPHER_meth_set_ctrl(_hidden_Gost28147_89_cipher, gost_cipher_ctl))) {
...
{
	if(_hidden_Gost28147_89_cbc == NULL && ((_hidden_Gost28147_89_cbc = EVP_CIPHER_meth_new(NID_gost89_cbc, 8 /* block_size */, 32 /* key_size */)) == NULL
	    || !EVP_CIPHER_meth_set_iv_length(_hidden_Gost28147_89_cbc, 8) || !EVP_CIPHER_meth_set_flags(_hidden_Gost28147_89_cbc,
	    EVP_CIPH_CBC_MODE|EVP_CIPH_CUSTOM_IV|EVP_CIPH_RAND_KEY|EVP_CIPH_ALWAYS_CALL_INIT)
	    || !EVP_CIPHER_meth_set_init(_hidden_Gost28147_89_cbc, gost_cipher_init_cbc)
	    || !EVP_CIPHER_meth_set_do_cipher(_hidden_Gost28147_89_cbc, gost_cipher_do_cbc)
	    || !EVP_CIPHER_meth_set_cleanup(_hidden_Gost28147_89_cbc, gost_cipher_cleanup)
	    || !EVP_CIPHER_meth_set_impl_ctx_size(_hidden_Gost28147_89_cbc, sizeof(struct ossl_gost_cipher_ctx))
	    || !EVP_CIPHER_meth_set_set_asn1_params(_hidden_Gost28147_89_cbc, gost89_set_asn1_parameters)
	    || !EVP_CIPHER_meth_set_get_asn1_params(_hidden_Gost28147_89_cbc, gost89_get_asn1_parameters)
	    || !EVP_CIPHER_meth_set_ctrl(_hidden_Gost28147_89_cbc, gost_cipher_ctl))) {
...
{
	if(_hidden_gost89_cnt_12 == NULL && ((_hidden_gost89_cnt_12 = EVP_CIPHER_meth_new(NID_gost89_cnt_12, 1 /* block_size */, 32 /* key_size */)) == NULL
	    ...
	    || !EVP_CIPHER_meth_set_init(_hidden_gost89_cnt_12, gost_cipher_init_cp_12)
	    || !EVP_CIPHER_meth_set_do_cipher(_hidden_gost89_cnt_12, gost_cipher_do_cnt)
	    || !EVP_CIPHER_meth_set_cleanup(_hidden_gost89_cnt_12, gost_cipher_cleanup)
	    || !EVP_CIPHER_meth_set_impl_ctx_size(_hidden_gost89_cnt_12, sizeof(struct ossl_gost_cipher_ctx))
	    || !EVP_CIPHER_meth_set_set_asn1_params(_hidden_gost89_cnt_12, gost89_set_asn1_parameters)
	    || !EVP_CIPHER_meth_set_get_asn1_params(_hidden_gost89_cnt_12, gost89_get_asn1_parameters)
	    || !EVP_CIPHER_meth_set_ctrl(_hidden_gost89_cnt_12, gost_cipher_ctl))) {
...
{
	if(_hidden_magma_ctr == NULL && ((_hidden_magma_ctr = EVP_CIPHER_meth_new(NID_magma_ctr, 1 /* block_size */,32 /* key_size */)) == NULL
	    || !EVP_CIPHER_meth_set_iv_length(_hidden_magma_ctr, 8) || !EVP_CIPHER_meth_set_flags(_hidden_magma_ctr,
	    EVP_CIPH_CTR_MODE |EVP_CIPH_NO_PADDING |EVP_CIPH_CUSTOM_IV |EVP_CIPH_RAND_KEY |EVP_CIPH_ALWAYS_CALL_INIT)
	    || !EVP_CIPHER_meth_set_init(_hidden_magma_ctr, magma_cipher_init) || !EVP_CIPHER_meth_set_do_cipher(_hidden_magma_ctr, magma_cipher_do_ctr)
	    || !EVP_CIPHER_meth_set_cleanup(_hidden_magma_ctr, gost_cipher_cleanup)
	    || !EVP_CIPHER_meth_set_impl_ctx_size(_hidden_magma_ctr, sizeof(struct ossl_gost_cipher_ctx))
	    || !EVP_CIPHER_meth_set_set_asn1_params(_hidden_magma_ctr, gost89_set_asn1_parameters)
	    || !EVP_CIPHER_meth_set_get_asn1_params(_hidden_magma_ctr, gost89_get_asn1_parameters)
...
{
	if(_hidden_magma_cbc == NULL && ((_hidden_magma_cbc =
...
const struct gost_cipher_info * get_encryption_params(ASN1_OBJECT * obj)
{
	...
	if(!obj) {
		...
		if(!params || !strlen(params)) {
			for(int i = 0; gost_cipher_list[i].nid != NID_undef; i++)
...
{
	const struct gost_cipher_info * param = get_encryption_params((nid == NID_undef ? NULL : OBJ_nid2obj(nid)));
	if(!param)
		return 0;
...
static int gost_cipher_init_param(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc, int paramNID, int mode)
{
	struct ossl_gost_cipher_ctx * c = static_cast<struct ossl_gost_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
static int magma_cipher_init_param(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc, int paramNID, int mode)
{
	struct ossl_gost_cipher_ctx * c = static_cast<struct ossl_gost_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
	if(EVP_CIPHER_CTX_get_app_data(ctx) == NULL) {
...
static int gost_cipher_init_cnt(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, gost_subst_block * block)
{
	struct ossl_gost_cipher_ctx * c = static_cast<struct ossl_gost_cipher_ctx *>(EVP_CIPHER_CTX_get_cipher_data(ctx));
...
/* Initializes EVP_CIPHER_CTX with default values */
int gost_cipher_init_cbc(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	return gost_cipher_init_param(ctx, key, iv, enc, NID_undef, EVP_CIPH_CBC_MODE);
}

/* Initializes EVP_CIPHER_CTX with default values */
int magma_cipher_init(EVP_CIPHER_CTX * ctx, const unsigned char * key, const unsigned char * iv, int enc)
{
	return magma_cipher_init_param(ctx, key, iv, enc, NID_undef, EVP_CIPH_CBC_MODE);
}
/*
...
{
	struct ossl_gost_cipher_ctx * c = static_cast<struct ossl_gost_cipher_ctx *>(ctx);
...
static void gost_cnt_next(void * ctx, unsigned char * iv, unsigned char * buf)
{
	...
	g += 0x01010101;
	buf1[0] = (uchar)(g & 0xff);
	buf1[1] = (uchar)((g >> 8) & 0xff);
	buf1[2] = (uchar)((g >> 16) & 0xff);
	buf1[3] = (uchar)((g >> 24) & 0xff);
	...
	if(go > g)              /* overflow */
		g++;
	buf1[4] = (uchar)(g & 0xff);
	buf1[5] = (uchar)((g >> 8) & 0xff);
	buf1[6] = (uchar)((g >> 16) & 0xff);
	buf1[7] = (uchar)((g >> 24) & 0xff);
	memcpy(iv, buf1, 8);
...
int gost_cipher_do_cbc(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	unsigned char b[8];
...
int magma_cipher_do_cbc(EVP_CIPHER_CTX * ctx, unsigned char * out, const unsigned char * in, size_t inl)
{
	unsigned char b[8];
	unsigned char d[8];
	const unsigned char * in_ptr = in;
	...
	switch(type) {
		...
		case EVP_CTRL_SET_SBOX:
		    if(ptr) {
			    ...
			    nid = OBJ_txt2nid(static_cast<const char *>(ptr));
			    if(nid == NID_undef) {
				    return 0;
			    }
...
int gost89_set_asn1_parameters(EVP_CIPHER_CTX * ctx, ASN1_TYPE * params)
{
	...
	p = buf = static_cast<unsigned char *>(OPENSSL_malloc(len));
	if(!buf) {
		GOST_CIPHER_PARAMS_free(gcp);
		GOSTerr(GOST_F_GOST89_SET_ASN1_PARAMETERS, ERR_R_MALLOC_FAILURE);
		return 0;
...
int gost89_get_asn1_parameters(EVP_CIPHER_CTX * ctx, ASN1_TYPE * params)
{
	...
	memcpy((uchar *)EVP_CIPHER_CTX_original_iv(ctx), gcp->iv->data, EVP_CIPHER_CTX_iv_length(ctx));
	GOST_CIPHER_PARAMS_free(gcp);
	return 1;
}
...
{
	struct ossl_gost_imit_ctx * c = static_cast<struct ossl_gost_imit_ctx *>(EVP_MD_CTX_md_data(ctx));
	memzero(c->buffer, sizeof(c->buffer));
	memzero(c->partial_block, sizeof(c->partial_block));
	c->count = 0;
	c->bytes_left = 0;
	c->key_meshing = 1;
	c->dgst_size = 4;
...
int gost_imit_ctrl(EVP_MD_CTX * ctx, int type, int arg, void * ptr)
{
	switch(type) {
	    ...
	    {
		    struct ossl_gost_imit_ctx * gost_imit_ctx = static_cast<struct ossl_gost_imit_ctx *>(EVP_MD_CTX_md_data(ctx));
		    ...
		    if(arg == 0) {
			    ...
			    gost_key(&(gost_imit_ctx->cctx), static_cast<const byte *>(ptr));
			    gost_imit_ctx->key_set = 1;
			    return 1;
		    }
...
int gost_imit_copy(EVP_MD_CTX * to, const EVP_MD_CTX * from)
{
	if(EVP_MD_CTX_md_data(to) && EVP_MD_CTX_md_data(from)) {
		memcpy(EVP_MD_CTX_md_data(to), EVP_MD_CTX_md_data(from), sizeof(struct ossl_gost_imit_ctx));
	}
	return 1;
}
...
{
	memzero(EVP_MD_CTX_md_data(ctx), sizeof(struct ossl_gost_imit_ctx));
	return 1;
}
