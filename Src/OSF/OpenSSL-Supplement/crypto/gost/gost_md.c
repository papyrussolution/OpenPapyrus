...
{
	struct ossl_gost_digest_ctx * c = static_cast<struct ossl_gost_digest_ctx *>(EVP_MD_CTX_md_data(ctx));
	memzero(&(c->dctx), sizeof(gost_hash_ctx));
...
{
	return hash_block((gost_hash_ctx*)EVP_MD_CTX_md_data(ctx), static_cast<const byte *>(data), count);
}
...
{
	struct ossl_gost_digest_ctx * md_ctx = static_cast<struct ossl_gost_digest_ctx *>(EVP_MD_CTX_md_data(to));
	if(EVP_MD_CTX_md_data(to) && EVP_MD_CTX_md_data(from)) {
		memcpy(EVP_MD_CTX_md_data(to), EVP_MD_CTX_md_data(from), sizeof(struct ossl_gost_digest_ctx));
...
int gost_digest_cleanup(EVP_MD_CTX * ctx)
{
	if(EVP_MD_CTX_md_data(ctx))
		memzero(EVP_MD_CTX_md_data(ctx), sizeof(struct ossl_gost_digest_ctx));
	return 1;
}
