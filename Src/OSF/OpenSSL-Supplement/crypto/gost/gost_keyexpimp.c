...
static int Stub__EVP_DigestFinalXOF(EVP_MD_CTX *ctx, unsigned char *md, size_t size) // @sobolev @v10.6.3
{
	int ret = 0;
	/*
	OSSL_PARAM params[2];
 ...
 * */
int gost_kexp15(const unsigned char * shared_key, const int shared_len, int cipher_nid, const unsigned char * cipher_key,
    int mac_nid, unsigned char * mac_key, const unsigned char * iv, const size_t ivlen, unsigned char * out, int * out_len)
{
	...
	int ret = 0;
	int len;
	mac_len = (cipher_nid == NID_magma_ctr) ? 8 : (cipher_nid == NID_grasshopper_ctr) ? 16 : 0;
	if(mac_len == 0) {
	...
	if(EVP_DigestInit_ex(mac, EVP_get_digestbynid(mac_nid), NULL) <= 0
	    ...
	    || Stub__EVP_DigestFinalXOF(mac, mac_buf, mac_len) <= 0) {
 ...
 * */
int gost_kimp15(const unsigned char * expkey, const size_t expkeylen, int cipher_nid, const unsigned char * cipher_key,
    int mac_nid, unsigned char * mac_key, const unsigned char * iv, const size_t ivlen, unsigned char * shared_key)
{
	...
	if(EVP_CipherInit_ex (ciph, EVP_get_cipherbynid(cipher_nid), NULL, NULL, NULL, 0) <= 0
...
int gost_kdftree2012_256(unsigned char * keyout, size_t keyout_len, const unsigned char * key, size_t keylen,
    const unsigned char * label, size_t label_len, const unsigned char * seed, size_t seed_len, const size_t representation)
{
	int iters, i = 0;
	...
	for(i = 1; i <= iters; i++) {
		...
		unsigned char * rep_ptr = ((uchar *)&iter_net) + (4 - representation);
		if(HMAC_Init_ex(ctx, key, keylen, EVP_get_digestbynid(NID_id_GostR3411_2012_256), NULL) <= 0
...
int gost_tlstree(int cipher_nid, const unsigned char * in, unsigned char * out, const unsigned char * tlsseq)
{
#ifndef L_ENDIAN
	uint64_t gh_c1 = 0xFFFFFFFF00000000, gh_c2 = 0xFFFFFFFFFFF80000, gh_c3 = 0xFFFFFFFFFFFFFFC0;
	uint64_t mg_c1 = 0xFFFFFFC000000000, mg_c2 = 0xFFFFFFFFFE000000, mg_c3 = 0xFFFFFFFFFFFFF000;
#else
	uint64_t gh_c1 = 0x00000000FFFFFFFF, gh_c2 = 0x0000F8FFFFFFFFFF, gh_c3 = 0xC0FFFFFFFFFFFFFF;
	uint64_t mg_c1 = 0x00000000C0FFFFFF, mg_c2 = 0x000000FEFFFFFFFF, mg_c3 = 0x00F0FFFFFFFFFFFF;
#endif
	uint64_t c1, c2, c3;
	...
	seed2 = seq & c2;
	seed3 = seq & c3;
	if(gost_kdftree2012_256(ko1, 32, in, 32, (const uchar*)"level1", 6, (const uchar*)&seed1, 8, 1) <= 0
	    || gost_kdftree2012_256(ko2, 32, ko1, 32, (const uchar*)"level2", 6, (const uchar*)&seed2, 8, 1) <= 0
	    || gost_kdftree2012_256(out, 32, ko2, 32, (const uchar*)"level3", 6, (const uchar*)&seed3, 8, 1) <= 0)
		return 0;
