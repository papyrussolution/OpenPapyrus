...
/*
 * Pack bignum into byte buffer of given size, filling all leading bytes by zeros
 */
...
static int pkey_bits_gost(const EVP_PKEY * pk)
{
	if(pk) {
		switch(EVP_PKEY_base_id(pk)) {
			...
			case NID_id_GostR3410_2012_256: return 256;
			case NID_id_GostR3410_2012_512: return 512;
		}
	}
	return -1;
}
...
static ASN1_STRING * encode_gost_algor_params(const EVP_PKEY * key)
{
	...
	switch(EVP_PKEY_base_id(key)) {
		case NID_id_GostR3410_2012_256:
		    pkey_param_nid = EC_GROUP_get_curve_name(EC_KEY_get0_group(static_cast<EC_KEY *>(key_ptr)));
		    switch(pkey_param_nid) {
			    ...
			    case NID_id_GostR3410_2001_CryptoPro_XchB_ParamSet: gkp->hash_params = OBJ_nid2obj(NID_id_GostR3411_2012_256);
		    }
		    break;
		case NID_id_GostR3410_2012_512:
		    ...
		    switch(pkey_param_nid) {
			    ...
			    case NID_id_tc26_gost_3410_2012_512_paramSetB: gkp->hash_params = OBJ_nid2obj(NID_id_GostR3411_2012_512);
		    }
		    break;
		case NID_id_GostR3410_2001:
		    pkey_param_nid = EC_GROUP_get_curve_name(EC_KEY_get0_group(static_cast<EC_KEY *>(key_ptr)));
		...
		case NID_id_GostR3410_2001:
		    if(!key_ptr) {
			    ...
			    if(!EVP_PKEY_assign(pkey, pkey_nid, key_ptr)) {
				    EC_KEY_free(static_cast<EC_KEY *>(key_ptr));
				    break;
			    }
		    }
		    return fill_GOST_EC_params(static_cast<EC_KEY *>(key_ptr), param_nid);
	}
...
const BIGNUM * gost_get0_priv_key(const EVP_PKEY * pkey)
{
	switch(EVP_PKEY_base_id(pkey)) {
		case NID_id_GostR3410_2012_512:
		case NID_id_GostR3410_2012_256:
		case NID_id_GostR3410_2001:
	    {
		    EC_KEY * ec = static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)pkey));
		    if(ec)
			    return EC_KEY_get0_private_key(ec);
		    break;
	    }
	}
	return NULL;
...
static int pkey_ctrl_gost(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	...
	switch(nid) {
		case NID_id_GostR3410_2012_512: md_nid = NID_id_GostR3411_2012_512; break;
		case NID_id_GostR3410_2012_256: md_nid = NID_id_GostR3411_2012_256; break;
		case NID_id_GostR3410_2001:
		case NID_id_GostR3410_94: md_nid = NID_id_GostR3411_94; break;
		default: return -1;
	}
	switch(op) {
		...
		case ASN1_PKEY_CTRL_PKCS7_ENCRYPT:
		    if(arg1 == 0) {
			    ...
			    PKCS7_RECIP_INFO_get0_alg(static_cast<PKCS7_RECIP_INFO *>(arg2), &alg1);
		...
		case ASN1_PKEY_CTRL_CMS_ENVELOPE:
		    if(arg1 == 0) {
			    ...
			    CMS_RecipientInfo_ktri_get0_algs(static_cast<CMS_RecipientInfo *>(arg2), NULL, NULL, &alg1);
			    X509_ALGOR_set0(alg1, OBJ_nid2obj(EVP_PKEY_id(pkey)),
				V_ASN1_SEQUENCE, params);
		    }
		    return 1;
...
static BIGNUM * unmask_priv_key(EVP_PKEY * pk, const unsigned char * buf, int len, int num_masks)
{
	BIGNUM * q = NULL;
	const EC_KEY * key_ptr = (pk) ? static_cast<EC_KEY *>(EVP_PKEY_get0(pk)) : NULL;
	const EC_GROUP * group = (key_ptr) ? EC_KEY_get0_group(key_ptr) : NULL;
	BIGNUM * pknum_masked = hashsum2bn(buf, len);
	if(!pknum_masked)
		return NULL;
	if(num_masks > 0) {
		/*
...
static int priv_decode_gost(EVP_PKEY * pk, const PKCS8_PRIV_KEY_INFO * p8inf)
{
	...
	if(priv_len % expected_key_len == 0) {
		...
		pk_num = unmask_priv_key(pk, pkey_buf, expected_key_len, priv_len / expected_key_len - 1);
	}
		...
		pk_num = unmask_priv_key(pk, mgk->masked_priv_key->data, expected_key_len, priv_len / expected_key_len - 1);
		MASKED_GOST_KEY_free(mgk);
	}
	else {
		GOSTerr(GOST_F_PRIV_DECODE_GOST, EVP_R_DECODE_ERROR);
...
static int priv_encode_gost(PKCS8_PRIV_KEY_INFO * p8, const EVP_PKEY * pk)
{
	...
	if(key_len == 0 || !(buf = static_cast<uchar *>(OPENSSL_malloc(key_len)))) {
		return 0;
	}
	...
	return PKCS8_pkey_set0(p8, algobj, 0, V_ASN1_SEQUENCE, params, buf, key_len);
}
...
static int print_gost_priv(BIO * out, const EVP_PKEY * pkey, int indent)
{
	...
	{
		const BIGNUM * key = gost_get0_priv_key(pkey);
		if(!key)
...
static int print_gost_ec(BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx, int type)
{
	if(type == 2) {
...
static int param_print_gost_ec(BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
	return print_gost_ec(out, pkey, indent, pctx, 0);
}

static int pub_print_gost_ec(BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
	return print_gost_ec(out, pkey, indent, pctx, 1);
}

static int priv_print_gost_ec(BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
...
{
	const EC_KEY * ec = static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)pk));
	if(!ec)
		return 1;
...
{
	EC_KEY * eto = static_cast<EC_KEY *>(EVP_PKEY_get0(to));
	const EC_KEY * efrom = static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)from));
...
static int param_cmp_gost_ec(const EVP_PKEY * a, const EVP_PKEY * b)
{
	...
	EC_KEY * ec_a = static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)a));
	EC_KEY * ec_b = static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)b));
	if(!ec_a || !ec_b)
		return 0;
...
static int pub_decode_gost_ec(EVP_PKEY * pk, X509_PUBKEY * pub)
{
	...
	if(!decode_gost_algor_params(pk, palg))
		return 0;
	group = EC_KEY_get0_group(static_cast<EC_KEY *>(EVP_PKEY_get0(pk)));
	octet = d2i_ASN1_OCTET_STRING(NULL, &pubkey_buf, pub_len);
	if(!octet) {
		GOSTerr(GOST_F_PUB_DECODE_GOST_EC, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	databuf = static_cast<uchar *>(OPENSSL_malloc(octet->length));
	if(databuf == NULL) {
		GOSTerr(GOST_F_PUB_DECODE_GOST_EC, ERR_R_MALLOC_FAILURE);
	...
	BN_free(X);
	BN_free(Y);
	if(!EC_KEY_set_public_key(static_cast<EC_KEY *>(EVP_PKEY_get0(pk)), pub_key)) {
		GOSTerr(GOST_F_PUB_DECODE_GOST_EC, ERR_R_EC_LIB);
		EC_POINT_free(pub_key);
		return 0;
	}
...
static int pub_encode_gost_ec(X509_PUBKEY * pub, const EVP_PKEY * pk)
{
	...
	const EC_KEY * ec = static_cast<const EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)pk));
	...
	databuf = static_cast<uchar *>(OPENSSL_zalloc(data_len));
	if(databuf == NULL) {
		GOSTerr(GOST_F_PUB_ENCODE_GOST_EC, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
{
	const EC_KEY * ea = static_cast<const EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)a));
	const EC_KEY * eb = static_cast<const EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)b));
...
static int pkey_size_gost(const EVP_PKEY * pk)
{
	if(pk) {
		switch(EVP_PKEY_base_id(pk)) {
			...
			case NID_id_GostR3410_2001:
			case NID_id_GostR3410_2012_256: return 64;
			case NID_id_GostR3410_2012_512: return 128;
		}
	}
	return -1;
}
...
static int mac_ctrl_gost(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
		    if(arg2) {
			    *static_cast<int *>(arg2) = NID_id_Gost28147_89_MAC;
			    return 2;
		    }
	}
	return -2;
...
static int mac_ctrl_gost_12(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
		    if(arg2) {
			    *static_cast<int *>(arg2) = NID_gost_mac_12;
			    return 2;
		    }
	}
	return -2;
...
static int mac_ctrl_magma(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
		    if(arg2) {
			    *static_cast<int *>(arg2) = NID_magma_mac;
			    return 2;
		    }
	}
	return -2;
...
static int mac_ctrl_grasshopper(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
		    if(arg2) {
			    *static_cast<int *>(arg2) = NID_grasshopper_mac;
			    return 2;
		    }
	}
	return -2;
...
{
	int nid = EC_GROUP_get_curve_name(EC_KEY_get0_group(static_cast<EC_KEY *>(EVP_PKEY_get0((EVP_PKEY*)pkey))));
	...
	*ameth = EVP_PKEY_asn1_new(nid, ASN1_PKEY_SIGPARAM_NULL, pemstr, info);
	...
	switch(nid) {
		case NID_id_GostR3410_2001:
		    ...
		    EVP_PKEY_asn1_set_private(*ameth, priv_decode_gost, priv_encode_gost, priv_print_gost_ec);
		    EVP_PKEY_asn1_set_param(*ameth, gost2001_param_decode, gost2001_param_encode, param_missing_gost_ec, param_copy_gost_ec, param_cmp_gost_ec, param_print_gost_ec);
		    EVP_PKEY_asn1_set_public(*ameth, pub_decode_gost_ec, pub_encode_gost_ec, pub_cmp_gost_ec, pub_print_gost_ec, pkey_size_gost, pkey_bits_gost);
		...
		case NID_id_GostR3410_2012_512:
		    EVP_PKEY_asn1_set_free(*ameth, pkey_free_gost_ec);
		    EVP_PKEY_asn1_set_private(*ameth, priv_decode_gost, priv_encode_gost, priv_print_gost_ec);
		    EVP_PKEY_asn1_set_param(*ameth, NULL, NULL, param_missing_gost_ec, param_copy_gost_ec, param_cmp_gost_ec, NULL);
		    EVP_PKEY_asn1_set_public(*ameth, pub_decode_gost_ec, pub_encode_gost_ec, pub_cmp_gost_ec, pub_print_gost_ec, pkey_size_gost, pkey_bits_gost);
		    EVP_PKEY_asn1_set_ctrl(*ameth, pkey_ctrl_gost);
		    EVP_PKEY_asn1_set_security_bits(*ameth, pkey_bits_gost);
		    break;
		...
		case NID_gost_mac_12:
		    ...
		    break;
		case NID_magma_mac: EVP_PKEY_asn1_set_free(*ameth, mackey_free_gost); EVP_PKEY_asn1_set_ctrl(*ameth, mac_ctrl_magma); break;
