...
void ngx_mail_imap_init_session(ngx_mail_session_t * s, ngx_connection_t * c)
{
	...
	c->P_EvRd->F_EvHandler = ngx_mail_imap_init_protocol;
...
void ngx_mail_imap_init_protocol(ngx_event_t * rev)
{
	...
	if(s->buffer == NULL) {
		...
		iscf = (ngx_mail_imap_srv_conf_t*)ngx_mail_get_module_srv_conf(s, ngx_mail_imap_module);
	...
	c->P_EvRd->F_EvHandler = ngx_mail_imap_auth_state;
...
static ngx_int_t ngx_mail_imap_capability(ngx_mail_session_t * s, ngx_connection_t * c)
{
	ngx_mail_imap_srv_conf_t  * iscf = (ngx_mail_imap_srv_conf_t*)ngx_mail_get_module_srv_conf(s, ngx_mail_imap_module);
#if (NGX_MAIL_SSL)
	if(c->ssl == NULL) {
