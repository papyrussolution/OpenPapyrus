...
struct ngx_mail_auth_http_ctx_s {
	...
	ngx_uint_t state;
	uchar  * header_name_start;
	uchar  * header_name_end;
	uchar  * header_start;
	uchar  * header_end;
	ngx_str_t addr;
	ngx_str_t port;
...
void ngx_mail_auth_http_init(ngx_mail_session_t * s)
{
	...
	ctx = (ngx_mail_auth_http_ctx_t *)ngx_pcalloc(pool, sizeof(ngx_mail_auth_http_ctx_t));
	if(!ctx) {
	...
	ctx->pool = pool;
	ahcf = (ngx_mail_auth_http_conf_t *)ngx_mail_get_module_srv_conf(s, ngx_mail_auth_http_module);
	...
	s->connection->P_EvRd->F_EvHandler = ngx_mail_auth_http_block_read;
	ctx->peer.connection->P_EvRd->F_EvHandler = ngx_mail_auth_http_read_handler;
	ctx->peer.connection->P_EvWr->F_EvHandler = ngx_mail_auth_http_write_handler;
...
static void ngx_mail_auth_http_write_handler(ngx_event_t * wev)
{
	...
	if(n > 0) {
		ctx->request->pos += n;
		if(n == size) {
			wev->F_EvHandler = ngx_mail_auth_http_dummy_handler;
			if(wev->timer_set) {
				ngx_del_timer(wev);
			}
	...
	for(;;) {
		...
		if(rc == NGX_OK) {
#if (NGX_DEBUG)
			{
				...
				ngx_log_debug2(NGX_LOG_DEBUG_MAIL, s->connection->log, 0, "mail auth http header: \"%V: %V\"", &key, &value);
			}
#endif
			len = ctx->header_name_end - ctx->header_name_start;
			if(len == sizeof("Auth-Status") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Status", sizeof("Auth-Status") - 1) == 0) {
				len = ctx->header_end - ctx->header_start;
				if(len == 2 && ctx->header_start[0] == 'O' && ctx->header_start[1] == 'K') {
					continue;
				}
				if(len == 4 && ctx->header_start[0] == 'W' && ctx->header_start[1] == 'A' && ctx->header_start[2] == 'I' && ctx->header_start[3] == 'T') {
					s->auth_wait = 1;
					continue;
				}
				ctx->errmsg.len = len;
				...
				p = (uchar *)ngx_pnalloc(s->connection->pool, size);
				if(!p) {
					ngx_close_connection(ctx->peer.connection);
					ngx_destroy_pool(ctx->pool);
					ngx_mail_session_internal_server_error(s);
			...
			if(len == sizeof("Auth-Server") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Server", sizeof("Auth-Server") - 1) == 0) {
				ctx->addr.len = ctx->header_end - ctx->header_start;
				ctx->addr.data = ctx->header_start;
				continue;
			}
			if(len == sizeof("Auth-Port") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Port", sizeof("Auth-Port") - 1) == 0) {
				ctx->port.len = ctx->header_end - ctx->header_start;
				ctx->port.data = ctx->header_start;
				continue;
			}
			if(len == sizeof("Auth-User") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-User", sizeof("Auth-User") - 1) == 0) {
				s->login.len = ctx->header_end - ctx->header_start;
				s->login.data = (uchar *)ngx_pnalloc(s->connection->pool, s->login.len);
			...
			if(len == sizeof("Auth-Pass") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Pass", sizeof("Auth-Pass") - 1) == 0) {
				s->passwd.len = ctx->header_end - ctx->header_start;
				s->passwd.data = (uchar *)ngx_pnalloc(s->connection->pool, s->passwd.len);
			...
			if(len == sizeof("Auth-Wait") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Wait", sizeof("Auth-Wait") - 1) == 0) {
			...
			if(len == sizeof("Auth-Error-Code") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Auth-Error-Code", sizeof("Auth-Error-Code") - 1) == 0) {
				ctx->errcode.len = ctx->header_end - ctx->header_start;
				ctx->errcode.data = (uchar *)ngx_pnalloc(s->connection->pool, ctx->errcode.len);
		...
		if(rc == NGX_DONE) {
			...
			if(ctx->err.len) {
				...
				if(s->protocol == NGX_MAIL_SMTP_PROTOCOL) {
					...
					p = (uchar *)ngx_pnalloc(s->connection->pool, ctx->err.len);
					if(!p) {
						ngx_destroy_pool(ctx->pool);
						ngx_mail_session_internal_server_error(s);
						return;
				...
				s->connection->P_EvRd->F_EvHandler = ngx_mail_auth_sleep_handler;
				return;
			}
			if(s->auth_wait) {
				timer = ctx->sleep;
			...
			peer = (ngx_addr_t *)ngx_pcalloc(s->connection->pool, sizeof(ngx_addr_t));
			if(peer == NULL) {
				ngx_destroy_pool(ctx->pool);
				ngx_mail_session_internal_server_error(s);
				return;
			...
			peer->name.len = len;
			peer->name.data = static_cast<uchar *>(ngx_pnalloc(s->connection->pool, len));
...
static void ngx_mail_auth_sleep_handler(ngx_event_t * rev)
{
	...
	if(rev->timedout) {
		...
		if(s->auth_wait) {
			...
			rev->F_EvHandler = cscf->protocol->auth_state;
			s->mail_state = 0;
...
static ngx_buf_t * ngx_mail_auth_http_create_request(ngx_mail_session_t * s, ngx_pool_t * pool, ngx_mail_auth_http_conf_t * ahcf)
{
	...
	if(c->ssl && sslcf->verify) { // certificate details 
		...
		if(ngx_ssl_get_fingerprint(c, pool, &fingerprint) != NGX_OK)
			return NULL;
		if(ahcf->pass_client_cert) { // certificate itself, if configured 
...
static ngx_int_t ngx_mail_auth_http_escape(ngx_pool_t * pool, ngx_str_t * text, ngx_str_t * escaped)
{
	...
	if(n == 0) {
		*escaped = *text;
		...
		p = (uchar *)ngx_pnalloc(pool, escaped->len);
		if(!p) {
			return NGX_ERROR;
		}
