...
static void ngx_mail_smtp_resolve_addr_handler(ngx_resolver_ctx_t * ctx)
{
	...
	c->P_EvRd->handler = ngx_mail_smtp_resolve_name;
	...
	if(ctx->state) {
...
#if (NGX_DEBUG)
		{
			...
			for(i = 0; i < ctx->naddrs; i++) {
				...
				ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "name was resolved to %V", &addr);
			}
		}
#endif
		for(i = 0; i < ctx->naddrs; i++) {
...
static void ngx_mail_smtp_greeting(ngx_mail_session_t * s, ngx_connection_t * c)
{
	...
	ngx_add_timer(c->P_EvRd, timeout);
...
void ngx_mail_smtp_init_protocol(ngx_event_t * rev)
{
	...
	c->P_EvRd->handler = ngx_mail_smtp_auth_state;
...
static ngx_int_t ngx_mail_smtp_helo(ngx_mail_session_t * s, ngx_connection_t * c)
{
	if(s->args.nelts != 1) {
		...
		ngx_mail_smtp_srv_conf_t  * sscf;
		ngx_str_t * arg = (ngx_str_t*)s->args.elts;
...
{
	ngx_str_t * arg, cmd;
	ngx_mail_smtp_srv_conf_t  * sscf = (ngx_mail_smtp_srv_conf_t*)ngx_mail_get_module_srv_conf(s, ngx_mail_smtp_module);
...
static ngx_int_t ngx_mail_smtp_discard_command(ngx_mail_session_t * s, ngx_connection_t * c,
    char * err)
{
	ssize_t n;
...
static void ngx_mail_smtp_log_rejected_command(ngx_mail_session_t * s, ngx_connection_t * c,
    char * err)
{
	u_char ch;
	ngx_str_t cmd;
