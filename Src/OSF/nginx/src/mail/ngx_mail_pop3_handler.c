...
static uchar pop3_greeting[] = "+OK POP3 ready" CRLF;
static uchar pop3_ok[] = "+OK" CRLF;
static uchar pop3_next[] = "+ " CRLF;
static uchar pop3_username[] = "+ VXNlcm5hbWU6" CRLF;
static uchar pop3_password[] = "+ UGFzc3dvcmQ6" CRLF;
static uchar pop3_invalid_command[] = "-ERR invalid command" CRLF;
...
void ngx_mail_pop3_init_session(ngx_mail_session_t * s, ngx_connection_t * c)
{
	...
	if(pscf->auth_methods & (NGX_MAIL_AUTH_APOP_ENABLED|NGX_MAIL_AUTH_CRAM_MD5_ENABLED)) {
		...
		s->out.data = (uchar *)ngx_pnalloc(c->pool, sizeof(pop3_greeting) + s->salt.len);
		if(s->out.data == NULL) {
			ngx_mail_session_internal_server_error(s);
			return;
		}
...
static ngx_int_t ngx_mail_pop3_pass(ngx_mail_session_t * s, ngx_connection_t * c)
{
	...
	ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "pop3 passwd: \"%V\"", &s->passwd);
#endif
	return NGX_DONE;
}
...
static ngx_int_t ngx_mail_pop3_capa(ngx_mail_session_t * s, ngx_connection_t * c, ngx_int_t stls)
{
	ngx_mail_pop3_srv_conf_t  * pscf = (ngx_mail_pop3_srv_conf_t*)ngx_mail_get_module_srv_conf(s, ngx_mail_pop3_module);
#if (NGX_MAIL_SSL)
	if(stls && c->ssl == NULL) {
		ngx_mail_ssl_conf_t  * sslcf = (ngx_mail_ssl_conf_t*)ngx_mail_get_module_srv_conf(s, ngx_mail_ssl_module);
