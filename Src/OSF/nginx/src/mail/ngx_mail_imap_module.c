...
static void * ngx_mail_imap_create_srv_conf(ngx_conf_t * cf)
{
	ngx_mail_imap_srv_conf_t  * iscf = (ngx_mail_imap_srv_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_mail_imap_srv_conf_t));
	if(iscf == NULL) {
		return NULL;
	}
...
static char * ngx_mail_imap_merge_srv_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_mail_imap_srv_conf_t * prev = (ngx_mail_imap_srv_conf_t *)parent;
	ngx_mail_imap_srv_conf_t * conf = (ngx_mail_imap_srv_conf_t *)child;
	uchar * p, * auth;
	size_t size;
	ngx_str_t * c, * d;
	ngx_uint_t i, m;
	...
	p = static_cast<uchar *>(ngx_pnalloc(cf->pool, size));
	if(!p) {
		return NGX_CONF_ERROR;
	}
