...
void ngx_mail_init_connection(ngx_connection_t * c)
{
	...
	port = (ngx_mail_port_t *)c->listening->servers;
	if(port->naddrs > 1) {
		/*
		...
		switch(sa->sa_family) {
			...
			case AF_INET6:
			    ...
			    addr6 = (ngx_mail_in6_addr_t *)port->addrs;
			    /* the last address is "*" */
			...
			default: /* AF_INET */
			    ...
			    addr = (ngx_mail_in_addr_t *)port->addrs;
			    /* the last address is "*" */
			    for(i = 0; i < port->naddrs - 1; i++) {
...
#endif
	s->passwd.data = (u_char *)ngx_pnalloc(c->pool, ngx_base64_decoded_length(arg[0].len));
...
u_char * ngx_mail_log_error(ngx_log_t * log, u_char * buf, size_t len)
{
	...
	ctx = (ngx_mail_log_ctx_t *)log->data;
