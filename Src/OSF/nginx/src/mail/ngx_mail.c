...
static const char * ngx_mail_block(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	*(ngx_mail_conf_ctx_t**)conf = ctx;
	...
	cscfp = (ngx_mail_core_srv_conf_t**)cmcf->servers.elts;
	for(m = 0; cf->cycle->modules[m]; m++) {
		if(cf->cycle->modules[m]->type != NGX_MAIL_MODULE) {
			continue;
		}
	...
	*cf = pcf;
	...
	listen = (ngx_mail_listen_t *)cmcf->listen.elts;
...
static ngx_int_t ngx_mail_add_ports(ngx_conf_t * cf, ngx_array_t * ports, ngx_mail_listen_t * listen)
{
	...
	port = (ngx_mail_conf_port_t*)ports->elts;
	...
	port = (ngx_mail_conf_port_t*)ngx_array_push(ports);
	if(port == NULL) {
		return NGX_ERROR;
	}
...
found:
	addr = (ngx_mail_conf_addr_t *)ngx_array_push(&port->addrs);
	if(addr == NULL) {
		return NGX_ERROR;
	}
	addr->opt = *listen;
...
static char * ngx_mail_optimize_servers(ngx_conf_t * cf, ngx_array_t * ports)
{
	...
	for(p = 0; p < ports->nelts; p++) {
		...
		addr = (ngx_mail_conf_addr_t *)port[p].addrs.elts;
		...
		while(i < last) {
			...
			ls->pool_size = 256;
			cscf = (ngx_mail_core_srv_conf_t*)addr->opt.ctx->srv_conf[ngx_mail_core_module.ctx_index];
...
#endif
			mport = (ngx_mail_port_t *)ngx_palloc(cf->pool, sizeof(ngx_mail_port_t));
			if(mport == NULL) {
				return NGX_CONF_ERROR;
			}
			ls->servers = mport;
...
static ngx_int_t ngx_mail_add_addrs(ngx_conf_t * cf, ngx_mail_port_t * mport, ngx_mail_conf_addr_t * addr)
{
	...
	addrs = (ngx_mail_in_addr_t *)mport->addrs;
...
static ngx_int_t ngx_mail_add_addrs6(ngx_conf_t * cf, ngx_mail_port_t * mport, ngx_mail_conf_addr_t * addr)
{
	...
	addrs6 = (ngx_mail_in6_addr_t *)mport->addrs;
	for(i = 0; i < mport->naddrs; i++) {
