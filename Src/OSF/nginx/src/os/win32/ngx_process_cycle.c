			...
			if(ngx_cache_manager_mutex == NULL) {
				...
				if(ngx_start_worker_processes(pCycle, NGX_PROCESS_RESPAWN) == 0) {
					...
					for(;; ) {
						...
						err = ngx_errno;
						ngx_time_update();
						ngx_log_debug1(NGX_LOG_DEBUG_CORE, pCycle->log, 0, "master WaitForMultipleObjects: %ul", ev);
						if(ev == WAIT_OBJECT_0) {
							ngx_log_error(NGX_LOG_NOTICE, pCycle->log, 0, "exiting");
							if(ResetEvent(ngx_stop_event) == 0)
								ngx_log_error(NGX_LOG_ALERT, pCycle->log, 0, "ResetEvent(\"%s\") failed", ngx_stop_event_name);
							SETIFZ(timer, (ngx_current_msec + 5000));
							ngx_terminate = 1;
							ngx_quit_worker_processes(pCycle, 0);
						}
						else if(ev == WAIT_OBJECT_0 + 1) {
							ngx_log_error(NGX_LOG_NOTICE, pCycle->log, 0, "shutting down");
							if(ResetEvent(ngx_quit_event) == 0)
								ngx_log_error(NGX_LOG_ALERT, pCycle->log, 0, "ResetEvent(\"%s\") failed", ngx_quit_event_name);
							ngx_quit = 1;
							ngx_quit_worker_processes(pCycle, 0);
						}
						else if(ev == WAIT_OBJECT_0 + 2) {
							ngx_log_error(NGX_LOG_NOTICE, pCycle->log, 0, "reopening logs");
							if(ResetEvent(ngx_reopen_event) == 0)
								ngx_log_error(NGX_LOG_ALERT, pCycle->log, 0, "ResetEvent(\"%s\") failed", ngx_reopen_event_name);
							ngx_reopen_files(pCycle, -1);
							ngx_reopen_worker_processes(pCycle);
						}
						else if(ev == WAIT_OBJECT_0 + 3) {
							...
							if(pCycle == NULL) {
								...
								ngx_cycle = pCycle;
								ngx_close_listening_sockets(pCycle);
								if(ngx_start_worker_processes(pCycle, NGX_PROCESS_JUST_RESPAWN))
									ngx_quit_worker_processes(pCycle, 1);
							}
						}
						else if(ev > (WAIT_OBJECT_0 + 3) && ev < (WAIT_OBJECT_0 + nev)) {
							ngx_log_debug0(NGX_LOG_DEBUG_CORE, pCycle->log, 0, "reap worker");
							live = ngx_reap_worker(pCycle, events[ev]);
							if(!live && (ngx_terminate || ngx_quit))
								ngx_master_process_exit(pCycle);
						}
						else if(ev == WAIT_TIMEOUT) {
							ngx_terminate_worker_processes(pCycle);
							ngx_master_process_exit(pCycle); // EXIT
						}
						else if(ev == WAIT_FAILED)
							ngx_log_error(NGX_LOG_ALERT, pCycle->log, err, "WaitForMultipleObjects() failed");
						else 
							ngx_log_error(NGX_LOG_ALERT, pCycle->log, 0, "WaitForMultipleObjects() returned unexpected value %ul", ev);
					}
				}
			}
		}
...
static int __stdcall ngx_console_handler(u_long type)
{
	...
	switch(type) {
		...
		case CTRL_CLOSE_EVENT: msg = "console closing, exiting"; break;
		case CTRL_LOGOFF_EVENT: msg = "user logs off, exiting"; break;
		default: return 0;
	}
...
{
	for(ngx_int_t n = 0; n < ngx_last_process; n++) {
		ngx_log_debug5(NGX_LOG_DEBUG_CORE, cycle->log, 0, "process: %d %P %p e:%d j:%d",
...
static int ngx_worker_process_cycle(ngx_cycle_t * cycle, char * mevn)
{
	...
	mev = OpenEvent(EVENT_MODIFY_STATE, 0, SUcSwitch(mevn));
	if(mev == NULL) {
...
/*static*/ ngx_thread_value_t __stdcall ngx_worker_thread(void * data)
{
	...
	while(!ngx_quit) {
		...
		ngx_process_events_and_timers(cycle); // Рабочая функция, обрабатывающая события
		if(ngx_terminate) {
			return 0;
		}
		if(ngx_quit) {
...
static void ngx_cache_manager_process_handler(void)
{
	...
	ngx_path_t ** path = (ngx_path_t **)ngx_cycle->paths.elts;
