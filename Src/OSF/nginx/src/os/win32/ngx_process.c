...
ngx_pid_t ngx_spawn_process(ngx_cycle_t * cycle, char * name, ngx_int_t respawn)
{
	...
	memzero(&ngx_processes[s], sizeof(ngx_process_t));
	...
	if(respawn < 0) {
		switch(respawn) {
			case NGX_PROCESS_RESPAWN:      ngx_processes[s].just_spawn = 0; break;
			case NGX_PROCESS_JUST_RESPAWN: ngx_processes[s].just_spawn = 1; break;
		}
