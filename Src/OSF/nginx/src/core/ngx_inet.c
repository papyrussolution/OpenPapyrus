...
ngx_int_t ngx_inet6_addr(uchar * p, size_t len, uchar * addr)
{
	...
	for(/* void */; len; len--) {
		...
		if(c == '.' && nibbles) {
			...
			word = ntohl(word);
			*addr++ = (uchar)((word >> 24) & 0xff);
			*addr++ = (uchar)((word >> 16) & 0xff);
			n--;
			break;
		}
		if(++nibbles > 4) {
...
#if (NGX_HAVE_INET6)
		case AF_INET6:
		    return ngx_inet6_ntop((uchar *)addr, text, len);
#endif
		default:
		    return 0;
	}
...
ngx_int_t ngx_parse_addr(ngx_pool_t * pool, ngx_addr_t * addr, uchar * text, size_t len)
{
	...
	addr->sockaddr->sa_family = (uchar)family;
	addr->socklen = len;
	switch(family) {
#if (NGX_HAVE_INET6)
		case AF_INET6:
...
ngx_int_t ngx_parse_addr_port(ngx_pool_t * pool, ngx_addr_t * addr, uchar * text, size_t len)
{
	...
	size_t plen;
	ngx_int_t port;
	ngx_int_t rc = ngx_parse_addr(pool, addr, text, len);
	if(rc != NGX_DECLINED) {
		return rc;
	}
	last = text + len;
...
{
	const uchar * p = u->url.data;
	size_t len = u->url.len;
	if(len >= 5 && ngx_strncasecmp(p, (uchar *)"unix:", 5) == 0)
...
static ngx_int_t ngx_parse_unix_domain_url(ngx_pool_t * pool, ngx_url_t * u)
{
	...
	(void)ngx_cpystrn((uchar *)saun->sun_path, path, len);
...
static ngx_int_t ngx_parse_inet_url(ngx_pool_t * pool, ngx_url_t * u)
{
	uchar * p, * host, * port, * last, * uri, * args;
	size_t len;
	ngx_int_t n;
	struct sockaddr_in * sin;
#if (NGX_HAVE_INET6)
	...
	if(u->listen && len == 1 && *host == '*') {
		...
		if(sin->sin_addr.s_addr != INADDR_NONE) {
			...
			sin = (struct sockaddr_in *)ngx_pcalloc(pool, sizeof(struct sockaddr_in));
			if(sin == NULL) {
				return NGX_ERROR;
			}
			...
			p = (uchar *)ngx_pnalloc(pool, u->host.len + sizeof(":65535") - 1);
			if(!p) {
				return NGX_ERROR;
			}
...
static ngx_int_t ngx_parse_inet6_url(ngx_pool_t * pool, ngx_url_t * u)
{
	...
	sin6 = (struct sockaddr_in6 *)ngx_pcalloc(pool, sizeof(struct sockaddr_in6));
	if(sin6 == NULL) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_inet_resolve_host(ngx_pool_t * pool, ngx_url_t * u)
{
	...
	struct sockaddr_in * sin;
	struct sockaddr_in6  * sin6;
	in_port_t port = htons(u->port);
	uchar * host = (uchar *)ngx_alloc(u->host.len + 1, pool->log);
	if(host == NULL) {
		return NGX_ERROR;
	}
	...
	u->addrs = (ngx_addr_t *)ngx_pcalloc(pool, i * sizeof(ngx_addr_t));
	if(u->addrs == NULL) {
		goto failed;
	}
	u->naddrs = i;
	...
	for(rp = res; rp != NULL; rp = rp->ai_next) {
		...
		sin = (struct sockaddr_in *)ngx_pcalloc(pool, rp->ai_addrlen);
		if(sin == NULL) {
			goto failed;
		}
		...
		p = static_cast<uchar *>(ngx_pnalloc(pool, len));
		if(!p) {
			goto failed;
		}
	...
	for(rp = res; rp != NULL; rp = rp->ai_next) {
		...
		sin6 = (struct sockaddr_in6 *)ngx_pcalloc(pool, rp->ai_addrlen);
		if(sin6 == NULL) {
			goto failed;
		}
...
ngx_int_t ngx_inet_resolve_host(ngx_pool_t * pool, ngx_url_t * u)
{
	...
	/* AF_INET only */
	in_port_t port = htons(u->port);
	in_addr_t in_addr = ngx_inet_addr(u->host.data, u->host.len);
	if(in_addr == INADDR_NONE) {
		host = (uchar *)ngx_alloc(u->host.len + 1, pool->log);
		if(host == NULL) {
			return NGX_ERROR;
		}
		(void)ngx_cpystrn(host, u->host.data, u->host.len + 1);
