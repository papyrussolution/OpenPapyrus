...
{
	ngx_thread_task_t  * task;
	task = (ngx_thread_task_t *)ngx_pcalloc(pool, sizeof(ngx_thread_task_t) + size);
	if(task == NULL) {
		return NULL;
	}
	task->ctx = task + 1;
...
static void * ngx_thread_pool_create_conf(ngx_cycle_t * cycle)
{
	ngx_thread_pool_conf_t  * tcf = (ngx_thread_pool_conf_t *)ngx_pcalloc(cycle->pool, sizeof(ngx_thread_pool_conf_t));
	if(tcf == NULL) {
		return NULL;
	}
...
static char * ngx_thread_pool_init_conf(ngx_cycle_t * cycle, void * conf)
{
	ngx_thread_pool_conf_t * tcf = (ngx_thread_pool_conf_t *)conf;
	ngx_uint_t i;
	ngx_thread_pool_t  ** tpp = (ngx_thread_pool_t **)tcf->pools.elts;
...
ngx_thread_pool_t * ngx_thread_pool_add(ngx_conf_t * cf, ngx_str_t * name)
{
	...
	tp = (ngx_thread_pool_t *)ngx_pcalloc(cf->pool, sizeof(ngx_thread_pool_t));
	if(tp == NULL) {
		return NULL;
	}
	...
	tpp = (ngx_thread_pool_t **)ngx_array_push(&tcf->pools);
	if(tpp == NULL) {
		return NULL;
	}
...
ngx_thread_pool_t * ngx_thread_pool_get(ngx_cycle_t * cycle, ngx_str_t * name)
{
	...
	tpp = (ngx_thread_pool_t **)tcf->pools.elts;
