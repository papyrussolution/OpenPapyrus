...
#endif
	n = (uchar *)ngx_pnalloc(pool, len + name->len + 1);
	if(n == NULL) {
		return NGX_ERROR;
	}
    ...
    ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
	...
	file->name.data = (uchar *)ngx_pnalloc(pool, file->name.len + 1);
...
{
	size_t i = path->name.len + 1;
	file[path->name.len + path->len]  = '/';
	for(ngx_uint_t n = 0; n < NGX_MAX_PATH_LEVEL; n++) {
		size_t level = path->level[n];
		if(level == 0) {
			break;
		}
		len -= level;
		file[i - 1] = '/';
		memcpy(&file[i], &file[len], level);
		i += level + 1;
	}
}
...
{
	ngx_err_t err;
	size_t pos = path->name.len;
	for(ngx_uint_t i = 0; i < NGX_MAX_PATH_LEVEL; i++) {
		if(path->level[i] == 0) {
			break;
		}
		pos += path->level[i] + 1;
...
{
	ngx_atomic_uint_t add = collision ? ngx_random_number : 1;
	ngx_atomic_uint_t n = ngx_atomic_fetch_add(ngx_temp_number, add);
	return n + add;
}
...
char * ngx_conf_merge_path_value(ngx_conf_t * cf, ngx_path_t ** path, ngx_path_t * prev, ngx_path_init_t * init)
{
	...
	*path = (ngx_path_t *)ngx_pcalloc(cf->pool, sizeof(ngx_path_t));
	if(*path == NULL) {
		return NGX_CONF_ERROR;
	}
	(*path)->name = init->name;
...
ngx_int_t ngx_create_paths(ngx_cycle_t * cycle, ngx_uid_t user)
{
	ngx_err_t err;
	ngx_uint_t i;
	ngx_path_t ** path = (ngx_path_t **)cycle->paths.elts;
...
ngx_int_t ngx_ext_rename_file(ngx_str_t * src, ngx_str_t * to, ngx_ext_rename_file_t * ext)
{
	...
	if(err == NGX_EXDEV) {
		...
		cf.time = ext->time;
		cf.log = ext->log;
		name = (uchar *)ngx_alloc(to->len + 1 + 10 + 1, ext->log);
		if(!name) {
			return NGX_ERROR;
		}
...
ngx_int_t ngx_walk_tree(ngx_tree_ctx_t * ctx, ngx_str_t * tree)
{
	...
	for(;;) {
		...
		if(file.len + NGX_DIR_MASK_LEN > buf.len) {
			...
			buf.data = (uchar *)ngx_alloc(buf.len + 1, ctx->log);
			if(buf.data == NULL) {
				goto failed;
			}
		}
