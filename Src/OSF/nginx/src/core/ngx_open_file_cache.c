...
create:
	...
	file = (ngx_cached_open_file_t *)ngx_alloc(sizeof(ngx_cached_open_file_t), pool->log);
	if(file == NULL) {
		goto failed;
	}
...
found:
	...
	ngx_log_debug5(NGX_LOG_DEBUG_CORE, pool->log, 0, "cached open file: %s, fd:%d, c:%d, e:%d, u:%d", file->name, file->fd, file->count, file->err, file->uses);
	if(of->err == 0) {
		if(!of->is_dir) {
...
static void ngx_open_file_add_event(ngx_open_file_cache_t * cache, ngx_cached_open_file_t * file, ngx_open_file_info_t * of, ngx_log_t * log)
{
	...
	file->use_event = 0;
	file->event = (ngx_event_t *)ngx_calloc(sizeof(ngx_event_t), log);
	if(file->event== NULL) {
		return;
	}
...
static void ngx_open_file_cleanup(void * data)
{
	ngx_open_file_cache_cleanup_t  * c = (ngx_open_file_cache_cleanup_t *)data;
	c->file->count--;
...
static void ngx_close_cached_file(ngx_open_file_cache_t * cache, ngx_cached_open_file_t * file, ngx_uint_t min_uses, ngx_log_t * log)
{
	ngx_log_debug5(NGX_LOG_DEBUG_CORE, log, 0, "close cached open file: %s, fd:%d, c:%d, u:%d, %d", file->name, file->fd, file->count, file->uses, file->close);
	if(!file->close) {
