...
static ngx_int_t ngx_output_chain_align_file_buf(ngx_output_chain_ctx_t * ctx, nginx_off_t bsize);
		...
		while(ctx->in) {
			...
			if(bsize == 0 && !ngx_buf_special(ctx->in->buf)) {
				ngx_log_error(NGX_LOG_ALERT, ctx->pool->log, 0, "zero size buf in output t:%d r:%d f:%d %p %p-%p %p %O-%O",
				    ctx->in->buf->temporary, ctx->in->buf->recycled, ctx->in->buf->in_file, ctx->in->buf->start,
				    ctx->in->buf->pos, ctx->in->buf->last, ctx->in->buf->file, ctx->in->buf->file_pos, ctx->in->buf->file_last);
				ngx_debug_point();
				ctx->in = ctx->in->next;
				continue;
			}
...
static ngx_int_t ngx_output_chain_align_file_buf(ngx_output_chain_ctx_t * ctx, nginx_off_t bsize)
{
	...
	if(size == 0) {
		if(bsize >= (nginx_off_t)ctx->bufs.size) {
			return NGX_DECLINED;
		}
		size = (size_t)bsize;
	}
	...
	// we do not set ctx->buf->tag, because we do not want to reuse the buf via ctx->free list
...
static ngx_int_t ngx_output_chain_get_buf(ngx_output_chain_ctx_t * ctx, nginx_off_t bsize)
{
	ngx_buf_t * b;
	ngx_buf_t * in = ctx->in->buf;
	size_t size = ctx->bufs.size;
	ngx_uint_t recycled = 1;
	if(in->last_in_chain) {
		if(bsize < (nginx_off_t)size) {
			/*
		...
		else if(!ctx->directio && ctx->bufs.num == 1 && (bsize < (nginx_off_t)(size + size / 4))) {
			/*
...
static ngx_int_t ngx_output_chain_copy_buf(ngx_output_chain_ctx_t * ctx)
{
	ssize_t n;
	ngx_uint_t sendfile;
	ngx_buf_t * src = ctx->in->buf;
	ngx_buf_t * dst = ctx->buf;
	nginx_off_t size = ngx_buf_size(src);
	...
	for(size = 0; in; in = in->next) {
		...
		if(ngx_buf_size(in->buf) == 0 && !ngx_buf_special(in->buf)) {
			ngx_log_error(NGX_LOG_ALERT, ctx->pool->log, 0, "zero size buf in chain writer t:%d r:%d f:%d %p %p-%p %p %O-%O",
			    in->buf->temporary, in->buf->recycled, in->buf->in_file, in->buf->start, in->buf->pos, in->buf->last,
