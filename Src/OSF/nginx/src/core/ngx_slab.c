...
void ngx_slab_init(ngx_slab_pool_t * pool)
{
	...
	memzero(pool->stats, n * sizeof(ngx_slab_stat_t));
	p += n * sizeof(ngx_slab_stat_t);
	size -= n * (sizeof(ngx_slab_page_t) + sizeof(ngx_slab_stat_t));
	pages = (ngx_uint_t)(size / (ngx_pagesize + sizeof(ngx_slab_page_t)));
	pool->pages = (ngx_slab_page_t*)p;
	memzero(pool->pages, pages * sizeof(ngx_slab_page_t));
	page = pool->pages;
	/* only "next" is used in list head */
	pool->free.slab = 0;
	pool->free.next = page;
...
void * ngx_slab_alloc_locked(ngx_slab_pool_t * pool, size_t size)
{
	...
	if(size > ngx_slab_max_size) {
		...
		p = page ? ngx_slab_page_addr(pool, page) : 0;
		goto done;
	}
	...
	if(page->next != page) {
		if(shift < ngx_slab_exact_shift) {
			...
			mask = (static_cast<uintptr_t>(1) << (ngx_pagesize >> shift)) - 1;
			mask <<= NGX_SLAB_MAP_SHIFT;
			for(m = static_cast<uintptr_t>(1) << NGX_SLAB_MAP_SHIFT, i = 0; m &mask; m <<= 1, i++) {
				if(page->slab & m) {
					continue;
				}
				page->slab |= m;
	...
	if(page) {
		if(shift < ngx_slab_exact_shift) {
			...
			m = (static_cast<uintptr_t>(1) << ((n + 1) % (8 * sizeof(uintptr_t)))) - 1;
			bitmap[i] = m;
			map = (ngx_pagesize >> shift) / (8 * sizeof(uintptr_t));
			for(i = i + 1; i < map; i++) {
				bitmap[i] = 0;
			...
			page->slab = (static_cast<uintptr_t>(1) << NGX_SLAB_MAP_SHIFT) | shift;
			page->next = &slots[slot];
...
{
	void  * p = ngx_slab_alloc_locked(pool, size);
	if(p) {
		memzero(p, size);
	}
	return p;
...
void ngx_slab_free_locked(ngx_slab_pool_t * pool, void * p)
{
	...
	switch(type) {
		case NGX_SLAB_SMALL:
		    ...
		    m = static_cast<uintptr_t>(1) << (n % (8 * sizeof(uintptr_t)));
		    ...
		    if(bitmap[n] & m) {
			    ...
			    m = (static_cast<uintptr_t>(1) << (n % (8 * sizeof(uintptr_t)))) - 1;
			    if(bitmap[i] & ~m) {
				    goto done;
			    }
			    map = (ngx_pagesize >> shift) / (8 * sizeof(uintptr_t));
		...
		case NGX_SLAB_EXACT:
		    m = static_cast<uintptr_t>(1) << (((uintptr_t)p & (ngx_pagesize - 1)) >> ngx_slab_exact_shift);
		...
		case NGX_SLAB_BIG:
		    ...
		    m = static_cast<uintptr_t>(1) << ((((uintptr_t)p & (ngx_pagesize - 1)) >> shift) + NGX_SLAB_MAP_SHIFT);
		    if(slab & m) {
			    slot = shift - pool->min_shift;
			    if(page->next == NULL) {
				    slots = ngx_slab_slots(pool);
...
static void ngx_slab_free_pages(ngx_slab_pool_t * pool, ngx_slab_page_t * page, ngx_uint_t pages)
{
	...
	page->slab = pages--;
	if(pages) {
		memzero(&page[1], pages * sizeof(ngx_slab_page_t));
	}
	if(page->next) {
		prev = ngx_slab_page_prev(page);
		prev->next = page->next;
