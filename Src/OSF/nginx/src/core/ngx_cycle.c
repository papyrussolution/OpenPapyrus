...
ngx_uint_t ngx_test_config__;
ngx_uint_t ngx_dump_config__;
...
ngx_cycle_t * ngx_init_cycle(ngx_cycle_t * old_cycle, const NgxStartUpOptions & rO)
{
	...
	pool->log = log;
	cycle = (ngx_cycle_t *)ngx_pcalloc(pool, sizeof(ngx_cycle_t));
	if(cycle == NULL) {
	...
	cycle->conf_file.data = (u_char *)ngx_pnalloc(pool, old_cycle->conf_file.len + 1);
	...
	cycle->conf_ctx = (void ****)ngx_pcalloc(pool, ngx_max_module * sizeof(void *));
	...
	cycle->hostname.data = (u_char *)ngx_pnalloc(pool, cycle->hostname.len);
	...
	if(/*ngx_test_config*/rO.Flags & rO.fTestConf) {
		if(ngx_create_pidfile(&ccf->pid, log, rO) != NGX_OK) {
			goto failed;
		}
	}
