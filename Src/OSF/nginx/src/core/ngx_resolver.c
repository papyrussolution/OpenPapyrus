...
#define ngx_resolver_node(n) (ngx_resolver_node_t*)((uchar *)(n) - offsetof(ngx_resolver_node_t, node))
...
static ngx_int_t ngx_resolver_send_udp_query(ngx_resolver_t * r, ngx_resolver_connection_t * rec, uchar * query, ushort qlen);
static ngx_int_t ngx_resolver_send_tcp_query(ngx_resolver_t * r, ngx_resolver_connection_t * rec, uchar * query, ushort qlen);
...
static void ngx_resolver_process_response(ngx_resolver_t * r, uchar * buf, size_t n, ngx_uint_t tcp);
static void ngx_resolver_process_a(ngx_resolver_t * r, uchar * buf, size_t n,
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t qtype, ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans);
static void ngx_resolver_process_srv(ngx_resolver_t * r, uchar * buf, size_t n,
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans);
static void ngx_resolver_process_ptr(ngx_resolver_t * r, uchar * buf, size_t n, ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan);
...
static ngx_int_t ngx_resolver_copy(ngx_resolver_t * r, ngx_str_t * name, uchar * buf, uchar * src, uchar * last);
...
static uchar * ngx_resolver_log_error(ngx_log_t * log, uchar * buf, size_t len);
...
ngx_resolver_t * ngx_resolver_create(ngx_conf_t * cf, ngx_str_t * names, ngx_uint_t n)
{
	...
	r = (ngx_resolver_t *)ngx_calloc(sizeof(ngx_resolver_t), cf->log);
	if(r == NULL) {
		return NULL;
	}
	cln->data = r;
	r->P_Ev = (ngx_event_t *)ngx_calloc(sizeof(ngx_event_t), cf->log);
	if(r->P_Ev == NULL) {
		return NULL;
	}
...
#endif
	r->P_Ev->F_EvHandler = ngx_resolver_resend_handler;
	r->P_Ev->P_Data = r;
	r->P_Ev->log = &cf->cycle->new_log;
	r->P_Ev->cancelable = 1;
	r->ident = -1;
	r->resend_timeout = 5;
	r->tcp_timeout = 5;
...
static void ngx_resolver_cleanup(void * data)
{
	ngx_resolver_t  * r = (ngx_resolver_t *)data;
	ngx_uint_t i;
	ngx_resolver_connection_t  * rec;
	if(r) {
...
ngx_resolver_ctx_t * ngx_resolve_start(ngx_resolver_t * r, ngx_resolver_ctx_t * temp)
{
	...
	if(r->connections.nelts == 0) {
		return (ngx_resolver_ctx_t *)NGX_NO_RESOLVER;
	}
	ctx = (ngx_resolver_ctx_t *)ngx_resolver_calloc(r, sizeof(ngx_resolver_ctx_t));
	if(ctx) {
		ctx->resolver = r;
	}
	return ctx;
...
ngx_int_t ngx_resolve_name(ngx_resolver_ctx_t * ctx)
{
	...
	if(ctx->service.len) {
		...
		name.data = (uchar *)ngx_resolver_alloc(r, name.len);
		if(name.data == NULL) {
			goto failed;
		}
...
void ngx_resolve_name_done(ngx_resolver_ctx_t * ctx)
{
	...
	// unlock alloc mutex 
	if(r->P_Ev->timer_set && ngx_resolver_resend_empty(r)) {
		ngx_del_timer(r->P_Ev);
	}
}
...
static ngx_int_t ngx_resolve_name_locked(ngx_resolver_t * r, ngx_resolver_ctx_t * ctx, ngx_str_t * name)
{
	...
	if(rn) {
		...
		if(rn->valid >= ngx_time()) {
			...
			naddrs = (rn->naddrs == (ushort) -1) ? 0 : rn->naddrs;
#if (NGX_HAVE_INET6)
			naddrs += (rn->naddrs6 == (ushort) -1) ? 0 : rn->naddrs6;
#endif
			if(naddrs) {
		...
		if(rn->naddrs > 1 && rn->naddrs != (ushort) -1) {
			ngx_resolver_free_locked(r, rn->u.addrs);
		}
#if (NGX_HAVE_INET6)
		if(rn->naddrs6 > 1 && rn->naddrs6 != (ushort) -1) {
		...
		rn = (ngx_resolver_node_t *)ngx_resolver_alloc(r, sizeof(ngx_resolver_node_t));
		if(rn == NULL) {
			return NGX_ERROR;
		}
		rn->name = (uchar *)ngx_resolver_dup(r, name->data, name->len);
		if(rn->name == NULL) {
			ngx_resolver_free(r, rn);
			return NGX_ERROR;
		}
		rn->node.key = hash;
		rn->nlen = (ushort)name->len;
		rn->query = NULL;
#if (NGX_HAVE_INET6)
		rn->query6 = NULL;
#endif
	...
	if(ngx_resolver_resend_empty(r)) {
		ngx_add_timer(r->P_Ev, (ngx_msec_t)(r->resend_timeout * 1000));
	}
...
ngx_int_t ngx_resolve_addr(ngx_resolver_ctx_t * ctx)
{
	...
	if(rn) {
		if(rn->valid >= ngx_time()) {
			...
			rn->expire = ngx_time() + r->expire;
			ngx_queue_insert_head(expire_queue, &rn->queue);
			name = (uchar *)ngx_resolver_dup(r, rn->name, rn->nlen);
			if(!name) {
				goto failed;
			}
...
static ngx_int_t ngx_resolver_send_query(ngx_resolver_t * r, ngx_resolver_node_t * rn)
{
	...
	if(rn->query6 && rn->naddrs6 == (ushort) -1) {
		rc = rn->tcp6
...
static ngx_int_t ngx_resolver_send_tcp_query(ngx_resolver_t * r, ngx_resolver_connection_t * rec, uchar * query, ushort qlen)
{
	...
	if(rec->tcp == NULL) {
		...
		if(!b) {
			...
			b->start = (uchar *)ngx_resolver_alloc(r, NGX_RESOLVER_TCP_RSIZE);
			if(b->start == NULL) {
		...
		if(!b) {
			...
			b->start = (uchar *)ngx_resolver_alloc(r, NGX_RESOLVER_TCP_WSIZE);
			if(b->start == NULL) {
				ngx_resolver_free(r, b);
				return NGX_ERROR;
			}
...
static void ngx_resolver_resend_handler(ngx_event_t * ev)
{
	...
	time_t a6timer;
#endif
	ngx_resolver_t  * r = (ngx_resolver_t *)ev->P_Data;
...
#endif
	if(timer) {
		ngx_add_timer(r->P_Ev, (ngx_msec_t)(timer * 1000));
	}
}
...
static void ngx_resolver_process_response(ngx_resolver_t * r, uchar * buf, size_t n, ngx_uint_t tcp)
{
	...
	if(code == NGX_RESOLVE_FORMERR) {
		times = 0;
		for(q = ngx_queue_head(&r->name_resend_queue); q != ngx_queue_sentinel(&r->name_resend_queue) && times++ < 100; q = ngx_queue_next(q)) {
			rn = ngx_queue_data(q, ngx_resolver_node_t, queue);
    ...
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t qtype, ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
	...
	in_addr_t  * addr;
	ngx_str_t name;
	ngx_uint_t type, qident, naddrs, a, i, j, start;
	ngx_uint_t cls__;
#if (NGX_HAVE_INET6)
	...
	switch(qtype) {
#if (NGX_HAVE_INET6)
		case NGX_RESOLVE_AAAA:
		    if(rn->query6 == NULL || rn->naddrs6 != (ushort) -1) {
			    ngx_log_error(r->log_level, r->log, 0, "unexpected response for %V", &name);
			    ngx_resolver_free(r, name.data);
			    goto failed;
		    }
		...
		default: /* NGX_RESOLVE_A */
		    if(rn->query == NULL || rn->naddrs != (ushort) -1) {
			    ngx_log_error(r->log_level, r->log, 0, "unexpected response for %V", &name);
			    ngx_resolver_free(r, name.data);
			    goto failed;
		    }
...
found:
		...
		cls__ = (an->class_hi << 8) + an->class_lo;
		len = (an->len_hi << 8) + an->len_lo;
		ttl = (an->ttl[0] << 24) + (an->ttl[1] << 16)
		    + (an->ttl[2] << 8) + (an->ttl[3]);
		if(cls__ != 1) {
			ngx_log_error(r->log_level, r->log, 0, "unexpected RR class %ui", cls__);
			goto failed;
		}
		...
		switch(qtype) {
			...
			case NGX_RESOLVE_AAAA:
			    if(naddrs == 1) {
				    ...
				    addr6 = (in6_addr *)ngx_resolver_alloc(r, naddrs * sizeof(struct in6_addr));
				    if(addr6 == NULL) {
					    goto failed;
				    }

				    rn->u6.addrs6 = addr6;
				    rn->naddrs6 = (ushort)naddrs;
			    }
			...
			default: /* NGX_RESOLVE_A */
			    if(naddrs == 1) {
				    ...
				    addr = (in_addr_t *)ngx_resolver_alloc(r, naddrs * sizeof(in_addr_t));
				    if(addr == NULL) {
					    goto failed;
				    }

				    rn->u.addrs = addr;
				    rn->naddrs = (ushort)naddrs;
			    }
    ...
    ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
	...
	uint32_t hash;
	ngx_str_t name;
	ngx_uint_t type, qident, start, nsrvs, a, i, j;
	ngx_uint_t cls__;
	ngx_resolver_an_t   * an;
	ngx_resolver_ctx_t  * ctx, * next;
...
found:
	...
	if(nsrvs) {
		srvs = (ngx_resolver_srv_t *)ngx_resolver_calloc(r, nsrvs * sizeof(ngx_resolver_srv_t));
		if(srvs == NULL) {
			goto failed;
		}
		rn->u.srvs = srvs;
		rn->nsrvs = (ushort)nsrvs;
		j = 0;
		i = ans;
		for(a = 0; a < nan; a++) {
			for(;;) {
...
static void ngx_resolver_resolve_srv_names(ngx_resolver_ctx_t * ctx, ngx_resolver_node_t * rn)
{
	...
	ctx->valid = rn->valid;
	ctx->count = rn->nsrvs;
	srvs = (ngx_resolver_srv_name_t *)ngx_resolver_calloc(r, rn->nsrvs * sizeof(ngx_resolver_srv_name_t));
	if(srvs == NULL) {
		goto failed;
	}
	ctx->srvs = srvs;
	...
	for(i = 0; i < (ngx_uint_t)rn->nsrvs; i++) {
		srvs[i].name.data = (uchar *)ngx_resolver_alloc(r, rn->u.srvs[i].name.len);
		if(srvs[i].name.data == NULL) {
			goto failed;
		}
		srvs[i].name.len = rn->u.srvs[i].name.len;
		memcpy(srvs[i].name.data, rn->u.srvs[i].name.data, srvs[i].name.len);
...
static void ngx_resolver_srv_names_handler(ngx_resolver_ctx_t * cctx)
{
	...
	ngx_resolver_t * r = cctx->resolver;
	ngx_resolver_ctx_t  * ctx = (ngx_resolver_ctx_t *)cctx->data;
	ngx_resolver_srv_name_t  * srv = cctx->srvs;
	ctx->count--;
	...
	if(cctx->naddrs) {
		...
		sockaddr = (ngx_sockaddr_t *)ngx_resolver_alloc(r, cctx->naddrs * sizeof(ngx_sockaddr_t));
		if(sockaddr == NULL) {
			ngx_resolver_free(r, addrs);
			srv->state = NGX_ERROR;
			goto done;
		...
		for(i = 0; i < cctx->naddrs; i++) {
			...
			memcpy(&sockaddr[i], cctx->addrs[i].sockaddr, addrs[i].socklen);
...
static void ngx_resolver_process_ptr(ngx_resolver_t * r, uchar * buf, size_t n, ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan)
{
	...
	ngx_int_t octet;
	ngx_str_t name;
	ngx_uint_t mask, type, qident, a, i, start;
	ngx_uint_t cls__;
	ngx_queue_t   * expire_queue;
	ngx_rbtree_t  * tree;
	ngx_resolver_an_t  * an;
	...
	if(sstreqi_ascii(&buf[i], (uchar *)"\7in-addr\4arpa")) {
...
invalid_in_addr_arpa:
	...
	for(octet = 15; octet >= 0; octet--) {
		...
		addr6.s6_addr[octet] = (uchar)digit;
		if(buf[i++] != '\1') {
			goto invalid_ip6_arpa;
		}
		digit = ngx_hextoi(&buf[i++], 1);
		...
		addr6.s6_addr[octet] += (uchar)(digit * 16);
	}
	if(sstreqi_ascii(&buf[i], (uchar *)"\3ip6\4arpa")) {
		i += sizeof("\3ip6\4arpa");
		/* lock addr mutex */
...
found:
		...
		ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0, "resolver qt:%ui cl:%ui len:%uz", type, cls__, len);
		i += sizeof(ngx_resolver_an_t);
		switch(type) {
			case NGX_RESOLVE_PTR:
			    goto ptr;
...
ptr:
	...
	if(name.len != (size_t)rn->nlen || ngx_strncmp(name.data, rn->name, name.len) != 0) {
		...
		rn->nlen = (ushort)name.len;
		rn->name = name.data;
		name.data = (uchar *)ngx_resolver_dup(r, rn->name, name.len);
		if(name.data == NULL) {
			goto failed;
		}
	}
...
static ngx_resolver_node_t * ngx_resolver_lookup_name(ngx_resolver_t * r, ngx_str_t * name, uint32_t hash)
{
	ngx_int_t rc;
	ngx_resolver_node_t  * rn;
	ngx_rbtree_node_t * node = r->name_rbtree.root;
	ngx_rbtree_node_t * sentinel = r->name_rbtree.sentinel;
	while(node != sentinel) {
		if(hash < node->key) {
			node = node->left;
			continue;
...
static ngx_resolver_node_t * ngx_resolver_lookup_srv(ngx_resolver_t * r, ngx_str_t * name, uint32_t hash)
{
	ngx_rbtree_node_t * node = r->srv_rbtree.root;
	ngx_rbtree_node_t * sentinel = r->srv_rbtree.sentinel;
	while(node != sentinel) {
		if(hash < node->key) {
			node = node->left;
			continue;
		...
		{
			/* hash == node->key */
			ngx_resolver_node_t * rn = ngx_resolver_node(node);
			ngx_int_t rc = ngx_memn2cmp(name->data, rn->name, name->len, rn->nlen);
			if(rc == 0) {
				return rn;
			}
			node = (rc < 0) ? node->left : node->right;
...
static ngx_resolver_node_t * ngx_resolver_lookup_addr(ngx_resolver_t * r, in_addr_t addr)
{
	ngx_rbtree_node_t * node = r->addr_rbtree.root;
	ngx_rbtree_node_t * sentinel = r->addr_rbtree.sentinel;
	while(node != sentinel) {
		if(addr < node->key) {
			node = node->left;
			continue;
...
static ngx_resolver_node_t * ngx_resolver_lookup_addr6(ngx_resolver_t * r, const struct in6_addr * addr, uint32_t hash)
{
	ngx_int_t rc;
	ngx_resolver_node_t  * rn;
	ngx_rbtree_node_t * node = r->addr6_rbtree.root;
	ngx_rbtree_node_t * sentinel = r->addr6_rbtree.sentinel;
	while(node != sentinel) {
		if(hash < node->key) {
			node = node->left;
			continue;
...
static ngx_int_t ngx_resolver_create_name_query(ngx_resolver_t * r, ngx_resolver_node_t * rn, ngx_str_t * name)
{
...
#if (NGX_HAVE_INET6)
	p = (uchar *)ngx_resolver_alloc(r, r->ipv6 ? len * 2 : len);
#else
	p = (uchar *)ngx_resolver_alloc(r, len);
#endif
	if(!p) {
		return NGX_ERROR;
	}
	...
	query->ident_hi = (uchar)((ident >> 8) & 0xff);
	query->ident_lo = (uchar)(ident & 0xff);
	/* recursion query */
...
static ngx_int_t ngx_resolver_create_srv_query(ngx_resolver_t * r, ngx_resolver_node_t * rn, ngx_str_t * name)
{
	...
	uchar * p = (uchar *)ngx_resolver_alloc(r, len);
	if(!p) {
		return NGX_ERROR;
	}
	rn->qlen = (ushort)len;
...
invalid:
	...
	dst = (uchar *)ngx_resolver_alloc(r, len);
	if(dst == NULL) {
		return NGX_ERROR;
	}
	name->data = dst;
...
static ngx_int_t ngx_resolver_set_timeout(ngx_resolver_t * r, ngx_resolver_ctx_t * ctx)
{
	...
	ctx->event = (ngx_event_t *)ngx_resolver_calloc(r, sizeof(ngx_event_t));
	if(ctx->event == NULL) {
		return NGX_ERROR;
	}
...
static void ngx_resolver_timeout_handler(ngx_event_t * ev)
{
	ngx_resolver_ctx_t * ctx = (ngx_resolver_ctx_t *)ev->P_Data;
...
{
	/* lock alloc mutex */
	uchar  * p = (uchar *)ngx_alloc(size, r->log);
	/* unlock alloc mutex */
	return p;
}
...
static void * ngx_resolver_calloc(ngx_resolver_t * r, size_t size)
{
	uchar  * p = (uchar *)ngx_resolver_alloc(r, size);
	if(p) {
		memzero(p, size);
	}
	return p;
...
static ngx_resolver_addr_t * ngx_resolver_export(ngx_resolver_t * r, ngx_resolver_node_t * rn, ngx_uint_t rotate)
{
	...
	n += rn->naddrs6;
#endif
	dst = (ngx_resolver_addr_t *)ngx_resolver_calloc(r, n * sizeof(ngx_resolver_addr_t));
	if(dst == NULL) {
		return NULL;
	}
	sockaddr = (ngx_sockaddr_t *)ngx_resolver_calloc(r, n * sizeof(ngx_sockaddr_t));
	if(sockaddr == NULL) {
...
static void ngx_resolver_report_srv(ngx_resolver_t * r, ngx_resolver_ctx_t * ctx)
{
	ngx_uint_t nw, i, j, k, l, m, n, w;
	ngx_resolver_srv_name_t  * srvs = ctx->srvs;
	ngx_uint_t nsrvs = ctx->nsrvs;
	ngx_uint_t naddrs = 0;
	for(i = 0; i < nsrvs; i++) {
	...
	if(naddrs == 0) {
		...
		ngx_resolver_addr_t * addrs = (ngx_resolver_addr_t *)ngx_resolver_calloc(r, naddrs * sizeof(ngx_resolver_addr_t));
		if(addrs == NULL) {
			ctx->state = NGX_ERROR;
			ctx->valid = ngx_time() + (r->valid ? r->valid : 10);
			ctx->handler(ctx);
...
static ngx_int_t ngx_udp_connect(ngx_resolver_connection_t * rec)
{
	...
	ngx_event_t  * rev, * wev;
	ngx_connection_t  * c;
	ngx_socket_t s = ngx_socket(rec->sockaddr->sa_family, SOCK_DGRAM, 0);
	...
	wev->ready = 1;
	event = (ngx_event_flags & NGX_USE_CLEAR_EVENT) ? /* kqueue, epoll */ NGX_CLEAR_EVENT : /* select, poll, /dev/poll */ NGX_LEVEL_EVENT;
		...
		if(err != NGX_EINPROGRESS
			...
			if(err == NGX_ECONNREFUSED
			   ...
			   * for unix sockets if listen queue is full
			   ...
			   || err == NGX_EAGAIN
#endif
			   || oneof5(err, NGX_ECONNRESET, NGX_ENETDOWN, NGX_ENETUNREACH, NGX_EHOSTDOWN, NGX_EHOSTUNREACH)) {
				level = NGX_LOG_ERR;
			}
			else {
				level = NGX_LOG_CRIT;
			}
			ngx_log_error(level, c->log, err, "connect() to %V failed", &rec->server);
			ngx_close_connection(c);
			rec->tcp = NULL;
			return NGX_ERROR;
		}
...
static ngx_int_t ngx_resolver_cmp_srvs(const void * one, const void * two)
{
	const ngx_resolver_srv_t * first = (const ngx_resolver_srv_t *)one;
	const ngx_resolver_srv_t * second = (const ngx_resolver_srv_t *)two;
	ngx_int_t p1 = first->priority;
	ngx_int_t p2 = second->priority;
	return p1 - p2;
}
