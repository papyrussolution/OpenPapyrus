...
void ngx_array_destroy(ngx_array_t * a)
{
	...
	if((uchar *)a + sizeof(ngx_array_t) == p->d.last) {
		p->d.last = (uchar *)a;
	}
}
...
void * FASTCALL ngx_array_push(ngx_array_t * a)
{
	...
	if(a->nelts == a->nalloc) {
		...
		ngx_pool_t * p = a->pool;
		if((uchar *)a->elts + size == p->d.last && p->d.last + a->size <= p->d.end) {
			// 
...
void * FASTCALL ngx_array_push_n(ngx_array_t * a, ngx_uint_t n)
{
	...
	if(a->nelts + n > a->nalloc) {
		/* the array is full */
		p = a->pool;
		if((uchar *)a->elts + a->size * a->nalloc == p->d.last && p->d.last + size <= p->d.end) {
			/*
			 * the array allocation is the last in the pool
			 * and there is space for new allocation
			 */
