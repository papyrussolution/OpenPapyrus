...
ngx_int_t ngx_event_post_acceptex(ngx_listening_t * ls, ngx_uint_t n)
{
	...
	for(ngx_uint_t i = 0; i < n; i++) {
		...
		if(s == (ngx_socket_t)-1) {
			ngx_log_error(NGX_LOG_ALERT, &ls->log, ngx_socket_errno, ngx_socket_n " failed");
			return NGX_ERROR;
		}
		else {
			...
			log = (ngx_log_t *)ngx_palloc(c->pool, sizeof(ngx_log_t));
			if(log == NULL) {
				ngx_close_posted_connection(c);
				return NGX_ERROR;
			}
			...
			c->local_sockaddr = (struct sockaddr *)ngx_palloc(c->pool, ls->socklen);
			if(c->local_sockaddr == NULL) {
				ngx_close_posted_connection(c);
				return NGX_ERROR;
			}
			c->sockaddr = (struct sockaddr *)ngx_palloc(c->pool, ls->socklen);
			if(c->sockaddr == NULL) {
				ngx_close_posted_connection(c);
				return NGX_ERROR;
			}
