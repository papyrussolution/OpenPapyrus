...
void ngx_event_accept(ngx_event_t * ev)
{
	...
	ecf = (ngx_event_conf_t*)ngx_event_get_conf(ngx_cycle->conf_ctx, ngx_event_core_module);
	...
	do {
		...
		if(s == (ngx_socket_t)-1) {
			...
			else if(oneof2(err, NGX_EMFILE, NGX_ENFILE)) {
				level = NGX_LOG_CRIT;
			}
#if (NGX_HAVE_ACCEPT4)
			...
			if(oneof2(err, NGX_EMFILE, NGX_ENFILE)) {
