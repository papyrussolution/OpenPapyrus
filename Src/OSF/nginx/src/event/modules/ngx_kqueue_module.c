...
static ngx_int_t ngx_kqueue_init(ngx_cycle_t * cycle, ngx_msec_t timer)
{
	...
	kcf = (ngx_kqueue_conf_t *)ngx_event_get_conf(cycle->conf_ctx, ngx_kqueue_module);
	...
	if(max_changes < kcf->changes) {
		...
		change_list = (struct kevent *)ngx_alloc(kcf->changes * sizeof(struct kevent), cycle->log);
		if(change_list == NULL) {
			return NGX_ERROR;
		}
	}
	...
	if(nevents < kcf->events) {
		...
		event_list = (struct kevent *)ngx_alloc(kcf->events * sizeof(struct kevent), cycle->log);
		if(event_list == NULL) {
			return NGX_ERROR;
		}
	}
...
{
	ngx_kqueue_conf_t  * kcf;
	kcf = (ngx_kqueue_conf_t *)ngx_palloc(cycle->pool, sizeof(ngx_kqueue_conf_t));
	if(kcf == NULL) {
		return NULL;
	}
...
static char * ngx_kqueue_init_conf(ngx_cycle_t * cycle, void * conf)
{
	ngx_kqueue_conf_t * kcf = (ngx_kqueue_conf_t *)conf;
