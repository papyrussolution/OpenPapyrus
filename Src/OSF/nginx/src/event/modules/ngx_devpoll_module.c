...
static ngx_int_t ngx_devpoll_init(ngx_cycle_t * cycle, ngx_msec_t timer)
{
	...
	dpcf = (ngx_devpoll_conf_t*)ngx_event_get_conf(cycle->conf_ctx, ngx_devpoll_module);
	if(dp == -1) {
	...
	if(max_changes < dpcf->changes) {
		...
		change_list = (struct pollfd *)ngx_alloc(sizeof(struct pollfd) * dpcf->changes,
		    cycle->log);
		if(change_list == NULL) {
			return NGX_ERROR;
		}
		...
		change_index = (ngx_event_t**)ngx_alloc(sizeof(ngx_event_t *) * dpcf->changes,
		    cycle->log);
	...
	if(nevents < dpcf->events) {
		...
		event_list = (struct pollfd *)ngx_alloc(sizeof(struct pollfd) * dpcf->events,
		    cycle->log);
		if(event_list == NULL) {
			return NGX_ERROR;
		}
...
static ngx_int_t ngx_devpoll_process_events(ngx_cycle_t * cycle, ngx_msec_t timer,
    ngx_uint_t flags)
{
...
static void * ngx_devpoll_create_conf(ngx_cycle_t * cycle)
{
	ngx_devpoll_conf_t  * dpcf = (ngx_devpoll_conf_t *)ngx_palloc(cycle->pool, sizeof(ngx_devpoll_conf_t));
	if(dpcf == NULL) {
		return NULL;
	}
...
static char * ngx_devpoll_init_conf(ngx_cycle_t * cycle, void * conf)
{
	ngx_devpoll_conf_t * dpcf = (ngx_devpoll_conf_t *)conf;
