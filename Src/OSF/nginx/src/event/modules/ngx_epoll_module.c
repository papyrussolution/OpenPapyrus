...
static ngx_int_t ngx_epoll_init(ngx_cycle_t * cycle, ngx_msec_t timer)
{
	ngx_epoll_conf_t  * epcf = (ngx_epoll_conf_t *)ngx_event_get_conf(cycle->conf_ctx, ngx_epoll_module);
	if(ep == -1) {
	...
	if(nevents < epcf->events) {
		...
		event_list = (struct epoll_event *)ngx_alloc(sizeof(struct epoll_event) * epcf->events, cycle->log);
		if(event_list == NULL) {
			return NGX_ERROR;
		}
	}
...
static void * ngx_epoll_create_conf(ngx_cycle_t * cycle)
{
	ngx_epoll_conf_t  * epcf = (ngx_epoll_conf_t *)ngx_palloc(cycle->pool, sizeof(ngx_epoll_conf_t));
	if(epcf == NULL) {
		return NULL;
	}
...
static char * ngx_epoll_init_conf(ngx_cycle_t * cycle, void * conf)
{
	ngx_epoll_conf_t * epcf = (ngx_epoll_conf_t *)conf;
