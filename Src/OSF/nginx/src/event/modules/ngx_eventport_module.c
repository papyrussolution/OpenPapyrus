...
static ngx_int_t ngx_eventport_init(ngx_cycle_t * cycle, ngx_msec_t timer)
{
	...
	struct sigevent sev;
	ngx_eventport_conf_t  * epcf = (ngx_eventport_conf_t *)ngx_event_get_conf(cycle->conf_ctx, ngx_eventport_module);
	if(ep == -1) {
		ep = port_create();
		if(ep == -1) {
	...
	if(nevents < epcf->events) {
		...
		event_list = (int*)ngx_alloc(sizeof(port_event_t) * epcf->events, cycle->log);
		if(event_list == NULL) {
			return NGX_ERROR;
		}
	}
...
static ngx_int_t ngx_eventport_process_events(ngx_cycle_t * cycle, ngx_msec_t timer,
    ngx_uint_t flags)
{
	int n, revents;
	u_int events;
...
static void * ngx_eventport_create_conf(ngx_cycle_t * cycle)
{
	ngx_eventport_conf_t  * epcf = (ngx_eventport_conf_t *)ngx_palloc(cycle->pool, sizeof(ngx_eventport_conf_t));
	if(epcf == NULL) {
		return NULL;
	}
...
static char * ngx_eventport_init_conf(ngx_cycle_t * cycle, void * conf)
{
	ngx_eventport_conf_t * epcf = (ngx_eventport_conf_t *)conf;
