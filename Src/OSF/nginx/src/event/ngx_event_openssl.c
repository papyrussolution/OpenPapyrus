...
{
	ngx_str_t * cert = (ngx_str_t *)certs->elts;
	ngx_str_t * key = (ngx_str_t *)keys->elts;
	for(ngx_uint_t i = 0; i < certs->nelts; i++) {
...
ngx_int_t ngx_ssl_certificate(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_str_t * cert, ngx_str_t * key, ngx_array_t * passwords)
{
	...
	if(bio == NULL) {
		ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "BIO_new_file(\"%s\") failed", cert->data);
		return NGX_ERROR;
	}
	...
	for(;;) {
		...
		if(x509 == NULL) {
			...
			/* some real error */
			ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "PEM_read_bio_X509(\"%s\") failed", cert->data);
			BIO_free(bio);
			return NGX_ERROR;
		}
		...
		if(SSL_CTX_add0_chain_cert(ssl->ctx, x509) == 0) {
			ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "SSL_CTX_add0_chain_cert(\"%s\") failed", cert->data);
			X509_free(x509);
			BIO_free(bio);
			return NGX_ERROR;
		}
	...
	if(ngx_strncmp(key->data, "engine:", sizeof("engine:") - 1) == 0) {
		...
		ENGINE * engine;
		EVP_PKEY  * pkey;
		uchar * p = key->data + sizeof("engine:") - 1;
		uchar * last = (uchar *)ngx_strchr(p, ':');
		if(!last) {
		...
		*last = '\0';
		...
		if(engine == NULL) {
			ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "ENGINE_by_id(\"%s\") failed", p);
			return NGX_ERROR;
		}
		*last++ = ':';
		pkey = ENGINE_load_private_key(engine, (char *)last, 0, 0);
		if(pkey == NULL) {
			ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "ENGINE_load_private_key(\"%s\") failed", last);
			ENGINE_free(engine);
			return NGX_ERROR;
		}
		ENGINE_free(engine);
		if(SSL_CTX_use_PrivateKey(ssl->ctx, pkey) == 0) {
			ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "SSL_CTX_use_PrivateKey(\"%s\") failed", last);
			EVP_PKEY_free(pkey);
			return NGX_ERROR;
		}
		EVP_PKEY_free(pkey);
		return NGX_OK;
#else
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "loading \"engine:...\" certificate keys is not supported");
		return NGX_ERROR;
	...
	for(;;) {
		...
		ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "SSL_CTX_use_PrivateKey_file(\"%s\") failed", key->data);
		return NGX_ERROR;
	}
...
static int ngx_ssl_password_callback(char * buf, int size, int rwflag, void * userdata)
{
	ngx_str_t * pwd = (ngx_str_t *)userdata;
	if(rwflag) {
...
{
#if (NGX_DEBUG)
	ngx_ssl_conn_t * ssl_conn = (ngx_ssl_conn_t *)X509_STORE_CTX_get_ex_data(x509_store, SSL_get_ex_data_X509_STORE_CTX_idx());
...
ngx_array_t * ngx_ssl_read_password_file(ngx_conf_t * cf, ngx_str_t * file)
{
	...
	uchar buf[NGX_SSL_PASSWORD_BUFFER_SIZE];
	...
	do {
		...
		for(;;) {
			...
			if(len) {
				...
				pwd->len = len;
				pwd->data = (uchar *)ngx_pnalloc(cf->temp_pool, len);
				if(pwd->data == NULL) {
					passwords->nelts--;
					passwords = NULL;
					goto cleanup;
...
static void ngx_ssl_passwords_cleanup(void * data)
{
	ngx_array_t * passwords = (ngx_array_t *)data;
	ngx_uint_t i;
	ngx_str_t * pwd = (ngx_str_t *)passwords->elts;
	for(i = 0; i < passwords->nelts; i++) {
		memzero(pwd[i].data, pwd[i].len);
	}
}
...
ngx_int_t ngx_ssl_handshake(ngx_connection_t * c)
{
	...
	if(n == 1) {
...
#if (NGX_DEBUG)
		{
			...
			const
#endif
			SSL_CIPHER  * cipher = SSL_get_current_cipher(c->ssl->connection);
			if(cipher) {
...
ssize_t ngx_ssl_recv(ngx_connection_t * c, uchar * buf, size_t size)
{
	int n, bytes;
...
static ngx_int_t ngx_ssl_handle_recv(ngx_connection_t * c, int n)
{
	...
	if(n > 0) {
		if(c->ssl->saved_write_handler) {
			...
			ngx_post_event(c->P_EvWr, &ngx_posted_events);
		}
		return NGX_OK;
	}
	sslerr = SSL_get_error(c->ssl->connection, n);
	...
	if(sslerr == SSL_ERROR_WANT_WRITE) {
		ngx_log_error(NGX_LOG_INFO, c->log, 0, "peer started SSL renegotiation");
		c->P_EvWr->ready = 0;
		if(ngx_handle_write_event(c->P_EvWr, 0) != NGX_OK) {
			return NGX_ERROR;
		}
...
ngx_chain_t * ngx_ssl_send_chain(ngx_connection_t * c, ngx_chain_t * in, nginx_off_t limit)
{
	...
	if(limit == 0 || limit > (nginx_off_t)(NGX_MAX_INT32_VALUE - ngx_pagesize)) {
	...
	if(buf->start == NULL) {
		buf->start = (uchar *)ngx_palloc(c->pool, c->ssl->buffer_size);
		if(buf->start == NULL) {
			return NGX_CHAIN_ERROR;
		}
		buf->pos = buf->start;
	...
	for(;;) {
		while(in && buf->last < buf->end && send < limit) {
			...
			memcpy(buf->last, in->buf->pos, size);
			buf->last += size;
			in->buf->pos += size;
			send += size;
			if(in->buf->pos == in->buf->last) {
	...
	buf->flush = flush;
	SETFLAG(c->buffered, NGX_SSL_BUFFERED, (buf->pos < buf->last));
	return in;
}

ssize_t ngx_ssl_write(ngx_connection_t * c, uchar * data, size_t size)
{
	int n, sslerr;
	ngx_err_t err;
	ngx_ssl_clear_error(c->log);
...
void ngx_cdecl ngx_ssl_error(ngx_uint_t level, ngx_log_t * log, ngx_err_t err, const char * fmt, ...)
{
	...
	va_end(args);
	p = ngx_cpystrn(p, (uchar *)" (SSL:", last - p);
	for(;;) {
		...
		*p++ = ' ';
		...
		if(p < last && *data && (flags & ERR_TXT_STRING)) {
			*p++ = ':';
			p = ngx_cpystrn(p, (uchar *)data, last - p);
		}
next:
		(void)ERR_get_error();
	}
...
static ngx_int_t ngx_ssl_session_id_context(ngx_ssl_t * ssl, ngx_str_t * sess_ctx)
{
	...
	for(cert = (X509*)SSL_CTX_get_ex_data(ssl->ctx, ngx_ssl_certificate_index); cert; cert = (X509*)X509_get_ex_data(cert, ngx_ssl_next_certificate_index)) {
...
ngx_int_t ngx_ssl_session_cache_init(ngx_shm_zone_t * shm_zone, void * data)
{
	...
	cache = (ngx_ssl_session_cache_t *)ngx_slab_alloc(shpool, sizeof(ngx_ssl_session_cache_t));
	if(cache == NULL) {
		return NGX_ERROR;
	}
	shpool->data = cache;
...
static int ngx_ssl_new_session(ngx_ssl_conn_t * ssl_conn, ngx_ssl_session_t * sess)
{
	uchar * p, * id, * cached_sess, * session_id;
	uint32_t hash;
	SSL_CTX  * ssl_ctx;
	...
	ngx_ssl_session_cache_t  * cache;
	uchar buf[NGX_SSL_MAX_SESSION_SIZE];
	...
	shm_zone = (ngx_shm_zone_t *)SSL_CTX_get_ex_data(ssl_ctx, ngx_ssl_session_cache_index);
	cache = (ngx_ssl_session_cache_t *)shm_zone->data;
	shpool = (ngx_slab_pool_t*)shm_zone->shm.addr;
	ngx_shmtx_lock(&shpool->mutex);
	/* drop one or two expired sessions */
	ngx_ssl_expire_sessions(cache, shpool, 1);
	cached_sess = (uchar *)ngx_slab_alloc_locked(shpool, len);
	if(cached_sess == NULL) {
	...
	sess_id = (ngx_ssl_sess_id_t *)ngx_slab_alloc_locked(shpool, sizeof(ngx_ssl_sess_id_t));
	if(sess_id == NULL) {
		/* drop the oldest non-expired session and try once more */
		ngx_ssl_expire_sessions(cache, shpool, 0);
		sess_id = (ngx_ssl_sess_id_t *)ngx_slab_alloc_locked(shpool, sizeof(ngx_ssl_sess_id_t));
...
#if OPENSSL_VERSION_NUMBER >= 0x0090800fL
	session_id = (uchar *)SSL_SESSION_get_id(sess, &session_id_length);
#else
	...
	id = sess_id->sess_id;
#else
	id = (uchar *)ngx_slab_alloc_locked(shpool, session_id_length);
	if(id == NULL) {
		/* drop the oldest non-expired session and try once more */
		ngx_ssl_expire_sessions(cache, shpool, 0);
		id = (uchar *)ngx_slab_alloc_locked(shpool, session_id_length);
    ...
    uchar * id, int len, int * copy)
{
	...
	uchar buf[NGX_SSL_MAX_SESSION_SIZE];
	ngx_connection_t  * c;
	uint32_t hash = ngx_crc32_short((uchar *)(uintptr_t)id, (size_t)len);
	*copy = 0;
	c = (ngx_connection_t *)ngx_ssl_get_connection(ssl_conn);
	ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl get session: %08XD:%d", hash, len);
	shm_zone = (ngx_shm_zone_t *)SSL_CTX_get_ex_data(c->ssl->session_ctx, ngx_ssl_session_cache_index);
	cache = (ngx_ssl_session_cache_t *)shm_zone->data;
	sess = NULL;
	shpool = (ngx_slab_pool_t*)shm_zone->shm.addr;
	ngx_shmtx_lock(&shpool->mutex);
	...
	while(node != sentinel) {
		...
		rc = ngx_memn2cmp((uchar *)(uintptr_t)id, sess_id->id, (size_t)len, (size_t)node->data);
		if(rc == 0) {
			if(sess_id->expire > ngx_time()) {
				memcpy(buf, sess_id->session, sess_id->len);
				ngx_shmtx_unlock(&shpool->mutex);
				p = buf;
...
static void ngx_ssl_remove_session(SSL_CTX * ssl, ngx_ssl_session_t * sess)
{
	...
	ngx_ssl_sess_id_t * sess_id;
	ngx_ssl_session_cache_t  * cache;
	shm_zone = (ngx_shm_zone_t *)SSL_CTX_get_ex_data(ssl, ngx_ssl_session_cache_index);
	if(shm_zone == NULL) {
		return;
	}
	cache = (ngx_ssl_session_cache_t *)shm_zone->data;
#if OPENSSL_VERSION_NUMBER >= 0x0090800fL
	id = (uchar *)SSL_SESSION_get_id(sess, &len);
#else
	id = sess->session_id;
...
ngx_int_t ngx_ssl_session_ticket_keys(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_array_t * paths)
{
	...
	if(SSL_CTX_set_ex_data(ssl->ctx, ngx_ssl_session_ticket_keys_index, keys) == 0) {
		ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "SSL_CTX_set_ex_data() failed");
		return NGX_ERROR;
	}
	if(SSL_CTX_set_tlsext_ticket_key_cb(ssl->ctx, ngx_ssl_session_ticket_key_callback) == 0) {
...
static int ngx_ssl_session_ticket_key_callback(ngx_ssl_conn_t * ssl_conn, uchar * name, uchar * iv, EVP_CIPHER_CTX * ectx, HMAC_CTX * hctx, int enc)
{
	...
	if(enc == 1) {
		...
		ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl session ticket encrypt, key: \"%*s\" (%s session)",
		    ngx_hex_dump(buf, key[0].name, 16) - buf, buf, SSL_session_reused(ssl_conn) ? "reused" : "new");
		if(key[0].size == 48) {
...
{
	ngx_ssl_t * ssl = (ngx_ssl_t *)data;
	X509 * cert = (X509*)SSL_CTX_get_ex_data(ssl->ctx, ngx_ssl_certificate_index);
	while(cert) {
		X509 * next = (X509*)X509_get_ex_data(cert, ngx_ssl_next_certificate_index);
		X509_free(cert);
		cert = next;
	}
...
{
	s->data = (uchar *)SSL_get_version(c->ssl->connection);
	return NGX_OK;
}

ngx_int_t ngx_ssl_get_cipher_name(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	s->data = (uchar *)SSL_get_cipher_name(c->ssl->connection);
	return NGX_OK;
}
...
ngx_int_t ngx_ssl_get_ciphers(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	int    bytes = SSL_get0_raw_cipherlist(c->ssl->connection, NULL);
	int    n = SSL_get0_raw_cipherlist(c->ssl->connection, &ciphers);
	if(n <= 0) {
		s->len = 0;
		return NGX_OK;
	}
	...
	s->data = static_cast<uchar *>(ngx_pnalloc(pool, len));
	if(!s->data) {
		return NGX_ERROR;
	}
	p = s->data;
	...
	s->data = (uchar *)ngx_pnalloc(pool, s->len);
	if(!s->data) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_get_curves(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	uchar * p;
	size_t len;
	int    n = SSL_get1_curves(c->ssl->connection, NULL);
	if(n <= 0) {
		s->len = 0;
		return NGX_OK;
	}
...
ngx_int_t ngx_ssl_get_session_id(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	uchar * buf;
	unsigned int len;
	SSL_SESSION * sess = SSL_get0_session(c->ssl->connection);
	if(sess == NULL) {
		s->len = 0;
		return NGX_OK;
	}
#if OPENSSL_VERSION_NUMBER >= 0x0090800fL
	buf = (uchar *)SSL_SESSION_get_id(sess, &len);
#else
	buf = sess->session_id;
	len = sess->session_id_length;
#endif
	s->len = 2 * len;
	s->data = (uchar *)ngx_pnalloc(pool, 2 * len);
	if(!s->data) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_get_server_name(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	const char  * servername = SSL_get_servername(c->ssl->connection, TLSEXT_NAMETYPE_host_name);
	if(servername) {
		s->data = (uchar *)servername;
...
ngx_int_t ngx_ssl_get_client_verify(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	s->data = (uchar *)ngx_pnalloc(pool, sizeof("FAILED:") - 1 + ngx_strlen(str));
	if(!s->data) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_get_client_v_remain(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	s->data = (uchar *)ngx_pnalloc(pool, NGX_TIME_T_LEN);
	if(!s->data) {
		X509_free(cert);
		return NGX_ERROR;
	}
    ...
    ASN1_TIME * asn1time)
{
	...
	time = ngx_parse_http_time((uchar *)value, len);
	BIO_free(bio);
	return time;
}
...
static void * ngx_openssl_create_conf(ngx_cycle_t * cycle)
{
	ngx_openssl_conf_t  * oscf = (ngx_openssl_conf_t *)ngx_pcalloc(cycle->pool, sizeof(ngx_openssl_conf_t));
	if(oscf == NULL) {
		return NULL;
	}
	/*
