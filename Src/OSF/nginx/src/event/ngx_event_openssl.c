...
ngx_chain_t * ngx_ssl_send_chain(ngx_connection_t * c, ngx_chain_t * in, nginx_off_t limit)
{
	...
	if(buf->start == NULL) {
		buf->start = (u_char *)ngx_palloc(c->pool, c->ssl->buffer_size);
		if(buf->start == NULL) {
			return NGX_CHAIN_ERROR;
		}
		buf->pos = buf->start;
...
static int ngx_ssl_new_session(ngx_ssl_conn_t * ssl_conn, ngx_ssl_session_t * sess)
{
	...
	cached_sess = (u_char *)ngx_slab_alloc_locked(shpool, len);
	if(cached_sess == NULL) {
	...
	id = sess_id->sess_id;
#else
	id = (u_char *)ngx_slab_alloc_locked(shpool, session_id_length);
	if(id == NULL) {
		/* drop the oldest non-expired session and try once more */
		ngx_ssl_expire_sessions(cache, shpool, 0);
		id = (u_char *)ngx_slab_alloc_locked(shpool, session_id_length);
    ...
    u_char * id, int len, int * copy)
{
	...
	u_char buf[NGX_SSL_MAX_SESSION_SIZE];
	ngx_connection_t  * c;
	uint32_t hash = ngx_crc32_short((u_char *)(uintptr_t)id, (size_t)len);
	*copy = 0;
	c = (ngx_connection_t *)ngx_ssl_get_connection(ssl_conn);
...
ngx_int_t ngx_ssl_session_ticket_keys(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_array_t * paths)
{
	...
	if(SSL_CTX_set_tlsext_ticket_key_cb(ssl->ctx, ngx_ssl_session_ticket_key_callback) == 0) {
		ngx_log_error(NGX_LOG_WARN, cf->log, 0,
		    "nginx was built with Session Tickets support, however, now it is linked dynamically to an OpenSSL library "
		    "which has no tlsext support, therefore Session Tickets are not available");
	}
	return NGX_OK;
failed:
	if(ngx_close_file(file.fd) == NGX_FILE_ERROR) {
...
#endif
	s->len = 2 * len;
	s->data = (u_char *)ngx_pnalloc(pool, 2 * len);
	if(s->data == NULL) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_get_client_verify(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	s->data = (u_char *)ngx_pnalloc(pool, sizeof("FAILED:") - 1 + ngx_strlen(str));
	if(s->data == NULL) {
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_get_client_v_remain(ngx_connection_t * c, ngx_pool_t * pool, ngx_str_t * s)
{
	...
	s->data = (u_char *)ngx_pnalloc(pool, NGX_TIME_T_LEN);
	if(s->data == NULL) {
		X509_free(cert);
		return NGX_ERROR;
	}
