...
static uchar * ngx_ssl_ocsp_log_error(ngx_log_t * log, uchar * buf, size_t len);

ngx_int_t ngx_ssl_stapling(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_str_t * file, ngx_str_t * responder, ngx_uint_t verify)
{
	for(X509 * cert = (X509 *)SSL_CTX_get_ex_data(ssl->ctx, ngx_ssl_certificate_index); cert; cert = (X509 *)X509_get_ex_data(cert, ngx_ssl_next_certificate_index)) {
...
static ngx_int_t ngx_ssl_stapling_certificate(ngx_conf_t * cf, ngx_ssl_t * ssl, X509 * cert, ngx_str_t * file, ngx_str_t * responder, ngx_uint_t verify)
{
	...
	ngx_ssl_stapling_t  * staple = (ngx_ssl_stapling_t *)ngx_pcalloc(cf->pool, sizeof(ngx_ssl_stapling_t));
	if(staple == NULL) {
		return NGX_ERROR;
	}
	...
	staple->cert = cert;
	staple->name = (uchar *)X509_get_ex_data(staple->cert, ngx_ssl_certificate_name_index);
	if(file->len) {
...
static ngx_int_t ngx_ssl_stapling_file(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_ssl_stapling_t * staple, ngx_str_t * file)
{
	...
	buf = (uchar *)ngx_alloc(len, ssl->log);
	if(!buf) {
		goto failed;
	}
	p = buf;
...
static ngx_int_t ngx_ssl_stapling_issuer(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_ssl_stapling_t * staple)
{
	...
	if(rc == 0) {
		ngx_log_error(NGX_LOG_WARN, ssl->log, 0, "\"ssl_stapling\" ignored, issuer certificate not found for certificate \"%s\"", staple->name);
		X509_STORE_CTX_free(store_ctx);
		return NGX_DECLINED;
	}
	X509_STORE_CTX_free(store_ctx);
...
static ngx_int_t ngx_ssl_stapling_responder(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_ssl_stapling_t * staple, ngx_str_t * responder)
{
	...
	if(responder->len == 0) {
		...
		responder->data = (uchar *)ngx_palloc(cf->pool, responder->len);
	...
	u.default_port = 80;
	u.uri_part = 1;
	if(u.url.len > 7 && ngx_strncasecmp(u.url.data, (uchar *)"http://", 7) == 0) {
		u.url.len -= 7;
		u.url.data += 7;
	}
	else {
		ngx_log_error(NGX_LOG_WARN, ssl->log, 0, "\"ssl_stapling\" ignored, invalid URL prefix in OCSP responder \"%V\" in the certificate \"%s\"", &u.url, staple->name);
		return NGX_DECLINED;
	}
	if(ngx_parse_url(cf->pool, &u) != NGX_OK) {
		if(u.err) {
			ngx_log_error(NGX_LOG_WARN, ssl->log, 0, "\"ssl_stapling\" ignored, %s in OCSP responder \"%V\" in the certificate \"%s\"", u.err, &u.url, staple->name);
			return NGX_DECLINED;
		}
		return NGX_ERROR;
	}
...
ngx_int_t ngx_ssl_stapling_resolver(ngx_conf_t * cf, ngx_ssl_t * ssl, ngx_resolver_t * resolver, ngx_msec_t resolver_timeout)
{
	...
	for(cert = (X509 *)SSL_CTX_get_ex_data(ssl->ctx, ngx_ssl_certificate_index); cert; cert = (X509 *)X509_get_ex_data(cert, ngx_ssl_next_certificate_index)) {
		staple = (ngx_ssl_stapling_t *)X509_get_ex_data(cert, ngx_ssl_stapling_index);
...
static int ngx_ssl_certificate_status_callback(ngx_ssl_conn_t * ssl_conn, void * data)
{
	...
	if(staple->staple.len && staple->valid >= ngx_time()) {
		...
		p = (uchar *)OPENSSL_malloc(staple->staple.len);
		if(!p) {
...
static void ngx_ssl_stapling_ocsp_handler(ngx_ssl_ocsp_ctx_t * ctx)
{
	...
	ngx_ssl_stapling_t  * staple = (ngx_ssl_stapling_t *)ctx->data;
	time_t now = ngx_time();
	...
	response.len = len;
	response.data = (uchar *)ngx_alloc(response.len, ctx->log);
...
static void ngx_ssl_stapling_cleanup(void * data)
{
	ngx_ssl_stapling_t  * staple = (ngx_ssl_stapling_t *)data;
	if(staple->issuer) {
...
static ngx_ssl_ocsp_ctx_t * ngx_ssl_ocsp_start(void)
{
	...
	log = (ngx_log_t *)ngx_palloc(pool, sizeof(ngx_log_t));
	if(log == NULL) {
		ngx_destroy_pool(pool);
		return NULL;
	}
...
static void ngx_ssl_ocsp_resolve_handler(ngx_resolver_ctx_t * resolve)
{
	ngx_ssl_ocsp_ctx_t * ctx = (ngx_ssl_ocsp_ctx_t *)resolve->data;
	uchar * p;
	size_t len;
	socklen_t socklen;
	ngx_uint_t i;
	...
	if(resolve->state) {
		ngx_log_error(NGX_LOG_ERR, ctx->log, 0, "%V could not be resolved (%i: %s)", &resolve->name, resolve->state, ngx_resolver_strerror(resolve->state));
		goto failed;
	}
#if (NGX_DEBUG)
	{
		...
		for(i = 0; i < resolve->naddrs; i++) {
			addr.len = ngx_sock_ntop(resolve->addrs[i].sockaddr, resolve->addrs[i].socklen, text, NGX_SOCKADDR_STRLEN, 0);
			ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "name was resolved to %V", &addr);
		}
	}
#endif
	ctx->naddrs = resolve->naddrs;
	ctx->addrs = (ngx_addr_t *)ngx_pcalloc(ctx->pool, ctx->naddrs * sizeof(ngx_addr_t));
	if(ctx->addrs == NULL) {
		goto failed;
	}
	for(i = 0; i < resolve->naddrs; i++) {
		socklen = resolve->addrs[i].socklen;
		sockaddr = (struct sockaddr *)ngx_palloc(ctx->pool, socklen);
		if(sockaddr == NULL) {
			goto failed;
		}
		...
		p = (uchar *)ngx_pnalloc(ctx->pool, NGX_SOCKADDR_STRLEN);
		if(!p) {
			goto failed;
		}
...
static void ngx_ssl_ocsp_connect(ngx_ssl_ocsp_ctx_t * ctx)
{
	...
	if(rc == NGX_OK) {
		ngx_ssl_ocsp_write_handler(ctx->peer.connection->P_EvWr);
		return;
	}
}
...
static ngx_int_t ngx_ssl_ocsp_create_request(ngx_ssl_ocsp_ctx_t * ctx)
{
	...
	binary.len = len;
	binary.data = (uchar *)ngx_palloc(ctx->pool, len);
	if(binary.data == NULL) {
		goto failed;
	}
	p = binary.data;
	...
	base64.data = (uchar *)ngx_palloc(ctx->pool, base64.len);
	if(base64.data == NULL) {
		goto failed;
	}
	...
	if(escape == 0) {
		...
		p = (uchar *)ngx_escape_uri(p, base64.data, base64.len, NGX_ESCAPE_URI_COMPONENT);
	}
...
static ngx_int_t ngx_ssl_ocsp_process_status_line(ngx_ssl_ocsp_ctx_t * ctx)
{
	ngx_int_t rc = ngx_ssl_ocsp_parse_status_line(ctx);
	if(rc == NGX_OK) {
...
static ngx_int_t ngx_ssl_ocsp_process_headers(ngx_ssl_ocsp_ctx_t * ctx)
{
	...
	for(;;) {
		...
		if(rc == NGX_OK) {
			...
			if(len == sizeof("Content-Type") - 1 && ngx_strncasecmp(ctx->header_name_start, (uchar *)"Content-Type", sizeof("Content-Type") - 1) == 0) {
				len = ctx->header_end - ctx->header_start;
				if(len != sizeof("application/ocsp-response") - 1 || ngx_strncasecmp(ctx->header_start,
					    (uchar *)"application/ocsp-response", sizeof("application/ocsp-response") - 1) != 0) {
...
static ngx_int_t ngx_ssl_ocsp_parse_header_line(ngx_ssl_ocsp_ctx_t * ctx)
{
	...
	for(p = ctx->response->pos; p < ctx->response->last; p++) {
		ch = *p;
#if 0
		ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "s:%d in:'%02Xd:%c'", state, ch, ch);
#endif
		switch(state) {
			/* first char */
			case sw_start:
...
static uchar * ngx_ssl_ocsp_log_error(ngx_log_t * log, uchar * buf, size_t len)
{
	...
	ctx = (ngx_ssl_ocsp_ctx_t *)log->data;
	if(ctx) {
