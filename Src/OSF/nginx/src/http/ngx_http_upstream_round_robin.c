...
{
	ngx_uint_t n;
	ngx_http_upstream_rr_peer_data_t  * rrp = (ngx_http_upstream_rr_peer_data_t*)r->upstream->peer.data;
	if(rrp == NULL) {
		rrp = (ngx_http_upstream_rr_peer_data_t*)ngx_palloc(r->pool, sizeof(ngx_http_upstream_rr_peer_data_t));
		if(rrp == NULL) {
			return NGX_ERROR;
		}
	...
	if(n <= 8 * sizeof(uintptr_t)) {
		...
		rrp->tried = (uintptr_t*)ngx_pcalloc(r->pool, n * sizeof(uintptr_t));
		if(rrp->tried == NULL) {
			return NGX_ERROR;
		}
	}
...
ngx_int_t ngx_http_upstream_create_round_robin_peer(ngx_http_request_t * r, ngx_http_upstream_resolved_t * ur)
{
	...
	peers = (ngx_http_upstream_rr_peers_t*)ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_rr_peers_t));
	if(peers == NULL) {
		return NGX_ERROR;
	}
	peer = (ngx_http_upstream_rr_peer_t*)ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_rr_peer_t) * ur->naddrs);
	if(peer == NULL) {
		return NGX_ERROR;
	}
	...
	if(ur->sockaddr) {
		...
		for(i = 0; i < ur->naddrs; i++) {
			...
			p = (uchar *)ngx_pnalloc(r->pool, NGX_SOCKADDR_STRLEN);
			if(!p) {
				return NGX_ERROR;
			}
...
ngx_int_t ngx_http_upstream_set_round_robin_peer_session(ngx_peer_connection_t * pc,
    void * data)
{
	ngx_http_upstream_rr_peer_data_t  * rrp = (ngx_http_upstream_rr_peer_data_t *)data;
...
#endif
	ssl_session = (ngx_ssl_session_t*)peer->ssl_session;
	rc = ngx_ssl_set_session(pc->connection, ssl_session);
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "set session: %p", ssl_session);
	return rc;
}
...
void ngx_http_upstream_save_round_robin_peer_session(ngx_peer_connection_t * pc, void * data)
{
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "save session: %p", ssl_session);
	peer = rrp->current;
	old_ssl_session = (ngx_ssl_session_t*)peer->ssl_session;
