...
static void ngx_http_upstream_ssl_handshake(ngx_http_request_t * r, ngx_http_upstream_t * u,
    ngx_connection_t * c)
{
	long rc;
...
static ngx_int_t ngx_http_upstream_ssl_name(ngx_http_request_t * r, ngx_http_upstream_t * u,
    ngx_connection_t * c)
{
	u_char     * p, * last;
	ngx_str_t name;
...
static void ngx_http_upstream_send_response(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	p->temp_file = (ngx_temp_file_t *)ngx_pcalloc(r->pool, sizeof(ngx_temp_file_t));
	if(p->temp_file == NULL) {
		ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
		return;
	}
	...
	if(u->cacheable) {
		p->buf_to_file = (ngx_buf_t *)ngx_calloc_buf(r->pool);
			...
			if(p->upstream_done) {
				...
				if(p->length == -1
				    && (u->headers_in.content_length_n == -1
					    ...
					    == tf->offset - (nginx_off_t)r->cache->body_start)) {
...
static void * ngx_http_upstream_create_main_conf(ngx_conf_t * cf)
{
	...
	umcf = (ngx_http_upstream_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_main_conf_t));
	if(umcf == NULL) {
		return NULL;
	}
