...
static void ngx_http_upstream_process_header(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	if(c->P_EvRd->timedout) {
		...
		if(u->buffer.start == NULL) {
			u->buffer.start = (u_char *)ngx_palloc(r->pool, u->conf->buffer_size);
			if(u->buffer.start == NULL) {
				ngx_http_upstream_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
				return;
			}
...
static void ngx_http_upstream_send_response(ngx_http_request_t * pReq, ngx_http_upstream_t * pUpstr)
{
	...
	if(pUpstr->cacheable) {
		...
		if(valid) {
			pReq->cache->date = now;
			pReq->cache->body_start = (u_short)(pUpstr->buffer.pos - pUpstr->buffer.start);
...
static void ngx_http_upstream_process_upgraded(ngx_http_request_t * r, ngx_uint_t from_upstream, ngx_uint_t do_write)
{
	...
	if(from_upstream) {
		...
		if(b->start == NULL) {
			b->start = (u_char *)ngx_palloc(r->pool, u->conf->buffer_size);
			if(b->start == NULL) {
				ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
				return;
			}
...
static ngx_int_t ngx_http_upstream_rewrite_refresh(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	if(ho == NULL) {
		...
		*ho = *h;
		if(r->upstream->rewrite_redirect) {
			const u_char * p = ngx_strcasestrn(ho->value.data, "url=", 4 - 1);
			if(p) {
