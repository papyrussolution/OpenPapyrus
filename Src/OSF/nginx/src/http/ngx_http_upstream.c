...
static const char * ngx_http_upstream(ngx_conf_t * cf, const ngx_command_t * cmd, void * dummy); // F_SetHandler
static const char * ngx_http_upstream_server(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
...
static ngx_http_upstream_header_t ngx_http_upstream_headers_in[] = {
	...
	{ ngx_string("Date"), ngx_http_upstream_process_header_line, offsetof(ngx_http_upstream_headers_in_t, date), ngx_http_upstream_copy_header_line, offsetof(ngx_http_headers_out_t, date), 0 },
	{ ngx_string("Last-Modified"), ngx_http_upstream_process_last_modified, 0, ngx_http_upstream_copy_last_modified, 0, 0 },
	{ ngx_string("ETag"), ngx_http_upstream_process_header_line, offsetof(ngx_http_upstream_headers_in_t, etag), ngx_http_upstream_copy_header_line, offsetof(ngx_http_headers_out_t, etag), 0 },
	{ ngx_string("Server"), ngx_http_upstream_process_header_line, offsetof(ngx_http_upstream_headers_in_t, server), ngx_http_upstream_copy_header_line, offsetof(ngx_http_headers_out_t, server), 0 },
	...
	{ ngx_string("Set-Cookie"), ngx_http_upstream_process_set_cookie, offsetof(ngx_http_upstream_headers_in_t, cookies), ngx_http_upstream_rewrite_set_cookie, 0, 1 },
	{ ngx_string("Content-Disposition"), ngx_http_upstream_ignore_header_line, 0, ngx_http_upstream_copy_header_line, 0, 1 },
	{ ngx_string("Cache-Control"), ngx_http_upstream_process_cache_control, 0, ngx_http_upstream_copy_multi_header_lines, offsetof(ngx_http_headers_out_t, cache_control), 1 },
	...
	{ ngx_string("Accept-Ranges"), ngx_http_upstream_process_header_line, 
		offsetof(ngx_http_upstream_headers_in_t, accept_ranges), ngx_http_upstream_copy_allow_ranges, offsetof(ngx_http_headers_out_t, accept_ranges), 1 },
	{ ngx_string("Content-Range"), ngx_http_upstream_ignore_header_line, 0,
		ngx_http_upstream_copy_header_line, offsetof(ngx_http_headers_out_t, content_range), 0 },
...
#if (NGX_HTTP_GZIP)
	{ ngx_string("Content-Encoding"), ngx_http_upstream_process_header_line, offsetof(ngx_http_upstream_headers_in_t, content_encoding), ngx_http_upstream_copy_content_encoding, 0, 0 },
#endif
...
static ngx_http_variable_t ngx_http_upstream_vars[] = {
	...
	{ ngx_string("upstream_cache_last_modified"), NULL, ngx_http_upstream_cache_last_modified, 0, NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },
...
ngx_int_t ngx_http_upstream_create(ngx_http_request_t * r)
{
	...
	u = (ngx_http_upstream_t *)ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_t));
	if(u == NULL) {
		return NGX_ERROR;
	}
	r->upstream = u;
...
void ngx_http_upstream_init(ngx_http_request_t * pReq)
{
	ngx_connection_t * c = pReq->connection;
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http init upstream, client timer: %d", c->P_EvRd->timer_set);
#if (NGX_HTTP_V2)
	if(pReq->stream) {
		ngx_http_upstream_init_request(pReq);
		return;
	}
#endif
	...
	if(ngx_event_flags & NGX_USE_CLEAR_EVENT) {
		if(!c->P_EvWr->active) {
			if(ngx_add_event(c->P_EvWr, NGX_WRITE_EVENT, NGX_CLEAR_EVENT) == NGX_ERROR) {
...
static void ngx_http_upstream_init_request(ngx_http_request_t * pReq)
{
	ngx_str_t * host;
	ngx_uint_t i;
	...
	if(!pReq->aio) {
		ngx_http_upstream_t * p_upstr = pReq->upstream;
#if (NGX_HTTP_CACHE)
		if(p_upstr->conf->cache) {
			ngx_int_t rc = ngx_http_upstream_cache(pReq, p_upstr);
			if(rc == NGX_BUSY) {
				pReq->write_event_handler = ngx_http_upstream_init_request;
				return;
			}
			else {
				...
				if(rc == NGX_ERROR) {
					...
					if(rc == NGX_OK) {
						rc = ngx_http_upstream_cache_send(pReq, p_upstr);
						if(rc == NGX_DONE) {
							return;
						}
						else {
							...
							if(ngx_http_upstream_cache_background_update(pReq, p_upstr) != NGX_OK) {
								rc = NGX_ERROR;
							}
						}
					}
...
#endif
		p_upstr->store = p_upstr->conf->store;
		if(!p_upstr->store && !pReq->post_action && !p_upstr->conf->ignore_client_abort) {
			pReq->read_event_handler = ngx_http_upstream_rd_check_broken_connection;
			pReq->write_event_handler = ngx_http_upstream_wr_check_broken_connection;
		}
		if(pReq->request_body) {
			p_upstr->request_bufs = pReq->request_body->bufs;
		}
		if(p_upstr->create_request(pReq) != NGX_OK) {
			ngx_http_finalize_request(pReq, NGX_HTTP_INTERNAL_SERVER_ERROR);
			return;
		}
		if(ngx_http_upstream_set_local(pReq, p_upstr, p_upstr->conf->local) != NGX_OK) {
			ngx_http_finalize_request(pReq, NGX_HTTP_INTERNAL_SERVER_ERROR);
			return;
		}
		clcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(pReq, ngx_http_core_module);
		p_upstr->output.alignment = clcf->directio_alignment;
		p_upstr->output.pool = pReq->pool;
		p_upstr->output.bufs.num = 1;
		p_upstr->output.bufs.size = clcf->client_body_buffer_size;
		if(p_upstr->output.output_filter == NULL) {
			p_upstr->output.output_filter = ngx_chain_writer;
			p_upstr->output.filter_ctx = &p_upstr->writer;
		}
		p_upstr->writer.pool = pReq->pool;
		if(pReq->upstream_states == NULL) {
			pReq->upstream_states = ngx_array_create(pReq->pool, 1, sizeof(ngx_http_upstream_state_t));
			if(pReq->upstream_states == NULL) {
				ngx_http_finalize_request(pReq, NGX_HTTP_INTERNAL_SERVER_ERROR);
				return;
			}
			...
			p_upstr->state = (ngx_http_upstream_state_t *)ngx_array_push(pReq->upstream_states);
			if(p_upstr->state == NULL) {
				ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
				return;
			}
			memzero(p_upstr->state, sizeof(ngx_http_upstream_state_t));
		}
		cln = ngx_http_cleanup_add(pReq, 0);
		if(cln == NULL) {
			ngx_http_finalize_request(pReq, NGX_HTTP_INTERNAL_SERVER_ERROR);
			return;
		}
		...
		cln->data = pReq;
		p_upstr->cleanup = &cln->handler;
		if(p_upstr->resolved == NULL) {
			uscf = p_upstr->conf->upstream;
		}
		else {
#if (NGX_HTTP_SSL)
			p_upstr->ssl_name = p_upstr->resolved->host;
#endif
			host = &p_upstr->resolved->host;
			umcf = (ngx_http_upstream_main_conf_t *)ngx_http_get_module_main_conf(pReq, ngx_http_upstream_module);
			uscfp = (ngx_http_upstream_srv_conf_t **)umcf->upstreams.elts;
			for(i = 0; i < umcf->upstreams.nelts; i++) {
				uscf = uscfp[i];
				if(uscf->host.len == host->len && ((uscf->port == 0 && p_upstr->resolved->no_port) || uscf->port == p_upstr->resolved->port) && ngx_strncasecmp(uscf->host.data, host->data, host->len) == 0) {
					goto found;
				}
			}
			if(p_upstr->resolved->sockaddr) {
				if(p_upstr->resolved->port == 0 && p_upstr->resolved->sockaddr->sa_family != AF_UNIX) {
					ngx_log_error(NGX_LOG_ERR, pReq->connection->log, 0, "no port in upstream \"%V\"", host);
					ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
				}
				else if(ngx_http_upstream_create_round_robin_peer(pReq, p_upstr->resolved) != NGX_OK)
					ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
				else
					ngx_http_upstream_connect(pReq, p_upstr);
			}
			else if(p_upstr->resolved->port == 0) {
				ngx_log_error(NGX_LOG_ERR, pReq->connection->log, 0, "no port in upstream \"%V\"", host);
				ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
			}
			else {
				...
				if(!ctx)
					...
					ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_BAD_GATEWAY);
				}
				else {
					ctx->name = *host;
					...
					p_upstr->resolved->ctx = ctx;
					if(ngx_resolve_name(ctx) != NGX_OK) {
						p_upstr->resolved->ctx = NULL;
						ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
					}
				}
			}
...
found:
		if(uscf == NULL) {
			ngx_log_error(NGX_LOG_ALERT, pReq->connection->log, 0, "no upstream configuration");
			ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
		}
		else {
			p_upstr->upstream = uscf;
#if (NGX_HTTP_SSL)
			p_upstr->ssl_name = uscf->host;
#endif
			if(uscf->peer.init(pReq, uscf) != NGX_OK) {
				ngx_http_upstream_finalize_request(pReq, p_upstr, NGX_HTTP_INTERNAL_SERVER_ERROR);
			}
			else {
				p_upstr->peer.start_time = ngx_current_msec;
				if(p_upstr->conf->next_upstream_tries && p_upstr->peer.tries > p_upstr->conf->next_upstream_tries)
					p_upstr->peer.tries = p_upstr->conf->next_upstream_tries;
				ngx_http_upstream_connect(pReq, p_upstr);
			}
		}
	}
...
static ngx_int_t ngx_http_upstream_cache_send(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	if(rc == NGX_OK) {
		return (ngx_http_upstream_process_headers(r, u) != NGX_OK) ? NGX_DONE : ngx_http_cache_send(r);
	}
	if(rc == NGX_ERROR) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_upstream_cache_check_range(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	nginx_off_t offset;
	uchar * p, * start;
	ngx_table_elt_t  * h = r->headers_in.range;
	...
	if(h->value.len < 7 || ngx_strncasecmp(h->value.data, (uchar *)"bytes=", 6) != 0) {
		return NGX_OK;
	}
	p = h->value.data + 6;
	while(*p == ' ') {
...
static void ngx_http_upstream_resolve_handler(ngx_resolver_ctx_t * ctx)
{
	...
	ngx_http_request_t * r = (ngx_http_request_t *)ctx->data;
...
static void ngx_http_upstream_rd_check_broken_connection(ngx_http_request_t * r)
{
	ngx_http_upstream_check_broken_connection(r, r->connection->P_EvRd);
}

static void ngx_http_upstream_wr_check_broken_connection(ngx_http_request_t * r)
{
	ngx_http_upstream_check_broken_connection(r, r->connection->P_EvWr);
}
...
static void ngx_http_upstream_connect(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	u->state = (ngx_http_upstream_state_t *)ngx_array_push(r->upstream_states);
	if(u->state == NULL) {
		ngx_http_upstream_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
		return;
	}
	memzero(u->state, sizeof(ngx_http_upstream_state_t));
	...
	if(rc == NGX_AGAIN) {
		ngx_add_timer(c->P_EvWr, u->conf->connect_timeout);
		return;
	}
#if (NGX_HTTP_SSL)
	if(u->ssl && c->ssl == NULL) {
...
static void ngx_http_upstream_ssl_handshake(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_connection_t * c)
{
	...
	if(c->ssl->handshaked) {
		if(u->conf->ssl_verify) {
			...
			if(rc != X509_V_OK) {
				ngx_log_error(NGX_LOG_ERR, c->log, 0, "upstream SSL certificate verify error: (%l:%s)", rc, X509_verify_cert_error_string(rc));
				goto failed;
			}
...
static ngx_int_t ngx_http_upstream_ssl_name(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_connection_t * c)
{
	 ...
	 */
	p = (uchar *)ngx_pnalloc(r->pool, name.len + 1);
	if(!p) {
		return NGX_ERROR;
	}
...
static void ngx_http_upstream_send_request(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_uint_t do_write)
{
	...
	if(!u->request_sent && ngx_http_upstream_test_connect(c) != NGX_OK)
		...
		ngx_int_t rc = ngx_http_upstream_send_request_body(r, u, do_write);
		if(rc == NGX_ERROR)
			ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
			...
			if(!c->P_EvWr->ready) {
				ngx_add_timer(c->P_EvWr, u->conf->send_timeout);
			}
			else if(c->P_EvWr->timer_set) {
				ngx_del_timer(c->P_EvWr);
			}
			if(ngx_handle_write_event(c->P_EvWr, u->conf->send_lowat) != NGX_OK)
				ngx_http_upstream_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
		}
		else {
			/* rc == NGX_OK */
			...
			if(ngx_handle_write_event(c->P_EvWr, 0) != NGX_OK) {
				...
				ngx_add_timer(c->P_EvRd, u->conf->read_timeout);
				if(c->P_EvRd->ready)
...
static void ngx_http_upstream_send_request_handler(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	if(c->P_EvWr->timedout) {
		...
		if(u->header_sent) {
			u->write_event_handler = ngx_http_upstream_dummy_handler;
			(void)ngx_handle_write_event(c->P_EvWr, 0);
		}
		else
			ngx_http_upstream_send_request(r, u, 1);
	}
...
static void ngx_http_upstream_process_header(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	else if(!u->request_sent && ngx_http_upstream_test_connect(c) != NGX_OK) {
		ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
	}
	else {
		if(u->buffer.start == NULL) {
			u->buffer.start = (uchar *)ngx_palloc(r->pool, u->conf->buffer_size);
			if(u->buffer.start == NULL) {
				ngx_http_upstream_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
				return;
			}
...
static ngx_int_t ngx_http_upstream_test_next(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	const ngx_uint_t status = u->headers_in.status_n;
...
static ngx_int_t ngx_http_upstream_intercept_errors(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	ngx_http_core_loc_conf_t  * clcf;
	ngx_int_t status = u->headers_in.status_n;
...
#if (NGX_HTTP_CACHE)
			if(r->cache) {
				time_t valid = ngx_http_file_cache_valid(u->conf->cache_valid, status);
				if(valid) {
...
static ngx_int_t ngx_http_upstream_process_headers(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	if(u->headers_in.x_accel_redirect && !(u->conf->ignore_headers & NGX_HTTP_UPSTREAM_IGN_XA_REDIRECT)) {
		...
		for(i = 0; /* void */; i++) {
			...
			hh = (ngx_http_upstream_header_t *)ngx_hash_find(&umcf->headers_in_hash, h[i].hash, h[i].lowcase_key, h[i].key.len);
			if(hh && hh->redirect) {
...
static void ngx_http_upstream_process_body_in_memory(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "http upstream process body in memory");
	if(rev->timedout) {
...
static void ngx_http_upstream_send_response(ngx_http_request_t * pReq, ngx_http_upstream_t * pUpstr)
{
	ssize_t n;
	ngx_event_pipe_t * p;
	ngx_connection_t * c;
	...
	if(rc == NGX_ERROR || rc > NGX_OK || pReq->post_action) {
		ngx_http_upstream_finalize_request(pReq, pUpstr, rc);
		return;
	}
	pUpstr->header_sent = 1;
	if(pUpstr->upgrade) {
#if (NGX_HTTP_CACHE)
		if(pReq->cache) {
			ngx_http_file_cache_free(pReq->cache, pUpstr->pipe->temp_file);
		}
#endif
		ngx_http_upstream_upgrade(pReq, pUpstr);
		return;
	}
	c = pReq->connection;
	if(pReq->header_only) {
		...
		if(!pUpstr->cacheable && !pUpstr->store) {
			ngx_http_upstream_finalize_request(pReq, pUpstr, rc);
			return;
		}
		pUpstr->pipe->downstream_error = 1;
	}
	if(pReq->request_body && pReq->request_body->temp_file && pReq == pReq->main && !pReq->preserve_body) {
		ngx_pool_run_cleanup_file(pReq->pool, pReq->request_body->temp_file->file.fd);
		pReq->request_body->temp_file->file.fd = NGX_INVALID_FILE;
	}
	clcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(pReq, ngx_http_core_module);
	if(!pUpstr->buffering) {
#if (NGX_HTTP_CACHE)
...
#endif
		if(pUpstr->input_filter == NULL) {
			pUpstr->input_filter_init = ngx_http_upstream_non_buffered_filter_init;
			pUpstr->input_filter = ngx_http_upstream_non_buffered_filter;
			pUpstr->input_filter_ctx = pReq;
		}
		pUpstr->read_event_handler = ngx_http_upstream_process_non_buffered_upstream;
		pReq->write_event_handler = ngx_http_upstream_process_non_buffered_downstream;
		pReq->limit_rate = 0;
		if(pUpstr->input_filter_init(pUpstr->input_filter_ctx) == NGX_ERROR) {
			ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
			return;
		}
		if(clcf->tcp_nodelay && ngx_tcp_nodelay(c) != NGX_OK) {
			ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
		...
		n = pUpstr->buffer.last - pUpstr->buffer.pos;
		if(n) {
			pUpstr->buffer.last = pUpstr->buffer.pos;
			pUpstr->state->response_length += n;
			if(pUpstr->input_filter(pUpstr->input_filter_ctx, n) == NGX_ERROR) {
				ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
				return;
			}
			ngx_http_upstream_process_non_buffered_downstream(pReq);
		}
		else {
			pUpstr->buffer.pos = pUpstr->buffer.start;
			pUpstr->buffer.last = pUpstr->buffer.start;
			if(ngx_http_send_special(pReq, NGX_HTTP_FLUSH) == NGX_ERROR) {
				ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
				return;
			}
			if(pUpstr->peer.connection->P_EvRd->ready || pUpstr->length == 0) {
				ngx_http_upstream_process_non_buffered_upstream(pReq, pUpstr);
			}
		}
		return;
	}
	...
	if(pReq->cache && pReq->cache->file.fd != NGX_INVALID_FILE) {
		...
		pReq->cache->file.fd = NGX_INVALID_FILE;
	}
	switch(ngx_http_test_predicates(pReq, pUpstr->conf->no_cache)) {
		case NGX_ERROR:
		    ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
		    return;
		case NGX_DECLINED:
		    ...
		    break;
		default: /* NGX_OK */
		    if(pUpstr->cache_status == NGX_HTTP_CACHE_BYPASS) {
			    /* create cache if previously bypassed */
			    if(ngx_http_file_cache_create(pReq) != NGX_OK) {
				    ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
				    return;
			    }
		    }
	...
	if(pUpstr->cacheable) {
		time_t now = ngx_time();
		time_t valid = pReq->cache->valid_sec;
		if(valid == 0) {
			valid = ngx_http_file_cache_valid(pUpstr->conf->cache_valid, pUpstr->headers_in.status_n);
			if(valid) {
				pReq->cache->valid_sec = now + valid;
			}
		}
		if(valid) {
			pReq->cache->date = now;
			pReq->cache->body_start = (ushort)(pUpstr->buffer.pos - pUpstr->buffer.start);
			if(pUpstr->headers_in.status_n == NGX_HTTP_OK || pUpstr->headers_in.status_n == NGX_HTTP_PARTIAL_CONTENT) {
				pReq->cache->last_modified = pUpstr->headers_in.last_modified_time;
				if(pUpstr->headers_in.etag) {
					pReq->cache->etag = pUpstr->headers_in.etag->value;
				}
				else {
					ngx_str_null(&pReq->cache->etag);
				}
			}
			else {
				pReq->cache->last_modified = -1;
				ngx_str_null(&pReq->cache->etag);
			}
			if(ngx_http_file_cache_set_header(pReq, pUpstr->buffer.start) != NGX_OK) {
				ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
				return;
			}
		}
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http cacheable: %d", pUpstr->cacheable);
	if(pUpstr->cacheable == 0 && pReq->cache) {
		ngx_http_file_cache_free(pReq->cache, pUpstr->pipe->temp_file);
	}
	if(pReq->header_only && !pUpstr->cacheable && !pUpstr->store) {
		ngx_http_upstream_finalize_request(pReq, pUpstr, 0);
		return;
	}
#endif
	p = pUpstr->pipe;
	...
	p->output_ctx = pReq;
	p->tag = pUpstr->output.tag;
	p->bufs = pUpstr->conf->bufs;
	p->busy_size = pUpstr->conf->busy_buffers_size;
	p->upstream = pUpstr->peer.connection;
	p->downstream = c;
	p->pool = pReq->pool;
	p->log = c->log;
	p->limit_rate = pUpstr->conf->limit_rate;
	p->start_sec = ngx_time();
	p->cacheable = pUpstr->cacheable || pUpstr->store;
	p->temp_file = (ngx_temp_file_t *)ngx_pcalloc(pReq->pool, sizeof(ngx_temp_file_t));
	if(p->temp_file == NULL) {
		ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
		return;
	}
	...
	p->temp_file->path = pUpstr->conf->temp_path;
	p->temp_file->pool = pReq->pool;
	if(p->cacheable) {
		p->temp_file->persistent = 1;
#if (NGX_HTTP_CACHE)
		if(pReq->cache && !pReq->cache->file_cache->use_temp_path) {
			p->temp_file->path = pReq->cache->file_cache->path;
			p->temp_file->file.name = pReq->cache->file.name;
		}
#endif
	}
	else {
	...
	p->max_temp_file_size = pUpstr->conf->max_temp_file_size;
	p->temp_file_write_size = pUpstr->conf->temp_file_write_size;
#if (NGX_THREADS)
...
#endif
	p->preread_bufs = ngx_alloc_chain_link(pReq->pool);
	if(p->preread_bufs == NULL) {
		ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
		return;
	}
	p->preread_bufs->buf = &pUpstr->buffer;
	p->preread_bufs->next = NULL;
	pUpstr->buffer.recycled = 1;
	p->preread_size = pUpstr->buffer.last - pUpstr->buffer.pos;
	if(pUpstr->cacheable) {
		p->buf_to_file = (ngx_buf_t *)ngx_calloc_buf(pReq->pool);
		if(p->buf_to_file == NULL) {
			ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
			return;
		}
		p->buf_to_file->start = pUpstr->buffer.start;
		p->buf_to_file->pos = pUpstr->buffer.start;
		p->buf_to_file->last = pUpstr->buffer.pos;
	 ...
	 */
	pUpstr->buffer.last = pUpstr->buffer.pos;
	if(pUpstr->conf->cyclic_temp_file) {
		/*
	...
	p->read_timeout = pUpstr->conf->read_timeout;
	p->send_timeout = clcf->send_timeout;
	p->send_lowat = clcf->send_lowat;
	p->length = -1;
	if(pUpstr->input_filter_init && pUpstr->input_filter_init(p->input_ctx) != NGX_OK) {
		ngx_http_upstream_finalize_request(pReq, pUpstr, NGX_ERROR);
		return;
	}
	pUpstr->read_event_handler = ngx_http_upstream_process_upstream;
	pReq->write_event_handler = ngx_http_upstream_process_downstream;
	ngx_http_upstream_process_upstream(pReq, pUpstr);
}
...
static void ngx_http_upstream_upgrade(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	else if(u->peer.connection->P_EvRd->ready || u->buffer.pos != u->buffer.last) {
...
static void ngx_http_upstream_process_upgraded(ngx_http_request_t * r, ngx_uint_t from_upstream, ngx_uint_t do_write)
{
	...
	ssize_t n;
	ngx_buf_t * b;
	ngx_connection_t   * downstream, * upstream, * dst, * src;
	ngx_http_core_loc_conf_t  * clcf;
	ngx_connection_t   * c = r->connection;
	ngx_http_upstream_t  * u = r->upstream;
	...
	if(downstream->P_EvWr->timedout) {
		c->timedout = 1;
		ngx_connection_error(c, NGX_ETIMEDOUT, "client timed out");
		ngx_http_upstream_finalize_request(r, u, NGX_HTTP_REQUEST_TIME_OUT);
		return;
	}
	if(upstream->P_EvRd->timedout || upstream->P_EvWr->timedout) {
		ngx_connection_error(c, NGX_ETIMEDOUT, "upstream timed out");
		ngx_http_upstream_finalize_request(r, u, NGX_HTTP_GATEWAY_TIME_OUT);
		return;
	}
	if(from_upstream) {
		...
		if(b->start == NULL) {
			b->start = (uchar *)ngx_palloc(r->pool, u->conf->buffer_size);
			if(b->start == NULL) {
				ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
				return;
			}
	...
	clcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_core_module);
	if(ngx_handle_write_event(upstream->P_EvWr, u->conf->send_lowat) != NGX_OK) {
		ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
		return;
	}
	if(upstream->P_EvWr->active && !upstream->P_EvWr->ready) {
		ngx_add_timer(upstream->P_EvWr, u->conf->send_timeout);
	}
	else if(upstream->P_EvWr->timer_set) {
		ngx_del_timer(upstream->P_EvWr);
	}
	if(ngx_handle_read_event(upstream->P_EvRd, 0) != NGX_OK) {
		ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
		return;
	}
	if(upstream->P_EvRd->active && !upstream->P_EvRd->ready) {
		ngx_add_timer(upstream->P_EvRd, u->conf->read_timeout);
	}
	else if(upstream->P_EvRd->timer_set) {
		ngx_del_timer(upstream->P_EvRd);
	}
	if(ngx_handle_write_event(downstream->P_EvWr, clcf->send_lowat) != NGX_OK) {
		ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
		return;
	}
	if(ngx_handle_read_event(downstream->P_EvRd, 0) != NGX_OK) {
		ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
		return;
	}
	if(downstream->P_EvWr->active && !downstream->P_EvWr->ready) {
		ngx_add_timer(downstream->P_EvWr, clcf->send_timeout);
	}
	else if(downstream->P_EvWr->timer_set) {
		ngx_del_timer(downstream->P_EvWr);
	}
}
...
static void ngx_http_upstream_process_non_buffered_request(ngx_http_request_t * r, ngx_uint_t do_write)
{
	...
	ngx_http_core_loc_conf_t * clcf;
	ngx_http_upstream_t * u = r->upstream;
	ngx_connection_t * downstream = r->connection;
	...
	for(;;) {
		if(do_write) {
			...
			if(u->busy_bufs == NULL) {
				if(u->length == 0 || (upstream->P_EvRd->eof && u->length == -1)) {
					ngx_http_upstream_finalize_request(r, u, 0);
					return;
				}
				if(upstream->P_EvRd->eof) {
					ngx_log_error(NGX_LOG_ERR, upstream->log, 0, "upstream prematurely closed connection");
					ngx_http_upstream_finalize_request(r, u, NGX_HTTP_BAD_GATEWAY);
					return;
				}
				if(upstream->P_EvRd->error) {
					ngx_http_upstream_finalize_request(r, u, NGX_HTTP_BAD_GATEWAY);
					return;
				}
				b->pos = b->start;
...
static ngx_int_t ngx_http_upstream_thread_handler(ngx_thread_task_t * task, ngx_file_t * file)
{
	...
	ngx_event_pipe_t   * p = r->upstream->pipe;
...
static void ngx_http_upstream_process_upstream(ngx_http_request_t * pReq, ngx_http_upstream_t * u)
{
	ngx_connection_t  * c = u->peer.connection;
	ngx_event_pipe_t  * p = u->pipe;
	ngx_event_t  * rev = c->P_EvRd;
	...
	if(rev->timedout) {
		...
		if(rev->delayed) {
			...
			if(ngx_handle_read_event(rev, 0) != NGX_OK) {
				ngx_http_upstream_finalize_request(pReq, u, NGX_ERROR);
			}
			return;
		}
	...
	ngx_http_upstream_process_request(pReq, u);
}
	...
	if(u->peer.connection) {
		...
		if(u->cacheable) {
			if(p->upstream_done) {
				...
				tf = p->temp_file;
				if(p->length == -1 && (u->headers_in.content_length_n == -1 || u->headers_in.content_length_n == tf->offset - (nginx_off_t)r->cache->body_start)) {
...
static void ngx_http_upstream_store(ngx_http_request_t * r, ngx_http_upstream_t * u)
{
	...
	ngx_temp_file_t * tf = u->pipe->temp_file;
...
static void ngx_http_upstream_next(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_uint_t ft_type)
{
	...
	if(u->peer.sockaddr) {
		if(oneof2(ft_type, NGX_HTTP_UPSTREAM_FT_HTTP_403, NGX_HTTP_UPSTREAM_FT_HTTP_404)) {
			state = NGX_PEER_NEXT;
		}
		else {
	...
	switch(ft_type) {
		...
		case NGX_HTTP_UPSTREAM_FT_HTTP_429: status = NGX_HTTP_TOO_MANY_REQUESTS; break;
		// NGX_HTTP_UPSTREAM_FT_BUSY_LOCK and NGX_HTTP_UPSTREAM_FT_MAX_WAITING never reach here
		default: status = NGX_HTTP_BAD_GATEWAY; break;
	}
...
static void ngx_http_upstream_finalize_request(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_int_t rc)
{
	...
	*u->cleanup = NULL;
	...
	if(r->cache) {
		if(u->cacheable) {
			if(rc == NGX_HTTP_BAD_GATEWAY || rc == NGX_HTTP_GATEWAY_TIME_OUT) {
				time_t valid = ngx_http_file_cache_valid(u->conf->cache_valid, rc);
				if(valid) {
					r->cache->valid_sec = ngx_time() + valid;
					r->cache->error = rc;
				}
	...
	if(!u->header_sent || rc == NGX_HTTP_REQUEST_TIME_OUT || rc == NGX_HTTP_CLIENT_CLOSED_REQUEST || (u->pipe && u->pipe->downstream_error)) {
...
static ngx_int_t ngx_http_upstream_process_set_cookie(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	ngx_table_elt_t      ** ph;
	ngx_http_upstream_t * u = r->upstream;
	ngx_array_t * pa = &u->headers_in.cookies;
	if(pa->elts == NULL) {
		if(ngx_array_init(pa, r->pool, 1, sizeof(ngx_table_elt_t *)) != NGX_OK) {
			return NGX_ERROR;
		}
	}
	ph = (ngx_table_elt_t **)ngx_array_push(pa);
	if(ph == NULL) {
		return NGX_ERROR;
	}
	*ph = h;
...
static ngx_int_t ngx_http_upstream_process_cache_control(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	ngx_table_elt_t     ** ph;
	ngx_http_upstream_t  * u = r->upstream;
	ngx_array_t   * pa = &u->headers_in.cache_control;
	if(pa->elts == NULL) {
	...
	*ph = h;
#if (NGX_HTTP_CACHE)
	{
		...
		if(ngx_strlcasestrn(start, last, (uchar *)"no-cache", 8 - 1) != NULL || ngx_strlcasestrn(start, last, (uchar *)"no-store", 8 - 1) != NULL 
			|| ngx_strlcasestrn(start, last, (uchar *)"private", 7 - 1) != NULL) {
			u->cacheable = 0;
			return NGX_OK;
		}
		p = ngx_strlcasestrn(start, last, (uchar *)"s-maxage=", 9 - 1);
		offset = 9;
		if(!p) {
			p = ngx_strlcasestrn(start, last, (uchar *)"max-age=", 8 - 1);
			offset = 8;
		}
		if(p) {
			n = 0;
		...
		p = ngx_strlcasestrn(start, last, (uchar *)"stale-while-revalidate=", 23 - 1);
		if(p) {
			n = 0;
			for(p += 23; p < last; p++) {
				if(oneof3(*p, ',', ';', ' ')) {
		...
		p = ngx_strlcasestrn(start, last, (uchar *)"stale-if-error=", 15 - 1);
		if(p) {
			n = 0;
			for(p += 15; p < last; p++) {
				if(oneof3(*p, ',', ';', ' ')) {
...
static ngx_int_t ngx_http_upstream_process_buffering(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	if(u->conf->change_buffering) {
		if(h->value.len == 2) {
			c0 = stolower_ascii(h->value.data[0]);
			c1 = stolower_ascii(h->value.data[1]);
			if(c0 == 'n' && c1 == 'o') {
				u->buffering = 0;
			}
		}
			...
			c0 = stolower_ascii(h->value.data[0]);
			c1 = stolower_ascii(h->value.data[1]);
			c2 = stolower_ascii(h->value.data[2]);
			if(c0 == 'y' && c1 == 'e' && c2 == 's') {
				u->buffering = 1;
			}
		}
...
static ngx_int_t ngx_http_upstream_process_connection(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	if(ngx_strlcasestrn(h->value.data, h->value.data + h->value.len, (uchar *)"close", 5 - 1) != NULL) {
...
static ngx_int_t ngx_http_upstream_process_transfer_encoding(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	if(ngx_strlcasestrn(h->value.data, h->value.data + h->value.len, (uchar *)"chunked", 7 - 1) != NULL) {
...
static ngx_int_t ngx_http_upstream_copy_content_type(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	for(uchar * p = h->value.data; *p; p++) {
		if(*p == ';') {
			...
			else if(ngx_strncasecmp(p, (uchar *)"charset=", 8) == 0) {
				p += 8;
...
static ngx_int_t ngx_http_upstream_rewrite_refresh(ngx_http_request_t * r, ngx_table_elt_t * h, ngx_uint_t offset)
{
	...
	if(ho == NULL) {
		...
		*ho = *h;
		if(r->upstream->rewrite_redirect) {
			const uchar * p = ngx_strcasestrn(ho->value.data, "url=", 4 - 1);
			if(p) {
				ngx_int_t rc = r->upstream->rewrite_redirect(r, ho, p + 4 - ho->value.data);
				if(rc == NGX_DECLINED) {
					return NGX_OK;
				}
				else {
...
static ngx_int_t ngx_http_upstream_add_variables(ngx_conf_t * cf)
{
	for(ngx_http_variable_t * v = ngx_http_upstream_vars; v->name.len; v++) {
...
static ngx_int_t ngx_http_upstream_cache_status(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	if(r->upstream == NULL || r->upstream->cache_status == 0) {
		...
		ngx_uint_t n = r->upstream->cache_status - 1;
		v->valid = 1;
		v->no_cacheable = 0;
		v->not_found = 0;
...
static ngx_int_t ngx_http_upstream_cache_last_modified(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	if(r->upstream == NULL || !r->upstream->conf->cache_revalidate || r->upstream->cache_status != NGX_HTTP_CACHE_EXPIRED || r->cache->last_modified == -1) {
		...
		uchar  * p = (uchar *)ngx_pnalloc(r->pool, sizeof("Mon, 28 Sep 1970 06:00:00 GMT") - 1);
		if(!p) {
			return NGX_ERROR;
		}
		else {
...
static const char * ngx_http_upstream(ngx_conf_t * cf, const ngx_command_t * cmd, void * dummy) // F_SetHandler
{
	...
	u.no_resolve = 1;
	u.no_port = 1;
	uscf = ngx_http_upstream_add(cf, &u, NGX_HTTP_UPSTREAM_CREATE|NGX_HTTP_UPSTREAM_WEIGHT|
		NGX_HTTP_UPSTREAM_MAX_CONNS|NGX_HTTP_UPSTREAM_MAX_FAILS|NGX_HTTP_UPSTREAM_FAIL_TIMEOUT|NGX_HTTP_UPSTREAM_DOWN|NGX_HTTP_UPSTREAM_BACKUP);
	if(uscf == NULL) {
...
static const char * ngx_http_upstream_server(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	ngx_http_upstream_srv_conf_t  * uscf = (ngx_http_upstream_srv_conf_t *)conf;
	time_t fail_timeout;
	ngx_str_t * value, s;
	ngx_url_t u;
	ngx_int_t weight, max_conns, max_fails;
	ngx_uint_t i;
	ngx_http_upstream_server_t  * us = (ngx_http_upstream_server_t *)ngx_array_push(uscf->servers);
	if(us == NULL) {
		return (char *)NGX_CONF_ERROR;
	}
...
ngx_http_upstream_srv_conf_t * ngx_http_upstream_add(ngx_conf_t * cf, ngx_url_t * u, ngx_uint_t flags)
{
	...
	uscf = (ngx_http_upstream_srv_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_srv_conf_t));
	if(uscf) {
		uscf->flags = flags;
		uscf->host = u->host;
		...
		if(u->naddrs == 1 && (u->port || u->family == AF_UNIX)) {
			...
			us = (ngx_http_upstream_server_t *)ngx_array_push(uscf->servers);
			if(us == NULL) {
				return NULL;
			}
			memzero(us, sizeof(ngx_http_upstream_server_t));
		...
		uscfp = (ngx_http_upstream_srv_conf_t **)ngx_array_push(&umcf->upstreams);
		if(uscfp == NULL) {
			return NULL;
		}
		*uscfp = uscf;
...
const char * ngx_http_upstream_bind_set_slot(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	ngx_http_upstream_local_t ** plocal = (ngx_http_upstream_local_t**)(p + cmd->offset);
	...
	local = (ngx_http_upstream_local_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_local_t));
	if(local == NULL) {
		return (char *)NGX_CONF_ERROR;
	}
	*plocal = local;
	if(cv.lengths) {
		local->value = (ngx_http_complex_value_t *)ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
		if(local->value == NULL) {
			return (char *)NGX_CONF_ERROR;
		}
		*local->value = cv;
...
static ngx_int_t ngx_http_upstream_set_local(ngx_http_request_t * r, ngx_http_upstream_t * u, ngx_http_upstream_local_t * local)
{
	...
	addr = (ngx_addr_t *)ngx_palloc(r->pool, sizeof(ngx_addr_t));
	if(addr == NULL) {
		return NGX_ERROR;
	}
...
const char * ngx_http_upstream_param_set_slot(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	param = (ngx_http_upstream_param_t *)ngx_array_push(*a);
	if(param == NULL) {
	...
	if(conf->hide_headers != NGX_CONF_UNSET_PTR) {
		...
		for(i = 0; i < conf->hide_headers->nelts; i++) {
			...
			for(j = 0; j < hide_headers.nelts; j++) {
				if(sstreqi_ascii(h[i].data, hk[j].key.data)) {
					goto exist;
				}
			}
			hk = (ngx_hash_key_t *)ngx_array_push(&hide_headers);
...
exist:
	...
	hash->pool = cf->pool;
	hash->temp_pool = NULL;
	if(ngx_hash_init(hash, (ngx_hash_key_t *)hide_headers.elts, hide_headers.nelts) != NGX_OK) {
		return NGX_ERROR;
	}
	/*
...
static void * ngx_http_upstream_create_main_conf(ngx_conf_t * cf)
{
	ngx_http_upstream_main_conf_t  * umcf = (ngx_http_upstream_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_main_conf_t));
	if(umcf) {
...
static char * ngx_http_upstream_init_main_conf(ngx_conf_t * cf, void * conf)
{
	ngx_http_upstream_main_conf_t * umcf = (ngx_http_upstream_main_conf_t *)conf;
	ngx_uint_t i;
	ngx_array_t headers_in;
	ngx_hash_init_t hash;
	ngx_http_upstream_srv_conf_t ** uscfp = (ngx_http_upstream_srv_conf_t **)umcf->upstreams.elts;
	for(i = 0; i < umcf->upstreams.nelts; i++) {
		ngx_http_upstream_init_pt init = uscfp[i]->peer.init_upstream ? uscfp[i]->peer.init_upstream : ngx_http_upstream_init_round_robin;
