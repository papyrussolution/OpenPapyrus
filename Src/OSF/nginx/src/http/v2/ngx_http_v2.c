...
static uchar * ngx_http_v2_state_proxy_protocol(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_preface(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_preface_end(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_head(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_data(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_read_data(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_headers(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_header_block(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_field_len(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_field_huff(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_field_raw(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_field_skip(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_process_header(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_header_complete(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_handle_continuation(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end, ngx_http_v2_handler_pt handler);
static uchar * ngx_http_v2_state_priority(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_rst_stream(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_settings(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_settings_params(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_push_promise(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_ping(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_goaway(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_window_update(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_continuation(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_complete(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_skip_padded(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_skip(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end);
static uchar * ngx_http_v2_state_save(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end, ngx_http_v2_handler_pt handler);
static uchar * ngx_http_v2_state_headers_save(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end, ngx_http_v2_handler_pt handler);
static uchar * ngx_http_v2_connection_error(ngx_http_v2_connection_t * h2c, ngx_uint_t err);
static ngx_int_t ngx_http_v2_parse_int(ngx_http_v2_connection_t * h2c, uchar ** pos, uchar * end, ngx_uint_t prefix);
...
static ngx_http_v2_out_frame_t * ngx_http_v2_get_frame(ngx_http_v2_connection_t * h2c, size_t length, ngx_uint_t type, uchar flags, ngx_uint_t sid);
...
static ngx_int_t ngx_http_v2_parse_path(ngx_http_request_t * r, ngx_http_v2_header_t * header);
static ngx_int_t ngx_http_v2_parse_method(ngx_http_request_t * r, ngx_http_v2_header_t * header);
static ngx_int_t ngx_http_v2_parse_scheme(ngx_http_request_t * r, ngx_http_v2_header_t * header);
static ngx_int_t ngx_http_v2_parse_authority(ngx_http_request_t * r, ngx_http_v2_header_t * header);
...
static ngx_int_t ngx_http_v2_process_request_body(ngx_http_request_t * r, uchar * pos, size_t size, ngx_uint_t last);
...
void ngx_http_v2_init(ngx_event_t * rev)
{
	...
	if(h2mcf->recv_buffer == NULL) {
		h2mcf->recv_buffer = (uchar *)ngx_palloc(ngx_cycle->pool, h2mcf->recv_buffer_size);
		if(h2mcf->recv_buffer == NULL) {
			ngx_http_close_connection(c);
			return;
		}
	}
	h2c = (ngx_http_v2_connection_t *)ngx_pcalloc(c->pool, sizeof(ngx_http_v2_connection_t));
	if(h2c == NULL) {
		ngx_http_close_connection(c);
		return;
	}
	...
	h2scf = (ngx_http_v2_srv_conf_t *)ngx_http_get_module_srv_conf(hc->conf_ctx, ngx_http_v2_module);
	...
	cln->data = h2c;
	h2c->streams_index = (ngx_http_v2_node_t **)ngx_pcalloc(c->pool, ngx_http_v2_index_size(h2scf) * sizeof(ngx_http_v2_node_t *));
...
static void ngx_http_v2_read_handler(ngx_event_t * rev)
{
	...
	h2mcf = (ngx_http_v2_main_conf_t *)ngx_http_get_module_main_conf(h2c->http_connection->conf_ctx, ngx_http_v2_module);
...
ngx_int_t ngx_http_v2_send_output_queue(ngx_http_v2_connection_t * h2c)
{
	...
	clcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(h2c->http_connection->conf_ctx,
...
static void ngx_http_v2_handle_connection(ngx_http_v2_connection_t * h2c)
{
	...
	h2scf = (ngx_http_v2_srv_conf_t *)ngx_http_get_module_srv_conf(h2c->http_connection->conf_ctx,
	    ngx_http_v2_module);
...
static uchar * ngx_http_v2_state_preface(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	static const uchar preface[] = "PRI * HTTP/2.0\r\n";
...
static uchar * ngx_http_v2_state_data(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	size_t size = h2c->state.length;
	if(h2c->state.flags & NGX_HTTP_V2_PADDED_FLAG) {
		if(h2c->state.length == 0) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent padded DATA frame with incorrect length: 0");
		...
		if(h2c->state.padding >= size) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent padded DATA frame with incorrect length: %uz, padding: %uz",
	...
	if(size > h2c->recv_window) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client violated connection flow control: received DATA frame length %uz, available window %uz",
	...
	if(size > stream->recv_window) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client violated flow control for stream %ui: received DATA frame length %uz, available window %uz",
...
static uchar * ngx_http_v2_state_headers(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	if(padded) {
		...
		if(h2c->state.padding > h2c->state.length) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent padded HEADERS frame with incorrect length: %uz, padding: %uz", h2c->state.length, h2c->state.padding);
			return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_PROTOCOL_ERROR);
		}
	...
	ngx_log_debug4(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 HEADERS frame sid:%ui on %ui excl:%ui weight:%ui", h2c->state.sid, depend, excl, weight);
	if(h2c->state.sid % 2 == 0 || h2c->state.sid <= h2c->last_sid) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent HEADERS frame with incorrect identifier %ui, the last was %ui", h2c->state.sid, h2c->last_sid);
		return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_PROTOCOL_ERROR);
	}
	...
	if(depend == h2c->state.sid) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent HEADERS frame for stream %ui with incorrect dependency", h2c->state.sid);
	...
	if(!h2c->settings_ack && !(h2c->state.flags & NGX_HTTP_V2_END_STREAM_FLAG) && h2scf->preread_size < NGX_HTTP_V2_DEFAULT_WINDOW) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent stream with data before settings were acknowledged");
		status = NGX_HTTP_V2_REFUSED_STREAM;
		goto rst_stream;
	}
...
static uchar * ngx_http_v2_state_field_len(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 hpack %s string length: %i", huff ? "encoded" : "raw", len);
	h2scf = (ngx_http_v2_srv_conf_t *)ngx_http_get_module_srv_conf(h2c->http_connection->conf_ctx, ngx_http_v2_module);
	...
	h2c->state.field_start = (uchar *)ngx_pnalloc(h2c->state.pool, alloc);
...
static uchar * ngx_http_v2_state_field_huff(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	if(ngx_http_v2_huff_decode(&h2c->state.field_state, pos, size, &h2c->state.field_end,
		    h2c->state.field_rest == 0, h2c->connection->log) != NGX_OK) {
...
static uchar * ngx_http_v2_state_process_header(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	ngx_http_v2_header_t  * header = &h2c->state.header;
	...
	if(header->name.len == cookie.len && memcmp(header->name.data, cookie.data, cookie.len) == 0) {
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2 http header: \"%V: %V\"", &header->name, &header->value);
	return ngx_http_v2_state_header_complete(h2c, pos, end);
error:
...
static uchar * ngx_http_v2_state_priority(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	ngx_log_debug4(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 PRIORITY frame sid:%ui on %ui excl:%ui weight:%ui", h2c->state.sid, depend, excl, weight);
	if(h2c->state.sid == 0) {
...
static uchar * ngx_http_v2_state_settings(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	if(h2c->state.flags == NGX_HTTP_V2_ACK_FLAG) {
		if(h2c->state.length != 0) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent SETTINGS frame with the ACK flag and nonzero length");
			return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_SIZE_ERROR);
		}
		h2c->settings_ack = 1;
		return ngx_http_v2_state_complete(h2c, pos, end);
		...
		switch(id) {
			case NGX_HTTP_V2_INIT_WINDOW_SIZE_SETTING:
			    if(value > NGX_HTTP_V2_MAX_WINDOW) {
				    ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent SETTINGS frame with incorrect INITIAL_WINDOW_SIZE value %ui", value);
			...
			case NGX_HTTP_V2_MAX_FRAME_SIZE_SETTING:
			    if(value > NGX_HTTP_V2_MAX_FRAME_SIZE || value < NGX_HTTP_V2_DEFAULT_FRAME_SIZE) {
				    ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent SETTINGS frame with incorrect MAX_FRAME_SIZE value %ui", value);
				    return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_PROTOCOL_ERROR);
			    }
...
static uchar * ngx_http_v2_state_ping(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 PING frame, flags: %ud", h2c->state.flags);
	if(h2c->state.flags & NGX_HTTP_V2_ACK_FLAG) {
		return ngx_http_v2_state_skip(h2c, pos, end);
	}
...
static uchar * ngx_http_v2_state_goaway(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2 GOAWAY frame: last sid %ui, error %ui", last_sid, error);
#endif
	return ngx_http_v2_state_skip(h2c, pos, end);
}
...
static uchar * ngx_http_v2_state_window_update(ngx_http_v2_connection_t * h2c, uchar * pos, uchar * end)
{
	...
	if(h2c->state.length != NGX_HTTP_V2_WINDOW_UPDATE_SIZE) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent WINDOW_UPDATE frame with incorrect length %uz", h2c->state.length);
		return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_SIZE_ERROR);
	}
	...
	if(window == 0) {
		if(h2c->state.sid == 0) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent WINDOW_UPDATE frame with incorrect window increment 0");
			return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_PROTOCOL_ERROR);
		}
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client sent WINDOW_UPDATE frame for stream %ui with incorrect window increment 0", h2c->state.sid);
		node = ngx_http_v2_get_node_by_id(h2c, h2c->state.sid, 0);
		if(node && node->stream) {
			if(ngx_http_v2_terminate_stream(h2c, node->stream, NGX_HTTP_V2_PROTOCOL_ERROR) == NGX_ERROR) {
				return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
	...
	if(h2c->state.sid) {
		...
		if(window > (size_t)(NGX_HTTP_V2_MAX_WINDOW - stream->send_window)) {
			ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client violated flow control for stream %ui: received WINDOW_UPDATE frame with window increment %uz not allowed for window %z",
		...
		if(stream->exhausted) {
			stream->exhausted = 0;
			wev = stream->request->connection->P_EvWr;
			wev->active = 0;
			wev->ready = 1;
			if(!wev->delayed) {
				wev->F_EvHandler(wev);
	...
	if(window > NGX_HTTP_V2_MAX_WINDOW - h2c->send_window) {
		ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0, "client violated connection flow control: received WINDOW_UPDATE frame with window increment %uz not allowed for window %uz",
...
static ngx_http_v2_out_frame_t * ngx_http_v2_get_frame(ngx_http_v2_connection_t * h2c, size_t length, ngx_uint_t type, uchar flags, ngx_uint_t sid)
{
	ngx_buf_t  * buf;
	ngx_pool_t * pool;
	ngx_http_v2_out_frame_t  * frame = h2c->free_frames;
	if(frame) {
		...
		frame = (ngx_http_v2_out_frame_t *)ngx_pcalloc(pool, sizeof(ngx_http_v2_out_frame_t));
		if(frame == NULL) {
			return NULL;
		}
	...
	if(length > NGX_HTTP_V2_FRAME_BUFFER_SIZE - NGX_HTTP_V2_FRAME_HEADER_SIZE) {
		ngx_log_error(NGX_LOG_ALERT, h2c->connection->log, 0, "requested control frame is too large: %uz", length);
		return NULL;
	}
	frame->length = length;
#endif
...
static ngx_http_v2_stream_t * ngx_http_v2_create_stream(ngx_http_v2_connection_t * h2c)
{
	...
	ngx_http_v2_srv_conf_t  * h2scf;
	ngx_http_core_srv_conf_t  * cscf;
	ngx_connection_t * fc = h2c->free_fake_connections;
	if(fc) {
		h2c->free_fake_connections = (ngx_connection_t *)fc->data;
		rev = fc->P_EvRd;
		wev = fc->P_EvWr;
		log = fc->log;
		...
		fc = (ngx_connection_t *)ngx_palloc(h2c->pool, sizeof(ngx_connection_t));
		if(fc == NULL) {
			return NULL;
		}
		...
		wev = (ngx_event_t *)ngx_palloc(h2c->pool, sizeof(ngx_event_t));
		if(wev == NULL) {
			return NULL;
		}
		log = (ngx_log_t *)ngx_palloc(h2c->pool, sizeof(ngx_log_t));
		if(log == NULL) {
			return NULL;
		}
...
static ngx_http_v2_node_t * ngx_http_v2_get_node_by_id(ngx_http_v2_connection_t * h2c, ngx_uint_t sid,
    ngx_uint_t alloc)
{
	ngx_uint_t index;
	ngx_http_v2_node_t * node;
...
static ngx_http_v2_node_t * ngx_http_v2_get_closed_node(ngx_http_v2_connection_t * h2c)
{
	...
	ngx_http_v2_srv_conf_t  * h2scf = (ngx_http_v2_srv_conf_t *)ngx_http_get_module_srv_conf(h2c->http_connection->conf_ctx, ngx_http_v2_module);
	h2c->closed_nodes--;
	...
	memzero(node, sizeof(ngx_http_v2_node_t));
	return node;
}
...
static ngx_int_t ngx_http_v2_parse_path(ngx_http_request_t * r, ngx_http_v2_header_t * header)
{
	...
	if(ngx_http_parse_uri(r) != NGX_OK) {
		ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "client sent invalid :path header: \"%V\"", &header->value);
		return NGX_DECLINED;
	}
...
{
	uchar  * p;
	static const uchar ending[] = " HTTP/2.0";
	if(r->method_name.len == 0 || r->schema_start == NULL || r->unparsed_uri.len == 0) {
		ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
		return NGX_ERROR;
	}
	r->request_line.len = r->method_name.len + 1 + r->unparsed_uri.len + sizeof(ending) - 1;

	p = (uchar *)ngx_pnalloc(r->pool, r->request_line.len + 1);
	if(!p) {
	...
	*p++ = ' ';
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2 http request line: \"%V\"", &r->request_line);
	return NGX_OK;
}
...
{
	ngx_str_t  * val;
	ngx_array_t  * cookies = r->stream->cookies;
	if(cookies == NULL) {
	...
	val = (ngx_str_t *)ngx_array_push(cookies);
	if(val == NULL) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_v2_construct_cookie_header(ngx_http_request_t * r)
{
	...
	len -= 2;
	buf = (uchar *)ngx_pnalloc(r->pool, len + 1);
	if(!buf) {
		ngx_http_v2_close_stream(r->stream, NGX_HTTP_INTERNAL_SERVER_ERROR);
		return NGX_ERROR;
	}
...
ngx_int_t ngx_http_v2_read_request_body(ngx_http_request_t * r)
{
	...
	if(r->request_body_no_buffering && !stream->in_closed) {
		if(len < 0 || len > (nginx_off_t)clcf->client_body_buffer_size) {
		...
		if(len < (nginx_off_t)h2scf->preread_size) {
	...
	else if(len >= 0 && len <= (nginx_off_t)clcf->client_body_buffer_size
		...
		rb->buf = (ngx_buf_t *)ngx_calloc_buf(r->pool);
...
static ngx_int_t ngx_http_v2_process_request_body(ngx_http_request_t * r, uchar * pos, size_t size, ngx_uint_t last)
{
	...
	if(size) {
		if(buf->sync) {
			...
			if(size > (size_t)(buf->end - buf->last)) {
				ngx_log_error(NGX_LOG_INFO, fc->log, 0, "client intended to send body data larger than declared");
	...
	if(last) {
		rb->rest = 0;
		if(fc->P_EvRd->timer_set) {
			ngx_del_timer(fc->P_EvRd);
		}
		if(r->request_body_no_buffering) {
			ngx_post_event(fc->P_EvRd, &ngx_posted_events);
			return NGX_OK;
		}
	...
	clcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_core_module);
	ngx_add_timer(fc->P_EvRd, clcf->client_body_timeout);
	if(r->request_body_no_buffering) {
		ngx_post_event(fc->P_EvRd, &ngx_posted_events);
		return NGX_OK;
	}
...
static ngx_int_t ngx_http_v2_filter_request_body(ngx_http_request_t * r)
{
	...
	if(buf->pos != buf->last) {
		...
		if(r->headers_in.content_length_n != -1) {
			if(rb->received > r->headers_in.content_length_n) {
				ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "client intended to send body data larger than declared");
				return NGX_HTTP_BAD_REQUEST;
			}
		}
		else {
			...
			if(clcf->client_max_body_size && rb->received > clcf->client_max_body_size) {
				ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "client intended to send too large chunked body: %O bytes", rb->received);
	...
	if(!rb->rest) {
		if(r->headers_in.content_length_n != -1 && r->headers_in.content_length_n != rb->received) {
			ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "client prematurely closed stream: only %O out of %O bytes of request body received", rb->received, r->headers_in.content_length_n);
			return NGX_HTTP_BAD_REQUEST;
		}
		b->last_buf = 1;
	}
...
void ngx_http_v2_close_stream(ngx_http_v2_stream_t * stream, ngx_int_t rc)
{
	...
	if(!stream->rst_sent && !h2c->connection->error) {
		if(!stream->out_closed) {
			if(ngx_http_v2_send_rst_stream(h2c, node->id, fc->timedout ? NGX_HTTP_V2_PROTOCOL_ERROR : NGX_HTTP_V2_INTERNAL_ERROR) != NGX_OK) {
				h2c->connection->error = 1;
			}
		}
		else if(!stream->in_closed) {
#if 0
			if(ngx_http_v2_send_rst_stream(h2c, node->id, NGX_HTTP_V2_NO_ERROR) != NGX_OK) {
				h2c->connection->error = 1;
			}
#else
			/*
			 ...
			 */
			if(stream->recv_window < NGX_HTTP_V2_MAX_WINDOW && ngx_http_v2_send_window_update(h2c, node->id, NGX_HTTP_V2_MAX_WINDOW - stream->recv_window) != NGX_OK) {
				h2c->connection->error = 1;
			}
#endif
		}
...
static void ngx_http_v2_handle_connection_handler(ngx_event_t * rev)
{
	...
	ngx_http_v2_handle_connection((ngx_http_v2_connection_t *)c->data);
}
...
static void ngx_http_v2_idle_handler(ngx_event_t * rev)
{
	...
	if(ngx_event_flags & NGX_USE_KQUEUE_EVENT) {
		if(rev->pending_eof) {
			c->log->handler = NULL;
			ngx_log_error(NGX_LOG_INFO, c->log, rev->kq_errno, "kevent() reported that client %V closed idle connection", &c->addr_text);
#if (NGX_HTTP_SSL)
			if(c->ssl) {
...
static void ngx_http_v2_finalize_connection(ngx_http_v2_connection_t * h2c, ngx_uint_t status)
{
	...
	ngx_http_v2_node_t * node;
	ngx_http_v2_srv_conf_t  * h2scf;
	ngx_connection_t * c = h2c->connection;
	h2c->blocked = 1;
	...
	for(i = 0; i < size; i++) {
		for(node = h2c->streams_index[i]; node; node = node->index) {
			ngx_http_v2_stream_t * stream = node->stream;
			if(stream) {
				stream->waiting = 0;
				r = stream->request;
				fc = r->connection;
...
{
	ngx_http_v2_srv_conf_t * h2scf = (ngx_http_v2_srv_conf_t *)ngx_http_get_module_srv_conf(h2c->http_connection->conf_ctx, ngx_http_v2_module);
	const ngx_uint_t size = ngx_http_v2_index_size(h2scf);
	for(ngx_uint_t i = 0; i < size; i++) {
		for(ngx_http_v2_node_t * node = h2c->streams_index[i]; node; node = node->index) {
			ngx_http_v2_stream_t * stream = node->stream;
			if(stream) {
...
static void ngx_http_v2_set_dependency(ngx_http_v2_connection_t * h2c, ngx_http_v2_node_t * node, ngx_uint_t depend, ngx_uint_t exclusive)
{
	...
	ngx_http_v2_node_t * child, * next;
	ngx_http_v2_node_t * parent = depend ? ngx_http_v2_get_node_by_id(h2c, depend, 0) : NULL;
	if(parent == NULL) {
		parent = (ngx_http_v2_node_t *)NGX_HTTP_V2_ROOT;
		if(depend != 0) {
			exclusive = 0;
		}
		node->rank = 1;
...
static void ngx_http_v2_node_children_update(ngx_http_v2_node_t * node)
{
	for(ngx_queue_t * q = ngx_queue_head(&node->children); q != ngx_queue_sentinel(&node->children); q = ngx_queue_next(q)) {
		ngx_http_v2_node_t * child = ngx_queue_data(q, ngx_http_v2_node_t, queue);
...
static void ngx_http_v2_pool_cleanup(void * data)
{
	ngx_http_v2_connection_t * h2c = (ngx_http_v2_connection_t *)data;
	if(h2c->state.pool) {
		ngx_destroy_pool(h2c->state.pool);
	}
	if(h2c->pool) {
