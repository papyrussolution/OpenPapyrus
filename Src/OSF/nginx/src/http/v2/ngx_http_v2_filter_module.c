...
static ngx_chain_t * ngx_http_v2_filter_get_shadow(ngx_http_v2_stream_t * stream, ngx_buf_t * buf, nginx_off_t offset, nginx_off_t size);
...
static ngx_int_t ngx_http_v2_header_filter(ngx_http_request_t * r)
{
	...
	switch(r->headers_out.status) {
		...
		default:
		    ...
		    r->headers_out.last_modified = NULL;
		    switch(r->headers_out.status) {
			    case NGX_HTTP_BAD_REQUEST: status = ngx_http_v2_indexed(NGX_HTTP_V2_STATUS_400_INDEX); break;
			    case NGX_HTTP_NOT_FOUND: status = ngx_http_v2_indexed(NGX_HTTP_V2_STATUS_404_INDEX); break;
			    case NGX_HTTP_INTERNAL_SERVER_ERROR: status = ngx_http_v2_indexed(NGX_HTTP_V2_STATUS_500_INDEX); break;
			    default: status = 0;
		    }
	}
	...
	if(r->gzip_vary) {
		if(clcf->gzip_vary) {
			len += 1 + /*sizeof*/sstrlen(accept_encoding);
		}
		else {
			r->gzip_vary = 0;
		}
	...
	if(r->headers_out.server == NULL) {
		...
		*pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_SERVER_INDEX);
		if(clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
			...
			pos = ngx_cpymem(pos, nginx, /*sizeof*/sstrlen(nginx));
		}
	}
	if(r->headers_out.date == NULL) {
...
static ngx_http_v2_out_frame_t * ngx_http_v2_create_trailers_frame(ngx_http_request_t * r)
{
	...
	size_t len = 0;
	size_t tmp_len = 0;
	ngx_list_part_t  * part = &r->headers_out.trailers.part;
	...
	for(i = 0; /* void */; i++) {
		...
		if(header[i].key.len > NGX_HTTP_V2_MAX_FIELD) {
			ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, "too long response trailer name: \"%V\"", &header[i].key);
			return NULL;
		}
		if(header[i].value.len > NGX_HTTP_V2_MAX_FIELD) {
			ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, "too long response trailer value: \"%V: %V\"", &header[i].key, &header[i].value);
			return NULL;
		}
		len += 1 + NGX_HTTP_V2_INT_OCTETS + header[i].key.len + NGX_HTTP_V2_INT_OCTETS + header[i].value.len;
		if(header[i].key.len > tmp_len) {
...
static ngx_http_v2_out_frame_t * ngx_http_v2_create_headers_frame(ngx_http_request_t * r, u_char * pos, u_char * end, ngx_uint_t fin)
{
	...
	size_t rest = end - pos;
	ngx_http_v2_out_frame_t  * frame = (ngx_http_v2_out_frame_t *)ngx_palloc(r->pool, sizeof(ngx_http_v2_out_frame_t));
	if(frame == NULL) {
		return NULL;
	}
	...
	for(;; ) {
		...
		*ll = cl;
		...
		cl->next = NULL;
		frame->last = cl;
		ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http2:%ui create HEADERS frame %p: len:%uz", stream->node->id, frame, frame->length);
		return frame;
	}
}
...
static ngx_chain_t * ngx_http_v2_send_chain(ngx_connection_t * fc, ngx_chain_t * in, nginx_off_t limit)
{
	...
	if(limit == 0 || limit > (nginx_off_t)h2c->send_window) {
		limit = h2c->send_window;
	}
	if(limit > stream->send_window) {
		limit = (stream->send_window > 0) ? stream->send_window : 0;
	}
	h2lcf = (ngx_http_v2_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_v2_module);
...
static ngx_http_v2_out_frame_t * ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t * stream, size_t len, ngx_chain_t * first, ngx_chain_t * last)
{
	...
	if(frame) {
		...
		frame = (ngx_http_v2_out_frame_t *)ngx_palloc(stream->request->pool, sizeof(ngx_http_v2_out_frame_t));
		if(frame == NULL) {
			return NULL;
		}
	}
	flags = last->buf->last_buf ? NGX_HTTP_V2_END_STREAM_FLAG : 0;
	ngx_log_debug4(NGX_LOG_DEBUG_HTTP, stream->request->connection->log, 0, "http2:%ui create DATA frame %p: len:%uz flags:%ui", stream->node->id, frame, len, (ngx_uint_t)flags);
...
static ngx_inline ngx_int_t ngx_http_v2_flow_control(ngx_http_v2_connection_t * h2c, ngx_http_v2_stream_t * stream)
{
	ngx_log_debug3(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui available windows: conn:%uz stream:%z", stream->node->id, h2c->send_window, stream->send_window);
...
static void ngx_http_v2_waiting_queue(ngx_http_v2_connection_t * h2c, ngx_http_v2_stream_t * stream)
{
	if(!stream->waiting) {
		...
		for(q = ngx_queue_last(&h2c->waiting); q != ngx_queue_sentinel(&h2c->waiting); q = ngx_queue_prev(q)) {
			ngx_http_v2_stream_t * s = ngx_queue_data(q, ngx_http_v2_stream_t, queue);
			if(s->node->rank < stream->node->rank || (s->node->rank == stream->node->rank && s->node->rel_weight >= stream->node->rel_weight)) {
				break;
			}
		}
...
static ngx_int_t ngx_http_v2_data_frame_handler(ngx_http_v2_connection_t * h2c, ngx_http_v2_out_frame_t * frame)
{
	...
	if(cl->buf->tag == (ngx_buf_tag_t)&ngx_http_v2_module) {
		if(cl->buf->pos != cl->buf->last) {
			ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0, "http2:%ui DATA frame %p was sent partially", stream->node->id, frame);
			return NGX_AGAIN;
		}
		ln = cl->next;
		cl->next = stream->free_frame_headers;
...
static ngx_inline void ngx_http_v2_handle_frame(ngx_http_v2_stream_t * stream, ngx_http_v2_out_frame_t * frame)
{
	ngx_http_request_t  * r = stream->request;
...
static void ngx_http_v2_filter_cleanup(void * data)
{
	ngx_http_v2_stream_t * stream = (ngx_http_v2_stream_t *)data;
	if(stream->waiting) {
		stream->waiting = 0;
	...
	if(stream->queued) {
		...
		if(h2c->send_window == 0 && window) {
			while(!ngx_queue_empty(&h2c->waiting)) {
				ngx_queue_t * q = ngx_queue_head(&h2c->waiting);
				ngx_queue_remove(q);
