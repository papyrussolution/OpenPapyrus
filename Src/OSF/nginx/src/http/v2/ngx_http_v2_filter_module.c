...
static ngx_chain_t * ngx_http_v2_send_chain(ngx_connection_t * fc,
    ngx_chain_t * in, nginx_off_t limit);
...
static ngx_http_v2_out_frame_t * ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t * stream, size_t len, ngx_chain_t * first,
    ngx_chain_t * last);
...
static u_char * ngx_http_v2_string_encode(u_char * dst, u_char * src, size_t len, u_char * tmp,
    ngx_uint_t lower)
{
	size_t hlen;
...
static ngx_http_v2_out_frame_t * ngx_http_v2_create_headers_frame(ngx_http_request_t * r, u_char * pos,
    u_char * end, ngx_uint_t fin)
{
	...
	frame = (ngx_http_v2_out_frame_t *)ngx_palloc(r->pool, sizeof(ngx_http_v2_out_frame_t));
	if(frame == NULL) {
		return NULL;
	}
...
static ngx_chain_t * ngx_http_v2_filter_get_shadow(ngx_http_v2_stream_t * stream, ngx_buf_t * buf,
    nginx_off_t offset, nginx_off_t size)
{
	ngx_buf_t    * chunk;
	ngx_chain_t  * cl;
...
static ngx_http_v2_out_frame_t * ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t * stream,
    size_t len, ngx_chain_t * first, ngx_chain_t * last)
{
	...
	if(frame) {
		...
		frame = (ngx_http_v2_out_frame_t *)ngx_palloc(stream->request->pool,
...
static ngx_inline ngx_int_t ngx_http_v2_flow_control(ngx_http_v2_connection_t * h2c,
    ngx_http_v2_stream_t * stream)
{
...
static ngx_int_t ngx_http_v2_headers_frame_handler(ngx_http_v2_connection_t * h2c,
    ngx_http_v2_out_frame_t * frame)
{
	ngx_chain_t           * cl, * ln;
	ngx_http_v2_stream_t  * stream;
...
static ngx_inline void ngx_http_v2_handle_frame(ngx_http_v2_stream_t * stream,
    ngx_http_v2_out_frame_t * frame)
{
	ngx_http_request_t  * r;
