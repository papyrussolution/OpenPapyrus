	...
	for(;;) {
		...
		if(ngx_read_dir(&dir) == NGX_ERROR) {
			...
			if(err != NGX_ENOMOREFILES) {
				ngx_log_error(NGX_LOG_CRIT, r->connection->log, err, ngx_read_dir_n " \"%V\" failed", &path);
				return ngx_http_autoindex_error(r, &dir, &path);
			}
			break;
		}
		...
		if(!dir.valid_info) {
			...
			if(path.len + 1 + len + 1 > allocated) {
				...
				filename = (uchar *)ngx_pnalloc(pool, allocated);
				if(!filename) {
					return ngx_http_autoindex_error(r, &dir, &path);
				}
			...
			if(ngx_de_info(filename, &dir) == NGX_FILE_ERROR) {
				...
				if(err != NGX_ENOENT && err != NGX_ELOOP) {
					ngx_log_error(NGX_LOG_CRIT, r->connection->log, err, ngx_de_info_n " \"%s\" failed", filename);
					if(err == NGX_EACCES) {
						continue;
					}
					return ngx_http_autoindex_error(r, &dir, &path);
				...
				if(ngx_de_link_info(filename, &dir) == NGX_FILE_ERROR) {
					ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno, ngx_de_link_info_n " \"%s\" failed", filename);
					return ngx_http_autoindex_error(r, &dir, &path);
				}
			}
		}
		entry = (ngx_http_autoindex_entry_t *)ngx_array_push(&entries);
		if(entry == NULL) {
			return ngx_http_autoindex_error(r, &dir, &path);
		}
		entry->name.len = len;
		entry->name.data = (uchar *)ngx_pnalloc(pool, len + 1);
...
static ngx_buf_t * ngx_http_autoindex_html(ngx_http_request_t * r, ngx_array_t * entries)
{
	...
	ngx_http_autoindex_entry_t   * entry;
	ngx_http_autoindex_loc_conf_t  * alcf;
	static uchar title[] = "<html>" CRLF "<head><title>Index of ";
	static uchar header[] = "</title></head>" CRLF "<body bgcolor=\"white\">" CRLF "<h1>Index of ";
	static uchar tail[] = "</body>" CRLF "</html>" CRLF;
	static const char * months[] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
	if(r->headers_out.charset.len == 5 && ngx_strncasecmp(r->headers_out.charset.data, (uchar *)"utf-8", 5) == 0) {
		utf8 = 1;
	}
	else {
		utf8 = 0;
	...
	if(escape_html) {
		b->last = (uchar *)ngx_escape_html(b->last, r->uri.data, r->uri.len);
	...
	b->last = ngx_cpymem(b->last, "<hr><pre><a href=\"../\">../</a>" CRLF, sizeof("<hr><pre><a href=\"../\">../</a>" CRLF) - 1);
	alcf = (ngx_http_autoindex_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_autoindex_module);
	tp = ngx_timeofday();
	for(i = 0; i < entries->nelts; i++) {
		...
		*b->last++ = '>';
		...
		if(entry[i].name.len != len) {
			...
			if(entry[i].escape_html) {
				b->last = (uchar *)ngx_escape_html(last, entry[i].name.data, b->last - last);
			}
			last = b->last;
		}
		else {
			if(entry[i].escape_html) {
				...
				b->last = (uchar *)ngx_escape_html(b->last, entry[i].name.data, char_len);
				last = b->last;
			}
			else {
		...
		*b->last++ = ' ';
		...
		b->last = ngx_sprintf(b->last, "%02d-%s-%d %02d:%02d ", tm.ngx_tm_mday,
		    months[tm.ngx_tm_mon - 1], tm.ngx_tm_year, tm.ngx_tm_hour, tm.ngx_tm_min);
		if(alcf->exact_size) {
			if(entry[i].dir) {
...
static ngx_buf_t * ngx_http_autoindex_json(ngx_http_request_t * r, ngx_array_t * entries, ngx_str_t * callback)
{
	...
	for(i = 0; i < entries->nelts; i++) {
		...
		len += sizeof("{  }," CRLF) - 1 + sizeof("\"name\":\"\"") - 1
		    + entry[i].name.len + entry[i].escape + sizeof(", \"type\":\"directory\"") - 1
	...
	*b->last++ = '[';
	for(i = 0; i < entries->nelts; i++) {
		...
		if(entry[i].escape) {
			b->last = (uchar *)ngx_escape_json(b->last, entry[i].name.data, entry[i].name.len);
		}
		else {
			b->last = ngx_cpymem(b->last, entry[i].name.data, entry[i].name.len);
		}
...
static ngx_int_t ngx_http_autoindex_jsonp_callback(ngx_http_request_t * r, ngx_str_t * callback)
{
	uchar * p, c, ch;
	ngx_uint_t i;
	if(ngx_http_arg(r, (uchar *)"callback", 8, callback) != NGX_OK) {
		callback->len = 0;
		return NGX_OK;
	}
	if(callback->len > 128) {
...
static ngx_buf_t * ngx_http_autoindex_xml(ngx_http_request_t * r, ngx_array_t * entries)
{
	...
	ngx_uint_t i;
	ngx_http_autoindex_entry_t   * entry;
	static uchar head[] = "<?xml version=\"1.0\"?>" CRLF "<list>" CRLF;
	static uchar tail[] = "</list>" CRLF;
	len = sizeof(head) - 1 + sizeof(tail) - 1;
	entry = (ngx_http_autoindex_entry_t *)entries->elts;
	for(i = 0; i < entries->nelts; i++) {
		entry[i].escape = ngx_escape_html(NULL, entry[i].name.data, entry[i].name.len);
		len += sizeof("<directory></directory>" CRLF) - 1 + entry[i].name.len + entry[i].escape + sizeof(" mtime=\"1986-12-31T10:00:00Z\"") - 1;
		if(entry[i].file) {
			len += sizeof(" size=\"\"") - 1 + NGX_OFF_T_LEN;
		}
	}
	...
	for(i = 0; i < entries->nelts; i++) {
		...
		*b->last++ = '"'; *b->last++ = '>';
		if(entry[i].escape) {
			b->last = (uchar *)ngx_escape_html(b->last, entry[i].name.data, entry[i].name.len);
		}
		else {
			b->last = ngx_cpymem(b->last, entry[i].name.data, entry[i].name.len);
		}
...
static ngx_buf_t * ngx_http_autoindex_alloc(ngx_http_autoindex_ctx_t * ctx, size_t size)
{
	ngx_chain_t  * cl;
	if(ctx->buf) {
