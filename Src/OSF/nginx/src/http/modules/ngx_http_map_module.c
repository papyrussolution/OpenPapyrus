...
static void * ngx_http_map_create_conf(ngx_conf_t * cf)
{
	ngx_http_map_conf_t  * mcf = (ngx_http_map_conf_t *)ngx_palloc(cf->pool, sizeof(ngx_http_map_conf_t));
	if(mcf == NULL) {
		return NULL;
	}
...
static char * ngx_http_map(ngx_conf_t * cf, ngx_command_t * dummy, void * conf)
{
	...
	var = (ngx_http_variable_value_t*)ngx_palloc(ctx->keys.pool, sizeof(ngx_http_variable_value_t));
	if(var == NULL) {
		return NGX_CONF_ERROR;
	}
...
found:
	...
	if(value[0].len && value[0].data[0] == '~') {
		...
		regex = (ngx_http_map_regex_t *)ngx_array_push(&ctx->regexes);
		if(regex == NULL) {
			return NGX_CONF_ERROR;
		}
		value[0].len--;
