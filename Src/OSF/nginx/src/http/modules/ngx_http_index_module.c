...
static ngx_command_t ngx_http_index_commands[] = {
	{ ngx_string("index"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_http_index_set_index, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	ngx_null_command
};
...
static ngx_int_t ngx_http_index_handler(ngx_http_request_t * r)
{
	...
	ilcf = (ngx_http_index_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_index_module);
	...
	path.data = NULL;
	index = (ngx_http_index_t *)ilcf->indices->elts;
...
static char * ngx_http_index_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_index_loc_conf_t  * prev = (ngx_http_index_loc_conf_t *)parent;
	ngx_http_index_loc_conf_t  * conf = (ngx_http_index_loc_conf_t *)child;
	ngx_http_index_t  * index;
	...
	if(conf->indices == NULL) {
		...
		index = (ngx_http_index_t *)ngx_array_push(conf->indices);
		if(index == NULL) {
			return NGX_CONF_ERROR;
		}
...
static const char * ngx_http_index_set_index(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	for(i = 1; i < cf->args->nelts; i++) {
		...
		index = (ngx_http_index_t *)ngx_array_push(ilcf->indices);
		if(index == NULL) {
			return NGX_CONF_ERROR;
		}
