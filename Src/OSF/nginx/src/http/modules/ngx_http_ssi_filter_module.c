...
static uchar ngx_http_ssi_string[] = "<!--";
...
static ngx_int_t ngx_http_ssi_header_filter(ngx_http_request_t * r)
{
	...
	ngx_http_ssi_loc_conf_t  * slcf = (ngx_http_ssi_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_ssi_filter_module);
...
static ngx_int_t ngx_http_ssi_body_filter(ngx_http_request_t * r, ngx_chain_t * in)
{
	...
	ngx_http_ssi_ctx_t * ctx = (ngx_http_ssi_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_ssi_filter_module);
	...
	if(ctx->wait) {
		if(r != r->connection->data) {
			ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" non-active", &ctx->wait->uri, &ctx->wait->args);
			return NGX_AGAIN;
		}
		if(ctx->wait->done) {
			ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" done", &ctx->wait->uri, &ctx->wait->args);
			ctx->wait = NULL;
		}
		else {
	...
	slcf = (ngx_http_ssi_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_ssi_filter_module);
	while(ctx->in || ctx->buf) {
		...
		while(ctx->pos < ctx->buf->last) {
			...
			ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "parse: %i, looked: %uz %p-%p", rc, ctx->looked, ctx->copy_start, ctx->copy_end);
			if(rc == NGX_ERROR) {
				return rc;
			}
				...
				if(!ctx->output && !cmd->block) {
					if(ctx->block) {
						...
						*b->last++ = '>';
						...
						mctx = (ngx_http_ssi_ctx_t *)ngx_http_get_module_ctx(r->main,
...
static void ngx_http_ssi_buffered(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx)
{
	SETFLAG(r->buffered, NGX_HTTP_SSI_BUFFERED, (ctx->in || ctx->buf));
}
...
static ngx_int_t ngx_http_ssi_parse(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx)
{
	uchar * p, ch;
	uchar * value;
	ngx_http_ssi_state_e state = (ngx_http_ssi_state_e)ctx->state;
	size_t looked = ctx->looked;
	uchar * last = ctx->buf->last;
	uchar * copy_end = ctx->copy_end;
		...
		switch(state) {
			...
			case ssi_precommand_state:
			    switch(ch) {
				    ...
				    default:
						ctx->command.len = 1;
						ctx->command.data = (uchar *)ngx_pnalloc(r->pool, NGX_HTTP_SSI_COMMAND_LEN);
			...
			case ssi_preparam_state:
			    switch(ch) {
				    ...
				    case '-':
						...
						break;
				    default:
						ctx->param = (ngx_table_elt_t *)ngx_array_push(&ctx->params);
						if(ctx->param == NULL) {
							return NGX_ERROR;
						}
						ctx->param->key.len = 1;
						ctx->param->key.data = (uchar *)ngx_pnalloc(r->pool, NGX_HTTP_SSI_PARAM_LEN);
						...
						if(ctx->value_buf == NULL) {
							ctx->param->value.data = (uchar *)ngx_pnalloc(r->pool, ctx->value_len + 1);
							if(ctx->param->value.data == NULL) {
								return NGX_ERROR;
							}
						}
						else {
							ctx->param->value.data = (uchar *)ctx->value_buf;
						}
			...
			case ssi_preequal_state:
			    switch(ch) {
				    ...
				    case '=':
						...
						break;
				    default:
						state = (ch == '-') ? ssi_error_end0_state : ssi_error_state;
			...
			case ssi_postparam_state:
			    if(ctx->param->value.len + 1 < ctx->value_len / 2) {
				    value = (uchar *)ngx_pnalloc(r->pool, ctx->param->value.len + 1);
				    if(value == NULL) {
					    return NGX_ERROR;
				    }
...
static ngx_str_t * ngx_http_ssi_get_variable(ngx_http_request_t * r, ngx_str_t * name, ngx_uint_t key)
{
	...
	ngx_http_ssi_ctx_t  * ctx = (ngx_http_ssi_ctx_t *)ngx_http_get_module_ctx(r->main, ngx_http_ssi_filter_module);
#if (NGX_PCRE)
	{
		ngx_str_t * value;
		if(key >= '0' && key <= '9') {
			i = key - '0';
			if(i < ctx->ncaptures) {
				value = (ngx_str_t *)ngx_palloc(r->pool, sizeof(ngx_str_t));
				if(value == NULL) {
					return NULL;
				}
				i *= 2;
...
static ngx_int_t ngx_http_ssi_evaluate_string(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx, ngx_str_t * text, ngx_uint_t flags)
{
	uchar ch, * p, ** value, * data, * part_data;
	size_t * size, len, prefix, part_len;
	ngx_str_t var, * val;
	ngx_int_t key;
	ngx_uint_t i, bracket, quoted;
	ngx_array_t lengths, values;
	ngx_http_variable_value_t  * vv;
	ngx_uint_t n = ngx_http_script_variables_count(text);
	if(n == 0) {
		data = text->data;
		p = data;
		if((flags & NGX_HTTP_SSI_ADD_PREFIX) && text->data[0] != '/') {
			...
			if(prefix) {
				len = prefix + text->len;
				data = static_cast<uchar *>(ngx_pnalloc(r->pool, len));
				if(!data) {
					return NGX_ERROR;
				}
	...
	if(ngx_array_init(&values, r->pool, 8, sizeof(uchar *)) != NGX_OK) {
		return NGX_ERROR;
	}
	len = 0;
	i = 0;
	while(i < text->len) {
		if(text->data[i] == '$') {
			...
			if(bracket) {
				ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "the closing bracket in \"%V\" variable is missing", &var);
				return NGX_HTTP_SSI_ERROR;
			}
			if(var.len == 0) {
				goto invalid_variable;
		...
		len += part_len;
		size = (size_t *)ngx_array_push(&lengths);
		if(size == NULL) {
			return NGX_ERROR;
		}
		*size = part_len;
		value = (uchar **)ngx_array_push(&values);
		if(value == NULL) {
			return NGX_ERROR;
		}
		*value = part_data;
	...
	p = (uchar *)ngx_pnalloc(r->pool, len + ((flags & NGX_HTTP_SSI_ADD_ZERO) ? 1 : 0));
	if(!p) {
		return NGX_ERROR;
	}
	text->len = len;
...
static ngx_int_t ngx_http_ssi_regex_match(ngx_http_request_t * r, ngx_str_t * pattern, ngx_str_t * str)
{
#if (NGX_PCRE)
	int rc, * captures;
	uchar * p, errstr[NGX_MAX_CONF_ERRSTR];
	size_t size;
	ngx_int_t key;
	...
	memzero(&rgc, sizeof(ngx_regex_compile_t));
	rgc.pattern = *pattern;
	rgc.pool = r->pool;
	...
	ctx = (ngx_http_ssi_ctx_t *)ngx_http_get_module_ctx(r->main, ngx_http_ssi_filter_module);
	ctx->ncaptures = rc;
...
static ngx_int_t ngx_http_ssi_include(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx, ngx_str_t ** params)
{
	...
	ngx_str_t * uri = params[NGX_HTTP_SSI_INCLUDE_VIRTUAL];
	ngx_str_t * file = params[NGX_HTTP_SSI_INCLUDE_FILE];
	ngx_str_t * wait = params[NGX_HTTP_SSI_INCLUDE_WAIT];
	ngx_str_t * set = params[NGX_HTTP_SSI_INCLUDE_SET];
	ngx_str_t * stub = params[NGX_HTTP_SSI_INCLUDE_STUB];
	if(uri && file) {
	...
	if(wait) {
		...
		if(wait->len == 2 && ngx_strncasecmp(wait->data, (uchar *)"no", 2) == 0) {
			wait = NULL;
		}
		else if(wait->len != 3 || ngx_strncasecmp(wait->data, (uchar *)"yes", 3) != 0) {
...
found:
	...
	if(set) {
		...
		if(psr->data == NULL) {
			...
			var = (ngx_http_ssi_var_t *)ngx_list_push(mctx->variables);
			if(var == NULL) {
				return NGX_ERROR;
			}
			var->name = *set;
...
static ngx_int_t ngx_http_ssi_echo(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx, ngx_str_t ** params)
{
	...
	ngx_int_t key;
	ngx_buf_t * b;
	ngx_str_t * value, * enc, text;
	ngx_chain_t  * cl;
	ngx_http_variable_value_t  * vv;
	ngx_str_t * var = params[NGX_HTTP_SSI_ECHO_VAR];
	...
	switch(ctx->encoding) {
		case NGX_HTTP_SSI_URL_ENCODING:
		    ...
		    NGX_ESCAPE_HTML);
		    if(len) {
			    p = (uchar *)ngx_pnalloc(r->pool, value->len + len);
			    if(!p) {
				    return NGX_HTTP_SSI_ERROR;
			    }
...
static ngx_int_t ngx_http_ssi_config(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx, ngx_str_t ** params)
{
	ngx_str_t  * value = params[NGX_HTTP_SSI_CONFIG_TIMEFMT];
	if(value) {
		ctx->timefmt.len = value->len;
		ctx->timefmt.data = (uchar *)ngx_pnalloc(r->pool, value->len + 1);
...
static ngx_int_t ngx_http_ssi_if(ngx_http_request_t * r, ngx_http_ssi_ctx_t * ctx, ngx_str_t ** params)
{
	...
	for(p = left.data; p < last; p++) {
		...
		if((*p >= 'a' && *p <= 'z') || (*p >= '0' && *p <= '9') || *p == '$' || *p == '{' || *p == '}' || *p == '_' || *p == '"' || *p == '\'') {
			continue;
		}
...
static ngx_int_t ngx_http_ssi_date_gmt_local_variable(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t gmt)
{
	...
	v->not_found = 0;
	now = ngx_time();
	ctx = (ngx_http_ssi_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_ssi_filter_module);
	timefmt = ctx ? &ctx->timefmt : &ngx_http_ssi_timefmt;
	if(timefmt->len == sizeof("%s") - 1 && timefmt->data[0] == '%' && timefmt->data[1] == 's') {
		v->data = (uchar *)ngx_pnalloc(r->pool, NGX_TIME_T_LEN);
		if(v->data == NULL) {
			return NGX_ERROR;
		}
...
static void * ngx_http_ssi_create_main_conf(ngx_conf_t * cf)
{
	ngx_http_ssi_main_conf_t  * smcf = (ngx_http_ssi_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_ssi_main_conf_t));
	if(smcf) {
...
static char * ngx_http_ssi_init_main_conf(ngx_conf_t * cf, void * conf)
{
	ngx_http_ssi_main_conf_t * smcf = (ngx_http_ssi_main_conf_t *)conf;
	...
	hash.pool = cf->pool;
	hash.temp_pool = NULL;
	if(ngx_hash_init(&hash, (ngx_hash_key_t *)smcf->commands.keys.elts, smcf->commands.keys.nelts) != NGX_OK) {
...
static char * ngx_http_ssi_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_ssi_loc_conf_t * prev = (ngx_http_ssi_loc_conf_t *)parent;
	ngx_http_ssi_loc_conf_t * conf = (ngx_http_ssi_loc_conf_t *)child;
