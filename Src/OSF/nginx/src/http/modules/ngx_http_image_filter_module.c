...
static ngx_int_t ngx_http_image_header_filter(ngx_http_request_t * r)
{
	...
	ctx = (ngx_http_image_filter_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_image_filter_module);

	if(ctx) {
		ngx_http_set_ctx(r, NULL, ngx_http_image_filter_module);
		return ngx_http_next_header_filter(r);
	}
	conf = (ngx_http_image_filter_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_image_filter_module);
	if(conf->filter == NGX_HTTP_IMAGE_OFF) {
		return ngx_http_next_header_filter(r);
	}
	if(r->headers_out.content_type.len >= sizeof("multipart/x-mixed-replace") - 1 && ngx_strncasecmp(r->headers_out.content_type.data,
		    (u_char*)"multipart/x-mixed-replace", sizeof("multipart/x-mixed-replace") - 1) == 0) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "image filter: multipart/x-mixed-replace response");
		return NGX_ERROR;
	}
	ctx = (ngx_http_image_filter_ctx_t *)ngx_pcalloc(r->pool, sizeof(ngx_http_image_filter_ctx_t));
	if(ctx == NULL) {
		return NGX_ERROR;
	}
	ngx_http_set_ctx(r, ctx, ngx_http_image_filter_module);
	len = r->headers_out.content_length_n;
	if(len != -1 && len > (nginx_off_t)conf->buffer_size) {
...
static ngx_int_t ngx_http_image_send(ngx_http_request_t * r, ngx_http_image_filter_ctx_t * ctx,
    ngx_chain_t * in)
{
	ngx_int_t rc;
...
static ngx_int_t ngx_http_image_read(ngx_http_request_t * r, ngx_chain_t * in)
{
	...
	ngx_buf_t                    * b;
	ngx_chain_t                  * cl;
	ngx_http_image_filter_ctx_t  * ctx = (ngx_http_image_filter_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_image_filter_module);
	if(ctx->image == NULL) {
...
static ngx_buf_t * ngx_http_image_json(ngx_http_request_t * r, ngx_http_image_filter_ctx_t * ctx)
{
	...
	len = sizeof("{ \"img\" : "
	    ...
	    + 2 * NGX_SIZE_T_LEN;
	b->pos = (u_char *)ngx_pnalloc(r->pool, len);
	if(b->pos == NULL) {
		return NULL;
	}
...
static u_char * ngx_http_image_out(ngx_http_request_t * r, ngx_uint_t type, gdImagePtr img,
    int * size)
{
	char                          * failed;
	u_char                        * out;
...
static char * ngx_http_image_filter_jpeg_quality(ngx_conf_t * cf, ngx_command_t * cmd,
    void * conf)
{
	ngx_http_image_filter_conf_t * imcf = conf;
...
static char * ngx_http_image_filter_webp_quality(ngx_conf_t * cf, ngx_command_t * cmd,
    void * conf)
{
	ngx_http_image_filter_conf_t * imcf = conf;
