...
static ngx_uint_t ngx_http_charset_recode(ngx_buf_t * b, uchar * table);
...
static const char * ngx_http_charset_map_block(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
static const char * ngx_http_charset_map(ngx_conf_t * cf, const ngx_command_t * dummy, void * conf); // F_SetHandler
...
static ngx_command_t ngx_http_charset_filter_commands[] = {
	...
	{ ngx_string("charset_types"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_http_types_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_charset_loc_conf_t, types_keys), &ngx_http_charset_default_types[0] },
	{ ngx_string("charset_map"), NGX_HTTP_MAIN_CONF|NGX_CONF_BLOCK|NGX_CONF_TAKE2,
...
static /*ngx_inline*/void FASTCALL ngx_http_set_charset(ngx_http_request_t * pReq, ngx_str_t * charset)
{
	if(pReq == pReq->main) {
		if(pReq->headers_out.status == NGX_HTTP_MOVED_PERMANENTLY || pReq->headers_out.status == NGX_HTTP_MOVED_TEMPORARILY) {
			// do not set charset for the redirect because NN 4.x use this charset instead of the next page charset
			pReq->headers_out.charset.len = 0;
		}
		else
			pReq->headers_out.charset = *charset;
	}
}

static ngx_int_t ngx_http_charset_header_filter(ngx_http_request_t * pReq)
{
	...
	if(pReq == pReq->main) {
		charset = ngx_http_destination_charset(pReq, &dst);
	}
	else {
		charset = ngx_http_main_request_charset(pReq, &dst);
	}
	...
	source_charset = ngx_http_source_charset(pReq, &src);
	if(source_charset == NGX_ERROR) {
		return NGX_ERROR;
	}
	/*
	 * source_charset: charset index, NGX_HTTP_NO_CHARSET, or NGX_HTTP_CHARSET_OFF
	 */
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pReq->connection->log, 0, "charset: \"%V\" > \"%V\"", &src, &dst);
	if(source_charset == NGX_HTTP_CHARSET_OFF) {
		ngx_http_set_charset(pReq, &dst);
		return ngx_http_next_header_filter(pReq);
	}
	...
	if(source_charset == charset) {
		pReq->headers_out.content_type.len = pReq->headers_out.content_type_len;
		ngx_http_set_charset(pReq, &dst);
		return ngx_http_next_header_filter(pReq);
	}
	/* source_charset != charset */
	if(pReq->headers_out.content_encoding && pReq->headers_out.content_encoding->value.len) {
		return ngx_http_next_header_filter(pReq);
	}
	mcf = (ngx_http_charset_main_conf_t *)ngx_http_get_module_main_conf(pReq, ngx_http_charset_filter_module);
	...
	pReq->headers_out.content_type.len = pReq->headers_out.content_type_len;
	ngx_http_set_charset(pReq, &dst);
	return ngx_http_charset_ctx(pReq, charsets, charset, source_charset);
no_charset_map:
	ngx_log_error(NGX_LOG_ERR, pReq->connection->log, 0, "no \"charset_map\" between the charsets \"%V\" and \"%V\"", &src, &dst);
	return ngx_http_next_header_filter(pReq);
}
...
static ngx_int_t ngx_http_destination_charset(ngx_http_request_t * r, ngx_str_t * name)
{
	...
	mlcf = (ngx_http_charset_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_charset_filter_module);
	...
	if(charset < NGX_HTTP_CHARSET_VAR) {
		mcf = (ngx_http_charset_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_charset_filter_module);
		charsets = (ngx_http_charset_t *)mcf->charsets.elts;
...
static ngx_int_t ngx_http_main_request_charset(ngx_http_request_t * r, ngx_str_t * src)
{
	...
	ngx_http_charset_ctx_t  * ctx = (ngx_http_charset_ctx_t *)ngx_http_get_module_ctx(r->main, ngx_http_charset_filter_module);
	if(ctx) {
...
static ngx_int_t ngx_http_source_charset(ngx_http_request_t * r, ngx_str_t * name)
{
	...
	lcf = (ngx_http_charset_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_charset_filter_module);
...
static ngx_int_t ngx_http_get_charset(ngx_http_request_t * r, ngx_str_t * name)
{
	ngx_http_charset_main_conf_t  * mcf = (ngx_http_charset_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_charset_filter_module);
	ngx_http_charset_t  * charset = (ngx_http_charset_t *)mcf->charsets.elts;
	ngx_uint_t n = mcf->charsets.nelts;
...
static ngx_int_t ngx_http_charset_body_filter(ngx_http_request_t * r, ngx_chain_t * in)
{
	...
	ngx_http_charset_ctx_t  * ctx = (ngx_http_charset_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_charset_filter_module);
...
recode:
	...
	while(src < buf->last) {
		...
		len = buf->last - src;
		n = ngx_utf8_decode((const uchar **)&src, len);
		if(n < 0x10000) {
			p = table[n >> 8];
			if(p) {
				c = p[n & 0xff];
...
static ngx_chain_t * ngx_http_charset_get_buf(ngx_pool_t * pool, ngx_http_charset_ctx_t * ctx)
{
	...
	if(cl) {
		...
		if(cl) {
			cl->buf = (ngx_buf_t*)ngx_calloc_buf(pool);
			if(cl->buf == NULL) {
				return NULL;
			}
			cl->next = NULL;
...
static const char * ngx_http_charset_map_block(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	ngx_http_charset_main_conf_t  * mcf = (ngx_http_charset_main_conf_t *)conf;
	char * rv;
	uchar * p, * dst2src, ** pp;
	ngx_uint_t i, n;
	ngx_conf_t pvcf;
	...
	if(src == dst) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"charset_map\" between the same charsets \"%V\" and \"%V\"", &value[1], &value[2]);
		return NGX_CONF_ERROR;
	}
	...
	for(i = 0; i < mcf->tables.nelts; i++) {
		if((src == table->src && dst == table->dst) || (src == table->dst && dst == table->src)) {
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "duplicate \"charset_map\" between \"%V\" and \"%V\"", &value[1], &value[2]);
			return NGX_CONF_ERROR;
		}
	}
	table = (ngx_http_charset_tables_t *)ngx_array_push(&mcf->tables);
	if(table == NULL) {
		return NGX_CONF_ERROR;
	}
	table->src = src;
	table->dst = dst;
	if(sstreqi_ascii(value[2].data, (uchar *)"utf-8")) {
		table->src2dst = (uchar *)ngx_pcalloc(cf->pool, 256 * NGX_UTF_LEN);
		if(table->src2dst == NULL) {
			return NGX_CONF_ERROR;
		}
		table->dst2src = (uchar *)ngx_pcalloc(cf->pool, 256 * sizeof(void *));
		if(table->dst2src == NULL) {
			return NGX_CONF_ERROR;
		}
		dst2src = (uchar *)ngx_pcalloc(cf->pool, 256);
		if(dst2src == NULL) {
			return NGX_CONF_ERROR;
		}
		pp = (uchar**)&table->dst2src[0];
		pp[0] = dst2src;
		for(i = 0; i < 128; i++) {
		...
		table->src2dst = (uchar *)ngx_palloc(cf->pool, 256);
		if(table->src2dst == NULL) {
			return NGX_CONF_ERROR;
		}
		table->dst2src = (uchar *)ngx_palloc(cf->pool, 256);
		if(table->dst2src == NULL) {
			return NGX_CONF_ERROR;
		}
		for(i = 0; i < 128; i++) {
			table->src2dst[i] = (uchar)i;
			table->dst2src[i] = (uchar)i;
		}
		for(/* void */; i < 256; i++) {
			table->src2dst[i] = '?';
			table->dst2src[i] = '?';
...
static const char * ngx_http_charset_map(ngx_conf_t * cf, const ngx_command_t * dummy, void * conf) // F_SetHandler
{
	...
	if(ctx->charset->utf8) {
		...
		*p++ = (uchar)(value[1].len / 2);
		...
		n = ngx_utf8_decode((const uchar **)&p, i);
		if(n > 0xffff) {
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid value \"%V\"", &value[1]);
			return NGX_CONF_ERROR;
		}
		...
		table->src2dst[src] = (uchar)dst;
		table->dst2src[dst] = (uchar)src;
	}
	return NGX_CONF_OK;
}
			...
			*cp = NGX_HTTP_CHARSET_OFF;
			...
			if(value[1].data[0] == '$') {
				...
				*cp += NGX_HTTP_CHARSET_VAR;
				...
				mcf = (ngx_http_charset_main_conf_t *)ngx_http_conf_get_module_main_conf(cf, ngx_http_charset_filter_module);
...
{
	ngx_uint_t i;
	ngx_http_charset_t * c = (ngx_http_charset_t *)charsets->elts;
	for(i = 0; i < charsets->nelts; i++) {
		if(name->len == c[i].name.len && sstreqi_ascii(name->data, c[i].name.data))
			break;
	}
	if(i < charsets->nelts) {
		return i;
	...
	c->name = *name;
	c->length = 0;
	if(sstreqi_ascii(name->data, (uchar *)"utf-8")) {
		c->utf8 = 1;
	}
	else {
		c->utf8 = 0;
...
static void * ngx_http_charset_create_main_conf(ngx_conf_t * cf)
{
	ngx_http_charset_main_conf_t * mcf = (ngx_http_charset_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_charset_main_conf_t));
	if(mcf) {
		if(ngx_array_init(&mcf->charsets, cf->pool, 2, sizeof(ngx_http_charset_t)) != NGX_OK)
			return NULL;
		else if(ngx_array_init(&mcf->tables, cf->pool, 1, sizeof(ngx_http_charset_tables_t)) != NGX_OK)
			return NULL;
		else if(ngx_array_init(&mcf->recodes, cf->pool, 2, sizeof(ngx_http_charset_recode_t)) != NGX_OK)
			return NULL;
	}
	return mcf;
}
...
static void * ngx_http_charset_create_loc_conf(ngx_conf_t * cf)
{
	ngx_http_charset_loc_conf_t  * lcf = (ngx_http_charset_loc_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_charset_loc_conf_t));
	if(lcf == NULL) {
		return NULL;
	}
	/*
...
static char * ngx_http_charset_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_charset_loc_conf_t * prev = (ngx_http_charset_loc_conf_t *)parent;
	ngx_http_charset_loc_conf_t * conf = (ngx_http_charset_loc_conf_t *)child;
	ngx_uint_t i;
	ngx_http_charset_recode_t   * recode;
	ngx_http_charset_main_conf_t  * mcf;
	if(ngx_http_merge_types(cf, &conf->types_keys, &conf->types, &prev->types_keys, &prev->types, ngx_http_charset_default_types) != NGX_OK) {
		return NGX_CONF_ERROR;
	}
	...
	recode = (ngx_http_charset_recode_t *)ngx_array_push(&mcf->recodes);
	if(recode == NULL) {
		return NGX_CONF_ERROR;
	}
...
static ngx_int_t ngx_http_charset_postconfiguration(ngx_conf_t * cf)
{
	...
	ngx_int_t c;
	ngx_uint_t i, t;
	ngx_http_charset_main_conf_t  * mcf = (ngx_http_charset_main_conf_t *)ngx_http_conf_get_module_main_conf(cf, ngx_http_charset_filter_module);
	ngx_http_charset_recode_t   * recode = (ngx_http_charset_recode_t *)mcf->recodes.elts;
	ngx_http_charset_tables_t   * tables = (ngx_http_charset_tables_t *)mcf->tables.elts;
	ngx_http_charset_t * charset = (ngx_http_charset_t *)mcf->charsets.elts;
	for(i = 0; i < mcf->recodes.nelts; i++) {
		c = recode[i].src;
...
next:
	...
	for(t = 0; t < mcf->tables.nelts; t++) {
		...
		if(!src) {
			src = (uchar **)ngx_pcalloc(cf->pool, sizeof(uchar *) * mcf->charsets.nelts);
			if(!src) {
				return NGX_ERROR;
			}
		...
		if(dst == NULL) {
			dst = (uchar **)ngx_pcalloc(cf->pool, sizeof(uchar *) * mcf->charsets.nelts);
			if(dst == NULL) {
				return NGX_ERROR;
			}
