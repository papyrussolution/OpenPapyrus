...
static ngx_int_t ngx_http_dav_error(ngx_log_t * log, ngx_err_t err, ngx_int_t not_found, const char * failed, uchar * path);
static ngx_int_t ngx_http_dav_location(ngx_http_request_t * r, uchar * path);
...
static ngx_command_t ngx_http_dav_commands[] = {
	{ ngx_string("dav_methods"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_dav_loc_conf_t, methods), &ngx_http_dav_methods_mask },
	...
	{ ngx_string("min_delete_depth"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
	  ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_dav_loc_conf_t, min_delete_depth), NULL },
	{ ngx_string("dav_access"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE123,
	  ngx_conf_set_access_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_dav_loc_conf_t, access), NULL },
	ngx_null_command
};
...
{
	ngx_int_t rc;
	ngx_http_dav_loc_conf_t  * dlcf = (ngx_http_dav_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_dav_module);
	...
	if(ngx_file_info(path.data, &fi) == NGX_FILE_ERROR) {
		...
		if(ngx_is_dir(&fi)) {
			...
			if(ngx_delete_file(temp->data) == NGX_FILE_ERROR) {
				ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno, ngx_delete_file_n " \"%s\" failed", temp->data);
			}
...
static ngx_int_t ngx_http_dav_copy_move_handler(ngx_http_request_t * r)
{
	...
	if(ngx_strncmp(host, r->headers_in.server.data, len) != 0) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "\"Destination\" URI \"%V\" is handled by different repository than the source URI",
		    &dest->value);
		return NGX_HTTP_BAD_REQUEST;
	}
...
destination_done:
	...
	if((r->uri.data[r->uri.len - 1] == '/' && *(last - 1) != '/') || (r->uri.data[r->uri.len - 1] != '/' && *(last - 1) == '/')) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "both URI \"%V\" and \"Destination\" URI \"%V\" should be either collections or non-collections",
		    &r->uri, &dest->value);
		return NGX_HTTP_CONFLICT;
	}
	depth = ngx_http_dav_depth(r, NGX_HTTP_DAV_INFINITY_DEPTH);
...
overwrite_done:
	...
	if(ngx_link_info(copy.path.data, &fi) == NGX_FILE_ERROR) {
		...
		if(ngx_is_dir(&fi) && !slash) {
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "\"%V\" could not be %Ved to collection \"%V\"", &r->uri, &r->method_name, &dest->value);
			return NGX_HTTP_CONFLICT;
		}
		if(!overwrite) {
...
static ngx_int_t ngx_http_dav_copy_dir(ngx_tree_ctx_t * ctx, ngx_str_t * path)
{
	...
	dir = (uchar *)ngx_alloc(len + 1, ctx->log);
	if(dir == NULL) {
		return NGX_ABORT;
	}
...
static ngx_int_t ngx_http_dav_copy_dir_time(ngx_tree_ctx_t * ctx, ngx_str_t * path)
{
...
#if (NGX_WIN32)
	{
		...
		if(ngx_close_file(fd) == NGX_FILE_ERROR) {
			ngx_log_error(NGX_LOG_ALERT, ctx->log, ngx_errno, ngx_close_file_n " \"%s\" failed", dir);
		}
	}
failed:
#else
...
static ngx_int_t ngx_http_dav_copy_tree_file(ngx_tree_ctx_t * ctx, ngx_str_t * path)
{
	...
	copy = (ngx_http_dav_copy_ctx_t *)ctx->data;
	len = copy->path.len + path->len;
	file = (uchar *)ngx_alloc(len + 1, ctx->log);
	if(file == NULL) {
		return NGX_ABORT;
	}
...
static ngx_int_t ngx_http_dav_location(ngx_http_request_t * r, uchar * path)
{
	...
	if(!clcf->alias && clcf->root_lengths == NULL) {
		...
		location = (uchar *)ngx_pnalloc(r->pool, r->uri.len);
		if(location == NULL) {
...
static char * ngx_http_dav_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_dav_loc_conf_t  * prev = (ngx_http_dav_loc_conf_t *)parent;
	ngx_http_dav_loc_conf_t  * conf = (ngx_http_dav_loc_conf_t *)child;
