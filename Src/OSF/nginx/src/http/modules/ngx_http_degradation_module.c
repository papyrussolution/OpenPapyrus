...
ngx_uint_t ngx_http_degraded(ngx_http_request_t * r)
{
	...
	ngx_http_degradation_main_conf_t  * dmcf = (ngx_http_degradation_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_degradation_module);
	if(dmcf->sbrk_size) {
		log = 0;
		now = ngx_time();
		/* lock mutex */
...
{
	ngx_http_degradation_loc_conf_t * dlcf = (ngx_http_degradation_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_degradation_module);
...
static void * ngx_http_degradation_create_main_conf(ngx_conf_t * cf)
{
	ngx_http_degradation_main_conf_t  * dmcf = (ngx_http_degradation_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_degradation_main_conf_t));
	if(dmcf == NULL) {
		return NULL;
	}
	return dmcf;
...
static void * ngx_http_degradation_create_loc_conf(ngx_conf_t * cf)
{
	...
	conf = (ngx_http_degradation_loc_conf_t *)ngx_palloc(cf->pool, sizeof(ngx_http_degradation_loc_conf_t));
	if(conf == NULL) {
		return NULL;
	}
...
{
	ngx_http_degradation_loc_conf_t  * prev = (ngx_http_degradation_loc_conf_t *)parent;
	ngx_http_degradation_loc_conf_t  * conf = (ngx_http_degradation_loc_conf_t *)child;
...
{
	ngx_http_degradation_main_conf_t  * dmcf = (ngx_http_degradation_main_conf_t *)conf;
	ngx_str_t  s;
