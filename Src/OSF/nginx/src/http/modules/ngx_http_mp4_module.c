...
struct ngx_http_mp4_trak_t {
	...
	nginx_off_t start_offset;
	nginx_off_t end_offset;
	size_t tkhd_size;
	size_t mdhd_size;
	size_t hdlr_size;
	size_t vmhd_size;
...
struct ngx_http_mp4_file_t {
	...
	uchar moov_atom_header[8];
	uchar mdat_atom_header[16];
};
...
#define ngx_mp4_get_32value(p)						      \
	( ((uint32_t)((uchar *)(p))[0] << 24)				       \
	    + (           ((uchar *)(p))[1] << 16)				    \
	    + (           ((uchar *)(p))[2] << 8)				    \
	    + (           ((uchar *)(p))[3]) )

#define ngx_mp4_set_32value(p, n)					      \
	((uchar *)(p))[0] = (uchar)((n) >> 24);			       \
	((uchar *)(p))[1] = (uchar)((n) >> 16);			       \
	((uchar *)(p))[2] = (uchar)((n) >> 8);				       \
	((uchar *)(p))[3] = (uchar)(n)

#define ngx_mp4_get_64value(p)						      \
	( ((uint64_t)((uchar *)(p))[0] << 56)				       \
	    + ((uint64_t)((uchar *)(p))[1] << 48)				   \
	    + ((uint64_t)((uchar *)(p))[2] << 40)				   \
	    + ((uint64_t)((uchar *)(p))[3] << 32)				   \
	    + ((uint64_t)((uchar *)(p))[4] << 24)				   \
	    + (           ((uchar *)(p))[5] << 16)				    \
	    + (           ((uchar *)(p))[6] << 8)				    \
	    + (           ((uchar *)(p))[7]) )

#define ngx_mp4_set_64value(p, n)					      \
	((uchar *)(p))[0] = (uchar)((uint64_t)(n) >> 56);		      \
	((uchar *)(p))[1] = (uchar)((uint64_t)(n) >> 48);		      \
	((uchar *)(p))[2] = (uchar)((uint64_t)(n) >> 40);		      \
	((uchar *)(p))[3] = (uchar)((uint64_t)(n) >> 32);		      \
	((uchar *)(p))[4] = (uchar)(           (n) >> 24);		       \
	((uchar *)(p))[5] = (uchar)(           (n) >> 16);		       \
	((uchar *)(p))[6] = (uchar)(           (n) >> 8);		       \
	((uchar *)(p))[7] = (uchar)(n)
...
static ngx_int_t ngx_http_mp4_atofp(uchar * line, size_t n, size_t point);
...
static size_t ngx_http_mp4_update_mdat_atom(ngx_http_mp4_file_t * mp4, nginx_off_t start_offset, nginx_off_t end_offset);
...
static ngx_int_t ngx_http_mp4_handler(ngx_http_request_t * r)
{
	...
	b = NULL;
	if(r->args.len) {
		if(ngx_http_arg(r, (const uchar *)"start", 5, &value) == NGX_OK) {
			/*
		...
		if(ngx_http_arg(r, (const uchar *)"end", 3, &value) == NGX_OK) {
	...
	if(start >= 0) {
		r->single_range = 1;
		mp4 = (ngx_http_mp4_file_t *)ngx_pcalloc(r->pool, sizeof(ngx_http_mp4_file_t));
		if(mp4 == NULL) {
			return NGX_HTTP_INTERNAL_SERVER_ERROR;
		}
		mp4->file.fd = of.fd;
...
static ngx_int_t ngx_http_mp4_process(ngx_http_mp4_file_t * mp4)
{
	nginx_off_t start_offset, end_offset, adjustment;
	ngx_int_t rc;
	ngx_uint_t i, j;
	ngx_chain_t ** prev;
	...
	conf = (ngx_http_mp4_conf_t *)ngx_http_get_module_loc_conf(mp4->request, ngx_http_mp4_module);
	...
	*prev = &mp4->moov_atom;
	...
	end_offset = 0;
	trak = (ngx_http_mp4_trak_t *)mp4->trak.elts;
	...
	*prev = &mp4->mdat_atom;
	...
	adjustment = mp4->ftyp_size + mp4->moov_size
	    + ngx_http_mp4_update_mdat_atom(mp4, start_offset, end_offset) - start_offset;
    ...
    ngx_http_mp4_atom_handler_t * atom, uint64_t atom_data_size)
{
	...
	uchar * atom_header, * atom_name;
	uint64_t atom_size;
	ngx_int_t rc;
	ngx_uint_t n;
	nginx_off_t end = mp4->offset + atom_data_size;
	while(mp4->offset < end) {
		...
		if(atom_size > (uint64_t)(NGX_MAX_OFF_T_VALUE - mp4->offset) || mp4->offset + (nginx_off_t)atom_size > end) {
...
static ngx_int_t ngx_http_mp4_read(ngx_http_mp4_file_t * mp4, size_t size)
{
	...
	if(mp4->offset + (nginx_off_t)mp4->buffer_size > mp4->end) {
	...
	if(mp4->buffer == NULL) {
		mp4->buffer = (uchar *)ngx_palloc(mp4->request->pool, mp4->buffer_size);
		if(mp4->buffer == NULL) {
			return NGX_ERROR;
		}
	...
	if((size_t)n != mp4->buffer_size) {
		ngx_log_error(NGX_LOG_CRIT, mp4->file.log, 0, ngx_read_file_n " read only %z of %z from \"%s\"", n, mp4->buffer_size, mp4->file.name.data);
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_mp4_read_ftyp_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	...
	ftyp_atom = (uchar *)ngx_palloc(mp4->request->pool, atom_size);
	if(ftyp_atom == NULL) {
		return NGX_ERROR;
	}
	 ...
	 */
	memcpy(ftyp_atom + sizeof(ngx_mp4_atom_header_t), ngx_mp4_atom_data(mp4), (size_t)atom_data_size);
...
static ngx_int_t ngx_http_mp4_read_moov_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	...
	if(atom_data_size > mp4->buffer_size) {
		if(atom_data_size > conf->max_buffer_size) {
			ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0, "\"%s\" mp4 moov atom is too large:%uL, you may want to increase mp4_max_buffer_size",
...
static size_t ngx_http_mp4_update_mdat_atom(ngx_http_mp4_file_t * mp4, nginx_off_t start_offset,
    nginx_off_t end_offset)
{
	nginx_off_t atom_data_size;
	uchar   * atom_header;
...
typedef struct {
	...
	uchar reserved[10];
	uchar matrix[36];
	uchar preview_time[4];
	uchar preview_duration[4];
	uchar poster_time[4];
	uchar selection_time[4];
	uchar selection_duration[4];
	uchar current_time[4];
...
typedef struct {
	...
	uchar version[1];
	uchar flags[3];
	uchar creation_time[8];
	uchar modification_time[8];
	uchar timescale[4];
	uchar duration[8];
	uchar rate[4];
	uchar volume[2];
...
static ngx_int_t ngx_http_mp4_read_mvhd_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	...
	ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mvhd timescale:%uD, duration:%uL, time:%.3fs", timescale, duration, (double)duration / timescale);
...
static ngx_int_t ngx_http_mp4_read_trak_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	uchar * atom_header, * atom_end;
	nginx_off_t atom_file_end;
	ngx_int_t rc;
	ngx_buf_t  * atom;
	ngx_http_mp4_trak_t  * trak;
	ngx_log_debug0(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 trak atom");
	trak = (ngx_http_mp4_trak_t *)ngx_array_push(&mp4->trak);
	if(trak == NULL) {
		return NGX_ERROR;
	}
	memzero(trak, sizeof(ngx_http_mp4_trak_t));
	atom_header = ngx_mp4_atom_header(mp4);
...
static ngx_int_t ngx_http_mp4_read_mdhd_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	...
	ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mdhd timescale:%uD, duration:%uL, time:%.3fs", timescale, duration, (double)duration / timescale);
	start_time = (uint64_t)mp4->start * timescale / 1000;
	if(duration <= start_time) {
...
static ngx_int_t ngx_http_mp4_read_stsd_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	...
	ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "stsd entries:%uD, media:%*s", ngx_mp4_get_32value(stsd_atom->entries), (size_t)4, stsd_atom->media_name);
	trak = ngx_mp4_last_trak(mp4);
...
static ngx_int_t ngx_http_mp4_read_stts_atom(ngx_http_mp4_file_t * mp4, uint64_t atom_data_size)
{
	uchar * atom_header, * atom_table, * atom_end;
	uint32_t entries;
	ngx_buf_t  * atom, * data;
...
static void ngx_http_mp4_update_ctts_atom(ngx_http_mp4_file_t * mp4, ngx_http_mp4_trak_t * trak)
{
	...
	ngx_buf_t * data = trak->out[NGX_HTTP_MP4_CTTS_DATA].buf;
	if(data) {
		...
		if(trak->composition_offset_entries == 0) {
			...
			ngx_buf_t * atom;
			ngx_mp4_ctts_atom_t * ctts_atom;
			size_t atom_size = sizeof(ngx_mp4_ctts_atom_t) + (data->last - data->pos);
			trak->size += atom_size;
...
static void ngx_http_mp4_crop_ctts_data(ngx_http_mp4_file_t * mp4, ngx_http_mp4_trak_t * trak, ngx_uint_t start)
{
	...
	while(entry < end) {
		count = ngx_mp4_get_32value(entry->count);
		ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "sample:%uD, count:%uD, offset:%uD", start_sample, count, ngx_mp4_get_32value(entry->offset));
...
static ngx_int_t ngx_http_mp4_crop_stsc_data(ngx_http_mp4_file_t * mp4, ngx_http_mp4_trak_t * trak, ngx_uint_t start)
{
	...
	ngx_buf_t   * data, * buf;
	ngx_uint_t target_chunk, chunk_samples;
	ngx_mp4_stsc_entry_t  * entry, * end, * first;
	ngx_uint_t entries = trak->sample_to_chunk_entries - 1;
	if(start) {
	...
	while(entry < end) {
		...
		ngx_log_debug5(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "sample:%uD, chunk:%uD, chunks:%uD, samples:%uD, id:%uD",
	...
	ngx_log_debug4(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "sample:%uD, chunk:%uD, chunks:%uD, samples:%uD", start_sample, chunk, next_chunk - chunk, samples);
	n = (next_chunk - chunk) * samples;
	if(start_sample > n) {
		ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0, "%s time is out mp4 stsc chunks in \"%s\"", start ? "start" : "end", mp4->file.name.data);
		return NGX_ERROR;
	}
found:
	entries++;
	...
	if(start) {
		...
		ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "start_chunk:%ui, start_chunk_samples:%ui", trak->start_chunk, trak->start_chunk_samples);
	}
	else {
		if(start_sample) {
			data->last = (uchar *)(entry + 1);
		...
		samples = chunk_samples;
		next_chunk = chunk + 1;
		ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "end_chunk:%ui, end_chunk_samples:%ui", trak->end_chunk, trak->end_chunk_samples);
	}
	if(chunk_samples && next_chunk - target_chunk == 2) {
		...
		buf->temporary = 1;
		buf->pos = (uchar *)first;
		buf->last = (uchar *)first + sizeof(ngx_mp4_stsc_entry_t);
...
static void ngx_http_mp4_adjust_co64_atom(ngx_http_mp4_file_t * mp4, ngx_http_mp4_trak_t * trak, nginx_off_t adjustment)
{
	...
	ngx_buf_t * data = trak->out[NGX_HTTP_MP4_CO64_DATA].buf;
	uint64_t * end = (uint64_t*)data->last;
	uint64_t * entry = (uint64_t*)data->pos;
	while(entry < end) {
		uint64_t offset = ngx_mp4_get_64value(entry);
		offset += adjustment;
...
static void * ngx_http_mp4_create_conf(ngx_conf_t * cf)
{
	ngx_http_mp4_conf_t  * conf = (ngx_http_mp4_conf_t *)ngx_palloc(cf->pool, sizeof(ngx_http_mp4_conf_t));
	if(conf) {
...
static char * ngx_http_mp4_merge_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_mp4_conf_t * prev = (ngx_http_mp4_conf_t *)parent;
	ngx_http_mp4_conf_t * conf = (ngx_http_mp4_conf_t *)child;
