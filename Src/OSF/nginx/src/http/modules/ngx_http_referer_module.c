...
static char * ngx_http_referer_merge_conf(ngx_conf_t * cf, void * parent, void * child)
{
	...
	if(conf->keys->keys.nelts) {
		...
		if(ngx_hash_init(&hash, (ngx_hash_key_t*)conf->keys->keys.elts, conf->keys->keys.nelts)
		    != NGX_OK) {
			return NGX_CONF_ERROR;
		}
	}
	...
	if(conf->keys->dns_wc_head.nelts) {
		...
		if(ngx_hash_wildcard_init(&hash, (ngx_hash_key_t*)conf->keys->dns_wc_head.elts,
	...
	if(conf->keys->dns_wc_tail.nelts) {
		...
		if(ngx_hash_wildcard_init(&hash, (ngx_hash_key_t*)conf->keys->dns_wc_tail.elts,
