...
struct ngx_http_addition_conf_t {
...
struct ngx_http_addition_ctx_t {
	ngx_uint_t before_body_sent;
};

static void * ngx_http_addition_create_conf(ngx_conf_t * cf);
static char * ngx_http_addition_merge_conf(ngx_conf_t * cf, void * parent, void * child); // @callback
...
static ngx_command_t ngx_http_addition_commands[] = {
	...
	{ ngx_string("addition_types"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_http_types_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_addition_conf_t, types_keys), &ngx_http_html_default_types[0] },
	ngx_null_command
};
...
static ngx_int_t ngx_http_addition_header_filter(ngx_http_request_t * r)
{
	if(r->headers_out.status != NGX_HTTP_OK || r != r->main) {
		...
		ngx_http_addition_conf_t * conf = (ngx_http_addition_conf_t*)ngx_http_get_module_loc_conf(r, ngx_http_addition_filter_module);
		if(conf->before_body.len == 0 && conf->after_body.len == 0) {
			return ngx_http_next_header_filter(r);
		}
		else if(ngx_http_test_content_type(r, &conf->types) == NULL) {
			return ngx_http_next_header_filter(r);
		}
		else {
...
static char * ngx_http_addition_merge_conf(ngx_conf_t * cf, void * parent, void * child) // @callback
{
	ngx_http_addition_conf_t * prev = (ngx_http_addition_conf_t *)parent;
	ngx_http_addition_conf_t * conf = (ngx_http_addition_conf_t *)child;
