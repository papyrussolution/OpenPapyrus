...
struct ngx_http_realip_loc_conf_t {
...
static ngx_command_t ngx_http_realip_commands[] = {
	{ ngx_string("set_real_ip_from"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
	  ngx_http_realip_from, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("real_ip_header"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
...
static ngx_int_t ngx_http_realip_handler(ngx_http_request_t * r)
{
	...
	ngx_http_realip_loc_conf_t  * rlcf = (ngx_http_realip_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_realip_module);
	if(rlcf->from == NULL) {
		return NGX_DECLINED;
	}
...
static ngx_int_t ngx_http_realip_set_addr(ngx_http_request_t * r, ngx_addr_t * addr)
{
	ngx_pool_cleanup_t * cln = ngx_pool_cleanup_add(r->pool, sizeof(ngx_http_realip_ctx_t));
	if(cln == NULL) {
...
static void ngx_http_realip_cleanup(void * data)
{
	ngx_http_realip_ctx_t * ctx = (ngx_http_realip_ctx_t *)data;
...
static const char * ngx_http_realip_from(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	if(rc != NGX_ERROR) {
		...
		cidr = (ngx_cidr_t *)ngx_array_push(rlcf->from);
		if(cidr == NULL) {
			return NGX_CONF_ERROR;
		}
		*cidr = c;
	...
	cidr = (ngx_cidr_t*)ngx_array_push_n(rlcf->from, u.naddrs);
	if(cidr == NULL) {
		return NGX_CONF_ERROR;
	}
...
static void * ngx_http_realip_create_loc_conf(ngx_conf_t * cf)
{
	ngx_http_realip_loc_conf_t  * conf = (ngx_http_realip_loc_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_realip_loc_conf_t));
	if(conf) {
		/*
		 * set by ngx_pcalloc():
		 *
...
static char * ngx_http_realip_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_realip_loc_conf_t  * prev = (ngx_http_realip_loc_conf_t *)parent;
	ngx_http_realip_loc_conf_t  * conf = (ngx_http_realip_loc_conf_t *)child;
	if(conf->from == NULL) {
		conf->from = prev->from;
	}
...
static ngx_int_t ngx_http_realip_init(ngx_conf_t * cf)
{
	...
	ngx_http_handler_pt * h = (ngx_http_handler_pt *)ngx_array_push(&cmcf->phases[NGX_HTTP_POST_READ_PHASE].handlers);
	if(h == NULL) {
		return NGX_ERROR;
	}
...
static ngx_http_realip_ctx_t * ngx_http_realip_get_module_ctx(ngx_http_request_t * r)
{
	...
	ngx_http_realip_ctx_t  * ctx = (ngx_http_realip_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_realip_module);
	if(ctx == NULL && (r->internal || r->filter_finalize)) {
		...
		for(cln = r->pool->cleanup; cln; cln = cln->next) {
			if(cln->handler == ngx_http_realip_cleanup) {
				ctx = (ngx_http_realip_ctx_t *)cln->data;
				break;
			}
		}
	}
...
static ngx_int_t ngx_http_realip_remote_addr_variable(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	ngx_http_realip_ctx_t  * ctx = ngx_http_realip_get_module_ctx(r);
	ngx_str_t * addr_text = ctx ? &ctx->addr_text : &r->connection->addr_text;
	v->len = addr_text->len;
	v->valid = 1;
	v->no_cacheable = 0;
	v->not_found = 0;
...
static ngx_int_t ngx_http_realip_remote_port_variable(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	ngx_uint_t port;
	ngx_http_realip_ctx_t  * ctx = ngx_http_realip_get_module_ctx(r);
	struct sockaddr * sa = ctx ? ctx->sockaddr : r->connection->sockaddr;
	v->len = 0;
	v->valid = 1;
	v->no_cacheable = 0;
	v->not_found = 0;
