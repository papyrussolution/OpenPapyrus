...
static ngx_int_t ngx_http_slice_range_variable(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	if(!ctx) {
		...
		p = (uchar *)ngx_pnalloc(r->pool, sizeof("bytes=-") - 1 + 2 * NGX_OFF_T_LEN);
		if(!p) {
			return NGX_ERROR;
		}
...
static nginx_off_t ngx_http_slice_get_start(ngx_http_request_t * r)
{
	...
	if(h == NULL
	   || h->value.len < 7
	   || ngx_strncasecmp(h->value.data, (uchar *)"bytes=", 6) != 0) {
		return 0;
	}
...
{
	ngx_http_slice_loc_conf_t  * slcf;
	slcf = (ngx_http_slice_loc_conf_t *)ngx_palloc(cf->pool, sizeof(ngx_http_slice_loc_conf_t));
	if(slcf == NULL) {
		return NULL;
	}
...
{
	ngx_http_slice_loc_conf_t * prev = (ngx_http_slice_loc_conf_t *)parent;
	ngx_http_slice_loc_conf_t * conf = (ngx_http_slice_loc_conf_t *)child;
...
static ngx_int_t ngx_http_slice_add_variables(ngx_conf_t * cf)
{
	ngx_http_variable_t  * var = ngx_http_add_variable(cf, &ngx_http_slice_range_name, 0);
	if(var == NULL) {
		return NGX_ERROR;
	}
