...
static void * ngx_http_fastcgi_create_main_conf(ngx_conf_t * cf)
{
	...
	conf = (ngx_http_fastcgi_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_fastcgi_main_conf_t));
	if(conf == NULL) {
		return NULL;
	}
#if (NGX_HTTP_CACHE)
...
static void * ngx_http_fastcgi_create_loc_conf(ngx_conf_t * cf)
{
	...
	conf = (ngx_http_fastcgi_loc_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_fastcgi_loc_conf_t));
	if(conf == NULL) {
		return NULL;
	}
...
next:
	...
	for(i = 0; i < nsrc; i++) {
		...
		copy = (ngx_http_script_copy_code_t *)ngx_array_push_n(params->lengths,
