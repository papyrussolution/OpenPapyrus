...
struct ngx_http_fastcgi_header_t {
	...
	uchar request_id_hi;
	uchar request_id_lo;
	uchar content_length_hi;
	uchar content_length_lo;
...
static ngx_int_t ngx_http_fastcgi_init_params(ngx_conf_t * cf, ngx_http_fastcgi_loc_conf_t * conf, ngx_http_fastcgi_params_t * params, ngx_keyval_t * default_params);
...
static const char * ngx_http_fastcgi_pass(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
static const char * ngx_http_fastcgi_split_path_info(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
static const char * ngx_http_fastcgi_store(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
#if (NGX_HTTP_CACHE)
	static const char * ngx_http_fastcgi_cache(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
	static const char * ngx_http_fastcgi_cache_key(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
#endif
...
static ngx_command_t ngx_http_fastcgi_commands[] = {
	{ ngx_string("fastcgi_pass"), NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1, ngx_http_fastcgi_pass, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("fastcgi_index"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, index), NULL },
	{ ngx_string("fastcgi_split_path_info"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_http_fastcgi_split_path_info, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("fastcgi_store"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_http_fastcgi_store, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("fastcgi_store_access"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE123, ngx_conf_set_access_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.store_access), NULL },
	{ ngx_string("fastcgi_buffering"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.buffering), NULL },
	{ ngx_string("fastcgi_request_buffering"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.request_buffering), NULL },
	{ ngx_string("fastcgi_ignore_client_abort"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.ignore_client_abort), NULL },
	{ ngx_string("fastcgi_bind"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE12, ngx_http_upstream_bind_set_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.local), NULL },
	{ ngx_string("fastcgi_connect_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.connect_timeout), NULL },
	{ ngx_string("fastcgi_send_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.send_timeout), NULL },
	{ ngx_string("fastcgi_send_lowat"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.send_lowat), &ngx_http_fastcgi_lowat_post },
	{ ngx_string("fastcgi_buffer_size"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.buffer_size), NULL },
	{ ngx_string("fastcgi_pass_request_headers"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.pass_request_headers), NULL },
	{ ngx_string("fastcgi_pass_request_body"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.pass_request_body), NULL },
	{ ngx_string("fastcgi_intercept_errors"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.intercept_errors), NULL },
	{ ngx_string("fastcgi_read_timeout"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.read_timeout), NULL },
	{ ngx_string("fastcgi_buffers"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE2, ngx_conf_set_bufs_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.bufs), NULL },
	{ ngx_string("fastcgi_busy_buffers_size"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.busy_buffers_size_conf), NULL },
	{ ngx_string("fastcgi_force_ranges"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.force_ranges), NULL },
	{ ngx_string("fastcgi_limit_rate"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.limit_rate), NULL },
#if (NGX_HTTP_CACHE)
	{ngx_string("fastcgi_cache"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_http_fastcgi_cache, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("fastcgi_cache_key"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_http_fastcgi_cache_key, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL },
	{ ngx_string("fastcgi_cache_path"), NGX_HTTP_MAIN_CONF|NGX_CONF_2MORE, ngx_http_file_cache_set_slot, NGX_HTTP_MAIN_CONF_OFFSET, offsetof(ngx_http_fastcgi_main_conf_t, caches), &ngx_http_fastcgi_module },
	{ ngx_string("fastcgi_cache_bypass"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_http_set_predicate_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.cache_bypass), NULL },
	{ ngx_string("fastcgi_no_cache"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE, ngx_http_set_predicate_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.no_cache), NULL },
	...
	{ ngx_string("fastcgi_cache_use_stale"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.cache_use_stale), &ngx_http_fastcgi_next_upstream_masks },
	{ ngx_string("fastcgi_cache_methods"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
	  ngx_conf_set_bitmask_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, upstream.cache_methods), &ngx_http_upstream_cache_method_mask },
	...
	{ ngx_string("fastcgi_catch_stderr"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1, ngx_conf_set_str_array_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, catch_stderr), NULL },
	{ ngx_string("fastcgi_keep_conn"), NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG, ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_fastcgi_loc_conf_t, keep_conn), NULL },
...
static ngx_http_variable_t ngx_http_fastcgi_vars[] = {
	{ ngx_string("fastcgi_script_name"), NULL, ngx_http_fastcgi_script_name_variable, 0, NGX_HTTP_VAR_NOCACHEABLE|NGX_HTTP_VAR_NOHASH, 0 },
...
static ngx_int_t ngx_http_fastcgi_handler(ngx_http_request_t * r)
{
	...
	f = (ngx_http_fastcgi_ctx_t *)ngx_pcalloc(r->pool, sizeof(ngx_http_fastcgi_ctx_t));
	if(!f) {
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
	ngx_http_set_ctx(r, f, ngx_http_fastcgi_module);
	flcf = (ngx_http_fastcgi_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_fastcgi_module);
...
#if (NGX_HTTP_CACHE)
	fmcf = (ngx_http_fastcgi_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_fastcgi_module);
	...
	u->pipe = (ngx_event_pipe_t *)ngx_pcalloc(r->pool, sizeof(ngx_event_pipe_t));
	if(u->pipe == NULL) {
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
...
static ngx_int_t ngx_http_fastcgi_create_request(ngx_http_request_t * pReq)
{
	ngx_int_t result = NGX_OK;
	nginx_off_t file_pos;
	uchar ch, * pos, * lowcase_key;
	size_t size, /*key_len,*/ val_len, padding, allocated;
	ngx_uint_t i, n, next, hash, skip_empty;
	ngx_buf_t * p_temp_buf;
	ngx_chain_t * cl, * body;
	...
	size_t len = 0;
	ngx_uint_t header_params = 0;
	ngx_table_elt_t ** ignored = NULL;
	ngx_http_upstream_t * u = pReq->upstream;
	ngx_http_fastcgi_loc_conf_t * flcf = (ngx_http_fastcgi_loc_conf_t *)ngx_http_get_module_loc_conf(pReq, ngx_http_fastcgi_module);
#if (NGX_HTTP_CACHE)
	ngx_http_fastcgi_params_t * params = u->cacheable ? &flcf->params_cache : &flcf->params;
#else
	ngx_http_fastcgi_params_t * params = &flcf->params;
#endif
	if(params->lengths) {
		memzero(&le, sizeof(ngx_http_script_engine_t));
		ngx_http_script_flush_no_cacheable_variables(pReq, params->flushes);
		le.flushed = 1;
		le.ip = (uchar *)params->lengths->elts;
		le.request = pReq;
		while(*(uintptr_t*)le.ip) {
			ngx_http_script_len_code_pt lcode = *(ngx_http_script_len_code_pt*)le.ip;
			size_t key_len = lcode(&le);
	...
	if(flcf->upstream.pass_request_headers) {
		...
		if(params->number) {
			n = 0;
			part = &pReq->headers_in.headers.part;
			while(part) {
				n += part->nelts;
				part = part->next;
			}
			ignored = (ngx_table_elt_t **)ngx_palloc(pReq->pool, n * sizeof(void *));
			THROW(ignored);
		}
		part = &pReq->headers_in.headers.part;
		...
		for(i = 0; /* void */; i++) {
			...
			if(params->number) {
				if(allocated < header[i].key.len) {
					...
					lowcase_key = (uchar *)ngx_pnalloc(pReq->pool, allocated);
					if(lowcase_key == NULL) {
						return NGX_ERROR;
					}
				}
	...
	if(len > 65535) {
		ngx_log_error(NGX_LOG_ALERT, pReq->connection->log, 0, "fastcgi request record is too big: %uz", len);
		CALLEXCEPT();
	}
	padding = 8 - len % 8;
	...
	p_temp_buf = ngx_create_temp_buf(pReq->pool, size);
	THROW(p_temp_buf);
	cl = ngx_alloc_chain_link(pReq->pool);
	THROW(cl);
	cl->buf = p_temp_buf;
	ngx_http_fastcgi_request_start.br.flags = flcf->keep_conn ? NGX_HTTP_FASTCGI_KEEP_CONN : 0;
	memcpy(p_temp_buf->pos, &ngx_http_fastcgi_request_start, sizeof(ngx_http_fastcgi_request_start_t));
	h = (ngx_http_fastcgi_header_t*)(p_temp_buf->pos + sizeof(ngx_http_fastcgi_header_t) + sizeof(ngx_http_fastcgi_begin_request_t));
	h->content_length_hi = (uchar)((len >> 8) & 0xff);
	h->content_length_lo = (uchar)(len & 0xff);
	h->padding_length = (uchar)padding;
	h->reserved = 0;
	p_temp_buf->last = p_temp_buf->pos + sizeof(ngx_http_fastcgi_header_t) + sizeof(ngx_http_fastcgi_begin_request_t) + sizeof(ngx_http_fastcgi_header_t);
	if(params->lengths) {
		memzero(&e, sizeof(ngx_http_script_engine_t));
		e.ip = (uchar *)params->values->elts;
		e.pos = p_temp_buf->last;
		e.request = pReq;
		e.flushed = 1;
		le.ip = (uchar *)params->lengths->elts;
		while(*(uintptr_t*)le.ip) {
			ngx_http_script_len_code_pt lcode = *(ngx_http_script_len_code_pt*)le.ip;
			const size_t key_len = (uchar)lcode(&le);
			lcode = *(ngx_http_script_len_code_pt*)le.ip;
			skip_empty = lcode(&le);
			for(val_len = 0; *(uintptr_t*)le.ip; val_len += lcode(&le)) {
				lcode = *(ngx_http_script_len_code_pt*)le.ip;
			...
			*e.pos++ = (uchar)key_len;
			if(val_len > 127) {
				*e.pos++ = (uchar)(((val_len >> 24) & 0x7f) | 0x80);
				*e.pos++ = (uchar)((val_len >> 16) & 0xff);
			...
			e.ip += sizeof(uintptr_t);
			ngx_log_debug4(NGX_LOG_DEBUG_HTTP, pReq->connection->log, 0, "fastcgi param: \"%*s: %*s\"",
			    key_len, e.pos - (key_len + val_len), val_len, e.pos - val_len);
		}
		p_temp_buf->last = e.pos;
	}
	if(flcf->upstream.pass_request_headers) {
		part = &pReq->headers_in.headers.part;
		header = (ngx_table_elt_t *)part->elts;
		for(i = 0; /* void */; i++) {
			...
			{
				const size_t key_len = sizeof("HTTP_") - 1 + header[i].key.len;
				if(key_len > 127) {
					*p_temp_buf->last++ = (uchar)(((key_len >> 24) & 0x7f) | 0x80);
					*p_temp_buf->last++ = (uchar)((key_len >> 16) & 0xff);
					*p_temp_buf->last++ = (uchar)((key_len >> 8) & 0xff);
					*p_temp_buf->last++ = (uchar)(key_len & 0xff);
				}
				else {
					*p_temp_buf->last++ = (uchar)key_len;
				}
				val_len = header[i].value.len;
				if(val_len > 127) {
					*p_temp_buf->last++ = (uchar)(((val_len >> 24) & 0x7f) | 0x80);
					*p_temp_buf->last++ = (uchar)((val_len >> 16) & 0xff);
					*p_temp_buf->last++ = (uchar)((val_len >> 8) & 0xff);
					*p_temp_buf->last++ = (uchar)(val_len & 0xff);
				}
				else {
					*p_temp_buf->last++ = (uchar)val_len;
				}
				p_temp_buf->last = ngx_cpymem(p_temp_buf->last, "HTTP_", sizeof("HTTP_") - 1);
				for(n = 0; n < header[i].key.len; n++) {
					ch = header[i].key.data[n];
					if(ch >= 'a' && ch <= 'z') {
						ch &= ~0x20;
				...
				p_temp_buf->last = ngx_copy(p_temp_buf->last, header[i].value.data, val_len);
				ngx_log_debug4(NGX_LOG_DEBUG_HTTP, pReq->connection->log, 0, "fastcgi param: \"%*s: %*s\"",
					key_len, p_temp_buf->last - (key_len + val_len), val_len, p_temp_buf->last - val_len);
			}
next:
			continue;
		}
	...
	if(padding) {
		memzero(p_temp_buf->last, padding);
		p_temp_buf->last += padding;
	}
	h = (ngx_http_fastcgi_header_t *)p_temp_buf->last;
	p_temp_buf->last += sizeof(ngx_http_fastcgi_header_t);
	h->version = 1;
	...
	if(pReq->request_body_no_buffering) {
		...
		while(body) {
			...
			next = 0;
			do {
				p_temp_buf = (ngx_buf_t*)ngx_alloc_buf(pReq->pool);
				THROW(p_temp_buf);
				memcpy(p_temp_buf, body->buf, sizeof(ngx_buf_t));
				if(body->buf->in_file) {
					p_temp_buf->file_pos = file_pos;
					file_pos += 32 * 1024;
					if(file_pos >= body->buf->file_last) {
						file_pos = body->buf->file_last;
						next = 1;
					}
					p_temp_buf->file_last = file_pos;
					len = (ngx_uint_t)(file_pos - p_temp_buf->file_pos);
				}
				else {
					p_temp_buf->pos = pos;
					p_temp_buf->start = pos;
					pos += 32 * 1024;
					...
					p_temp_buf->last = pos;
					len = (ngx_uint_t)(pos - p_temp_buf->pos);
				}
				padding = 8 - len % 8;
				padding = (padding == 8) ? 0 : padding;
				...
				h->padding_length = (uchar)padding;
				h->reserved = 0;
				cl->next = ngx_alloc_chain_link(pReq->pool);
				THROW(cl->next);
				cl = cl->next;
				cl->buf = p_temp_buf;
				p_temp_buf = ngx_create_temp_buf(pReq->pool, sizeof(ngx_http_fastcgi_header_t) + padding);
				THROW(p_temp_buf);
				if(padding) {
					memzero(p_temp_buf->last, padding);
					p_temp_buf->last += padding;
...
static ngx_int_t ngx_http_fastcgi_body_output_filter(void * data, ngx_chain_t * in)
{
	...
	f = (ngx_http_fastcgi_ctx_t *)ngx_http_get_module_ctx(r, ngx_http_fastcgi_module);
	if(in == NULL) {
		out = in;
		goto out;
	}
	...
	cl = ngx_chain_get_free_buf(r->pool, &f->P_Free);
	if(cl == NULL) {
		return NGX_ERROR;
	}
	b = cl->buf;
	...
	if(b->start == NULL) {
		...
		b->start = (uchar *)ngx_palloc(r->pool, sizeof(ngx_http_fastcgi_header_t) + 7);
		if(b->start == NULL) {
			return NGX_ERROR;
		}
		b->pos = b->start;
	...
	*ll = cl;
	...
	while(in) {
		ngx_log_debug7(NGX_LOG_DEBUG_EVENT, r->connection->log, 0,
		    "fastcgi output in  l:%d f:%d %p, pos %p, size: %z file: %O, size: %O",
		    in->buf->last_buf, in->buf->in_file, in->buf->start, in->buf->pos, in->buf->last - in->buf->pos,
		    in->buf->file_pos, in->buf->file_last - in->buf->file_pos);
		if(in->buf->last_buf) {
			last = 1;
		}
		...
		next = 0;
		do {
			tl = ngx_chain_get_free_buf(r->pool, &f->P_Free);
			if(tl == NULL) {
				return NGX_ERROR;
			}
			b = tl->buf;
			start = b->start;
			memcpy(b, in->buf, sizeof(ngx_buf_t));
			//
			// restore b->start to preserve memory allocated in the buffer, to reuse it later for headers and padding
			//
			b->start = start;
			if(in->buf->in_file) {
				b->file_pos = file_pos;
...
out:
	...
	for(cl = out; cl; cl = cl->next) {
		ngx_log_debug7(NGX_LOG_DEBUG_EVENT, r->connection->log, 0, "fastcgi output out l:%d f:%d %p, pos %p, size: %z file: %O, size: %O",
		    cl->buf->last_buf, cl->buf->in_file, cl->buf->start, cl->buf->pos, cl->buf->last - cl->buf->pos, cl->buf->file_pos, cl->buf->file_last - cl->buf->file_pos);
	}
#endif
	rc = ngx_chain_writer(&r->upstream->writer, out);
	ngx_chain_update_chains(r->pool, &f->P_Free, &f->P_Busy, &out, (ngx_buf_tag_t)&ngx_http_fastcgi_body_output_filter);
	for(cl = f->P_Free; cl; cl = cl->next) {
...
static ngx_int_t ngx_http_fastcgi_process_header(ngx_http_request_t * r)
{
	uchar * p, * msg, * start, * last, * part_start, * part_end;
	size_t size;
	...
	for(;;) {
		...
		if(f->large_stderr && r->cache) {
			...
			if(len >= 0) {
				...
				fh->request_id_hi = 0;
				fh->request_id_lo = 1;
				fh->content_length_hi = (uchar)((len >> 8) & 0xff);
				fh->content_length_lo = (uchar)(len & 0xff);
				fh->padding_length = 0;
				fh->reserved = 0;
			}
			else {
		...
		for(;;) {
			...
			if(rc == NGX_OK) {
				...
				h = (ngx_table_elt_t *)ngx_list_push(&u->headers_in.headers);
				if(!h) {
					return NGX_ERROR;
				}
				if(f->split_parts && f->split_parts->nelts) {
					...
					p = (uchar *)ngx_pnalloc(r->pool, size);
					if(!p) {
						h->hash = 0;
						return NGX_ERROR;
					}
		...
		part = (ngx_http_fastcgi_split_part_t *)ngx_array_push(f->split_parts);
		if(part == NULL) {
			return NGX_ERROR;
		}
...
{
	ngx_http_request_t * r = (ngx_http_request_t *)data;
	ngx_http_fastcgi_loc_conf_t  * flcf = (ngx_http_fastcgi_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_fastcgi_module);
	r->upstream->pipe->length = flcf->keep_conn ? (nginx_off_t)sizeof(ngx_http_fastcgi_header_t) : -1;
	return NGX_OK;
}
...
static ngx_int_t ngx_http_fastcgi_non_buffered_filter(void * data, ssize_t bytes)
{
	...
	if(r->subrequest_in_memory) {
		...
		for(cl = u->out_bufs; cl; cl = cl->next) {
			ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http fastcgi in memory %p-%p %O", cl->buf->pos, cl->buf->last, ngx_buf_size(cl->buf));
...
static ngx_int_t ngx_http_fastcgi_process_record(ngx_http_request_t * r, ngx_http_fastcgi_ctx_t * f)
{
	ngx_http_fastcgi_state_e state = f->state;
	for(uchar * p = f->pos; p < f->last; p++) {
		uchar ch = *p;
...
static ngx_int_t ngx_http_fastcgi_add_variables(ngx_conf_t * cf)
{
	for(ngx_http_variable_t * v = ngx_http_fastcgi_vars; v->name.len; v++) {
...
static void * ngx_http_fastcgi_create_loc_conf(ngx_conf_t * cf)
{
	...
	conf->catch_stderr = (ngx_array_t *)NGX_CONF_UNSET_PTR;
...
static char * ngx_http_fastcgi_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_fastcgi_loc_conf_t * prev = (ngx_http_fastcgi_loc_conf_t *)parent;
	ngx_http_fastcgi_loc_conf_t * conf = (ngx_http_fastcgi_loc_conf_t *)child;
	size_t size;
	ngx_int_t rc;
	...
	if(conf->upstream.max_temp_file_size != 0 && conf->upstream.max_temp_file_size < size) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
		    "\"fastcgi_max_temp_file_size\" must be equal to zero to disable temporary files usage or must be equal to or greater than "
		    "the maximum of the value of \"fastcgi_buffer_size\" and one of the \"fastcgi_buffers\"");
		return NGX_CONF_ERROR;
	}
    ...
    ngx_http_fastcgi_params_t * params, ngx_keyval_t * default_params)
{
	...
	if(conf->params_source == NULL && default_params == NULL) {
		params->hash.buckets = (ngx_hash_elt_t **)1;
		return NGX_OK;
	}
	...
	if(default_params) {
		...
		while(h->key.len) {
			...
			for(i = 0; i < nsrc; i++) {
				if(sstreqi_ascii(h->key.data, src[i].key.data)) {
					goto next;
				}
			}
			s = (ngx_http_upstream_param_t *)ngx_array_push(&params_merged);
...
next:
	...
	for(i = 0; i < nsrc; i++) {
		...
		copy = (ngx_http_script_copy_code_t *)ngx_array_push_n(params->lengths, sizeof(ngx_http_script_copy_code_t));
		if(copy == NULL) {
			return NGX_ERROR;
		}
		...
		copy = (ngx_http_script_copy_code_t *)ngx_array_push_n(params->values, size);
		if(copy == NULL) {
			return NGX_ERROR;
		}
	...
	*code = (uintptr_t)NULL;
	...
	hash.pool = cf->pool;
	hash.temp_pool = NULL;
	return ngx_hash_init(&hash, (ngx_hash_key_t *)headers_names.elts, headers_names.nelts);
}
...
static ngx_int_t ngx_http_fastcgi_script_name_variable(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar * p;
	ngx_http_fastcgi_loc_conf_t  * flcf = (ngx_http_fastcgi_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_fastcgi_module);
	ngx_http_fastcgi_ctx_t * f = ngx_http_fastcgi_split(r, flcf);
	if(!f) {
		return NGX_ERROR;
	}
	...
	memcpy(p, flcf->index.data, flcf->index.len);
	return NGX_OK;
}
...
static ngx_http_fastcgi_ctx_t * ngx_http_fastcgi_split(ngx_http_request_t * r, ngx_http_fastcgi_loc_conf_t * flcf)
{
	...
	ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, ngx_regex_exec_n " failed: %i on \"%V\" using \"%V\"", n, &r->uri, &flcf->split_name);
	return NULL;
#else
	ngx_http_fastcgi_ctx_t * f = ngx_http_get_module_ctx(r, ngx_http_fastcgi_module);
	if(!f) {
...
static const char * ngx_http_fastcgi_split_path_info(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	return NGX_CONF_OK;
#else
	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"%V\" requires PCRE library", &cmd->name);
	return NGX_CONF_ERROR;
#endif
}
...
static const char * ngx_http_fastcgi_cache(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	if(cv.lengths) {
		flcf->upstream.cache_value = (ngx_http_complex_value_t *)ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
...
static char * ngx_http_fastcgi_lowat_check(ngx_conf_t * cf, void * post, void * data)
{
	...
	if((ulong) *np >= ngx_freebsd_net_inet_tcp_sendspace) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"fastcgi_send_lowat\" must be less than %d (sysctl net.inet.tcp.sendspace)", ngx_freebsd_net_inet_tcp_sendspace);
		return NGX_CONF_ERROR;
	}
