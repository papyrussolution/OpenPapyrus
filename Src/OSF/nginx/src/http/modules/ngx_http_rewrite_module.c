...
static char * ngx_http_rewrite_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_http_rewrite_loc_conf_t * prev = (ngx_http_rewrite_loc_conf_t *)parent;
	ngx_http_rewrite_loc_conf_t * conf = (ngx_http_rewrite_loc_conf_t *)child;
	uintptr_t  * code;
...
static ngx_int_t ngx_http_rewrite_init(ngx_conf_t * cf)
{
	...
	ngx_http_handler_pt * h = (ngx_http_handler_pt *)ngx_array_push(&cmcf->phases[NGX_HTTP_SERVER_REWRITE_PHASE].handlers);
	if(!h) {
		return NGX_ERROR;
	}
...
static const char * ngx_http_rewrite(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	ngx_http_script_regex_code_t * regex = (ngx_http_script_regex_code_t *)ngx_http_script_start_code(cf->pool, &lcf->codes, sizeof(ngx_http_script_regex_code_t));
	if(regex == NULL) {
		return NGX_CONF_ERROR;
	}
	...
	regex_end = (ngx_http_script_regex_end_code_t *)ngx_http_script_add_code(lcf->codes,
	    sizeof(ngx_http_script_regex_end_code_t), &regex);
	if(regex_end == NULL) {
		return NGX_CONF_ERROR;
	}
	...
	regex->next = (uchar *)lcf->codes->elts + lcf->codes->nelts - (uchar *)regex;
	return NGX_CONF_OK;
}
...
static const char * ngx_http_rewrite_return(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	ret = (ngx_http_script_return_code_t *)ngx_http_script_start_code(cf->pool, &lcf->codes, sizeof(ngx_http_script_return_code_t));
	if(ret == NULL) {
		return NGX_CONF_ERROR;
	}
	value = static_cast<ngx_str_t *>(cf->args->elts);
	...
	if(ret->status == (uintptr_t)NGX_ERROR) {
		if(cf->args->nelts == 2 && (ngx_strncmp(p, "http://", sizeof("http://") - 1) == 0 || 
			ngx_strncmp(p, "https://", sizeof("https://") - 1) == 0 || ngx_strncmp(p, "$scheme", sizeof("$scheme") - 1) == 0)) {
...
static const char * ngx_http_rewrite_if(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	if_code = (ngx_http_script_if_code_t *)ngx_array_push_n(lcf->codes, sizeof(ngx_http_script_if_code_t));
	if(if_code == NULL) {
		return NGX_CONF_ERROR;
	}
	if_code->code = ngx_http_script_if_code;
	elts = (uchar *)lcf->codes->elts;
	...
	*cf = save;
	...
	if(elts != lcf->codes->elts) {
		if_code = (ngx_http_script_if_code_t*)((uchar *)if_code + ((uchar *)lcf->codes->elts - elts));
	}
	if_code->next = (uchar *)lcf->codes->elts + lcf->codes->nelts - (uchar *)if_code;
...
static char * ngx_http_rewrite_variable(ngx_conf_t * cf, ngx_http_rewrite_loc_conf_t * lcf, ngx_str_t * value)
{
	...
	var_code = (ngx_http_script_var_code_t *)ngx_http_script_start_code(cf->pool, &lcf->codes, sizeof(ngx_http_script_var_code_t));
	if(var_code == NULL) {
		return NGX_CONF_ERROR;
	}
...
static char * ngx_http_rewrite_value(ngx_conf_t * cf, ngx_http_rewrite_loc_conf_t * lcf, ngx_str_t * value)
{
	...
	complex = (ngx_http_script_complex_value_code_t *)ngx_http_script_start_code(cf->pool, &lcf->codes, sizeof(ngx_http_script_complex_value_code_t));
	if(complex == NULL) {
		return NGX_CONF_ERROR;
	}
