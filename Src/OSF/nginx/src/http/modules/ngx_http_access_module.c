...
struct ngx_http_access_rule6_t {
...
struct ngx_http_access_loc_conf_t {
...
#endif
static ngx_int_t FASTCALL ngx_http_access_found(ngx_http_request_t * r, ngx_uint_t deny);
static const char * ngx_http_access_rule(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf); // F_SetHandler
static void * ngx_http_access_create_loc_conf(ngx_conf_t * cf);
static char * ngx_http_access_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child); // @callback
...
#endif
	ngx_http_access_loc_conf_t  * alcf = (ngx_http_access_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_access_module);
...
static ngx_int_t ngx_http_access_inet(ngx_http_request_t * r, ngx_http_access_loc_conf_t * alcf, in_addr_t addr)
{
	ngx_http_access_rule_t  * rule = (ngx_http_access_rule_t *)alcf->rules->elts;
	for(ngx_uint_t i = 0; i < alcf->rules->nelts; i++) {
...
static ngx_int_t ngx_http_access_inet6(ngx_http_request_t * r, ngx_http_access_loc_conf_t * alcf, u_char * p)
{
	ngx_uint_t n;
	ngx_uint_t i;
	ngx_http_access_rule6_t  * rule6 = (ngx_http_access_rule6_t *)alcf->rules6->elts;
	for(i = 0; i < alcf->rules6->nelts; i++) {
#if (NGX_DEBUG)
		{
			...
			ngx_log_debug6(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "access: %*s %*s %*s", cl, ct, ml, mt, al, at);
		}
#endif
		for(n = 0; n < 16; n++) {
...
static const char * ngx_http_access_rule(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	if(cidr.family == AF_INET || all) {
		...
		rule = (ngx_http_access_rule_t *)ngx_array_push(alcf->rules);
		if(rule == NULL) {
			return NGX_CONF_ERROR;
		}
	...
	if(cidr.family == AF_INET6 || all) {
		...
		rule6 = (ngx_http_access_rule6_t *)ngx_array_push(alcf->rules6);
		if(rule6 == NULL) {
			return NGX_CONF_ERROR;
		}
...
static char * ngx_http_access_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child) // @callback
{
	const ngx_http_access_loc_conf_t * prev = (const ngx_http_access_loc_conf_t *)parent;
	ngx_http_access_loc_conf_t * conf = (ngx_http_access_loc_conf_t *)child;
	if(conf->rules == NULL
#if (NGX_HAVE_INET6)
	    && conf->rules6 == NULL
#endif
