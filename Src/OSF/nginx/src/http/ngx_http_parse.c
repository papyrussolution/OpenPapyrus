	...
	for(p = b->pos; p < b->last; p++) {
		...
		switch(state) {
				    ...
				    switch(p - m) {
					    ...
					    case 4:
							if(m[1] == 'O') {
								...
								else if(ngx_str3Ocmp(m, 'C', 'O', 'P', 'Y')) {
									r->method = NGX_HTTP_COPY;
									break;
								}
								else if(ngx_str3Ocmp(m, 'M', 'O', 'V', 'E')) {
									r->method = NGX_HTTP_MOVE;
									break;
								}
								else if(ngx_str3Ocmp(m, 'L', 'O', 'C', 'K')) {
			...
			case sw_schema_slash:
			    switch(ch) {
				    case '/': state = sw_schema_slash_slash; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
			case sw_schema_slash_slash:
			    switch(ch) {
				    case '/': state = sw_host_start; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
			case sw_host_start:
			...
			case sw_http_H:
			    switch(ch) {
				    case 'T': state = sw_http_HT; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
			case sw_http_HT:
			    switch(ch) {
				    case 'T': state = sw_http_HTT; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
			case sw_http_HTT:
			    switch(ch) {
				    case 'P': state = sw_http_HTTP; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
			case sw_http_HTTP:
			    switch(ch) {
				    case '/': state = sw_first_major_digit; break;
				    default: return NGX_HTTP_PARSE_INVALID_REQUEST;
			    }
			    break;
...
ngx_int_t ngx_http_parse_status_line(ngx_http_request_t * r, ngx_buf_t * b, ngx_http_status_t * status)
{
	...
	for(p = b->pos; p < b->last; p++) {
		...
		switch(state) {
			...
			case sw_start:
			    switch(ch) {
				    case 'H': state = sw_H; break;
				    default: return NGX_ERROR;
			    }
			    break;
			case sw_H:
			    switch(ch) {
				    case 'T': state = sw_HT; break;
				    default: return NGX_ERROR;
			    }
			    break;
			case sw_HT:
			    switch(ch) {
				    case 'T': state = sw_HTT; break;
				    default: return NGX_ERROR;
			    }
			    break;
			case sw_HTT:
			    switch(ch) {
				    case 'P': state = sw_HTTP; break;
				    default: return NGX_ERROR;
			    }
			    break;
			case sw_HTTP:
...
ngx_int_t ngx_http_parse_set_cookie_lines(ngx_array_t * headers, ngx_str_t * name, ngx_str_t * value)
{
	...
	for(i = 0; i < headers->nelts; i++) {
		...
		if(start == end || *start++ != '=') {
			continue; // the invalid header value 
		}
		while(start < end && *start == ' ') {
			start++;
		}
	...
	for(pos = b->pos; pos < b->last; pos++) {
		...
		switch(state) {
			...
			case sw_chunk_size:
			    ...
			    if(ctx->size == 0) {
				    switch(ch) {
					    case __CR: state = sw_last_chunk_extension_almost_done; break;
					    case LF: state = sw_trailer; break;
					    case ';':
					    case ' ':
					    case '\t': state = sw_last_chunk_extension; break;
					    default: goto invalid;
				    }
				    break;
			    }
			    switch(ch) {
				    case __CR: state = sw_chunk_extension_almost_done; break;
				    case LF: state = sw_chunk_data; break;
				    case ';':
				    case ' ':
				    case '\t': state = sw_chunk_extension; break;
				    default: goto invalid;
			    }
			    break;
			...
			case sw_after_data:
			    switch(ch) {
				    case __CR: state = sw_after_data_almost_done; break;
				    case LF: state = sw_chunk_start;
			    }
			    break;
			...
			case sw_trailer:
			    switch(ch) {
				    case __CR: state = sw_trailer_almost_done; break;
				    case LF: goto done;
				    default: state = sw_trailer_header;
			    }
			    break;
			...
			case sw_trailer_header:
			    switch(ch) {
				    case __CR: state = sw_trailer_header_almost_done; break;
				    case LF: state = sw_trailer;
			    }
			    break;
...
data:
	...
	switch(state) {
		case sw_chunk_start:
		    ...
		    break;
		case sw_chunk_size:
		    ctx->length = 1 /* LF */ + (ctx->size ? ctx->size + 4   /* LF "0" LF LF */ : 1 /* LF */);
		    break;
		case sw_chunk_extension:
		case sw_chunk_extension_almost_done:
