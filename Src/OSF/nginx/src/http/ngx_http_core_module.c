...
static ngx_int_t ngx_http_core_find_location(ngx_http_request_t * r)
{
	...
	pclcf = (ngx_http_core_loc_conf_t *)ngx_http_get_module_loc_conf(r, ngx_http_core_module);
...
void * ngx_http_test_content_type(ngx_http_request_t * r, ngx_hash_t * types_hash)
{
	...
	if(r->headers_out.content_type_lowcase == NULL) {
		lowcase = (u_char *)ngx_pnalloc(r->pool, len);
		if(lowcase == NULL) {
			return NULL;
		}
...
ngx_int_t ngx_http_set_content_type(ngx_http_request_t * r)
{
	...
	if(r->exten.len) {
		...
		for(i = 0; i < r->exten.len; i++) {
			...
			if(c >= 'A' && c <= 'Z') {
				exten = (u_char *)ngx_pnalloc(r->pool, r->exten.len);
				if(exten == NULL) {
					return NGX_ERROR;
				}
		...
		type = (ngx_str_t *)ngx_hash_find(&clcf->types_hash, hash,
...
ngx_int_t ngx_http_set_etag(ngx_http_request_t * r)
{
	...
	etag = (ngx_table_elt_t *)ngx_list_push(&r->headers_out.headers);
	if(etag == NULL) {
		return NGX_ERROR;
	}
	...
	etag->value.data = (u_char *)ngx_pnalloc(r->pool, NGX_OFF_T_LEN + NGX_TIME_T_LEN + 3);
...
void ngx_http_weak_etag(ngx_http_request_t * r)
{
	...
	p = (u_char *)ngx_pnalloc(r->pool, etag->value.len + 2);
	if(p == NULL) {
		r->headers_out.etag->hash = 0;
		r->headers_out.etag = NULL;
		return;
...
ngx_int_t ngx_http_auth_basic_user(ngx_http_request_t * r)
{
	...
	auth.data = (u_char *)ngx_pnalloc(r->pool, auth.len + 1);
	if(auth.data == NULL) {
		return NGX_ERROR;
	}
...
static void * ngx_http_core_create_main_conf(ngx_conf_t * cf)
{
	...
	cmcf = (ngx_http_core_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_core_main_conf_t));
	if(cmcf == NULL) {
		return NULL;
	}
...
static void * ngx_http_core_create_loc_conf(ngx_conf_t * cf)
{
	...
	clcf = (ngx_http_core_loc_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_core_loc_conf_t));
	if(clcf == NULL) {
		return NULL;
	}
...
static char * ngx_http_core_merge_loc_conf(ngx_conf_t * cf, void * parent, void * child)
{
	...
	if(prev->types && prev->types_hash.buckets == NULL) {
		...
		if(ngx_hash_init(&types_hash, (ngx_hash_key_t *)prev->types->elts, prev->types->nelts)
		    != NGX_OK) {
			return NGX_CONF_ERROR;
		}
	}
	...
	if(conf->types_hash.buckets == NULL) {
		...
		if(ngx_hash_init(&types_hash, (ngx_hash_key_t *)conf->types->elts, conf->types->nelts)
		    != NGX_OK) {
			return NGX_CONF_ERROR;
		}
	}
