...
static ngx_http_file_cache_node_t * ngx_http_file_cache_lookup(ngx_http_file_cache_t * cache, uchar * key);
...
static void ngx_http_file_cache_vary(ngx_http_request_t * r, uchar * vary, size_t len, uchar * hash);
...
static void ngx_http_file_cache_delete(ngx_http_file_cache_t * cache, ngx_queue_t * q, uchar * name);
...
static uchar ngx_http_file_cache_key[] = { LF, 'K', 'E', 'Y', ':', ' ' };

static ngx_int_t ngx_http_file_cache_init(ngx_shm_zone_t * shm_zone, void * data)
{
	ngx_http_file_cache_t  * ocache = (ngx_http_file_cache_t *)data;
	size_t len;
	ngx_uint_t n;
	...
	cache->sh = (ngx_http_file_cache_sh_t *)ngx_slab_alloc(cache->shpool, sizeof(ngx_http_file_cache_sh_t));
	if(cache->sh == NULL) {
		return NGX_ERROR;
	}
	...
	cache->shpool->log_ctx = (uchar *)ngx_slab_alloc(cache->shpool, len);
...
void ngx_http_file_cache_create_key(ngx_http_request_t * r)
{
	...
	memcpy(c->main, c->key, NGX_HTTP_CACHE_KEY_LEN);
}
...
static void ngx_http_file_cache_lock_wait_handler(ngx_event_t * ev)
{
	ngx_http_request_t  * r = (ngx_http_request_t *)ev->P_Data;
...
static void ngx_http_file_cache_lock_wait(ngx_http_request_t * r, ngx_http_cache_t * c)
{
	...
	ngx_msec_t timer = c->wait_time - now;
...
static ngx_int_t ngx_http_file_cache_exists(ngx_http_file_cache_t * cache, ngx_http_cache_t * c)
{
	...
	fcn = (ngx_http_file_cache_node_t *)ngx_slab_calloc_locked(cache->shpool, sizeof(ngx_http_file_cache_node_t));
	if(fcn == NULL) {
	...
	cache->sh->count++;
	memcpy((uchar *)&fcn->node.key, c->key, sizeof(ngx_rbtree_key_t));
...
static ngx_int_t ngx_http_file_cache_name(ngx_http_request_t * r, ngx_path_t * path)
{
	...
	c->file.name.data = (uchar *)ngx_pnalloc(r->pool, c->file.name.len + 1);
	if(c->file.name.data == NULL) {
		return NGX_ERROR;
	}
	memcpy(c->file.name.data, path->name.data, path->name.len);
...
static ngx_http_file_cache_node_t * ngx_http_file_cache_lookup(ngx_http_file_cache_t * cache, uchar * key)
{
	...
	ngx_http_file_cache_node_t  * fcn;
	memcpy((uchar *)&node_key, key, sizeof(ngx_rbtree_key_t));
	...
	while(node != sentinel) {
		...
		rc = memcmp(&key[sizeof(ngx_rbtree_key_t)], fcn->key, NGX_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t));
		if(rc == 0) {
			return fcn;
		}
...
static void ngx_http_file_cache_rbtree_insert_value(ngx_rbtree_node_t * temp, ngx_rbtree_node_t * node, ngx_rbtree_node_t * sentinel)
{
	...
	for(;;) {
		if(node->key < temp->key) {
			...
			p = (memcmp(cn->key, cnt->key, NGX_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t)) < 0) ? &temp->left : &temp->right;
		}
		if(*p == sentinel) {
			break;
		}
...
static void ngx_http_file_cache_vary(ngx_http_request_t * r, uchar * vary, size_t len, uchar * hash)
{
	...
	ngx_str_t name;
	ngx_md5_t md5;
	uchar buf[NGX_HTTP_CACHE_VARY_LEN];
	...
	while(p < last) {
		...
		ngx_md5_update(&md5, (uchar *)":", sizeof(":") - 1);
		ngx_http_file_cache_vary_header(r, &md5, &name);
		ngx_md5_update(&md5, (uchar *)CRLF, sizeof(CRLF) - 1);
	}
...
static void ngx_http_file_cache_vary_header(ngx_http_request_t * r, ngx_md5_t * md5, ngx_str_t * name)
{
	...
	if(name->len == sizeof("Accept-Charset") - 1 && ngx_strncasecmp(name->data, (uchar *)"Accept-Charset", sizeof("Accept-Charset") - 1) == 0) {
		normalize = 1;
	}
	else if(name->len == sizeof("Accept-Encoding") - 1 && ngx_strncasecmp(name->data, (uchar *)"Accept-Encoding", sizeof("Accept-Encoding") - 1) == 0) {
		normalize = 1;
	}
	else if(name->len == sizeof("Accept-Language") - 1 && ngx_strncasecmp(name->data, (uchar *)"Accept-Language", sizeof("Accept-Language") - 1) == 0) {
		normalize = 1;
	}
...
static ngx_int_t ngx_http_file_cache_reopen(ngx_http_request_t * r, ngx_http_cache_t * c)
{
	...
	memcpy(c->key, c->variant, NGX_HTTP_CACHE_KEY_LEN);
...
ngx_int_t ngx_http_file_cache_set_header(ngx_http_request_t * r, uchar * buf)
{
	...
	h->date = c->date;
	h->crc32 = c->crc32;
	h->valid_msec = (ushort)c->valid_msec;
	h->header_start = (ushort)c->header_start;
	h->body_start = (ushort)c->body_start;
	if(c->etag.len <= NGX_HTTP_CACHE_ETAG_LEN) {
		h->etag_len = (uchar)c->etag.len;
		memcpy(h->etag, c->etag.data, c->etag.len);
	}
	if(c->vary.len) {
		...
		h->vary_len = (uchar)c->vary.len;
		memcpy(h->vary, c->vary.data, c->vary.len);
...
static ngx_int_t ngx_http_file_cache_update_variant(ngx_http_request_t * r, ngx_http_cache_t * c)
{
	...
	if(c->vary.len && memcmp(c->variant, c->key, NGX_HTTP_CACHE_KEY_LEN) == 0) {
		return NGX_OK;
	}
	/*
	...
	ngx_shmtx_unlock(&cache->shpool->mutex);
	c->file.name.len = 0;
	memcpy(c->key, c->main, NGX_HTTP_CACHE_KEY_LEN);
	if(ngx_http_file_cache_exists(cache, c) == NGX_ERROR) {
		return NGX_ERROR;
	}
	if(ngx_http_file_cache_name(r, cache->path) != NGX_OK) {
...
void ngx_http_file_cache_update_header(ngx_http_request_t * r)
{
	...
	//
	// make sure cache file wasn't replaced; if it was, do nothing
	//
	...
	n = ngx_read_file(&file, (uchar *)&h, sizeof(ngx_http_file_cache_header_t), 0);
	if(n == NGX_ERROR) {
		goto done;
	}
	if((size_t)n != sizeof(ngx_http_file_cache_header_t)) {
		ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, ngx_read_file_n " read only %z of %z from \"%s\"",
	...
	(void)ngx_write_file(&file, (uchar *)&h, sizeof(ngx_http_file_cache_header_t), 0);
done:
...
static void ngx_http_file_cache_cleanup(void * data)
{
	ngx_http_cache_t * c = (ngx_http_cache_t  *)data;
	if(!c->updated) {
...
static time_t ngx_http_file_cache_forced_expire(ngx_http_file_cache_t * cache)
{
	...
	for(;;) {
		...
		p = ngx_hex_dump(key, (uchar *)&fcn->node.key, sizeof(ngx_rbtree_key_t));
...
static void ngx_http_file_cache_delete(ngx_http_file_cache_t * cache, ngx_queue_t * q, uchar * name)
{
	...
	if(fcn->exists) {
		...
		p = ngx_hex_dump(p, (uchar *)&fcn->node.key, sizeof(ngx_rbtree_key_t));
		len = NGX_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t);
...
{
	ngx_http_file_cache_t  * cache = (ngx_http_file_cache_t *)data;
	nginx_off_t size;
	time_t wait;
...
{
	ngx_msec_t elapsed;
	ngx_http_file_cache_t  * cache = (ngx_http_file_cache_t *)ctx->data;
...
static ngx_int_t ngx_http_file_cache_add_file(ngx_tree_ctx_t * ctx, ngx_str_t * name)
{
	...
	if(ctx->size < (nginx_off_t)sizeof(ngx_http_file_cache_header_t)) {
		ngx_log_error(NGX_LOG_CRIT, ctx->log, 0, "cache file \"%s\" is too small", name->data);
		return NGX_ERROR;
	}
	memzero(&c, sizeof(ngx_http_cache_t));
...
{
	if(cache_valid) {
		const ngx_http_cache_valid_t * valid = (ngx_http_cache_valid_t *)cache_valid->elts;
		for(ngx_uint_t i = 0; i < cache_valid->nelts; i++) {
			if(valid[i].status == 0) {
...
const char * ngx_http_file_cache_set_slot(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	ngx_http_file_cache_t * cache = (ngx_http_file_cache_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_file_cache_t));
	if(cache == NULL) {
		return NGX_CONF_ERROR;
	}
	cache->path = (ngx_path_t *)ngx_pcalloc(cf->pool, sizeof(ngx_path_t));
	if(cache->path == NULL) {
		return NGX_CONF_ERROR;
	}
	use_temp_path = 1;
...
invalid_levels:
		...
		else if(ngx_strncmp(value[i].data, "max_size=", 9) == 0) {
			s.len = value[i].len - 9;
			s.data = value[i].data + 9;
		...
		else if(ngx_strncmp(value[i].data, "loader_files=", 13) == 0) {
		...
		else if(ngx_strncmp(value[i].data, "loader_sleep=", 13) == 0) {
		...
		else if(ngx_strncmp(value[i].data, "loader_threshold=", 17) == 0) {
		...
		else if(ngx_strncmp(value[i].data, "manager_files=", 14) == 0) {
		...
		else if(ngx_strncmp(value[i].data, "manager_sleep=", 14) == 0) {
		...
		else if(ngx_strncmp(value[i].data, "manager_threshold=", 18) == 0) {
	...
	cache->shm_zone = ngx_shared_memory_add(cf, &name, size, cmd->P_Post);
	...
	ce = (ngx_http_file_cache_t **)ngx_array_push(caches);
	if(ce == NULL) {
		return NGX_CONF_ERROR;
	}
	*ce = cache;
