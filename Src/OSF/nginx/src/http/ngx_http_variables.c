...
static ngx_int_t ngx_http_variable_headers_internal(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data, uchar sep);
...
ngx_http_variable_t * ngx_http_add_variable(ngx_conf_t * cf, ngx_str_t * name, ngx_uint_t flags)
{
	...
	v = (ngx_http_variable_t *)ngx_palloc(cf->pool, sizeof(ngx_http_variable_t));
	if(!v) {
		return NULL;
	}
	v->name.len = name->len;
...
static ngx_http_variable_t * ngx_http_add_prefix_variable(ngx_conf_t * cf, ngx_str_t * name, ngx_uint_t flags)
{
	...
	ngx_http_variable_t * v = (ngx_http_variable_t *)cmcf->prefix_variables.elts;
	...
	v = (ngx_http_variable_t *)ngx_array_push(&cmcf->prefix_variables);
	if(!v) {
		return NULL;
	}
	v->name.len = name->len;
...
ngx_int_t FASTCALL ngx_http_get_variable_index(ngx_conf_t * cf, const ngx_str_t * name)
{
	...
	v = (ngx_http_variable_t *)ngx_array_push(&cmcf->variables);
	if(!v) {
		return NGX_ERROR;
	}
	v->name.len = name->len;
...
{
	ngx_http_variable_value_t  * v = &r->variables[index];
...
ngx_http_variable_value_t * ngx_http_get_variable(ngx_http_request_t * r, ngx_str_t * name, ngx_uint_t key)
{
	...
	ngx_http_core_main_conf_t  * cmcf = (ngx_http_core_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_core_module);
	ngx_http_variable_t * v = (ngx_http_variable_t *)ngx_hash_find(&cmcf->variables_hash, key, name->data, name->len);
	if(v) {
...
static ngx_int_t ngx_http_variable_request_get_size(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	v->data = (uchar *)ngx_pnalloc(r->pool, NGX_SIZE_T_LEN);
	if(v->data == NULL) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_variable_argument(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	ngx_str_t * name = (ngx_str_t *)data;
	ngx_str_t value;
	size_t len = name->len - (sizeof("arg_") - 1);
	const uchar * arg = name->data + sizeof("arg_") - 1;
...
static ngx_int_t ngx_http_variable_tcpinfo(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	switch(data) {
		...
		default: value = 0; break; // suppress warning 
	}
...
static ngx_int_t ngx_http_variable_content_length(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	if(r->headers_in.content_length) {
		...
		p = (uchar *)ngx_pnalloc(r->pool, NGX_OFF_T_LEN);
		if(!p) {
			return NGX_ERROR;
		}
...
static ngx_int_t ngx_http_variable_host(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	if(r->headers_in.server.len) {
		...
		const ngx_http_core_srv_conf_t * cscf = (const ngx_http_core_srv_conf_t *)ngx_http_get_module_srv_conf(r, ngx_http_core_module);
...
static ngx_int_t ngx_http_variable_server_addr(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	s.data = (uchar *)ngx_pnalloc(r->pool, s.len);
	if(s.data == NULL) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_variable_realpath_root(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	len = ngx_strlen(real);
	v->data = static_cast<uchar *>(ngx_pnalloc(r->pool, len));
	if(v->data == NULL) {
#if !(NGX_HAVE_MAX_PATH)
		SAlloc::F(real);
#endif
...
static ngx_int_t ngx_http_variable_bytes_sent(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar  * p = (uchar *)ngx_pnalloc(r->pool, NGX_OFF_T_LEN);
	if(!p) {
		return NGX_ERROR;
	}
...
{
	uchar  * p;
	nginx_off_t sent = r->connection->sent - r->header_size;
	if(sent < 0) {
		sent = 0;
	}
	p = (uchar *)ngx_pnalloc(r->pool, NGX_OFF_T_LEN);
...
{
	ngx_uint_t status;
	v->data = (uchar *)ngx_pnalloc(r->pool, NGX_INT_T_LEN);
	if(v->data == NULL) {
		return NGX_ERROR;
	}
	if(r->err_status) {
...
static ngx_int_t ngx_http_variable_sent_last_modified(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	...
	if(r->headers_out.last_modified_time >= 0) {
		p = (uchar *)ngx_pnalloc(r->pool, sizeof("Mon, 28 Sep 1970 06:00:00 GMT") - 1);
		if(!p) {
			return NGX_ERROR;
		}
...
{
	if(r->keepalive) {
		ngx_http_core_loc_conf_t  * clcf = static_cast<ngx_http_core_loc_conf_t *>(ngx_http_get_module_loc_conf(r, ngx_http_core_module));
		if(clcf->keepalive_header) {
			uchar * p = static_cast<uchar *>(ngx_pnalloc(r->pool, sizeof("timeout=") - 1 + NGX_TIME_T_LEN));
			if(!p) {
				return NGX_ERROR;
			}
...
static ngx_int_t ngx_http_variable_request_time(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	ngx_time_t * tp;
	ngx_msec_int_t ms;
	uchar * p = (uchar *)ngx_pnalloc(r->pool, NGX_TIME_T_LEN + 4);
	if(!p) {
		return NGX_ERROR;
	}
	tp = ngx_timeofday();
...
#endif
	uchar * id = (uchar *)ngx_pnalloc(r->pool, 32);
	if(id == NULL) {
		return NGX_ERROR;
	}
	v->valid = 1;
...
#endif
	ngx_sprintf(id, "%08xD%08xD%08xD%08xD", (uint32_t)ngx_random(), (uint32_t)ngx_random(), (uint32_t)ngx_random(), (uint32_t)ngx_random());
	return NGX_OK;
}
...
static ngx_int_t ngx_http_variable_connection_requests(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar * p = (uchar *)ngx_pnalloc(r->pool, NGX_INT_T_LEN);
	if(!p) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_variable_pid(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar * p = (uchar *)ngx_pnalloc(r->pool, NGX_INT64_LEN);
	if(!p) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_variable_time_iso8601(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar * p = (uchar *)ngx_pnalloc(r->pool, ngx_cached_http_log_iso8601.len);
	if(!p) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_variable_time_local(ngx_http_request_t * r, ngx_http_variable_value_t * v, uintptr_t data)
{
	uchar  * p = (uchar *)ngx_pnalloc(r->pool, ngx_cached_http_log_time.len);
	if(!p) {
		return NGX_ERROR;
	}
...
void * ngx_http_map_find(ngx_http_request_t * r, ngx_http_map_t * map, ngx_str_t * match)
{
	...
	if(len) {
		low = static_cast<uchar *>(ngx_pnalloc(r->pool, len));
		if(low == NULL) {
			return NULL;
		}
	}
...
#if (NGX_PCRE)
	if(len && map->nregex) {
		ngx_http_map_regex_t  * reg = map->regex;
		for(ngx_uint_t i = 0; i < map->nregex; i++) {
			const ngx_int_t n = ngx_http_regex_exec(r, reg[i].regex, match);
			if(n == NGX_OK) {
				return reg[i].value;
			}
			if(n == NGX_DECLINED) {
...
ngx_http_regex_t * ngx_http_regex_compile(ngx_conf_t * cf, ngx_regex_compile_t * rc)
{
	...
	re = (ngx_http_regex_t *)ngx_pcalloc(cf->pool, sizeof(ngx_http_regex_t));
	if(re == NULL) {
		return NULL;
	}
	re->regex = rc->regex;
	...
	rv = (ngx_http_regex_variable_t *)ngx_palloc(rc->pool, n * sizeof(ngx_http_regex_variable_t));
	if(rv == NULL) {
		return NULL;
	}
	re->variables = rv;
...
ngx_int_t ngx_http_regex_exec(ngx_http_request_t * r, ngx_http_regex_t * re, ngx_str_t * s)
{
	...
	if(re->ncaptures) {
		len = cmcf->ncaptures;
		if(r->captures == NULL) {
			r->captures = (int *)ngx_palloc(r->pool, len * sizeof(int));
			if(r->captures == NULL) {
				return NGX_ERROR;
			}
		}
...
ngx_int_t ngx_http_variables_init_vars(ngx_conf_t * cf)
{
	...
	ngx_http_core_main_conf_t  * cmcf = (ngx_http_core_main_conf_t *)ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);
	ngx_http_variable_t * v = (ngx_http_variable_t *)cmcf->variables.elts;
	ngx_http_variable_t * pv = (ngx_http_variable_t *)cmcf->prefix_variables.elts;
	ngx_hash_key_t * key = (ngx_hash_key_t *)cmcf->variables_keys->keys.elts;
	for(i = 0; i < cmcf->variables.nelts; i++) {
