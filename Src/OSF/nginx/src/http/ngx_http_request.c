...
void ngx_http_init_connection(ngx_connection_t * c)
{
	...
	hc = (ngx_http_connection_t *)ngx_pcalloc(c->pool, sizeof(ngx_http_connection_t));
	if(hc == NULL) {
...
ngx_http_request_t * ngx_http_create_request(ngx_connection_t * c)
{
	...
	if(ngx_list_init(&r->headers_out.trailers, r->pool, 4, sizeof(ngx_table_elt_t))
	    != NGX_OK) {
		ngx_destroy_pool(r->pool);
		return NULL;
	}
...
void ngx_http_free_request(ngx_http_request_t * r, ngx_int_t rc)
{
	...
	if(r->connection->timedout) {
		...
		if(clcf->reset_timedout_connection) {
			linger.l_onoff = 1;
			linger.l_linger = 0;
			if(setsockopt(r->connection->fd, SOL_SOCKET, SO_LINGER, (const char *)&linger, sizeof(struct linger)) == -1) {
...
static void ngx_http_log_request(ngx_http_request_t * r)
{
	...
	ngx_http_core_main_conf_t  * cmcf;
	cmcf = (ngx_http_core_main_conf_t *)ngx_http_get_module_main_conf(r, ngx_http_core_module);
	log_handler = (ngx_http_handler_pt *)cmcf->phases[NGX_HTTP_LOG_PHASE].handlers.elts;
