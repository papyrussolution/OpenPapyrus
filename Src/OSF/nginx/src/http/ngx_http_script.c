...
#define ngx_http_script_exit  (uchar *)&ngx_http_script_exit_code
...
ngx_int_t ngx_http_complex_value(ngx_http_request_t * r, ngx_http_complex_value_t * val, ngx_str_t * value)
{
	...
	if(val->lengths == NULL) {
		*value = val->value;
		...
		value->len = len;
		value->data = static_cast<uchar *>(ngx_pnalloc(r->pool, len));
		if(value->data == NULL) {
			return NGX_ERROR;
		}
		e.ip = (uchar *)val->values;
		e.pos = value->data;
		e.buf = *value;
		while(*(uintptr_t*)e.ip) {
...
ngx_int_t ngx_http_compile_complex_value(ngx_http_compile_complex_value_t * ccv)
{
	...
	n = (nv * (2 * sizeof(ngx_http_script_copy_code_t) + sizeof(ngx_http_script_var_code_t)) + sizeof(uintptr_t) + v->len + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
...
const char * ngx_http_set_complex_value_slot(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	*cv = (ngx_http_complex_value_t *)ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
	if(*cv == NULL) {
...
{
	if(predicates) {
		ngx_http_complex_value_t  * cv = (ngx_http_complex_value_t *)predicates->elts;
		for(ngx_uint_t i = 0; i < predicates->nelts; i++) {
			ngx_str_t val;
			if(ngx_http_complex_value(r, &cv[i], &val) != NGX_OK) {
				return NGX_ERROR;
			}
			else if(val.len && (val.len != 1 || val.data[0] != '0')) {
				return NGX_DECLINED;
			}
		}
	}
...
const char * ngx_http_set_predicate_slot(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	for(i = 1; i < cf->args->nelts; i++) {
		cv = (ngx_http_complex_value_t *)ngx_array_push(*a);
		if(cv == NULL) {
			return NGX_CONF_ERROR;
		}
		memzero(&ccv, sizeof(ngx_http_compile_complex_value_t));
...
#if (NGX_PCRE)
				ngx_uint_t n = sc->source->data[i] - '0';
...
uchar * ngx_http_script_run(ngx_http_request_t * r, ngx_str_t * value,
...
static ngx_int_t ngx_http_script_add_copy_code(ngx_http_script_compile_t * sc, ngx_str_t * value, ngx_uint_t last)
{
	...
	code = (ngx_http_script_copy_code_t *)ngx_http_script_add_code(*sc->lengths,
	...
	code = (ngx_http_script_copy_code_t *)ngx_http_script_add_code(*sc->values, size, &sc->main);
	if(!code) {
		return NGX_ERROR;
	}
	...
	p = ngx_cpymem((uchar *)code + sizeof(ngx_http_script_copy_code_t),
...
static ngx_int_t ngx_http_script_add_var_code(ngx_http_script_compile_t * sc, ngx_str_t * name)
{
	...
	code = (ngx_http_script_var_code_t *)ngx_http_script_add_code(*sc->lengths, sizeof(ngx_http_script_var_code_t), NULL);
	if(!code) {
		return NGX_ERROR;
	}
	...
	code = (ngx_http_script_var_code_t *)ngx_http_script_add_code(*sc->values, sizeof(ngx_http_script_var_code_t), &sc->main);
	if(!code) {
		return NGX_ERROR;
	}
...
void ngx_http_script_regex_start_code(ngx_http_script_engine_t * e)
{
	...
	e->buf.data = (uchar *)ngx_pnalloc(r->pool, e->buf.len);
	if(e->buf.data == NULL) {
		e->ip = ngx_http_script_exit;
		e->status = NGX_HTTP_INTERNAL_SERVER_ERROR;
		return;
...
{
	uchar   * dst, * src;
	ngx_http_script_regex_end_code_t  * code = (ngx_http_script_regex_end_code_t*)e->ip;
	ngx_http_request_t  * r = e->request;
	e->quote = 0;
...
static ngx_int_t ngx_http_script_add_capture_code(ngx_http_script_compile_t * sc, ngx_uint_t n)
{
	ngx_http_script_copy_capture_code_t  * code = (ngx_http_script_copy_capture_code_t *)ngx_http_script_add_code(*sc->lengths, sizeof(ngx_http_script_copy_capture_code_t), NULL);
	if(!code) {
		return NGX_ERROR;
	}
	code->code = (ngx_http_script_code_pt)ngx_http_script_copy_capture_len_code;
	code->n = 2 * n;
	code = (ngx_http_script_copy_capture_code_t *)ngx_http_script_add_code(*sc->values, sizeof(ngx_http_script_copy_capture_code_t), &sc->main);
	if(!code) {
		return NGX_ERROR;
	}
...
void ngx_http_script_copy_capture_code(ngx_http_script_engine_t * e)
{
	...
	if(n < r->ncaptures) {
		...
		p = r->captures_data;
		if((e->is_args || e->quote) && (e->request->quoted_uri || e->request->plus_in_uri)) {
			e->pos = (uchar *)ngx_escape_uri(pos, &p[cap[n]], cap[n + 1] - cap[n], NGX_ESCAPE_ARGS);
		}
		else {
...
static ngx_int_t ngx_http_script_add_full_name_code(ngx_http_script_compile_t * sc)
{
	ngx_http_script_full_name_code_t  * code = (ngx_http_script_full_name_code_t *)ngx_http_script_add_code(*sc->lengths,
	    sizeof(ngx_http_script_full_name_code_t), NULL);
	if(!code) {
		return NGX_ERROR;
	}
	code->code = (ngx_http_script_code_pt)ngx_http_script_full_name_len_code;
	code->conf_prefix = sc->conf_prefix;
	code = (ngx_http_script_full_name_code_t *)ngx_http_script_add_code(*sc->values, sizeof(ngx_http_script_full_name_code_t), &sc->main);
	if(!code) {
		return NGX_ERROR;
	}
...
{
	ngx_http_script_if_code_t  * code = (ngx_http_script_if_code_t*)e->ip;
...
void ngx_http_script_complex_value_code(ngx_http_script_engine_t * e)
{
	...
	ngx_http_script_engine_t le;
	ngx_http_script_len_code_pt lcode;
	ngx_http_script_complex_value_code_t  * code = (ngx_http_script_complex_value_code_t*)e->ip;
	...
	e->buf.len = len;
	e->buf.data = (uchar *)ngx_pnalloc(e->request->pool, len);
	if(e->buf.data == NULL) {
		e->ip = ngx_http_script_exit;
		e->status = NGX_HTTP_INTERNAL_SERVER_ERROR;
		return;
...
{
	ngx_http_script_value_code_t  * code = (ngx_http_script_value_code_t*)e->ip;
	e->ip += sizeof(ngx_http_script_value_code_t);
	e->sp->len = code->text_len;
	e->sp->data = (uchar *)code->text_data;
...
void ngx_http_script_set_var_code(ngx_http_script_engine_t * e)
{
...
#if (NGX_DEBUG)
	{
		...
		ngx_http_variable_t * v = (ngx_http_variable_t *)cmcf->variables.elts;
