...
static ngx_int_t ngx_http_header_filter(ngx_http_request_t * r)
{
	...
	len = sizeof("HTTP/1.x ") - 1 + sizeof(CRLF) - 1 /* the end of the header */ + sizeof(CRLF) - 1;
	/* status line */
	...
	c = r->connection;
	if(r->headers_out.location && r->headers_out.location->value.len && r->headers_out.location->value.data[0] == '/' && clcf->absolute_redirect) {
...
#if (NGX_HTTP_GZIP)
	if(r->gzip_vary) {
		b->last = ngx_cpymem(b->last, "Vary: Accept-Encoding" CRLF, sizeof("Vary: Accept-Encoding" CRLF) - 1);
	}
#endif
	part = &r->headers_out.headers.part;
	header = (ngx_table_elt_t*)part->elts;
