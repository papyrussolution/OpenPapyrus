...
{
	int    result = NGX_OK;
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_POST_READ_PHASE].handlers, cf->pool, 1, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_SERVER_REWRITE_PHASE].handlers, cf->pool, 1, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_REWRITE_PHASE].handlers, cf->pool, 1, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_PREACCESS_PHASE].handlers, cf->pool, 1, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_ACCESS_PHASE].handlers, cf->pool, 2, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_PRECONTENT_PHASE].handlers, cf->pool, 2, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_CONTENT_PHASE].handlers, cf->pool, 4, sizeof(ngx_http_handler_pt)) == NGX_OK);
	THROW(ngx_array_init(&cmcf->phases[NGX_HTTP_LOG_PHASE].handlers, cf->pool, 1, sizeof(ngx_http_handler_pt)) == NGX_OK);
	CATCH
		result = NGX_ERROR;
	ENDCATCH
	return result;
...
static ngx_int_t ngx_http_init_headers_in_hash(ngx_conf_t * cf, ngx_http_core_main_conf_t * cmcf)
{
	...
	hash.pool = cf->pool;
	hash.temp_pool = NULL;
	if(ngx_hash_init(&hash, (ngx_hash_key_t *)headers_in.elts, headers_in.nelts) != NGX_OK) {
		return NGX_ERROR;
	}
	return NGX_OK;
}
...
static ngx_int_t ngx_http_init_phase_handlers(ngx_conf_t * cf, ngx_http_core_main_conf_t * cmcf)
{
	...
	ph = (ngx_http_phase_handler_t *)ngx_pcalloc(cf->pool, n * sizeof(ngx_http_phase_handler_t) + sizeof(void *));
	if(ph == NULL) {
		return NGX_ERROR;
	}
	else {
...
static ngx_int_t ngx_http_init_locations(ngx_conf_t * cf, ngx_http_core_srv_conf_t * cscf, ngx_http_core_loc_conf_t * pclcf)
{
	...
	ngx_queue_t * regex;
#endif
	ngx_queue_t * locations = pclcf->locations;
	if(locations == NULL) {
		return NGX_OK;
	}
	...
	if(named) {
		clcfp = (ngx_http_core_loc_conf_t**)ngx_palloc(cf->pool, (n + 1) * sizeof(ngx_http_core_loc_conf_t *));
		if(clcfp == NULL) {
			return NGX_ERROR;
		}
...
#if (NGX_PCRE)
	if(regex) {
		clcfp = (ngx_http_core_loc_conf_t**)ngx_palloc(cf->pool, (r + 1) * sizeof(ngx_http_core_loc_conf_t *));
		if(clcfp == NULL) {
			return NGX_ERROR;
		}
...
ngx_int_t ngx_http_add_location(ngx_conf_t * cf, ngx_queue_t ** locations, ngx_http_core_loc_conf_t * clcf)
{
	ngx_http_location_queue_t  * lq;
	if(*locations == NULL) {
		*locations = (ngx_queue_t *)ngx_palloc(cf->temp_pool, sizeof(ngx_http_location_queue_t));
		if(*locations == NULL) {
			return NGX_ERROR;
		}
		ngx_queue_init(*locations);
	}
	lq = (ngx_http_location_queue_t *)ngx_palloc(cf->temp_pool, sizeof(ngx_http_location_queue_t));
	if(lq == NULL) {
		return NGX_ERROR;
	}
	if(clcf->exact_match
...
{
	ngx_int_t rc;
	ngx_http_location_queue_t  * lq1 = (ngx_http_location_queue_t*)one;
    ...
    ngx_http_conf_port_t * port, ngx_http_listen_opt_t * lsopt)
{
	...
	for(i = 0; i < port->addrs.nelts; i++) {
		if(ngx_cmp_sockaddr(&lsopt->sockaddr.sockaddr, lsopt->socklen, &addr[i].opt.sockaddr.sockaddr, addr[i].opt.socklen, 0) != NGX_OK) {
			continue;
		}
...
static ngx_int_t ngx_http_add_server(ngx_conf_t * cf, ngx_http_core_srv_conf_t * cscf, ngx_http_conf_addr_t * addr)
{
	...
	server = (ngx_http_core_srv_conf_t **)ngx_array_push(&addr->servers);
	if(server == NULL) {
		return NGX_ERROR;
	}
	*server = cscf;
...
static ngx_int_t ngx_http_server_names(ngx_conf_t * cf, ngx_http_core_main_conf_t * cmcf, ngx_http_conf_addr_t * addr)
{
	...
	ngx_uint_t regex = 0;
#endif
	memzero(&ha, sizeof(ngx_hash_keys_arrays_t));
	...
	if(ha.keys.nelts) {
		hash.hash = &addr->hash;
		hash.temp_pool = NULL;
		if(ngx_hash_init(&hash, (ngx_hash_key_t *)ha.keys.elts, ha.keys.nelts) != NGX_OK) {
			goto failed;
		}
	}
	if(ha.dns_wc_head.nelts) {
		...
		if(ngx_hash_wildcard_init(&hash, (ngx_hash_key_t *)ha.dns_wc_head.elts, ha.dns_wc_head.nelts) != NGX_OK) {
			goto failed;
		}
	...
	if(ha.dns_wc_tail.nelts) {
		...
		hash.hash = NULL;
		hash.temp_pool = ha.temp_pool;
		if(ngx_hash_wildcard_init(&hash, (ngx_hash_key_t *)ha.dns_wc_tail.elts, ha.dns_wc_tail.nelts) != NGX_OK) {
			goto failed;
		}
	...
	addr->nregex = regex;
	addr->regex = (ngx_http_server_name_t *)ngx_palloc(cf->pool, regex * sizeof(ngx_http_server_name_t));
	if(addr->regex == NULL) {
		return NGX_ERROR;
	}
	i = 0;
...
static ngx_int_t ngx_http_add_addrs(ngx_conf_t * cf, ngx_http_port_t * hport, ngx_http_conf_addr_t * addr)
{
	...
	for(i = 0; i < hport->naddrs; i++) {
		...
		vn = (ngx_http_virtual_names_t *)ngx_palloc(cf->pool, sizeof(ngx_http_virtual_names_t));
		if(vn == NULL) {
			return NGX_ERROR;
		}
