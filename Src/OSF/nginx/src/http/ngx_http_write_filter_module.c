...
{
	nginx_off_t size, sent, nsent, limit;
	...
	for(cl = r->out; cl; cl = cl->next) {
		...
		if(ngx_buf_size(cl->buf) == 0 && !ngx_buf_special(cl->buf)) {
			ngx_log_error(NGX_LOG_ALERT, c->log, 0, "zero size buf in writer t:%d r:%d f:%d %p %p-%p %p %O-%O",
			    cl->buf->temporary, cl->buf->recycled, cl->buf->in_file, cl->buf->start,
			    cl->buf->pos, cl->buf->last, cl->buf->file, cl->buf->file_pos, cl->buf->file_last);
			ngx_debug_point();
			return NGX_ERROR;
		}
#endif
	...
	*ll = NULL;
	...
	if(r->limit_rate) {
		...
		limit = (nginx_off_t)r->limit_rate * (ngx_time() - r->start_sec + 1)
		...
		if(clcf->sendfile_max_chunk
		    && (nginx_off_t)clcf->sendfile_max_chunk < limit) {
