...
static char * ngx_stream_block(ngx_conf_t * cf, ngx_command_t * cmd, void * conf)
{
	...
	*(ngx_stream_conf_ctx_t**)conf = ctx;
	...
	cmcf = (ngx_stream_core_main_conf_t *)ctx->main_conf[ngx_stream_core_module.ctx_index];
	cscfp = (ngx_stream_core_srv_conf_t **)cmcf->servers.elts;
	for(m = 0; cf->cycle->modules[m]; m++) {
		if(cf->cycle->modules[m]->type == NGX_STREAM_MODULE) {
			module = (ngx_stream_module_t*)cf->cycle->modules[m]->ctx;
			mi = cf->cycle->modules[m]->ctx_index;
	...
	*cf = pcf;
	...
	listen = (ngx_stream_listen_t *)cmcf->listen.elts;
...
static ngx_int_t ngx_stream_init_phase_handlers(ngx_conf_t * cf, ngx_stream_core_main_conf_t * cmcf)
{
	...
	ph = (ngx_stream_phase_handler_t *)ngx_pcalloc(cf->pool, n * sizeof(ngx_stream_phase_handler_t) + sizeof(void *));
	if(ph == NULL) {
		return NGX_ERROR;
	}
	...
	n = 0;
	for(i = 0; i < NGX_STREAM_LOG_PHASE; i++) {
		h = (ngx_stream_handler_pt*)cmcf->phases[i].handlers.elts;
		switch(i) {
...
static ngx_int_t ngx_stream_add_ports(ngx_conf_t * cf, ngx_array_t * ports, ngx_stream_listen_t * listen)
{
	...
	port = (ngx_stream_conf_port_t *)ngx_array_push(ports);
	if(port == NULL) {
		return NGX_ERROR;
	}
...
found:
	addr = (ngx_stream_conf_addr_t *)ngx_array_push(&port->addrs);
	if(addr == NULL) {
		return NGX_ERROR;
	}
	addr->opt = *listen;
...
static char * ngx_stream_optimize_servers(ngx_conf_t * cf, ngx_array_t * ports)
{
	...
	for(p = 0; p < ports->nelts; p++) {
		...
		addr = (ngx_stream_conf_addr_t *)port[p].addrs.elts;
		...
		while(i < last) {
			...
			cscf = (ngx_stream_core_srv_conf_t *)addr->opt.ctx->srv_conf[ngx_stream_core_module.ctx_index];
...
#endif
			stport = (ngx_stream_port_t *)ngx_palloc(cf->pool, sizeof(ngx_stream_port_t));
			if(stport == NULL) {
				return NGX_CONF_ERROR;
			}
			ls->servers = stport;
...
static ngx_int_t ngx_stream_add_addrs(ngx_conf_t * cf, ngx_stream_port_t * stport, ngx_stream_conf_addr_t * addr)
{
	...
	addrs = (ngx_stream_in_addr_t *)stport->addrs;
...
static ngx_int_t ngx_stream_add_addrs6(ngx_conf_t * cf, ngx_stream_port_t * stport, ngx_stream_conf_addr_t * addr)
{
	...
	addrs6 = (ngx_stream_in6_addr_t *)stport->addrs;
	for(i = 0; i < stport->naddrs; i++) {
...
static ngx_int_t ngx_stream_cmp_conf_addrs(const void * one, const void * two)
{
	const ngx_stream_conf_addr_t * first  = (const ngx_stream_conf_addr_t*)one;
	const ngx_stream_conf_addr_t * second = (const ngx_stream_conf_addr_t*)two;
	if(first->opt.wildcard) // a wildcard must be the last resort, shift it to the end 
		return 1;
	else if(second->opt.wildcard) // a wildcard must be the last resort, shift it to the end 
		return -1;
	else if(first->opt.bind && !second->opt.bind) // shift explicit bind()ed addresses to the start 
		return -1;
	else if(!first->opt.bind && second->opt.bind) // shift explicit bind()ed addresses to the start 
		return 1;
	else 
