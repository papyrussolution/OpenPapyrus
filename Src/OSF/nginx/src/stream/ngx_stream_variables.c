...
{
	ngx_stream_variable_value_t  * v = &s->variables[index];
...
ngx_stream_regex_t * ngx_stream_regex_compile(ngx_conf_t * cf, ngx_regex_compile_t * rc)
{
	...
	re = (ngx_stream_regex_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_regex_t));
	if(re == NULL) {
		return NULL;
	}
	re->regex = rc->regex;
	...
	rv = (ngx_stream_regex_variable_t *)ngx_palloc(rc->pool, n * sizeof(ngx_stream_regex_variable_t));
	if(rv == NULL) {
		return NULL;
	}
    ...
    ngx_str_t * str)
{
	...
	if(re->ncaptures) {
		len = cmcf->ncaptures;
		if(s->captures == NULL) {
			s->captures = (int *)ngx_palloc(s->connection->pool, len * sizeof(int));
			if(s->captures == NULL) {
				return NGX_ERROR;
			}
		}
...
#if (NGX_DEBUG)
		{
			ngx_stream_variable_t * v = (ngx_stream_variable_t *)cmcf->variables.elts;
			ngx_log_debug2(NGX_LOG_DEBUG_STREAM, s->connection->log, 0, "stream regex set $%V to \"%v\"", &v[index].name, vv);
		}
#endif
	}
	s->ncaptures = rc * 2;
...
ngx_int_t ngx_stream_variables_add_core_vars(ngx_conf_t * cf)
{
	...
	cmcf->variables_keys = (ngx_hash_keys_arrays_t *)ngx_pcalloc(cf->temp_pool, sizeof(ngx_hash_keys_arrays_t));
...
ngx_int_t ngx_stream_variables_init_vars(ngx_conf_t * cf)
{
	...
	v = (ngx_stream_variable_t *)cmcf->variables.elts;
	pv = (ngx_stream_variable_t *)cmcf->prefix_variables.elts;
	key = (ngx_hash_key_t *)cmcf->variables_keys->keys.elts;
...
next:
	...
	hash.pool = cf->pool;
	hash.temp_pool = NULL;
	if(ngx_hash_init(&hash, (ngx_hash_key_t *)cmcf->variables_keys->keys.elts, cmcf->variables_keys->keys.nelts) != NGX_OK) {
		return NGX_ERROR;
	}
