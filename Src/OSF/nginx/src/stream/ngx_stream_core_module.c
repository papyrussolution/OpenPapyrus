...
static char * ngx_stream_core_init_main_conf(ngx_conf_t * cf, void * conf)
{
	ngx_stream_core_main_conf_t * cmcf = (ngx_stream_core_main_conf_t *)conf;
...
static void * ngx_stream_core_create_srv_conf(ngx_conf_t * cf)
{
	ngx_stream_core_srv_conf_t * cscf = (ngx_stream_core_srv_conf_t*)ngx_pcalloc(cf->pool, sizeof(ngx_stream_core_srv_conf_t));
	if(cscf == NULL) {
		return NULL;
	}
	/*
...
static char * ngx_stream_core_merge_srv_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_stream_core_srv_conf_t * prev = (ngx_stream_core_srv_conf_t *)parent;
	ngx_stream_core_srv_conf_t * conf = (ngx_stream_core_srv_conf_t *)child;
...
static const char * ngx_stream_core_server(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	cscf = (ngx_stream_core_srv_conf_t*)ctx->srv_conf[ngx_stream_core_module.ctx_index];
	cscf->ctx = ctx;

	cmcf = (ngx_stream_core_main_conf_t*)ctx->main_conf[ngx_stream_core_module.ctx_index];

	cscfp = (ngx_stream_core_srv_conf_t**)ngx_array_push(&cmcf->servers);
	if(cscfp == NULL) {
		return NGX_CONF_ERROR;
	}
	*cscfp = cscf;
...
static const char * ngx_stream_core_listen(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	ls = (ngx_stream_listen_t *)ngx_array_push(&cmcf->listen);
	if(ls == NULL) {
		return NGX_CONF_ERROR;
	}
	...
	ls->ctx = (ngx_stream_conf_ctx_t *)cf->ctx;
#if (NGX_HAVE_INET6)
	ls->ipv6only = 1;
#endif
	backlog = 0;
...
invalid_so_keepalive:
	...
	als = (ngx_stream_listen_t *)cmcf->listen.elts;
