...
static const char * ngx_stream_realip_from(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	if(rc != NGX_ERROR) {
		...
		cidr = (ngx_cidr_t *)ngx_array_push(rscf->from);
		if(cidr == NULL) {
			return NGX_CONF_ERROR;
		}
		*cidr = c;
	...
	cidr = (ngx_cidr_t *)ngx_array_push_n(rscf->from, u.naddrs);
	if(cidr == NULL) {
		return NGX_CONF_ERROR;
	}
...
static char * ngx_stream_realip_merge_srv_conf(ngx_conf_t * cf, void * parent, void * child)
{
	ngx_stream_realip_srv_conf_t * prev = (ngx_stream_realip_srv_conf_t *)parent;
	ngx_stream_realip_srv_conf_t * conf = (ngx_stream_realip_srv_conf_t *)child;
	if(conf->from == NULL) {
		conf->from = prev->from;
	}
	return NGX_CONF_OK;
...
static ngx_int_t ngx_stream_realip_add_variables(ngx_conf_t * cf)
{
	for(ngx_stream_variable_t * v = ngx_stream_realip_vars; v->name.len; v++) {
...
static ngx_int_t ngx_stream_realip_init(ngx_conf_t * cf)
{
	...
	ngx_stream_handler_pt * h = (ngx_stream_handler_pt *)ngx_array_push(&cmcf->phases[NGX_STREAM_POST_ACCEPT_PHASE].handlers);
	if(h == NULL) {
		return NGX_ERROR;
	}
