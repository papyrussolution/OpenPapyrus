    ...
    ngx_stream_upstream_srv_conf_t * us)
{
	...
	peers = (ngx_stream_upstream_rr_peers_t*)us->peer.data;
	npoints = peers->total_weight * 160;
	size = sizeof(ngx_stream_upstream_chash_points_t) + sizeof(ngx_stream_upstream_chash_point_t) * (npoints - 1);
	points = (ngx_stream_upstream_chash_points_t *)ngx_palloc(cf->pool, size);
	if(points == NULL) {
		return NGX_ERROR;
	}
    ...
    ngx_stream_upstream_srv_conf_t * us)
{
	...
	hp = (ngx_stream_upstream_hash_peer_data_t *)s->upstream->peer.data;
	hcf = (ngx_stream_upstream_hash_srv_conf_t *)ngx_stream_conf_upstream_srv_conf(us, ngx_stream_upstream_hash_module);
