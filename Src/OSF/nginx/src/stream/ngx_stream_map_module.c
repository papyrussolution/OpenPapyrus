    ...
    uintptr_t data)
{
	...
	value = (ngx_stream_variable_value_t*)ngx_stream_map_find(s, &map->map, &val);
...
static void * ngx_stream_map_create_conf(ngx_conf_t * cf)
{
	...
	mcf = (ngx_stream_map_conf_t *)ngx_palloc(cf->pool, sizeof(ngx_stream_map_conf_t));
	if(mcf == NULL) {
		return NULL;
	}
...
static const char * ngx_stream_map(ngx_conf_t * cf, const ngx_command_t * dummy, void * conf) // F_SetHandler
{
	...
	var->no_cacheable = 0;
	var->not_found = 0;
	vp = (ngx_stream_variable_value_t**)ngx_array_push(&ctx->values_hash[key]);
	if(vp == NULL) {
		return NGX_CONF_ERROR;
	}
	*vp = var;
