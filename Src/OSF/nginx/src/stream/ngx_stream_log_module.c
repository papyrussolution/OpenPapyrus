...
static ssize_t ngx_stream_log_gzip(ngx_fd_t fd, u_char * buf, size_t len, ngx_int_t level, ngx_log_t * log)
{
	...
	ngx_log_debug4(NGX_LOG_DEBUG_STREAM, log, 0, "deflate in: ni:%p no:%p ai:%ud ao:%ud",
	...
	ngx_log_debug5(NGX_LOG_DEBUG_STREAM, log, 0, "deflate out: ni:%p no:%p ai:%ud ao:%ud rc:%d",
...
static void ngx_stream_log_flush(ngx_open_file_t * file, ngx_log_t * log)
{
	...
	ssize_t n;
	ngx_stream_log_buf_t  * buffer;
	buffer = (ngx_stream_log_buf_t *)file->data;
...
static void * ngx_stream_log_create_main_conf(ngx_conf_t * cf)
{
	...
	conf = (ngx_stream_log_main_conf_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_log_main_conf_t));
	if(conf == NULL) {
		return NULL;
	}
...
{
	ngx_stream_log_srv_conf_t * prev = (ngx_stream_log_srv_conf_t *)parent;
	ngx_stream_log_srv_conf_t * conf = (ngx_stream_log_srv_conf_t *)child;
...
static char * ngx_stream_log_set_log(ngx_conf_t * cf, ngx_command_t * cmd, void * conf)
{
	...
	lmcf = (ngx_stream_log_main_conf_t *)ngx_stream_conf_get_module_main_conf(cf, ngx_stream_log_module);
	log = (ngx_stream_log_t *)ngx_array_push(lscf->logs);
	if(log == NULL) {
		return NGX_CONF_ERROR;
	}
...
static ngx_int_t ngx_stream_log_init(ngx_conf_t * cf)
{
	...
	h = (ngx_stream_handler_pt*)ngx_array_push(&cmcf->phases[NGX_STREAM_LOG_PHASE].handlers);
	if(h == NULL) {
		return NGX_ERROR;
	}
