...
static char * ngx_stream_geo_block(ngx_conf_t * cf, ngx_command_t * cmd, void * conf)
{
	...
	geo = (ngx_stream_geo_ctx_t *)ngx_palloc(cf->pool, sizeof(ngx_stream_geo_ctx_t));
	if(geo == NULL) {
    ...
    ngx_str_t * value)
{
	...
	val = (ngx_stream_variable_value_t *)ngx_palloc(ctx->pool, sizeof(ngx_stream_variable_value_t));
	if(val == NULL) {
		return NULL;
	}
...
static void ngx_stream_geo_create_binary_base(ngx_stream_geo_conf_ctx_t * ctx)
{
	...
	header = (ngx_stream_geo_header_t *)fm.addr;
