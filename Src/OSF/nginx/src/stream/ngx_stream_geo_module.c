...
struct ngx_stream_geo_high_ranges_t {
	ngx_stream_geo_range_t  ** low;
	ngx_stream_variable_value_t  * default_value;
};

struct ngx_stream_geo_variable_value_node_t {
	ngx_str_node_t sn;
	ngx_stream_variable_value_t  * value;
	size_t offset;
};

struct ngx_stream_geo_conf_ctx_t {
	ngx_stream_variable_value_t  * value;
	ngx_str_t  * net;
...
static uchar * ngx_stream_geo_copy_values(uchar * base, uchar * p, ngx_rbtree_node_t * node, ngx_rbtree_node_t * sentinel);
...
static const char * ngx_stream_geo_block(ngx_conf_t * cf, const ngx_command_t * cmd, void * conf) // F_SetHandler
{
	...
	memzero(&ctx, sizeof(ngx_stream_geo_conf_ctx_t));
			...
			for(i = 0; i < 0x10000; i++) {
				...
				if(a) {
					if(a->nelts == 0) {
						...
						ctx.high.low[i] = (ngx_stream_geo_range_t*)ngx_palloc(cf->pool, len + sizeof(void *));
...
static char * ngx_stream_geo_range(ngx_conf_t * cf, ngx_stream_geo_conf_ctx_t * ctx, ngx_str_t * value)
{
	...
	if(ctx->high.low == NULL) {
		ctx->high.low = (ngx_stream_geo_range_t**)ngx_pcalloc(ctx->pool,
...
{
	ngx_int_t rc;
	ngx_stream_variable_value_t * old;
	ngx_stream_variable_value_t * val = ngx_stream_geo_value(cf, ctx, value);
	if(val == NULL) {
		return NGX_CONF_ERROR;
	}
	switch(cidr->family) {
...
static ngx_stream_variable_value_t * ngx_stream_geo_value(ngx_conf_t * cf, ngx_stream_geo_conf_ctx_t * ctx, ngx_str_t * value)
{
	...
	ngx_stream_geo_variable_value_node_t  * gvvn = (ngx_stream_geo_variable_value_node_t*)ngx_str_rbtree_lookup(&ctx->rbtree, value, hash);
	if(gvvn) {
		return gvvn->value;
	}
	else {
		ngx_stream_variable_value_t * val = (ngx_stream_variable_value_t *)ngx_palloc(ctx->pool, sizeof(ngx_stream_variable_value_t));
		if(val) {
			val->len = value->len;
			...
			val->no_cacheable = 0;
			val->not_found = 0;
			gvvn = (ngx_stream_geo_variable_value_node_t *)ngx_palloc(ctx->temp_pool, sizeof(ngx_stream_geo_variable_value_node_t));
			if(gvvn == NULL) {
				return NULL;
			}
...
static ngx_int_t ngx_stream_geo_include_binary_base(ngx_conf_t * cf, ngx_stream_geo_conf_ctx_t * ctx, ngx_str_t * name)
{
	...
	if(ctx->binary_include) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "second binary geo range base \"%s\" cannot be mixed with \"%s\"", name->data, ctx->include_name.data);
		rc = NGX_ERROR;
		goto done;
	}
	...
	while(vv->data) {
		...
		vv->data += (size_t)base;
		vv = (ngx_stream_variable_value_t*)((uchar *)vv + len);
	}
	ngx_crc32_update(&crc32, (uchar *)vv, sizeof(ngx_stream_variable_value_t));
	vv++;
	ranges = (ngx_stream_geo_range_t**)vv;
	for(i = 0; i < 0x10000; i++) {
		ngx_crc32_update(&crc32, (uchar *)&ranges[i], sizeof(void *));
		if(ranges[i]) {
			ranges[i] = (ngx_stream_geo_range_t*)((uchar *)ranges[i] + (size_t)base);
		}
	}
	range = (ngx_stream_geo_range_t*)&ranges[0x10000];
	while((uchar *)range < base + size) {
		while(range->value) {
			ngx_crc32_update(&crc32, (uchar *)range, sizeof(ngx_stream_geo_range_t));
			range->value = (ngx_stream_variable_value_t*)((uchar *)range->value + (size_t)base);
			range++;
		}
		ngx_crc32_update(&crc32, (uchar *)range, sizeof(void *));
		range = (ngx_stream_geo_range_t*)((uchar *)range + sizeof(void *));
	}
...
failed:
	...
	if(ngx_close_file(file.fd) == NGX_FILE_ERROR) {
		ngx_log_error(NGX_LOG_ALERT, cf->log, ngx_errno, ngx_close_file_n " \"%s\" failed", name->data);
	}
	return rc;
}
...
static void ngx_stream_geo_create_binary_base(ngx_stream_geo_conf_ctx_t * ctx)
{
	...
	if(fm.name) {
		...
		p = ngx_stream_geo_copy_values((uchar *)fm.addr, p, ctx->rbtree.root, ctx->rbtree.sentinel);
		...
		for(i = 0; i < 0x10000; i++) {
			...
			if(r) {
				...
				ranges[i] = (ngx_stream_geo_range_t*)(p - (uchar *)fm.addr);
				do {
					s.len = r->value->len;
					s.data = r->value->data;
		...
		}
		{
			...
			header->crc32 = ngx_crc32_long((uchar *)fm.addr + sizeof(ngx_stream_geo_header_t), fm.size - sizeof(ngx_stream_geo_header_t));
		}
...
static uchar * ngx_stream_geo_copy_values(uchar * base, uchar * p, ngx_rbtree_node_t * node, ngx_rbtree_node_t * sentinel)
{
	if(node == sentinel) {
		...
		ngx_stream_variable_value_t * vv;
		ngx_stream_geo_variable_value_node_t  * gvvn = (ngx_stream_geo_variable_value_node_t*)node;
		gvvn->offset = p - base;
