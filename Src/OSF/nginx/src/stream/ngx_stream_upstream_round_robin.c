...
#if (NGX_STREAM_SSL)
	static ngx_int_t ngx_stream_upstream_set_round_robin_peer_session(ngx_peer_connection_t * pc, void * data);
...
ngx_int_t ngx_stream_upstream_init_round_robin(ngx_conf_t * cf, ngx_stream_upstream_srv_conf_t * us)
{
	...
	if(us->servers) {
		server = (ngx_stream_upstream_server_t *)us->servers->elts;
		n = 0;
		w = 0;
		...
		peers = (ngx_stream_upstream_rr_peers_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_upstream_rr_peers_t));
		if(peers == NULL) {
			return NGX_ERROR;
		}
		peer = (ngx_stream_upstream_rr_peer_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_upstream_rr_peer_t) * n);
		if(peer == NULL) {
			return NGX_ERROR;
		}
		peers->single = (n == 1);
		...
		backup = (ngx_stream_upstream_rr_peers_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_upstream_rr_peers_t));
		if(backup == NULL) {
			return NGX_ERROR;
		}
		peer = (ngx_stream_upstream_rr_peer_t *)ngx_pcalloc(cf->pool, sizeof(ngx_stream_upstream_rr_peer_t) * n);
...
{
	ngx_uint_t n;
	ngx_stream_upstream_rr_peer_data_t * rrp = (ngx_stream_upstream_rr_peer_data_t *)s->upstream->peer.data;
	if(rrp == NULL) {
		rrp = (ngx_stream_upstream_rr_peer_data_t *)ngx_palloc(s->connection->pool, sizeof(ngx_stream_upstream_rr_peer_data_t));
		if(rrp == NULL) {
			return NGX_ERROR;
		}
	...
	if(n <= 8 * sizeof(uintptr_t)) {
		...
		rrp->tried = (uintptr_t *)ngx_pcalloc(s->connection->pool, n * sizeof(uintptr_t));
		if(rrp->tried == NULL) {
			return NGX_ERROR;
		}
	}
...
ngx_int_t ngx_stream_upstream_create_round_robin_peer(ngx_stream_session_t * s, ngx_stream_upstream_resolved_t * ur)
{
	...
	peers = (ngx_stream_upstream_rr_peers_t *)ngx_pcalloc(s->connection->pool, sizeof(ngx_stream_upstream_rr_peers_t));
	if(peers == NULL) {
		return NGX_ERROR;
	}
	peer = (ngx_stream_upstream_rr_peer_t *)ngx_pcalloc(s->connection->pool, sizeof(ngx_stream_upstream_rr_peer_t) * ur->naddrs);
	if(peer == NULL) {
		return NGX_ERROR;
	}
	...
	if(ur->sockaddr) {
		...
		for(i = 0; i < ur->naddrs; i++) {
			...
			sockaddr = (struct sockaddr *)ngx_palloc(s->connection->pool, socklen);
			if(sockaddr == NULL) {
				return NGX_ERROR;
			}
			memcpy(sockaddr, ur->addrs[i].sockaddr, socklen);
			ngx_inet_set_port(sockaddr, ur->port);
			p = (uchar *)ngx_pnalloc(s->connection->pool, NGX_SOCKADDR_STRLEN);
			if(!p) {
				return NGX_ERROR;
			}
...
{
	ngx_stream_upstream_rr_peer_data_t  * rrp = (ngx_stream_upstream_rr_peer_data_t *)data;
	ngx_stream_upstream_rr_peer_t  * peer = rrp->current;
...
static ngx_int_t ngx_stream_upstream_set_round_robin_peer_session(ngx_peer_connection_t * pc, void * data)
{
	...
	if(peers->shpool) {
		...
		ngx_log_debug1(NGX_LOG_DEBUG_STREAM, pc->log, 0, "set session: %p", ssl_session);
...
static void ngx_stream_upstream_save_round_robin_peer_session(ngx_peer_connection_t * pc, void * data)
{
	...
	if(peers->shpool) {
		...
		ngx_log_debug1(NGX_LOG_DEBUG_STREAM, pc->log, 0, "save session: %p", ssl_session);
