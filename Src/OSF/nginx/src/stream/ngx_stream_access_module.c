...
{
	ngx_uint_t i;
	ngx_stream_access_rule_t  * rule = (ngx_stream_access_rule_t *)ascf->rules->elts;
...
static ngx_int_t ngx_stream_access_inet6(ngx_stream_session_t * s, ngx_stream_access_srv_conf_t * ascf, u_char * p)
{
	ngx_uint_t n;
	ngx_uint_t i;
	ngx_stream_access_rule6_t * rule6 = (ngx_stream_access_rule6_t *)ascf->rules6->elts;
...
static char * ngx_stream_access_rule(ngx_conf_t * cf, ngx_command_t * cmd, void * conf)
{
	...
	if(cidr.family == AF_INET || all) {
		...
		rule = (ngx_stream_access_rule_t *)ngx_array_push(ascf->rules);
		if(rule == NULL) {
			return NGX_CONF_ERROR;
		}
	...
	if(cidr.family == AF_INET6 || all) {
		...
		rule6 = (ngx_stream_access_rule6_t *)ngx_array_push(ascf->rules6);
		if(rule6 == NULL) {
			return NGX_CONF_ERROR;
		}
...
{
	ngx_stream_access_srv_conf_t  * prev = (ngx_stream_access_srv_conf_t *)parent;
	ngx_stream_access_srv_conf_t  * conf = (ngx_stream_access_srv_conf_t *)child;
	if(conf->rules == NULL
#if (NGX_HAVE_INET6)
	    && conf->rules6 == NULL
#endif
