...
#define ngx_stream_script_exit  (uchar *)&ngx_stream_script_exit_code
...
ngx_int_t ngx_stream_complex_value(ngx_stream_session_t * s, ngx_stream_complex_value_t * val, ngx_str_t * value)
{
	...
	value->len = len;
	value->data = static_cast<uchar *>(ngx_pnalloc(s->connection->pool, len));
	if(value->data == NULL) {
		return NGX_ERROR;
	}
...
uchar * ngx_stream_script_run(ngx_stream_session_t * s, ngx_str_t * value, void * code_lengths, size_t len, void * code_values)
{
	ngx_uint_t i;
	ngx_stream_script_code_pt code;
	ngx_stream_script_engine_t e;
...
{
	if(indices) {
		const ngx_uint_t * index = (const ngx_uint_t *)indices->elts;
		for(ngx_uint_t n = 0; n < indices->nelts; n++) {
...
static ngx_int_t ngx_stream_script_init_arrays(ngx_stream_script_compile_t * sc)
{
	...
	if(*sc->values == NULL) {
		n = (sc->variables * (2 * sizeof(ngx_stream_script_copy_code_t) + sizeof(ngx_stream_script_var_code_t))
		    + sizeof(uintptr_t) + sc->source->len + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
...
{
	uchar * elts = (uchar *)codes->elts;
	void * p_new = ngx_array_push_n(codes, size);
	if(p_new) {
		if(code) {
			if(elts != codes->elts) {
				uchar ** p = (uchar **)code;
    ...
    ngx_str_t * value, ngx_uint_t last)
{
	...
	code->len = len;
	p = ngx_cpymem((uchar *)code + sizeof(ngx_stream_script_copy_code_t), value->data, value->len);
	if(zero) {
		*p = '\0';
		sc->zero = 0;
	}
...
{
	ngx_stream_script_copy_code_t  * code = (ngx_stream_script_copy_code_t*)e->ip;
...
static ngx_int_t ngx_stream_script_add_var_code(ngx_stream_script_compile_t * sc, ngx_str_t * name)
{
	...
	ngx_int_t index = ngx_stream_get_variable_index(sc->cf, name);
	if(index == NGX_ERROR) {
		return NGX_ERROR;
	}
	if(sc->flushes) {
...
size_t ngx_stream_script_copy_capture_len_code(ngx_stream_script_engine_t * e)
{
	int  * cap;
	ngx_uint_t n;
	ngx_stream_session_t * s = e->session;
	ngx_stream_script_copy_capture_code_t  * code = (ngx_stream_script_copy_capture_code_t*)e->ip;
