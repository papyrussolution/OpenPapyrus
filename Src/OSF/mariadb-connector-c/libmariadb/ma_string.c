...
bool ma_dynstr_append_mem(DYNAMIC_STRING * str, const char * append, size_t length)
{
	...
	if(str->length+length >= str->max_length) {
		size_t new_length = (str->length+length+str->alloc_increment)/str->alloc_increment;
		new_length *= str->alloc_increment;
		if(!(new_ptr = (char *)SAlloc::R(str->str, new_length)))
			return TRUE;
		str->str = new_ptr;
...
char * FASTCALL ma_strmake(char * dst, const char * src, size_t length)
{
	while(length--)
		if(!(*dst++ = *src++))
			return dst-1;
