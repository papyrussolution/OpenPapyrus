...
void ma_init_alloc_root(MA_MEM_ROOT * mem_root, size_t block_size, size_t pre_alloc_size)
{
	...
	if(pre_alloc_size) {
		if((mem_root->free = mem_root->pre_alloc = (MA_USED_MEM *)SAlloc::M(pre_alloc_size+ ALIGN_SIZE(sizeof(MA_USED_MEM))))) {
...
char * FASTCALL ma_strdup_root(MA_MEM_ROOT * root, const char * str)
{
	size_t len = strlen(str)+1;
	char * pos;
	if((pos = (char *)ma_alloc_root(root, len)))
		memcpy(pos, str, len);
	return pos;
}
...
void * ma_multi_malloc(myf myFlags, ...)
{
	...
	if(!(start = (char *)SAlloc::M(tot_length)))
		return 0;
