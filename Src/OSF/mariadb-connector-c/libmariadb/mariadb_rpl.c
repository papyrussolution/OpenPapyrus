...
MARIADB_RPL_EVENT * STDCALL mariadb_rpl_fetch(MARIADB_RPL * rpl, MARIADB_RPL_EVENT * event)
{
	...
	while(1) {
		...
		if(event) {
			...
			memzero(rpl_event, sizeof(MARIADB_RPL_EVENT));
			rpl_event->memroot = memroot;
		}
		else {
			if(!(rpl_event = (MARIADB_RPL_EVENT*)SAlloc::M(sizeof(MARIADB_RPL_EVENT))))
				goto mem_error;
			memzero(rpl_event, sizeof(MARIADB_RPL_EVENT));
		...
		switch(rpl_event->event_type) {
			...
			case GTID_LIST_EVENT:
		    {
				...
				(MARIADB_GTID*)ma_alloc_root(&rpl_event->memroot,
			...
			case DELETE_ROWS_EVENT_V1:
			    rpl_event->event.rows.type = (mariadb_row_event_type)(rpl_event->event_type - WRITE_ROWS_EVENT_V1);
