...
MA_FILE * ma_open(const char * location, const char * mode, MYSQL * mysql)
{
	...
	/* See CONC-44: we need to support non ascii filenames too, so we convert
	   current character set to wchar_t and try to open the file via _wsopen */
		...
		wchar_t * w_mode = NULL;
		DWORD Length;
		int len = MultiByteToWideChar(CodePage, 0, location, (int)strlen(location), NULL, 0);
		if(!len)
			return NULL;
		if(!(w_filename = (wchar_t *)SAlloc::C(1, (len + 1) * sizeof(wchar_t)))) {
		...
		len = (int)strlen(mode);
		if(!(w_mode = (wchar_t *)SAlloc::C(1, (len + 1) * sizeof(wchar_t)))) {
			my_set_error(mysql, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			SAlloc::F(w_filename);
			return NULL;
		}
...
#endif
	if(fp) {
		ma_file = (MA_FILE *)SAlloc::M(sizeof(MA_FILE));
		if(!ma_file) {
			fclose(fp);
			my_set_error(mysql, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			return NULL;
