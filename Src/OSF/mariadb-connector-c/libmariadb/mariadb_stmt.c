...
#define UPDATE_STMT_ERROR(stmt) SET_CLIENT_STMT_ERROR((stmt), (stmt)->mysql->net.last_errno, (stmt)->mysql->net.sqlstate, (stmt)->mysql->net.last_error)
...
void stmt_set_error(MYSQL_STMT * stmt, unsigned int error_nr, const char * sqlstate, const char * format, ...)
{
	va_list ap;
	...
	va_start(ap, format);
	vsnprintf(stmt->last_error, MYSQL_ERRMSG_SIZE, format ? format : error ? error : "", ap);
	va_end(ap);
	return;
}
...
static bool madb_reset_stmt(MYSQL_STMT * stmt, unsigned int flags);
static bool mysql_stmt_internal_reset(MYSQL_STMT * stmt, bool is_close);

static int stmt_unbuffered_eof(MYSQL_STMT * stmt __attribute__((unused)), uchar ** row __attribute__((unused)))
{
...
{
	ulong pkt_len = ma_net_safe_read(stmt->mysql);
...
int mthd_stmt_read_all_rows(MYSQL_STMT * stmt)
{
	...
	ulong packet_len;
	uchar * p;
	MYSQL_ROWS ** pprevious = &result->data;
...
{
	ulong packet_len;
	int in_resultset = stmt->state > MYSQL_STMT_EXECUTED && stmt->state < MYSQL_STMT_FETCH_DONE;
	while((packet_len = ma_net_safe_read(stmt->mysql)) != packet_error) {
...
uchar* mysql_stmt_execute_generate_simple_request(MYSQL_STMT * stmt, size_t * request_len)
{
	...
	if(stmt->param_count) {
		...
		if(null_count + 20 > free_bytes) {
			...
			if(!(start = (uchar *)SAlloc::R(start, length)))
				goto mem_error;
			p = start + offset;
		}
				...
				if(has_data) {
					switch(stmt->params[i].buffer_type) {
						...
						case MYSQL_TYPE_SET:
						    size += 5; /* max 8 bytes for size */
						    if(!stmt->param_callback) {
							    if(indicator == STMT_INDICATOR_NTS || (!stmt->row_size && ma_get_length(stmt, i, j) == -1)) {
...
bool STDCALL mysql_stmt_bind_param(MYSQL_STMT * stmt, const MYSQL_BIND * bind)
{
	MYSQL * mysql = stmt->mysql;
	if(!mysql) {
...
bool STDCALL mysql_stmt_bind_result(MYSQL_STMT * stmt, const MYSQL_BIND * bind)
{
	uint i;
	...
	for(i = 0; i < stmt->field_count; i++) {
		if(stmt->mysql->methods->db_supported_buffer_type && !stmt->mysql->methods->db_supported_buffer_type(bind[i].buffer_type)) {
			SET_CLIENT_STMT_ERROR(stmt, CR_UNSUPPORTED_PARAM_TYPE, SQLSTATE_UNKNOWN, 0);
			return 1;
		}
		if(!stmt->bind[i].is_null)
...
bool mthd_stmt_get_result_metadata(MYSQL_STMT * stmt)
{
	MYSQL_DATA * result;
	MA_MEM_ROOT * fields_ma_alloc_root = &((MADB_STMT_EXTENSION*)stmt->extension)->fields_ma_alloc_root;
	if(!(result = stmt->mysql->methods->db_read_rows(stmt->mysql, (MYSQL_FIELD*)0, 7 + ma_extended_type_info_rows(stmt->mysql))))
		return 1;
...
int STDCALL mysql_stmt_prepare(MYSQL_STMT * stmt, const char * query, unsigned long length)
{
	...
	if(stmt->state > MYSQL_STMT_INITTED) {
		...
		stmt->params = 0;
		int4store(stmt_id, stmt->stmt_id);
		if(mysql->methods->db_command(mysql, COM_STMT_CLOSE, stmt_id, sizeof(stmt_id), 1, stmt))
			goto fail;
	}
...
static int madb_alloc_stmt_fields(MYSQL_STMT * stmt)
{
	...
	if(stmt->mysql->field_count) {
		...
		for(i = 0; i < stmt->field_count; i++) {
			...
			stmt->fields[i].extension = stmt->mysql->fields[i].extension ? ma_field_extension_deep_dup(fields_ma_alloc_root, (MA_FIELD_EXTENSION *)stmt->mysql->fields[i].extension) : NULL;
		}
		if(!(stmt->bind = (MYSQL_BIND*)ma_alloc_root(fields_ma_alloc_root, stmt->field_count * sizeof(MYSQL_BIND)))) {
			SET_CLIENT_STMT_ERROR(stmt, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			return 1;
...
int stmt_read_execute_response(MYSQL_STMT * stmt)
{
	...
	if(ret) {
		SET_CLIENT_STMT_ERROR(stmt, mysql->net.last_errno, mysql->net.sqlstate, mysql->net.last_error);
		stmt->state = MYSQL_STMT_PREPARED;
		return 1;
	}
		...
		if(!stmt->field_count || mysql->server_status & SERVER_MORE_RESULTS_EXIST) { /* fix for ps_bug: test_misc */
			...
			for(i = 0; i < stmt->field_count; i++) {
				...
				/* since  all pointers will be incorrect if another statement will
				   be executed, so we need to allocate memory and copy the information */
...
{
	return (stmt->stmt_id > 0 && stmt->stmt_id != (unsigned long)-1) ? mysql_stmt_internal_reset(stmt, 0) : 0;
}
...
{
	MYSQL_ROW_OFFSET old_row = stmt->result_cursor; /* for returning old position */
...
bool STDCALL mysql_stmt_send_long_data(MYSQL_STMT * stmt, uint param_number, const char * data, unsigned long length)
{
	...
	if(length || !stmt->params[param_number].long_data_used) {
		...
		ret = stmt->mysql->methods->db_command(stmt->mysql, COM_STMT_SEND_LONG_DATA, (char *)cmd_buff, packet_len, 1, stmt);
		if(ret)
			UPDATE_STMT_ERROR(stmt);
		SAlloc::F(cmd_buff);
		return(ret);
...
int STDCALL mariadb_stmt_execute_direct(MYSQL_STMT * stmt, const char * stmt_str, size_t length)
{
	...
	// Server versions < 10.2 don't support execute_direct, so we need to emulate it 
	if(emulate_cmd) {
		int rc;
