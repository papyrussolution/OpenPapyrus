...
MARIADB_PVIO * ma_pvio_init(MA_PVIO_CINFO * cinfo)
{
	...
	switch(cinfo->type) {
		...
		case PVIO_TYPE_SOCKET: type = 0; break;
#ifdef _WIN32
		case PVIO_TYPE_NAMEDPIPE: type = 1; break;
		case PVIO_TYPE_SHAREDMEM: type = 2; break;
#endif
		default:
		    return NULL;
	}
	if(!(pvio_plugin = (MARIADB_PVIO_PLUGIN*)
	    mysql_client_find_plugin(cinfo->mysql, pvio_plugins[type],
	    MARIADB_CLIENT_PVIO_PLUGIN))) {
		/* error already set in mysql_client_find_plugin */
		return NULL;
	}
	/* coverity[var_deref_op] */
	if(!(pvio = (MARIADB_PVIO*)SAlloc::C(1, sizeof(MARIADB_PVIO)))) {
	...
	if(!(pvio->cache = (uchar *)SAlloc::C(1, PVIO_READ_AHEAD_CACHE_SIZE))) {
...
ssize_t ma_pvio_read(MARIADB_PVIO * pvio, uchar * buffer, size_t length)
{
	...
	if(pvio_callback) {
		...
		while(p) {
			callback = reinterpret_cast<void (*)(int, MYSQL *, const uchar *, size_t)>(p->data);
...
ssize_t FASTCALL ma_pvio_cache_read(MARIADB_PVIO * pvio, uchar * buffer, size_t length)
{
	ssize_t r;
	if(!pvio)
		return -1;
...
ssize_t ma_pvio_write(MARIADB_PVIO * pvio, const uchar * buffer, size_t length)
{
	...
	if(pvio_callback) {
		...
		LIST * p = pvio_callback;
		while(p) {
			callback = static_cast<void (*)(int, MYSQL *, const uchar *, size_t)>(p->data);
...
bool ma_pvio_start_ssl(MARIADB_PVIO * pvio)
{
	 ...
	 */
	if((pvio->mysql->client_flag & CLIENT_SSL_VERIFY_SERVER_CERT) && ma_pvio_tls_verify_server_cert(pvio->ctls))
		return 1;
