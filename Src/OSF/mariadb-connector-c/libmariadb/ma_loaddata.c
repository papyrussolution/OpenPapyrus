...
static int mysql_local_infile_init(void ** ptr, const char * filename, void * userdata)
{
	MYSQL * mysql = (MYSQL*)userdata;
	MYSQL_INFILE_INFO * info = (MYSQL_INFILE_INFO*)SAlloc::M(sizeof(MYSQL_INFILE_INFO));
	if(!info) {
		return 1;
	}
...
bool mysql_handle_local_infile(MYSQL * conn, const char * filename, bool can_local_infile)
{
	...
	if(!(conn->options.client_flag & CLIENT_LOCAL_FILES) ||
		...
		ma_net_write(&conn->net, (uchar *)"", 0);
		ma_net_flush(&conn->net);
		goto infile_error;
	}

	/* allocate buffer for reading data */
	buf = (uchar *)SAlloc::M(buflen);
	...
	while((bufread = conn->options.local_infile_read(info, (char *)buf, buflen)) > 0) {
		if(ma_net_write(&conn->net, (uchar *)buf, bufread)) {
	...
	if(ma_net_write(&conn->net, (uchar *)"", 0) ||
	    ma_net_flush(&conn->net)) {
		my_set_error(conn, CR_SERVER_LOST, SQLSTATE_UNKNOWN, NULL);
		goto infile_error;
	}
