...
int my_ssl_read_async(struct mysql_async_context * b, SSL * ssl, void * buf, int size)
{
	int res;
	for(;;) {
...
int my_ssl_write_async(struct mysql_async_context * b, SSL * ssl, const void * buf, int size)
{
	int res;
	for(;;) {
...
#define MK_ASYNC_START_BODY_RESULTCAST(call, mysql_val, parms_assign, err_val, ok_val, extra1, result_type) \
	int res;                                                                    \
	struct mysql_async_context * b;                                              \
	struct call ## _params parms;                                               \
	extra1 b = mysql_val->options.extension->async_context; \
	...
	if(res < 0) {                                                                 \
		...
		*ret = (result_type)(b->ret_result.ok_val);                             \
	return 0;
...
{
	MK_ASYNC_CONT_BODY_RESULTCAST(mysql, NULL, r_ptr, MYSQL *)
}
...
int STDCALL mysql_real_query_start(int * ret, MYSQL * mysql, const char * stmt_str, unsigned long length)
{
	int res;
...
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_fetch_row, result->handle,
	{
...
{
	MK_ASYNC_CONT_BODY_RESULTCAST(result->handle, NULL, r_ptr, MYSQL_ROW)
}
...
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_store_result, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
...
{
	MK_ASYNC_CONT_BODY_RESULTCAST(mysql, NULL, r_ptr, MYSQL_RES *)
}
...
/*
   These following are not available inside the server (neither blocking or non-blocking).
 */
#ifndef MYSQL_SERVER
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_shutdown, (parms->mysql, parms->shutdown_level), parms->mysql, int, r_int)
}
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_dump_debug_info, (parms->mysql), parms->mysql, int, r_int)
}
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_kill, (parms->mysql, parms->pid), parms->mysql, int, r_int)
}

int STDCALL mysql_kill_start(int * ret, MYSQL * mysql, unsigned long pid)
{
	MK_ASYNC_START_BODY(mysql_kill, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
		parms.pid = pid;
...
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_stat, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
...
{
	MK_ASYNC_CONT_BODY_RESULTCAST(mysql, NULL, r_const_ptr, const char *)
}
...
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_list_dbs, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
		parms.wild = wild;
...
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_list_tables, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
		parms.wild = wild;
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_list_processes, (parms->mysql), parms->mysql, MYSQL_RES *, r_ptr)
}

int STDCALL mysql_list_processes_start(MYSQL_RES ** ret, MYSQL * mysql)
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_list_processes, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
	}, NULL, r_ptr, /* Nothing */, MYSQL_RES *)
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_list_fields, (parms->mysql, parms->table, parms->wild), parms->mysql, MYSQL_RES *, r_ptr)
}

int STDCALL mysql_list_fields_start(MYSQL_RES ** ret, MYSQL * mysql, const char * table, const char * wild)
{
	MK_ASYNC_START_BODY_RESULTCAST(mysql_list_fields, mysql,
	{
		WIN_SET_NONBLOCKING(mysql)
		parms.mysql = mysql;
		parms.table = table;
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_stmt_prepare, (parms->stmt, parms->query, parms->length), parms->stmt->mysql, int, r_int)
}
...
int STDCALL mysql_stmt_prepare_start(int * ret, MYSQL_STMT * stmt, const char * query, unsigned long length)
{
	MK_ASYNC_START_BODY(mysql_stmt_prepare, stmt->mysql,
	{
		...
		parms.query = query;
		parms.length = length;
	}, 1, r_int, /* If stmt->mysql==NULL then we will not block so can call directly. */
		if(!stmt->mysql) {
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_stmt_fetch, (parms->stmt), parms->stmt->mysql, int, r_int)
}

int STDCALL mysql_stmt_fetch_start(int * ret, MYSQL_STMT * stmt)
{
	MK_ASYNC_START_BODY(mysql_stmt_fetch, stmt->mysql,
	{
		WIN_SET_NONBLOCKING(stmt->mysql)
		parms.stmt = stmt;
	},
...
{
	MK_ASYNC_INTERNAL_BODY(mysql_stmt_store_result, (parms->stmt), parms->stmt->mysql, int, r_int)
}
