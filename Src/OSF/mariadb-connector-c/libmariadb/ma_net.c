...
static bool net_realloc(NET * net, size_t length)
{
	...
	/* reallocate buffer: size= pkt_length + NET_HEADER_SIZE + COMP_HEADER_SIZE */
...
int ma_net_flush(NET * net)
{
	...
	if(net->buff != net->write_pos) {
		error = ma_net_real_write(net, (char *)net->buff, (size_t)(net->write_pos - net->buff));
		net->write_pos = net->buff;
	}
	if(net->compress)
...
int ma_net_write(NET * net, const uchar * packet, size_t len)
{
	...
	while(len >= MAX_PACKET_LENGTH) {
		...
		if(ma_net_write_buff(net, (char *)buff, NET_HEADER_SIZE) || ma_net_write_buff(net, (char *)packet, max_len))
			return 1;
		packet += max_len;
		len -= max_len;
	}
	...
	int3store(buff, len);
	buff[3] = (uchar)net->pkt_nr++;
	if(ma_net_write_buff(net, (char *)buff, NET_HEADER_SIZE) || ma_net_write_buff(net, (char *)packet, len))
		return 1;
	return 0;
}
...
int ma_net_write_command(NET * net, uchar command, const char * packet, size_t len, bool disable_flush)
{
	...
	if(length >= MAX_PACKET_LENGTH) {
		...
		do {
			...
			if(ma_net_write_buff(net, (char *)buff, buff_size) || ma_net_write_buff(net, packet, len))
				return 1;
			packet += len;
	...
	buff[3] = (net->compress) ? 0 : (uchar)(net->pkt_nr++);
	rc = test(ma_net_write_buff(net, (char *)buff, buff_size) || ma_net_write_buff(net, packet, len));
	if(!rc && !disable_flush)
...
static int ma_net_write_buff(NET * net, const char * packet, size_t len)
{
	...
	if(len > left_length) {
		if(net->write_pos != net->buff) {
			...
			if(ma_net_real_write(net, (char *)net->buff, (size_t)(net->write_pos - net->buff) + left_length))
				return 1;
			packet += left_length;
			len -= left_length;
			net->write_pos = net->buff;
...
// 
static ulong FASTCALL ma_real_read(NET * net, size_t * complen)
{
	uchar * pos;
	ssize_t length;
	uint i;
...
ulong ma_net_read(NET * net)
{
	...
	for(;;) {
		if(buffer_length - current >= 4) {
			...
			if(!packet_length) {
				current += 4; /* length + sequence_id, no more data will follow */
				break;
			}
			if(packet_length + 4 <= buffer_length - current) {
				if(!is_multi_packet) {
					...
					memmove(net->buff + current, net->buff + current + 4, buffer_length - current);
					buffer_length -= 4;
...
int net_add_multi_command(NET * net, uchar command, const uchar * packet, size_t length)
{
	if(net->extension->multi_status == COM_MULTI_OFF) {
		return 1;
	}
