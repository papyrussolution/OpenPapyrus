...
static void convert_froma_string(MYSQL_BIND * r_param, char * buffer, size_t len)
{
	...
	switch(r_param->buffer_type) {
	    ...
	    {
		    longlong val = my_atoll(buffer, buffer + len, &error);
		    *r_param->error = error ? 1 : r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX32) : NUMERIC_TRUNCATION(val,
			    INT_MIN32, INT_MAX32) || error > 0;
...
static void convert_from_long(MYSQL_BIND * r_param, const MYSQL_FIELD * field, longlong val, bool is_unsigned)
{
	switch(r_param->buffer_type) {
		case MYSQL_TYPE_TINY:
		    ...
		    *r_param->error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX8) : NUMERIC_TRUNCATION(val,
			    INT_MIN8,
			    INT_MAX8);
		...
		case MYSQL_TYPE_YEAR:
		    shortstore(r_param->buffer, (short)val);
		    *r_param->error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX16) : NUMERIC_TRUNCATION(val,
			    INT_MIN16,
			    INT_MAX16);
		...
		case MYSQL_TYPE_LONG:
		    longstore(r_param->buffer, (int32)val);
		    *r_param->error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX32) : NUMERIC_TRUNCATION(val,
			    INT_MIN32,
			    INT_MAX32);
		...
		case MYSQL_TYPE_DOUBLE:
	    {
		    volatile double dbl = (is_unsigned) ? ulonglong2double((ulonglong)val) : (double)val;
		    doublestore(r_param->buffer, dbl);
		    *r_param->error = (dbl != ceil(dbl)) || (is_unsigned ? (ulonglong)dbl != (ulonglong)val : (longlong)dbl != (longlong)val);
		    r_param->buffer_length = 8;
		    break;
	    }
		case MYSQL_TYPE_FLOAT:
	...
	switch(r_param->buffer_type) {
		...
		default:
	    {
		    ...
		    switch(field->type) {
			    ...
			    case MYSQL_TYPE_DATETIME:
			    case MYSQL_TYPE_TIMESTAMP:
				length = sprintf(dtbuffer, "%04u-%02u-%02u %02u:%02u:%02u",
...
static void ps_fetch_bin(MYSQL_BIND * r_param, const MYSQL_FIELD * field, uchar ** row)
{
	if(field->charsetnr == 63) {
		...
		if(copylen < r_param->buffer_length && (r_param->buffer_type == MYSQL_TYPE_STRING || r_param->buffer_type == MYSQL_TYPE_JSON))
			((char *)r_param->buffer)[copylen] = 0;
		*r_param->error = copylen > r_param->buffer_length;
		(*row) += field_length;
	}
...
{
	memzero(mysql_ps_fetch_functions, sizeof(mysql_ps_fetch_functions));
