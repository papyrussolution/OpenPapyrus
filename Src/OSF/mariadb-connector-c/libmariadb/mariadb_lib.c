...
extern bool ma_init_done;
extern bool mysql_ps_subsystem_initialized;
extern bool mysql_handle_local_infile(MYSQL * mysql, const char * filename, bool can_local_infile);
extern const MARIADB_CHARSET_INFO * mysql_find_charset_nr(uint charsetnr);
extern const MARIADB_CHARSET_INFO * mysql_find_charset_name(const char * name);
extern bool set_default_charset_by_name(const char * cs_name, myf flags __attribute__((unused)));
...
extern bool mthd_supported_buffer_type(enum enum_field_types type);
...
extern bool _mariadb_read_options(MYSQL * mysql, const char * dir, const char * config_file, const char * group, uint recursion);
...
extern struct st_mariadb_methods MARIADB_DEFAULT_METHODS;
...
#define IS_CONNHDLR_ACTIVE(mysql) ((mysql)->extension && (mysql)->extension->conn_hdlr)
...
/* net_get_error */
void net_get_error(char * buf, size_t buf_len, char * error, size_t error_len, uint * error_no, char * sqlstate)
{
	char * p = buf;
...
restart:
	...
	if(len == packet_error || len == 0) {
		end_server(mysql);
		my_set_error(mysql, net->last_errno == ER_NET_PACKET_TOO_LARGE ? CR_NET_PACKET_TOO_LARGE : CR_SERVER_LOST, SQLSTATE_UNKNOWN, 0, errno);
		return packet_error;
	}
	if(net->read_pos[0] == 255) {
		if(len > 3) {
			...
			pos += 2;
			len -= 2;
			if(last_errno== 65535 && ((mariadb_connection(mysql) && (mysql->server_capabilities & CLIENT_PROGRESS)) ||
...
static uint64 net_field_length_ll(uchar ** packet)
{
	...
	if(*pos == 251) {
		(*packet)++;
		return (uint64)NULL_LENGTH;
	}
	if(*pos == 252) {
		(*packet) += 3;
		return (uint64)uint2korr(pos+1);
	}
	if(*pos == 253) {
		(*packet) += 4;
		return (uint64)uint3korr(pos+1);
	}
	(*packet) += 9;                         /* Must be 254 when here */
#ifdef NO_CLIENT_LONGLONG
	return (uint64)uint4korr(pos+1);
#else
	return (uint64)uint8korr(pos+1);
#endif
}
...
int mthd_my_send_cmd(MYSQL * mysql, enum enum_server_command command, const char * arg,
    size_t length, bool skipp_check, void * opt_arg)
{
	...
	if(mysql->status != MYSQL_STATUS_READY || mysql->server_status & SERVER_MORE_RESULTS_EXIST) {
...
int ma_simple_command(MYSQL * mysql, enum enum_server_command command, const char * arg,
    size_t length, bool skipp_check, void * opt_arg)
{
	if((mysql->options.client_flag & CLIENT_LOCAL_FILES) && mysql->options.extension && mysql->extension->auto_local_infile == WAIT_FOR_QUERY &&
	    arg && (*arg == 'l' || *arg == 'L') && command == COM_QUERY) {
...
static void FASTCALL free_old_query(MYSQL * mysql)
{
	if(mysql->fields)
...
struct st_default_options mariadb_defaults[] =
{
	...
	{(mysql_option)0, (enum_option_type)0, "disable-local-infile", },
	...
	{(mysql_option)0, (enum_option_type)0, NULL}
};
...
bool _mariadb_set_conf_option(MYSQL * mysql, const char * config_option, const char * config_value)
{
	if(config_option) {
		int i;
		char * c;
...
MYSQL_FIELD * unpack_fields(const MYSQL * mysql,
    ...
    bool default_value)
{
	...
	char    * p;
	uint i, field_count = sizeof(rset_field_offsets)/sizeof(size_t)/2;
    ...
    uint fields)
{
	...
	while(*(cp = net->read_pos) != 254 || pkt_len >= 8) {
		result->rows++;
		if(!(cur = (MYSQL_ROWS*)ma_alloc_root(&result->alloc, sizeof(MYSQL_ROWS))) || !(cur->data = ((MYSQL_ROW)ma_alloc_root(&result->alloc, (fields+1)*sizeof(char *)+fields+pkt_len)))) {
			free_rows(result);
			SET_CLIENT_ERROR(mysql, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			return 0;
		}
...
MYSQL * STDCALL mysql_init(MYSQL * mysql)
{
	if(mysql_server_init(0, NULL, NULL))
		return NULL;
	if(!mysql) {
		if(!(mysql = (MYSQL*)SAlloc::C(1, sizeof(MYSQL))))
			return 0;
		mysql->free_me = 1;
		mysql->net.pvio = 0;
		...
		memzero((char *)(mysql), sizeof(*(mysql)));
		mysql->net.pvio = 0;
		mysql->free_me = 0;
		mysql->net.extension = 0;
	}
...
const char * STDCALL mysql_get_ssl_cipher(MYSQL * mysql __attribute__((unused)))
{
#ifdef HAVE_TLS
...
static bool ma_set_connect_attrs(MYSQL * mysql, const char * host)
{
	char buffer[255];
	int rc = mysql_options(mysql, MYSQL_OPT_CONNECT_ATTR_DELETE, "_client_name") +
    ...
    uint port, const char * unix_socket, unsigned long client_flag)
{
	...
	SETIFZ(mysql->methods, &MARIADB_DEFAULT_METHODS);
	if(connection_handler || (host && (end = (char *)strstr(host, "://")))) {
		...
		if(!(plugin = (MARIADB_CONNECTION_PLUGIN*)mysql_client_find_plugin(mysql, plugin_name, MARIADB_CLIENT_CONNECTION_PLUGIN)))
			return NULL;
		if(!(mysql->extension->conn_hdlr = (MA_CONNECTION_HANDLER*)SAlloc::C(1, sizeof(MA_CONNECTION_HANDLER)))) {
			SET_CLIENT_ERROR(mysql, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			return NULL;
		}
		...
		if(plugin && plugin->connect) {
			...
			if(!my) {
				SAlloc::F(mysql->extension->conn_hdlr);
...
#define MAX_SCHANNEL_CONNECT_ATTEMPTS 3
	{
		...
		MYSQL * my = NULL;
		while(ssl_retry) {
			if((my = mysql->methods->db_connect(mysql, host, user, passwd, db, port, unix_socket, client_flag | CLIENT_REMEMBER_OPTIONS)))
				return my;
    ...
    const char * passwd, const char * db, uint port, const char * unix_socket, unsigned long client_flag)
{
	...
	MA_PVIO_CINFO cinfo = {NULL, NULL, 0, (enum_pvio_type)-1, NULL};
	...
	if(mysql->options.my_cnf_file || mysql->options.my_cnf_group) {
		_mariadb_read_options(mysql, NULL, (mysql->options.my_cnf_file ? mysql->options.my_cnf_file : NULL), mysql->options.my_cnf_group, 0);
		SAlloc::F(mysql->options.my_cnf_file);
		SAlloc::F(mysql->options.my_cnf_group);
	...
	if(!passwd) {
		...
		SETIFZ(passwd, getenv("MYSQL_PWD")); // get it from environment (haneke) 
		SETIFZ(passwd, "");
#endif
	}
	if(!db || !db[0])
		db = mysql->options.db;
	SETIFZ(port, mysql->options.port);
	SETIFZ(unix_socket, mysql->options.unix_socket);
	...
	*/
	...
	if(ma_pvio_wait_io_or_timeout(net->pvio, FALSE, 0) < 1) {
		my_set_error(mysql, CR_SERVER_LOST, SQLSTATE_UNKNOWN, ER(CR_SERVER_LOST_EXTENDED), "handshake: waiting for initial communication packet", errno);
		goto error;
	}
	*/
	if((pkt_length = ma_net_safe_read(mysql)) == packet_error) {
		if(mysql->net.last_errno == CR_SERVER_LOST)
			my_set_error(mysql, CR_SERVER_LOST, SQLSTATE_UNKNOWN, ER(CR_SERVER_LOST_EXTENDED), "handshake: reading initial communication packet", errno);
		goto error;
	}
	...
	if(mysql->protocol_version == 0XFF) {
		net_get_error(end, pkt_length - 1, net->last_error, sizeof(net->last_error), &net->last_errno, net->sqlstate);
		/* fix for bug #26426 */
	...
	if(mysql->protocol_version <  PROTOCOL_VERSION) {
		...
		sprintf(net->last_error, ER(CR_VERSION_ERROR), mysql->protocol_version, PROTOCOL_VERSION);
		goto error;
	}
	...
	// This is the first part of scramble packet. In 4.1 and later a second package will follow later 
	scramble_data = end;
	...
	if(run_plugin_auth(mysql, scramble_data, scramble_len, scramble_plugin, db))
		goto error;
	if(mysql->client_flag & CLIENT_COMPRESS)
		net->compress = 1;
	/* last part: select default db */
	if(!(mysql->server_capabilities & CLIENT_CONNECT_WITH_DB) && (db && !mysql->db)) {
		if(mysql_select_db(mysql, db)) {
			my_set_error(mysql, CR_SERVER_LOST, SQLSTATE_UNKNOWN, ER(CR_SERVER_LOST_EXTENDED), "Setting intital database", errno);
			goto error;
		}
	}
	if(mysql->options.init_command) {
		...
		bool save_reconnect = mysql->options.reconnect;
		mysql->options.reconnect = 0;
		for(; begin < end; begin++) {
			if(mysql_real_query(mysql, *begin, (ulong)strlen(*begin)))
				goto error;
...
error:
	...
	if(!(client_flag & CLIENT_REMEMBER_OPTIONS) && !mysql->options.extension->async_context)
		mysql_close_options(mysql);
	return 0;
}
 ...
 */
static void my_suspend_hook(bool suspend, void * data)
{
...
bool STDCALL mariadb_reconnect(MYSQL * mysql)
{
	...
	if(!mysql->options.reconnect || (mysql->server_status & SERVER_STATUS_IN_TRANS) || !mysql->host_info) {
	...
	memzero(&mysql->options, sizeof(mysql->options));
	mysql->free_me = 0;
	mysql_close(mysql);
	*mysql = tmp_mysql;
...
static void mysql_close_options(MYSQL * mysql)
{
	if(mysql->options.init_command) {
		...
		SAlloc::F(mysql->options.init_command);
	}
	SAlloc::F(mysql->options.user);
	SAlloc::F(mysql->options.host);
	SAlloc::F(mysql->options.password);
	SAlloc::F(mysql->options.unix_socket);
	SAlloc::F(mysql->options.db);
	SAlloc::F(mysql->options.my_cnf_file);
	SAlloc::F(mysql->options.my_cnf_group);
	SAlloc::F(mysql->options.charset_dir);
	SAlloc::F(mysql->options.charset_name);
	SAlloc::F(mysql->options.bind_address);
	SAlloc::F(mysql->options.ssl_key);
	SAlloc::F(mysql->options.ssl_cert);
	...
	if(mysql->options.extension) {
		...
		SAlloc::F(mysql->options.extension->plugin_dir);
		SAlloc::F(mysql->options.extension->default_auth);
		SAlloc::F(mysql->options.extension->db_driver);
		SAlloc::F(mysql->options.extension->ssl_crl);
		SAlloc::F(mysql->options.extension->ssl_crlpath);
		SAlloc::F(mysql->options.extension->tls_fp);
		SAlloc::F(mysql->options.extension->tls_fp_list);
		SAlloc::F(mysql->options.extension->tls_pw);
		SAlloc::F(mysql->options.extension->tls_version);
		SAlloc::F(mysql->options.extension->url);
		SAlloc::F(mysql->options.extension->connection_handler);
	...
	SAlloc::F(mysql->options.extension);
	/* clear all pointer */
	memzero(&mysql->options, sizeof(mysql->options));
}
...
static void ma_clear_session_state(MYSQL * mysql)
{
	...
	memzero(mysql->extension->session_state, sizeof(struct st_mariadb_session_state) * SESSION_TRACK_TYPES);
}
					...
					while(pos < end) {
						...
						switch(si_type) {
							...
							case SESSION_TRACK_GTIDS:
							    ...
							    if(pos + plen > end)
								    goto corrupted;
							    if(!(session_item = (LIST *)ma_multi_malloc(0, &session_item, sizeof(LIST), &str, sizeof(MYSQL_LEX_STRING), &data, plen, NULL)))
								    goto oom;
							    str->length = plen;
							    str->str = data;
							    ...
							    mysql->extension->session_state[si_type].list = list_add(
							    ...
							    if(si_type == SESSION_TRACK_SCHEMA) {
								    SAlloc::F(mysql->db);
								    mysql->db = (char *)SAlloc::M(plen + 1);
								    memcpy(mysql->db, str->str, plen);
								    mysql->db[plen] = 0;
							    }
							    else if(si_type == SESSION_TRACK_SYSTEM_VARIABLES) {
								    bool set_charset = 0;
								    /* make sure that we update charset in case it has
								       changed */
...
void STDCALL mysql_data_seek(MYSQL_RES * result, uint64 row)
{
	MYSQL_ROWS    * tmp = 0;
	if(result->data)
...
int mysql_optionsv(MYSQL * mysql, enum mysql_option option, ...)
{
	...
	switch(option) {
		...
		case MYSQL_OPT_RECONNECT:
		    mysql->options.reconnect = *(bool*)arg1;
		    break;
		...
		case MYSQL_REPORT_DATA_TRUNCATION:
		    mysql->options.report_data_truncation = *(bool*)arg1;
		    break;
		...
		case MYSQL_OPT_MAX_ALLOWED_PACKET:
		    if(mysql)
			    mysql->options.max_allowed_packet = (ulong)(*(size_t*)arg1);
		    else
			    max_allowed_packet = (ulong)(*(size_t*)arg1);
		    break;
		case MYSQL_OPT_NET_BUFFER_LENGTH:
		    net_buffer_length = (ulong)(*(size_t*)arg1);
		    break;
		...
		case MYSQL_OPT_SSL_ENFORCE:
		    mysql->options.use_ssl = (*(bool*)arg1);
		    break;
		...
		case MARIADB_OPT_USERDATA:
	    {
		    ...
		    p = (uchar *)hash_search(&mysql->options.extension->userdata, (uchar *)key, (uint)strlen(key));
		    if(p) {
			    p += strlen(key) + 1;
			    memcpy(p, &data, sizeof(void *));
			    break;
		    }
		    if(!(buffer = (uchar *)SAlloc::M(strlen(key) + 1 + sizeof(void *)))) {
			    SET_CLIENT_ERROR(mysql, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			    goto end;
		    }
		    p = buffer;
		...
		case MYSQL_OPT_CONNECT_ATTR_ADD:
	    {
		    ...
		    if((buffer = (uchar *)SAlloc::M(key_len + value_len))) {
			    uchar * p = buffer;
			    strcpy((char *)p, (const char *)arg1);
			    p += (strlen((const char *)arg1) + 1);
			    if(arg2)
				    strcpy((char *)p, (const char *)arg2);
		...
		case MYSQL_SECURE_AUTH:
		    mysql->options.secure_auth = *(bool*)arg1;
		    break;
		case MYSQL_OPT_BIND:
		...
		case MARIADB_OPT_CONNECTION_READ_ONLY:
		    OPT_SET_EXTENDED_VALUE_INT(&mysql->options, read_only, *(bool*)arg1);
		    break;
		...
		case MARIADB_OPT_IO_WAIT:
		    CHECK_OPT_EXTENSION_SET(&mysql->options);
		    mysql->options.extension->io_wait = (int (*)(my_socket, bool, int))arg1;
		    break;
		default:
		    va_end(ap);
	...
	switch(option) {
		case MYSQL_OPT_CONNECT_TIMEOUT:
		    *((uint*)arg) = mysql->options.connect_timeout;
		    break;
		case MYSQL_OPT_COMPRESS:
		    *((bool*)arg) = mysql->options.compress;
		    break;
		case MYSQL_OPT_NAMED_PIPE:
		    *((bool*)arg) = mysql->options.named_pipe;
		    break;
		case MYSQL_OPT_LOCAL_INFILE:    /* Allow LOAD DATA LOCAL ?*/
		...
		case MYSQL_SET_CHARSET_NAME:
		    ...
		    break;
		case MYSQL_OPT_RECONNECT:
		    *((bool*)arg) = mysql->options.reconnect;
		    break;
		case MYSQL_OPT_PROTOCOL:
		...
		case MYSQL_DEFAULT_AUTH:
		    *((char **)arg) = mysql->options.extension ? mysql->options.extension->default_auth : NULL;
		    break;
		case MYSQL_OPT_NONBLOCK:
		    *((bool*)arg) = test(mysql->options.extension && mysql->options.extension->async_context);
		    break;
		case MYSQL_OPT_CAN_HANDLE_EXPIRED_PASSWORDS:
		    *((bool*)arg) = test(mysql->options.client_flag & CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS);
		    break;
		case MYSQL_OPT_SSL_ENFORCE:
		    *((bool*)arg) = mysql->options.use_ssl;
		    break;
		case MYSQL_OPT_SSL_VERIFY_SERVER_CERT:
		    *((bool*)arg) = test(mysql->options.client_flag & CLIENT_SSL_VERIFY_SERVER_CERT);
		    break;
		case MYSQL_OPT_SSL_KEY:
		...
		case MARIADB_OPT_TLS_PASSPHRASE:
		    *((char **)arg) = mysql->options.extension ? mysql->options.extension->tls_pw : NULL;
		    break;
		case MARIADB_OPT_CONNECTION_READ_ONLY:
		    *((bool*)arg) = mysql->options.extension ? mysql->options.extension->read_only : 0;
		    break;
		case MARIADB_OPT_USERDATA:
		...
		case MARIADB_OPT_CONNECTION_HANDLER:
		    *((char **)arg) = mysql->options.extension ? mysql->options.extension->connection_handler : NULL;
		    break;
		case MARIADB_OPT_IO_WAIT:
		    *((int (**)(my_socket, bool, int))arg) = mysql->options.extension ? mysql->options.extension->io_wait : NULL;
		    break;
		default:
		    va_end(ap);
		    SET_CLIENT_ERROR(mysql, CR_NOT_IMPLEMENTED, SQLSTATE_UNKNOWN, 0);
...
void STDCALL mysql_server_end(void)
{
	...
	ma_init_done = 0;
#ifdef WIN32
	// @sobolev init_once = (INIT_ONCE)INIT_ONCE_STATIC_INIT;
	init_once.Ptr = 0; // @sobolev
#else
...
/*bool*/int mariadb_get_infov(MYSQL * mysql, enum mariadb_value value, void * arg, ...)
{
	...
	switch(value) {
		...
		case MARIADB_CONNECTION_ASYNC_TIMEOUT:
		    if(mysql && mysql->options.extension && mysql->options.extension->async_context) {
			    uint timeout = mysql->options.extension->async_context->timeout_value;
			    if(timeout > UINT_MAX - 999)
...
struct st_mariadb_methods MARIADB_DEFAULT_METHODS = {
	mthd_my_real_connect, /* open a connection */
	mysql_close_slow_part, /* close connection */
	mthd_my_send_cmd, /* send command to server */
	mthd_my_skip_result, /* skip result set */
	mthd_my_read_query_result, /* read response packet */
	mthd_my_read_rows, /* read all rows from a result set */
	mthd_my_read_one_row, /* read one/next row */
	mthd_supported_buffer_type, /* check if datatype is supported */
	mthd_stmt_read_prepare_response, /* read response packet from prepare */
	mthd_my_read_query_result, /* read response from stmt execute */
	mthd_stmt_get_result_metadata, /* get result set metadata for a prepared statement */
	mthd_stmt_get_param_metadata, /* get param metadata for a prepared statement */
	mthd_stmt_read_all_rows, /* read all rows (buffered) */
	mthd_stmt_fetch_row, /* fetch one row (unbuffered) */
	mthd_stmt_fetch_to_bind, /* store values in bind buffer */
	mthd_stmt_flush_unbuffered, /* skip unbuffered stmt result */
	my_set_error, /* set error */
	ma_invalidate_stmts, /* invalidate statements */
	&MARIADB_API /* API functions */
};
