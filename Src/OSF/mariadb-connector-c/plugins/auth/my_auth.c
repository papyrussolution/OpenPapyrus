...
static int dummy_fallback_auth_client(MYSQL_PLUGIN_VIO * vio, MYSQL * mysql __attribute__((unused)))
{
	...
	if(last_errno) {
		strnzcpy(last_error, ((MCPVIO_EXT*)vio)->mysql->net.last_error, sizeof(last_error));
	}
	...
	if(last_errno) {
		...
		strnzcpy(mysql->net.last_error, last_error, sizeof(mysql->net.last_error));
	}
	return CR_ERROR;
}
...
static int send_client_reply_packet(MCPVIO_EXT * mpvio, const uchar * data, int data_len)
{
	...
	if(mysql->options.ssl_key || mysql->options.ssl_cert || mysql->options.ssl_ca || mysql->options.ssl_capath ||
	    mysql->options.ssl_cipher || mysql->options.use_ssl || (mysql->options.client_flag & CLIENT_SSL_VERIFY_SERVER_CERT))
	...
	if(mysql->options.use_ssl && !(mysql->server_capabilities & CLIENT_SSL)) {
		if((mysql->client_flag & CLIENT_SSL_VERIFY_SERVER_CERT) || (mysql->options.extension && (mysql->options.extension->tls_fp ||
		    mysql->options.extension->tls_fp_list))) {
			my_set_error(mysql, CR_SSL_CONNECTION_ERROR, SQLSTATE_UNKNOWN, ER(CR_SSL_CONNECTION_ERROR), "SSL is required, but the server does not support it");
			goto error;
		}
	}
	// Remove options that server doesn't support 
	mysql->client_flag = mysql->client_flag & (~(CLIENT_COMPRESS | CLIENT_SSL | CLIENT_PROTOCOL_41) | mysql->server_capabilities);
	...
	if(data_len) {
		if(mysql->server_capabilities & CLIENT_SECURE_CONNECTION) {
			if(mysql->server_capabilities & CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA) {
				...
				// Without CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA capability password length is limited up to 255 chars 
				if(data_len > 0xFF)
					goto error;
				*end++ = data_len;
			}
 ...
 */
static int client_mpvio_write_packet(struct st_plugin_vio * mpv, const uchar * pkt, size_t pkt_len)
{
	int res;
	MCPVIO_EXT * mpvio = (MCPVIO_EXT*)mpv;
...
/**
   ...
   @param mysql       mysql
   @param data        pointer to the plugin auth data (scramble) in the handshake packet
   @param data_len    the length of the data
   @param data_plugin a plugin that data were prepared for or 0 if it's mysql_change_user()
