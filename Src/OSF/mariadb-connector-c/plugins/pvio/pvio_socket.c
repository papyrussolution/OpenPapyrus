...
#endif
/* Function prototypes */
bool pvio_socket_set_timeout(MARIADB_PVIO * pvio, enum enum_pvio_timeout type, int timeout);
...
int pvio_socket_wait_io_or_timeout(MARIADB_PVIO * pvio, bool is_read, int timeout);
int pvio_socket_blocking(MARIADB_PVIO * pvio, bool value, bool * old_value);
bool pvio_socket_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo);
bool pvio_socket_close(MARIADB_PVIO * pvio);
int pvio_socket_fast_send(MARIADB_PVIO * pvio);
int pvio_socket_keepalive(MARIADB_PVIO * pvio);
bool pvio_socket_get_handle(MARIADB_PVIO * pvio, void * handle);
bool pvio_socket_is_blocking(MARIADB_PVIO * pvio);
bool pvio_socket_is_alive(MARIADB_PVIO * pvio);
bool pvio_socket_has_data(MARIADB_PVIO * pvio, ssize_t * data_len);
...
static bool pvio_socket_initialized = FALSE;

static int pvio_socket_init(char * errmsg __attribute__((unused)), size_t errmsg_length __attribute__((unused)), 
	int unused __attribute__((unused)), va_list va __attribute__((unused)))
{
...
bool pvio_socket_change_timeout(MARIADB_PVIO * pvio, enum enum_pvio_timeout type, int timeout)
{
	struct timeval tm;
	int rc = 0;
...
int pvio_socket_get_timeout(MARIADB_PVIO * pvio, enum enum_pvio_timeout type)
{
	return pvio ? (pvio->timeout[type] / 1000) : -1;
}
...
ssize_t pvio_socket_read(MARIADB_PVIO * pvio, uchar * buffer, size_t length)
{
	...
	while((r = ma_recv(csock->socket, (uchar *)buffer, length, read_flags)) == -1) {
...
ssize_t pvio_socket_write(MARIADB_PVIO * pvio, const uchar * buffer, size_t length)
{
	...
	while((r = ma_send(csock->socket, (uchar *)buffer, length, send_flags)) == -1) {
		int err = socket_errno;
		if((err != SOCKET_EAGAIN
#ifdef HAVE_SOCKET_EWOULDBLOCK
		 && err != SOCKET_EWOULDBLOCK
...
int pvio_socket_wait_io_or_timeout(MARIADB_PVIO * pvio, bool is_read, int timeout)
{
	...
	if(!pvio || !pvio->data)
		return 0;
	if(pvio->mysql->options.extension && pvio->mysql->options.extension->io_wait != NULL) {
		my_socket handle;
	...
	csock = (struct st_pvio_socket *)pvio->data;
	{
		...
		rc = select(0, (is_read) ? &fds : NULL, (is_read) ? NULL : &fds, &exc_fds, (timeout >= 0) ? &tv : NULL);
		if(rc == SOCKET_ERROR) {
...
int pvio_socket_blocking(MARIADB_PVIO * pvio, bool block, bool * previous_mode)
{
	bool is_blocking;
	struct st_pvio_socket * csock;
	int new_fcntl_mode;
	...
	{
		ulong arg = block ? 0 : 1;
		if(ioctlsocket(csock->socket, FIONBIO, (u_long *)&arg)) {
...
static int pvio_socket_internal_connect(MARIADB_PVIO * pvio, const struct sockaddr * name, size_t namelen)
{
	int rc = 0;
	struct st_pvio_socket * csock = NULL;
	int timeout;
	...
	if(timeout != 0 && rc == -1 && errno == EINPROGRESS) {
			...
			return -1;
		{
			...
			if((rc = getsockopt(csock->socket, SOL_SOCKET, SO_ERROR, (char *)&error, &error_len)) < 0)
				return errno;
			else if(error)
				return error;
		}
...
bool pvio_socket_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo)
{
	...
	if(cinfo->type == PVIO_TYPE_UNIXSOCKET) {
		...
		memzero((char *)&UNIXaddr, sizeof(UNIXaddr));
...
/* todo: error, not supported */
		...
		// if client has multiple interfaces, we will bind socket to given bind_address 
		if(cinfo->mysql->options.bind_address) {
			...
			if(gai_rc != 0 || !bind_res) {
				PVIO_SET_ERROR(cinfo->mysql, CR_BIND_ADDR_FAILED, SQLSTATE_UNKNOWN, CER(CR_BIND_ADDR_FAILED), cinfo->mysql->options.bind_address, gai_rc);
				goto error;
			}
		}
		...
		if(gai_rc != 0 || !res) {
			PVIO_SET_ERROR(cinfo->mysql, CR_UNKNOWN_HOST, SQLSTATE_UNKNOWN, ER(CR_UNKNOWN_HOST), cinfo->host, gai_rc);
			if(bind_res)
...
bool pvio_socket_is_alive(MARIADB_PVIO * pvio)
{
	...
	csock = (struct st_pvio_socket *)pvio->data;
#ifndef _WIN32
	memzero(&poll_fd, sizeof(struct pollfd));
