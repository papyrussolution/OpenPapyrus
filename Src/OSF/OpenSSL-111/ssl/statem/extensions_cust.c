...
int custom_exts_copy(custom_ext_methods * dst, const custom_ext_methods * src)
{
	...
	if(src->meths_count > 0) {
		dst->meths = static_cast<custom_ext_method *>(OPENSSL_memdup(src->meths, sizeof(*src->meths) * src->meths_count));
		if(dst->meths == NULL)
			return 0;
    ...
    void * parse_arg)
{
	...
	if(custom_ext_find(exts, role, ext_type, NULL))
		return 0;
	tmp = static_cast<custom_ext_method *>(OPENSSL_realloc(exts->meths, (exts->meths_count + 1) * sizeof(custom_ext_method)));
	if(tmp == NULL)
		return 0;
	exts->meths = tmp;
    ...
    custom_ext_add_cb add_cb, custom_ext_free_cb free_cb, void * add_arg, custom_ext_parse_cb parse_cb, void * parse_arg)
{
	...
	custom_ext_parse_cb_wrap * parse_cb_wrap = static_cast<custom_ext_parse_cb_wrap *>(OPENSSL_malloc(sizeof(*parse_cb_wrap)));
	int ret;
	...
	ret = add_custom_ext_intern(ctx, role, ext_type, context, custom_ext_add_old_cb_wrap, custom_ext_free_old_cb_wrap,
		add_cb_wrap, custom_ext_parse_old_cb_wrap, parse_cb_wrap);
	if(!ret) {
		OPENSSL_free(add_cb_wrap);
		OPENSSL_free(parse_cb_wrap);
	}
