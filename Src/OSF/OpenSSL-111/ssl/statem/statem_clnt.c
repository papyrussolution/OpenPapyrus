...
int tls_construct_client_hello(SSL * s, WPACKET * pkt)
{
	...
	if(sess == NULL
		...
		if(s->hello_retry_request == ssl_st::SSL_HRR_NONE && !ssl_get_new_session(s, 0)) {
			/* SSLfatal() already called */
			return 0;
		}
	}
	...
	if(s->new_session || s->session->ssl_version == TLS1_3_VERSION) {
		if(s->version == TLS1_3_VERSION
			...
			if(s->hello_retry_request == ssl_st::SSL_HRR_NONE && RAND_bytes(s->tmp_session_id, sess_id_len) <= 0) {
...
int tls_process_cert_status_body(SSL * s, PACKET * pkt)
{
	...
	s->ext.ocsp.resp = static_cast<uchar *>(OPENSSL_malloc(resplen));
...
static int tls_construct_cke_psk_preamble(SSL * s, WPACKET * pkt)
{
	...
	if(psklen > PSK_MAX_PSK_LEN) {
		SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_F_TLS_CONSTRUCT_CKE_PSK_PREAMBLE, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	else if(psklen == 0) {
		SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE,
...
static int tls_construct_cke_dhe(SSL * s, WPACKET * pkt)
{
	...
	if(!WPACKET_sub_allocate_bytes_u16(pkt, BN_num_bytes(pub_key), &keybytes)) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_CKE_DHE, ERR_R_INTERNAL_ERROR);
		goto err;
	}
