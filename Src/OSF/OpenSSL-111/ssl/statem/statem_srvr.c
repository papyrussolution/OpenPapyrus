...
static int ssl_check_srp_ext_ClientHello(SSL * s)
{
	...
	if((s->s3->tmp.new_cipher->algorithm_mkey & SSL_kSRP) && (s->srp_ctx.TLS_ext_srp_username_callback)) {
		if(s->srp_ctx.login == NULL) {
			/*
			 * RFC 5054 says SHOULD reject, we do so if There is no srp
			 * login name
			 */
			SSLfatal(s, SSL_AD_UNKNOWN_PSK_IDENTITY, SSL_F_SSL_CHECK_SRP_EXT_CLIENTHELLO, SSL_R_PSK_IDENTITY_NOT_FOUND);
			return -1;
		}
		else {
...
int dtls_raw_hello_verify_request(WPACKET * pkt, uchar * cookie, size_t cookie_len)
{
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL * s, PACKET * pkt)
{
	...
	static const uchar null_compression = 0;
	...
	if(s->renegotiate == 0 && !SSL_IS_FIRST_HANDSHAKE(s)) {
		...
		if((s->options & SSL_OP_NO_RENEGOTIATION) != 0 || (!s->s3->send_connection_binding && (s->options & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION) == 0)) {
	...
	clienthello = static_cast<CLIENTHELLO_MSG *>(OPENSSL_zalloc(sizeof(*clienthello)));
	if(clienthello == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_CLIENT_HELLO, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	/*
	...
	if(clienthello->isv2) {
		...
		if(!SSL_IS_FIRST_HANDSHAKE(s) || s->hello_retry_request != ssl_st::SSL_HRR_NONE) {
			SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_TLS_PROCESS_CLIENT_HELLO, SSL_R_UNEXPECTED_MESSAGE);
			goto err;
		}
		/*-
	...
	if(!PACKET_get_net_2(pkt, &clienthello->legacy_version)) {
		SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PROCESS_CLIENT_HELLO, SSL_R_LENGTH_TOO_SHORT);
		goto err;
	}
	...
	if(clienthello->isv2) {
		 ...
		 */
		uint ciphersuite_len, session_id_len, challenge_len;
		PACKET challenge;
		...
		if(!PACKET_copy_bytes(pkt, clienthello->random, SSL3_RANDOM_SIZE) || !PACKET_get_length_prefixed_1(pkt, &session_id)
		   || !PACKET_copy_all(&session_id, clienthello->session_id, SSL_MAX_SSL_SESSION_ID_LENGTH, &clienthello->session_id_len)) {
			SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PROCESS_CLIENT_HELLO, SSL_R_LENGTH_MISMATCH);
			goto err;
		}
...
static int tls_handle_status_request(SSL * s)
{
	 ...
	 */
	if(s->ext.status_type != TLSEXT_STATUSTYPE_nothing && s->ctx && s->ctx->ext.status_cb) {
		int ret;
...
int tls_handle_alpn(SSL * s)
{
	...
	if(s->ctx->ext.alpn_select_cb && s->s3->alpn_proposed) {
	...
	/* Check ALPN is consistent with session */
	if(s->session->ext.alpn_selected) {
		/* Not consistent so can't be used for early_data */
		s->ext.early_data_ok = 0;
	}
...
int tls_construct_server_hello(SSL * s, WPACKET * pkt)
{
	...
	if(!WPACKET_put_bytes_u16(pkt, version)
	     ...
	     */
	   || !WPACKET_memcpy(pkt, s->hello_retry_request == ssl_st::SSL_HRR_PENDING ? hrrrandom : s->s3->server_random, SSL3_RANDOM_SIZE)) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_SERVER_HELLO, ERR_R_INTERNAL_ERROR);
		return 0;
	}
	...
	if(!tls_construct_extensions(s, pkt, s->hello_retry_request == ssl_st::SSL_HRR_PENDING ? SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST : (SSL_IS_TLS13(s)
	    ? SSL_EXT_TLS1_3_SERVER_HELLO : SSL_EXT_TLS1_2_SERVER_HELLO),
	    NULL, 0)) {
		/* SSLfatal() already called */
		return 0;
	}
...
int tls_construct_server_key_exchange(SSL * s, WPACKET * pkt)
{
	...
	if(type & (SSL_kDHE | SSL_kDHEPSK)) {
		...
		if((pkdhp == NULL) && (s->cert->dh_tmp_cb)) {
		...
		if(pkdhp == NULL) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE, SSL_R_MISSING_TMP_DH_KEY);
			goto err;
		}
		if(!ssl_security(s, SSL_SECOP_TMP_DH,
		    EVP_PKEY_security_bits(pkdhp), 0, pkdhp)) {
			SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_F_TLS_CONSTRUCT_SERVER_KEY_EXCHANGE, SSL_R_DH_KEY_TOO_SMALL);
			goto err;
		}
		if(s->s3->tmp.pkey) {
...
int tls_construct_certificate_request(SSL * s, WPACKET * pkt)
{
	if(SSL_IS_TLS13(s)) {
		...
		if(s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
			...
			if((s->pha_context = static_cast<uint8_t *>(OPENSSL_malloc(s->pha_context_len))) == NULL
...
static int tls_process_cke_psk_preamble(SSL * s, PACKET * pkt)
{
	...
	s->s3->tmp.psk = static_cast<uchar *>(OPENSSL_memdup(psk, psklen));
...
static int tls_process_cke_rsa(SSL * s, PACKET * pkt)
{
	...
	uchar rand_premaster_secret[SSL_MAX_MASTER_KEY_LENGTH];
	int decrypt_len;
	uchar decrypt_good, version_good;
	...
	rsa_decrypt = static_cast<uchar *>(OPENSSL_malloc(RSA_size(rsa)));
	if(rsa_decrypt == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS_PROCESS_CKE_RSA,
...
MSG_PROCESS_RETURN tls_process_client_certificate(SSL * s, PACKET * pkt)
{
	...
	if(SSL_IS_TLS13(s) && (!PACKET_get_length_prefixed_1(pkt, &context) || (s->pha_context == NULL && PACKET_remaining(&context) != 0)
	   || (s->pha_context && !PACKET_equal(&context, s->pha_context, s->pha_context_len)))) {
	...
	for(chainidx = 0; PACKET_remaining(&spkt) > 0; chainidx++) {
		...
		certstart = certbytes;
		x = d2i_X509(NULL, (const uchar **)&certbytes, l);
		if(!x) {
    ...
    uchar * tick_nonce)
{
	...
	senc = static_cast<uchar *>(OPENSSL_malloc(slen_full));
	if(senc == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR,
...
{
	SSL_CTX * tctx = s->session_ctx;
	uchar tick_nonce[TICKET_NONCE_SIZE];
	union {
		uchar age_add_c[sizeof(uint32_t)];
		uint32_t age_add;
	} age_add_u;
	...
	if(tctx->generate_ticket_cb && tctx->generate_ticket_cb(s, tctx->ticket_cb_data) == 0)
		goto err;
