...
static hm_fragment * dtls1_hm_fragment_new(size_t frag_len, int reassembly)
{
	...
	if(reassembly) {
		bitmask = static_cast<uchar *>(OPENSSL_zalloc(RSMBLY_BITMASK_SIZE(frag_len)));
		if(bitmask == NULL) {
			SSLerr(SSL_F_DTLS1_HM_FRAGMENT_NEW, ERR_R_MALLOC_FAILURE);
			OPENSSL_free(buf);
			OPENSSL_free(frag);
...
int dtls1_get_queue_priority(ushort seq, int is_ccs)
{
	/*
	 ...
	 * priority queues) and fits in the ushort variable.
	 */
	return seq * 2 - is_ccs;
}
...
int dtls1_retransmit_buffered_messages(SSL * s)
{
	...
	for(item = pqueue_next(&iter); item != NULL; item = pqueue_next(&iter)) {
		frag = (hm_fragment*)item->data;
		if(dtls1_retransmit_message(s, (ushort)
...
int dtls1_buffer_message(SSL * s, int is_ccs)
{
	...
	if(is_ccs) {
		...
		if(!ossl_assert(s->d1->w_msg_hdr.msg_len + ((s->version == DTLS1_BAD_VER) ? 3 : DTLS1_CCS_HEADER_LENGTH) == (uint)s->init_num))
			return 0;
	}
	else {
		if(!ossl_assert(s->d1->w_msg_hdr.msg_len + DTLS1_HM_HEADER_LENGTH == (uint)s->init_num))
			return 0;
	}
...
int dtls1_retransmit_message(SSL * s, ushort seq, int * found)
{
	int ret;
