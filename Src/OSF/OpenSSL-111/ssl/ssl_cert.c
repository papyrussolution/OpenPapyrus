...
{
	CERT * ret = static_cast<CERT *>(OPENSSL_zalloc(sizeof(*ret)));
	if(ret == NULL) {
...
CERT * ssl_cert_dup(CERT * cert)
{
	...
	for(i = 0; i < SSL_PKEY_NUM; i++) {
		...
		if(cert->pkeys[i].serverinfo != NULL) {
			...
			ret->pkeys[i].serverinfo = static_cast<uchar *>(OPENSSL_malloc(cert->pkeys[i].serverinfo_length));
	...
	if(cert->conf_sigalgs) {
		ret->conf_sigalgs = static_cast<uint16_t *>(OPENSSL_malloc(cert->conf_sigalgslen * sizeof(*cert->conf_sigalgs)));
	...
	if(cert->client_sigalgs) {
		ret->client_sigalgs = static_cast<uint16_t *>(OPENSSL_malloc(cert->client_sigalgslen * sizeof(*cert->client_sigalgs)));
	...
	if(cert->ctype) {
		ret->ctype = static_cast<uint8_t *>(OPENSSL_memdup(cert->ctype, cert->ctype_len));
		if(ret->ctype == NULL)
			goto err;
    ...
    void * ex)
{
	...
	switch(op) {
	    ...
	    {
		    const SSL_CIPHER * c = static_cast<const SSL_CIPHER *>(other);
