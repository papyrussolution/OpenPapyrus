...
{
	DTLS_RECORD_LAYER * d;
	if((d = static_cast<DTLS_RECORD_LAYER *>(OPENSSL_malloc(sizeof(*d)))) == NULL) {
...
int dtls1_buffer_record(SSL * s, record_pqueue * queue, uchar * priority)
{
	...
	if(pqueue_size(queue->q) >= 100)
		return 0;
	rdata = static_cast<DTLS1_RECORD_DATA *>(OPENSSL_malloc(sizeof(*rdata)));
	...
	switch(SSL3_RECORD_get_type(rr)) {
		...
		case SSL3_RT_APPLICATION_DATA:
		    ...
		    if(s->s3->in_read_app_data && (s->s3->total_renegotiations != 0) && ossl_statem_app_data_allowed(s)) {
			    ...
			    SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_DTLS1_READ_BYTES, SSL_R_UNEXPECTED_RECORD);
			    return -1;
		    }
	}
	/* not reached */
...
int dtls1_write_bytes(SSL * s, int type, const void * buf, size_t len, size_t * written)
{
	...
	if(!ossl_assert(len <= SSL3_RT_MAX_PLAIN_LENGTH)) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DTLS1_WRITE_BYTES, ERR_R_INTERNAL_ERROR);
		return -1;
	}
	s->rwstate = SSL_NOTHING;
	i = do_dtls1_write(s, type, static_cast<const uchar *>(buf), len, 0, written);
	return i;
}
