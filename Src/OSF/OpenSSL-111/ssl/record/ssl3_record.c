...
static const uchar ssl3_pad_1[48] = {
...
static const uchar ssl3_pad_2[48] = {
...
void SSL3_RECORD_set_seq_num(SSL3_RECORD * r, const uchar * seq_num)
{
...
static int ssl3_record_app_data_waiting(SSL * s)
{
	...
	uchar * p = SSL3_BUFFER_get_buf(rbuf);
	if(!p)
		return 0;
...
int early_data_count_ok(SSL * s, size_t length, size_t overhead, int send)
{
	...
	if(!s->server && sess->ext.max_early_data == 0) {
		if(!ossl_assert(s->psksession != NULL && s->psksession->ext.max_early_data > 0)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_EARLY_DATA_COUNT_OK, ERR_R_INTERNAL_ERROR);
			return 0;
		}
		sess = s->psksession;
	}
	if(!s->server)
		...
		max_early_data = s->recv_max_early_data < sess->ext.max_early_data ? s->recv_max_early_data : sess->ext.max_early_data;
	if(max_early_data == 0) {
...
int ssl3_get_record(SSL * s)
{
	...
	do {
		...
		if((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
			...
			if(s->server && RECORD_LAYER_is_first_record(&s->rlayer)
			 ...
			 && (type == SSL2_MT_CLIENT_HELLO)) {
				 ...
				 * that explicitly
				 */
				if(!s->first_packet && !SSL_IS_TLS13(s) && s->hello_retry_request != ssl_st::SSL_HRR_PENDING && version != (uint)s->version) {
					if((s->version & 0xFF00) == (version & 0xFF00) && !s->enc_write_ctx && !s->write_hash) {
						if(thisrr->type == SSL3_RT_ALERT) {
							 ...
							 * end.
							 */
							SSLfatal(s, SSL_AD_NO_ALERT, SSL_F_SSL3_GET_RECORD, SSL_R_WRONG_VERSION_NUMBER);
							return -1;
						}
						/*
						 * Send back error using their minor version number :-)
						 */
						s->version = (ushort)version;
					}
				...
				if((version >> 8) != SSL3_VERSION_MAJOR) {
					if(RECORD_LAYER_is_first_record(&s->rlayer)) {
						...
						SSLfatal(s, SSL_AD_PROTOCOL_VERSION, SSL_F_SSL3_GET_RECORD, SSL_R_WRONG_VERSION_NUMBER);
						return -1;
					}
				}
				if(SSL_IS_TLS13(s) && s->enc_read_ctx) {
					if(thisrr->type != SSL3_RT_APPLICATION_DATA && (thisrr->type != SSL3_RT_CHANGE_CIPHER_SPEC || !SSL_IS_FIRST_HANDSHAKE(s)) && 
						(thisrr->type != SSL3_RT_ALERT || s->statem.enc_read_state != ENC_READ_STATE_ALLOW_PLAIN_ALERTS)) {
						SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_GET_RECORD, SSL_R_BAD_RECORD_TYPE);
						return -1;
					}
					if(thisrr->rec_version != TLS1_2_VERSION) {
						SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_SSL3_GET_RECORD, SSL_R_WRONG_VERSION_NUMBER);
						return -1;
					}
				}
				if(thisrr->length > SSL3_BUFFER_get_len(rbuf) - SSL3_RT_HEADER_LENGTH) {
					SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_F_SSL3_GET_RECORD, SSL_R_PACKET_LENGTH_TOO_LONG);
					return -1;
				}
			}
		...
		if(SSL_IS_TLS13(s)) {
			if(thisrr->length > SSL3_RT_MAX_TLS13_ENCRYPTED_LENGTH) {
				SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_F_SSL3_GET_RECORD, SSL_R_ENCRYPTED_LENGTH_TOO_LONG);
				return -1;
			}
		}
		else {
	...
	} while(num_recs < max_recs && thisrr->type == SSL3_RT_APPLICATION_DATA && SSL_USE_EXPLICIT_IV(s)
	 && s->enc_read_ctx && (EVP_CIPHER_flags(EVP_CIPHER_CTX_cipher(s->enc_read_ctx)) & EVP_CIPH_FLAG_PIPELINE)
	 && ssl3_record_app_data_waiting(s));
	if(num_recs == 1 && thisrr->type == SSL3_RT_CHANGE_CIPHER_SPEC && (SSL_IS_TLS13(s) || s->hello_retry_request != ssl_st::SSL_HRR_NONE) && SSL_IS_FIRST_HANDSHAKE(s)) {
		/*
		 * CCS messages must be exactly 1 byte long, containing the value 0x01
		 */
		if(thisrr->length != 1 || thisrr->data[0] != 0x01) {
			SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_F_SSL3_GET_RECORD, SSL_R_INVALID_CCS_MESSAGE);
			return -1;
		}
		/*
		...
		if(RECORD_LAYER_get_empty_record_count(&s->rlayer) > MAX_EMPTY_RECORDS) {
			SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_GET_RECORD, SSL_R_UNEXPECTED_CCS_MESSAGE);
			return -1;
		}
		thisrr->read = 1;
	...
	if(SSL_READ_ETM(s) && s->read_hash) {
		...
		if(!ossl_assert(imac_size >= 0 && imac_size <= EVP_MAX_MD_SIZE)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_GET_RECORD, ERR_LIB_EVP);
			return -1;
		}
		...
		for(j = 0; j < num_recs; j++) {
			...
			if(thisrr->length < mac_size) {
				SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_SSL3_GET_RECORD, SSL_R_LENGTH_TOO_SHORT);
				return -1;
			}
	...
	printf("dec %lu\n", (ulong)rr[0].length);
	{
		for(size_t z = 0; z < rr[0].length; z++)
	...
	if(sess && s->enc_read_ctx && (!SSL_READ_ETM(s) && EVP_MD_CTX_md(s->read_hash))) {
		...
		if(!ossl_assert(mac_size <= EVP_MAX_MD_SIZE)) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_GET_RECORD, ERR_R_INTERNAL_ERROR);
			return -1;
		}
		for(j = 0; j < num_recs; j++) {
			thisrr = &rr[j];
			...
			if(thisrr->orig_len < mac_size ||
			    ...
			    (EVP_CIPHER_CTX_mode(s->enc_read_ctx) == EVP_CIPH_CBC_MODE && thisrr->orig_len < mac_size + 1)) {
				SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_SSL3_GET_RECORD, SSL_R_LENGTH_TOO_SHORT);
				return -1;
			}
	...
	for(j = 0; j < num_recs; j++) {
		...
		if(s->expand) {
			if(thisrr->length > SSL3_RT_MAX_COMPRESSED_LENGTH) {
				SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_F_SSL3_GET_RECORD, SSL_R_COMPRESSED_LENGTH_TOO_LONG);
				return -1;
			}
			if(!ssl3_do_uncompress(s, thisrr)) {
				SSLfatal(s, SSL_AD_DECOMPRESSION_FAILURE, SSL_F_SSL3_GET_RECORD, SSL_R_BAD_DECOMPRESSION);
				return -1;
			}
		}
		if(SSL_IS_TLS13(s) && s->enc_read_ctx && thisrr->type != SSL3_RT_ALERT) {
			size_t end;
		...
		if(SSL_IS_TLS13(s) && (thisrr->type == SSL3_RT_HANDSHAKE || thisrr->type == SSL3_RT_ALERT) && thisrr->length == 0) {
			SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_GET_RECORD, SSL_R_BAD_LENGTH);
			return -1;
		}
		if(thisrr->length > SSL3_RT_MAX_PLAIN_LENGTH) {
			SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_F_SSL3_GET_RECORD, SSL_R_DATA_LENGTH_TOO_LONG);
			return -1;
		}
		/* If received packet overflows current Max Fragment Length setting */
		if(s->session && USE_MAX_FRAGMENT_LENGTH_EXT(s->session) && thisrr->length > GET_MAX_FRAGMENT_LENGTH(s->session)) {
			SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_F_SSL3_GET_RECORD, SSL_R_DATA_LENGTH_TOO_LONG);
			return -1;
		}
		thisrr->off = 0;
...
int ssl3_enc(SSL * s, SSL3_RECORD * inrecs, size_t n_recs, int sending)
{
	...
	if((s->session == NULL) || (ds == NULL) || (enc == NULL)) {
		...
		/* TODO(size_t): Convert this call */
		if(EVP_Cipher(ds, rec->data, rec->input, (uint)l) < 1)
			return -1;
		if(EVP_MD_CTX_md(s->read_hash)) {
			/* TODO(size_t): convert me */
			imac_size = EVP_MD_CTX_size(s->read_hash);
			if(imac_size < 0) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_ENC, ERR_R_INTERNAL_ERROR);
				return -1;
			}
			mac_size = (size_t)imac_size;
		}
...
int tls1_enc(SSL * s, SSL3_RECORD * recs, size_t n_recs, int sending)
{
	...
	uchar buf[SSL_MAX_PIPELINES][EVP_AEAD_TLS1_AAD_LEN];
	...
	const EVP_CIPHER * enc;
	if(n_recs == 0) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_TLS1_ENC, ERR_R_INTERNAL_ERROR);
		return 0;
	}
...
int n_ssl3_mac(SSL * ssl, SSL3_RECORD * rec, uchar * md, int sending)
{
	uchar * mac_sec, * seq;
...
int tls1_mac(SSL * ssl, SSL3_RECORD * rec, uchar * md, int sending)
{
	uchar * seq;
	EVP_MD_CTX * hash;
	size_t md_size;
    ...
    size_t block_size, size_t mac_size)
{
	...
	for(i = 0; i < to_check; i++) {
		uchar mask = constant_time_ge_8_s(padding_length, i);
		uchar b = rec->data[rec->length - 1 - i];
		/*
...
int ssl3_cbc_copy_mac(uchar * out,
    const SSL3_RECORD * rec, size_t md_size)
{
#if defined(CBC_MAC_ROTATE_IN_PLACE)
	uchar rotated_mac_buf[64 + EVP_MAX_MD_SIZE];
	uchar * rotated_mac;
#else
	uchar rotated_mac[EVP_MAX_MD_SIZE];
#endif
	...
	for(i = 0; i < md_size; i++) {
		...
		((volatile uchar *)rotated_mac)[rotate_offset ^ 32];
...
#endif
	/* r->length is now the compressed data plus mac */
	if(sess && !SSL_READ_ETM(s) && s->enc_read_ctx && EVP_MD_CTX_md(s->read_hash)) {
		/* s->read_hash != NULL => mac_size != -1 */
		uchar * mac = NULL;
		uchar mac_tmp[EVP_MAX_MD_SIZE];
		/* TODO(size_t): Convert this to do size_t properly */
...
again:
	...
	if((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
		...
		if(s->session && USE_MAX_FRAGMENT_LENGTH_EXT(s->session) && rr->length > GET_MAX_FRAGMENT_LENGTH(s->session)) {
			/* record too long, silently discard it */
			rr->length = 0;
			rr->read = 1;
			RECORD_LAYER_reset_packet_length(&s->rlayer);
...
{
	SSL3_RECORD * rr = RECORD_LAYER_get_rrec(&s->rlayer);
