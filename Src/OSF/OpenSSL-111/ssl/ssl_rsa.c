...
#define  SYNTHV1CONTEXT     (SSL_EXT_TLS1_2_AND_BELOW_ONLY|SSL_EXT_CLIENT_HELLO|SSL_EXT_TLS1_2_SERVER_HELLO|SSL_EXT_IGNORE_ON_RESUMPTION)
    ...
    size_t serverinfo_length)
{
	...
	new_serverinfo = static_cast<uchar *>(OPENSSL_realloc(ctx->cert->key->serverinfo, serverinfo_length));
...
int SSL_CTX_use_serverinfo_file(SSL_CTX * ctx, const char * file)
{
	...
	for(num_extensions = 0;; num_extensions++) {
		...
		tmp = static_cast<uchar *>(OPENSSL_realloc(serverinfo, serverinfo_length + extension_length + contextoff));
		if(tmp == NULL) {
