    ...
    int * groups, size_t ngroups)
{
	...
	if((glist = static_cast<uint16_t *>(OPENSSL_malloc(ngroups * sizeof(*glist)))) == NULL) {
...
{
	nid_cb_st * narg = static_cast<nid_cb_st *>(arg);
	size_t i;
	int nid;
	char etmp[20];
	if(!elem)
...
int tls1_check_group_id(SSL * s, uint16_t group_id, int check_own_groups)
{
	...
	if(tls1_suiteb(s) && s->s3->tmp.new_cipher) {
		ulong cid = s->s3->tmp.new_cipher->id;
		if(cid == TLS1_CK_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256) {
	...
	if(idx == -1) {
		if(s->server) {
			...
			if(idx == SSL_PKEY_GOST01 && s->s3->tmp.new_cipher->algorithm_auth != SSL_aGOST01) {
				for(int real_idx = SSL_PKEY_GOST12_512; real_idx >= SSL_PKEY_GOST01; real_idx--) {
    ...
    size_t sesslen, SSL_SESSION ** psess)
{
	...
	sdec = static_cast<uchar *>(OPENSSL_malloc(eticklen));
	 ...
	 * performs any action
	 */
	if(s->session_ctx->decrypt_ticket_cb && (ret == SSL_TICKET_EMPTY || ret == SSL_TICKET_NO_DECRYPT || ret == SSL_TICKET_SUCCESS || ret == SSL_TICKET_SUCCESS_RENEW)) {
...
static int tls1_set_shared_sigalgs(SSL * s)
{
	...
	if(nmatch) {
		if((salgs = static_cast<const SIGALG_LOOKUP **>(OPENSSL_malloc(nmatch * sizeof(*salgs)))) == NULL) {
...
int tls1_save_u16(PACKET * pkt, uint16_t ** pdest, size_t * pdestlen)
{
	...
	if((buf = static_cast<uint16_t *>(OPENSSL_malloc(size * sizeof(*buf)))) == NULL) {
...
int SSL_get_sigalgs(SSL * s, int idx, int * psign, int * phash, int * psignhash, uchar * rsig, uchar * rhash)
{
	...
	if(idx >= 0) {
		...
		if(psign)
			*psign = lu ? lu->sig : NID_undef;
		if(phash)
			*phash = lu ? lu->hash : NID_undef;
		if(psignhash)
			*psignhash = lu ? lu->sigandhash : NID_undef;
	}
...
{
	sig_cb_st * sarg = static_cast<sig_cb_st *>(arg);
	size_t i;
	const SIGALG_LOOKUP * s;
...
{
	uint16_t * sigalgs;
	if((sigalgs = static_cast<uint16_t *>(OPENSSL_malloc(salglen * sizeof(*sigalgs)))) == NULL) {
...
int tls1_set_sigalgs(CERT * c, const int * psig_nids, size_t salglen, int client)
{
	...
	if(salglen & 1)
		return 0;
	if((sigalgs = static_cast<uint16_t *>(OPENSSL_malloc((salglen / 2) * sizeof(*sigalgs)))) == NULL) {
    ...
    int idx)
{
	...
	if(TLS1_get_version(s) >= TLS1_2_VERSION && strict_mode) {
		int default_nid;
		int rsign = 0;
		if(s->s3->tmp.peer_cert_sigalgs || s->s3->tmp.peer_sigalgs) {
			default_nid = 0;
    ...
    EVP_PKEY * pkey)
{
	...
	ERR_pop_to_mark();
	if(s->s3->tmp.peer_cert_sigalgs) {
		for(i = 0; i < s->s3->tmp.peer_cert_sigalgslen; i++) {
			lu = tls1_lookup_sigalg(s->s3->tmp.peer_cert_sigalgs[i]);
			if(lu == NULL || !X509_get_signature_info(x, &mdnid, &pknid, NULL, NULL))
				continue;
			/*
...
static const SIGALG_LOOKUP * find_sig_alg(SSL * s, X509 * x, EVP_PKEY * pkey)
{
	...
	for(i = 0; i < s->shared_sigalgslen; i++) {
		...
		if(!tls1_lookup_md(lu, NULL))
			continue;
		if((pkey == NULL && !has_usable_cert(s, lu, -1)) || (pkey && !is_cert_usable(s, lu, x, pkey)))
			continue;
		tmppkey = pkey ? pkey : s->cert->pkeys[lu->sig_idx].privatekey;
		if(lu->sig == EVP_PKEY_EC) {
#ifndef OPENSSL_NO_EC
			if(curve == -1) {
			...
			if(!rsa_pss_check_min_key_size(static_cast<const RSA *>(EVP_PKEY_get0(tmppkey)), lu))
				continue;
		}
		break;
	}
			...
			if(s->s3->tmp.peer_sigalgs) {
				...
				for(i = 0; i < s->shared_sigalgslen; i++) {
					...
					if(lu->sig == EVP_PKEY_RSA_PSS) {
						...
						if(!rsa_pss_check_min_key_size(static_cast<const RSA *>(EVP_PKEY_get0(pkey)), lu))
							continue;
					}
#ifndef OPENSSL_NO_EC
