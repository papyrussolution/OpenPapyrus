...
SSL_SESSION * SSL_SESSION_new(void)
{
	...
	ss = static_cast<SSL_SESSION *>(OPENSSL_zalloc(sizeof(*ss)));
	if(ss == NULL) {
...
int SSL_SESSION_set1_alpn_selected(SSL_SESSION * s, const uchar * alpn, size_t len)
{
	...
	s->ext.alpn_selected = static_cast<uchar *>(OPENSSL_memdup(alpn, len));
...
int SSL_set_session_ticket_ext(SSL * s, void * ext_data, int ext_len)
{
	if(s->version >= TLS1_VERSION) {
		...
		s->ext.session_ticket = static_cast<TLS_SESSION_TICKET_EXT *>(OPENSSL_malloc(sizeof(TLS_SESSION_TICKET_EXT) + ext_len));
