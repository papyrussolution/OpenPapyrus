...
{
	SSL_CONF_CTX * cctx = static_cast<SSL_CONF_CTX *>(usr);
	size_t i;
	const ssl_flag_tbl * tbl;
	int onoff = 1;
	/*
...
static const ssl_conf_cmd_tbl * ssl_conf_cmd_lookup(SSL_CONF_CTX * cctx, const char * cmd)
{
	...
	for(i = 0, t = ssl_conf_cmds; i < OSSL_NELEM(ssl_conf_cmds); i++, t++) {
		if(ssl_conf_cmd_allowed(cctx, t)) {
			...
			if(cctx->flags & SSL_CONF_FLAG_FILE) {
				if(t->str_file && sstreqi_ascii(t->str_file, cmd))
					return t;
			}
		}
	}
...
{
	SSL_CONF_CTX * ret = static_cast<SSL_CONF_CTX *>(OPENSSL_zalloc(sizeof(*ret)));
	return ret;
}
