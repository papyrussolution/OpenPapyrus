...
{
	SSL3_STATE * s3;
	if((s3 = static_cast<SSL3_STATE *>(OPENSSL_zalloc(sizeof(*s3)))) == NULL)
		goto err;
	s->s3 = s3;
...
long ssl3_ctrl(SSL * s, int cmd, long larg, void * parg)
{
	...
	switch(cmd) {
		...
		case SSL_CTRL_SET_TLSEXT_STATUS_REQ_EXTS:
		    s->ext.ocsp.exts = static_cast<X509_EXTENSIONS *>(parg);
		    ret = 1;
		    break;
		...
		case SSL_CTRL_SET_TLSEXT_STATUS_REQ_IDS:
		    s->ext.ocsp.ids = static_cast<STACK_OF(OCSP_RESPID) *>(parg);
		    ret = 1;
		    break;
		...
		case SSL_CTRL_SET_TLSEXT_STATUS_REQ_OCSP_RESP:
		    ...
		    s->ext.ocsp.resp = static_cast<uchar *>(parg);
		...
		case SSL_CTRL_SET_GROUPS: return tls1_set_groups(&s->ext.supportedgroups, &s->ext.supportedgroups_len, static_cast<int *>(parg), larg);
		case SSL_CTRL_SET_GROUPS_LIST: return tls1_set_groups_list(&s->ext.supportedgroups, &s->ext.supportedgroups_len, static_cast<const char *>(parg));
...
#endif
		case SSL_CTRL_SET_SIGALGS: return tls1_set_sigalgs(s->cert, static_cast<const int *>(parg), larg, 0);
		case SSL_CTRL_SET_SIGALGS_LIST: return tls1_set_sigalgs_list(s->cert, static_cast<const char *>(parg), 0);
		case SSL_CTRL_SET_CLIENT_SIGALGS: return tls1_set_sigalgs(s->cert, static_cast<const int *>(parg), larg, 1);
		...
		case SSL_CTRL_SET_CLIENT_CERT_TYPES:
		    if(!s->server)
			    return 0;
		    return ssl3_set_req_cert_type(s->cert, static_cast<const uchar *>(parg), larg);
		case SSL_CTRL_BUILD_CERT_CHAIN: return ssl_build_cert_chain(s, NULL, larg);
		case SSL_CTRL_SET_VERIFY_CERT_STORE: return ssl_cert_set_cert_store(s->cert, static_cast<X509_STORE *>(parg), 0, larg);
		case SSL_CTRL_SET_CHAIN_CERT_STORE: return ssl_cert_set_cert_store(s->cert, static_cast<X509_STORE *>(parg), 1, larg);
	...
	switch(cmd) {
		...
		case SSL_CTRL_GET_TLSEXT_TICKET_KEYS:
	    {
		    ...
		    if(cmd == SSL_CTRL_SET_TLSEXT_TICKET_KEYS) {
			    memcpy(ctx->ext.tick_key_name, keys, sizeof(ctx->ext.tick_key_name));
			    memcpy(ctx->ext.secure->tick_hmac_key, keys + sizeof(ctx->ext.tick_key_name),
				sizeof(ctx->ext.secure->tick_hmac_key));
			    memcpy(ctx->ext.secure->tick_aes_key, keys + sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key),
				sizeof(ctx->ext.secure->tick_aes_key));
		    }
		    else {
			    memcpy(keys, ctx->ext.tick_key_name, sizeof(ctx->ext.tick_key_name));
			    memcpy(keys + sizeof(ctx->ext.tick_key_name), ctx->ext.secure->tick_hmac_key, sizeof(ctx->ext.secure->tick_hmac_key));
			    memcpy(keys + sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key),
				ctx->ext.secure->tick_aes_key, sizeof(ctx->ext.secure->tick_aes_key));
		    }
		    return 1;
	    }
		case SSL_CTRL_GET_TLSEXT_STATUS_REQ_TYPE:
...
#ifndef OPENSSL_NO_EC
		case SSL_CTRL_SET_GROUPS: return tls1_set_groups(&ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, static_cast<int *>(parg), larg);
		case SSL_CTRL_SET_GROUPS_LIST: return tls1_set_groups_list(&ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, static_cast<const char *>(parg));
#endif
		case SSL_CTRL_SET_SIGALGS: return tls1_set_sigalgs(ctx->cert, static_cast<const int *>(parg), larg, 0);
		case SSL_CTRL_SET_SIGALGS_LIST: return tls1_set_sigalgs_list(ctx->cert, static_cast<const char *>(parg), 0);
		case SSL_CTRL_SET_CLIENT_SIGALGS: return tls1_set_sigalgs(ctx->cert, static_cast<const int *>(parg), larg, 1);
		case SSL_CTRL_SET_CLIENT_SIGALGS_LIST: return tls1_set_sigalgs_list(ctx->cert, static_cast<const char *>(parg), 1);
		case SSL_CTRL_SET_CLIENT_CERT_TYPES: return ssl3_set_req_cert_type(ctx->cert, static_cast<const uchar *>(parg), larg);
		case SSL_CTRL_BUILD_CERT_CHAIN: return ssl_build_cert_chain(NULL, ctx, larg);
		case SSL_CTRL_SET_VERIFY_CERT_STORE: return ssl_cert_set_cert_store(ctx->cert, static_cast<X509_STORE *>(parg), 0, larg);
		case SSL_CTRL_SET_CHAIN_CERT_STORE: return ssl_cert_set_cert_store(ctx->cert, static_cast<X509_STORE *>(parg), 1, larg);
...
long ssl3_ctx_callback_ctrl(SSL_CTX * ctx, int cmd, void (*fp)(void))
{
	switch(cmd) {
		...
		case SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB:
		    ctx->ext.ticket_key_cb = (int (*)(SSL *, uchar *, uchar *, EVP_CIPHER_CTX *, HMAC_CTX *, int))fp;
		    break;
#ifndef OPENSSL_NO_SRP
 ...
 */
const SSL_CIPHER * ssl3_choose_cipher(SSL * s, STACK_OF(SSL_CIPHER) * clnt, STACK_OF(SSL_CIPHER) * srvr)
{
...
static int ssl3_set_req_cert_type(CERT * c, const uchar * p, size_t len)
{
	...
	if(len > 0xff)
		return 0;
	c->ctype = static_cast<uint8_t *>(OPENSSL_memdup(p, len));
	if(c->ctype == NULL)
		return 0;
	c->ctype_len = len;
	return 1;
...
int ssl3_write(SSL * s, const void * buf, size_t len, size_t * written)
{
	...
	return s->method->ssl_write_bytes(s, SSL3_RT_APPLICATION_DATA, buf, len, written);
}

static int ssl3_read_internal(SSL * s, void * buf, size_t len, int peek, size_t * readbytes)
{
	int ret;
	clear_sys_error();
	if(s->s3->renegotiate)
	...
	ret = s->method->ssl_read_bytes(s, SSL3_RT_APPLICATION_DATA, NULL, static_cast<uchar *>(buf), len, peek, readbytes);
    ...
    int free_pms)
{
	...
	if(alg_k & SSL_PSK) {
		...
		pskpms = static_cast<uchar *>(OPENSSL_malloc(pskpmslen));
		if(pskpms == NULL)
			goto err;
		t = pskpms;
		s2n(pmslen, t);
