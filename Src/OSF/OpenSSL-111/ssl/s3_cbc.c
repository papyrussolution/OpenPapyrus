 ...
 */
static void tls1_md5_final_raw(void * ctx, uchar * md_out)
{
	MD5_CTX * md5 = static_cast<MD5_CTX *>(ctx);
...
static void tls1_sha1_final_raw(void * ctx, uchar * md_out)
{
	SHA_CTX * sha1 = static_cast<SHA_CTX *>(ctx);
	l2n(sha1->h0, md_out);
	l2n(sha1->h1, md_out);
	l2n(sha1->h2, md_out);
	l2n(sha1->h3, md_out);
...
static void tls1_sha256_final_raw(void * ctx, uchar * md_out)
{
	SHA256_CTX * sha256 = static_cast<SHA256_CTX *>(ctx);
	for(uint i = 0; i < 8; i++) {
		l2n(sha256->h[i], md_out);
	}
}

static void tls1_sha512_final_raw(void * ctx, uchar * md_out)
{
	SHA512_CTX * sha512 = static_cast<SHA512_CTX *>(ctx);
	for(uint i = 0; i < 8; i++) {
...
int ssl3_cbc_digest_record(const EVP_MD_CTX * ctx,
    uchar * md_out,
    size_t * md_out_size,
    const uchar header[13],
    const uchar * data,
    size_t data_plus_mac_size,
    size_t data_plus_mac_plus_padding_size,
    const uchar * mac_secret,
    size_t mac_secret_length, char is_sslv3)
{
	union {
		double align;
		uchar c[sizeof(LARGEST_DIGEST_CTX)];
	} md_state;

	void (* md_final_raw) (void * ctx, uchar * md_out);
	void (* md_transform) (void * ctx, const uchar * block);
	...
	uchar first_block[MAX_HASH_BLOCK_SIZE];
	uchar mac_out[EVP_MAX_MD_SIZE];
	size_t i, j;
	...
	switch(EVP_MD_CTX_type(ctx)) {
		case NID_md5:
		    ...
		    md_transform =
			(void (*)(void * ctx, const uchar * block))MD5_Transform;
		    md_size = 16;
		    sslv3_pad_length = 48;
		...
		case NID_sha1:
		    ...
		    md_transform =
			(void (*)(void * ctx, const uchar * block))SHA1_Transform;
		    md_size = 20;
		    break;
		case NID_sha224:
		    ...
		    md_transform =
			(void (*)(void * ctx, const uchar * block))SHA256_Transform;
		    md_size = 224 / 8;
		    break;
		case NID_sha256:
		...
		case NID_sha384:
		    ...
		    md_transform =
			(void (*)(void * ctx, const uchar * block))SHA512_Transform;
		    md_size = 384 / 8;
		    md_block_size = 128;
		    md_length_size = 16;
		    break;
	...
	if(!is_sslv3) {
		...
		memzero(hmac_pad, md_block_size);
	...
	memzero(mac_out, sizeof(mac_out));
	/*
	...
	for(i = num_starting_blocks; i <= num_starting_blocks + variance_blocks;
	    i++) {
		uchar block[MAX_HASH_BLOCK_SIZE];
		uchar is_block_a = constant_time_eq_8_s(i, index_a);
		uchar is_block_b = constant_time_eq_8_s(i, index_b);
		for(j = 0; j < md_block_size; j++) {
			uchar b = 0, is_past_c, is_past_cp1;
			if(k < header_length)
				b = header[k];
	...
	if(is_sslv3) {
		...
		if(EVP_DigestUpdate(md_ctx, mac_secret, mac_secret_length) <= 0 || EVP_DigestUpdate(md_ctx, hmac_pad, sslv3_pad_length) <= 0 || EVP_DigestUpdate(md_ctx, mac_out, md_size) <= 0)
			goto err;
	}
	else {
