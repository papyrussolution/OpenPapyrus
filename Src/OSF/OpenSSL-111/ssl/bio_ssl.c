...
{
	BIO_SSL * bs = static_cast<BIO_SSL *>(OPENSSL_zalloc(sizeof(*bs)));
	if(bs == NULL) {
...
static int ssl_read(BIO * b, char * buf, size_t size, size_t * readbytes)
{
	...
	switch(SSL_get_error(ssl, ret)) {
		case SSL_ERROR_NONE:
		    ...
		    break;
		case SSL_ERROR_WANT_READ: BIO_set_retry_read(b); break;
...
static long ssl_ctrl(BIO * b, int cmd, long num, void * ptr)
{
	...
	BIO * dbio, * bio;
	long ret = 1;
	BIO_SSL * bs = static_cast<BIO_SSL *>(BIO_get_data(b));
	BIO * next = BIO_next(b);
	SSL * ssl = bs->ssl;
	if(!ssl && (cmd != BIO_C_SET_SSL))
		return 0;
	switch(cmd) {
		case BIO_CTRL_RESET:
		    SSL_shutdown(ssl);
		...
		case BIO_C_DO_STATE_MACHINE:
		    ...
		    switch(SSL_get_error(ssl, (int)ret)) {
			    case SSL_ERROR_WANT_READ: BIO_set_flags(b, BIO_FLAGS_READ | BIO_FLAGS_SHOULD_RETRY); break;
			    case SSL_ERROR_WANT_WRITE: BIO_set_flags(b, BIO_FLAGS_WRITE | BIO_FLAGS_SHOULD_RETRY); break;
			    case SSL_ERROR_WANT_CONNECT:
		...
		case BIO_CTRL_DUP:
		    dbio = (BIO*)ptr;
		    dbs = static_cast<BIO_SSL *>(BIO_get_data(dbio));
		    SSL_free(dbs->ssl);
		    dbs->ssl = SSL_dup(ssl);
...
static long ssl_callback_ctrl(BIO * b, int cmd, BIO_info_cb * fp)
{
	...
	SSL * ssl = bs->ssl;
	switch(cmd) {
		case BIO_CTRL_SET_CALLBACK: ret = BIO_callback_ctrl(ssl->rbio, cmd, fp); break;
		default: ret = 0; break;
	}
	return ret;
}
...
int BIO_ssl_copy_session_id(BIO * t, BIO * f)
{
	...
	if((t == NULL) || (f == NULL))
		return 0;
	tdata = static_cast<BIO_SSL *>(BIO_get_data(t));
	fdata = static_cast<BIO_SSL *>(BIO_get_data(f));
