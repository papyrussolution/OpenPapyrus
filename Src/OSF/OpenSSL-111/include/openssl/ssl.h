...
#else
#define TLS_DEFAULT_CIPHERSUITES "TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256"
#endif
/*
...
typedef int (* tls_session_ticket_ext_cb_fn)(SSL * s, const uchar * data, int len, void * arg);
...
typedef int (* custom_ext_add_cb)(SSL * s, uint ext_type, const uchar ** out, size_t * outlen, int * al, void * add_arg);
typedef void (* custom_ext_free_cb)(SSL * s, uint ext_type, const uchar * out, void * add_arg);
typedef int (* custom_ext_parse_cb)(SSL * s, uint ext_type, const uchar * in, size_t inlen, int * al, void * parse_arg);
typedef int (* SSL_custom_ext_add_cb_ex)(SSL * s, uint ext_type, uint context, const uchar ** out, size_t * outlen, X509 * x,
    size_t chainidx, int * al, void * add_arg);
typedef void (* SSL_custom_ext_free_cb_ex)(SSL * s, uint ext_type, uint context, const uchar * out, void * add_arg);
typedef int (* SSL_custom_ext_parse_cb_ex)(SSL * s, uint ext_type, uint context, const uchar * in, size_t inlen, X509 * x, size_t chainidx,
 ...
 */
#define SSL_OP_ALL        (SSL_OP_CRYPTOPRO_TLSEXT_BUG|SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS|SSL_OP_LEGACY_SERVER_CONNECT| \
...
#define SSL_CERT_FLAG_SUITEB_128_LOS_ONLY       0x10000 /* Suite B modes, takes same values as certificate verify flags
	                                                   */
#define SSL_CERT_FLAG_SUITEB_192_LOS            0x20000 /* Suite B 192 bit only mode */
#define SSL_CERT_FLAG_SUITEB_128_LOS            0x30000 /* Suite B 128 bit mode allowing 192 bit algorithms */

#define SSL_CERT_FLAG_BROKEN_PROTOCOL           0x10000000 /* Perform all sorts of protocol violations for testing
	                                                      purposes */

/* Flags for building certificate chains */
#define SSL_BUILD_CHAIN_FLAG_UNTRUSTED          0x1  /* Treat any existing certificates as untrusted CAs */
#define SSL_BUILD_CHAIN_FLAG_NO_ROOT            0x2  /* Don't include root CA in chain */
#define SSL_BUILD_CHAIN_FLAG_CHECK              0x4  /* Just check certificates already there */
#define SSL_BUILD_CHAIN_FLAG_IGNORE_ERROR       0x8  /* Ignore verification errors */
#define SSL_BUILD_CHAIN_FLAG_CLEAR_ERROR        0x10 /* Clear verification errors from queue */

/* Flags returned by SSL_check_chain */
#define CERT_PKEY_VALID         0x1  /* Certificate can be used with this session */
#define CERT_PKEY_SIGN          0x2  /* Certificate can also be used for signing */
#define CERT_PKEY_EE_SIGNATURE  0x10 /* EE certificate signing algorithm OK */
#define CERT_PKEY_CA_SIGNATURE  0x20 /* CA signature algorithms OK */
#define CERT_PKEY_EE_PARAM      0x40 /* EE certificate parameters OK */
#define CERT_PKEY_CA_PARAM      0x80 /* CA certificate parameters OK */
#define CERT_PKEY_EXPLICIT_SIGN 0x100 /* Signing explicitly allowed as opposed to SHA1 fallback */
#define CERT_PKEY_ISSUER_NAME   0x200 /* Client CA issuer names match (always set for server cert) */
#define CERT_PKEY_CERT_TYPE     0x400 /* Cert type matches client types (always set for server cert) */
#define CERT_PKEY_SUITEB        0x800 /* Cert chain suitable to Suite B */
...
ulong SSL_get_options(const SSL * s);
ulong SSL_CTX_clear_options(SSL_CTX * ctx, ulong op);
ulong SSL_clear_options(SSL * s, ulong op);
ulong SSL_CTX_set_options(SSL_CTX * ctx, ulong op);
ulong SSL_set_options(SSL * s, ulong op);
...
void SSL_CTX_set_msg_callback(SSL_CTX * ctx, void (*cb)(int write_p,
    ...
    SSL * ssl,
    void * arg));
void SSL_set_msg_callback(SSL * ssl, void (*cb)(int write_p,
    int version,
    int content_type,
    const void * buf,
    size_t len,
 ...
 */
typedef int (* GEN_SESSION_CB) (SSL * ssl, uchar * id,
    uint * id_len);
...
__owur int SSL_CTX_set_alpn_protos(SSL_CTX * ctx, const uchar * protos,
    uint protos_len);
__owur int SSL_set_alpn_protos(SSL * ssl, const uchar * protos,
    uint protos_len);
...
void SSL_CTX_set_alpn_select_cb(SSL_CTX * ctx,
    ...
    void * arg);
void SSL_get0_alpn_selected(const SSL * ssl, const uchar ** data,
    uint * len);
...
typedef uint (* SSL_psk_client_cb_func)(SSL * ssl,
    const char * hint,
    char * identity,
    uint max_identity_len,
    uchar * psk,
    uint max_psk_len);
void SSL_CTX_set_psk_client_callback(SSL_CTX * ctx, SSL_psk_client_cb_func cb);
void SSL_set_psk_client_callback(SSL * ssl, SSL_psk_client_cb_func cb);

typedef uint (* SSL_psk_server_cb_func)(SSL * ssl,
    const char * identity,
    uchar * psk,
    uint max_psk_len);
...
__owur int SSL_CTX_add_custom_ext(SSL_CTX * ctx, uint ext_type,
    uint context,
...
__owur int SSL_extension_supported(uint ext_type);
...
__owur int SSL_use_RSAPrivateKey_ASN1(SSL * ssl, const uchar * d,
    long len);
#endif
__owur int SSL_use_PrivateKey(SSL * ssl, EVP_PKEY * pkey);
__owur int SSL_use_PrivateKey_ASN1(int pk, SSL * ssl, const uchar * d,
    long len);
__owur int SSL_use_certificate(SSL * ssl, X509 * x);
__owur int SSL_use_certificate_ASN1(SSL * ssl, const uchar * d, int len);
...
__owur int SSL_CTX_use_serverinfo(SSL_CTX * ctx, const uchar * serverinfo,
    size_t serverinfo_length);
__owur int SSL_CTX_use_serverinfo_ex(SSL_CTX * ctx, uint version,
...
__owur ulong SSL_SESSION_get_ticket_lifetime_hint(const SSL_SESSION * s);
...
__owur int SSL_SESSION_set1_id_context(SSL_SESSION * s,
    const uchar * sid_ctx,
    uint sid_ctx_len);
__owur int SSL_SESSION_set1_id(SSL_SESSION * s, const uchar * sid,
    uint sid_len);
__owur int SSL_SESSION_is_resumable(const SSL_SESSION * s);

__owur SSL_SESSION * SSL_SESSION_new(void);
__owur SSL_SESSION * SSL_SESSION_dup(SSL_SESSION * src);
const uchar * SSL_SESSION_get_id(const SSL_SESSION * s,
    uint * len);
const uchar * SSL_SESSION_get0_id_context(const SSL_SESSION * s,
    uint * len);
__owur uint SSL_SESSION_get_compress_id(const SSL_SESSION * s);
...
__owur int i2d_SSL_SESSION(SSL_SESSION * in, uchar ** pp);
...
__owur int SSL_CTX_use_RSAPrivateKey_ASN1(SSL_CTX * ctx, const uchar * d,
    long len);
#endif
...
__owur int SSL_set_session_id_context(SSL * ssl, const uchar * sid_ctx,
    uint sid_ctx_len);
...
__owur int SSL_dane_tlsa_add(SSL * s, uint8_t usage, uint8_t selector, uint8_t mtype, const uchar * data, size_t dlen);
__owur int SSL_get0_dane_authority(SSL * s, X509 ** mcert, EVP_PKEY ** mspki);
__owur int SSL_get0_dane_tlsa(SSL * s, uint8_t * usage, uint8_t * selector, uint8_t * mtype, const uchar ** data, size_t * dlen);
/*
 ...
 * DANE flags
 */
ulong SSL_CTX_dane_set_flags(SSL_CTX * ctx, ulong flags);
ulong SSL_CTX_dane_clear_flags(SSL_CTX * ctx, ulong flags);
ulong SSL_dane_set_flags(SSL * ssl, ulong flags);
ulong SSL_dane_clear_flags(SSL * ssl, ulong flags);
...
uint SSL_client_hello_get0_legacy_version(SSL * s);
size_t SSL_client_hello_get0_random(SSL * s, const uchar ** out);
size_t SSL_client_hello_get0_session_id(SSL * s, const uchar ** out);
size_t SSL_client_hello_get0_ciphers(SSL * s, const uchar ** out);
size_t SSL_client_hello_get0_compression_methods(SSL * s, const uchar ** out);
int SSL_client_hello_get1_extensions_present(SSL * s, int ** out, size_t * outlen);
int SSL_client_hello_get0_ext(SSL * s, uint type, const uchar ** out, size_t * outlen);
...
__owur size_t SSL_get_client_random(const SSL * ssl, uchar * out,
    size_t outlen);
__owur size_t SSL_get_server_random(const SSL * ssl, uchar * out,
    size_t outlen);
...
__owur uint SSL_CONF_CTX_clear_flags(SSL_CONF_CTX * cctx, uint flags);
...
#define SSL_TICKET_FATAL_ERR_MALLOC 0 /* fatal error, malloc failure */
#define SSL_TICKET_FATAL_ERR_OTHER  1 /* fatal error, either from parsing or decrypting the ticket */
#define SSL_TICKET_NONE             2 /* No ticket present */
#define SSL_TICKET_EMPTY            3 /* Empty ticket present */
#define SSL_TICKET_NO_DECRYPT       4 /* the ticket couldn't be decrypted */
#define SSL_TICKET_SUCCESS          5 /* a ticket was successfully decrypted */
#define SSL_TICKET_SUCCESS_RENEW    6 /* same as above but the ticket needs to be renewed */

typedef int SSL_TICKET_RETURN; /* Return codes for the decrypt session ticket callback */

#define SSL_TICKET_RETURN_ABORT             0 /* An error occurred */
#define SSL_TICKET_RETURN_IGNORE            1 /* Do not use the ticket, do not send a renewed ticket to the client */
#define SSL_TICKET_RETURN_IGNORE_RENEW      2 /* Do not use the ticket, send a renewed ticket to the client */
#define SSL_TICKET_RETURN_USE               3 /* Use the ticket, do not send a renewed ticket to the client */
#define SSL_TICKET_RETURN_USE_RENEW         4 /* Use the ticket, send a renewed ticket to the client */

typedef int (* SSL_CTX_generate_session_ticket_fn)(SSL * s, void * arg);
typedef SSL_TICKET_RETURN (* SSL_CTX_decrypt_session_ticket_fn)(SSL * s, SSL_SESSION * ss, const uchar * keyname, size_t keyname_length,
...
typedef uint (* DTLS_timer_cb)(SSL * s, uint timer_us);
