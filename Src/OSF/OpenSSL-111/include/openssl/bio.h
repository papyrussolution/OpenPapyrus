 ...
 * the code.
 */
#define BIO_RR_SSL_X509_LOOKUP          0x01 // Returned from the SSL bio when the certificate retrieval code had an
	                                     // error
#define BIO_RR_CONNECT                  0x02 /* Returned from the connect BIO when a connect would have blocked */
#define BIO_RR_ACCEPT                   0x03 /* Returned from the accept BIO when an accept would have blocked */
...
#define BIO_dgram_get_mtu_overhead(b) (uint)BIO_ctrl((b), BIO_CTRL_DGRAM_GET_MTU_OVERHEAD, 0, NULL)
...
int    FASTCALL BIO_read(BIO * b, void * data, int dlen);
int    BIO_read_ex(BIO * b, void * data, size_t dlen, size_t * readbytes);
int    BIO_gets(BIO * bp, char * buf, int size);
int    FASTCALL BIO_write(BIO * b, const void * data, int dlen);
...
DEPRECATEDIN_1_1_0(int BIO_get_port(const char * str, ushort * port_ptr))
DEPRECATEDIN_1_1_0(int BIO_get_host_ip(const char * str, uchar * ip))
