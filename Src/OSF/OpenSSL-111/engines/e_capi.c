...
static int capi_ctrl(ENGINE * e, int cmd, long i, void * p, void (*f)(void))
{
	...
	switch(cmd) {
		...
		case CAPI_CMD_LOOKUP_CERT:
		    ret = capi_list_certs(ctx, out, static_cast<char *>(p));
		    break;
		...
		case CAPI_CMD_STORE_NAME:
		    tmpstr = OPENSSL_strdup(static_cast<const char *>(p));
		    if(tmpstr != NULL) {
		...
		case CAPI_CMD_SET_CSP_NAME:
		    ret = capi_ctx_set_provname(ctx, static_cast<LPSTR>(p), ctx->csptype, 1);
		    break;
...
{
	CAPI_CTX * ctx = static_cast<CAPI_CTX *>(ENGINE_get_ex_data(e, capi_idx));
	capi_ctx_free(ctx);
...
static EVP_PKEY * capi_get_pkey(ENGINE * eng, CAPI_KEY * key)
{
	...
	pubkey = static_cast<uchar *>(OPENSSL_malloc(len));
	if(pubkey == NULL)
		goto memerr;
...
static EVP_PKEY * capi_load_privkey(ENGINE * eng, const char * key_id, UI_METHOD * ui_method, void * callback_data)
{
	CAPI_KEY * key;
	EVP_PKEY * ret;
	CAPI_CTX * ctx = static_cast<CAPI_CTX *>(ENGINE_get_ex_data(eng, capi_idx));
	if(!ctx) {
    ...
    uchar * sigret, uint * siglen, const RSA * rsa)
{
	...
	capi_key = static_cast<CAPI_KEY *>(RSA_get_ex_data(rsa, rsa_capi_idx));
	if(!capi_key) {
    ...
    uchar * to, RSA * rsa, int padding)
{
	...
	if(flen <= 0)
		return flen;
	ctx = static_cast<CAPI_CTX *>(ENGINE_get_ex_data(RSA_get0_engine(rsa), capi_idx));
	...
	if((tmpbuf = static_cast<uchar *>(OPENSSL_malloc(flen))) == NULL) {
    ...
    DWORD idx)
{
	...
	name = static_cast<LPTSTR>(OPENSSL_malloc(len));
	if(name == NULL) {
...
static int capi_list_containers(CAPI_CTX * ctx, BIO * out)
{
	...
	if(ctx->cspname != NULL) {
		if((clen = MultiByteToWideChar(CP_ACP, 0, ctx->cspname, -1, NULL, 0))) {
			cspname = static_cast<LPWSTR>(alloca(clen * sizeof(WCHAR)));
	...
	if(buflen == 0)
		buflen = 1024;
	cname = static_cast<LPSTR>(OPENSSL_malloc(buflen));
	if(cname == NULL) {
		CAPIerr(CAPI_F_CAPI_LIST_CONTAINERS, ERR_R_MALLOC_FAILURE);
		goto err;
	}
    ...
    PCCERT_CONTEXT cert)
{
	...
	if(!CertGetCertificateContextProperty(cert, CERT_KEY_PROV_INFO_PROP_ID, NULL, &len))
		return NULL;
	pinfo = static_cast<CRYPT_KEY_PROV_INFO *>(OPENSSL_malloc(len));
	if(pinfo == NULL) {
...
static char * capi_cert_get_fname(CAPI_CTX * ctx, PCCERT_CONTEXT cert)
{
	...
	if(!CertGetCertificateContextProperty(cert, CERT_FRIENDLY_NAME_PROP_ID,
	    NULL, &dlen))
		return NULL;
	wfname = static_cast<LPWSTR>(OPENSSL_malloc(dlen));
	if(wfname == NULL)
		return NULL;
	if(CertGetCertificateContextProperty(cert, CERT_FRIENDLY_NAME_PROP_ID,
	    wfname, &dlen)) {
...
CAPI_KEY * capi_find_key(CAPI_CTX * ctx, const char * id)
{
	...
	switch(ctx->lookup_method) {
		...
		case CAPI_LU_CONTNAME:
	    {
		    ...
		    DWORD len;
		    if((len = MultiByteToWideChar(CP_ACP, 0, id, -1, NULL, 0)) && (contname = static_cast<WCHAR *>(alloca(len * sizeof(WCHAR))),
				MultiByteToWideChar(CP_ACP, 0, id, -1, contname, len)) && (len = MultiByteToWideChar(CP_ACP, 0, ctx->cspname, -1, NULL, 0)) &&
				(provname = static_cast<WCHAR *>(alloca(len * sizeof(WCHAR))), MultiByteToWideChar(CP_ACP, 0, ctx->cspname, -1, provname, len)))
...
static int capi_ctx_set_provname(CAPI_CTX * ctx, LPSTR pname, DWORD type, int check)
{
	...
	if(check) {
		...
		if((len = MultiByteToWideChar(CP_ACP, 0, pname, -1, NULL, 0))) {
			name = static_cast<LPWSTR>(alloca(len * sizeof(WCHAR)));
    ...
    void * callback_data)
{
	...
	*pkey = NULL;
	...
	for(i = 0; i < sk_X509_num(certs); i++) {
		...
		if(i == client_cert_idx)
			*pcert = x;
			...
			key = static_cast<CAPI_KEY *>(X509_get_ex_data(x, cert_capi_idx));
			capi_free_key(key);
			X509_free(x);
		}
	}
	...
	key = static_cast<CAPI_KEY *>(X509_get_ex_data(*pcert, cert_capi_idx));
...
#define dlg_columns      CRYPTUI_SELECT_LOCATION_COLUMN|CRYPTUI_SELECT_INTENDEDUSE_COLUMN
