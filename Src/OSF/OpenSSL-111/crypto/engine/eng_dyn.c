...
static int dynamic_ctrl(ENGINE * e, int cmd, long i, void * p, void (*f)(void))
{
	...
	switch(cmd) {
		case DYNAMIC_CMD_SO_PATH:
		    ...
		    OPENSSL_free(ctx->DYNAMIC_LIBNAME);
		    if(p)
			    ctx->DYNAMIC_LIBNAME = OPENSSL_strdup(static_cast<const char *>(p));
		    else
		...
		case DYNAMIC_CMD_ID:
		    ...
		    OPENSSL_free(ctx->engine_id);
		    if(p)
			    ctx->engine_id = OPENSSL_strdup(static_cast<const char *>(p));
		    else
			    ctx->engine_id = NULL;
		...
		case DYNAMIC_CMD_DIR_ADD:
		    ...
		    {
			    char * tmp_str = OPENSSL_strdup(static_cast<const char *>(p));
			    if(tmp_str == NULL) {
...
static int dynamic_load(ENGINE * e, dynamic_data_ctx * ctx)
{
	ENGINE cpy;
	dynamic_fns fns;
	SETIFZ(ctx->dynamic_dso, DSO_new());
	...
	if(ctx->list_add_value > 0) {
		if(!ENGINE_add(e)) {
			...
			if(ctx->list_add_value > 1) {
				 ...
				 */
				ENGINEerr(ENGINE_F_DYNAMIC_LOAD, ENGINE_R_CONFLICTING_ENGINE_ID);
				return 0;
			}
			/* Tolerate */
			ERR_clear_error();
