...
static int clean_devcrypto_session(struct session_op * sess) 
{
	...
	memzero(sess, sizeof(struct session_op));
	return 1;
}
    ...
    const uchar * in, size_t inl)
{
	...
	/* full blocks */
	if(inl > (uint)cipher_ctx->blocksize) {
	...
	/* final partial block */
	if(inl) {
		memzero(cipher_ctx->partial, cipher_ctx->blocksize);
...
static int cipher_ctrl(EVP_CIPHER_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		case EVP_CTRL_COPY:
		    ...
		    memzero(&to_cipher_ctx->sess, sizeof(to_cipher_ctx->sess));
		    return cipher_init(to_ctx, cipher_ctx->sess.key, EVP_CIPHER_CTX_iv(ctx),
			       (cipher_ctx->op == COP_ENCRYPT));

		case EVP_CTRL_INIT:
		    memzero(&cipher_ctx->sess, sizeof(cipher_ctx->sess));
		    return 1;
...
static int digest_init(EVP_MD_CTX * ctx)
{
	...
	const struct digest_data_st * digest_d = get_digest_data(EVP_MD_CTX_type(ctx));
	digest_ctx->init_called = 1;
	memzero(&digest_ctx->sess, sizeof(digest_ctx->sess));
...
static int devcrypto_test_digest(size_t digest_data_index)
{
	...
	struct cphash_op cphash;
	int ret = 0;
	memzero(&sess1, sizeof(sess1));
	memzero(&sess2, sizeof(sess2));
...
#endif
	if(!ENGINE_set_id(e, "devcrypto") || !ENGINE_set_name(e, "/dev/crypto engine")
/*
