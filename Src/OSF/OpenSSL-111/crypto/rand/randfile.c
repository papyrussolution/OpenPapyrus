...
const char * RAND_file_name(char * buf, size_t size)
{
	...
	if(envlen != 0) {
		int sz;
		WCHAR * val = static_cast<WCHAR *>(_alloca(envlen * sizeof(WCHAR)));
		if(GetEnvironmentVariableW(var, val, envlen) < envlen && (sz = WideCharToMultiByte(CP_UTF8, 0, val, -1, NULL, 0, NULL, NULL)) != 0) {
			s = static_cast<char *>(_alloca(sz));
