...
{
	EVP_CIPHER * cipher = static_cast<EVP_CIPHER *>(OPENSSL_zalloc(sizeof(EVP_CIPHER)));
	if(cipher != NULL) {
...
{
	EVP_CIPHER * to = EVP_CIPHER_meth_new(cipher->nid, cipher->block_size, cipher->key_len);
	if(to != NULL)
