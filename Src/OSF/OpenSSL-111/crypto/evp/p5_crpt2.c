...
int PKCS5_v2_PBE_keyivgen(EVP_CIPHER_CTX * ctx, const char * pass, int passlen, ASN1_TYPE * param, const EVP_CIPHER * c, const EVP_MD * md, int en_de)
{
	...
	EVP_PBE_KEYGEN * kdf;
	int rv = 0;
	pbe2 = static_cast<PBE2PARAM *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(PBE2PARAM), param));
	if(pbe2 == NULL) {
    ...
    const EVP_CIPHER * c, const EVP_MD * md, int en_de)
{
	...
	/* Decode parameter */
	kdf = static_cast<PBKDF2PARAM *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(PBKDF2PARAM), param));
	if(kdf == NULL) {
