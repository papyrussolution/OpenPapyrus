...
static int init(EVP_MD_CTX * evp_ctx, uchar pad)
{
	KECCAK1600_CTX * ctx = static_cast<KECCAK1600_CTX *>(evp_ctx->md_data);
...
static int sha3_final(EVP_MD_CTX * evp_ctx, uchar * md)
{
	KECCAK1600_CTX * ctx = static_cast<KECCAK1600_CTX *>(evp_ctx->md_data);
	size_t bsz = ctx->block_size;
	size_t num = ctx->num;
