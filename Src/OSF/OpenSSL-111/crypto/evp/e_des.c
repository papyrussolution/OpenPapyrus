...
static int des_ofb_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	while(inl >= EVP_MAXCHUNK) {
		...
		DES_ofb64_encrypt(in, out, (long)EVP_MAXCHUNK, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
	...
	if(inl) {
		int num = EVP_CIPHER_CTX_num(ctx);
		DES_ofb64_encrypt(in, out, (long)inl, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
		    (DES_cblock*)EVP_CIPHER_CTX_iv_noconst(ctx), &num);
		EVP_CIPHER_CTX_set_num(ctx, num);
	}
	return 1;
...
static int des_cbc_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	...
	while(inl >= EVP_MAXCHUNK) {
		DES_ncbc_encrypt(in, out, (long)EVP_MAXCHUNK, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
	...
	if(inl)
		DES_ncbc_encrypt(in, out, (long)inl, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
		    (DES_cblock*)EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_encrypting(ctx));
	return 1;
}
    ...
    const uchar * in, size_t inl)
{
	while(inl >= EVP_MAXCHUNK) {
		int num = EVP_CIPHER_CTX_num(ctx);
		DES_cfb64_encrypt(in, out, (long)EVP_MAXCHUNK, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
	...
	if(inl) {
		int num = EVP_CIPHER_CTX_num(ctx);
		DES_cfb64_encrypt(in, out, (long)inl, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
		    (DES_cblock*)EVP_CIPHER_CTX_iv_noconst(ctx), &num, EVP_CIPHER_CTX_encrypting(ctx));
		EVP_CIPHER_CTX_set_num(ctx, num);
	}
	return 1;
    ...
    const uchar * in, size_t inl)
{
	...
	while(inl && inl >= chunk) {
		for(n = 0; n < chunk * 8; ++n) {
			...
			DES_cfb_encrypt(c, d, 1, 1, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
			    (DES_cblock*)EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_encrypting(ctx));
			out[n / 8] = (out[n / 8] & ~(0x80 >> (uint)(n % 8))) | ((d[0] & 0x80) >> (uint)(n % 8));
		}
		inl -= chunk;
		in += chunk;
		out += chunk;
...
static int des_cfb8_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	...
	if(inl)
		DES_cfb_encrypt(in, out, 8, (long)inl, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)),
		    (DES_cblock*)EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_encrypting(ctx));
	return 1;
}
...
#endif
	DES_set_key_unchecked(deskey, static_cast<DES_key_schedule *>(EVP_CIPHER_CTX_get_cipher_data(ctx)));
	return 1;
}
...
static int des_ctrl(EVP_CIPHER_CTX * c, int type, int arg, void * ptr)
{
	switch(type) {
		case EVP_CTRL_RAND_KEY:
		    if(RAND_priv_bytes(static_cast<uchar *>(ptr), 8) <= 0)
			    return 0;
