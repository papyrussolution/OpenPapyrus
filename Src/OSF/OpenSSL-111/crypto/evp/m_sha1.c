...
static int ctrl(EVP_MD_CTX * ctx, int cmd, int mslen, void * ms)
{
	...
	if(ctx == NULL)
		return 0;
	sha1 = static_cast<SHA_CTX *>(EVP_MD_CTX_md_data(ctx));
...
{
	return SHA224_Init(static_cast<SHA256_CTX *>(EVP_MD_CTX_md_data(ctx)));
}
...
{
	return SHA256_Init(static_cast<SHA256_CTX *>(EVP_MD_CTX_md_data(ctx)));
}
...
{
	return sha512_224_init(static_cast<SHA512_CTX *>(EVP_MD_CTX_md_data(ctx)));
}

static int init512_256(EVP_MD_CTX * ctx)
{
	return sha512_256_init(static_cast<SHA512_CTX *>(EVP_MD_CTX_md_data(ctx)));
}

static int init384(EVP_MD_CTX * ctx)
{
	return SHA384_Init(static_cast<SHA512_CTX *>(EVP_MD_CTX_md_data(ctx)));
}
...
{
	return SHA512_Init(static_cast<SHA512_CTX *>(EVP_MD_CTX_md_data(ctx)));
}
