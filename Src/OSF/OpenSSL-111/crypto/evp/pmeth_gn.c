...
{
	EVP_PKEY_CTX * ctx = static_cast<EVP_PKEY_CTX *>(BN_GENCB_get_arg(gcb));
	ctx->keygen_info[0] = a;
	ctx->keygen_info[1] = b;
...
int EVP_PKEY_public_check(EVP_PKEY_CTX * ctx)
{
	...
	if(pkey->ameth == NULL || pkey->ameth->pkey_public_check == NULL) {
		EVPerr(EVP_F_EVP_PKEY_PUBLIC_CHECK, EVP_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE);
		return -2;
	}
...
int EVP_PKEY_param_check(EVP_PKEY_CTX * ctx)
{
	...
	if(pkey->ameth == NULL || pkey->ameth->pkey_param_check == NULL) {
		EVPerr(EVP_F_EVP_PKEY_PARAM_CHECK, EVP_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE);
		return -2;
	}
