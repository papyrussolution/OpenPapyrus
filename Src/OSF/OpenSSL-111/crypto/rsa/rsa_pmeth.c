...
{
	RSA_PKEY_CTX * rctx = static_cast<RSA_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*rctx)));
...
static int pkey_rsa_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_rsa_init(dst))
		return 0;
	sctx = static_cast<RSA_PKEY_CTX *>(src->data);
	dctx = static_cast<RSA_PKEY_CTX *>(dst->data);
	...
	if(sctx->oaep_label) {
		...
		dctx->oaep_label = static_cast<uchar *>(OPENSSL_memdup(sctx->oaep_label, sctx->oaep_labellen));
		if(!dctx->oaep_label)
			return 0;
...
static int setup_tbuf(RSA_PKEY_CTX * ctx, EVP_PKEY_CTX * pk)
{
	if(ctx->tbuf != NULL)
		return 1;
	if((ctx->tbuf = static_cast<uchar *>(OPENSSL_malloc(EVP_PKEY_size(pk->pkey)))) == NULL) {
...
bad_pad:
		...
		case EVP_PKEY_CTRL_RSA_KEYGEN_PUBEXP:
		    ...
		    BN_free(rctx->pub_exp);
		    rctx->pub_exp = static_cast<BIGNUM *>(p2);
		    return 1;
		...
		case EVP_PKEY_CTRL_GET_RSA_OAEP_MD:
		    ...
		    if(type == EVP_PKEY_CTRL_GET_RSA_OAEP_MD)
			    *(const EVP_MD**)p2 = rctx->md;
			    ...
			    rctx->md = static_cast<const EVP_MD *>(p2);
		    return 1;

		case EVP_PKEY_CTRL_MD:
		    if(!check_padding_md(static_cast<EVP_MD *>(p2), rctx->pad_mode))
			    return 0;
		    if(rsa_pss_restricted(rctx)) {
			    if(EVP_MD_type(rctx->md) == EVP_MD_type(static_cast<EVP_MD *>(p2)))
				    return 1;
		...
		case EVP_PKEY_CTRL_GET_RSA_MGF1_MD:
		    ...
		    if(type == EVP_PKEY_CTRL_GET_RSA_MGF1_MD) {
			    ...
			    if(rsa_pss_restricted(rctx)) {
				    if(EVP_MD_type(rctx->mgf1md) == EVP_MD_type(static_cast<EVP_MD *>(p2)))
					    return 1;
				    RSAerr(RSA_F_PKEY_RSA_CTRL, RSA_R_MGF1_DIGEST_NOT_ALLOWED);
				    return 0;
			    }
			    rctx->mgf1md = static_cast<const EVP_MD *>(p2);
		    }
		    return 1;
		...
		case EVP_PKEY_CTRL_RSA_OAEP_LABEL:
		    ...
		    OPENSSL_free(rctx->oaep_label);
		    if(p2 && p1 > 0) {
			    rctx->oaep_label = static_cast<uchar *>(p2);
    ...
    const char * type, const char * value)
{
	...
	if(sstreq(type, "rsa_padding_mode")) {
		int pm;

		if(sstreq(value, "pkcs1")) {
			pm = RSA_PKCS1_PADDING;
		}
		else if(sstreq(value, "sslv23")) {
			pm = RSA_SSLV23_PADDING;
		}
		else if(sstreq(value, "none")) {
			pm = RSA_NO_PADDING;
		}
		else if(sstreq(value, "oeap")) {
			pm = RSA_PKCS1_OAEP_PADDING;
		}
		else if(sstreq(value, "oaep")) {
			pm = RSA_PKCS1_OAEP_PADDING;
		}
		else if(sstreq(value, "x931")) {
			pm = RSA_X931_PADDING;
		}
		else if(sstreq(value, "pss")) {
	...
	if(sstreq(type, "rsa_pss_saltlen")) {
		int saltlen;
	...
	if(sstreq(type, "rsa_keygen_bits")) {
		int nbits = atoi(value);

		return EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, nbits);
	}

	if(sstreq(type, "rsa_keygen_pubexp")) {
		int ret;
	...
	if(sstreq(type, "rsa_keygen_primes")) {
		int nprimes = atoi(value);

		return EVP_PKEY_CTX_set_rsa_keygen_primes(ctx, nprimes);
	}

	if(sstreq(type, "rsa_mgf1_md"))
		return EVP_PKEY_CTX_md(ctx,
			   EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT,
			   EVP_PKEY_CTRL_RSA_MGF1_MD, value);

	if(pkey_ctx_is_pss(ctx)) {
		if(sstreq(type, "rsa_pss_keygen_mgf1_md"))
			return EVP_PKEY_CTX_md(ctx, EVP_PKEY_OP_KEYGEN,
				   EVP_PKEY_CTRL_RSA_MGF1_MD, value);

		if(sstreq(type, "rsa_pss_keygen_md"))
			return EVP_PKEY_CTX_md(ctx, EVP_PKEY_OP_KEYGEN,
				   EVP_PKEY_CTRL_MD, value);

		if(sstreq(type, "rsa_pss_keygen_saltlen")) {
