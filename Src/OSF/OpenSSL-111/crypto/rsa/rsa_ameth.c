...
static X509_ALGOR * rsa_mgf1_decode(X509_ALGOR * alg)
{
	if(OBJ_obj2nid(alg->algorithm) != NID_mgf1)
		return NULL;
	return static_cast<X509_ALGOR *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(X509_ALGOR), alg->parameter));
}
...
static RSA_PSS_PARAMS * rsa_pss_decode(const X509_ALGOR * alg)
{
	RSA_PSS_PARAMS * pss = static_cast<RSA_PSS_PARAMS *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(RSA_PSS_PARAMS), alg->parameter));
	if(pss == NULL)
		return NULL;
...
static int rsa_sig_print(BIO * bp, const X509_ALGOR * sigalg, const ASN1_STRING * sig, int indent, ASN1_PCTX * pctx)
{
	if(OBJ_obj2nid(sigalg->algorithm) == EVP_PKEY_RSA_PSS) {
		...
		int rv = rsa_pss_param_print(bp, 0, pss, indent);
		RSA_PSS_PARAMS_free(pss);
		if(!rv)
			return 0;
	}
...
static int rsa_pkey_ctrl(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	...
	switch(op) {
		case ASN1_PKEY_CTRL_PKCS7_SIGN:
		    if(arg1 == 0)
			    PKCS7_SIGNER_INFO_get0_algs(static_cast<PKCS7_SIGNER_INFO *>(arg2), NULL, NULL, &alg);
		    break;
		case ASN1_PKEY_CTRL_PKCS7_ENCRYPT:
		    if(pkey_is_pss(pkey))
			    return -2;
		    if(arg1 == 0)
			    PKCS7_RECIP_INFO_get0_alg(static_cast<PKCS7_RECIP_INFO *>(arg2), &alg);
		    break;
#ifndef OPENSSL_NO_CMS
		...
		case ASN1_PKEY_CTRL_CMS_ENVELOPE:
		    if(pkey_is_pss(pkey))
			    return -2;
		    if(arg1 == 0)
			    return rsa_cms_encrypt(static_cast<CMS_RecipientInfo *>(arg2));
		    else if(arg1 == 1)
			    return rsa_cms_decrypt(static_cast<CMS_RecipientInfo *>(arg2));
		    break;
