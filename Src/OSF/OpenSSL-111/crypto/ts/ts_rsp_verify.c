    ...
    uchar ** imprint, unsigned * imprint_len)
{
	...
	*imprint = NULL;
	...
	if(length < 0)
		goto err;
	*imprint_len = length;
	if((*imprint = static_cast<uchar *>(OPENSSL_malloc(*imprint_len))) == NULL) {
...
static int ts_check_signer_name(GENERAL_NAME * tsa_name, X509 * signer)
{
	...
	if(tsa_name->type == GEN_DIRNAME && X509_name_cmp(tsa_name->d.dirn, X509_get_subject_name(signer)) == 0)
		return 1;
	gen_names = static_cast<STACK_OF(GENERAL_NAME) *>(X509_get_ext_d2i(signer, NID_subject_alt_name, NULL, &idx));
