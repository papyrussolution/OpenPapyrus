    ...
    EVP_CIPHER_CTX * ctx)
{
	...
	if((tmp = static_cast<uchar *>(OPENSSL_malloc(inlen))) == NULL) {
...
static int kek_wrap_key(uchar * out, size_t * outlen, const uchar * in, size_t inlen, EVP_CIPHER_CTX * ctx)
{
	...
	if(out) {
		...
		if(olen > inlen + 4 && RAND_bytes(out + 4 + inlen, olen - 4 - inlen) <= 0)
			return 0;
		/* Encrypt twice */
...
int cms_RecipientInfo_pwri_crypt(CMS_ContentInfo * cms, CMS_RecipientInfo * ri, int en_de)
{
	...
	kekalg = static_cast<X509_ALGOR *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(X509_ALGOR), algtmp->parameter));
	if(kekalg == NULL) {
