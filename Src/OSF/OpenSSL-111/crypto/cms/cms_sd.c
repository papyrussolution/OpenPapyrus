...
static int cms_copy_messageDigest(CMS_ContentInfo * cms, CMS_SignerInfo * si)
{
	...
	for(i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
		...
		if(OBJ_cmp(si->digestAlgorithm->algorithm,
			...
			continue;
		messageDigest = static_cast<ASN1_OCTET_STRING *>(CMS_signed_get0_data_by_OBJ(sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING));
		if(!messageDigest) {
...
int CMS_SignerInfo_sign(CMS_SignerInfo * si)
{
	...
	if(EVP_DigestSignFinal(mctx, NULL, &siglen) <= 0)
		goto err;
	OPENSSL_free(abuf);
	abuf = static_cast<uchar *>(OPENSSL_malloc(siglen));
	if(abuf == NULL)
		goto err;
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo * si, BIO * chain)
{
	...
	if(CMS_signed_get_attr_count(si) >= 0) {
		os = static_cast<ASN1_OCTET_STRING *>(CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING));
		if(!os) {
