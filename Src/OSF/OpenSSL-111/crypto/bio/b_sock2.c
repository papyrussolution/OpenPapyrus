...
int BIO_connect(int sock, const BIO_ADDR * addr, int options)
{
	...
	if(options & BIO_SOCK_KEEPALIVE) {
		if(setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, reinterpret_cast<const char *>(&on), sizeof(on)) != 0) {
	...
	if(options & BIO_SOCK_NODELAY) {
		if(setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char *>(&on), sizeof(on)) != 0) {
			SYSerr(SYS_F_SETSOCKOPT, get_last_socket_error());
...
int BIO_listen(int sock, const BIO_ADDR * addr, int options)
{
	...
	if(getsockopt(sock, SOL_SOCKET, SO_TYPE, reinterpret_cast<char *>(&socktype), &socktype_len) != 0 || socktype_len != sizeof(socktype)) {
	...
	if(BIO_ADDR_family(addr) == AF_INET6) {
		...
		if(setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char *>(&on), sizeof(on)) != 0) {
			SYSerr(SYS_F_SETSOCKOPT, get_last_socket_error());
