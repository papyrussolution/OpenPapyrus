...
{
	BIO_ACCEPT * ret;
	if((ret = static_cast<BIO_ACCEPT *>(OPENSSL_zalloc(sizeof(*ret)))) == NULL) {
		...
		case BIO_C_SET_ACCEPT:
		    if(ptr) {
			    if(num == 0) {
				    ...
				    ret = BIO_parse_hostserv(static_cast<const char *>(ptr), &data->param_addr, &data->param_serv, BIO_PARSE_PRIO_SERV);
				    ...
				    data->param_serv = BUF_strdup(static_cast<const char *>(ptr));
				    b->init = 1;
			    }
			    else if(num == 2) {
