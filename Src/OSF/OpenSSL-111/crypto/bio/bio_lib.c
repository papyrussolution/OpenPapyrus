 ...
 */
#define HAS_LEN_OPER(o) oneof3((o), BIO_CB_READ, BIO_CB_WRITE, BIO_CB_GETS)
/*
...
{
	BIO * bio = static_cast<BIO *>(OPENSSL_zalloc(sizeof(*bio)));
	if(bio == NULL) {
...
{
	int ret = bio_read_intern(b, data, dlen, readbytes);
	if(ret > 0)
		ret = 1;
	else
		ret = 0;
...
{
	int ret = bio_write_intern(b, data, dlen, written);
	if(ret > 0)
		ret = 1;
	else
		ret = 0;
...
int FASTCALL BIO_puts(BIO * b, const char * buf)
{
	int ret;
	size_t written = 0;
