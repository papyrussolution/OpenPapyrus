...
BIO * BIO_new_mem_buf(const void * buf, int len)
{
	...
	sz = (len < 0) ? strlen(static_cast<const char *>(buf)) : (size_t)len;
	if((ret = BIO_new(BIO_s_mem())) == NULL)
		return NULL;
	bb = (BIO_BUF_MEM*)ret->ptr;
	b = bb->buf;
	/* Cast away const and trust in the MEM_RDONLY flag. */
	b->data = (char *)buf; // @badcast
	b->length = sz;
	b->max = sz;
	*bb->readp = *bb->buf;
...
static int mem_init(BIO * bi, ulong flags)
{
	...
	if((bb->readp = static_cast<buf_mem_st *>(OPENSSL_zalloc(sizeof(*bb->readp)))) == NULL) {
		BUF_MEM_free(bb->buf);
		OPENSSL_free(bb);
		return 0;
	}
...
static long mem_ctrl(BIO * b, int cmd, long num, void * ptr)
{
	...
	switch(cmd) {
		...
		case BIO_C_SET_BUF_MEM:
		    mem_buf_free(b);
		    b->shutdown = (int)num;
		    bbm->buf = static_cast<buf_mem_st *>(ptr);
		    *bbm->readp = *bbm->buf;
		    break;
