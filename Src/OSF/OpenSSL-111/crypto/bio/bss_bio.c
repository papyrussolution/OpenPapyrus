...
static int bio_new(BIO * bio)
{
	struct bio_bio_st * b = static_cast<bio_bio_st *>(OPENSSL_zalloc(sizeof(*b)));
	if(b == NULL)
		return 0;
...
static long bio_ctrl(BIO * bio, int cmd, long num, void * ptr)
{
	...
	switch(cmd) {
	    ...
	    {
		    BIO * other_bio = static_cast<BIO *>(ptr);
		...
		case BIO_C_NREAD0:
		    ...
		    ret = (long)bio_nread0(bio, static_cast<char **>(ptr));
		    break;
		case BIO_C_NREAD:
		    /* non-copying read */
		    ret = (long)bio_nread(bio, static_cast<char **>(ptr), (size_t)num);
		    break;
		case BIO_C_NWRITE0:
		    /* prepare for non-copying write */
		    ret = (long)bio_nwrite0(bio, static_cast<char **>(ptr));
		    break;
		case BIO_C_NWRITE:
		    /* non-copying write */
		    ret = (long)bio_nwrite(bio, static_cast<char **>(ptr), (size_t)num);
		    break;
		...
		case BIO_CTRL_PENDING:
		    if(b->peer != NULL) {
			    struct bio_bio_st * peer_b = static_cast<bio_bio_st *>(b->peer->ptr);
			    ret = (long)peer_b->len;
		    }
		    else
			    ret = 0;
		    ...
		    /* See BIO_dup_chain for circumstances we have to expect. */
	    {
		    ...
		    other_b = static_cast<bio_bio_st *>(other_bio->ptr);
		    assert(other_b != NULL);
...
static int bio_make_pair(BIO * bio1, BIO * bio2)
{
	...
	assert(bio1 != NULL);
	assert(bio2 != NULL);
	b1 = static_cast<bio_bio_st *>(bio1->ptr);
	b2 = static_cast<bio_bio_st *>(bio2->ptr);
	...
	if(b1->buf == NULL) {
		b1->buf = static_cast<char *>(OPENSSL_malloc(b1->size));
		if(b1->buf == NULL) {
	...
	if(b2->buf == NULL) {
		b2->buf = static_cast<char *>(OPENSSL_malloc(b2->size));
		if(b2->buf == NULL) {
			BIOerr(BIO_F_BIO_MAKE_PAIR, ERR_R_MALLOC_FAILURE);
			return 0;
		}
