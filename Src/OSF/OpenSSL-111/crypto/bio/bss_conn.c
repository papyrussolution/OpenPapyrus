		...
		switch(c->state) {
			...
			case BIO_CONN_S_CONNECT:
			    ...
			    if(ret == 0) {
				    ...
				    else if((c->addr_iter = BIO_ADDRINFO_next(c->addr_iter)) != NULL) {
					    /*
...
{
	BIO_CONNECT * ret;
	if((ret = static_cast<BIO_CONNECT *>(OPENSSL_zalloc(sizeof(*ret)))) == NULL) {
		...
		case BIO_C_SET_CONNECT:
		    if(ptr) {
			    ...
			    if(num == 0) {
				    ...
				    ret = BIO_parse_hostserv(static_cast<const char *>(ptr), &data->param_hostname, &data->param_service, BIO_PARSE_PRIO_HOST);
				    if(hold_service != data->param_service)
					    OPENSSL_free(hold_service);
			    }
			    else if(num == 1) {
				    OPENSSL_free(data->param_service);
				    data->param_service = BUF_strdup(static_cast<const char *>(ptr));
			    }
			    else if(num == 2) {
