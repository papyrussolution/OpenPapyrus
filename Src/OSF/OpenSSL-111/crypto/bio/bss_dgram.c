...
static void dgram_adjust_rcv_timeout(BIO * b)
{
	...
	if(data->next_timeout.tv_sec > 0 || data->next_timeout.tv_usec > 0) {
		...
		sz.i = sizeof(timeout);
		if(getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char *>(&timeout), &sz.i) < 0) {
			perror("getsockopt");
		}
		else {
	...
	switch(cmd) {
		...
		case BIO_CTRL_DGRAM_SET_CONNECTED:
		    if(ptr != NULL) {
			    ...
			    data->connected = 0;
			    memzero(&data->peer, sizeof(data->peer));
		    }
		    break;
		...
		case BIO_CTRL_DGRAM_SET_SEND_TIMEOUT:
#ifdef OPENSSL_SYS_WINDOWS
		    {
			    struct timeval * tv = (struct timeval *)ptr;
			    int timeout = tv->tv_sec * 1000 + tv->tv_usec / 1000;
			    if(setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char *>(&timeout), sizeof(timeout)) < 0) {
				    perror("setsockopt");
				    ret = -1;
			    }
		    }
		...
		case BIO_CTRL_DGRAM_GET_SEND_TIMEOUT:
	    {
		    ...
		    struct timeval * tv = (struct timeval *)ptr;
		    sz.i = sizeof(timeout);
		    if(getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<char *>(&timeout), &sz.i) < 0) {
			    perror("getsockopt");
			    ret = -1;
		    }
		    else {
		...
		case BIO_CTRL_DGRAM_SET_DONT_FRAG:
		    ...
		    switch(data->peer.sa.sa_family) {
				...
				if((ret = setsockopt(b->num, IPPROTO_IPV6, IPV6_DONTFRAG, reinterpret_cast<const char *>(&sockopt_val), sizeof(sockopt_val))) < 0) {
					perror("setsockopt");
					ret = -1;
				}
...
static int dgram_sctp_write(BIO * b, const char * in, int inl)
{
	 ...
	 */
	if(in[0] != 23) {
		memzero(&handshake_sinfo, sizeof(handshake_sinfo));
	...
	memzero(sndinfo, sizeof(*sndinfo));
