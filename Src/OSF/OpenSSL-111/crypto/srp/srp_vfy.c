...
{
	SRP_user_pwd * ret;
	if((ret = static_cast<SRP_user_pwd *>(OPENSSL_malloc(sizeof(*ret)))) == NULL) {
...
{
	SRP_VBASE * vb = static_cast<SRP_VBASE *>(OPENSSL_malloc(sizeof(*vb)));
	if(vb == NULL)
		return NULL;
...
int SRP_VBASE_init(SRP_VBASE * vb, char * verifier_file)
{
	...
	for(i = 0; i < sk_OPENSSL_PSTRING_num(tmpdb->data); i++) {
		...
		if(pp[DB_srptype][0] == DB_SRP_INDEX) {
			 ...
			 */
			if((gN = static_cast<SRP_gN *>(OPENSSL_malloc(sizeof(*gN)))) == NULL)
				goto err;
    ...
    char ** verifier, const char * N, const char * g)
{
	...
	if(((vf = static_cast<char *>(OPENSSL_malloc(vfsize))) == NULL))
		goto err;
	...
	if(*salt == NULL) {
		...
		if((tmp_salt = static_cast<char *>(OPENSSL_malloc(SRP_RANDOM_SALT_LEN * 2))) == NULL) {
			goto err;
		}
