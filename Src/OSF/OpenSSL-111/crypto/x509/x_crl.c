...
static int crl_set_issuers(X509_CRL * crl)
{
	...
	for(i = 0; i < sk_X509_REVOKED_num(revoked); i++) {
		...
		X509_EXTENSION * ext;
		gtmp = static_cast<GENERAL_NAMES *>(X509_REVOKED_get_ext_d2i(rev, NID_certificate_issuer, &j, NULL));
		if(!gtmp && (j != -1)) {
		...
		rev->issuer = gens;
		reason = static_cast<ASN1_ENUMERATED *>(X509_REVOKED_get_ext_d2i(rev, NID_crl_reason, &j, NULL));
		if(!reason && (j != -1)) {
			crl->flags |= EXFLAG_INVALID;
			return 1;
		}
...
static int crl_cb(int operation, ASN1_VALUE ** pval, const ASN1_ITEM * it, void * exarg)
{
	...
	switch(operation) {
		...
		case ASN1_OP_D2I_POST:
		    ...
		    crl->idp = static_cast<ISSUING_DIST_POINT *>(X509_CRL_get_ext_d2i(crl, NID_issuing_distribution_point, NULL, NULL));
		    if(crl->idp)
			    setup_idp(crl, crl->idp);
		    crl->akid = static_cast<AUTHORITY_KEYID *>(X509_CRL_get_ext_d2i(crl, NID_authority_key_identifier, NULL, NULL));
		    crl->crl_number = static_cast<ASN1_INTEGER *>(X509_CRL_get_ext_d2i(crl, NID_crl_number, NULL, NULL));
		    crl->base_crl_number = static_cast<ASN1_INTEGER *>(X509_CRL_get_ext_d2i(crl, NID_delta_crl, NULL, NULL));
...
X509_CRL_METHOD * X509_CRL_METHOD_new(int (*crl_init)(X509_CRL * crl),
    ...
    EVP_PKEY * pk))
{
	X509_CRL_METHOD * m = static_cast<X509_CRL_METHOD *>(OPENSSL_malloc(sizeof(*m)));
	if(m == NULL) {
