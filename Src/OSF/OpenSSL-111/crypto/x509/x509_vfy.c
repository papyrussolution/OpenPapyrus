...
#define CRL_SCORE_VALID (CRL_SCORE_NOCRITICAL|CRL_SCORE_TIME|CRL_SCORE_SCOPE) /* If this score or above CRL is probably valid */
...
static int get_issuer_sk(X509 ** issuer, X509_STORE_CTX * ctx, X509 * x)
{
	*issuer = find_issuer(ctx, static_cast<STACK_OF(X509) *>(ctx->other_ctx), x);
	if(*issuer) {
		X509_up_ref(*issuer);
		return 1;
	}
...
static STACK_OF(X509) *lookup_certs_sk(X509_STORE_CTX *ctx, X509_NAME *nm)
{
	...
	X509 * x;
	int i;
	for(i = 0; i < sk_X509_num(static_cast<STACK_OF(X509) *>(ctx->other_ctx)); i++) {
		x = sk_X509_value(static_cast<STACK_OF(X509) *>(ctx->other_ctx), i);
...
static int has_san_id(X509 * x, int gtype)
{
	int i;
	int ret = 0;
	GENERAL_NAMES * gs = static_cast<GENERAL_NAMES *>(X509_get_ext_d2i(x, NID_subject_alt_name, NULL, NULL));
	if(gs == NULL)
		return 0;
