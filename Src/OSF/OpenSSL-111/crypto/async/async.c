...
static async_ctx * async_ctx_new(void)
{
	...
	if(!ossl_init_thread_start(OPENSSL_INIT_THREAD_ASYNC))
		return NULL;
	nctx = static_cast<async_ctx *>(OPENSSL_malloc(sizeof(*nctx)));
	if(nctx == NULL) {
...
{
	ASYNC_JOB * job = static_cast<ASYNC_JOB *>(OPENSSL_zalloc(sizeof(*job)));
	if(job == NULL) {
...
int ASYNC_init_thread(size_t max_size, size_t init_size)
{
	...
	if(!ossl_init_thread_start(OPENSSL_INIT_THREAD_ASYNC))
		return 0;
	pool = static_cast<async_pool *>(OPENSSL_zalloc(sizeof(*pool)));
	if(pool == NULL) {
