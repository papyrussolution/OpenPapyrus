    ...
    const EVP_MD * md, ENGINE * impl)
{
	...
	if(key != NULL) {
		...
		if(!ossl_assert(j <= (int)sizeof(ctx->key)))
			return 0;
		if(j < len) {
			if(!EVP_DigestInit_ex(ctx->md_ctx, md, impl) || !EVP_DigestUpdate(ctx->md_ctx, key, len) || !EVP_DigestFinal_ex(ctx->md_ctx, ctx->key, &ctx->key_length))
				return 0;
		}
		else {
		...
		if(ctx->key_length != HMAC_MAX_MD_CBLOCK_SIZE)
			memzero(&ctx->key[ctx->key_length], HMAC_MAX_MD_CBLOCK_SIZE - ctx->key_length);
	}
