    ...
    uchar * out, const EVP_MD * md_type)
{
	...
	if(u < 0 || v <= 0)
		goto err;
	D = static_cast<uchar *>(OPENSSL_malloc(v));
	Ai = static_cast<uchar *>(OPENSSL_malloc(u));
	B = static_cast<uchar *>(OPENSSL_malloc(v + 1));
	...
	if(passlen)
		...
		Plen = 0;
	Ilen = Slen + Plen;
	I = static_cast<uchar *>(OPENSSL_malloc(Ilen));
