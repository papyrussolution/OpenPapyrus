...
static int pkey_tls1_prf_init(EVP_PKEY_CTX * ctx)
{
	...
	if((kctx = static_cast<TLS1_PRF_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*kctx)))) == NULL) {
...
static int pkey_tls1_prf_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_TLS_SECRET:
		    ...
		    OPENSSL_cleanse(kctx->seed, kctx->seedlen);
		    kctx->seedlen = 0;
		    kctx->sec = static_cast<uchar *>(OPENSSL_memdup(p2, p1));
		    if(kctx->sec == NULL)
			    return 0;
		    kctx->seclen  = p1;
		    return 1;
    ...
    uchar * out, size_t olen)
{
	if(EVP_MD_type(md) == NID_md5_sha1) {
		...
		if((tmp = static_cast<uchar *>(OPENSSL_malloc(olen))) == NULL) {
