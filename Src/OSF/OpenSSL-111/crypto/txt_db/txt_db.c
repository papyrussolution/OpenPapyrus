...
TXT_DB * TXT_DB_read(BIO * in, int num)
{
	...
	if(!BUF_MEM_grow(buf, size))
		goto err;
	if((ret = static_cast<TXT_DB *>(OPENSSL_malloc(sizeof(*ret)))) == NULL)
		goto err;
	ret->num_fields = num;
	ret->index = NULL;
	ret->qual = NULL;
	if((ret->data = sk_OPENSSL_PSTRING_new_null()) == NULL)
		goto err;
	if((ret->index = static_cast<LHASH_OF(OPENSSL_STRING) **>(OPENSSL_malloc(sizeof(*ret->index) * num))) == NULL)
		goto err;
	if((ret->qual = static_cast<int (**)(OPENSSL_STRING *)>(OPENSSL_malloc(sizeof(*(ret->qual)) * num))) == NULL)
		goto err;
	for(i = 0; i < num; i++) {
		ret->index[i] = NULL;
		ret->qual[i] = NULL;
	...
	for(;;) {
		...
		if(buf->data[offset - 1] != '\n')
			...
			if((p = static_cast<char *>(OPENSSL_malloc(add + offset))) == NULL)
				goto err;
			offset = 0;
		}
		pp = (char **)p;
