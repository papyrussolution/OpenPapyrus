...
static int pkey_siphash_init(EVP_PKEY_CTX * ctx)
{
	...
	if((pctx = static_cast<SIPHASH_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*pctx)))) == NULL) {
...
static int pkey_siphash_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_siphash_init(dst))
		return 0;
	sctx = static_cast<SIPHASH_PKEY_CTX *>(EVP_PKEY_CTX_get_data(src));
	dctx = static_cast<SIPHASH_PKEY_CTX *>(EVP_PKEY_CTX_get_data(dst));
...
{
	SIPHASH_PKEY_CTX * pctx = static_cast<SIPHASH_PKEY_CTX *>(EVP_PKEY_CTX_get_data(EVP_MD_CTX_pkey_ctx(ctx)));
	SipHash_Update(&pctx->ctx, static_cast<const uchar *>(data), count);
	return 1;
}
...
static int siphash_signctx(EVP_PKEY_CTX * ctx, uchar * sig, size_t * siglen,
    EVP_MD_CTX * mctx)
{
	SIPHASH_PKEY_CTX * pctx = static_cast<SIPHASH_PKEY_CTX *>(ctx->data);
