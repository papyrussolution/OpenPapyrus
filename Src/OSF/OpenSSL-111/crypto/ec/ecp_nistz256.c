    ...
    const EC_POINT ** point, size_t num, BN_CTX * ctx)
{
	...
	table = reinterpret_cast<P256_POINT(*)[16]>(ALIGNPTR(table_storage, 64));
	...
	for(i = 0; i < num; i++) {
		...
		for(j = 0; j < bn_get_top(scalars[i]) * BN_BYTES; j += BN_BYTES) {
			...
			p_str[i][j + 0] = (uchar)d;
			p_str[i][j + 1] = (uchar)(d >> 8);
			p_str[i][j + 2] = (uchar)(d >> 16);
			p_str[i][j + 3] = (uchar)(d >>= 24);
			if(BN_BYTES == 8) {
				d >>= 8;
				p_str[i][j + 4] = (uchar)d;
				p_str[i][j + 5] = (uchar)(d >> 8);
				p_str[i][j + 6] = (uchar)(d >> 16);
				p_str[i][j + 7] = (uchar)(d >> 24);
			}
		}
		for(; j < 33; j++)
			p_str[i][j] = 0;
...
__owur static int ecp_nistz256_mult_precompute(EC_GROUP * group, BN_CTX * ctx)
{
	...
	w = 7;
	if((precomp_storage = static_cast<uchar *>(OPENSSL_malloc(37 * 64 * sizeof(P256_POINT_AFFINE) + 64))) == NULL) {
		ECerr(EC_F_ECP_NISTZ256_MULT_PRECOMPUTE, ERR_R_MALLOC_FAILURE);
		goto err;
	}
	preComputedTable = reinterpret_cast<PRECOMP256_ROW *>(ALIGNPTR(precomp_storage, 64));
    ...
    const BIGNUM * scalars[], BN_CTX * ctx)
{
		...
		if(preComputedTable) {
			...
			for(i = 0; i < bn_get_top(scalar) * BN_BYTES; i += BN_BYTES) {
				...
				if(BN_BYTES == 8) {
					...
					p_str[i + 4] = (uchar)d;
					p_str[i + 5] = (uchar)(d >> 8);
					p_str[i + 6] = (uchar)(d >> 16);
					p_str[i + 7] = (uchar)(d >> 24);
				}
			}
	...
	if(no_precomp_for_generator) {
		 ...
		 */
		new_scalars = static_cast<const BIGNUM **>(OPENSSL_malloc((num + 1) * sizeof(BIGNUM *)));
		if(new_scalars == NULL) {
			ECerr(EC_F_ECP_NISTZ256_POINTS_MUL, ERR_R_MALLOC_FAILURE);
			goto err;
		}

		new_points = static_cast<const EC_POINT **>(OPENSSL_malloc((num + 1) * sizeof(EC_POINT *)));
		if(new_points == NULL) {
			ECerr(EC_F_ECP_NISTZ256_POINTS_MUL, ERR_R_MALLOC_FAILURE);
			goto err;
		}
...
static NISTZ256_PRE_COMP * ecp_nistz256_pre_comp_new(const EC_GROUP * group)
{
	...
	if(!group)
		return NULL;
	ret = static_cast<NISTZ256_PRE_COMP *>(OPENSSL_zalloc(sizeof(*ret)));
	if(ret == NULL) {
