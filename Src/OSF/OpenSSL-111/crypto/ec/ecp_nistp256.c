 ...
 * smallfelems
 */
static void point_double_small(smallfelem x_out, smallfelem y_out, smallfelem z_out,
    ...
    const BIGNUM * scalars[], BN_CTX * ctx)
{
	...
	if(num_points > 0) {
		 ...
		 */
		memzero(secrets, sizeof(*secrets) * num_points);
		memzero(pre_comp, sizeof(*pre_comp) * num_points);
	...
	if((scalar != NULL) && (have_pre_comp)) {
		memzero(g_secret, sizeof(g_secret));
		/* reduce scalar to 0 <= scalar < 2^256 */
...
int ec_GFp_nistp256_precompute_mult(EC_GROUP * group, BN_CTX * ctx)
{
	...
	for(i = 0; i < 2; i++) {
		...
		memzero(pre->g_pre_comp[i][0], sizeof(pre->g_pre_comp[i][0]));
