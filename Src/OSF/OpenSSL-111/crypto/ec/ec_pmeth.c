...
{
	EC_PKEY_CTX * dctx;
	if((dctx = static_cast<EC_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*dctx)))) == NULL) {
...
static int pkey_ec_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_ec_init(dst))
		return 0;
	sctx = static_cast<EC_PKEY_CTX *>(src->data);
	dctx = static_cast<EC_PKEY_CTX *>(dst->data);
	if(sctx->gen_group) {
    ...
    uchar * key, size_t * keylen)
{
	...
	if(!pkey_ec_derive(ctx, NULL, &ktmplen))
		return 0;
	if((ktmp = static_cast<uchar *>(OPENSSL_malloc(ktmplen))) == NULL) {
...
static int pkey_ec_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_MD:
		    ...
		    dctx->md = static_cast<const EVP_MD *>(p2);
		    return 1;
