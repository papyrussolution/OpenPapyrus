...
static EC_PRE_COMP * ec_pre_comp_new(const EC_GROUP * group)
{
	...
	if(!group)
		return NULL;
	ret = static_cast<EC_PRE_COMP *>(OPENSSL_zalloc(sizeof(*ret)));
	if(ret == NULL) {
    ...
    BN_CTX * ctx)
{
	...
	wsize = static_cast<size_t *>(OPENSSL_malloc(totalnum * sizeof(wsize[0])));
	wNAF_len = static_cast<size_t *>(OPENSSL_malloc(totalnum * sizeof(wNAF_len[0])));
	/* include space for pivot */
	wNAF = static_cast<signed char **>(OPENSSL_malloc((totalnum + 1) * sizeof(wNAF[0])));
	 ...
	 */
	val = static_cast<EC_POINT **>(OPENSSL_malloc((num_val + 1) * sizeof(val[0])));
	if(val == NULL) {
		ECerr(EC_F_EC_WNAF_MUL, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
int ec_wNAF_precompute_mult(EC_GROUP * group, BN_CTX * ctx)
{
	...
	points = static_cast<EC_POINT **>(OPENSSL_malloc(sizeof(*points) * (num + 1)));
	if(points == NULL) {
