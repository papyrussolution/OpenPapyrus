...
static int eckey_pub_encode(X509_PUBKEY * pk, const EVP_PKEY * pkey)
{
	...
	if(penclen <= 0)
		goto err;
	penc = static_cast<uchar *>(OPENSSL_malloc(penclen));
	if(penc == NULL)
		goto err;
	p = penc;
...
err:
	if(ptype == V_ASN1_OBJECT)
		ASN1_OBJECT_free(static_cast<ASN1_OBJECT *>(pval));
	else
		ASN1_STRING_free(static_cast<ASN1_STRING *>(pval));
	OPENSSL_free(penc);
	return 0;
}
...
static EC_KEY * eckey_type2param(int ptype, const void * pval)
{
	...
	if(ptype == V_ASN1_SEQUENCE) {
		...
		const ASN1_OBJECT * poid = static_cast<const ASN1_OBJECT *>(pval);
		/*
...
static int eckey_priv_encode(PKCS8_PRIV_KEY_INFO * p8, const EVP_PKEY * pkey)
{
	...
	ep = static_cast<uchar *>(OPENSSL_malloc(eplen));
	if(ep == NULL) {
...
static int ec_pkey_ctrl(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		...
		case ASN1_PKEY_CTRL_CMS_ENVELOPE:
		    if(arg1 == 1)
			    return ecdh_cms_decrypt(static_cast<CMS_RecipientInfo *>(arg2));
		    else if(arg1 == 0)
			    return ecdh_cms_encrypt(static_cast<CMS_RecipientInfo *>(arg2));
		    return -2;
		...
		case ASN1_PKEY_CTRL_SET1_TLS_ENCPT:
		    return EC_KEY_oct2key(EVP_PKEY_get0_EC_KEY(pkey), static_cast<const uchar *>(arg2), arg1, NULL);
		case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
		    return EC_KEY_key2buf(EVP_PKEY_get0_EC_KEY(pkey), POINT_CONVERSION_UNCOMPRESSED, static_cast<uchar **>(arg2), NULL);
		default:
		    return -2;
	}
}
