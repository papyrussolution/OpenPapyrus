...
{
	int err;
	struct zlib_state * state = static_cast<zlib_state *>(OPENSSL_zalloc(sizeof(*state)));
	if(state == NULL)
		goto err;
...
static int bio_zlib_free(BIO * bi)
{
	...
	if(!bi)
		return 0;
	ctx = static_cast<BIO_ZLIB_CTX *>(BIO_get_data(bi));
	if(ctx->ibuf) {
...
static int bio_zlib_read(BIO * b, char * out, int outl)
{
	...
	if(!ctx->ibuf) {
		ctx->ibuf = static_cast<uchar *>(OPENSSL_malloc(ctx->ibufsize));
		if(ctx->ibuf == NULL) {
...
static int bio_zlib_flush(BIO * b)
{
	...
	z_stream * zout;
	BIO * next = BIO_next(b);
	BIO_ZLIB_CTX * ctx = static_cast<BIO_ZLIB_CTX *>(BIO_get_data(b));
