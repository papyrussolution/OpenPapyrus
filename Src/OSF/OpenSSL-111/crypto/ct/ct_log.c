...
{
	CTLOG_STORE * ret = static_cast<CTLOG_STORE *>(OPENSSL_zalloc(sizeof(*ret)));
	if(ret == NULL) {
...
{
	CTLOG_STORE_LOAD_CTX * load_ctx = static_cast<CTLOG_STORE_LOAD_CTX *>(arg);
	CTLOG * ct_log = NULL;
...
{
	CTLOG * ret = static_cast<CTLOG *>(OPENSSL_zalloc(sizeof(*ret)));
	if(ret == NULL) {
