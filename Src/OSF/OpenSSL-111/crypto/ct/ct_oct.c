...
SCT * o2i_SCT(SCT ** psct, const uchar ** in, size_t len)
{
	...
	if((sct = SCT_new()) == NULL)
		goto err;
	p = *in;
	sct->version = static_cast<sct_version_t>(*p);
	if(sct->version == SCT_VERSION_V1) {
		int sig_len;
		size_t len2;
		/*-
		...
		len -= 43;
		p++;
		sct->log_id = static_cast<uchar *>(BUF_memdup(p, CT_V1_HASHLEN));
		if(sct->log_id == NULL)
			goto err;
		...
		if(len2 > 0) {
			sct->ext = static_cast<uchar *>(BUF_memdup(p, len2));
			if(sct->ext == NULL)
				goto err;
		}
		sct->ext_len = len2;
		...
		*in = p + len;
		...
		sct->sct = static_cast<uchar *>(BUF_memdup(p, len));
		if(sct->sct == NULL)
			goto err;
		sct->sct_len = len;
		*in = p + len;
