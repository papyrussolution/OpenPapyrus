...
CRYPTO_RWLOCK * CRYPTO_THREAD_lock_new(void)
{
	...
	if(!InitializeCriticalSectionAndSpinCount(static_cast<LPCRITICAL_SECTION>(lock), 0x400)) {
		OPENSSL_free(lock);
		return NULL;
	}
#else
...
{
	*ret = InterlockedExchangeAdd(reinterpret_cast<volatile LONG *>(val), amount) + amount;
	return 1;
}
