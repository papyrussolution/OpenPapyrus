...
{
	OSSL_STORE_INFO * info = static_cast<OSSL_STORE_INFO *>(OPENSSL_zalloc(sizeof(*info)));
	if(info == NULL)
		return NULL;
	info->type = type;
	info->_.data = data;
...
OSSL_STORE_INFO * OSSL_STORE_INFO_new_NAME(char * name)
{
	...
	if(info == NULL) {
		OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_INFO_NEW_NAME, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
OSSL_STORE_SEARCH * OSSL_STORE_SEARCH_by_issuer_serial(X509_NAME * name, const ASN1_INTEGER * serial)
{
	OSSL_STORE_SEARCH * search = static_cast<OSSL_STORE_SEARCH *>(OPENSSL_zalloc(sizeof(*search)));
	if(search == NULL) {
		OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_SEARCH_BY_ISSUER_SERIAL, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
OSSL_STORE_SEARCH * OSSL_STORE_SEARCH_by_alias(const char * alias)
{
	OSSL_STORE_SEARCH * search = static_cast<OSSL_STORE_SEARCH *>(OPENSSL_zalloc(sizeof(*search)));
	if(search == NULL) {
		OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_SEARCH_BY_ALIAS, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
OSSL_STORE_CTX * ossl_store_attach_pem_bio(BIO * bp, const UI_METHOD * ui_method, void * ui_data)
{
	OSSL_STORE_CTX * ctx = NULL;
	const OSSL_STORE_LOADER * loader = NULL;
	OSSL_STORE_LOADER_CTX * loader_ctx = NULL;
