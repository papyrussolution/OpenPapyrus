...
OSSL_STORE_LOADER * OSSL_STORE_LOADER_new(ENGINE * e, const char * scheme)
{
	...
	if((res = static_cast<OSSL_STORE_LOADER *>(OPENSSL_zalloc(sizeof(*res)))) == NULL) {
...
{
	OSSL_STORE_LOADER __template;
	OSSL_STORE_LOADER * loader = NULL;
	__template.scheme = scheme;
	__template.open = NULL;
	__template.load = NULL;
	__template.eof = NULL;
	__template.close = NULL;
	if(!ossl_store_init_once())
		return NULL;
	if(!RUN_ONCE(&registry_init, do_registry_init)) {
		OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_GET0_LOADER_INT, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
	CRYPTO_THREAD_write_lock(registry_lock);
	loader = lh_OSSL_STORE_LOADER_retrieve(loader_register, &__template);
	if(loader == NULL) {
		OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_GET0_LOADER_INT, OSSL_STORE_R_UNREGISTERED_SCHEME);
...
OSSL_STORE_LOADER * ossl_store_unregister_loader_int(const char * scheme)
{
	...
	loader = lh_OSSL_STORE_LOADER_delete(loader_register, &__template);
	if(loader == NULL) {
