...
{
	struct pem_pass_data * pass_data = static_cast<struct pem_pass_data *>(data);
...
{
	STACK_OF(OSSL_STORE_INFO) *ctx = static_cast<STACK_OF(OSSL_STORE_INFO) *>(ctx_);
...
static OSSL_STORE_LOADER_CTX * file_open(const OSSL_STORE_LOADER * loader, const char * uri, const UI_METHOD * ui_method, void * ui_data)
{
	...
	if(S_ISDIR(st.st_mode)) {
		...
		BIO * buff = NULL;
		char peekbuf[4096] = { 0, };
		if((buff = BIO_new(BIO_f_buffer())) == NULL || (ctx->_.file.file = BIO_new_file(path, "rb")) == NULL) {
			BIO_free_all(buff);
			goto err;
		}
