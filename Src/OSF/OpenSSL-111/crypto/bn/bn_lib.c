...
int FASTCALL BN_num_bits(const BIGNUM * a)
{
	...
	if(a->flags & BN_FLG_CONSTTIME) {
		 ...
		 * been preallocated in advance to a public length if we hit this branch.
		 *
		 */
...
void BN_with_flags(BIGNUM * dest, const BIGNUM * b, int flags)
{
	...
	dest->dmax = b->dmax;
	dest->neg = b->neg;
	dest->flags = ((dest->flags & BN_FLG_MALLOCED) | (b->flags & ~BN_FLG_MALLOCED) | BN_FLG_STATIC_DATA | flags);
}

BN_GENCB * BN_GENCB_new(void)
{
	BN_GENCB * ret;
	if((ret = static_cast<BN_GENCB *>(OPENSSL_malloc(sizeof(*ret)))) == NULL) {
