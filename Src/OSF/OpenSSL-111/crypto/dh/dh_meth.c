...
{
	DH_METHOD * dhm = static_cast<DH_METHOD *>(OPENSSL_zalloc(sizeof(*dhm)));
	if(dhm != NULL) {
		dhm->flags = flags;
...
{
	DH_METHOD * ret = static_cast<DH_METHOD *>(OPENSSL_malloc(sizeof(*ret)));
	if(ret != NULL) {
...
{
	dhm->app_data = static_cast<char *>(app_data);
	return 1;
}
