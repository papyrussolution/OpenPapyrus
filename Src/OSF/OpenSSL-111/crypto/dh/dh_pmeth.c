...
{
	DH_PKEY_CTX * dctx;
	if((dctx = static_cast<DH_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*dctx)))) == NULL) {
...
static int pkey_dh_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_dh_init(dst))
		return 0;
	sctx = static_cast<DH_PKEY_CTX *>(src->data);
	dctx = static_cast<DH_PKEY_CTX *>(dst->data);
	...
	if(sctx->kdf_ukm != NULL) {
		dctx->kdf_ukm = static_cast<uchar *>(OPENSSL_memdup(sctx->kdf_ukm, sctx->kdf_ukmlen));
...
static int pkey_dh_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_DH_KDF_MD:
		    dctx->kdf_md = static_cast<const EVP_MD *>(p2);
		    return 1;
		...
		case EVP_PKEY_CTRL_DH_KDF_UKM:
		    OPENSSL_free(dctx->kdf_ukm);
		    dctx->kdf_ukm = static_cast<uchar *>(p2);
		    if(p2)
			    dctx->kdf_ukmlen = p1;
		    else
			    dctx->kdf_ukmlen = 0;
		...
		case EVP_PKEY_CTRL_DH_KDF_OID:
		    ASN1_OBJECT_free(dctx->kdf_oid);
		    dctx->kdf_oid = static_cast<ASN1_OBJECT *>(p2);
		    return 1;
...
{
	if(sstreq(type, "dh_paramgen_prime_len")) {
		int len;
		len = atoi(value);
	...
	if(sstreq(type, "dh_paramgen_generator")) {
		int len;
		len = atoi(value);
		return EVP_PKEY_CTX_set_dh_paramgen_generator(ctx, len);
	}
	if(sstreq(type, "dh_paramgen_subprime_len")) {
		int len;
		len = atoi(value);
		return EVP_PKEY_CTX_set_dh_paramgen_subprime_len(ctx, len);
	}
	if(sstreq(type, "dh_paramgen_type")) {
		int typ;
		typ = atoi(value);
		return EVP_PKEY_CTX_set_dh_paramgen_type(ctx, typ);
	}
	if(sstreq(type, "dh_pad")) {
		int pad;
		pad = atoi(value);
    ...
    size_t * keylen)
{
	...
	if(dctx->kdf_type == EVP_PKEY_DH_KDF_NONE) {
		...
		*keylen = ret;
		...
		ret = 0;
		Zlen = DH_size(dh);
		Z = static_cast<uchar *>(OPENSSL_malloc(Zlen));
		if(Z == NULL) {
			goto err;
		}
