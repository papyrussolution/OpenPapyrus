...
static int pkey_poly1305_init(EVP_PKEY_CTX * ctx)
{
	...
	if((pctx = static_cast<POLY1305_PKEY_CTX *>(OPENSSL_zalloc(sizeof(*pctx)))) == NULL) {
...
static int pkey_poly1305_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_poly1305_init(dst))
		return 0;
	sctx = static_cast<POLY1305_PKEY_CTX *>(EVP_PKEY_CTX_get_data(src));
	dctx = static_cast<POLY1305_PKEY_CTX *>(EVP_PKEY_CTX_get_data(dst));
