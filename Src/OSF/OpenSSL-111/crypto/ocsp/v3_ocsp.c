...
{
	OCSP_CRLID * a = static_cast<OCSP_CRLID *>(in);
	if(a->crlUrl) {
...
static int i2r_ocsp_acutoff(const X509V3_EXT_METHOD * method, void * cutoff, BIO * bp, int ind)
{
	if(BIO_printf(bp, "%*s", ind, "") <= 0)
		return 0;
	if(!ASN1_GENERALIZEDTIME_print(bp, static_cast<ASN1_GENERALIZEDTIME *>(cutoff)))
		return 0;
	return 1;
}
...
static int i2r_object(const X509V3_EXT_METHOD * method, void * oid, BIO * bp, int ind)
{
	if(BIO_printf(bp, "%*s", ind, "") <= 0)
		return 0;
	if(i2a_ASN1_OBJECT(bp, static_cast<ASN1_OBJECT *>(oid)) <= 0)
		return 0;
	return 1;
}
/*
...
static void ocsp_nonce_free(void * a)
{
	ASN1_OCTET_STRING_free(static_cast<ASN1_OCTET_STRING *>(a));
}
    ...
    BIO * out, int indent)
{
	if(BIO_printf(out, "%*s", indent, "") <= 0)
		return 0;
	if(i2a_ASN1_STRING(out, static_cast<ASN1_STRING *>(nonce), V_ASN1_OCTET_STRING) <= 0)
		return 0;
	return 1;
}
...
{
	int i;
	OCSP_SERVICELOC * a = static_cast<OCSP_SERVICELOC *>(in);
