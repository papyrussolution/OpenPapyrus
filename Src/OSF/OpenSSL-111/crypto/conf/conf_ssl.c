...
{
	if(ssl_names) {
		for(size_t i = 0; i < ssl_names_count; i++) {
...
static int ssl_module_init(CONF_IMODULE * md, const CONF * cnf)
{
	size_t i, j, cnt;
	int rv = 0;
	const char * ssl_conf_section = CONF_imodule_get_value(md);
	STACK_OF(CONF_VALUE) * cmd_lists = NCONF_get_section(cnf, ssl_conf_section);
	...
	ssl_module_free(md);
	ssl_names = static_cast<ssl_conf_name_st *>(OPENSSL_zalloc(sizeof(*ssl_names) * cnt));
	if(ssl_names == NULL)
		goto err;
	ssl_names_count = cnt;
	for(i = 0; i < ssl_names_count; i++) {
		...
		if(sk_CONF_VALUE_num(cmds) <= 0) {
			if(cmds == NULL)
				CONFerr(CONF_F_SSL_MODULE_INIT, CONF_R_SSL_COMMAND_SECTION_NOT_FOUND);
			else
				CONFerr(CONF_F_SSL_MODULE_INIT, CONF_R_SSL_COMMAND_SECTION_EMPTY);
		...
		ssl_name->cmds = static_cast<SSL_CONF_CMD *>(OPENSSL_zalloc(cnt * sizeof(struct ssl_conf_cmd_st)));
