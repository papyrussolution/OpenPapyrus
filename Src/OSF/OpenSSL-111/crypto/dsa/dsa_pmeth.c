...
{
	DSA_PKEY_CTX * dctx = static_cast<DSA_PKEY_CTX *>(OPENSSL_malloc(sizeof(*dctx)));
	if(dctx == NULL)
		return 0;
	dctx->nbits = 2048;
	dctx->qbits = 224;
...
static int pkey_dsa_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_dsa_init(dst))
		return 0;
	sctx = static_cast<DSA_PKEY_CTX *>(src->data);
	dctx = static_cast<DSA_PKEY_CTX *>(dst->data);
...
static int pkey_dsa_ctrl(EVP_PKEY_CTX * ctx, int type, int p1, void * p2)
{
	...
	switch(type) {
		...
		case EVP_PKEY_CTRL_DSA_PARAMGEN_MD:
		    ...
		    dctx->pmd = static_cast<const EVP_MD *>(p2);
		    return 1;
    ...
    const char * type, const char * value)
{
	...
	if(sstreq(type, "dsa_paramgen_q_bits")) {
		int qbits = atoi(value);
		return EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_DSA, EVP_PKEY_OP_PARAMGEN,
			   EVP_PKEY_CTRL_DSA_PARAMGEN_Q_BITS, qbits,
			   NULL);
	}
	if(sstreq(type, "dsa_paramgen_md")) {
