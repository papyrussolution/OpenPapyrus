...
{
	OPENSSL_STACK * ret;
	if((ret = static_cast<OPENSSL_STACK *>(OPENSSL_malloc(sizeof(*ret)))) == NULL) {
	...
	*ret = *sk;
	...
	if((ret->data = static_cast<const void **>(OPENSSL_malloc(sizeof(*ret->data) * sk->num_alloc))) == NULL)
		goto err;
...
static int sk_reserve(OPENSSL_STACK * st, int n, int exact)
{
	...
	if(st->data == NULL) {
		 ...
		 */
		if((st->data = static_cast<const void **>(OPENSSL_zalloc(sizeof(void *) * num_alloc))) == NULL) {
...
{
	OPENSSL_STACK * st = static_cast<OPENSSL_STACK *>(OPENSSL_zalloc(sizeof(OPENSSL_STACK)));
	if(st == NULL)
		return NULL;
	st->comp = c;
	if(n <= 0)
