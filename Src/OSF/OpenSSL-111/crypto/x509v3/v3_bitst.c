...
{
	for(BIT_STRING_BITNAME * bnam = static_cast<BIT_STRING_BITNAME *>(method->usr_data); bnam->lname; bnam++) {
    ...
    STACK_OF(CONF_VALUE) * nval)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(nval); i++) {
		...
		for(bnam = static_cast<BIT_STRING_BITNAME *>(method->usr_data); bnam->lname; bnam++) {
