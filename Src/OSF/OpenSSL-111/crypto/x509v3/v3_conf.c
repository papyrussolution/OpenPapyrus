    ...
    int crit, const char * value)
{
	...
	if(method->it)
		ASN1_item_free(static_cast<ASN1_VALUE *>(ext_struc), ASN1_ITEM_ptr(method->it));
	else
...
static X509_EXTENSION * do_ext_i2d(const X509V3_EXT_METHOD * method, int ext_nid, int crit, void * ext_struc)
{
	...
	if(method->it) {
		ext_der = NULL;
		ext_len = ASN1_item_i2d(static_cast<ASN1_VALUE *>(ext_struc), &ext_der, ASN1_ITEM_ptr(method->it));
		if(ext_len < 0)
			goto merr;
	}
	else {
...
{
	return NCONF_get_string(static_cast<const CONF *>(db), section, value);
}

static STACK_OF(CONF_VALUE) *nconf_get_section(void * db, const char * section)
{
	return NCONF_get_section(static_cast<CONF *>(db), section);
}
...
static char * conf_lhash_get_string(void * db, const char * section, const char * value)
{
	return CONF_get_string(static_cast<LHASH_OF(CONF_VALUE) *>(db), section, value);
}

static STACK_OF(CONF_VALUE) *conf_lhash_get_section(void * db, const char * section)
{
	return CONF_get_section(static_cast<LHASH_OF(CONF_VALUE) *>(db), section);
}
