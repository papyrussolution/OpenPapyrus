...
static int policy_cache_new(X509 * x)
{
	...
	if(x->policy_cache != NULL)
		return 1;
	cache = static_cast<X509_POLICY_CACHE *>(OPENSSL_malloc(sizeof(*cache)));
	if(cache == NULL) {
	 ...
	 */
	ext_pcons = static_cast<POLICY_CONSTRAINTS *>(X509_get_ext_d2i(x, NID_policy_constraints, &i, NULL));
	if(!ext_pcons) {
		if(i != -1)
			goto bad_cache;
	}
	...
	ext_cpols = static_cast<CERTIFICATEPOLICIES *>(X509_get_ext_d2i(x, NID_certificate_policies, &i, NULL));
	/*
	...
	if(i <= 0)
		return i;
	ext_pmaps = static_cast<POLICY_MAPPINGS *>(X509_get_ext_d2i(x, NID_policy_mappings, &i, NULL));
	if(!ext_pmaps) {
		/* If not absent some problem with extension */
		if(i != -1)
			goto bad_cache;
	...
	ext_any = static_cast<ASN1_INTEGER *>(X509_get_ext_d2i(x, NID_inhibit_any_policy, &i, NULL));
	if(!ext_any) {
		if(i != -1)
			goto bad_cache;
	}
