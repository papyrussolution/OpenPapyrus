...
int X509V3_add_value(const char * name, const char * value, STACK_OF(CONF_VALUE) ** extlist)
{
	...
	if(value && (tvalue = OPENSSL_strdup(value)) == NULL)
		goto err;
	if((vtmp = static_cast<CONF_VALUE *>(OPENSSL_malloc(sizeof(*vtmp)))) == NULL)
		goto err;
...
static char * bignum_to_string(const BIGNUM * bn)
{
	...
	if(tmp == NULL)
		return NULL;
	len = strlen(tmp) + 3;
	ret = static_cast<char *>(OPENSSL_malloc(len));
	if(ret == NULL) {
...
int X509V3_get_value_bool(const CONF_VALUE * value, int * asn1_bool)
{
	...
	if(btmp == NULL)
		goto err;
	if(sstreqi_ascii(btmp, "true") || sstreqi_ascii(btmp, "y") || sstreqi_ascii(btmp, "yes")) {
		*asn1_bool = 0xff;
		return 1;
	}
	if(sstreqi_ascii(btmp, "false") || sstreqi_ascii(btmp, "n") || sstreqi_ascii(btmp, "no")) {
		*asn1_bool = 0;
		return 1;
	}
err:
...
{
	GENERAL_NAMES * gens = static_cast<GENERAL_NAMES *>(X509_get_ext_d2i(x, NID_subject_alt_name, NULL, NULL));
	STACK_OF(OPENSSL_STRING) * ret = get_email(X509_get_subject_name(x), gens);
...
STACK_OF(OPENSSL_STRING) *X509_get1_ocsp(X509 *x)
{
	...
	int i;
	AUTHORITY_INFO_ACCESS * info = static_cast<AUTHORITY_INFO_ACCESS *>(X509_get_ext_d2i(x, NID_info_access, NULL, NULL));
	if(!info)
		return NULL;
...
STACK_OF(OPENSSL_STRING) *X509_REQ_get1_email(X509_REQ *x)
{
	...
	GENERAL_NAMES * gens = static_cast<GENERAL_NAMES *>(X509V3_get_d2i(exts, NID_subject_alt_name, NULL, NULL));
	STACK_OF(OPENSSL_STRING) *ret = get_email(X509_REQ_get_subject_name(x), gens);
	sk_GENERAL_NAME_pop_free(gens, GENERAL_NAME_free);
...
static int do_x509_check(X509 * x, const char * chk, size_t chklen, uint flags, int check_type, char ** peername)
{
	...
	gens = static_cast<GENERAL_NAMES *>(X509_get_ext_d2i(x, NID_subject_alt_name, NULL, NULL));
	if(gens) {
		for(i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
			ASN1_STRING * cstr;
...
static int ipv6_cb(const char * elem, int len, void * usr)
{
	IPV6_STAT * s = static_cast<IPV6_STAT *>(usr);
