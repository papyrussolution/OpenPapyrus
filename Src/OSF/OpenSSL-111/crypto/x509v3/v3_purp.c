...
static X509_PURPOSE xstandard[] = {
	...
	{X509_PURPOSE_TIMESTAMP_SIGN, X509_TRUST_TSA, 0, check_purpose_timestamp_sign, "Time Stamp signing", "timestampsign",
	 NULL},
};
    ...
    const char * name, const char * sname, void * arg)
{
	...
	/* Need a new entry */
	if(idx == -1) {
		if((ptmp = static_cast<X509_PURPOSE *>(OPENSSL_malloc(sizeof(*ptmp)))) == NULL) {
...
static void setup_dp(X509 * x, DIST_POINT * dp)
{
	...
	SETIFZ(iname, X509_get_issuer_name(x));
	DIST_POINT_set_dpname(dp->distpoint, iname);
}

static void setup_crldp(X509 * x)
{
	x->crldp = static_cast<STACK_OF(DIST_POINT) *>(X509_get_ext_d2i(x, NID_crl_distribution_points, NULL, NULL));
	for(int i = 0; i < sk_DIST_POINT_num(x->crldp); i++)
...
static void x509v3_cache_extensions(X509 * x)
{
	...
	if((bs = static_cast<BASIC_CONSTRAINTS *>(X509_get_ext_d2i(x, NID_basic_constraints, NULL, NULL)))) {
		if(bs->ca)
	...
	/* Handle key usage */
	if((usage = static_cast<ASN1_BIT_STRING *>(X509_get_ext_d2i(x, NID_key_usage, NULL, NULL)))) {
		if(usage->length > 0) {
	...
	x->ex_xkusage = 0;
	if((extusage = static_cast<EXTENDED_KEY_USAGE *>(X509_get_ext_d2i(x, NID_ext_key_usage, NULL, NULL)))) {
	...
	if((ns = static_cast<ASN1_BIT_STRING *>(X509_get_ext_d2i(x, NID_netscape_cert_type, NULL, NULL)))) {
		if(ns->length > 0)
	...
	x->skid = static_cast<ASN1_OCTET_STRING *>(X509_get_ext_d2i(x, NID_subject_key_identifier, NULL, NULL));
	x->akid = static_cast<AUTHORITY_KEYID *>(X509_get_ext_d2i(x, NID_authority_key_identifier, NULL, NULL));
	...
	x->altname = static_cast<STACK_OF(GENERAL_NAME) *>(X509_get_ext_d2i(x, NID_subject_alt_name, NULL, NULL));
	x->nc = static_cast<NAME_CONSTRAINTS *>(X509_get_ext_d2i(x, NID_name_constraints, &i, NULL));
	if(!x->nc && (i != -1))
		x->ex_flags |= EXFLAG_INVALID;
	setup_crldp(x);

#ifndef OPENSSL_NO_RFC3779
	x->rfc3779_addr = static_cast<STACK_OF(IPAddressFamily) *>(X509_get_ext_d2i(x, NID_sbgp_ipAddrBlock, NULL, NULL));
	x->rfc3779_asid = static_cast<ASIdentifiers_st *>(X509_get_ext_d2i(x, NID_sbgp_autonomousSysNum, NULL, NULL));
#endif
