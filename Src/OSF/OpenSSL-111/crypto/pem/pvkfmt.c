...
static int do_i2b(unsigned char ** out, EVP_PKEY * pk, int ispub)
{
	...
	if(*out)
		...
		if((p = static_cast<uchar *>(OPENSSL_malloc(outlen))) == NULL) {
    ...
    const unsigned char * pass, int passlen)
{
	...
	int rv = 1;
	if(mctx == NULL || !EVP_DigestInit_ex(mctx, EVP_sha1(), NULL) || !EVP_DigestUpdate(mctx, salt, saltlen) || 
		!EVP_DigestUpdate(mctx, pass, passlen) || !EVP_DigestFinal_ex(mctx, key, NULL))
		rv = 0;
	EVP_MD_CTX_free(mctx);
	return rv;
}

static EVP_PKEY * do_PVK_body(const unsigned char ** in, unsigned int saltlen, unsigned int keylen, pem_password_cb * cb, void * u)
{
	EVP_PKEY * ret = NULL;
	const unsigned char * p = *in;
	unsigned int magic;
	...
	if(saltlen) {
		...
		enctmp = static_cast<uchar *>(OPENSSL_malloc(keylen + 8));
		if(enctmp == NULL) {
