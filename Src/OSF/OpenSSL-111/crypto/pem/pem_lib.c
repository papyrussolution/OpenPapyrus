...
int PEM_def_callback(char * buf, int num, int rwflag, void * userdata)
{
	...
	SETIFZ(prompt, "Enter PEM pass phrase:");
	/*
...
static int check_pem(const char * nm, const char * name)
{
	...
	if(strcmp(name, PEM_STRING_PARAMETERS) == 0) {
		const EVP_PKEY_ASN1_METHOD * ameth;
		int slen = pem_check_suffix(nm, "PARAMETERS");
		if(slen > 0) {
			ENGINE * e;
...
static void pem_free(void * p, uint flags, size_t num)
{
...
static void * pem_malloc(int num, uint flags)
{
	return (flags & PEM_FLAG_SECURE) ? OPENSSL_secure_malloc(num) : OPENSSL_malloc(num);
}

static int pem_bytes_read_bio_flags(uchar ** pdata, long * plen, char ** pnm, const char * name, BIO * bp, pem_password_cb * cb, void * u, uint flags)
{
    ...
    int klen, pem_password_cb * callback, void * u)
{
	...
	data = static_cast<uchar *>(OPENSSL_malloc((uint)dsize + 20));
	if(data == NULL) {
...
static int sanitize_line(char * linebuf, int len, uint flags)
{
	int i;
...
static int get_name(BIO * bp, char ** name, uint flags)
{
	char * linebuf;
	int ret = 0;
	int len;
	 ...
	 */
	linebuf = static_cast<char *>(pem_malloc(LINESIZE + 1, flags));
	if(linebuf == NULL) {
...
int PEM_read_bio_ex(BIO * bp, char ** name_out, char ** header,
    uchar ** data, long * len_out, uint flags)
{
	EVP_ENCODE_CTX * ctx = EVP_ENCODE_CTX_new();
	...
	*data = static_cast<uchar *>(pem_malloc(len, flags));
