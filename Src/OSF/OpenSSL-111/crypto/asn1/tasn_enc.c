...
static int asn1_item_flags_i2d(ASN1_VALUE * val, uchar ** out, const ASN1_ITEM * it, int flags)
{
	if(out && !*out) {
		uchar * p, * buf;
		int len = ASN1_item_ex_i2d(&val, NULL, it, -1, flags);
		if(len <= 0)
			return len;
...
int ASN1_item_ex_i2d(ASN1_VALUE ** pval, uchar ** out, const ASN1_ITEM * it, int tag, int aclass)
{
	...
	switch(it->itype) {
		...
		case ASN1_ITYPE_EXTERN:
		    ...
		    ef = static_cast<const ASN1_EXTERN_FUNCS *>(it->funcs);
		...
		case ASN1_ITYPE_SEQUENCE:
		    ...
		    for(i = 0, tt = it->templates; i < it->tcount; tt++, i++) {
			    ...
			    if(!seqtt)
					...
					int tmplen = asn1_template_ex_i2d(pseqval, NULL, seqtt, -1, aclass);
...
static int asn1_template_ex_i2d(ASN1_VALUE ** pval, uchar ** out, const ASN1_TEMPLATE * tt, int tag, int iclass)
{
...
{
	const DER_ENC * d1 = static_cast<const DER_ENC *>(a);
	const DER_ENC * d2 = static_cast<const DER_ENC *>(b);
	int cmplen = (d1->length < d2->length) ? d1->length : d2->length;
	int i = memcmp(d1->data, d2->data, cmplen);
	if(i)
		return i;
...
static int asn1_set_seq_out(STACK_OF(ASN1_VALUE) * sk, uchar ** out, int skcontlen, const ASN1_ITEM * item, int do_sort, int iclass)
{
	...
	if(do_sort) {
		...
		if(sk_ASN1_VALUE_num(sk) < 2)
			...
			derlst = static_cast<DER_ENC *>(OPENSSL_malloc(sk_ASN1_VALUE_num(sk) * sizeof(*derlst)));
			if(derlst == NULL)
				return 0;
			tmpdat = static_cast<uchar *>(OPENSSL_malloc(skcontlen));
			if(tmpdat == NULL) {
				OPENSSL_free(derlst);
				return 0;
			}
...
static int asn1_i2d_ex_primitive(ASN1_VALUE ** pval, uchar ** out, const ASN1_ITEM * it, int tag, int aclass)
{
	 ...
	 */
	int len = asn1_ex_i2c(pval, NULL, &utype, it);
	/*
	 * If SEQUENCE, SET or OTHER then header is included in pseudo content
	 * octets so don't include tag+length. We need to check here because the
	 * call to asn1_ex_i2c() could change utype.
	 */
	int usetag = (oneof3(utype, V_ASN1_SEQUENCE, V_ASN1_SET, V_ASN1_OTHER)) ? 0 : 1;
	// -1 means omit type 
	if(len == -1)
		return 0;
...
static int asn1_ex_i2c(ASN1_VALUE ** pval, uchar * cout, int * putype, const ASN1_ITEM * it)
{
	...
	uchar c;
	int len;
	const ASN1_PRIMITIVE_FUNCS * pf = static_cast<const ASN1_PRIMITIVE_FUNCS *>(it->funcs);
	if(pf && pf->prim_i2c)
