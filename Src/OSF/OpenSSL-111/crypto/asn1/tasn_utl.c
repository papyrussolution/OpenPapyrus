...
{
	int * sel = static_cast<int *>(offset2ptr(*pval, it->utype));
	return *sel;
}
...
{
	int * sel, ret;
	sel = static_cast<int *>(offset2ptr(*pval, it->utype));
	ret = *sel;
	*sel = value;
	return ret;
}
...
int asn1_do_lock(ASN1_VALUE ** pval, int op, const ASN1_ITEM * it)
{
	...
	if((it->itype != ASN1_ITYPE_SEQUENCE) && (it->itype != ASN1_ITYPE_NDEF_SEQUENCE))
		return 0;
	aux = static_cast<const ASN1_AUX *>(it->funcs);
	if(!aux || !(aux->flags & ASN1_AFLG_REFCOUNT))
		return 0;
	lck = static_cast<CRYPTO_REF_COUNT *>(offset2ptr(*pval, aux->ref_offset));
	lock = static_cast<CRYPTO_RWLOCK **>(offset2ptr(*pval, aux->ref_lock));
	switch(op) {
		case 0:
		    *lck = ret = 1;
...
static ASN1_ENCODING * asn1_get_enc_ptr(ASN1_VALUE ** pval, const ASN1_ITEM * it)
{
	...
	if(!aux || !(aux->flags & ASN1_AFLG_ENCODING))
		return NULL;
	return static_cast<ASN1_ENCODING *>(offset2ptr(*pval, aux->enc_offset));
}
    ...
    const ASN1_ITEM * it)
{
	...
	OPENSSL_free(enc->enc);
	if((enc->enc = static_cast<uchar *>(OPENSSL_malloc(inlen))) == NULL) {
...
{
	ASN1_VALUE ** pvaltmp = static_cast<ASN1_VALUE **>(offset2ptr(*pval, tt->offset));
	/*
    ...
    int nullerr)
{
	...
	sfld = static_cast<ASN1_VALUE **>(offset2ptr(*pval, adb->offset));
	/* Check if NULL */
	if(*sfld == NULL) {
		if(!adb->null_tt)
			goto err;
