...
int ASN1_sign(i2d_of_void * i2d, X509_ALGOR * algor1, X509_ALGOR * algor2, ASN1_BIT_STRING * signature, char * data, EVP_PKEY * pkey, const EVP_MD * type)
{
	...
	inll = (size_t)inl;
	buf_in = static_cast<uchar *>(OPENSSL_malloc(inll));
	outll = outl = EVP_PKEY_size(pkey);
	buf_out = static_cast<uchar *>(OPENSSL_malloc(outll));
...
int ASN1_item_sign(const ASN1_ITEM * it, X509_ALGOR * algor1, X509_ALGOR * algor2, ASN1_BIT_STRING * signature, void * asn,
    ...
    ASN1_BIT_STRING * signature, void * asn, EVP_MD_CTX * ctx)
{
	...
	buf_len = ASN1_item_i2d(static_cast<ASN1_VALUE *>(asn), &buf_in, it);
	if(buf_len <= 0) {
		outl = 0;
