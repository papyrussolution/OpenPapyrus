...
static X509_ALGOR *pkcs5_scrypt_set(const uchar * salt, size_t saltlen, size_t keylen, uint64_t N, uint64_t r, uint64_t p);

/*
 * Return an algorithm identifier for a PKCS#5 v2.0 PBE algorithm using scrypt
 */
X509_ALGOR * PKCS5_pbe2_set_scrypt(const EVP_CIPHER * cipher, const uchar * salt, int saltlen, uchar * aiv, uint64_t N, uint64_t r, uint64_t p)
{
	...
	if(alg_nid == NID_undef) {
		ASN1err(ASN1_F_PKCS5_PBE2_SET_SCRYPT, ASN1_R_CIPHER_HAS_NO_OBJECT_IDENTIFIER);
		goto err;
	}
	pbe2 = PBE2PARAM_new();
	if(pbe2 == NULL)
	...
	if(EVP_CIPHER_param_to_asn1(ctx, scheme->parameter) <= 0) {
		ASN1err(ASN1_F_PKCS5_PBE2_SET_SCRYPT, ASN1_R_ERROR_SETTING_CIPHER_PARAMS);
		goto err;
	}
    ...
    const EVP_CIPHER * c, const EVP_MD * md, int en_de)
{
	...
	/* Decode parameter */
	sparam = static_cast<SCRYPT_PARAMS *>(ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(SCRYPT_PARAMS), param));
	if(sparam == NULL) {
	...
	if(EVP_PBE_scrypt(pass, passlen, salt, saltlen, N, r, p, 0, key, keylen) == 0)
		goto err;
