...
static ASN1_VALUE * b64_read_asn1(BIO * bio, const ASN1_ITEM * it)
{
	...
	val = static_cast<ASN1_VALUE *>(ASN1_item_d2i_bio(it, bio, NULL));
	if(!val)
...
static MIME_HEADER * mime_hdr_new(const char * name, const char * value)
{
	...
	mhdr = static_cast<MIME_HEADER *>(OPENSSL_malloc(sizeof(*mhdr)));
	if(mhdr == NULL)
		goto err;
	mhdr->name = tmpname;
	mhdr->value = tmpval;
...
static int mime_hdr_addparam(MIME_HEADER * mhdr, const char * name, const char * value)
{
	...
	mparam = static_cast<MIME_PARAM *>(OPENSSL_malloc(sizeof(*mparam)));
	if(mparam == NULL)
		goto err;
