    ...
    int * perr)
{
	...
	new_der = static_cast<uchar *>(OPENSSL_malloc(len));
	if(new_der == NULL)
		goto err;
...
static int asn1_cb(const char * elem, int len, void * bitstr)
{
	tag_exp_arg * arg = static_cast<tag_exp_arg *>(bitstr);
	int i;
	int utype;
	int vlen = 0;
...
static ASN1_TYPE * asn1_str2type(const char * str, int format, int utype)
{
	...
	switch(utype) {
		...
		case V_ASN1_NUMERICSTRING:
		    ...
		    if(ASN1_mbstring_copy(&atmp->value.asn1_string, (uchar *)str,
...
static int bitstr_cb(const char * elem, int len, void * bitstr)
{
	...
	if(!ASN1_BIT_STRING_set_bit(static_cast<ASN1_BIT_STRING *>(bitstr), bitnum, 1)) {
