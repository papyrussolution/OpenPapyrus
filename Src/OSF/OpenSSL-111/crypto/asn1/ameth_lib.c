...
{
	EVP_PKEY_ASN1_METHOD * ameth = static_cast<EVP_PKEY_ASN1_METHOD *>(OPENSSL_zalloc(sizeof(*ameth)));
	if(ameth == NULL)
		return NULL;
	ameth->pkey_id = id;
...
void EVP_PKEY_asn1_set_param(EVP_PKEY_ASN1_METHOD * ameth, int (*param_decode)(EVP_PKEY * pkey, const uchar ** pder, int derlen),
    int (*param_encode)(const EVP_PKEY * pkey, uchar ** pder), int (*param_missing)(const EVP_PKEY * pk),
...
void EVP_PKEY_asn1_set_item(EVP_PKEY_ASN1_METHOD * ameth, int (*item_verify)(EVP_MD_CTX * ctx, const ASN1_ITEM * it, void * asn,
    X509_ALGOR * a, ASN1_BIT_STRING * sig, EVP_PKEY * pkey), int (*item_sign)(EVP_MD_CTX * ctx, const ASN1_ITEM * it, void * asn,
