...
int ASN1_bn_print(BIO * bp, const char * number, const BIGNUM * num, uchar * ign, int indent)
{
	int n, rv = 0;
	const char * neg;
	...
	if(BN_num_bytes(num) <= BN_BYTES) {
		if(BIO_printf(bp, "%s %s%lu (%s0x%lx)\n", number, neg, (ulong)bn_get_words(num)[0], neg, (ulong)bn_get_words(num)[0]) <= 0)
			return 0;
		return 1;
	}
	buflen = BN_num_bytes(num) + 1;
	buf = tmp = static_cast<uchar *>(OPENSSL_malloc(buflen));
	if(!buf)
		goto err;
	buf[0] = 0;
