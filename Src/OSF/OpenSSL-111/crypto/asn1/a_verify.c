...
int ASN1_verify(i2d_of_void * i2d, X509_ALGOR * a, ASN1_BIT_STRING * signature, char * data, EVP_PKEY * pkey)
{
	...
	buf_in = static_cast<uchar *>(OPENSSL_malloc((uint)inl));
	if(buf_in == NULL) {
		ASN1err(ASN1_F_ASN1_VERIFY, ERR_R_MALLOC_FAILURE);
		goto err;
	}
	...
	if(EVP_VerifyFinal(ctx, (uchar *)signature->data,
	    (uint)signature->length, pkey) <= 0) {
		ASN1err(ASN1_F_ASN1_VERIFY, ERR_R_EVP_LIB);
		ret = 0;
		goto err;
	}
    ...
    ASN1_BIT_STRING * signature, void * asn, EVP_PKEY * pkey)
{
	...
	inl = ASN1_item_i2d(static_cast<ASN1_VALUE *>(asn), &buf_in, it);
	if(inl <= 0) {
