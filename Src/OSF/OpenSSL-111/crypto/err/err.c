...
ERR_STATE * ERR_get_state(void)
{
	...
	if(!RUN_ONCE(&err_init, err_do_init))
		return NULL;
	state = static_cast<ERR_STATE *>(CRYPTO_THREAD_get_local(&err_thread_local));
	if(state == (ERR_STATE*)-1)
		return NULL;
	if(state == NULL) {
		if(!CRYPTO_THREAD_set_local(&err_thread_local, (ERR_STATE*)-1))
			return NULL;
		if((state = static_cast<ERR_STATE *>(OPENSSL_zalloc(sizeof(*state)))) == NULL) {
			CRYPTO_THREAD_set_local(&err_thread_local, NULL);
			return NULL;
		}

		if(!ossl_init_thread_start(OPENSSL_INIT_THREAD_ERR_STATE) || !CRYPTO_THREAD_set_local(&err_thread_local, state)) {
			ERR_STATE_free(state);
			CRYPTO_THREAD_set_local(&err_thread_local, NULL);
			return NULL;
		}
...
void ERR_add_error_vdata(int num, va_list args)
{
	...
	char * str, * p, * a;
	int s = 80;
	if((str = static_cast<char *>(OPENSSL_malloc(s + 1))) == NULL) {
