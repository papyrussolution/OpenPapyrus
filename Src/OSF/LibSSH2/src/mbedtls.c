...
static void _libssh2_mbedtls_safe_free(void * buf, int len)
{
...
int _libssh2_mbedtls_cipher_init(_libssh2_cipher_ctx * ctx, _libssh2_cipher_type(algo), uchar * iv, uchar * secret, int encrypt)
{
...
int _libssh2_mbedtls_cipher_crypt(_libssh2_cipher_ctx * ctx, _libssh2_cipher_type(algo),
    int encrypt, uchar * block, size_t blocklen)
{
	int ret;
	uchar * output;
...
{
	int ret = mbedtls_md_finish(ctx, hash);
	mbedtls_md_free(ctx);
	return ret == 0 ? 0 : -1;
}
...
{
	_libssh2_bn * bignum = (_libssh2_bn*)mbedtls_calloc(1, sizeof(_libssh2_bn));
	if(bignum) {
...
int _libssh2_mbedtls_rsa_new_private_frommemory(libssh2_rsa_ctx ** rsa, LIBSSH2_SESSION * session,
...
int _libssh2_mbedtls_rsa_sha1_verify(libssh2_rsa_ctx * rsa, const uchar * sig,
    ulong sig_len, const uchar * m, ulong m_len)
{
...
int _libssh2_mbedtls_rsa_sha1_sign(LIBSSH2_SESSION * session, libssh2_rsa_ctx * rsa,
    const uchar * hash, size_t hash_len, uchar ** signature, size_t * signature_len)
{
	int ret;
	uchar * sig;
	uint sig_len;
	...
	ret = mbedtls_rsa_pkcs1_sign(rsa, NULL, NULL, MBEDTLS_RSA_PRIVATE, MBEDTLS_MD_SHA1, SHA_DIGEST_LENGTH, hash, sig);
	if(ret) {
...
static int _libssh2_mbedtls_pub_priv_key(LIBSSH2_SESSION * session, uchar ** method, size_t * method_len,
    uchar ** pubkeydata, size_t * pubkeydata_len, mbedtls_pk_context * pkey)
{
	...
	if(mbedtls_pk_get_type(pkey) != MBEDTLS_PK_RSA) {
		mbedtls_pk_free(pkey);
		return _libssh2_error(session, LIBSSH2_ERROR_FILE, "Key type not supported");
	}
	// write method
	mthlen = 7;
...
int _libssh2_mbedtls_pub_priv_keyfile(LIBSSH2_SESSION * session, uchar ** method, size_t * method_len,
    uchar ** pubkeydata, size_t * pubkeydata_len, const char * privatekey, const char * passphrase)
{
	...
	ret = _libssh2_mbedtls_pub_priv_key(session, method, method_len, pubkeydata, pubkeydata_len, &pkey);
	mbedtls_pk_free(&pkey);
	return ret;
}

int _libssh2_mbedtls_pub_priv_keyfilememory(LIBSSH2_SESSION * session, uchar ** method,
    size_t * method_len, uchar ** pubkeydata, size_t * pubkeydata_len, const char * privatekeydata,
