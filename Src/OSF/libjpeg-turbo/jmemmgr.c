...
#define __STDC_LIMIT_MACROS  // @sobolev (for SIZE_MAX in stdint.h)
#include "jinclude.h"
#include "jpeglib.h"
...
/* Allocate a "small" object */
{
	...
	if(sizeofobject > MAX_ALLOC_CHUNK) {
		// This prevents overflow/wrap-around in round_up_pow2() if sizeofobject is close to SIZE_MAX.
...
/* Allocate the in-memory buffers for any unrealized virtual arrays */
{
	...
	for(sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
		if(sptr->mem_buffer == NULL) { /* if not realized yet */
			size_t new_space = (long)sptr->rows_in_array * (long)sptr->samplesperrow * sizeof(JSAMPLE);
	...
	for(bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
		if(bptr->mem_buffer == NULL) { /* if not realized yet */
			size_t new_space = (long)bptr->rows_in_array * (long)bptr->blocksperrow * sizeof(JBLOCK);
