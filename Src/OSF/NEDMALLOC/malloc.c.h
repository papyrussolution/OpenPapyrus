...
#if (defined(__GNUC__) && ((__GNUC__*10000+__GNUC_MINOR__*100+__GNUC_PATCHLEVEL__) > 40100)) || (defined(_MSC_VER) && \
        _MSC_VER>=1310)
...
static FORCEINLINE void * win32mmap(size_t size)
{
	...
	if(largepagesize && size >= largepagesize && !(size&(largepagesize-1))) {
		...
		if(!ptr) {
			if(ERROR_PRIVILEGE_NOT_HELD==GetLastError()) {
				...
				OutputDebugStringA(
					"nedmalloc: Failed to allocate large memory pages (does the user running this process have the right to lock pages in memory?). Large pages will not be used.\n");
  ...
  #endif
		if(!ptr) ptr = VirtualAlloc(baseaddress, size, baseaddress ? MEM_COMMIT : MEM_RESERVE|MEM_COMMIT,
				PAGE_READWRITE);
  #ifdef DEFAULT_GRANULARITY_ALIGNED
   #if DEBUG
...
static FORCEINLINE void * win32direct_mremap(void ** handle, void * ptr, size_t oldsize, size_t newsize, int flags,
	unsigned flags2)
{
	void * newptr = 0;
	if(!*handle)
...
static FORCEINLINE int pthread_acquire_lock(MLOCK_T * sl)
{
	...
	for(;; ) {
		if(*lp != 0) {
			...
			int cmp = 0;
			int val = 1;
			int ret = __sync_bool_compare_and_swap(lp, cmp, val);
			if(ret) {
				assert(!sl->threadid);
...
static int init_mparams(void)
{
	...
	if(mparams.magic == 0) {
...
#else /* WIN32 */
		{
			...
			gsize = ((DEFAULT_GRANULARITY > system_info.dwAllocationGranularity) ? DEFAULT_GRANULARITY : system_info.dwAllocationGranularity);
 ...
 #else /* WIN32 */
		{
			GetLargePageMinimum_t GetLargePageMinimum_ = (GetLargePageMinimum_t)GetProcAddress(
				GetModuleHandle(__T("kernel32.dll")), "GetLargePageMinimum");
...
static mchunkptr mmap_resize(mstate m, mchunkptr oldp, size_t nb, unsigned flags)
{
	...
	if(oldsize >= nb+SIZE_T_SIZE &&
	   (oldsize-nb) <= (mparams.granularity<<1))
		...
		void * mmaph = *(void **)mm;
		char * cp =
		        (char *)CALL_DIRECT_MREMAP(&mmaph, mm, oldmmsize, newmmsize,
				(flags&M2_PREVENT_MOVE) ? 0 : MREMAP_MAYMOVE,
				flags);
		if(cp != CMFAIL) {
...
static void * internal_realloc(mstate m, void * oldmem, size_t bytes, size_t alignment, unsigned flags)
{
	...
	if(!PREACTION(m)) {
		...
		if(newp != 0) {
			...
			void * newmem = (alignment > MALLOC_ALIGNMENT) ? internal_memalign(m, alignment, bytes, flags) : internal_malloc(m, bytes, flags);
			if(newmem != 0) {
