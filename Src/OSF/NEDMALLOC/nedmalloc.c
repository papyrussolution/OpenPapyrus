...
/* This can provide a very significant speed boost */
 ...
 #define MUNMAP(h, a, \
                s)                    (!OSHavePhysicalPageSupport() ? MUNMAP_DEFAULT((h), (a), (s)) : userpage_free((a), \
					       (s)))
 #define MMAP(s, f)                         (!OSHavePhysicalPageSupport() ? MMAP_DEFAULT((s)) : userpage_malloc((s), (f)))
 #define MREMAP(addr, osz, nsz, \
                mv)         (!OSHavePhysicalPageSupport() ? MREMAP_DEFAULT((addr), (osz), (nsz), \
				     (mv)) : userpage_realloc((addr), \
				     (osz), (nsz), (mv), 0))
 #define DIRECT_MMAP(h, s, \
                     f)               (!OSHavePhysicalPageSupport() ? DIRECT_MMAP_DEFAULT((h), (s), \
					       (f)) : userpage_malloc((s), (f)| \
					       USERPAGE_TOPDOWN))
 #define DIRECT_MREMAP(h, a, os, ns, f, \
                       f2) (!OSHavePhysicalPageSupport() ? DIRECT_MREMAP_DEFAULT((h), (a), (os), (ns), (f), \
				    (f2)) : userpage_realloc((a), \
				    (os), (ns), (f), (f2)|USERPAGE_TOPDOWN))
...
/* The maximum size to be allocated from the thread cache */
 ...
 #ifdef __GNUC__
  #warning \
        If you are changing THREADCACHEMAX, do you also need to change THREADCACHEMAXBINS=(topbitpos(THREADCACHEMAX)-4)?
 #elif defined(_MSC_VER)
  #pragma \
        message(__FILE__ ": WARNING: If you are changing THREADCACHEMAX, do you also need to change THREADCACHEMAXBINS=(topbitpos(THREADCACHEMAX)-4)?")
 #endif
#endif
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * CallMalloc(void * RESTRICT mspace, size_t size,
	size_t alignment,
	unsigned flags) THROWSPEC
{
	void * RESTRICT ret = 0;
	...
	if(ret) {
		...
		if(!leastusedaddress || (void *)((mstate)mspace)->least_addr<leastusedaddress) leastusedaddress =
			        (void *)((mstate)mspace)->least_addr;
		if(!largestusedblock || truesize>largestusedblock) largestusedblock =
			        (truesize+mparams.page_size)&~(mparams.page_size-1);
	}
 #endif
#endif
	if(!ret) return 0;
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * CallRealloc(void * RESTRICT mspace, void * RESTRICT mem,
	int isforeign, size_t oldsize, size_t newsize, size_t alignment,
	unsigned flags) THROWSPEC
{
	void * RESTRICT ret = 0;
#if USE_MAGIC_HEADERS
 ...
 #endif
#endif
	if(!ret) { /* Put it back the way it was */
#if USE_MAGIC_HEADERS
...
static NEDMALLOCNOALIASATTR mstate nedblkmstate(void * RESTRICT mem) THROWSPEC
{
    ...
    #pragma pack(pop)
		unsigned int header = ((unsigned int *)mem)[-1], mask1 = 0x8080E100, result1, mask2 = 0xFFFFFF06,
		             result2;
		result1 = header&mask1; /* Positive testing for NT heap */
		result2 = header&mask2; /* Positive testing for dlmalloc */
		if(result1==0x00000100 && result2!=0x00000102) { /* This is likely a NT heap block */
			return 0;
		}
   #endif
   #ifdef __linux__
...
NEDMALLOCNOALIASATTR void nedsetvalue(void * v) THROWSPEC
{
	nedpsetvalue((nedpool *)0, v);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedmalloc(size_t size) THROWSPEC
{
	return nedpmalloc((nedpool *)0, size);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedcalloc(size_t no, size_t size) THROWSPEC
{
	return nedpcalloc((nedpool *)0, no, size);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedrealloc(void * mem, size_t size) THROWSPEC
{
	return nedprealloc((nedpool *)0, mem, size);
}
NEDMALLOCNOALIASATTR void   nedfree(void * mem) THROWSPEC
{
	nedpfree((nedpool *)0, mem);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedmemalign(size_t alignment, size_t bytes) THROWSPEC
{
	return nedpmemalign((nedpool *)0, alignment, bytes);
}
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedrealloc2(void * mem, size_t size, size_t alignment,
	unsigned flags) THROWSPEC
{
	return nedprealloc2((nedpool *)0, mem, size, alignment, flags);
}
NEDMALLOCNOALIASATTR void   nedfree2(void * mem, unsigned flags) THROWSPEC
{
	nedpfree2((nedpool *)0, mem, flags);
}
NEDMALLOCNOALIASATTR struct nedmallinfo nedmallinfo(void) THROWSPEC
{
	return nedpmallinfo((nedpool *)0);
}
NEDMALLOCNOALIASATTR int    nedmallopt(int parno, int value) THROWSPEC
{
	return nedpmallopt((nedpool *)0, parno, value);
}
NEDMALLOCNOALIASATTR int    nedmalloc_trim(size_t pad) THROWSPEC
{
	return nedpmalloc_trim((nedpool *)0, pad);
}
void   nedmalloc_stats() THROWSPEC
{
	nedpmalloc_stats((nedpool *)0);
}
NEDMALLOCNOALIASATTR size_t nedmalloc_footprint() THROWSPEC
{
	return nedpmalloc_footprint((nedpool *)0);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void ** nedindependent_calloc(size_t elemsno, size_t elemsize,
	void ** chunks) THROWSPEC
{
	return nedpindependent_calloc((nedpool *)0, elemsno, elemsize, chunks);
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void ** nedindependent_comalloc(size_t elems, size_t * sizes,
	void ** chunks) THROWSPEC
{
	return nedpindependent_comalloc((nedpool *)0, elems, sizes, chunks);
}
...
struct threadcacheblk_t {       /* Keep less than 32 bytes as sizeof(threadcacheblk) is the minimum allocation size */
...
static nedpool syspool;
   ...
   #define COPY_STRING(d, s, maxlen) { size_t len = strlen(s); len = (len>maxlen) ? maxlen-1 : len; memcpy(d, s, len); \
		                       d[len] = 0; }
...
static DWORD64 __stdcall GetModBase(HANDLE hProcess, DWORD64 dwAddr) THROWSPEC
{
	...
	if(modulebase)
		...
		if(0 != VirtualQueryEx( hProcess, (LPCVOID)(size_t)dwAddr, &stMBI, sizeof(stMBI))) {
			...
			if(!SymLoadModule64( hProcess, NULL, (PSTR)( (dwPathLen) ? szFile : 0),
				   (PSTR)( (dwNameLen) ? szModuleName : 0),
...
static void DoStackWalk(logentry * p) THROWSPEC
{
	...
	if(!sym_myprocess) {
		...
		DuplicateHandle(GetCurrentProcess(), GetCurrentProcess(),
			GetCurrentProcess(), &sym_myprocess, 0, FALSE, DUPLICATE_SAME_ACCESS);
	...
	if((VOID (WINAPI *)(PCONTEXT))-1==RtlCaptureContextAddr)
		RtlCaptureContextAddr = (VOID (WINAPI *)(PCONTEXT))GetProcAddress(GetModuleHandle(
				L"kernel32"), "RtlCaptureContext");
	if(RtlCaptureContextAddr)
		RtlCaptureContextAddr(&ct);
	else {  // This is nasty, but it works
		__try
		{
			int * foo = 0;
			*foo = 78;
...
static void DoStackWalk(logentry * p) THROWSPEC
{
	...
	for(i2 = 0; i2<size; i2++) { // Format can be <file path>(<mangled symbol>+0x<offset>) [<pc>]
 ...
 #endif
#endif
static FORCEINLINE logentry * LogOperation(threadcache * tc, nedpool * np, LogEntryType type, int mspace, size_t size,
	void * mem, size_t alignment, unsigned flags,
	void * returned) THROWSPEC
{
#if ENABLE_LOGGING
	if(tc->logentries && NEDMALLOC_TESTLOGENTRY(tc, np, type, mspace, size, mem, alignment, flags, returned)) {
		...
		if(tc->logentriesptr==tc->logentriesend) {
			...
			le =
			        (logentry *)CallRealloc(0, tc->logentries, 0, logentrieslen, (logentrieslen*3)/2, 0,
					M2_ZERO_MEMORY|M2_ALWAYS_MMAP|M2_RESERVE_MULT(
						8));
			if(!le) return 0;
		...
		if(haslogentries) {
			...
			if(oh) {
				...
				if(pos1==pos2)
					fprintf(
						oh,
						"Timestamp, Pool, Operation, MSpace, Size, Block, Alignment, Flags, Returned,\"Stack Backtrace\"\n");
				for(n = 0; n<THREADCACHEMAXCACHES; n++) {
					if((tc = p->caches[n]) && tc->logentries) {
						logentry * le;
						for(le = tc->logentries; le<tc->logentriesptr; le++) {
							const char * LogEntryTypeString =
							        LogEntryTypeStrings[size2binidx(((size_t)le->type)<<4)];
							char stackbacktrace[16384] = "?";
 #if NEDMALLOC_STACKBACKTRACEDEPTH
							char * sbtp = stackbacktrace;
							int i;
							for(i = 0; i<NEDMALLOC_STACKBACKTRACEDEPTH && le->stack[i].pc;
							    i++) {
								sbtp += sprintf(sbtp, "0x%p:%s:%s (%s:%u),",
									le->stack[i].pc, le->stack[i].module,
									le->stack[i].functname, le->stack[i].file,
									le->stack[i].lineno);
							...
							if(strchr(sbtp,
								   0)>=stackbacktrace+sizeof(stackbacktrace)) abort();
 #endif
							fprintf(
								oh,
								"%llu, 0x%p, %s, %d, %Iu, 0x%p, %Iu, 0x%x, 0x%p,\"%s\"\n",
								le->timestamp, le->np, LogEntryTypeString, le->mspace,
								le->size, le->mem, le->alignment, le->flags,
								le->returned,
								stackbacktrace);
						}
...
static NOINLINE threadcache * AllocCache(nedpool * RESTRICT p) THROWSPEC
{
...
#if ENABLE_LOGGING
	{
		...
		tc->logentries = tc->logentriesptr = (logentry *)CallMalloc(
			p->m[0], logentrieslen*sizeof(logentry), 0, M2_ZERO_MEMORY|M2_ALWAYS_MMAP|M2_RESERVE_MULT(8));
		if(!tc->logentries) {
 #if USE_LOCKS
			RELEASE_LOCK(&p->mutex);
 #endif
...
static void * threadcache_malloc(nedpool * RESTRICT p, threadcache * RESTRICT tc, size_t * RESTRICT _size) THROWSPEC
{
	...
	if(blk) {
		...
		*binsptr = blk->next;
		...
		assert(nedblksize(0, blk, 0)>=sizeof(threadcacheblk) && nedblksize(0, blk,
				0)<=THREADCACHEMAX+CHUNK_OVERHEAD);
	...
	if(!(tc->mallocs&0xfff)) {
		printf("*** threadcache=%u, mallocs=%u (%f), free=%u (%f), freeInCache=%u\n",
			...
			(float)tc->successes/tc->mallocs, tc->frees, (float)tc->successes/tc->frees,
			(unsigned int)tc->freeInCache);
	}
#endif
#ifdef FULLSANITYCHECKS
	tcfullsanitycheck(tc);
...
static void threadcache_free(nedpool * RESTRICT p, threadcache * RESTRICT tc, int mymspace, void * RESTRICT mem,
	size_t size,
	int isforeign) THROWSPEC
{
	unsigned int bestsize;
	unsigned int idx = size2binidx(size);
...
static NOINLINE mstate FindMSpace(nedpool * RESTRICT p, threadcache * RESTRICT tc, int * RESTRICT lastUsed,
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR nedpool * nedcreatepool(size_t capacity, int threads) THROWSPEC
{
	...
	if(poollist->length==poollist->size) {
		...
		poollist = newpoollist;
		memset(&poollist->list[poollist->size], 0, newsize-
			((size_t)&poollist->list[poollist->size]-(size_t)&poollist->list[0]));
...
void neddestroypool(nedpool * p) THROWSPEC
{
	...
	memmove(&poollist->list[n], &poollist->list[n+1],
		(size_t)&poollist->list[poollist->length]-(size_t)&poollist->list[n]);
	if(!--poollist->length) {
...
{
	if(!p) {
		p = &syspool; if(!syspool.threads) InitPool(&syspool, 0, -1);
	}
	p->uservalue = v;
}
...
static FORCEINLINE mstate GetMSpace(nedpool * RESTRICT p, threadcache * RESTRICT tc, int mymspace,
	size_t size) THROWSPEC
...
static NOINLINE void GetThreadCache_cold2(nedpool * RESTRICT * RESTRICT p, threadcache * RESTRICT * RESTRICT tc,
	int * RESTRICT mymspace,
	int mycache) THROWSPEC
{
	if(!mycache) { /* Need to allocate a new cache */
		*tc = AllocCache(*p);
		if(!*tc) { /* Disable */
	...
	else {  /* Cache disabled, but we do have an assigned thread pool */
		*tc = 0;
		*mymspace = -mycache-1;
	}
}

static FORCEINLINE void GetThreadCache(nedpool * RESTRICT * RESTRICT p, threadcache * RESTRICT * RESTRICT tc,
	int * RESTRICT mymspace,
	size_t * RESTRICT size) THROWSPEC
{
	int mycache;
#if THREADCACHEMAX
	...
	mycache = (int)(size_t)TLSGET((*p)->mycache);
	if(mycache>0) { /* Already have a cache */
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedpmalloc2(nedpool * p, size_t size, size_t alignment,
	unsigned flags) THROWSPEC
{
	void * ret = 0;
	threadcache * tc;
...
#if THREADCACHEMAX
	if(alignment<=MALLOC_ALIGNMENT && !(flags&NM_FLAGS_MASK) && tc && size<=THREADCACHEMAX) { /* Use the thread cache */
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedprealloc2(nedpool * p, void * mem, size_t size, size_t alignment,
	unsigned flags) THROWSPEC
{
	void * ret = 0;
	threadcache * tc;
	...
	GetThreadCache(&p, &tc, &mymspace, &size);
#if THREADCACHEMAX
	if(alignment<=MALLOC_ALIGNMENT && !(flags&NM_FLAGS_MASK) && tc && size && size<=THREADCACHEMAX) { /* Use the thread cache */
		if((ret = threadcache_malloc(p, tc, &size))) {
...
{       /* Frees always happen in the mspace they happened in, so skip
	...
	int mymspace, isforeign = 1;
	size_t memsize;
	if(!mem) { /* If you tried this on FreeBSD you'd be sorry! */
#ifdef DEBUG
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void ** nedpindependent_calloc(nedpool * p, size_t elemsno, size_t elemsize,
	void ** chunks) THROWSPEC
{
	void ** ret;
	threadcache * tc;
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void ** nedpindependent_comalloc(nedpool * p, size_t elems, size_t * sizes,
	void ** chunks) THROWSPEC
{
	void ** ret;
	threadcache * tc;
