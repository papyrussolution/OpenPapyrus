...
int fann_reallocate_neurons(Fann * ann, uint total_neurons)
{
	FannLayer * layer_it;
	uint num_neurons = 0;
	uint num_neurons_so_far = 0;
	FannNeuron * neurons = (FannNeuron*)realloc(ann->P_FirstLayer->P_FirstNeuron, total_neurons * sizeof(FannNeuron));
	...
	if(neurons != ann->P_FirstLayer->P_FirstNeuron) {
		...
		for(layer_it = ann->P_FirstLayer; layer_it != ann->P_LastLayer; layer_it++) {
			num_neurons = (uint)(layer_it->P_LastNeuron - layer_it->P_FirstNeuron);
			layer_it->P_FirstNeuron = neurons + num_neurons_so_far;
			layer_it->P_LastNeuron = layer_it->P_FirstNeuron + num_neurons;
...
void fann_update_candidate_slopes(Fann * ann)
{
	...
	FannNeuron * last_cand = first_cand + fann_get_cascade_num_candidates(ann);
	FannNeuron * cand_it;
...
void fann_update_candidate_weights(Fann * ann, uint num_data)
{
	...
	FannNeuron * last_cand = first_cand + fann_get_cascade_num_candidates(ann) - 1;
	switch(ann->training_algorithm) {
		case FANN_TRAIN_RPROP:
...
FANN_EXTERNAL uint FANN_API fann_get_cascade_num_candidates(Fann * ann)
{
