...
FANN_EXTERNAL float FANN_API fann_train_epoch_batch_parallel(struct fann * ann, FannTrainData * data, const uint threadnumb)
{
...
FANN_EXTERNAL float FANN_API fann_train_epoch_irpropm_parallel(struct fann * ann,
    FannTrainData * data, const uint threadnumb)
{
	struct fann** ann_vect = (struct fann**)malloc(threadnumb * sizeof(struct fann*));
	int i = 0, j = 0;
	...
	}
	{
		...
		#pragma omp parallel private(next_step)
		{
			...
			for(i = first_weight; i < (int)past_end; i++) {
				...
				const fann_type prev_step = MAX(prev_steps[i], (fann_type)0.0001);         //
				                                                                                // prev_step
				                                                                                // may
				                                                                                // not
				                                                                                // be
...
FANN_EXTERNAL float FANN_API fann_train_epoch_incremental_mod(struct fann * ann, FannTrainData * data)
{
	uint i;
	fann_reset_MSE(ann);
