...
Bit8u FCB_ReadFile(Bit16u seg, Bit16u offset, Bit16u numBlocks);
Bit8u FCB_WriteFile(Bit16u seg, Bit16u offset, Bit16u numBlocks);
...
static Bit16u long2para(Bit32u size)
	{
	if (size > 0xFFFF0)
		return 0xffff;
...
#define sGet(s ,m) GetIt(sizeof(((s *)&pt)->m), (PhysPt)offsetof(s, m))
#define sSave(s, m, val) SaveIt(sizeof(((s *)&pt)->m), (PhysPt)offsetof(s, m), val)
...
private:
	...
	struct sPSP {
		...
		Bit16u	nn_flags;
		Bit16u	dos_version;
		Bit8u	fill_2[14];			// Lot's of unused stuff i can't care about
...
private:
	...
	struct sDTA {
		...
		Bit32u nop;						// Custom: Windows search handle for multiple searches at the same time
		Bit8u fill[4];
		Bit8u attr;
		Bit16u time;
		Bit16u date;
