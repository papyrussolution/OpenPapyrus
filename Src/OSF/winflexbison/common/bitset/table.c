...
#define EBITSET_ZERO_SET(BSET) ((BSET)->b.cindex = BITSET_WINDEX_MAX, (BSET)->b.cdata = 0)
...
#define EBITSET_NONZERO_SET(BSET) (EBITSET_CACHE_DISABLE(BSET), (BSET)->b.cdata = (bitset_word*)~0)
/* A conservative estimate of whether the bitset is zero. This is non-zero only if we know for sure that the bitset is zero.  */
#define EBITSET_ZERO_P(BSET) ((BSET)->b.cdata == 0)
/* Enable cache to point to element with table index EINDEX. The element must exist.  */
#define EBITSET_CACHE_SET(BSET, EINDEX) ((BSET)->b.cindex = (EINDEX)*EBITSET_ELT_WORDS, (BSET)->b.cdata = EBITSET_WORDS(EBITSET_ELTS(BSET) [EINDEX]))
...
static bitset_bindex tbitset_resize(bitset src, bitset_bindex n_bits)
{
	...
	if(oldsize < newsize) {
		...
		if(newsize > EBITSET_ASIZE(src)) {
			...
			EBITSET_ELTS(src) = (tbitset_elt_struct **)SAlloc::R(EBITSET_ELTS(src), size * sizeof(tbitset_elt *));
			EBITSET_ASIZE(src) = size;
		}
		memzero(EBITSET_ELTS(src) + oldsize, (newsize - oldsize) * sizeof(tbitset_elt *));
	}
	else {
		/* The bitset needs to shrink.  There's no point deallocating
		   the memory unless it is shrinking by a reasonable amount.  */
		if((oldsize - newsize) >= oldsize / 2) {
			EBITSET_ELTS(src) = (tbitset_elt_struct **)SAlloc::R(EBITSET_ELTS(src), newsize * sizeof(tbitset_elt *));
...
static inline tbitset_elt * tbitset_elt_alloc(void)
{
	tbitset_elt * elt;
...
static inline tbitset_elt * tbitset_elt_calloc(void)
{
	tbitset_elt * elt = tbitset_elt_alloc();
	memzero(EBITSET_WORDS(elt), sizeof(EBITSET_WORDS(elt)));
	return elt;
}

static inline void tbitset_elt_free(tbitset_elt * elt)
{
...
static inline void tbitset_elt_remove(bitset bset, bitset_windex eindex)
{
...
static inline void tbitset_elt_add(bitset bset, tbitset_elt * elt, bitset_windex eindex)
{
	tbitset_elts * elts = EBITSET_ELTS(bset);
...
static inline bool tbitset_elt_zero_p(tbitset_elt * elt)
{
...
static tbitset_elt * tbitset_elt_find(bitset bset, bitset_bindex bindex,
...
static inline bitset_windex tbitset_weed(bitset bset)
{
...
static inline bool tbitset_equal_p(bitset dst, bitset src)
{
	if(src == dst)
		return true;
...
static inline void tbitset_copy_(bitset dst, bitset src)
{
	if(src == dst)
		return;
...
/* Copy bits from bitset SRC to bitset DST.  Return true if
   bitsets different.  */
static inline bool tbitset_copy_cmp(bitset dst, bitset src)
{
	if(src == dst)
		return false;
...
static void tbitset_set(bitset dst, bitset_bindex bitno)
{
...
static void tbitset_reset(bitset dst, bitset_bindex bitno)
{
	bitset_windex windex = bitno / BITSET_WORD_BITS;
...
static bool tbitset_test(bitset src, bitset_bindex bitno)
{
	bitset_windex windex = bitno / BITSET_WORD_BITS;
	return (tbitset_elt_find(src, bitno, EBITSET_FIND) && ((src->b.cdata[windex - src->b.cindex] >> (bitno % BITSET_WORD_BITS)) & 1));
}

static void tbitset_free(bitset bset)
{
	tbitset_zero(bset);
	SAlloc::F(EBITSET_ELTS(bset));
}

/* Find list of up to NUM bits set in BSET starting from and including
 * NEXT and store in array LIST.  Return with actual number of bits
   found and with *NEXT indicating where search stopped.  */
static bitset_bindex tbitset_list_reverse(bitset bset, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
	if(EBITSET_ZERO_P(bset))
		return 0;
	bitset_windex size = EBITSET_SIZE(bset);
	...
	do {
		...
		if(elt) {
			...
			do {
				for(bitset_word word = srcp[woffset] << (BITSET_WORD_BITS - 1 - bcount); word; bcount--) {
					if(word & BITSET_MSB) {
 ...
 * NEXT and store in array LIST.  Return with actual number of bits
   found and with *NEXT indicating where search stopped.  */
static bitset_bindex tbitset_list(bitset bset, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
	if(EBITSET_ZERO_P(bset))
		return 0;
...
static inline void tbitset_unused_clear(bitset dst)
{
...
static void tbitset_ones(bitset dst)
{
...
static bool tbitset_empty_p(bitset dst)
{
	if(EBITSET_ZERO_P(dst))
		return true;
...
static void tbitset_not(bitset dst, bitset src)
{
	tbitset_resize(dst, BITSET_NBITS_(src));
...
/* Is DST == DST | SRC?  */
static bool tbitset_subset_p(bitset dst, bitset src)
{
	tbitset_elts * selts = EBITSET_ELTS(src);
	tbitset_elts * delts = EBITSET_ELTS(dst);
...
/* Is DST & SRC == 0?  */
static bool tbitset_disjoint_p(bitset dst, bitset src)
{
	tbitset_elts * selts = EBITSET_ELTS(src);
	tbitset_elts * delts = EBITSET_ELTS(dst);
...
static bool tbitset_op3_cmp(bitset dst, bitset src1, bitset src2, enum bitset_ops op)
{
	bool changed = false;
...
static bool tbitset_and_cmp(bitset dst, bitset src1, bitset src2)
{
...
static void tbitset_and(bitset dst, bitset src1, bitset src2)
{
	tbitset_and_cmp(dst, src1, src2);
}

static bool tbitset_andn_cmp(bitset dst, bitset src1, bitset src2)
{
	if(EBITSET_ZERO_P(src2))
...
static void tbitset_andn(bitset dst, bitset src1, bitset src2)
{
	tbitset_andn_cmp(dst, src1, src2);
}

static bool tbitset_or_cmp(bitset dst, bitset src1, bitset src2)
{
	if(EBITSET_ZERO_P(src2))
		return tbitset_copy_cmp(dst, src1);
	else if(EBITSET_ZERO_P(src1))
...
static void tbitset_or(bitset dst, bitset src1, bitset src2)
{
	tbitset_or_cmp(dst, src1, src2);
}

static bool tbitset_xor_cmp(bitset dst, bitset src1, bitset src2)
{
	if(EBITSET_ZERO_P(src2))
		return tbitset_copy_cmp(dst, src1);
	else if(EBITSET_ZERO_P(src1))
...
static void tbitset_xor(bitset dst, bitset src1, bitset src2)
{
	tbitset_xor_cmp(dst, src1, src2);
}

static void tbitset_copy(bitset dst, bitset src)
{
...
size_t tbitset_bytes(bitset_bindex n_bits ATTRIBUTE_UNUSED)
{
	return sizeof(struct bitset_union::tbitset_struct);
}

/* Initialize a bitset.  */

bitset tbitset_init(bitset bset, bitset_bindex n_bits)
{
