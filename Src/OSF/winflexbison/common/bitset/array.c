...
static bitset_bindex abitset_small_list(bitset src, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
...
static void abitset_set(bitset dst ATTRIBUTE_UNUSED, bitset_bindex bitno ATTRIBUTE_UNUSED)
{
...
static void abitset_reset(bitset dst ATTRIBUTE_UNUSED, bitset_bindex bitno ATTRIBUTE_UNUSED)
{
	/* This should never occur for abitsets since we should always hit
	   the cache.  It is likely someone is trying to access outside the
...
static bool abitset_test(bitset src ATTRIBUTE_UNUSED, bitset_bindex bitno ATTRIBUTE_UNUSED)
{
...
/* Find list of up to NUM bits set in BSET in reverse order, starting
   ...
   stopped.  */
static bitset_bindex abitset_list_reverse(bitset src, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
 ...
 * NEXT and store in array LIST.  Return with actual number of bits
   found and with *NEXT indicating where search stopped.  */
static bitset_bindex abitset_list(bitset src, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
	...
	if(!bitno) {
		...
		if(windex >= size)
			return 0;
		/* If num is 1, we could speed things up with a binary search of the current word.  */
...
static inline void abitset_unused_clear(bitset dst)
{
	unsigned last_bit = BITSET_SIZE_(dst) % BITSET_WORD_BITS;
	if(last_bit)
		ABITSET_WORDS(dst)[dst->b.csize - 1] &= ((bitset_word)1 << last_bit) - 1;
}
...
static bool abitset_empty_p(bitset dst)
{
	bitset_word * dstp = ABITSET_WORDS(dst);
...
static bool abitset_equal_p(bitset dst, bitset src)
{
	bitset_word * srcp = ABITSET_WORDS(src);
	bitset_word * dstp = ABITSET_WORDS(dst);
	bitset_windex size = dst->b.csize;
...
static bool abitset_subset_p(bitset dst, bitset src)
{
	bitset_word * srcp = ABITSET_WORDS(src);
	bitset_word * dstp = ABITSET_WORDS(dst);
	bitset_windex size = dst->b.csize;
...
static bool abitset_disjoint_p(bitset dst, bitset src)
{
	bitset_word * srcp = ABITSET_WORDS(src);
	bitset_word * dstp = ABITSET_WORDS(dst);
	bitset_windex size = dst->b.csize;
...
static bool abitset_and_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_andn_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_or_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_xor_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_and_or_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_andn_or_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
...
static bool abitset_or_and_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	bool changed = false;
	bitset_word * src1p = ABITSET_WORDS(src1);
	bitset_word * src2p = ABITSET_WORDS(src2);
