...
#define LBITSET_ELT_BITS ((unsigned)(LBITSET_ELT_WORDS * LBITSET_WORD_BITS)) /* Number of bits stored in each element.  */
...
static inline lbitset_elt * lbitset_elt_alloc(void)
{
	lbitset_elt * elt;
...
static inline lbitset_elt * lbitset_elt_calloc(void)
{
	lbitset_elt * elt = lbitset_elt_alloc();
	memzero(elt->words, sizeof(elt->words));
	return elt;
}

static inline void lbitset_elt_free(lbitset_elt * elt)
{
...
static inline void lbitset_elt_unlink(bitset bset, lbitset_elt * elt)
{
...
static inline bool lbitset_elt_zero_p(lbitset_elt * elt)
{
...
static inline void lbitset_weed(bitset bset)
{
	lbitset_elt * next;
...
/* Is DST == SRC?  */
static inline bool lbitset_equal_p(bitset dst, bitset src)
{
	if(src == dst)
		return true;
...
static inline void lbitset_copy(bitset dst, bitset src)
{
	if(src == dst)
		return;
...
static bool lbitset_empty_p(bitset dst)
{
	lbitset_elt * elt;
	lbitset_elt * next;
...
static inline void lbitset_unused_clear(bitset dst)
{
...
static void lbitset_ones(bitset dst)
{
	...
	bitset_windex windex = (BITSET_SIZE_(dst) + BITSET_WORD_BITS - 1) / BITSET_WORD_BITS;
...
size_t lbitset_bytes(bitset_bindex n_bits ATTRIBUTE_UNUSED)
{
	return sizeof(struct bitset_union::lbitset_struct);
}

/* Initialize a bitset.  */
bitset lbitset_init(bitset bset, bitset_bindex n_bits ATTRIBUTE_UNUSED)
{
