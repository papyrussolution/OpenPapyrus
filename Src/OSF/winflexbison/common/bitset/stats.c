...
/* Accessor macros.  */
#define BITSET_STATS_ALLOCS_INC(TYPE)          bitset_stats_info->types[(TYPE)].allocs++
#define BITSET_STATS_FREES_INC(BSET)           bitset_stats_info->types[BITSET_TYPE_(BSET)].frees++
#define BITSET_STATS_SETS_INC(BSET)            bitset_stats_info->types[BITSET_TYPE_(BSET)].sets++
#define BITSET_STATS_CACHE_SETS_INC(BSET)      bitset_stats_info->types[BITSET_TYPE_(BSET)].cache_sets++
#define BITSET_STATS_RESETS_INC(BSET)          bitset_stats_info->types[BITSET_TYPE_(BSET)].resets++
#define BITSET_STATS_CACHE_RESETS_INC(BSET)    bitset_stats_info->types[BITSET_TYPE_(BSET)].cache_resets++
#define BITSET_STATS_TESTS_INC(BSET)           bitset_stats_info->types[BITSET_TYPE_(BSET)].tests++
#define BITSET_STATS_CACHE_TESTS_INC(BSET)     bitset_stats_info->types[BITSET_TYPE_(BSET)].cache_tests++
#define BITSET_STATS_LISTS_INC(BSET)           bitset_stats_info->types[BITSET_TYPE_(BSET)].lists++
#define BITSET_STATS_LIST_COUNTS_INC(BSET, I)  bitset_stats_info->types[BITSET_TYPE_(BSET)].list_counts[(I)]++
#define BITSET_STATS_LIST_SIZES_INC(BSET, I)   bitset_stats_info->types[BITSET_TYPE_(BSET)].list_sizes[(I)]++
#define BITSET_STATS_LIST_DENSITY_INC(BSET, I) bitset_stats_info->types[BITSET_TYPE_(BSET)].list_density[(I)]++
...
struct bitset_type_info_struct {
	...
	uint   tests;
	uint   cache_tests;
	uint   list_counts[BITSET_LOG_COUNT_BINS];
	uint   list_sizes[BITSET_LOG_SIZE_BINS];
	uint   list_density[BITSET_DENSITY_BINS];
};
...
static void bitset_percent_histogram_print(FILE * file, const char * name, const char * msg, unsigned n_bins, unsigned * bins)
{
	uint   total = 0;
	...
	for(uint i = 0; i < n_bins; i++)
		fprintf(file, "%.0f-%.0f%%\t%8u (%5.1f%%)\n", i * 100.0 / n_bins, (i + 1) * 100.0 / n_bins, bins[i], (100.0 * bins[i]) / total);
}

/* Print a log histogram with message MSG to FILE.  */
static void bitset_log_histogram_print(FILE * file, const char * name, const char * msg, unsigned n_bins, unsigned * bins)
{
	uint   total = 0;
	for(uint i = 0; i < n_bins; i++)
		total += bins[i];
	...
	uint   max_width = 2 * (uint)(0.30103 * (n_bins - 1) + 0.9999) + 1;
	fprintf(file, "%s %s", name, msg);
	{
		uint i;
		for(i = 0; i < 2; i++)
		...
		for(; i < n_bins; i++)
			fprintf(file, "%*lu-%lu\t%8u (%5.1f%%)\n", max_width - ((uint)(0.30103 * (i) + 0.9999) + 1),
			    1UL << (i - 1), (1UL << i) - 1, bins[i], (100.0 * bins[i]) / total);
	}
}

/* Print bitset statistics to FILE.  */
static void bitset_stats_print_1(FILE * file, const char * name, struct bitset_type_info_struct * stats)
{
	if(!stats)
		return;
	fprintf(file, "%s:\n", name);
	fprintf(file, _("%u bitset_allocs, %u freed (%.2f%%).\n"), stats->allocs, stats->frees, stats->allocs ? 100.0 * stats->frees / stats->allocs : 0);
	fprintf(file, _("%u bitset_sets, %u cached (%.2f%%)\n"), stats->sets, stats->cache_sets, stats->sets ? 100.0 * stats->cache_sets / stats->sets : 0);
	fprintf(file, _("%u bitset_resets, %u cached (%.2f%%)\n"), stats->resets, stats->cache_resets, stats->resets ? 100.0 * stats->cache_resets / stats->resets : 0);
	fprintf(file, _("%u bitset_tests, %u cached (%.2f%%)\n"), stats->tests, stats->cache_tests, stats->tests ? 100.0 * stats->cache_tests / stats->tests : 0);
...
static void bitset_stats_print(FILE * file, bool verbose ATTRIBUTE_UNUSED)
{
	...
	for(int i = 0; i < BITSET_TYPE_NUM; i++)
		bitset_stats_print_1(file, bitset_type_names[i], &bitset_stats_info->types[i]);
}

/* Initialise bitset statistics logging.  */
void bitset_stats_enable(void)
{
	SETIFZ(bitset_stats_info, &bitset_stats_info_data);
...
void bitset_stats_read(const char * file_name)
{
	if(!bitset_stats_info)
		return;
	SETIFZ(file_name, BITSET_STATS_FILE);
...
void bitset_stats_write(const char * file_name)
{
	...
	if(file) {
		if(fwrite(&bitset_stats_info_data, sizeof(bitset_stats_info_data), 1, file) != 1)
...
static bool bitset_stats_toggle(bitset src, bitset_bindex bitno)
{
...
static bitset_bindex bitset_stats_resize(bitset src, bitset_bindex size) { return BITSET_RESIZE_(src->s.bset, size); }
static bitset_bindex bitset_stats_size(bitset src) { return BITSET_SIZE_(src->s.bset); }
static bitset_bindex bitset_stats_count(bitset src) { return BITSET_COUNT_(src->s.bset); }
static bool bitset_stats_empty_p(bitset dst) { return BITSET_EMPTY_P_(dst->s.bset); }
static void bitset_stats_ones(bitset dst) { BITSET_ONES_(dst->s.bset); }
static void bitset_stats_zero(bitset dst) { BITSET_ZERO_(dst->s.bset); }
...
static bool bitset_stats_disjoint_p(bitset dst, bitset src)
{
	BITSET_CHECK2_(dst, src);
	return BITSET_DISJOINT_P_(dst->s.bset, src->s.bset);
}

static bool bitset_stats_equal_p(bitset dst, bitset src)
{
	BITSET_CHECK2_(dst, src);
...
static bool bitset_stats_subset_p(bitset dst, bitset src)
{
	BITSET_CHECK2_(dst, src);
...
static bool bitset_stats_and_cmp(bitset dst, bitset src1, bitset src2)
{
	BITSET_CHECK3_(dst, src1, src2);
...
static bool bitset_stats_andn_cmp(bitset dst, bitset src1, bitset src2)
{
	BITSET_CHECK3_(dst, src1, src2);
...
static bool bitset_stats_or_cmp(bitset dst, bitset src1, bitset src2)
{
	BITSET_CHECK3_(dst, src1, src2);
...
static bool bitset_stats_xor_cmp(bitset dst, bitset src1, bitset src2)
{
	BITSET_CHECK3_(dst, src1, src2);
...
static bool bitset_stats_and_or_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	BITSET_CHECK4_(dst, src1, src2, src3);
...
static bool bitset_stats_andn_or_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	BITSET_CHECK4_(dst, src1, src2, src3);
...
static bool bitset_stats_or_and_cmp(bitset dst, bitset src1, bitset src2, bitset src3)
{
	BITSET_CHECK4_(dst, src1, src2, src3);
...
static bitset_bindex bitset_stats_list_reverse(bitset bset, bitset_bindex * list, bitset_bindex num, bitset_bindex * next)
{
...
{
	return sizeof(struct bitset_union::bitset_stats_struct);
}
...
bitset bitset_stats_init(bitset bset, bitset_bindex n_bits, enum bitset_type type)
{
	...
	// Set up the actual bitset implementation that we are a wrapper over. 
	switch(type) {
		default:
		    abort();
		case BITSET_ARRAY:
			{
				...
				bset->s.bset = (bitset)xcalloc(1, bytes);
