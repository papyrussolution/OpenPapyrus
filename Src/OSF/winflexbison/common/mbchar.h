...
/* EOF (not a real character) is represented with bytes = 0 and wc_valid = false.  */
...
#define MB_UNPRINTABLE_WIDTH 1 /* Unprintable characters appear as a small box of width 1.  */

int wcwidth(wchar_t wc);

MBCHAR_INLINE int mb_width_aux(wint_t wc)
{
	int w = wcwidth(wc);
	/* For unprintable characters, arbitrarily return 0 for control characters and MB_UNPRINTABLE_WIDTH otherwise.  */
...
/* Assignment.  */
#define mb_setascii(mbc, sc) ((mbc)->ptr = (mbc)->buf, (mbc)->bytes = 1, (mbc)->wc_valid = 1, (mbc)->wc = (mbc)->buf[0] = (sc))

/* Copying a character.  */
MBCHAR_INLINE void mb_copy(mbchar_t * new_mbc, const mbchar_t * old_mbc)
{
