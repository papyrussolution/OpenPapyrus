...
#define CALL_CHUNKFUN(h, size) (((h)->use_extra_arg) ? (*(h)->ChunkFunc)((h)->extra_arg, (size)) : (*(struct _obstack_chunk *(*)(long))(h)->ChunkFunc)((size)))

#define CALL_FREEFUN(h, old_chunk) \
	do { \
		if((h)->use_extra_arg) \
			(*(h)->FreeFunc)((h)->extra_arg, (old_chunk)); \
		else \
			(*(void (*)(void *))(h)->FreeFunc)((old_chunk)); \
	} while(0)
...
int _obstack_begin(struct obstack * h, int size, int alignment, void *(*chunkfun)(long), void (*freefun)(void *))
{
	...
	h->ChunkFunc = (struct _obstack_chunk * (*)(void *, long))chunkfun;
	h->FreeFunc = (void (*)(void *, struct _obstack_chunk *))freefun;
	h->chunk_size = size;
...
void _obstack_newchunk(struct obstack * h, int length)
{
	...
	long new_size = (obj_size + length) + (obj_size >> 3) + h->alignment_mask + 100;
...
{
	struct _obstack_chunk * lp = h->chunk; /* below addr of any objects in this chunk */
	struct _obstack_chunk * plp; /* point to previous chunk if any */
	...
	while(lp != 0 && ((void*)lp >= obj || (void*)(lp)->limit < obj)) {
		...
		CALL_FREEFUN(h, lp);
		lp = plp;
		// If we switch chunks, we can't tell whether the new current chunk contains an empty object, so assume that it may. 
	...
	if(lp) {
		...
		abort(); /* obj is not in any of the chunks! */
}
