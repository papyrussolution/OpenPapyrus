...
/* The extra casts in the following macros work around compiler bugs, e.g., in Cray C 5.0.3.0.  */

/* True if the arithmetic type T is an integer type.  bool counts as an integer.  */
#define TYPE_IS_INTEGER(t) ((t) 1.5 == 1)
/* True if the real type T is signed.  */
#define TYPE_SIGNED(t) (! ((t) 0 < (t) -1))
/* Return 1 if the real expression E, after promotion, has a signed or floating type.  Do not evaluate E.  */
...
/* Does the __typeof__ keyword work?  This could be done by 'configure', but for now it's easier to do it by hand.  */
...
#define INT_STRLEN_BOUND(t) (INT_BITS_STRLEN_BOUND (TYPE_WIDTH (t) - _GL_SIGNED_TYPE_OR_EXPR (t)) + _GL_SIGNED_TYPE_OR_EXPR (t))
/* Bound on buffer size needed to represent an integer type or expression T, including the terminating null.  */
...
/* Return 1 if A + B would overflow in [MIN,MAX] arithmetic. See above for restrictions.  */
...
/* Return 1 if - A would overflow in [MIN,MAX] arithmetic. See above for restrictions.  */
...
#define INT_MULTIPLY_RANGE_OVERFLOW(a, b, min, max) ((b) < 0 ? ((a) < 0 ? (a) < (max) / (b) : (b) == -1 ? 0 : (min) / (b) < (a)) : (b) == 0 ? 0 : ((a) < 0 ? (a) < (min) / (b) : (max) / (b) < (a)))
...
/* True if __builtin_add_overflow (A, B, P) and __builtin_sub_overflow (A, B, P) work when P is non-null.  */
...
#ifdef __clang__
	#define _GL_HAS_BUILTIN_MUL_OVERFLOW 0 /* Work around Clang bug <https://bugs.llvm.org/show_bug.cgi?id=16404>.  */
#else
