...
int glthread_lock_init(gl_lock_t * lock)
{
...
int glthread_lock_lock(gl_lock_t * lock)
{
	if(lock->init_needed)
...
int glthread_lock_unlock(gl_lock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
...
int glthread_lock_destroy(gl_lock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
...
int glthread_rwlock_rdlock(gl_rwlock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
...
int glthread_rwlock_wrlock(gl_rwlock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
	if(mtx_lock(&lock->lock) != thrd_success)
...
int glthread_rwlock_unlock(gl_rwlock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
	if(mtx_lock(&lock->lock) != thrd_success)
...
int glthread_rwlock_destroy(gl_rwlock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
...
int glthread_recursive_lock_init(gl_recursive_lock_t * lock)
{
...
int glthread_recursive_lock_lock(gl_recursive_lock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
	if(mtx_lock(&lock->mutex) != thrd_success)
...
int glthread_recursive_lock_unlock(gl_recursive_lock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
	if(mtx_unlock(&lock->mutex) != thrd_success)
...
int glthread_recursive_lock_destroy(gl_recursive_lock_t * lock)
{
	if(lock->init_needed)
		call_once(&lock->init_once, lock->init_func);
	mtx_destroy(&lock->mutex);
...
int glthread_rwlock_rdlock_multithreaded(gl_rwlock_t * lock)
{
	if(!lock->initialized) {
		int err;
...
int glthread_rwlock_wrlock_multithreaded(gl_rwlock_t * lock)
{
	if(!lock->initialized) {
		int err;
...
int glthread_rwlock_unlock_multithreaded(gl_rwlock_t * lock)
{
	if(!lock->initialized)
		return EINVAL;
...
int glthread_rwlock_destroy_multithreaded(gl_rwlock_t * lock)
{
	int err;
...
int glthread_recursive_lock_lock_multithreaded(gl_recursive_lock_t * lock)
{
	if(!lock->initialized) {
		int err;
...
int glthread_recursive_lock_unlock_multithreaded(gl_recursive_lock_t * lock)
{
	if(!lock->initialized)
		return EINVAL;
...
int glthread_recursive_lock_destroy_multithreaded(gl_recursive_lock_t * lock)
{
	int err;
