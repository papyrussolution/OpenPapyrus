...
#define EXTRA_CONTEXT_FIELDS \
	...
	/* The number of edits (= number of elements inserted, plus the number of elements deleted), temporarily minus edit_count_limit. */\
...
/* We don't need USE_HEURISTIC, since it is unlikely in typical uses of fstrcmp().  */
#include "diffseq.h" 
...
double fstrcmp_bounded(const char * string1, const char * string2, double lower_bound)
{
	struct context ctxt;
	...
	if(xvec_length == 0 || yvec_length == 0) /* Prob: 1% */
		return length_sum == 0;
	if(!(xvec_length <= length_sum && length_sum <= MIN(UINTPTR_MAX, PTRDIFF_MAX) - 3))
		xalloc_die();
	if(lower_bound > 0) {
