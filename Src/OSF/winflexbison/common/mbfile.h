...
/* The macros in this file implement multi-byte character input from a stream.

   mb_file_t: is the type for multibyte character input stream, usable for variable declarations.
   mbf_char_t: is the type for multibyte character or EOF, usable for variable declarations.
   mbf_init(mbf, stream): initializes the MB_FILE for reading from stream.
   mbf_getc(mbc, mbf): reads the next multibyte character from mbf and stores it in mbc.
   mb_iseof(mbc): returns true if mbc represents the EOF value.
...
MBFILE_INLINE void mbfile_multi_getc(struct mbchar * mbc, struct mbfile_multi * mbf)
{
	size_t bytes;
...
MBFILE_INLINE void mbfile_multi_ungetc(const struct mbchar * mbc, struct mbfile_multi * mbf)
{
...
#define mbf_init(mbf, stream)                                           \
	...
	(mbf).eof_seen = false,                                              \
	(mbf).have_pushback = false,                                         \
	memzero(&(mbf).state, sizeof(mbstate_t)),                     \
	(mbf).bufcount = 0)
