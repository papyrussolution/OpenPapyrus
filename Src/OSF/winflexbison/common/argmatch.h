...
ptrdiff_t argmatch(char const * arg, char const * const * arglist, void const * vallist, size_t valsize) _GL_ATTRIBUTE_PURE;

#define ARGMATCH(Arg, Arglist, Vallist) argmatch(Arg, Arglist, (void const*)(Vallist), sizeof *(Vallist))
...
#define invalid_arg(Context, Value, Problem) argmatch_invalid(Context, Value, Problem)

/* Report on stderr the list of possible arguments.  */
void argmatch_valid(char const * const * arglist, void const * vallist, size_t valsize);
#define ARGMATCH_VALID(Arglist, Vallist)  argmatch_valid(Arglist, (void const*)(Vallist), sizeof *(Vallist))

/* Same as argmatch, but upon failure, report an explanation of the failure, and exit using the function EXIT_FN. */
ptrdiff_t __xargmatch_internal(char const * context, char const * arg, char const * const * arglist, void const * vallist, size_t valsize, argmatch_exit_fn exit_fn);

/* Programmer friendly interface to __xargmatch_internal. */
#define XARGMATCH(Context, Arg, Arglist, Vallist) ((Vallist) [__xargmatch_internal(Context, Arg, Arglist, (void const*)(Vallist), sizeof *(Vallist), argmatch_die)])

/* Convert a value into a corresponding argument. */
char const * argmatch_to_argument(void const * value, char const * const * arglist, void const * vallist, size_t valsize) _GL_ATTRIBUTE_PURE;

#define ARGMATCH_TO_ARGUMENT(Value, Arglist, Vallist) argmatch_to_argument(Value, Arglist, (void const*)(Vallist), sizeof *(Vallist))
...
#define ARGMATCH_DEFINE_GROUP(Name, Type)                              \
	...
	enum argmatch_ ## Name ## _size_enum { argmatch_ ## Name ## _size = sizeof(argmatch_ ## Name ## _type) }; \
	/* Argument mapping of this group.  */                                \
	typedef struct {                                                      \
		const char * arg; /* Argument (e.g., "simple").  */                \
		const argmatch_ ## Name ## _type val; /* Value (e.g., simple_backups).  */ \
	} argmatch_ ## Name ## _arg;                                              \
                                                                        \
	/* Documentation of this group.  */                                   \
	typedef struct {                                                      \
		const char * arg; /* Argument (e.g., "simple").  */                 \
		const char * doc; /* Documentation (e.g., N_("always make simple backups")).  */ \
	} argmatch_ ## Name ## _doc;                                              \
                                                                        \
	...
	typedef struct { \
		...
		const char * doc_pre; /* Printed before the usage message.  */ \
		const char * doc_post; /* Printed after the usage message.  */ \
	...
	/* If nonnegative, the index I of ARG in ARGS, i.e, ARGS[I] == ARG. Return -1 for invalid argument, -2 for ambiguous argument. */ \
	ptrdiff_t argmatch_ ## Name ## _choice(const char * arg);                 \
	/* A pointer to the corresponding value if it exists, or report an error and exit with failure if the argument was not recognized. */ \
	const argmatch_ ## Name ## _type * argmatch_ ## Name ## _value(const char * context, const char * arg); \
	/* The first argument in ARGS that matches this value, or NULL.  */   \
	const char * argmatch_ ## Name ## _argument(const argmatch_ ## Name ## _type *val); \
                                                                        \
	ptrdiff_t argmatch_ ## Name ## _choice(const char * arg) \
	{                                                                     \
		...
		for(/*size_t*/int i = 0; g->args[i].arg; i++)                             \
			if(!strncmp(g->args[i].arg, arg, arglen)) {                     \
				if(strlen(g->args[i].arg) == arglen)                        \
					...
					/* There is a real ambiguity, or we could not disambiguate. */ \
					ambiguous = true;                                           \
			}                                                               \
	...
	static void argmatch_ ## Name ## _valid(FILE *out) \
	{                                                                     \
		const argmatch_ ## Name ## _group_type * g = &argmatch_ ## Name ## _group;   \
		size_t size = argmatch_ ## Name ## _size;                               \
		/* Try to put synonyms on the same line.  Synonyms are expected to follow each other. */ \
	...
	static int argmatch_ ## Name ## _doc_col(void)                            \
	{                                                                     \
		const argmatch_ ## Name ## _group_type * g = &argmatch_ ## Name ## _group;   \
		size_t size = argmatch_ ## Name ## _size;                               \
		int res = 0;                                                        \
	...
	{                                                                     \
		...
		/* Width of the screen.  Help2man does not seem to support \
		   arguments on several lines, so in that case pretend a very large width. */ \
