...
static size_t raw_hasher(const void * data, size_t n)
{
	...
	size_t val = /*rotr_sz*/SBits::Rotr((size_t)data, 3);
	return val % n;
}
...
static size_t _GL_ATTRIBUTE_PURE compute_bucket_size(size_t candidate, const Hash_tuning * tuning)
{
...
#if USE_OBSTACK
		p_new = obstack_alloc(&table->entry_stack, sizeof *p_new);
#else
		p_new = (struct hash_entry *)SAlloc::M(sizeof *p_new);
#endif
	}
	return p_new;
}
...
bool hash_rehash(Hash_table * table, size_t candidate)
{
	...
	if(new_size == table->n_buckets)
		return true;
	new_table = &storage;
	new_table->bucket = (struct hash_entry *)SAlloc::C(new_size, sizeof *new_table->bucket);
