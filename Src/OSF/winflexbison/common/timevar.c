...
struct timevar_def {
	struct timevar_time_def elapsed; /* Elapsed time for this variable.  */
	struct timevar_time_def start_time; /* If this variable is timed independently of the timing stack, using timevar_start, this contains the start time.  */
	const char * name; /* The name of this timing variable.  */
	unsigned standalone : 1; /* Non-zero if this timing variable is running as a standalone timer.  */
	unsigned used : 1; /* Non-zero if this timing variable was ever started or pushed onto the timing stack.  */
};
...
struct timevar_stack_def {
	struct timevar_def * timevar; /* The timing variable at this stack level.  */
	struct timevar_stack_def * next; /* The next lower timing variable context in the stack.  */
};

static struct timevar_def timevars[TIMEVAR_LAST]; /* Declared timing variables.  Constructed from the contents of timevar.def.  */
static struct timevar_stack_def * stack; /* The top of the timing stack.  */
...
static void set_to_current_time(struct timevar_time_def * now)
{
	now->user = 0;
	now->sys  = 0;
	now->wall = 0;
...
static struct timevar_time_def get_current_time(void) 
{
...
static void timevar_accumulate(struct timevar_time_def * timer, const struct timevar_time_def * start, const struct timevar_time_def * stop)
{
