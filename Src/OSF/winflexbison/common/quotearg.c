...
struct quoting_options {
	enum quoting_style style; /* Basic quoting style.  */
...
static struct quoting_options default_quoting_options; /* The default quoting options.  */
...
{
	uchar uc = c;
	uint * p = (o ? o : &default_quoting_options)->quote_these_too + uc / INT_BITS;
...
{
	SETIFZ(o, &default_quoting_options);
...
static struct quoting_options /* NOT PURE!! */ quoting_options_from_style(enum quoting_style style)
{
	struct quoting_options o = { (enum quoting_style)0, 0, { 0 }, NULL, NULL };
...
c_escape:
			       ...
			       we can't easily escape single characters within it.  */
		    {
			    ...
			    if(unibyte_locale) {
				    ...
				    mbstate_t mbstate;
				    memzero(&mbstate, sizeof mbstate);
				    m = 0;
				    printable = true;
				    if(argsize == SIZE_MAX)
					    argsize = strlen(arg);
					    ...
					    if(bytes == 0)
						    ...
						    if('[' == 0x5b && elide_outer_quotes && quoting_style == shell_always_quoting_style) {
							    ...
							    for(j = 1; j < bytes; j++)
								    switch(arg[i + m + j]) {
									    case '[': case '\\': case '^':
									    case '`': case '|': goto force_outer_quoting_style;
									    default:
										break;
								    }
						    }
		...
		if(!((backslash_escapes || elide_outer_quotes) && quote_these_too && 
			quote_these_too[c / INT_BITS] & (1 << (c % INT_BITS))) && !is_right_quote)
			goto store_c;
store_escape:
		if(elide_outer_quotes)
			goto force_outer_quoting_style;
...
force_outer_quoting_style:
	...
	return quotearg_buffer_restyled(buffer, buffersize, arg, argsize, quoting_style,
		   flags & ~QA_ELIDE_OUTER_QUOTES, NULL, left_quote, right_quote);
}
...
size_t quotearg_buffer(char * buffer, size_t buffersize, char const * arg, size_t argsize, struct quoting_options const * o)
{
	...
	int e = errno;
	size_t r = quotearg_buffer_restyled(buffer, buffersize, arg, argsize, p->style, p->flags, p->quote_these_too,
...
char * quotearg_alloc_mem(char const * arg, size_t argsize, size_t * size, struct quoting_options const * o)
{
	...
	size_t bufsize = quotearg_buffer_restyled(0, 0, arg, argsize, p->style, flags, p->quote_these_too,
		p->left_quote, p->right_quote) + 1;
	char * buf = xcharalloc(bufsize);
	quotearg_buffer_restyled(buf, bufsize, arg, argsize, p->style, flags, p->quote_these_too, p->left_quote, p->right_quote);
	errno = e;
	ASSIGN_PTR(size, bufsize - 1);
	return buf;
}
...
struct quoting_options quote_quoting_options = { locale_quoting_style, 0, { 0 }, NULL, NULL };
...
char const * quote_n(int n, char const * arg) { return quote_n_mem(n, arg, SIZE_MAX); }
char const * quote(char const * arg) { return quote_n(0, arg); }
