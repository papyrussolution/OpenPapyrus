...
size_t bitset_bytes(enum bitset_type type, bitset_bindex n_bits)
{
	...
	switch(type) {
		default: abort();
		case BITSET_ARRAY: return abitset_bytes(n_bits);
		case BITSET_LIST: return lbitset_bytes(n_bits);
		case BITSET_TABLE: return tbitset_bytes(n_bits);
		case BITSET_VECTOR: return vbitset_bytes(n_bits);
	}
}
...
bitset bitset_init(bitset bset, bitset_bindex n_bits, enum bitset_type type)
{
	...
	switch(type) {
		default: abort();
		case BITSET_ARRAY: return abitset_init(bset, n_bits);
		case BITSET_LIST: return lbitset_init(bset, n_bits);
		case BITSET_TABLE: return tbitset_init(bset, n_bits);
		case BITSET_VECTOR: return vbitset_init(bset, n_bits);
	}
}
...
enum bitset_type bitset_type_choose(bitset_bindex n_bits ATTRIBUTE_UNUSED, unsigned attr)                 
{
	/* Check attributes.  */
...
bitset bitset_alloc(bitset_bindex n_bits, enum bitset_type type)
{
	...
	bitset bset = (bitset)xcalloc(1, bytes);
...
/* Create a bitset of N_BITS and with attribute hints specified by
   ATTR.  */
bitset bitset_create(bitset_bindex n_bits, unsigned attr)
{
...
/* Find first set bit.   */
bitset_bindex bitset_first(bitset src) { return bitset_next(src, 0); }

/* Find last set bit.   */
bitset_bindex bitset_last(bitset src) { return bitset_prev(src, 0); }
...
void bitset_dump(FILE * file, bitset bset) { bitset_print(file, bset, false); }
...
{
	// This routine is for completeness.  It could be optimized if required. 
...
bitset_bindex bitset_size_(bitset src) { return BITSET_NBITS_(src); }
...
static inline bool bitset_op4_cmp(bitset dst, bitset src1, bitset src2, bitset src3, enum bitset_ops op)
{
	bool changed = false;
...
/* DST = (SRC1 & SRC2) | SRC3.  */
void bitset_and_or_(bitset dst, bitset src1, bitset src2, bitset src3) { bitset_and_or_cmp_(dst, src1, src2, src3); }

/* DST = (SRC1 & SRC2) | SRC3.  Return non-zero if
   DST != (SRC1 & SRC2) | SRC3.  */
bool bitset_and_or_cmp_(bitset dst, bitset src1, bitset src2, bitset src3) { return bitset_op4_cmp(dst, src1, src2, src3, BITSET_OP_AND_OR); }

/* DST = (SRC1 & ~SRC2) | SRC3.  */
void bitset_andn_or_(bitset dst, bitset src1, bitset src2, bitset src3) { bitset_andn_or_cmp_(dst, src1, src2, src3); }

/* DST = (SRC1 & ~SRC2) | SRC3.  Return non-zero if
   DST != (SRC1 & ~SRC2) | SRC3.  */
bool bitset_andn_or_cmp_(bitset dst, bitset src1, bitset src2, bitset src3) { return bitset_op4_cmp(dst, src1, src2, src3, BITSET_OP_ANDN_OR); }

/* DST = (SRC1 | SRC2) & SRC3.  */
void bitset_or_and_(bitset dst, bitset src1, bitset src2, bitset src3) { bitset_or_and_cmp_(dst, src1, src2, src3); }

/* DST = (SRC1 | SRC2) & SRC3.  Return non-zero if
   DST != (SRC1 | SRC2) & SRC3.  */
bool bitset_or_and_cmp_(bitset dst, bitset src1, bitset src2, bitset src3) { return bitset_op4_cmp(dst, src1, src2, src3, BITSET_OP_OR_AND); }
