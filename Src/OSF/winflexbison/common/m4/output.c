...
#define COPY_BUFFER_SIZE (32 * 512) /* Size of buffer size to use while copying files.  */

/* Output functions.  Most of the complexity is for handling cpp like sync lines.
...
static gl_oset_t diversion_table; /* Table of diversions 1 through INT_MAX.  */
static m4_diversion div0; /* Diversion 0 (not part of diversion_table).  */
static m4_diversion * free_list; /* Linked list of reclaimed diversion storage.  */
static struct obstack diversion_storage; /* Obstack from which diversion storage is allocated.  */
static int total_buffer_size; /* Total size of all in-memory buffer sizes.  */
int current_diversion; /* The number of the currently active diversion.  This variable is maintained for the `divnum' builtin function.  */
...
static int output_unused; /* Cache of output_diversion->size - output_diversion->used, only valid when output_diversion->size is non-zero.  */
int output_current_line; /* Number of input line we are generating output for.  */
static m4_temp_dir * output_temp_dir; /* Temporary directory holding all spilled diversion files.  */
...
static bool tmp_file2_recent; /* True if tmp_file2 is more recently used.  */

/* Internal routines.  */
//
// Callback for comparing list elements ELT1 and ELT2 for order in diversion_table.
//
static int cmp_diversion_CB(const void * elt1, const void * elt2)
{
	const m4_diversion * d1 = (const m4_diversion*)elt1;
	const m4_diversion * d2 = (const m4_diversion*)elt2;
	// No need to worry about overflow, since we don't create diversions with negative divnum.
	return d1->divnum - d2->divnum;
}

/* Callback for comparing list element ELT against THRESHOLD.  */
static bool threshold_diversion_CB(const void * elt, const void * threshold)
{
...
//
static const char * m4_tmpname(int divnum)
{
	static char * buffer;
	static char * tail;
	if(buffer == NULL) {
...
/* Create a temporary file for diversion DIVNUM open for reading and
   ...
   open file.  */
static FILE * m4_tmpfile(int divnum)
{
	const char * name;
	FILE * file = 0;
	if(output_temp_dir == NULL) {
		...
		if(output_temp_dir == NULL)
			M4ERROR((EXIT_FAILURE, errno, "cannot create temporary file for diversion"));
...
//    M4ERROR((warning_status, errno, "Warning: cannot protect diversion across forks"));
	return file;
}
...
/* Reopen a temporary file for diversion DIVNUM for reading and
   ...
   file.  */
static FILE * m4_tmpopen(int divnum, bool reread)
{
	const char * name;
	FILE * file = 0;
	if(tmp_file1_owner == divnum) {
		if(reread && fseek(tmp_file1, 0, SEEK_SET) != 0)
			m4_error(EXIT_FAILURE, errno, _("cannot seek within diversion"));
...
/* Close, but don't delete, a temporary FILE for diversion DIVNUM.  To
   ...
   into EMFILE limits.  */
static int m4_tmpclose(FILE * file, int divnum)
{
	int result = 0;
...
static int m4_tmpremove(int divnum)
{
...
static FILE* m4_tmprename(int oldnum, int newnum)
{
	...
	if(oldnum == tmp_file1_owner) {
		...
		if(RENAME_OPEN_FILE_WORKS)
			...
			if(close_stream_temp(tmp_file1))
				m4_error(EXIT_FAILURE, errno, _("cannot close temporary file for diversion"));
			tmp_file1_owner = 0;
		}
	}
	...
	if(rename(oldname, newname))
		m4_error(EXIT_FAILURE, errno, _("cannot create temporary file for diversion"));
...
{
	diversion_table = gl_oset_create_empty(GL_RBTREE_OSET, cmp_diversion_CB, NULL);
...
static void make_room_for(int length)
{
	int wanted_size;
	...
	for(wanted_size = output_diversion->size; wanted_size < output_diversion->used + length; wanted_size = wanted_size == 0 ? INITIAL_BUFFER_SIZE : wanted_size * 2)
		;
	...
	if(total_buffer_size - output_diversion->size + wanted_size > MAXIMUM_TOTAL_SIZE) {
		...
		if(selected_diversion->used > 0) {
			count = fwrite(selected_buffer, (size_t)selected_diversion->used, 1, selected_diversion->u.file);
			if(count != 1)
				M4ERROR((EXIT_FAILURE, errno, "ERROR: cannot flush diversion to temporary file"));
		}
		/* Reclaim the buffer space for other diversions.  */
		SAlloc::F(selected_buffer);
...
static void output_character_helper(int character)
{
	make_room_for(1);
	if(output_file)
...
static void insert_diversion_helper(m4_diversion * diversion)
{
