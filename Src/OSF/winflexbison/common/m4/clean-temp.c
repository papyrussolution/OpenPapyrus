...
struct temp_dir * create_temp_dir(const char * prefix, const char * parentdir,
    bool cleanup_verbose){
	...
	if(tmpdirp == NULL) {
		...
		if(cleanup_list.tempdir_count == cleanup_list.tempdir_allocated) {
			...
			if(old_array != NULL)
				SAlloc::F((struct tempdir **)old_array);
		}
...
void register_temp_file(struct temp_dir * dir,
...
void unregister_temp_file(struct temp_dir * dir,
    const char * absolute_file_name)
{
	struct tempdir * tmpdir = (struct tempdir *)dir;
...
void register_temp_subdir(struct temp_dir * dir,
...
/* Unregister the given ABSOLUTE_DIR_NAME as being a subdirectory inside DIR,
   ...
   created.  */
void unregister_temp_subdir(struct temp_dir * dir,
    const char * absolute_dir_name)
{
	struct tempdir * tmpdir = (struct tempdir *)dir;
...
static int do_unlink(struct temp_dir * dir, const char * absolute_file_name)
{
...
/* Remove a directory, with optional error message.
   Return 0 upon success, or -1 if there was some problem.  */
static int do_rmdir(struct temp_dir * dir, const char * absolute_dir_name)
{
...
/* Remove the given ABSOLUTE_FILE_NAME and unregister it.
   Return 0 upon success, or -1 if there was some problem.  */
int cleanup_temp_file(struct temp_dir * dir,
    const char * absolute_file_name)
{
	int err;
...
/* Remove the given ABSOLUTE_DIR_NAME and unregister it.
   Return 0 upon success, or -1 if there was some problem.  */
int cleanup_temp_subdir(struct temp_dir * dir,
    const char * absolute_dir_name)
{
	int err;
...
/* Remove all registered files and subdirectories inside DIR.
   Return 0 upon success, or -1 if there was some problem.  */
int cleanup_temp_dir_contents(struct temp_dir * dir)
{
	struct tempdir * tmpdir = (struct tempdir *)dir;
	int err = 0;
	gl_list_t list;
...
/* Remove all registered files and subdirectories inside DIR and DIR itself.
   ...
   Return 0 upon success, or -1 if there was some problem.  */
int cleanup_temp_dir(struct temp_dir * dir)
{
	struct tempdir * tmpdir = (struct tempdir *)dir;
	int err = 0;
	size_t i;
...
static bool supports_delete_on_close()
{
...
int open_temp(const char * file_name, int flags, mode_t mode)
{
	int fd;
	int saved_errno;
	//block_fatal_signals ();
...
/* Open a temporary file in a temporary directory.
   Registers the resulting file descriptor to be closed.  */
FILE * fopen_temp(const char * file_name, const char * mode)
{
	FILE * fp;
	int saved_errno;
