...
static gl_oset_t gl_tree_nx_create_empty(gl_oset_implementation_t implementation, gl_setelement_compar_fn compar_fn, gl_setelement_dispose_fn dispose_fn)
{
	struct gl_oset_impl * set = (struct gl_oset_impl *)SAlloc::M(sizeof(struct gl_oset_impl));
	if(set == NULL)
		return NULL;
...
static size_t gl_tree_size(gl_oset_t set)
{
	return set->count;
}

/* Returns the next node in the tree, or NULL if there is none.  */
static inline gl_oset_node_t gl_tree_next_node(gl_oset_node_t node)
{
	if(node->right != NULL) {
		node = node->right;
...
static inline gl_oset_node_t gl_tree_prev_node(gl_oset_node_t node)
{
	if(node->left != NULL) {
		node = node->left;
...
static bool gl_tree_search(gl_oset_t set, const void * elt)
{
	gl_setelement_compar_fn compar = set->base.compar_fn;
	gl_oset_node_t node;
	for(node = set->root; node != NULL;) {
		int cmp = (compar != NULL ? compar(node->value, elt) : (node->value > elt ? 1 : node->value < elt ? -1 : 0));
		if(cmp < 0)
			node = node->right;
		else if(cmp > 0)
			node = node->left;
...
static bool gl_tree_search_atleast(gl_oset_t set, gl_setelement_threshold_fn threshold_fn, const void * threshold, const void ** eltp)
{
	gl_oset_node_t node;
	for(node = set->root; node != NULL;) {
...
static gl_oset_node_t gl_tree_search_node(gl_oset_t set, const void * elt)
{
	gl_setelement_compar_fn compar = set->base.compar_fn;
	gl_oset_node_t node;
	for(node = set->root; node != NULL;) {
...
static int gl_tree_nx_add(gl_oset_t set, const void * elt)
{
...
static bool gl_tree_remove(gl_oset_t set, const void * elt)
{
	gl_oset_node_t node = gl_tree_search_node(set, elt);
	return node ? gl_tree_remove_node(set, node) : false;
}

#ifndef GL_SKIP_TREE_UPDATE_IMPLEMENTATION // @sobolev {
static int gl_tree_update(gl_oset_t set, const void * elt, void (*action)(const void * /*elt*/, void * /*action_data*/), void * action_data)
{
	...
	if(old_node) {
		if(set->count > 1) {
			...
			if(!(compar ? (prev_node == NULL || compar(prev_node->value, elt) < 0)
			    && (next_node == NULL || compar(next_node->value, elt) > 0) : (prev_node == NULL || prev_node->value < elt)
				...
				node = set->root;
				for(;;) {
					int cmp = (compar ? compar(node->value, elt) : (node->value > elt ? 1 : node->value < elt ? -1 : 0));
					if(cmp < 0) {
						if(node->right == NULL) {
...
#endif // } GL_SKIP_TREE_UPDATE_IMPLEMENTATION @sobolev

static void gl_tree_oset_free(gl_oset_t set)
{
...
static gl_oset_iterator_t gl_tree_iterator(gl_oset_t set)
{
...
static bool gl_tree_iterator_next(gl_oset_iterator_t * iterator, const void ** eltp)
{
...
static void gl_tree_iterator_free(gl_oset_iterator_t * iterator _GL_ATTRIBUTE_MAYBE_UNUSED)
{
}
