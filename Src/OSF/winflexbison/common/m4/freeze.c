...
static void issue_expect_message(int expected)
{
	if(expected == '\n')
		...
		M4ERROR((EXIT_FAILURE, 0, "expecting character `%c' in frozen file", expected));
}
...
#define GET_NUMBER(Number, AllowNeg)                            \
	do {                                                         \
		...
		if(((AllowNeg) ? INT_MIN : INT_MAX) + 0U < n || isdigit(character)) \
			m4_error(EXIT_FAILURE, 0, _("integer overflow in frozen file")); \
		(Number) = n;                                             \
	} while(0)
...
#define GET_STRING(i)                                                   \
	...
	if(number[0] > 1)
		M4ERROR((EXIT_MISMATCH, 0, "frozen file version %d greater than max supported of 1", number[0]));
	else if(number[0] < 1)
		M4ERROR((EXIT_FAILURE, 0, "ill-formed frozen file, version directive expected"));
	VALIDATE('\n');
