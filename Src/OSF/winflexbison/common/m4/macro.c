...
static void expand_token(struct obstack * obs, token_type t, token_data * td, int line)
{
	...
	switch(t) { /* TOKSW */
		...
		case TOKEN_SIMPLE:
		case TOKEN_STRING:
		    shipout_text(obs, TOKEN_DATA_TEXT(td), strlen(TOKEN_DATA_TEXT(td)), line);
		    break;
		case TOKEN_WORD:
		    sym = lookup_symbol(TOKEN_DATA_TEXT(td), SYMBOL_LOOKUP);
		    if(sym == NULL || SYMBOL_TYPE(sym) == TOKEN_VOID || (SYMBOL_TYPE(sym) == TOKEN_FUNC && SYMBOL_BLIND_NO_ARGS(sym) && peek_token() != TOKEN_OPEN)) {
		    ...
		    break;
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad token type in expand_token ()"));
		    abort();
	}
}
...
static bool expand_argument(struct obstack * obs, token_data * argp)
{
	token_type t;
	token_data td;
	char * text;
...
void call_macro(SymbolTableEntry * sym, int argc, token_data ** argv, struct obstack * expansion)
{
	switch(SYMBOL_TYPE(sym)) {
		...
		case TOKEN_VOID:
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad symbol type in call_macro ()"));
		    abort();
	}
}
...
static void expand_macro(SymbolTableEntry * sym)
{
	...
	if(nesting_limit > 0 && expansion_level > nesting_limit)
		M4ERROR((EXIT_FAILURE, 0, "recursion limit of %d exceeded, use -L<N> to change it", nesting_limit));
	macro_call_id++;
