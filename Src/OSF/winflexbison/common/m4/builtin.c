...
const builtin * find_builtin_by_addr(builtin_func * func)
{
	const builtin * bp;
...
static void free_pattern_buffer(struct re_pattern_buffer * buf, struct re_registers * regs)
{
	regfree(buf);
	SAlloc::F(regs->start);
	SAlloc::F(regs->end);
...
| checked during define and pushdef.  Exit on failure.             |
   `-----------------------------------------------------------------*/
void set_macro_sequence(const char * regexp)
{
	const char * msg;
	if(!regexp)
	...
	if(msg != NULL) {
		M4ERROR((EXIT_FAILURE, 0, "--warn-macro-sequence: bad regular expression `%s': %s", regexp, msg));
	}
	re_set_registers(&macro_sequence_buf, &macro_sequence_regs, macro_sequence_regs.num_regs, macro_sequence_regs.start, macro_sequence_regs.end);
	...
	if(macro_sequence_inuse && text) {
		...
		while((offset = re_search(&macro_sequence_buf, defn, len, offset,
			...
			if(macro_sequence_regs.start[0] == macro_sequence_regs.end[0])
				...
				tmp = defn[offset];
				defn[offset] = '\0';
				M4ERROR((warning_status, 0, "Warning: definition of `%s' contains sequence `%s'", name, defn + macro_sequence_regs.start[0]));
				defn[offset] = tmp;
			}
		}
		if(offset == -2)
			M4ERROR((warning_status, 0, "error checking --warn-macro-sequence for macro `%s'", name));
	}
}
...
static bool FASTCALL bad_argc(token_data * name, int argc, int min, int max)
{
	...
	if(min > 0 && argc < min) {
		if(!suppress_warnings)
			M4ERROR((warning_status, 0, "Warning: too few arguments to builtin `%s'", TOKEN_DATA_TEXT(name)));
		isbad = true;
	}
	else if(max > 0 && argc > max && !suppress_warnings)
		M4ERROR((warning_status, 0, "Warning: excess arguments to builtin `%s' ignored", TOKEN_DATA_TEXT(name)));
	return isbad;
}
...
static bool numeric_arg(token_data * macro, const char * arg, int * valuep)
{
	char * endp;
	if(*arg == '\0') {
		*valuep = 0;
		M4ERROR((warning_status, 0, "empty string treated as 0 in builtin `%s'", TOKEN_DATA_TEXT(macro)));
	}
	else {
		errno = 0;
		*valuep = strtol(arg, &endp, 10);
		if(*endp != '\0') {
			M4ERROR((warning_status, 0, "non-numeric argument to builtin `%s'", TOKEN_DATA_TEXT(macro)));
			return false;
		}
		if(isspace(to_uchar(*arg)))
			M4ERROR((warning_status, 0, "leading whitespace ignored in builtin `%s'", TOKEN_DATA_TEXT(macro)));
		else if(errno == ERANGE)
			M4ERROR((warning_status, 0, "numeric overflow detected in builtin `%s'", TOKEN_DATA_TEXT(macro)));
	}
	return true;
}
...
const char * ntoa(int32_t value, int radix)
{
	bool negative;
	uint32_t uvalue;
	static char str[256];
...
static void dump_args(struct obstack * obs, int argc, token_data ** argv,
...
static void define_macro(int argc, token_data ** argv, symbol_lookup mode)
{
	const builtin * bp;
	if(bad_argc(argv[0], argc, 2, 3))
		return;
	if(TOKEN_DATA_TYPE(argv[1]) != TOKEN_TEXT) {
		M4ERROR((warning_status, 0, "Warning: %s: invalid macro name ignored", ARG(0)));
		return;
	}
	if(argc == 2) {
	...
	switch(TOKEN_DATA_TYPE(argv[2])) {
		...
		case TOKEN_VOID:
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad token data type in define_macro ()"));
			abort();
	}
}

static void m4_define(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	define_macro(argc, argv, SYMBOL_INSERT);
}

static void m4_undefine(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	int i;
...
static void m4_pushdef(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
...
static void m4_ifelse(struct obstack * obs, int argc, token_data ** argv)
{
	const char * result;
	...
	if(argc == 1) {
		...
		for(i = 1; i < argc; i++) {
			...
			if(s != NULL && SYMBOL_TYPE(s) != TOKEN_VOID)
				...
				M4ERROR((warning_status, 0, "undefined macro `%s'", TOKEN_DATA_TEXT(argv[i])));
		}
	}
	...
	for(; data.size > 0; --data.size, data.base++) {
		...
		switch(SYMBOL_TYPE(data.base[0])) {
			...
			case TOKEN_FUNC:
			    ...
			    if(bp == NULL) {
				    M4ERROR((warning_status, 0, "INTERNAL ERROR: builtin not found in builtin table"));
				    abort();
			    }
			    DEBUG_PRINT1("<%s>\n", bp->name);
			    break;
			...
			case TOKEN_VOID:
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad token data type in m4_dumpdef ()"));
			    abort();
			    break;
		}
	}
...
static void m4_builtin(struct obstack * obs, int argc, token_data ** argv)
{
	const builtin * bp;
	const char * name;
	if(bad_argc(argv[0], argc, 2, -1))
	...
	if(bp->func == m4_placeholder)
		M4ERROR((warning_status, 0, "undefined builtin `%s'", name));
	else {
		int i;
...
static void m4_indir(struct obstack * obs, int argc, token_data ** argv)
{
	...
	if(s == NULL || SYMBOL_TYPE(s) == TOKEN_VOID)
		M4ERROR((warning_status, 0, "undefined macro `%s'", name));
	else {
		int i;
	...
	for(i = 1; i < (uint)argc; i++) {
		...
		switch(SYMBOL_TYPE(s)) {
			...
			case TOKEN_FUNC:
			    b = SYMBOL_FUNC(s);
			    if(b == m4_placeholder)
				    M4ERROR((warning_status, 0, "builtin `%s' requested by frozen file is not supported", arg));
			    else if(argc != 2)
				    M4ERROR((warning_status, 0, "Warning: cannot concatenate builtin `%s'", arg));
			    else
				    push_macro(b);
			    break;
			case TOKEN_VOID:
			    ...
			    break;
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad symbol type in m4_defn ()"));
			    abort();
		}
	}
}
...
static void m4_syscmd(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
...
static void m4_esyscmd(struct obstack * obs, int argc, token_data ** argv)
{
	const char * cmd = ARG(1);
	char * old_cat_string = cat_string;
	size_t len = 0;
...
static void m4_sysval(struct obstack * obs, int argc M4_GNUC_UNUSED,
...
static void m4_eval(struct obstack * obs, int argc, token_data ** argv)
{
	int32_t value = 0;
	int radix = 10;
	int min = 1;
	...
	if(radix < 1 || radix > (int)strlen(digits)) {
		M4ERROR((warning_status, 0, "radix %d in builtin `%s' out of range", radix, ARG(0)));
		return;
	}
	...
	if(argc >= 4 && !numeric_arg(argv[0], ARG(3), &min))
		return;
	if(min < 0) {
		M4ERROR((warning_status, 0, "negative width to builtin `%s'", ARG(0)));
		return;
	}
	if(!*ARG(1))
		M4ERROR((warning_status, 0, "empty string treated as 0 in builtin `%s'", ARG(0)));
...
static void m4_incr(struct obstack * obs, int argc, token_data ** argv)
{
	int value;
...
static void m4_decr(struct obstack * obs, int argc, token_data ** argv)
{
	int value;
...
static void m4_divert(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	int i = 0;
...
static void m4_divnum(struct obstack * obs, int argc, token_data ** argv)
{
...
static void m4_undivert(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	int i, file;
	FILE * fp;
	char * endp;
	if(argc == 1)
		...
		for(i = 1; i < argc; i++) {
			...
			if(*endp == '\0' && !isspace(to_uchar(*ARG(i))))
				...
				M4ERROR((warning_status, 0, "non-numeric argument to builtin `%s'", ARG(0)));
			else {
				fp = m4_path_search(ARG(i), NULL);
				if(fp != NULL) {
					insert_file(fp);
					if(fclose(fp) == EOF)
						M4ERROR((warning_status, errno, "error undiverting `%s'", ARG(i)));
				}
				else
					M4ERROR((warning_status, errno, "cannot undivert `%s'", ARG(i)));
			}
		}
}
...
static void m4_dnl(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 1))
		return;
...
static void m4_shift(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, -1))
		return;
...
static void m4_changequote(struct obstack * obs M4_GNUC_UNUSED, int argc,
    token_data ** argv)
{
...
static void m4_changecom(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 3))
		return;
...
static void m4_changeword(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, 2))
		return;
...
static void include(int argc, token_data ** argv, bool silent)
{
	FILE * fp;
	char * name;
	if(bad_argc(argv[0], argc, 2, 2))
...
static void m4_include(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
...
static void m4_sinclude(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
...
static void mkstemp_helper(struct obstack * obs, const char * me, const char * pattern,
    size_t len)
{
	int fd;
	size_t i;
...
static void m4_maketemp(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, 2))
		return;
	if(no_gnu_extensions) {
...
static void m4_mkstemp(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, 2))
		return;
	mkstemp_helper(obs, ARG(0), ARG(1), strlen(ARG(1)));
...
static void m4_errprint(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, -1))
		return;
...
static void m4___file__(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 1))
		return;
	obstack_grow(obs, lquote.string, lquote.length);
...
static void m4___line__(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 1))
		return;
...
static void m4___program__(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 1))
		return;
	obstack_grow(obs, lquote.string, lquote.length);
	obstack_grow(obs, get_program_name(), strlen(get_program_name()));
	obstack_grow(obs, rquote.string, rquote.length);
}
...
static void M4_GNUC_NORETURN m4_m4exit(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	...
	if(exit_code < 0 || exit_code > 255) {
		M4ERROR((warning_status, 0, "exit status out of range: `%d'", exit_code));
		exit_code = EXIT_FAILURE;
	}
...
static void m4_m4wrap(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, -1))
		return;
	if(no_gnu_extensions)
...
static void m4_debugmode(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	int new_debug_level;
	int change_flag;
	if(bad_argc(argv[0], argc, 1, 2))
	...
	if(argc == 1)
		...
		if(new_debug_level < 0)
			M4ERROR((warning_status, 0, "Debugmode: bad debug flags: `%s'", ARG(1)));
		else {
			switch(change_flag) {
				case 0: debug_level = new_debug_level; break;
				case '+': debug_level |= new_debug_level; break;
				case '-': debug_level &= ~new_debug_level; break;
				default: M4ERROR((warning_status, 0, "INTERNAL ERROR: bad flag in m4_debugmode ()")); abort();
			}
		}
	}
}
...
static void m4_debugfile(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 1, 2))
		return;
	if(argc == 1)
		...
		M4ERROR((warning_status, errno, "cannot set debug file `%s'", ARG(1)));
}
...
static void m4_len(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, 2))
		return;
...
static void m4_index(struct obstack * obs, int argc, token_data ** argv)
{
	const char * haystack;
	const char * result;
	int retval;
...
static void m4_substr(struct obstack * obs, int argc, token_data ** argv)
{
	int start = 0;
	int length, avail;
...
static void m4_translit(struct obstack * obs, int argc, token_data ** argv)
{
...
static void m4_format(struct obstack * obs, int argc, token_data ** argv)
{
	if(bad_argc(argv[0], argc, 2, -1))
		return;
		...
		switch(ch) {
			case '0':
			    if(!substitute_warned) {
				    M4ERROR((warning_status, 0, "Warning: \\0 will disappear, use \\& instead in replacements"));
				    substitute_warned = 1;
			    }
			/* Fall through.  */
			...
			case '7': case '8': case '9':
			    ...
			    if(regs->num_regs - 1 <= ind)
				    M4ERROR((warning_status, 0, "Warning: sub-expression %d not present", ch));
			...
			case '\0':
			    M4ERROR((warning_status, 0, "Warning: trailing \\ ignored in replacement"));
			    return;
			default:
			    obstack_1grow(obs, ch);
			    repl++;
...
void init_pattern_buffer(struct re_pattern_buffer * buf, struct re_registers * regs)
{
	buf->translate = NULL;
	buf->fastmap = NULL;
	buf->buffer = NULL;
...
static void m4_regexp(struct obstack * obs, int argc, token_data ** argv)
{
	...
	if(startpos == -2)
		M4ERROR((warning_status, 0, "error matching regular expression `%s'", regexp));
	else if(argc == 3)
...
static void m4_patsubst(struct obstack * obs, int argc, token_data ** argv)
{
	const char * victim;    /* first argument */
	const char * regexp;    /* regular expression */
...
void m4_placeholder(struct obstack * obs M4_GNUC_UNUSED, int argc, token_data ** argv)
{
	M4ERROR((warning_status, 0, "builtin `%s' requested by frozen file is not supported", ARG(0)));
}
