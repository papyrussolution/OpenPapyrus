...
static int profile_strcmp(const char * s1, const char * s2)
{
	int i = 1;
	int result;
	while(*s1 && *s1 == *s2) {
...
void symtab_init(void)
{
...
#ifdef DEBUG_SYM
	{
		...
		if(e != 0)
			M4ERROR((warning_status, 0, "INTERNAL ERROR: unable to show symtab profile"));
	}
#endif /* DEBUG_SYM */
}
...
void free_symbol(symbol * sym)
{
	if(SYMBOL_PENDING_EXPANSIONS(sym) > 0)
		...
		SAlloc::F(SYMBOL_NAME(sym));
		if(SYMBOL_TYPE(sym) == TOKEN_TEXT)
			SAlloc::F(SYMBOL_TEXT(sym));
		SAlloc::F(sym);
	}
}
...
symbol * lookup_symbol(const char * name, symbol_lookup mode)
{
	...
	switch(mode)
	{
		...
		case SYMBOL_LOOKUP:
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: invalid mode to symbol_lookup ()"));
		    abort();
		    return NULL;
	}
}
...
static void M4_GNUC_UNUSED symtab_debug(void)
{
	token_data td;
	const char * text;
	symbol * s;
