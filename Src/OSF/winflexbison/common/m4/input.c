...
const char * current_file; /* Current input file name.  */
int current_line; /* Current input line number.  */
static struct obstack token_stack; /* Obstack for storing individual tokens.  */
static struct obstack file_names; /* Obstack for storing file names.  */
static struct obstack * wrapup_stack; /* Wrapup input stack.  */
static struct obstack * current_input; /* Current stack, from input or wrapup.  */
static void * token_bottom; /* Bottom of token_stack, for obstack_free.  */
static input_block * isp; /* Pointer to top of current_input.  */
static input_block * wsp; /* Pointer to top of wrapup_stack.  */
static input_block * next; /* Aux. for handling split push_string ().  */
static bool start_of_input_line; /* Flag for next_char () to increment current_line.  */
static bool input_change; /* Flag for next_char () to recognize change in input block.  */
...
static void pop_input(void)
{
	...
	switch(isp->type) {
		...
		case INPUT_FILE:
		    if(debug_level & DEBUG_TRACE_INPUT) {
			    if(tmp)
				    DEBUG_MESSAGE2("input reverted to %s, line %d", tmp->file, tmp->line);
			    else
		...
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: input stack botch in pop_input ()"));
		    abort();
	}
...
static void init_macro_token(token_data * td)
{
	if(isp->type != INPUT_MACRO) {
		M4ERROR((warning_status, 0, "INTERNAL ERROR: bad call to init_macro_token ()"));
		abort();
	}
...
static int peek_input(void)
{
	...
	while(1) {
		...
		switch(block->type) {
			...
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: input stack botch in peek_input ()"));
			    abort();
		}
		block = block->prev;
	}
...
#define next_char() (isp && isp->type == INPUT_STRING && isp->u.u_s.string[0] && !input_change ? to_uchar(*isp->u.u_s.string++) : next_char_1())
...
static int next_char_1(void)
{
	...
	while(1) {
		...
		switch(isp->type) {
			...
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: input stack botch in next_char ()"));
			    abort();
		}
...
void skip_line(void)
{
	...
	if(ch == CHAR_EOF)
		...
		M4ERROR_AT_LINE((warning_status, 0, file, line, "Warning: end of file treated as newline"));
...
void set_quotes(const char * lq, const char * rq)
{
	SAlloc::F(lquote.string);
	SAlloc::F(rquote.string);
...
void set_comment(const char * bc, const char * ec)
{
...
token_type next_token(token_data * td, int * line)
{
	int ch;
	int quote_level;
	token_type type;
	...
	*line = current_line;
	if(MATCH(ch, bcomm.string, true)) {
		...
		if(ch != CHAR_EOF)
			...
			/* current_file changed to "" if we see CHAR_EOF, use the
			   previous value we stored earlier.  */
			M4ERROR_AT_LINE((EXIT_FAILURE, 0, file, *line, "ERROR: end of file in comment"));
		type = TOKEN_STRING;
	}
		...
		switch(ch) {
			...
			case ')': type = TOKEN_CLOSE; break;
			default: type = TOKEN_SIMPLE; break;
		}
		obstack_1grow(&token_stack, ch);
	}
	else {
		...
		while(1) {
			...
			if(ch == CHAR_EOF)
				...
				M4ERROR_AT_LINE((EXIT_FAILURE, 0, file, *line, "ERROR: end of file in string"));
...
token_type peek_token(void)
{
	...
	if(ch == CHAR_EOF) {
		...
		switch(ch) {
			case '(': result = TOKEN_OPEN; break;
			case ',': result = TOKEN_COMMA; break;
...
static const char * token_type_string(token_type t)
{
	switch(t) { /* TOKSW */
		...
		case TOKEN_MACDEF: return "MACDEF";
		default: abort();
	}
}

static void print_token(const char * s, token_type t, token_data * td)
{
	xfprintf(stderr, "%s: ", s);
	switch(t) { /* TOKSW */
		case TOKEN_OPEN:
		case TOKEN_COMMA:
		case TOKEN_CLOSE:
		case TOKEN_SIMPLE: xfprintf(stderr, "char:"); break;
		case TOKEN_WORD: xfprintf(stderr, "word:"); break;
		case TOKEN_STRING: xfprintf(stderr, "string:"); break;
		case TOKEN_MACDEF: xfprintf(stderr, "macro: %p\n", TOKEN_DATA_FUNC(td)); break;
		case TOKEN_EOF: xfprintf(stderr, "eof\n"); break;
	}
	xfprintf(stderr, "\t\"%s\"\n", TOKEN_DATA_TEXT(td));
}

static void M4_GNUC_UNUSED lex_debug(void)
{
	token_type t;
	token_data td;
