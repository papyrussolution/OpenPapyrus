...
static size_t next_prime(size_t estimate)
{
...
static void hash_resize(gl_list_t list, size_t estimate)
{
	...
	if(new_size > list->table_size) {
		...
		if(size_overflow_p(xtimes(new_size, sizeof(gl_hash_entry_t))))
			goto fail;
		new_table = (gl_hash_entry_t*)SAlloc::C(new_size, sizeof(gl_hash_entry_t));
		if(new_table == NULL)
			goto fail;
