...
#include "gl_anyhash_list1.h" /* Generic hash-table code.  */
#include "gl_anylinked_list1.h" /* Generic linked list code.  */
#include "gl_anyhash_list2.h" /* Generic hash-table code.  */

/* Resize the hash table if needed, after list->count was incremented.  */
static void hash_resize_after_add(gl_list_t list)
{
...
static void remove_from_bucket(gl_list_t list, gl_list_node_t node)
{
	size_t bucket = node->h.hashcode % list->table_size;
	gl_hash_entry_t * p;
