...
static FILE * m4_fopen(const char * file)
{
	...
	if(fp) {
		...
		//       M4ERROR ((warning_status, errno, "Warning: cannot protect input file across forks"));
	}
	return fp;
}
...
FILE * m4_path_search(const char * file, char ** result)
{
	FILE * fp;
	includes * incl;
