...
static void rebalance(gl_oset_t set, gl_oset_node_t node, int height_diff, gl_oset_node_t parent)
{
	for(;;) {
...
static gl_oset_node_t gl_tree_nx_add_first(gl_oset_t set, const void * elt)
{
	/* Create new node.  */
	gl_oset_node_t new_node = (struct gl_oset_node_impl *)SAlloc::M(sizeof(struct gl_oset_node_impl));
	if(new_node == NULL)
		return NULL;
	new_node->left = NULL;
	new_node->right = NULL;
...
static gl_oset_node_t gl_tree_nx_add_before(gl_oset_t set, gl_oset_node_t node, const void * elt)
{
	/* Create new node.  */
	gl_oset_node_t new_node = (struct gl_oset_node_impl *)SAlloc::M(sizeof(struct gl_oset_node_impl));
	bool height_inc;
...
static gl_oset_node_t gl_tree_nx_add_after(gl_oset_t set, gl_oset_node_t node, const void * elt)
{
	/* Create new node.  */
	gl_oset_node_t new_node = (struct gl_oset_node_impl *)SAlloc::M(sizeof(struct gl_oset_node_impl));
	bool height_inc;
...
static bool gl_tree_remove_node(gl_oset_t set, gl_oset_node_t node)
{
...
#define GL_SKIP_TREE_UPDATE_IMPLEMENTATION // @sobolev
...
void gl_avltree_oset_check_invariants(gl_oset_t set)
{
	size_t counter = 0;
	if(set->root != NULL)
