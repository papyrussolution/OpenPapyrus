...
int debug_decode(const char * opts)
{
	int level;
	if(opts == NULL || *opts == '\0')
		...
		for(level = 0; *opts; opts++) {
			switch(*opts) {
				case 'a': level |= DEBUG_TRACE_ARGS; break;
				case 'e': level |= DEBUG_TRACE_EXPANSION; break;
				case 'q': level |= DEBUG_TRACE_QUOTE; break;
				case 't': level |= DEBUG_TRACE_ALL; break;
				case 'l': level |= DEBUG_TRACE_LINE; break;
				case 'f': level |= DEBUG_TRACE_FILE; break;
				case 'p': level |= DEBUG_TRACE_PATH; break;
				case 'c': level |= DEBUG_TRACE_CALL; break;
				case 'i': level |= DEBUG_TRACE_INPUT; break;
				case 'x': level |= DEBUG_TRACE_CALLID; break;
				case 'V': level |= DEBUG_TRACE_VERBOSE; break;
				default: return -1;
			}
		}
	}
	// This is to avoid screwing up the trace output due to changes in the debug_level.
...
static void debug_set_file(FILE * fp)
{
	...
	if(_debug != NULL && _debug != stdout) {
		...
		if(stdout_stat.st_ino == debug_stat.st_ino && stdout_stat.st_dev == debug_stat.st_dev && stdout_stat.st_ino != 0) {
...
bool debug_set_output(const char * name)
{
	FILE * fp;
	if(name == NULL)
		debug_set_file(stderr);
...
//        M4ERROR ((warning_status, errno, "Warning: cannot protect debug file across forks"));
		debug_set_file(fp);
	}
	return true;
}
...
static void trace_format(const char * fmt, ...)
{
	va_list args;
	char ch;
...
static void trace_header(int id)
{
...
void trace_prepre(const char * name, int id)
{
	trace_header(id);
	trace_format("%s ...", name);
	trace_flush();
...
void trace_pre(const char * name, int id, int argc, token_data ** argv)
{
	int i;
	const builtin * bp;
		...
		for(i = 1; i < argc; i++) {
			...
			switch(TOKEN_DATA_TYPE(argv[i]))
			{
				...
				case TOKEN_FUNC:
				    ...
				    if(bp == NULL) {
					    M4ERROR((warning_status, 0, "INTERNAL ERROR: builtin not found in builtin table! (trace_pre ())"));
					    abort();
				    }
				...
				case TOKEN_VOID:
				default:
				    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad token data type (trace_pre ())"));
				    abort();
			}
		}
		trace_format(")");
...
void trace_post(const char * name, int id, int argc, const char * expanded)
{
	if(debug_level & DEBUG_TRACE_CALL) {
		trace_header(id);
