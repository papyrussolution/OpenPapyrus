...
void m4_error(int status, int errnum, const char * format, ...)
{
	va_list args;
...
#define WRITE(f, b, l) ignore_value(_write(f, b, l))
		WRITE(STDERR_FILENO, get_program_name(), strlen(get_program_name()));
		WRITE(STDERR_FILENO, ": ", 2);
		WRITE(STDERR_FILENO, program_error_message, strlen(program_error_message));
...
#endif /* DEBUG_STKOVF */
	head = tail = NULL; /* First, we decode the arguments, to size up tables and stuff.  */
	getopt_init(); /* reinit getopt module */
	while((optchar = getopt_long(argc, (char**)argv, OPTSTRING, long_options, NULL)) != -1)
		switch(optchar) {
			default:
			    usage(EXIT_FAILURE);
			case 'B':
			...
			case 'E':
			    ...
			    break;
			case 'F': frozen_file_to_write = optarg; break;
			case 'G': no_gnu_extensions = 1; break;
			case 'H':
			    hash_table_size = strtol(optarg, NULL, 10);
			    if(hash_table_size == 0)
				    hash_table_size = HASHMAX;
			    break;
			case 'I': add_include_directory(optarg); break;
			case 'L': nesting_limit = strtol(optarg, NULL, 10); break;
			case 'P': prefix_all_builtins = 1; break;
			case 'Q': suppress_warnings = 1; break;
			case 'R': frozen_file_to_read = optarg; break;
#ifdef ENABLE_CHANGEWORD
			case 'W': user_word_regexp = optarg; break;
#endif
			case 'd':
			...
			case 'g': no_gnu_extensions = 0; break;
			case 'l':
			...
			case VERSION_OPTION:
			    ...
			    exit(EXIT_SUCCESS);
			    break;
			case HELP_OPTION: usage(EXIT_SUCCESS); break;
		}
	defines = head;
