...
static long arg_long(const char * str)
{
	char * endp;
	long value;
	size_t len = strlen(str);
...
static double arg_double(const char * str)
{
	char * endp;
	double value;
	size_t len = strlen(str);
...
void expand_format(struct obstack * obs, int argc, token_data ** argv)
{
	...
	while(1) {
		...
		*p = '\0';
		switch(datatype) {
			case CHAR: str = xasprintf(fstart, width, ARG_INT(argc, argv)); break;
			case INT: str = xasprintf(fstart, width, prec, ARG_INT(argc, argv)); break;
			case LONG: str = xasprintf(fstart, width, prec, ARG_LONG(argc, argv)); break;
			case DOUBLE: str = xasprintf(fstart, width, prec, ARG_DOUBLE(argc, argv)); break;
			case STR: str = xasprintf(fstart, width, prec, ARG_STR(argc, argv)); break;
			default: abort();
		}
