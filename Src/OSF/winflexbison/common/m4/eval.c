...
typedef enum eval_token {
	EVAL_TOKEN_ERROR = 0, // @sobolev ERROR-->EVAL_TOKEN_ERROR = 0
	BADOP,
	PLUS, 
	MINUS,
	EXPONENT,
...
typedef enum eval_error {
	EVAL_NO_ERROR = 0, // @sobolev NO_ERROR-->EVAL_NO_ERROR = 0
	DIVIDE_ZERO,
	MODULO_ZERO,
	NEGATIVE_EXPONENT,
...
static const char * eval_text; /* Pointer to next character of input text.  */
...
static eval_token eval_lex(int32_t * val)
{
...
bool evaluate(const char * expr, int32_t * val)
{
	eval_token et;
	eval_error err;
	eval_init_lex(expr);
	...
	if(err == EVAL_NO_ERROR && *eval_text != '\0') {
	...
	switch(err) {
		case EVAL_NO_ERROR:
		    break;
		case MISSING_RIGHT:
		    M4ERROR((warning_status, 0, "bad expression in eval (missing right parenthesis): %s", expr));
		    break;
		case SYNTAX_ERROR:
		    M4ERROR((warning_status, 0, "bad expression in eval: %s", expr));
		    break;
		case UNKNOWN_INPUT:
		    M4ERROR((warning_status, 0, "bad expression in eval (bad input): %s", expr));
		    break;
		case EXCESS_INPUT:
		    M4ERROR((warning_status, 0, "bad expression in eval (excess input): %s", expr));
		    break;
		case INVALID_OPERATOR:
		    M4ERROR((warning_status, 0, "invalid operator in eval: %s", expr));
		    retcode = EXIT_FAILURE;
		    break;
		case DIVIDE_ZERO:
		    M4ERROR((warning_status, 0, "divide by zero in eval: %s", expr));
		    break;
		case MODULO_ZERO:
		    M4ERROR((warning_status, 0, "modulo by zero in eval: %s", expr));
		    break;
		case NEGATIVE_EXPONENT:
		    M4ERROR((warning_status, 0, "negative exponent in eval: %s", expr));
		    break;
		default:
		    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad error code in evaluate ()"));
		    abort();
	}
	return err != EVAL_NO_ERROR;
}
...
static eval_error logical_or_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	if((er = logical_and_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((et = eval_lex(&v2)) == LOR) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
	...
	if(et == EVAL_TOKEN_ERROR)
		return UNKNOWN_INPUT;
	eval_undo();
	return EVAL_NO_ERROR;
}

static eval_error logical_and_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	if((er = or_term(et, v1)) != EVAL_NO_ERROR)
		return er;
...
static eval_error or_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	if((er = xor_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((et = eval_lex(&v2)) == OR) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = xor_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		*v1 |= v2;
	}
	if(et == EVAL_TOKEN_ERROR)
...
static eval_error xor_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	if((er = and_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((et = eval_lex(&v2)) == XOR) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = and_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		*v1 ^= v2;
	}
	if(et == EVAL_TOKEN_ERROR)
...
static eval_error and_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	if((er = equality_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((et = eval_lex(&v2)) == AND) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = equality_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		*v1 &= v2;
	}
	if(et == EVAL_TOKEN_ERROR)
...
static eval_error equality_term(eval_token et, int32_t * v1)
{
	eval_token op;
	int32_t v2;
	eval_error er;
	if((er = cmp_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	...
	while((op = eval_lex(&v2)) == EQ || op == NOTEQ || op == ASSIGN) {
		...
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = cmp_term(et, &v2)) != EVAL_NO_ERROR)
			return er;

		if(op == ASSIGN) {
			M4ERROR((warning_status, 0, "Warning: recommend ==, not =, for equality operator"));
			op = EQ;
		}
		*v1 = (op == EQ) == (*v1 == v2);
	}
	if(op == EVAL_TOKEN_ERROR)
		return UNKNOWN_INPUT;
...
static eval_error cmp_term(eval_token et, int32_t * v1)
{
	eval_token op;
	int32_t v2;
	eval_error er;
	if((er = shift_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((op = eval_lex(&v2)) == GT || op == GTEQ || op == LS || op == LSEQ) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = shift_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		switch(op) {
			case GT:
			    *v1 = *v1 > v2;
			...
			case LSEQ:
			    *v1 = *v1 <= v2;
			    break;
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad comparison operator in cmp_term ()"));
			    abort();
		}
	}
	if(op == EVAL_TOKEN_ERROR)
...
static eval_error shift_term(eval_token et, int32_t * v1)
{
	eval_token op;
	int32_t v2;
	uint32_t u1;
	eval_error er;
	if((er = add_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((op = eval_lex(&v2)) == LSHIFT || op == RSHIFT) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = add_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		...
		switch(op) {
			...
			case RSHIFT:
			    ...
			    *v1 = *v1 < 0 ? ~u1 : u1;
			    break;
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad shift operator in shift_term ()"));
			    abort();
		}
	}
	if(op == EVAL_TOKEN_ERROR)
...
static eval_error add_term(eval_token et, int32_t * v1)
{
	eval_token op;
	int32_t v2;
	eval_error er;
	if((er = mult_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((op = eval_lex(&v2)) == PLUS || op == MINUS) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = mult_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
...
static eval_error mult_term(eval_token et, int32_t * v1)
{
	eval_token op;
	int32_t v2;
	eval_error er;
	if((er = exp_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	while((op = eval_lex(&v2)) == TIMES || op == DIVIDE || op == MODULO) {
		et = eval_lex(&v2);
		if(et == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = exp_term(et, &v2)) != EVAL_NO_ERROR)
			return er;
		...
		switch(op) {
			...
			default:
			    M4ERROR((warning_status, 0, "INTERNAL ERROR: bad operator in mult_term ()"));
			    abort();
		}
	}
	if(op == EVAL_TOKEN_ERROR)
...
static eval_error exp_term(eval_token et, int32_t * v1)
{
	uint32_t result;
	int32_t v2;
	eval_error er;
	if((er = unary_term(et, v1)) != EVAL_NO_ERROR)
		return er;
...
static eval_error unary_term(eval_token et, int32_t * v1)
{
	eval_error er;
	if(et == PLUS || et == MINUS || et == NOT || et == LNOT) {
		eval_token et2 = eval_lex(v1);
		if(et2 == EVAL_TOKEN_ERROR)
			return UNKNOWN_INPUT;
		if((er = unary_term(et2, v1)) != EVAL_NO_ERROR)
			return er;
	...
	else if((er = simple_term(et, v1)) != EVAL_NO_ERROR)
		return er;
	return EVAL_NO_ERROR;
}

static eval_error simple_term(eval_token et, int32_t * v1)
{
	int32_t v2;
	eval_error er;
	switch(et) {
		case LEFTP:
		    ...
		    if(et == EVAL_TOKEN_ERROR)
			    return UNKNOWN_INPUT;
		    if((er = logical_or_term(et, v1)) != EVAL_NO_ERROR)
			    return er;
		    et = eval_lex(&v2);
		    if(et == EVAL_TOKEN_ERROR)
			    return UNKNOWN_INPUT;
