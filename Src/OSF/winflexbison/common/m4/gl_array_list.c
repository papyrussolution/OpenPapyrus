...
static gl_list_t gl_array_nx_create(gl_list_implementation_t implementation,
    gl_listelement_equals_fn equals_fn,
    gl_listelement_hashcode_fn hashcode_fn,
    gl_listelement_dispose_fn dispose_fn,
    bool allow_duplicates,
    size_t count, const void ** contents)
{
	...
	if(count > 0) {
		if(size_overflow_p(xtimes(count, sizeof(const void *))))
			goto fail;
		list->elements = (const void**)SAlloc::M(count * sizeof(const void *));
...
static const void * _GL_ATTRIBUTE_PURE gl_array_node_value(gl_list_t list, gl_list_node_t node)
{
...
static gl_list_node_t _GL_ATTRIBUTE_PURE gl_array_next_node(gl_list_t list, gl_list_node_t node)
{
	uintptr_t index = NODE_TO_INDEX(node);
	if(!(index < list->count))
		abort(); // Invalid argument
...
static int grow(gl_list_t list)
{
	...
	if(size_overflow_p(memory_size))
		...
		return -1;
	memory = (const void**)SAlloc::R(list->elements, memory_size);
	if(memory == NULL)
		/* Out of memory.  */
		return -1;
...
static gl_list_node_t gl_array_nx_add_first(gl_list_t list, const void * elt)
{
	size_t count = list->count;
	const void ** elements;
	size_t i;
...
static gl_list_node_t gl_array_nx_add_last(gl_list_t list, const void * elt)
{
	size_t count = list->count;
...
static gl_list_node_t gl_array_nx_add_before(gl_list_t list, gl_list_node_t node, const void * elt)
{
	size_t count = list->count;
	uintptr_t index = NODE_TO_INDEX(node);
	size_t position;
...
static gl_list_node_t gl_array_nx_add_after(gl_list_t list, gl_list_node_t node, const void * elt)
{
	size_t count = list->count;
	uintptr_t index = NODE_TO_INDEX(node);
	size_t position;
...
static gl_list_node_t gl_array_nx_add_at(gl_list_t list, size_t position, const void * elt)
{
	size_t count = list->count;
	const void ** elements;
	size_t i;
...
static bool gl_array_remove_node(gl_list_t list, gl_list_node_t node)
{
	size_t count = list->count;
	uintptr_t index = NODE_TO_INDEX(node);
	size_t position;
...
static bool gl_array_remove_at(gl_list_t list, size_t position)
{
	size_t count = list->count;
	const void ** elements;
	size_t i;
...
static bool gl_array_remove(gl_list_t list, const void * elt)
{
...
static void gl_array_iterator_free(gl_list_iterator_t * iterator _GL_UNUSED)
{
}
