...
/* Return an extended time value that contains S seconds and NS
   nanoseconds.  */
XTIME_INLINE xtime_t xtime_make(xtime_t s, long int ns)
{
...
XTIME_INLINE xtime_t xtime_sec(xtime_t t)
{
	return (XTIME_PRECISION == 1 ? t : t < 0 ? (t + XTIME_PRECISION - 1) / XTIME_PRECISION - 1 : xtime_nonnegative_sec(t));
}

/* Return the number of nanoseconds in T, which must be nonnegative.  */
XTIME_INLINE long int xtime_nonnegative_nsec(xtime_t t)
{
	return t % XTIME_PRECISION;
}

/* Return the number of nanoseconds in T.  */
XTIME_INLINE long int xtime_nsec(xtime_t t)
{
