...
struct context {
	...
	bool heuristic;
  #endif
	OFFSET too_expensive; /* Edit scripts longer than this are too expensive to compute.  */
...
static void diag(OFFSET xoff, OFFSET xlim, OFFSET yoff, OFFSET ylim, bool find_minimal, struct partition * part, struct context * ctxt)
{
  ...
  #define XREF_YREF_EQUAL(x, y)  XVECREF_YVECREF_EQUAL(ctxt, x, y)
	...
	OFFSET c;               /* Cost. */
	bool odd = (fmid - bmid) & 1; /* True if southeast corner is on an odd diagonal with respect to the northwest. */
	fd[fmid] = xoff;
	bd[bmid] = xlim;
	for(c = 1;; ++c) {
		...
		for(d = fmax; d >= fmin; d -= 2) {
			...
			for(x = x0, y = x0 - d; x < xlim && y < ylim && XREF_YREF_EQUAL(x, y); x++, y++)
				continue;
			if(x - x0 > SNAKE_LIMIT)
				big_snake = true;
			fd[d] = x;
		...
		for(d = bmax; d >= bmin; d -= 2) {
			...
			for(x = x0, y = x0 - d; xoff < x && yoff < y && XREF_YREF_EQUAL(x - 1, y - 1); x--, y--)
				continue;
			if(x0 - x > SNAKE_LIMIT)
				big_snake = true;
			bd[d] = x;
...
static bool compareseq(OFFSET xoff, OFFSET xlim, OFFSET yoff, OFFSET ylim, bool find_minimal, struct context * ctxt)
{
#ifdef ELEMENT
