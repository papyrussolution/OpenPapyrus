...
static void rule_rhs_print_xml(rule const * r, FILE * out, int level)
{
	if(*r->rhs >= 0) {
		xml_puts(out, level, "<rhs>");
		for(item_number * rhsp = r->rhs; 0 <= *rhsp; ++rhsp)
			xml_printf(out, level + 1, "<symbol>%s</symbol>", xml_escape(symbols[*rhsp]->tag));
		xml_puts(out, level, "</rhs>");
	}
	else {
		xml_puts(out, level, "<rhs>");
...
void grammar_rules_print_xml(FILE * out, int level)
{
	...
	for(rule_number r = 0; r < nrules + nuseless_productions; r++) {
		...
		first = false;
		{
			char const * usefulness = rule_useless_in_grammar_p(&rules[r]) ? "useless-in-grammar" : rule_useless_in_parser_p(&rules[r])  ? "useless-in-parser" : "useful";
			xml_indent(out, level + 2);
			fprintf(out, "<rule number=\"%d\" usefulness=\"%s\"", rules[r].number, usefulness);
			if(rules[r].precsym)
				fprintf(out, " percent_prec=\"%s\"", xml_escape(rules[r].precsym->symbol->tag));
			fputs(">\n", out);
		}
...
static void section(FILE * out, const char * s)
{
	fprintf(out, "%s\n", s);
...
{
	fprintf(out, "%s\n\n", title);
	fprintf(out, "ntokens = %d, nnterms = %d, nsyms = %d, nrules = %d, nritems = %d\n\n", ntokens, nnterms, nsyms, nrules, nritems);
	section(out, "Tokens");
	{
		fprintf(out, "Value  Sprec  Sassoc  Tag\n");
		for(symbol_number i = 0; i < ntokens; i++)
			fprintf(out, "%5d  %5d   %5d  %s\n", i, symbols[i]->content->prec, symbols[i]->content->assoc, symbols[i]->tag);
		fprintf(out, "\n\n");
	}
	...
	section(out, "Rules");
	{
		fprintf(out, "Num (Prec, Assoc, Useful, UselessChain) Lhs -> (Ritem Range) Rhs\n");
...
void grammar_rules_useless_report(const char * message)
{
	for(rule_number r = 0; r < nrules; ++r)
		/* Don't complain about rules whose LHS is useless, we already
		   complained about it.  */
		if(!reduce_nonterminal_useless_in_grammar(rules[r].lhs) && !rules[r].useful)
