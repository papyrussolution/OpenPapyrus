...
static symgraph ** prec_nodes; /* Precedence relation graph. */
static bool * used_assoc = NULL; /* Store which associativity is used.  */
bool tag_seen = false;

/* Whether SYM was defined by the user.  */

static bool symbol_is_user_defined(Symbol * sym)
{
	const bool eof_is_user_defined = !eoftoken->alias || STRNEQ(eoftoken->alias->tag, "$end");
	return sym->tag[0] != '$' && (eof_is_user_defined || (sym != eoftoken && sym->alias != errtoken))
	       && sym != errtoken && sym->alias != errtoken && sym != undeftoken && sym->alias != undeftoken;
}
...
static sym_content * sym_content_new(Symbol * s)
{
	sym_content * res = (sym_content *)xmalloc(sizeof *res);
	res->symbol = s;
	res->type_name = NULL;
...
static Symbol * symbol_new(uniqstr tag, Location loc)
{
	Symbol * res = (Symbol *)xmalloc(sizeof *res);
	uniqstr_assert(tag);
	/* If the tag is not a string (starts with a double quote), check
	   that it is valid for Yacc. */
	if(tag[0] != '\"' && tag[0] != '\'' && strchr(tag, '-'))
		complain(&loc, Wyacc, _("POSIX Yacc forbids dashes in symbol names: %s"), tag);
	res->tag = tag;
	res->location = loc;
...
static void sym_content_free(sym_content * sym)
{
	SAlloc::F(sym);
}
...
static void symbol_free(void * ptr)
{
...
/* If needed, swap first and second so that first has the earliest
   ...
   declaration first.
 */
static void symbols_sort(Symbol ** first, Symbol ** second)
{
...
static void locations_sort(Location * first, Location * second)
{
...
static semantic_type * semantic_type_new(uniqstr tag, const Location * loc)
{
	semantic_type * res = (semantic_type *)xmalloc(sizeof *res);
	uniqstr_assert(tag);
	res->tag = tag;
...
#define SYMBOL_ATTR_PRINT(Attr) if(s->content && s->content->Attr) fprintf(f, " %s { %s }", #Attr, s->content->Attr)
#define SYMBOL_CODE_PRINT(Attr) if(s->content && s->content->props[Attr].code) fprintf(f, " %s { %s }", #Attr, s->content->props[Attr].code)
...
static bool is_identifier(uniqstr s)
{
...
static void complain_symbol_redeclared(Symbol * s, const char * what, Location first, Location second)
{
...
static void complain_semantic_type_redeclared(semantic_type * s, const char * what, Location first, Location second)
{
	locations_sort(&first, &second);
	complain(&second, complaint, _("%s redeclaration for <%s>"), what, s->tag);
	subcomplain(&first, complaint, _("previous declaration"));
...
static void complain_class_redeclared(Symbol * sym, symbol_class cls, Location second)
{
	complain(&second, complaint, cls == token_sym ? _("symbol %s redeclared as a token") : _("symbol %s redeclared as a nonterminal"), sym->tag);
	if(!location_empty(sym->location))
		subcomplain(&sym->location, complaint, _("previous definition"));
}

static const Symbol * symbol_from_uniqstr_fuzzy(const uniqstr key)
{
	aver(symbols_sorted);
#define FSTRCMP_THRESHOLD 0.6
	...
	for(size_t i = 0; i < count; ++i) {
		...
		if(STRNEQ(key, sym->tag) && (sym->content->status == declared || sym->content->status == undeclared)) {
...
static void complain_symbol_undeclared(Symbol * sym)
{
...
void symbol_type_set(Symbol * sym, uniqstr type_name, Location loc)
{
	if(type_name) {
		...
		if(sym->content->type_name)
			complain_symbol_redeclared(sym, "%type", sym->content->type_loc, loc);
		else {
...
void symbol_code_props_set(Symbol * sym, code_props_type kind, code_props const * code)
{
	if(sym->content->props[kind].code)
		complain_symbol_redeclared(sym, code_props_type_string(kind), sym->content->props[kind].location, code->location);
	else
...
code_props * symbol_code_props_get(Symbol * sym, code_props_type kind)
{
	...
	/* Per-type code props.  */
	if(sym->content->type_name) {
		code_props * code = &semantic_type_get(sym->content->type_name, NULL)->props[kind];
		if(code->code)
			return code;
	}
...
void symbol_precedence_set(Symbol * sym, int prec, assoc a, Location loc)
{
	if(a != undef_assoc) {
		...
		if(s->prec)
			complain_symbol_redeclared(sym, assoc_to_string(a), s->prec_loc, loc);
		else {
			s->prec = prec;
			s->assoc = a;
			s->prec_loc = loc;
...
static void complain_pct_type_on_token(Location * loc)
{
	complain(loc, Wyacc, _("POSIX yacc reserves %%type to nonterminals"));
}

void symbol_class_set(Symbol * sym, symbol_class cls, Location loc, bool declaring)
{
	aver(cls != unknown_sym);
	sym_content * s = sym->content;
	if(cls == pct_type_sym) {
		if(s->Cls == token_sym)
			complain_pct_type_on_token(&loc);
		else if(s->Cls == unknown_sym)
			s->Cls = cls;
	}
	else if(s->Cls != unknown_sym && s->Cls != pct_type_sym && s->Cls != cls)
		complain_class_redeclared(sym, cls, loc);
	else {
		if(cls == token_sym && s->Cls == pct_type_sym)
			complain_pct_type_on_token(&sym->location);
		if(cls == nterm_sym && s->Cls != nterm_sym)
			s->number = nnterms++;
		else if(cls == token_sym && s->number == NUMBER_UNDEFINED)
			s->number = ntokens++;
		s->Cls = cls;
		if(declaring) {
			if(s->status == declared) {
				complain(&loc, Wother, _("symbol %s redeclared"), sym->tag);
				subcomplain(&sym->location, Wother, _("previous declaration"));
			}
			else {
				sym->location = loc;
				s->status = declared;
...
void symbol_code_set(Symbol * sym, int code, Location loc)
{
	...
	if(sym->content->Cls != token_sym)
		complain(&loc, complaint, _("nonterminals cannot be given a token code"));
	else if(*codep != CODE_UNDEFINED && *codep != code)
		complain(&loc, complaint, _("redefining code of token %s"), sym->tag);
	else if(code == INT_MAX)
		complain(&loc, complaint, _("code of token %s too large"), sym->tag);
	else {
		*codep = code;
...
static void symbol_check_defined(Symbol * sym)
{
	sym_content * s = sym->content;
	if(s->Cls == unknown_sym || s->Cls == pct_type_sym) {
		complain_symbol_undeclared(sym);
		s->Cls = nterm_sym;
		s->number = nnterms++;
	}
	if(s->Cls == token_sym && sym->tag[0] == '"' && !sym->is_alias)
		complain(&sym->location, Wdangling_alias, _("string literal %s not attached to a symbol"), sym->tag);
	for(int i = 0; i < 2; ++i)
		symbol_code_props_get(sym, (code_props_type)i)->is_used = true;
...
static void semantic_type_check_defined(semantic_type * sem_type)
{
	...
	if(sem_type->status == declared || !*sem_type->tag || sstreq(sem_type->tag, "*")) {
		...
		complain(&sem_type->location, Wother, _("type <%s> is used, but is not associated to any symbol"), sem_type->tag);
}
...
static void symbol_merge_properties(Symbol * sym, Symbol * str)
{
	if(str->content->type_name != sym->content->type_name) {
		if(str->content->type_name)
			symbol_type_set(sym, str->content->type_name, str->content->type_loc);
		else
			symbol_type_set(str, sym->content->type_name, sym->content->type_loc);
	}
	for(int i = 0; i < CODE_PROPS_SIZE; ++i)
		if(str->content->props[i].code)
			symbol_code_props_set(sym, (code_props_type)i, &str->content->props[i]);
		else if(sym->content->props[i].code)
			symbol_code_props_set(str, (code_props_type)i, &sym->content->props[i]);

	if(sym->content->prec || str->content->prec) {
		if(str->content->prec)
			symbol_precedence_set(sym, str->content->prec, str->content->assoc, str->content->prec_loc);
		else
			symbol_precedence_set(str, sym->content->prec, sym->content->assoc, sym->content->prec_loc);
	}
}
...
{
	if(sym->content->Cls != token_sym)
		complain(&loc, complaint, _("nonterminals cannot be given a string alias"));
	else if(str->alias)
		complain(&loc, Wother, _("symbol %s used more than once as a literal string"), str->tag);
	else if(sym->alias)
		complain(&loc, Wother, _("symbol %s given more than one literal string"), sym->tag);
	else {
...
static void symbol_pack(Symbol * sym)
{
	aver(sym->content->number != NUMBER_UNDEFINED);
	if(sym->content->Cls == nterm_sym)
		sym->content->number += ntokens;
	symbols[sym->content->number] = sym->content->symbol;
}

static void complain_code_redeclared(int num, Symbol * first, Symbol * second)
{
	symbols_sort(&first, &second);
	complain(&second->location, complaint, _("code %d reassigned to token %s"), num, second->tag);
	subcomplain(&first->location, complaint, _("previous declaration for %s"), first->tag);
}
...
static void symbol_translation(Symbol * sym)
{
	/* Nonterminal? */
	if(sym->content->Cls == token_sym && !sym->is_alias) {
		/* A token which translation has already been set?*/
		if(token_translations[sym->content->code] != undeftoken->content->number)
			complain_code_redeclared (sym->content->code, symbols[token_translations[sym->content->code]], sym);
		else
...
static inline bool hash_compare_symbol(const Symbol * m1, const Symbol * m2)
{
	/* Since tags are unique, we can compare the pointers themselves.  */
	return UNIQSTR_EQ(m1->tag, m2->tag);
}

static inline bool hash_compare_semantic_type(const semantic_type * m1, const semantic_type * m2)
{
	/* Since names are unique, we can compare the pointers themselves.  */
	return UNIQSTR_EQ(m1->tag, m2->tag);
}

static bool hash_symbol_comparator(void const * m1, void const * m2)
{
	return hash_compare_symbol((const Symbol *)m1, (const Symbol *)m2);
}

static bool hash_semantic_type_comparator(void const * m1, void const * m2)
{
	return hash_compare_semantic_type((const semantic_type *)m1, (const semantic_type *)m2);
}

static inline size_t hash_symbol(const Symbol * m, size_t tablesize)
{
	/* Since tags are unique, we can hash the pointer itself.  */
	return ((uintptr_t)m->tag) % tablesize;
}

static inline size_t hash_semantic_type(const semantic_type * m, size_t tablesize)
{
...
{
	return hash_symbol((const Symbol *)m, tablesize);
}

static size_t hash_semantic_type_hasher(void const * m, size_t tablesize)
{
	return hash_semantic_type((const semantic_type *)m, tablesize);
}
...
void symbols_new(void)
{
	...
	undeftoken->content->Cls = token_sym;
	...
	semantic_type_table = hash_xinitialize(HT_INITIAL_CAPACITY, NULL, hash_semantic_type_hasher, hash_semantic_type_comparator, free);
}
...
Symbol * symbol_from_uniqstr(const uniqstr key, Location loc)
{
	Symbol probe;
	probe.tag = key;
	Symbol * res = (Symbol *)hash_lookup(symbol_table, &probe);
	if(!res) {
...
semantic_type * semantic_type_from_uniqstr(const uniqstr key, const Location * loc)
{
	semantic_type probe;
	probe.tag = key;
	semantic_type * res = (semantic_type *)hash_lookup(semantic_type_table, &probe);
	if(!res) {
		/* First insertion in the hash. */
...
void symbols_free(void)
{
	...
	SAlloc::F(symbols);
	SAlloc::F(symbols_sorted);
	SAlloc::F(semantic_types_sorted);
}

static int symbol_cmp(void const * a, void const * b)
{
	return location_cmp((*(Symbol * const*)a)->location, (*(Symbol * const*)b)->location);
}

/* Store in *SORTED an array of pointers to the symbols contained in
   TABLE, sorted (alphabetically) by tag. */

static void table_sort(struct hash_table * table, Symbol *** sorted)
{
	aver(!*sorted);
	size_t count = hash_get_n_entries(table);
	*sorted = (Symbol **)xnmalloc(count + 1, sizeof **sorted);
...
static void symbols_token_translations_init(void)
{
	...
	token_translations = (symbol_number *)xnmalloc(max_code + 1, sizeof *token_translations);
	// Initialize all entries for literal tokens to the internal token number for $undefined, which represents all invalid inputs.  
...
void symbols_pack(void)
{
	...
	else if(startsymbol->content->Cls == token_sym)
		complain(&startsymbol_loc, fatal, _("the start symbol %s is a token"), startsymbol->tag);
...
static void init_prec_nodes(void)
{
	...
	for(int i = 0; i < nsyms; ++i) {
		prec_nodes[i] = (symgraph *)xmalloc(sizeof *prec_nodes[i]);
...
static symgraphlink * symgraphlink_new(graphid id, symgraphlink * next)
{
	symgraphlink * res = (symgraphlink *)xmalloc(sizeof *res);
	res->id = id;
	res->next = next;
	return res;
}
...
static bool register_precedence_second_symbol(symgraphlink ** first, graphid sym)
{
...
static void linkedlist_free(symgraphlink * node)
{
	if(node) {
		while(node->next) {
...
static inline bool is_assoc_useless(Symbol * s)
{
	return s && s->content->assoc != undef_assoc && s->content->assoc != precedence_assoc && !used_assoc[s->content->number];
}
...
void print_precedence_warnings(void)
{
	...
	for(int i = 0; i < nsyms; ++i) {
		Symbol * s = symbols[i];
		if(s && s->content->prec != 0 && !prec_nodes[i]->pred && !prec_nodes[i]->succ) {
			if(is_assoc_useless(s))
				complain(&s->content->prec_loc, Wprecedence, _("useless precedence and associativity for %s"), s->tag);
			else if(s->content->assoc == precedence_assoc)
				complain(&s->content->prec_loc, Wprecedence, _("useless precedence for %s"), s->tag);
		}
		else if(is_assoc_useless(s))
			complain(&s->content->prec_loc, Wprecedence, _("useless associativity for %s, use %%precedence"), s->tag);
	}
	SAlloc::F(used_assoc);
	assoc_free();
}
