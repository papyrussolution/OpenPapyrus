/* Declaration for error-reporting function for Bison.
   ...
   Copyright (C) 2000-2002, 2004-2006, 2009-2015, 2018-2020 Free Software Foundation, Inc.
...
// Internet.
static const char * diagnostics_url = "https://www.gnu.org/software/bison/manual/html_node/Diagnostics.html";
...
void begin_use_class(const char * s, FILE * out)
{
	if(out == stderr) {
		if(color_debug)
			fprintf(out, "<%s>", s);
...
void end_use_class(const char * s, FILE * out)
{
	if(out == stderr) {
		if(color_debug)
			fprintf(out, "</%s>", s);
...
static void begin_hyperlink(FILE * out, const char * ref)
{
	if(out == stderr)
		styled_ostream_set_hyperlink(errstream, ref, NULL);
}

static void end_hyperlink(FILE * out)
{
	if(out == stderr)
...
bool is_styled(FILE * out)
{
	...
	return (color_mode == color_yes || color_mode == color_html || (color_mode == color_tty && isatty(STDERR_FILENO)));
#else
	return false;
#endif
}
...
static const char* severity_style(severity s)
{
	switch(s) {
		case severity_disabled:
		case severity_unset: return "note";
		case severity_warning: return "warning";
		case severity_error:
		case severity_fatal: return "error";
	}
	abort();
}

/* Prefix for this type of message.  */
static const char* severity_prefix(severity s)
{
	switch(s) {
		case severity_disabled:
		case severity_unset: return "";
		case severity_warning: return _("warning");
		case severity_error: return _("error");
		case severity_fatal: return _("fatal error");
	}
	abort();
}

static void severity_print(severity s, FILE * out)
{
...
#endif
	errstream = color_mode == color_html ? html_styled_ostream_create(file_ostream_create(stderr), style_file_name) : styled_ostream_create(STDERR_FILENO, "(stderr)", TTYCTL_AUTO, style_file_name);
#if defined __clang__
...
void complain_init()
{
	...
	for(size_t b = 0; b < warnings_size; ++b) {
		warnings_flag[b] = (1 << b & warnings_default ? severity_warning : severity_unset);
...
static severity warning_severity(warnings flags)
{
	if(flags & fatal)
...
static void warnings_print_categories(warnings warn_flags, FILE * out)
{
	for(int wbit = 0; wbit < warnings_size; ++wbit)
		if(warn_flags & (1 << wbit)) {
			warnings w = (warnings)(1 << wbit);
			...
			char ref[200];
			snprintf(ref, sizeof ref, "%s#W%s", diagnostics_url, warning);
			begin_hyperlink(out, ref);
			ostream_printf(errstream, "-W%s%s", s == severity_error ? "error=" : "", warning);
			end_hyperlink(out);
 ...
 */
static void error_message(const Location * loc, warnings flags, severity sever, const char * message, va_list args)
{
...
static void complains(const Location * loc, warnings flags, const char * message, va_list args)
{
...
void complain(Location const * loc, warnings flags, const char * message, ...)
{
	va_list args;
...
void subcomplain(Location const * loc, warnings flags, const char * message, ...)
{
	va_list args;
	va_start(args, message);
	complains(loc, (warnings)(flags | note | silent), message, args);
	va_end(args);
}

void complain_args(Location const * loc, warnings w, int argc, char * argv[])
{
	switch(argc) {
		case 1: complain(loc, w, "%s", _(argv[0])); break;
		case 2: complain(loc, w, _(argv[0]), argv[1]); break;
		case 3: complain(loc, w, _(argv[0]), argv[1], argv[2]); break;
		case 4: complain(loc, w, _(argv[0]), argv[1], argv[2], argv[3]); break;
		case 5: complain(loc, w, _(argv[0]), argv[1], argv[2], argv[3], argv[4]); break;
		default: complain(loc, fatal, "too many arguments for complains"); break;
	}
}

void bison_directive(Location const * loc, char const * directive)
{
	complain(loc, Wyacc, _("POSIX Yacc does not support %s"), directive);
}

void deprecated_directive(Location const * loc, char const * old, char const * upd)
{
	if(warning_is_enabled(Wdeprecated)) {
		complain(loc, Wdeprecated, _("deprecated directive: %s, use %s"), quote(old), quote_n(1, upd));
...
#undef CASE
	...
	while(*format)
		if(format[0] == '%' && c_isdigit(format[1]) && format[2] == '$' && format[3] == 's' && (format[1] - '0') < argc) {
			int i = format[1] - '0';
