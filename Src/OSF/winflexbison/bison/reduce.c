...
static bitset V; /* Set of accessible symbols.  */
static bitset V1; /* Set of symbols used to define rule precedence (so they are 'useless', but no warning should be issued).  */
...
static bool useful_production(rule_number r, bitset N0)
{
...
static void useless_nonterminals(void)
{
	...
	while(1) {
		...
		for(rule_number r = 0; r < nrules; ++r)
			if(!bitset_test(P, r) && useful_production(r, N)) {
...
static void inaccessable_symbols(void)
{
	...
	if(bitset_test(N, acceptsymbol->content->number - ntokens)) {
		...
		while(1) {
			bitset_copy(Vp, V);
			for(rule_number r = 0; r < nrules; ++r)
				if(!bitset_test(Pp, r) && bitset_test(P, r) && bitset_test(V, rules[r].lhs->number)) {
					for(item_number * rhsp = rules[r].rhs; 0 <= *rhsp; ++rhsp)
	...
	bitset_free(V);
	V = Vp;
	/* These tokens (numbered 0, 1, and 2) are internal to Bison. Consider them useful. */
...
static void reduce_grammar_tables(void)
{
	...
	/* Map the nonterminals to their new index: useful first, useless afterwards.  Kept for later report.  */
	{
		int useful = 0;
		int useless = nrules - nuseless_productions;
		rule * rules_sorted = (rule *)xnmalloc(nrules, sizeof *rules_sorted);
		for(rule_number r = 0; r < nrules; ++r)
...
{
	nterm_map = (symbol_number *)xnmalloc(nnterms, sizeof *nterm_map);
	// Map the nonterminals to their new index: useful first, useless afterwards.  Kept for later report.
	{
		...
		for(symbol_number i = ntokens; i < nsyms; ++i)
			if(!bitset_test(V, i)) {
				...
				if(symbols[i]->content->status != used)
					complain(&symbols[i]->location, Wother, _("nonterminal useless in grammar: %s"), symbols[i]->tag);
			}
	}
	/* Shuffle elements of tables indexed by symbol number.  */
	{
		Symbol ** symbols_sorted = (Symbol **)xnmalloc(nnterms, sizeof *symbols_sorted);
		for(symbol_number i = ntokens; i < nsyms; ++i)
...
void reduce_output(FILE * out)
{
	...
	if(nuseless_productions)
		grammar_rules_partial_print(out, _("Rules useless in grammar"), rule_useless_in_grammar_p);
}
...
{
	if(nuseless_nonterminals)
		complain(NULL, Wother, ngettext("%d nonterminal useless in grammar", "%d nonterminals useless in grammar", nuseless_nonterminals), nuseless_nonterminals);
	if(nuseless_productions)
		complain(NULL, Wother, ngettext("%d rule useless in grammar", "%d rules useless in grammar", nuseless_productions), nuseless_productions);
}
