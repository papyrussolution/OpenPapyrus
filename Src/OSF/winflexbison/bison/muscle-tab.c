...
//
// A key-value pair, along with storage that can be reclaimed when this pair is no longer needed.
//
struct muscle_entry {
	char const * key;
	char const * value;
...
/* The name of muscle for the %define variable VAR (corresponding to
   FIELD, if defined).  */
static uniqstr muscle_name(char const * var, char const * field)
{
	if(field) {
		//return UNIQSTR_CONCAT("percent_define_", field, "(", var, ")");
		return uniqstr_concat(5, "percent_define_", field, "(", var, ")");
	}
	else {
		//return UNIQSTR_CONCAT("percent_define(", var, ")");
		return uniqstr_concat(3, "percent_define(", var, ")");
	}
}

struct obstack muscle_obstack; /* An obstack used to create some entries.  */
#define HT_INITIAL_CAPACITY 257 /* Initial capacity of muscles hash table.  */
static struct hash_table * muscle_table = NULL;

static bool hash_compare_muscles(void const * x, void const * y)
{
...
{
	muscle_entry const * m = (muscle_entry const *)x;
	return hash_string(m->key, tablesize);
}

/* Create a fresh muscle name KEY, and insert in the hash table.  */
static void * muscle_entry_new(char const * key)
{
	muscle_entry * res = (muscle_entry *)xmalloc(sizeof *res);
	res->key = key;
	res->value = NULL;
	res->storage = NULL;
...
{
	muscle_entry * mentry = (muscle_entry *)entry;
	SAlloc::F(mentry->storage);
	SAlloc::F(mentry);
}
...
void muscle_init(void)
{
	...
	muscle_table = hash_xinitialize(HT_INITIAL_CAPACITY, NULL, hash_muscle, hash_compare_muscles, muscle_entry_free);
...
static muscle_entry * FASTCALL muscle_lookup(char const * key)
{
	muscle_entry probe;
	probe.key = key;
	return (muscle_entry *)hash_lookup(muscle_table, &probe);
}
...
void muscle_insert(char const * key, char const * value)
{
	...
	if(entry)
		SAlloc::F(entry->storage);
	else
		entry = (muscle_entry *)muscle_entry_new(key); // First insertion in the hash. 
	entry->value = value;
	entry->storage = NULL;
}
...
static void muscle_grow(const char * key, const char * val, const char * separator, const char * terminator)
{
	muscle_entry * entry = muscle_lookup(key);
	if(entry) {
		...
		entry = (muscle_entry *)muscle_entry_new(key);
	obstack_sgrow(&muscle_obstack, val);
	size_t vals = strlen(val);
	size_t terms = strlen(terminator);
	if(terms <= vals && STRNEQ(val + vals - terms, terminator))
...
static void muscle_syncline_grow(char const * key, Location loc)
{
	obstack_printf(&muscle_obstack, "]b4_syncline(%d, ", loc.start.line);
	obstack_quote(&muscle_obstack, quotearg_style(c_quoting_style, loc.start.file));
...
void muscle_code_grow(const char * key, const char * val, Location loc)
{
	muscle_syncline_grow(key, loc);
	muscle_grow(key, val, "", "\n");
}

void muscle_pair_list_grow(const char * muscle, const char * a1, const char * a2)
{
...
static void muscle_boundary_grow(char const * key, boundary bound)
{
	obstack_sgrow(&muscle_obstack, "[[");
...
static char * string_decode(char const * key)
{
	...
	char const * value_decoded = (char const *)obstack_finish(&muscle_obstack);
...
static Location location_decode(char const * value)
{
	aver(value);
	aver(*value == '[');
	++value; aver(*value == '[');
...
void muscle_user_name_list_grow(char const * key, char const * user_name, Location loc)
{
	muscle_grow(key, "[[[[", ",", "");
...
static char * define_directive(char const * assignment, muscle_kind kind, char const * value)
{
	char * eq = (char *)strchr(assignment, '=');
	char const * fmt = eq || !value || !*value ? "%%define %s" : kind == muscle_code     ? "%%define %s {%s}" : kind == muscle_string   ? "%%define %s \"%s\"" : "%%define %s %s";
	char * res = (char*)xmalloc(strlen(fmt) + strlen(assignment) + (value ? strlen(value) : 0));
...
static char const * muscle_percent_variable_update(char const * variable, muscle_kind kind, char const ** value, char ** old, char ** upd)
{
	typedef struct {
		const char * obsolete;
		const char * updated;
	...
	const conversion_type conversion[] =
	{
		...
		{ "lex_symbol",                 "api.token.constructor",     (muscle_kind)-1 },
		...
		{ "stype",                      "api.value.type",            (muscle_kind)-1 },
		{ "variant=",                   "api.value.type=variant",    (muscle_kind)-1 },
...
void muscle_percent_define_insert(char const * var,
    ...
    muscle_percent_define_how how)
{
	...
	SAlloc::F(upd);
	if(variable != var)
		SAlloc::F((char*)variable);
}

/* This is used for backward compatibility, e.g., "%define api.pure"
   supersedes "%pure-parser".  */
void muscle_percent_define_ensure(char const * variable, Location loc, bool value)
{
	uniqstr name = muscle_name(variable, NULL);
...
static void muscle_percent_define_use(char const * variable)
{
...
static muscle_kind muscle_percent_define_get_kind(char const * variable)
{
	return muscle_kind_new(muscle_percent_define_get_raw(variable, "kind"));
}

/* Check the kind of VARIABLE.  An error if undefined.  */
static void muscle_percent_define_check_kind(char const * variable, muscle_kind kind)
{
...
bool muscle_percent_define_flag_if(char const * variable)
{
	...
	if(muscle_percent_define_ifdef(variable)) {
		...
		if(value[0] == '\0' || sstreq(value, "true"))
			...
			Location loc = muscle_percent_define_get_loc(variable);
			complain(&loc, complaint, _("invalid value for %%define Boolean variable %s"), quote(variable));
		}
		SAlloc::F(value);
	}
	else
		complain(NULL, fatal, _("%s: undefined %%define variable %s"), "muscle_percent_define_flag", quote(variable));
	return res;
}
	...
	for(; *values; ++values) {
		...
		if(value) {
			...
			if(!*values) {
				...
				complain(&loc, complaint, _("invalid value for %%define variable %s: %s"), quote(*variablep),
				    quote_n(1, value));
				for(values = variablep + 1; *values; ++values)
					subcomplain(&loc, (warnings)(complaint | no_caret | silent), _("accepted value: %s"), quote(*values));
			}
			else
				while(*values)
					++values;
			...
			complain(NULL, fatal, _("%s: undefined %%define variable %s"), "muscle_percent_define_check_values", quote(*variablep));
	}
}

void muscle_percent_code_grow(char const * qualifier, Location qualifier_loc, char const * code, Location code_loc)
{
	//char const * name = UNIQSTR_CONCAT("percent_code(", qualifier, ")");
	char const * name = uniqstr_concat(3, "percent_code(", qualifier, ")");
...
static inline bool muscle_m4_output(muscle_entry * entry, FILE * out)
{
	fprintf(out, "m4_define([b4_%s],\n[[%s]])\n\n\n", entry->key, entry->value);
	return true;
}

static bool muscle_m4_output_processor(void * entry, void * out)
{
	return muscle_m4_output((muscle_entry *)entry, (FILE *)out);
}
