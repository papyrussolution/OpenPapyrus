...
static inline void log_resolution(rule * r, symbol_number token, enum conflict_resolution resolution)
{
	if(report_flag & report_solved_conflicts) {
		...
		switch(resolution) {
			...
			case right_resolution:
			    ...
			    obstack_printf(&solved_conflicts_obstack, _("Conflict between rule %d and token %s resolved as shift"), r->number, symbols[token]->tag);
			    break;
			case reduce_resolution:
			case left_resolution:
			    obstack_sgrow(&solved_conflicts_obstack, "    ");
			    obstack_printf(&solved_conflicts_obstack, _("Conflict between rule %d and token %s resolved as reduce"), r->number, symbols[token]->tag);
			    break;
			case nonassoc_resolution:
			    obstack_sgrow(&solved_conflicts_obstack, "    ");
			    obstack_printf(&solved_conflicts_obstack, _("Conflict between rule %d and token %s resolved as an error"), r->number, symbols[token]->tag);
			    break;
		}
		...
		switch(resolution) {
			case shift_resolution:
			    obstack_printf(&solved_conflicts_obstack, " (%s < %s)", r->prec->symbol->tag, symbols[token]->tag);
			    break;
			case reduce_resolution:
			    obstack_printf(&solved_conflicts_obstack, " (%s < %s)", symbols[token]->tag, r->prec->symbol->tag);
			    break;
			case left_resolution:
			    obstack_printf(&solved_conflicts_obstack, " (%%left %s)", symbols[token]->tag);
			    break;
			case right_resolution:
			    obstack_printf(&solved_conflicts_obstack, " (%%right %s)", symbols[token]->tag);
			    break;
			case nonassoc_resolution:
			    obstack_printf(&solved_conflicts_obstack, " (%%nonassoc %s)", symbols[token]->tag);
			    break;
		}
		obstack_sgrow(&solved_conflicts_obstack, ".\n");
	}
	...
	if(xml_flag) {
		...
		switch(resolution)
		{
			...
			case left_resolution:
			    ...
			    break;
			case nonassoc_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "        <resolution rule=\"%d\" symbol=\"%s\" type=\"error\">", r->number, xml_escape(symbols[token]->tag));
			    break;
		}
		/* The reason. */
		switch(resolution) {
			case shift_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "%s &lt; %s", xml_escape_n(0, r->prec->symbol->tag), xml_escape_n(1, symbols[token]->tag));
			    break;
			case reduce_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "%s &lt; %s", xml_escape_n(0, symbols[token]->tag), xml_escape_n(1, r->prec->symbol->tag));
			    break;
			case left_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "%%left %s", xml_escape(symbols[token]->tag)); 
				break;
			case right_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "%%right %s", xml_escape(symbols[token]->tag));
			    break;
			case nonassoc_resolution:
			    obstack_printf(&solved_conflicts_xml_obstack, "%%nonassoc %s", xml_escape(symbols[token]->tag));
			    break;
		}
...
static void flush_shift(state * s, int token)
{
	...
	for(int i = 0; i < trans->num; ++i)
		if(!TRANSITION_IS_DISABLED(trans, i) && TRANSITION_SYMBOL(trans, i) == token)
...
static void flush_reduce(bitset lookaheads, int token)
{
...
static void resolve_sr_conflict(state * s, int ruleno, Symbol ** errors, int * nerrs)
{
	...
	for(symbol_number i = 0; i < ntokens; ++i)
		if(bitset_test(lookaheads, i) && bitset_test(lookahead_set, i) && symbols[i]->content->prec) {
...
static void set_conflicts(state * s, Symbol ** errors)
{
	...
	for(int i = 0; i < reds->num; ++i)
		if(reds->rules[i]->prec && reds->rules[i]->prec->prec && !bitset_disjoint_p(reds->lookaheads[i], lookahead_set))
...
void conflicts_solve(void)
{
	...
	conflicts = (bool *)xcalloc(nstates, sizeof(*conflicts));
	...
	for(state_number i = 0; i < nstates; ++i) {
		...
		/* For uniformity of the code, make sure all the states have a valid 'errs' member.  */
		if(!states[i]->errs)
...
static size_t count_state_sr_conflicts(const state * s)
{
	transitions * trans = s->transitions;
	reductions * reds = s->reductions;
	if(!trans)
...
static size_t count_state_rr_conflicts(const state * s)
{
	reductions * reds = s->reductions;
	size_t res = 0;
	for(symbol_number i = 0; i < ntokens; ++i) {
...
static size_t count_rule_state_sr_conflicts(rule * r, state * s)
{
	size_t res = 0;
	transitions * trans = s->transitions;
	reductions * reds = s->reductions;
...
static size_t count_rule_sr_conflicts(rule * r)
{
	size_t res = 0;
	for(state_number i = 0; i < nstates; ++i)
		if(conflicts[i])
...
static size_t count_rule_state_rr_conflicts(rule * r, state * s)
{
	size_t res = 0;
	...
	for(int i = 0; i < reds->num; ++i)
		if(reds->rules[i] == r)
			for(int j = 0; j < reds->num; ++j)
				if(reds->rules[j] != r) {
					bitset_and(lookaheads, reds->lookaheads[i], reds->lookaheads[j]);
...
static size_t count_rule_rr_conflicts(rule * r)
{
	size_t res = 0;
	for(state_number i = 0; i < nstates; ++i)
	...
	for(rule_number i = 0; i < nrules; i += 1) {
		...
		if(expected_sr != -1 || expected_rr != -1) {
			...
			if(sr != expected_sr && (sr != 0 || expected_sr != -1))
				complain(&r->location, complaint, _("shift/reduce conflicts for rule %d: %d found, %d expected"), r->code, sr, expected_sr);
			int rr = count_rule_rr_conflicts(r);
			if(rr != expected_rr && (rr != 0 || expected_rr != -1))
				complain(&r->location, complaint, _("reduce/reduce conflicts for rule %d: %d found, %d expected"), r->code, rr, expected_rr);
		}
	}
}
...
void conflicts_print(void)
{
	   ...
	   would make it ugly.  */
	{
		...
		int expected = (expected_sr_conflicts == -1 && expected_rr_conflicts != -1) ? 0 : expected_sr_conflicts;
		if(expected != -1) {
			if(expected != total) {
				complain(NULL, complaint, _("shift/reduce conflicts: %d found, %d expected"), total, expected);
				if(total)
					unexpected_conflicts_warning = complaint;
			}
		}
		else if(total) {
			complain(NULL, Wconflicts_sr, ngettext("%d shift/reduce conflict", "%d shift/reduce conflicts", total), total);
...

	{
		...
		int expected = (expected_rr_conflicts == -1 && expected_sr_conflicts != -1) ? 0 : expected_rr_conflicts;
		if(expected != -1) {
			if(expected != total) {
				complain(NULL, complaint, _("reduce/reduce conflicts: %d found, %d expected"), total, expected);
				if(total)
					unexpected_conflicts_warning = complaint;
			}
		}
		else if(total) {
			complain(NULL, Wconflicts_rr, ngettext("%d reduce/reduce conflict", "%d reduce/reduce conflicts", total), total);
