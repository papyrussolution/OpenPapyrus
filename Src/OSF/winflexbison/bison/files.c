/* Open and close files for Bison.
   Copyright (C) 1984, 1986, 1989, 1992, 2000-2015, 2018-2020 Free Software Foundation, Inc.
...
FILE * xfopen(const char * name, const char * mode)
{
	...
	if(!res)
		error(EXIT_FAILURE, get_errno(), _("%s: cannot open"), quotearg_colon(name));
	return res;
}
...
char * map_file_name(char const * filename)
{
	...
	char * end = _stpcpy(s, p->newprefix);
	_stpcpy(end, filename + oldprefix_len);
	return s;
}
...
static void compute_exts_from_src(const char * ext)
{
...
char * get_local_pkgdatadir()
{
	...
	if(!last_divider)
		return const_cast<char *>(PKGDATADIR); // @v11.2.11 @badcast
	++last_divider;
	dir_len = last_divider - program_path;
	local_pkgdatadir_len = dir_len + strlen(PKGDATADIR);
	local_pkgdatadir = (char*)SAlloc::M((local_pkgdatadir_len + 1) * sizeof(char));
...
void output_file_names_free()
{
	...
	SAlloc::F(spec_mapped_header_file);
	SAlloc::F(parser_file_name);
	SAlloc::F(dir_prefix);
	SAlloc::F(mapped_dir_prefix);
	for(int i = 0; i < generated_files_size; i++)
