...
static bitset ielr_compute_ritem_sees_lookahead_set(void)
{
	...
	while(0 < i) {
		--i;
		while(!item_number_is_rule_number(ritem[i]) && ISVAR(ritem[i]) && nullable [item_number_as_symbol_number(ritem[i]) - ntokens])
			bitset_set(result, i--);
 ...
 *       same.
 */
static void ielr_compute_internal_follow_edges(bitset ritem_sees_lookahead_set, goto_number *** edgesp, int ** edge_countsp)
{
	*edgesp = (goto_number **)xnmalloc(ngotos, sizeof **edgesp);
	*edge_countsp = (int *)xnmalloc(ngotos, sizeof **edge_countsp);
	{
		...
		for(goto_number i = 0; i < ngotos; ++i) {
			int nsources = 0;
			{
				for(rule ** rulep = derives[states[to_state[i]]->accessing_symbol - ntokens]; *rulep; ++rulep) {
					...
					if(bitset_test(ritem_sees_lookahead_set, (*rulep)->rhs - ritem)) {
						goto_number source = map_goto(from_state[i], item_number_as_symbol_number(*(*rulep)->rhs));
			...
			if(nsources == 0)
				...
				(*edgesp)[i] = (goto_number *)xnmalloc(nsources + 1, sizeof *(*edgesp)[i]);
				{
					bitset_iterator biter_source;
					bitset_bindex source;
					int j = 0;
					BITSET_FOR_EACH(biter_source, sources, source, 0)(*edgesp)[i][j++] = source;
				}
 ...
 */
static void ielr_compute_follow_kernel_items(bitset ritem_sees_lookahead_set, goto_number ** internal_follow_edges, bitsetv * follow_kernel_itemsp)
{
	{
		size_t max_nitems = 0;
 ...
 */
static void ielr_compute_always_follows(goto_number *** edgesp, int const edge_counts[], bitsetv * always_followsp)
{
	*always_followsp = bitsetv_create(ngotos, ntokens, BITSET_FIXED);
	{
		goto_number * edge_array = (goto_number *)xnmalloc(ngotos, sizeof *edge_array);
		for(goto_number i = 0; i < ngotos; ++i) {
...
static state *** ielr_compute_predecessors(void)
{
	...
	for(state_number i = 0; i < nstates; ++i) {
		res[i] = (state **)xnmalloc(predecessor_counts[i]+1, sizeof *res[i]);
 ...
 */
static void ielr_compute_auxiliary_tables(bitsetv * follow_kernel_itemsp, bitsetv * always_followsp, state **** predecessorsp)
{
	goto_number ** edges;
	int * edge_counts;
	{
...
bool ielr_item_has_lookahead(state * s, symbol_number lhs, size_t item, symbol_number lookahead, state *** predecessors, bitset ** item_lookahead_sets)
{
	if(!item_lookahead_sets[s->number]) {
		item_lookahead_sets[s->number] = (bitset *)xnmalloc(s->nitems, sizeof item_lookahead_sets[s->number][0]);
		for(size_t i = 0; i < s->nitems; ++i)
			item_lookahead_sets[s->number][i] = NULL;
	}
	if(!item_lookahead_sets[s->number][item]) {
		item_lookahead_sets[s->number][item] = bitset_create(ntokens, BITSET_FIXED);
			...
			/* If this kernel item is later in the RHS, then check all
			   predecessor items' lookahead sets.  */
				...
				for(state ** predecessor = predecessors[s->number]; *predecessor; ++predecessor) {
					...
					for(predecessor_item = 0; predecessor_item < (*predecessor)->nitems; ++predecessor_item)
						if((*predecessor)->items[predecessor_item] == s->items[item] - 1)
							break;
					aver(predecessor_item != (*predecessor)->nitems);
					ielr_item_has_lookahead(*predecessor, lhs, predecessor_item, 0 /*irrelevant*/, predecessors, item_lookahead_sets);
					bitset_or(item_lookahead_sets[s->number][item], item_lookahead_sets[s->number][item], item_lookahead_sets[(*predecessor)->number][predecessor_item]);
				}
			}
		}
	}
 ...
 *     state.
 */
static void ielr_compute_annotation_lists(bitsetv follow_kernel_items, bitsetv always_follows, state *** predecessors,
    AnnotationIndex * max_annotationsp, InadequacyList *** inadequacy_listsp, AnnotationList *** annotation_listsp, struct obstack * annotations_obstackp)
{
	bitset ** item_lookahead_sets = (bitset **)xnmalloc(nstates, sizeof *item_lookahead_sets);
	AnnotationIndex * annotation_counts = (AnnotationIndex *)xnmalloc(nstates, sizeof *annotation_counts);
	ContributionIndex max_contributions = 0;
	int total_annotations = 0;
	*inadequacy_listsp = (InadequacyList **)xnmalloc(nstates, sizeof **inadequacy_listsp);
	*annotation_listsp = (AnnotationList **)xnmalloc(nstates, sizeof **annotation_listsp);
	for(state_number i = 0; i < nstates; ++i) {
	...
	*max_annotationsp = 0;
	...
	for(state_number i = 0; i < nstates; ++i)
		if(item_lookahead_sets[i]) {
			...
			SAlloc::F(item_lookahead_sets[i]);
		}
	SAlloc::F(item_lookahead_sets);
	SAlloc::F(annotation_counts);
}
    ...
    ContributionIndex work1[], bitsetv work2, state ** tp)
{
	...
	/* Create a new isocore.  */
		...
		(*last_statep)->next = *this_isocorep = (state_list *)xmalloc(sizeof **last_statep);
		*last_statep = *this_isocorep;
		...
		if(has_lookaheads) {
			(*last_statep)->lookaheads = (bitset *)xnmalloc(t->nitems, sizeof(*last_statep)->lookaheads);
			for(size_t i = 0; i < t->nitems; ++i) {
				if(bitset_empty_p(lookaheads[i]))
    ...
    AnnotationIndex max_annotations)
{
	...
	/* Set up state list and some reusable bitsets.  */
	{
		...
		for(state_number i = 0; i < nstates; ++i) {
			*nodep = states[i]->state_list = last_state = (state_list *)xmalloc(sizeof **nodep);
	...
	/* Recompute states.  */
	{
		ContributionIndex * work = (ContributionIndex *)xnmalloc(max_annotations, sizeof *work);
		for(state_list * this_state = first_state; this_state; this_state = this_state->next) {
			state * s = this_state->state;
			for(int i = 0; i < s->transitions->num; ++i) {
				state * t = s->transitions->states[i];
				if(annotation_lists)
					AnnotationList__computeLookaheadFilter(annotation_lists[t->state_list->lr0Isocore->state->number], t->nitems, lookahead_filter);
				ielr_compute_lookaheads(follow_kernel_items, always_follows, this_state, t, lookahead_filter, lookaheads);
				ielr_compute_state(follow_kernel_items, always_follows, annotation_lists, t, lookaheads, &last_state, work, lookahead_filter, &s->transitions->states[i]);
			}
		}
		SAlloc::F(work);
	}
	...
	if(!annotation_lists) {
		...
		for(state_list * node = first_state; node; node = node->next)
			if(!node->state->consistent) {
				...
				for(size_t r = 0; (int)r < node->state->reductions->num; ++r) {
					rule * this_rule = node->state->reductions->rules[r];
					bitset lookahead_set = node->state->reductions->lookaheads[r];
					if(item_number_is_rule_number(*this_rule->rhs))
						ielr_compute_goto_follow_set(follow_kernel_items, always_follows, node, this_rule->lhs, lookahead_set);
						...
						/* We don't need to start the kernel item search back at
						   ...
						   number.  */
						while(!item_number_is_rule_number(ritem[itemset[i]]) || item_number_as_rule_number(ritem[itemset[i]]) != this_rule->number) {
							++i;
...
void ielr(void)
{
...

	{
		...
		SAlloc::F(inadequacy_lists);
		if(annotation_lists)
			obstack_free(&annotations_obstack, NULL);
		SAlloc::F(annotation_lists);
