/* Type definitions for the finite state machine for Bison.
   ...
   Copyright (C) 1984, 1989, 2000-2004, 2007, 2009-2015, 2018-2020 Free Software Foundation, Inc.
...
#define TRANSITION_SYMBOL(Transitions, Num) (Transitions->states[Num]->accessing_symbol)

/* Is the TRANSITIONS->states[Num] a shift? (as opposed to gotos).  */

#define TRANSITION_IS_SHIFT(Transitions, Num) (ISTOKEN(TRANSITION_SYMBOL(Transitions, Num)))

/* Is the TRANSITIONS->states[Num] a goto?. */
#define TRANSITION_IS_GOTO(Transitions, Num) (!TRANSITION_IS_SHIFT(Transitions, Num))

/* Is the TRANSITIONS->states[Num] labelled by the error token?  */

#define TRANSITION_IS_ERROR(Transitions, Num) (TRANSITION_SYMBOL(Transitions, Num) == errtoken->content->number)
/* When resolving a SR conflicts, if the reduction wins, the shift is disabled.  */
#define TRANSITION_DISABLE(Transitions, Num) (Transitions->states[Num] = NULL)
#define TRANSITION_IS_DISABLED(Transitions, Num) (Transitions->states[Num] == NULL)

/* Iterate over each transition over a token (shifts).  */
#define FOR_EACH_SHIFT(Transitions, Iter)                      \
	for(Iter = 0; Iter < Transitions->num && (TRANSITION_IS_DISABLED(Transitions, Iter) || TRANSITION_IS_SHIFT(Transitions, Iter)); ++Iter) \
...
typedef struct {
	int num;
	bitset * lookaheads;
	rule * rules[1]; // Sorted ascendingly on rule number.
} reductions;
...
state * state_new(symbol_number accessing_symbol, size_t core_size, item_index * core);
state * state_new_isocore(state const * s);
void state_transitions_set(state * s, int num, state ** dst); /* Record that from S we can reach all the DST states (NUM of them).  */
void state_transitions_print(const state * s, FILE * out); /* Print the transitions of state s for debug.  */
void state_reductions_set(state * s, int num, rule ** reds); /* Set the reductions of STATE.  */
...
void state_errs_set(state * s, int num, Symbol ** errors); /* Set the errs of STATE.  */

/* Print on OUT all the lookahead tokens such that this STATE wants to reduce R.  */
