...
#ifdef TIOCGWINSZ
		struct winsize ws;
		if(ioctl(STDERR_FILENO, TIOCGWINSZ, &ws) != -1 && 0 < ws.ws_col && ws.ws_col == (size_t)ws.ws_col)
			res = ws.ws_col;
#endif
	}
...
static void boundary_compute(boundary * cur, char const * token, size_t size)
{
	int line = cur->line;
...
/* Open FILE for quoting, if needed, and if possible.  Return whether
   the file can quoted. */
static bool caret_set_file(const char * file)
{
...
/* Getc, but smash \r\n as \n.  */
static void caret_getc_internal(mbchar_t * res)
{
...
static bool caret_set_line(int line)
{
...
static bool caret_set_column(int col)
{
