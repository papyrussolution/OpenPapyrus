...
static int get_merge_function(uniqstr name)
{
	...
	for(syms = &head, n = 1; syms->next; syms = syms->next, n += 1)
		if(UNIQSTR_EQ(name, syms->next->name))
			break;
	if(syms->next == NULL) {
		syms->next = (merger_list *)xmalloc(sizeof syms->next[0]);
...
static void assign_named_ref(symbol_list * p, named_ref * name)
{
	...
	if(name->id == sym->tag) {
		complain(&name->loc, Wother, _("duplicated symbol name for %s ignored"), quote(sym->tag));
		named_ref_free(name);
	}
	else
		p->named_ref = name;
...
void grammar_current_rule_begin(Symbol * lhs, Location loc, named_ref * lhs_name)
{
	...
	if(lhs->content->Cls == unknown_sym || lhs->content->Cls == pct_type_sym)
		symbol_class_set(lhs, nterm_sym, empty_loc, false);
	else if(lhs->content->Cls == token_sym)
		complain(&loc, complaint, _("rule given for %s, which is a token"), lhs->tag);
}
...
static bool symbol_should_be_used(symbol_list const * s, bool * midrule_warning)
{
	if(symbol_code_props_get(s->content.sym, destructor)->code)
		return true;
	if((s->midrule && s->midrule->action_props.is_value_used) || (s->midrule_parent_rule && (symbol_list_n_get(s->midrule_parent_rule, s->midrule_parent_rhs_index)->action_props.is_value_used))) {
...
static void grammar_rule_check_and_complete(symbol_list * r)
{
	...
	if(!r->action_props.code && r->content.sym->content->type_name) {
		...
		if(first_rhs) {
			...
			char const * rhs_type = first_rhs->content->type_name ? first_rhs->content->type_name : "";
...
void grammar_midrule_action(void)
{
	...
	grammar_current_rule_symbol_append(dummy, dummy_loc, action_name);
...
void grammar_current_rule_symbol_append(Symbol * sym, Location loc, named_ref * name)
{
...
void grammar_current_rule_action_append(const char * action, Location loc, named_ref * name, uniqstr type)
{
	if(current_rule->action_props.code)
		grammar_midrule_action();
	if(type)
	...
	*ritem++ = 0;
	...
	for(symbol_list * p = grammar; p; p = p->next) {
		...
		if(lhs->midrule_parent_rule /* i.e., symbol_is_dummy (lhs->content.sym).  */
		    && (lhs->action_props.is_value_used || (symbol_list_n_get(lhs->midrule_parent_rule, lhs->midrule_parent_rhs_index)->action_props.is_value_used)))
		...
		/* Traverse the rhs.  */
		{
			...
			for(p = lhs->next; p->content.sym; p = p->next) {
				...
				/* A rule gets by default the precedence and associativity
				   of its last token.  */
				if(p->content.sym->content->Cls == token_sym && default_prec)
...
static void check_and_convert_grammar(void)
{
	...
	if(!eoftoken) {
		...
		eoftoken->content->Cls = token_sym;
