...
static void max_length(size_t * width, const char * str)
{
...
static void print_core(FILE * out, const state * s)
{
...
| OUT.                                                        |
   `------------------------------------------------------------*/
static void print_transitions(const state * s, FILE * out, bool display_transitions_p)
{
	transitions * trans = s->transitions;
	size_t width = 0;
	/* Compute the width of the lookahead token column.  */
	for(int i = 0; i < trans->num; i++)
		if(!TRANSITION_IS_DISABLED(trans, i) && TRANSITION_IS_SHIFT(trans, i) == display_transitions_p) {
...
static void print_errs(FILE * out, const state * s)
{
	errs * errp = s->errs;
	size_t width = 0;
	/* Compute the width of the lookahead token column.  */
...
| R/R conflicts).                                                    |
   `-------------------------------------------------------------------*/
static void print_reduction(FILE * out, size_t width, const char * lookahead, rule * r, bool enabled)
{
	...
	if(!enabled)
		fputc('[', out);
	if(r->number)
		fprintf(out, _("reduce using rule %d (%s)"), r->number, r->lhs->symbol->tag);
	else
...
static void print_reductions(FILE * out, const state * s)
{
			...
			for(int j = 0; j < reds->num; ++j)
				if(bitset_test(reds->lookaheads[j], i)) {
					if(!count) {
						if(reds->rules[j] != default_reduction) {
							default_reduction_only = false;
							print_reduction(out, width, symbols[i]->tag, reds->rules[j], true);
						}
						else
							defaulted = true;
						count = true;
						...
						default_reduction_only = false;
						if(defaulted)
							print_reduction(out, width, symbols[i]->tag, default_reduction, true);
						defaulted = false;
						print_reduction(out, width, symbols[i]->tag, reds->rules[j], false);
					}
				}
		}
...
static void print_actions(FILE * out, const state * s)
{
	/* Print shifts.  */
...
static void print_state(FILE * out, const state * s)
{
	fputs("\n\n", out);
	...
	if(has_conflicts(s) && (report_flag & report_cex || warning_is_enabled(Wcounterexamples))) {
		fputc('\n', out);
...
static void print_terminal_symbols(FILE * out)
{
...
static void print_nonterminal_symbols(FILE * out)
{
...
{
	// We used to use just .out if SPEC_NAME_PREFIX (-p) was used, but that conflicts with Posix.
	FILE * out = xfopen(spec_verbose_file, "w");
	reduce_output(out);
	grammar_rules_partial_print(out, _("Rules useless in parser due to conflicts"), rule_useless_in_parser_p);
