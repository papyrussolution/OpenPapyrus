 ...
 */
static AnnotationList* AnnotationList__alloc_on_obstack(ContributionIndex contribution_count, struct obstack * annotations_obstackp)
{
	AnnotationList * res;
	size_t contributions_size = contribution_count * sizeof res->contributions[0];
	res = (AnnotationList *)obstack_alloc(annotations_obstackp, offsetof(AnnotationList, contributions) + contributions_size);
	res->next = NULL;
 ...
 *     "always" contribution.
 */
static bool AnnotationList__isContributionAlways(AnnotationList const * self, ContributionIndex ci)
{
 ...
 */
static bool AnnotationList__insertInto(AnnotationList * self, AnnotationList ** list,
    size_t nitems)
{
	AnnotationList ** node;
...
static bitset AnnotationList__compute_conflicted_tokens(bitset shift_tokens,
    reductions * reds)
{
...
static bool AnnotationList__compute_lhs_contributions(state * s, const rule * the_rule,
...
after_sbitset__for_each:        ;
			...
			if(annotation_node->contributions[ci]) {
				    ...
				    (*predecessor)->nitems, biter, i)
				{
					...
					if(!lookaheads) {
						lookaheads = (bitset *)xnmalloc((*predecessor)->nitems, sizeof *lookaheads);
		...
		if(potential_contribution) {
			if(ContributionIndex__none != AnnotationList__computeDominantContribution(annotation_node, (*predecessor)->nitems, lookaheads, true)) {
			...
			if(annotation_node) {
				if(AnnotationList__insertInto(annotation_node, &annotation_lists[(*predecessor)->number], (*predecessor)->nitems)) {
					++annotation_counts[(*predecessor)->number];
					AnnotationList__computePredecessorAnnotations(annotation_node, *predecessor,
						follow_kernel_items, always_follows, predecessors, item_lookahead_sets, annotation_lists, annotation_counts,
						annotations_obstackp);
				}
				else
					obstack_free(annotations_obstackp, annotation_node);
...
void AnnotationList__compute_from_inadequacies(state * s, bitsetv follow_kernel_items, bitsetv always_follows,
    ...
    InadequacyListNodeCount * inadequacy_list_node_count)
{
	...
	bitset conflicted_tokens = AnnotationList__compute_conflicted_tokens(shift_tokens, s->reductions);
	...
	BITSET_FOR_EACH(biter_conflict, conflicted_tokens, conflicted_token, 0)
	{
		...
		/* Allocate the annotation node.  */
		{
			for(int rule_i = 0; rule_i < s->reductions->num; ++rule_i)
				if(bitset_test(s->reductions->lookaheads[rule_i], conflicted_token))
					++contribution_count;
		...
		bool potential_contribution = false;
		// Add a contribution for each reduction that has conflicted_token as a lookahead. 
		{
			...
			for(int rule_i = 0; rule_i < s->reductions->num; ++rule_i) {
				...
				if(bitset_test(s->reductions->lookaheads[rule_i],
					...
					if(!item_number_is_rule_number(the_rule->rhs[0])) {
						annotation_node->contributions[ci] = Sbitset__new_on_obstack(s->nitems, annotations_obstackp);
						/* Catch item_i up to rule_i.  This works because both are sorted on rule number.  */
					...
					/* Otherwise, add the kernel items whose lookahead sets
					   ...
					   lookahead set.  */
					else if(AnnotationList__compute_lhs_contributions(s, the_rule, conflicted_token, follow_kernel_items,
						    always_follows, predecessors, item_lookahead_sets, &annotation_node->contributions[ci], annotations_obstackp)) {
						annotation_node->contributions[ci++] = NULL;
						continue;
					}
					/* The lookahead token has to come from somewhere.  */
					aver(!Sbitset__isEmpty(annotation_node->contributions[ci], s->nitems));
					++ci;
					potential_contribution = true;
				}
			}
		...
		if(potential_contribution) {
			...
			{
				InadequacyList * conflict_node = InadequacyList__new_conflict(s, symbols[conflicted_token], actions, inadequacy_list_node_count);
				actions = NULL;
				annotation_node->inadequacyNode = conflict_node;
				if(ContributionIndex__none != AnnotationList__computeDominantContribution(annotation_node, s->nitems, all_lookaheads, true)) {
					obstack_free(annotations_obstackp, annotation_node);
					...
					{
						bool b = AnnotationList__insertInto(annotation_node, &annotation_lists[s->number], s->nitems);
						aver(b); (void)b;
					}
					...
					aver(!bitset_test(shift_tokens, conflicted_token) || symbols[conflicted_token]->content->prec);
					++annotation_counts[s->number];
					if(contribution_count > *max_contributionsp)
						*max_contributionsp = contribution_count;
					AnnotationList__computePredecessorAnnotations(annotation_node, s,
						follow_kernel_items, always_follows, predecessors,
						item_lookahead_sets, annotation_lists, annotation_counts, annotations_obstackp);
				}
			}
		}
		else {
 ...
 */
static bool AnnotationList__stateMakesContribution(AnnotationList const * self,
    ...
    bitset * lookaheads)
{
	...
	if(!lookaheads)
		return false;
	{
		symbol_number token = InadequacyList__getContributionToken(self->inadequacyNode, ci)->content->number;
		Sbitset__Index item;
		Sbitset biter;
		SBITSET__FOR_EACH(self->contributions[ci], nitems, biter, item)
...
{
	ContributionIndex const ci_shift = InadequacyList__getShiftContributionIndex(self->inadequacyNode);
	...
	if(ci_shift != ContributionIndex__none) {
		...
		if(find_stable_domination_over_shift || find_stable_error_action_domination)
			return ContributionIndex__none;
	...
	for(ContributionIndex ci = 0; ci < self->inadequacyNode->contributionCount; ++ci)
		if(AnnotationList__stateMakesContribution(self, nitems, ci, lookaheads)) {
			if(require_split_stable && !AnnotationList__isContributionAlways(self, ci))
				return ContributionIndex__none;
			return ci;
		}
	return ContributionIndex__none;
