...
// Copyright (C) 2006-2007, 2009-2015, 2018-2020 Free Software Foundation, Inc.
...
//
static char * local_quote(char const * name)
{
...
static void no_reduce_bitset_init(state const * s, bitset * no_reduce_set)
{
...
static void conclude_red(struct obstack * out, int source, rule_number ruleno,
    bool enabled, bool first, FILE * fout)
{
	...
	if(first)
		...
		fprintf(fout, " \"%dR%d%s\" [label=\"", source, ruleno, ed);
		if(ruleno)
...
static bool print_token(struct obstack * out, bool first, char const * tok)
{
	if(!first)
		obstack_sgrow(out, ", ");
...
void output_red(state const * s, reductions const * reds, FILE * fout)
{
	...
	rule * default_reduction = yydefact[s->number] ? &rules[yydefact[s->number] - 1] : NULL;
	...
	for(int j = 0; j < reds->num; ++j) {
		...
		/* Build the lookahead tokens lists, one for enabled transitions and one for disabled transitions. */
		bool firstd = true;
		bool firste = true;
