...
struct symbol_list {
	...
	named_ref * named_ref; /* Named reference. */
	...
	| when several RHSs are bound to a single lhs via "|").         |
	   `--------------------------------------------------------------*/
	Location rhs_loc; /* Location of the RHS. */
	Symbol * ruleprec; /* Precedence/associativity.  */
	/* The action is attached to the LHS of a rule, but action properties for
	 * each RHS are also stored here.  */
	code_props action_props;
	Location percent_empty_loc; /* The location of the first %empty for this rule, or \a empty_loc.  */
	int dprec;
	Location dprec_loc;
	int merger;
...
symbol_list * symbol_list_type_set(symbol_list * syms, uniqstr type_name); /** Assign the type \c type_name to all the members of \c syms. \returns \c syms */
...
symbol_list * symbol_list_prepend(symbol_list * list, symbol_list * node); /** Prepend \c node to \c list.  */
symbol_list * symbol_list_append(symbol_list * list, symbol_list * node); /** Append \c node to \c list.  */
void symbol_list_free(symbol_list * list); /** Free \c list, but not the items it contains.  */
int symbol_list_length(symbol_list const * l); /** Return the length of \c l. */
...
void symbol_list_code_props_set(symbol_list * node, code_props_type kind, code_props const * cprops);
