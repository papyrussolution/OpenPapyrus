...
/* Find the value of muscle KEY.  Unlike MUSCLE_FIND, this is always reliable to determine whether KEY has a value.  */
...
#define MUSCLE_INSERT_INT(Key, Value) MUSCLE_INSERTF(Key, "%d", Value)
#define MUSCLE_INSERT_LONG_INT(Key, Value) MUSCLE_INSERTF(Key, "%ld", Value)

/* Key -> Value, but don't apply escaping to Value. */
#define MUSCLE_INSERT_STRING_RAW(Key, Value) MUSCLE_INSERTF(Key, "%s", Value)
...
#define MUSCLE_INSERT_C_STRING(Key, Value) MUSCLE_INSERT_STRING(Key, quotearg_style(c_quoting_style, Value))
...
void muscle_percent_define_insert(char const * variable, Location variable_loc, muscle_kind kind, char const * value, muscle_percent_define_how how);
...
void muscle_percent_define_ensure(char const * variable, Location variable_loc, bool value);
...
/* Output the definition of all the current muscles into a list of m4_defines.  */
