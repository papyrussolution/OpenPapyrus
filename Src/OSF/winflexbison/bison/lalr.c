...
static goto_list * goto_list_new(goto_number value, struct goto_list * next)
{
	goto_list * res = (goto_list *)xmalloc(sizeof *res);
	res->next = next;
	res->value = value;
	return res;
}
...
/* "(p, A) includes (p', B)" iff
   ...
   INCLUDES[(p, A)] = [(p', B),...] */
static goto_number ** PP_Includes;

/* "(q, A → ω) lookback (p, A)" iff state p reaches state q on label ω.

   Definition p.621 [DeRemer 1982]. */
static goto_list ** PP_Lookback;

static void goto_print(goto_number i, FILE * out)
{
...
void set_goto_map(void)
{
	...
	goto_number * temp_map = (goto_number *)xnmalloc(nnterms + 1, sizeof *temp_map);
	{
		goto_number k = 0;
	...
	from_state = (state_number *)xcalloc(ngotos, sizeof *from_state);
	to_state = (state_number *)xcalloc(ngotos, sizeof *to_state);
	for(state_number s = 0; s < nstates; ++s) {
...
static void follows_print(const char* title, FILE * out)
{
	fprintf(out, "%s:\n", title);
	for(goto_number i = 0; i < ngotos; ++i) {
		fputs("    FOLLOWS[", out);
...
static void initialize_goto_follows(void)
{
	...
	for(goto_number i = 0; i < ngotos; ++i) {
		...
		if(nedges == 0)
			...
			reads[i] = (goto_number *)xnmalloc(nedges + 1, sizeof reads[i][0]);
...
static const state * lookback_find_state(int lookback_index)
{
	state * res = NULL;
	for(int j = 0; j < nstates; ++j)
		if(states[j]->reductions && states[j]->reductions->lookaheads) {
...
static void lookback_print(FILE * out)
{
	fputs("lookback:\n", out);
	for(int i = 0; i < (int)(nLA); ++i) {
		if(PP_Lookback[i]) {
			fprintf(out, "   %3d = ", i);
			...
			fputs(") ->", out);
			for(goto_list * sp = PP_Lookback[i]; sp; sp = sp->next) {
				fputc(' ', out);
...
/* Add (S, R) -> GOTONO to LOOKBACK.
   ...
   The goto number GOTONO, whose source is S (which is inconsistent), */
static void add_lookback_edge(state * s, rule const * r, goto_number gotono)
{
	int ri = state_reduction_find(s, r);
	int idx = (s->reductions->lookaheads - LA) + ri;
	PP_Lookback[idx] = goto_list_new(gotono, PP_Lookback[idx]);
}
...
static void build_relations(void)
{
	...
	PP_Includes = (goto_number **)xnmalloc(ngotos, sizeof(*PP_Includes));
	...
	for(goto_number i = 0; i < ngotos; ++i) {
		...
		if(nedges == 0)
			...
			PP_Includes[i] = (goto_number *)xnmalloc(nedges + 1, sizeof(PP_Includes[i][0]));
			for(int j = 0; j < nedges; ++j)
				PP_Includes[i][j] = edge[j];
			PP_Includes[i][nedges] = END_NODE;
		}
	}
	SAlloc::F(edge);
	SAlloc::F(path);
	relation_transpose(&PP_Includes, ngotos);
	if(trace_flag & trace_automaton)
		relation_print("includes", PP_Includes, ngotos, goto_print, stderr);
}
...
static void compute_lookaheads(void)
{
	...
	/* Free LOOKBACK. */
	for(size_t i = 0; i < nLA; ++i)
		LIST_FREE(goto_list, PP_Lookback[i]);
	SAlloc::F(PP_Lookback);
}
...
static int state_lookaheads_count(state * s, bool default_reduction_only_for_accept)
{
	...
	/* We need a lookahead either to distinguish different reductions
	   ...
	   reductions.  */
	s->consistent = !(reds->num > 1 || (reds->num == 1 && trans->num && TRANSITION_IS_SHIFT(trans, 0)) || (reds->num == 1 && reds->rules[0]->number != 0 && default_reduction_only_for_accept));
...
void initialize_LA(void)
{
	...
	// Initialize the members LOOKAHEADS for each state whose reductions require lookahead tokens.
	for(state_number i = 0; i < nstates; ++i) {
...
static void lookaheads_print(FILE * out)
{
	fputs("Lookaheads:\n", out);
	for(state_number i = 0; i < nstates; ++i) {
...
void lalr(void)
{
	...
	PP_Lookback = (goto_list **)xcalloc(nLA, sizeof(*PP_Lookback));
	build_relations();
	compute_follows();
	compute_lookaheads();
	if(trace_flag & trace_sets)
...
void lalr_update_state_numbers(state_number old_to_new[], state_number nstates_old)
{
	...
	for(goto_number i = 0; i < ngotos; ++i) {
		...
		// If old_to_new[from_state[i]] = nstates_old, remove this goto entry.
		if(old_to_new[from_state[i]] != nstates_old) {
			// from_state[i] is not removed, so it and thus to_state[i] are reachable, so to_state[i] != nstates_old.
