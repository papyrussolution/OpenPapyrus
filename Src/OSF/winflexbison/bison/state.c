...
static transitions * transitions_new(int num, state ** dst)
{
...
state * state_new(symbol_number accessing_symbol, size_t nitems, item_index * core)
{
...
{
	SAlloc::F(s->transitions);
	SAlloc::F(s->reductions);
	SAlloc::F(s->errs);
	SAlloc::F(s);
}
...
static bool state_comparator(void const * s1, void const * s2) { return state_compare((const state *)s1, (const state *)s2); }
...
| Create the states hash table.  |
   `-------------------------------*/
void state_hash_new(void) { state_table = hash_xinitialize(HT_INITIAL_CAPACITY, NULL, state_hasher, state_comparator, NULL); }

/*---------------------------------------------.
| Free the states hash table, not the states.  |
   `---------------------------------------------*/
void state_hash_free(void) { hash_free(state_table); }

/*-----------------------------------.
| Insert S in the state hash table.  |
   `-----------------------------------*/
void state_hash_insert(state * s) { hash_xinsert(state_table, s); }
...
state * state_hash_lookup(size_t nitems, const item_index * core)
{
	size_t items_size = nitems * sizeof *core;
	state * probe = (state *)xmalloc(offsetof(state, items) + items_size);
	probe->nitems = nitems;
	memcpy(probe->items, core, items_size);
	state * entry = (state *)hash_lookup(state_table, probe);
	SAlloc::F(probe);
	return entry;
}
