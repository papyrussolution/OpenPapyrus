...
static counterexample * new_counterexample(derivation * d1, derivation * d2, bool shift_reduce, bool u, bool t)
{
	counterexample * res = (counterexample *)xmalloc(sizeof *res);
...
static void free_counterexample(counterexample * cex)
{
...
static void counterexample_print(const counterexample * cex, FILE * out, const char * prefix)
{
	const bool flat = (bool)getenv("YYFLAT");
	const char * example1_label = cex->unifying ? _("Example") : _("First example");
	const char * example2_label = cex->unifying ? _("Example") : _("Second example");
	const char * deriv1_label = cex->shift_reduce ? _("Shift derivation") : _("First reduce derivation");
	const char * deriv2_label = cex->shift_reduce ? _("Reduce derivation") : _("Second reduce derivation");
	const int width = max_int(max_int(mbswidth(example1_label, 0), mbswidth(example2_label, 0)), max_int(mbswidth(deriv1_label, 0),   mbswidth(deriv2_label, 0)));
	if(flat)
		fprintf(out, "  %s%s%*s ", prefix, example1_label, width - mbswidth(example1_label, 0), "");
	else
		fprintf(out, "  %s%s: ", prefix, example1_label);
	derivation_print_leaves(cex->d1, out);
	if(flat)
		fprintf(out, "  %s%s%*s ", prefix, deriv1_label, width - mbswidth(deriv1_label, 0), "");
	else
...
static si_bfs_node * si_bfs_new(state_item_number si, si_bfs_node * parent)
{
	si_bfs_node * res = (si_bfs_node *)xcalloc(1, sizeof *res);
	res->si = si;
	res->parent = parent;
...
static bool si_bfs_contains(const si_bfs_node * n, state_item_number sin)
{
 ...
 */
static derivation * complete_diverging_example(symbol_number conflict_sym,
...
static state_item_list nonunifying_shift_path(state_item_list reduce_path, state_item * shift_conflict)
{
...
static search_state * initial_search_state(state_item * conflict1, state_item * conflict2)
{
	search_state * res = (search_state *)xmalloc(sizeof *res);
...
static void search_state_free_children(search_state * ss)
{
...
static void search_state_print(search_state * ss)
{
...
static inline bool search_state_list_next(gl_list_iterator_t * it, search_state ** ss)
{
	const void * p = NULL;
 ...
 */
static inline void ss_set_parse_state(search_state * ss, int idx, parse_state * ps)
{
...
static void ssb_append(search_state * ss)
{
	...
	search_state_bundle * ssb = (search_state_bundle *)xmalloc(sizeof *ssb);
	ssb->complexity = ss->complexity;
	gl_list_node_t n = gl_list_search(ssb_queue, ssb);
	if(!n) {
		ssb->states = gl_list_create_empty(GL_LINKED_LIST, NULL, NULL, (gl_listelement_dispose_fn)search_state_free_children, true);
 ...
 */
static void production_step(search_state * ss, int parser_state)
{
...
static inline int reduction_cost(const parse_state * ps)
{
	int shifts;
	int productions;
 ...
 */
static void search_state_prepend(search_state * ss, symbol_number sym, bitset guide)
{
 ...
 */
static bool have_common_prefix(const item_number * itm1, const item_number * itm2)
{
	int i = 0;
...
static const item_number * item_rule_end(const item_number * item)
{
	const item_number * res = NULL;
	...
	while(gl_list_size(ssb_queue) > 0) {
		...
		for(gl_list_iterator_t it = gl_list_iterator(ssb->states);
			...
			if(TIME_LIMIT_ENFORCED) {
				float time_passed = (float)(difftime(time(NULL), start));
				if(!assurance_printed && time_passed > ASSURANCE_LIMIT && stage3result) {
					fputs("Productions leading up to the conflict state found.  Still finding a possible unifying counterexample...", stderr);
		...
		gl_sortedlist_remove(ssb_queue, (gl_listelement_compar_fn)ssb_comp, ssb);
	}
cex_search_end:;
	if(!cex) {
 ...
 */
static void counterexample_report(state_item_number itm1, state_item_number itm2, symbol_number next_sym, bool shift_reduce, FILE * out, const char * prefix)
{
	...
	bitset_zero(rpp_set);
	state_item * si = NULL;
	for(gl_list_iterator_t it = gl_list_iterator(shortest_path); state_item_list_next(&it, &si);) {
		bitset_set(scp_set, si->state->number);
		reduce_prod_reached = reduce_prod_reached || item_rule(si->item) == reduce_rule;
	...
	counterexample * cex = difftime(t, cumulative_time) < CUMULATIVE_TIME_LIMIT ? 
		unifying_example(itm1, itm2, shift_reduce, shortest_path, next_sym) : example_from_path(shift_reduce, itm2, shortest_path, next_sym);
...
static void counterexample_report_shift_reduce(state_item_number itm1, state_item_number itm2, symbol_number next_sym, FILE * out, const char * prefix)
{
	if(out == stderr)
		complain(NULL, Wcounterexamples, _("shift/reduce conflict on token %s"), symbols[next_sym]->tag);
	else {
		fputs(prefix, out);
...
static void counterexample_report_reduce_reduce(state_item_number itm1, state_item_number itm2, bitset conflict_syms, FILE * out, const char * prefix)
{
	{
		...
		if(out == stderr)
			complain(NULL, Wcounterexamples, ngettext("reduce/reduce conflict on token %s", "reduce/reduce conflict on tokens %s",
			    bitset_count(conflict_syms)), tokens);
		else {
			fputs(prefix, out);
			fprintf(out, ngettext("reduce/reduce conflict on token %s", "reduce/reduce conflict on tokens %s",
			    bitset_count(conflict_syms)), tokens);
			fprintf(out, "%s\n", _(":"));
		}
