...
static void print_core(struct obstack * oout, state * s)
{
	...
	for(size_t i = 0; i < snritems; ++i) {
		...
		if(report_flag & report_lookaheads && item_number_is_rule_number(*sp1)) {
...
static void print_actions(state const * s, FILE * fgraph)
{
	...
	for(int i = 0; i < trans->num; i++)
		if(!TRANSITION_IS_DISABLED(trans, i)) {
			...
			if(TRANSITION_IS_ERROR(trans, i) && STRNEQ(symbols[sym]->tag, "error"))
				abort();
			output_edge(s->number, s1->number, TRANSITION_IS_ERROR(trans, i) ? NULL : symbols[sym]->tag, style, fgraph);
		}
	/* Display reductions. */
...
static void print_state(state * s, FILE * fgraph)
{
