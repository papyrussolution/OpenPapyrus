...
{
	infinity = size + 2;
	indexes = (relation_nodes)xcalloc(size + 1, sizeof *indexes);
	vertices = (relation_nodes)xnmalloc(size + 1, sizeof *vertices);
	top = 0;
	R = r;
	F = function;
	for(relation_node i = 0; i < size; i++)
...
void relation_transpose(relation * R_arg, relation_node size)
{
	...
	size_t * nedges = (size_t *)xcalloc(size, sizeof *nedges);
	for(relation_node i = 0; i < size; i++)
		if(r[i])
			for(relation_node j = 0; r[i][j] != END_NODE; ++j)
				++nedges[r[i][j]];
	...
	/* Allocate. */
	/* The result. */
	relation new_R = (relation)xnmalloc(size, sizeof *new_R);
	/* END_R[I] -- next entry of NEW_R[I]. */
	relation end_R = (relation)xnmalloc(size, sizeof *end_R);
	for(relation_node i = 0; i < size; i++) {
		relation_node * sp = NULL;
		if(nedges[i] > 0) {
			sp = (relation_node *)xnmalloc(nedges[i] + 1, sizeof *sp);
