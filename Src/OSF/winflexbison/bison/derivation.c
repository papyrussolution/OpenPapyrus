...
{
	derivation * res = (derivation *)xmalloc(sizeof *res);
	res->sym = sym;
...
void derivation_retain(derivation * d)
{
	++d->reference_count;
}
...
static int derivation_depth(const derivation * deriv)
{
	if(deriv->children) {
...
static bool all_spaces(const char * s)
{
	while(c_isspace(*s))
		s++;
	return *s == '\0';
...
static int fputs_if(bool cond, FILE * out, int * padding, const char * s)
{
...
static void derivation_print_tree(const derivation * deriv, FILE * out, const char * prefix)
{
	fputc('\n', out);
...
static bool derivation_print_flat_impl(derivation * deriv, FILE * out, bool leaves_only, int * counter, const char * prefix)
{
	if(deriv->children) {
		const Symbol * sym = symbols[deriv->sym];
		deriv->color = *counter;
...
static void derivation_print_flat(const derivation * deriv, FILE * out, const char * prefix)
{
	int counter = 0;
	fputs(prefix, out);
