...
struct lssi {
	...
	lssi * parent;
	bitset lookahead; // this is the precise lookahead set (follow_L from the CupEx paper)
	bool free_lookahead;
};

static lssi * new_lssi(state_item_number si, lssi * p, bitset l, bool free_l)
{
	lssi * res = (lssi *)xmalloc(sizeof *res);
	if(res) {
		res->si = si;
		res->parent = p;
		res->lookahead = l;
...
static size_t lssi_hasher(lssi * sn, size_t max)
{
	size_t hash = sn->si;
...
static bool lssi_comparator(lssi * s1, lssi * s2)
{
	if(s1->si == s2->si) {
...
static inline bool append_lssi(lssi * sn, Hash_table * visited, lssi_list queue)
{
 ...
 */
static bitset eligible_state_items(state_item * target)
{
...
state_item_list shortest_path_from_start(state_item_number target, symbol_number next_sym)
{
	...
	Hash_table * visited = hash_initialize(32, NULL, (Hash_hasher)lssi_hasher,
		(Hash_comparator)lssi_comparator, (Hash_data_freer)lssi_free);
	bitset il = bitset_create(nsyms, BITSET_FIXED);
	bitset_set(il, 0);
	lssi * init = new_lssi(0, NULL, il, true);
	lssi_list queue = gl_list_create_empty(GL_LINKED_LIST, NULL, NULL, NULL, true);
		...
		if(si->prods) {
			...
			BITSET_FOR_EACH(biter, si->prods, nextSI, 0)
			{
				if(!bitset_test(eligible, nextSI))
					continue;
				lssi * next = new_lssi(nextSI, n, lookahead, !lookahead_used);
				lookahead_used = append_lssi(next, visited, queue) || lookahead_used;
			}
			if(!lookahead_used)
	...
	state_item_list res = gl_list_create_empty(GL_LINKED_LIST, NULL, NULL, NULL, true);
