...
{
	struct parse_state::deriv_chunk * dc = (struct parse_state::deriv_chunk *)&ps->derivs;
...
parse_state * new_parse_state(const state_item * si)
{
...
static parse_state * copy_parse_state(bool prepend, parse_state * parent)
{
...
// signature.
static void list_add_last(gl_list_t list, const void * elt)
{
    ...
    int si_index, parse_state * ret)
{
	...
	gl_list_t ret_chunks[4] = { ret->state_items.contents, NULL, ret->derivs.contents, popped_derivs};
	...
	if(s_size > 0) {
		ret->state_items.tail_elt = (const state_item *)gl_list_get_at(ret->state_items.contents, s_size - 1);
		ret->state_items.head_elt = (const state_item *)gl_list_get_at(ret->state_items.contents, 0);
	}
	else {
	...
	if(d_size > 0) {
		ret->derivs.tail_elt = (const derivation *)gl_list_get_at(ret->derivs.contents, d_size - 1);
		ret->derivs.head_elt = (const derivation *)gl_list_get_at(ret->derivs.contents, 0);
	}
	else {
...
void parse_state_lists(parse_state * ps, state_item_list * sitems, derivation_list * derivs)
{
 ...
 */
static void nullable_closure(parse_state * ps, state_item * si, parse_state_list state_list)
{
...
parse_state_list simulate_reduction(parse_state * ps, int rule_len, bitset symbol_set)
{
	...
	if(s_size != rule_len + 1) {
		...
		if(gl_list_size(prev) == 0) {
			...
			for(gl_list_iterator_t it = gl_list_iterator(prev); state_item_list_next(&it, &psis);) {
