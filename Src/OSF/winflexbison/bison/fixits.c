...
{
	fixit * res = (fixit *)xmalloc(sizeof *res);
	res->location = *loc;
	res->fix = xstrdup(fix);
	return res;
}

static int fixit_cmp(const fixit * a, const fixit * b)
{
...
static void fixit_print(fixit const * f, FILE * out)
{
...
void fixits_run(void)
{
	...
	if(rename(input, backup))
		error(EXIT_FAILURE, get_errno(), _("%s: cannot backup"), quotearg_colon(input));
	...
	while(gl_list_iterator_next(&iter, &p, NULL)) {
		fixit const * f = (fixit const * )p;
		/* Look for the correct line. */
		while((int)line < f->location.start.line) {
			int c = getc(in);
			if(c == EOF)
				break;
			if(c == '\n') {
		...
		if(need_eol)
			putc('\n', out);
		/* Skip the bad input. */
		while((int)line < f->location.end.line) {
			int c = getc(in);
			if(c == EOF)
				break;
			if(c == '\n') {
		...
		while((int)offset < f->location.end.byte) {
			int c = getc(in);
			if(c == EOF)
				break;
			++offset;
		}
		/* If erasing the content of a full line, also remove the end-of-line. */
