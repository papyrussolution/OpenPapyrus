...
static bitset hash_pair_lookup(Hash_table * tab, int key)
{
	hash_pair probe;
	probe.key = key;
	hash_pair * hp = (hash_pair *)hash_lookup(tab, &probe);
	return hp ? hp->l : NULL;
}

static void hash_pair_insert(Hash_table * tab, int key, bitset val)
{
	hash_pair * hp = (hash_pair *)xmalloc(sizeof *hp);
	hp->key = key;
	hp->l = val;
	hash_pair * res = (hash_pair *)hash_xinsert(tab, hp);
...
static inline void state_item_set(state_item_number sidx, const state * s, item_number off)
{
...
static void init_state_items(void)
{
	...
	state_item_map = (state_item_number *)xnmalloc(nstates + 1, sizeof(state_item_number));
	state_items = (state_item *)xnmalloc(nstate_items, sizeof(state_item));
...
static void init_trans(void)
{
	for(state_number i = 0; i < nstates; ++i) {
		...
		Hash_table * transition_set = hash_xinitialize(t->num, NULL, (Hash_hasher)state_sym_hasher, (Hash_comparator)state_sym_comparator, NULL);
...
static inline void disable_state_item(state_item * si)
{
	si->trans = -2;
	bitset_free(si->revs);
	if(si->prods)
