...
void symbol_list_free(symbol_list * list)
{
	for(symbol_list * next; list; list = next) {
		...
		if(list->content_type == symbol_list::SYMLIST_TYPE)
			SAlloc::F(list->content.sem_type);
		SAlloc::F(list);
	}
}
...
void symbol_list_code_props_set(symbol_list * node, code_props_type kind, code_props const * cprops)
{
	switch(node->content_type) {
		...
		case symbol_list::SYMLIST_TYPE:
		    semantic_type_code_props_set(semantic_type_get(node->content.sem_type->tag, &node->content.sem_type->location), kind, cprops);
