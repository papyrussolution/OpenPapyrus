 ...
 */
static void flag_argmatch(const char * opt, xargmatch_fn xargmatch, int all, int * flags, char * arg, size_t no)
{
 ...
 */
static void flags_argmatch(const char * opt, xargmatch_fn xargmatch, usage_fn usage, int all, int * flags, char * args)
{
	if(!args)
		*flags |= all;
		...
		for(args = strtok(args, ","); args; args = strtok(NULL, ",")) {
			...
			flag_argmatch(opt, xargmatch, all, flags, args, no);
		}
}
...
#define FLAGS_ARGMATCH(FlagName, Args, All)                             \
	flags_argmatch("--" #FlagName, (xargmatch_fn*)argmatch_ ## FlagName ## _value,       \
	    argmatch_ ## FlagName ## _usage, All, &FlagName ## _flag, Args)
...
static const argmatch_report_arg argmatch_report_args[] =
{
	...
	{ "lookaheads",      (argmatch_report_type)(report_states | report_lookaheads) },
	{ "solved",          (argmatch_report_type)(report_states | report_solved_conflicts) },
...
void skeleton_arg(char const * arg, int prio, Location loc)
{
	if(prio < skeleton_prio) {
		...
		complain(&loc, complaint, _("multiple skeleton declarations are invalid"));
}
...
static void getargs_colors(int argc, char * argv[])
{
