...
#define Sbitset__nbytes(NBITS) (((NBITS) + CHAR_BIT - 1) / CHAR_BIT)
#define Sbitset__byteAddress(SELF, INDEX) (((SELF) + (INDEX) / CHAR_BIT))
#define Sbitset__bit_mask(INDEX) (1 << (CHAR_BIT - 1 - (INDEX) % CHAR_BIT))
#define Sbitset__last_byte_mask(NBITS) (UCHAR_MAX << (CHAR_BIT - 1 - ((NBITS)-1) % CHAR_BIT))
...
#define Sbitset__test(SELF, INDEX) ((*Sbitset__byteAddress((SELF), (INDEX)) & Sbitset__bit_mask(INDEX)) != 0)
...
#define Sbitset__zero(SELF, NBITS)             \
	do {                                          \
		memzero(SELF, Sbitset__nbytes(NBITS));  \
	} while(0)
