...
FOLDER_INTERFACE(IFolderFolder, 0x00) { INTERFACE_FolderFolder(PURE) };
...
FOLDER_INTERFACE(IFolderAltStreams, 0x17) { INTERFACE_FolderAltStreams(PURE) };
FOLDER_INTERFACE(IFolderWasChanged, 0x04) { STDMETHOD(WasChanged) (Int32 *wasChanged) PURE; };
...
FOLDER_INTERFACE(IFolderOperations, 0x13) { INTERFACE_FolderOperations(PURE) };
/*
 ...
 */
FOLDER_INTERFACE(IFolderGetSystemIconIndex, 0x07) { STDMETHOD(GetSystemIconIndex) (UInt32 index, Int32 *iconIndex) PURE; };
FOLDER_INTERFACE(IFolderGetItemFullSize, 0x08) { STDMETHOD(GetItemFullSize) (UInt32 index, PROPVARIANT *value, IProgress *progress) PURE; };
FOLDER_INTERFACE(IFolderCalcItemFullSize, 0x14) { STDMETHOD(CalcItemFullSize) (UInt32 index, IProgress *progress) PURE; };
FOLDER_INTERFACE(IFolderClone, 0x09) { STDMETHOD(Clone) (IFolderFolder **resultFolder) PURE; };
FOLDER_INTERFACE(IFolderSetFlatMode, 0x0A) { STDMETHOD(SetFlatMode) (Int32 flatMode) PURE; };
/*
...
FOLDER_INTERFACE(IFolderProperties, 0x0E) { INTERFACE_FolderProperties(PURE) };
...
FOLDER_INTERFACE(IFolderArcProps, 0x10) { INTERFACE_IFolderArcProps(PURE) };
FOLDER_INTERFACE(IGetFolderArcProps, 0x11) { STDMETHOD(GetFolderArcProps) (IFolderArcProps **object) PURE; };
FOLDER_INTERFACE(IFolderCompare, 0x15) { STDMETHOD_(Int32, CompareItems) (UInt32 index1, UInt32 index2, PROPID propID, Int32 propIsRaw) PURE; };
...
FOLDER_INTERFACE(IFolderGetItemName, 0x16) { INTERFACE_IFolderGetItemName(PURE) };

#define FOLDER_MANAGER_INTERFACE(i, x)  DECL_INTERFACE(i, 9, x)

#define INTERFACE_IFolderManager(x) \
	STDMETHOD(OpenFolderFile) (IInStream *inStream, const wchar_t * filePath, const wchar_t * arcFormat, IFolderFolder **resultFolder, \
...
FOLDER_MANAGER_INTERFACE(IFolderManager, 0x05) { INTERFACE_IFolderManager(PURE); };
