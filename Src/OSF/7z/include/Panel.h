...
struct CPropColumn {
	...
	int Compare(const CPropColumn &a) const {
...
public:
	...
	HRESULT CreateShellContextMenu(const CRecordVector<UInt32> &operatedIndices,
	...
	bool IsDeviceDrivesPrefix() const {
		return _currentFolderPrefix == L"\\\\.\\";
	}

	bool IsSuperDrivesPrefix() const {
		return _currentFolderPrefix == L"\\\\?\\";
	}
...
public:
		...
		CDisableNotify(CPanel &panel) : _panel(panel) {
			Disable();
		}
	...
	void RefreshTitle(bool always = false) {
		_panelCallback->RefreshTitle(always);
	}
