...
// @v11.4.0 (replaced with SBits::Rotr) #define rotr32 rotrFixed
#define BLAKE2S_NUM_ROUNDS 10
#define BLAKE2S_FINAL_FLAG (~(uint32)0)

static const uint32 k_Blake2s_IV[8] = { 0x6A09E667UL, 0xBB67AE85UL, 0x3C6EF372UL, 0xA54FF53AUL, 0x510E527FUL, 0x9B05688CUL, 0x1F83D9ABUL, 0x5BE0CD19UL };
...
static void Blake2s_Compress(CBlake2s * p)
{
  ...
  #define G(r, i, a, b, c, d) \
	a += b + m[sigma[2*i+0]];  d ^= a; d = SBits::Rotr(d, 16);  c += d;  b ^= c; b = SBits::Rotr(b, 12); \
	a += b + m[sigma[2*i+1]];  d ^= a; d = SBits::Rotr(d,  8);  c += d;  b ^= c; b = SBits::Rotr(b,  7); \
...
#define Blake2s_Increment_Counter(S, inc) { p->t[0] += (inc); p->t[1] += (p->t[0] < (inc)); }
#define Blake2s_Set_LastBlock(p) { p->f[0] = BLAKE2S_FINAL_FLAG; p->f[1] = p->lastNode_f1; }
...
static void Blake2s_Final(CBlake2s * p, Byte * digest)
{
	...
	memzero(p->buf + p->bufPos, BLAKE2S_BLOCK_SIZE - p->bufPos);
	Blake2s_Compress(p);
	for(uint i = 0; i < 8; i++)
...
{
	p->bufPos = 0;
	for(uint i = 0; i < BLAKE2SP_PARALLEL_DEGREE; i++)
