...
#define DEF_GetHeads2(name, v, action) \
	static void GetHeads ## name(const Byte *p, uint32 pos,	uint32 *hash, uint32 hashMask, uint32 *heads, uint32 numHeads, const uint32 *crc) \
	{ action; for(; numHeads != 0; numHeads--) { const uint32 value = (v); p++; *heads++ = pos - hash[value]; hash[value] = pos++;  } }
...
static void BtGetMatches(CMatchFinderMt * p, uint32 * distances)
{
	...
	while(curPos < limit) {
		...
		}
		{
			...
			{
				uint32 posRes;
				curPos = limit - GetMatchesSpecN(lenLimit,
				    pos,
				    p->buffer,
				    p->son,
				    cyclicBufferPos,
...
/*
   ...
   if(curMatch4 >= matchMinPos)
    if(cur[(ptrdiff_t)curMatch4 - lzPos] == cur[0] && cur[(ptrdiff_t)curMatch4 - lzPos + 3] == cur[3]) {
   *distances++ = 4;
...
{
	SKIP_HEADER2_MT { p->btNumAvailBytes--; SKIP_FOOTER_MT }
	...
	static void MatchFinderMt3_Skip(CMatchFinderMt * p, uint32 num)
	{
		SKIP_HEADER_MT(3)
		uint32 h2, h3;
		MT_HASH3_CALC(hash + kFix3HashSize)[h3] = hash[h2] = p->lzPos;
		SKIP_FOOTER_MT
	}
/*
