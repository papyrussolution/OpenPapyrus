...
void CAgentFolder::GetRealIndices(const UInt32 * indices,
    UInt32 numItems,
    bool includeAltStreams,
    bool includeFolderSubItemsInFlatMode,
...
STDMETHODIMP CAgent::Open(IInStream * inStream,
...
STDMETHODIMP CAgent::Extract(NExtract::NPathMode::EEnum pathMode,
    NExtract::NOverwriteMode::EEnum overwriteMode,
    const wchar_t * path,
    Int32 testMode,
    IFolderArchiveExtractCallback * extractCallback2)
...
STDMETHODIMP CAgent::GetArcProp(UInt32 level, PROPID propID, PROPVARIANT * value)
{
	...
	if(level == (UInt32)_archiveLink.Arcs.Size()) {
		...
		switch(propID)
		{
			...
			case kpidErrorType: if(arc.ErrorInfo.ErrorFormatIndex >= 0) prop =
				    g_CodecsObj->Formats[arc.ErrorInfo.ErrorFormatIndex].Name; break;
			case kpidErrorFlags:
		    {
