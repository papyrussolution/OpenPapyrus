...
static HRESULT DecompressArchive(CCodecs * codecs, const CArchiveLink &arcLink, uint64 packSize,
    const NWildcard::CCensorNode &wildcardCensor, const CExtractOptions &options, bool calcCrc,
    IExtractCallbackUI * callback, CArchiveExtractCallback * ecs, UString &errorMessage, uint64 &stdInProcessed)
{
	...
	if(!options.StdInMode) {
		...
		for(uint32 i = 0; i < numItems; i++) {
			...
			if(elimIsPossible) {
	  #ifdef SUPPORT_ALT_STREAMS
			    const UString & s = item.MainPath;
	  #else
			    const UString & s = item.Path;
	  #endif
	...
	ecs->Init(options.NtOptions, options.StdInMode ? &wildcardCensor : NULL, &arc, callback, options.StdOutMode, options.TestMode, outDir, removePathParts, false, packSize);
#ifdef SUPPORT_LINKS
...
#endif
	HRESULT result;
	int32 testMode = BIN(options.TestMode && !calcCrc);
	if(options.StdInMode) {
...
{
	for(uint left = 0, right = fileName.Size(); left != right;) {
...
HRESULT Extract(CCodecs * codecs, const CObjectVector<COpenType> &types, const CIntVector &excludedFormats,
    UStringVector & arcPaths, UStringVector & arcPathsFull, const NWildcard::CCensorNode &wildcardCensor, const CExtractOptions &options,
    ...
    UString &errorMessage, CDecompressStat &st)
{
	...
	uint   numArcs = options.StdInMode ? 1 : arcPaths.Size();
	uint   i;
	for(i = 0; i < numArcs; i++) {
		NFind::CFileInfo fi;
		fi.Size = 0;
		...
		if(!skipArcs[i]) {
			...
			if(result != S_OK) {
				...
				if(!options.StdInMode) {
					...
					// totalPackSize -= DeleteUsedFileNamesFromList(arcLink, i + 1, arcPaths, arcPathsFull,
					// &arcSizes);
						...
						FOR_VECTOR(v, arcLink.VolumePaths) {
							...
							if(index >= 0) {
								if((uint)index > i) {
									...
									correctionSize -= arcSizes[(uint)index];
								}
							}
						}
						if(correctionSize != 0) {
				...
				/*
				   ...
				   if(passwordIsDefined) { RINOK(extractCallback->SetPassword(password)); }
				   #endif
				 */
				...
				bool calcCrc = false;
			#endif
				RINOK(DecompressArchive(codecs, arcLink, fi.Size + arcLink.VolumesSize, wildcardCensor, options, calcCrc, extractCallback, ecs, errorMessage, packProcessed));
				if(!options.StdInMode)
