...
#ifndef _SFX
	void COutStreamWithHash::SetStream(ISequentialOutStream * stream) { _stream = stream; }
	void COutStreamWithHash::ReleaseStream() { _stream.Release(); }
	void COutStreamWithHash::Init(bool calculate/* = true*/)
	{
		InitCRC();
		_size = 0;
		_calculate = calculate;
	}
	void COutStreamWithHash::EnableCalc(bool calculate) { _calculate = calculate; }
	void COutStreamWithHash::InitCRC() { _hash->InitForNewFile(); }
...
#endif
#ifdef SUPPORT_LINKS
	void CHardLinks::Clear()
	{
		IDs.Clear();
		Links.Clear();
	}
	void CHardLinks::PrepareLinks()
	{
...
CExtractNtOptions::CExtractNtOptions() : ReplaceColonForAltStream(false), WriteToAltStreamIfColon(false)
{
	SymLinks.Val = true;
	HardLinks.Val = true;
	AltStreams.Val = true;
#ifdef _WIN32
	PreAllocateOutFile = true;
#else
	PreAllocateOutFile = false;
#endif
}

CArchiveExtractCallback::CArchiveExtractCallback() : WriteCTime(true), WriteATime(true), WriteMTime(true), _multiArchives(false)
{
...
void CArchiveExtractCallback::Init(const CExtractNtOptions &ntOptions, const NWildcard::CCensorNode * wildcardCensor,
    const CArc * arc, IFolderArchiveExtractCallback * extractCallback2, bool stdOutMode, bool testMode,
    const FString &directoryPath, const UStringVector &removePathParts, bool removePartsForAltStreams, uint64 packSize)
{
...
STDMETHODIMP CArchiveExtractCallback::SetTotal(uint64 size)
{
	COM_TRY_BEGIN
	    _progressTotal = size;
	_progressTotal_Defined = true;
	return (!_multiArchives && _extractCallback2) ? _extractCallback2->SetTotal(size) : S_OK;
	COM_TRY_END
}
...
void CArchiveExtractCallback::InitForMulti(bool multiArchives, NExtract::NPathMode::EEnum pathMode, NExtract::NOverwriteMode::EEnum overwriteMode)
{
...
#ifndef _SFX
void FASTCALL CArchiveExtractCallback::SetHashMethods(IHashCalc * hash)
{
	if(hash) {
...
void FASTCALL CArchiveExtractCallback::SetBaseParentFolderIndex(uint32 indexInArc)
{
...
bool CensorNode_CheckPath2(const NWildcard::CCensorNode &node, const CReadArcItem &item, bool &include)
{
	...
	bool include2;
	if(node.CheckPathVect(pathParts2, true /*isFile*/, include2)) {
		include = include2;
		return true;
	}
  #endif
...
{
	bool include;
	return CensorNode_CheckPath2(node, item, include) ? include : false;
}
...
STDMETHODIMP CArchiveExtractCallback::GetStream(uint32 index, ISequentialOutStream ** outStream, int32 askExtractMode)
{
	...
	* outStream = NULL;
  ...
  #endif
	{
				...
				if(_item.IsDir) {
					...
					if(fileInfo.Find(fullProcessedPath)) {
						switch(_overwriteMode) {
							...
							case NExtract::NOverwriteMode::kAsk:
						    {
							    ...
							    RINOK(_extractCallback2->AskOverwrite(
									    fs2us(realFullProcessedPath), &fileInfo.MTime, &fileInfo.Size,
									    _item.Path,
						...
						if(_overwriteMode == NExtract::NOverwriteMode::kRename) {
							...
							// MyMoveFile can raname folders. So it's OK to use it for
							// folders too
							...
							if(fileInfo.IsDir()) {
								...
								if(!RemoveDir(fullProcessedPath)) {
									RINOK(SendMessageError_with_LastError(kCantDeleteOutputDir,
										    fullProcessedPath));
									return S_OK;
								}
							}
								...
								if(needDelete) {
									if(!DeleteFileAlways(fullProcessedPath)) {
										RINOK(SendMessageError_with_LastError(kCantDeleteOutputFile,
											    fullProcessedPath));
										return S_OK;
										// return E_FAIL;
									}
						...
						if(colonPos >= 0 && fullProcessedPath[(uint)colonPos + 1] != 0) {
							...
							if(parentFi.Find(parentFsPath)) {
								if(parentFi.IsReadOnly())
									SetFileAttrib(parentFsPath,
									    parentFi.Attrib & ~FILE_ATTRIBUTE_READONLY);
							}
						}
      #endif
					}
						...
						if(!IsSafePath(relatPath)) {
							...
							if(!existPath.IsEmpty()) {
								if(isHardLink /* || isCopyLink */) {
									...
									// bool isSymLink = true; // = false for
									// junction
									if(_item.IsDir && !isRelative) {
										// if it's before Vista we use Junction
										// Point
										// isJunction = true;
										// convertToAbs = true;
									}
		...
		if(_hashStream) {
			if(askExtractMode == NArchive::NExtractArc::NAskMode::kExtract ||
			    askExtractMode == NArchive::NExtractArc::NAskMode::kTest) {
		...
		if(outStreamLoc) {
			/*
			   ...
			   return SetOperationResult(NArchive::NExtractArc::NOperationResult::kOK);
			   }
	...
	STDMETHODIMP CArchiveExtractCallback::PrepareOperation(int32 askExtractMode)
	{
		...
		switch(askExtractMode) {
			case NArchive::NExtractArc::NAskMode::kExtract:
			    if(_testMode)
				    askExtractMode = NArchive::NExtractArc::NAskMode::kTest;
			    else
				    _extractMode = true;
		...
		return _extractCallback2->PrepareOperation(_item.Path, BoolToInt(_item.IsDir), askExtractMode, _isSplit ? &_position : 0);
		COM_TRY_END
	}
	...
	STDMETHODIMP CArchiveExtractCallback::SetOperationResult(int32 opRes)
	{
		...
		if(_outFileStream) {
			HRESULT hres = S_OK;
			_outFileStreamSpec->SetTime((WriteCTime && _fi.CTimeDefined) ? &_fi.CTime : NULL,
			    (WriteATime && _fi.ATimeDefined) ? &_fi.ATime : NULL, (WriteMTime && _fi.MTimeDefined) ? &_fi.MTime : (_arc->MTimeDefined ? &_arc->MTime : NULL));
...
#define RINOZ(x) { int __tt = (x); if(__tt != 0) return __tt; }
	...
	HRESULT CArchiveExtractCallback::SetDirsTimes()
	{
		...
		for(i = 0; i < pairs.Size(); i++) {
			...
			SetDirTime(_extractedFolderPaths[pairIndex], (WriteCTime && CTimeDefined) ? &CTime : NULL,
			    (WriteATime && ATimeDefined) ? &ATime : NULL, (WriteMTime && MTimeDefined) ? &MTime : (_arc->MTimeDefined ? &_arc->MTime : NULL));
		}
		return S_OK;
	}
