...
CDirItemsStat::CDirItemsStat() : NumDirs(0), NumFiles(0), NumAltStreams(0), FilesSize(0), AltStreamsSize(0), NumErrors(0)
{
}

uint64 CDirItemsStat::Get_NumItems() const { return NumDirs + NumFiles + NumAltStreams; }
uint64 CDirItemsStat::Get_NumDataItems() const { return NumFiles + NumAltStreams; }
uint64 CDirItemsStat::GetTotalBytes() const { return FilesSize + AltStreamsSize; }
bool CDirItemsStat::IsEmpty() const { return 0 == NumDirs && 0 == NumFiles && 0 == NumAltStreams && 0 == FilesSize && 0 == AltStreamsSize && 0 == NumErrors; }

void CDirItems::AddDirFileInfo(int phyParent, int logParent, int secureIndex, const NFind::CFileInfo &fi)
{
	CDirItem di;
	di.Size = fi.Size;
	di.CTime = fi.CTime;
...
{
	Stat.NumErrors++;
	return Callback ? Callback->ScanError(path, errorCode) : S_OK;
}
...
static const uint kScanProgressStepMask = (1 << 12) - 1;

HRESULT CDirItems::ScanProgress(const FString &dirPath)
{
	return Callback ? Callback->ScanProgress(Stat, dirPath, true) : S_OK;
}
...
FString CDirItems::GetPhyPath(uint index) const
{
	const CDirItem &di = Items[index];
	return us2fs(GetPrefixesPath(PhyParents, di.PhyParent, di.Name));
}

UString CDirItems::GetLogPath(uint index) const
{
	const CDirItem &di = Items[index];
...
CDirItems::CDirItems() : SymLinks(false), ScanAltStreams(false)
...
HRESULT CDirItems::AddSecurityItem(const FString &path, int &secureIndex)
{
	...
	SETIFZ(errorCode, ERROR_INVALID_FUNCTION);
...
HRESULT CDirItems::EnumerateItems2(const FString &phyPrefix, const UString &logPrefix, const FStringVector &filePaths, FStringVector * requestedPaths)
{
	...
	FOR_VECTOR(i, filePaths) {
		...
		CALLPTRMEMB(requestedPaths, Add(phyPath));
...
static HRESULT EnumerateDirItems(const NWildcard::CCensorNode &curNode, int phyParent, int logParent, const FString &phyPrefix,
    const UStringVector &addArchivePrefix, CDirItems &dirItems, bool enterToSubFolders);

static HRESULT EnumerateDirItems_Spec(const NWildcard::CCensorNode &curNode, int phyParent, int logParent, const FString &curFolderName,
    const FString &phyPrefix, const UStringVector &addArchivePrefix, CDirItems &dirItems, bool enterToSubFolders)
{
...
#ifndef UNDER_CE
	#ifdef _WIN32
		static HRESULT EnumerateAltStreams(const NFind::CFileInfo &fi, const NWildcard::CCensorNode &curNode, int phyParent, int logParent, const FString &fullPath,
	...
	#endif
	HRESULT CDirItems::SetLinkInfo(CDirItem &dirItem, const NFind::CFileInfo &fi, const FString &phyPrefix)
	{
...
static HRESULT EnumerateForItem(NFind::CFileInfo &fi, const NWildcard::CCensorNode &curNode, int phyParent, int logParent, const FString &phyPrefix,
    const UStringVector &addArchivePrefix/*prefix from curNode*/, CDirItems &dirItems, bool enterToSubFolders)
{
	...
	if(dirItems.ScanAltStreams) {
		RINOK(EnumerateAltStreams(fi, curNode, phyParent, logParent, phyPrefix + fi.Name, addArchivePrefixNew, dirItems));
	}
	if(dirItemIndex >= 0) {
	...
	return EnumerateDirItems_Spec(*nextNode, phyParent, logParent, fi.Name, phyPrefix, addArchivePrefixNew, dirItems, enterToSubFolders2);
}
		...
		if(CanUseFsDirect(curNode)) {
			...
			for(i = 0; i < curNode.IncludeItems.Size(); i++) {
				...
				if(needAltStreams && dirItems.ScanAltStreams) {
					UStringVector pathParts;
					pathParts.Add(fs2us(fi.Name));
					RINOK(EnumerateAltStreams(fi, curNode, phyParent, logParent, fullPath, pathParts, dirItems));
				}
	#endif
				if(!isDir)
					continue;
				...
				RINOK(EnumerateDirItems_Spec(*nextNode, phyParent, logParent, fi.Name, phyPrefix, addArchivePrefixNew, dirItems, true));
			}
			for(i = 0; i < curNode.SubNodes.Size(); i++) {
				...
				RINOK(EnumerateDirItems_Spec(nextNode, phyParent, logParent, fi.Name, phyPrefix, UStringVector(), dirItems, false));
			}
			return S_OK;
		}
	}
	...
	if(phyPrefix.IsEmpty() && curNode.IncludeItems.Size() > 0) {
		...
		if(i == curNode.IncludeItems.Size()) {
			...
			for(i = 0; i < driveStrings.Size(); i++) {
				...
				RINOK(EnumerateForItem(fi, curNode, phyParent, logParent, phyPrefix, addArchivePrefix, dirItems, enterToSubFolders));
			}
			return S_OK;
		}
	}
...
void CDirItems::FillFixedReparse()
{
	...
	if(SymLinks) {
		FOR_VECTOR(i, Items) {
			...
			if(item.ReparseData.Size()) {
				CReparseAttr attr;
				if(attr.Parse(item.ReparseData, item.ReparseData.Size()) && !attr.IsRelative()) {
					...
					if(IsDrivePath(link)) {
						...
						if(NDir::MyGetFullPathName(GetPhyPath(i), fullPathF)) {
							...
							if(logPath.Len() < fullPath.Len() && CompareFileNames(logPath, fullPath.RightPtr(logPath.Len())) == 0) {
								...
								if(IsPathSepar(prefix.Back())) {
									...
									if(rootPrefixSize) {
										if(rootPrefixSize == prefix.Len()/*simple case: paths are from root*/)
											continue;  
										if(link.Len() > prefix.Len() && CompareFileNames(link.Left(prefix.Len()), prefix) == 0) {
