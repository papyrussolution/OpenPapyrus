...
CUpdateCallbackConsole::CUpdateCallbackConsole() : DeleteMessageWasShown(false)
    #ifndef _NO_CRYPTO
	, PasswordIsDefined(false), AskPassword(false)
    #endif
{
}

HRESULT CUpdateCallbackConsole::OpenResult(const CCodecs * codecs, const CArchiveLink &arcLink, const wchar_t * name, HRESULT result)
{
	ClosePercents2();
...
void CCallbackConsoleBase::CommonError(const FString &path, DWORD systemError, bool isWarning)
{
	...
	if(_se) {
		CALLPTRMEMB(_so, Flush());
		*_se << endl << (isWarning ? kWarning : kError) << NError::MyFormatMessage(systemError) << endl << fs2us(path) << endl << endl;
		_se->Flush();
	}
}
...
CCallbackConsoleBase::CCallbackConsoleBase() : StdOutMode(false), NeedFlush(false), PercentsNameLevel(1), LogLevel(0)
{
}

bool   CCallbackConsoleBase::NeedPercents() const { return _percent._so != NULL; };
void   CCallbackConsoleBase::SetWindowWidth(uint width) { _percent.MaxLen = width - 1; }

void   CCallbackConsoleBase::Init(CStdOutStream * outStream, CStdOutStream * errorStream, CStdOutStream * percentStream)
{
	FailedFiles.Clear();
	_so = outStream;
	_se = errorStream;
...
void CCallbackConsoleBase::ClosePercents2()
{
	if(NeedPercents())
		_percent.ClosePrint(true);
}

void CCallbackConsoleBase::ClosePercents_for_so()
{
...
HRESULT CUpdateCallbackConsole::GetStream(const wchar_t * name, bool /* isDir */, bool isAnti, uint32 mode)
{
	...
	const char * s;
	if(oneof2(mode, NUpdateNotifyOp::kAdd, NUpdateNotifyOp::kUpdate)) {
		if(isAnti)
			s = "Anti";
