...
static HRESULT CheckBreak2() { return NConsoleClose::TestBreakSignal() ? E_ABORT : S_OK; }

CHashCallbackConsole::CHashCallbackConsole() : PrintNameInPercents(true), PrintHeaders(false), PrintSize(true), PrintName(true)
{
}

CHashCallbackConsole::~CHashCallbackConsole() 
{
}

HRESULT CHashCallbackConsole::CheckBreak() { return CheckBreak2(); }
HRESULT CHashCallbackConsole::ScanError(const FString &path, DWORD systemError) { return ScanError_Base(path, systemError); }
HRESULT CHashCallbackConsole::SetNumFiles(uint64 /* numFiles */) { return CheckBreak2(); }

void CHashCallbackConsole::AddSpacesBeforeName()
{
	_s.Add_Space();
	_s.Add_Space();
}
...
static const uint kSizeField_Len = 13;
static const uint kNameField_Len = 12;
static const uint kHashColumnWidth_Min = 4 * 2;
...
void CHashCallbackConsole::PrintResultLine(uint64 fileSize, const CObjectVector<CHasherState> &hashers, unsigned digestIndex, bool showHash)
{
	ClosePercents_for_so();
	_s.Empty();
...
static const char * const k_DigestTitles[] = { " : ", " for data:              ", " for data and names:    ", " for streams and names: " };
