...
static void PrintStringRight(CStdOutStream &so, const char * s, uint size)
{
	uint len = sstrlen(s);
	for(uint i = len; i < size; i++)
		so << ' ';
	so << s;
}

static void PrintUInt32(CStdOutStream &so, uint32 val, uint size)
{
	char s[16];
...
static void PrintString(CStdOutStream &so, const UString &s, uint size)
{
	const uint len = s.Len();
	so << s;
	for(uint i = len; i < size; i++)
		so << ' ';
}

static char FASTCALL GetHex(unsigned val) { return (char)((val < 10) ? ('0' + val) : ('A' + (val - 10))); }

void CCallbackConsoleBase::CErrorPathCodes::AddError(const FString &path, DWORD systemError)
{
	Paths.Add(path);
	Codes.Add(systemError);
}

void CCallbackConsoleBase::CErrorPathCodes::Clear()
{
	Paths.Clear();
	Codes.Clear();
}

static void PrintWarningsPaths(const CCallbackConsoleBase::CErrorPathCodes & pc, CStdOutStream & so)
{
...
static int WarningsCheck(HRESULT result, const CCallbackConsoleBase &callback, const CUpdateErrorInfo &errorInfo,
    CStdOutStream * so, CStdOutStream * se, bool showHeaders)
{
	...
	if(callback.ScanErrors.Paths.Size() != 0) {
		if(se) {
			...
			*se << "Scan WARNINGS: " << (uint32)callback.ScanErrors.Paths.Size();
			*se << endl;
		}
	...
	if(numErrors == 0) {
		...
		if(se) {
			...
			*se << "WARNING: Cannot open " << (uint32)numErrors << " file";
			if(numErrors > 1)
				*se << 's';
			*se << endl;
		}
...
static void PrintTime(const char * s, uint64 val, uint64 total)
{
	...
	*g_StdStream << '%';
	...
	EXTERN_C_BEGIN
		typedef BOOL (WINAPI *Func_GetProcessMemoryInfo)(HANDLE Process, PPROCESS_MEMORY_COUNTERS ppsmemCounters, DWORD cb);
...
static void PrintStat()
{
	...
	ULONG64 cycleTime = 0;
	{
		...
		Func_GetProcessMemoryInfo my_GetProcessMemoryInfo = (Func_GetProcessMemoryInfo)::GetProcAddress(kern, "K32GetProcessMemoryInfo");
		...
		if(my_GetProcessMemoryInfo)
	        ...
	        // FreeLibrary(lib);
		Func_QueryProcessCycleTime my_QueryProcessCycleTime = (Func_QueryProcessCycleTime)::GetProcAddress(kern, "QueryProcessCycleTime");
  ...
  #ifndef UNDER_CE
	if(memDefined) 
		PrintMemUsage("Virtual ", m.PeakPagefileUsage);
  #endif
	PrintTime("Global ", totalTime, totalTime);
  #ifndef UNDER_CE
	if(memDefined) 
		PrintMemUsage("Physical", m.PeakWorkingSetSize);
  #endif
	*g_StdStream << endl;
}
...
#endif
{
	...
	if(codecs->Formats.Size() == 0 && (isExtractGroupCommand || options.Command.CommandType == NCommandType::kList || options.Command.IsFromUpdateGroup())) {
	...
	FOR_VECTOR(k, options.ExcludedArcTypes) {
		...
		if(!codecs->FindFormatForArchiveType(options.ExcludedArcTypes[k], tempIndices) || tempIndices.Size() != 1)
			throw kUnsupportedArcTypeMessage;
		excludedFormats.AddToUniqueSorted(tempIndices[0]);
		// excludedFormats.Sort();
	}
  #ifdef EXTERNAL_CODECS
	if(isExtractGroupCommand || options.Command.CommandType == NCommandType::kHash || options.Command.CommandType == NCommandType::kBenchmark)
	...
	/*
	   if(!options.EnableHeaders || options.TechMode)
	   showStat = false;
	 */
	if(options.Command.CommandType == NCommandType::kInfo) {
		...
		const uint   kNumArcFlags = (uint)sstrlen(kArcFlags);
		...
		if(options.StdInMode) {
			...
			CDirItemsStat st;
			scan.StartScanning();
			hresultMain = EnumerateDirItemsAndSort(options.arcCensor, NWildcard::k_RelatPath, UString(), /*addPathPrefix*/
		...
		if(hresultMain == S_OK)
			if(isExtractGroupCommand) {
				...
				hresultMain = Extract(codecs, types, excludedFormats, ArchivePathsSorted,
				    ArchivePathsFullSorted, options.Censor.Pairs.Front().Head, eo, ecs, ecs, hashCalc, errorMessage, stat);
				ecs->ClosePercents();
		...
		callback.Init(/*NULL,*/ g_StdStream, g_ErrStream, percentsStream);
		CUpdateErrorInfo errorInfo;
		/*
		   if(!uo.Init(codecs, types, options.ArchiveName))
		   throw kUnsupportedUpdateArcType;
		 */
		hresultMain = UpdateArchive(codecs, types, options.ArchiveName, options.Censor, uo, errorInfo, &openCallback, &callback, true);
		callback.ClosePercents2();
		CStdOutStream * se = g_StdStream;
		SETIFZ(se, g_ErrStream);
		retCode = WarningsCheck(hresultMain, callback, errorInfo, g_StdStream, se, true /*options.EnableHeaders*/);
	}
		...
		hresultMain = HashCalc(EXTERNAL_CODECS_VARS_L options.Censor, uo, errorInfoString, &callback);
		CUpdateErrorInfo errorInfo;
