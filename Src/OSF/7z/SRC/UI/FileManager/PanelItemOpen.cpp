...
public:
	...
	bool GetFirstProcess(PROCESSENTRY32 * pe) {
		return BOOLToBool(Process32First(_handle, pe));
	}

	bool GetNextProcess(PROCESSENTRY32 * pe) {
		return BOOLToBool(Process32Next(_handle, pe));
	}
};
...
public:
	...
	void DisableWait(unsigned index) 
	{
		NeedWait[index] = false;
	}
...
/*
   static HRESULT GetTime(IFolderFolder *folder, UInt32 index, PROPID propID, FILETIME &filetime, bool
      &filetimeIsDefined)
   {
	...
	if(tryAsArchive) {
		...
		if(getStream) {
			...
			if(subSeqStream) {
				...
				if(subStream) {
					bool encrypted;
					HRESULT res = OpenAsArc_Msg(subStream,
					    tempFileInfo,
					    fullVirtPath,
					    type ? type : L"",
					    encrypted,
