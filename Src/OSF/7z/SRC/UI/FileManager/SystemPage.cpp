...
static const UInt32 kLangIDs[] = { IDT_SYSTEM_ASSOCIATE };
...
CSysString CModifiedExtInfo::GetString() const
{
	if(State == kExtState_7Zip)
		return CSysString(TEXT("7-Zip"));
	if(State == kExtState_Clear)
		return CSysString(TEXT(""));
	if(Other7Zip)
		return CSysString(TEXT("[7-Zip]"));
	return ProgramKey;
};
...
bool CSystemPage::OnListKeyDown(LPNMLVKEYDOWN keyDownInfo)
{
	...
	if(alt)
		return false;
	if((ctrl && keyDownInfo->wVKey == 'A') || (!ctrl && keyDownInfo->wVKey == VK_MULTIPLY)) {
		_listView.SelectAll();
		return true;
	}
