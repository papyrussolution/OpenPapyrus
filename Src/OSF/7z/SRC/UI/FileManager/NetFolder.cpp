...
static const Byte kProps[] = { kpidName, kpidLocalName, kpidComment, kpidProvider };
...
STDMETHODIMP CNetFolder::LoadItems()
{
	...
	/*
	   ...
	   for (char c = 'a'; c <= 'z'; c++)
	   {
	    ...
	    if (!findFile.FindFirst(us2fs(resource.RemoteName) + FString(FCHAR_PATH_SEPARATOR) + FCHAR_ANY_MASK,
	       fileInfo))
	      continue;
