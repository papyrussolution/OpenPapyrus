...
void FASTCALL AesCbc_Encode(uint32 * p, Byte * data, size_t numBlocks)
{
...
void FASTCALL AesCbc_Decode(uint32 * p, Byte * data, size_t numBlocks)
{
	uint32 in[4], out[4];
	for(; numBlocks != 0; numBlocks--, data += AES_BLOCK_SIZE) {
		in[0] = GetUi32(data);
...
void FASTCALL AesCtr_Code(uint32 * p, Byte * data, size_t numBlocks)
{
