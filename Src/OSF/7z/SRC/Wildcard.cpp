...
{
	return g_CaseSensitive ? IsString1PrefixedByString2(s1, s2) : IsString1PrefixedByString2_NoCase(s1, s2);
}

int CompareFileNames(const wchar_t * s1, const wchar_t * s2) STRING_UNICODE_THROW
{
	return g_CaseSensitive ? MyStringCompare(s1, s2) : MyStringCompareNoCase(s1, s2);
}
...
void SplitPathToParts_2(const UString &path, UString &dirPrefix, UString &name)
{
	...
	for(; p != start; p--)
		if(IsPathSepar(*(p - 1)))
			break;
	dirPrefix.SetFrom(path, (uint)(p - start));
	name = p;
}
...
{
	return (Name.IsEmpty() && SubNodes.IsEmpty() && ExcludeItems.IsEmpty() && IncludeItems.Size() == 1) ? IncludeItems.Front().AreAllAllowed() : false;
}
...
void CCensor::AddItem(ECensorPathMode pathMode, bool include, const UString &path, bool recursive, bool wildcardMatching)
{
	...
	if(pathParts.Size() >= 3 && pathParts[0].IsEmpty() && pathParts[1].IsEmpty() && pathParts[2] == L"?")
	...
	if(pathMode != k_AbsPath) {
		...
		{
			int dotsIndex = -1;
			for(uint i = numPrefixParts; i < pathParts.Size(); i++) {
				const UString &part = pathParts[i];
				if(part == L".." || part == L".")
					dotsIndex = i;
			}
			if(dotsIndex >= 0)
				numSkipParts = (dotsIndex == ((int)pathParts.Size() - 1)) ? pathParts.Size() : (pathParts.Size() - 1);
		}
		for(uint i = 0; i < numSkipParts; i++) {
			{
