...
namespace NArchive {
	namespace NGz {
		...
		public:
			...
			AString Comment;
			// CByteBuffer Extra;
			CItem() : Flags(0), ExtraFlags(0), HostOS(0), Time(0), Crc(0), Size32(0) 
			{
			}
			void Clear()
			{
		...
		API_FUNC_static_IsArc IsArc_Gz(const Byte * p, size_t size)
		{
			if(size < 10)
				return k_IsArc_Res_NEED_MORE;
			if(p[0] != kSignature_0 || p[1] != kSignature_1 || p[2] != kSignature_2)
				return k_IsArc_Res_NO;
			Byte flags = p[3];
			...
			if(extraFlags != 0 && extraFlags != NExtraFlags::kMaximum && extraFlags != NExtraFlags::kFastest)
				return k_IsArc_Res_NO;
			size -= 10;
			p += 10;
	...
	HRESULT CItem::ReadHeader(NDecoder::CCOMCoder * stream)
	{
		...
		if(buf[0] != kSignature_0 || buf[1] != kSignature_1 || buf[2] != kSignature_2)
			return S_FALSE;
		Flags = buf[3];
		if(!IsSupported())
			return S_FALSE;
	...
	static const Byte kProps[] = { kpidPath, kpidSize, kpidPackSize, kpidMTime, kpidHostOS, kpidCRC /*,// kpidComment*/ };
	static const Byte kArcProps[] = { kpidHeadersSize, kpidNumStreams };
	...
	STDMETHODIMP CHandler::GetProperty(uint32 /* index */, PROPID propID, PROPVARIANT * value)
	{
		...
		switch(propID) {
			case kpidPath:
				...
				break;
			// case kpidComment: if(_item.CommentIsPresent()) prop = MultiByteToUnicodeString(_item.Comment,
			// CP_ACP); break;
			case kpidMTime:
				if(_item.Time != 0) {
					FILETIME utc;
	...
	STDMETHODIMP CHandler::Extract(const uint32 * indices, uint32 numItems, int32 testMode, IArchiveExtractCallback * extractCallback)
	{
		...
		try {
			for(;; ) {
				...
				if(item.Crc != outStreamSpec->GetCRC() || item.Size32 != (uint32)(unpackedSize - startOffset)) {
					crcError = true;
					result = S_FALSE;
					break;
				}
		...
		outStream.Release();
		int32 retResult = NExtractArc::NOperationResult::kDataError;
		if(!_isArc)
			retResult = NExtractArc::NOperationResult::kIsNotArc;
		else if(_needMoreInput)
			retResult = NExtractArc::NOperationResult::kUnexpectedEnd;
		else if(crcError)
			retResult = NExtractArc::NOperationResult::kCRCError;
		else if(_dataAfterEnd)
			retResult = NExtractArc::NOperationResult::kDataAfterEnd;
		else if(result == S_FALSE)
			retResult = NExtractArc::NOperationResult::kDataError;
		else if(result == S_OK)
			retResult = NExtractArc::NOperationResult::kOK;
		else
			return result;
	...
	#ifdef _WIN32
		static const Byte kHostOS = NHostOS::kFAT;
	#else
		static const Byte kHostOS = NHostOS::kUnix;
	#endif

	static HRESULT UpdateArchive(ISequentialOutStream * outStream, uint64 unpackSize, CItem & item, const CSingleMethodProps & props, IArchiveUpdateCallback * updateCallback)
	{
		uint64 complexity = 0;
		...
		item.ExtraFlags = props.GetLevel() >= 7 ? NExtraFlags::kMaximum : NExtraFlags::kFastest;
		item.HostOS = kHostOS;
	...
	REGISTER_ARC_IO("gzip", "gz gzip tgz tpz apk", "* * .tar .tar .tar", 0xEF, k_Signature, 0, NArcInfoFlags::kKeepName, IsArc_Gz)
	}
}
