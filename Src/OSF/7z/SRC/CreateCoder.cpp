...
static const uint kNumCodecsMax = 64;
...
static const uint kNumHashersMax = 16;
...
HRESULT CreateCoder(DECL_EXTERNAL_CODECS_LOC_VARS CMethodId methodId, bool encode, CCreatedCoder &cod)
{
	...
	HRESULT res = CreateCoder(EXTERNAL_CODECS_LOC_VARS methodId, encode, filter, cod);
	if(filter) {
		cod.IsFilter = true;
...
{
	CCreatedCoder cod;
	HRESULT res = CreateCoder(EXTERNAL_CODECS_LOC_VARS methodId, encode, cod);
	coder = cod.Coder;
	return res;
}
...
{
	CCreatedCoder cod;
	return CreateCoder(EXTERNAL_CODECS_LOC_VARS methodId, encode, filter, cod);
}
