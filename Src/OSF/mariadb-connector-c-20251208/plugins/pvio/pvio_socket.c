...
static int pvio_socket_init(char * unused1, size_t unused2, int unused3, va_list);
...
#else
	/* Windows doesn't support MSG_DONTWAIT, so we need to set socket to non blocking */
...
ssize_t pvio_socket_write(MARIADB_PVIO * pvio, const uchar * buffer, size_t length)
{
	...
	while((r = ma_send(csock->socket, (const uchar *)buffer, length, send_flags)) == -1) {
		int err = socket_errno;
		if((err != SOCKET_EAGAIN
#ifdef HAVE_SOCKET_EWOULDBLOCK
		    && err != SOCKET_EWOULDBLOCK
...
static int pvio_socket_internal_connect(MARIADB_PVIO * pvio, const struct sockaddr * name, size_t namelen)
{
	...
	do {
		...
		if(time(NULL) - start_t > (time_t)timeout/1000)
			break;
		/* CONC-612: Since usleep may fail and will set errno (On MacOSX usleep always sets errno=ETIMEDOUT), we need to save and restore errno */
		save_errno = errno;
		usleep(wait_conn);
		errno = save_errno;
...
/* todo: error, not supported */
		...
		if(cinfo->mysql->options.bind_address) {
			...
			while((gai_rc = getaddrinfo(cinfo->mysql->options.bind_address, 0,
				...
				uint timeout = mysql->options.connect_timeout ?
