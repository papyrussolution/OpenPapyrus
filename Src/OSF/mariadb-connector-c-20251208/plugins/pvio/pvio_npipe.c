...
/* Function prototypes */
bool pvio_npipe_set_timeout(MARIADB_PVIO * pvio, enum enum_pvio_timeout type, int timeout);
...
bool pvio_npipe_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo);
bool pvio_npipe_close(MARIADB_PVIO * pvio);
int pvio_npipe_fast_send(MARIADB_PVIO * pvio);
int pvio_npipe_keepalive(MARIADB_PVIO * pvio);
bool pvio_npipe_get_handle(MARIADB_PVIO * pvio, void * handle);
bool pvio_npipe_is_blocking(MARIADB_PVIO * pvio);
int pvio_npipe_shutdown(MARIADB_PVIO * pvio);
bool pvio_npipe_is_alive(MARIADB_PVIO * pvio);
...
bool pvio_npipe_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo)
{
	...
	if(cinfo->type == PVIO_TYPE_NAMEDPIPE) {
		...
		while(1) {
			if((cpipe->pipe = CreateFileA(szPipeName, GENERIC_READ|GENERIC_WRITE, 0/* no sharing */, NULL/* default security attributes */,
			    OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL)) != INVALID_HANDLE_VALUE)
				break;
			if(GetLastError() != ERROR_PIPE_BUSY) {
				pvio->set_error(pvio->mysql, CR_NAMEDPIPEOPEN_ERROR, "HY000", 0, cinfo->host, cinfo->unix_socket, GetLastError());
				goto end;
			}
			Sleep(backoff);
			if(!backoff)
			...
			if(wait_ms > INFINITE)
				wait_ms = INFINITE;
			if((wait_ms <= 0) || !WaitNamedPipeA(szPipeName, (DWORD)wait_ms)) {
				pvio->set_error(pvio->mysql, CR_NAMEDPIPEWAIT_ERROR, "HY000", 0, cinfo->host, cinfo->unix_socket, ERROR_TIMEOUT);
				goto end;
			}
		}
		if(!(cpipe->overlapped.hEvent = CreateEvent(NULL, FALSE, FALSE, NULL))) {
			pvio->set_error(pvio->mysql, CR_EVENT_CREATE_FAILED, "HY000", 0, GetLastError());
			goto end;
		}
		return 0;
	}
