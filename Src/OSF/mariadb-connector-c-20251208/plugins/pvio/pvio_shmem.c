...
bool pvio_shm_set_timeout(MARIADB_PVIO * pvio, enum enum_pvio_timeout type, int timeout);
...
int pvio_shm_wait_io_or_timeout(MARIADB_PVIO * pvio, bool is_read, int timeout);
int pvio_shm_blocking(MARIADB_PVIO * pvio, bool value, bool * old_value);
bool pvio_shm_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo);
bool pvio_shm_close(MARIADB_PVIO * pvio);
int pvio_shm_shutdown(MARIADB_PVIO * pvio);
bool pvio_shm_is_alive(MARIADB_PVIO * pvio);
bool pvio_shm_get_handle(MARIADB_PVIO * pvio, void * handle);
...
int pvio_shm_wait_io_or_timeout(MARIADB_PVIO * pvio, bool is_read, int timeout) { return 0; }
int pvio_shm_blocking(MARIADB_PVIO * pvio, bool block, bool * previous_mode) { return 0; /* not supported */ }
int pvio_shm_keepalive(MARIADB_PVIO * pvio) { return 0; /* not supported */ }
int pvio_shm_fast_send(MARIADB_PVIO * pvio) { return 0; /* not supported */ }
...
bool pvio_shm_connect(MARIADB_PVIO * pvio, MA_PVIO_CINFO * cinfo)
{
	...
	HANDLE hdlConnectRequestAnswer = NULL;
	...
	if(!hdlConnectRequest) {
		PVIO_SET_ERROR(cinfo->mysql, CR_SHARED_MEMORY_CONNECT_ERROR, "HY000", 0,
		    "Opening CONNECT_REQUEST event failed", GetLastError());
		goto error;
	}
	strcpy(shm_suffix, "CONNECT_ANSWER");
	if(!(hdlConnectRequestAnswer = OpenEventA(dwDesiredAccess, 0, shm_name))) {
		PVIO_SET_ERROR(cinfo->mysql, CR_SHARED_MEMORY_CONNECT_ERROR, "HY000",
		    0, "Opening CONNECT_ANSWER event failed", GetLastError());
		goto error;
	}
	/* get connection id, so we can build the filename used for connection */
	strcpy(shm_suffix, "CONNECT_DATA");
	if(!(file_map = OpenFileMappingA(FILE_MAP_WRITE, 0, shm_name))) {
	...
	if(!SetEvent(hdlConnectRequest)) {
		PVIO_SET_ERROR(cinfo->mysql, CR_SHARED_MEMORY_CONNECT_ERROR, "HY000", 0, "Failed sending connection request",
		    GetLastError());
		goto error;
	}
	...
	pvio_shm->file_map = OpenFileMappingA(FILE_MAP_WRITE, 0, shm_name);
	if(pvio_shm->file_map == NULL) {
		PVIO_SET_ERROR(cinfo->mysql, CR_SHARED_MEMORY_CONNECT_ERROR, "HY000", 0, "OpenFileMapping failed", GetLastError());
		goto error;
	}
	if(!(pvio_shm->map = (LPVOID *)MapViewOfFile(pvio_shm->file_map, FILE_MAP_WRITE, 0, 0, PVIO_SHM_BUFFER_SIZE))) { // @sobolev (LPVOID *)
...
bool pvio_shm_get_socket(MARIADB_PVIO * pvio, void * handle)
{
	return 1;
}

bool pvio_shm_is_blocking(MARIADB_PVIO * pvio)
{
	return 1;
}
