...
static int native_password_auth_client(MYSQL_PLUGIN_VIO * vio, MYSQL * mysql)
{
	...
	if(((MCPVIO_EXT*)vio)->mysql_change_user) {
		// in mysql_change_user() the client sends the first packet. we use the old scramble.
	...
	if(mysql && mysql->passwd[0]) {
		...
		memzero(scrambled, SCRAMBLE_LENGTH + 1);
...
static int dummy_fallback_auth_client(MYSQL_PLUGIN_VIO * vio, MYSQL * mysql __attribute__((unused)))
{
	...
	if(last_errno) {
		...
		memcpy(mysql->net.last_error, last_error, sizeof(mysql->net.last_error) - 1);
...
static int send_client_reply_packet(MCPVIO_EXT * mpvio, const uchar * data, int data_len)
{
	...
	if(mysql->options.ssl_key || mysql->options.ssl_cert || mysql->options.ssl_ca || mysql->options.ssl_capath ||
	    mysql->options.ssl_cipher || mysql->options.use_ssl || mysql->options.extension->tls_verify_server_cert)
	...
	/* CONC-635: For connections via named pipe or shared memory the server
		indicates the capability for secure connections (TLS), but doesn't support it. */
	if((mysql->server_capabilities & CLIENT_SSL) && (mysql->net.pvio->type == PVIO_TYPE_NAMEDPIPE || mysql->net.pvio->type == PVIO_TYPE_SHAREDMEM)) {
	...
	if(mysql->options.use_ssl && !(mysql->server_capabilities & CLIENT_SSL)) {
		if(mysql->options.extension->tls_verify_server_cert || (mysql->options.extension && (mysql->options.extension->tls_fp ||
		    mysql->options.extension->tls_fp_list))) {
			my_set_error(mysql, CR_SSL_CONNECTION_ERROR, SQLSTATE_UNKNOWN, ER(CR_SSL_CONNECTION_ERROR),
	...
	if(mysql->client_flag & CLIENT_PROTOCOL_41) {
		...
		if(!(mysql->server_capabilities & CLIENT_MYSQL)) {
			...
			mysql->extension->mariadb_client_flag = server_extended_cap & client_extended_cap;
