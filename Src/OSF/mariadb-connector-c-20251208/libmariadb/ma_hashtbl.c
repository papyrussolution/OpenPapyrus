...
bool _ma_hashtbl_init(MA_HASHTBL * hash, uint size, uint key_offset, uint key_length, hash_get_key get_key, void (*free_element)(void*), uint flags CALLER_INFO_PROTO)
{
	hash->records = 0;
...
static inline char* hash_key(MA_HASHTBL * hash, const uchar * record, uint * length, bool first)
{
	if(hash->get_key)
...
static uint hash_rec_mask(MA_HASHTBL * hash, MA_HASHTBL_LINK * pos, uint buffmax, uint maxlength)
{
	uint length;
...
static inline
#endif
uint rec_hashnr(MA_HASHTBL * hash, const uchar * record)
{
	uint length;
...
bool ma_hashtbl_insert(MA_HASHTBL * info, const uchar * record)
{
	int flag;
