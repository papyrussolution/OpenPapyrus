...
bool ma_tls_connect(MARIADB_TLS * ctls)
{
	...
	sRet = AcquireCredentialsHandleA(NULL, const_cast<char *>(UNISP_NAME_A), SECPKG_CRED_OUTBOUND, NULL, &Cred, NULL, NULL, &sctx->CredHdl, NULL);
	...
	if(ma_schannel_client_handshake(ctls) != SEC_E_OK)
		goto end;
	verify_certs =  mysql->options.ssl_ca || mysql->options.ssl_capath || (mysql->options.extension->tls_verify_server_cert);
	if(verify_certs) {
...
{
	for(size_t i = 0; i < sizeof(openssl_ciphers)/sizeof(openssl_ciphers[0]); i++) {
