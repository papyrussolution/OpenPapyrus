...
#include <winsock2.h>
#include <ws2tcpip.h>
// @sobolev @v12.5.0 #include <winhttp.h>
...
SECURITY_STATUS add_dir_to_store(HCERTSTORE trust_store, const char* dir, PEM_TYPE type, char* errmsg, size_t errmsg_len)
{
	...
	SECURITY_STATUS status = SEC_E_OK;
	if((dwAttr = GetFileAttributesA(dir)) == INVALID_FILE_ATTRIBUTES) {
		SetLastError(SEC_E_INVALID_PARAMETER);
 ...
 */
SECURITY_STATUS schannel_create_store(const char* CAFile,
    const char* CAPath,
    const char* CRLFile,
    const char* CRLPath,
...
/*
   ...
   and chain engine.
 */
static SECURITY_STATUS VerifyServerCertificate(PCCERT_CONTEXT pServerCert, HCERTSTORE hStore,
    LPWSTR pwszServerName, DWORD dwRevocationCheckFlags, DWORD dwVerifyFlags, LPSTR errmsg, size_t errmsg_len)
{
    ...
    const char* server_name, BOOL check_server_name, char * errmsg, size_t errmsg_len)
{
	...
	status = VerifyServerCertificate(cert, store, wserver_name ? wserver_name : const_cast<wchar_t *>(L"SERVER_NAME"), dwRevocationFlags, dwVerifyFlags, errmsg, errmsg_len);
cleanup:
...
static SECURITY_STATUS load_private_key(client_cert_handle * cert_handle, char * private_key_str,
    size_t len, char * errmsg, size_t errmsg_len)
{
	...
	DWORD pki_len = 0;
	SECURITY_STATUS status = SEC_E_OK;
	BYTE * derbuf = (BYTE *)LocalAlloc(0, derlen);
	if(!derbuf) {
		FAIL("LocalAlloc failed");
	}
	 ...
	 */
	if(CryptDecodeObjectEx(X509_ASN_ENCODING, PKCS_PRIVATE_KEY_INFO, derbuf, derlen, CRYPT_DECODE_ALLOC_FLAG, NULL, &pki, &pki_len)) {
		...
		if(!CryptDecodeObjectEx(X509_ASN_ENCODING|PKCS_7_ASN_ENCODING, PKCS_RSA_PRIVATE_KEY,
			    pki->PrivateKey.pbData, pki->PrivateKey.cbData, CRYPT_DECODE_ALLOC_FLAG, NULL, &keyblob, &keyblob_len)) {
			FAIL("Failed to parse private key");
		}
	}
	else if(!CryptDecodeObjectEx(X509_ASN_ENCODING|PKCS_7_ASN_ENCODING, PKCS_RSA_PRIVATE_KEY, derbuf, derlen, CRYPT_DECODE_ALLOC_FLAG, NULL, &keyblob, &keyblob_len)) {
		FAIL("Failed to parse private key");
	}
	if(!crypt_acquire_context(cert_handle)) {
		FAIL("CryptAcquireContext failed");
	}
	if(!CryptImportKey(cert_handle->prov, keyblob, keyblob_len, 0, 0, &cert_handle->key)) {
 ...
 */
static SECURITY_STATUS create_client_certificate_mem(char* cert_file_content, char* key_file_content,
    client_cert_handle* cert_handle, char* errmsg, size_t errmsg_len)
{
...
SECURITY_STATUS schannel_create_cert_context(char* cert_file, char* key_file, client_cert_handle * cert_handle, char* errmsg, size_t errmsg_len)
{
	char* key_file_content = NULL;
	char* cert_file_content = NULL;
	SECURITY_STATUS status = SEC_E_INTERNAL_ERROR;
	memzero(cert_handle, sizeof(client_cert_handle));
