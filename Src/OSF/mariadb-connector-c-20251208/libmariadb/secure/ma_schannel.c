...
SECURITY_STATUS ma_schannel_handshake_loop(MARIADB_PVIO * pvio, bool InitialRead, SecBuffer * pExtraData)
{
	...
	if(!(IoBuffer = (PUCHAR)SAlloc::M(SC_IO_BUFFER_SIZE)))
	...
	while(rc == SEC_I_CONTINUE_NEEDED || rc == SEC_E_INCOMPLETE_MESSAGE || rc == SEC_I_INCOMPLETE_CREDENTIALS) {
		/* Read data */
...
SECURITY_STATUS ma_schannel_client_handshake(MARIADB_TLS * ctls)
{
	...
	sRet = InitializeSecurityContextA(&sctx->CredHdl, NULL, pvio->mysql->host,
		SFlags, 0, SECURITY_NATIVE_DREP, NULL, 0, &sctx->hCtxt, &BufferOut, &OutFlags, NULL);
