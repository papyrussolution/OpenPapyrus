...
MARIADB_PVIO * ma_pvio_init(MA_PVIO_CINFO * cinfo)
{
	...
	pvio_plugin = (MARIADB_PVIO_PLUGIN *)mysql_client_find_plugin(cinfo->mysql, pvio_plugins[type], MARIADB_CLIENT_PVIO_PLUGIN);
	if(!pvio_plugin) {
		/* error already set in mysql_client_find_plugin */
		return NULL;
	}
	/* coverity[var_deref_op] */
	pvio = (MARIADB_PVIO*)SAlloc::C(1, sizeof(MARIADB_PVIO));
	if(!pvio) {
...
ssize_t ma_pvio_read(MARIADB_PVIO * pvio, uchar * buffer, size_t length)
{
	...
	if(pvio_callback) {
		...
		while(p) {
			callback = (void (*)(int, MYSQL *, const uchar *, size_t))p->data;
...
/* {{{ ma_pvio_register_callback */
int ma_pvio_register_callback(bool register_callback, void (* callback_function)(int mode, MYSQL * mysql, const uchar * buffer, size_t length))
{
	LIST * list;
