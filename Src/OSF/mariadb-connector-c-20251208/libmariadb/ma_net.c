...
int ma_net_init(NET * net, MARIADB_PVIO* pvio)
{
	...
	if(!net->extension)
		return 1;
	memzero(net->buff, net_buffer_length);
...
int ma_net_real_write(NET * net, const char * packet, size_t len)
{
	...
	if(net->compress) {
		...
		if(!(b = (uchar*)SAlloc::M(len + NET_HEADER_SIZE + COMP_HEADER_SIZE + 1))) {
	...
	while(pos != end) {
		if((length = ma_pvio_write(net->pvio, (uchar*)pos, (size_t)(end-pos))) <= 0) {
			...
			net->pvio->set_error(net->pvio->mysql, CR_ERR_NET_WRITE, SQLSTATE_UNKNOWN, 0, errmsg, save_errno);
			net->reading_or_writing = 0;
#ifdef HAVE_COMPRESS
			if(net->compress)
