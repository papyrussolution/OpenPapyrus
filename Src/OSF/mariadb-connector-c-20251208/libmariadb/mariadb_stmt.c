...
#define UPDATE_STMT_ERROR(stmt) stmt_set_error((stmt), (stmt)->mysql->net.last_errno, (stmt)->mysql->net.sqlstate, (stmt)->mysql->net.last_error)
...
static bool net_stmt_close(MYSQL_STMT * stmt, bool remove);

static bool is_not_null = 0;
static bool is_null = 1;

void stmt_set_error(MYSQL_STMT * stmt, uint error_nr, const char * sqlstate, const char * format, ...)
{
	va_list ap;
...
bool mthd_supported_buffer_type(enum enum_field_types type)
{
	switch(type) {
		case MYSQL_TYPE_BIT:
...
static bool madb_reset_stmt(MYSQL_STMT * stmt, uint flags);
						...
						if(mysql_ps_fetch_functions[stmt->fields[i].type].pack_len < 0) {
							...
							switch(stmt->fields[i].type) {
								...
								case MYSQL_TYPE_TIMESTAMP:
									...
									mysql_ps_fetch_functions[stmt->fields[i].type].max_len;
								    break;
								default:
							...
							if(stmt->fields[i].flags & ZEROFILL_FLAG) {
								...
								if(stmt->fields[i].flags & UNSIGNED_FLAG &&
									...
									/*
									   Unsigned integers has one character less than
									      signed integers
...
int mthd_stmt_fetch_to_bind(MYSQL_STMT * stmt, uchar * row)
{
	uint i;
			...
			if(!stmt->bind_result_done ||
				...
				*stmt->bind[i].is_null = 0;
				...
				if(stmt->mysql->options.report_data_truncation)
					truncations += *stmt->bind[i].P_Error;
			}
		}
		if(!((bit_offset <<= 1) & 255)) {
			bit_offset = 1; /* To next byte */
...
{
	if(!MARIADB_STMT_BULK_SUPPORTED(stmt) || !stmt->array_size || !stmt->params[param_nr].u.indicator)
		return 0;
	if(stmt->param_callback)
...
uchar* mysql_stmt_execute_generate_simple_request(MYSQL_STMT * stmt, size_t * request_len)
{
	...
	if(stmt->param_count) {
		...
		if(null_count + 20 > free_bytes) {
			...
			if(!(tmp_start = (uchar*)SAlloc::R(start, length)))
				goto mem_error;
			start = tmp_start;
			p = start + offset;
		}
		null_byte_offset = p - start;
		memzero(p, null_count);
		p += null_count;
		...
		for(i = 0; i < stmt->param_count; i++) {
			...
			if(((stmt->params[i].is_null && *stmt->params[i].is_null) || stmt->params[i].buffer_type == MYSQL_TYPE_NULL || !stmt->params[i].buffer)) {
				has_data = FALSE;
				(start + null_byte_offset)[i/8] |= (uchar)(1 << (i & 7));
			}
			if(has_data) {
...
bool mysql_stmt_skip_paramset(MYSQL_STMT * stmt, uint row)
{
						...
						case MYSQL_TYPE_SET:
						    ...
						    if(!stmt->param_callback) {
								...
								(!stmt->row_size && ma_get_length(stmt, i, j) == -1)) {
								    size += strlen((const char *)ma_get_buffer_offset(stmt, stmt->params[i].buffer_type, stmt->params[i].buffer, j));
							    }
							    else
 ...
 *******************************************************************************
   ...
   \fn        uint64 mysql_stmt_affected_rows
...
bool STDCALL mysql_stmt_attr_set(MYSQL_STMT * stmt, enum enum_stmt_attr_type attr_type, const void * value)
{
	switch(attr_type) {
		case STMT_ATTR_UPDATE_MAX_LENGTH:
		    stmt->update_max_length = *(bool*)value;
		    break;
		case STMT_ATTR_CURSOR_TYPE:
		    if(*(ulong*)value > (ulong)CURSOR_TYPE_READ_ONLY) {
...
bool STDCALL mysql_stmt_bind_param(MYSQL_STMT * stmt, MYSQL_BIND * bind)
{
	MYSQL * mysql = stmt->mysql;
	if(!mysql) {
	...
	if((stmt->state < MYSQL_STMT_PREPARED || stmt->state >= MYSQL_STMT_EXECUTED) && stmt->prebind_params > 0) {
		if(!stmt->params && stmt->prebind_params) {
			...
			memzero(stmt->params, stmt->prebind_params * sizeof(MYSQL_BIND));
		}
		stmt->param_count = stmt->prebind_params;
	}
	...
	if(stmt->param_count && bind) {
		...
		for(uint i = 0; i < stmt->param_count; i++) {
			if(stmt->mysql->methods->db_supported_buffer_type && !stmt->mysql->methods->db_supported_buffer_type(stmt->params[i].buffer_type)) {
				stmt_set_error(stmt, CR_UNSUPPORTED_PARAM_TYPE, SQLSTATE_UNKNOWN, 0);
				return 1;
			}
...
bool STDCALL mysql_stmt_bind_result(MYSQL_STMT * stmt, MYSQL_BIND * bind)
{
	uint i;
	...
	if(!bind)
		return 1;
	// In case of a stored procedure we don't allocate memory for bind in mysql_stmt_prepare
...
int STDCALL mysql_stmt_fetch_column(MYSQL_STMT * stmt, MYSQL_BIND * bind, uint column, ulong offset)
{
	...
	if(!stmt->bind[column].u.row_ptr) {
		...
		if(!bind[0].P_Error)
			bind[0].P_Error = &bind[0].error_value;
		*bind[0].P_Error = 0;
...
{
	return madb_reset_stmt(stmt, MADB_RESET_LONGDATA|MADB_RESET_STORED|MADB_RESET_BUFFER|MADB_RESET_ERROR);
}
...
int STDCALL mysql_stmt_prepare(MYSQL_STMT * stmt, const char * query, ulong length)
{
	...
	CLEAR_CLIENT_STMT_ERROR(stmt);
	CLEAR_CLIENT_ERROR(stmt->mysql);
	stmt->upsert_status.affected_rows = mysql->affected_rows = (uint64)~0;
	...
	if(stmt->param_count) {
		if(stmt->prebind_params) {
			...
			memzero(stmt->params, stmt->param_count * sizeof(MYSQL_BIND));
		}
	}
...
static int madb_alloc_stmt_fields(MYSQL_STMT * stmt)
{
	...
	if(stmt->mysql->field_count) {
		...
		for(i = 0; i < stmt->field_count; i++) {
			...
			stmt->fields[i].extension = stmt->mysql->fields[i].extension ? ma_field_extension_deep_dup(
				fields_ma_alloc_root, (const MA_FIELD_EXTENSION *)stmt->mysql->fields[i].extension) : NULL;
		}
		if(!(stmt->bind = (MYSQL_BIND*)ma_alloc_root(fields_ma_alloc_root, stmt->field_count * sizeof(MYSQL_BIND)))) {
			stmt_set_error(stmt, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
			return 1;
		}
		memzero(stmt->bind, stmt->field_count * sizeof(MYSQL_BIND));
...
int stmt_read_execute_response(MYSQL_STMT * stmt)
{
	...
	if(!mysql)
		return 1;
	ret = test((mysql->methods->db_read_stmt_result && mysql->methods->db_read_stmt_result(mysql)));
	...
	if(mysql->field_count) {
		if(!stmt->field_count || mysql->server_status & SERVER_MORE_RESULTS_EXIST) { /* fix for ps_bug: test_misc */
			MA_MEM_ROOT * fields_ma_alloc_root = &((MADB_STMT_EXTENSION*)stmt->extension)->fields_ma_alloc_root;
			uint i;
			ma_free_root(fields_ma_alloc_root, MYF(0));
			if(!(stmt->bind = (MYSQL_BIND*)ma_alloc_root(fields_ma_alloc_root, sizeof(MYSQL_BIND) * mysql->field_count)) ||
			    !(stmt->fields = (MYSQL_FIELD*)ma_alloc_root(fields_ma_alloc_root, sizeof(MYSQL_FIELD) * mysql->field_count))) {
				stmt_set_error(stmt, CR_OUT_OF_MEMORY, SQLSTATE_UNKNOWN, 0);
				return 1;
			}
			memzero(stmt->bind, sizeof(MYSQL_BIND) * mysql->field_count);
			...
			for(i = 0; i < stmt->field_count; i++) {
				...
				stmt->fields[i].extension = mysql->fields[i].extension ? ma_field_extension_deep_dup(fields_ma_alloc_root, (const MA_FIELD_EXTENSION *)mysql->fields[i].extension) : NULL;
			}
		}
    ...
    const char * data, ulong length)
{
	...
	if(length || !stmt->params[param_number].long_data_used) {
		...
		uchar * cmd_buff = (uchar*)SAlloc::C(1, packet_len);
    ...
    size_t length)
{
	...
	if(emulate_cmd) {
		...
		if((rc = mysql_stmt_prepare(stmt, stmt_str, (ulong)length)))
			return rc;
