    ...
    uchar ** row, uint byte_count)
{
	...
	switch(byte_count) {
		case 1:
		    ...
		    *r_param->P_Error = (is_unsigned != r_param->is_unsigned && *(uchar*)r_param->buffer > INT_MAX8);
		    break;
		case 2:
		    shortstore(r_param->buffer, ((ushort)sint2korr(*row)));
		    *r_param->P_Error = (is_unsigned != r_param->is_unsigned && *(ushort*)r_param->buffer > INT_MAX16);
		    break;
		case 4:
	    {
		    longstore(r_param->buffer, ((uint32)sint4korr(*row)));
		    *r_param->P_Error = (is_unsigned != r_param->is_unsigned && *(uint32*)r_param->buffer > INT_MAX32);
	    }
	    break;
		case 8:
	    {
		    ...
		    *r_param->P_Error = (is_unsigned != r_param->is_unsigned && val > LONGLONG_MAX);
	    }
	    break;
		default:
...
static void convert_froma_string(MYSQL_BIND * r_param, char * buffer, size_t len)
{
	...
	switch(r_param->buffer_type) {
		case MYSQL_TYPE_TINY:
	    {
		    ...
		    *r_param->P_Error = error ? 1 : (r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX8) : (NUMERIC_TRUNCATION(val, INT_MIN8, INT_MAX8) || error > 0));
	    ...
	    {
		    longlong val = my_atoll(buffer, buffer + len, &error);
		    *r_param->P_Error = error ? 1 : (r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX16) : (NUMERIC_TRUNCATION(val, INT_MIN16, INT_MAX16) || error > 0));
	    ...
	    {
		    longlong val = my_atoll(buffer, buffer + len, &error);
		    *r_param->P_Error = error ? 1 : r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX32) : NUMERIC_TRUNCATION(val, INT_MIN32, INT_MAX32) || error > 0;
		...
		default:
	    {
		    if(len >= r_param->offset) {
			    ...
			    *r_param->P_Error = (copylen > r_param->buffer_length);
		    }
...
static void convert_from_long(MYSQL_BIND * r_param, const MYSQL_FIELD * field, longlong val, bool is_unsigned)
{
	switch(r_param->buffer_type) {
		case MYSQL_TYPE_TINY:
		    ...
		    *r_param->P_Error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX8) : NUMERIC_TRUNCATION(val, INT_MIN8, INT_MAX8);
		    r_param->buffer_length = 1;
		    break;
		case MYSQL_TYPE_SHORT:
		case MYSQL_TYPE_YEAR:
		    shortstore(r_param->buffer, (short)(val & 0xffff));
		    *r_param->P_Error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX16) : NUMERIC_TRUNCATION(val, INT_MIN16, INT_MAX16);
		    r_param->buffer_length = 2;
		    break;
		case MYSQL_TYPE_LONG:
		    longstore(r_param->buffer, (int32)(val & 0xffffffff));
		    *r_param->P_Error = r_param->is_unsigned ? NUMERIC_TRUNCATION(val, 0, UINT_MAX32) : NUMERIC_TRUNCATION(val, INT_MIN32, INT_MAX32);
		    r_param->buffer_length = 4;
		    break;
		case MYSQL_TYPE_LONGLONG:
		    *r_param->P_Error = (val < 0 && r_param->is_unsigned != is_unsigned);
		    longlongstore(r_param->buffer, val);
		...
		case MYSQL_TYPE_DOUBLE:
	    {
		    ...
		    *r_param->P_Error = (dbl != ceil(dbl)) || (is_unsigned ? (ulonglong)dbl != (ulonglong)val : (longlong)dbl != (longlong)val);
		    r_param->buffer_length = 8;
		    break;
	    }
		case MYSQL_TYPE_FLOAT:
	    {
		    ...
		    *r_param->P_Error = (fval != ceilf(fval)) || (is_unsigned ? (ulonglong)fval != (ulonglong)val : (longlong)fval != val);
		    r_param->buffer_length = 4;
	    }
	    break;
		default:
	    {
		    char * endptr;
		    uint len;
		    /*bool*/int zf_truncated = 0;
		    char * buffer = (char *)alloca(MAX(field->length, 22));
...
static void convert_from_float(MYSQL_BIND * r_param, const MYSQL_FIELD * field, float val, int size __attribute__((unused)))
{
	...
	switch(r_param->buffer_type)
	{
		case MYSQL_TYPE_TINY:
		    ...
		    *r_param->P_Error = check_trunc_val != (r_param->is_unsigned ? (double)((uint8)*buf) : (double)((int8)*buf));
		    r_param->buffer_length = 1;
		    break;
		case MYSQL_TYPE_SHORT:
		case MYSQL_TYPE_YEAR:
	    {
		    if(r_param->is_unsigned) {
			    ...
			    shortstore(buf, sval);
			    *r_param->P_Error = check_trunc_val != (double)sval;
		    }
		    else {
		...
		case MYSQL_TYPE_LONG:
	    {
		    if(r_param->is_unsigned) {
			    ...
			    longstore(buf, lval);
			    *r_param->P_Error = (check_trunc_val != (double)lval);
		    }
		    else {
		...
		case MYSQL_TYPE_LONGLONG:
	    {
		    if(r_param->is_unsigned) {
			    ...
			    *r_param->P_Error = (check_trunc_val != (double)llval);
		    }
		    else {
...
static void convert_from_double(MYSQL_BIND * r_param, const MYSQL_FIELD * field, double val, int size __attribute__((unused)))
{
	...
	switch(r_param->buffer_type)
	{
		...
		case MYSQL_TYPE_FLOAT:
	    {
		    ...
		    *r_param->P_Error = (*(float*)buf != fval);
		    r_param->buffer_length = 4;
	    }
	    break;
		default:
...
/* {{{ ps_fetch_double */
static
void ps_fetch_double(MYSQL_BIND * r_param, const MYSQL_FIELD * field, uchar ** row)
{
	switch(r_param->buffer_type) {
		case MYSQL_TYPE_DOUBLE:
	    {
...
/* {{{ ps_fetch_float */
static
void ps_fetch_float(MYSQL_BIND * r_param, const MYSQL_FIELD * field, uchar ** row)
{
	switch(r_param->buffer_type) {
		case MYSQL_TYPE_FLOAT:
	    {
		    ...
		    switch(field->type) {
				...
				if(field->decimals && (field->decimals <= SEC_PART_DIGITS ||
					...
					length = sprintf(dtbuffer,
						...
						tm.second,
						decimals,
						(uint32_t)(tm.second_part / sec_part_digits[decimals]));
				}
				else
