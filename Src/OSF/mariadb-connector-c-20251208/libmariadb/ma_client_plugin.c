...
typedef struct st_mysql_client_plugin_AUTHENTICATION auth_plugin_t; // @sobolev
...
extern /*struct st_mysql_client_plugin*/auth_plugin_t mysql_native_password_client_plugin;
extern /*struct st_mysql_client_plugin*/st_mysql_client_plugin_AUTHENTICATION mysql_old_password_client_plugin;
extern /*struct st_mysql_client_plugin*/MARIADB_PVIO_PLUGIN pvio_socket_client_plugin;
...
static int is_not_initialized(MYSQL * mysql, const char * name)
{
	if(initialized)
		return 0;
	my_set_error(mysql, CR_AUTH_PLUGIN_CANNOT_LOAD, SQLSTATE_UNKNOWN, ER(CR_AUTH_PLUGIN_CANNOT_LOAD), name, "not initialized");
	return 1;
}
...
{
	if(plugin->interface_version < version || (plugin->interface_version >> 8) > (version >> 8))
 ...
 */
static struct st_mysql_client_plugin * add_plugin(MYSQL * mysql, struct st_mysql_client_plugin * plugin, void * dlhandle, int argc, va_list args)
{
	const char * errmsg;
	struct st_client_plugin_int plugin_int;
	struct st_client_plugin_int * p;
	char errbuf[1024];
	int plugin_nr;
	DBUG_ASSERT(initialized);
	...
	plugin_nr = get_plugin_nr(plugin->type);
	if(plugin_nr == -1) {
...
static void load_env_plugins(MYSQL * mysql)
{
	char * plugs;
	char * free_env;
	char * s = getenv("LIBMYSQL_PLUGINS");
...
int mysql_client_plugin_init()
{
	...
	if(!initialized) {
		memzero(&mysql, sizeof(mysql)); /* dummy mysql for set_mysql_extended_error */
		pthread_mutex_init(&LOCK_load_client_plugin, NULL);
		ma_init_alloc_root(&mem_root, 128, 128);
		memzero(&plugin_list, sizeof(plugin_list));
		initialized = 1;
...
struct st_mysql_client_plugin * STDCALL mysql_client_register_plugin(MYSQL * mysql, struct st_mysql_client_plugin * plugin)
{
...
/* see <mysql/client_plugin.h> for a full description */
struct st_mysql_client_plugin * STDCALL mysql_load_plugin_v(MYSQL * mysql, const char * name, int type, int argc, va_list args)
{
	const char * errmsg;
#ifdef _WIN32
	char errbuf[1024];
...
err:
	if(dlhandle)
		dlclose(dlhandle);
	pthread_mutex_unlock(&LOCK_load_client_plugin);
	my_set_error(mysql, CR_AUTH_PLUGIN_CANNOT_LOAD, SQLSTATE_UNKNOWN, ER(CR_AUTH_PLUGIN_CANNOT_LOAD), name, errmsg);
	return NULL;
}

/* see <mysql/client_plugin.h> for a full description */
struct st_mysql_client_plugin * STDCALL mysql_load_plugin(MYSQL * mysql, const char * name, int type, int argc, ...) 
{
...
/* see <mysql/client_plugin.h> for a full description */
struct st_mysql_client_plugin * STDCALL mysql_client_find_plugin(MYSQL * mysql, const char * name, int type) 
{
	struct st_mysql_client_plugin * p;
	int plugin_nr = get_plugin_nr(type);
	if(is_not_initialized(mysql, name))
		return NULL;
	if(plugin_nr == -1) {
		my_set_error(mysql, CR_AUTH_PLUGIN_CANNOT_LOAD, SQLSTATE_UNKNOWN, ER(CR_AUTH_PLUGIN_CANNOT_LOAD), name, "invalid type");
	}
