...
MARIADB_RPL_EVENT * STDCALL mariadb_rpl_fetch(MARIADB_RPL * rpl, MARIADB_RPL_EVENT * event)
{
	...
	while(1) {
		...
		rpl_event->event_type = /*(uchar)*/(mariadb_rpl_event)*(rpl->buffer + 5);
		...
		switch(rpl_event->event_type) {
			...
			case ANNOTATE_ROWS_EVENT:
			    len = (uint32)(rpl->buffer + rpl->buffer_size - (uchar *)ev - (rpl->use_checksum ? 4 : 0));
			...
			case DELETE_ROWS_EVENT_V1:
			    ...
			    if(!len)
				    break;
			    if(!(rpl_event->event.rows.column_bitmap = (char*)ma_alloc_root(&rpl_event->memroot, (len + 7) / 8)))
				    goto mem_error;
			    memcpy(rpl_event->event.rows.column_bitmap, ev, (len + 7) / 8);
			    ev += (len + 7) / 8;
			    if(rpl_event->event_type == UPDATE_ROWS_EVENT_V1) {
				    if(!(rpl_event->event.rows.column_update_bitmap = (char*)ma_alloc_root(&rpl_event->memroot, (len + 7) / 8)))
					    goto mem_error;
