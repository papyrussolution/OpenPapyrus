...
static CURLcode ftp_state_use_port(struct Curl_easy * data, /*ftpport*/int fcmd/* start with this */)
{
...
static CURLcode ftp_state_port_resp(struct Curl_easy * data, int ftpcode)
{
	...
	CURLcode result = CURLE_OK;
	// The FTP spec tells a positive response should have code 200. Be more permissive here to tolerate deviant servers.
	if(ftpcode / 100 != 2) {
		// the command failed
		if(EPRT == fcmd) {
		...
		if(fcmd == DONE) {
			...
			result = ftp_state_use_port(data, fcmd); // try next
	}
	else {
...
static void wc_data_dtor(void * ptr)
{
	struct ftp_wc * ftpwc = (ftp_wc *)ptr;
...
static CURLcode init_wc_data(struct Curl_easy * data)
{
	...
	ftpwc = (ftp_wc *)calloc(1, sizeof(struct ftp_wc));
	if(!ftpwc) {
		result = CURLE_OUT_OF_MEMORY;
		goto fail;
	}
	...
	ftpwc->backup.file_descriptor = (FILE *)data->set.out;
		...
		case FTPFILE_MULTICWD: {
		    ...
		    if(dirAlloc) {
			    ...
			    while((slashPos = strchr(curPos, '/'))) {
				    ...
				    if(compLen > 0) {
					    char * comp = static_cast<char *>(calloc(1, compLen + 1));
					    if(!comp) {
						    free(rawPath);
						    return CURLE_OUT_OF_MEMORY;
					    }
...
static CURLcode ftp_setup_connection(struct Curl_easy * data, struct connectdata * conn)
{
	...
	CURLcode result = CURLE_OK;
	struct ftp_conn * ftpc = &conn->proto.ftpc;
	struct FTP * ftp = (FTP *)calloc(sizeof(struct FTP), 1);
	if(!ftp)
		return CURLE_OUT_OF_MEMORY;
