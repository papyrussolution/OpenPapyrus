 ...
 */
	...
	#define vqualifier volatile // workaround icc 9.1 optimizer issue 
#else
	#define vqualifier
#endif
	...
	*result = NULL; /* assume failure */
	...
	for(ai = aihead; ai != NULL; ai = ai->ai_next) {
		...
		if((size_t)ai->ai_addrlen < ss_size)
			continue;
		ca = (Curl_addrinfo *)SAlloc::M(sizeof(struct Curl_addrinfo) + ss_size + namelen);
		if(!ca) {
			error = EAI_MEMORY;
			break;
		}
		...
		ca->ai_next      = NULL;
		ca->ai_addr = (sockaddr *)((char *)ca + sizeof(struct Curl_addrinfo));
...
struct Curl_addrinfo *Curl_he2ai(const struct hostent * he, int port)                       {
	...
	for(i = 0; (curr = he->h_addr_list[i]) != NULL; i++) {
		...
		ai = (Curl_addrinfo *)SAlloc::C(1, sizeof(struct Curl_addrinfo) + ss_size + namelen);
		if(!ai) {
...
struct Curl_addrinfo *Curl_unix2addr(const char * path, bool * longpath, bool abstract)
{
	...
	size_t path_len;
	*longpath = FALSE;
	ai = (Curl_addrinfo *)SAlloc::C(1, sizeof(struct Curl_addrinfo) + sizeof(struct sockaddr_un));
	if(!ai)
		return NULL;
	ai->ai_addr = (sockaddr *)((char *)ai + sizeof(struct Curl_addrinfo));
