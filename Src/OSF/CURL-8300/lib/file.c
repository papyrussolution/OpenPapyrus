...
static CURLcode file_do(struct Curl_easy * data, bool * done)
{
	 ...
	 */
	CURLcode result = CURLE_OK;
	struct_stat statbuf; // struct_stat instead of struct stat just to allow the Windows version to have a different struct without
	...
	*done = TRUE; /* unconditionally */
	...
	if(fstated) {
		...
		if(expected_size >= 0) {
			headerlen = msnprintf(header, sizeof(header), "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", expected_size);
			result = Curl_client_write(data, CLIENTWRITE_HEADER, header, headerlen);
			if(result)
				return result;
			result = Curl_client_write(data, CLIENTWRITE_HEADER, accept_ranges, strlen(accept_ranges));
			if(result != CURLE_OK)
				return result;
		}
		...
		headerlen = msnprintf(header, sizeof(header), "Last-Modified: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n%s",
			...
			tm->tm_hour, tm->tm_min, tm->tm_sec, data->req.no_body ? "": "\r\n");
		result = Curl_client_write(data, CLIENTWRITE_HEADER, header, headerlen);
		if(result)
			return result;
	...
	while(!result) {
		...
		size_t bytestoread;
		if(size_known) {
			bytestoread = (expected_size < data->set.buffer_size) ? curlx_sotouz(expected_size) : (size_t)data->set.buffer_size;
		}
		else
