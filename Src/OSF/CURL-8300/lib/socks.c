...
struct socks_state {
	...
	uchar * outp; /* send from this pointer */
    ...
    struct Curl_easy * data)
{
	...
	uchar * socksreq = (uchar *)data->state.buffer;
	CURLcode result;
	...
	switch(sx->state) {
		case CONNECT_SOCKS_INIT:
		    ...
		    socksreq[2] = (uchar)((sx->remote_port >> 8) & 0xff); /* MSB */
		    socksreq[3] = (uchar)(sx->remote_port & 0xff); /* LSB */
...
CONNECT_REQ_INIT:
	...
	switch(socksreq[1]) {
		...
		case 91:
			...
			(((uchar)socksreq[2] << 8) | (uchar)socksreq[3]),
			(uchar)socksreq[1]);
    ...
    struct Curl_easy * data)
{
	...
	ssize_t len = 0;
	const uchar auth = data->set.socks5auth;
...
CONNECT_AUTH_INIT:
		case CONNECT_AUTH_INIT: {
		    ...
		    socksreq[len++] = (uchar)proxy_user_len;
...
CONNECT_REQ_SEND:
		case CONNECT_REQ_SEND:
		    /* PORT MSB */
		    socksreq[len++] = (uchar)((sx->remote_port >> 8) & 0xff);
		    /* PORT LSB */
		    socksreq[len++] = (uchar)(sx->remote_port & 0xff);
...
{
	struct socks_state * sxstate = (socks_state *)cf->ctx;
	if(sxstate) {
		SAlloc::F(sxstate);
		cf->ctx = NULL;
	}
 ...
 */
static CURLcode socks_proxy_cf_connect(struct Curl_cfilter * cf, struct Curl_easy * data, bool blocking, bool * done)
{
	CURLcode result;
	struct connectdata * conn = cf->conn;
	int sockindex = cf->sockindex;
	struct socks_state * sx = (socks_state *)cf->ctx;
	if(cf->connected) {
		*done = TRUE;
		return CURLE_OK;
	}
	...
	if(result || !*done)
		return result;
	if(!sx) {
		sx = (socks_state *)SAlloc::C(sizeof(*sx), 1);
		if(!sx)
...
static int socks_cf_get_select_socks(struct Curl_cfilter * cf, struct Curl_easy * data, curl_socket_t * socks)
{
	struct socks_state * sx = (socks_state *)cf->ctx;
	int fds = cf->next->cft->get_select_socks(cf->next, data, socks);
