...
static CURLcode http_setup_conn(struct Curl_easy * data, struct connectdata * conn)
{
	...
	http = (HTTP *)calloc(1, sizeof(struct HTTP));
	if(!http)
	...
	switch(httpreq) {
		...
		case HTTPREQ_POST_FORM:
		    /* Convert the form structure into a mime structure, then keep
		       the conversion */
		    if(!data->state.formp) {
			    data->state.formp = (curl_mimepart *)calloc(sizeof(curl_mimepart), 1);
			    if(!data->state.formp)
				    return CURLE_OUT_OF_MEMORY;
    ...
    const char * authority, size_t a_len, const char * path, size_t p_len)
{
	...
	if(m_len + 1 >= sizeof(req->method))
		return CURLE_BAD_FUNCTION_ARGUMENT;
	req = (httpreq *)calloc(1, sizeof(*req));
	if(!req)
		goto out;
...
CURLcode Curl_http_resp_make(struct http_resp ** presp, int status, const char * description)
{
	...
	CURLcode result = CURLE_OUT_OF_MEMORY;
	resp = (http_resp *)calloc(1, sizeof(*resp));
	if(!resp)
		goto out;
	resp->status = status;
	if(description) {
