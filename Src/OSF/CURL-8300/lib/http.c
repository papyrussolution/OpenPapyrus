...
CURLcode Curl_http_auth_act(struct Curl_easy * data)
{
	...
	if(!data->set.str[STRING_BEARER])
		authmask &= (ulong)~CURLAUTH_BEARER;
...
static size_t readmoredata(char * buffer, size_t size, size_t nitems, void * userp)
{
	...
	data->req.forbidchunk = (http->sending == HTTP::HTTPSEND_REQUEST) ? TRUE : FALSE;
	if(data->set.max_send_speed && (data->set.max_send_speed < (curl_off_t)fullsize) && (data->set.max_send_speed < http->postsize))
		fullsize = (size_t)data->set.max_send_speed; // speed limit
		...
		if(http->backup.postsize) {
			...
			// @sobolev http->sending++; // move one step up 
			http->sending = static_cast<HTTP::HttpSendingPhase>(http->sending + 1); // @sobolev
		...
		*bytes_written += (long)amount;
		...
		if(http) {
			...
			if((size_t)amount != size) {
				...
				http->sending = HTTP::HTTPSEND_REQUEST;
				return CURLE_OK;
			}
...
{
	const char * ptr;
	struct UrlState::dynamically_allocated_data * aptr = &data->state.aptr;
	...
	switch(httpreq) {
		...
		case HTTPREQ_POST_FORM:
		    /* Convert the form structure into a mime structure, then keep
		       the conversion */
		    if(!data->state.formp) {
			    data->state.formp = (curl_mimepart *)SAlloc::C(sizeof(curl_mimepart), 1);
			    if(!data->state.formp)
				    return CURLE_OUT_OF_MEMORY;
...
CURLcode Curl_http(struct Curl_easy * data, bool * done)
{
	...
	*done = TRUE;
	...
	if((http->postsize > -1) && (http->postsize <= data->req.writebytecount) && (http->sending != HTTP::HTTPSEND_REQUEST))
    ...
    char * headp)
{
	...
	/* If enabled, the header is incoming and this is over HTTPS */
		...
		result = Curl_altsvc_parse(data, data->asi,
			...
			id, conn->host.name,
			curlx_uitous((uint)conn->remote_port));
		if(result)
			return result;
	}
#endif
    ...
    bool connect_only)
{
	...
	if(delta < MAX_HTTP_RESP_HEADER_SIZE) {
		if(!connect_only)
			data->req.headerbytecount += (uint)delta;
		data->info.header_size += (uint)delta;
		...
		if(!end_ptr) {
			...
			if(!k->headerline) {
				...
				if(st == STATUS_BAD) {
					...
					k->header = FALSE;
					k->badheader = SingleRequest::HEADER_ALLBAD;
		...
		if(!k->headerline) {
			...
			if(st == STATUS_BAD) {
				...
				if(*nread)
					...
					k->badheader = SingleRequest::HEADER_PARTHEADER;
				else {
			...
			if(conn->handler->protocol & PROTO_FAMILY_HTTP) {
				...
				if(fine_statusline) {
					...
					switch(httpversion) {
						...
						case 30:
#endif
						    conn->httpversion = (uchar)httpversion;
						    break;
						default:
...
CURLcode Curl_http_req_make(struct httpreq ** preq, const char * method, size_t m_len, const char * scheme, size_t s_len,
    const char * authority, size_t a_len, const char * path, size_t p_len)
{
	struct httpreq * req;
...
CURLcode Curl_http_req_make2(struct httpreq ** preq, const char * method, size_t m_len, CURLU * url, const char * scheme_default)
{
	struct httpreq * req;
	CURLcode result = CURLE_OUT_OF_MEMORY;
	CURLUcode uc;
