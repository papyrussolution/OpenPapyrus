...
static CURLcode http_proxy_cf_connect(struct Curl_cfilter * cf, struct Curl_easy * data, bool blocking, bool * done)
{
...
static void http_proxy_cf_destroy(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct cf_proxy_ctx * ctx = static_cast<cf_proxy_ctx *>(cf->ctx);
	(void)data;
...
CURLcode Curl_cf_http_proxy_insert_after(struct Curl_cfilter * cf_at, struct Curl_easy * data)
{
