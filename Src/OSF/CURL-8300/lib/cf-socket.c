...
static void tcpnodelay(struct Curl_easy * data, curl_socket_t sockfd)
{
	...
	(void)data;
#endif
	if(setsockopt(sockfd, level, TCP_NODELAY, (char *)&onoff, sizeof(onoff)) < 0)
		infof(data, "Could not set TCP_NODELAY: %s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
#else
	(void)data;
	(void)sockfd;
#endif
...
static void tcpkeepalive(struct Curl_easy * data, curl_socket_t sockfd)
{
	...
	if(setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, (char *)&optval, sizeof(optval)) < 0) {
		...
		if(WSAIoctl(sockfd, SIO_KEEPALIVE_VALS, (LPVOID)&vals, sizeof(vals), NULL, 0, &dummy, NULL, NULL) != 0) {
			infof(data, "Failed to set SIO_KEEPALIVE_VALS on fd %d: %d", (int)sockfd, WSAGetLastError());
		}
#else
#ifdef TCP_KEEPIDLE
		optval = curlx_sltosi(data->set.tcp_keepidle);
...
static CURLcode cf_socket_open(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	int error = 0;
...
static int do_connect(struct Curl_cfilter * cf, struct Curl_easy * data, bool is_tcp_fastopen)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
...
static ssize_t cf_socket_recv(struct Curl_cfilter * cf, struct Curl_easy * data, char * buf, size_t len, CURLcode * err)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	curl_socket_t fdsave;
	ssize_t nread;
...
static void conn_set_primary_ip(struct Curl_cfilter * cf, struct Curl_easy * data)
{
...
static CURLcode cf_socket_cntrl(struct Curl_cfilter * cf, struct Curl_easy * data, int event, int arg1, void * arg2)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	(void)arg1;
	(void)arg2;
...
static bool cf_socket_conn_is_alive(struct Curl_cfilter * cf, struct Curl_easy * data, bool * input_pending)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	struct pollfd pfd[1];
	int r;
...
static CURLcode cf_socket_query(struct Curl_cfilter * cf, struct Curl_easy * data, int query, int * pres1, void * pres2)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	switch(query) {
		...
		case CF_QUERY_TIMER_CONNECT: {
		    struct curltime * when = (curltime *)pres2;
...
static CURLcode cf_udp_setup_quic(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	int rc;
...
static CURLcode cf_udp_connect(struct Curl_cfilter * cf, struct Curl_easy * data, bool blocking, bool * done)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
	CURLcode result = CURLE_COULDNT_CONNECT;
	(void)blocking;
...
static void set_accepted_remote_ip(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct cf_socket_ctx * ctx = (cf_socket_ctx *)cf->ctx;
#ifdef HAVE_GETPEERNAME
	char buffer[STRERROR_LEN];
...
CURLcode Curl_cf_socket_peek(struct Curl_cfilter * cf, struct Curl_easy * data, curl_socket_t * psock,
    const struct Curl_sockaddr_ex ** paddr, const char ** pr_ip_str, int * pr_port, const char ** pl_ip_str, int * pl_port)
{
