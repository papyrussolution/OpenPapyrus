    ...
    enum alpnid dstalpnid, uint srcport, uint dstport)
{
	...
	as->src.host = (char *)Curl_memdup(srchost, hlen + 1);
	if(!as->src.host)
		goto error;
	as->src.host[hlen] = 0;

	as->dst.host = (char *)Curl_memdup(dsthost, dlen + 1);
	if(!as->dst.host)
		goto error;
	as->dst.host[dlen] = 0;
...
void Curl_altsvc_cleanup(struct altsvcinfo ** altsvcp)
{
	...
	if(*altsvcp) {
		...
		for(e = altsvc->list.head; e; e = n) {
			struct altsvc * as = (struct altsvc *)e->ptr;
			n = e->next;
			altsvc_free(as);
		}
 ...
 */
bool Curl_altsvc_lookup(struct altsvcinfo * asi, enum alpnid srcalpnid, const char * srchost,
    int srcport, struct altsvc ** dstentry, const int versions/* one or more bits */)
{
	struct Curl_llist_element * e;
	struct Curl_llist_element * n;
