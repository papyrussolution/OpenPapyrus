...
static size_t mime_subparts_read(char * buffer, size_t size, size_t nitems, void * instream, bool * hasread)
{
	...
	while(nitems) {
		size_t sz = 0;
		curl_mimepart * part = (curl_mimepart *)mime->state.ptr;
...
CURLcode curl_mime_filedata(curl_mimepart * part, const char * filename)
{
	...
	if(filename) {
		...
		/* As a side effect, set the filename to the current file's base name.
		   ...
		   curl_mime_filename() with a NULL filename argument after the current call. */
...
curl_off_t Curl_mime_size(curl_mimepart * part)
{
	...
	if(part->kind == MIMEKIND_MULTIPART)
		part->datasize = multipart_size((curl_mime *)part->arg);
	size = part->datasize;
	if(part->encoder)
...
const char *Curl_mime_contenttype(const char * filename)
{
	...
	if(filename) {
		...
		for(uint i = 0; i < sizeof(ctts) / sizeof(ctts[0]); i++) {
...
#else /* !CURL_DISABLE_MIME && (!CURL_DISABLE_HTTP || !CURL_DISABLE_SMTP || !CURL_DISABLE_IMAP) */
