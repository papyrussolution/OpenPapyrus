...
static CURLcode get_cert_location(TCHAR * path, DWORD * store_name, TCHAR ** store_path,
    TCHAR ** thumbprint)
{
	TCHAR * sep;
...
static CURLcode schannel_connect_step1(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	ssize_t written = -1;
	struct ssl_connect_data * connssl = cf->ctx;
	struct schannel_ssl_backend_data * backend =
...
#error "compiler too old to support requisite manual cert verify for Win CE"
	...
	if(backend->use_alpn) {
		...
		extension_len = (uint *)(void *)(&alpn_buffer[cur]);
		cur += (int)sizeof(uint);

		/* The next four bytes are an indicator that this buffer will contain
		   ALPN data, as opposed to NPN, for example. */
		*(uint *)(void *)&alpn_buffer[cur] =
		...
		*extension_len = *list_len + (ushort)sizeof(uint) + (ushort)sizeof(ushort);
...
static CURLcode schannel_connect_step2(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct ssl_connect_data * connssl = cf->ctx;
	struct schannel_ssl_backend_data * backend =
	    (struct schannel_ssl_backend_data *)connssl->backend;
...
static bool add_cert_to_certinfo(const CERT_CONTEXT * ccert_context, bool reverse_order,
    void * raw_arg)
{
...
static CURLcode schannel_connect_step3(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	struct ssl_connect_data * connssl = cf->ctx;
	struct schannel_ssl_backend_data * backend =
	    (struct schannel_ssl_backend_data *)connssl->backend;
		...
		if(sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE ||
			...
			if(inbuf[1].BufferType == SECBUFFER_DATA) {
				...
				if(backend->decdata_length - backend->decdata_offset < size ||
					...
					reallocated_buffer = SAlloc::R(backend->decdata_buffer, reallocated_length);
					if(!reallocated_buffer) {
						*err = CURLE_OUT_OF_MEMORY;
						failf(data, "schannel: unable to re-allocate memory");
						goto cleanup;
...
static void schannel_checksum(const uchar * input,
    size_t inputlen,
    uchar * checksum,
    size_t checksumlen,
    DWORD provType,
...
static CURLcode schannel_sha256sum(const uchar * input,
    size_t inputlen,
    uchar * sha256sum,
    size_t sha256len)
{
