...
{
	struct Curl_cfilter * cf = (Curl_cfilter *)BIO_get_data(bio);
	long ret = 1;
	(void)cf;
	(void)ptr;
	switch(cmd) {
...
static int SSL_CTX_use_certificate_blob(SSL_CTX * ctx, const struct curl_blob * blob,
...
static int SSL_CTX_use_PrivateKey_blob(SSL_CTX * ctx, const struct curl_blob * blob,
    int type, const char * key_passwd)
{
	int ret = 0;
	EVP_PKEY * pkey = NULL;
...
static int SSL_CTX_use_certificate_chain_blob(SSL_CTX * ctx, const struct curl_blob * blob,
		...
		switch(file_type) {
...
#if defined(USE_OPENSSL_ENGINE) && defined(ENGINE_CTRL_GET_CMD_FROM_NAME)
		    {
			    ...
			    if(data->state.engine) {
				    ...
				    if(!ENGINE_ctrl_cmd((ENGINE *)data->state.engine, cmd_name, 0, &params, NULL, 1)) {
		...
		switch(file_type) {
...
#ifdef USE_OPENSSL_ENGINE
		    {
			    ...
			    if(data->state.engine) {
				    ...
				    priv_key = (EVP_PKEY *)
					ENGINE_load_private_key((ENGINE *)data->state.engine, key_file, ui_method, key_passwd);
...
#ifdef USE_OPENSSL_ENGINE
	if(data->state.engine) {
		if(ENGINE_set_default((ENGINE *)data->state.engine, ENGINE_METHOD_ALL) > 0) {
			infof(data, "set default crypto engine '%s'", ENGINE_get_id((ENGINE *)data->state.engine));
		}
		else {
			failf(data, "set default crypto engine '%s' failed", ENGINE_get_id((ENGINE *)data->state.engine));
...
static CURLcode ossl_verifyhost(struct Curl_easy * data, struct connectdata * conn,
		...
		if(i >= 0) {
			...
			if(tmp) {
				if(ASN1_STRING_type(tmp) == V_ASN1_UTF8STRING) {
					...
					if(peerlen >= 0) {
						peer_CN = (uchar *)OPENSSL_malloc(peerlen + 1);
						if(peer_CN) {
...
static int ossl_new_session_cb(SSL * ssl, SSL_SESSION * ssl_sessionid)
{
	...
	connssl = cf ? (ssl_connect_data *)cf->ctx : NULL;
...
static bool cached_x509_store_different(struct Curl_cfilter * cf,
    const struct multi_ssl_backend_data * mb)
{
	struct ssl_primary_config * conn_config = Curl_ssl_cf_get_primary_config(cf);
    ...
    struct Curl_easy * data)
{
	...
	if(ssl_config->primary.sessionid) {
		...
		if(!Curl_ssl_getsessionid(cf, data, &ssl_sessionid, NULL)) {
			...
			if(!SSL_set_session(backend->handle, (SSL_SESSION *)ssl_sessionid)) {
				Curl_ssl_sessionid_unlock(data);
	...
	if(rc) {
		...
		if(lerr != X509_V_OK) {
			if(conn_config->verifypeer) {
				...
				infof(data, " SSL certificate verify result: %s (%ld), continuing anyway.", X509_verify_cert_error_string(lerr), lerr);
		}
		else
	...
	if(rc <= 0) {
		...
		switch(err) {
			...
			case SSL_ERROR_SSL: {
			    ...
			    struct ssl_connect_data * connssl_next = cf_ssl_next ? (ssl_connect_data *)cf_ssl_next->ctx : NULL;
			    sslerror = ERR_get_error();
