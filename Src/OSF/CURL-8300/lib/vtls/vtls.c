...
static bool ssl_prefs_check(struct Curl_easy * data)
{
	...
	const uchar sslver = data->set.ssl.primary.version;
...
static struct ssl_connect_data *cf_ctx_new(struct Curl_easy * data, const struct alpn_spec * alpn)
{
	...
	(void)data;
	ctx = (ssl_connect_data *)SAlloc::C(1, sizeof(*ctx));
	if(!ctx)
		return NULL;
	ctx->alpn = alpn;
	ctx->backend = SAlloc::C(1, Curl_ssl->sizeof_ssl_backend_data);
	if(!ctx->backend) {
		SAlloc::F(ctx);
		return NULL;
	}
...
static CURLcode ssl_connect_nonblocking(struct Curl_cfilter * cf, struct Curl_easy * data,
    bool * done)
{
	if(!ssl_prefs_check(data))
		return CURLE_SSL_CONNECT_ERROR;
...
int Curl_ssl_get_select_socks(struct Curl_cfilter * cf, struct Curl_easy * data, curl_socket_t * socks)
{
	struct ssl_connect_data * connssl = (ssl_connect_data *)cf->ctx;
...
CURLcode Curl_ssl_initsessions(struct Curl_easy * data, size_t amount)
{
	...
	session = (Curl_ssl_session *)SAlloc::C(amount, sizeof(struct Curl_ssl_session));
	if(!session)
		return CURLE_OUT_OF_MEMORY;
...
CURLcode Curl_ssl_init_certinfo(struct Curl_easy * data, int num)
{
	...
	table = (curl_slist **)SAlloc::C((size_t)num, sizeof(struct curl_slist *));
	if(!table)
		return CURLE_OUT_OF_MEMORY;
	ci->num_of_certs = num;
	ci->certinfo = table;
...
CURLcode Curl_ssl_push_certinfo_len(struct Curl_easy * data, int certnum, const char * label, const char * value, size_t valuelen)
{
	...
	char * output = (char *)SAlloc::M(outlen);
	if(!output)
		return CURLE_OUT_OF_MEMORY;
...
static void free_hostname(struct ssl_connect_data * connssl)
{
	if(connssl->dispname != connssl->hostname)
		SAlloc::F(connssl->dispname);
	SAlloc::F(connssl->hostname);
...
void *Curl_ssl_get_internals(struct Curl_easy * data, int sockindex, CURLINFO info, int n)
{
	...
	if(data->conn) {
		...
		if(cf) {
			struct cf_call_data save;
			CF_DATA_SAVE(save, cf, data);
			result = Curl_ssl->get_internals((ssl_connect_data *)cf->ctx, info);
			CF_DATA_RESTORE(cf, save);
		}
	}
	return result;
