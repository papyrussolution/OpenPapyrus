...
static CURLcode blobdup(struct curl_blob ** dest, struct curl_blob * src)
{
	...
	if(src) {
		...
		struct curl_blob * d = (curl_blob *)malloc(sizeof(struct curl_blob) + src->len);
		if(!d)
...
static struct ssl_connect_data *cf_ctx_new(struct Curl_easy * data, const struct alpn_spec * alpn)
{
	...
	(void)data;
	ctx = (ssl_connect_data *)calloc(1, sizeof(*ctx));
	if(!ctx)
		return NULL;
	ctx->alpn = alpn;
...
CURLcode Curl_ssl_initsessions(struct Curl_easy * data, size_t amount)
{
	...
	session = (Curl_ssl_session *)calloc(amount, sizeof(struct Curl_ssl_session));
	if(!session)
		return CURLE_OUT_OF_MEMORY;
...
CURLcode Curl_ssl_init_certinfo(struct Curl_easy * data, int num)
{
	...
	table = (curl_slist **)calloc((size_t)num, sizeof(struct curl_slist *));
	if(!table)
		return CURLE_OUT_OF_MEMORY;
	ci->num_of_certs = num;
	ci->certinfo = table;
