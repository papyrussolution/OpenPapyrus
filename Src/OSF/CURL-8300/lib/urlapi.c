...
static CURLUcode parseurl(const char * url, CURLU * u, uint flags)
{
	...
	if(fragment) {
		...
		if(fraglen > 1) {
			...
			if(flags & CURLU_URLENCODE) {
				...
				u->fragment = (char *)Curl_memdup(fragment + 1, fraglen);
				if(!u->fragment) {
					result = CURLUE_OUT_OF_MEMORY;
					goto fail;
				}
	...
	if(query) {
		...
		if(qlen > 1) {
			if(flags & CURLU_URLENCODE) {
				...
				u->query = (char *)Curl_memdup(query + 1, qlen);
				if(!u->query) {
					result = CURLUE_OUT_OF_MEMORY;
					goto fail;
				}
	...
	if(pathlen <= 1) {
		...
		if(!u->path) {
			u->path = (char *)Curl_memdup(path, pathlen + 1);
			if(!u->path) {
				result = CURLUE_OUT_OF_MEMORY;
				goto fail;
			}
    ...
    char ** part, uint flags)
{
	...
	*part = NULL;
	...
	if(ptr) {
		...
		size_t i = 0;
		*part = (char *)Curl_memdup(ptr, partlen + 1);
		if(!*part)
			return CURLUE_OUT_OF_MEMORY;
		if(plusdecode) {
			/* convert + to space */
