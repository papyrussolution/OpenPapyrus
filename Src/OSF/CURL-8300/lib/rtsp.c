...
static CURLcode rtsp_setup_connection(struct Curl_easy * data,
    struct connectdata * conn);
static uint rtsp_conncheck(struct Curl_easy * data,
    struct connectdata * check,
    uint checks_to_perform);
...
static CURLcode rtsp_setup_connection(struct Curl_easy * data, struct connectdata * conn)
{
	struct RTSP * rtsp;
	(void)conn;
	data->req.p.rtsp = rtsp = static_cast<RTSP *>(SAlloc::C(1, sizeof(struct RTSP)));
	if(!rtsp)
 ...
 */
static uint rtsp_conncheck(struct Curl_easy * data, struct connectdata * conn, uint checks_to_perform)
{
	uint ret_val = CONNRESULT_NONE;
	(void)data;
	...
	*done = TRUE;
	...
	if(rtspreq == RTSPREQ_ANNOUNCE || rtspreq == RTSPREQ_SET_PARAMETER || rtspreq == RTSPREQ_GET_PARAMETER) {
		if(data->state.upload) {
			...
			postsize = (data->state.infilesize != -1) ? data->state.infilesize : (curl_off_t)sstrlen((const char *)data->set.postfields);
		...
		if(putsize > 0 || postsize > 0) {
			...
			if(!Curl_checkheaders(data, STRCONST("Content-Length"))) {
				result = Curl_dyn_addf(&req_buffer, "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", (data->state.upload ? putsize : postsize));
				if(result)
					return result;
			}
			if(rtspreq == RTSPREQ_SET_PARAMETER ||
				...
				if(!Curl_checkheaders(data, STRCONST("Content-Type"))) {
					result = Curl_dyn_addn(&req_buffer, STRCONST("Content-Type: text/parameters\r\n"));
					if(result)
						return result;
				}
			}
			if(rtspreq == RTSPREQ_ANNOUNCE) {
				if(!Curl_checkheaders(data, STRCONST("Content-Type"))) {
					result = Curl_dyn_addn(&req_buffer, STRCONST("Content-Type: application/sdp\r\n"));
					if(result)
						return result;
				}
			}
...
CURLcode Curl_rtsp_parseheader(struct Curl_easy * data, char * header)
{
	if(checkprefix("CSeq:", header)) {
		...
		if(data->set.str[STRING_RTSP_SESSION_ID]) {
			...
			data->set.str[STRING_RTSP_SESSION_ID] = (char *)SAlloc::M(idlen + 1);
			if(!data->set.str[STRING_RTSP_SESSION_ID])
				return CURLE_OUT_OF_MEMORY;
		...
		if(checkprefix("interleaved=", start)) {
			...
			if(p != endp && chan1 >= 0 && chan1 <= 255) {
				...
				for(chan = chan1; chan <= chan2; chan++) {
					long idx = chan / 8;
					long off = chan % 8;
					rtp_channel_mask[idx] |= (uchar)(1 << off);
				}
			}
			else {
				infof(data, "Unable to read the interleaved parameter from "
