...
{
	const struct pair * aa = (const struct pair *)a;
	const struct pair * bb = (const struct pair *)b;
	return strncmp(aa->p, bb->p, aa->len < bb->len ? aa->len : bb->len);
}

#define MAX_QUERYPAIRS 64

static CURLcode canon_query(struct Curl_easy * data, const char * query, struct dynbuf * dq)
{
