...
#else
	#define SCP_SEND(a, b, c, d) libssh2_scp_send64(a, b, (int)(c), (uint64)d, 0, 0)
#endif
/*
		...
		if(remotekey) {
			...
			if(!keybit)
				...
				if(keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) {
					...
					knownkey.len = 0;
					knownkey.keytype = (curl_khtype)convert_ssh2_keytype(sshkeytype);
					knownkeyp = &knownkey;
				}
				/* setup 'foundkey' */
				foundkey.key = remotekey;
				foundkey.len = keylen;
				foundkey.keytype = (curl_khtype)convert_ssh2_keytype(sshkeytype);
				/*
...
static CURLcode ssh_check_fingerprint(struct Curl_easy * data)
{
	...
	if(pubkey_sha256) {
		...
		if(hostkey) {
			if(!Curl_sha256it(hash, (const uchar *)hostkey, len))
	...
	do {
		switch(sshc->state) {
			    ...
			    /* Send any quote commands */
		    {
			    ...
			    }
			    {
					...
					strncasecompare(cmd, "mtime ", 6)) {
					    ...
					    memzero(&sshp->quote_attrs, sizeof(LIBSSH2_SFTP_ATTRIBUTES));
			...
			case SSH_SFTP_QUOTE_STAT:
		    {
			    ...
			    if(strncasecompare(cmd, "chgrp", 5)) {
				    ...
				    if(strncasecompare(cmd, "atime", 5))
					    sshp->quote_attrs.atime = (ulong)date;
				    else /* mtime */
					    sshp->quote_attrs.mtime = (ulong)date;
			...
			case SSH_SFTP_QUOTE_SYMLINK:
			    rc = libssh2_sftp_symlink_ex(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)), 
					sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)), LIBSSH2_SFTP_SYMLINK);
			    if(rc == LIBSSH2_ERROR_EAGAIN) {
				    break;
			    }
			    if(rc && !sshc->acceptfail) {
			...
			case SSH_SFTP_QUOTE_RENAME:
			    rc = libssh2_sftp_rename_ex(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)), sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
			...
			case SSH_SFTP_UPLOAD_INIT:
		    {
			    ...
			    if(!sshc->sftp_handle) {
				    ...
				    if(LIBSSH2_ERROR_SFTP_PROTOCOL == rc)
					    /* only when there was an SFTP protocol error can we extract the sftp error! */
					    sftperr = libssh2_sftp_last_error(sshc->sftp_session);
				    else
				    ...
				    if(((sftperr == LIBSSH2_FX_NO_SUCH_FILE) || (sftperr == LIBSSH2_FX_FAILURE) || (sftperr == LIBSSH2_FX_NO_SUCH_PATH)) &&
			...
			case SSH_SFTP_READDIR_INIT:
			    ...
			    sshp->readdir_filename = (char *)SAlloc::M(PATH_MAX + 1);
			    if(!sshp->readdir_filename) {
				    state(data, SSH_SFTP_CLOSE);
				    sshc->actualcode = CURLE_OUT_OF_MEMORY;
				    break;
			    }
			    sshp->readdir_longentry = (char *)SAlloc::M(PATH_MAX + 1);
			    if(!sshp->readdir_longentry) {
				    ZFREE(sshp->readdir_filename);
				    state(data, SSH_SFTP_CLOSE);
				    sshc->actualcode = CURLE_OUT_OF_MEMORY;
				    break;
			    }
			...
			case SSH_SFTP_DOWNLOAD_STAT:
		    {
			    ...
			    if(rc || !(attrs.flags & LIBSSH2_SFTP_ATTR_SIZE) || (attrs.filesize == 0)) {
				    ...
				    if(data->state.use_range) {
					    ...
					    char * ptr2;
					    CURLofft to_t;
					    CURLofft from_t = curlx_strtoofft(data->state.range, &ptr, 10, &from);
					    ...
					    if(to_t == CURL_OFFT_FLOW)
						    return CURLE_RANGE_ERROR;
					    if((to_t == CURL_OFFT_INVAL) /* no "to" value given */ || (to >= size)) {
						    to = size - 1;
					    }
					    if(from_t) {
...
static CURLcode ssh_setup_connection(struct Curl_easy * data, struct connectdata * conn)
{
	struct SSHPROTO * ssh;
	(void)conn;
	data->req.p.ssh = ssh = static_cast<SSHPROTO *>(SAlloc::C(1, sizeof(struct SSHPROTO)));
	if(!ssh)
		return CURLE_OUT_OF_MEMORY;
	return CURLE_OK;
}
...
static ssize_t ssh_tls_recv(libssh2_socket_t sock, void * buffer, size_t length, int flags, void ** abstract)
{
	...
	(void)flags;
	// swap in the TLS reader function for this call only, and then swap back the SSH one again
	conn->recv[0] = ssh->tls_recv;
	result = Curl_read(data, sock, (char *)buffer, length, &nread);
	conn->recv[0] = backup;
...
static ssize_t scp_send(struct Curl_easy * data, int sockindex, const void * mem, size_t len, CURLcode * err)
{
	...
	nwrite = (ssize_t)libssh2_channel_write(sshc->ssh_channel, (const char *)mem, len);
