...
struct connectbundle *Curl_conncache_find_bundle(struct Curl_easy * data,
	...
	if(connc) {
		...
		bundle = (connectbundle *)Curl_hash_pick(&connc->hash, key, strlen(key));
	}
	return bundle;
}
    ...
    struct connectdata * conn, void * param))
{
	...
	while(he) {
		struct connectbundle * bundle;
		bundle = (connectbundle *)he->ptr;
		he = Curl_hash_next_element(&iter);
		curr = bundle->conn_list.head;
		while(curr) {
			/* Yes, we need to update curr before calling func(), because func()
			   might decide to remove the connection */
			struct connectdata * conn = (connectdata *)curr->ptr;
			curr = curr->next;
...
static struct connectdata * conncache_find_first_connection(struct conncache * connc)                            
{
	...
	while(he) {
		...
		curr = bundle->conn_list.head;
		if(curr) {
			return static_cast<connectdata *>(curr->ptr);
		}
		he = Curl_hash_next_element(&iter);
	}
	return NULL;
