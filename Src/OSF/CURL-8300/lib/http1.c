...
static ssize_t detect_line(struct h1_req_parser * parser, const char * buf, const size_t buflen, CURLcode * err)
{
	const char  * line_end;
	DEBUGASSERT(!parser->line);
	line_end = (const char *)memchr(buf, '\n', buflen);
	if(!line_end) {
		*err = CURLE_AGAIN;
		return -1;
	}
...
static CURLcode start_req(struct h1_req_parser * parser, const char * scheme_default, int options)
{
	...
	p = (const char *)memchr(parser->line, ' ', parser->line_len);
	if(!p || p == parser->line)
		goto out;
