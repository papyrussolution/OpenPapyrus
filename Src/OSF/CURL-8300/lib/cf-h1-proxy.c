...
static CURLcode tunnel_init(struct h1_tunnel_state ** pts, struct Curl_easy * data, struct connectdata * conn, int sockindex)
{
...
static void h1_tunnel_go_state(struct Curl_cfilter * cf, struct h1_tunnel_state * ts, H1TunnelStateId new_state, struct Curl_easy * data)
{
...
{
	struct h1_tunnel_state * ts = (h1_tunnel_state *)cf->ctx;
	if(ts) {
...
static CURLcode CONNECT_host(struct Curl_easy * data, struct connectdata * conn, const char * hostname,
    int remote_port, char ** connecthostp, char ** hostp)
{
...
static CURLcode send_CONNECT(struct Curl_easy * data, struct connectdata * conn, struct h1_tunnel_state * ts, bool * done)
{
	...
	if(!ts->nsend)
		http->sending = HTTP::HTTPSEND_NADA;
out:
	if(result)
		failf(data, "Failed sending CONNECT to proxy");
	*done = (http->sending != HTTP::HTTPSEND_REQUEST);
	return result;
}
...
#define SELECT_ERROR   1
	...
	*done = FALSE;
	...
	while(ts->keepon) {
		...
		if(ts->keepon == h1_tunnel_state::KEEPON_IGNORE) {
		...
		if(('\r' == linep[0]) ||
			...
			if((407 == k->httpcode) && !data->state.authproblem) {
				/* If we get a 407 response code with content length
				   ...
				   whole response-body */
				ts->keepon = h1_tunnel_state::KEEPON_IGNORE;
				if(ts->cl) {
	...
	*done = (ts->keepon == h1_tunnel_state::KEEPON_DONE);
...
static int cf_h1_proxy_get_select_socks(struct Curl_cfilter * cf, struct Curl_easy * data, curl_socket_t * socks)
{
	struct h1_tunnel_state * ts = (h1_tunnel_state *)cf->ctx;
	int fds;
	...
	if(!fds && cf->next->connected && !cf->connected) {
		...
		if(ts) {
			/* when we've sent a CONNECT to a proxy, we should rather either
			   ...
			   for write. */
			if(ts->CONNECT.sending == HTTP::HTTPSEND_REQUEST) {
...
static void cf_h1_proxy_close(struct Curl_cfilter * cf, struct Curl_easy * data)
{
	...
	cf->connected = FALSE;
	if(cf->ctx) {
		h1_tunnel_go_state(cf, (h1_tunnel_state *)cf->ctx, H1_TUNNEL_INIT, data);
	}
	if(cf->next)
