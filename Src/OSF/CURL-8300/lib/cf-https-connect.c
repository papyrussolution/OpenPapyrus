...
static bool time_to_start_h21(struct Curl_cfilter * cf, struct Curl_easy * data, struct curltime now)
{
	struct cf_hc_ctx * ctx = (cf_hc_ctx *)cf->ctx;
...
static CURLcode cf_hc_connect(struct Curl_cfilter * cf, struct Curl_easy * data, bool blocking, bool * done)
{
	struct cf_hc_ctx * ctx = (cf_hc_ctx *)cf->ctx;
	struct curltime now;
	CURLcode result = CURLE_OK;
...
static int cf_hc_get_select_socks(struct Curl_cfilter * cf, struct Curl_easy * data, curl_socket_t * socks)
{
	struct cf_hc_ctx * ctx = (cf_hc_ctx *)cf->ctx;
	size_t i, j, s;
...
static bool cf_hc_data_pending(struct Curl_cfilter * cf, const struct Curl_easy * data)
{
	struct cf_hc_ctx * ctx = (cf_hc_ctx *)cf->ctx;
	if(cf->connected)
		return cf->next->cft->has_data_pending(cf->next, data);
	CURL_TRC_CF((struct Curl_easy *)data, cf, "data_pending");
	return cf_hc_baller_data_pending(&ctx->h3_baller, data) || cf_hc_baller_data_pending(&ctx->h21_baller, data);
}

static struct curltime cf_get_max_baller_time(struct Curl_cfilter * cf, struct Curl_easy * data, int query)
{
	struct cf_hc_ctx * ctx = (cf_hc_ctx *)cf->ctx;
...
static CURLcode cf_hc_query(struct Curl_cfilter * cf, struct Curl_easy * data, int query, int * pres1, void * pres2)
{
	if(!cf->connected) {
		switch(query) {
