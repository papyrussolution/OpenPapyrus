...
#else
	#define CURL_OFF_T_MAX CURL_OFF_T_C(0x7FFFFFFFFFFFFFFF) /* assume SIZEOF_CURL_OFF_T == 8 */
#endif
...
int getpwuid_r(uid_t uid, struct passwd * pwd, char * buf, size_t buflen, struct passwd ** result);
	...
	#define USE_HTTP2
#endif
#if (defined(USE_NGTCP2) && defined(USE_NGHTTP3)) || defined(USE_QUICHE) || defined(USE_MSH3)
	#define ENABLE_QUIC
	#define USE_HTTP3
#endif
...
/* Replicating logic present in afunix.h (distributed with newer Windows 10 SDK versions only) */
...
#endif
#endif
// OpenSSLv3 marks DES, MD5 and ENGINE functions deprecated but we have no replacements (yet) so tell the compiler to not warn for them. 
#ifdef USE_OPENSSL
