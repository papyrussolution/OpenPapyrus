...
static CURLcode dupset(struct Curl_easy * dst, struct Curl_easy * src)
{
	...
	memzero(dst->set.str, STRING_LAST * sizeof(char *));
	{
		...
		for(int idx = /*(enum dupstring)*/0; idx < STRING_LASTZEROTERMINATED; idx++) {
			result = Curl_setstropt(&dst->set.str[idx], src->set.str[idx]);
			if(result)
				return result;
		}
	}
...
CURLcode curl_easy_pause(struct Curl_easy * data, int action)
{
	...
	if((k->keepon & ~newstate & KEEP_SEND_PAUSE) && (data->mstate == MSTATE_PERFORMING || data->mstate == MSTATE_RATELIMITING) && data->state.fread_func == (curl_read_callback)Curl_mime_read) {
    ...
    size_t * n)
{
	...
	*n = 0;
	result = Curl_read(data, sfd, (char *)buffer, buflen, &n1);
	if(result)
		return result;
	*n = (size_t)n1;
	return CURLE_OK;
...
{
	struct curltime * now = (curltime *)param;
...
static CURLcode upkeep(struct conncache * conn_cache, void * data)
{
	...
	Curl_conncache_foreach((Curl_easy *)data, conn_cache, &now, conn_upkeep);
	return CURLE_OK;
}
/*
