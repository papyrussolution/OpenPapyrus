...
struct FreeTypeFont {
	bool  InitFont(FT_Library ft_library, const ImFontConfig& cfg, uint extra_user_flags); // Initialize from an external data buffer. Doesn't copy data, and you must ensure it stays valid up to this object lifetime.
	void  CloseFont();
	void  SetPixelHeight(int pixel_height); // Change font pixel size. All following calls to RasterizeGlyph() will use this size
	const FT_Glyph_Metrics* LoadGlyph(uint32_t in_codepoint);
	const FT_Bitmap*        RenderGlyphAndGetInfo(GlyphInfo* out_glyph_info);
	void  BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, uchar * multiply_table = nullptr);
	~FreeTypeFont()
	{ 
		CloseFont(); 
	}
	...
	FT_Face Face;
	uint UserFlags;             // = ImFontConfig::RasterizerFlags
	FT_Int32 LoadFlags;
...
bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig& cfg, uint extra_font_builder_flags)
{
...
void FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, uchar * multiply_table)
{
...
struct ImFontBuildSrcGlyphFT {
	...
	GlyphInfo Info;
	uint32_t Codepoint;
	uint * BitmapData;     // Point within one of the dst_tmp_bitmap_buffers[] array
};
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, uint extra_flags)
{
	...
	memzero((void*)src_tmp_array.Data, (size_t)src_tmp_array.size_in_bytes());
	memzero((void*)dst_tmp_array.Data, (size_t)dst_tmp_array.size_in_bytes());
	...
	for(int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
		...
		assert(sizeof(src_tmp.GlyphsSet.Storage.Data[0]) == sizeof(ImU32));
	...
	ImVector<uchar*> buf_bitmap_buffers;
	buf_bitmap_buffers.push_back((uchar*)IM_ALLOC(BITMAP_BUFFERS_CHUNK_SIZE));
	...
	for(int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
		...
		for(int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++) {
			...
			assert(buf_bitmap_current_used_bytes + bitmap_size_in_bytes <= BITMAP_BUFFERS_CHUNK_SIZE); // We could probably allocate custom-sized buffer instead.

			// Blit rasterized pixels to our temporary buffer and keep a pointer to it.
			src_glyph.BitmapData = (uint*)(buf_bitmap_buffers.back() + buf_bitmap_current_used_bytes);
	...
	if(src_load_color) {
		...
		atlas->TexPixelsRGBA32 = (uint*)IM_ALLOC(tex_size);
		memzero(atlas->TexPixelsRGBA32, tex_size);
	}
	else {
		size_t tex_size = (size_t)atlas->TexWidth * atlas->TexHeight * 1;
		atlas->TexPixelsAlpha8 = (uchar*)IM_ALLOC(tex_size);
		memzero(atlas->TexPixelsAlpha8, tex_size);
	}
	...
	for(int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
		...
		for(int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
			...
			assert(dst_glyph->Codepoint == src_glyph.Codepoint);
			...
			uint * blit_src = src_glyph.BitmapData;
			if(atlas->TexPixelsAlpha8 != nullptr) {
				uchar * blit_dst = atlas->TexPixelsAlpha8 + (ty * blit_dst_stride) + tx;
				for(int y = 0; y < info.Height; y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
					for(int x = 0; x < info.Width; x++)
						blit_dst[x] = (uchar)((blit_src[x] >> IM_COL32_A_SHIFT) & 0xFF);
			}
			else {
				uint * blit_dst = atlas->TexPixelsRGBA32 + (ty * blit_dst_stride) + tx;
				for(int y = 0; y < info.Height; y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
					for(int x = 0; x < info.Width; x++)
