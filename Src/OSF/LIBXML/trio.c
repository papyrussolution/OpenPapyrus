...
typedef struct {
	...
	int width; /* The width qualifier */
	int precision; /* The precision qualifier */
	int base; /* The base qualifier */
	int varsize; /* The size for the variable size qualifier */
	int indexAfterSpecifier; /* The marker of the end of the specifier */
...
static int TrioGetPosition(const char * format, int * indexPointer)
{
	...
	int number = (int)trio_to_long(&format[index], &tmpformat, BASE_DECIMAL);
...
static trio_long_double_t TrioPower(int number, int exponent)
{
	...
	if(number == 10) {
		switch(exponent) {
			...
			case 0: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E-1); break;
			case 1: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+0); break;
			case 2: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+1); break;
			case 3: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+2); break;
			case 4: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+3); break;
			case 5: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+4); break;
			case 6: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+5); break;
			case 7: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+6); break;
			case 8: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+7); break;
			case 9: result = (trio_long_double_t)number * TRIO_SUFFIX_LONG(1E+8); break;
			default: result = powl((trio_long_double_t)number, (trio_long_double_t)exponent); break;
		}
	}
	else {
...
static int TrioParse(int type, const char * format, trio_parameter_t * parameters, TRIO_VA_LIST_PTR arglist, trio_pointer_t * argarray)
{
	...
	while(format[index]) {
		...
		if(CHAR_IDENTIFIER == format[index++]) {
			...
			if(!positional) {
				currentParam = parameterPosition; // We have no positional, get the next counter 
			}
			if(currentParam >= MAX_PARAMETERS) {
				return TRIO_ERROR_RETURN(TRIO_ETOOMANY, index); // Bail out completely to make the error more obvious 
			}
				...
				switch(ch) {
					...
					case QUALIFIER_FIXED_SIZE:
					    if(flags & FLAGS_FIXED_SIZE)
						    return TRIO_ERROR_RETURN(TRIO_EINVAL, index);
					    if(flags & (FLAGS_ALL_SIZES | FLAGS_LONGDOUBLE | FLAGS_WIDECHAR | FLAGS_VARSIZE_PARAMETER))
						    return TRIO_ERROR_RETURN(TRIO_EINVAL, index);
					    if((format[index] == '6') && (format[index + 1] == '4')) {
						    varsize = sizeof(trio_int64_t);
						    index += 2;
					    }
					    else if((format[index] == '3') && (format[index + 1] == '2')) {
						    varsize = sizeof(trio_int32_t);
						    index += 2;
					    }
					    else if((format[index] == '1') && (format[index + 1] == '6')) {
...
#endif
					default:
					    return TRIO_ERROR_RETURN(TRIO_EINVAL, index); // Bail out completely to make the error more obvious 
				}
			} /* while qualifier */
			/*
				...
				case SPECIFIER_USER_DEFINED_BEGIN:
			    {
				    ...
				    while((ch = format[index])) {
					    ...
					    if(ch == SPECIFIER_USER_DEFINED_END) {
						    ...
						    /* Copy the user data */
						    max = (uint)(&format[index] - tmpformat);
						    if(max > MAX_USER_DATA)
							    max = MAX_USER_DATA;
						    trio_copy_max(parameters[pos].user_data, max, tmpformat);
						    break; /* while */
					    }
		...
		switch(parameters[i].type) {
			...
			case FORMAT_STRING:
			    ...
			    if(flags & FLAGS_WIDECHAR) {
				    parameters[i].data.wstring = (!argarray)
...
#endif
			    {
				    parameters[i].data.string = (!argarray) ? va_arg(TRIO_VA_LIST_DEREF(arglist), char *) : (char *)(argarray[num]);
			    }
			    break;
			    ...
			    if(TYPE_SCAN == type) {
				    if(!argarray)
					    ...
					    if(parameters[i].type == FORMAT_CHAR)
						    parameters[i].data.pointer = ((char *)argarray[num]);
					    else if(parameters[i].flags & FLAGS_SHORT)
						    parameters[i].data.pointer = ((short*)argarray[num]);
					    else
						    parameters[i].data.pointer = ((int *)argarray[num]);
				    }
			    }
			    else {
				    ...
				    if(parameters[i].flags & FLAGS_PTRDIFF_T)
					    parameters[i].data.number.as_unsigned = (!argarray) ? (trio_uintmax_t)va_arg(TRIO_VA_LIST_DEREF(arglist), ptrdiff_t) : (trio_uintmax_t)(*((ptrdiff_t*)argarray[num]));
				    else
#endif
#if defined(QUALIFIER_INTMAX_T)
				    if(parameters[i].flags & FLAGS_INTMAX_T)
					    parameters[i].data.number.as_unsigned = (!argarray) ? (trio_uintmax_t)va_arg(TRIO_VA_LIST_DEREF(arglist), trio_intmax_t) : (trio_uintmax_t)(*((trio_intmax_t*)argarray[num]));
				    else
#endif
				    if(parameters[i].flags & FLAGS_QUAD)
					    parameters[i].data.number.as_unsigned = (!argarray) ? (trio_uintmax_t)va_arg(TRIO_VA_LIST_DEREF(arglist), trio_ulonglong_t) : (trio_uintmax_t)(*((trio_ulonglong_t*)argarray[num]));
				    else if(parameters[i].flags & FLAGS_LONG)
					    parameters[i].data.number.as_unsigned = (!argarray) ? (trio_uintmax_t)va_arg(TRIO_VA_LIST_DEREF(arglist), long) : (trio_uintmax_t)(*((long *)argarray[num]));
				    else {
					    if(!argarray)
...
static void TrioWriteDouble(trio_class_t * self, trio_long_double_t number, trio_flags_t flags, int width, int precision, int base)
{
	...
	if(flags & FLAGS_FLOAT_G) {
		if(precision == 0)
			precision = 1;
		if((number < 1.0E-4) || (number > powl(base, (trio_long_double_t)precision))) {
	...
	fractionDigits = ((flags & FLAGS_FLOAT_G) && (zeroes == 0)) ? precision - integerDigits : zeroes + precision;
	...
	expectedWidth = integerDigits + fractionDigits + (keepDecimalPoint ? internalDecimalPointLength : 0)
	    + ((flags & FLAGS_QUOTE) ? TrioCalcThousandSeparatorLength(integerDigits) : 0);
	if(isNegative || (flags & FLAGS_SHOWSIGN) || (flags & FLAGS_SPACE))
		expectedWidth += sizeof("-") - 1;
	if(exponentDigits > 0)
		expectedWidth += exponentDigits + ((requireTwoDigitExponent ? sizeof("E+0") : sizeof("E+")) - 1);
	if(isHex)
	...
	for(i = 0; i < integerDigits; i++) {
		...
		if(((flags & (FLAGS_FLOAT_E | FLAGS_QUOTE)) == FLAGS_QUOTE) && TrioFollowedBySeparator(integerDigits - i)) {
			for(groupingPointer = internalThousandSeparator; *groupingPointer != NIL; groupingPointer++) {
	...
	/* Output exponent */
	if(exponentDigits > 0) {
		self->OutStream(self, isHex ? ((flags & FLAGS_UPPER) ? 'P' : 'p') : ((flags & FLAGS_UPPER) ? 'E' : 'e'));
				...
				switch(parameters[i].type) {
					...
					case FORMAT_USER_DEFINED:
				    {
					    ...
					    if(parameters[i].user_name[0] == NIL) {
						    ...
						    /* Look up namespace */
						    def = TrioFindNamespace(parameters[i].user_name, 0);
					    }
					    if(def) {
						    reference.data = data;
						    reference.parameter = &parameters[i];
...
TRIO_PUBLIC int trio_printf(const char * format, TRIO_VA_DECL/*va_alist*/)
{
	...
	status = TrioFormat(stdout, 0, TrioOutStreamFile, format, TRIO_VA_LIST_ADDR(args), 0);
	TRIO_VA_END(args);
	return status;
}
...
TRIO_PUBLIC int trio_fprintf(FILE * file, const char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioFormat(file, 0, TrioOutStreamFile, format, TRIO_VA_LIST_ADDR(args), 0);
	TRIO_VA_END(args);
	return status;
}
/**
...
TRIO_PUBLIC int trio_dprintf(int fd, const char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioFormat(&fd, 0, TrioOutStreamFileDescriptor, format, TRIO_VA_LIST_ADDR(args), 0);
	TRIO_VA_END(args);
	return status;
}
/**
...
TRIO_PUBLIC int trio_cprintf(trio_outstream_t stream, trio_pointer_t closure, const char * format, TRIO_VA_DECL)
{
	...
	status = TrioFormat(&data, 0, TrioOutStreamCustom, format, TRIO_VA_LIST_ADDR(args), 0);
	TRIO_VA_END(args);
	return status;
}
...
TRIO_PUBLIC int trio_snprintf(char * buffer, size_t max, const char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioFormat(&buffer, max > 0 ? max - 1 : 0, TrioOutStreamStringMax, format, TRIO_VA_LIST_ADDR(args), 0);
	if(max > 0)
		*buffer = NIL;
	TRIO_VA_END(args);
	return status;
...
TRIO_PUBLIC int trio_snprintfv(char * buffer, size_t max, const char * format, trio_pointer_t * args)
{
	...
	assert(VALID(buffer));
	assert(VALID(format));
	status = TrioFormat(&buffer, max > 0 ? max - 1 : 0, TrioOutStreamStringMax, format, NULL, args);
	if(max > 0)
		*buffer = NIL;
	return status;
}
...
TRIO_PUBLIC char * trio_aprintf(const char * format, TRIO_VA_DECL)
{
	...
	if(info) {
		TRIO_VA_START(args, format);
		TrioFormat(info, 0, TrioOutStreamStringDynamic, format, TRIO_VA_LIST_ADDR(args), 0);
		TRIO_VA_END(args);
...
TRIO_PUBLIC int trio_asprintf(char ** result, const char * format, TRIO_VA_DECL)
{
	...
	*result = NULL;
	...
	if(info == NULL) {
		...
		TRIO_VA_START(args, format);
		status = TrioFormat(info, 0, TrioOutStreamStringDynamic, format, TRIO_VA_LIST_ADDR(args), 0);
		TRIO_VA_END(args);
		if(status >= 0) {
			trio_string_terminate(info);
			*result = trio_string_extract(info);
...
{
#if defined(FORMAT_USER_DEFINED)
	assert(static_cast<trio_reference_t *>(ref)->parameter->type == FORMAT_USER_DEFINED);
#endif
	return (static_cast<trio_reference_t *>(ref)->parameter->user_data);
}
...
trio_pointer_t trio_get_argument(trio_pointer_t ref)
{
	...
	assert(static_cast<trio_reference_t *>(ref)->parameter->type == FORMAT_USER_DEFINED);
#endif
	return static_cast<trio_reference_t *>(ref)->parameter->data.pointer;
}

int  trio_get_width(trio_pointer_t ref) { return static_cast<trio_reference_t *>(ref)->parameter->width; }
void trio_set_width(trio_pointer_t ref, int width) { static_cast<trio_reference_t *>(ref)->parameter->width = width; }
int  trio_get_precision(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->precision); }
void trio_set_precision(trio_pointer_t ref, int precision) { static_cast<trio_reference_t *>(ref)->parameter->precision = precision; }
int  trio_get_base(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->base); }
void trio_set_base(trio_pointer_t ref, int base) { static_cast<trio_reference_t *>(ref)->parameter->base = base; }
int  trio_get_long(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_LONG) ? TRUE : FALSE; }
void trio_set_long(trio_pointer_t ref, int is_long) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_LONG, is_long); }
int  trio_get_longlong(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_QUAD) ? TRUE : FALSE; }
void trio_set_longlong(trio_pointer_t ref, int is_longlong) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_QUAD, is_longlong); }
int  trio_get_longdouble(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_LONGDOUBLE) ? TRUE : FALSE; }
void trio_set_longdouble(trio_pointer_t ref, int is_longdouble) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_LONGDOUBLE, is_longdouble); }
int  trio_get_short(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_SHORT) ? TRUE : FALSE; }
void trio_set_short(trio_pointer_t ref, int is_short) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_SHORT, is_short); }
int  trio_get_shortshort(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_SHORTSHORT) ? TRUE : FALSE; }
void trio_set_shortshort(trio_pointer_t ref, int is_shortshort) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_SHORTSHORT, is_shortshort); }
int  trio_get_alternative(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_ALTERNATIVE) ? TRUE : FALSE; }
void trio_set_alternative(trio_pointer_t ref, int is_alternative) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_ALTERNATIVE, is_alternative); }
int  trio_get_alignment(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_LEFTADJUST) ? TRUE : FALSE; }
void trio_set_alignment(trio_pointer_t ref, int is_leftaligned) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_LEFTADJUST, is_leftaligned); }
int  trio_get_spacing(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_SPACE) ? TRUE : FALSE; }
void trio_set_spacing(trio_pointer_t ref, int is_space) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_SPACE, is_space); }
int  trio_get_sign(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_SHOWSIGN) ? TRUE : FALSE; }
void trio_set_sign(trio_pointer_t ref, int is_sign) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_SHOWSIGN, is_sign); }
int  trio_get_padding(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_NILPADDING) ? TRUE : FALSE; }
void trio_set_padding(trio_pointer_t ref, int is_padding) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_NILPADDING, is_padding); }
int  trio_get_quote(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_QUOTE) ? TRUE : FALSE; }
void trio_set_quote(trio_pointer_t ref, int is_quote) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_QUOTE, is_quote); }
int  trio_get_upper(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_UPPER) ? TRUE : FALSE; }
void trio_set_upper(trio_pointer_t ref, int is_upper) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_UPPER, is_upper); }
int  trio_get_ptrdiff(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_PTRDIFF_T) ? TRUE : FALSE; }
void trio_set_ptrdiff(trio_pointer_t ref, int is_ptrdiff) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_PTRDIFF_T, is_ptrdiff); }
#if TRIO_C99
	int  trio_get_largest(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_INTMAX_T) ? TRUE : FALSE; }
	void trio_set_largest(trio_pointer_t ref, int is_largest) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_INTMAX_T, is_largest); }
	int  trio_get_size(trio_pointer_t ref) { return (static_cast<trio_reference_t *>(ref)->parameter->flags & FLAGS_SIZE_T) ? TRUE : FALSE; }
	void trio_set_size(trio_pointer_t ref, int is_size) { SETFLAG(static_cast<trio_reference_t *>(ref)->parameter->flags, FLAGS_SIZE_T, is_size); }
#endif
...
int trio_print_ref(trio_pointer_t ref, const char * format, /*va_alist*/TRIO_VA_DECL)
{
	...
	status = TrioFormatRef((trio_reference_t*)ref, format, TRIO_VA_LIST_ADDR(arglist), 0);
	TRIO_VA_END(arglist);
	return status;
}
...
static int TrioGetCharacterClass(const char * format, int * indexPointer, trio_flags_t * flagsPointer, int * characterclass)
{
	...
	*flagsPointer &= ~FLAGS_EXCLUDE;
	...
	/* Collect characters */
	for(ch = format[index]; (ch != SPECIFIER_UNGROUP) && (ch != NIL); ch = format[++index]) {
		switch(ch) {
			...
			case SPECIFIER_GROUP:
			    switch(format[index + 1]) {
					 ...
					 * characters have been implemented. Until now, we ignore this feature.
					 */
					for(i = index + 2;; i++) {
						if(format[i] == NIL)
							return -1; /* Error in syntax */
				...
				break;
					...
					if(trio_equal_max(CLASS_ALNUM, sizeof(CLASS_ALNUM) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isalnum(i))
								characterclass[i]++;
						index += sizeof(CLASS_ALNUM) - 1;
					}
					else if(trio_equal_max(CLASS_ALPHA, sizeof(CLASS_ALPHA) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isalpha(i))
								characterclass[i]++;
						index += sizeof(CLASS_ALPHA) - 1;
					}
					else if(trio_equal_max(CLASS_CNTRL, sizeof(CLASS_CNTRL) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(iscntrl(i))
								characterclass[i]++;
						index += sizeof(CLASS_CNTRL) - 1;
					}
					else if(trio_equal_max(CLASS_DIGIT, sizeof(CLASS_DIGIT) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isdigit(i))
								characterclass[i]++;
						index += sizeof(CLASS_DIGIT) - 1;
					}
					else if(trio_equal_max(CLASS_GRAPH, sizeof(CLASS_GRAPH) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isgraph(i))
								characterclass[i]++;
						index += sizeof(CLASS_GRAPH) - 1;
					}
					else if(trio_equal_max(CLASS_LOWER, sizeof(CLASS_LOWER) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(islower(i))
								characterclass[i]++;
						index += sizeof(CLASS_LOWER) - 1;
					}
					else if(trio_equal_max(CLASS_PRINT, sizeof(CLASS_PRINT) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isprint(i))
								characterclass[i]++;
						index += sizeof(CLASS_PRINT) - 1;
					}
					else if(trio_equal_max(CLASS_PUNCT, sizeof(CLASS_PUNCT) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(ispunct(i))
								characterclass[i]++;
						index += sizeof(CLASS_PUNCT) - 1;
					}
					else if(trio_equal_max(CLASS_SPACE, sizeof(CLASS_SPACE) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isspace(i))
								characterclass[i]++;
						index += sizeof(CLASS_SPACE) - 1;
					}
					else if(trio_equal_max(CLASS_UPPER, sizeof(CLASS_UPPER) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isupper(i))
								characterclass[i]++;
						index += sizeof(CLASS_UPPER) - 1;
					}
					else if(trio_equal_max(CLASS_XDIGIT, sizeof(CLASS_XDIGIT) - 1, &format[index])) {
						for(i = 0; i < MAX_CHARACTER_CLASS; i++)
							if(isxdigit(i))
								characterclass[i]++;
...
static BOOLEAN_T TrioReadString(trio_class_t * self, char * target, trio_flags_t flags, int width)
{
	 ...
	 */
	for(i = 0; ((width == NO_WIDTH) || (i < width)) && (!((self->current == EOF) || isspace(self->current))); i++) {
...
static BOOLEAN_T TrioReadGroup(trio_class_t * self, char * target, int * characterclass, trio_flags_t flags, int width)
{
	...
	assert(VALID(self->InStream));
	ch = self->current;
	for(i = 0; ((width == NO_WIDTH) || (i < width)) && (!((ch == EOF) || (((flags & FLAGS_EXCLUDE) != 0) ^ (characterclass[ch] == 0)))); i++) {
		if(target)
			target[i] = (char)ch;
		self->InStream(self, &ch);
	}
...
static BOOLEAN_T TrioReadDouble(trio_class_t * self, trio_pointer_t target, trio_flags_t flags, int width)
{
	...
	if(ch == '.') {
		...
		doubleString[index++] = (char)ch;
		self->InStream(self, &ch);
		while((isHex ? isxdigit(ch) : isdigit(ch)) && (index - start < width)) {
			doubleString[index++] = (char)ch;
			self->InStream(self, &ch);
		}
...
static int TrioScanProcess(trio_class_t * data, const char * format, trio_parameter_t * parameters)
{
	...
	int assignment = 0;
	int ch;
	int index = 0; /* Index of format string */
	int i = 0; /* Index of current parameter */
	trio_flags_t flags;
	int width;
	int base;
	trio_pointer_t pointer;
	...
	while(format[index]) {
		...
		if(CHAR_IDENTIFIER == format[index]) {
			...
			// Find width 
			width = parameters[i].width;
			if(flags & FLAGS_WIDTH_PARAMETER) { // Get width from parameter list 
				width = (int)parameters[width].data.number.as_signed;
			}
			/* Find base */
			base = parameters[i].base;
			if(flags & FLAGS_BASE_PARAMETER) { // Get base from parameter list 
				base = (int)parameters[base].data.number.as_signed;
			}
			switch(parameters[i].type) {
				case FORMAT_INT:
			...
			if(isspace((int)format[index])) // Whitespaces may match any amount of whitespaces 
...
TRIO_PUBLIC int trio_cscanf(trio_instream_t stream, trio_pointer_t closure, const char * format, TRIO_VA_DECL)
{
	...
	data.closure = closure;
	status = TrioScan(&data, 0, TrioInStreamCustom, format, TRIO_VA_LIST_ADDR(args), 0);
	TRIO_VA_END(args);
	return status;
}
...
TRIO_PUBLIC const char * trio_strerror(int errorcode)
{
	...
	switch(TRIO_ERROR_CODE(errorcode)) {
		...
		case TRIO_ETOOMANY: return "Too many arguments";
		case TRIO_EDBLREF: return "Double reference";
		case TRIO_EGAP: return "Reference gap";
		case TRIO_ENOMEM: return "Out of memory";
		case TRIO_ERANGE: return "Invalid range";
		case TRIO_ECUSTOM: return "Custom error";
