...
TRIO_PRIVATE int TrioFormat(trio_pointer_t destination, size_t destinationSize, 
	void (*OutStream)TRIO_PROTO((trio_class_t *, int)), TRIO_CONST char * format, TRIO_VA_LIST_PTR arglist, trio_pointer_t * argarray)
{
	int status;
	trio_class_t data;
	trio_parameter_t parameters[MAX_PARAMETERS];
...
TRIO_PUBLIC int trio_snprintf(char * buffer, size_t max, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioFormat(&buffer, max > 0 ? max - 1 : 0, TrioOutStreamStringMax, format, TRIO_VA_LIST_ADDR(args), NULL);
	if(max > 0)
		*buffer = NIL;
	TRIO_VA_END(args);
	return status;
...
TRIO_PUBLIC int trio_snprintfcat(char * buffer, size_t max, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	status = TrioFormat(&buffer, max - 1 - buf_len, TrioOutStreamStringMax, format, TRIO_VA_LIST_ADDR(args), NULL);
	TRIO_VA_END(args);
	*buffer = NIL;
	return status;
}
...
TRIO_PUBLIC char * trio_aprintf TRIO_VARGS2((format, va_alist), TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	if(info) {
		TRIO_VA_START(args, format);
		TrioFormat(info, 0, TrioOutStreamStringDynamic, format, TRIO_VA_LIST_ADDR(args), NULL);
		TRIO_VA_END(args);
...
TRIO_PUBLIC int trio_asprintf(char ** result, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	*result = NULL;
	...
	if(info == NULL) {
		...
		TRIO_VA_START(args, format);
		status = TrioFormat(info, 0, TrioOutStreamStringDynamic, format, TRIO_VA_LIST_ADDR(args), NULL);
		TRIO_VA_END(args);
		if(status >= 0) {
			trio_string_terminate(info);
			*result = trio_string_extract(info);
 ...
 * TrioSkipWhitespaces
 */
TRIO_PRIVATE int TrioSkipWhitespaces(trio_class_t * self)
{
	int ch = self->current;
	while(isspace(ch)) {
 ...
 * TrioScan
 */
TRIO_PRIVATE int TrioScan(trio_pointer_t source, size_t sourceSize, void (*InStream)TRIO_PROTO((trio_class_t *, int *)),
...
TRIO_PUBLIC int trio_scanf TRIO_VARGS2((format, va_alist), TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioScan((trio_pointer_t)stdin, 0, TrioInStreamFile, format, TRIO_VA_LIST_ADDR(args), NULL);
	TRIO_VA_END(args);
	return status;
}
...
{
	assert(VALID(format));
	return TrioScan((trio_pointer_t)stdin, 0, TrioInStreamFile, format, TRIO_VA_LIST_ADDR(args), NULL);
}
...
TRIO_PUBLIC int trio_fscanf TRIO_VARGS3((file, format, va_alist), FILE * file, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioScan((trio_pointer_t)file, 0, TrioInStreamFile, format, TRIO_VA_LIST_ADDR(args), NULL);
	TRIO_VA_END(args);
	return status;
}
...
TRIO_PUBLIC int trio_vfscanf TRIO_ARGS3((file, format, args), FILE * file, TRIO_CONST char * format, va_list args)
{
	assert(VALID(file));
	assert(VALID(format));
	return TrioScan((trio_pointer_t)file, 0, TrioInStreamFile, format, TRIO_VA_LIST_ADDR(args), NULL);
}
...
TRIO_PUBLIC int trio_dscanf TRIO_VARGS3((fd, format, va_alist), int fd, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioScan((trio_pointer_t)&fd, 0, TrioInStreamFileDescriptor, format, TRIO_VA_LIST_ADDR(args), NULL);
	TRIO_VA_END(args);
	return status;
}
...
{
	assert(VALID(format));
	return TrioScan((trio_pointer_t)&fd, 0, TrioInStreamFileDescriptor, format, TRIO_VA_LIST_ADDR(args), NULL);
}
...
TRIO_PUBLIC int trio_sscanf TRIO_VARGS3((buffer, format, va_alist), TRIO_CONST char * buffer, TRIO_CONST char * format, TRIO_VA_DECL)
{
	...
	assert(VALID(format));
	TRIO_VA_START(args, format);
	status = TrioScan((trio_pointer_t)&buffer, 0, TrioInStreamString, format, TRIO_VA_LIST_ADDR(args), NULL);
	TRIO_VA_END(args);
	return status;
}
...
TRIO_PUBLIC int trio_vsscanf TRIO_ARGS3((buffer, format, args), TRIO_CONST char * buffer, TRIO_CONST char * format, va_list args)
{
	assert(VALID(buffer));
	assert(VALID(format));
	return TrioScan((trio_pointer_t)&buffer, 0, TrioInStreamString, format, TRIO_VA_LIST_ADDR(args), NULL);
}
