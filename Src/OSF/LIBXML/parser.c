...
static void xmlFatalErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * info)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, 0, info, NULL, NULL, 0, 0, &errstr[0], info);
	if(ctxt) {
		ctxt->wellFormed = 0;
		if(ctxt->recovery == 0)
			ctxt->disableSAX = 1;
...
static void xmlFatalErrMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "%s", msg);
	if(ctxt) {
		ctxt->wellFormed = 0;
		if(ctxt->recovery == 0)
			ctxt->disableSAX = 1;
...
static void xmlWarningMsg(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	...
	if(ctxt && (ctxt->disableSAX != 0) && (ctxt->instate == XML_PARSER_EOF))
		return;
	if(ctxt && (ctxt->sax != NULL) && (ctxt->sax->initialized == XML_SAX2_MAGIC))
...
static void xmlFatalErrMsgInt(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, int val)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, val, 0, msg, val);
	if(ctxt) {
		ctxt->wellFormed = 0;
		if(ctxt->recovery == 0)
			ctxt->disableSAX = 1;
...
static void xmlFatalErrMsgStrIntStr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * str1, int val, const xmlChar * str2)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, 0, (const char*)str1, (const char*)str2, NULL, val, 0, msg, str1, val, str2);
	if(ctxt) {
		ctxt->wellFormed = 0;
		if(ctxt->recovery == 0)
			ctxt->disableSAX = 1;
...
static void xmlFatalErrMsgStr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * val)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, 0, (const char*)val, NULL, NULL, 0, 0, msg, val);
	if(ctxt) {
		ctxt->wellFormed = 0;
		if(ctxt->recovery == 0)
			ctxt->disableSAX = 1;
...
static void xmlErrMsgStr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * val)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_ERROR, NULL, 0, (const char*)val, NULL, NULL, 0, 0, msg, val);
}
...
static void xmlNsErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * info1, const xmlChar * info2, const xmlChar * info3)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_ERROR, NULL, 0, (const char*)info1, (const char*)info2, (const char*)info3, 0, 0, msg, info1, info2, info3);
	if(ctxt)
		ctxt->nsWellFormed = 0;
}
...
static void xmlNsWarn(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * info1, const xmlChar * info2, const xmlChar * info3)
{
	...
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_NAMESPACE, error,
	    XML_ERR_WARNING, NULL, 0, (const char*)info1, (const char*)info2, (const char*)info3, 0, 0, msg, info1, info2, info3);
}
...
static void xmlAddDefAttrs(xmlParserCtxtPtr ctxt, const xmlChar * fullname, const xmlChar * fullattr, const xmlChar * value)
{
	...
	if(defaults == NULL) {
		...
		xmlDefAttrsPtr temp = (xmlDefAttrsPtr)xmlRealloc(defaults, sizeof(xmlDefAttrs) + (2 * defaults->maxAttrs * 5) * sizeof(const xmlChar *));
		if(temp == NULL)
			goto mem_error;
		defaults = temp;
		defaults->maxAttrs *= 2;
...
int nodePush(xmlParserCtxtPtr ctxt, xmlNodePtr value)
{
	...
	if(ctxt->nodeNr >= ctxt->nodeMax) {
		xmlNodePtr * tmp = (xmlNodePtr*)xmlRealloc(ctxt->nodeTab, ctxt->nodeMax * 2 * sizeof(ctxt->nodeTab[0]));
		if(tmp == NULL) {
			xmlErrMemory(ctxt, NULL);
			return (-1);
		}
...
xmlNodePtr nodePop(xmlParserCtxtPtr ctxt)
{
	...
	if(ctxt->nodeNr <= 0)
		return 0;
	ctxt->nodeNr--;
	ctxt->node = (ctxt->nodeNr > 0) ? ctxt->nodeTab[ctxt->nodeNr - 1] : NULL;
 ...
 */
static const xmlChar * FASTCALL nameNsPop(xmlParserCtxtPtr ctxt)
{
	const xmlChar * ret = 0;
	if(ctxt->nameNr > 0) {
		ctxt->nameNr--;
...
static int FASTCALL spacePush(xmlParserCtxtPtr ctxt, int val)
{
	if(ctxt->spaceNr >= ctxt->spaceMax) {
		ctxt->spaceMax *= 2;
		int * tmp = (int*)xmlRealloc(ctxt->spaceTab, ctxt->spaceMax * sizeof(ctxt->spaceTab[0]));
		if(tmp == NULL) {
			xmlErrMemory(ctxt, NULL);
			ctxt->spaceMax /= 2;
			return -1;
...
{
	return (ctxt && str) ? xmlStringLenDecodeEntities(ctxt, str, xmlStrlen(str), what, end, end2, end3) : 0;
}
...
static int xmlIsNameStartChar(xmlParserCtxtPtr ctxt, int c)
{
	if((ctxt->options & XML_PARSE_OLD10) == 0) {
		 ...
		 * Update 5 of XML-1.0
		 */
		if((c != ' ') && (c != '>') && (c != '/') && /* accelerators */ (IsLetterASCII(c) ||
			(c == '_') || (c == ':') ||
			((c >= 0xC0) && (c <= 0xD6)) ||
			((c >= 0xD8) && (c <= 0xF6)) ||
			((c >= 0xF8) && (c <= 0x2FF)) ||
...
static const xmlChar * FASTCALL xmlParseNCName(xmlParserCtxtPtr ctxt)
{
	const xmlChar * ret = 0;
#ifdef DEBUG
	nbParseNCName++;
...
static xmlChar * xmlParseStringName(xmlParserCtxtPtr ctxt, const xmlChar** str)
{
	...
	while(xmlIsNameChar(ctxt, c)) {
		...
		if(len >= XML_MAX_NAMELEN) { /* test bigentname.xml */
			 ...
			 */
			int max = len * 2;
			xmlChar * buffer = (xmlChar*)xmlMallocAtomic(max * sizeof(xmlChar));
			if(buffer == NULL) {
				xmlErrMemory(ctxt, NULL);
				return 0;
			}
...
xmlChar * xmlParseEntityValue(xmlParserCtxtPtr ctxt, xmlChar ** orig)
{
	...
	while(((IS_CHAR(c)) && ((c != stop) || /* checked */ (ctxt->input != input))) && (ctxt->instate != XML_PARSER_EOF)) {
		if(len + 5 >= size) {
			size *= 2;
			xmlChar * tmp = (xmlChar*)xmlRealloc(buf, size * sizeof(xmlChar));
			if(tmp == NULL) {
				xmlErrMemory(ctxt, NULL);
				free(buf);
				return 0;
		...
		if(isParameter) {
			...
			if((RAW == '"') || (RAW == '\'')) {
				...
				if(CMP5(CUR_PTR, 'N', 'D', 'A', 'T', 'A')) {
					 ...
					 */
					if(ctxt->replaceEntities && (!ctxt->myDoc || (xmlStrEqual(ctxt->myDoc->version, SAX_COMPAT_MODE)))) {
						if(ctxt->myDoc == NULL) {
							ctxt->myDoc = xmlNewDoc(SAX_COMPAT_MODE);
							if(ctxt->myDoc == NULL) {
								xmlErrMemory(ctxt, "New Doc failed");
...
static xmlEntityPtr xmlParseStringEntityRef(xmlParserCtxtPtr ctxt, const xmlChar ** str)
{
	 ...
	 * not contain a <.
	 */
	else if((ctxt->instate == XML_PARSER_ATTRIBUTE_VALUE) && (ent != NULL) && (ent->content != NULL) &&
	    (ent->etype != XML_INTERNAL_PREDEFINED_ENTITY) && (xmlStrchr(ent->content, '<'))) {
		xmlFatalErrMsgStr(ctxt, XML_ERR_LT_IN_ATTRIBUTE, "'<' in entity '%s' is not allowed in attributes values\n", name);
	}
 ...
 */
static const xmlChar * FASTCALL xmlGetNamespace(xmlParserCtxtPtr ctxt, const xmlChar * prefix)
{
...
static const xmlChar * FASTCALL xmlParseQName(xmlParserCtxtPtr ctxt, const xmlChar ** prefix)
{
	const xmlChar * l, * p;
	GROW;
	...
	while(IS_CHAR(cur) && ((r != ']') || (s != ']') || (cur != '>'))) {
		if(len + 5 >= size) {
			...
			if((size > XML_MAX_TEXT_LENGTH) && ((ctxt->options & XML_PARSE_HUGE) == 0)) {
				xmlFatalErrMsgStr(ctxt, XML_ERR_CDATA_NOT_FINISHED, "CData section too big found", NULL);
				free(buf);
				return;
			}
...
xmlChar * xmlParseVersionNum(xmlParserCtxtPtr ctxt)
{
	...
	int size = 10;
	xmlChar cur;
	xmlChar * buf = (xmlChar*)xmlMallocAtomic(size * sizeof(xmlChar));
	if(buf == NULL) {
		xmlErrMemory(ctxt, NULL);
		return 0;
	}
...
int xmlParseDocument(xmlParserCtxtPtr ctxt)
{
	 ...
	 */
	if(ctxt->myDoc && xmlStrEqual(ctxt->myDoc->version, SAX_COMPAT_MODE)) {
...
int xmlParseCtxtExternalEntity(xmlParserCtxtPtr ctx, const xmlChar * URL,
	...
	if((CMP5(CUR_PTR, '<', '?', 'x', 'm', 'l')) && (IS_BLANK_CH(NXT(5)))) {
		 ...
		 */
		if((xmlStrEqual(ctx->version, BAD_CAST "1.0")) && (!xmlStrEqual(ctxt->input->version, BAD_CAST "1.0"))) {
...
xmlParserErrors xmlParseInNodeContext(xmlNodePtr node, const char * data, int datalen, int options, xmlNodePtr * lst)
{
	...
	while(node && !oneof3(node->type, XML_ELEMENT_NODE, XML_DOCUMENT_NODE, XML_HTML_DOCUMENT_NODE))
		node = node->parent;
	if(node == NULL)
		return XML_ERR_INTERNAL_ERROR;
	doc = (node->type == XML_ELEMENT_NODE) ? node->doc : (xmlDocPtr)node;
	if(doc == NULL)
		return(XML_ERR_INTERNAL_ERROR);
	/*
 ...
 * current scope
 */
#define DICT_FREE(str) if((str) && ((!dict) || (xmlDictOwns(dict, (const xmlChar*)(str)) == 0))) free((char*)(str));
...
xmlDocPtr xmlReadFile(const char * filename, const char * encoding, int options)
{
	...
	return ctxt ? xmlDoRead(ctxt, NULL, encoding, options, 0) : 0;
}
...
xmlDocPtr xmlReadMemory(const char * buffer, int size, const char * URL, const char * encoding, int options)
{
	...
	xmlInitParser();
	ctxt = xmlCreateMemoryParserCtxt(buffer, size);
	return ctxt ? xmlDoRead(ctxt, URL, encoding, options, 0) : 0;
}
