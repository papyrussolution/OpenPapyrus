...
#define SAX_COMPAT_MODE reinterpret_cast<const xmlChar *>("SAX compatibility mode document")
...
{
	if(!xmlParserCtxt::IsEofInNonSaxMode(ctxt)) {
		xmlStructuredErrorFunc schannel = (ctxt && ctxt->sax && ctxt->sax->initialized == XML_SAX2_MAGIC) ? ctxt->sax->serror : 0;
		if(ctxt) {
...
static void xmlAddDefAttrs(xmlParserCtxt * ctxt, const xmlChar * fullname, const xmlChar * fullattr, const xmlChar * value)
{
	...
	defaults->values[5 * defaults->nbAttrs + 4] = ctxt->external ? (reinterpret_cast<const xmlChar *>("external")) : 0;
	defaults->nbAttrs++;
	return;
mem_error:
	xmlErrMemory(ctxt, 0);
...
static void xmlAddSpecialAttr(xmlParserCtxt * ctxt, const xmlChar * fullname, const xmlChar * fullattr, int type)
{
	...
	if(xmlHashLookup2(ctxt->attsSpecial, fullname, fullattr))
		return;
	xmlHashAddEntry2(ctxt->attsSpecial, fullname, fullattr, reinterpret_cast<void *>(type));
	return;
mem_error:
	xmlErrMemory(ctxt, 0);
	return;
...
static int nsPush(xmlParserCtxt * ctxt, const xmlChar * prefix, const xmlChar * URL)
{
	...
	if((ctxt->nsMax == 0) || (ctxt->nsTab == NULL)) {
		ctxt->nsMax = 10;
		ctxt->nsNr = 0;
		ctxt->nsTab = static_cast<const xmlChar **>(SAlloc::M(ctxt->nsMax * sizeof(xmlChar *)));
		if(ctxt->nsTab == NULL) {
			xmlErrMemory(ctxt, 0);
			ctxt->nsMax = 0;
			return -1;
		...
		const xmlChar ** tmp;
		ctxt->nsMax *= 2;
		tmp = static_cast<const xmlChar **>(SAlloc::R((char *)ctxt->nsTab, ctxt->nsMax * sizeof(ctxt->nsTab[0])));
		if(!tmp) {
			xmlErrMemory(ctxt, 0);
			ctxt->nsMax /= 2;
			return -1;
...
#define CMP4(s, c1, c2, c3, c4)	(PTR8C(s)[0] == c1 && PTR8C(s)[1] == c2 && PTR8C(s)[2] == c3 && PTR8C(s)[3] == c4)
#define CMP5(s, c1, c2, c3, c4, c5) (CMP4(s, c1, c2, c3, c4) && PTR8C(s)[4] == c5)
#define CMP6(s, c1, c2, c3, c4, c5, c6)	(CMP5(s, c1, c2, c3, c4, c5) && PTR8C(s)[5] == c6)
#define CMP7(s, c1, c2, c3, c4, c5, c6, c7) (CMP6(s, c1, c2, c3, c4, c5, c6) && PTR8C(s)[6] == c7)
#define CMP8(s, c1, c2, c3, c4, c5, c6, c7, c8)	(CMP7(s, c1, c2, c3, c4, c5, c6, c7) && PTR8C(s)[7] == c8)
#define CMP9(s, c1, c2, c3, c4, c5, c6, c7, c8, c9) (CMP8(s, c1, c2, c3, c4, c5, c6, c7, c8) && PTR8C(s)[8] == c9)
#define CMP10(s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10) (CMP9(s, c1, c2, c3, c4, c5, c6, c7, c8, c9) && PTR8C(s)[9] == c10)
...
#define GROW if(!ctxt->progressive && (ctxt->input->end - ctxt->input->cur < INPUT_CHUNK)) xmlGROW(ctxt);
...
#define COPY_BUF(l, b, i, v) if(l == 1) b[i++] = static_cast<xmlChar>(v); else i += xmlCopyCharMultiByte(&b[i], v)
...
static xmlParserInput * xmlNewBlanksWrapperInputStream(xmlParserCtxt * ctxt, xmlEntity * entity)
{
	...
	if(!entity) {
		...
		if(input) {
			...
			xmlChar * buffer = static_cast<xmlChar *>(SAlloc::M(length));
			if(!buffer) {
				xmlErrMemory(ctxt, 0);
				SAlloc::F(input);
			}
				...
				input->FnFree = deallocblankswrapper;
				input->base = buffer;
				input->cur = buffer;
				input->length = length;
...
void FASTCALL xmlParserHandlePEReference(xmlParserCtxt * ctxt)
{
	...
	if(!name) {
		...
		if(RAW == ';') {
			...
			else if(ctxt->input->FnFree != deallocblankswrapper) {
...
xmlChar * xmlStringLenDecodeEntities(xmlParserCtxt * ctxt, const xmlChar * str, int len, int what, xmlChar end, xmlChar end2, xmlChar end3)
{
	...
	buffer = static_cast<xmlChar *>(SAlloc::M(buffer_size));
	if(!buffer)
		goto mem_error;
	/*
 ...
 */
static int FASTCALL xmlIsNameStartChar(const xmlParserCtxt * ctxt, int c)
{
...
static int FASTCALL xmlIsNameChar(const xmlParserCtxt * ctxt, int c)
{
	if((ctxt->options & XML_PARSE_OLD10) == 0) {
		/*
...
static const xmlChar * xmlParseNameAndCompare(xmlParserCtxt * ctxt, xmlChar const * other)
{
	...
	if(!ctxt->IsEof()) {
		...
		if(*cmp == 0 && (*in == '>' || IS_BLANK_CH(*in))) {
			/* success */
			ctxt->input->cur = in;
			ret = reinterpret_cast<const xmlChar *>(1);
		}
		else {
...
xmlChar * xmlParseEntityValue(xmlParserCtxt * ctxt, xmlChar ** orig)
{
	 ...
	 */
	while(((IS_CHAR(c)) && ((c != stop) || /* checked */ (ctxt->input != input))) && !ctxt->IsEof()) {
		if(len + 5 >= size) {
			size *= 2;
...
static xmlChar * xmlParseAttValueComplex(xmlParserCtxt * ctxt, int * attlen, int normalize)
{
	 ...
	 */
	c = CUR_CHAR(l);
	while(((NXT(0) != limit) && /* checked */ (IS_CHAR(c)) && (c != '<')) && !ctxt->IsEof()) {
		/*
...
static void xmlParseCommentComplex(xmlParserCtxt * ctxt, xmlChar * buf, size_t len, size_t size)
{
	...
	if(cur == 0) {
		...
		xmlNextChar(ctxt);
		if(ctxt->IsSaxInUse() && ctxt->sax->comment)
...
static void xmlParseCatalogPI(xmlParserCtxt * ctxt, const xmlChar * catalog)
{
	...
	while(IS_BLANK_CH(*tmp))
		tmp++;
	if(xmlStrncmp(tmp, reinterpret_cast<const xmlChar *>("catalog"), 7))
		goto error;
	tmp += 7;
	...
	if((RAW == '<') && (NXT(1) == '?')) {
		...
		if(target) {
			if((RAW == '?') && (NXT(1) == '>')) {
				 ...
				 * SAX: PI detected.
				 */
				if(ctxt->IsSaxInUse() && ctxt->sax->processingInstruction)
		...
		if(!IS_BLANK_CH(CUR)) {
			...
			if(!name) {
				...
				if(RAW == '>') {
					...
					xmlNextChar(ctxt);
					if(ctxt->IsSaxInUse() && ctxt->sax->notationDecl)
	...
	if(CMP8(CUR_PTR, '<', '!', 'E', 'N', 'T', 'I', 'T', 'Y')) {
		...
		if(isParameter) {
			if((RAW == '"') || (RAW == '\'')) {
				...
				if(value) {
					if(ctxt->IsSaxInUse() && ctxt->sax->entityDecl)
			...
			if((RAW == '"') || (RAW == '\'')) {
				...
				if((ctxt->myDoc == NULL) || (sstreq(ctxt->myDoc->version, SAX_COMPAT_MODE))) {
					...
					SETIFZ(ctxt->myDoc->intSubset, xmlNewDtd(ctxt->myDoc, reinterpret_cast<const xmlChar *>("fake"), NULL, NULL));
				...
				if(CMP5(CUR_PTR, 'N', 'D', 'A', 'T', 'A')) {
					...
					if(ctxt->IsSaxInUse() && ctxt->sax->unparsedEntityDecl)
...
void xmlParseAttributeListDecl(xmlParserCtxt * ctxt)
{
	...
	if(CMP9(CUR_PTR, '<', '!', 'A', 'T', 'T', 'L', 'I', 'S', 'T')) {
		...
		while((RAW != '>') && !ctxt->IsEof()) {
			...
			if(ctxt->IsSaxInUse() && ctxt->sax->attributeDecl)
 ...
 */
static xmlElementContent * FASTCALL xmlParseElementChildrenContentDeclPriv(xmlParserCtxt * ctxt, int inputchk, int depth)
{
	xmlElementContent * ret = NULL;
	xmlElementContent * cur = NULL;
	...
	if(RAW == '(') {
		...
		xmlNextChar(ctxt);
		SKIP_BLANKS;
		cur = ret = xmlParseElementChildrenContentDeclPriv(ctxt, inputid, depth + 1); // @recursion
		SKIP_BLANKS;
		GROW;
	}
	else {
	...
	while((RAW != ')') && !ctxt->IsEof()) {
		...
		if(RAW == '(') {
			...
			xmlNextChar(ctxt);
			SKIP_BLANKS;
			last = xmlParseElementChildrenContentDeclPriv(ctxt, inputid, depth + 1); // @recursion
			SKIP_BLANKS;
		}
		else {
			elem = xmlParseName(ctxt);
	...
	if(RAW == '?') {
		...
		if(ret) {
			int found = 0;
			ret->ocur = oneof2(ret->ocur, XML_ELEMENT_CONTENT_OPT, XML_ELEMENT_CONTENT_MULT) ? XML_ELEMENT_CONTENT_MULT : XML_ELEMENT_CONTENT_PLUS;
			// 
			// Some normalization:
			// (a | b*)+ == (a | b)*
			// (a | b?)+ == (a | b)*
...
int xmlParseElementDecl(xmlParserCtxt * ctxt)
{
	...
	if(CMP9(CUR_PTR, '<', '!', 'E', 'L', 'E', 'M', 'E', 'N', 'T')) {
		...
		if(RAW != '>') {
			...
			xmlNextChar(ctxt);
			if(ctxt->IsSaxInUse() && ctxt->sax->elementDecl) {
				if(content)
					content->parent = NULL;
...
static void xmlParseConditionalSections(xmlParserCtxt * ctxt) 
{
	...
	if(CMP7(CUR_PTR, 'I', 'N', 'C', 'L', 'U', 'D', 'E')) {
		...
		while(((RAW != 0) && ((RAW != ']') || (NXT(1) != ']') || (NXT(2) != '>'))) && !ctxt->IsEof()) {
			const xmlChar * check = CUR_PTR;
		...
		while(((depth >= 0) && (RAW != 0)) && !ctxt->IsEof()) {
			if((RAW == '<') && (NXT(1) == '!') && (NXT(2) == '[')) {
				depth++;
				SKIP(3);
				continue;
...
void xmlParseReference(xmlParserCtxt * ctxt)
{
	...
	if(NXT(1) == '#') {
		...
		if(value) {
			if(ctxt->charset != XML_CHAR_ENCODING_UTF8) {
				...
				if(value <= 0xFF) {
					out[0] = value;
					out[1] = 0;
					if(ctxt->IsSaxInUse() && ctxt->sax->characters)
					...
					if(ctxt->IsSaxInUse() && ctxt->sax->reference)
		...
		if(!ent->children) {
			...
			if(ctxt->IsSaxInUse() && ctxt->sax->reference && !ctxt->replaceEntities) {
				// 
		...
		// 
		else if(ctxt->IsSaxInUse() && ctxt->sax->reference && !ctxt->replaceEntities) {
			...
			if(ctxt->P_Node && ent->children) {
				...
				if((!list && !ent->owner) || (ctxt->parseMode == XML_PARSE_READER)) {
					...
					// 
					const xmlChar * nbktext = xmlDictLookupSL(ctxt->dict, reinterpret_cast<const xmlChar *>("nbktext"));
...
xmlEntity * xmlParseEntityRef(xmlParserCtxt * ctxt)
{
	...
	if(!ent) {
		if((ctxt->standalone == 1) || ((ctxt->hasExternalSubset == 0) && (ctxt->hasPErefs == 0))) {
			...
			if((ctxt->inSubset == 0) && ctxt->sax && ctxt->sax->reference) {
	 ...
	 * not contain a <.
	 */
	else if((ctxt->instate == XML_PARSER_ATTRIBUTE_VALUE) && ent && (ent->etype != XML_INTERNAL_PREDEFINED_ENTITY)) {
		if(((ent->checked & 1) || (ent->checked == 0)) && ent->content && (xmlStrchr(ent->content, '<'))) {
...
static int xmlLoadEntityContent(xmlParserCtxt * ctxt, xmlEntity * entity)
{
	...
	int l, c;
	int count = 0;
	if(!ctxt || !entity || !oneof2(entity->etype, XML_EXTERNAL_PARAMETER_ENTITY, XML_EXTERNAL_GENERAL_PARSED_ENTITY) || entity->content) {
...
void xmlParseDocTypeDecl(xmlParserCtxt * ctxt)
{
	 ...
	 */
	if(ctxt->IsSaxInUse() && ctxt->sax->internalSubset)
...
const xmlChar * xmlParseStartTag(xmlParserCtxt * ctxt)
{
	...
	if(RAW == '<') {
		...
		if(!name) {
			...
			SKIP_BLANKS;
			GROW;
			while((RAW != '>' && (RAW != '/' || NXT(1) != '>') && (IS_BYTE_CHAR(RAW))) && !ctxt->IsEof()) {
				...
				if(attname && attvalue) {
					...
					if(!atts) {
						...
						maxatts *= 2;
						const xmlChar ** n = static_cast<const xmlChar **>(SAlloc::R((void *)atts, maxatts * sizeof(const xmlChar *)));
						if(!n) {
							xmlErrMemory(ctxt, 0);
							SAlloc::F(attvalue);
							goto failed;
...
failed:
			...
			// 
			if(ctxt->IsSaxInUse() && ctxt->sax->startElement) {
...
static void xmlParseEndTag1(xmlParserCtxt * ctxt, int line)
{
	...
	if(name != (xmlChar *)1) {
		if(!name)
			name = reinterpret_cast<const xmlChar *>("unparseable");
 ...
 */
static const xmlChar * FASTCALL xmlGetNamespace(const xmlParserCtxt * ctxt, const xmlChar * prefix)
{
	...
	if(prefix == NULL)
		...
		if((*cmp == 0) && (*in == ':')) {
			...
			if(*cmp == 0 && (*in == '>' || IS_BLANK_CH(*in))) {
				// success 
				ctxt->input->cur = in;
				return reinterpret_cast<const xmlChar *>(1);
			}
		}
		// all strings coms from the dictionary, equality can be done directly
		ret = xmlParseQName(ctxt, &prefix2);
		return ((ret == name) && (prefix == prefix2)) ? reinterpret_cast<const xmlChar *>(1) : ret;
	}
}
/**
...
failed:
	...
	*URI = nsname;
	...
	// SAX: Start of Element !
	// 
	if(ctxt->IsSaxInUse() && ctxt->sax->startElementNs) {
		if(nbNs > 0)
...
static void xmlParseEndTag2(xmlParserCtxt * ctxt, const xmlChar * prefix, const xmlChar * URI, int line, int nsNr, int tlen)
{
	 ...
	 */
	if(name != (xmlChar *)1) {
		SETIFZ(name, reinterpret_cast<const xmlChar *>("unparseable"));
		if((line == 0) && ctxt->P_Node)
	...
	if(ctxt->IsSaxInUse() && ctxt->sax->endElementNs)
...
{
	GROW;
	while((RAW != 0) && ((RAW != '<') || (NXT(1) != '/')) && (!ctxt->IsEof())) {
...
const xmlChar * xmlParseEncodingDecl(xmlParserCtxt * ctxt)
{
	...
	if(CMP8(CUR_PTR, 'e', 'n', 'c', 'o', 'd', 'i', 'n', 'g')) {
		 ...
		 */
		if(encoding && (sstreqi_ascii(encoding, reinterpret_cast<const xmlChar *>("UTF-16")) || sstreqi_ascii(encoding, reinterpret_cast<const xmlChar *>("UTF16")))) {
			/*
		 ...
		 */
		else if(encoding && (sstreqi_ascii(encoding, reinterpret_cast<const xmlChar *>("UTF-8")) || sstreqi_ascii(encoding, reinterpret_cast<const xmlChar *>("UTF8")))) {
			SAlloc::F((xmlChar *)ctxt->encoding);
			ctxt->encoding = encoding;
		}
		else if(encoding) {
...
{
	while(!ctxt->IsEof() && (((RAW == '<') && (NXT(1) == '?')) || (CMP4(CUR_PTR, '<', '!', '-', '-')) || IS_BLANK_CH(CUR))) {
		if((RAW == '<') && (NXT(1) == '?')) {
			xmlParsePI(ctxt);
		}
		else if(IS_BLANK_CH(CUR)) {
		...
		switch(ctxt->instate) {
			...
			case XML_PARSER_CDATA_SECTION: {
			    ...
			    if(base < 0) {
				    ...
				    if(ctxt->IsSaxInUse() && base == 0 && ctxt->sax->cdataBlock) {
					    // 
				    ...
				    else if(ctxt->IsSaxInUse() && base > 0) {
					    if(ctxt->sax->cdataBlock)
 ...
 */
static int xmlParseCheckTransition(const xmlParserCtxt * ctxt, const char * chunk, int size)
{
...
xmldecl_done:
	if(size > 0 && chunk && ctxt->input && ctxt->input->buf && (!ctxt->IsEof())) {
		size_t base = xmlBufGetInputBase(ctxt->input->buf->buffer, ctxt->input);
...
xmlParserCtxt * xmlCreatePushParserCtxt(xmlSAXHandler * sax, void * user_data, const char * chunk, int size, const char * filename)
{
	...
	else if(ctxt->input && ctxt->input->buf) {
		size_t base = xmlBufGetInputBase(ctxt->input->buf->buffer, ctxt->input);
		size_t cur = ctxt->input->cur - ctxt->input->base;
 ...
 * Returns the new parser context or NULL
 */
xmlParserCtxt * xmlCreateIOParserCtxt(xmlSAXHandler * sax, void * user_data, xmlInputReadCallback ioread, xmlInputCloseCallback ioclose,
...
xmlDtd * xmlIOParseDTD(xmlSAXHandler * sax, xmlParserInputBuffer * input, xmlCharEncoding enc)
{
	...
	ctxt->myDoc->properties = XML_DOC_INTERNAL;
	ctxt->myDoc->extSubset = xmlNewDtd(ctxt->myDoc, reinterpret_cast<const xmlChar *>("none"), reinterpret_cast<const xmlChar *>("none"), reinterpret_cast<const xmlChar *>("none"));
	...
	xmlParseExternalSubset(ctxt, reinterpret_cast<const xmlChar *>("none"), reinterpret_cast<const xmlChar *>("none"));
	if(ctxt->myDoc) {
		if(ctxt->wellFormed) {
...
xmlDtd * xmlSAXParseDTD(xmlSAXHandler * sax, const xmlChar * ExternalID, const xmlChar * SystemID)
{
	...
	if(!input->filename)
		input->filename = reinterpret_cast<const char *>(systemIdCanonic);
	else
		SAlloc::F(systemIdCanonic);
	input->line = 1;
	input->col = 1;
	...
	ctxt->myDoc->properties = XML_DOC_INTERNAL;
	ctxt->myDoc->extSubset = xmlNewDtd(ctxt->myDoc, reinterpret_cast<const xmlChar *>("none"), ExternalID, SystemID);
...
int xmlParseCtxtExternalEntity(xmlParserCtxt * ctx, const xmlChar * URL, const xmlChar * ID, xmlNode ** lst) 
{
	...
	if(!URL && !ID)
		return -1;
	if(!ctx->myDoc) /* @@ relax but check for dereferences */
		return -1;
	...
	ctxt->sax = ctx->sax;
	xmlDetectSAX2(ctxt);
	newDoc = xmlNewDoc(reinterpret_cast<const xmlChar *>("1.0"));
	if(!newDoc) {
		xmlFreeParserCtxt(ctxt);
		return -1;
	}
	...
	newRoot = xmlNewDocNode(newDoc, NULL, reinterpret_cast<const xmlChar *>("pseudoroot"), 0);
	if(newRoot == NULL) {
		ctxt->sax = oldsax;
		xmlFreeParserCtxt(ctxt);
	...
	ctxt->dict = ctx->dict;
	ctxt->str_xml = xmlDictLookup(ctxt->dict, reinterpret_cast<const xmlChar *>("xml"), 3);
	ctxt->str_xmlns = xmlDictLookup(ctxt->dict, reinterpret_cast<const xmlChar *>("xmlns"), 5);
 ...
 *  the parser error code otherwise
 */
static xmlParserErrors xmlParseExternalEntityPrivate(xmlDoc * doc, xmlParserCtxt * oldctxt, xmlSAXHandler * sax, 
