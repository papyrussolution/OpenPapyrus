...
turtle_comparison:
	 ...
	 */
	for(depth2 = 0, cur = node2; cur->parent; cur = cur->parent) {
		if(cur == node1)
			return 1;
		depth2++;
	}
	root = cur;
	for(depth1 = 0, cur = node1; cur->parent; cur = cur->parent) {
		if(cur == node2)
			return -1;
		depth1++;
	}
 ...
 */
static int xmlXPathGetSign(double val)
{
...
void xmlXPathErr(xmlXPathParserContextPtr ctxt, int error)
{
	...
	if(ctxt->context->error) {
		...
		__xmlRaiseError(0, 0, 0, 0, ctxt->context->debugNode, XML_FROM_XPATH,
		    error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK,
		    XML_ERR_ERROR, NULL, 0, (const char*)ctxt->base, NULL, NULL,
		    ctxt->cur - ctxt->base, 0, "%s", xmlXPathErrorMessages[error]);
	}
}
/**
 * xmlXPatherror:
...
static int xmlPointerListAddSize(xmlPointerListPtr list, void * item, int initialSize)
{
	if(list->items == NULL) {
		...
		list->size *= 2;
		list->items = (void**)SAlloc::R(list->items,
...
{
	xmlPointerListPtr ret = (xmlPointerListPtr)SAlloc::M(sizeof(xmlPointerList));
	if(!ret) {
		xmlXPathErrMemory(NULL, "xmlPointerListCreate: allocating item");
	}
	else {
...
static void xmlXPathDebugDumpValueTree(FILE * output, xmlNodeSetPtr cur, int depth)
{
	...
	for(i = 0; ((i < depth) && (i < 25)); i++)
		shift[2 * i] = shift[2 * i + 1] = ' ';
	shift[2 * i] = shift[2 * i + 1] = 0;
	if(!cur || (cur->nodeNr == 0) || (cur->nodeTab[0] == NULL)) {
		fprintf(output, "%s", shift);
...
static xmlXPathContextCachePtr xmlXPathNewCache()
{
	...
	if(!ret) {
		...
		memzero(ret, (size_t)sizeof(xmlXPathContextCache));
		ret->maxNodeset = 100;
		ret->maxString = 100;
		ret->maxBoolean = 100;
		ret->maxNumber = 100;
...
static void FASTCALL xmlXPathCacheFreeObjectList(xmlPointerListPtr list)
{
	if(list) {
		for(int i = 0; i < list->number; i++) {
			...
			*/
			if(obj->nodesetval) {
				SAlloc::F(obj->nodesetval->nodeTab);
 ...
 * Returns the created or reused object.
 */
static xmlXPathObjectPtr xmlXPathCacheNewNodeSet(xmlXPathContextPtr ctxt, xmlNode * val)
{
	if(ctxt && ctxt->cache) {
		xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr)ctxt->cache;
 ...
 */
static xmlNode * xmlXPathNodeSetDupNs(xmlNode * node, xmlNs * ns)
{
	xmlNs * cur;
 ...
 * Returns the newly created object.
 */
xmlNodeSetPtr xmlXPathNodeSetCreate(xmlNode * val)
{
	xmlNodeSetPtr ret = (xmlNodeSetPtr)SAlloc::M(sizeof(xmlNodeSet));
	if(!ret) {
		xmlXPathErrMemory(NULL, "creating nodeset");
		return 0;
	}
	memzero(ret, (size_t)sizeof(xmlNodeSet));
	if(val) {
		ret->nodeTab = (xmlNodePtr*)SAlloc::M(XML_NODESET_DEFAULT * sizeof(xmlNode *));
		if(ret->nodeTab == NULL) {
			xmlXPathErrMemory(NULL, "creating nodeset");
			SAlloc::F(ret);
			return 0;
		}
		memzero(ret->nodeTab, XML_NODESET_DEFAULT * (size_t)sizeof(xmlNode *));
		ret->nodeMax = XML_NODESET_DEFAULT;
		if(val->type == XML_NAMESPACE_DECL) {
			xmlNs * ns = (xmlNs *)val;
			ret->nodeTab[ret->nodeNr++] = xmlXPathNodeSetDupNs((xmlNode *)ns->next, ns);
		}
		else
...
static xmlNodeSetPtr xmlXPathNodeSetCreateSize(int size)
{
	...
	ret->nodeTab = (xmlNodePtr*)SAlloc::M(size * sizeof(xmlNode *));
	if(ret->nodeTab == NULL) {
		xmlXPathErrMemory(NULL, "creating nodeset");
		SAlloc::F(ret);
		return 0;
	}
	memzero(ret->nodeTab, size * (size_t)sizeof(xmlNode *));
	ret->nodeMax = size;
	return ret;
}
 ...
 */
int xmlXPathNodeSetContains(xmlNodeSetPtr cur, xmlNode * val)
{
	int i;
	if(!cur || (val == NULL))
		return 0;
	if(val->type == XML_NAMESPACE_DECL) {
		for(i = 0; i < cur->nodeNr; i++) {
			if(cur->nodeTab[i]->type == XML_NAMESPACE_DECL) {
				xmlNs * ns1 = (xmlNs *)val;
				xmlNs * ns2 = (xmlNs *)cur->nodeTab[i];
				if(ns1 == ns2)
					return 1;
				if(ns1->next && (ns2->next == ns1->next) && (sstreq(ns1->prefix, ns2->prefix)))
					return 1;
			}
		}
	}
 ...
 */
int xmlXPathNodeSetAddNs(xmlNodeSetPtr cur, xmlNode * node, xmlNs * ns) 
{
	int i;
	if(!cur || (ns == NULL) || (node == NULL) || (ns->type != XML_NAMESPACE_DECL) || (node->type != XML_ELEMENT_NODE))
		return -1;
	 ...
	 */
	for(i = 0; i < cur->nodeNr; i++) {
		if(cur->nodeTab[i] && (cur->nodeTab[i]->type == XML_NAMESPACE_DECL) && (((xmlNs *)cur->nodeTab[i])->next == (xmlNs *)node) && (sstreq(ns->prefix, ((xmlNs *)cur->nodeTab[i])->prefix)))
			return 0;
	}
	 ...
	 */
	if(cur->nodeMax == 0) {
		cur->nodeTab = (xmlNodePtr*)SAlloc::M(XML_NODESET_DEFAULT * sizeof(xmlNode *));
		if(cur->nodeTab == NULL) {
			xmlXPathErrMemory(NULL, "growing nodeset");
			return -1;
		}
		memzero(cur->nodeTab, XML_NODESET_DEFAULT * (size_t)sizeof(xmlNode *));
		...
		temp = (xmlNodePtr*)SAlloc::R(cur->nodeTab, cur->nodeMax * 2 * sizeof(xmlNode *));
		if(temp == NULL) {
			xmlXPathErrMemory(NULL, "growing nodeset");
			return -1;
		}
 ...
 * Returns 0 in case of success, and -1 in case of error
 */
int xmlXPathNodeSetAdd(xmlNodeSetPtr cur, xmlNode * val) 
{
	int i;
	...
	if(cur->nodeMax == 0) {
		...
		if(cur->nodeMax >= XPATH_MAX_NODESET_LENGTH) {
			...
			xmlNode ** temp = (xmlNode **)SAlloc::R(cur->nodeTab, cur->nodeMax * 2 * sizeof(xmlNode *));
			if(temp == NULL) {
				xmlXPathErrMemory(NULL, "growing nodeset");
				return -1;
			}
	...
	if(val->type == XML_NAMESPACE_DECL) {
		xmlNs * ns = (xmlNs *)val;
		cur->nodeTab[cur->nodeNr++] =  xmlXPathNodeSetDupNs((xmlNode *)ns->next, ns);
	}
	else
 ...
 */
int xmlXPathNodeSetAddUnique(xmlNodeSetPtr cur, xmlNode * val) 
{
	if(!cur || (val == NULL)) 
		return -1;
	/* @@ with_ns to check whether namespace nodes should be looked at @@ */
...
xmlNodeSetPtr xmlXPathNodeSetMerge(xmlNodeSetPtr val1, xmlNodeSetPtr val2) 
{
	...
	for(i = 0; i < val2->nodeNr; i++) {
		 ...
		 */
		if(val1->nodeMax == 0) {
			val1->nodeTab = (xmlNodePtr*)SAlloc::M(XML_NODESET_DEFAULT * sizeof(xmlNode *));
			if(val1->nodeTab == NULL) {
				xmlXPathErrMemory(NULL, "merging nodeset");
				return 0;
			}
			memzero(val1->nodeTab, XML_NODESET_DEFAULT * (size_t)sizeof(xmlNode *));
			...
			temp = (xmlNodePtr*)SAlloc::R(val1->nodeTab, val1->nodeMax * 2 * sizeof(xmlNode *));
			if(temp == NULL) {
				xmlXPathErrMemory(NULL, "merging nodeset");
				return 0;
			}
		...
		if(n2->type == XML_NAMESPACE_DECL) {
			xmlNs * ns = (xmlNs *)n2;
			val1->nodeTab[val1->nodeNr++] = xmlXPathNodeSetDupNs((xmlNode *)ns->next, ns);
		}
		else
...
static xmlNodeSetPtr xmlXPathNodeSetMergeAndClear(xmlNodeSetPtr set1, xmlNodeSetPtr set2, int hasNullEntries)
{
	if(!set1 && !hasNullEntries) {
		...
		if(set1 == NULL)
			return 0;
		if(set2->nodeNr != 0) {
			memcpy(set1->nodeTab, set2->nodeTab, set2->nodeNr * sizeof(xmlNode *));
		...
		for(i = 0; i < set2->nodeNr; i++) {
			 ...
			 */
			if(set1->nodeMax == 0) {
				set1->nodeTab = (xmlNodePtr*)SAlloc::M(XML_NODESET_DEFAULT * sizeof(xmlNode *));
				if(set1->nodeTab == NULL) {
					xmlXPathErrMemory(NULL, "merging nodeset");
					return 0;
				}
				memzero(set1->nodeTab, XML_NODESET_DEFAULT * (size_t)sizeof(xmlNode *));
				...
				temp = (xmlNodePtr*)SAlloc::R(set1->nodeTab, set1->nodeMax * 2 * sizeof(xmlNode *));
				if(temp == NULL) {
					xmlXPathErrMemory(NULL, "merging nodeset");
					return 0;
				}
			...
			if(n2->type == XML_NAMESPACE_DECL) {
				xmlNs * ns = (xmlNs *)n2;
				set1->nodeTab[set1->nodeNr++] = xmlXPathNodeSetDupNs((xmlNode *)ns->next, ns);
			}
			else
	...
	if((set1 == NULL) && (hasNullEntries == 0)) {
		...
		for(i = 0; i < set2->nodeNr; i++) {
			...
			if(n2 == NULL)
				continue;
			if(set1->nodeMax == 0) {
				set1->nodeTab = (xmlNodePtr*)SAlloc::M(
				    XML_NODESET_DEFAULT * sizeof(xmlNode *));
				if(set1->nodeTab == NULL) {
					xmlXPathErrMemory(NULL, "merging nodeset");
					return 0;
				}
 ...
 */
void xmlXPathNodeSetDel(xmlNodeSetPtr cur, xmlNode * val) 
{
	int i;
	if(!cur) return;
	if(!val) return;
	...
	if(cur->nodeTab[i] && (cur->nodeTab[i]->type == XML_NAMESPACE_DECL))
		xmlXPathNodeSetFreeNs((xmlNs *)cur->nodeTab[i]);
	cur->nodeNr--;
	for(; i < cur->nodeNr; i++)
...
void xmlXPathNodeSetRemove(xmlNodeSetPtr cur, int val)
{
	if(cur && val < cur->nodeNr) {
		if(cur->nodeTab[val] && (cur->nodeTab[val]->type == XML_NAMESPACE_DECL))
			xmlXPathNodeSetFreeNs((xmlNs *)cur->nodeTab[val]);
		cur->nodeNr--;
	...
	if(obj) {
		if(obj->nodeTab) {
			...
			for(int i = 0; i < obj->nodeNr; i++)
				if(obj->nodeTab[i] && (obj->nodeTab[i]->type == XML_NAMESPACE_DECL))
					xmlXPathNodeSetFreeNs((xmlNs *)obj->nodeTab[i]);
			SAlloc::F(obj->nodeTab);
		}
		SAlloc::F(obj);
	}
}
		...
		if(hasNsNodes) {
			for(int i = 0; i < set->nodeNr; i++) {
				...
				if(node && node->type == XML_NAMESPACE_DECL)
					xmlXPathNodeSetFreeNs((xmlNs *)node);
			}
		}
		set->nodeNr = 0;
	}
 ...
 */
xmlNodeSetPtr xmlXPathNodeLeadingSorted(xmlNodeSetPtr nodes, xmlNode * node)
{
	int i, l;
	xmlNode * cur;
	xmlNodeSetPtr ret;
 ...
 *         doesn't contain @node
 */
xmlNodeSetPtr xmlXPathNodeLeading(xmlNodeSetPtr nodes, xmlNode * node)
{
	xmlXPathNodeSetSort(nodes);
 ...
 *         doesn't contain @node
 */
xmlNodeSetPtr xmlXPathNodeTrailingSorted(xmlNodeSetPtr nodes, xmlNode * node)
{
	int i, l;
	xmlNodeSetPtr ret = 0;
	if(node) {
 ...
 *         doesn't contain @node
 */
xmlNodeSetPtr xmlXPathNodeTrailing(xmlNodeSetPtr nodes, xmlNode * node)
{
	xmlXPathNodeSetSort(nodes);
 ...
 */
static void FASTCALL xmlXPathReleaseObject(xmlXPathContextPtr ctxt, xmlXPathObjectPtr pObj)
{
#define XP_CACHE_ADD(sl, o) if(!sl) { sl = xmlPointerListCreate(10); if(sl == NULL) goto free_obj; } \
	if(xmlPointerListAddSize(sl, pObj, 0) == -1) goto free_obj;
	...
	if(pObj) {
		if(!ctxt || (ctxt->cache == NULL)) {
			...
			switch(pObj->type) {
				...
				case XPATH_LOCATIONSET:
					if(pObj->user) {
						xmlXPtrFreeLocationSet((xmlLocationSetPtr)pObj->user);
					}
					goto free_obj;
	#endif
				default:
			...
			if(pObj->nodesetval) {
				...
				if(tmpset->nodeNr > 1) {
					for(int i = 0; i < tmpset->nodeNr; i++) {
						xmlNode * node = tmpset->nodeTab[i];
						if(node && (node->type == XML_NAMESPACE_DECL))
							xmlXPathNodeSetFreeNs((xmlNs *)node);
					}
				}
				else if(tmpset->nodeNr == 1) {
					if(tmpset->nodeTab[0] && (tmpset->nodeTab[0]->type == XML_NAMESPACE_DECL))
						xmlXPathNodeSetFreeNs((xmlNs *)tmpset->nodeTab[0]);
				}
				tmpset->nodeNr = 0;
...
xmlXPathContextPtr xmlXPathNewContext(xmlDocPtr doc)
{
	...
	memzero(ret, (size_t)sizeof(xmlXPathContext));
	ret->doc = doc;
	ret->node = NULL;
	ret->varHash = NULL;
	ret->nb_types = 0;
...
#define CHECK_CONTEXT(ctxt) if(!ctxt || !ctxt->doc || !ctxt->doc->children) { \
 ...
 */
static uint xmlXPathNodeValHash(xmlNode * node)
{
	int len = 2;
	...
	for(i = 0; i < ns1->nodeNr; i++) {
		...
		for(j = 0; j < ns2->nodeNr; j++) {
			...
			if(hashs1[i] != hashs2[j]) {
				...
				SETIFZ(values1[i], xmlNodeGetContent(ns1->nodeTab[i]));
				SETIFZ(values2[j], xmlNodeGetContent(ns2->nodeTab[j]));
...
int xmlXPathCompareValues(xmlXPathParserContextPtr ctxt, int inf, int strict) {
	...
	if(!ctxt || (ctxt->context == NULL)) return 0;
	arg2 = valuePop(ctxt);
	arg1 = valuePop(ctxt);
	if((arg1 == NULL) || (arg2 == NULL)) {
		if(arg1)
...
{
	if(!ctxt || (ctxt->context == NULL)) return;
	CAST_TO_NUMBER;
...
xmlNode * xmlXPathNextChild(xmlXPathParserContextPtr ctxt, xmlNode * cur) 
{
	if(!ctxt || (ctxt->context == NULL)) return 0;
	if(!cur) {
		if(ctxt->context->node == NULL) return 0;
...
xmlNode * xmlXPathNextDescendant(xmlXPathParserContextPtr ctxt, xmlNode * cur)
{
	...
	if(!cur) {
		...
		if(ctxt->context->node == (xmlNode *)ctxt->context->doc)
	...
	if(!cur) {
		...
		switch(ctxt->context->node->type) {
			...
			case XML_ENTITY_DECL:
			    ...
			    if((ctxt->context->node->parent->type == XML_ELEMENT_NODE) && ((ctxt->context->node->parent->name[0] == ' ') ||
				    (sstreq(ctxt->context->node->parent->name, "fake node libxslt"))))
				    return 0;
			...
			case XML_DOCB_DOCUMENT_NODE:
			    ...
			    return 0;
			case XML_NAMESPACE_DECL: {
			    xmlNs * ns = (xmlNs *)ctxt->context->node;
			    if(ns->next && (ns->next->type != XML_NAMESPACE_DECL))
...
xmlNode * xmlXPathNextAncestor(xmlXPathParserContextPtr ctxt, xmlNode * cur) 
{
	...
	switch(cur->type) {
		...
		case XML_XINCLUDE_END:
		    if(cur->parent == NULL)
			    return 0;
		    if((cur->parent->type == XML_ELEMENT_NODE) && ((cur->parent->name[0] == ' ') ||
			    (sstreq(cur->parent->name, "fake node libxslt"))))
			    return 0;
		    return(cur->parent);
		case XML_ATTRIBUTE_NODE: {
		    xmlAttrPtr att = (xmlAttrPtr)ctxt->context->node;
...
xmlNode * xmlXPathNextPrecedingSibling(xmlXPathParserContextPtr ctxt, xmlNode * cur)
{
	if(!ctxt || (ctxt->context == NULL))
		return 0;
	if(oneof2(ctxt->context->node->type, XML_ATTRIBUTE_NODE, XML_NAMESPACE_DECL))
		return 0;
	if(cur == (xmlNode *)ctxt->context->doc)
		return 0;
	if(!cur)
 ...
 */
static int xmlXPathIsAncestor(xmlNode * ancestor, xmlNode * node)
{
	if(ancestor && node) {
		...
		if(ancestor == (xmlNode *)node->doc) 
			return 1;
		if(node == (xmlNode *)ancestor->doc) 
			return 0;
		while(node->parent) {
...
xmlNode * xmlXPathNextAttribute(xmlXPathParserContextPtr ctxt, xmlNode * cur) 
{
	...
	if(!cur) {
		if(ctxt->context->node == (xmlNode *)ctxt->context->doc)
			return 0;
		return((xmlNode *)ctxt->context->node->properties);
	}
...
void xmlXPathCountFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
	...
	if(!cur || !cur->nodesetval)
		...
		if((cur->nodesetval->nodeNr != 1) || (cur->nodesetval->nodeTab == NULL)) {
			...
			int i = 0;
			xmlNode * tmp = cur->nodesetval->nodeTab[0];
		...
		if((cur->nodesetval == NULL) || (cur->nodesetval->nodeNr == 0)) {
			...
			switch(cur->nodesetval->nodeTab[i]->type) {
				...
				case XML_PI_NODE:
					...
					break;
				case XML_NAMESPACE_DECL:
					valuePush(ctxt, xmlXPathCacheNewString(ctxt->context, ((xmlNs *)cur->nodesetval->nodeTab[i])->prefix));
					break;
				default:
					valuePush(ctxt, xmlXPathCacheNewCString(ctxt->context, ""));
			}
...
void xmlXPathStringLengthFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
	...
	if(nargs == 0) {
		...
		if(ctxt->context->node == NULL) {
			...
			xmlChar * content = xmlXPathCastNodeToString(ctxt->context->node);
...
static int xmlXPathNodeCollectAndTest(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNode ** first, xmlNode ** last, int toBool)
{
#define XP_TEST_HIT \
	if(hasAxisRange != 0) {	\
		if(++pos == maxPos) { \
...
#define XP_TEST_HIT_NS \
	if(hasAxisRange != 0) {	\
		if(++pos == maxPos) { \
			hasNsNodes = 1;	\
			if(xmlXPathNodeSetAddNs(seq, xpctxt->node, (xmlNs *)cur) < 0) \
				ctxt->error = XPATH_MEMORY_ERROR; \
			goto axis_range_end; } \
	} else { \
		hasNsNodes = 1;	\
			...
			switch(test) {
				...
				case NODE_TEST_TYPE:
				    ...
				    if(type == NODE_TYPE_NODE) {
					    ...
					    break;
					    case NODE_TEST_PI:
						if((cur->type == XML_PI_NODE) && ((name == NULL) || sstreq(name, cur->name))) {
							XP_TEST_HIT
						}
						break;
					    case NODE_TEST_ALL:
...
error:
	...
	static int xmlXPathCompOpEvalFilterFirst(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNode ** first);
 ...
 */
	static int xmlXPathCompOpEvalLast(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNode ** last)
	{
		int total = 0, cur;
		xmlXPathCompExprPtr comp;
		xmlXPathObjectPtr arg1, arg2;
	...
	static int xmlXPathCompOpEvalFilterFirst(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNode ** first)
	{
		...
		if((op->ch1 != -1) && (op->ch2 != -1) && (comp->steps[op->ch1].op == XPATH_OP_SORT) && (comp->steps[op->ch2].op == XPATH_OP_SORT)) {
			...
			if((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) && (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0) && 
				...
				if(ctxt->value && (ctxt->value->type == XPATH_NODESET) &&
				    ctxt->value->nodesetval && ctxt->value->nodesetval->nodeTab && (ctxt->value->nodesetval->nodeNr > 1)) {
		...
		if((oldset == NULL) || (oldset->nodeNr == 0)) {
			 ...
			 * key() evaluation are attempted on the predicate
			 */
			xmlNodeSetPtr newset = xmlXPathNodeSetCreate(NULL);
			...
			for(i = 0; i < oldset->nodeNr; i++) {
				...
				if(!tmp) {
					...
					if(xmlXPathNodeSetAddUnique(tmp->nodesetval, ctxt->context->node) < 0) {
						ctxt->error = XPATH_MEMORY_ERROR;
					}
				}
				valuePush(ctxt, tmp);
			...
			case XPATH_OP_FILTER: {
			    ...
			    if((op->ch1 != -1) && (op->ch2 != -1) && (comp->steps[op->ch1].op == XPATH_OP_SORT) && (comp->steps[op->ch2].op == XPATH_OP_SORT)) {
				    ...
				    if((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) && (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0) &&
					     ...
					     */
					    if(ctxt->value && (ctxt->value->type == XPATH_NODESET) &&
						    ctxt->value->nodesetval && ctxt->value->nodesetval->nodeTab &&
						    (ctxt->value->nodesetval->nodeNr > 1)) {
						    ctxt->value->nodesetval->nodeTab[0] = ctxt->value->nodesetval->nodeTab[ctxt->value->nodesetval->nodeNr - 1];
						    ctxt->value->nodesetval->nodeNr = 1;
					    }
 ...
 */
	int xmlXPathSetContextNode(xmlNode * node, xmlXPathContextPtr ctx)
	{
 ...
 *         the caller has to free the object.
 */
	xmlXPathObjectPtr xmlXPathNodeEval(xmlNode * node, const xmlChar * str, xmlXPathContextPtr ctx)
	{
		return (str && xmlXPathSetContextNode(node, ctx) >= 0) ? xmlXPathEval(str, ctx) : 0;
	}
 ...
 */
	void xmlXPathRegisterAllFunctions(xmlXPathContextPtr ctxt)
	{
		...
		xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"escape-uri", (const xmlChar*)"http://www.w3.org/2002/08/xquery-functions",
		    xmlXPathEscapeUriFunction);
	}
