...
int valuePush(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr value)
{
	...
	if(ctxt->valueNr >= ctxt->valueMax) {
		...
		tmp = (xmlXPathObjectPtr*)SAlloc::R(ctxt->valueTab, 2 * ctxt->valueMax * sizeof(ctxt->valueTab[0]));
		if(tmp == NULL) {
...
static xmlNodePtr xmlXPathNodeSetDupNs(xmlNodePtr node, xmlNsPtr ns)
{
	 ...
	 */
	cur = (xmlNsPtr)SAlloc::M(sizeof(xmlNs));
	if(!cur) {
	...
	if(val->type == XML_NAMESPACE_DECL) {
		for(i = 0; i < cur->nodeNr; i++) {
			if(cur->nodeTab[i]->type == XML_NAMESPACE_DECL) {
				xmlNsPtr ns1 = (xmlNsPtr)val;
				xmlNsPtr ns2 = (xmlNsPtr)cur->nodeTab[i];
				if(ns1 == ns2)
					return 1;
...
int xmlXPathNodeSetAddNs(xmlNodeSetPtr cur, xmlNodePtr node, xmlNsPtr ns) {
	...
	for(i = 0; i < cur->nodeNr; i++) {
		if(cur->nodeTab[i] && (cur->nodeTab[i]->type == XML_NAMESPACE_DECL) &&
		    ...
		    (sstreq(ns->prefix, ((xmlNsPtr)cur->nodeTab[i])->prefix)))
			return 0;
	}
...
int xmlXPathNodeSetAdd(xmlNodeSetPtr cur, xmlNodePtr val) {
	...
	if(val->type == XML_NAMESPACE_DECL) {
		...
		cur->nodeTab[cur->nodeNr++] =
		    xmlXPathNodeSetDupNs((xmlNode *)ns->next, ns);
	}
	else
...
int xmlXPathNodeSetAddUnique(xmlNodeSetPtr cur, xmlNodePtr val) 
{
	...
	if(cur->nodeMax == 0) {
		...
		if(cur->nodeMax >= XPATH_MAX_NODESET_LENGTH) {
			...
			xmlNodePtr * temp = (xmlNodePtr*)SAlloc::R(cur->nodeTab, cur->nodeMax * 2 * sizeof(xmlNode *));
			if(temp == NULL) {
				xmlXPathErrMemory(NULL, "growing nodeset");
				return -1;
			}
		...
		for(i = 0; i < set2->nodeNr; i++) {
			...
			for(j = 0; j < initNbSet1; j++) {
				...
				if(n1 == n2) {
					...
					if((((xmlNsPtr)n1)->next == ((xmlNsPtr)n2)->next) && (sstreq(((xmlNsPtr)n1)->prefix, ((xmlNsPtr)n2)->prefix))) {
						/*
...
xmlNodeSetPtr xmlXPathNodeTrailingSorted(xmlNodeSetPtr nodes, xmlNodePtr node)
{
	...
	if(node) {
		...
		if(ret) {
			...
			l = xmlXPathNodeSetGetLength(nodes);
			for(i = l - 1; i >= 0; i--) {
				xmlNodePtr cur = xmlXPathNodeSetItem(nodes, i);
				if(cur == node)
					break;
				if(xmlXPathNodeSetAddUnique(ret, cur) < 0)
					break;
...
static void xmlXPathReleaseObject(xmlXPathContextPtr ctxt, xmlXPathObjectPtr obj)
{
#define XP_CACHE_ADD(sl, o) if(!sl) { \
...
obj_cached:
		...
		if(obj->nodesetval) {
			...
			tmpset->nodeNr = 0;
			memzero(obj, sizeof(xmlXPathObject));
 ...
 */
static uint xmlXPathNodeValHash(xmlNodePtr node)
{
	int len = 2;
...
static int xmlXPathEqualNodeSetString(xmlXPathObjectPtr arg, const xmlChar * str, int neq)
{
	...
	for(i = 0; i < ns->nodeNr; i++) {
		if(xmlXPathNodeValHash(ns->nodeTab[i]) == hash) {
			...
			else if((str2 == NULL) && (sstreq(str, BAD_CAST ""))) {
				if(neq)
					continue;
				return 1;
			}
 ...
 * Returns the next element following that axis
 */
static xmlNodePtr xmlXPathNextDescendantOrSelfElemParent(xmlNodePtr cur,
		...
		switch(ctxt->context->node->type) {
			...
			case XML_ENTITY_DECL:
			    ...
			    ((ctxt->context->node->parent->name[0] == ' ') ||
				    (sstreq(ctxt->context->node->parent->name,
...
void xmlXPathCountFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
	...
	if(!cur || !cur->nodesetval)
		...
		if((cur->nodesetval->nodeNr != 1) || (cur->nodesetval->nodeTab == NULL)) {
			...
			int i = 0;
			xmlNodePtr tmp = cur->nodesetval->nodeTab[0];
...
void xmlXPathStringFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
	...
	CHECK_ARITY(1);
	cur = valuePop(ctxt);
	if(!cur) XP_ERROR(XPATH_INVALID_OPERAND);
...
int xmlXPathIsNodeType(const xmlChar * name) {
	...
	if(sstreq(name, BAD_CAST "node"))
		return 1;
	if(sstreq(name, BAD_CAST "text"))
		return 1;
	if(sstreq(name, BAD_CAST "comment"))
		return 1;
	if(sstreq(name, BAD_CAST "processing-instruction"))
		return 1;
	return 0;
}
...
static xmlChar * xmlXPathCompNodeTest(xmlXPathParserContextPtr ctxt, xmlXPathTestVal * test,
	...
	*prefix = NULL;
	...
	if(CUR == '(') {
		...
		else if(sstreq(name, BAD_CAST "node"))
			*type = NODE_TYPE_NODE;
		else if(sstreq(name, BAD_CAST "processing-instruction"))
			*type = NODE_TYPE_PI;
		else if(sstreq(name, BAD_CAST "text"))
			*type = NODE_TYPE_TEXT;
		else {
			SAlloc::F(name);
			XP_ERRORNULL(XPATH_EXPR_ERROR);
...
static xmlXPathAxisVal xmlXPathIsAxisName(const xmlChar * name) {
	...
	switch(name[0]) {
		case 'a':
		    if(sstreq(name, BAD_CAST "ancestor"))
			    ret = AXIS_ANCESTOR;
		    if(sstreq(name, BAD_CAST "ancestor-or-self"))
			    ret = AXIS_ANCESTOR_OR_SELF;
		    if(sstreq(name, BAD_CAST "attribute"))
			    ret = AXIS_ATTRIBUTE;
		    break;
		case 'c':
		    if(sstreq(name, BAD_CAST "child"))
			    ret = AXIS_CHILD;
		    break;
		case 'd':
		    if(sstreq(name, BAD_CAST "descendant"))
			    ret = AXIS_DESCENDANT;
		    if(sstreq(name, BAD_CAST "descendant-or-self"))
			    ret = AXIS_DESCENDANT_OR_SELF;
		    break;
		case 'f':
		    if(sstreq(name, BAD_CAST "following"))
			    ret = AXIS_FOLLOWING;
		    if(sstreq(name, BAD_CAST "following-sibling"))
			    ret = AXIS_FOLLOWING_SIBLING;
		    break;
		case 'n':
		    if(sstreq(name, BAD_CAST "namespace"))
			    ret = AXIS_NAMESPACE;
		    break;
		case 'p':
		    if(sstreq(name, BAD_CAST "parent"))
			    ret = AXIS_PARENT;
		    if(sstreq(name, BAD_CAST "preceding"))
			    ret = AXIS_PRECEDING;
		    if(sstreq(name, BAD_CAST "preceding-sibling"))
			    ret = AXIS_PRECEDING_SIBLING;
		    break;
		case 's':
		    if(sstreq(name, BAD_CAST "self"))
			    ret = AXIS_SELF;
		    break;
	}
	return ret;
...
static void xmlXPathCompStep(xmlXPathParserContextPtr ctxt) {
	...
	if((CUR == '.') && (NXT(1) == '.')) {
		...
		if(ctxt->xptr) {
			name = xmlXPathParseNCName(ctxt);
			if(name && (sstreq(name, BAD_CAST "range-to"))) {
				op2 = ctxt->comp->last;
				SAlloc::F(name);
				SKIP_BLANKS;
				if(CUR != '(') {
	...
	static int xmlXPathCompOpEvalFilterFirst(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNodePtr * first)
	{
		...
		if((op->ch1 != -1) && (op->ch2 != -1) && (comp->steps[op->ch1].op == XPATH_OP_SORT) && (comp->steps[op->ch2].op == XPATH_OP_SORT)) {
			...
			if((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) && (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0) && 
				comp->steps[f].value4 && (sstreq((const xmlChar *)comp->steps[f].value4, BAD_CAST "last"))) {
				...
				if((*cptr >= 'A' && *cptr <= 'Z') || (*cptr >= 'a' && *cptr <= 'z') ||
				    ...
				    *cptr == '\''|| *cptr == '(' || *cptr == ')' ||
				    (*cptr == '%' && ((cptr[1] >= 'A' && cptr[1] <= 'F') || (cptr[1] >= 'a' && cptr[1] <= 'f') ||
						    (cptr[1] >= '0' && cptr[1] <= '9')) && ((cptr[2] >= 'A' && cptr[2] <= 'F') ||
						    (cptr[2] >= 'a' && cptr[2] <= 'f') || (cptr[2] >= '0' && cptr[2] <= '9'))) ||
				    (!escape_reserved && (*cptr == ';' || *cptr == '/' || *cptr == '?' ||
						    *cptr == ':' || *cptr == '@' || *cptr == '&' ||
						    *cptr == '=' || *cptr == '+' || *cptr == '$' ||
						    *cptr == ','))) {
