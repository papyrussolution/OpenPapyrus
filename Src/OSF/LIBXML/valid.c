...
{
	xmlEnumerationPtr ret = (xmlEnumerationPtr)SAlloc::M(sizeof(xmlEnumeration));
	if(!ret) {
		xmlVErrMemory(NULL, "malloc failed");
	}
	else {
...
static void xmlFreeAttribute(xmlAttribute * pAttr)
{
	if(pAttr) {
		...
		if(dict) {
			if((pAttr->elem != NULL) && (!xmlDictOwns(dict, pAttr->elem)))
				SAlloc::F((xmlChar*)pAttr->elem);
			if((pAttr->name != NULL) && (!xmlDictOwns(dict, pAttr->name)))
				SAlloc::F((xmlChar*)pAttr->name);
			if((pAttr->prefix != NULL) && (!xmlDictOwns(dict, pAttr->prefix)))
				SAlloc::F((xmlChar*)pAttr->prefix);
			if((pAttr->defaultValue != NULL) && (!xmlDictOwns(dict, pAttr->defaultValue)))
...
xmlAttributePtr xmlAddAttributeDecl(xmlValidCtxtPtr ctxt,
	...
	ret = (xmlAttributePtr)SAlloc::M(sizeof(xmlAttribute));
	if(!ret) {
		xmlVErrMemory(ctxt, "malloc failed");
		xmlFreeEnumeration(tree);
		return 0;
	...
	if(elemDef != NULL) {
		 ...
		 * processed first.
		 */
		if((sstreq(ret->name, BAD_CAST "xmlns")) || ((ret->prefix != NULL && (sstreq(ret->prefix, "xmlns"))))) {
			ret->nexth = elemDef->attributes;
			elemDef->attributes = ret;
		}
		else {
			xmlAttributePtr tmp = elemDef->attributes;
			while(tmp && ((sstreq(tmp->name, BAD_CAST "xmlns")) || ((ret->prefix != NULL && (sstreq(ret->prefix, "xmlns")))))) {
				if(tmp->nexth == NULL)
					break;
				tmp = tmp->nexth;
			}
...
{
	xmlAttributePtr cur = (xmlAttributePtr)SAlloc::M(sizeof(xmlAttribute));
	if(!cur) {
		xmlVErrMemory(NULL, "malloc failed");
		return 0;
	}
...
xmlNotationPtr xmlAddNotationDecl(xmlValidCtxtPtr ctxt, xmlDtdPtr dtd,
	...
	ret = (xmlNotationPtr)SAlloc::M(sizeof(xmlNotation));
	if(!ret) {
		xmlVErrMemory(ctxt, "malloc failed");
		return 0;
	}
...
static xmlNotationPtr xmlCopyNotation(xmlNotationPtr nota) {
	...
	cur = (xmlNotationPtr)SAlloc::M(sizeof(xmlNotation));
	if(!cur) {
		xmlVErrMemory(NULL, "malloc failed");
		return 0;
	}
...
int xmlValidateAttributeDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc,
	...
	if((attr->defaultValue != NULL) && (attr->tree != NULL)) {
		...
		while(tree != NULL) {
			if(sstreq(tree->name, attr->defaultValue)) break;
			tree = tree->next;
		}
		if(tree == NULL) {
			...
			if(cur->c1->type == XML_ELEMENT_CONTENT_ELEMENT) {
				...
				while(next != NULL) {
					if(next->type == XML_ELEMENT_CONTENT_ELEMENT) {
						if((sstreq(next->name, name)) &&
						    (sstreq(next->prefix, cur->c1->prefix))) {
					...
					if((sstreq(next->c1->name, name)) &&
					    (sstreq(next->c1->prefix, cur->c1->prefix))) {
						if(cur->c1->prefix == NULL) {
							xmlErrValidNode(ctxt, (xmlNode *)elem, XML_DTD_CONTENT_ERROR,
    ...
    xmlNodePtr elem, xmlAttrPtr attr, const xmlChar * value)
{
	...
	if((attrDecl->def == XML_ATTRIBUTE_FIXED) &&
	    (!sstreq(attrDecl->defaultValue, value))) {
		xmlErrValidNode(ctxt, elem, XML_DTD_ATTRIBUTE_VALUE,
	...
	for(i = 0; i < nb_elements; i++) {
		...
		if(xmlValidateOneElement(&vctxt, parent->doc, parent)) {
			...
			for(j = 0; j < nb_valid_elements; j++)
				if(sstreq(elements[i], names[j])) break;
