 ...
 */
static void xmlErrValidNode(xmlValidCtxtPtr ctxt, xmlNodePtr node, xmlParserErrors error, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
 ...
 * Handle a validation error, provide contextual informations
 */
static void xmlErrValidNodeNr(xmlValidCtxtPtr ctxt, xmlNodePtr node, xmlParserErrors error, const char * msg, const xmlChar * str1, int int2, const xmlChar * str3)
{
	xmlStructuredErrorFunc schannel = NULL;
	xmlGenericErrorFunc channel = NULL;
	xmlParserCtxtPtr pctxt = NULL;
...
static int vstateVPush(xmlValidCtxtPtr ctxt, xmlElementPtr elemDecl, xmlNodePtr node) {
	...
	if((elemDecl != NULL) && (elemDecl->etype == XML_ELEMENT_TYPE_ELEMENT)) {
		...
		if(elemDecl->contModel != NULL) {
			...
			xmlErrValidNode(ctxt, (xmlNode *)elemDecl,
...
static int vstateVPush(xmlValidCtxtPtr ctxt, xmlElementContentPtr cont,
    xmlNodePtr node, uchar depth, long occurs,
    uchar state) {
...
int xmlValidBuildContentModel(xmlValidCtxtPtr ctxt, xmlElementPtr elem) {
	...
	if(ctxt->am == NULL) {
		xmlErrValidNode(ctxt, (xmlNode *)elem,
		    XML_ERR_INTERNAL_ERROR,
	...
	if(cur->c2 != NULL) {
		...
		while(cur) {
			...
			if(tmp == NULL) {
				...
				prev->c2 = tmp;
				if(cur->name != NULL) {
					tmp->name = dict ? xmlDictLookup(dict, cur->name, -1) : xmlStrdup(cur->name);
				}
				if(cur->prefix != NULL) {
					tmp->prefix = dict ? xmlDictLookup(dict, cur->prefix, -1) : xmlStrdup(cur->prefix);
				}
				if(cur->c1 != NULL)
	...
	while(cur) {
		...
		if(dict) {
			if((cur->name != NULL) && (!xmlDictOwns(dict, cur->name)))
				free((xmlChar*)cur->name);
			if((cur->prefix != NULL) && (!xmlDictOwns(dict, cur->prefix)))
				free((xmlChar*)cur->prefix);
		}
		else {
			if(cur->name != NULL) free((xmlChar*)cur->name);
			if(cur->prefix != NULL) free((xmlChar*)cur->prefix);
		}
		free(cur);
		cur = next;
	}
...
static void xmlFreeElement(xmlElementPtr elem) {
	if(elem == NULL) return;
	xmlUnlinkNode((xmlNode *)elem);
	xmlFreeDocElementContent(elem->doc, elem->content);
	if(elem->name != NULL)
		free((xmlChar*)elem->name);
	if(elem->prefix != NULL)
		free((xmlChar*)elem->prefix);
#ifdef LIBXML_REGEXP_ENABLED
	if(elem->contModel != NULL)
...
xmlElementPtr xmlAddElementDecl(xmlValidCtxtPtr ctxt, xmlDtdPtr dtd, const xmlChar * name, xmlElementTypeVal type, xmlElementContentPtr content)
{
	...
	if(ret) {
		if(ret->etype != XML_ELEMENT_TYPE_UNDEFINED) {
			 ...
			 */
			xmlErrValidNode(ctxt, (xmlNode *)dtd, XML_DTD_ELEM_REDEFINED,
...
{
	xmlEnumerationPtr ret = (xmlEnumerationPtr)xmlMalloc(sizeof(xmlEnumeration));
	if(ret == NULL) {
		xmlVErrMemory(NULL, "malloc failed");
	}
	else {
...
static int xmlScanIDAttributeDecl(xmlValidCtxtPtr ctxt, xmlElementPtr elem, int err) {
	...
	while(cur) {
		if(cur->atype == XML_ATTRIBUTE_ID) {
			ret++;
			if((ret > 1) && (err))
				xmlErrValidNode(ctxt, (xmlNode *)elem, XML_DTD_MULTIPLE_ID,
...
static void xmlFreeAttribute(xmlAttributePtr attr)
{
	...
	if(dict) {
		if((attr->elem != NULL) && (!xmlDictOwns(dict, attr->elem)))
			free((xmlChar*)attr->elem);
		if((attr->name != NULL) && (!xmlDictOwns(dict, attr->name)))
			free((xmlChar*)attr->name);
		if((attr->prefix != NULL) && (!xmlDictOwns(dict, attr->prefix)))
			free((xmlChar*)attr->prefix);
		if((attr->defaultValue != NULL) &&
		    (!xmlDictOwns(dict, attr->defaultValue)))
			free((xmlChar*)attr->defaultValue);
	}
	else {
		if(attr->elem != NULL)
			free((xmlChar*)attr->elem);
...
xmlAttributePtr xmlAddAttributeDecl(xmlValidCtxtPtr ctxt,
	...
	if((defaultValue != NULL) &&
		...
		xmlErrValidNode(ctxt, (xmlNode *)dtd, XML_DTD_ATTRIBUTE_DEFAULT,
	...
	if(xmlHashAddEntry3(table, ret->name, ret->prefix, ret->elem, ret) < 0) {
		 ...
		 */
		xmlErrValidWarning(ctxt, (xmlNode *)dtd, XML_DTD_ATTRIBUTE_REDEFINED,
	...
	if(elemDef != NULL) {
		...
		if((type == XML_ATTRIBUTE_ID) &&
			...
			xmlErrValidNode(ctxt, (xmlNode *)dtd, XML_DTD_MULTIPLE_ID,
...
{
	xmlAttributePtr cur = (xmlAttributePtr)xmlMalloc(sizeof(xmlAttribute));
	if(!cur) {
		xmlVErrMemory(NULL, "malloc failed");
		return 0;
	}
...
static void xmlFreeNotation(xmlNotationPtr nota) {
	if(nota == NULL) return;
	if(nota->name != NULL)
		free((xmlChar*)nota->name);
	if(nota->PublicID != NULL)
		free((xmlChar*)nota->PublicID);
	if(nota->SystemID != NULL)
		free((xmlChar*)nota->SystemID);
	free(nota);
}
...
xmlIDPtr xmlAddID(xmlValidCtxtPtr ctxt, xmlDocPtr doc, const xmlChar * value, xmlAttrPtr attr)
{
	...
	if(doc && value && attr) {
		 ...
		 */
		xmlIDTablePtr table = (xmlIDTablePtr)doc->ids;
		if(table == NULL) {
...
int xmlIsID(xmlDocPtr doc, xmlNodePtr elem, xmlAttrPtr attr)
{
	...
	if((doc->intSubset == NULL) && (doc->extSubset == NULL) && (doc->type != XML_HTML_DOCUMENT_NODE)) {
		...
		if((xmlStrEqual(BAD_CAST "id", attr->name)) || ((xmlStrEqual(BAD_CAST "name", attr->name)) && (!elem || (xmlStrEqual(elem->name, BAD_CAST "a")))))
			return 1;
		return 0;
	}
	else if(elem == NULL) {
...
static void xmlFreeRef(xmlLinkPtr lk)
{
	...
	if(ref) {
		free((xmlChar*)ref->value);
		free((xmlChar*)ref->name);
		free(ref);
	}
}
/**
...
int xmlValidateNotationUse(xmlValidCtxtPtr ctxt, xmlDocPtr doc,
	...
	if((notaDecl == NULL) && (ctxt)) {
		xmlErrValidNode(ctxt, (xmlNode *)doc, XML_DTD_UNKNOWN_NOTATION,
...
int xmlValidateNotationDecl(xmlValidCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlDocPtr doc ATTRIBUTE_UNUSED,
...
static int xmlValidateAttributeValueInternal(xmlDocPtr doc, xmlAttributeType type,
    const xmlChar * value) {
	switch(type) {
		case XML_ATTRIBUTE_ENTITIES:
		case XML_ATTRIBUTE_IDREFS:
...
static int xmlValidateAttributeValue2(xmlValidCtxtPtr ctxt, xmlDocPtr doc,
	...
	switch(type) {
		...
		case XML_ATTRIBUTE_ENTITY: {
		    ...
		    if(ent == NULL) {
			    xmlErrValidNode(ctxt, (xmlNode *)doc,
...
static void xmlValidateAttributeIdCallback(xmlAttributePtr attr, int * count,
...
int xmlValidateAttributeDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc,
	...
	if(attr->defaultValue != NULL) {
		val = xmlValidateAttributeValueInternal(doc, attr->atype,
		    attr->defaultValue);
		if(val == 0) {
			xmlErrValidNode(ctxt, (xmlNode *)attr, XML_DTD_ATTRIBUTE_DEFAULT,
	...
	if((attr->atype == XML_ATTRIBUTE_ID)&&
		...
		xmlErrValidNode(ctxt, (xmlNode *)attr, XML_DTD_ID_FIXED,
	...
	if(attr->atype == XML_ATTRIBUTE_ID) {
		...
		if(nbId > 1) {
			xmlErrValidNodeNr(ctxt, (xmlNode *)attr, XML_DTD_ID_SUBSET,
			...
			if(extId > 1) {
				...
				xmlErrValidNode(ctxt, (xmlNode *)attr, XML_DTD_ID_SUBSET,
	...
	if((attr->defaultValue != NULL) && (attr->tree != NULL)) {
		...
		if(tree == NULL) {
			xmlErrValidNode(ctxt, (xmlNode *)attr, XML_DTD_ATTRIBUTE_VALUE,
...
int xmlValidateElementDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc,
					...
					if(next->type == XML_ELEMENT_CONTENT_ELEMENT) {
						if((xmlStrEqual(next->name, name)) &&
							...
							if(cur->c1->prefix == NULL) {
								xmlErrValidNode(ctxt, (xmlNode *)elem, XML_DTD_CONTENT_ERROR,
	...
	if((tst != NULL ) && (tst != elem) &&
		...
		xmlErrValidNode(ctxt, (xmlNode *)elem, XML_DTD_ELEM_REDEFINED,
		    "Redefinition of element %s\n",
		    elem->name, NULL, NULL);
		ret = 0;
	}
		...
		while(cur) {
			switch(cur->type) {
				...
				case XML_ELEMENT_NODE:
				     ...
				     * what's required
				     */
				    tmp = (xmlNode *)xmlMalloc(sizeof(xmlNode));
				    if(tmp == NULL) {
					    xmlVErrMemory(ctxt, "malloc failed");
 ...
 *
 */
static void xmlValidateCheckRefCallback(xmlListPtr ref_list, xmlValidCtxtPtr ctxt,
    const xmlChar * name) {
...
static void xmlValidateAttributeCallback(xmlAttributePtr cur, xmlValidCtxtPtr ctxt,
    const xmlChar * name ATTRIBUTE_UNUSED) {
	int ret;
	xmlDocPtr doc;
