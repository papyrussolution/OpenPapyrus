 ...
 */
static int xmlXPtrGetArity(xmlNode * cur) 
{
	int i;
 ...
 */
static xmlNode * xmlXPtrGetNthChild(xmlNode * cur, int no)
{
	int i;
	if(!cur || (cur->type == XML_NAMESPACE_DECL))
		return cur;
 ...
 */
static xmlXPathObjectPtr xmlXPtrNewPoint(xmlNode * node, int indx)
{
	...
	if(cur && val) {
		...
		*/
		if(cur->locMax == 0) {
			...
			memzero(cur->locTab, XML_RANGESET_DEFAULT * (size_t)sizeof(xmlXPathObjectPtr));
...
xmlXPathObjectPtr xmlXPtrNewLocationSetNodeSet(xmlNodeSetPtr set)
{
	...
	if(!ret) {
		...
		ret->type = XPATH_LOCATIONSET;
		if(set) {
			xmlLocationSetPtr newset = xmlXPtrLocationSetCreate(NULL);
			if(newset) {
...
static void xmlXPtrEvalXPtrPart(xmlXPathParserContextPtr ctxt, xmlChar * name)
{
	...
	*cur = 0;
	...
	if(sstreq(name, (xmlChar*)"xpointer")) {
		 ...
		 */
		ctxt->context->node = (xmlNode *)ctxt->context->doc;
		...
		if(tmp) {
			if(tmp != init) {
				if(tmp->type == XPATH_NODESET) {
					...
					xmlNodeSetPtr set;
					set = tmp->nodesetval;
					if((set->nodeNr != 1) || (set->nodeTab[0] != (xmlNode *)ctx->doc))
						stack++;
				}
				else
					stack++;
 ...
 *         the caller has to free the node tree.
 */
xmlNode * xmlXPtrBuildNodeList(xmlXPathObjectPtr obj)
{
	xmlNode * list = NULL;
	xmlNode * last = NULL;
	int i;
 ...
 */
static int xmlXPtrNbLocChildren(xmlNode * node)
{
	int ret = 0;
	if(!node)
		return -1;
	...
	switch(loc->type) {
		...
		case XPATH_RANGE:
		    if(loc->user2) {
			    ...
			    xmlNode * node = (xmlNode *)loc->user;
			    if(node == (xmlNode *)ctxt->context->doc) {
...
static xmlXPathObjectPtr xmlXPtrInsideRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc)
{
	...
	switch(loc->type) {
		...
		case XPATH_RANGE: {
		    xmlNode * node = (xmlNode *)loc->user;
		    if(loc->user2) {
			    return xmlXPtrNewRange(node, loc->index, (xmlNode *)loc->user2, loc->index2);
		    }
		    else {
			    switch(node->type) {
				    case XML_PI_NODE:
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrAdvanceChar(xmlNode ** node, int * indx, int bytes)
{
	int len;
	if(node && indx) {
		xmlNode * cur = *node;
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetLastChar(xmlNode ** node, int * indx) 
{
	xmlNode * cur;
	int pos, len = 0;
 ...
 * Returns -1 in case of failure, 0 otherwise
 */
static int xmlXPtrGetStartPoint(xmlXPathObjectPtr obj, xmlNode ** node, int * indx)
{
	if(obj && node && indx) {
		switch(obj->type) {
			case XPATH_POINT:
				*node = (xmlNode *)obj->user;
				if(obj->index <= 0)
					*indx = 0;
				else
					*indx = obj->index;
		...
		if((oldset == NULL) || (oldset->locNr == 0)) {
			...
			*/
			...
			for(i = 0; i < oldset->locNr; i++) {
				...
				* in the nodeset.
				*/
				ctxt->context->node = (xmlNode *)oldset->locTab[i]->user;
				tmp = xmlXPathNewNodeSet(ctxt->context->node);
				valuePush(ctxt, tmp);
