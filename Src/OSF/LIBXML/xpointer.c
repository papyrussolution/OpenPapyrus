...
static void xmlXPtrRangeCheckOrder(xmlXPathObjectPtr range)
{
	...
	if(range->user2 == NULL)
		return;
	tmp = xmlXPtrCmpPoints((xmlNodePtr)range->user, range->index, (xmlNodePtr)range->user2, range->index2);
	if(tmp == -1) {
...
static xmlXPathObjectPtr xmlXPtrCoveringRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc)
{
	...
	switch(loc->type) {
		case XPATH_POINT:
		    return xmlXPtrNewRange((xmlNodePtr)loc->user, loc->index, (xmlNodePtr)loc->user, loc->index);
		case XPATH_RANGE:
		    if(loc->user2) {
			    return xmlXPtrNewRange((xmlNodePtr)loc->user, loc->index, (xmlNodePtr)loc->user2, loc->index2);
		    }
		    else {
...
static xmlXPathObjectPtr xmlXPtrInsideRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc)
{
	...
	switch(loc->type) {
		...
		case XPATH_RANGE: {
		    xmlNodePtr node = (xmlNodePtr)loc->user;
		    if(loc->user2) {
			    return xmlXPtrNewRange(node, loc->index, (xmlNodePtr)loc->user2, loc->index2);
		    }
		    else {
			    switch(node->type) {
				    case XML_PI_NODE:
