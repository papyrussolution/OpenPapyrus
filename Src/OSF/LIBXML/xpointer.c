...
{
	__xmlRaiseError(0, 0, 0, 0, 0, XML_FROM_XPOINTER,
...
static void xmlXPtrErr(xmlXPathParserContextPtr ctxt, int error, const char * msg, const xmlChar * extra)
{
	...
	if(ctxt->context->error) {
		...
		__xmlRaiseError(0, 0, 0, NULL, ctxt->context->debugNode, XML_FROM_XPOINTER, error, XML_ERR_ERROR, NULL, 0, (const char*)extra, (const char*)ctxt->base, NULL,
			...
			if(tmp) {
				...
				switch(tmp->type) {
					case XPATH_POINT:
						...
						break;
					case XPATH_RANGE: {
						xmlNodePtr node = (xmlNode *)tmp->user;
						if(node) {
		...
		for(i = 0; i < oldset->locNr; i++) {
			...
			switch(tmp->type) {
				case XPATH_POINT:
				    ...
				    break;
				case XPATH_RANGE: {
				    xmlNodePtr node = (xmlNode *)tmp->user2;
				    if(node) {
					    if(node->type == XML_ATTRIBUTE_NODE) {
						    /* TODO: Namespace Nodes ??? */
						    xmlXPathFreeObject(obj);
	...
	switch(loc->type) {
		...
		case XPATH_RANGE:
		    if(loc->user2) {
			    ...
			    xmlNodePtr node = (xmlNode *)loc->user;
 ...
 *            of the range
 */
static int xmlXPtrSearchString(const xmlChar * string, xmlNodePtr * start, int * startindex, xmlNodePtr * end, int * endindex)
{
	xmlNodePtr cur;
	const xmlChar * str;
	int pos; /* 0 based */
