...
static int glob(const char * pattern, int flags,
	...
	while(FindNextFile(hFind, &FindFileData)) {
		...
		if(ret->gl_pathc + 2 > nb_paths) {
			char ** tmp = (char **)SAlloc::R(ret->gl_pathv, nb_paths * 2 * sizeof(char *));
			if(tmp == NULL)
				break;
			ret->gl_pathv = tmp;
			nb_paths *= 2;
 ...
 * Returns 0 in case of success, an error code otherwise
 */
static int notRecursiveHugeTest(const char * filename ATTRIBUTE_UNUSED,
    const char * result ATTRIBUTE_UNUSED,const char * err ATTRIBUTE_UNUSED, int options ATTRIBUTE_UNUSED) 
{
