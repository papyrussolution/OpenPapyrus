...
{
	xmlRegexpPtr ret = (xmlRegexpPtr)xmlMalloc(sizeof(xmlRegexp));
	if(ret == NULL) {
...
#endif
		transitions = (int*)xmlMalloc((nbstates + 1) * (nbatoms + 1) * sizeof(int));
		if(transitions == NULL) {
			free(stateRemap);
			free(stringRemap);
			free(stringMap);
...
{
	xmlRegParserCtxtPtr ret = (xmlRegParserCtxtPtr)xmlMalloc(sizeof(xmlRegParserCtxt));
	if(ret) {
 ...
 */
static xmlRegRangePtr xmlRegNewRange(xmlRegParserCtxtPtr ctxt, int neg, xmlRegAtomType type, int start, int end)
{
	xmlRegRangePtr ret = (xmlRegRangePtr)xmlMalloc(sizeof(xmlRegRange));
	if(ret == NULL) {
...
{
	xmlRegAtomPtr ret = (xmlRegAtomPtr)xmlMalloc(sizeof(xmlRegAtom));
	if(ret == NULL) {
...
{
	xmlRegStatePtr ret = (xmlRegStatePtr)xmlMalloc(sizeof(xmlRegState));
	if(ret == NULL) {
    ...
    int neg, xmlRegAtomType type, int start, int end, xmlChar * blockName) 
{
	...
	if(atom->maxRanges == 0) {
		...
		atom->maxRanges *= 2;
		xmlRegRangePtr * tmp = (xmlRegRangePtr*)xmlRealloc(atom->ranges, atom->maxRanges * sizeof(xmlRegRangePtr));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "adding ranges");
			atom->maxRanges /= 2;
			return;
...
static int xmlRegGetCounter(xmlRegParserCtxtPtr ctxt) 
{
	if(ctxt->maxCounters == 0) {
		ctxt->maxCounters = 4;
		ctxt->counters = (xmlRegCounter*)xmlMalloc(ctxt->maxCounters * sizeof(xmlRegCounter));
		...
		xmlRegCounter * tmp;
		ctxt->maxCounters *= 2;
		tmp = (xmlRegCounter*)xmlRealloc(ctxt->counters, ctxt->maxCounters * sizeof(xmlRegCounter));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "allocating counter");
			ctxt->maxCounters /= 2;
			return -1;
...
static int xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) 
{
	...
	if(ctxt->maxAtoms == 0) {
		ctxt->maxAtoms = 4;
		ctxt->atoms = (xmlRegAtomPtr*)xmlMalloc(ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
		if(ctxt->atoms == NULL) {
		...
		xmlRegAtomPtr * tmp;
		ctxt->maxAtoms *= 2;
		tmp = (xmlRegAtomPtr*)xmlRealloc(ctxt->atoms, ctxt->maxAtoms * sizeof(xmlRegAtomPtr));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "allocating counter");
			ctxt->maxAtoms /= 2;
			return -1;
...
static void xmlRegStateAddTransTo(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr target, int from) 
{
	if(target->maxTransTo == 0) {
		...
		target->maxTransTo *= 2;
		int * tmp = (int*)xmlRealloc(target->transTo, target->maxTransTo * sizeof(int));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "adding transition");
			target->maxTransTo /= 2;
			return;
    ...
    xmlRegAtomPtr atom, xmlRegStatePtr target, int counter, int count) 
{
	...
	if(state->maxTrans == 0) {
		state->maxTrans = 8;
		state->trans = (xmlRegTrans*)xmlMalloc(state->maxTrans * sizeof(xmlRegTrans));
		if(state->trans == NULL) {
			xmlRegexpErrMemory(ctxt, "adding transition");
			state->maxTrans = 0;
			return;
		...
		state->maxTrans *= 2;
		xmlRegTrans * tmp = (xmlRegTrans*)xmlRealloc(state->trans, state->maxTrans * sizeof(xmlRegTrans));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "adding transition");
			state->maxTrans /= 2;
			return;
...
static int xmlRegStatePush(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state) 
{
	...
	if(ctxt->maxStates == 0) {
		ctxt->maxStates = 4;
		ctxt->states = (xmlRegStatePtr*)xmlMalloc(ctxt->maxStates * sizeof(xmlRegStatePtr));
		if(ctxt->states == NULL) {
		...
		ctxt->maxStates *= 2;
		xmlRegStatePtr * tmp = (xmlRegStatePtr*)xmlRealloc(ctxt->states, ctxt->maxStates * sizeof(xmlRegStatePtr));
		if(tmp == NULL) {
			xmlRegexpErrMemory(ctxt, "adding state");
			ctxt->maxStates /= 2;
			return -1;
...
static int xmlFAEqualAtoms(xmlRegAtomPtr atom1, xmlRegAtomPtr atom2, int deep)
{
	...
	switch(atom1->type) {
		...
		case XML_REGEXP_STRING:
			ret = (!deep) ? (atom1->valuep == atom2->valuep) : xmlStrEqual((xmlChar*)atom1->valuep, (xmlChar*)atom2->valuep);
		    break;
 ...
 *
 */
static int xmlFARecurseDeterminism(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state, int to, xmlRegAtomPtr atom)
{
	int ret = 1;
	int res;
	int transnr, nbTrans;
...
static void xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec, const xmlChar * value, void * data)
{
	...
	if(exec->inputStackMax == 0) {
		...
		exec->inputStack = (xmlRegInputTokenPtr)xmlMalloc(exec->inputStackMax * sizeof(xmlRegInputToken));
		if(exec->inputStack == NULL) {
		...
		tmp = (xmlRegInputTokenPtr)xmlRealloc(exec->inputStack, exec->inputStackMax * sizeof(xmlRegInputToken));
		if(tmp == NULL) {
			xmlRegexpErrMemory(NULL, "pushing input string");
			exec->inputStackMax /= 2;
			return;
 ...
 */
static int xmlRegCompactPushString(xmlRegExecCtxtPtr exec,
    xmlRegexpPtr comp,
    const xmlChar * value,
    void * data) {
 ...
 *     a negative value in case of error.
 */
static int xmlRegExecPushStringInternal(xmlRegExecCtxtPtr exec, const xmlChar * value,
			...
			if(trans->count == REGEXP_ALL_LAX_COUNTER) {
				...
				if((value == NULL) && (final)) {
					...
					for(i = 0; i < exec->state->nbTrans; i++) {
						...
						if((count < counter->max) && (t->atom != NULL) && (xmlStrEqual(value, (xmlChar *)t->atom->valuep))) {
							ret = 0;
							break;
						}
						if((count >= counter->min) && (count < counter->max) && (t->atom != NULL) && (xmlStrEqual(value, (xmlChar *)t->atom->valuep))) {
							ret = 1;
							break;
						}
					}
				...
				if((ret == 1) && (atom->min > 0) && (atom->max > 0)) {
					...
					do {
						...
						ret = xmlStrEqual(value, (xmlChar *)atom->valuep);
						exec->transcount++;
					} while(ret == 1);
					if(exec->transcount < atom->min)
						ret = 0;
 ...
 */
static int xmlRegExecGetValues(xmlRegExecCtxtPtr exec, int err,
    int * nbval, int * nbneg,
 ...
 */
xmlAutomataStatePtr xmlAutomataNewTransition(xmlAutomataPtr am, xmlAutomataStatePtr from,
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewCountTrans2(xmlAutomataPtr am, xmlAutomataStatePtr from,
    xmlAutomataStatePtr to, const xmlChar * token,
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewCountTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    xmlAutomataStatePtr to, const xmlChar * token,
    int min, int max, void * data) {
	xmlRegAtomPtr atom;
	int counter;
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewEpsilon(xmlAutomataPtr am, xmlAutomataStatePtr from,
    xmlAutomataStatePtr to) {
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewAllTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewCountedTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
 ...
 * Returns the target state or NULL in case of error
 */
xmlAutomataStatePtr xmlAutomataNewCounterTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    xmlAutomataStatePtr to, int counter) {
	if((am == NULL) || (from == NULL) || (counter < 0))
		return 0;
...
xmlExpCtxtPtr xmlExpNewCtxt(int maxNodes, xmlDictPtr dict)
{
	...
	ret->table = (xmlExpNodePtr *)xmlMalloc(size * sizeof(xmlExpNodePtr));
	if(ret->table == NULL) {
		free(ret);
		return 0;
	}
...
static int xmlExpDivide(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub,
