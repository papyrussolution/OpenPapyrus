...
static wchar_t * __xmlIOWin32UTF8ToWChar(const char * u8String)
{
	...
	if(u8String) {
		...
		if(wLen) {
			wString = (wchar_t *)xmlMalloc(wLen * sizeof(wchar_t));
			if(wString) {
...
{
	return (xmlStrncasecmp(BAD_CAST filename, BAD_CAST "http://", 7) == 0) ? 1 : 0;
}
...
void * xmlIOHTTPOpenW(const char * post_uri, int compression)
{
	...
	if(post_uri) {
		ctxt = (xmlIOHTTPWriteCtxtPtr)xmlMalloc(sizeof(xmlIOHTTPWriteCtxt));
		if(!ctxt) {
...
{
	return (xmlStrncasecmp(BAD_CAST filename, BAD_CAST "ftp://", 6) == 0) ? 1 : 0;
}
...
{
	xmlParserInputBufferPtr ret = (xmlParserInputBufferPtr)xmlMalloc(sizeof(xmlParserInputBuffer));
	if(ret == NULL) {
...
{
	xmlOutputBufferPtr ret = (xmlOutputBufferPtr)xmlMalloc(sizeof(xmlOutputBuffer));
	if(ret == NULL) {
...
xmlParserInputPtr xmlCheckHTTPInput(xmlParserCtxtPtr ctxt, xmlParserInputPtr ret)
{
	...
	if((ret != NULL) && (ret->buf != NULL) &&
		...
		if(code >= 400) {
			...
			if(redir != NULL) {
				if(ret->filename != NULL)
					free((xmlChar*)ret->filename);
				if(ret->directory != NULL) {
					free((xmlChar*)ret->directory);
					ret->directory = NULL;
				}
				ret->filename =
...
static xmlChar * xmlResolveResourceFromCatalog(const char * URL, const char * ID, xmlParserCtxtPtr ctxt)
{
	...
	if((pref != XML_CATA_ALLOW_NONE) && (!xmlNoNetExists(URL))) {
		...
		if(resource && !xmlNoNetExists((const char*)resource)) {
			xmlChar * tmp = NULL;
			if(ctxt && (ctxt->catalogs != NULL) && ((pref == XML_CATA_ALLOW_ALL) || (pref == XML_CATA_ALLOW_DOCUMENT))) {
