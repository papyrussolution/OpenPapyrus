...
void xmlCleanupInputCallbacks()
{
	if(xmlInputCallbackInitialized) {
		for(int i = xmlInputCallbackNr - 1; i >= 0; i--) {
...
void xmlCleanupOutputCallbacks()
{
	if(xmlOutputCallbackInitialized) {
		for(int i = xmlOutputCallbackNr - 1; i >= 0; i--) {
...
static int xmlWrapStatUtf8(const char * path, struct stat * info)
{
	...
	/* maybe path in native encoding */
	return (retval < 0) ? _stat64(path, info) : retval;
#else
	return -1;
#endif
}
...
static int xmlFileWrite(void * context, const char * buffer, int len)
{
	if(!context || !buffer)
		...
		const int items = fwrite(&buffer[0], len, 1, static_cast<FILE *>(context));
		if(!items && ferror(static_cast<FILE *>(context))) {
			xmlIOErr(0, "fwrite()");
			return -1;
		}
		else
...
{
	int ret = gzread((gzFile)context, &buffer[0], len);
	if(ret < 0)
		xmlIOErr(0, "gzread()");
	return ret;
}
...
{
	int ret =  (gzclose((gzFile)context) == Z_OK ) ? 0 : -1;
	if(ret < 0)
		xmlIOErr(0, "gzclose()");
	return ret;
}
...
static void * xmlXzfileOpen_real(const char * filename)
{
	...
	if(!xmlStrncasecmp(BAD_CAST filename, reinterpret_cast<const xmlChar *>("file://localhost/"), 17)) {
		...
		path = &filename[5]; // lots of generators seems to lazy to read RFC 1738
	}
	else
		path = filename;
	if(!path)
...
{
	int ret = __libxml2_xzread((xzFile)context, &buffer[0], len);
	if(ret < 0)
		xmlIOErr(0, "xzread()");
	return ret;
}
...
{
	int ret =  (__libxml2_xzclose((xzFile)context) == LZMA_OK ) ? 0 : -1;
	if(ret < 0)
		xmlIOErr(0, "xzclose()");
	return ret;
}
...
static void xmlFreeZMemBuff(xmlZMemBuffPtr buff)
{
	if(buff) {
		SAlloc::F(buff->zbuff);
#ifdef DEBUG_HTTP
		int z_err = deflateEnd(&buff->zctrl);
		if(z_err != Z_OK)
			xmlGenericError(0, "xmlFreeZMemBuff:  Error releasing zlib context:  %d\n", z_err);
#else
...
static void * xmlCreateZMemBuff(int compression)
{
	...
	if((compression >= 1) && (compression <= 9)) {
		...
		if(buff == NULL) {
			...
			memzero(buff, sizeof(xmlZMemBuff));
			buff->size = INIT_HTTP_BUFF_SIZE;
			buff->zbuff = SAlloc::M(buff->size);
			if(buff->zbuff == NULL) {
				...
				int z_err = deflateInit2(&buff->zctrl, compression, Z_DEFLATED, DFLT_WBITS, DFLT_MEM_LVL, Z_DEFAULT_STRATEGY);
				if(z_err != Z_OK) {
					xmlChar msg[500];
					xmlFreeZMemBuff(buff);
					buff = NULL;
...
static int xmlZMemBuffExtend(xmlZMemBuffPtr buff, size_t ext_amt)
{
	...
	if(cur_used > new_size)
		xmlGenericError(0, "xmlZMemBuffExtend:  %s\n%s %d bytes.\n",
		    "Buffer overwrite detected during compressed memory", "buffer extension.  Overflowed by", (cur_used - new_size));
#endif
	tmp_ptr = SAlloc::R(buff->zbuff, new_size);
	if(tmp_ptr) {
		rc = 0;
		buff->size  = new_size;
		buff->zbuff = tmp_ptr;
		...
		xmlChar msg[500];
		xmlStrPrintf(msg, 500, (const xmlChar *)"xmlZMemBuffExtend:  %s %lu bytes.\n",
...
static int xmlZMemBuffAppend(xmlZMemBuffPtr buff, const char * src, int len)
{
	...
	buff->crc = crc32(buff->crc, (uchar *)src, len);
	return ( len );
}
...
{
	return (xmlStrncasecmp(BAD_CAST filename, reinterpret_cast<const xmlChar *>("http://"), 7) == 0) ? 1 : 0;
}
...
void * xmlIOHTTPOpenW(const char * post_uri, int compression)
{
	...
	if(post_uri) {
		ctxt = (xmlIOHTTPWriteCtxtPtr)SAlloc::M(sizeof(xmlIOHTTPWriteCtxt));
		if(!ctxt) {
...
{
	return (buffer && len >= 0) ? xmlNanoHTTPRead(context, &buffer[0], len) : -1;
}
	...
	if(http_content == NULL) {
		...
		if(http_ctxt) {
			...
			*/
			...
			if(dump_name) {
				snprintf(buffer, sizeof(buffer), "%s.content", dump_name);
				tst_file = fopen(buffer, "wb");
				if(tst_file) {
					xmlGenericError(0, "Transmitted content saved in file:  %s\n", buffer);
					fwrite(http_content, sizeof(char), content_lgth, tst_file);
					fclose(tst_file);
				}
				snprintf(buffer, sizeof(buffer), "%s.reply", dump_name);
				tst_file = fopen(buffer, "wb");
				if(tst_file) {
					xmlGenericError(0, "Reply content saved in file:  %s\n", buffer);
...
{
	return (xmlStrncasecmp(BAD_CAST filename, reinterpret_cast<const xmlChar *>("ftp://"), 6) == 0) ? 1 : 0;
}
...
{
	xmlOutputBuffer * ret = static_cast<xmlOutputBuffer *>(SAlloc::M(sizeof(xmlOutputBuffer)));
	if(!ret) {
		...
		if(!ret->buffer) {
			...
			if(encoder) {
				...
				if(ret->conv == NULL) {
					...
					xmlCharEncOutput(ret, 1); // This call is designed to initiate the encoder state
			}
		}
	}
	return ret;
...
xmlOutputBuffer * __xmlOutputBufferCreateFilename(const char * URI, xmlCharEncodingHandler * encoder, int compression ATTRIBUTE_UNUSED)
{
	...
	if(puri) {
#ifdef HAVE_ZLIB_H
		if(puri->scheme && (!sstreq(BAD_CAST puri->scheme, reinterpret_cast<const xmlChar *>("file"))))
			is_file_uri = 0;
#endif
		/*
...
#ifdef DEBUG_INPUT
	xmlGenericError(0, "I/O: read %d chars, buffer %d\n", nbchars, xmlBufUse(in->buffer));
#endif
	return (nbchars);
}
...
int xmlOutputBufferFlush(xmlOutputBuffer * out)
{
	...
	out->written += ret;
#ifdef DEBUG_INPUT
	xmlGenericError(0, "I/O: flushed %d chars\n", ret);
#endif
	return ret;
}
#endif /* LIBXML_OUTPUT_ENABLED */
...
xmlParserInput * xmlCheckHTTPInput(xmlParserCtxt * ctxt, xmlParserInput * ret)
{
	...
	if(ret && ret->buf && (ret->buf->readcallback == xmlIOHTTPRead) && ret->buf->context) {
		...
		const char * redir;
		const char * mime;
		int code = xmlNanoHTTPReturnCode(ret->buf->context);
		if(code >= 400) {
			/* fatal error */
			if(ret->filename)
			...
			if((xmlStrstr(BAD_CAST mime, reinterpret_cast<const xmlChar *>("/xml"))) || (xmlStrstr(BAD_CAST mime, reinterpret_cast<const xmlChar *>("+xml")))) {
			...
			else if(xmlStrstr(BAD_CAST mime, reinterpret_cast<const xmlChar *>("html"))) {
#endif
			}
...
#endif
	if(ctxt && (ctxt->options & XML_PARSE_NONET)) {
...
#ifdef LIBXML_CATALOG_ENABLED
		xmlChar * resource = xmlResolveResourceFromCatalog(URL, ID, ctxt);
#else
		xmlChar * resource = 0;
#endif
		SETIFZ(resource, (xmlChar *)URL);
		if(resource == NULL) {
			SETIFZ(ID, "NULL");
...
xmlParserInput * xmlNoNetExternalEntityLoader(const char * URL, const char * ID, xmlParserCtxt * ctxt)
{
	...
	SETIFZ(resource, (xmlChar *)URL);
	if(resource) {
		if((!xmlStrncasecmp(BAD_CAST resource, reinterpret_cast<const xmlChar *>("ftp://"), 6)) || (!xmlStrncasecmp(BAD_CAST resource, reinterpret_cast<const xmlChar *>("http://"), 7))) {
