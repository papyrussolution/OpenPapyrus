...
{
	__xmlRaiseError(0, 0, 0, 0, 0, XML_FROM_CATALOG, XML_ERR_NO_MEMORY, XML_ERR_ERROR, NULL, 0, extra, NULL, NULL, 0, 0, "Memory allocation failed : %s\n", extra);
}
 ...
 */
static void xmlCatalogErr(xmlCatalogEntryPtr catal, xmlNodePtr node, int error, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
	__xmlRaiseError(0, 0, 0, catal, node, XML_FROM_CATALOG, error, XML_ERR_ERROR, NULL, 0, (const char*)str1, (const char*)str2, (const char*)str3, 0, 0,
	    msg, str1, str2, str3);
}
...
{
	xmlChar * normid = NULL;
	xmlCatalogEntryPtr ret = (xmlCatalogEntryPtr)xmlMalloc(sizeof(xmlCatalogEntry));
	if(!ret) {
		...
		ret->name = name ? xmlStrdup(name) : 0;
		free(normid);
		ret->value = value ? xmlStrdup(value) : 0;
		SETIFZ(URL, value);
		ret->URL = URL ? xmlStrdup(URL) : 0;
		ret->prefer = prefer;
		ret->dealloc = 0;
		ret->depth = 0;
		ret->group = group;
...
{
	xmlCatalogPtr ret = (xmlCatalogPtr)xmlMalloc(sizeof(xmlCatalog));
	if(ret == NULL) {
...
{
	while(cur != NULL) {
		if((cur->ns != NULL) && (cur->ns->href != NULL) && (xmlStrEqual(cur->ns->href, XML_CATALOGS_NAMESPACE))) {
...
static xmlCatalogEntryPtr xmlParseXMLCatalogFile(xmlCatalogPrefer prefer, const xmlChar * filename) 
{
	...
	if(cur && (xmlStrEqual(cur->name, BAD_CAST "catalog")) && cur->ns && cur->ns->href && (xmlStrEqual(cur->ns->href, XML_CATALOGS_NAMESPACE))) {
		parent = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, (const xmlChar*)filename, NULL, prefer, NULL);
		if(parent == NULL) {
			xmlFreeDoc(doc);
			return 0;
		}
...
static int xmlAddXMLCatalog(xmlCatalogEntryPtr catal, const xmlChar * type, const xmlChar * orig, const xmlChar * replace) 
{
	...
	if(typ == XML_CATA_NONE) {
		if(xmlDebugCatalogs)
			xmlGenericError(xmlGenericErrorContext, "Failed to add unknown element %s to catalog\n", type);
		return -1;
	}
	cur = catal->children;
	/*
	...
	if(cur != NULL) {
		while(cur != NULL) {
			if((orig != NULL) && (cur->type == typ) && (xmlStrEqual(orig, cur->name))) {
				if(xmlDebugCatalogs)
					xmlGenericError(xmlGenericErrorContext, "Updating element %s to catalog\n", type);
				free(cur->value);
				free(cur->URL);
...
static int xmlDelXMLCatalog(xmlCatalogEntryPtr catal, const xmlChar * value) 
{
	...
	cur = catal->children;
	while(cur != NULL) {
		if(((cur->name != NULL) && (xmlStrEqual(value, cur->name))) || (xmlStrEqual(value, cur->value))) {
			if(xmlDebugCatalogs) {
				xmlGenericError(xmlGenericErrorContext, "Removing element %s from catalog\n", cur->name ? cur->name : cur->value);
			}
...
static xmlChar * xmlCatalogXMLResolve(xmlCatalogEntryPtr catal, const xmlChar * pubID, const xmlChar * sysID) 
{
	...
	if(sysID != NULL) {
			...
			switch(cur->type) {
				case XML_CATA_SYSTEM:
				    if(xmlStrEqual(sysID, cur->name)) {
					    if(xmlDebugCatalogs)
						    xmlGenericError(xmlGenericErrorContext, "Found system match %s, using %s\n", cur->name, cur->URL);
					    catal->depth--;
		...
		if(haveDelegate) {
			...
			cur = catal;
			while(cur != NULL) {
				if((cur->type == XML_CATA_DELEGATE_SYSTEM) && (!xmlStrncmp(sysID, cur->name, xmlStrlen(cur->name)))) {
					for(i = 0; i < nbList; i++)
					...
					if(cur->children != NULL) {
						if(xmlDebugCatalogs)
							xmlGenericError(xmlGenericErrorContext, "Trying system delegate %s\n", cur->URL);
	...
	if(pubID != NULL) {
		...
		while(cur != NULL) {
			switch(cur->type) {
				case XML_CATA_PUBLIC:
				    ...
				    break;
				case XML_CATA_DELEGATE_PUBLIC:
				    if(!xmlStrncmp(pubID, cur->name, xmlStrlen(cur->name)) && (cur->prefer == XML_CATA_PREFER_PUBLIC))
					    haveDelegate++;
				    break;
				case XML_CATA_NEXT_CATALOG:
				    if(sysID == NULL)
		...
		if(haveDelegate) {
			...
			cur = catal;
			while(cur != NULL) {
				if((cur->type == XML_CATA_DELEGATE_PUBLIC) && (cur->prefer == XML_CATA_PREFER_PUBLIC) && (!xmlStrncmp(pubID, cur->name, xmlStrlen(cur->name)))) {
					for(i = 0; i < nbList; i++)
						if(xmlStrEqual(cur->URL, delegates[i]))
							break;
					if(i < nbList) {
					...
					if(cur->children != NULL) {
						if(xmlDebugCatalogs)
							xmlGenericError(xmlGenericErrorContext, "Trying public delegate %s\n", cur->URL);
...
static xmlChar * xmlCatalogXMLResolveURI(xmlCatalogEntryPtr catal, const xmlChar * URI) 
{
	...
	if(haveDelegate) {
		...
		cur = catal;
		while(cur != NULL) {
			if(((cur->type == XML_CATA_DELEGATE_SYSTEM) || (cur->type == XML_CATA_DELEGATE_URI)) && (!xmlStrncmp(URI, cur->name, xmlStrlen(cur->name)))) {
				for(i = 0; i < nbList; i++)
					if(xmlStrEqual(cur->URL, delegates[i]))
						break;
				if(i < nbList) {
...
static xmlChar * xmlCatalogListXMLResolve(xmlCatalogEntryPtr catal, const xmlChar * pubID, const xmlChar * sysID) 
{
	...
	if(!xmlStrncmp(pubID, BAD_CAST XML_URN_PUBID, sizeof(XML_URN_PUBID) - 1)) {
		...
		if(xmlDebugCatalogs) {
			if(urnID == NULL)
				xmlGenericError(xmlGenericErrorContext, "Public URN ID %s expanded to NULL\n", pubID);
			else
				xmlGenericError(xmlGenericErrorContext, "Public URN ID expanded to %s\n", urnID);
		}
	...
	if(!xmlStrncmp(sysID, BAD_CAST XML_URN_PUBID, sizeof(XML_URN_PUBID) - 1)) {
		...
		if(xmlDebugCatalogs) {
			if(urnID == NULL)
				xmlGenericError(xmlGenericErrorContext, "System URN ID %s expanded to NULL\n", sysID);
			else
				xmlGenericError(xmlGenericErrorContext, "System URN ID expanded to %s\n", urnID);
		}
		if(pubID == NULL)
	...
	while((cur != NULL) && (cur[0] != 0)) {
		...
		if((cur[0] == '-') && (cur[1] == '-')) {
			...
			if(cur == NULL) {
				...
				if(filename != NULL) {
					xmlCatalogEntryPtr entry = xmlNewCatalogEntry(type, name, filename, NULL, XML_CATA_PREFER_NONE, NULL);
				...
				if(super) {
					xmlCatalogEntryPtr entry = xmlNewCatalogEntry(type, sysid, NULL, NULL, XML_CATA_PREFER_NONE, NULL);
...
xmlCatalogPtr xmlLoadACatalog(const char * filename)
{
	...
	if(*first != '<') {
		...
		catal->xml = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST filename, xmlCatalogDefaultPrefer, NULL);
	}
	free(content);
	return (catal);
}
...
static int xmlExpandCatalog(xmlCatalogPtr catal, const char * filename)
{
	...
	if(catal->type == XML_SGML_CATALOG_TYPE) {
		...
		xmlCatalogEntryPtr tmp = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST filename, xmlCatalogDefaultPrefer, NULL);
	...
	if(catal) {
		if(catal->type == XML_XML_CATALOG_TYPE) {
			...
			if(cattype != XML_CATA_NONE) {
				xmlCatalogEntryPtr entry = xmlNewCatalogEntry(cattype, orig, replace, NULL, XML_CATA_PREFER_NONE, NULL);
...
int xmlCatalogIsEmpty(xmlCatalogPtr catal) 
{
	...
	if(catal->type == XML_XML_CATALOG_TYPE) {
		if(catal->xml == NULL)
			return 1;
		if(!oneof2(catal->xml->type, XML_CATA_CATALOG, XML_CATA_BROKEN_CATALOG))
			return -1;
		if(catal->xml->children == NULL)
			return 1;
		return 0;
			...
			if(catal != NULL) {
				...
				while(*cur != '\0') {
					...
					if(*cur != 0) {
						...
						if(path != NULL) {
							*nextent = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, NULL, BAD_CAST path, xmlCatalogDefaultPrefer, NULL);
							if(*nextent != NULL)
...
int xmlCatalogAdd(const xmlChar * type, const xmlChar * orig, const xmlChar * replace) 
{
	 ...
	 */
	if(!xmlDefaultCatalog && (xmlStrEqual(type, BAD_CAST "catalog"))) {
		xmlDefaultCatalog = xmlCreateNewCatalog(XML_XML_CATALOG_TYPE, xmlCatalogDefaultPrefer);
		xmlDefaultCatalog->xml = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, orig, NULL,  xmlCatalogDefaultPrefer, NULL);
		xmlRMutexUnlock(xmlCatalogMutex);
		res = 0;
	}
	else {
	...
	if(xmlDebugCatalogs) {
		switch(allow) {
			...
			case XML_CATA_ALLOW_GLOBAL:
			    xmlGenericError(xmlGenericErrorContext, "Allowing only global catalogs\n");
			    break;
			case XML_CATA_ALLOW_DOCUMENT:
			    xmlGenericError(xmlGenericErrorContext, "Allowing only catalogs from the document\n");
			    break;
...
xmlCatalogPrefer xmlCatalogSetDefaultPrefer(xmlCatalogPrefer prefer) 
{
	...
	if(prefer != XML_CATA_PREFER_NONE) {
		if(xmlDebugCatalogs) {
			switch(prefer) {
				case XML_CATA_PREFER_PUBLIC:
					xmlGenericError(xmlGenericErrorContext, "Setting catalog preference to PUBLIC\n");
					break;
				case XML_CATA_PREFER_SYSTEM:
					xmlGenericError(xmlGenericErrorContext, "Setting catalog preference to SYSTEM\n");
					break;
				default:
					return ret;
			}
...
xmlChar * xmlCatalogLocalResolve(void * catalogs, const xmlChar * pubID, const xmlChar * sysID) 
{
	...
	if(pubID || sysID) {
		if(xmlDebugCatalogs) {
			if(pubID && sysID) {
				xmlGenericError(xmlGenericErrorContext, "Local Resolve: pubID %s sysID %s\n", pubID, sysID);
			}
			else if(pubID) {
...
const xmlChar * xmlCatalogGetSystem(const xmlChar * sysID) 
{
	...
	if(!xmlCatalogInitialized)
		xmlInitializeCatalog();
	if(msg == 0) {
		xmlGenericError(xmlGenericErrorContext, "Use of deprecated xmlCatalogGetSystem() call\n");
		msg++;
	}
	if(sysID == NULL)
		return 0;
...
const xmlChar * xmlCatalogGetPublic(const xmlChar * pubID) 
{
	...
	if(!xmlCatalogInitialized)
		xmlInitializeCatalog();
	if(msg == 0) {
		xmlGenericError(xmlGenericErrorContext, "Use of deprecated xmlCatalogGetPublic() call\n");
		msg++;
	}
	if(pubID == NULL)
		return 0;
