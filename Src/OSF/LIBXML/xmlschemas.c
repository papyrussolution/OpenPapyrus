...
#define WXS_IS_SIMPLE(item) ((item->type == XML_SCHEMA_TYPE_SIMPLE) || ((item->type == XML_SCHEMA_TYPE_BASIC) && \
...
#define WXS_IS_TYPE_NOT_FIXED(i)  (((i)->type != XML_SCHEMA_TYPE_BASIC) && \
	    (((i)->flags & XML_SCHEMAS_TYPE_INTERNAL_RESOLVED) == 0))
#define WXS_IS_TYPE_NOT_FIXED_1(item) (((item)->type != XML_SCHEMA_TYPE_BASIC) && \
 ...
 */
#define WXS_HAS_COMPLEX_CONTENT(item) ((item->contentType == XML_SCHEMA_CONTENT_MIXED) || (item->contentType == XML_SCHEMA_CONTENT_EMPTY) || (item->contentType == XML_SCHEMA_CONTENT_ELEMENTS))
#define WXS_HAS_SIMPLE_CONTENT(item) ((item->contentType == XML_SCHEMA_CONTENT_SIMPLE) || (item->contentType == XML_SCHEMA_CONTENT_BASIC))
 ...
 * Misc macros.
 */
#define IS_SCHEMA(node, type) (node && node->ns && (sstreq(node->name, (const xmlChar*)type)) && (sstreq(node->ns->href, xmlSchemaNs)))
#define FREE_AND_NULL(str) if((str) != NULL) { SAlloc::F((xmlChar*)(str)); str = NULL; }
...
static int xmlSchemaParseInclude(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node);
static int xmlSchemaParseRedefine(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node);
static int xmlSchemaTypeFixup(xmlSchemaTypePtr type, xmlSchemaAbstractCtxtPtr ctxt);
static const xmlChar * xmlSchemaFacetTypeToString(xmlSchemaTypeType type);
static int xmlSchemaParseImport(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node);
...
static xmlSchemaTreeItemPtr xmlSchemaParseModelGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node, xmlSchemaTypeType type, int withParticle);
...
static xmlSchemaQNameRefPtr xmlSchemaParseAttributeGroupRef(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNodePtr node);
...
static const xmlChar* xmlSchemaFormatQNameNs(xmlChar ** buf, xmlNsPtr ns, const xmlChar * localName)
{
	if(ns)
...
static const xmlChar * xmlSchemaGetComponentTargetNs(xmlSchemaBasicItemPtr item)
{
	switch(item->type) {
...
{
	return (xmlSchemaFormatQName(buf, xmlSchemaGetComponentTargetNs((xmlSchemaBasicItemPtr)item),
...
static const xmlChar* xmlSchemaGetComponentDesignation(xmlChar ** buf, void * item)
{
	...
	*buf = xmlStrcat(*buf, BAD_CAST " '");
	*buf = xmlStrcat(*buf, xmlSchemaGetComponentQName(&str, (xmlSchemaBasicItemPtr)item));
	*buf = xmlStrcat(*buf, BAD_CAST "'");
	FREE_AND_NULL(str);
	return(*buf);
}
 ...
 */
static xmlChar* xmlSchemaFormatItemForReport(xmlChar ** buf, const xmlChar * itemDes, xmlSchemaBasicItemPtr item, xmlNodePtr itemNode)
{
	xmlChar * str = NULL;
	int named = 1;
	if(*buf != NULL) {
	...
	if(itemDes != NULL) {
		*buf = xmlStrdup(itemDes);
		...
		named = 0;
	if(!named && itemNode) {
		xmlNodePtr elem = (itemNode->type == XML_ATTRIBUTE_NODE) ? itemNode->parent : itemNode;
		...
		for(facet = type->facets; facet != NULL; facet = facet->next) {
			if(facet->type == XML_SCHEMA_FACET_ENUMERATION) {
				...
				if(res == -1) {
					...
					*buf = *buf ? xmlStrcat(*buf, BAD_CAST ", '") : xmlStrdup(BAD_CAST "'");
 ...
 */
static void xmlSchemaPErrMemory(xmlSchemaParserCtxtPtr ctxt, const char * extra, xmlNodePtr node)
{
	if(ctxt)
		ctxt->nberrors++;
 ...
 */
static void xmlSchemaPErr(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int error,
...
static void xmlSchemaPErr2(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node,
    xmlNodePtr child, int error, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	xmlSchemaPErr(ctxt, NZOR(child, node), error, msg, str1, str2);
}
 ...
 * Handle a parser error
 */
static void xmlSchemaPErrExt(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int error,
 ...
 * Handle an out of memory condition
 */
static void xmlSchemaVErrMemory(xmlSchemaValidCtxtPtr ctxt, const char * extra, xmlNodePtr node)
{
	if(ctxt) {
		ctxt->nberrors++;
...
static void xmlSchemaPSimpleInternalErr(xmlNodePtr node, const char * msg, const xmlChar * str)
{
	__xmlSimpleError(XML_FROM_SCHEMASP, XML_SCHEMAP_INTERNAL, node, msg, (const char*)str);
}
    ...
    const xmlChar * str3, const xmlChar * str4)
{
	...
	if(ctxt) {
		if(ctxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
			 ...
			 */
			if(line == 0) {
				if(!node && (vctxt->depth >= 0) && vctxt->inode) {
					node = vctxt->inode->node;
				}
				/*
				 * Get filename and line if no node-tree.
				 */
				if(!node && vctxt->parserCtxt && vctxt->parserCtxt->input) {
				...
				else if((vctxt->parserCtxt != NULL) && (vctxt->parserCtxt->input != NULL))
					file = vctxt->parserCtxt->input->filename;
			}
 ...
 * Handle a validation error
 */
static void xmlSchemaErr3(xmlSchemaAbstractCtxtPtr actxt,
...
static void xmlSchemaErr4(xmlSchemaAbstractCtxtPtr actxt,
    int error, xmlNodePtr node, const char * msg,
    const xmlChar * str1, const xmlChar * str2,
    const xmlChar * str3, const xmlChar * str4)
{
...
static void xmlSchemaErr(xmlSchemaAbstractCtxtPtr actxt,
    int error, xmlNodePtr node, const char * msg,
    const xmlChar * str1, const xmlChar * str2)
{
	xmlSchemaErr4(actxt, error, node, msg, str1, str2, 0, 0);
}

static xmlChar * xmlSchemaFormatNodeForError(xmlChar ** msg, xmlSchemaAbstractCtxtPtr actxt, xmlNodePtr node)
{
	xmlChar * str = NULL;
	*msg = NULL;
	if((node != NULL) && (node->type != XML_ELEMENT_NODE) && (node->type != XML_ATTRIBUTE_NODE)) {
		/*
	...
	if(node) {
		...
		if(node->ns != NULL)
			*msg = xmlStrcat(*msg, xmlSchemaFormatQName(&str, node->ns->href, node->name));
		else
...
{
	if(actxt) {
		xmlChar * msg = xmlStrdup(BAD_CAST "Internal error: ");
...
{
	xmlChar * msg = NULL;
	if(!node && item && (actxt->type == XML_SCHEMA_CTXT_PARSER)) {
...
static void xmlSchemaCustomErr(xmlSchemaAbstractCtxtPtr actxt,
    ...
    const xmlChar * str2)
{
	xmlSchemaCustomErr4(actxt, error, node, item, message, str1, str2, 0, 0);
}
    ...
    const xmlChar * str3)
{
	...
	xmlSchemaErr4Line(actxt, XML_ERR_WARNING, error, node, 0, (const char*)msg, str1, str2, str3, NULL);
	FREE_AND_NULL(msg)
}
    ...
    const char * message, const xmlChar * str1, const xmlChar * str2)
{
	...
	xmlSchemaErr4Line(ACTXT_CAST vctxt, XML_ERR_ERROR,
	    error, NULL, idcNode->nodeLine, (const char*)msg, xmlSchemaFormatQName(&qname,
		    ...
		    (xmlChar *)vctxt->nodeQNames->items[idcNode->nodeQNameID]), str1, str2, NULL);
	FREE_AND_NULL(qname);
	FREE_AND_NULL(msg);
}

static int xmlSchemaEvalErrorNodeType(xmlSchemaAbstractCtxtPtr actxt, xmlNodePtr node)
{
	if(node)
		return (node->type);
	if((actxt->type == XML_SCHEMA_CTXT_VALIDATOR) && (((xmlSchemaValidCtxtPtr)actxt)->inode != NULL))
    ...
    int displayValue)
{
	xmlChar * msg = NULL;
	xmlSchemaFormatNodeForError(&msg, actxt, node);
	if(displayValue || (xmlSchemaEvalErrorNodeType(actxt, node) == XML_ATTRIBUTE_NODE))
	...
	if(displayValue || (xmlSchemaEvalErrorNodeType(actxt, node) == XML_ATTRIBUTE_NODE))
		...
		xmlSchemaErr(actxt, error, node, (const char*)msg, 0, 0);
	FREE_AND_NULL(msg)
}

static const xmlChar * xmlSchemaFormatErrorNodeQName(xmlChar ** str, xmlSchemaNodeInfoPtr ni, xmlNodePtr node)
{
	if(node) {
		if(node->ns != NULL)
...
static void xmlSchemaIllegalAttrErr(xmlSchemaAbstractCtxtPtr actxt, xmlParserErrors error, xmlSchemaAttrInfoPtr ni, xmlNodePtr node)
{
	xmlChar * msg = NULL, * str = NULL;
	xmlSchemaFormatNodeForError(&msg, actxt, node);
	msg = xmlStrcat(msg, BAD_CAST "The attribute '%s' is not allowed.\n");
	xmlSchemaErr(actxt, error, node, (const char*)msg, xmlSchemaFormatErrorNodeQName(&str, (xmlSchemaNodeInfoPtr)ni, node), NULL);
	FREE_AND_NULL(str)
	FREE_AND_NULL(msg)
}
    ...
    const xmlChar * str2)
{
	...
	if(message == NULL) {
		...
		if(oneof3(facetType, XML_SCHEMA_FACET_LENGTH, XML_SCHEMA_FACET_MINLENGTH, XML_SCHEMA_FACET_MAXLENGTH)) {
			...
			if(nodeType == XML_ATTRIBUTE_NODE)
				xmlSchemaErr3(actxt, error, node, (const char*)msg, value, (const xmlChar*)actLen, (const xmlChar*)len);
			else
				xmlSchemaErr(actxt, error, node, (const char*)msg, (const xmlChar*)actLen, (const xmlChar*)len);
		}
		else if(facetType == XML_SCHEMA_FACET_ENUMERATION) {
			msg = xmlStrcat(msg, BAD_CAST "The value '%s' is not an element of the set {%s}.\n");
			xmlSchemaErr(actxt, error, node, (const char*)msg, value, xmlSchemaFormatFacetEnumSet(actxt, &str, type));
		}
 ...
 */
static void xmlSchemaPMissingAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaBasicItemPtr ownerItem,
	xmlNodePtr ownerElem, const char * name, const char * message)
{
	xmlChar * des = NULL;
 ...
 */
static void xmlSchemaPResCompAttrErr(xmlSchemaParserCtxtPtr ctxt,
    xmlParserErrors error,
    ...
    const char * msg)
{
	...
	if(attr == NULL) {
		xmlSchemaPErrExt(ctxt, NULL, error, NULL, NULL, NULL,
		    ...
		    (const xmlChar*)msg, 0, 0);
	}
	else {
		xmlSchemaPErrExt(ctxt, (xmlNode *)attr, error, NULL, NULL, NULL,
		    "%s, attribute '%s': %s.\n",
 ...
 * Reports an illegal attribute during the parse.
 */
static void xmlSchemaPIllegalAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaBasicItemPtr ownerComp ATTRIBUTE_UNUSED, xmlAttrPtr attr)
{
	xmlChar * strA = NULL, * strB = NULL;
	xmlSchemaFormatNodeForError(&strA, ACTXT_CAST ctxt, attr->parent);
	xmlSchemaErr4(ACTXT_CAST ctxt, error, (xmlNode *)attr, "%sThe attribute '%s' is not allowed.\n", BAD_CAST strA, xmlSchemaFormatQNameNs(&strB, attr->ns, attr->name), NULL, NULL);
	FREE_AND_NULL(strA);
	FREE_AND_NULL(strB);
}
    ...
    const xmlChar * str3, const xmlChar * str4)
{
	...
	msg = xmlStrcat(msg, (const xmlChar*)message);
	msg = xmlStrcat(msg, BAD_CAST ".\n");
	xmlSchemaErr4(ACTXT_CAST ctxt, error, node, (const char*)msg, str1, str2, str3, str4);
	SAlloc::F(msg);
}
...
static void xmlSchemaPIllegalFacetAtomicErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaTypePtr type, xmlSchemaTypePtr baseType, xmlSchemaFacetPtr facet)
{
	...
	xmlSchemaPErrExt(ctxt, type->node, error, NULL, NULL, NULL,
	    ...
	    BAD_CAST des, xmlSchemaFacetTypeToString(facet->type), xmlSchemaFormatItemForReport(&strT, NULL, WXS_BASIC_CAST baseType, NULL), NULL, NULL);
	FREE_AND_NULL(des);
	FREE_AND_NULL(strT);
}
...
static void xmlSchemaPMutualExclAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
    xmlSchemaBasicItemPtr ownerItem, xmlAttrPtr attr, const char * name1, const char * name2)
{
	xmlChar * des = NULL;
 ...
 */
static void xmlSchemaPSimpleTypeErr(xmlSchemaParserCtxtPtr ctxt,
    xmlParserErrors error,
    ...
    const xmlChar * str2)
{
	...
	if(message == NULL) {
		...
		if(node->type == XML_ATTRIBUTE_NODE)
			...
			xmlSchemaPErr(ctxt, node, error, (const char*)msg, 0, 0);
	}
	else {
		msg = xmlStrcat(msg, BAD_CAST message);
		msg = xmlStrcat(msg, BAD_CAST ".\n");
		xmlSchemaPErrExt(ctxt, node, error, NULL, NULL, NULL, (const char*)msg, str1, str2, NULL, 0, 0);
	}
	/* Cleanup. */
	FREE_AND_NULL(msg)
}
 ...
 */
static void xmlSchemaPContentErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaBasicItemPtr ownerItem,
    xmlNodePtr ownerElem, xmlNodePtr child, const char * message, const char * content)
{
	xmlChar * des = NULL;
	xmlSchemaFormatItemForReport(&des, NULL, ownerItem, ownerElem);
	if(message != NULL)
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static xmlSchemaAnnotPtr xmlSchemaNewAnnot(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node)
{
...
static void xmlSchemaBucketFree(xmlSchemaBucket * pBucket)
{
	if(pBucket) {
		...
		if((!pBucket->preserveDoc) && (pBucket->doc != NULL)) {
			xmlFreeDoc(pBucket->doc);
		}
		if(pBucket->type == XML_SCHEMA_SCHEMA_IMPORT) {
			if(WXS_IMPBUCKET(pBucket)->schema != NULL)
...
{
	while(link != NULL) {
		xmlSchemaTypeLinkPtr next = link->next;
		SAlloc::F(link);
		link = next;
	}
}
...
void xmlSchemaFreeType(xmlSchemaType * pType)
{
	if(pType) {
		...
		if(pType->facets != NULL) {
			xmlSchemaFacetPtr next;
			xmlSchemaFacetPtr facet = pType->facets;
			while(facet != NULL) {
				next = facet->next;
		...
		if(pType->memberTypes != NULL)
			xmlSchemaFreeTypeLinkList(pType->memberTypes);
		if(pType->facetSet != NULL) {
			xmlSchemaFacetLinkPtr next;
			xmlSchemaFacetLinkPtr link = pType->facetSet;
			do {
				next = link->next;
				SAlloc::F(link);
				link = next;
...
static void xmlSchemaElementDump(xmlSchemaElementPtr elem, FILE * output,
    ...
    const xmlChar * pNamespace ATTRIBUTE_UNUSED,
...
static const xmlChar * xmlSchemaGetNodeContent(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node)
{
	const xmlChar * ret;
 ...
 */
static const xmlChar * xmlSchemaGetProp(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, const char * name)
{
	const xmlChar * ret = 0;
	xmlChar * val = xmlGetNoNsProp(node, BAD_CAST name);
	if(val) {
		ret = xmlDictLookup(ctxt->dict, val, -1);
		SAlloc::F(val);
	}
...
static xmlSchemaTypePtr xmlSchemaGetType(xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName)
{
	...
	if((nsName != NULL) && sstreq(nsName, xmlSchemaNs)) {
 ...
 * Returns the group definition or NULL if not found.
 */
static xmlSchemaBasicItemPtr xmlSchemaGetNamedComponent(xmlSchemaPtr schema,
    xmlSchemaTypeType itemType,
    const xmlChar * name,
 ...
 */
static xmlSchemaBasicItemPtr xmlSchemaFindRedefCompInGraph(xmlSchemaBucketPtr bucket,
    xmlSchemaTypeType type, const xmlChar * name, const xmlChar * nsName)
{
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaAttributeUsePtr xmlSchemaAddAttributeUse(xmlSchemaParserCtxtPtr pctxt,
    xmlNodePtr node)
{
...
static xmlSchemaAttributeGroupPtr xmlSchemaAddAttributeGroupDefinition(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema ATTRIBUTE_UNUSED, const xmlChar * name, const xmlChar * nsName, xmlNodePtr node)
{
	xmlSchemaAttributeGroupPtr ret = NULL;
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaModelGroupPtr xmlSchemaAddModelGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlSchemaTypeType type, xmlNodePtr node)
{
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaParticlePtr xmlSchemaAddParticle(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int min, int max)
{
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaModelGroupDefPtr xmlSchemaAddModelGroupDefinition(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName, xmlNodePtr node)
{
	xmlSchemaModelGroupDefPtr ret = NULL;
	if(!ctxt || (schema == NULL) || (name == NULL))
		return 0;
...
static xmlSchemaIDCPtr xmlSchemaAddIDC(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    const xmlChar * name, const xmlChar * nsName, int category, xmlNodePtr node)
{
	xmlSchemaIDCPtr ret = NULL;
	if(!ctxt || (schema == NULL) || (name == NULL))
		return 0;
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaWildcardPtr xmlSchemaAddWildcard(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
...
static xmlSchemaSubstGroupPtr xmlSchemaSubstGroupAdd(xmlSchemaParserCtxtPtr pctxt, xmlSchemaElementPtr head)
{
	...
	ret = (xmlSchemaSubstGroupPtr)SAlloc::M(sizeof(xmlSchemaSubstGroup));
	if(!ret) {
		xmlSchemaPErrMemory(NULL, "allocating a substitution group container", NULL);
		return 0;
	}
...
static xmlSchemaSubstGroupPtr xmlSchemaSubstGroupGet(xmlSchemaParserCtxtPtr pctxt, xmlSchemaElementPtr head)
{
	if(WXS_SUBST_GROUPS(pctxt) == NULL)
		return 0;
	return (xmlSchemaSubstGroupPtr)xmlHashLookup2(WXS_SUBST_GROUPS(pctxt), head->name, head->targetNamespace);
}
 ...
 */
static int xmlSchemaPValAttrNodeQNameValue(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaPtr schema,
    xmlSchemaBasicItemPtr ownerItem,
    xmlAttrPtr attr,
    const xmlChar * value,
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrNodeQName(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlSchemaBasicItemPtr ownerItem,
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrQName(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlSchemaBasicItemPtr ownerItem,
    xmlNodePtr ownerElem, const char * name, const xmlChar ** uri, const xmlChar ** local)
{
	xmlAttrPtr attr = xmlSchemaGetPropNode(ownerElem, name);
	if(attr == NULL) {
		*local = NULL;
		*uri = NULL;
		return 0;
    ...
    int min, int max, int def, const char * expected)
{
	...
	int ret = 0;
	xmlAttrPtr attr = xmlSchemaGetPropNode(node, "maxOccurs");
	if(attr == NULL)
		return (def);
 ...
 * Returns 0 or 1.
 */
static int xmlSchemaPGetBoolNodeValue(xmlSchemaParserCtxtPtr ctxt, xmlSchemaBasicItemPtr ownerItem, xmlNodePtr node)
{
	int res = 0;
	...
	if(sstreq(value, "true"))
		...
		xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_INVALID_BOOLEAN, ownerItem, node, xmlSchemaGetBuiltInType(XML_SCHEMAS_BOOLEAN), NULL, BAD_CAST value, NULL, 0, 0);
	}
	SAlloc::F(value);
	return (res);
}
 ...
 */
static int xmlGetBooleanProp(xmlSchemaParserCtxtPtr ctxt,
    xmlNodePtr node,
    const char * name, int def)
{
	...
	if(sstreq(val, "true"))
		...
		xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_INVALID_BOOLEAN, NULL,
		    (xmlNode *)xmlSchemaGetPropNode(node, name), xmlSchemaGetBuiltInType(XML_SCHEMAS_BOOLEAN), NULL, val, NULL, 0, 0);
	}
	return (def);
}
...
static xmlSchemaBasicItemPtr xmlSchemaParseLocalAttribute(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node,
...
static xmlSchemaTypePtr xmlSchemaParseList(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node);
static xmlSchemaWildcardPtr xmlSchemaParseAnyAttribute(xmlSchemaParserCtxtPtr ctxt,
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaPValAttrNode(xmlSchemaParserCtxtPtr ctxt, xmlSchemaBasicItemPtr ownerItem,
    xmlAttrPtr attr, xmlSchemaTypePtr type, const xmlChar ** value)
{
	const xmlChar * val;
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaPValAttr(xmlSchemaParserCtxtPtr ctxt, xmlSchemaBasicItemPtr ownerItem, xmlNodePtr ownerElem,
    const char * name, xmlSchemaTypePtr type, const xmlChar ** value)
{
	xmlAttrPtr attr;
	if(!ctxt || (type == NULL)) {
		ASSIGN_PTR(value, NULL);
		return -1;
	}
	if(type->type != XML_SCHEMA_TYPE_BASIC) {
		ASSIGN_PTR(value, NULL);
...
static int xmlSchemaCheckReference(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema ATTRIBUTE_UNUSED,
    xmlNodePtr node, xmlAttrPtr attr, const xmlChar * namespaceName)
{
	 ...
	 */
	{
		xmlNodePtr n = (attr != NULL) ? (xmlNode *)attr : node;
		if(namespaceName == NULL)
			...
			xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_SRC_RESOLVE, n, NULL,
			    "References from this schema to components in the namespace '%s' are not allowed, since not indicated by an import statement", namespaceName, NULL);
	}
 ...
 */
static int xmlSchemaParseLocalAttributes(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr * child, xmlSchemaItemListPtr * list, int parentType, int * hasRefs)
{
	void * item;
 ...
 */
static xmlSchemaAnnotPtr xmlSchemaParseAnnotation(xmlSchemaParserCtxtPtr ctxt, xmlNodePtr node, int needed)
{
	xmlSchemaAnnotPtr ret;
	...
	while(attr) {
		if(((attr->ns == NULL) && (!sstreq(attr->name, "id"))) || ((attr->ns != NULL) && sstreq(attr->ns->href, xmlSchemaNs))) {
	...
	while(child != NULL) {
		if(IS_SCHEMA(child, "appinfo")) {
			...
			while(attr) {
				if(((attr->ns == NULL) && (!sstreq(attr->name, "source"))) || ((attr->ns != NULL) && sstreq(attr->ns->href, xmlSchemaNs))) {
					xmlSchemaPIllegalAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_NOT_ALLOWED, NULL, attr);
				}
				attr = attr->next;
			}
 ...
 */
static xmlSchemaFacetPtr xmlSchemaParseFacet(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node)
{
	xmlSchemaFacetPtr facet;
	xmlNodePtr child = NULL;
	...
	if(IS_SCHEMA(node, "minInclusive")) {
		...
		xmlSchemaPErr2(ctxt, node, child, XML_SCHEMAP_UNKNOWN_FACET_TYPE, "Unknown facet type %s\n", node->name, NULL);
		xmlSchemaFreeFacet(facet);
		return 0;
	}
	xmlSchemaPValAttrID(ctxt, node, BAD_CAST "id");
	...
	if((facet->type != XML_SCHEMA_FACET_PATTERN) && (facet->type != XML_SCHEMA_FACET_ENUMERATION)) {
		const xmlChar * fixed = xmlSchemaGetProp(ctxt, node, "fixed");
		if(fixed != NULL) {
	...
	if(child) {
		xmlSchemaPErr2(ctxt, node, child, XML_SCHEMAP_UNKNOWN_FACET_CHILD, "Facet %s has unexpected child content\n", node->name, NULL);
	}
	return (facet);
}
...
static int xmlSchemaPCheckParticleCorrect_2(xmlSchemaParserCtxtPtr ctxt,
 ...
 */
static xmlSchemaParticlePtr xmlSchemaParseAny(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node)
{
 ...
 */
static xmlSchemaNotationPtr xmlSchemaParseNotation(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node)
{
	const xmlChar * name;
...
static xmlSchemaAttributePtr xmlSchemaParseGlobalAttribute(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node)
{
	...
	xmlSchemaPValAttrQName(pctxt, schema, NULL, node, "type", &ret->typeNs, &ret->typeName);
 ...
 */
static xmlSchemaQNameRefPtr xmlSchemaParseAttributeGroupRef(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node)
{
	...
	attr = xmlSchemaGetPropNode(node, "ref");
	if(attr == NULL) {
		xmlSchemaPMissingAttrErr(pctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node, "ref", NULL);
		return 0;
	}
	...
	if(child) {
		xmlSchemaPContentErr(pctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL, "(annotation?)");
	}
	/*
 ...
 */
static xmlSchemaAttributeGroupPtr xmlSchemaParseAttributeGroupDefinition(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node)
{
	const xmlChar * name;
...
static int xmlSchemaPValAttrBlockFinal(const xmlChar * value,
    int * flags,
    int flagAll,
    int flagExtension,
    int flagRestriction,
 ...
 */
static xmlSchemaIDCSelectPtr xmlSchemaParseIDCSelectorAndField(xmlSchemaParserCtxtPtr ctxt, xmlSchemaIDCPtr idc, xmlNodePtr node, int isField)
{
	...
	if(item == NULL) {
		xmlSchemaPErrMemory(ctxt, "allocating a 'selector' of an identity-constraint definition", NULL);
		return 0;
	}
	...
	if(attr == NULL) {
		xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node, "name", NULL);
	}
	else {
	...
	if(IS_SCHEMA(child, "annotation")) {
		 ...
		 */
		xmlSchemaAddAnnotation((xmlSchemaAnnotItemPtr)idc, xmlSchemaParseAnnotation(ctxt, child, 1));
		child = child->next;
	}
	if(child) {
		xmlSchemaPContentErr(ctxt,
    ...
    const xmlChar * targetNamespace)
{
	...
	item = xmlSchemaAddIDC(ctxt, schema, name, targetNamespace, idcCategory, node);
	if(item == NULL)
		return 0;
	xmlSchemaPValAttrID(ctxt, node, BAD_CAST "id");
	if(idcCategory == XML_SCHEMA_TYPE_IDC_KEYREF) {
		...
		if(attr == NULL) {
			...
			xmlSchemaCheckReference(ctxt, schema, node, attr, item->ref->targetNamespace);
		}
	}
	/*
	 * And now for the children...
	...
	if(IS_SCHEMA(child, "selector")) {
		...
		if(IS_SCHEMA(child, "field")) {
			...
			xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL, "(annotation?, (selector, field+))");
		}
	}
	if(child) {
		xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL, "(annotation?, (selector, field+))");
 ...
 */
static xmlSchemaBasicItemPtr xmlSchemaParseElement(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node, int * isElemRef, int topLevel)
{
 ...
 */
static int xmlSchemaParseUnion(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node)
{
	xmlSchemaTypePtr type;
	xmlNodePtr child = NULL;
	xmlAttrPtr attr;
		...
		do {
			...
			if(xmlSchemaPValAttrNodeQNameValue(ctxt, schema, NULL, attr, BAD_CAST tmp, &nsName, &localName) == 0) {
				...
				link = (xmlSchemaTypeLinkPtr)
				    SAlloc::M(sizeof(xmlSchemaTypeLink));
				if(link == NULL) {
 ...
 *         1 in case of success.
 */
static xmlSchemaTypePtr xmlSchemaParseList(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node)
{
	xmlSchemaTypePtr type;
	xmlNodePtr child = NULL;
 ...
 * 1 in case of success.
 */
static xmlSchemaTypePtr xmlSchemaParseSimpleType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node, int topLevel)
{
	...
	if(topLevel == 0) {
		...
		type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_SIMPLE, xmlDictLookup(ctxt->dict, (const xmlChar*)buf, -1), ctxt->targetNamespace, node, 0);
#else
		type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_SIMPLE, NULL, ctxt->targetNamespace, node, 0);
#endif
		if(type == NULL)
			return 0;
 ...
 */
static xmlSchemaTreeItemPtr xmlSchemaParseModelGroupDefRef(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node)
{
 ...
 *         1 in case of success.
 */
static xmlSchemaModelGroupDefPtr xmlSchemaParseModelGroupDefinition(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaPtr schema,
    xmlNodePtr node)
{
	xmlSchemaModelGroupDefPtr item;
	...
	item = xmlSchemaAddModelGroupDefinition(ctxt, schema, name, ctxt->targetNamespace, node);
	if(item == NULL)
		return 0;
	/*
	 * Check for illegal attributes.
	...
	if(child) {
		xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL, "(annotation?, (all | choice | sequence)?)");
	}
	return (item);
}
...
{
	xmlNodePtr p_delete, cur;
	if(!ctxt || (root == NULL)) return;
	/*
...
static int xmlSchemaParseSchemaElement(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node)
{
	xmlAttrPtr attr;
	const xmlChar * val;
	...
	if(attr != NULL) {
		res = xmlSchemaPValAttrNode(ctxt, NULL, attr, xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI), NULL);
		HFAILURE;
		if(res != 0) {
	...
	if(attr != NULL) {
		...
		HFAILURE;
		if(res != 0) {
			xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_ELEMFORMDEFAULT_VALUE, NULL, (xmlNode *)attr, NULL,
			    "(qualified | unqualified)", val, NULL, 0, 0);
		}
	}
...
static xmlSchemaConstructionCtxtPtr xmlSchemaConstructionCtxtCreate(xmlDictPtr dict)
{
	xmlSchemaConstructionCtxtPtr ret;
	ret = (xmlSchemaConstructionCtxtPtr)SAlloc::M(sizeof(xmlSchemaConstructionCtxt));
	if(!ret) {
	...
	if(ret->pending == NULL) {
		xmlSchemaPErrMemory(NULL, "allocating list of pending global components", NULL);
 ...
 */
static xmlSchemaParserCtxtPtr xmlSchemaNewParserCtxtUseDict(const char * URL, xmlDictPtr dict)
{
	xmlSchemaParserCtxtPtr ret = xmlSchemaParserCtxtCreate();
	if(ret) {
		ret->dict = dict;
		xmlDictReference(dict);
		if(URL)
...
static int xmlSchemaParseNewDocWithContext(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlSchemaBucketPtr bucket)
{
	...
	if((bucket->targetNamespace != NULL) && sstreq(bucket->targetNamespace, xmlSchemaNs)) {
		/*
...
static const xmlChar * xmlSchemaBuildAbsoluteURI(xmlDictPtr dict, const xmlChar* location, xmlNodePtr ctxtNode)
{
	/*
...
doc_load:
	...
	if(schemaDoc != NULL) {
		...
		if(parserCtxt == NULL) {
			xmlSchemaPErrMemory(NULL, "xmlSchemaGetDoc, allocating a parser context", NULL);
			goto exit_failure;
		}
 ...
 */
static int xmlSchemaParseImport(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema,
    xmlNodePtr node)
{
	xmlNodePtr child;
	...
	if((bucket == NULL) && (schemaLocation != NULL)) {
		xmlSchemaCustomWarning(ACTXT_CAST pctxt,
		    ...
		    "Skipping the import", schemaLocation, 0, 0);
	}
    ...
    int type)
{
	...
	*schemaLocation = NULL;
	...
	if(attr != NULL) {
		...
		xmlSchemaPMissingAttrErr(pctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node, "schemaLocation", NULL);
		goto exit_error;
	}
	/*
	...
	if(sstreq(*schemaLocation, pctxt->URL)) {
		if(type == XML_SCHEMA_SCHEMA_REDEFINE) {
			xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_SRC_REDEFINE, NULL, node, "The schema document '%s' cannot redefine itself.", *schemaLocation);
		}
		else {
			xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_SRC_INCLUDE, NULL, node, "The schema document '%s' cannot include itself.", *schemaLocation);
		}
		goto exit_error;
	}
	return 0;
...
static int xmlSchemaParseIncludeOrRedefine(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNodePtr node, int type)
{
	xmlNodePtr child = NULL;
	 ...
	 */
	res = xmlSchemaParseIncludeOrRedefineAttrs(pctxt, schema, node, (xmlChar**)(&schemaLocation), type);
	if(res != 0)
		return(res);
	/*
	 * Load and add the schema document.
	 */
	res = xmlSchemaAddSchemaDoc(pctxt, type, schemaLocation, NULL, NULL, 0, node, pctxt->targetNamespace, NULL, &bucket);
	if(res != 0)
		return(res);
	/*
	...
	if((bucket == NULL) || (bucket->doc == NULL)) {
		if(type == XML_SCHEMA_SCHEMA_INCLUDE) {
			...
			xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)res, node, NULL, "Failed to load the document '%s' for inclusion", schemaLocation, NULL);
		}
		else {
			/*
			...
			xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)res, node, NULL, "Failed to load the document '%s' for redefinition", schemaLocation, NULL);
		}
	}
	else {
		/*
		...
		if(bucket->origTargetNamespace != NULL) {
			...
			else if(!sstreq(bucket->origTargetNamespace,
	...
	if(child) {
		...
		if(type == XML_SCHEMA_SCHEMA_REDEFINE) {
			xmlSchemaPContentErr(pctxt, (xmlParserErrors)res, NULL, node, child, NULL,
			    "(annotation | (simpleType | complexType | group | attributeGroup))*");
		}
		else {
			xmlSchemaPContentErr(pctxt, (xmlParserErrors)res, NULL, node, child, NULL, "(annotation?)");
		}
	}
	return(res);
...
static int xmlSchemaParseRedefine(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNodePtr node)
{
	int res;
	...
	return 0;
#endif
	res = xmlSchemaParseIncludeOrRedefine(pctxt, schema, node, XML_SCHEMA_SCHEMA_REDEFINE);
	if(res != 0)
		return(res);
	return 0;
}

static int xmlSchemaParseInclude(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNodePtr node)
{
	int res = xmlSchemaParseIncludeOrRedefine(pctxt, schema, node, XML_SCHEMA_SCHEMA_INCLUDE);
	if(res != 0)
		return(res);
	return 0;
}
 ...
 *         1 in case of success.
 */
static xmlSchemaTreeItemPtr xmlSchemaParseModelGroup(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node, xmlSchemaTypeType type, int withParticle)
{
 ...
 */
static xmlSchemaTypePtr xmlSchemaParseRestriction(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node, xmlSchemaTypeType parentType)
{
	...
	if(xmlSchemaPValAttrQName(ctxt, schema, NULL, node, "base",
		...
		if((type->base == NULL) && (type->type == XML_SCHEMA_TYPE_COMPLEX)) {
			...
			else if((!sstreq(type->base, type->name)) ||
			    (!sstreq(type->baseNs, type->targetNamespace))) {
 ...
 */
static xmlSchemaTypePtr xmlSchemaParseExtension(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node, xmlSchemaTypeType parentType)
{
	xmlSchemaTypePtr type;
	xmlNodePtr child = NULL;
	 ...
	 */
	if((xmlSchemaPValAttrQName(ctxt, schema, NULL, node, "base", &(type->baseNs), &(type->base)) == 0) && (type->base == NULL)) {
		xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, node, "base", NULL);
	}
	/*
	 * And now for the children...
	 */
	...
	if(IS_SCHEMA(child, "annotation")) {
		 ...
		 */
		xmlSchemaAddAnnotation((xmlSchemaAnnotItemPtr)type, xmlSchemaParseAnnotation(ctxt, child, 1));
		child = child->next;
	}
	if(parentType == XML_SCHEMA_TYPE_COMPLEX_CONTENT) {
		/*
	...
	if(child) {
		if(parentType == XML_SCHEMA_TYPE_COMPLEX_CONTENT) {
			...
			xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL,
			    "(annotation?, ((group | all | choice | sequence)?, "
			    "((attribute | attributeGroup)*, anyAttribute?)))");
		}
		else {
    ...
    int * hasRestrictionOrExtension)
{
	...
	*hasRestrictionOrExtension = 0;
	...
	if(child == NULL) {
		xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_MISSING, NULL, node, NULL, NULL, "(annotation?, (restriction | extension))");
	}
	if(child == NULL) {
		xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_MISSING, NULL, node, NULL, NULL, "(annotation?, (restriction | extension))");
	}
	...
	if(child) {
		xmlSchemaPContentErr(ctxt, XML_SCHEMAP_S4S_ELEM_NOT_ALLOWED, NULL, node, child, NULL, "(annotation?, (restriction | extension))");
	}
	return 0;
}
 ...
 * Returns the type definition or NULL in case of error
 */
static int xmlSchemaParseComplexContent(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNodePtr node, int * hasRestrictionOrExtension)
{
	xmlSchemaTypePtr type;
	xmlNodePtr child = NULL;
	xmlAttrPtr attr;
 ...
 * Returns the type definition or NULL in case of error
 */
static xmlSchemaTypePtr xmlSchemaParseComplexType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlNodePtr node, int topLevel)
{
	...
	if(topLevel == 0) {
		 ...
		 */
		type = xmlSchemaAddType(ctxt, schema, XML_SCHEMA_TYPE_COMPLEX, name, ctxt->targetNamespace, node, 1);
		if(type == NULL)
			return 0;
		type->node = node;
		type->type = XML_SCHEMA_TYPE_COMPLEX;
 ...
 */
static int xmlSchemaBuildContentModelForSubstGroup(xmlSchemaParserCtxtPtr pctxt,
...
static int xmlSchemaGetEffectiveValueConstraint(xmlSchemaAttributeUsePtr attruse, int * fixed, const xmlChar ** value, xmlSchemaValPtr * val)
{
	...
	*value = NULL;
	...
	else if((attruse->attrDecl != NULL) && (attruse->attrDecl->defValue != NULL)) {
...
static int xmlSchemaCheckCVCWildcardNamespace(xmlSchemaWildcardPtr wild, const xmlChar* ns)
{
	...
	else if((wild->negNsSet != NULL) && (ns != NULL) && (!sstreq(wild->negNsSet->value, ns)))
		return 0;
	return 1;
}
 ...
 *
 */
static int xmlSchemaCheckDerivationOKRestriction2to4(xmlSchemaParserCtxtPtr pctxt,
    int action,
    xmlSchemaBasicItemPtr item,
			...
			for(j = 0; j < baseUses->nbItems; j++) {
				...
				if((WXS_ATTRUSE_DECL_NAME(cur) == WXS_ATTRUSE_DECL_NAME(bcur)) && (WXS_ATTRUSE_DECL_TNS(cur) == WXS_ATTRUSE_DECL_TNS(bcur))) {
					...
					if((cur->occurs == XML_SCHEMAS_ATTR_USE_OPTIONAL) && (bcur->occurs == XML_SCHEMAS_ATTR_USE_REQUIRED)) {
						 ...
						 */
						xmlSchemaGetEffectiveValueConstraint(bcur, &effFixed, &bEffValue, NULL);
						/*
		...
		for(j = 0; j < baseUses->nbItems; j++) {
			...
			if(!found) {
				...
				xmlSchemaCustomErr4(ACTXT_CAST pctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_3,
				    ...
				    WXS_ACTION_STR(action), xmlSchemaGetComponentDesignation(&strB, baseItem), NULL);
				FREE_AND_NULL(strA);
				FREE_AND_NULL(strB);
			}
		}
 ...
 */
static int xmlSchemaCheckUnionTypeDefCircularRecur(xmlSchemaParserCtxtPtr pctxt,
 ...
 */
#if 0
static int xmlSchemaCheckSRCSimpleType(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaTypePtr type)
{
	/*
...
static int xmlSchemaCreateVCtxtOnPCtxt(xmlSchemaParserCtxtPtr ctxt)
{
	if(ctxt->vctxt == NULL) {
		...
		// TODO: Pass user data.
		xmlSchemaSetValidErrors(ctxt->vctxt, ctxt->error, ctxt->warning, ctxt->errCtxt);
...
static int xmlSchemaVCheckCVCSimpleType(xmlSchemaAbstractCtxtPtr actxt,
    xmlNodePtr node, xmlSchemaTypePtr type, const xmlChar * value, xmlSchemaValPtr * retVal,
    ...
    xmlSchemaValPtr * val)
{
	...
	if(WXS_IS_SIMPLE(type))
		ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, node, type, value, val, 1, 1, 0);
	else if(WXS_HAS_SIMPLE_CONTENT(type))
		ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, node, type->contentTypeDef, value, val, 1, 1, 0);
	else
		return ret;
	if(ret < 0) {
    ...
    xmlSchemaTypePtr type)
{
	...
	if(!WXS_IS_COMPLEX(base)) {
		xmlSchemaCustomErr(ACTXT_CAST ctxt,
		    ...
		    "The base type must be a complex type", 0, 0);
		return(ctxt->err);
	}
	if(base->flags & XML_SCHEMAS_TYPE_FINAL_RESTRICTION) {
		...
		xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAP_DERIVATION_OK_RESTRICTION_1,
		    type->node, WXS_BASIC_CAST type, "The 'final' of the base type definition contains 'restriction'", 0, 0);
		return (ctxt->err);
	}
	/*
	 * SPEC (2), (3) and (4)
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckRCaseNSRecurseCheckCardinality(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaParticlePtr r,
    xmlSchemaParticlePtr b)
{
	xmlSchemaParticlePtr part;
...
#define FACET_RESTR_MUTUAL_ERR(fac1, fac2) \
	xmlSchemaPCustomErrExt(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST fac1, fac1->node, \
	    "It is an error for both '%s' and '%s' to be specified on the same type definition", \
	    BAD_CAST xmlSchemaFacetTypeToString(fac1->type), BAD_CAST xmlSchemaFacetTypeToString(fac2->type), NULL);
#define FACET_RESTR_ERR(fac1, msg) \
	xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST fac1, fac1->node, msg, NULL);
#define FACET_RESTR_FIXED_ERR(fac) \
	xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, \
	    WXS_BASIC_CAST fac, fac->node, "The base type's facet is 'fixed', thus the value must not differ", NULL);
...
{
	xmlChar * msg = xmlStrdup(BAD_CAST "'");
	...
	if(ofBase)
		...
		msg = xmlStrcat(msg, BAD_CAST "'");
	xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST facet1, NULL, (const char*)msg, NULL);
	SAlloc::F(msg);
}
    ...
    xmlSchemaTypePtr type)
{
	...
	for(cur = base->facetSet; cur != NULL; cur = cur->next) {
		...
		if(link == NULL) {
			...
			if(link == NULL) {
				xmlSchemaPErrMemory(pctxt, "deriving facets, creating a facet link", NULL);
				return -1;
			}
	...
	switch(facet->type) {
		...
		case XML_SCHEMA_FACET_ENUMERATION: {
		    ...
		    if(ret != 0) {
			    ...
			    if(ctxtGiven) {
				    xmlChar * str = NULL;
				    xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->node, WXS_BASIC_CAST facet,
		...
		case XML_SCHEMA_FACET_PATTERN:
		    ...
		    if(facet->regexp == NULL) {
			    ...
			    /* No error message for RelaxNG. */
			    if(ctxtGiven) {
				    xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->node, WXS_BASIC_CAST typeDecl,
		...
		case XML_SCHEMA_FACET_MINLENGTH:
		    ...
		    if(ret != 0) {
			    ...
			    if(ctxtGiven) {
				    /* error code */
				    xmlSchemaCustomErr4(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->node, WXS_BASIC_CAST typeDecl,
		...
		case XML_SCHEMA_FACET_WHITESPACE: {
		    if(sstreq(facet->value, BAD_CAST "preserve")) {
			    ...
			    if(ctxtGiven) {
				    ...
				    xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->node, WXS_BASIC_CAST typeDecl,
					    "The value '%s' of the facet 'whitespace' is not valid", facet->value, NULL);
			    }
		    }
	    }
		default:
 ...
 * otherwise NULL.
 */
static xmlSchemaTreeItemPtr xmlSchemaGetCircModelGrDefRef(xmlSchemaModelGroupDefPtr groupDef,
 ...
 */
static void xmlSchemaModelGroupToModelGroupDefFixup(xmlSchemaParserCtxtPtr ctxt ATTRIBUTE_UNUSED,
 ...
 */
static xmlSchemaQNameRefPtr xmlSchemaCheckAttrGroupCircularRecur(xmlSchemaAttributeGroupPtr ctxtGr, xmlSchemaItemListPtr list)
{
	...
	for(i = 0; i < list->nbItems; i++) {
		...
		if((ref->type == XML_SCHEMA_EXTRA_QNAMEREF) && (ref->itemType == XML_SCHEMA_TYPE_ATTRIBUTEGROUP) && (ref->item != NULL)) {
			...
			if((gr->attrUses) && (gr->flags & XML_SCHEMAS_ATTRGROUP_HAS_REFS)) {
				...
				circ = xmlSchemaCheckAttrGroupCircularRecur(ctxtGr, (xmlSchemaItemListPtr)gr->attrUses);
				gr->flags ^= XML_SCHEMAS_ATTRGROUP_MARKED;
				if(circ != NULL)
					return (circ);
			}
    ...
    xmlSchemaAttributePtr attr)
{
	...
	if(attr->defValue != NULL) {
		 ...
		 */
		ret = xmlSchemaVCheckCVCSimpleType(ACTXT_CAST pctxt, attr->node, WXS_ATTR_TYPEDEF(attr), attr->defValue, &(attr->defVal),
		    1, 1, 0);
		if(ret != 0) {
			if(ret < 0) {
	...
	if((elemDecl->value != NULL) &&
		...
		if(elemDecl->node != NULL) {
			if(elemDecl->flags & XML_SCHEMAS_ELEM_FIXED)
				node = (xmlNode *)xmlHasProp(elemDecl->node, BAD_CAST "fixed");
			else
				node = (xmlNode *)xmlHasProp(elemDecl->node, BAD_CAST "default");
		}
 ...
 */
static void xmlSchemaResolveModelGroupParticleReferences(xmlSchemaParserCtxtPtr ctxt,
    xmlSchemaModelGroupPtr mg)
{
	xmlSchemaParticlePtr particle = WXS_MODELGROUP_PARTICLE(mg);
	xmlSchemaQNameRefPtr ref;
...
static int xmlSchemaAreValuesEqual(xmlSchemaValPtr x,
    xmlSchemaValPtr y)
{
    ...
    xmlSchemaParserCtxtPtr ctxt)
{
...

	{
		...
		if(ause->attrDecl == NULL) {
			xmlSchemaPResCompAttrErr(ctxt, XML_SCHEMAP_SRC_RESOLVE,
			    WXS_BASIC_CAST ause, ause->node, "ref", ref->name, ref->targetNamespace, XML_SCHEMA_TYPE_ATTRIBUTE, NULL);
			return(ctxt->err);
		}
	}
	return 0;
...
static int xmlSchemaResolveAttrUseProhibReferences(xmlSchemaAttributeUseProhibPtr prohib,
    xmlSchemaParserCtxtPtr pctxt)
{
...
static int xmlSchemaCheckSRCRedefineFirst(xmlSchemaParserCtxtPtr pctxt)
{
	...
	do {
		...
		if(wasRedefined) {
			xmlChar * str = NULL;
			xmlNodePtr node =  redef->reference ? WXS_ITEM_NODE(redef->reference) : WXS_ITEM_NODE(redef->item);
			xmlSchemaCustomErr(ACTXT_CAST pctxt,
			    /* TODO: error code. */
			    XML_SCHEMAP_SRC_REDEFINE, node, NULL,
...
xmlSchemaPtr xmlSchemaParse(xmlSchemaParserCtxtPtr ctxt)
{
	...
	if(bucket == NULL) {
		...
		if(ctxt->URL)
			xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAP_FAILED_LOAD, NULL, NULL, "Failed to locate the main schema resource at '%s'", ctxt->URL, NULL);
		else
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaAssembleByLocation(xmlSchemaValidCtxtPtr vctxt, xmlSchemaPtr schema, xmlNodePtr node, const xmlChar * nsName, const xmlChar * location)
{
	int ret = 0;
	xmlSchemaParserCtxtPtr pctxt;
	xmlSchemaBucketPtr bucket = NULL;
	 ...
	 */
	ret = xmlSchemaAddSchemaDoc(pctxt, XML_SCHEMA_SCHEMA_IMPORT, location, NULL, NULL, 0, node, NULL, nsName, &bucket);
	if(ret != 0)
		return ret;
	if(bucket == NULL) {
		/*
		 ...
		 */
		xmlSchemaCustomWarning(ACTXT_CAST vctxt, XML_SCHEMAV_MISC, node, NULL, "The document at location '%s' could not be acquired", location, 0, 0);
		return ret;
	}
	/*
...
static xmlSchemaAttrInfoPtr xmlSchemaGetMetaAttrInfo(xmlSchemaValidCtxtPtr vctxt, int metaType)
{
		...
		if(end == cur) {
			if(iattr->metaType ==
				...
				xmlSchemaCustomWarning(ACTXT_CAST vctxt, XML_SCHEMAV_MISC,
				    ...
				    "name and the document's URI", NULL, 0, 0);
			}
			break;
		}
		count++; /* TODO: Don't use the schema's dict. */
...
static void xmlSchemaAugmentIDC(xmlSchemaIDCPtr idcDef, xmlSchemaValidCtxtPtr vctxt)
{
	...
	if(aidc == NULL) {
		xmlSchemaVErrMemory(vctxt, "xmlSchemaAugmentIDC: allocating an augmented IDC definition", NULL);
		return;
	}
	aidc->keyrefDepth = -1;
	aidc->def = idcDef;
...
static int xmlSchemaIDCStoreNodeTableItem(xmlSchemaValidCtxtPtr vctxt, xmlSchemaPSVIIDCNodePtr item)
{
	...
	if(vctxt->idcNodes == NULL) {
		...
		if(vctxt->idcNodes == NULL) {
			xmlSchemaVErrMemory(vctxt, "allocating the IDC node table item list", NULL);
			return -1;
		}
		...
		if(vctxt->idcNodes == NULL) {
			xmlSchemaVErrMemory(vctxt, "re-allocating the IDC node table item list", NULL);
			return -1;
		}
	}
...
static int xmlSchemaIDCStoreKey(xmlSchemaValidCtxtPtr vctxt, xmlSchemaPSVIIDCKeyPtr key)
{
	...
	if(vctxt->idcKeys == NULL) {
		...
		if(vctxt->idcKeys == NULL) {
			xmlSchemaVErrMemory(vctxt, "re-allocating the IDC key storage list", NULL);
			return -1;
		}
	}
...
static int xmlSchemaIDCAppendNodeTableItem(xmlSchemaPSVIIDCBindingPtr bind, xmlSchemaPSVIIDCNodePtr ntItem)
{
	if(bind->nodeTable == NULL) {
		...
		if(bind->nodeTable == NULL) {
			xmlSchemaVErrMemory(NULL, "allocating an array of IDC node-table items", NULL);
			return -1;
		}
	}
		...
		if(bind->nodeTable == NULL) {
			xmlSchemaVErrMemory(NULL, "re-allocating an array of IDC node-table items", NULL);
			return -1;
		}
	}
 ...
 */
static xmlSchemaPSVIIDCBindingPtr xmlSchemaIDCAcquireBinding(xmlSchemaValidCtxtPtr vctxt,
...
static xmlSchemaItemListPtr xmlSchemaIDCAcquireTargetList(xmlSchemaValidCtxtPtr vctxt ATTRIBUTE_UNUSED,
    xmlSchemaIDCMatcherPtr matcher)
{
...
static int xmlSchemaXPathEvaluate(xmlSchemaValidCtxtPtr vctxt, xmlElementType nodeType)
{
	...
	while(sto != head) {
		...
		if(sto->history == NULL) {
			...
			if(sto->history == NULL) {
				xmlSchemaVErrMemory(NULL, "re-allocating the state object history", NULL);
				return -1;
			}
		}
		...
		if(sto->type == XPATH_STATE_OBJ_TYPE_IDC_FIELD) {
			...
			if((key == NULL) && (vctxt->inode->val == NULL)) {
				...
				if(matcher->keySeqs == NULL) {
					...
					if(matcher->keySeqs == NULL) {
						xmlSchemaVErrMemory(NULL, "reallocating an array of key-sequences", NULL);
						return -1;
					}
					/*
...
selector_key_error:
			{
				...
				xmlSchemaCustomErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_IDC, NULL,
				    WXS_BASIC_CAST idc, "Not all fields of %s evaluate to a node", xmlSchemaGetIDCDesignation(&str, idc), NULL);
				FREE_AND_NULL(str);
			}
selector_leave:
			/*
		...
		if(parBind != NULL) {
			...
			for(i = 0; i < bind->nbNodes; i++) {
				...
				if(oldDupls) {
					...
					while(j < oldDupls) {
						if(nbFields == 1) {
							ret = xmlSchemaAreValuesEqual(node->keys[0]->val, dupls[j]->keys[0]->val);
							if(ret == -1)
								goto internal_error;
							if(ret == 0) {
								j++;
							...
							parNode = dupls[j];
							for(k = 0; k < nbFields; k++) {
								ret = xmlSchemaAreValuesEqual(node->keys[k]->val, parNode->keys[k]->val);
								if(ret == -1)
									goto internal_error;
								if(ret == 0)
									break;
				...
				if(oldNum) {
					...
					while(j < oldNum) {
						parNode = parNodes[j];
						if(nbFields == 1) {
							ret = xmlSchemaAreValuesEqual(node->keys[0]->val, parNode->keys[0]->val);
							if(ret == -1)
								goto internal_error;
							if(ret == 0) {
								j++;
					...
					if(j != oldNum) {
						...
						if(parBind->nodeTable == NULL) {
							...
							if(parBind->nodeTable == NULL) {
								xmlSchemaVErrMemory(NULL, "allocating IDC list of node-table items", NULL);
								goto internal_error;
							}
							parBind->sizeNodes = 1;
						}
							...
							if(parBind->nodeTable == NULL) {
								xmlSchemaVErrMemory(NULL, "re-allocating IDC list of node-table items", NULL);
								goto internal_error;
							}
						}
						parNodes = parBind->nodeTable;
 ...
 */
static void xmlSchemaClearElemInfo(xmlSchemaValidCtxtPtr vctxt, xmlSchemaNodeInfoPtr ielem)
{
	ielem->hasKeyrefs = 0;
    ...
    int fireErrors)
{
	...
	for(facetLink = type->facetSet; facetLink != NULL; facetLink = facetLink->next) {
		...
		if(ret < 0) {
			...
			if(fireErrors)
				xmlSchemaFacetErr(actxt, (xmlParserErrors)ret, node, value, len, type, facetLink->facet, NULL, 0, 0);
			else
				return ret;
			if(error == 0)
				error = ret;
...
WXS_IS_LIST:
	...
	for(facetLink = type->facetSet; facetLink != NULL; facetLink = facetLink->next) {
		switch(facetLink->facet->type) {
			...
			case XML_SCHEMA_FACET_MINLENGTH:
			case XML_SCHEMA_FACET_MAXLENGTH:
			    ret = xmlSchemaValidateListSimpleTypeFacet(facetLink->facet, value, length, NULL);
			    break;
			default:
			    continue;
		}
		if(ret < 0) {
			...
			if(fireErrors)
				xmlSchemaFacetErr(actxt, (xmlParserErrors)ret, node, value, length, type, facetLink->facet, NULL, 0, 0);
			else
				return ret;
			if(error == 0)
				error = ret;
...
pattern_and_enum:
	if(error >= 0) {
		...
		if(found && (ret == 0)) {
			...
			if(fireErrors) {
				xmlSchemaFacetErr(actxt, (xmlParserErrors)ret, node, value, 0, type, NULL, NULL, 0, 0);
			}
			else
				return ret;
			if(error == 0)
	...
	if(error >= 0) {
		...
		do {
			...
			if(found && (ret != 1)) {
				...
				if(fireErrors) {
					xmlSchemaFacetErr(actxt, (xmlParserErrors)ret, node, value, 0, type, facet, NULL, 0, 0);
				}
				else
					return ret;
				if(error == 0)
	...
	if(prefix != NULL) {
		...
		if(nsName == NULL) {
			...
			xmlSchemaCustomErr(ACTXT_CAST vctxt, (xmlParserErrors)ret, NULL,
			    ...
			    "The QName value '%s' has no corresponding namespace declaration in scope", value, NULL);
			if(local != NULL)
				SAlloc::F(local);
			return ret;
		}
		...
		if(elemDecl != NULL) {
			...
			if(xmlSchemaCheckCOSDerivedOK(ACTXT_CAST vctxt, *localType, elemDecl->subtypes, set) != 0) {
				...
				xmlSchemaCustomErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_ELT_4_3, NULL, NULL,
				    ...
				    xmlSchemaFormatQName(&str, (*localType)->targetNamespace, (*localType)->name), NULL);
				FREE_AND_NULL(str);
				ret = vctxt->err;
				*localType = NULL;
			}
...
static int xmlSchemaVAttributesSimple(xmlSchemaValidCtxtPtr vctxt)
{
	...
	if(vctxt->nbAttrInfos) {
		for(int i = 0; i < vctxt->nbAttrInfos; i++) {
			...
			if(!iattr->metaType) {
				ACTIVATE_ATTRIBUTE(iattr)
				xmlSchemaIllegalAttrErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_TYPE_3_1_1, iattr, NULL);
		...
		if(iattr->state == XML_SCHEMAS_ATTR_DEFAULT) {
			...
			if(defAttrOwnerElem) {
				...
				if(iattr->nsName == NULL) {
					...
					xmlNsPtr ns = xmlSearchNsByHref(defAttrOwnerElem->doc, defAttrOwnerElem, iattr->nsName);
					if(ns == NULL) {
						xmlChar prefix[12];
						int counter = 0;
						/*
...
static int xmlSchemaVCheckINodeDataType(xmlSchemaValidCtxtPtr vctxt,
    xmlSchemaNodeInfoPtr inode, xmlSchemaTypePtr type, const xmlChar * value)
{
...
default_psvi:
		 ...
		 */
		if((vctxt->options & XML_SCHEMA_VAL_VC_I_CREATE) && inode->node) {
...
static int xmlSchemaValidateChildElem(xmlSchemaValidCtxtPtr vctxt)
{
	...
	if(ptype->builtInType == XML_SCHEMAS_ANYTYPE) {
		...
		if(vctxt->inode->decl == NULL) {
			...
			iattr = xmlSchemaGetMetaAttrInfo(vctxt, XML_SCHEMA_ATTR_INFO_META_XSI_TYPE);
			if(iattr != NULL) {
				ret = xmlSchemaProcessXSIType(vctxt, iattr, &(vctxt->inode->typeDef), NULL);
				if(ret != 0) {
					if(ret == -1) {
...
static int xmlSchemaVPushText(xmlSchemaValidCtxtPtr vctxt,
    int nodeType, const xmlChar * value, int len, int mode, int * consumed)
{
	/*
...
static void xmlSchemaSAXHandleStartElementNs(void * ctx,
    const xmlChar * localname,
	...
	if(nb_namespaces != 0) {
		...
		for(i = 0, j = 0; i < nb_namespaces; i++, j += 2) {
			...
			if(ielem->nsBindings == NULL) {
				...
				if(ielem->nsBindings == NULL) {
					xmlSchemaVErrMemory(vctxt, "allocating namespace bindings for SAX validation", NULL);
					goto internal_error;
				}
				ielem->nbNsBindings = 0;
				...
				if(ielem->nsBindings == NULL) {
					xmlSchemaVErrMemory(vctxt, "re-allocating namespace bindings for SAX validation", NULL);
					goto internal_error;
				}
			}
...
static void xmlSchemaSAXHandleEndElementNs(void * ctx,
    ...
    const xmlChar * URI ATTRIBUTE_UNUSED)
{
	 ...
	 */
	if((!sstreq(vctxt->inode->localName, localname)) ||
	    (!sstreq(vctxt->inode->nsName, URI))) {
...
xmlSchemaValidCtxtPtr xmlSchemaNewValidCtxt(xmlSchemaPtr schema)
{
	...
	ret = (xmlSchemaValidCtxtPtr)SAlloc::M(sizeof(xmlSchemaValidCtxt));
	if(!ret) {
	...
	if(ctxt) {
		...
		if(ctxt->idcNodes != NULL) {
			for(int i = 0; i < ctxt->nbIdcNodes; i++) {
				xmlSchemaPSVIIDCNodePtr item = ctxt->idcNodes[i];
				SAlloc::F(item->keys);
				SAlloc::F(item);
			}
	...
	while(node) {
		...
		if(node->type == XML_ELEMENT_NODE) {
			...
			if(node->properties != NULL) {
				attr = node->properties;
				do {
					nsName = (attr->ns != NULL) ? attr->ns->href : NULL;
		...
		else if((node->type == XML_ENTITY_NODE) || (node->type == XML_ENTITY_REF_NODE)) {
			/*
...
{
	if(!ctxt || (elem == NULL) || (elem->type != XML_ELEMENT_NODE))
		return -1;
	if(ctxt->schema == NULL)
		return -1;
	ctxt->doc = elem->doc;
...
int xmlSchemaValidateDoc(xmlSchemaValidCtxtPtr ctxt, xmlDocPtr doc)
{
	...
	if(ctxt->node == NULL) {
		xmlSchemaCustomErr(ACTXT_CAST ctxt, XML_SCHEMAV_DOCUMENT_ELEMENT_MISSING, (xmlNode *)doc, NULL,
		    "The document has no document element", 0, 0);
		return (ctxt->err);
	}
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->isStandalone != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->hasInternalSubset != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->hasExternalSubset != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->externalSubset != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->resolveEntity != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && ctxt->user_sax->getEntity)
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->getParameterEntity != NULL))
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->warning != NULL)) {
		TODO
	}
}
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->error != NULL)) {
		TODO
	}
}
...
{
	xmlSchemaSAXPlugPtr ctxt = (xmlSchemaSAXPlugPtr)ctx;
	if(ctxt && ctxt->user_sax && (ctxt->user_sax->fatalError != NULL)) {
		TODO
	}
}
	...
	if(ctxt) {
		if((ctxt->user_sax != NULL) && (ctxt->user_sax->startElementNs != NULL))
			ctxt->user_sax->startElementNs(ctxt->user_data, localname, prefix,
				URI, nb_namespaces, namespaces, nb_attributes, nb_defaulted, attributes);
		if(ctxt->ctxt != NULL)
...
xmlSchemaSAXPlugPtr xmlSchemaSAXPlug(xmlSchemaValidCtxtPtr ctxt, xmlSAXHandlerPtr * sax, void ** user_data)
{
	...
	if((old_sax != NULL) && (old_sax->initialized != XML_SAX2_MAGIC))
		return 0;
	if((old_sax != NULL) && (old_sax->startElementNs == NULL) && (old_sax->endElementNs == NULL) &&
	...
	if(old_sax == NULL) {
		...
		*user_data = ctxt;
		...
		if(old_sax->ignorableWhitespace && old_sax->ignorableWhitespace != old_sax->characters)
