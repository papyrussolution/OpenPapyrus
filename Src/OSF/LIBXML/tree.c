...
int __xmlRegisterCallbacks = 0; // @global
// 
...
xmlDtd * FASTCALL xmlGetIntSubset(const xmlDoc * doc)
{
	if(!doc)
        ...
        for(const xmlNode * cur = doc->children; cur; cur = cur->next) {
...
xmlChar * xmlGetNodePath(const xmlNode * pNode)
{
	...
	do {
		...
		if(oneof2(cur->type, XML_DOCUMENT_NODE, XML_HTML_DOCUMENT_NODE)) {
			...
			tmp = cur->prev;
			while(tmp) {
				if(oneof2(tmp->type, XML_TEXT_NODE, XML_CDATA_SECTION_NODE))
					occur++;
				tmp = tmp->prev;
			}
			// 
...
{
	xmlNode * old = NULL;
	if(!doc || !root || root->type == XML_NAMESPACE_DECL) 
		return 0;
	xmlUnlinkNode(root);
...
static xmlNs * xmlNewReconciliedNs(xmlDoc * doc, xmlNode * tree, xmlNs * ns) 
{
	...
	xmlChar prefix[50];
	int counter = 1;
	if(!tree || tree->type != XML_ELEMENT_NODE) {
#ifdef DEBUG_TREE
...
static xmlAttr * FASTCALL xmlGetPropNodeInternal(const xmlNode * pNode, const xmlChar * name, const xmlChar * nsName, int useDTD)
{
	xmlAttr * prop;
	...
	if(pNode->properties) {
		...
		if(!nsName) {
			...
			// 
			do {
				if(!prop->ns && sstreq(prop->name, name))
					return prop;
				prop = prop->next;
			} while(prop);
		}
	 ...
	 */
	if(pNode->doc && pNode->doc->intSubset) {
		...
		if(pNode->ns && pNode->ns->prefix) {
			tmpstr = sstrdup(pNode->ns->prefix);
			tmpstr = xmlStrcat(tmpstr, reinterpret_cast<const xmlChar *>(":"));
			tmpstr = xmlStrcat(tmpstr, pNode->name);
			if(tmpstr == NULL)
				return 0;
			elemQName = tmpstr;
		}
		else
			elemQName = (xmlChar *)pNode->name;
		if(!nsName) {
			//
			...
			//
			xmlNs ** nsList = xmlGetNsList(pNode->doc, pNode);
			if(!nsList) {
				SAlloc::F(tmpstr);
				return 0;
			}
...
xmlAttr * FASTCALL xmlHasProp(const xmlNode * pNode, const xmlChar * name) 
{
	...
	if(doc) {
		if(doc->intSubset) {
			xmlAttribute * attrDecl = xmlGetDtdAttrDesc(doc->intSubset, pNode->name, name);
			if(!attrDecl && doc->extSubset)
				attrDecl = xmlGetDtdAttrDesc(doc->extSubset, pNode->name, name);
			if(attrDecl && attrDecl->defaultValue)
				return (xmlAttr *)attrDecl; // return attribute declaration only if a default value is given (that includes #FIXED declarations) 
		}
	}
	return 0;
}
...
xmlAttr * FASTCALL xmlSetNsProp(xmlNode * pNode, xmlNs * ns, const xmlChar * name, const xmlChar * value)
{
	...
	if(prop) {
		...
		if(value) {
			if(!xmlCheckUTF8(value)) {
				xmlTreeErr(XML_TREE_NOT_UTF8, (xmlNode *)pNode->doc, 0);
				if(pNode->doc)
					pNode->doc->encoding = sstrdup(reinterpret_cast<const xmlChar *>("ISO-8859-1"));
			}
			prop->children = xmlNewDocText(pNode->doc, value);
			prop->last = NULL;
			{
		...
		if(prop->atype == XML_ATTRIBUTE_ID)
			xmlAddID(NULL, pNode->doc, value, prop);
		return prop;
	}
	/*
 ...
 *
 * Returns the aquired ns struct or NULL in case of an API or internal error.
 */
					    ...
					    if(optRemoveRedundantNS && XML_NSMAP_NOTEMPTY(nsMap)) {
						    XML_NSMAP_FOREACH(nsMap, mi) {
							    if((mi->depth >= XML_TREE_NSMAP_PARENT) && (mi->shadowDepth == -1) &&
									((ns->prefix == mi->newNs->prefix) || sstreq(ns->prefix, mi->newNs->prefix)) && ((ns->href == mi->newNs->href) || sstreq(ns->href, mi->newNs->href))) {
 ...
 * Returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
static int xmlDOMWrapAdoptBranch(xmlDOMWrapCtxtPtr ctxt, xmlDoc * sourceDoc, xmlNode * pNode, xmlDoc * destDoc, xmlNode * destParent, int options ATTRIBUTE_UNUSED)
{
	int ret = 0;
	xmlNode * cur;
	xmlNode * curElem = NULL;
	...
	while(cur) {
		...
		if(cur->doc != sourceDoc) {
			...
			do {
				cur = cur->next;
				if(cur->type == XML_XINCLUDE_END || cur->doc == pNode->doc)
					break;
			} while(cur->next);
			if(cur->doc != pNode->doc)
				goto leave_node;
		}
		cur->doc = destDoc;
		switch(cur->type) {
		...
		*/
		...
		if(ctxt && ctxt->getNsForNodeFunc) {
			...
			if(xmlDOMWrapNSNormAquireNormalizedNs(destDoc,
				    ...
				    ancestorsOnly,
			            /* ns-decls must be prefixed for attributes. */
				    BIN(cur->type == XML_ATTRIBUTE_NODE)) == -1)
				goto internal_error;
			clone->ns = ns;
		}
end_ns_reference:
	...
	if(attr->ns) {
		...
		if(IS_STR_XML(attr->ns->prefix)) {
			...
			if(xmlSearchNsByNamespaceStrict(destDoc, destParent, attr->ns->href, &ns, 1) == -1)
				goto internal_error;
			SETIFZ(ns, xmlDOMWrapNSNormDeclareNsForced(destDoc, destParent, attr->ns->href, attr->ns->prefix, 1));
		}
		if(!ns)
			goto internal_error;
		attr->ns = ns;
