...
void FASTCALL xmlFreeDtd(xmlDtd * pCur) 
{
	...
	if(pCur) {
		...
		if(pCur->children) {
			xmlNodePtr next, c = pCur->children;
			// 
...
xmlDocPtr xmlNewDoc(const xmlChar * version)
{
	 ...
	 */
	xmlDocPtr cur = (xmlDocPtr)SAlloc::M(sizeof(xmlDoc));
	if(!cur) {
			...
			if((cur + 2 < end) && (cur[1] == '#') && (cur[2] == 'x')) {
				...
				while(tmp != ';') { /* Non input consuming loop */
					if((tmp >= '0') && (tmp <= '9'))
						...
						xmlTreeErr(XML_TREE_INVALID_HEX, (xmlNode *)doc,
						    NULL);
						charval = 0;
						break;
					}
				...
				while(tmp != ';') { /* Non input consuming loops */
					if((tmp >= '0') && (tmp <= '9'))
						...
						xmlTreeErr(XML_TREE_INVALID_DEC, (xmlNode *)doc,
						    NULL);
						charval = 0;
						break;
					}
				...
				if((cur >= end) || (*cur == 0)) {
					xmlTreeErr(XML_TREE_UNTERMINATED_ENTITY, (xmlNode *)doc,
					    (const char*)q);
					goto out;
				}
				if(cur != q) {
...
static xmlAttrPtr xmlNewPropInternal(xmlNodePtr node, xmlNsPtr ns, const xmlChar * name, const xmlChar * value, int eatname)
{
	...
	if(value) {
		...
		if(!xmlCheckUTF8(value)) {
			xmlTreeErr(XML_TREE_NOT_UTF8, (xmlNode *)doc, NULL);
			if(doc != NULL)
...
{
	xmlNodePtr cur = xmlNewDocNode(doc, ns, name, NULL);
	if(cur) {
		cur->doc = doc;
		if(content != NULL) {
		...
		if(doc != NULL) {
			if(doc->intSubset != NULL) {
				if(xmlHashLookup((xmlHashTablePtr)doc->intSubset->entities, cur->name) == cur)
					xmlHashRemoveEntry((xmlHashTablePtr)doc->intSubset->entities, cur->name, NULL);
				if(xmlHashLookup((xmlHashTablePtr)doc->intSubset->pentities, cur->name) == cur)
					xmlHashRemoveEntry((xmlHashTablePtr)doc->intSubset->pentities, cur->name, NULL);
			}
			if(doc->extSubset != NULL) {
				if(xmlHashLookup((xmlHashTablePtr)doc->extSubset->entities, cur->name) == cur)
					xmlHashRemoveEntry((xmlHashTablePtr)doc->extSubset->entities, cur->name, NULL);
				if(xmlHashLookup((xmlHashTablePtr)doc->extSubset->pentities, cur->name) == cur)
					xmlHashRemoveEntry((xmlHashTablePtr)doc->extSubset->pentities, cur->name, NULL);
			}
		}
	}
	if(cur->parent != NULL) {
...
{
	return xmlStaticCopyNodeList(node, doc, NULL);
}
...
void xmlNodeSetContent(xmlNodePtr cur, const xmlChar * content) {
	...
	switch(cur->type) {
		...
		case XML_ELEMENT_NODE:
		case XML_ATTRIBUTE_NODE:
		    if(cur->children) xmlFreeNodeList(cur->children);
    ...
    const xmlChar * value)
{
	...
	if(prop != NULL) {
		...
		if(value) {
			...
			if(!xmlCheckUTF8(value)) {
				xmlTreeErr(XML_TREE_NOT_UTF8, (xmlNode *)node->doc,
				    NULL);
				if(node->doc != NULL)
...
{
	if(doc) {
		xmlNsPtr ns = xmlTreeEnsureXMLDecl(doc);
		if(ns) {
			if(ns->next) {
				/* Reuse. */
				ns = ns->next;
 ...
 */
static int xmlDOMWrapNSNormAquireNormalizedNs(xmlDocPtr doc,
    xmlNodePtr elem,
    xmlNsPtr ns,
    xmlNsPtr * retNs,
    xmlNsMapPtr * nsMap,
