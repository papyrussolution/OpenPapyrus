...
xmlDocPtr xmlNewDoc(const xmlChar * version)
{
	 ...
	 */
	xmlDocPtr cur = (xmlDocPtr)xmlMalloc(sizeof(xmlDoc));
	if(cur == NULL) {
 ...
 *
 * Returns a pointer to the string copy, the caller must free it with free().
 */
	...
	if(list) {
		...
		while(node != NULL) {
			if(oneof2(node->type, XML_TEXT_NODE, XML_CDATA_SECTION_NODE)) {
				...
				xmlGenericError(xmlGenericErrorContext, "xmlGetNodeListString : invalid node type %d\n", node->type);
			}
	#endif
			node = node->next;
		}
...
xmlNodePtr xmlNewDocPI(xmlDocPtr doc, const xmlChar * name, const xmlChar * content)
{
	 ...
	 */
	cur = (xmlNode *)xmlMalloc(sizeof(xmlNode));
	if(cur == NULL) {
...
xmlNodePtr xmlNewDocFragment(xmlDocPtr doc)
{
	...
	//
	xmlNodePtr cur = (xmlNode *)xmlMalloc(sizeof(xmlNode));
	if(!cur)
...
xmlChar * xmlGetNodePath(const xmlNode * node)
{
	...
	do {
		...
		if((cur->type == XML_DOCUMENT_NODE) || (cur->type == XML_HTML_DOCUMENT_NODE)) {
			...
			tmp = cur->prev;
			while(tmp != NULL) {
				if((tmp->type == XML_ELEMENT_NODE) && (generic || (xmlStrEqual(cur->name, tmp->name) &&
...
{
	xmlBufferPtr ret = (xmlBufferPtr)xmlMalloc(sizeof(xmlBuffer));
	if(ret == NULL) {
...
{
	xmlDOMWrapCtxtPtr ret = (xmlDOMWrapCtxtPtr)xmlMalloc(sizeof(xmlDOMWrapCtxt));
	if(ret == NULL) {
    ...
    xmlNodePtr node)
{
	 ...
	 */
	cur = node;
	while((cur != NULL) && (cur != (xmlNode *)cur->doc)) {
		if(cur->type == XML_ELEMENT_NODE) {
			if(cur->nsDef != NULL) {
				ns = cur->nsDef;
				do {
	...
	while(cur != NULL) {
		...
		switch(cur->type) {
			...
			case XML_ENTITY_NODE:
			     ...
			     */
			    clone = (xmlNode *)xmlMalloc(sizeof(xmlNode));
			    if(clone == NULL) {
			...
			case XML_ATTRIBUTE_NODE:
			     ...
			     * Attributes (xmlAttr).
			     */
			    clone = (xmlNode *)xmlMalloc(sizeof(xmlAttr));
			    if(clone == NULL) {
