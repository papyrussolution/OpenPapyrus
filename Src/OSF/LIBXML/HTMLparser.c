...
static void htmlErrMemory(xmlParserCtxtPtr ctxt, const char * extra)
{
	...
	if(extra)
		__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL, NULL, 0, 0, "Memory allocation failed : %s\n", extra);
	else
		__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "Memory allocation failed\n");
}
...
static void htmlParseErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_HTML, error, XML_ERR_ERROR, NULL, 0, (const char*)str1, (const char*)str2, NULL, 0, 0, msg, str1, str2);
	if(ctxt)
		ctxt->wellFormed = 0;
}
/**
...
static void htmlParseErrInt(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char * msg, int val)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, NULL, XML_FROM_HTML, error, XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, val, 0, msg, val);
	if(ctxt)
		ctxt->wellFormed = 0;
}
...
{
	int i = 0;
	while(htmlEndPriority[i].name && !xmlStrEqual((const xmlChar*)htmlEndPriority[i].name, name))
		i++;
...
{
	htmlParserInputPtr input = (xmlParserInputPtr)xmlMalloc(sizeof(htmlParserInput));
	if(input == NULL) {
	...
	if(encoding != NULL) {
		...
		if(enc != XML_CHAR_ENCODING_ERROR) {
			if(((enc == XML_CHAR_ENCODING_UTF16LE) || (enc == XML_CHAR_ENCODING_UTF16BE) || (enc == XML_CHAR_ENCODING_UCS4LE) ||
				    (enc == XML_CHAR_ENCODING_UCS4BE)) && (ctxt->input->buf != NULL) && (ctxt->input->buf->encoder == NULL)) {
...
htmlParserCtxtPtr htmlCreateFileParserCtxt(const char * filename, const char * encoding)
{
	...
	if(encoding) {
		...
		if(l < 1000) {
			content = (xmlChar *)xmlMallocAtomic(xmlStrlen(content_line) + l + 1);
			if(content) {
