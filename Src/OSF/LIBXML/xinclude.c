...
static int xmlXIncludeDoProcess(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr tree);
 ...
 */
static void xmlXIncludeErrMemory(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node, const char * extra)
{
	if(ctxt)
		ctxt->nbErrors++;
	__xmlRaiseError(0, 0, 0, ctxt, node, XML_FROM_XINCLUDE, XML_ERR_NO_MEMORY, XML_ERR_ERROR, NULL, 0, extra, 0, 0, 0, 0, "Memory allocation failed : %s\n", extra);
}
/**
 * xmlXIncludeErr:
 ...
 */
static void xmlXIncludeErr(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node, int error, const char * msg, const xmlChar * extra)
{
	if(ctxt)
		ctxt->nbErrors++;
	__xmlRaiseError(0, 0, 0, ctxt, node, XML_FROM_XINCLUDE, error, XML_ERR_ERROR, NULL, 0, (const char*)extra, NULL, NULL, 0, 0, msg, (const char*)extra);
}
#if 0
/**
 * xmlXIncludeWarn:
 ...
 */
static void xmlXIncludeWarn(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node, int error, const char * msg, const xmlChar * extra)
{
 ...
 * Returns the new set
 */
static xmlXIncludeRefPtr xmlXIncludeNewRef(xmlXIncludeCtxtPtr ctxt, const xmlChar * URI, xmlNodePtr ref) 
{
...
static void xmlXIncludeRecurseDoc(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc,
	...
	if(newctxt != NULL) {
		...
		newctxt->incTab = (xmlXIncludeRefPtr*)SAlloc::M(newctxt->incMax *
 ...
 */
static void xmlXIncludeAddTxt(xmlXIncludeCtxtPtr ctxt, xmlNodePtr txt, const xmlURL url) {
#ifdef DEBUG_XINCLUDE
...
static xmlNodePtr xmlXIncludeCopyNodeList(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
 ...
 */
static xmlNodePtr xmlXIncludeCopyRange(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target, xmlDocPtr source, xmlXPathObjectPtr range)
{
 ...
 *         the caller has to free the node tree.
 */
static xmlNodePtr xmlXIncludeCopyXPointer(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
 ...
 */
static void xmlXIncludeMergeEntity(xmlEntityPtr ent, xmlXIncludeMergeDataPtr data,
...
error:
	...
	xmlXIncludeErr(ctxt, (xmlNode *)ent, XML_XINCLUDE_ENTITY_DEF_MISMATCH,
...
static int xmlXIncludeLoadDoc(xmlXIncludeCtxtPtr ctxt, const xmlChar * url, int nr) {
	 ...
	 */
	if((URL[0] == 0) || (URL[0] == '#') ||
	    ((ctxt->doc != NULL) && (sstreq(URL, ctxt->doc->URL)))) {
		doc = NULL;
		goto loaded;
	}
			...
			while(node) {
				...
				if(node->type == XML_ELEMENT_NODE) {
					...
					if(curBase == NULL) {
						 ...
						 */
						if(sstreq(curBase, node->doc->URL)) {
							xmlNodeSetBase(node, base);
						}
						else {
							/*
 ...
 * Returns 0 in case of success, -1 in case of failure
 */
static int xmlXIncludeLoadFallback(xmlXIncludeCtxtPtr ctxt, xmlNodePtr fallback, int nr) 
{
	xmlXIncludeCtxtPtr newctxt;
	int ret = 0;
 ...
 *
 */
static void xmlXIncludePreProcessNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node) 
{
...
static int xmlXIncludeTestNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node) 
{
	...
	if(node->ns == NULL)
		return 0;
	if((sstreq(node->ns->href, XINCLUDE_NS)) ||
	    (sstreq(node->ns->href, XINCLUDE_OLD_NS))) {
		if(sstreq(node->ns->href, XINCLUDE_OLD_NS)) {
			if(ctxt->legacy == 0) {
		...
		if(sstreq(node->name, XINCLUDE_NODE)) {
			xmlNodePtr child = node->children;
			int nb_fallback = 0;
			while(child) {
				if((child->type == XML_ELEMENT_NODE) && child->ns && ((sstreq(child->ns->href, XINCLUDE_NS)) ||
					    (sstreq(child->ns->href, XINCLUDE_OLD_NS)))) {
					if(sstreq(child->name, XINCLUDE_NODE)) {
						xmlXIncludeErr(ctxt, node, XML_XINCLUDE_INCLUDE_IN_INCLUDE, "%s has an 'include' child\n", XINCLUDE_NODE);
						return 0;
					}
			...
			if(nb_fallback > 1) {
				xmlXIncludeErr(ctxt, node, XML_XINCLUDE_FALLBACKS_IN_INCLUDE, "%s has multiple fallback children\n", XINCLUDE_NODE);
				return 0;
			}
			return 1;
		}
		if(sstreq(node->name, XINCLUDE_FALLBACK)) {
			if(!node->parent || (node->parent->type != XML_ELEMENT_NODE) ||
			    !node->parent->ns || ((!sstreq(node->parent->ns->href, XINCLUDE_NS)) &&
				    (!sstreq(node->parent->ns->href, XINCLUDE_OLD_NS))) ||
			    (!sstreq(node->parent->name, XINCLUDE_NODE))) {
