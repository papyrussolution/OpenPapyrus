...
int xmlAddEncodingAlias(const char * name, const char * alias)
{
	...
	if(EncBlk.xmlCharEncodingAliases == NULL) {
		...
		EncBlk.xmlCharEncodingAliases = (xmlCharEncodingAliasPtr)xmlMalloc(EncBlk.xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
		if(EncBlk.xmlCharEncodingAliases == NULL)
			return -1;
	}
	else if(EncBlk.xmlCharEncodingAliasesNb >= EncBlk.xmlCharEncodingAliasesMax) {
		EncBlk.xmlCharEncodingAliasesMax *= 2;
		EncBlk.xmlCharEncodingAliases = (xmlCharEncodingAliasPtr)xmlRealloc(EncBlk.xmlCharEncodingAliases, EncBlk.xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
	}
	//
	// Walk down the list looking for a definition of the alias
	//
	for(i = 0; i < EncBlk.xmlCharEncodingAliasesNb; i++) {
		if(strcmp(EncBlk.xmlCharEncodingAliases[i].alias, upper) == 0) {
			...
			free((char*)EncBlk.xmlCharEncodingAliases[i].name);
...
int xmlDelEncodingAlias(const char * alias)
{
	...
	for(i = 0; i < EncBlk.xmlCharEncodingAliasesNb; i++) {
		if(strcmp(EncBlk.xmlCharEncodingAliases[i].alias, alias) == 0) {
			free((char*)EncBlk.xmlCharEncodingAliases[i].name);
			free((char*)EncBlk.xmlCharEncodingAliases[i].alias);
			EncBlk.xmlCharEncodingAliasesNb--;
...
static int xmlUconvWrapper(uconv_t * cd, int toUnicode, uchar * out, int * outlen, const uchar * in, int * inlen)
{
	...
	if(toUnicode) {
		...
		ucnv_convertEx(cd->utf8, cd->uconv, &ucv_out, ucv_out + *outlen,
		    &ucv_in, ucv_in + *inlen, NULL, NULL, NULL, NULL, 0, TRUE, &err);
	}
	else {
...
int xmlCharEncFirstLineInt(xmlCharEncodingHandler * handler, xmlBufferPtr out, xmlBufferPtr in, int len)
{
	...
	switch(ret) {
		...
		case -1:
		    ...
		    break;
		case -2:
		    xmlGenericError(0, "input conversion failed due to input error\n");
		    break;
		case -3:
		    xmlGenericError(0, "converted %d bytes to %d bytes of input, %d left\n",
		    toconv, written, in->use);
		    break;
		default:
		    xmlGenericError(0, "Unknown input conversion failed %d\n", ret);
	}
...
int xmlCharEncInput(xmlParserInputBufferPtr input, int flush)
{
	...
	xmlBufPtr in;
	xmlBufPtr out;
	if((input == NULL) || (input->encoder == NULL) || (input->buffer == NULL) || (input->raw == NULL))
		return (-1);
	out = input->buffer;
	in = input->raw;
	toconv = xmlBufUse(in);
...
int xmlCharEncOutput(xmlOutputBufferPtr output, int init)
{
	...
	xmlBufPtr out;
	int charref_len = 0;
	if((output == NULL) || (output->encoder == NULL) || (output->buffer == NULL) || (output->conv == NULL))
		return (-1);
	out = output->conv;
	in = output->buffer;
retry:
 ...
 * The value of @outlen after return is the number of ocetes consumed.
 */
static int UTF8ToISO8859x(uchar* out, int * outlen,
    const uchar* in, int * inlen,
    uchar const * xlattable) {
	const uchar* outstart = out;
	const uchar* inend;
	const uchar* instart = in;
	const uchar* processed = in;
...
static int ISO8859xToUTF8(uchar* out, int * outlen, const uchar* in, int * inlen, ushort const * unicodetable)
{
	...
	const uchar* instop;
	uint c;
	if((out == NULL) || (outlen == NULL) || (inlen == NULL) || (in == NULL) || (unicodetable == NULL))
		return -1;
	outend = out + *outlen;
	inend = in + *inlen;
	instop = inend;
...
void LibXmlEncoderBlock::InitCharEncodingHandlers()
{
	if(!PP_Tab) {
		ushort tst = 0x1234;
		uchar * ptr = (uchar*)&tst;
		PP_Tab = (xmlCharEncodingHandlerPtr*)xmlMalloc(MAX_ENCODING_HANDLERS * sizeof(xmlCharEncodingHandlerPtr));
		if(*ptr == 0x12)
			xmlLittleEndian = 0;
		else if(*ptr == 0x34)
			xmlLittleEndian = 1;
...
xmlCharEncodingHandlerPtr LibXmlEncoderBlock::FindCharEncodingHandler(const char * name)
{
	...
	if(isempty(name))
		...
		if(ucv_in && ucv_out) {
			encu = (xmlCharEncodingHandlerPtr)xmlMalloc(sizeof(xmlCharEncodingHandler));
			if(encu == NULL) {
	...
	if(PP_Tab) {
		while(nbCharEncodingHandler > 0) {
			...
			if(PP_Tab[nbCharEncodingHandler]) {
				free(PP_Tab[nbCharEncodingHandler]->name);
				free(PP_Tab[nbCharEncodingHandler]);
			}
		}
		free(PP_Tab);
		PP_Tab = 0;
