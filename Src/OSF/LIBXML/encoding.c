...
int xmlCharEncFirstLineInt(xmlCharEncodingHandler * handler, xmlBufferPtr out, xmlBufferPtr in, int len)
{
	...
	switch(ret) {
		...
		case -1:
		    ...
		    break;
		case -2:
		    xmlGenericError(xmlGenericErrorContext, "input conversion failed due to input error\n");
		    break;
		case -3:
		    xmlGenericError(xmlGenericErrorContext, "converted %d bytes to %d bytes of input, %d left\n",
		    toconv, written, in->use);
...
retry:
	...
	switch(ret) {
		...
		case -1:
#ifdef DEBUG_ENCODING
		    xmlGenericError(xmlGenericErrorContext, "output conversion failed by lack of space\n");
#endif
		    break;
		case -3:
#ifdef DEBUG_ENCODING
...
xmlCharEncodingHandlerPtr LibXmlEncoderBlock::FindCharEncodingHandler(const char * name)
{
	...
	if(isempty(name))
		...
		#ifdef DEBUG_ENCODING
				xmlGenericError(xmlGenericErrorContext,"Found ICU converter handler for encoding %s\n", name);
		#endif
			}
			return encu;
		}
