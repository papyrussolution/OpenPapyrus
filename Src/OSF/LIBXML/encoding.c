...
//
static int UTF8ToUTF8(uchar * out, int * outlen, const uchar * inb, int * inlenb)
{
	int len;
	if(!out || !inb || !outlen || !inlenb)
		return -1;
	if(*outlen > *inlenb) {
		len = *inlenb;
	}
...
//     if the return value is positive, else unpredictable.
//
static int UTF16LEToUTF8(uchar * out, int * outlen, const uchar * inb, int * inlenb)
{
	uchar * outstart = out;
...
//     if the transcoding failed.
//
static int UTF8ToUTF16(uchar * outb, int * outlen, const uchar * in, int * inlen)
{
	if(!in) {
		/*
...
int xmlAddEncodingAlias(const char * name, const char * alias)
{
	...
	if(EncBlk.xmlCharEncodingAliases == NULL) {
		EncBlk.xmlCharEncodingAliasesNb = 0;
		EncBlk.xmlCharEncodingAliasesMax = 20;
		EncBlk.xmlCharEncodingAliases = (xmlCharEncodingAliasPtr)SAlloc::M(EncBlk.xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
		if(EncBlk.xmlCharEncodingAliases == NULL)
			return -1;
	}
	else if(EncBlk.xmlCharEncodingAliasesNb >= EncBlk.xmlCharEncodingAliasesMax) {
		EncBlk.xmlCharEncodingAliasesMax *= 2;
		EncBlk.xmlCharEncodingAliases = (xmlCharEncodingAliasPtr)SAlloc::R(EncBlk.xmlCharEncodingAliases, EncBlk.xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
	}
	//
	// Walk down the list looking for a definition of the alias
	//
...
#ifdef DEBUG_ENCODING
				xmlGenericError(0, "Registered encoding handler for %s\n", name);
#endif
			}
		}
	}
...
static int xmlUconvWrapper(uconv_t * cd, int toUnicode, uchar * out, int * outlen, const uchar * in, int * inlen)
{
	...
	if(toUnicode) {
		...
		ucnv_convertEx(cd->utf8, cd->uconv, &ucv_out, ucv_out + *outlen, &ucv_in, ucv_in + *inlen, NULL, NULL, NULL, NULL, 0, TRUE, &err);
	}
	else {
...
int xmlCharEncFirstLineInt(xmlCharEncodingHandler * handler, xmlBuffer * out, xmlBuffer * in, int len)
{
	 ...
	 * The actual value depending on guessed encoding is passed as @len if provided
	 */
	if(len >= 0) {
		SETMIN(toconv, len);
	}
...
#ifdef DEBUG_ENCODING
	switch(ret) {
		case 0: xmlGenericError(0, "converted %d bytes to %d bytes of input\n", toconv, written); break;
		case -1: xmlGenericError(0, "converted %d bytes to %d bytes of input, %d left\n", toconv, written, in->use); break;
		case -2: xmlGenericError(0, "input conversion failed due to input error\n"); break;
		case -3: xmlGenericError(0, "converted %d bytes to %d bytes of input, %d left\n", toconv, written, in->use); break;
		default: xmlGenericError(0, "Unknown input conversion failed %d\n", ret);
	}
#endif /* DEBUG_ENCODING */
	// Ignore when input buffer is not on a boundary
	return oneof2(ret, -1, -3) ? 0 : ret;
}
...
int xmlCharEncFirstLineInput(xmlParserInputBuffer * input, int len)
{
	...
	xmlBufPtr in;
	xmlBufPtr out;
	if(!input || (input->encoder == NULL) || (input->buffer == NULL) || (input->raw == NULL))
		return -1;
	out = input->buffer;
	in = input->raw;
	toconv = xmlBufUse(in);
	...
	switch(ret) {
		case 0:
#ifdef DEBUG_ENCODING
		    xmlGenericError(0, "converted %d bytes to %d bytes of input\n", c_in, c_out);
#endif
		    break;
		case -1:
#ifdef DEBUG_ENCODING
		    xmlGenericError(0, "converted %d bytes to %d bytes of input, %d left\n", c_in, c_out, (int)xmlBufUse(in));
#endif
		    break;
		case -3:
#ifdef DEBUG_ENCODING
		...
		case -2: {
		    ...
		    snprintf(&buf[0], 49, "0x%02X 0x%02X 0x%02X 0x%02X", content[0], content[1], content[2], content[3]);
		    buf[49] = 0;
...
int xmlCharEncInput(xmlParserInputBuffer * input, int flush)
{
	...
	xmlBufPtr in;
	xmlBufPtr out;
	if(!input || !input->encoder || !input->buffer || !input->raw)
		return -1;
	out = input->buffer;
	in = input->raw;
	toconv = xmlBufUse(in);
...
int xmlCharEncInFunc(xmlCharEncodingHandler * handler, xmlBuffer * out, xmlBuffer * in)
{
	...
	switch(ret) {
		...
		case -2: {
		    char buf[50];
		    snprintf(&buf[0], 49, "0x%02X 0x%02X 0x%02X 0x%02X", in->content[0], in->content[1], in->content[2], in->content[3]);
		    buf[49] = 0;
		    xmlEncodingErr(XML_I18N_CONV_FAILED, "input conversion failed due to input error, bytes %s\n", buf);
	    }
	}
...
int FASTCALL xmlCharEncOutput(xmlOutputBuffer * output, int init)
{
	...
	xmlBuf * out;
	int charref_len = 0;
	if(!output || !output->encoder || !output->buffer || !output->conv)
		return -1;
	out = output->conv;
	in = output->buffer;
retry:
...
#endif /* LIBXML_ICU_ENABLED */
#ifdef DEBUG_ENCODING
		xmlGenericError(0, "initialized encoder\n");
#endif
		return 0;
	}
	//
	...
	switch(ret) {
		case 0:
#ifdef DEBUG_ENCODING
		    xmlGenericError(0, "converted %d bytes to %d bytes of output\n", c_in, c_out);
#endif
		    break;
		case -1:
#ifdef DEBUG_ENCODING
		    xmlGenericError(0, "output conversion failed by lack of space\n");
#endif
		    break;
		case -3:
#ifdef DEBUG_ENCODING
		    xmlGenericError(0, "converted %d bytes to %d bytes of output %d left\n", c_in, c_out, (int)xmlBufUse(in));
#endif
		    break;
		case -2: {
...
#ifdef DEBUG_ENCODING
			    xmlGenericError(0, "handling output conversion error\n");
			    xmlGenericError(0, "Bytes: 0x%02X 0x%02X 0x%02X 0x%02X\n", in->content[0], in->content[1], in->content[2], in->content[3]);
#endif
				//
			    // Removes the UTF8 sequence, and replace it by a charref
			    // and continue the transcoding phase, hoping the error
...
{
	return EncBlk.CharEncCloseFunc(handler);
}
/**
 * xmlByteConsumed:
		...
		if((in->end - in->cur) > 0) {
			...
			if(handler->output) {
				...
				do {
					toconv = in->end - cur;
					written = 32000;
					ret = xmlIconvWrapper(handler->iconv_out, &convbuf[0], &written, cur, &toconv);
					if(ret < 0) {
						if(written > 0)
							ret = -2;
						else
				...
				do {
					toconv = in->end - cur;
					written = 32000;
					ret = xmlUconvWrapper(handler->uconv_out, 0, &convbuf[0], &written, cur, &toconv);
					if(ret < 0) {
						if(written > 0)
							ret = -2;
						else
 ...
 */
static int ISO8859xToUTF8(uchar * out, int * outlen, const uchar * in, int * inlen, ushort const * unicodetable)
{
	if(!out || !outlen || !inlen || !in || !unicodetable)
		return -1;
	else {
		const uchar * const outstart = out;
		const uchar * const instart = in;
		uchar * const outend = out + *outlen;
		const uchar * const inend = in + *inlen;
...
void LibXmlEncoderBlock::RegisterCharEncodingHandler(xmlCharEncodingHandler * handler)
{
	...
	if(!handler || !PP_Tab)
		...
		PP_Tab[nbCharEncodingHandler++] = handler;
}
//
// xmlGetCharEncodingHandler:
// @enc:  an xmlCharEncoding value.
...
#ifdef DEBUG_ENCODING
	xmlGenericError(0, "No handler found for encoding %d\n", enc);
#endif
	return 0;
}
//
...
xmlCharEncodingHandler * LibXmlEncoderBlock::FindCharEncodingHandler(const char * name)
{
	...
	if(isempty(name))
		...
		if((icv_in != (iconv_t)-1) && (icv_out != (iconv_t)-1)) {
			enc = (xmlCharEncodingHandler *)SAlloc::M(sizeof(xmlCharEncodingHandler));
			if(enc == NULL) {
				iconv_close(icv_in);
				iconv_close(icv_out);
			}
		...
		#ifdef DEBUG_ENCODING
				xmlGenericError(0, "Found iconv handler for encoding %s\n", name);
		#endif
			}
			return enc;
		}
		...
		if(ucv_in && ucv_out) {
			encu = (xmlCharEncodingHandler *)SAlloc::M(sizeof(xmlCharEncodingHandler));
			if(encu == NULL) {
		...
		#ifdef DEBUG_ENCODING
				xmlGenericError(0,"Found ICU converter handler for encoding %s\n", name);
		#endif
			}
			return encu;
		}
		else if(ucv_in || ucv_out) {
			closeIcuConverter(ucv_in);
			closeIcuConverter(ucv_out);
			xmlEncodingErr(XML_ERR_INTERNAL_ERROR, "ICU converter : problems with filters for '%s'\n", name);
		}
	#endif /* LIBXML_ICU_ENABLED */

	#ifdef DEBUG_ENCODING
		xmlGenericError(0, "No handler found for encoding %s\n", name);
	#endif
		//
...
// Returns 0 if success, or -1 in case of error
//
int LibXmlEncoderBlock::CharEncCloseFunc(xmlCharEncodingHandler * handler)
{
	int ret = 0;
	int tofree = 0;
...
#ifdef DEBUG_ENCODING
		if(ret)
			xmlGenericError(0, "failed to close the encoding handler\n");
		else
			xmlGenericError(0, "closed the encoding handler\n");
#endif
	}
	return ret;
}

void LibXmlEncoderBlock::CleanupEncodingAliases()
{
	if(xmlCharEncodingAliases) {
		...
		xmlCharEncodingAliasesNb = 0;
		xmlCharEncodingAliasesMax = 0;
		SAlloc::F(xmlCharEncodingAliases);
		xmlCharEncodingAliases = 0;
	}
}

void LibXmlEncoderBlock::CleanupCharEncodingHandlers()
{
	CleanupEncodingAliases();
	if(PP_Tab) {
		while(nbCharEncodingHandler > 0) {
			nbCharEncodingHandler--;
			if(PP_Tab[nbCharEncodingHandler]) {
				SAlloc::F(PP_Tab[nbCharEncodingHandler]->name);
				SAlloc::F(PP_Tab[nbCharEncodingHandler]);
			}
		}
		SAlloc::F(PP_Tab);
		PP_Tab = 0;
...
//
static ushort const xmlunicodetable_ISO8859_2 [128] = {
...
static uchar const xmltranscodetable_ISO8859_2 [] /* @sobolev 48 + 6 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_3 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_3 [] /* @sobolev 48 + 7 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_4 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_4 [] /* @sobolev 48 + 6 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_5 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_5 [] /* @sobolev 48 + 6 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_6 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_6 [] /* @sobolev 48 + 5 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_7 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_7 [] /* @sobolev 48 + 7 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_8 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_8 [] /* @sobolev 48 + 7 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_9 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_9 [] /* @sobolev 48 + 5 * 64*/  = {
...
static uchar const xmltranscodetable_ISO8859_14 [] /* @sobolev 48 + 10 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_15 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_15 [] /* @sobolev 48 + 6 * 64*/  = {
...
static ushort const xmlunicodetable_ISO8859_16 [128] = {
	0x0080, 0x0081, 0x0082, 0x0083, 0x0084, 0x0085, 0x0086, 0x0087,
	0x0088, 0x0089, 0x008a, 0x008b, 0x008c, 0x008d, 0x008e, 0x008f,
	0x0090, 0x0091, 0x0092, 0x0093, 0x0094, 0x0095, 0x0096, 0x0097,
	0x0098, 0x0099, 0x009a, 0x009b, 0x009c, 0x009d, 0x009e, 0x009f,
...
static uchar const xmltranscodetable_ISO8859_16 [] /* @sobolev 48 + 9 * 64*/  = {
