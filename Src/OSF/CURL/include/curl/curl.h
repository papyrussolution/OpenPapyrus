...
#define CURL_HTTPPOST_CALLBACK (1<<6)    // upload file contents by using the regular read callback to get the data
...
/*
 * The following typedef's are signatures of malloc, free, realloc, sstrdup and
...
typedef enum {
	...
	CURLE_REMOTE_ACCESS_DENIED, // 9 a service was denied by the server
		// due to lack of access - when login fails this is not returned.
	...
	CURLE_OUT_OF_MEMORY,     // 27
		// Note: CURLE_OUT_OF_MEMORY may sometimes indicate a conversion error instead
		// of a memory allocation error if CURL_DOES_CONVERSIONS is defined
	...
	CURLE_CONV_REQD,         // 76 - caller must register conversion callbacks using curl_easy_setopt options
...
#define CURLOPT_ENCODING CURLOPT_ACCEPT_ENCODING // compatibility with older names
#define CURLE_UNKNOWN_TELNET_OPTION CURLE_UNKNOWN_OPTION // The following were added in 7.21.5, April 2011
...
// STRINGPOINT is an alias for OBJECTPOINT to allow tools to extract the string options from the header file
...
typedef enum {
	...
	CINIT(USERPWD, OBJECTPOINT, 5), // "user:password;options" to use when fetching.
	CINIT(PROXYUSERPWD, OBJECTPOINT, 6), // "user:password" to use with proxy.
	CINIT(RANGE, OBJECTPOINT, 7), // Range to get, specified as an ASCII string.

	/* not used */
	CINIT(READDATA, OBJECTPOINT, 9), // Specified file stream to upload from (use as input):
...
typedef enum {
	...
	CURLSSLBACKEND_MBEDTLS = 11 // @curlv7.4.6
} curl_sslbackend;
