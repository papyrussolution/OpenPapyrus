...
void * curl_docalloc(size_t wanted_elements, size_t wanted_size, int line, const char * source)
{
	struct memdebug * mem;
...
char * curl_dostrdup(const char * str, int line, const char * source)
{
	...
	size_t len;
	assert(str != NULL);
	if(countcheck("sstrdup", line, source))
		return NULL;
	len = strlen(str)+1;
	...
	if(mem)
		memcpy(mem, str, len);
	if(source)
		curl_memlog("MEM %s:%d sstrdup(%p) (%zu) = %p\n", source, line, (void*)str, len, (void*)mem);
	return mem;
}
...
FILE * curl_fopen(const char * file, const char * mode, int line, const char * source)
{
...
#ifdef HAVE_FDOPEN
FILE * curl_fdopen(int filedes, const char * mode, int line, const char * source)
{
...
void curl_memlog(const char * format, ...)
{
	...
	va_list ap;
	if(logfile) {
		char * buf = (Curl_cmalloc)(LOGLINE_BUFSIZE);
		if(buf) {
			va_start(ap, format);
