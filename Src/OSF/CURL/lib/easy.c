...
/*
 * sstrdup (and other memory functions) is redefined in complicated
 * ways, but at this point it must be defined as the system-supplied sstrdup
...
CURLcode curl_global_init_mem(long flags, curl_malloc_callback m,
    curl_free_callback f, curl_realloc_callback r, curl_strdup_callback s, curl_calloc_callback c)
{
...
static CURLcode wait_or_timeout(struct Curl_multi * multi, struct events * ev)
{
	...
	while(!done) {
		...
		before = curlx_tvnow(); /* get the time stamp to use to figure out how long poll takes */
		pollrc = Curl_poll(fds, numfds, (int)ev->ms); /* wait for activity or timeout */
		after = curlx_tvnow();
...
static CURLcode easy_transfer(CURLM * multi)
{
	...
	while(!done && !mcode) {
		...
		if(mcode == CURLM_OK) {
			if(ret == -1) {
				result = CURLE_RECV_ERROR; // poll() failed not on EINTR, indicate a network problem 
				break;
			}
			else if(ret == 0) {
	...
	if(mcode) { // Make sure to return some kind of error if there was a multi problem 
...
static CURLcode easy_connection(struct SessionHandle * data, curl_socket_t * sfd, struct connectdata ** connp)
{
	if(data == NULL)
		return CURLE_BAD_FUNCTION_ARGUMENT;
	else if(!data->set.connect_only) { // only allow these to be called on handles with CURLOPT_CONNECT_ONLY
