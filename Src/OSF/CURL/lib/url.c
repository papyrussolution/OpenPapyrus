...
static void FASTCALL free_fixed_hostname(struct hostname * host);
...
static uint FASTCALL get_protocol_family(uint protocol);
/*
 * Protocol table.
 */
...
void Curl_freeset(struct Curl_easy * data)
{
	...
	// @soboelv enum dupstring i;
	int    i; // @sobolev
	for(i = /* @sobolev (enum dupstring)*/0; i < STRING_LAST; i++) {
...
static CURLcode FASTCALL setstropt(char ** charp, const char * s)
{
...
CURLcode Curl_dupset(struct Curl_easy * dst, struct Curl_easy * src)
{
	...
	for(i = /* @sobolev (enum dupstring)*/0; i< STRING_LASTZEROTERMINATED; i++) {
...
CURLcode Curl_close(struct Curl_easy * data)
{
	...
	if(data) {
		...
		m = data->multi;
		if(m) // This handle is still part of a multi handle, take care of this first and detach this handle from there.
		...
		// /normally/ done by curl_multi_remove_handle() but this is "just in case"
		Curl_llist_destroy(&data->state.timeoutlist, 0);
		data->magic = 0; // force a clear AFTER the possibly enforced removal from the multi handle, since that function uses the magic field!
		if(data->state.rangestringalloc)
			SAlloc::F(data->state.range);
		ZFREE(data->state.pathbuffer); // Free the pathbuffer
		data->state.path = NULL;
		// freed here just in case DONE wasn't called
		Curl_free_request_state(data);
		Curl_ssl_close_all(data); // Close down all open SSL info and sessions
...
#endif
	set->ssh_auth_types = CURLSSH_AUTH_DEFAULT; // defaults to any auth type
	set->ssl.primary.sessionid = TRUE; // session ID caching enabled by default
...
CURLcode Curl_open(struct Curl_easy ** curl)
{
	...
	data->state.headerbuff = (char *)SAlloc::M(HEADERSIZE);
...
CURLcode Curl_setopt(struct Curl_easy * data, CURLoption option, va_list param)
{
	...
	switch(option) {
		...
		case CURLOPT_MAXCONNECTS:
		    //
		    // Set the absolute number of maximum simultaneous alive connection that libcurl is allowed to have.
		    //
		...
		case CURLOPT_HEADER: // Set to include the header in the general data output stream.
		    ...
		    break;
		case CURLOPT_NOPROGRESS: // Shut off the internal supported progress meter
		    data->set.hide_progress = (0 != va_arg(param, long)) ? TRUE : FALSE;
			SETFLAG(data->progress.flags, PGRS_HIDE, data->set.hide_progress);
		    break;
		case CURLOPT_NOBODY: // Do not include the body part in the output data stream.
		    data->set.opt_no_body = (0 != va_arg(param, long)) ? TRUE : FALSE;
		    break;
		case CURLOPT_FAILONERROR: // Don't output the >=400 error code HTML-page, but instead only return error
		...
		case CURLOPT_FTP_CREATE_MISSING_DIRS:
		    ...
		    switch(va_arg(param, long)) {
			    ...
			    default: result = CURLE_UNKNOWN_OPTION; break; // reserve other values for future use
		    }
		    break;
		...
		case CURLOPT_TIMECONDITION:
			//
			// Set HTTP time condition. This must be one of the defines in the curl/curl.h header file.
			//
		...
		case CURLOPT_PROXY_SSLVERSION:
		    ...
		    break;
#ifndef CURL_DISABLE_HTTP
		case CURLOPT_AUTOREFERER: // Switch on automatic referer that gets set if curl follows locations
		...
		case CURLOPT_TRANSFER_ENCODING:
		    ...
		    break;
		case CURLOPT_FOLLOWLOCATION: // Follow Location: header hints on a HTTP-server
		    data->set.http_follow_location = (0 != va_arg(param, long)) ? TRUE : FALSE;
		    break;
		case CURLOPT_UNRESTRICTED_AUTH: // Send authentication (user+password) when following locations, even when hostname changed
		...
		case CURLOPT_USERAGENT: // String to use in the HTTP User-Agent field
		    ...
		    break;
		case CURLOPT_HTTPHEADER: // Set a list with HTTP headers to use (or replace internals with)
		...
		case CURLOPT_PROXYHEADER:
		    ...
		    break;
		case CURLOPT_HEADEROPT: // Set header option
		    arg = va_arg(param, long);
		    data->set.sep_headers = (arg & CURLHEADER_SEPARATE) ? TRUE : FALSE;
		    break;
		case CURLOPT_HTTP200ALIASES: // Set a list of aliases for HTTP 200 in response header
		...
		case CURLOPT_COOKIEJAR: // Set cookie file name to dump all cookies to when we're done
			{
				...
				//
				// Activate the cookie parser. This may or may not already have been made.
				//
		...
		case CURLOPT_COOKIESESSION:
		     ...
		     * have a Max-Age.
		     *
		     * We run mostly with the original cookie spec, as hardly anyone implements anything else.
		     */
		...
		case CURLOPT_COOKIELIST:
		    ...
		    if(argptr) {
				if(sstreqi_ascii(argptr, "ALL")) {
					...
					Curl_flush_cookies(data, 0); // flush cookies to file, takes care of the locking
				}
				else if(sstreqi_ascii(argptr, "RELOAD")) {
					Curl_cookie_loadfiles(data); // reload cookies from file
					break;
				}
				else {
					SETIFZ(data->cookies, Curl_cookie_init(data, NULL, NULL, TRUE)); // if cookie engine was not running, activate it
					argptr = _strdup(argptr);
					if(!argptr || !data->cookies) {
						...
						Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
						if(checkprefix("Set-Cookie:", argptr)) // HTTP Header format line
							Curl_cookie_add(data, data->cookies, TRUE, argptr + 11, 0, 0);
						else // Netscape format line
		...
		case CURLOPT_HTTPGET: // Set to force us do HTTP GET
		    if(va_arg(param, long)) {
			    data->set.httpreq = HTTPREQ_GET;
		...
		case CURLOPT_HTTP_VERSION:
		    //
			// This sets a requested HTTP version to be used. The value is one of the listed enums in curl/curl.h.
			//
		    arg = va_arg(param, long);
#ifndef USE_NGHTTP2
...
#endif
#ifndef USE_SPNEGO
				auth &= ~CURLAUTH_NEGOTIATE; // no Negotiate (SPNEGO) auth without GSS-API or SSPI
#endif
		...
		case CURLOPT_HTTPPROXYTUNNEL:
		    ...
		    break;
		case CURLOPT_PROXYPORT: // Explicitly set HTTP proxy port number.
		    data->set.proxyport = va_arg(param, long);
		    break;
		case CURLOPT_PROXYAUTH: // Set HTTP Authentication type BITMASK
			{
				int bitcheck;
				bool authbits;
				ulong auth = va_arg(param, ulong);
		...
		case CURLOPT_PRE_PROXY:
		    ...
		    break;
		case CURLOPT_PROXYTYPE: // Set proxy type. HTTP/HTTP_1_0/SOCKS4/SOCKS4a/SOCKS5/SOCKS5_HOSTNAME
		    data->set.proxytype = (curl_proxytype)va_arg(param, long);
		    break;
		case CURLOPT_PROXY_TRANSFER_MODE: // set transfer mode (;type=<a|i>) when doing FTP via an HTTP proxy
		    switch(va_arg(param, long)) {
		...
		case CURLOPT_PROXY_SERVICE_NAME:
		    ...
		    break;
#endif
#if !defined(CURL_DISABLE_CRYPTO_AUTH) || defined(USE_KERBEROS5) || defined(USE_SPNEGO)
		...
		case CURLOPT_WRITEDATA: // FILE pointer to write to. Or possibly used as argument to the write callback
		    ...
		    break;
		case CURLOPT_FTPPORT: // Use FTP PORT, this also specifies which IP address to use
		...
		case CURLOPT_READDATA:
		    ...
		    break;
		case CURLOPT_INFILESIZE: // If known, this should inform curl about the file size of the to-be-uploaded file
		    data->set.filesize = va_arg(param, long);
		    break;
		case CURLOPT_INFILESIZE_LARGE: // If known, this should inform curl about the file size of the to-be-uploaded file
		...
		case CURLOPT_PORT:
		    ...
		    break;
		case CURLOPT_TIMEOUT: // The maximum time you allow curl to use for a single transfer operation
		    data->set.timeout = va_arg(param, long) * 1000L;
		    break;
		case CURLOPT_TIMEOUT_MS:
		    data->set.timeout = va_arg(param, long);
		    break;
		case CURLOPT_CONNECTTIMEOUT: // The maximum time you allow curl to use to connect
		    data->set.connecttimeout = va_arg(param, long) * 1000L;
		    break;
		case CURLOPT_CONNECTTIMEOUT_MS:
		    data->set.connecttimeout = va_arg(param, long);
		    break;
		case CURLOPT_ACCEPTTIMEOUT_MS: // The maximum time you allow curl to wait for server connect
		    data->set.accepttimeout = va_arg(param, long);
		    break;
		case CURLOPT_USERPWD: // user:password to use in the operation
		    result = setstropt_userpwd(va_arg(param, char *), &data->set.str[STRING_USERNAME], &data->set.str[STRING_PASSWORD]);
		    break;
		case CURLOPT_USERNAME: // authentication user name to use in the operation
		    result = setstropt(&data->set.str[STRING_USERNAME], va_arg(param, char *));
		    break;
		case CURLOPT_PASSWORD: // authentication password to use in the operation
		    result = setstropt(&data->set.str[STRING_PASSWORD], va_arg(param, char *));
		    break;
		case CURLOPT_LOGIN_OPTIONS: // authentication options to use in the operation
		...
		case CURLOPT_PROXY_CAINFO:
		    //
		    // Set CA info SSL connection for proxy. Specify file name of the CA certificate
		    //
		...
		case CURLOPT_PROXY_CRLFILE:
		    ...
		    break;
		case CURLOPT_ISSUERCERT: // Set Issuer certificate file to check certificates issuer
		    result = setstropt(&data->set.str[STRING_SSL_ISSUERCERT_ORIG], va_arg(param, char *));
		    break;
		case CURLOPT_TELNETOPTIONS: // Set a linked list of telnet options
		...
		case CURLOPT_BUFFERSIZE:
		    ...
		    if(data->set.buffer_size > BUFSIZE) {
			    data->state.buffer = (char *)SAlloc::R(data->state.buffer, data->set.buffer_size + 1);
			    if(!data->state.buffer) {
		...
		case CURLOPT_SHARE:
	    {
		    ...
		    data->share = set; // use new share if it set
		    if(data->share) {
			    Curl_share_lock(data, CURL_LOCK_DATA_SHARE, CURL_LOCK_ACCESS_SINGLE);
			    data->share->dirty++;
			    ...
			    if(data->share->cookies) {
				    Curl_cookie_cleanup(data->cookies); // use shared cookie list, first free own one if any
				    data->cookies = data->share->cookies; // enable cookies since we now use a share that uses cookies!
			    }
#endif   /* CURL_DISABLE_HTTP */
		    ...
		    // check for host cache not needed, it will be done by curl_easy_perform
	    }
	    break;
		case CURLOPT_PRIVATE: // Set private data pointer
		    data->set.private_data = va_arg(param, void *);
		    break;
		case CURLOPT_MAXFILESIZE: // Set the maximum size of a file to download
		    data->set.max_filesize = va_arg(param, long);
		    break;
#ifdef USE_SSL
		case CURLOPT_USE_SSL: // Make transfers attempt to use SSL/TLS
		...
		case CURLOPT_PROXY_SSL_OPTIONS:
		    ...
		    break;
#endif
		case CURLOPT_FTPSSLAUTH: // Set a specific auth for FTP-SSL transfers
		    data->set.ftpsslauth = (curl_ftpauth)va_arg(param, long);
		    break;
		case CURLOPT_IPRESOLVE:
		    data->set.ipver = va_arg(param, long);
		    break;
		case CURLOPT_MAXFILESIZE_LARGE: // Set the maximum size of a file to download
		    data->set.max_filesize = va_arg(param, curl_off_t);
		    break;
		case CURLOPT_TCP_NODELAY: // Enable or disable TCP_NODELAY, which will disable/enable the Nagle algorithm
		...
		case CURLOPT_IGNORE_CONTENT_LENGTH:
		    ...
		    break;
		case CURLOPT_CONNECT_ONLY: // No data transfer, set up connection and let application use the socket
		    data->set.connect_only = (0 != va_arg(param, long)) ? TRUE : FALSE;
		    break;
		case CURLOPT_FTP_ALTERNATIVE_TO_USER:
		    result = setstropt(&data->set.str[STRING_FTP_ALTERNATIVE_TO_USER], va_arg(param, char *));
		    break;
		case CURLOPT_SOCKOPTFUNCTION: // socket callback function: called after socket() but before connect()
		    data->set.fsockopt = va_arg(param, curl_sockopt_callback);
		    break;
		case CURLOPT_SOCKOPTDATA: // socket callback data pointer. Might be NULL
		    data->set.sockopt_client = va_arg(param, void *);
		    break;
		case CURLOPT_OPENSOCKETFUNCTION: // open/create socket callback function: called instead of socket(), before connect()
		    data->set.fopensocket = va_arg(param, curl_opensocket_callback);
		    break;
		case CURLOPT_OPENSOCKETDATA: // socket callback data pointer. Might be NULL
		...
		case CURLOPT_ADDRESS_SCOPE:
		    ...
		    // know that an uint will always hold the value so we blindly
		...
		case CURLOPT_RTSP_STREAM_URI:
		    ...
		    break;
		case CURLOPT_RTSP_TRANSPORT: // The content of the Transport: header for the RTSP request
		...
		case CURLOPT_RTSP_CLIENT_CSEQ:
		    ...
		    break;
		case CURLOPT_RTSP_SERVER_CSEQ: // Same as the above, but for server-initiated requests
		    data->state.rtsp_next_client_CSeq = va_arg(param, long);
		    break;
		case CURLOPT_INTERLEAVEDATA:
		    data->set.rtp_out = va_arg(param, void *);
		    break;
		case CURLOPT_INTERLEAVEFUNCTION: // Set the user defined RTP write function
...
static void conn_free(struct connectdata * conn)
{
	if(conn) {
		...
		ZFREE(conn->allocptr.te);
		ZFREE(conn->allocptr.rangeline);
		ZFREE(conn->allocptr.ref);
		ZFREE(conn->allocptr.host);
		ZFREE(conn->allocptr.cookiehost);
 ...
 *
 */
CURLcode FASTCALL Curl_disconnect(struct connectdata * conn, bool dead_connection)
{
	...
	Curl_http_ntlm_cleanup(conn); // Cleanup NTLM connection-related data
#endif
	if(conn->handler->disconnect)
		conn->handler->disconnect(conn, dead_connection); // This is set if protocol-specific cleanups should be made
	// unlink ourselves!
 ...
 */
static bool FASTCALL SocketIsDead(curl_socket_t sock)
{
	bool ret_val = TRUE;
	int sval = SOCKET_READABLE(sock, 0);
	if(sval == 0)
...
//
static bool FASTCALL IsPipeliningPossible(const struct Curl_easy * handle, const struct connectdata * conn)
{
...
{
	struct curl_llist_element * curr = pipeline->head;
	return curr ? (struct Curl_easy*)curr->ptr : 0;
}
...
static bool disconnect_if_dead(struct connectdata * conn, struct Curl_easy * data)
{
	...
	if(!pipeLen && !conn->inuse) {
		...
		// handles in pipeline and the connection isn't already marked in use
		bool dead;
		if(conn->handler->protocol & CURLPROTO_RTSP)
			dead = Curl_rtsp_connisdead(conn); // RTSP is a special case due to RTP interleaving
		else
 ...
 */
static bool ConnectionExists(struct Curl_easy * data, struct connectdata * needle, struct connectdata ** usethis,
    bool * force_reuse, bool * waitpipe)
{
	...
	*waitpipe = FALSE;
	...
	if(bundle) {
		...
		while(curr) {
			 ...
			 */
			check = (struct connectdata *)curr->ptr;
			curr = curr->next;
			...
			if(!needle->bits.httpproxy || (needle->handler->flags&PROTOPT_SSL) || needle->bits.tunnel_proxy) {
				...
				if((strcasecompare(needle->handler->scheme, check->handler->scheme) ||
				    ...
				    strcasecompare(needle->host.name, check->host.name) && needle->remote_port == check->remote_port) {
...
{
	return conn->handler->proto_getsock ? conn->handler->proto_getsock(conn, socks, numsocks) : GETSOCK_BLANK;
}

int Curl_doing_getsock(struct connectdata * conn, curl_socket_t * socks, int numsocks)
{
	return (conn && conn->handler->doing_getsock) ? conn->handler->doing_getsock(conn, socks, numsocks) : GETSOCK_BLANK;
}
 ...
 */
static void FASTCALL fix_hostname(struct connectdata * conn, struct hostname * host)
{
	size_t len;
	...
	// @sobolev if(!is_ASCII_name(host->name)) {
	if(!sisascii(host->name, len)) { // @sobolev
#ifdef USE_LIBIDN2
...
#elif defined(USE_WIN32_IDN)
	SAlloc::F(host->encalloc); // must be freed withidn_free() since this was allocated by curl_win32_idn_to_ascii
	host->encalloc = NULL;
#else
	(void)host;
#endif
...
{
	struct connectdata * conn = (struct connectdata *)SAlloc::C(1, sizeof(struct connectdata));
	if(!conn)
		return NULL;
	...
	conn->data = data; /* Setup the association between this connection and the Curl_easy */
	...
	conn->bits.proxy = BIN(data->set.str[STRING_PROXY] && *data->set.str[STRING_PROXY]);
	conn->bits.httpproxy = BIN(conn->bits.proxy && oneof3(conn->http_proxy.proxytype, CURLPROXY_HTTP, CURLPROXY_HTTP_1_0, CURLPROXY_HTTPS));
	conn->bits.socksproxy = BIN(conn->bits.proxy && !conn->bits.httpproxy);
	...
	if(Curl_pipeline_wanted(data->multi, CURLPIPE_HTTP1) && !conn->master_buffer) {
		...
		conn->master_buffer = (char *)SAlloc::C(BUFSIZE, sizeof(char));
	...
	if(data->set.str[STRING_DEVICE]) {
		conn->localdev = _strdup(data->set.str[STRING_DEVICE]);
		if(!conn->localdev)
			goto error;
	}
...
static CURLcode parseurlandfillconn(struct Curl_easy * data, struct connectdata * conn, bool * prot_missing, char ** userp, char ** passwdp, char ** optionsp)
{
	...
	*prot_missing = FALSE;
	...
	if((('a' <= data->change.url[0] && data->change.url[0] <= 'z') || ('A' <= data->change.url[0] && data->change.url[0] <= 'Z')) &&
	    data->change.url[1] == ':' && data->set.str[STRING_DEFAULT_PROTOCOL] && sstreqi_ascii(data->set.str[STRING_DEFAULT_PROTOCOL], "file")) {
		; /* do nothing */
	}
	...
	if((url_has_scheme && strncasecompare(data->change.url, "file:", 5)) || (!url_has_scheme && data->set.str[STRING_DEFAULT_PROTOCOL] && 
		sstreqi_ascii(data->set.str[STRING_DEFAULT_PROTOCOL], "file"))) {
		...
		if(path[0] != '/' && !path_has_drive) {
			// the URL includes a host name, it must match "localhost" or "127.0.0.1" to be valid
			char * ptr;
		...
		if(3 > rc) {
			...
			if(!protop) {
				// Note: if you add a new protocol, please update the list in lib/version.c too!
	...
	if(query) {
		...
		// move the existing path plus the zero byte forward, to make room for the host-name part 
	...
	if(path[0] != '/') {
		...
		if(!sstreq(newp, path)) {
			rebuild_url = TRUE;
			SAlloc::F(data->state.pathbuffer);
	...
	if(rebuild_url) {
		...
		if(!reurl)
			return CURLE_OUT_OF_MEMORY;
		memcpy(reurl, data->change.url, prefixlen); // copy the prefix
		memcpy(&reurl[prefixlen], path, plen + 1); // append the trailing piece + zerobyte
	...
	if(result)
		return result;
	// 
	// Parse the login details from the URL and strip them out of the host name
	// 
...
static CURLcode setup_range(struct Curl_easy * data)
{
	...
	if(s->resume_from || data->set.str[STRING_SET_RANGE]) {
		if(s->rangestringalloc)
			SAlloc::F(s->range);
		s->range = s->resume_from ? aprintf("%" CURL_FORMAT_CURL_OFF_TU "-", s->resume_from) : _strdup(data->set.str[STRING_SET_RANGE]);
...
static CURLcode setup_connection_internals(struct connectdata * conn)
{
	...
	if(conn->port < 0) // we check for -1 here since if proxy was detected already, this was very likely already set to the proxy port 
		conn->port = p->defport;
	return CURLE_OK;
}
...
static char * detect_proxy(struct connectdata * conn)
{
	...
	if(prox)
		...
		SETIFZ(proxy, curl_getenv("ALL_PROXY"));
	}
	return proxy;
}
...
static CURLcode parse_proxy(struct Curl_easy * data, struct connectdata * conn, char * proxy, curl_proxytype proxytype)
{
	...
	if(*proxyptr == '[') {
		...
		if(*ptr == ']')
			*ptr++ = 0; // yeps, it ended nicely with a bracket as well 
		else
			infof(data, "Invalid IPv6 address format\n");
		portptr = ptr;
	...
	if(prox_portno) {
		...
		*prox_portno = 0x0; /* cut off number from host name */
		...
		else if(proxytype == CURLPROXY_HTTPS)
	...
	if(*proxyptr) {
		...
		proxyinfo->host.rawalloc = _strdup(proxyptr);
...
static CURLcode create_conn_helper_init_proxy(struct connectdata * conn)
{
	...
	*************************************************************/
	if(data->set.str[STRING_PROXY]) {
		proxy = _strdup(data->set.str[STRING_PROXY]);
	...
	if(data->set.str[STRING_PRE_PROXY]) {
		socksproxy = _strdup(data->set.str[STRING_PRE_PROXY]);
	...
	SETIFZ(no_proxy, curl_getenv("NO_PROXY"));
	...
	if(proxy && (!*proxy || (conn->handler->flags & PROTOPT_NONETWORK))) {
		SAlloc::F(proxy); // Don't bother with an empty proxy string or if the protocol doesn't work with network
		proxy = NULL;
	}
	if(socksproxy && (!*socksproxy || (conn->handler->flags & PROTOPT_NONETWORK))) {
		SAlloc::F(socksproxy); // Don't bother with an empty socks proxy string or if the protocol doesn't work with network
		socksproxy = NULL;
	}
	/***********************************************************************
...
static CURLcode parse_remote_port(struct Curl_easy * data, struct connectdata * conn)
{
	...
	if((1 == sscanf(conn->host.name, "[%*45[0123456789abcdefABCDEF:.]%c", &endbracket)) && (']' == endbracket)) {
		...
		if(Curl_inet_pton(AF_INET6, conn->host.name, &in6) > 0) {
			// This is a numerical IPv6 address, meaning this is a wrongly formatted URL
	...
	if(data->set.use_port && data->state.allow_port) {
		...
		conn->remote_port = (ushort)data->set.use_port;
		if(portptr)
			*portptr = '\0'; // cut off the name there anyway - if there was a port number - since the port number is to be ignored!
		if(conn->bits.httpproxy) {
			 ...
			 */
			url = aprintf("%s://%s%s%s:%hu%s%s%s", conn->given->scheme, conn->bits.ipv6_ip ? "[" : "", conn->host.name,
			    conn->bits.ipv6_ip ? "]" : "", conn->remote_port, data->state.slash_removed ? "/" : "", data->state.path, type);
			if(!url)
				return CURLE_OUT_OF_MEMORY;
			if(data->change.url_alloc) {
				ZFREE(data->change.url);
		...
		char * rest;
		long port = strtol(portptr+1, &rest, 10); /* Port number must be decimal */
	...
	// only if remote_port was not already parsed off the URL we use the default port number 
	if(conn->remote_port < 0)
		conn->remote_port = (ushort)conn->given->defport;
	return CURLE_OK;
}
/*
...
static CURLcode override_login(struct Curl_easy * data, struct connectdata * conn, char ** userp, char ** passwdp, char ** optionsp)
{
	if(data->set.str[STRING_USERNAME]) {
		SAlloc::F(*userp);
		*userp = _strdup(data->set.str[STRING_USERNAME]);
		if(!*userp)
			return CURLE_OUT_OF_MEMORY;
	}
	if(data->set.str[STRING_PASSWORD]) {
		SAlloc::F(*passwdp);
		*passwdp = _strdup(data->set.str[STRING_PASSWORD]);
		if(!*passwdp)
			return CURLE_OUT_OF_MEMORY;
	}
	if(data->set.str[STRING_OPTIONS]) {
		SAlloc::F(*optionsp);
		*optionsp = _strdup(data->set.str[STRING_OPTIONS]);
		if(!*optionsp)
			return CURLE_OUT_OF_MEMORY;
	}
...
static CURLcode set_login(struct connectdata * conn, const char * user, const char * passwd, const char * options)
{
	...
	if((conn->handler->flags & PROTOPT_NEEDSPWD) && !conn->bits.user_passwd) {
		conn->user = _strdup(CURL_DEFAULT_USER); // Store the default user
		conn->passwd = conn->user ? _strdup(CURL_DEFAULT_PASSWORD) : NULL; // Store the default password
		// This is the default password, so DON'T set conn->bits.user_passwd
	}
	else {
		conn->user = _strdup(user); // Store the user, zero-length if not set
		conn->passwd = conn->user ? _strdup(passwd) : NULL; // Store the password (only if user is present), zero-length if not set
	}
	if(!conn->user || !conn->passwd)
		result = CURLE_OUT_OF_MEMORY;
	// Store the options, null if not set 
	if(!result && options[0]) {
		conn->options = _strdup(options);
		if(!conn->options)
			result = CURLE_OUT_OF_MEMORY;
	}
	return result;
...
static CURLcode parse_connect_to_host_port(struct Curl_easy * data, const char * host, char ** hostname_result, int * port_result)
{
	...
	*port_result = -1;
	...
	if(hostptr) {
		*hostname_result = _strdup(hostptr);
		if(!*hostname_result) {
			SAlloc::F(host_dup);
			return CURLE_OUT_OF_MEMORY;
		}
...
static CURLcode resolve_server(struct Curl_easy * data, struct connectdata * conn, bool * async)
{
	...
	*************************************************************/
	if(conn->bits.reuse)
		...
		// fix_hostname() was called already in create_conn() for the re-use case.
		*async = FALSE;
	else {
		...
		if(conn->unix_domain_socket) {
			...
			hostaddr = SAlloc::C(1, sizeof(struct Curl_dns_entry));
			if(!hostaddr)
				result = CURLE_OUT_OF_MEMORY;
			else {
				bool longpath = FALSE;
...
#endif
		if(!conn->bits.proxy) {
			struct hostname * connhost = (conn->bits.conn_to_host) ? &conn->conn_to_host : &conn->host;
...
static void reuse_conn(struct connectdata * old_conn, struct connectdata * conn)
{
	...
	SAlloc::F(old_conn->http_proxy.host.rawalloc);
	SAlloc::F(old_conn->socks_proxy.host.rawalloc);
	...
	free_fixed_hostname(&conn->host);
	free_fixed_hostname(&conn->conn_to_host);
	ZFREE(conn->host.rawalloc);
	ZFREE(conn->conn_to_host.rawalloc);
	...
	ZFREE(old_conn->user);
	ZFREE(old_conn->passwd);
	ZFREE(old_conn->http_proxy.user);
	ZFREE(old_conn->socks_proxy.user);
	ZFREE(old_conn->http_proxy.passwd);
...
#ifdef USE_UNIX_SOCKETS
	ZFREE(old_conn->unix_domain_socket);
#endif
}
/**
...
static CURLcode create_conn(struct Curl_easy * data, struct connectdata ** in_connect, bool * async)
{
	...
	*************************************************************/
	...
	// parent can cleanup any possible allocs we may have done before any failure 
	*in_connect = conn;
	// This initing continues below, see the comment "Continue connectdata initialization here" 
	// 
...
#define LEAST_PATH_ALLOC 256
	...
	SETMAX(urllen, LEAST_PATH_ALLOC);
	// 
	// We SAlloc::M() the buffers below urllen+2 to make room for 2 possibilities:
	...
	if(prot_missing) {
		...
		char * ch_lower;
		char * reurl = aprintf("%s://%s", conn->handler->scheme, data->change.url);
		if(!reurl) {
			result = CURLE_OUT_OF_MEMORY;
			goto out;
		}
	...
	if((conn->given->flags&PROTOPT_NOURLQUERY)) {
		...
		if(path_q_sep) {
			...
			// 
			path_q_sep[0] = 0; // terminate the string 
		}
	}
	if(data->set.str[STRING_BEARER]) {
		conn->oauth_bearer = _strdup(data->set.str[STRING_BEARER]);
	...
	if(data->set.str[STRING_UNIX_SOCKET_PATH]) {
		conn->unix_domain_socket = _strdup(data->set.str[STRING_UNIX_SOCKET_PATH]);
	...
	// re-use an already existing one or if we have to create a new one.
	// 
	//
	// reuse_fresh is TRUE if we are told to use a new connection by force, but
	// we only acknowledge this option if this is not a re-used connection
	// already (which happens due to follow-location or during a HTTP authentication phase). 
	reuse = (data->set.reuse_fresh && !data->state.this_is_a_follow) ? FALSE : ConnectionExists(data, conn, &conn_temp, &force_reuse, &waitpipe);
	...
	if(reuse) {
		...
		// just allocated before we can move along and use the previously existing one.
		// 
		conn_temp->inuse = TRUE; // mark this as being in use so that no other handle in a multi stack may nick it
		reuse_conn(conn, conn_temp);
		SAlloc::F(conn); /* we don't need this anymore */
		conn = conn_temp;
		*in_connect = conn;
	...
	conn->inuse = TRUE; // Mark the connection as used 
	Curl_init_do(data, conn); // Setup and init stuff before DO starts, in preparing for the transfer. 
	/*
...
CURLcode Curl_setup_conn(struct connectdata * conn, bool * protocol_done)
{
	...
	*protocol_done = FALSE; /* default to not done */
	...
	conn->now = Curl_tvnow(); // time this *after* the connect is done, we set this here perhaps a second time
#ifdef __EMX__
	/*
...
// nothing in here depends on stuff that are setup dynamically for the transfer.
// 
...
// 
// Descr: This is used to return the protocol family for a given protocol.
// Parameters:
//   protocol  [in]  - A single bit protocol identifier such as HTTP or HTTPS.
// Returns the family as a single bit protocol identifier.
// 
uint FASTCALL get_protocol_family(uint protocol)
{
	uint family;
	switch(protocol) {
		case CURLPROTO_HTTP:
		case CURLPROTO_HTTPS: family = CURLPROTO_HTTP; break;
		case CURLPROTO_FTP:
		case CURLPROTO_FTPS: family = CURLPROTO_FTP; break;
		...
		case CURLPROTO_POP3:
		case CURLPROTO_POP3S: family = CURLPROTO_POP3; break;
		case CURLPROTO_SMTP:
		case CURLPROTO_SMTPS: family = CURLPROTO_SMTP; break;
		case CURLPROTO_RTSP:  family = CURLPROTO_RTSP; break;
		case CURLPROTO_RTMP:
		case CURLPROTO_RTMPS: family = CURLPROTO_RTMP; break;
		case CURLPROTO_RTMPT:
		case CURLPROTO_RTMPTS: family = CURLPROTO_RTMPT; break;
