...
static
CURLcode telrcv(struct connectdata *, const uchar * inbuf, /* Data received from socket */ ssize_t count /* Number of bytes received */); 
...
typedef BOOL (WSAAPI * Fn_CloseEvent)(WSAEVENT hEvent);
typedef WSAEVENT (WSAAPI * Fn_CreateEvent)();
typedef int (WSAAPI * Fn_EventSelect)(SOCKET s, WSAEVENT hEventObject, long lNetworkEvents);
typedef int (WSAAPI * Fn_EnumNetevents)(SOCKET s, WSAEVENT hEventObject, LPWSANETWORKEVENTS lpNetworkEvents);
...
static CURLcode telnet_do(struct connectdata * conn, bool * done)
{
...
#ifdef USE_WINSOCK
	HMODULE wsock2;
	Fn_CloseEvent close_event_func;
	Fn_CreateEvent create_event_func;
	Fn_EventSelect event_select_func;
	Fn_EnumNetevents enum_netevents_func;
	...
	*/
	...
	create_event_func = (Fn_CreateEvent)GetProcAddress(wsock2, "WSACreateEvent");
	...
	/* And WSACloseEvent */
	close_event_func = (Fn_CloseEvent)GetProcAddress(wsock2, "WSACloseEvent");
	...
	/* And WSAEventSelect */
	event_select_func = (Fn_EventSelect)GetProcAddress(wsock2, "WSAEventSelect");
	...
	enum_netevents_func = (Fn_EnumNetevents)GetProcAddress(wsock2, "WSAEnumNetworkEvents");
			...
			case WAIT_TIMEOUT:
		    {
			    for(;; ) {
				    if(data->set.is_fread_set) {
					    ...
					    result = (CURLcode)data->set.fread_func(buf, 1, BUFSIZE - 1, data->set.in);
