...
struct Curl_multi {
	...
	long type;
	struct Curl_easy * easyp; // We have a doubly-linked circular list with easy handles 
	struct Curl_easy * easylp; /* last node */
	int num_easy; // amount of entries in the linked list above. 
	int num_alive; // amount of easy handles that are added but have not yet reached COMPLETE state 
	struct curl_llist msglist; // a list of messages from completed transfers 
	struct curl_llist pending; // Curl_easys that are in the CURLM_STATE_CONNECT_PEND state 
	curl_socket_callback socket_cb; // callback function and user data pointer for the *socket() API 
	void * socket_userp;
	curl_push_callback push_cb; // callback function and user data pointer for server push 
	void * push_userp;
	struct curl_hash hostcache; // Hostname cache 
	struct Curl_tree * timetree; // timetree points to the splay-tree of time nodes to figure out expire times of all currently set timers 
	...
	long maxconnects; // if >0, a fixed limit of the maximum number of entries we're allowed to grow the connection cache to 
	...
	long max_pipeline_length; /* if >0, maximum number of requests in a pipeline */
	long content_length_penalty_size; /* a connection with a content-length bigger than this is not considered for pipelining */
	long chunk_length_penalty_size; /* a connection with a chunk length bigger than this is not considered for pipelining */
	struct curl_llist pipelining_site_bl; /* List of sites that are blacklisted from pipelining */
	struct curl_llist pipelining_server_bl; /* List of server types that are blacklisted from pipelining */
	curl_multi_timer_callback timer_cb; // timer callback and user data pointer for the *socket() API 
	void * timer_userp;
	struct timeval timer_lastcall; // the fixed time for the timeout for the previous callback 
};
