...
uint Curl_sasl_decode_mech(const char * ptr, size_t maxlen, size_t * len)
{
	for(uint i = 0; mechtable[i].name; i++) {
		if(maxlen >= mechtable[i].len && !memcmp(ptr, mechtable[i].name, mechtable[i].len)) {
			ASSIGN_PTR(len, mechtable[i].len);
			if(maxlen == mechtable[i].len)
				return mechtable[i].bit;
			else {
				char c = ptr[mechtable[i].len];
...
CURLcode Curl_sasl_parse_url_auth_option(struct SASL * sasl, const char * value, size_t len)
{
	...
	if(!len)
		...
		if(!strncmp(value, "*", len))
			...
			size_t mechlen;
			uint mechbit = Curl_sasl_decode_mech(value, len, &mechlen);
...
bool Curl_sasl_can_authenticate(struct SASL * sasl, struct connectdata * conn)
{
	...
	return (sasl->authmechs & sasl->prefmech & SASL_MECH_EXTERNAL) ? TRUE : FALSE;
}
	...
	*progress = SASL_IDLE;
	...
	if((enabledmechs & SASL_MECH_EXTERNAL) && !conn->passwd[0]) {
		...
		if((enabledmechs & SASL_MECH_NTLM) && Curl_auth_is_ntlm_supported()) {
			...
			if(force_ir || data->set.sasl_ir)
				result = Curl_auth_create_ntlm_type1_message(conn->user, conn->passwd, &conn->ntlm, &resp, &len);
		}
		else
#endif
...
#if defined(USE_KERBEROS5)
		case SASL_GSSAPI:
		    result = Curl_auth_create_gssapi_user_message(data, conn->user, conn->passwd,
				service, data->easy_conn->host.name, sasl->mutual_auth, NULL, &conn->krb5, &resp, &len);
		    newstate = SASL_GSSAPI_TOKEN;
		    break;
		case SASL_GSSAPI_TOKEN:
		    sasl->params->getmessage(data->state.buffer, &serverdata);
		    if(sasl->mutual_auth) {
			    // Decode the user token challenge and create the optional response message 
			    result = Curl_auth_create_gssapi_user_message(data, NULL, NULL, NULL, NULL,
					sasl->mutual_auth, serverdata, &conn->krb5, &resp, &len);
			    newstate = SASL_GSSAPI_NO_DATA;
		    }
		    else // Decode the security challenge and create the response message 
