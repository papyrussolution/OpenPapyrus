...
int FASTCALL Curl_poll(struct pollfd ufds[], uint nfds, int timeout_ms)
{
...
#ifdef USE_WINSOCK
	// WinSock select() can't handle zero events.  See the comment about this in Curl_check_socket(). 
	if(fds_read.fd_count == 0 && fds_write.fd_count == 0 && fds_err.fd_count == 0) {
		r = Curl_wait_ms(timeout_ms);
		return r;
	}
#endif
	...
	do {
	    ...
	    // don't give it such arguments.  See the comment about this in Curl_check_socket().
		r = select((int)maxfd + 1, (fds_read.fd_count ? &fds_read : 0), (fds_write.fd_count ? &fds_write : 0), (fds_err.fd_count ? &fds_err : 0), ptimeout);
#else
		r = select((int)maxfd + 1, &fds_read, &fds_write, &fds_err, ptimeout);
#endif
		if(r != -1)
...
{
	int rc = tpf_select_bsd(maxfds, reads, writes, excepts, tv);
	tpf_process_signals();
	return rc;
}
#endif /* TPF */
