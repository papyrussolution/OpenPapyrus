...
static CURLcode tftp_set_timeouts(tftp_state_data_t * state)
{
	...
	if(start) {
		...
		if(state->retry_max < 1)
			state->retry_max = 1; // avoid division by zero below 
		// Compute the re-start interval to suit the timeout 
		state->retry_time = (int)timeout/state->retry_max;
		SETMAX(state->retry_time, 1);
	}
	else {
		if(timeout_ms > 0)
			maxtime = (time_t)(timeout_ms + 500) / 1000;
			...
			maxtime = 3600;
		state->max_time = state->start_time+maxtime;
		timeout = maxtime; // Set per-block timeout to total 
		state->retry_max = (int)timeout/5; // Average reposting an ACK after 5 seconds 
	}
	// But bound the total number 
	SETMAX(state->retry_max, 3);
	SETMIN(state->retry_max, 50);
	// Compute the re-ACK interval to suit the timeout 
	state->retry_time = (int)(timeout/state->retry_max);
	SETMAX(state->retry_time, 1);
	infof(state->conn->data, "set timeouts for state %d; Total %ld, retry %d maxtry %d\n",
	    (int)state->state, (long)(state->max_time-state->start_time), state->retry_time, state->retry_max);
	/* init RX time */
	time(&state->rx_time);
	return CURLE_OK;
}
...
{
	size_t loc = Curl_strnlen(buf, len);
	loc++; /* NULL term */
	if(loc >= len)
		return NULL;
	*option = buf;
    ...
    const char * ptr, int len)
{
	...
	while(tmp < ptr + len) {
		...
		if(checkprefix(option, TFTP_OPTION_BLKSIZE)) {
			...
			infof(data, "%s (%d) %s (%d)\n", "blksize parsed from OACK", state->blksize, "requested", state->requested_blksize);
		}
		else if(checkprefix(option, TFTP_OPTION_TSIZE)) {
			long tsize = strtol(value, NULL, 10);
...
static CURLcode tftp_connect_for_tx(tftp_state_data_t * state, tftp_event_t event)
{
	...
	return result ? result : tftp_tx(state, event);
}
...
static CURLcode tftp_connect_for_rx(tftp_state_data_t * state, tftp_event_t event)
{
	...
	result = tftp_set_timeouts(state);
	return result ? result : tftp_rx(state, event);
}
...
// the next blocknum is x + 1 but it needs to wrap at an unsigned 16bit boundary 
	...
	switch(event) {
		case TFTP_EVENT_DATA:
		    ...
		    if(NEXT_BLOCKNUM(state->block) == rblock) {
			    ...
			    infof(data, "Received unexpected DATA packet block %d, expecting block %d\n", rblock, NEXT_BLOCKNUM(state->block));
			    break;
		    }
		...
		case TFTP_EVENT_ERROR:
		    ...
		    setpacketblock(&state->spacket, state->block);
		    (void)sendto(state->sockfd, (const char *)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
		...
		default:
		    ...
		    return CURLE_TFTP_ILLEGAL; // not really the perfect return code for this 
	}
	return CURLE_OK;
}
		    ...
		    if(event == TFTP_EVENT_ACK) {
			    ...
			    !(state->block == 0 && rblock == 65535)) {
				    ...
				    if(state->retries>state->retry_max) {
					    ...
					    sbytes = sendto(state->sockfd, (const char *)state->spacket.data, 4+state->sbytes, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr, state->remote_addrlen);
			    ...
			    // This is the expected packet.  Reset the counters and send the next block 
			    time(&state->rx_time);
			    state->block++;
		    }
		    else
...
static CURLcode tftp_translate_code(tftp_error_t error)
{
	...
	if(error != TFTP_ERR_NONE) {
		switch(error) {
			case TFTP_ERR_NOTFOUND: result = CURLE_TFTP_NOTFOUND; break;
			case TFTP_ERR_PERM: result = CURLE_TFTP_PERM; break;
			case TFTP_ERR_DISKFULL: result = CURLE_REMOTE_DISK_FULL; break;
			case TFTP_ERR_UNDEF: 
			case TFTP_ERR_ILLEGAL: result = CURLE_TFTP_ILLEGAL; break;
			case TFTP_ERR_UNKNOWNID: result = CURLE_TFTP_UNKNOWNID; break;
			case TFTP_ERR_EXISTS: result = CURLE_REMOTE_FILE_EXISTS; break;
			case TFTP_ERR_NOSUCHUSER: result = CURLE_TFTP_NOSUCHUSER; break;
			case TFTP_ERR_TIMEOUT: result = CURLE_OPERATION_TIMEDOUT; break;
			case TFTP_ERR_NORESPONSE: result = CURLE_COULDNT_CONNECT; break;
			default: result = CURLE_ABORTED_BY_CALLBACK; break;
		}
	}
	else
		result = CURLE_OK;
...
static CURLcode tftp_connect(struct connectdata * conn, bool * done)
{
	...
	// we don't keep TFTP connections up basically because there's none or very little gain for UDP 
	connclose(conn, "TFTP");
	state->conn = conn;
	...
	((struct sockaddr*)&state->local_addr)->sa_family = (ushort)(conn->ip_addr->ai_family);
	...
	if(!conn->bits.bound) {
		 ...
		 * IPv4 and IPv6...
		 */
		rc = bind(state->sockfd, (struct sockaddr*)&state->local_addr, conn->ip_addr->ai_addrlen);
		if(rc) {
			failf(conn->data, "bind() failed; %s", Curl_strerror(conn, SOCKERRNO));
...
static CURLcode tftp_receive_packet(struct connectdata * conn)
{
	...
	struct SingleRequest  * k = &data->req;
	/* Receive the packet */
	curl_socklen_t fromlen = sizeof(fromaddr);
	state->rbytes = (int)recvfrom(state->sockfd, (char *)state->rpacket.data, state->blksize+4, 0, (struct sockaddr*)&fromaddr, &fromlen);
...
static long tftp_state_timeout(struct connectdata * conn, tftp_event_t * event)
{
	time_t current;
	tftp_state_data_t * state = (tftp_state_data_t*)conn->proto.tftpc;
	ASSIGN_PTR(event, TFTP_EVENT_NONE);
	time(&current);
	...
	if(current > state->rx_time+state->retry_time) {
		ASSIGN_PTR(event, TFTP_EVENT_TIMEOUT);
...
static CURLcode tftp_doing(struct connectdata * conn, bool * dophase_done)
{
	CURLcode result = tftp_multi_statemach(conn, dophase_done);
	if(*dophase_done) {
...
static CURLcode tftp_setup_connection(struct connectdata * conn)
{
	...
	// TFTP URLs support an extension like ";mode=<typecode>" that we'll try to get now! 
	type = strstr(data->state.path, ";mode=");
	SETIFZ(type, strstr(conn->host.rawalloc, ";mode="));
	if(type) {
