...
#define DOS_FILESYSTEM 1
	...
	#define open_readonly(p, f) _open((p), (f), (0))
#else
	#define open_readonly(p, f) _open((p), (f))
#endif
//
...
#endif
	int real_path_len;
	char * real_path = curl_easy_unescape(data, data->state.path, 0, &real_path_len);
	if(!real_path)
...
static CURLcode file_do(struct connectdata * conn, bool * done)
{
	...
	*done = TRUE; /* unconditionally */
	...
	if(data->set.opt_no_body && data->set.include_header && fstated) {
		snprintf(buf, sizeof(data->state.buffer), "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", expected_size);
	...
	while(!result) {
		...
		size_t bytestoread = (expected_size < CURL_OFF_T_C(BUFSIZE) - CURL_OFF_T_C(1)) ? curlx_sotouz(expected_size) : BUFSIZE - 1;
