...
{
	return ((uint64)smb_swap32((uint)x) << 32) |
...
static CURLcode smb_connect(struct connectdata * conn, bool * done)
{
	...
	if(slash) {
		...
		smbc->user = conn->user;
		smbc->domain = _strdup(conn->host.name);
		if(!smbc->domain)
			return CURLE_OUT_OF_MEMORY;
	}
...
static CURLcode smb_send_and_recv(struct connectdata * conn, void ** msg)
{
	...
	if(!smbc->send_size && smbc->upload_size) {
		int nread = smbc->upload_size > BUFSIZE ? BUFSIZE : (int)smbc->upload_size;
...
static CURLcode smb_parse_url_path(struct connectdata * conn)
{
	...
	req->share = _strdup((*path == '/' || *path == '\\') ? path + 1 : path);
	if(!req->share) {
		SAlloc::F(path);
		return CURLE_OUT_OF_MEMORY;
	}
	...
	// Parse the path for the file path converting any forward slashes into backslashes 
	*slash++ = 0;
	req->path = slash;
	for(; *slash; slash++) {
		if(*slash == '/')
