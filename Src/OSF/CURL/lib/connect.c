...
static CURLcode singleipconnect(struct connectdata * conn, const Curl_addrinfo * ai/* start connecting to this */, curl_socket_t * sock);
/*
...
time_t FASTCALL Curl_timeleft(struct Curl_easy * data, struct timeval * nowp, bool duringconnect)
{
	...
	if(duringconnect)
		timeout_ms -= Curl_tvdiff(*nowp, data->progress.t_startsingle); // since this most recent connect started 
	else
		timeout_ms -= Curl_tvdiff(*nowp, data->progress.t_startop); // since the entire operation started 
	if(!timeout_ms)
    ...
    curl_socket_t sockfd, int af, uint scope)
{
	...
	*************************************************************/
	...
	for(;; ) {
		if(bind(sockfd, sock, sizeof_sa) >= 0) {
			...
			if(getsockname(sockfd, (struct sockaddr *)&add, &size) < 0) {
				...
				failf(data, "getsockname() failed with errno %d: %s", error, Curl_strerror(conn, error));
				return CURLE_INTERFACE_FAILED;
			}
	...
	data->state.os_errno = error = SOCKERRNO;
	failf(data, "bind failed with errno %d: %s", error, Curl_strerror(conn, error));
	return CURLE_INTERFACE_FAILED;
}
/*
...
static bool verifyconnect(curl_socket_t sockfd, int * error)
{
...
#endif
#endif
	if(0 != getsockopt(sockfd, SOL_SOCKET, SO_ERROR, (char *)&err, &errSize))
		err = SOCKERRNO;
#ifdef _WIN32_WCE
	...
	if((0 == err) || (EISCONN == err))
		rc = TRUE; // we are connected, awesome! 
	else
		rc = FALSE; // This wasn't a successful connect 
	ASSIGN_PTR(error, err);
#else
	(void)sockfd;
	ASSIGN_PTR(error, SOCKERRNO);
#endif
	return rc;
}
//
...
void Curl_updateconninfo(struct connectdata * conn, curl_socket_t sockfd)
{
	...
	struct Curl_easy * data = conn->data;
	if(conn->socktype == SOCK_DGRAM) // there's no connection! 
		return;
	if(!conn->bits.reuse && !conn->bits.tcp_fastopen) {
		...
		if(getpeername(sockfd, (struct sockaddr *)&ssrem, &len)) {
			error = SOCKERRNO;
			failf(data, "getpeername() failed with errno %d: %s", error, Curl_strerror(conn, error));
			return;
		}
		len = sizeof(struct Curl_sockaddr_storage);
		...
		if(!getaddressinfo((struct sockaddr *)&ssrem, conn->primary_ip, &conn->primary_port)) {
			error = ERRNO;
			failf(data, "ssrem inet_ntop() failed with errno %d: %s", error, Curl_strerror(conn, error));
			return;
		}
		memcpy(conn->ip_addr_str, conn->primary_ip, MAX_IPADR_LEN);
		if(!getaddressinfo((struct sockaddr *)&ssloc, conn->local_ip, &conn->local_port)) {
			error = ERRNO;
			failf(data, "ssloc inet_ntop() failed with errno %d: %s", error, Curl_strerror(conn, error));
			return;
		}
	}
	...
	*connected = FALSE; /* a very negative world view is best */
	...
	for(i = 0; i<2; i++) {
		...
		if(rc == 0) { /* no connection yet */
			...
			if(i == 0 && conn->tempaddr[1] == NULL && curlx_tvdiff(now, conn->connecttime) >= HAPPY_EYEBALLS_TIMEOUT) {
	...
	if(result) {
		...
		failf(data, "Failed to connect to %s port %ld: %s", hostname, conn->port, Curl_strerror(conn, error));
	}
	return result;
}
...
void Curl_tcpnodelay(struct connectdata * conn, curl_socket_t sockfd)
{
	...
	(void)conn;
#endif
	if(setsockopt(sockfd, level, TCP_NODELAY, (const char *)&onoff, sizeof(onoff)) < 0)
    ...
    curl_socket_t * sockp)
{
	...
	*sockp = CURL_SOCKET_BAD;
	...
	if(!getaddressinfo((struct sockaddr *)&addr.sa_addr, ipaddress, &port)) {
		/* malformed address or bug in inet_ntop, try next address */
		error = ERRNO;
		failf(data, "sa_addr inet_ntop() failed with errno %d: %s", error, Curl_strerror(conn, error));
...
CURLcode Curl_connecthost(struct connectdata * conn/* context */, const struct Curl_dns_entry * remotehost)
{
	struct Curl_easy * data = conn->data;
	...
	if(conn->tempsock[0] == CURL_SOCKET_BAD) {
		SETIFZ(result, CURLE_COULDNT_CONNECT);
		return result;
	}
...
curl_socket_t Curl_getconnectinfo(struct Curl_easy * data, struct connectdata ** connp)
{
	...
	if(data->state.lastconnect && (data->multi_easy || data->multi)) {
		...
		find.found = FALSE;
		Curl_conncache_foreach(data->multi_easy ? &data->multi_easy->conn_cache : &data->multi->conn_cache, &find, conn_is_conn);
		if(!find.found) {
			data->state.lastconnect = NULL;
			return CURL_SOCKET_BAD;
		}
		ASSIGN_PTR(connp, c); // only store this if the caller cares for it 
...
int FASTCALL Curl_closesocket(struct connectdata * conn, curl_socket_t sock)
{
	...
	if(conn) // tell the multi-socket code about this 
		Curl_multi_closed(conn, sock);
	sclose(sock);
	return 0;
}
...
CURLcode Curl_socket(struct connectdata * conn, const Curl_addrinfo * ai, struct Curl_sockaddr_ex * addr, curl_socket_t * sockfd)
{
	...
	SETIFZ(addr, &dummy); // if the caller doesn't want info back, use a local temp copy 
	/*
	...
	SETMIN(addr->addrlen, sizeof(struct Curl_sockaddr_storage));
	...
	else // opensocket callback not set, so simply create the socket now 
		*sockfd = socket(addr->family, addr->socktype, addr->protocol);
	if(*sockfd == CURL_SOCKET_BAD) // no socket, no connection 
		return CURLE_COULDNT_CONNECT;
 ...
 */
void FASTCALL Curl_conncontrol(struct connectdata * conn, int ctrl/* see defines in header */
    ...
    )
{
	...
	if((ctrl == CONNCTRL_STREAM) && (conn->handler->flags & PROTOPT_STREAM))
		...
		conn->bits.close = closeit; // the only place in the source code that should assign this bit 
	}
}
