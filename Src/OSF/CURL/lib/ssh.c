...
#define sftp_libssh2_realpath(s, p, t, m) libssh2_sftp_symlink_ex((s), (p), curlx_uztoui(sstrlen(p)), (t), (m), LIBSSH2_SFTP_REALPATH)
    ...
    const LIBSSH2_USERAUTH_KBDINT_PROMPT * prompts, LIBSSH2_USERAUTH_KBDINT_RESPONSE * responses, void ** abstract)
{
	...
	if(num_prompts == 1) {
		...
		responses[0].length = curlx_uztoui(sstrlen(conn->passwd));
	}
	(void)prompts;
	(void)abstract;
} /* kbd_callback */
...
static void FASTCALL state(struct connectdata * conn, sshstate nowstate)
{
...
static CURLcode ssh_getworkingpath(struct connectdata * conn, char * homedir/* when SFTP is used */, char ** path/* returns the  allocated real path to work with */)
{
	...
	if(conn->handler->protocol & CURLPROTO_SCP) {
		...
		if((working_path_len > 1) && (working_path[1] == '~')) {
			size_t homelen = sstrlen(homedir);
...
static CURLcode ssh_check_fingerprint(struct connectdata * conn)
{
	...
	if(fingerprint) {
		...
		for(i = 0; i < 16; i++)
			snprintf(&md5buffer[i*2], 3, "%02x", (uchar)fingerprint[i]);
	 ...
	 */
	if(pubkey_md5 && sstrlen(pubkey_md5) == 32) {
	...
	do {
		switch(sshc->state) {
			...
			case SSH_AUTHLIST:
			    ...
			    conn->user,
			    curlx_uztoui(sstrlen(conn->user)));
			...
			case SSH_AUTH_PASS:
			    ...
			    curlx_uztoui(sstrlen(conn->user)),
			    conn->passwd,
			    curlx_uztoui(sstrlen(conn->passwd)),
			    NULL);
			    if(rc == LIBSSH2_ERROR_EAGAIN) {
				    break;
			    }
			    ...
			    /* Send any quote commands */
		    {
			    ...
			    if(strcasecompare("pwd", cmd)) {
				    ...
				    /* this sends an FTP-like "header" to the header callback so that the
				       ...
				       using ordinary FTP. */
				    result = Curl_client_write(conn, CLIENTWRITE_HEADER, tmp, sstrlen(tmp));
				    SAlloc::F(tmp);
				    if(result) {
					    state(conn, SSH_SFTP_CLOSE);
					    sshc->nextstate = SSH_NO_STATE;
				    ...
				    if(strncasecompare(cmd, "chgrp ", 6) ||
					    ...
					    if(result) {
						    if(result == CURLE_OUT_OF_MEMORY)
							    ...
							    failf(data, "Syntax error in chgrp/chmod/chown: Bad second parameter");
			...
			case SSH_SFTP_QUOTE_SYMLINK:
			    ...
			    curlx_uztoui(sstrlen(sshc->quote_path1)),
			    sshc->quote_path2,
			    curlx_uztoui(sstrlen(sshc->quote_path2)),
			...
			case SSH_SFTP_FILETIME:
		    {
			    ...
			    rc = libssh2_sftp_stat_ex(sshc->sftp_session, sftp_scp->path,
				    curlx_uztoui(sstrlen(sftp_scp->path)),
			...
			case SSH_SFTP_TRANS_INIT:
			    if(data->set.upload)
				    ...
				    if(sftp_scp->path[sstrlen(sftp_scp->path)-1] == '/')
			...
			case SSH_SFTP_READDIR:
			    ...
			    if(sshc->readdir_len > 0) {
				    ...
				    if(data->set.ftp_list_only) {
					    ...
					    /* output debug output if that is requested */
					    if(data->set.verbose) {
						    Curl_debug(data, CURLINFO_DATA_OUT, sshc->readdir_filename, sshc->readdir_len, conn);
					    }
				    }
				    else {
					    sshc->readdir_currLen = (int)sstrlen(sshc->readdir_longentry);
			...
			case SSH_SFTP_READDIR_BOTTOM:
			    ...
			    if(!result) {
				    /* output debug output if that is requested */
				    if(data->set.verbose) {
					    Curl_debug(data, CURLINFO_DATA_OUT, sshc->readdir_line, sshc->readdir_currLen, conn);
				    }
		    ...
		    {
			    LIBSSH2_SFTP_ATTRIBUTES attrs;
			    rc = libssh2_sftp_stat_ex(sshc->sftp_session, sftp_scp->path, curlx_uztoui(sstrlen(sftp_scp->path)), LIBSSH2_SFTP_STAT, &attrs);
			    if(rc == LIBSSH2_ERROR_EAGAIN) {
				    break;
			    }
			    if(rc || !(attrs.flags & LIBSSH2_SFTP_ATTR_SIZE) || (attrs.filesize == 0)) {
				    ...
				    if(conn->data->state.use_range) {
					    ...
					    char * ptr;
					    char * ptr2;
					    curl_off_t from = curlx_strtoofft(conn->data->state.range, &ptr, 0);
					    while(*ptr && (ISSPACE(*ptr) || (*ptr=='-')))
						    ptr++;
					    to = curlx_strtoofft(ptr, &ptr2, 0);
					    if((ptr == ptr2) /* no "to" value given */ || (to >= size)) {
						    to = size - 1;
					    }
					    if(from < 0) {
			    ...
			    if(result) {
				    /* this should never occur; the close state should be entered at the time the error occurs */
				    state(conn, SSH_SFTP_CLOSE);
				    sshc->actualcode = result;
			    }
			    else {
...
static CURLcode get_pathname(const char ** cpp, char ** path)
{
	...
	*path = (char *)SAlloc::M(sstrlen(cp) + 1);
	if(*path == NULL)
		return CURLE_OUT_OF_MEMORY;
