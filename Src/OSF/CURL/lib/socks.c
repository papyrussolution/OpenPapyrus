 ...
 */
CURLcode Curl_SOCKS4(const char * proxy_name, const char * hostname,
    int remote_port, int sockindex, struct connectdata * conn, bool protocol4a)
{
	...
	if(!protocol4a) {
		...
		if(hp) {
			...
			if(4 == sscanf(buf, "%hu.%hu.%hu.%hu", &ip[0], &ip[1], &ip[2], &ip[3])) {
				/* Set DSTIP */
				socksreq[4] = (uchar)ip[0];
				socksreq[5] = (uchar)ip[1];
				socksreq[6] = (uchar)ip[2];
				socksreq[7] = (uchar)ip[3];
			}
			else
				hp = NULL;  /* fail! */
	 ...
	 */
	{
		...
		switch(socksreq[1]) {
			...
			case 91:
			    ...
			    (uchar)socksreq[4], (uchar)socksreq[5],
			    (uchar)socksreq[6], (uchar)socksreq[7],
    ...
    struct connectdata * conn)
{
	...
	if(socksreq[1] != 0) { /* Anything besides 0 is an error */
		if(socksreq[3] == 1) {
			...
			failf(data,
			    ...
			    (uchar)socksreq[4], (uchar)socksreq[5],
			    (uchar)socksreq[6], (uchar)socksreq[7],
			    (uchar)socksreq[8], (uchar)socksreq[9],
			    (uchar)socksreq[10], (uchar)socksreq[11],
			    (uchar)socksreq[12], (uchar)socksreq[13],
			    (uchar)socksreq[14], (uchar)socksreq[15],
			    (uchar)socksreq[16], (uchar)socksreq[17],
			    (uchar)socksreq[18], (uchar)socksreq[19],
			    ((socksreq[8] << 8) | socksreq[9]),
			    socksreq[1]);
		}
		return CURLE_COULDNT_CONNECT;
	...
	if(len > 10) {
		len -= 10;
		result = Curl_blockread_all(conn, sock, (char*)&socksreq[10], len, &actualread);
		if(result || (len != actualread)) {
			failf(data, "Failed to receive SOCKS5 connect request ack.");
			return CURLE_COULDNT_CONNECT;
		}
