...
bool Curl_pipeline_penalized(struct SessionHandle * data, struct connectdata * conn)
{
	if(data) {
		...
		if(conn->recv_pipe && conn->recv_pipe->head) {
			struct SessionHandle * recv_handle = (struct SessionHandle *)conn->recv_pipe->head->ptr;
...
static CURLcode addHandleToPipeline(struct SessionHandle * data, struct curl_llist * pipeline)
{
...
CURLcode Curl_add_handle_to_pipeline(struct SessionHandle * handle, struct connectdata * conn)
{
	struct curl_llist_element * sendhead = conn->send_pipe->head;
	struct curl_llist * pipeline = conn->send_pipe;
	CURLcode result = addHandleToPipeline(handle, pipeline);
 ...
 */
void Curl_move_handle_from_send_to_recv_pipe(struct SessionHandle * handle, struct connectdata * conn)
{
	struct curl_llist_element * curr = conn->send_pipe->head;
	while(curr) {
		if(curr->ptr == handle) {
...
bool Curl_pipeline_site_blacklisted(struct SessionHandle * handle, struct connectdata * conn)
{
	if(handle->multi) {
		...
		if(blacklist) {
			...
			while(curr) {
				...
				site = (struct site_blacklist_entry *)curr->ptr;
    ...
    char * server_name)
{
	if(handle->multi && server_name) {
		struct curl_llist * blacklist = Curl_multi_pipelining_server_bl(handle->multi);
		if(blacklist) {
			struct curl_llist_element * curr = blacklist->head;
			while(curr) {
				char * bl_server_name = (char *)curr->ptr;
				if(Curl_raw_nequal(bl_server_name, server_name, strlen(bl_server_name))) {
...
CURLMcode Curl_pipeline_set_server_blacklist(char ** servers, struct curl_llist ** list_ptr)
{
	...
	if(servers) {
		...
		/* Parse the URLs and populate the list */
		while(*servers) {
			char * server_name = sstrdup(*servers);
			if(!server_name)
				return CURLM_OUT_OF_MEMORY;
...
static bool pipe_head(struct SessionHandle * data, struct curl_llist * pipeline)
{
...
bool Curl_recvpipe_head(struct SessionHandle * data, struct connectdata * conn)
{
	return pipe_head(data, conn->recv_pipe);
}

/* returns TRUE if the given handle is head of the send pipe */
bool Curl_sendpipe_head(struct SessionHandle * data, struct connectdata * conn)
{
...
bool Curl_pipeline_checkget_write(struct SessionHandle * data, struct connectdata * conn)
{
...
bool Curl_pipeline_checkget_read(struct SessionHandle * data, struct connectdata * conn)
{
	if(conn->bits.multiplex)
		/* when multiplexing, we can use it at once */
		return TRUE;
