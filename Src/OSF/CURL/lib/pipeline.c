...
bool Curl_pipeline_penalized(struct Curl_easy * data, struct connectdata * conn)
{
	if(data) {
		...
		if(conn->recv_pipe.head) {
			struct Curl_easy * recv_handle = (struct Curl_easy *)conn->recv_pipe.head->ptr;
...
bool Curl_pipeline_site_blacklisted(struct Curl_easy * handle, struct connectdata * conn)
{
	if(handle->multi) {
		...
		if(blacklist) {
			...
			while(curr) {
				struct site_blacklist_entry * site = (struct site_blacklist_entry *)curr->ptr;
...
{
	if(conn->bits.multiplex) // when multiplexing, we can use it at once 
		return TRUE;
