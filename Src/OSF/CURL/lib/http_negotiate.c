    ...
    const char * header)
{
	...
	len = strlen(header);
	if(len > 0) {
		result = Curl_base64_decode(header, (uchar**)&input_token.value,
		    &rawlen);
		if(result)
			return result;
...
CURLcode Curl_output_negotiate(struct connectdata * conn, bool proxy)
{
	...
	result = Curl_base64_encode(conn->data, neg_ctx->output_token.value, neg_ctx->output_token.length, &encoded, &len);
	if(result) {
...
static void cleanup(struct negotiatedata * neg_ctx)
{
	...
	memzero(neg_ctx, sizeof(*neg_ctx));
}
