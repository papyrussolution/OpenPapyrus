...
CURLcode Curl_input_negotiate(struct connectdata * conn, bool proxy, const char * header)
{
	...
	if(!len) {
		...
		if(neg_ctx->context) {
			// The server rejected our authentication and hasn't suppled any more negotiation mechanisms 
...
CURLcode Curl_output_negotiate(struct connectdata * conn, bool proxy)
{
	...
	size_t len = 0;
	char * userp;
	CURLcode result = Curl_auth_create_spnego_message(conn->data, neg_ctx, &base64, &len);
	if(result)
		return result;
