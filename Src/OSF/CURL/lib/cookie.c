...
{
	size_t len;
	char * new_path = sstrdup(cookie_path);
	if(!new_path)
		return NULL;
...
/*
 * strstore() makes a sstrdup() on the 'newstr' and if '*str' is non-NULL
 * that will be freed before the allocated string is stored there.
 *
 * It is meant to easily replace sstrdup()
 */
                                       ...
                                       unless set */
{
	...
	if(httpheader) {
		...
		const char * ptr;
		const char * semiptr;
		char * what = (char *)malloc(MAX_COOKIE_LINE);
		if(!what) {
			free(co);
			return NULL;
		}
			...
			if(1 <= sscanf(ptr, "%" MAX_NAME_TXT "[^;\r\n =] =%"
				...
				if(!len) {
					...
					if(Curl_raw_equal("secure", name))
						...
						done = FALSE; // there was a '=' so we're not done parsing this field
				}
				if(done)
					;
		...
		if(!badcookie && (7 != fields))
			badcookie = TRUE; // we did not find the sufficient number of fields
		if(badcookie) {
			freecookie(co);
			return NULL;
		}
		...
		if(Curl_raw_equal(clist->name, co->name)) {
			...
			if(clist->domain && co->domain) {
				if(Curl_raw_equal(clist->domain, co->domain))
					replace_old = TRUE; // The domains are identical
			}
...
struct CookieInfo * Curl_cookie_init(struct SessionHandle * data, const char * file, struct CookieInfo * inc, bool newsession)
{
	...
	if(NULL == inc) {
		...
		c->filename = sstrdup(file ? file : "none"); /* copy the name just in case */
		if(!c->filename)
...
static int cookie_output(struct CookieInfo * c, const char * dumphere)
{
	...
	if(c) {
		...
		for(co = c->cookies; co; co = co->next) {
			if(co->domain) {
				char * format_ptr = get_netscape_format(co);
				if(format_ptr == NULL) {
					fprintf(out, "#\n# Fatal libcurl error\n");
					if(!use_stdout)
						fclose(out);
