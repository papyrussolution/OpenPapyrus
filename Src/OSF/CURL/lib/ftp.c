...
static void FASTCALL freedirs(struct ftp_conn * ftpc)
{
	if(ftpc->dirs) {
...
static CURLcode AcceptServerConnect(struct connectdata * conn)
{
	...
	// when this happens within the DO state it is important that we mark us as not needing DO_MORE anymore 
...
static time_t ftp_timeleft_accept(struct Curl_easy * data)
{
	...
	if(other && (other < timeout_ms))
		// note that this also works fine for when other happens to be negative due to it already having elapsed 
		timeout_ms = other;
	else {
		...
		default:
		    if(result & CURL_CSELECT_IN2) {
			    ...
			    *received = TRUE;
			    ...
			    Curl_GetFTPResponse(&nread, conn, &ftpcode);
			    return (ftpcode/100 > 3) ? CURLE_FTP_ACCEPT_FAILED : CURLE_WEIRD_SERVER_REPLY;
		    }
		    break;
	} /* switch() */
	return CURLE_OK;
...
{
	struct Curl_easy * data = conn->data;
	struct FTP * ftp = static_cast<struct FTP *>(data->req.protop);
	...
	if(conn->proto.ftpc.state_saved == FTP_STOR) {
		*(ftp->bytecountp) = 0;
		...
		Curl_setup_transfer(conn, -1, -1, FALSE, NULL/* no download */, SECONDARYSOCKET, ftp->bytecountp);
	}
	else {
		/* FTP download: */
		Curl_setup_transfer(conn, SECONDARYSOCKET, conn->proto.ftpc.retr_size_saved, FALSE, ftp->bytecountp, -1, 0); /* no upload here */
	}
 ...
 * us. This function checks whether data connection is established if so it is accepted.
 */
	...
	*connected = FALSE;
	...
	if(*connected) {
		...
		if(!result && *connected == FALSE) {
			Curl_expire(data, (data->set.accepttimeout > 0) ? data->set.accepttimeout : DEFAULT_ACCEPT_TIMEOUT);
		}
	}
	return result;
}
...
static CURLcode ftp_readresp(curl_socket_t sockfd, struct pingpong * pp, int * ftpcode/* return the ftp-code if done */, size_t * size/* size of the response */)
{
	...
	switch(code) {
		...
		default: break; // normal ftp stuff we pass through! 
	}
#endif
	/* store the latest code for later retrieval */
	data->info.httpcode = code;
	ASSIGN_PTR(ftpcode, code);
	if(code == 421) {
 ...
 *
 */
CURLcode Curl_GetFTPResponse(ssize_t * nreadp/* return number of bytes read */, struct connectdata * conn, int * ftpcode/* return the ftp-code */)
{
	/*
	...
	if(ftpcode) // 0 for errors 
		*ftpcode = 0;  
	else // make the pointer point to something for the rest of this function 
	...
	*nreadp = 0;
	while(!*ftpcode && !result) {
		...
		if(result)
			break;
		if(!nread && pp->cache) // bump cache skip counter as on repeated skips we must wait for more data 
			cache_skip++;
		else // when we got data or there is no cache left, we reset the cache skip counter 
			cache_skip = 0;
		*nreadp += nread;
...
static const char * const ftp_state_names[] = {
	"STOP", "WAIT220", "AUTH", "USER", "PASS", "ACCT", "PBSZ", "PROT", "CCC", "PWD", "SYST", "NAMEFMT", "QUOTE",
	"RETR_PREQUOTE", "STOR_PREQUOTE", "POSTQUOTE", "CWD", "MKD", "MDTM", "TYPE", "LIST_TYPE", "RETR_TYPE", "STOR_TYPE",
	"SIZE", "RETR_SIZE", "STOR_SIZE", "REST", "RETR_REST", "PORT", "PRET", "PASV", "LIST", "RETR", "STOR", "QUIT"
};
#endif
...
{
	CURLcode result;
	struct FTP * ftp = static_cast<struct FTP *>(conn->data->req.protop);
	/* send USER */
...
static CURLcode ftp_state_use_port(struct connectdata * conn, /*ftpport*/int fcmd/*start with this*/) // @sobolev ftpport-->int
{
	...
	if(data->set.str[STRING_FTPPORT] && (sstrlen(data->set.str[STRING_FTPPORT]) > 1)) {
		...
		/* correct errors like:
		 *  :1234-1230
		 *  :-4711,  in this case port_min is static_cast<uint>(-1),
		 *      therefore port_min > port_max for all cases
		 *      but port_max = static_cast<uint>(-1)
		 */
...
static CURLcode ftp_state_rest(struct connectdata * conn)
{
	...
	if((ftp->transfer != FTPTRANSFER_BODY) && ftpc->file) {
		...
		// Determine if server can respond to REST command and therefore whether it supports range 
...
static CURLcode ftp_state_type(struct connectdata * conn)
{
	...
	// information. Which in FTP can't be much more than the file size and date. 
	if(data->set.opt_no_body && ftpc->file && ftp_need_type(conn, data->set.prefer_ascii)) {
		/* The SIZE command is _not_ RFC 959 specified, and therefor many servers
		   may not support it! It is however the only way we have to get a file's size! */
...
static CURLcode ftp_state_ul_setup(struct connectdata * conn, bool sizechecked)
{
	...
	if((data->state.resume_from && !sizechecked) || ((data->state.resume_from > 0) && sizechecked)) {
		...
		/* 1. get already existing file's size. We use the SIZE command for this
		   which may not exist in the server!  The SIZE command is not in RFC959. */
		...
		if(data->state.infilesize>0) {
			...
			if(data->state.infilesize <= 0) {
				...
				// Set ->transfer so that we won't get any error in ftp_done() because we didn't transfer anything! 
...
static CURLcode ftp_state_quote(struct connectdata * conn, bool init, ftpstate instate)
{
	...
	switch(instate) {
		case FTP_QUOTE:
		default: item = data->set.quote; break;
		case FTP_RETR_PREQUOTE:
		case FTP_STOR_PREQUOTE: item = data->set.prequote; break;
...
static char * control_address(struct connectdata * conn)
{
	/* Returns the control connection IP address.
	   ...
	   not the ftp host. */
	return (conn->bits.tunnel_proxy || conn->bits.socksproxy) ? conn->host.name : conn->ip_addr_str;
}
...
static CURLcode ftp_state_port_resp(struct connectdata * conn, int ftpcode)
{
	struct Curl_easy * data = conn->data;
	struct ftp_conn * ftpc = &conn->proto.ftpc;
	/*ftpport*/int fcmd = (ftpport)ftpc->count1; // @sobolev ftpport-->int
	CURLcode result = CURLE_OK;
...
static CURLcode ftp_state_mdtm_resp(struct connectdata * conn, int ftpcode)
{
	...
	switch(ftpcode) {
		case 213:
	    {
		    ...
		    if(6 == sscanf(buf+4, "%04d%02d%02d%02d%02d%02d", &year, &month, &day, &hour, &minute, &second)) {
			    ...
			    snprintf(buf, CURL_BUFSIZE(conn->data->set.buffer_size), "%04d%02d%02d %02d:%02d:%02d GMT", year, month, day, hour, minute, second);
	...
	SETIFZ(result, ftp_state_type(conn));
	return result;
}
...
static CURLcode ftp_state_size_resp(struct connectdata * conn, int ftpcode, ftpstate instate)
{
	...
	curl_off_t filesize = (ftpcode == 213) ? curlx_strtoofft(buf+4, NULL, 0) : -1;
	if(instate == FTP_SIZE) {
#ifdef CURL_FTP_HTTPSTYLE_HEAD
		if(-1 != filesize) {
...
static CURLcode ftp_done(struct connectdata * conn, CURLcode status, bool premature)
{
	...
	switch(status) {
		...
		// @fallthrough
		default: // by default, an error means the control connection is wedged and should not be used anymore 
		    ftpc->ctl_valid = FALSE;
		    ftpc->cwdfail = TRUE; // set this TRUE to prevent us to remember the current path, as this connection is going 
	...
	SETIFZ(result, Curl_urldecode(data, path_to_use, 0, &path, NULL, FALSE)); // get the "raw" path 
	if(result) {
		// We can limp along anyway (and should try to since we may already be in the error path) 
	...
	if(conn->sock[SECONDARYSOCKET] != CURL_SOCKET_BAD) {
		...
		if(conn->ssl[SECONDARYSOCKET].use) {
			// The secondary socket is using SSL so we must close down that part first before we close the socket for real
			Curl_ssl_close(conn, SECONDARYSOCKET);
			// Note that we keep "use" set to TRUE since that (next) connection is still requested to use SSL 
		}
		close_secondarysocket(conn);
	}
...
static CURLcode ftp_range(struct connectdata * conn)
{
	...
	if(data->state.use_range && data->state.range) {
		...
		if((-1 == to) && (from>=0)) {
			...
			DEBUGF(infof(conn->data, "FTP RANGE %" CURL_FORMAT_CURL_OFF_T " to end of file\n", from));
		}
		else if(from < 0) {
			/* -Y */
			data->req.maxdownload = -from;
			data->state.resume_from = from;
			DEBUGF(infof(conn->data, "FTP RANGE the last %" CURL_FORMAT_CURL_OFF_T " bytes\n", -from));
		}
		else {
			/* X-Y */
...
static CURLcode ftp_do_more(struct connectdata * conn, int * completep)
{
	...
	if(CONNECT_SECONDARYSOCKET_PROXY_SSL())
		return result;
	if(conn->bits.tunnel_proxy && conn->bits.httpproxy && conn->tunnel_state[SECONDARYSOCKET] != TUNNEL_COMPLETE)
		return result;
	if(ftpc->state) {
		// already in a state so skip the initial commands. They are only done to kickstart the do_more state 
		result = ftp_multi_statemach(conn, &complete);
		*completep = (int)complete;
		// if we got an error or if we don't wait for a data connection return immediately 
	...
	if(ftp->transfer <= FTPTRANSFER_INFO) {
		...
		if(ftpc->wait_data_conn == TRUE) {
			...
			if(serv_conned) {
				...
				SETIFZ(result, InitiateTransfer(conn));
				if(result)
					return result;
				*completep = 1; // this state is now complete when the server has connected back to us 
			}
		}
		else if(data->set.upload) {
	...
	if(!result && (ftp->transfer != FTPTRANSFER_BODY))
		// no data to transfer. FIX: it feels like a kludge to have this here too! 
		Curl_setup_transfer(conn, -1, -1, FALSE, NULL, -1, 0);
	if(!ftpc->wait_data_conn) {
...
static CURLcode init_wc_data(struct connectdata * conn)
{
	...
	char * last_slash = strrchr(conn->data->state.path, '/');
	if(last_slash) {
		last_slash++;
		if(last_slash[0] == '\0') {
...
static CURLcode wc_statemach(struct connectdata * conn)
{
	...
	switch(wildcard->state) {
		case CURLWC_INIT:
		    ...
		    if(wildcard->state == CURLWC_CLEAN) // only listing! 
			    break;
		...
		case CURLWC_MATCHING: {
		    ...
		    struct ftp_wc_tmpdata * ftp_tmp = (struct ftp_wc_tmpdata *)wildcard->tmp;
		...
		case CURLWC_DOWNLOADING: {
		    ...
		    if(conn->data->set.chunk_bgn) {
			    long userresponse = conn->data->set.chunk_bgn(finfo, wildcard->customptr, (int)wildcard->filelist.size);
			    switch(userresponse) {
	...
	*done = FALSE; /* default to false */
	...
	if(conn->data->set.wildcardmatch) {
		...
		if(conn->data->wildcard.state == CURLWC_SKIP || conn->data->wildcard.state == CURLWC_DONE)
			return CURLE_OK; // do not call ftp_regular_transfer 
		else if(result) // error, loop or skipping the file 
			return result;
	}
...
static CURLcode ftp_parse_url_path(struct connectdata * conn)
{
	...
	const char * cur_pos = path_to_use; // current position in path. point at the begin of next path component 
	ftpc->ctl_valid = FALSE;
	ftpc->cwdfail = FALSE;
	...
	if(ftpc->prevpath) {
		// prevpath is "raw" so we convert the input path before we compare the strings 
		size_t dlen;
		char * path;
...
static CURLcode ftp_setup_connection(struct connectdata * conn)
{
	...
	conn->data->req.protop = ftp = static_cast<struct FTP *>(SAlloc::M(sizeof(struct FTP)));
	if(!ftp)
		return CURLE_OUT_OF_MEMORY;
	...
	SETIFZ(type, strstr(conn->host.rawalloc, ";type="));
	if(type) {
