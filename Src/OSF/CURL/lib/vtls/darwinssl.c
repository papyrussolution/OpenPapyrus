...
#pragma unused(server_certs)
	...
	if(err == noErr && trust) {
		...
		for(i = 0L; i < count; i++) {
			...
			memzero(server_cert_summary_c, 128);
...
static CURLcode darwinssl_connect_step3(struct connectdata * conn,
    int sockindex)
{
	struct Curl_easy * data = conn->data;
	struct ssl_connect_data * connssl = &conn->ssl[sockindex];
...
static CURLcode darwinssl_connect_common(struct connectdata * conn,
    int sockindex,
    bool nonblocking,
    bool * done)
{
...
CURLcode Curl_darwinssl_connect(struct connectdata * conn,
    int sockindex)
{
	CURLcode result;
	bool done = FALSE;
