...
static CURLcode cyassl_connect_step2(struct connectdata * conn,
    int sockindex)
{
	int ret = -1;
	struct SessionHandle * data = conn->data;
	...
	if(data->set.str[STRING_SSL_PINNEDPUBLICKEY]) {
		...
		result = Curl_pin_peer_pubkey(data, data->set.str[STRING_SSL_PINNEDPUBLICKEY], 
			(const uchar*)pubkey->header, (size_t)(pubkey->end - pubkey->header));
		if(result) {
...
static CURLcode cyassl_connect_common(struct connectdata * conn,
    int sockindex,
    bool nonblocking,
    bool * done)
{
...
CURLcode Curl_cyassl_connect(struct connectdata * conn,
    int sockindex)
{
	CURLcode result;
	bool done = FALSE;
