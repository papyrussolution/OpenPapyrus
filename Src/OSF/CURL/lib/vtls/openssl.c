 ...
 */
static char * SSL_strerror(ulong error, char * buf, size_t size)
{
...
int Curl_ossl_init(void)
{
	...
	/* 
		...
		calls CONF_modules_load_file() and we use that instead and we ignore its return code! 
	*/

	// CONF_MFLAGS_DEFAULT_SECTION introduced some time between 0.9.8b and 0.9.8e
...
{
	EVP_cleanup(); /* Free ciphers and digests lists */
#ifdef HAVE_ENGINE_CLEANUP
	ENGINE_cleanup(); /* Free engine list */
#endif
#ifdef HAVE_CRYPTO_CLEANUP_ALL_EX_DATA
	CRYPTO_cleanup_all_ex_data(); /* Free OpenSSL ex_data table */
#endif
...
{
	char   buf;
	int    rc = SSL_peek(conn->ssl[FIRSTSOCKET].handle, (void*)&buf, 1);
	if(rc > 0)
...
int Curl_ossl_shutdown(struct connectdata * conn, int sockindex)
{
	...
	struct ssl_connect_data * connssl = &conn->ssl[sockindex];
	struct SessionHandle * data = conn->data;
	char buf[120]; // We will use this for the OpenSSL error buffer, so it has to be at least 120 bytes long.
	ulong sslerror;
	ssize_t nread;
	int buffsize;
	int err;
	...
	*/
	...
	if(connssl->handle) {
		...
		while(!done) {
			int what = Curl_socket_ready(conn->sock[sockindex], CURL_SOCKET_BAD, SSL_SHUTDOWN_TIMEOUT);
			if(what > 0) {
				ERR_clear_error();
...
{
	SSL_SESSION_free((SSL_SESSION *)ptr); /* free the ID */
}
...
#if (OPENSSL_VERSION_NUMBER >= 0x0090808fL) && !defined(OPENSSL_NO_TLSEXT) && !defined(OPENSSL_IS_BORINGSSL)
    ...
    const char * pinnedpubkey)
{
	...
	do {
		...
		buff1 = temp = (uchar *)OPENSSL_malloc(len1);
		if(!buff1)
			break;  /* failed */
