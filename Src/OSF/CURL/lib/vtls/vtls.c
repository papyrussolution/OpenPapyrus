...
bool Curl_clone_ssl_config(struct ssl_config_data * source, struct ssl_config_data * dest)
{
	...
	if(source->CAfile) {
		dest->CAfile = sstrdup(source->CAfile);
		if(!dest->CAfile)
			return FALSE;
	}
	else
		dest->CAfile = NULL;
	if(source->CApath) {
		dest->CApath = sstrdup(source->CApath);
		if(!dest->CApath)
			return FALSE;
	}
	else
	...
	if(source->cipher_list) {
		dest->cipher_list = sstrdup(source->cipher_list);
		if(!dest->cipher_list)
			return FALSE;
	}
	else
	...
	if(source->egdsocket) {
		dest->egdsocket = sstrdup(source->egdsocket);
		if(!dest->egdsocket)
			return FALSE;
	}
	else
	...
	if(source->random_file) {
		dest->random_file = sstrdup(source->random_file);
		if(!dest->random_file)
			return FALSE;
	}
	else
...
uint Curl_rand(struct SessionHandle * data)
{
	uint r = 0;
	...
	/* data may be NULL! */
	if(!Curl_ssl_random(data, (uchar*)&r, sizeof(r)))
		return r;
    ...
    size_t idsize)
{
	...
	clone_host = sstrdup(conn->host.name);
	if(!clone_host)
    ...
    const uchar * pubkey, size_t pubkeylen)
{
	...
	if(strncmp(pinnedpubkey, "sha256//", 8) == 0) {
		...
		sha256sumdigest = (uchar *)malloc(SHA256_DIGEST_LENGTH);
		if(!sha256sumdigest)
			return CURLE_OUT_OF_MEMORY;
