    ...
    struct negotiatedata * nego)
{
	...
	if(!nego->credentials) {
		...
		nego->credentials = SAlloc::M(sizeof(CredHandle));
		if(!nego->credentials)
			return CURLE_OUT_OF_MEMORY;
		memzero(nego->credentials, sizeof(CredHandle));
		/* Acquire our credentials handle */
		nego->status = s_pSecFn->AcquireCredentialsHandle(NULL, (TCHAR*)TEXT(SP_NAME_NEGOTIATE),
		    SECPKG_CRED_OUTBOUND, NULL, nego->p_identity, NULL, NULL, nego->credentials, &expiry);
		if(nego->status != SEC_E_OK)
			return CURLE_LOGIN_DENIED;
		/* Allocate our new context handle */
		nego->context = SAlloc::M(sizeof(CtxtHandle));
		if(!nego->context)
			return CURLE_OUT_OF_MEMORY;
		memzero(nego->context, sizeof(CtxtHandle));
	}
	if(chlg64 && *chlg64) {
