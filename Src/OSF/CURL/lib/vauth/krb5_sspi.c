    ...
    char ** outptr, size_t * outlen)
{
	...
	if(!krb5->credentials) {
		...
		krb5->credentials = SAlloc::M(sizeof(CredHandle));
		if(!krb5->credentials)
			return CURLE_OUT_OF_MEMORY;
		memzero(krb5->credentials, sizeof(CredHandle));
		// Acquire our credentials handle 
		status = s_pSecFn->AcquireCredentialsHandle(NULL, (TCHAR*)TEXT(SP_NAME_KERBEROS),
		    SECPKG_CRED_OUTBOUND, NULL, krb5->p_identity, NULL, NULL, krb5->credentials, &expiry);
		if(status != SEC_E_OK)
			return CURLE_LOGIN_DENIED;
		/* Allocate our new context handle */
		krb5->context = SAlloc::M(sizeof(CtxtHandle));
		if(!krb5->context)
			return CURLE_OUT_OF_MEMORY;
		memzero(krb5->context, sizeof(CtxtHandle));
	}
	if(chlg64 && *chlg64) {
