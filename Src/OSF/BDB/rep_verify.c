 ...
 * PUBLIC:     int, __time64_t));
 */
int __rep_verify(ENV * env, __rep_control_args * rp, DBT * rec, int eid, __time64_t savetime)
{
	DBT mylog;
	DB_LOG * dblp;
	DB_LOGC * logc;
	...
	if((ret = __logc_get(logc, &rp->lsn, &mylog, logflag)) != 0)
		goto out;
	match = 0;
	if(mylog.size == rec->size && memcmp(mylog.data, rec->data, rec->size) == 0)
		match = 1;
	/*
	...
	if(match == 0) {
		...
		if(!lp->db_log_inmemory && rectype == DB___txn_ckp) {
			if((ret = __txn_ckp_read(env, mylog.data, &ckp_args)) != 0)
				goto out;
			...
			if(IS_ZERO_LSN(lsn)) {
				 ...
				 * way back to file #1.
				 */
				if((ret = __logc_get(logc, &lsn, &mylog, DB_FIRST)) != 0)
					goto out;
				if(lsn.file != 1) {
...
static int __rep_internal_init(ENV * env, uint32 abbrev)
{
	...
	if(ret == 0 && master != DB_EID_INVALID)
		__rep_send_message(env, master, REP_UPDATE_REQ, NULL, NULL, 0, 0);
	return ret;
}
/*
 * __rep_verify_fail --
...
int __rep_verify_fail(ENV * env, __rep_control_args * rp)
{
	...
	if(rep->sync_state == SYNC_LOG &&
	   ...
	   LOG_COMPARE(&rep->last_lsn, &rp->lsn) >= 0) {
		...
		if(ISSET_LOCKOUT_BDB(rep)) {
			RPRINT(env, (env, DB_VERB_REP_SYNC, "VERIFY_FAIL is cleaning up old internal init for missing log"));
			if((ret = __rep_init_cleanup(env, rep, DB_FORCE)) != 0) {
				RPRINT(env, (env, DB_VERB_REP_SYNC, "VERIFY_FAIL error cleaning up internal init for missing log: %d", ret));
				goto msglck;
			}
	 ...
	 */
	if((rep->sync_state == SYNC_VERIFY && LOG_COMPARE(&rp->lsn, &lp->verify_lsn) == 0) ||
	   (rep->sync_state == SYNC_LOG && LOG_COMPARE(&rep->first_lsn, &rp->lsn) <= 0 &&
	    LOG_COMPARE(&rep->last_lsn, &rp->lsn) >= 0) ||
	   (rep->sync_state == SYNC_OFF && LOG_COMPARE(&rp->lsn, &lp->ready_lsn) >= 0)) {
		/*
		 * Update stats.
		 */
...
int __rep_dorecovery(ENV * env, DB_LSN * lsnp, DB_LSN * trunclsnp)
{
	...
	rollback = 0;
	while(update == 0 && (ret = __logc_get(logc, &lsn, &mylog, DB_PREV)) == 0 && LOG_COMPARE(&lsn, lsnp) > 0) {
		LOGCOPY_32(env, &rectype, mylog.data);
		/*
		...
		if(rectype == DB___txn_regop || rectype == DB___txn_ckp || rectype == DB___dbreg_register)
			skip_rec = 0;
		if(rectype == DB___txn_regop) {
			if(rep->version >= DB_REPVERSION_44) {
				if((ret = __txn_regop_read(env, mylog.data, &txnrec)) != 0)
					goto err;
				opcode = txnrec->opcode;
				__os_free(env, txnrec);
			}
			else {
				if((ret = __txn_regop_42_read(env, mylog.data, &txn42rec)) != 0)
					goto err;
 ...
 *
 * PUBLIC: int __rep_verify_match __P((ENV *, DB_LSN *, __time64_t));
 */
int __rep_verify_match(ENV * env, DB_LSN * reclsnp, __time64_t savetime)
{
	DB_LSN trunclsn;
	DB_THREAD_INFO * ip;
	REGENV * renv;
	REGINFO * infop;
	int done, event, master;
	uint32 unused;
	DB_LOG * dblp = env->lg_handle;
	DB_REP * db_rep = env->rep_handle;
	REP * rep = db_rep->region;
	 ...
	 */
	REP_SYSTEM_LOCK(env);
	if(FLD_ISSET(rep->lockout_flags, REP_LOCKOUT_MSG) || (rep->sync_state != SYNC_LOG && ISSET_LOCKOUT_BDB(rep))) {
		/*
	...
	REP_SYSTEM_UNLOCK(env);
	if((ret = __rep_dorecovery(env, reclsnp, &trunclsn)) != 0 || (ret = __rep_remove_init_file(env)) != 0) {
		REP_SYSTEM_LOCK(env);
