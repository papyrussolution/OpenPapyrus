...
int __dbreg_close_files(ENV * env, int do_restored)
{
	...
	for(i = 0; i < dblp->dbentry_cnt; i++) {
		...
		if((dbp = dblp->dbentry[i].dbp) != NULL) {
			 ...
			 */
			if(do_restored && !F_ISSET(dbp->log_filename, DB_FNAME_RESTORED))
				continue;
			MUTEX_UNLOCK(env, dblp->mtx_dbreg);
...
int __dbreg_invalidate_files(ENV * env, int do_restored)
{
	...
	SH_TAILQ_FOREACH(fnp, &lp->fq, q, __fname) {
		...
		if(!F_ISSET(fnp, DB_FNAME_RESTORED) && do_restored)
			continue;
		if(fnp->id != DB_LOGFILEID_INVALID) {
			if((ret = __dbreg_log_close(env, fnp, NULL, DBREG_RCLOSE)) != 0)
				goto err;
			fnp->old_id = fnp->id;
...
int __dbreg_id_to_db(ENV * env, DB_TXN * txn, DB ** dbpp, int32 ndx, int tryopen)
{
	 ...
	 */
	if(ndx >= dblp->dbentry_cnt || (!dblp->dbentry[ndx].deleted && dblp->dbentry[ndx].dbp == NULL)) {
		 ...
		 */
		if((ret = __dbreg_do_open(env, txn, dblp, fname->ufid, name, fname->s_type, ndx, fname->meta_pgno,
			    NULL, TXN_INVALID, F_ISSET(fname, DB_FNAME_INMEM) ? DBREG_REOPEN : DBREG_OPEN)) != 0)
			return ret;
 ...
 * PUBLIC:     int32, db_pgno_t, void *, uint32, uint32));
 */
int __dbreg_do_open(ENV * env, DB_TXN * txn, DB_LOG * lp, uint8 * uid, char * name, DBTYPE ftype, int32 ndx, db_pgno_t meta_pgno, void * info, uint32 id, uint32 opcode)
{
	DB * dbp;
...
retry_inmem:
	...
	if((ret = __db_open(dbp, NULL, txn, fname, dname, ftype, DB_DURABLE_UNKNOWN|DB_ODDFILESIZE, DB_MODE_600, meta_pgno)) == 0) {
skip_open:
		/*
		 * Verify that we are opening the same file that we were
		 * referring to when we wrote this log record.
		 */
		if((meta_pgno != PGNO_BASE_MD && __dbreg_check_master(env, uid, name) != 0) || memcmp(uid, dbp->fileid, DB_FILE_ID_LEN) != 0)
...
static int __dbreg_check_master(ENV * env, uint8 * uid, char * name)
{
	...
	if((ret = __db_create_internal(&dbp, env, 0)) != 0)
		return ret;
	F_SET(dbp, DB_AM_RECOVER);
	ret = __db_open(dbp, NULL, NULL, name, NULL, DB_BTREE, 0, DB_MODE_600, PGNO_BASE_MD);
