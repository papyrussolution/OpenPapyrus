...
public:
	...
	virtual int compact(DbTxn * txnid, Dbt * start, Dbt * stop, DB_COMPACT * c_data, u_int32_t flags, Dbt * end);
	...
	virtual int pget(DbTxn * txnid, Dbt * key, Dbt * pkey, Dbt * data, u_int32_t flags);
...
public:
	...
	virtual int dbremove(DbTxn * txn, const char * name, const char * subdb, u_int32_t flags);
	virtual int dbrename(DbTxn * txn, const char * name, const char * subdb, const char * newname, u_int32_t flags);
	virtual void err(int, const char *, ...);
	virtual void errx(const char *, ...);
	...
	static void runtime_error_lock_get(DbEnv * dbenv, const char * caller, int err, db_lockop_t op,
		db_lockmode_t mode, Dbt * obj, DbLock lock, int index,
		int error_policy);

	// Lock functions
	//
	virtual int lock_detect(u_int32_t flags, u_int32_t atype, int * aborted);
	virtual int lock_get(u_int32_t locker, u_int32_t flags, Dbt * obj, db_lockmode_t lock_mode, DbLock * lock);
	...
	virtual int txn_recover(DbPreplist * preplist, long count, long * retp, u_int32_t flags);
	...
	virtual int repmgr_channel(int eid, DbChannel ** channel, u_int32_t flags);
	...
	virtual int repmgr_site(const char * host, u_int port, DbSite ** site, u_int32_t flags);
	...
	static int _isalive_intercept(DB_ENV * dbenv, pid_t pid, db_threadid_t thrid, u_int32_t flags);
...
private:
	...
	int (* rep_send_callback_)(DbEnv *, const Dbt *, const Dbt *, const DbLsn *, int, u_int32_t);
	void (* thread_id_callback_)(DbEnv *, pid_t *, db_threadid_t *);
	char *(*thread_id_string_callback_)(DbEnv*, pid_t, db_threadid_t,
	                                    char *);
	void (* message_dispatch_callback_)(DbEnv *, DbChannel *, Dbt *, u_int32_t, u_int32_t);
};
...
public:
	...
	void set_flags(u_int32_t value)
	{
		flags = value;
	}
