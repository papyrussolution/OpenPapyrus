...
public:
	...
	int send_request(Dbt * request, uint32 nrequest, Dbt * response, db_timeout_t timeout, uint32 flags);
...
public:
	...
	virtual int dbremove(DbTxn * txn, const char * name, const char * subdb, uint32 flags);
	virtual int dbrename(DbTxn * txn, const char * name, const char * subdb, const char * newname, uint32 flags);
	virtual void err(int, const char *, ...);
	virtual void errx(const char *, ...);
	...
	virtual int get_isalive(int (* *)(DbEnv*, pid_t, db_threadid_t, uint32));
	virtual int set_isalive(int (*)(DbEnv *, pid_t, db_threadid_t, uint32));
	virtual int set_encrypt(const char *, uint32);
	virtual void get_errcall(void (* *)(const DbEnv*, const char *, const char *));
	virtual void set_errcall(void (*)(const DbEnv *, const char *, const char *));
	virtual void get_errfile(FILE **);
	...
	virtual int get_app_dispatch(int (* *)(DbEnv*, Dbt*, DbLsn*, db_recops));
	virtual int set_app_dispatch(int (*)(DbEnv *, Dbt *, DbLsn *, db_recops));
	virtual int get_tx_timestamp(__time64_t *);
	virtual int set_tx_timestamp(__time64_t *);
	...
	// used internally
	static void runtime_error(DbEnv * dbenv, const char * caller, int err, int error_policy);
	static void runtime_error_dbt(DbEnv * dbenv, const char * caller, Dbt * dbt, int error_policy);
	static void runtime_error_lock_get(DbEnv * dbenv, const char * caller, int err, db_lockop_t op,
		db_lockmode_t mode, Dbt * obj, DbLock lock, int index, int error_policy);

	// Lock functions
	//
	virtual int lock_detect(uint32 flags, uint32 atype, int * aborted);
	virtual int lock_get(uint32 locker, uint32 flags, Dbt * obj, db_lockmode_t lock_mode, DbLock * lock);
	...
	virtual int memp_register(int ftype, pgin_fcn_type pgin_fcn, pgout_fcn_type pgout_fcn);
	virtual int memp_stat(DB_MPOOL_STAT ** gsp, DB_MPOOL_FSTAT *** fsp, uint32 flags);
	...
	virtual int txn_checkpoint(uint32 kbyte, uint32 min, uint32 flags);
	virtual int txn_recover(DbPreplist * preplist, long count, long * retp, uint32 flags);
	...
	virtual int repmgr_channel(int eid, DbChannel ** channel, uint32 flags);
	...
	virtual int repmgr_site(const char * host, uint port, DbSite ** site, uint32 flags);
	virtual int repmgr_site_by_eid(int eid, DbSite ** site);
	virtual int repmgr_site_list(uint * countp, DB_REPMGR_SITE ** listp);
	...
	// of this class.
	//
	static int _app_dispatch_intercept(DB_ENV * dbenv, DBT * dbt, DB_LSN * lsn, db_recops op);
	...
	static int _isalive_intercept(DB_ENV * dbenv, pid_t pid, db_threadid_t thrid, uint32 flags);
	static int _rep_send_intercept(DB_ENV * dbenv, const DBT * cntrl, const DBT * data, const DB_LSN * lsn, int id,
		uint32 flags);
	static void _stream_error_function(const DB_ENV * dbenv, const char * prefix, const char * message);
	static void _stream_message_function(const DB_ENV * dbenv, const char * message);
	static void _thread_id_intercept(DB_ENV * dbenv, pid_t * pidp, db_threadid_t * thridp);
	static char * _thread_id_string_intercept(DB_ENV * dbenv, pid_t pid, db_threadid_t thrid, char * buf);
	static void _message_dispatch_intercept(DB_ENV * dbenv, DB_CHANNEL * dbchannel, DBT * request,
		uint32 nrequest,
		uint32 cb_flags);
...
private:
	...
	int (* rep_send_callback_)(DbEnv *, const Dbt *, const Dbt *, const DbLsn *, int, uint32);
	void (* thread_id_callback_)(DbEnv *, pid_t *, db_threadid_t *);
	char *(*thread_id_string_callback_)(DbEnv*, pid_t, db_threadid_t,
	                                    char *);
	void (* message_dispatch_callback_)(DbEnv *, DbChannel *, Dbt *, uint32, uint32);
};
...
public:
	db_pgno_t get_pgno() const
	{
		return pgno;
	}
	void set_pgno(db_pgno_t value)
	{
		pgno = value;
	}

	db_indx_t get_indx() const
	{
		return indx;
	}
	void set_indx(db_indx_t value)
	{
		indx = value;
	}
...
public:
	...
	int get(DbTxn * txnid, int32 delta, db_seq_t * retp, uint32 flags);
	int get_cachesize(int32 * sizep);
	int set_cachesize(int32 size);
	int get_flags(uint32 * flagsp);
...
public:
	int close();
	int get_address(const char ** hostp, uint * port);
...
public:
	...
	uint32 get_ulen() const
	{
		return ulen;
	}
	void set_ulen(uint32 value)
	{
		ulen = value;
	}

	// RO: get/put record length.
	uint32 get_dlen() const
	{
		return dlen;
	}
	void set_dlen(uint32 value)
	{
		dlen = value;
	}

	// RO: get/put record offset.
	uint32 get_doff() const
	{
		return doff;
	}
	void set_doff(uint32 value)
	{
		doff = value;
	}
	...
	const DBT * get_const_DBT() const
	{
		return (const DBT *)this;
	}

	static Dbt * get_Dbt(DBT * dbt)
	{
		return (Dbt *)dbt;
	}
...
public:
	...
	DbLockNotGrantedException(const char * prefix, db_lockop_t op, db_lockmode_t mode, const Dbt * obj,
		const DbLock lock,
		int index);
