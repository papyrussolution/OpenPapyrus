...
static int  __env_des_get(ENV*, REGINFO*, REGINFO*, REGION**);
static int  __env_faultmem(ENV*, void *, size_t, int);
static int  __env_sys_attach(ENV*, REGINFO*, REGION *);
...
int __env_attach(ENV * env, uint32 * init_flagsp, int create_ok, int retry_ok)
{
	...
	size_t max, nrw, size;
	long segid;
	uint32 bytes, i, mbytes, nregions;
	uint retry_cnt = 0;
	int majver, minver, patchver, ret;
	char buf[sizeof(DB_REGION_FMT)+20];
	// Initialization 
	DB_ENV * dbenv = env->dbenv;
	const uint32 signature = __env_struct_sig();
...
loop:   
	 ...
	 * we're done.)
	 */
	if((ret = __os_open(env, infop->name, 0, DB_OSO_REGION, 0, &env->lockfhp)) != 0)
		goto err;
	/*
	 * !!!
...
user_map_functions:
	...
	if(renv->majver != DB_VERSION_MAJOR || renv->minver != DB_VERSION_MINOR) {
		if(renv->majver != 0 || renv->minver != 0) {
			__db_errx(env, DB_STR_A("1538", "Program version %d.%d doesn't match environment version %d.%d", "%d %d %d %d"), DB_VERSION_MAJOR, DB_VERSION_MINOR, renv->majver, renv->minver);
	 ...
	 */
	__env_faultmem(env, infop->primary, rp->size, 0);
...
creation:
	...
	if((tregion.max = dbenv->memory_max) == 0) {
		...
		__db_errx(env, DB_STR_A("1542", "Minimum environment memory size %ld is bigger than spcified max %ld.", "%ld %ld"), (ulong)tregion.size, (ulong)tregion.max);
		ret = EINVAL;
		goto err;
	}
	 ...
	 */
	__env_faultmem(env, infop->addr, tregion.size, 1);
...
find_err:       
		__db_errx(env, DB_STR_A("1544", "%s: unable to find environment", "%s"), infop->name);
		SETIFZ(ret, EINVAL);
		goto err;
	}
	infop->rp = rp;
	...
	if(tregion.segid != INVALID_REGION_SEGID) {
		...
		ref.max = tregion.max;
		if((ret = __os_write(env, env->lockfhp, &ref, sizeof(ref), &nrw)) != 0) {
			__db_err(env, ret, DB_STR_A("1545", "%s: unable to write out public environment ID", "%s"), infop->name);
			goto err;
		}
	}
#ifndef HAVE_MUTEX_FCNTL
...
int __env_remove_env(ENV * env)
{
	...
	DB_ENV * dbenv = env->dbenv;
	/*
	 * We do not want to hang on a mutex request, nor do we care about panics.
	 */
	const uint32 flags_orig = F_ISSET(dbenv, DB_ENV_NOLOCKING|DB_ENV_NOPANIC);
...
#endif
		__env_region_detach(env, &reginfo, 1);
	}
...
static void __env_remove_file(ENV * env)
{
	...
	snprintf(buf, sizeof(buf), "%s", DB_REGION_ENV);
	if((ret = __db_appname(env, DB_APP_NONE, buf, NULL, &path)) != 0)
		return;
	...
	*p = saved_char;
	...
	for(lastrm = -1, cnt = fcnt; --cnt >= 0;) {
		...
		/* Remove the file. */
		if(__db_appname(env, DB_APP_NONE, names[cnt], NULL, &path) == 0) {
			/*
			 ...
			 * overwritten.
			 */
			__os_unlink(env, path, 1);
			__os_free(env, path);
		}
	}
	if(lastrm != -1)
		if(__db_appname(env, DB_APP_NONE, names[lastrm], NULL, &path) == 0) {
			__os_unlink(env, path, 1);
			__os_free(env, path);
		}
...
int __env_region_attach(ENV * env, REGINFO * infop, size_t init, size_t max)
{
	...
	snprintf(buf, sizeof(buf), DB_REGION_FMT, infop->id);
	if((ret = __db_appname(env, DB_APP_NONE, buf, NULL, &infop->name)) != 0)
		goto err;
	 ...
	 * reading them.
	 */
	__env_faultmem(env, infop->addr, rp->size, F_ISSET(infop, REGION_CREATE));
	/*
	 * !!!
...
#define OS_VMROUNDOFF(i) {                                              \
	...
	if(rp->size > DB_REGIONSIZE_MAX) {
		__db_errx(env, DB_STR_A("1548", "region size %lu is too large; maximum is %lu", "%lu %lu"), (ulong)rp->size, (ulong)DB_REGIONSIZE_MAX);
		return EINVAL;
	}
	if(rp->max > DB_REGIONSIZE_MAX) {
		__db_errx(env, DB_STR_A("1549", "region max %lu is too large; maximum is %lu", "%lu %lu"), (ulong)rp->max, (ulong)DB_REGIONSIZE_MAX);
		return EINVAL;
	}
#endif
	/*
...
static int __env_des_get(ENV * env, REGINFO * env_infop, REGINFO * infop, REGION ** rpp)
{
	...
	*rpp = NULL;
	...
	for(rp = (REGION *)R_ADDR(env_infop, renv->region_off), i = 0; i < renv->region_cnt; ++i, ++rp) {
		...
		if(infop->type == rp->type && F_ISSET(infop, REGION_JOIN_OK) && (first_type == NULL || first_type->id > rp->id))
			first_type = rp;
		if(rp->id > maxid)
			maxid = rp->id;
	}
