...
#define TIMESTAMP_CHECK(env, ts, renv) do {                             \
		if(renv->op_timestamp != 0 && renv->op_timestamp+DB_REGENV_TIMEOUT < ts) { \
			REP_SYSTEM_LOCK(env);                                   \
...
int __rep_bulk_message(ENV*env, REP_BULK * bulk, REP_THROTTLE * repth, DB_LSN * lsn, const DBT * dbt, uint32 flags)
{
	...
	if(recsize > bulk->len) {
		...
		__rep_send_bulk(env, bulk, flags);
		/*
	...
	while(recsize+*(bulk->offp) > bulk->len) {
		RPRINT(env, (env, DB_VERB_REP_MSGS,
			     ...
			     (ulong)recsize, (ulong)recsize,
			     (ulong)bulk->len, (ulong)bulk->len));
	 ...
	 */
	typemore = (bulk->type == REP_BULK_LOG) ? REP_LOG_MORE : REP_PAGE_MORE;
	if(repth != NULL) {
		if((ret = __rep_send_throttle(env, bulk->eid, repth, REP_THROTTLE_ONLY, flags)) != 0) {
			MUTEX_UNLOCK(env, rep->mtx_clientdb);
			return ret;
		}
		if(repth->type == typemore) {
			VPRINT(env, (env, DB_VERB_REP_MSGS, "bulk_msg: Record %lu (0x%lx) hit throttle limit.", (ulong)recsize, (ulong)recsize));
			MUTEX_UNLOCK(env, rep->mtx_clientdb);
			return ret;
		}
	}
	...
	// Copy the record into the buffer.  Then if we need to, send the buffer.
	//
	...
	b_args.bulkdata = *dbt;
	//
	// If we're the first record, we need to save the first LSN in the bulk structure.
	//
	if(*(bulk->offp) == 0)
		bulk->lsn = *lsn;
	...
	else if((ret = __rep_bulk_marshal(env, &b_args, p, bulk->len, &len)) != 0)
		goto err;
	*(bulk->offp) = (roff_t)(p+len-bulk->addr);
	...
	if(LF_ISSET(REPCTL_PERM)) {
		VPRINT(env, (env, DB_VERB_REP_MSGS, "bulk_msg: Send buffer after copy due to PERM"));
...
int FASTCALL __rep_send_message(ENV * env, int eid, uint32 rtype, DB_LSN * lsnp, const DBT * dbt, uint32 ctlflags, uint32 repflags)
{
	...
	memzero(&cntrl46, sizeof(cntrl46));
	if(lsnp == NULL)
		ZERO_LSN(cntrl.lsn);
	else
		cntrl.lsn = *lsnp;
	...
	if(rep->version == DB_REPVERSION)
		...
		__db_errx(env, DB_STR_A("3503", "rep_send_message: Unknown rep version %lu, my version %lu", "%lu %lu"), (ulong)rep->version, (ulong)DB_REPVERSION);
	...
	if(rep->version <= DB_REPVERSION_45) {
		...
		cntrl46.msg_time.tv_sec = (__time64_t)cntrl.msg_sec;
...
static void __rep_print_logmsg(ENV * env, const DBT * logdbt, DB_LSN * lsnp)
{
	...
	if(first) {
		...
		__dbreg_init_print(env, &dtab);
		__fop_init_print(env, &dtab);
		__ham_init_print(env, &dtab);
		__qam_init_print(env, &dtab);
...
int __rep_new_master(ENV*env, __rep_control_args * cntrl, int eid)
{
	...
	if(change) {
		...
		if((ret = __rep_lockout_msg(env, rep, 1)) != 0)
			goto errlck;
		__memp_set_config(env->dbenv, DB_MEMP_SYNC_INTERRUPT, 1);
		lockout_msg = 1;
		/*
		...
		RPRINT(env, (env, DB_VERB_REP_MISC,
			     ...
			     (ulong)rep->gen, (ulong)cntrl->gen, eid));
		SET_GEN(cntrl->gen);
		rep->mgen = cntrl->gen;
		if((ret = __rep_notify_threads(env, AWAIT_GEN)) != 0)
			goto errlck;
		__rep_write_gen(env, rep, rep->gen);
		if(rep->egen <= rep->gen)
			rep->egen = rep->gen+1;
		rep->master_id = eid;
		...
		RPRINT(env, (env, DB_VERB_REP_MISC,
			     ...
			     (ulong)rep->egen, (ulong)rep->version));
		/*
		...
		__memp_set_config(env->dbenv, DB_MEMP_SYNC_INTERRUPT, 0);
		lockout_msg = 0;
	}
	else
		__rep_elect_done(env, rep);
	...
	if(!F_ISSET(rep, REP_F_DELAY))
		__rep_send_message(env, eid, REP_VERIFY_REQ, &lsn, NULL, 0, DB_REP_ANYWHERE);
	goto newmaster_complete;
...
notfound:
	 ...
	 */
	RPRINT(env, (env, DB_VERB_REP_MISC, "No commit or ckp found.  Truncate log."));
...
void __rep_elect_done(ENV*env, REP * rep)
{
	...
	if(inelect) {
		if(timespecisset(&rep->etime)) {
			...
			rep->stat.st_election_usec = (uint32)(endtime.tv_nsec/NS_PER_US);
#endif
			RPRINT(env, (env, DB_VERB_REP_ELECT, "Election finished in %lu.%09lu sec", (ulong)endtime.tv_sec, (ulong)endtime.tv_nsec));
			timespecclear(&rep->etime);
		}
		rep->egen++;
	}
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Election done; egen %lu", (ulong)rep->egen));
}
...
int __env_rep_enter(ENV*env, int checklock)
{
	...
	for(cnt = 0; FLD_ISSET(rep->lockout_flags, REP_LOCKOUT_API); ) {
		...
		__os_yield(env, 1, 0);
		if(++cnt%60 == 0 &&
		   (ret = __rep_show_progress(env, DB_STR_P("DB_ENV handle"), cnt/60)) != 0)
			return ret;
		REP_SYSTEM_LOCK(env);
	}
	rep->handle_cnt++;
...
#define PAGE_ARGS (ulong)rep->curfile, (ulong)rep->nfiles, (ulong)rep->npages, (ulong)rep->ready_pg
...
#define LOG_LSN_ARGS LSN_ARG(ready_lsn), LSN_ARG(rep->first_lsn), LSN_ARG(rep->last_lsn)
 ...
 #define LOG_ARGS LOG_LSN_ARGS, (ulong)rep->stat.st_log_queued
#else
...
int __op_rep_enter(ENV*env, int local_nowait, int obey_user)
{
	...
	for(cnt = 0; FLD_ISSET(rep->lockout_flags, REP_LOCKOUT_OP); ) {
		...
		__os_yield(env, 5, 0);
		cnt += 5;
		if(++cnt%60 == 0 && (ret = __rep_show_progress(env, "__op_rep_enter", cnt/60)) != 0)
			return ret;
		REP_SYSTEM_LOCK(env);
	}
	rep->op_cnt++;
...
#ifdef DIAGNOSTIC
		if(wait_cnt == 5) {
			RPRINT(env, (env, DB_VERB_REP_MISC, "Waiting for %s (%lu) to complete lockout to %lu", msg, (ulong)*fieldp, (ulong)field_val));
			__db_errx(env, DB_STR_A("3510", "Waiting for %s (%lu) to complete replication lockout", "%s %lu"), msg, (ulong)*fieldp);
		}
		if(++wait_cnt%60 == 0)
			__db_errx(env, DB_STR_A("3511", "Waiting for %s (%lu) to complete replication lockout for %d minutes", "%s %lu %d"), msg, (ulong)*fieldp, wait_cnt/60);
#endif
		REP_SYSTEM_LOCK(env);
	}
	COMPQUIET(msg, 0);
...
int __rep_send_throttle(ENV*env, int eid, REP_THROTTLE * repth, uint32 flags, uint32 ctlflags)
{
	 ...
	 */
snd:    
	if((repth->type == typemore || !LF_ISSET(REP_THROTTLE_ONLY)) &&
	   (__rep_send_message(env, eid, repth->type, &repth->lsn, repth->data_dbt, (REPCTL_RESEND|ctlflags), 0) != 0))
...
int __rep_print_system(ENV * env, uint32 verbose, const char * fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	int ret = __rep_print_int(env, verbose|DB_VERB_REP_SYSTEM, fmt, ap);
	va_end(ap);
	return ret;
}
...
int __rep_print(ENV * env, uint32 verbose, const char * fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	int ret = __rep_print_int(env, verbose, fmt, ap);
	va_end(ap);
	return ret;
}
/*
...
static int __rep_print_int(ENV*env, uint32 verbose, const char * fmt, va_list ap)
{
	...
	uint32 tmp_verbose = env->dbenv->verbose;
	...
	regular_msg = FLD_ISSET(tmp_verbose, verbose|DB_VERB_REPLICATION);
	/*
	...
	__db_msgadd(env, &mb, "[%lu:%lu][%s] %s: ", (ulong)ts.tv_sec, (ulong)ts.tv_nsec/NS_PER_US, env->dbenv->thread_id_string(env->dbenv, pid, tid, buf), s);
...
void __rep_fire_event(ENV*env, uint32 event, void * info)
{
	 ...
	 */
	int ret = __repmgr_handle_event(env, event, info);
...
int __rep_notify_threads(ENV*env, rep_waitreason_t wake_reason)
{
	...
	SH_TAILQ_FOREACH(waiter, &rep->waiters, links, __rep_waiter) {
		...
		else if(wake_reason == goal->why || (goal->why == AWAIT_HISTORY && wake_reason == AWAIT_LSN)) {
			/*
...
int __rep_log_backup(ENV * env, DB_LOGC * logc, DB_LSN * lsn, uint32 match)
{
	...
	while((ret = __logc_get(logc, lsn, &mylog, DB_PREV)) == 0) {
		...
		if((match == REP_REC_COMMIT && rectype == DB___txn_regop) || (match == REP_REC_PERM && (rectype == DB___txn_ckp || rectype == DB___txn_regop)))
			break;
	}
	return ret;
}
...
retry:
	...
	if((ret = __dbc_close(dbc)) == 0 && (ret = __rep_lsn_hist_key_unmarshal(env, &key, key_buf, __REP_LSN_HIST_KEY_SIZE, NULL)) == 0)
		*data_genp = key.gen;
out:
