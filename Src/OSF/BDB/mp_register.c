 ...
 * PUBLIC:     int (*)(DB_ENV *, db_pgno_t, void *, DBT *)));
 */
int __memp_register_pp(DB_ENV * dbenv, int ftype, int (*pgin)__P((DB_ENV*, db_pgno_t, void *, DBT *)), int (*pgout)__P((DB_ENV*, db_pgno_t, void *, DBT *)))
{
	DB_THREAD_INFO * ip;
	int ret;
	ENV * env = dbenv->env;
 ...
 * PUBLIC:     int (*)(DB_ENV *, db_pgno_t, void *, DBT *)));
 */
int __memp_register(ENV * env, int ftype, int (*pgin)__P((DB_ENV*, db_pgno_t, void *, DBT *)), int (*pgout)__P((DB_ENV*, db_pgno_t, void *, DBT *)))
{
	DB_MPREG * mpreg;
	int ret;
