...
int __rep_allreq(ENV*env, __rep_control_args * rp, int eid)
{
	...
	if(ret == 0 && repth.lsn.file != 1 && flags == DB_FIRST) {
		if(F_ISSET(rep, REP_F_CLIENT))
			...
			__rep_send_message(env, eid, REP_VERIFY_FAIL, &repth.lsn, NULL, 0, 0);
		goto err;
	}
	/*
	...
	for(end_flag = 0; ret == 0 && repth.type != REP_LOG_MORE && end_flag == 0; ret = __logc_get(logc, &repth.lsn, &data_dbt, DB_NEXT)) {
		...
		if(repth.lsn.file != oldfilelsn.file) {
			...
			if(rep->version < DB_REPVERSION_47)
				DB_INIT_DBT(newfiledbt, &nf_args.version, sizeof(nf_args.version));
			else {
				if((ret = __rep_newfile_marshal(env, &nf_args, buf, __REP_NEWFILE_SIZE, &len)) != 0)
					goto err;
				DB_INIT_DBT(newfiledbt, buf, len);
			}
			__rep_send_message(env, eid, REP_NEWFILE, &oldfilelsn, &newfiledbt, REPCTL_RESEND, 0);
		}
		/*
		 ...
		 */
		end_flag = (LOG_COMPARE(&repth.lsn, &log_end) >= 0 && (F_ISSET(rep, REP_F_MASTER) || rep->stat.st_startup_complete)) ? REPCTL_LOG_END : 0;
		/*
 ...
 */
int __rep_log(ENV * env, DB_THREAD_INFO * ip, __rep_control_args * rp, DBT * rec, int eid, __time64_t savetime, DB_LSN * ret_lsnp)
{
	DB_LSN last_lsn, lsn;
	LOG * lp;
	int master;
 ...
 */
int __rep_bulk_log(ENV * env, DB_THREAD_INFO * ip, __rep_control_args * rp, DBT * rec, __time64_t savetime, DB_LSN * ret_lsnp)
{
	DB_LSN last_lsn;
	DB_REP * db_rep = env->rep_handle;
	REP * rep = db_rep->region;
 ...
 */
static int __rep_log_split(ENV * env, DB_THREAD_INFO * ip, __rep_control_args * rp, DBT * rec, DB_LSN * ret_lsnp, DB_LSN * last_lsnp)
{
	DBT logrec;
...
int __rep_logreq(ENV * env, __rep_control_args * rp, DBT * rec, int eid)
{
	...
	if((ret = __logc_get(logc, &lsn, &data_dbt, DB_SET)) == 0) {
		...
		if(LOG_COMPARE(&firstlsn, &rp->lsn) > 0) {
			...
			__rep_send_message(env, eid, REP_VERIFY_FAIL, &rp->lsn, NULL, 0, 0);
			ret = 0;
			goto err;
		}
		ret = __rep_chk_newfile(env, logc, rep, rp, eid);
	...
	repth.data_dbt = &data_dbt;
	REP_SYSTEM_UNLOCK(env);
	while(ret == 0 && rec != NULL && rec->size != 0 && repth.type == REP_LOG) {
		if((ret = __logc_get(logc, &repth.lsn, &data_dbt, DB_NEXT)) != 0) {
			/*
...
int __rep_loggap_req(ENV * env, REP * rep, DB_LSN * lsnp, uint32 gapflags)
{
	 ...
	 */
	if(FLD_ISSET(gapflags, (REP_GAP_FORCE|REP_GAP_REREQUEST)) || IS_ZERO_LSN(lp->max_wait_lsn) ||
	   (lsnp != NULL && LOG_COMPARE(lsnp, &lp->max_wait_lsn) == 0)) {
		 ...
		 */
		if(rep->sync_state == SYNC_LOG && IS_ZERO_LSN(lp->max_wait_lsn))
		...
		if(FLD_ISSET(gapflags, REP_GAP_FORCE)) {
			if(LOG_COMPARE(&lp->max_wait_lsn, lsnp) <= 0) {
				if(rep->sync_state == SYNC_LOG) {
					DB_ASSERT(env, LOG_COMPARE(lsnp, &rep->last_lsn) <= 0);
					lp->max_wait_lsn = rep->last_lsn;
				}
				else
		...
		if(rep->version < DB_REPVERSION_47)
			DB_INIT_DBT(max_lsn_dbt, &lp->max_wait_lsn, sizeof(DB_LSN));
		else {
	...
	if((master = rep->master_id) != DB_EID_INVALID) {
		...
		__rep_send_message(env, master, type, &next_lsn, max_lsn_dbtp, ctlflags, flags);
	}
	else
 ...
 * can return it.
 *
 * PUBLIC: int __rep_logready __P((ENV *, REP *, __time64_t, DB_LSN *));
 */
int __rep_logready(ENV * env, REP * rep, __time64_t savetime, DB_LSN * last_lsnp)
{
	int ret;
	...
	if(endlsn.file > rp->lsn.file) {
		...
		if((ret = __logc_get(logc, &endlsn, &data_dbt, DB_SET)) != 0 || (ret = __logc_get(logc, &endlsn, &data_dbt, DB_PREV)) != 0) {
			...
			if((ret = __logc_version(logc, &nf_args.version)) == 0) {
				...
				__rep_send_message(env, eid, REP_NEWFILE, &endlsn, &newfiledbt, REPCTL_RESEND, 0);
			}
		}
	}
	else
