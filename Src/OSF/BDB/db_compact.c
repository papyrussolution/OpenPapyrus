...
#define RESTORE_START do { \
		c_data->compact_pages_free = save_data.compact_pages_free; \
		c_data->compact_levels = save_data.compact_levels;      \
		c_data->compact_truncate = save_data.compact_truncate;  \
		c_data->compact_empty_buckets = save_data.compact_empty_buckets; \
		ret = __db_retcopy(env, &current, save_start.data, save_start.size, &current.data, &current.ulen); \
	} while(0)
...
int __db_compact_int(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, DBT * start, DBT * stop, DB_COMPACT * c_data, uint32 flags, DBT * end)
{
	...
	if(txn_local && (ret = __txn_begin(env, ip, txn_orig, &txn, 0)) != 0)
		goto err;
	if((ret = __db_free_truncate(dbp, ip, txn, flags, c_data, &list, &nelems, &last_pgno)) != 0) {
...
no_free:
	...
	SETIFZ(c_data->compact_pages, DB_MAX_PAGES);
	do {
		deadlock = 0;
		SAVE_START;
		if(ret != 0)
		...
		if(txn_local) {
			if((ret = __txn_begin(env, ip, txn_orig, &txn, 0)) != 0)
				break;
			if(c_data->compact_timeout != 0 && (ret = __txn_set_timeout(txn, c_data->compact_timeout, DB_SET_LOCK_TIMEOUT)) != 0)
				goto err;
		}
...
#ifdef HAVE_HASH
		if(dbp->type == DB_HASH)
			ret = __ham_compact_int(dbc, &current, stop, factor, c_data, &isdone, flags);
		else
#endif
		ret = __bam_compact_int(dbc, &current, stop, factor, &span, c_data, &isdone);
		...
		if(ret == DB_LOCK_NOTGRANTED) {
			ret = (txn_local || retry++ < 5) ? 0 : DB_LOCK_DEADLOCK;
		}
		else
			retry = 0;
...
err:
	...
	if(LF_ISSET(DB_FREE_SPACE)) {
		DBMETA * meta;
		db_pgno_t pgno = PGNO_BASE_MD;
		isdone = 1;
		if(ret == 0 && !LF_ISSET(DB_FREELIST_ONLY) && __memp_fget(dbp->mpf, &pgno, ip, txn, 0, &meta) == 0) {
		...
		if(dbp->type == DB_HASH) {
			...
			if(!isdone || c_data->compact_empty_buckets != 0)
				ret = __ham_compact_hash(dbp, ip, txn_orig, c_data);
			c_data->compact_empty_buckets += empty_buckets;
		}
		else
 #endif
...
static int __db_free_freelist(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn)
{
	...
	if(IS_DB_AUTO_COMMIT(dbp, txn)) {
		...
		if(__txn_begin(dbp->env, ip, txn, &txn, 0) == 0) {
			__lock_set_timeout(dbp->env, txn->locker, 0, DB_SET_TXN_TIMEOUT);
			__lock_set_timeout(dbp->env, txn->locker, 0, DB_SET_LOCK_TIMEOUT);
			auto_commit = 1;
		}
		/* Get a cursor so we can call __db_lget. */
		if((ret = __db_cursor(dbp, ip, txn, &dbc, 0)) != 0)
			return ret;
		if((ret = __db_lget(dbc, 0, PGNO_BASE_MD, DB_LOCK_WRITE, 0, &lock)) != 0)
			goto err;
	}
 ...
 */
int __db_exchange_page(DBC * dbc, PAGE ** pgp, PAGE * opg, db_pgno_t newpgno, int flags)
{
	BTREE_CURSOR * cp;
	DB * dbp;
	DBT data, * dp, hdr;
	...
	*pgp = newpage;
	...
	if(DBC_LOGGING(dbc)) {
		if((ret = __db_pgno_log(dbp, dbc->txn, &LSN(epg->page), 0, PGNO(epg->page), &LSN(epg->page), (uint32)epg->indx, *pgnop, PGNO(newpage))) != 0)
			return ret;
	}
	else
	...
	*pgnop = PGNO(newpage);
	...
	return 0;
err:
	__memp_fput(dbp->mpf, dbc->thread_info, newpage, dbc->priority);
	__TLPUT(dbc, lock);
	return ret;
}
/*
 ...
 */
int __db_truncate_overflow(DBC * dbc, db_pgno_t pgno, PAGE ** ppg, DB_COMPACT * c_data)
{
	DB * dbp;
	DB_LOCK lock;
	PAGE * page;
	...
	LOCK_INIT(lock);
	have_lock = ppg == NULL;
	if((ret = __memp_fget(dbp->mpf, &pgno, dbc->thread_info, dbc->txn, 0, &page)) != 0)
		return ret;
	while((pgno = NEXT_PGNO(page)) != PGNO_INVALID) {
		if((ret = __memp_fput(dbp->mpf, dbc->thread_info, page, dbc->priority)) != 0)
			return ret;
		if((ret = __memp_fget(dbp->mpf, &pgno, dbc->thread_info, dbc->txn, 0, &page)) != 0)
			return ret;
		...
		if((ret = __db_exchange_page(dbc, &page, NULL, PGNO_INVALID, DB_EXCH_FREE)) != 0)
			break;
	}
err:
 ...
 * PUBLIC:      PAGE *, uint32, db_pgno_t *, uint32));
 */
