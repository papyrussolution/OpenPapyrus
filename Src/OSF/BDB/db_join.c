...
int __db_join(DB * primary, DBC ** curslist, DBC ** dbcp, uint32 flags)
{
	 ...
	 * function beware!
	 */
	if((ret = __dbc_dup(jc->j_curslist[0], jc->j_workcurs, DB_POSITION)) != 0)
		goto err;
...
static int __db_join_get(DBC * dbc, DBT * key_arg, DBT * data_arg, uint32 flags)
{
	...
	F_CLR(jc, JOIN_RETRY);
retry:
	ret = __dbc_get(jc->j_workcurs[0], &jc->j_key, key_n, opmods|(jc->j_exhausted[0] ? DB_NEXT_DUP : DB_CURRENT));
...
retry2:
		...
		if(ret == DB_BUFFER_SMALL) {
			...
			if((ret = __os_realloc(env, jc->j_key.ulen, &jc->j_key.data)) != 0) {
mem_err:
				__db_errx(env, DB_STR_A("0517", "Allocation failed for join key, len = %lu", "%lu"), (ulong)jc->j_key.ulen);
				goto err;
			}
			goto retry2;
		}
		 ...
		 */
		if(SORTED_SET(jc, i) && jc->j_fdupcurs[i] == NULL && (ret = __dbc_dup(cp, &jc->j_fdupcurs[i], DB_POSITION)) != 0)
			goto err;
	}
err:
	if(ret != 0)
...
int __db_join_close(DBC * dbc)
{
	DB_THREAD_INFO * ip;
	uint32 i;
	JOIN_CURSOR * jc = (JOIN_CURSOR *)dbc->internal;
	DB * dbp = dbc->dbp;
	ENV * env = dbp->env;
	int ret = 0;
	int t_ret = 0;
...
static int __db_join_cmp(const void * a, const void * b)
{
	...
	DBC * dbca = *((DBC * const *)a);
	DBC * dbcb = *((DBC * const *)b);
