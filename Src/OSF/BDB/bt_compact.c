...
static int __bam_csearch(DBC*, DBT*, uint32, int);
static int __bam_lock_tree(DBC*, EPG*, EPG*csp, uint32, uint32);
static int __bam_lock_subtree(DBC*, PAGE*, uint32, uint32);
static int __bam_merge(DBC*, DBC*,  uint32, DBT*, DB_COMPACT*, int *);
static int __bam_merge_internal(DBC*, DBC*, int, DB_COMPACT*, int *);
static int __bam_merge_pages(DBC*, DBC*, DB_COMPACT *);
static int __bam_merge_records(DBC*, DBC*,  uint32, DB_COMPACT *);
static int __bam_truncate_internal_overflow(DBC*, PAGE*, DB_COMPACT *);
static int __bam_truncate_root_page(DBC*, PAGE*, uint32, DB_COMPACT *);
 ...
 */
int __bam_compact_int(DBC * dbc, DBT * start, DBT * stop, uint32 factor, int * spanp, DB_COMPACT * c_data, int * donep)
{
	BTREE_CURSOR * cp, * ncp;
	DB * dbp;
	DBC * ndbc = 0;
 ...
 #define PTRACE(dbc, location, p, start, f) do {                         \
		char __buf[32];                                         \
		snprintf(__buf, sizeof(__buf), "pgno: %lu", (ulong)p);             \
...
next:   /*
	...
	if((nentry = NUM_ENT(pg)) != 0) {
		...
		if(dbc->dbtype == DB_RECNO) {
			if((ret = __db_retcopy(dbp->env, start, &cp->recno, sizeof(cp->recno), &start->data, &start->ulen)) != 0)
				goto err;
		}
		else if(((next_p == 1 && npgno == PGNO_INVALID) || start->size == 0) && (ret = __db_ret(dbc, pg, 0, start, &start->data, &start->ulen)) != 0)
			goto err;
		next_p = 0;
		/*
		 ...
		 * page.
		 */
		if(npgno == PGNO_INVALID && (!check_trunc || PGNO(pg) <= c_data->compact_truncate || PGNO(pg) == BAM_ROOT_PGNO(dbc))) {
...
retry:
	...
	if(*spanp) {
		...
		if(check_trunc && PGNO(pg) > c_data->compact_truncate) {
			...
			if((ret = __db_exchange_page(dbc, &cp->csp->page, ncp->csp->page, PGNO_INVALID, DB_EXCH_DEFAULT)) != 0)
				goto err1;
			if((ret = __TLPUT(dbc, prev_lock)) != 0)
				goto err1;
			LOCK_INIT(prev_lock);
		...
		*spanp = 0;
		 ...
		 */
		if(P_FREESPACE(dbp, pg) <= factor || cp->csp[-1].indx != NUM_ENT(cp->csp[-1].page)-1)
			goto next_page;
		/*
		...
		for(epg = cp->sp; epg != cp->csp; epg++) {
			...
			if((ret = __bam_merge_internal(dbc, ndbc, LEVEL(epg->page), c_data, &merged)) != 0)
				break;
			if(merged)
				break;
		}
...
next_page:
	...
	if((ret = __bam_stkrel(dbc, STK_PGONLY)) != 0)
		goto err;
	if(npgno != PGNO_INVALID && (ret = __db_lget(dbc, 0, npgno, DB_LOCK_READ, 0, &next_lock)) != 0)
		goto err;
 ...
 */
static int __bam_merge(DBC * dbc, DBC * ndbc, uint32 factor, DBT * stop, DB_COMPACT * c_data, int * donep)
{
	BTREE_CURSOR * cp, * ncp;
	DB * dbp;
	PAGE * pg, * npg;
	 ...
	 */
	if((dbc->dbtype == DB_BTREE && ncp->csp[-1].indx == 0 && ncp->csp[-1].entries != 1) ||
	   (int)(P_FREESPACE(dbp, pg)-((dbp->pgsize-P_OVERHEAD(dbp))-P_FREESPACE(dbp, npg))) < (int)factor)
...
static int __bam_merge_records(DBC * dbc, DBC * ndbc, uint32 factor, DB_COMPACT * c_data)
{
	BINTERNAL * bi;
	BKEYDATA * bk, * tmp_bk;
	BTREE * t;
	...
	size_t(*func)(DB*, const DBT*, const DBT *);
	...
	if(c_data->compact_truncate != PGNO_INVALID && PGNO(ncp->csp->page) > c_data->compact_truncate) {
		/* Get a fresh low numbered page. */
		if((ret = __db_exchange_page(ndbc, &ncp->csp->page, pg, PGNO_INVALID, DB_EXCH_DEFAULT)) != 0)
			goto err;
	}
	npg = ncp->csp->page;
	nent = NUM_ENT(npg);
	...
	for(indx = 0; indx < nent; indx += adj) {
		 ...
		 */
		n_ok = adj;
		if(TYPE(pg) == P_LBTREE && indx < nent-adj && ninp[indx] == ninp[indx+adj]) {
			do {
				/* Size of index for key reference. */
				size += sizeof(db_indx_t);
				n_ok++;
				/* Size of data item. */
				size += BITEM_PSIZE(GET_BKEYDATA(dbp, npg, indx+n_ok));
				n_ok++;
	 ...
	 */
	if(indx == nent) {
		if(ncp->csp[-1].indx != 0 || ncp->csp[-1].entries == 1 || BINTERNAL_PSIZE(GET_BINTERNAL(dbp, ncp->csp[-1].page, 1)->len) <= pfree)
			return __bam_merge_pages(dbc, ndbc, c_data);
		indx -= adj;
	}
	bk = GET_BKEYDATA(dbp, npg, indx);
...
no_check:
	...
	do {
		...
		if(is_dup && !first_dup && (pind%2) == 0) {
			/* Duplicate key. */
			if((ret = __bam_adjindx(dbc, pg, pind, pind-P_INDX, 1)) != 0)
				goto err;
			if(!next_dup)
				is_dup = 0;
		}
				...
				data.size = bk->len;
				data.data = bk->data;
				if((ret = __db_pitem(dbc, pg, pind, BKEYDATA_SIZE(bk->len), &hdr, &data)) != 0)
					goto err;
				break;
			    case B_OVERFLOW:
			    case B_DUPLICATE:
				...
				break;
			    default:
				__db_errx(env, DB_STR_A("1022", "Unknown record format, page %lu, indx 0", "%lu"), (ulong)PGNO(pg));
				ret = EINVAL;
				goto err;
			}
		pind++;
		if(next_dup && (NUM_ENT(npg)%2) == 0) {
			...
			if((ret = __db_ditem(ndbc, npg, 0, BITEM_SIZE(bk))) != 0)
				goto err;
		}
		adjust++;
	} while(--indx != 0);
...
free_page:
	 ...
	 */
	if((ret = __db_relink(dbc, ncp->csp->page, cp->csp->page, PGNO_INVALID)) != 0)
		goto err;
	/* Drop the duplicate reference to the sub tree root. */
	cp->sp->page = NULL;
	LOCK_INIT(cp->sp->lock);
	if(PGNO(ncp->sp->page) == BAM_ROOT_PGNO(ndbc) && NUM_ENT(ncp->sp->page) == 2) {
	...
	if((ret = __bam_dpages(ndbc, 0, ndbc->dbtype == DB_RECNO ? 0 : BTD_UPDATE)) != 0)
		goto err;
	npg = NULL;
	c_data->compact_pages_free++;
	...
	if(level != 0) {
		...
		if(level == LEVEL(npg))
			level = 0;
		if((ret = __memp_fput(dbmp, dbc->thread_info, npg, dbc->priority)) != 0)
			goto err;
 ...
 */
static int __bam_merge_internal(DBC * dbc, DBC * ndbc, int level, DB_COMPACT * c_data, int * merged)
{
...
fits:   memzero(&bi, sizeof(bi));
	 ...
	 */
	do {
		if((ret = __memp_fput(dbmp, dbc->thread_info, nsave_csp->page, dbc->priority)) != 0)
			goto err;
		nsave_csp->page = NULL;
	...
	if(NUM_ENT(npg) == 0) {
		...
		if(PGNO(ncp->sp->page) == BAM_ROOT_PGNO(ndbc) && NUM_ENT(ncp->sp->page) == 2) {
			...
			level = 0;
		COMPACT_TRUNCATE(c_data);
		ret = __bam_dpages(ndbc, 0, ndbc->dbtype == DB_RECNO ? BTD_RELINK : BTD_UPDATE|BTD_RELINK);
		c_data->compact_pages_free++;
		if(ret == 0 && level != 0) {
			pgno = PGNO_INVALID;
			BAM_GET_ROOT(ndbc, pgno, npg, 0, DB_LOCK_READ, root_lock, ret);
		...
		if(NUM_ENT(npg) != 0 && c_data->compact_truncate != PGNO_INVALID && PGNO(npg) > c_data->compact_truncate && ncp->csp != ncp->sp) {
			if((ret = __db_exchange_page(ndbc, &ncp->csp->page, pg, PGNO_INVALID, DB_EXCH_DEFAULT)) != 0)
				goto err;
		}
		if(c_data->compact_truncate != PGNO_INVALID && PGNO(pg) > c_data->compact_truncate && cp->csp != cp->sp) {
			if((ret = __db_exchange_page(dbc, &cp->csp->page, ncp->csp->page, PGNO_INVALID, DB_EXCH_DEFAULT)) != 0)
				goto err;
		}
	}
...
static int __bam_compact_dups(DBC * dbc, PAGE ** ppg, uint32 factor, int have_lock, DB_COMPACT * c_data, int * donep)
{
	...
	for(i = 0; i <  NUM_ENT(*ppg); i++) {
		...
		if(bo->pgno > c_data->compact_truncate) {
			...
			if((ret = __bam_truncate_root_page(dbc, *ppg, i, c_data)) != 0)
				goto err;
			/* Just in case it should move.  Could it? */
			bo = GET_BOVERFLOW(dbp, *ppg, i);
		}
		if(B_TYPE(bo->type) == B_OVERFLOW) {
			if((ret = __db_truncate_overflow(dbc, bo->pgno, have_lock ? NULL : ppg, c_data)) != 0)
				goto err;
			(*donep)++;
			continue;
		}
		if((ret = __bam_compact_opd(dbc, bo->pgno, have_lock ? NULL : ppg, factor, c_data, donep)) != 0)
			goto err;
	}
err:
	return ret;
 ...
 */
int __bam_compact_opd(DBC * dbc, db_pgno_t root_pgno, PAGE ** ppg, uint32 factor, DB_COMPACT * c_data, int * donep)
{
	BTREE_CURSOR * cp;
	DBC * opd;
	DBT start;
	 ...
	 */
	if((ret = __memp_fget(dbmp, &root_pgno, dbc->thread_info, dbc->txn, 0, &dpg)) != 0)
		goto err;
	level = dpg->level;
	if((ret = __memp_fput(dbmp, dbc->thread_info, dpg, dbc->priority)) != 0)
		goto err;
	if(level == LEAFLEVEL)
		goto done;
 ...
 */
static int __bam_truncate_internal_overflow(DBC * dbc, PAGE * page, DB_COMPACT * c_data)
{
	BINTERNAL * bi;
	BOVERFLOW * bo;
	db_indx_t indx;
	...
	for(indx = 0; indx < NUM_ENT(page); indx++) {
		...
		if(B_TYPE(bi->type) != B_OVERFLOW)
			continue;
		bo = (BOVERFLOW *)(bi->data);
		if(bo->pgno > c_data->compact_truncate && (ret = __bam_truncate_root_page(dbc, page, indx, c_data)) != 0)
			break;
		if((ret = __db_truncate_overflow(dbc, bo->pgno, NULL, c_data)) != 0)
			break;
	}
	return ret;
}
	...
	*isdone = 0;
	...
	if(dbc->dbtype == DB_RECNO) {
		...
		*isdone = cp->recno > recno;
		...
		if((ret = __bam_cmp(dbc, stop, pg, 0, t->bt_compare, &cmp)) != 0)
			return ret;
		*isdone = cmp <= 0;
	}
	return 0;
...
static int __bam_lock_tree(DBC * dbc, EPG * sp, EPG * csp, uint32 start, uint32 stop)
{
	 ...
	 * leaf.
	 */
	if(start == 0 && sp+1 != csp && pgno == PGNO(cpage) && (ret = __bam_lock_tree(dbc, sp+1, csp, 0, NUM_ENT(cpage))) != 0)
		return ret;
	/*
...
static int __bam_lock_subtree(DBC * dbc, PAGE * page, uint32 indx, uint32 stop)
{
	...
	for(; indx < stop; indx++) {
		...
		if(LEVEL(page)-1 == LEAFLEVEL) {
			...
			if((ret = __memp_fget(dbp->mpf, &pgno, dbc->thread_info, dbc->txn, 0, &cpage)) != 0)
				return ret;
...
static int __bam_savekey(DBC * dbc, int next, DBT * start)
{
	...
	if(dbc->dbtype == DB_RECNO) {
		...
		return __db_retcopy(env, start, &cp->recno, sizeof(cp->recno), &start->data, &start->ulen);
	...
	while(len == 0) {
		...
		pgno = bi->pgno;
		level = LEVEL(pg);
		if(pg != cp->csp->page && (ret = __memp_fput(dbp->mpf, dbc->thread_info, pg, dbc->priority)) != 0) {
			pg = NULL;
			goto err;
		}
		pg = NULL;
...
no_key:
	...
	if(B_TYPE(bi->type) == B_OVERFLOW) {
		bo = (BOVERFLOW *)(data);
		ret = __db_goff(dbc, start, bo->tlen, bo->pgno, &start->data, &start->ulen);
	}
	else
 ...
 */
int __bam_truncate_ipages(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, DB_COMPACT * c_data)
{
	BTMETA * meta;
	BTREE * bt;
	BTREE_CURSOR * cp;
...
again:
	if(F_ISSET(dbp, DB_AM_SUBDB) && (bt->bt_root > c_data->compact_truncate || bt->bt_meta > c_data->compact_truncate)) {
		...
		if(bt->bt_root > c_data->compact_truncate) {
			...
			if((ret = __db_exchange_page(dbc, &root, NULL, PGNO_INVALID, DB_EXCH_FREE)) != 0)
				goto err;
