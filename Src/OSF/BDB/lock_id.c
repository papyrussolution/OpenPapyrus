...
int __lock_id_free_pp(DB_ENV * dbenv, uint32 id)
{
	...
	region = (DB_LOCKREGION *)lt->reginfo.primary;
	LOCK_LOCKERS(env, region);
	if((ret = __lock_getlocker_int(env->lk_handle, id, 0, &sh_locker)) == 0) {
		if(sh_locker != NULL)
			ret = __lock_freelocker_int(lt, region, sh_locker, 1);
		else {
			__db_errx(env, DB_STR_A("2045", "Unknown locker id: %lx", "%lx"), (ulong)id);
			ret = EINVAL;
		}
	}
	UNLOCK_LOCKERS(env, region);
...
int __lock_id_free(ENV * env, DB_LOCKER * sh_locker)
{
	...
	if(sh_locker->nlocks != 0) {
		__db_errx(env, DB_STR("2046", "Locker still has locks"));
		ret = EINVAL;
		goto err;
	}
	LOCK_LOCKERS(env, region);
...
int __lock_getlocker_int(DB_LOCKTAB * lt, uint32 locker, int create, DB_LOCKER ** retp)
{
	DB_LOCKER * sh_locker;
	DB_LOCKREGION * region;
	DB_THREAD_INFO * ip;
	...
	if(sh_locker == NULL && create) {
		...
		if((ret = __mutex_alloc(env, MTX_LOGICAL_LOCK, DB_MUTEX_LOGICAL_LOCK|DB_MUTEX_SELF_BLOCK, &mutex)) != 0)
			return ret;
		else
		...
		if((sh_locker = SH_TAILQ_FIRST(&region->free_lockers, __db_locker)) == NULL) {
			...
			while(__env_alloc(&lt->reginfo, nlockers*sizeof(struct __db_locker), &sh_locker) != 0)
				if((nlockers>>1) == 0)
					break;
...
static int __lock_freelocker_int(DB_LOCKTAB * lt, DB_LOCKREGION * region, DB_LOCKER * sh_locker, int reallyfree)
{
	...
	if(SH_LIST_FIRST(&sh_locker->heldby, __db_lock) != NULL) {
		__db_errx(env, DB_STR("2047", "Freeing locker with locks"));
		return EINVAL;
	}
