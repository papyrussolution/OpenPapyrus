...
static int __bam_ca_undodup_func(DBC * dbc, DBC * my_dbc, uint32 * countp, db_pgno_t fpgno, uint32 fi, void * vargs)
{
	 ...
	 */
	if(orig_cp->pgno != fpgno || orig_cp->indx != args->first ||
	   orig_cp->opd == NULL || ((BTREE_CURSOR *)orig_cp->opd->internal)->indx != args->ti || MVCC_SKIP_CURADJ(dbc, fpgno))
		return 0;
	MUTEX_UNLOCK(dbp->env, dbp->mutex);
...
static int __bam_ca_undosplit_func(DBC * dbc, DBC * my_dbc, uint32 * foundp, db_pgno_t frompgno,
	uint32 split_indx, void * vargs)
{
	DBC_INTERNAL * cp;
	struct __bam_ca_split_args * args;
	COMPQUIET(my_dbc, 0);
