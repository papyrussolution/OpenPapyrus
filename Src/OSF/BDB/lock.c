...
up_done:
			...
			if(sh_locker == NULL)
				break;
			SH_LIST_FOREACH(lp, &sh_locker->heldby, locker_links, __db_lock) __lock_printlock(lt, NULL, lp, 1);
			break;
#endif
		    default:
...
int __lock_get_internal(DB_LOCKTAB * lt, DB_LOCKER * sh_locker, uint32 flags, const DBT * obj, db_lockmode_t lock_mode, db_timeout_t timeout, DB_LOCK * lock)
{
	...
	if(lock_mode >= (db_lockmode_t)region->nmodes) {
		__db_errx(env, DB_STR_A("2037", "DB_ENV->lock_get: invalid lock mode %lu", "%lu"), (ulong)lock_mode);
		return EINVAL;
	}
again:
	if(obj == NULL) {
		...
		ndx = lock->ndx;
		if((ret = __lock_getobj(lt, obj, lock->ndx, !LF_ISSET(DB_LOCK_CHECK), &sh_obj)) != 0)
			goto err;
#ifdef DIAGNOSTIC
		if(sh_obj == NULL) {
			ret = ENOENT;
		...
		if(LF_ISSET(DB_LOCK_UPGRADE)) {
			...
			DB_ASSERT(env, lock->gen == lp->gen);
			DB_ASSERT(env, SH_OFF_TO_PTR(lp, lp->obj, DB_LOCKOBJ) == sh_obj);
		}
#endif
	}
#ifdef HAVE_STATISTICS
	if(LF_ISSET(DB_LOCK_UPGRADE))
		STAT_INC_VERB(env, lock, upgrade, lt->obj_stat[ndx].st_nupgrade, (DBT *)obj, sh_locker->id);
	else if(!LF_ISSET(DB_LOCK_SWITCH|DB_LOCK_CHECK))
		STAT_INC_VERB(env, lock, request, lt->obj_stat[ndx].st_nrequests, (DBT *)obj, sh_locker->id);
#endif
	...
	switch(action) {
		 ...
		 */
		if(++lt->obj_stat[ndx].st_nlocks > lt->obj_stat[ndx].st_maxnlocks)
			lt->obj_stat[ndx].st_maxnlocks = lt->obj_stat[ndx].st_nlocks;
		if(++lt->part_array[part_id].part_stat.st_nlocks > lt->part_array[part_id].part_stat.st_maxnlocks)
			lt->part_array[part_id].part_stat.st_maxnlocks = lt->part_array[part_id].part_stat.st_nlocks;
#endif
		...
		no_dd = sh_locker->master_locker == INVALID_ROFF &&
			SH_LIST_FIRST(&sh_locker->child_locker, __db_locker) == NULL && SH_LIST_FIRST(&sh_locker->heldby, __db_lock) == NULL;
...
upgrade:
	...
	switch(action) {
		 ...
		 */
		if(__clock_expired(env, &sh_locker->lk_expire, &sh_locker->tx_expire)) {
		...
		if(!LF_ISSET(DB_LOCK_SET_TIMEOUT)) {
			timeout = (F_ISSET(sh_locker, DB_LOCKER_TIMEOUT)) ? sh_locker->lk_timeout : region->lk_timeout;
		}
		/*
		...
		if(timeout != 0)
			__clock_set_expires(env, &sh_locker->lk_expire, timeout);
		else
			timespecclear(&sh_locker->lk_expire);
		if(timespecisset(&sh_locker->tx_expire) && (timeout == 0 || __clock_expired(env, &sh_locker->lk_expire, &sh_locker->tx_expire)))
			sh_locker->lk_expire = sh_locker->tx_expire;
		if(timespecisset(&sh_locker->lk_expire) && (!timespecisset(&region->next_timeout) || timespeccmp(&region->next_timeout, &sh_locker->lk_expire, >)))
...
in_abort:
		...
		if(LF_ISSET(DB_LOCK_SWITCH) && (ret = __lock_put_nolock(env, lock, &ihold, 0)) != 0) {
...
expired:
			...
			if(timespeccmp(&sh_locker->lk_expire, &sh_locker->tx_expire, ==))
				STAT_INC(env, lock, txntimeout, lt->obj_stat[ndx].st_ntxntimeouts, (DBT *)obj);
			else
				STAT_INC(env, lock, locktimeout, lt->obj_stat[ndx].st_nlocktimeouts, (DBT *)obj);
#endif
			ret = DB_LOCK_NOTGRANTED;
			timespecclear(&sh_locker->lk_expire);
			goto err;
		    ...
		    default:
			__db_errx(env, DB_STR_A("2038", "Unexpected lock status: %d", "%d"), (int)newl->status);
...
static int __lock_put_internal(DB_LOCKTAB * lt, struct __db_lock * lockp, uint32 obj_ndx, uint32 flags)
{
	...
	if(!OBJ_LINKS_VALID(lockp)) {
		 ...
		 * free list.
		 */
		__lock_freelock(lt, lockp, NULL, DB_LOCK_FREE);
		return 0;
	}
#ifdef HAVE_STATISTICS
	...
	if(!LF_ISSET(DB_LOCK_DOALL) && lockp->refcount > 1) {
		lockp->refcount--;
		PERFMON2(env, lock, put_reduce_count, &(SH_OFF_TO_PTR(lockp, lockp->obj, DB_LOCKOBJ))->lockobj, flags);
		return 0;
	}
	...
	if(lockp->status != DB_LSTAT_HELD && lockp->status != DB_LSTAT_PENDING) {
		DB_ASSERT(env, lockp != SH_TAILQ_FIRST(&sh_obj->holders, __db_lock));
		if((ret = __lock_remove_waiter(lt, sh_obj, lockp, DB_LSTAT_ABORTED)) != 0)
			return ret;
	}
	else {
		DB_ASSERT(env, lockp != SH_TAILQ_FIRST(&sh_obj->waiters, __db_lock));
		SH_TAILQ_REMOVE(&sh_obj->holders, lockp, links, __db_lock);
		lockp->links.stqe_prev = -1;
	}
	if(LF_ISSET(DB_LOCK_NOPROMOTE))
		state_changed = 0;
	else if((ret = __lock_promote(lt, sh_obj, &state_changed, flags)) != 0)
		return ret;
	/* Check if object should be reclaimed. */
	if(SH_TAILQ_FIRST(&sh_obj->holders, __db_lock) == NULL && SH_TAILQ_FIRST(&sh_obj->waiters, __db_lock) == NULL) {
...
int __lock_locker_same_family(ENV * env, DB_LOCKER * locker1, DB_LOCKER * locker2, int * retp)
{
	 ...
	 * no parents.
	 */
	*retp = (locker1 == NULL) ? 0 : __lock_same_family(lt, locker1, locker2);
	return 0;
}
/*
...
static int __lock_inherit_locks(DB_LOCKTAB * lt, DB_LOCKER * sh_locker, uint32 flags)
{
	...
	for(lp = SH_LIST_FIRST(&sh_locker->heldby, __db_lock); lp != NULL; lp = SH_LIST_FIRST(&sh_locker->heldby, __db_lock)) {
		...
		if(hlp != NULL) {
			...
			__lock_freelock(lt, lp, sh_locker, DB_LOCK_FREE);
		}
		else {
...
{
	DB_LOCKREGION * region = (DB_LOCKREGION *)lt->reginfo.primary;
	int do_wakeup = lockp->status == DB_LSTAT_WAITING;
