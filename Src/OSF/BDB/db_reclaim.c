 ...
 */
int __db_traverse_big(DBC * dbc, db_pgno_t pgno, int (*callback)__P((DBC*, PAGE*, void *, int *)), void * cookie)
{
	PAGE * p;
	int did_put, ret;
	...
	do {
		...
		if(callback == __db_truncate_callback && OV_REF(p) != 1)
			pgno = PGNO_INVALID;
		if((ret = callback(dbc, p, cookie, &did_put)) == 0 && !did_put)
			ret = __memp_fput(mpf, dbc->thread_info, p, dbc->priority);
...
{
	DBT ddbt, ldbt;
	db_indx_t indx, len, off, tlen;
	uint8 * hk, type;
	int ret;
	db_indx_t top = NUM_ENT(p);
	DB * dbp = dbc->dbp;
	DB_MPOOLFILE * mpf = dbp->mpf;
	uint32 * countp = (uint32 *)cookie;
	*putp = 1;
	switch(TYPE(p)) {
	    case P_LBTREE:
...
reinit:
			...
			*putp = 0;
			if(DBC_LOGGING(dbc)) {
				...
				if((ret = __db_pg_init_log(dbp, dbc->txn, &LSN(p), 0, p->pgno, &ldbt, &ddbt)) != 0)
					return ret;
			}
			else
				LSN_NOT_LOGGED(LSN(p));
			P_INIT(p, dbp->pgsize, PGNO(p), PGNO_INVALID, PGNO_INVALID, type == P_HASH ? 0 : 1, type);
		}
		break;
	    default:
		return __db_pgfmt(dbp->env, p->pgno);
	...
	if(*putp == 1) {
		...
		if((ret = __memp_fput(mpf, dbc->thread_info, p, dbc->priority)) != 0)
			return ret;
		*putp = 1;
	}
	return 0;
