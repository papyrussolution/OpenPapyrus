...
int __heap_truncate(DBC * dbc, uint32 * countp)
{
	...
	pgno = PGNO_BASE_MD;
	if((ret = __db_lget(dbc, LCK_ALWAYS, pgno, DB_LOCK_WRITE, 0, &meta_lock)) != 0)
		return ret;
	...
	for(;; ) {
		...
		if((ret = __db_lget(dbc, LCK_COUPLE, pgno, DB_LOCK_WRITE, 0, &lock)) != 0)
			break;
		if((ret = __memp_fget(mpf, &pgno, dbc->thread_info, dbc->txn, DB_MPOOL_DIRTY, &pg)) != 0) {
		...
		if((ret = __memp_fput(mpf, dbc->thread_info, pg, dbc->priority)) != 0)
			break;
		if((ret = __memp_fget(mpf, &pgno, dbc->thread_info, dbc->txn, DB_MPOOL_FREE, &pg)) != 0)
			break;
	}
