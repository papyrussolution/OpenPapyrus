...
static int __ham_stat_callback(DBC*, PAGE*, void *, int *);
...
int __ham_stat(DBC*dbc, void * spp, uint32 flags)
{
	...
	sp->hash_nkeys = 0;
	sp->hash_ndata = 0;
	if((ret = __ham_traverse(dbc, DB_LOCK_READ, __ham_stat_callback, sp, 0)) != 0)
		goto err;
	if(!F_ISSET(dbp, DB_AM_RDONLY)) {
		 ...
		 * update in place.
		 */
		if((ret = __ham_dirty_meta(dbc, (dbc->txn == NULL) ? DB_MPOOL_EDIT : 0)) != 0)
			goto err;
...
int __ham_stat_print(DBC * dbc, uint32 flags)
{
	...
	__db_msg(env, "%lx\tHash magic number", (ulong)sp->hash_magic);
	__db_msg(env, "%lu\tHash version number", (ulong)sp->hash_version);
	...
	__db_dl(env, "Number of pages in the database", (ulong)sp->hash_pagecnt);
	__db_dl(env, "Underlying database page size", (ulong)sp->hash_pagesize);
	__db_dl(env, "Specified fill factor", (ulong)sp->hash_ffactor);
	__db_dl(env, "Number of keys in the database", (ulong)sp->hash_nkeys);
	__db_dl(env, "Number of data items in the database", (ulong)sp->hash_ndata);
	__db_dl(env, "Number of hash buckets", (ulong)sp->hash_buckets);
	__db_dl_pct(env, "Number of bytes free on bucket pages", (ulong)sp->hash_bfree, DB_PCT_PG(sp->hash_bfree, sp->hash_buckets, sp->hash_pagesize), "ff");
	__db_dl(env, "Number of overflow pages", (ulong)sp->hash_bigpages);
	__db_dl_pct(env, "Number of bytes free in overflow pages", (ulong)sp->hash_big_bfree, DB_PCT_PG(sp->hash_big_bfree, sp->hash_bigpages, sp->hash_pagesize), "ff");
	__db_dl(env, "Number of bucket overflow pages", (ulong)sp->hash_overflows);
	__db_dl_pct(env, "Number of bytes free in bucket overflow pages", (ulong)sp->hash_ovfl_free, DB_PCT_PG(sp->hash_ovfl_free, sp->hash_overflows, sp->hash_pagesize), "ff");
	__db_dl(env, "Number of duplicate pages", (ulong)sp->hash_dup);
	__db_dl_pct(env, "Number of bytes free in duplicate pages", (ulong)sp->hash_dup_free, DB_PCT_PG(sp->hash_dup_free, sp->hash_dup, sp->hash_pagesize), "ff");
	__db_dl(env, "Number of pages on the free list", (ulong)sp->hash_free);
	__os_ufree(env, sp);
	return 0;
}
...
void __ham_print_cursor(DBC * dbc)
{
	...
	ENV * env = dbc->env;
	HASH_CURSOR * cp = (HASH_CURSOR *)dbc->internal;
 ...
 */
int __ham_traverse(DBC * dbc, db_lockmode_t mode, int (*callback)__P((DBC*, PAGE*, void *, int *)), void * cookie, int look_past_max)
{
	DB * dbp;
	DBC * opd;
	DB_MPOOLFILE * mpf;
	...
	for(bucket = 0;; bucket++) {
		...
		if(look_past_max) {
			...
			if(spares_entry >= NCACHED || hcp->hdr->spares[spares_entry] == 0)
				break;
		}
		else {
		...
		for(ret = __ham_get_cpage(dbc, mode); ret == 0; ret = __ham_next_cpage(dbc, pgno)) {
			/*
			...
			for(i = 0; i < NUM_ENT(hcp->page); i++) {
				...
				switch(HPAGE_PTYPE(hk)) {
					...
					if((ret = __dbc_newopd(dbc, opgno, NULL, &opd)) != 0)
						return ret;
					if((ret = __bam_traverse(opd, DB_LOCK_READ, opgno, callback, cookie)) != 0)
						goto err;
					...
					if((ret = __db_traverse_big(dbc, opgno, callback, cookie)) != 0)
						goto err;
					break;
				    case H_KEYDATA:
				    case H_DUPLICATE:
