...
int __lock_get_lk_conflicts(DB_ENV * dbenv, const uint8 ** lk_conflictsp, int * lk_modesp)
{
	...
	if(LOCKING_ON(env)) {
		...
		ASSIGN_PTR(lk_conflictsp, lt->conflicts);
		ASSIGN_PTR(lk_modesp, ((DB_LOCKREGION *)(lt->reginfo.primary))->nmodes);
	}
	else {
		ASSIGN_PTR(lk_conflictsp, dbenv->lk_conflicts);
		ASSIGN_PTR(lk_modesp, dbenv->lk_modes);
	}
	return 0;
}
/*
...
int __lock_set_lk_detect(DB_ENV * dbenv, uint32 lk_detect)
{
	...
	int ret;
	ENV * env = dbenv->env;
	ENV_NOT_CONFIGURED(env, env->lk_handle, "DB_ENV->set_lk_detect", DB_INIT_LOCK);
	switch(lk_detect) {
...
int __lock_get_lk_max_locks(DB_ENV * dbenv, uint32 * lk_maxp)
{
	...
	if(LOCKING_ON(env)) {
		/* Cannot be set after open, no lock required to read. */
		*lk_maxp = ((DB_LOCKREGION *)env->lk_handle->reginfo.primary)->stat.st_maxlocks;
	}
	else
