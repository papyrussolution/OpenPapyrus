...
int __repmgr_queue_get(ENV*env, REPMGR_MESSAGE ** msgp, REPMGR_RUNNABLE * th)
{
	...
	int ret = 0;
	db_rep = env->rep_handle;
	while((m = available_work(env)) == NULL && !db_rep->finished && !th->quit_requested) {
#ifdef DB_WIN32
		/*
		 ...
		 * not true.
		 */
		if(STAILQ_EMPTY(&db_rep->input_queue.header) && !db_rep->finished && !ResetEvent(db_rep->msg_avail)) {
			ret = GetLastError();
			goto err;
		}
...
#else
		if((ret = pthread_cond_wait(&db_rep->msg_avail, db_rep->mutex)) != 0)
			goto err;
#endif
	}
	if(db_rep->finished || th->quit_requested)
		...
		STAILQ_REMOVE(&db_rep->input_queue.header, m, __repmgr_message, entries);
...
/*
 * PUBLIC: int __repmgr_queue_size(ENV *);
 *
 * !!!
 * Caller must hold repmgr->mutex.
 */
int __repmgr_queue_size(ENV*env)
{
