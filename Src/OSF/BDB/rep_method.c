...
#define OK_FLAGS                                                        \
	...
	ENV_NOT_CONFIGURED(env, db_rep->region, "DB_ENV->rep_get_config", DB_INIT_REP);
	mapped = 0;
...
#define REPMGR_FLAGS (REP_C_2SITE_STRICT|REP_C_ELECTIONS)
	...
	if(REP_ON(env)) {
		 ...
		 */
		if(!FLD_ISSET(orig, REP_C_ELECTIONS) && FLD_ISSET(rep->config, REP_C_ELECTIONS) &&
 ...
 *
 * PUBLIC: int __rep_start_int __P((ENV *, DBT *, uint32));
 */
int __rep_start_int(ENV * env, DBT * dbt, uint32 flags)
{
	DB * dbp;
	DB_LOG * dblp;
	DB_LOGC * logc;
	...
	if((ret = __log_flush(env, NULL)) != 0)
		goto out;
	REP_SYSTEM_LOCK(env);
	role_chg = (!F_ISSET(rep, REP_F_MASTER) && role == DB_REP_MASTER) || (!F_ISSET(rep, REP_F_CLIENT) && role == DB_REP_CLIENT);
	/*
	...
	if(FLD_ISSET(rep->lockout_flags, REP_LOCKOUT_MSG)) {
		 ...
		 */
		RPRINT(env, (env, DB_VERB_REP_MISC, "Thread already in msg lockout"));
		REP_SYSTEM_UNLOCK(env);
		goto out;
	}
	...
	if(role == DB_REP_MASTER) {
		if(role_chg) {
			...
			if(F_ISSET(rep, REP_F_MASTERELECT)) {
				 ...
				 */
				RPRINT(env, (env, DB_VERB_REP_MISC, "Appointed new master while running in-memory replication."));
			if(rep->egen <= rep->gen)
				rep->egen = rep->gen+1;
		...
		if(IS_USING_LEASES(env) && (role_chg || !IS_REP_STARTED(env))) {
			...
			if((ret = __rep_lease_table_alloc(env, rep->config_nsites)) != 0)
				goto errunlock;
		}
		rep->master_id = rep->eid;
		STAT_INC(env, rep, master_change, rep->stat.st_master_changes, rep->eid);
#ifdef  DIAGNOSTIC
		if(!F_ISSET(rep, REP_F_GROUP_ESTD))
			RPRINT(env, (env, DB_VERB_REP_MISC, "Establishing group as master."));
#endif
		/*
		 ...
		 * recovery functions.
		 */
		VPRINT(env, (env, DB_VERB_REP_MISC, "rep_start: Old log version was %lu", (ulong)oldvers));
		...
		if(role_chg) {
			...
			if(region->stat.st_nrestores == 0 && (t_ret = __rep_restore_prepared(env)) != 0 && ret == 0)
				ret = t_ret;
			if(region->stat.st_nrestores != 0) {
				if((t_ret = __dbreg_mark_restored(env)) != 0 && ret == 0)
					ret = t_ret;
			}
			else {
			 ...
			 */
			if((t_ret = __rep_save_lsn_hist(env, ip, &lsn)) != 0 && ret == 0)
				ret = t_ret;
			REP_SYSTEM_LOCK(env);
...
static int __rep_save_lsn_hist(ENV * env, DB_THREAD_INFO * ip, DB_LSN * lsnp)
{
	 ...
	 * our update.
	 */
	if((dbp = db_rep->lsn_db) == NULL && (ret = __rep_open_sysdb(env, ip, txn, REPLSNHIST, DB_CREATE, &dbp)) != 0)
		goto err;
...
int __rep_open_sysdb(ENV * env, DB_THREAD_INFO * ip, DB_TXN * txn, const char * dbname, uint32 flags, DB ** dbpp)
{
	 ...
	 */
	if((ret = __db_open(dbp, ip, txn, REPSYSDBNAME, dbname, DB_BTREE, myflags, 0, PGNO_BASE_MD)) == 0)
		goto found;
	if(ret != ENOENT)
		goto err;
	/*
	...
	if(ret != 0 || (ret = __db_create_internal(&dbp, env, 0)) != 0)
		goto err;
	if((ret = __db_open(dbp, ip, txn, NULL, dbname, DB_BTREE, myflags, 0, PGNO_BASE_MD)) == 0)
		goto found;
	if(ret != ENOENT)
		goto err;
	/*
	...
	if(LF_ISSET(DB_CREATE)) {
		...
		fname = FLD_ISSET(rep->config, REP_C_INMEM) ? NULL : REPSYSDBNAME;
		if((ret = __db_open(dbp, ip, txn, fname, dbname, DB_BTREE, myflags, 0, PGNO_BASE_MD)) == 0)
			goto found;
	}
	else
		ret = ENOENT;
err:
	if(dbp != NULL && (t_ret = __db_close(dbp, txn, DB_NOSYNC)) != 0 && (ret == 0 || ret == ENOENT))
		ret = t_ret;
	return ret;
found:
	*dbpp = dbp;
...
int __rep_client_dbinit(ENV * env, int startup, repdb_t which)
{
	...
	if(startup) {
		 ...
		 */
		if(FLD_ISSET(rep->config, REP_C_INMEM))
			__db_set_flags(dbp, DB_TXN_NOT_DURABLE);
		/*
		 * Ignore errors, because if the file doesn't exist, this
		 * is perfectly OK.
		 */
		__db_remove(dbp, ip, NULL, fname, subdb, DB_FORCE);
	}
	if((ret = __db_create_internal(&dbp, env, 0)) != 0)
		goto err;
	if(which == REP_DB &&
 ...
 * care about the LSNs.
 */
static int __rep_bt_cmp(DB * dbp, const DBT * dbt1, const DBT * dbt2)
{
	DB_LSN lsn1, lsn2;
...
#define PREPLISTSIZE    50
	...
	op = DB_FIRST;
	do {
		if((ret = __txn_recover(env, prep, PREPLISTSIZE, &count, op)) != 0)
			return ret;
		for(i = 0; i < count; i++) {
			p = &prep[i];
...
static int __rep_restore_prepared(ENV * env)
{
	...
	if(IS_ZERO_LSN(rep->max_prep_lsn)) {
		VPRINT(env, (env, DB_VERB_REP_MISC, "restore_prep: No prepares. Skip."));
		return 0;
	}
	txninfo = NULL;
	ckp_args = NULL;
...
/*
 * PUBLIC: int __rep_set_priority(DB_ENV *, uint32);
 */
int __rep_set_priority(DB_ENV * dbenv, uint32 priority)
{
	REP * rep;
	uint32 prev;
	int ret;
...
int __rep_set_timeout(DB_ENV * dbenv, int which, db_timeout_t timeout)
{
	...
	if(which == DB_REP_ACK_TIMEOUT || which == DB_REP_CONNECTION_RETRY || which == DB_REP_ELECTION_RETRY || which == DB_REP_HEARTBEAT_MONITOR || which == DB_REP_HEARTBEAT_SEND)
		repmgr_timeout = 1;
...
int __rep_get_request(DB_ENV * dbenv, db_timeout_t * minp, db_timeout_t * maxp)
{
	...
	if(REP_ON(env)) {
		...
		if(minp != NULL)
			DB_TIMESPEC_TO_TIMEOUT((*minp), &db_rep->request_gap, 0);
		if(maxp != NULL)
 ...
 * PUBLIC:     int, uint32)));
 */
int __rep_set_transport_pp(DB_ENV * dbenv, int eid, int (*f_send)__P((DB_ENV*, const DBT*, const DBT*, const DB_LSN*, int, uint32)))
{
	DB_REP * db_rep;
	DB_THREAD_INFO * ip;
	ENV * env;
 ...
 * PUBLIC:     int, uint32)));
 */
int __rep_set_transport_int(ENV * env, int eid, int (*f_send)__P((DB_ENV*, const DBT*, const DBT*, const DB_LSN*, int, uint32)))
{
	REP * rep;
	DB_REP * db_rep = env->rep_handle;
	db_rep->send = f_send;
 ...
 *
 * PUBLIC: int __rep_flush(DB_ENV *);
 */
int __rep_flush(DB_ENV * dbenv)
{
	DBT rec;
	DB_LOGC * logc;
	DB_LSN lsn;
	...
	if((ret = __logc_get(logc, &lsn, &rec, DB_LAST)) != 0)
		goto err;
	__rep_send_message(env, DB_EID_BROADCAST, REP_LOG, &lsn, &rec, 0, 0);
err:
...
int __rep_sync(DB_ENV * dbenv, uint32 flags)
{
	...
	__rep_send_message(env, master, type, &lsn, NULL, 0, repflags);
out:
	ENV_LEAVE(env, ip);
	return ret;
}
...
static int __rep_check_applied(ENV * env, DB_THREAD_INFO * ip, DB_COMMIT_INFO * commit_info, struct rep_waitgoal * reasonp)
{
	...
	if(commit_info->gen == gen) {
		ret = __rep_read_lsn_history(env, ip, &txn, &dbc, gen, &hist, reasonp, DB_SET);
		if(ret == DB_NOTFOUND) {
			/*
		 ...
		 * [base,next).
		 */
		ret = __rep_read_lsn_history(env, ip, &txn, &dbc, commit_info->gen, &hist, reasonp, DB_SET);
		t_ret = __rep_read_lsn_history(env, ip, &txn, &dbc, commit_info->gen, &hist2, reasonp, DB_NEXT);
		if(ret == DB_NOTFOUND) {
			/*
		 ...
		 */
		if(F_ISSET(rep, REP_F_CLIENT) && LOG_COMPARE(&commit_info->lsn, &lsn) > 0) {
...
static int __rep_read_lsn_history(ENV * env, DB_THREAD_INFO * ip, DB_TXN ** txn, DBC ** dbc, uint32 gen, __rep_lsn_hist_data_args * gen_infop, struct rep_waitgoal * reasonp, uint32 flags)
{
	...
	tries = 0;
retry:
	if(*txn == NULL && (ret = __txn_begin(env, ip, NULL, txn, 0)) != 0)
		return ret;
	if((dbp = db_rep->lsn_db) == NULL) {
		if((ret = __rep_open_sysdb(env, ip, *txn, REPLSNHIST, 0, &dbp)) != 0) {
			/*
			 ...
			 */
			if(ret == ENOENT && !F_ISSET(rep, REP_F_NIMDBS_LOADED|REP_F_MASTER)) {
				ret = DB_TIMEOUT;
