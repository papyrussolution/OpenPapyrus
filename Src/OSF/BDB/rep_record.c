...
static int __rep_collect_txn(ENV*, DB_LSN*, LSN_COLLECTION *);
static int __rep_do_ckp(ENV*, DBT*, __rep_control_args *);
static int __rep_fire_newmaster(ENV*, uint32, int);
static int __rep_fire_startupdone(ENV*, uint32, int);
static int __rep_getnext(ENV*, DB_THREAD_INFO *);
static int __rep_lsn_cmp(const void *, const void *);
...
#define RECOVERING_LOG_SKIP do {                                        \
	if(F_ISSET(rep, REP_F_DELAY) || rep->master_id == DB_EID_INVALID || (recovering && (rep->sync_state != SYNC_LOG || LOG_COMPARE(&rp->lsn, &rep->last_lsn) >= 0))) { \
		/* Not holding region mutex, may miscount */            \
		STAT(rep->stat.st_msgs_recover++);                      \
		ret = __rep_skip_msg(env, rep, eid, rp->rectype);       \
		goto errlock;                                           \
 ...
 */
int __rep_process_message_int(ENV*env, DBT * control, DBT * rec, int eid, DB_LSN * ret_lsnp)
{
	DBT data_dbt;
	DB_LOG * dblp;
	DB_LSN last_lsn, lsn;
	...
	if(rep_version <= DB_REPVERSION_45) {
		...
		if(rep_version == DB_REPVERSION_45 && F_ISSET(orp, REPCTL_INIT_45)) {
		...
		tmprp.msg_sec = (uint32)rp46->msg_time.tv_sec;
		tmprp.msg_nsec = (uint32)rp46->msg_time.tv_nsec;
	}
	else if((ret = __rep_control_unmarshal(env, &tmprp, (uint8 *)control->data, control->size, NULL)) != 0)
		return ret;
	rp = &tmprp;
	if(ret_lsnp != NULL)
		ZERO_LSN(*ret_lsnp);
	...
	if(rp->rep_version < DB_REPVERSION) {
		if(rp->rep_version < DB_REPVERSION_MIN) {
			__db_errx(env, DB_STR_A("3516", "unsupported old replication message version %lu, minimum version %d", "%lu %d"), (ulong)rp->rep_version, DB_REPVERSION_MIN);
			ret = EINVAL;
			goto errlock;
		}
		VPRINT(env, (env, DB_VERB_REP_MSGS, "Received record %lu with old rep version %lu", (ulong)rp->rectype, (ulong)rp->rep_version));
		 ...
		 * versions.
		 */
		VPRINT(env, (env, DB_VERB_REP_MSGS, "Converted to record %lu with old rep version %lu", (ulong)rp->rectype, (ulong)rp->rep_version));
	}
	else if(rp->rep_version > DB_REPVERSION) {
		__db_errx(env, DB_STR_A("3517", "unexpected replication message version %lu, expected %d", "%lu %d"), (ulong)rp->rep_version, DB_REPVERSION);
		ret = EINVAL;
		goto errlock;
	}
	if(rp->log_version < DB_LOGVERSION) {
		if(rp->log_version < DB_LOGVERSION_MIN) {
			__db_errx(env, DB_STR_A("3518", "unsupported old replication log version %lu, minimum version %d", "%lu %d"), (ulong)rp->log_version, DB_LOGVERSION_MIN);
			ret = EINVAL;
			goto errlock;
		}
		VPRINT(env, (env, DB_VERB_REP_MSGS, "Received record %lu with old log version %lu", (ulong)rp->rectype, (ulong)rp->log_version));
	}
	else if(rp->log_version > DB_LOGVERSION) {
		__db_errx(env, DB_STR_A("3519", "unexpected log record version %lu, expected %d", "%lu %d"), (ulong)rp->log_version, DB_LOGVERSION);
		ret = EINVAL;
		goto errlock;
	}
	/*
	...
	if(rp->gen > gen) {
		 ...
		 */
		if(oneof3(rp->rectype, REP_ALIVE, REP_VOTE1, REP_VOTE2)) {
			REP_SYSTEM_LOCK(env);
			RPRINT(env, (env, DB_VERB_REP_MSGS, "Updating gen from %lu to %lu", (ulong)gen, (ulong)rp->gen));
	...
	if(F_ISSET(rp, REPCTL_GROUP_ESTD)) {
		...
		if(!F_ISSET(rep, REP_F_GROUP_ESTD))
			RPRINT(env, (env, DB_VERB_REP_MSGS, "I am now part of an established group"));
#endif
	...
	switch(rp->rectype) {
		...
		if(rp->rep_version < DB_REPVERSION_47)
			egen_arg.egen = *(uint32 *)rec->data;
		else if((ret = __rep_egen_unmarshal(env, &egen_arg, (uint8 *)rec->data, rec->size, NULL)) != 0)
			return ret;
		REP_SYSTEM_LOCK(env);
		if(egen_arg.egen > rep->egen) {
			...
			RPRINT(env, (env, DB_VERB_REP_MSGS, "Received ALIVE egen of %lu, mine %lu", (ulong)egen_arg.egen, (ulong)rep->egen));
		...
		if(rep->version < DB_REPVERSION_47)
			DB_INIT_DBT(data_dbt, &egen_arg.egen, sizeof(egen_arg.egen));
		else {
			if((ret = __rep_egen_marshal(env, &egen_arg, buf, __REP_EGEN_SIZE, &len)) != 0)
				goto errlock;
			DB_INIT_DBT(data_dbt, buf, len);
		}
		__rep_send_message(env, eid, REP_ALIVE, &lsn, &data_dbt, 0, 0);
		break;
	    case REP_ALL_REQ:
		RECOVERING_SKIP;
		CLIENT_MASTERCHK;
		...
		if(eid == rep->master_id) {
			...
			if(ISSET_LOCKOUT_BDB(rep)) {
				RPRINT(env, (env, DB_VERB_REP_MSGS, "FILE_FAIL is cleaning up old internal init"));
#ifdef  CONFIG_TEST
		...
		if(F_ISSET(rep, REP_F_MASTER)) {
			...
			lsn = lp->lsn;
			LOG_SYSTEM_UNLOCK(env);
			__rep_send_message(env, eid, REP_NEWMASTER, &lsn, NULL, 0, 0);
		}
		ret = DB_REP_NEWSITE;
		break;
	    case REP_NEWCLIENT:
		...
		if(F_ISSET(rep, REP_F_CLIENT)) {
			...
			if(eid == rep->master_id) {
				 ...
				 * conditions.
				 */
				if((t_ret = __rep_lockout_msg(env, rep, 1)) != 0) {
					ret = t_ret;
					goto errhlk;
				}
				lockout = 1;
				 ...
				 */
				if(ISSET_LOCKOUT_BDB(rep)) {
					RPRINT(env, (env, DB_VERB_REP_MSGS, "NEWCLIENT is cleaning up old internal init for invalid master"));
					t_ret = __rep_init_cleanup(env, rep, DB_FORCE);
					F_CLR(rep, REP_F_ABBREVIATED);
					CLR_RECOVERY_SETTINGS(rep);
				}
				MUTEX_UNLOCK(env, rep->mtx_clientdb);
				if(t_ret != 0) {
					ret = t_ret;
					RPRINT(env, (env, DB_VERB_REP_MSGS, "NEWCLIENT error cleaning up internal init for invalid master: %d", ret));
					goto errhlk;
				}
				FLD_CLR(rep->lockout_flags, REP_LOCKOUT_MSG);
				lockout = 0;
		...
		break;
	    default:
		__db_errx(env, DB_STR_A("3521", "DB_ENV->rep_process_message: unknown replication message: type %lu", "%lu"), (ulong)rp->rectype);
		ret = EINVAL;
		break;
	}
errlock:
 ...
 */
int __rep_apply(ENV * env, DB_THREAD_INFO * ip, __rep_control_args * rp, DBT * rec, DB_LSN * ret_lsnp, int * is_dupp, DB_LSN * last_lsnp)
{
	DB * dbp;
	...
	if(cmp == 0) {
		 ...
		 */
		while(ret == 0 && LOG_COMPARE(&lp->ready_lsn, &lp->waiting_lsn) == 0) {
			/*
			 * We just filled in a gap in the log record stream.
			 * Write subsequent records to the log.
			 */
gap_check:
			if((ret = __rep_remfirst(env, ip, &control_dbt, &rec_dbt)) != 0)
				goto err;
		 ...
		 */
		if(!IS_ZERO_LSN(lp->waiting_lsn) && LOG_COMPARE(&lp->ready_lsn, &lp->waiting_lsn) != 0) {
			/*
		 ...
		 */
		if(IS_USING_LEASES(env) && F_ISSET(rp, REPCTL_LEASE) && timespecisset(&msg_time)) {
...
err:    
	 ...
	 */
	if((ret == 0 || ret == DB_REP_ISPERM) && rep->stat.st_startup_complete == 0 && rep->sync_state != SYNC_LOG &&
	   ((cmp <= 0 && F_ISSET(rp, REPCTL_LOG_END)) || (cmp == 0 && !F_ISSET(rp, REPCTL_RESEND)))) {
	 ...
	 */
	if(ret == DB_REP_ISPERM && IS_USING_LEASES(env) && timespecisset(&max_ts)) {
 ...
 * processing.
 *
 * PUBLIC: int __rep_process_txn(ENV *, DBT *);
 */
int __rep_process_txn(ENV*env, DBT * rec)
{
	...
	if(rectype == DB___txn_regop) {
		...
		if(rep->version >= DB_REPVERSION_44) {
			...
			if((ret = __txn_regop_42_read(env, rec->data, &txn42_args)) != 0)
				return ret;
		...
		if((ret = __txn_prepare_read(env, rec->data, &prep_args)) != 0)
			return ret;
	...
	if((ret = __lock_get_list(env, locker, 0, DB_LOCK_WRITE, lock_dbt)) != 0)
		goto err;
...
err:
	...
	if((t_ret = __lock_vec(env, locker, 0, &req, 1, &lvp)) != 0 && ret == 0)
		ret = t_ret;
 ...
 */
static int __rep_collect_txn(ENV*env, DB_LSN * lsnp, LSN_COLLECTION * lc)
{
...
static int __rep_newfile(ENV * env, __rep_control_args * rp, DBT * rec)
{
	...
	if(rp->lsn.file+1 > lp->ready_lsn.file) {
		if(rec == NULL || rec->size == 0) {
			RPRINT(env, (env, DB_VERB_REP_MISC, "rep_newfile: Old-style NEWFILE msg.  Use control msg log version: %lu", (ulong)rp->log_version));
			nf_args.version = rp->log_version;
		}
		else if(rp->rep_version < DB_REPVERSION_47)
			nf_args.version = *(uint32 *)rec->data;
		else if((ret = __rep_newfile_unmarshal(env, &nf_args, (uint8 *)rec->data, rec->size, NULL)) != 0)
			return ret;
		RPRINT(env, (env, DB_VERB_REP_MISC, "rep_newfile: File %lu vers %lu", (ulong)rp->lsn.file+1, (ulong)nf_args.version));
		/*
 ...
 */
static int __rep_do_ckp(ENV*env, DBT * rec, __rep_control_args * rp)
{
...
int __rep_resend_req(ENV*env, int rereq)
{
	...
	if(msgtype != REP_INVALID) {
		...
		if(master == DB_EID_INVALID)
			...
			__rep_send_message(env, master, msgtype, lsnp, NULL, 0, sendflags);
	}
	return ret;
}
/*
...
int __rep_check_missing(ENV*env, uint32 gen, DB_LSN * master_perm_lsn)
{
	 ...
	 */
	if(!F_ISSET(rep, REP_F_CLIENT) || rep->master_id == DB_EID_INVALID || gen != rep->gen || FLD_ISSET(rep->lockout_flags, REP_LOCKOUT_MSG)) {
		REP_SYSTEM_UNLOCK(env);
		MUTEX_UNLOCK(env, rep->mtx_clientdb);
		/*
	...
	if((do_req = __rep_check_doreq(env, rep))) {
		...
		REP_SYSTEM_LOCK(env);
		if(rep->sync_state == SYNC_PAGE && rep->curinfo_off != INVALID_ROFF) {
			GET_CURINFO(rep, infop, curinfo);
			has_page_gap = rep->waiting_pg != PGNO_INVALID || rep->ready_pg <= curinfo->max_pgno;
		}
		REP_SYSTEM_UNLOCK(env);
	}
	...
	if(do_req && !has_page_gap) {
		 ...
		 */
		end_lsn = (rep->sync_state == SYNC_LOG) ? &rep->last_lsn : master_perm_lsn;
		has_log_gap = !IS_ZERO_LSN(lp->waiting_lsn) || LOG_COMPARE(&lp->ready_lsn, end_lsn) <= 0;
	}
	MUTEX_UNLOCK(env, rep->mtx_clientdb);
	/*
	 ...
	 */
	do_req = do_req && (has_log_gap || has_page_gap || oneof3(rep->sync_state, SYNC_LOG, SYNC_UPDATE, SYNC_VERIFY));
	/*
