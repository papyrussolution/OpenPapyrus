...
{
	int ret;
	ulong onoff = 1; // any non-zero value 
...
int __repmgr_await_cond(ENV*env, PREDICATE pred, void * ctx, db_timeout_t timeout, waiter_t * waiters_p)
{
	...
	COND_WAITERS_TABLE * waiters = *waiters_p;
...
int __repmgr_select_loop(ENV*env)
{
	...
	if(!IS_SUBORDINATE(db_rep) && WSAEventSelect(db_rep->listen_fd, listen_event, FD_ACCEPT) == SOCKET_ERROR) {
		ret = net_errno;
		__db_err(env, ret, DB_STR("3591", "can't enable event for listener"));
...
static int prepare_io(ENV * env, REPMGR_CONNECTION * conn, void * info_)
{
	...
	if(WSAEventSelect(conn->fd, conn->event_object, desired_events) == SOCKET_ERROR) {
		ret = net_errno;
