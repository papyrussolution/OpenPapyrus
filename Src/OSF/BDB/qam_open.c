...
int __qam_open(DB*dbp, DB_THREAD_INFO * ip, DB_TXN * txn, const char * name, db_pgno_t base_pgno, int mode, uint32 flags)
{
	 ...
	 * lock instead.
	 */
	if((ret = __db_cursor(dbp, ip, txn, &dbc, LF_ISSET(DB_CREATE) && CDB_LOCKING(env) ? DB_WRITECURSOR : 0)) != 0)
		return ret;
	/*
	...
	if(qmeta->dbmeta.magic != DB_QAMMAGIC) {
		__db_errx(env, DB_STR_A("1136", "__qam_open: %s: unexpected file type or format", "%s"), name);
		ret = EINVAL;
		goto err;
	}
...
int __qam_set_ext_data(DB * dbp, const char * name)
{
	...
	t->pginfo.flags = F_ISSET(dbp, (DB_AM_CHKSUM|DB_AM_ENCRYPT|DB_AM_SWAP));
...
int __qam_metachk(DB * dbp, const char * name, QMETA * qmeta)
{
	...
	switch(vers) {
	    case 1:
	    case 2:
		__db_errx(env, DB_STR_A("1137", "%s: queue version %lu requires a version upgrade", "%s %lu"), name, (ulong)vers);
		return DB_OLD_VERSION;
	    case 3:
	    case 4:
		break;
	    default:
		__db_errx(env, DB_STR_A("1138", "%s: unsupported qam version: %lu", "%s %lu"), name, (ulong)vers);
		return EINVAL;
	}
	/* Swap the page if we need to. */
	if(F_ISSET(dbp, DB_AM_SWAP) && (ret = __qam_mswap(env, (PAGE *)qmeta)) != 0)
		return ret;
	/* Check the type. */
...
static int __qam_init_meta(DB*dbp, QMETA * meta)
{
	...
	if(QAM_RECNO_PER_PAGE(dbp) < 1) {
		__db_errx(env, DB_STR_A("1139", "Record size of %lu too large for page size of %lu", "%lu %lu"), (ulong)t->re_len, (ulong)dbp->pgsize);
		return EINVAL;
	}
	return 0;
}
...
err1:
		...
		if((ret = __db_pgout(env->dbenv, PGNO_BASE_MD, meta, &pdbt)) != 0)
			goto err2;
		ret = __fop_write(env, txn, name, dbp->dirname, DB_APP_DATA, fhp, dbp->pgsize, 0, 0, meta, dbp->pgsize, 1,
