...
#define CLR_MAP(M, B)   ((M)[(B)/32] &= ~((uint)1<<((B)%32)))
#define OR_MAP(D, S, N) { uint32 __i; for(__i = 0; __i < (N); __i++) D[__i] |= S[__i]; }
...
int __lock_detect(ENV * env, uint32 atype, int * rejectp)
{
	...
	if((ret = __os_calloc(env, (size_t)nlockers, sizeof(uint32)*nalloc, &copymap)) != 0)
		goto err;
	memcpy(copymap, bitmap, nlockers*sizeof(uint32)*nalloc);
	if((ret = __os_calloc(env, sizeof(uint32), nalloc, &tmpmap)) != 0)
		goto err;
	/* Find a deadlock. */
	if((ret = __dd_find(env, bitmap, idmap, nlockers, nalloc, &deadlist)) != 0)
		return ret;
	/*
	...
	if(TXN_ON(env)) {
		TXN_SYSTEM_LOCK(env);
		txn_max = ((DB_TXNREGION *)env->tx_handle->reginfo.primary)->cur_maxid;
	...
	for(deadp = deadlist; *deadp != NULL; deadp++) {
		...
		if(keeper == BAD_KILLID || __dd_verify(idmap, *deadp, tmpmap, copymap, nlockers, nalloc, keeper) == 0)
			killid = BAD_KILLID;
		if(!pri_set && killid != BAD_KILLID && (atype == DB_LOCK_DEFAULT || atype == DB_LOCK_RANDOM))
			goto dokill;
		/*
		 ...
		 *	YOUNGEST: largest id
		 */
		for(i = (limit+1)%nlockers; i != limit; i = (i+1)%nlockers) {
			...
			switch(atype) {
			    case DB_LOCK_OLDEST:
					if(__dd_isolder(idmap[cid].id, idmap[i].id, lock_max, txn_max))
						continue;
					break;
			    case DB_LOCK_YOUNGEST:
					if(__dd_isolder(idmap[i].id, idmap[cid].id, lock_max, txn_max))
						continue;
					break;
			    case DB_LOCK_MAXLOCKS:
...
use_next:
			keeper = i;
			if(__dd_verify(idmap, *deadp, tmpmap, copymap, nlockers, nalloc, i))
				killid = i;
		}
dokill:
		if(killid == BAD_KILLID) {
		...
		if(status != 0) {
			if(status != DB_ALREADY_ABORTED)
				__db_errx(env, DB_STR_A("2049", "warning: unable to abort locker %lx", "%lx"), (ulong)idmap[killid].id);
			else
				region->need_dd = 1;
		}
		else if(FLD_ISSET(env->dbenv->verbose, DB_VERB_DEADLOCK))
			__db_msg(env, DB_STR_A("2050", "Aborting locker %lx", "%lx"), (ulong)idmap[killid].id);
	}
err:    
...
skip:
		...
		for(; op != NULL; op = np) {
			...
			SH_TAILQ_FOREACH(lp, &op->waiters, links, __db_lock) {
				...
				if(lp->status == DB_LSTAT_WAITING) {
					...
					if(timespecisset(&lockerp->lk_expire) && (!timespecisset(&min_timeout) || timespeccmp(&min_timeout, &lockerp->lk_expire, >)))
	...
	if(FLD_ISSET(env->dbenv->verbose, DB_VERB_DEADLOCK))
		__db_msg(env, DB_STR_A("2051", "%lu lockers", "%lu"), (ulong)count);
	nentries = (uint32)DB_ALIGN(count, 32)/32;
	/* Allocate enough space for a count by count bitmap matrix. */
	if((ret = __os_calloc(env, (size_t)count, sizeof(uint32)*nentries, &bitmap)) != 0) {
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		return ret;
	}
	if((ret = __os_calloc(env, sizeof(uint32), nentries, &tmpmap)) != 0) {
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		__os_free(env, bitmap);
		return ret;
	}
	if((ret = __os_calloc(env, (size_t)count, sizeof(locker_info), &id_array)) != 0) {
		UNLOCK_LOCKERS(env, region);
		LOCK_SYSTEM_UNLOCK(lt, region);
		__os_free(env, bitmap);
		__os_free(env, tmpmap);
	...
	*pri_set = 0;
	SH_TAILQ_FOREACH(lip, &region->lockers, ulinks, __db_locker) {
		if(lip->master_locker == INVALID_ROFF) {
			...
			if(lip->dd_id > 0 && id_array[lip->dd_id-1].priority != lip->priority)
				*pri_set = 1;
			switch(atype) {
			    case DB_LOCK_MINLOCKS:
			    case DB_LOCK_MAXLOCKS:
...
again:
		...
		SH_TAILQ_FOREACH(lp, &op->holders, links, __db_lock) {
			...
			if(lockerp->dd_id == DD_INVALID_ID) {
				...
				if(lockerp->master_locker == INVALID_ROFF)
					continue;
				dd = ((DB_LOCKER *)R_ADDR(&lt->reginfo, lockerp->master_locker))->dd_id;
				if(dd == DD_INVALID_ID)
					continue;
				lockerp->dd_id = dd;
				switch(atype) {
	...
	for(id = 0; id < count; id++) {
		if(!id_array[id].valid)
			continue;
		if((ret = __lock_getlocker_int(lt, id_array[id].id, 0, &lockerp)) != 0 || lockerp == NULL)
			continue;
		/*
...
c_retry:
				...
				if(lp != SH_LIST_FIRST(&child->heldby, __db_lock) || ndx != lp->indx) {
...
l_retry:
		...
		if(!SH_LIST_EMPTY(&lockerp->heldby) && lp != NULL) {
			ndx = lp->indx;
			OBJECT_LOCK_NDX(lt, region, ndx);
			if(lp != SH_LIST_FIRST(&lockerp->heldby, __db_lock) || lp->indx != ndx) {
				OBJECT_UNLOCK(lt, region, ndx);
				goto l_retry;
			}
			id_array[id].last_locker_id = lockerp->id;
get_lock:
			id_array[id].last_lock = R_OFFSET(&lt->reginfo, lp);
...
#define INITIAL_DEAD_ALLOC      8
	uint ndeadalloc = INITIAL_DEAD_ALLOC;
	uint ndead = 0;
	if((ret = __os_malloc(env, ndeadalloc*sizeof(uint32 *), &retp)) != 0)
		return ret;
	/*
	...
	for(mymap = bmp, i = 0; i < nlockers; i++, mymap += nalloc) {
		...
		for(j = 0; j < nlockers; j++) {
			...
			if(ndead+2 >= ndeadalloc) {
				 ...
				 */
				if(__os_realloc(env, ndeadalloc*sizeof(uint32 *), &retp) != 0) {
					retp[ndead] = NULL;
					*deadp = retp;
					return 0;
				}
...
static int __dd_abort(ENV * env, locker_info * info, int * statusp)
{
	...
	*statusp = 0;
	 ...
	 */
	if((ret = __lock_getlocker_int(lt, info->last_locker_id, 0, &lockerp)) != 0)
		goto err;
...
static void __dd_debug(ENV * env, locker_info * idmap, uint32 * bitmap, uint32 nlockers, uint32 nalloc)
{
	...
	for(mymap = bitmap, i = 0; i < nlockers; i++, mymap += nalloc) {
		if(!idmap[i].valid)
			continue;
		__db_msgadd(env, &mb, /* Waiter. */ "%lx/%lu:\t", (ulong)idmap[i].id, (ulong)idmap[i].pgno);
		for(j = 0; j < nlockers; j++)
			if(ISSET_MAP(mymap, j))
				__db_msgadd(env, &mb, " %lx", (ulong)idmap[j].id);
		__db_msgadd(env, &mb, " %lu", (ulong)idmap[i].last_lock);
