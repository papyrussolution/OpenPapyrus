 ...
 */
int __qam_position(DBC * dbc, db_recno_t * recnop, uint32 get_mode /* flags to __memp_fget */, int * exactp /* indicate if it was found */)
{
	QAMDATA * qp;
	db_pgno_t pg;
	int ret;
...
int __qam_pitem(DBC * dbc, QPAGE * pagep, uint32 indx, db_recno_t recno, DBT * data)
{
	...
	if(F_ISSET(data, DB_DBT_PARTIAL)) {
		if(data->doff+data->dlen > t->re_len) {
			__db_errx(env, DB_STR_A("1142", "Record length error: data offset plus length larger than record size of %lu", "%s %lu"), (ulong)t->re_len);
			return EINVAL;
		}
...
no_partial:
	if(DBC_LOGGING(dbc)) {
		...
		if((ret = __qam_add_log(dbp, dbc->txn, &LSN(pagep), 0, &LSN(pagep), pagep->pgno, indx, recno, datap, qp->flags, olddata.size == 0 ? NULL : &olddata)) != 0)
			goto err;
	}
...
recheck:
	...
	if(writelock == 0 && (ret = __memp_dirty(mpf, &meta, dbc->thread_info, dbc->txn, dbc->priority, DB_MPOOL_DIRTY)) != 0)
		goto done;
	if(writelock++ == 0)
		goto recheck;
...
again:
	...
	if(t_ret == 0 && (ret == DB_LOCK_NOTGRANTED || ret == DB_LOCK_DEADLOCK)) {
		waited = 1;
		ret = __db_lget(dbc, 0, recno, DB_LOCK_WRITE, DB_LOCK_RECORD, &lock);
	}
	/*
	...
	if((ret = __qam_fget(dbc, &pg, DB_MPOOL_CREATE|DB_MPOOL_DIRTY, &page)) != 0)
		goto err;
	...
	else if(waited && F_ISSET(QAM_GET_RECORD(dbp, page, QAM_RECNO_INDEX(dbp, pg, recno)), QAM_VALID)) {
...
static int __qamc_del(DBC * dbc, uint32 flags)
{
	...
	if((ret = __qam_position(dbc, &cp->recno, DB_MPOOL_DIRTY, &exact)) != 0)
		goto err;
	if(!exact) {
		ret = DB_NOTFOUND;
		goto err;
	...
	if(DBC_LOGGING(dbc)) {
		if(((QUEUE *)dbp->q_internal)->page_ext == 0 || ((QUEUE *)dbp->q_internal)->re_len == 0) {
			if((ret = __qam_del_log(dbp, dbc->txn, &LSN(pagep), 0, &LSN(pagep), pagep->pgno, cp->indx, cp->recno)) != 0)
				goto err;
		}
		else {
...
static int __qamc_get(DBC * dbc, DBT * key, DBT * data, uint32 flags, db_pgno_t * pgnop)
{
	...
	*pgnop = 0;
	...
	DEBUG_LREAD(dbc, dbc->txn, "qamc_get", oneof2(flags, DB_SET, DB_SET_RANGE) ? key : NULL, NULL, flags);
...
retry:  /* Update the record number. */
	switch(flags) {
		...
		if(cp->recno != RECNO_OOB) {
			if(with_delete && !inorder && QAM_BEFORE_FIRST(meta, cp->recno))
			...
			if(QAM_AFTER_CURRENT(meta, cp->recno)) {
				 ...
				 * wait for it.
				 */
				if(skip == 1 && !QAM_AFTER_CURRENT(meta, first)) {
					retrying = 1;
					cp->recno = first;
					goto dolock;
				}
...
dolock: 
	...
	// Lock the record. 
	if(((ret = __db_lget(dbc, LCK_COUPLE, cp->recno, lock_mode, (with_delete && !inorder && !retrying) ?
	     DB_LOCK_NOWAIT|DB_LOCK_RECORD : DB_LOCK_RECORD, &lock)) == DB_LOCK_DEADLOCK || ret == DB_LOCK_NOTGRANTED) && with_delete) {
#ifdef QDEBUG
		if(DBC_LOGGING(dbc))
			__log_printf(env, dbc->txn, "Queue S: %x %d %d", dbc->locker ? dbc->locker->id : 0, cp->recno, first);
#endif
		skip = 1;
		goto retry;
	}
	...
	switch(flags) {
		...
		if((is_first && cp->recno != meta->first_recno) ||
		   (flags == DB_LAST && cp->recno != meta->cur_recno-1)) {
			...
			if(QAM_BEFORE_FIRST(meta, cp->recno)) {
				if(oneof2(flags, DB_SET_RANGE, DB_GET_BOTH_RANGE)) {
					cp->lock = lock;
					LOCK_INIT(lock);
					goto release_retry;
				}
...
release_retry:  /* Release locks and retry, if possible. */
		...
		if(with_delete && DBC_LOGGING(dbc)) {
			__log_printf(dbp->env, dbc->txn, "Queue E: %x %d %d", dbc->locker ? dbc->locker->id : 0, cp->recno, first);
		}
#endif
		if(pg != NULL)
			__qam_fput(dbc, cp->pgno, pg, dbc->priority);
		cp->page = pg = NULL;
		if(with_delete) {
			...
			goto err1;
		if(meta == NULL && (ret = __memp_fget(mpf, &metapno, dbc->thread_info, dbc->txn, meta_mode, &meta)) != 0)
			goto err1;
		/*
		...
		switch(flags) {
			...
			if(!with_delete)
				...
				first++;
			if(QAM_BEFORE_FIRST(meta, cp->recno) && DONT_NEED_LOCKS(dbc))
				flags = DB_FIRST;
			break;
		    case DB_LAST:
		    case DB_PREV:
		    case DB_PREV_NODUP:
			flags = (QAM_AFTER_CURRENT(meta, cp->recno) && DONT_NEED_LOCKS(dbc)) ? DB_LAST : DB_PREV;
			break;

		    case DB_GET_BOTH_RANGE:
		    case DB_SET_RANGE:
			flags = (QAM_BEFORE_FIRST(meta, cp->recno) && DONT_NEED_LOCKS(dbc)) ? DB_FIRST : DB_NEXT;
			break;
	...
	if(with_delete && cp->recno == first) {
		if(meta == NULL && (ret = __memp_fget(mpf, &metapno, dbc->thread_info, dbc->txn, DB_MPOOL_DIRTY|DB_MPOOL_TRY, &meta)) != 0) {
	...
	if(with_delete) {
		...
		if(DB_IS_PRIMARY(dbp)) {
			...
			if((ret = __qam_fput(dbc, cp->pgno, cp->page, dbc->priority)) != 0)
				goto err1;
			cp->page = NULL;
			if(meta != NULL && (ret = __memp_fput(mpf, dbc->thread_info, meta, dbc->priority)) != 0)
				goto err1;
			meta = NULL;
			if((ret = __dbc_del_primary(dbcdup)) != 0) {
				// The __dbc_del_primary return is more interesting.
				__dbc_close(dbcdup);
				goto err1;
			}
			if((ret = __dbc_close(dbcdup)) != 0)
				goto err1;
			if((ret = __qam_fget(dbc, &cp->pgno, DB_MPOOL_DIRTY, &cp->page)) != 0)
				goto err;
		}
		else if((ret = __qam_dirty(dbc, cp->pgno, &cp->page, dbc->priority)) != 0)
			goto err1;
		pg = (PAGE *)cp->page;
		if(DBC_LOGGING(dbc)) {
			if(t->page_ext == 0 || t->re_len == 0) {
				if((ret = __qam_del_log(dbp, dbc->txn, &LSN(pg), 0, &LSN(pg), pg->pgno, cp->indx, cp->recno)) != 0)
					goto err1;
			}
			else {
				tmp.data = qp->data;
				tmp.size = t->re_len;
				if((ret = __qam_delext_log(dbp, dbc->txn, &LSN(pg), 0, &LSN(pg), pg->pgno, cp->indx, cp->recno, &tmp)) != 0)
					goto err1;
			}
		}
		else
...
#ifdef QDEBUG
		if(DBC_LOGGING(dbc))
			__log_printf(env, dbc->txn, "Queue D: %x %d %d %d", dbc->locker ? dbc->locker->id : 0, cp->recno, first, meta->first_recno);
#endif
...
static int __qam_consume(DBC * dbc, QMETA * meta, db_recno_t first)
{
	...
	for(;; ) {
		 ...
		 */
		if(rec_extent != 0 && ((exact = (first%rec_extent == 0)) || (first%meta->rec_page == 0) || first == UINT32_MAX)) {
#ifdef QDEBUG
			if(DBC_LOGGING(dbc))
...
next_pg:
	...
	do {
		...
		if(pg != NULL) {
			...
			if(F_ISSET(qp, QAM_VALID)) {
				valid = 1;
				space -= (int)((is_key ? 3 : 2)*sizeof(*offp));
				if(space < 0)
					goto get_space;
				if(need_pg) {
					dp = np;
...
get_space:
						if(offp == endp) {
							data->size = (uint32)DB_ALIGN((uint32)size+pagesize, sizeof(uint32));
							ret = DB_BUFFER_SMALL;
							break;
						}
						if(indx != 0)
					...
					memcpy(dp, (uint8 *)pg+QPAGE_SZ(dbp), (uint)size);
					need_pg = 0;
					space -= size;
					np += size;
				}
				if(is_key)
					*offp-- = cp->recno;
				*offp-- = (uint32)((((uint8 *)qp-(uint8 *)pg)-QPAGE_SZ(dbp))+(dp-dbuf)+SSZA(QAMDATA, data));
				*offp-- = re_len;
			}
		}
		if(!valid && is_key == 0) {
		...
		cp->recno++;
	} while(++indx < recs && cp->recno != RECNO_OOB && !QAM_AFTER_CURRENT(meta, cp->recno));
	if(cp->page != NULL) {
		if((t_ret = __qam_fput(dbc, cp->pgno, cp->page, dbc->priority)) != 0 && ret == 0)
			ret = t_ret;
		cp->page = NULL;
	}
	if(ret == 0 && space > 0 && (indx >= recs || cp->recno == RECNO_OOB) && !QAM_AFTER_CURRENT(meta, cp->recno))
		goto next_pg;
	/*
...
int __qam_truncate(DBC * dbc, uint32 * countp)
{
	...
	for(count = 0; (ret = __qamc_get(dbc, NULL, NULL, DB_CONSUME, &metapno)) == 0; )
		count++;
	if(ret != DB_NOTFOUND)
		return ret;
	mpf = dbp->mpf;
	...
	metapno = ((QUEUE *)dbp->q_internal)->q_meta;
	if((ret = __memp_fget(mpf, &metapno, dbc->thread_info, dbc->txn, DB_MPOOL_DIRTY, &meta)) != 0)
		return ret;
	/* Remove the last extent file. */
	if(meta->cur_recno > 1 && ((QUEUE *)dbp->q_internal)->page_ext != 0) {
		if((ret = __qam_fremove(dbp, QAM_RECNO_PAGE(dbp, meta->cur_recno-1))) != 0)
			goto err;
	}
	if(DBC_LOGGING(dbc)) {
