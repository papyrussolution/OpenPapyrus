...
int __fop_lock_handle(ENV * env, DB * dbp, DB_LOCKER * locker, db_lockmode_t mode, DB_LOCK * elockp, uint32 flags)
{
	...
	if(!LOCKING_ON(env) || F_ISSET(dbp, DB_AM_COMPENSATE|DB_AM_RECOVER))
		return 0;
	//
	// If we are in recovery, the only locking we should be doing is on the global environment.
	//
...
int __fop_file_setup(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, const char * name, int mode, uint32 flags, uint32 * retidp)
{
	...
	*retidp = TXN_INVALID;
	...
	APPNAME aflags = LF_ISSET(DB_INTERNAL_DB) ? DB_APP_NONE : DB_APP_DATA;
	LF_CLR(DB_INTERNAL_DB);
	DBTYPE save_type = dbp->type;
	/*
	 ...
	 */
	if(LOCKING_ON(env) && !F_ISSET(dbp, DB_AM_COMPENSATE) && !F_ISSET(dbp, DB_AM_RECOVER) && dbp->locker == DB_LOCK_INVALIDID) {
	...
	if(F_ISSET(dbp, DB_AM_INMEM))
		...
		if((ret = __db_appname(env, aflags, name, &dbp->dirname, &real_name)) != 0)
			goto err;
		// Fill in the default file mode. 
		SETIFZ(mode, DB_MODE_660);
...
retry:
	...
	// be that the backup filename exists (that is, is left over from a previous crash).
	//
	if(++retries > DB_RETRY) {
...
reopen:         
		...
		if(F_ISSET(dbp, DB_AM_INMEM)) {
			...
			ret = __fop_read_meta(env, real_name, mbuf, sizeof(mbuf), fhp, LF_ISSET(DB_NOERROR) || (LF_ISSET(DB_FCNTL_LOCKING) && txn == NULL) ? 1 : 0, &len);
			// Case 3: 0-length, no txns.
			if(ret != 0 && len == 0 && txn == NULL) {
				if(LF_ISSET(DB_EXCL)) {
					//
					// Case 1b: DB_EXCL and 0-length file exists.
					//
					ret = EEXIST;
					goto err;
				}
		...
		if((ret = __fop_lock_handle(env, dbp, locker, DB_LOCK_READ, NULL, DB_LOCK_NOWAIT)) == 0) {
			...
			// To be sure we have the correct information we try again.
			// 
			if((ret = __db_refresh(dbp, txn, DB_NOSYNC, NULL, 1)) != 0)
				goto err;
			if((ret = __ENV_LPUT(env, dbp->handle_lock)) != 0) {
				LOCK_INIT(dbp->handle_lock);
				goto err;
			}
			goto retry;
		...
		if(F_ISSET(dbp, DB_AM_IN_RENAME)) {
			...
			if(create_ok) {
				...
				else if((ret = __os_closehandle(env, fhp)) != 0)
					goto err;
				LF_SET(DB_CREATE);
				goto create;
			}
	...
	if(F_ISSET(dbp, DB_AM_INMEM)) {
		...
		if((ret = __fop_create(env, stxn, &fhp, tmpname, &dbp->dirname, aflags, mode, dflags)) != 0) {
			...
			if(!TXN_ON(env) && ret == EEXIST) {
				PERFMON3(env, race, fop_file_setup, tmpname, ret, flags);
...
creat2:
	if(!F_ISSET(dbp, DB_AM_INMEM)) {
		if((ret = __db_appname(env, aflags, tmpname, &dbp->dirname, &real_tmpname)) != 0)
			goto err;
		// Set the pagesize if it isn't yet set. 
		if(dbp->pgsize == 0 && (ret = __fop_set_pgsize(dbp, fhp, real_tmpname)) != 0)
			goto errmsg;
		// Construct a file_id. 
		if((ret = __os_fileid(env, real_tmpname, 1, dbp->fileid)) != 0)
			goto errmsg;
	}
	 ...
	 */
	if(!F_ISSET(dbp, DB_AM_COMPENSATE) && !F_ISSET(dbp, DB_AM_RECOVER) && !LOCK_ISSET(elock))
		GET_ENVLOCK(env, locker, &elock);
	if(F_ISSET(dbp, DB_AM_IN_RENAME)) {
	...
	else if(!F_ISSET(dbp, DB_AM_INMEM) && __os_exists(env, real_name, NULL) == 0) {
		//
	...
	if(name && (ret = __fop_lock_handle(env, dbp, locker, DB_LOCK_WRITE, NULL, NOWAIT_FLAG(txn))) != 0)
		goto err;
	if(tmpname && tmpname != name && (ret = __fop_rename(env, stxn, tmpname, name, &dbp->dirname, dbp->fileid, aflags, 1, dflags)) != 0)
		goto err;
	if((ret = __ENV_LPUT(env, elock)) != 0)
		goto err;
	if(stxn != NULL) {
...
err:            
	...
	// exact same string, so we need to make sure that we do not free twice.
	// 
	if(!truncating && tmpname && tmpname != name)
		__os_free(env, tmpname);
...
retry:
	...
	// the locker id, we need to acquire the handle lock for this subdatabase.
	// 
	...
	lkmode = F_ISSET(dbp, DB_AM_CREATED) || LF_ISSET(DB_WRITEOPEN) ? DB_LOCK_WRITE : DB_LOCK_READ;
	if((ret = __fop_lock_handle(env, dbp, txn == NULL ? dbp->locker : txn->locker, lkmode, NULL, NOWAIT_FLAG(txn))) != 0)
		goto err;
	if((ret = __db_init_subdb(mdbp, dbp, name, ip, txn)) != 0) {
		...
		if(F_ISSET(dbp, DB_AM_CREATED) && txn == NULL)
			__db_master_update(mdbp, dbp, ip, txn, name, dbp->type, MU_REMOVE, NULL, 0);
	...
	//
	// File exists and we have the appropriate locks; we should now process a normal open.
	//
	if(F_ISSET(mdbp, DB_AM_CREATED)) {
...
err:
	...
	if(!F_ISSET(dbp, DB_AM_RECOVER) && IS_REAL_TXN(txn)) {
		...
		__txn_remlock(env, txn, &mdbp->handle_lock, DB_LOCK_INVALIDID);
	...
	// mpool.  If we're opening a subdb in an existing file, we can skip the sync.
	// 
...
retry:
	...
	if((ret = __fop_lock_handle(env, dbp, dbp->locker, DB_LOCK_WRITE, NULL, DB_LOCK_NOWAIT)) != 0) {
		...
		if(ret != DB_LOCK_NOTGRANTED || (txn != NULL && F_ISSET(txn, TXN_NOWAIT)))
			...
			goto err;
		if(F_ISSET(dbp, DB_AM_INMEM)) {
			__lock_put(env, &dbp->handle_lock);
...
int __fop_read_meta(ENV * env, const char * name, uint8 * buf, size_t size, DB_FH * fhp, int errok, size_t * nbytesp)
{
	size_t nr = 0;
	//
	// Our caller wants to know the number of bytes read, even if we return an error.
	//
 ...
 */
int __fop_dummy(DB * dbp, DB_TXN * txn, const char * old, const char * pNewName)
{
	int ret, t_ret;
	uint8 mbuf[DBMETASIZE];
	ENV * env = dbp->env;
	...
	if((ret = __db_backup_name(env, pNewName, stxn, &back)) != 0)
		goto err;
 ...
 */
int __fop_dbrename(DB * dbp, const char * old, const char * pNewName)
{
	DB_LOCK elock;
	ENV * env;
	...
	LOCK_INIT(elock);
	if(F_ISSET(dbp, DB_AM_INMEM)) {
		real_new = (char *)pNewName;
		real_old = (char *)old;
	}
	else {
		/* Get full names. */
		if((ret = __db_appname(env, DB_APP_DATA, old, &dbp->dirname, &real_old)) != 0)
			goto err;
		if((ret = __db_appname(env, DB_APP_DATA, pNewName, &dbp->dirname, &real_new)) != 0)
			goto err;
	}
	/*
	...
	ret = __memp_nameop(env, dbp->fileid, pNewName, real_old, real_new, F_ISSET(dbp, DB_AM_INMEM));
err:
...
static int __fop_inmem_create(DB * dbp, const char * name, DB_TXN * txn, uint32 flags)
{
	...
	if(!name) {
		...
		p32++;
		strncpy((char *)p32, name, DB_FILE_ID_LEN-sizeof(uint32));
		dbp->preserve_fid = 1;
...
static int __fop_inmem_read_meta(DB * dbp, DB_TXN * txn, const char * name, uint32 flags)
{
	DBMETA * metap;
	DB_THREAD_INFO * ip;
	db_pgno_t pgno;
...
static int __fop_ondisk_swap(DB * dbp, DB * tmpdbp, DB_TXN * txn, const char * old, const char * pNewName, const char * back, DB_LOCKER * locker)
{
	...
	fhp = NULL;
	dflags = F_ISSET(dbp, DB_AM_NOT_DURABLE) ? DB_LOG_NOT_DURABLE : 0;
	if((ret = __db_appname(env, DB_APP_DATA, pNewName, &dbp->dirname, &realnew)) != 0)
		goto err;
...
retry:
	...
	if(__os_exists(env, realnew, NULL) == 0) {
		...
		if((ret = __fop_lock_handle(env, tmpdbp, locker, DB_LOCK_READ, NULL, DB_LOCK_NOWAIT)) != 0) {
			...
			if((ret = __fop_lock_handle(env, tmpdbp, locker, DB_LOCK_WRITE, &elock, 0)) != 0)
				goto err;
			if((ret = __lock_put(env, &tmpdbp->handle_lock)) != 0)
				goto err;
	 ...
	 */
	if((ret = __fop_rename(env, txn, old, pNewName, &dbp->dirname, dbp->fileid, DB_APP_DATA, 1, dflags)) != 0)
		goto err;
	if((ret = __fop_rename(env, txn, back, old, &dbp->dirname, tmpdbp->fileid, DB_APP_DATA, 0, dflags)) != 0)
		goto err;
	...
	txn = NULL;
	/*
	 * If the pNewName name is available because it was previously renamed
	...
	if((ret = __db_appname(env, DB_APP_DATA, old, &dbp->dirname, &realold)) != 0)
		goto err;
...
static int __fop_inmem_swap(DB * olddbp, DB * backdbp, DB_TXN * txn, const char * old, const char * pNewName, const char * back, DB_LOCKER * locker)
{
	DB * tmpdbp;
...
retry:
	...
	GET_ENVLOCK(env, locker, &elock);
	if((ret = __env_mpool(tmpdbp, pNewName, 0)) == 0) {
		/*
		 ...
		 * and allow this rename to succeed if that's the case.
		 */
		if((ret = __fop_inmem_read_meta(tmpdbp, txn, pNewName, 0)) != 0) {
			ret = EEXIST;
			goto err;
		}
		/*
		...
		if((ret = __fop_lock_handle(env, tmpdbp, locker, DB_LOCK_READ, NULL, DB_LOCK_NOWAIT)) != 0) {
			...
			__lock_put(env, &tmpdbp->handle_lock);
			__db_close(tmpdbp, NULL, DB_NOSYNC);
			__ENV_LPUT(env, elock);
			goto retry;
		}
		else {
	 ...
	 */
	if(F_ISSET(tmpdbp, DB_AM_IN_RENAME)) {
		if((ret = __memp_nameop(env, tmpdbp->fileid, NULL, pNewName, NULL, 1)) != 0)
			goto err;
		__txn_remrem(env, parent, pNewName);
	}
	if((ret = __memp_nameop(env, olddbp->fileid, pNewName, old, pNewName, 1)) != 0)
		goto err;
