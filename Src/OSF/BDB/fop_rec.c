...
int __fop_create_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		...
		if(__os_open(env, real_name, 0, 0, 0, &fhp) == 0) {
			if(__fop_read_meta(env, real_name, mbuf, DBMETASIZE, fhp, 1, NULL) == 0 && __db_chk_meta(env, NULL, meta, 1) == 0) {
				if((ret = __memp_nameop(env, meta->uid, NULL, real_name, NULL, 0)) != 0)
					goto out;
			}
			else {
...
do_unlink:
		__os_unlink(env, real_name, 0);
	}
	else if(DB_REDO(op)) {
		if((ret = __os_open(env, real_name, 0, DB_OSO_CREATE, (int)argp->mode, &fhp)) == 0)
			__os_closehandle(env, fhp);
		else
			goto out;
	}
...
int __fop_create_42_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		 ...
		 */
		if(__os_open(env, real_name, 0, 0, 0, &fhp) == 0) {
			if(__fop_read_meta(env, real_name, mbuf, DBMETASIZE, fhp, 1, NULL) == 0 &&
...
int __fop_remove_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_REDO(op))
		__memp_nameop(env, (uint8 *)argp->fid.data, NULL, real_name, NULL, 0);
	*lsnp = argp->prev_lsn;
out:
	__os_free(env, real_name);
	REC_NOOP_CLOSE;
...
static int __fop_rename_recover_int(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info, int undo)
{
	...
	if((ret = __db_appname(env, appname, (const char *)argp->newname.data, &dirname, &real_new)) != 0)
		goto out;
	if((ret = __db_appname(env, appname, (const char *)argp->oldname.data, &dirname, &real_old)) != 0)
		goto out;
	/*
	...
	if(op != DB_TXN_ABORT && op != DB_TXN_APPLY) {
		...
		if(__os_open(env, src, 0, 0, 0, &fhp) != 0)
			goto done;
		if(__fop_read_meta(env, src, mbuf, DBMETASIZE, fhp, 1, NULL) != 0)
			goto done;
		...
		if(DB_REDO(op)) {
			 ...
			 * point.
			 */
			if(__os_open(env, real_new, 0, 0, 0, &fhp) == 0 && __fop_read_meta(env, src, mbuf, DBMETASIZE, fhp, 1, NULL) == 0 &&
			   __db_chk_meta(env, NULL, meta, 1) == 0 && memcmp(argp->fileid.data, meta->uid, DB_FILE_ID_LEN) != 0) {
				__memp_nameop(env, fileid, NULL, real_old, NULL, 0);
				goto done;
			}
		}
	}
	if(undo && DB_UNDO(op))
		__memp_nameop(env, fileid, (const char *)argp->oldname.data, real_new, real_old, 0);
	if(DB_REDO(op))
		__memp_nameop(env, fileid, (const char *)argp->newname.data, real_old, real_new, 0);
done:
	*lsnp = argp->prev_lsn;
out:
...
static int __fop_rename_42_recover_int(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info, int undo)
{
	...
	REC_NOOP_INTRO(__fop_rename_read);
	fileid = (uint8 *)argp->fileid.data;
	if((ret = __db_appname(env, (APPNAME)argp->appname, (const char *)argp->newname.data, NULL, &real_new)) != 0)
		goto out;
	if((ret = __db_appname(env, (APPNAME)argp->appname, (const char *)argp->oldname.data, NULL, &real_old)) != 0)
		goto out;
	/*
	 * Verify that we are manipulating the correct file.  We should always
	 * be OK on an ABORT or an APPLY, but during recovery, we have to
