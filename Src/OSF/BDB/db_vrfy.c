...
static int __db_vrfy_pagezero __P((DB*, VRFY_DBINFO*, DB_FH*, uint32));
 ...
 */
int __db_verify_internal(DB * dbp, const char * fname, const char * dname, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	DB_THREAD_INFO * ip;
	int ret, t_ret;
	ENV * env = dbp->env;
...
static int __db_verify_arg(DB * dbp, const char * dname, void * handle, uint32 flags)
{
	 ...
	 */
	if((ret = __db_fcchk(env, "DB->verify", flags, DB_ORDERCHKONLY, DB_SALVAGE|DB_NOORDERCHK)) != 0)
		return ret;
 ...
 */
int __db_verify(DB * dbp, DB_THREAD_INFO * ip, const char * name, const char * subdb, void * handle, int (*callback)__P((void *, const void *)), void * lp, void * rp, uint32 flags)
{
	uint32 sflags;
	 ...
	 */
	if((ret = __env_setup(dbp, NULL, name, subdb, TXN_INVALID, DB_ODDFILESIZE|DB_RDONLY)) != 0)
		goto err;
	/*
	 * Set our name in the Queue subsystem;  we may need it later
	 * to deal with extents.
	 */
	if(dbp->type == DB_QUEUE && (ret = __qam_set_ext_data(dbp, name)) != 0)
		goto err;
 ...
 */
static int __db_vrfy_pagezero(DB * dbp, VRFY_DBINFO * vdp, DB_FH * fhp, uint32 flags)
{
	DBMETA * meta;
	ENV * env;
	VRFY_PAGEINFO * pip;
	...
	if((ret = __os_seek(env, fhp, 0, 0, 0)) != 0 ||
	   (ret = __os_read(env, fhp, mbuf, DBMETASIZE, &nr)) != 0) {
		__db_err(env, ret, DB_STR_A("0520", "Metadata page %lu cannot be read", "%lu"), (ulong)PGNO_BASE_MD);
		return ret;
	}
	if(nr != DBMETASIZE) {
...
static int __db_vrfy_walkpages(DB * dbp, VRFY_DBINFO * vdp, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	...
	for(i = 0; i <= vdp->last_pgno; i++) {
		...
		if((t_ret = __memp_fget(mpf, &i, vdp->thread_info, NULL, 0, &h)) != 0) {
			if(dbp->type == DB_HASH) {
				if((t_ret = __db_vrfy_getpageinfo(vdp, i, &pip)) != 0)
					goto err1;
				pip->type = P_INVALID;
				pip->pgno = i;
...
err1:
		...
		if(LF_ISSET(DB_SALVAGE)) {
			...
			switch(TYPE(h)) {
			    ...
			    case __P_DUPLICATE:
				isbad = 1;
				EPRINT((env, DB_STR_A("0531", "Page %lu: old-style duplicate page", "%lu"), (ulong)i));
				break;
				...
				break;
			    case P_HASHMETA:
				ret = __ham_vrfy_meta(dbp, vdp, (HMETA *)h, i, flags);
				break;
			    case P_HEAPMETA:
				ret = __heap_vrfy_meta(dbp, vdp, (HEAPMETA *)h, i, flags);
				break;
			    case P_BTREEMETA:
				ret = __bam_vrfy_meta(dbp, vdp, (BTMETA *)h, i, flags);
				break;
			    case P_QAMMETA:
				ret = __qam_vrfy_meta(dbp, vdp, (QMETA *)h, i, flags);
				break;
			    case P_QAMDATA:
				ret = __qam_vrfy_data(dbp, vdp, (QPAGE *)h, i, flags);
				break;
			    default:
				EPRINT((env, DB_STR_A("0532", "Page %lu: unknown page type %lu", "%lu %lu"), (ulong)i, (ulong)TYPE(h)));
				isbad = 1;
				break;
			}
			/*
	...
	for(i = 0; i < vdp->last_pgno+1; i++) {
		...
		if(F_ISSET(pip, VRFY_IS_ALLZEROES)
		   ...
		   && i <= vdp->meta_last_pgno
			...
			EPRINT((env, DB_STR_A("0535", "Page %lu: totally zeroed page", "%lu"), (ulong)i));
			isbad = 1;
		}
		if((ret = __db_vrfy_putpageinfo(env, vdp, pip)) != 0)
			goto err;
...
int __db_vrfy_common(DB * dbp, VRFY_DBINFO * vdp, PAGE * h, db_pgno_t pgno, uint32 flags)
{
	...
	switch(h->type) {
		...
		break;
	    default:
		EPRINT((env, DB_STR_A("0537", "Page %lu: bad page type %lu", "%lu %lu"), (ulong)pgno, (ulong)h->type));
		ret = DB_VERIFY_BAD;
	}
	pip->type = h->type;
err:
...
static int __db_vrfy_invalid(DB * dbp, VRFY_DBINFO * vdp, PAGE * h, db_pgno_t pgno, uint32 flags)
{
	...
	if(!IS_VALID_PGNO(NEXT_PGNO(h))) {
		EPRINT((env, DB_STR_A("0538", "Page %lu: invalid next_pgno %lu", "%lu %lu"), (ulong)pgno, (ulong)NEXT_PGNO(h)));
		ret = DB_VERIFY_BAD;
	}
	else
...
int __db_vrfy_datapage(DB * dbp, VRFY_DBINFO * vdp, PAGE * h, db_pgno_t pgno, uint32 flags)
{
	 ...
	 */
	if(TYPE(h) != P_IBTREE && TYPE(h) != P_IRECNO && TYPE(h) != P_HEAP && TYPE(h) != P_IHEAP) {
		if(!IS_VALID_PGNO(PREV_PGNO(h)) || PREV_PGNO(h) == pip->pgno) {
			isbad = 1;
			EPRINT((env, DB_STR_A("0539", "Page %lu: invalid prev_pgno %lu", "%lu %lu"),
				(ulong)pip->pgno, (ulong)PREV_PGNO(h)));
		}
		if(!IS_VALID_PGNO(NEXT_PGNO(h)) || NEXT_PGNO(h) == pip->pgno) {
			isbad = 1;
			EPRINT((env, DB_STR_A("0540", "Page %lu: invalid next_pgno %lu", "%lu %lu"),
				(ulong)pip->pgno, (ulong)NEXT_PGNO(h)));
		}
...
int __db_vrfy_meta(DB * dbp, VRFY_DBINFO * vdp, DBMETA * meta, db_pgno_t pgno, uint32 flags)
{
	...
	if(!__db_is_valid_magicno(meta->magic, &magtype)) {
		isbad = 1;
		EPRINT((env, DB_STR_A("0545", "Page %lu: invalid magic number", "%lu"), (ulong)pgno));
	}
	if(magtype != dbtype) {
		isbad = 1;
	...
	if(meta->pagesize != dbp->pgsize) {
		isbad = 1;
		EPRINT((env, DB_STR_A("0548", "Page %lu: invalid pagesize %lu", "%lu %lu"), (ulong)pgno, (ulong)meta->pagesize));
	}
	/* Flags */
	if(meta->metaflags != 0) {
		if(FLD_ISSET(meta->metaflags, ~(DBMETA_CHKSUM|DBMETA_PART_RANGE|DBMETA_PART_CALLBACK))) {
			isbad = 1;
			EPRINT((env, DB_STR_A("0549", "Page %lu: bad meta-data flags value %#lx", "%lu %#lx"), (ulong)PGNO_BASE_MD, (ulong)meta->metaflags));
		}
		if(FLD_ISSET(meta->metaflags, DBMETA_CHKSUM))
			F_SET(pip, VRFY_HAS_CHKSUM);
		if(FLD_ISSET(meta->metaflags, DBMETA_PART_RANGE))
	...
	if(pgno != PGNO_BASE_MD && meta->free != PGNO_INVALID) {
		isbad = 1;
		EPRINT((env, DB_STR_A("0550", "Page %lu: nonempty free list on subdatabase metadata page", "%lu"), (ulong)pgno));
	}
...
static int __db_vrfy_subdbs(DB * dbp, VRFY_DBINFO * vdp, const char * dbname, uint32 flags)
{
	...
	while((ret = __dbc_get(dbc, &key, &data, DB_NEXT)) == 0) {
		...
		if(meta_pgno == PGNO_INVALID || meta_pgno > vdp->last_pgno) {
			EPRINT((env, DB_STR_A("0557", "Subdatabase entry references invalid page %lu", "%lu"), (ulong)meta_pgno));
			isbad = 1;
			goto err;
		}
...
static int __db_vrfy_orderchkonly(DB * dbp, VRFY_DBINFO * vdp, const char * name, const char * subdb, uint32 flags)
{
	...
	switch(TYPE(h)) {
		 ...
		 */
		if(h_internal == NULL) {
			EPRINT((env, DB_STR_A("0560", "Page %lu: DB->h_internal field is NULL", "%lu"), (ulong)meta_pgno));
			ret = DB_VERIFY_BAD;
			goto err;
		}
		...
		if(hmeta->h_charkey != h_internal->h_hash(dbp, CHARKEY, sizeof(CHARKEY))) {
			EPRINT((env, DB_STR_A("0561", "Page %lu: incorrect hash function for database", "%lu"), (ulong)meta_pgno));
			ret = DB_VERIFY_BAD;
			goto err;
		}
		/*
		...
		break;
	    default:
		EPRINT((env, DB_STR_A("0562", "Page %lu: database metapage of bad type %lu", "%lu %lu"), (ulong)meta_pgno, (ulong)TYPE(h)));
		ret = DB_VERIFY_BAD;
		break;
	}
err:
 ...
 */
int __db_salvage_pg(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, PAGE * h, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	VRFY_PAGEINFO * pip;
	...
	if(!F_ISSET(pip, VRFY_HAS_SUBDBS) && !LF_ISSET(DB_VERIFY_PARTITION))
		ret = __db_prheader(dbp, NULL, 0, keyflag, handle, callback, vdp, pgno);
	if((t_ret = __db_vrfy_putpageinfo(env, vdp, pip)) != 0 && ret == 0)
		ret = t_ret;
	return ret;
}
 ...
 * PUBLIC:     PAGE *, void *, int (*)(void *, const void *), uint32));
 */
int __db_salvage_leaf(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, PAGE * h, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	ENV * env = dbp->env;
	DB_ASSERT(env, LF_ISSET(DB_SALVAGE));
	/* If we got this page in the subdb pass, we can safely skip it. */
 ...
 */
static int __db_salvage_unknowns(DB * dbp, VRFY_DBINFO * vdp, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	DBC * dbc;
	DBT unkdbt, key, * dbt;
	DB_MPOOLFILE * mpf;
	...
	while((t_ret = __db_salvage_getnext(vdp, &dbc, &pgno, &pgtype, 1)) == 0) {
		...
		switch(pgtype) {
		    ...
		    case SALVAGE_LBTREE:
		    case SALVAGE_LRECNO:
			if((t_ret = __bam_salvage(dbp, vdp, pgno, pgtype, h, handle, callback, dbt, tmp_flags|flags)) != 0 && ret == 0)
				ret = t_ret;
			break;
	...
	while((t_ret = __db_salvage_getnext(vdp, &dbc, &pgno, &pgtype, 0)) == 0) {
		...
		switch(pgtype) {
			 ...
			 */
			if((t_ret = __db_safe_goff(dbp, vdp, pgno, &key, &ovflbuf, &ovfl_bufsz, flags)) != 0 ||
 ...
 * the entry stores.
 */
#define INP_OFFSET(dbp, h, i)  ((db_indx_t)((uint8 *)((P_INP(dbp, (h)))+(i))-(uint8 *)(h)))
...
int __db_vrfy_inpitem(DB * dbp, PAGE * h, db_pgno_t pgno, uint32 i, int is_btree, uint32 flags, uint32 * himarkp, uint32 * offsetp)
{
	...
	if(inp+i >= (db_indx_t *)((uint8 *)h+*himarkp)) {
		...
		EPRINT((env, DB_STR_A("0563", "Page %lu: entries listing %lu overlaps data", "%lu %lu"), (ulong)pgno, (ulong)i));
		return DB_VERIFY_FATAL;
	}
	offset = inp[i];
	/*
	...
	if(offset <= INP_OFFSET(dbp, h, i) || offset >= dbp->pgsize) {
		EPRINT((env, DB_STR_A("0564", "Page %lu: bad offset %lu at page index %lu",
			"%lu %lu %lu"), (ulong)pgno, (ulong)offset, (ulong)i));
		return DB_VERIFY_BAD;
	}
	...
	if(is_btree) {
		...
		if(offset != DB_ALIGN(offset, sizeof(uint32))) {
			EPRINT((env, DB_STR_A("0565", "Page %lu: unaligned offset %lu at page index %lu",
				"%lu %lu %lu"), (ulong)pgno, (ulong)offset, (ulong)i));
			return DB_VERIFY_BAD;
		}
		/*
		...
		switch(B_TYPE(bk->type)) {
			...
			break;
		    default:
			EPRINT((env, DB_STR_A("0566", "Page %lu: item %lu of unrecognizable type", "%lu %lu"), (ulong)pgno, (ulong)i));
			return DB_VERIFY_BAD;
		}
		if((size_t)(offset+len) > dbp->pgsize) {
			EPRINT((env, DB_STR_A("0567", "Page %lu: item %lu extends past page boundary", "%lu %lu"), (ulong)pgno, (ulong)i));
			return DB_VERIFY_BAD;
		}
	}
	if(offsetp)
...
int __db_vrfy_duptype(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, uint32 flags)
{
	...
	switch(pip->type) {
		...
		if(!LF_ISSET(DB_ST_DUPSORT)) {
			EPRINT((env, DB_STR_A("0568", "Page %lu: sorted duplicate set in unsorted-dup database", "%lu"), (ulong)pgno));
			isbad = 1;
		}
		break;
	    case P_IRECNO:
	    case P_LRECNO:
		if(LF_ISSET(DB_ST_DUPSORT)) {
			EPRINT((env, DB_STR_A("0569", "Page %lu: unsorted duplicate set in sorted-dup database", "%lu"), (ulong)pgno));
			isbad = 1;
		}
		break;
	    default:
		...
		if(F_ISSET(pip, VRFY_IS_ALLZEROES))
			...
			EPRINT((env, DB_STR_A("0570",
					...
					"%lu %lu"), (ulong)pgno, (ulong)pip->type));
		isbad = 1;
		break;
	}
	if((ret = __db_vrfy_putpageinfo(env, vdp, pip)) != 0)
 ...
 */
int __db_salvage_duptree(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, DBT * key, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	PAGE * h;
	int ret, t_ret;
	DB_MPOOLFILE * mpf = dbp->mpf;
	...
	switch(TYPE(h)) {
		...
		if((ret = __db_vrfy_common(dbp, vdp, h, pgno, flags)) != 0)
			goto err;
		if((ret = __bam_vrfy(dbp, vdp, h, pgno, flags|DB_NOORDERCHK)) != 0 || (ret = __db_salvage_markdone(vdp, pgno)) != 0)
			goto err;
		/*
		 * We have a known-healthy internal page.  Walk it.
		 */
		if((ret = __bam_salvage_walkdupint(dbp, vdp, h, key, handle, callback, flags)) != 0)
			goto err;
		break;
	    case P_LRECNO:
	    case P_LDUP:
		if((ret = __bam_salvage(dbp, vdp, pgno, TYPE(h), h, handle, callback, key, flags)) != 0)
			goto err;
		break;
	    default:
		ret = DB_VERIFY_BAD;
...
static int __db_salvage_all(DB * dbp, VRFY_DBINFO * vdp, void * handle, int (*callback)__P((void *, const void *)), uint32 flags, int * hassubsp)
{
	...
	*hassubsp = 0;
	...
	if((t_ret = __memp_fget(mpf, &meta_pgno, vdp->thread_info, NULL, 0, &h)) == 0 &&
	   (t_ret = __db_vrfy_common(dbp, vdp, h, PGNO_BASE_MD, flags)) == 0 &&
	   (t_ret = __db_salvage_pg(dbp, vdp, PGNO_BASE_MD, h, handle, callback, flags)) == 0 &&
	...
	while((t_ret = __db_vrfy_pgset_next(pgsc, &p)) == 0) {
		if((t_ret = __memp_fget(mpf, &p, vdp->thread_info, NULL, 0, &h)) == 0 &&
		   (t_ret = __db_vrfy_common(dbp, vdp, h, p, flags)) == 0 &&
		   (t_ret = __bam_vrfy(dbp, vdp, h, p, flags|DB_NOORDERCHK)) == 0)
 ...
 */
static int __db_salvage_subdbpg(DB * dbp, VRFY_DBINFO * vdp, PAGE * master, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	...
	for(i = 0; i < NUM_ENT(master); i += P_INDX) {
		...
		if(B_TYPE(bkkey->type) == B_OVERFLOW) {
			 ...
			 */
			bo = (BOVERFLOW *)bkkey;
			if((ret = __db_safe_goff(dbp, vdp, bo->pgno, &key, &subdbname, &ovfl_bufsz, flags)) != 0) {
				err_ret = DB_VERIFY_BAD;
				continue;
			}
			/* Nul-terminate it. */
			if(ovfl_bufsz < key.size+1) {
				if((ret = __os_realloc(env, key.size+1, &subdbname)) != 0)
					goto err;
			...
			if(ovfl_bufsz < (uint32)bkkey->len+1) {
				if((ret = __os_realloc(env, bkkey->len+1, &subdbname)) != 0)
					goto err;
		...
		memcpy(&meta_pgno, (db_pgno_t *)bkdata->data, sizeof(db_pgno_t));
		...
		if(!IS_VALID_PGNO(meta_pgno) || (ret = __memp_fget(mpf, &meta_pgno, vdp->thread_info, NULL, 0, &subpg)) != 0) {
			err_ret = ret;
			continue;
		}
		/*
		...
		if((ret = __db_vrfy_common(dbp, vdp, subpg, meta_pgno, flags)) != 0) {
			err_ret = ret;
			__memp_fput(mpf, vdp->thread_info, subpg, dbp->priority);
			continue;
		}
		switch(TYPE(subpg)) {
		    case P_BTREEMETA:
			if((ret = __bam_vrfy_meta(dbp, vdp, (BTMETA *)subpg, meta_pgno, flags)) != 0) {
				err_ret = ret;
				__memp_fput(mpf, vdp->thread_info, subpg, dbp->priority);
				continue;
			}
			break;
		    case P_HASHMETA:
			if((ret = __ham_vrfy_meta(dbp, vdp, (HMETA *)subpg, meta_pgno, flags)) != 0) {
				err_ret = ret;
				__memp_fput(mpf, vdp->thread_info, subpg, dbp->priority);
				continue;
			}
		...
		if((ret = __memp_fput(mpf, vdp->thread_info, subpg, dbp->priority)) != 0) {
			err_ret = ret;
			continue;
		}
		/* Print a subdatabase header. */
		if((ret = __db_prheader(dbp, subdbname, 0, 0, handle, callback, vdp, meta_pgno)) != 0)
			goto err;
		/* Salvage meta_pgno's tree. */
		if((ret = __db_salvage(dbp, vdp, meta_pgno, handle, callback, flags)) != 0)
			err_ret = ret;
...
static int __db_salvage(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t meta_pgno, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	...
	if((ret = __db_cursor(pgset, vdp->thread_info, NULL, &pgsc, 0)) != 0)
		goto err;
	if(dbp->type == DB_QUEUE && (ret = __db_cursor(dbp, vdp->thread_info, NULL, &dbc, 0)) != 0)
		goto err;
	...
	while((ret = __db_vrfy_pgset_next(pgsc, &p)) == 0) {
		...
		if((ret = __db_salvage_pg(dbp, vdp, p, subpg, handle, callback, flags)) != 0)
			err_ret = ret;
		if(dbp->type == DB_QUEUE)
#ifdef HAVE_QUEUE
 ...
 */
static uint __db_guesspgsize(ENV * env, DB_FH * fhp)
{
	db_pgno_t i;
	size_t nr;
	uint32 guess;
