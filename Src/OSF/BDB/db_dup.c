...
int __db_ditem_nolog(DBC * dbc, PAGE * pagep, uint32 indx, uint32 nbytes)
{
	...
	if(indx != NUM_ENT(pagep))
		memmove(&inp[indx], &inp[indx+1], sizeof(db_indx_t)*(NUM_ENT(pagep)-indx));
	return 0;
}
/*
 * __db_ditem --
...
int __db_ditem(DBC * dbc, PAGE * pagep, uint32 indx, uint32 nbytes)
{
	...
	if(DBC_LOGGING(dbc)) {
		...
		if((ret = __db_addrem_log(dbp, dbc->txn, &LSN(pagep), 0,
			OP_SET(DB_REM_DUP, pagep), PGNO(pagep), (uint32)indx, nbytes, &ldbt, NULL, &LSN(pagep))) != 0)
			return ret;
	}
	else
...
int __db_pitem_nolog(DBC * dbc, PAGE * pagep, uint32 indx, uint32 nbytes, DBT * hdr, DBT * data)
{
	...
	if(indx != NUM_ENT(pagep))
		memmove(&inp[indx+1], &inp[indx], sizeof(db_indx_t)*(NUM_ENT(pagep)-indx));
	HOFFSET(pagep) -= nbytes;
...
int __db_pitem(DBC * dbc, PAGE * pagep, uint32 indx, uint32 nbytes, DBT * hdr, DBT * data)
{
	...
	if(DBC_LOGGING(dbc)) {
		...
		else if((ret = __db_addrem_log(dbp, dbc->txn, &LSN(pagep), 0, OP_SET(DB_ADD_DUP, pagep), PGNO(pagep),
