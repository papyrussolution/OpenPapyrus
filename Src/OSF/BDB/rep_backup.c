 ...
 */
static int __rep_walk_dir(ENV*env, const char * dir, FILE_LIST_CTX * context)
{
	...
	if(dir == NULL) {
		...
		VPRINT(env, (env, DB_VERB_REP_SYNC, "Walk_dir: Getting info for dir: %s", dir));
	...
	for(i = 0; i < cnt; i++) {
		...
		if((ret = __rep_get_fileinfo(env, file, subdb, &tmpfp, uid)) != 0) {
			/*
		...
		RPRINT(env, (env, DB_VERB_REP_SYNC, "Walk_dir: File %s at 0x%lx: pgsize %lu, max_pgno %lu", names[i], P_TO_ULONG(context->fillptr), (ulong)tmpfp.pgsize, (ulong)tmpfp.max_pgno));
		/*
...
retry:
		...
		ret = __rep_fileinfo_marshal(env, context->version, &tmpfp, context->fillptr, avail, &len);
		if(ret == ENOMEM) {
			/*
			...
			context->size *= 2;
			if((ret = __os_realloc(env, context->size, &context->buf)) != 0)
				goto err;
...
static int __rep_is_replicated_db(const char * name, const char * dir)
{
	 ...
	 */
	if(dir == NULL) {
		if(strcmp(name, REPMEMBERSHIP) == 0 || strcmp(name, REPLSNHIST) == 0)
			return 1;
	}
	else {
...
static int __rep_get_fileinfo(ENV*env, const char * file, const char * subdb, __rep_fileinfo_args * rfp, uint8 * uid)
{
	...
	if((ret = __db_create_internal(&dbp, env, 0)) != 0)
		goto err;
	if((ret = __db_open(dbp, ip, NULL, file, subdb, DB_UNKNOWN, DB_RDONLY|(F_ISSET(env, ENV_THREAD) ? DB_THREAD : 0), 0, PGNO_BASE_MD)) != 0)
		goto err;
	if((ret = __db_cursor(dbp, ip, NULL, &dbc, 0)) != 0)
		goto err;
	if((ret = __memp_fget(dbp->mpf, &dbp->meta_pgno, ip, dbc->txn, 0, &pagep)) != 0)
		goto err;
	/*
...
int __rep_page_req(ENV * env, DB_THREAD_INFO * ip, int eid, __rep_control_args * rp, DBT * rec)
{
	...
	RPRINT(env, (env, DB_VERB_REP_SYNC, "page_req: file %d page %lu to %lu", msgfp->filenum, (ulong)msgfp->pgno, (ulong)msgfp->max_pgno));
	/*
	 * We need to open the file and then send its pages.
	 * If we cannot open the file, we send REP_FILE_FAIL.
	 */
	VPRINT(env, (env, DB_VERB_REP_SYNC, "page_req: Open %d via mpf_open", msgfp->filenum));
	if((ret = __rep_mpf_open(env, &mpf, msgfp, 0)) != 0) {
		RPRINT(env, (env, DB_VERB_REP_SYNC, "page_req: Open %d failed", msgfp->filenum));
		if(F_ISSET(rep, REP_F_MASTER))
			__rep_send_message(env, eid, REP_FILE_FAIL, NULL, rec, 0, 0);
		else
			ret = DB_NOTFOUND;
		goto err;
	}
	...
	if(ret == 0 && t_ret != 0)
		ret = t_ret;
err:
	DB_TEST_RECOVERY_LABEL __os_free(env, msgfp);
	return ret;
}
 ...
 * PUBLIC:     DBT *, __time64_t, DB_LSN *));
 */
int __rep_update_setup(ENV * env, int eid, __rep_control_args * rp, DBT * rec, __time64_t savetime, DB_LSN * lsn)
{
	DB_LOG * dblp;
	DB_REP * db_rep;
	DB_THREAD_INFO * ip;
	...
	if((ret = __rep_update_unmarshal(env, rp->rep_version, &rup, (uint8 *)rec->data, rec->size, &next)) != 0)
		return ret;
	...
	if(F_ISSET(rep, REP_F_ABBREVIATED)) {
		...
		if((ret = __rep_walk_filelist(env, rp->rep_version, next, size, count, __rep_find_inmem, NULL)) == 0) {
			/*
			...
			REP_SYSTEM_UNLOCK(env);
			if(ret == 0 && (ret = __rep_verify_match(env, &verify_lsn, savetime)) == DB_REP_WOULDROLLBACK)
				*lsn = verify_lsn;
			__os_free(env, rup);
			return ret;
		}
...
err:    /*
	...
	if(ret != 0) {
		...
		RPRINT(env, (env, DB_VERB_REP_SYNC, "Update_setup: Error: Clear PAGE, set UPDATE again. %s", db_strerror(ret)));
...
static int __rep_remove_file(ENV * env, __rep_fileinfo_args * rfp, void * unused)
{
	 ...
	 */
	if(rfp->type == (uint32)DB_QUEUE && !FLD_ISSET(rfp->db_flags, DB_AM_INMEM)) {
		if((ret = __db_create_internal(&dbp, env, 0)) != 0)
			return ret;
		/*
		...
		if((ret = __lock_id(env, NULL, &dbp->locker)) != 0)
			goto out;
		ENV_GET_THREAD_INFO(env, ip);
		VPRINT(env, (env, DB_VERB_REP_SYNC, "QAM: Unlink %s via __qam_remove", name));
		if((ret = __qam_remove(dbp, ip, NULL, name, NULL, 0)) != 0) {
			RPRINT(env, (env, DB_VERB_REP_SYNC, "qam_remove returned %d", ret));
			goto out;
		}
	}
#endif
	...
	if(FLD_ISSET(rfp->db_flags, DB_AM_INMEM)) {
		...
		ret = __fop_remove(env, NULL, (uint8 *)rfp->uid.data, name, NULL,
			__rep_is_internal_rep_file((char *)rfp->info.data) ? DB_APP_NONE : DB_APP_DATA, 0);
#ifdef HAVE_QUEUE
out:
#endif
...
int __rep_bulk_page(ENV * env, DB_THREAD_INFO * ip, int eid, __rep_control_args * rp, DBT * rec)
{
	...
	for(ep = (uint8 *)rec->data+rec->size, p = (uint8 *)rec->data; p < ep; ) {
		...
		VPRINT(env, (env, DB_VERB_REP_SYNC, "rep_bulk_page: rep_page ret %d", ret));
		/*
...
int __rep_page(ENV*env, DB_THREAD_INFO * ip, int eid, __rep_control_args * rp, DBT * rec)
{
	...
	VPRINT(env, (env, DB_VERB_REP_SYNC, "%s: Received page %lu from file %d", msg, (ulong)msgfp->pgno, msgfp->filenum));
	/*
	...
	if(rp->rectype != REP_PAGE_FAIL) {
		...
		if((ret = __rep_write_page(env, ip, rep, msgfp)) != 0) {
			 ...
			 * DB_RUNRECOVERY?
			 */
			__db_del(db_rep->file_dbp, NULL, NULL, &key, 0);
			goto err;
		}
	}
...
static int __rep_write_page(ENV * env, DB_THREAD_INFO * ip, REP * rep, __rep_fileinfo_args * msgfp)
{
...
#ifdef HAVE_QUEUE
		ret = __qam_fget(db_rep->queue_dbc, &msgfp->pgno, DB_MPOOL_CREATE|DB_MPOOL_DIRTY, &dst);
#else
		/*
	 ...
	 */
	if((F_ISSET(env, ENV_LITTLEENDIAN) && !FLD_ISSET(msgfp->finfo_flags, REPINFO_PG_LITTLEENDIAN)) || (!F_ISSET(env, ENV_LITTLEENDIAN) &&
...
static int __rep_page_gap(ENV*env, REP * rep, __rep_fileinfo_args * msgfp, uint32 type)
{
	 ...
	 */
	VPRINT(env, (env, DB_VERB_REP_SYNC, "PAGE_GAP: pgno %lu, max_pg %lu ready %lu, waiting %lu max_wait %lu",
		(ulong)msgfp->pgno, (ulong)rfp->max_pgno, (ulong)rep->ready_pg, (ulong)rep->waiting_pg, (ulong)rep->max_wait_pg));
	if(msgfp->pgno > rep->ready_pg) {
		/*
		 * We receive a page larger than the one we're expecting.
		 */
		__os_gettime(env, &rep->last_pg_ts, 1);
		if(rep->waiting_pg == PGNO_INVALID || msgfp->pgno < rep->waiting_pg)
		...
		if(rep->ready_pg == rep->waiting_pg) {
			...
			memzero(&data, sizeof(data));
			ENV_GET_THREAD_INFO(env, ip);
			if((ret = __db_cursor(db_rep->file_dbp, ip, NULL, &dbc, 0)) != 0)
				goto err;
			/*
		...
		while(ret == 0 && rep->ready_pg == rep->waiting_pg) {
			...
			if(ret == DB_NOTFOUND || ret == DB_KEYEMPTY) {
				...
				VPRINT(env, (env, DB_VERB_REP_SYNC, "PAGE_GAP: Next cursor No next - ready %lu, waiting %lu",
					(ulong)rep->ready_pg, (ulong)rep->waiting_pg));
				break;
			}
			/*
			...
			rep->waiting_pg--;
			VPRINT(env, (env, DB_VERB_REP_SYNC, "PAGE_GAP: Next cursor ready %lu, waiting %lu", (ulong)rep->ready_pg, (ulong)rep->waiting_pg));
		}
	}
	/*
	 ...
	 */
	if((rep->waiting_pg != PGNO_INVALID && rep->ready_pg != rep->waiting_pg) || type == REP_PAGE_MORE) {
		/*
		...
		if((__rep_check_doreq(env, rep) || type == REP_PAGE_MORE) &&
		   ((ret = __rep_pggap_req(env, rep, rfp, (type == REP_PAGE_MORE) ? REP_GAP_FORCE : 0)) != 0))
			goto err;
	}
	else {
		lp->wait_ts = rep->request_gap;
...
static int __rep_clean_interrupted(ENV*env)
{
	...
	/* Step 2. */
	if((t_ret = __rep_walk_filelist(env, rep->infoversion, (uint8 *)R_ADDR(infop, rep->originfo_off), rep->originfolen, rep->nfiles, __rep_remove_by_list, NULL)) != 0 && ret == 0)
		ret = t_ret;
	/*
...
static int __rep_filedone(ENV * env, DB_THREAD_INFO * ip, int eid, REP * rep, __rep_fileinfo_args * msgfp, uint32 type)
{
	 ...
	 */
	int ret = __rep_page_gap(env, rep, msgfp, type);
	/*
	 ...
	 */
	VPRINT(env, (env, DB_VERB_REP_SYNC, "FILEDONE: have %lu pages. Need %lu.", (ulong)rep->npages, (ulong)rfp->max_pgno+1));
...
static int __rep_nextfile(ENV*env, int eid, REP * rep)
{
	...
	while(rep->curfile < rep->nfiles) {
		...
		if((ret = __rep_fileinfo_unmarshal(env, rep->infoversion, &rfp, info_ptr, rep->infolen, &nextinfo)) != 0) {
		...
		memcpy(R_ADDR(infop, rep->curinfo_off), (uint8 *)rfp, sizeof(__rep_fileinfo_args));
		...
		if(rfp->uid.size > 0)
			memcpy(curinfo->uid.data, rfp->uid.data, rfp->uid.size);
		if(rfp->info.size > 0)
			memcpy(curinfo->info.data, rfp->info.data, rfp->info.size);
		__os_free(env, rfp);
		/* Skip over regular DB's in "abbreviated" internal inits. */
		if(F_ISSET(rep, REP_F_ABBREVIATED) && !FLD_ISSET(curinfo->db_flags, DB_AM_INMEM)) {
			VPRINT(env, (env, DB_VERB_REP_SYNC, "Skipping file %d in abbreviated internal init", curinfo->filenum));
			MUTEX_LOCK(env, renv->mtx_regenv);
			__env_alloc_free(infop, R_ADDR(infop, rep->curinfo_off));
			MUTEX_UNLOCK(env, renv->mtx_regenv);
			rep->curinfo_off = INVALID_ROFF;
		...
		RPRINT(env, (env, DB_VERB_REP_SYNC, "Next file %d: pgsize %lu, maxpg %lu",
			curinfo->filenum, (ulong)curinfo->pgsize, (ulong)curinfo->max_pgno));
		msgsz = __REP_FILEINFO_SIZE+curinfo->uid.size+curinfo->info.size;
		if((ret = __os_calloc(env, 1, msgsz, &buf)) != 0)
			return ret;
		if((ret = __rep_fileinfo_marshal(env, rep->infoversion, curinfo, buf, msgsz, &len)) != 0) {
			__os_free(env, buf);
			return ret;
		}
		DB_INIT_DBT(dbt, buf, len);
		__rep_send_message(env, eid, REP_PAGE_REQ, NULL, &dbt, 0, DB_REP_ANYWHERE);
		__os_free(env, buf);

		return 0;
	}
	RPRINT(env, (env, DB_VERB_REP_SYNC, "NEXTFILE: have %d files.  RECOVER_LOG now", rep->nfiles));
	/*
	...
	if((ret = __rep_logreq_marshal(env, &lr_args, lrbuf, __REP_LOGREQ_SIZE, &len)) != 0)
		return ret;
	...
	__os_gettime(env, &lp->rcvd_ts, 1);
	lp->wait_ts = rep->request_gap;
	__rep_send_message(env, eid, REP_LOG_REQ, &rep->first_lsn, &dbt, REPCTL_INIT, DB_REP_ANYWHERE);
	REP_SYSTEM_LOCK(env);
	return 0;
}
/*
...
static int __rep_mpf_open(ENV * env, DB_MPOOLFILE ** mpfp, __rep_fileinfo_args * rfp, uint32 flags)
{
	...
	if(F_ISSET(&db, DB_AM_INMEM))
		__memp_set_flags(db.mpf, DB_MPOOL_NOFILE, 1);
	if((ret = __env_mpool(&db, (const char *)rfp->info.data, flags)) != 0) {
		__memp_fclose(db.mpf, 0);
		*mpfp = NULL;
	}
	return ret;
}
...
int __rep_pggap_req(ENV * env, REP * rep, __rep_fileinfo_args * reqfp, uint32 gapflags)
{
	...
	msgsz = __REP_FILEINFO_SIZE+tmpfp->uid.size+tmpfp->info.size;
	if((ret = __os_calloc(env, 1, msgsz, &buf)) != 0)
		goto err;
	...
	if((master = rep->master_id) != DB_EID_INVALID) {
		STAT_INC(env, rep, pg_request, rep->stat.st_pg_requested, master);
		/*
		 ...
		 */
		if((ret = __rep_fileinfo_marshal(env, rep->infoversion, tmpfp, buf, msgsz, &len)) == 0) {
			DB_INIT_DBT(max_pg_dbt, buf, len);
			DB_ASSERT(env, len == max_pg_dbt.size);
			__rep_send_message(env, master, REP_PAGE_REQ, NULL, &max_pg_dbt, 0, flags);
		}
	}
	else
...
static int __rep_queue_filedone(ENV * env, DB_THREAD_INFO * ip, REP * rep, __rep_fileinfo_args * rfp)
{
	...
	if(db_rep->queue_dbc == NULL) {
		...
		if((ret = __memp_sync_int(env, NULL, 0, DB_SYNC_CACHE|DB_SYNC_INTERRUPT_OK, NULL, NULL)) != 0)
			goto out;
		if((ret = __db_create_internal(&queue_dbp, env, 0)) != 0)
			goto out;
		flags = DB_NO_AUTO_COMMIT|(F_ISSET(env, ENV_THREAD) ? DB_THREAD : 0);
		/*
		 * We need to check whether this is in-memory so that we pass
		 * the name correctly as either the file or the database name.
		 */
		if((ret = __db_open(queue_dbp, ip, NULL, FLD_ISSET(rfp->db_flags, DB_AM_INMEM) ? NULL : (const char *)rfp->info.data,
	...
	if((ret = __queue_pageinfo(queue_dbp, &first, &last, &empty, 0, 0)) != 0)
		goto out;
	RPRINT(env, (env, DB_VERB_REP_SYNC, "Queue fileinfo: first %lu, last %lu, empty %d", (ulong)first, (ulong)last, empty));
	/*
	...
	if(rfp->max_pgno == 0) {
		...
		if(empty)
			goto out;
		if(first > last) {
			rfp->max_pgno = QAM_RECNO_PAGE(db_rep->queue_dbc->dbp, UINT32_MAX);
		}
		else
			rfp->max_pgno = last;
		RPRINT(env, (env, DB_VERB_REP_SYNC, "Queue fileinfo: First req: first %lu, last %lu", (ulong)first, (ulong)rfp->max_pgno));
		goto req;
	}
		...
		first = 1;
		rfp->max_pgno = last;
		RPRINT(env, (env, DB_VERB_REP_SYNC, "Queue fileinfo: Wrap req: first %lu, last %lu", (ulong)first, (ulong)last));
req:
		/*
...
int __rep_remove_init_file(ENV * env)
{
	...
	if(F_ISSET(rep, REP_F_ABBREVIATED))
		return 0;
	if((ret = __db_appname(env, DB_APP_NONE, REP_INITNAME, NULL, &name)) != 0)
		return ret;
	__os_unlink(env, name, 0);
	__os_free(env, name);
	return 0;
...
int __rep_reset_init(ENV * env)
{
	...
	__rep_update_args * rup = NULL;
	dbt.data = NULL;
	if((ret = __db_appname(env, DB_APP_NONE, REP_INITNAME, NULL, &init_name)) != 0)
		return ret;
	if((ret = __os_open(env, init_name, 0, DB_OSO_RDONLY, DB_MODE_600, &fhp)) != 0) {
		if(ret == ENOENT)
			ret = 0;
		goto out;
	}
	...
	if(env->dbenv->db_log_dir == NULL)
		...
		if((ret = __db_appname(env, DB_APP_NONE, env->dbenv->db_log_dir, NULL, &dir)) != 0)
			goto out;
		allocated_dir = dir;
	}
	if((ret = __rep_remove_by_prefix(env, dir, LFPREFIX, sizeof(LFPREFIX)-1, DB_APP_LOG)) != 0)
		goto out;
	/*
	 * Remove databases according to the list, and queue extent files by
	 * searching them out on a walk through the data_dir's.
	 */
	if((ret = __rep_update_unmarshal(env, dbtvers, &rup, (uint8 *)dbt.data, dbt.size, &next)) != 0)
		goto out;
	if((ret = __rep_unlink_by_list(env, dbtvers, next, dbt.size, rup->num_files)) != 0)
		goto out;
	/* Here, we've established that the file exists. */
rm:
	__os_unlink(env, init_name, 0);
out:
	__os_free(env, rup);
 ...
 */
static int __rep_get_file_list(ENV * env, DB_FH * fhp, uint32 fvers, uint32 * dbtvers, DBT * dbt)
{
#ifdef HAVE_REPLICATION_THREADS
	DBT mgrdbt;
#endif
	...
	for(i = 1; i <= 2; i++) {
		...
		if(cnt != sizeof(length))
			goto err;
		if((ret = __os_realloc(env, (size_t)length, &dbt->data)) != 0)
			goto err;
		if((ret = __os_read(env, fhp, dbt->data, length, &cnt)) != 0 || cnt != (size_t)length)
			goto err;
	}
#ifdef HAVE_REPLICATION_THREADS
	if(i == 3) {
		if((ret = __os_read(env, fhp, &mgrdbt.size, sizeof(mgrdbt.size), &cnt)) != 0)
			goto err;
		if(cnt == 0)
			goto absent;
		if(cnt != sizeof(mgrdbt.size))
			goto err;
		if((ret = __os_malloc(env, (size_t)mgrdbt.size, &mgrdbt.data)) != 0)
			goto err;
		if((ret = __os_read(env, fhp, mgrdbt.data, mgrdbt.size, &cnt)) != 0 &&
...
static int __rep_remove_by_prefix(ENV * env, const char * dir, const char * prefix, size_t pref_len, APPNAME appname /* What kind of name. */)
{
	...
	for(i = 0; i < cnt; i++) {
		if(strncmp(names[i], prefix, pref_len) == 0) {
			if((ret = __db_appname(env, appname, names[i], NULL, &namep)) != 0)
				goto out;
...
static int __rep_walk_filelist(ENV * env, uint32 version, uint8 * files, uint32 size, uint32 count, FILE_WALK_FN * fn, void * arg)
{
	...
	__rep_fileinfo_args * rfp = NULL;
	while(count-- > 0) {
		if((ret = __rep_fileinfo_unmarshal(env, version, &rfp, files, size, &next)) != 0)
			break;
