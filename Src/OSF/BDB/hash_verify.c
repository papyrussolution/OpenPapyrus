...
int __ham_vrfy_meta(DB * dbp, VRFY_DBINFO * vdp, HMETA * m, db_pgno_t pgno, uint32 flags)
{
	 ...
	 * check them.
	 */
	if(!F_ISSET(pip, VRFY_INCOMPLETE) && (ret = __db_vrfy_meta(dbp, vdp, &m->dbmeta, pgno, flags)) != 0) {
	...
	if(!LF_ISSET(DB_NOORDERCHK))
		if(m->h_charkey != hfunc(dbp, CHARKEY, sizeof(CHARKEY))) {
			EPRINT((env, DB_STR_A("1096", "Page %lu: database has custom hash function; reverify with DB_NOORDERCHK set", "%lu"), (ulong)pgno));
			/*
	...
	if(m->max_bucket > vdp->last_pgno) {
		EPRINT((env, DB_STR_A("1097", "Page %lu: Impossible max_bucket %lu on meta page", "%lu %lu"), (ulong)pgno, (ulong)m->max_bucket));
		/*
	...
	if(m->high_mask != pwr-1) {
		EPRINT((env, DB_STR_A("1098", "Page %lu: incorrect high_mask %lu, should be %lu", "%lu %lu %lu"), (ulong)pgno, (ulong)m->high_mask, (ulong)pwr-1));
		isbad = 1;
	}
	pwr >>= 1;
	if(m->low_mask != pwr-1) {
		EPRINT((env, DB_STR_A("1099", "Page %lu: incorrect low_mask %lu, should be %lu", "%lu %lu %lu"), (ulong)pgno, (ulong)m->low_mask, (ulong)pwr-1));
		isbad = 1;
	}
	...
	if(m->nelem > 0x80000000) {
		EPRINT((env, DB_STR_A("1100", "Page %lu: suspiciously high nelem of %lu", "%lu %lu"), (ulong)pgno, (ulong)m->nelem));
		isbad = 1;
		pip->h_nelem = 0;
	}
	else
	...
	for(i = 0; i < NCACHED && m->spares[i] != 0; i++) {
		...
		if(BS_TO_PAGE(mbucket, m->spares) > vdp->last_pgno) {
			EPRINT((env, DB_STR_A("1101", "Page %lu: spares array entry %d is invalid", "%lu %d"), (ulong)pgno, i));
			isbad = 1;
		}
	}
err:
...
int __ham_vrfy(DB * dbp, VRFY_DBINFO * vdp, PAGE * h, db_pgno_t pgno, uint32 flags)
{
	...
	for(ent = 0, himark = dbp->pgsize,
	    inpend = (uint32)((uint8 *)inp-(uint8 *)h);
	    ent < NUM_ENT(h); ent++)
		if(inp[ent] >= himark) {
			EPRINT((env, DB_STR_A("1102", "Page %lu: item %lu is out of order or nonsensical", "%lu %lu"), (ulong)pgno, (ulong)ent));
			isbad = 1;
			goto err;
		}
		else if(inpend >= himark) {
			EPRINT((env, DB_STR_A("1103", "Page %lu: entries array collided with data", "%lu"), (ulong)pgno));
			isbad = 1;
			goto err;
...
static int __ham_vrfy_item(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, PAGE * h, uint32 i, uint32 flags)
{
	...
	switch(HPAGE_TYPE(dbp, h, i)) {
		...
		for(offset = 0; offset < len; offset += DUP_SIZE(dlen)) {
			...
			// Make sure the second copy of the length is the same as the first.
		...
		break;
	    default:
		EPRINT((dbp->env, DB_STR_A("1109", "Page %lu: item %lu has bad type", "%lu %lu"), (ulong)pip->pgno, (ulong)i));
		ret = DB_VERIFY_BAD;
		break;
	}
err:
...
int __ham_vrfy_structure(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t meta_pgno, uint32 flags)
{
	...
	if((ret = __db_vrfy_pgset_get(pgset, vdp->thread_info, vdp->txn, meta_pgno, &p)) != 0)
		return ret;
	if(p != 0) {
		EPRINT((dbp->env, DB_STR_A("1110", "Page %lu: Hash meta page referenced twice", "%lu"), (ulong)meta_pgno));
		return DB_VERIFY_BAD;
	}
	if((ret = __db_vrfy_pgset_inc(pgset, vdp->thread_info, vdp->txn, meta_pgno)) != 0)
		return ret;
	/* Get the meta page;  we'll need it frequently. */
	if((ret = __memp_fget(mpf, &meta_pgno, vdp->thread_info, NULL, 0, &m)) != 0)
		return ret;
	 ...
	 */
	for(bucket = m->max_bucket+1; spares_entry = __db_log2(bucket+1), spares_entry < NCACHED && m->spares[spares_entry] != 0; bucket++) {
		...
		if(pip->type != P_HASH && pip->type != P_HASH_UNSORTED) {
			EPRINT((dbp->env, DB_STR_A("1111", "Page %lu: hash bucket %lu maps to non-hash page", "%lu %lu"), (ulong)pgno, (ulong)bucket));
			isbad = 1;
		}
		else if(pip->entries != 0) {
			EPRINT((dbp->env, DB_STR_A("1112", "Page %lu: non-empty page in unused hash bucket %lu", "%lu %lu"), (ulong)pgno, (ulong)bucket));
			isbad = 1;
		}
		else {
			if((ret = __db_vrfy_pgset_get(pgset, vdp->thread_info, vdp->txn, pgno, &p)) != 0)
				goto err;
			if(p != 0) {
				EPRINT((dbp->env, DB_STR_A("1113", "Page %lu: above max_bucket referenced", "%lu"), (ulong)pgno));
				isbad = 1;
			}
			else {
		...
		__db_vrfy_putpageinfo(dbp->env, vdp, pip);
		goto err;
	}
err:
...
static int __ham_vrfy_bucket(DB * dbp, VRFY_DBINFO * vdp, HMETA * m, uint32 bucket, uint32 flags)
{
	...
	if(pgno > vdp->last_pgno || (pip->type != P_HASH && pip->type != P_HASH_UNSORTED)) {
		EPRINT((env, DB_STR_A("1114", "Page %lu: impossible first page in bucket %lu", "%lu %lu"), (ulong)pgno, (ulong)bucket));
		/* Unsafe to continue. */
		isbad = 1;
		goto err;
	}
	if(pip->prev_pgno != PGNO_INVALID) {
		EPRINT((env, DB_STR_A("1115", "Page %lu: first page in hash bucket %lu has a prev_pgno", "%lu %lu"), (ulong)pgno, (ulong)bucket));
		isbad = 1;
	}
	/*
	...
	for(;; ) {
		...
		if((ret = __db_vrfy_pgset_get(vdp->pgset, vdp->thread_info, vdp->txn, pgno, &p)) != 0)
			goto err;
		if(p != 0) {
			EPRINT((env, DB_STR_A("1116", "Page %lu: hash page referenced twice", "%lu"), (ulong)pgno));
			isbad = 1;
			/* Unsafe to continue. */
			goto err;
		}
		else if((ret = __db_vrfy_pgset_inc(vdp->pgset, vdp->thread_info, vdp->txn, pgno)) != 0)
			goto err;
		/*
		...
		if(F_ISSET(pip, VRFY_HAS_DUPS) && !F_ISSET(mip, VRFY_HAS_DUPS)) {
			EPRINT((env, DB_STR_A("1117", "Page %lu: duplicates present in non-duplicate database", "%lu"), (ulong)pgno));
			isbad = 1;
		}
		/*
		...
		if(F_ISSET(mip, VRFY_HAS_DUPSORT) && F_ISSET(pip, VRFY_DUPS_UNSORTED)) {
			EPRINT((env, DB_STR_A("1118", "Page %lu: unsorted dups in sorted-dup database", "%lu"), (ulong)pgno));
			isbad = 1;
		}
		...
		for(ret = __db_vrfy_ccset(cc, pip->pgno, &child); ret == 0;
			...
			if(child->type == V_OVERFLOW) {
				if((ret = __db_vrfy_ovfl_structure(dbp, vdp, child->pgno, child->tlen, flags|DB_ST_OVFL_LEAF)) != 0) {
					if(ret == DB_VERIFY_BAD)
						isbad = 1;
					else
						goto err;
				...
				if((ret = __db_vrfy_duptype(dbp, vdp, child->pgno, flags)) != 0) {
					isbad = 1;
					continue;
				}
				if((ret = __bam_vrfy_subtree(dbp, vdp, child->pgno, NULL, NULL,
					    flags|DB_ST_RECNUM|DB_ST_DUPSET|DB_ST_TOPLEVEL, NULL, NULL, NULL)) != 0) {
					if(ret == DB_VERIFY_BAD)
						isbad = 1;
					else
						goto err;
		...
		if((ret = __db_vrfy_ccclose(cc)) != 0)
			goto err;
		if((ret = __db_cursor_int(dbp, vdp->thread_info, NULL, DB_HASH, PGNO_INVALID, 0, DB_LOCK_INVALIDID, &cc)) != 0)
			goto err;
		/* If it's safe to check that things hash properly, do so. */
		if(isbad == 0 && !LF_ISSET(DB_NOORDERCHK) && (ret = __ham_vrfy_hashing(cc, pip->entries, m, bucket, pgno, flags, hfunc)) != 0) {
			if(ret == DB_VERIFY_BAD)
				isbad = 1;
			else
				goto err;
		...
		if(pip->prev_pgno != pgno) {
			EPRINT((env, DB_STR_A("1120", "Page %lu: hash page has bad prev_pgno", "%lu"), (ulong)next_pgno));
			isbad = 1;
		}
		pgno = next_pgno;
	}
 ...
 */
int __ham_vrfy_hashing(DBC * dbc, uint32 nentries, HMETA * m, uint32 thisbucket, db_pgno_t pgno, uint32 flags, uint32(*hfunc) __P((DB*, const void *, uint32)))
{
	DBT dbt;
	DB_THREAD_INFO * ip;
	PAGE * h;
	...
	for(i = 0; i < nentries; i += 2) {
		...
		if(bucket != thisbucket) {
			EPRINT((dbp->env, DB_STR_A("1121", "Page %lu: item %lu hashes incorrectly", "%lu %lu"), (ulong)pgno, (ulong)i));
			isbad = 1;
		}
	}
err:
 ...
 */
int __ham_salvage(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, PAGE * h, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	DBT dbt, key_dbt, unkdbt;
	db_pgno_t dpgno;
	int ret, err_ret, t_ret;
	uint32 himark, i, ovfl_bufsz;
	uint8 * hk, * p;
	void * buf, * key_buf;
...
keydata:
				...
				if((ret = __os_malloc(dbp->env, dbt.size, &key_buf)) != 0)
					return ret;
				...
				for(tlen = 0; tlen+sizeof(db_indx_t) < len; tlen += dlen+2*sizeof(db_indx_t)) {
					/*
					 ...
					 */
					if(tlen != 0) {
						if((ret = __db_vrfy_prdbt(&key_dbt, 0, " ", handle, callback, 0, 0, vdp)) != 0)
							err_ret = ret;
					}
					 ...
					 */
					if(dlen+tlen+sizeof(db_indx_t) > len) {
						dlen = len-(tlen+sizeof(db_indx_t));
						err_ret = DB_VERIFY_BAD;
					}
					memcpy(buf, p, dlen);
					dbt.size = dlen;
...
int __ham_meta2pgset(DB * dbp, VRFY_DBINFO * vdp, HMETA * hmeta, uint32 flags, DB * pgset)
{
	...
	for(bucket = 0; bucket <= hmeta->max_bucket; bucket++) {
		...
		for(;; ) {
			 ...
			 */
			if((ret = __db_vrfy_pgset_get(pgset, vdp->thread_info, vdp->txn, pgno, &val)) != 0)
				return ret;
			if(val != 0)
				break;
		}
