...
redo:
	if(DB_REDO(op)) {
		...
		if(rootsplit) {
			...
			P_INIT(_lp, file_dbp->pgsize, PGNO(sp), ISINTERNAL(sp) ? PGNO_INVALID : PREV_PGNO(sp), ISINTERNAL(sp) ? PGNO_INVALID : argp->right, LEVEL(sp), TYPE(sp));
			P_INIT(_rp, file_dbp->pgsize, argp->right, ISINTERNAL(sp) ? PGNO_INVALID : sp->pgno, ISINTERNAL(sp) ? PGNO_INVALID : NEXT_PGNO(sp), LEVEL(sp), TYPE(sp));
		}
		/* Split the page. */
		if((ret = __bam_copy(file_dbp, sp, _lp, 0, argp->indx)) != 0 || (ret = __bam_copy(file_dbp, sp, _rp, argp->indx, NUM_ENT(sp))) != 0)
			goto out;
		if(l_update) {
		 ...
		 */
		if(lp && (ret = __memp_fput(mpf, ip, lp, file_dbp->priority)) && ret == 0)
			goto out;
		lp = NULL;
		if(rp && (ret = __memp_fput(mpf, ip, rp, file_dbp->priority)) && ret == 0)
			goto out;
		rp = NULL;
		/*
...
check_next:
		...
		if(rootsplit) {
			...
			if(lp && (ret = __memp_fput(mpf, ip, lp, file_dbp->priority)) != 0)
				goto out;
			lp = pp;
			pp = NULL;
		}
...
out:    /* Free any pages that are left. */
	if(lp && (t_ret = __memp_fput(mpf, ip, lp, file_dbp->priority)) != 0 && ret == 0)
		ret = t_ret;
	if(np && (t_ret = __memp_fput(mpf, ip, np, file_dbp->priority)) != 0 && ret == 0)
		ret = t_ret;
	if(rp && (t_ret = __memp_fput(mpf, ip, rp, file_dbp->priority)) != 0 && ret == 0)
		ret = t_ret;
	if(pp && (t_ret = __memp_fput(mpf, ip, pp, file_dbp->priority)) != 0 && ret == 0)
		ret = t_ret;
...
redo:
	if(DB_REDO(op)) {
		...
		if(p_update) {
			REC_DIRTY(mpf, ip, file_dbp->priority, &pp);
			ptype = (argp->opflags&SPL_RECNO) ? P_IRECNO : P_IBTREE;
			if(rootsplit) {
...
redo:   
	if(DB_REDO(op)) {
		...
		if(rootsplit) {
			...
			CHECK_LSN(env, op, cmp, &LSN(pp), &LSN(argp->pg.data));
			p_update = cmp  == 0;
		}
do_left:        
		if(lp != NULL) {
...
int __bam_adj_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		/* Need to redo update described. */
		REC_DIRTY(mpf, ip, dbc->priority, &pagep);
		if((ret = __bam_adjindx(dbc, pagep, argp->indx, argp->indx_copy, argp->is_insert)) != 0)
			goto out;
		LSN(pagep) = *lsnp;
	}
	else if(cmp_n == 0 && DB_UNDO(op)) {
...
int __bam_irep_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		if((ret = __bam_ritem_nolog(dbc, pagep, argp->indx, &argp->hdr, &argp->data, bn->type)) != 0)
			goto out;
		LSN(pagep) = *lsnp;
	}
	else if(cmp_n == 0 && DB_UNDO(op)) {
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __bam_merge_44_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		if(TYPE(pagep) == P_OVERFLOW) {
			...
			ninp = (db_indx_t *)argp->ind.data;
			for(i = 0; i < (int)(argp->ind.size/sizeof(*ninp)); i++)
				*pinp++ = *ninp++ - (file_dbp->pgsize-HOFFSET(pagep));
		...
		for(i = 0; i < (int)(argp->ind.size/sizeof(*ninp)); i++) {
			...
			if(P_INP(file_dbp, pagep)[indx] == P_INP(file_dbp, pagep)[indx-P_INDX]) {
				NUM_ENT(pagep)--;
				continue;
			}
			switch(TYPE(pagep)) {
			    ...
			    case P_IBTREE:
				size = BINTERNAL_SIZE(GET_BINTERNAL(file_dbp, pagep, indx)->len);
				break;
			    case P_IRECNO:
				size = RINTERNAL_SIZE;
				break;
	...
	if((ret = __memp_fput(mpf, ip, pagep, dbc->priority)) != 0)
		goto out;
next:
	if((ret = __memp_fget(mpf, &argp->npgno, ip, NULL, 0, &pagep)) != 0) {
		if(ret != DB_PAGE_NOTFOUND) {
			ret = __db_pgerr(file_dbp, argp->pgno, ret);
			goto out;
		}
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __bam_relink_43_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
...
next2:
	...
	if(modified) {
		pagep->lsn = DB_UNDO(op) ? argp->lsn_next : *lsnp;
	}
	if((ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
		goto out;
	pagep = NULL;
prev:
	...
	if(modified) {
		pagep->lsn = DB_UNDO(op) ? argp->lsn_prev : *lsnp;
	}
	if((ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
		goto out;
	pagep = NULL;
