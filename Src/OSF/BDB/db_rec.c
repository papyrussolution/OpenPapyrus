...
check_meta:
	...
	else if((ret = __memp_fget(mpf, &argp->pgno, ip, NULL, DB_MPOOL_CREATE, &pagep)) != 0)
		goto out;
	...
	if(DB_REDO(op) && (!cmp_p || !cmp_n || (IS_ZERO_LSN(copy_lsn) && LOG_COMPARE(&LSN(pagep), &argp->meta_lsn) <= 0))) {
		 ...
		 */
		if(is_meta && meta->last_pgno <= argp->pgno && argp->last_pgno <= argp->pgno) {
			if((ret = __memp_fput(mpf, ip, pagep, DB_PRIORITY_VERY_LOW)) != 0)
				goto out;
			pagep = NULL;
trunc:                  
			if((ret = __memp_ftruncate(mpf, NULL, ip, argp->pgno, MP_TRUNC_RECOVER)) != 0)
				goto out;
		}
	...
	if(op == DB_TXN_ABORT && argp->pgno != argp->last_pgno) {
		...
		if(lp != NULL) {
			...
			if(!is_meta) {
				...
				DB_ASSERT(env, argp->meta_pgno == lp[pos-1]);
			}
...
int __db_pg_init_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	CHECK_ABORT(env, op, cmp_n, &LSN(pagep), lsnp);
	if(cmp_p == 0 && DB_REDO(op)) {
		type = (TYPE(pagep) == P_HASH) ? P_HASH : ((file_dbp->type == DB_RECNO) ? P_LRECNO : P_LBTREE);
		REC_DIRTY(mpf, ip, file_dbp->priority, &pagep);
...
int __db_pg_trunc_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_REDO(op)) {
		...
		if((ret = __db_pg_truncate(dbc, NULL, pglist, NULL, &nelem, argp->next_free, &last_pgno, lsnp, 1)) != 0)
			goto out;
		if(argp->last_free != PGNO_INVALID) {
			 ...
			 */
			if((ret = __memp_fget(mpf, &argp->last_free, ip, NULL, 0, &meta)) == 0) {
				if(LOG_COMPARE(&LSN(meta), &argp->last_lsn) == 0) {
		...
		if((ret = __memp_fget(mpf, &argp->meta, ip, NULL, 0, &meta)) != 0)
			goto out;
		if(LOG_COMPARE(&LSN(meta), &argp->meta_lsn) == 0) {
			REC_DIRTY(mpf, ip, dbc->priority, &meta);
			if(argp->last_free == PGNO_INVALID) {
				meta->free = (nelem == 0) ? PGNO_INVALID : pglist->pgno;
			}
		...
		for(lp = pglist; lp < &pglist[nelem]; lp++) {
			if((ret = __memp_fget(mpf, &lp->pgno, ip, NULL, DB_MPOOL_CREATE, &pagep)) != 0)
				goto out;
			if(IS_ZERO_LSN(LSN(pagep)) || LOG_COMPARE(&LSN(pagep), lsnp) == 0) {
				REC_DIRTY(mpf, ip, dbc->priority, &pagep);
				P_INIT(pagep, file_dbp->pgsize, lp->pgno, PGNO_INVALID, lp->next_pgno, 0, P_INVALID);
				LSN(pagep) = lp->lsn;
			}
			if((ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
				goto out;
		 ...
		 */
		if(argp->last_free != PGNO_INVALID) {
			if((ret = __memp_fget(mpf, &argp->last_free, ip, NULL, DB_MPOOL_EDIT, &meta)) == 0) {
		...
		if((ret = __memp_fget(mpf, &argp->meta, ip, NULL, DB_MPOOL_EDIT, &meta)) != 0)
			goto out;
		if(LOG_COMPARE(&LSN(meta), lsnp) == 0) {
			REC_DIRTY(mpf, ip, dbc->priority, &meta);
			/*
	...
	if(op == DB_TXN_ABORT) {
		...
		if(list != NULL) {
			...
			else if(felem != 0 && list[felem-1] == pglist->pgno)
				goto done;
			else
				pos = felem;
			if((ret = __memp_extend_freelist(mpf, felem+nelem, &list)) != 0)
				goto out;
			if(pos != felem)
				memmove(&list[nelem+pos], &list[pos], sizeof(*list)*(felem-pos));
			for(lp = pglist; lp < &pglist[nelem]; lp++)
				list[pos++] = lp->pgno;
		}
	}
...
int __db_realloc_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	for(lp = pglist; lp < &pglist[nelem]; lp++) {
		...
		if(DB_REDO(op) && LOG_COMPARE(&LSN(pagep), &lp->lsn) == 0) {
			REC_DIRTY(mpf, ip, dbc->priority, &pagep);
			P_INIT(pagep, file_dbp->pgsize, lp->pgno, PGNO_INVALID, PGNO_INVALID, 0, argp->ptype);
			LSN(pagep) = *lsnp;
		}
		else if(DB_UNDO(op) && (IS_ZERO_LSN(LSN(pagep)) || LOG_COMPARE(&LSN(pagep), lsnp) == 0)) {
			REC_DIRTY(mpf, ip, dbc->priority, &pagep);
			P_INIT(pagep, file_dbp->pgsize, lp->pgno, PGNO_INVALID, lp->next_pgno, 0, P_INVALID);
			LSN(pagep) = lp->lsn;
		}
	...
	if((ret = __memp_fget(mpf, &argp->prev_pgno, ip, NULL, 0, &pagep)) != 0)
		goto out;
	cmp_n = LOG_COMPARE(lsnp, &LSN(pagep));
	cmp_p = LOG_COMPARE(&LSN(pagep), &argp->page_lsn);
	CHECK_LSN(env, op, cmp_p, &LSN(pagep), &argp->page_lsn);
	...
	if(op == DB_TXN_ABORT) {
		...
		if(list != NULL) {
			...
			pos++;
			if(pos != felem)
				memmove(&list[pos+nelem], &list[pos], nelem*sizeof(*list));
			for(lp = pglist; lp < &pglist[nelem]; lp++)
				list[pos++] = lp->pgno;
		}
	}
...
int __db_pg_sort_44_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_REDO(op)) {
		...
		if((ret = __db_pg_truncate(dbc, NULL, pglist, NULL, &nelem, PGNO_INVALID, &pgno, lsnp, 1)) != 0)
			goto out;
		if(argp->last_free != PGNO_INVALID) {
			if((ret = __memp_fget(mpf, &argp->last_free, ip, NULL, 0, &meta)) == 0) {
				if(LOG_COMPARE(&LSN(meta), &argp->last_lsn) == 0) {
		...
		for(lp = pglist; lp < &pglist[nelem]; lp++) {
			...
			if(IS_ZERO_LSN(LSN(pagep)) || LOG_COMPARE(&LSN(pagep), lsnp) == 0) {
				REC_DIRTY(mpf, ip, dbc->priority, &pagep);
				pgno = (lp == &pglist[nelem-1]) ? PGNO_INVALID : lp[1].pgno;
				P_INIT(pagep, file_dbp->pgsize, lp->pgno, PGNO_INVALID, pgno, 0, P_INVALID);
				LSN(pagep) = lp->lsn;
			}
			if((ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
				goto out;
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __db_pg_alloc_42_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	 ...
	 * but the page will be empty.
	 */
	if(IS_ZERO_LSN(LSN(pagep)) || (IS_ZERO_LSN(argp->page_lsn) && IS_INIT_LSN(LSN(pagep))))
		cmp_p = 0;
	CHECK_LSN(env, op, cmp_p, &LSN(pagep), &argp->page_lsn);
	/*
	 * Another special case we have to handle is if we ended up with a
...
do_truncate:
	 ...
	 */
	if((!pagep || IS_ZERO_LSN(LSN(pagep))) && IS_ZERO_LSN(argp->page_lsn) && DB_UNDO(op)) {
no_rollback:    
 ...
 */
static int __db_pg_free_recover_42_int(ENV * env, DB_THREAD_INFO * ip, __db_pg_freedata_42_args * argp, DB * file_dbp, DB_LSN * lsnp, DB_MPOOLFILE * mpf, db_recops op, int data)
{
	DBMETA * meta = 0;
	DB_LSN copy_lsn;
	PAGE * pagep = 0;
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __db_pg_free_42_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __db_pg_freedata_42_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
 ...
 * PUBLIC:  (ENV *, DBT *, DB_LSN *, db_recops, void *);
 */
int __db_relink_42_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
...
next2:
	...
	if((argp->opcode == DB_REM_PAGE_COMPAT && cmp_p == 0 && DB_REDO(op)) || (argp->opcode == DB_ADD_PAGE_COMPAT && cmp_n == 0 && DB_UNDO(op))) {
		/* Redo the remove or undo the add. */
		REC_DIRTY(mpf, ip, file_dbp->priority, &pagep);
		pagep->prev_pgno = argp->prev;
		modified = 1;
	}
	else if((argp->opcode == DB_REM_PAGE_COMPAT && cmp_n == 0 && DB_UNDO(op)) || (argp->opcode == DB_ADD_PAGE_COMPAT && cmp_p == 0 && DB_REDO(op))) {
...
int __db_relink_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(argp->next_pgno == PGNO_INVALID)
		goto prev;
	if((ret = __memp_fget(mpf, &argp->next_pgno, ip, NULL, 0, &pagep)) != 0) {
		if(ret != DB_PAGE_NOTFOUND) {
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		REC_DIRTY(mpf, ip, file_dbp->priority, &pagep);
		pagep->prev_pgno = (argp->new_pgno == PGNO_INVALID) ? argp->prev_pgno : argp->new_pgno;
		pagep->lsn = *lsnp;
	}
	else if(cmp_n == 0 && DB_UNDO(op)) {
...
prev:
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		/* Redo the relink. */
		REC_DIRTY(mpf, ip, file_dbp->priority, &pagep);
		pagep->next_pgno = (argp->new_pgno == PGNO_INVALID) ? argp->next_pgno : argp->new_pgno;
		pagep->lsn = *lsnp;
	}
	else if(cmp_n == 0 && DB_UNDO(op)) {
		/* Undo the relink. */
...
int __db_merge_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		if(argp->pg_copy) {
			...
			P_INIT(pagep, file_dbp->pgsize, pagep->pgno, PREV_PGNO(argp->hdr.data), NEXT_PGNO(argp->hdr.data), LEVEL(argp->hdr.data), TYPE(argp->hdr.data));
		}
...
err:
		...
		for(i = 0; i < NUM_ENT(argp->hdr.data); i++) {
			...
			if(TYPE(pagep) == P_LBTREE && indx != 0 && P_INP(file_dbp, pagep)[indx] == P_INP(file_dbp, pagep)[indx-P_INDX]) {
				NUM_ENT(pagep)--;
				continue;
			}
			switch(TYPE(pagep)) {
			    ...
			    case P_IRECNO:
					...
					break;
			    case P_HASH:
					size = LEN_HITEM(file_dbp, pagep, file_dbp->pgsize, indx);
					break;
			    default:
...
next:
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		if(op == DB_TXN_ABORT) {
			...
			/*
			 * If we detected a metadata page above, move the handle locks to the new page.
			 */
			if(i == 1) {
				handle_lock = file_dbp->handle_lock;
				if((ret = __fop_lock_handle(file_dbp->env, file_dbp, file_dbp->locker, DB_LOCK_READ, NULL, 0)) != 0)
					goto out;
				/* Move the other handles to the new lock. */
				if((ret = __lock_change(file_dbp->env, &handle_lock, &file_dbp->handle_lock)) != 0)
					goto out;
			}
		}
	}
...
int __db_pgno_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if((cmp_p == 0 && DB_REDO(op)) || (cmp_n == 0 && !DB_REDO(op))) {
		switch(TYPE(pagep)) {
		    case P_IBTREE:
				// An internal record can have both a overflow and child pointer.  Fetch the page to see which it is.
				bi = GET_BINTERNAL(file_dbp, pagep, argp->indx);
				if(B_TYPE(bi->type) == B_OVERFLOW) {
					REC_FGET(mpf, ip, argp->npgno, &npagep, out);
					pgnop = (TYPE(npagep) == P_OVERFLOW) ? &((BOVERFLOW *)(bi->data))->pgno : &bi->pgno;
					if((ret = __memp_fput(mpf, ip, npagep, file_dbp->priority)) != 0)
						goto out;
					break;
				}
				pgnop = &bi->pgno;
				break;
		    case P_IRECNO:
				pgnop = &GET_RINTERNAL(file_dbp, pagep, argp->indx)->pgno;
				break;
		    case P_HASH:
				pgnop = &pgno;
				break;
		    default:
				pgnop = &GET_BOVERFLOW(file_dbp, pagep, argp->indx)->pgno;
				break;
		}
		if(DB_REDO(op)) {
			/* Need to redo update described. */
		...
		if(TYPE(pagep) == P_HASH)
			memcpy(HOFFDUP_PGNO(P_ENTRY(file_dbp, pagep, argp->indx)), pgnop, sizeof(db_pgno_t));
	}
	if((ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
		goto out;
done:
