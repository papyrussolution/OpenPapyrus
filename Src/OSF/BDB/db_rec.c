...
trunc:                  if((ret = __memp_ftruncate(mpf, NULL, ip,
	...
	if(meta != NULL && (ret = __memp_fput(mpf, ip,  meta, file_dbp->priority)) != 0)
		goto out;
	meta = NULL;
	ret = 0;
out:
...
check_meta:
	...
	if(pagep != NULL && (ret = __memp_fput(mpf, ip, pagep, file_dbp->priority)) != 0)
		goto out;
	pagep = NULL;
	if(meta != NULL && (ret = __memp_fput(mpf, ip, meta, file_dbp->priority)) != 0)
		goto out;
