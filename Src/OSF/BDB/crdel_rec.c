...
int __crdel_metasub_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if((ret = __memp_fget(mpf, &argp->pgno, ip, NULL, 0, &pagep)) != 0) {
		 ...
		 */
		if((file_dbp->type == DB_HEAP || F_ISSET(file_dbp, DB_AM_INMEM)) && (ret = __memp_fget(mpf, &argp->pgno, ip, NULL,
...
int __crdel_inmem_rename_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_REDO(op))
		__memp_nameop(env, fileid, (const char *)argp->newname.data, (const char *)argp->oldname.data, (const char *)argp->newname.data, 1);
	if(DB_UNDO(op))
		__memp_nameop(env, fileid, (const char *)argp->oldname.data, (const char *)argp->newname.data, (const char *)argp->oldname.data, 1);
	*lsnp = argp->prev_lsn;
	ret = 0;
	REC_NOOP_CLOSE;
}
