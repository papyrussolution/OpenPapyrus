...
int __bam_stat_print(DBC * dbc, uint32 flags)
{
	...
	__db_msg(env, "%lx\tBtree magic number", (ulong)sp->bt_magic);
	__db_msg(env, "%lu\tBtree version number", (ulong)sp->bt_version);
	...
	if(dbp->type == DB_BTREE)
		__db_dl(env, "Minimum keys per-page", (ulong)sp->bt_minkey);
	if(dbp->type == DB_RECNO) {
		__db_dl(env, "Fixed-length record size", (ulong)sp->bt_re_len);
		__db_msg(env, "%#x\tFixed-length record pad", (uint)sp->bt_re_pad);
	}
	__db_dl(env, "Underlying database page size", (ulong)sp->bt_pagesize);
	if(dbp->type == DB_BTREE)
		__db_dl(env, "Overflow key/data size", ((BTREE_CURSOR *)dbc->internal)->ovflsize);
	__db_dl(env, "Number of levels in the tree", (ulong)sp->bt_levels);
	__db_dl(env, dbp->type == DB_BTREE ? "Number of unique keys in the tree" : "Number of records in the tree", (ulong)sp->bt_nkeys);
	__db_dl(env, "Number of data items in the tree", (ulong)sp->bt_ndata);
	__db_dl(env, "Number of tree internal pages", (ulong)sp->bt_int_pg);
	__db_dl_pct(env, "Number of bytes free in tree internal pages", (ulong)sp->bt_int_pgfree, DB_PCT_PG(sp->bt_int_pgfree, sp->bt_int_pg, sp->bt_pagesize), "ff");
	__db_dl(env, "Number of tree leaf pages", (ulong)sp->bt_leaf_pg);
	__db_dl_pct(env, "Number of bytes free in tree leaf pages", (ulong)sp->bt_leaf_pgfree, DB_PCT_PG(sp->bt_leaf_pgfree, sp->bt_leaf_pg, sp->bt_pagesize), "ff");
	__db_dl(env, "Number of tree duplicate pages", (ulong)sp->bt_dup_pg);
	__db_dl_pct(env, "Number of bytes free in tree duplicate pages", (ulong)sp->bt_dup_pgfree, DB_PCT_PG(sp->bt_dup_pgfree, sp->bt_dup_pg, sp->bt_pagesize), "ff");
	__db_dl(env, "Number of tree overflow pages", (ulong)sp->bt_over_pg);
	__db_dl_pct(env, "Number of bytes free in tree overflow pages", (ulong)sp->bt_over_pgfree, DB_PCT_PG(sp->bt_over_pgfree, sp->bt_over_pg, sp->bt_pagesize), "ff");
	__db_dl(env, "Number of empty pages", (ulong)sp->bt_empty_pg);
	__db_dl(env, "Number of pages on the free list", (ulong)sp->bt_free);
	__os_ufree(env, sp);
	return 0;
}
/*
...
int __bam_key_range(DBC * dbc, DBT * dbt, DB_KEY_RANGE * kp, uint32 flags)
{
	...
	for(sp = cp->sp; sp <= cp->csp; ++sp) {
		...
		if(sp->indx == 0)
			...
			kp->greater += factor*((sp->entries-sp->indx)-1)/sp->entries;
		}
 ...
 */
int __bam_traverse(DBC * dbc, db_lockmode_t mode, db_pgno_t root_pgno, int (*callback)__P((DBC*, PAGE*, void *, int *)), void * cookie)
{
	BINTERNAL * bi;
	BKEYDATA * bk;
	DB_LOCK lock;
	...
	switch(TYPE(h)) {
		...
		for(indx = 0; indx < NUM_ENT(h); indx += O_INDX) {
			...
			if(B_TYPE(bi->type) == B_OVERFLOW && (ret = __db_traverse_big(dbc, ((BOVERFLOW *)bi->data)->pgno, callback, cookie)) != 0)
				goto err;
			if((ret = __bam_traverse(dbc, mode, bi->pgno, callback, cookie)) != 0)
				goto err;
		}
		break;
	    case P_IRECNO:
		for(indx = 0; indx < NUM_ENT(h); indx += O_INDX) {
			...
			if((ret = __bam_traverse(dbc, mode, ri->pgno, callback, cookie)) != 0)
				goto err;
		}
		break;
	    case P_LBTREE:
		...
		for(indx = 0; indx < NUM_ENT(h); indx += P_INDX) {
			...
			if(B_TYPE(bk->type) == B_OVERFLOW && (indx+P_INDX >= NUM_ENT(h) || inp[indx] != inp[indx+P_INDX])) {
				if((ret = __db_traverse_big(dbc, GET_BOVERFLOW(dbp, h, indx)->pgno, callback, cookie)) != 0)
					goto err;
			}
			bk = GET_BKEYDATA(dbp, h, indx+O_INDX);
			if(B_TYPE(bk->type) == B_DUPLICATE && (ret = __bam_traverse(dbc, mode, GET_BOVERFLOW(dbp, h, indx+O_INDX)->pgno, callback, cookie)) != 0)
				goto err;
			if(B_TYPE(bk->type) == B_OVERFLOW && (ret = __db_traverse_big(dbc, GET_BOVERFLOW(dbp, h, indx+O_INDX)->pgno, callback, cookie)) != 0)
				goto err;
		}
		break;
	    case P_LDUP:
		...
		for(indx = 0; indx < NUM_ENT(h); indx += O_INDX) {
			bk = GET_BKEYDATA(dbp, h, indx);
			if(B_TYPE(bk->type) == B_OVERFLOW && (ret = __db_traverse_big(dbc, GET_BOVERFLOW(dbp, h, indx)->pgno, callback, cookie)) != 0)
				goto err;
		}
		break;
	    default:
