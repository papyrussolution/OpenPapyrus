...
static int __env_print_stats(ENV * env, uint32 flags)
{
	...
	__db_dlbytes(env, "Current region size", (ulong)0, (ulong)0, (ulong)infop->rp->size);
	__db_dlbytes(env, "Maximum region size", (ulong)0, (ulong)0, (ulong)infop->rp->max);
	return 0;
}
/*
 * __env_print_all --
...
static int __env_print_dbenv_all(ENV * env, uint32 flags)
{
	...
	__db_prflags(env, NULL, dbenv->flags, db_env_fn, NULL, "\tPublic environment flags");
	return 0;
}
/*
 * __env_print_env_all --
...
static int __env_print_env_all(ENV * env, uint32 flags)
{
	...
	for(rp = (REGION *)R_ADDR(infop, renv->region_off), i = 0; i < renv->region_cnt; ++i, ++rp) {
		...
		__db_dlbytes(env, "Size", (ulong)0, (ulong)0, (ulong)rp->size);
	}
	__db_prflags(env, NULL, renv->init_flags, ofn, NULL, "\tInitialization flags");
 ...
 *	Display a big value.
 *
 * PUBLIC: void __db_dl __P((ENV *, const char *, ulong));
 */
void __db_dl(ENV * env, const char * msg, ulong value)
{
	/*
 ...
 * PUBLIC:          __P((ENV *, const char *, ulong, int, const char *));
 */
void __db_dl_pct(ENV * env, const char * msg, ulong value, int pct, const char * tag)
{
	DB_MSGBUF mb;
	DB_MSGBUF_INIT(&mb);
	/*
 ...
 * PUBLIC:     __P((ENV *, const char *, ulong, ulong, ulong));
 */
void __db_dlbytes(ENV * env, const char * msg, ulong gbytes, ulong mbytes, ulong bytes)
{
	DB_MSGBUF mb;
	const char * sep;
	DB_MSGBUF_INIT(&mb);
