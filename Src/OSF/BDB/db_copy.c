...
retry:
	...
	strcpy(path, target);
	strncat(path, &PATH_SEPARATOR[0], 1);
	strcat(path, dbfile);
	if((ret = __os_open(env, path, 0, DB_OSO_CREATE|DB_OSO_TRUNC, DB_MODE_600, &fp)) != 0) {
		dbenv->err(dbenv, ret, DB_STR_A("0704", "Cannot open traget file: %s", "%s"), path);
		goto err;
	}
	mpf = dbp->get_mpf(dbp);
 ...
 */
static int copy_queue_extents(DB * dbp, const char * target, const char * passwd)
{
	DBC * dbc;
	DB_ENV * dbenv;
	DB_FH * fp;
...
again:
	...
	for(pgno = QAM_RECNO_PAGE(dbp, first); pgno <= stop; pgno++) {
		if(extid != QAM_PAGE_EXTENT(dbp, pgno)) {
			...
			sprintf(path, QUEUE_EXTENT, target, PATH_SEPARATOR[0], dbp->fname, extid);
			if((ret = __os_open(env, path, 0, DB_OSO_CREATE|DB_OSO_TRUNC, DB_MODE_600, &fp)) != 0) {
				dbenv->err(dbenv, ret,  DB_STR_A("0706", "Cannot open traget file: %s", "%s"), path);
				goto err;
			}
		}
		...
		if(ret == DB_PAGE_NOTFOUND || ret == ENOENT) {
			 ...
			 * the next extent.
			 */
			pgno = QAM_PAGE_EXTENT(dbp, pgno+page_ext)*page_ext;
