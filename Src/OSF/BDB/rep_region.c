...
int __rep_open(ENV*env)
{
	...
	if(renv->rep_off == INVALID_ROFF) {
		 ...
		 */
		if((F_ISSET(db_rep, DBREP_APP_REPMGR) && F_ISSET(rep, REP_F_APP_BASEAPI)) ||
		   (F_ISSET(db_rep, DBREP_APP_BASEAPI) && F_ISSET(rep, REP_F_APP_REPMGR))) {
...
int __rep_env_refresh(ENV*env)
{
	...
	if(F_ISSET(env, ENV_PRIVATE)) {
		if(rep != NULL) {
			...
			while((waiter = SH_TAILQ_FIRST(&rep->free_waiters, __rep_waiter)) != NULL) {
...
static int __rep_egen_init(ENV*env, REP * rep)
{
	...
	size_t cnt;
	char * p;
	if((ret = __db_appname(env, DB_APP_NONE, REP_EGENNAME, NULL, &p)) != 0)
		return ret;
	/*
	...
	if(__os_exists(env, p, NULL) != 0) {
		...
		if((ret = __os_read(env, fhp, &rep->egen, sizeof(uint32), &cnt)) != 0 || cnt != sizeof(uint32))
			goto err1;
		RPRINT(env, (env, DB_VERB_REP_MISC, "Read in egen %lu", (ulong)rep->egen));
err1:
...
static int __rep_gen_init(ENV*env, REP * rep)
{
	...
	size_t cnt;
	char * p;
	if((ret = __db_appname(env, DB_APP_NONE, REP_GENNAME, NULL, &p)) != 0)
		return ret;
	if(__os_exists(env, p, NULL) != 0) {
		/*
		...
		if((ret = __os_read(env, fhp, &rep->gen, sizeof(uint32), &cnt)) < 0 || cnt == 0)
			goto err1;
		RPRINT(env, (env, DB_VERB_REP_MISC, "Read in gen %lu", (ulong)rep->gen));
err1:
		__os_closehandle(env, fhp);
	}
err:
