...
int __db_remove_int(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, const char * name, const char * subdb, uint32 flags)
{
	 ...
	 */
	if(!F_ISSET(dbp, DB_AM_INMEM) && (ret = __db_appname(env, DB_APP_DATA, name, &dbp->dirname, &real_name)) != 0)
		goto err;
	/*
	 ...
	 * might not exist.
	 */
	if(!F_ISSET(dbp, DB_AM_INMEM) && LF_ISSET(DB_FORCE) && (ret = __db_backup_name(env, real_name, NULL, &tmpname)) == 0)
...
static int __db_dbtxn_remove(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, const char * name, const char * subdb)
{
	ENV * env;
	int ret;
	char * tmpname;
	 ...
	 */
	if((ret = __db_backup_name(env, F_ISSET(dbp, DB_AM_INMEM) ? subdb : name, txn, &tmpname)) != 0)
		return ret;
	DB_TEST_RECOVERY(dbp, DB_TEST_PREDESTROY, ret, name);
	if((ret = __db_rename_int(dbp, txn->thread_info, txn, name, subdb, tmpname, DB_NOSYNC)) != 0)
		goto err;
	/*
