...
master:         LOG_SYSTEM_LOCK(env);
	 ...
	 */
	if(IS_USING_LEASES(env) && (timeout = __rep_lease_waittime(env)) != 0) {
		 ...
		 * check this flag.
		 */
		if((ret = __rep_wait(env, &timeout, 0, egen, REP_E_PHASE0)) != 0)
			goto envleave;
		REP_SYSTEM_LOCK(env);
		 ...
		 */
		RPRINT(env, (env, DB_VERB_REP_ELECT, "after PHASE0 wait, flags 0x%x, elect_flags 0x%x", rep->flags, rep->elect_flags));
		if(!FLD_ISSET(repflags, REP_E_PHASE0) || __rep_islease_granted(env) || egen != rep->egen)
			goto unlck_lv;
	...
	if((ret = __rep_elect_init(env, nsites, ack, &in_progress, &orig_tally)) != 0)
		goto unlck_lv;
	/*
	...
	rep->elect_th++;
	egen = rep->egen;
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Election thread owns egen %lu", (ulong)egen));
	 ...
	 */
	INIT_LSN(lsn);
	if(ISSET_LOCKOUT_BDB(rep) || IN_INTERNAL_INIT(rep) || rep->sync_state == SYNC_UPDATE) {
		RPRINT(env, (env, DB_VERB_REP_ELECT, "Setting priority 0, unelectable, due to internal init/recovery"));
		priority = 0;
		ctlflags = 0;
		data_gen = 0;
	}
		 ...
		 */
		if((ret = __rep_log_backup(env, logc, &lsn, REP_REC_COMMIT)) == DB_NOTFOUND) {
			INIT_LSN(lsn);
			ret = 0;
		}
	...
	if((ret = __rep_write_egen(env, rep, egen+1)) != 0)
		goto err_locked;
	/* Tally our own vote */
	if((ret = __rep_tally(env, rep, rep->eid, &rep->sites, egen, 1)) != 0) {
		/*
	...
	__rep_cmp_vote(env, rep, rep->eid, &lsn, priority, rep->gen, data_gen, tiebreaker, ctlflags);
...
vote:
		...
		if(rep->winner == rep->eid) {
			if((ret = __rep_tally(env, rep, rep->eid, &rep->votes, egen, 2)) != 0 && ret != DB_REP_IGNORE)
				goto err_locked;
			RPRINT(env, (env, DB_VERB_REP_ELECT, "Counted my vote %d", rep->votes));
		}
	...
	if(send_vote != rep->eid) {
		...
		__rep_send_vote(env, NULL, 0, 0, 0, 0, egen, 0, send_vote, REP_VOTE2, 0);
		/*
...
phase2:
	...
	RPRINT(env, (env, DB_VERB_REP_ELECT, "After phase 2: votes %d, nvotes %d, nsites %d", rep->votes, rep->nvotes, rep->nsites));
	if(I_HAVE_WON(rep, rep->winner)) {
...
out:
	...
	rep->elect_th--;
	if(rep->elect_th == 0) {
		need_req = F_ISSET(rep, REP_F_SKIPPED_APPLY) && !I_HAVE_WON(rep, rep->winner);
	...
	if(elected) {
		...
		if((t_ret = __rep_fire_elected(env, rep, egen)) != 0 && ret == 0)
			ret = t_ret;
	}
	RPRINT(env, (env, DB_VERB_REP_ELECT, "%s %d, e_th %lu, egen %lu, flag 0x%lx, e_fl 0x%lx, lo_fl 0x%lx",
		"Ended election with ", ret, (ulong)rep->elect_th, (ulong)rep->egen,
		(ulong)rep->flags, (ulong)rep->elect_flags, (ulong)rep->lockout_flags));
	if(0) {
...
int __rep_vote1(ENV * env, __rep_control_args * rp, DBT * rec, int eid)
{
	...
	/*
	 * In 4.7 we changed to having fixed sized uint32's from
	 * non-fixed 'int' fields in the vote structure.
	 */
	if(rp->rep_version < DB_REPVERSION_47) {
		ovi = (REP_OLD_VOTE_INFO *)rec->data;
		tmpvi.egen = ovi->egen;
		tmpvi.nsites = (uint32)ovi->nsites;
		tmpvi.nvotes = (uint32)ovi->nvotes;
		tmpvi.priority = (uint32)ovi->priority;
	 ...
	 */
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Received vote1 egen %lu, egen %lu", (ulong)vi->egen, (ulong)rep->egen));
	if(vi->egen < rep->egen) {
		RPRINT(env, (env, DB_VERB_REP_ELECT, "Received old vote %lu, egen %lu, ignoring vote1", (ulong)vi->egen, (ulong)rep->egen));
	...
	if(vi->egen > rep->egen) {
		RPRINT(env, (env, DB_VERB_REP_ELECT, "Received VOTE1 from egen %lu, my egen %lu", (ulong)vi->egen, (ulong)rep->egen));
		/*
	...
	if(IS_PHASE1_DONE(rep)) {
		...
		__rep_send_vote(env, NULL, 0, 0, 0, 0, egen, 0, master, REP_VOTE2, 0);
	}
	else
err:
	REP_SYSTEM_UNLOCK(env);
	...
	if(elected)
		...
		__rep_send_vote(env, &vote1.lsn, vote1.nsites, vote1.nvotes, vote1.priority,
			vote1.tiebreaker, egen, vote1.data_gen, eid, REP_VOTE1, vote1.ctlflags);
	return ret;
}
/*
 * __rep_vote2 --
 ...
 */
int __rep_vote2(ENV * env, __rep_control_args * rp, DBT * rec, int eid)
{
	DB_LSN lsn;
	REP_OLD_VOTE_INFO * ovi;
	__rep_vote_info_args tmpvi, * vi;
	...
	if(!IN_ELECTION_TALLY(rep) && vi->egen >= rep->egen) {
		RPRINT(env, (env, DB_VERB_REP_ELECT, "Not in election gen %lu, at %lu, got vote", (ulong)vi->egen, (ulong)rep->egen));
		ret = DB_REP_HOLDELECTION;
		goto err;
	}
	/*
	 ...
	 */
	if(vi->egen != rep->egen) {
		RPRINT(env, (env, DB_VERB_REP_ELECT, "Bad vote egen %lu.  Mine %lu", (ulong)vi->egen, (ulong)rep->egen));
		ret = 0;
		goto err;
	}
	/*
	...
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Counted vote %d of %d", rep->votes, rep->nvotes));
	if(I_HAVE_WON(rep, rep->winner)) {
		__rep_elect_master(env, rep);
...
static int __rep_tally(ENV * env, REP * rep, int eid, uint32 * countp, uint32 egen, int phase)
{
	...
	for(i = 0; i < *countp; ) {
		 ...
		 */
		if(vtp->eid == eid) {
			RPRINT(env, (env, DB_VERB_REP_ELECT, "Tally found[%d] (%d, %lu), this vote (%d, %lu)", i, vtp->eid, (ulong)vtp->egen, eid, (ulong)egen));
			if(vtp->egen >= egen)
	 ...
	 * this vote.
	 */
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Tallying VOTE%d[%d] (%d, %lu)", phase, i, eid, (ulong)egen));
	vtp->eid = eid;
	vtp->egen = egen;
	(*countp)++;
	return 0;
...
static void __rep_cmp_vote(ENV * env, REP * rep, int eid, DB_LSN * lsnp, uint32 priority, uint32 gen, uint32 data_gen, uint32 tiebreaker, uint32 flags)
{
	 ...
	 */
	if(rep->sites > 1 && (priority != 0 || LF_ISSET(REPCTL_ELECTABLE))) {
		/*
		 ...
		 */
		like_pri = (priority == 0 && rep->w_priority == 0) || (priority != 0 && rep->w_priority != 0);
		if((priority != 0 && rep->w_priority == 0) || (like_pri && data_gen > rep->w_datagen) ||
		   (like_pri && data_gen == rep->w_datagen && cmp > 0) ||
		   (cmp == 0 && (priority > rep->w_priority || (priority == rep->w_priority && (tiebreaker > rep->w_tiebreaker))))) {
			RPRINT(env, (env, DB_VERB_REP_ELECT, "Accepting new vote"));
			rep->winner = eid;
...
static void __rep_elect_master(ENV*env, REP * rep)
{
	...
	STAT_INC(env, rep, election_won, rep->stat.st_elections_won, rep->egen);
	RPRINT(env, (env, DB_VERB_REP_ELECT, "Got enough votes to win; election done; (prev) gen %lu", (ulong)rep->gen));
}
...
static int __rep_wait(ENV * env, db_timeout_t * timeoutp, int full_elect, uint32 egen, uint32 flags)
{
	...
	REP  * rep = db_rep->region;
	int    done = 0;
	uint32 timeout = *timeoutp;
	uint32 sleeptime = SLEEPTIME(timeout);
	uint32 sleeptotal = 0;
...
static int __rep_grow_sites(ENV * env, uint32 nsites)
{
	 ...
	 */
	if((ret = __env_alloc(infop, (size_t)nalloc*sizeof(REP_VTALLY), &tally)) == 0) {
 ...
 */
static void __rep_send_vote(ENV * env, DB_LSN * lsnp, uint32 nsites, uint32 nvotes,
	uint32 pri, uint32 tie, uint32 egen, uint32 data_gen, int eid, uint32 vtype, uint32 flags)
{
	DBT vote_dbt;
	REP_OLD_VOTE_INFO ovi;
	...
	if(rep->version < DB_REPVERSION_47) {
		...
		vi5.nvotes = nvotes;
		vi5.tiebreaker = tie;
		__rep_vote_info_v5_marshal(env, &vi5, buf, __REP_VOTE_INFO_SIZE, &len);
		...
		__rep_vote_info_marshal(env, &vi, buf, __REP_VOTE_INFO_SIZE, &len);
		DB_INIT_DBT(vote_dbt, buf, len);
	}
	__rep_send_message(env, eid, vtype, lsnp, &vote_dbt, flags, 0);
}
