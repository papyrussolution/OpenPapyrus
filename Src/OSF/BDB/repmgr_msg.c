...
static int rescind_pending __P((ENV*, DB_THREAD_INFO*, int, uint32, uint32));
...
static int send_permlsn_conn __P((ENV*, REPMGR_CONNECTION*, uint32, DB_LSN *));
static int serve_join_request __P((ENV*, DB_THREAD_INFO*, REPMGR_MESSAGE *));
static int serve_remove_request __P((ENV*, DB_THREAD_INFO*, REPMGR_MESSAGE *));
...
static int dispatch_app_message(ENV * env, REPMGR_MESSAGE * msg)
{
	...
	flags = F_ISSET(&meta, REPMGR_REQUEST_MSG_TYPE) ? DB_REPMGR_NEED_RESPONSE : 0;
...
static int process_message(ENV*env, DBT * control, DBT * rec, int eid)
{
	...
	generation = rep->gen;
	ret = 0;
	switch(t_ret = __rep_process_message_int(env, control, rec, eid, &lsn)) {
	    case 0:
		...
		LOCK_MUTEX(db_rep->mutex);
		ret = __repmgr_init_election(env, ELECT_F_IMMED|ELECT_F_INVITEE);
		UNLOCK_MUTEX(db_rep->mutex);
		break;
		 ...
		 * code.
		 */
		if((ret = __repmgr_become_client(env)) == 0 && FLD_ISSET(rep->config, REP_C_LEASE|REP_C_ELECTIONS) == REP_C_ELECTIONS) {
			LOCK_MUTEX(db_rep->mutex);
		...
		if(env->test_abort == DB_TEST_REPMGR_PERM)
			VPRINT(env, (env, DB_VERB_REPMGR_MISC, "ISPERM: Test hook.  Skip ACK for permlsn [%lu][%lu]", (ulong)lsn.file, (ulong)lsn.offset));
#endif
		...
		RPRINT(env, (env, DB_VERB_REPMGR_MISC, "repmgr fires would-rollback event"));
...
static int send_permlsn_conn(ENV*env, REPMGR_CONNECTION * conn, uint32 generation, DB_LSN * lsn)
{
	...
	if(conn->state == CONN_READY) {
		 ...
		 */
		if((ret = __repmgr_send_one(env, conn, REPMGR_PERMLSN, &control2, &rec2, 0)) == DB_REP_UNAVAIL)
...
static int serve_remove_request(ENV*env, DB_THREAD_INFO * ip, REPMGR_MESSAGE * msg)
{
	...
	if(eid == DB_EID_INVALID) {
		...
		RPRINT(env, (env, DB_VERB_REPMGR_MISC, "Reject request to remove current master"));
		ret = DB_REP_UNAVAIL;
		goto err;
	}
	switch(status) {
...
static int resolve_limbo_int(ENV*env, DB_THREAD_INFO * ip)
{
	...
	if((ret = finish_gmdb_update(env, ip, &key_dbt, orig_status, status, &logrec)) != 0)
		goto out;
	/* Track modified membership status in our in-memory sites array. */
	LOCK_MUTEX(db_rep->mutex);
	locked = TRUE;
	if((ret = __repmgr_set_membership(env, addr.host, addr.port, status)) != 0)
		goto out;
 ...
 * PUBLIC:     DB_THREAD_INFO *, int, uint32));
 */
int __repmgr_update_membership(ENV * env, DB_THREAD_INFO * ip, int eid, uint32 pstatus /* Provisional status */)
{
	DB_REP * db_rep;
	REPMGR_SITE * site;
	DB_TXN * txn;
...
retry:
	 ...
	 */
	if((ret = __repmgr_set_membership(env, addr.host, addr.port, pstatus)) != 0)
		goto err;
	__repmgr_set_sites(env);
	...
	UNLOCK_MUTEX(db_rep->mutex);
	locked = FALSE;
	if((ret = finish_gmdb_update(env, ip, &key_dbt, pstatus, ult_status, &logrec)) != 0)
		goto err;
	/* Track modified membership status in our in-memory sites array. */
	LOCK_MUTEX(db_rep->mutex);
	locked = TRUE;
...
retry:
	...
	UNLOCK_MUTEX(db_rep->mutex);
	marshal_site_key(env, &addr, key_buf, &key_dbt, &logrec);
	if((ret = finish_gmdb_update(env, ip, &key_dbt, cur_status, new_status, &logrec)) != 0)
		goto err;
	/* Track modified membership status in our in-memory sites array. */
	LOCK_MUTEX(db_rep->mutex);
 ...
 * really deleted.
 */
static int finish_gmdb_update(ENV*env, DB_THREAD_INFO * ip, DBT * key_dbt, uint32 prev_status, uint32 status, __repmgr_member_args * logrec)
{
	DB_REP * db_rep;
	DB_LSN lsn;
	DB_TXN * txn;
	...
	if((ret = incr_gm_version(env, ip, txn)) != 0)
		goto err;
	ZERO_LSN(lsn);
	if((ret = __repmgr_member_log(env, txn, &lsn, 0, db_rep->membership_version, prev_status, status, &logrec->host, logrec->port)) != 0)
		goto err;
err:
	if((t_ret = __db_txn_auto_resolve(env, txn, 0, ret)) != 0 && ret == 0)
		ret = t_ret;
 ...
 */
int __repmgr_hold_master_role(ENV*env, REPMGR_CONNECTION * conn)
{
	int ret, t_ret;
	DB_REP * db_rep = env->rep_handle;
	...
	if((ret = __repmgr_await_gmdbop(env)) == 0) {
		 ...
		 * lost a close race.
		 */
		if(rep->master_id != db_rep->self_eid || db_rep->client_intent)
			ret = DB_REP_UNAVAIL;
		else
...
static void marshal_site_key(ENV*env, repmgr_netaddr_t * addr, uint8 * buf, DBT * dbt, __repmgr_member_args * logrec)
{
	...
	ret = __repmgr_membership_key_marshal(env, &key, buf, MAX_MSG_BUF, &len);
	DB_ASSERT(env, ret == 0);
	DB_INIT_DBT(*dbt, buf, len);
}

static void marshal_site_data(ENV*env, uint32 status, uint8 * buf, DBT * dbt)
{
