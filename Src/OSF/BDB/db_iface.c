 ...
 */
static int __db_associate_arg(DB * dbp, DB * sdbp, int (*callback)__P((DB*, const DBT*, const DBT*, DBT *)), uint32 flags)
{
	int    ret = 0;
	ENV  * env = dbp->env;
	if(sdbp->type == DB_HEAP) {
		__db_errx(env, "Heap databases may not be used as secondary databases");
		ret = EINVAL;
	}
	else if(F_ISSET(sdbp, DB_AM_SECONDARY)) {
		__db_errx(env, DB_STR("0573", "Secondary index handles may not be re-associated"));
		ret = EINVAL;
	}
	else if(F_ISSET(dbp, DB_AM_SECONDARY)) {
	 ...
	 */
	else if(dbp->env != sdbp->env && (!F_ISSET(dbp->env, ENV_DBLOCAL) || !F_ISSET(sdbp->env, ENV_DBLOCAL))) {
		__db_errx(env, DB_STR("0577", "The primary and secondary must be opened in the same environment"));
		ret = EINVAL;
	}
	else if((DB_IS_THREADED(dbp) && !DB_IS_THREADED(sdbp)) || (!DB_IS_THREADED(dbp) && DB_IS_THREADED(sdbp))) {
		__db_errx(env, DB_STR("0578", "The DB_THREAD setting must be the same for primary and secondary"));
		ret = EINVAL;
	}
	else if(callback == NULL && (!F_ISSET(dbp, DB_AM_RDONLY) || !F_ISSET(sdbp, DB_AM_RDONLY))) {
		__db_errx(env, DB_STR("0579", "Callback function may be NULL only when database handles are read-only"));
		ret = EINVAL;
	}
	else
		ret = __db_fchk(env, "DB->associate", flags, DB_CREATE|DB_IMMUTABLE_KEY);
	return ret;
}
/*
 * __db_close_pp --
 ...
 */
static int __db_del_arg(DB * dbp, DBT * key, uint32 flags)
{
	int ret;
	ENV * env = dbp->env;
 ...
 *
 * PUBLIC: int __db_get_pp __P((DB *, DB_TXN *, DBT *, DBT *, uint32));
 */
 ...
 */
static int __db_open_arg(DB * dbp, DB_TXN * txn, const char * fname, const char * dname, DBTYPE type, uint32 flags)
{
	uint32 ok_flags;
	int ret;
	ENV * env = dbp->env;
...
#define OKFLAGS (DB_AUTO_COMMIT|DB_CREATE|DB_EXCL|DB_FCNTL_LOCKING|DB_MULTIVERSION|DB_NOMMAP|DB_NO_AUTO_COMMIT|DB_RDONLY|DB_RDWRMASTER|DB_READ_UNCOMMITTED|DB_THREAD|DB_TRUNCATE)
	...
	switch(type) {
		...
		break;
	    default:
		__db_errx(env, DB_STR_A("0593", "unknown type: %lu", "%lu"), (ulong)type);
		return EINVAL;
	}
	if(ok_flags)
 ...
 * PUBLIC:     __P((DB *, DB_TXN *, DBT *, DBT *, DBT *, uint32));
 */
 ...
 */
static int __db_put_arg(DB * dbp, DBT * key, DBT * data, uint32 flags)
{
	int    ret;
	ENV  * env = dbp->env;
	int    returnkey = 0;
 ...
 * PUBLIC:     uint32));
 */
int __db_associate_foreign_pp(DB * fdbp, DB * dbp, int (*callback)__P((DB*, const DBT*, DBT*, const DBT*, int *)), uint32 flags)
{
...
int __dbc_close_pp(DBC * dbc)
{
	...
	if(txn) {
		...
		DB_ASSERT(env, dbc->txn_cursors.tqe_next == NULL && dbc->txn_cursors.tqe_prev == NULL);
	}
	ret = __dbc_close(dbc);
	/* Release replication block. */
 ...
 *
 * PUBLIC: int __dbc_pget_pp __P((DBC *, DBT *, DBT *, DBT *, uint32));
 */
...
static int __dbc_pget_arg(DBC * dbc, DBT * pkey, uint32 flags)
{
	...
	switch(LF_ISSET(DB_OPFLAGS_MASK)) {
	    ...
	    case DB_GET_BOTH_RANGE:
			...
			if(pkey == NULL) {
				__db_errx(env, DB_STR_A("0626", "%s requires both a secondary and a primary key",
						"%s"), LF_ISSET(DB_GET_BOTH) ? "DB_GET_BOTH" : "DB_GET_BOTH_RANGE");
				return EINVAL;
			}
...
err:
	...
	if(!IS_INITIALIZED(dbc) && flags != 0 && flags != DB_KEYFIRST &&
	   flags != DB_KEYLAST && flags != DB_NODUPDATA && flags != DB_OVERWRITE_DUP)
		return __db_curinval(env);
	return 0;
}
...
static int __dbt_ferr(const DB * dbp, const char * name, const DBT * dbt, int check_thread)
{
	 ...
	 */
	if((ret = __db_fchk(env, name, dbt->flags, DB_DBT_APPMALLOC|DB_DBT_BULK|DB_DBT_DUPOK|
		DB_DBT_MALLOC|DB_DBT_REALLOC|DB_DBT_USERCOPY|DB_DBT_USERMEM|DB_DBT_PARTIAL|DB_DBT_READONLY)) != 0)
		return ret;
	switch(F_ISSET(dbt, DB_DBT_MALLOC|DB_DBT_REALLOC|DB_DBT_USERCOPY|DB_DBT_USERMEM)) {
	    case 0:
	    case DB_DBT_MALLOC:
	    case DB_DBT_REALLOC:
