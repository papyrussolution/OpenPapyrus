...
int __heap_addrem_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	else if((cmp_n == 0 && DB_UNDO(op) && argp->opcode == DB_ADD_HEAP) || (cmp_p == 0 && DB_REDO(op) && argp->opcode == DB_REM_HEAP)) {
...
int __heap_pg_alloc_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_p == 0 && DB_REDO(op)) {
		...
		if(argp->ptype == P_IHEAP && HEAP_REGION_NUM(file_dbp, argp->pgno) > meta->nregions)
		...
		if(meta->dbmeta.last_pgno != argp->last_pgno) {
			if(file_dbp->mpf->mfp->last_pgno == meta->dbmeta.last_pgno)
				trunc = 1;
		...
		if(argp->ptype == P_IHEAP && HEAP_REGION_NUM(file_dbp, argp->pgno) == meta->nregions) {
			do
				meta->nregions--;
			while(argp->last_pgno < (meta->nregions-1)*HEAP_REGION_SIZE(file_dbp));
		}
	}
	/*
...
int __heap_trunc_meta_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(cmp_n == 0 && DB_UNDO(op)) {
		...
		meta->nregions = 1;
		LSN(meta) = *lsnp;
		if((ret = __memp_ftruncate(mpf, dbc->txn, ip, PGNO_BASE_MD+1, MP_TRUNC_NOCACHE)) != 0)
			goto out;
	}
done:
	*lsnp = argp->prev_lsn;
...
int __heap_trunc_page_recover(ENV * env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op) && IS_ZERO_LSN(LSN(pagep))) {
		...
		if((ret = __memp_fput(mpf, ip, pagep, dbc->priority)) != 0)
			goto out;
		pagep = NULL;
		if((ret = __memp_fget(mpf, &argp->pgno, dbc->thread_info, dbc->txn, DB_MPOOL_FREE, &pagep)) != 0)
			goto out;
	}
done:
	*lsnp = argp->prev_lsn;
