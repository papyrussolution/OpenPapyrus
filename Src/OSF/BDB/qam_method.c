...
static int __qam_rr(DB*, DB_THREAD_INFO*, DB_TXN*, const char *, const char *, const char *, qam_name_op);
...
static int __qam_rr(DB*dbp, DB_THREAD_INFO * ip, DB_TXN * txn, const char * name, const char * subdb, const char * newname, qam_name_op op)
{
	...
	if(F_ISSET(dbp, DB_AM_OPEN_CALLED))
		...
		if((ret = __db_open(tmpdbp, ip, txn, name, NULL, DB_QUEUE, DB_RDONLY, 0, PGNO_BASE_MD)) != 0)
			goto err;
	}
