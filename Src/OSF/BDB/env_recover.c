...
int __db_apprec(ENV * env, DB_THREAD_INFO * ip, DB_LSN * max_lsn, DB_LSN * trunclsn, int update, uint32 flags)
{
	...
	do {
		...
		LOGCOPY_32(env, &txnid, (uint8 *)data.data+sizeof(uint32));
		if(txnid != 0)
			break;
	...
	if(ret != 0 || (ret = __logc_get(logc, &first_lsn, &data, DB_SET)) != 0)
		goto err;
	/* Initialize the transaction list. */
	if((ret = __db_txnlist_init(env, ip, txnid, hi_txn, max_lsn, &txninfo)) != 0)
		goto err;
	/*
	 * Pass #1
	 * Run forward through the log starting at the first relevant lsn.
	 */
	if((ret = __env_openfiles(env, logc, txninfo, &data, &first_lsn, &last_lsn, nfiles, 1)) != 0)
		goto err;
	 ...
	 */
	if((ret = __memp_sync_int(env, NULL, 0, DB_SYNC_CACHE|DB_SYNC_SUPPRESS_WRITE, NULL, NULL)) != 0)
		goto err;
	 ...
	 * log records.
	 */
	if(!max_lsn && !LF_ISSET(DB_NO_CHECKPOINT) &&
	   (ret = __txn_checkpoint(env, 0, 0, DB_CKP_INTERNAL|DB_FORCE)) != 0) {
		/*
		...
		if(max_lsn == NULL && ret == ENOSPC) {
			if(FLD_ISSET(dbenv->verbose, DB_VERB_RECOVERY))
				__db_msg(env, DB_STR_A("1515", "Recovery continuing after non-fatal checkpoint error: %s", "%s"), db_strerror(ret));
			all_recovered = 0;
		}
		else
			goto err;
	...
	if(max_lsn != NULL) {
		 ...
		 * log this fact.
		 */
		if((rectype != DB___txn_recycle || hi_txn != 0) && (ret = __txn_reset(env)) != 0)
			goto err;
	}
	else {
...
int __env_init_rec(ENV * env, uint32 version)
{
	...
	if(version != DB_LOGVERSION_42) {
		__db_errx(env, DB_STR_A("1523", "Unknown version %lu", "%lu"), (ulong)version);
		ret = EINVAL;
		goto err;
	}
...
static int __env_init_rec_42(ENV * env)
{
	int ret;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __db_relink_42_recover, DB___db_relink_42)) != 0)
		goto err;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __db_pg_alloc_42_recover, DB___db_pg_alloc_42)) != 0)
		goto err;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __db_pg_free_42_recover, DB___db_pg_free_42)) != 0)
		goto err;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __db_pg_freedata_42_recover, DB___db_pg_freedata_42)) != 0)
		goto err;
#ifdef HAVE_HASH
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __ham_metagroup_42_recover, DB___ham_metagroup_42)) != 0)
		goto err;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __ham_groupalloc_42_recover, DB___ham_groupalloc_42)) != 0)
		goto err;
#endif
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __txn_ckp_42_recover, DB___txn_ckp_42)) != 0)
		goto err;
err:
	return ret;
}

static int __env_init_rec_43(ENV * env)
{
	int ret;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __bam_relink_43_recover, DB___bam_relink_43)) != 0)
		goto err;
	/*
	 * We want to use the 4.2-based txn_regop record.
	 */
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __txn_regop_42_recover, DB___txn_regop_42)) != 0)
		goto err;
err:
	return ret;
}

static int __env_init_rec_46(ENV * env)
{
	int ret;
	if((ret = __db_add_recovery_int(env, &env->recover_dtab, __bam_merge_44_recover, DB___bam_merge_44)) != 0)
		goto err;
err:
	return ret;
}
