...
int __heap_metachk(DB * dbp, const char * name, HEAPMETA * hm)
{
	...
	if(F_ISSET(dbp, DB_AM_SWAP) && (ret = __heap_mswap(env, (PAGE *)hm)) != 0)
		return ret;
...
int __heap_read_meta(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, db_pgno_t meta_pgno, uint32 flags)
{
	...
	if((ret = __db_lget(dbc, 0, meta_pgno, DB_LOCK_READ, 0, &metalock)) != 0)
		goto err;
...
int __heap_new_file(DB * dbp, DB_THREAD_INFO * ip, DB_TXN * txn, DB_FH * fhp, const char * name)
{
	...
	if(F_ISSET(dbp, DB_AM_INMEM)) {
		...
		P_INIT(region, dbp->pgsize, 1, PGNO_INVALID, PGNO_INVALID, 0, P_IHEAP);
		LSN_NOT_LOGGED(region->lsn);
		if((ret = __db_pgout(dbp->dbenv, region->pgno, region, &pdbt)) != 0)
			goto err;
...
int __heap_create_region(DBC * dbc, db_pgno_t pgno)
{
	...
	if((ret = __db_lget(dbc, LCK_ALWAYS, meta_pgno, DB_LOCK_WRITE, 0, &meta_lock)) != 0)
		return ret;
	if((ret = __memp_fget(mpf, &meta_pgno, dbc->thread_info, NULL, DB_MPOOL_DIRTY, &meta)) != 0) {
		__LPUT(dbc, meta_lock);
		return ret;
	}
	ret = __memp_fget(mpf, &pgno, dbc->thread_info, NULL, DB_MPOOL_CREATE|DB_MPOOL_DIRTY, &region);
