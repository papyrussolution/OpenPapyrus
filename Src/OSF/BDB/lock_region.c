...
err:
	if(lt->reginfo.addr != NULL) {
		if(region_locked)
			LOCK_REGION_UNLOCK(env);
		__env_region_detach(env, &lt->reginfo, 0);
	}
	env->lk_handle = NULL;
	__os_free(env, lt);
	return ret;
...
static int __lock_region_init(ENV * env, DB_LOCKTAB * lt)
{
	...
	if((ret = __env_alloc(&lt->reginfo, region->object_t_size*sizeof(DB_HASHTAB), &addr)) != 0)
		goto mem_err;
	 ...
	 */
	extra_locks = region->stat.st_locks-((region->stat.st_locks/region->part_t_size)*region->part_t_size);
	extra_objects = region->stat.st_objects-((region->stat.st_objects/region->part_t_size)*region->part_t_size);
	for(j = 0; j < region->part_t_size; j++) {
		...
		if((ret = __env_alloc(&lt->reginfo, sizeof(struct __db_lock)*max, &lp)) != 0)
			goto mem_err;
	...
	if((ret = __env_alloc(&lt->reginfo, sizeof(DB_LOCKER)*region->stat.st_lockers, &lidp)) != 0)
		goto mem_err;
...
int __lock_env_refresh(ENV * env)
{
	...
	if(F_ISSET(env, ENV_PRIVATE)) {
		...
		for(j = 0; j < lr->part_t_size; j++) {
			...
			__env_alloc_free(reginfo, R_ADDR(reginfo, lt->part_array[j].lock_mem_off));
			__env_alloc_free(reginfo, R_ADDR(reginfo, lt->part_array[j].lockobj_mem_off));
		}
		/* Discard the object partition array. */
		__env_alloc_free(reginfo, R_ADDR(reginfo, lr->part_off));
		SH_TAILQ_INIT(&lr->free_lockers);
		__env_alloc_free(reginfo, R_ADDR(reginfo, lr->locker_mem_off));
	}
...
size_t __lock_region_max(ENV * env)
{
	...
	if(count > dbenv->lk_init_objects)
		retval += __env_alloc_size(sizeof(DB_LOCKOBJ))*(count-dbenv->lk_init_objects);
	if((count = dbenv->lk_max_lockers) == 0)
		count = DB_LOCK_DEFAULT_N;
	if(count > dbenv->lk_init_lockers)
		retval += __env_alloc_size(sizeof(DB_LOCKER))*(count-dbenv->lk_init_lockers);
...
size_t __lock_region_size(ENV * env, size_t other_alloc)
{
	...
	if(dbenv->lk_max_lockers != 0)
		...
		if(dbenv->memory_max != 0)
			count = (uint32)(((dbenv->memory_max-other_alloc)/10)/sizeof(DB_LOCKER));
		else
	...
	if((count = dbenv->lk_max_objects) == 0) {
		if(dbenv->memory_max != 0)
			count = (uint32)(((dbenv->memory_max-other_alloc)/2)/sizeof(DB_LOCKOBJ));
		else
			count = DB_LOCK_DEFAULT_N;
		if(count < dbenv->lk_init_objects)
	...
	retval += __env_alloc_size(__db_tablesize(dbenv->object_t_size)*(sizeof(DB_HASHTAB)));
#ifdef HAVE_STATISTICS
	retval += __env_alloc_size(__db_tablesize(dbenv->object_t_size)*(sizeof(DB_LOCK_HSTAT)));
#endif
	retval += __env_alloc_size(dbenv->lk_partitions*(sizeof(DB_LOCKPART)));
