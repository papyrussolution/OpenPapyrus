...
int db_env_create(DB_ENV ** dbenvpp, uint32 flags)
{
	...
	dbenv->env = env;
	env->dbenv = dbenv;
	if((ret = __db_env_init(dbenv)) != 0 || (ret = __lock_env_create(dbenv)) != 0 ||
		(ret = __log_env_create(dbenv)) != 0 || (ret = __memp_env_create(dbenv)) != 0 ||
 ...
 */
int __env_get_alloc(DB_ENV * dbenv, void *(**mal_funcp)__P((size_t)), void *(**real_funcp)__P((void *, size_t)), void (**free_funcp)__P((void *)))
{
	ASSIGN_PTR(mal_funcp, dbenv->db_malloc);
	ASSIGN_PTR(real_funcp, dbenv->db_realloc);
	ASSIGN_PTR(free_funcp, dbenv->db_free);
	return 0;
}
/*
 * __env_set_alloc --
 ...
 * PUBLIC:          void *(*)(void *, size_t), void (*)(void *)));
 */
int __env_set_alloc(DB_ENV * dbenv, void *(*mal_func)__P((size_t)), void *(*real_func)__P((void *, size_t)), void (*free_func)__P((void *)))
{
	ENV * env = dbenv->env;
...
{
	const char * __p_funcnam = "DB_ENV->get_memory_init";
	ENV * env = dbenv->env;
	switch(type) {
	    case DB_MEM_LOCK:
 ...
 */
static int __env_get_app_dispatch(DB_ENV * dbenv, int (**app_dispatchp)__P((DB_ENV*, DBT*, DB_LSN*, db_recops)))
{
	ASSIGN_PTR(app_dispatchp, dbenv->app_dispatch);
	return 0;
}
/*
 ...
 */
static int __env_set_app_dispatch(DB_ENV * dbenv, int (*app_dispatch)__P((DB_ENV*, DBT*, DB_LSN*, db_recops)))
{
	ENV * env = dbenv->env;
 ...
 */
void __env_map_flags(const FLAG_MAP * flagmap, uint mapsize, uint32 * inflagsp, uint32 * outflagsp)
{
	const FLAG_MAP * fmp;
	uint i;
...
#define OK_FLAGS (DB_AUTO_COMMIT|DB_CDB_ALLDB|DB_DATABASE_LOCKING|DB_DIRECT_DB|DB_DSYNC_DB|DB_MULTIVERSION|\
	...
	if(LF_ISSET(DB_PANIC_ENVIRONMENT)) {
		...
		if(on) {
			...
			__env_panic(env, DB_RUNRECOVERY);
		}
		else
...
#define DATA_INIT_CNT   20                      /* Start with 20 data slots. */
	if(dbenv->db_data_dir == NULL) {
		...
		dbenv->data_cnt *= 2;
		if((ret = __os_realloc(env, (uint)dbenv->data_cnt*sizeof(char **), &dbenv->db_data_dir)) != 0)
			return ret;
	}
	ret = __os_strdup(env, dir, &dbenv->db_data_dir[dbenv->data_next++]);
 ...
 */
void __env_get_errcall(DB_ENV * dbenv, void (**errcallp)__P((const DB_ENV*, const char *, const char *)))
{
...
static int __env_set_feedback(DB_ENV * dbenv, void (*feedback)__P((DB_ENV*, int, int)))
{
 ...
 */
static int __env_set_thread_id(DB_ENV * dbenv, void (*id)__P((DB_ENV*, pid_t*, db_threadid_t *)))
{
	dbenv->thread_id = id;
	return 0;
}
 ...
 */
static int __env_set_thread_id_string(DB_ENV * dbenv, char *(*thread_id_string)__P((DB_ENV*, pid_t, db_threadid_t, char *)))
{
 ...
 *	DB_ENV->get_isalive
 */
static int __env_get_isalive(DB_ENV * dbenv, int (**is_alivep)__P((DB_ENV*, pid_t, db_threadid_t, uint32)))
{
	ENV * env = dbenv->env;
 ...
 *	DB_ENV->set_isalive
 */
static int __env_set_isalive(DB_ENV * dbenv, int (*is_alive)__P((DB_ENV*, pid_t, db_threadid_t, uint32)))
{
	ENV * env = dbenv->env;
	if(F_ISSET(env, ENV_OPEN_CALLED) && env->thr_nbucket == 0) {
 ...
 * PUBLIC:     __P((DB_ENV *, void (*)(const DB_ENV *, const char *)));
 */
void __env_set_msgcall(DB_ENV * dbenv, void (*msgcall)__P((const DB_ENV*, const char *)))
{
 ...
 */
int __env_set_paniccall(DB_ENV * dbenv, void (*paniccall)__P((DB_ENV*, int)))
{
 ...
 */
static int __env_set_event_notify(DB_ENV * dbenv, void (*event_func)__P((DB_ENV*, uint32, void *)))
{
