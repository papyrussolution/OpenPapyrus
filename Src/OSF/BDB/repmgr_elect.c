...
int __repmgr_init_election(ENV * env, uint32 flags)
{
	...
	if(i == db_rep->aelect_threads) {
		...
		if((ret = __os_realloc(env, sizeof(REPMGR_RUNNABLE *)*new_size, &db_rep->elect_threads)) != 0)
			return ret;
		db_rep->aelect_threads = new_size;
		STAT(db_rep->region->mstat.st_max_elect_threads = new_size);
		th = db_rep->elect_threads[i] = NULL;
	}
	if(th == NULL && (ret = __os_malloc(env, sizeof(REPMGR_RUNNABLE), &th)) != 0)
		return ret;
...
static int __repmgr_elect_main(ENV * env, REPMGR_RUNNABLE * th)
{
	DB_REP * db_rep;
	REP * rep;
#ifdef DB_WIN32
	...
	for(;;) {
		 ...
		 */
		suppress_election = LF_ISSET(ELECT_F_STARTUP) ? db_rep->init_policy == DB_REP_CLIENT : !FLD_ISSET(rep->config, REP_C_ELECTIONS);
		...
		if(timespeccmp(&now, &target, >=)) {
			 ...
			 */
			action = suppress_election ? REPSTART : (done_repstart ? ELECTION : REPSTART);
		}
		...
		if(timespecisset(&wait_til)) {
			...
			if((ret = SignalObjectAndWait(*db_rep->mutex, db_rep->check_election, duration, FALSE)) != WAIT_OBJECT_0 && ret != WAIT_TIMEOUT)
				goto out;
			LOCK_MUTEX(db_rep->mutex);
			/*
			 ...
			 * been woken up".
			 */
			if(ret == WAIT_OBJECT_0 && db_rep->preferred_elect_thr == th && !ResetEvent(db_rep->check_election)) {
				ret = GetLastError();
				goto unlock;
			}
#else
			...
			if((ret = pthread_cond_timedwait(&db_rep->check_election, db_rep->mutex, &deadline)) != ETIMEDOUT && ret != 0)
				goto unlock;
#endif
			continue;
		}
		...
		if(action == ELECTION) {
			...
			if((ret = __repmgr_elect(env, 0, &failtime)) == DB_REP_UNAVAIL)
				done_repstart = FALSE;
			else
				goto out;
			LOCK_MUTEX(db_rep->mutex);
 ...
 */
static int send_membership(ENV*env)
{
	DB_REP * db_rep;
	uint8 * buf;
	size_t len;
