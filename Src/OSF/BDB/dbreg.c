...
int __dbreg_assign_id(DB*dbp, int32 id, int deleted)
{
	 ...
	 */
cont:
	if((ret = __dbreg_pluck_id(env, id)) != 0)
		goto err;
	if(id >= lp->fid_max)
		lp->fid_max = id+1;
...
int __dbreg_failchk(ENV*env)
{
	...
	for(fnp = SH_TAILQ_FIRST(&lp->fq, __fname); fnp != NULL; fnp = nnp) {
		...
		__db_msg(env, DB_STR_A("1502", "Freeing log information for process: %s, (ref %lu)", "%s %lu"),
			dbenv->thread_id_string(dbenv, fnp->pid, 0, buf), (ulong)fnp->txn_ref);
...
int __dbreg_log_close(ENV*env, FNAME * fnp, DB_TXN * txn, uint32 op)
{
	...
	if((ret = __dbreg_register_log(env, txn, &r_unused,
		...
		__dbreg_rem_dbentry(dblp, fnp->id);
	}
	return ret;
}
/*
...
static int __dbreg_push_id(ENV * env, int32 id)
{
	...
	int ret;
	DB_LOG * dblp = env->lg_handle;
	REGINFO * infop = &dblp->reginfo;
	LOG * lp = (LOG *)infop->primary;
	if(id == lp->fid_max-1) {
		lp->fid_max--;
		return 0;
	}
	/* Check if we have room on the stack. */
	if(lp->free_fid_stack == INVALID_ROFF || lp->free_fids_alloced <= lp->free_fids+1) {
		LOG_SYSTEM_LOCK(env);
		if((ret = __env_alloc(infop, (lp->free_fids_alloced+20)*sizeof(uint32), &newstack)) != 0) {
			LOG_SYSTEM_UNLOCK(env);
			return ret;
		}
		if(lp->free_fid_stack != INVALID_ROFF) {
			stack = (int32 *)R_ADDR(infop, lp->free_fid_stack);
			memcpy(newstack, stack, lp->free_fids_alloced*sizeof(uint32));
...
int __dbreg_log_id(DB*dbp, DB_TXN * txn, int32 id, int needlock)
{
	...
	fid_dbt.size = DB_FILE_ID_LEN;
	op = !F_ISSET(dbp, DB_AM_OPEN_CALLED) ? DBREG_PREOPEN : (F_ISSET(dbp, DB_AM_INMEM) ? DBREG_REOPEN : DBREG_OPEN);
	ret = __dbreg_register_log(env, txn, &unused, F_ISSET(dbp, DB_AM_NOT_DURABLE) ?
		DB_LOG_NOT_DURABLE : 0, op|F_ISSET(fnp, DB_FNAME_DBREG_MASK), r_name.size == 0 ? NULL : &r_name, &fid_dbt, id,
