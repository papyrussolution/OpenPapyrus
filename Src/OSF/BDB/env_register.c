...
#define REGISTRY_LOCK(env, pos, nowait) __os_fdlock(env, (env)->dbenv->registry, (off_t)(pos), 1, nowait)
#define REGISTRY_UNLOCK(env, pos)       __os_fdlock(env, (env)->dbenv->registry, (off_t)(pos), 0, 0)
...
int __envreg_register(ENV * env, int * need_recoveryp, uint32 flags)
{
	...
	*need_recoveryp = 0;
	...
	if(mbytes == 0 && bytes == 0) {
		if(FLD_ISSET(dbenv->verbose, DB_VERB_REGISTER))
			__db_msg(env, DB_STR_A("1525", "%lu: creating %s", "%lu %s"), (ulong)pid, pp);
		*need_recoveryp = 1;
	}
...
kill_all:       /*
	...
	if(need_recovery) {
		if(FLD_ISSET(dbenv->verbose, DB_VERB_REGISTER))
			__db_msg(env, "%lu: recovery required", (ulong)pid);
		if(LF_ISSET(DB_FAILCHK) || LF_ISSET(DB_FAILCHK_ISALIVE)) {
			if(FLD_ISSET(dbenv->verbose, DB_VERB_REGISTER))
				__db_msg(env, "%lu: performing failchk", (ulong)pid);
 ...
 * PUBLIC:   __P((DB_ENV *, pid_t, db_threadid_t, uint32));
 */
int __envreg_isalive(DB_ENV * dbenv, pid_t pid, db_threadid_t tid, uint32 flags)
{
	...
	if(!((flags == 0) || (flags == DB_MUTEX_PROCESS_ONLY)))
		return EINVAL;
	if(DB_GLOBAL(active_pids) == NULL || DB_GLOBAL(num_active_pids) == 0 || dbenv == NULL)
		return 0;
	/*
	 ...
	 */
	if(bsearch(&pid, DB_GLOBAL(active_pids), DB_GLOBAL(num_active_pids), sizeof(pid_t), __envreg_pid_compare))
		return 1;
	return 0;
}
/*
...
static int __envreg_create_active_pid(ENV * env, char * my_pid)
{
	...
	for(lcnt = 0;; ++lcnt) {
		...
		if(REGISTRY_LOCK(env, pos, 1) == 0) {
			...
			if(DB_GLOBAL(num_active_pids)+1 > DB_GLOBAL(size_active_pids)) {
				tmpsize = DB_GLOBAL(size_active_pids)*sizeof(pid_t);
				/* start with 512, then double if must grow */
				tmpsize = tmpsize>0 ? tmpsize*2 : 512;
				if((ret = __os_malloc (env, tmpsize, &tmparray )) != 0)
					return ret;
				/* if array exists, then copy and free */
				if(DB_GLOBAL(active_pids)) {
					memcpy( tmparray, DB_GLOBAL(active_pids), DB_GLOBAL(num_active_pids)*sizeof(pid_t));
					__os_free( env, DB_GLOBAL(active_pids));
				}
	...
	qsort(DB_GLOBAL(active_pids), DB_GLOBAL(num_active_pids), sizeof(pid_t), __envreg_pid_compare);
	return ret;
}
