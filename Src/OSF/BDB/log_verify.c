...
int __log_verify_pp(DB_ENV * dbenv, const DB_LOG_VERIFY_CONFIG * lvconfig)
{
	...
	if(lvconfig->start_time != 0 || lvconfig->end_time != 0)
		timerg = 1;
	if((!IS_ZERO_LSN(lvconfig->start_lsn) && lvconfig->start_time != 0) || (!IS_ZERO_LSN(lvconfig->end_lsn) && lvconfig->end_time != 0) || (lsnrg && timerg)) {
...
int __log_verify(DB_ENV * dbenv, const DB_LOG_VERIFY_CONFIG * lvconfig, DB_THREAD_INFO * ip)
{
	...
	uint32 logcflag, max_fileno;
	DB_LOGC * logc;
	ENV * env;
	DBT data;
	DB_DISTAB dtab;
	DB_LSN key, start, start2, stop, stop2, verslsn;
	uint32 newversion, version;
	int cmp, fwdscroll, goprev, ret, tret;
	__time64_t starttime, endtime;
	const char * okmsg;
...
vrfyscroll:
	...
	if(starttime != 0 || endtime != 0) {
		if((ret = __find_lsnrg_by_timerg(logvrfy_hdl, starttime, endtime, &start2, &stop2)) != 0)
			goto err;
	...
	if(lvconfig->dbfile != NULL) {
		F_SET(logvrfy_hdl, DB_LOG_VERIFY_DBFILE|DB_LOG_VERIFY_PARTIAL);
...
static int __env_init_verify(ENV * env, uint32 version, DB_DISTAB * dtabp)
{
	...
	switch(version) {
	    ...
	    default:
		__db_errx(env, DB_STR_A("2505", "Not supported version %lu", "%lu"), (ulong)version);
		ret = EINVAL;
		break;
	}
err:
