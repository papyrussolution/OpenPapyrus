 ...
 */
#define CD_SET(cp) { if(F_ISSET(cp, C_RENUMBER)) F_SET(cp, C_DELETED); }
#define CD_CLR(cp) {                                                    \
...
#define CD_ISSET(cp) (F_ISSET(cp, C_RENUMBER) && F_ISSET(cp, C_DELETED) ? 1 : 0)
 ...
 */
#define C_LESSTHAN(cp1, cp2) (((cp1)->recno < (cp2)->recno) || (((cp1)->recno == (cp2)->recno) && \
	((CD_ISSET((cp1)) && CD_ISSET((cp2)) && (cp1)->order < (cp2)->order) || (CD_ISSET((cp1)) && !CD_ISSET((cp2))))))
 ...
 */
#define C_EQUAL(cp1, cp2) ((cp1)->recno == (cp2)->recno && CD_ISSET((cp1)) == CD_ISSET((cp2)) && \
	(!CD_ISSET((cp1)) || (cp1)->order == (cp2)->order))

/*
 * Do we need to log the current cursor adjustment?
 */
#define CURADJ_LOG(dbc) (DBC_LOGGING((dbc)) && (dbc)->txn != NULL && (dbc)->txn->parent != NULL)
/*
...
retry:
	...
	if(F_ISSET(cp, C_RENUMBER)) {
		...
		if((ret = __ram_ca(dbc, CA_DELETE, &nc)) != 0)
			goto err;
		if(nc > 0 && CURADJ_LOG(dbc) && (ret = __bam_rcuradj_log(dbp, dbc->txn, &lsn, 0, CA_DELETE, BAM_ROOT_PGNO(dbc), cp->recno, cp->order)) != 0)
			goto err;
		/*
...
retry:
	...
	for(;; ++cp->recno) {
		...
		if((ret = __bam_rsearch(dbc, &cp->recno, F_ISSET(dbc, DBC_RMW) ? SR_FIND_WR : SR_FIND, 1, &exact)) != 0)
			goto err;
		if(!exact) {
			ret = DB_NOTFOUND;
			goto err;
		...
		if(flags == DB_GET_BOTH || flags == DB_GET_BOTHC || flags == DB_GET_BOTH_RANGE) {
			if((ret = __bam_cmp(dbc, data, (PAGE *)cp->page, cp->indx, __bam_defcmp, &cmp)) != 0)
				return ret;
			if(cmp == 0)
				break;
			if(!F_ISSET(dbc, DBC_OPD)) {
	...
	if(F_ISSET(dbc, DBC_OPD))
		switch(flags) {
			...
			if((ret = __ram_add(dbc, &cp->recno, data, DB_APPEND, 0)) != 0)
				return ret;
			if(CURADJ_LOG(dbc) && (ret = __bam_rcuradj_log(dbp, dbc->txn, &lsn, 0, CA_ICURRENT, BAM_ROOT_PGNO(dbc), cp->recno, cp->order)) != 0)
				return ret;
			return 0;
		    default:
			break;
...
split:
	...
	switch(flags) {                         /* Adjust the cursors. */
		...
		if(nc > 0 && CURADJ_LOG(dbc) && (ret = __bam_rcuradj_log(dbp, dbc->txn, &lsn, 0, CA_IAFTER,
		...
		--cp->recno;
		/* Only log if __ram_ca found any relevant cursors. */
		if(nc > 0 && CURADJ_LOG(dbc) && (ret = __bam_rcuradj_log(dbp, dbc->txn, &lsn, 0, CA_IBEFORE,
			    BAM_ROOT_PGNO(dbc), cp->recno, cp->order)) != 0)
			goto err;
		break;
	    case DB_CURRENT:
		...
		if((ret = __ram_ca(dbc, CA_ICURRENT, &nc)) != 0)
			goto err;
		if(nc > 0 && CURADJ_LOG(dbc) && (ret = __bam_rcuradj_log(dbp, dbc->txn, &lsn, 0, CA_ICURRENT, BAM_ROOT_PGNO(dbc), cp->recno, cp->order)) != 0)
			goto err;
		break;
	    default:
		break;
	...
	if(!F_ISSET(dbc, DBC_OPD) && (flags == DB_AFTER || flags == DB_BEFORE) && key != NULL)
		ret = __db_retcopy(env, key, &cp->recno, sizeof(cp->recno), &dbc->rkey->data, &dbc->rkey->ulen);
	/* The cursor was reset, no further delete adjustment is necessary. */
err:
	CD_CLR(cp);
	if(!DB_RETOK_DBCDEL(ret))
...
static int __ram_ca_setorder(DBC * dbc, DBC * my_dbc, uint32 * foundp, db_pgno_t pgno, uint32 order, void * args)
{
	...
	switch(op) {
		...
		else if(recno == cp->recno && !CD_ISSET(cp)) {
			CD_SET(cp);
			cp->order = order;
			/*
...
int __ram_ca(DBC * dbc_arg, ca_recno_arg op, int * foundp)
{
	...
	DB * dbp = dbc_arg->dbp;
	ENV * env = dbp->env;
	BTREE_CURSOR * cp_arg = (BTREE_CURSOR *)dbc_arg->internal;
	db_recno_t recno = cp_arg->recno;
	/*
	...
	for(keyno = 1;; ++keyno) {
		switch(ret = __db_get(dbp, ip, NULL, &key, &data, 0)) {
			...
			if(F_ISSET(dbp, DB_AM_FIXEDLEN)) {
				if(pad == NULL) {
					if((ret = __os_malloc(env, t->re_len, &pad)) != 0)
						goto err;
...
static int __ram_sread(DBC * dbc, db_recno_t top)
{
	...
	rdata = &dbc->my_rkey;
	if(rdata->ulen < len) {
		if((ret = __os_realloc(dbp->env, len, &rdata->data)) != 0) {
			rdata->ulen = 0;
			rdata->data = NULL;
			return ret;
		}
		...
		if(F_ISSET(dbp, DB_AM_FIXEDLEN))
			...
			for(;; ) {
				...
				if(data.size == rdata->ulen) {
					if((ret = __os_realloc(dbp->env, rdata->ulen *= 2, &rdata->data)) != 0) {
						rdata->ulen = 0;
						rdata->data = NULL;
						return ret;
					}
...
retry:
	 ...
	 * location.
	 */
	switch(ret = __bam_iitem(dbc, NULL, data, exact ? DB_CURRENT : DB_BEFORE, bi_flags)) {
	    case 0:
		/*
