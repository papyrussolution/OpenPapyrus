...
{
	int ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_create_verify, DB___fop_create)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_remove_verify, DB___fop_remove)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_write_verify, DB___fop_write)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_rename_verify, DB___fop_rename)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_rename_verify, DB___fop_rename_noundo)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __fop_file_remove_verify, DB___fop_file_remove)) != 0)
		return ret;
	return 0;
}
...
int __ham_init_verify(ENV * env, DB_DISTAB * dtabp)
{
	...
	if((ret = __db_add_recovery_int(env, dtabp, __ham_groupalloc_verify, DB___ham_groupalloc)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __ham_changeslot_verify, DB___ham_changeslot)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __ham_contract_verify, DB___ham_contract)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __ham_curadj_verify, DB___ham_curadj)) != 0)
		return ret;
	if((ret = __db_add_recovery_int(env, dtabp, __ham_chgpg_verify, DB___ham_chgpg)) != 0)
		return ret;
	return 0;
}
