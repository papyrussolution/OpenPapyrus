...
static int __rep_stat(ENV * env, DB_REP_STAT ** statp, uint32 flags)
{
	...
	*statp = NULL;
	...
	if(F_ISSET(rep, REP_F_CLIENT)) {
		...
		stats->st_max_lease_usec = (uint32)(lp->max_lease_ts.tv_nsec/NS_PER_US);
	}
	else {
		if(F_ISSET(rep, REP_F_MASTER)) {
			LOG_SYSTEM_LOCK(env);
...
static int __rep_print_stats(ENV*env, uint32 flags)
{
	...
	__db_dl(env, "Number of times the master has changed", (ulong)sp->st_master_changes);
	__db_dl(env, "Number of messages received with a bad generation number", (ulong)sp->st_msgs_badgen);
	__db_dl(env, "Number of messages received and processed", (ulong)sp->st_msgs_processed);
	PRINT_MSGSRECOVER(sp);
	PRINT_MSGSSENDFAILURES(sp);
	__db_dl(env, "Number of messages sent", (ulong)sp->st_msgs_sent);
	__db_dl(env, "Number of new site messages received", (ulong)sp->st_newsites);
	__db_dl(env, "Number of environments used in the last election", (ulong)(sp)->st_nsites);
	__db_dl(env, "Transmission limited", (ulong)sp->st_nthrottles);
	__db_dl(env, "Number of outdated conditions detected", (ulong)sp->st_outdated);
	__db_dl(env, "Number of duplicate page records received", (ulong)sp->st_pg_duplicated);
	__db_dl(env, "Number of page records received and added to databases", (ulong)sp->st_pg_records);
	__db_dl(env, "Number of page records missed and requested", (ulong)sp->st_pg_requested);
	PRINT_STARTUPCOMPLETE(sp);
	__db_dl(env, "Number of transactions applied", (ulong)sp->st_txns_applied);
	__db_dl(env, "Number of startsync messages delayed", (ulong)sp->st_startsync_delayed);
	__db_dl(env, "Number of elections held", (ulong)sp->st_elections);
	__db_dl(env, "Number of elections won", (ulong)sp->st_elections_won);
	if(sp->st_election_status == 0) {
		...
		__db_dl(env, "Tiebreaker value of the winner of the current or last election", (ulong)sp->st_election_tiebreaker);
		__db_dl(env, "Number of votes received during the current election", (ulong)sp->st_election_votes);
	}
	__db_dl(env, "Number of bulk buffer sends triggered by full buffer", (ulong)sp->st_bulk_fills);
	__db_dl(env, "Number of single records exceeding bulk buffer size", (ulong)sp->st_bulk_overflows);
	__db_dl(env, "Number of records added to a bulk buffer", (ulong)sp->st_bulk_records);
	__db_dl(env, "Number of bulk buffers sent", (ulong)sp->st_bulk_transfers);
	__db_dl(env, "Number of re-request messages received", (ulong)sp->st_client_rerequests);
...
static int __rep_print_all(ENV*env, uint32 flags)
{
	...
	__db_dlbytes(env, "Limit on data sent in a single call", rep->gbytes, (ulong)0, rep->bytes);
	STAT_LONG("Request gap seconds", rep->request_gap.tv_sec);
	STAT_LONG("Request gap microseconds", rep->request_gap.tv_nsec/NS_PER_US);
	STAT_LONG("Maximum gap seconds", rep->max_gap.tv_sec);
	STAT_LONG("Maximum gap microseconds", rep->max_gap.tv_nsec/NS_PER_US);
	...
	__db_msg(env, "%.24s\tRecovery timestamp", renv->rep_timestamp == 0 ? "0" : __os_ctime(&renv->rep_timestamp, time_buf));
...
static int __rep_stat_summary_print(ENV*env)
{
	...
	if((ret = __rep_stat(env, &sp, 0)) == 0) {
		 ...
		 */
		__db_dl(env, "Number of environments in the replication group", (ulong)rep->config_nsites);
		PRINT_MSGSSENDFAILURES(sp);
		PRINT_MSGSRECOVER(sp);
		PRINT_LOGQUEUED(sp);
		__os_ufree(env, sp);
