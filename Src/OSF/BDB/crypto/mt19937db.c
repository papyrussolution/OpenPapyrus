...
int __db_generate_iv(ENV*env, uint32 * iv)
{
	...
	if(env->mt == NULL) {
		if((ret = __os_calloc(env, 1, N*sizeof(ulong), &env->mt)) != 0)
			return ret;
...
static void __db_sgenrand(ulong seed, ulong mt[], int * mtip)
{
...
static ulong __db_genrand(ENV * env)
{
	db_timespec ts;
	ulong y;
	...
	if(env->mti >= N) { /* generate N words at one time */
		...
		if(env->mti == N+1) { /* if sgenrand() has not been called, */
			...
			do {
				...
				__db_chksum(NULL, (uint8 *)&ts.tv_sec, sizeof(ts.tv_sec), NULL, (uint8 *)&seed);
			} while(seed == 0);
			__db_sgenrand((ulong)seed, env->mt, &env->mti);
		}
		for(kk = 0; kk<N-M; kk++) {
