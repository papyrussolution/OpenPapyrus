 ...
 *
 * PUBLIC: uint __aes_adj_size __P((size_t));
 */
uint __aes_adj_size(size_t len)
{
	return (len%DB_AES_CHUNK == 0) ? 0 : (DB_AES_CHUNK-(uint)(len%DB_AES_CHUNK));
}
...
static int __aes_derivekeys(ENV*env, DB_CIPHER * db_cipher, uint8 * passwd, size_t plen)
{
	...
	if((ipp_ret = ippsRijndael128Init((const Ipp8u *)temp,
		IppsRijndaelKey128, (IppsRijndael128Spec *)aes->ipp_ctx)) != ippStsNoErr) {
		__aes_err(env, (int)ipp_ret);
		return EAGAIN;
	}
#else
...
static void __aes_err(ENV * env, int err)
{
	...
	switch(err) {
		...
		errstr = DB_STR("0184", "IPP AES context does not match operation");
		break;
		...
		break;
	    case BAD_KEY_MAT:
		errstr = DB_STR("0187", "AES key material not of correct length");
		break;
	    case BAD_KEY_INSTANCE:
		errstr = DB_STR("0188", "AES key passwd not valid");
		break;
	    case BAD_CIPHER_MODE:
		errstr = DB_STR("0189", "AES cipher in wrong state (not initialized)");
		break;
