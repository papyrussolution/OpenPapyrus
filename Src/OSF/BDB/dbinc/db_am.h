...
#define	IS_ENV_AUTO_COMMIT(env, txn, flags)				\
	(LF_ISSET(DB_AUTO_COMMIT) || (((txn) == NULL || F_ISSET((txn), TXN_FAMILY)) && F_ISSET((env)->dbenv, DB_ENV_AUTO_COMMIT) && !LF_ISSET(DB_NO_AUTO_COMMIT)))

/*
 * IS_DB_AUTO_COMMIT --
 *	Auto-commit test for database operations.
 */
#define	IS_DB_AUTO_COMMIT(dbp, txn) (((txn) == NULL || F_ISSET((txn), TXN_FAMILY)) && F_ISSET((dbp), DB_AM_TXN))
...
#define	REC_INTRO(func, ip, do_cursor) do {				\
	...
	if((ret = func(env, &file_dbp, (info != NULL) ? ((DB_TXNHEAD *)info)->td : NULL, dbtp->data, &argp)) != 0) {					\
		if(ret	== DB_DELETED) {				\
			ret = 0;					\
			goto done;					\
		}							\
...
#define	REC_CLOSE {							\
	int __t_ret;							\
	__os_free(env, argp);					\
	if(dbc && (__t_ret = __dbc_close(dbc)) != 0 && ret == 0)		\
		ret = __t_ret;						\
	}								\
	return (ret)
 ...
 */
#ifdef DEBUG_RECOVER
	#define	REC_PRINT(func) func(env, dbtp, lsnp, op, info);
#else
 ...
 */
#define	DB_IS_READONLY(dbp) (F_ISSET(dbp, DB_AM_RDONLY) || (IS_REP_CLIENT((dbp)->env) && !F_ISSET((dbp), DB_AM_NOT_DURABLE)))
