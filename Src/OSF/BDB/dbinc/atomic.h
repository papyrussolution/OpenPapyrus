 ...
 */
	...
	#define	atomic_inc(env, p) (WINCE_ATOMIC_MAGIC(p), InterlockedIncrement((interlocked_val)(&(p)->value)))
#else
	typedef LONG volatile *interlocked_val;
	#define	atomic_inc(env, p) InterlockedIncrement((interlocked_val)(&(p)->value))
#endif

#define	atomic_dec(env, p) (WINCE_ATOMIC_MAGIC(p), InterlockedDecrement((interlocked_val)(&(p)->value)))
#if defined(_MSC_VER) && _MSC_VER < 1300
#define	atomic_compare_exchange(env, p, oldval, newval)			\
	(WINCE_ATOMIC_MAGIC(p), (InterlockedCompareExchange((PVOID *)(&(p)->value), (PVOID)(newval), (PVOID)(oldval)) == (PVOID)(oldval)))
#else
#define	atomic_compare_exchange(env, p, oldval, newval)			\
	(WINCE_ATOMIC_MAGIC(p), (InterlockedCompareExchange((interlocked_val)(&(p)->value), (newval), (oldval)) == (oldval)))
#endif
#endif
...
#include <atomic.h>
#define	atomic_inc(env, p) atomic_inc_uint_nv((volatile unsigned int *) &(p)->value)
#define	atomic_dec(env, p) atomic_dec_uint_nv((volatile unsigned int *) &(p)->value)
#define	atomic_compare_exchange(env, p, oval, nval) (atomic_cas_32((volatile unsigned int *) &(p)->value, (oval), (nval)) == (oval))
#endif
...
#define	atomic_compare_exchange(env, p, o, n) __atomic_compare_exchange((p), (o), (n))
 ...
 */
static inline int __atomic_compare_exchange(db_atomic_t *p, atomic_value_t oldval, atomic_value_t newval)
{
	atomic_value_t was;
...
#define	atomic_compare_exchange(env, p, oldval, newval) (DB_ASSERT(env, atomic_read(p) == (oldval)), atomic_init(p, (newval)), 1)
#else
