 ...
 */
#define	IS_WRITELOCK(m)         ((m) == DB_LOCK_WRITE || (m) == DB_LOCK_WWRITE || (m) == DB_LOCK_IWRITE || (m) == DB_LOCK_IWR)

/*
 * Macros to lock/unlock the lock region as a whole. Mostly used for
 * initialization.
 */
#define	LOCK_REGION_LOCK(env)   MUTEX_LOCK(env, ((DB_LOCKREGION *)(env)->lk_handle->reginfo.primary)->mtx_region)
#define	LOCK_REGION_UNLOCK(env) MUTEX_UNLOCK(env, ((DB_LOCKREGION *)(env)->lk_handle->reginfo.primary)->mtx_region)
...
typedef struct __db_lockregion { /* SHARED */
	...
	uint32	need_dd;	/* flag for deadlock detector */
	uint32	detect;		/* run dd on every conflict */
	...
	uint32	nlockers;	/* Current number of lockers. */
