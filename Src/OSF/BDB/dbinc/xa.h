...
struct xa_switch_t {
	...
	int (*xa_open_entry) __P((char *, int, long)); /* xa_open function pointer */
	int (*xa_close_entry) __P((char *, int, long)); /* xa_close function pointer */
	...
	int (*xa_recover_entry) __P((XID *, long, int, long)); /* xa_recover function pointer */
	int (*xa_forget_entry)(XID *, int, long); /* xa_forget function pointer */
	int (*xa_complete_entry) __P((int *, int *, int, long)); /* xa_complete function pointer */
};
...
#define	XA_HEURMIX	5		/* the transaction branch has been heuristically committed and rolled */
