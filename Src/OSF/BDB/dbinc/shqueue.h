...
#define	SH_CHAIN_INIT(elm, field)        (elm)->field.sce_next = (elm)->field.sce_prev =	-1
#define	SH_CHAIN_HASNEXT(elm, field)     ((elm)->field.sce_next != -1)
#define	SH_CHAIN_NEXTP(elm, field, type) ((struct type *)((uint8 *)(elm) + (elm)->field.sce_next))
#define	SH_CHAIN_NEXT(elm, field, type)	 (SH_CHAIN_HASNEXT(elm, field) ? SH_CHAIN_NEXTP(elm, field, type) : (struct type *)NULL)
#define	SH_CHAIN_HASPREV(elm, field)	 ((elm)->field.sce_prev != -1)
#define	SH_CHAIN_PREVP(elm, field, type) ((struct type *)((uint8 *)(elm) + (elm)->field.sce_prev))
#define	SH_CHAIN_PREV(elm, field, type)  (SH_CHAIN_HASPREV(elm, field) ? SH_CHAIN_PREVP(elm, field, type) : (struct type *)NULL)
#define	SH_CHAIN_SINGLETON(elm, field)   (!(SH_CHAIN_HASNEXT(elm, field) || SH_CHAIN_HASPREV(elm, field)))
...
#define	SH_CHAIN_REMOVE(elm, field, type) do {				\
	...
	if(__next)						\
		__next->field.sce_prev = (__prev == NULL) ? -1 : SH_PTR_TO_OFF(__next, __prev);			\
	if(__prev)						\
