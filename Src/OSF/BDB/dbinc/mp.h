 ...
 */
#define	MPF_ILLEGAL_AFTER_OPEN(dbmfp, name)  if(F_ISSET(dbmfp, MP_OPEN_CALLED)) return (__db_mi_open((dbmfp)->env, name, 1));
#define	MPF_ILLEGAL_BEFORE_OPEN(dbmfp, name) if(!F_ISSET(dbmfp, MP_OPEN_CALLED)) return (__db_mi_open((dbmfp)->env, name, 0));
/*
...
#define	FNBUCKET(id, len) (FHASH(id, len) % MPOOL_FILE_BUCKETS)

/* Macros to lock/unlock the mpool region as a whole. */
#define	MPOOL_SYSTEM_LOCK(env)   MUTEX_LOCK(env, ((MPOOL *)(env)->mp_handle->reginfo[0].primary)->mtx_region)
#define	MPOOL_SYSTEM_UNLOCK(env) MUTEX_UNLOCK(env, ((MPOOL *)(env)->mp_handle->reginfo[0].primary)->mtx_region)

/* Macros to lock/unlock a specific mpool region. */
#define	MPOOL_REGION_LOCK(env, infop)   MUTEX_LOCK(env, ((MPOOL *)(infop)->primary)->mtx_region)
#define	MPOOL_REGION_UNLOCK(env, infop) MUTEX_UNLOCK(env, ((MPOOL *)(infop)->primary)->mtx_region)
 ...
 */
#define	NREGION(mp, bucket) ((bucket) / (mp)->htab_buckets)
...
#define	MVCC_BHALIGN(p) do {						\
	...
	DB_ASSERT(env,							\
	    (uint8 *)__bhp >= (uint8 *)__orig);			\
	DB_ASSERT(env, (uint8 *)p + mfp->pagesize <			\
	    (uint8 *)__orig + len);					\
	__bhp->align_off =						\
	    (uint16)((uint8 *)__bhp - (uint8 *)__orig);	\
	p = __bhp;							\
} while (0)
