...
static int FASTCALL __bamc_next(DBC*, int, int);
static int FASTCALL __bamc_physdel(DBC *);
static int FASTCALL __bamc_prev(DBC *);
static int __bamc_put(DBC*, DBT*, DBT*, uint32, db_pgno_t *);
static int FASTCALL __bamc_search(DBC*, db_pgno_t, const DBT*, uint32, int *);
static int __bamc_writelock(DBC *);
static int __bam_getboth_finddatum(DBC*, DBT*, uint32);
static int __bam_getbothc(DBC*, DBT *);
static int FASTCALL __bam_get_prev(DBC *);
static int FASTCALL __bam_isopd(DBC*, db_pgno_t *);
...
static int __bamc_close(DBC * dbc, db_pgno_t root_pgno, int * rmroot)
{
	...
	if(F_ISSET(cp_opd, C_DELETED)) {                /* Case #2. */
		// We will not have been provided a root page number.  Acquire one from the primary database.
		if((h = (PAGE *)cp->page) == NULL && (ret = __memp_fget(mpf, &cp->pgno, dbc->thread_info, dbc->txn, 0, &h)) != 0)
			goto err;
...
do_del: 
	...
	if(dbc_c->dbtype == DB_BTREE) {
		if((ret = __memp_fget(mpf, &cp_c->pgno, dbc->thread_info, dbc->txn, DB_MPOOL_DIRTY, &cp_c->page)) != 0)
			goto err_c;
		if((ret = __bamc_physdel(dbc_c)) != 0)
			goto err_c;
	}
	/*
	 * If we're not working in an off-page duplicate tree, then we're done.
	 */
	 ...
	 */
	if(((h = (PAGE *)dbc_c->internal->page) == NULL || h->pgno != root_pgno) && (ret = __memp_fget(mpf, &root_pgno, dbc->thread_info, dbc->txn, 0, &h)) != 0)
		goto err_c;
...
err:
	...
	/*
	 * Discard the page references and locks, and confirm that the stack has been emptied.
	 */
	if(dbc_opd)
...
int __bamc_count(DBC * dbc, db_recno_t * recnop)
{
	...
	if(cp->opd == NULL) {
		...
		for(indx = cp->indx;; indx -= P_INDX)
			if(indx == 0 || !IS_DUPLICATE(dbc, indx, indx-P_INDX))
				break;
		for(recno = 0, top = NUM_ENT(cp->page)-P_INDX;; indx += P_INDX) {
			if(!IS_DELETED(dbp, cp->page, indx))
				++recno;
			if(indx == top || !IS_DUPLICATE(dbc, indx, indx+P_INDX))
				break;
		}
	}
	else {
		 ...
		 * duplicate tree.
		 */
		if((ret = __memp_fget(mpf, &cp->opd->internal->root, dbc->thread_info, dbc->txn, 0, &cp->page)) != 0)
			return ret;
		/*
		...
		if(TYPE(cp->page) == P_LDUP)
			for(recno = 0, indx = 0, top = NUM_ENT(cp->page)-O_INDX;; indx += O_INDX) {
				if(!IS_DELETED(dbp, cp->page, indx))
					++recno;
				if(indx == top)
					break;
...
static int __bamc_del(DBC * dbc, uint32 flags)
{
	...
	/* Log the change. */
	if(DBC_LOGGING(dbc)) {
		if((ret = __bam_cdel_log(dbp, dbc->txn, &LSN(cp->page), 0, PGNO(cp->page), &LSN(cp->page), cp->indx)) != 0)
			goto err;
	}
	else
...
err:    /*
	...
	cp->page = NULL;
	/*
	 * Update the cursors last, after all chance of recoverable failure is past.
	 */
	SETIFZ(ret, __bam_ca_delete(dbp, cp->pgno, cp->indx, 1, &count));
	if(F_ISSET(dbc, DBC_OPD))
...
int __bamc_dup(DBC * orig_dbc, DBC * new_dbc, uint32 flags)
{
	...
	p_new_cursor->ovflsize = orig->ovflsize;
...
static int __bamc_get(DBC * dbc, DBT * key, DBT * data, uint32 flags, db_pgno_t * pgnop)
{
	...
	switch(flags) {
		...
		if(cp->pgno == PGNO_INVALID) {
			if((ret = __bamc_search(dbc, PGNO_INVALID, NULL, DB_FIRST, &exact)) != 0)
				goto err;
		}
		else
...
static int FASTCALL __bam_get_prev(DBC * dbc)
{
	BTREE_CURSOR * cp;
	DBT key, data;
	db_pgno_t pgno;
...
static int __bam_bulk(DBC * dbc, DBT * data, uint32 flags)
{
	...
	int32 * endp, * offp, * saveoffp;
	uint8 * dp, * np;
	uint32 key_size, space;
	int adj, is_key, need_pg, next_key, no_dup, rec_key;
	int ret = 0;
	int32 key_off = 0;
	uint32 size = 0;
...
next_pg:
	...
	do {
		if(IS_DELETED(dbc->dbp, pg, indx)) {
			...
			cp->recno++;
			/*
			 * If we are not returning recnos then we need to fill in every slot so the user can calculate the record numbers.
			 */
			if(rec_key != 0)
				continue;
			space -= 2*sizeof(*offp);
		 ...
		 * If so, then see if we need to put the key on the page.  If its already there then we just point to it.
		 */
...
get_key_space:
				...
				key_size = bk->len;
				key_off = (int32)((inp[indx]-HOFFSET(pg))+(dp-dbuf)+SSZA(BKEYDATA, data));
				pg_keyoff = inp[indx];
			}
		}
		/*
		...
		if(B_TYPE(bk->type) == B_DUPLICATE) {
			 ...
			 */
			saveoffp = offp;
			if((ret = __bam_bulk_duplicates(dbc, bo->pgno, dbuf, is_key ? offp+P_INDX : NULL, &offp, &np, &space, no_dup)) != 0) {
...
back_up:
					// 
					// Back up the index so that the last record in the buffer is CURRENT
					// 
					if(indx >= adj)
						indx -= adj;
					else {
...
int __bam_bulk_duplicates(DBC * dbc, db_pgno_t pgno, uint8 * dbuf, int32 * keyoff, int32 ** offpp, uint8 ** dpp, uint32 * spacep, int no_dup)
{
	...
	if(opd == NULL) {
		if((ret = __dbc_newopd(dbc, pgno, NULL, &opd)) != 0)
			return ret;
		cp->opd = opd;
		if((ret = opd->am_get(opd, &key, &data, DB_FIRST, NULL)) != 0)
			goto close_opd;
	}
...
static int __bam_getbothc(DBC * dbc, DBT * data)
{
	...
	if(F_ISSET(dbc, DBC_OPD)) {
		 ...
		 */
		if((ret = __bam_cmp(dbc, data, (PAGE *)cp->page, cp->indx, dbp->dup_compare == NULL ? __bam_defcmp : dbp->dup_compare, &cmp)) != 0)
			return ret;
		if(cmp <= 0)
			return DB_NOTFOUND;
...
static int __bam_getlte(DBC * dbc, DBT * key, DBT * data)
{
	...
	if(cp->indx == NUM_ENT(cp->page) || IS_CUR_DELETED(dbc)) {
		/*
		 * Move to the next entry if we're past the end of the page or on a deleted entry.
		 */
	...
	if(__bam_isopd(dbc, &pgno)) {
		...
		/*
		 * If we got an exact match with on-page duplicates, we need to search in them.
		 */
		...
		if(ret == DB_NOTFOUND)
			...
			/* Check if we're still on the correct data */
			if((ret = __bam_cmp(dbc, data, (PAGE *)cp->page, cp->indx+O_INDX, dbp->dup_compare, &exact)) != 0)
				goto end;
			exact = (exact == 0);
		}
		if(exact == 0) {
...
static int __bam_getboth_finddatum(DBC * dbc, DBT * data, uint32 flags)
{
	...
	if(dbp->dup_compare == NULL) {
		for(;; cp->indx += P_INDX) {
			if(!IS_CUR_DELETED(dbc)) {
				if((ret = __bam_cmp(dbc, data, (PAGE *)cp->page, cp->indx+O_INDX, __bam_defcmp, &cmp)) != 0)
					return ret;
				if(cmp == 0)
					return 0;
			}
	...
	for(base = top = cp->indx; top < NUM_ENT(cp->page); top += P_INDX)
		if(!IS_DUPLICATE(dbc, cp->indx, top))
			break;
	if(base == (top-P_INDX)) {
		if((ret = __bam_cmp(dbc, data, (PAGE *)cp->page, cp->indx+O_INDX, dbp->dup_compare, &cmp)) != 0)
			return ret;
	...
	for(lim = (top-base)/(db_indx_t)P_INDX; lim != 0; lim >>= 1) {
		...
		if(cmp == 0) {
			/*
			 * XXX
			 * No duplicate duplicates in sorted duplicate sets, so there can be only one.
			 */
			if(!IS_CUR_DELETED(dbc))
				return 0;
			break;
	 ...
	 * or a last + O_INDX index, and may be deleted.  Find an undeleted item.
	 */
	cp->indx = base;
...
split:
	...
	switch(flags) {
		...
		for(;; cp->indx += P_INDX) {
			...
			if(cp->indx+P_INDX >= NUM_ENT(cp->page) || P_INP(dbp, ((PAGE *)cp->page))[cp->indx] != P_INP(dbp, ((PAGE *)cp->page))[cp->indx+P_INDX]) {
				iiop = DB_AFTER;
				break;
			}
		}
	...
	switch(ret = __bam_iitem(dbc, key, data, iiop, 0)) {
		 ...
		 */
		if(oneof3(flags, DB_AFTER, DB_BEFORE, DB_CURRENT)) {
...
static int __bamc_writelock(DBC * dbc)
{
	 ...
	 * code calls us first with the primary cursor so we can acquire the appropriate lock.
	 */
	ACQUIRE_WRITE_LOCK(dbc, ret);
	return ret;
}
 ...
 */
static int FASTCALL __bamc_next(DBC * dbc, int initial_move, int deleted_okay)
{
	db_indx_t adjust;
 ...
 */
static int FASTCALL __bamc_prev(DBC * dbc)
{
	db_indx_t adjust;
	db_lockmode_t lock_mode;
	db_pgno_t pgno;
 ...
 */
static int FASTCALL __bamc_search(DBC * dbc, db_pgno_t root_pgno, const DBT * key, uint32 flags, int * exactp)
{
	PAGE * h;
	...
	switch(flags) {
	    ...
	    case DB_SET_RECNO:
			if((ret = __ram_getno(dbc, key, &recno, 0)) != 0)
				return ret;
			sflags = (F_ISSET(dbc, DBC_RMW) ?  SR_FIND_WR : SR_FIND)|SR_EXACT;
	    ...
	    case DB_GET_BOTH_RANGE:
			...
			goto search;
	    case DB_SET_RANGE:
			sflags = (F_ISSET(dbc, DBC_RMW) ? SR_WRITE : SR_READ)|SR_DUPFIRST;
			goto search;
	    case DB_KEYFIRST:
	    case DB_NOOVERWRITE:
			sflags = SR_KEYFIRST;
	 ...
	 * cursor, check whether this operation belongs on the same page as the last one.
	 */
	if(bulk)
		bt_lpgno = cp->pgno;
	else {
	...
	if(F_ISSET(dbp, DB_AM_SUBDB) && LOG_COMPARE(&t->bt_llsn, &LSN(h)) != 0)
		goto fast_miss;
	/*
	...
	if(h->next_pgno == PGNO_INVALID) {
		...
		if((ret = __bam_cmp(dbc, key, h, indx, t->bt_compare, &cmp)) != 0)
			goto fast_miss;
		if(cmp > 0) {
	...
	if(bulk) {
		DB_BINARY_SEARCH_FOR(base, lim, NUM_ENT(h), P_INDX) {
			...
			if(cmp == 0)
				goto fast_hit;
			if(cmp > 0)
				DB_BINARY_SEARCH_SHIFT_BASE(indx, base, lim, P_INDX);
		}
		/*
...
search:
	...
	if(flags == DB_FIRST && (NUM_ENT(cp->page) == 0 || IS_CUR_DELETED(dbc)))
		if((ret = __bamc_next(dbc, 0, 0)) != 0)
			return ret;
	if(flags == DB_LAST && (NUM_ENT(cp->page) == 0 || IS_CUR_DELETED(dbc)))
		if((ret = __bamc_prev(dbc)) != 0)
			return ret;
	return 0;
}
 ...
 */
static int FASTCALL __bamc_physdel(DBC * dbc)
{
	DBT key;
	 ...
	 */
	if(delete_page && !F_ISSET(dbc, DBC_OPD) && F_ISSET(dbp, DB_AM_REVSPLITOFF))
		delete_page = 0;
	/*
	...
	if(!delete_page)
		return 0;
	/*
	 * Lock the previous and next pages before latching the parent sub tree.
	 */
	if(STD_LOCKING(dbc)) {
...
/*
 *	Return if the cursor references an off-page duplicate tree via its page number.
 */
static int FASTCALL __bam_isopd(DBC * dbc, db_pgno_t * pgnop)
{
	if(TYPE(dbc->internal->page) != P_LBTREE)
		return 0;
	else {
		BOVERFLOW * bo = GET_BOVERFLOW(dbc->dbp, dbc->internal->page, dbc->internal->indx+O_INDX);
