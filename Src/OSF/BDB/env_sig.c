...
static void __LogStrucSigEntry(const char * pStrucName, uint strucSize)
{
	if(!isempty(pStrucName)) {
		SString msg_buf;
		msg_buf.Cat(pStrucName).Tab().Cat(strucSize);
		SLS.LogMessage("bdb-struc-sig", msg_buf, 0);
	}
}
//
// __env_struct_sig --
...
//
// PUBLIC: uint32 __env_struct_sig();
//
