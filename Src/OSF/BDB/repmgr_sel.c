...
int __repmgr_accept(ENV * env)
{
	...
	if((s = accept(db_rep->listen_fd, (struct sockaddr *)&siaddr, &addrlen)) == -1) {
		...
		switch(ret = net_errno) {
		    ...
		    case ENETUNREACH:
#endif
			VPRINT(env, (env, DB_VERB_REPMGR_MISC, "accept error %d considered innocuous", ret));
			return 0;
		    default:
...
/*
 * PUBLIC: REPMGR_CONNECTION *__repmgr_master_connection(ENV *);

 */
REPMGR_CONNECTION * __repmgr_master_connection(ENV * env)
{
	DB_REP * db_rep;
	REP * rep;
	REPMGR_SITE * master;
...
{
	REPMGR_CONNECTION * conn = __repmgr_master_connection(env);
	DB_ASSERT(env, conn != NULL);
	RPRINT(env, (env, DB_VERB_REPMGR_MISC, "heartbeat monitor timeout expired"));
 ...
 */
static int __repmgr_try_one(ENV * env, uint eid)
{
	DB_REP * db_rep;
	REPMGR_SITE * site;
	REPMGR_RUNNABLE * th;
...
static void * __repmgr_connector_thread(void * argsp)
{
	...
	ENV * env = th->env;
	RPRINT(env, (env, DB_VERB_REPMGR_MISC, "starting connector thread, eid %u", th->args.eid));
...
static int send_version_response(ENV * env, REPMGR_CONNECTION * conn)
{
	...
	if(vi.size == 0) {
		...
		if(DB_REPMGR_VERSION >= versions.min && DB_REPMGR_VERSION <= versions.max)
			 ...
			 */
			__db_errx(env, DB_STR_A("3622", "No available version between %lu and %lu", "%lu %lu"), (ulong)versions.min, (ulong)versions.max);
			return DB_REP_UNAVAIL;
		}
...
static int read_version_response(ENV*env, REPMGR_CONNECTION * conn)
{
	...
	if(vi.size == 0) {
		...
		if(conf.version >= DB_REPMGR_MIN_VERSION && conf.version <= DB_REPMGR_VERSION)
			 ...
			 */
			__db_errx(env, DB_STR_A("3623", "Can't support confirmed version %lu", "%lu"), (ulong)conf.version);
			return DB_REP_UNAVAIL;
		}
		if((ret = accept_handshake(env, conn, hostname)) != 0)
			return ret;
...
static int accept_handshake(ENV*env, REPMGR_CONNECTION * conn, char * hostname)
{
	__repmgr_handshake_args hs;
	...
	if(conn->state == CONN_CONNECTED) {
		...
		if((site = __repmgr_lookup_site(env, host, port)) != NULL && site->membership == SITE_PRESENT) {
			...
			if(LF_ISSET(REPMGR_SUBORDINATE)) {
				 ...
				 */
				TAILQ_INSERT_TAIL(&site->sub_conns, conn, entries);
				conn->eid = eid;
				...
				DB_EVENT(env, DB_EVENT_REP_CONNECT_ESTD, &eid);
				switch(site->state) {
				    case SITE_PAUSING:
					RPRINT(env, (env, DB_VERB_REPMGR_MISC, "handshake from paused site %s:%u EID %u", host, port, eid));
...
static int record_permlsn(ENV*env, REPMGR_CONNECTION * conn)
{
	...
	int ret;
	db_rep = env->rep_handle;
	DB_ASSERT(env, conn->version > 0 && IS_READY_STATE(conn->state) && IS_VALID_EID(conn->eid));
	site = SITE_FROM_EID(conn->eid);
	/*
	...
	if(ackp->generation < gen) {
		VPRINT(env, (env, DB_VERB_REPMGR_MISC, "ignoring stale ack (%lu<%lu), from %s", (ulong)ackp->generation, (ulong)gen, __repmgr_format_site_loc(site, location)));
		return 0;
	}
