...
static int __qam_adjust_first(DB*file_dbp, DBC * dbc, QMETA * meta, db_recno_t recno)
{
	...
	while(meta->first_recno != meta->cur_recno &&
		...
		if((ret = __qam_position(dbc, &meta->first_recno, 0, &exact)) != 0)
			return ret;
		if(cp->page != NULL && (ret = __qam_fput(dbc, cp->pgno, cp->page, dbc->priority)) != 0)
			return ret;
		if(exact == 1)
			break;
		if(cp->page != NULL && rec_ext != 0 && meta->first_recno%rec_ext == 0)
			if((ret = __qam_fremove(file_dbp, cp->pgno)) != 0)
				return ret;
		REC_DIRTY(file_dbp->mpf, dbc->thread_info, dbc->priority, &meta);
...
int __qam_incfirst_recover(ENV*env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		...
		if((ret = __qam_adjust_first(file_dbp, dbc, meta, argp->recno+1)) != 0)
			goto err;
	}
	...
	*lsnp = argp->prev_lsn;
	...
	if(0) {
err:
		__memp_fput(mpf, ip, meta, dbc->priority);
	}
out:
	REC_CLOSE;
}
...
int __qam_mvptr_recover(ENV*env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		...
		REC_DIRTY(mpf, ip, dbc->priority, &meta);
		cp = (QUEUE_CURSOR *)dbc->internal;
		if((argp->opcode&QAM_SETFIRST) && meta->first_recno == argp->old_first) {
		...
		if((argp->opcode&QAM_SETCUR) && meta->cur_recno == argp->old_cur) {
			if(argp->old_cur < argp->new_cur)
				meta->cur_recno = argp->new_cur;
			else {
				if((ret = __qam_position(dbc, &meta->cur_recno, 0, &exact)) != 0)
					goto err;
				if(!exact)
					meta->cur_recno = argp->new_cur;
				if(cp->page && (ret = __qam_fput(dbc, cp->pgno, cp->page, dbc->priority)) != 0)
...
int __qam_del_recover(ENV*env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		...
		if(meta->first_recno == RECNO_OOB || (QAM_BEFORE_FIRST(meta, argp->recno) && (meta->first_recno <= meta->cur_recno ||
		      meta->first_recno-argp->recno < argp->recno-meta->cur_recno))) {
			REC_DIRTY(mpf, ip, dbc->priority, &meta);
			meta->first_recno = argp->recno;
		}
		...
		if((ret = __qam_fput(dbc, argp->pgno, pagep, dbc->priority)) != 0)
			goto err;
		pagep = NULL;
		if((ret = __qam_adjust_first(file_dbp, dbc, meta, argp->recno)) != 0)
			goto err;
	}
done:   
	*lsnp = argp->prev_lsn;
	ret = 0;
err:    
	if(pagep && (t_ret = __qam_fput(dbc, argp->pgno, pagep, dbc->priority)) != 0 && ret == 0)
		ret = t_ret;
...
int __qam_delext_recover(ENV*env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_UNDO(op)) {
		...
		QAM_DIRTY(dbc, pagep->pgno, &pagep);
		if((ret = __qam_pitem(dbc, pagep, argp->indx, argp->recno, &argp->data)) != 0)
			goto err;
		/*
		 * Move the LSN back to this point;  do not move it forward.
		 * If we're in an abort, because we don't hold a page lock,
...
int __qam_add_recover(ENV*env, DBT * dbtp, DB_LSN * lsnp, db_recops op, void * info)
{
	...
	if(DB_REDO(op)) {
		...
		if(argp->recno == meta->cur_recno || QAM_AFTER_CURRENT(meta, argp->recno)) {
			REC_DIRTY(mpf, ip, dbc->priority, &meta);
		...
		if(argp->olddata.size != 0) {
			QAM_DIRTY(dbc, pagep->pgno, &pagep);
			if((ret = __qam_pitem(dbc, pagep, argp->indx, argp->recno, &argp->olddata)) != 0)
				goto err;
	...
	*lsnp = argp->prev_lsn;
	...
	if(0) {
err:
		__qam_fput(dbc, argp->pgno, pagep, dbc->priority);
	}
out:    REC_CLOSE;
}
