...
int __qam_vrfy_meta(DB * dbp, VRFY_DBINFO * vdp, const QMETA * meta, db_pgno_t pgno, uint32 flags)
{
	...
	if(!F_ISSET(pip, VRFY_INCOMPLETE))
		EPRINT((env, DB_STR_A("1146", "Page %lu: queue databases must be one-per-file", "%lu"), (ulong)pgno));
	/*
	 ...
	 */
	if(DB_ALIGN(meta->re_len+sizeof(QAMDATA)-1, sizeof(uint32))*meta->rec_page+QPAGE_SZ(dbp) > dbp->pgsize) {
		EPRINT((env, DB_STR_A("1147", "Page %lu: queue record length %lu too high for page size and recs/page",
				"%lu %lu"), (ulong)pgno, (ulong)meta->re_len));
		ret = DB_VERIFY_FATAL;
		goto err;
	}
	else {
	...
	if(F_ISSET(vdp, VRFY_QMETA_SET)) {
		isbad = 1;
		EPRINT((env, DB_STR_A("1148", "Page %lu: database contains multiple Queue metadata pages", "%lu"), (ulong)pgno));
		goto err;
	}
	...
	for(i = nextents = 0; i < count; i++) {
		if(strncmp(names[i], buf, len) == 0) {
			...
			if(qp->page_ext != 0 && (last > first ? (extid >= first && extid <= last) : (extid >= first || extid <= last)))
				continue;
			if(extents == NULL && (ret = __os_malloc(env, (size_t)(count-i)*sizeof(extid), &extents)) != 0)
				goto err;
...
begin:
	for(; pgno <= stop; pgno += pg_ext) {
		...
		for(i = 0; i < pg_ext && pgno+i <= last; i++)
			if((ret = __db_vrfy_pgset_inc(pgset, vdp->thread_info, vdp->txn, pgno+i)) != 0)
				goto err;
...
int __qam_vrfy_data(DB * dbp, VRFY_DBINFO * vdp, QPAGE * h, db_pgno_t pgno, uint32 flags)
{
	...
	for(i = 0; i < vdp->rec_page; i++) {
		...
		if((uint8 *)qp >= (uint8 *)h+dbp->pgsize) {
			EPRINT((dbp->env, DB_STR_A("1150", "Page %lu: queue record %lu extends past end of page", "%lu %lu"), (ulong)pgno, (ulong)i));
			return DB_VERIFY_BAD;
		}
		if(qp->flags&~(QAM_VALID|QAM_SET)) {
			EPRINT((dbp->env, DB_STR_A("1151", "Page %lu: queue record %lu has bad flags (%#lx)", "%lu %lu %#lx"), (ulong)pgno, (ulong)i, (ulong)qp->flags));
			return DB_VERIFY_BAD;
		}
	}
	return 0;
...
int __qam_vrfy_structure(DB * dbp, VRFY_DBINFO * vdp, uint32 flags)
{
	...
	if(pip->type != P_QAMMETA) {
		EPRINT((dbp->env, DB_STR_A("1152", "Page %lu: queue database has no meta page", "%lu"), (ulong)PGNO_BASE_MD));
		isbad = 1;
		goto err;
	}
	...
	for(i = 1; i <= vdp->last_pgno; i++) {
		...
		if(!F_ISSET(pip, VRFY_IS_ALLZEROES) && pip->type != P_QAMDATA) {
			EPRINT((dbp->env, DB_STR_A("1153", "Page %lu: queue database page of incorrect type %lu", "%lu %lu"), (ulong)i, (ulong)pip->type));
			isbad = 1;
			goto err;
		}
 ...
 */
int __qam_vrfy_walkqueue(DB * dbp, VRFY_DBINFO * vdp, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	DBC * dbc;
	ENV * env;
	PAGE * h;
...
begin:  for(; i <= stop; i++) {
		...
		if(LF_ISSET(DB_SALVAGE)) {
			...
			if(pip->type != P_QAMDATA) {
				EPRINT((env, DB_STR_A("1154", "Page %lu: queue database page of incorrect type %lu", "%lu %lu"), (ulong)i, (ulong)pip->type));
				isbad = 1;
				goto err;
			}
			if((ret = __db_vrfy_pgset_inc(vdp->pgset, vdp->thread_info, vdp->txn, i)) != 0)
				goto err;
...
put:
	...
	if(LF_ISSET(DB_SALVAGE) && nextents != 0) {
		nextents--;
		i = 1+vdp->extents[nextents]*vdp->page_ext;
		stop = i+vdp->page_ext;
		goto begin;
	}
	if(0) {
 ...
 */
int __qam_salvage(DB * dbp, VRFY_DBINFO * vdp, db_pgno_t pgno, PAGE * h, void * handle, int (*callback)__P((void *, const void *)), uint32 flags)
{
	DBT dbt, key;
	QAMDATA * qp, * qep;
	db_recno_t recno;
	...
	for(qp = QAM_GET_RECORD(dbp, h, i); qp < qep; recno++, i++, qp = QAM_GET_RECORD(dbp, h, i)) {
		...
		if(!LF_ISSET(DB_AGGRESSIVE) && !F_ISSET(qp, QAM_VALID))
			continue;
		dbt.data = qp->data;
		if((ret = __db_vrfy_prdbt(&key, 0, " ", handle, callback, 1, 0, vdp)) != 0)
			err_ret = ret;
