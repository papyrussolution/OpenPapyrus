...
void * FASTCALL gravity_calloc(gravity_vm * vm, size_t count, size_t size) 
{
	if(vm && ((count * size) >= gravity_vm_maxmemblock(vm))) {
		gravity_vm_seterror(vm, "Maximum memory allocation block size reached (req: %d, max: %lld).",
		    (count * size), (int64_t)gravity_vm_maxmemblock(vm));
		return NULL;
	}
	return SAlloc::C(count, size);
}

void * FASTCALL gravity_realloc(gravity_vm * vm, void * ptr, size_t new_size) 
{
	if(vm && (new_size >= gravity_vm_maxmemblock(vm))) {
		gravity_vm_seterror(vm, "Maximum memory re-allocation block size reached (req: %d, max: %lld).",
		    new_size, (int64_t)gravity_vm_maxmemblock(vm));
		return NULL;
	}
	return SAlloc::R(ptr, new_size);
}
...
static uint32 _ptr_lookup(void * ptr);
...
typedef struct {
	...
	uint32 currmem;
	uint32 maxmem;
	uint32 nslot;                   // number of slot filled with data
	uint32 aslot;                   // number of allocated slot
	memslot                    * slot;
} _memdebug;
...
void memdebug_init() 
{
	...
	memdebug.slot = (memslot *)SAlloc::M(sizeof(memslot) * SLOT_MIN);
...
void * memdebug_realloc(gravity_vm * vm, void * ptr, size_t new_size) 
{
	...
	void * new_ptr = SAlloc::R(ptr, new_size);
	if(!ptr) {
...
void _ptr_add(void * ptr, size_t size) 
{
	...
	if(memdebug.nslot + 1 >= memdebug.aslot) {
		...
		memslot * new_slot = (memslot *)SAlloc::R(memdebug.slot, old_size+new_size);
		if(!new_slot) {
...
{
	CHECK_FLAG();
	uint32 index = _ptr_lookup(old_ptr);
	if(index == SLOT_NOTFOUND) {
