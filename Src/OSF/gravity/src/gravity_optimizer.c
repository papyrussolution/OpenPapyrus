...
inline static bool pop1_instruction(ircode_t * code, uint32 index, inst_t ** inst1) 
{
	*inst1 = NULL;
...
inline static bool pop2_instructions(ircode_t * code, uint32 index, inst_t ** inst1, inst_t ** inst2) 
{
	*inst1 = NULL;
	*inst2 = NULL;
	for(int32 i = index-1; i>=0; --i) {
...
inline static inst_t * current_instruction(ircode_t * code, uint32 i) {
	while(1) {
		inst_t * inst = ircode_get(code, i);
...
static bool optimize_neg_instruction(ircode_t * code, inst_t * inst, uint32 i) 
{
	inst_t * inst1 = NULL;
...
static bool optimize_math_instruction(ircode_t * code, inst_t * inst, uint32 i) 
{
...
static bool optimize_move_instruction(ircode_t * code, inst_t * inst, uint32 i) 
{
	inst_t * inst1 = NULL;
	pop1_instruction(code, i, &inst1);
	if(inst1 == NULL) return false;
...
static bool optimize_return_instruction(ircode_t * code, inst_t * inst, uint32 i) 
{
	inst_t * inst1 = NULL;
	pop1_instruction(code, i, &inst1);
	if(!ircode_register_istemp(code, inst->p1)) 
		return false;
