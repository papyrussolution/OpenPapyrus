...
{
	*idx_start = (uint32)TOK_KEY_FUNC;
	*idx_end = (uint32)TOK_KEY_CURRARGS;
};

gtoken_t token_special_builtin(gtoken_s * token) 
{
	const char * buffer = token->value;
	int32 len = token->bytes;
	switch(len) {
		case 8:
...
bool token_isvariable_assignment(gtoken_t token) { return (token == TOK_OP_ASSIGN); }
bool token_isaccess_specifier(gtoken_t token) { return ((token == TOK_KEY_PRIVATE) || (token == TOK_KEY_INTERNAL) || (token == TOK_KEY_PUBLIC)); }
bool token_isstorage_specifier(gtoken_t token) { return ((token == TOK_KEY_STATIC) || (token == TOK_KEY_EXTERN) || (token == TOK_KEY_LAZY)); }
...
bool token_isimport_statement(gtoken_t token) { return (token == TOK_KEY_IMPORT); }
bool token_isspecial_statement(gtoken_t token) { return (token == TOK_SPECIAL); }
bool token_isoperator(gtoken_t token) { return ((token >= TOK_OP_SHIFT_LEFT) && (token <= TOK_OP_NOT)); }
bool token_ismacro(gtoken_t token) { return (token == TOK_MACRO); }
bool token_iserror(gtoken_t token) { return (token == TOK_ERROR); }
bool token_iseof(gtoken_t token) { return (token == TOK_EOF); }
