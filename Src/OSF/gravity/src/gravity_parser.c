...
struct gravity_parser_t {
	...
	{
		return static_cast<gravity_lexer_t *>(lexer->getLast());
	}
	//#define DECLARE_LEXER_                           gravity_lexer_t * lexer = parser->GetCurrentLexer(); DEBUG_LEXER(lexer)
	gravity_lexer_t * GetCurrentLexerAndDebug()
	{
		//gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
		gravity_lexer_t * p_lexer = static_cast<gravity_lexer_t *>(lexer->getLast());
		DEBUG_LEXER(p_lexer);
		return p_lexer;
	}
	...
	GravityArray <uint16> vdecl; // to keep track of func expression in variable declaration nondes
	double time;
	uint32 nerrors;
	uint32 unique_id;
	uint32 last_error_lineno;
	uint32 depth;                                     // to keep track of maximum statements depth
	uint32 expr_depth;                                // to keep track of maximum expression depth
...
struct grammar_rule {
	grammar_rule() : prefix(0), infix(0), precedence(PREC_LOWEST), name(0), right(false)
	{
	}
	grammar_rule(parse_func fnPrefix, parse_func fnInfix, prec_level prec, const char * pName, bool isRight) :
		prefix(fnPrefix), infix(fnInfix), precedence(prec), name(pName), right(isRight)
	{
	}
	parse_func prefix;
	parse_func infix;
...
#define PUSH_DECLARATION(_decl)                 parser->declarations->insert(_decl)
#define POP_DECLARATION()                       parser->declarations->pop()
#define LAST_DECLARATION()                      ((parser->declarations)->getCount() ? parser->declarations->getLast() : NULL)
...
#define POP_LEXER                               (parser->lexer->pop())
//#define CURRENT_LEXER                           (static_cast<gravity_lexer_t *>(parser->lexer->getLast()))
//#define DECLARE_LEXER_                           gravity_lexer_t * lexer = parser->GetCurrentLexer(); DEBUG_LEXER(lexer)
...
#define STATIC_TOKEN_CSTRING(_s, _n, _l, _b, _t)    char _s[_n] = {0}; uint32 _l = 0;            \
	const char * _b = token_string(_t, &_l);        \
	if(_l) memcpy(_s, _b, MIN(_n, _l))

// MARK: - Prototypes -
static const char * FASTCALL parse_identifier(gravity_parser_t * parser);
static gnode_t * parse_statement(gravity_parser_t * parser);
static GravityArray <gnode_t *> * parse_optional_parameter_declaration(gravity_parser_t * parser, bool is_implicit, bool * has_default_values);
...
{
	int32 n = (int32)gnode_array_size(parser->declarations);
	if(n) {
		--n;
		while(n >= 0) {
			gnode_t * decl = gnode_array_get(parser->declarations, static_cast<size_t>(n));
			if(!decl) 
				return NULL;
			else if(decl->Tag == tag) 
				return decl;
			--n;
		}
	}
...
static void report_error(gravity_parser_t * parser, GravityErrorType error_type, gtoken_s token, const char * format, ...) 
{
	// just one error for each line
	if(parser->last_error_lineno == token.lineno) 
		return;
	...
	// setup error struct
	GravityErrorDescription error_desc(token.lineno, token.colno, token.fileid, token.position);
	// finally call error callback
	if(error_fn) 
		error_fn(NULL, error_type, buffer, &error_desc, data);
	else 
		printf("%s\n", buffer);
}

static gnode_t * FASTCALL parse_error(gravity_parser_t * parser) 
{
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
...
static bool FASTCALL parse_required(gravity_parser_t * parser, gtoken_t token) 
{
	if(parse_optional(parser, token)) 
		return true;
...
static gnode_t * local_store_declaration(gravity_parser_t * parser, const char * identifier, const char * annotation_type,
    gtoken_t access_specifier, gtoken_t storage_specifier, gnode_t * declaration) 
{
	GravityArray <gnode_t *> * decls = gnode_array_create();
	...
	gnode_t * decl = gnode_variable_create(declaration->Token, identifier ? sstrdup(identifier) : NULL,
		annotation_type, declaration, LAST_DECLARATION(), vdecl);
...
static gliteral_t decode_number_octal(gtoken_s token, int64_t * n) 
{
	...
	if(len) 
		*n = (int64_t)number_from_oct(&str[2], len-2);
	return LITERAL_INT;
}
...
static gliteral_t decode_number_hex(gtoken_s token, int64_t * n, double * d) 
{
	STATIC_TOKEN_CSTRING(str, 512, len, buffer, token);
	if(len) 
		*n = (int64_t)number_from_hex(str, token.bytes);
	return LITERAL_INT;
}
...
static gnode_t * parse_parentheses_expression(gravity_parser_t * parser) 
{
	...
	parse_required(parser, TOK_OP_OPEN_PARENTHESIS); // check and consume TOK_OP_OPEN_PARENTHESIS
	gnode_t * expr = parse_expression(parser); // parse expression
	CHECK_NODE(expr);
	parse_required(parser, TOK_OP_CLOSED_PARENTHESIS); // check and consume TOK_OP_CLOSED_PARENTHESIS
	return expr;
}
...
static gnode_t * parse_list_expression(gravity_parser_t * parser) 
{
	...
	GravityArray <gnode_t *> * list1 = gnode_array_create();
	GravityArray <gnode_t *> * list2 = (ismap) ? gnode_array_create() : NULL;
	if(expr1) 
		gnode_array_push(list1, expr1);
	if(ismap) {
		parse_required(parser, TOK_OP_COLON);
		gnode_t * expr2 = parse_expression(parser);
		if(expr2) 
			gnode_array_push(list2, expr2);
	}
...
static gnode_t * parse_function_expression(gravity_parser_t * parser) 
{
	...
	gnode_t * node = parse_function(parser, false, /*0*/TOK_EOF, /*0*/TOK_EOF);
	return node;
}
...
static gnode_t * parse_identifier_or_keyword_expression(gravity_parser_t * parser) 
{
	...
	uint32 idx_start, idx_end;
	token_keywords_indexes(&idx_start, &idx_end);
	gtoken_t peek = gravity_lexer_peek(lexer);
	if((static_cast<uint32>(peek) >= idx_start) && (static_cast<uint32>(peek) <= idx_end)) {
		// consume token keyword
		gtoken_t keyword = gravity_lexer_next(lexer);
		gtoken_s token = gravity_lexer_token(lexer);
		// convert from keyword to identifier
		const char * identifier = sstrdup(token_name(keyword));
		return gnode_identifier_expr_create(token, identifier, NULL, LAST_DECLARATION());
	}
	// default case
...
static gnode_t * parse_number_expression(gravity_parser_t * parser, gtoken_s token) 
{
	...
	if(token.builtin != gtoken_s::BUILTIN_NONE) {
		if(token.builtin == gtoken_s::BUILTIN_LINE) 
			return gnode_literal_int_expr_create(token, token.lineno, LAST_DECLARATION());
		else if(token.builtin == gtoken_s::BUILTIN_COLUMN) 
			return gnode_literal_int_expr_create(token, token.colno, LAST_DECLARATION());
	}
...
report_node:
	if(type == LITERAL_FLOAT) 
		return gnode_literal_float_expr_create(token, (double)d, LAST_DECLARATION());
	else if(type == LITERAL_INT) 
		return gnode_literal_int_expr_create(token, n, LAST_DECLARATION());
	else 
		assert(0);
	return NULL;
}

static gnode_t * parse_analyze_literal_string(gravity_parser_t * parser, gtoken_s token, const char * s, uint32 len) 
{
	// check for special built-in cases first
	if(token.builtin != gtoken_s::BUILTIN_NONE) {
		if(token.builtin == gtoken_s::BUILTIN_FILE) {
			if(parser->delegate && parser->delegate->filename_callback) {
				...
				SETIFZ(filename, "");
				return gnode_literal_string_expr_create(token, (char *)filename, (uint32)strlen(filename), false, LAST_DECLARATION());
			}
		}
		else if(token.builtin == gtoken_s::BUILTIN_FUNC) {
			...
			return gnode_literal_string_expr_create(token, (char *)identifier, (uint32)strlen(identifier), false, LAST_DECLARATION());
		}
		else if(token.builtin == gtoken_s::BUILTIN_CLASS) {
	...
	for(uint32 i = 0; i < len;) {
		...
		if(c == '\\') {
			...
			if(i+1 >= len) {
				REPORT_ERROR(token, "Unexpected EOF inside a string literal"); goto return_string;
			}
			switch(s[i+1]) {
				case '\'': c = '\''; ++i; break;
				case '"':  c = '"'; ++i; break;
				...
				case 'u':  {
				    ...
				    uint32 n = (uint32)strtoul(b, NULL, 16);
				...
				case '(': {
				    ...
				    if(subnode) {
						...
						SETIFZ(r, gnode_array_create());
					    if(length) 
							gnode_array_push(r, gnode_literal_string_expr_create(token, buffer, length, true, LAST_DECLARATION()));
				    ...
				    if(!subnode) 
						goto return_string;
				    buffer = static_cast<char *>(mem_alloc(NULL, len+1));
				    length = 0;
				    continue;
			    }
				default:
...
gnode_t * parse_literal_expression(gravity_parser_t * parser) 
{
	...
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	const gtoken_t type = gravity_lexer_next(lexer);
	const gtoken_s token = gravity_lexer_token(lexer);
	if(type == TOK_STRING) {
		uint32 len = 0;
	...
	if(oneof2(type, TOK_KEY_TRUE, TOK_KEY_FALSE)) {
		return gnode_literal_bool_expr_create(token, (int32)BIN(type == TOK_KEY_TRUE), LAST_DECLARATION());
	}
	if(type != TOK_NUMBER) {
...
static GravityArray <gnode_t *> * parse_arguments_expression(gravity_parser_t * parser) 
{
	...
	// if the call is not a named call then the useless array is discarded
	...
	while(1) {
		...
		if(peek == TOK_OP_COMMA) {
			...
			if(oneof2(peek, TOK_EOF, TOK_OP_CLOSED_PARENTHESIS))
				break;
...
static gnode_t * parse_postfix_expression(gravity_parser_t * parser, gtoken_t tok) 
{
	...
	while(1) {
		...
		if(tok == TOK_OP_OPEN_SQUAREBRACKET) {
			...
			GravityArray <gnode_t *> * args = parse_arguments_expression(parser); // can be NULL and it's OK
			gtoken_s subtoken = gravity_lexer_token(lexer);
			parse_required(parser, TOK_OP_CLOSED_PARENTHESIS);
...
static gnode_t * parse_infix(gravity_parser_t * parser) 
{
	...
	grammar_rule * rule = &rules[tok];
	prec_level precedence = (rule->right) ? static_cast<prec_level>(rule->precedence-1) : rule->precedence;
...
static gnode_t * parse_getter_setter(gravity_parser_t * parser) 
{
	...
	while(gravity_lexer_peek(lexer) != TOK_OP_CLOSED_CURLYBRACE) {
		...
		gtoken_s token = gravity_lexer_token(lexer);
		GravityArray <gnode_t *> * params = NULL;
		// getter case: does not have explicit parameters (only implicit self)
		if(sstreq(identifier, GETTER_FUNCTION_NAME)) {
			is_getter = true;
			params = gnode_array_create(); // add implicit SELF param
			gnode_array_push(params,
			    gnode_variable_create(NO_TOKEN, sstrdup(SELF_PARAMETER_NAME), NULL, NULL, LAST_DECLARATION(), NULL));
		}

		// setter case: could have explicit parameters (otherwise value is implicit)
		if(sstreq(identifier, SETTER_FUNCTION_NAME)) {
			is_getter = false;
			...
			if(gravity_lexer_peek(lexer) == TOK_OP_OPEN_PARENTHESIS) {
				...
				gnode_array_push(params,
				    gnode_variable_create(NO_TOKEN, sstrdup(SELF_PARAMETER_NAME), NULL, NULL, LAST_DECLARATION(), NULL));
				gnode_array_push(params,
				    gnode_variable_create(NO_TOKEN, sstrdup(SETTER_PARAMETER_NAME), NULL, NULL, LAST_DECLARATION(),
				    NULL));
			}
		}
		mem_free(identifier);
		...
		gnode_t * f = gnode_function_decl_create(token, NULL, /*0*/TOK_EOF, /*0*/TOK_EOF, params, NULL, LAST_DECLARATION());
...
static gnode_t * parse_variable_declaration(gravity_parser_t * parser, bool isstatement, gtoken_t access_specifier, gtoken_t storage_specifier) 
{
	...
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	GravityArray <gnode_t *> * decls = NULL;
	gnode_t * decl = NULL;
	gnode_t * expr = NULL;
	const char * identifier = NULL;
	const char * type_annotation = NULL;
...
static gnode_t * parse_enum_declaration(gravity_parser_t * parser, gtoken_t access_specifier, gtoken_t storage_specifier) 
{
	...
	uint32 enum_counter = 0;          // enum internal counter (first value (if any) determines enum type)
	...
	while(1) {
		...
		if(gravity_lexer_peek(lexer) == TOK_OP_CLOSED_CURLYBRACE) 
			break;
		...
		if(enum_value) {
			...
			if(enum_value->Tag == NODE_LITERAL_EXPR) {
				enum_literal = (gnode_literal_expr_t *)enum_value;
			}
			else if(enum_value->Tag == NODE_UNARY_EXPR) {
				...
				if(unary->op == TOK_OP_SUB) {
					...
					if(enum_literal->type == LITERAL_FLOAT) {
						...
						temp = gnode_literal_float_expr_create(enum_value->Token, -enum_literal->value.d, LAST_DECLARATION());
					}
					else if(enum_literal->type == LITERAL_INT) {
						//enum_literal->value.n64 = -enum_literal->value.n64;
						temp = gnode_literal_int_expr_create(enum_value->Token, -enum_literal->value.n64, LAST_DECLARATION());
					}
					if(temp) {
						gnode_free(enum_value);
			...
			if(enum_counter == 0) {
				if(oneof3(enum_literal->type, LITERAL_STRING, LITERAL_FLOAT, LITERAL_BOOL))
					enum_type = enum_literal->type;
			}
		...
		++enum_counter; // update internal enum counter
		// enum identifier could be NULL due to an already reported error
		if(enum_id) {
			if(!symboltable_insert(symtable, enum_id, enum_value)) {
				REPORT_ERROR(enumid_token, "Identifier %s redeclared.", enum_id);
				gnode_free(enum_value); // free value here because it has not beed saved into symbol table
			}
		...
		if(peek != TOK_OP_COMMA) 
			break;
		gravity_lexer_next(lexer); // consume TOK_OP_COMMA and continue loop
	}
	parse_required(parser, TOK_OP_CLOSED_CURLYBRACE); // check and consume TOK_OP_CLOSED_CURLYBRACE
	parse_semicolon(parser); // consume semicolon
	// check for empty enum (not allowed)
	if(enum_counter == 0) {
		REPORT_ERROR(token, "Empty enum %s not allowed.", identifier);
	}
	if(IS_FUNCTION_ENCLOSED()) 
		return local_store_declaration(parser, identifier, NULL, access_specifier, storage_specifier, node);
	return node;
}
...
static gnode_t * parse_module_declaration(gravity_parser_t * parser, gtoken_t access_specifier, gtoken_t storage_specifier) 
{
	...
	GravityArray <gnode_t *> * declarations = gnode_array_create();
	// create module node
	gnode_t * node = NULL;//gnode_module_decl_create(token, identifier, access_specifier, storage_specifier,
	                      // declarations, meta, LAST_DECLARATION());
	while(token_isdeclaration_statement(gravity_lexer_peek(lexer))) {
		gnode_t * decl = parse_declaration_statement(parser);
		if(decl) 
			gnode_array_push(declarations, decl);
	}
...
static GravityArray <gnode_t *> * parse_protocols(gravity_parser_t * parser) 
{
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	gnode_t * node = NULL;
	GravityArray <gnode_t *> * list = NULL;
	// (id) (',' id)*
	gtoken_t peek = gravity_lexer_peek(lexer);
	if(peek == TOK_OP_GREATER) 
		return NULL; // just an empty protocols implementation statement
	list = gnode_array_create();
loop:
	if(!token_isidentifier(peek)) 
		goto abort;
...
static gnode_t * parse_class_declaration(gravity_parser_t * parser, gtoken_t access_specifier, gtoken_t storage_specifier) 
{
	...
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	// access_specifier? storage_specifier? 'class' IDENTIFIER class_superclass? class_protocols? '{'
	...
	gnode_t * super = NULL;
	GravityArray <gnode_t *> * protocols = NULL;
	gtoken_t peek = gravity_lexer_peek(lexer);
	if(peek == TOK_OP_COLON) {
		gravity_lexer_next(lexer); // consume TOK_OP_COLON
	...
	if(IS_CLASS_ENCLOSED()) {
		GravityArray <gnode_t *> * decls = gnode_array_create();
		gnode_t * outer_var = gnode_variable_create(NO_TOKEN, sstrdup(OUTER_IVAR_NAME), NULL, NULL, LAST_DECLARATION(), NULL);
		gnode_array_push(decls, outer_var);
		gnode_t * outer_decl = gnode_variable_decl_create(NO_TOKEN, TOK_KEY_VAR, TOK_KEY_PRIVATE, /*0*/TOK_EOF, decls, LAST_DECLARATION());
		gnode_array_push(declarations, outer_decl);
	}
	// create class declaration node
	gnode_class_decl_t * node = (gnode_class_decl_t *)gnode_class_decl_create(token, identifier, access_specifier,
		storage_specifier, super, protocols, NULL, is_struct, LAST_DECLARATION());
	if(storage_specifier != TOK_KEY_EXTERN) {
		...
		while(token_isdeclaration_statement(peek) || token_ismacro(peek)) {
			gnode_t * decl = parse_declaration_statement(parser);
			if(decl) 
				gnode_array_push(declarations, decl_check_access_specifier(decl));
			peek = gravity_lexer_peek(lexer);
		}
		POP_DECLARATION();
	}
	...
	if(IS_FUNCTION_ENCLOSED()) 
		return local_store_declaration(parser, identifier, sstrdup(class_manifest_type), access_specifier, storage_specifier, node);
	return node;
}
...
static GravityArray <gnode_t *> * parse_optional_parameter_declaration(gravity_parser_t * parser, bool is_implicit, bool * has_default_values) 
{
	...
	GravityArray <gnode_t *> * params = gnode_array_create();
	assert(params);
	...
	// sstrdup mandatory here because when the node will be freed
	// memory for the identifier will be deallocated
	node = gnode_variable_create(token, sstrdup(SELF_PARAMETER_NAME), type_annotation, NULL, LAST_DECLARATION(), NULL);
...
{
	if(sstreq(identifier, "name")) return UNITTEST_NAME;
	if(sstreq(identifier, "note")) return UNITTEST_NOTE;
	if(sstreq(identifier, "error")) return UNITTEST_ERROR;
	if(sstreq(identifier, "error_row")) return UNITTEST_ERROR_ROW;
	if(sstreq(identifier, "error_col")) return UNITTEST_ERROR_COL;
	if(sstreq(identifier, "result")) return UNITTEST_RESULT;
...
static gnode_t * parse_unittest_macro(gravity_parser_t * parser) 
{
	...
	while(gravity_lexer_peek(lexer) != TOK_OP_CLOSED_CURLYBRACE) {
		...
		if(type == UNITTEST_NAME) {
			...
			if(oneof2(peek, TOK_OP_SUB, TOK_OP_ADD))
			...
			if(value_node == NULL) 
				goto handle_error;
			// if a negative sign has been parsed then manually fix the literal expression (if it is a
			// number)
			if(op == TOK_OP_SUB) {
				if(value_node->type == LITERAL_INT) 
					value_node->value.n64 = -value_node->value.n64;
				else if(value_node->type == LITERAL_FLOAT) 
					value_node->value.d = -value_node->value.d;
			}
		}
		else {
...
handle_continue:
	...
	parse_semicolon(parser);
	{
		...
		char buffer[1024];
		char buffer2[1024];
		GravityErrorType expected_error = GRAVITY_ERROR_NONE;
		GravityValue expected_value = GravityValue::from_null();
		int32 expected_nrow = -1;
		int32 expected_ncol = -1;
		...
		if(err_node) {
			if(sstreq(err_node->value, "NONE"))
				expected_error = GRAVITY_ERROR_NONE;
			else if(sstreq(err_node->value, "SYNTAX"))
				expected_error = GRAVITY_ERROR_SYNTAX;
			else if(sstreq(err_node->value, "SEMANTIC"))
				expected_error = GRAVITY_ERROR_SEMANTIC;
			else if(sstreq(err_node->value, "RUNTIME"))
				expected_error = GRAVITY_ERROR_RUNTIME;
			else if(sstreq(err_node->value, "WARNING"))
				expected_error = GRAVITY_WARNING;
		}
		// decode error line/col
		if((row_node) && (row_node->type == LITERAL_INT)) {
			expected_nrow = (int32)row_node->value.n64;
		}
		if((col_node) && (col_node->type == LITERAL_INT)) {
			expected_ncol = (int32)col_node->value.n64;
		}
		// decode unittest expected result
		if(value_node) {
			if(value_node->type == LITERAL_STRING)
				expected_value = gravity_zstring_to_value(NULL, value_node->value.str);
			else if(value_node->type == LITERAL_INT)
				expected_value = GravityValue::from_int((gravity_int_t)value_node->value.n64);
			else if(value_node->type == LITERAL_FLOAT)
				expected_value = GravityValue::from_float((gravity_float_t)value_node->value.d);
		...
		if(parser->delegate && parser->delegate->unittest_callback) {
			...
			// it was unit test responsability to free expected_value but if no unit test delegate is set I should
			// take care of it
			gravity_value_free(NULL, expected_value);
		}
		// free temp nodes
		gnode_free(name_node);
		gnode_free(note_node);
		gnode_free(err_node);
		gnode_free(row_node);
		gnode_free(col_node);
		gnode_free(value_node);
		// always return NULL
		return NULL;
	}
handle_error:
...
loop:
	...
	if(source) 
		newlexer = gravity_lexer_create(source, size, fileid, is_static);
	if(newlexer) {
		// push new lexer into lexer stack
		parser->lexer->insert(newlexer);
	}
	else {
		REPORT_ERROR(token, "Unable to load file %s.", module_name);
	}
	// cleanup memory
	mem_free(module_name);
...
static gnode_t * parse_flow_statement(gravity_parser_t * parser) {
	...
	return gnode_flow_stat_create(token, cond, stmt1, stmt2, LAST_DECLARATION(),
		   end_token.position + end_token.length - token.position);
}
...
static gnode_t * parse_loop_statement(gravity_parser_t * parser) {
	...
	if(type == TOK_KEY_FOR) {
		...
		if(token_isvariable_declaration(gravity_lexer_peek(lexer))) {
			cond = parse_variable_declaration(parser, false, /*0*/TOK_EOF, /*0*/TOK_EOF);
		}
		else {
			cond = parse_expression(parser);
		}
...
static gnode_t * parse_jump_statement(gravity_parser_t * parser) 
{
	...
	return new gnode_jump_stmt_t(token, LAST_DECLARATION(), expr);
}
...
static gnode_t * parse_compound_statement(gravity_parser_t * parser) 
{
	...
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	gtoken_s token = gravity_lexer_token(lexer);
	GravityArray <gnode_t *> * stmts = gnode_array_create();
...
static gnode_t * parse_declaration_statement(gravity_parser_t * parser) 
{
	...
	gravity_lexer_t * lexer = parser->GetCurrentLexerAndDebug();
	gtoken_t peek = gravity_lexer_peek(lexer);
	gtoken_t access_specifier = TOK_EOF/*0*/; // 0 means no access specifier set
	gtoken_t storage_specifier = TOK_EOF/*0*/; // 0 means no storage specifier set
...
static void parser_register_core_classes(gravity_parser_t * parser) 
{
	...
	while(list[i]) {
		...
		gnode_t * node = gnode_variable_create(NO_TOKEN, sstrdup(identifier), NULL, NULL, LAST_DECLARATION(), NULL);
		gnode_array_push(decls, node);
		++i;
	}
	// register a variable declaration node in global statements
	gnode_t * node = gnode_variable_decl_create(NO_TOKEN, TOK_KEY_VAR, /*0*/TOK_EOF, TOK_KEY_EXTERN, decls, LAST_DECLARATION());;
	gnode_array_push(parser->statements, node);
}
...
static void parser_register_optional_classes(gravity_parser_t * parser) 
{
	...
	while(list[i]) {
		const char * identifier = list[i];
		gnode_t * decl = gnode_variable_create(NO_TOKEN, sstrdup(identifier), NULL, NULL, LAST_DECLARATION(), NULL);
		gnode_array_push(decls, decl);
		++i;
	}
	...
	if(parser->delegate && parser->delegate->optional_classes) {
		...
		while(list[i]) {
			const char * identifier = list[i];
			gnode_t * decl_node = gnode_variable_create(NO_TOKEN, sstrdup(identifier), NULL, NULL, LAST_DECLARATION(), NULL);
...
static uint32 parser_run(gravity_parser_t * parser) 
{
	...
	do {
		while(gravity_lexer_peek(parser->GetCurrentLexer())) {
			gnode_t * node = parse_statement(parser);
			if(node) 
				gnode_array_push(parser->statements, node);
		}
		// since it is a stack of lexers then check if it is a real EOF
		gravity_lexer_t * lexer = parser->GetCurrentLexer();
		gravity_lexer_free(lexer);
		parser->lexer->pop();
	} while(parser->lexer->getCount());
...
gnode_t * gravity_parser_run(gravity_parser_t * parser, gravity_delegate_t * delegate) 
{
	...
	gravity_lexer_setdelegate(parser->GetCurrentLexer(), delegate);
