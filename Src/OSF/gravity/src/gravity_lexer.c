...
static inline bool is_whitespace(int c) { return ((c == ' ') || (c == '\t') || (c == '\v') || (c == '\f')); }
...
static inline bool is_comment(int c1, int c2) { return (c1 == '/') && ((c2 == '*') || (c2 == '/')); }
static inline bool is_semicolon(int c) { return (c == ';'); }
static inline bool is_alpha(int c) { return (c == '_') ? true : LOGIC(isalpha(c)); }
...
static inline bool is_special(int c) { return (c == '@'); }
...
static /*inline*/ bool FASTCALL next_utf8(gravity_lexer_t * lexer, int * result) 
{
	int c = NEXT;
	INC_TOKLEN;
	// the following explicit conversion fixes an issue with big-endian processor (like Motorola 68000, PowerPC, Sun
	// Sparc and IBM S/390)
...
static gtoken_t lexer_scan_identifier(gravity_lexer_t * lexer) 
{
	...
	if(type == TOK_IDENTIFIER) 
		type = token_keyword(lexer->token.value, lexer->token.bytes);
	SET_TOKTYPE(type);
    #if GRAVITY_LEXEM_DEBUG
	if(type == TOK_IDENTIFIER) 
		DEBUG_LEXEM("Found identifier: %.*s", TOKEN_BYTES(lexer->token), TOKEN_VALUE(lexer->token));
	else 
		DEBUG_LEXEM("Found keyword: %s", token_name(type));
    #endif
	return type;
}
...
static gtoken_t lexer_scan_number(gravity_lexer_t * lexer) 
{
	...
	if(PEEK_CURRENT == '0') {
		if(toupper(PEEK_NEXT) == 'X') {
			ntype = NUMBER_HEX; 
			floatAllowed = false; 
			expAllowed = false;
		}
		else if(toupper(PEEK_NEXT) == 'B') {
			ntype = NUMBER_BIN; 
			floatAllowed = false; 
			expAllowed = false;
		}
		else if(toupper(PEEK_NEXT) == 'O') {
			ntype = NUMBER_OCT; 
			floatAllowed = false; 
			expAllowed = false;
		}
...
loop:
	...
	if(expAllowed) {
		if((c == expChar) && (!expFound)) {
			expFound = true; signAllowed = true; goto accept_char;
		}
	}
	if(floatAllowed) {
		if((c == floatChar) && (!is_digit(PEEK_NEXT, ntype))) goto report_token;
		if((c == floatChar) && (!dotFound)) {
			dotFound = true; goto accept_char;
		}
	}
	if(signAllowed) {
		if((c == plusSign) || (c == minusSign)) {
			signAllowed = false; goto accept_char;
		}
	}
...
static gtoken_t lexer_scan_string(gravity_lexer_t * lexer) 
{
	...
	INC_OFFSET_POSITION; // skip last escape character
	TOKEN_FINALIZE(TOK_STRING); // string is from buffer->[nseek] and it is nlen length
	DEBUG_LEXEM("Found string: %.*s", TOKEN_BYTES(lexer->token), TOKEN_VALUE(lexer->token));
	return TOK_STRING;
}

static gtoken_t FASTCALL lexer_scan_operator(gravity_lexer_t * lexer) 
{
	TOKEN_RESET;
	INC_TOKLEN;
	int c = NEXT;
	int c2 = PEEK_CURRENT;
	/*int*/gtoken_t tok = (gtoken_t)0;
	switch(c) {
		case '=':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; c2 = PEEK_CURRENT;
			    if(c2 == '=') {
				    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_ISIDENTICAL;
			    }
		...
		case '+':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_ADD_ASSIGN;
		    }
		    else tok = TOK_OP_ADD;
		    break;
		case '-':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_SUB_ASSIGN;
		    }
		    else tok = TOK_OP_SUB;
		    break;
		case '*':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_MUL_ASSIGN;
		    }
		    else tok = TOK_OP_MUL;
		    break;
		case '/':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_DIV_ASSIGN;
		    }
		    else tok = TOK_OP_DIV;
		    break;
		case '%':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_REM_ASSIGN;
		    }
		    else tok = TOK_OP_REM;
		    break;
		case '<':
		    if(c2 == '=') {
				...
				c2 = PEEK_CURRENT;
			    if(c2 == '=') {
				    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_SHIFT_LEFT_ASSIGN;
			    }
			    else 
					tok = TOK_OP_SHIFT_LEFT;
		    }
		...
		case '>':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; 
				INC_TOKLEN; 
				tok = TOK_OP_GREATER_EQUAL;
		    }
		    else if(c2 == '>') {
			    INC_OFFSET_POSITION; INC_TOKLEN; c2 = PEEK_CURRENT;
			    if(c2 == '=') {
				    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_SHIFT_RIGHT_ASSIGN;
			    }
		...
		case '&':
		    if(c2 == '&') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_AND;
		    }
		    else if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_BIT_AND_ASSIGN;
		    }
		    else tok = TOK_OP_BIT_AND;
		    break;
		case '|':
		    if(c2 == '|') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_OR;
		    }
		    else if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_BIT_OR_ASSIGN;
		    }
		    else tok = TOK_OP_BIT_OR;
		    break;
		case '.': // check for special .digit case
		    if(is_digit(c2, NUMBER_INTEGER)) {
			    DEC_OFFSET_POSITION; DEC_TOKLEN; tok = lexer_scan_number(lexer);
		    }
		    else if(c2 == '.') {
		...
		case '!':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; c2 = PEEK_CURRENT;
			    if(c2 == '=') {
				    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_ISNOTIDENTICAL;
			    }
		...
		case '^':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_BIT_XOR_ASSIGN;
		    }
		    else tok = TOK_OP_BIT_XOR;
		    break;
		case '~':
		    if(c2 == '=') {
			    INC_OFFSET_POSITION; INC_TOKLEN; tok = TOK_OP_PATTERN_MATCH;
		    }
		    else tok = TOK_OP_BIT_NOT;
		    break;
		case ':': tok = TOK_OP_COLON; break;
		case '{': tok = TOK_OP_OPEN_CURLYBRACE; break;
		case '}': tok = TOK_OP_CLOSED_CURLYBRACE; break;
		case '[': tok = TOK_OP_OPEN_SQUAREBRACKET; break;
		case ']': tok = TOK_OP_CLOSED_SQUAREBRACKET; break;
		case '(': tok = TOK_OP_OPEN_PARENTHESIS; break;
		case ')': tok = TOK_OP_CLOSED_PARENTHESIS; break;
		case '?': tok = TOK_OP_TERNARY; break;
		default: return lexer_error(lexer, "Unrecognized Operator");
	}
	TOKEN_FINALIZE(tok);
...
{
	gravity_lexer_t * lexer = static_cast<gravity_lexer_t *>(mem_alloc(NULL, sizeof(gravity_lexer_t)));
	if(lexer) {
		memzero(lexer, sizeof(gravity_lexer_t));
		lexer->is_static = is_static;
		lexer->lineno = 1;
		lexer->buffer = source;
		lexer->length = (uint32)len;
		lexer->fileid = fileid;
		lexer->peeking = false;
		lexer->cache = TOK_END;
	}
...
loop:
	...
	if(is_comment(c, PEEK_NEXT)) {
		lexer_scan_comment(lexer); goto loop;
	}
	if(is_semicolon(c)) {
		token = lexer_scan_semicolon(lexer); goto return_result;
	}
	if(is_alpha(c)) {
		token = lexer_scan_identifier(lexer); goto return_result;
	}
	if(is_digit(c, NUMBER_INTEGER)) {
		token = lexer_scan_number(lexer); goto return_result;
	}
	if(is_string(c)) {
		token = lexer_scan_string(lexer); goto return_result;
	}
	if(is_builtin_operator(c)) {
		token = lexer_scan_operator(lexer); goto return_result;
	}
	if(is_special(c)) {
		token = lexer_scan_special(lexer); goto return_result;
	}
	if(is_preprocessor(c)) {
		token = lexer_scan_preprocessor(lexer); goto return_result;
	}
...
gtoken_s FASTCALL gravity_lexer_token(const gravity_lexer_t * lexer) { return lexer->token; }
...
gtoken_t FASTCALL gravity_lexer_token_type(const gravity_lexer_t * lexer) { return lexer->token.type; }
...
uint32 FASTCALL gravity_lexer_lineno(const gravity_lexer_t * lexer) { return lexer->lineno; }
...
void gravity_lexer_token_dump(gtoken_s token) 
{
	...
	{
		static uint32 offset = UINT32_MAX;
		if(lexer->peeking) 
			return;
		gtoken_s token = lexer->token;
		if((token.lineno == 0) && (token.colno == 0)) 
			return;
