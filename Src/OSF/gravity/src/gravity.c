...
struct unittest_data {
	unittest_data() : processed(false), is_fuzzy(false), ncount(0), nsuccess(0), nfailure(0), expected_error(GRAVITY_ERROR_NONE),
		expected_row(0), expected_col(0)
	{
	}
	bool processed;
	bool is_fuzzy;
...
static void report_error(gravity_vm * vm, GravityErrorType error_type, const char * message, const GravityErrorDescription * pErrDesc, void * xdata) 
{
	(void)vm, (void)xdata;
	const char * type = "N/A";
	switch(error_type) {
	...
	if(error_type == GRAVITY_ERROR_RUNTIME) 
		...
		printf("%s ERROR on %d (%d,%d): ", type, pErrDesc->fileid, pErrDesc->lineno, pErrDesc->colno);
	printf("%s\n", message);
}

static const char * load_file(const char * file, size_t * size, uint32 * fileid, void * xdata, bool * is_static) 
{
...
static void unittest_error(gravity_vm * vm, GravityErrorType error_type, const char * message, const GravityErrorDescription * pErrDesc, void * xdata) 
{
	(void)vm;
	unittest_data * data = (unittest_data*)xdata;
	...
	bool same_row = (data->expected_row != -1) ? (data->expected_row == pErrDesc->lineno) : true;
	bool same_col = (data->expected_col != -1) ? (data->expected_col == pErrDesc->colno) : true;
	if(data->is_fuzzy) {
		++data->nsuccess;
		printf("\tSUCCESS\n");
		return;
...
static void unittest_scan(const char * folder_path, unittest_data * data) 
{
	...
	while((target_file = directory_read(dir, outbuffer))) {
		...
		assert(source_code);
		unittest_init(target_file, data); // start unit test
		...
		gravity_delegate_t delegate(data);
		delegate.error_callback = unittest_error;
		delegate.unittest_callback = unittest_callback;
		delegate.loadfile_callback = unittest_read;
...
int main(int argc, const char * argv[]) 
{
	...
	// setup compiler/VM delegate
	gravity_delegate_t delegate(0);
	delegate.error_callback = report_error;
	delegate.loadfile_callback = load_file;
	// create VM
	gravity_vm * vm = gravity_vm_new(&delegate);
	...
	if((type == OP_COMPILE) || (type == OP_COMPILE_RUN) || (type == OP_INLINE_RUN)) {
		...
		if(type == OP_INLINE_RUN) {
			char * buffer = static_cast<char *>(mem_alloc(NULL, size+1024));
			assert(buffer);
