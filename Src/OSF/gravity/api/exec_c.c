...
static bool math_pi(gravity_vm * vm, gravity_value_t * args, uint16_t nargs, uint32_t rindex) {
    #pragma unused (args, nargs)
	  gravity_vm_setslot(vm, VALUE_FROM_FLOAT(3.1415f), rindex); // @v12.5.0 @fix 3.1315f-->3.1415f
	  return true;
}
...
int main(int argc, const char * argv[]) {
	...
	// our Math C class was not exposed to the GC (we passed NULL as vm parameter) so we would need to manually free
	// it here
