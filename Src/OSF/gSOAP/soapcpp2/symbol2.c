...
Tnode * mktype(Type type, void * ref, int width)
{
	...
	if(type != Tstruct && type != Tclass && type != Tunion && (type != Tenum || ref)) {
		for(p = Tptr[type]; p; p =
		            p->next)
		{
			if(p->ref == ref && p->sym == (Symbol *)0 && p->width == width && p->transient ==
			   t)
			{
...
void compile(Table * table)
{
	...
	for(p = classtable->list; p; p = p->next) {
		if(p->info.typ->type == Tclass && p->info.typ->transient <=
		   0)                                            {
			classflag = 1;
			break;
		}
	...
	for(filenum = 1; partnum == 0; filenum++) {
		...
		else {fprintf(fmsg, "Saving %s XML serializers\n", pathsoapC);
		      partnum = 1; }
		...
		if(filenum == 1) {
			...
			if(!cflag) {
					...
					for(p = classtable->list; p; p = p->next)
						if((p->info.typ->type == Tclass || p->info.typ->type == Tstruct) && !is_transient(p->info.typ)) {
							if(is_header_or_fault(p->info.typ) || is_body(p->info.typ))
								fprintf(fout, "\n#ifndef WITH_NOGLOBAL");
							fprintf(fout,
								"\n\tcase %s:\n\t\treturn (void*)soap_instantiate_%s(soap, -1, type, arrayType, n);",
								soap_type(p->info.typ), c_ident(p->info.typ));
							if(is_header_or_fault(p->info.typ) || is_body(p->info.typ))
								fprintf(fout, "\n#endif");
						}
				if(typetable)
					for(p = typetable->list; p; p = p->next)
						if((p->info.typ->type == Tclass ||
						    p->info.typ->type == Tstruct) && !is_transient(p->info.typ)) {
							if(is_header_or_fault(p->info.typ) || is_body(p->info.typ))
								fprintf(fout, "\n#ifndef WITH_NOGLOBAL");
							fprintf(fout,
								"\n\tcase %s:\n\t\treturn (void*)soap_instantiate_%s(soap, -1, type, arrayType, n);",
				...
				if(Tptr[Ttemplate]) {
					...
					fprintf(fhead,
						"\n\nSOAP_FMAC3 void SOAP_FMAC4 %s_container_insert(struct soap*, int, int, void*, size_t, const void*, size_t);",
						prefix);
					if(!lflag) {
						fprintf(fout,
							"\n\nSOAP_FMAC3 void SOAP_FMAC4 %s_container_insert(struct soap *soap, int st, int tt, void *p, size_t len, const void *q, size_t n)",
							prefix);
						fprintf(fout,
	...
	if(is_volatile(typ))
		...
		if(has_ns(typ) || is_untyped(typ))
			fprintf(fd, "/* Sequence of %s schema type: */\n", x);
			      ...
			      sprintf(
				      errbuf,
				      "array '%s' is not compliant with WS-I Basic Profile 1.0a, reason: SOAP encoded array",
				      c_type(typ));
	...
	if(typ->type == Tstruct) {
			...
			for(Eptr = ((Table *)typ->ref)->list; Eptr; Eptr = Eptr->next) {
				...
				if(Eptr->info.sto&Sattribute) {
					if(Eptr->info.minOccurs >= 1)
						fprintf(fd, "\t/* required attribute of type %s */",
							wsdl_type(Eptr->info.typ, ""));
					else
						fprintf(fd, "\t/* optional attribute of type %s */",
							wsdl_type(Eptr->info.typ, ""));
				}
				...
				else if(Eptr->info.typ->type != Tfun &&
				        !(Eptr->info.sto&(Sconst|Sprivate|Sprotected)) &&
				        !(Eptr->info.sto&Sattribute) &&
				        !is_transient(Eptr->info.typ) && !is_external(Eptr->info.typ) &&
		...
		if(typ->ref) {
			...
			for(Eptr = ((Table *)typ->ref)->list; Eptr; Eptr = Eptr->next) {
				...
				if(!is_dynamic_array(typ) && !is_primclass(typ)) {
					if(!strncmp(Eptr->sym->name, "__size",
						   6))                                                   {
						if(!Eptr->next || Eptr->next->info.typ->type !=
						   Tpointer)
						{
							sprintf(
								errbuf,
								"Member field '%s' is not followed by a pointer member field in struct '%s'",
								Eptr->sym->name, typ->id->name);
							semwarn(errbuf);
						}
					}
						...
						if(!Eptr->next ||
						   ((Eptr->next->info.typ->type != Tpointer ||
						     ((Tnode *)Eptr->next->info.typ->ref)->type !=
						     Tvoid)))                                                  {
							sprintf(
								errbuf,
								"Member field '%s' is not followed by a void pointer or union member field in struct '%s'",
								Eptr->sym->name, typ->id->name);
							semwarn(errbuf);
						}
					}
			...
			if(!is_transient(typ) && !is_volatile(typ)) {
				...
				if(!has_constructor(typ)) { /* Table *t;
					                       Entry *p;
					                       int c = ':'; */
					fprintf(fd, "\n\t         %s()", ident(typ->id->name));
						...
						for(p = t->list; p; p = p->next)        {
							if(!(p->info.sto&
								...
								if(p->info.typ->type == Tpointer) {
									...
									fprintf(fd, "%c %s((%s)0)", c,
										ident(p->sym->name), c_type(p->info.typ));
									c = ',';
								}
								else if(p->info.typ->type >= Tchar &&
								        p->info.typ->type < Tenum) {
									fprintf(fd, "%c %s(0)", c, ident(p->sym->name));
									c = ',';
								}
 #endif
		...
		for(p = typetable->list; p; p = p->next) {
			if(!is_primitive_or_string(p->info.typ) && !is_external(p->info.typ) && !is_XML(p->info.typ) &&
			   ...
			   !is_template(p->info.typ))                                           {
				sprintf(
					errbuf,
					"typedef '%s' is not namespace qualified: schema definition for '%s' in WSDL file output may be invalid",
					p->sym->name, p->sym->name);
				semwarn(errbuf);
			}
			if(p->info.typ->type == Tclass &&
			   is_eq(p->info.typ->sym->name,
				   "xsd__QName") && !is_external(p->info.typ) && !is_imported(p->info.typ))
				continue;
			...
			for(i = 1; i < n; i++) {
				if(s[i] == ':'                         ||
				   ...
				   (s[i-1] != '_' && (!strncmp(s+i, "___DOT", 6) ||
				                      ...
				                      (!strncmp(s+i, "___x",
							       4) && isxdigit(s[i+4]) && isxdigit(s[i+5]) &&
				                       isxdigit(s[i+6]) && isxdigit(s[i+7]))))) {
					if(s[i+1] ==
					   ':')
					{
						i++;
					...
					if(!strcmp(buf, "SOAP_ENV") || !strcmp(buf, "SOAP_ENC") || !strcmp(buf, "xsd") ||
					   !strcmp(buf, "xsi") || !strcmp(buf, "xml") || !strcmp(buf, "std") || !strncmp(buf, "soap_", 5))
						goto nsnext;
					...
					if(!cflag) {
						if(iflag || jflag)             {
							...
							if(!Sflag) {
								...
								gen_proxy_header(fd, t, ns, name1, URL, executable, URI,
									encoding);
								fclose(fd);
								...
								copyrightnote(fd, buf);
								gen_proxy_code(fd, t, ns, name1, URL, executable, URI,
									encoding);
								fclose(fd);
							}
							if(!Cflag) {
								...
								sprintf(buf, "%s%s.h", prefix, name1);
								copyrightnote(fd, buf);
								gen_object_header(fd, t, ns, name1, URL, executable,
									URI,
									encoding);
								fclose(fd);
								sprintf(buf, "%s%s%s.cpp", dirpath, prefix, name1);
								...
								sprintf(buf, "%s%s.cpp", prefix, name1);
								copyrightnote(fd, buf);
								gen_object_code(fd, t, ns, name1, URL, executable, URI,
									encoding);
								fclose(fd);
							}
						}
						else {if(!Sflag && sp && sp->name)      {
							      sprintf(buf, "%s%s%s.h", dirpath, prefix,
								      ns_cname(name, "Proxy"));
							      ...
							      copyrightnote(fd, buf);
							      gen_proxy(fd, t, ns, "Service", URL, executable, URI,
								      encoding);
							      fclose(fd);
						      }
						      if(!Cflag && sp && sp->name) {
							      sprintf(buf, "%s%s%s.h", dirpath, prefix,
								      ns_cname(name, "Object"));
							      ...
							      copyrightnote(fd, buf);
							      gen_object(fd, t, ns, name, URL, executable, URI,
								      encoding);
							      fclose(fd);
						      }
						      else if(!Cflag) {
							      ...
							      copyrightnote(fd, buf);
							      gen_object(fd, t, ns, "Service", URL, executable, URI,
								      encoding);
							      fclose(fd);
						      }
						}
						   ...
						   !(method->info.sto&
							...
							if(iflag || jflag)
								fprintf(
									...
									method->sym->name, name1,
									ns_cname(method->sym->name, NULL));
							else
								fprintf(
									fd,
									"\n\n/** Auto-test server operation %s */\nint %s(struct soap *soap",
									method->sym->name, ident(method->sym->name));
							...
							/* params to wrapped params echo */
								 ...
								 (((Tnode *)p->info.typ->ref)->type == Tstruct ||
								  ((Tnode *)p->info.typ->ref)->type == Tclass)) {
								      const char * s, * a;
								      int d = 1;
								      s = ident(p->sym->name);
								      if(p->info.typ->type == Treference)
									      ...
									      a = "->";
								      for(p =
								                  ((Table *)((Tnode *)p->info.typ->ref)
								                   ->ref)->
								                  list, q = r->list;
								          p && q; p = p->next, q = q->next) {
									      if(p->info.typ == q->info.typ)
										      fprintf(fd, "\n\t%s%s%s = %s;", s,
											      a, ident(
												      p->sym->name),
											      ident(q->sym->name));
									      else if(q->info.typ->type == Tpointer &&
									              p->info.typ ==
									              (Tnode *)q->info.typ->ref)
										      fprintf(fd, "\n\t%s%s%s = *%s;",
											      s, a, ident(
												      p->sym->name),
											      ident(q->sym->name));
...
int chkhdr(char * part)
{
	...
	if(p)
		for(p = ((Table *)p->info.typ->ref)->list; p; p = p->next)
			if(has_ns_eq(NULL,
				   p->sym->name) && (!strcmp(part, p->sym->name) || is_eq_nons(part, p->sym->name)))
				return 1;
	sprintf(errbuf,
		"Cannot define method-header-part in WSDL: SOAP_ENV__Header \"%s\" member field is not qualified",
		part);
	semwarn(errbuf);
	return 0;
}
	...
	char * encoding)
{
	...
	for(s = nslist; s; s = s->next) {
		...
		else if(!strcmp(s->name, S_SoapEnvNs))
			fprintf(fd, "\n xmlns:%s=\"%s\"", S_SoapEnvNs, envURI);
		else if(!strcmp(s->name, S_SoapEncNs))
			...
			if(r)
				for(q = r->list; q; q = q->next)
					if(!is_transient(q->info.typ) && !is_repetition(q) && !is_anytype(q) &&
					   !(q->info.sto&Sattribute) && q->info.typ->type != Tfun &&
					   has_ns_eq(NULL, q->sym->name)) {
						fprintf(fd, " <part name=\"%s\" element=\"%s\"", ns_remove(
								q->sym->name), ns_add(q->sym->name, ns));
						if(gen_member_documentation(fd, p->sym, q, ns))
							fprintf(fd, " </part>\n");
						...
						if(p) {
							...
							if(custom_fault) {
								...
								r =
										...
										lookup("SOAP_ENV__Detail"))->info.typ->
								        ref;
								if(r)
									for(q = r->list; q; q = q->next)
										if(!is_transient(q->info.typ) &&
	...
	char * encoding)
{
	...
	Method * m;
	int flag;
	if(!strcmp(ns, S_SoapEnvNs) || !strcmp(ns, S_SoapEncNs) || !strcmp(ns, "xsi") || !strcmp(ns, "xsd"))
		return;
	for(sp = services; sp; sp = sp->next)
	...
	if(sp && (sp->elementForm || sp->attributeForm))
		fprintf(fd, "  elementFormDefault=\"%s\"\n  attributeFormDefault=\"%s\">\n",
			sp->elementForm ? sp->elementForm : "unqualified",
			sp->attributeForm ? sp->attributeForm : "unqualified");
			...
			if(p->info.typ->type != Ttemplate && !is_transient(p->info.typ) &&
			   ...
			   ((has_ns_eq(ns, p->sym->name))))                                                          { /* typedefs that are used for SOAP Fault details */
				...
				if(is_primitive_or_string(p->info.typ) ||
				   (p->info.typ->type == Tpointer &&
				    is_primitive_or_string((Tnode *)p->info.typ->ref))) {
					...
					if(is_primitive(p->info.typ) ||
					   (p->info.typ->type == Tpointer && is_primitive((Tnode *)p->info.typ->ref) &&
...
void gen_nsmap(FILE * fd, Symbol * ns, char * URI)
{
	...
	for(ns1 = nslist; ns1; ns1 = ns1->next) {
		...
		if(sp) {
		...
		else if(!strcmp(ns1->name, S_SoapEnvNs))
			if(is_soap12(NULL))
			...
			fprintf(fd, "\t{\"%s\", \"%s/%s.xsd\", NULL, NULL},\n", ns_convert(
					ns1->name), tmpURI, ns_convert(ns1->name));
	}
	/* fprintf(fd, "\t{\"%s\", \"%s\"},\n", ns_convert(ns->name), URI); */
	fprintf(fd, "\t{NULL, NULL, NULL, NULL}\n};\n");
}

void gen_proxy(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI,
	char * encoding)
{
	Entry * p, * q, * r;
	Table * t, * output;
	...
	if(nflag)
		fprintf(fd,
			"\t%s() { soap = soap_new(); if(soap) soap->namespaces = %s_namespaces; endpoint = \"%s\"; };\n", name1, prefix, URL);
	else {
		fprintf(fd,
...
void gen_object(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI,
	char * encoding)
{
	char * name1;
	Entry * method;
	name1 = ns_cname(name, NULL);
	fprintf(fd, "\n\n#ifndef %s%sObject_H\n#define %s%sObject_H\n#include \"%sH.h\"", prefix, name1, prefix, name1,
		prefix);
...
void gen_proxy_header(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI,
	char * encoding)
{
	Entry * p, * method;
	Table * t;
...
void gen_proxy_code(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI,
	char * encoding)
{
...
void gen_object_header(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI,
	char * encoding)
{
	Entry * p, * method;
	Table * t;
...
void gen_method(FILE * fd, Table * table, Entry * method, int server)
{
	...
	if(server || !is_transient(result->info.typ)) {
		...
		if(!server) {
			...
			fprintf(fd, "; }");
			fprintf(fd, "\n\tvirtual\tint %s(const char *endpoint, const char *soap_action",
				ns_cname(method->sym->name, NULL));
		...
		if(is_transient(result->info.typ))
	...
	else {fprintf(fd, "\n\n\t/// Web service one-way send operation 'send_%s' (returns error code or SOAP_OK)",
		      ns_remove(
			      method->sym->name));
	      ...
	      gen_args(fd, params, result, 1);
	      fprintf(fd, "; }");
	      fprintf(fd, "\n\tvirtual\tint send_%s(const char *endpoint, const char *soap_action",
		      ns_cname(method->sym->name, NULL));
	      gen_params(fd, params, result, 1);
	      fprintf(fd, ";\n\t/// Web service one-way receive operation 'recv_%s' (returns error code or SOAP_OK)",
		      ns_remove(
			      method->sym->name));
	      ...
	      fprintf(
		      fd,
		      "\n\t/// Web service receive of HTTP Accept acknowledgment for one-way send operation 'send_%s' (returns error code or SOAP_OK)",
		      ns_remove(method->sym->name));
	      fprintf(fd, "\n\tvirtual\tint recv_%s_empty_response() { return soap_recv_empty_response(%s); }",
		      ns_cname(method->sym->name,
			      NULL), soap);
	      fprintf(
		      fd,
		      "\n\t/// Web service one-way synchronous send operation '%s' with HTTP Accept/OK response receive (returns error code or SOAP_OK)",
		      ns_remove(method->sym->name));
	      fprintf(fd, "\n\tvirtual\tint %s(", ns_cname(method->sym->name, NULL));
	      gen_params(fd, params, result, 0);
	      fprintf(fd, " { return %s(NULL, NULL", ns_cname(method->sym->name, NULL));
...
void gen_params(FILE * fd, Table * params, Entry * result, int flag)
{
	...
	for(param = params->list; param; param = param->next)
		fprintf(fd, "%s%s%s", flag || param != params->list ? ", " : "", c_storage(param->info.sto),
			c_type_id(param->info.typ, param->sym->name));
	if(!result || is_transient(result->info.typ))
		fprintf(fd, ")");
	else
		fprintf(fd, "%s%s%s)", flag || params->list ? ", " : "", c_storage(result->info.sto),
			c_type_id(result->info.typ, result->sym->name));
}
...
void gen_call_method(FILE * fd, Table * table, Entry * method, char * name)
{
	...
	fprintf(fd, "\n\t\t || soap_put_%s(soap, &soap_tmp_%s, \"%s\", NULL)", ident(method->sym->name),
		ident(method->sym->name), ns_convert(method->sym->name));
	...
	fprintf(fd, "\n\t\treturn soap_closesock(soap);");
	if(is_transient(result->info.typ)) {
		fprintf(fd, "\n\tsoap_get_%s(soap, %s, \"%s\", NULL);", ident(method->sym->name),
			ident(result->sym->name), ns_convert(method->sym->name));
		fprintf(fd,
	...
	if(response) {
		...
		fprintf(fd, "\n\t%s->soap_get(soap, \"%s\", \"%s\");", ident(result->sym->name), xtag, xtyp);
	        ...
	        !is_external((Tnode *)result->info.typ->ref) && !is_volatile((Tnode *)result->info.typ->ref) &&
	        !is_dynamic_array((Tnode *)result->info.typ->ref)) {
		fprintf(fd, "\n\tsoap_get_%s(soap, &%s, \"%s\", \"%s\");", c_ident(
				(Tnode *)result->info.typ->ref), ident(result->sym->name), xtag, xtyp);
	}
	else if(result->info.typ->type == Tpointer && ((Tnode *)result->info.typ->ref)->type == Tstruct &&
	        !is_dynamic_array((Tnode *)result->info.typ->ref)) {
		fprintf(fd, "\n\tsoap_get_%s(soap, %s, \"%s\", \"%s\");", c_ident(
				(Tnode *)result->info.typ->ref), ident(result->sym->name), xtag, xtyp);
	}
	else if(result->info.typ->type == Tpointer && is_XML((Tnode *)result->info.typ->ref) &&
	        is_string((Tnode *)result->info.typ->ref)) {
		fprintf(fd, "\n\tsoap_inliteral(soap, NULL, %s);", ident(result->sym->name));
	}
	else if(result->info.typ->type == Treference && is_XML((Tnode *)result->info.typ->ref) &&
	        is_string((Tnode *)result->info.typ->ref)) {
		fprintf(fd, "\n\tsoap_inliteral(soap, NULL, &%s);", ident(result->sym->name));
	}
	else if(result->info.typ->type == Tpointer && is_XML((Tnode *)result->info.typ->ref) &&
	        is_wstring((Tnode *)result->info.typ->ref)) {
		...
		fprintf(fd, "\n\tsoap_get_%s(soap, &%s, \"%s\", \"%s\");", c_ident(result->info.typ),
			ident(result->sym->name), xtag, xtyp);
	}
	else {fprintf(fd, "\n\tsoap_get_%s(soap, %s, \"%s\", \"%s\");", c_ident(result->info.typ),
		      ident(result->sym->name), xtag, xtyp); }
	fflush(fd);
	...
	if(response) {
		if(result->info.typ->type == Tarray)
			...
			fprintf(fd, "\n\t%s = soap_tmp_%s->%s;", ident(result->sym->name), c_ident(
					response->info.typ), ident(result->sym->name));
...
void gen_serve_method(FILE * fd, Table * table, Entry * param, char * name)
{
	...
	if(!is_transient(pout->info.typ)) {
		fprintf(fd, "\n\tsoap_serializeheader(soap);");
		if(pout->info.typ->type == Tarray && response)
			fprintf(fd, "\n\tsoap_serialize_%s(soap, &soap_tmp_%s);", c_ident(response->info.typ),
				c_ident(response->info.typ));
		else if(response)
			fprintf(fd, "\n\tsoap_serialize_%s(soap, &soap_tmp_%s);", c_ident(response->info.typ),
				c_ident(response->info.typ));
		...
		for(p = t->list; p; p = p->next)
			if(p->info.typ->type == Tfun && !(p->info.sto&Sextern) && has_ns_eq(ns, p->sym->name)) {
				...
				if(!method_response_encoding)
					method_response_encoding = method_encoding;
				/* request */
				fd = gen_env(buf, ns_remove(
						p->sym->name), 0, t, ns, name, URL, executable, URI, method_encoding);
				if(!fd)
					return;
				q = entry(classtable, p->sym);
				if(yflag) {
				...
				if(q && !is_transient(q->info.typ)) {
					fd = gen_env(buf, ns_remove(
							p->sym->name), 1, t, ns, name, URL, executable, URI,
					...
					if(q && !is_response(q->info.typ))
						...
						else {gen_response_begin(fd, 2, ns_convert(p->sym->name));
...
void gen_field(FILE * fd, int n, Entry * p, char * nse, char * nsa, char * encoding)
{
	...
	if(!(p->info.sto&(Sattribute|Sconst|Sprivate|Sprotected)) && !is_transient(p->info.typ) && p->info.typ->type !=
	   Tfun && strncmp(p->sym->name, "__size", 6) && strncmp(p->sym->name, "__type", 6) && !is_choice(p)) {
		...
		if(is_XML(p->info.typ)) {
			...
			gen_element_end(fd, n, ns_add(p->sym->name, nse));
		...
		else {if(!is_string(p->info.typ) && n >= 10 && p->info.minOccurs <= 0)      { /* Do not generate nil, since some tools don't accept it:
				                                                                 if(!is_invisible(p->sym->name))
				                                                                 { gen_element_begin(fd, n, ns_add(p->sym->name, nse), NULL);
			      ...
			      fprintf(
				      fd,
				      "%*s<!-- WARNING max depth exceeded: schema appears to incorrectly define infinitely large documents in recursion over mandatory elements with minOccurs>0 -->\n",
				      n, "");
			      return;
		      }
			      ...
			      else {d = get_Darraydims(p->info.typ);
				    if(d) {
					    for(i = 0; i < d-1; i++) {
						    tmp[2*i] = ',';
						    tmp[2*i+1] = '1';
			      ...
			      gen_val(fd, n, p->info.typ, nse, nsa, encoding);
			       ...
			       p->info.typ->type == Treference) && is_dynamic_array((Tnode *)p->info.typ->ref) &&
			      !is_binary((Tnode *)p->info.typ->ref)) {
			      if(!eflag &&
			         (has_ns((Tnode *)p->info.typ->ref) ||
			          is_untyped((Tnode *)p->info.typ->ref)))
			      {
				      if(yflag)
					      fprintf(fd, "%*s<!-- %s -->\n", n, "",
						      c_type_id(p->info.typ, p->sym->name));
				      gen_element_begin(fd, n,
					      ns_add(p->sym->name, nse), xsi_type((Tnode *)p->info.typ->ref));
				      gen_atts(fd, n, (Table *)((Tnode *)p->info.typ->ref)->ref, nsa);
			      }
			      else {d = get_Darraydims((Tnode *)p->info.typ->ref);
				    if(d) {
					    for(i = 0; i < d-1; i++)        {
						    tmp[2*i] = ',';
						    tmp[2*i+1] = '1';
						...
						if(!is_invisible(p->sym->name)) {
							if(yflag)
								fprintf(fd, "%*s<!-- %s -->\n", n, "",
									c_type_id(p->info.typ, p->sym->name));
							gen_element_begin(fd, n,
								ns_add(p->sym->name, nse), xsi_type_u(p->info.typ));
							if(p->info.typ->type == Ttemplate) {
								if(((Tnode *)p->info.typ->ref)->type ==
								   Tpointer                                     &&
								   (((Tnode *)((Tnode *)p->info.typ->ref)->ref)->type
								    == Tclass ||
								    ((Tnode *)((Tnode *)p->info.typ->ref)->ref)->type
								    == Tstruct))
									gen_atts(
										fd, n,
										(Table *)((Tnode *)((Tnode *)p->info.
										                    typ
										                    ->ref)->ref)->ref,
										nsa);
								else if(((Tnode *)p->info.typ->ref)->type == Tclass ||
				...
				if(is_eq(q->sym->name, "id"))
					...
					switch(p->type) {
						...
						if(q->info.hasval && p->ref) {
							...
							fprintf(fd, "%s",
								ns_remove2((((Table *)p->ref)->list)->sym->name));
						else
							fprintf(fd, "0");
						break;
					    case Tpointer:
						...
						if(is_string(p)) {
							...
							else if(q->info.typ->minLength > 0 && q->info.typ->minLength <
							        10000)
	...
	char * encoding)
{
	...
	else {sprintf(strrchr(tmp, '.'), "_%s_res.xml", method);
...
{
	if(p->sym && p->type == Tclass && is_volatile(p) &&
	   (is_eq(p->sym->name, "xsd__QName") || is_eq(p->sym->name, "QName")))
		return 1;
	return p->id && p->type == Tclass && is_volatile(p) &&
	       (is_eq(p->id->name, "xsd__QName") || is_eq(p->id->name, "QName"));
}
...
int is_XML(Tnode * p)
{
	return (p->sym &&
	        (is_string(p) ||
	         is_wstring(p)) &&
	        is_eq(p->sym->name,
			"XML")) || ((p->type == Tpointer || p->type == Treference) && is_XML((Tnode *)p->ref));
}
...
int is_response(Tnode * p)
{
	return (p->type == Tpointer || p->type == Treference) &&
	       p->ref &&
	       has_ns((Tnode *)p->ref) &&
	       ((((Tnode *)p->ref)->type == Tstruct ||
	         ((Tnode *)p->ref)->type == Tclass) && !is_primclass((Tnode *)p->ref) &&
	        !is_dynamic_array((Tnode *)p->ref) &&
	        !is_stdstring((Tnode *)p->ref) && !is_stdwstring((Tnode *)p->ref));
}
...
int is_unmatched(Symbol * sym)
{
	return sym->name[0] == '_' &&
	       ...
	       (strncmp(sym->name, "_x",
			2) || !isxdigit(sym->name[2]) || !isxdigit(sym->name[3]) || !isxdigit(sym->name[4]) ||
	        !isxdigit(sym->name[5]));
}
	...
	if(typ->type == Tstruct || typ->type == Tclass) {
		if(!is_dynamic_array(typ))                                                  {
			...
			while(t) {
				...
				for(p = t->list; p; p = p->next)
					if(!is_item(p))
						if(p->info.typ->type != Tfun && !is_transient(p->info.typ) &&
						   p->info.sto != Sattribute && p->info.sto != Sprivate &&
						   p->info.sto !=
						   Sprotected)
							return 0;
				t = t->prev;
...
{
	if(p)
		return p->next && p->next->info.typ->type == Tpointer &&
		       (p->info.typ->type == Tint ||
		        p->info.typ->type == Tuint) && ((p->info.sto&Sspecial) || !strncmp(p->sym->name, "__size", 6));
	return 0;
}
...
{
	if(p)
		if(p->next && p->next->info.typ->type == Tunion && p->info.typ->type == Tint &&
		   ((p->info.sto&Sspecial) || !strncmp(p->sym->name, "__union", 7)))
			return 1;
	return 0;
}
...
int has_ns_t(Tnode * typ)
{
	...
	if(typ->sym) {
		...
		return s && s[1] && s[2] && (s[2] != '_' ||
		                             (s[2] == '_' && s[3] == 'x' && isxdigit(s[4]) && isxdigit(s[5]) &&
...
char * soap_type(Tnode * typ)
{
	...
	else {s = (char *)emalloc(strlen(t)+11);
	      strcpy(s, "SOAP_TYPE_"); }
	strcat(s, t);
	return s;
}
...
char * t_ident(Tnode * typ)
{
	...
	switch(typ->type) {
	    ...
	    case Ttemplate:
		if(typ->ref) {
			p = (char *)emalloc((11+strlen(res_remove(typ->id->name))+strlen(q = c_ident(
							     (Tnode *)typ->ref)))*sizeof(char));
...
char * ns_convert(char * tag)
{
	...
	if(*tag == '_') {
		if(!strncmp(tag, "__ptr", 5))                  {
			if(tag[5])
		        ...
		        (strncmp(tag, "_x",
				 2) || !isxdigit(tag[2]) || !isxdigit(tag[3]) || !isxdigit(tag[4]) || !isxdigit(tag[5])))
			tag++;  /* skip leading _ */
	}
	...
	for(i = 0; i < n; i++) {
		if(tag[i] == '_')                         {
			if(tag[i+1] == '_' &&
			   !(tag[i+2] == 'x' && isxdigit(tag[i+3]) && isxdigit(tag[i+4]) && isxdigit(tag[i+5]) &&
			     isxdigit(tag[i+6])))
				break;
			...
			else if(!strncmp(tag+i, "_x",
					2) && isxdigit(tag[i+2]) && isxdigit(tag[i+3]) && isxdigit(tag[i+4]) &&
			        isxdigit(tag[i+5])) {
				char d[5];
				strncpy(d, tag+i+2, 4);
				d[4] = '\0';
...
char * c_type_id(Tnode * typ, char * name)
{
	...
	switch(typ->type) {
		...
		else {p = (char *)emalloc((2+strlen(ident(typ->id->name))+strlen(id))*sizeof(char));
		      strcpy(p, ident(typ->id->name)); }
		strcat(p, " ");
		strcat(p, id);
		break;
		...
		if(typ->ref) {
			p =
			        (char *)emalloc((strlen(q = c_type((Tnode *)typ->ref))+strlen(ident(typ->id->name))+
					         strlen(
							 id)+4)*sizeof(char));
			strcpy(p, ident(typ->id->name));
			strcat(p, "<");
...
void def_table(Table * table)
{
	...
	for(i = 0; i < TYPES; i++) {
		for(p = Tptr[i]; p; p = p->next)                             {
			if(!p->generated && !is_transient(p) && p->type != Twchar &&
			   !is_void(p))                                                               {
				p->generated = True;
				generate(p);
				if(fflag)
	...
	for(i = 0; i < TYPES; i++) {
		for(p = Tptr[i]; p; p = p->next)                             {
			...
			if(is_element(p)) {
				...
				if(p->type == Tarray)
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn soap_out_%s(soap, \"%s\", id, (%s)ptr, NULL);",
						soap_type(p), c_ident(p), s, c_type_id((Tnode *)p->ref, "(*)"));
				else if(p->type == Tclass && !is_external(p) && !is_volatile(p) && !is_typedef(p))
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn ((%s)ptr)->soap_out(soap, \"%s\", id, NULL);",
						soap_type(p), c_type_id(p, "*"), s);
				else if(is_string(p))
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn soap_out_string(soap, \"%s\", id, (char*const*)&ptr, NULL);",
						soap_type(p), s);
				else if(is_wstring(p))
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn soap_out_wstring(soap, \"%s\", id, (wchar_t*const*)&ptr, NULL);",
						soap_type(p), s);
				else if(p->type == Tpointer)
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn soap_out_%s(soap, \"%s\", id, (%s)ptr, NULL);",
						soap_type(p), c_ident(p), s, c_type_id(p, "const*"));
				else if(p->type != Tnone && p->type != Ttemplate && p->type != Twchar && !is_void(p) &&
				        p->type != Tfun && p->type != Treference && p->type != Tunion)
					fprintf(fout,
						"\n\tcase %s:\n\t\treturn soap_out_%s(soap, \"%s\", id, (const %s)ptr, NULL);",
						soap_type(p), c_ident(p), s, c_type_id(p, "*"));
			}
			else {s = xsi_type(p);
			      if(!*s)
				      s = wsdl_type(p, "");
			      ...
			      if(p->type == Tarray)
				      ...
				      fprintf(fout,
					      "\n\tcase %s:\n\t\treturn soap_out_string(soap, tag, id, (char*const*)&ptr, \"%s\");",
					      soap_type(p), s);
			      else if(is_wstring(p))
				      fprintf(fout,
					      "\n\tcase %s:\n\t\treturn soap_out_wstring(soap, tag, id, (wchar_t*const*)&ptr, \"%s\");",
					      soap_type(p), s);
			      else if(p->type == Tpointer)
				      fprintf(fout,
					      "\n\tcase %s:\n\t\treturn soap_out_%s(soap, tag, id, (%s)ptr, \"%s\");",
		...
		for(p = Tptr[i]; p; p = p->next)                             {
			...
			if(p->type == Tarray)
				fprintf(fout, "\n\tcase %s:\n\t\tsoap_serialize_%s(soap, (%s)ptr);\n\t\tbreak;",
					soap_type(p), c_ident(p), c_type_id((Tnode *)p->ref, "(*)"));
			else if(p->type == Tclass && !is_external(p) && !is_volatile(p) && !is_typedef(p))
				fprintf(fout, "\n\tcase %s:\n\t\t((%s)ptr)->soap_serialize(soap);\n\t\tbreak;",
					soap_type(p), c_type_id(p, "*"));
			else if(is_string(p))
				fprintf(fout,
				...
				fprintf(fout, "\n\tcase %s:\n\t\tsoap_serialize_%s(soap, (%s)ptr);\n\t\tbreak;",
					soap_type(p), c_ident(p), c_type_id(p, "const*"));
			else if(p->type == Ttemplate && p->ref)
				fprintf(fout, "\n\tcase %s:\n\t\tsoap_serialize_%s(soap, (const %s)ptr);\n\t\tbreak;",
					soap_type(p), c_ident(p), c_type_id(p, "*"));
			else if(!is_primitive(p) && p->type != Tnone && p->type != Ttemplate && !is_void(p) &&
			        p->type != Tfun && p->type != Treference && p->type != Tunion)
				fprintf(fout, "\n\tcase %s:\n\t\tsoap_serialize_%s(soap, (const %s)ptr);\n\t\tbreak;",
					soap_type(p), c_ident(p), c_type_id(p, "*"));
		}
...
void soap_instantiate_class(Tnode * typ)
{
	...
	fprintf(
		fhead,
		"\nSOAP_FMAC3 void SOAP_FMAC4 soap_copy_%s(struct soap*, int, int, void*, size_t, const void*, size_t);",
		c_ident(typ));
	/* fprintf(fhead,"\n#ifdef __cplusplus\n}\n#endif"); */
	/* fprintf(fout,"\n\n#ifdef __cplusplus\nextern \"C\" {\n#endif"); */
	fprintf(
		fout,
		"\n\nSOAP_FMAC3 void SOAP_FMAC4 soap_copy_%s(struct soap *soap, int st, int tt, void *p, size_t len, const void *q, size_t n)",
		c_ident(typ));
...
void soap_serialize(Tnode * typ)
{
	...
	if((p = is_dynamic_array(typ))) {
		if(typ->type == Tclass && !is_volatile(typ))                                  {
			...
			if(is_binary(typ)) {
		      ...
		      fprintf(fhead, "\nSOAP_FMAC3 void SOAP_FMAC4 soap_serialize_%s(struct soap*, %s);", c_ident(
				      typ), c_type_id(typ, "const*"));
		      fprintf(fout, "\n\nSOAP_FMAC3 void SOAP_FMAC4 soap_serialize_%s(struct soap *soap, %s)\n{",
			      c_ident(
				      typ), c_type_id(typ, "const*a"));
		      if(is_binary(typ)) {
			      if(is_attachment(typ))                     {
				      fprintf(
					      fout,
			      ...
			      fprintf(fout, "\n\t\t{");
			      if(has_ptr((Tnode *)p->info.typ->ref))
				      fprintf(fout, "\tsoap_embedded(soap, a->%s + i, %s);", ident(
						      p->sym->name), soap_type((Tnode *)p->info.typ->ref));
			      if(((Tnode *)p->info.typ->ref)->type == Tclass && !is_XML((Tnode *)p->info.typ->ref) &&
			         !is_external((Tnode *)p->info.typ->ref) && !is_volatile((Tnode *)p->info.typ->ref) &&
			         !is_typedef((Tnode *)p->info.typ->ref))
	...
	switch(typ->type) {
				...
				for(p = t->list; p; p = p->next) {
					if(p->info.sto&(Sconst|Sprivate|Sprotected))
						...
						if(!is_XML(p->next->info.typ)) {
							...
							if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
							   ...
							   !is_typedef((Tnode *)p->next->info.typ->ref))
								fprintf(fout,
									"\n\t\t\tthis->%s::%s[i].soap_serialize(soap);",
									ident(
										t->sym->name), ident(p->next->sym->name));
						...
						fprintf(fout,
							...
							c_ident(
								p->next->info.typ), ident(t->sym->name),
							ident(p->sym->name),
							ident(t->sym->name), ident(p->next->sym->name));
						p = p->next;
					}
			...
			for(p = t->list; p; p = p->next)                                 {
				if(p->info.sto&(Sconst|Sprivate|Sprotected))
					...
					fprintf(fout, "\n\tsoap_serialize_%s(soap, a->%s, &a->%s);",
						c_ident(p->next->info.typ), ident(p->sym->name),
						ident(p->next->sym->name));
					p = p->next;
				}
				else if(p->info.typ->type==Tarray) {
					if(has_ptr(p->info.typ))
						fprintf(fout, "\n\tsoap_embedded(soap, a->%s, %s);", ident(
								p->sym->name), soap_type(p->info.typ));
					fprintf(fout, "\n\tsoap_serialize_%s(soap, a->%s);", c_ident(
							p->info.typ), ident(p->sym->name));
				}
				else if(p->info.typ->type == Tclass && !is_external(p->info.typ) &&
				        !is_volatile(p->info.typ) && !is_typedef(p->info.typ)) {
					if(has_ptr(p->info.typ))
						fprintf(fout, "\n\tsoap_embedded(soap, &a->%s, %s);",
							ident(p->sym->name), soap_type(p->info.typ));
	    ...
	    case Tunion:
		if(is_external(typ) && !is_volatile(typ)) {
			fprintf(fhead, "\nSOAP_FMAC1 void SOAP_FMAC2 soap_serialize_%s(struct soap*, int, const %s);",
				c_ident(
					typ), c_type_id(typ, "*"));
			return;
		}
		...
		fprintf(fhead, "\nSOAP_FMAC3 void SOAP_FMAC4 soap_serialize_%s(struct soap*, %s);", c_ident(
				typ), c_type_id(typ, "const"));
		fprintf(fout, "\n\nSOAP_FMAC3 void SOAP_FMAC4 soap_serialize_%s(struct soap *soap, %s)", c_ident(
				typ), c_type_id(typ, "const a"));
...
void soap_default(Tnode * typ)
{
		...
		if(typ->type == Tclass && !is_volatile(typ))        {
			...
			if(d) {
				...
				for(i = 0; i < d; i++) {
					fprintf(fout, "\n\tthis->__size[%d] = 0;", i);
					if(has_offset(typ) &&
					   (((Table *)typ->ref)->list->next->next->info.sto&Sconst) == 0)
						fprintf(fout, "\n\tthis->__offset[%d] = 0;", i);
				}
			}
			else {fprintf(fout, "\n\tthis->__size = 0;\n\tthis->%s = NULL;", ident(p->sym->name));
	...
	if(is_primitive(typ) || is_string(typ)) {
		if(is_external(typ))                                          {
			fprintf(fhead, "\nSOAP_FMAC1 void SOAP_FMAC2 soap_default_%s(struct soap*, %s);", c_ident(
					typ), c_type_id(typ, "*"));
			return;
		}
		fprintf(fhead, "\nSOAP_FMAC3 void SOAP_FMAC4 soap_default_%s(struct soap*, %s);", c_ident(
	...
	switch(typ->type) {
			...
			for(t = table; t; t = t->prev) {
				for(p = t->list; p; p = p->next)                                 {
					...
					if(p->info.sto&Sconst)
						...
						fprintf(fout, "\n\tthis->%s::%s = 0;", ident(t->sym->name),
							ident(p->sym->name));
						p = p->next;
					}
					else if(is_repetition(p) || is_anytype(p)) {
						fprintf(fout, "\n\tthis->%s::%s = 0;\n\tthis->%s::%s = NULL;",
							ident(t->sym->name), ident(p->sym->name), ident(t->sym->name),
							ident(p->next->sym->name));
						p = p->next;
					}
					else {
						if(is_fixedstring(p->info.typ)) {
							...
							if(p->info.typ->type == Tpointer &&
							   is_stdstring((Tnode *)p->info.typ->ref))
								fprintf(
									fout,
									"\n\tstatic std::string soap_tmp_%s(\"%s\");\n\tthis->%s::%s = &soap_tmp_%s;",
									ident(p->sym->name), p->info.val.s, ident(
										t->sym->name), ident(
										p->sym->name), ident(p->sym->name));
							else
								fprintf(fout, "\n\tthis->%s::%s%s;", ident(
										t->sym->name), ident(
										p->sym->name), c_init(p));
							...
							fprintf(fout, "\n\t/* transient %s skipped */",
						        ...
						        (!is_string(p->info.typ) || is_XML(p->info.typ)))
							fprintf(fout, "\n\tthis->%s::%s = NULL;", ident(
									t->sym->name), ident(p->sym->name));
			...
			for(p = t->list; p; p = p->next)                                 {
				...
				if(p->info.sto&Sconst)
					...
					if(is_fixedstring(p->info.typ)) {
						if(p->info.hasval)
							fprintf(fout, "\n\tstrcpy(a->%s, \"%s\");", ident(
									p->sym->name), cstring(p->info.val.s));
						else
							fprintf(fout, "\n\ta->%s[0] = '\\0';", ident(p->sym->name));
					}
					else if(p->info.typ->type==Tarray)
						fprintf(fout, "\n\tsoap_default_%s(soap, a->%s);", c_ident(
								p->info.typ), ident(p->sym->name));
					else if(p->info.typ->type == Tclass && !is_external(p->info.typ) &&
					        !is_volatile(p->info.typ) && !is_typedef(p->info.typ) &&
					        !is_transient(p->info.typ))
						fprintf(fout, "\n\ta->%s.%s::soap_default(soap);", ident(
								p->sym->name), c_ident(p->info.typ));
					else if(p->info.hasval) {
						if(p->info.typ->type == Tpointer &&
						   is_stdstring((Tnode *)p->info.typ->ref))
							fprintf(
								fout,
								"\n\tstatic std::string soap_tmp_%s(\"%s\");\n\ta->%s = &soap_tmp_%s;",
								ident(p->sym->name), p->info.val.s, ident(p->sym->name),
								ident(p->sym->name));
						else
		...
		fprintf(fhead, "\nSOAP_FMAC3 void SOAP_FMAC4 soap_default_%s(struct soap*, %s);", c_ident(typ),
			c_type(typ));
		fprintf(fout, "\n\nSOAP_FMAC3 void SOAP_FMAC4 soap_default_%s(struct soap *soap, %s)\n{", c_ident(
				typ), c_type_id(typ, "a"));
		fprintf(fout, "\n\tint i;");
		...
		if(typ->type == Tclass && !is_volatile(typ))                                  {
			...
			if(is_binary(typ)) {
				...
				if(((Tnode *)p->info.typ->ref)->type == Tclass && !is_external((Tnode *)p->info.typ->ref) &&
				   !is_volatile((Tnode *)p->info.typ->ref) && !is_typedef((Tnode *)p->info.typ->ref))
					...
					fprintf(fout, "\n\t\t\tsoap_traverse_%s(soap, this->%s + i, \"%s\", p, q);",
						c_ident(
							(Tnode *)p->info.typ->ref), ident(p->sym->name),
						ident(p->sym->name));
		      ...
		      if(is_binary(typ)) {
			      ...
			      fprintf(fout, "\n\t\t}\n\t\tif(q) q(soap, (void*)a, %s, s, \"%s\");", soap_type(
					      typ), c_type(typ));
			      fprintf(fout, "\n\t}\n}");
			      fflush(fout);
			      return;
				...
				for(p = t->list; p; p = p->next) {
					if(p->info.sto&(Sconst|Sprivate|Sprotected))
						...
						fprintf(
							...
							c_ident(p->next->info.typ), ident(t->sym->name), ident(
								p->sym->name), ident(t->sym->name), ident(
								p->next->sym->name), p->next->sym->name);
						p = p->next;
					}
					else if(p->info.typ->type==Tarray) {
						...
						fprintf(fout, "\n\tsoap_traverse_%s(soap, this->%s::%s, \"%s\", p, q);",
							c_ident(
								p->info.typ), ident(t->sym->name), ident(
								p->sym->name), p->sym->name);
					}
					else if(p->info.typ->type==Tclass && !is_external(p->info.typ) &&
					        !is_volatile(p->info.typ) && !is_typedef(p->info.typ)) {
						...
						fprintf(fout, "\n\tthis->%s::%s.soap_traverse(soap, \"%s\", p, q);",
							ident(
								t->sym->name), ident(p->sym->name), p->sym->name);
					}
					else if(p->info.typ->type != Tfun && !is_void(p->info.typ)) {
						if(!is_template(p->info.typ))
							if(has_ptr(p->info.typ))
			...
			for(p = t->list; p; p = p->next)                                 {
				if(p->info.sto&(Sconst|Sprivate|Sprotected))
					...
					if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					   ...
					   !is_typedef((Tnode *)p->next->info.typ->ref))
						...
						fprintf(fout,
							"\n\t\t\tsoap_traverse_%s(soap, a->%s + i, \"%s\", p, q);",
							c_ident(
								(Tnode *)p->next->info.typ->ref), ident(
								p->next->sym->name), p->next->sym->name);
					fprintf(fout, "\n\t\t}\n\t}");
					p = p->next;
				}
					...
					fprintf(fout, "\n\tsoap_traverse_%s(soap, a->%s, &a->%s, \"%s\", p, q);",
						c_ident(
							p->next->info.typ), ident(p->sym->name), ident(
							p->next->sym->name), p->next->sym->name);
					p = p->next;
				}
				else if(p->info.typ->type==Tarray) {
...
void soap_put(Tnode * typ)
{
	...
	if(typ->type == Tclass)
		...
		if(!is_external(typ) && namespaceid)
			fprintf(
				...
				c_ident(typ), c_ident(typ), namespaceid, c_ident(typ), namespaceid, c_ident(
					typ), xml_tag(typ));
		else
			fprintf(
				fhead,
	...
	if(is_invisible(typ->id->name))
		...
		if(typ->type == Tclass && !is_external(typ) && !is_volatile(typ) && !is_typedef(typ)) {
			if(d)
				fprintf(fout,
					"soap_embed(soap, (void*)this, (struct soap_array*)&this->%s, %d, tag, %s);",
					ident(
						p->sym->name), d, soap_type(typ));
			else
				fprintf(fout,
					"soap_embed(soap, (void*)this, (struct soap_array*)&this->%s, 1, tag, %s);",
					ident(
						p->sym->name), soap_type(typ));
		}
		else if(d)
			fprintf(fout, "soap_embed(soap, (void*)a, (struct soap_array*)&a->%s, %d, tag, %s);",
				ident(p->sym->name), d, soap_type(typ));
		else
			fprintf(fout, "soap_embed(soap, (void*)a, (struct soap_array*)&a->%s, 1, tag, %s);",
				ident(p->sym->name), soap_type(typ));
	}
	else if(typ->type == Tclass && !is_external(typ) && !is_volatile(typ) && !is_typedef(typ))
	...
	if((typ->type == Tstruct || typ->type == Tclass) && typ->ref) {
		for(t = (Table *)typ->ref; t; t =
			...
			if(p && p->info.typ->type == Tpointer && !strncmp(ident(p->sym->name), "__ptr", 5))
				if(p->next &&
				   (p->next->info.typ->type == Tint || p->next->info.typ->type == Tulong ||
				    (p->next->info.typ->type == Tarray &&
				     (((Tnode *)p->next->info.typ->ref)->type == Tint ||
				  ((Tnode *)p->next->info.typ->ref)->type == Tuint))) &&
				   !strcmp(ident(p->next->sym->name), "__size"))
					return p;
		}
	}
	return 0;
...
Entry * is_discriminant(Tnode * typ)
{
	...
	if((typ->type == Tstruct || typ->type == Tclass) && typ->ref) {
		...
		if(t && !t->prev) {
			p = t->list;
			if(p && p->info.typ->type == Tint &&
			   ((p->info.sto&Sspecial) || !strncmp(ident(p->sym->name), "__union", 7)))
	...
	if((typ->type == Tstruct || typ->type == Tclass) && typ->ref) {
		for(t = (Table *)typ->ref; t; t =
			...
			while(p && p->info.typ->type == Tfun)
				p = p->next;
			if(p && p->info.typ->type == Tpointer && !strncmp(ident(p->sym->name), "__ptr", 5))
				if(p->next && p->next->info.typ->type == Tarray &&
				   (((Tnode *)p->next->info.typ->ref)->type == Tint ||
				    ((Tnode *)p->next->info.typ->ref)->type == Tuint) &&
				   !strcmp(ident(p->next->sym->name), "__size"))
	...
	if(typ->type == Tstruct || typ->type == Tclass) {
		for(t = (Table *)typ->ref; t; t = t->prev)                                                  {
			for(p = t->list; p; p =
			            p->next)
			{
				if((p->info.typ->type == Tint ||
				    (p->info.typ->type == Tarray &&
				     ((Tnode *)p->info.typ->ref)->type == Tint)) &&
				   !strcmp(ident(p->sym->name), "__offset"))
					return 1;
			}
		}
	}
...
int is_boolean(Tnode * typ)
{
	if(typ->type == Tenum) {
		if((Table *)typ->ref == booltable)
			return 1;
		else {size_t n = strlen(ident(typ->id->name));
...
int is_hexBinary(Tnode * typ)
{
	...
	if((typ->type == Tstruct || typ->type == Tclass) && n >= 9 && is_eq(ident(typ->id->name)+n-9, "hexBinary")) {
		            ...
		            t->prev)
		{
			...
			while(p && p->info.typ->type == Tfun)
				p = p->next;
			if(p && p->info.typ->type == Tpointer && ((Tnode *)p->info.typ->ref)->type == Tuchar &&
			   !strcmp(ident(p->sym->name), "__ptr")) {
				p = p->next;
				return p &&
				       (p->info.typ->type == Tint || p->info.typ->type == Tuint) && !strcmp(ident(
						p->sym->name), "__size");
			}
		}
	}
...
int is_header_or_fault(Tnode * typ)
{
	...
	return (typ->type == Tstruct || typ->type == Tclass) && (!strcmp(ident(typ->id->name), "SOAP_ENV__Header") ||
	        !strcmp(ident(typ->id->name), "SOAP_ENV__Fault") || !strcmp(ident(typ->id->name),
			"SOAP_ENV__Code") || !strcmp(ident(typ->id->name), "SOAP_ENV__Detail") || !strcmp(ident(typ->id->name), "SOAP_ENV__Reason"));
}
...
{
	return !strcmp(envURI,
		"http://www.w3.org/2003/05/soap-envelope") ||
	       (enc && !strcmp(enc, "http://www.w3.org/2003/05/soap-encoding"));
}
	...
	if(typ->type == Tclass)
		for(t = (Table *)typ->ref; t; t = t->prev)
			for(p = t->list; p; p = p->next)
				if(p->info.typ->type == Tfun &&
				   !strcmp(p->sym->name,
					   typ->id->name) && ((FNinfo *)p->info.typ->ref)->ret->type == Tnone) {
...
int has_getter(Tnode * typ)
{
	...
	if(typ->type == Tclass)
		for(t = (Table *)typ->ref; t; t = t->prev)
			for(p = t->list; p; p = p->next)
				if(p->info.typ->type == Tfun &&
				   !strcmp(p->sym->name, "get") && ((FNinfo *)p->info.typ->ref)->ret->type == Tint) {
					q = ((FNinfo *)p->info.typ->ref)->args->list;
...
int has_setter(Tnode * typ)
{
	...
	if(typ->type == Tclass)
		for(t = (Table *)typ->ref; t; t = t->prev)
			for(p = t->list; p; p = p->next)
				if(p->info.typ->type == Tfun &&
				   !strcmp(p->sym->name, "set") && ((FNinfo *)p->info.typ->ref)->ret->type == Tint) {
					q = ((FNinfo *)p->info.typ->ref)->args->list;
					if(q && q->info.typ->type == Tpointer && ((Tnode *)q->info.typ->ref)->type ==
					   Tstruct && ((Tnode *)q->info.typ->ref)->id == lookup("soap"))
						return 1;
...
void soap_out(Tnode * typ)
{
	...
	switch(typ->type) {
			...
			fprintf(fout, "\n}");
		      ...
		      for(t = table; t; t = t->prev) {
			      for(p = t->list; p; p = p->next)                                 {
				      ...
				      if((p->info.sto&SmustUnderstand) &&
				         !(p->info.sto&(Sconst|Sprivate|Sprotected)) && !is_transient(p->info.typ) &&
				      ...
				      if(p->info.sto&(Sconst|Sprivate|Sprotected))
					      ...
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					         ...
					         !is_typedef((Tnode *)p->next->info.typ->ref))
						      fprintf(
							      ...
							      ident(p->next->sym->name), ns_add(p->next->sym->name,
								      nse),
							      xsi_type_cond_u((Tnode *)p->next->info.typ->ref,
								      !has_ns_eq(NULL, p->next->sym->name)));
					      ...
					      fprintf(
						      ...
						      ident(p->next->sym->name),
						      ns_add(p->next->sym->name, nse), ident(p->sym->name));
					      p = p->next;
				      }
				      else if(is_choice(p)) {
					      fprintf(
					      ...
					      fprintf(fout, "\n\tsoap_out_%s(soap, \"%s\", -1, a->%s, \"%s\");",
						      c_ident(
							      p->info.typ), ns_add(p->sym->name, nse), ident(
							      p->sym->name),
					      ...
					      fprintf(fout, "\n\tsoap_outliteral(soap, \"%s\", &a->%s, NULL);",
						      ns_add(p->sym->name, nse), ident(p->sym->name));
				      else if(is_XML(p->info.typ) && is_wstring(p->info.typ))
					      fprintf(fout, "\n\tsoap_outwliteral(soap, \"%s\", &a->%s, NULL);",
						      ns_add(p->sym->name, nse), ident(p->sym->name));
				      else if(p->info.typ->type == Tpointer && !is_void(p->info.typ) &&
				              p->info.minOccurs > 0)
					      fprintf(
						      fout,
						      ...
						      xsi_type_cond_u(p->info.typ,
							      !has_ns_eq(NULL, p->sym->name)), ns_add(p->sym->name, nse));
				      else if(p->info.typ->type != Tfun && !is_void(p->info.typ))
					      fprintf(
						      fout,
						      "\n\tif(soap_out_%s(soap, \"%s\", -1, &a->%s, \"%s\"))\n\t\treturn soap->error;",
			...
			if(table->prev) {
				if(is_XML(p->info.typ) && is_string(p->info.typ))
					fprintf(fout, "\n\treturn soap_outliteral(soap, tag, &(a->%s::%s), \"%s\");",
						ident(
							t->sym->name), ident(p->sym->name), xsi_type(typ));
				else if(is_XML(p->info.typ) && is_wstring(p->info.typ))
			 ...
			 them */
		      ...
		      for(; i > 0; i--) {
			      ...
			      for(p = t->list; p != (Entry *)0; p = p->next) {
				      ...
				      if((p->info.sto&SmustUnderstand) &&
				         !(p->info.sto&(Sconst|Sprivate|Sprotected)) && !(p->info.sto&Sattribute) &&
					      ...
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					         ...
					         !is_typedef((Tnode *)p->next->info.typ->ref))
						      ...
						      fprintf(fout,
							      ...
							      c_ident((Tnode *)p->next->info.typ->ref),
							      ns_add_overridden(t, p->next,
								      nse1), ident(t->sym->name), ident(
								      p->next->sym->name),
							      xsi_type_cond_u((Tnode *)p->next->info.typ->ref,
								      !has_ns_eq(NULL, p->next->sym->name)));
					      fprintf(fout, "\n\t}");
		...
		for(p = table->list; p; p = p->next) {
			if(p->info.sto&(Sconst|Sprivate|Sprotected))
				...
				fprintf(fout, "\n\t\treturn a->%s.soap_out(soap, \"%s\", -1, \"%s\");",
					ident(p->sym->name),
					ns_add(p->sym->name,
						nse), xsi_type_cond_u(p->info.typ, !has_ns_eq(NULL, p->sym->name)));
			}
		...
		if(is_template(typ)) {
			...
			fprintf(fout, "\n\treturn soap_out_%s(soap, tag, id, *a, type);", c_ident((Tnode *)typ->ref));
		      ...
		      if(p) {
			      ...
			      fprintf(fout, "\n\tid = soap_element_id(soap, tag, id, *a, NULL, 0, type, %s);",
				      soap_type(
					      (Tnode *)typ->ref));
		      fprintf(fout, "\n\tif(id < 0)\n\t\treturn soap->error;");
		      if(((Tnode *)typ->ref)->type == Tclass && !is_external((Tnode *)typ->ref) &&
		...
		fprintf(fout, "\n\tsoap_array_begin_out(soap, tag, soap_embedded_id(soap, id, a, %s), \"%s[%d]\", 0);",
			soap_type(
				typ), xsi_type_Tarray(typ), get_dimension(typ));
		n = (Tnode *)typ->ref;
		cardinality = 1;
		while(n->type==Tarray) {
			n = (Tnode *)n->ref;
		...
		if(((Tnode *)typ->ref)->type == Tclass && !is_external((Tnode *)typ->ref) &&
		   !is_volatile((Tnode *)typ->ref) && !is_typedef((Tnode *)typ->ref)) {
			if(cardinality>1)
				fprintf(fout, "\n\t\ta[i].soap_out(soap, \"item\", -1, \"%s\")",
					xsi_type_u((Tnode *)typ->ref));
			else fprintf(fout, "\n\t\t(a+i)->soap_out(soap, \"item\", -1, \"%s\")",
					xsi_type_u((Tnode *)typ->ref));
		}
		else {if(((Tnode *)typ->ref)->type != Tarray)      {
			      if(((Tnode *)typ->ref)->type == Tpointer)
				      fprintf(
					      fout,
		...
		fprintf(fhead, "\n\nSOAP_FMAC3S const char* SOAP_FMAC4S soap_%s2s(struct soap*, %s);", c_ident(
				typ), c_type(typ));
		fprintf(fout, "\n\nSOAP_FMAC3S const char* SOAP_FMAC4S soap_%s2s(struct soap *soap, %s)", c_ident(
				typ), c_type_id(typ, "n"));
		if(is_typedef(typ))
		...
		if(n->type==Tarray)
			...
			fprintf(fout, "\n\t\tbool b = (*i);\n\t\tif(soap_out_%s(soap, tag, id, &b, \"%s\"))",
				c_ident(n), xsi_type_u(typ));
		else
			fprintf(fout, "\n\t\tif(soap_out_%s(soap, tag, id, &(*i), \"%s\"))", c_ident(n),
				xsi_type_u(typ));
		fprintf(fout, "\n\t\t\treturn soap->error;");
		fprintf(fout, "\n\t}\n\treturn SOAP_OK;\n}");
		break;
...
void soap_out_Darray(Tnode * typ)
{
		...
		if(is_untyped(p->info.typ))                                                          {
			if(has_offset(typ))
				...
				fprintf(fout,
					...
					ident(
						p->sym->name), wsdl_type(p->info.typ, "xsd"), d);
			else
	...
	if(is_binary(typ) && !is_hexBinary(typ))
		...
		fprintf(fout, "\n\tif(soap_puthex(soap, a->__ptr, a->__size))\n\t\treturn soap->error;");
	      ...
	      if(!has_ns(typ) && !is_untyped(typ)) {
		      if(d)                                       {
			      ...
			      if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
				      fprintf(fout, "\n\t\tsoap_outliteral(soap, \"%s\", &a->%s[i], NULL);", item,
					      ident(
						      p->sym->name));
			      else if(is_XML((Tnode *)p->info.typ->ref) && is_wstring((Tnode *)p->info.typ->ref))
				      fprintf(fout, "\n\t\tsoap_outwliteral(soap, \"%s\", &a->%s[i], NULL);", item,
					      ident(
						      p->sym->name));
			      else if(((Tnode *)p->info.typ->ref)->type == Tclass &&
			              !is_external((Tnode *)p->info.typ->ref) &&
			              !is_volatile((Tnode *)p->info.typ->ref) &&
			              !is_typedef((Tnode *)p->info.typ->ref))
	      ...
	      else {if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
			    fprintf(fout, "\n\t\tsoap_outliteral(soap, \"%s\", &a->%s[i], NULL);", item,
				    ident(p->sym->name));
		    else if(is_XML((Tnode *)p->info.typ->ref) && is_wstring((Tnode *)p->info.typ->ref))
			    fprintf(fout, "\n\t\tsoap_outwliteral(soap, \"%s\", &a->%s[i], NULL);", item,
...
void soap_in(Tnode * typ)
{
	...
	switch(typ->type) {
				      ...
				      for(p = t->list; p; p =
					         ...
					         !is_template(p->info.typ))
					      {
						      ...
						      if(is_repetition(p)) {
							      ...
							      fprintf(fout,
								      "\n\tsize_t soap_flag_%s = " SOAP_LONG_FORMAT ";",
								      ident(
									      p->sym->name), p->info.maxOccurs);
					      }
				      }
			      }
		      }
			...
			/* fprintf(fout,"\n\tif(soap->alloced)"); */
			      ...
			      for(p = t->list; p; p = p->next)                                 {
				      if(p->info.sto&(Sconst|Sprivate|Sprotected))
					      ...
					      if(is_unmatched(p->next->sym))
						      ...
						      fprintf(fout, "!soap_element_begin_in(soap, \"%s\", 1, NULL))",
							      ns_add(p->next->sym->name,
								      nse));
					      fprintf(
						      fout,
					      ...
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass ||
					         has_class((Tnode *)p->next->info.typ->ref))
						      fprintf(fout, "\n\t\t\t\t\tSOAP_PLACEMENT_NEW(a->%s, %s);",
							      ident(p->next->sym->name),
							      c_type((Tnode *)p->next->info.typ->ref));
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					         !is_external((Tnode *)p->next->info.typ->ref) &&
					         !is_volatile((Tnode *)p->next->info.typ->ref) &&
					         !is_typedef((Tnode *)p->next->info.typ->ref))
						      fprintf(fout, "\n\t\t\t\t\ta->%s->soap_default(soap);",
							      ident(p->next->sym->name));
					      else if(((Tnode *)p->next->info.typ->ref)->type != Tpointer  &&
					              !is_XML((Tnode *)p->next->info.typ->ref))
					      ...
					      if(!is_invisible(p->next->sym->name))
						      fprintf(fout, "\n\t\t\t\tsoap_revert(soap);");
					      if(is_unmatched(p->next->sym)) {
						      if(is_XML((Tnode *)p->next->info.typ->ref) &&
						         is_string((Tnode *)p->next->info.typ->ref))
							      fprintf(
								      fout,
								      "\n\t\t\t\tif(soap_inliteral(soap, NULL, a->%s))",
								      ident(p->next->sym->name));
						      else if(is_XML((Tnode *)p->next->info.typ->ref) &&
						              is_wstring((Tnode *)p->next->info.typ->ref))
							      fprintf(
					      ...
					      fprintf(fout,
						      ...
						      ident(
							      p->next->sym->name));
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					         !is_volatile(p->next->info.typ->ref))
						      fprintf(
							      fout,
							      "\n\t\t\t{\t%s;\n\t\t\t\t%s;\n\t\t\t\tq.soap_default(soap);\n\t\t\t\tif(soap_new_block(soap) == NULL)\n\t\t\t\t\treturn NULL;",
							      c_type_id(p->next->info.typ,
								      "p"),
							      c_type_id((Tnode *)p->next->info.typ->ref, "q"));
					      else if(((Tnode *)p->next->info.typ->ref)->type == Tclass ||
					              has_class(p->next->info.typ->ref))
						      fprintf(
							      fout,
							      "\n\t\t\t{\t%s;\n\t\t\t\t%s;\n\t\t\t\tif(soap_new_block(soap) == NULL)\n\t\t\t\t\treturn NULL;",
							      c_type_id(p->next->info.typ,
					      ...
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass ||
					         has_class(p->next->info.typ->ref))
						      fprintf(fout, "\n\t\t\t\t\tmemcpy(p, &q, sizeof(%s));",
							      c_type((Tnode *)p->next->info.typ->ref));
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass &&
					         !is_external(p->next->info.typ->ref) &&
					         !is_volatile(p->next->info.typ->ref) &&
					         !is_typedef(p->next->info.typ->ref))
						      fprintf(fout, "\n\t\t\t\t\tp->soap_default(soap);");
					      else if(((Tnode *)p->next->info.typ->ref)->type != Tpointer  &&
					              !is_XML(p->next->info.typ->ref))
					      ...
					      fprintf(fout, "\n\t\t\t\t}");
					      fprintf(fout,
						      "\n\t\t\t\ta->%s = (%s)soap_save_block(soap, NULL, NULL, 1);",
						      ident(
							      p->next->sym->name), c_type(p->next->info.typ));
					      fprintf(
						      fout,
						      "\n\t\t\t\tif(!soap_flag_%s && soap->error == SOAP_TAG_MISMATCH)\n\t\t\t\t\tcontinue;\n\t\t\t}",
						      ident(p->next->sym->name));
					      ...
					      fprintf(fout,
						      ...
						      ident(
							      p->next->sym->name));
					      fprintf(fout, "\n\t\t\t\tif((a->%s = soap_getelement(soap, &a->%s)))",
						      ident(
							      p->next->sym->name), ident(p->sym->name));
					      fprintf(fout, "\n\t\t\t\t{\tsoap_flag_%s = 0;", ident(p->next->sym->name));
					      fprintf(fout, "\n\t\t\t\t\tcontinue;");
					      fprintf(fout, "\n\t\t\t\t}");
					      p = p->next;
					    ...
					    if(!is_invisible(p->sym->name) && !is_primclass(typ) &&
					       p->info.typ->type != Tfun && !is_void(p->info.typ)) {
						    if(is_string(p->info.typ) || is_wstring(p->info.typ) ||
						       is_stdstr(p->info.typ))
							    fprintf(
								    fout,
								    "\n\t\t\tif(soap_flag_%s && (soap->error == SOAP_TAG_MISMATCH || soap->error == SOAP_NO_TAG))",
								    ident(p->sym->name));
		      ...
		      if(!is_discriminant(typ)) {
			      for(t = table; t; t = t->prev)                            {
				      for(p = t->list; p; p =
				                  p->next)                                                            {
					      if(is_repetition(p) || is_anytype(p) ||
					         is_choice(p))
					      {
						      p = p->next;
						      continue;
			      ...
			      for(p = table->list; p; p = p->next)
				      if(is_repetition(p)) {
					      fprintf(fout,
						      ...
						      ident(
							      p->next->sym->name), ident(p->next->sym->name));
					      fprintf(
						      fout,
						      "\n\t\tif(a->%s)\n\t\t\ta->%s = (%s)soap_save_block(soap, soap_blist_%s, NULL, 1);\n\t\telse\n\t\t{\ta->%s = NULL;\n\t\t\tif(soap_blist_%s)\n\t\t\t\tsoap_end_block(soap, soap_blist_%s);\n\t\t}",
						      ident(p->sym->name), ident(p->next->sym->name), c_type(
							      p->next->info.typ), ident(p->next->sym->name), ident(
							      p->next->sym->name), ident(p->next->sym->name),
						      ident(p->next->sym->name));
					      p = p->next;
				      }
		      ...
		      if(table && !is_discriminant(typ)) {
			      for(p = table->list; p; p = p->next)                                     {
				      if(p->info.minOccurs > 0 && p->info.maxOccurs >= 0 &&
				         !(p->info.sto&(Sconst|Sprivate|Sprotected)) && !(p->info.sto&Sattribute) &&
				         p->info.typ->type != Tfun && !is_void(p->info.typ) &&
				         !is_transient(p->info.typ) && !is_template(p->info.typ) &&
				         !is_repetition(p) &&
				         !is_choice(p) && p->info.hasval ==
				         False)
				      {
					      if(is_item(p))
						      continue;
					      if(is_anytype(p))
						      p = p->next;
					      if(a==0) {
						      fprintf(fout, "\n\tif(%s(soap_flag_%s > " SOAP_LONG_FORMAT "",
							      strict_check(), ident(
								      p->sym->name), p->info.maxOccurs-
							      p->info.minOccurs);
						      a = 1;
						      ...
						      fprintf(fout, " || soap_flag_%s > " SOAP_LONG_FORMAT "",
							      ident(p->sym->name), p->info.maxOccurs-p->info.minOccurs);
				      }
				      else if(is_template(p->info.typ)) {
					      if(p->info.minOccurs > 0)                                    {
						         ...
						         Tpointer)
						      {
							         ...
							         0)
							      {
								      fprintf(
									      fout,
									      "\n\tif(%s(!a->%s || a->%s->size() < " SOAP_LONG_FORMAT
									      "", strict_check(), ident(p->sym->name),
								      ...
								      fprintf(
									      fout,
									      " || !a->%s || a->%s->size() < " SOAP_LONG_FORMAT
									      "", ident(p->sym->name), ident(
										      p->sym->name), p->info.minOccurs);
						      }
						      else {if(a==0)      {
								    fprintf(
									    fout,
					      ...
					      if(p->info.minOccurs != 0)                        {
						      if(a==0)                                                    {
							      fprintf(fout, "\n\tif(%s(soap_flag_%s",
								      strict_check(), ident(p->next->sym->name));
							      a = 1;
						      }
						      else
							      fprintf(fout, " || soap_flag_%s",
								      ident(p->next->sym->name));
					      }
					      p = p->next;
				      }
		...
		if(is_primclass(typ)) {
			...
			for(t = (Table *)typ->ref; t; t = t->prev) {
				...
				for(p = t->list; p; p = p->next)
					if(p->info.sto&Sattribute)
						soap_attr_value(p,
							ptr_cast(t,
								"a"), ident(p->sym->name), ns_add(p->sym->name, nsa1));
			}
			fflush(fout);
			for(table = (Table *)typ->ref; table; table = table->prev) {
				p = table->list;
			...
			if(is_XML(p->info.typ) && is_string(p->info.typ)) {
				fprintf(fout, "\n\tif(!soap_inliteral(soap, tag, &(a->%s::%s)))",
					ident(table->sym->name), ident(p->sym->name));
			}
			else if(is_XML(p->info.typ) && is_wstring(p->info.typ)) {
				fprintf(fout, "\n\tif(!soap_inwliteral(soap, tag, &(a->%s::%s)))",
					ident(table->sym->name), ident(p->sym->name));
			}
			else if(p->info.typ->type==Tarray) {
				fprintf(fout, "\n\tif(!soap_in_%s(soap, tag, a->%s::%s, \"%s\"))", c_ident(
						p->info.typ), ident(table->sym->name), ident(p->sym->name),
					xsi_type(typ));
			}
			else if(p->info.typ->type==Tclass && !is_external(p->info.typ) && !is_volatile(p->info.typ) &&
			        !is_typedef(p->info.typ)) {
				fprintf(fout, "\n\tif(!(a->%s::%s).soap_in(soap, tag, \"%s\"))",
					ident(table->sym->name), ident(p->sym->name), xsi_type(typ));
			}
			else if(p->info.typ->type != Tfun && !is_void(p->info.typ)) {
						...
						if(!(p->info.sto&(Sconst|Sprivate|Sprotected)) &&
						   ...
						   !is_template(p->info.typ))                                 {
							...
							if(is_repetition(p)) {
								...
								fprintf(
									fout,
									"\n\tsize_t soap_flag_%s%d = " SOAP_LONG_FORMAT
									";",
									ident(p->sym->name), i, p->info.maxOccurs);
						}
					}
				}
				if(a)
				...
				for(p = t->list; p; p = p->next) {
					if(is_item(p))
						...
						fprintf(
							...
							ident(t->sym->name), ident(p->next->sym->name), ident(
								p->next->sym->name), i, ident(
								p->next->sym->name), i, ident(
								t->sym->name), ident(p->next->sym->name), c_type(
								p->next->info.typ), ident(p->next->sym->name), i,
							c_type((Tnode *)p->next->info.typ->ref), ident(t->sym->name),
							ident(p->next->sym->name));
						...
						if(is_unmatched(p->next->sym)) {
							if(is_XML((Tnode *)p->next->info.typ->ref) &&
							   is_string((Tnode *)p->next->info.typ->ref))
								fprintf(
									fout,
									"\n\t\t\t\tif(soap_inliteral(soap, NULL, a->%s::%s))",
									ident(t->sym->name), ident(p->next->sym->name));
							else if(is_XML((Tnode *)p->next->info.typ->ref) &&
							        is_wstring((Tnode *)p->next->info.typ->ref))
								fprintf(
									fout,
									"\n\t\t\t\tif(soap_inwliteral(soap, NULL, a->%s::%s))",
									ident(t->sym->name), ident(p->next->sym->name));
							else
								fprintf(
									fout,
							 ...
							 is_string((Tnode *)p->next->info.typ->ref))
							      fprintf(
								      ...
								      ns_add(p->next->sym->name, nse1), ident(
									      t->sym->name), ident(p->next->sym->name));
						      else if(is_XML((Tnode *)p->next->info.typ->ref) &&
							      is_wstring((Tnode *)p->next->info.typ->ref))
							      fprintf(
						...
						if(is_unmatched(p->next->sym))
							fprintf(
								...
								ident(t->sym->name), ident(p->sym->name),
								ns_add_overridden(t, p->next, nse1), ident(t->sym->name));
						else if(is_invisible(p->next->sym->name))
							fprintf(
								fout,
								"\n\t\t\t\tfor(a->%s::%s = 0; !soap_peek_element(soap); a->%s::%s++)",
								ident(t->sym->name), ident(p->sym->name), ident(
									t->sym->name), ident(p->sym->name));
						else
							fprintf(
								fout,
							...
							if(is_XML(p->next->info.typ->ref) &&
							 ...
							 is_string(p->next->info.typ->ref))
							      fprintf(
								      fout,
								      "\n\t\t\t\t\tif(!soap_inliteral(soap, \"%s\", p))",
								      ns_add_overridden(t, p->next, nse1));
						      else if(is_XML(p->next->info.typ->ref) &&
							      is_wstring(p->next->info.typ->ref))
							      fprintf(
								      fout,
							      ...
							      fprintf(
								      ...
								      c_ident(p->next->info.typ->ref),
								      ns_add_overridden(t, p->next,
									      nse1), xsi_type(p->next->info.typ->ref)); }
						fprintf(fout, "\n\t\t\t\t\t\tbreak;");
						fprintf(fout, "\n\t\t\t\t\tsoap_flag_%s%d = 0;",
							ident(p->next->sym->name), i);
						fprintf(fout, "\n\t\t\t\t}");
						fprintf(
							fout,
							"\n\t\t\t\ta->%s::%s = (%s)soap_save_block(soap, NULL, NULL, 1);",
							ident(t->sym->name), ident(p->next->sym->name),
							c_type(p->next->info.typ));
						fprintf(
							fout,
						...
						fprintf(
							...
							"\n\t\t\tif(soap_flag_%s%d && soap->error == SOAP_TAG_MISMATCH)",
							ident(p->next->sym->name), i);
						fprintf(fout, "\n\t\t\t\tif(soap_in_%s(soap, &a->%s::%s, &a->%s::%s))",
							c_ident(
								p->next->info.typ), ident(t->sym->name),
							ident(p->sym->name),
							ident(t->sym->name), ident(p->next->sym->name));
					      ...
					      if(is_unmatched(p->sym)) {
						      if(is_XML(p->info.typ) && is_string(p->info.typ)) {
							      ...
							      fprintf(
								      fout,
								      "\n\t\t\t\tif(soap_in_%s(soap, NULL, a->%s::%s, \"%s\"))",
								      c_ident(p->info.typ), ident(t->sym->name),
						      ...
						      if(is_XML(p->info.typ) && is_string(p->info.typ)) {
							      fprintf(
								      fout,
								      "\n\t\t\t\tif(soap_inliteral(soap, \"%s\", &(a->%s::%s)))",
								      ns_add_overridden(t, p, nse1), ident(
									      t->sym->name), ident(p->sym->name));
						      }
						      else if(is_XML(p->info.typ) && is_wstring(p->info.typ)) {
							      fprintf(
							      ...
							      fprintf(
								      fout,
								      "\n\t\t\t\tif(soap_in_%s(soap, \"%s\", a->%s::%s, \"%s\"))",
								      c_ident(p->info.typ), ns_add_overridden(t, p,
									      nse1), ident(t->sym->name), ident(
									      p->sym->name), xsi_type(p->info.typ));
						      }
						      else if(p->info.typ->type==Tclass && !is_external(p->info.typ) &&
							      ...
							      fprintf(
								      ...
								      ident(t->sym->name), ident(
									      p->sym->name),
								      ns_add_overridden(t, p,
									      nse1), xsi_type(p->info.typ));
						      }
						      else if(p->info.typ->type != Tfun && !is_void(p->info.typ)) {
							      fprintf(
								      fout,
						   ...
						   p->info.hasval == False)                                 {
							...
							if(a==0) {
								fprintf(
									...
									"",
									strict_check(), ident(
										p->sym->name), i, p->info.maxOccurs-
									p->info.minOccurs);
								a = 1;
							}
							else
								fprintf(fout,
									" || soap_flag_%s%d > " SOAP_LONG_FORMAT "",
									ident(
										p->sym->name), i, p->info.maxOccurs-
									p->info.minOccurs);
						}
							...
							if(p->info.minOccurs > 0)                                    {
								   ...
								   Tpointer)
								{
									   ...
									   0)
									{
										fprintf(
											fout,
											"\n\tif(%s(!a->%s::%s || a->%s::%s->size() < " SOAP_LONG_FORMAT
											"", strict_check(), ident(
												t->sym->name), ident(
												p->sym->name), ident(
												t->sym->name), ident(
										...
										fprintf(
											fout,
											" || !a->%s::%s || a->%s::%s->size() < " SOAP_LONG_FORMAT
											"", ident(t->sym->name),
											ident(p->sym->name), ident(
												t->sym->name), ident(
												p->sym->name),
	...
	if((has_ns(typ) || is_untyped(typ)) && is_binary(typ)) {
		...
		fprintf(fout,
			"\n\t\tif((!a->__ptr && soap->error) || soap_element_end_in(soap, tag))\n\t\t\treturn NULL;");
		      ...
		      if(((Tnode *)p->info.typ->ref)->type == Tclass) {
			      fprintf(fout, "\n\t\t{\ta->%s = soap_new_%s(soap, n);", ident(p->sym->name),
				      c_ident((Tnode *)p->info.typ->ref));
			      if(!is_external((Tnode *)p->info.typ->ref) && !is_volatile((Tnode *)p->info.typ->ref) &&
			         !is_typedef((Tnode *)p->info.typ->ref))
				      fprintf(fout,
		      ...
		      else {fprintf(fout, "\n\t\t{\ta->%s = (%s)soap_malloc(soap, n*sizeof(%s));", ident(p->sym->name),
		    ...
		    if(((Tnode *)p->info.typ->ref)->type == Tclass) {
				...
				fprintf(fout, "\n\t\t{\ta->%s = (%s)soap_malloc(soap, sizeof(%s) * a->__size);", ident(p->sym->name),
				  c_type_id((Tnode *)p->info.typ->ref, "*"),  c_type((Tnode *)p->info.typ->ref));
			  if(((Tnode *)p->info.typ->ref)->type == Tpointer)
				  fprintf(fout, "\n\t\t\tfor(i = 0; i < a->__size; i++)\n\t\t\t\ta->%s[i] = NULL;", ident(p->sym->name));
			  else if(!is_XML((Tnode *)p->info.typ->ref))
	      ...
	      if(p->info.maxOccurs > 1) {
		      if(d)
			      fprintf(fout,
				      "\n\t\t\tfor(a->__size[0] = 0; a->__size[0] <= " SOAP_LONG_FORMAT "; a->__size[0]++)",
				      p->info.maxOccurs);
		      else
			      fprintf(fout,
				      "\n\t\t\tfor(a->__size = 0; a->__size <= " SOAP_LONG_FORMAT "; a->__size++)",
				      p->info.maxOccurs);
	      }
	      else {
			  if(d)
		 ...
		 !is_volatile((Tnode *)p->info.typ->ref) && !is_typedef((Tnode *)p->info.typ->ref))
	      ...
	      if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
		      ...
		      fprintf(fout, "\n\t\t\t\tif(!soap_in_%s(soap, NULL, p, \"%s\"))",
			      c_ident((Tnode *)p->info.typ->ref), xsi_type((Tnode *)p->info.typ->ref));
	      fprintf(fout, "\n\t\t\t\t{\tif(soap->error != SOAP_NO_TAG)\n\t\t\t\t\t\treturn NULL;");
	      fprintf(fout, "\n\t\t\t\t\tsoap->error = SOAP_OK;");
	      fprintf(fout, "\n\t\t\t\t\tbreak;");
	      fprintf(fout, "\n\t\t\t\t}");
	      ...
	      if(((Tnode *)p->info.typ->ref)->type == Tclass || has_class((Tnode *)p->info.typ->ref))
		      fprintf(fout,
			      ...
			      ident(p->sym->name), c_ident((Tnode *)p->info.typ->ref), c_type(
				      (Tnode *)p->info.typ->ref), ident(p->sym->name));
	      else
		      fprintf(fout, "\n\t\t\ta->%s = (%s)soap_malloc(soap, soap->blist->size);", ident(
				      p->sym->name), c_type(p->info.typ));
