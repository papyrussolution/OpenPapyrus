...
static long FASTCALL minlen(const Tnode * typ);
static long FASTCALL maxlen(const Tnode * typ);
...
static const char * FASTCALL ident(const char * name)
{
...
static int FASTCALL is_primclass(const Tnode * typ)
{
	if(typ->type == Tstruct || typ->type == Tclass) {
		if(!is_dynamic_array(typ)) {
			const Table * t = static_cast<const Table *>(typ->ref);
			while(t) {
			...
			if(!t)
				...
				t = static_cast<const Table *>(typ->ref);
				while(t) {
		...
		return is_primclass(static_cast<const Tnode *>(typ->ref)); // @recursion
	return 0;
}

static int FASTCALL is_imported(const Tnode * typ) { return (typ->imported != NULL); }
...
const char * FASTCALL soap_type(const Tnode *);
char * c_storage(int/*Storage*/);
char * FASTCALL c_init(const Entry *);
const char * c_type(const Tnode * typ);
static const char * FASTCALL xsi_type(const Tnode *);
static const char * FASTCALL xsi_type_cond(Tnode * typ, int flag) { return flag ? xsi_type(typ) : ""; }
const char * FASTCALL xsi_type_u(const Tnode *);
static const char * FASTCALL xsi_type_cond_u(Tnode * typ, int flag) { return flag ? xsi_type_u(typ) : ""; }
const char * FASTCALL wsdl_type(const Tnode *, const char *);
const char * FASTCALL base_type(const Tnode *, const char *);
const char * xml_tag(Tnode *);

static char * FASTCALL prefix_of(char * s)
{
	char * t;
	while(*s == '_' || *s == ':')
		s++;
...
static char * FASTCALL ns_qualifiedElement(const Tnode * typ)
{
	Service * sp;
	char * s = NULL;
	if(typ->sym)
...
char * ns_qualifiedAttribute(const Tnode *);
...
static char * FASTCALL ns_convert(const char * tag)
{
	char * t;
	char * s;
	size_t i, n;
...
static char * FASTCALL ns_add(const char * tag, const char * ns)
{
	char * n, * t;
...
char * FASTCALL ns_remove(const char *);
char * ns_remove1(char *);
char * ns_remove2(char *);
char * res_remove(char *);
const char * FASTCALL ns_name(const char * tag);
char * ns_cname(const char *, const char *);
char * ns_fname(char *);

static int FASTCALL has_class(const Tnode * typ)
{
	const Entry * p;
	if(typ->type == Tstruct && typ->ref) {
		for(p = static_cast<const Table *>(typ->ref)->list; p; p = p->next) {
...
static int FASTCALL has_constructor(const Tnode * typ)
{
	if(typ->type == Tclass) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next) {
				if(p->info.typ->type == Tfun && sstreq(p->sym->name, typ->id->name) && static_cast<const FNinfo *>(p->info.typ->ref)->ret->type == Tnone) {
					const Entry * q = static_cast<const FNinfo *>(p->info.typ->ref)->args->list;
					if(!q)
						return 1;
				}
			}
...
static int FASTCALL has_destructor(const Tnode * typ)
{
	if(typ->type == Tclass) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next)
...
static int FASTCALL has_getter(const Tnode * typ)
{
	if(typ->type == Tclass) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next) {
				if(p->info.typ->type == Tfun && sstreq(p->sym->name, "get") && ((FNinfo *)p->info.typ->ref)->ret->type == Tint) {
					const Entry * q = static_cast<const FNinfo *>(p->info.typ->ref)->args->list;
					if(q && q->info.typ->type == Tpointer && static_cast<const Tnode *>(q->info.typ->ref)->type == Tstruct && 
						static_cast<const Tnode *>(q->info.typ->ref)->id == lookup("soap"))
						return 1;
				}
			}
		}
...
static int  FASTCALL has_ns_eq(const char * ns, const char * s)
{
	size_t n;
	while(*s == '_' || *s == ':')
		s++;
...
static int FASTCALL has_ns(const Tnode * typ)
{
	return oneof3(typ->type, Tstruct, Tclass, Tenum) ? has_ns_eq(NULL, typ->id->name) : 0;
}

static int FASTCALL has_ns_t(const Tnode * typ)
{
	if(typ->sym) {
...
static int has_offset(const Tnode * typ)
{
	if(oneof2(typ->type, Tstruct, Tclass)) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next) {
				if((p->info.typ->type == Tint || (p->info.typ->type == Tarray && static_cast<const Tnode *>(p->info.typ->ref)->type == Tint)) && sstreq(ident(p->sym->name), "__offset"))
					return 1;
			}
		}
	}
...
static int reflevel(const Tnode * typ)
{
	int level = 0;
	for(; typ->type == Tpointer; level++)
		typ = static_cast<const Tnode *>(typ->ref);
	return level;
}

static int FASTCALL is_string(const Tnode * typ)
	{ return (typ->type == Tpointer && static_cast<const Tnode *>(typ->ref)->type == Tchar && !static_cast<const Tnode *>(typ->ref)->sym); }
static int FASTCALL is_wstring(const Tnode * typ)
	{ return (typ->type == Tpointer && static_cast<const Tnode *>(typ->ref)->type == Twchar && !static_cast<const Tnode *>(typ->ref)->sym); }

static const Tnode * FASTCALL reftype(const Tnode * typ)
{
...
int  is_external(const Tnode * typ);
int  FASTCALL is_anyType(const Tnode *);
int  FASTCALL is_anyAttribute(const Tnode *);
int  is_binary(const Tnode * typ);
int  FASTCALL is_fixedstring(const Tnode *);
int  FASTCALL is_stdstring(const Tnode * typ);
int  FASTCALL is_stdwstring(const Tnode *);
int  is_stdstr(const Tnode * typ);
static int FASTCALL is_typedef(const Tnode * typ) { return (typ->sym && !is_transient(typ)); }
static int FASTCALL get_dimension(const Tnode * typ)
	{ return (static_cast<const Tnode *>(typ->ref)->width) ? typ->width / static_cast<const Tnode *>(typ->ref)->width : 0; }
static int FASTCALL is_document(const char * style)
	{ return (!eflag && !style) || (style && sstreq(style, "document")); }
static int FASTCALL is_literal(const char * encoding)
	{ return (!eflag && !encoding) || (encoding && sstreq(encoding, "literal")); }
int  is_keyword(const char *);
int  FASTCALL is_repetition(const Entry *);
int  FASTCALL is_choice(const Entry *);
int  is_sequence(Entry *);
int  FASTCALL is_anytype(const Entry * p);
const char * FASTCALL xsi_type_Tarray(const Tnode *);
char * FASTCALL xsi_type_Darray(const Tnode * typ);
...
void soap_get(Tnode *);
void soap_in(const Tnode *);
void soap_in_Darray(const Tnode *);
void soap_instantiate_class(Tnode *);
int  FASTCALL get_Darraydims(const Tnode * typ);
...
static const char * FASTCALL c_type_id(const Tnode * typ, const char * name);
static const char * FASTCALL the_type(const Tnode * typ);
static int FASTCALL is_hexBinary(const Tnode * typ);
static int FASTCALL is_primitive(const Tnode * typ);
static const char * FASTCALL has_soapref(const Tnode * typ);

//char * t_ident(Tnode *);
//
// t_ident gives the name of a type in identifier format
//
static const char * FASTCALL t_ident(const Tnode * typ)
{
	char * p;
	const char * q;
	switch(typ->type) {
	    ...
	    case Treference: return c_ident(static_cast<const Tnode *>(typ->ref));
	    case Tpointer:
			if(is_string(typ))
				return "string";
			if(is_wstring(typ))
	    ...
	    case Tarray:
			...
			if(static_cast<const Tnode *>(typ->ref)->width)
...
void compile(Table * table)
{
	...
	fprintf(fhead, "\nSOAP_FMAC3 void * FASTCALL soap_getelement(struct soap *, int *);");
	if(!cflag && !namespaceid)
		fprintf(fhead, "\n\n#ifdef __cplusplus\n\t}\n#endif");
	...
	for(filenum = 1; partnum == 0; filenum++) {
		...
		if(filenum == 1) {
			if(!lflag) {
				...
				if(!cflag && !namespaceid)
					fprintf(fout, "\n\n#ifdef __cplusplus\nextern \"C\" {\n#endif");
				fprintf(fout, "\nSOAP_FMAC3 void * FASTCALL soap_getelement(struct soap * soap, int * type)\n{");
			...
			if(!cflag) {
				...
				fprintf(fout, "\n\t}\n\treturn SOAP_OK;");
				fprintf(fout, "\n}");
				fprintf(fhead, "\nSOAP_FMAC3 void * FASTCALL soap_class_id_enter(struct soap*, const char*, void*, int, size_t, const char*, const char*);");
				if(!lflag) {
					fprintf(fout, "\n\nSOAP_FMAC3 void * FASTCALL soap_class_id_enter(struct soap *soap, const char *id, void *p, int t, size_t n, const char *type, const char *arrayType)");
		...
		if(t) {
			for(p = t->list; p; p = p->next)        {
				if(p->info.typ->type == Tfun && !(p->info.sto&Sextern) && has_ns_eq(ns->name, p->sym->name)) {
					...
					if(!cflag) {
						if(iflag || jflag) {
							const char * sname = (sp && sp->name) ? sp->name : "";
							if(!Sflag) {
	...
	char * encoding)
{
	...
	int mimein, mimeout;
	const char * action;
	char * comment, * method_style = NULL, * method_encoding = NULL, * method_response_encoding = NULL;
	char * portname;
	char * binding;
	fprintf(fd, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
	for(sp = services; sp; sp = sp->next)
	...
	if(t) {
		...
		if(custom_header) {
			...
			r = static_cast<Table *>(entry(classtable, lookup("SOAP_ENV__Header"))->info.typ->ref);
			if(r) {
		...
		if(custom_fault) {
			...
			Table * r = static_cast<Table *>(entry(classtable, lookup("SOAP_ENV__Detail"))->info.typ->ref);
			if(r)
				for(q = r->list; q; q = q->next)
				...
				if(!eflag) {
					...
					if(q) {
						if(is_document(method_style)) {
							...
							for(q = ((Table *)q->info.typ->ref)->list; q; q = q->next) {
								if(!is_repetition(q) && !is_anytype(q) && !has_ns_eq(NULL, q->sym->name) && !is_transient(q->info.typ) && q->info.typ->type != Tfun &&
									...
									if(e) {
										if((e->info.sto&Sattribute) != (q->info.sto&Sattribute)|| reftype(e->info.typ) != reftype(q->info.typ)) {
											sprintf(errbuf,
												...
												q->sym->name, c_type(q->info.typ), q->lineno, c_type(e->info.typ), ns_convert(q->sym->name));
											semwarn(errbuf);
										}
									}
									else {
						...
						if(q && !is_transient(q->info.typ)) {
							if(!is_response(q->info.typ)) {
									...
									if(e) {
										if((e->info.sto&Sattribute) != (q->info.sto&Sattribute) || reftype(e->info.typ) != reftype(q->info.typ)) {
											sprintf(errbuf,
												...
												q->sym->name, c_type(q->info.typ), ns_convert(q->sym->name));
											semwarn(errbuf);
										}
									}
									else {
								...
								if(is_document(method_style)) {
									...
									for(q = ((Table *)((Tnode *)q->info.typ->ref)->ref)->list; q; q = q->next) {
										if(!is_repetition(q) && !is_anytype(q) && !has_ns_eq(NULL, q->sym->name) && !is_transient(q->info.typ) &&
										   q->info.typ->type != Tfun && !(q->info.sto & Sattribute)) {
											e = entry(r, q->sym);
											if(e) {
												if((e->info.sto & Sattribute) != (q->info.sto & Sattribute) || reftype(e->info.typ) != reftype(q->info.typ)) {
													sprintf(errbuf, "Qualified member field '%s' has a type that does not correspond to the unique type '%s' defined for elements '<%s>'",
														q->sym->name, c_type(q->info.typ), ns_convert(q->sym->name));
													semwarn(errbuf);
												}
											}
...
void gen_proxy_code(FILE * fd, Table * table, Symbol * ns, char * name, char * URL, char * executable, char * URI, char * encoding)
{
	...
	Table * t;
	const char * soap = iflag ? "this" : "this->soap";
	...
	fprintf(fd, "\n\nvoid %s::destroy()\n{\n\tsoap_destroy(%s);\n\tsoap_end(%s);\n}", name, soap, soap);
	fprintf(fd, "\n\nvoid %s::reset()\n{\n\tdestroy();\n\tsoap_done(%s);\n\tsoap_init(%s);\n\t%s_init(SOAP_IO_DEFAULT, SOAP_IO_DEFAULT);\n}", name, soap, soap, name);
	fprintf(fd, "\n\nvoid %s::soap_noheader() { %s->header = NULL; }", name, soap);
	if(!namespaceid) {
		p = entry(classtable, lookup("SOAP_ENV__Header"));
		if(p) {
			t = (Table *)p->info.typ->ref;
	...
	fprintf(fd, "\n\nconst SOAP_ENV__Header * %s::soap_header() { return %s->header; }", name, soap);
	fprintf(fd, "\nconst SOAP_ENV__Fault * %s::soap_fault() { return %s->fault; }", name, soap);
	fprintf(fd, "\nconst char * %s::soap_fault_string() { return *soap_faultstring(%s); }", name, soap);
	fprintf(fd, "\nconst char * %s::soap_fault_detail() { return *soap_faultdetail(%s); }", name, soap);
	fprintf(fd, "\nint %s::soap_close_socket() { return soap_closesock(%s); }", name, soap);
...
void gen_params(FILE * fd, Table * params, Entry * result, int flag)
{
	for(Entry * param = params->list; param; param = param->next)
...
void gen_call_method(FILE * fd, Table * table, Entry * method, char * name)
{
		...
		if(has_ns_eq(sp->ns, method->sym->name)) {
			...
			for(m = sp->list; m; m = m->next) {
				if(is_eq_nons(m->name, method->sym->name)) {
					if(oneof2(m->mess, ACTION, REQUEST_ACTION))
						action = m->part;
	...
	fprintf(fd, "\n\tif(soap_connect(soap, soap_endpoint, soap_action) || soap_envelope_begin_out(soap) || soap_putheader(soap) || soap_body_begin_out(soap)");
	fprintf(fd, "\n\t || soap_put_%s(soap, &soap_tmp_%s, \"%s\", NULL)", ident(method->sym->name), ident(method->sym->name), ns_convert(method->sym->name));
	...
	if(!is_literal(method_response_encoding)) {
		...
		xtag = response ? xml_tag(response->info.typ) : ns_convert(result->sym->name);
		xtyp = xsi_type(result->info.typ);
	}
	else {
		xtag = response ? xml_tag(response->info.typ) : xml_tag(result->info.typ);
		xtyp = ""; 
	}
	if(response) {
	...
	if(!is_transient(pout->info.typ)) {
		...
		if(pout->info.typ->type == Tarray && response)
			...
			fprintf(fd, "\n\tsoap_serialize_%s(soap, &%s);", c_ident((Tnode *)pout->info.typ->ref),
				ident(pout->sym->name));
		else if(((Tnode *)pout->info.typ->ref)->type == Tclass && !is_stdstring((Tnode *)pout->info.typ->ref) && !is_stdwstring((Tnode *)pout->info.typ->ref) &&
		        !is_dynamic_array((Tnode *)pout->info.typ->ref))
...
static void FASTCALL gen_element_begin(FILE * fd, int n, const char * s, const char * t)
{
	if(!is_invisible(s)) {
		if(tflag && t && *t)
	...
	if(t) {
		for(p = t->list; p; p = p->next)
			if(p->info.typ->type == Tfun && !(p->info.sto&Sextern) && has_ns_eq(ns, p->sym->name)) {
				...
				SETIFZ(method_response_encoding, method_encoding);
				/* request */
...
FILE * gen_env(char * buf, char * method, int response, Table * t, char * ns, char * name, char * URL, char * executable, char * URI, char * encoding)
{
	Symbol * s;
	Service * sp = NULL;
	char tmp[1024];
...
{
	char * p = static_cast<char *>(SAlloc::M(n));
	if(!p)
...
static int FASTCALL is_qname(const Tnode * p)
{
...
static int FASTCALL is_stdqname(const Tnode * p)
{
...
int is_transient(const Tnode * typ)
{
	if(!typ)
		...
		switch(typ->type) {
			...
			case Treference:
			case Tarray:
			case Ttemplate: return is_transient(static_cast<const Tnode *>(typ->ref));
			case Tnone:
			case Tvoid: return 1;
			default: break;
		}
...
int is_external(const Tnode * typ) { return (typ->transient == -1); }
int FASTCALL is_anyType(const Tnode * typ)
	{ return (typ->type == Tpointer) ? is_anyType(static_cast<const Tnode *>(typ->ref)) : (is_external(typ) && typ->type == Tstruct && sstreq(typ->id->name, "soap_dom_element")); }
int FASTCALL is_anyAttribute(const Tnode * typ)
	{ return (typ->type == Tpointer) ? is_anyAttribute(static_cast<const Tnode *>(typ->ref)) : (is_external(typ) && typ->type == Tstruct && sstreq(typ->id->name, "soap_dom_attribute")); }
int FASTCALL is_volatile(const Tnode * typ) { return (typ->transient == -2); }
int FASTCALL is_template(const Tnode * p) { return (p->type == Tpointer) ? is_template(static_cast<const Tnode *>(p->ref)) : p->type == Ttemplate; }

int FASTCALL is_repetition(const Entry * p)
{
	if(p)
...
int FASTCALL is_choice(const Entry * p)
{
	if(p)
...
{
	if(p) {
		const Tnode * q = p->info.typ;
		if(q->type == Tpointer)
			q = static_cast<const Tnode *>(q->ref);
...
// we should use an annotation for soapcpp2's input this in the future instead of a hard-coded member name 
// 
void FASTCALL needs_lang(const Entry * e)
{
...
{
	const Symbol * s = t->sym;
	if(s)
		do {
			for(const Entry * q = t->list; q; q = q->next)
...
const char * c_ident(const Tnode * typ)
{
	return (typ->sym && strcmp(typ->sym->name, "/*?*/")) ? res_remove(typ->sym->name) : t_ident(typ);
}

const char * FASTCALL soap_type(const Tnode * typ)
{
	char * s;
...
char * ns_qualifiedAttribute(const Tnode * typ)
{
	Service * sp;
	char * s = NULL;
	if(typ->sym)
...
char * ns_cname(const char * tag, const char * suffix)
{
	char * s;
	const char * t;
	size_t i, n;
...
char * FASTCALL ns_remove(const char * tag)
{
...
const char * FASTCALL xsi_type_u(const Tnode * typ)
{
	Service * sp;
	const char * s = NULL;
	if(tflag)
...
static const char * FASTCALL xsi_type(const Tnode * typ)
{
	if(!typ)
		return "NULL";
...
const char * FASTCALL wsdl_type(const Tnode * typ, const char * ns)
{
	if(!typ)
		return "NULL";
...
const char * FASTCALL base_type(const Tnode * typ, const char * ns)
{
	int d;
	char * s, * t;
	if(is_string(typ) || is_wstring(typ) || is_stdstring(typ) || is_stdwstring(typ)) {
	...
	if(is_dynamic_array(typ) && !is_binary(typ) && !has_ns(typ) && !is_untyped(typ)) {
		s = ns_remove(wsdl_type(static_cast<const Table *>(typ->ref)->list->info.typ, NULL));
		if(ns && *ns) {
...
const char * c_type(const Tnode * typ)
{
	...
	switch(typ->type) {
	    ...
	    case Tstruct:
			...
			break;
	    case Tclass:
			p = const_cast<char *>(ident(typ->id->name));
			break;
	    case Tunion: 
	    ...
	    case Tenum:
			...
			break;
	    case Tpointer:
			p = const_cast<char *>(c_type_id((Tnode *)typ->ref, "*")); // @badcast
			break;
	    case Treference:
			p = const_cast<char *>(c_type_id((Tnode *)typ->ref, "&")); // @badcast
			break;
...
char * FASTCALL c_init(const Entry * e)
{
	static char buf[1024];
	buf[0] = '\0';
	if(e && e->info.hasval) {
...
// Allows you to specify the identifier that acts acts as teh name of teh type of array 
// 
static const char * FASTCALL c_type_id(const Tnode * typ, const char * name)
{
	const char * id;
	...
	if(!typ)
		...
		switch(typ->type) {
			case Tnone:
				p = const_cast<char *>(id);
				break;
			case Tvoid:
				p = static_cast<char *>(emalloc(6+strlen(id)));
				strcpy(p, "void ");
				strcat(p, id);
				break;
			case Tchar:
				...
				break;
			case Twchar:
				p = static_cast<char *>(emalloc(9+strlen(id)));
				strcpy(p, "wchar_t ");
				strcat(p, id);
				break;
			case Tshort:
				p = static_cast<char *>(emalloc(7+strlen(id)));
				strcpy(p, "short ");
				strcat(p, id);
				break;
			case Tint:
				p = static_cast<char *>(emalloc(5+strlen(id)));
				strcpy(p, "int ");
				strcat(p, id);
				break;
			case Tlong:
				...
				break;
			case Tllong:
				p = static_cast<char *>(emalloc(8+strlen(id)));
				strcpy(p, "LONG64 ");
				strcat(p, id);
				break;
			case Tfloat:
			...
			case Tdouble:
				...
				break;
			case Tldouble:
				p = static_cast<char *>(emalloc(13+strlen(id)));
				strcpy(p, "long double ");
				strcat(p, id);
				break;
			case Tuchar:
				p = static_cast<char *>(emalloc(15+strlen(id)));
				strcpy(p, "uchar ");
				strcat(p, id);
				break;
			case Tushort:
				p = static_cast<char *>(emalloc(16+strlen(id)));
				strcpy(p, "ushort ");
				strcat(p, id);
				break;
			case Tuint:
				p = static_cast<char *>(emalloc(14+strlen(id)));
				strcpy(p, "uint ");
				strcat(p, id);
				break;
			case Tulong:
			...
			case Ttime:
				...
				break;
			case Tstruct:
				p = static_cast<char *>(emalloc((9+strlen(ident(typ->id->name))+strlen(id))*sizeof(char)));
				strcpy(p, "struct ");
				strcat(p, ident(typ->id->name));
				strcat(p, " ");
				strcat(p, id);
			...
			case Tclass:
				if(!typ->classed && !is_imported(typ)) {
					p = static_cast<char *>(emalloc((8+strlen(ident(typ->id->name))+strlen(id))*sizeof(char)));
					strcpy(p, "class ");
					strcat(p, ident(typ->id->name));
					const_cast<Tnode *>(typ)->classed = True; // @badcast
				}
				else {
					p = static_cast<char *>(emalloc((2+strlen(ident(typ->id->name))+strlen(id))*sizeof(char)));
					strcpy(p, ident(typ->id->name)); 
				}
				strcat(p, " ");
				strcat(p, id);
			...
			case Tpointer:
				...
				strcpy(p+1, id);
				p[0] = '*';
				p = const_cast<char *>(c_type_id((Tnode *)typ->ref, p)); // @badcast
				break;
			case Treference:
				p = (char *)emalloc(strlen(id)+2);
				strcpy(p+1, id);
...
const char * FASTCALL xsi_type_Tarray(const Tnode * typ)
{
	int cardinality;
	char * p;
	const char * s;
	Tnode * t = static_cast<Tnode *>(typ->ref);
	if(is_fixedstring(typ)) {
		return typ->sym ? ns_convert(typ->sym->name) : "xsd:string";
	}
	cardinality = 1;
...
char * FASTCALL xsi_type_Darray(const Tnode * typ)
{
	...
	const char * s;
	if(!typ->ref)
		return const_cast<char *>(""); // @badcast
	else {
...
{
	int d;
	const Entry * p = is_dynamic_array(typ);
	fprintf(fmatlab, "{\n");
...
const char * get_mxClassID(Tnode * typ)
{
	switch(typ->type) {
...
int no_of_var(Tnode * typ)
{
	...
	if(oneof2(static_cast<const Tnode *>(typ->ref)->type, Tstruct, Tclass)) {
		const Table * t = static_cast<const Table *>(static_cast<const Tnode *>(typ->ref)->ref);
		for(const Entry * p = t->list; p; p = p->next) {
			if(p->info.typ->type==Tpointer)
				i++;
		}
...
void in_defs2(Table * table)
{
	...
	Tnode * p;
	const char * s;
	for(int i = 0; i < TYPES; i++) { /* make sure (wrapper) classes are checked first */
		if(i == 0)
			j = Tclass;
		else if(i == Tclass)
			continue;
...
void soap_serialize(Tnode * typ)
{
	...
	if((p = is_dynamic_array(typ))) {
		if(typ->type == Tclass && !is_volatile(typ)) {
			...
			if(is_external(typ)) {
			      fprintf(fhead, "\nSOAP_FMAC1 void FASTCALL soap_serialize_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "const*"));
			      return;
		      }
		      fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "const*"));
		      fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap *soap, %s)\n{", c_ident(typ), c_type_id(typ, "const*a"));
		      if(is_binary(typ)) {
			      if(is_attachment(typ)) {
	...
	if(is_stdstring(typ) || is_stdwstring(typ)) {
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap*, const %s);", c_ident(typ), c_type_id(typ, "*"));
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap *soap, const %s)\n{\n\t(void)soap; (void)a; /* appease -Wall -Werror */\n}",
			c_ident(typ), c_type_id(typ, "*a"));
		return;
	}
	switch(typ->type) {
		...
		if(!is_volatile(typ)) {
			if(is_external(typ)) {
				fprintf(fhead, "\nSOAP_FMAC1 void FASTCALL soap_serialize_%s(struct soap*, const %s);", c_ident(typ), c_type_id(typ, "*"));
				return;
			}
			table = (Table *)typ->ref;
			fprintf(fout, "\n\nvoid %s::soap_serialize(struct soap *soap) const\n{", ident(typ->id->name));
		...
		if(is_external(typ) && !is_volatile(typ)) {
			fprintf(fhead, "\nSOAP_FMAC1 void FASTCALL soap_serialize_%s(struct soap*, const %s);",
				c_ident(typ), c_type_id(typ, "*"));
			return;
		}
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap*, const %s);", c_ident(
				typ), c_type_id(typ, "*"));
		if(!typ->ref)
			return;
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap *soap, const %s)\n{",
	    ...
	    case Tunion:
		if(is_external(typ) && !is_volatile(typ)) {
			fprintf(fhead, "\nSOAP_FMAC1 void FASTCALL soap_serialize_%s(struct soap*, int, const %s);", c_ident(typ), c_type_id(typ, "*"));
			return;
		}
		table = (Table *)typ->ref;
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap*, int, const %s);", c_ident(typ), c_type_id(typ, "*"));
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap *soap, int choice, const %s)\n{", c_ident(typ), c_type_id(typ, "*a"));
		fprintf(fout, "\n\tswitch(choice) {\n\t");
		for(t = table; t; t = t->prev) {
			for(p = t->list; p; p = p->next) {
				if(p->info.sto&(Sconst|Sprivate|Sprotected))
	    ...
	    case Tarray:
		if(is_external(typ)) {
			fprintf(fhead, "\nSOAP_FMAC1 void FASTCALL soap_serialize_%s(struct soap*, %s);", c_ident(
					typ), c_type_id(typ, "const"));
			return;
		}
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "const"));
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_serialize_%s(struct soap *soap, %s)", c_ident(typ), c_type_id(typ, "const a"));
...
void soap_default(Tnode * typ)
{
	...
	if(p) {
		if(typ->type == Tclass && !is_volatile(typ)) {
			...
			if(is_external(typ)) {
		      ...
		      fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "*"));
		      fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap *soap, %s)\n{\n\t(void)soap;", c_ident(typ), c_type_id(typ, "*a"));
		      if((s = has_soapref(typ)))
			      fprintf(fout, "\n\ta->%s = soap;", s);
			  else {
				;
	...
	if(is_primitive(typ) || is_string(typ)) {
		...
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "*"));
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap *, %s)\n{\n#ifdef SOAP_DEFAULT_%s\n\t*a = SOAP_DEFAULT_%s;\n#else\n\t*a = static_cast<%s>(0);\n#endif\n}",
			c_ident(typ), c_type_id(typ, "*a"), c_ident(typ), c_ident(typ), c_type(typ));
		return;
	}
	if(is_fixedstring(typ)) {
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap*, char[]);", c_ident(typ));
		fprintf(fout,  "\n\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap *, char a[])\n{\n\ta[0] = '\\0';\n}", c_ident(typ));
		return;
	}
	if(is_stdstring(typ) || is_stdwstring(typ)) {
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap*, %s);", c_ident(typ), c_type_id(typ, "*"));
	...
	switch(typ->type) {
		...
		fprintf(fhead, "\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap*, %s);", c_ident(typ), c_type(typ));
		fprintf(fout, "\n\nSOAP_FMAC3 void FASTCALL soap_default_%s(struct soap *soap, %s)\n{", c_ident(typ), c_type_id(typ, "a"));
		fprintf(fout, "\n\tint i;");
		fprintf(fout, "\n\tfor(i = 0; i < %d; i++)", get_dimension(typ));
		temp = (Tnode *)typ->ref;
...
void soap_traverse(Tnode * typ)
{
	...
	if(is_primitive_or_string(typ) || is_fixedstring(typ)) {
		fprintf(fhead, "\nSOAP_FMAC3 void SOAP_FMAC4 soap_traverse_%s(struct soap*, %s, const char *s, soap_walker p, soap_walker q);",
			c_ident(typ), c_type_id(typ, "*"));
		fprintf(fout, "\n\nSOAP_FMAC3 void SOAP_FMAC4 soap_traverse_%s(struct soap *, %s, const char * s, soap_walker p, soap_walker)\n{",
			c_ident(typ), c_type_id(typ, "*a"));
...
void soap_put(Tnode * typ)
{
	...
	if(typ->type == Tarray) {
		fprintf(fhead, "\n\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap*, %s, const char*, const char*);", ci, ctc);
		fprintf(fout, "\n\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap *soap, %s, const char *tag, const char *type)\n{", ci, ctca);
	}
		...
		fprintf(fhead, "\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap*, %s, const char*, const char*);", ci, ctcp);
		fprintf(fout, "\n\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap *soap, %s, const char *tag, const char *type)\n{", ci, ctcpa);
	}
	else {
		fprintf(fhead, "\n\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap*, const %s, const char*, const char*);", ci, ctp);
		fprintf(fout, "\n\nSOAP_FMAC3 int FASTCALL soap_put_%s(struct soap *soap, const %s, const char *tag, const char *type)\n{", ci, ctpa); 
	}
	fflush(fout);
	...
	/*
	if(typ->type == Tclass && !is_external(typ) && !is_volatile(typ) && !is_typedef(typ))
		fprintf(fout, "\n\tif(this->soap_out(soap, NZOR(tag, \"%s\"), id, type))\n\t\treturn soap->error;", xml_tag(typ));
	else
		fprintf(fout, "\n\tif(soap_out_%s(soap, NZOR(tag, \"%s\"), id, a, type))\n\t\treturn soap->error;", ci, xml_tag(typ));
	if(!is_invisible(typ->id->name))
	...
	*/
	...
	fprintf(fout, "\n\treturn ");
	if(typ->type == Tclass && !is_external(typ) && !is_volatile(typ) && !is_typedef(typ))
		fprintf(fout, "this->soap_out(soap, NZOR(tag, \"%s\"), local_id, type) ? soap->error : ", xml_tag(typ)); // @v10.3.0 id-->local_id
	else
		fprintf(fout, "soap_out_%s(soap, NZOR(tag, \"%s\"), local_id, a, type) ? soap->error : ", ci, xml_tag(typ)); // @v10.3.0 id-->local_id
	if(!is_invisible(typ->id->name))
...
static int is_boolean(const Tnode * typ)
{
	if(typ->type == Tenum) {
		if(static_cast<const Table *>(typ->ref) == booltable)
			return 1;
		else {
...
static int FASTCALL is_hexBinary(const Tnode * typ)
{
	...
	const Table * t;
	size_t n = strlen(ident(typ->id->name));
	if(oneof2(typ->type, Tstruct, Tclass) && n >= 9 && is_eq(ident(typ->id->name)+n-9, "hexBinary")) {
		for(t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			p = t->list;
			while(p && p->info.typ->type == Tfun)
				p = p->next;
			if(p && p->info.typ->type == Tpointer && static_cast<const Tnode *>(p->info.typ->ref)->type == Tuchar && sstreq(ident(p->sym->name), "__ptr")) {
				p = p->next;
...
static long FASTCALL maxlen(const Tnode * typ) { return (typ->maxLength < 0 || (typ->maxLength>>31) != 0) ? -1 : (long)typ->maxLength; }
...
static const char * FASTCALL has_soapref(const Tnode * typ)
{
	if(typ->type == Tstruct || typ->type == Tclass) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next)
				if(p->info.typ->type == Tpointer && static_cast<const Tnode *>(p->info.typ->ref)->type == Tstruct && static_cast<const Tnode *>(p->info.typ->ref)->id == lookup("soap"))
...
static int FASTCALL has_setter(const Tnode * typ)
{
	if(typ->type == Tclass) {
		for(const Table * t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
			for(const Entry * p = t->list; p; p = p->next) {
				if(p->info.typ->type == Tfun && sstreq(p->sym->name, "set") && static_cast<const FNinfo *>(p->info.typ->ref)->ret->type == Tint) {
					const Entry * q = static_cast<const FNinfo *>(p->info.typ->ref)->args->list;
					if(q && q->info.typ->type == Tpointer && ((Tnode *)q->info.typ->ref)->type == Tstruct && static_cast<const Tnode *>(q->info.typ->ref)->id == lookup("soap"))
						return 1;
				}
			}
		}
...
int is_primitive_or_string(const Tnode * typ)
{
...
{
	if(typ->type == Tpointer)
		return is_stdstring(static_cast<const Tnode *>(typ->ref)) || is_stdwstring(static_cast<const Tnode *>(typ->ref));
	return is_stdstring(typ) || is_stdwstring(typ);
}

static void FASTCALL soap_set_attr(const Entry * p, const char * obj, const char * name, char * tag)
{
	Tnode * typ = p->info.typ;
...
static void FASTCALL soap_attr_value(const Entry * p, const char * obj, const char * name, char * tag)
{
	int flag = 0;
	const Tnode * typ = p->info.typ;
...
static char * ptr_cast(const Table * t, const char * name)
{
...
void soap_out(Tnode * typ)
{
	...
	switch(typ->type) {
		...
		if(is_primclass(typ)) {
			for(table = static_cast<Table *>(typ->ref); table; table = table->prev) {
				p = table->list;
				if(p && is_item(p))
					break;
			}
			      ...
			      for(p = t->list; p != (Entry *)0; p = p->next) {
				      ...
				      if(is_item(p))
					      ...
					      fprintf(fout, "\n\tif(soap_putelement(soap, a->%s::%s, \"%s\", -1, a->%s::%s))\n\t\treturn soap->error;",
						      ident(t->sym->name), ident(p->next->sym->name), ns_add(p->sym->name, nse1), ident(t->sym->name), ident(p->sym->name));
					      p = p->next;
				      }
				      else if(is_choice(p)) {
					      ...
					      fprintf(fout, "\n\tif(soap_out_%s(soap, \"%s\", -1, (char*const*)&soap_tmp_%s, \"%s\"))\n\t\treturn soap->error;",
						      c_ident(p->info.typ), ns_add_overridden(t, p, nse1), ident(p->sym->name),
						      xsi_type_cond_u(p->info.typ, !has_ns_eq(NULL, p->sym->name)));
				      else if(is_stdqname(p->info.typ))
					      fprintf(fout, "\n\tif(soap_out_%s(soap, \"%s\", -1, &soap_tmp_%s, \"%s\"))\n\t\treturn soap->error;",
						      c_ident(p->info.typ), ns_add_overridden(t, p, nse1), ident(p->sym->name),
					      ...
					      fprintf(fout, "\n\tif(soap_out_%s(soap, \"%s\", -1, (char*const*)&soap_tmp_%s, \"%s\"))\n\t\treturn soap->error;",
						      c_ident((Tnode *)p->info.typ->ref), ns_add(p->sym->name, nse1), ident(p->sym->name),
						      xsi_type_cond_u(p->info.typ, !has_ns_eq(NULL, p->sym->name)));
				      else if(p->info.typ->type == Tpointer && is_stdqname((Tnode *)p->info.typ->ref))
					      fprintf(fout, "\n\tif(soap_out_%s(soap, \"%s\", -1, &soap_tmp_%s, \"%s\"))\n\t\treturn soap->error;",
		...
		if(is_template(typ)) {
			...
			p = is_dynamic_array(static_cast<const Tnode *>(typ->ref));
		      if(p) {
			      d = get_Darraydims((Tnode *)typ->ref);
			      if(d)
		...
		if(is_typedef(typ))
			...
			fprintf(fout, "\n\treturn NZOR(s, soap_long2s(soap, static_cast<long>(n)));"); // @v10.7.8
			// @v10.7.8 fprintf(fout, "\n\tif(s)\n\t\treturn s;");
...
void soap_out_Darray(Tnode * typ)
{
	...
	char * nse = ns_qualifiedElement(typ);
	char * nsa = ns_qualifiedAttribute(typ);
	const Table * table = static_cast<const Table *>(typ->ref);
	fprintf(fhead, "\nSOAP_FMAC3 int FASTCALL soap_out_%s(struct soap*, const char*, int, const %s, const char*);",
		c_ident(typ), c_type_id(typ, "*"));
	if(is_external(typ))
		return;
	...
	{
		if(typ->type == Tclass) {
			for(const Table * t = table; t; t = t->prev) {
				for(const Entry * p = t->list; p; p = p->next) {
	...
	{
		const Entry * p = is_dynamic_array(typ);
		char * item = (p->sym->name[5]) ? ns_add(p->sym->name+5, nse) : ns_add("item", nse);
		const char * p_sym_name_ident = ident(p->sym->name); // @sobolev
		if(!has_ns(typ) && !is_untyped(typ) && !is_binary(typ)) {
			if(is_untyped(p->info.typ)) {
				if(has_offset(typ))
					if(d)
						fprintf(fout, "\n\tchar *t = a->%s ? soap_putsizesoffsets(soap, \"%s\", a->__size, a->__offset, %d) : NULL;", p_sym_name_ident, wsdl_type(p->info.typ, "xsd"), d);
					else
						fprintf(fout, "\n\tchar *t = a->%s ? soap_putsize(soap, \"%s\", n + a->__offset) : NULL;", p_sym_name_ident, wsdl_type(p->info.typ, "xsd"));
				else if(d)
					fprintf(fout, "\n\tchar *t = a->%s ? soap_putsizes(soap, \"%s\", a->__size, %d) : NULL;", p_sym_name_ident, wsdl_type(p->info.typ, "xsd"), d);
				else
					fprintf(fout, "\n\tchar *t = a->%s ? soap_putsize(soap, \"%s\", n) : NULL;", p_sym_name_ident, wsdl_type(p->info.typ, "xsd"));
			}
			else {
				if(has_offset(typ))
					  if(d)
						  fprintf(fout, "\n\tchar *t = a->%s ? soap_putsizesoffsets(soap, \"%s\", a->__size, a->__offset, %d) : NULL;", p_sym_name_ident, xsi_type(typ), d);
					  else
						  fprintf(fout, "\n\tchar *t = a->%s ? soap_putsize(soap, \"%s\", n + a->__offset) : NULL;", p_sym_name_ident, xsi_type(typ));
				  else if(d)
					  fprintf(fout, "\n\tchar *t = a->%s ? soap_putsizes(soap, \"%s\", a->__size, %d) : NULL;", p_sym_name_ident, xsi_type(typ), d);
				  else
					  fprintf(fout, "\n\tchar *t = a->%s ? soap_putsize(soap, \"%s\", a->__size) : NULL;", p_sym_name_ident, xsi_type(typ)); 
			}
		}
		if(d)
			fprintf(fout, "\n\tid = soap_element_id(soap, tag, id, a, (struct soap_array*)&a->%s, %d, type, %s);", p_sym_name_ident, d, soap_type(typ));
		else if(is_attachment(typ)) {
			fprintf(fout, "\n#ifndef WITH_LEANER\n\tid = soap_attachment(soap, tag, id, a, (struct soap_array*)&a->%s, a->id, a->type, a->options, 1, type, %s);", p_sym_name_ident, soap_type(typ));
			fprintf(fout, "\n#else\n\tid = soap_element_id(soap, tag, id, a, (struct soap_array*)&a->%s, 1, type, %s);\n#endif", p_sym_name_ident, soap_type(typ));
		}
		else
			fprintf(fout, "\n\tid = soap_element_id(soap, tag, id, a, (struct soap_array*)&a->%s, 1, type, %s);", p_sym_name_ident, soap_type(typ));
		fprintf(fout, "\n\tif(id < 0)\n\t\treturn soap->error;");
		fprintf(fout, "\n\tif(");
		if(has_ns(typ) || is_untyped(typ) || is_binary(typ)) {
			if(table->prev)
			...
			if(!has_ns(typ) && !is_untyped(typ)) {
				  if(d) {
					  ...
					  if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
						  fprintf(fout, "\n\t\tsoap_outliteral(soap, \"%s\", &a->%s[i], NULL);", item, p_sym_name_ident);
					  else if(is_XML((Tnode *)p->info.typ->ref) && is_wstring((Tnode *)p->info.typ->ref))
						  fprintf(fout, "\n\t\tsoap_outwliteral(soap, \"%s\", &a->%s[i], NULL);", item, p_sym_name_ident);
					  else if(((Tnode *)p->info.typ->ref)->type == Tclass && !is_external((Tnode *)p->info.typ->ref) &&
							!is_volatile((Tnode *)p->info.typ->ref) && !is_typedef((Tnode *)p->info.typ->ref))
						  fprintf(fout, "\n\t\ta->%s[i].soap_out(soap, \"item\", -1, \"%s\");", p_sym_name_ident, xsi_type_u(((Tnode *)p->info.typ->ref)));
					  else
						  fprintf(fout, "\n\t\tsoap_out_%s(soap, \"%s\", -1, &a->%s[i], \"%s\");",
							  c_ident(((Tnode *)p->info.typ->ref)), item, p_sym_name_ident, xsi_type_u(((Tnode *)p->info.typ->ref)));
				  }
				  else {
					fprintf(fout, "\n\t\tsoap->position = 1;\n\t\tsoap->positions[0] = i;");
					if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
						...
						fprintf(fout, "\n\t\ta->%s[i].soap_out(soap, \"%s\", -1, \"%s\");",
							p_sym_name_ident, item, xsi_type_u(((Tnode *)p->info.typ->ref)));
					else
						fprintf(fout, "\n\t\tsoap_out_%s(soap, \"%s\", -1, &a->%s[i], \"%s\");",
							c_ident(((Tnode *)p->info.typ->ref)), item, p_sym_name_ident, xsi_type_u(((Tnode *)p->info.typ->ref))); 
				  }
				...
				if(is_XML((Tnode *)p->info.typ->ref) && is_string((Tnode *)p->info.typ->ref))
					...
					fprintf(fout, "\n\t\tsoap_outwliteral(soap, \"%s\", &a->%s[i], NULL);", item, p_sym_name_ident);
						...
						!is_volatile((Tnode *)p->info.typ->ref) && !is_typedef((Tnode *)p->info.typ->ref))
					fprintf(fout, "\n\t\ta->%s[i].soap_out(soap, \"%s\", -1, \"%s\");", p_sym_name_ident, item, xsi_type_u(((Tnode *)p->info.typ->ref)));
				else
					fprintf(fout, "\n\t\tsoap_out_%s(soap, \"%s\", -1, &a->%s[i], \"%s\");",
						c_ident(((Tnode *)p->info.typ->ref)), item, p_sym_name_ident, xsi_type_u(((Tnode *)p->info.typ->ref))); 
			  }
...
void soap_in(const Tnode * typ)
{
	...
	if(is_primitive_or_string(typ) && typ->type != Tenum) {
		...
		if(is_stdstring(typ)) {
			...
			fprintf(fhead, "\nSOAP_FMAC3 std::string * FASTCALL soap_in_%s(struct soap*, const char*, std::string*, const char*);",
				c_ident(typ));
			if(is_stdXML(typ))
				fprintf(fout, "\n\nSOAP_FMAC3 std::string * FASTCALL soap_in_%s(struct soap * soap, const char *tag, std::string * s, const char *)\n{\n\tchar * t;\n\tif(soap_inliteral(soap, tag, &t)) {\n\t\tif(!s)\n\t\t\ts = soap_new_std__string(soap, -1);\n\t\ts->assign(t);\n\t\treturn s;\n\t}\n\treturn NULL;\n}",
					c_ident(typ));
			else {
		...
		if(is_stdwstring(typ)) {
			...
			if(is_stdXML(typ))
				...
				fprintf(fhead, "\nSOAP_FMAC3 std::wstring * FASTCALL soap_in_%s(struct soap*, const char*, std::wstring*, const char*);",
				      c_ident(typ));
			      fprintf(fout, "\n\nSOAP_FMAC3 std::wstring * FASTCALL soap_in_%s(struct soap *soap, const char *tag, std::wstring *s, const char *)\n{\n\tif(soap_element_begin_in(soap, tag, 1, NULL))\n\t\treturn NULL;\n\tif(!s)\n\t\ts = soap_new_std__wstring(soap, -1);\n\tif(soap->null)\n\t\tif(s)\n\t\t\ts->erase();",
				      c_ident(typ));
			      fprintf(fout, "\n\tif(soap->body && !*soap->href) {\n\t\twchar_t *t;\n\t\ts = static_cast<std::wstring *>(soap_class_id_enter(soap, soap->id, s, %s, sizeof(std::wstring), soap->type, soap->arrayType));\n\t\tif(s)\n\t\t{\tif(!(t = soap_wstring_in(soap, 1, %ld, %ld)))\n\t\t\t\treturn NULL;\n\t\t\ts->assign(t);\n\t\t}\n\t}\n\telse\n\t\ts = (std::wstring*)soap_id_forward(soap, soap->href, soap_class_id_enter(soap, soap->id, s, %s, sizeof(std::wstring), soap->type, soap->arrayType), 0, %s, 0, sizeof(std::wstring), 0, soap_copy_%s);\n\tif(soap->body && soap_element_end_in(soap, tag))\n\t\treturn NULL;\n\treturn s;\n}",
				      soap_type(typ), minlen(typ), maxlen(typ), soap_type(typ), soap_type(typ), c_ident(typ)); 
			}
			return;
		}
		...
		*/
		...
		if(is_wstring(typ)) {
			//fprintf(fout, "\n\t%s = soap_inwstring(soap, tag, a, type, %s, %ld, %ld);", c_type_id(typ, "* p"), soap_type(typ), minlen(typ), maxlen(typ));
			//fprintf(fout, "\n\treturn p;\n}");
			fprintf(fout, "\n\treturn soap_inwstring(soap, tag, a, type, %s, %ld, %ld);\n}", soap_type(typ), minlen(typ), maxlen(typ));
		}
		else if(is_string(typ)) {
			//fprintf(fout, "\n\t%s = soap_instring(soap, tag, a, type, %s, %d, %ld, %ld);", c_type_id(typ, "* p"), soap_type(typ), is_qname(typ)+1, minlen(typ), maxlen(typ));
			//fprintf(fout, "\n\treturn p;\n}");
	...
	switch(typ->type) {
		...
		if(is_primclass(typ)) {
			fprintf(fout, "\n\t(void)type; /* appease -Wall -Werror */\n\tif(soap_element_begin_in(soap, tag, 1, NULL))\n\t\treturn NULL;");
			if(has_class(typ))
				fprintf(fout, "\n\tif(!(a = static_cast<%s>(soap_class_id_enter(soap, soap->id, a, %s, sizeof(%s), soap->type, soap->arrayType))))\n\t\treturn NULL;",
					c_type_id(typ, "*"), soap_type(typ), c_type(typ));
			else
				fprintf(fout, "\n\tif(!(a = (%s)soap_id_enter(soap, soap->id, a, %s, sizeof(%s), 0, 0, 0, 0)))\n\t\treturn NULL;",
					c_type_id(typ, "*"), soap_type(typ), c_type(typ));
			fprintf(fout, "\n\tsoap_revert(soap);\n\t*soap->id = '\\0';");
			/* fprintf(fout,"\n\tif(soap->alloced)"); */
			...
			if(!is_discriminant(typ)) {
		      ...
		      if(has_class(typ)) {
			      if(is_invisible(typ->id->name))
				      fprintf(fout, "\n\ta = static_cast<%s>(soap_class_id_enter(soap, \"\", a, %s, sizeof(%s), soap->type, soap->arrayType));",
					      c_type_id(typ, "*"), soap_type(typ), c_type(typ));
			      else
				      fprintf(fout, "\n\ta = static_cast<%s>(soap_class_id_enter(soap, soap->id, a, %s, sizeof(%s), soap->type, soap->arrayType));",
					      c_type_id(typ, "*"), soap_type(typ), c_type(typ));
		      }
		      else if(is_invisible(typ->id->name))
			      fprintf(fout, "\n\ta = (%s)soap_id_enter(soap, \"\", a, %s, sizeof(%s), 0, 0, 0, 0);",
				      c_type_id(typ, "*"), soap_type(typ), c_type(typ));
		      else
			      fprintf(fout, "\n\ta = (%s)soap_id_enter(soap, soap->id, a, %s, sizeof(%s), 0, 0, 0, 0);",
				      c_type_id(typ, "*"), soap_type(typ), c_type(typ));
		      fprintf(fout, "\n\tif(!a)\n\t\treturn NULL;");
			/* fprintf(fout,"\n\tif(soap->alloced)"); */
		      fprintf(fout, "\n\tsoap_default_%s(soap, a);", c_ident(typ));
		      ...
		      for(t = table; t; t = t->prev) {
			      for(p = t->list; p; p = p->next) {
				      if(p->info.sto&(Sconst|Sprivate|Sprotected))
					      ...
					      if(((Tnode *)p->next->info.typ->ref)->type == Tclass && !is_external((Tnode *)p->next->info.typ->ref) && !is_volatile((Tnode *)p->next->info.typ->ref) &&
					      ...
					      if(is_unmatched(p->next->sym)) {
						      if(is_XML(p->next->info.typ->ref) && is_string(p->next->info.typ->ref))
							      ...
							      fprintf(fout, "\n\t\t\t\t\tif(!soap_in_%s(soap, NULL, p, \"%s\"))",
								      c_ident(p->next->info.typ->ref), xsi_type(p->next->info.typ->ref));
					      }
					      else {
							  if(is_XML(p->next->info.typ->ref) && is_string(p->next->info.typ->ref))
							    ...
							    fprintf(fout, "\n\t\t\t\t\tif(!soap_in_%s(soap, \"%s\", p, \"%s\"))", c_ident(p->next->info.typ->ref),
								    ns_add(p->next->sym->name, nse), xsi_type(p->next->info.typ->ref)); }
					      fprintf(fout, "\n\t\t\t\t\t\tbreak;");
		...
		if(is_primclass(typ)) {
			...
			fprintf(fout, "\n\t}");
			for(t = static_cast<const Table *>(typ->ref); t; t = t->prev) {
				const Entry * e = entry(classtable, t->sym);
			...
			fflush(fout);
			for(table = static_cast<const Table *>(typ->ref); table; table = table->prev) {
				p = table->list;
				if(p && is_item(p))
					break;
			}
			...
			if(!is_discriminant(typ)) {
				...
				fprintf(fout, "\n\ta->soap_default(soap);");
			table = static_cast<const Table *>(typ->ref);
			for(t = table; t; t = t->prev) {
				const Entry * e = entry(classtable, t->sym);
				const char * nsa1 = e ? ns_qualifiedAttribute(e->info.typ) : nsa;
				for(p = t->list; p; p = p->next)
	    ...
	    case Tarray:
			{
				const Tnode * temp = typ;
				while(temp->type == Tarray) {
					temp = static_cast<const Tnode *>(temp->ref);
				}
				if(is_external(typ)) {
					fprintf(fhead,"\nSOAP_FMAC1 %s SOAP_FMAC2 soap_in_%s(struct soap*, const char*, %s, const char*);",
				...
				fprintf(fhead, "\nSOAP_FMAC3 %s FASTCALL soap_in_%s(struct soap*, const char*, %s, const char*);", c_type_id(temp, "*"), c_ident(typ), c_type(typ));
				fprintf(fout, "\n\nSOAP_FMAC3 %s FASTCALL soap_in_%s(struct soap *soap, const char *tag, %s, const char *type)\n{",
				...
				fprintf(fout, "\n\ta = (%s)soap_id_enter(soap, soap->id, a, %s, sizeof(%s), 0, 0, 0, 0);", c_type_id((Tnode *)typ->ref, "(*)"), soap_type(typ), c_type(typ));
				fprintf(fout, "\n\tif(!a)\n\t\treturn NULL;");
				fprintf(fout, "\n\tsoap_default_%s(soap, a);", c_ident(typ));
				fprintf(fout, "\n\tif(soap->body && !*soap->href)");
				...
				fprintf(fout, "\n\t}\n\telse {\n\t\ta = (%s)soap_id_forward(soap, soap->href, (void *)soap_id_enter(soap, soap->id, a, %s, sizeof(%s), 0, 0, 0, 0), 0, %s, 0, sizeof(%s), 0, NULL);",
		...
		fprintf(fout, "\n\ta = (%s)soap_id_enter(soap, soap->id, a, %s, sizeof(%s), 0, 0, 0, 0);", c_type_id(typ, "*"), soap_type(typ), c_type(typ));
		fprintf(fout, "\n\tif(!a)\n\t\treturn NULL;");
		fprintf(fout, "\n\tif(soap->body && !*soap->href) {\n\t");
...
void soap_in_Darray(const Tnode * typ)
{
	int    i, j;
	char * nsa = ns_qualifiedAttribute(typ);
	const Table * table = static_cast<const Table *>(typ->ref);
