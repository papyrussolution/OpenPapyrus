...
{
	char * t = NULL;
	if(s && (t = (char *)soap_malloc(soap, sstrlen(s)+1)))
		strcpy(t, s);
	return t;
}
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_save_block(struct soap * soap, struct soap_blist * b, char * p, int flag)
{
	SETIFZ(b, soap->blist);
	if(b->size) {
		SETIFZ(p, (char *)soap_malloc(soap, b->size));
	...
	if(soap->mode&SOAP_IO_LENGTH) {
		if(pp->mark1 != 3) {
			...
			if(!strncmp(aid, "cid:", 4)) { /* RFC 2111 */
				if(soap->mode&SOAP_ENC_MTOM) {
					char * s = (char *)soap_malloc(soap, sstrlen(aid)-1);
					if(s) {
						*s = '<';
						strcpy(s+1, aid+4);
						strcat(s, ">");
...
SOAP_FMAC1 int SOAP_FMAC2 soap_s2wchar(struct soap * soap, const char * s, wchar_t ** t, long minlen, long maxlen)
{
	if(s) {
		long l;
		wchar_t * r;
		*t = r = (wchar_t*)soap_malloc(soap, sizeof(wchar_t)*(sstrlen(s)+1));
		if(!r)
			return soap->error = SOAP_EOM;
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_s2base64(struct soap * soap, const uchar * s, char * t, int n)
{
	...
	ulong m;
	char * p;
	SETIFZ(t, (char *)soap_malloc(soap, (n+2)/3*4+1));
	if(!t)
		return NULL;
	p = t;
	t[0] = '\0';
...
{
	char * p;
	SETIFZ(t, (char *)soap_malloc(soap, 2*n+1));
	if(!t)
		return NULL;
	p = t;
	t[0] = '\0';
