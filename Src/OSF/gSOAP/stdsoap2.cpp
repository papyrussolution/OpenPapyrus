...
#ifndef WITH_LEANER
 #ifndef PALM_1
static struct soap_multipart * soap_new_multipart(struct soap *, struct soap_multipart **, struct soap_multipart **,
                                                  char *,
                                                  size_t);
...
static int fsend(struct soap * soap, const char * s, size_t n)
{
			...
			if(nwritten <= 0) {
				...
				if(err == SOAP_EWOULDBLOCK || err == SOAP_EAGAIN) {
					...
					if(soap->ssl && r == SSL_ERROR_WANT_READ)
						r =
						        tcp_select(soap, soap->socket, SOAP_TCP_SELECT_RCV|
								SOAP_TCP_SELECT_ERR,
  ...
  #endif
						r =
						        tcp_select(soap, soap->socket, SOAP_TCP_SELECT_SND|
								SOAP_TCP_SELECT_ERR,
								soap->send_timeout ? soap->send_timeout : -10000);
		...
		for(;; )                                      {
  ...
  #endif
			{
				...
				if((soap->omode&SOAP_IO_UDP)) {
					...
					r =
					        recvfrom(soap->socket, s, (SOAP_WINSOCKINT)n, soap->socket_flags,
							(struct sockaddr *)&soap->peer, &k);                            /* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
		...
		for(;; ) {
			...
			*t = '\0';
			...
			if(soap->buflen > soap->chunkbuflen) {
				...
				soap->chunkbuflen = 0;
				DBGLOG(TEST,
					SOAP_MESSAGE(fdebug, "Passed end of buffer for chunked HTTP (%u bytes left)\n",
...
SOAP_FMAC2 soap_recv(struct soap * soap)
{
	...
	if(soap->mode&SOAP_ENC_DIME) {
		...
		if(soap->dime.chunksize) {
			DBGLOG(TEST,
				SOAP_MESSAGE(fdebug, "Get next DIME hdr for chunked DIME (%u bytes chunk)\n",
			...
			if(soap->buflen-soap->bufidx >= soap->dime.chunksize) {
				...
				soap->dime.chunksize -= soap->buflen-soap->bufidx;
			DBGLOG(TEST,
				SOAP_MESSAGE(fdebug, "%lu bytes remaining, count=%lu\n",
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_move(struct soap * soap, long n)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_dime_forward(struct soap * soap, unsigned char ** ptr, int * size, char ** id, char ** type,
	char ** options)
{
...
SOAP_FMAC2 soap_push_block(struct soap * soap, struct soap_blist * b, size_t n)
{
	...
	if(!b)
		b = soap->blist;
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "Push block of %u bytes (%u bytes total)\n", (unsigned int)n,
...
SOAP_FMAC2 soap_update_pointers(struct soap * soap, char * start, char * end, char * p1, char * p2)
{
	...
	for(i = 0; i < SOAP_IDHASH; i++) {
		for(ip = soap->iht[i]; ip; ip = ip->next)                                   {
			if(ip->ptr && (char *)ip->ptr >= start && (char *)ip->ptr <
			   end)                                                                               {
				DBGLOG(TEST,
					SOAP_MESSAGE(fdebug, "Update id='%s' %p -> %p\n", ip->id, ip->ptr,
						(char *)ip->ptr+(p1-p2)));
			...
			for(fp = ip->flist; fp; fp = fp->next) {
				if((char *)fp->ptr >= start && (char *)fp->ptr <
				   end)                                          {
	...
	for(xp = soap->xlist; xp; xp = xp->next) {
		if(xp->ptr && (char *)xp->ptr >= start && (char *)xp->ptr <
		   end)                                            {
			DBGLOG(TEST,
	...
	for(i = 0; i < SOAP_IDHASH; i++) {
		for(ip = soap->iht[i]; ip; ip = ip->next)                                   {
			if(ip->ptr)                                                                              {
				...
				r = ip->ptr;
				DBGLOG(TEST,
					if(q) SOAP_MESSAGE(fdebug, "Traversing link chain to resolve id='%s'\n", ip->id));
					while(q) {
					        p = *q;
					        *q = r;
...
SOAP_FMAC2 soap_save_block(struct soap * soap, struct soap_blist * b, char * p, int flag)
{
	...
	if(b->size) {
		...
		DBGLOG(TEST,
			SOAP_MESSAGE(fdebug, "Save all blocks in contiguous memory space of %u bytes (%p->%p)\n",
				(unsigned int) b->size, b->ptr, p));
		if(p) {
		        for(s = p, q = soap_first_block(soap, b); q; q = soap_next_block(soap, b))        {
		                n = soap_block_size(soap, b);
		                DBGLOG(TEST,
					SOAP_MESSAGE(fdebug, "Copy %u bytes from %p to %p\n", (unsigned int)n, q, s));
 #ifndef WITH_NOIDREF
		                if(flag)
...
SOAP_FMAC1
char *
SOAP_FMAC2 soap_putsizesoffsets(struct soap * soap, const char * type, const int * size, const int * offset, int dim)
{
	int i;
	if(!type)
		return NULL;
	if(soap->version == 2) {
		...
		for(i = 1; i < dim; i++)
	      ...
	      else {sprintf(soap->type, "%s[%d", type, size[0]);
		    for(i = 1; i < dim; i++)
			    sprintf(soap->type+strlen(soap->type), ",%d", size[i]); }
	      strcat(soap->type, "]"); }
	return soap->type;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_match_namespace(struct soap * soap, const char * id1, const char * id2, size_t n1, size_t n2)
{
...
SOAP_FMAC2 soap_current_namespace(struct soap * soap, const char * tag)
{
	...
	else {while(np && (strncmp(np->id, tag, s-tag) || np->id[s-tag]))
		      np = np->next;
	      if(!np)
	...
	if(t) {
		if(s)        {
			...
			if(t != tag2 && (err = soap_match_namespace(soap, tag1, tag2, s-tag1, t-tag2))) {
				DBGLOG(TEST,
					SOAP_MESSAGE(fdebug, "Tags '%s' and '%s' match but namespaces differ\n", tag1,
						tag2));
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_ssl_server_context(struct soap * soap, unsigned short flags, const char * keyfile,
	...
	if(!(soap->ssl_flags&SOAP_SSL_NO_DEFAULT_CA_PATH)) {
		if(!SSL_CTX_set_default_verify_paths(soap->ctx))
			return soap_set_receiver_error(soap, "SSL/TLS error",
				"Can't read default CA file and/or directory",
				SOAP_SSL_ERROR);
	}
...
/* Suggested alternative approach to check the key file for certs (cafile=NULL):*/
	...
	if(!soap->cafile || !SSL_CTX_use_certificate_chain_file(soap->ctx, soap->cafile)) {
		if(soap->keyfile)                                                                                    {
			if(!SSL_CTX_use_certificate_chain_file(soap->ctx, soap->keyfile))
				return soap_set_receiver_error(soap, "SSL/TLS error",
					"Can't read certificate or key file",
					SOAP_SSL_ERROR);
			if(!SSL_CTX_use_PrivateKey_file(soap->ctx, soap->keyfile, SSL_FILETYPE_PEM))
				return soap_set_receiver_error(soap, "SSL/TLS error", "Can't read key file",
					SOAP_SSL_ERROR);
		}
	}
   #endif
	if((soap->ssl_flags&SOAP_SSL_RSA)) {
		...
		if(n >= 512 && s && *s == '\0')
			dh = DH_generate_parameters(n, 2 /*or 5*/, NULL, NULL);
		      ...
		      if(!bio)
			      return soap_set_receiver_error(soap, "SSL/TLS error", "Can't read DH file",
				      SOAP_SSL_ERROR);
	...
	if(!soap->xcred) {
		...
		if(soap->cafile) {
			if(gnutls_certificate_set_x509_trust_file(soap->xcred, soap->cafile, GNUTLS_X509_FMT_PEM) < 0)
				return soap_set_receiver_error(soap, "SSL/TLS error", "Can't read CA file",
					SOAP_SSL_ERROR);
		}
		if(soap->crlfile) {
			if(gnutls_certificate_set_x509_crl_file(soap->xcred, soap->crlfile, GNUTLS_X509_FMT_PEM) < 0)
				return soap_set_receiver_error(soap, "SSL/TLS error", "Can't read CRL file",
					SOAP_SSL_ERROR);
		}
		if(soap->keyfile) {
		...
		if(soap->cafile || soap->crlfile || soap->keyfile) {
			...
			if(ret < 0)
				return soap_set_receiver_error(soap, soap_ssl_error(soap,
						ret), "SSL/TLS set priority error", SOAP_SSL_ERROR);
	...
	if(!ok) {
		...
		fprintf(stderr, "SSL verify error or warning with certificate at depth %d: %s\n",
			X509_STORE_CTX_get_error_depth(
				store), X509_verify_cert_error_string(X509_STORE_CTX_get_error(store)));
...
SOAP_FMAC2 soap_ssl_accept(struct soap * soap)
{
	...
	if(r <= 0) {
		soap_set_receiver_error(soap, soap_ssl_error(soap,
				r), "SSL_accept() failed in soap_ssl_accept()", SOAP_SSL_ERROR);
		soap_closesock(soap);
...
again:
	...
	if(!soap_tag_cmp(endpoint, "https:*")) {
		...
		do {if((r = SSL_connect(soap->ssl)) <= 0)    {
			    int err = SSL_get_error(soap->ssl,
				    r);
			    if(err == SSL_ERROR_WANT_CONNECT || err == SSL_ERROR_WANT_READ || err ==
			       SSL_ERROR_WANT_WRITE) {
				    register int s;
				    if(err == SSL_ERROR_WANT_READ)
					    s = tcp_select(soap, fd, SOAP_TCP_SELECT_RCV|SOAP_TCP_SELECT_ERR, -100000);
				    ...
				    if(s < 0 && soap->errnum != SOAP_EINTR) {
					    DBGLOG(TEST,
						    SOAP_MESSAGE(fdebug, "SSL_connect/select error in tcp_connect\n"));
			    ...
			    else {soap_set_sender_error(soap, soap_ssl_error(soap,
		...
		if((soap->ssl_flags&SOAP_SSL_REQUIRE_SERVER_AUTHENTICATION)) {
			int err;
			if((err = SSL_get_verify_result(soap->ssl)) != X509_V_OK) {
				soap_set_sender_error(soap, X509_verify_cert_error_string(
						err),
	...
	for(;; ) {
		...
		if(soap_valid_socket(soap->socket)) {
			...
			if(setsockopt(soap->socket, IPPROTO_TCP, TCP_NODELAY, (char *)&set, sizeof(int))) {
				soap->errnum = soap_socket_errno(soap->socket);
				soap_set_receiver_error(soap, tcp_error(
						soap), "setsockopt TCP_NODELAY failed in soap_accept()", SOAP_TCP_ERROR);
				soap_closesock(soap);
				return SOAP_INVALID_SOCKET;
			}
   #endif
...
int http_parse(struct soap * soap)
{
	...
	*soap->endpoint = '\0';
	...
	soap->action = NULL;
	soap->status = 0;
	do {if(soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))    {
	...
	if(soap->length > 0 ||
	   (soap->http_content &&
	    (!soap->keep_alive || soap->recv_timeout)) || (soap->imode&SOAP_IO) == SOAP_IO_CHUNK) {
		if((soap->status > 200 && soap->status <=
		    299)
		   ||
		   soap->status == 400 ||
		   soap->status == 500)
...
static const char * soap_decode(char * buf, size_t len, const char * val, const char * sep)
{
	...
	else {while(*s && !soap_blank((soap_wchar)*s) && !strchr(sep, *s) && --len)      {
		      if(*s == '%')                                                                             {
	...
	if(!status || status == SOAP_HTML || status == SOAP_FILE) {
		...
		if(status == 401) {
			sprintf(soap->tmpbuf, "Basic realm=\"%s\"",
				(soap->authrealm && strlen(soap->authrealm) < sizeof(soap->tmpbuf)-14) ? soap->authrealm : "gSOAP Web Service");
		...
		const char * s = *soap_faultcode(soap);
	      ...
	      else if(soap->version == 2 && (!s || !strcmp(s, S_GetNsKeyw(S_SoapEnvNs, "Sender"))))
		      s = "400 Bad Request";
	      else
...
SOAP_FMAC2 soap_cookie(struct soap * soap, const char * name, const char * domain, const char * path)
{
	...
	for(p = soap->cookies; p; p = p->next) {
		DBGLOG(TEST,
			SOAP_MESSAGE(fdebug, "Cookie in database: %s=%s domain=%s path=%s env=%hd\n", p->name,
...
SOAP_FMAC1
struct soap_cookie *
SOAP_FMAC2 soap_set_cookie(struct soap * soap, const char * name, const char * value, const char * domain,
	const char * path)
{
...
SOAP_FMAC2 soap_clr_cookie(struct soap * soap, const char * name, const char * domain, const char * path)
{
	...
	for(p = &soap->cookies, q = *p; q; q = *p) {
		if(!strcmp(q->name,
			   name) &&
		   !strcmp(q->domain,
			   domain) && !strncmp(q->path, path, strlen(q->path)))                                             {
			if(q->value)
				SOAP_FREE(soap, q->value);
			if(q->domain)
				SOAP_FREE(soap, q->domain);
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_clr_cookie_session(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie * p;
	if((p = soap_cookie(soap, name, domain, path))) {
		p->session = 0;
...
SOAP_FMAC1
struct soap *
SOAP_FMAC2 soap_versioning(soap_new) (soap_mode imode, soap_mode omode)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_embed(struct soap * soap, const void * p, const struct soap_array * a, int n, const char * tag,
	int type)
{
	register int i;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_pointer_enter(struct soap * soap, const void * p, const struct soap_array * a, int n, int type,
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_array_pointer_lookup(struct soap * soap, const void * p, const struct soap_array * a, int n, int type,
	struct soap_plist ** ppp)
{
	register struct soap_plist * pp;
	*ppp = NULL;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_array_reference(struct soap * soap, const void * p, const struct soap_array * a, int n, int t)
{
	register int i;
	struct soap_plist * pp;
	if(!p || !a->__ptr)
	...
	if(i) {
		...
		return 1;
	      ...
	      pp->mark2 = 0; }
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "Array reference %p ptr=%p dim=%d type=%d (%d %d)\n", p, a->__ptr, n, t,
...
SOAP_FMAC2 soap_track_malloc(struct soap * soap, const char * file, int line, size_t size)
{
	...
	if(soap) {
		...
		if(soap->fdebug[SOAP_INDEX_TEST]) {
			DBGLOG(TEST,
				SOAP_MESSAGE(fdebug, "%s(%d): malloc(%lu) = %p\n", file, line, (unsigned long)size, p));
		}
		mp->next = soap->mht[h];
		mp->ptr = p;
		mp->file = file;
...
SOAP_FMAC1
void *
SOAP_FMAC2 soap_id_lookup(struct soap * soap, const char * id, void ** p, int t, size_t n, unsigned int k)
{
	...
	if(!ip) {
		if(!(ip = soap_enter(soap, id))) /* new hash table entry for string id */
			return NULL;
		DBGLOG(TEST,
			SOAP_MESSAGE(fdebug, "Forwarding first href='%s' type=%d %p (%u bytes)\n", id, t, p,
				(unsigned int)n));
		ip->type = t;
		ip->size = n;
		ip->link = p;
		...
		*p = NULL;
		...
		DBGLOG(TEST,
			SOAP_MESSAGE(fdebug, "Resolved href='%s' type=%d location=%p (%u bytes)\n", id, t, ip->ptr,
				(unsigned int)n));
		if(ip->type != t) {
			strcpy(soap->id, id);
...
SOAP_FMAC2 soap_end_recv(struct soap * soap)
{
	...
	if(soap->mode&SOAP_ENC_ZLIB) { /* Make sure end of compressed content is reached */
		...
		if(soap->zlib_in == SOAP_ZLIB_GZIP) {
			...
			if(soap->z_crc !=
			   ((uLong)(unsigned char)soap->z_buf[0]|
			    ((uLong)(unsigned char)soap->z_buf[1]<<
			     8)|
			    ((uLong)(unsigned char)soap->z_buf[2]<<
			     16)|((uLong)(unsigned char)soap->z_buf[3]<<24))) {
				DBGLOG(TEST,
					SOAP_MESSAGE(fdebug,
			...
			if(soap->d_stream->total_out !=
			   ((uLong)(unsigned char)soap->z_buf[4]|
			    ((uLong)(unsigned char)soap->z_buf[5]<<
			     8)|
			    ((uLong)(unsigned char)soap->z_buf[6]<<
			     16)|((uLong)(unsigned char)soap->z_buf[7]<<24))) {
...
SOAP_FMAC1
void
SOAP_FMAC2 soap_versioning(soap_init) (struct soap * soap, soap_mode imode, soap_mode omode)
{
	size_t i;
...
SOAP_FMAC2 soap_element(struct soap * soap, const char * tag, int id, const char * type)
{
	...
	if(!soap->ns) {
		...
		if(soap->ns == 1 &&
		   soap_send_raw(soap, soap_indent, soap->level <
			   sizeof(soap_indent) ? soap->level : sizeof(soap_indent)-1))
			return soap->error;
		soap->body = 1;
	}
	if((soap->mode&SOAP_XML_DEFAULTNS) && (s = strchr(tag, ':'))) {
		...
		for(; ns && ns->id; ns++) {
			if(*ns->id &&
			   (ns->out || ns->ns) && !strncmp(ns->id, tag, n) && !ns->id[n])                            {
	...
	if(soap->null && soap->position > 0) {
		...
		strcat(soap->tmpbuf, "]");
		if(soap_attribute(soap, S_GetNsKeyw(S_SoapEncNs, "position"), soap->tmpbuf))
			return soap->error;
	}
	if(soap->mustUnderstand) {
		if(soap->actor && *soap->actor) {
			if(soap_attribute(soap, soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "role") : S_GetNsKeyw(S_SoapEnvNs, "actor"), soap->actor))
				return soap->error;
		}
		if(soap_attribute(soap, S_GetNsKeyw(S_SoapEnvNs, "mustUnderstand"), soap->version == 2 ? "true" : "1"))
			return soap->error;
		soap->mustUnderstand = 0;
	}
	if(soap->encoding) {
		if(soap->encodingStyle && soap->local_namespaces) {
			...
			if(soap->encodingStyle && soap_attribute(soap, S_GetNsKeyw(S_SoapEnvNs, "encodingStyle"), soap->encodingStyle))
				return soap->error;
		}
		soap->encoding = 0;
	}
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_array_begin_out(struct soap * soap, const char * tag, int id, const char * type, const char * offset)
{
	if(!type || !*type)
		return soap_element_begin_out(soap, tag, id, NULL);
	if(soap_element(soap, tag, id, S_GetNsKeyw(S_SoapEncNs, "Array")))
		return soap->error;
	if(soap->version == 2) {
		const char * s;
		s = soap_strrchr(type, '[');
		if((size_t)(s-type) < sizeof(soap->tmpbuf)) {
			...
			if(soap_attribute(soap, S_GetNsKeyw(S_SoapEncNs, "itemType"), soap->tmpbuf))
				return soap->error;
			if(s && (soap_attribute(soap, S_GetNsKeyw(S_SoapEncNs, "arraySize"), s+1)))
				return soap->error;
		}
	}
	else {
		if(offset && soap_attribute(soap, S_GetNsKeyw(S_SoapEncNs, "offset"), offset))
		      return soap->error;
	      if(soap_attribute(soap, S_GetNsKeyw(S_SoapEncNs, "arrayType"), type))
		      return soap->error; 
	}
 #ifndef WITH_LEAN
	if((soap->mode&SOAP_XML_CANONICAL))
...
SOAP_FMAC2 soap_element_end_out(struct soap * soap, const char * tag)
{
	...
	if(soap->mode&SOAP_XML_INDENT) {
		if(!soap->body)                                   {
			if(soap_send_raw(soap, soap_indent, soap->level <
				   sizeof(soap_indent) ? soap->level : sizeof(soap_indent)-1))
				return soap->error;
		}
		soap->body = 0;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_element_href(struct soap * soap, const char * tag, int id, const char * ref, const char * val)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_element_id(struct soap * soap, const char * tag, int id, const void * p, const struct soap_array * a,
		...
		if((soap->mode&SOAP_XML_CANONICAL)) {
			...
			if(!strncmp(name, "xmlns", 5)) {
				...
				for(; *tpp; tpp = &(*tpp)->next)
			      ...
			      if(np)
				      tp->ns = np->ns;
			      else {struct soap_attribute * tq;
				    for(tq = soap->attributes; tq; tq = tq->next) {
					    if(!strncmp(tq->name, "xmlns:",
						       6) &&
					       !strncmp(tq->name+6, name,
						       s-name) &&
					       !tq->name[6+s-name])                                                {
						    tp->ns = tq->ns;
						    break;
...
SOAP_FMAC2 soap_append_lab(struct soap * soap, const char * s, size_t n)
{
	if(soap->labidx+n >= soap->lablen) {
		...
		DBGLOG(TEST,
			SOAP_MESSAGE(fdebug, "Enlarging look-aside buffer to append data, old size=%lu",
...
SOAP_FMAC2 soap_peek_element(struct soap * soap)
{
	...
	*s = '\0';
	...
	while((int)c != EOF && c != '>' && c != '/') {
		...
		*s = '\0';
		...
		if(!tp) {
			tp =
			        (struct soap_attribute *)SOAP_MALLOC(soap, sizeof(struct soap_attribute)+
					strlen(soap->tmpbuf));
			if(!tp)
				return soap->error = SOAP_EOM;
		...
		if(c == '=') {
			...
			if(soap_getattrval(soap, tp->value, tp->size, c)) {
				...
				if(tp->value)
					SOAP_FREE(soap, tp->value);
				for(;; ) {
					if(soap_getattrval(soap, soap->labbuf+soap->labidx, soap->lablen-soap->labidx,
						   c))          {
						if(soap->error != SOAP_EOM)
							return soap->error;
						soap->error = SOAP_OK;
				...
				if(soap->labidx)
					tp->size = soap->lablen;
				else {tp->size = strlen(soap->labbuf)+1;
	...
	for(tp = soap->attributes; tp; tp = tp->next) {
		if(tp->visible && tp->value)                                                {
			...
			if(!soap_match_tag(soap, tp->name, "xsi:type")) {
				...
				if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "arrayType"))) {
				...
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "offset")))
					strncpy(soap->arrayOffset, tp->value, sizeof(soap->arrayOffset));
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "position")))
					soap->position = soap_getposition(tp->value, soap->positions);
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "root")))
					soap->root = ((!strcmp(tp->value, "1") || !strcmp(tp->value, "true")));
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEnvNs, "mustUnderstand")) && (!strcmp(tp->value, "1") || !strcmp(tp->value, "true")))
					soap->mustUnderstand = 1;
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEnvNs, "actor"))) {
 ...
 #ifndef WITH_NOIDREF
				if(!strcmp(tp->name, "ref") || !soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "ref"))) {
					*soap->href = '#';
 ...
 #endif
				if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "itemType")))
					strncpy(soap->arrayType, tp->value, sizeof(soap->arrayType)-1);
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEncNs, "arraySize")))
					strncpy(soap->arraySize, tp->value, sizeof(soap->arraySize)-1);
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEnvNs, "mustUnderstand")) && (!strcmp(tp->value, "1") || !strcmp(tp->value, "true")))
					soap->mustUnderstand = 1;
				else if(!soap_match_tag(soap, tp->name, S_GetNsKeyw(S_SoapEnvNs, "role"))) {
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outint(struct soap * soap, const char * tag, int id, const int * p, const char * type, int n)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outlong(struct soap * soap, const char * tag, int id, const long * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outLONG64(struct soap * soap, const char * tag, int id, const LONG64 * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outbyte(struct soap * soap, const char * tag, int id, const char * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
	   soap_string_out(soap, soap_long2s(soap, (long)*p), 0))
		return soap->error;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outshort(struct soap * soap, const char * tag, int id, const short * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
	   soap_string_out(soap, soap_long2s(soap, (long)*p), 0))
		return soap->error;
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outfloat(struct soap * soap, const char * tag, int id, const float * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outdouble(struct soap * soap, const char * tag, int id, const double * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
unsigned char *
SOAP_FMAC2 soap_inunsignedByte(struct soap * soap, const char * tag, unsigned char * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outunsignedShort(struct soap * soap, const char * tag, int id, const unsigned short * p,
	const char * type,
	int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
unsigned short *
SOAP_FMAC2 soap_inunsignedShort(struct soap * soap, const char * tag, unsigned short * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outunsignedInt(struct soap * soap, const char * tag, int id, const unsigned int * p, const char * type,
	int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
	   soap_string_out(soap, soap_unsignedLong2s(soap, (unsigned long)*p), 0))
...
SOAP_FMAC1
unsigned int *
SOAP_FMAC2 soap_inunsignedInt(struct soap * soap, const char * tag, unsigned int * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1
unsigned long *
SOAP_FMAC2 soap_inunsignedLong(struct soap * soap, const char * tag, unsigned long * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outULONG64(struct soap * soap, const char * tag, int id, const ULONG64 * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outstring(struct soap * soap, const char * tag, int id, char * const * p, const char * type, int n)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outwstring(struct soap * soap, const char * tag, int id, wchar_t * const * p, const char * type, int n)
{
	id = soap_element_id(soap, tag, id, *p, NULL, 0, type, n);
	if(id < 0)
		return soap->error;
...
SOAP_FMAC1
wchar_t **
SOAP_FMAC2 soap_inwstring(struct soap * soap, const char * tag, wchar_t ** p, const char * type, int t, long minlen,
	long maxlen)
{
	(void)type;
	if(soap_element_begin_in(soap, tag, 1, NULL)) {
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_outdateTime(struct soap * soap, const char * tag, int id, const time_t * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) ||
...
static size_t soap_count_attachments(struct soap * soap)
{
	...
	if((soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM)) {
		...
		for(content = soap->dime.first; content; content = content->next) {
			...
			if(content->options)
				count +=
				        ((((unsigned char)content->options[2]<<
				           8)|((unsigned char)content->options[3]))+7)&(~3);
			DBGLOG(TEST,
...
SOAP_FMAC2 soap_putdime(struct soap * soap)
{
	...
	for(content = soap->dime.first; content; content = content->next) {
		...
		if(soap->fdimereadopen &&
		   ((handle =
		             soap->fdimereadopen(soap, (void *)content->ptr, content->id, content->type,
...
SOAP_FMAC2 soap_getdimehdr(struct soap * soap)
{
	...
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "DIME size=%lu flags=0x%X\n", (unsigned long)soap->dime.size, soap->dime.flags));
	...
	if(!(soap->dime.type = soap_getdimefield(soap, typelen)) && soap->error)
		return soap->error;
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "DIME id=%s, type=%s, options=%s\n", soap->dime.id ? soap->dime.id : SOAP_STR_EOS,
			soap->dime.type ? soap->dime.type : "", soap->dime.options ? soap->dime.options+
			4 : SOAP_STR_EOS));
...
SOAP_FMAC2 soap_getdime(struct soap * soap)
{
	...
	for(;; ) {
		...
		if(soap->fdimewriteopen &&
		   ((soap->dime.ptr =
		             (char *)soap->fdimewriteopen(soap, soap->dime.id, soap->dime.type,
			...
			for(;; ) {
				...
				for(;; ) {
					...
					if(size < n)
						n = size;
					if((soap->error =
					            soap->fdimewrite(soap, (void *)soap->dime.ptr, soap->buf+
							    soap->bufidx, n)))
						break;
					size -= n;
					if(!size) {
...
SOAP_FMAC2 soap_getmimehdr(struct soap * soap)
{
	struct soap_multipart * content;
	do {if(soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
		    return soap->error; } while(!*soap->msgbuf);
		...
		if(val) {
			*val = '\0';
			...
			if(!soap_tag_cmp(key, "Content-ID"))
				...
				content->encoding = (enum soap_mime_encoding)soap_code_int(mime_codes, val,
					(long)SOAP_MIME_NONE);
		}
...
SOAP_FMAC2 soap_get_mime_attachment(struct soap * soap, void * handle)
{
	...
	if(!content) {
		...
		if(soap->fmimewriteopen &&
		   ((content->ptr =
		             (char *)soap->fmimewriteopen(soap, (void *)handle, content->id, content->type,
				     content->description,
				     content->encoding)) || soap->error))                                        {
			if(!content->ptr)
				return NULL;
		}
	}
	DBGLOG(TEST,
		SOAP_MESSAGE(fdebug, "Parsing MIME content id=%s type=%s\n", content->id ? content->id : SOAP_STR_EOS,
	...
	*s = '\0'; /* force 0-terminated */
	...
	else {content->size = soap_size_block(soap, NULL, i+1)-1;
	...
	else {while(c != '\r' && (int)c != EOF && soap_blank(c))
		      c = soap_getchar(soap);
	...
	if(content->id) {
		...
		while(*xp) {
			...
			if(!soap_match_cid(soap, xq->id, content->id)) {
				DBGLOG(TEST,
					SOAP_MESSAGE(fdebug, "Found matching attachment %s for content id=%s\n", xq->id,
						content->id));
				*xp = xq->next;
...
SOAP_FMAC2 soap_putmime(struct soap * soap)
{
	...
	for(content = soap->mime.first; content; content = content->next) {
		...
		if(soap->fmimereadopen &&
		   ((handle =
		             soap->fmimereadopen(soap, (void *)content->ptr, content->id, content->type,
			...
			if(!size) {
				if((soap->mode&SOAP_ENC_XML) || (soap->mode&SOAP_IO) == SOAP_IO_CHUNK ||
				   (soap->mode&SOAP_IO) == SOAP_IO_STORE)            {
					...
					do {size = soap->fmimeread(soap, handle, soap->tmpbuf, sizeof(soap->tmpbuf));
					    DBGLOG(TEST,
						    SOAP_MESSAGE(fdebug, "fmimeread returned %lu bytes\n",
							    (unsigned long)size));
		...
		else {if(soap_putmimehdr(soap, content)      ||
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_set_dime_attachment(struct soap * soap, char * ptr, size_t size, const char * type, const char * id,
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_set_mime_attachment(struct soap * soap, char * ptr, size_t size, enum soap_mime_encoding encoding,
...
SOAP_FMAC2 soap_envelope_begin_out(struct soap * soap)
{
	...
	size_t n = 0;
	if((soap->mode&
	    SOAP_ENC_MIME) && soap->mime.boundary && soap->mime.start && strlen(soap->mime.boundary)+
	...
	return soap_element_begin_out(soap, S_GetNsKeyw(S_SoapEnvNs, "Envelope"), 0, NULL);
}
...
{
	if(soap_element_end_out(soap, S_GetNsKeyw(S_SoapEnvNs, "Envelope")) || soap_send_raw(soap, "\r\n", 2)) /* 2.8: always emit \r\n */
		return soap->error;
 #ifndef WITH_LEANER
	if((soap->mode&SOAP_IO_LENGTH) && (soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM)) {
		...
		soap->count += 12+
		               ((strlen(soap->dime.id)+
		                 3)&(~3))+(soap->dime.type ? ((strlen(soap->dime.type)+3)&(~3)) : 0);
	}
	if((soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM))
...
SOAP_FMAC1
char *
SOAP_FMAC2 soap_get_http_body(struct soap * soap)
{
  #ifndef WITH_LEAN
...
SOAP_FMAC2 soap_envelope_begin_in(struct soap * soap)
{
	register struct Namespace * p;
	soap->part = SOAP_IN_ENVELOPE;
	if(soap_element_begin_in(soap, S_GetNsKeyw(S_SoapEnvNs, "Envelope"), 0, NULL)) {
...
{
	soap->part = SOAP_END_ENVELOPE;
	return soap_element_end_in(soap, S_GetNsKeyw(S_SoapEnvNs, "Envelope"));
}
...
SOAP_FMAC2 soap_body_begin_out(struct soap * soap)
{
	...
	if((soap->mode&SOAP_SEC_WSUID) && soap_set_attr(soap, "wsu:Id", "Body", 1))
		return soap->error;
 #endif
	return soap_element_begin_out(soap, S_GetNsKeyw(S_SoapEnvNs, "Body"), 0, NULL);
}
...
{
	if(soap_element_end_out(soap, S_GetNsKeyw(S_SoapEnvNs, "Body")))
		return soap->error;
...
{
	soap->part = SOAP_IN_BODY;
	if(soap_element_begin_in(soap, S_GetNsKeyw(S_SoapEnvNs, "Body"), 0, NULL))
		return soap->error;
	if(!soap->body)
...
SOAP_FMAC2 soap_body_end_in(struct soap * soap)
{
	if(soap->part == SOAP_NO_BODY)
		return soap->error = SOAP_OK;
	soap->part = SOAP_END_BODY;
	return soap_element_end_in(soap, S_GetNsKeyw(S_SoapEnvNs, "Body"));
}
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_connect_command(struct soap * soap, int http_command, const char * endpoints, const char * action)
{
	char * endpoint;
	const char * s;
	...
	if(soap->fopen && *soap->host) {
		if(!soap->keep_alive || !soap_valid_socket(soap->socket) ||
		   ...
		   soap->fpoll(soap))                                 {
			...
			soap_closesock(soap);
			DBGLOG(TEST,
				SOAP_MESSAGE(fdebug, "Connect/reconnect to host='%s' path='%s' port=%d\n", soap->host,
					soap->path, soap->port));
			if(!soap->keep_alive || !soap_valid_socket(soap->socket)) {
...
SOAP_FMAC2 soap_puthttphdr(struct soap * soap, int status, size_t count)
{
	if(soap->status != SOAP_GET && soap->status != SOAP_DEL && soap->status != SOAP_CONNECT) {
		...
		if((status == SOAP_FILE || soap->status == SOAP_PUT ||
		    soap->status == SOAP_POST_FILE) && soap->http_content)
			s = soap->http_content;
...
{
	if(*soap->tag)
		sprintf(soap->msgbuf, "Validation constraint violation: %s%s in element '%s'", s, t ? t : SOAP_STR_EOS,
			soap->tag);
	else
...
SOAP_FMAC2 soap_set_fault(struct soap * soap)
{
	...
	if(!*c) {
		if(soap->version == 2)
			*c = S_GetNsKeyw(S_SoapEnvNs, "Sender");
		else
			*c = S_GetNsKeyw(S_SoapEnvNs, "Client");
	}
	if(*s)
		return;
	switch(soap->error) {
		...
		*s = soap_set_validation_fault(soap, "multiple definitions (use the SOAP_XML_TREE flag) of the same id ", soap->id);
		if(soap->version == 2)
			*soap_faultsubcode(soap) = S_GetNsKeyw(S_SoapEncNs, "DuplicateID");
		break;
	    case SOAP_MISSING_ID:
		*s = soap_set_validation_fault(soap, "missing id for ref ", soap->id);
		if(soap->version == 2)
			*soap_faultsubcode(soap) = S_GetNsKeyw(S_SoapEncNs, "MissingID");
		break;
	    case SOAP_HREF:
...
SOAP_FMAC2 soap_recv_fault(struct soap * soap, int check)
{
	...
	if(soap_getfault(soap)) { /* check flag set: check if SOAP Fault is present, if not just return */
		...
		*soap_faultcode(soap) = (soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "Sender") : S_GetNsKeyw(S_SoapEnvNs, "Client"));
		soap->error = status;
		soap_set_fault(soap);
	}
	else {
		register const char * s = *soap_faultcode(soap);
	      if(!soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "Server")) || !soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "Receiver")))
		      status = SOAP_SVR_FAULT;
	      else if(!soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "Client")) || !soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "Sender")))
		      status = SOAP_CLI_FAULT;
	      else if(!soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "MustUnderstand")))
		      status = SOAP_MUSTUNDERSTAND;
	      else if(!soap_match_tag(soap, s, S_GetNsKeyw(S_SoapEnvNs, "VersionMismatch")))
		...
		*soap->msgbuf = '\0';
		len =
				...
				MAKELANGID(LANG_NEUTRAL,
					SUBLANG_DEFAULT), (LPSTR)soap->msgbuf, (DWORD)sizeof(soap->msgbuf), NULL);
   #else
		DWORD i, len;
		*soap->msgbuf = '\0';
		len =
		        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM|FORMAT_MESSAGE_IGNORE_INSERTS, NULL, err, 0,
				(LPTSTR)soap->msgbuf, (DWORD)(sizeof(soap->msgbuf)/sizeof(TCHAR)),
				NULL);
		for(i = 0; i <= len; i++) {
...
#ifndef PALM_2
SOAP_FMAC1
int SOAP_FMAC2 soap_set_sender_error(struct soap * soap, const char * faultstring, const char * faultdetailXML, int soaperror)
{
	return soap_set_error(soap, soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "Sender") : S_GetNsKeyw(S_SoapEnvNs, "Client"), NULL, faultstring,
...
#ifndef PALM_2
SOAP_FMAC1
int SOAP_FMAC2 soap_set_receiver_error(struct soap * soap, const char * faultstring, const char * faultdetailXML, int soaperror)
{
	return soap_set_error(soap, soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "Receiver") : S_GetNsKeyw(S_SoapEnvNs, "Server"), NULL, faultstring,
		faultdetailXML, soaperror);
}
...
int SOAP_FMAC2 soap_sender_fault_subcode(struct soap * soap, const char * faultsubcodeQName, const char * faultstring,
	const char * faultdetailXML)
{
	return soap_copy_fault(soap, soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "Sender") : S_GetNsKeyw(S_SoapEnvNs, "Client"), faultsubcodeQName,
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_receiver_fault(struct soap * soap, const char * faultstring, const char * faultdetailXML)
{
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_receiver_fault_subcode(struct soap * soap, const char * faultsubcodeQName, const char * faultstring,
	const char * faultdetailXML)
{
	return soap_copy_fault(soap, soap->version == 2 ? S_GetNsKeyw(S_SoapEnvNs, "Receiver") : S_GetNsKeyw(S_SoapEnvNs, "Server"), faultsubcodeQName,
		faultstring, faultdetailXML);
}
...
SOAP_FMAC1
int
SOAP_FMAC2 soap_register_plugin_arg(struct soap * soap, int (* fcreate)(struct soap *, struct soap_plugin *,
		void *), void * arg)
{
