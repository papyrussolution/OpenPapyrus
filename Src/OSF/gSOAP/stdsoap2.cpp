...
const char * S_SoapEnvNs = "SOAP-ENV";
const char * S_SoapEncNs = "SOAP-ENC";

const char * S_GetNsKeyw(const char * pNs, const char * pKeyw)
{
...
FILE * FASTCALL soap_get_dbg_file(struct soap * pSoap, int idx)
{
	if(pSoap) {
		if(!pSoap->fdebug[idx])
			soap_open_logfile(pSoap, idx);
		return pSoap->fdebug[idx];
	}
	else
		return 0;
}
...
#else
	#define soap_hash_ptr(p)        ((size_t)(((ulong)(p)>>3)&(SOAP_PTRHASH-1)))
#endif
...
static soap_wchar FASTCALL soap_char(struct soap *);
static soap_wchar soap_get_pi(struct soap *);
//static int FASTCALL soap_isxdigit(int);
...
static int FASTCALL soap_putdimefield(struct soap *, const char *, size_t);
static char * FASTCALL soap_getdimefield(struct soap *, size_t);
static void soap_select_mime_boundary(struct soap *);
static int soap_valid_mime_boundary(struct soap *);
static void FASTCALL soap_resolve_attachment(struct soap *, struct soap_multipart *);
	#endif
#endif
#ifdef WITH_GZIP
...
GCRY_THREAD_OPTION_PTH_IMPL;
	...
	#ifndef PALM_1
		static const char * FASTCALL soap_decode(char *, size_t, const char *, const char *);
	#endif
#endif
#ifndef WITH_NOHTTP
	#ifndef PALM_1
		...
		static const char * FASTCALL tcp_error(struct soap *);
		#ifndef WITH_IPV6
		...
		static int FASTCALL tcp_select(struct soap *, SOAP_SOCKET, int, int);
	...
	#if defined(WIN32)
		static void FASTCALL _soap_sockblock(SOCKET fd)
		{
			u_long blocking = 0; 
			ioctlsocket(fd, FIONBIO, &blocking);
		}
		static void FASTCALL _soap_socknonblock(SOCKET fd)
		{
			u_long nonblocking = 1; 
			ioctlsocket(fd, FIONBIO, &nonblocking);
		}
		//#define SOAP_SOCKBLOCK(fd)    { u_long blocking = 0; ioctlsocket(fd, FIONBIO, &blocking); }
		//#define SOAP_SOCKNONBLOCK(fd) { u_long nonblocking = 1; ioctlsocket(fd, FIONBIO, &nonblocking); }
		#define SOAP_SOCKBLOCK(fd)    _soap_sockblock(fd);
		#define SOAP_SOCKNONBLOCK(fd) _soap_socknonblock(fd);
	#elif defined(VXWORKS)
		#define SOAP_SOCKBLOCK(fd) { u_long blocking = 0; ioctl(fd, FIONBIO, (int)(&blocking)); }
		#define SOAP_SOCKNONBLOCK(fd)	{ u_long nonblocking = 1; ioctl(fd, FIONBIO, (int)(&nonblocking)); }
	#elif defined(__VMS)
...
static int fsend(struct soap * soap, const char * s, size_t n)
{
		...
		if(soap_valid_socket(soap->socket)) {
			if(soap->send_timeout) {
				for(;; ) {
					...
					if(!err)
						return soap->error;
					if(!oneof3(err, SOAP_EINTR, SOAP_EAGAIN, SOAP_EWOULDBLOCK))
						return SOAP_EOF;
				}
			}
  #ifdef WITH_OPENSSL
			...
			if((soap->omode&SOAP_IO_UDP)) {
				...
				if(nwritten < 0) {
					...
					udp_delay = ((uint)soap_random%201)+50; /* UDP_MIN_DELAY .. UDP_MAX_DELAY */
					do {
  ...
  #endif
				if(oneof2(err, SOAP_EWOULDBLOCK, SOAP_EAGAIN)) {
    ...
    #else
     #ifdef WIN32
			nwritten = _write(soap->sendfd, s, (uint)n);
     #else
			nwritten = write(soap->sendfd, s, (uint)n);
     #endif
    #endif
   #endif
  #endif
			if(nwritten <= 0) {
				...
				err = EOF;
  #endif
				if(err && !oneof3(err, SOAP_EINTR, SOAP_EWOULDBLOCK, SOAP_EAGAIN)) {
					soap->errnum = err;
					return SOAP_EOF;
				}
				nwritten = 0; /* and call write() again */
...
#ifndef PALM_1
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_send_raw(struct soap * soap, const char * s, size_t n)
{
	if(!n)
		return SOAP_OK;
 #ifndef WITH_LEANER
...
SOAP_FMAC1 int SOAP_FMAC2 soap_flush(struct soap * soap)
{
	...
	if(n) {
		...
		if(soap->mode&SOAP_ENC_ZLIB) {
			...
			soap->d_stream->avail_in = (uint)n;
  #ifdef WITH_GZIP
			soap->z_crc = crc32(soap->z_crc, (Byte *)soap->buf, (uint)n);
  #endif
			do {
...
{
	if((soap->mode&SOAP_IO) == SOAP_IO_STORE) {
		char * t = static_cast<char *>(soap_push_block(soap, NULL, n));
		if(!t)
	...
	if((soap->mode&SOAP_IO) == SOAP_IO_CHUNK) {
		char t[16];
		sprintf(t, "\r\n%lX\r\n"+(soap->chunksize ? 0 : 2), (ulong)n);
		DBGMSG(SENT, t, sstrlen(t));
		if((soap->error = soap->fsend(soap, t, sstrlen(t))))
			return soap->error;
		soap->chunksize += n;
	}
	DBGMSG(SENT, s, n);
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_send(struct soap * soap, const char * s)
{
	return s ? soap_send_raw(soap, s, sstrlen(s)) : SOAP_OK;
}
#endif
...
static size_t frecv(struct soap * soap, char * s, size_t n)
{
		...
		for(;; ) {
  ...
  #endif
			{
				...
				if((soap->omode&SOAP_IO_UDP)) {
					...
					memzero(&soap->peer, sizeof(soap->peer));
			...
			if(r < 0) {
				r = soap->errnum;
				if(!oneof3(r, SOAP_EINTR, SOAP_EAGAIN, SOAP_EWOULDBLOCK))
					return 0;
			}
			if(retries-- <= 0)
				return 0;
    ...
    #endif
    #ifdef WIN32
	r = _read(soap->recvfd, s, (uint)n);
    #else
	r = read(soap->recvfd, s, (uint)n);
    #endif
	if(r >= 0)
		return (size_t)r;
		...
		if(soap->d_stream->avail_in || !soap->d_stream->avail_out) {
			...
			if(r == Z_OK || r == Z_STREAM_END) {
				...
				if(soap->zlib_in == SOAP_ZLIB_GZIP)
					soap->z_crc = crc32(soap->z_crc, (Byte *)soap->buf, (uint)ret);
				if(r == Z_STREAM_END) {
...
zlib_again:
	...
	if((soap->mode&SOAP_IO) == SOAP_IO_CHUNK) { /* read HTTP chunked transfer */
		for(;; ) {
			...
			DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Getting chunk size (idx=%u len=%u)\n", (uint)soap->bufidx, (uint)soap->buflen));
			while(!ishex((int)(c = soap_getchunkchar(soap)))) {
				if((int)c == EOF) {
					soap->ahead = EOF;
					return EOF;
				}
			...
			do {
				*t++ = (char)c;
			} while(ishex((int)(c = soap_getchunkchar(soap))) && (size_t)(t-tmp) < sizeof(tmp)-1);
			while((int)c != EOF && c != '\n')
 ...
 #endif
	{
		...
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Read %u bytes from socket=%d/fd=%d\n", (uint)ret, soap->socket, soap->recvfd));
		DBGMSG(RECV, soap->buf, ret);
	}
 #ifdef WITH_ZLIB
	if(soap->mode&SOAP_ENC_ZLIB) {
		...
		soap->d_stream->avail_in = (uint)ret;
		soap->d_stream->next_out = (Byte *)soap->buf;
		soap->d_stream->avail_out = SOAP_BUFLEN;
		r = inflate(soap->d_stream, Z_NO_FLUSH);
		if(r == Z_NEED_DICT && soap->z_dict)
		...
		if(r == Z_OK || r == Z_STREAM_END) {
			...
			if(soap->zlib_in == SOAP_ZLIB_GZIP)
				soap->z_crc = crc32(soap->z_crc, (Byte *)soap->buf, (uint)soap->buflen);
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_recv(struct soap * soap)
{
 #ifndef WITH_LEANER
...
SOAP_FMAC1 soap_wchar /*SOAP_FMAC2*/FASTCALL soap_getchar(struct soap * soap)
{
...
SOAP_FMAC1 const struct soap_code_map * /*SOAP_FMAC2*/FASTCALL soap_code(const struct soap_code_map * code_map, const char * str)
{
	if(code_map && str) {
		while(code_map->string) {
			if(sstreq(str, code_map->string)) /* case sensitive */
				return code_map;
			code_map++;
		}
	}
...
SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_code_str(const struct soap_code_map * code_map, long code)
{
	if(!code_map)
		return NULL;
	else {
	...
	if(code_map) {
		while(str && *str) {
			...
			for(p = code_map; p->string; p++) {
				size_t n = sstrlen(p->string);
				if(memcmp(p->string, str, n) == 0 && soap_blank((soap_wchar)str[n])) {
					bits |= p->code;
					str += n;
					while(*str > 0 && *str <= 32)
						str++;
...
static soap_wchar FASTCALL soap_char(struct soap * soap)
{
	char tmp[8];
	char * s = tmp;
	for(int i = 0; i < 7; i++) {
	...
	*s = '\0';
	if(*tmp == '#')
		return (tmp[1] == 'x' || tmp[1] == 'X') ? (soap_wchar)soap_strtol(tmp+2, NULL, 16) : (soap_wchar)soap_strtol(tmp+1, NULL, 10);
	else if(sstreq(tmp, "lt"))
		return '<';
	else if(sstreq(tmp, "gt"))
		return '>';
	else if(sstreq(tmp, "amp"))
		return '&';
	else if(sstreq(tmp, "quot"))
		return '"';
	else if(sstreq(tmp, "apos"))
		return '\'';
	else {
 #ifndef WITH_LEAN
...
{
	return (soap->bufidx >= soap->buflen && soap_recv(soap)) ? EOF : (uchar)soap->buf[soap->bufidx];
}
...
#ifndef PALM_1
SOAP_FMAC1 soap_wchar /*SOAP_FMAC2*/FASTCALL soap_get(struct soap * soap)
{
	soap_wchar c = soap->ahead;
	if(c) {
		if((int)c != EOF)
	...
	while((int)c != EOF) {
		...
		switch(c) {
			...
			case '"': return SOAP_QT;
			case '\'': return SOAP_AP;
			case '&': return soap_char(soap)|0x80000000;
		}
		break;
	}
	return c;
...
static soap_wchar soap_get_pi(struct soap * soap)
{
	...
	*s = '\0';
	...
	return c;
...
#ifndef WITH_LEANER
	#ifndef PALM_1
		SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_move(struct soap * soap, long n)
		{
		...
		{
			return pSoap->count - pSoap->buflen + pSoap->bufidx - (pSoap->ahead != 0);
		}
	#endif
#endif
#ifndef PALM_1
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_pututf8(struct soap * soap, ulong c)
{
	char tmp[16];
	if(c < 0x80 && c > 0) {
		*tmp = (char)c;
...
SOAP_FMAC1 soap_wchar /*SOAP_FMAC2*/FASTCALL soap_getutf8(struct soap * soap)
{
...
SOAP_FMAC1 int SOAP_FMAC2 soap_puthex(struct soap * soap, const uchar * s, int n)
{
	char d[2];
	int i;
 #ifdef WITH_DOM
...
SOAP_FMAC1 uchar * SOAP_FMAC2 soap_gethex(struct soap * soap, int * n)
{
 #ifdef WITH_DOM
	if((soap->mode&SOAP_XML_DOM) && soap->dom) {
...
SOAP_FMAC1 int SOAP_FMAC2 soap_putbase64(struct soap * soap, const uchar * s, int n)
{
	int i;
	ulong m;
	char d[4];
...
SOAP_FMAC1 uchar * SOAP_FMAC2 soap_getbase64(struct soap * soap, int * n, int malloc_flag)
{
	(void)malloc_flag;
 #ifdef WITH_DOM
	if((soap->mode&SOAP_XML_DOM) && soap->dom) {
			...
			for(i = 0; i < k-2; i += 3) {
				...
				do {
				    ...
				    if(c == '=' || c < 0) {
					    ...
					    p = static_cast<uchar *>(soap_malloc(soap, soap->lablen+i-k));
					    if(p)
						    memcpy(p, soap->labbuf, soap->lablen+i-k);
					    if(c >= 0) {
...
#ifndef WITH_LEANER
 #ifndef PALM_1
SOAP_FMAC1 int SOAP_FMAC2 soap_xop_forward(struct soap * soap, uchar ** ptr, int * size, char ** id, char ** type, char ** options)
...
SOAP_FMAC1 int SOAP_FMAC2 soap_dime_forward(struct soap * soap, uchar ** ptr, int * size, char ** id, char ** type, char ** options)
{
	...
	*id = soap_strdup(soap, soap->href);
	xp = static_cast<struct soap_xlist *>(SOAP_MALLOC(soap, sizeof(struct soap_xlist)));
	if(!xp)
		return soap->error = SOAP_EOM;
	xp->next = soap->xlist;
	xp->ptr = ptr;
...
#ifndef PALM_1
SOAP_FMAC1 char * /*SOAP_FMAC2*/FASTCALL soap_strdup(struct soap * soap, const char * s)
{
	char * t = NULL;
	if(s && (t = static_cast<char *>(soap_malloc(soap, sstrlen(s)+1))))
		strcpy(t, s);
	return t;
}
...
SOAP_FMAC1 wchar_t * SOAP_FMAC2 soap_wstrdup(struct soap * soap, const wchar_t * s)
{
	...
	if(s) {
		...
		while(s[n])
			n++;
		if((t = static_cast<wchar_t *>(soap_malloc(soap, sizeof(wchar_t)*(n+1)))))
			memcpy(t, s, sizeof(wchar_t)*(n+1));
	}
	return t;
}

SOAP_FMAC1 struct soap_blist * /*SOAP_FMAC2*/FASTCALL soap_new_block(struct soap * soap)
{
	struct soap_blist * p;
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "New block sequence (prev=%p)\n", soap->blist));
	if(!(p = static_cast<struct soap_blist *>(SOAP_MALLOC(soap, sizeof(struct soap_blist))))) {
		soap->error = SOAP_EOM;
		return NULL;
	}
	p->next = soap->blist;
...
SOAP_FMAC1 void * /*SOAP_FMAC2*/FASTCALL soap_push_block(struct soap * soap, struct soap_blist * b, size_t n)
{
	char * p;
	SETIFZ(b, soap->blist);
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Push block of %u bytes (%u bytes total)\n", (uint)n, (uint)b->size+ (uint)n));
	if(!(p = (char *)SOAP_MALLOC(soap, n+sizeof(char *)+sizeof(size_t)))) {
		soap->error = SOAP_EOM;
		return NULL;
	}
	else {
		*reinterpret_cast<char **>(p) = b->ptr;
		*reinterpret_cast<size_t *>(p+sizeof(char *)) = n;
		b->ptr = p;
		b->size += n;
...
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_pop_block(struct soap * soap, struct soap_blist * b)
{
	SETIFZ(b, soap->blist);
	if(b->ptr) {
			...
			for(ip = soap->iht[i]; ip; ip = ip->next) {
				if(ip->ptr && !soap_has_copies(soap, (const char *)ip->ptr, (const char *)ip->ptr+ip->size)) {
					...
					for(fp = ip->flist; fp; fp = ip->flist) {
						...
						while(ip->level < k) {
							void ** q = static_cast<void **>(soap_malloc(soap, sizeof(void *)));
							if(!q)
								return soap->error;
							*q = p;
...
#ifndef PALM_1
SOAP_FMAC1 size_t /*SOAP_FMAC2*/FASTCALL soap_size_block(struct soap * soap, struct soap_blist * b, size_t n)
{
	SETIFZ(b, soap->blist);
	if(b->ptr) {
...
SOAP_FMAC1 char * /*SOAP_FMAC2*/FASTCALL soap_first_block(struct soap * soap, struct soap_blist * b)
{
	char * p, * q, * r;
	SETIFZ(b, soap->blist);
	p = b->ptr;
...
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_end_block(struct soap * soap, struct soap_blist * b)
{
	...
	if(b) {
		...
		if(soap->blist == b)
			...
			for(struct soap_blist * bp = soap->blist; bp; bp = bp->next) {
			      if(bp->next == b) {
				      bp->next = b->next;
				      break;
			      }
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_save_block(struct soap * soap, struct soap_blist * b, char * p, int flag)
{
	SETIFZ(b, soap->blist);
	if(b->size) {
		SETIFZ(p, static_cast<char *>(soap_malloc(soap, b->size)));
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Save all blocks in contiguous memory space of %u bytes (%p->%p)\n", (uint) b->size, b->ptr, p));
		if(p) {
			for(char * s = p, * q = soap_first_block(soap, b); q; q = soap_next_block(soap, b)) {
				size_t n = soap_block_size(soap, b);
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_putsizesoffsets(struct soap * soap, const char * type, const int * size, const int * offset, int dim)
{
	...
	if(soap->version == 2) {
		...
		for(i = 1; i < dim; i++)
			sprintf(soap->type+sstrlen(soap->type), " %d", size[i]);
	}
	else {
		if(offset) {
		      sprintf(soap->type, "%s[%d", type, size[0]+offset[0]);
		      for(i = 1; i < dim; i++)
			      sprintf(soap->type+sstrlen(soap->type), ",%d", size[i]+offset[i]);
	      }
	      else {
			  sprintf(soap->type, "%s[%d", type, size[0]);
				for(i = 1; i < dim; i++)
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_putoffsets(struct soap * soap, const int * offset, int dim)
{
	...
	for(int i = 1; i < dim; i++)
		sprintf(soap->arrayOffset+sstrlen(soap->arrayOffset), ",%d", offset[i]);
...
SOAP_FMAC1 struct soap_nlist * /*SOAP_FMAC2*/FASTCALL soap_push_namespace(struct soap * soap, const char * id, const char * ns)
{
	...
	short i = -1;
	size_t n = sstrlen(id);
	size_t k = sstrlen(ns)+1;
	struct Namespace * p = soap->local_namespaces;
	if(p) {
		for(i = 0; p->id; p++, i++) {
			if(p->ns && sstreq(ns, p->ns)) {
				if(p->out) {
...
SOAP_FMAC1 int SOAP_FMAC2 soap_match_namespace(struct soap * soap, const char * id1, const char * id2, size_t n1, size_t n2)
{
	...
	if(np) {
		if(!(soap->mode&SOAP_XML_IGNORENS))
			if(np->index < 0 || ((s = soap->local_namespaces[np->index].id) && (strncmp(s, id2, n2) || (s[n2] && s[n2] != '_'))))
	...
	if(t) {
		if(s) {
			...
			if(t != tag2 && (err = soap_match_namespace(soap, tag1, tag2, s-tag1, t-tag2))) {
				...
				return (err == SOAP_NAMESPACE) ? SOAP_TAG_MISMATCH : err;
			}
		}
...
#ifndef PALM_2
SOAP_FMAC1 int SOAP_FMAC2 soap_ssl_server_context(struct soap * soap, ushort flags, const char * keyfile,
	const char * password, const char * cafile, const char * capath, const char * dhfile, const char * randfile, const char * sid)
{
	int err;
	...
	if(dhfile) {
		...
		if(n >= 512 && s && *s == '\0')
			gnutls_dh_params_generate2(soap->dh_params, (uint)n);
		else {
			unsigned int dparams_len;
		      uchar dparams_buf[1024];
		      FILE * fd = fopen(dhfile, "r");
		      if(!fd)
			      return soap_set_receiver_error(soap, "SSL/TLS error", "Invalid DH file", SOAP_SSL_ERROR);
		      dparams_len = (uint)fread(dparams_buf, 1, sizeof(dparams_buf), fd);
		      fclose(fd);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_ssl_client_context(struct soap * soap, ushort flags, const char * keyfile, const char * password, const char * cafile, const char * capath, const char * randfile)
{
	...
	soap->dhfile = NULL;
	soap->randfile = randfile;
	soap->fsslverify = (flags&SOAP_SSL_ALLOW_EXPIRED_CERTIFICATE) ==
		    0 ? ssl_verify_callback : ssl_verify_callback_allow_expired_certificate;
  #endif
  #ifdef WITH_GNUTLS
	if(soap->session) {
		gnutls_deinit(soap->session);
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_ssl_error(struct soap * soap, int ret)
{
	...
	if(ERR_peek_error()) {
		...
		while((r = ERR_get_error()))
			ERR_error_string_n(r, soap->msgbuf+sstrlen(soap->msgbuf), sizeof(soap->msgbuf)-sstrlen(soap->msgbuf));
	}
	else {switch(ret)      {
		      case 0:
...
static int ssl_auth_init(struct soap * soap)
{
	...
	if(!soap->xcred) {
		...
		if(soap->keyfile) {
			if(gnutls_certificate_set_x509_key_file(soap->xcred, soap->keyfile, soap->keyfile, GNUTLS_X509_FMT_PEM) < 0) /*
	...
	if((soap->ssl_flags&SOAP_SSL_CLIENT)) {
		...
		if(soap->cafile || soap->crlfile || soap->keyfile) {
			ret = gnutls_priority_set_direct(soap->session, "PERFORMANCE", 0);
			if(ret < 0)
			...
			gnutls_init(&soap->session, GNUTLS_CLIENT);
			gnutls_priority_set_direct(soap->session, "PERFORMANCE:+ANON-DH:!ARCFOUR-128", 0);
		...
		if(!soap->cache)
			gnutls_priority_init(&soap->cache, "NORMAL", 0);
...
{
	if(num < sstrleni(static_cast<const char *>(userdata))+1)
		return 0;
	return sstrleni(strcpy(buf, static_cast<const char *>(userdata)));
}
...
SOAP_FMAC1 int SOAP_FMAC2 soap_ssl_accept(struct soap * soap)
{
	...
	if(r) {
		soap_closesock(soap);
		return soap_set_receiver_error(soap, soap_ssl_error(soap,
				    r), "SSL/TLS handshake failed", SOAP_SSL_ERROR);
	}
	if((soap->ssl_flags&SOAP_SSL_REQUIRE_CLIENT_AUTHENTICATION)) {
		const char * err = ssl_verify(soap, 0);
		if(err) {
			soap_closesock(soap);
...
static const char * FASTCALL tcp_error(struct soap * soap)
{
	const char * msg = NULL;
	switch(soap->errmode) {
		case 0: msg = soap_strerror(soap); break;
		case 1: msg = "WSAStartup failed"; break;
		case 2:
	    {
  #ifndef WITH_LEAN
...
static int tcp_gethost(struct soap * soap, const char * addr, struct in_addr * inaddr)
{
	...
	memzero(/*(void *)*/&ht_data, sizeof(ht_data));
...
static SOAP_SOCKET tcp_connect(struct soap * soap, const char * endpoint, const char * host, int port)
{
	...
	soap->errmode = 0;
  #ifdef WITH_IPV6
	memzero(/*(void *)*/&hints, sizeof(hints));
...
again:
		...
		{ 
		  ...
		  if(err == SOAP_EADDRINUSE) {
			  ...
			  if(!getsockopt(fd, SOL_SOCKET, SO_ERROR, (char *)&soap->errnum, &k) && !soap->errnum) // portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h
				  break;
			  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not connect to host\n"));
				SETIFZ(soap->errnum, soap_socket_errno(fd));
			  soap_set_sender_error(soap, tcp_error(soap), "connect failed in tcp_connect()", SOAP_TCP_ERROR);
			  soap->fclosesocket(soap, fd);
   #ifdef WITH_IPV6
			  freeaddrinfo(ressave);
	...
	if(!soap_tag_cmp(endpoint, "https:*")) {
		...
		if(soap->proxy_host) {
			...
			soap->keep_alive = 1;
			if((soap->error = soap->fpost(soap, endpoint, host, port, NULL, NULL, 0)) || soap_end_send(soap)) {
				soap->fclosesocket(soap, fd);
				return SOAP_INVALID_SOCKET;
			}
		...
		if(!soap->ssl) {
			...
			SSL_clear(soap->ssl);
		if(soap->session) {
			if(sstreq(soap->session_host, host) && soap->session_port == port)
					...
					for(int i = 0; i < ext_count; i++) {
						...
						if(ext_str && sstreq(ext_str, "subjectAltName")) {
							...
							if(meth->it)
								...
								// meth->d2i(NULL, (const uchar**)&data, ...
								// and others prefer meth->d2i(NULL, &data,
								// ext->value->length);
								//
...
static int FASTCALL tcp_select(struct soap * soap, SOAP_SOCKET s, int flags, int timeout)
{
	int r;
	struct timeval tv;
...
SOAP_FMAC1 SOAP_SOCKET SOAP_FMAC2 soap_accept(struct soap * soap)
{
	...
	for(;; ) {
		...
		if(soap_valid_socket(soap->socket)) {
			...
			soap->ip = (ulong)ip1<<24|(ulong)ip2<<16|(ulong)ip3<<8|(ulong)ip4;
...
#ifndef PALM_1
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_closesock(struct soap * soap)
{
	...
	*soap->endpoint = '\0';
	...
	do {
		if(soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf))) {
		    return (soap->error == SOAP_EOF) ? SOAP_EOF : (soap->error = 414);
	    }
	    if((s = sstrchr(soap->msgbuf, ' '))) {
		    soap->status = (ushort)soap_strtoul(s, &s, 10);
	...
	if(soap->status == 0) {
		...
		if(s && httpcmd) {
			size_t m = sstrlen(soap->endpoint);
...
static int http_parse_header(struct soap * soap, const char * key, const char * val)
{
	if(!soap_tag_cmp(key, "Host")) {
		...
		if(action) {
			if(*action == '"') {
				...
				soap->action[sstrlen(soap->action)-1] = '\0';
			}
			else
   ...
   #ifdef WITH_GZIP
		if(sstrchr(val, '*') || soap_get_header_attribute(soap, val, "gzip"))
			soap->zlib_out = SOAP_ZLIB_GZIP;
		else
   #endif
		if(sstrchr(val, '*') || soap_get_header_attribute(soap, val, "deflate"))
...
#if !defined(WITH_NOHTTP) || !defined(WITH_LEANER)
#ifndef PALM_1
SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_get_header_attribute(struct soap * soap, const char * line, const char * key)
{
	const char * s = line;
	if(s) {
		while(*s) {
...
SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_decode_key(char * buf, size_t len, const char * val)
{
	return soap_decode(buf, len, val, "=,;");
}

SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_decode_val(char * buf, size_t len, const char * val)
{
	if(*val != '=') {
		*buf = '\0';
		return val;
...
static const char * FASTCALL soap_decode(char * buf, size_t len, const char * val, const char * sep)
{
	const char * s;
	char * t = buf;
	for(s = val; *s; s++)
		if(*s != ' ' && *s != '\t' && !sstrchr(sep, *s))
			break;
	if(*s == '"') {
		s++;
		while(*s && *s != '"' && --len)
		...
		while(*s && !soap_blank((soap_wchar)*s) && !sstrchr(sep, *s) && --len) {
		      if(*s == '%') {
...
static int http_post(struct soap * soap, const char * endpoint, const char * host, int port, const char * path, const char * action, size_t count)
{
	...
	int err;
	switch(soap->status) {
		case SOAP_GET: s = "GET"; break;
		case SOAP_PUT: s = "PUT"; break;
		case SOAP_DEL: s = "DELETE"; break;
		case SOAP_CONNECT: s = "CONNECT"; break;
		default: s = "POST";
	}
  ...
  #endif
  #ifndef WITH_LEAN
	if(soap->userid && soap->passwd && sstrlen(soap->userid)+sstrlen(soap->passwd) < 761) {
   #ifdef WITH_NTLM
		if(soap->ntlm_challenge && sstrlen(soap->ntlm_challenge)+6 < sizeof(soap->tmpbuf))
	...
	if(soap->proxy_userid && soap->proxy_passwd && sstrlen(soap->proxy_userid)+sstrlen(soap->proxy_passwd) < 761) {
   #ifdef WITH_NTLM
		if(soap->ntlm_challenge && sstrlen(soap->ntlm_challenge)+6 < sizeof(soap->tmpbuf))
			sprintf(soap->tmpbuf, "NTLM %s", soap->ntlm_challenge);
		else
   ...
   #endif
  #endif
	if(action && !oneof2(soap->status, SOAP_GET, SOAP_DEL)) {
		sprintf(soap->tmpbuf, "\"%s\"", action && sstrlen(action) < sizeof(soap->tmpbuf)-3 ? action : SOAP_STR_EOS);
...
static int http_response(struct soap * soap, int status, size_t count)
{
  ...
  #endif
	if(sstrlen(soap->http_version) > 4)
		return soap->error = SOAP_EOM;
	if(!status || status == SOAP_HTML || status == SOAP_FILE) {
		const char * s = (count || ((soap->omode&SOAP_IO) == SOAP_IO_CHUNK)) ? "200 OK" : "202 Accepted";
  ...
  #ifndef WITH_LEAN
		if(status == 401) {
			sprintf(soap->tmpbuf, "Basic realm=\"%s\"", (soap->authrealm && sstrlen(soap->authrealm) < sizeof(soap->tmpbuf)-14) ? soap->authrealm : "gSOAP Web Service");
		...
		const char * s = *soap_faultcode(soap);
	      ...
	      else if(soap->version == 2 && (!s || sstreq(s, "SOAP-ENV:Sender")))
		      s = "400 Bad Request";
	      else
		      s = "500 Internal Server Error";
	      DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Error %s (status=%d)\n", s, status));
  #ifdef WMW_RPM_IO
	      if(soap->rpmreqid || soap_valid_socket(soap->master) || soap_valid_socket(soap->socket)) /* RPM behaves as
			                                                                                 if standalone
			                                                                                 */
  #else
...
{
	size_t count;
	if(!(soap->omode&(SOAP_ENC_XML|SOAP_IO_STORE /* this tests for chunking too */)) && oneof2(status, SOAP_HTML, SOAP_FILE))
		soap->omode = (soap->omode&~SOAP_IO)|SOAP_IO_STORE;
	soap->status = status;
...
SOAP_FMAC1 struct soap_cookie * SOAP_FMAC2 soap_cookie(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie * p;
	SETIFZ(domain, soap->cookie_domain);
	SETIFZ(path, soap->cookie_path);
	if(!path)
		path = SOAP_STR_EOS;
	else if(*path == '/')
		path++;
	...
	for(p = soap->cookies; p; p = p->next) {
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Cookie in database: %s=%s domain=%s path=%s env=%hd\n", p->name,
			p->value ? p->value : "(null)", p->domain ? p->domain : "(null)", p->path ? p->path : "(null)", p->env));
		if(sstreq(p->name, name) && p->domain && p->path && sstreq(p->domain, domain) && (!*p->path || !strncmp(p->path, path, sstrlen(p->path))))
			break;
	}
	return p;
}
...
SOAP_FMAC1 struct soap_cookie * SOAP_FMAC2 soap_set_cookie(struct soap * soap, const char * name, const char * value, const char * domain, const char * path)
{
	...
	if(!q) {
		if((q = (struct soap_cookie*)SOAP_MALLOC(soap, sizeof(struct soap_cookie)))) {
			if((q->name = (char *)SOAP_MALLOC(soap, sstrlen(name)+1)))
				strcpy(q->name, name);
			q->value = NULL;
			q->domain = NULL;
			q->path = NULL;
			...
			for(p = &soap->cookies, n = soap->cookie_max; *p && n; p = &(*p)->next, n--)
				if(sstreq((*p)->name, name) && (*p)->path && path && strcmp((*p)->path, path) < 0)
					break;
			if(n) {
				q->next = *p;
				*p = q;
	...
	if(q) {
		...
		if(value && *value && !q->value && (q->value = (char *)SOAP_MALLOC(soap, sstrlen(value)+1)))
		...
		if(domain && !q->domain && (q->domain = (char *)SOAP_MALLOC(soap, sstrlen(domain)+1)))
			strcpy(q->domain, domain);
		if(q->path) {
			if(!path || strncmp(path, q->path, sstrlen(q->path))) {
				SOAP_FREE(soap, q->path);
				q->path = NULL;
			}
		}
		if(path && !q->path && (q->path = (char *)SOAP_MALLOC(soap, sstrlen(path)+1)))
			strcpy(q->path, path);
		q->session = 1;
		q->env = 0;
	}
...
SOAP_FMAC1 void SOAP_FMAC2 soap_clr_cookie(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie ** p, * q;
	SETIFZ(domain, soap->cookie_domain);
	if(!domain) {
	...
	for(p = &soap->cookies, q = *p; q; q = *p) {
		if(sstreq(q->name, name) && sstreq(q->domain, domain) && !strncmp(q->path, path, sstrlen(q->path))) {
			SOAP_FREE(soap, q->value);
			SOAP_FREE(soap, q->domain);
			SOAP_FREE(soap, q->path);
			*p = q->next;
...
{
	struct soap_cookie * p = soap_cookie(soap, name, domain, path);
	return (p && p->env) ? p->value : NULL;
}

SOAP_FMAC1 time_t SOAP_FMAC2 soap_cookie_expire(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie * p = soap_cookie(soap, name, domain, path);
	return p ? p->expire : -1;
}

SOAP_FMAC1 int SOAP_FMAC2 soap_set_cookie_expire(struct soap * soap, const char * name, long expire, const char * domain, const char * path)
{
	struct soap_cookie * p;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_set_cookie_session(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie * p = soap_cookie(soap, name, domain, path);
	if(p) {
		p->session = 1;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_clr_cookie_session(struct soap * soap, const char * name, const char * domain, const char * path)
{
	struct soap_cookie * p;
	if((p = soap_cookie(soap, name, domain, path))) {
		p->session = 0;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_putsetcookies(struct soap * soap)
{
	...
	for(p = soap->cookies; p; p = p->next) {
		if(p->modified
			...
			if(p->domain && sstrleni(p->domain) < tmp-s+4064) {
				strcpy(s, ";Domain=");
				strcat(s, p->domain);
			}
			else if(soap->cookie_domain && sstrleni(soap->cookie_domain) < tmp-s+4064) {
				strcpy(s, ";Domain=");
			...
			if(t) {
				if(*t == '/')
					t++;
				if(sstrleni(t) < tmp-s+4064) {
					if(sstrchr(t, '%')) {                                  /* already URL encoded? */
						strcpy(s, t);
						s += sstrlen(s);
					}
					else
...
SOAP_FMAC1 int SOAP_FMAC2 soap_putcookies(struct soap * soap, const char * domain, const char * path, int secure)
{
			...
			*p = q->next;
			...
			/* domain-level cookies, cannot compile when WITH_NOIO set */
		      ...
		      if(!flag) {
			      ...
			      if(hostent) {
				      const char * r = sstrchr(hostent->h_name, '.');
				      if(!r)
					      r = hostent->h_name;
				      flag = !strncmp(t, r, n);
			      }
 ...
 #endif
		      if(flag && (!q->path || !strncmp(q->path, path, sstrlen(q->path))) && (!q->secure || secure)) {
			      size_t n = 12;
			      if(q->name)
				      n += 3*sstrlen(q->name);
			      if(q->value && *q->value)
				      n += 3*sstrlen(q->value)+1;
			      if(q->path && *q->path)
				      n += sstrlen(q->path)+9;
			      if(q->domain)
				      n += sstrlen(q->domain)+11;
...
SOAP_FMAC1 void SOAP_FMAC2 soap_getcookies(struct soap * soap, const char * val)
{
	...
	while(*s) {
		...
		if(!soap_tag_cmp(tmp, "$Version")) {
			...
			if(*tmp) {
				if((t = (char *)SOAP_MALLOC(soap, sstrlen(tmp)+1)))
					strcpy(t, tmp);
			}
			else
				t = NULL;
			...
			s = soap_decode_val(tmp, sizeof(tmp), s);
			if(*tmp) {
				if((p->path = (char *)SOAP_MALLOC(soap, sstrlen(tmp)+1)))
					strcpy(p->path, tmp);
			}
			else
				p->path = NULL;
			...
			s = soap_decode_val(tmp, sizeof(tmp), s);
			if(*tmp) {
				if((p->domain = (char *)SOAP_MALLOC(soap, sstrlen(tmp)+1)))
					strcpy(p->domain, tmp);
			}
			else
				p->domain = NULL;
			...
			s = soap_decode_val(tmp, sizeof(tmp), s);
			p->version = (uint)soap_strtoul(tmp, NULL, 10);
		}
			...
			if(p) {
		      ...
		      if((p = (struct soap_cookie*)SOAP_MALLOC(soap, sizeof(struct soap_cookie)))) {
			      p->name = (char *)SOAP_MALLOC(soap, sstrlen(tmp)+1);
			      strcpy(p->name, tmp);
			      s = soap_decode_val(tmp, sizeof(tmp), s);
			      if(*tmp) {
				      p->value = (char *)SOAP_MALLOC(soap, sstrlen(tmp)+1);
				      strcpy(p->value, tmp);
			      }
			      else
				      p->value = NULL;
			      if(domain)
				      ...
				      p->domain = (char *)SOAP_MALLOC(soap, sstrlen(soap->host)+1);
			      ...
			      if(path)
				      ...
				      p->path = (char *)SOAP_MALLOC(soap, sstrlen(soap->path)+1);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_getenv_cookies(struct soap * soap)
{
	struct soap_cookie * p;
	const char * s;
...
SOAP_FMAC1 struct soap_cookie * SOAP_FMAC2 soap_copy_cookies(struct soap * copy, const struct soap * soap)
{
	struct soap_cookie * p, * r;
	struct soap_cookie ** q = &r;
	for(p = soap->cookies; p; p = p->next) {
		if(!(*q = (struct soap_cookie*)SOAP_MALLOC(copy, sizeof(struct soap_cookie))))
			return r;
		**q = *p;
		if(p->name) {
			if(((*q)->name = (char *)SOAP_MALLOC(copy, sstrlen(p->name)+1)))
				strcpy((*q)->name, p->name);
		}
		if(p->value) {
			if(((*q)->value = (char *)SOAP_MALLOC(copy, sstrlen(p->value)+1)))
				strcpy((*q)->value, p->value);
		}
		if(p->domain) {
			if(((*q)->domain = (char *)SOAP_MALLOC(copy, sstrlen(p->domain)+1)))
				strcpy((*q)->domain, p->domain);
		}
		if(p->path) {
			if(((*q)->path = (char *)SOAP_MALLOC(copy, sstrlen(p->path)+1)))
...
SOAP_FMAC1 void SOAP_FMAC2 soap_free_cookies(struct soap * soap)
{
	struct soap_cookie * p;
...
#ifndef WITH_NOIDREF
#ifndef PALM_2
SOAP_FMAC1 size_t /*SOAP_FMAC2*/FASTCALL soap_hash(const char * s)
{
	size_t h = 0;
	while(*s)
		h = 65599*h+*s++;
...
static void soap_init_pht(struct soap * soap)
{
	soap->pblk = NULL;
	soap->pidx = 0;
	for(int i = 0; i < (int)SOAP_PTRHASH; i++)
		soap->pht[i] = NULL;
}
#endif
#endif // !WITH_NOIDREF
...
static void soap_free_pht(struct soap * soap)
{
	...
	for(struct soap_pblk * pb = soap->pblk; pb; pb = next) {
		next = pb->next;
		SOAP_FREE(soap, pb);
	}
	soap->pblk = NULL;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_array_pointer_lookup(struct soap * soap, const void * p, const struct soap_array * a, int n, int type, struct soap_plist ** ppp)
{
	*ppp = NULL;
	if(p && a->__ptr) {
		for(struct soap_plist * pp = soap->pht[soap_hash_ptr(a->__ptr)]; pp; pp = pp->next) {
...
#ifndef PALM_1
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_begin_count(struct soap * soap)
{
	soap_free_ns(soap);
 #ifndef WITH_LEANER
	...
	if((soap->mode&SOAP_IO) == SOAP_IO_STORE || (((soap->mode&SOAP_IO) == SOAP_IO_CHUNK || (soap->mode&SOAP_ENC_XML))
#ifndef WITH_LEANER
		&& !soap->fpreparesend
#endif
		))
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_end_count(struct soap * soap)
{
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_begin_send(struct soap * soap)
{
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_reference(struct soap * soap, const void * p, int t)
{
	struct soap_plist * pp;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_array_reference(struct soap * soap, const void * p, const struct soap_array * a, int n, int t)
{
	if(!p || !a->__ptr)
		...
		struct soap_plist * pp;
		const int i = soap_array_pointer_lookup(soap, p, a, n, t, &pp);
		if(i) {
			if(pp->mark1 == 0) {
				pp->mark1 = 2;
				pp->mark2 = 2;
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_embedded_id(struct soap * soap, int id, const void * p, int t)
{
...
SOAP_FMAC1 int SOAP_FMAC2 soap_is_embedded(struct soap * soap, const struct soap_plist * pp)
{
	if(!pp)
		return 0;
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Is embedded? %d %d\n", (int)pp->mark1, (int)pp->mark2));
	if(soap->version == 1 && soap->encodingStyle && !(soap->mode&SOAP_XML_GRAPH) && soap->part != SOAP_IN_HEADER)
		return (soap->mode&SOAP_IO_LENGTH) ? (pp->mark1 != 0) : (pp->mark2 != 0);
	else if(soap->mode&SOAP_IO_LENGTH)
		return (pp->mark1 == 1);
	else
		return (pp->mark2 == 1);
}

SOAP_FMAC1 int SOAP_FMAC2 soap_is_single(const struct soap * soap, const struct soap_plist * pp)
{
...
SOAP_FMAC1 void SOAP_FMAC2 soap_set_embedded(const struct soap * pSoap, struct soap_plist * pp)
{
	if(pp) {
	...
	if(soap->mode&SOAP_IO_LENGTH) {
		if(pp->mark1 != 3) {
			...
			if(!strncmp(aid, "cid:", 4)) { /* RFC 2111 */
				if(soap->mode&SOAP_ENC_MTOM) {
					char * s = static_cast<char *>(soap_malloc(soap, sstrlen(aid)-1));
					if(s) {
						*s = '<';
						strcpy(s+1, aid+4);
						strcat(s, ">");
...
#ifndef WITH_NOIDREF
#ifndef PALM_2
SOAP_FMAC1 struct soap_ilist * /*SOAP_FMAC2*/FASTCALL soap_lookup(struct soap * soap, const char * id)
{
	struct soap_ilist * ip = NULL;
...
SOAP_FMAC1 struct soap_ilist * /*SOAP_FMAC2*/FASTCALL soap_enter(struct soap * soap, const char * id)
{
	struct soap_ilist * ip = static_cast<struct soap_ilist *>(SOAP_MALLOC(soap, sizeof(struct soap_ilist)+sstrlen(id)));
	if(ip) {
		size_t h = soap_hash(id);
...
#ifndef PALM_2
SOAP_FMAC1 void * /*SOAP_FMAC2*/FASTCALL soap_malloc(struct soap * soap, size_t n)
{
	char * p;
	if(!n)
	...
	if(soap->fmalloc)
		p = static_cast<char *>(soap->fmalloc(soap, n));
	else {
		n += sizeof(short);
		n += (-(long)n)&(sizeof(void *)-1); /* align at 4-, 8- or 16-byte boundary */
	      if(!(p = static_cast<char *>(SOAP_MALLOC(soap, n+sizeof(void *)+sizeof(size_t))))) {
		      soap->error = SOAP_EOM;
		      return NULL;
	      }
		/* set the canary to detect corruption */
	      *reinterpret_cast<ushort *>(p+n-sizeof(ushort)) = (ushort)SOAP_CANARY;
...
SOAP_FMAC1 void * SOAP_FMAC2 soap_track_malloc(struct soap * soap, const char * file, int line, size_t size)
{
	void * p = SAlloc::M(size);
	if(soap) {
		size_t h = soap_hash_ptr(p);
		struct soap_mlist * mp = (struct soap_mlist*)SAlloc::M(sizeof(struct soap_mlist));
		if(soap->fdebug[SOAP_INDEX_TEST]) {
			DBGLOG(TEST, SOAP_MESSAGE(fdebug, "%s(%d): SAlloc::M(%lu) = %p\n", file, line, (ulong)size, p));
		}
		mp->next = soap->mht[h];
		mp->ptr = p;
		mp->file = file;
...
SOAP_FMAC1 void SOAP_FMAC2 soap_track_free(struct soap * soap, const char * file, int line, void * p)
{
	size_t h = soap_hash_ptr(p);
...
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_delete(struct soap * soap, void * p)
{
	if(!soap_check_state(soap)) {
...
SOAP_FMAC1 void SOAP_FMAC2 soap_delegate_deletion(struct soap * soap, struct soap * soap_to)
{
	...
	*q = static_cast<char *>(soap_to->alist);
...
SOAP_FMAC1 struct soap_clist * /*SOAP_FMAC2*/FASTCALL soap_link(struct soap * soap, void * p, int t, int n, int (* fdelete)(struct soap_clist *))
{
	struct soap_clist * cp = static_cast<struct soap_clist *>(SOAP_MALLOC(soap, sizeof(struct soap_clist)));
	if(cp) {
		cp->next = soap->clist;
		cp->type = t;
		cp->size = n;
...
#ifndef WITH_NOIDREF
#ifndef PALM_2
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_lookup_type(struct soap * soap, const char * id)
{
...
SOAP_FMAC1 void * /*SOAP_FMAC2*/FASTCALL soap_id_lookup(struct soap * soap, const char * id, void ** p, int t, size_t n, unsigned int k)
{
	struct soap_ilist * ip;
	void ** q;
	if(!p || !id || !*id)
	...
	if(!ip) {
		...
		*p = NULL;
		...
		while(ip->level < k) {
			q = static_cast<void **>(soap_malloc(soap, sizeof(void *)));
			if(!q)
				return NULL;
			*p = (void *)q;
			p = q;
...
SOAP_FMAC1 void * /*SOAP_FMAC2*/FASTCALL soap_id_forward(struct soap * soap, const char * href, void * p, size_t len, int st, int tt, size_t n,
	uint k, void (* fcopy)(struct soap *, int, int, void *, size_t, const void *, size_t))
{
	struct soap_ilist * ip;
	if(!p || !href || !*href)
		return p;
	...
	if(!ip) {
		...
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Type incompatibility id='%s' expect type=%d size=%lu level=%u got type=%d size=%lu\n",
			href, ip->type, (ulong)ip->size, k, st, (ulong)n));
		strcpy(soap->id, href);
		soap->error = SOAP_HREF;
		return NULL;
	}
...
#ifndef PALM_2
SOAP_FMAC1 void * /*SOAP_FMAC2*/FASTCALL soap_id_enter(struct soap * soap, const char * id, void * p, int t, size_t n, unsigned int k,
	const char * type, const char * arrayType, void *(*finstantiate)(struct soap *, int, const char *, const char *, size_t *))
{
 #ifndef WITH_NOIDREF
	struct soap_ilist * ip;
 #endif
	...
	soap->alloced = 0;
	if(!p) {
		p = finstantiate ? finstantiate(soap, t, type, arrayType, &n) : soap_malloc(soap, n);
		if(p)
			soap->alloced = 1;
	}
 #ifndef WITH_NOIDREF
	...
	if(!ip) {
		...
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Type incompatibility id='%s' expect type=%d size=%lu level=%u got type=%d size=%lu\n",
			id, ip->type, (ulong)ip->size, k, t, (ulong)n));
		strcpy(soap->id, id);
		soap->error = SOAP_HREF;
		return NULL;
	}
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_end_send(struct soap * soap)
{
 #ifndef WITH_LEANER
	int err;
	if(soap->dime.list) { /* SOAP body referenced attachments must appear first */
 ...
 #endif
			for(p = soap_first_block(soap, 0); p; p = soap_next_block(soap, NULL)) {
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_end_recv(struct soap * soap)
{
	soap->part = SOAP_END;
 #ifndef WITH_LEAN
	...
	if(soap->xlist) {
		for(struct soap_multipart * content = soap->mime.list; content; content = content->next)
			soap_resolve_attachment(soap, content);
	}
 #endif
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "End of receive message ok\n"));
 #ifdef WITH_ZLIB
	if(soap->mode&SOAP_ENC_ZLIB) { /* Make sure end of compressed content is reached */
...
SOAP_FMAC1 void SOAP_FMAC2 soap_copy_stream(struct soap * copy, struct soap * soap)
{
 ...
 #ifdef WITH_OPENSSL
	copy->bio = NULL;
	copy->ctx = (soap->state == SOAP_COPY) ? soap->ctx : NULL;
	copy->ssl = soap->ssl ? SSL_dup(soap->ssl) : NULL;
 #endif
 #ifdef WITH_GNUTLS
	...
	if(soap->nlist && soap->local_namespaces) {
		...
		while(np) {
			...
			if(!s && np->index >= 0) {
				...
				SETIFZ(s, soap->local_namespaces[np->index].ns);
			}
...
SOAP_FMAC1 void SOAP_FMAC2 soap_versioning(soap_init) (struct soap * soap, soap_mode imode, soap_mode omode)
{
	...
	soap->plugins = NULL;
	soap->user = NULL;
	for(i = 0; i < SIZEOFARRAY(soap->data); i++)
		soap->data[i] = NULL;
	soap->userid = NULL;
	soap->passwd = NULL;
	soap->authrealm = NULL;
  ...
  #else
	soap->c_locale = newlocale(LC_ALL_MASK, "C", 0);
  #endif
 #else
	soap->c_locale = NULL;
 #endif
...
#ifndef PALM_2
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_begin(struct soap * soap)
{
...
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_end(struct soap * soap)
{
	if(soap_check_state(soap))
		return;
	soap_free_temp(soap);
...
static struct soap_nlist * soap_push_ns(struct soap * soap, const char * id, const char * ns, short utilized)
{
	...
	if(!utilized) {
		for(np = soap->nlist; np; np = np->next) {
			if(sstreq(np->id, id) && (!np->ns || sstreq(np->ns, ns)))
				break;
		}
		if(np) {
	...
	soap->nlist = np;
	strcpy((char *)np->id, id);
	np->ns = ns ? strcpy((char *)np->id+n+1, ns) : 0;
	np->level = soap->level;
	np->index = utilized;
	return np;
}
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_element(struct soap * soap, const char * tag, int id, const char * type)
{
	...
	if((soap->mode&SOAP_XML_DEFAULTNS) && (s = sstrchr(tag, ':'))) {
		struct Namespace * ns = soap->local_namespaces;
		size_t n = s-tag;
...
#ifndef HAVE_STRTOUL
SOAP_FMAC1 ulong SOAP_FMAC2 soap_strtoul(const char * s, char ** t, int b)
{
	ulong n = 0;
	int c;
	while(*s > 0 && *s <= 32)
...
{
	if(!type || !*type)
		return soap_element_begin_out(soap, tag, id, 0);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_element_start_end_out(struct soap * soap, const char * tag)
{
 ...
 #ifdef WITH_DOM
	if((soap->mode&SOAP_XML_DOM) && soap->dom) {
		struct soap_dom_attribute ** att = &soap->dom->atts;
		for(tp = soap->attributes; tp; tp = tp->next) {
			if(tp->visible) {
 ...
 #ifndef WITH_LEAN
			const char * s;
			if((soap->mode&SOAP_XML_DEFAULTNS) && (s = sstrchr(tp->name, ':'))) {
				size_t n = s-tp->name;
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_element_end_out(struct soap * soap, const char * tag)
{
 #ifndef WITH_LEAN
	const char * s;
 #endif
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_element_result(struct soap * soap, const char * tag)
{
	if(soap->version == 2 && soap->encodingStyle) {
		if(soap_element(soap, "SOAP-RPC:result", 0, NULL) || soap_attribute(soap, "xmlns:SOAP-RPC", soap_rpc) ||
			soap_element_start_end_out(soap, NULL) || soap_string_out(soap, tag, 0) || soap_element_end_out(soap, "SOAP-RPC:result"))
			return soap->error;
	}
	return SOAP_OK;
}

SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_check_result(struct soap * soap, const char * tag)
{
	if(soap->version == 2 && soap->encodingStyle) {
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_element_begin_in(struct soap * soap, const char * tag, int nillable, const char * type)
{
	if(!soap_peek_element(soap)) {
		if(soap->other)
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_element_end_in(struct soap * soap, const char * tag)
{
	soap_wchar c;
	char * s;
	int n = 0;
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_set_attr(struct soap * soap, const char * name, const char * value, int flag)
{
	struct soap_attribute * tp;
	if(*name == '-')
		return SOAP_OK;
	...
	for(tp = soap->attributes; tp; tp = tp->next) {
		if(sstreq(tp->name, name))
			break;
	}
	if(!tp) {
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Allocate attribute %s\n", name));
		if(!(tp = (struct soap_attribute*)SOAP_MALLOC(soap, sizeof(struct soap_attribute)+sstrlen(name))))
			return soap->error = SOAP_EOM;
		tp->ns = NULL;
 #ifndef WITH_LEAN
		if((soap->mode&SOAP_XML_CANONICAL)) {
	...
	else if(value && tp->value && tp->size <= sstrlen(value)) {
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_append_lab(struct soap * soap, const char * s, size_t n)
{
	if(soap->labidx+n >= soap->lablen) {
		char * t = soap->labbuf;
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Enlarging look-aside buffer to append data, old size=%lu", (ulong)soap->lablen));
		SETIFZ(soap->lablen, SOAP_LABLEN);
...
SOAP_FMAC1 int FASTCALL soap_peek_element(struct soap * soap)
{
	...
	int i;
	if(soap->peeked) {
		return (!*soap->tag) ? (soap->error = SOAP_NO_TAG) : SOAP_OK;
	}
	soap->peeked = 1;
	soap->id[0] = '\0';
	soap->href[0] = '\0';
		...
		if(tp->visible && tp->value) {
			...
			if(!soap_match_tag(soap, tp->name, "xsi:type")) {
				...
				if(!soap_match_tag(soap, tp->name, "SOAP-ENC:arrayType")) {
					...
					soap->root = ((sstreq(tp->value, "1") || sstreq(tp->value, "true")));
...
SOAP_FMAC1 void /*SOAP_FMAC2*/FASTCALL soap_revert(struct soap * soap)
{
	if(!soap->peeked) {
		soap->peeked = 1;
		if(soap->body)
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_string_out(struct soap * soap, const char * s, int flag)
{
	const char * t;
	soap_wchar c;
		...
		switch(c) {
			...
			default:
			    ...
			    if((c&mask) || !(c&0xFFFFFFE0UL)) {
				    if(soap_send_raw(soap, s, t-s-1) || soap_pututf8(soap, (uchar)c))
					    return soap->error;
				    s = t;
			    }
 #endif
		...
		switch(c) {
			...
			default:
			    if(c >= 0x20 && c < 0x80) {
				    ...
				    if(sizeof(wchar_t) < 4 && (c&0xD800) == 0xD800) { // http://unicode.org/faq/utf_bom.html#utf16-2
					    if((*s&0xD800) == 0xD800)
						    c = (c<<10)+*s+++0x10000-(0xD800<<10)-0xDC00;
					    else
						    c = 0xFFFD;  /* Malformed */
				    }
				    if(soap_pututf8(soap, (ulong)c))
					    return soap->error;
			    }
			    continue;
		}
...
SOAP_FMAC1 int * SOAP_FMAC2 soap_inint(struct soap * soap, const char * tag, int * p, const char * type, int t)
{
	...
	if(*soap->type && soap_match_tag(soap, soap->type, type) &&
		soap_match_tag(soap, soap->type, ":int") && soap_match_tag(soap, soap->type, ":short") && soap_match_tag(soap, soap->type, ":byte")) {
		soap->error = SOAP_TYPE;
		soap_revert(soap);
		return NULL;
	}
...
SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_long2s(struct soap * soap, long n)
{
...
{
	return (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) || soap_string_out(soap, soap_LONG642s(soap, *p), 0)) ? 
		soap->error : soap_element_end_out(soap, tag);
}
...
SOAP_FMAC1 LONG64 * SOAP_FMAC2 soap_inLONG64(struct soap * soap, const char * tag, LONG64 * p, const char * type, int t)
{
 ...
 #endif
	p = (LONG64*)soap_id_enter(soap, soap->id, p, t, sizeof(LONG64), 0, 0, 0, 0);
	if(*soap->href)
		p = (LONG64*)soap_id_forward(soap, soap->href, p, 0, t, 0, sizeof(LONG64), 0, 0);
	else if(p) {
...
SOAP_FMAC1 char * SOAP_FMAC2 soap_inbyte(struct soap * soap, const char * tag, char * p, const char * type, int t)
{
 ...
 #endif
	p = (char *)soap_id_enter(soap, soap->id, p, t, sizeof(char), 0, 0, 0, 0);
	if(*soap->href)
		p = (char *)soap_id_forward(soap, soap->href, p, 0, t, 0, sizeof(char), 0, 0);
	else if(p) {
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_float2s(struct soap * soap, float n)
{
	...
	s = sstrchr(s, ','); /* convert decimal comma to DP */
	ASSIGN_PTR(s, '.');
 #endif
	return soap->tmpbuf;
}
...
SOAP_FMAC1 float * SOAP_FMAC2 soap_infloat(struct soap * soap, const char * tag, float * p, const char * type, int t)
{
	...
	if(*soap->type != '\0' && soap_isnumeric(soap, type))
		return NULL;
 #endif
	p = (float *)soap_id_enter(soap, soap->id, p, t, sizeof(float), 0, 0, 0, 0);
	if(*soap->href)
		p = (float *)soap_id_forward(soap, soap->href, p, 0, t, 0, sizeof(float), 0, 0);
	else if(p) {
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_unsignedByte2s(struct soap * soap, uchar n)
{
	return soap_unsignedLong2s(soap, (ulong)n);
}

SOAP_FMAC1 int SOAP_FMAC2 soap_outunsignedByte(struct soap * soap, const char * tag, int id, const uchar * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) || soap_string_out(soap, soap_unsignedLong2s(soap, (ulong)*p), 0))
		return soap->error;
	return soap_element_end_out(soap, tag);
}

SOAP_FMAC1 int SOAP_FMAC2 soap_s2unsignedByte(struct soap * soap, const char * s, uchar * p)
{
	if(s) {
		char * r;
		ulong n = soap_strtoul(s, &r, 10);
		if(s == r || *r || n > 255)
			soap->error = SOAP_TYPE;
		*p = (uchar)n;
	}
...
SOAP_FMAC1 uchar * SOAP_FMAC2 soap_inunsignedByte(struct soap * soap, const char * tag, uchar * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_unsignedShort2s(struct soap * soap, ushort n)
{
	return soap_unsignedLong2s(soap, (ulong)n);
}

SOAP_FMAC1 int SOAP_FMAC2 soap_outunsignedShort(struct soap * soap, const char * tag, int id, const ushort * p, const char * type, int n)
{
	if(soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, p, n), type) || soap_string_out(soap, soap_unsignedLong2s(soap, (ulong)*p), 0))
		return soap->error;
	return soap_element_end_out(soap, tag);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_s2unsignedShort(struct soap * soap, const char * s, ushort * p)
{
	if(s) {
		char * r;
		ulong n = soap_strtoul(s, &r, 10);
...
SOAP_FMAC1 ushort * SOAP_FMAC2 soap_inunsignedShort(struct soap * soap, const char * tag, ushort * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_unsignedLong2s(struct soap * soap, ulong n)
{
...
SOAP_FMAC1 int SOAP_FMAC2 soap_s2unsignedLong(struct soap * soap, const char * s, ulong * p)
{
	if(s) {
		char * r;
 #ifndef WITH_NOIO
...
SOAP_FMAC1 ulong * SOAP_FMAC2 soap_inunsignedLong(struct soap * soap, const char * tag, ulong * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
 #ifndef WITH_LEAN
...
SOAP_FMAC1 ULONG64 * SOAP_FMAC2 soap_inULONG64(struct soap * soap, const char * tag, ULONG64 * p, const char * type, int t)
{
	if(soap_element_begin_in(soap, tag, 0, NULL))
		return NULL;
	if(*soap->type && soap_match_tag(soap, soap->type, type) &&
		soap_match_tag(soap, soap->type, ":positiveInteger") && soap_match_tag(soap, soap->type, ":nonNegativeInteger") &&
		soap_match_tag(soap, soap->type, ":unsignedLong") && soap_match_tag(soap, soap->type, ":unsignedInt") &&
				...
				if(np) {
					if(np->index >= 0 && soap->local_namespaces && (q = soap->local_namespaces[np->index].id)) {
						...
						if(q[k-1] != '_')
							...
							soap_append_lab(soap, soap->local_namespaces[np->index].ns, sstrlen(soap->local_namespaces[np->index].ns));
						      soap_append_lab(soap, "\"", 1); 
						}
					}
					else if(np->ns) {
						soap_append_lab(soap, "\"", 1);
						soap_append_lab(soap, np->ns, sstrlen(np->ns));
						soap_append_lab(soap, "\"", 1);
					}
					else {
		...
		for(;; ) {
			...
			if(*s != '"') {
				...
				if(q) {
					...
					if(p && p->id) {
						soap_append_lab(soap, p->id, sstrlen(p->id));
					}
						...
						soap_append_lab(soap, soap->tmpbuf+6, sstrlen(soap->tmpbuf+6));
					}
...
SOAP_FMAC1 int SOAP_FMAC2 soap_s2wchar(struct soap * soap, const char * s, wchar_t ** t, long minlen, long maxlen)
{
	if(s) {
		long l;
		wchar_t * r;
		*t = r = static_cast<wchar_t *>(soap_malloc(soap, sizeof(wchar_t)*(sstrlen(s)+1)));
		if(!r)
			return soap->error = SOAP_EOM;
...
#ifndef PALM_2
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_outstring(struct soap * soap, const char * tag, int id, char * const * p, const char * type, int n)
{
...
SOAP_FMAC1 char ** /*SOAP_FMAC2*/FASTCALL soap_instring(struct soap * soap, const char * tag, char ** p,
	const char * type, int t, int flag, long minLen, long maxLen)
{
	(void)type;
...
SOAP_FMAC1 wchar_t ** SOAP_FMAC2 soap_inwstring(struct soap * soap, const char * tag,
	wchar_t ** p, const char * type, int t, long minlen, long maxlen)
{
	(void)type;
	if(soap_element_begin_in(soap, tag, 1, NULL)) {
		if(!tag || *tag != '-' || soap->error != SOAP_NO_TAG)
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_dateTime2s(struct soap * soap, time_t n)
{
	...
	if(localtime_r(&n, pT)) {
		...
		sprintf(soap->tmpbuf+sstrlen(soap->tmpbuf), "%+03d:%02d", -tz.tz_minuteswest/60+(pT->tm_isdst!=0), abs(tz.tz_minuteswest)%60);
	}
  #else
	if((pT = localtime(&n))) {
		struct timeval tv;
   ...
   #endif
		strftime(soap->tmpbuf, sizeof(soap->tmpbuf), "%Y-%m-%dT%H:%M:%S", pT);
		sprintf(soap->tmpbuf+sstrlen(soap->tmpbuf), "%+03d:%02d", -t.timezone/60+(pT->tm_isdst!=0), abs(t.timezone)%60);
	}
  #else
	if((pT = localtime(&n))) {
   #ifdef __BORLANDC__
...
SOAP_FMAC1 time_t * SOAP_FMAC2 soap_indateTime(struct soap * soap, const char * tag, time_t * p, const char * type, int t)
{
	...
	p = (time_t*)soap_id_enter(soap, soap->id, p, t, sizeof(time_t), 0, 0, 0, 0);
	if(*soap->href)
		p = (time_t*)soap_id_forward(soap, soap->href, p, 0, t, 0, sizeof(time_t), 0, 0);
	else if(p) {
...
SOAP_FMAC1 int SOAP_FMAC2 soap_outliteral(struct soap * soap, const char * tag, char * const * p, const char * type)
{
	...
	const char * t = NULL;
	if(tag && *tag != '-') {
		if(soap->local_namespaces && (t = sstrchr(tag, ':'))) {
			strncpy(soap->tmpbuf, tag, t-tag);
			soap->tmpbuf[t-tag] = '\0';
			for(i = 0; soap->local_namespaces[i].id; i++)
				if(sstreq(soap->tmpbuf, soap->local_namespaces[i].id))
					break;
			t++;
			if(soap_element(soap, t, 0, type) || soap_attribute(soap, "xmlns", soap->local_namespaces[i].ns ? soap->local_namespaces[i].ns : SOAP_STR_EOS) || 
	...
	return t ? soap_element_end_out(soap, t) : SOAP_OK;
}
...
#ifndef PALM_2
SOAP_FMAC1 const char * /*SOAP_FMAC2*/FASTCALL soap_value(struct soap * soap)
{
	size_t i;
	soap_wchar c = 0;
	char * s = soap->tmpbuf;
 ...
 #ifndef PALM_2
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_getline(struct soap * soap, char * s, int len)
{
	int i = len;
	soap_wchar c = 0;
	for(;; ) {
...
static size_t soap_count_attachments(struct soap * soap)
{
	...
	if((soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM)) {
		...
		for(struct soap_multipart * content = soap->dime.first; content; content = content->next) {
			count += 12+((content->size+3)&(~3));
			if(content->id)
				count += ((sstrlen(content->id)+3)&(~3));
			if(content->type)
				count += ((sstrlen(content->type)+3)&(~3));
			if(content->options)
				count += ((((uchar)content->options[2]<<8)|((uchar)content->options[3]))+7)&(~3);
			DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Size of DIME attachment content is %lu bytes\n", (ulong)content->size));
		}
	}
	if((soap->mode&SOAP_ENC_MIME) && soap->mime.boundary) {
		size_t n = sstrlen(soap->mime.boundary);
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Calculating the size of MIME attachments\n"));
		for(struct soap_multipart * content = soap->mime.first; content; content = content->next) {
			const char * s;
			/* count \r\n--boundary\r\n */
			count += 6+n;
			/* count Content-Type: ...\r\n */
			if(content->type)
				count += 16+sstrlen(content->type);
			...
			/* count Content-ID: ...\r\n */
			if(content->id)
				count += 14+sstrlen(content->id);
			/* count Content-Location: ...\r\n */
			if(content->location)
				count += 20+sstrlen(content->location);
			/* count Content-Description: ...\r\n */
			if(content->description)
				count += 23+sstrlen(content->description);
			/* count \r\n...content */
...
#ifndef WITH_LEANER
#ifndef PALM_1
static int FASTCALL soap_putdimefield(struct soap * soap, const char * s, size_t n)
{
	return soap_send_raw(soap, s, n) ? soap->error : soap_send_raw(soap, SOAP_STR_PADDING, -(long)n&3);
}

SOAP_FMAC1 char * SOAP_FMAC2 soap_dime_option(struct soap * soap, ushort optype, const char * option)
{
	char * s = NULL;
	if(option) {
		size_t n = sstrlen(option);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_putdimehdr(struct soap * soap)
{
	...
	if(soap->dime.options)
		optlen = (((uchar)soap->dime.options[2]<<8)|((uchar)soap->dime.options[3]))+4;
	if(soap->dime.id) {
		idlen = sstrlen(soap->dime.id);
		if(idlen > 0x0000FFFF)
			idlen = 0x0000FFFF;
	}
	if(soap->dime.type) {
		typelen = sstrlen(soap->dime.type);
...
static char * FASTCALL soap_getdimefield(struct soap * soap, size_t n)
{
	char * p = NULL;
	if(n) {
...
SOAP_FMAC1 int SOAP_FMAC2 soap_getdimehdr(struct soap * soap)
{
	...
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "DIME id=%s, type=%s, options=%s\n", soap->dime.id ? soap->dime.id : SOAP_STR_EOS,
		soap->dime.type ? soap->dime.type : "", soap->dime.options ? soap->dime.options+4 : SOAP_STR_EOS));
...
SOAP_FMAC1 int SOAP_FMAC2 soap_getdime(struct soap * soap)
{
	...
	for(;; ) {
		...
		if(soap->fdimewriteopen && ((soap->dime.ptr = (char *)soap->fdimewriteopen(soap, soap->dime.id, soap->dime.type, soap->dime.options)) || soap->error)) {
			...
			const char * id = soap->dime.id;
			const char * type = soap->dime.type;
...
static SOAP_FMAC1 int SOAP_FMAC2 soap_getmimehdr(struct soap * soap)
{
	struct soap_multipart * content;
	do {
		if(soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
	...
	if(soap->msgbuf[0] == '-' && soap->msgbuf[1] == '-') {
		char * s = soap->msgbuf+sstrlen(soap->msgbuf)-1;
		/* remove white space */
...
SOAP_FMAC1 void SOAP_FMAC2 soap_post_check_mime_attachments(struct soap * soap) { soap->imode |= SOAP_MIME_POSTCHECK; }
SOAP_FMAC1 int SOAP_FMAC2 soap_check_mime_attachments(struct soap * soap) { return (soap->mode&SOAP_MIME_POSTCHECK) ? (soap_get_mime_attachment(soap, NULL) != NULL) : SOAP_OK; }
...
SOAP_FMAC1 struct soap_multipart * SOAP_FMAC2 soap_get_mime_attachment(struct soap * soap, void * handle)
{
	...
	if(!content) {
		...
		if(soap->fmimewriteopen && ((content->ptr = (char *)soap->fmimewriteopen(soap, handle, content->id, content->type, content->description, content->encoding)) || soap->error)) {
			if(!content->ptr)
				return NULL;
		}
	}
	...
	for(;; ) {
		...
		if(content->ptr && soap->fmimewrite) {
			if((soap->error = soap->fmimewrite(soap, content->ptr, soap->tmpbuf, i)))
				break;
		}
	}
end:
	*s = '\0'; /* force 0-terminated */
	if(content->ptr) {
		if(!soap->error && soap->fmimewrite)
			soap->error = soap->fmimewrite(soap, content->ptr, soap->tmpbuf, i);
		if(soap->fmimewriteclose)
			soap->fmimewriteclose(soap, content->ptr);
		if(soap->error)
			return NULL;
	}
	else {
...
SOAP_FMAC1 int SOAP_FMAC2 soap_match_cid(struct soap * soap, const char * s, const char * t)
{
	...
	if(!s)
		...
		return (!strncmp(soap->tmpbuf, t, n) && !soap->tmpbuf[n]) ? 0 : 1;
	}
}

static void FASTCALL soap_resolve_attachment(struct soap * soap, struct soap_multipart * content)
{
	if(content->id) {
...
{
	struct soap_multipart * content = (struct soap_multipart*)soap_malloc(soap, sizeof(struct soap_multipart));
	if(content) {
		content->next = NULL;
		content->ptr = ptr;
		content->size = size;
...
SOAP_FMAC1 int SOAP_FMAC2 soap_set_dime_attachment(struct soap * soap, char * ptr, size_t size, const char * type, const char * id, ushort optype, const char * option)
{
...
SOAP_FMAC1 int SOAP_FMAC2 soap_set_mime_attachment(struct soap * soap, char * ptr, size_t size, enum soap_mime_encoding encoding,
...
static void soap_select_mime_boundary(struct soap * soap)
{
	...
	while(!soap->mime.boundary || soap_valid_mime_boundary(soap)) {
		...
		n -= 4;
		while(n) {
			*s++ = p_basis[soap_random&0x3F];
			n--;
		}
		strcpy(s, "==");
	}
	SETIFZ(soap->mime.start, "<SOAP-ENV:Envelope>");
}
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_begin_recv(struct soap * soap)
{
	soap_wchar c;
...
#ifndef PALM_2
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_envelope_begin_out(struct soap * soap)
{
#ifndef WITH_LEANER
	size_t n = 0;
	if((soap->mode&SOAP_ENC_MIME) && soap->mime.boundary && soap->mime.start && sstrlen(soap->mime.boundary)+sstrlen(soap->mime.start) < sizeof(soap->tmpbuf)-80) {
		const char * s;
		if((soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM))
			s = "application/dime";
		else if(soap->version == 2) {
	...
	return soap_element_begin_out(soap, "SOAP-ENV:Envelope", 0, 0);
}

SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_envelope_end_out(struct soap * soap)
{
	...
	if((soap->mode&SOAP_IO_LENGTH) && (soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM)) {
		...
		if(soap->local_namespaces) {
			soap->dime.type = soap->local_namespaces[0].out ? (char *)soap->local_namespaces[0].out : (char *)soap->local_namespaces[0].ns;
		}
		soap->dime.options = NULL;
		soap->dime.flags = SOAP_DIME_MB|SOAP_DIME_ABSURI;
		if(!soap->dime.first)
			soap->dime.flags |= SOAP_DIME_ME;
		soap->count += 12+((sstrlen(soap->dime.id)+3)&(~3))+(soap->dime.type ? ((sstrlen(soap->dime.type)+3)&(~3)) : 0);
	}
	if((soap->mode&SOAP_ENC_DIME) && !(soap->mode&SOAP_ENC_MTOM))
...
#ifndef PALM_1
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_envelope_begin_in(struct soap * soap)
{
	struct Namespace * p;
	soap->part = SOAP_IN_ENVELOPE;
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_envelope_end_in(struct soap * soap)
{
	soap->part = SOAP_END_ENVELOPE;
...
#ifndef PALM_2
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_body_begin_out(struct soap * soap)
{
	...
	if((soap->mode&SOAP_SEC_WSUID) && soap_set_attr(soap, "wsu:Id", "Body", 1))
		return soap->error;
 #endif
	return soap_element_begin_out(soap, "SOAP-ENV:Body", 0, 0);
}

SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_body_end_out(struct soap * soap)
{
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_body_begin_in(struct soap * soap)
{
	soap->part = SOAP_IN_BODY;
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_body_end_in(struct soap * soap)
{
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_recv_header(struct soap * soap)
{
...
SOAP_FMAC1 int SOAP_FMAC2 soap_connect_command(struct soap * soap, int http_command, const char * endpoints, const char * action)
{
	...
	if(endpoints && (s = sstrchr(endpoints, ' '))) {
		...
		for(;; ) {
			...
			endpoints = s;
			s = sstrchr(endpoints, ' ');
			SETIFZ(s, endpoints+sstrlen(endpoints));
		}
...
{ /* requires libntlm from http://www.nongnu.org/libntlm/ */
	...
	if(soap->ntlm_challenge && userid && passwd && soap->authrealm) {
		...
		if(!*soap->ntlm_challenge) {
			...
			soap->keep_alive = 1;
			soap->status = command;
			if(soap->fpost(soap, endpoint, host, port, NULL, NULL, 0) || soap_end_send(soap))
				return soap->error;
			soap->mode = m;
			soap->keep_alive = k;
...
#ifndef WITH_LEAN
SOAP_FMAC1 char * SOAP_FMAC2 soap_s2base64(struct soap * soap, const uchar * s, char * t, int n)
{
	int i;
	ulong m;
	char * p;
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_base642s(struct soap * soap, const char * s, char * t, size_t l, int * n)
{
	...
	if(!s || !*s) {
		ASSIGN_PTR(n, 0);
		return soap->error ? NULL : SOAP_NON_NULL;
	}
	if(!t) {
		l = (sstrlen(s)+3)/4*3+1; /* make sure enough space for \0 */
...
SOAP_FMAC1 const char * SOAP_FMAC2 soap_hex2s(struct soap * soap, const char * s, char * t, size_t l, int * n)
{
	...
	if(!s || !*s) {
		ASSIGN_PTR(n, 0);
		return soap->error ? 0 : SOAP_NON_NULL;
	}
	if(!t) {
		l = sstrlen(s)/2+1; /* make sure enough space for \0 */
		t = static_cast<char *>(soap_malloc(soap, l));
	}
	if(!t)
		return NULL;
	p = t;
...
{
	if(!oneof3(soap->status, SOAP_GET, SOAP_DEL, SOAP_CONNECT)) {
		const char * s = "text/xml; charset=utf-8";
		int err = SOAP_OK;
  #ifndef WITH_LEANER
		const char * r = NULL;
  #endif
		if((status == SOAP_FILE || oneof2(soap->status, SOAP_PUT, SOAP_POST_FILE)) && soap->http_content)
			s = soap->http_content;
		...
		if(soap->mode&(SOAP_ENC_DIME|SOAP_ENC_MTOM)) {
			if(soap->mode&SOAP_ENC_MTOM) {
				r = (soap->version == 2) ? "application/soap+xml" : "text/xml";
				s = "application/xop+xml";
			}
			else
				s = "application/dime";
		}
		if((soap->mode&SOAP_ENC_MIME) && soap->mime.boundary && sstrlen(soap->mime.boundary)+sstrlen(soap->mime.start ? soap->mime.start : SOAP_STR_EOS) < sizeof(soap->tmpbuf)-80) {
			const char * t = sstrchr(s, ';');
		...
		if(status == SOAP_OK && soap->version == 2 && soap->action && sstrlen(soap->action)+sstrlen(s) < sizeof(soap->tmpbuf)-80)
			sprintf(soap->tmpbuf+sstrlen(s), "; action=\"%s\"", soap->action);
  #endif
		err = soap->fposthdr(soap, "Content-Type", s);
		if(err)
			return err;
  #ifdef WITH_ZLIB
  ...
  #endif
		if(s) {
			sprintf(soap->tmpbuf, "%lu", (ulong)count);
...
SOAP_FMAC1 void SOAP_FMAC2 soap_set_fault(struct soap * soap)
{
	...
	if(!*c)
		*c = (soap->version == 2) ? "SOAP-ENV:Sender" : "SOAP-ENV:Client";
	if(*s)
		return;
	switch(soap->error) {
 #ifndef WITH_LEAN
		case SOAP_CLI_FAULT: *s = "Client fault"; break;
		case SOAP_SVR_FAULT: *s = "Server fault"; break;
		case SOAP_TAG_MISMATCH: *s = soap_set_validation_fault(soap, "tag name or namespace mismatch", 0); break;
		case SOAP_TYPE: *s = soap_set_validation_fault(soap, "data type mismatch ", soap->type); break;
		case SOAP_SYNTAX_ERROR: *s = "Well-formedness violation"; break;
		case SOAP_NO_TAG: *s = "No XML root element"; break;
		...
		case SOAP_DATAENCODINGUNKNOWN:
		    ...
		    *s = "Unsupported SOAP data encoding";
		    break;
		case SOAP_NAMESPACE:
		    *s = soap_set_validation_fault(soap, "namespace error", 0);
		    break;
		case SOAP_USER_ERROR: *s = "User data error"; break;
		case SOAP_FATAL_ERROR: *s = "Fatal error"; break;
		case SOAP_NO_METHOD:
		    sprintf(soap->msgbuf, "Method '%s' not implemented: method name or namespace not recognized", soap->tag);
		    *s = soap->msgbuf;
		    break;
		case SOAP_NO_DATA: *s = "Data required for operation"; break;
		...
		case SOAP_NULL: *s = soap_set_validation_fault(soap, "nil not allowed", 0); break;
		...
		case SOAP_MISSING_ID:
		    *s = soap_set_validation_fault(soap, "missing id for ref ", soap->id);
		    ...
		    break;
		case SOAP_HREF: *s = soap_set_validation_fault(soap, "incompatible object type ref/id pair ", soap->id); break;
		case SOAP_FAULT:
		    break;
  #ifndef WITH_NOIO
		case SOAP_UDP_ERROR: *s = "Message too large for UDP packet"; break;
		case SOAP_TCP_ERROR:
		    *s = tcp_error(soap);
		    break;
  #endif
		case SOAP_HTTP_ERROR: *s = "An HTTP processing error occurred"; break;
		case SOAP_NTLM_ERROR: *s = "An HTTP NTLM authentication error occurred"; break;
		case SOAP_SSL_ERROR:
  #ifdef WITH_OPENSSL
		    *s = "SSL/TLS error";
  #else
  ...
  #endif
		    break;
		case SOAP_PLUGIN_ERROR: *s = "Plugin registry error"; break;
		case SOAP_DIME_ERROR: *s = "DIME format error"; break;
		case SOAP_DIME_HREF: *s = "DIME href to missing attachment"; break;
		case SOAP_DIME_MISMATCH: *s = "DIME version/transmission error"; break;
		case SOAP_DIME_END: *s = "End of DIME error"; break;
		case SOAP_MIME_ERROR: *s = "MIME format error"; break;
		case SOAP_MIME_HREF: *s = "MIME href to missing attachment"; break;
		case SOAP_MIME_END: *s = "End of MIME error"; break;
  ...
  #endif
		    break;
		case SOAP_REQUIRED: *s = soap_set_validation_fault(soap, "missing required attribute", 0); break;
		case SOAP_PROHIBITED: *s = soap_set_validation_fault(soap, "prohibited attribute present", 0); break;
		case SOAP_OCCURS: *s = soap_set_validation_fault(soap, "occurrence violation", 0); break;
		case SOAP_LENGTH: *s = soap_set_validation_fault(soap, "content range or length violation", 0); break;
		case SOAP_FD_EXCEEDED: *s = "Maximum number of open connections was reached (no define HAVE_POLL): increase FD_SETSIZE";
		    break;
		case SOAP_UTF_ERROR: *s = "UTF content encoding error"; break;
		case SOAP_STOP: *s = "Stopped: no response to be sent or received (informative)"; break;
 #endif
		case SOAP_EOF:
 #ifndef WITH_NOIO
		    strcpy(soap->msgbuf, soap_strerror(soap));
  #ifndef WITH_LEAN
		    if(sstrlen(soap->msgbuf)+25 < sizeof(soap->msgbuf)) {
			    memmove(soap->msgbuf+25, soap->msgbuf, sstrlen(soap->msgbuf)+1);
...
SOAP_FMAC1 int SOAP_FMAC2 soap_send_fault(struct soap * soap)
{
	...
	if(status != SOAP_EOF || (!soap->recv_timeout && !soap->send_timeout)) {
		...
		if(r > 0) {
			...
			soap_end_count(soap);
			if(soap_response(soap, status) || soap_envelope_begin_out(soap) || soap_putheader(soap) || soap_body_begin_out(soap) ||
...
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_recv_fault(struct soap * soap, int check)
{
	int status = soap->error;
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Check if receiving SOAP Fault\n"));
	if(!check) { /* try getfault when no tag or tag mismatched at level 2, otherwise ret */
		if(soap->error != SOAP_NO_TAG && (soap->error != SOAP_TAG_MISMATCH || soap->level != 2))
			return soap->error;
	}
	soap->error = SOAP_OK;
	if(soap_getfault(soap)) { /* check flag set: check if SOAP Fault is present, if not just return */
...
static const char * soap_strerror(struct soap * soap)
{
	...
	if(err) {
		...
		*soap->msgbuf = '\0';
		...
		if(rt)
			sprintf(s+sstrlen(s), " (%d%cs receive delay)", rt, ru);
		if(st)
			sprintf(s+sstrlen(s), " (%d%cs send delay)", st, su);
  #endif
	}
	return soap->msgbuf;
}
...
static int soap_set_error(struct soap * soap, const char * faultcode, const char * faultsubcodeQName, const char * faultstring, const char * faultdetailXML, int soaperror)
{
	...
	*soap_faultstring(soap) = faultstring;
	if(faultdetailXML && *faultdetailXML) {
		...
		ASSIGN_PTR(s, faultdetailXML);
	}
	return soap->error = soaperror;
}

SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_set_sender_error(struct soap * soap, const char * faultstring, const char * faultdetailXML, int soaperror)
	{ return soap_set_error(soap, soap->version == 2 ? "SOAP-ENV:Sender" : "SOAP-ENV:Client", NULL, faultstring, faultdetailXML, soaperror); }
SOAP_FMAC1 int /*SOAP_FMAC2*/FASTCALL soap_set_receiver_error(struct soap * soap, const char * faultstring, const char * faultdetailXML, int soaperror)
...
{
	char * r = faultsubcodeQName ? soap_strdup(soap, faultsubcodeQName) : 0;
	char * s = faultstring ? soap_strdup(soap, faultstring) : 0;
	char * t = faultdetailXML ? soap_strdup(soap, faultdetailXML) : 0;
...
SOAP_FMAC1 void SOAP_FMAC2 soap_print_fault(struct soap * soap, FILE * fd)
{
	if(soap_check_state(soap))
		...
		fprintf(fd, "%s%d fault: %s [%s]\n\"%s\"\nDetail: %s\n", soap->version ? "SOAP 1." : "Error ",
			soap->version ? (int)soap->version : soap->error, *c, v ? v : "no subcode", s ? s : "[no reason]", d ? d : "[no detail]");
	}
}
#endif
#endif
...
//
			...
			SOAP_FMAC1 void SOAP_FMAC2 soap_stream_fault(struct soap * soap, std::ostream & os)
			{
				if(soap_check_state(soap))
					...
					s = *soap_faultstring(soap);
					d = soap_check_faultdetail(soap);
					os<<(soap->version ? "SOAP 1." : "Error ")<<(soap->version ? (int)soap->version : soap->error)<<" fault: "<<*c<<
					"["<<(v ? v : "no subcode")<<"]"<<std::endl<<"\""<<(s ? s : "[no reason]")<<"\""<<std::endl<<"Detail: "<<
					(d ? d : "[no detail]")<<std::endl;
				}
			}
		#endif
	#endif
	...
	if(soap_check_state(soap))
		...
		snprintf
			...
			(buf, len, "%s%d fault: %s [%s]\n\"%s\"\nDetail: %s\n", soap->version ? "SOAP 1." : "Error ",
			    soap->version ? (int)soap->version : soap->error, * c, v ? v : "no subcode", s ? s : "[no reason]", d ? d : "[no detail]");
  #else
		if(len > 40+sstrlen(v)+sstrlen(s)+sstrlen(d))
			sprintf(buf, "%s%d fault: %s [%s]\n\"%s\"\nDetail: %s\n", soap->version ? "SOAP 1." : "Error ",
				    soap->version ? (int)soap->version : soap->error, *c, v ? v : "no subcode",
				    s ? s : "[no reason]", d ? d : "[no detail]");
		else if(len > 40)
...
{
	for(struct soap_plugin * p = soap->plugins; p; p = p->next)
		if(p->id == id || sstreq(p->id, id))
			return p->data;
	return NULL;
}
#endif
#ifndef PALM_2
	SOAP_FMAC1 void * SOAP_FMAC2 soap_lookup_plugin(struct soap * soap, const char * id) { return soap->fplugin(soap, id); }
#endif
...
uchar soap::Get0() { return ((bufidx>=buflen && soap_recv(this)) ? EOF : (uchar)buf[bufidx]); }
uchar soap::Get1() { return ((bufidx>=buflen && soap_recv(this)) ? EOF : (uchar)buf[bufidx++]); }
#endif
