...
SOAP_FMAC1 SOAP_SOCKET SOAP_FMAC2 soap_accept(struct soap * soap)
{
	...
	for(;; ) {
		if(soap->accept_timeout || soap->send_timeout || soap->recv_timeout) {
			for(;; ) {
				...
				if(r < 0) {
					...
					if(r != SOAP_EINTR) {
						soap_closesock(soap);
						soap_set_sender_error(soap, tcp_error(
								    soap), "accept failed in soap_accept()", SOAP_TCP_ERROR);
						return SOAP_INVALID_SOCKET;
					}
				}
			}
		...
		if(soap_valid_socket(soap->socket)) {
			...
			soap->ip = (ulong)ip1<<24|(ulong)ip2<<16|(ulong)ip3<<8|
				    (ulong)ip4;
...
static int http_post(struct soap * soap, const char * endpoint, const char * host, int port, const char * path, const char * action, size_t count)
{
	...
	if(soap->proxy_userid && soap->proxy_passwd && strlen(soap->proxy_userid)+strlen(soap->proxy_passwd) < 761) {
		...
		{ strcpy(soap->tmpbuf, "Basic ");
		  ...
		  soap_s2base64(soap, (const uchar*)(soap->tmpbuf+262), soap->tmpbuf+6,
...
SOAP_FMAC1 void * SOAP_FMAC2 soap_id_forward(struct soap * soap, const char * href, void * p, size_t len, int st, int tt, size_t n,
	    unsigned int k, void (* fcopy)(struct soap *, int, int, void *, size_t, const void *, size_t))
{
	struct soap_ilist * ip;
	if(!p || !href || !*href)
		return p;
	...
	if(soap->mode&SOAP_IO) { /* need to flush the remaining data in buffer */
		...
		if(soap->mode&SOAP_ENC_ZLIB) {
			...
			if(deflateEnd(soap->d_stream) != Z_OK || r != Z_STREAM_END) {
				DBGLOG(TEST,
					    SOAP_MESSAGE(fdebug, "Unable to end deflate: %s\n",
						    soap->d_stream->msg ? soap->d_stream->msg : SOAP_STR_EOS));
				return soap->error = SOAP_ZLIB_ERROR;
			}
  #ifdef WITH_GZIP
...
SOAP_FMAC1 int SOAP_FMAC2 soap_recv_fault(struct soap * soap, int check)
{
	...
	if(soap_getfault(soap)) { /* check flag set: check if SOAP Fault is present, if not just return */
		...
		*soap_faultcode(soap) = (soap->version == 2 ? "SOAP-ENV:Sender" : "SOAP-ENV:Client");
		...
		else {DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Received SOAP Fault code %s\n", s));
		      status = SOAP_FAULT; }
...
uchar soap::Get0()
{
	return ((bufidx>=buflen && soap_recv(this)) ? EOF : (uchar)buf[bufidx]);
}
