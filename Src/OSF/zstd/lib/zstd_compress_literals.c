...
size_t ZSTD_noCompressLiterals(void * dst, size_t dstCapacity, const void * src, size_t srcSize)
{
	...
	DEBUGLOG(5, "Raw (uncompressed) literals: %u -> %u", (uint32)srcSize, (uint32)(srcSize + flSize));
...
#define COMPRESS_LITERALS_SIZE_MIN 63
	...
	switch(lhSize) {
		case 3: /* 2 - 2 - 10 - 10 */
			{   
				const uint32 lhc = hType + ((uint32)(!singleStream) << 2) + ((uint32)srcSize<<4) + ((uint32)cLitSize<<14);
	...
	DEBUGLOG(5, "Compressed literals: %u -> %u", (uint32)srcSize, (uint32)(lhSize+cLitSize));
