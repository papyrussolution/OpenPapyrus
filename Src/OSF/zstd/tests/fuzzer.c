...
static uint32 FUZ_highbit32(uint32 v32)
{
	uint nbBits = 0;
	if(v32==0) 
		return 0;
...
*   Test macros
   =============================================*/
#define CHECK_Z(f) { const size_t err = f; if(ZSTD_isError(err)) { DISPLAY("Error => %s : %s ", #f, ZSTD_getErrorName(err)); exit(1); }}
...
#define CHECK_OP(op, lhs, rhs) { if(!((lhs)op(rhs))) { DISPLAY("Error L%u => FAILED %s %s %s ", __LINE__, #lhs, #op, #rhs); goto _output_error; }}
...
static void * FUZ_mallocDebug(void * counter, size_t size)
{
	...
	void * const ptr = SAlloc::M(size);
	if(!ptr)
		return NULL;
	DISPLAYLEVEL(4, "allocating %u KB => effectively %u KB \n", (uint)(size >> 10), (uint)(malloc_size(ptr) >> 10)); /* OS-X specific */
...
{
	DISPLAYLEVEL(3, "peak:%6u KB,  nbMallocs:%2u, total:%6u KB \n", (uint)(count.peakMalloc >> 10), count.nbMalloc, (uint)(count.totalMalloc >> 10));
}

static int FUZ_mallocTests_internal(unsigned seed, double compressibility, unsigned part, void * inBuffer, size_t inSize, void * outBuffer, size_t outSize)
{
	...
	if(part <= 2) {
		for(int compressionLevel = 1; compressionLevel<=6; compressionLevel++) {
			mallocCounter_t malcount = INIT_MALLOC_COUNTER;
			ZSTD_customMem const cMem = { FUZ_mallocDebug, FUZ_freeDebug, &malcount };
	...
	if(part <= 3) {
		for(int nbThreads = 1; nbThreads<=4; nbThreads++) {
			for(int compressionLevel = 1; compressionLevel<=6; compressionLevel++) {
				mallocCounter_t malcount = INIT_MALLOC_COUNTER;
				ZSTD_customMem const cMem = { FUZ_mallocDebug, FUZ_freeDebug, &malcount };
				ZSTD_CCtx* const cctx = ZSTD_createCCtx_advanced(cMem);
				...
				ZSTD_freeCCtx(cctx);
				DISPLAYLEVEL(3, "compress_generic,-T%i,end level %i : ", nbThreads, compressionLevel);
				FUZ_displayMallocStats(malcount);
			}
		}
	}
	...
	if(part <= 4) {
		...
		for(nbThreads = 1; nbThreads<=4; nbThreads++) {
			...
			for(compressionLevel = 1; compressionLevel<=6; compressionLevel++) {
				...
				ZSTD_freeCCtx(cctx);
				DISPLAYLEVEL(3, "compress_generic,-T%i,continue level %i : ", nbThreads, compressionLevel);
				FUZ_displayMallocStats(malcount);
			}
		}
	}
...
{
	size_t const inSize = SMEGABYTE(64) + SMEGABYTE(16) + SMEGABYTE(4) + SMEGABYTE(1) + 
		SKILOBYTE(256) + SKILOBYTE(64); /* 85.3 MB */
	size_t const outSize = ZSTD_compressBound(inSize);
	void * const inBuffer = SAlloc::M(inSize);
	void * const outBuffer = SAlloc::M(outSize);
	int result;
	/* Create compressible noise */
	if(!inBuffer || !outBuffer) {
		DISPLAY("Not enough memory, aborting \n");
	...
	result = FUZ_mallocTests_internal(seed, compressibility, part, inBuffer, inSize, outBuffer, outSize);
	SAlloc::F(inBuffer);
	SAlloc::F(outBuffer);
	return result;
}
...
static void FUZ_decodeSequences(BYTE* dst, ZSTD_Sequence* seqs, size_t seqsSize, BYTE* src, size_t size, ZSTD_sequenceFormat_e format)
{
	for(size_t i = 0; i < seqsSize; ++i) {
		...
		if(seqs[i].offset != 0) {
			for(size_t j = 0; j < seqs[i].matchLength; ++j)
...
static int threadPoolTests(void) 
{
	int testResult = 0;
	size_t err;
	size_t const CNBuffSize = SMEGABYTE(5);
	void * const CNBuffer = SAlloc::M(CNBuffSize);
	size_t const compressedBufferSize = ZSTD_compressBound(CNBuffSize);
	void * const compressedBuffer = SAlloc::M(compressedBufferSize);
	void * const decodedBuffer = SAlloc::M(CNBuffSize);
	...
	{
		void * decodedBuffer2 = SAlloc::M(CNBuffSize);
		void * compressedBuffer2 = SAlloc::M(compressedBufferSize);
		ZSTD_threadPool* pool = ZSTD_createThreadPool(kPoolNumThreads);
		ZSTD_CCtx* cctx1 = ZSTD_createCCtx();
		ZSTD_CCtx* cctx2 = ZSTD_createCCtx();
		ZSTD_pthread_t t1;
		ZSTD_pthread_t t2;
		threadPoolTests_compressionJob_payload p1 = {cctx1, pool, CNBuffer, CNBuffSize, compressedBuffer, compressedBufferSize, decodedBuffer, 0 /* err */ };
		threadPoolTests_compressionJob_payload p2 = {cctx2, pool, CNBuffer, CNBuffSize, compressedBuffer2, compressedBufferSize, decodedBuffer2, 0 /* err */ };
		...
		SAlloc::F(decodedBuffer2);
		SAlloc::F(compressedBuffer2);
		ZSTD_freeThreadPool(pool);
		ZSTD_freeCCtx(cctx1);
		ZSTD_freeCCtx(cctx2);
		if(p1.err || p2.err) 
...
_end:
	SAlloc::F(CNBuffer);
	SAlloc::F(compressedBuffer);
	SAlloc::F(decodedBuffer);
	return testResult;
_output_error:
	testResult = 1;
...
// Unit tests
// 
static int basicUnitTests(const uint32 seed, double compressibility)
{
	const size_t CNBuffSize = SMEGABYTE(5);
	void * CNBuffer = SAlloc::M(CNBuffSize);
	const size_t compressedBufferSize = ZSTD_compressBound(CNBuffSize);
	void * const compressedBuffer = SAlloc::M(compressedBufferSize);
	void * const decodedBuffer = SAlloc::M(CNBuffSize);
	int testResult = 0;
	unsigned testNb = 0;
	...
	DISPLAYLEVEL(3, "test%3u : ZSTD_adjustCParams : ", testNb++);
	{
		...
		if(params.hashLog != 18) 
			goto _output_error;
		if(params.chainLog != 17) 
			goto _output_error;
	}
	DISPLAYLEVEL(3, "OK \n");
	DISPLAYLEVEL(3, "test%3u : compress %u bytes : ", testNb++, (uint)CNBuffSize);
	{
		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
		if(!cctx) 
			goto _output_error;
		CHECK_VAR(cSize, ZSTD_compressCCtx(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize, 1) );
	...
	DISPLAYLEVEL(3, "OK \n");
	DISPLAYLEVEL(3, "test%3i : decompress %u bytes : ", testNb++, (uint)CNBuffSize);
	{ 
	...
	DISPLAYLEVEL(3, "test%3i : misc unaccounted for zstd symbols : ", testNb++);
	{
		...
		* to do it...) So we have to fall back to our trusty friend memcpy. */
		unsigned (*const funcptr_getDictID)(const ZSTD_DDict* ddict) = ZSTD_getDictID_fromDDict;
		ZSTD_DStream* (*const funcptr_createDStream)(ZSTD_customMem customMem) = ZSTD_createDStream_advanced;
		void (*const funcptr_copyDCtx)(ZSTD_DCtx* dctx, const ZSTD_DCtx* preparedDCtx) = ZSTD_copyDCtx;
		ZSTD_nextInputType_e(*const funcptr_nextInputType)(ZSTD_DCtx* dctx) = ZSTD_nextInputType;
		...
		memcpy((void *)&voidptr_getDictID, (const void *)&funcptr_getDictID, sizeof(void *));
		memcpy((void *)&voidptr_createDStream, (const void *)&funcptr_createDStream, sizeof(void *));
		memcpy((void *)&voidptr_copyDCtx, (const void *)&funcptr_copyDCtx, sizeof(void *));
		memcpy((void *)&voidptr_nextInputType, (const void *)&funcptr_nextInputType, sizeof(void *));
	...
	{ 
		...
		CHECK_VAR(cSize, ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize) );
		ZSTD_freeCCtx(cctx);
	}
	...
	DISPLAYLEVEL(3, "test%3d: check DCtx size is reduced after many oversized calls : ", testNb++);
	{
		...
		size_t srcSize = ZSTD_compress((void *)dst, compressedBufferSize, CNBuffer, largeFrameSrcSize, 3);
		for(i = 0; i < nbFrames; i++)
			srcSize += ZSTD_compress((void *)(dst + srcSize), compressedBufferSize - srcSize, CNBuffer, smallFrameSrcSize, 3);
	...
	DISPLAYLEVEL(3, "test%3i : ldm fill dict out-of-bounds check", testNb++);
	{
		...
		void * dict = SAlloc::M(size);
		void * src = SAlloc::M(size);
		void * dst = SAlloc::M(dstCapacity);
	...
	DISPLAYLEVEL(3, "test%3i : testing dict compression with enableLdm and forceMaxWindow : ", testNb++);
	{
		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
		ZSTD_DCtx* const dctx = ZSTD_createDCtx();
		void * dict = SAlloc::M(CNBuffSize);
		for(int nbWorkers = 0; nbWorkers < 3; ++nbWorkers) {
	...
	DISPLAYLEVEL(3, "test%3i : testing dict compression for determinism : ", testNb++);
	{
		...
		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
		ZSTD_DCtx* const dctx = ZSTD_createDCtx();
		char* dict = (char *)SAlloc::M(2 * testSize);
	...
	{   
		ZSTD_CCtx* cctx = ZSTD_createCCtx();
	    ZSTD_DCtx* dctx = ZSTD_createDCtx();
	    size_t const srcSize = SKILOBYTE(300);
	    size_t const flushSize = SKILOBYTE(128) + 5;
	    ...
	    memcpy(src + SKILOBYTE(125), src, SKILOBYTE(3) + 5);
	...
	DISPLAYLEVEL(3, "test%3i : testing refPrefx vs refPrefx + ldm (size comparison) : ", testNb++);
	{
		...
		size_t const size = SMEGABYTE(100);
		int const windowLog = 27;
		size_t const dstSize = ZSTD_compressBound(size);
		void * dict = SAlloc::M(size);
		void * src = SAlloc::M(size);
		void * dst = SAlloc::M(dstSize);
		void * recon = SAlloc::M(size);
	...
	{   
	    ...
	    if(superCSize > normalCSize + allowedExpansion) {
		    DISPLAYLEVEL(1, "Superblock too big: %u > %u + %u \n", (uint32)superCSize, (uint32)normalCSize, (uint32)allowedExpansion);
		    goto _output_error;
	    }
	    ZSTD_freeCCtx(cctx);
	}
	...
	{   
	    ...
	    {   
			size_t const inSize = 2* SKILOBYTE(128);
			size_t const outSize = ZSTD_compressBound(inSize);
	...
	{   
	    ...
	    size_t const nbCompressions = ((1U << 31) / CNBuffSize) + 2; // ensure uint32 overflow protection is triggered 
	    size_t cnb;
	    assert(cctx);
		...
		CHECK_Z(ZSTD_compressEnd(largeCCtx, compressedBuffer, compressedBufferSize, CNBuffer, 1) );
		{
			size_t const largeCCtxSize = ZSTD_sizeof_CCtx(largeCCtx);   /* size of context must be measured after compression */
			{
				...
				CHECK_Z(ZSTD_compressCCtx(smallCCtx, compressedBuffer, compressedBufferSize, CNBuffer, 1, 1));
				{
					...
					DISPLAYLEVEL(5, "(large) %zuKB > 32*%zuKB (small) : ", largeCCtxSize>>10, smallCCtxSize>>10);
					assert(largeCCtxSize > 32* smallCCtxSize); // note : "too large" definition is handled within zstd_compress.c . make this test case extreme, so
						// that it doesn't depend on a possibly fluctuating definition 
				}
				ZSTD_freeCCtx(smallCCtx);
			}
			{
				const uint32 maxNbAttempts = 1100;   /* nb of usages before triggering size down is handled
...
#define STATIC_CCTX_LEVEL 4
	DISPLAYLEVEL(3, "test%3i : create static CCtx for level %u : ", testNb++, STATIC_CCTX_LEVEL);
	{
		...
		void * const staticCCtxBuffer = SAlloc::M(staticCStreamSize);
		size_t const staticDCtxSize = ZSTD_estimateDCtxSize();
		void * const staticDCtxBuffer = SAlloc::M(staticDCtxSize);
		DISPLAYLEVEL(4, "CStream size = %u, ", (uint32)staticCStreamSize);
		if(staticCCtxBuffer==NULL || staticDCtxBuffer==NULL) {
			SAlloc::F(staticCCtxBuffer);
			SAlloc::F(staticDCtxBuffer);
			DISPLAY("Not enough memory, aborting\n");
			testResult = 1;
			goto _end;
		}
		{
			size_t const smallInSize = SKILOBYTE(32);
			...
			DISPLAYLEVEL(4, "Full CCtx size = %u, ", (uint32)staticCCtxSize);
			DISPLAYLEVEL(4, "CCtx for 32 KB = %u, ", (uint32)smallCCtxSize);
			if((staticCCtx==NULL) || (staticDCtx==NULL))
				goto _output_error;
			DISPLAYLEVEL(3, "OK \n");
			DISPLAYLEVEL(3, "test%3i : compress small input with small static CCtx : ", testNb++);
			CHECK_VAR(cSize,
			    ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize, CNBuffer, smallInSize,
			    STATIC_CCTX_LEVEL) );
			DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (uint)cSize, (double)cSize/smallInSize*100);
			...
			CHECK_VAR(cSize,
			    ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize,
			    STATIC_CCTX_LEVEL) );
			DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (uint)cSize, (double)cSize/CNBuffSize*100);
			DISPLAYLEVEL(3, "test%3i : compress small input often enough to trigger context reduce : ", testNb++);
			{
				int nbc;
				assert(staticCCtxSize > smallCCtxSize * ZSTD_WORKSPACETOOLARGE_FACTOR); /* ensure size
				                                                                           down scenario
				                                                                           */
			...
			{   ZSTD_outBuffer output = { compressedBuffer, compressedBufferSize, 0 };
			    ...
			    size_t const r = ZSTD_compressStream(staticCCtx, &output, &input); /* now allocates, should
				                                                                  fail */
			    if(!ZSTD_isError(r)) goto _output_error; }
			DISPLAYLEVEL(3, "OK \n");
			...
			{ 
				size_t const r = ZSTD_initCStream_usingDict(staticCCtx, CNBuffer, SKILOBYTE(64), 1);
			  if(!ZSTD_isError(r)) goto _output_error; 
			}
			DISPLAYLEVEL(3, "OK \n");
	...
	{   
	    ...
	    DISPLAYLEVEL(3, "test%3u : compress %u bytes with 2 threads : ", testNb++, (uint)CNBuffSize);
	...
	DISPLAYLEVEL(3, "test%3i : setting multithreaded parameters : ", testNb++)
	{
		...
		int const jobSize = SKILOBYTE(512);
		int value;
	...
	{   
	    ...
	    const uint32 skipLen = SKILOBYTE(129);
	    char* const skipBuff = (char *)SAlloc::M(skipLen);
	    assert(skipBuff != NULL);
	    memzero(skipBuff, skipLen);
	    for(i = 0; i < segs; i++) {
		    CHECK_NEWV(r, ZSTD_compress((BYTE*)compressedBuffer + off, CNBuffSize - off, (BYTE*)CNBuffer + segSize * (size_t)i, segSize, 5) );
		    off += r;
		    if(i == segs/2) {
			    /* insert skippable frame */
			    size_t const skippableSize = ZSTD_writeSkippableFrame((BYTE*)compressedBuffer + off, compressedBufferSize, skipBuff, skipLen, seed % 15);
	...
	{   uint32 i;
	    ...
	    if(memcmp(decodedBuffer, skipBuff, skipLen) != 0) 
			goto _output_error;
	    SAlloc::F(skipBuff);
	}
	DISPLAYLEVEL(3, "OK \n");
	...
	/* Dictionary and CCtx Duplication tests */
	{
		...
		CHECK(ZSTD_copyCCtx(ctxDuplicated, ctxOrig, 0) ); /* Begin_usingDict implies unknown srcSize, so match
		                                                     that */
		DISPLAYLEVEL(3, "OK \n");
		...
		CHECKPLUS(r, ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, CNBuffer, dictSize),
		...
		DISPLAYLEVEL(3, "test%3i : decompress with static DDict : ", testNb++);
		{
			...
			void * const ddictBuffer = SAlloc::M(ddictBufferSize);
			if(ddictBuffer == NULL)
				goto _output_error;
			{
				const ZSTD_DDict* const ddict = ZSTD_initStaticDDict(ddictBuffer, ddictBufferSize, CNBuffer,
					dictSize, ZSTD_dlm_byCopy, ZSTD_dct_auto);
				size_t const r = ZSTD_decompress_usingDDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, ddict);
				if(r != CNBuffSize - dictSize) 
					goto _output_error;
			}
			SAlloc::F(ddictBuffer);
			DISPLAYLEVEL(3, "OK (size of static DDict : %u) \n", (uint)ddictBufferSize);
		}
		...
		{   
		    ...
		    {   
				...
				if(ZSTD_getFrameHeader(&zfh, compressedBuffer, cSize)) 
					goto _output_error;
				if((zfh.frameContentSize != testSize) && (zfh.frameContentSize != 0)) 
					goto _output_error; 
			}
		}
		DISPLAYLEVEL(3, "OK \n");
		...
		if((int)(compressibility * 100 + 0.1) == FUZ_compressibility_default) { /* test only valid with known input */
			...
			DISPLAYLEVEL(4, "content hash : %016llx;  dict hash : %016llx \n",
			    (unsigned long long)XXH64(contentStart, contentSize, 0), (unsigned long long)XXH64(dict, flatdictSize, 0));
			for(; l <= maxLevel; l++) {
				size_t const nodict_cSize = ZSTD_compress(compressedBuffer, compressedBufferSize, contentStart, contentSize, l);
				if(nodict_cSize > target_nodict_cSize[l]) {
					DISPLAYLEVEL(1, "error : compression at level %i worse than expected (%u > %u) \n", l, (uint)nodict_cSize, (uint)target_nodict_cSize[l]);
					goto _output_error;
				}
				DISPLAYLEVEL(4, "level %i : max expected %u >= reached %u \n", l, (uint)target_nodict_cSize[l], (uint)nodict_cSize);
			}
			for(l = 1; l <= maxLevel; l++) {
				size_t const wdict_cSize = ZSTD_compress_usingDict(ctxOrig, compressedBuffer, compressedBufferSize, contentStart, contentSize, dict, flatdictSize, l);
				if(wdict_cSize > target_wdict_cSize[l]) {
					DISPLAYLEVEL(1, "error : compression with dictionary at level %i worse than expected (%u > %u) \n", l, (uint)wdict_cSize, (uint)target_wdict_cSize[l]);
					goto _output_error;
				}
				DISPLAYLEVEL(4, "level %i with dictionary : max expected %u >= reached %u \n", l, (uint)target_wdict_cSize[l], (uint)wdict_cSize);
			}
			 ...
			 */
			for(; rowLevel <= rowLevelEnd; ++rowLevel) 
				target_nodict_cSize[rowLevel] += 5;
			for(l = 1; l <= maxLevel; l++) {
				ZSTD_CCtx* const cctx = ZSTD_createCCtx();
				size_t nodict_cSize;
				ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, l);
				ZSTD_CCtx_setParameter(cctx, ZSTD_c_useRowMatchFinder, ZSTD_ps_enable);
				nodict_cSize = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, contentStart, contentSize);
				if(nodict_cSize > target_nodict_cSize[l]) {
			...
			for(l = 1; l <= maxLevel; l++) {
				...
				if(wdict_cSize > target_wdict_cSize[l]) {
					DISPLAYLEVEL(1,
					    "error : compression with dictionary and compress2 at level %i worse than expected (%u > %u) \n",
					    l,
					    (uint)wdict_cSize,
					    (uint)target_wdict_cSize[l]);
					goto _output_error;
				}
	...
	{   ZSTD_CCtx* const cctx = ZSTD_createCCtx();
	    size_t const dictBufferCapacity = SKILOBYTE(16);
	    void * const dictBuffer = SAlloc::M(dictBufferCapacity);
	    size_t const totalSampleSize = SMEGABYTE(1);
	    size_t const sampleUnitSize = SKILOBYTE(8);
	    const uint32 nbSamples = (uint32)(totalSampleSize / sampleUnitSize);
	    size_t* const samplesSizes = (size_t*)SAlloc::M(nbSamples * sizeof(size_t));
	    size_t dictSize;
	    uint32 dictID;
	    ...
	    if(dictBuffer==NULL || samplesSizes==NULL) {
		    SAlloc::F(dictBuffer);
		    SAlloc::F(samplesSizes);
		    goto _output_error;
	    }

	    DISPLAYLEVEL(3, "test%3i : dictBuilder on cyclic data : ", testNb++);
	    assert(compressedBufferSize >= totalSampleSize);
	    { uint32 u; for(u = 0; u<totalSampleSize; u++) ((BYTE*)decodedBuffer)[u] = (BYTE)u; }
	    { uint32 u; for(u = 0; u<nbSamples; u++) samplesSizes[u] = sampleUnitSize; }
	    {   size_t const sDictSize = ZDICT_trainFromBuffer(dictBuffer, dictBufferCapacity, decodedBuffer, samplesSizes, nbSamples);
	    ...
	    { 
			for(uint32 u = 0; u<nbSamples; u++) 
				samplesSizes[u] = sampleUnitSize; 
		}
		dictSize = ZDICT_trainFromBuffer(dictBuffer, dictBufferCapacity, CNBuffer, samplesSizes, nbSamples);
		...
		}
	    {
		    ...
		    memzero(&coverParams, sizeof(coverParams));
		    coverParams.steps = 8;
		    coverParams.nbThreads = 4;
		    dictSize = ZDICT_optimizeTrainFromBuffer_cover(dictBuffer, dictBufferCapacity, CNBuffer, samplesSizes, nbSamples/8, /* less samples for faster tests */ &coverParams);
		    if(ZDICT_isError(dictSize)) 
				goto _output_error;
	    }
	    DISPLAYLEVEL(3, "OK, created dictionary of size %u \n", (uint)dictSize);
	    ...
	    }
	    {
		    ...
		    memzero(&fastCoverParams, sizeof(fastCoverParams));
		    fastCoverParams.steps = 8;
		    fastCoverParams.nbThreads = 4;
		    dictSize = ZDICT_optimizeTrainFromBuffer_fastCover(dictBuffer, dictBufferCapacity, CNBuffer, samplesSizes, nbSamples, &fastCoverParams);
		    if(ZDICT_isError(dictSize))
			    goto _output_error;
	    }
	    DISPLAYLEVEL(3, "OK, created dictionary of size %u \n", (uint)dictSize);
	    ...
	    {   
			...
			if(dictHeaderSize != 115) 
				goto _output_error; 
		}
	    DISPLAYLEVEL(3, "OK : %u \n", (uint)dictHeaderSize);
	    DISPLAYLEVEL(3, "test%3i : compress with dictionary : ", testNb++);
	    cSize = ZSTD_compress_usingDict(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize, dictBuffer, dictSize, 4);
	    if(ZSTD_isError(cSize)) 
			goto _output_error;
	    DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (uint)cSize, (double)cSize/CNBuffSize*100);
	    DISPLAYLEVEL(3, "test%3i : retrieve dictID from dictionary : ", testNb++);
	    {   
			const uint32 did = ZSTD_getDictID_fromDict(dictBuffer, dictSize);
			if(did != dictID) 
	    ...
	    {   
			const uint32 did = ZSTD_getDictID_fromFrame(compressedBuffer, cSize);
			if(did != dictID) 
				goto _output_error; /* non-conformant (content-only) dictionary */
	    }
	    DISPLAYLEVEL(3, "OK \n");
	    DISPLAYLEVEL(3, "test%3i : frame built with dictionary should be decompressible : ", testNb++);
	    {
		    ZSTD_DCtx* const dctx = ZSTD_createDCtx(); 
			assert(dctx != NULL);
		    CHECKPLUS(r, ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dictBuffer, dictSize), { if(r != CNBuffSize) goto _output_error; });
		    ZSTD_freeDCtx(dctx);
	    }
	    DISPLAYLEVEL(3, "OK \n");
	    ...
	    DISPLAYLEVEL(3, "test%3i : compress with CDict ", testNb++);
	    {
		    ...
		    assert(cdict != NULL);
		    DISPLAYLEVEL(3, "(size : %u) : ", (uint)ZSTD_sizeof_CDict(cdict));
		    assert(ZSTD_getDictID_fromDict(dictBuffer, dictSize) == ZSTD_getDictID_fromCDict(cdict));
		    cSize = ZSTD_compress_usingCDict(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize, cdict);
		    ZSTD_freeCDict(cdict);
		    if(ZSTD_isError(cSize)) 
				goto _output_error;
	    }
	    ...
	    DISPLAYLEVEL(3, "test%3i : frame built with dictionary should be decompressible : ", testNb++);
	    {
			...
			assert(dctx != NULL);
		    CHECKPLUS(r,
			ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dictBuffer, dictSize), { if(r != CNBuffSize) goto _output_error; });
		    ZSTD_freeDCtx(dctx);
	    }
	    DISPLAYLEVEL(3, "OK \n");
	    DISPLAYLEVEL(3, "test%3i : compress with static CDict : ", testNb++);
	    {
		    ...
		    for(int level = 1; level <= maxLevel; ++level) {
			    ...
			    void * const cdictBuffer = SAlloc::M(cdictSize);
			    if(cdictBuffer==NULL) 
					goto _output_error;
			    {
				    const ZSTD_CDict* const cdict = ZSTD_initStaticCDict(cdictBuffer, cdictSize, dictBuffer, dictSize, ZSTD_dlm_byCopy, ZSTD_dct_auto, cParams);
				    if(cdict == NULL) {
					    DISPLAY("ZSTD_initStaticCDict failed ");
					    goto _output_error;
				    }
				    cSize = ZSTD_compress_usingCDict(cctx, compressedBuffer, compressedBufferSize, CNBuffer, MIN(SKILOBYTE(10), CNBuffSize), cdict);
				    if(ZSTD_isError(cSize)) {
					    DISPLAY("ZSTD_compress_usingCDict failed ");
					    goto _output_error;
				    }
			    }
			    SAlloc::F(cdictBuffer);
		    }
	    }
	    DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (uint)cSize, (double)cSize/CNBuffSize*100);
	    ...
	    {
		    uint64 const contentSize = ZSTD_getFrameContentSize(compressedBuffer, cSize);
	    ...
	    DISPLAYLEVEL(3, "test%3i : frame built without dictID should be decompressible : ", testNb++);
	    {
		    ZSTD_DCtx* const dctx = ZSTD_createDCtx();
		    assert(dctx != NULL);
		    CHECKPLUS(r, ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dictBuffer, dictSize),
			{ if(r != CNBuffSize) goto _output_error; });
		    ZSTD_freeDCtx(dctx);
	    }
	    DISPLAYLEVEL(3, "OK \n");
	    DISPLAYLEVEL(3, "test%3i : ZSTD_compress_advanced, no dictID : ", testNb++);
	    {
		    ...
		    cSize = ZSTD_compress_advanced(cctx,
			    compressedBuffer,
			    compressedBufferSize,
			    CNBuffer,
			    CNBuffSize,
	    ...
	    DISPLAYLEVEL(3, "test%3i : dictionary containing only header should return error : ", testNb++);
	    {
		    ...
		    assert(dctx != NULL);
		    {
			    const size_t ret = ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize,
	    ...
	    DISPLAYLEVEL(3, "OK \n");
	    {
		    char* rawDictBuffer = (char *)SAlloc::M(dictSize);
	    ...
	    DISPLAYLEVEL(3, "test%3i : Loading dictionary before setting parameters is the same as loading after : ", testNb++);
	    {
		    ...
		    ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
		    CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, 7) );
		    CHECK_Z(ZSTD_CCtx_loadDictionary(cctx, CNBuffer, MIN(CNBuffSize, SKILOBYTE(10))) );
		    size1 = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, MIN(CNBuffSize, SKILOBYTE(100)));
		    if(ZSTD_isError(size1)) goto _output_error;

		    ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
		    CHECK_Z(ZSTD_CCtx_loadDictionary(cctx, CNBuffer, MIN(CNBuffSize, SKILOBYTE(10))) );
		    CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, 7) );
		    size2 = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, MIN(CNBuffSize, SKILOBYTE(100)));
	    ...
	    cSize = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, MIN(CNBuffSize, SKILOBYTE(100)));
	    CHECK_Z(cSize);
		 ...
		 */
	    {
		    ...
		    cSize = ZSTD_compress_usingCDict(cctx, compressedBuffer, compressedBufferSize, data, sizeof(data), cdict);
		    ZSTD_freeCDict(cdict);
		    if(ZSTD_isError(cSize)) {
			    DISPLAYLEVEL(5, "Compression error %s : ", ZSTD_getErrorName(cSize)); goto _output_error;
		    }
		    dSize = ZSTD_decompress_usingDict(dctx, decodedBuffer, sizeof(data), compressedBuffer, cSize, dictBuffer, dictSize);
		    if(ZSTD_isError(dSize)) {
			    DISPLAYLEVEL(5, "Decompression error %s : ", ZSTD_getErrorName(dSize)); goto _output_error;
		    }
	    ...
	    DISPLAYLEVEL(3, "test%3i : ZSTD_decompressDCtx() with multiple ddicts : ", testNb++);
	    {
		    ...
		    size_t i;
		    ZSTD_DCtx* dctx = ZSTD_createDCtx();
		    ZSTD_DDict** ddictTable = (ZSTD_DDict**)SAlloc::M(sizeof(ZSTD_DDict*)*numDicts);
		    ZSTD_CDict** cdictTable = (ZSTD_CDict**)SAlloc::M(sizeof(ZSTD_CDict*)*numDicts);
		    uint32 dictIDSeed = seed;
		    /* Create new compressed buffer that will hold frames with differing dictIDs */
		    char* dictBufferMulti = (char *)SAlloc::M(sizeof(char) * dictBufferFixedSize); // Modifiable copy of fixed full dict buffer 
		    memcpy(dictBufferMulti, dictBufferFixed, dictBufferFixedSize);
		    ...
		    for(i = 0; i < numDicts; ++i) {
				...
				ZSTD_getDictID_fromCDict(cdictTable[i]) != ZSTD_getDictID_fromDDict(ddictTable[i])) {
				    goto _output_error;
			    }
		    }
		    /* Compress a few frames using random CDicts */
		    {
			    ...
			    for(i = 0; i < numFrames; i++) {
				    ...
				    ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
				    {   
						CHECK_NEWV(r, ZSTD_compress_usingCDict(cctx, (BYTE*)compressedBuffer + off, CNBuffSize - off, (BYTE*)CNBuffer + segSize * (size_t)i, segSize, cdictTable[dictIdx]));
						off += r;
					}
			    }
			    cSize = off;
	...
	{   
		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
	    size_t dictSize = SKILOBYTE(16);
	    size_t optDictSize = dictSize;
	    void * dictBuffer = SAlloc::M(dictSize);
	    size_t const totalSampleSize = SMEGABYTE(1);
	    size_t const sampleUnitSize = SKILOBYTE(8);
	    const uint32 nbSamples = (uint32)(totalSampleSize / sampleUnitSize);
	    size_t* const samplesSizes = (size_t*)SAlloc::M(nbSamples * sizeof(size_t));
	    ...
	    dictSize = ZDICT_trainFromBuffer_cover(dictBuffer, dictSize, CNBuffer, samplesSizes, nbSamples, params);
	    if(ZDICT_isError(dictSize)) 
			goto _output_error;
	    DISPLAYLEVEL(3, "OK, created dictionary of size %u \n", (uint)dictSize);
	    DISPLAYLEVEL(3, "test%3i : check dictID : ", testNb++);
	    ...
	    memzero(&params, sizeof(params));
	    params.steps = 4;
	    optDictSize = ZDICT_optimizeTrainFromBuffer_cover(dictBuffer, optDictSize, CNBuffer, samplesSizes, nbSamples / 4, &params);
	...
	{   
	    ...
	    {   
			...
			{   
				size_t const compressionResult = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, srcSize);
	...
	{   
	    ...
	    {   
			...
			{   
				size_t const compressedSize = ZSTD_compress2(cctx, compressedBuffer, ZSTD_compressBound(inputSize), CNBuffer, inputSize);
				CHECK(compressedSize);
				cSize = compressedSize;
				xxh64 = XXH64(compressedBuffer, compressedSize, 0);
			}
			DISPLAYLEVEL(3, "OK (compress : %u -> %u bytes)\n", (uint)inputSize, (uint)cSize);
			ZSTD_freeCCtx(cctx);
		}
	    {   
			ZSTD_CCtx* cctx = ZSTD_createCCtx();
			...
			CHECK(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, 2) );
			{   
				size_t const result = ZSTD_compress2(cctx, compressedBuffer, ZSTD_compressBound(inputSize), CNBuffer, inputSize);
				CHECK(result);
				if(result != cSize) 
					goto _output_error; /* must result in same compressed result, hence same size */
				if(XXH64(compressedBuffer, result, 0) != xxh64) 
					goto _output_error; // must result in exactly same content, hence same hash 
				DISPLAYLEVEL(3, "OK (compress : %u -> %u bytes)\n", (uint)inputSize, (uint)result);
			}
			ZSTD_freeCCtx(cctx);
		}
	}
	...
	{   ZSTD_CCtx* const cctx = ZSTD_createCCtx();
	    ZSTD_DCtx* const dctx = ZSTD_createDCtx();
	    static const size_t dictSize = SKILOBYTE(65);
	    static const size_t blockSize = SKILOBYTE(100); /* won't cause pb with small dict size */
	    size_t cSize2;
	    assert(cctx); assert(dctx != NULL);
	    ...
	    CHECK(ZSTD_compressBegin(cctx, -199) ); /* we just want to quickly overflow internal uint32 index */
	    CHECK(ZSTD_getBlockSize(cctx) >= blockSize);
	    {   
		uint64 const toCompress = 5000000000ULL;/* > 4 GB */
		uint64 compressed = 0;
		while(compressed < toCompress) {
			size_t const blockCSize = ZSTD_compressBlock(cctx,
				compressedBuffer,
	    ...
	    CHECK_VAR(cSize,
		ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize), (char *)CNBuffer+dictSize, blockSize));
	    RDG_genBuffer((char *)CNBuffer+dictSize+blockSize, blockSize, 0.0, 0.0, seed); /* create a non-compressible
		                                                                             second block */
	    { CHECK_NEWV(r,
		  ZSTD_compressBlock(cctx, (char *)compressedBuffer+cSize, ZSTD_compressBound(blockSize),
		  (char *)CNBuffer+dictSize+blockSize, blockSize) );                                                                                               /*
			                                                                                                                                             for
			                                                                                                                                             cctx
			                                                                                                                                             history
			                                                                                                                                             consistency
	    ...
	    memcpy((char *)compressedBuffer+cSize, (char *)CNBuffer+dictSize+blockSize, blockSize); /* send non-compressed
		                                                                                     block (without
		                                                                                     header) */
	    ...
	    ZSTD_insertBlock(dctx, (char *)decodedBuffer+blockSize, blockSize); /* insert non-compressed block into dctx
		                                                                  history */
	    {   CHECK_NEWV(r,
		    ZSTD_decompressBlock(dctx, (char *)decodedBuffer+2*blockSize, blockSize, (char *)compressedBuffer+cSize+blockSize,
		    cSize2) );
		if(r != blockSize) {
			DISPLAYLEVEL(1,
			    "ZSTD_decompressBlock() with _usingDict() and after insertBlock() fails : %u, instead of %u expected \n",
			    (uint)r, (uint)blockSize);
			goto _output_error;
		}
	    }
	...
	/* long rle test */
	{
		...
		memset((char *)CNBuffer+sampleSize, 'B', SKILOBYTE(256) - 1);
		sampleSize += SKILOBYTE(256) - 1;
		memset((char *)CNBuffer+sampleSize, 'A', SKILOBYTE(96));
		sampleSize += SKILOBYTE(96);
	...
	{
		size_t srcSize = SKILOBYTE(150);
		BYTE* src = (BYTE*)CNBuffer;
		BYTE* decoded = (BYTE*)compressedBuffer;
		ZSTD_CCtx* cctx = ZSTD_createCCtx();
		ZSTD_Sequence* seqs = (ZSTD_Sequence*)SAlloc::M(srcSize * sizeof(ZSTD_Sequence));
		size_t seqsSize;
		if(seqs == NULL) 
			goto _output_error;
		assert(cctx);
	...
	{
		const size_t srcSize = SKILOBYTE(500);
		const BYTE* const src = (BYTE*)CNBuffer;
		BYTE* const dst = (BYTE*)compressedBuffer;
		const size_t dstCapacity = ZSTD_compressBound(srcSize);
		const size_t decompressSize = srcSize;
		char* const decompressBuffer = (char *)SAlloc::M(decompressSize);
		size_t compressedSize;

		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
		ZSTD_Sequence* const seqs = (ZSTD_Sequence*)SAlloc::M(srcSize * sizeof(ZSTD_Sequence));
		size_t nbSeqs;
	...
	/* creates a buffer full of 3-bytes sequences */
	{
		...
		{
			for(int i = 0; i < _3BYTESTESTLENGTH; i += 3) { /* note : CNBuffer size > _3BYTESTESTLENGTH+3 */
				const uint32 id = FUZ_rand(&rSeed) & NB3BYTESSEQMASK;
	...
	CHECK_VAR(cSize, ZSTD_compress(compressedBuffer, ZSTD_compressBound(_3BYTESTESTLENGTH), CNBuffer, _3BYTESTESTLENGTH, 19) );
	DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (uint)cSize, (double)cSize/_3BYTESTESTLENGTH*100);
	...
	{ /* Train a dictionary on low characters */
		size_t dictSize = SKILOBYTE(16);
		void * const dictBuffer = SAlloc::M(dictSize);
		size_t const totalSampleSize = SMEGABYTE(1);
		size_t const sampleUnitSize = SKILOBYTE(8);
		const uint32 nbSamples = (uint32)(totalSampleSize / sampleUnitSize);
		size_t* const samplesSizes = (size_t*)SAlloc::M(nbSamples * sizeof(size_t));
		...
		{ /* Compress the data */
			...
			void * const outputBuffer = SAlloc::M(outputSize);
			ZSTD_CCtx* const cctx = ZSTD_createCCtx();
	...
	RDG_genBuffer(CNBuffer, CNBuffSize, compressibility, 0., seed);
	{
		size_t const size = MIN(SKILOBYTE(128), CNBuffSize);
		ZSTD_CCtx* const cctx = ZSTD_createCCtx();
		ZSTD_CDict* const lgCDict = ZSTD_createCDict(CNBuffer, size, 1);
		ZSTD_CDict* const smCDict = ZSTD_createCDict(CNBuffer, SKILOBYTE(1), 1);
	...
	DISPLAYLEVEL(3, "test%3i : testing FSE_normalizeCount() PR#1255: ", testNb++);
	{
		short norm[32];
		uint  count[32];
		const uint tableLog = 5;
		const size_t nbSeq = 32;
		const uint maxSymbolValue = 31;
	...
	DISPLAYLEVEL(3, "test%3i : testing FSE_writeNCount() PR#2779: ", testNb++);
	{
		...
		const unsigned tableLog = 5;
		const unsigned maxSymbolValue = 10;
		BYTE* outBuf = (BYTE*)SAlloc::M(outBufSize*sizeof(BYTE));
	...
	{
		uint32 seed_copy = seed; // need non-const seed to avoid compiler warning for FUZ_rand(&seed)
		uint32 rand32 = FUZ_rand(&seed_copy);
		uint64 rand64 = ((uint64)FUZ_rand(&seed_copy) << 32) | FUZ_rand(&seed_copy);
		uint32 lowbit_only_32 = 1;
		uint64 lowbit_only_64 = 1;
		uint32 highbit_only_32 = (uint32)1 << 31;
		uint64 highbit_only_64 = (uint64)1 << 63;
		uint32 i;
		if(rand32 == 0) 
			rand32 = 1; // CLZ and CTZ are undefined on 0
		if(rand64 == 0) 
	...
	{
		ZSTD_CCtx* cctx = ZSTD_createCCtx();
		size_t const srcSize = SMEGABYTE(1) + 5; /* A little more than ZSTDMT_JOBSIZE_MIN */
		size_t const dstSize = ZSTD_compressBound(srcSize);
		void * const src = CNBuffer;
		void * const dst = compressedBuffer;
		void * dict = SAlloc::M(srcSize);
	...
	{
		size_t const kSizeIncrement = SKILOBYTE(2);
		int level = -3;
		for(; level <= ZSTD_maxCLevel(); ++level) {
			size_t dictSize = 0;
			for(; dictSize <= SKILOBYTE(256); dictSize += 8 * kSizeIncrement) {
				size_t srcSize = SKILOBYTE(2);
				for(; srcSize < SKILOBYTE(300); srcSize += kSizeIncrement) {
					...
					if(cctxSizeUsingLevel < cctxSizeUsingCParams || ZSTD_isError(cctxSizeUsingCParams) || ZSTD_isError(cctxSizeUsingLevel)) {
...
static int longUnitTests(const uint32 seed, double compressibility)
{
	size_t const CNBuffSize = SMEGABYTE(5);
	void * const CNBuffer = SAlloc::M(CNBuffSize);
	size_t const compressedBufferSize = ZSTD_compressBound(CNBuffSize);
	...
	DISPLAYLEVEL(3, "longtest%3i : table cleanliness through index reduction : ", testNb++);
	{
		...
		size_t maxIndex = ((3U << 29) + (1U << ZSTD_WINDOWLOG_MAX)); /* ZSTD_CURRENT_MAX from
		...
		* this without ever reallocating, which would reset the indices. */
		...
		void * const staticCCtxBuffer = SAlloc::M(staticCCtxSize);
		ZSTD_CCtx* const cctx = ZSTD_initStaticCCtx(staticCCtxBuffer, staticCCtxSize);
		// bump the indices so the following compressions happen at high indices.
		{   
			ZSTD_outBuffer out = { compressedBuffer, compressedBufferSize, 0 };
		...
		for(cLevel = 1; cLevel <= 22; cLevel++) {
			ZSTD_outBuffer out = { compressedBuffer, compressedBufferSize / (uint)cLevel, 0 };
			ZSTD_inBuffer in = { CNBuffer, CNBuffSize, 0 };
			ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
		...
		// do a bunch of compressions again in low indices and ensure we don't hit untracked invalid indices 
		for(cLevel = 1; cLevel <= 22; cLevel++) {
			ZSTD_outBuffer out = { compressedBuffer, compressedBufferSize / (uint)cLevel, 0 };
			ZSTD_inBuffer in = { CNBuffer, CNBuffSize, 0 };
			ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
	...
	{   
	    ...
	    for(dictSize = CNBuffSize; dictSize; dictSize = dictSize >> 3) {
		    DISPLAYLEVEL(3, "\n    Testing with dictSize %u ", (uint32)dictSize);
		    for(cLevel = 4; cLevel < 13; cLevel++) {
			    for(i = 0; i < 8; ++i) {
				    ...
				    ZSTD_CDict* cdict;
				    DISPLAYLEVEL(5, "\n      dictSize %u cLevel %d iter %d ", (uint32)dictSize, cLevel, i);
				    ...
				    cSize = ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize);
				    CHECK_Z(cSize);
				    CHECK_Z(ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dict,
					dictSize));
				    DISPLAYLEVEL(5, "compressed to %u bytes ", (uint32)cSize);
...
static size_t FUZ_rLogLength(uint32* seed, uint32 logLength)
{
	size_t const lengthMask = ((size_t)1 << logLength) - 1;
	return (lengthMask+1) + (FUZ_rand(seed) & lengthMask);
}

static size_t FUZ_randomLength(uint32* seed, uint32 maxLog)
{
	const uint32 logLength = FUZ_rand(seed) % maxLog;
...
static int fuzzerTests(uint32 seed, unsigned nbTests, unsigned startTest, const uint32 maxDurationS, double compressibility, int bigTests)
{
	static const uint32 maxSrcLog = 23;
	static const uint32 maxSampleLog = 22;
	size_t const srcBufferSize = (size_t)1<<maxSrcLog;
	size_t const dstBufferSize = (size_t)1<<maxSampleLog;
	size_t const cBufferSize   = ZSTD_compressBound(dstBufferSize);
	BYTE* cNoiseBuffer[5];
	BYTE* const cBuffer = (BYTE*)SAlloc::M(cBufferSize);
	BYTE* const dstBuffer = (BYTE*)SAlloc::M(dstBufferSize);
	BYTE* const mirrorBuffer = (BYTE*)SAlloc::M(dstBufferSize);
	ZSTD_CCtx* const refCtx = ZSTD_createCCtx();
	ZSTD_CCtx* const ctx = ZSTD_createCCtx();
	ZSTD_DCtx* const dctx = ZSTD_createDCtx();
	uint32 result = 0;
	unsigned testNb = 0;
	uint32 coreSeed = seed;
	UTIL_time_t const startClock = UTIL_getTime();
	uint64 const maxClockSpan = maxDurationS * SEC_TO_MICRO;
	int const cLevelLimiter = bigTests ? 3 : 2;

	/* allocation */
	cNoiseBuffer[0] = (BYTE*)SAlloc::M(srcBufferSize);
	cNoiseBuffer[1] = (BYTE*)SAlloc::M(srcBufferSize);
	cNoiseBuffer[2] = (BYTE*)SAlloc::M(srcBufferSize);
	cNoiseBuffer[3] = (BYTE*)SAlloc::M(srcBufferSize);
	cNoiseBuffer[4] = (BYTE*)SAlloc::M(srcBufferSize);
	CHECK(!cNoiseBuffer[0] || !cNoiseBuffer[1] || !cNoiseBuffer[2] || !cNoiseBuffer[3] || !cNoiseBuffer[4] || 
		!dstBuffer || !mirrorBuffer || !cBuffer || !refCtx || !ctx || !dctx, "Not enough memory, fuzzer tests cancelled");
	...
	for(; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < maxClockSpan); testNb++) {
		...
		{   
		    ...
		    if(buffNb & 7) 
			    ...
			    buffNb >>= 3;
			    if(buffNb & 7) {
				    const uint32 tnb[2] = { 1, 3 }; /* barely/highly compressible */
				    buffNb = tnb[buffNb >> 3];
			    }
			    else {
				    const uint32 tnb[2] = { 0, 4 }; /* not compressible / sparse */
				    buffNb = tnb[buffNb >> 3];
			    }
		    }
		...
		sampleBuffer = (BYTE*)SAlloc::M(sampleSize);
		...
		/* compression tests */
		{
			int const cLevelPositive = (int)( FUZ_rand(&lseed) % ((uint32)ZSTD_maxCLevel() - (FUZ_highbit32((uint32)sampleSize) / (uint32)cLevelLimiter)) ) + 1;
			int const cLevel = ((FUZ_rand(&lseed) & 15) == 3) ? -(int)((FUZ_rand(&lseed) & 7) + 1) : /* test negative cLevel */ cLevelPositive;
			...
			assert(cSize > 3);
			{
				...
				DISPLAYLEVEL(5, "fuzzer t%u: compress into too small buffer of size %u (missing %u bytes) \n",
				    testNb, (uint)tooSmallSize, (uint)missing);
				{
					size_t const errorCode = ZSTD_compressCCtx(ctx, dstBuffer, tooSmallSize, sampleBuffer, sampleSize, cLevel);
					CHECK(ZSTD_getErrorCode(errorCode) != ZSTD_error_dstSize_tooSmall, "ZSTD_compressCCtx should have failed ! (buffer too small : %u < %u)", (uint)tooSmallSize, (uint)cSize);
				}
				{
					unsigned endCheck; 
		...
		DISPLAYLEVEL(5, "fuzzer t%u: simple decompression test \n", testNb);
		{
			...
			CHECK(dSize != sampleSize, "ZSTD_decompress failed (%s) (srcSize : %u ; cSize : %u)", ZSTD_getErrorName(dSize), (uint)sampleSize, (uint)cSize);
			{
				uint64 const crcDest = XXH64(dstBuffer, sampleSize, 0);
				CHECK(crcOrig != crcDest, "decompression result corrupted (pos %u / %u)", (uint)findDiff(sampleBuffer, dstBuffer, sampleSize), (uint)sampleSize);
			}
		}
		SAlloc::F(sampleBuffer); /* no longer useful after this point */
		...
		DISPLAYLEVEL(5, "fuzzer t%u: decompression of truncated source \n", testNb);
		{
			...
			void * cBufferTooSmall = SAlloc::M(tooSmallSize); /* valgrind will catch read overflows */
		...
		if(sampleSize > 3) {
			...
			dstBuffer[tooSmallSize] = token;
			{
				...
				CHECK(ZSTD_getErrorCode(errorCode) != ZSTD_error_dstSize_tooSmall,
				    "ZSTD_decompress should have failed : %u > %u (dst buffer too small)", (uint)errorCode, (uint)tooSmallSize);
			}
		...
		if(cSize > 6) {
			...
			{
				const uint32 maxNbBits = FUZ_highbit32((uint32)(cSize-4));
				size_t pos = 4; /* preserve magic number (too easy to detect) */
				for(;;) {
					/* keep some original src */
					{
						const uint32 nbBits = FUZ_rand(&lseed) % maxNbBits;
					...
					/* add noise */
					{
						const uint32 nbBitsCodes = FUZ_rand(&lseed) % maxNbBits;
						const uint32 nbBits = nbBitsCodes ? nbBitsCodes-1 : 0;
						size_t const mask = (1<<nbBits) - 1;
			...
			{
				const uint32 endMark = 0xA9B1C3D6;
				memcpy(dstBuffer+sampleSize, &endMark, 4);
				{
					size_t const decompressResult = ZSTD_decompress(dstBuffer, sampleSize, cBuffer, cSize);
					/* result *may* be an unlikely success, but even then, it must strictly respect
					   dst buffer boundaries */
					CHECK((!ZSTD_isError(decompressResult)) && (decompressResult>sampleSize),
					    "ZSTD_decompress on noisy src : result is too large : %u > %u (dst buffer)", (uint)decompressResult, (uint)sampleSize);
				}
				{
					uint32 endCheck; memcpy(&endCheck, dstBuffer+sampleSize, 4);
		...
		{
			const uint32 testLog = FUZ_rand(&lseed) % maxSrcLog;
			const uint32 dictLog = FUZ_rand(&lseed) % maxSrcLog;
			int const cLevel = (int)(FUZ_rand(&lseed) % ((uint32)ZSTD_maxCLevel() - (MAX(testLog, dictLog) / (uint32)cLevelLimiter))) + 1;
			...
			DISPLAYLEVEL(6, "fuzzer t%u: Compressing up to <=%u bytes at level %i with dictionary size %u \n",
			    testNb, (uint)maxTestSize, cLevel, (uint)dictSize);
			if(FUZ_rand(&lseed) & 0xF) {
				CHECK_Z(ZSTD_compressBegin_usingDict(refCtx, dict, dictSize, cLevel) );
			}
			else {
				ZSTD_compressionParameters const cPar = ZSTD_getCParams(cLevel, ZSTD_CONTENTSIZE_UNKNOWN, dictSize);
				ZSTD_frameParameters const fPar = { FUZ_rand(&lseed)&1 /* contentSizeFlag */, !(FUZ_rand(&lseed)&3) /* contentChecksumFlag*/,
		...
		{
			const uint32 nbChunks = (FUZ_rand(&lseed) & 127) + 2;
			uint32 n;
		...
		{
			ZSTD_frameHeader zfh;
			CHECK(ZSTD_getFrameHeader(&zfh, cBuffer, ZSTD_FRAMEHEADERSIZE_MAX), "ZSTD_getFrameHeader(): error retrieving frame information");
			{   
				...
				(uint)roundBuffSize, (uint)totalTestSize);
			}
		}
		if(dictSize<8) {
			dictSize = 0;
			dict = NULL; /* disable dictionary */
		}
		...
		{
			uint64 const crcDest = XXH64(dstBuffer, totalTestSize, 0);
			CHECK(crcOrig != crcDest, "streaming decompressed data corrupted (pos %u / %u)",
			    (uint)findDiff(mirrorBuffer, dstBuffer, totalTestSize), (uint)totalTestSize);
		}
	} /* for ( ; (testNb <= nbTests) */
...
_cleanup:
	...
	ZSTD_freeCCtx(ctx);
	ZSTD_freeDCtx(dctx);
	SAlloc::F(cNoiseBuffer[0]);
	SAlloc::F(cNoiseBuffer[1]);
	SAlloc::F(cNoiseBuffer[2]);
	SAlloc::F(cNoiseBuffer[3]);
	SAlloc::F(cNoiseBuffer[4]);
	SAlloc::F(cBuffer);
	SAlloc::F(dstBuffer);
	SAlloc::F(mirrorBuffer);
	return (int)result;
_output_error:
	result = 1;
	goto _cleanup;
...
static uint readU32FromChar(const char** stringPtr)
{
	uint result = 0;
	while((**stringPtr >='0') && (**stringPtr <='9'))
		result *= 10, result += (uint)(**stringPtr - '0'), (*stringPtr)++;
...
{
	const size_t comSize = strlen(longCommand);
	const int result = !strncmp(*stringPtr, longCommand, comSize);
	if(result) 
		*stringPtr += comSize;
	return result;
}
...
enum {
	zstdfuzzerfBigTest   = 0x0001,
	zstdfuzzerfLongTest  = 0x0002,
	zstdfuzzerfMainPause = 0x0004
};

int ZStdFuzzer(const char * pOutFileName, uint displayLevel_, uint memTestOnly_/*0..*/, uint maxDurationSec, uint flags)
{
	int    ok = 1;
	uint32 seed = 0;
	int seedset = 0;
	...
	double probfloat;
	int result = 0;
	uint32 mainPause = BIN(flags & zstdfuzzerfMainPause);
	uint32 maxDuration = maxDurationSec;
	//int bigTests = 1;
	//int longTests = 0;
	//uint32 memTestsOnly = 0;
	g_displayLevel = displayLevel_;
	//const char * const programName = argv[0];
	/* Get Seed */
	DISPLAY("Starting zstd tester (%i-bits, %s)\n", (int)(sizeof(size_t)*8), ZSTD_VERSION_STRING);
	if(!seedset) {
		const time_t t = time(NULL);
		const uint32 h = XXH32(&t, sizeof(t), 1);
		seed = h % 10000;
	}
	DISPLAY("Seed = %u\n", (uint)seed);
	...
	if(memTestOnly_) {
		...
		return FUZ_mallocTests(seed, probfloat, memTestOnly_);
	}
	SETMAX(nbTests, testNb);
	if(testNb==0) {
		result = basicUnitTests(0, probfloat); /* constant seed for predictability */
		if(!result && (flags & zstdfuzzerfLongTest)) {
			result = longUnitTests(0, probfloat);
		}
	}
	SETIFZQ(result, 
		fuzzerTests(seed, (uint)nbTests, (uint)testNb, maxDuration, ((double)proba) / 100, 
		/*bigTests*/BIN(flags & zstdfuzzerfBigTest)));
	if(mainPause) {
		int unused;
		DISPLAY("Press Enter \n");
		unused = getchar();
...
int main(int argc, const char** argv)
{
	...
	for(argNb = 1; argNb<argc; argNb++) {
		...
		if(argument[0]=='-') {
			...
			if(!strcmp(argument, "--long-tests")) {
				longTests = 1; 
				continue;
			}
			if(!strcmp(argument, "--no-long-tests")) {
				longTests = 0; 
				continue;
			}
			argument++;
	...
	SETIFZQ(result, fuzzerTests(seed, (uint)nbTests, (uint)testNb, maxDuration, ((double)proba) / 100, bigTests));
	if(mainPause) {
		int unused;
		DISPLAY("Press Enter \n");
		unused = getchar();
