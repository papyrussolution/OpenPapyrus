...
static void usage(FILE* f, const char* programName)
{
	...
	DISPLAY_F(f,
	    "  -1 .. -%d            compression level (faster .. better; default: %d)\n",
	    ZSTDCLI_CLEVEL_MAX,
...
static size_t readSizeTFromChar(const char** stringPtr) 
{
	...
	size_t result;
	if(readSizeTFromCharChecked(stringPtr, &result)) {
		errorOut(errorMsg);
	}
	return result;
}
...
static unsigned parseCoverParameters(const char* stringPtr, ZDICT_cover_params_t* params)
{
	...
	for(;;) {
		if(longCommandWArg(&stringPtr, "k=")) {
			params->k = readU32FromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
		if(longCommandWArg(&stringPtr, "d=")) {
			params->d = readU32FromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
		if(longCommandWArg(&stringPtr, "steps=")) {
			params->steps = readU32FromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
	...
	DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nsteps=%u\nsplit=%u\nshrink%u\n", params->k, params->d, params->steps,
	    (uint)(params->splitPoint * 100), params->shrinkDictMaxRegression);
	return 1;
}
...
static unsigned parseFastCoverParameters(const char* stringPtr, ZDICT_fastCover_params_t* params)
{
	...
	for(;;) {
		...
		if(longCommandWArg(&stringPtr, "f=")) {
			params->f = readU32FromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
...
static unsigned parseAdaptParameters(const char* stringPtr, int* adaptMinPtr, int* adaptMaxPtr)
{
	for(;;) {
		if(longCommandWArg(&stringPtr, "min=")) {
			*adaptMinPtr = readIntFromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
		if(longCommandWArg(&stringPtr, "max=")) {
			*adaptMaxPtr = readIntFromChar(&stringPtr); if(stringPtr[0]==',') {
				stringPtr++; continue;
			}
			else break;
		}
...
int main(int argCount, const char* argv[])
{
	...
	int nbWorkers = 0;
	int adapt = 0;
	int useRowMatchFinder = 0;
	int adaptMin = MINCLEVEL;
	int adaptMax = MAXCLEVEL;
	int rsyncable = 0;
	int nextArgumentsAreFiles = 0;
	int operationResult = 0;
	int separateFiles = 0;
	int setRealTimePrio = 0;
	int singleThread = 0;
	int defaultLogicalCores = 0;
	int showDefaultCParams = 0;
	int ultra = 0;
	int contentSize = 1;
	...
	(void)memLimit;
	assert(argCount >= 1);
	if((filenames==NULL) || (file_of_names==NULL)) {
	...
	if(exeNameMatch(programName, ZSTD_CAT)) {
		operation = zom_decompress; FIO_overwriteMode(prefs); forceStdout = 1; followLinks = 1; outFileName = stdoutmark;
		g_displayLevel = 1;
	...
	if(exeNameMatch(programName, ZSTD_GUNZIP)) {
		operation = zom_decompress; FIO_setRemoveSrcFile(prefs, 1);
	...
	if(exeNameMatch(programName, ZSTD_LZMA)) {
		suffix = LZMA_EXTENSION; FIO_setCompressionType(prefs, FIO_lzmaCompression); FIO_setRemoveSrcFile(prefs, 1);
	} // behave like lzma
	if(exeNameMatch(programName, ZSTD_UNLZMA)) {
		operation = zom_decompress; FIO_setCompressionType(prefs, FIO_lzmaCompression); FIO_setRemoveSrcFile(prefs, 1);
	} // behave like unlzma, also supports multiple formats
	if(exeNameMatch(programName, ZSTD_XZ)) {
		suffix = XZ_EXTENSION; FIO_setCompressionType(prefs, FIO_xzCompression); FIO_setRemoveSrcFile(prefs, 1);
	} // behave like xz
	if(exeNameMatch(programName, ZSTD_UNXZ)) {
		operation = zom_decompress; FIO_setCompressionType(prefs, FIO_xzCompression); FIO_setRemoveSrcFile(prefs, 1);
	} // behave like unxz, also supports multiple formats
	if(exeNameMatch(programName, ZSTD_LZ4)) {
		suffix = LZ4_EXTENSION; FIO_setCompressionType(prefs, FIO_lz4Compression);
	} // behave like lz4
	if(exeNameMatch(programName, ZSTD_UNLZ4)) {
		operation = zom_decompress; FIO_setCompressionType(prefs, FIO_lz4Compression);
	} // behave like unlz4, also supports multiple formats
	memzero(&compressionParams, sizeof(compressionParams));
	/* init crash handler */
	FIO_addAbortHandler();
	/* command switches */
	for(argNb = 1; argNb<argCount; argNb++) {
		...
		if(argument[0]=='-') {
			if(argument[1]=='-') {
				...
				if(sstreq(argument, "--")) {
					nextArgumentsAreFiles = 1; continue;
				} // only file names allowed from now on 
				if(sstreq(argument, "--list")) {
					operation = zom_list; continue;
				}
				if(sstreq(argument, "--compress")) {
					operation = zom_compress; continue;
				}
				if(sstreq(argument, "--decompress")) {
					operation = zom_decompress; continue;
				}
				if(sstreq(argument, "--uncompress")) {
					operation = zom_decompress; continue;
				}
				if(sstreq(argument, "--force")) {
					FIO_overwriteMode(prefs); forceStdin = 1; forceStdout = 1; followLinks = 1; allowBlockDevices = 1;
					continue;
				}
				if(sstreq(argument, "--version")) {
					printVersion(); CLEAN_RETURN(0);
				}
				if(sstreq(argument, "--help")) {
					usage_advanced(programName); CLEAN_RETURN(0);
				}
				if(sstreq(argument, "--verbose")) {
					g_displayLevel++; continue;
				}
				if(sstreq(argument, "--quiet")) {
					g_displayLevel--; continue;
				}
				if(sstreq(argument, "--stdout")) {
					forceStdout = 1; outFileName = stdoutmark; FIO_setRemoveSrcFile(prefs, 0);
					g_displayLevel -= (g_displayLevel==2); continue;
				}
				if(sstreq(argument, "--ultra")) {
					ultra = 1; continue;
				}
				if(sstreq(argument, "--check")) {
					FIO_setChecksumFlag(prefs, 2); continue;
				}
				if(sstreq(argument, "--no-check")) {
					FIO_setChecksumFlag(prefs, 0); continue;
				}
				if(sstreq(argument, "--sparse")) {
					FIO_setSparseWrite(prefs, 2); continue;
				}
				if(sstreq(argument, "--no-sparse")) {
					FIO_setSparseWrite(prefs, 0); continue;
				}
				if(sstreq(argument, "--test")) {
					operation = zom_test; 
					continue;
				}
				if(sstreq(argument, "--asyncio")) {
					FIO_setAsyncIOFlag(prefs, 1); continue;
				}
				if(sstreq(argument, "--no-asyncio")) {
					FIO_setAsyncIOFlag(prefs, 0); continue;
				}
				if(sstreq(argument, "--train")) {
					operation = zom_train; if(outFileName==NULL) outFileName = g_defaultDictName; continue;
				}
				if(sstreq(argument, "--no-dictID")) {
					FIO_setDictIDFlag(prefs, 0); continue;
				}
				if(sstreq(argument, "--keep")) {
					FIO_setRemoveSrcFile(prefs, 0); continue;
				}
				if(sstreq(argument, "--rm")) {
					FIO_setRemoveSrcFile(prefs, 1); continue;
				}
				if(sstreq(argument, "--priority=rt")) {
					setRealTimePrio = 1; continue;
				}
				if(sstreq(argument, "--show-default-cparams")) {
					showDefaultCParams = 1; continue;
				}
				if(sstreq(argument, "--content-size")) {
					contentSize = 1; continue;
				}
				if(sstreq(argument, "--no-content-size")) {
					contentSize = 0; continue;
				}
				if(sstreq(argument, "--adapt")) {
					adapt = 1; continue;
				}
				if(sstreq(argument, "--no-row-match-finder")) {
					useRowMatchFinder = 1; continue;
				}
				if(sstreq(argument, "--row-match-finder")) {
					useRowMatchFinder = 2; continue;
				}
				if(longCommandWArg(&argument, "--adapt=")) {
					adapt = 1; 
					if(!parseAdaptParameters(argument, &adaptMin, &adaptMax)) {
						badusage(programName); CLEAN_RETURN(1);
					}
					continue;
				}
				if(sstreq(argument, "--single-thread")) {
					nbWorkers = 0; singleThread = 1; continue;
				}
				if(sstreq(argument, "--format=zstd")) {
					suffix = ZSTD_EXTENSION; 
					FIO_setCompressionType(prefs, FIO_zstdCompression); continue;
				}
#ifdef ZSTD_GZCOMPRESS
				if(sstreq(argument, "--format=gzip")) {
					suffix = GZ_EXTENSION; 
					FIO_setCompressionType(prefs, FIO_gzipCompression); continue;
				}
				if(exeNameMatch(programName, ZSTD_GZ)) { /* behave like gzip */
					if(sstreq(argument, "--best")) {
						dictCLevel = cLevel = 9; continue;
					}
					if(sstreq(argument, "--no-name")) { /* ignore for now */
						; continue;
					}
				}
#endif
#ifdef ZSTD_LZMACOMPRESS
				if(sstreq(argument, "--format=lzma")) {
					suffix = LZMA_EXTENSION; FIO_setCompressionType(prefs, FIO_lzmaCompression);  continue;
				}
				if(sstreq(argument, "--format=xz")) {
					suffix = XZ_EXTENSION; FIO_setCompressionType(prefs, FIO_xzCompression);  continue;
				}
#endif
#ifdef ZSTD_LZ4COMPRESS
				if(sstreq(argument, "--format=lz4")) {
					suffix = LZ4_EXTENSION; FIO_setCompressionType(prefs, FIO_lz4Compression);  continue;
				}
#endif
				if(sstreq(argument, "--rsyncable")) {
					rsyncable = 1; continue;
				}
				if(sstreq(argument, "--compress-literals")) {
					literalCompressionMode = ZSTD_ps_enable; continue;
				}
				if(sstreq(argument, "--no-compress-literals")) {
					literalCompressionMode = ZSTD_ps_disable; continue;
				}
				if(sstreq(argument, "--no-progress")) {
					FIO_setProgressSetting(FIO_ps_never); continue;
				}
				if(sstreq(argument, "--progress")) {
					FIO_setProgressSetting(FIO_ps_always); continue;
				}
				if(sstreq(argument, "--exclude-compressed")) {
					FIO_setExcludeCompressedFile(prefs, 1); continue;
				}
				...
				if(longCommandWArg(&argument, "--threads")) {
					NEXT_UINT32(nbWorkers); continue;
				}
				if(longCommandWArg(&argument, "--memlimit")) {
					NEXT_UINT32(memLimit); continue;
				}
				...
				if(longCommandWArg(&argument, "--block-size=")) {
					blockSize = readSizeTFromChar(&argument); continue;
				}
				if(longCommandWArg(&argument, "--maxdict")) {
					NEXT_UINT32(maxDictSize); continue;
				}
				if(longCommandWArg(&argument, "--dictID")) {
					NEXT_UINT32(dictID); continue;
				}
				...
				if(longCommandWArg(&argument, "--stream-size=")) {
					streamSrcSize = readSizeTFromChar(&argument); continue;
				}
				if(longCommandWArg(&argument, "--target-compressed-block-size=")) {
					targetCBlockSize = readSizeTFromChar(&argument); continue;
				}
				if(longCommandWArg(&argument, "--size-hint=")) {
					srcSizeHint = readSizeTFromChar(&argument); continue;
				}
				if(longCommandWArg(&argument, "--output-dir-flat")) {
					NEXT_FIELD(outDirName); continue;
				}
				...
				if(longCommandWArg(&argument, "--output-dir-mirror")) {
					NEXT_FIELD(outMirroredDirName); continue;
				}
#endif
#ifndef ZSTD_NOTRACE
				if(longCommandWArg(&argument, "--trace")) {
					char const* traceFile; NEXT_FIELD(traceFile); TRACE_enable(traceFile); continue;
				}
#endif
				if(longCommandWArg(&argument, "--patch-from")) {
					NEXT_FIELD(patchFromDictFileName); continue;
				}
			...
			while(argument[0]!=0) {
				...
				switch(argument[0]) {
					...
					case 'c': forceStdout = 1; outFileName = stdoutmark; FIO_setRemoveSrcFile(prefs, 0); argument++;
					    break;
	...
	if(!followLinks) {
		...
		for(u = 0, fileNamesNb = 0; u<nbFilenames; u++) {
			if(UTIL_isLink(filenames->fileNames[u]) && !UTIL_isFIFO(filenames->fileNames[u])) {
			...
			if(separateFiles) {
				...
				for(i = 0; i < filenames->tableSize; i++) {
					...
					for(c = cLevel; c <= cLevelLast; c++) {
						BMK_benchFilesAdvanced(&filenames->fileNames[i],
						    1,
						    dictFileName,
						    c,
						    &compressionParams,
	...
	if(operation==zom_train) {
		...
		if(dict == cover) {
			...
			operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames, (int)filenames->tableSize, blockSize,
				NULL, &coverParams, NULL, optimize, memLimit);
		}
		else if(dict == fastCover) {
			int const optimize = !fastCoverParams.k || !fastCoverParams.d;
			fastCoverParams.nbThreads = (unsigned)nbWorkers;
			fastCoverParams.zParams = zParams;
			operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames, (int)filenames->tableSize,
				blockSize, NULL, NULL, &fastCoverParams, optimize, memLimit);
		}
		else {
			ZDICT_legacy_params_t dictParams;
			memzero(&dictParams, sizeof(dictParams));
			dictParams.selectivityLevel = dictSelect;
			dictParams.zParams = zParams;
			operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames, (int)filenames->tableSize, blockSize,
				&dictParams, NULL, NULL, 0, memLimit);
		}
#else
	...
	if(operation==zom_test) {
		...
		outFileName = nulmark; FIO_setRemoveSrcFile(prefs, 0);
	} // test mode
#endif
	...
	if(sstreq(filenames->fileNames[0], stdinmark) && !outFileName)
		outFileName = stdoutmark; /* when input is stdin, default output is stdout */
	/* Check if input/output defined as console; trigger an error in this case */
	if(!forceStdin && sstreq(filenames->fileNames[0], stdinmark) && IS_CONSOLE(stdin) ) {
		DISPLAYLEVEL(1, "stdin is a console, aborting\n");
		CLEAN_RETURN(1);
	}
	if(outFileName && sstreq(outFileName, stdoutmark) && IS_CONSOLE(stdout) && sstreq(filenames->fileNames[0], stdinmark) && !forceStdout && operation!=zom_decompress) {
	...
	hasStdout = outFileName && sstreq(outFileName, stdoutmark);
...
#else
		(void)contentSize; (void)suffix; (void)adapt; (void)rsyncable; (void)ultra; (void)cLevel; (void)ldmFlag;
		(void)literalCompressionMode; (void)targetCBlockSize; (void)streamSrcSize; (void)srcSizeHint; (void)ZSTD_strategyMap;
		(void)useRowMatchFinder; // not used when ZSTD_NOCOMPRESS set
