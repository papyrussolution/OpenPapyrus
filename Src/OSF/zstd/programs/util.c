...
int UTIL_compareStr(const void * p1, const void * p2) { return strcmp(*(char* const*)p1, *(char* const*)p2); }
...
{
	FileNamesTable* const table = (FileNamesTable*)SAlloc::M(sizeof(*table));
	CONTROL(table != NULL);
...
{
	const char** const fnTable = (const char**)SAlloc::M(tableSize * sizeof(*fnTable));
	FileNamesTable* fnt;
...
static int UTIL_prepareFileList(const char * dirName, char** bufStart, size_t* pos, char** bufEnd, int followLinks)
{
	...
	while((entry = readdir(dir)) != NULL) {
		...
		if(UTIL_isDirectory(path)) {
			...
			if(*bufStart == NULL) {
				SAlloc::F(path); closedir(dir); return 0;
			}
		}
		else {
			if(*bufStart + *pos + pathLength >= *bufEnd) {
...
static int firstIsParentOrSameDirOfSecond(const char* firstDir, const char* secondDir)
{
	...
	size_t secondDirLen = strlen(secondDir);
...
static void makeUniqueMirroredDestDirs(char** srcDirNames, unsigned nbFile, const char* outDirName)
{
	...
	if(nbFile == 0)
		return;
	uniqueDirNames = (char**)SAlloc::M(nbFile * sizeof(char *));
...
void UTIL_mirrorSourceFilesDirectories(const char** inFileNames, unsigned int nbFile, const char* outDirName)
{
	...
	for(i = 0; i < validFilenamesNr; i++)
		SAlloc::F(srcFileNames[i]);
	SAlloc::F(srcFileNames);
}
...
FileNamesTable* UTIL_createExpandedFNT(const char* const* inputNames, size_t nbIfns, int followLinks)
{
	...
	{   
	    ...
	    const char** const fileNamesTable = (const char**)SAlloc::M(fntCapacity * sizeof(*fileNamesTable));
	    if(!fileNamesTable) {
		    SAlloc::F(buf); 
			return NULL;
	    }
	    ...
	    while(!done) {
		    ...
		    if(FALSE == rc) {
			    if(GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
				    SAlloc::F(buffer);
				    buffer = (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION)SAlloc::M(returnLength);
				    if(buffer == NULL) {
					    perror("zstd");
					    exit(1);
				    }
...
int UTIL_countPhysicalCores(void) { return UTIL_countCores(0); }
int UTIL_countLogicalCores(void) { return UTIL_countCores(1); }
