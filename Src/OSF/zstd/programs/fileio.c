...
{
	FIO_prefs_t* const ret = (FIO_prefs_t*)SAlloc::M(sizeof(FIO_prefs_t));
...
{
	FIO_ctx_t* const ret = (FIO_ctx_t*)SAlloc::M(sizeof(FIO_ctx_t));
	if(!ret) 
...
void FIO_freePreferences(FIO_prefs_t* const prefs) { SAlloc::F(prefs); }
void FIO_freeContext(FIO_ctx_t* const fCtx) { SAlloc::F(fCtx); }
...
static FILE* FIO_openSrcFile(const FIO_prefs_t* const prefs, const char* srcFileName)
{
	...
	if(sstreq(srcFileName, stdinmark)) {
	...
	if(!UTIL_isRegularFileStat(&statbuf) && !UTIL_isFIFOStat(&statbuf) && !(allowBlockDevices && UTIL_isBlockDevStat(&statbuf))) {
...
static FILE* FIO_openDstFile(FIO_ctx_t* fCtx, FIO_prefs_t* const prefs, const char* srcFileName, const char* dstFileName, const int mode)
{
	if(prefs->testMode) 
		return NULL; /* do not open file in test mode */
	assert(dstFileName != NULL);
	if(sstreq(dstFileName, stdoutmark)) {
...
static size_t FIO_createDictBuffer(void** bufferPtr, const char* fileName, FIO_prefs_t* const prefs)
{
	...
	*bufferPtr = SAlloc::M((size_t)fileSize);
    ...
    int displayLevelCutoff)
{
	...
	if(fCtx->nbFilesTotal > 1 && !prefs->overwrite) {
		if(g_display_prefs.displayLevel <= displayLevelCutoff) {
			...
			if(sstreq(outFileName, stdoutmark)) {
...
static cRess_t FIO_createCResources(FIO_prefs_t* const prefs, const char* dictFileName, unsigned long long const maxSrcFileSize,
    int cLevel, ZSTD_compressionParameters comprParams) 
{
	cRess_t ress;
	memzero(&ress, sizeof(ress));
	...
	CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_c_contentSizeFlag, prefs->contentSize) ); /* always enable content
	                                                                                          size when available
	                                                                                          (note: supposed to be
	                                                                                          default) */
	...
	do {
		...
		*readsize += inSize;
		...
		while((inBuff.pos != inBuff.size) /* input buffer must be entirely ingested */
			...
			/* count stats */
			inputPresented++;
			if(oldIPos == inBuff.pos) inputBlocked++; /* input buffer is full and can't take any more :
			...
			if(READY_FOR_UPDATE()) {
				...
				double const cShare = (double)zfp.produced / (double)(zfp.consumed + !zfp.consumed /*avoid
				                                                                                      div0*/     ) * 100;
				...
				if(g_display_prefs.displayLevel >= 3) {
					...
					if(fCtx->nbFilesTotal > 1) {
						...
						if(srcFileNameSize > 18) {
							...
							DISPLAYLEVEL(1, "Compress: %u/%u files. Current: %*s ",
							    fCtx->currFileIdx+1, fCtx->nbFilesTotal, (int)(18-srcFileNameSize),
							    srcFileName);
						}
					}
				...
				if(prefs->adaptiveMode) {
					...
					if(zfp.currentJobID > 1) { /* only possible if nbWorkers >= 1 */
						 ...
						 * or because input is slow and no job can start while waiting for at
						 *least one buffer to be filled.
						 * note : exclude starting part, since currentJobID > 1 */
						if( (zfp.consumed == previous_zfp_update.consumed) // no data compressed : no data available, or no more buffer to
							// compress to, OR compression is really slow (compression of a single block is slower than update rate)
						    && (zfp.nbActiveWorkers == 0) /* confirmed : no compression ongoing */) {
							DISPLAYLEVEL(6, "all buffers full : compression stopped => slow down \n")
							speedChange = slower;
						}
						previous_zfp_update = zfp;
						if( (newlyProduced > (newlyFlushed * 9 / 8)) // compression produces more data than output can flush (though
					...
					if(zfp.currentJobID > lastJobID) {
						...
						if(zfp.currentJobID > (unsigned)(prefs->nbWorkers+1)) { // warm up period, to fill all workers 
							if(inputBlocked <= 0) {
								...
								DISPLAYLEVEL(6,
								    "input blocked %u/%u(%.2f) - ingested:%u vs %u:consumed - flushed:%u vs %u:produced \n",
								    inputBlocked,
								    inputPresented,
 ...
 */
static int FIO_compressFilename_internal(FIO_ctx_t* const fCtx,
    FIO_prefs_t* const prefs,
    cRess_t ress,
    ...
    int compressionLevel)
{
	...
	if(closeDstFile) {
		...
		if( (result != 0) /* operation failure */
			...
			FIO_removeFile(dstFileName); /* remove compression artefact; note don't do anything special if
			                                remove() fails */
		}
	}
 ...
 *            1 : missing or pb opening srcFileName
 */
static int FIO_compressFilename_srcFile(FIO_ctx_t* const fCtx,
    FIO_prefs_t* const prefs,
    cRess_t ress,
    const char* dstFileName,
    const char* srcFileName,
	...
	if(outFileName != NULL) { /* output into a single destination (stdout typically) */
		...
		if(dstFile == NULL) { /* could not open outFileName */
			...
			for(; fCtx->currFileIdx < fCtx->nbFilesTotal; ++fCtx->currFileIdx) {
				status = FIO_compressFilename_srcFile(fCtx,
					prefs,
					ress,
					outFileName,
...
static void FIO_zstdErrorHelp(const FIO_prefs_t* const prefs, const dRess_t* ress, size_t err, const char* srcFileName)
{
    ...
    uint64 alreadyDecoded)                      /* for multi-frames streams */
{
	...
	while(1) {
		...
		if(fCtx->nbFilesTotal > 1) {
			...
			DISPLAYUPDATE(displayLevel, "\r%-20.20s : %.*f%s...     ", srcFileName, hrs.precision, hrs.value, hrs.suffix);
		}
...
static unsigned long long FIO_decompressGzFrame(dRess_t* ress, const char* srcFileName)
{
...
static unsigned long long FIO_decompressLzmaFrame(dRess_t* ress,
...
static unsigned long long FIO_decompressLz4Frame(dRess_t* ress, const char* srcFileName)
{
    ...
    const char* dstFileName, const char* srcFileName)
{
	...
	for(;;) {
		...
		else if((prefs->overwrite) && sstreq(dstFileName, stdoutmark)) { /* pass-through mode */
...
/** FIO_decompressDstFile() :
    ...
    @return : 0 : OK
              1 : operation aborted
 */
static int FIO_decompressDstFile(FIO_ctx_t* const fCtx, FIO_prefs_t* const prefs, dRess_t ress, const char* dstFileName, const char* srcFileName)
{
	int result;
	stat_t statbuf;
	...
	if(releaseDstFile) {
		...
		if( (result != 0) /* operation failure */
			...
			FIO_removeFile(dstFileName); /* remove decompression artefact; note: don't do anything special
			                                if remove() fails */
		}
	}
...
/** FIO_decompressSrcFile() :
    ...
    @return : 0 : OK
              1 : error
 */
static int FIO_decompressSrcFile(FIO_ctx_t* const fCtx,
    FIO_prefs_t* const prefs,
    dRess_t ress,
    const char* dstFileName,
    const char* srcFileName)
{
	...
	if(prefs->removeSrcFile /* --rm */ && (result==0) /* decompression successful */ && strcmp(srcFileName, stdinmark) ) { /* not stdin */
		// We must clear the handler, since after this point calling it would delete both the source and destination files.
		clearHandler();
		if(FIO_removeFile(srcFileName)) {
...
static const char* FIO_determineDstName(const char* srcFileName, const char* outDirName)
{
	...
	if(srcSuffix == NULL) {
		DISPLAYLEVEL(1, "zstd: %s: unknown suffix (%s expected). Can't derive the output file name. Specify it with -o dstFileName. Ignoring.\n",
	...
	srcSuffixLen = strlen(srcSuffix);
	{
		...
		for(matchedSuffixPtr = suffixList; *matchedSuffixPtr != NULL; matchedSuffixPtr++) {
			if(sstreq(*matchedSuffixPtr, srcSuffix)) {
				break;
			}
		}
...
int FIO_decompressMultipleFilenames(FIO_ctx_t* const fCtx, FIO_prefs_t* const prefs, const char** srcNamesTable,
    const char* outMirroredRootDirName, const char* outDirName, const char* outFileName, const char* dictFileName)
{
	int status;
	int error = 0;
		    ...
		    if(magicNumber == ZSTD_MAGICNUMBER) {
			    ...
			    {   
					...
					if(contentChecksumFlag) {
						info->usesCheck = 1;
						ERROR_IF(fseek(srcFile, 4, SEEK_CUR) != 0, info_frame_error, "Error: could not skip past checksum");
					}
			    }
...
int FIO_listMultipleFiles(unsigned numFiles, const char** filenameTable, int displayLevel)
{
	...
	{   
	    for(uint u = 0; u<numFiles; u++) {
		    ERROR_IF(sstreq(filenameTable[u], stdinmark), 1, "zstd: --list does not support reading from standard input");
	    }
	}
	if(numFiles == 0) {
		if(!IS_CONSOLE(stdin)) {
