...
static void BMK_initCCtx(ZSTD_CCtx* ctx, const void* dictBuffer, size_t dictBufferSize, int cLevel, const ZSTD_compressionParameters* comprParams, const BMK_advancedParams_t* adv)
{
...
static size_t local_defaultCompress(const void* srcBuffer, size_t srcSize,
...
/* `addArgs` is the context */
static size_t local_defaultDecompress(const void* srcBuffer, size_t srcSize,
...
static BMK_benchOutcome_t BMK_benchMemAdvancedNoAlloc(const void** srcPtrs, size_t* srcSizes,
    ...
    const BMK_advancedParams_t* adv)
{
	...
	if(adv->mode == BMK_decodeOnly) { /* benchmark only decompression : source must be already compressed */
		...
		for(fileNb = 0; fileNb<nbFiles; fileNb++) {
			uint64 const fSize64 = ZSTD_findDecompressedSize(srcPtr, fileSizes[fileNb]);
		...
		{   
		    ...
		    assert((uint64)decodedSize == totalDSize64); /* check overflow */
		    SAlloc::F(*resultBufferPtr);
		    *resultBufferPtr = SAlloc::M(decodedSize);
		    if(!(*resultBufferPtr)) {
	...
	{   
	    ...
	    for(nbBlocks = 0, fileNb = 0; fileNb<nbFiles; fileNb++) {
		    ...
		    const uint32 nbBlocksforThisFile = (adv->mode == BMK_decodeOnly) ? 1 : (uint32)((remaining + (blockSize-1)) / blockSize);
		    const uint32 blockEnd = nbBlocks + nbBlocksforThisFile;
		    for(; nbBlocks<blockEnd; nbBlocks++) {
			    ...
			    resSizes[nbBlocks] =
				(adv->mode == BMK_decodeOnly) ? (size_t)ZSTD_findDecompressedSize(srcPtr,
	...
	/* Bench */
	{   
		uint64 const crcOrig = (adv->mode == BMK_decodeOnly) ? 0 : XXH64(srcBuffer, srcSize, 0);
#define NB_MARKS 4
	    ...
	    while(!(compressionCompleted && decompressionCompleted)) {
		    if(!compressionCompleted) {
			    ...
			    {   
					...
					{   
						...
						newResult.cSpeed = (uint64)((double)srcSize * TIMELOOP_NANOSEC / cResult.nanoSecPerRun);
			    ...
			    {   
					...
					OUTPUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (x%5.*f), %6.*f MB/s \r", marks[markNb], displayName,
						(unsigned)srcSize, (unsigned)cSize, ratioAccuracy, ratio, benchResult.cSpeed < (10 * MB_UNIT) ? 2 : 1, (double)benchResult.cSpeed / MB_UNIT);
				}
		    ...
		    if(!decompressionCompleted) {
			    ...
			    {   
					...
					uint64 const newDSpeed = (uint64)((double)srcSize * TIMELOOP_NANOSEC / dResult.nanoSecPerRun);
			    ...
			    {   
					...
					OUTPUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (x%5.*f), %6.*f MB/s, %6.1f MB/s\r",
						marks[markNb], displayName, (unsigned)srcSize, (unsigned)cSize, ratioAccuracy, ratio,
						benchResult.cSpeed < (10 * MB_UNIT) ? 2 : 1, (double)benchResult.cSpeed / MB_UNIT, (double)benchResult.dSpeed / MB_UNIT);
				}
		...
		uint64 const crcCheck = XXH64(resultBuffer, srcSize, 0);
...
BMK_benchOutcome_t BMK_benchMemAdvanced(const void* srcBuffer, size_t srcSize, void* dstBuffer, size_t dstCapacity,
    const size_t* fileSizes, unsigned nbFiles, int cLevel, const ZSTD_compressionParameters* comprParams,
    const void* dictBuffer, size_t dictBufferSize, int displayLevel, const char* displayName, const BMK_advancedParams_t* adv)
{
	...
	void* const internalDstBuffer = dstBuffer ? NULL : SAlloc::M(maxCompressedSize);
	void* const compressedBuffer = dstBuffer ? dstBuffer : internalDstBuffer;
	BMK_benchOutcome_t outcome = BMK_benchOutcome_error(); /* error by default */
	void* resultBuffer = srcSize ? SAlloc::M(srcSize) : NULL;
    ...
    int displayLevel, const BMK_advancedParams_t* adv)
{
	...
	/* Memory allocation */
	srcBuffer = SAlloc::M(benchedSize);
...
static size_t BMK_findMaxMem(uint64 requiredMem)
{
	...
	do {
		testmem = (BYTE*)SAlloc::M((size_t)requiredMem);
		requiredMem -= step;
...
static int BMK_loadFiles(void* buffer, size_t bufferSize, size_t* fileSizes, const char* const * fileNamesTable, unsigned nbFiles, int displayLevel)
{
	size_t pos = 0, totalSize = 0;
	unsigned n;
	for(n = 0; n<nbFiles; n++) {
		uint64 fileSize = UTIL_getFileSize(fileNamesTable[n]); /* last file may be shortened */
...
BMK_benchOutcome_t BMK_benchFilesAdvanced(const char* const * fileNamesTable, unsigned nbFiles, const char* dictFileName, int cLevel,
    const ZSTD_compressionParameters* compressionParams, int displayLevel, const BMK_advancedParams_t* adv)
{
	...
	BMK_benchOutcome_t res;
	uint64 const totalSizeToLoad = UTIL_getTotalFileSize(fileNamesTable, nbFiles);
	if(!nbFiles) {
	...
	fileSizes = (size_t*)SAlloc::C(nbFiles, sizeof(size_t));
	if(!fileSizes) 
		RETURN_ERROR(12, BMK_benchOutcome_t, "not enough memory for fileSizes");
	/* Load dictionary */
	if(dictFileName != NULL) {
		uint64 const dictFileSize = UTIL_getFileSize(dictFileName);
		...
		dictBuffer = SAlloc::M(dictBufferSize);
		if(dictBuffer==NULL) {
			SAlloc::F(fileSizes);
			RETURN_ERROR(11, BMK_benchOutcome_t, "not enough memory for dictionary (%u bytes)", (unsigned)dictBufferSize);
		}
		{   
			int const errorCode = BMK_loadFiles(dictBuffer, dictBufferSize, fileSizes, &dictFileName /*?*/, 1 /*?*/, displayLevel);
		    if(errorCode) {
	...
	if((uint64)benchedSize > totalSizeToLoad) benchedSize = (size_t)totalSizeToLoad;
	if(benchedSize < totalSizeToLoad)
		DISPLAY("Not enough memory; testing %u MB only...\n", (unsigned)(benchedSize >> 20));
	srcBuffer = benchedSize ? SAlloc::M(benchedSize) : NULL;
	if(!srcBuffer) {
		SAlloc::F(dictBuffer);
		SAlloc::F(fileSizes);
	...
	{   
	    ...
	    {   
			...
			res = BMK_benchCLevel(srcBuffer, benchedSize, fileSizes, nbFiles, cLevel, compressionParams, dictBuffer, dictBufferSize, displayLevel, displayName, adv);
		}   
	}
_cleanUp:
	SAlloc::F(srcBuffer);
...
BMK_benchOutcome_t BMK_benchFiles(const char* const * fileNamesTable, unsigned nbFiles,
    const char* dictFileName, int cLevel, const ZSTD_compressionParameters* compressionParams, int displayLevel)
{
	BMK_advancedParams_t const adv = BMK_initAdvancedParams();
