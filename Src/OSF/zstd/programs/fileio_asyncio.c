...
static unsigned AIO_fwriteSparse(FILE* file, const void* buffer, size_t bufferSize, const FIO_prefs_t* const prefs, unsigned storedSkips)
{
	...
	if(!prefs->sparseFileSupport) { /* normal write */
		...
		if(sizeCheck != bufferSize)
			EXM_THROW(70, "Write error : cannot write block : %s", strerror(errno));
		return 0;
	}
	/* avoid int overflow */
	if(storedSkips > 1 GB) {
...
static void AIO_fwriteSparseEnd(const FIO_prefs_t* const prefs, FILE* file, unsigned storedSkips)
{
	if(prefs->testMode) 
...
static IOJob_t * AIO_IOPool_createIoJob(IOPoolCtx_t * ctx, size_t bufferSize) 
{
	...
	void* const buffer = SAlloc::M(bufferSize);
	if(!job || !buffer)
...
{
	WritePoolCtx_t* const ctx = (WritePoolCtx_t*)SAlloc::M(sizeof(WritePoolCtx_t));
