...
*
* If, in the command prompt, there is false specification of which
* accessibility priorities to check, false accessibility checks will be
...
static void getTextNode(TidyDocImpl* doc, Node* node)
{
	tmbstr txtnod = doc->access.textNode;
        ...
        /*
           Continues to traverse through container element until it false
...
static void CheckImage(TidyDocImpl* doc, Node* node)
{
	bool HasAlt = false;
	bool HasIsMap = false;
	bool HasLongDesc = false;
	bool HasDLINK = false;
	bool HasValidHeight = false;
	bool HasValidWidthBullet = false;
	bool HasValidWidthHR = false;
	bool HasTriggeredMissingLongDesc = false;
	...
	if(Level1_Enabled(doc)) {
			...
			if(attrIsALT(av) ) {
				if(av->value != NULL) {
					if((TY_(tmbstrlen) (av->value) < 150) &&
					    (IsPlaceholderAlt(av->value) == false) &&
					    (IsPlaceHolderObject(av->value) == false) &&
					    (EndsWithBytes(av->value) == false) &&
					    (IsImage(av->value) == false)) {
						HasAlt = true;
					}
		...
		if(HasTriggeredMissingLongDesc == false) {
			if((HasDLINK == true)&&
			    (HasLongDesc == false)) {
...
* message is displayed to indicate the error.  If false 'ALT'
...
static void CheckRows(TidyDocImpl* doc, Node* node)
{
	...
	if(numTR == numValidTH)
		doc->access.HasValidRowHeaders = true;

	if(numTR >= 2 &&
	    numTR > numValidTH &&
	    numValidTH >= 2 &&
	    doc->access.HasTH == true)
		doc->access.HasInvalidRowHeader = true;
}
...
static void CheckColumns(TidyDocImpl* doc, Node* node)
{
	...
	if(!isMissingHeader && numTH > 0)
		doc->access.HasValidColumnHeaders = true;

	if(isMissingHeader && numTH >= 2)
		doc->access.HasInvalidColumnHeader = true;
}
...
static void CheckMultiHeaders(TidyDocImpl* doc, Node* node)
{
	...
	bool validColSpanRows = true;
	bool validColSpanColumns = true;
...
static void FreeAccessibilityChecks(TidyDocImpl* ARG_UNUSED(doc) )
{
        /* free any memory allocated for the lists
           ...
           AREA tags are encountered.  Same algorithm, but false
...
static void AccessibilityCheckNode(TidyDocImpl* doc, Node* node)
{
	...
	if(nodeIsBODY(node) ) {
		...
		if(!CheckMetaData(doc, node, false) )
