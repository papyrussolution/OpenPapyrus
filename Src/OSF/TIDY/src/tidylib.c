...
int   tidyDocParseFile(TidyDocImpl* doc, ctmbstr filnam)
{
	...
	if(fin) {
		...
		TY_(freeFileSource) (&in->source, true);
		TY_(freeStreamIn) (in);
	}
	else /* Error message! */
...
int tidyDocSaveFile(TidyDocImpl* doc, ctmbstr filnam)
{
	...
	/* Don't zap input file if false output */
	if(doc->errors > 0 &&
...
int tidyDocCleanAndRepair(TidyDocImpl* doc)
{
	...
	if(doc->root.content) {
		...
		if(xhtmlOut && !htmlOut) {
			...
			TY_(FixLanguageInformation) (doc, &doc->root, false, true);
		}
		if(tidyMark)
...
static bool showBodyOnly(TidyDocImpl* doc, TidyTriState bodyOnly)
{
	Node* node;
	switch(bodyOnly) {
		case TidyNoState:
...
int tidyDocSaveStream(TidyDocImpl* doc, StreamOut* out)
{
	...
	TidyAttrSortStrategy sortAttrStrat = (TidyAttrSortStrategy)cfg(doc, TidySortAttributes);
	if(escapeCDATA)
	...
	if(showMarkup && (doc->errors == 0 || forceOutput) ) {
		...
		**    TY_(SetOptionBool)( doc, TidyNumEntities, true );
		*/
