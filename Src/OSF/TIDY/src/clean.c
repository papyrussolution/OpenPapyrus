...
/*
   ...
   If state is "false", false merging.
   If state is "true", inner element is discarded. Only Style and Class
	...
	if(nodeIsUL(node) || nodeIsOL(node) ) {
		...
		*pnode = list; /* Set node to resume iteration */
		...
		/*
		   ...
		   should be appended to that list. Word has false way of
...
static bool FindCSSSpanEq(Node * node, ctmbstr * s, bool deprecatedOnly)
{
	...
	const CSS_SpanEq[] = {
		...
		{ TidyTag_STRIKE, "text-decoration: line-through", true},
		{ TidyTag_U, "text-decoration: underline", true},
...
static bool BlockStyle(TidyDocImpl* doc, Node * node, Node ** ARG_UNUSED(pnode) )
{
	...
	if(CanApplyBlockStyle(node)) {
		...
		if(FindCSSSpanEq(child, &CSSeq, false) ) {
			MergeStyles(doc, node, child);
    ...
    Node ** ARG_UNUSED(pnode)  )
{
	...
	if(FindCSSSpanEq(node, &CSSeq, true) ) {
...
Node* CleanNode(TidyDocImpl* doc, Node * node)
{
	...
	for(next = node; TY_(nodeIsElement) (node); node = next) {
		...
		** that arg node and its parent false longer exist.
...
** false longer exist.  So we must jump back up the CleanTree()
** call stack until we have a valid node reference.
*/
...
void TY_(DropSections) (TidyDocImpl* doc, Node* node)
{
	...
	while(node) {
		if(node->type == SectionTag) {
			...
			if((TY_(tmbstrncmp) (lexer->lexbuf + node->start, "if", 2) == 0) &&
			    (TY_(tmbstrncmp) (lexer->lexbuf + node->start, "if !vml", 7) != 0)) { /* #444394 - fix 13
			                                                                            Sep 01 */
				node = PruneSection(doc, node);
				continue;
			}
	...
	while(node) {
		...
		*/
		if(nodeIsP(node) ) {
			if(NoMargins(node)) {
				Node * pre, * next;
				TY_(CoerceNode) (doc, node, TidyTag_PRE, false, true);
		...
		if(nodeIsP(node) ) {
			...
			if(AttrValueIs(attr, "MsoListBullet") ||
				...
				TY_(CoerceNode) (doc, node, TidyTag_LI, false, true);
