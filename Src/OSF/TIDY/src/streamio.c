	...
	for(;; ) {
		...
		if(in->encoding == UTF16LE ||
			...
			if(!TY_(IsValidUTF16FromUCS4) (c) ) {
				...
				TY_(ReportEncodingError) (in->doc, INVALID_UTF16, c, true);
				c = 0;
			}
...
static uint ReadCharFromStream(StreamIn* in)
{
	...
	if(in->encoding == UTF8) {
		...
		if(!err && (n == (uint)EndOfStream) && (count == 1)) /* EOF */
			...
			TY_(ReportEncodingError) (in->doc, INVALID_UTF8, n, false);
	...
	if((in->encoding == BIG5) || (in->encoding == SHIFTJIS)) {
		if(c < 128)
			return c;
		else if((in->encoding == SHIFTJIS) && (c >= 0xa1 && c <= 0xdf)) { /* 461643 - fix suggested by Rick
		                                                                    Cameron 14 Sep 01 */
			/*
