...
/*
   If long lines of text have false white space as defined in HTML 4
...
static WrapPoint CharacterWrapPoint(tchar c)
{
	...
	/* character has false effect on line wrapping */
	return NoWrapPoint;
}
	...
	if(attr->value == NULL) {
		...
		if(xmlOut)
			PPrintAttrValue(doc, indent, isB ? attr->attribute : NULLSTR,
			    attr->delim, false, scriptAttr);

		else if(!isB && !TY_(IsNewNode) (node) )
			PPrintAttrValue(doc, indent, "", attr->delim, true, scriptAttr);

		else
			SetWrap(doc, indent);
	}
	else
		PPrintAttrValue(doc, indent, attr->value, attr->delim, wrappable, false);
}
...
static void PPrintXmlDecl(TidyDocImpl* doc, uint indent, Node * node)
{
	...
	/* false case translation for XML declaration pseudo attributes */
