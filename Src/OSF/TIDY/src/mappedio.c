...
int TY_(initFileSource) (TidyAllocator *allocator, TidyInputSource* inp, FILE* fp)
{
	...
	if(!fin)
		return -1;
	fd = fileno(fp);
	if(fstat(fd, &sbuf) == -1 || sbuf.st_size == 0 || (fin->base = mmap(0, fin->size = sbuf.st_size, PROT_READ, MAP_SHARED, fd, 0)) == MAP_FAILED) {
...
{
	DWORD numb = ( ( data->size - data->pos ) > data->gran ) ? data->gran : (DWORD)( data->size - data->pos );
	if(data->view) {
		UnmapViewOfFile(data->view);
		data->view = NULL;
	}
	data->view = (byte *)MapViewOfFile(data->map, FILE_MAP_READ, (DWORD)( data->pos >> 32 ), (DWORD)data->pos, numb);
	if(!data->view) 
		return -1;
	data->iter = data->view;
...
static int TIDY_CALL mapped_getByte(void * sourceData)
{
	MappedFileSource * data = (MappedFileSource *)sourceData;
...
int TY_(DocParseFileWithMappedFile) (TidyDocImpl* doc, ctmbstr filnam) 
{
	...
	if(fin != INVALID_HANDLE_VALUE && cfgBool(doc, TidyKeepFileTimes) && GetFileTime(fin, NULL, (FILETIME*)&actime, (FILETIME*)&modtime) ) {
...
#define TY_I64(str) TYDYAPPEND(str, i64)
	...
	if(fin != INVALID_HANDLE_VALUE) {
		StreamIn* in = MappedFileInput(doc, fin, cfg(doc, TidyInCharEncoding));
		if(!in) {
			CloseHandle(fin);
			return -ENOMEM;
		}
		status = TY_(DocParseStream) (doc, in);
		freeMappedFileSource(&in->source, true);
		TY_(freeStreamIn) (in);
	}
	else /* Error message! */
