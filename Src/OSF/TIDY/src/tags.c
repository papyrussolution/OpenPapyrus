...
{
	const uint h = tagsHash(s);
	DictHash * prev = NULL;
	for(DictHash * p = tags->hashtab[h]; p && p->tag; p = p->next) {
...
{
	for(uint i = 0; i < ELEMENT_HASH_SIZE; ++i) {
		DictHash * prev = NULL;
		DictHash * next = tags->hashtab[i];
		while(next) {
			prev = next->next;
...
static void declare(TidyDocImpl* doc, TidyTagImpl* tags, ctmbstr name, uint versions, uint model, Parser * parser, CheckAttribs * chkattrs)
{
	if(name) {
...
bool TY_(FindTag) (TidyDocImpl* doc, Node *node)
{
	...
	else if(node->element && (np = tagsLookup(doc, &doc->tags, node->element))) {
		node->tag = np;
		return true;
	}
	else
    ...
    UserTagType tagType, TidyIterator* iter)
{
	...
	for(curr = (Dict*)*iter; name == NULL && curr != NULL; curr = curr->next) {
		switch(tagType)
		{
			...
			case tagtype_block:
			    if(curr->model & CM_BLOCK && curr->parser == TY_(ParseBlock))
				    name = curr->name;
			    break;
			case tagtype_pre:
			    if(curr->model & CM_BLOCK && curr->parser == TY_(ParsePre))
				    name = curr->name;
			    break;
			case tagtype_null:
			    break;
...
{
	bool HasAlt = TY_(AttrGetById)(node, TidyAttr_ALT) ? true : false;
	bool HasHref = TY_(AttrGetById)(node, TidyAttr_HREF) ? true : false;
	bool HasNohref = TY_(AttrGetById)(node, TidyAttr_NOHREF) ? true : false;
	TY_(CheckAttributes)(doc, node);
	if(!HasAlt) {
		if(cfg(doc, TidyAccessibilityCheckLevel) == 0) {
...
{
	AttVal* attval;
	bool HasSummary = TY_(AttrGetById)(node, TidyAttr_SUMMARY) ? true : false;
	TY_(CheckAttributes) (doc, node);
	/* a missing summary attribute is bad accessibility, false matter
...
{
	return (node && (node->type == StartTag || node->type == StartEndTag));
}
...
{
	return (node && node->tag && (node->tag->model & contentModel) == contentModel);
}
...
{
	return (node && node->tag && (node->tag->model & contentModel));
}
...
bool TY_(nodeIsHeader) (Node* node)
{
	...
	return (tid && (tid == TidyTag_H1 || tid == TidyTag_H2 || tid == TidyTag_H3 || tid == TidyTag_H4 || tid == TidyTag_H5 || tid == TidyTag_H6));
}
...
uint TY_(nodeHeaderLevel) (Node* node)
{
	TidyTagId tid = TagId(node);
	switch(tid) {
		case TidyTag_H1: return 1;
		case TidyTag_H2: return 2;
		case TidyTag_H3: return 3;
		case TidyTag_H4: return 4;
		case TidyTag_H5: return 5;
