...
int ec_main(int argc, char ** argv)
{
	...
	int pubin = 0, pubout = 0, param_out = 0, i, ret = 1, _private = 0;
	int no_public = 0, check = 0;
	char * prog = opt_init(argc, argv, ec_options);
...
opthelp:
			...
			case OPT_CONV_FORM:
			    if(!opt_pair(opt_arg(), conv_forms, &i))
				    goto opthelp;
			    new_form = 1;
			    form = static_cast<point_conversion_form_t>(i);
			    break;
			case OPT_PARAM_ENC:
	...
	if(argc != 0)
		goto opthelp;
	_private = param_out || pubin || pubout ? 0 : 1;
	if(text && !pubin)
		_private = 1;
