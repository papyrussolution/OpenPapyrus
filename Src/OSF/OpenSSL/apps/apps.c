...
int chopup_args(ARGS * arg, char * buf)
{
	...
	if(arg->size == 0) {
		arg->size = 20;
		arg->argv = static_cast<char **>(app_malloc(sizeof(*arg->argv) * arg->size, "argv space"));
	}
	...
	for(p = buf;;) {
		...
		if(arg->argc >= arg->size) {
			char ** tmp;
			arg->size += 20;
			tmp = static_cast<char **>(OPENSSL_realloc(arg->argv, sizeof(*arg->argv) * arg->size));
			if(tmp == NULL)
				return 0;
			arg->argv = tmp;
		}
...
static int ui_read(UI * ui, UI_STRING * uis)
{
	if(UI_get_input_flags(uis) & UI_INPUT_FLAG_DEFAULT_PWD
		...
		switch(UI_get_string_type(uis)) {
			...
			case UIT_VERIFY:
		    {
			    const char * password = static_cast<const char *>(((PW_CB_DATA*)UI_get0_user_data(ui))->password);
...
int password_callback(char * buf, int bufsiz, int verify, PW_CB_DATA * cb_tmp)
{
	...
	if(ui) {
		...
		if(ok >= 0 && verify) {
			buff = static_cast<char *>(app_malloc(bufsiz, "password buffer"));
...
CA_DB * load_index(const char * dbfile, DB_ATTR * db_attr)
{
	...
	retdb = static_cast<CA_DB *>(app_malloc(sizeof(*retdb), "new DB"));
	retdb->db = tmpdb;
	tmpdb = NULL;
	if(db_attr)
...
int index_index(CA_DB * db)
{
	...
	if(db->attributes.unique_subject && !TXT_DB_create_index(db->db, DB_name, index_name_qual, LHASH_HASH_FN(index_name), LHASH_COMP_FN(index_name))) {
...
int save_index(const char * dbfile, const char * suffix, CA_DB * db)
{
	char buf[3][BSIZE];
	BIO * out;
	int j = strlen(dbfile) + strlen(suffix);
	if(j + 6 >= BSIZE) {
		BIO_printf(bio_err, "file name too long\n");
		goto err;
	}
...
{
	char buf[5][BSIZE];
	int i = strlen(dbfile) + strlen(old_suffix);
	int j = strlen(dbfile) + strlen(new_suffix);
	if(i > j)
		j = i;
	if(j + 6 >= BSIZE) {
		BIO_printf(bio_err, "file name too long\n");
...
X509_NAME * parse_name(const char * cp, long chtype, int canmulti)
{
	...
	while(*cp) {
		...
		while(*cp && *cp != '=')
			*bp++ = *cp++;
		if(*cp == '\0') {
			BIO_printf(bio_err, "%s: Hit end of string before finding the equals.\n", opt_getprog());
			goto err;
		}
		*bp++ = '\0';
		++cp;
...
int pkey_ctrl_string(EVP_PKEY_CTX * ctx, const char * value)
{
	int rv;
	char * vtmp = NULL;
	char * stmp = OPENSSL_strdup(value);
	if(!stmp)
		return -1;
	vtmp = strchr(stmp, ':');
	if(vtmp) {
...
unsigned char * next_protos_parse(size_t * outlen, const char * in)
{
	...
	if(len >= 65535)
		return NULL;
	out = static_cast<uchar *>(app_malloc(strlen(in) + 1, "NPN buffer"));
	for(i = 0; i <= len; ++i) {
		if(i == len || in[i] == ',') {
			if(i - start > 255) {
				OPENSSL_free(out);
...
static int WIN32_rename(const char * from, const char * to)
{
	...
	if(sizeof(TCHAR) == 1) {
		...
		tfrom = static_cast<TCHAR *>(malloc(sizeof(*tfrom) * (flen + tlen)));
		if(tfrom == NULL)
			goto err;
		tto = tfrom + flen;
