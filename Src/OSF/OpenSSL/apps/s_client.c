...
static unsigned int psk_client_cb(SSL * ssl, const char * hint, char * identity, unsigned int max_identity_len, unsigned char * psk, unsigned int max_psk_len)
{
	...
	if(key_len > max_psk_len) {
		BIO_printf(bio_err, "psk buffer of callback is too small (%d) for key (%ld)\n", max_psk_len, key_len);
		OPENSSL_free(key);
		return 0;
	}
...
static int srp_Verify_N_and_g(const BIGNUM * N, const BIGNUM * g)
{
	...
	BIGNUM * p = BN_new();
	BIGNUM * r = BN_new();
	int ret = g != NULL && N != NULL && bn_ctx != NULL && BN_is_odd(N) &&
...
{
	SRP_ARG * srp_arg = (SRP_ARG*)arg;
	char * pass = static_cast<char *>(app_malloc(PWD_STRLEN + 1, "SRP password buffer"));
	PW_CB_DATA cb_tmp;
	int l;
...
{
	tlsextnextprotoctx * ctx = static_cast<tlsextnextprotoctx *>(arg);
	if(!c_quiet) {
...
static ossl_ssize_t hexdecode(const char ** inptr, void * result)
{
	...
	const char * in = *inptr;
	unsigned char * ret = static_cast<uchar *>(app_malloc(strlen(in) / 2, "hexdecode"));
...
static ossl_ssize_t checked_uint8(const char ** inptr, void * out)
{
	...
	v = strtol(in, &endp, 10);
	e = restore_errno();
	if(((v == LONG_MIN || v == LONG_MAX) && e == ERANGE) || endp == in || !isspace(_UC(*endp)) ||
	    v != (*result = (uint8_t)v)) {
		return -1;
	}
...
#define IS_INET_FLAG(o) (o == OPT_4 || o == OPT_6 || o == OPT_HOST || o == OPT_PORT || o == OPT_CONNECT)
#define IS_UNIX_FLAG(o) (o == OPT_UNIX)
#define IS_PROT_FLAG(o) (o == OPT_SSL3 || o == OPT_TLS1 || o == OPT_TLS1_1 || o == OPT_TLS1_2 || o == OPT_DTLS || o == OPT_DTLS1 || o == OPT_DTLS1_2)
...
# endif
	...
	cbuf = static_cast<char *>(app_malloc(BUFSIZZ, "cbuf"));
	sbuf = static_cast<char *>(app_malloc(BUFSIZZ, "sbuf"));
	mbuf = static_cast<char *>(app_malloc(BUFSIZZ, "mbuf"));
...
opthelp:
			...
			case OPT_S_CASES:
			    ...
			    if(ssl_args == NULL || !sk_OPENSSL_STRING_push(ssl_args, opt_flag()) || !sk_OPENSSL_STRING_push(ssl_args, opt_arg())) {
			...
			case OPT_DANE_TLSA_RRDATA:
				SETIFZ(dane_tlsa_rrset, sk_OPENSSL_STRING_new_null());
	...
	if(proxystr) {
		...
		if(tmp_port != port)
			OPENSSL_free(tmp_port);
		if(!res) {
			BIO_printf(bio_err, "%s: -connect argument malformed or ambiguous\n", prog);
			goto end;
		}
	}
	...
	if(crl_file) {
		X509_CRL * crl = load_crl(crl_file, crl_format);
		if(crl == NULL) {
...
# endif
	...
	if(dane_tlsa_domain != NULL) {
		...
		if(dane_tlsa_rrset == NULL) {
			BIO_printf(bio_err, "%s: DANE TLSA authentication requires at least one -dane_tlsa_rrdata option.\n", prog);
			goto end;
		}
		if(tlsa_import_rrset(con, dane_tlsa_rrset) <= 0) {
			BIO_printf(bio_err, "%s: Failed to import any TLSA records.\n", prog);
			goto end;
		}
		if(dane_ee_no_name)
		...
		BIO_printf(bio_err, "%s: DANE TLSA authentication requires the -dane_tlsa_domain option.\n", prog);
		goto end;
	}
re_start:
	...
	if(nbio_test) {
		BIO * test = BIO_new(BIO_f_nbio_test());
	...
	switch((PROTOCOL_CHOICE)starttls_proto) {
		...
		case PROTO_SMTP:
	    {
		    ...
		    BIO_free(fbio);
		    if(!foundit)
			    BIO_printf(bio_err, "didn't find starttls in server response, trying anyway...\n");
	    ...
	    {
		    int seen = 0;
		    BIO_printf(sbio, "<stream:stream xmlns:stream='http://etherx.jabber.org/streams' xmlns='jabber:%s' to='%s' version='1.0'>",
				starttls_proto == PROTO_XMPP ? "client" : "server", xmpphost ? xmpphost : host);
		...
		case PROTO_TELNET:
	    {
		    static const unsigned char tls_do[] = {/* IAC    DO   START_TLS */ 255,   253, 46 };
		    static const unsigned char tls_will[] = { /* IAC  WILL START_TLS */ 255, 251, 46 };
		    static const unsigned char tls_follows[] = { /* IAC  SB   START_TLS FOLLOWS IAC  SE */ 255, 250, 46,       1,      255, 240 };
		    /* Telnet server should demand we issue START_TLS */
		    int bytes = BIO_read(sbio, mbuf, BUFSIZZ);
		...
		case PROTO_IRC:
	    {
		    ...
		    do {
				...
				&& !BIO_pending(fbio)
				&& !BIO_pending(sbio)
				&& select(width, static_cast<fd_set *>(&readfds), NULL, NULL,
				&timeout) < 1) {
				    BIO_printf(bio_err, "Timeout waiting for response (%d seconds).\n", S_CLIENT_IRC_READ_TIMEOUT);
				    break;
			    }
			    ...
			    if((numeric == 451 || numeric == 421) && strstr(mbuf, "STARTTLS") != NULL) {
		...
		if(!ssl_pending) {
			...
			if(!write_tty) {
				if(read_tty) {
					...
					i = select(width, &readfds, &writefds, NULL, timeoutp);
			}
#else
			i = select(width, (void*)&readfds, (void*)&writefds, NULL, timeoutp);
#endif
			if(i < 0) {
...
static void print_stuff(BIO * bio, SSL * s, int full)
{
	...
	if(SSL_get_session(s) != NULL && keymatexportlabel != NULL) {
		...
		exportedkeymat = static_cast<uchar *>(app_malloc(keymatexportlen, "export key"));
...
static int ocsp_resp_cb(SSL * s, void * arg)
{
	...
	BIO_puts(static_cast<BIO *>(arg), "OCSP response: ");
	if(!p) {
		BIO_puts(static_cast<BIO *>(arg), "no response sent\n");
		return 1;
	}
	rsp = d2i_OCSP_RESPONSE(NULL, &p, len);
	if(!rsp) {
		BIO_puts(static_cast<BIO *>(arg), "response parse error\n");
		BIO_dump_indent(static_cast<BIO *>(arg), (char*)p, len, 4);
		return 0;
	}
	BIO_puts(static_cast<BIO *>(arg), "\n======================================\n");
	OCSP_RESPONSE_print(static_cast<BIO *>(arg), rsp, 0);
	BIO_puts(static_cast<BIO *>(arg), "======================================\n");
