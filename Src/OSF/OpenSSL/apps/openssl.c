...
static int apps_startup()
{
	...
	if(!OPENSSL_init_crypto(OPENSSL_INIT_ENGINE_ALL_BUILTIN|OPENSSL_INIT_LOAD_CONFIG, NULL))
		return 0;
...
static char * make_config_name()
{
	...
	p = static_cast<char *>(app_malloc(len, "config filename buffer"));
	strcpy(p, t);
...
{
	if(c)
		BIO_printf(static_cast<BIO *>(arg), "%s\n", EVP_CIPHER_name(c));
	else {
		SETIFZ(from, "<undefined>");
		SETIFZ(to, "<undefined>");
		BIO_printf(static_cast<BIO *>(arg), "%s => %s\n", from, to);
	}
}
...
OPTIONS list_options[] = {
	...
	{"digest-commands", OPT_DIGEST_COMMANDS, '-', "List of message digest commands"},
	{"digest-algorithms", OPT_DIGEST_ALGORITHMS, '-', "List of message digest algorithms"},
	{"cipher-commands", OPT_CIPHER_COMMANDS, '-', "List of cipher commands"},
	{"cipher-algorithms", OPT_CIPHER_ALGORITHMS, '-', "List of cipher algorithms"},
	{"public-key-algorithms", OPT_PK_ALGORITHMS, '-', "List of public key algorithms"},
...
int list_main(int argc, char ** argv)
{
	HELPLIST_CHOICE o;
	int done = 0;
	char * prog = opt_init(argc, argv, list_options);
	while((o = static_cast<HELPLIST_CHOICE>(opt_next())) != OPT_EOF) {
		switch(o) {
...
int help_main(int argc, char ** argv)
{
	...
	FUNC_TYPE tp;
	HELPLIST_CHOICE o;
	char * prog = opt_init(argc, argv, help_options);
	while((o = static_cast<HELPLIST_CHOICE>(opt_next())) != OPT_EOF) {
		switch(o) {
			default:
			    BIO_printf(bio_err, "%s: Use -help for summary.\n", prog);
...
{
	int i = 0;
	for(FUNCTION * fp = functions; fp->name != NULL; fp++)
		if(fp->type == ft) {
			if((i++ % COLUMNS) == 0)
				BIO_printf(bio_out, "\n");
...
static void list_pkey(void)
{
	for(int i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
		...
		EVP_PKEY_asn1_get0_info(&pkey_id, &pkey_base_id, &pkey_flags, &pinfo, &pem_str, ameth);
...
{
	const FUNCTION * f1 = static_cast<const FUNCTION *>(_f1);
	const FUNCTION * f2 = static_cast<const FUNCTION *>(_f2);
	if(f1->type != f2->type)
