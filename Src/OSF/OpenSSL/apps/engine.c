...
static int append_buf(char ** buf, int * size, const char * s)
{
	if(*buf == NULL) {
		*size = 256;
		*buf = static_cast<char *>(app_malloc(*size, "engine buffer"));
		**buf = '\0';
	}
	...
	if(strlen(*buf) + strlen(s) >= (unsigned int)*size) {
		char * tmp;
		*size += 256;
		tmp = static_cast<char *>(OPENSSL_realloc(*buf, *size));
		if(tmp == NULL) {
			OPENSSL_free(*buf);
			*buf = NULL;
			return 0;
	...
	do {
		...
		if(!(flags & ENGINE_CMD_FLAG_INTERNAL) || verbose >= 4) {
			...
			if((len = ENGINE_ctrl(e, ENGINE_CTRL_GET_NAME_LEN_FROM_CMD, num,
			    NULL, NULL)) <= 0)
				goto err;
			name = static_cast<char *>(app_malloc(len + 1, "name buffer"));
			...
			if((len = ENGINE_ctrl(e, ENGINE_CTRL_GET_DESC_LEN_FROM_CMD, num,
				...
				goto err;
			if(len > 0) {
				desc = static_cast<char *>(app_malloc(len + 1, "description buffer"));
