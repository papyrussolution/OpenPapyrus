...
int ssl_print_curves(BIO * out, SSL * s, int noshared)
{
	...
	if(ncurves <= 0)
		return 1;
	curves = static_cast<int *>(app_malloc(ncurves * sizeof(int), "curves to print"));
    ...
    unsigned int * cookie_len)
{
	...
	length += sizeof(port);
	buffer = static_cast<uchar *>(app_malloc(length, "cookie generate buffer"));
...
{
	int i, rv;
	SSL_EXCERT * exc = static_cast<SSL_EXCERT *>(arg);
...
{
	SSL_EXCERT * exc = static_cast<SSL_EXCERT *>(app_malloc(sizeof(*exc), "prepend cert"));
...
static char * hexencode(const unsigned char * data, size_t len)
{
	...
	cp = out = static_cast<char *>(app_malloc(ilen, "TLSA hex data buffer"));
...
{
	security_debug_ex * sdb = static_cast<security_debug_ex *>(ex);
	...
	switch(op & SSL_SECOP_OTHER_TYPE) {
		case SSL_SECOP_OTHER_CIPHER:
		    BIO_puts(sdb->out, SSL_CIPHER_get_name(static_cast<SSL_CIPHER *>(other)));
		    break;
#ifndef OPENSSL_NO_EC
	    ...
	    {
		    if(cert_md) {
			    int sig_nid = X509_get_signature_nid(static_cast<const X509 *>(other));
			    BIO_puts(sdb->out, OBJ_nid2sn(sig_nid));
		    }
		    else {
			    EVP_PKEY * pkey = static_cast<EVP_PKEY *>(X509_get0_pubkey(static_cast<const X509 *>(other)));
			    const char * algname = "";
	    ...
	    {
		    const unsigned char * salg = static_cast<const uchar *>(other);
