...
{
	tlsextstatusctx * srctx = static_cast<tlsextstatusctx *>(arg);
...
{
	tlsextnextprotoctx * next_proto = static_cast<tlsextnextprotoctx *>(arg);
...
{
	tlsextalpnctx * alpn_ctx = reinterpret_cast<tlsextalpnctx *>(arg);
	if(!s_quiet) {
...
# ifndef OPENSSL_NO_NEXTPROTONEG
	...
	if(!SSL_CTX_set_session_id_context(ctx, reinterpret_cast<const uchar *>(&s_server_session_id_context), sizeof s_server_session_id_context)) {
	...
	if(ctx2) {
		...
		if(!SSL_CTX_set_session_id_context(ctx2, reinterpret_cast<const uchar *>(&s_server_session_id_context), sizeof s_server_session_id_context)) {
			BIO_printf(bio_err, "error setting session id context\n");
			ERR_print_errors(bio_err);
			goto end;
		}
...
static int sv_body(int s, int stype, unsigned char * context)
{
	...
	buf = static_cast<char *>(app_malloc(bufsize, "server buffer"));
	if(s_nbio) {
...
static int www_body(int s, int stype, unsigned char * context)
{
	...
	buf = static_cast<char *>(app_malloc(bufsize, "server www buffer"));
...
static int rev_body(int s, int stype, unsigned char * context)
{
	...
	buf = static_cast<char *>(app_malloc(bufsize, "server rev buffer"));
	io = BIO_new(BIO_f_buffer());
	ssl_bio = BIO_new(BIO_f_ssl());
	if((io == NULL) || (ssl_bio == NULL))
		goto err;
...
{
	simple_ssl_session * sess = reinterpret_cast<simple_ssl_session *>(app_malloc(sizeof(*sess), "get session"));
	unsigned char * p;
	...
	sess->id = static_cast<uchar *>(OPENSSL_memdup(SSL_SESSION_get_id(session, NULL), sess->idlen));
	sess->der = static_cast<uchar *>(app_malloc(sess->derlen, "get session buffer"));
	if(!sess->id) {
