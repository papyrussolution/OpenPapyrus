...
opthelp:
			...
			case OPT_CONV_FORM:
			    if(!opt_pair(opt_arg(), forms, &new_form))
				    goto opthelp;
			    form = static_cast<point_conversion_form_t>(new_form);
			    new_form = 1;
			    break;
			case OPT_PARAM_ENC:
	...
	if(list_curves) {
		...
		size_t n;
		curves = static_cast<EC_builtin_curve *>(app_malloc((int)sizeof(*curves) * crv_len, "list curves"));
	...
	if(C) {
		...
		buffer = static_cast<uchar *>(app_malloc(buf_len, "BN buffer"));
