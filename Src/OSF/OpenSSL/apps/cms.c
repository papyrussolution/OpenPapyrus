...
opthelp:
			...
			case OPT_RCTFORM:
			    if(rctformat == FORMAT_SMIME)
				    ...
				    rcms = 0;// @sobolev PEM_read_bio_CMS(rctin, NULL, NULL, NULL);
			    else if(rctformat == FORMAT_ASN1)
				    if(!opt_format(opt_arg(), OPT_FMT_PEMDER | OPT_FMT_SMIME, &rctformat))
					    goto opthelp;
			    break;
			case OPT_CERTFILE:
			    certfile = opt_arg();
			...
			case OPT_KEYOPT:
			    ...
			    if(key_param == NULL || key_param->idx != keyidx) {
				    ...
				    nparam = static_cast<cms_key_param *>(app_malloc(sizeof(*nparam), "key param buffer"));
				    nparam->idx = keyidx;
...
# endif
	...
	if(operation & SMIME_IP) {
		if(informat == FORMAT_SMIME)
			...
			cms = 0; //@sobolev PEM_read_bio_CMS(in, NULL, NULL, NULL);
