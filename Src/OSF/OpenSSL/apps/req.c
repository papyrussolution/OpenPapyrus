...
int req_main(int argc, char ** argv)
{
	...
	char * _template = default_config_file, * keyout = NULL;
	const char * keyalg = NULL;
	OPTION_CHOICE o;
	int ret = 1, x509 = 0, days = 30, i = 0, newreq = 0, verbose = 0;
	int pkey_type = -1, _private = 0;
...
opthelp:
	...
	req_conf = app_load_config(_template);
	if(_template != default_config_file && !app_load_modules(req_conf))
		goto end;
	if(req_conf != NULL) {
	...
	if(newreq && (pkey == NULL)) {
		...
		out = bio_open_owner(keyout, outformat, _private);
		if(out == NULL)
			goto end;
...
{
	char c = '*';
	BIO * b = static_cast<BIO *>(EVP_PKEY_CTX_get_app_data(ctx));
	int p;
