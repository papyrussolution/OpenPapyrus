...
again2:
	...
	row[DB_exp_date] = static_cast<OPENSSL_STRING>(app_malloc(tm->length + 1, "row expdate"));
	...
	irow = static_cast<OPENSSL_STRING *>(app_malloc(sizeof(*irow) * (DB_NUMBER + 1), "row space"));
	for(i = 0; i < DB_NUMBER; i++) {
		irow[i] = row[i];
		row[i] = NULL;
	}
...
static int do_revoke(X509 * x509, CA_DB * db, int type, char * value)
{
	...
	if(rrow == NULL) {
		...
		row[DB_exp_date] = static_cast<char *>(app_malloc(tm->length + 1, "row exp_data"));
		memcpy(row[DB_exp_date], tm->data, tm->length);
		row[DB_exp_date][tm->length] = '\0';
		row[DB_rev_date] = NULL;
		row[DB_file] = OPENSSL_strdup("unknown");

		irow = static_cast<char **>(app_malloc(sizeof(*irow) * (DB_NUMBER + 1), "row ptr"));
		for(i = 0; i < DB_NUMBER; i++) {
			irow[i] = row[i];
			row[i] = NULL;
		}
...
static int get_certificate_status(const char * serial, CA_DB * db)
{
	...
	row[DB_serial] = static_cast<char *>(app_malloc(serial_len + 2, "row serial#"));
...
static int do_updatedb(CA_DB * db)
{
	...
	a_tm_s = static_cast<char *>(app_malloc(a_tm->length + 1, "time string"));
...
char * make_revocation_str(int rev_type, char * rev_arg)
{
	...
	str = static_cast<char *>(app_malloc(i, "revocation reason"));
