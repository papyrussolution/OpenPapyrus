...
# endif
	...
	if(passwds == NULL) {
		...
		passwd = passwd_malloc = static_cast<char *>(app_malloc(passwd_malloc_size, "password buffer"));
	}
...
# ifndef OPENSSL_NO_DES
		if(usecrypt) {
			if(*salt_malloc_p == NULL) {
				*salt_p = *salt_malloc_p = static_cast<char *>(app_malloc(3, "salt buffer"));
			}
...
# ifndef NO_MD5CRYPT_1
		if(use1 || useapr1) {
			int i;
			if(*salt_malloc_p == NULL) {
				*salt_p = *salt_malloc_p = static_cast<char *>(app_malloc(9, "salt buffer"));
			}
