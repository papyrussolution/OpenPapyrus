...
# endif
	...
	int ret = 1, macver = 1, add_lmk = 0, _private = 0;
	int noprompt = 0;
...
opthelp:
	...
	if(export_cert) {
		...
		if(options & NOCERTS)
			chain = 0;
		if(!(options & NOKEYS)) {
			key = load_key(keyname ? keyname : infile, FORMAT_PEM, 1, passin, e, "private key");
			if(!key)
				goto export_end;
		}
		...
		assert(_private);
		out = bio_open_owner(outfile, FORMAT_PKCS12, _private);
		if(out == NULL)
			goto end;
...
static int alg_print(const X509_ALGOR * alg)
{
	...
	if(pbenid == NID_pbes2) {
		...
		if(aparamtype == V_ASN1_SEQUENCE)
			pbe2 = static_cast<PBE2PARAM *>(ASN1_item_unpack(static_cast<const ASN1_STRING *>(aparam), ASN1_ITEM_rptr(PBE2PARAM)));
		if(pbe2 == NULL) {
		...
		if(pbenid == NID_id_pbkdf2) {
			...
			int prfnid;
			if(aparamtype == V_ASN1_SEQUENCE)
				kdf = static_cast<PBKDF2PARAM *>(ASN1_item_unpack(static_cast<const ASN1_STRING *>(aparam), ASN1_ITEM_rptr(PBKDF2PARAM)));
			if(kdf == NULL) {
				BIO_puts(bio_err, "<unsupported parameters>");
				goto done;
			}
		...
		if(aparamtype == V_ASN1_SEQUENCE)
			pbe = static_cast<PBEPARAM *>(ASN1_item_unpack(static_cast<const ASN1_STRING *>(aparam), ASN1_ITEM_rptr(PBEPARAM)));
		if(pbe == NULL) {
			BIO_puts(bio_err, "<unsupported parameters>");
			goto done;
		}
