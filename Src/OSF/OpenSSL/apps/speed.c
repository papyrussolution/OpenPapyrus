...
typedef void *(* kdf_fn) (const void * in, size_t inlen, void * out, size_t * xoutlen);
...
OPTIONS speed_options[] = {
	...
	{"mb", OPT_MB, '-', "Enable (tls1.1) multi-block mode on evp_cipher requested with -evp"},
...
static void * KDF1_SHA1(const void * in, size_t inlen, void * out, size_t * outlen)
{
	...
	*outlen = SHA_DIGEST_LENGTH;
	return SHA1(static_cast<const uchar *>(in), inlen, static_cast<uchar *>(out));
}
#endif                          /* OPENSSL_NO_EC */
...
#endif
#ifndef OPENSSL_NO_RSA
	static const unsigned int rsa_bits[RSA_NUM] = { 512, 1024, 2048, 3072, 4096, 7680, 15360 };
	static const unsigned char * rsa_data[RSA_NUM] = { test512, test1024, test2048, test3072, test4096, test7680, test15360 };
...
opterr:
			...
			case OPT_MB:
			    ...
			    BIO_printf(bio_err, "%s: -mb specified but multi-block support is disabled\n", prog);
			    goto end;
#endif
			    break;
		}
...
# endif
	...
	loopargs = static_cast<loopargs_t *>(app_malloc(loopargs_len * sizeof(loopargs_t), "array of loopargs"));
	...
	for(i = 0; i < loopargs_len; i++) {
		...
		loopargs[i].buf_malloc = static_cast<uchar *>(app_malloc((int)BUFSIZE + MAX_MISALIGNMENT + 1, "input buffer"));
		loopargs[i].buf2_malloc = static_cast<uchar *>(app_malloc((int)BUFSIZE + MAX_MISALIGNMENT + 1, "input buffer"));
		/* Align the start of buffers on a 64 byte boundary */
		loopargs[i].buf = loopargs[i].buf_malloc + misalign;
		loopargs[i].buf2 = loopargs[i].buf2_malloc + misalign;
#ifndef OPENSSL_NO_EC
		loopargs[i].secret_a = static_cast<uchar *>(app_malloc(MAX_ECDH_SIZE, "ECDH secret a"));
		loopargs[i].secret_b = static_cast<uchar *>(app_malloc(MAX_ECDH_SIZE, "ECDH secret b"));
#endif
	}
...
static void multiblock_speed(const EVP_CIPHER * evp_cipher)
{
	...
	inp = static_cast<uchar *>(app_malloc(mblengths[num - 1], "multiblock input buffer"));
	out = static_cast<uchar *>(app_malloc(mblengths[num - 1] + 1024, "multiblock output buffer"));
