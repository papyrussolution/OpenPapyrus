...
static int validate_argv(int argc)
{
	...
	if(argc >= size) {
		...
		while(argc >= size)
			size += 64;
		ptr = static_cast<char **>(realloc(newargv, size * sizeof(newargv[0])));
		if(ptr == NULL)
			return 0;
...
static int process_glob(WCHAR * wstr, int wlen)
{
	...
	do {
		...
		arg = static_cast<char *>(malloc(udlen + uflen));
		if(arg == NULL)
			break;
...
void win32_utf8argv(int * argc, char ** argv[])
{
	...
	p = static_cast<WCHAR *>(_alloca((wlen + 1) * sizeof(WCHAR)));
	wcscpy(p, wcmdline);
	...
	while(*p != L'\0') {
		...
		if(wlen == 0 || !process_glob(warg, wlen)) {
			...
			arg = static_cast<char *>(malloc(ulen + 1));
			if(arg == NULL) {
				valid = 0;
				break;
			}
