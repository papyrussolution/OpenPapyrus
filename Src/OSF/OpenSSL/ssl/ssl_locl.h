...
#define SSL_PSK     (SSL_kPSK | SSL_kRSAPSK | SSL_kECDHEPSK | SSL_kDHEPSK) /* all PSK */
...
struct ssl_method_st {
	...
	int (* ssl_read_bytes)(SSL * s, int type, int * recvd_type, unsigned char * buf, int len, int peek);
...
struct ssl_session_st {
	 ...
	 * appropriate context. It is up to the application to set this, via SSL_new
	 */
...
struct ssl_ctx_st {
	...
	struct {
		...
		int sess_cb_hit; // session-id that was not in the cache was passed back via the callback.  This
...
# endif
	...
	unsigned int max_pipelines; /* Up to how many pipelines should we use? If 0 then 1 is assumed */
	size_t default_read_buf_len; /* The default read buffer length to use (0 means not set) */
# ifndef OPENSSL_NO_ENGINE
	ENGINE * client_cert_engine; // Engine to pass requests for client certs to
# endif
	int (* tlsext_servername_callback)(SSL *, int *, void *); /* TLS extensions servername callback */
	...
	int (* tlsext_status_cb)(SSL * ssl, void * arg); /* Callback for status request */
...
# endif                         /* OPENSSL_NO_EC */
	int tlsext_status_type; // ext status type used for CSR extension (OCSP Stapling) 
	CRYPTO_RWLOCK * lock;
};
...
struct ssl_st {
	...
	OSSL_STATEM statem; /* where we are */
	...
	X509_VERIFY_PARAM * param;
	SSL_DANE dane; /* Per connection DANE state */
	/* crypto */
	STACK_OF(SSL_CIPHER) *cipher_list;
...
# endif
	SSL_CTX * ctx;
	STACK_OF(X509) * verified_chain; /* Verified chain of peer */
	long verify_result;
	CRYPTO_EX_DATA ex_data; /* extra application data */
	STACK_OF(X509_NAME) *client_CA; /* for server side, keep the list of CA_dn we can use */
	int references;
	...
	void (* tlsext_debug_cb)(SSL * s, int client_server, int type, const unsigned char * data, int len, void * arg);
...
# ifndef OPENSSL_NO_CT
	 ...
	 */
	STACK_OF(SCT) *scts;
	unsigned char * tlsext_scts; /* Raw extension data, if seen */
	uint16_t tlsext_scts_len; /* Length of raw extension data, if seen */
	int scts_parsed; /* Have we attempted to find/parse SCTs yet? */
# endif
	int tlsext_status_expected; /* Expect OCSP CertificateStatus message */
	STACK_OF(OCSP_RESPID) *tlsext_ocsp_ids; /* OCSP status request only */
	X509_EXTENSIONS * tlsext_ocsp_exts;
	unsigned char * tlsext_ocsp_resp; /* OCSP response received or to be sent */
	int tlsext_ocsp_resplen;
	int tlsext_ticket_expected; /* RFC4507 session ticket expected to be received or sent */
# ifndef OPENSSL_NO_EC
	size_t tlsext_ecpointformatlist_length;
	/* our list */
	unsigned char * tlsext_ecpointformatlist;
...
# endif
	...
	unsigned char * alpn_client_proto_list;
	unsigned alpn_client_proto_list_len;
	int tlsext_use_etm; /* Set to one if we have negotiated ETM */
	/*-
...
# endif
		int cert_request;
		unsigned char * ciphers_raw; /* Raw values of the cipher list from a client */
		size_t ciphers_rawlen;
		unsigned char * pms; /* Temporary storage for premaster secret */
		size_t pmslen;
# ifndef OPENSSL_NO_PSK
		unsigned char * psk; /* Temporary storage for PSK key */
		size_t psklen;
# endif
		/*
		...
		size_t peer_sigalgslen; /* Size of above array */
		const EVP_MD * peer_md; /* Digest peer uses for signing */
		const EVP_MD * md[SSL_PKEY_NUM]; /* Array of digests used for signing */
		/*
...
# endif
	...
	unsigned char * alpn_proposed; /* used by the server to know what options were proposed */
	size_t alpn_proposed_len;
	int alpn_sent; /* used by the client to know if it actually sent alpn */
# ifndef OPENSSL_NO_EC
	/*
...
struct dtls1_timeout_st {
	unsigned int read_timeouts; /* Number of read timeouts so far */
	unsigned int write_timeouts; /* Number of write timeouts so far */
	unsigned int num_alerts; /* Number of alerts received so far */
};
...
typedef struct dtls1_state_st {
	...
	pqueue * buffered_messages; /* Buffered handshake messages */
	pqueue * sent_messages; /* Buffered (sent) handshake records */
	...
	unsigned short timeout_duration; /* Timeout duration */
...
#define SSL_ENC_FLAG_EXPLICIT_IV        0x1 /* Uses explicit IV for CBC mode */
#define SSL_ENC_FLAG_SIGALGS            0x2 /* Uses signature algorithms extension */
#define SSL_ENC_FLAG_SHA256_PRF         0x4 /* Uses SHA256 default PRF */
#define SSL_ENC_FLAG_DTLS               0x8 /* Is DTLS */
/*
 * Allow TLS 1.2 ciphersuites: applies to DTLS 1.2 as well as TLS 1.2: may
 * apply to others in future.
 */
...
#define IMPLEMENT_ssl3_meth_func(func_name, s_accept, s_connect) const SSL_METHOD *func_name(void)  \
	{ \
		static const SSL_METHOD func_name ## _data = { \
			SSL3_VERSION, \
...
__owur int ssl_get_prev_session(SSL * s, const PACKET * ext, const PACKET * session_id);
...
__owur STACK_OF(SSL_CIPHER) *ssl_create_cipher_list(const SSL_METHOD *meth, STACK_OF(SSL_CIPHER) **pref, STACK_OF(SSL_CIPHER) **sorted, const char * rule_str, CERT *c);
void ssl_update_cache(SSL * s, int mode);
__owur int ssl_cipher_get_evp(const SSL_SESSION * s, const EVP_CIPHER ** enc, const EVP_MD ** md, int * mac_pkey_type, int * mac_secret_size, SSL_COMP ** comp, int use_etm);
...
__owur int ssl3_final_finish_mac(SSL * s, const char * sender, int slen, unsigned char * p);
...
__owur unsigned int dtls_raw_hello_verify_request(unsigned char * buf, unsigned char * cookie, unsigned char cookie_len);
...
__owur int tls1_final_finish_mac(SSL * s, const char * str, int slen, unsigned char * p);
__owur int tls1_generate_master_secret(SSL * s, unsigned char * out, unsigned char * p, int len);
__owur int tls1_export_keying_material(SSL * s, unsigned char * out, size_t olen, const char * label,
...
SSL_COMP * ssl3_comp_find(STACK_OF(SSL_COMP) *sk, int n);
	...
	__owur int tls1_set_curves(unsigned char ** pext, size_t * pextlen, int * curves, size_t ncurves);
	__owur int tls1_set_curves_list(unsigned char ** pext, size_t * pextlen, const char * str);
	__owur int tls1_check_ec_tmp_key(SSL * s, unsigned long id);
	__owur EVP_PKEY * ssl_generate_pkey_curve(int id);
#endif // OPENSSL_NO_EC 
__owur int tls1_shared_list(SSL * s, const unsigned char * l1, size_t l1len, const unsigned char * l2, size_t l2len, int nmatch);
__owur unsigned char * ssl_add_clienthello_tlsext(SSL * s, unsigned char * buf, unsigned char * limit, int * al);
__owur unsigned char * ssl_add_serverhello_tlsext(SSL * s, unsigned char * buf, unsigned char * limit, int * al);
...
#endif
__owur int tls_check_serverhello_tlsext_early(SSL * s, const PACKET * ext, const PACKET * session_id, SSL_SESSION ** ret);
__owur int tls12_get_sigandhash(unsigned char * p, const EVP_PKEY * pk, const EVP_MD * md);
...
__owur int ssl_add_serverhello_renegotiate_ext(SSL * s, unsigned char * p, int * len, int maxlen);
__owur int ssl_parse_serverhello_renegotiate_ext(SSL * s, PACKET * pkt, int * al);
__owur int ssl_add_clienthello_renegotiate_ext(SSL * s, unsigned char * p, int * len, int maxlen);
__owur int ssl_parse_clienthello_renegotiate_ext(SSL * s, PACKET * pkt, int * al);
__owur long ssl_get_algorithm2(SSL * s);
__owur size_t tls12_copy_sigalgs(SSL * s, unsigned char * out, const unsigned char * psig, size_t psiglen);
...
__owur int tls12_check_peer_sigalg(const EVP_MD ** pmd, SSL * s, const unsigned char * sig, EVP_PKEY * pkey);
void ssl_set_client_disabled(SSL * s);
__owur int ssl_cipher_disabled(SSL * s, const SSL_CIPHER * c, int op);
__owur int ssl_add_clienthello_use_srtp_ext(SSL * s, unsigned char * p, int * len, int maxlen);
__owur int ssl_parse_clienthello_use_srtp_ext(SSL * s, PACKET * pkt, int * al);
__owur int ssl_add_serverhello_use_srtp_ext(SSL * s, unsigned char * p, int * len, int maxlen);
...
__owur int ssl3_cbc_digest_record(const EVP_MD_CTX * ctx, unsigned char * md_out, size_t * md_out_size,
    const unsigned char header[13], const unsigned char * data, size_t data_plus_mac_size, size_t data_plus_mac_plus_padding_size,
    const unsigned char * mac_secret, unsigned mac_secret_length, char is_sslv3);
__owur int tls_fips_digest_extra(const EVP_CIPHER_CTX * cipher_ctx, EVP_MD_CTX * mac_ctx, const unsigned char * data, size_t data_len, size_t orig_len);
...
__owur int custom_ext_parse(SSL * s, int server, unsigned int ext_type, const unsigned char * ext_data, size_t ext_size, int * al);
__owur int custom_ext_add(SSL * s, int server, unsigned char ** pret, unsigned char * limit, int * al);
