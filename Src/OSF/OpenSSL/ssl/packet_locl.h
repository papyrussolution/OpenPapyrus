...
typedef struct {
	const unsigned char * curr; /* Pointer to where we are currently reading from */
	size_t remaining; /* Number of bytes remaining */
} PACKET;
 ...
 */
static ossl_inline size_t PACKET_remaining(const PACKET * pkt) { return pkt->remaining; }
/*
 ...
 */
static ossl_inline const unsigned char * PACKET_end(const PACKET * pkt) { return pkt->curr + pkt->remaining; }
/*
 * Returns a pointer to the PACKET's current position.
 * For use in non-PACKETized APIs.
 */
static ossl_inline const unsigned char * PACKET_data(const PACKET * pkt) { return pkt->curr; }
/*
...
{
	return (PACKET_remaining(pkt) != num) ? 0 : (CRYPTO_memcmp(pkt->curr, ptr, num) == 0);
}
/*
...
{
	return (PACKET_remaining(pkt) < len) ? 0 : PACKET_buf_init(subpkt, pkt->curr, len);
}
/*
