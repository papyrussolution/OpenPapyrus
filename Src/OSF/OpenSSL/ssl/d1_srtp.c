...
static int FASTCALL ssl_ctx_make_profiles(const char * profiles_string, STACK_OF(SRTP_PROTECTION_PROFILE) ** out)
{
...
int ssl_add_clienthello_use_srtp_ext(SSL * s, uchar * p, int * len, int maxlen)
{
	...
	int ct = sk_SRTP_PROTECTION_PROFILE_num(clnt); /* -1 if clnt == 0 */
	if(p) {
		if(ct == 0) {
...
int ssl_parse_clienthello_use_srtp_ext(SSL * s, PACKET * pkt, int * al)
{
	...
	if(!PACKET_get_net_2(pkt, &ct) || (ct & 1) != 0 || !PACKET_get_sub_packet(pkt, &subpkt, ct)) {
	...
	while(PACKET_remaining(&subpkt)) {
		 ...
		 * current match.
		 * If no profiles have been have been configured then this does nothing.
		 */
...
int ssl_parse_serverhello_use_srtp_ext(SSL * s, PACKET * pkt, int * al)
{
	...
	SRTP_PROTECTION_PROFILE * prof;
	if(!PACKET_get_net_2(pkt, &ct) || ct != 2 || !PACKET_get_net_2(pkt, &id) || !PACKET_get_1(pkt, &mki) || PACKET_remaining(pkt) != 0) {
