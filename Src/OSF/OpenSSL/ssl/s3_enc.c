...
static int ssl3_generate_key_block(SSL * s, uchar * km, int num)
{
	...
	for(i = 0; (int)i < num; i += MD5_DIGEST_LENGTH) {
		...
		for(j = 0; j < k; j++)
			buf[j] = c;
		c++;
		if(!EVP_DigestInit_ex(s1, EVP_sha1(), NULL) || !EVP_DigestUpdate(s1, buf, k) || 
			!EVP_DigestUpdate(s1, s->session->master_key, s->session->master_key_length) || 
			!EVP_DigestUpdate(s1, s->s3->server_random, SSL3_RANDOM_SIZE) || 
			!EVP_DigestUpdate(s1, s->s3->client_random, SSL3_RANDOM_SIZE) || 
			!EVP_DigestFinal_ex(s1, smd, NULL) || !EVP_DigestInit_ex(m5, EVP_md5(), NULL) || 
...
int ssl3_change_cipher_state(SSL * s, int which)
{
	...
	if(s->msg_callback) {
		int wh = which & SSL3_CC_WRITE ? TLS1_RT_CRYPTO_WRITE : TLS1_RT_CRYPTO_READ;
		s->msg_callback(2, s->version, wh | TLS1_RT_CRYPTO_MAC, mac_secret, EVP_MD_size(m), s, s->msg_callback_arg);
		if(c->key_len)
			s->msg_callback(2, s->version, wh | TLS1_RT_CRYPTO_KEY, key, c->key_len, s, s->msg_callback_arg);
		if(k) {
			s->msg_callback(2, s->version, wh | TLS1_RT_CRYPTO_IV, iv, k, s, s->msg_callback_arg);
		}
	}
#endif
...
int ssl3_final_finish_mac(SSL * s, const char * sender, int len, uchar * p)
{
	...
	if((sender != NULL && EVP_DigestUpdate(ctx, sender, len) <= 0) || EVP_MD_CTX_ctrl(ctx, EVP_CTRL_SSL3_MASTER_SECRET,
		s->session->master_key_length, s->session->master_key) <= 0 || EVP_DigestFinal_ex(ctx, p, NULL) <= 0) {
		SSLerr(SSL_F_SSL3_FINAL_FINISH_MAC, ERR_R_INTERNAL_ERROR);
		ret = 0;
	}
	EVP_MD_CTX_free(ctx);
...
int ssl3_generate_master_secret(SSL * s, uchar * out, uchar * p, int len)
{
	...
	if(ret > 0 && s->msg_callback) {
		...
		s->msg_callback(2, s->version, TLS1_RT_CRYPTO_MASTER, tmpout, SSL3_MASTER_SECRET_SIZE, s, s->msg_callback_arg);
	}
#endif
