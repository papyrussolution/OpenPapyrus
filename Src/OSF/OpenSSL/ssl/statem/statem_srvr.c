...
MSG_PROCESS_RETURN tls_process_client_hello(SSL * s, PACKET * pkt)
{
	...
	if(is_v2_record) {
		...
		if(!PACKET_copy_bytes(&challenge, s->s3->client_random + SSL3_RANDOM_SIZE - challenge_len, challenge_len)
...
static int tls_process_cke_rsa(SSL * s, PACKET * pkt, int * al)
{
	...
	rsa_decrypt = (uchar *)OPENSSL_malloc(RSA_size(rsa));
	if(rsa_decrypt == NULL) {
		*al = SSL_AD_INTERNAL_ERROR;
...
static int tls_process_cke_gost(SSL * s, PACKET * pkt, int * al)
{
	...
	if(alg_a & SSL_aGOST12) {
		...
		SETIFZ(pk, s->cert->pkeys[SSL_PKEY_GOST12_256].privatekey);
		SETIFZ(pk, s->cert->pkeys[SSL_PKEY_GOST01].privatekey);
	}
	...
	if(ASN1_get_object((const uchar**)&data, &Tlen, &Ttag, &Tclass, sess_key_len) != V_ASN1_CONSTRUCTED || Ttag != V_ASN1_SEQUENCE || Tclass != V_ASN1_UNIVERSAL) {
		*al = SSL_AD_DECODE_ERROR;
...
MSG_PROCESS_RETURN tls_process_next_proto(SSL * s, PACKET * pkt)
{
	...
	s->next_proto_negotiated_len = (uchar)next_proto_len;
