    ...
    int reassembly)
{
	...
	uchar * buf = NULL;
	uchar * bitmask = NULL;
	frag = (hm_fragment*)OPENSSL_malloc(sizeof(*frag));
	if(frag == NULL)
		return NULL;
	...
	if(reassembly) {
		bitmask = (uchar *)OPENSSL_zalloc(RSMBLY_BITMASK_SIZE(frag_len));
		if(bitmask == NULL) {
			OPENSSL_free(buf);
			OPENSSL_free(frag);
			return NULL;
...
static int dtls_get_reassembled_message(SSL * s, long * len)
{
	uchar wire[DTLS1_HM_HEADER_LENGTH];
...
int dtls1_buffer_message(SSL * s, int is_ccs)
{
	...
	if(is_ccs) {
		...
		OPENSSL_assert(s->d1->w_msg_hdr.msg_len +
		    DTLS1_HM_HEADER_LENGTH == (uint)s->init_num);
	}
