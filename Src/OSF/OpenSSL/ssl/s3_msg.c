...
int ssl3_do_change_cipher_spec(SSL * s)
{
	const char * sender;
	int slen;
	int i = s->server ? SSL3_CHANGE_CIPHER_SERVER_READ : SSL3_CHANGE_CIPHER_CLIENT_READ;
...
int FASTCALL ssl3_send_alert(SSL * s, int level, int desc)
{
	...
	if(level == SSL3_AL_FATAL && s->session)
		SSL_CTX_remove_session(s->session_ctx, s->session);
	s->s3->alert_dispatch = 1;
	s->s3->send_alert[0] = level;
	s->s3->send_alert[1] = desc;
	if(!RECORD_LAYER_write_pending(&s->rlayer)) // data still being written out? 
		return s->method->ssl_dispatch_alert(s);
	else  // else data is still being written out, we will get written some time in the future
		return -1;
}
