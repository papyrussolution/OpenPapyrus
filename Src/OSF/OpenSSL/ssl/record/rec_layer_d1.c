...
int dtls1_buffer_record(SSL * s, record_pqueue * queue, uchar * priority)
{
	...
	memzero(&s->rlayer.rbuf, sizeof(s->rlayer.rbuf));
	memzero(&s->rlayer.rrec, sizeof(s->rlayer.rrec));
...
start:
	...
	if(type == SSL3_RECORD_get_type(rr)
		...
		if((uint)len > SSL3_RECORD_get_length(rr))
	...
	if(s->rlayer.d->alert_fragment_len >= DTLS1_AL_HEADER_LENGTH) {
		...
		if(alert_level == SSL3_AL_WARNING) {
			...
			if(alert_descr == DTLS1_AD_MISSING_HANDSHAKE_MESSAGE) {
				unsigned short seq;
				uint frag_off;
				uchar * p = &(s->rlayer.d->alert_fragment[2]);
    ...
    int len)
{
	if((type == SSL3_RT_HANDSHAKE)
		...
		uchar * src = s->rlayer.d->handshake_fragment;
		uchar * dst = buf;
		uint k, n;
...
int dtls1_write_bytes(SSL * s, int type, const void * buf, int len)
{
	...
	s->rwstate = SSL_NOTHING;
	i = do_dtls1_write(s, type, (const uchar *)buf, len, 0);
	return i;
}

int do_dtls1_write(SSL * s, int type, const uchar * buf,
    uint len, int create_empty_fragment)
{
	uchar * p, * pseq;
...
void dtls1_reset_seq_numbers(SSL * s, int rw)
{
	...
	if(rw & SSL3_CC_READ) {
		...
		memzero(&s->rlayer.d->next_bitmap, sizeof(s->rlayer.d->next_bitmap));
		/*
