...
void RECORD_LAYER_clear(RECORD_LAYER * rl)
{
	 ...
	 * previously get reset by SSL_clear...so I'll keep it that way..but is that right?
	 */
	rl->packet = NULL;
	rl->packet_length = 0;
	rl->wnum = 0;
	memzero(rl->alert_fragment, sizeof(rl->alert_fragment));
...
int RECORD_LAYER_set_data(RECORD_LAYER * rl, const uchar * buf, int len)
{
...
int ssl3_pending(const SSL * s)
{
	...
	if(s->rlayer.rstate == SSL_ST_READ_BODY)
		return 0;
	for(uint i = 0; i < RECORD_LAYER_get_numrpipes(&s->rlayer); i++) {
		if(SSL3_RECORD_get_type(&s->rlayer.rrec[i]) != SSL3_RT_APPLICATION_DATA)
			return 0;
...
int ssl3_read_n(SSL * s, int n, int max, int extend, int clearold)
{
	...
	if(!extend) {
		...
		if(left == 0)
			...
			if(pkt[0] == SSL3_RT_APPLICATION_DATA && (pkt[3] << 8 | pkt[4]) >= 128) {
				 ...
				 * effect on memmove arguments and therefore no buffer overrun can be triggered.
				 */
	...
	// operation returns the whole packet at once (as long as it fits into the buffer).
	// 
	if(SSL_IS_DTLS(s)) {
		if(left == 0 && extend)
			return 0;
	...
	while(left < n) {
		...
		// need to read in more until we have len+n (up to len+max if possible)
		clear_sys_error();
		if(s->rbio != NULL) {
...
int ssl3_write_bytes(SSL * s, int type, const void * buf_, int len)
{
	...
	int tot;
	uint n, split_send_fragment, maxpipes;
#if !defined(OPENSSL_NO_MULTIBLOCK) && EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK
	uint max_send_fragment, nw;
	uint u_len = (uint)len;
#endif
	 ...
	 */
	if(type == SSL3_RT_APPLICATION_DATA && u_len >= 4 * (max_send_fragment = s->max_send_fragment) &&
	    s->compress == NULL && s->msg_callback == NULL && !SSL_WRITE_ETM(s) && SSL_USE_EXPLICIT_IV(s) &&
	    EVP_CIPHER_flags(EVP_CIPHER_CTX_cipher(s->enc_write_ctx)) & EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK) {
		uchar aad[13];
		...
		if(tot == 0 || wb->buf == NULL) { /* allocate jumbo buffer */
			...
			packlen = EVP_CIPHER_CTX_ctrl(s->enc_write_ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_MAX_BUFSIZE, max_send_fragment, 0);
		...
		for(;; ) {
			...
			mb_param.len = nw;
			if(EVP_CIPHER_CTX_ctrl(s->enc_write_ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT, sizeof(mb_param), &mb_param) <= 0)
				return -1;
	 ...
	 * processing then we also only use 1 pipeline, or if we're not using explicit IVs
	 */
	...
	for(;; ) {
		uint pipelens[SSL_MAX_PIPELINES], tmppipelen, remain;
		uint numpipes, j;
		if(n == 0)
			numpipes = 1;
		else
		...
		if((i == (int)n) || (type == SSL3_RT_APPLICATION_DATA && (s->mode & SSL_MODE_ENABLE_PARTIAL_WRITE))) {
			...
			if((i == (int)n) && s->mode & SSL_MODE_RELEASE_BUFFERS && !SSL_IS_DTLS(s))
				ssl3_release_write_buffer(s);
			return tot + i;
		}
		n -= i;
...
int do_ssl3_write(SSL * s, int type, const uchar * buf, uint * pipelens, uint numpipes, int create_empty_fragment)
{
	uchar * outbuf[SSL_MAX_PIPELINES], * plen[SSL_MAX_PIPELINES];
	...
	for(j = 0; j < numpipes; j++) {
		...
		*(outbuf[j]++) = (s->version >> 8);
		 ...
		 */
		if(SSL_get_state(s) == TLS_ST_CW_CLNT_HELLO && !s->renegotiate && TLS1_get_version(s) > TLS1_VERSION)
			*(outbuf[j]++) = 0x1;
		else
		...
		SSL3_RECORD_set_input(&wr[j], (uchar *)&buf[totlen]);
		totlen += pipelens[j];
		/*
		 ...
		 * wr->input.  Length should be wr->length. wr->data still points in the wb->buf
		 */
		if(!SSL_WRITE_ETM(s) && mac_size != 0) {
			if(s->method->ssl3_enc->mac(s, &wr[j], &(outbuf[j][wr[j].length + eivlen]), 1) < 0)
				goto err;
	...
	for(j = 0; j < numpipes; j++) {
		if(SSL_WRITE_ETM(s) && mac_size != 0) {
			if(s->method->ssl3_enc->mac(s, &wr[j], outbuf[j] + wr[j].length, 1) < 0)
				goto err;
			SSL3_RECORD_add_length(&wr[j], mac_size);
		}
		...
		if(s->msg_callback)
			s->msg_callback(1, 0, SSL3_RT_HEADER, plen[j] - 5, 5, s, s->msg_callback_arg);
		/*
...
start:
	...
	if(s->s3->change_cipher_spec /* set when we receive ChangeCipherSpec, reset by ssl3_get_finished */ && (SSL3_RECORD_get_type(rr) != SSL3_RT_HANDSHAKE)) {
	...
	if(type == SSL3_RECORD_get_type(rr) || (SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC && type == SSL3_RT_HANDSHAKE && recvd_type)) {
		...
		if(read_bytes == 0) {
			goto start; // We must have read empty records. Get more data 
		}
	...
	if((!s->server) && (s->rlayer.handshake_fragment_len >= 4) &&
	    (s->rlayer.handshake_fragment[0] == SSL3_MT_HELLO_REQUEST) && s->session && s->session->cipher) {
		...
		if(SSL_is_init_finished(s) && !(s->s3->flags & SSL3_FLAGS_NO_RENEGOTIATE_CIPHERS) && !s->s3->renegotiate) {
			...
			SSL3_RECORD_set_read(rr); // Does this ever happen? 
		}
		/*
	...
	switch(SSL3_RECORD_get_type(rr)) {
		...
		case SSL3_RT_APPLICATION_DATA:
			...
			// application data at this point (session renegotiation not yet started), we will indulge it.
			// 
 ...
 */
uint RECORD_LAYER_get_rrec_length(RECORD_LAYER * rl)
{
