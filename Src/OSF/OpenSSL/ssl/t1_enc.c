...
static int tls1_generate_key_block(SSL * s, uchar * km, int num)
{
	int ret = tls1_PRF(s, TLS_MD_KEY_EXPANSION_CONST,
...
{
	uchar * p, * mac_secret;
	uchar tmp1[EVP_MAX_KEY_LENGTH];
	uchar tmp2[EVP_MAX_KEY_LENGTH];
	uchar iv1[EVP_MAX_IV_LENGTH * 2];
	...
	if(s->msg_callback) {
		...
		if(*mac_secret_size)
			s->msg_callback(2, s->version, wh | TLS1_RT_CRYPTO_MAC, mac_secret, *mac_secret_size, s, s->msg_callback_arg);
		if(c->key_len)
			s->msg_callback(2, s->version, wh | TLS1_RT_CRYPTO_KEY, key, c->key_len, s, s->msg_callback_arg);
		if(k) {
			if(EVP_CIPHER_mode(c) == EVP_CIPH_GCM_MODE)
...
int tls1_final_finish_mac(SSL * s, const char * str, int slen, uchar * out)
{
	int hashlen;
...
int tls1_export_keying_material(SSL * s, uchar * out, size_t olen, const char * label, size_t llen, const uchar * context, size_t contextlen, int use_context)
{
	...
	rv = tls1_PRF(s, val, vallen, NULL, 0, NULL, 0, NULL, 0, NULL, 0,
	    s->session->master_key, s->session->master_key_length, out, olen);
	goto ret;
err1:
...
int tls1_alert_code(int code)
{
	switch(code) {
		...
		case SSL_AD_NO_APPLICATION_PROTOCOL: return (TLS1_AD_NO_APPLICATION_PROTOCOL);
		default:
		    return (-1);
	}
}
