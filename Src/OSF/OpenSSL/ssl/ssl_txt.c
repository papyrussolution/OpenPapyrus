...
int SSL_SESSION_print(BIO * bp, const SSL_SESSION * x)
{
	...
	if(BIO_puts(bp, "\n    Master-Key: ") <= 0)
		goto err;
	for(i = 0; i < (uint)x->master_key_length; i++) {
	...
	if(x->tlsext_tick_lifetime_hint) {
		if(BIO_printf(bp, "\n    TLS session ticket lifetime hint: %ld (seconds)", x->tlsext_tick_lifetime_hint) <= 0)
			goto err;
	}
	if(x->tlsext_tick) {
		if(BIO_puts(bp, "\n    TLS session ticket:\n") <= 0)
			goto err;
		if(BIO_dump_indent(bp, (const char *)x->tlsext_tick, x->tlsext_ticklen, 4) <= 0)
			goto err;
	}
