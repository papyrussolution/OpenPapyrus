...
void des_t4_cbc_encrypt(const void * inp, void * out, size_t len, const DES_key_schedule * ks, uchar iv[8]);
void des_t4_cbc_decrypt(const void * inp, void * out, size_t len, const DES_key_schedule * ks, uchar iv[8]);
# endif
...
static int des_cfb64_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	while(inl >= EVP_MAXCHUNK) {
		int num = EVP_CIPHER_CTX_num(ctx);
 ...
 * way, so wrap it here
 */
static int des_cfb1_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
