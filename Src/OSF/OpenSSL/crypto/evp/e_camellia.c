...
static int camellia_init_key(EVP_CIPHER_CTX * ctx, const uchar * key, const uchar * iv, int enc)
{
	...
	int ret = Camellia_set_key(key, EVP_CIPHER_CTX_key_length(ctx) * 8, &dat->ks);
	if(ret < 0) {
...
static int camellia_cbc_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_CAMELLIA_KEY * dat = EVP_C_DATA(EVP_CAMELLIA_KEY, ctx);
	if(dat->stream.cbc)
...
static int camellia_ecb_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
...
static int camellia_ofb_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_CAMELLIA_KEY * dat = EVP_C_DATA(EVP_CAMELLIA_KEY, ctx);
...
static int camellia_cfb_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_CAMELLIA_KEY * dat = EVP_C_DATA(EVP_CAMELLIA_KEY, ctx);
	int num = EVP_CIPHER_CTX_num(ctx);
...
static int camellia_cfb8_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_CAMELLIA_KEY * dat = EVP_C_DATA(EVP_CAMELLIA_KEY, ctx);
	int num = EVP_CIPHER_CTX_num(ctx);
...
static int camellia_ctr_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
