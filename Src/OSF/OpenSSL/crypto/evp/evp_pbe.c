...
int EVP_PBE_CipherInit(ASN1_OBJECT * pbe_obj, const char * pass, int passlen, ASN1_TYPE * param, EVP_CIPHER_CTX * ctx, int en_de)
{
	...
	EVP_PBE_KEYGEN * keygen;
	if(!EVP_PBE_find(EVP_PBE_TYPE_OUTER, OBJ_obj2nid(pbe_obj), &cipher_nid, &md_nid, &keygen)) {
		char obj_tmp[80];
...
static int pbe2_cmp(const EVP_PBE_CTL * pbe1, const EVP_PBE_CTL * pbe2)
{
	...
	return ret ? ret : (pbe1->pbe_nid - pbe2->pbe_nid);
}
...
static int pbe_cmp(const EVP_PBE_CTL * const * a, const EVP_PBE_CTL * const * b)
{
	...
	return ret ? ret : ((*a)->pbe_nid - (*b)->pbe_nid);
}
...
{
	int cipher_nid = cipher ? EVP_CIPHER_nid(cipher) : -1;
	int md_nid = md ? EVP_MD_type(md) : -1;
...
int EVP_PBE_find(int type, int pbe_nid, int * pcnid, int * pmnid, EVP_PBE_KEYGEN ** pkeygen)
{
	...
	if(pbetmp == NULL)
		return 0;
	ASSIGN_PTR(pcnid, pbetmp->cipher_nid);
	ASSIGN_PTR(pmnid, pbetmp->md_nid);
	ASSIGN_PTR(pkeygen, pbetmp->keygen);
	return 1;
}
...
int EVP_PBE_get(int * ptype, int * ppbe_nid, size_t num)
{
	if(num >= OSSL_NELEM(builtin_pbe))
		...
		const EVP_PBE_CTL * tpbe = builtin_pbe + num;
		ASSIGN_PTR(ptype, tpbe->pbe_type);
		ASSIGN_PTR(ppbe_nid, tpbe->pbe_nid);
		return 1;
	}
}
