...
{
	return (pkey->ameth == &dhx_asn1_meth) ? d2i_DHxparams(NULL, pp, length) : d2i_DHparams(NULL, pp, length);
}

static int i2d_dhp(const EVP_PKEY * pkey, const DH * a, uchar ** pp)
{
	return (pkey->ameth == &dhx_asn1_meth) ? i2d_DHxparams(a, pp) : i2d_DHparams(a, pp);
}
...
static int dh_priv_decode(EVP_PKEY * pkey, const PKCS8_PRIV_KEY_INFO * p8)
{
	...
	if((dh->priv_key = BN_secure_new()) == NULL || !ASN1_INTEGER_to_BN(privkey, dh->priv_key)) {
...
static int dh_param_decode(EVP_PKEY * pkey, const uchar ** pder, int derlen)
{
	DH * dh;
...
static int do_dh_print(BIO * bp, const DH * x, int indent, int ptype)
{
	...
	BIGNUM * priv_key = (ptype == 2) ? x->priv_key : NULL;
	BIGNUM * pub_key = (ptype > 0) ? x->pub_key : NULL;
	if(x->p == NULL || (ptype == 2 && priv_key == NULL) || (ptype > 0 && pub_key == NULL)) {
	...
	if(x->seed) {
		...
		for(i = 0; i < x->seedlen; i++) {
			...
			if(BIO_printf(bp, "%02x%s", x->seed[i], ((i + 1) == x->seedlen) ? "" : ":") <= 0)
				goto err;
		}
		if(BIO_write(bp, "\n", 1) <= 0)
			return 0;
...
{
	return (a->pkey.dh == NULL || a->pkey.dh->p == NULL || a->pkey.dh->g == NULL) ? 1 : 0;
}
...
static int dh_cms_set_shared_info(EVP_PKEY_CTX * pctx, CMS_RecipientInfo * ri)
{
	...
	if(EVP_PKEY_CTX_set0_dh_kdf_oid(pctx, OBJ_nid2obj(EVP_CIPHER_type(kekcipher))) <= 0)
		goto err;
	if(ukm) {
