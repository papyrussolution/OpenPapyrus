...
static X509 * ocsp_find_signer_sk(STACK_OF(X509) * certs, OCSP_RESPID * id)
{
	...
	for(i = 0; i < sk_X509_num(certs); i++) {
		...
		X509_pubkey_digest(x, EVP_sha1(), tmphash, 0);
    ...
    STACK_OF(OCSP_SINGLERESP) * sresp)
{
	...
	if(cid) {
		...
		if(memcmp(md, cid->issuerNameHash.data, mdlen))
			return 0;
		X509_pubkey_digest(cert, dgst, md, 0);
		...
		for(i = 0; i < sk_OCSP_SINGLERESP_num(sresp); i++) {
			tmpid = sk_OCSP_SINGLERESP_value(sresp, i)->certId;
			ret = ocsp_match_issuerid(cert, tmpid, 0);
			if(ret <= 0)
				return ret;
		}
		return 1;
 ...
 * verify. Just find the signers certificate and verify it against a given trust value.
 */
int OCSP_request_verify(OCSP_REQUEST * req, STACK_OF(X509) * certs, X509_STORE * store, ulong flags)
{
	X509 * signer;
	X509_NAME * nm;
	GENERAL_NAME * gen;
	...
	if(!(flags & OCSP_NOVERIFY)) {
		int init_res;
		if(flags & OCSP_NOCHAIN)
			init_res = X509_STORE_CTX_init(ctx, store, signer, 0);
		else
