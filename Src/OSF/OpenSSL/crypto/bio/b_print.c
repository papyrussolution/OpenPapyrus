		...
		switch(state) {
			...
			case DP_S_CONV:
			    switch(ch) {
					...
					switch(cflags) {
						case DP_C_SHORT:
						    value = (unsigned short int)va_arg(args, uint);
						    break;
						case DP_C_LONG:
						    value = (LLONG)va_arg(args, ulong);
						    break;
						case DP_C_LLONG:
						    value = va_arg(args, unsigned LLONG);
						    break;
						default:
						    value = (LLONG)va_arg(args, uint);
						    break;
					}
...
static int fmtint(char ** sbuffer, char ** buffer, size_t * currlen, size_t * maxlen, LLONG value, int base, int min, int max, int flags)
{
	...
	if(flags & DP_F_UP)
		caps = 1;
	do {
		convert[place++] = (caps ? "0123456789ABCDEF" : "0123456789abcdef")[uvalue % (uint)base];
...
static int fmtfp(char ** sbuffer, char ** buffer, size_t * currlen, size_t * maxlen, LDOUBLE fvalue, int min, int max, int flags, int style)
{
	...
	if(ufvalue > ULONG_MAX) { // Number too big 
		return 0; 
	}
