    ...
    BN_CTX * ctx)
{
	...
	union {
		...
		uint ui[BN_NIST_192_TOP * sizeof(BN_ULONG) /
		    sizeof(uint)];
	} buf;
...
# endif
	...
	if(carry > 0) {
		...
		mask = 0 - (PTR_SIZE_INT)carry;
		u.p = ((PTR_SIZE_INT)bn_sub_words & mask) | ((PTR_SIZE_INT)bn_add_words & ~mask);
	}
	else
		carry = 1;
...
int BN_nist_mod_256(BIGNUM * r, const BIGNUM * a, const BIGNUM * field, BN_CTX * ctx)
{
	...
	union {
		...
		uint ui[BN_NIST_256_TOP * sizeof(BN_ULONG) / sizeof(uint)];
	} buf;
    ...
    BN_CTX * ctx)
{
	...
	union {
		...
		uint ui[BN_NIST_384_TOP * sizeof(BN_ULONG) /
		    sizeof(uint)];
	} buf;
