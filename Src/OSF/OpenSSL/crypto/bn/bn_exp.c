...
# ifdef MONT_EXP_WORD
		if(a->top == 1 && !a->neg && (BN_get_flags(p, BN_FLG_CONSTTIME) == 0)) {
			BN_ULONG A = a->d[0];
			ret = BN_mod_exp_mont_word(r, A, p, m, ctx, 0);
		}
		else
# endif
		ret = BN_mod_exp_mont(r, a, p, m, ctx, 0);
	}
	else
#endif
#ifdef RECP_MUL_MOD
...
static int MOD_EXP_CTIME_COPY_TO_PREBUF(const BIGNUM * b, int top, uchar * buf, int idx, int window)
{
	int i, j;
	int width = 1 << window;
...
static int MOD_EXP_CTIME_COPY_FROM_PREBUF(BIGNUM * b, int top, uchar * buf, int idx, int window)
{
	int i, j;
	int width = 1 << window;
	/*
		...
		if(window > 1) {
			...
			for(i = 3; i < numPowers; i++) {
				...
				if(!BN_mod_mul_montgomery(&tmp, &am, &tmp, mont, ctx))
					goto err;
				if(!MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, i, window))
					goto err;
			}
		}
		bits--;
...
#define BN_MOD_MUL_WORD(r, w, m) \
	 ...
	 * never negative (the result of BN_mod does not depend on the sign of the modulus).
	 */
