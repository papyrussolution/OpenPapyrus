...
#  endif
	 ...
	 */
	OPENSSL_ia32cap_P[0] = (uint)vec | (1 << 10);
...
int OPENSSL_isservice(void)
{
	...
	if(_OPENSSL_isservice.p == NULL) {
		...
		if(_OPENSSL_isservice.p == NULL)
			_OPENSSL_isservice.p = reinterpret_cast<void *>(-1);
	}
	if(_OPENSSL_isservice.p != reinterpret_cast<void *>(-1))
...
int FASTCALL CRYPTO_memcmp(const volatile void * volatile in_a, const volatile void * volatile in_b, size_t len)
{
	...
	const volatile uchar * b = (const volatile uchar *)in_b;
	uchar x = 0;
	for(i = 0; i < len; i++)
		x |= a[i] ^ b[i];
	return x;
