...
static BIGNUM * srp_Calc_xy(const BIGNUM * x, const BIGNUM * y, const BIGNUM * N)
{
	...
	if(y != N && BN_ucmp(y, N) >= 0)
		return NULL;
	if((tmp = (uchar *)OPENSSL_malloc(numN * 2)) == NULL)
		goto err;
...
BIGNUM * SRP_Calc_B(const BIGNUM * b, const BIGNUM * N, const BIGNUM * g, const BIGNUM * v)
{
	...
	if((kv = BN_new()) == NULL || (gb = BN_new()) == NULL || (B = BN_new()) == NULL)
		goto err;
	/* B = g**b + k*v */
	if(!BN_mod_exp(gb, g, b, N, bn_ctx) || (k = srp_Calc_k(N, g)) == NULL || !BN_mod_mul(kv, v, k, N, bn_ctx) || !BN_mod_add(B, gb, kv, N, bn_ctx)) {
		BN_free(B);
		B = NULL;
	}
err:
...
BIGNUM * SRP_Calc_x(const BIGNUM * s, const char * user, const char * pass)
{
	...
	if(ctxt == NULL)
		return NULL;
	if((cs = (uchar *)OPENSSL_malloc(BN_num_bytes(s))) == NULL)
		goto err;
