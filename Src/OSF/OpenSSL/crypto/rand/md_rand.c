...
static uchar state[STATE_SIZE + MD_DIGEST_LENGTH];
static uchar md[MD_DIGEST_LENGTH];
...
static uint crypto_lock_rand = 0;
...
static int rand_bytes(uchar * buf, int num, int pseudo);
static int rand_nopseudo_bytes(uchar * buf, int num);
#if OPENSSL_API_COMPAT < 0x10100000L
static int rand_pseudo_bytes(uchar * buf, int num);
#endif
...
static int rand_add(const void * buf, int num, double add)
{
	int i, j, k, st_idx;
	long md_c[2];
	uchar local_md[MD_DIGEST_LENGTH];
	EVP_MD_CTX * m;
	int do_not_lock;
	int rv = 0;
...
void rand_hw_xor(uchar * buf, size_t num)
{
	size_t rnd;
	if(!(OPENSSL_ia32cap_P[1] & (1 << (62 - 32))))
		return;
