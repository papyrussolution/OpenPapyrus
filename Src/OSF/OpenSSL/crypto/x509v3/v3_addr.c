...
{
	return ((f && f->addressFamily && f->addressFamily->data) ? ((f->addressFamily->data[0] << 8) | (f->addressFamily->data[1])) : 0);
}
/*
...
{
	for(int i = 0; i < sk_IPAddressOrRange_num(aors); i++) {
...
static int i2r_IPAddrBlocks(const X509V3_EXT_METHOD * method, void * ext, BIO * out, int indent)
{
	...
	for(i = 0; i < sk_IPAddressFamily_num(addr); i++) {
		...
		switch(afi) {
			case IANA_AFI_IPV4: BIO_printf(out, "%*sIPv4", indent, ""); break;
			case IANA_AFI_IPV6: BIO_printf(out, "%*sIPv6", indent, ""); break;
			default: BIO_printf(out, "%*sUnknown AFI %u", indent, "", afi); break;
		}
		if(f->addressFamily->length > 2) {
			switch(f->addressFamily->data[2]) {
				case 1: BIO_puts(out, " (Unicast)"); break;
				case 2: BIO_puts(out, " (Multicast)"); break;
				case 3: BIO_puts(out, " (Unicast/Multicast)"); break;
				case 4: BIO_puts(out, " (MPLS)"); break;
				case 64: BIO_puts(out, " (Tunnel)"); break;
 ...
 */
static int range_should_be_prefix(const uchar * min, const uchar * max, const int length)
{
	uchar mask;
	int i, j;
 ...
 * Construct a prefix.
 */
static int make_addressPrefix(IPAddressOrRange ** result, uchar * addr, const int prefixlen)
{
 ...
 */
static int make_addressRange(IPAddressOrRange ** result, uchar * min, uchar * max, const int length)
{
 ...
 * Add a range.
 */
int X509v3_addr_add_range(IPAddrBlocks * addr, const unsigned afi, const unsigned * safi, uchar * min, uchar * max)
{
	IPAddressOrRanges * aors = make_prefix_or_range(addr, afi, safi);
	IPAddressOrRange * aor;
 ...
 */
static int extract_min_max(IPAddressOrRange * aor, uchar * min, uchar * max, int length)
{
	if(aor && min && max) {
		switch(aor->type) {
			case IPAddressOrRange_addressPrefix:
 ...
 */
int X509v3_addr_get_range(IPAddressOrRange * aor, const unsigned afi, uchar * min, uchar * max, const int length)
{
	int afi_length = length_from_afi(afi);
	if(!aor || !min || !max || !afi_length || length < afi_length || 
...
static void * v2i_IPAddrBlocks(const struct v3_ext_method * method, struct v3_ext_ctx * ctx, STACK_OF(CONF_VALUE) * values)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(values); i++) {
		...
		switch(afi) {
			case IANA_AFI_IPV4: addr_chars = v4addr_chars; break;
			case IANA_AFI_IPV6: addr_chars = v6addr_chars; break;
		}
...
int X509v3_addr_subset(IPAddrBlocks * a, IPAddrBlocks * b)
{
	...
	for(i = 0; i < sk_IPAddressFamily_num(a); i++) {
		...
		if(!fb || !addr_contains(fb->ipAddressChoice->u.addressesOrRanges, fa->ipAddressChoice->u.addressesOrRanges, length_from_afi(X509v3_addr_get_afi(fb))))
			return 0;
	}
	return 1;
}
...
int X509v3_addr_validate_resource_set(STACK_OF(X509) * chain, IPAddrBlocks * ext, int allow_inheritance)
{
	if(ext == NULL)
		return 1;
	else if(chain == NULL || sk_X509_num(chain) == 0)
		return 0;
	else if(!allow_inheritance && X509v3_addr_inherits(ext))
		return 0;
	else
