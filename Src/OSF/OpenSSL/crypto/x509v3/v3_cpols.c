...
static STACK_OF(POLICYINFO) *r2i_certpol(X509V3_EXT_METHOD *method, X509V3_CTX *ctx, const char * value);
...
} ASN1_ADB_END(POLICYQUALINFO, 0, pqualid, 0, &policydefault_tt, 0);
    ...
    STACK_OF(CONF_VALUE) * polstrs, int ia5org)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(polstrs); i++) {
		...
		if(sstreq(cnf->name, "policyIdentifier")) {
			ASN1_OBJECT * pobj;
    ...
    STACK_OF(CONF_VALUE) * unot, int ia5org)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(unot); i++) {
		...
		else if(sstreq(cnf->name, "noticeNumbers")) {
			NOTICEREF * nref;
