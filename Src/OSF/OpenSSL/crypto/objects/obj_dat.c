...
{
	int i = ca->type - cb->type;
	if(i)
		return (i);
	else {
		ASN1_OBJECT * a = ca->obj;
		ASN1_OBJECT * b = cb->obj;
		switch(ca->type) {
			case ADDED_DATA:
				i = (a->length - b->length);
				if(i)
		...
		if(*s != '\0') {
			*(s++) = '\0';
			...
			if(*s == '\0')
				...
				l = s;
				while((*l != '\0') && !isspace((uchar)*l))
					l++;
				if(*l != '\0') {
					*(l++) = '\0';
					while(isspace((uchar)*l))
						l++;
					if(*l == '\0')
						l = NULL;
				}
