...
static int old_rsa_priv_decode(EVP_PKEY * pkey, const uchar ** pder, int derlen)
{
	RSA * rsa;
...
{
	uchar * rk = NULL;
	int rklen = i2d_RSAPrivateKey(pkey->pkey.rsa, &rk);
	if(rklen <= 0) {
...
static RSA_PSS_PARAMS * rsa_pss_decode(const X509_ALGOR * alg, X509_ALGOR ** pmaskHash)
{
	RSA_PSS_PARAMS * pss;
	*pmaskHash = NULL;
	pss = (RSA_PSS_PARAMS*)ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(RSA_PSS_PARAMS), alg->parameter);
	if(!pss)
		return NULL;
    ...
    X509_ALGOR ** pmaskHash)
{
	RSA_OAEP_PARAMS * pss;
	*pmaskHash = NULL;
	pss = (RSA_OAEP_PARAMS*)ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(RSA_OAEP_PARAMS), alg->parameter);
	if(!pss)
		return NULL;
