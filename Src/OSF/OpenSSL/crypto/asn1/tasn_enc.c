...
static int asn1_template_ex_i2d(ASN1_VALUE ** pval, uchar ** out, const ASN1_TEMPLATE * tt, int tag, int iclass)
{
	...
	ndef = ((flags & ASN1_TFLG_NDEF) && (iclass & ASN1_TFLG_NDEF)) ? 2 : 1;
	if(flags & ASN1_TFLG_SK_MASK) {
		...
		if((ttag != -1) && !(flags & ASN1_TFLG_EXPTAG)) {
			...
			sktag = isset ? V_ASN1_SET : V_ASN1_SEQUENCE;
		}
...
static int asn1_set_seq_out(STACK_OF(ASN1_VALUE) * sk, uchar ** out, int skcontlen, const ASN1_ITEM * item, int do_sort, int iclass)
{
	...
	if(do_sort) {
		...
		if(sk_ASN1_VALUE_num(sk) < 2)
			...
			derlst = (DER_ENC*)OPENSSL_malloc(sk_ASN1_VALUE_num(sk) * sizeof(*derlst));
			if(derlst == NULL)
				return 0;
			tmpdat = (uchar *)OPENSSL_malloc(skcontlen);
			if(tmpdat == NULL) {
				OPENSSL_free(derlst);
				return 0;
			}
