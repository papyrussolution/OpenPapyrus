...
static int zlib_stateful_expand_block(COMP_CTX * ctx, uchar * out, uint olen, uchar * in, uint ilen)
{
	...
	return (err != Z_OK) ? -1 : (olen - state->istream.avail_out);
}
...
# endif
	if(!zlib_loaded) {
		...
		if(zlib_dso != NULL) {
			...
			if(p_compress && p_inflateEnd && p_inflate && p_inflateInit_ && p_deflateEnd && p_deflate && p_deflateInit_ && p_zError)
				zlib_loaded++;
...
static int bio_zlib_flush(BIO * b)
{
	...
	z_stream * zout;
	BIO * next = BIO_next(b);
	BIO_ZLIB_CTX * ctx = (BIO_ZLIB_CTX *)BIO_get_data(b);
...
{
	BIO * next = BIO_next(b);
	return next ? BIO_callback_ctrl(next, cmd, fp) : 0;
}
