    ...
    const uchar * p, int plen, ecx_key_op_t op)
{
	...
	xkey = (X25519_KEY *)OPENSSL_zalloc(sizeof(*xkey));
	if(xkey == NULL) {
...
static int ecx_pub_encode(X509_PUBKEY * pk, const EVP_PKEY * pkey)
{
	...
	if(!X509_PUBKEY_set0_param(pk, OBJ_nid2obj(NID_X25519), V_ASN1_UNDEF, NULL, penc, X25519_KEYLEN)) {
		OPENSSL_free(penc);
		ECerr(EC_F_ECX_PUB_ENCODE, ERR_R_MALLOC_FAILURE);
		return 0;
	}
...
{
	const X25519_KEY * akey = (const X25519_KEY *)a->pkey.ptr;
	const X25519_KEY * bkey = (const X25519_KEY *)b->pkey.ptr;
...
{
	X25519_KEY * xkey = (X25519_KEY *)pkey->pkey.ptr;
	if(xkey)
		OPENSSL_secure_free(xkey->privkey);
	OPENSSL_free(xkey);
}
...
static int ecx_key_print(BIO * bp, const EVP_PKEY * pkey, int indent, ASN1_PCTX * ctx, ecx_key_op_t op)
{
	const X25519_KEY * xkey = (const X25519_KEY *)pkey->pkey.ptr;
...
static int pkey_ecx_derive(EVP_PKEY_CTX * ctx, uchar * key, size_t * keylen)
{
	...
	pkey = (const X25519_KEY *)ctx->pkey->pkey.ptr;
	peerkey = (const X25519_KEY *)ctx->peerkey->pkey.ptr;
