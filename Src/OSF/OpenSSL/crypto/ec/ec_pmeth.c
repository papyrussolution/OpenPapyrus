...
{
	EC_PKEY_CTX * dctx = (EC_PKEY_CTX*)OPENSSL_zalloc(sizeof(*dctx));
	if(dctx == NULL)
		return 0;
...
static int pkey_ec_sign(EVP_PKEY_CTX * ctx, uchar * sig, size_t * siglen, const uchar * tbs, size_t tbslen)
{
	int ret, type;
	uint sltmp;
	EC_PKEY_CTX * dctx = (EC_PKEY_CTX*)ctx->data;
...
static int pkey_ec_ctrl_str(EVP_PKEY_CTX * ctx, const char * type, const char * value)
{
	...
	else if(sstreq(type, "ecdh_kdf_md")) {
		const EVP_MD * md;
	...
	else if(sstreq(type, "ecdh_cofactor_mode")) {
		int co_mode;
		co_mode = atoi(value);
