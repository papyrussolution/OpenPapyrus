...
void EC_ec_pre_comp_free(EC_PRE_COMP * pre)
{
	if(pre) {
		...
		REF_ASSERT_ISNT(i < 0);
		if(pre->points) {
			for(EC_POINT ** pts = pre->points; *pts != NULL; pts++)
				EC_POINT_free(*pts);
...
#define EC_window_bits_for_scalar_size(b) \
	((size_t)((b) >= 2000 ? 6 : (b) >=  800 ? 5 : (b) >=  300 ? 4 : (b) >= 70 ? 3 : (b) >= 20 ? 2 : 1))
    ...
    size_t num, const EC_POINT * points[], const BIGNUM * scalars[], BN_CTX * ctx)
{
	...
	int num_scalar = 0; // flag: will be set to 1 if 'scalar' must be treated like other scalars, i.e. precomputation is not available 
	int ret = 0;
	...
	if(scalar) {
		...
		if(pre_comp && pre_comp->numblocks && (EC_POINT_cmp(group, generator, pre_comp->points[0], ctx) == 0)) {
			...
			SETMIN(numblocks, pre_comp->numblocks);
...
err:
	...
	if(points) {
		for(EC_POINT ** p = points; *p != NULL; p++)
			EC_POINT_free(*p);
		OPENSSL_free(points);
	}
