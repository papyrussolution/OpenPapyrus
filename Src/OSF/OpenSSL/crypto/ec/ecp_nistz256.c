    ...
    size_t num, BN_CTX * ctx)
{
	...
	if((num * 16 + 6) > OPENSSL_MALLOC_MAX_NELEMS(P256_POINT)
	    ...
	    || (p_str = (uchar (*)[33])OPENSSL_malloc(num * 33 * sizeof(uchar))) == NULL
	    || (scalars = (const BIGNUM **)OPENSSL_malloc(num * sizeof(BIGNUM *))) == NULL) {
		ECerr(EC_F_ECP_NISTZ256_WINDOWED_MUL, ERR_R_MALLOC_FAILURE);
		goto err;
	}

	table = (P256_POINT(*)[16])ALIGNPTR(table_storage, 64);
...
__owur static int ecp_nistz256_mult_precompute(EC_GROUP * group, BN_CTX * ctx)
{
	...
	if((precomp_storage = (uchar *)OPENSSL_malloc(37 * 64 * sizeof(P256_POINT_AFFINE) + 64)) == NULL) {
		ECerr(EC_F_ECP_NISTZ256_MULT_PRECOMPUTE, ERR_R_MALLOC_FAILURE);
		goto err;
	}
	preComputedTable = (PRECOMP256_ROW *)ALIGNPTR(precomp_storage, 64);
    ...
    const BIGNUM * scalars[], BN_CTX * ctx)
{
	...
	if(no_precomp_for_generator) {
		 ...
		 */
		new_scalars = (const BIGNUM **)OPENSSL_malloc((num + 1) * sizeof(BIGNUM *));
		if(new_scalars == NULL) {
			ECerr(EC_F_ECP_NISTZ256_POINTS_MUL, ERR_R_MALLOC_FAILURE);
			goto err;
		}

		new_points = (const EC_POINT **)OPENSSL_malloc((num + 1) * sizeof(EC_POINT *));
		if(new_points == NULL) {
			ECerr(EC_F_ECP_NISTZ256_POINTS_MUL, ERR_R_MALLOC_FAILURE);
			goto err;
		}
