...
{
	return cms_signed_data_init(cms) ? 1 : 0;
}
...
static int cms_copy_messageDigest(CMS_ContentInfo * cms, CMS_SignerInfo * si)
{
	...
	for(i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
		...
		if(CMS_signed_get_attr_count(sitmp) < 0)
			continue;
		if(OBJ_cmp(si->digestAlgorithm->algorithm, sitmp->digestAlgorithm->algorithm))
			continue;
		messageDigest = (ASN1_OCTET_STRING*)CMS_signed_get0_data_by_OBJ(sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
		if(!messageDigest) {
			CMSerr(CMS_F_CMS_COPY_MESSAGEDIGEST, CMS_R_ERROR_READING_MESSAGEDIGEST_ATTRIBUTE);
			return 0;
		}
		if(CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, messageDigest, -1))
			return 1;
		else
			return 0;
	}
...
CMS_SignerInfo * CMS_add1_signer(CMS_ContentInfo * cms, X509 * signer, EVP_PKEY * pk, const EVP_MD * md, uint flags)
{
	...
	if(!X509_check_private_key(signer, pk)) {
		CMSerr(CMS_F_CMS_ADD1_SIGNER, CMS_R_PRIVATE_KEY_DOES_NOT_MATCH_CERTIFICATE);
		return NULL;
	}
	...
	if(flags & CMS_KEY_PARAM) {
		if(flags & CMS_NOATTR) {
			si->pctx = EVP_PKEY_CTX_new(si->pkey, 0);
			if(si->pctx == NULL)
				goto err;
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo * si, BIO * chain)
{
	...
	if(CMS_signed_get_attr_count(si) >= 0) {
		os = (ASN1_OCTET_STRING*)CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
		if(!os) {
			CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT, CMS_R_ERROR_READING_MESSAGEDIGEST_ATTRIBUTE);
			goto err;
		}
	}
	if(!cms_DigestAlgorithm_find_ctx(mctx, chain, si->digestAlgorithm))
	...
	if(EVP_DigestFinal_ex(mctx, mval, &mlen) <= 0) {
		CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT, CMS_R_UNABLE_TO_FINALIZE_CONTEXT);
		goto err;
	}
	/* If messageDigest found compare it */
	if(os) {
		if(mlen != (uint)os->length) {
			CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT, CMS_R_MESSAGEDIGEST_ATTRIBUTE_WRONG_LENGTH);
			goto err;
		}
		if(memcmp(mval, os->data, mlen)) {
			CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT, CMS_R_VERIFICATION_FAILURE);
			r = 0;
		}
		else
			r = 1;
		...
		pkctx = EVP_PKEY_CTX_new(si->pkey, 0);
		if(pkctx == NULL)
			goto err;
		...
		if(!cms_sd_asn1_ctrl(si, 1))
			goto err;
		r = EVP_PKEY_verify(pkctx, si->signature->data, si->signature->length, mval, mlen);
		if(r <= 0) {
			CMSerr(CMS_F_CMS_SIGNERINFO_VERIFY_CONTENT, CMS_R_VERIFICATION_FAILURE);
			r = 0;
		}
...
int CMS_add_smimecap(CMS_SignerInfo * si, STACK_OF(X509_ALGOR) * algs)
{
	int    r = 0;
	uchar * smder = NULL;
	int    smderlen = i2d_X509_ALGORS(algs, &smder);
	if(smderlen > 0) {
		r = CMS_signed_add1_attr_by_NID(si, NID_SMIMECapabilities, V_ASN1_SEQUENCE, smder, smderlen);
		OPENSSL_free(smder);
	}
	return r;
}
