...
void PKCS12_get0_mac(const ASN1_OCTET_STRING ** pmac, const X509_ALGOR ** pmacalg, const ASN1_OCTET_STRING ** psalt, const ASN1_INTEGER ** piter, const PKCS12 * p12)
{
	if(p12->mac) {
		...
		ASSIGN_PTR(psalt, p12->mac->salt);
		ASSIGN_PTR(piter, p12->mac->iter);
	}
	else {
		ASSIGN_PTR(pmac, 0);
		ASSIGN_PTR(pmacalg, 0);
		ASSIGN_PTR(psalt, 0);
		ASSIGN_PTR(piter, 0);
	}
}
...
static int pkcs12_gen_gost_mac_key(const char * pass, int passlen, const uchar * salt, int saltlen, int iter, int keylen, uchar * key, const EVP_MD * digest)
{
	uchar out[96];
	if(keylen != TK26_MAC_KEY_LEN) {
		return 0;
	}
	if(!PKCS5_PBKDF2_HMAC(pass, passlen, salt, saltlen, iter, digest, sizeof(out), out)) {
		return 0;
	}
    ...
    int (* pkcs12_key_gen)(const char * pass, int passlen, uchar * salt, int slen, int id, int iter, int n, uchar * out, const EVP_MD * md_type))
{
	...
	HMAC_CTX * hmac = NULL;
	uchar key[EVP_MAX_MD_SIZE], * salt;
	int saltlen, iter;
	int md_size = 0;
	int md_type_nid;
	...
	X509_SIG_get0(p12->mac->dinfo, &macalg, 0);
...
int PKCS12_gen_mac(PKCS12 * p12, const char * pass, int passlen, uchar * mac, uint * maclen)
{
	return pkcs12_gen_mac(p12, pass, passlen, mac, maclen, 0);
}
...
int PKCS12_setup_mac(PKCS12 * p12, int iter, uchar * salt, int saltlen, const EVP_MD * md_type)
{
	X509_ALGOR * macalg;
	...
	SETIFZ(saltlen, PKCS12_SALT_LEN);
	if((p12->mac->salt->data = (uchar *)OPENSSL_malloc(saltlen)) == NULL) {
		PKCS12err(PKCS12_F_PKCS12_SETUP_MAC, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	...
	X509_SIG_getm(p12->mac->dinfo, &macalg, 0);
