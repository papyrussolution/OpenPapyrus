...
X509_REQ * X509_to_X509_REQ(X509 * x, EVP_PKEY * pkey, const EVP_MD * md)
{
	...
	ri = &ret->req_info;
	ri->version->length = 1;
	ri->version->data = (uchar*)OPENSSL_malloc(1);
...
{
	return req ? X509_PUBKEY_get(req->req_info.pubkey) : 0;
}

EVP_PKEY * X509_REQ_get0_pubkey(X509_REQ * req)
{
	return req ? X509_PUBKEY_get0(req->req_info.pubkey) : 0;
}
...
{
	int ok = 0;
	EVP_PKEY * xk = X509_REQ_get_pubkey(x);
...
{
	return X509at_add1_attr(&req->req_info.attributes, attr) ?  1 : 0;
}

int X509_REQ_add1_attr_by_OBJ(X509_REQ * req, const ASN1_OBJECT * obj, int type, const uchar * bytes, int len)
{
	return X509at_add1_attr_by_OBJ(&req->req_info.attributes, obj, type, bytes, len) ? 1 : 0;
}

int X509_REQ_add1_attr_by_NID(X509_REQ * req, int nid, int type, const uchar * bytes, int len)
{
	return X509at_add1_attr_by_NID(&req->req_info.attributes, nid, type, bytes, len) ? 1 : 0;
}

int X509_REQ_add1_attr_by_txt(X509_REQ * req, const char * attrname, int type, const uchar * bytes, int len)
{
	return X509at_add1_attr_by_txt(&req->req_info.attributes, attrname, type, bytes, len) ? 1 : 0;
}
