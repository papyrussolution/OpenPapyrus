...
static int get_cert_by_subject(X509_LOOKUP * xl, X509_LOOKUP_TYPE type, X509_NAME * name, X509_OBJECT * ret)
{
	BY_DIR * ctx;
	union {
		X509 st_x509;
	...
	for(i = 0; i < sk_BY_DIR_ENTRY_num(ctx->dirs); i++) {
		...
		for(;; ) {
			...
			if(c == '\0') {
				 ...
				 */
				BIO_snprintf(b->data, b->max, "%s%08lx.%s%d", ent->dir, h, postfix, k);
			}
			else {
				BIO_snprintf(b->data, b->max, "%s%c%08lx.%s%d", ent->dir, c, h, postfix, k);
			}
#ifndef OPENSSL_NO_POSIX_IO
# ifdef _WIN32
#define stat _stat
# endif
		...
		tmp = (j != -1) ? sk_X509_OBJECT_value(xl->store_ctx->objs, j) : NULL;
		CRYPTO_THREAD_unlock(ctx->lock);
		...
		if(tmp) {
			 ...
			 */
			/*- CRYPTO_add(&tmp->data.x509->references,1, CRYPTO_LOCK_X509);*/
			goto finish;
		}
	}
finish:
