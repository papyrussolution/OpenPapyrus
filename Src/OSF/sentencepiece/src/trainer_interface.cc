...
public:
	...
	size_t total_size() const { return sampler_.get() ? sampler_->total_size() : sentences_->size(); }
private:
...
MultiFileSentenceIterator::MultiFileSentenceIterator(const std::vector<std::string> &files) : files_(files) 
{
	Next();
}

bool MultiFileSentenceIterator::done() const { return (!read_done_ && file_index_ == files_.size()); }
...
bool TrainerInterface::IsValidSentencePiece(const string_util::UnicodeText &sentencepiece) const 
{
	// Returns false if the length of piece is invalid.
	if(sentencepiece.empty() || sentencepiece.size() > static_cast<size_t>(trainer_spec_.max_sentencepiece_length())) {
		return false;
	}
	...
	bool all_whitespace_piece = std::all_of(sentencepiece.begin(), sentencepiece.end(), [](char32 c) { return c == kWSChar; });
		...
		if(c == kWSChar) {
			...
			if(!trainer_spec_.allow_whitespace_only_pieces() ||
				...
				if(trainer_spec_.treat_whitespace_as_suffix()) {
					if((trainer_spec_.split_by_whitespace() && pos < sentencepiece.size() - 1) || (!trainer_spec_.split_by_whitespace() && pos < sentencepiece.size() - 1 && pos == 0)) {
						return false;
					}
				}
				else {
					if((trainer_spec_.split_by_whitespace() && pos > 0) || (!trainer_spec_.split_by_whitespace() && pos > 0 && pos == sentencepiece.size() - 1)) {
						return false;
					}
				}
			}
			...
			if(trainer_spec_.split_by_unicode_script() && s != kAnyType && prev_script != kAnyType && prev_script != s) {
				return false;
			}
			prev_script = s;
		}
...
template <typename T> void AddDPNoise(const TrainerSpec &trainer_spec, absl::BitGen &generator, T * to_update) // @sobolev SharedBitGen-->BitGen
{
	if(trainer_spec.differential_privacy_noise_level() > 0) {
		float random_num = absl::Gaussian<float>(generator, 0, trainer_spec.differential_privacy_noise_level());
...
util::Status TrainerInterface::LoadSentences() 
{
	...
	CHECK_OR_RETURN(trainer_spec_.input_format().empty() || trainer_spec_.input_format() == "text" || trainer_spec_.input_format() == "tsv") << "Supported formats are 'text' and 'tsv'.";
	CHECK_OR_RETURN((sentence_iterator_ != nullptr && trainer_spec_.input().empty()) || (sentence_iterator_ == nullptr && !trainer_spec_.input().empty()))
		<< "SentenceIterator and trainer_spec.input() must be exclusive.";
	CHECK_OR_RETURN((output_model_proto_ != nullptr && trainer_spec_.model_prefix().empty()) || (output_model_proto_ == nullptr && !trainer_spec_.model_prefix().empty()))
	...
	random::ReservoirSampler<std::string> test_sentence_sampler(&self_test_samples_, trainer_spec_.self_test_sample_size());
	int too_long_lines = 0;
	std::unique_ptr<SentenceIterator> sentence_iterator_impl;
	if(sentence_iterator_ == nullptr) {
		LOG(INFO) << "SentenceIterator is not specified. Using MultiFileSentenceIterator.";
		sentence_iterator_impl = absl::make_unique<MultiFileSentenceIterator>(std::vector<std::string>(trainer_spec_.input().begin(), trainer_spec_.input().end()));
	...
	for(; !sentence_iterator_->done(); sentence_iterator_->Next()) {
		...
		if(is_tsv) {
			...
			CHECK_EQ_OR_RETURN(v.size(), 2) << "Input format must be: word <tab> freq. " << sentence;
			sentence = v[0];
		...
		if(sentence.empty()) 
			continue;
		if(static_cast<int>(sentence.size()) > trainer_spec_.max_sentence_length()) {
			if(too_long_lines == 0) {
...
END:
	...
	if(trainer_spec_.enable_differential_privacy()) {
		...
		const auto num_workers = std::min<uint64>(trainer_spec_.num_threads(), sentences_.size() - 1);
		{
			...
			for(int n = 0; n < num_workers; ++n) {
				pool->Schedule([&, n]() {
						...
						absl::BitGen generator; // @sobolev SharedBitGen-->BitGen
						for(size_t i = n; i < sentences_.size(); i += num_workers) {
							AddDPNoise<int64>(trainer_spec_, generator, &(sentences_[i].second));
						}
					});
			}
		}
		...
		auto it = std::remove_if(sentences_.begin(), sentences_.end(), [](const Sentence &s) { return s.second <= 0; });
		...
		LOG(INFO) << "DP noise resulted in " << 1.0 * num_erased / before_size << " fraction of sentences removed.";
	}
	...
	*(model_proto->mutable_normalizer_spec()) = normalizer_spec_;
	...
	if(!trainer_spec_.hard_vocab_limit() ||
		...
		CHECK_EQ_OR_RETURN(trainer_spec_.vocab_size(), model_proto->pieces_size())
			<< absl::StrFormat("Vocabulary size too high (%d). Please set it to a value <= %d.", trainer_spec_.vocab_size(), model_proto->pieces_size());
