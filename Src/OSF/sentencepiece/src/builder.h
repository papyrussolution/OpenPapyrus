...
namespace sentencepiece {
	namespace normalizer {
		...
		public:
			Builder() = delete;
			~Builder() = delete;
			using Chars = std::vector<char32>; // Basic Unicode character sequence.
			using CharsMap = std::map<Chars, Chars>; // String-to-string mapping.
			static util::Status CompileCharsMap(const CharsMap &chars_map, std::string * output);
			...
			static util::Status GetPrecompiledCharsMap(absl::string_view name, std::string * output);
			...
			// Makes an NFKC-based mapping with NMT specific modifications around whitespaces.
			...
			static util::Status SaveCharsMap(absl::string_view filename, const CharsMap &chars_map);
		private:
