...
static constexpr char kDefaultNormalizerName[] = "nfkc";
	...
	static constexpr char kCompileError[] = "NFK compile is not enabled. rebuild with ./configure --enable-nfkc-compile";
#endif
#ifdef ENABLE_NFKC_COMPILE
// Normalize `input` with ICU's normalizer with `mode`.
Builder::Chars UnicodeNormalize(UNormalizationMode mode, const Builder::Chars &input) 
{
...
Builder::Chars ToNFKD(const Builder::Chars &input) { return UnicodeNormalize(UNORM_NFKD, input); }
Builder::Chars ToNFKC(const Builder::Chars &input) { return UnicodeNormalize(UNORM_NFKC, input); }
Builder::Chars ToNFC(const Builder::Chars &input) { return UnicodeNormalize(UNORM_NFC, input); }
Builder::Chars ToNFD(const Builder::Chars &input) { return UnicodeNormalize(UNORM_NFD, input); }
...
std::vector<Builder::Chars> ExpandUnnormalized(const Builder::Chars &nfkd, const std::map<char32, std::set<char32> > &norm2orig) 
{
	CHECK(!nfkd.empty());
...
/*static*/util::Status Builder::CompileCharsMap(const CharsMap &chars_map, std::string * output) 
{
	...
	absl::string_view trie_blob(static_cast<const char *>(trie.array()), trie.size() * trie.unit_size());
	*output = Normalizer::EncodePrecompiledCharsMap(trie_blob, normalized);
	LOG(INFO) << "Generated normalizer blob. size=" << output->size();
	return util::OkStatus();
}

/*static*/util::Status Builder::DecompileCharsMap(absl::string_view blob, Builder::CharsMap * chars_map) 
{
...
/*static*/util::Status Builder::GetPrecompiledCharsMap(absl::string_view name, std::string * output) 
{
	CHECK_OR_RETURN(output);
	if(name == "identity") {
		output->clear();
...
/*static*/util::Status Builder::MergeUnicodeCaseFoldMap(Builder::CharsMap * chars_map) 
{
#ifdef ENABLE_NFKC_COMPILE
...
/*static*/util::Status Builder::BuildNFKC_CFMap(CharsMap * chars_map) 
{
#ifdef ENABLE_NFKC_COMPILE
	CharsMap nfkc_map;
	RETURN_IF_ERROR(Builder::BuildNFKCMap(&nfkc_map));
...
/*static*/util::Status Builder::BuildNmtNFKC_CFMap(CharsMap * chars_map) 
{
#ifdef ENABLE_NFKC_COMPILE
	CharsMap nfkc_map;
...
/*static*/util::Status Builder::BuildNFKDMap(CharsMap * chars_map) 
{
#ifdef ENABLE_NFKC_COMPILE
	constexpr int kMaxUnicode = 0x10FFFF;
	for(char32 cp = 1; cp <= kMaxUnicode; ++cp) {
...
/*static*/util::Status Builder::LoadCharsMap(absl::string_view filename, CharsMap * chars_map) 
{
...
/*static*/util::Status Builder::SaveCharsMap(absl::string_view filename, const Builder::CharsMap &chars_map) 
{
	...
	for(const auto &c : chars_map) {
		...
		line = absl::StrReplaceAll(line, {{"\b", " "}, {"\v", " "}, {"\f", " "}, {"\n", " "}, {"\r", " "}});
		output->WriteLine(line);
	}
	return util::OkStatus();
}

/*static*/util::Status Builder::RemoveRedundantMap(CharsMap * chars_map) 
{
	CHECK_OR_RETURN(chars_map);
	...
	for(size_t len = 2; len <= max_len; ++len) {
		for(const auto &p : *chars_map) {
			if(p.first.size() == len && p.second != Normalize(new_chars_map, p.first, len - 1)) {
				new_chars_map.insert(p);
			}
		}
	}
