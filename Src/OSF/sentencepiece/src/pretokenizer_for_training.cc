...
std::string PretokenizerForTrainingInterface::PreTokenize(absl::string_view text) const 
{
	return Postprocess(Tokenize(Preprocess(text)));
}

/*static*/std::string PretokenizerForTrainingInterface::Preprocess(absl::string_view text) 
{
	// Escapes kWSStr (_) as this character may not be processed by pre-tokenizer.
	return absl::StrReplaceAll(text, {{kWSStr, " "}});
}

/*static*/std::string PretokenizerForTrainingInterface::Postprocess(const SentencePieceText &spt) 
{
