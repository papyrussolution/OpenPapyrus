...
void ModelInterface::InitializePieces() {
	...
	if(model_proto_->trainer_spec().byte_fallback()) {
		...
		if(std::find(byte_found.begin(), byte_found.end(), false) != byte_found.end()) {
			status_ = util::InternalError("there are not 256 byte pieces although `byte_fallback` is true.");
			return;
		}
	}
...
std::vector<absl::string_view> SplitIntoWords(absl::string_view text, bool treat_ws_as_suffix, bool allow_ws_only_pieces) 
{
