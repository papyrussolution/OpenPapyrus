...
std::vector<absl::string_view> SplitIntoWords(absl::string_view text, bool treat_ws_as_suffix = false,
...
public:
	...
	virtual NBestEncodeResult NBestEncode(absl::string_view normalized, int nbest_size) const 
	{
		LOG(ERROR) << "Not implemented.";
		return NBestEncodeResult();
	}
	virtual EncodeResult SampleEncode(absl::string_view normalized, float alpha) const 
	{
		LOG(ERROR) << "Not implemented.";
		return EncodeResult();
	}
	...
	virtual NBestEncodeResult SampleEncodeAndScore(absl::string_view normalized, float alpha, int samples, bool wor, bool include_best) const 
	{
		LOG(ERROR) << "Not implemented.";
	...
	virtual float CalculateEntropy(absl::string_view normalized, float alpha) const 
	{
		LOG(ERROR) << "Not implemented.";
		return 0.0;
	}
	...
	virtual const std::string &IdToPiece(int id) const { return model_proto_->pieces(id).piece(); }
...
protected:
	...
	inline float GetScoreInlined(int id) const { return model_proto_->pieces(id).score(); }
	inline bool IsUnknownInlined(int id) const { return (model_proto_->pieces(id).type() == ModelProto::SentencePiece::UNKNOWN); }
	inline bool IsControlInlined(int id) const { return (model_proto_->pieces(id).type() == ModelProto::SentencePiece::CONTROL); }
	inline bool IsUnusedInlined(int id) const { return (model_proto_->pieces(id).type() == ModelProto::SentencePiece::UNUSED); }
	inline bool IsUserDefinedInlined(int id) const { return (model_proto_->pieces(id).type() == ModelProto::SentencePiece::USER_DEFINED); }
	inline bool IsByteInlined(int id) const { return (model_proto_->pieces(id).type() == ModelProto::SentencePiece::BYTE); }
	...
	PieceToIdMap pieces_; // piece -> id map for normal pieces
	PieceToIdMap reserved_id_map_; // piece -> id map for control, unknown, and byte pieces
	int unk_id_ = 0; // unknown id.
	util::Status status_; // status.
};
}  // namespace sentencepiece
