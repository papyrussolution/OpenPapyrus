...
template <typename Target> inline bool lexical_cast(absl::string_view arg, Target * result) 
{
	std::stringstream ss;
	return (ss << arg.data() && ss >> *result);
}

template <> inline bool lexical_cast(absl::string_view arg, bool * result) 
{
...
template <> inline bool lexical_cast(absl::string_view arg, std::string * result) 
{
	*result = std::string(arg);
	return true;
}

template <typename T> inline bool DecodePOD(absl::string_view str, T * result) 
{
...
template <typename T> inline std::string IntToHex(T value) 
{
...
template <typename T> inline T HexToInt(absl::string_view value) 
{
	T n;
...
template <typename T> inline size_t Itoa(T val, char * s) 
{
	char * org = s;
	if(val < 0) {
		*s++ = '-';
...
public:
	...
	template <typename T> StatusBuilder &operator<<(const T &value) 
	{
		os_ << value;
		return *this;
	}
...
namespace port {
	template <typename T> void STLDeleteElements(std::vector<T *> * vec) 
	{
		for(auto item : *vec) {
			delete item;
		}
