 ...
 */
struct gdstate {
	struct GetDateToken * tokenp; /* Pointer to next token. */
...
static int nexttoken(const char ** in, time_t * value)
{
	...
	for(;;) {
		 ...
		 */
		if(isdigit((uchar)(c = **in))) {
			for(*value = 0; isdigit((uchar)(c = *(*in)++));)
				*value = 10 * *value + c - '0';
			(*in)--;
			return (tUNUMBER);
		}
...
{
	struct GetDateToken tokens[256];
	struct gdstate _gds;
	struct GetDateToken * lasttoken;
	struct gdstate  * gds;
	struct tm local, * tm;
	...
	memzero(tokens, sizeof(tokens));
	/* Initialize the parser state. */
	memzero(&_gds, sizeof(_gds));
	gds = &_gds;
