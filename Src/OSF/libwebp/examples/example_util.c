...
static void ResetCommandLineArguments(int argc, const char* argv[], CommandLineArguments* const args) 
{
	assert(args != NULL);
	args->argc_ = argc;
	args->argv_ = argv;
...
int ExUtilInitCommandLineArguments(int argc, const char* argv[], CommandLineArguments* const args) 
{
	...
	if(argc == 1 && argv[0][0] != '-') {
		...
		fprintf(stderr, "Error: Reading arguments from a file is a feature unavailable with Unicode binaries.\n");
		return 0;
#endif
		...
		if(args->argv_ == NULL) 
			return 0;
		argc = 0;
		for(cur = strtok((char*)args->argv_data_.bytes, sep); cur != NULL; cur = strtok(NULL, sep)) {
			if(argc == MAX_ARGC) {
...
int ExUtilReadFileToWebPData(const char* const filename, WebPData* const webp_data) 
{
	const uint8* data;
	size_t size;
