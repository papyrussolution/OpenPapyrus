...
static int ALPHDecode(VP8Decoder* const dec, int row, int num_rows) {
	...
	if(alph_dec->method_ == ALPHA_NO_COMPRESSION) {
		int y;
		const uint8* prev_line = dec->alpha_prev_line_;
		const uint8* deltas = dec->alpha_data_ + ALPHA_HEADER_LEN + row * width;
		uint8* dst = dec->alpha_plane_ + row * width;
...
static int AllocateAlphaPlane(VP8Decoder* const dec, const VP8Io* const io) {
	...
	dec->alpha_plane_mem_ =
	    (uint8*)WebPSafeMalloc(alpha_size, sizeof(*dec->alpha_plane_));
	...
	if(!dec->is_alpha_decoded_) {
		...
		if(dec->is_alpha_decoded_) { // finished?
			...
			if(dec->alpha_dithering_ > 0) {
				uint8* const alpha = dec->alpha_plane_ + io->crop_top * width
				    + io->crop_left;
