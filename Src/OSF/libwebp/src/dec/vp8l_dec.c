...
static const uint8 kLiteralMap[HUFFMAN_CODES_PER_META_CODE] = {
	0, 1, 1, 1, 0
};
...
static int ReadHuffmanCode(int alphabet_size, VP8LDecoder* const dec, int* const code_lengths, HuffmanCode* const table) 
{
	...
	memzero(code_lengths, alphabet_size * sizeof(*code_lengths));
...
static int AllocateAndInitRescaler(VP8LDecoder* const dec, VP8Io* const io) {
	...
	uint8* memory = (uint8*)WebPSafeMalloc(memory_size, sizeof(*memory));
	if(memory == NULL) {
		dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
		return 0;
	}
	...
	if(!WebPRescalerInit(dec->rescaler, in_width, in_height,
	    (uint8*)scaled_data, out_width, out_height,
	    0, num_channels, work)) {
		return 0;
	}
	return 1;
...
static int EmitRescaledRowsRGBA(const VP8LDecoder* const dec,
    uint8* in, int in_stride, int mb_h,
    uint8* const out, int out_stride) {
	const WEBP_CSP_MODE colorspace = dec->output_->colorspace;
	int num_lines_in = 0;
	int num_lines_out = 0;
	while(num_lines_in < mb_h) {
		uint8* const row_in = in + (uint64_t)num_lines_in * in_stride;
		uint8* const row_out = out + (uint64_t)num_lines_out * out_stride;
...
static int EmitRows(WEBP_CSP_MODE colorspace,
    const uint8* row_in, int in_stride,
    int mb_w, int mb_h,
    uint8* const out, int out_stride) {
	int lines = mb_h;
	uint8* row_out = out;
...
static void ConvertToYUVA(const uint32_t* const src, int width, int y_pos, const WebPDecBuffer* const output) 
{
	...
	// then U/V planes
	{
		uint8* const u = buf->u + (y_pos >> 1) * buf->u_stride;
		uint8* const v = buf->v + (y_pos >> 1) * buf->v_stride;
...
static int EmitRowsYUVA(const VP8LDecoder* const dec, const uint8* in, int in_stride, int mb_w, int num_rows) 
{
	int y_pos = dec->last_out_row_;
	while(num_rows-- > 0) {
...
static FORCEINLINE int GetMetaIndex(const uint32_t* const image, int xsize, int bits, int x, int y) 
{
	if(bits == 0) return 0;
	return image[xsize * (y >> bits) + (x >> bits)];
}

static FORCEINLINE HTreeGroup* GetHtreeGroupForPos(VP8LMetadata* const hdr,
    int x, int y) {
...
static void ProcessRows(VP8LDecoder* const dec, int row) {
	...
	if(num_rows > 0) { // Emit output.
		...
		uint8* rows_data = (uint8*)dec->argb_cache_;
...
static void AlphaApplyFilter(ALPHDecoder* const alph_dec,
    ...
    uint8* out, int stride) {
	if(alph_dec->filter_ != WEBP_FILTER_NONE) {
		int y;
		const uint8* prev_line = alph_dec->prev_line_;
...
static void ExtractPalettedAlphaRows(VP8LDecoder* const dec, int last_row) {
	...
	if(last_row > first_row) {
		...
		uint8* out = alph_dec->output_ + width * first_row;
		const uint8* const in =
		    (uint8*)dec->pixels_ + dec->width_ * first_row;
...
static FORCEINLINE void CopyBlock8b(uint8* const dst, int dist, int length) {
...
static FORCEINLINE void CopySmallPattern32b(const uint32_t* src,
    uint32_t* dst,
...
static FORCEINLINE void CopyBlock32b(uint32_t* const dst,
    int dist, int length) {
...
static int ExpandColorMap(int num_colors, VP8LTransform* const transform) 
{
	...
	if(new_color_map == NULL) {
		...
		uint8* const data = (uint8*)transform->data_;
...
static int AllocateInternalBuffers8b(VP8LDecoder* const dec) {
	...
	dec->argb_cache_ = NULL; // for soundness
	dec->pixels_ = (uint32_t*)WebPSafeMalloc(total_num_pixels, sizeof(uint8));
	if(dec->pixels_ == NULL) {
		dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
		return 0;
	}
...
static void ExtractAlphaRows(VP8LDecoder* const dec, int last_row) 
{
	...
	while(num_rows > 0) {
		...
		ALPHDecoder* const alph_dec = (ALPHDecoder*)dec->io_->opaque;
		uint8* const output = alph_dec->output_;
...
int VP8LDecodeAlphaImageStream(ALPHDecoder* const alph_dec, int last_row) {
	...
	return alph_dec->use_8b_decode_ ?
	       DecodeAlphaData(dec, (uint8*)dec->pixels_, dec->width_, dec->height_,
		   last_row) :
