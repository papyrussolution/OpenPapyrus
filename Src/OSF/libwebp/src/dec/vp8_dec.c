...
static void ResetSegmentHeader(VP8SegmentHeader* const hdr) 
{
	...
	memzero(hdr->quantizer_, sizeof(hdr->quantizer_));
	memzero(hdr->filter_strength_, sizeof(hdr->filter_strength_));
}
...
static int GetCoeffsFast(VP8BitReader* const br,
    const VP8BandProbas* const prob[],
    int ctx, const quant_t dq, int n, int16_t* out) {
	const uint8* p = prob[n]->probas_[ctx];
	for(; n < 16; ++n) {
...
static FORCEINLINE uint32_t NzCodeBits(uint32_t nz_coeffs, int nz, int dc_nz) {
	nz_coeffs <<= 2;
...
void VP8Clear(VP8Decoder* const dec) 
{
	if(dec) {
		...
		dec->mem_ = NULL;
		dec->mem_size_ = 0;
		memzero(&dec->br_, sizeof(dec->br_));
		dec->ready_ = 0;
	}
}
