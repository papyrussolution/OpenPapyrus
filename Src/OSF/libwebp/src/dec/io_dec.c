...
static int EmitYUV(const VP8Io* const io, WebPDecParams* const p) 
{
	...
	uint8* const u_dst = buf->u + (size_t)(io->mb_y >> 1) * buf->u_stride;
	uint8* const v_dst = buf->v + (size_t)(io->mb_y >> 1) * buf->v_stride;
...
static int EmitSampledRGB(const VP8Io* const io, WebPDecParams* const p) {
	...
	uint8* const dst = buf->rgba + (size_t)io->mb_y * buf->stride;
...
static int EmitFancyRGB(const VP8Io* const io, WebPDecParams* const p) {
	...
	uint8* dst = buf->rgba + (size_t)io->mb_y * buf->stride;
	WebPUpsampleLinePairFunc upsample = WebPUpsamplers[p->output->colorspace];
	const uint8* cur_y = io->y;
	const uint8* cur_u = io->u;
	const uint8* cur_v = io->v;
	const uint8* top_u = p->tmp_u;
	const uint8* top_v = p->tmp_v;
	int y = io->mb_y;
...
static void FillAlphaPlane(uint8* dst, int w, int h, int stride) {
	int j;
	for(j = 0; j < h; ++j) {
...
static int EmitAlphaYUV(const VP8Io* const io, WebPDecParams* const p,
	...
	const uint8* alpha = io->a;
	const WebPYUVABuffer* const buf = &p->output->u.YUVA;
	const int mb_w = io->mb_w;
	const int mb_h = io->mb_h;
	uint8* dst = buf->a + (size_t)io->mb_y * buf->a_stride;
	int j;
...
static int GetAlphaSourceRow(const VP8Io* const io,
    const uint8** alpha, int* const num_rows) {
	int start_y = io->mb_y;
	*num_rows = io->mb_h;
...
static int EmitAlphaRGB(const VP8Io* const io, WebPDecParams* const p,
	...
	if(alpha != NULL) {
		...
		uint8* const base_rgba = buf->rgba + start_y * buf->stride;
...
static int Rescale(const uint8* src, int src_stride,
...
static int EmitRescaledYUV(const VP8Io* const io, WebPDecParams* const p) {
	...
	if(WebPIsAlphaMode(p->output->colorspace) && io->a != NULL) {
		...
		WebPMultRows((uint8*)io->y, io->y_stride,
...
static int EmitRescaledAlphaYUV(const VP8Io* const io, WebPDecParams* const p,
    int expected_num_lines_out) {
	const WebPYUVABuffer* const buf = &p->output->u.YUVA;
	uint8* const dst_a = buf->a + (size_t)p->last_y * buf->a_stride;
	if(io->a != NULL) {
		uint8* const dst_y = buf->y + (size_t)p->last_y * buf->y_stride;
...
static int ExportRGB(WebPDecParams* const p, int y_pos) {
	...
	const WebPRGBABuffer* const buf = &p->output->u.RGBA;
	uint8* dst = buf->rgba + (size_t)y_pos * buf->stride;
	int num_lines_out = 0;
...
static int ExportAlpha(WebPDecParams* const p, int y_pos, int max_lines_out) {
	const WebPRGBABuffer* const buf = &p->output->u.RGBA;
	uint8* const base_rgba = buf->rgba + (size_t)y_pos * buf->stride;
	const WEBP_CSP_MODE colorspace = p->output->colorspace;
	const int alpha_first =
	    (colorspace == MODE_ARGB || colorspace == MODE_Argb);
...
static int InitRGBRescaler(const VP8Io* const io, WebPDecParams* const p) {
	...
	uint8* tmp; // tmp storage for scaled YUV444 samples before RGB conversion
