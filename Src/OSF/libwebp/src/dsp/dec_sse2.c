...
static FORCEINLINE void GetBaseDelta_SSE2(const __m128i* const p1,
...
static FORCEINLINE void DoSimpleFilter_SSE2(__m128i* const p0,
    __m128i* const q0,
...
// Pixels 'pi' and 'qi' are int8_t on input, uint8 on output (sign flip).
static FORCEINLINE void Update2Pixels_SSE2(__m128i* const pi, __m128i* const qi,
...
static FORCEINLINE void NeedsFilter_SSE2(const __m128i* const p1,
    const __m128i* const p0,
    const __m128i* const q0,
    const __m128i* const q1,
...
static FORCEINLINE void DoFilter4_SSE2(__m128i* const p1, __m128i* const p0,
...
static FORCEINLINE void DoFilter6_SSE2(__m128i* const p2, __m128i* const p1,
...
static FORCEINLINE void ComplexMask_SSE2(const __m128i* const p1,
    const __m128i* const p0,
    const __m128i* const q0,
    const __m128i* const q1,
...
static void VE4_SSE2(uint8* dst) {    // vertical
...
static void LD4_SSE2(uint8* dst) {   // Down-Left
	const __m128i one = _mm_set1_epi8(1);
...
static void VL4_SSE2(uint8* dst) {   // Vertical-Left
	const __m128i one = _mm_set1_epi8(1);
	const __m128i ABCDEFGH = _mm_loadl_epi64((__m128i*)(dst - BPS));
...
static void DC16NoLeft_SSE2(uint8* dst) {  // DC with left samples unavailable
	const __m128i zero = _mm_setzero_si128();
	const __m128i top = _mm_loadu_si128((const __m128i*)(dst - BPS));
	const __m128i sad8x2 = _mm_sad_epu8(top, zero);
	// sum the two sads: sad8x2[0:1] + sad8x2[8:9]
