...
static void ITransform_SSE2(const uint8* ref, const int16_t* in, uint8* dst,
	...
	// Add inverse transform to 'ref' and store.
	{
		...
		if(do_two) {
			...
			ref0 = _mm_cvtsi32_si128(SMem::Get32(&ref[0 * BPS]));
			ref1 = _mm_cvtsi32_si128(SMem::Get32(&ref[1 * BPS]));
			ref2 = _mm_cvtsi32_si128(SMem::Get32(&ref[2 * BPS]));
			ref3 = _mm_cvtsi32_si128(SMem::Get32(&ref[3 * BPS]));
		}
		// Convert to 16b.
		...
		if(do_two) {
			...
			SMem::Put(&dst[0 * BPS], _mm_cvtsi128_si32(ref0));
			SMem::Put(&dst[1 * BPS], _mm_cvtsi128_si32(ref1));
			SMem::Put(&dst[2 * BPS], _mm_cvtsi128_si32(ref2));
			SMem::Put(&dst[3 * BPS], _mm_cvtsi128_si32(ref3));
		}
	}
}
...
static void FTransform_SSE2(const uint8* src, const uint8* ref,
    int16_t* out) {
	const __m128i zero = _mm_setzero_si128();
	// Load src.
...
static void FTransform2_SSE2(const uint8* src, const uint8* ref,
    int16_t* out) {
	const __m128i zero = _mm_setzero_si128();
...
static FORCEINLINE void DC8uv_SSE2(uint8* dst, const uint8* left,
    const uint8* top) {
	const __m128i top_values = _mm_loadl_epi64((const __m128i*)top);
...
static FORCEINLINE void DC8uvNoLeft_SSE2(uint8* dst, const uint8* top) {
	const __m128i zero = _mm_setzero_si128();
	const __m128i top_values = _mm_loadl_epi64((const __m128i*)top);
...
static FORCEINLINE void DC8uvNoTop_SSE2(uint8* dst, const uint8* left) {
	// 'left' is contiguous so we can reuse the top summation.
	DC8uvNoLeft_SSE2(dst, left);
}

static FORCEINLINE void DC8uvNoTopLeft_SSE2(uint8* dst) {
	Put8x8uv_SSE2(0x80, dst);
}

static FORCEINLINE void DC8uvMode_SSE2(uint8* dst, const uint8* left,
    const uint8* top) {
	if(top != NULL) {
...
static FORCEINLINE void DC16_SSE2(uint8* dst, const uint8* left,
    const uint8* top) {
...
static FORCEINLINE void DC16NoLeft_SSE2(uint8* dst, const uint8* top) {
	const __m128i top_row = _mm_load_si128((const __m128i*)top);
	const int DC = VP8HorizontalAdd8b(&top_row) + 8;
	Put16_SSE2(DC >> 4, dst);
}

static FORCEINLINE void DC16NoTop_SSE2(uint8* dst, const uint8* left) {
	// 'left' is contiguous so we can reuse the top summation.
	DC16NoLeft_SSE2(dst, left);
}

static FORCEINLINE void DC16NoTopLeft_SSE2(uint8* dst) {
	Put16_SSE2(0x80, dst);
}

static FORCEINLINE void DC16Mode_SSE2(uint8* dst, const uint8* left,
    const uint8* top) {
	if(top != NULL) {
		if(left != NULL) { // top and left present
...
static FORCEINLINE void VE4_SSE2(uint8* dst, const uint8* top) // vertical
{ 
...
static FORCEINLINE void HE4_SSE2(uint8* dst, const uint8* top) // horizontal
{
	const int X = top[-1];
	const int I = top[-2];
	const int J = top[-3];
	const int K = top[-4];
	const int L = top[-5];
	SMem::Put(dst + 0 * BPS, 0x01010101U * AVG3(X, I, J));
	SMem::Put(dst + 1 * BPS, 0x01010101U * AVG3(I, J, K));
	SMem::Put(dst + 2 * BPS, 0x01010101U * AVG3(J, K, L));
	SMem::Put(dst + 3 * BPS, 0x01010101U * AVG3(K, L, L));
}

static FORCEINLINE void DC4_SSE2(uint8* dst, const uint8* top) 
{
	uint32_t dc = 4;
	for(int i = 0; i < 4; ++i) 
		dc += top[i] + top[-5 + i];
...
static FORCEINLINE void LD4_SSE2(uint8* dst, const uint8* top) // Down-Left
{
	const __m128i one = _mm_set1_epi8(1);
...
static FORCEINLINE void VR4_SSE2(uint8* dst, const uint8* top) // Vertical-Right
{
	const __m128i one = _mm_set1_epi8(1);
	const int I = top[-2];
	const int J = top[-3];
...
static FORCEINLINE void VL4_SSE2(uint8* dst, const uint8* top) // Vertical-Left
{
	const __m128i one = _mm_set1_epi8(1);
	const __m128i ABCDEFGH = _mm_loadl_epi64((const __m128i*)top);
...
static FORCEINLINE void RD4_SSE2(uint8* dst, const uint8* top) // Down-right
{
	const __m128i one = _mm_set1_epi8(1);
...
static FORCEINLINE void HU4_SSE2(uint8* dst, const uint8* top) 
{
	const int I = top[-2];
	const int J = top[-3];
	const int K = top[-4];
...
static FORCEINLINE void HD4_SSE2(uint8* dst, const uint8* top) 
{
	const int X = top[-1];
	const int I = top[-2];
	const int J = top[-3];
...
static FORCEINLINE void TM4_SSE2(uint8* dst, const uint8* top) 
{
	const __m128i zero = _mm_setzero_si128();
...
static FORCEINLINE int SSE_16xN_SSE2(const uint8* a, const uint8* b,
    int num_pairs) {
...
static int SSE16x16_SSE2(const uint8* a, const uint8* b) {
	return SSE_16xN_SSE2(a, b, 8);
}

static int SSE16x8_SSE2(const uint8* a, const uint8* b) {
...
static int SSE8x8_SSE2(const uint8* a, const uint8* b) {
	const __m128i zero = _mm_setzero_si128();
	int num_pairs = 4;
	__m128i sum = zero;
	int32_t tmp[4];
...
static int SSE4x4_SSE2(const uint8* a, const uint8* b) {
	const __m128i zero = _mm_setzero_si128();
...
static int TTransform_SSE2(const uint8* inA, const uint8* inB,
...
static int Disto4x4_SSE2(const uint8* const a, const uint8* const b,
    const uint16_t* const w) {
	const int diff_sum = TTransform_SSE2(a, b, w);
	return abs(diff_sum) >> 5;
}

static int Disto16x16_SSE2(const uint8* const a, const uint8* const b,
    const uint16_t* const w) {
	int D = 0;
	int x, y;
