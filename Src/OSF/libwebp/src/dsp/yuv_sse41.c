...
static FORCEINLINE __m128i Load_HI_16_SSE41(const uint8* src) {
	const __m128i zero = _mm_setzero_si128();
	return _mm_unpacklo_epi8(zero, _mm_loadl_epi64((const __m128i*)src));
}

// Load and replicate the U/V samples
static FORCEINLINE __m128i Load_UV_HI_8_SSE41(const uint8* src) {
	const __m128i zero = _mm_setzero_si128();
...
static void YUV444ToRGB_SSE41(const uint8* const y,
    const uint8* const u,
    const uint8* const v,
...
static void YUV420ToRGB_SSE41(const uint8* const y,
    const uint8* const u,
    const uint8* const v,
    __m128i* const R, __m128i* const G,
    __m128i* const B) {
...
static FORCEINLINE void PlanarTo24b_SSE41(__m128i* const in0, __m128i* const in1, __m128i* const in2,
...
void VP8YuvToRgb32_SSE41(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
...
void VP8YuvToBgr32_SSE41(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
	__m128i R0, R1, R2, R3, G0, G1, G2, G3, B0, B1, B2, B3;
...
static void YuvToRgbRow_SSE41(const uint8* y,
...
static void YuvToBgrRow_SSE41(const uint8* y,
    const uint8* u, const uint8* v,
    uint8* dst, int len) {
	int n;
	for(n = 0; n + 32 <= len; n += 32, dst += 32 * 3) {
...
static FORCEINLINE void RGB24PackedToPlanar_SSE41(const uint8* const rgb, __m128i* const out /*out[6]*/) {
...
static FORCEINLINE void RGB32PackedToPlanar_SSE41(const uint32_t* const argb, __m128i* const rgb /*in[6]*/) {
	const __m128i zero = _mm_setzero_si128();
...
static FORCEINLINE void ConvertRGBToY_SSE41(const __m128i* const R,
...
static FORCEINLINE void ConvertRGBToUV_SSE41(const __m128i* const R,
    const __m128i* const G,
    const __m128i* const B,
    __m128i* const U,
    __m128i* const V) {
...
static void ConvertRGB24ToY_SSE41(const uint8* rgb, uint8* y, int width) {
...
static void ConvertBGR24ToY_SSE41(const uint8* bgr, uint8* y, int width) {
	const int max_width = width & ~31;
	int i;
...
static void ConvertARGBToY_SSE41(const uint32_t* argb, uint8* y, int width) {
...
static FORCEINLINE void RGBA32PackedToPlanar_16b_SSE41(const uint16_t* const rgbx,
