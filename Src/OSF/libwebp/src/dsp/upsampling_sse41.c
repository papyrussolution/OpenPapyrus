...
#define SSE4_UPSAMPLE_FUNC(FUNC_NAME, FUNC, XSTEP)                             \
	static void FUNC_NAME(const uint8* top_y, const uint8* bottom_y,           \
		...
		uint8* const r_u = (uint8*)((uintptr_t)(uv_buf + 15) & ~15);             \
		...
		if(len > 1) {                                                               \
			...
			if(bottom_y) memcpy(tmp_bottom, bottom_y + pos, len - pos);       \
