...
static FORCEINLINE uint32_t VP8GetARGBIndex(uint32_t idx) { return (idx >> 8) & 0xff; }
static FORCEINLINE uint8 VP8GetAlphaIndex(uint8 idx) { return idx; }
static FORCEINLINE uint32_t VP8GetARGBValue(uint32_t val) { return val; }
static FORCEINLINE uint8 VP8GetAlphaValue(uint32_t val) { return (val >> 8) & 0xff; }

//------------------------------------------------------------------------------
// Misc methods.

// Computes sampled size of 'size' when sampling using 'sampling bits'.
static FORCEINLINE uint32_t VP8LSubSampleSize(uint32_t size, uint32_t sampling_bits) 
{
	return (size + (1 << sampling_bits) - 1) >> sampling_bits;
}

// Converts near lossless quality into max number of bits shaved off.
static FORCEINLINE int VP8LNearLosslessBits(int near_lossless_quality) 
{
	//    100 -> 0
	// 80..99 -> 1
	// 60..79 -> 2
...
static WEBP_UBSAN_IGNORE_UNSIGNED_OVERFLOW FORCEINLINE uint32_t VP8LSubPixels(uint32_t a, uint32_t b) 
{
