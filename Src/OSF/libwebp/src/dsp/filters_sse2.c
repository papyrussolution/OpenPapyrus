...
static void PredictLineTop_SSE2(const uint8* src, const uint8* pred,
    uint8* dst, int length) {
	int i;
...
static void PredictLineLeft_SSE2(const uint8* src, uint8* dst, int length) {
	int i;
	const int max_pos = length & ~31;
	assert(length >= 0);
	for(i = 0; i < max_pos; i += 32) {
...
static FORCEINLINE void DoHorizontalFilter_SSE2(const uint8* in,
...
static FORCEINLINE void DoVerticalFilter_SSE2(const uint8* in,
...
static FORCEINLINE int GradientPredictor_SSE2(uint8 a, uint8 b, uint8 c) {
	const int g = a + b - c;
	return ((g & ~0xff) == 0) ? g : (g < 0) ? 0 : 255; // clip to 8bit
}

static void GradientPredictDirect_SSE2(const uint8* const row,
    const uint8* const top,
    uint8* const out, int length) {
...
static FORCEINLINE void DoGradientFilter_SSE2(const uint8* in,
    int width, int height, int stride,
    int row, int num_rows,
    uint8* out) {
	const size_t start_offset = row * stride;
...
static void HorizontalFilter_SSE2(const uint8* data, int width, int height,
    int stride, uint8* filtered_data) {
	DoHorizontalFilter_SSE2(data, width, height, stride, 0, height,
	    filtered_data);
}

static void VerticalFilter_SSE2(const uint8* data, int width, int height,
    int stride, uint8* filtered_data) {
	DoVerticalFilter_SSE2(data, width, height, stride, 0, height, filtered_data);
}

static void GradientFilter_SSE2(const uint8* data, int width, int height,
    int stride, uint8* filtered_data) {
...
static void HorizontalUnfilter_SSE2(const uint8* prev, const uint8* in,
    uint8* out, int width) {
	int i;
	__m128i last;
...
static void VerticalUnfilter_SSE2(const uint8* prev, const uint8* in,
    uint8* out, int width) {
	if(prev == NULL) {
...
static void GradientPredictInverse_SSE2(const uint8* const in,
    const uint8* const top,
...
static void GradientUnfilter_SSE2(const uint8* prev, const uint8* in,
    uint8* out, int width) {
	if(prev == NULL) {
		HorizontalUnfilter_SSE2(NULL, in, out, width);
	}
