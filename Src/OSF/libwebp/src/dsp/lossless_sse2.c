...
static void ConvertBGRAToRGB_SSE2(const uint32_t* src, int num_pixels, uint8* dst) 
{
	...
	// left-overs
	if(num_pixels > 0) {
		VP8LConvertBGRAToRGB_C((const uint32_t*)in, num_pixels, (uint8*)out);
	}
}
...
static void ConvertBGRAToRGBA_SSE2(const uint32_t* src, int num_pixels, uint8* dst) 
{
	...
	// left-overs
	if(num_pixels > 0) {
		VP8LConvertBGRAToRGBA_C((const uint32_t*)in, num_pixels, (uint8*)out);
	}
}

static void ConvertBGRAToRGBA4444_SSE2(const uint32_t* src, int num_pixels, uint8* dst) 
{
	...
	// left-overs
	if(num_pixels > 0) {
		VP8LConvertBGRAToRGBA4444_C((const uint32_t*)in, num_pixels, (uint8*)out);
	}
}
...
static void ConvertBGRAToRGB565_SSE2(const uint32_t* src, int num_pixels, uint8* dst) 
{
	...
	// left-overs
	if(num_pixels > 0) {
		VP8LConvertBGRAToRGB565_C((const uint32_t*)in, num_pixels, (uint8*)out);
	}
}
...
static void ConvertBGRAToBGR_SSE2(const uint32_t* src,
	...
	const __m128i* in = (const __m128i*)src;
	const uint8* const end = dst + num_pixels * 3;
