...
static FORCEINLINE uint8 TransformColorRed(uint8 green_to_red, uint32_t argb) 
{
	const int8_t green = U32ToS8(argb >> 8);
	int new_red = argb >> 16;
...
static FORCEINLINE uint8 TransformColorBlue(uint8 green_to_blue, uint8 red_to_blue, uint32_t argb) 
{
	const int8_t green = U32ToS8(argb >>  8);
	const int8_t red   = U32ToS8(argb >> 16);
	uint8 new_blue = argb & 0xff;
...
void VP8LBundleColorMap_C(const uint8* const row, int width, int xbits,
    uint32_t* dst) {
	int x;
	if(xbits > 0) {
...
#define ADD(X, ARG, LEN) do {                                                  \
		if(a->is_used_[X]) {                                                        \
			...
			memzero(&out->ARG[0], (LEN)*sizeof(out->ARG[0]));                      \
		}                                                                            \
} while(0)
