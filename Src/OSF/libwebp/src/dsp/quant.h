...
static FORCEINLINE int IsFlat(const int16_t* levels, int num_blocks, int thresh) 
{
...
static FORCEINLINE int IsFlat(const int16_t* levels, int num_blocks, int thresh) 
{
	...
	while(num_blocks-- > 0) { // TODO(skal): refine positional scoring?
		for(int i = 1; i < 16; ++i) { // omit DC, we're only interested in AC
			score += (levels[i] != 0);
			if(score > thresh) 
				return 0;
		}
...
static FORCEINLINE int IsFlatSource16(const uint8* src) 
{
