...
static FORCEINLINE __m128i Load_HI_16_SSE2(const uint8* src) {
	const __m128i zero = _mm_setzero_si128();
	return _mm_unpacklo_epi8(zero, _mm_loadl_epi64((const __m128i*)src));
}

// Load and replicate the U/V samples
static FORCEINLINE __m128i Load_UV_HI_8_SSE2(const uint8* src) {
	const __m128i zero = _mm_setzero_si128();
...
static void YUV444ToRGB_SSE2(const uint8* const y,
    const uint8* const u,
    const uint8* const v,
...
static void YUV420ToRGB_SSE2(const uint8* const y,
    const uint8* const u,
    const uint8* const v,
    __m128i* const R, __m128i* const G,
    __m128i* const B) {
...
static FORCEINLINE void PackAndStore4_SSE2(const __m128i* const R,
...
static FORCEINLINE void PackAndStore4444_SSE2(const __m128i* const R,
    const __m128i* const G,
    const __m128i* const B,
    const __m128i* const A,
    uint8* const dst) {
...
static FORCEINLINE void PackAndStore565_SSE2(const __m128i* const R,
    const __m128i* const G,
    const __m128i* const B,
    uint8* const dst) {
...
static FORCEINLINE void PlanarTo24b_SSE2(__m128i* const in0, __m128i* const in1,
...
void VP8YuvToRgba32_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
...
void VP8YuvToBgra32_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
	const __m128i kAlpha = _mm_set1_epi16(255);
	int n;
	for(n = 0; n < 32; n += 8, dst += 32) {
...
void VP8YuvToArgb32_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
	const __m128i kAlpha = _mm_set1_epi16(255);
	int n;
	for(n = 0; n < 32; n += 8, dst += 32) {
...
void VP8YuvToRgba444432_SSE2(const uint8* y, const uint8* u,
    const uint8* v, uint8* dst) {
	const __m128i kAlpha = _mm_set1_epi16(255);
	int n;
...
void VP8YuvToRgb56532_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
	int n;
	for(n = 0; n < 32; n += 8, dst += 16) {
		__m128i R, G, B;
...
void VP8YuvToRgb32_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
...
void VP8YuvToBgr32_SSE2(const uint8* y, const uint8* u, const uint8* v,
    uint8* dst) {
	__m128i R0, R1, R2, R3, G0, G1, G2, G3, B0, B1, B2, B3;
...
static void YuvToBgraRow_SSE2(const uint8* y,
...
static void YuvToArgbRow_SSE2(const uint8* y,
    const uint8* u, const uint8* v,
    uint8* dst, int len) {
	const __m128i kAlpha = _mm_set1_epi16(255);
	int n;
...
static void YuvToRgbRow_SSE2(const uint8* y,
    const uint8* u, const uint8* v,
    uint8* dst, int len) {
	int n;
...
static void YuvToBgrRow_SSE2(const uint8* y,
    const uint8* u, const uint8* v,
    uint8* dst, int len) {
	int n;
	for(n = 0; n + 32 <= len; n += 32, dst += 32 * 3) {
...
static FORCEINLINE void RGB24PackedToPlanarHelper_SSE2(const __m128i* const in /*in[6]*/, __m128i* const out /*out[6]*/) {
...
static FORCEINLINE void RGB24PackedToPlanar_SSE2(const uint8* const rgb, __m128i* const out /*out[6]*/) {
	__m128i tmp[6];
...
static FORCEINLINE void RGB32PackedToPlanar_SSE2(const uint32_t* const argb,
...
static FORCEINLINE void ConvertRGBToY_SSE2(const __m128i* const R,
    const __m128i* const G,
    const __m128i* const B,
    __m128i* const Y) {
...
static FORCEINLINE void ConvertRGBToUV_SSE2(const __m128i* const R,
    const __m128i* const G,
    const __m128i* const B,
    __m128i* const U,
    __m128i* const V) {
...
static void ConvertRGB24ToY_SSE2(const uint8* rgb, uint8* y, int width) {
...
static void ConvertBGR24ToY_SSE2(const uint8* bgr, uint8* y, int width) {
	const int max_width = width & ~31;
	int i;
...
static void ConvertARGBToY_SSE2(const uint32_t* argb, uint8* y, int width) {
...
static FORCEINLINE void RGBA32PackedToPlanar_16b_SSE2(const uint16_t* const rgbx,
