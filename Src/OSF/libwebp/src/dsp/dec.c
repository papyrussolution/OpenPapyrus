...
static void TransformUV_C(const int16_t* in, uint8* dst) {
	VP8Transform(in + 0 * 16, dst, 1);
	VP8Transform(in + 2 * 16, dst + 4 * BPS, 1);
}

#if !WEBP_NEON_OMIT_C_CODE
static void TransformDC_C(const int16_t* in, uint8* dst) {
	const int DC = in[0] + 4;
	int i, j;
	for(j = 0; j < 4; ++j) {
		for(i = 0; i < 4; ++i) {
...
static void TransformDCUV_C(const int16_t* in, uint8* dst) {
...
#if !WEBP_NEON_OMIT_C_CODE
static FORCEINLINE void TrueMotion(uint8* dst, int size) {
...
static void TM4_C(uint8* dst)   {
	TrueMotion(dst, 4);
}

static void TM8uv_C(uint8* dst) {
	TrueMotion(dst, 8);
}

static void TM16_C(uint8* dst)  {
	TrueMotion(dst, 16);
}
//
// 16x16
//
static void VE16_C(uint8* dst)  // vertical
{     
	for(int j = 0; j < 16; ++j) {
...
static void HE16_C(uint8* dst) // horizontal
{
	for(int j = 16; j > 0; --j) {
		memset(dst, dst[-1], 16);
		dst += BPS;
...
static FORCEINLINE void Put16(int v, uint8* dst) 
{
	for(int j = 0; j < 16; ++j) {
...
static void DC16_C(uint8* dst) // DC
{
	int DC = 16;
	for(int j = 0; j < 16; ++j) {
...
static void DC16NoTop_C(uint8* dst) // DC with top samples not available
{
	int DC = 8;
	for(int j = 0; j < 16; ++j) {
		DC += dst[-1 + j * BPS];
...
static void DC16NoLeft_C(uint8* dst) // DC with left samples not available
{
	int DC = 8;
	for(int i = 0; i < 16; ++i) {
		DC += dst[i - BPS];
...
static void DC16NoTopLeft_C(uint8* dst) // DC with no top and left samples
{
	Put16(0x80, dst);
}
...
#if !WEBP_NEON_OMIT_C_CODE
static void VE4_C(uint8* dst) // vertical
{
	const uint8* top = dst - BPS;
	const uint8 vals[4] = {
...
static void HE4_C(uint8* dst) // horizontal
{
...
#if !WEBP_NEON_OMIT_C_CODE
static void DC4_C(uint8* dst) {   // DC
	uint32_t dc = 4;
	int i;
...
static void RD4_C(uint8* dst) {   // Down-right
...
static void LD4_C(uint8* dst) {   // Down-Left
	const int A = dst[0 - BPS];
	const int B = dst[1 - BPS];
	const int C = dst[2 - BPS];
	const int D = dst[3 - BPS];
...
static void VR4_C(uint8* dst) {   // Vertical-Right
	const int I = dst[-1 + 0 * BPS];
	const int J = dst[-1 + 1 * BPS];
	const int K = dst[-1 + 2 * BPS];
	const int X = dst[-1 - BPS];
...
static void VL4_C(uint8* dst) {   // Vertical-Left
	const int A = dst[0 - BPS];
	const int B = dst[1 - BPS];
	const int C = dst[2 - BPS];
	const int D = dst[3 - BPS];
...
static void HU4_C(uint8* dst) {   // Horizontal-Up
	const int I = dst[-1 + 0 * BPS];
	const int J = dst[-1 + 1 * BPS];
	const int K = dst[-1 + 2 * BPS];
	const int L = dst[-1 + 3 * BPS];
...
static void HD4_C(uint8* dst) {  // Horizontal-Down
	const int I = dst[-1 + 0 * BPS];
	const int J = dst[-1 + 1 * BPS];
	const int K = dst[-1 + 2 * BPS];
	const int L = dst[-1 + 3 * BPS];
...
//
#if !WEBP_NEON_OMIT_C_CODE
static void VE8uv_C(uint8* dst) {    // vertical
	int j;
	for(j = 0; j < 8; ++j) {
...
static void HE8uv_C(uint8* dst) {    // horizontal
	int j;
	for(j = 0; j < 8; ++j) {
		memset(dst, dst[-1], 8);
		dst += BPS;
...
static FORCEINLINE void Put8x8uv(uint8 value, uint8* dst) {
	int j;
	for(j = 0; j < 8; ++j) {
...
static void DC8uv_C(uint8* dst) {     // DC
	int dc0 = 8;
	int i;
	for(i = 0; i < 8; ++i) {
...
static void DC8uvNoLeft_C(uint8* dst) {   // DC with no left samples
	int dc0 = 4;
	int i;
	for(i = 0; i < 8; ++i) {
		dc0 += dst[i - BPS];
...
static void DC8uvNoTop_C(uint8* dst) {  // DC with no top samples
	int dc0 = 4;
	int i;
	for(i = 0; i < 8; ++i) {
		dc0 += dst[-1 + i * BPS];
...
static void DC8uvNoTopLeft_C(uint8* dst) {    // DC with nothing
	Put8x8uv(0x80, dst);
}
...
static FORCEINLINE void DoFilter2_C(uint8* p, int step) 
{
...
static FORCEINLINE void DoFilter4_C(uint8* p, int step) 
{
	const int p1 = p[-2*step], p0 = p[-step], q0 = p[0], q1 = p[step];
	const int a = 3 * (q0 - p0);
...
static FORCEINLINE void DoFilter6_C(uint8* p, int step) {
...
static FORCEINLINE int Hev(const uint8* p, int step, int thresh) {
	const int p1 = p[-2*step], p0 = p[-step], q0 = p[0], q1 = p[step];
...
#if !WEBP_NEON_OMIT_C_CODE
static FORCEINLINE int NeedsFilter_C(const uint8* p, int step, int t) {
	const int p1 = p[-2 * step], p0 = p[-step], q0 = p[0], q1 = p[step];
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static FORCEINLINE int NeedsFilter2_C(const uint8* p,
...
//
#if !WEBP_NEON_OMIT_C_CODE
static void SimpleVFilter16_C(uint8* p, int stride, int thresh) {
	int i;
...
static void SimpleHFilter16_C(uint8* p, int stride, int thresh) {
	int i;
	const int thresh2 = 2 * thresh + 1;
	for(i = 0; i < 16; ++i) {
...
static void SimpleVFilter16i_C(uint8* p, int stride, int thresh) {
	int k;
	for(k = 3; k > 0; --k) {
		p += 4 * stride;
...
static void SimpleHFilter16i_C(uint8* p, int stride, int thresh) {
	int k;
	for(k = 3; k > 0; --k) {
		p += 4;
...
//
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static FORCEINLINE void FilterLoop26_C(uint8* p,
...
static FORCEINLINE void FilterLoop24_C(uint8* p,
    int hstride, int vstride, int size,
    int thresh, int ithresh,
    int hev_thresh) {
	const int thresh2 = 2 * thresh + 1;
...
#if !WEBP_NEON_OMIT_C_CODE
// on macroblock edges
static void VFilter16_C(uint8* p, int stride,
    int thresh, int ithresh, int hev_thresh) {
	FilterLoop26_C(p, stride, 1, 16, thresh, ithresh, hev_thresh);
}

static void HFilter16_C(uint8* p, int stride,
    int thresh, int ithresh, int hev_thresh) {
	FilterLoop26_C(p, 1, stride, 16, thresh, ithresh, hev_thresh);
}

// on three inner edges
static void VFilter16i_C(uint8* p, int stride,
    int thresh, int ithresh, int hev_thresh) {
	int k;
	for(k = 3; k > 0; --k) {
		p += 4 * stride;
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static void HFilter16i_C(uint8* p, int stride,
    int thresh, int ithresh, int hev_thresh) {
	int k;
	for(k = 3; k > 0; --k) {
		p += 4;
...
static void VFilter8_C(uint8* u, uint8* v, int stride,
    int thresh, int ithresh, int hev_thresh) {
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static void HFilter8_C(uint8* u, uint8* v, int stride,
    int thresh, int ithresh, int hev_thresh) {
...
#if !WEBP_NEON_OMIT_C_CODE
static void VFilter8i_C(uint8* u, uint8* v, int stride,
    int thresh, int ithresh, int hev_thresh) {
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static void HFilter8i_C(uint8* u, uint8* v, int stride,
    int thresh, int ithresh, int hev_thresh) {
...
static void DitherCombine8x8_C(const uint8* dither, uint8* dst, int dst_stride) 
{
	int i, j;
	for(j = 0; j < 8; ++j) {
		for(i = 0; i < 8; ++i) {
...
void (* VP8DitherCombine8x8)(const uint8* dither, uint8* dst,
    int dst_stride);
