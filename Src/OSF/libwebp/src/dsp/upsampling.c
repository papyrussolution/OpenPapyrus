...
#else
static void EmptyUpsampleFunc(const uint8* top_y, const uint8* bottom_y,
    const uint8* top_u, const uint8* top_v,
    const uint8* cur_u, const uint8* cur_v,
    uint8* top_dst, uint8* bottom_dst, int len) {
	(void)top_y;
...
#define DUAL_SAMPLE_FUNC(FUNC_NAME, FUNC)                                      \
	static void FUNC_NAME(const uint8* top_y, const uint8* bot_y,              \
	    const uint8* top_u, const uint8* top_v,              \
	    const uint8* bot_u, const uint8* bot_v,              \
	    uint8* top_dst, uint8* bot_dst, int len) {           \
...
#define YUV444_FUNC(FUNC_NAME, FUNC, XSTEP)                                    \
	extern void FUNC_NAME(const uint8* y, const uint8* u, const uint8* v,    \
	    uint8* dst, int len);                                  \
	void FUNC_NAME(const uint8* y, const uint8* u, const uint8* v,           \
	    uint8* dst, int len) {                                        \
		int i;                                                                       \
...
#else
static void EmptyYuv444Func(const uint8* y,
