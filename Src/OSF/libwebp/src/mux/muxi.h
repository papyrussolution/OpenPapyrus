...
static FORCEINLINE size_t SizeWithPadding(size_t chunk_size) {
	assert(chunk_size <= MAX_CHUNK_PAYLOAD);
	return CHUNK_HEADER_SIZE + ((chunk_size + 1) & ~1U);
}

// Size of a chunk including header and padding.
static FORCEINLINE size_t ChunkDiskSize(const WebPChunk* chunk) {
...
static FORCEINLINE int IsWPI(WebPChunkId id) {
	switch(id) {
