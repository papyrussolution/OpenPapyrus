...
static FORCEINLINE int ComparePixelsLossless(const uint32_t* src, int src_step,
...
// 'max_allowed_diff'.
static FORCEINLINE int PixelsAreSimilar(uint32_t src, uint32_t dst, int max_allowed_diff) 
{
	...
	return (src_a == dst_a) && (abs(src_r - dst_r) * dst_a <= (max_allowed_diff * 255)) &&
	       (abs(src_g - dst_g) * dst_a <= (max_allowed_diff * 255)) && (abs(src_b - dst_b) * dst_a <= (max_allowed_diff * 255));
}
...
static FORCEINLINE int ComparePixelsLossy(const uint32_t* src, int src_step, const uint32_t* dst, int dst_step, int length, int max_allowed_diff) 
{
	assert(length > 0);
	while(length-- > 0) {
...
static int IsEmptyRect(const FrameRectangle* const rect) { return (rect->width_ == 0) || (rect->height_ == 0); }
...
static FORCEINLINE void SnapToEvenOffsets(FrameRectangle* const rect) 
{
...
static int GetSubRects(const WebPPicture* const prev_canvas, const WebPPicture* const curr_canvas, int is_key_frame, 
	int is_first_frame, float quality, SubFrameParams* const params) 
{
...
static uint32_t RectArea(const FrameRectangle* const rect) { return (uint32_t)rect->width_ * rect->height_; }
...
static int IsLosslessBlendingPossible(const WebPPicture* const src, const WebPPicture* const dst, const FrameRectangle* const rect) 
{
	...
	assert(rect->x_offset_ + rect->width_ <= dst->width);
	assert(rect->y_offset_ + rect->height_ <= dst->height);
	for(int j = rect->y_offset_; j < rect->y_offset_ + rect->height_; ++j) {
		for(int i = rect->x_offset_; i < rect->x_offset_ + rect->width_; ++i) {
...
static WebPEncodingError EncodeCandidate(WebPPicture* const sub_frame, const FrameRectangle* const rect, const WebPConfig* const encoder_config,
    int use_blending, Candidate* const candidate) 
{
...
{
	return enc ? enc->error_str_ : NULL;
}
