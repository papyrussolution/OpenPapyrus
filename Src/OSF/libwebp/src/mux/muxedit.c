...
static WebPMuxError CreateFrameData(int width, int height, const WebPMuxFrameInfo* const info,
...
static WebPMuxError AddDataToChunkList(const WebPData* const data, int copy_data, uint32_t tag,
...
WebPMuxError WebPMuxAssemble(WebPMux* mux, WebPData* assembled_data) 
{
	...
	memzero(assembled_data, sizeof(*assembled_data));
	if(mux == NULL) {
		return WEBP_MUX_INVALID_ARGUMENT;
	}
	// Finalize mux.
