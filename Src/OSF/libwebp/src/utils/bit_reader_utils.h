...
struct VP8BitReader {
	...
	// read buffer
	const uint8* buf_;  // next byte to be read
	const uint8* buf_end_; // end of read buffer
	const uint8* buf_max_; // max packed-read position on buffer
...
//   static FORCEINLINE int VP8GetSigned(VP8BitReader* const br, int v, ...)
...
typedef struct {
	...
	const uint8* buf_;  // input byte buffer
...
static FORCEINLINE void VP8LSetBitPos(VP8LBitReader* const br, int val) { br->bit_pos_ = val; }
