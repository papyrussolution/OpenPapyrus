...
struct VP8BitWriter {
	...
	uint8* buf_;  // internal buffer. Re-allocated regularly. Not owned.
	size_t pos_;
	size_t max_pos_;
...
static FORCEINLINE uint64_t VP8BitWriterPos(const VP8BitWriter* const bw) {
	const uint64_t nb_bits = 8 + bw->nb_bits_; // bw->nb_bits_ is <= 0, note
	return (bw->pos_ + bw->run_) * 8 + nb_bits;
}

// Returns a pointer to the internal buffer.
static FORCEINLINE uint8* VP8BitWriterBuf(const VP8BitWriter* const bw) {
	return bw->buf_;
}

// Returns the size of the internal buffer.
static FORCEINLINE size_t VP8BitWriterSize(const VP8BitWriter* const bw) {
	return bw->pos_;
}
...
typedef struct {
	...
	uint8*     buf_;// start of buffer
	uint8*     cur_;// current write position
	uint8*     end_;// end of buffer
...
static FORCEINLINE size_t VP8LBitWriterNumBytes(const VP8LBitWriter* const bw) {
...
static FORCEINLINE void VP8LPutBits(VP8LBitWriter* const bw,
