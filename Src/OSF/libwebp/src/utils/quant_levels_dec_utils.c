...
typedef struct {
	...
	uint8* src_;  // input pointer
	uint8* dst_;  // output pointer
...
#if defined(USE_DITHERING)
	const uint8* const dither = kOrderedDither[p->row_ % DSIZE];
#endif
	uint8* const dst = p->dst_;
	int x;
	for(x = 0; x < w; ++x) {
		const int v = dst[x];
...
static int InitParams(uint8* const data, int width, int height, int stride, int radius, SmoothParams* const p) 
{
	...
	uint8* mem = (uint8*)WebPSafeMalloc(1U, total_size);
	...
	p->top_ = p->end_ - width;
	memzero(p->top_, width * sizeof(*p->top_));
	mem += size_scratch_m;
...
int WebPDequantizeLevels(uint8* const data, int width, int height, int stride, int strength) 
{
