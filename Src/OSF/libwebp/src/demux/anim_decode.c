...
struct WebPAnimDecoder {
	...
	uint8* curr_frame_;      // Current canvas (not disposed).
	uint8* prev_frame_disposed_; // Previous canvas (properly disposed).
...
static void ZeroFillFrameRect(uint8* buf, int buf_stride, int x_offset, int y_offset, int width, int height) 
{
	int j;
	assert(width * NUM_CHANNELS <= buf_stride);
	buf += y_offset * buf_stride + x_offset * NUM_CHANNELS;
	for(j = 0; j < height; ++j) {
		memzero(buf, width * NUM_CHANNELS);
		buf += buf_stride;
	}
}
...
static uint32_t FASTCALL BlendPixelPremult(uint32_t src, uint32_t dst) 
{
	const uint8 src_a = (src >> 24) & 0xff;
...
static void FindBlendRangeAtRow(const WebPIterator* const src, const WebPIterator* const dst, int canvas_y,
    int* const left1, int* const width1, int* const left2, int* const width2) 
{
...
void WebPAnimDecoderReset(WebPAnimDecoder* dec) 
{
	if(dec != NULL) {
		...
		WebPDemuxReleaseIterator(&dec->prev_iter_);
		memzero(&dec->prev_iter_, sizeof(dec->prev_iter_));
