...
static int InitMemBuffer(MemBuffer* const mem, const uint8* data, size_t size) 
{
...
static FORCEINLINE size_t MemDataSize(const MemBuffer* const mem) {
	return (mem->end_ - mem->start_);
}

// Return true if 'size' exceeds the end of the RIFF chunk.
static FORCEINLINE int SizeIsInvalid(const MemBuffer* const mem, size_t size) {
	return (size > mem->riff_end_ - mem->start_);
}

static FORCEINLINE void Skip(MemBuffer* const mem, size_t size) {
	mem->start_ += size;
}

static FORCEINLINE void Rewind(MemBuffer* const mem, size_t size) {
	mem->start_ -= size;
}

static FORCEINLINE const uint8* GetBuffer(MemBuffer* const mem) {
	return mem->buf_ + mem->start_;
}

// Read from 'mem' and skip the read bytes.
static FORCEINLINE uint8 ReadByte(MemBuffer* const mem) {
	const uint8 byte = mem->buf_[mem->start_];
	Skip(mem, 1);
	return byte;
}

static FORCEINLINE int ReadLE16s(MemBuffer* const mem) {
	const uint8* const data = mem->buf_ + mem->start_;
	const int val = GetLE16(data);
	Skip(mem, 2);
	return val;
}

static FORCEINLINE int ReadLE24s(MemBuffer* const mem) {
	const uint8* const data = mem->buf_ + mem->start_;
...
static FORCEINLINE uint32_t ReadLE32(MemBuffer* const mem) {
	const uint8* const data = mem->buf_ + mem->start_;
...
static int AddFrame(WebPDemuxer* const dmux, Frame* const frame) 
{
	...
	if(last_frame && !last_frame->complete_) return 0;
...
static ParseStatus ParseSingleImage(WebPDemuxer* const dmux) {
	...
	if(dmux->frames_) return PARSE_ERROR;
	if(SizeIsInvalid(mem, min_size)) return PARSE_ERROR;
	if(MemDataSize(mem) < min_size) return PARSE_NEED_MORE_DATA;
...
static int IsValidExtendedFormat(const WebPDemuxer* const dmux) {
	...
	while(f) {
		...
		for(; f && f->frame_num_ == cur_frame_set; f = f->next_) {
...
{
	const Frame* f;
	for(f = dmux->frames_; f; f = f->next_) {
		if(frame_num == f->frame_num_) break;
	}
	return f;
}

static const uint8* GetFramePayload(const uint8* const mem_buf, const Frame* const frame, size_t * const data_size) 
{
	*data_size = 0;
	if(frame) {
...
{
	const uint8* const mem_buf = dmux->mem_.buf_;
...
static int ChunkCount(const WebPDemuxer* const dmux, const char fourcc[4]) 
{
	...
	const Chunk* c;
	int count = 0;
	for(c = dmux->chunks_; c; c = c->next_) {
		const uint8* const header = mem_buf + c->data_.offset_;
