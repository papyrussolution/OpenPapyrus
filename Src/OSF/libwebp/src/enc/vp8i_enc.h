...
// discarding bits.
static FORCEINLINE int QUANTDIV(uint32_t n, uint32_t iQ, uint32_t B) {
...
typedef struct {
	uint8 segments_[3]; // probabilities for segment tree
	uint8 skip_proba_; // final probability of being skipped.
...
typedef struct {
	...
	uint8 alpha_; // quantization-susceptibility
} VP8MBInfo;
...
typedef struct {
	...
	uint8*      yuv_in_;     // input samples
	uint8*      yuv_out_;    // output samples
	uint8*      yuv_out2_;   // secondary buffer swapped with yuv_out_.
	uint8*      yuv_p_;      // scratch buffer for prediction
	VP8Encoder*   enc_;        // back-pointer
	VP8MBInfo*    mb_;         // current macroblock
	VP8BitWriter* bw_;         // current bit-writer
	uint8*      preds_;      // intra mode predictors (4x4 blocks)
	uint32_t*     nz_;         // non-zero pattern
	uint8 i4_boundary_[37];  // 32+5 boundary samples needed by intra4x4
	uint8*      i4_top_;     // pointer to the current top boundary sample
	...
	uint8* y_left_; // left luma samples (addressable from index -1 to 15).
	uint8* u_left_; // left u samples (addressable from index -1 to 7)
	uint8* v_left_; // left v samples (addressable from index -1 to 7)

	uint8* y_top_; // top luma samples at position 'x_'
	uint8* uv_top_; // top u/v samples at position 'x_', packed as 16 bytes

	// memory for storing y/u/v_left_
	uint8 yuv_left_mem_[17 + 16 + 16 + 8 + WEBP_ALIGN_CST];
	// memory for yuv_*
	uint8 yuv_mem_[3 * YUV_SIZE_ENC + PRED_SIZE_ENC + WEBP_ALIGN_CST];
} VP8EncIterator;
...
struct VP8Encoder {
	...
	// transparency blob
	int has_alpha_;
	uint8* alpha_data_; // non-NULL if transparency is present
	...
	uint8*   y_top_;// top luma samples.
	uint8*   uv_top_;// top u/v samples.
