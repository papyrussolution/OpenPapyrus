...
void VP8LHistogramInit(VP8LHistogram* const p, int palette_code_bits,
	...
	if(init_arrays) {
		...
		p->red_cost_ = 0.;
		p->blue_cost_ = 0.;
		memzero(p->is_used_, sizeof(p->is_used_));
	}
}
...
void VP8LHistogramSetClear(VP8LHistogramSet* const set) 
{
	...
	const size_t total_size = HistogramSetTotalSize(size, cache_bits);
	uint8* memory = (uint8*)set;
	memzero(memory, total_size);
	memory += sizeof(*set);
	set->histograms = (VP8LHistogram**)memory;
	set->max_size = size;
	set->size = size;
...
static void HistogramSetRemoveHistogram(VP8LHistogramSet* const set, int i, int* const num_used) 
{
...
static FORCEINLINE double BitsEntropyRefine(const VP8LBitEntropy* entropy) {
	double mix;
...
static FORCEINLINE double GetCombinedEntropy(const uint32_t* const X,
...
static FORCEINLINE void HistogramAdd(const VP8LHistogram* const a,
    const VP8LHistogram* const b,
...
static void HistogramBuild(int xsize, int histo_bits, const VP8LBackwardRefs* const backward_refs,
...
static void OptimizeHistogramSymbols(const VP8LHistogramSet* const set,
	...
	cluster_max = 0;
	memzero(cluster_mappings_tmp, set->max_size * sizeof(*cluster_mappings_tmp));
...
int VP8LGetHistoImageSymbols(int xsize, int ysize,
	...
	int entropy_combine;
	uint16_t* const map_tmp = (uint16_t*)WebPSafeMalloc(2 * image_histo_raw_size, sizeof(map_tmp));
