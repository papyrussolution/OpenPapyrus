...
static FORCEINLINE int GetMax(int a, int b) {
	return (a < b) ? b : a;
}
...
static FORCEINLINE uint8 NearLosslessDiff(uint8 a, uint8 b) {
...
// predictions.
static FORCEINLINE void GetResidual(int width, int height, uint32_t* const upper_row,
    uint32_t* const current_row, const uint8* const max_diffs, int mode,
...
// the given pixel).
static int GetBestPredictorForTile(int width, int height, int tile_x, int tile_y, int bits, int accumulated[4][256],
    uint32_t* const argb_scratch, const uint32_t* const argb, int max_quantization,
    int exact, int used_subtract_green, const uint32_t* const modes) 
{
	...
	for(mode = 0; mode < kNumPredModes; ++mode) {
		float cur_diff;
		int relative_y;
		memzero(histo_argb, sizeof(histo_stack_1));
		if(start_y > 0) {
...
static FORCEINLINE void ColorCodeToMultipliers(uint32_t color_code, VP8LMultipliers* const m) 
{
...
static FORCEINLINE uint32_t MultipliersToColorCode(const VP8LMultipliers* const m) 
{
...
static float GetPredictionCostCrossColorRed(const uint32_t* argb, int stride, int tile_width, int tile_height,
	...
	if((uint8)green_to_red == prev_x.green_to_red_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if((uint8)green_to_red == prev_y.green_to_red_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if(green_to_red == 0) {
		cur_diff -= 3;
...
static void GetBestGreenToRed(const uint32_t* argb, int stride, int tile_width, int tile_height,
...
static float GetPredictionCostCrossColorBlue(const uint32_t* argb, int stride, int tile_width, int tile_height,
	...
	if((uint8)green_to_blue == prev_x.green_to_blue_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if((uint8)green_to_blue == prev_y.green_to_blue_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if((uint8)red_to_blue == prev_x.red_to_blue_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if((uint8)red_to_blue == prev_y.red_to_blue_) {
		cur_diff -= 3; // favor keeping the areas locally similar
	}
	if(green_to_blue == 0) {
		cur_diff -= 3;
...
static void GetBestGreenRedToBlue(const uint32_t* argb, int stride, int tile_width, int tile_height,
    VP8LMultipliers prev_x, VP8LMultipliers prev_y, int quality,
