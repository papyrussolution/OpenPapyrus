...
static FORCEINLINE int FindMatchLength(const uint32_t* const array1,
...
static PixOrCopyBlock* BackwardRefsNewBlock(VP8LBackwardRefs* const refs) {
	...
	if(!b) { // allocate new memory chunk
		...
		b->start_ = (PixOrCopy*)((uint8*)b + sizeof(*b)); // not always aligned
	}
...
static FORCEINLINE int MaxFindCopyLength(int len) {
...
int VP8LHashChainFill(VP8LHashChain* const p, int quality,
	...
	hash_to_first_index = (int32_t*)WebPSafeMalloc(HASH_SIZE, sizeof(*hash_to_first_index));
...
static FORCEINLINE void AddSingleLiteral(uint32_t pixel, int use_color_cache,
...
static VP8LBackwardRefs* GetBackwardReferencesLowEffort(int width, int height, const uint32_t* const argb,
...
extern int VP8LBackwardReferencesTraceBackwards(int xsize, int ysize, const uint32_t* const argb, int cache_bits,
    const VP8LHashChain* const hash_chain,
    const VP8LBackwardRefs* const refs_src, VP8LBackwardRefs* const refs_dst);

static int GetBackwardReferences(int width, int height, const uint32_t* const argb, int quality, int lz77_types_to_try, int cache_bits_max,
    int do_no_cache, const VP8LHashChain* const hash_chain, VP8LBackwardRefs* const refs, int* const cache_bits_best) 
{
	...
	int status = 0;
	memzero(&hash_chain_box, sizeof(hash_chain_box));
	histo = VP8LAllocateHistogram(MAX_COLOR_CACHE_BITS);
	if(histo == NULL) 
		goto Error;
	for(lz77_type = 1; lz77_types_to_try; lz77_types_to_try &= ~lz77_type, lz77_type <<= 1) {
		int res = 0;
		double bit_cost = 0.;
...
WebPEncodingError VP8LGetBackwardReferences(int width, int height, const uint32_t* const argb, int quality,
