...
static void ConvertPopulationCountTableToBitEstimates(int num_symbols, const uint32_t population_counts[], double output[]) 
{
	...
	if(nonzeros <= 1) {
		memzero(output, num_symbols * sizeof(*output));
	}
	else {
...
static FORCEINLINE double GetLiteralCost(const CostModel* const m, uint32_t v) {
...
static FORCEINLINE double GetCacheCost(const CostModel* const m, uint32_t idx) {
	const int literal_idx = VALUES_IN_BYTE + NUM_LENGTH_CODES + idx;
	return m->literal_[literal_idx];
}

static FORCEINLINE double GetLengthCost(const CostModel* const m,
    uint32_t length) {
	int code, extra_bits;
...
static FORCEINLINE double GetDistanceCost(const CostModel* const m,
    uint32_t distance) {
	int code, extra_bits;
...
static FORCEINLINE void AddSingleLiteralWithCostModel(const uint32_t* const argb, VP8LColorCache* const hashers,
...
static FORCEINLINE void UpdateCostPerInterval(CostManager* const manager,
...
static FORCEINLINE void PopInterval(CostManager* const manager,
    CostInterval* const interval) {
...
static FORCEINLINE void InsertInterval(CostManager* const manager,
...
static FORCEINLINE void PushInterval(CostManager* const manager,
...
static int BackwardReferencesHashChainDistanceOnly(int xsize, int ysize, const uint32_t* const argb, int cache_bits,
...
static int BackwardReferencesHashChainFollowChosenPath(const uint32_t* const argb, int cache_bits,
