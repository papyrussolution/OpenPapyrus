...
static uint32_t GetFilterMap(const uint8* alpha, int width, int height, int filter, int effort_level) 
{
	uint32_t bit_map = 0U;
	if(filter == WEBP_FILTER_FAST) {
		...
		filter = (num_colors <= kMinColorsForFilterNone) ? WEBP_FILTER_NONE : WebPEstimateBestFilter(alpha, width, height, width);
		bit_map |= 1 << filter;
...
static int ApplyFiltersAndEncode(const uint8* alpha, int width, int height, size_t data_size, int method, int filter,
    int reduce_levels, int effort_level, uint8** const output, size_t* const output_size, WebPAuxStats* const stats) 
{
	int ok = 1;
	FilterTrial best;
...
static int EncodeAlpha(VP8Encoder* const enc,
    ...
    int effort_level,
    uint8** const output, size_t* const output_size) {
