...
static FORCEINLINE void SwapColor(uint32_t* const col1, uint32_t* const col2) {
...
static void CoOccurrenceFindMax(const uint32_t* const cooccurrence,
    uint32_t num_colors, uint8* const c1,
    uint8* const c2) {
...
// Methods for Improving the Compression of Color-Indexed Images" by Armando J.
// Pinho and Antonio J. R. Neves.
static WebPEncodingError PaletteSortModifiedZeng(const WebPPicture* const pic, const uint32_t* const palette_sorted,
...
End:
	...
	if(!ok) {
		...
		memzero(huffman_codes, 5 * histogram_image_size * sizeof(*huffman_codes));
	}
	return ok;
}

static void StoreHuffmanTreeOfHuffmanTreeToBitMask(VP8LBitWriter* const bw, const uint8* code_length_bitdepth) {
...
static FORCEINLINE void WriteHuffmanCode(VP8LBitWriter* const bw,
...
static FORCEINLINE void WriteHuffmanCodeWithExtraBits(VP8LBitWriter* const bw,
    const HuffmanTreeCode* const code,
    int code_index,
    int bits,
    int n_bits) {
...
static WebPEncodingError StoreImageToBitMask(VP8LBitWriter* const bw, int width, int histo_bits,
...
static WebPEncodingError EncodeImageNoHuffman(VP8LBitWriter* const bw, const uint32_t* const argb,
...
static WebPEncodingError EncodeImageInternal(VP8LBitWriter* const bw, const uint32_t* const argb,
	...
	memzero(&hash_chain_histogram, sizeof(hash_chain_histogram));
	if(huff_tree == NULL || histogram_symbols == NULL || !VP8LHashChainInit(&hash_chain_histogram, histogram_image_xysize) || 
		!VP8LHashChainFill(hash_chain, quality, argb, width, height, low_effort)) {
		goto Error;
	}
	if(use_cache) {
...
static FORCEINLINE uint32_t SearchColorGreedy(const uint32_t palette[],
    int palette_size,
    uint32_t color) {
	(void)palette_size;
...
static FORCEINLINE uint32_t ApplyPaletteHash0(uint32_t color) {
...
static FORCEINLINE uint32_t ApplyPaletteHash1(uint32_t color) {
	// Forget about alpha.
	return ((uint32_t)((color & 0x00ffffffu) * 4222244071ull)) >>
	       (32 - PALETTE_INV_SIZE_BITS);
}

static FORCEINLINE uint32_t ApplyPaletteHash2(uint32_t color) {
	// Forget about alpha.
...
UserAbort:
	...
	if(picture->extra_info != NULL) {
		...
		memzero(picture->extra_info, mb_w * mb_h * sizeof(*picture->extra_info));
	}
Error:
	if(bw.error_) 
		err = VP8_ENC_ERROR_OUT_OF_MEMORY;
