...
static void DoFilter(const VP8EncIterator* const it, int level) {
	...
	uint8* const y_dst = it->yuv_out2_ + Y_OFF_ENC;
	uint8* const u_dst = it->yuv_out2_ + U_OFF_ENC;
	uint8* const v_dst = it->yuv_out2_ + V_OFF_ENC;

	// copy current block to yuv_out2_
	memcpy(y_dst, it->yuv_out_, YUV_SIZE_ENC * sizeof(uint8));
