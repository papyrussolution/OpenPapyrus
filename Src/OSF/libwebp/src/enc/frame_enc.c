...
{
	VP8EncProba* const proba = &enc->proba_;
	memzero(proba->stats_, sizeof(proba->stats_));
}
...
static void SetSegmentProbas(VP8Encoder* const enc) {
	...
	if(enc->segment_hdr_.num_segments_ > 1) {
		uint8* const probas = enc->proba_.segments_;
...
#if SEGMENT_VISU
	static void SetBlock(uint8* p, int value, int size) 
	{
...
{
	VP8Encoder* const enc = it->enc_;
	const uint8* const in = it->yuv_in_;
	const uint8* const out = it->yuv_out_;
...
static void StoreSideInfo(const VP8EncIterator* const it) {
	...
	if(pic->extra_info != NULL) {
		uint8* const info = &pic->extra_info[it->x_ + it->y_ * enc->mb_w_];
...
static void ResetSideInfo(const VP8EncIterator* const it) 
{
	...
	WebPPicture* const pic = enc->pic_;
	if(pic->stats != NULL) {
		memzero(enc->block_count_, sizeof(enc->block_count_));
	}
	ResetSSE(enc);
}
...
static void StoreSideInfo(const VP8EncIterator* const it) 
{
	...
	if(pic->extra_info != NULL) {
		if(it->x_ == 0 && it->y_ == 0) { // only do it once, at start
			memzero(pic->extra_info, enc->mb_w_ * enc->mb_h_ * sizeof(*pic->extra_info));
		}
	}
}
