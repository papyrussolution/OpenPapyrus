...
static void PrintBlockInfo(const VP8EncIterator* const it,
	...
	const uint8* const y_out = it->yuv_out_ + Y_OFF_ENC;
	const uint8* const uv_in = it->yuv_in_ + U_OFF_ENC;
	const uint8* const uv_out = it->yuv_out_ + U_OFF_ENC;
...
static const uint8 kFreqSharpening[16] = {
	0,  30, 60, 90,
	30, 60, 90, 90,
	60, 90, 90, 90,
	90, 90, 90, 90
...
static FORCEINLINE void SetRDScore(int lambda, VP8ModeScore* const rd) {
	rd->score = (rd->R + rd->H) * lambda + RD_DISTO_MULT * (rd->D + rd->SD);
}

static FORCEINLINE score_t RDScoreTrellis(int lambda, score_t rate,
    score_t distortion) {
...
static int TrellisQuantizeBlock(const VP8Encoder* const enc,
	...
	if(coeff_type == TYPE_I16_AC) {
		memzero(in + 1, 15 * sizeof(*in));
		memzero(out + 1, 15 * sizeof(*out));
	}
	else {
		memzero(in, 16 * sizeof(*in));
		memzero(out, 16 * sizeof(*out));
	}
	if(best_path[0] == -1) {
		return 0; // skip!
	}
...
static int ReconstructIntra4(VP8EncIterator* const it, int16_t levels[16], const uint8* const src, uint8* const yuv_out, int mode) 
{
	const VP8Encoder* const enc = it->enc_;
	const uint8* const ref = it->yuv_p_ + VP8I4ModeOffsets[mode];
	const VP8SegmentInfo* const dqm = &enc->dqm_[it->mb_->segment_];
	int nz = 0;
	int16_t tmp[16];
...
static void CorrectDCValues(const VP8EncIterator* const it, const VP8Matrix* const mtx, int16_t tmp[][16], VP8ModeScore* const rd) 
{
	//         | top[0] | top[1]
	// --------+--------+---------
...
static void SwapPtr(uint8** a, uint8** b) {
	uint8* const tmp = *a;
	*a = *b;
	*b = tmp;
}
...
static void PickBestIntra16(VP8EncIterator* const it, VP8ModeScore* rd) {
	...
	for(mode = 0; mode < NUM_PRED_MODES; ++mode) {
		uint8* const tmp_dst = it->yuv_out2_ + Y_OFF_ENC; // scratch buffer
...
static int PickBestIntra4(VP8EncIterator* const it, VP8ModeScore* const rd) {
	...
	const int tlambda = dqm->tlambda_;
	const uint8* const src0 = it->yuv_in_ + Y_OFF_ENC;
	uint8* const best_blocks = it->yuv_out2_ + Y_OFF_ENC;
	...
	do {
		...
		int mode;
		int best_mode = -1;
		const uint8* const src = src0 + VP8Scan[it->i4_];
		const uint16_t* const mode_costs = GetCostModeI4(it, rd->modes_i4);
		uint8* best_block = best_blocks + VP8Scan[it->i4_];
		uint8* tmp_dst = it->yuv_p_ + I4TMP; // scratch buffer.
...
static void PickBestUV(VP8EncIterator* const it, VP8ModeScore* const rd) {
	...
	const uint8* const src = it->yuv_in_ + U_OFF_ENC;
	uint8* tmp_dst = it->yuv_out2_ + U_OFF_ENC; // scratch buffer
	uint8* dst0 = it->yuv_out_ + U_OFF_ENC;
	uint8* dst = dst0;
	VP8ModeScore rd_best;
	int mode;
...
static void SimpleQuantize(VP8EncIterator* const it, VP8ModeScore* const rd) {
	...
	if(is_i16) {
		...
		do {
			...
			const uint8* const src = it->yuv_in_ + Y_OFF_ENC + VP8Scan[it->i4_];
			uint8* const dst = it->yuv_out_ + Y_OFF_ENC + VP8Scan[it->i4_];
			VP8MakeIntra4Preds(it);
	...
	if(try_both_modes || !is_i16) {
		...
		do {
			...
			if(score_i4 >= best_score || i4_bit_sum > bit_limit) {
				...
				uint8* const tmp_dst = it->yuv_out2_ + Y_OFF_ENC + VP8Scan[it->i4_];
				nz |= ReconstructIntra4(it, rd->y_ac_levels[it->i4_],
