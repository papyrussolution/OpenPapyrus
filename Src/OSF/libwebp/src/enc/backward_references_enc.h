...
static FORCEINLINE PixOrCopy PixOrCopyCreateCopy(uint32_t distance,
    uint16_t len) {
	PixOrCopy retval;
	retval.mode = kCopy;
...
static FORCEINLINE PixOrCopy PixOrCopyCreateCacheIdx(int idx) {
	PixOrCopy retval;
	assert(idx >= 0);
...
static FORCEINLINE PixOrCopy PixOrCopyCreateLiteral(uint32_t argb) {
	PixOrCopy retval;
...
static FORCEINLINE int PixOrCopyIsLiteral(const PixOrCopy* const p) {
	return (p->mode == kLiteral);
}

static FORCEINLINE int PixOrCopyIsCacheIdx(const PixOrCopy* const p) {
	return (p->mode == kCacheIdx);
}

static FORCEINLINE int PixOrCopyIsCopy(const PixOrCopy* const p) {
	return (p->mode == kCopy);
}

static FORCEINLINE uint32_t PixOrCopyLiteral(const PixOrCopy* const p,
    int component) {
	assert(p->mode == kLiteral);
	return (p->argb_or_distance >> (component * 8)) & 0xff;
}

static FORCEINLINE uint32_t PixOrCopyLength(const PixOrCopy* const p) {
	return p->len;
}

static FORCEINLINE uint32_t PixOrCopyCacheIdx(const PixOrCopy* const p) {
	assert(p->mode == kCacheIdx);
	assert(p->argb_or_distance < (1U << MAX_COLOR_CACHE_BITS));
	return p->argb_or_distance;
}

static FORCEINLINE uint32_t PixOrCopyDistance(const PixOrCopy* const p) {
	assert(p->mode == kCopy);
	return p->argb_or_distance;
}
...
static FORCEINLINE int VP8LHashChainFindOffset(const VP8LHashChain* const p,
    const int base_position) {
	return p->offset_length_[base_position] >> MAX_LENGTH_BITS;
}

static FORCEINLINE int VP8LHashChainFindLength(const VP8LHashChain* const p,
    const int base_position) {
	return p->offset_length_[base_position] & ((1U << MAX_LENGTH_BITS) - 1);
}

static FORCEINLINE void VP8LHashChainFindCopy(const VP8LHashChain* const p,
    int base_position,
...
static FORCEINLINE int VP8LRefsCursorOk(const VP8LRefsCursor* const c) {
...
static FORCEINLINE void VP8LRefsCursorNext(VP8LRefsCursor* const c) {
	assert(c != NULL);
