...
static FORCEINLINE WebPMux* WebPMuxNew(void) { return WebPNewInternal(WEBP_MUX_ABI_VERSION); }
...
static FORCEINLINE WebPMux* WebPMuxCreate(const WebPData* bitstream, int copy_data) 
{
...
WEBP_EXTERN WebPMuxError WebPMuxSetChunk(WebPMux* mux, const char fourcc[4], const WebPData* chunk_data,
    int copy_data);
...
struct WebPAnimEncoderOptions {
	...
	int minimize_size; // If true, minimize the output size (slow). Implicitly disables key-frame insertion.
	int kmin;
	...
	int allow_mixed; // If true, use mixed compression mode; may choose either lossy and lossless for each frame.
...
// 'enc_options' object.
static FORCEINLINE int WebPAnimEncoderOptionsInit(WebPAnimEncoderOptions* enc_options) 
{
...
//   Or NULL in case of memory error.
static FORCEINLINE WebPAnimEncoder* WebPAnimEncoderNew(int width, int height, const WebPAnimEncoderOptions* enc_options) 
{
...
WEBP_EXTERN int WebPAnimEncoderAdd(WebPAnimEncoder* enc, struct WebPPicture* frame, int timestamp_ms, const struct WebPConfig* config);
