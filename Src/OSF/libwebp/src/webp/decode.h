...
WEBP_EXTERN int WebPGetInfo(const uint8* data, size_t data_size, int* width, int* height);
...
WEBP_EXTERN uint8 * WebPDecodeRGBA(const uint8* data, size_t data_size, int* width, int* height);

// Same as WebPDecodeRGBA, but returning A, R, G, B, A, R, G, B... ordered data.
WEBP_EXTERN uint8 * WebPDecodeARGB(const uint8* data, size_t data_size, int* width, int* height);

// Same as WebPDecodeRGBA, but returning B, G, R, A, B, G, R, A... ordered data.
WEBP_EXTERN uint8 * WebPDecodeBGRA(const uint8* data, size_t data_size, int* width, int* height);

// Same as WebPDecodeRGBA, but returning R, G, B, R, G, B... ordered data.
// If the bitstream contains transparency, it is ignored.
WEBP_EXTERN uint8 * WebPDecodeRGB(const uint8* data, size_t data_size, int* width, int* height);

// Same as WebPDecodeRGB, but returning B, G, R, B, G, R... ordered data.
WEBP_EXTERN uint8 * WebPDecodeBGR(const uint8* data, size_t data_size, int* width, int* height);
...
static FORCEINLINE int WebPInitDecBuffer(WebPDecBuffer* buffer) { return WebPInitDecBufferInternal(buffer, WEBP_DECODER_ABI_VERSION); }
...
WEBP_EXTERN WebPIDecoder* WebPINewRGB(WEBP_CSP_MODE csp,
    uint8* output_buffer, size_t output_buffer_size, int output_stride);
...
WEBP_EXTERN WebPIDecoder* WebPINewYUVA(uint8* luma, size_t luma_size, int luma_stride,
    uint8* u, size_t u_size, int u_stride,
    uint8* v, size_t v_size, int v_stride,
...
WEBP_EXTERN WebPIDecoder* WebPINewYUV(uint8* luma, size_t luma_size, int luma_stride,
    uint8* u, size_t u_size, int u_stride,
    uint8* v, size_t v_size, int v_stride);
...
WEBP_EXTERN VP8StatusCode WebPIAppend(WebPIDecoder* idec, const uint8* data, size_t data_size);
...
WEBP_EXTERN VP8StatusCode WebPIUpdate(WebPIDecoder* idec, const uint8* data, size_t data_size);
...
// return.
WEBP_EXTERN uint8* WebPIDecGetRGB(const WebPIDecoder* idec, int* last_y,
    int* width, int* height, int* stride);

// Same as above function to get a YUVA image. Returns pointer to the luma
// plane or NULL in case of error. If there is no alpha information
// the alpha pointer '*a' will be returned NULL.
WEBP_EXTERN uint8* WebPIDecGetYUVA(const WebPIDecoder* idec, int* last_y,
    uint8** u, uint8** v, uint8** a,
...
// ALPH + VP8 <-- Not a valid WebP format: only allowed for internal purpose.
// VP8(L)     <-- Not a valid WebP format: only allowed for internal purpose.
static FORCEINLINE VP8StatusCode WebPGetFeatures(const uint8* data, size_t data_size, WebPBitstreamFeatures* features) 
{
...
static FORCEINLINE int WebPInitDecoderConfig(WebPDecoderConfig* config) 
{
...
WEBP_EXTERN WebPIDecoder* WebPIDecode(const uint8* data, size_t data_size, WebPDecoderConfig* config);

// Non-incremental version. This version decodes the full data at once, taking
// 'config' into account. Returns decoding status (which should be VP8_STATUS_OK
// if the decoding was successful). Note that 'config' cannot be NULL.
WEBP_EXTERN VP8StatusCode WebPDecode(const uint8* data, size_t data_size, WebPDecoderConfig* config);
