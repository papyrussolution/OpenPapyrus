...
int ImgIoUtilReadFromStdin(const uint8** data, size_t* data_size) 
{
	...
	*data_size = 0;
	...
	while(!feof(stdin)) {
		...
		void * const new_data = SAlloc::R(input, max_size + extra_size + 1);
		if(new_data == NULL) 
			goto Error;
		input = (uint8*)new_data;
		max_size += extra_size;
...
int ImgIoUtilReadFile(const char* const file_name, const uint8** data, size_t* data_size) 
{
	int ok;
	uint8* file_data;
	size_t file_size;
	...
	*data_size = 0;
	...
	fseek(in, 0, SEEK_SET);
	// we allocate one extra byte for the \0 terminator
	file_data = (uint8*)WebPMalloc(file_size + 1);
	if(file_data == NULL) {
		fclose(in);
...
int ImgIoUtilWriteFile(const char* const file_name, const uint8* data, size_t data_size) 
{
	int ok;
	FILE* out;
...
void ImgIoUtilCopyPlane(const uint8* src, int src_stride, uint8* dst, int dst_stride, int width, int height) 
{
	while(height-- > 0) {
