...
int LoadWebP(const char* const in_file, const uint8** data, size_t* data_size, WebPBitstreamFeatures* bitstream);
...
// Returns true on success.
int ReadWebP(const uint8* const data, size_t data_size, struct WebPPicture* const pic, int keep_alpha, struct Metadata* const metadata);
