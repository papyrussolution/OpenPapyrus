...
static FORCEINLINE uint32_t GetBE32(const uint8 buf[]) 
{
	return ((uint32_t)buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
}

WebPInputFileFormat WebPGuessImageType(const uint8* const data, size_t data_size) 
{
...
static int FailReader(const uint8* const data, size_t data_size, struct WebPPicture* const pic, int keep_alpha, struct Metadata* const metadata) 
{
	(void)data;
	(void)data_size;
	(void)pic;
...
WebPImageReader WebPGuessImageReader(const uint8* const data, size_t data_size) 
{
