...
typedef struct WICFormatImporter {
	...
	int bytes_per_pixel;
	int (*import)(WebPPicture* const, const uint8* const, int);
} WICFormatImporter;

// From Microsoft SDK 7.0a -- wincodec.h
// Create local copies for compatibility when building against earlier
// versions of the SDK.
WEBP_DEFINE_GUID(GUID_WICPixelFormat24bppBGR_, 0x6fddc324, 0x4e03, 0x4bfe, 0xb1, 0x85, 0x3d, 0x77, 0x76, 0x8d, 0xc9, 0x0c);
WEBP_DEFINE_GUID(GUID_WICPixelFormat24bppRGB_, 0x6fddc324, 0x4e03, 0x4bfe, 0xb1, 0x85, 0x3d, 0x77, 0x76, 0x8d, 0xc9, 0x0d);
WEBP_DEFINE_GUID(GUID_WICPixelFormat32bppBGRA_, 0x6fddc324, 0x4e03, 0x4bfe, 0xb1, 0x85, 0x3d, 0x77, 0x76, 0x8d, 0xc9, 0x0f);
WEBP_DEFINE_GUID(GUID_WICPixelFormat32bppRGBA_, 0xf5c7ad2d, 0x6a8d, 0x43dd, 0xa7, 0xa8, 0xa2, 0x99, 0x35, 0x26, 0x1a, 0xe9);
WEBP_DEFINE_GUID(GUID_WICPixelFormat64bppBGRA_, 0x1562ff7c, 0xd352, 0x46f9, 0x97, 0x9e, 0x42, 0x97, 0x6b, 0x79, 0x22, 0x46);
WEBP_DEFINE_GUID(GUID_WICPixelFormat64bppRGBA_, 0x6fddc324, 0x4e03, 0x4bfe, 0xb1, 0x85, 0x3d, 0x77, 0x76, 0x8d, 0xc9, 0x16);
...
static HRESULT ExtractICCP(IWICImagingFactory* const factory, IWICBitmapFrameDecode* const frame, MetadataPayload* const iccp) 
{
	HRESULT hr = S_OK;
	UINT i, count;
		...
		for(i = 0; SUCCEEDED(hr) && i < num_color_contexts; ++i) {
			...
			if(SUCCEEDED(hr) && type == WICColorContextProfile) {
				...
				if(SUCCEEDED(hr) && size > 0) {
					iccp->bytes = (uint8 *)SAlloc::M(size);
					if(iccp->bytes == NULL) {
						hr = E_OUTOFMEMORY;
						break;
					}
	...
	SAlloc::F(color_contexts);
	return hr;
}

static HRESULT ExtractMetadata(IWICImagingFactory* const factory, IWICBitmapFrameDecode* const frame, Metadata* const metadata) 
{
...
static int HasAlpha(IWICImagingFactory* const factory,
	...
	if(HasPalette(pixel_format)) {
		...
		if(SUCCEEDED(IWICImagingFactory_CreatePalette(factory, &global_palette)) && SUCCEEDED(IWICBitmapDecoder_CopyPalette(decoder, global_palette))) {
			IWICPalette_HasAlpha(global_palette, &global_palette_has_alpha);
		}
		has_alpha = frame_palette_has_alpha || global_palette_has_alpha;
#ifdef __cplusplus
		if(frame_palette != NULL) frame_palette->lpVtbl->Release(frame_palette);
		if(global_palette != NULL) global_palette->lpVtbl->Release(global_palette);
#else
		...
		has_alpha = IsEqualGUID(MAKE_REFGUID(pixel_format),
			MAKE_REFGUID(GUID_WICPixelFormat32bppRGBA_)) || IsEqualGUID(MAKE_REFGUID(pixel_format),
			MAKE_REFGUID(GUID_WICPixelFormat32bppBGRA_)) || IsEqualGUID(MAKE_REFGUID(pixel_format),
			MAKE_REFGUID(GUID_WICPixelFormat64bppRGBA_)) || IsEqualGUID(MAKE_REFGUID(pixel_format),
...
int ReadPictureWithWIC(const char* const filename, WebPPicture* const pic, int keep_alpha, Metadata* const metadata) 
{
...
#ifdef __cplusplus
	IFS(IWICFormatConverter_Initialize(converter, (IWICBitmapSource*)frame, *importer->pixel_format, WICBitmapDitherTypeNone, NULL, 0.0, WICBitmapPaletteTypeCustom));
#else
	IFS(IWICFormatConverter_Initialize(converter, (IWICBitmapSource*)frame, importer->pixel_format, WICBitmapDitherTypeNone, NULL, 0.0, WICBitmapPaletteTypeCustom));
#endif
	// Decode.
	...
	if(stride != (int)stride || !ImgIoUtilCheckSizeArgumentsOverflow(stride, height)) {
		hr = E_FAIL;
	}
	if(SUCCEEDED(hr)) {
		rgb = (BYTE *)SAlloc::M((size_t)stride * height);
		if(rgb == NULL)
			hr = E_OUTOFMEMORY;
	}
	IFS(IWICFormatConverter_CopyPixels(converter, NULL, (UINT)stride, (UINT)stride * height, rgb));
	// WebP conversion.
	if(SUCCEEDED(hr)) {
		int ok;
		pic->width = width;
