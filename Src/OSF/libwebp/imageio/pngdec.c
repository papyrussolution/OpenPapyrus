...
static void PNGAPI error_function(png_structp png, const char * error) 
{
	if(error) 
...
static /*png_voidp*/void * MallocFunc(png_structp png_ptr, LocalPngAllocSize size) 
{
	(void)png_ptr;
	if(size != (size_t)size)
		return NULL;
	if(!ImgIoUtilCheckSizeArgumentsOverflow(size, 1)) 
		return NULL;
	return /*(png_voidp)*/SAlloc::M((size_t)size);
}

static void FreeFunc(png_structp png_ptr, /*png_voidp*/void * ptr) 
{
	(void)png_ptr;
	SAlloc::F(ptr);
}
...
static uint8 * HexStringToBytes(const char* hexstring, size_t expected_length) 
{
	const char* src = hexstring;
	size_t actual_length = 0;
	uint8* const raw_data = (uint8*)SAlloc::M(expected_length);
	uint8* dst;
	if(raw_data == NULL) 
		return NULL;
...
static void ReadFunc(png_structp png_ptr, png_bytep data, size_t length) 
{
...
int ReadPNG(const uint8* const data, size_t data_size, struct WebPPicture* const pic, int keep_alpha, struct Metadata* const metadata) 
{
...
#else  // !WEBP_HAVE_PNG
int ReadPNG(const uint8* const data, size_t data_size,
