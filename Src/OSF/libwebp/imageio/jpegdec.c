...
static int StoreICCP(j_decompress_ptr dinfo, MetadataPayload* const iccp) 
{
	...
	memzero(iccp_segments, sizeof(iccp_segments));
	...
	iccp->bytes = (uint8*)SAlloc::M(total_size);
...
int ReadJPEG(const uint8* const data, size_t data_size,
	...
	memzero((j_decompress_ptr)&dinfo, sizeof(dinfo)); // for setjmp safety
...
Error:
	...
	rgb = (uint8*)SAlloc::M((size_t)stride * height);
	if(rgb == NULL) {
		goto Error;
	}
