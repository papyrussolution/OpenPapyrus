...
static size_t ReadLine(const uint8* const data, size_t off, size_t data_size,
...
int ReadPNM(const uint8* const data, size_t data_size,
    WebPPicture* const pic, int keep_alpha,
    struct Metadata* const metadata) {
	int ok = 0;
	int i, j;
	uint64_t stride, pixel_bytes, sample_size, depth;
	uint8* rgb = NULL, * tmp_rgb;
	size_t offset;
	PNMInfo info;
	...
	rgb = (uint8*)SAlloc::M((size_t)stride * info.height);
	if(rgb == NULL) 
		goto End;
	// Convert input.
