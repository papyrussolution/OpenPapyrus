...
int WebPWritePNG(const char* out_file_name, int use_stdout, const WebPDecBuffer* const buffer) 
{
	...
	uint8* const rgb = buffer->u.RGBA.rgba;
...
static int WritePPMPAM(FILE* fout, const WebPDecBuffer* const buffer, int alpha) 
{
	if(fout == NULL || buffer == NULL) {
		...
		const uint32_t width = buffer->width;
		const uint32_t height = buffer->height;
		const uint8* row = buffer->u.RGBA.rgba;
		const int stride = buffer->u.RGBA.stride;
...
int WebPWritePGM(FILE* fout, const WebPDecBuffer* const buffer) {
	if(fout == NULL || buffer == NULL) {
		...
		const uint8* src_y = yuv->y;
		const uint8* src_u = yuv->u;
		const uint8* src_v = yuv->v;
		const uint8* src_a = yuv->a;
