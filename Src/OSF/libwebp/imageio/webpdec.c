...
VP8StatusCode DecodeWebP(const uint8* const data, size_t data_size, WebPDecoderConfig* const config) 
{
	if(config == NULL) 
...
VP8StatusCode DecodeWebPIncremental(const uint8* const data, size_t data_size, WebPDecoderConfig* const config) 
{
...
// Metadata
//
static int ExtractMetadata(const uint8* const data, size_t data_size, Metadata* const metadata) 
{
	...
	if((flags & ICCP_FLAG) && WebPDemuxGetChunk(demux, "ICCP", 1, &chunk_iter)) {
		MetadataCopy((const char*)chunk_iter.chunk.bytes, chunk_iter.chunk.size, &metadata->iccp);
		WebPDemuxReleaseChunkIterator(&chunk_iter);
	}
	if((flags & EXIF_FLAG) && WebPDemuxGetChunk(demux, "EXIF", 1, &chunk_iter)) {
		MetadataCopy((const char*)chunk_iter.chunk.bytes, chunk_iter.chunk.size, &metadata->exif);
		WebPDemuxReleaseChunkIterator(&chunk_iter);
	}
	if((flags & XMP_FLAG) && WebPDemuxGetChunk(demux, "XMP ", 1, &chunk_iter)) {
		MetadataCopy((const char*)chunk_iter.chunk.bytes, chunk_iter.chunk.size, &metadata->xmp);
		WebPDemuxReleaseChunkIterator(&chunk_iter);
	}
...
int ReadWebP(const uint8* const data, size_t data_size, WebPPicture* const pic, int keep_alpha, Metadata* const metadata) 
{
	int ok = 0;
	VP8StatusCode status = VP8_STATUS_OK;
...
#endif
			output_buffer->u.RGBA.rgba = (uint8*)pic->argb;
