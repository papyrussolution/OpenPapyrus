...
static uint32_t Unmult(uint8 x, uint32_t mult) 
{
	const uint32_t v = (x * mult + HALF) >> MFIX;
	return (v > 255u) ? 255u : v;
}

static FORCEINLINE uint32_t GetScale(uint32_t a) { return (255u << MFIX) / a; }

#undef MFIX
#undef HALF

static void MultARGBRow(uint8* ptr, int width) 
{
...
int ReadTIFF(const uint8* const data, size_t data_size, WebPPicture* const pic, int keep_alpha, Metadata* const metadata) 
{
	...
	uint32 * raster = 0; // @sobolev uint32_t-->uint32
	int64_t alloc_size;
	int ok = 0;
	tdir_t dircount;
	...
	tif = TIFFClientOpen("Memory", "r", &my_data, MyRead, MyRead, MySeek, MyClose, MySize, MyMapFile, MyUnmapFile);
	if(tif == NULL) {
	...
	if(!(TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &image_width) && TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &image_height))) {
	...
	if(TIFFGetField(tif, TIFFTAG_TILEWIDTH, &tile_width) && TIFFGetField(tif, TIFFTAG_TILELENGTH, &tile_height)) {
		if((tile_width > 32 && tile_width / 2 > image_width) || (tile_height > 32 && tile_height / 2 > image_height) ||
		    ImgIoUtilCheckSizeArgumentsOverflow((uint64_t)tile_width * sizeof(*raster), tile_height)) {
			fprintf(stderr, "Error! TIFF tile dimension (%d x %d) is too large.\n", tile_width, tile_height);
			goto End;
		}
	}
	if(samples_per_px > 3 && !TIFFGetField(tif, TIFFTAG_EXTRASAMPLES, &extra_samples, &extra_samples_ptr)) {
		fprintf(stderr, "Error! Cannot retrieve TIFF ExtraSamples info.\n");
		goto End;
	}
	// _Tiffmalloc uses a signed type for size.
	alloc_size = (int64_t)(stride * image_height);
	if(alloc_size < 0 || alloc_size != (tsize_t)alloc_size) 
		goto End;
...
#else  // !WEBP_HAVE_TIFF
int ReadTIFF(const uint8* const data, size_t data_size, struct WebPPicture* const pic, int keep_alpha, struct Metadata* const metadata) 
{
	(void)data;
	(void)data_size;
	(void)pic;
