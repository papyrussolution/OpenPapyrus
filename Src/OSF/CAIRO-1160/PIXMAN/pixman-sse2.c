...
static void sse2_combine_over_reverse_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_in_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_in_reverse_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_out_reverse_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_out_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_atop_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_atop_reverse_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_xor_u(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * dst,
    const uint32 * src,
    const uint32 * mask,
...
static force_inline uint32 core_combine_saturate_u_pixel_sse2(uint32 src, uint32 dst)
{
	...
	uint32 da = ~dst >> 24;
	if(sa > da) {
		ms = pix_multiply_1x128(ms, expand_alpha_1x128(unpack_32_1x128(DIV_UN8(da, sa) << 24)));
	}
...
static void sse2_combine_src_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_over_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_over_reverse_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_in_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_in_reverse_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_out_ca(pixman_implementation_t * imp, pixman_op_t op, uint32 * pd, const uint32 * ps, const uint32 * pm, int w)
{
	uint32 s, m, d;
	__m128i xmm_alpha_lo, xmm_alpha_hi;
	__m128i xmm_src_lo, xmm_src_hi;
	...
	while(w && (uintptr_t)pd & 15) {
		...
		d = *pd;
		*pd++ = pack_1x128_32(
			pix_multiply_1x128(pix_multiply_1x128(unpack_32_1x128(s), unpack_32_1x128(m)), negate_1x128(expand_alpha_1x128(unpack_32_1x128(d)))));
		w--;
	}
	while(w >= 4) {
		xmm_dst_hi = load_128_aligned((__m128i*)pd);
	...
	while(w) {
		...
		m = *pm++;
		d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(pix_multiply_1x128(unpack_32_1x128(s), unpack_32_1x128(m)), negate_1x128(expand_alpha_1x128(unpack_32_1x128(d)))));
		w--;
	}
}

static void sse2_combine_out_reverse_ca(pixman_implementation_t * imp, pixman_op_t op, uint32 * pd, const uint32 * ps, const uint32 * pm, int w)
{
	uint32 s, m, d;
	__m128i xmm_alpha_lo, xmm_alpha_hi;
	__m128i xmm_src_lo, xmm_src_hi;
	...
	while(w && (uintptr_t)pd & 15) {
		...
		m = *pm++;
		d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(d), negate_1x128(pix_multiply_1x128(unpack_32_1x128(m), expand_alpha_1x128(unpack_32_1x128(s))))));
		w--;
	}
	while(w >= 4) {
		xmm_dst_hi = load_128_aligned((__m128i*)pd);
...
static void sse2_combine_atop_ca(pixman_implementation_t * imp, pixman_op_t op, uint32 * pd, const uint32 * ps, const uint32 * pm, int w)
{
	uint32 s, m, d;
	__m128i xmm_src_lo, xmm_src_hi;
	__m128i xmm_dst_lo, xmm_dst_hi;
...
static void sse2_combine_atop_reverse_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_xor_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_combine_add_ca(pixman_implementation_t * imp,
    pixman_op_t op,
    uint32 * pd,
    const uint32 * ps,
    const uint32 * pm,
...
static void sse2_composite_over_n_8888(pixman_implementation_t * imp,
    pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && (uintptr_t)dst & 15) {
			d = *dst;
			*dst++ = pack_1x128_32(over_1x128(xmm_src, xmm_alpha, unpack_32_1x128(d)));
			w--;
		}
		while(w >= 4) {
...
static void sse2_composite_over_n_0565(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && (uintptr_t)dst & 15) {
			d = *dst;
			*dst++ = pack_565_32_16(pack_1x128_32(over_1x128(xmm_src, xmm_alpha, expand565_16_1x128(d))));
			w--;
		}
	...
	while(height--) {
		...
		while(w && (uintptr_t)pd & 15) {
			...
			if(m) {
				...
				*pd = pack_1x128_32(_mm_adds_epu8(pix_multiply_1x128(mmx_mask, mmx_src), mmx_dest));
			}
			pd++;
			w--;
		}
		while(w >= 4) {
			...
			if(pack_cmp != 0xffff) {
				...
				save_128_aligned((__m128i*)pd, _mm_adds_epu8(xmm_mask_hi, xmm_dst));
			}
			pd += 4;
			pm += 4;
			w -= 4;
...
static void sse2_composite_src_x888_8888(pixman_implementation_t * imp,
    pixman_composite_info_t * info)
{
	PIXMAN_COMPOSITE_ARGS(info);
	uint32 * dst_line, * dst;
...
static void sse2_composite_over_x888_n_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && (uintptr_t)dst & 15) {
			...
			__m128i mask  = xmm_mask;
			__m128i dest  = unpack_32_1x128(d);
			*dst++ = pack_1x128_32(in_over_1x128(&src, &alpha, &mask, &dest));
			w--;
		}
		while(w >= 4) {
...
static void sse2_composite_over_n_8_8888(pixman_implementation_t * imp,
    pixman_composite_info_t * info)
{
	PIXMAN_COMPOSITE_ARGS(info);
	uint32 src, srca;
...
static void sse2_composite_src_n_8_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	srca = src >> 24;
	if(src == 0) {
		sse2_fill(imp, dest_image->bits.bits, dest_image->bits.rowstride, PIXMAN_FORMAT_BPP(dest_image->bits.format), dest_x, dest_y, width, height, 0);
		return;
	}
	PIXMAN_IMAGE_GET_LINE(dest_image, dest_x, dest_y, uint32, dst_stride, dst_line, 1);
	PIXMAN_IMAGE_GET_LINE(mask_image, mask_x, mask_y, uint8, mask_stride, mask_line, 1);
...
static void sse2_composite_over_n_8_0565(pixman_implementation_t * imp,
    pixman_composite_info_t * info)
{
	PIXMAN_COMPOSITE_ARGS(info);
	uint32 src;
...
static void sse2_composite_over_pixbuf_0565(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w) {
			...
			d = *dst;
			ms = unpack_32_1x128(s);
			*dst++ = pack_565_32_16(pack_1x128_32(over_rev_non_pre_1x128(ms, expand565_16_1x128(d))));
			w--;
		}
	}
}
...
static void sse2_composite_over_pixbuf_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w) {
			s = *src++;
			d = *dst;
			*dst++ = pack_1x128_32(over_rev_non_pre_1x128(unpack_32_1x128(s), unpack_32_1x128(d)));
			w--;
		}
	}
}
	...
	while(height--) {
		...
		while(w && ((uintptr_t)dst & 15)) {
			...
			if(m) {
				...
				mmx_mask = unpack_32_1x128(m);
				mmx_dest = expand565_16_1x128(d);
				*dst = pack_565_32_16(pack_1x128_32(in_over_1x128(&mmx_src, &mmx_alpha, &mmx_mask, &mmx_dest)));
			}
			w--;
			dst++;
			mask++;
...
static void sse2_composite_in_n_8_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && ((uintptr_t)dst & 15)) {
			m = (uint32)*mask++;
			d = (uint32)*dst;
			*dst++ = (uint8)pack_1x128_32(pix_multiply_1x128(pix_multiply_1x128(xmm_alpha, unpack_32_1x128(m)), unpack_32_1x128(d)));
			w--;
		}
		while(w >= 16) {
			xmm_mask = load_128_unaligned((__m128i*)mask);
...
static void sse2_composite_in_n_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	if(src == 0xff)
		return;
	if(src == 0x00) {
		pixman_fill(dest_image->bits.bits, dest_image->bits.rowstride, 8, dest_x, dest_y, width, height, src);
		return;
	}
	while(height--) {
		dst = dst_line;
		...
		while(w && ((uintptr_t)dst & 15)) {
			d = (uint32)*dst;
			*dst++ = (uint8)pack_1x128_32(pix_multiply_1x128(xmm_alpha, unpack_32_1x128(d)));
			w--;
		}
		while(w >= 16) {
			xmm_dst = load_128_aligned((__m128i*)dst);
			unpack_128_2x128(xmm_dst, &xmm_dst_lo, &xmm_dst_hi);
			pix_multiply_2x128(&xmm_alpha, &xmm_alpha, &xmm_dst_lo, &xmm_dst_hi, &xmm_dst_lo, &xmm_dst_hi);
			save_128_aligned((__m128i*)dst, pack_2x128_128(xmm_dst_lo, xmm_dst_hi));
			dst += 16;
			w -= 16;
		}
		while(w) {
			d = (uint32)*dst;
			*dst++ = (uint8)pack_1x128_32(
				pix_multiply_1x128(xmm_alpha, unpack_32_1x128(d)));
			w--;
		}
	}
}
...
static void sse2_composite_in_8_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && ((uintptr_t)dst & 15)) {
			s = (uint32)*src++;
			d = (uint32)*dst;
			*dst++ = (uint8)pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(s), unpack_32_1x128(d)));
			w--;
		}
		while(w >= 16) {
			xmm_src = load_128_unaligned((__m128i*)src);
...
static void sse2_composite_add_n_8_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && ((uintptr_t)dst & 15)) {
			m = (uint32)*mask++;
			d = (uint32)*dst;
			*dst++ = (uint8)pack_1x128_32(_mm_adds_epu16(pix_multiply_1x128(xmm_alpha, unpack_32_1x128(m)), unpack_32_1x128(d)));
			w--;
		}
		while(w >= 16) {
			xmm_mask = load_128_unaligned((__m128i*)mask);
...
static void sse2_composite_add_n_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	if(src == 0x00)
		return;
	if(src == 0xff) {
		pixman_fill(dest_image->bits.bits, dest_image->bits.rowstride, 8, dest_x, dest_y, width, height, 0xff);
		return;
	}
	...
	while(height--) {
		...
		w = width;
		while(w && ((uintptr_t)dst & 15)) {
			*dst = (uint8)_mm_cvtsi128_si32(_mm_adds_epu8(xmm_src, _mm_cvtsi32_si128(*dst)));
			w--;
			dst++;
		}
		while(w >= 16) {
			save_128_aligned((__m128i*)dst, _mm_adds_epu8(xmm_src, load_128_aligned((__m128i*)dst)));
			dst += 16;
			w -= 16;
		}
		while(w) {
...
static void sse2_composite_add_n_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	if(src == 0)
		return;
	if(src == ~0) {
		pixman_fill(dest_image->bits.bits, dest_image->bits.rowstride, 32, dest_x, dest_y, width, height, ~0);
		return;
	}
	xmm_src = _mm_set_epi32(src, src, src, src);
	while(height--) {
		...
		while(w && (uintptr_t)dst & 15) {
			d = *dst;
			*dst++ = _mm_cvtsi128_si32(_mm_adds_epu8(xmm_src, _mm_cvtsi32_si128(d)));
			w--;
		}
		while(w >= 4) {
			save_128_aligned((__m128i*)dst, _mm_adds_epu8(xmm_src, load_128_aligned((__m128i*)dst)));
...
static void sse2_composite_add_n_8_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && ((uintptr_t)dst & 15)) {
			uint8 m = *mask++;
			if(m) {
				*dst = pack_1x128_32(_mm_adds_epu16(pix_multiply_1x128(xmm_src, expand_pixel_8_1x128(m)), unpack_32_1x128(*dst)));
			}
			dst++;
			w--;
		}
    ...
    int height)
{
	...
	while(height--) {
		...
		while(w >= 64) {
			__m128i xmm0 = load_128_unaligned((__m128i*)(s));
			__m128i xmm1 = load_128_unaligned((__m128i*)(s + 16));
			__m128i xmm2 = load_128_unaligned((__m128i*)(s + 32));
			__m128i xmm3 = load_128_unaligned((__m128i*)(s + 48));
...
{
	PIXMAN_COMPOSITE_ARGS(info);
	sse2_blt(imp, src_image->bits.bits, dest_image->bits.bits, src_image->bits.rowstride, dest_image->bits.rowstride,
	    PIXMAN_FORMAT_BPP(src_image->bits.format), PIXMAN_FORMAT_BPP(dest_image->bits.format), src_x, src_y, dest_x, dest_y, width, height);
}
		...
		while(w >= 4) {
			...
			if(m) {
				...
				if(m == 0xffffffff && is_opaque(xmm_src)) {
					...
					expand_alpha_rev_2x128(xmm_mask_lo, xmm_mask_hi, &xmm_mask_lo, &xmm_mask_hi);
					in_over_2x128(&xmm_src_lo, &xmm_src_hi, &xmm_srca_lo, &xmm_srca_hi, &xmm_mask_lo, &xmm_mask_hi, &xmm_dst_lo, &xmm_dst_hi);
					save_128_aligned((__m128i*)dst, pack_2x128_128(xmm_dst_lo, xmm_dst_hi));
				}
			}
			src += 4;
			...
			if(m) {
				if(sa == 0xff && m == 0xff) {
					*dst = s;
					...
					__m128i ma = expand_alpha_rev_1x128(load_32_1x128(m));
					__m128i ms = unpack_32_1x128(s);
					__m128i md = unpack_32_1x128(d);
					__m128i msa = expand_alpha_rev_1x128(load_32_1x128(sa));
					*dst = pack_1x128_32(in_over_1x128(&ms, &msa, &ma, &md));
				}
			}
			dst++;
...
static void sse2_composite_over_reverse_n_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		w = width;
		while(w && (uintptr_t)dst & 15) {
			__m128i vd = unpack_32_1x128(*dst);
			*dst = pack_1x128_32(over_1x128(vd, expand_alpha_1x128(vd), xmm_src));
			w--;
			dst++;
		}
		while(w >= 4) {
			...
			tmp_lo = xmm_src;
			tmp_hi = xmm_src;
			over_2x128(&xmm_dst_lo, &xmm_dst_hi, &xmm_dsta_lo, &xmm_dsta_hi, &tmp_lo, &tmp_hi);
			save_128_aligned((__m128i*)dst, pack_2x128_128(tmp_lo, tmp_hi));
			w -= 4;
			dst += 4;
		}
		while(w) {
...
FAST_NEAREST_MAINLOOP(sse2_8888_8888_cover_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32, uint32, COVER)
FAST_NEAREST_MAINLOOP(sse2_8888_8888_none_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32, uint32, NONE)
...
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_cover_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, COVER, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_pad_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, PAD, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_none_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, NONE, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_normal_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, NORMAL, TRUE, TRUE)
...
#define BILINEAR_DECLARE_VARIABLES                                             \
	...
	const __m128i xmm_ux1 = _mm_set_epi16(unit_x, -unit_x, unit_x, -unit_x, unit_x, -unit_x, unit_x, -unit_x); \
	const __m128i xmm_ux4 = _mm_set_epi16(unit_x * 4, -unit_x * 4, unit_x * 4, -unit_x * 4, unit_x * 4, -unit_x * 4, unit_x * 4, -unit_x * 4); \
	const __m128i xmm_zero = _mm_setzero_si128();                              \
	__m128i xmm_x = _mm_set_epi16(vx + unit_x * 3, -(vx + 1) - unit_x * 3, vx + unit_x * 2, -(vx + 1) - unit_x * 2, vx + unit_x * 1, -(vx + 1) - unit_x * 1, \
...
#define BILINEAR_INTERPOLATE_ONE_PIXEL_HELPER(pix, phase_)                      \
	do {                                                                            \
		...
		if(phase <= 0) {                                                          \
			xmm_wh_state = _mm_add_epi16(xmm_addc, _mm_srli_epi16(xmm_x, 16 - BILINEAR_INTERPOLATION_BITS)); \
			xmm_x = _mm_add_epi16(xmm_x, (phase < 0) ? xmm_ux1 : xmm_ux4);         \
			phase = 0;                                                              \
		}                                                                           \
		xmm_wh = _mm_shuffle_epi32(xmm_wh_state, _MM_SHUFFLE(phase, phase, phase, phase)); \
		/* horizontal interpolation */                                              \
		xmm_a = _mm_madd_epi16(_mm_unpackhi_epi16(_mm_shuffle_epi32(xmm_a, _MM_SHUFFLE(1, 0, 3, 2)), xmm_a), xmm_wh); \
		/* shift the result */                                                      \
...
#define BILINEAR_DECLARE_VARIABLES                                             \
	...
	const __m128i xmm_ux4 = _mm_set_epi16(unit_x * 4, -unit_x * 4,             \
		...
		unit_x * 4, -unit_x * 4);            \
	const __m128i xmm_zero = _mm_setzero_si128();                              \
	__m128i xmm_x = _mm_set_epi16(vx, -(vx + 1), vx, -(vx + 1), vx, -(vx + 1), vx, -(vx + 1))
...
#define BILINEAR_INTERPOLATE_ONE_PIXEL_HELPER(pix, phase)                       \
	do {                                                                            \
		...
		xmm_a = _mm_add_epi16(xmm_a, xmm_b);                                       \
		/* calculate horizontal weights */                                          \
		xmm_wh = _mm_add_epi16(xmm_addc, _mm_srli_epi16(xmm_x, 16 - BILINEAR_INTERPOLATION_BITS)); \
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_cover_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32, uint32, uint32, COVER, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_pad_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32, uint32, uint32, PAD, FLAG_NONE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_x888_8888_cover_SRC, scaled_bilinear_scanline_sse2_x888_8888_SRC, uint32, uint32, uint32, COVER, FLAG_NONE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32, uint32, uint32, COVER, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32, uint32, uint32, PAD, FLAG_NONE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32, uint8, uint32, COVER, FLAG_HAVE_NON_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32, uint8, uint32, PAD, FLAG_HAVE_NON_SOLID_MASK)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, COVER, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, PAD, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_none_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, NONE, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_normal_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32, uint32, uint32, NORMAL, FLAG_HAVE_SOLID_MASK)
...
static uint32 * sse2_fetch_x8r8g8b8(pixman_iter_t * iter, const uint32 * mask)
{
	int w = iter->width;
...
static uint32 * sse2_fetch_r5g6b5(pixman_iter_t * iter, const uint32 * mask)
{
	int w = iter->width;
	uint32 * dst = iter->buffer;
...
static uint32 * sse2_fetch_a8(pixman_iter_t * iter, const uint32 * mask)
{
	int w = iter->width;
	uint32 * dst = iter->buffer;
	uint8 * src = iter->bits;
...
#define IMAGE_FLAGS (FAST_PATH_STANDARD_FLAGS | FAST_PATH_ID_TRANSFORM | FAST_PATH_BITS_IMAGE | FAST_PATH_SAMPLES_COVER_CLIP_NEAREST)
...
static const pixman_iter_info_t sse2_iters[] =
{
	...
	{ PIXMAN_r5g6b5, IMAGE_FLAGS, ITER_NARROW, _pixman_iter_init_bits_stride, sse2_fetch_r5g6b5, NULL},
	{ PIXMAN_a8, IMAGE_FLAGS, ITER_NARROW, _pixman_iter_init_bits_stride, sse2_fetch_a8, NULL},
	{ PIXMAN_null },
};
