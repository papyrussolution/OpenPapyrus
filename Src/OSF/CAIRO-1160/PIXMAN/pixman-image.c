...
PIXMAN_EXPORT void * pixman_image_get_destroy_data(pixman_image_t * image)
{
 ...
 */
PIXMAN_EXPORT void pixman_disable_out_of_bounds_workaround(void)
{
}
...
static void compute_image_info(pixman_image_t * image)
{
	...
	switch(image->type) {
		case SOLID:
		    ...
		    break;
		case BITS:
		    if(image->bits.width == 1 && image->bits.height == 1 && image->common.repeat != PIXMAN_REPEAT_NONE) {
			    code = PIXMAN_solid;
		    }
		    else {
		    ...
		    if(!PIXMAN_FORMAT_A(image->bits.format) && PIXMAN_FORMAT_TYPE(image->bits.format) != PIXMAN_TYPE_GRAY &&
...
PIXMAN_EXPORT void pixman_image_set_repeat(pixman_image_t * image, pixman_repeat_t repeat)
{
...
PIXMAN_EXPORT pixman_bool_t pixman_image_set_filter(pixman_image_t * image, pixman_filter_t filter, const pixman_fixed_t * params, int n_params)
{
	...
	if(filter == PIXMAN_FILTER_SEPARABLE_CONVOLUTION) {
		...
		return_val_if_fail(n_params == 4 + n_x_phases * width + n_y_phases * height, FALSE);
	}
	new_params = NULL;
	if(params) {
...
otherwise:
	...
	if(PIXMAN_FORMAT_TYPE(format) != PIXMAN_TYPE_ARGB && PIXMAN_FORMAT_TYPE(format) != PIXMAN_TYPE_ARGB_SRGB) {
		result = (((result & 0xff000000) >>  0) | ((result & 0x00ff0000) >> 16) | ((result & 0x0000ff00) >>  0) | ((result & 0x000000ff) << 16));
	}
	return result;
}
