...
static void dummy_composite_rect(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
}
    ...
    pixman_format_code_t dest_format, uint32_t dest_flags, pixman_implementation_t ** out_imp, pixman_composite_func_t * out_func)
{
	...
	cache_t * cache = PIXMAN_GET_THREAD_LOCAL(fast_path_cache);
    ...
    int x, int y, int width, int height, uint8_t * buffer, iter_flags_t iter_flags, uint32_t image_flags)
{
	...
	while(imp) {
		if(imp->iter_info) {
			for(const pixman_iter_info_t * info = imp->iter_info; info->format != PIXMAN_null; ++info) {
...
static const pixman_fast_path_t empty_fast_path[] = { { PIXMAN_OP_NONE } };
...
pixman_implementation_t * _pixman_choose_implementation(void)
{
	...
	if(_pixman_disabled("wholeops")) {
		 ...
		 */
		for(pixman_implementation_t * cur = imp; cur->fallback; cur = cur->fallback)
