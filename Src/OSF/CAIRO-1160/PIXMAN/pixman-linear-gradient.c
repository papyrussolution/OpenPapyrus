...
static boolint linear_gradient_is_horizontal(pixman_image_t * image, int x, int y, int width, int height)
{
	...
	if(image->common.transform) {
		...
		if(image->common.transform->matrix[2][0] != 0 || image->common.transform->matrix[2][1] != 0 || image->common.transform->matrix[2][2] == 0) {
			return FALSE;
		}
	 ...
	 */
	inc = height * (double)pixman_fixed_1 * pixman_fixed_1 * (dx * v.vector[0] + dy * v.vector[1]) / (v.vector[2] * (double)l);
...
static uint32 * linear_get_scanline_narrow(pixman_iter_t * iter, const uint32 * mask)
{
	...
	if(l == 0 || unit.vector[2] == 0) {
		...
		if(((pixman_fixed_32_32_t)(inc * width)) == 0) {
			uint32 color = _pixman_gradient_walker_pixel(&walker, t);
			while(buffer < end)
				*buffer++ = color;
		}
		else {
...
static uint32 * linear_get_scanline_wide(pixman_iter_t * iter, const uint32 * mask)
{
	uint32 * buffer = linear_get_scanline_narrow(iter, NULL);
...
PIXMAN_EXPORT pixman_image_t * pixman_image_create_linear_gradient(const pixman_point_fixed_t *  p1,
