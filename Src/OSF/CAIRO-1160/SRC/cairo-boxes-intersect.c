...
static inline void pqueue_push(sweep_line_t * sweep, rectangle_t * rectangle)
{
	rectangle_t ** elements;
	int i, parent;
	...
	for(i = ++sweep->pq.size; i != PQ_FIRST_ENTRY && rectangle_compare_stop(rectangle, elements[parent = PQ_PARENT_INDEX(i)]) < 0; i = parent) {
...
static inline void pqueue_pop(pqueue_t * pq)
{
	...
	for(i = PQ_FIRST_ENTRY; (child = PQ_LEFT_CHILD_INDEX(i)) <= pq->size; i = child) {
		if(child != pq->size && rectangle_compare_stop(elements[child+1], elements[child]) < 0) {
			child++;
		}
...
static inline rectangle_t * rectangle_pop_start(sweep_line_t * sweep_line)
{
	return *sweep_line->rectangles++;
}

static inline rectangle_t * rectangle_peek_stop(sweep_line_t * sweep_line)
{
...
static inline void active_edges(sweep_line_t * sweep, cairo_boxes_t * out)
{
...
static inline void insert_edge(edge_t * edge, edge_t * pos)
{
	if(pos->x != edge->x) {
		if(pos->x > edge->x) {
			do {
...
static cairo_status_t _cairo_boxes_intersect_with_box(const cairo_boxes_t * boxes, const cairo_box_t * box, cairo_boxes_t * out)
{
	cairo_status_t status;
	int i, j;
...
cairo_status_t _cairo_boxes_intersect(const cairo_boxes_t * a, const cairo_boxes_t * b, cairo_boxes_t * out)
{
