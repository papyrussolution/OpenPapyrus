 ...
 *
 * Contributor(s): Kristian Høgsberg <krh@redhat.com> Carl Worth <cworth@cworth.org>
 */
#include "cairoint.h"
#pragma hdrstop
/**
 ...
 **/
void FASTCALL _cairo_array_init(cairo_array_t * array, uint element_size)
{
	array->size = 0;
...
cairo_status_t FASTCALL _cairo_array_grow_by(cairo_array_t * array, uint additional)
{
	...
	uint required_size = array->num_elements + additional;
	uint new_size;
 ...
 **/
void * FASTCALL _cairo_array_index(cairo_array_t * array, uint index)
{
 ...
 * This space was left intentionally blank, but gtk-doc filled it.
 **/
uint FASTCALL _cairo_array_size(const cairo_array_t * array)
{
	return array->size;
}
/**
...
cairo_status_t _cairo_user_data_array_copy(cairo_user_data_array_t * dst, const cairo_user_data_array_t * src)
{
	...
	return _cairo_array_append_multiple(dst, _cairo_array_index_const(src, 0), src->num_elements);
}
