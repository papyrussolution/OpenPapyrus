...
static pixman_image_t * _pixman_image_for_gradient(const cairo_gradient_pattern_t * pattern, const cairo_rectangle_int_t * extents, int * ix, int * iy)
{
...
static pixman_image_t * _pixman_image_for_mesh(const cairo_mesh_pattern_t * pattern, const cairo_rectangle_int_t * extents, int * tx, int * ty)
{
	pixman_image_t * image;
	int width, height;
	TRACE_FUNCTION_SIMPLE();
...
{
	struct acquire_source_cleanup * data = static_cast<struct acquire_source_cleanup *>(closure);
...
{
	return MAX(2, static_cast<int>(ceil(r * 4)));
}
    ...
    int * ix, int * iy)
{
	...
	*ix = *iy = 0;
	...
	if(!_cairo_matrix_is_identity(&pattern->base.matrix)) {
		...
		limit.x = static_cast<int>(floor(x1));
		limit.y = static_cast<int>(floor(y1));
		limit.width  = static_cast<int>(ceil(x2)) - limit.x;
		limit.height = static_cast<int>(ceil(y2)) - limit.y;
