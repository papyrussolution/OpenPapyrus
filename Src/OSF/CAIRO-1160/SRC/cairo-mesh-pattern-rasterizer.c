 ...
 */
static inline int sqsteps2shift(double steps_sq)
{
	int r;
 ...
 */
static inline void fd_init(double x, double y, double z, double w, double f[4])
{
	f[0] = x;
	f[1] = w - x;
	f[2] = 6. * (w - 2. * z + y);
 ...
 * the output f.
 */
static inline void fd_down(double f[4])
{
	f[3] *= 0.125;
	f[2] = f[2] * 0.25 - f[3];
	f[1] = (f[1] - f[2]) * 0.5;
 ...
 */
static inline void fd_fwd(double f[4])
{
	f[0] += f[1];
	f[1] += f[2];
	f[2] += f[3];
 ...
 */
static inline int intersect_interval(double a, double b, double c, double d)
{
	if(c <= a && b <= d)
		return INSIDE;
	else if(a >= d || b <= c)
