...
static void clear_bits(cairo_mempool_t * pool, size_t first, size_t last)
{
	size_t i, n = last;
...
static void merge_buddies(cairo_mempool_t * pool, struct _cairo_mempool::_cairo_memblock * block, int max_bits)
{
...
{
	struct _cairo_mempool::_cairo_memblock * block, * buddy, * next;
	int bits;
	for(bits = 0; bits < max_bits - 1; bits++) {
		cairo_list_foreach_entry_safe(block, next, struct _cairo_mempool::_cairo_memblock, &pool->free[bits], link)
		{
			...
			if(buddy == NULL)
				continue;
			if(buddy == next) {
				next = cairo_container_of(buddy->link.next, struct _cairo_mempool::_cairo_memblock, link);
			}
			cairo_list_del(&block->link);
...
static void * buddy_malloc(cairo_mempool_t * pool, int bits)
{
	...
	for(b = bits; b <= pool->max_free_bits; b++) {
		if(!cairo_list_is_empty(&pool->free[b])) {
			block = cairo_list_first_entry(&pool->free[b], struct _cairo_mempool::_cairo_memblock, link);
			break;
		}
	}
	assert(block != NULL);
...
cairo_status_t _cairo_mempool_init(cairo_mempool_t * pool, void * base, size_t bytes, int min_bits, int num_sizes)
{
	...
	pool->blocks = (struct _cairo_mempool::_cairo_memblock *)SAlloc::C(num_blocks, sizeof(struct _cairo_mempool::_cairo_memblock));
	if(pool->blocks == NULL)
	...
	pool->map = (unsigned char *)_cairo_malloc((num_blocks + 7) >> 3);
	if(pool->map == NULL) {
...
void _cairo_mempool_free(cairo_mempool_t * pool, void * storage)
{
	size_t block_offset;
	struct _cairo_mempool::_cairo_memblock * block;
