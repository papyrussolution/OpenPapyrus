...
static cairo_status_t FASTCALL _cairo_path_fixed_add(cairo_path_fixed_t * path, cairo_path_op_t op, const cairo_point_t * points, int num_points);
static void FASTCALL _cairo_path_fixed_add_buf(cairo_path_fixed_t * path, cairo_path_buf_t * buf);
static cairo_path_buf_t * FASTCALL _cairo_path_buf_create(int size_ops, int size_points);
static void FASTCALL _cairo_path_buf_destroy(cairo_path_buf_t * buf);
static void FASTCALL _cairo_path_buf_add_op(cairo_path_buf_t * buf, cairo_path_op_t op);
static void FASTCALL _cairo_path_buf_add_points(cairo_path_buf_t * buf, const cairo_point_t * points, int num_points);
...
ulong _cairo_path_fixed_size(const cairo_path_fixed_t * path)
{
	const cairo_path_buf_t * buf;
	int num_points = 0;
	int num_ops = 0;
...
boolint _cairo_path_fixed_equal(const cairo_path_fixed_t * a, const cairo_path_fixed_t * b)
{
	...
	if(a->extents.p1.x != b->extents.p1.x || a->extents.p1.y != b->extents.p1.y || a->extents.p2.x != b->extents.p2.x || a->extents.p2.y != b->extents.p2.y) {
		return FALSE;
	}
...
void FASTCALL _cairo_path_fixed_fini(cairo_path_fixed_t * path)
{
	...
	while(buf != cairo_path_head(path)) {
		cairo_path_buf_t * p_this = buf;
		buf = cairo_path_buf_next(buf);
		_cairo_path_buf_destroy(p_this);
	}
	VG(VALGRIND_MAKE_MEM_UNDEFINED(path, sizeof(cairo_path_fixed_t)));
}
...
static cairo_path_op_t FASTCALL _cairo_path_fixed_last_op(const cairo_path_fixed_t * path)
{
	const cairo_path_buf_t * buf = cairo_path_tail(path);
...
static cairo_status_t FASTCALL _cairo_path_fixed_move_to_apply(cairo_path_fixed_t * path)
{
	...
	if(path->fill_maybe_region) {
		path->fill_maybe_region = _cairo_fixed_is_integer(path->current_point.x) && _cairo_fixed_is_integer(path->current_point.y);
	}
	path->last_move_point = path->current_point;
...
cairo_status_t FASTCALL _cairo_path_fixed_line_to(cairo_path_fixed_t * path, cairo_fixed_t x, cairo_fixed_t y)
{
	...
	if(_cairo_path_fixed_last_op(path) == CAIRO_PATH_OP_LINE_TO) {
		const cairo_point_t * p = _cairo_path_fixed_penultimate_point(path);
	...
	if(path->stroke_is_rectilinear) {
		path->stroke_is_rectilinear = path->current_point.x == x || path->current_point.y == y;
...
{
	if(num_points) {
		memcpy(buf->points + buf->num_points, points, sizeof(points[0]) * num_points);
		buf->num_points += num_points;
	}
}

cairo_status_t FASTCALL _cairo_path_fixed_interpret(const cairo_path_fixed_t * path,
...
{
	cairo_path_fixed_append_closure_t * closure = static_cast<cairo_path_fixed_append_closure_t *>(abstract_closure);
	return _cairo_path_fixed_move_to(closure->path, point->x + closure->offset.x, point->y + closure->offset.y);
}

static cairo_status_t _append_line_to(void * abstract_closure, const cairo_point_t * point)
{
	cairo_path_fixed_append_closure_t * closure = static_cast<cairo_path_fixed_append_closure_t *>(abstract_closure);
	return _cairo_path_fixed_line_to(closure->path, point->x + closure->offset.x, point->y + closure->offset.y);
}
...
{
	cairo_path_fixed_append_closure_t * closure = static_cast<cairo_path_fixed_append_closure_t *>(abstract_closure);
	return _cairo_path_fixed_curve_to(closure->path, p0->x + closure->offset.x, p0->y + closure->offset.y,
		   p1->x + closure->offset.x, p1->y + closure->offset.y, p2->x + closure->offset.x, p2->y + closure->offset.y);
}
...
cairo_status_t FASTCALL _cairo_path_fixed_append(cairo_path_fixed_t * path, const cairo_path_fixed_t * other, cairo_fixed_t tx, cairo_fixed_t ty)
{
	cairo_path_fixed_append_closure_t closure;
	closure.path = path;
	closure.offset.x = tx;
	closure.offset.y = ty;
	return _cairo_path_fixed_interpret(other, _append_move_to, _append_line_to, _append_curve_to, _append_close_path, &closure);
}

static void _cairo_path_fixed_offset_and_scale(cairo_path_fixed_t * path, cairo_fixed_t offx, cairo_fixed_t offy,
    cairo_fixed_t scalex, cairo_fixed_t scaley)
{
	cairo_path_buf_t * buf;
	uint i;
...
void FASTCALL _cairo_path_fixed_translate(cairo_path_fixed_t * path, cairo_fixed_t offx, cairo_fixed_t offy)
{
	cairo_path_buf_t * buf;
	uint i;
	if(offx == 0 && offy == 0)
...
void FASTCALL _cairo_path_fixed_transform(cairo_path_fixed_t * path, const cairo_matrix_t * matrix)
{
	...
	if(matrix->yx == 0.0 && matrix->xy == 0.0) {
		...
		_cairo_path_fixed_offset_and_scale(path, _cairo_fixed_from_double(matrix->x0), _cairo_fixed_from_double(matrix->y0),
		    _cairo_fixed_from_double(matrix->xx), _cairo_fixed_from_double(matrix->yy));
		return;
	}
...
static inline boolint _points_form_rect(const cairo_point_t * points)
{
	if(points[0].y == points[1].y && points[1].x == points[2].x && points[2].y == points[3].y && points[3].x == points[0].x)
		return TRUE;
	if(points[0].x == points[1].x && points[1].y == points[2].y && points[2].x == points[3].x && points[3].y == points[0].y)
		return TRUE;
	return FALSE;
}
/*
