...
static cairo_script_surface_t * _cairo_script_surface_create_internal(cairo_script_context_t * ctx, cairo_content_t content, cairo_rectangle_t * extents, cairo_surface_t * passthrough);
...
static void _bitmap_release_id(struct _cairo_script_context::_bitmap * b, ulong token)
{
	struct _cairo_script_context::_bitmap ** prev = NULL;
	do {
...
{
	struct _cairo_script_context::_bitmap * bb, ** prev = NULL;
	ulong min = 0;
	do {
		if(b->min != min)
			break;
...
static const char * _direction_to_string(boolint backward)
{
...
static cairo_status_t _emit_tolerance(cairo_script_surface_t * surface, double tolerance, boolint force)
{
	assert(target_is_active(surface));
...
static cairo_status_t _emit_line_width(cairo_script_surface_t * surface, double line_width, boolint force)
{
	assert(target_is_active(surface));
...
static cairo_status_t _emit_miter_limit(cairo_script_surface_t * surface, double miter_limit, boolint force)
{
	assert(target_is_active(surface));
...
static boolint _dashes_equal(const double * a, const double * b, int num_dashes)
{
	while(num_dashes--) {
		if(fabs(*a - *b) > 1e-5)
			return FALSE;
...
static cairo_status_t _emit_dash(cairo_script_surface_t * surface, const double * dash, uint num_dashes, double offset, boolint force)
{
	uint n;
	assert(target_is_active(surface));
...
static cairo_status_t _emit_stroke_style(cairo_script_surface_t * surface, const cairo_stroke_style_t * style, boolint force)
{
...
static cairo_int_status_t _emit_image_surface(cairo_script_surface_t * surface, cairo_image_surface_t * image)
{
	...
	if(_cairo_int_status_is_error(status)) {
		...
		switch(clone->format) {
			...
			case CAIRO_FORMAT_INVALID:
			default: ASSERT_NOT_REACHED; len = 0; break;
		}
		len *= clone->height;
		if(len > 24) {
			_cairo_output_stream_puts(ctx->stream, "<|");
...
static cairo_int_status_t _emit_image_surface_pattern(cairo_script_surface_t * surface, cairo_surface_t * source)
{
	...
	void * extra;
	cairo_status_t status = _cairo_surface_acquire_source_image(source, &image, &extra);
...
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_surface_t * snapshot, * free_me = NULL;
...
{
	cairo_int_status_t status;
	cairo_surface_t * source = _cairo_raster_source_pattern_acquire(pattern, &surface->base, NULL);
...
static cairo_int_status_t _emit_pattern(cairo_script_surface_t * surface, const cairo_pattern_t * pattern)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_int_status_t status;
	boolint is_default_extend;
	boolint need_newline = TRUE;
	switch(pattern->type) {
...
static cairo_int_status_t FASTCALL _emit_identity(cairo_script_surface_t * surface, boolint * matrix_updated)
{
	assert(target_is_active(surface));
...
static cairo_status_t _emit_path(cairo_script_surface_t * surface, const cairo_path_fixed_t * path, boolint is_fill)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_box_t box;
	cairo_int_status_t status;
...
static boolint _scaling_matrix_equal(const cairo_matrix_t * a, const cairo_matrix_t * b)
{
	return fabs(a->xx - b->xx) < 1e-5 && fabs(a->xy - b->xy) < 1e-5 && fabs(a->yx - b->yx) < 1e-5 && fabs(a->yy - b->yy) < 1e-5;
}

static cairo_status_t _emit_scaling_matrix(cairo_script_surface_t * surface, const cairo_matrix_t * ctm, boolint * matrix_updated)
{
	cairo_script_context_t * ctx = to_context(surface);
	boolint was_identity;
	assert(target_is_active(surface));
...
static cairo_status_t _cairo_script_surface_finish(void * abstract_surface)
{
	...
	if(surface->emitted) {
		...
		if(!cairo_list_is_empty(&surface->operand.link)) {
			if(!ctx->active) {
				if(target_is_active(surface)) {
					...
					int depth = target_depth(surface);
					if(depth == 1) {
						_cairo_output_stream_printf(ctx->stream, "exch pop\n");
					}
					else {
						_cairo_output_stream_printf(ctx->stream, "%d -1 roll pop\n", depth);
					}
				}
				...
				if(link == NULL) {
					...
					cairo_list_swap(&link->operand.link, &surface->operand.link);
		...
		if(surface->defined) {
			_cairo_output_stream_printf(ctx->stream, "/s%u undef\n", surface->base.unique_id);
		}
	}
	if(status == CAIRO_STATUS_SUCCESS)
    ...
    cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, double tolerance, cairo_antialias_t antialias)
{
	...
	if(surface->width >= 0 && surface->height >= 0 && _cairo_path_fixed_is_box(path, &box)) {
		if(box.p1.x <= 0 && box.p1.y <= 0 && box.p2.x >= _cairo_fixed_from_double(surface->width) &&
...
static cairo_status_t FASTCALL active(cairo_script_surface_t * surface)
{
	cairo_status_t status = cairo_device_acquire(surface->base.device);
	if(UNLIKELY(status))
		return status;
...
static void FASTCALL inactive(cairo_script_surface_t * surface)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_list_t sorted;
	...
	while(!cairo_list_is_empty(&sorted)) {
		cairo_list_t * operand;
		struct deferred_finish * df = cairo_list_first_entry(&sorted, struct deferred_finish, link);
		int depth = 0;
		cairo_list_foreach(operand, &ctx->operands) {
			if(operand == &df->operand.link)
				break;
...
static cairo_int_status_t _cairo_script_surface_paint(void * abstract_surface, cairo_operator_t op,
    ...
    const cairo_clip_t * clip)
{
	...
	inactive(surface);
	if(_cairo_surface_wrapper_is_active(&surface->wrapper)) {
		return _cairo_surface_wrapper_stroke(&surface->wrapper, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
	}
	return CAIRO_STATUS_SUCCESS;
BAIL:
	inactive(surface);
    ...
    const cairo_clip_t * clip)
{
	...
	inactive(surface);
	if(_cairo_surface_wrapper_is_active(&surface->wrapper)) {
		return _cairo_surface_wrapper_fill(&surface->wrapper, op, source, path, fill_rule, tolerance, antialias, clip);
	}
	return CAIRO_STATUS_SUCCESS;
BAIL:
	inactive(surface);
...
static boolint _cairo_script_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}
...
static cairo_status_t _emit_font_options(cairo_script_surface_t * surface, cairo_font_options_t * font_options)
{
	...
	_cairo_output_stream_printf(ctx->stream, " >> set-font-options\n");
...
static cairo_status_t _emit_scaled_font(cairo_script_surface_t * surface,
...
static cairo_status_t _emit_scaled_glyph_bitmap(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font,
    cairo_script_font_t * font_private, cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_status_t status;
	ulong index = ++font_private->subset_glyph_index;
	scaled_glyph->dev_private_key = ctx;
	scaled_glyph->dev_private = (void *)index;
	_cairo_output_stream_printf(ctx->stream,
	    "%lu <<\n"
...
static cairo_status_t FASTCALL _emit_scaled_glyph_prologue(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font)
{
	cairo_script_context_t * ctx = to_context(surface);
...
static cairo_status_t _emit_scaled_glyphs(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font, cairo_glyph_t * glyphs, uint num_glyphs)
{
	...
	cairo_status_t status;
	uint n;
	boolint have_glyph_prologue = FALSE;
	if(num_glyphs == 0)
		return CAIRO_STATUS_SUCCESS;
	font_private = _cairo_script_font_get(ctx, scaled_font);
...
static boolint _cairo_script_surface_get_extents(void * abstract_surface, cairo_rectangle_int_t * rectangle)
{
	cairo_script_surface_t * surface = static_cast<cairo_script_surface_t *>(abstract_surface);
	if(_cairo_surface_wrapper_is_active(&surface->wrapper)) {
...
void _cairo_script_context_attach_snapshots(cairo_device_t * device, boolint enable)
{
