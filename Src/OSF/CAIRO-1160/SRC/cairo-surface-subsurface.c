...
cairo_surface_t * cairo_surface_create_for_rectangle(cairo_surface_t * target, double x, double y, double width, double height)
{
	...
	cairo_surface_subsurface_t * surface = static_cast<cairo_surface_subsurface_t *>(SAlloc::M_zon0(sizeof(cairo_surface_subsurface_t)));
	if(UNLIKELY(!surface))
	...
	surface->extents.x = fceili(x);
	surface->extents.y = fceili(y);
	surface->extents.width = ffloori(x + width) - surface->extents.x;
	surface->extents.height = ffloori(y + height) - surface->extents.y;
