...
#if 0
#define D(x)  x
static const char * _opstr(cairo_operator_t op)
{
	const char * ops[] = {
		"CLEAR",
		"SOURCE",
...
static QPainter::CompositionMode _qpainter_compositionmode_from_cairo_op(cairo_operator_t op)
{
	switch(op) {
		case CAIRO_OPERATOR_CLEAR: return QPainter::CompositionMode_Clear;
		case CAIRO_OPERATOR_SOURCE: return QPainter::CompositionMode_Source;
		case CAIRO_OPERATOR_OVER: return QPainter::CompositionMode_SourceOver;
		case CAIRO_OPERATOR_IN: return QPainter::CompositionMode_SourceIn;
		case CAIRO_OPERATOR_OUT: return QPainter::CompositionMode_SourceOut;
		case CAIRO_OPERATOR_ATOP: return QPainter::CompositionMode_SourceAtop;
		case CAIRO_OPERATOR_DEST: return QPainter::CompositionMode_Destination;
		case CAIRO_OPERATOR_DEST_OVER: return QPainter::CompositionMode_DestinationOver;
		case CAIRO_OPERATOR_DEST_IN: return QPainter::CompositionMode_DestinationIn;
		case CAIRO_OPERATOR_DEST_OUT: return QPainter::CompositionMode_DestinationOut;
		case CAIRO_OPERATOR_DEST_ATOP: return QPainter::CompositionMode_DestinationAtop;
		case CAIRO_OPERATOR_XOR: return QPainter::CompositionMode_Xor;
		default:
...
static bool _op_is_supported(cairo_qt_surface_t * qs, cairo_operator_t op)
{
	if(qs->p == NULL)
		return false;
...
static cairo_format_t _cairo_format_from_qimage_format(QImage::Format fmt)
{
	switch(fmt) {
		case QImage::Format_ARGB32_Premultiplied: return CAIRO_FORMAT_ARGB32;
...
static QImage::Format _qimage_format_from_cairo_format(cairo_format_t fmt)
{
	switch(fmt) {
...
static inline QMatrix _qmatrix_from_cairo_matrix(const cairo_matrix_t& m)
{
...
static cairo_status_t _cairo_path_to_qpainterpath_move_to(void * closure, const cairo_point_t * point)
{
...
static cairo_status_t _cairo_path_to_qpainterpath_line_to(void * closure, const cairo_point_t * point)
{
	qpainter_path_data * pdata = static_cast <qpainter_path_data *> (closure);
	double x = _cairo_fixed_to_double(point->x);
	double y = _cairo_fixed_to_double(point->y);
...
static cairo_status_t _cairo_path_to_qpainterpath_curve_to(void * closure,
...
static cairo_status_t _cairo_path_to_qpainterpath_close_path(void * closure)
{
	qpainter_path_data * pdata = static_cast <qpainter_path_data *> (closure);
...
static cairo_status_t _cairo_qt_surface_finish(void * abstract_surface)
{
	cairo_qt_surface_t * qs = (cairo_qt_surface_t*)abstract_surface;
...
static void _qimg_destroy(void * closure)
{
	QImage * qimg = (QImage*)closure;
	delete qimg;
}

static cairo_status_t _cairo_qt_surface_acquire_source_image(void * abstract_surface,
...
static void _cairo_qt_surface_release_source_image(void * abstract_surface,
...
static cairo_surface_t * map_qimage_to_image(QImage * qimg, const cairo_rectangle_int_t * extents)
{
...
static cairo_image_surface_t * _cairo_qt_surface_map_to_image(void * abstract_surface,
...
static cairo_int_status_t _cairo_qt_surface_unmap_image(void * abstract_surface,
    cairo_image_surface_t * image)
{
	cairo_qt_surface_t * qs = (cairo_qt_surface_t*)abstract_surface;
...
static cairo_status_t _cairo_qt_surface_clipper_intersect_clip_path(cairo_surface_clipper_t * clipper,
		...
		if(pattern->type == CAIRO_PATTERN_TYPE_SOLID) {
			...
			if(pattern->type == CAIRO_PATTERN_TYPE_LINEAR) {
				...
				if(r0 == r1) {
					 ...
					 * The CAIRO_EXTENT_REFLECT and CAIRO_EXTEND_REPEAT modes require to compute a
					 *new
					 * radial gradient, with an new outer circle, equal to r1 - r0 in the
					 *CAIRO_EXTEND_REPEAT
					 * case, and 2 * (r1 - r0) in the CAIRO_EXTENT_REFLECT case, and a new gradient
					 *stop
...
struct PatternToPenConverter {
		...
		mBrushConverter(source)
	{
		...
		if(style->dash && style->num_dashes) {
			...
			uint odd_dash = style->num_dashes % 2;

			QVector<qreal> dashes(odd_dash ? style->num_dashes * 2 : style->num_dashes);
			for(uint i = 0; i < odd_dash+1; i++) {
				for(uint j = 0; j < style->num_dashes; j++) {
...
static bool _cairo_qt_fast_fill(cairo_qt_surface_t * qs,
...
static cairo_int_status_t _cairo_qt_surface_paint(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_clip_t * clip)
{
...
static cairo_int_status_t _cairo_qt_surface_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
...
static cairo_int_status_t _cairo_qt_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_path_fixed_t * path,
    const cairo_stroke_style_t * style,
...
static cairo_int_status_t _cairo_qt_surface_show_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    ...
    const cairo_clip_t * clip)
{
	...
	QVarLengthArray<uint> glyphss(num_glyphs);
...
static cairo_int_status_t _cairo_qt_surface_mask(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
...
static cairo_status_t _cairo_qt_surface_mark_dirty(void * abstract_surface,
    int x, int y,
...
cairo_surface_t * cairo_qt_surface_create_with_qpixmap(cairo_content_t content, int width, int height)
{
	cairo_qt_surface_t * qs;
	...
	_cairo_surface_init(&qs->base, &cairo_qt_surface_backend,
	    NULL,              /* device */
	    content);
	_cairo_surface_clipper_init(&qs->clipper, _cairo_qt_surface_clipper_intersect_clip_path);
	qs->pixmap = pixmap;
