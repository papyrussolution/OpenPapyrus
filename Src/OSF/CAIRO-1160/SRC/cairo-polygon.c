...
static void _add_clipped_edge(cairo_polygon_t * polygon, const cairo_point_t * p1, const cairo_point_t * p2, const int top, const int bottom, const int dir)
{
	...
	for(n = 0; n < polygon->num_limits; n++) {
		...
		if(limits->p1.x <= pleft && pright <= limits->p2.x) {
			// Projection of the edge completely contained in the box: clip vertically by restricting top and bottom 
			_add_edge(polygon, p1, p2, top_y, bot_y, dir);
			assert_last_edge_is_valid(polygon, limits);
		}
		else if(pright <= limits->p1.x) {
			// Projection of the edge to the left of the box: replace with the left side of the box (clipped top/bottom)
			_add_edge(polygon, &limits->p1, &bot_left, top_y, bot_y, dir);
			assert_last_edge_is_valid(polygon, limits);
		}
		else if(limits->p2.x <= pleft) {
			// Projection of the edge to the right of the box: replace with the right side of the box (clipped top/bottom)
			 ...
			 * but we need to add edges for the range from top_y to bot_y.
			 *
...
{
	_cairo_polygon_add_edge(static_cast<cairo_polygon_t *>(polygon), p1, p2, 1);
