...
struct cairo_bo_intersect_ordinate_t {
	int32 ordinate;
	enum { 
		EXACT, 
		INEXACT 
...
struct cairo_bo_intersect_point_t {
...
static int edges_compare_x_for_y(const cairo_bo_edge_t * a, const cairo_bo_edge_t * b, int32 y)
{
...
static inline cairo_int128_t det64x32_128(int64 a, int32 b, int64 c, int32 d)
{
	/* det = a * d - b * c */
...
static boolint intersect_lines(cairo_bo_edge_t * a, cairo_bo_edge_t * b, cairo_bo_intersect_point_t * intersection)
{
	...
	int64 R;
	cairo_quorem64_t qr;
	int64 den_det = det32_64(dx1, dy1, dx2, dy2);
...
#else
	intersection->x.exactness = cairo_bo_intersect_ordinate_t::EXACT;
...
#else
	intersection->y.exactness = cairo_bo_intersect_ordinate_t::EXACT;
	if(!_cairo_int64_is_zero(qr.rem)) {
		if(_cairo_int64_negative(den_det) ^ _cairo_int64_negative(qr.rem))
			qr.rem = _cairo_int64_negate(qr.rem);
		qr.rem = _cairo_int64_mul(qr.rem, _cairo_int32_to_int64(2));
		if(_cairo_int64_ge(qr.rem, den_det)) {
			...
			intersection->y.exactness = cairo_bo_intersect_ordinate_t::INEXACT;
	}
#endif
...
static int _cairo_bo_intersect_ordinate_32_compare(cairo_bo_intersect_ordinate_t a, int32 b)
{
	...
	return cairo_bo_intersect_ordinate_t::INEXACT == a.exactness;
}
...
static inline cairo_status_t _cairo_bo_event_queue_insert_if_intersect_below_current_y(cairo_bo_event_queue_t * event_queue, cairo_bo_edge_t * left, cairo_bo_edge_t * right)
{
...
static void _cairo_bo_edge_end(cairo_bo_edge_t * left, int32 bot, cairo_polygon_t * polygon)
{
...
static inline void _cairo_bo_edge_start_or_continue(cairo_bo_edge_t * left, cairo_bo_edge_t * right, int top, cairo_polygon_t * polygon)
{
...
static inline void _active_edges_to_polygon(cairo_bo_edge_t * left, int32 top, cairo_fill_rule_t fill_rule, cairo_polygon_t * polygon)
{
	cairo_bo_edge_t * right;
	uint mask;
...
static cairo_status_t _cairo_bentley_ottmann_tessellate_bo_edges(cairo_bo_event_t  ** start_events,
    int num_events, cairo_fill_rule_t fill_rule, cairo_polygon_t * polygon)
{
	...
	while((event = _cairo_bo_event_dequeue(&event_queue))) {
		if(event->point.y != sweep_line.current_y) {
			_active_edges_to_polygon(sweep_line.head, sweep_line.current_y, fill_rule, polygon);
