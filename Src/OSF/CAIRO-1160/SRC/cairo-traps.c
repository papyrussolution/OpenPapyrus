    ...
    const cairo_line_t * _left, const cairo_line_t * _right)
{
	...
	if(traps->num_limits) {
		...
		// Trivially reject if trapezoid is entirely to the right or to the left of the limits. 
...
void _cairo_traps_extents(const cairo_traps_t * traps,
    cairo_box_t * extents)
{
	int i;
...
cairo_private cairo_status_t _cairo_traps_path(const cairo_traps_t * traps,
...
cairo_int_status_t _cairo_rasterise_polygon_to_traps(cairo_polygon_t * polygon, cairo_fill_rule_t fill_rule,
    cairo_antialias_t antialias, cairo_traps_t * traps)
{
