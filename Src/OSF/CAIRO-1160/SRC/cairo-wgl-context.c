...
static void _wgl_acquire(void * abstract_ctx)
{
	cairo_wgl_context_t * ctx = (cairo_wgl_context_t *)abstract_ctx;
	HDC current_dc;
...
static void _wgl_make_current(void * abstract_ctx, cairo_gl_surface_t * abstract_surface)
{
	cairo_wgl_context_t * ctx = (cairo_wgl_context_t *)abstract_ctx;
...
static void _wgl_release(void * abstract_ctx)
{
	cairo_wgl_context_t * ctx = (cairo_wgl_context_t *)abstract_ctx;
	if(ctx->prev_dc != wglGetCurrentDC() || ctx->prev_rc != wglGetCurrentContext()) {
		wglMakeCurrent(ctx->prev_dc,
		    ctx->prev_rc);
	}
}

static void _wgl_swap_buffers(void * abstract_ctx, cairo_gl_surface_t * abstract_surface)
{
	cairo_wgl_surface_t * surface = (cairo_wgl_surface_t*)abstract_surface;
	SwapBuffers(surface->dc);
}

static void _wgl_destroy(void * abstract_ctx)
{
	cairo_wgl_context_t * ctx = (cairo_wgl_context_t *)abstract_ctx;
	if(ctx->dummy_dc != 0) {
...
static cairo_status_t _wgl_dummy_ctx(cairo_wgl_context_t * ctx)
{
	WNDCLASSEXA wincl;
