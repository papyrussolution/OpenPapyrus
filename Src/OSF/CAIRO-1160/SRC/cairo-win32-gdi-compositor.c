...
static cairo_status_t copy_boxes(cairo_win32_display_surface_t * dst, const cairo_pattern_t * source, cairo_boxes_t * boxes)
{
...
static cairo_status_t upload_boxes(cairo_win32_display_surface_t * dst, const cairo_pattern_t * source, cairo_boxes_t * boxes)
{
	const cairo_surface_pattern_t * pattern;
...
static cairo_status_t opacity_boxes(cairo_composite_rectangles_t * composite, cairo_boxes_t * boxes)
{
	...
	if(!can_alpha_blend(dst))
		return CAIRO_INT_STATUS_UNSUPPORTED;
	return alpha_blend_boxes(dst, src, boxes, composite->mask_pattern.solid.color.alpha_short >> 8);
}
...
static boolint check_blit(cairo_composite_rectangles_t * composite)
{
	...
	if(dst->fallback)
		return FALSE;
	if(dst->win32.format != CAIRO_FORMAT_RGB24 && dst->win32.format != CAIRO_FORMAT_ARGB32)
		return FALSE;
...
static cairo_int_status_t _cairo_win32_gdi_compositor_paint(const cairo_compositor_t * compositor, cairo_composite_rectangles_t * composite)
{
...
static cairo_int_status_t _cairo_win32_gdi_compositor_mask(const cairo_compositor_t * compositor, cairo_composite_rectangles_t * composite)
{
	cairo_int_status_t status = CAIRO_INT_STATUS_UNSUPPORTED;
	if(check_blit(composite)) {
		cairo_boxes_t boxes;
...
static cairo_int_status_t _cairo_win32_gdi_compositor_stroke(const cairo_compositor_t * compositor, cairo_composite_rectangles_t * composite,
...
static cairo_int_status_t _cairo_win32_gdi_compositor_fill(const cairo_compositor_t * compositor,
    cairo_composite_rectangles_t * composite, const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule,
    double tolerance, cairo_antialias_t antialias)
{
	cairo_int_status_t status = CAIRO_INT_STATUS_UNSUPPORTED;
    ...
    cairo_composite_rectangles_t * composite, cairo_scaled_font_t * scaled_font, cairo_glyph_t * glyphs, int num_glyphs, boolint overlap)
{
	...
	if(check_blit(composite) && check_glyphs(composite, scaled_font)) {
		...
		if(status)
			return status;
		status = _cairo_win32_surface_emit_glyphs(&dst->win32, &composite->source_pattern.base, glyphs, num_glyphs, scaled_font, TRUE);
