...
extern const cairo_surface_backend_t cairo_recording_surface_backend;
...
static int bbtree_left_or_right(struct cairo_recording_surface_t::bbtree * bbt, const cairo_box_t * box)
{
	int left, right;
	if(bbt->left) {
...
{
	struct cairo_recording_surface_t::bbtree * bbt = static_cast<struct cairo_recording_surface_t::bbtree *>(SAlloc::M_zon0(sizeof(*bbt)));
	if(bbt == NULL)
		return NULL;
	bbt->extents = *box;
...
static void bbtree_init(struct cairo_recording_surface_t::bbtree * bbt, cairo_command_header_t * header)
{
...
static boolint FASTCALL box_outside(const cairo_box_t * a, const cairo_box_t * b)
{
	return (a->p1.x >= b->p2.x || a->p1.y >= b->p2.y || a->p2.x <= b->p1.x || a->p2.y <= b->p1.y);
}

static void bbtree_foreach_mark_visible(struct cairo_recording_surface_t::bbtree * bbt, const cairo_box_t * box, uint ** indices)
{
...
static inline int intcmp(const uint a, const uint b)
{
	return a - b;
}

CAIRO_COMBSORT_DECLARE(sort_indices, uint, intcmp)

static inline int sizecmp(uint a, uint b, void * pExtra)
{
	cairo_command_header_t ** pp_elements = (cairo_command_header_t **)pExtra;
	const cairo_rectangle_int_t * r;
	r = &pp_elements[a]->extents;
	a = r->width * r->height;
	r = &pp_elements[b]->extents;
	b = r->width * r->height;
	return b - a;
}

CAIRO_COMBSORT_DECLARE_WITH_DATA(sort_commands, uint, sizecmp)
...
static void _cairo_recording_surface_destroy_bbtree(cairo_recording_surface_t * surface)
{
	...
	elements = (cairo_command_t **)_cairo_array_index(&surface->commands, 0);
...
{
	cairo_recording_surface_t * surface = (cairo_recording_surface_t *)SAlloc::M_zon0(sizeof(cairo_recording_surface_t));
	if(UNLIKELY(!surface))
	...
	if(extents) {
		...
		surface->extents.y = static_cast<int>(floor(extents->y));
		surface->extents.width = static_cast<int>(ceil(extents->x + extents->width) - surface->extents.x);
		surface->extents.height = static_cast<int>(ceil(extents->y + extents->height) - surface->extents.y);
...
{
	cairo_recording_surface_t * surface = static_cast<cairo_recording_surface_t *>(abstract_surface);
	cairo_command_t ** elements;
	int i, num_elements;
	...
	for(i = 0; i < num_elements; i++) {
		...
		switch(command->header.type) {
			...
			case CAIRO_COMMAND_TAG:
			    SAlloc::F(command->tag.tag_name);
			    if(command->tag.begin) {
				    SAlloc::F(command->tag.attributes);
...
static cairo_status_t _cairo_recording_surface_acquire_source_image(void * abstract_surface, cairo_image_surface_t ** image_out, void ** image_extra)
{
	...
	image = _cairo_image_surface_create_with_content(surface->base.content, surface->extents.width, surface->extents.height);
	if(UNLIKELY(image->status))
		return image->status;
	/* Handle recursion by returning future reads from the current image */
	proxy = attach_proxy(static_cast<cairo_surface_t *>(abstract_surface), image);
	status = _cairo_recording_surface_replay(&surface->base, image);
	detach_proxy(static_cast<cairo_surface_t *>(abstract_surface), proxy);
	if(UNLIKELY(status)) {
...
static cairo_int_status_t _cairo_recording_surface_paint(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
	...
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_paint_t *)SAlloc::M_zon0(sizeof(cairo_command_paint_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
	status = _command_init(surface, &command->header, CAIRO_COMMAND_PAINT, op, &composite);
	if(UNLIKELY(status))
    ...
    const cairo_pattern_t * source, const cairo_pattern_t * mask, const cairo_clip_t * clip)
{
	...
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_mask_t *)SAlloc::M_zon0(sizeof(cairo_command_mask_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
...
static cairo_int_status_t _cairo_recording_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
    ...
    const cairo_clip_t * clip)
{
	...
	cairo_composite_rectangles_t composite;
	TRACE((stderr, "%s: surface=%d\n", __FUNCTION__, surface->base.unique_id));
	status = _cairo_composite_rectangles_init_for_stroke(&composite, &surface->base,
		op, source, path, style, ctm, clip);
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_stroke_t *)SAlloc::M_zon0(sizeof(cairo_command_stroke_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
	status = _command_init(surface, &command->header, CAIRO_COMMAND_STROKE, op, &composite);
	if(UNLIKELY(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(UNLIKELY(status))
...
static cairo_int_status_t _cairo_recording_surface_fill(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source,
    const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	...
	cairo_composite_rectangles_t composite;
	TRACE((stderr, "%s: surface=%d\n", __FUNCTION__, surface->base.unique_id));
	status = _cairo_composite_rectangles_init_for_fill(&composite, &surface->base, op, source, path, clip);
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_fill_t *)SAlloc::M_zon0(sizeof(cairo_command_fill_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
	status = _command_init(surface, &command->header, CAIRO_COMMAND_FILL, op, &composite);
	if(UNLIKELY(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(UNLIKELY(status))
...
static boolint _cairo_recording_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}

static cairo_int_status_t _cairo_recording_surface_show_text_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const char * utf8,
    int utf8_len,
    ...
    const cairo_clip_t * clip)
{
	...
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_show_text_glyphs_t *)SAlloc::M_zon0(sizeof(cairo_command_show_text_glyphs_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
	...
	if(utf8_len) {
		command->utf8 = (char *)SAlloc::M_zon0(utf8_len);
...
static cairo_int_status_t _cairo_recording_surface_tag(void * abstract_surface,
    boolint begin,
    const char * tag_name,
    ...
    const cairo_clip_t * clip)
{
	...
	if(UNLIKELY(status))
		return status;
	command = (cairo_command_tag_t *)SAlloc::C(1, sizeof(cairo_command_tag_t));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_COMPOSITE;
	}
	status = _command_init(surface, &command->header, CAIRO_COMMAND_TAG, CAIRO_OPERATOR_SOURCE, &composite);
	if(UNLIKELY(status))
		goto CLEANUP_COMMAND;
	command->begin = begin;
...
CLEANUP_SOURCE:
	_cairo_pattern_fini(&command->source.base);
CLEANUP_STRINGS:
	SAlloc::F(command->tag_name);
	SAlloc::F(command->attributes);
CLEANUP_COMMAND:
	_cairo_clip_destroy(command->header.clip);
	SAlloc::F(command);
CLEANUP_COMPOSITE:
...
static void _command_init_copy(cairo_recording_surface_t * surface,
...
{
	cairo_status_t status;
	cairo_command_paint_t * command = (cairo_command_paint_t *)SAlloc::M_zon0(sizeof(*command));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
	_command_init_copy(surface, &command->header, &src->header);
	status = _cairo_pattern_init_copy(&command->source.base, &src->paint.source.base);
	if(UNLIKELY(status))
		goto err_command;
	status = _cairo_recording_surface_commit(surface, &command->header);
	if(UNLIKELY(status))
...
{
	cairo_status_t status;
	cairo_command_mask_t * command = (cairo_command_mask_t *)SAlloc::M_zon0(sizeof(*command));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
	_command_init_copy(surface, &command->header, &src->header);
	status = _cairo_pattern_init_copy(&command->source.base, &src->mask.source.base);
	if(UNLIKELY(status))
		goto err_command;
	status = _cairo_pattern_init_copy(&command->mask.base, &src->mask.mask.base);
	if(UNLIKELY(status))
		goto err_source;
	status = _cairo_recording_surface_commit(surface, &command->header);
	if(UNLIKELY(status))
...
{
	cairo_status_t status;
	cairo_command_stroke_t * command = static_cast<cairo_command_stroke_t *>(SAlloc::M_zon0(sizeof(*command)));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
	...
	if(UNLIKELY(status))
		goto err_source;
	status = _cairo_stroke_style_init_copy(&command->style, &src->stroke.style);
	if(UNLIKELY(status))
		goto err_path;
...
{
	cairo_status_t status;
	cairo_command_fill_t * command = (cairo_command_fill_t *)SAlloc::M_zon0(sizeof(*command));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
	_command_init_copy(surface, &command->header, &src->header);
	status = _cairo_pattern_init_copy(&command->source.base, &src->fill.source.base);
	if(UNLIKELY(status))
		goto err_command;
...
{
	cairo_status_t status;
	cairo_command_show_text_glyphs_t * command = (cairo_command_show_text_glyphs_t *)SAlloc::M_zon0(sizeof(*command));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
	_command_init_copy(surface, &command->header, &src->header);
	status = _cairo_pattern_init_copy(&command->source.base, &src->show_text_glyphs.source.base);
	if(UNLIKELY(status))
		goto err_command;
	command->utf8 = NULL;
	...
	if(command->utf8_len) {
		command->utf8 = (char *)SAlloc::M_zon0(command->utf8_len);
		if(UNLIKELY(command->utf8 == NULL)) {
			status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
			goto err_arrays;
		}
	...
	if(command->num_clusters) {
		...
		memcpy(command->clusters, src->show_text_glyphs.clusters, sizeof(command->clusters[0]) * command->num_clusters);
	}
	command->cluster_flags = src->show_text_glyphs.cluster_flags;
	command->scaled_font = cairo_scaled_font_reference(src->show_text_glyphs.scaled_font);
	status = _cairo_recording_surface_commit(surface, &command->header);
	if(UNLIKELY(status))
		goto err_arrays;
	return CAIRO_STATUS_SUCCESS;
...
{
	cairo_status_t status;
	cairo_command_tag_t * command = (cairo_command_tag_t *)SAlloc::C(1, sizeof(*command));
	if(UNLIKELY(command == NULL)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto err;
	}
...
static cairo_status_t _cairo_recording_surface_copy(cairo_recording_surface_t * dst,
    cairo_recording_surface_t * src)
{
	...
	elements = (cairo_command_t **)_cairo_array_index(&src->commands, 0);
...
static boolint _cairo_recording_surface_get_extents(void * abstract_surface,
...
cairo_int_status_t _cairo_recording_surface_get_path(cairo_surface_t * abstract_surface,
...
static int _cairo_recording_surface_get_visible_commands(cairo_recording_surface_t * surface,
    const cairo_rectangle_int_t * extents)
{
	uint num_visible, * indices;
	cairo_box_t box;
...
static void _cairo_recording_surface_merge_source_attributes(cairo_recording_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * source)
{
...
static cairo_status_t _cairo_recording_surface_replay_internal(cairo_recording_surface_t * surface,
    const cairo_rectangle_int_t * surface_extents,
    const cairo_matrix_t * surface_transform,
    cairo_surface_t * target,
    const cairo_clip_t * target_clip,
    boolint surface_is_unbounded,
...
cairo_status_t _cairo_recording_surface_replay_one(cairo_recording_surface_t * surface,
    long unsigned index,
    cairo_surface_t * target)
{
	cairo_surface_wrapper_t wrapper;
 ...
 **/
cairo_status_t _cairo_recording_surface_replay(cairo_surface_t * surface,
    cairo_surface_t * target)
{
...
cairo_status_t _cairo_recording_surface_replay_with_clip(cairo_surface_t * surface,
    const cairo_matrix_t * surface_transform,
    cairo_surface_t * target,
    const cairo_clip_t * target_clip)
{
 ...
 */
cairo_status_t _cairo_recording_surface_replay_and_create_regions(cairo_surface_t * surface,
    const cairo_matrix_t * surface_transform,
    cairo_surface_t * target,
    boolint surface_is_unbounded)
{
...
cairo_status_t _cairo_recording_surface_replay_region(cairo_surface_t * surface,
    const cairo_rectangle_int_t * surface_extents,
    cairo_surface_t * target,
    cairo_recording_region_type_t region)
{
...
static cairo_status_t _recording_surface_get_ink_bbox(cairo_recording_surface_t * surface,
    cairo_box_t * bbox,
 ...
 * Since: 1.12
 **/
boolint cairo_recording_surface_get_extents(cairo_surface_t * surface, cairo_rectangle_t * extents)
{
...
boolint _cairo_recording_surface_has_only_bilevel_alpha(cairo_recording_surface_t * surface)
{
	return surface->has_bilevel_alpha;
}

boolint _cairo_recording_surface_has_only_op_over(cairo_recording_surface_t * surface)
{
