...
static void _cairo_win32_printing_surface_init_ps_mode(cairo_win32_printing_surface_t * surface)
{
...
static void _cairo_win32_printing_surface_done_solid_brush(cairo_win32_printing_surface_t * surface)
{
...
static cairo_status_t _cairo_win32_printing_surface_paint_recording_pattern(cairo_win32_printing_surface_t * surface, cairo_surface_pattern_t * pattern)
{
	...
	if(extend == CAIRO_EXTEND_REPEAT || extend == CAIRO_EXTEND_REFLECT) {
		...
		top = ffloori(clip.top / _cairo_fixed_to_double(bbox.p2.y - bbox.p1.y));
		bottom = fceili(clip.bottom / _cairo_fixed_to_double(bbox.p2.y - bbox.p1.y));
	}
	else {
		left = 0;
		right = 1;
    ...
    const cairo_pattern_t * pattern, const cairo_rectangle_int_t * extents)
{
	...
	status = _cairo_win32_printing_surface_acquire_image_pattern(surface, pattern, extents,
		&image_pattern, &pattern_width, &pattern_height, &image_extra);
	if(status)
		return status;
	...
	mime_type = BI_JPEG;
	status = _cairo_win32_printing_surface_check_jpeg(surface, image_pattern.surface, &mime_data, &mime_size, &mime_info);
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		mime_type = BI_PNG;
		status = _cairo_win32_printing_surface_check_png(surface, image_pattern.surface, &mime_data, &mime_size, &mime_info);
	}
	...
	if(pattern->extend == CAIRO_EXTEND_REPEAT || pattern->extend == CAIRO_EXTEND_REFLECT) {
		left = ffloori(clip.left / (double)opaque_image->width);
		right = fceili(clip.right / (double)opaque_image->width);
		top = ffloori(clip.top / (double)opaque_image->height);
		bottom = fceili(clip.bottom / (double)opaque_image->height);
	}
	else {
		left = 0;
		right = 1;
    ...
    cairo_linear_pattern_t * pattern)
{
	...
	GetClipBox(surface->win32.dc, &clip);
	if(extend == CAIRO_EXTEND_REPEAT || extend == CAIRO_EXTEND_REFLECT) {
		range_start = ffloori(clip.left / d);
		range_stop = fceili(clip.right / d);
	}
	else {
		range_start = 0;
		range_stop = 1;
...
static void _cairo_win32_printing_surface_get_font_options(void * abstract_surface,
    ...
    const cairo_clip_t * clip)
{
	...
	if(source->type == CAIRO_PATTERN_TYPE_SOLID) {
		...
		color = RGB(0, 0, 0); // Color not used as the pen will only be used by WidenPath() 
	}
	...
	pen = ExtCreatePen(pen_style, static_cast<DWORD>(scale * style->line_width), &brush, style->num_dashes, dash_array);
	if(pen == NULL) {
    ...
    const cairo_clip_t * clip)
{
	...
	EndPath(surface->win32.dc);
	switch(fill_rule) {
		case CAIRO_FILL_RULE_WINDING: SetPolyFillMode(surface->win32.dc, WINDING); break;
		case CAIRO_FILL_RULE_EVEN_ODD: SetPolyFillMode(surface->win32.dc, ALTERNATE); break;
    ...
    const cairo_clip_t * clip)
{
	...
	if(surface->has_ctm) {
		for(i = 0; i < num_glyphs; i++)
			cairo_matrix_transform_point(&surface->ctm, &glyphs[i].P.x, &glyphs[i].P.y);
	...
	for(i = 0; i < num_glyphs; i++) {
		status = _cairo_scaled_font_subsets_map_glyph(surface->font_subsets, scaled_font, glyphs[i].index, NULL, 0, &subset_glyph);
		if(status)
			goto fail;
    ...
    const cairo_clip_t * clip)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(status)
			break;
		surface->ctm = old_ctm;
		cairo_matrix_translate(&surface->ctm, glyphs[i].P.x, glyphs[i].P.y);
