...
#if CAIRO_HAS_XCB_SHM_FUNCTIONS
	cairo_xcb_shm_info_t * shm_info;
	shm_info = _cairo_user_data_array_get_data(&image->base.user_data, (const cairo_user_data_key_t*)surface->connection);
	if(shm_info != NULL) {
		...
		for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
			...
			for(i = 0; i < chunk->count; i++) {
				...
				_cairo_xcb_connection_shm_put_image(surface->connection, surface->drawable, gc,
				    surface->width, surface->height, x, y, width, height, x, y,
				    image->depth, shm_info->shm, shm_info->offset);
			}
		}
...
static cairo_status_t _put_image_boxes(cairo_xcb_surface_t * surface,
    cairo_image_surface_t * image,
    cairo_boxes_t * boxes)
{
	cairo_int_status_t status = CAIRO_INT_STATUS_SUCCESS;
...
static cairo_status_t _cairo_xcb_surface_flush(void * abstract_surface,
    unsigned flags)
{
	cairo_xcb_surface_t * surface = abstract_surface;
	cairo_status_t status;
 ...
 * Since: 1.12
 **/
void cairo_xcb_surface_set_drawable(cairo_surface_t * abstract_surface, xcb_drawable_t drawable, int width, int height)
{
	cairo_xcb_surface_t * surface;
	if(unlikely(abstract_surface->status))
		return;
