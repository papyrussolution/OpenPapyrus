 ...
 *	Alexandros Frantzis <alexandros.frantzis@linaro.org> Henry Song <hsong@sisa.samsung.com> Martin Robinson <mrobinson@igalia.com>
 */
#include "cairoint.h"
#pragma hdrstop
...
static cairo_int_status_t _blit_texture_to_renderbuffer(cairo_gl_surface_t * surface)
{
	...
	status = _cairo_gl_composite_set_operator(&setup, CAIRO_OPERATOR_SOURCE, FALSE);
	if(status)
		return status;
	setup.dst = surface;
...
cairo_int_status_t _cairo_gl_composite_set_source(cairo_gl_composite_t * setup,
...
void _cairo_gl_composite_set_source_operand(cairo_gl_composite_t * setup,
...
void _cairo_gl_composite_set_solid_source(cairo_gl_composite_t * setup,
...
cairo_int_status_t _cairo_gl_composite_set_mask(cairo_gl_composite_t * setup,
    const cairo_pattern_t * pattern,
    const cairo_rectangle_int_t * sample,
    const cairo_rectangle_int_t * extents,
    boolint use_texgen)
...
void _cairo_gl_composite_set_mask_operand(cairo_gl_composite_t * setup,
...
void _cairo_gl_composite_set_spans(cairo_gl_composite_t * setup)
{
	setup->spans = TRUE;
}

void _cairo_gl_composite_set_multisample(cairo_gl_composite_t * setup)
{
	setup->multisample = TRUE;
}

void _cairo_gl_composite_set_clip_region(cairo_gl_composite_t * setup,
    cairo_region_t * clip_region)
{
	setup->clip_region = clip_region;
}

void _cairo_gl_composite_set_clip(cairo_gl_composite_t * setup,
    cairo_clip_t * clip)
{
	setup->clip = clip;
}

static void _cairo_gl_composite_bind_to_shader(cairo_gl_context_t * ctx,
    cairo_gl_composite_t * setup)
{
...
static void _cairo_gl_texture_set_filter(cairo_gl_context_t * ctx,
    GLuint target,
...
void _cairo_gl_context_destroy_operand(cairo_gl_context_t * ctx,
...
static cairo_status_t _cairo_gl_composite_begin_component_alpha(cairo_gl_context_t * ctx,
    cairo_gl_composite_t * setup)
{
...
static void _scissor_to_doubles(cairo_gl_surface_t * surface,
    double x1, double y1,
    double x2, double y2)
{
	double height;
...
void _cairo_gl_scissor_to_rectangle(cairo_gl_surface_t * surface,
    const cairo_rectangle_int_t * r)
{
	_scissor_to_doubles(surface, r->x, r->y, r->x+r->width, r->y+r->height);
}

static void _scissor_to_box(cairo_gl_surface_t * surface,
...
static void _disable_stencil_buffer(void)
{
	glDisable(GL_STENCIL_TEST);
	glDepthMask(GL_FALSE);
}

static cairo_int_status_t _cairo_gl_composite_setup_painted_clipping(cairo_gl_composite_t * setup,
...
static cairo_int_status_t _cairo_gl_composite_setup_clipping(cairo_gl_composite_t * setup,
    cairo_gl_context_t * ctx,
    int vertex_size)
{
...
cairo_status_t _cairo_gl_set_operands_and_operator(cairo_gl_composite_t * setup,
    cairo_gl_context_t * ctx)
{
...
cairo_status_t _cairo_gl_composite_begin(cairo_gl_composite_t * setup,
...
static inline void _cairo_gl_composite_draw_tristrip(cairo_gl_context_t * ctx)
{
...
static inline void _cairo_gl_composite_draw_triangles(cairo_gl_context_t * ctx,
    uint count)
{
	if(!ctx->pre_shader) {
...
static void _cairo_gl_composite_draw_triangles_with_clip_region(cairo_gl_context_t * ctx,
    uint count)
{
...
static void _cairo_gl_composite_unmap_vertex_buffer(cairo_gl_context_t * ctx)
{
	ctx->vb_offset = 0;
}

void _cairo_gl_composite_flush(cairo_gl_context_t * ctx)
{
	uint count;
	int i;
	if(_cairo_gl_context_is_flushed(ctx))
...
static inline void _cairo_gl_composite_emit_alpha_vertex(cairo_gl_context_t * ctx, GLfloat x, GLfloat y, uint8 alpha)
{
	GLfloat * vb = (GLfloat*)(void *)&ctx->vb[ctx->vb_offset];
	union fi {
		float f;
...
{
	_cairo_gl_composite_prepare_buffer(ctx, 6, CAIRO_GL_PRIMITIVE_TYPE_TRIANGLES);
...
cairo_gl_emit_rect_t _cairo_gl_context_choose_emit_rect(cairo_gl_context_t * ctx)
{
	return _cairo_gl_composite_emit_rect;
}

void _cairo_gl_context_emit_rect(cairo_gl_context_t * ctx, GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2)
{
	_cairo_gl_composite_emit_rect(ctx, x1, y1, x2, y2);
}

static void _cairo_gl_composite_emit_span(cairo_gl_context_t * ctx, GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2, uint8 alpha)
{
	_cairo_gl_composite_prepare_buffer(ctx, 6, CAIRO_GL_PRIMITIVE_TYPE_TRIANGLES);
...
static void _cairo_gl_composite_emit_solid_span(cairo_gl_context_t * ctx, GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2, uint8 alpha)
{
	GLfloat * v;
	union fi {
		float f;
...
cairo_gl_emit_span_t _cairo_gl_context_choose_emit_span(cairo_gl_context_t * ctx)
{
...
static inline void _cairo_gl_composite_emit_glyph_vertex(cairo_gl_context_t * ctx,
    GLfloat x, GLfloat y,
...
static void _cairo_gl_composite_emit_glyph(cairo_gl_context_t * ctx, GLfloat x1, GLfloat y1,
    GLfloat x2, GLfloat y2, GLfloat glyph_x1, GLfloat glyph_y1, GLfloat glyph_x2, GLfloat glyph_y2)
{
	_cairo_gl_composite_prepare_buffer(ctx, 6, CAIRO_GL_PRIMITIVE_TYPE_TRIANGLES);
...
static void _cairo_gl_composite_emit_solid_glyph(cairo_gl_context_t * ctx, GLfloat x1, GLfloat y1,
    GLfloat x2, GLfloat y2, GLfloat glyph_x1, GLfloat glyph_y1, GLfloat glyph_x2, GLfloat glyph_y2)
{
	GLfloat * v;
	_cairo_gl_composite_prepare_buffer(ctx, 6, CAIRO_GL_PRIMITIVE_TYPE_TRIANGLES);
...
cairo_gl_emit_glyph_t _cairo_gl_context_choose_emit_glyph(cairo_gl_context_t * ctx)
{
	switch(ctx->operands[CAIRO_GL_TEX_SOURCE].type) {
		default:
		case CAIRO_GL_OPERAND_COUNT:
...
void _cairo_gl_composite_fini(cairo_gl_composite_t * setup)
{
	_cairo_gl_operand_destroy(&setup->src);
	_cairo_gl_operand_destroy(&setup->mask);
}
...
cairo_status_t _cairo_gl_composite_init(cairo_gl_composite_t * setup, cairo_operator_t op, cairo_gl_surface_t * dst, boolint assume_component_alpha)
{
	...
	memzero(setup, sizeof(cairo_gl_composite_t));
	status = _cairo_gl_composite_set_operator(setup, op, assume_component_alpha);
	if(status)
		return status;
	setup->dst = dst;
...
static cairo_int_status_t _cairo_gl_composite_append_vertex_indices(cairo_gl_context_t * ctx,
...
cairo_int_status_t _cairo_gl_composite_emit_quad_as_tristrip(cairo_gl_context_t * ctx,
    cairo_gl_composite_t * setup,
...
cairo_int_status_t _cairo_gl_composite_emit_triangle_as_tristrip(cairo_gl_context_t * ctx, cairo_gl_composite_t * setup, const cairo_point_t triangle[3])
{
	_cairo_gl_composite_prepare_buffer(ctx, 3, CAIRO_GL_PRIMITIVE_TYPE_TRISTRIPS);
