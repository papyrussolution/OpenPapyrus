...
typedef struct _cairo_sub_font {
	...
	boolint is_user;
	boolint use_latin_subset;
	boolint reserve_notdef;
...
static cairo_sub_font_glyph_t * _cairo_sub_font_glyph_create(ulong scaled_font_glyph_index,
    ...
    int utf8_len)
{
	cairo_sub_font_glyph_t * sub_font_glyph = (cairo_sub_font_glyph_t *)SAlloc::M_zon0(sizeof(cairo_sub_font_glyph_t));
...
static boolint _cairo_sub_fonts_equal(const void * key_a, const void * key_b)
{
...
{
	int i;
	cairo_sub_font_t * sub_font = (cairo_sub_font_t *)SAlloc::M_zon0(sizeof(cairo_sub_font_t));
...
static cairo_status_t _cairo_sub_font_glyph_map_to_unicode(cairo_sub_font_glyph_t * sub_font_glyph, const char * utf8, int utf8_len, boolint * is_mapped)
{
	*is_mapped = FALSE;
	if(utf8_len < 0)
		return CAIRO_STATUS_SUCCESS;
	if(utf8 && utf8_len != 0 && utf8[utf8_len - 1] == '\0')
		utf8_len--;
	if(utf8 && utf8_len != 0) {
		if(sub_font_glyph->utf8) {
			if(utf8_len == sub_font_glyph->utf8_len && memcmp(utf8, sub_font_glyph->utf8, utf8_len) == 0) {
			...
			sub_font_glyph->utf8 = (char *)SAlloc::M_zon0(utf8_len + 1);
...
static cairo_int_status_t _cairo_sub_font_lookup_glyph(cairo_sub_font_t * sub_font,
    ulong scaled_font_glyph_index, const char * utf8, int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
...
static cairo_status_t _cairo_sub_font_map_glyph(cairo_sub_font_t * sub_font, ulong scaled_font_glyph_index,
    const char * text_utf8, int text_utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	cairo_sub_font_glyph_t key, * sub_font_glyph;
	cairo_status_t status;
	_cairo_sub_font_glyph_init_key(&key, scaled_font_glyph_index);
	...
	if(sub_font_glyph == NULL) {
		...
		if(text_utf8 && text_utf8_len > 0) {
			...
			if(status == CAIRO_STATUS_SUCCESS) {
				if(ucs4_len == 1) {
					font_unicode = ucs4[0];
					SAlloc::F(font_utf8);
					font_utf8 = (char *)SAlloc::M_zon0(text_utf8_len + 1);
					if(font_utf8 == NULL) {
						SAlloc::F(ucs4);
						return _cairo_error(CAIRO_STATUS_NO_MEMORY);
					}
		...
		is_latin = FALSE;
		latin_character = -1;
		if(sub_font->use_latin_subset && (!_cairo_font_face_is_user(sub_font->scaled_font->font_face))) {
			latin_character = _cairo_unicode_to_winansi(font_unicode);
			if(latin_character > 0 || (latin_character == 0 && sub_font->num_glyphs_in_latin_subset > 0)) {
	...
	subset_glyph->x_advance = sub_font_glyph->x_advance;
	subset_glyph->y_advance = sub_font_glyph->y_advance;
	status = _cairo_sub_font_glyph_map_to_unicode(sub_font_glyph, text_utf8, text_utf8_len, &subset_glyph->utf8_is_mapped);
	subset_glyph->unicode = sub_font_glyph->unicode;
	return status;
}
...
static void _cairo_sub_font_collect(void * entry, void * closure)
{
	...
	for(i = 0; i <= sub_font->current_subset; i++) {
		...
		collection->status = (collection->font_subset_callback)(&subset, collection->font_subset_callback_closure);
		if(subset.glyph_names) {
...
{
	cairo_scaled_font_subsets_t * subsets = (cairo_scaled_font_subsets_t *)SAlloc::M_zon0(sizeof(cairo_scaled_font_subsets_t));
...
cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_scaled(void) { return _cairo_scaled_font_subsets_create_internal(CAIRO_SUBSETS_SCALED); }
cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_simple(void) { return _cairo_scaled_font_subsets_create_internal(CAIRO_SUBSETS_SIMPLE); }
cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_composite(void) { return _cairo_scaled_font_subsets_create_internal(CAIRO_SUBSETS_COMPOSITE); }
...
void _cairo_scaled_font_subsets_enable_latin_subset(cairo_scaled_font_subsets_t * font_subsets, boolint use_latin)
{
	font_subsets->use_latin_subset = use_latin;
}

cairo_status_t _cairo_scaled_font_subsets_map_glyph(cairo_scaled_font_subsets_t * subsets, cairo_scaled_font_t * scaled_font,
    ulong scaled_font_glyph_index, const char * utf8, int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	 ...
	 * subset. */
	if(scaled_font_glyph_index == 0 || _cairo_font_face_is_user(scaled_font->font_face)) {
		status = CAIRO_STATUS_SUCCESS;
	}
	else {
		_cairo_scaled_font_freeze_cache(scaled_font);
		status = _cairo_scaled_glyph_lookup(scaled_font, scaled_font_glyph_index, CAIRO_SCALED_GLYPH_INFO_PATH, &scaled_glyph);
		_cairo_scaled_font_thaw_cache(scaled_font);
	}
	if(_cairo_int_status_is_error(status))
		return status;
	if(status == CAIRO_INT_STATUS_SUCCESS && subsets->type != CAIRO_SUBSETS_SCALED && !_cairo_font_face_is_user(scaled_font->font_face)) {
		/* Path available. Add to unscaled subset. */
		key.is_scaled = FALSE;
		_cairo_sub_font_init_key(&key, scaled_font);
		sub_font = static_cast<cairo_sub_font_t *>(_cairo_hash_table_lookup(subsets->unscaled_sub_fonts, &key.base));
		if(sub_font == NULL) {
    ...
    cairo_scaled_font_subset_callback_func_t font_subset_callback, void * closure, cairo_subsets_foreach_type_t type)
{
	...
	boolint is_scaled = FALSE;
	boolint is_user = FALSE;
...
{
	return _cairo_scaled_font_subsets_foreach_internal(font_subsets, font_subset_callback, closure, CAIRO_SUBSETS_FOREACH_SCALED);
}

cairo_status_t _cairo_scaled_font_subsets_foreach_unscaled(cairo_scaled_font_subsets_t * font_subsets,
    cairo_scaled_font_subset_callback_func_t font_subset_callback, void * closure)
{
	return _cairo_scaled_font_subsets_foreach_internal(font_subsets, font_subset_callback, closure, CAIRO_SUBSETS_FOREACH_UNSCALED);
}

cairo_status_t _cairo_scaled_font_subsets_foreach_user(cairo_scaled_font_subsets_t * font_subsets,
    cairo_scaled_font_subset_callback_func_t font_subset_callback, void * closure)
{
	return _cairo_scaled_font_subsets_foreach_internal(font_subsets, font_subset_callback, closure, CAIRO_SUBSETS_FOREACH_USER);
}

static boolint _cairo_string_equal(const void * key_a, const void * key_b)
{
...
static void dump_glyph(void * entry, void * closure)
{
...
static void dump_subfont(cairo_sub_font_t * sub_font)
{
	while(sub_font) {
		printf("    font_id: %d\n", sub_font->font_id);
...
void dump_scaled_font_subsets(cairo_scaled_font_subsets_t * font_subsets)
{
	printf("font subsets\n");
	switch(font_subsets->type) {
		case CAIRO_SUBSETS_SCALED: printf("  type: CAIRO_SUBSETS_SCALED\n"); break;
		case CAIRO_SUBSETS_SIMPLE: printf("  type: CAIRO_SUBSETS_SIMPLE\n"); break;
		case CAIRO_SUBSETS_COMPOSITE: printf("  type: CAIRO_SUBSETS_COMPOSITE\n"); break;
	}
...
{
	*entry = static_cast<cairo_string_entry_t *>(SAlloc::M_zon0(sizeof(cairo_string_entry_t)));
...
cairo_int_status_t _cairo_scaled_font_subset_create_glyph_names(cairo_scaled_font_subset_t * subset)
{
	...
	for(; i < subset->num_glyphs; i++) {
		...
		if(utf16_len == 1) {
			...
			entry = static_cast<cairo_string_entry_t *>(_cairo_hash_table_lookup(names, &key.base));
			if(entry)
