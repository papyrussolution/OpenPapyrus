 ...
 *
 * Contributor(s): Keith Packard <keithp@keithp.com> Behdad Esfahbod <behdad@behdad.org>
 */
#include "cairoint.h"
#pragma hdrstop
/*
...
static cairo_status_t twin_font_face_set_properties_from_toy(cairo_font_face_t * twin_face,
...
static cairo_status_t twin_scaled_font_compute_properties(cairo_scaled_font_t * scaled_font, cairo_t * cr)
{
	...
	/* weight */
	props->weight = props->face_props->weight * (FSZ(4) / TWIN_WEIGHT_NORMAL);
	/* pen & margins */
	props->penx = props->peny = props->weight;
	props->marginl = props->marginr = FSZ(4);
...
static void twin_compute_snap(cairo_t * cr, twin_snap_info_t * info, const signed char * b)
{
	...
	for(s = 0; s < n; s++) {
		...
		info->snapped_x[s] = SNAPXI(FSZ(snap[s]));
	}
	...
	for(s = 0; s < n; s++) {
		...
		info->snapped_y[s] = SNAPYI(FSZ(snap[s]));
	}
}
...
static cairo_status_t twin_scaled_font_render_glyph(cairo_scaled_font_t * scaled_font, ulong glyph, cairo_t * cr, cairo_text_extents_t * metrics)
{
