...
static void _cairo_pattern_notify_observers(cairo_pattern_t * pattern, uint flags)
{
...
static cairo_status_t _cairo_mesh_pattern_init_copy(cairo_mesh_pattern_t * pattern, const cairo_mesh_pattern_t * other)
{
	*pattern = *other;
...
cairo_status_t FASTCALL _cairo_pattern_init_copy(cairo_pattern_t * pattern, const cairo_pattern_t * other)
{
	...
	switch(other->type) {
		case CAIRO_PATTERN_TYPE_SOLID: {
		    cairo_solid_pattern_t * dst = reinterpret_cast<cairo_solid_pattern_t *>(pattern);
		...
		case CAIRO_PATTERN_TYPE_MESH: {
		    cairo_mesh_pattern_t * dst = reinterpret_cast<cairo_mesh_pattern_t *>(pattern);
...
void FASTCALL _cairo_pattern_init_static_copy(cairo_pattern_t * pattern, const cairo_pattern_t * other)
{
	...
	switch(other->type) {
		default:
		    ASSERT_NOT_REACHED;
		case CAIRO_PATTERN_TYPE_SOLID: size = sizeof(cairo_solid_pattern_t); break;
		case CAIRO_PATTERN_TYPE_SURFACE: size = sizeof(cairo_surface_pattern_t); break;
		case CAIRO_PATTERN_TYPE_LINEAR: size = sizeof(cairo_linear_pattern_t); break;
		case CAIRO_PATTERN_TYPE_RADIAL: size = sizeof(cairo_radial_pattern_t); break;
		case CAIRO_PATTERN_TYPE_MESH: size = sizeof(cairo_mesh_pattern_t); break;
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: size = sizeof(cairo_raster_source_pattern_t); break;
	}
...
cairo_status_t _cairo_pattern_create_copy(cairo_pattern_t ** pattern_out, const cairo_pattern_t * other)
{
	...
	switch(other->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_solid_pattern_t)));
		    break;
		case CAIRO_PATTERN_TYPE_SURFACE:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_surface_pattern_t)));
		    break;
		case CAIRO_PATTERN_TYPE_LINEAR:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_linear_pattern_t)));
		    break;
		case CAIRO_PATTERN_TYPE_RADIAL:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_radial_pattern_t)));
		    break;
		case CAIRO_PATTERN_TYPE_MESH:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_mesh_pattern_t)));
		    break;
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
		    pattern = static_cast<cairo_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_raster_source_pattern_t)));
		    break;
		default:
		    ASSERT_NOT_REACHED;
...
cairo_pattern_t * _cairo_pattern_create_solid(const cairo_color_t * color)
{
	...
	if(UNLIKELY(!pattern)) {
		...
		pattern = static_cast<cairo_solid_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_solid_pattern_t)));
		if(UNLIKELY(!pattern)) {
...
cairo_pattern_t * cairo_pattern_create_for_surface(cairo_surface_t * surface)
{
	...
	if(UNLIKELY(!pattern)) {
		pattern = static_cast<cairo_surface_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_surface_pattern_t)));
		if(UNLIKELY(!pattern)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
cairo_pattern_t * cairo_pattern_create_linear(double x0, double y0, double x1, double y1)
{
	...
	if(UNLIKELY(!pattern)) {
		pattern = static_cast<cairo_linear_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_linear_pattern_t)));
		if(UNLIKELY(!pattern)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
cairo_pattern_t * cairo_pattern_create_radial(double cx0, double cy0, double radius0, double cx1, double cy1, double radius1)
{
	...
	if(UNLIKELY(!pattern)) {
		pattern = static_cast<cairo_radial_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_radial_pattern_t)));
		if(UNLIKELY(!pattern)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
cairo_pattern_t * cairo_pattern_create_mesh(void)
{
	...
	if(UNLIKELY(!pattern)) {
		pattern = static_cast<cairo_mesh_pattern_t *>(SAlloc::M_zon0(sizeof(cairo_mesh_pattern_t)));
		if(UNLIKELY(!pattern)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
static void _calc_control_point(cairo_mesh_patch_t * patch, int control_point)
{
	...
	int i, j;
	int cp_i = mesh_control_point_i[control_point];
	int cp_j = mesh_control_point_j[control_point];
	for(i = 0; i < 3; i++)
		for(j = 0; j < 3; j++)
...
static cairo_status_t _cairo_pattern_gradient_grow(cairo_gradient_pattern_t * pattern)
{
	...
	// we have a local buffer at pattern->stops_embedded.  try to fulfill the request from there. 
	...
	if(pattern->stops == pattern->stops_embedded) {
		new_stops = static_cast<cairo_gradient_stop_t *>(_cairo_malloc_ab(new_size, sizeof(cairo_gradient_stop_t)));
		if(new_stops)
...
static void _cairo_mesh_pattern_set_corner_color(cairo_mesh_pattern_t * mesh, uint corner_num, double red, double green, double blue, double alpha)
{
	cairo_color_t * color;
...
static void _cairo_pattern_add_color_stop(cairo_gradient_pattern_t * pattern,
    double offset,
    double red,
    double green,
    double blue,
 ...
 * Since: 1.0
 **/
void cairo_pattern_get_matrix(const cairo_pattern_t * pattern, cairo_matrix_t * matrix)
{
 ...
 * Since: 1.0
 **/
cairo_filter_t cairo_pattern_get_filter(const cairo_pattern_t * pattern)
{
 ...
 * Since: 1.0
 **/
cairo_extend_t cairo_pattern_get_extend(const cairo_pattern_t * pattern)
{
...
static boolint _gradient_is_clear(const cairo_gradient_pattern_t * gradient, const cairo_rectangle_int_t * extents)
{
	uint i;
	assert(gradient->base.type == CAIRO_PATTERN_TYPE_LINEAR || gradient->base.type == CAIRO_PATTERN_TYPE_RADIAL);
	if(gradient->n_stops == 0 || (gradient->base.extend == CAIRO_EXTEND_NONE && gradient->stops[0].offset == gradient->stops[gradient->n_stops - 1].offset))
		return TRUE;
...
static void _gradient_color_average(const cairo_gradient_pattern_t * gradient, cairo_color_t * color)
{
	...
	if(gradient->n_stops == 1) {
		_cairo_color_init_rgba(color, gradient->stops[0].color.red, gradient->stops[0].color.green, gradient->stops[0].color.blue, gradient->stops[0].color.alpha);
		return;
	}
		...
		case CAIRO_PATTERN_TYPE_RADIAL: 
			{
				...
				for(uint i = 1; i < gradient->n_stops; i++) {
...
boolint FASTCALL _cairo_gradient_pattern_is_solid(const cairo_gradient_pattern_t * gradient, const cairo_rectangle_int_t * extents, cairo_color_t * color)
{
	...
	if(gradient->base.type == CAIRO_PATTERN_TYPE_LINEAR) {
		...
		return FALSE;
	for(uint i = 1; i < gradient->n_stops; i++)
		if(!_cairo_color_stop_equal(&gradient->stops[0].color, &gradient->stops[i].color))
			return FALSE;
	_cairo_color_init_rgba(color, gradient->stops[0].color.red, gradient->stops[0].color.green, gradient->stops[0].color.blue, gradient->stops[0].color.alpha);
	return TRUE;
}
...
static boolint _gradient_is_opaque(const cairo_gradient_pattern_t * gradient, const cairo_rectangle_int_t * sample)
{
	...
	if(gradient->base.type == CAIRO_PATTERN_TYPE_LINEAR) {
		if(gradient->base.extend == CAIRO_EXTEND_NONE) {
			...
			if(sample == NULL)
				return FALSE;
			_cairo_linear_pattern_box_to_parameter(linear, sample->x, sample->y,
			    sample->x + sample->width, sample->y + sample->height, t);
			if(t[0] < 0.0 || t[1] > 1.0)
				return FALSE;
		}
	}
...
boolint FASTCALL _cairo_pattern_is_opaque(const cairo_pattern_t * abstract_pattern, const cairo_rectangle_int_t * sample)
{
	if(abstract_pattern->has_component_alpha)
		return FALSE;
	const cairo_pattern_union_t * pattern = (const cairo_pattern_union_t*)abstract_pattern;
	switch(pattern->base.type) {
		case CAIRO_PATTERN_TYPE_SOLID: return _cairo_pattern_is_opaque_solid(abstract_pattern);
		case CAIRO_PATTERN_TYPE_SURFACE: return _surface_is_opaque(&pattern->surface, sample);
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: return _raster_source_is_opaque(&pattern->raster_source, sample);
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL: return _gradient_is_opaque(&pattern->gradient.base, sample);
...
boolint FASTCALL _cairo_pattern_is_clear(const cairo_pattern_t * abstract_pattern)
{
	if(abstract_pattern->has_component_alpha)
		...
		switch(abstract_pattern->type) {
			...
			case CAIRO_PATTERN_TYPE_SURFACE: return _surface_is_clear(&pattern->surface);
			case CAIRO_PATTERN_TYPE_RASTER_SOURCE: return _raster_source_is_clear(&pattern->raster_source);
			case CAIRO_PATTERN_TYPE_LINEAR:
			case CAIRO_PATTERN_TYPE_RADIAL: return _gradient_is_clear(&pattern->gradient.base, NULL);
			case CAIRO_PATTERN_TYPE_MESH: return _mesh_is_clear(&pattern->mesh);
		}
		ASSERT_NOT_REACHED;
		return FALSE;
	}
	...
	switch(pattern->filter) {
		...
		case CAIRO_FILTER_FAST:
		    ...
		    if(_cairo_matrix_is_pixel_exact(&pattern->matrix)) {
			    ...
			    if(pattern->filter == CAIRO_FILTER_GOOD && use_bilinear(pattern->matrix.xx, pattern->matrix.xy, pattern->matrix.x0) && 
...
{
	double padx, pady;
	// Assume filters are interpolating, which means identity cannot change the image 
	if(_cairo_matrix_is_identity(&pattern->matrix)) {
		*sample = *extents;
		...
		double x1 = extents->x + 0.5;
		double y1 = extents->y + 0.5;
		double x2 = x1 + (extents->width - 1);
		double y2 = y1 + (extents->height - 1);
		...
		x1 = floor(x1 - padx);
		SETMAX(x1, CAIRO_RECT_INT_MIN);
		sample->x = static_cast<int>(x1);
		y1 = floor(y1 - pady);
		SETMAX(y1, CAIRO_RECT_INT_MIN);
		sample->y = static_cast<int>(y1);
		x2 = floor(x2 + padx) + 1.0;
		SETMIN(x2, CAIRO_RECT_INT_MAX);
		sample->width = static_cast<int>(x2 - x1);
		y2 = floor(y2 + pady) + 1.0;
		SETMIN(y2, CAIRO_RECT_INT_MAX);
		sample->height = static_cast<int>(y2 - y1);
	}
}
/**
...
void _cairo_pattern_get_extents(const cairo_pattern_t * pattern, cairo_rectangle_int_t * extents, boolint is_vector)
{
	...
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
	    {
		    const cairo_raster_source_pattern_t * raster = (const cairo_raster_source_pattern_t*)pattern;
...
HANDLE_FILTER:
		...
		case CAIRO_PATTERN_TYPE_RADIAL:
	    {
		    const cairo_radial_pattern_t * radial = (const cairo_radial_pattern_t*)pattern;
		    double cx1, cy1;
		    double cx2, cy2;
		    double r1, r2;
		...
		case CAIRO_PATTERN_TYPE_LINEAR:
	    {
		    const cairo_linear_pattern_t * linear = (const cairo_linear_pattern_t*)pattern;
		    if(pattern->extend != CAIRO_EXTEND_NONE)
			    goto UNBOUNDED;
		    if(_linear_pattern_is_degenerate(linear)) {
			    // cairo-gstate should have optimised degenerate patterns to solid ones, so we can again ignore them here.
			    goto EMPTY;
		    }
		    // TODO: to get tight extents, use the matrix to transform the pattern instead of transforming the extents later. 
		    ...
		    // The current linear renderer just point-samples in the middle of the pixels, similar to the NEAREST filter:
		    round_x = round_y = TRUE;
	    }
	    break;
		case CAIRO_PATTERN_TYPE_MESH:
	    {
		    const cairo_mesh_pattern_t * mesh = reinterpret_cast<const cairo_mesh_pattern_t *>(pattern);
	...
	ix1 = (x1 < CAIRO_RECT_INT_MIN) ? CAIRO_RECT_INT_MIN : _cairo_lround(x1);
	ix2 = (x2 > CAIRO_RECT_INT_MAX) ? CAIRO_RECT_INT_MAX : _cairo_lround(x2);
	...
	iy1 = (y1 < CAIRO_RECT_INT_MIN) ? CAIRO_RECT_INT_MIN : _cairo_lround(y1);
	iy2 = (y2 > CAIRO_RECT_INT_MAX) ? CAIRO_RECT_INT_MAX : _cairo_lround(y2);
...
static ulong _cairo_solid_pattern_hash(ulong hash, const cairo_solid_pattern_t * solid)
{
	hash = _cairo_hash_bytes(hash, &solid->color, sizeof(solid->color));
	return hash;
}

static ulong _cairo_gradient_color_stops_hash(ulong hash, const cairo_gradient_pattern_t * gradient)
{
...
static ulong _cairo_mesh_pattern_hash(ulong hash, const cairo_mesh_pattern_t * mesh)
{
	const cairo_mesh_patch_t * patch = (const cairo_mesh_patch_t *)_cairo_array_index_const(&mesh->patches, 0);
	uint i, n = _cairo_array_num_elements(&mesh->patches);
	for(i = 0; i < n; i++)
		hash = _cairo_hash_bytes(hash, patch + i, sizeof(cairo_mesh_patch_t));
	return hash;
}

static ulong _cairo_surface_pattern_hash(ulong hash, const cairo_surface_pattern_t * surface)
{
	hash ^= surface->surface->unique_id;
	return hash;
}

static ulong _cairo_raster_source_pattern_hash(ulong hash, const cairo_raster_source_pattern_t * raster)
{
...
ulong _cairo_pattern_hash(const cairo_pattern_t * pattern)
{
	...
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID: return _cairo_solid_pattern_hash(hash, reinterpret_cast<const cairo_solid_pattern_t *>(pattern));
		...
		case CAIRO_PATTERN_TYPE_MESH: return _cairo_mesh_pattern_hash(hash, reinterpret_cast<const cairo_mesh_pattern_t *>(pattern));
...
static boolint _cairo_gradient_color_stops_equal(const cairo_gradient_pattern_t * a, const cairo_gradient_pattern_t * b)
{
	if(a->n_stops != b->n_stops)
		...
		for(uint n = 0; n < a->n_stops; n++) {
			if(a->stops[n].offset != b->stops[n].offset)
				return FALSE;
			else if(!_cairo_color_stop_equal(&a->stops[n].color, &b->stops[n].color))
				return FALSE;
		}
		return TRUE;
	}
...
boolint _cairo_linear_pattern_equal(const cairo_linear_pattern_t * a, const cairo_linear_pattern_t * b)
{
	if(a->pd1.x != b->pd1.x)
		return FALSE;
	else if(a->pd1.y != b->pd1.y)
		return FALSE;
	else if(a->pd2.x != b->pd2.x)
		return FALSE;
	else if(a->pd2.y != b->pd2.y)
		return FALSE;
	else 
...
boolint _cairo_radial_pattern_equal(const cairo_radial_pattern_t * a, const cairo_radial_pattern_t * b)
{
	if(a->cd1.center.x != b->cd1.center.x)
		return FALSE;
	else if(a->cd1.center.y != b->cd1.center.y)
		return FALSE;
	else if(a->cd1.radius != b->cd1.radius)
		return FALSE;
	else if(a->cd2.center.x != b->cd2.center.x)
		return FALSE;
	else if(a->cd2.center.y != b->cd2.center.y)
		return FALSE;
	else if(a->cd2.radius != b->cd2.radius)
		return FALSE;
	else
		return _cairo_gradient_color_stops_equal(&a->base, &b->base);
}
...
{
	const uint num_patches_a = _cairo_array_num_elements(&a->patches);
	const uint num_patches_b = _cairo_array_num_elements(&b->patches);
	if(num_patches_a != num_patches_b)
		return FALSE;
	else {
		for(uint i = 0; i < num_patches_a; i++) {
			const cairo_mesh_patch_t * patch_a = (const cairo_mesh_patch_t *)_cairo_array_index_const(&a->patches, i);
			const cairo_mesh_patch_t * patch_b = (const cairo_mesh_patch_t *)_cairo_array_index_const(&b->patches, i);
...
{
	cairo_solid_pattern_t * solid = reinterpret_cast<cairo_solid_pattern_t *>(pattern);
	double r0, g0, b0, a0;
	if(pattern->status)
		return pattern->status;
	if(pattern->type != CAIRO_PATTERN_TYPE_SOLID)
...
cairo_path_t * cairo_mesh_pattern_get_path(cairo_pattern_t * pattern, uint patch_num)
{
	...
	path = static_cast<cairo_path_t *>(SAlloc::M_zon0(sizeof(cairo_path_t)));
	if(!path)
 ...
 * Since: 1.12
 **/
cairo_status_t cairo_mesh_pattern_get_corner_color_rgba(cairo_pattern_t * pattern, uint patch_num, uint corner_num, double * red, double * green, double * blue, double * alpha)
{
	cairo_mesh_pattern_t * mesh = reinterpret_cast<cairo_mesh_pattern_t *>(pattern);
	uint patch_count;
	const cairo_mesh_patch_t * patch;
...
void _cairo_debug_print_pattern(FILE * file, const cairo_pattern_t * pattern)
{
	...
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_MESH: _cairo_debug_print_mesh_pattern(file, reinterpret_cast<const cairo_mesh_pattern_t *>(pattern)); break;
	}
}
