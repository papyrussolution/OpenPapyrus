...
static void init_paint(struct _cairo_observation::Paint * p)
{
...
static void init_mask(struct _cairo_observation::Mask * m)
{
...
static void init_fill(struct _cairo_observation::Fill * f)
{
...
static void init_stroke(struct _cairo_observation::Stroke * s)
{
...
static void init_glyphs(struct _cairo_observation::Glyphs * g)
{
...
static int FASTCALL classify_pattern(const cairo_pattern_t * pattern, const cairo_surface_t * target)
{
	...
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SURFACE:
		    ...
		    break;
		default:
		case CAIRO_PATTERN_TYPE_SOLID: classify = 3; break;
		case CAIRO_PATTERN_TYPE_LINEAR: classify = 4; break;
		case CAIRO_PATTERN_TYPE_RADIAL: classify = 5; break;
		case CAIRO_PATTERN_TYPE_MESH: classify = 6; break;
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: classify = 7; break;
	}
	return classify;
}
...
static cairo_surface_t * _cairo_surface_observer_create_similar(void * abstract_other,
...
static cairo_image_surface_t * _cairo_surface_observer_map_to_image(void * abstract_surface, const cairo_rectangle_int_t * extents)
{
	cairo_surface_observer_t * surface = static_cast<cairo_surface_observer_t *>(abstract_surface);
...
static void record_target(cairo_observation_record_t * r,
...
static cairo_observation_record_t * record_stroke(cairo_observation_record_t * r, cairo_surface_t * target,
    ...
    const cairo_clip_t * clip, cairo_time_t elapsed)
{
	record_target(r, target);
	r->op = op;
	r->source = classify_pattern(source, target);
...
static cairo_observation_record_t * record_glyphs(cairo_observation_record_t * r, cairo_surface_t * target,
    cairo_operator_t op, const cairo_pattern_t * source, cairo_glyph_t * glyphs, int num_glyphs,
    cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip, cairo_time_t elapsed)
{
	record_target(r, target);
	r->op = op;
	r->source = classify_pattern(source, target);
...
static void add_record_fill(cairo_observation_t * log,
    cairo_surface_t * target,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_path_fixed_t * path,
    ...
    cairo_time_t elapsed)
{
	...
	if(log->record) {
		status = log->record->base.backend->show_text_glyphs(&log->record->base,
			op, source, NULL, 0, glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE,
			scaled_font, clip);
		assert(status == CAIRO_INT_STATUS_SUCCESS);
	}
...
static cairo_int_status_t _cairo_surface_observer_glyphs(void * abstract_surface, cairo_operator_t op,
    const cairo_pattern_t * source, cairo_glyph_t * glyphs, int num_glyphs, cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	status = _cairo_composite_rectangles_init_for_glyphs(&composite, surface->target, op, source, scaled_font, glyphs, num_glyphs, clip, NULL);
	if(UNLIKELY(status)) {
	...
	t = _cairo_time_get();
	status = _cairo_surface_show_text_glyphs(surface->target, op, source, NULL, 0, dev_glyphs, num_glyphs,
		NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font, clip);
	SAlloc::F(dev_glyphs);
	if(UNLIKELY(status))
		return status;
	_cairo_surface_sync(surface->target, x, y);
...
static cairo_surface_t * _cairo_surface_observer_source(void * abstract_surface, cairo_rectangle_int_t * extents)
{
	cairo_surface_observer_t * surface = static_cast<cairo_surface_observer_t *>(abstract_surface);
...
cairo_status_t cairo_surface_observer_add_paint_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	...
	surface = reinterpret_cast<cairo_surface_observer_t *>(abstract_surface);
...
static boolint replay_record(cairo_observation_t * log, cairo_observation_record_t * r, cairo_device_t * script)
{
...
static void _cairo_observation_print(cairo_output_stream_t * stream, cairo_observation_t * log)
{
	...
	_cairo_output_stream_printf(stream, "glyphs: count %d [no-op %d], elasped %f [%f%%]\n", log->glyphs.count, log->glyphs.noop,
	    _cairo_time_to_ns(log->glyphs.elapsed), percent(log->glyphs.elapsed, total));
	if(log->glyphs.count) {
