...
static void _cairo_pdf_operators_emit_glyph_index(cairo_pdf_operators_t * pdf_operators,
    cairo_output_stream_t * stream,
    uint glyph)
{
 ...
 */
static cairo_status_t _cairo_pdf_operators_set_text_position(cairo_pdf_operators_t * pdf_operators, double x, double y)
{
	 ...
	 */
	cairo_matrix_t inverse = pdf_operators->text_matrix;
    ...
    cairo_glyph_t * glyph, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	...
	_p = glyph->P;
	cairo_matrix_transform_point(&pdf_operators->cairo_to_pdftext, &_p.x, &_p.y);
	...
	if(fabs(_p.x - pdf_operators->glyph_buf_x_pos) > 10 || fabs(_p.y - pdf_operators->cur_y) > GLYPH_POSITION_TOLERANCE) {
		...
		if(UNLIKELY(status))
			return status;
		_p = glyph->P;
		cairo_matrix_transform_point(&pdf_operators->cairo_to_pdf, &_p.x, &_p.y);
...
// 
static cairo_int_status_t _cairo_pdf_operators_emit_cluster(cairo_pdf_operators_t * pdf_operators, const char * utf8,
    int utf8_len, cairo_glyph_t * glyphs, int num_glyphs, cairo_text_cluster_flags_t cluster_flags, cairo_scaled_font_t * scaled_font)
{
    ...
    cairo_scaled_font_t * scaled_font)
{
	...
	if(num_clusters > 0) {
		...
		for(i = 0; i < num_clusters; i++) {
			...
			status = _cairo_pdf_operators_emit_cluster(pdf_operators, cur_text, clusters[i].num_bytes,
				cur_glyph, clusters[i].num_glyphs, cluster_flags, scaled_font);
			if(UNLIKELY(status))
				return status;
		...
		for(i = 0; i < num_glyphs; i++) {
			status = _cairo_pdf_operators_emit_cluster(pdf_operators, NULL,
				-1/* no unicode string available */, &glyphs[i], 1, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font);
			if(UNLIKELY(status))
				return status;
		}
	}
...
cairo_int_status_t _cairo_pdf_operators_tag_begin(cairo_pdf_operators_t * pdf_operators, const char * tag_name, int mcid)
{
	...
	_cairo_output_stream_printf(pdf_operators->stream, "/%s << /MCID %d >> BDC\n", tag_name, mcid);
	return _cairo_output_stream_get_status(pdf_operators->stream);
}
