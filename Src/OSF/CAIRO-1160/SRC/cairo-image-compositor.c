...
{
	cairo_image_surface_t * surface = static_cast<cairo_image_surface_t *>(_surface);
...
{
	cairo_image_surface_t * dst = static_cast<cairo_image_surface_t *>(_dst);
	...
	for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
		for(i = 0; i < chunk->count; i++) {
			...
			if(dst->pixman_format != image->pixman_format || !pixman_blt((uint32 *)image->data, (uint32 *)dst->data,
			    image->stride / sizeof(uint32), dst->stride / sizeof(uint32), PIXMAN_FORMAT_BPP(image->pixman_format),
			    PIXMAN_FORMAT_BPP(dst->pixman_format), x + dx, y + dy, x, y, w, h)) {
				pixman_image_composite32(PIXMAN_OP_SRC, image->pixman_image, NULL, dst->pixman_image,
				    x + dx, y + dy, 0, 0, x, y, w, h);
			}
		}
	}
	return CAIRO_STATUS_SUCCESS;
...
static inline uint32 color_to_uint32(const cairo_color_t * color)
{
	return (color->alpha_short >> 8 << 24) | (color->red_short >> 8 << 16) | (color->green_short & 0xff00) | (color->blue_short >> 8);
}

static inline boolint color_to_pixel(const cairo_color_t * color, pixman_format_code_t format, uint32 * pixel)
{
	if(!oneof9(format, PIXMAN_a8r8g8b8, PIXMAN_x8r8g8b8, PIXMAN_a8b8g8r8, PIXMAN_x8b8g8r8, PIXMAN_b8g8r8a8, PIXMAN_b8g8r8x8, PIXMAN_r5g6b5, PIXMAN_b5g6r5, PIXMAN_a8)) {
		...
		if(PIXMAN_FORMAT_TYPE(format) == PIXMAN_TYPE_ABGR) {
			c = ((c & 0xff000000) >>  0) | ((c & 0x00ff0000) >> 16) | ((c & 0x0000ff00) >>  0) | ((c & 0x000000ff) << 16);
		}
		...
		if(format == PIXMAN_a8) {
			...
			c = ((((c) >> 3) & 0x001f) | (((c) >> 5) & 0x07e0) | (((c) >> 8) & 0xf800));
		}
		*pixel = c;
		return TRUE;
	}
...
{
	switch(op) {
		case CAIRO_OPERATOR_CLEAR: return PIXMAN_OP_CLEAR;
		case CAIRO_OPERATOR_SOURCE: return PIXMAN_OP_SRC;
		case CAIRO_OPERATOR_OVER: return PIXMAN_OP_OVER;
		case CAIRO_OPERATOR_IN: return PIXMAN_OP_IN;
		case CAIRO_OPERATOR_OUT: return PIXMAN_OP_OUT;
		case CAIRO_OPERATOR_ATOP: return PIXMAN_OP_ATOP;
		case CAIRO_OPERATOR_DEST: return PIXMAN_OP_DST;
		case CAIRO_OPERATOR_DEST_OVER: return PIXMAN_OP_OVER_REVERSE;
		case CAIRO_OPERATOR_DEST_IN: return PIXMAN_OP_IN_REVERSE;
		case CAIRO_OPERATOR_DEST_OUT: return PIXMAN_OP_OUT_REVERSE;
		case CAIRO_OPERATOR_DEST_ATOP: return PIXMAN_OP_ATOP_REVERSE;
		case CAIRO_OPERATOR_XOR: return PIXMAN_OP_XOR;
		case CAIRO_OPERATOR_ADD: return PIXMAN_OP_ADD;
		case CAIRO_OPERATOR_SATURATE: return PIXMAN_OP_SATURATE;
		case CAIRO_OPERATOR_MULTIPLY: return PIXMAN_OP_MULTIPLY;
		case CAIRO_OPERATOR_SCREEN: return PIXMAN_OP_SCREEN;
		case CAIRO_OPERATOR_OVERLAY: return PIXMAN_OP_OVERLAY;
		case CAIRO_OPERATOR_DARKEN: return PIXMAN_OP_DARKEN;
		case CAIRO_OPERATOR_LIGHTEN: return PIXMAN_OP_LIGHTEN;
		case CAIRO_OPERATOR_COLOR_DODGE: return PIXMAN_OP_COLOR_DODGE;
		case CAIRO_OPERATOR_COLOR_BURN: return PIXMAN_OP_COLOR_BURN;
		case CAIRO_OPERATOR_HARD_LIGHT: return PIXMAN_OP_HARD_LIGHT;
		case CAIRO_OPERATOR_SOFT_LIGHT: return PIXMAN_OP_SOFT_LIGHT;
		case CAIRO_OPERATOR_DIFFERENCE: return PIXMAN_OP_DIFFERENCE;
		case CAIRO_OPERATOR_EXCLUSION: return PIXMAN_OP_EXCLUSION;
		case CAIRO_OPERATOR_HSL_HUE: return PIXMAN_OP_HSL_HUE;
		case CAIRO_OPERATOR_HSL_SATURATION: return PIXMAN_OP_HSL_SATURATION;
		case CAIRO_OPERATOR_HSL_COLOR: return PIXMAN_OP_HSL_COLOR;
		case CAIRO_OPERATOR_HSL_LUMINOSITY: return PIXMAN_OP_HSL_LUMINOSITY;
		default:
		    ASSERT_NOT_REACHED;
...
static boolint fill_reduces_to_source(cairo_operator_t op, const cairo_color_t * color, const cairo_image_surface_t * dst, uint32 * pixel)
{
	return __fill_reduces_to_source(op, color, dst) ? color_to_pixel(color, dst->pixman_format, pixel) : FALSE;
}

static cairo_int_status_t fill_rectangles(void * _dst, cairo_operator_t op, const cairo_color_t * color, cairo_rectangle_int_t * rects, int num_rects)
{
	...
	if(fill_reduces_to_source(op, color, dst, &pixel)) {
		for(i = 0; i < num_rects; i++) {
			pixman_fill((uint32 *)dst->data, dst->stride / sizeof(uint32),
			    PIXMAN_FORMAT_BPP(dst->pixman_format), rects[i].x, rects[i].y, rects[i].width, rects[i].height, pixel);
		}
	}
	else {
		...
		for(i = 0; i < num_rects; i++) {
			pixman_image_composite32((pixman_op_t)op, src, NULL, dst->pixman_image, 0, 0, 0, 0, rects[i].x, rects[i].y, rects[i].width, rects[i].height);
		}
	...
	if(fill_reduces_to_source(op, color, dst, &pixel)) {
		for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
			for(i = 0; i < chunk->count; i++) {
				...
				pixman_fill((uint32 *)dst->data, dst->stride / sizeof(uint32), PIXMAN_FORMAT_BPP(dst->pixman_format), x, y, w, h, pixel);
			}
		}
	}
	else {
		...
		for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
			for(i = 0; i < chunk->count; i++) {
				...
				pixman_image_composite32((pixman_op_t)op, src, NULL, dst->pixman_image, 0, 0, 0, 0, x1, y1, x2-x1, y2-y1);
			}
		}
		pixman_image_unref(src);
	}
    ...
    int src_x, int src_y, int mask_x, int mask_y, int dst_x, int dst_y, uint width, uint height)
{
	...
	if(mask) {
		...
		pixman_image_composite32(_pixman_operator(op), src->pixman_image, NULL, to_pixman_image(static_cast<cairo_surface_t *>(_dst)),
    ...
    int src_x, int src_y, int mask_x, int mask_y, int dst_x, int dst_y, uint width, uint height)
{
	...
	TRACE((stderr, "%s - OUT_REVERSE (mask=%d/%p, dst=%d/%p)\n", __FUNCTION__,
	    mask->base.unique_id, mask->pixman_image, dst->base.unique_id, dst->pixman_image));
	pixman_image_composite32(PIXMAN_OP_OUT_REVERSE, mask->pixman_image, NULL, dst->pixman_image,
	    mask_x, mask_y, 0,      0, dst_x,  dst_y, width,  height);
	/* Now add the two results together */
	TRACE((stderr, "%s - ADD (src=%d/%p, mask=%d/%p, dst=%d/%p)\n", __FUNCTION__,
	    src->base.unique_id, src->pixman_image, mask->base.unique_id, mask->pixman_image, dst->base.unique_id, dst->pixman_image));
	pixman_image_composite32(PIXMAN_OP_ADD, src->pixman_image, mask->pixman_image, dst->pixman_image,
	    src_x,  src_y, mask_x, mask_y, dst_x,  dst_y, width,  height);
#endif
	return CAIRO_STATUS_SUCCESS;
}
...
{
	pixman_image_t * dst = to_pixman_image(static_cast<cairo_surface_t *>(_dst));
	...
	if(((cairo_surface_t*)_dst)->is_clear && (op == CAIRO_OPERATOR_SOURCE || op == CAIRO_OPERATOR_OVER || op == CAIRO_OPERATOR_ADD)) {
	...
	for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
		for(i = 0; i < chunk->count; i++) {
			...
			int x2 = _cairo_fixed_integer_part(chunk->base[i].p2.x);
			int y2 = _cairo_fixed_integer_part(chunk->base[i].p2.y);
			pixman_image_composite32((pixman_op_t)op, src, mask, dst, x1 + src_x, y1 + src_y, x1 + mask_x, y1 + mask_y, x1 + dst_x, y1 + dst_y, x2 - x1, y2 - y1);
		}
	}
	if(free_src)
...
void _pixman_image_add_tristrip(pixman_image_t * image,
    int dst_x, int dst_y,
    ...
    cairo_composite_glyphs_info_t * info)
{
	...
	for(i = 0; i < info->num_glyphs; i++) {
		...
		const void * glyph = pixman_glyph_cache_lookup(glyph_cache, info->font, (void *)index);
		if(!glyph) {
			 ...
			 */
			CAIRO_MUTEX_UNLOCK(_cairo_glyph_cache_mutex);
			status = _cairo_scaled_glyph_lookup(info->font, index, CAIRO_SCALED_GLYPH_INFO_SURFACE, &scaled_glyph);
			CAIRO_MUTEX_LOCK(_cairo_glyph_cache_mutex);
			if(UNLIKELY(status))
				goto out_thaw;
			glyph_surface = scaled_glyph->surface;
			glyph = pixman_glyph_cache_insert(glyph_cache, info->font, (void *)index, glyph_surface->base.device_transform.x0,
				glyph_surface->base.device_transform.y0, glyph_surface->pixman_image);
			if(UNLIKELY(!glyph)) {
				status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
				goto out_thaw;
			}
...
static cairo_int_status_t composite_one_glyph(void * _dst, cairo_operator_t op, cairo_surface_t * _src, int src_x,
    int src_y, int dst_x, int dst_y, cairo_composite_glyphs_info_t * info)
{
	cairo_image_surface_t * glyph_surface;
	cairo_scaled_glyph_t * scaled_glyph;
	...
	pixman_image_composite32(_pixman_operator(op), ((cairo_image_source_t *)_src)->pixman_image,
	    glyph_surface->pixman_image, to_pixman_image(static_cast<cairo_surface_t *>(_dst)),
    ...
    int src_x, int src_y, int dst_x, int dst_y, cairo_composite_glyphs_info_t * info)
{
	...
	if(i * info->extents.height > (int)sizeof(buf)) {
		...
		mask = pixman_image_create_bits(format, info->extents.width, info->extents.height, (uint32 *)buf, i);
	}
	if(UNLIKELY(mask == NULL)) {
		pixman_image_unref(white);
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	...
	for(i = 0; i < info->num_glyphs; i++) {
		...
		if(glyph_surface->width && glyph_surface->height) {
			...
			if(glyph_surface->pixman_format == format) {
				pixman_image_composite32(PIXMAN_OP_ADD, glyph_surface->pixman_image, NULL, mask, 0, 0, 0, 0,
				    x - info->extents.x, y - info->extents.y, glyph_surface->width, glyph_surface->height);
			}
			else {
				pixman_image_composite32(PIXMAN_OP_ADD, white, glyph_surface->pixman_image, mask, 0, 0, 0, 0,
				    x - info->extents.x, y - info->extents.y, glyph_surface->width, glyph_surface->height);
			}
		}
	}
	...
	pixman_image_composite32(_pixman_operator(op), ((cairo_image_source_t *)_src)->pixman_image, mask,
	    to_pixman_image(static_cast<cairo_surface_t *>(_dst)), info->extents.x + src_x, info->extents.y + src_y,
    ...
    int src_x, int src_y, int dst_x, int dst_y, cairo_composite_glyphs_info_t * info)
{
	...
	op = (cairo_operator_t)_pixman_operator(op);
	dst = to_pixman_image(static_cast<cairo_surface_t *>(_dst));
...
static cairo_status_t _cairo_image_spans(void * abstract_renderer, int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * mask, * row;
	int len;
	...
	do {
		...
		if(spans[0].coverage) {
			*row++ = static_cast<uint8>(r->opacity * spans[0].coverage);
			if(--len)
...
static cairo_status_t _fill8_spans(void * abstract_renderer, int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
		return CAIRO_STATUS_SUCCESS;
	if(LIKELY(h == 1)) {
		do {
			if(spans[0].coverage) {
				...
				if(len == 1)
					*d = static_cast<uint8>(r->u.fill.pixel);
				else
	...
	if(LIKELY(h == 1)) {
		do {
			if(spans[0].coverage) {
				...
				while(len-- > 0)
					*d++ = static_cast<uint16>(r->u.fill.pixel);
			}
			spans++;
		} while(--num_spans > 1);
	}
	...
	if(LIKELY(h == 1)) {
		do {
			if(spans[0].coverage) {
				int len = spans[1].x - spans[0].x;
				if(len > 32) {
					pixman_fill((uint32 *)r->u.fill.data, r->u.fill.stride / sizeof(uint32), r->bpp,
    ...
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	...
	do {
		if(spans[0].coverage) {
			pixman_fill((uint32 *)r->data, r->stride, r->bpp,
			    spans[0].x, y,
...
static cairo_status_t _mono_spans(void * abstract_renderer, int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
		return CAIRO_STATUS_SUCCESS;
    ...
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	...
	if(num_spans == 0) {
		pixman_image_composite32(PIXMAN_OP_CLEAR, r->src, NULL, r->u.composite.dst,
		    spans[0].x + r->u.composite.src_x,  y + r->u.composite.src_y,
		    0, 0, r->composite->unbounded.x, y, r->composite->unbounded.width, h);
	...
	if(y != r->u.composite.mask_y) {
		pixman_image_composite32(PIXMAN_OP_CLEAR, r->src, NULL, r->u.composite.dst,
		    spans[0].x + r->u.composite.src_x,  y + r->u.composite.src_y,
		    0, 0, r->composite->unbounded.x, r->u.composite.mask_y,
		    r->composite->unbounded.width, y - r->u.composite.mask_y);
	}
	if(spans[0].x != r->composite->unbounded.x) {
		pixman_image_composite32(PIXMAN_OP_CLEAR, r->src, NULL, r->u.composite.dst,
		    spans[0].x + r->u.composite.src_x,  y + r->u.composite.src_y,
		    0, 0, r->composite->unbounded.x, y, spans[0].x - r->composite->unbounded.x, h);
	}
	do {
		int op = spans[0].coverage ? r->op : PIXMAN_OP_CLEAR;
		pixman_image_composite32((pixman_op_t)op, r->src, NULL, r->u.composite.dst,
		    spans[0].x + r->u.composite.src_x,  y + r->u.composite.src_y, 0, 0, spans[0].x, y, spans[1].x - spans[0].x, h);
		spans++;
	} while(--num_spans > 1);
	if(spans[0].x != r->composite->unbounded.x + r->composite->unbounded.width) {
		pixman_image_composite32(PIXMAN_OP_CLEAR, r->src, NULL, r->u.composite.dst,
		    spans[0].x + r->u.composite.src_x,  y + r->u.composite.src_y,
		    0, 0, spans[0].x, y, r->composite->unbounded.x + r->composite->unbounded.width - spans[0].x, h);
	}
	r->u.composite.mask_y = y + h;
	return CAIRO_STATUS_SUCCESS;
}
...
#define G_SHIFT 8
static inline uint32 mul8x2_8(uint32 a, uint8 b)
{
	uint32 t = (a & RB_MASK) * b + RB_ONE_HALF;
	return ((t + ((t >> G_SHIFT) & RB_MASK)) >> G_SHIFT) & RB_MASK;
}

static inline uint32 add8x2_8x2(uint32 a, uint32 b)
{
	uint32 t = a + b;
	...
	if(LIKELY(h == 1)) {
		do {
			...
			if(a) {
				...
				if(a == 0xff) {
					if(len > 31) {
						pixman_fill((uint32 *)r->u.fill.data, r->u.fill.stride / sizeof(uint32), 32, spans[0].x, y, len, 1, r->u.fill.pixel);
					}
					else {
						uint32 * d = (uint32 *)(r->u.fill.data + r->u.fill.stride*y + spans[0].x*4);
						while(len-- > 0)
		...
		do {
			...
			if(a) {
				if(a == 0xff) {
					if(spans[1].x - spans[0].x > 16) {
						pixman_fill((uint32 *)r->u.fill.data, r->u.fill.stride / sizeof(uint32), 32,
						    spans[0].x, y, spans[1].x - spans[0].x, h,
						    r->u.fill.pixel);
					}
					else {
			...
			if(len >= r->u.composite.run_length && spans[0].coverage == 0xff) {
				if(x1 != x0) {
					pixman_image_composite32((pixman_op_t)r->op, r->src, r->mask, r->u.composite.dst,
					    x0 + r->u.composite.src_x, y + r->u.composite.src_y, 0, 0, x0, y, x1 - x0, h);
				}
				pixman_image_composite32((pixman_op_t)r->op, r->src, NULL, r->u.composite.dst,
				    spans[0].x + r->u.composite.src_x, y + r->u.composite.src_y, 0, 0, spans[0].x, y, len, h);
				mask = (uint8 *)pixman_image_get_data(r->mask);
				x0 = spans[1].x;
			}
			else if(spans[0].coverage == 0x0 && x1 - x0 > r->u.composite.run_length) {
				pixman_image_composite32((pixman_op_t)r->op, r->src, r->mask, r->u.composite.dst,
				    x0 + r->u.composite.src_x, y + r->u.composite.src_y, 0, 0, x0, y, x1 - x0, h);
				mask = (uint8 *)pixman_image_get_data(r->mask);
				x0 = spans[1].x;
...
static cairo_status_t _inplace_opacity_spans(void * abstract_renderer, int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * mask;
	int x0, x1;
...
static cairo_status_t _inplace_src_opacity_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans,
    unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
    ...
    cairo_antialias_t antialias, boolint needs_clip)
{
	...
	if(r->base.render_rows == NULL) {
		...
		r->mask = pixman_image_create_bits(PIXMAN_a8, width, composite->unbounded.height, (uint32 *)buf, 0);
		if(UNLIKELY(r->mask == NULL)) {
			pixman_image_unref(r->src);
			if(buf != r->_buf)
				SAlloc::F(buf);
...
static cairo_int_status_t span_renderer_init(cairo_abstract_span_renderer_t * _r, const cairo_composite_rectangles_t * composite, cairo_antialias_t antialias, boolint needs_clip)
{
	...
	r->op = op;
	r->src = _pixman_image_for_pattern(dst, source, FALSE,
		&composite->unbounded, &composite->source_sample_area, &r->u.mask.src_x, &r->u.mask.src_y);
	if(UNLIKELY(r->src == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	r->opacity = 1.0;
	if(composite->mask_pattern.base.type == CAIRO_PATTERN_TYPE_SOLID) {
		r->opacity = static_cast<float>(composite->mask_pattern.solid.color.alpha);
	}
	else {
		int mask_x, mask_y;
		pixman_image_t * mask = _pixman_image_for_pattern(dst, &composite->mask_pattern.base,
			TRUE, &composite->unbounded, &composite->mask_sample_area, &mask_x, &mask_y);
		if(UNLIKELY(mask == NULL))
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
		/* XXX Component-alpha? */
    ...
    cairo_int_status_t status)
{
	...
	if(LIKELY(status == CAIRO_INT_STATUS_SUCCESS && r->bpp == 0)) {
		...
		pixman_image_composite32((pixman_op_t)r->op, r->src, r->mask, to_pixman_image(composite->surface),
		    composite->unbounded.x + r->u.mask.src_x, composite->unbounded.y + r->u.mask.src_y,
		    0, 0, composite->unbounded.x, composite->unbounded.y, composite->unbounded.width, composite->unbounded.height);
	}
	if(r->src)
		pixman_image_unref(r->src);
	if(r->mask)
