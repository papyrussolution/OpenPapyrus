...
void * _cairo_freelist_alloc(cairo_freelist_t * freelist)
{
	...
	return SAlloc::M_zon0(freelist->nodesize);
}
...
void * _cairo_freepool_alloc_from_new_pool(cairo_freepool_t * freepool)
{
	...
	int poolsize;
	if(freepool->freepools) {
		pool = freepool->freepools;
		freepool->freepools = pool->next;
		poolsize = pool->size;
	}
	else {
		poolsize = (freepool->pools != &freepool->embedded_pool) ? (2 * freepool->pools->size) : ((128 * freepool->nodesize + 8191) & -8192);
		pool = (cairo_freelist_pool_t *)SAlloc::M_zon0(sizeof(cairo_freelist_pool_t) + poolsize);
