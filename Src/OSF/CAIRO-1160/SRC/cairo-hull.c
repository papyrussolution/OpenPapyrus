...
static void _cairo_hull_init(cairo_hull_t * hull, cairo_pen_vertex_t * vertices, int num_vertices)
{
	cairo_point_t * p, tmp;
	int i;
	cairo_point_t * extremum = &vertices[0].point;
...
static void _cairo_hull_eliminate_concave(cairo_hull_t * hull, int num_hull)
{
	cairo_slope_t slope_ij, slope_jk;
	int i = 0;
	int j = _cairo_hull_next_valid(hull, num_hull, i);
	int k = _cairo_hull_next_valid(hull, num_hull, j);
	do {
...
cairo_status_t _cairo_hull_compute(cairo_pen_vertex_t * vertices, int * num_vertices)
{
	...
	qsort(hull + 1, num_hull - 1, sizeof(cairo_hull_t), _cairo_hull_vertex_compare);
