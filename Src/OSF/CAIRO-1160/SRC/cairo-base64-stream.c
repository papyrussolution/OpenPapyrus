 ...
 *
 * Author(s): Kristian Høgsberg <krh@redhat.com> Chris Wilson <chris@chris-wilson.co.uk>
 */
#include "cairoint.h"
#pragma hdrstop
...
static cairo_status_t _cairo_base64_stream_close(cairo_output_stream_t * base)
{
	cairo_base64_stream_t * stream = (cairo_base64_stream_t*)base;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	if(stream->in_mem > 0) {
		memzero(stream->src + stream->in_mem, 3 - stream->in_mem);
...
cairo_output_stream_t * _cairo_base64_stream_create(cairo_output_stream_t * output)
{
	...
	stream = (cairo_base64_stream_t *)_cairo_malloc(sizeof(cairo_base64_stream_t));
