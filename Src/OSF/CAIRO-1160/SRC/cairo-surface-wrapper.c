...
cairo_status_t _cairo_surface_wrapper_show_text_glyphs(cairo_surface_wrapper_t * wrapper, cairo_operator_t op, const cairo_pattern_t * source,
    const char * utf8, int utf8_len, const cairo_glyph_t * glyphs, int num_glyphs, const cairo_text_cluster_t * clusters, int num_clusters,
    cairo_text_cluster_flags_t cluster_flags, cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	if(wrapper->needs_transform) {
		...
		for(i = 0; i < num_glyphs; i++) {
			...
			cairo_matrix_transform_point(&m, &dev_glyphs[i].P.x, &dev_glyphs[i].P.y);
		}
		status = cairo_matrix_invert(&m);
		assert(status == CAIRO_STATUS_SUCCESS);
		_copy_transformed_pattern(&source_copy.base, source, &m);
		...
		if(!cairo_font_options_equal(&options, &scaled_font->options)) {
			dev_scaled_font = cairo_scaled_font_create(scaled_font->font_face, &scaled_font->font_matrix, &scaled_font->ctm, &options);
		}
	...
	status = _cairo_surface_show_text_glyphs(wrapper->target, op, source, utf8, utf8_len, dev_glyphs, num_glyphs, clusters, num_clusters,
		cluster_flags, dev_scaled_font, dev_clip);
FINISH:
	_cairo_clip_destroy(dev_clip);
    ...
    const cairo_pattern_t * source, const cairo_stroke_style_t * stroke_style, const cairo_matrix_t * ctm, const cairo_matrix_t * ctm_inverse, const cairo_clip_t * clip)
{
	...
	status = _cairo_surface_tag(wrapper->target, begin, tag_name, attributes, source, stroke_style, &dev_ctm, &dev_ctm_inverse, dev_clip);
	_cairo_clip_destroy(dev_clip);
	return status;
}

cairo_surface_t * _cairo_surface_wrapper_create_similar(cairo_surface_wrapper_t * wrapper, cairo_content_t content, int width, int height)
{
...
void _cairo_surface_wrapper_init(cairo_surface_wrapper_t * wrapper, cairo_surface_t * target)
{
	...
	if(target) {
		wrapper->needs_transform = !_cairo_matrix_is_identity(&target->device_transform);
	}
}
...
boolint _cairo_surface_wrapper_get_target_extents(cairo_surface_wrapper_t * wrapper, boolint surface_is_unbounded, cairo_rectangle_int_t * extents)
{
	...
	if(has_clip && wrapper->needs_transform) {
		...
		clip.x = ffloori(x1);
		clip.y = ffloori(y1);
		clip.width  = fceili(x2) - clip.x;
		clip.height = fceili(y2) - clip.y;
	}
	if(has_clip) {
		if(wrapper->has_extents) {
			*extents = wrapper->extents;
