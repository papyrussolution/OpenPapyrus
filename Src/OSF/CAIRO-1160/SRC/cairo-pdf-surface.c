...
static cairo_int_status_t _cairo_pdf_surface_open_stream(cairo_pdf_surface_t * surface, cairo_pdf_resource_t * resource,
    boolint compressed, const char * fmt, ...) CAIRO_PRINTF_FORMAT(4, 5);
static cairo_int_status_t _cairo_pdf_surface_close_stream(cairo_pdf_surface_t * surface);
static cairo_int_status_t _cairo_pdf_surface_emit_surface(cairo_pdf_surface_t * surface, cairo_pdf_source_surface_t * source, boolint test, boolint * is_image);
...
static cairo_int_status_t _cairo_pdf_surface_write_patterns_and_smask_groups(cairo_pdf_surface_t * surface, boolint finish);
static cairo_int_status_t _cairo_pdf_surface_write_page(cairo_pdf_surface_t * surface);
static cairo_int_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface);
static boolint _cairo_pdf_source_surface_equal(const void * key_a, const void * key_b);
...
static void _cairo_pdf_surface_set_size_internal(cairo_pdf_surface_t * surface, double width, double height)
{
	...
	surface->surface_extents.width  = static_cast<int>(ceil(surface->width));
	surface->surface_extents.height = static_cast<int>(ceil(surface->height));
}

static boolint _path_covers_bbox(cairo_pdf_surface_t * surface, const cairo_path_fixed_t * path)
{
	cairo_box_t box;
...
static cairo_status_t _cairo_pdf_surface_clipper_intersect_clip_path(cairo_surface_clipper_t * clipper, cairo_path_fixed_t * path,
...
static cairo_surface_t * _cairo_pdf_surface_create_for_stream_internal(cairo_output_stream_t * output, double width, double height)
{
	...
	_cairo_surface_init(&surface->base, &cairo_pdf_surface_backend, NULL/* device */, CAIRO_CONTENT_COLOR_ALPHA, TRUE/* is_vector */);
...
static boolint _cairo_surface_is_pdf(cairo_surface_t * surface)
{
 ...
 */
static boolint _extract_pdf_surface(cairo_surface_t * surface, cairo_pdf_surface_t ** pdf_surface)
{
...
void cairo_pdf_surface_set_size(cairo_surface_t * surface, double width_in_points, double height_in_points)
{
	...
	status = _cairo_paginated_surface_set_size(pdf_surface->paginated_surface, static_cast<int>(width_in_points), static_cast<int>(height_in_points));
	if(status)
...
int cairo_pdf_surface_add_outline(cairo_surface_t * surface, int parent_id, const char * utf8, const char * link_attribs, cairo_pdf_outline_flags_t flags)
{
	...
	if(!_extract_pdf_surface(surface, &pdf_surface))
		return 0;
	status = _cairo_pdf_interchange_add_outline(pdf_surface, parent_id, utf8, link_attribs, flags, &id);
	if(status)
		status = _cairo_surface_set_error(surface, status);
	return id;
}
...
static void FASTCALL _cairo_pdf_group_resources_clear(cairo_pdf_group_resources_t * res)
{
	for(int i = 0; i < CAIRO_NUM_OPERATORS; i++)
		res->operators[i] = FALSE;
...
static cairo_int_status_t FASTCALL _cairo_pdf_surface_add_alpha(cairo_pdf_surface_t * surface, double alpha, int * index)
{
	int i;
	double other;
	cairo_int_status_t status;
...
static cairo_int_status_t FASTCALL _cairo_pdf_surface_add_smask(cairo_pdf_surface_t * surface, cairo_pdf_resource_t smask)
{
...
static cairo_int_status_t FASTCALL _cairo_pdf_surface_add_xobject(cairo_pdf_surface_t * surface, cairo_pdf_resource_t xobject)
{
...
static void _cairo_pdf_surface_emit_group_resources(cairo_pdf_surface_t * surface, cairo_pdf_group_resources_t * res)
{
	...
	if(num_alphas > 0 || num_smasks > 0) {
		...
		for(i = 0; i < num_smasks; i++) {
			smask = static_cast<cairo_pdf_resource_t *>(_cairo_array_index(&res->smasks, i));
	...
	if(num_resources > 0) {
		...
		for(i = 0; i < num_resources; i++) {
			pattern = static_cast<cairo_pdf_resource_t *>(_cairo_array_index(&res->patterns, i));
	...
	if(num_resources > 0) {
		...
		for(i = 0; i < num_resources; i++) {
			shading = static_cast<cairo_pdf_resource_t *>(_cairo_array_index(&res->shadings, i));
	...
	if(num_resources > 0) {
		...
		for(i = 0; i < num_resources; i++) {
			xobject = static_cast<cairo_pdf_resource_t *>(_cairo_array_index(&res->xobjects, i));
...
static cairo_pdf_smask_group_t * _cairo_pdf_surface_create_smask_group(cairo_pdf_surface_t * surface, const cairo_rectangle_int_t * extents)
{
	...
	if(extents != NULL) {
		...
		group->extents.x = 0;
		group->extents.y = 0;
		group->extents.width = static_cast<int>(surface->width);
		group->extents.height = static_cast<int>(surface->height);
	}
	return group;
}
    ...
    cairo_pdf_resource_t * gstate_res)
{
	...
	if(extents != NULL) {
		...
		pdf_pattern.extents.width  = static_cast<int>(surface->width);
		pdf_pattern.extents.height = static_cast<int>(surface->height);
	}
...
static void _get_bbox_from_extents(const cairo_rectangle_int_t * extents,
...
{
	cairo_int_status_t status = _cairo_pdf_surface_open_group(surface, bbox, NULL);
	if(unlikely(status))
		return status;
    ...
    boolint is_group)
{
	...
	if(is_form) {
		...
		_cairo_output_stream_printf(surface->output, "1 0 0 -1 0 %f cm\n", surface->height);
	}
	if(unlikely(status))
		return status;
...
static boolint _cairo_pdf_surface_supports_fine_grained_fallbacks(void * abstract_surface)
{
	return TRUE;
}

static boolint _cairo_pdf_surface_requires_thumbnail_image(void * abstract_surface,
    int * width,
    int * height)
{
	cairo_pdf_surface_t * surface = static_cast<cairo_pdf_surface_t *>(abstract_surface);
...
static cairo_int_status_t _cairo_pdf_surface_set_thumbnail_image(void * abstract_surface,
    cairo_image_surface_t * image)
{
	cairo_pdf_surface_t * surface = static_cast<cairo_pdf_surface_t *>(abstract_surface);
...
//
static cairo_int_status_t _cairo_pdf_surface_emit_smask(cairo_pdf_surface_t * surface, cairo_image_surface_t * image,
    boolint stencil_mask, boolint interpolate, cairo_pdf_resource_t * stream_res)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	char * alpha;
	ulong alpha_size;
	...
	assert(image->format == CAIRO_FORMAT_ARGB32 || image->format == CAIRO_FORMAT_RGB24 || image->format == CAIRO_FORMAT_A8 || image->format == CAIRO_FORMAT_A1);
	transparency = _cairo_image_analyze_transparency(image);
	if(stencil_mask) {
		assert(oneof2(transparency, CAIRO_IMAGE_IS_OPAQUE, CAIRO_IMAGE_HAS_BILEVEL_ALPHA));
	}
	else {
	...
	if(transparency == CAIRO_IMAGE_HAS_BILEVEL_ALPHA || transparency == CAIRO_IMAGE_IS_OPAQUE) {
		...
		alpha = static_cast<char *>(_cairo_malloc_ab((image->width+7) / 8, image->height));
	}
	else {
		alpha_size = image->height * image->width;
		alpha = static_cast<char *>(_cairo_malloc_ab(image->height, image->width));
	}
 ...
 **/
static cairo_int_status_t _cairo_pdf_surface_emit_image(cairo_pdf_surface_t * surface, cairo_image_surface_t * image_surf, cairo_pdf_source_surface_entry_t * surface_entry)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	char * data;
	ulong data_size;
	...
	cairo_image_transparency_t transparency;
	char smask_buf[30];
	cairo_image_surface_t * image  = image_surf;
	if(image->format != CAIRO_FORMAT_RGB24 && image->format != CAIRO_FORMAT_ARGB32 && image->format != CAIRO_FORMAT_A8 && image->format != CAIRO_FORMAT_A1) {
		cairo_surface_pattern_t pattern;
		cairo_surface_t * surf = _cairo_image_surface_create_with_content(image_surf->base.content, image_surf->width, image_surf->height);
		image = (cairo_image_surface_t*)surf;
		if(surf->status) {
			status = surf->status;
			goto CLEANUP;
	...
	if(surface_entry->smask || surface_entry->stencil_mask) {
		return _cairo_pdf_surface_emit_smask(surface, image, surface_entry->stencil_mask, surface_entry->interpolate, &surface_entry->surface_res);
	}
    ...
    cairo_surface_t * source, cairo_pdf_source_surface_entry_t * surface_entry, boolint test)
{
	...
	const uchar * ccitt_params_string;
	ulong ccitt_params_string_len;
	char * params, * p, * end;
	...
	params = (char *)SAlloc::M(ccitt_params_string_len + 1);
	...
	*p = 0;
	...
	p += snprintf(p, end - p, "/Columns %d /Rows %d /K %d", ccitt_params.columns, ccitt_params.rows, ccitt_params.k);
	...
	if(ccitt_params.damaged_rows_before_error > 0) {
		p += snprintf(p, end - p, " /DamagedRowsBeforeError %d", ccitt_params.damaged_rows_before_error);
	}
	if(surface_entry->stencil_mask) {
		status = _cairo_pdf_surface_open_stream(surface, &surface_entry->surface_res, FALSE,
			"   /Type /XObject\n"
...
static cairo_int_status_t _cairo_pdf_surface_emit_recording_surface(cairo_pdf_surface_t * surface,
 ...
 **/
static cairo_int_status_t _cairo_pdf_surface_emit_surface(cairo_pdf_surface_t * surface,
...
static cairo_int_status_t _cairo_pdf_surface_emit_surface_pattern(cairo_pdf_surface_t * surface,
    cairo_pdf_pattern_t * pdf_pattern)
{
...
{
	cairo_pdf_color_stop_t * allstops;
	cairo_pdf_color_stop_t * stops;
	uint n_stops;
	uint i;
	boolint emit_alpha = FALSE;
	cairo_int_status_t status;
	color_function->id = 0;
	alpha_function->id = 0;
...
static cairo_int_status_t _cairo_pdf_surface_emit_repeating_function(cairo_pdf_surface_t * surface, const cairo_gradient_pattern_t * pattern,
    ...
    cairo_pdf_pattern_t * pdf_pattern)
{
	...
	* coordinates in if there are not stops at offset 0 and 1. */
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		...
		repeat_begin = static_cast<int>(floor(domain[0]));
		repeat_end = static_cast<int>(ceil(domain[1]));
...
static cairo_int_status_t _cairo_pdf_surface_paint_pattern(cairo_pdf_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_rectangle_int_t * extents,
    double alpha,
...
static cairo_int_status_t _cairo_pdf_surface_select_operator(cairo_pdf_surface_t * surface,
    cairo_operator_t op)
{
	cairo_int_status_t status;
...
static cairo_int_status_t _cairo_pdf_surface_select_pattern(cairo_pdf_surface_t * surface,
    const cairo_pattern_t * pattern,
    cairo_pdf_resource_t pattern_res,
    boolint is_stroke)
{
...
{
	cairo_pdf_surface_t * surface = static_cast<cairo_pdf_surface_t *>(abstract_surface);
	cairo_int_status_t status = _cairo_array_append(&surface->page_heights, &surface->height);
	if(unlikely(status))
		return status;
...
static cairo_int_status_t _cairo_pdf_surface_emit_unicode_for_glyph(cairo_pdf_surface_t * surface, const char * utf8)
{
	...
	if(utf8 && *utf8) {
		status = _cairo_utf8_to_utf16(utf8, -1, &utf16, &utf16_len);
		if(unlikely(status == CAIRO_STATUS_INVALID_STRING)) {
			utf16 = NULL;
			utf16_len = 0;
		}
...
static void _create_font_subset_tag(cairo_scaled_font_subset_t * font_subset, const char * font_name, char * tag)
{
	...
	for(i = 0; i < 6; i++) {
		d = ldiv(numerator, 26);
		numerator = d.quot;
		tag[i] = 'A' + static_cast<char>(d.rem);
	}
	tag[i] = 0;
}

static cairo_int_status_t _cairo_pdf_surface_emit_to_unicode_stream(cairo_pdf_surface_t * surface,
    cairo_scaled_font_subset_t * font_subset, cairo_pdf_resource_t * stream)
{
	uint i, num_bfchar;
	cairo_int_status_t status;
	stream->id = 0;
...
static cairo_int_status_t _cairo_pdf_surface_emit_cff_font(cairo_pdf_surface_t * surface,
    ...
    cairo_cff_subset_t * subset)
{
	...
	if(subset_resource.id == 0)
		return CAIRO_STATUS_SUCCESS;
	status = _cairo_pdf_surface_open_stream(surface, NULL, TRUE,
	...
	_cairo_output_stream_printf(surface->output,
	    ...
	    ">>\n"
	    "endobj\n",
	    static_cast<long>(subset->x_min*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->y_min*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->x_max*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->y_max*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->ascent*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->descent*PDF_UNITS_PER_EM),
	    static_cast<long>(subset->y_max*PDF_UNITS_PER_EM),
	    stream.id);
	...
	if(font_subset->is_latin) {
		...
		_cairo_output_stream_printf(surface->output,
		    ...
		    "   /FontDescriptor %d 0 R\n"
		    "   /W [0 [",
		    cidfont_dict.id, tag, subset->ps_name, descriptor.id);
...
static cairo_int_status_t _cairo_pdf_surface_emit_type1_font(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset, cairo_type1_subset_t * subset)
{
	...
	if(font_subset->is_latin) {
		for(i = 32; i < last_glyph + 1; i++) {
			int glyph = font_subset->latin_to_subset_glyph_index[i];
			if(glyph > 0) {
				_cairo_output_stream_printf(surface->output, " %ld", static_cast<long>(subset->widths[glyph]*PDF_UNITS_PER_EM));
			}
			else {
				_cairo_output_stream_printf(surface->output, " 0");
			}
		...
		for(i = 0; i < font_subset->num_glyphs; i++)
			_cairo_output_stream_printf(surface->output, " %ld", static_cast<long>(subset->widths[i]*PDF_UNITS_PER_EM));
	}
	_cairo_output_stream_printf(surface->output, " ]\n");
	if(to_unicode_stream.id != 0)
		_cairo_output_stream_printf(surface->output, "    /ToUnicode %d 0 R\n", to_unicode_stream.id);
...
static cairo_int_status_t _cairo_pdf_surface_analyze_user_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	...
	cairo_surface_t * type3_surface = _cairo_type3_glyph_surface_create(font_subset->scaled_font, null_stream, _cairo_pdf_emit_imagemask, surface->font_subsets, FALSE);
...
static cairo_int_status_t _cairo_pdf_surface_emit_type3_font_subset(cairo_pdf_surface_t * surface,
    cairo_scaled_font_subset_t * font_subset)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
...
static cairo_int_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface)
{
	cairo_int_status_t status = _cairo_scaled_font_subsets_foreach_user(surface->font_subsets,
    ...
    boolint finish)
{
	...
	cairo_pdf_smask_group_t * group;
	cairo_pdf_source_surface_t src_surface;
	uint pattern_index, group_index, surface_index, doc_surface_index;
	cairo_int_status_t status;
	boolint is_image;
...
static cairo_int_status_t _cairo_pdf_surface_write_page(cairo_pdf_surface_t * surface)
{
	...
	cairo_pdf_resource_t * page;
	uint i, len, page_num, num_annots;
	cairo_int_status_t status = _cairo_pdf_interchange_write_page_objects(surface);
	if(unlikely(status))
		return status;
	_cairo_pdf_group_resources_clear(&surface->resources);
	if(surface->has_fallback_images) {
		...
		extents.x = 0;
		extents.y = 0;
		extents.width = static_cast<int>(ceil(surface->width));
		extents.height = static_cast<int>(ceil(surface->height));
		_get_bbox_from_extents(&extents, &bbox);
	...
	page = static_cast<cairo_pdf_resource_t *>(_cairo_array_index(&surface->pages, page_num - 1));
	...
	if(surface->page_parent_tree >= 0) {
		_cairo_output_stream_printf(surface->output, "   /StructParents %d\n", surface->page_parent_tree);
	}
	num_annots = _cairo_array_num_elements(&surface->page_annots);
	if(num_annots > 0) {
		_cairo_output_stream_printf(surface->output, "   /Annots [ ");
		for(i = 0; i < num_annots; i++) {
			_cairo_array_copy_element(&surface->page_annots, i, &res);
			_cairo_output_stream_printf(surface->output, "%d 0 R ", res.id);
		}
		_cairo_output_stream_printf(surface->output, "]\n");
	}
	if(thumbnail_res.id) {
		_cairo_output_stream_printf(surface->output, "   /Thumb %d 0 R\n", thumbnail_res.id);
	}
	_cairo_output_stream_printf(surface->output, ">>\nendobj\n");
...
static boolint _surface_pattern_supported(cairo_surface_pattern_t * pattern)
{
...
static boolint _pattern_supported(const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		case CAIRO_PATTERN_TYPE_LINEAR:
...
static boolint _pdf_operator_supported(cairo_operator_t op)
{
	switch(op) {
		case CAIRO_OPERATOR_OVER:
...
static cairo_int_status_t _cairo_pdf_surface_analyze_operation(cairo_pdf_surface_t * surface, cairo_operator_t op,
...
static boolint _cairo_pdf_surface_operation_supported(cairo_pdf_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * pattern,
    const cairo_rectangle_int_t * extents)
{
...
static cairo_int_status_t _cairo_pdf_surface_emit_stencil_mask(cairo_pdf_surface_t * surface, cairo_operator_t op,
    const cairo_pattern_t * source, const cairo_pattern_t * mask, const cairo_rectangle_int_t * extents)
{
	cairo_int_status_t status;
	cairo_image_surface_t * image;
	void * image_extra;
    ...
    cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
	...
	if(_can_paint_pattern(source)) {
		_cairo_output_stream_printf(surface->output, "q\n");
		status = _cairo_pdf_surface_paint_pattern(surface, op, source, &extents.bounded, 1.0/* alpha */, FALSE/* mask */);
		if(unlikely(status))
			goto cleanup;
		_cairo_output_stream_printf(surface->output, "Q\n");
...
static cairo_int_status_t _cairo_pdf_surface_mask(void * abstract_surface, cairo_operator_t op,
    ...
    const cairo_clip_t * clip)
{
	...
	gstate_res.id = 0;
	status = _cairo_pdf_surface_add_pdf_pattern(surface, fill_source, fill_op, &extents.bounded, &fill_pattern_res, &gstate_res);
	if(unlikely(status))
		goto cleanup;
	assert(gstate_res.id == 0);
	stroke_pattern_res.id = 0;
	gstate_res.id = 0;
	status = _cairo_pdf_surface_add_pdf_pattern(surface, stroke_source, stroke_op, &extents.bounded, &stroke_pattern_res, &gstate_res);
	if(unlikely(status))
		goto cleanup;
	assert(gstate_res.id == 0);
	// As PDF has separate graphics state for fill and stroke we can select both at the same time 
	status = _cairo_pdf_surface_select_pattern(surface, fill_source, fill_pattern_res, FALSE);
	if(unlikely(status))
		goto cleanup;
	status = _cairo_pdf_surface_select_pattern(surface, stroke_source, stroke_pattern_res, TRUE);
	if(unlikely(status))
		goto cleanup;
	status = _cairo_pdf_operators_fill_stroke(&surface->pdf_operators, path, fill_rule, stroke_style, stroke_ctm, stroke_ctm_inverse);
	if(unlikely(status))
		goto cleanup;
	status = _cairo_pdf_surface_unselect_pattern(surface);
	if(unlikely(status))
...
static boolint _cairo_pdf_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}
    ...
    const cairo_clip_t * clip)
{
	...
	cairo_composite_rectangles_t extents;
	boolint overlap;
	cairo_int_status_t status = _cairo_composite_rectangles_init_for_glyphs(&extents, &surface->base, op, source,
...
static cairo_int_status_t _cairo_pdf_surface_tag(void * abstract_surface,
    boolint begin,
    const char * tag_name,
...
static cairo_int_status_t _cairo_pdf_surface_set_paginated_mode(void * abstract_surface, cairo_paginated_mode_t paginated_mode)
{
	cairo_pdf_surface_t * surface = static_cast<cairo_pdf_surface_t *>(abstract_surface);
	cairo_int_status_t status;
