...
cairo_clip_t * _cairo_clip_intersect_rectilinear_path(cairo_clip_t * clip, const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, cairo_antialias_t antialias)
{
...
static cairo_clip_t * _cairo_clip_intersect_rectangle_box(cairo_clip_t * clip, const cairo_rectangle_int_t * r, const cairo_box_t * box)
{
	...
	if(clip->num_boxes == 1 && clip->boxes[0].p1.x >= box->p1.x &&
	    clip->boxes[0].p1.y >= box->p1.y && clip->boxes[0].p2.x <= box->p2.x && clip->boxes[0].p2.y <= box->p2.y) {
		return clip;
	}
...
cairo_clip_t * _cairo_clip_intersect_rectangle(cairo_clip_t * clip, const cairo_rectangle_int_t * r)
{
	cairo_box_t box;
	if(_cairo_clip_is_all_clipped(clip))
		return clip;
...
static void _add_edge(struct reduce * r, const cairo_point_t * p1, const cairo_point_t * p2)
{
	...
	for(n = 0; n < r->clip->num_boxes; n++) {
		...
		if(p1->x >= limits->p1.x && p2->x <= limits->p1.x) {
			...
			int p1_y, p2_y;
			p1_y = _cairo_edge_compute_intersection_y_for_x(p1, p2, limits->p1.x);
			p2_y = _cairo_edge_compute_intersection_y_for_x(p1, p2, limits->p2.x);
			if(p1_y < p2_y) {
				top_y = p1_y;
				bot_y = p2_y;
			}
