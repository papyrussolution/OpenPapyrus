...
static cairo_int_status_t combine_clip_as_traps(const cairo_traps_compositor_t * compositor,
    cairo_surface_t * mask, const cairo_clip_t * clip, const cairo_rectangle_int_t * extents)
{
	...
	if(UNLIKELY(status))
		return status;
	src = compositor->pattern_to_surface(mask, NULL, FALSE, extents, NULL, &src_x, &src_y);
	if(UNLIKELY(src->status)) {
		_cairo_traps_fini(&traps);
		return src->status;
	}
	status = compositor->composite_traps(mask, CAIRO_OPERATOR_IN, src, src_x, src_y,
		extents->x, extents->y, extents, antialias, &traps);
	...
	if(fixup.width < extents->width || fixup.height < extents->height) {
		...
		if(fixup.y != extents->y) {
			add_rect_with_offset(&clear, extents->x, extents->y, extents->x + extents->width, fixup.y, extents->x, extents->y);
		}
		/* left */
		if(fixup.x != extents->x) {
			add_rect_with_offset(&clear, extents->x, fixup.y, fixup.x, fixup.y + fixup.height, extents->x, extents->y);
		}
		/* right */
		if(fixup.x + fixup.width != extents->x + extents->width) {
			add_rect_with_offset(&clear, fixup.x + fixup.width, fixup.y, extents->x + extents->width, fixup.y + fixup.height, extents->x, extents->y);
		}
		/* bottom */
		if(fixup.y + fixup.height != extents->y + extents->height) {
			add_rect_with_offset(&clear, extents->x, fixup.y + fixup.height, extents->x + extents->width, extents->y + extents->height, extents->x, extents->y);
		}
		status = compositor->fill_boxes(mask, CAIRO_OPERATOR_CLEAR, CAIRO_COLOR_TRANSPARENT, &clear);
		_cairo_boxes_fini(&clear);
	}
	return status;
}

static cairo_status_t __clip_to_surface(const cairo_traps_compositor_t * compositor, const cairo_composite_rectangles_t * composite,
    const cairo_rectangle_int_t * extents, cairo_surface_t ** surface)
{
	...
	cairo_int_status_t status;
	TRACE_FUNCTION_SIMPLE();
	status = _cairo_clip_get_polygon(composite->clip, &polygon, &fill_rule, &antialias);
	if(status)
		return status;
	_cairo_traps_init(&traps);
	status = _cairo_bentley_ottmann_tessellate_polygon(&traps, &polygon, fill_rule);
	...
	if(UNLIKELY(status))
		return status;
	mask = _cairo_surface_create_scratch(composite->surface, CAIRO_CONTENT_ALPHA, extents->width, extents->height, NULL);
	...
	if(UNLIKELY(status))
		goto error;
	_cairo_boxes_init_from_rectangle(&clear, 0, 0, extents->width, extents->height);
	status = compositor->fill_boxes(mask, CAIRO_OPERATOR_CLEAR, CAIRO_COLOR_TRANSPARENT, &clear);
	if(UNLIKELY(status))
		goto error_release;
	status = compositor->composite_traps(mask, CAIRO_OPERATOR_ADD, src, src_x, src_y, extents->x, extents->y, extents, antialias, &traps);
	if(UNLIKELY(status))
		goto error_release;
...
static cairo_surface_t * traps_get_clip_surface(const cairo_traps_compositor_t * compositor,
    const cairo_composite_rectangles_t * composite, const cairo_rectangle_int_t * extents)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		surface = _cairo_surface_create_scratch(composite->surface, CAIRO_CONTENT_ALPHA, extents->width, extents->height, CAIRO_COLOR_WHITE);
 ...
 */
static cairo_status_t clip_and_composite_combine(const cairo_traps_compositor_t * compositor, const cairo_composite_rectangles_t* extents,
    draw_func_t draw_func, void * draw_closure, cairo_operator_t op, cairo_surface_t * src, int src_x, int src_y)
{
	cairo_surface_t * dst = extents->surface;
	cairo_surface_t * tmp, * clip;
	cairo_status_t status;
	TRACE_FUNCTION_SIMPLE();
	tmp = _cairo_surface_create_scratch(dst, dst->content, extents->bounded.width, extents->bounded.height, NULL);
	...
	compositor->composite(tmp, dst->is_clear ? CAIRO_OPERATOR_CLEAR : CAIRO_OPERATOR_SOURCE, dst, NULL,
	    extents->bounded.x,      extents->bounded.y, 0, 0, 0, 0, extents->bounded.width,  extents->bounded.height);
	...
	if(UNLIKELY((status = clip->status)))
		goto cleanup;
	if(dst->is_clear) {
		compositor->composite(dst, CAIRO_OPERATOR_SOURCE, tmp, clip, 0, 0, 0, 0, extents->bounded.x, extents->bounded.y, extents->bounded.width,  extents->bounded.height);
	}
	else {
		compositor->lerp(dst, tmp, clip, 0, 0, 0, 0, extents->bounded.x, extents->bounded.y, extents->bounded.width, extents->bounded.height);
	}
 ...
 */
static cairo_status_t clip_and_composite_source(const cairo_traps_compositor_t * compositor, cairo_surface_t * dst,
    draw_func_t draw_func, draw_func_t mask_func, void * draw_closure, cairo_surface_t * src, int src_x, int src_y, const cairo_composite_rectangles_t * extents)
{
	cairo_surface_t * mask;
	TRACE_FUNCTION_SIMPLE();
	...
	if(dst->is_clear) {
		...
		compositor->lerp(dst, src, mask, extents->bounded.x + src_x, extents->bounded.y + src_y,
		    0, 0, extents->bounded.x,     extents->bounded.y, extents->bounded.width, extents->bounded.height);
	}
skip:
	cairo_surface_destroy(mask);
    ...
    cairo_clip_t * clip)
{
	...
	TRACE_FUNCTION_SIMPLE();
	return compositor->composite_traps(dst, op, src, src_x - dst_x, src_y - dst_y, dst_x, dst_y, extents, info->antialias, &info->traps);
}
...
static cairo_status_t composite_aligned_boxes(const cairo_traps_compositor_t * compositor, cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
	cairo_surface_t * dst = extents->surface;
	cairo_operator_t op = extents->op;
	...
	if(!need_clip_mask && op_is_source && recording_pattern_contains_sample(&extents->source_pattern.base, &extents->source_sample_area)) {
...
static cairo_int_status_t boxes_for_traps(cairo_boxes_t * boxes,
    cairo_traps_t * traps,
    cairo_antialias_t antialias)
{
	int i, j;
...
static cairo_int_status_t composite_opacity_boxes(const cairo_traps_compositor_t * compositor, cairo_surface_t * dst,
...
static cairo_int_status_t composite_boxes(const cairo_traps_compositor_t * compositor, cairo_surface_t * dst,
    void * closure, cairo_operator_t op, cairo_surface_t * src, int src_x, int src_y, int dst_x, int dst_y,
    const cairo_rectangle_int_t * extents, cairo_clip_t * clip)
{
	cairo_traps_t traps;
...
static cairo_int_status_t composite_traps_as_boxes(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents, composite_traps_info_t * info)
{
	cairo_boxes_t boxes;
	TRACE_FUNCTION_SIMPLE();
...
static cairo_int_status_t clip_and_composite_traps(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents, composite_traps_info_t * info, unsigned flags)
{
	TRACE_FUNCTION_SIMPLE();
	cairo_int_status_t status = trim_extents_to_traps(extents, &info->traps);
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		if(!extents->is_bounded)
			flags |= FORCE_CLIP_REGION;
		status = clip_and_composite(compositor, extents, composite_traps, NULL, info, need_unbounded_clip(extents) | flags);
	}
	return status;
}

static cairo_int_status_t clip_and_composite_tristrip(const cairo_traps_compositor_t * compositor, cairo_composite_rectangles_t * extents, composite_tristrip_info_t * info)
{
	cairo_int_status_t status;
	uint flags = 0;
	TRACE_FUNCTION_SIMPLE();
	...
	if(!extents->is_bounded)
		flags |= FORCE_CLIP_REGION;
	status = clip_and_composite(compositor, extents, composite_tristrip, NULL, info, need_unbounded_clip(extents) | flags);
	return status;
}
...
static cairo_int_status_t composite_mask(const cairo_traps_compositor_t * compositor, cairo_surface_t * dst,
    void * closure, cairo_operator_t op, cairo_surface_t * src, int src_x, int src_y, int dst_x, int dst_y, const cairo_rectangle_int_t * extents, cairo_clip_t * clip)
{
	...
	if(src) {
		...
		compositor->composite(dst, op, data->mask, NULL, extents->x + data->mask_x, extents->y + data->mask_y,
...
static void composite_box(void * closure, int16 x, int16 y, int16 w, int16 h, uint16 coverage)
{
	...
	if(!CAIRO_ALPHA_SHORT_IS_OPAQUE(coverage)) {
		...
		_cairo_color_init_rgba(&color, 0, 0, 0, coverage / (double)0xffff);
		_cairo_pattern_init_solid(&solid, &color);
		mask = compositor->pattern_to_surface(info->dst, &solid.base, FALSE, &_cairo_unbounded_rectangle, &_cairo_unbounded_rectangle, &mask_x, &mask_y);
		if(LIKELY(mask->status == CAIRO_STATUS_SUCCESS)) {
			compositor->composite(info->dst, (cairo_operator_t)info->op, info->src, mask, x + info->src_x,  y + info->src_y, mask_x, mask_y, x, y, w, h);
		}
		cairo_surface_destroy(mask);
    ...
    cairo_operator_t op, cairo_surface_t * src, int src_x, int src_y, int dst_x, int dst_y, const cairo_rectangle_int_t * extents, cairo_clip_t * clip)
{
	...
	cairo_status_t status;
	TRACE_FUNCTION_SIMPLE();
	status = _cairo_clip_get_polygon(clip, &polygon, &fill_rule, &info.antialias);
	if(UNLIKELY(status))
		return status;
	_cairo_traps_init(&info.traps);
	status = _cairo_bentley_ottmann_tessellate_polygon(&info.traps, &polygon, fill_rule);
	_cairo_polygon_fini(&polygon);
	if(UNLIKELY(status))
		return status;
	status = composite_traps(compositor, dst, &info, CAIRO_OPERATOR_SOURCE, data->mask,
		data->mask_x + dst_x, data->mask_y + dst_y, dst_x, dst_y, extents, NULL);
    ...
    cairo_composite_rectangles_t * extents)
{
	...
	if(UNLIKELY(status))
		return status;
	if(extents->mask_pattern.base.type == CAIRO_PATTERN_TYPE_SOLID && extents->clip->path == NULL) {
		status = clip_and_composite(compositor, extents, composite_opacity_boxes, composite_opacity_boxes,
			&extents->mask_pattern, need_unbounded_clip(extents));
	}
	else {
		struct composite_mask data;
		data.mask = compositor->pattern_to_surface(extents->surface, &extents->mask_pattern.base,
			TRUE, &extents->bounded, &extents->mask_sample_area, &data.mask_x, &data.mask_y);
		if(UNLIKELY(data.mask->status))
			return data.mask->status;
		status = clip_and_composite(compositor, extents, composite_mask,
...
static cairo_int_status_t _cairo_traps_compositor_stroke(const cairo_compositor_t * _compositor, cairo_composite_rectangles_t * extents,
    ...
    double tolerance, cairo_antialias_t antialias)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		composite_traps_info_t info;
		unsigned flags;
		if(oneof2(antialias, CAIRO_ANTIALIAS_BEST, CAIRO_ANTIALIAS_GOOD)) {
...
static cairo_int_status_t _cairo_traps_compositor_fill(const cairo_compositor_t * _compositor, cairo_composite_rectangles_t * extents,
...
static cairo_int_status_t _cairo_traps_compositor_glyphs(const cairo_compositor_t * _compositor, cairo_composite_rectangles_t * extents,
