...
cairo_status_t _cairo_clip_combine_with_surface(const cairo_clip_t * clip, cairo_surface_t * dst, int dst_x, int dst_y)
{
	...
	while(status == CAIRO_STATUS_SUCCESS && clip_path) {
		status = _cairo_surface_fill(dst, CAIRO_OPERATOR_IN, &_cairo_pattern_white.base, &clip_path->path,
			clip_path->fill_rule, clip_path->tolerance, clip_path->antialias, clip);
...
cairo_surface_t * _cairo_clip_get_surface(const cairo_clip_t * clip, cairo_surface_t * target, int * tx, int * ty)
{
	...
	clip_path = copy_path;
	while(status == CAIRO_STATUS_SUCCESS && clip_path) {
		status = _cairo_surface_fill(surface, CAIRO_OPERATOR_IN, &_cairo_pattern_white.base, &clip_path->path, clip_path->fill_rule,
			clip_path->tolerance, clip_path->antialias, region);
		clip_path = clip_path->prev;
	}
	copy->path = copy_path;
	_cairo_clip_destroy(copy);
...
cairo_surface_t * _cairo_clip_get_image(const cairo_clip_t * clip,
