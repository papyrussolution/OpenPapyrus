...
{
	cairo_gstate_t * gstate = cairo_container_of(observer, cairo_gstate_t, device_transform_observer);
...
void _cairo_gstate_fini(cairo_gstate_t * gstate)
{
 ...
 **/
cairo_status_t _cairo_gstate_restore(cairo_gstate_t ** gstate, cairo_gstate_t ** freelist)
{
	cairo_gstate_t * top = *gstate;
	if(top->next == NULL)
...
cairo_status_t _cairo_gstate_redirect_target(cairo_gstate_t * gstate, cairo_surface_t * child)
{
	...
	cairo_list_move(&gstate->device_transform_observer.link, &gstate->target->device_transform_observers);
	// The clip is in surface backend coordinates for the previous target;
	// translate it into the child's backend coordinates. 
	_cairo_clip_destroy(gstate->clip);
	gstate->clip = _cairo_clip_copy_with_translation(gstate->next->clip,
		static_cast<int>(child->device_transform.x0 - gstate->parent_target->device_transform.x0),
 ...
 **/
cairo_surface_t * _cairo_gstate_get_original_target(cairo_gstate_t * gstate)
{
 ...
 **/
cairo_clip_t * _cairo_gstate_get_clip(cairo_gstate_t * gstate)
{
	return gstate->clip;
}
...
double _cairo_gstate_get_opacity(cairo_gstate_t * gstate)
{
...
cairo_status_t _cairo_gstate_set_dash(cairo_gstate_t * gstate, const double * dash, int num_dashes, double offset)
{
...
void FASTCALL _do_cairo_gstate_backend_to_user(const cairo_gstate_t * gstate, double * x, double * y)
{
	cairo_matrix_transform_point(&gstate->target->device_transform_inverse, x, y);
	cairo_matrix_transform_point(&gstate->ctm_inverse, x, y);
}

void _do_cairo_gstate_backend_to_user_distance(const cairo_gstate_t * gstate, double * x, double * y)
{
...
void _cairo_gstate_backend_to_user_rectangle(cairo_gstate_t * gstate, double * x1, double * y1, double * x2, double * y2, cairo_bool_t * is_tight)
{
	...
	if(!_cairo_matrix_is_identity(&gstate->target->device_transform_inverse) || !_cairo_matrix_is_identity(&gstate->ctm_inverse)) {
		cairo_matrix_multiply(&matrix_inverse, &gstate->target->device_transform_inverse, &gstate->ctm_inverse);
		_cairo_matrix_transform_bounding_box(&matrix_inverse, x1, y1, x2, y2, is_tight);
	}
	else {
...
{
	_cairo_gstate_copy_transformed_pattern(gstate, pattern, mask, &gstate->ctm_inverse);
}
...
cairo_status_t _cairo_gstate_mask(cairo_gstate_t * gstate, cairo_pattern_t * mask)
{
	...
	cairo_operator_t op;
	cairo_status_t status = _cairo_gstate_get_pattern_status(mask);
	if(unlikely(status))
		return status;
...
#undef M
		...
		status = _cairo_surface_paint(gstate->target, op, &source_pattern.base, gstate->clip);
	}
	else {
		status = _cairo_surface_mask(gstate->target, op, source, &mask_pattern.base, gstate->clip);
	}
	return status;
}
...
cairo_status_t _cairo_gstate_fill(cairo_gstate_t * gstate, cairo_path_fixed_t * path)
{
	...
	if(_cairo_path_fixed_fill_is_empty(path)) {
		if(_cairo_operator_bounded_by_mask(gstate->op))
			return CAIRO_STATUS_SUCCESS;
		status = _cairo_surface_paint(gstate->target, CAIRO_OPERATOR_CLEAR, &_cairo_pattern_clear.base, gstate->clip);
	}
	else {
		cairo_pattern_union_t source_pattern;
		const cairo_pattern_t * pattern;
		cairo_rectangle_int_t extents;
		cairo_box_t box;
		cairo_operator_t op = _reduce_op(gstate);
		if(op == CAIRO_OPERATOR_CLEAR) {
			pattern = &_cairo_pattern_clear.base;
		}
		else {
		...
		if(_cairo_surface_get_extents(gstate->target, &extents) && _cairo_path_fixed_is_box(path, &box) &&
		    box.p1.x <= _cairo_fixed_from_int(extents.x) && box.p1.y <= _cairo_fixed_from_int(extents.y) &&
		    box.p2.x >= _cairo_fixed_from_int(extents.x + extents.width) && box.p2.y >= _cairo_fixed_from_int(extents.y + extents.height)) {
			status = _cairo_surface_paint(gstate->target, op, pattern, gstate->clip);
		}
		else {
			status = _cairo_surface_fill(gstate->target, op, pattern, path, gstate->fill_rule, gstate->tolerance, gstate->antialias, gstate->clip);
		}
	}
	return status;
}
...
cairo_status_t _cairo_gstate_stroke_extents(cairo_gstate_t * gstate, cairo_path_fixed_t * path, double * x1, double * y1, double * x2, double * y2)
{
	...
	if(_cairo_path_fixed_stroke_is_rectilinear(path)) {
		...
		status = _cairo_path_fixed_stroke_rectilinear_to_boxes(path, &gstate->stroke_style, &gstate->ctm, gstate->antialias, &boxes);
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		status = _cairo_path_fixed_stroke_to_polygon(path, &gstate->stroke_style, &gstate->ctm, &gstate->ctm_inverse, gstate->tolerance, &polygon);
...
cairo_status_t _cairo_gstate_fill_extents(cairo_gstate_t * gstate, cairo_path_fixed_t * path, double * x1, double * y1, double * x2, double * y2)
{
	...
	if(_cairo_path_fixed_fill_is_rectilinear(path)) {
		cairo_boxes_t boxes;
		_cairo_boxes_init(&boxes);
		status = _cairo_path_fixed_fill_rectilinear_to_boxes(path, gstate->fill_rule, gstate->antialias, &boxes);
		empty = boxes.num_boxes == 0;
		if(!empty)
			_cairo_boxes_extents(&boxes, &extents);
		_cairo_boxes_fini(&boxes);
		...
		cairo_traps_t traps;
		_cairo_traps_init(&traps);
		status = _cairo_path_fixed_fill_to_traps(path, gstate->fill_rule, gstate->tolerance, &traps);
	...
	if(!empty) {
		_cairo_gstate_extents_to_user_rectangle(gstate, &extents, x1, y1, x2, y2);
	}
	return status;
}
...
{
	gstate->clip = _cairo_clip_intersect_path(gstate->clip, path, gstate->fill_rule, gstate->tolerance, gstate->antialias);
	/* XXX */
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _cairo_gstate_ensure_scaled_font(cairo_gstate_t * gstate)
{
	...
	cairo_matrix_multiply(&font_ctm, &gstate->ctm, &gstate->target->device_transform);
	scaled_font = cairo_scaled_font_create(gstate->font_face, &gstate->font_matrix, &font_ctm, &options);
...
cairo_status_t _cairo_gstate_glyph_extents(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs, int num_glyphs, cairo_text_extents_t * extents)
{
	cairo_status_t status = _cairo_gstate_ensure_scaled_font(gstate);
	if(unlikely(status))
		return status;
...
cairo_status_t _cairo_gstate_show_text_glyphs(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs, int num_glyphs, const cairo_glyph_text_info_t * info)
{
	...
	if(info != NULL) {
		...
		_cairo_gstate_transform_glyphs_to_backend(gstate, glyphs, num_glyphs, info->clusters, info->num_clusters,
		    info->cluster_flags, transformed_glyphs, &num_glyphs, transformed_clusters);
	}
	else {
		_cairo_gstate_transform_glyphs_to_backend(gstate, glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE,
		    transformed_glyphs, &num_glyphs, NULL);
	}
	if(num_glyphs == 0)
		goto CLEANUP_GLYPHS;
	op = _reduce_op(gstate);
	...
	if(cairo_surface_has_show_text_glyphs(gstate->target) || _cairo_scaled_font_get_max_scale(gstate->scaled_font) <= 10240) {
		if(info != NULL) {
			...
			status = _cairo_surface_show_text_glyphs(gstate->target, op, pattern, NULL, 0,
				transformed_glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE, gstate->scaled_font, gstate->clip);
		}
	}
	else {
		cairo_path_fixed_t path;
		...
		if(status == CAIRO_STATUS_SUCCESS) {
			status = _cairo_surface_fill(gstate->target, op, pattern, &path, CAIRO_FILL_RULE_WINDING,
				gstate->tolerance, gstate->scaled_font->options.antialias, gstate->clip);
		}
...
cairo_status_t _cairo_gstate_glyph_path(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs, int num_glyphs, cairo_path_fixed_t * path)
{
	cairo_glyph_t stack_transformed_glyphs[CAIRO_STACK_ARRAY_LENGTH(cairo_glyph_t)];
	cairo_glyph_t * transformed_glyphs;
	cairo_status_t status = _cairo_gstate_ensure_scaled_font(gstate);
...
cairo_status_t _cairo_gstate_set_antialias(cairo_gstate_t * gstate, cairo_antialias_t antialias)
{
...
static void _cairo_gstate_transform_glyphs_to_backend(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs, int num_glyphs,
    ...
    cairo_glyph_t * transformed_glyphs, int * num_transformed_glyphs, cairo_text_cluster_t * transformed_clusters)
{
...
#define KEEP_GLYPH(glyph) (x1 <= glyph.P.x && glyph.P.x <= x2 && y1 <= glyph.P.y && glyph.P.y <= y2)
	...
	if(_cairo_matrix_is_identity(ctm) && _cairo_matrix_is_identity(device_transform) && font_matrix->x0 == 0 && font_matrix->y0 == 0) {
		if(!drop) {
			...
			memcpy(transformed_clusters, clusters, num_clusters * sizeof(cairo_text_cluster_t));
			j = num_glyphs;
		}
	...
	else if(_cairo_matrix_is_translation(ctm) && _cairo_matrix_is_translation(device_transform)) {
		const double tx = font_matrix->x0 + ctm->x0 + device_transform->x0;
		const double ty = font_matrix->y0 + ctm->y0 + device_transform->y0;
		...
		cairo_matrix_t aggregate_transform;
		cairo_matrix_init_translate(&aggregate_transform, gstate->font_matrix.x0, gstate->font_matrix.y0);
		cairo_matrix_multiply(&aggregate_transform, &aggregate_transform, ctm);
		cairo_matrix_multiply(&aggregate_transform, &aggregate_transform, device_transform);
		if(!drop || num_clusters == 0) {
			for(i = 0; i < num_glyphs; i++) {
				transformed_glyphs[j] = glyphs[i];
				cairo_matrix_transform_point(&aggregate_transform, &transformed_glyphs[j].P.x, &transformed_glyphs[j].P.y);
				if(!drop || KEEP_GLYPH(transformed_glyphs[j]))
					j++;
			}
			memcpy(transformed_clusters, clusters, num_clusters * sizeof(cairo_text_cluster_t));
