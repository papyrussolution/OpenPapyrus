...
boolint FASTCALL _cairo_rectangle_intersect(cairo_rectangle_int_t * dst, const cairo_rectangle_int_t * src)
{
	...
	const int x2 = MIN(dst->x + (int)dst->width,  src->x + (int)src->width);
	const int y2 = MIN(dst->y + (int)dst->height, src->y + (int)src->height);
	if(x1 >= x2 || y1 >= y2) {
		dst->x = 0;
		dst->y = 0;
		dst->width  = 0;
...
{
	_cairo_box_add_point(static_cast<cairo_box_t *>(closure), point);
	return CAIRO_STATUS_SUCCESS;
}

/* assumes a has been previously added */
void _cairo_box_add_curve_to(cairo_box_t * extents, const cairo_point_t * a, const cairo_point_t * b,
...
{
	recti->x = ffloori(rectf->x);
	recti->y = ffloori(rectf->y);
	recti->width  = fceili(rectf->x + rectf->width) - ffloori(rectf->x);
	recti->height = fceili(rectf->y + rectf->height) - ffloori(rectf->y);
}
