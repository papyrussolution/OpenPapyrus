...
static cairo_int_status_t set_clip_region(void * _surface,
...
static cairo_int_status_t copy_image_boxes(void * _dst,
    ...
    cairo_xlib_surface_t * surface)
{
	...
	if(_cairo_fixed_integer_part(b->p1.x) > 0 || _cairo_fixed_integer_part(b->p1.y) > 0)
		return FALSE;
	if(_cairo_fixed_integer_part(b->p2.x) < surface->width || _cairo_fixed_integer_part(b->p2.y) < surface->height)
		return FALSE;
	return TRUE;
}
...
static cairo_int_status_t fill_rectangles(void * abstract_surface,
    cairo_operator_t op,
    const cairo_color_t * color,
    ...
    int num_rects)
{
	...
	int i;
	//X_DEBUG ((display->display, "fill_rectangles (dst=%x)", (uint) surface->drawable));
...
static cairo_int_status_t fill_boxes(void * abstract_surface,
    cairo_operator_t op,
    const cairo_color_t * color,
    cairo_boxes_t * boxes)
{
...
//X_DEBUG ((display->display, "composite (dst=%x)", (uint) dst->drawable));
...
static cairo_int_status_t composite(void * abstract_dst,
    cairo_operator_t op,
...
static cairo_int_status_t lerp(void * abstract_dst,
    cairo_surface_t * abstract_src,
    cairo_surface_t * abstract_mask,
    int src_x,
    int src_y,
...
static cairo_int_status_t composite_boxes(void * abstract_dst,
    cairo_operator_t op,
    cairo_surface_t * abstract_src,
    cairo_surface_t * abstract_mask,
    int src_x,
...
void _cairo_xlib_font_close(cairo_xlib_font_t * priv)
{
...
static int _cairo_xlib_get_glyphset_index_for_format(cairo_format_t format)
{
...
static cairo_xlib_font_glyphset_t * _cairo_xlib_font_get_glyphset_info_for_format(cairo_xlib_display_t * display,
    cairo_scaled_font_t * font,
...
static cairo_xlib_font_glyphset_t * find_pending_free_glyph(cairo_xlib_display_t * display,
    cairo_scaled_font_t * font,
    ulong glyph_index,
...
static cairo_status_t _cairo_xlib_surface_add_glyph(cairo_xlib_display_t * display, cairo_scaled_font_t * font, cairo_scaled_glyph_t ** pscaled_glyph)
{
	...
	ulong glyph_index = _cairo_scaled_glyph_index(glyph);
	/* check to see if we have a pending XRenderFreeGlyph for this glyph */
	cairo_xlib_font_glyphset_t * info = find_pending_free_glyph(display, font, glyph_index, glyph_surface);
	if(info)
	...
	if(glyph_surface->format != info->format) {
		...
		cairo_surface_t * tmp_surface = cairo_image_surface_create(info->format, glyph_surface->width, glyph_surface->height);
		status = tmp_surface->status;
		if(UNLIKELY(status))
			goto BAIL;
		tmp_surface->device_transform = glyph_surface->base.device_transform;
	...
	XRenderAddGlyphs(display->display, info->glyphset, &glyph_index, &glyph_info, 1, (char *)data, glyph_surface->stride * glyph_surface->height);
...
typedef void (* cairo_xrender_composite_text_func_t)(Display * dpy, int op, Picture src, Picture dst,
    _Xconst XRenderPictFormat * maskFormat, int xSrc, int ySrc, int xDst, int yDst, _Xconst XGlyphElt8 * elts, int nelt);
 ...
 */
#define _start_new_glyph_elt(count, glyph) (((count) & 127) == 0 || (glyph)->i.x || (glyph)->i.y)

static cairo_status_t _emit_glyphs_chunk(cairo_xlib_display_t * display, cairo_xlib_surface_t * dst, int dst_x, int dst_y,
    cairo_xlib_glyph_t * glyphs, int num_glyphs, cairo_scaled_font_t * font, boolint use_mask, cairo_operator_t op,
    cairo_xlib_source_t * src, int src_x, int src_y, /* info for this chunk */int num_elts, int width, cairo_xlib_font_glyphset_t * info)
{
	...
	char * p_char8 = (char *)glyphs;
	unsigned short * p_char16 = (unsigned short*)glyphs;
	uint * p_char32 = (uint *)glyphs;
	int i;
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(_start_new_glyph_elt(j, &glyphs[i])) {
			...
			elts[nelt].chars = p_char8 + size * j;
			elts[nelt].glyphset = info->glyphset;
			elts[nelt].xOff = glyphs[i].i.x;
			elts[nelt].yOff = glyphs[i].i.y;
		}
		switch(width) {
			case 1: p_char8 [j] = (char)glyphs[i].index; break;
			case 2: p_char16[j] = (unsigned short)glyphs[i].index; break;
			default:
			case 4: p_char32[j] = (uint)glyphs[i].index; break;
		}
		n++;
		j++;
	}
	...
	composite_text_func(display->display, op, src->picture, dst->picture, use_mask ? info->xrender_format : NULL,
	    src_x + elts[0].xOff + dst_x, src_y + elts[0].yOff + dst_y, elts[0].xOff, elts[0].yOff, (XGlyphElt8*)elts, nelt);
	if(elts != stack_elts)
		SAlloc::F(elts);
	return CAIRO_STATUS_SUCCESS;
}

static cairo_int_status_t check_composite_glyphs(const cairo_composite_rectangles_t * extents, cairo_scaled_font_t * font,
...
static cairo_int_status_t composite_glyphs(void * surface,
    cairo_operator_t op,
    cairo_surface_t * _src,
    int src_x,
    int src_y,
    ...
    cairo_composite_glyphs_info_t * info)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		// Convert absolute glyph position to relative-to-current-point position 
	...
	if(num_elts) {
		status = _emit_glyphs_chunk(display, dst, dst_x, dst_y, glyphs, i, info->font, info->use_mask, op, src, src_x, src_y, num_elts, width, glyphset);
	}
	return status;
}
...
#if 0
static cairo_int_status_T check_composite_trapezoids()
{
...
static cairo_int_status_t composite_traps(void * abstract_dst,
    cairo_operator_t op,
    cairo_surface_t * abstract_src,
    int src_x,
    int src_y,
    ...
    cairo_traps_t * traps)
{
	...
	//X_DEBUG ((display->display, "composite_trapezoids (dst=%x)", (uint) dst->drawable));
...
static cairo_int_status_t composite_tristrip(void * abstract_dst,
    cairo_operator_t op,
    cairo_surface_t * abstract_src,
    int src_x,
    int src_y,
...
const cairo_compositor_t * _cairo_xlib_traps_compositor_get(void)
{
	static cairo_atomic_once_t once = CAIRO_ATOMIC_ONCE_INIT;
	static cairo_traps_compositor_t compositor;
	if(_cairo_atomic_init_once_enter(&once)) {
		_cairo_traps_compositor_init(&compositor, _cairo_xlib_mask_compositor_get());
		compositor.acquire = acquire;
		compositor.release = release;
		compositor.set_clip_region = set_clip_region;
		compositor.pattern_to_surface = _cairo_xlib_source_create_for_pattern;
...
#endif // } 0 CAIRO_HAS_XLIB_SURFACE 