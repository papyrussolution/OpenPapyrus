...
{
	cairo_font_options_t * options = (cairo_font_options_t *)SAlloc::M_zon0(sizeof(cairo_font_options_t));
	if(!options) {
...
cairo_font_options_t * cairo_font_options_copy(const cairo_font_options_t * original)
{
	...
	if(cairo_font_options_status((cairo_font_options_t*)original))
		return (cairo_font_options_t*)&_cairo_font_options_nil;
	options = (cairo_font_options_t *)SAlloc::M_zon0(sizeof(cairo_font_options_t));
	if(!options) {
		_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
		return (cairo_font_options_t*)&_cairo_font_options_nil;
	}
