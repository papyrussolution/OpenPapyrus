 ...
 * Since: 1.0
 **/
void FASTCALL cairo_matrix_init(cairo_matrix_t * matrix, double xx, double yx, double xy, double yy, double x0, double y0)
{
 ...
 * Since: 1.0
 **/
void FASTCALL cairo_matrix_init_translate(cairo_matrix_t * matrix, double tx, double ty)
{
 ...
 * Since: 1.0
 **/
void FASTCALL cairo_matrix_init_rotate(cairo_matrix_t * matrix, double radians)
{
...
static cairo_status_t _cairo_matrix_to_pixman_matrix(const cairo_matrix_t * matrix, pixman_transform_t * pixman_transform, double xc, double yc)
{
	...
	// We didn't find an exact match between cairo and pixman, but the matrix should be mostly correct 
	return CAIRO_STATUS_SUCCESS;
}
	...
	if(is_pixman_translation) {
		*out_transform = pixman_identity_transform;
		...
		if(m.x0 != 0.0 || m.y0 != 0.0) {
			...
			tx = floor(tx);
			ty = floor(ty);
			*x_offset = static_cast<int>(-tx);
			*y_offset = static_cast<int>(-ty);
