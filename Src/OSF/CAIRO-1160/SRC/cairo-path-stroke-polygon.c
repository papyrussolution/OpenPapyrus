...
static void contour_add_point(struct stroker * stroker, struct stroker::stroke_contour * c, const cairo_point_t * point)
{
...
static void inner_close(struct stroker * stroker, const cairo_stroke_face_t * in, cairo_stroke_face_t * out)
{
	...
	for(chain = &inner->contour.chain; chain; chain = chain->next) {
		for(i = 0; i < chain->num_points; i++) {
			p = &chain->points[i];
			if((d_p = sign * distance_from_face(in, p)) >= line_width && distance_from_edge(stroker, inpt, &last, p) < line_width) {
				goto out;
			}
...
static void outer_close(struct stroker * stroker, const cairo_stroke_face_t * in, const cairo_stroke_face_t * out)
{
	...
	struct stroker::stroke_contour * outer;
	int clockwise;
...
static void add_cap(struct stroker * stroker, const cairo_stroke_face_t * f, struct stroker::stroke_contour * c)
{
...
static void add_trailing_cap(struct stroker * stroker, const cairo_stroke_face_t * face, struct stroker::stroke_contour * c)
{
