...
static void FASTCALL _cairo_scaled_glyph_fini(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t * scaled_glyph)
{
	while(!cairo_list_is_empty(&scaled_glyph->dev_privates)) {
		cairo_scaled_glyph_private_t * p_private = cairo_list_first_entry(&scaled_glyph->dev_privates, cairo_scaled_glyph_private_t, link);
...
static const cairo_scaled_font_t _cairo_scaled_font_nil = {
	...
	{ /* options */
		...
		CAIRO_LCD_FILTER_DEFAULT, // @sobolev
		CAIRO_HINT_STYLE_DEFAULT,
		CAIRO_HINT_METRICS_DEFAULT,
		CAIRO_ROUND_GLYPH_POS_DEFAULT, // @sobolev
		0 // @sobolev
	},
	FALSE,                  /* placeholder */
	FALSE,                  /* holdover */
...
static cairo_scaled_font_map_t * _cairo_scaled_font_map_lock(void)
{
	...
	if(cairo_scaled_font_map == NULL) {
		...
		cairo_scaled_font_map->hash_table = _cairo_hash_table_create(_cairo_scaled_font_keys_equal);
...
void _cairo_scaled_font_map_destroy(void)
{
	...
	while(font_map->num_holdovers) {
		...
		_cairo_hash_table_remove(font_map->hash_table, &scaled_font->hash_entry);
...
{
	cairo_scaled_glyph_page_t * page = static_cast<cairo_scaled_glyph_page_t *>(closure);
	cairo_scaled_font_t * scaled_font;
...
cairo_status_t _cairo_scaled_font_register_placeholder_and_unlock_font_map(cairo_scaled_font_t * scaled_font)
{
	...
	status = _cairo_scaled_font_init(placeholder_scaled_font, scaled_font->font_face, &scaled_font->font_matrix, &scaled_font->ctm, &scaled_font->options, NULL);
	if(UNLIKELY(status))
		goto FREE_PLACEHOLDER;
	placeholder_scaled_font->placeholder = TRUE;
	placeholder_scaled_font->hash_entry.hash = _cairo_scaled_font_compute_hash(placeholder_scaled_font);
	status = _cairo_hash_table_insert(cairo_scaled_font_map->hash_table, &placeholder_scaled_font->hash_entry);
	if(UNLIKELY(status))
...
FREE_PLACEHOLDER:
	SAlloc::F(placeholder_scaled_font);
...
void _cairo_scaled_font_unregister_placeholder_and_lock_font_map(cairo_scaled_font_t * scaled_font)
{
	...
	placeholder_scaled_font = (cairo_scaled_font_t *)_cairo_hash_table_lookup(cairo_scaled_font_map->hash_table, &scaled_font->hash_entry);
	...
	_cairo_hash_table_remove(cairo_scaled_font_map->hash_table, &placeholder_scaled_font->hash_entry);
	CAIRO_MUTEX_UNLOCK(_cairo_scaled_font_map_mutex);
...
static uint32 FASTCALL _cairo_scaled_font_compute_hash(cairo_scaled_font_t * scaled_font)
{
	...
	hash ^= (ulong)scaled_font->original_font_face;
...
static void _cairo_scaled_font_init_key(cairo_scaled_font_t * scaled_font, cairo_font_face_t * font_face,
...
static boolint _cairo_scaled_font_keys_equal(const void * abstract_key_a, const void * abstract_key_b)
{
	...
	return key_a->original_font_face == key_b->original_font_face && memcmp((uchar *)(&key_a->font_matrix.xx), (uchar *)(&key_b->font_matrix.xx),
		   sizeof(cairo_matrix_t)) == 0 && memcmp((uchar *)(&key_a->ctm.xx),
    ...
    const cairo_matrix_t * font_matrix, const cairo_matrix_t * ctm, const cairo_font_options_t * options)
{
	return scaled_font->original_font_face == font_face && memcmp((uchar *)(&scaled_font->font_matrix.xx), (uchar *)(&font_matrix->xx), sizeof(cairo_matrix_t)) == 0 &&
		memcmp((uchar *)(&scaled_font->ctm.xx), (uchar *)(&ctm->xx), sizeof(cairo_matrix_t)) == 0 && cairo_font_options_equal(&scaled_font->options, options);
}
/*
 * Basic #cairo_scaled_font_t object management
 */

cairo_status_t _cairo_scaled_font_init(cairo_scaled_font_t * scaled_font, cairo_font_face_t * font_face,
    const cairo_matrix_t * font_matrix, const cairo_matrix_t * ctm, const cairo_font_options_t * options, const cairo_scaled_font_backend_t * backend)
{
	...
	scaled_font->max_scale = MAX(fabs(scaled_font->scale.xx) + fabs(scaled_font->scale.xy), fabs(scaled_font->scale.yx) + fabs(scaled_font->scale.yy));
	...
	if(UNLIKELY(status)) {
		...
		if(_cairo_matrix_is_scale_0(&scaled_font->scale)) {
			cairo_matrix_init(&scaled_font->scale_inverse, 0, 0, 0, 0, -scaled_font->scale.x0, -scaled_font->scale.y0);
		}
		else
			return status;
	}
...
void FASTCALL _cairo_scaled_font_freeze_cache(cairo_scaled_font_t * scaled_font)
{
...
void FASTCALL _cairo_scaled_font_thaw_cache(cairo_scaled_font_t * scaled_font)
{
	assert(scaled_font->cache_frozen);
...
void FASTCALL _cairo_scaled_font_reset_cache(cairo_scaled_font_t * scaled_font)
{
	...
	assert(!scaled_font->global_cache_frozen);
	CAIRO_MUTEX_LOCK(_cairo_scaled_glyph_page_cache_mutex);
	cairo_list_foreach_entry(page, cairo_scaled_glyph_page_t, &scaled_font->glyph_pages, link) {
		cairo_scaled_glyph_page_cache.size -= page->cache_entry.size;
		_cairo_hash_table_remove(cairo_scaled_glyph_page_cache.hash_table, (cairo_hash_entry_t*)&page->cache_entry);
	}
	CAIRO_MUTEX_UNLOCK(_cairo_scaled_glyph_page_cache_mutex);
	...
	while(!cairo_list_is_empty(&scaled_font->glyph_pages)) {
		page = cairo_list_first_entry(&scaled_font->glyph_pages, cairo_scaled_glyph_page_t, link);
		_cairo_scaled_glyph_page_destroy(scaled_font, page);
	}
	CAIRO_MUTEX_UNLOCK(scaled_font->mutex);
}

cairo_status_t _cairo_scaled_font_set_metrics(cairo_scaled_font_t * scaled_font, const cairo_font_extents_t * fs_metrics)
{
	cairo_status_t status;
	double font_scale_x, font_scale_y;
	scaled_font->fs_extents = *fs_metrics;
	status = _cairo_matrix_compute_basis_scale_factors(&scaled_font->font_matrix, &font_scale_x, &font_scale_y, 1);
	if(UNLIKELY(status))
		return status;
	/*
...
void _cairo_scaled_font_attach_private(cairo_scaled_font_t * scaled_font, cairo_scaled_font_private_t * pPrivate,
    const void * key, void (*destroy)(cairo_scaled_font_private_t *, cairo_scaled_font_t *))
{
	pPrivate->key = key;
	pPrivate->destroy = destroy;
	cairo_list_add(&pPrivate->link, &scaled_font->dev_privates);
}

cairo_scaled_font_private_t * _cairo_scaled_font_find_private(cairo_scaled_font_t * scaled_font, const void * key)
{
	cairo_scaled_font_private_t * priv;
	cairo_list_foreach_entry(priv, cairo_scaled_font_private_t, &scaled_font->dev_privates, link)
	{
		if(priv->key == key) {
...
void _cairo_scaled_glyph_attach_private(cairo_scaled_glyph_t * scaled_glyph,
    cairo_scaled_glyph_private_t * pPrivate, const void * key,
    void (*destroy)(cairo_scaled_glyph_private_t *, cairo_scaled_glyph_t *, cairo_scaled_font_t *))
{
	pPrivate->key = key;
	pPrivate->destroy = destroy;
	cairo_list_add(&pPrivate->link, &scaled_glyph->dev_privates);
}

cairo_scaled_glyph_private_t * _cairo_scaled_glyph_find_private(cairo_scaled_glyph_t * scaled_glyph, const void * key)
{
	cairo_scaled_glyph_private_t * priv;
	cairo_list_foreach_entry(priv, cairo_scaled_glyph_private_t, &scaled_glyph->dev_privates, link)
	{
		if(priv->key == key) {
    ...
    const cairo_font_options_t * options)
{
	...
	while((scaled_font = (cairo_scaled_font_t *)_cairo_hash_table_lookup(font_map->hash_table, &key.hash_entry))) {
	...
	assert(!scaled_font->cache_frozen);
	assert(!scaled_font->global_cache_frozen);
	scaled_font->original_font_face = cairo_font_face_reference(original_font_face);
	scaled_font->hash_entry.hash = _cairo_scaled_font_compute_hash(scaled_font);
	status = _cairo_hash_table_insert(font_map->hash_table, &scaled_font->hash_entry);
...
void _cairo_scaled_font_reset_static_data(void)
{
	int status;
	CAIRO_MUTEX_LOCK(_cairo_scaled_font_error_mutex);
	for(status = CAIRO_STATUS_SUCCESS; status <= CAIRO_STATUS_LAST_STATUS; status++) {
		SAlloc::F(_cairo_scaled_font_nil_objects[status]);
		_cairo_scaled_font_nil_objects[status] = NULL;
	}
	CAIRO_MUTEX_UNLOCK(_cairo_scaled_font_error_mutex);
	CAIRO_MUTEX_LOCK(_cairo_scaled_glyph_page_cache_mutex);
	if(cairo_scaled_glyph_page_cache.hash_table) {
...
{
	if(scaled_font == NULL || CAIRO_REFERENCE_COUNT_IS_INVALID(&scaled_font->ref_count))
		return scaled_font;
	assert(CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&scaled_font->ref_count));
	_cairo_reference_count_inc(&scaled_font->ref_count);
	return scaled_font;
	...
	if(!CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&scaled_font->ref_count)) {
		if(!scaled_font->placeholder && scaled_font->hash_entry.hash != ZOMBIE) {
			...
			if(font_map->num_holdovers == CAIRO_SCALED_FONT_MAX_HOLDOVERS) {
				...
				font_map->num_holdovers--;
				memmove(&font_map->holdovers[0], &font_map->holdovers[1], font_map->num_holdovers * sizeof(cairo_scaled_font_t*));
			}
 ...
 * Since: 1.4
 **/
cairo_status_t cairo_scaled_font_set_user_data(cairo_scaled_font_t * scaled_font, const cairo_user_data_key_t * key,
    void * user_data, cairo_destroy_func_t destroy)
{
	if(CAIRO_REFERENCE_COUNT_IS_INVALID(&scaled_font->ref_count))
		return scaled_font->status;
	return _cairo_user_data_array_set_data(&scaled_font->user_data, key, user_data, destroy);
}
...
void cairo_scaled_font_glyph_extents(cairo_scaled_font_t * scaled_font, const cairo_glyph_t * glyphs, int num_glyphs, cairo_text_extents_t * extents)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(scaled_glyph->metrics.width == 0 || scaled_glyph->metrics.height == 0)
			continue;
		left = scaled_glyph->metrics.x_bearing + glyphs[i].P.x;
		right = left + scaled_glyph->metrics.width;
		top = scaled_glyph->metrics.y_bearing + glyphs[i].P.y;
	...
	if(visible) {
		extents->x_bearing = min_x - glyphs[0].P.x;
		extents->y_bearing = min_y - glyphs[0].P.y;
	...
	if(num_glyphs) {
		double x0 = glyphs[0].P.x;
		double y0 = glyphs[0].P.y;
		// scaled_glyph contains the glyph for num_glyphs - 1 already.
		double x1 = glyphs[num_glyphs - 1].P.x + scaled_glyph->metrics.x_advance;
		double y1 = glyphs[num_glyphs - 1].P.y + scaled_glyph->metrics.y_advance;
...
static cairo_status_t cairo_scaled_font_text_to_glyphs_internal_cached(cairo_scaled_font_t * scaled_font,
    double x, double y, const char * utf8, cairo_glyph_t * glyphs, cairo_text_cluster_t ** clusters, int num_chars)
{
	...
	for(i = 0; i < num_chars; i++) {
		...
		if(glyph_lut_unicode[idx] == unicode) {
			...
			ulong g = scaled_font->backend->ucs4_to_index(scaled_font, unicode);
			status = _cairo_scaled_glyph_lookup(scaled_font, g, CAIRO_SCALED_GLYPH_INFO_METRICS, &scaled_glyph);
			if(UNLIKELY(status))
				return status;
...
static cairo_status_t cairo_scaled_font_text_to_glyphs_internal_uncached(cairo_scaled_font_t * scaled_font, double x, double y,
    const char * utf8, cairo_glyph_t * glyphs, cairo_text_cluster_t ** clusters, int num_chars)
{
	const char * p = utf8;
    ...
    int utf8_len, cairo_glyph_t ** glyphs, int * num_glyphs, cairo_text_cluster_t ** clusters, int * num_clusters, cairo_text_cluster_flags_t * cluster_flags)
{
	...
	if(cluster_flags) {
		*cluster_flags = CAIRO_TEXT_CLUSTER_FLAG_NONE;
	}
	if(!clusters && cluster_flags) {
		cluster_flags = NULL;
	}
	/* Apart from that, no negatives */
	if(utf8_len < 0 || *num_glyphs < 0 || (num_clusters && *num_clusters < 0)) {
    ...
    cairo_region_t * clip_region)
{
	...
	if(scaled_font->backend->show_glyphs) {
...
static cairo_status_t FASTCALL _add_unit_rectangle_to_path(cairo_path_fixed_t * path, cairo_fixed_t x, cairo_fixed_t y)
{
	cairo_status_t status = _cairo_path_fixed_move_to(path, x, y);
	if(UNLIKELY(status))
		return status;
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(1), _cairo_fixed_from_int(0));
	if(UNLIKELY(status))
		return status;
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(0), _cairo_fixed_from_int(1));
	if(UNLIKELY(status))
		return status;
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(-1), _cairo_fixed_from_int(0));
	if(UNLIKELY(status))
...
cairo_status_t _cairo_scaled_font_glyph_path(cairo_scaled_font_t * scaled_font, const cairo_glyph_t * glyphs, int num_glyphs, cairo_path_fixed_t * path)
{
	...
	for(int i = 0; i < num_glyphs; i++) {
		...
		if(status == CAIRO_INT_STATUS_SUCCESS) {
			...
			if(UNLIKELY(status))
				goto BAIL;
			status = _trace_mask_to_path(scaled_glyph->surface, path, glyphs[i].P.x, glyphs[i].P.y);
		}
		if(UNLIKELY(status))
			goto BAIL;
	}
...
static cairo_status_t _cairo_scaled_font_allocate_glyph(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t ** scaled_glyph)
{
	...
	if(!cairo_list_is_empty(&scaled_font->glyph_pages)) {
		page = cairo_list_last_entry(&scaled_font->glyph_pages, cairo_scaled_glyph_page_t, link);
	...
	if(scaled_font->global_cache_frozen == FALSE) {
		if(UNLIKELY(cairo_scaled_glyph_page_cache.hash_table == NULL)) {
			status = _cairo_cache_init(&cairo_scaled_glyph_page_cache, NULL,
				_cairo_scaled_glyph_page_can_remove, _cairo_scaled_glyph_page_pluck, MAX_GLYPH_PAGES_CACHED);
			if(UNLIKELY(status)) {
				CAIRO_MUTEX_UNLOCK(_cairo_scaled_glyph_page_cache_mutex);
				SAlloc::F(page);
				return status;
	...
	status = _cairo_cache_insert(&cairo_scaled_glyph_page_cache, &page->cache_entry);
	CAIRO_MUTEX_UNLOCK(_cairo_scaled_glyph_page_cache_mutex);
	if(UNLIKELY(status)) {
		SAlloc::F(page);
		return status;
...
static void FASTCALL _cairo_scaled_font_free_last_glyph(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_scaled_glyph_page_t * page;
	assert(scaled_font->cache_frozen);
 ...
 **/
cairo_int_status_t FASTCALL _cairo_scaled_glyph_lookup(cairo_scaled_font_t * scaled_font, ulong index, cairo_scaled_glyph_info_t info, cairo_scaled_glyph_t ** scaled_glyph_ret)
{
	...
	*scaled_glyph_ret = NULL;
	 ...
	 */
	scaled_glyph = (cairo_scaled_glyph_t *)_cairo_hash_table_lookup(scaled_font->glyphs, (cairo_hash_entry_t*)&index);
	if(scaled_glyph == NULL) {
		status = _cairo_scaled_font_allocate_glyph(scaled_font, &scaled_glyph);
		if(UNLIKELY(status))
			goto err;
		memzero(scaled_glyph, sizeof(cairo_scaled_glyph_t));
		_cairo_scaled_glyph_set_index(scaled_glyph, index);
		cairo_list_init(&scaled_glyph->dev_privates);
		/* ask backend to initialize metrics and shape fields */
		status = scaled_font->backend->scaled_glyph_init(scaled_font, scaled_glyph, info | CAIRO_SCALED_GLYPH_INFO_METRICS);
		if(UNLIKELY(status)) {
			_cairo_scaled_font_free_last_glyph(scaled_font, scaled_glyph);
			goto err;
		}
		status = _cairo_hash_table_insert(scaled_font->glyphs, &scaled_glyph->hash_entry);
		if(UNLIKELY(status)) {
			_cairo_scaled_font_free_last_glyph(scaled_font, scaled_glyph);
			goto err;
		}
	 ...
	 */
	need_info = (cairo_scaled_glyph_info_t)(info & ~scaled_glyph->has_info);
	if(need_info) {
		status = scaled_font->backend->scaled_glyph_init(scaled_font, scaled_glyph, need_info);
		if(UNLIKELY(status))
			goto err;
...
cairo_font_face_t * cairo_scaled_font_get_font_face(cairo_scaled_font_t * scaled_font)
{
	...
	if(scaled_font->original_font_face)
...
{
	if(scaled_font->backend && scaled_font->backend->has_color_glyphs)
