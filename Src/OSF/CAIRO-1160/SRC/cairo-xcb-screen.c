...
static void _cairo_xcb_init_screen_font_options(cairo_xcb_screen_t * screen)
{
	...
	// the rest of the code in this function is copied from _cairo_xlib_init_screen_font_options in cairo-xlib-screen.c
	if(res.xft_hinting) {
		switch(res.xft_hintstyle) {
			case FC_HINT_NONE: hint_style = CAIRO_HINT_STYLE_NONE; break;
			case FC_HINT_SLIGHT: hint_style = CAIRO_HINT_STYLE_SLIGHT; break;
			case FC_HINT_MEDIUM: hint_style = CAIRO_HINT_STYLE_MEDIUM; break;
			case FC_HINT_FULL: hint_style = CAIRO_HINT_STYLE_FULL; break;
			default: hint_style = CAIRO_HINT_STYLE_DEFAULT;
		}
	}
	else {
	...
	switch(res.xft_rgba) {
		case FC_RGBA_RGB: subpixel_order = CAIRO_SUBPIXEL_ORDER_RGB; break;
		case FC_RGBA_BGR: subpixel_order = CAIRO_SUBPIXEL_ORDER_BGR; break;
		case FC_RGBA_VRGB: subpixel_order = CAIRO_SUBPIXEL_ORDER_VRGB; break;
		case FC_RGBA_VBGR: subpixel_order = CAIRO_SUBPIXEL_ORDER_VBGR; break;
...
cairo_status_t _cairo_xcb_screen_store_linear_picture(cairo_xcb_screen_t * screen, const cairo_linear_pattern_t * linear, cairo_surface_t * picture)
{
	...
	entry->key.hash = _cairo_linear_pattern_hash(SlConst::DjbHashInit32, linear);
	entry->key.size = 1;
	...
	entry->screen = screen;
	status = _cairo_cache_insert(&screen->linear_pattern_cache, &entry->key);
	if(UNLIKELY(status)) {
...
cairo_surface_t * _cairo_xcb_screen_lookup_linear_picture(cairo_xcb_screen_t * screen, const cairo_linear_pattern_t * linear)
{
	...
	struct pattern_cache_entry * entry;
	assert(CAIRO_MUTEX_IS_LOCKED(screen->connection->device.mutex));
	tmpl.key.hash = _cairo_linear_pattern_hash(SlConst::DjbHashInit32, linear);
...
cairo_status_t _cairo_xcb_screen_store_radial_picture(cairo_xcb_screen_t * screen, const cairo_radial_pattern_t * radial, cairo_surface_t * picture)
{
	...
	if(UNLIKELY(entry == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	entry->key.hash = _cairo_radial_pattern_hash(SlConst::DjbHashInit32, radial);
	entry->key.size = 1;
...
cairo_surface_t * _cairo_xcb_screen_lookup_radial_picture(cairo_xcb_screen_t * screen, const cairo_radial_pattern_t * radial)
{
	cairo_surface_t * picture = NULL;
	struct pattern_cache_entry tmpl;
	struct pattern_cache_entry * entry;
	assert(CAIRO_MUTEX_IS_LOCKED(screen->connection->device.mutex));
	tmpl.key.hash = _cairo_radial_pattern_hash(SlConst::DjbHashInit32, radial);
