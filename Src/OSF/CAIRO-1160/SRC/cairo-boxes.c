...
void _cairo_boxes_init_from_rectangle(cairo_boxes_t * boxes, int x, int y, int w, int h)
{
...
void _cairo_boxes_init_for_array(cairo_boxes_t * boxes, cairo_box_t * array, int num_boxes)
{
	int n;
	boxes->status = CAIRO_STATUS_SUCCESS;
	boxes->num_limits = 0;
	...
	for(n = 0; n < num_boxes; n++) {
		if(!_cairo_fixed_is_integer(array[n].p1.x) || !_cairo_fixed_is_integer(array[n].p1.y) || 
			!_cairo_fixed_is_integer(array[n].p2.x) || !_cairo_fixed_is_integer(array[n].p2.y)) {
			break;
		}
	}
 ...
 */
cairo_box_t * _cairo_boxes_to_array(const cairo_boxes_t * boxes, int * num_boxes)
{
	const cairo_boxes_t::_cairo_boxes_chunk * chunk;
	cairo_box_t * box;
	int i, j;
...
void _cairo_debug_print_boxes(FILE * stream, const cairo_boxes_t * boxes)
{
	const cairo_boxes_t::_cairo_boxes_chunk * chunk;
	cairo_box_t extents;
	int i;
