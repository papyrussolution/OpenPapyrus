    ...
    HDC original_dc, cairo_format_t format, int width, int height, uchar ** bits_out, int * rowstride_out)
{
	...
	switch(format) {
		...
		case CAIRO_FORMAT_RGB30: return _cairo_error(CAIRO_STATUS_INVALID_FORMAT);
		case CAIRO_FORMAT_ARGB32:
		case CAIRO_FORMAT_RGB24: num_palette = 0; break;
		case CAIRO_FORMAT_A8: num_palette = 256; break;
...
static cairo_surface_t * _cairo_win32_display_surface_create_for_dc(HDC original_dc, cairo_format_t format, int width, int height)
{
	cairo_status_t status;
	cairo_device_t * device;
	uchar * bits;
	int rowstride;
	cairo_win32_display_surface_t * surface = static_cast<cairo_win32_display_surface_t *>(SAlloc::M_zon0(sizeof(*surface)));
	if(surface == NULL)
...
static cairo_surface_t * _cairo_win32_display_surface_create_similar_image(void * abstract_other, cairo_format_t format, int width, int height)
{
...
static cairo_int_status_t _cairo_win32_display_surface_unmap_image(void * abstract_surface, cairo_image_surface_t * image)
{
	...
	if(surface->fallback) {
		...
		r.x = static_cast<int>(image->base.device_transform_inverse.x0);
		r.y = static_cast<int>(image->base.device_transform_inverse.y0);
		r.width  = image->width;
...
static cairo_int_status_t _cairo_win32_save_initial_clip(HDC hdc, cairo_win32_display_surface_t * surface)
{
	...
	// using a device offset.  Cairo does not handle extents with negative offsets.
	// 
	surface->win32.x_ofs = 0;
	surface->win32.y_ofs = 0;
	/* @sobolev Этот участок кода приводит к тому, что изображение дочернего окна смещается к началу родительского. 
	  Вероятно, нужны дополнительные изыскания для корректировки.
	if((surface->win32.extents.x < 0) || (surface->win32.extents.y < 0)) {
		// Negative offsets occurs for (and ONLY for) the desktop DC (virtual
		// desktop), when a monitor extend to the left or above the primary monitor.
		// 
		...
		// raster operations between the DC (may have to perform offset translation).
...
cairo_status_t _cairo_win32_display_surface_set_clip(const cairo_win32_display_surface_t * surface, const cairo_clip_t * clip)
{
	...
	if(data_size > sizeof(stack)) {
		data = static_cast<RGNDATA *>(SAlloc::M_zon0(data_size));
		if(!data)
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	}
	else
...
{
	XFORM saved_xform;
	const cairo_win32_surface_t & r_w32 = surface->win32;
	int gm = GetGraphicsMode(r_w32.dc);
	if(gm == GM_ADVANCED) {
		GetWorldTransform(r_w32.dc, &saved_xform);
		ModifyWorldTransform(r_w32.dc, NULL, MWT_IDENTITY);
	}
	// initial_clip_rgn will either be a real region or NULL (which means reset to no clip region) 
	SelectClipRgn(r_w32.dc, surface->initial_clip_rgn);
	if(surface->had_simple_clip) {
		// then if we had a simple clip, intersect 
		IntersectClipRect(r_w32.dc, r_w32.extents.x, r_w32.extents.y, r_w32.extents.x + r_w32.extents.width, r_w32.extents.y + r_w32.extents.height);
	}
	if(gm == GM_ADVANCED)
		SetWorldTransform(r_w32.dc, &saved_xform);
}
...
static cairo_int_status_t _cairo_win32_display_surface_paint(void * surface, cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
...
cairo_surface_t * cairo_win32_surface_create_with_format(HDC hdc, cairo_format_t format)
{
	...
	surface = static_cast<cairo_win32_display_surface_t *>(SAlloc::M_zon0(sizeof(*surface)));
	if(surface == NULL)
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
