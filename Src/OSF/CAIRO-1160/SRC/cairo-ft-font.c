 ...
 *
 * Contributor(s): Graydon Hoare <graydon@redhat.com> Owen Taylor <otaylor@redhat.com> Keith Packard <keithp@keithp.com> Carl Worth <cworth@cworth.org>
 */
#include "cairoint.h"
#pragma hdrstop
...
typedef struct _cairo_ft_options {
	...
	uint load_flags; /* flags for FT_Load_Glyph */
	uint synth_flags;
} cairo_ft_options_t;
...
extern const cairo_unscaled_font_backend_t cairo_ft_unscaled_font_backend;

#if CAIRO_HAS_FC_FONT
static cairo_status_t _cairo_ft_font_options_substitute(const cairo_font_options_t * options,
    FcPattern * pattern);
static cairo_font_face_t * _cairo_ft_resolve_pattern(FcPattern * pattern,
...
static cairo_status_t _ft_to_cairo_error(FT_Error error)
{
	 ...
	 * Populate as needed. */
	switch(error) {
		case FT_Err_Out_Of_Memory: return CAIRO_STATUS_NO_MEMORY;
		default: return CAIRO_STATUS_FREETYPE_ERROR;
	}
}
...
static boolint _cairo_ft_scaled_font_is_vertical(cairo_scaled_font_t * scaled_font);
...
{
	cairo_ft_unscaled_font_t * unscaled = (cairo_ft_unscaled_font_t *)entry;
	cairo_ft_unscaled_font_map_t * font_map = (cairo_ft_unscaled_font_map_t *)closure;
	_cairo_hash_table_remove(font_map->hash_table, &unscaled->base.hash_entry);
...
static void _cairo_ft_unscaled_font_init_key(cairo_ft_unscaled_font_t * key, boolint from_face, char * filename, int id, FT_Face face)
{
	ulong hash;
 ...
 **/
static cairo_status_t _cairo_ft_unscaled_font_init(cairo_ft_unscaled_font_t * unscaled, boolint from_face, const char * filename, int id, FT_Face face)
{
	_cairo_unscaled_font_init(&unscaled->base, &cairo_ft_unscaled_font_backend);
	...
	if(from_face) {
...
#ifdef HAVE_FT_GET_VAR_DESIGN_COORDINATES
		{
			...
			if(0 == FT_Get_MM_Var(face, &ft_mm_var)) {
				unscaled->variations = SAlloc::C(ft_mm_var->num_axis, sizeof(FT_Fixed));
    ...
    FT_Face font_face, cairo_ft_unscaled_font_t ** out)
{
	...
	cairo_ft_unscaled_font_map_t * font_map = _cairo_ft_unscaled_font_map_lock();
	if(UNLIKELY(font_map == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	...
	status = _cairo_hash_table_insert(font_map->hash_table, &unscaled->base.hash_entry);
	if(UNLIKELY(status))
...
static cairo_status_t _cairo_ft_unscaled_font_create_for_pattern(FcPattern * pattern, cairo_ft_unscaled_font_t ** out)
{
	...
	char * filename = NULL;
	int id = 0;
	FcResult ret = FcPatternGetFTFace(pattern, FC_FT_FACE, 0, &font_face);
	...
	*out = NULL;
	return CAIRO_STATUS_SUCCESS;
DONE:
	return _cairo_ft_unscaled_font_create_internal(font_face != NULL, filename, id, font_face, out);
}
...
static boolint _cairo_ft_unscaled_font_destroy(void * abstract_font)
{
	cairo_ft_unscaled_font_t * unscaled  = (cairo_ft_unscaled_font_t *)abstract_font;
	cairo_ft_unscaled_font_map_t * font_map = _cairo_ft_unscaled_font_map_lock();
...
static boolint _has_unlocked_face(const void * entry)
{
	const cairo_ft_unscaled_font_t * unscaled = (const cairo_ft_unscaled_font_t *)entry;
 ...
 */
static cairo_warn FT_Face _cairo_ft_unscaled_font_lock_face(cairo_ft_unscaled_font_t * unscaled)
{
	cairo_ft_unscaled_font_map_t * font_map;
	FT_Face face = NULL;
	FT_Error error;
...
static cairo_status_t _compute_transform(cairo_ft_font_transform_t * sf, cairo_matrix_t * scale, cairo_ft_unscaled_font_t * unscaled)
{
	...
	if(unscaled && (unscaled->face->face_flags & FT_FACE_FLAG_SCALABLE) == 0) {
		...
		for(int i = 0; i < unscaled->face->num_fixed_sizes; i++) {
	...
	_cairo_matrix_get_affine(&normalized, &sf->shape[0][0], &sf->shape[0][1], &sf->shape[1][0], &sf->shape[1][1], NULL, NULL);
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _cairo_ft_unscaled_font_set_scale(cairo_ft_unscaled_font_t * unscaled, cairo_matrix_t * scale)
{
	...
	if(unscaled->have_scale && scale->xx == unscaled->current_scale.xx &&
	    scale->yx == unscaled->current_scale.yx && scale->xy == unscaled->current_scale.xy && scale->yy == unscaled->current_scale.yy)
		return CAIRO_STATUS_SUCCESS;
 ...
 */
static void _fill_xrender_bitmap(FT_Bitmap * target,
    FT_GlyphSlot slot,
    FT_Render_Mode mode,
    int bgr)
{
		    ...
		    if(!bgr) {
			    for(h = height; h > 0; h--, srcLine += 3 * src_pitch, dstLine += pitch) {
				    ...
				    for(x = 0; x < width; x++, src += 1) {
					    uint pix;
					    pix = ((uint)src[0] << 16) |
						((uint)src[src_pitch] <<  8) |
						((uint)src[src_pitch*2]      ) |
						((uint)src[src_pitch] << 24);
					    dst[x] = pix;
				    }
			    }
		    }
 ...
 */
static cairo_status_t _get_bitmap_surface(FT_Bitmap * bitmap, FT_Library library, boolint own_buffer,
    cairo_font_options_t * font_options, cairo_image_surface_t ** surface)
{
	uchar * data;
	int format = CAIRO_FORMAT_A8;
	int stride;
	cairo_image_surface_t * image;
	boolint component_alpha = FALSE;
	uint width = bitmap->width;
	uint height = bitmap->rows;
	if(width == 0 || height == 0) {
		*surface = reinterpret_cast<cairo_image_surface_t *>(cairo_image_surface_create_for_data(NULL, static_cast<cairo_format_t>(format), 0, 0, 0));
	...
	switch(bitmap->pixel_mode) {
		case FT_PIXEL_MODE_MONO:
		    ...
		    if(own_buffer) {
			    ...
			    data = (uchar *)_cairo_malloc_ab(height, stride);
			    if(!data)
				    return _cairo_error(CAIRO_STATUS_NO_MEMORY);
			    if(stride == bitmap->pitch) {
				    ...
				    for(i = height; i; i--) {
					    ...
					    memzero(dest + bitmap->pitch, stride - bitmap->pitch);
					    source += bitmap->pitch;
					    dest += stride;
				    }
			    }
		...
		case FT_PIXEL_MODE_LCD_V:
		case FT_PIXEL_MODE_GRAY:
		    if(font_options->antialias != CAIRO_ANTIALIAS_SUBPIXEL || bitmap->pixel_mode == FT_PIXEL_MODE_GRAY) {
			    stride = bitmap->pitch;
			    ...
			    if(own_buffer) {
				    ...
				    data = static_cast<uchar *>(_cairo_malloc_ab(height, stride));
				    if(!data)
					    return _cairo_error(CAIRO_STATUS_NO_MEMORY);
				    memcpy(data, bitmap->buffer, stride * height);
			    }
...
convert:
		    if(!own_buffer && library) {
			    ...
			    FT_Error error;
			    format = CAIRO_FORMAT_A8;
			    align = cairo_format_stride_for_width(static_cast<cairo_format_t>(format), bitmap->width);
			    FT_Bitmap_New(&tmp);
			    ...
			    *bitmap = tmp;
			    ...
			    if(bitmap->num_grays != 256) {
				    uint x, y;
				    uint mul = 255 / (bitmap->num_grays - 1);
	...
	/* XXX */
	*surface = image = (cairo_image_surface_t*)cairo_image_surface_create_for_data(data, static_cast<cairo_format_t>(format), width, height, stride);
 ...
 */
static cairo_status_t _render_glyph_outline(FT_Face face,
    ...
    cairo_image_surface_t ** surface)
{
	...
	width = (uint)((cbox.xMax - cbox.xMin) >> 6);
	height = (uint)((cbox.yMax - cbox.yMin) >> 6);
	...
	if(width * height == 0) {
		...
		if(bitmap_size < 0)
			return _cairo_error(CAIRO_STATUS_INVALID_FORMAT);
		bitmap.buffer = static_cast<uchar *>(SAlloc::C(1, bitmap_size));
...
static cairo_status_t _render_glyph_bitmap(FT_Face face, cairo_font_options_t * font_options, cairo_image_surface_t ** surface)
{
	 ...
	 */
	FT_Error error = FT_Render_Glyph(glyphslot, FT_RENDER_MODE_NORMAL);
	/* XXX ignoring all other errors for now.  They are not fatal, typically
	 * just a glyph-not-found. */
	if(error == FT_Err_Out_Of_Memory)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _get_bitmap_surface(&glyphslot->bitmap, glyphslot->library, FALSE, font_options, surface);
	if(UNLIKELY(status))
		return status;
	/*
	 * Note: the font's coordinate system is upside down from ours, so the
	 ...
	 * Another negation.
	 */
	cairo_surface_set_device_offset(&(*surface)->base, -glyphslot->bitmap_left, +glyphslot->bitmap_top);
	return CAIRO_STATUS_SUCCESS;
}
...
extern const cairo_scaled_font_backend_t _cairo_ft_scaled_font_backend;
    ...
    cairo_ft_options_t * other)
{
	...
	if(other->base.variations) {
		if(options->base.variations) {
			...
			char * p = static_cast<char *>(SAlloc::M(strlen(other->base.variations) + strlen(options->base.variations) + 2));
			p[0] = 0;
			strcat(p, other->base.variations);
			strcat(p, ",");
			strcat(p, options->base.variations);
			SAlloc::F(options->base.variations);
...
static cairo_status_t _cairo_ft_font_face_scaled_font_create(void * abstract_font_face, const cairo_matrix_t * font_matrix,
    const cairo_matrix_t * ctm, const cairo_font_options_t * options, cairo_scaled_font_t   ** font_out)
{
	cairo_ft_font_face_t * font_face = static_cast<cairo_ft_font_face_t *>(abstract_font_face);
	...
	status = _cairo_scaled_font_init(&scaled_font->base, &font_face->base, font_matrix, ctm, options, &_cairo_ft_scaled_font_backend);
	if(UNLIKELY(status))
		goto CLEANUP_SCALED_FONT;
	status = _cairo_ft_unscaled_font_set_scale(unscaled, &scaled_font->base.scale);
	if(UNLIKELY(status)) {
...
boolint _cairo_scaled_font_is_ft(cairo_scaled_font_t * scaled_font)
{
...
static void cairo_ft_apply_variations(FT_Face face, cairo_ft_scaled_font_t * scaled_font)
{
	...
	if(!ret) {
		...
		uint i;
		const char * p;
		FT_Fixed * coords = (FT_Fixed *)SAlloc::M(sizeof(FT_Fixed) * ft_mm_var->num_axis);
		/* FIXME check coords. */
...
static cairo_int_status_t _cairo_ft_scaled_glyph_load_glyph(cairo_ft_scaled_font_t * scaled_font,
    cairo_scaled_glyph_t * scaled_glyph, FT_Face face, int load_flags, boolint use_em_size, boolint vertical_layout)
{
	FT_Error error;
	cairo_status_t status;
	if(use_em_size) {
		...
		status = _cairo_ft_unscaled_font_set_scale(scaled_font->unscaled, &scaled_font->base.scale);
	}
	if(UNLIKELY(status))
		return status;
	cairo_ft_apply_variations(face, scaled_font);
	error = FT_Load_Glyph(face, _cairo_scaled_glyph_index(scaled_glyph), load_flags);
	/* XXX ignoring all other errors for now.  They are not fatal, typically
	 * just a glyph-not-found. */
	if(error == FT_Err_Out_Of_Memory)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_int_status_t _cairo_ft_scaled_glyph_init(void * abstract_font, cairo_scaled_glyph_t * scaled_glyph, cairo_scaled_glyph_info_t info)
{
	...
	if((info & CAIRO_SCALED_GLYPH_INFO_PATH) != 0 && (info & (CAIRO_SCALED_GLYPH_INFO_SURFACE|CAIRO_SCALED_GLYPH_INFO_COLOR_SURFACE)) == 0) {
	...
	if(info & CAIRO_SCALED_GLYPH_INFO_METRICS) {
		boolint hint_metrics = scaled_font->base.options.hint_metrics != CAIRO_HINT_METRICS_OFF;
		status = _cairo_ft_scaled_glyph_load_glyph(scaled_font, scaled_glyph, face, load_flags, !hint_metrics, vertical_layout);
		if(UNLIKELY(status))
			goto FAIL;
		glyph = face->glyph;
...
LOAD:
	if(info & (CAIRO_SCALED_GLYPH_INFO_SURFACE | CAIRO_SCALED_GLYPH_INFO_COLOR_SURFACE)) {
		...
		if(!scaled_glyph_loaded) {
			status = _cairo_ft_scaled_glyph_load_glyph(scaled_font, scaled_glyph, face, load_flags, FALSE, vertical_layout);
			if(UNLIKELY(status))
				goto FAIL;
			glyph = face->glyph;
		...
		if(glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
			status = _render_glyph_outline(face, &scaled_font->ft_options.base, &surface);
		}
		else {
		...
		if(UNLIKELY(status))
			goto FAIL;
		if(pixman_image_get_format(surface->pixman_image) == PIXMAN_a8r8g8b8 && !pixman_image_get_component_alpha(surface->pixman_image)) {
			_cairo_scaled_glyph_set_color_surface(scaled_glyph, &scaled_font->base, surface);
		}
		else {
	...
	if(((info & (CAIRO_SCALED_GLYPH_INFO_SURFACE | CAIRO_SCALED_GLYPH_INFO_COLOR_SURFACE)) != 0) &&
		...
		info = (cairo_scaled_glyph_info_t)((int)info & ~CAIRO_SCALED_GLYPH_INFO_METRICS);
		load_flags &= ~FT_LOAD_COLOR;
		goto LOAD;
	}
	...
	*is_synthetic = FALSE;
	...
	if(face->face_flags & FT_FACE_FLAG_MULTIPLE_MASTERS) {
		...
		*is_synthetic = TRUE;
		...
		coords = static_cast<FT_Fixed *>(_cairo_malloc_ab(num_axis, sizeof(FT_Fixed)));
		if(!coords) {
			status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
			goto cleanup;
		}
...
static cairo_int_status_t _cairo_index_to_glyph_name(void * abstract_font, char ** glyph_names, int num_glyph_names,
    ulong glyph_index, ulong * glyph_array_index)
{
	cairo_ft_scaled_font_t * scaled_font = static_cast<cairo_ft_scaled_font_t *>(abstract_font);
	cairo_ft_unscaled_font_t * unscaled = scaled_font->unscaled;
...
static boolint _ft_is_type1(FT_Face face)
{
...
static boolint _cairo_ft_has_color_glyphs(void * scaled)
{
...
static cairo_status_t _cairo_ft_font_face_create_for_toy(cairo_toy_font_face_t * toy_face,
    cairo_font_face_t ** font_face_out)
{
	...
	if(!FcPatternAddString(pattern, FC_FAMILY, (uchar *)toy_face->family)) {
		_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
		goto FREE_PATTERN;
	}
...
static boolint _cairo_ft_font_face_destroy(void * abstract_face)
{
	cairo_ft_font_face_t * font_face = static_cast<cairo_ft_font_face_t *>(abstract_face);
...
static cairo_font_face_t * _cairo_ft_font_face_get_implementation(void * abstract_face, const cairo_matrix_t * font_matrix,
    const cairo_matrix_t * ctm, const cairo_font_options_t * options)
{
	cairo_ft_font_face_t * font_face = static_cast<cairo_ft_font_face_t *>(abstract_face);
...
#endif
	return static_cast<cairo_font_face_t *>(abstract_face);
}
    ...
    cairo_ft_options_t * ft_options)
{
	...
	if(unscaled->faces && unscaled->faces->unscaled == NULL) {
		/* This "zombie" font_face (from _cairo_ft_font_face_destroy) is no longer needed. */
...
static cairo_status_t _cairo_ft_font_options_substitute(const cairo_font_options_t * options, FcPattern * pattern)
{
	...
	if(options->antialias != CAIRO_ANTIALIAS_DEFAULT) {
		if(FcPatternGet(pattern, FC_ANTIALIAS, 0, &v) == FcResultNoMatch) {
			if(!FcPatternAddBool(pattern, FC_ANTIALIAS, options->antialias != CAIRO_ANTIALIAS_NONE))
				return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
cairo_font_face_t * cairo_ft_font_face_create_for_ft_face(FT_Face face, int load_flags)
{
	...
	cairo_font_face_t * font_face;
	cairo_ft_options_t ft_options;
	cairo_status_t status = _cairo_ft_unscaled_font_create_from_face(face, &unscaled);
	if(UNLIKELY(status))
		return (cairo_font_face_t*)&_cairo_font_face_nil;
