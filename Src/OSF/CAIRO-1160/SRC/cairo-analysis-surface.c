...
typedef struct {
	...
	boolint first_op;
	boolint has_supported;
	boolint has_unsupported;
...
static cairo_int_status_t _analyze_recording_surface_pattern(cairo_analysis_surface_t * surface, const cairo_pattern_t * pattern, cairo_rectangle_int_t * extents)
{
...
{
	cairo_analysis_surface_t * surface = static_cast<cairo_analysis_surface_t *>(abstract_surface);
...
static boolint _cairo_analysis_surface_get_extents(void * abstract_surface, cairo_rectangle_int_t * rectangle)
{
	cairo_analysis_surface_t * surface = static_cast<cairo_analysis_surface_t *>(abstract_surface);
	return _cairo_surface_get_extents(surface->target, rectangle);
}

static void _rectangle_intersect_clip(cairo_rectangle_int_t * extents, const cairo_clip_t * clip)
{
	if(clip != NULL)
...
{
	boolint is_empty = _cairo_surface_get_extents(&surface->base, extents);
	...
	if(backend_status == CAIRO_INT_STATUS_ANALYZE_RECORDING_SURFACE_PATTERN) {
		...
		if(source->type == CAIRO_PATTERN_TYPE_SURFACE) {
			...
			if(_cairo_surface_is_recording(src_surface)) {
				backend_source_status = _analyze_recording_surface_pattern(surface, source, &rec_extents);
		...
		if(mask->type == CAIRO_PATTERN_TYPE_SURFACE) {
			...
			if(_cairo_surface_is_recording(mask_surface)) {
				backend_mask_status = _analyze_recording_surface_pattern(surface, mask, &rec_extents);
...
static cairo_int_status_t _cairo_analysis_surface_show_glyphs(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source,
...
static boolint _cairo_analysis_surface_has_show_text_glyphs(void * abstract_surface)
{
	cairo_analysis_surface_t * surface = static_cast<cairo_analysis_surface_t *>(abstract_surface);
    ...
    const cairo_clip_t * clip)
{
	...
	if(surface->target->backend->show_text_glyphs != NULL) {
		backend_status = surface->target->backend->show_text_glyphs(surface->target, op, source, utf8, utf8_len,
			glyphs, num_glyphs, clusters, num_clusters, cluster_flags, scaled_font, clip);
		if(_cairo_int_status_is_error(backend_status))
			return backend_status;
	}
	if(backend_status == CAIRO_INT_STATUS_UNSUPPORTED && surface->target->backend->show_glyphs != NULL) {
		backend_status = surface->target->backend->show_glyphs(surface->target, op, source, glyphs, num_glyphs, scaled_font, clip);
		if(_cairo_int_status_is_error(backend_status))
			return backend_status;
	}
...
static cairo_int_status_t _cairo_analysis_surface_tag(void * abstract_surface, boolint begin, const char * tag_name,
    const char * attributes, const cairo_pattern_t * source, const cairo_stroke_style_t * stroke_style,
    const cairo_matrix_t * ctm, const cairo_matrix_t * ctm_inverse, const cairo_clip_t * clip)
{
	cairo_analysis_surface_t * surface = static_cast<cairo_analysis_surface_t *>(abstract_surface);
	cairo_int_status_t backend_status = CAIRO_INT_STATUS_SUCCESS;
	if(surface->target->backend->tag != NULL) {
		backend_status = surface->target->backend->tag(surface->target, begin, tag_name, attributes, source,
			stroke_style, ctm, ctm_inverse, clip);
		if(_cairo_int_status_is_error(backend_status))
			return backend_status;
	}
	return backend_status;
...
cairo_surface_t * _cairo_analysis_surface_create(cairo_surface_t * target)
{
	...
	_cairo_surface_init(&surface->base, &cairo_analysis_surface_backend, NULL/* device */, CAIRO_CONTENT_COLOR_ALPHA, target->is_vector);
