...
static cairo_clip_path_t * _cairo_clip_path_create(cairo_clip_t * clip)
{
	...
	if(unlikely(clip_path == NULL)) {
		clip_path = (cairo_clip_path_t *)_cairo_malloc(sizeof(cairo_clip_path_t));
		if(unlikely(clip_path == NULL))
			return NULL;
	}
...
cairo_clip_t * _cairo_clip_create(void)
{
	...
	if(unlikely(clip == NULL)) {
		clip = (cairo_clip_t *)_cairo_malloc(sizeof(cairo_clip_t));
		if(unlikely(clip == NULL))
			return NULL;
	}
...
cairo_clip_t * _cairo_clip_intersect_path(cairo_clip_t * clip, const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule,
...
static cairo_clip_t * _cairo_clip_intersect_clip_path(cairo_clip_t * clip, const cairo_clip_path_t * clip_path)
{
	if(clip_path->prev)
...
static cairo_status_t _cairo_path_fixed_init_from_boxes(cairo_path_fixed_t * path, const cairo_boxes_t * boxes)
{
	cairo_status_t status;
...
cairo_clip_t * _cairo_clip_transform(cairo_clip_t * clip, const cairo_matrix_t * m)
{
	...
	if(_cairo_matrix_is_translation(m))
		return _cairo_clip_translate(clip, static_cast<int>(m->x0), static_cast<int>(m->y0));
	copy = _cairo_clip_create();
	if(clip->num_boxes) {
		cairo_path_fixed_t path;
		cairo_boxes_t boxes;
...
const cairo_rectangle_int_t * FASTCALL _cairo_clip_get_extents(const cairo_clip_t * clip)
{
	if(clip == NULL)
...
cairo_rectangle_list_t * _cairo_rectangle_list_create_in_error(cairo_status_t status)
{
	...
	list = (cairo_rectangle_list_t *)_cairo_malloc(sizeof(*list));
