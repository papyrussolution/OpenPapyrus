 ...
 *
 * Contributor(s): Carl D. Worth <cworth@cworth.org> M Joonas Pihlaja <jpihlaja@cc.helsinki.fi> Chris Wilson <chris@chris-wilson.co.uk>
 */
...
static boolint bo_edge_contains_intersect_point(const edge_t * edge, const cairo_bo_intersect_point_t * point)
{
	...
	if(cmp_top == 0) {
		...
		cairo_fixed_t bot_x;
		bot_x = line_compute_intersection_x_for_y(&edge->edge.line, edge->edge.bottom);
...
inline static void coverage_rewind(struct _sweep_line::coverage * cells)
{
	cells->cursor = &cells->head;
}

static void coverage_init(struct _sweep_line::coverage * cells)
{
...
static void coverage_fini(struct _sweep_line::coverage * cells)
{
	_cairo_freepool_fini(&cells->pool);
}

inline static void coverage_reset(struct _sweep_line::coverage * cells)
{
	cells->head.next = &cells->tail;
	cells->tail.prev = &cells->head;
	cells->count = 0;
...
static cairo_status_t _cairo_botor_scan_converter_generate(void * converter, cairo_span_renderer_t * renderer)
{
	...
	event_t ** event_ptrs;
	struct _cairo_botor_scan_converter::_cairo_botor_scan_converter_chunk * chunk;
	cairo_status_t status;
	int i, j;
