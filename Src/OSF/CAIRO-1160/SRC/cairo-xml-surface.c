...
extern const cairo_surface_backend_t _cairo_xml_surface_backend;
...
static void CAIRO_PRINTF_FORMAT(2, 3) _cairo_xml_printf(cairo_xml_t *xml, const char * fmt, ...)
{
	va_list ap;
	char indent[80];
...
static void CAIRO_PRINTF_FORMAT(2, 3) _cairo_xml_printf_start(cairo_xml_t *xml, const char * fmt, ...)
{
	char indent[80];
	int len = MIN(xml->indent, ARRAY_LENGTH(indent));
	memset(indent, ' ', len);
...
static cairo_xml_t * to_xml(cairo_xml_surface_t * surface)
{
...
static cairo_status_t _cairo_xml_surface_emit_clip_path(cairo_xml_surface_t * surface,
...
static cairo_status_t _cairo_xml_surface_emit_clip(cairo_xml_surface_t * surface,
...
static cairo_status_t _cairo_xml_emit_image(cairo_xml_t * xml, cairo_image_surface_t * image)
{
	cairo_output_stream_t * stream;
	cairo_status_t status;
	_cairo_xml_printf_start(xml, "<image width='%d' height='%d' format='%s'>", image->width, image->height, _format_to_string(image->format));
...
static cairo_int_status_t _cairo_xml_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
...
static cairo_int_status_t _cairo_xml_surface_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_path_fixed_t* path,
...
static cairo_status_t _cairo_xml_emit_scaled_font(cairo_xml_t * xml, cairo_scaled_font_t * scaled_font, cairo_glyph_t * glyphs, int num_glyphs)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		status = _cairo_xml_emit_type3_font(xml, scaled_font, glyphs, num_glyphs);
	}
	_cairo_xml_indent(xml, -2);
	_cairo_xml_printf(xml, "<scaled-font>");
	return status;
...
cairo_status_t cairo_xml_for_recording_surface(cairo_device_t * device, cairo_surface_t * recording_surface)
{
	...
	_cairo_xml_printf(xml, "<surface content='%s' width='%d' height='%d'>",
	    _content_to_string(recording_surface->content), extents.width, extents.height);
	_cairo_xml_indent(xml, 2);
