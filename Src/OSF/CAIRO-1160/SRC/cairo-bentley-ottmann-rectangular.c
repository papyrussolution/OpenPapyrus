...
static inline void rectangle_pop_stop(sweep_line_t * sweep)
{
...
static void edge_end_box(sweep_line_t * sweep_line, edge_t * left, int32 bot)
{
	...
	if(LIKELY(left->top < bot)) {
		if(sweep_line->do_traps) {
			...
			_cairo_traps_add_trap((cairo_traps_t *)sweep_line->container, left->top, bot, &_left, &_right);
 ...
 */
static edge_t * sort_edges(edge_t * list,
    uint level,
    edge_t ** head_out)
{
...
static inline void active_edges_to_traps(sweep_line_t * sweep)
{
