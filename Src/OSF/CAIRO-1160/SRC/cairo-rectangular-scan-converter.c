...
static void sweep_line_init(sweep_line_t * sweep)
{
	...
	_cairo_freepool_init(&sweep->coverage.pool, sizeof(struct sweep_line_t::coverage::cell));
...
{
	struct sweep_line_t::coverage::cell * cell = sweep->coverage.cursor;
	if(cell->x > x) {
		do {
			UNROLL3({
				if(cell->prev->x < x)
...
static inline void _active_edges_to_spans(sweep_line_t * sweep)
{
	...
	int prev_x;
	struct sweep_line_t::coverage::cell * cell;
	sweep->num_spans = 0;
...
static cairo_status_t generate_box(cairo_rectangular_scan_converter_t * self, cairo_span_renderer_t * renderer)
{
	...
	if(y2 > y1) {
		if(!_cairo_fixed_is_integer(r->top)) {
			generate_row(renderer, r, y1, 1, 256 - _cairo_fixed_fractional_part(r->top));
			y1++;
		}
		if(y2 > y1)
...
{
	cairo_rectangular_scan_converter_t * self = (cairo_rectangular_scan_converter_t *)converter;
	struct cairo_rectangular_scan_converter_t::_cairo_rectangular_scan_converter_chunk * chunk, * next;
