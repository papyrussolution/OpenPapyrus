...
typedef struct  {
	...
	boolint src_surface_bounded;
	const cairo_rectangle_int_t * src_op_extents; /* operation extents in src space */
	cairo_filter_t filter;
	boolint stencil_mask; /* TRUE if source is to be used as a mask */
...
static boolint _cairo_ps_surface_get_extents(void * abstract_surface, cairo_rectangle_int_t * rectangle);
static void _cairo_ps_form_emit(void * entry, void * closure);

static const cairo_ps_level_t _cairo_ps_levels[] = { CAIRO_PS_LEVEL_2, CAIRO_PS_LEVEL_3 };
...
{
	const cairo_ps_form_t * a = (const cairo_ps_form_t *)key_a;
	const cairo_ps_form_t * b = (const cairo_ps_form_t *)key_b;
...
static void _cairo_ps_form_pluck(void * entry, void * closure)
{
	cairo_ps_form_t * surface_entry = (cairo_ps_form_t *)entry;
...
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(closure);
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_surface_t * type3_surface = _cairo_type3_glyph_surface_create(font_subset->scaled_font, NULL, _cairo_ps_emit_imagemask, surface->font_subsets, TRUE);
	for(uint i = 0; i < font_subset->num_glyphs; i++) {
...
static cairo_int_status_t _cairo_ps_surface_emit_unscaled_font_subset(cairo_scaled_font_subset_t * font_subset,
    void * closure)
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(closure);
...
static cairo_int_status_t _cairo_ps_surface_emit_forms(cairo_ps_surface_t * surface)
{
	_cairo_hash_table_foreach(surface->forms, _cairo_ps_form_emit, surface);
...
static boolint _path_covers_bbox(cairo_ps_surface_t * surface, const cairo_path_fixed_t * path)
{
	cairo_box_t box;
...
static boolint _ps_page_dimension_equal(int a, int b)
{
	return (abs(a - b) < 5);
}
...
static cairo_surface_t * _cairo_ps_surface_create_for_stream_internal(cairo_output_stream_t * stream, double width, double height)
{
	...
	_cairo_array_init(&surface->recording_surf_stack, sizeof(uint));
...
static boolint _cairo_surface_is_ps(cairo_surface_t * surface)
{
 ...
 */
static boolint _extract_ps_surface(cairo_surface_t * surface, boolint set_error_on_failure, cairo_ps_surface_t ** ps_surface)
{
    ...
    double height_in_points)
{
	...
	ps_surface->surface_extents.width  = fceili(ps_surface->width);
	ps_surface->surface_extents.height = fceili(ps_surface->height);
	_cairo_pdf_operators_set_cairo_to_pdf_matrix(&ps_surface->pdf_operators,
	    &ps_surface->cairo_to_ps);
	status = _cairo_paginated_surface_set_size(ps_surface->paginated_surface, static_cast<int>(width_in_points), static_cast<int>(height_in_points));
	if(status)
...
static cairo_status_t _cairo_ps_surface_finish(void * abstract_surface)
{
	...
	if(UNLIKELY(status))
		goto CLEANUP;
	_cairo_output_stream_printf(surface->final_stream, "%%%%EndSetup\n");
	status = _cairo_ps_surface_emit_body(surface);
	if(UNLIKELY(status))
		goto CLEANUP;
	_cairo_ps_surface_emit_footer(surface);
CLEANUP:
	_cairo_hash_table_foreach(surface->forms, _cairo_ps_form_pluck, surface->forms);
...
static boolint color_is_gray(double red, double green, double blue)
{
...
static boolint _gradient_pattern_supported(cairo_ps_surface_t * surface,
    const cairo_pattern_t * pattern)
{
...
static boolint pattern_supported(cairo_ps_surface_t * surface, const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
...
static boolint mask_supported(cairo_ps_surface_t * surface,
...
static boolint _cairo_ps_surface_operation_supported(cairo_ps_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * pattern,
    const cairo_pattern_t * mask,
    const cairo_rectangle_int_t * extents)
...
static cairo_output_stream_t * _base85_strings_stream_create(cairo_output_stream_t * output)
{
	...
	_cairo_output_stream_init(&stream->base, _base85_string_wrap_stream_write, NULL, _base85_string_wrap_stream_close);
...
static cairo_status_t _cairo_ps_surface_emit_base85_string(cairo_ps_surface_t * surface, const uchar * data, ulong length,
    cairo_ps_compress_t compress, boolint use_strings)
{
	...
	for(y = 0; y < ps_image->height; y++) {
		if(params->stencil_mask || use_mask) {
			...
			if(ps_image->format == CAIRO_FORMAT_A1) {
				...
				pixel8 = (uint8 *)(ps_image->data + y * ps_image->stride);
				pixel32 = (uint32 *)(ps_image->data + y * ps_image->stride);
				bit = 7;
...
static cairo_int_status_t _cairo_ps_surface_emit_jpeg_image(cairo_ps_surface_t * surface,
    cairo_emit_surface_mode_t mode,
    cairo_emit_surface_params_t * params)
{
	cairo_status_t status;
...
static cairo_int_status_t _cairo_ps_surface_emit_ccitt_image(cairo_ps_surface_t * surface,
    cairo_emit_surface_mode_t mode,
    cairo_emit_surface_params_t * params)
{
	cairo_status_t status;
	...
	if(UNLIKELY(params->src_surface->status))
		return params->src_surface->status;
	cairo_surface_get_mime_data(params->src_surface, CAIRO_MIME_TYPE_CCITT_FAX, &ccitt_data, &ccitt_data_len);
	if(ccitt_data == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	cairo_surface_get_mime_data(params->src_surface, CAIRO_MIME_TYPE_CCITT_FAX_PARAMS, &ccitt_params_data, &ccitt_params_data_len);
	if(ccitt_params_data == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	/* ensure params_string is null terminated */
	ccitt_params_string = (char *)SAlloc::M(ccitt_params_data_len + 1);
	...
	if(!params->stencil_mask) {
		_cairo_output_stream_printf(surface->stream, "/DeviceGray setcolorspace\n");
	}
	_cairo_output_stream_printf(surface->stream,
	    "<<\n"
	    "  /ImageType 1\n"
...
static cairo_status_t _cairo_ps_surface_emit_eps(cairo_ps_surface_t * surface,
    cairo_emit_surface_mode_t mode,
    cairo_emit_surface_params_t * params)
{
	cairo_status_t status;
	...
	if(surface->ps_level == CAIRO_PS_LEVEL_2)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	cairo_surface_get_mime_data(params->src_surface, CAIRO_MIME_TYPE_EPS, &eps_data, &eps_data_len);
	if(eps_data == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	cairo_surface_get_mime_data(params->src_surface, CAIRO_MIME_TYPE_EPS_PARAMS, &eps_params_string, &eps_params_string_len);
	if(eps_params_string == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	/* ensure params_string is null terminated */
	params_string = (char *)SAlloc::M(eps_params_string_len + 1);
	...
	cairo_matrix_init_translate(&mat, params->src_surface_extents->x, params->src_surface_extents->y);
	cairo_matrix_scale(&mat, params->src_surface_extents->width/eps_width, params->src_surface_extents->height/eps_height);
...
static cairo_status_t _cairo_ps_surface_emit_recording_surface(cairo_ps_surface_t * surface,
    ...
    boolint subsurface)
{
	...
	status = _cairo_recording_surface_replay_region(recording_surface, subsurface ? recording_extents : NULL, &surface->base, CAIRO_RECORDING_REGION_NATIVE);
...
static void _cairo_ps_surface_flatten_transparency(cairo_ps_surface_t * surface, const cairo_color_t * color, double * red, double * green, double * blue)
{
	*red   = color->red;
	*green = color->green;
	*blue  = color->blue;
...
static void _cairo_ps_surface_emit_solid_pattern(cairo_ps_surface_t * surface, const cairo_solid_pattern_t * pattern)
{
 ...
 * image or recording.
 */
static cairo_int_status_t _cairo_ps_surface_use_form(cairo_ps_surface_t * surface,
    cairo_emit_surface_params_t * params,
    boolint test,
    cairo_ps_form_t ** ps_form)
{
	...
	source_entry = (cairo_ps_form_t *)_cairo_hash_table_lookup(surface->forms, &source_key.base);
	if(source_entry) {
	...
	source_entry = (cairo_ps_form_t *)SAlloc::C(sizeof(cairo_ps_form_t), 1);
...
fail:
	SAlloc::F(unique_id);
	SAlloc::F(source_entry);
	return status;
}

static cairo_int_status_t _cairo_ps_surface_emit_form(cairo_ps_surface_t * surface, cairo_emit_surface_params_t * params, boolint test)
{
	cairo_ps_form_t * ps_form = NULL;
	cairo_status_t status = _cairo_ps_surface_use_form(surface, params, test, &ps_form);
	if(test || status)
		return status;
 ...
 */
static cairo_int_status_t _cairo_ps_surface_emit_surface(cairo_ps_surface_t * surface,
    cairo_emit_surface_mode_t mode,
    cairo_emit_surface_params_t * params)
{
	cairo_int_status_t status;
...
static void _cairo_ps_form_emit(void * entry, void * closure)
{
	cairo_ps_form_t * form = (cairo_ps_form_t *)entry;
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(closure);
	cairo_emit_surface_params_t params;
	cairo_int_status_t status;
	...
	_cairo_output_stream_printf(surface->final_stream, "%%%%BeginResource: form cairoform-%d\n", form->id);
	_cairo_output_stream_printf(surface->final_stream, "/cairo_paint_form-%d", form->id);
	if(surface->ps_level == CAIRO_PS_LEVEL_3) {
		surface->paint_proc = FALSE;
		_cairo_output_stream_printf(surface->final_stream,
		    "\n"
...
static void _path_fixed_init_rectangle(cairo_path_fixed_t * path, cairo_rectangle_int_t * rect)
{
	cairo_status_t status;
	_cairo_path_fixed_init(path);
	status = _cairo_path_fixed_move_to(path, _cairo_fixed_from_int(rect->x), _cairo_fixed_from_int(rect->y));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(rect->width), _cairo_fixed_from_int(0));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(0), _cairo_fixed_from_int(rect->height));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(path, _cairo_fixed_from_int(-rect->width), _cairo_fixed_from_int(0));
	assert(status == CAIRO_STATUS_SUCCESS);
...
static cairo_status_t _cairo_ps_surface_paint_surface(cairo_ps_surface_t * surface, const cairo_pattern_t * pattern,
    cairo_rectangle_int_t * extents, cairo_operator_t op, boolint stencil_mask)
{
	cairo_rectangle_int_t src_surface_extents;
	boolint src_surface_bounded;
	cairo_rectangle_int_t src_op_extents;
...
static void _cairo_ps_surface_emit_linear_colorgradient(cairo_ps_surface_t * surface,
...
static cairo_status_t _cairo_ps_surface_emit_pattern_stops(cairo_ps_surface_t * surface, cairo_gradient_pattern_t * pattern)
{
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		...
		if(stops[n_stops-1].offset < 1.0 - COLOR_STOP_EPSILON) {
			if(pattern->base.extend == CAIRO_EXTEND_REFLECT) {
				memcpy(&stops[n_stops], &stops[n_stops - 1], sizeof(cairo_ps_color_stop_t));
			}
			else {
...
static cairo_status_t _cairo_ps_surface_emit_repeating_function(cairo_ps_surface_t * surface, const cairo_gradient_pattern_t * pattern, int begin, int end)
{
	_cairo_output_stream_printf(surface->stream,
	    "/CairoFunction\n"
	    "<< /FunctionType 3\n"
...
static cairo_status_t _cairo_ps_surface_emit_gradient(cairo_ps_surface_t * surface, cairo_gradient_pattern_t * pattern, boolint is_ps_pattern)
{
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		int repeat_begin = ffloori(domain[0]);
		int repeat_end = fceili(domain[1]);
	...
	if(is_ps_pattern) {
		_cairo_output_stream_printf(surface->stream, "<< /PatternType 2\n   /Shading\n");
	}
...
static cairo_status_t _cairo_ps_surface_emit_pattern(cairo_ps_surface_t * surface, const cairo_pattern_t * pattern,
    cairo_rectangle_int_t * extents, cairo_operator_t op)
{
	cairo_status_t status;
	if(pattern->type == CAIRO_PATTERN_TYPE_SOLID) {
		const cairo_solid_pattern_t * solid = reinterpret_cast<const cairo_solid_pattern_t *>(pattern);
		if(surface->current_pattern_is_solid_color == FALSE || !_cairo_color_equal(&surface->current_color, &solid->color)) {
			status = _cairo_pdf_operators_flush(&surface->pdf_operators);
			if(UNLIKELY(status))
				return status;
			_cairo_ps_surface_emit_solid_pattern(surface, reinterpret_cast<const cairo_solid_pattern_t *>(pattern));
...
static boolint _can_paint_pattern(const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		    return FALSE;
...
static cairo_int_status_t _cairo_ps_surface_set_clip(cairo_ps_surface_t * surface,
    ...
    const cairo_clip_t * clip)
{
	...
	if(_can_paint_pattern(source)) {
		...
		_cairo_output_stream_printf(stream, "%d %d %d %d rectfill\n", surface->surface_extents.x,
		    surface->surface_extents.y, surface->surface_extents.width,
		    surface->surface_extents.height);
	}
cleanup_composite:
...
static cairo_int_status_t _cairo_ps_surface_mask(void * abstract_surface, cairo_operator_t op,
    const cairo_pattern_t * source, const cairo_pattern_t * mask, const cairo_clip_t * clip)
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(abstract_surface);
	cairo_output_stream_t * stream = surface->stream;
	cairo_composite_rectangles_t extents;
	cairo_status_t status = _cairo_composite_rectangles_init_for_mask(&extents, &surface->base, op, source, mask, clip);
	if(UNLIKELY(status))
		return status;
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
...
#if DEBUG_PS
	_cairo_output_stream_printf(stream, "%% _cairo_ps_surface_mask\n");
#endif
	status = _cairo_ps_surface_set_clip(surface, &extents);
	if(UNLIKELY(status))
		goto cleanup_composite;
...
static cairo_int_status_t _cairo_ps_surface_stroke(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source,
    ...
    double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(abstract_surface);
	cairo_composite_rectangles_t extents;
	cairo_int_status_t status = _cairo_composite_rectangles_init_for_stroke(&extents, &surface->base, op, source,
		path, style, ctm, clip);
	if(UNLIKELY(status))
		return status;
	...
	assert(_cairo_ps_surface_operation_supported(surface, op, source, NULL, &extents.bounded));
#if DEBUG_PS
	_cairo_output_stream_printf(surface->stream, "%% _cairo_ps_surface_stroke\n");
#endif
	status = _cairo_ps_surface_set_clip(surface, &extents);
	if(UNLIKELY(status))
		goto cleanup_composite;
...
static boolint _cairo_ps_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}
...
static cairo_int_status_t _cairo_ps_surface_set_paginated_mode(void * abstract_surface, cairo_paginated_mode_t paginated_mode)
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(abstract_surface);
	cairo_status_t status;
...
static cairo_int_status_t _cairo_ps_surface_set_bounding_box(void * abstract_surface, cairo_box_t * analysis_bbox)
{
	cairo_ps_surface_t * surface = static_cast<cairo_ps_surface_t *>(abstract_surface);
	int i, num_comments;
	char ** comments;
	boolint has_page_media, has_page_bbox;
	...
	bbox_p1.y = fceili(surface->height) - (page_bbox.y + page_bbox.height);
	bbox_p2.x = page_bbox.x + page_bbox.width;
	bbox_p2.y = fceili(surface->height) - page_bbox.y;
	...
	_cairo_output_stream_printf(surface->stream, "%%%%BeginPageSetup\n");
...
static boolint _cairo_ps_surface_supports_fine_grained_fallbacks(void * abstract_surface)
{
	return TRUE;
}
