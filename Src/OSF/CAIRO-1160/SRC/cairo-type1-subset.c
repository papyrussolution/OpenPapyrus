...
struct cairo_type1_font_subset_t {
...
static cairo_status_t cairo_type1_font_subset_get_matrix(cairo_type1_font_subset_t * font, const char * name, double * a, double * b, double * c, double * d)
{
...
static cairo_status_t cairo_type1_font_subset_get_bbox(cairo_type1_font_subset_t * font)
{
	double x_min, y_min, x_max, y_max;
	double xx, yx, xy, yy;
	cairo_status_t status = cairo_type1_font_subset_get_matrix(font, "/FontBBox", &x_min, &y_min, &x_max, &y_max);
	if(UNLIKELY(status))
		return status;
...
static cairo_status_t cairo_type1_font_subset_get_fontname(cairo_type1_font_subset_t * font)
{
	...
	const char * segment_end = font->header_segment + font->header_segment_size;
	const char * start = find_token(font->header_segment, segment_end, "/FontName");
	if(start == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	...
	s = (char *)SAlloc::M_zon0(end - start + 1);
	if(UNLIKELY(s == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_status_t cairo_type1_font_subset_write_header(cairo_type1_font_subset_t * font, const char * name)
{
	...
	if(start) {
		...
		if(start + 5 < segment_end && memcmp(start, "known", 5) == 0) {
			_cairo_output_stream_write(font->output, font->header_segment, start + 5 - font->header_segment);
...
static cairo_status_t cairo_type1_font_subset_decrypt_eexec_segment(cairo_type1_font_subset_t * font)
{
	...
	font->cleartext = (char *)SAlloc::M_zon0(font->eexec_segment_size + 1);
    ...
    const char * encrypted_charstring, int encrypted_charstring_length)
{
	...
	const uchar * p;
	int command;
	uchar * charstring = static_cast<uchar *>(SAlloc::M_zon0(encrypted_charstring_length));
			...
			switch(command) {
				...
				case TYPE1_CHARSTRING_COMMAND_CALLSUBR:
				    if(font->subset_subrs && font->build_stack.sp > 0) {
					    ...
					    if(modf(font->build_stack.stack[--font->build_stack.sp], &int_val) == 0.0) {
						    int subr_num = static_cast<int>(int_val);
				...
				case TYPE1_CHARSTRING_COMMAND_ESCAPE:
				    ...
				    switch(command) {
						...
						status = use_standard_encoding_glyph(font, static_cast<int>(font->build_stack.stack[3]));
						if(UNLIKELY(status))
							goto cleanup;
						status = use_standard_encoding_glyph(font, static_cast<int>(font->build_stack.stack[4]));
						if(UNLIKELY(status))
							goto cleanup;
						font->build_stack.sp = 0;
						break;
						...
						while(font->build_stack.sp)
							font->ps_stack.stack[font->ps_stack.sp++] = static_cast<int>(font->build_stack.stack[--font->build_stack.sp]);
						break;
    ...
    int glyph_number, const char * name, int name_length, const char * encrypted_charstring, int encrypted_charstring_length)
{
	glyph_data_t glyph;
	cairo_status_t status;
	char * s = (char *)SAlloc::M_zon0(name_length + 1);
	if(UNLIKELY(s == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_status_t cairo_type1_font_subset_write_private_dict(cairo_type1_font_subset_t * font, const char * name)
{
	...
	if((lenIV_start = find_token(font->cleartext, font->cleartext_end, "/lenIV")) != NULL) {
		...
		if(lenIV_end == NULL)
			return CAIRO_INT_STATUS_UNSUPPORTED;
		lenIV_str = static_cast<char *>(SAlloc::M_zon0(lenIV_end - lenIV_start + 1));
	...
	if(font->num_subrs <= 0)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	font->subrs = (cairo_type1_font_subset_t::Subrs *)SAlloc::C(font->num_subrs, sizeof(font->subrs[0]));
	...
	status = cairo_type1_font_for_each_subr(font, array_start, font->cleartext_end, cairo_type1_font_subset_build_subr_list, &array_end);
	if(UNLIKELY(status))
		return status;
	p = array_end;
skip_subrs:
	...
	status = cairo_type1_font_subset_for_each_glyph(font, dict_start, font->cleartext_end, cairo_type1_font_subset_build_glyph_list, &dict_end);
	if(UNLIKELY(status))
		return status;
	...
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		...
		status = backend->index_to_glyph_name(font->scaled_font_subset->scaled_font,
			font->glyph_names, font->base.num_glyphs, font->scaled_font_subset->glyphs[i], &index);
		if(UNLIKELY(status))
			return status;
	...
	status = cairo_type1_font_subset_write_encrypted(font, glyph_count_end, dict_start - glyph_count_end);
	if(UNLIKELY(status))
		return status;
...
{
	cairo_type1_font_subset_t * font = static_cast<cairo_type1_font_subset_t *>(closure);
...
{
	cairo_type1_font_subset_t * font = static_cast<cairo_type1_font_subset_t *>(abstract_font);
	cairo_status_t status;
	ulong data_length;
	...
	font->type1_data = static_cast<char *>(SAlloc::M_zon0(font->type1_length));
	if(UNLIKELY(font->type1_data == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = scaled_font->backend->load_type1_data(scaled_font, 0, reinterpret_cast<uchar *>(font->type1_data), &data_length);
	if(UNLIKELY(status))
		return status;
	...
	if(UNLIKELY(status))
		return status;
	font->base.data = static_cast<const char *>(_cairo_array_index(&font->contents, 0));
	return status;
}
...
static cairo_status_t _cairo_type1_font_subset_fini(cairo_type1_font_subset_t * font)
{
	...
	uint i;
	// If the subset generation failed, some of the pointers below may be NULL depending on at which point the error occurred.
	_cairo_array_fini(&font->contents);
	SAlloc::F(font->type1_data);
	for(i = 0; i < _cairo_array_num_elements(&font->glyph_names_array); i++) {
		char ** s = static_cast<char **>(_cairo_array_index(&font->glyph_names_array, i));
		SAlloc::F(*s);
	}
