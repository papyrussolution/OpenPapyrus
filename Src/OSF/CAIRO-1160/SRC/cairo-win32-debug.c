...
void _cairo_win32_debug_dump_hrgn(HRGN rgn, char * header)
{
	...
	slfprintf_stderr(" %ld rects, bounds: %ld %ld %ld %ld\n",
	    ...
	    rd->rdh.rcBound.right - rd->rdh.rcBound.left, rd->rdh.rcBound.bottom - rd->rdh.rcBound.top);
	for(z = 0; z < rd->rdh.nCount; z++) {
		RECT r = reinterpret_cast<const RECT *>(rd->Buffer)[z];
		slfprintf_stderr(" [%d]: [%ld %ld %ld %ld]\n", z, r.left, r.top, r.right - r.left, r.bottom - r.top);
	}
	SAlloc::F(rd);
	fflush(stderr);
}
