...
cairo_status_t _cairo_surface_offset_paint(cairo_surface_t * target, int x, int y,
...
cairo_status_t _cairo_surface_offset_mask(cairo_surface_t * target, int x, int y, cairo_operator_t op, const cairo_pattern_t * source,
    const cairo_pattern_t * mask, const cairo_clip_t * clip)
{
	...
	status = _cairo_surface_mask(target, op, source, mask, dev_clip);
	if(dev_clip != clip)
		_cairo_clip_destroy(dev_clip);
	return status;
}

cairo_status_t _cairo_surface_offset_stroke(cairo_surface_t * surface, int x, int y,
    ...
    double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	...
	if(x | y) {
		...
		if(unlikely(status))
			goto FINISH;
		_cairo_path_fixed_translate(&path_copy, _cairo_fixed_from_int(-x), _cairo_fixed_from_int(-y));
		dev_path = &path_copy;
		cairo_matrix_init_translate(&m, -x, -y);
	...
	status = _cairo_surface_stroke(surface, op, source, dev_path, stroke_style, &dev_ctm, &dev_ctm_inverse, tolerance, antialias, dev_clip);
FINISH:
	if(dev_path != path)
...
cairo_status_t _cairo_surface_offset_fill(cairo_surface_t * surface, int x, int y, cairo_operator_t op, const cairo_pattern_t* source,
    const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	...
	status = _cairo_surface_fill(surface, op, source, dev_path, fill_rule, tolerance, antialias, dev_clip);
FINISH:
	if(dev_path != path)
		_cairo_path_fixed_fini(dev_path);
	if(dev_clip != clip)
    ...
    const cairo_clip_t * clip)
{
	...
	status = _cairo_surface_show_text_glyphs(surface, op, source, NULL, 0, dev_glyphs, num_glyphs,
		NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font, dev_clip);
	if(dev_clip != clip)
		_cairo_clip_destroy(dev_clip);
	SAlloc::F(dev_glyphs);
	return status;
