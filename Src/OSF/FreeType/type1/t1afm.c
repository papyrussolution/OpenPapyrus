...
FT_LOCAL_DEF(void) T1_Done_Metrics(FT_Memory memory, AFM_FontInfo fi)
{
...
static FT_Int t1_get_index(const char*  name, FT_Offset len, void*        user_data)
{
	...
	for(n = 0; n < type1->num_glyphs; n++) {
		...
		if(gname && gname[0] == name[0] && ft_strlen(gname) == len && ft_strncmp(gname, name, len) == 0)
			return n;
	}
	return 0;
}
...
FT_CALLBACK_DEF(int) compare_kern_pairs(const void*  a, const void*  b)
{
...
FT_LOCAL_DEF(void) T1_Get_Kerning(AFM_FontInfo fi, FT_UInt glyph1, FT_UInt glyph2, FT_Vector*    kerning)
{
	AFM_KernPair mid;
	FT_ULong idx = KERN_INDEX(glyph1, glyph2);
	/* simple binary search */
	AFM_KernPair min = fi->KernPairs;
	AFM_KernPair max = min + fi->NumKernPair - 1;
	while(min <= max) {
		FT_ULong midi;
		mid  = min + ( max - min ) / 2;
...
FT_LOCAL_DEF(FT_Error) T1_Get_Track_Kerning(FT_Face face, FT_Fixed ptsize, FT_Int degree, FT_Fixed*  kerning)
{
