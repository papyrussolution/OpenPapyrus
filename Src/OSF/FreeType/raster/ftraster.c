...
/* @sobolev typedef struct TPoint_ {
	Long x;
	Long y;
} TPoint; */

typedef struct FtLPoint_ { // @sobolev
	Long x;
	Long y;
} FtLPoint; 
...
struct  black_TWorker_ {
	...
	FtLPoint * arc;            /* current Bezier arc pointer          */
	...
	Bool second_pass;           /* indicates whether a horizontal pass */
	                            ...
	                            /* Render_Glyph.                       */
	FtLPoint arcs[3 * MaxBezier + 1];  /* The Bezier stack               */
...
{
	Long h = (Long)( ras.top - ras.cProfile->offset );
	if(h < 0) {
...
{
	Int n       = ras.numTurns - 1;
	PLong y_turns = ras.sizeBuff - ras.numTurns;
 ...
 */
static void Split_Conic(FtLPoint *  base)
{
	Long a, b;
	base[4].x = base[2].x;
	a = base[0].x + base[1].x;
 ...
 *   performance.
 */
static void Split_Cubic(FtLPoint *  base)
{
	Long a, b, c;
	base[6].x = base[3].x;
	a = base[0].x + base[1].x;
...
typedef void (* TSplitter)(FtLPoint *  base);
...
static void FT_Outline_Get_CBox(const FT_Outline*  outline,
    FT_BBox           * acbox)
{
