 ...
 *   step).
 */
static FT_Fixed* ft_var_readpackeddeltas(FT_Stream stream,
    FT_ULong size,
    FT_UInt delta_cnt)
{
...
static FT_Error ft_var_load_item_variation_store(TT_Face face,
    FT_ULong offset,
 ...
 */
static FT_Error ft_var_load_hvvar(TT_Face face,
    FT_Bool vertical)
{
	FT_Stream stream = FT_FACE_STREAM(face);
	FT_Memory memory = stream->memory;
...
static FT_Int ft_var_get_item_delta(TT_Face face,
    GX_ItemVarStore itemStore,
    FT_UInt outerIndex,
    FT_UInt innerIndex)
{
 ...
 */
static FT_Error tt_hvadvance_adjust(TT_Face face,
    FT_UInt gindex,
    FT_Int  * avalue,
    FT_Bool vertical)
{
...
static FT_Short* ft_var_get_value_pointer(TT_Face face,
    FT_ULong mvar_tag)
{
	FT_Short*  p;
...
static FT_Error tt_size_reset_iterator(FT_ListNode node,
    void*        user)
{
...
static void ft_var_to_normalized(TT_Face face,
    FT_UInt num_coords,
    FT_Fixed*  coords,
    FT_Fixed*  normalized)
{
...
static void ft_var_to_design(TT_Face face,
    FT_UInt num_coords,
    FT_Fixed*  coords,
    FT_Fixed*  design)
{
...
static FT_Error tt_set_mm_blend(TT_Face face,
    FT_UInt num_coords,
    FT_Fixed*  coords,
...
{
	FT_Error error = tt_set_mm_blend(face, num_coords, coords, 1);
	if(error)
		return error;
	if(num_coords)
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) TT_Get_MM_Blend(TT_Face face, FT_UInt num_coords, FT_Fixed*  coords)
{
	FT_Error error = FT_Err_Ok;
	GX_Blend blend;
	FT_UInt i, nc;
...
static FT_Error tt_cvt_ready_iterator(FT_ListNode node,
    void*        user)
{
	TT_Size size = (TT_Size)node->data;
...
static void tt_delta_shift(int p1,
    int p2,
    int ref,
    FT_Vector*  in_points,
...
static void tt_delta_interpolate(int p1,
    int p2,
    int ref1,
    int ref2,
    FT_Vector*  in_points,
...
static void tt_interpolate_deltas(FT_Outline*  outline,
    FT_Vector*   out_points,
    FT_Vector*   in_points,
    FT_Bool*     has_delta)
{
