...
FT_LOCAL_DEF(FT_Error) tt_face_load_colr(TT_Face face, FT_Stream stream)
{
	FT_Error error;
...
FT_LOCAL_DEF(FT_Error) tt_face_colr_blend_layer(TT_Face face, FT_UInt color_index, FT_GlyphSlot dstSlot, FT_GlyphSlot srcSlot)
{
	FT_Error error;
	FT_UInt x, y;
	FT_Byte b, g, r, alpha;
	...
	if(!dstSlot->bitmap.buffer) {
		...
		if(error)
			return error;
		memzero(dstSlot->bitmap.buffer, size);
	}
	else{
...
typedef int _tt_colr_dummy; /* ANSI C doesn't like empty source files */
