...
#include <libpng/png.h> // @sobolev <png.h>--><libpng/png.h>
#include "pngshim.h"
#include "sferrors.h"
...
static void error_callback(png_structp png, const char * error_msg)
{
...
static void warning_callback(png_structp png, const char * error_msg)
{
	FT_UNUSED(png);
	FT_UNUSED(error_msg);
...
FT_LOCAL_DEF(FT_Error) Load_SBit_Png(FT_GlyphSlot slot, FT_Int x_offset, FT_Int y_offset, FT_Int pix_bits,
    TT_SBit_Metrics metrics, FT_Memory memory, FT_Byte * data, FT_UInt png_len, FT_Bool populate_map_and_metrics, FT_Bool metrics_only)
{
	...
	FT_Int i;
	uint8 ** rows = NULL; /* pacify compiler */
	if(x_offset < 0 || y_offset < 0) {
		error = FT_THROW(Invalid_Argument);
		goto Exit;
	}
	if(!populate_map_and_metrics && ( (FT_UInt)x_offset + metrics->width  > map->width ||
	    (FT_UInt)y_offset + metrics->height > map->rows  || pix_bits != 32 || map->pixel_mode != FT_PIXEL_MODE_BGRA)) {
		error = FT_THROW(Invalid_Argument);
		goto Exit;
	}
	...
	if(setjmp(png_jmpbuf(png))) { // @sobolev ft_setjmp-->setjmp
