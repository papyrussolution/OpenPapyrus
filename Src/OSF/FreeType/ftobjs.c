...
Adjust:
	...
	bitmap->pixel_mode = (uchar)pixel_mode;
	...
	if(hinter &&
		...
		if(( load_flags & FT_LOAD_FORCE_AUTOHINT ) ||
			...
			if(( mode == FT_RENDER_MODE_LIGHT && ( !FT_DRIVER_HINTS_LIGHTLY(driver) &&
			    !is_light_type1                    )) || ( FT_IS_SFNT(face) &&
			    ttface->num_locations && ttface->max_profile.maxSizeOfInstructions == 0 && ttface->font_program_size == 0 && 
    ...
    FT_Face    * aface)
{
	...
	for(i = 0; i < resource_cnt; i++) {
		...
		if(FT_MAC_RFORK_MAX_LEN < temp || FT_MAC_RFORK_MAX_LEN - temp < pfb_len + 6) {
			FT_TRACE2(( "             MacOS resource length cannot exceed 0x%08lx\n", FT_MAC_RFORK_MAX_LEN ));
	...
	FT_TRACE2(( "             total buffer size to concatenate %ld POST fragments: 0x%08lx\n", resource_cnt, pfb_len + 2 ));
	if(pfb_len + 2 < 6) {
		FT_TRACE2(( "             too long fragment length makes pfb_len confused: pfb_len=0x%08lx\n", pfb_len ));
...
#define FT_COMPONENT  raccess
	...
	for(i = 0; i < FT_RACCESS_N_RULES; i++) {
		...
		FT_TRACE3(( "Try rule %d: %s (offset=%ld) ...", i, args2.pathname, offsets[i]));
...
#define FT_COMPONENT  checksum
	...
	if(!error &&
		...
		if(!err) {
			...
			FT_TRACE3(( "FT_Render_Glyph: bitmap %dx%d, %s (mode %d)\n", pitch,
			    rows, pixel_modes[slot->bitmap.pixel_mode], slot->bitmap.pixel_mode ));
			for(i = 0; i < rows; i++)
				for(j = 0; j < pitch; j++)
			...
			MD5_Init(&ctx);
			if(bitmap.buffer)
				MD5_Update(&ctx, bitmap.buffer, (ulong)rows * (ulong)pitch);
			MD5_Final(md5, &ctx);
	...
	if(!error &&
		...
		if(slot->bitmap.rows  < 128U &&
			...
			uchar * topleft = slot->bitmap.buffer;
    ...
    FT_Bool value_is_string)
{
	FT_Module*           cur;
	FT_Module*           limit;
	FT_Module_Interface p_interface;
	...
	p_interface = cur[0]->clazz->get_interface(cur[0], FT_SERVICE_ID_PROPERTIES);
	if(!p_interface) {
		FT_ERROR(( "%s: module `%s' doesn't support properties\n", func_name, module_name ));
		return FT_THROW(Unimplemented_Feature);
	}
	service = (FT_Service_Properties)p_interface;
	if(set)
...
/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Bool) FT_Get_Color_Glyph_Layer(FT_Face face, FT_UInt base_glyph, FT_UInt *aglyph_index, FT_UInt *acolor_index, FT_LayerIterator*  iterator)
{
	TT_Face ttface;
	SFNT_Service sfnt;
