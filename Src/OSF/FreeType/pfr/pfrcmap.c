...
FT_CALLBACK_DEF(FT_Error) pfr_cmap_init(PFR_CMap cmap, FT_Pointer pointer)
{
	...
	{
		for(FT_UInt n = 1; n < cmap->num_chars; n++) {
...
FT_CALLBACK_DEF(FT_UInt) pfr_cmap_char_index(PFR_CMap cmap, FT_UInt32 char_code)
{
	FT_UInt min = 0;
	FT_UInt max = cmap->num_chars;
	while(min < max) {
		FT_UInt mid   = min + ( max - min ) / 2;
		PFR_Char gchar = cmap->chars + mid;
...
FT_CALLBACK_DEF(FT_UInt32) pfr_cmap_char_next(PFR_CMap cmap, FT_UInt32  *pchar_code)
{
	FT_UInt result    = 0;
