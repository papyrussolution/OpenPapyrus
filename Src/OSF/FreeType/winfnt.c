...
static FT_Error fnt_face_get_dll_font(FNT_Face face, FT_Int face_instance_index)
{
	...
	if(mz_header.magic == WINFNT_MZ_MAGIC) {
		...
		if(ne_header.magic == WINFNT_NE_MAGIC) {
			...
			if(pe32_header.magic != WINFNT_PE_MAGIC /* check full signature */ || pe32_header.machine != 0x014C /* i386 */ || pe32_header.size_of_optional_header != 0xE0 /* FIXME */ || pe32_header.magic32 != 0x10B) {
...
Found_rsrc_section:
			...
			for(i = 0; i < root_dir.number_of_named_entries +
				...
				for(j = 0; j < name_dir.number_of_named_entries +
					...
					if(FT_STREAM_SEEK(name_dir_offset + 16 + j * 8) || FT_STREAM_READ_FIELDS(winpe_rsrc_dir_entry_fields,
					    &dir_entry2))
						goto Exit;
