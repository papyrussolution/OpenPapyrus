...
/*FT_LOCAL_DEF(FT_Error)*/FT_Error t42_parser_init(T42_Parser parser, FT_Stream stream, FT_Memory memory, PSAux_Service psaux)
{
...
static void t42_parse_font_matrix(T42_Face face, T42_Loader loader)
{
	...
	FT_Fixed temp[6];
	FT_Fixed temp_scale;
	FT_Int result = T1_ToFixedArray(parser, 6, temp, 0);
	if(result < 6) {
...
static void t42_parse_encoding(T42_Face face, T42_Loader loader)
{
	...
	if(ft_isdigit(*cur) || *cur == '[') {
		...
		if(FT_NEW_ARRAY(encode->char_index, count) || FT_NEW_ARRAY(encode->char_name,  count) || FT_SET_ERROR(psaux->ps_table_funcs->init(char_table, count, memory))) {
...
static void t42_parse_charstrings(T42_Face face, T42_Loader loader)
{
	...
	if(ft_isdigit(*parser->root.cursor)) {
		...
		if(loader->num_glyphs > ( limit - parser->root.cursor ) >> 2) {
			FT_TRACE0(( "t42_parse_charstrings: adjusting number of glyphs (from %d to %ld)\n",
	...
	if(swap_table->init) {
		FT_ERROR(( "t42_parse_charstrings: only one CharStrings array allowed\n" ));
		error = FT_THROW(Invalid_File_Format);
		goto Fail;
	}
...
/*FT_LOCAL_DEF(FT_Error)*/FT_Error t42_parse_dict(T42_Face face, T42_Loader loader, FT_Byte*    base, FT_Long size)
{
	T42_Parser parser  = &loader->parser;
	FT_Byte*    limit;
...
/*FT_LOCAL_DEF(void)*/void t42_loader_init(T42_Loader loader, T42_Face face)
{
	FT_UNUSED(face);
	FT_ZERO(loader);
	loader->num_glyphs = 0;
...
/*FT_LOCAL_DEF(void)*/void t42_loader_done(T42_Loader loader)
{
	T42_Parser parser = &loader->parser;
	/* finalize tables */
