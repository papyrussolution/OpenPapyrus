...
static char* af_print_idx(char* p,
    int idx)
{
	if(idx == -1) {
		p[0] = '-';
...
static int af_get_segment_index(AF_GlyphHints hints,
    int point_idx,
    int dimension)
{
...
static int af_get_edge_index(AF_GlyphHints hints,
    int segment_idx,
    int dimension)
{
	AF_AxisHints axis    = &hints->axis[dimension];
...
static int af_get_strong_edge_index(AF_GlyphHints hints,
...
FT_LOCAL_DEF(void) af_glyph_hints_init(AF_GlyphHints hints, FT_Memory memory)
{
	...
	memzero(hints, sizeof( *hints ) - sizeof( hints->embedded ) );
	hints->memory = memory;
}
