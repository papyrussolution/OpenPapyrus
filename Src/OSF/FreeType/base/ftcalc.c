...
FT_EXPORT_DEF(FT_Fixed) FT_RoundFix(FT_Fixed a) { return ( ADD_LONG(a, 0x8000L - ( a < 0 ) ) ) & ~0xFFFFL; }

/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Fixed) FT_CeilFix(FT_Fixed a) { return ( ADD_LONG(a, 0xFFFFL) ) & ~0xFFFFL; }

/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Fixed) FT_FloorFix(FT_Fixed a) { return a & ~0xFFFFL; }

#ifndef FT_MSB

FT_BASE_DEF(FT_Int) FT_MSB(FT_UInt32 z)
{
	FT_Int shift = 0;
...
FT_BASE_DEF(FT_Fixed) FT_Hypot(FT_Fixed x, FT_Fixed y)
{
	FT_Vector v;
	v.x = x;
	v.y = y;
...
/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Long) FT_MulDiv(FT_Long a_, FT_Long b_, FT_Long c_)
{
	FT_Int s = 1;
	FT_Long d_;
	FT_UInt64 a = (FT_UInt64)a_;
	FT_UInt64 b = (FT_UInt64)b_;
	FT_UInt64 c = (FT_UInt64)c_;
	FT_UInt64 d;
	FT_MOVE_SIGN(a_, a, s);
	FT_MOVE_SIGN(b_, b, s);
	FT_MOVE_SIGN(c_, c, s);
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_Long) FT_MulDiv_No_Round(FT_Long a_, FT_Long b_, FT_Long c_)
{
	FT_Int s = 1;
	FT_Long d_;
	FT_UInt64 a = (FT_UInt64)a_;
...
/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Long) FT_MulFix(FT_Long a_, FT_Long b_)
{
#ifdef FT_MULFIX_ASSEMBLER
...
/* documentation is in freetype.h */
FT_EXPORT_DEF(FT_Long) FT_DivFix(FT_Long a_, FT_Long b_)
{
	FT_Int s = 1;
	FT_UInt64 a, b, q;
	FT_Long q_;
...
{
	FT_UInt32 lo = x->lo + y->lo;
	FT_UInt32 hi = x->hi + y->hi + ( lo < x->lo );
	z->lo = lo;
	z->hi = hi;
}
...
FT_EXPORT_DEF(void) FT_Matrix_Multiply(const FT_Matrix*  a, FT_Matrix        *b)
{
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(void) FT_Matrix_Multiply_Scaled(const FT_Matrix*  a, FT_Matrix        *b, FT_Long scaling)
{
	FT_Fixed xx, xy, yx, yy;
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_Bool) FT_Matrix_Check(const FT_Matrix*  matrix)
{
	FT_Matrix m;
	FT_Fixed val[4];
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(void) FT_Vector_Transform_Scaled(FT_Vector* vector, const FT_Matrix*  matrix, FT_Long scaling)
{
	FT_Pos xz, yz;
	FT_Long val = 0x10000L * scaling;
	if(!vector || !matrix)
		return;
	xz = ADD_LONG(FT_MulDiv(vector->x, matrix->xx, val), FT_MulDiv(vector->y, matrix->xy, val) );
	yz = ADD_LONG(FT_MulDiv(vector->x, matrix->yx, val), FT_MulDiv(vector->y, matrix->yy, val) );
	vector->x = xz;
	vector->y = yz;
}

/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_UInt32) FT_Vector_NormLen(FT_Vector*  vector)
{
	...
	FT_UInt32 x = (FT_UInt32)x_;
	FT_UInt32 y = (FT_UInt32)y_;
...
#if 0
/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_Int32) FT_SqrtFixed(FT_Int32 x)
{
	FT_UInt32 rem_hi, rem_lo, test_div;
	FT_Int count;
	FT_UInt32 root = 0;
	if(x > 0) {
		rem_hi = 0;
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_Int) ft_corner_orientation(FT_Pos in_x, FT_Pos in_y, FT_Pos out_x, FT_Pos out_y)
{
...
/* documentation is in ftcalc.h */
FT_BASE_DEF(FT_Int) ft_corner_is_flat(FT_Pos in_x, FT_Pos in_y, FT_Pos out_x, FT_Pos out_y)
{
