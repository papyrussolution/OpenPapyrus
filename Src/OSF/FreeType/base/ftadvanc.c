...
#define LOAD_ADVANCE_FAST_CHECK(face, flags) (flags & ( FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING) || FT_LOAD_TARGET_MODE(flags) == FT_RENDER_MODE_LIGHT)
...
FT_EXPORT_DEF(FT_Error) FT_Get_Advance(FT_Face face, FT_UInt gindex, FT_Int32 flags, FT_Fixed  *padvance)
{
	...
	if(func && LOAD_ADVANCE_FAST_CHECK(face, flags) ) {
		FT_Error error = func(face, gindex, 1, flags, padvance);
		if(!error)
...
FT_EXPORT_DEF(FT_Error) FT_Get_Advances(FT_Face face, FT_UInt start, FT_UInt count, FT_Int32 flags, FT_Fixed  *padvances)
{
	...
	for(nn = 0; nn < count; nn++) {
		...
		padvances[nn] = ( flags & FT_LOAD_VERTICAL_LAYOUT ) ? face->glyph->advance.y * factor : face->glyph->advance.x * factor;
	}
	return error;
}
