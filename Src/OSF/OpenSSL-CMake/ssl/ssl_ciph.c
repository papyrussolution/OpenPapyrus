...
static STACK_OF(SSL_COMP) *ssl_comp_methods = NULL; // @global
#ifndef OPENSSL_NO_COMP
	static CRYPTO_ONCE ssl_load_builtin_comp_once = CRYPTO_ONCE_STATIC_INIT; // @global
#endif
/*
...
static int ssl_cipher_info_find(const ssl_cipher_table * table, size_t table_cnt, uint32_t mask)
{
	for(size_t i = 0; i < table_cnt; i++, table++) {
		if(table->mask == mask)
			return i;
	}
	return -1;
...
static int ssl_mac_secret_size[SSL_MD_NUM_IDX]; // @global
...
{
	int pkey_id = 0;
	const EVP_PKEY_ASN1_METHOD * ameth = EVP_PKEY_asn1_find_str(NULL, pkey_name, -1);
...
static int get_optional_pkey_id(const char * pkey_name)
{
	ENGINE * tmpeng = NULL;
	int pkey_id = 0;
	const EVP_PKEY_ASN1_METHOD * ameth = EVP_PKEY_asn1_find_str(&tmpeng, pkey_name, -1);
	if(ameth) {
		if(EVP_PKEY_asn1_get0_info(&pkey_id, NULL, NULL, NULL, NULL, ameth) <= 0)
			pkey_id = 0;
	}
...
//
static uint32_t disabled_enc_mask; // @global
static uint32_t disabled_mac_mask; // @global
static uint32_t disabled_mkey_mask; // @global
static uint32_t disabled_auth_mask; // @global
...
int ssl_cipher_get_evp(const SSL_SESSION * s, const EVP_CIPHER ** enc, const EVP_MD ** md, int * mac_pkey_type, int * mac_secret_size, SSL_COMP ** comp, int use_etm)
{
	int i;
	...
	if(i == -1)
		...
		*enc = (i == SSL_ENC_NULL_IDX) ? EVP_enc_null() : ssl_cipher_methods[i];
	}
	i = ssl_cipher_info_lookup(ssl_cipher_table_mac, c->algorithm_mac);
	if(i == -1) {
		*md = NULL;
		ASSIGN_PTR(mac_pkey_type, NID_undef);
		ASSIGN_PTR(mac_secret_size, 0);
		if(c->algorithm_mac == SSL_AEAD)
			mac_pkey_type = NULL;
	}
	else {
		*md = ssl_digest_methods[i];
		ASSIGN_PTR(mac_pkey_type, ssl_mac_pkey_id[i]);
		ASSIGN_PTR(mac_secret_size, ssl_mac_secret_size[i]);
	}
	if((*enc != NULL) && (*md != NULL || (EVP_CIPHER_flags(*enc) & EVP_CIPH_FLAG_AEAD_CIPHER)) && (!mac_pkey_type || *mac_pkey_type != NID_undef)) {
		...
		if(FIPS_mode())
			return 1;
		if(c->algorithm_enc == SSL_RC4 && c->algorithm_mac == SSL_MD5 && (evp = EVP_get_cipherbyname("RC4-HMAC-MD5")))
			*enc = evp, *md = NULL;
		else if(c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA1 && (evp = EVP_get_cipherbyname("AES-128-CBC-HMAC-SHA1")))
			*enc = evp, *md = NULL;
		else if(c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA1 && (evp = EVP_get_cipherbyname("AES-256-CBC-HMAC-SHA1")))
			*enc = evp, *md = NULL;
		else if(c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA256 && (evp = EVP_get_cipherbyname("AES-128-CBC-HMAC-SHA256")))
			*enc = evp, *md = NULL;
	...
	for(;; ) {
		...
		if(strength_bits >= 0) {
			...
			if((algo_strength & SSL_STRONG_MASK) && !(algo_strength & SSL_STRONG_MASK & cp->algo_strength))
				continue;
			if((algo_strength & SSL_DEFAULT_MASK) && !(algo_strength & SSL_DEFAULT_MASK & cp->algo_strength))
				continue;
		}
...
static int ssl_cipher_process_rulestr(const char * rule_str, CIPHER_ORDER ** head_p, CIPHER_ORDER ** tail_p, const SSL_CIPHER ** ca_list, CERT * c)
{
	...
	int min_tls;
	const char * buf;
	int j, multi, found, rule, ok, buflen;
	uint32_t cipher_id = 0;
	char ch;
	int retval = 1;
	const char * l = rule_str;
	for(;; ) {
		ch = *l;
		if(ch == '\0')
			break;  /* done */
		...
		if(rule == CIPHER_SPECIAL) { /* special command */
			...
			ssl_cipher_apply_rule(cipher_id, alg_mkey, alg_auth, alg_enc, alg_mac, min_tls, algo_strength, rule, -1, head_p, tail_p);
		}
		else {
			while((*l != '\0') && !ITEM_SEP(*l))
				l++;
...
# ifndef OPENSSL_NO_EC
	switch(suiteb_flags) {
		case SSL_CERT_FLAG_SUITEB_128_LOS:
		    *prule_str = suiteb_comb2 ? "ECDHE-ECDSA-AES256-GCM-SHA384" : "ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-ECDSA-AES256-GCM-SHA384";
		    break;
...
int SSL_CIPHER_get_bits(const SSL_CIPHER * c, int * alg_bits)
{
	int ret = 0;
	if(c != NULL) {
		ASSIGN_PTR(alg_bits, (int)c->alg_bits);
...
{
	int i = c ? ssl_cipher_info_lookup(ssl_cipher_table_cipher, c->algorithm_enc) : -1;
	return (i == -1) ? NID_undef : ssl_cipher_table_cipher[i].nid;
}

int SSL_CIPHER_get_digest_nid(const SSL_CIPHER * c)
{
	int i = ssl_cipher_info_lookup(ssl_cipher_table_mac, c->algorithm_mac);
	return (i == -1) ? NID_undef : ssl_cipher_table_mac[i].nid;
}

int SSL_CIPHER_get_kx_nid(const SSL_CIPHER * c)
{
	int i = ssl_cipher_info_lookup(ssl_cipher_table_kx, c->algorithm_mkey);
	return (i == -1) ? NID_undef : ssl_cipher_table_kx[i].nid;
}

int SSL_CIPHER_get_auth_nid(const SSL_CIPHER * c)
{
	int i = ssl_cipher_info_lookup(ssl_cipher_table_auth, c->algorithm_auth);
	return (i == -1) ? NID_undef : ssl_cipher_table_auth[i].nid;
}
