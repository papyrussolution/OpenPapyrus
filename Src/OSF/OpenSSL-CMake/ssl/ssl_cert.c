...
int ssl_verify_cert_chain(SSL * s, STACK_OF(X509) * sk)
{
	 ...
	 * the context: if its a server it will verify SSL client certificates or vice versa.
	 */
...
int ssl_add_cert_chain(SSL * s, CERT_PKEY * cpk, unsigned long * l)
{
	 ...
	 */
	extra_certs = cpk->chain ? cpk->chain : s->ctx->extra_certs;
    ...
    int op, int bits, int nid, void * other, void * ex)
{
	...
	switch(op) {
	    ...
	    {
		    const SSL_CIPHER * c = (const SSL_CIPHER*)other;
