...
const SSL_CIPHER * ssl3_choose_cipher(SSL * s, STACK_OF(SSL_CIPHER) * clnt, STACK_OF(SSL_CIPHER) * srvr)
{
	...
	for(i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
		...
		if(ii >= 0) {
			...
			if(!ssl_security(s, SSL_SECOP_CIPHER_SHARED, c->strength_bits, 0, (void*)c))
				continue;
#if !defined(OPENSSL_NO_EC)
			if((alg_k & SSL_kECDHE) && (alg_a & SSL_aECDSA) && s->s3->is_probably_safari) {
				SETIFZ(ret, sk_SSL_CIPHER_value(allow, ii));
				continue;
			}
#endif
    ...
    int free_pms)
{
	...
	if(alg_k & SSL_PSK) {
		...
		s->s3->tmp.psk = NULL;
		s->session->master_key_length = s->method->ssl3_enc->generate_master_secret(s, s->session->master_key, pskpms, pskpmslen);
