...
static int capi_ctrl(ENGINE * e, int cmd, long i, void * p, void (* f)(void))
{
	...
	switch(cmd) {
		...
		case CAPI_CMD_SET_CSP_NAME:
		    ret = capi_ctx_set_provname(ctx, (LPSTR)p, ctx->csptype, 1);
		    break;
    ...
    UI_METHOD * ui_method, void * callback_data)
{
	...
	CAPI_KEY * key;
	EVP_PKEY * ret;
	ctx = (CAPI_CTX*)ENGINE_get_ex_data(eng, capi_idx);
    ...
    uchar * sigret, uint * siglen, const RSA * rsa)
{
	...
	ctx = (CAPI_CTX*)ENGINE_get_ex_data(RSA_get0_engine(rsa), capi_idx);

	CAPI_trace(ctx, "Called CAPI_rsa_sign()\n");

	capi_key = (CAPI_KEY*)RSA_get_ex_data(rsa, rsa_capi_idx);
	if(!capi_key) {
    ...
    DSA * dsa)
{
	...
	ctx = (CAPI_CTX*)ENGINE_get_ex_data(DSA_get0_engine(dsa), capi_idx);

	CAPI_trace(ctx, "Called CAPI_dsa_do_sign()\n");

	capi_key = (CAPI_KEY*)DSA_get_ex_data(dsa, dsa_capi_idx);
...
static int capi_list_containers(CAPI_CTX * ctx, BIO * out)
{
	...
	if(ctx->cspname && sizeof(TCHAR) != sizeof(char)) {
		if((clen =
			...
			cspname = (LPTSTR)alloca(clen * sizeof(WCHAR));
	...
	if(buflen == 0)
		buflen = 1024;
	cname = (LPSTR)OPENSSL_malloc(buflen);
	if(cname == NULL) {
		CAPIerr(CAPI_F_CAPI_LIST_CONTAINERS, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
static CRYPT_KEY_PROV_INFO * capi_get_prov_info(CAPI_CTX * ctx, PCCERT_CONTEXT cert)
{
	...
	if(!CertGetCertificateContextProperty
		...
		return NULL;
	pinfo = (CRYPT_KEY_PROV_INFO*)OPENSSL_malloc(len);
	if(pinfo == NULL) {
...
static char * capi_cert_get_fname(CAPI_CTX * ctx, PCCERT_CONTEXT cert)
{
	...
	if(!CertGetCertificateContextProperty
		...
		return NULL;
	wfname = (LPWSTR)OPENSSL_malloc(dlen);
	if(wfname == NULL)
		return NULL;
	if(CertGetCertificateContextProperty
...
int capi_list_certs(CAPI_CTX * ctx, BIO * out, char * id)
{
	...
	const char * storename = NZOR(ctx->storename, "MY");
...
{
	DWORD dwFlags = 0;
	CAPI_KEY * key = (CAPI_KEY*)OPENSSL_malloc(sizeof(*key));
...
static CAPI_KEY * capi_get_cert_key(CAPI_CTX * ctx, PCCERT_CONTEXT cert)
{
	...
	char * provname = NULL;
	char * contname = NULL;
	CRYPT_KEY_PROV_INFO * pinfo = capi_get_prov_info(ctx, cert);
	if(pinfo) {
		if(sizeof(TCHAR) != sizeof(char))
			key = capi_get_key(ctx, (TCHAR*)pinfo->pwszContainerName, (TCHAR*)pinfo->pwszProvName, pinfo->dwProvType, pinfo->dwKeySpec);
		else {
			provname = wide_to_asc(pinfo->pwszProvName);
			contname = wide_to_asc(pinfo->pwszContainerName);
			if(!provname || !contname)
				goto err;
			key = capi_get_key(ctx, (TCHAR*)contname, (TCHAR*)provname, pinfo->dwProvType, pinfo->dwKeySpec);
		}
	}
err:
	OPENSSL_free(pinfo);
	...
	switch(ctx->lookup_method) {
		...
		case CAPI_LU_CONTNAME:
		    if(sizeof(TCHAR) != sizeof(char)) {
			    ...
			    key = capi_get_key(ctx, (TCHAR*)id, (TCHAR*)ctx->cspname, ctx->csptype, ctx->keytype);
		    break;
	}
	return key;
}
	...
	if(check) {
		...
		if(sizeof(TCHAR) != sizeof(char)) {
			...
			if((len = MultiByteToWideChar(CP_ACP, 0, pname, -1, NULL, 0))) {
				name = (LPTSTR)alloca(len * sizeof(WCHAR));
				MultiByteToWideChar(CP_ACP, 0, pname, -1, (WCHAR*)name, len);
			}
		}
		else
			name = (TCHAR*)pname;
		if(!name || !CryptAcquireContext(&hprov, NULL, name, type, CRYPT_VERIFYCONTEXT)) {
    ...
    EVP_PKEY ** pkey, STACK_OF(X509) ** pother, UI_METHOD * ui_method, void * callback_data)
{
	...
	CAPI_KEY * key;
	CAPI_CTX * ctx = (CAPI_CTX*)ENGINE_get_ex_data(e, capi_idx);
	const char * storename = NZOR(ctx->ssl_client_store, "MY");
	HCERTSTORE hstore = capi_open_store(ctx, storename);
	*pcert = NULL;
	*pkey = NULL;
	if(!hstore)
		return 0;
	...
	for(i = 0;; i++) {
		...
		if(cert_issuer_match(ca_dn, x) && X509_check_purpose(x, X509_PURPOSE_SSL_CLIENT, 0)) {
			key = capi_get_cert_key(ctx, cert);
			if(!key) {
				X509_free(x);
				continue;
	...
	for(i = 0; i < sk_X509_num(certs); i++) {
		...
		if(i == client_cert_idx)
			*pcert = x;
			...
			key = (CAPI_KEY*)X509_get_ex_data(x, cert_capi_idx);
			capi_free_key(key);
			X509_free(x);
		}
	}
