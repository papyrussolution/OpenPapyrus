...
#  ifndef OPENSSL_NO_RSA
	hptr = (HWCryptoHook_RSAKeyHandle*)OPENSSL_malloc(sizeof(*hptr));
	if(hptr == NULL) {
...
static int hwcrhk_rsa_finish(RSA * rsa)
{
	HWCryptoHook_RSAKeyHandle * hptr = (HWCryptoHook_RSAKeyHandle*)RSA_get_ex_data(rsa, hndidx_rsa);
	if(hptr) {
		p_hwcrhk_RSAUnloadKey(*hptr, 0);
		OPENSSL_free(hptr);
		RSA_set_ex_data(rsa, hndidx_rsa, 0);
	}
	return 1;
}
...
static int hwcrhk_insert_card(const char * prompt_info, const char * wrong_info, HWCryptoHook_PassphraseContext * ppctx, HWCryptoHook_CallerContext * cactx)
{
	...
	if(ui) {
		...
		if(wrong_info && *wrong_info)
			BIO_snprintf(buf, sizeof(buf) - 1, "Current card: \"%s\"\n", wrong_info);
		else
			buf[0] = 0;
		ok = UI_dup_info_string(ui, buf);
		if(ok >= 0 && prompt_info) {
			BIO_snprintf(buf, sizeof(buf) - 1, "Insert card \"%s\"", prompt_info);
...
static void hwcrhk_log_message(void * logstr, const char * message)
{
	BIO * lstream = logstr ? *(BIO**)logstr : 0;
	if(lstream)
...
{
	if(id && (strcmp(id, engine_hwcrhk_id) != 0) && (strcmp(id, engine_hwcrhk_id_alt) != 0))
		return 0;
	if(!bind_helper(e))
		return 0;
	return 1;
