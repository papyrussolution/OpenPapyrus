...
static CONF_MODULE * module_add(DSO * dso, const char * name, conf_init_func * ifunc, conf_finish_func * ffunc)
{
	...
	if(supported_modules) {
		...
		if(tmod) {
			...
			else if(!sk_CONF_MODULE_push(supported_modules, tmod)) {
...
static CONF_MODULE * module_find(const char * name)
{
	...
	for(int i = 0; i < sk_CONF_MODULE_num(supported_modules); i++) {
		CONF_MODULE * tmod = sk_CONF_MODULE_value(supported_modules, i);
...
{
	return module_add(NULL, name, ifunc, ffunc) ? 1 : 0;
}
...
int CONF_parse_list(const char * list_, int sep, int nospc, int (* list_cb)(const char * elem, int len, void * usr), void * arg)
{
	...
	if(list_ == NULL) {
		...
		const char * lstart = list_;
		for(;; ) {
			if(nospc) {
				while(*lstart && isspace((uchar)*lstart))
					lstart++;
			}
			const char * p = strchr(lstart, sep);
			if(p == lstart || !*lstart)
				ret = list_cb(NULL, 0, arg);
			else {
				const char * tmpend = p ? (p - 1) : (lstart + strlen(lstart) - 1);
				if(nospc) {
					while(isspace((uchar)*tmpend))
						tmpend--;
				}
