...
{
	X509_STORE * ret;
	if((ret = (X509_STORE*)OPENSSL_zalloc(sizeof(*ret))) == NULL)
		return NULL;
...
static int x509_object_idx_cnt(STACK_OF(X509_OBJECT) * h, X509_LOOKUP_TYPE type, X509_NAME * name, int * pnmatch)
{
	X509_OBJECT stmp;
	X509 x509_s;
	X509_CRL crl_s;
