...
int PKCS7_set_type(PKCS7 * p7, int type)
{
	 ...
	 */
	ASN1_OBJECT * obj = OBJ_nid2obj(type); /* will not fail */
	switch(type) {
		case NID_pkcs7_signed:
		    p7->type = obj;
...
int PKCS7_add_signer(PKCS7 * p7, PKCS7_SIGNER_INFO * psi)
{
	...
	if(!j) {                /* we need to add another algorithm */
		if((alg = X509_ALGOR_new()) == NULL || (alg->parameter = ASN1_TYPE_new()) == NULL) {
			X509_ALGOR_free(alg);
...
int PKCS7_SIGNER_INFO_set(PKCS7_SIGNER_INFO * p7i, X509 * x509, EVP_PKEY * pkey, const EVP_MD * dgst)
{
	...
	if(!(p7i->issuer_and_serial->serial = ASN1_INTEGER_dup(X509_get_serialNumber(x509))))
		goto err;
	/* lets keep the pkey around for a while */
	EVP_PKEY_up_ref(pkey);
	p7i->pkey = pkey;
	/* Set the algorithms */
	X509_ALGOR_set0(p7i->digest_alg, OBJ_nid2obj(EVP_MD_type(dgst)), V_ASN1_NULL, 0);
...
{
	ASSIGN_PTR(pk, si->pkey);
	ASSIGN_PTR(pdig, si->digest_alg);
	ASSIGN_PTR(psig, si->digest_enc_alg);
}

void PKCS7_RECIP_INFO_get0_alg(PKCS7_RECIP_INFO * ri, X509_ALGOR ** penc)
{
	ASSIGN_PTR(penc, ri->key_enc_algor);
}
