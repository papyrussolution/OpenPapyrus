    ...
    const EVP_MD * md, const EVP_MD * mgf1md)
{
	int i, emlen = tlen - 1;
	uchar * db, * seed;
	uchar * dbmask, seedmask[EVP_MAX_MD_SIZE];
	int mdlen;
	...
	dbmask = (uchar*)OPENSSL_malloc(emlen - mdlen);
	if(dbmask == NULL) {
    ...
    const EVP_MD * mgf1md)
{
	...
	dblen = num - mdlen - 1;
	db = (uchar*)OPENSSL_malloc(dblen);
	em = (uchar*)OPENSSL_malloc(num);
	if(db == NULL || em == NULL) {
    ...
    const uchar * seed, long seedlen, const EVP_MD * dgst)
{
	...
	for(i = 0; outlen < len; i++) {
		cnt[0] = (uchar)((i >> 24) & 255);
		cnt[1] = (uchar)((i >> 16) & 255);
		cnt[2] = (uchar)((i >> 8)) & 255;
		cnt[3] = (uchar)(i & 255);
