...
struct sct_st {
    ...
    unsigned char *sct; /* If version is not SCT_VERSION_V1, this contains the encoded SCT */
    size_t sct_len;
    unsigned char *log_id; /* If version is SCT_VERSION_V1, fields below contain components of the SCT */
    size_t log_id_len;
    /*
    ...
    */
    ...
    unsigned char *sig;
    size_t sig_len;
    ct_log_entry_type_t entry_type; /* Log entry type */
    sct_source_t source; /* Where this SCT was found, e.g. certificate, OCSP response, etc. */
    sct_validation_status_t validation_status; /* The result of the last attempt to validate this SCT. */
};

/* Miscellaneous data that is useful when verifying an SCT  */
struct sct_ctx_st {
    EVP_PKEY *pkey; /* Public key */
    unsigned char *pkeyhash; /* Hash of public key */
    size_t pkeyhashlen;
    unsigned char *ihash; /* For pre-certificate: issuer public key hash */
    size_t ihashlen;
    unsigned char *certder; /* certificate encoding */
    size_t certderlen;
    unsigned char *preder; /* pre-certificate encoding */
    size_t prederlen;
    uint64_t epoch_time_in_ms; /* milliseconds since epoch (to check that the SCT isn't from the future) */
};
...
struct ct_policy_eval_ctx_st {
    ...
    uint64_t epoch_time_in_ms; /* milliseconds since epoch (to check that SCTs aren't from the future) */
};
