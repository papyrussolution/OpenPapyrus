...
char * i2s_ASN1_ENUMERATED(X509V3_EXT_METHOD * method, const ASN1_ENUMERATED * a)
{
	...
	if(!a)
		return NULL;
	if((bntmp = ASN1_ENUMERATED_to_BN(a, NULL)) == NULL || (strtmp = BN_bn2dec(bntmp)) == NULL)
...
char * i2s_ASN1_INTEGER(X509V3_EXT_METHOD * method, const ASN1_INTEGER * a)
{
	...
	if(!a)
		return NULL;
	if((bntmp = ASN1_INTEGER_to_BN(a, NULL)) == NULL || (strtmp = BN_bn2dec(bntmp)) == NULL)
...
static int do_check_string(const ASN1_STRING * a, int cmp_type, equal_fn equal,
    uint flags, const char * b, size_t blen,
    char ** peername)
{
	int rv = 0;
...
static int do_x509_check(X509 * x, const char * chk, size_t chklen,
    uint flags, int check_type, char ** peername)
{
