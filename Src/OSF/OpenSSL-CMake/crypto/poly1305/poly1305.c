 ...
 */
static void
poly1305_blocks(void *ctx, const uchar *inp, size_t len, u32 padbit);
...
static void hex_decode(uchar *out, const char *hex)
{
    size_t j = 0;

    while (*hex != 0) {
        uchar v = hex_digit(*hex++);
        v <<= 4;
        v |= hex_digit(*hex++);
        out[j++] = v;
    }
...
static void hexdump(uchar *a, size_t len)
{
    size_t i;
...
int main()
{
    ...
    unsigned i;
    uchar key[32], out[16], expected[16];
    POLY1305 poly1305;
...
# ifdef OPENSSL_CPUID_OBJ
    {
        uchar buf[8192];
        ulong long stopwatch;
        ulong long OPENSSL_rdtsc();
