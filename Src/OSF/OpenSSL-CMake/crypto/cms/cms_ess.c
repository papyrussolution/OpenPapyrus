...
int CMS_get1_ReceiptRequest(CMS_SignerInfo * si, CMS_ReceiptRequest ** prr)
{
	...
	if(prr)
		*prr = NULL;
	str = (ASN1_STRING*)CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_id_smime_aa_receiptRequest), -3, V_ASN1_SEQUENCE);
	if(!str)
		return 0;
...
void CMS_ReceiptRequest_get0_values(CMS_ReceiptRequest * rr, ASN1_STRING ** pcid,
    int * pallorfirst, STACK_OF(GENERAL_NAMES) ** plist, STACK_OF(GENERAL_NAMES) ** prto)
{
...
{
	uchar dig[EVP_MAX_MD_SIZE];
	uint diglen;
	if(!cms_msgSigDigest(src, dig, &diglen)) {
		CMSerr(CMS_F_CMS_MSGSIGDIGEST_ADD1, CMS_R_MSGSIGDIGEST_ERROR);
		return 0;
	}
	if(!CMS_signed_add1_attr_by_NID(dest, NID_id_smime_aa_msgSigDigest, V_ASN1_OCTET_STRING, dig, diglen)) {
...
int cms_Receipt_verify(CMS_ContentInfo * cms, CMS_ContentInfo * req_cms)
{
	...
	STACK_OF(CMS_SignerInfo) * osis = CMS_get0_SignerInfos(req_cms);
	...
	msig = (ASN1_OCTET_STRING*)CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_id_smime_aa_msgSigDigest), -3, V_ASN1_OCTET_STRING);
	if(!msig) {
	...
	if(diglen != (uint)msig->length) {
		CMSerr(CMS_F_CMS_RECEIPT_VERIFY, CMS_R_MSGSIGDIGEST_WRONG_LENGTH);
		goto err;
	}
	if(memcmp(dig, msig->data, diglen)) {
		CMSerr(CMS_F_CMS_RECEIPT_VERIFY, CMS_R_MSGSIGDIGEST_VERIFICATION_FAILURE);
		goto err;
	}
	/* Compare content types */
	octype = (ASN1_OBJECT*)CMS_signed_get0_data_by_OBJ(osi, OBJ_nid2obj(NID_pkcs9_contentType), -3, V_ASN1_OBJECT);
	if(!octype) {
	...
	if(ASN1_STRING_cmp(rr->signedContentIdentifier, rct->signedContentIdentifier)) {
...
ASN1_OCTET_STRING * cms_encode_Receipt(CMS_SignerInfo * si)
{
	...
	ctype = (ASN1_OBJECT*)CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_contentType), -3, V_ASN1_OBJECT);
	if(!ctype) {
