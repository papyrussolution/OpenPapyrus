 ...
 */
//#include "internal/cryptlib.h" // нельзя до "bio_lcl.h"
...
static const char * ossl_hstrerror(int herr)
{
	switch(herr) {
		...
		case NO_DATA: return "No data"; // NO_ADDRESS is a synonym 
...
int BIO_ADDR_rawmake(BIO_ADDR * ap, int family, const void * where, size_t wherelen, unsigned short port)
{
	...
	if(family == AF_UNIX) {
		if(wherelen + 1 > sizeof(ap->s_un.sun_path))
			return 0;
		memzero(&ap->s_un, sizeof(ap->s_un));
	...
	if(family == AF_INET) {
		if(wherelen != sizeof(struct in_addr))
			return 0;
		memzero(&ap->s_in, sizeof(ap->s_in));
	...
	if(family == AF_INET6) {
		if(wherelen != sizeof(struct in6_addr))
			return 0;
		memzero(&ap->s_in6, sizeof(ap->s_in6));
...
int BIO_ADDRINFO_protocol(const BIO_ADDRINFO * bai)
{
	if(bai) {
		...
		switch(bai->bai_socktype) {
			case SOCK_STREAM: return IPPROTO_TCP;
			case SOCK_DGRAM: return IPPROTO_UDP;
			default:
			    break;
		}
	}
...
# pragma pointer_size restore
			...
			if(endp != service && *endp == '\0'
				...
				se_fallback.s_port = htons((u_short)portnum);
