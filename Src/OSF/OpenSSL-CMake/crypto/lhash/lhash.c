...
{
	doall_util_fn(lh, 0, func, (OPENSSL_LH_DOALL_FUNCARG)0, 0);
}
...
static int expand(OPENSSL_LHASH * lh)
{
	...
	*n2 = NULL;
	...
	if((lh->p) >= lh->pmax) {
		...
		n = (OPENSSL_LH_NODE**)OPENSSL_realloc(lh->b, (int)(sizeof(OPENSSL_LH_NODE *) * j));
		if(n == NULL) {
			lh->error++;
			lh->num_nodes--;
			lh->p = 0;
...
static void contract(OPENSSL_LHASH * lh)
{
	...
	if(lh->p == 0) {
		n = (OPENSSL_LH_NODE**)OPENSSL_realloc(lh->b,
		    (uint)(sizeof(OPENSSL_LH_NODE *) * lh->pmax));
		if(n == NULL) {
