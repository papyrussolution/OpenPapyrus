...
int CRYPTO_ocb128_init(OCB128_CONTEXT * ctx, void * keyenc, void * keydec, block128_f encrypt, block128_f decrypt, ocb128_f stream)
{
	...
	ctx->l_index = 0;
	ctx->max_l_index = 5;
	ctx->l = (OCB_BLOCK *)OPENSSL_malloc(ctx->max_l_index * 16);
	if(ctx->l == NULL)
		return 0;
    ...
    void * keyenc, void * keydec)
{
	...
	if(keydec)
		dest->keydec = keydec;
	if(src->l) {
		dest->l = (OCB_BLOCK *)OPENSSL_malloc(src->max_l_index * 16);
		if(dest->l == NULL)
			return 0;
    ...
    size_t len)
{
	...
	/*
	 * Check if we have any partial blocks left over. This is only valid in the last call to this function
	 */
	last_len = len % 16;
	if(last_len > 0) {
    ...
    size_t len)
{
	...
	if(last_len > 0) {
		...
		ocb_block_xor(in + (len / 16) * 16, (unsigned char*)&pad, last_len, out + (num_blocks * 16));
