...
int EC_GROUP_cmp(const EC_GROUP * a, const EC_GROUP * b, BN_CTX * ctx)
{
	...
	if(EC_METHOD_get_field_type(EC_GROUP_method_of(a)) != EC_METHOD_get_field_type(EC_GROUP_method_of(b)))
		return 1;
	...
	if(r || EC_POINT_cmp(a, EC_GROUP_get0_generator(a), EC_GROUP_get0_generator(b), ctx))
		r = 1;
	if(!r) {
		// compare the order and cofactor 
		const BIGNUM * ao = EC_GROUP_get0_order(a);
		const BIGNUM * bo = EC_GROUP_get0_order(b);
		const BIGNUM * ac = EC_GROUP_get0_cofactor(a);
		const BIGNUM * bc = EC_GROUP_get0_cofactor(b);
		if(ao == NULL || bo == NULL) {
			BN_CTX_end(ctx);
			BN_CTX_free(ctx_new);
			return -1;
...
int EC_POINT_set_Jprojective_coordinates_GFp(const EC_GROUP * group, EC_POINT * point, const BIGNUM * x,
    const BIGNUM * y, const BIGNUM * z, BN_CTX * ctx)
{
...
int EC_POINT_get_Jprojective_coordinates_GFp(const EC_GROUP * group, const EC_POINT * point, BIGNUM * x,
...
int EC_POINT_cmp(const EC_GROUP * group, const EC_POINT * a, const EC_POINT * b, BN_CTX * ctx)
{
	...
	else if((group->meth != a->meth) || (a->meth != b->meth)) {
...
{
	return group->order ? BN_num_bits(group->order) : 0;
}
