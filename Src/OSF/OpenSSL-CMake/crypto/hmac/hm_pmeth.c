...
{
	HMAC_PKEY_CTX * hctx;
	hctx = (HMAC_PKEY_CTX*)OPENSSL_zalloc(sizeof(*hctx));
	if(hctx == NULL)
		return 0;
...
static int pkey_hmac_copy(EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
	...
	if(!pkey_hmac_init(dst))
		return 0;
	sctx = (HMAC_PKEY_CTX*)EVP_PKEY_CTX_get_data(src);
	dctx = (HMAC_PKEY_CTX*)EVP_PKEY_CTX_get_data(dst);
	dctx->md = sctx->md;
...
static void pkey_hmac_cleanup(EVP_PKEY_CTX * ctx)
{
	HMAC_PKEY_CTX * hctx = (HMAC_PKEY_CTX*)EVP_PKEY_CTX_get_data(ctx);
...
static int hmac_signctx(EVP_PKEY_CTX * ctx, uchar * sig, size_t * siglen,
    EVP_MD_CTX * mctx)
{
	uint hlen;
	HMAC_PKEY_CTX * hctx = (HMAC_PKEY_CTX*)ctx->data;
