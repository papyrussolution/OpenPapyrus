...
{
	struct thread_local_inits_st * local = (struct thread_local_inits_st *)CRYPTO_THREAD_get_local(&threadstopkey);
	if(local == NULL && alloc) {
		local = (struct thread_local_inits_st *)OPENSSL_zalloc(sizeof *local);
...
static void ossl_init_thread_stop(struct thread_local_inits_st * locals)
{
	...
	if(locals->async) {
#ifdef OPENSSL_INIT_DEBUG
		fprintf(stderr, "OPENSSL_INIT: ossl_init_thread_stop: ASYNC_cleanup_thread()\n");
#endif
...
# else
		 ...
		 */
		{
			DSO * dso = DSO_dsobyaddr(handlersym.sym, DSO_FLAG_NO_UNLOAD_ON_FREE);
			DSO_free(dso);
		}
# endif
	}
#endif
	newhand = (OPENSSL_INIT_STOP *)OPENSSL_malloc(sizeof(*newhand));
	if(newhand == NULL)
		return 0;
