...
static int dsa_sign_setup(DSA * dsa, BN_CTX * ctx_in, BIGNUM ** kinvp, BIGNUM ** rp, const uchar * dgst, int dlen);
static int dsa_do_verify(const uchar * dgst, int dgst_len, DSA_SIG * sig, DSA * dsa);
...
static int dsa_sign_setup(DSA * dsa, BN_CTX * ctx_in, BIGNUM ** kinvp, BIGNUM ** rp, const uchar * dgst, int dlen)
{
	...
	if(dsa->flags & DSA_FLAG_CACHE_MONT_P) {
		if(!BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->p, ctx))
			goto err;
	}
...
static int dsa_do_verify(const uchar * dgst, int dgst_len, DSA_SIG * sig, DSA * dsa)
{
	...
	if(!BN_mod_mul(u2, r, u2, dsa->q, ctx))
		goto err;
	if(dsa->flags & DSA_FLAG_CACHE_MONT_P) {
		mont = BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->p, ctx);
		if(!mont)
			goto err;
	}
	if(dsa->meth->dsa_mod_exp != NULL) {
		if(!dsa->meth->dsa_mod_exp(dsa, t1, dsa->g, u1, dsa->pub_key, u2, dsa->p, ctx, mont))
			goto err;
	}
	else {
