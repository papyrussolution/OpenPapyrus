    ...
    const char * kdfname, ssh_string kdfoptions, ssh_auth_callback auth_fn, void * auth_data)
{
	...
	if(ssh_string_len(blob) % cipher.blocksize != 0) {
		SSH_LOG(SSH_LOG_WARN, "Encrypted string not multiple of blocksize: %zu", ssh_string_len(blob));
		return SSH_ERROR;
	}
	...
	memzero(passphrase_buffer, sizeof(passphrase_buffer));
	cipher.set_decrypt_key(&cipher, key_material, key_material + cipher.keysize/8);
	cipher.decrypt(&cipher, ssh_string_data(blob), ssh_string_data(blob), ssh_string_len(blob));
 ...
 */
static ssh_key ssh_pki_openssh_import(const char * text_key, const char * passphrase, ssh_auth_callback auth_fn, void * auth_data, bool isPrivate)
{
	...
	ssh_key key = NULL;
	uint8 padding;
	int cmp = strncmp(ptr, OPENSSH_HEADER_BEGIN, strlen(OPENSSH_HEADER_BEGIN));
	if(cmp != 0) {
	...
	base64 = (char *)SAlloc::M(end - ptr + 1);
	if(base64 == NULL) {
		goto out;
	}
	for(i = 0; ptr < end; ptr++) {
	...
	rc = ssh_buffer_unpack(buffer, "PssSdSS", strlen(OPENSSH_AUTH_MAGIC) + 1, &magic, &ciphername, &kdfname,
		&kdfoptions, &nkeys, &pubkey0, &privkeys);
	if(rc == SSH_ERROR) {
	...
	SSH_LOG(SSH_LOG_INFO, "Opening OpenSSH private key: ciphername: %s, kdf: %s, nkeys: %d", ciphername, kdfname, nkeys);
	if(nkeys != 1) {
	...
	rc = pki_private_key_decrypt(privkeys, passphrase, ciphername, kdfname, kdfoptions, auth_fn, auth_data);
	if(rc == SSH_ERROR) {
		goto out;
	}
    ...
    ssh_string salt)
{
	...
	rc = bcrypt_pbkdf(passphrase, strlen(passphrase), (const uint8 *)ssh_string_data(salt), ssh_string_len(salt), key_material, key_material_len, rounds);
	if(rc < 0) {
		return SSH_ERROR;
	}
	cipher.set_encrypt_key(&cipher, key_material, key_material + cipher.keysize/8);
	cipher.encrypt(&cipher, ssh_buffer_get(privkey_buffer), ssh_buffer_get(privkey_buffer), ssh_buffer_get_len(privkey_buffer));
	ssh_cipher_clear(&cipher);
	memzero(passphrase_buffer, sizeof(passphrase_buffer));
	return SSH_OK;
}
...
ssh_string ssh_pki_openssh_privkey_export(const ssh_key privkey, const char * passphrase, ssh_auth_callback auth_fn, void * auth_data)
{
	...
	b64 = bin_to_base64((const uint8 *)ssh_buffer_get(buffer), ssh_buffer_get_len(buffer));
	if(b64 == NULL) {
		goto error;
	}
