...
static SSH_MUTEX ssh_init_mutex = SSH_MUTEX_STATIC_INIT; /* Declare static mutex */
static int _ssh_initialized = 0; /* Counter for initializations */
static int _ssh_init_ret = 0; /* Cache the returned value */

void libssh_constructor() CONSTRUCTOR_ATTRIBUTE;
void libssh_destructor() DESTRUCTOR_ATTRIBUTE;

static int _ssh_init(uint constructor) 
{
	int rc = 0;
	if(!constructor) {
...
static int _ssh_finalize(uint destructor) 
{
	if(!destructor) {
		ssh_mutex_lock(&ssh_init_mutex);
		if(_ssh_initialized > 1) {
