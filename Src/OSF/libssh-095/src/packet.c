...
struct ssh_crypto_struct * ssh_packet_get_current_crypto(ssh_session session, enum ssh_crypto_direction_e direction)
{
	...
	else if(session->next_crypto != NULL && session->next_crypto->used & direction) {
...
static bool ssh_packet_need_rekey(ssh_session session, const uint32_t payloadsize)
{
	...
	/* Time based rekeying */
	if(session->opts.rekey_time != 0 && ssh_timeout_elapsed(&session->last_rekey_time, session->opts.rekey_time)) {
		return true;
	}
...
int ssh_packet_socket_callback(const void * data, size_t receivedlen, void * user)
{
	...
	int to_be_read;
	int rc;
	uint8 * cleartext_packet = NULL;
	uint8 * packet_second_block = NULL;
	uint8 * mac = NULL;
...
#ifdef DEBUG_PACKET
	SSH_LOG(SSH_LOG_PACKET, "rcv packet cb (len=%zu, state=%s)", receivedlen, session->packet_state == PACKET_STATE_INIT ?
	    "INIT" : session->packet_state == PACKET_STATE_SIZEREAD ? "SIZE_READ" : session->packet_state == PACKET_STATE_PROCESSING ? "PROCESSING" : "unknown");
#endif
	switch(session->packet_state) {
		case PACKET_STATE_INIT:
		    if(receivedlen < lenfield_blocksize + etm_packet_offset) {
			     ...
			     */
#ifdef DEBUG_PACKET
			    SSH_LOG(SSH_LOG_PACKET, "Waiting for more data (%zu < %u)", receivedlen, lenfield_blocksize);
#endif
			    return 0;
		    }
		    session->in_packet.Z(); //= (struct packet_struct) { .type = 0, };
		    ...
		    if(packet_len > MAX_PACKET_LEN) {
			    ssh_set_error(session, SSH_FATAL, "read_packet(): Packet len too high(%u %.4x)", packet_len, packet_len);
			    goto error;
		    }
		    if(to_be_read < 0) {
		...
		case PACKET_STATE_SIZEREAD:
		    ...
		    if(to_be_read != 0) {
			    if(receivedlen  < (uint)to_be_read) {
		    ...
		    packet_remaining = packet_len - (lenfield_blocksize - sizeof(uint32_t) + etm_packet_offset);
		    cleartext_packet = (uint8 *)ssh_buffer_allocate(session->in_buffer, packet_remaining);
		    ...
		    if(padding > ssh_buffer_get_len(session->in_buffer)) {
			    ssh_set_error(session, SSH_FATAL, "Invalid padding: %d (%d left)", padding, ssh_buffer_get_len(session->in_buffer));
			    goto error;
		    }
...
#ifdef WITH_ZLIB
		    if(crypto && crypto->do_compress_in && ssh_buffer_get_len(session->in_buffer) > 0) {
		    ...
		    session->recv_seq++;
		    if(crypto != NULL) {
			    struct ssh_cipher_struct * cipher = crypto->in_cipher;
			    cipher->packets++;
		    ...
		    SSH_LOG(SSH_LOG_PACKET, "packet: read type %hhd [len=%d,padding=%hhd,comp=%d,payload=%d]", session->in_packet.type, packet_len, padding, compsize, payloadsize);
		    ...
		    switch(filter_result) {
				...
				ssh_set_error(session, SSH_FATAL, "Packet filter: rejected packet (type %d)", session->in_packet.type);
				goto error;
...
{
	int rc = ssh_buffer_pack(session->out_buffer, "bd", SSH2_MSG_UNIMPLEMENTED, seqnum);
	if(rc != SSH_OK) {
...
SSH_PACKET_CALLBACK(ssh_packet_unimplemented)
{
	...
	if(rc != SSH_OK) {
		SSH_LOG(SSH_LOG_WARNING, "Could not unpack SSH_MSG_UNIMPLEMENTED packet");
	}
	SSH_LOG(SSH_LOG_RARE, "Received SSH_MSG_UNIMPLEMENTED (sequence number %d)", seq);
...
{
	int rc = SSH_ERROR;
	rc = ssh_socket_write(session->socket, ssh_buffer_get(session->out_buffer), ssh_buffer_get_len(session->out_buffer));
	return rc;
}
...
static int packet_send2(ssh_session session)
{
...
#ifdef WITH_ZLIB
	if(crypto != NULL && crypto->do_compress_out && ssh_buffer_get_len(session->out_buffer) > 0) {
	...
	if(crypto) {
		int ok = ssh_get_random(padding_data, padding_size, 0);
		if(!ok) {
...
#ifdef WITH_PCAP
	if(session->pcap_ctx != NULL) {
		ssh_pcap_context_write(session->pcap_ctx, SSH_PCAP_DIR_OUT, ssh_buffer_get(session->out_buffer), ssh_buffer_get_len(session->out_buffer), ssh_buffer_get_len(session->out_buffer));
	}
#endif
	hmac = ssh_packet_encrypt(session, ssh_buffer_get(session->out_buffer), ssh_buffer_get_len(session->out_buffer));
	if(hmac) {
	...
	session->send_seq++;
	if(crypto) {
		struct ssh_cipher_struct * cipher = crypto->out_cipher;
		cipher->packets++;
		cipher->blocks += payloadsize / cipher->blocksize;
	}
	if(session->raw_counter) {
		session->raw_counter->out_bytes += payloadsize;
		session->raw_counter->out_packets++;
	}
	SSH_LOG(SSH_LOG_PACKET, "packet: wrote [type=%u, len=%u, padding_size=%hhd, comp=%u, payload=%u]", type, finallen, padding_size, compsize, payloadsize);
...
static bool ssh_packet_is_kex(uchar type)
{
	return type >= SSH2_MSG_DISCONNECT && type <= SSH2_MSG_KEX_DH_GEX_REQUEST && type != SSH2_MSG_SERVICE_REQUEST &&
	       type != SSH2_MSG_SERVICE_ACCEPT && type != SSH2_MSG_IGNORE && type != SSH2_MSG_EXT_INFO;
}
...
static bool ssh_packet_in_rekey(ssh_session session)
{
	 ...
	 */
	return (session->flags & SSH_SESSION_FLAG_AUTHENTICATED) && (session->dh_handshake_state != DH_STATE_FINISHED);
}
...
int FASTCALL ssh_packet_send(ssh_session session)
{
	...
	int rc;
	uint32_t payloadsize = ssh_buffer_get_len(session->out_buffer);
	if(payloadsize < 1) {
		return SSH_ERROR;
	}
	payload = (uint8*)ssh_buffer_get(session->out_buffer);
	...
	if(rc == SSH_OK && type == SSH2_MSG_NEWKEYS) {
		struct ssh_iterator * it;
		for(it = ssh_list_get_iterator(session->out_queue); it != NULL; it = ssh_list_get_iterator(session->out_queue)) {
...
static void ssh_init_rekey_state(struct ssh_session_struct * session, struct ssh_cipher_struct * cipher)
{
	...
	if(session->opts.rekey_data != 0) {
		cipher->max_blocks = MIN(cipher->max_blocks, session->opts.rekey_data / cipher->blocksize);
	}
	SSH_LOG(SSH_LOG_PROTOCOL, "Set rekey after %" PRIu64 " blocks", cipher->max_blocks);
}
/*
 * Once we got SSH2_MSG_NEWKEYS we can switch next_crypto and
 * current_crypto for our desired direction
 */
int ssh_packet_set_newkeys(ssh_session session, enum ssh_crypto_direction_e direction)
{
	int rc;
	SSH_LOG(SSH_LOG_TRACE, "called, direction =%s%s", direction & SSH_DIRECTION_IN ? " IN " : "", direction & SSH_DIRECTION_OUT ? " OUT " : "");
	if(session->next_crypto == NULL) {
		return SSH_ERROR;
	}
	session->next_crypto->used = static_cast<enum ssh_crypto_direction_e>(session->next_crypto->used | direction);
	if(session->current_crypto != NULL) {
		...
		session->current_crypto->used = static_cast<enum ssh_crypto_direction_e>(session->current_crypto->used & ~direction);
	}
	...
	if(session->next_crypto->used == SSH_DIRECTION_BOTH) {
		...
		session->next_crypto->session_id = (uchar *)SAlloc::M(digest_len);
		if(session->next_crypto->session_id == NULL) {
			ssh_set_error_oom(session);
			return SSH_ERROR;
		}
		memcpy(session->next_crypto->session_id, session->current_crypto->session_id, digest_len);
		return SSH_OK;
	}
	// Initialize common structures so the next context can be used in either direction 
	if(session->client) {
	...
	if(session->next_crypto->in_cipher == NULL || session->next_crypto->out_cipher == NULL) {
		return SSH_ERROR;
	}
	/* Initialize rekeying states */
	ssh_init_rekey_state(session, session->next_crypto->out_cipher);
	ssh_init_rekey_state(session, session->next_crypto->in_cipher);
	if(session->opts.rekey_time != 0) {
		ssh_timestamp_init(&session->last_rekey_time);
		SSH_LOG(SSH_LOG_PROTOCOL, "Set rekey after %" PRIu32 " seconds", session->opts.rekey_time/1000);
	}
	/* Initialize the encryption and decryption keys in next_crypto */
	rc = session->next_crypto->in_cipher->set_decrypt_key(session->next_crypto->in_cipher, session->next_crypto->decryptkey, session->next_crypto->decryptIV);
	if(rc < 0) {
		/* On error, make sure it is not used */
		session->next_crypto->used = (enum ssh_crypto_direction_e)0;
		return SSH_ERROR;
	}
	rc = session->next_crypto->out_cipher->set_encrypt_key(session->next_crypto->out_cipher, session->next_crypto->encryptkey, session->next_crypto->encryptIV);
	if(rc < 0) {
		/* On error, make sure it is not used */
		session->next_crypto->used = (enum ssh_crypto_direction_e)0;
		return SSH_ERROR;
