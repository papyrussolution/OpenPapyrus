...
ssh_agent ssh_agent_new(struct ssh_session_struct * session) 
{
	...
	agent = SAlloc::M(sizeof(struct ssh_agent_struct));
	if(agent == NULL) {
		return NULL;
	}
	ZERO_STRUCTP(agent);
...
static int agent_talk(struct ssh_session_struct * session, struct ssh_buffer_struct * request, struct ssh_buffer_struct * reply) 
{
	...
	if(atomicio(session->agent, payload, 4, 0) == 4) {
		if(atomicio(session->agent, ssh_buffer_get(request), len, 0) != len) {
			SSH_LOG(SSH_LOG_WARN, "atomicio sending request failed: %s", strerror(errno));
			return -1;
		}
	}
	else {
		SSH_LOG(SSH_LOG_WARN, "atomicio sending request length failed: %s", strerror(errno));
		return -1;
	}
...
uint32_t ssh_agent_get_ident_count(struct ssh_session_struct * session)
{
	...
	if(agent_failed(type)) {
		...
		ssh_set_error(session, SSH_FATAL, "Bad authentication reply message type: %u", type);
		SSH_BUFFER_FREE(reply);
		return 0;
	}
	rc = ssh_buffer_get_u32(reply, &count);
	if(rc != 4) {
		ssh_set_error(session, SSH_FATAL, "Failed to read count");
		SSH_BUFFER_FREE(reply);
		return 0;
	}
...
ssh_key ssh_agent_get_first_ident(struct ssh_session_struct * session, char ** comment) 
{
    ...
    struct ssh_buffer_struct * data)
{
	 ...
	 * - 1 x uint8
	 * - 2 x uint32_t
	 * - 1 x ssh_string (uint8 + data)
	 */
	rc = ssh_buffer_allocate_size(request, sizeof(uint8) * 2 + sizeof(uint32_t) * 2 + ssh_string_len(key_blob));
	if(rc < 0) {
		SSH_BUFFER_FREE(request);
		return NULL;
	}
	...
	if(ssh_buffer_get_u8(reply, (uint8 *)&type) != sizeof(uint8)) {
		SSH_BUFFER_FREE(reply);
		return NULL;
	}
#ifdef SL_BIGENDIAN
	...
	if(agent_failed(type)) {
		...
		ssh_set_error(session, SSH_FATAL, "Bad authentication response: %u", type);
		SSH_BUFFER_FREE(reply);
		return NULL;
	}
