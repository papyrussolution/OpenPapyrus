...
{
	int opt = 1;
	return setsockopt(socket, IPPROTO_TCP, TCP_NODELAY, (const char *)&opt, sizeof(opt));
}
/**
 * @internal
 *
...
socket_t ssh_connect_host_nonblocking(ssh_session session, const char * host, const char * bind_addr, int port)
{
	...
	for(itr = ai; itr; itr = itr->ai_next) {
		...
		if(s < 0) {
			ssh_set_error(session, SSH_FATAL, "Socket create failed: %s", strerror(errno));
			continue;
		}
		if(bind_addr) {
		...
		if(rc < 0) {
			ssh_set_error(session, SSH_FATAL, "Failed to set socket non-blocking for %s:%d", host, port);
			ssh_connect_socket_close(s);
			s = -1;
			continue;
		}
