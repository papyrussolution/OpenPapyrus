...
int ssh_client_ecdh_init(ssh_session session)
{
	...
	session->next_crypto->ecdh_privkey = SAlloc::M(sizeof(mbedtls_ecp_keypair));
	...
	client_pubkey = make_ecpoint_string(&grp, &session->next_crypto->ecdh_privkey->Q);
...
int ecdh_build_k(ssh_session session)
{
	...
	session->next_crypto->shared_secret = SAlloc::M(sizeof(mbedtls_mpi));
