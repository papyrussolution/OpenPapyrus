...
static struct ssh_packet_callbacks_struct ssh_dhgex_client_callbacks = { SSH2_MSG_KEX_DH_GEX_GROUP, 3, dhgex_client_callbacks, NULL };
...
{
	int rc = ssh_dh_init_common(session->next_crypto);
	if(rc != SSH_OK) {
		goto error;
	}
...
SSH_PACKET_CALLBACK(ssh_packet_client_dhgex_group)
{
	...
	rc = ssh_buffer_unpack(packet, "BB", &modulus, &generator);
	if(rc != SSH_OK) {
		ssh_set_error(session, SSH_FATAL, "Invalid DH_GEX_GROUP packet");
		goto error;
	}
	/* basic checks */
	if(ssh_fips_mode() && !ssh_dh_is_known_group(modulus, generator)) {
		ssh_set_error(session, SSH_FATAL, "The received DH group is not FIPS approved");
		goto error;
	}
	...
	if(blen < DH_PMIN || blen > DH_PMAX) {
		ssh_set_error(session, SSH_FATAL, "Invalid dh group parameter p: %d not in [%d:%d]", blen, DH_PMIN, DH_PMAX);
		goto error;
	}
	...
	if(bignum_cmp(generator, one) <= 0 || bignum_cmp(generator, pmin1) > 0) {
...
static struct ssh_packet_callbacks_struct ssh_dhgex_server_callbacks = { SSH2_MSG_KEX_DH_GEX_REQUEST_OLD, 5, dhgex_server_callbacks, NULL };
