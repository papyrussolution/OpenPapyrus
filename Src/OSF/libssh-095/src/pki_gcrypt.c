...
static int load_iv(const char * header, uchar * iv, int iv_len) 
{
	int i;
	int j;
	int k;
...
static uint32_t char_to_u32(uchar * data, uint32_t size) {
	uint32_t ret;
	uint32_t i;
...
static ssh_string asn1_get(ssh_buffer buffer, uchar want) 
{
	ssh_string str;
	uchar type;
	uint32_t size;
...
static ssh_string asn1_get_int(ssh_buffer buffer) { return asn1_get(buffer, ASN1_INTEGER); }
...
static ssh_string asn1_get_bit_string(ssh_buffer buffer)
{
	...
	uint32_t size;
	uchar unused, last, * p;
	uint32_t len = ssh_buffer_get_data(buffer, &type, 1);
...
static int asn1_check_tag(ssh_buffer buffer, uchar tag) 
{
	uchar tmp;
	uint32_t len;
...
static int passphrase_to_key(char * data, uint datalen, uchar * salt, uchar * key, uint keylen) 
{
	MD5CTX md;
	uchar digest[MD5_DIGEST_LEN] = {0};
	uint i;
	uint j;
	uint md_not_empty;
...
static int privatekey_decrypt(int algo, int mode, uint key_len,
    uchar * iv, uint iv_len,
    ssh_buffer data, ssh_auth_callback cb,
    void * userdata,
    const char * desc)
{
	char passphrase[MAX_PASSPHRASE_SIZE] = {0};
	uchar key[MAX_KEY_SIZE] = {0};
	uchar * tmp = NULL;
...
static int privatekey_dek_header(const char * header, uint header_len,
    int * algo, int * mode, uint * key_len, uchar ** iv,
    uint * iv_len) {
	uint iv_pos;
...
static ssh_buffer privatekey_string_to_buffer(const char * pkey, int type,
	...
	uint header_begin_size;
	uint header_end_size;
	uint key_len = 0;
	uint iv_len = 0;
	int algo = 0;
	int mode = 0;
    ...
    enum ssh_digest_e hash_type)
{
	...
	switch(type) {
		...
		case SSH_KEYTYPE_RSA:
		    ...
		    if(len > rsalen) {
			    SSH_LOG(SSH_LOG_WARN, "Signature is to big size: %lu", (ulong)len);
			    ssh_signature_free(sig);
			    return NULL;
		    }
		    if(len < rsalen) {
...
#ifdef DEBUG_CRYPTO
		    SSH_LOG(SSH_LOG_DEBUG, "RSA signature len: %lu", (ulong)len);
		    ssh_log_hexdump("RSA signature", ssh_string_data(sig_blob), len);
#endif
		    err = gcry_sexp_build(&sig->rsa_sig, NULL, "(sig-val(rsa(s %b)))", ssh_string_len(sig_blob), ssh_string_data(sig_blob));
		    if(err) {
			    ssh_signature_free(sig);
			    return NULL;
		    }
...
ssh_signature pki_sign_data(const ssh_key privkey,
    ...
    size_t input_len)
{
	uchar hash[SHA512_DIGEST_LEN] = {0};
	const uchar * sign_input = NULL;
	uint32_t hlen = 0;
	int rc;
    ...
    size_t input_len)
{
	...
	uchar ghash[SHA512_DIGEST_LEN + 1] = {0};
	uchar * hash = ghash + 1;
	uint32_t hlen = 0;

	const uchar * verify_input = NULL;
	...
	switch(pubkey->type) {
		...
		case SSH_KEYTYPE_ECDSA_P521_CERT01:
#ifdef HAVE_GCRYPT_ECC
		    err = gcry_sexp_build(&sexp, NULL, "(data(flags raw)(value %b))", hlen, hash);
		    if(err) {
		    ...
		    if(err) {
			    ...
			    if(gcry_err_code(err) != GPG_ERR_BAD_SIGNATURE) {
				    SSH_LOG(SSH_LOG_TRACE, "ECDSA verify error: %s", gcry_strerror(err));
			    }
			    return SSH_ERROR;
		    }
		    break;
