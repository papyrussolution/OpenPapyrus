...
{
	struct ssh_cipher_struct * cipher = (struct ssh_cipher_struct *)SAlloc::M(sizeof(struct ssh_cipher_struct));
	if(cipher == NULL) {
		return NULL;
	}
...
{
	struct ssh_crypto_struct * crypto = (struct ssh_crypto_struct *)SAlloc::M(sizeof(struct ssh_crypto_struct));
	if(crypto == NULL) {
		return NULL;
	}
	ZERO_STRUCTP(crypto);
...
static int crypt_set_algorithms2(ssh_session session)
{
	...
	if(ssh_hmactab[i].name == NULL) {
		ssh_set_error(session, SSH_FATAL, "crypt_set_algorithms2: no hmac algorithm function found for %s", wanted);
		return SSH_ERROR;
	}
