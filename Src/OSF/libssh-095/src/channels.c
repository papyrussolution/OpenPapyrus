...
ssh_channel ssh_channel_new(ssh_session session)
{
	...
	channel = (ssh_channel)SAlloc::C(1, sizeof(struct ssh_channel_struct));
	if(!channel) {
...
SSH_PACKET_CALLBACK(ssh_packet_channel_open_conf)
{
	...
	rc = ssh_buffer_unpack(packet, "ddd", &channel->remote_channel, &channel->remote_window, &channel->remote_maxpacket);
	if(rc != SSH_OK)
		goto error;
	SSH_LOG(SSH_LOG_PROTOCOL, "Received a CHANNEL_OPEN_CONFIRMATION for channel %d:%d", channel->local_channel, channel->remote_channel);
	if(channel->state != SSH_CHANNEL_STATE_OPENING) {
		SSH_LOG(SSH_LOG_RARE, "SSH2_MSG_CHANNEL_OPEN_CONFIRMATION received in incorrect channel state %d", channel->state);
		goto error;
	}
	SSH_LOG(SSH_LOG_PROTOCOL, "Remote window : %" PRIu32 ", maxpacket : %" PRIu32, (uint32_t)channel->remote_window, (uint32_t)channel->remote_maxpacket);
...
SSH_PACKET_CALLBACK(ssh_packet_channel_open_fail)
{
	...
	if(channel->state != SSH_CHANNEL_STATE_OPENING) {
		SSH_LOG(SSH_LOG_RARE, "SSH2_MSG_CHANNEL_OPEN_FAILURE received in incorrect channel state %d", channel->state);
		goto error;
	}
...
static int ssh_channel_open_termination(void * c)
{
	...
	if(channel->state != SSH_CHANNEL_STATE_OPENING || channel->session->session_state == SSH_SESSION_STATE_ERROR)
		return 1;
	else
		return 0;
}
 ...
 */
static int channel_open(ssh_channel channel, const char * type, uint32_t window, uint32_t maxpacket, ssh_buffer payload)
{
	ssh_session session = channel->session;
	int err = SSH_ERROR;
	int rc;
	switch(channel->state) {
		case SSH_CHANNEL_STATE_NOT_OPEN: break;
		case SSH_CHANNEL_STATE_OPENING: goto pending;
		case SSH_CHANNEL_STATE_OPEN:
		case SSH_CHANNEL_STATE_CLOSED:
		case SSH_CHANNEL_STATE_OPEN_DENIED: goto end;
		default: ssh_set_error(session, SSH_FATAL, "Bad state in channel_open: %d", channel->state);
	}
	channel->local_channel = ssh_channel_new_id(session);
	channel->local_maxpacket = maxpacket;
	channel->local_window = window;
	SSH_LOG(SSH_LOG_PROTOCOL, "Creating a channel %d with %d window and %d max packet", channel->local_channel, window, maxpacket);
	rc = ssh_buffer_pack(session->out_buffer, "bsddd", SSH2_MSG_CHANNEL_OPEN, type, channel->local_channel, channel->local_window, channel->local_maxpacket);
	if(rc != SSH_OK) {
		ssh_set_error_oom(session);
		return err;
	}
	...
	SSH_LOG(SSH_LOG_PACKET, "Sent a SSH_MSG_CHANNEL_OPEN type %s for channel %d", type, channel->local_channel);
pending:
 ...
 * @return            SSH_OK if successful; SSH_ERROR otherwise.
 */
static int grow_window(ssh_session session, ssh_channel channel, uint32_t minimumsize)
{
	uint32_t new_window = minimumsize > WINDOWBASE ? minimumsize : WINDOWBASE;
	int rc;
	if(new_window <= channel->local_window) {
		SSH_LOG(SSH_LOG_PROTOCOL, "growing window (channel %d:%d) to %d bytes : not needed (%d bytes)", channel->local_channel, channel->remote_channel, new_window, channel->local_window);
		return SSH_OK;
	}
	 ...
	 */
	rc = ssh_buffer_pack(session->out_buffer, "bdd", SSH2_MSG_CHANNEL_WINDOW_ADJUST, channel->remote_channel, new_window - channel->local_window);
	if(rc != SSH_OK) {
		ssh_set_error_oom(session);
		goto error;
	}
	...
	SSH_LOG(SSH_LOG_PROTOCOL, "growing window (channel %d:%d) to %d bytes", channel->local_channel, channel->remote_channel, new_window);
...
static ssh_channel channel_from_msg(ssh_session session, ssh_buffer packet) 
{
	ssh_channel channel;
	uint32_t chan;
	int rc = ssh_buffer_unpack(packet, "d", &chan);
	if(rc != SSH_OK) {
	...
	if(!channel) {
		ssh_set_error(session, SSH_FATAL, "Server specified invalid channel %" PRIu32, (uint32_t)chan);
	}
	return channel;
}
...
SSH_PACKET_CALLBACK(channel_rcv_change_window) 
{
	...
	if(channel == NULL || rc != SSH_OK) {
		SSH_LOG(SSH_LOG_PACKET, "Error getting a window adjust message: invalid packet");
		return SSH_PACKET_USED;
	}
	SSH_LOG(SSH_LOG_PROTOCOL, "Adding %d bytes to channel (%d:%d) (from %d bytes)", bytes, channel->local_channel, channel->remote_channel, channel->remote_window);
...
SSH_PACKET_CALLBACK(channel_rcv_data)
{
	...
	SSH_LOG(SSH_LOG_PACKET, "Channel receiving %" PRIdS " bytes data in %d (local win=%d remote win=%d)", len, is_stderr, channel->local_window, channel->remote_window);
	/* What shall we do in this case? Let's accept it anyway */
	if(len > channel->local_window) {
		SSH_LOG(SSH_LOG_RARE, "Data packet too big for our window(%" PRIdS " vs %d)", len, channel->local_window);
	}
	...
	SSH_LOG(SSH_LOG_PACKET, "Channel windows are now (local win=%d remote win=%d)", channel->local_window, channel->remote_window);
	SSH_STRING_FREE(str);
	if(is_stderr) {
	...
	ssh_callbacks_iterate(channel->callbacks, ssh_channel_callbacks, channel_data_function) {
		if(ssh_buffer_get(buf) == NULL) {
			break;
		}
		rest = ssh_callbacks_iterate_exec(channel_data_function, channel->session, channel, ssh_buffer_get(buf), ssh_buffer_get_len(buf), is_stderr);
		if(rest > 0) {
			if(channel->counter) {
...
SSH_PACKET_CALLBACK(channel_rcv_eof) 
{
	...
	SSH_LOG(SSH_LOG_PACKET, "Received eof on channel (%d:%d)", channel->local_channel, channel->remote_channel);
	/* channel->remote_window = 0; */
	channel->remote_eof = 1;
	ssh_callbacks_execute_list(channel->callbacks, ssh_channel_callbacks, channel_eof_function, channel->session, channel);
	return SSH_PACKET_USED;
}
...
SSH_PACKET_CALLBACK(channel_rcv_close) 
{
	(void)user;
	(void)type;
	ssh_channel channel = channel_from_msg(session, packet);
	if(!channel) {
		SSH_LOG(SSH_LOG_FUNCTIONS, "%s", ssh_get_error(session));
		return SSH_PACKET_USED;
	}
	SSH_LOG(SSH_LOG_PACKET, "Received close on channel (%d:%d)", channel->local_channel, channel->remote_channel);
	if((channel->stdout_buffer && ssh_buffer_get_len(channel->stdout_buffer) > 0) || (channel->stderr_buffer && ssh_buffer_get_len(channel->stderr_buffer) > 0)) {
	...
	if(channel->remote_eof == 0) {
		SSH_LOG(SSH_LOG_PACKET, "Remote host not polite enough to send an eof before close");
	}
	channel->remote_eof = 1;
	/*
	 ...
	 */
	ssh_callbacks_execute_list(channel->callbacks, ssh_channel_callbacks, channel_close_function, channel->session, channel);
...
SSH_PACKET_CALLBACK(channel_rcv_request) 
{
	...
	rc = ssh_buffer_unpack(packet, "sb", &request, &status);
	if(rc != SSH_OK) {
	...
	if(strcmp(request, "exit-status") == 0) {
		...
		ssh_callbacks_execute_list(channel->callbacks, ssh_channel_callbacks, channel_exit_status_function, channel->session, channel, channel->exit_status);
		return SSH_PACKET_USED;
	}
	if(strcmp(request, "signal") == 0) {
		...
		ssh_callbacks_execute_list(channel->callbacks, ssh_channel_callbacks, channel_signal_function, channel->session, channel, sig);
		ZFREE(sig);
		return SSH_PACKET_USED;
	}
	if(strcmp(request, "exit-signal") == 0) {
		...
		uint8 core_dumped;
		ZFREE(request);
		rc = ssh_buffer_unpack(packet, "sbss", &sig/* signal name */, &core_dumped/* core dumped */, &errmsg/* error message */, &lang);
		if(rc != SSH_OK) {
			SSH_LOG(SSH_LOG_PACKET, "Invalid MSG_CHANNEL_REQUEST");
			return SSH_PACKET_USED;
		}
		...
		SSH_LOG(SSH_LOG_PACKET, "Remote connection closed by signal SIG %s %s", sig, core);
		ssh_callbacks_execute_list(channel->callbacks, ssh_channel_callbacks, channel_exit_signal_function, channel->session,
		    channel, sig, core_dumped, errmsg, lang);
		ZFREE(lang);
		ZFREE(errmsg);
		ZFREE(sig);
		return SSH_PACKET_USED;
 ...
 */
int channel_default_bufferize(ssh_channel channel, void * data, size_t len, bool is_stderr)
{
	ssh_session session;
	if(!channel) {
		return -1;
	...
	SSH_LOG(SSH_LOG_PACKET, "placing %zu bytes into channel buffer (%s)", len, is_stderr ? "stderr" : "stdout");
	if(!is_stderr) {
		/* stdout */
		if(channel->stdout_buffer == NULL) {
			channel->stdout_buffer = ssh_buffer_new();
...
int ssh_channel_open_auth_agent(ssh_channel channel)
{
	...
	return channel_open(channel, "auth-agent@openssh.com", CHANNEL_INITIAL_WINDOW, CHANNEL_MAX_PACKET, NULL);
}
    ...
    uint32_t len, int is_stderr)
{
	...
	if(len > INT_MAX) {
		SSH_LOG(SSH_LOG_PROTOCOL, "Length (%u) is bigger than INT_MAX", len);
		return SSH_ERROR;
	}
	/*
	...
	if(channel->local_eof) {
		ssh_set_error(session, SSH_REQUEST_DENIED, "Can't write to channel %d:%d  after EOF was sent", channel->local_channel, channel->remote_channel);
		return -1;
	}
	...
	if(ssh_waitsession_unblocked(session) == 0) {
		rc = ssh_handle_packets_termination(session, SSH_TIMEOUT_DEFAULT, ssh_waitsession_unblocked, session);
		if(rc == SSH_ERROR || !ssh_waitsession_unblocked(session))
			goto out;
	}
	while(len > 0) {
		if(channel->remote_window < len) {
			SSH_LOG(SSH_LOG_PROTOCOL, "Remote window is %d bytes. going to write %d bytes", channel->remote_window, len);
			/* What happens when the channel window is zero? */
			if(channel->remote_window == 0) {
				/* nothing can be written */
				SSH_LOG(SSH_LOG_PROTOCOL, "Wait for a growing window message...");
				rc = ssh_handle_packets_termination(session, SSH_TIMEOUT_DEFAULT, ssh_channel_waitwindow_termination, channel);
				if(rc == SSH_ERROR || !ssh_channel_waitwindow_termination(channel) || session->session_state == SSH_SESSION_STATE_ERROR || channel->state == SSH_CHANNEL_STATE_CLOSED)
					goto out;
				continue;
			}
 ...
 */
uint32_t ssh_channel_window_size(ssh_channel channel) { return channel->remote_window; }
/**
 ...
 */
int ssh_channel_write(ssh_channel channel, const void * data, uint32_t len) { return channel_write_common(channel, data, len, 0); }
/**
 ...
 */
int ssh_channel_is_open(ssh_channel channel) { return channel ? (channel->state == SSH_CHANNEL_STATE_OPEN && channel->session->alive != 0) : 0; }
/**
 ...
 */
int ssh_channel_is_closed(ssh_channel channel) { return channel ? (channel->state != SSH_CHANNEL_STATE_OPEN || channel->session->alive == 0) : SSH_ERROR; }
/**
...
SSH_PACKET_CALLBACK(ssh_packet_channel_success)
{
	...
	SSH_LOG(SSH_LOG_PACKET, "Received SSH_CHANNEL_SUCCESS on channel (%d:%d)", channel->local_channel, channel->remote_channel);
	if(channel->request_state != SSH_CHANNEL_REQ_STATE_PENDING) {
		SSH_LOG(SSH_LOG_RARE, "SSH_CHANNEL_SUCCESS received in incorrect state %d", channel->request_state);
	}
	else {
...
SSH_PACKET_CALLBACK(ssh_packet_channel_failure)
{
	...
	SSH_LOG(SSH_LOG_PACKET, "Received SSH_CHANNEL_FAILURE on channel (%d:%d)", channel->local_channel, channel->remote_channel);
	if(channel->request_state != SSH_CHANNEL_REQ_STATE_PENDING) {
		SSH_LOG(SSH_LOG_RARE, "SSH_CHANNEL_FAILURE received in incorrect state %d", channel->request_state);
	}
	else {
...
{
	ssh_channel channel = (ssh_channel)c;
	if(channel->request_state != SSH_CHANNEL_REQ_STATE_PENDING || channel->session->session_state == SSH_SESSION_STATE_ERROR)
		return 1;
	else
		return 0;
}
...
static int channel_request(ssh_channel channel, const char * request, ssh_buffer buffer, int reply) 
{
	...
	SSH_LOG(SSH_LOG_PACKET, "Sent a SSH_MSG_CHANNEL_REQUEST %s", request);
	if(reply == 0) {
		channel->request_state = SSH_CHANNEL_REQ_STATE_NONE;
		return SSH_OK;
	}
pending:
	rc = ssh_handle_packets_termination(session, SSH_TIMEOUT_DEFAULT, ssh_channel_request_termination, channel);
	...
	switch(channel->request_state) {
		...
		case SSH_CHANNEL_REQ_STATE_ACCEPTED:
		    SSH_LOG(SSH_LOG_PROTOCOL, "Channel request %s success", request);
		    rc = SSH_OK;
		    break;
...
static char * generate_cookie() 
{
	...
	uchar rnd[16];
	int i;
	int ok = ssh_get_random(rnd, sizeof(rnd), 0);
	if(!ok) {
		return NULL;
	}
	for(i = 0; i < 16; i++) {
...
SSH_PACKET_CALLBACK(ssh_request_success)
{
	...
	(void)user;
	(void)packet;
	SSH_LOG(SSH_LOG_PACKET, "Received SSH_REQUEST_SUCCESS");
	if(session->global_req_state != SSH_CHANNEL_REQ_STATE_PENDING) {
		SSH_LOG(SSH_LOG_RARE, "SSH_REQUEST_SUCCESS received in incorrect state %d", session->global_req_state);
	}
	else {
...
SSH_PACKET_CALLBACK(ssh_request_denied)
{
	...
	(void)user;
	(void)packet;
	SSH_LOG(SSH_LOG_PACKET, "Received SSH_REQUEST_FAILURE");
	if(session->global_req_state != SSH_CHANNEL_REQ_STATE_PENDING) {
		SSH_LOG(SSH_LOG_RARE, "SSH_REQUEST_DENIED received in incorrect state %d", session->global_req_state);
	}
	else {
...
{
	ssh_session session = (ssh_session)s;
	if(session->global_req_state != SSH_CHANNEL_REQ_STATE_PENDING || session->session_state == SSH_SESSION_STATE_ERROR)
		return 1;
	else
		return 0;
}
...
static int ssh_channel_read_termination(void * s)
{
	...
	if(ssh_buffer_get_len(ctx->buffer) >= ctx->count || ctx->channel->remote_eof || ctx->channel->session->session_state == SSH_SESSION_STATE_ERROR)
		return 1;
	else
		return 0;
}
...
int ssh_channel_read_timeout(ssh_channel channel, void * dest, uint32_t count, int is_stderr, int timeout_ms)
{
	...
	rc = ssh_handle_packets_termination(session, timeout_ms, ssh_channel_read_termination, &ctx);
	if(rc == SSH_ERROR) {
		return rc;
	}
	/*
...
int ssh_channel_poll_timeout(ssh_channel channel, int timeout, int is_stderr)
{
	...
	ctx.channel = channel;
	ctx.count = 1;
	rc = ssh_handle_packets_termination(channel->session, timeout, ssh_channel_read_termination, &ctx);
	if(rc == SSH_ERROR || session->session_state == SSH_SESSION_STATE_ERROR) {
		rc = SSH_ERROR;
		goto out;
	}
...
static int ssh_channel_exit_status_termination(void * c)
{
	...
	if(channel->exit_status != -1 ||
	    /* When a channel is closed, no exit status message can come anymore */
	    (channel->flags & SSH_CHANNEL_FLAG_CLOSED_REMOTE) || channel->session->session_state == SSH_SESSION_STATE_ERROR)
		return 1;
	else
		return 0;
}
...
int ssh_channel_get_exit_status(ssh_channel channel) 
{
	...
	rc = ssh_handle_packets_termination(channel->session, SSH_TIMEOUT_DEFAULT, ssh_channel_exit_status_termination, channel);
	if(rc == SSH_ERROR || channel->session->session_state == SSH_SESSION_STATE_ERROR)
		return SSH_ERROR;
...
int ssh_channel_select(ssh_channel * readchans, ssh_channel * writechans, ssh_channel * exceptchans, struct timeval * timeout) 
{
	...
	rchans = (ssh_channel *)SAlloc::C(count_ptrs(readchans) + 1, sizeof(ssh_channel));
	if(rchans == NULL) {
		return SSH_ERROR;
	}
...
int ssh_channel_request_send_exit_signal(ssh_channel channel, const char * sig, int core, const char * errmsg, const char * lang) 
{
	...
	rc = ssh_buffer_pack(buffer, "sbss", sig, core ? 1 : 0, errmsg, lang);
	if(rc != SSH_OK) {
		ssh_set_error_oom(channel->session);
		goto error;
	}
