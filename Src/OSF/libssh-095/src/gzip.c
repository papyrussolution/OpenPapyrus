...
static z_stream * initcompress(ssh_session session, int level) 
{
	...
	int status;
	stream = (z_stream *)SAlloc::C(1, sizeof(z_stream));
	if(stream == NULL) {
		return NULL;
	}
...
static ssh_buffer gzip_compress(ssh_session session, ssh_buffer source, int level)
{
	...
	ulong in_size = ssh_buffer_get_len(source);
	ssh_buffer dest = NULL;
	uchar out_buf[BLOCKSIZE] = {0};
	ulong len;
	int status;
	struct ssh_crypto_struct * crypto = ssh_packet_get_current_crypto(session, SSH_DIRECTION_OUT);
	if(crypto == NULL) {
		return NULL;
	}
	zout = (z_stream *)crypto->compress_out_ctx;
	if(zout == NULL) {
		zout = (z_stream *)(crypto->compress_out_ctx = initcompress(session, level));
		if(zout == NULL) {
			return NULL;
		}
	}
...
{
	int status;
	z_stream * stream = (z_stream *)SAlloc::C(1, sizeof(z_stream));
	if(stream == NULL) {
		return NULL;
	}
...
static ssh_buffer gzip_decompress(ssh_session session, ssh_buffer source, size_t maxlen)
{
	...
	if(zin == NULL) {
		zin = (z_stream *)(crypto->compress_in_ctx = initdecompress(session));
		if(zin == NULL) {
			return NULL;
		}
	}
