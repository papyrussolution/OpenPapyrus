...
{
	int rc;
	if(sshbind->ecdsakey == NULL && sshbind->dsakey == NULL && sshbind->rsakey == NULL && sshbind->ed25519key == NULL) {
		ssh_set_error(sshbind, SSH_FATAL, "ECDSA, ED25519, DSA, or RSA host key file must be set");
		return SSH_ERROR;
	}
#ifdef HAVE_ECC
...
int ssh_bind_listen(ssh_bind sshbind) {
	...
	socket_t fd;
	int rc;
	if(sshbind->rsa == NULL && sshbind->dsa == NULL && sshbind->ecdsa == NULL && sshbind->ed25519 == NULL) {
	...
	if(sshbind->bindfd == SSH_INVALID_SOCKET) {
		...
		if(listen(fd, 10) < 0) {
			ssh_set_error(sshbind, SSH_FATAL, "Listening to socket %d: %s", fd, strerror(errno));
			CLOSE_SOCKET(fd);
			ssh_key_free(sshbind->dsa);
			sshbind->dsa = NULL;
			ssh_key_free(sshbind->rsa);
...
int ssh_bind_accept_fd(ssh_bind sshbind, ssh_session session, socket_t fd)
{
	...
	for(i = 0; i < SSH_KEX_METHODS; i++) {
		if(sshbind->wanted_methods[i]) {
			session->opts.wanted_methods[i] = sstrdup(sshbind->wanted_methods[i]);
