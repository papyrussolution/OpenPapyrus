...
#ifdef WITH_ZLIB
	#define ZLIB_METHS "none,zlib,zlib@openssh.com"
#else
	#define ZLIB_METHS "none"
#endif
...
SSH_PACKET_CALLBACK(ssh_packet_kexinit)
{
	...
	uint8 first_kex_packet_follows = 0;
...
int ssh_set_client_kex(ssh_session session)
{
	...
	kex_tmp = (char *)SAlloc::R(kex, kex_len);
	if(kex_tmp == NULL) {
		SAlloc::F(kex);
		ssh_set_error_oom(session);
		return SSH_ERROR;
...
int ssh_make_sessionid(ssh_session session)
{
...
#ifdef DEBUG_CRYPTO
	ssh_log_hexdump("hash buffer", (const uchar *)ssh_buffer_get(buf), ssh_buffer_get_len(buf));
#endif
	switch(session->next_crypto->kex_type) {
		case SSH_KEX_DH_GROUP1_SHA1:
		case SSH_KEX_DH_GROUP14_SHA1:
		...
		case SSH_KEX_DH_GEX_SHA1:
		    ...
		    session->next_crypto->secret_hash = (uchar *)SAlloc::M(session->next_crypto->digest_len);
		    if(session->next_crypto->secret_hash == NULL) {
			    ssh_set_error_oom(session);
			    goto error;
		    }
		    sha1((const uchar *)ssh_buffer_get(buf), ssh_buffer_get_len(buf),
		...
		case SSH_KEX_DH_GEX_SHA256:
		    ...
		    sha256((const uchar *)ssh_buffer_get(buf), ssh_buffer_get_len(buf),
			session->next_crypto->secret_hash);
		    break;
		case SSH_KEX_ECDH_SHA2_NISTP384:
		    ...
		    sha384((const uchar *)ssh_buffer_get(buf), ssh_buffer_get_len(buf),
			session->next_crypto->secret_hash);
		    break;
		case SSH_KEX_DH_GROUP16_SHA512:
		case SSH_KEX_DH_GROUP18_SHA512:
		case SSH_KEX_ECDH_SHA2_NISTP521:
		    ...
		    sha512((const uchar *)ssh_buffer_get(buf), ssh_buffer_get_len(buf), session->next_crypto->secret_hash);
		    break;
	}
	...
	if(!session->next_crypto->session_id) {
		session->next_crypto->session_id = (uchar *)SAlloc::M(session->next_crypto->digest_len);
		if(session->next_crypto->session_id == NULL) {
			ssh_set_error_oom(session);
			goto error;
		}
		memcpy(session->next_crypto->session_id, session->next_crypto->secret_hash, session->next_crypto->digest_len);
	}
#ifdef DEBUG_CRYPTO
	printf("Session hash: \n");
...
int ssh_hashbufout_add_cookie(ssh_session session)
{
	...
	rc = ssh_buffer_allocate_size(session->out_hashbuf, sizeof(uint8) + 16);
	if(rc < 0) {
		ssh_buffer_reinit(session->out_hashbuf);
		return -1;
	}
...
int ssh_hashbufin_add_cookie(ssh_session session, uchar * cookie)
{
	int rc;
	...
	rc = ssh_buffer_allocate_size(session->in_hashbuf, sizeof(uint8) + 20 + 16);
	if(rc < 0) {
...
int ssh_generate_session_keys(ssh_session session)
{
	...
	uchar * key = NULL;
	uchar * IV_cli_to_srv = NULL;
	uchar * IV_srv_to_cli = NULL;
	uchar * enckey_cli_to_srv = NULL;
	uchar * enckey_srv_to_cli = NULL;
	uchar * intkey_cli_to_srv = NULL;
	uchar * intkey_srv_to_cli = NULL;
	size_t key_len = 0;
	size_t IV_len = 0;
	...
	int rc = -1;
	ssh_string k_string = ssh_make_bignum_string(crypto->shared_secret);
	if(k_string == NULL) {
		ssh_set_error_oom(session);
		goto error;
	}
	...
	IV_cli_to_srv = (uchar *)SAlloc::M(IV_len);
	IV_srv_to_cli = (uchar *)SAlloc::M(IV_len);
	enckey_cli_to_srv = (uchar *)SAlloc::M(enckey_cli_to_srv_len);
	enckey_srv_to_cli = (uchar *)SAlloc::M(enckey_srv_to_cli_len);
	intkey_cli_to_srv = (uchar *)SAlloc::M(intkey_cli_to_srv_len);
	intkey_srv_to_cli = (uchar *)SAlloc::M(intkey_srv_to_cli_len);
	if(IV_cli_to_srv == NULL || IV_srv_to_cli == NULL || enckey_cli_to_srv == NULL || enckey_srv_to_cli == NULL || intkey_cli_to_srv == NULL || intkey_srv_to_cli == NULL) {
		ssh_set_error_oom(session);
		goto error;
	}
	/* IV */
	...
	/* Encryption Key */
	rc = ssh_kdf(crypto, key, key_len, 'C', enckey_cli_to_srv, enckey_cli_to_srv_len);
	if(rc < 0) {
		goto error;
	}
	rc = ssh_kdf(crypto, key, key_len, 'D', enckey_srv_to_cli, enckey_srv_to_cli_len);
	if(rc < 0) {
		goto error;
	}
	/* Integrity Key */
	rc = ssh_kdf(crypto, key, key_len, 'E', intkey_cli_to_srv, intkey_cli_to_srv_len);
	if(rc < 0) {
		goto error;
	}
	rc = ssh_kdf(crypto, key, key_len, 'F', intkey_srv_to_cli, intkey_srv_to_cli_len);
	if(rc < 0) {
		goto error;
	}
	if(session->client) {
	...
	ssh_log_hexdump("Client to Server Encryption Key", enckey_cli_to_srv, enckey_cli_to_srv_len);
	ssh_log_hexdump("Server to Client Encryption Key", enckey_srv_to_cli, enckey_srv_to_cli_len);
	ssh_log_hexdump("Client to Server Integrity Key", intkey_cli_to_srv, intkey_cli_to_srv_len);
	ssh_log_hexdump("Server to Client Integrity Key", intkey_srv_to_cli, intkey_srv_to_cli_len);
#endif
	rc = 0;
error:
	...
	if(rc) {
		...
		SAlloc::F(enckey_srv_to_cli);
		SAlloc::F(intkey_cli_to_srv);
		SAlloc::F(intkey_srv_to_cli);
	}
	return rc;
}
