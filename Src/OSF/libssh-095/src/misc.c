...
char * ssh_get_user_home_dir() 
{
	...
	if(SHGetSpecialFolderPathA(NULL, tmp, CSIDL_PROFILE, TRUE)) {
		szPath = (char *)SAlloc::M(strlen(tmp) + 1);
		if(szPath == NULL) {
			return NULL;
		}
		strcpy(szPath, tmp);
...
int ssh_is_ipaddr_v4(const char * str) 
{
	...
	rc = WSAStringToAddressA((LPSTR)str, AF_INET, NULL, (struct sockaddr*)&ss, &sslen);
	if(rc == 0) {
		return 1;
	}
	return 0;
 ...
 */
void ssh_print_hexa(const char * descr, const uchar * what, size_t len) 
{
...
{
	struct ssh_iterator * iterator = (struct ssh_iterator *)SAlloc::M(sizeof(struct ssh_iterator));
	if(!iterator)
		return NULL;
	iterator->next = NULL;
	iterator->data = data;
...
int ssh_analyze_banner(ssh_session session, int server)
{
	...
	if(openssh) {
		...
		if(strlen(openssh) > 9) {
			...
			if((tmp == (openssh + 8)) || ((errno == ERANGE) && (major == ULONG_MAX)) || ((errno != 0) && (major == 0)) || ((major < 1) || (major > 100))) {
				/* invalid major */
				goto done;
			}
			minor = strtoul(openssh + 10, &tmp, 10);
			if((tmp == (openssh + 10)) || ((errno == ERANGE) && (major == ULONG_MAX)) || ((errno != 0) && (major == 0)) || (minor > 100)) {
				/* invalid minor */
				goto done;
			}
