...
{
	struct ssh_pcap_file_struct * pcap = (struct ssh_pcap_file_struct *)SAlloc::M(sizeof(struct ssh_pcap_file_struct));
	if(pcap == NULL) {
		return NULL;
	}
	ZERO_STRUCTP(pcap);
...
int ssh_pcap_file_write_packet(ssh_pcap_file pcap, ssh_buffer packet, uint32_t original_len)
{
	...
	err = ssh_buffer_allocate_size(header, sizeof(uint32_t) * 4 + ssh_buffer_get_len(packet));
	if(err < 0) {
		goto error;
	}
...
static int ssh_pcap_context_connect(ssh_pcap_context ctx)
{
	...
	struct sockaddr_in local;// = { .sin_family = AF_UNSPEC, };
	struct sockaddr_in remote;// = { .sin_family = AF_UNSPEC, };
	socket_t fd;
	socklen_t len;
	int rc;
	MEMSZERO(local);
 ...
 */
int ssh_pcap_context_write(ssh_pcap_context ctx, enum ssh_pcap_direction direction, void * data, uint32_t len, uint32_t origlen)
{
	ssh_buffer ip;
	int rc;
