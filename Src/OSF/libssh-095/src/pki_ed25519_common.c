...
{
	if(ssh_string_len(pubkey) != ED25519_KEY_LEN || ssh_string_len(privkey) != (2 * ED25519_KEY_LEN)) {
	...
	key->ed25519_privkey = (uint8 *)SAlloc::M(ED25519_KEY_LEN);
#else
	/* In the internal implementation, the private key is the concatenation of
	 * the private seed with the public key. */
	key->ed25519_privkey = (uint8 *)SAlloc::M(2 * ED25519_KEY_LEN);
#endif
	if(key->ed25519_privkey == NULL) {
		goto error;
	}
	key->ed25519_pubkey = (uint8 *)SAlloc::M(ED25519_KEY_LEN);
	if(key->ed25519_pubkey == NULL) {
		goto error;
	}
#ifdef HAVE_OPENSSL_ED25519
	memcpy(key->ed25519_privkey, ssh_string_data(privkey), ED25519_KEY_LEN);
#else
...
int pki_ed25519_key_dup(ssh_key pNew, const ssh_key key)
{
	...
	if(key->ed25519_privkey != NULL) {
		...
		/* In OpenSSL implementation, the private key is the original private
		 * seed, without the public key. */
		pNew->ed25519_privkey = (uint8 *)SAlloc::M(ED25519_KEY_LEN);
#else
		/* In the internal implementation, the private key is the concatenation
		 * of the private seed with the public key. */
		pNew->ed25519_privkey = (uint8 *)SAlloc::M(2 * ED25519_KEY_LEN);
#endif
		if(pNew->ed25519_privkey == NULL) {
			return SSH_ERROR;
		}
#ifdef HAVE_OPENSSL_ED25519
		memcpy(pNew->ed25519_privkey, key->ed25519_privkey, ED25519_KEY_LEN);
#else
		memcpy(pNew->ed25519_privkey, key->ed25519_privkey, 2 * ED25519_KEY_LEN);
#endif
	}
	if(key->ed25519_pubkey != NULL) {
...
#else
	sig->ed25519_sig = SAlloc::M(ED25519_SIG_LEN);
	if(sig->ed25519_sig == NULL) {
		return SSH_ERROR;
	}
