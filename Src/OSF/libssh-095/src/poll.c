...
static int bsd_poll(ssh_pollfd_t * fds, nfds_t nfds, int timeout)
{
	...
	for(rc = -1, max_fd = 0, i = 0; i < nfds; i++) {
		...
		if(fds[i].fd > max_fd && (fds[i].events & (POLLIN|POLLOUT|POLLPRI|POLLRDNORM|POLLRDBAND|POLLWRNORM|POLLWRBAND))) {
			max_fd = fds[i].fd;
			rc = 0;
		}
	}
...
{
	ssh_poll_handle p = (ssh_poll_handle)SAlloc::M(sizeof(struct ssh_poll_handle_struct));
	if(p) {
		ZERO_STRUCTP(p);
		p->x.fd = fd;
		p->events = events;
...
{
	return p->ctx ? p->ctx->pollfds[p->x.idx].fd : p->x.fd;
}
/**
...
{
	ssh_poll_ctx ctx = (ssh_poll_ctx)SAlloc::M(sizeof(struct ssh_poll_ctx_struct));
	if(ctx) {
		ZERO_STRUCTP(ctx);
		SETIFZQ(chunk_size, SSH_POLL_CTX_CHUNK);
	...
	if(ctx) {
		if(ctx->polls_allocated > 0) {
			while(ctx->polls_used > 0) {
				...
				// The free function calls ssh_poll_ctx_remove() and decrements ctx->polls_used
				ssh_poll_free(p);
			}
			ZFREE(ctx->pollptrs);
			ZFREE(ctx->pollfds);
...
{
	ssh_pollfd_t * pollfds;
	ssh_poll_handle * pollptrs = (ssh_poll_handle *)SAlloc::R(ctx->pollptrs, sizeof(ssh_poll_handle) * new_size);
	if(pollptrs == NULL) {
		return -1;
	}
	ctx->pollptrs = pollptrs;
	pollfds = (ssh_pollfd_t *)SAlloc::R(ctx->pollfds, sizeof(ssh_pollfd_t) * new_size);
	if(pollfds == NULL) {
		pollptrs = (ssh_poll_handle *)SAlloc::R(ctx->pollptrs, sizeof(ssh_poll_handle) * ctx->polls_allocated);
		if(pollptrs == NULL) {
			return -1;
		}
		ctx->pollptrs = pollptrs;
...
int ssh_poll_ctx_add(ssh_poll_ctx ctx, ssh_poll_handle p) 
{
	...
	if(ctx->polls_used == ctx->polls_allocated && ssh_poll_ctx_resize(ctx, ctx->polls_allocated + ctx->chunk_size) < 0) {
		return -1;
	}
	fd = p->x.fd;
...
{
	ssh_event event = (ssh_event)SAlloc::M(sizeof(struct ssh_event_struct));
	if(event == NULL) {
		return NULL;
	}
	ZERO_STRUCTP(event);
...
static int ssh_event_fd_wrapper_callback(ssh_poll_handle p, socket_t fd, int revents, void * userdata) 
{
...
int ssh_event_add_fd(ssh_event event, socket_t fd, short events, ssh_event_callback cb, void * userdata) 
{
	...
	if(event == NULL || event->ctx == NULL || cb == NULL || fd == SSH_INVALID_SOCKET) {
		return SSH_ERROR;
	}
	pw = (struct ssh_event_fd_wrapper *)SAlloc::M(sizeof(struct ssh_event_fd_wrapper));
	if(pw == NULL) {
		return SSH_ERROR;
	}
	pw->cb = cb;
...
#ifdef WITH_SERVER
	for(iterator = ssh_list_get_iterator(event->sessions); iterator; iterator = iterator->next) {
...
int ssh_event_dopoll(ssh_event event, int timeout) 
{
	if(event == NULL || event->ctx == NULL) {
		...
		int rc = ssh_poll_ctx_dopoll(event->ctx, timeout);
		return rc;
	}
}
/**
...
int ssh_event_remove_session(ssh_event event, ssh_session session) 
{
	...
	for(i = 0; i < used; i++) {
		...
		if(p->session == session) {
			// ssh_poll_ctx_remove() decrements event->ctx->polls_used
			ssh_poll_ctx_remove(event->ctx, p);
			p->session = NULL;
			ssh_poll_ctx_add(session->default_poll_ctx, p);
			rc = SSH_OK;
			// Restart the loop! A session can initially have two pollhandlers.
			used = event->ctx->polls_used;
			i = 0;
		}
	}
...
void FASTCALL ssh_event_free(ssh_event event)
{
	if(event) {
		if(event->ctx) {
			size_t used = event->ctx->polls_used;
