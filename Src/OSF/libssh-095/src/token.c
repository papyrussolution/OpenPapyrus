...
struct ssh_tokens_st * ssh_tokenize(const char * chain, char separator)
{
	...
	tokens->tokens = (char **)SAlloc::C(num_tokens + 1, sizeof(char *));
	if(tokens->tokens == NULL) {
		goto error;
	}
 ...
 * NULL otherwise
 */
char * ssh_find_all_matching(const char * available_list, const char * preferred_list)
{
	struct ssh_tokens_st * a_tok = NULL, * p_tok = NULL;
	int i, j;
	char * ret = NULL;
...
char * ssh_remove_duplicates(const char * list)
{
	...
	should_copy = (bool *)SAlloc::C(1, max_len);
	if(should_copy == NULL) {
		goto out;
	}
    ...
    const char * appended_list)
{
	...
	concat_len += 2;
	concat = (char *)SAlloc::C(1, concat_len);
	if(concat == NULL) {
		return NULL;
	}
	if(list) {
