...
static void socket_callback_connected(int code, int errno_code, void * user)
{
	...
	if(session->session_state != SSH_SESSION_STATE_CONNECTING &&
		...
		ssh_set_error(session, SSH_FATAL, "Wrong state in socket_callback_connected : %d", session->session_state);
		return;
	}
...
int ssh_send_banner(ssh_session session, int server)
{
	...
	if(server == 1) {
		if(session->opts.custombanner == NULL) {
			session->serverbanner = sstrdup(banner);
			...
			session->serverbanner = (char *)SAlloc::M(len + 8 + 1);
			if(session->serverbanner == NULL) {
				goto end;
			}
			snprintf(session->serverbanner, len + 8 + 1, "SSH-2.0-%s", session->opts.custombanner);
		}
		...
		snprintf(buffer, sizeof(buffer), "%s%s", session->clientbanner, terminator);
	}
...
static int ssh_service_request_termination(void * s)
{
	...
	if(session->session_state == SSH_SESSION_STATE_ERROR || session->auth.service_state != SSH_AUTH_SERVICE_SENT)
		return 1;
	else
		return 0;
}
...
int ssh_service_request(ssh_session session, const char * service) 
{
	...
	if(session->auth.service_state != SSH_AUTH_SERVICE_NONE)
		goto pending;
	rc = ssh_buffer_pack(session->out_buffer, "bs", SSH2_MSG_SERVICE_REQUEST, service);
	if(rc != SSH_OK) {
...
pending:
	rc = ssh_handle_packets_termination(session, SSH_TIMEOUT_USER, ssh_service_request_termination, session);
	if(rc == SSH_ERROR) {
		return SSH_ERROR;
	}
...
static void ssh_client_connection_callback(ssh_session session)
{
	...
	switch(session->session_state) {
		...
		case SSH_SESSION_STATE_BANNER_RECEIVED:
		    ...
		    SSH_LOG(SSH_LOG_PROTOCOL, "SSH server banner: %s", session->serverbanner);
		    /* Here we analyze the different protocols the server allows. */
		    rc = ssh_analyze_banner(session, 0);
		    if(rc < 0) {
			    ssh_set_error(session, SSH_FATAL, "No version of SSH protocol usable (banner: %s)", session->serverbanner);
			    goto error;
		    }
...
static int ssh_connect_termination(void * user)
{
	...
	switch(session->session_state) {
		case SSH_SESSION_STATE_ERROR:
		case SSH_SESSION_STATE_AUTHENTICATING:
		case SSH_SESSION_STATE_DISCONNECTED: return 1;
		default: return 0;
	}
}
/**
...
int ssh_connect(ssh_session session)
{
	...
	session->alive = 0;
	session->client = 1;
	if(session->opts.fd == SSH_INVALID_SOCKET && session->opts.host == NULL && session->opts.ProxyCommand == NULL) {
	...
	if(session->opts.fd != SSH_INVALID_SOCKET) {
		...
		ret = ssh_socket_connect_proxycommand(session->socket, session->opts.ProxyCommand);
#endif
	}
	else {
		ret = ssh_socket_connect(session->socket, session->opts.host, session->opts.port > 0 ? session->opts.port : 22, session->opts.bindaddr);
	}
	if(ret == SSH_ERROR) {
		return SSH_ERROR;
	}
...
pending:
	...
	if(ssh_is_blocking(session)) {
		int timeout = (session->opts.timeout * 1000) + (session->opts.timeout_usec / 1000);
		SETIFZ(timeout, 10 * 1000);
		...
		if(session->session_state != SSH_SESSION_STATE_ERROR && (ret == SSH_ERROR || !ssh_connect_termination(session))) {
			ssh_set_error(session, SSH_FATAL, "Timeout connecting to %s", session->opts.host);
			session->session_state = SSH_SESSION_STATE_ERROR;
		}
	}
	else {
		ret = ssh_handle_packets_termination(session, SSH_TIMEOUT_NONBLOCKING, ssh_connect_termination, session);
		if(ret == SSH_ERROR) {
			session->session_state = SSH_SESSION_STATE_ERROR;
		}
	}
...
{
	return (session && session->banner) ? ssh_string_to_char(session->banner) : 0;
}
/**
