...
static void buffer_verify(ssh_buffer buf)
{
	...
	if(buf->data) {
		if(buf->used > buf->allocated) {
			slfprintf_stderr("BUFFER ERROR: allocated %zu, used %zu\n", buf->allocated, buf->used);
			do_abort = true;
		}
		if(buf->pos > buf->used) {
			slfprintf_stderr("BUFFER ERROR: position %zu, used %zu\n", buf->pos, buf->used);
			do_abort = true;
		}
		if(buf->pos > buf->allocated) {
			slfprintf_stderr("BUFFER ERROR: position %zu, allocated %zu\n", buf->pos, buf->allocated);
			do_abort = true;
		}
		if(do_abort) {
			abort();
...
{
	int rc = ssh_buffer_add_data(buffer, &data, sizeof(data));
	if(rc < 0) {
		return -1;
	}
	return 0;
 ...
 * @return              0 on success, -1 on error.
 */
int ssh_buffer_add_u8(struct ssh_buffer_struct * buffer, uint8 data)
{
	int rc = ssh_buffer_add_data(buffer, &data, sizeof(uint8));
	if(rc < 0) {
		return -1;
	}
	return 0;
...
{
	int rc = ssh_buffer_add_data(buffer, ssh_buffer_get(source), ssh_buffer_get_len(source));
	if(rc < 0) {
		return -1;
	}
	return 0;
 ...
 */
void * FASTCALL ssh_buffer_get(struct ssh_buffer_struct * buffer)
{
 ...
 * @see ssh_buffer_get()
 */
uint32_t FASTCALL ssh_buffer_get_len(const struct ssh_buffer_struct * buffer)
{
	buffer_verify(buffer);
 ...
 */
uint32_t FASTCALL ssh_buffer_get_data(struct ssh_buffer_struct * buffer, void * data, uint32_t len)
{
	/*
	 * Check for a integer overflow first, then check if not enough data is in
	 * the buffer.
	 */
	int rc = ssh_buffer_validate_length(buffer, len);
	if(rc != SSH_OK) {
		return 0;
	}
 ...
 * @internal
 *
 * @brief Get a 8 bits uint out of the buffer and adjusts the read
 * pointer.
 *
 * @param[in]  buffer   The buffer to read.
 *
 * @param[in]  data     A pointer to a uint8 where to store the data.
 *
 ...
 */
int FASTCALL ssh_buffer_validate_length(const struct ssh_buffer_struct * buffer, size_t len)
{
...
int ssh_buffer_pack_va(struct ssh_buffer_struct * buffer, const char * format, size_t argc, va_list ap)
{
	...
	for(p = format, count = 0; *p != '\0'; p++, count++) {
		...
		switch(*p) {
			case 'b':
			    o.byte = (uint8)va_arg(ap, uint);
			    rc = ssh_buffer_add_u8(buffer, o.byte);
			    break;
			case 'w':
			    o.word = (uint16_t)va_arg(ap, uint);
			    o.word = htons(o.word);
...
cleanup:
	...
	if(rc != SSH_OK) {
		...
		for(p = format; p<last; ++p) {
			switch(*p) {
				case 'b':
				    o.byte = va_arg(ap_copy, uint8 *);
				    if(buffer->secure) {
					    memzero(o.byte, sizeof(uint8));
					    break;
				    }
				    break;
				case 'w':
				    ...
				    if(buffer->secure) {
					    memzero(o.word, sizeof(uint16_t));
					    break;
				    }
				    break;
				case 'd':
				    ...
				    if(buffer->secure) {
					    memzero(o.dword, sizeof(uint32_t));
					    break;
				    }
				    break;
				case 'q':
				    ...
				    if(buffer->secure) {
					    memzero(o.qword, sizeof(uint64_t));
					    break;
				    }
				    break;
				case 'B':
