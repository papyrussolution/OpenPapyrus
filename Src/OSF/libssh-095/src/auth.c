...
static int ssh_userauth_request_service(ssh_session session) 
{
	...
	if((rc != SSH_OK) && (rc != SSH_AGAIN)) {
		SSH_LOG(SSH_LOG_WARN, "Failed to request \"ssh-userauth\" service");
	}
	return rc;
}
...
static const char * ssh_auth_get_current_method(ssh_session session)
{
	...
	switch(session->auth.current_method) {
		case SSH_AUTH_METHOD_NONE: method = "none"; break;
		case SSH_AUTH_METHOD_PASSWORD: method = "password"; break;
		case SSH_AUTH_METHOD_PUBLICKEY: method = "publickey"; break;
		case SSH_AUTH_METHOD_HOSTBASED: method = "hostbased"; break;
		case SSH_AUTH_METHOD_INTERACTIVE: method = "keyboard interactive"; break;
		case SSH_AUTH_METHOD_GSSAPI_MIC: method = "gssapi"; break;
		default: break;
	}
	return method;
}
...
{
	int rc = ssh_handle_packets_termination(session, SSH_TIMEOUT_USER, ssh_auth_response_termination, session);
	if(rc == SSH_ERROR)
		return SSH_AUTH_ERROR;
	else if(!ssh_auth_response_termination(session))
		return SSH_AUTH_AGAIN;
	else {
		switch(session->auth.state) {
			case SSH_AUTH_STATE_ERROR: rc = SSH_AUTH_ERROR; break;
			case SSH_AUTH_STATE_FAILED: rc = SSH_AUTH_DENIED; break;
			case SSH_AUTH_STATE_INFO: rc = SSH_AUTH_INFO; break;
			case SSH_AUTH_STATE_PARTIAL: rc = SSH_AUTH_PARTIAL; break;
			case SSH_AUTH_STATE_PK_OK:
			case SSH_AUTH_STATE_SUCCESS: rc = SSH_AUTH_SUCCESS; break;
			case SSH_AUTH_STATE_KBDINT_SENT:
			case SSH_AUTH_STATE_GSSAPI_REQUEST_SENT:
			case SSH_AUTH_STATE_GSSAPI_TOKEN:
			case SSH_AUTH_STATE_GSSAPI_MIC_SENT:
			...
			case SSH_AUTH_STATE_PASSWORD_AUTH_SENT:
			case SSH_AUTH_STATE_AUTH_NONE_SENT:
			case SSH_AUTH_STATE_NONE: rc = SSH_AUTH_ERROR; break; /* not reached */
		}
		return rc;
	}
}
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_banner) 
{
	(void)type;
	(void)user;
	ssh_string banner = ssh_buffer_get_ssh_string(packet);
	if(banner == NULL) {
		SSH_LOG(SSH_LOG_WARN, "Invalid SSH_USERAUTH_BANNER packet");
	}
	else {
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_failure) 
{
	...
	if(rc != SSH_OK) {
		ssh_set_error(session, SSH_FATAL, "Invalid SSH_MSG_USERAUTH_FAILURE message");
		session->auth.state = SSH_AUTH_STATE_ERROR;
		goto end;
	}
	if(partial) {
		session->auth.state = SSH_AUTH_STATE_PARTIAL;
		SSH_LOG(SSH_LOG_INFO, "Partial success for '%s'. Authentication that can continue: %s", current_method, auth_methods);
	}
	else {
		session->auth.state = SSH_AUTH_STATE_FAILED;
		ssh_set_error(session, SSH_REQUEST_DENIED, "Access denied for '%s'. Authentication that can continue: %s", current_method, auth_methods);
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_pk_ok) 
{
	...
	if(session->auth.state == SSH_AUTH_STATE_KBDINT_SENT) {
		...
		SSH_LOG(SSH_LOG_TRACE, "keyboard-interactive context, assuming SSH_USERAUTH_INFO_REQUEST");
...
int ssh_userauth_none(ssh_session session, const char * username) 
{
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsss", SSH2_MSG_USERAUTH_REQUEST, username ? username : session->opts.username, "ssh-connection", "none");
	if(rc < 0) {
		goto fail;
	}
    ...
    const ssh_key pubkey)
{
	...
	if(sig_type_c == NULL) {
		ssh_set_error(session, SSH_REQUEST_DENIED, "Invalid key type (unknown)");
		return SSH_AUTH_DENIED;
	}
	if(!ssh_key_algorithm_allowed(session, sig_type_c)) {
		ssh_set_error(session, SSH_REQUEST_DENIED, "The key algorithm '%s' is not allowed to be used by PUBLICKEY_ACCEPTED_TYPES configuration option", sig_type_c);
		return SSH_AUTH_DENIED;
	}
	rc = ssh_userauth_request_service(session);
	if(rc == SSH_AGAIN) {
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsssbsS", SSH2_MSG_USERAUTH_REQUEST,
		username ? username : session->opts.username, "ssh-connection", "publickey",
		0, /* private key ? */ sig_type_c, /* algo */ pubkey_s /* public key */);
	if(rc < 0) {
		goto fail;
	}
...
int ssh_userauth_publickey(ssh_session session, const char * username, const ssh_key privkey)
{
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsssbsS", SSH2_MSG_USERAUTH_REQUEST, username ? username : session->opts.username,
		"ssh-connection", "publickey", 1, /* private key */ sig_type_c, /* algo */ str /* public key or cert */);
	if(rc < 0) {
		goto fail;
	}
	SSH_STRING_FREE(str);
    ...
    ssh_key pubkey)
{
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsssbsS", SSH2_MSG_USERAUTH_REQUEST, username ? username : session->opts.username,
		"ssh-connection", "publickey", 1, /* private key */ sig_type_c, /* algo */ pubkey_s /* public key */);
	SSH_STRING_FREE(pubkey_s);
	if(rc < 0) {
		goto fail;
	}
...
int ssh_userauth_agent(ssh_session session,
	...
	if(!session->agent_state) {
		session->agent_state = SAlloc::M(sizeof(struct ssh_agent_state_struct));
		if(!session->agent_state) {
			ssh_set_error_oom(session);
			return SSH_AUTH_ERROR;
		}
	...
	while(state->pubkey != NULL) {
		...
		if(state->state == SSH_AGENT_STATE_NONE ||
			...
			if(rc == SSH_AUTH_ERROR) {
				...
				SSH_LOG(SSH_LOG_DEBUG, "Public key of %s refused by server", state->comment);
				SSH_STRING_FREE_CHAR(state->comment);
				state->comment = NULL;
				ssh_key_free(state->pubkey);
			...
			SSH_LOG(SSH_LOG_DEBUG, "Public key of %s accepted by server", state->comment);
		...
		if(state->state == SSH_AGENT_STATE_AUTH) {
			...
			if(rc == SSH_AUTH_ERROR || rc == SSH_AUTH_PARTIAL) {
				...
				SSH_LOG(SSH_LOG_INFO, "Server accepted public key but refused the signature");
				ssh_key_free(state->pubkey);
				state->pubkey = ssh_agent_get_next_ident(session, &state->comment);
				state->state = SSH_AGENT_STATE_NONE;
				continue;
...
int ssh_userauth_publickey_auto(ssh_session session, const char * username, const char * passphrase)
{
	...
	if(!session->auth.auto_state) {
		session->auth.auto_state = (struct ssh_auth_auto_state_struct *)SAlloc::C(1, sizeof(struct ssh_auth_auto_state_struct));
		if(!session->auth.auto_state) {
			ssh_set_error_oom(session);
			return SSH_AUTH_ERROR;
		}
	...
	while(state->it != NULL) {
		...
		if(state->state == SSH_AUTH_AUTO_STATE_PUBKEY) {
			SSH_LOG(SSH_LOG_DEBUG, "Trying to authenticate with %s", privkey_file);
			state->privkey = NULL;
			state->pubkey = NULL;
			snprintf(pubkey_file, sizeof(pubkey_file), "%s.pub", privkey_file);
			rc = ssh_pki_import_pubkey_file(pubkey_file, &state->pubkey);
			if(rc == SSH_ERROR) {
				ssh_set_error(session, SSH_FATAL, "Failed to import public key: %s", pubkey_file);
				ZFREE(session->auth.auto_state);
				return SSH_AUTH_ERROR;
			}
			else if(rc == SSH_EOF) {
				/* Read the private key and save the public key to file */
				rc = ssh_pki_import_privkey_file(privkey_file, passphrase, auth_fn, auth_data, &state->privkey);
				if(rc == SSH_ERROR) {
					ssh_set_error(session, SSH_FATAL, "Failed to read private key: %s", privkey_file);
					state->it = state->it->next;
					continue;
				}
				else if(rc == SSH_EOF) {
					/* If the file doesn't exist, continue */
					SSH_LOG(SSH_LOG_DEBUG, "Private key %s doesn't exist.", privkey_file);
					state->it = state->it->next;
					continue;
				}
				...
				if(rc == SSH_ERROR) {
					SSH_LOG(SSH_LOG_WARN, "Could not write public key to file: %s", pubkey_file);
				}
			}
		...
		if(state->state == SSH_AUTH_AUTO_STATE_KEY_IMPORTED) {
			rc = ssh_userauth_try_publickey(session, username, state->pubkey);
			if(rc == SSH_AUTH_ERROR) {
				SSH_LOG(SSH_LOG_WARN, "Public key authentication error for %s", privkey_file);
				ssh_key_free(state->privkey);
				state->privkey = NULL;
				ssh_key_free(state->pubkey);
				state->pubkey = NULL;
				...
				SSH_LOG(SSH_LOG_DEBUG, "Public key for %s refused by server", privkey_file);
				ssh_key_free(state->privkey);
				state->privkey = NULL;
				ssh_key_free(state->pubkey);
				state->pubkey = NULL;
		...
		if(state->state == SSH_AUTH_AUTO_STATE_PUBKEY_ACCEPTED) {
			...
			if(state->privkey == NULL) {
				...
				if(rc == SSH_ERROR) {
					...
					ssh_key_free(state->pubkey);
					state->pubkey = NULL;
					SSH_LOG(SSH_LOG_INFO, "Private key %s doesn't exist.", privkey_file);
					state->it = state->it->next;
					state->state = SSH_AUTH_AUTO_STATE_PUBKEY;
					continue;
				}
			...
			if(rc != SSH_AUTH_AGAIN && rc != SSH_AUTH_DENIED) {
				...
				if(rc == SSH_AUTH_SUCCESS) {
					SSH_LOG(SSH_LOG_INFO, "Successfully authenticated using %s", privkey_file);
				}
				return rc;
			}
			if(rc == SSH_AUTH_AGAIN) {
			...
			ssh_key_free(state->privkey);
			ssh_key_free(state->pubkey);
			SSH_LOG(SSH_LOG_WARN, "The server accepted the public key but refused the signature");
			state->it = state->it->next;
			state->state = SSH_AUTH_AUTO_STATE_PUBKEY;
			/* continue */
		}
	}
	SSH_LOG(SSH_LOG_INFO, "Tried every public key, none matched");
	ZFREE(session->auth.auto_state);
	return SSH_AUTH_DENIED;
}
...
int ssh_userauth_password(ssh_session session, const char * username, const char * password) 
{
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsssbs", SSH2_MSG_USERAUTH_REQUEST, username ? username : session->opts.username,
		"ssh-connection", "password", 0, /* false */ password);
	if(rc < 0) {
		goto fail;
	}
...
{
	ssh_kbdint kbd = (ssh_kbdint)SAlloc::C(1, sizeof(struct ssh_kbdint_struct));
	if(kbd == NULL) {
		return NULL;
	}
	return kbd;
 ...
 */
static int ssh_userauth_kbdint_init(ssh_session session, const char * username, const char * submethods)
{
	int rc;
	...
	/* request */
	rc = ssh_buffer_pack(session->out_buffer, "bsssss", SSH2_MSG_USERAUTH_REQUEST,
		username ? username : session->opts.username, "ssh-connection", "keyboard-interactive", "", /* lang (ignore it) */ submethods ? submethods : "");
	if(rc < 0) {
		goto fail;
	}
	session->auth.state = SSH_AUTH_STATE_KBDINT_SENT;
	session->pending_call_state = SSH_PENDING_CALL_AUTH_KBDINT_INIT;
	SSH_LOG(SSH_LOG_DEBUG, "Sending keyboard-interactive init request");
	rc = ssh_packet_send(session);
	if(rc == SSH_ERROR) {
		return SSH_AUTH_ERROR;
	}
...
static int ssh_userauth_kbdint_send(ssh_session session)
{
	...
	session->kbdint = NULL;
	SSH_LOG(SSH_LOG_DEBUG, "Sending keyboard-interactive response packet");
	rc = ssh_packet_send(session);
	if(rc == SSH_ERROR) {
		return SSH_AUTH_ERROR;
	}
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_info_request) 
{
	...
	SSH_LOG(SSH_LOG_DEBUG, "%d keyboard-interactive prompts", nprompts);
	if(nprompts > KBDINT_MAX_PROMPT) {
		ssh_set_error(session, SSH_FATAL, "Too much prompts requested by the server: %u (0x%.4x)", nprompts, nprompts);
		ssh_kbdint_free(session->kbdint);
		session->kbdint = NULL;
	...
	session->kbdint->prompts = (char **)SAlloc::C(nprompts, sizeof(char *));
	...
	session->kbdint->echo = (uchar *)SAlloc::C(nprompts, sizeof(uchar));
...
{
	if(session == NULL || session->kbdint == NULL || session->kbdint->answers == NULL) {
		return NULL;
	}
...
int ssh_userauth_kbdint_setanswer(ssh_session session, uint i, const char * answer) 
{
	...
	if(session->kbdint->answers == NULL) {
		session->kbdint->answers = (char **)SAlloc::C(session->kbdint->nprompts, sizeof(char *));
		if(session->kbdint->answers == NULL) {
			ssh_set_error_oom(session);
			return -1;
		}
