...
int ssh_client_ecdh_init(ssh_session session)
{
	...
	int rc = ssh_buffer_add_u8(session->out_buffer, SSH2_MSG_KEX_ECDH_INIT);
	if(rc < 0) {
		BN_CTX_free(ctx);
		return SSH_ERROR;
	}
	...
	len = EC_POINT_point2oct(group, pubkey, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx);
	...
	EC_POINT_point2oct(group, pubkey, POINT_CONVERSION_UNCOMPRESSED, static_cast<uchar *>(ssh_string_data(client_pubkey)), len, ctx);
	BN_CTX_free(ctx);
...
int ecdh_build_k(ssh_session session) 
{
	...
	if(session->server) {
		rc = EC_POINT_oct2point(group, pubkey,
			static_cast<const uchar *>(ssh_string_data(session->next_crypto->ecdh_client_pubkey)),
			ssh_string_len(session->next_crypto->ecdh_client_pubkey), ctx);
	}
	else {
		rc = EC_POINT_oct2point(group, pubkey,
			static_cast<const uchar *>(ssh_string_data(session->next_crypto->ecdh_server_pubkey)),
	...
	rc = ECDH_compute_key(buffer, len, pubkey, session->next_crypto->ecdh_privkey, NULL);
	EC_POINT_clear_free(pubkey);
	if(rc <= 0) {
		SAlloc::F(buffer);
		return -1;
	}
	bignum_bin2bn(static_cast<const uchar *>(buffer), len, &session->next_crypto->shared_secret);
	SAlloc::F(buffer);
...
SSH_PACKET_CALLBACK(ssh_packet_server_ecdh_init)
{
	...
	len = EC_POINT_point2oct(group, ecdh_pubkey, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx);
	...
	EC_POINT_point2oct(group, ecdh_pubkey, POINT_CONVERSION_UNCOMPRESSED, static_cast<uchar *>(ssh_string_data(q_s_string)), len, ctx);
	BN_CTX_free(ctx);
	...
	rc = ssh_buffer_pack(session->out_buffer, "bSSS", SSH2_MSG_KEXDH_REPLY, pubkey_blob/* host's pubkey */,
		q_s_string/* ecdh public key */, sig_blob/* signature blob */);
	SSH_STRING_FREE(sig_blob);
