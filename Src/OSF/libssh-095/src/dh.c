...
int ssh_dh_init()
{
	ulong g_int = 2; /* G is defined as 2 by the ssh2 standards */
	int rc;
...
{
	return ssh_pki_import_pubkey_blob(pubkey_blob, &session->next_crypto->server_pubkey);
}

static SSH_PACKET_CALLBACK(ssh_packet_client_dh_reply);

static ssh_packet_callback dh_client_callbacks[] = { ssh_packet_client_dh_reply };
static struct ssh_packet_callbacks_struct ssh_dh_client_callbacks = { SSH2_MSG_KEXDH_REPLY, 1, dh_client_callbacks, NULL };
...
int ssh_client_dh_init(ssh_session session)
{
	...
	const_bignum pubkey;
	int rc = ssh_dh_init_common(crypto);
	if(rc == SSH_ERROR) {
		goto error;
	}
...
static ssh_packet_callback dh_server_callbacks[] = { ssh_packet_server_dh_init, };
static struct ssh_packet_callbacks_struct ssh_dh_server_callbacks = { SSH2_MSG_KEXDH_INIT, 1, dh_server_callbacks, NULL };
...
int ssh_server_dh_process_init(ssh_session session, ssh_buffer packet)
{
	...
	int packet_type;
	int rc = ssh_buffer_unpack(packet, "B", &client_pubkey);
	if(rc == SSH_ERROR) {
 ...
 */
int ssh_fallback_group(uint32_t pmax, bignum * modulus, bignum * generator)
{
	*modulus = NULL;
	*generator = NULL;
	if(pmax < 3072) {
...
bool ssh_dh_is_known_group(bignum modulus, bignum generator)
{
	int cmp;
	bignum m = NULL;
	int bits = bignum_num_bits(modulus);
	if(bits < 3072) {
		m = ssh_dh_group14;
	}
	else if(bits < 6144) {
...
{
	return session->current_crypto ? session->current_crypto->server_pubkey : 0;
}
 ...
 * @see ssh_string_free_char()
 */
char * ssh_get_fingerprint_hash(enum ssh_publickey_hash_type type, uchar * hash, size_t len)
{
	...
	switch(type) {
		...
		case SSH_PUBLICKEY_HASH_SHA256: fingerprint = ssh_get_b64_unpadded(hash, len); break;
		case SSH_PUBLICKEY_HASH_MD5: fingerprint = ssh_get_hexa(hash, len); break;
	}
	if(fingerprint == NULL) {
		return NULL;
	}
	switch(type) {
		case SSH_PUBLICKEY_HASH_MD5: prefix = "MD5"; break;
		case SSH_PUBLICKEY_HASH_SHA1: prefix = "SHA1"; break;
		case SSH_PUBLICKEY_HASH_SHA256: prefix = "SHA256"; break;
	}
 ...
 */
void ssh_print_hash(enum ssh_publickey_hash_type type, uchar * hash, size_t len)
{
