...
{
	uint x = a;
	x -= (uint)b; /* 0..65535: yes; 4294901761..4294967295: no */
	x >>= 31; /* 0: yes; 1: no */
	x ^= 1; /* 1: yes; 0: no */
	return x;
}
...
void fe25519_unpack(fe25519 * r, const uchar x[32])
{
	for(int i = 0; i < 32; i++) {
		r->v[i] = x[i];
	}
...
void fe25519_pack(uchar r[32], const fe25519 * x)
{
	fe25519 y = *x;
	fe25519_freeze(&y);
	for(int i = 0; i < 32; i++) {
...
void fe25519_cmov(fe25519 * r, const fe25519 * x, uchar b)
{
	uint32_t mask = b;
	mask = -mask;
	for(int i = 0; i < 32; i++) {
...
uchar fe25519_getparity(const fe25519 * x)
{
	fe25519 t = *x;
	fe25519_freeze(&t);
	return t.v[0] & 1;
