...
static void bcrypt_hash(uint8 * sha2pass, uint8 * sha2salt, uint8 * out)
{
	ssh_blf_ctx state;
	uint8 ciphertext[/*BCRYPT_HASHSIZE*/] = "OxychromaticBlowfishSwatDynamite";
...
int bcrypt_pbkdf(const char * pass, size_t passlen, const uint8 * salt, size_t saltlen,
    uint8 * key, size_t keylen, uint rounds)
{
	uint8 sha2pass[SHA512_DIGEST_LENGTH];
	uint8 sha2salt[SHA512_DIGEST_LENGTH];
	uint8 out[BCRYPT_HASHSIZE];
	uint8 tmpout[BCRYPT_HASHSIZE];
	uint8 * countsalt;
	...
	if(passlen == 0 || saltlen == 0 || keylen == 0 ||
		...
		return -1;
	if((countsalt = (uint8 *)SAlloc::C(1, saltlen + 4)) == NULL)
		return -1;
