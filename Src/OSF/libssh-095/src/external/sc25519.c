...
/*Arithmetic modulo the group order m = 2^252 +  27742317777372353535851937790883648493 =
...
static uint32_t lt(uint32_t a, uint32_t b) /* 16-bit inputs */
{
	...
	x -= (uint)b; /* 0..65535: no; 4294901761..4294967295: yes */
	x >>= 31; /* 0: no; 1: yes */
...
void sc25519_2interleave2(uchar r[127],
    const sc25519 * s1,
    const sc25519 * s2)
{
	int i;
