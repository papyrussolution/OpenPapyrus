 ...
 * @{
 */
static int hash_hostname(const char * name, uchar * salt, uint salt_size, uchar ** hash, uint * hash_size)
{
	HMACCTX mac_ctx = hmac_init(salt, salt_size, SSH_HMAC_SHA1);
	if(mac_ctx == NULL) {
		return SSH_ERROR;
	}
...
static int ssh_known_hosts_entries_compare(struct ssh_knownhosts_entry * k1,
    ...
    struct ssh_list ** entries)
{
	...
	for(rc = known_hosts_read_line(fp, line, sizeof(line), &len, &lineno);
		...
		for(it = ssh_list_get_iterator(*entries); it; it = it->next) {
...
struct ssh_list * ssh_known_hosts_get_algorithms(ssh_session session)
{
	...
	for(it = ssh_list_get_iterator(entry_list); it; it = ssh_list_get_iterator(entry_list)) {
		...
		/* Check for duplicates */
		for(it2 = ssh_list_get_iterator(list); it2; it2 = it2->next) {
...
int ssh_known_hosts_parse_line(const char * hostname, const char * line, struct ssh_knownhosts_entry ** entry)
{
	...
	rc = ssh_pki_import_pubkey_base64(p, key_type, &e->publickey);
	if(rc != SSH_OK) {
		SSH_LOG(SSH_LOG_WARN, "Failed to parse %s key for entry: %s!", ssh_key_type_to_char(key_type), e->unparsed);
		goto out;
	}
	/* comment */
	p = strtok(NULL, " ");
...
static enum ssh_known_hosts_e ssh_known_hosts_check_server_key(const char * hosts_entry, const char * filename, ssh_key server_key, struct ssh_knownhosts_entry ** pentry)
{
	struct ssh_list * entry_list = NULL;
	struct ssh_iterator * it = NULL;
