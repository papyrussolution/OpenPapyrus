...
int server_set_kex(ssh_session session)
{
	...
	if(session->srv.ed25519_key != NULL) {
		snprintf(hostkeys, sizeof(hostkeys), "%s", ssh_key_type_to_char(ssh_key_type(session->srv.ed25519_key)));
	}
#ifdef HAVE_ECC
	if(session->srv.ecdsa_key != NULL) {
		len = strlen(hostkeys);
		snprintf(hostkeys + len, sizeof(hostkeys) - len, ",%s", session->srv.ecdsa_key->type_c);
	}
#endif
#ifdef HAVE_DSA
	...
	// It is expected for the list of allowed hostkeys to be ordered by preference 
	kept = ssh_find_all_matching(hostkeys[0] == ',' ? hostkeys + 1 : hostkeys, allowed);
	if(kept == NULL) {
		/* Nothing was allowed */
		return -1;
	}
...
static int ssh_server_send_extensions(ssh_session session) 
{
	...
	rc = ssh_buffer_pack(session->out_buffer, "bdss", SSH2_MSG_EXT_INFO, 1/* nr. of extensions */, "server-sig-algs", hostkey_algorithms);
	if(rc != SSH_OK) {
		goto error;
	}
...
static int ssh_server_kex_termination(void * s)
{
	...
	if(session->session_state != SSH_SESSION_STATE_ERROR && session->session_state != SSH_SESSION_STATE_AUTHENTICATING && session->session_state != SSH_SESSION_STATE_DISCONNECTED)
		return 0;
	else
		return 1;
}
...
int ssh_message_auth_interactive_request(ssh_message msg, const char * name,
    const char * instruction, uint num_prompts,
	...
	if(num_prompts > 0) {
		msg->session->kbdint->prompts = (char **)SAlloc::C(num_prompts, sizeof(char *));
		...
		msg->session->kbdint->echo = (uchar *)SAlloc::C(num_prompts, sizeof(uchar));
...
int ssh_message_auth_reply_pk_ok_simple(ssh_message msg) 
{
	...
	int ret;
	ssh_string algo = ssh_string_from_char(msg->auth_request.pubkey->type_c);
	if(algo == NULL) {
		return SSH_ERROR;
	}
