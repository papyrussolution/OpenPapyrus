...
ssh_public_key publickey_from_privatekey(ssh_private_key prv) 
{
	...
	privkey->type = (enum ssh_keytypes_e)prv->type;
...
ssh_private_key privatekey_from_file(ssh_session session, const char * filename, int type, const char * passphrase) 
{
	...
	rc = ssh_pki_import_privkey_file(filename, passphrase, auth_fn, auth_data, &key);
	if(rc == SSH_ERROR) {
		return NULL;
	}
	privkey = (ssh_private_key)SAlloc::M(sizeof(struct ssh_private_key_struct));
	if(privkey == NULL) {
		ssh_key_free(key);
		return NULL;
	}
...
enum ssh_keytypes_e ssh_privatekey_type(ssh_private_key privatekey)
{
	...
	return (enum ssh_keytypes_e)(privatekey->type);
}
...
ssh_public_key publickey_from_string(ssh_session session, ssh_string pubkey_s) 
{
	...
	pubkey = (struct ssh_public_key_struct *)SAlloc::M(sizeof(struct ssh_public_key_struct));
	if(pubkey == NULL) {
		ssh_key_free(key);
		return NULL;
	}
...
int ssh_publickey_to_file(ssh_session session, const char * file, ssh_string pubkey, int type)
{
	...
	pubkey_64 = bin_to_base64((const uint8 *)ssh_string_data(pubkey), ssh_string_len(pubkey));
	if(pubkey_64 == NULL) {
		return SSH_ERROR;
	}
