...
static sftp_ext sftp_ext_new() 
{
	return (sftp_ext)SAlloc::C(1, sizeof(struct sftp_ext_struct));
}
...
sftp_session sftp_new(ssh_session session)
{
	...
	sftp = (sftp_session)SAlloc::C(1, sizeof(struct sftp_session_struct));
	if(sftp == NULL) {
	...
	sftp->read_packet = (sftp_packet)SAlloc::C(1, sizeof(struct sftp_packet_struct));
...
int sftp_server_init(sftp_session sftp)
{
	...
	rc = ssh_buffer_pack(reply, "dssss", LIBSFTP_VERSION, "posix-rename@openssh.com", "1", "hardlink@openssh.com", "1");
	if(rc != SSH_OK) {
		ssh_set_error_oom(session);
...
ssize_t sftp_packet_write(sftp_session sftp, uint8 type, ssh_buffer payload)
{
	...
	/* Add size of type */
	payload_size = ssh_buffer_get_len(payload) + sizeof(uint8);
	...
	if((uint32_t)size != ssh_buffer_get_len(payload)) {
		SSH_LOG(SSH_LOG_PACKET, "Had to write %d bytes, wrote only %zd", ssh_buffer_get_len(payload), size);
	}
	return size;
}
...
{
	uint8 buffer[SFTP_BUFFER_SIZE_MAX];
	...
	do {
		...
		if(s < 0) {
			...
			if(is_eof) {
				ssh_set_error(sftp->session, SSH_FATAL, "Received EOF while reading sftp packet size");
	...
	do {
		...
		if(nread < 0) {
			...
			is_eof = ssh_channel_is_eof(sftp->channel);
			if(is_eof) {
				ssh_set_error(sftp->session, SSH_FATAL, "Received EOF while reading sftp packet type");
				sftp_set_error(sftp, SSH_FX_EOF);
				goto error;
			}
		}
	...
	while(size > 0 && size < SFTP_PACKET_SIZE_MAX) {
		nread = ssh_channel_read(sftp->channel, buffer, sizeof(buffer) > size ? size : sizeof(buffer), 0);
		if(nread < 0) {
		...
		if(nread > 0) {
			...
			is_eof = ssh_channel_is_eof(sftp->channel);
			if(is_eof) {
				ssh_set_error(sftp->session, SSH_REQUEST_DENIED, "Received EOF while reading sftp packet");
				sftp_set_error(sftp, SSH_FX_EOF);
				goto error;
			}
		}
...
static sftp_message sftp_get_message(sftp_packet packet)
{
	...
	msg = (sftp_message)SAlloc::C(1, sizeof(struct sftp_message_struct));
	if(!msg) {
		ssh_set_error_oom(sftp->session);
		sftp_set_error(packet->sftp, SSH_FX_FAILURE);
		return NULL;
	...
	SSH_LOG(SSH_LOG_PACKET, "Packet with id %d type %d", msg->id, msg->packet_type);
	return msg;
}
...
int sftp_init(sftp_session sftp) 
{
	...
	SSH_LOG(SSH_LOG_PROTOCOL, "SFTP server version %d", version);
...
{
	return (sftp && sftp->ext) ? sftp->ext->count : 0;
}

const char * sftp_extensions_get_name(sftp_session sftp, uint idx) 
{
	if(sftp == NULL)
		return NULL;
...
const char * sftp_extensions_get_data(sftp_session sftp, uint idx) 
{
	if(sftp == NULL)
		return NULL;
	if(sftp->ext == NULL || sftp->ext->name == NULL) {
...
int sftp_extension_supported(sftp_session sftp, const char * name, const char * data) 
{
	...
	for(i = 0; i < n; i++) {
		...
		if(ext_name && ext_data && strcmp(ext_name, name) == 0 && strcmp(ext_data, data) == 0) {
			return 1;
		}
	}
	return 0;
...
{
	sftp_request_queue queue = (sftp_request_queue)SAlloc::C(1, sizeof(struct sftp_request_queue_struct));
	if(queue == NULL) {
...
{
	sftp_request_queue ptr;
	sftp_request_queue queue = request_queue_new(msg);
	if(queue == NULL) {
		return -1;
	}
	SSH_LOG(SSH_LOG_PACKET, "Queued msg id %d type %d", msg->id, msg->packet_type);
	if(sftp->queue == NULL) {
		sftp->queue = queue;
	}
	else {
...
static sftp_message sftp_dequeue(sftp_session sftp, uint32_t id)
{
	...
	while(queue) {
		if(queue->message->id == id) {
			...
			SSH_LOG(SSH_LOG_PACKET, "Dequeued msg id %d type %d", msg->id, msg->packet_type);
			return msg;
		}
		prev = queue;
		queue = queue->next;
...
static sftp_status_message parse_status_msg(sftp_message msg)
{
	...
	status = (sftp_status_message)SAlloc::C(1, sizeof(struct sftp_status_message_struct));
	if(status == NULL) {
		ssh_set_error_oom(msg->sftp->session);
		sftp_set_error(msg->sftp, SSH_FX_FAILURE);
		return NULL;
...
static sftp_file parse_handle_msg(sftp_message msg)
{
	...
	file = (sftp_file)SAlloc::C(1, sizeof(struct sftp_file_struct));
	if(!file) {
		ssh_set_error_oom(msg->sftp->session);
		sftp_set_error(msg->sftp, SSH_FX_FAILURE);
		return NULL;
...
sftp_dir sftp_opendir(sftp_session sftp, const char * path)
{
	...
	rc = ssh_buffer_pack(payload, "ds", id, path);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
...
static sftp_attributes sftp_parse_attr_4(sftp_session sftp, ssh_buffer buf, int expectnames) 
{
	...
	(void)expectnames;
	attr = (sftp_attributes)SAlloc::C(1, sizeof(struct sftp_attributes_struct));
	if(attr == NULL) {
		ssh_set_error_oom(sftp->session);
		sftp_set_error(sftp, SSH_FX_FAILURE);
		return NULL;
	...
	do {
		...
		if(flags & SSH_FILEXFER_ATTR_PERMISSIONS) {
			...
			switch(attr->permissions & SSH_S_IFMT) {
				...
				case SSH_S_IFBLK:
				case SSH_S_IFCHR:
				case SSH_S_IFIFO: attr->type = SSH_FILEXFER_TYPE_SPECIAL; break;
				case SSH_S_IFLNK: attr->type = SSH_FILEXFER_TYPE_SYMLINK; break;
				case SSH_S_IFREG: attr->type = SSH_FILEXFER_TYPE_REGULAR; break;
				case SSH_S_IFDIR: attr->type = SSH_FILEXFER_TYPE_DIRECTORY; break;
				default: attr->type = SSH_FILEXFER_TYPE_UNKNOWN; break;
			}
		}
		...
		if(flags & SSH_FILEXFER_ATTR_EXTENDED) {
			...
			while(attr->extended_count && (attr->extended_type = ssh_buffer_get_ssh_string(buf)) && (attr->extended_data = ssh_buffer_get_ssh_string(buf))) {
				attr->extended_count--;
			}
...
static sftp_attributes sftp_parse_attr_3(sftp_session sftp, ssh_buffer buf, int expectname) 
{
	...
	if(expectname) {
		rc = ssh_buffer_unpack(buf, "ss", &attr->name, &attr->longname);
		if(rc != SSH_OK) {
			goto error;
		}
	...
	SSH_LOG(SSH_LOG_PROTOCOL, "Flags: %.8" PRIx32 "\n", (uint32_t)attr->flags);
	if(attr->flags & SSH_FILEXFER_ATTR_SIZE) {
		rc = ssh_buffer_unpack(buf, "q", &attr->size);
		if(rc != SSH_OK) {
			goto error;
		}
		SSH_LOG(SSH_LOG_PROTOCOL, "Size: %" PRIu64 "\n", (uint64_t)attr->size);
	}
...
int sftp_server_version(sftp_session sftp) { return sftp->server_version; }
...
sftp_attributes sftp_readdir(sftp_session sftp, sftp_dir dir)
{
	...
	if(dir->buffer == NULL) {
		...
		id = sftp_get_new_id(sftp);
		rc = ssh_buffer_pack(payload, "dS", id, dir->handle);
		if(rc) {
			ssh_set_error_oom(sftp->session);
			sftp_set_error(sftp, SSH_FX_FAILURE);
			SSH_BUFFER_FREE(payload);
...
static int sftp_handle_close(sftp_session sftp, ssh_string handle)
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "dS", id, handle);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
    ...
    mode_t mode)
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "dsd", id, file, sftp_flags);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
void sftp_file_set_blocking(sftp_file handle) { handle->nonblocking = 0; }
...
ssize_t sftp_read(sftp_file handle, void * buf, size_t count) 
{
	...
	rc = ssh_buffer_pack(buffer, "dSqd", id, handle->handle, handle->offset, count);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
	...
	switch(msg->packet_type) {
		...
		case SSH_FXP_DATA:
		    ...
		    if(ssh_string_len(datastring) > size) {
			    ssh_set_error(sftp->session, SSH_FATAL, "Received a too big DATA packet from sftp server: %" PRIdS " and asked for %u", ssh_string_len(datastring), size);
			    SSH_STRING_FREE(datastring);
			    return SSH_ERROR;
		    }
...
ssize_t sftp_write(sftp_file file, const void * buf, size_t count) 
{
	...
	rc = ssh_buffer_pack(buffer, "dSqdP", id, file->handle, file->offset, count/* len of datastring */, (size_t)count, buf);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
ulong sftp_tell(sftp_file file) { return (ulong)file->offset; }
/* Report current byte position in file. */
uint64_t sftp_tell64(sftp_file file) { return (uint64_t)file->offset; }
...
int sftp_unlink(sftp_session sftp, const char * file) 
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "ds", id, file);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
int sftp_rmdir(sftp_session sftp, const char * directory) 
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "ds", id, directory);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
int sftp_rename(sftp_session sftp, const char * original, const char * newname) 
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "dss", id, original, newname);
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
int sftp_symlink(sftp_session sftp, const char * target, const char * dest) 
{
	...
	if(ssh_get_openssh_version(sftp->session)) {
		rc = ssh_buffer_pack(buffer, "dss", id, target, dest);
	}
	else {
		rc = ssh_buffer_pack(buffer, "dss", id, dest, target);
	}
	if(rc != SSH_OK) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
...
int sftp_fsync(sftp_file file)
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "dsS", id, "fsync@openssh.com", file->handle);
	if(rc < 0) {
		ssh_set_error_oom(sftp->session);
		sftp_set_error(sftp, SSH_FX_FAILURE);
		goto done;
...
sftp_statvfs_t sftp_fstatvfs(sftp_file file)
{
	...
	id = sftp_get_new_id(sftp);
	rc = ssh_buffer_pack(buffer, "dsS", id, "fstatvfs@openssh.com", file->handle);
	if(rc < 0) {
		ssh_set_error_oom(sftp->session);
		SSH_BUFFER_FREE(buffer);
		sftp_set_error(sftp, SSH_FX_FAILURE);
...
char * sftp_canonicalize_path(sftp_session sftp, const char * path)
{
	...
	if(msg->packet_type == SSH_FXP_NAME) {
		uint32_t ignored = 0;
		char * cname = NULL;
		rc = ssh_buffer_unpack(msg->payload, "ds", &ignored, &cname);
		sftp_message_free(msg);
		if(rc != SSH_OK) {
			ssh_set_error(sftp->session, SSH_ERROR, "Failed to parse canonicalized path");
			sftp_set_error(sftp, SSH_FX_FAILURE);
			return NULL;
		}
		return cname;
...
sftp_attributes sftp_stat(sftp_session session, const char * path) { return sftp_xstat(session, path, SSH_FXP_STAT); }
sftp_attributes sftp_lstat(sftp_session session, const char * path) { return sftp_xstat(session, path, SSH_FXP_LSTAT); }
