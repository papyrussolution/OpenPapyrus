...
uchar * ssh_packet_encrypt(ssh_session session, void * data, uint32_t len)
{
	struct ssh_crypto_struct * crypto = NULL;
	struct ssh_cipher_struct * cipher = NULL;
	HMACCTX ctx = NULL;
	...
	if(cipher->aead_encrypt != NULL) {
		cipher->aead_encrypt(cipher, data, out, len, crypto->hmacbuf, session->send_seq);
		memcpy(data, out, len);
	}
	else {
...
#ifdef DEBUG_CRYPTO
		ssh_log_hexdump("mac: ", (const uchar *)data, len);
