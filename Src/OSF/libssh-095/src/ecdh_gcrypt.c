...
int ssh_client_ecdh_init(ssh_session session)
{
	...
	err = gcry_sexp_build(&param, NULL, "(genkey(ecdh(curve %s)))", curve);
	if(err) {
		rc = SSH_ERROR;
		goto out;
	}
	...
	client_pubkey = ssh_sexp_extract_mpi(key, "q", GCRYMPI_FMT_USG, GCRYMPI_FMT_STD);
...
int ecdh_build_k(ssh_session session)
{
	...
	pubkey_raw = session->server ? session->next_crypto->ecdh_client_pubkey : session->next_crypto->ecdh_server_pubkey;
	err = gcry_sexp_build(&pubkey, NULL, "(key-data(public-key(ecdh(curve %s)(q %b))))",
		curve, ssh_string_len(pubkey_raw), ssh_string_data(pubkey_raw));
	if(err) {
		goto out;
	}
	privkey = ssh_sexp_extract_mpi(session->next_crypto->ecdh_privkey, "d", GCRYMPI_FMT_USG, GCRYMPI_FMT_STD);
	if(privkey == NULL) {
		goto out;
	}
	err = gcry_sexp_build(&data, NULL, "(data(flags raw)(value %b))", ssh_string_len(privkey), ssh_string_data(privkey));
	if(err) {
		goto out;
	}
