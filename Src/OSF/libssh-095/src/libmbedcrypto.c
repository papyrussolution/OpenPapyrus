...
void hmac_final(HMACCTX c, uchar * hashmacbuf, uint * len)
{
...
static int cipher_set_encrypt_key(struct ssh_cipher_struct * cipher,
    void * key,
    void * IV)
{
	int rc;
...
static int cipher_set_encrypt_key_cbc(struct ssh_cipher_struct * cipher,
    void * key,
    void * IV)
{
	int rc;
...
#ifdef MBEDTLS_GCM_C
static int cipher_set_key_gcm(struct ssh_cipher_struct * cipher,
    void * key,
    void * IV)
{
	const mbedtls_cipher_info_t * cipher_info = NULL;
...
static int cipher_set_decrypt_key(struct ssh_cipher_struct * cipher,
    void * key,
    void * IV)
{
	int rc;
...
static int cipher_set_decrypt_key_cbc(struct ssh_cipher_struct * cipher,
    void * key,
    void * IV)
{
	int rc;
...
static void cipher_encrypt(struct ssh_cipher_struct * cipher, void * in, void * out, size_t len)
{
	size_t outlen = 0;
	size_t total_len = 0;
...
static void cipher_encrypt_cbc(struct ssh_cipher_struct * cipher, void * in, void * out, ulong len)
{
	size_t outlen = 0;
	int rc = mbedtls_cipher_update(&cipher->encrypt_ctx, in, len, out, &outlen);
	if(rc != 0) {
...
#ifdef MBEDTLS_GCM_C
static int cipher_gcm_get_length(struct ssh_cipher_struct * cipher, void * in, uint8 * out, size_t len, uint64_t seq)
{
	(void)cipher;
	(void)seq;
...
static void cipher_encrypt_gcm(struct ssh_cipher_struct * cipher, void * in, void * out, size_t len, uint8 * tag, uint64_t seq)
{
...
int ssh_mbedtls_random(void * where, int len, int strong)
{
	int rc = 0;
	if(strong) {
		mbedtls_ctr_drbg_set_prediction_resistance(&ssh_mbedtls_ctr_drbg, MBEDTLS_CTR_DRBG_PR_ON);
		rc = mbedtls_ctr_drbg_random(&ssh_mbedtls_ctr_drbg, where, len);
		mbedtls_ctr_drbg_set_prediction_resistance(&ssh_mbedtls_ctr_drbg, MBEDTLS_CTR_DRBG_PR_OFF);
	}
	else {
		rc = mbedtls_ctr_drbg_random(&ssh_mbedtls_ctr_drbg, where, len);
	}
...
mbedtls_ctr_drbg_context * ssh_get_mbedtls_ctr_drbg_context()
{
