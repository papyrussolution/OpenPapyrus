...
ssh_key pki_private_key_from_base64(const char * b64_key, const char * passphrase, ssh_auth_callback auth_fn, void * auth_data)
{
	...
	switch(type) {
		...
		case SSH_KEYTYPE_ECDSA_P521:
		    ecdsa = SAlloc::M(sizeof(mbedtls_pk_context));
		    if(ecdsa == NULL) {
			    return NULL;
		    }
	...
	if(ecdsa != NULL) {
		...
		key->ecdsa = SAlloc::M(sizeof(mbedtls_ecdsa_context));
		if(key->ecdsa == NULL) {
			goto fail;
		}
    ...
    ssh_string q)
{
	...
	key->rsa = SAlloc::M(sizeof(mbedtls_pk_context));
	if(key->rsa == NULL) {
		return SSH_ERROR;
	}
...
ssh_key pki_key_dup(const ssh_key key, int demote)
{
	...
	switch(key->type) {
		case SSH_KEYTYPE_RSA: {
		    ...
		    new->rsa = SAlloc::M(sizeof(mbedtls_pk_context));
		    if(new->rsa == NULL) {
			    goto fail;
		    }
		...
		case SSH_KEYTYPE_ECDSA_P521:
		    ...
		    new->ecdsa = SAlloc::M(sizeof(mbedtls_ecdsa_context));
...
static ssh_string rsa_do_sign_hash(const uchar * digest,
    int dlen,
