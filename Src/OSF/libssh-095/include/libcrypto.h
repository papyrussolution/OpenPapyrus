...
#define bignum_safe_free(num) do { if((num) != NULL) { BN_clear_free((num)); (num)=NULL; } } while(0)
...
#define bignum_bn2hex(num, dest) (*dest)=(uchar *)BN_bn2hex(num)
...
#define bignum_dup(orig, dest) do { if(*(dest) == NULL) { *(dest) = BN_dup(orig); } else { BN_copy(*(dest), orig); } } while(0)
