...
#define SSH_LOG_NONE 0 /** No logging at all */
#define SSH_LOG_WARN 1 /** Show only warnings */
#define SSH_LOG_INFO 2 /** Get some information what's going on */
#define SSH_LOG_DEBUG 3 /** Get detailed debuging information **/
#define SSH_LOG_TRACE 4 /** Get trace output, packet information, ... */
...
LIBSSH_API void ssh_clean_pubkey_hash(uchar ** hash);
...
LIBSSH_API int ssh_connector_set_in_channel(ssh_connector connector, ssh_channel channel, enum ssh_connector_flags_e flags);
LIBSSH_API int ssh_connector_set_out_channel(ssh_connector connector, ssh_channel channel, enum ssh_connector_flags_e flags);
...
LIBSSH_API const char * ssh_copyright();
LIBSSH_API void ssh_disconnect(ssh_session session);
LIBSSH_API char * ssh_dirname(const char * path);
LIBSSH_API int ssh_finalize();
...
LIBSSH_API char * ssh_get_hexa(const uchar * what, size_t len);
...
LIBSSH_API int ssh_get_publickey_hash(const ssh_key key, enum ssh_publickey_hash_type type, uchar ** hash, size_t * hlen);

/* DEPRECATED FUNCTIONS */
SSH_DEPRECATED LIBSSH_API int ssh_get_pubkey_hash(ssh_session session, uchar ** hash);
...
SSH_DEPRECATED LIBSSH_API void ssh_print_hexa(const char * descr, const uchar * what, size_t len);

LIBSSH_API int ssh_get_random(void * where, int len, int strong);
LIBSSH_API int ssh_get_version(ssh_session session);
LIBSSH_API int ssh_get_status(ssh_session session);
LIBSSH_API int ssh_get_poll_flags(ssh_session session);
LIBSSH_API int ssh_init();
...
LIBSSH_API int ssh_get_log_level();
LIBSSH_API void * ssh_get_log_userdata();
LIBSSH_API int ssh_set_log_userdata(void * data);
LIBSSH_API void _ssh_log(int verbosity, const char * function, const char * format, ...) PRINTF_ATTRIBUTE(3, 4);

/* legacy */
SSH_DEPRECATED LIBSSH_API void ssh_log(ssh_session session, int prioriry, const char * format, ...) PRINTF_ATTRIBUTE(3, 4);
...
#define SSH_MESSAGE_FREE(x) do { if((x) != NULL) { ssh_message_free(x); (x) = NULL; } } while(0)
...
LIBSSH_API ssh_session ssh_new();
...
LIBSSH_API int ssh_options_get_port(ssh_session session, uint * port_target);
LIBSSH_API int ssh_pcap_file_close(ssh_pcap_file pcap);
LIBSSH_API void ssh_pcap_file_free(ssh_pcap_file pcap);
LIBSSH_API ssh_pcap_file ssh_pcap_file_new();
...
LIBSSH_API ssh_key ssh_key_new();
#define SSH_KEY_FREE(x) do { if((x) != NULL) { ssh_key_free(x); x = NULL; } } while(0)
...
LIBSSH_API char * ssh_get_fingerprint_hash(enum ssh_publickey_hash_type type, uchar * hash, size_t len);
LIBSSH_API void ssh_print_hash(enum ssh_publickey_hash_type type, uchar * hash, size_t len);
...
LIBSSH_API const char * ssh_userauth_kbdint_getprompt(ssh_session session, uint i, char * echo);
LIBSSH_API int ssh_userauth_kbdint_getnanswers(ssh_session session);
LIBSSH_API const char * ssh_userauth_kbdint_getanswer(ssh_session session, uint i);
LIBSSH_API int ssh_userauth_kbdint_setanswer(ssh_session session, uint i, const char * answer);
...
LIBSSH_API ssh_event ssh_event_new();
...
LIBSSH_API void FASTCALL ssh_event_free(ssh_event event);
...
LIBSSH_API ssh_buffer ssh_buffer_new();
LIBSSH_API void ssh_buffer_free(ssh_buffer buffer);
#define SSH_BUFFER_FREE(x) do { if((x) != NULL) { ssh_buffer_free(static_cast<ssh_buffer>(x)); x = NULL; } } while(0)
LIBSSH_API int FASTCALL ssh_buffer_reinit(ssh_buffer buffer);
LIBSSH_API int ssh_buffer_add_data(ssh_buffer buffer, const void * data, uint32_t len);
LIBSSH_API uint32_t FASTCALL ssh_buffer_get_data(ssh_buffer buffer, void * data, uint32_t requestedlen);
LIBSSH_API void * FASTCALL ssh_buffer_get(ssh_buffer buffer);
LIBSSH_API uint32_t FASTCALL ssh_buffer_get_len(const ssh_buffer buffer);
