...
#define ED25519_KEY_LEN 32 // This definition is used for both OpenSSL and internal implementations
...
struct ssh_key_struct {
	...
	uint8 * ed25519_pubkey;
	uint8 * ed25519_privkey;
#else
...
#endif /* HAVE_LIBGCRYPT */
#ifdef HAVE_OPENSSL_ED25519 // @sobolev #ifndef-->#ifdef
...
int ssh_pki_signature_verify(ssh_session session, ssh_signature sig, const ssh_key key, const uchar * digest, size_t dlen);
...
ssh_string ssh_pki_do_sign(ssh_session session, ssh_buffer sigbuf, const ssh_key privatekey, enum ssh_digest_e hash_type);
