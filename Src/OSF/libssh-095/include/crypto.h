...
struct ssh_crypto_struct {
	...
	uchar * session_id;
	uchar * secret_hash; /* Secret hash is same as session id until re-kex */
	uchar * encryptIV;
	uchar * decryptIV;
	uchar * decryptkey;
	uchar * encryptkey;
...
struct ssh_cipher_struct {
#if 0 // {
	ssh_cipher_struct(const char * pName, uint blockSize, ssh_cipher_e cipherType, uint keySize,
	    int(*setEncryptKeyFunc)(ssh_cipher_struct *, void *, void *),
	    int(*setDecryptKeyFunc)(ssh_cipher_struct *, void *, void *),
	    void(*encryptFunc)(ssh_cipher_struct *, void *, void *, size_t),
	    void(*decryptFunc)(ssh_cipher_struct *, void *, void *, size_t),
	    void(*cleanupFunc)(ssh_cipher_struct *)) : name(pName), blocksize(blockSize), ciphertype(cipherType),
		lenfield_blocksize(0), keylen(0),
#ifdef HAVE_LIBGCRYPT
		key(0),
		//uchar last_iv[AES_GCM_IVLEN];
#elif defined HAVE_LIBCRYPTO
		des3_key(0), aes_key(0), cipher(0), ctx(0),
#elif defined HAVE_LIBMBEDCRYPTO
		des3_key(0), aes_key(0), cipher(0), ctx(0),
#elif defined HAVE_LIBMBEDCRYPTO
...
#endif
#endif
		chacha20_schedule(0), tag_size(0), packets(0), blocks(0), max_blocks(0),
		keysize(keySize), set_encrypt_key(setEncryptKeyFunc), set_decrypt_key(setDecryptKeyFunc),
		encrypt(encryptFunc), decrypt(decryptFunc), cleanup(cleanupFunc)
	{
#ifdef HAVE_LIBGCRYPT
		MEMSZERO(last_iv);
#elif defined HAVE_LIBCRYPTO
		;
#elif defined HAVE_LIBMBEDCRYPTO
		MEMSZERO(encrypt_ctx);
		MEMSZERO(decrypt_ctx);
		MEMSZERO(type);
#ifdef MBEDTLS_GCM_C
		MEMSZERO(gcm_ctx);
		MEMSZERO(last_iv);
	...
	uint blocksize; // blocksize of the algo
	...
	uint keysize; /* bytes of key used. != keylen */
	...
	void (* aead_encrypt)(struct ssh_cipher_struct * cipher, void * in, void * out, size_t len, uint8 * mac, uint64_t seq);
	int (* aead_decrypt_length)(struct ssh_cipher_struct * cipher, void * in, uint8 * out, size_t len, uint64_t seq);
	int (* aead_decrypt)(struct ssh_cipher_struct * cipher, void * complete_packet, uint8 * out, size_t encrypted_size, uint64_t seq);
