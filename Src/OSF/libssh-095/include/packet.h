...
int FASTCALL ssh_packet_send(ssh_session session);
...
void ssh_packet_process(ssh_session session, uint8 type);

/* PACKET CRYPT */
uint32_t ssh_packet_decrypt_len(ssh_session session, uint8 *destination, uint8 *source);
int ssh_packet_decrypt(ssh_session session, uint8 *destination, uint8 *source,
        size_t start, size_t encrypted_size);
uchar *ssh_packet_encrypt(ssh_session session,
                                  void *packet,
                                  uint len);
int ssh_packet_hmac_verify(ssh_session session, const void *data, size_t len,
                           uchar *mac, enum ssh_hmac_e type);
