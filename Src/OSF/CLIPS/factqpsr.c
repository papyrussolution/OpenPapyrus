...
static void                    ReplaceSlotReference(void *, EXPRESSION *, EXPRESSION *, struct FunctionDefinition *,
	int);
...
***********************************************************************/
globle EXPRESSION * FactParseQueryNoAction(void * theEnv, EXPRESSION * top, char * readSource)
{
	struct token queryInputToken;
	EXPRESSION * factQuerySetVars = ParseQueryRestrictions(theEnv, top, readSource, &queryInputToken);
...
***************************************************/
static intBool ReplaceTemplateNameWithReference(void * theEnv, EXPRESSION * theExp)
{
	...
	if(theExp->type == SYMBOL) {
		...
		theDeftemplate = (struct deftemplate *)FindImportedConstruct(theEnv, "deftemplate", NULL, theTemplateName, &count, TRUE, NULL);
		if(theDeftemplate == NULL) { /* TBD */
...
/*************************************************************
   ...
   NOTES        : Expects top != NULL
*************************************************************/
static int ParseQueryTestExpression(void * theEnv, EXPRESSION * top, char * readSource)
{
	EXPRESSION * qtest;
	int error = FALSE;
	struct BindInfo * oldBindList = GetParsedBindNames(theEnv);
...
***********************************************************************************/
static void ReplaceFactVariables(void * theEnv, EXPRESSION * vlist, EXPRESSION * bexp, int sdirect,
	int ndepth)
{
	EXPRESSION * eptr;
	int posn;
	struct FunctionDefinition * rindx_func = FindFunction(theEnv, "(query-fact)");
	struct FunctionDefinition * rslot_func = FindFunction(theEnv, "(query-fact-slot)");
	while(bexp != NULL) {
		...
		if(bexp->argList != NULL) {
			...
			else {ReplaceFactVariables(theEnv, vlist, bexp->argList, sdirect, ndepth); }
		}
		bexp = bexp->nextArg;
	}
}
