	...
	void * theEnv)
{
	...
	/*=============================*/
	if(!bloaded) {
		DoForAllConstructs(theEnv, DestroyDefclassAction, DefclassData(
				theEnv)->DefclassModuleIndex, FALSE, NULL);
	...
	if(!bloaded) {
		if(DefclassData(theEnv)->ClassIDMap != NULL) {
			genfree(theEnv, DefclassData(theEnv)->ClassIDMap, DefclassData(theEnv)->AvailClassID*
				sizeof(DEFCLASS *));
		}
	}
		...
		for(cls = DefclassData(theEnv)->ClassTable[j]; cls != NULL; cls = cls->nxtHash) {
			for(i = 0; i < cls->slotCount; i++) {
				if((cls->slots[i].defaultValue != NULL) && (cls->slots[i].dynamicDefault == 0)) {
					...
					EvaluateAndStoreInDataObject(theEnv, (int)cls->slots[i].multiple,
						(EXPRESSION *)tmpexp,
...
globle void CreateSystemClasses(void * theEnv)
{
	...
	DefclassData(theEnv)->PrimitiveClassMap[EXTERNAL_ADDRESS] = AddSystemClass(theEnv, EXTERNAL_ADDRESS_TYPE_NAME,
		address);
	...
	DefclassData(theEnv)->PrimitiveClassMap[INSTANCE_ADDRESS] = AddSystemClass(theEnv, INSTANCE_ADDRESS_TYPE_NAME,
		instance);
	DefclassData(theEnv)->PrimitiveClassMap[INSTANCE_NAME] = AddSystemClass(theEnv, INSTANCE_NAME_TYPE_NAME,
		instance);
  #if DEFRULE_CONSTRUCT
	...
	/* ================================================================================
	    ...
	    =============================================================================== */
	AddClassLink(theEnv, &DefclassData(
			theEnv)->PrimitiveClassMap[INSTANCE_ADDRESS]->directSuperclasses, address, -1);
...
static void SetupDefclasses(void * theEnv)
{
	...
	DefclassData(theEnv)->DefclassModuleIndex = RegisterModuleItem(theEnv, "defclass",
 #if (!RUN_TIME)
	...
	EnvDefineFunction2(theEnv, "slot-allowed-values", 'm', PTIEF SlotAllowedValuesCommand,
		"SlotAllowedValuesCommand",
		"22w");
	EnvDefineFunction2(theEnv, "slot-allowed-classes", 'm', PTIEF SlotAllowedClassesCommand,
		"SlotAllowedClassesCommand",
		"22w");
	EnvDefineFunction2(theEnv, "slot-range", 'm', PTIEF SlotRangeCommand, "SlotRangeCommand", "22w");
	EnvDefineFunction2(theEnv, "slot-cardinality", 'm', PTIEF SlotCardinalityCommand, "SlotCardinalityCommand",
		"22w");
	...
	EnvDefineFunction2(theEnv, "get-class-defaults-mode", 'w', PTIEF GetClassDefaultsModeCommand,
		"GetClassDefaultsModeCommand",
		"00");
	EnvDefineFunction2(theEnv, "set-class-defaults-mode", 'w', PTIEF SetClassDefaultsModeCommand,
		"SetClassDefaultsModeCommand",
		"11w");
 #endif

 #if DEBUGGING_FUNCTIONS
	AddWatchItem(theEnv, "instances", 0, &DefclassData(
			theEnv)->WatchInstances, 75, DefclassWatchAccess, DefclassWatchPrint);
...
static DEFCLASS * AddSystemClass(void * theEnv, char * name, DEFCLASS * parent)
{
	...
	DEFCLASS * sys = NewClass(theEnv, (SYMBOL_HN *)EnvAddSymbol(theEnv, name));
	sys->abstract = 1;
  #if DEFRULE_CONSTRUCT
	sys->reactive = 0;
  #endif
