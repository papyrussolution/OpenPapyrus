...
static int ReplaceProcBinds(void *, EXPRESSION *, int (*)(void *, EXPRESSION *, void *), void *);
	...
	void * theEnv)
{
	...
	AllocateEnvironmentData(theEnv, PROCEDURAL_PRIMITIVE_DATA, sizeof(struct proceduralPrimitiveData),
...
#if !DEFFUNCTION_CONSTRUCT
	memcpy(&ProceduralPrimitiveData(theEnv)->DeffunctionEntityRecord, &deffunctionEntityRecord,
	...
	void * theEnv)
{
	...
	if(ProceduralPrimitiveData(theEnv)->ProcParamArray != NULL)
		rm(theEnv, (void *)ProceduralPrimitiveData(theEnv)->ProcParamArray,
			(sizeof(DATA_OBJECT)*ProceduralPrimitiveData(theEnv)->ProcParamArraySize));
#if DEFGENERIC_CONSTRUCT
	...
	if(ProceduralPrimitiveData(theEnv)->WildcardValue != NULL) {
		...
		if(ProceduralPrimitiveData(theEnv)->WildcardValue->value !=
		   ProceduralPrimitiveData(theEnv)->NoParamValue)
			AddToMultifieldList(theEnv, (MULTIFIELD_PTR)ProceduralPrimitiveData(
					theEnv)->WildcardValue->value);
	...
	void * theEnv)
{
	...
	for(i = 0; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize; i++) {
		...
		ProceduralPrimitiveData(theEnv)->ProcParamExpressions[i].nextArg =
		        ((i+1) != ProceduralPrimitiveData(theEnv)->ProcParamArraySize) ? &ProceduralPrimitiveData(
				theEnv)->ProcParamExpressions[i+1] : NULL;
	}
	return ProceduralPrimitiveData(theEnv)->ProcParamExpressions;
}
	...
	void (* crtproc)(void *))
{
	...
	if((ProceduralPrimitiveData(theEnv)->WildcardValue != NULL) ? (result->value ==
...
{
	register DATA_OBJECT * src = &ProceduralPrimitiveData(theEnv)->ProcParamArray[*((int *)ValueToBitMap(value))-1];
...
{
	PACKED_PROC_VAR * pvar = (PACKED_PROC_VAR *)ValueToBitMap(value);
	register DATA_OBJECT * src = &ProceduralPrimitiveData(theEnv)->LocalVarArray[pvar->first-1];
...
{
	register DATA_OBJECT * dst = &ProceduralPrimitiveData(theEnv)->LocalVarArray[*((int *)ValueToBitMap(value))-1];
...
/***************************************************
   ...
   NOTES        : None
***************************************************/
static int FindProcParameter(SYMBOL_HN * name, EXPRESSION * parameterList, SYMBOL_HN * wildcard)
{
	int i = 1;
	while(parameterList != NULL) {
