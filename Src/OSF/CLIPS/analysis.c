...
static int                     PropagateVariableToNodes(void *, struct lhsParseNode *, int, struct symbolHashNode *,
	struct lhsParseNode *, int, int,
	int);
static struct lhsParseNode * CheckExpression(void *, struct lhsParseNode *, struct lhsParseNode *, int,
	struct symbolHashNode *,
	int);
static void                    VariableReferenceErrorMessage(void *, struct symbolHashNode *, struct lhsParseNode *,
	int, struct symbolHashNode *,
	int);
static int                     ProcessField(void * theEnv, struct lhsParseNode *, struct lhsParseNode *,
	struct lhsParseNode *);
...
static int                     PropagateVariableDriver(void *, struct lhsParseNode *, struct lhsParseNode *,
	struct lhsParseNode *, int, struct symbolHashNode *, struct lhsParseNode *,
	int);
	...
	struct lhsParseNode * patternPtr)
{
	...
	while(patternPtr != NULL) {
		...
		/*==============================================================*/
			...
			else {patternPtr->networkTest = GetvarReplace(theEnv, patternPtr->expression); }
		}
		/*=====================================================*/
	...
	int theField)
{
	...
	while(exprPtr != NULL) {
		...
		/*============================================*/
			...
			if((rv =
			            CheckExpression(theEnv, exprPtr->bottom, exprPtr, whichCE, slotName,
					    theField)) != NULL) {
				return rv;
			}
		}
...
/************************************************************/
static void VariableMixingErrorMessage(void * theEnv, struct symbolHashNode * theVariable)
{
