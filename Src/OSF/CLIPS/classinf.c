...
{
	int inhp;
	void * clsptr = ClassInfoFnxArgs(theEnv, "class-slots", &inhp);
	if(clsptr == NULL) {
...
{
	int inhp;
	void * clsptr = ClassInfoFnxArgs(theEnv, "class-superclasses", &inhp);
	if(clsptr == NULL) {
		EnvSetMultifieldErrorValue(theEnv, result);
		return;
	}
...
{
	int inhp;
	void * clsptr = ClassInfoFnxArgs(theEnv, "class-subclasses", &inhp);
	if(clsptr == NULL) {
		EnvSetMultifieldErrorValue(theEnv, result);
		return;
	}
...
globle void EnvClassSlots(void * theEnv, void * clsptr, DATA_OBJECT * result, int inhp)
{
	...
	register DEFCLASS * cls = (DEFCLASS *)clsptr;
	unsigned long size = inhp ? cls->instanceSlotCount : cls->slotCount;
		...
		for(classi = 0; classi < classiLimit; classi++) {
			...
			for(j = 0; j < supcls->handlerCount; j++) {
				...
				SetMFValue(result->value, i++,
					EnvAddSymbol(theEnv,
						MessageHandlerData(theEnv)->hndquals[supcls->handlers[j].type]));
			}
...
/*****************************************************
   ...
   NOTES        : None
*****************************************************/
static void SlotInfoSupportFunction(void * theEnv, DATA_OBJECT * result, char * fnxname, void (* fnx)(void *, void *, char *, DATA_OBJECT *))
{
	DEFCLASS * cls;
	SYMBOL_HN * ssym = CheckClassAndSlot(theEnv, fnxname, &cls);
	if(ssym == NULL) {
		EnvSetMultifieldErrorValue(theEnv, result);
		return;
	}
...
/*********************************************************
   ...
   NOTES        : None
*********************************************************/
static SLOT_DESC * SlotInfoSlot(void * theEnv, DATA_OBJECT * result, DEFCLASS * cls, char * sname, char * fnxname)
{
	SYMBOL_HN * ssym;
	int i;
