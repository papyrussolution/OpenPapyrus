	...
	void * theEnv)
{
	...
	for(bfPtr = BloadData(theEnv)->ClearBloadReadyFunctions;
		...
		else {ready = (*((int (*)(void))bfPtr->func))(); }
		if(ready == FALSE) {
			if(!error) {
...
/*************************************************/
static void AbortBload(void * theEnv)
{
	struct callFunctionItem * bfPtr;
	for(bfPtr = BloadData(theEnv)->AbortBloadFunctions; bfPtr != NULL; bfPtr = bfPtr->next) {
		if(bfPtr->environmentAware) {
			(*bfPtr->func)(theEnv);
		}
		else {(*(void (*)(void))bfPtr->func)(); }
...
{
	BloadData(theEnv)->BeforeBloadFunctions = AddFunctionToCallList(theEnv, name, priority, func, BloadData(theEnv)->BeforeBloadFunctions, TRUE);
}
...
{
	BloadData(theEnv)->AfterBloadFunctions = AddFunctionToCallList(theEnv, name, priority, func, BloadData(theEnv)->AfterBloadFunctions, TRUE);
}
...
{
	BloadData(theEnv)->ClearBloadReadyFunctions = AddFunctionToCallList(theEnv, name, priority,
		(void (*)(void *))func, BloadData(theEnv)->ClearBloadReadyFunctions, TRUE);
}
...
{
	BloadData(theEnv)->AbortBloadFunctions = AddFunctionToCallList(theEnv, name, priority, func, BloadData(
			theEnv)->AbortBloadFunctions, TRUE);
}
...
*******************************************************/
  ...
  #pragma argsused
 #endif
static int BloadOutOfMemoryFunction(void * theEnv, unsigned long size)
{
