	...
	void * theEnv)
{
	...
	AddWatchItem(theEnv, "activations", 1, &AgendaData(
			theEnv)->WatchActivations, 40, DefruleWatchAccess, DefruleWatchPrint);
 #endif
 #if !RUN_TIME
...
globle void * EnvGetNextActivation(void * theEnv, void * actPtr)
{
	struct defruleModule * theModuleItem;
	if(actPtr == NULL) {
		theModuleItem = (struct defruleModule *)GetModuleItem(theEnv, NULL, DefruleData(
				theEnv)->DefruleModuleIndex);
		if(theModuleItem == NULL) return NULL;
		return (void *)theModuleItem->agenda;
	}
	else {return (void *)(((struct activation *)actPtr)->next);  }
}
...
{
	struct activation * tempPtr;
	struct activation * theActivation = GetDefruleModuleItem(theEnv, NULL)->agenda;
	...
	void * theEnv)
{
	...
	else {theModule = ((struct defmodule *)EnvGetCurrentModule(theEnv)); }
	/*===============================================*/
...
/*****************************************************************/
static char * SalienceEvaluationName(int strategy)
{
	char * sname = 0;
	switch(strategy) {
	    case WHEN_DEFINED: sname = "when-defined"; break;
	    case WHEN_ACTIVATED: sname = "when-activated"; break;
	    case EVERY_CYCLE: sname = "every-cycle"; break;
...
{
	int ov = AgendaData(theEnv)->SalienceEvaluation;
