...
static OBJECT_PATTERN_NODE * FindObjectPatternNode(OBJECT_PATTERN_NODE *, struct lhsParseNode *, OBJECT_PATTERN_NODE **,
	unsigned);
...
static struct lhsParseNode * FilterObjectPattern(void *, struct patternParser *, struct lhsParseNode *,
			...
			if(TestBitMap(tmpset->map, i)) {
				...
				if(incompatibleConstraint) {
					...
					for(subNode = tmpNode->bottom; subNode != NULL; subNode = subNode->right) {
						...
						crossConstraints = IntersectConstraints(theEnv, theConstraint,
							sd->constraint);
	...
	while(upperLevel != NULL) {
		if((upperLevel->leftNode == NULL) &&
		   (upperLevel->rightNode == NULL)) {
			...
			else {upperLevel->nextLevel = patternPtr->rightNode; }
			...
			if(TestBitMap(clsset->map, cls->id)) {
				...
				if((si != -1) ? cls->instanceTemplate[si]->reactive : FALSE) {
					if(cls->instanceTemplate[si]->multiple)
						*multip = TRUE;
					tmpConstraints =
					        UnionConstraints(theEnv, cls->instanceTemplate[si]->constraint,
							totalConstraints);
