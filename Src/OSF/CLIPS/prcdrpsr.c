...
{
	int theIndex = 1;
	struct BindInfo * var_ptr = ProcedureParserData(theEnv)->ListOfParsedBindNames;
	while(var_ptr != NULL) {
...
/* FindBindConstraints:                               */
/********************************************************/
globle struct constraintRecord * FindBindConstraints(void * theEnv, SYMBOL_HN * nameSought)
{
	struct BindInfo * theVariable = ProcedureParserData(theEnv)->ListOfParsedBindNames;
...
/****************************************************************/
static int AddBindName(void * theEnv, SYMBOL_HN * variableName, CONSTRAINT_RECORD * theConstraint)
{
	...
	while(currentBind != NULL) {
		if(currentBind->name == variableName) {
			if(theConstraint != NULL) {
				...
				currentBind->constraints = UnionConstraints(theEnv, theConstraint,
...
{
	struct BindInfo * prv = NULL;
	struct BindInfo * tmp = ProcedureParserData(theEnv)->ListOfParsedBindNames;
