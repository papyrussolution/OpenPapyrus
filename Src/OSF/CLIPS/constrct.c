	...
	void * theEnv)
{
	...
	for(theFunction = ConstructData(theEnv)->ListOfClearFunctions;
		...
		else {(*(void (*)(void))theFunction->func)(); }
	}
	/*=============================*/
...
{
	int found;
	ConstructData(theEnv)->ListOfClearReadyFunctions = RemoveFunctionFromCallList(theEnv, name, ConstructData(theEnv)->ListOfClearReadyFunctions, &found);
	if(found) 
		return TRUE;
	return FALSE;
}
...
{
	ConstructData(theEnv)->ListOfClearFunctions = AddFunctionToCallList(theEnv, name, priority,
		(void (*)(void *))functionPtr, ConstructData(theEnv)->ListOfClearFunctions, TRUE);
	return 1;
}
...
{
	int found;
	ConstructData(theEnv)->ListOfClearFunctions = RemoveFunctionFromCallList(theEnv, name, ConstructData(theEnv)->ListOfClearFunctions, &found);
	if(found) 
		return TRUE;
	return FALSE;
}
	...
	char *(*nameFunction)(void *, void *))
{
	...
	for(theConstruct = (*nextFunction)(theEnv, NULL); theConstruct != NULL; theConstruct = (*nextFunction)(theEnv, theConstruct)) {
		count++;
	}
	/*===========================*/
...
globle void DeinstallConstructHeader(void * theEnv, struct constructHeader * theHeader)
{
	...
	if(theHeader->ppForm != NULL) {
		rm(theEnv, theHeader->ppForm, sizeof(char)*(strlen(theHeader->ppForm)+1));
...
#if (!RUN_TIME) && (!BLOAD_ONLY)
	ConstructData(theEnv)->ListOfSaveFunctions = AddFunctionToCallList(theEnv, name, priority,
			(void (*)(void *))functionPtr, ConstructData(theEnv)->ListOfSaveFunctions, TRUE);
#else
 #if MAC_MCW || IBM_MCW || MAC_XCD
  #pragma unused(theEnv)
 #endif
