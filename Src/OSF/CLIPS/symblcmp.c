...
/**************************************************************/
globle void AtomicValuesToCode(void * theEnv, char * fileName)
{
	int version;
...
/*****************************************************/
static int SymbolHashNodesToCode(void * theEnv, char * fileName, int version)
{
	unsigned long i, j;
	...
	for(i = 1; i <= (unsigned long)(numberOfEntries/ConstructCompilerData(theEnv)->MaxIndices)+1; i++) {
		fprintf(ConstructCompilerData(
				theEnv)->HeaderFP, "extern struct symbolHashNode S%d_%ld[];\n", ConstructCompilerData(
				theEnv)->ImageID, i);
	}
	/*==================*/
	/* Create the file. */
		...
		for(hashPtr = symbolTable[i];
			...
			if(hashPtr->next == NULL) {
				...
				if((j+1) >= (unsigned long)ConstructCompilerData(theEnv)->MaxIndices) {
					fprintf(fp, "{&S%d_%d[%d],", ConstructCompilerData(
							theEnv)->ImageID, arrayVersion+1, 0);
				}
				else {fprintf(fp, "{&S%d_%d[%ld],", ConstructCompilerData(
						      theEnv)->ImageID, arrayVersion, j+1); }
			}
	...
	int version)
{
	...
	for(i = 1; i <= (numberOfEntries/ConstructCompilerData(theEnv)->MaxIndices)+1; i++) {
		fprintf(ConstructCompilerData(
				theEnv)->HeaderFP, "extern struct bitMapHashNode B%d_%d[];\n",
		...
		for(hashPtr = bitMapTable[i];
			...
			if(hashPtr->next == NULL) {
				...
				if((j+1) >= ConstructCompilerData(theEnv)->MaxIndices) {
					fprintf(fp, "{&B%d_%d[%d],", ConstructCompilerData(
							theEnv)->ImageID, arrayVersion+1, 0);
				}
				else {fprintf(fp, "{&B%d_%d[%d],", ConstructCompilerData(
						      theEnv)->ImageID, arrayVersion, j+1); }
			}
	...
	int version)
{
	...
	for(i = 1; i <= (numberOfEntries/ConstructCompilerData(theEnv)->MaxIndices)+1; i++) {
		fprintf(ConstructCompilerData(
				theEnv)->HeaderFP, "extern unsigned long L%d_%d[];\n", ConstructCompilerData(
				theEnv)->ImageID, i);
	}
	/*==================*/
	/* Create the file. */
	...
	int version)
{
	...
	for(i = 1; i <= (numberOfEntries/ConstructCompilerData(theEnv)->MaxIndices)+1; i++) {
		fprintf(ConstructCompilerData(
				theEnv)->HeaderFP, "extern struct floatHashNode F%d_%d[];\n",
			ConstructCompilerData(theEnv)->ImageID, i);
	}
	/*==================*/
	/* Create the file. */
	...
	for(i = 0; i < FLOAT_HASH_SIZE; i++) {
		for(hashPtr = floatTable[i];
			...
			if(hashPtr->next == NULL) {
				...
				if((j+1) >= ConstructCompilerData(theEnv)->MaxIndices) {
					fprintf(fp, "{&F%d_%d[%d],", ConstructCompilerData(
							theEnv)->ImageID, arrayVersion+1, 0);
				}
				else {fprintf(fp, "{&F%d_%d[%d],", ConstructCompilerData(
						      theEnv)->ImageID, arrayVersion, j+1); }
			}
	...
	int version)
{
	...
	for(i = 1; i <= (numberOfEntries/ConstructCompilerData(theEnv)->MaxIndices)+1; i++) {
		fprintf(ConstructCompilerData(
				theEnv)->HeaderFP, "extern struct integerHashNode I%d_%d[];\n", ConstructCompilerData(
				theEnv)->ImageID, i);
	}
	/*==================*/
	/* Create the file. */
	...
	for(i = 0; i < INTEGER_HASH_SIZE; i++) {
		for(hashPtr = integerTable[i];
			...
			if(hashPtr->next == NULL) {
				...
				if((j+1) >= ConstructCompilerData(theEnv)->MaxIndices) {
					fprintf(fp, "{&I%d_%d[%d],", ConstructCompilerData(
							theEnv)->ImageID, arrayVersion+1, 0);
				}
				else {fprintf(fp, "{&I%d_%d[%d],", ConstructCompilerData(
						      theEnv)->ImageID, arrayVersion, j+1); }
			}
			fprintf(fp, "%ld,0,1,0,0,%d,", hashPtr->count+1, i);
...
/*   created using the constructs-to-c function.                */
/****************************************************************/
static int HashTablesToCode(void * theEnv, char * fileName)
{
	unsigned long i;
	FILE * fp;
	struct symbolHashNode ** symbolTable;
	...
	fprintf(ConstructCompilerData(
			theEnv)->HeaderFP, "extern struct symbolHashNode *sht%d[];\n", ConstructCompilerData(
			theEnv)->ImageID);
	...
	fprintf(ConstructCompilerData(
			theEnv)->HeaderFP, "extern struct floatHashNode *fht%d[];\n", ConstructCompilerData(
			theEnv)->ImageID);
	...
	fprintf(ConstructCompilerData(
			theEnv)->HeaderFP, "extern struct integerHashNode *iht%d[];\n", ConstructCompilerData(
			theEnv)->ImageID);
	fprintf(fp, "struct integerHashNode *iht%d[%d] = {\n", ConstructCompilerData(
			theEnv)->ImageID, INTEGER_HASH_SIZE);
	...
	fprintf(ConstructCompilerData(
			theEnv)->HeaderFP, "extern struct bitMapHashNode *bmht%d[];\n", ConstructCompilerData(
			theEnv)->ImageID);
...
globle void PrintSymbolReference(void * theEnv, FILE * theFile, struct symbolHashNode * theSymbol)
{
	...
	else fprintf(theFile, "&S%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/*********************************************************/
static void PrintCString(FILE * theFile, char * str)
{
	unsigned i;
	size_t slen;
	/*============================================*/
