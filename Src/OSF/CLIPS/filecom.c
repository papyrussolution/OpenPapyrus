...
{
	struct batchEntry * nextEntry;
	struct batchEntry * theEntry = FileCommandData(theEnv)->TopOfBatchList;
	while(theEntry != NULL) {
	...
	int rv)
{
	...
	/*=====================================================*/
		...
		FileCommandData(theEnv)->DribbleBuffer = ExpandStringWithChar(theEnv, rv, FileCommandData(
				theEnv)->DribbleBuffer,
...
/*********************************************************/
static int UngetcDribble(void * theEnv, int ch, char * logicalName)
{
	int rv;
	...
	int returnOnEOF)
{
	...
	while((rv == EOF) && (flag == 1)) {
		...
		else {rv = EnvGetcRouter(theEnv, (char *)FileCommandData(theEnv)->BatchSource); }
		if(rv == EOF) {
			if(FileCommandData(theEnv)->BatchCurrentPosition > 0) EnvPrintRouter(
					theEnv, "stdout", (char *)FileCommandData(theEnv)->BatchBuffer);
	...
	/*=========================================================*/
	if(rv == EOF) {
		if(FileCommandData(theEnv)->BatchCurrentPosition >
		   0) EnvPrintRouter(theEnv, "stdout", (char *)FileCommandData(
					theEnv)->BatchBuffer);
		...
		else {return EnvGetcRouter(theEnv, logicalName);  }
	}
	/*========================================*/
	/* Add the character to the batch buffer. */
	/*========================================*/

	FileCommandData(theEnv)->BatchBuffer = ExpandStringWithChar(theEnv, (char)rv, FileCommandData(
			theEnv)->BatchBuffer, &FileCommandData(theEnv)->BatchCurrentPosition,
		&FileCommandData(theEnv)->BatchMaximumPosition, FileCommandData(
			theEnv)->BatchMaximumPosition+BUFFER_SIZE);
	...
	char * logicalName)
{
 ...
 #pragma unused(logicalName)
	...
	if(FileCommandData(theEnv)->BatchBuffer !=
	   NULL) FileCommandData(theEnv)->BatchBuffer[FileCommandData(theEnv)->BatchCurrentPosition] = EOS;
	if(FileCommandData(theEnv)->BatchType == FILE_BATCH) {
	...
	void * theEnv)
{
	...
	if(FileCommandData(theEnv)->TopOfBatchList->batchType == FILE_BATCH) {
		...
		rm(theEnv, FileCommandData(theEnv)->TopOfBatchList->theString,
...
{
	char * fileName;
	if(EnvArgCountCheck(theEnv, "batch*", EXACTLY, 1) == -1) 
		return FALSE;
