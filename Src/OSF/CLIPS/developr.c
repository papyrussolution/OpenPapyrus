...
/* EnableGCHeuristics:      */
/******************************************************/
globle void EnableGCHeuristics(void * theEnv)
{
...
/* DisableGCHeuristics:      */
/******************************************************/
globle void DisableGCHeuristics(void * theEnv)
{
...
/******************************************************/
globle void PrimitiveTablesInfo(void * theEnv)
{
	unsigned long i;
	...
	void * theEnv)
{
	...
	for(i = 0; i < SYMBOL_HASH_SIZE; i++) {
		...
		else {symbolCounts[COUNT_SIZE-1]++; }
	}
	/*===================================*/
	/* Count entries in the float table. */
	/*===================================*/
		...
		while(alphaPtr != NULL) {
			...
			for(i = 0; i <= cbmp->maxid; i++)
				if(TestBitMap(cbmp->map, i)) {
					EnvPrintRouter(theEnv, WDISPLAY, " ");
					EnvPrintRouter(theEnv, WDISPLAY,
						EnvGetDefclassName(theEnv, (void *)DefclassData(theEnv)->ClassIDMap[i]));
				}
				...
				for(i = NAME_ID; i <= sbmp->maxid; i++)
					if(TestBitMap(sbmp->map, i)) {
						...
						if(uptr == NULL) {
							EnvPrintRouter(theEnv, WDISPLAY, " ");
							EnvPrintRouter(theEnv, WDISPLAY,
								ValueToString(FindIDSlotName(theEnv, i)));
						}
					}
			}
			EnvPrintRouter(theEnv, WDISPLAY, "\n");
...
/******************************************************/
globle void InstanceTableUsage(void * theEnv)
{
	unsigned long i;
	...
	for(i = 0; i < INSTANCE_TABLE_HASH_SIZE; i++) {
		...
		else {instanceCounts[COUNT_SIZE-1]++; }
	}
	/*========================*/
	/* Print the information. */
	/*========================*/
