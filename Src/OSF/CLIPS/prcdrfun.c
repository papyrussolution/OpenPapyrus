...
/* GetLoopCount                                 */
/************************************************/
globle long GetLoopCount(void * theEnv)
{
	int depth = ValueToInteger(GetFirstArgument()->value);
	LOOP_COUNTER_STACK * tmpCounter = ProcedureFunctionData(theEnv)->LoopCounterStack;
	while(depth > 0) {
...
globle void IfFunction(void * theEnv, DATA_OBJECT_PTR returnValue)
{
	...
	/*=========================================*/
	if((returnValue->value == EnvFalseSymbol(theEnv)) && (returnValue->type == SYMBOL) && (numArgs == 3)) {
...
globle void BindFunction(void * theEnv, DATA_OBJECT_PTR returnValue)
{
	...
	int found = FALSE, unbindVar = FALSE;
	...
	else {ValueDeinstall(theEnv, theBind); }
	/*================================*/
...
{
	int i;
	int numa = EnvRtnArgCount(theEnv);
	if(numa == 0) {
		returnValue->type = SYMBOL;
		returnValue->value = EnvFalseSymbol(theEnv);
		return;
