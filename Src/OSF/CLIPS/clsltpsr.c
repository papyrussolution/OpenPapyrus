		...
		if((sd->defaultSpecified == 0) && (compslot->defaultSpecified == 1)) {
			...
			if(compslot->defaultValue != NULL) {
				if(sd->dynamicDefault) {
					sd->defaultValue = (void *)PackExpression(theEnv,
...
/***************************************************
   ...
   NOTES        : None
***************************************************/
static intBool CheckForFacetConflicts(void * theEnv, SLOT_DESC * sd, CONSTRAINT_PARSE_RECORD * parsedConstraint)
{
	if(sd->multiple == 0) {
...
********************************************************************/
static intBool EvaluateSlotDefaultValue(void * theEnv, SLOT_DESC * sd, char * specbits)
{
	DATA_OBJECT temp;
