...
/****************************************************************************/
globle int TextLookupFetch(void * theEnv, char * file)
{
	...
	* menu[1];   /*Buffer for the name of the current main topic       */
	...
	do {
		...
		if(EvaluationData(theEnv)->HaltExecution) {
			while(status != EXIT)
				if((fp =
				            GetEntries(theEnv, TextProcessingData(theEnv)->help_file, menu, NULL,
						    &status)) != NULL)
					GenClose(theEnv, fp);
		}
	} while(status != EXIT);
...
globle void HelpPathFunction(void * theEnv)
{
	...
	if(EnvRtnArgCount(theEnv) == 0) {
		...
		if(TextProcessingData(theEnv)->help_file != NULL) {
			...
			rm(theEnv, (void *)TextProcessingData(theEnv)->help_file,
	...
	void * theEnv)
{
	...
	if(TextProcessingData(theEnv)->help_file != NULL)
		if((strcmp(file,
			    TextProcessingData(theEnv)->help_file) == 0) &&
		   (TextProcessingData(theEnv)->HELP_INIT == TRUE)) {
			rm(theEnv, (void *)TextProcessingData(theEnv)->help_file,
				strlen(TextProcessingData(theEnv)->help_file)+1);
...
/*********************************************************/
static void DeallocateTextProcessingData(void * theEnv)
{
 ...
 #if HELP_FUNCTIONS
	if(TextProcessingData(theEnv)->help_file != NULL) {
		rm(theEnv, TextProcessingData(theEnv)->help_file, strlen(TextProcessingData(theEnv)->help_file)+1);
	}
 #endif
}
