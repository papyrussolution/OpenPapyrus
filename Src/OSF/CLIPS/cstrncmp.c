...
/***********************************************/
globle int ConstraintsToCode(void * theEnv, char * fileName, int fileID, FILE * headerFP, int imageID, int maxIndices)
{
	int i, j, count;
	int newHeader = TRUE;
	FILE * fp;
	...
	for(i = 0; i < SIZE_CONSTRAINT_HASH; i++) {
		for(tmpPtr = ConstraintData(theEnv)->ConstraintHashtable[i]; tmpPtr != NULL; tmpPtr = tmpPtr->next) {
			if(newHeader) {
			...
			if(tmpPtr->next == NULL) {
				...
				else {fprintf(fp, ",&C%d_%d[%d],", imageID, arrayVersion, j+1); }
			}
...
globle void PrintConstraintReference(void * theEnv, FILE * fp, CONSTRAINT_RECORD * cPtr, int imageID, int maxIndices)
{
	...
	else fprintf(fp, "&C%d_%d[%d]", imageID, (int)(cPtr->bsaveIndex/maxIndices)+1, (int)cPtr->bsaveIndex%maxIndices);
}
