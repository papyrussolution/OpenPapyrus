 ...
 *   07/22/98    stephen     Changed to match C++ style currencySymbol -> fCurrencySymbol Constants changed from CAPS to kCaps
 *   06/24/99    helena      Integrated Alan's NF enhancements and Java2 bug fixes
 *   09/22/00    grhoten     Marked deprecation tags with a pointer to replacement functions.
 */
#ifndef DCFMTSYM_H
#define DCFMTSYM_H
...
public:
	 ...
	 * @stable ICU 2.0
	 */
	void setSymbol(ENumberFormatSymbol symbol, const UnicodeString &value, const bool propagateDigits);
...
private:
	...
	void initialize(const Locale& locale, UErrorCode& success,
	    bool useLastResortData = false, const NumberingSystem* ns = nullptr);
...
public:
	 ...
	 */
	inline bool isCustomCurrencySymbol() const {
	 ...
	 * @internal For ICU use only
	 */
	inline bool isCustomIntlCurrencySymbol() const {
...
private:
	...
	bool fIsCustomCurrencySymbol;
	bool fIsCustomIntlCurrencySymbol;
};

inline UnicodeString DecimalFormatSymbols::getSymbol(ENumberFormatSymbol symbol) const
{
...
inline const UnicodeString & DecimalFormatSymbols::getConstSymbol(ENumberFormatSymbol symbol) const
{
	const UnicodeString * strPtr;
	if(symbol < kFormatSymbolCount) {
		strPtr = &fSymbols[symbol];
...
inline void DecimalFormatSymbols::setSymbol(ENumberFormatSymbol symbol, const UnicodeString &value, const bool propagateDigits = true) 
{
...
inline Locale DecimalFormatSymbols::getLocale() const { return locale; }

#ifndef U_HIDE_INTERNAL_API
	inline const char16_t* DecimalFormatSymbols::getCurrencyPattern() const { return currPattern; }
#endif /* U_HIDE_INTERNAL_API */
