...
static constexpr int32_t kMaxIntFracSig = 999; // ICU4J Equivalent: RoundingUtils.MAX_INT_FRAC_SIG
static constexpr RoundingMode kDefaultMode = RoundingMode::UNUM_FOUND_HALFEVEN; // ICU4J Equivalent: RoundingUtils.DEFAULT_ROUNDING_MODE
static constexpr char16_t kFallbackPaddingString[] = u" "; // ICU4J Equivalent: Padder.FALLBACK_PADDING_STRING
...
enum AffixPatternType {
	TYPE_CODEPOINT = 0, // Represents a literal character; the value is stored in the code point field.
	TYPE_MINUS_SIGN = -1, // Represents a minus sign symbol '-'.
	TYPE_PLUS_SIGN = -2, // Represents a plus sign symbol '+'.
	TYPE_APPROXIMATELY_SIGN = -3, // Represents an approximately sign symbol '~'.
	TYPE_PERCENT = -4, // Represents a percent sign symbol '%'.
	TYPE_PERMILLE = -5, // Represents a permille sign symbol '‰'.
	TYPE_CURRENCY_SINGLE = -6, // Represents a single currency symbol '¤'.
	TYPE_CURRENCY_DOUBLE = -7, // Represents a double currency symbol '¤¤'.
	TYPE_CURRENCY_TRIPLE = -8, // Represents a triple currency symbol '¤¤¤'.
	TYPE_CURRENCY_QUAD = -9, // Represents a quadruple currency symbol '¤¤¤¤'.
	TYPE_CURRENCY_QUINT = -10, // Represents a quintuple currency symbol '¤¤¤¤¤'.
	TYPE_CURRENCY_OVERFLOW = -15 // Represents a sequence of six or more currency symbols.
};
 ...
 *
 * A Modifier is usually immutable, except in cases such as {@link MutablePatternModifier}, which are mutable for
 *performance
 * reasons.
 *
...
public:
	 ...
	 * @param leftIndex
	 *            The left index of the string within the builder. Equal to 0 when only one number is being
	 *formatted.
	 * @param rightIndex
	 *            The right index of the string within the string builder. Equal to length when only one number is
	 *being
	 *            formatted.
	...
	/**
	 * Whether this modifier is strong. If a modifier is strong, it should always be applied immediately and not
	 *allowed
...
public:
	...
	/**
	 * Maps a magnitude to a multiplier in powers of ten. For example, in compact notation in English, a magnitude
	 *of 5
