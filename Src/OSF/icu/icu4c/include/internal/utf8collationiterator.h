...
public:
    UTF8CollationIterator(const CollationData *d, bool numeric,
                          const uint8 *s, int32_t p, int32_t len)
...
protected:
  ...
  * where handleGetTrailSurrogate() will return the matching trail surrogate.
    ...
    virtual bool forbidSurrogateCodePoints() const override;
...
public:
    FCDUTF8CollationIterator(const CollationData *data, bool numeric,
                             const uint8 *s, int32_t p, int32_t len)
...
private:
  ...
  * @return true if success
     */
    bool nextSegment(UErrorCode & errorCode);
  ...
  * @return true if success
    ...
    bool normalize(const UnicodeString & s, UErrorCode & errorCode);
