...
public:
	...
	/**
	 *  Find duplicate (redundant) character classes. Begin looking with categories.first. Duplicate, if found are
	 *returned in the categories parameter. This is an iterator-like function, used to identify character classes
	 *(state table columns) that can be eliminated.
	 *  @param categories in/out parameter, specifies where to start looking for duplicates, and returns the first
	 *pair of duplicates found, if any.
	 *  @return true if duplicate char classes were found, false otherwise.
	 */
	bool     findDuplCharClassFrom(IntPair * categories);

	/** Remove a column from the state table. Used when two character categories have been found equivalent, and
	   merged together, to eliminate the unneeded table column.
	 */
...
private:
	...
	/**
	 * Merge redundant state table columns, eliminating character classes with identical behavior. Done after the
	 *state tables are generated, just before converting to their run-time format.
	 */
	...
	/**
	 *  Find duplicate (redundant) states, beginning at the specified pair, within this state table. This is an
	 *iterator-like function, used to identify states (state table rows) that can be eliminated.
	 *  @param states in/out parameter, specifies where to start looking for duplicates, and returns the first pair
	 *of duplicates found, if any.
	 *  @return true if duplicate states were found, false otherwise.
	 */
	bool findDuplicateState(IntPair * states);

	/** Remove a duplicate state.
	 * @param duplStates The duplicate states. The first is kept, the second is removed. All references to the
	 *second in the state table are retargeted to the first.
	 */
	...
	bool    setEquals(UVector * a, UVector * b);
...
private:
	RBBIRuleBuilder  * fRB;
	RBBINode         *&fTree;          // The root node of the parse tree to build a table for.
	UErrorCode       * fStatus;
	/** State Descriptors, UVector<RBBIStateDescriptor> */
	UVector          * fDStates;       //  D states (Aho's terminology) Index is state number Contents are RBBIStateDescriptor pointers.
	...
	/* Counter used when assigning lookahead rule numbers. Contains the last look-ahead number already in use. The
	   first look-ahead number is 2; Number 1 (ACCEPTING_UNCONDITIONAL) is reserved for non-lookahead accepting
	   states. See the declarations of RBBIStateTableRowT.   */
...
public:
	...
	UVector          * fTagVals;
	int32_t fTagsIdx;
	UVector          * fPositions;     // Set of parse tree positions associated with this state.  Unordered (it's a set).
	                                   //   UVector contents are RBBINode *
	UVector32        * fDtran;         // Transitions out of this state. indexed by input character contents is int index of dest state in RBBITableBuilder.fDStates
