...
enum PatternSignType {
	PATTERN_SIGN_TYPE_POS, /** Render using normal positive subpattern rules */
	PATTERN_SIGN_TYPE_POS_SIGN, /** Render using rules to force the display of a plus sign */
	PATTERN_SIGN_TYPE_NEG, /** Render using negative subpattern rules */
	PATTERN_SIGN_TYPE_COUNT /** Count for looping over the possibilities */
};
...
private:
	struct U_I18N_API ParserState {
		...
		inline void toParseException(const char16_t* message) {
			(void)message;
		}
	} state;
...
public:
	 ...
	 *            Whether to leave out rounding information (minFrac, maxFrac, and rounding increment) when parsing
	 *the
	...
	/**
	 * Parses a pattern string into an existing property bag. All properties that can be encoded into a pattern
	 *string
	 * will be overwritten with either their default value or with the value coming from the pattern string.
	 *Properties
...
public:
	 ...
	 * example, in locale <em>fr-FR</em>, the period in the pattern "0.000" means "decimal" in standard notation (as
	 *it
	 * does in every other locale), but it means "grouping" in localized notation.
	 *
	 * <p>
	 * A greedy string-substitution strategy is used to substitute locale symbols. If two symbols are ambiguous or
	 *have
