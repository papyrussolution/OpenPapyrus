...
public:
	 ...
	 */
	int32_t formatAsPrefixSuffix(FormattedStringBuilder& result, int32_t startIndex, int32_t endIndex,
	 ...
	 */
	static int32_t formatTwoArgPattern(const SimpleFormatter& compiled, FormattedStringBuilder& result,
...
public:
	ConstantMultiFieldModifier(const FormattedStringBuilder &prefix,
...
public:
	/** Safe code path */
	CurrencySpacingEnabledModifier(const FormattedStringBuilder &prefix,
	    const FormattedStringBuilder &suffix,
	    bool overwrite,
	    bool strong,
	...
	/** Unsafe code path */
	static int32_t applyCurrencySpacing(FormattedStringBuilder &output, int32_t prefixStart, int32_t prefixLen,
...
private:
	...
	static UnicodeSet getUnicodeSet(const DecimalFormatSymbols &symbols, EPosition position, EAffix affix,
	    UErrorCode & status);

	static UnicodeString getInsertString(const DecimalFormatSymbols &symbols, EAffix affix, UErrorCode & status);
};
...
public:
	...
	void adoptModifier(Signum signum, StandardPlural::Form plural, const Modifier * mod) {
		assert(mods[getModIndex(signum, plural)] == nullptr);
	...
	void adoptModifierWithoutPlural(Signum signum, const Modifier * mod) {
		assert(mods[getModIndex(signum, DEFAULT_STANDARD_PLURAL)] == nullptr);
...
private:
	...
	inline static int32_t getModIndex(Signum signum, StandardPlural::Form plural) {
		assert(signum >= 0 && signum < SIGNUM_COUNT);
		assert(plural >= 0 && plural < StandardPlural::COUNT);
