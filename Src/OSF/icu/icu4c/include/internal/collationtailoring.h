...
struct U_I18N_API CollationTailoring : public SharedObject {
	 ...
	 */
	bool isBogus() { return settings == NULL; }
	bool ensureOwnedData(UErrorCode & errorCode);
...
struct U_I18N_API CollationCacheEntry : public SharedObject {
	CollationCacheEntry(const Locale &loc, const CollationTailoring * t) : validLocale(loc), tailoring(t) 
	{
		if(t) {
			t->addRef();
		}
