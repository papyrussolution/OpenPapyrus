...
U_CFUNC void ulocimp_getKeywords(const char * localeID, char prev, icu::ByteSink& sink, bool valuesToo, UErrorCode * status);
 ...
 * @param sink      the output sink receiving the BCP47 language tag for this Locale.
 * @param strict    boolean value indicating if the function returns an error for an ill-formed input locale ID.
 * @param err       error information if receiving the language tag failed.
 * @return          The length of the BCP47 language tag.
 *
 * @internal ICU 64
 */
U_CAPI void U_EXPORT2 ulocimp_toLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, UErrorCode * err);
/**
 ...
 * @param sink      the output sink receiving a locale ID for the specified BCP47 language tag.
 * @param parsedLength  if not NULL, successfully parsed length for the input language tag is set.
...
U_CFUNC bool ultag_isExtensionSubtags(const char * s, int32_t len);
U_CFUNC bool ultag_isLanguageSubtag(const char * s, int32_t len);
U_CFUNC bool ultag_isPrivateuseValueSubtags(const char * s, int32_t len);
U_CFUNC bool ultag_isRegionSubtag(const char * s, int32_t len);
U_CFUNC bool ultag_isScriptSubtag(const char * s, int32_t len);
...
U_CFUNC bool ultag_isUnicodeLocaleAttribute(const char * s, int32_t len);
...
U_CFUNC bool ultag_isUnicodeLocaleType(const char * s, int32_t len);
U_CFUNC bool ultag_isVariantSubtags(const char * s, int32_t len);
 ...
 * do {
 *     tempBuffer.requestedCapacity = uloc_doSomething(localeID, tempBuffer.getBuffer(), tempBuffer.getCapacity(),
 *&err);
...
public:
	...
	char * getBuffer() { return heapBuffer ? heapBuffer : stackBuffer; }
	...
	bool needToTryAgain(UErrorCode * err) 
	{
		...
		if(*err == U_BUFFER_OVERFLOW_ERROR || *err == U_STRING_NOT_TERMINATED_WARNING) {
			int32_t newCapacity = requestedCapacity + 2; // one for the terminating null, one just for
			                                             // paranoia
