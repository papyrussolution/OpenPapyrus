...
public:
	~EraRules();
	static EraRules* createInstance(const char * calType, bool includeTentativeEra, UErrorCode & status);
	/**
	 * Gets number of effective eras
	 * @return  number of effective eras
	 */
	inline int32_t getNumberOfEras() const { return numEras; }
	/**
	 * Gets start date of an era
	 * @param eraIdx    Era index
	 * @param fields    Receives date fields. The result includes values of year, month, day of month in this order.
	 *When an era has no start date, the result will be January 1st in year whose value is minimum integer.
	...
	/**
	 * Gets the current era index. This is calculated only once for an instance of EraRules. The current era
	 *calculation is based on the default time zone at the time of instantiation.
	 *
	 * @return era index of current era (or 0, when current date is before the first era)
	 */
	inline int32_t getCurrentEraIndex() const { return currentEra; }
private:
