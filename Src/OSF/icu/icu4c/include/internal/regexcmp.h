...
private:
	...
	int32_t blockTopLoc(bool reserve);      // Locate a position in the compiled pattern
	...
	void   compileInterval(int32_t InitOp,  // Generate the code for a {min,max} quantifier.
	    int32_t LoopOp);
	bool       compileInlineInterval();         // Generate inline code for a {min,max} quantifier
	void   literalChar(UChar32 c);          // Compile a literal char
	void   fixLiterals(bool split = false);// Generate code for pending literal characters.
...
private:
	...
	bool fSetModeFlag; // true for (?ismx, false for (?-ismx
