...
private:
  ...
  * need not re-encode the CEs multiple times.
    ...
    void addTailComposites(const UnicodeString & nfdPrefix, const UnicodeString & nfdString,
                           UErrorCode & errorCode);
    bool mergeCompositeIntoString(const UnicodeString & nfdString, int32_t indexAfterLastStarter,
                                   UChar32 composite, const UnicodeString & decomp,
                                   UnicodeString & newNFDString, UnicodeString & newString,
                                   UErrorCode & errorCode) const;

    bool ignorePrefix(const UnicodeString & s, UErrorCode & errorCode) const;
    bool ignoreString(const UnicodeString & s, UErrorCode & errorCode) const;
    bool isFCD(const UnicodeString & s, UErrorCode & errorCode) const;
    ...
    uint32_t addIfDifferent(const UnicodeString & prefix, const UnicodeString & str,
                            const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32,
                            UErrorCode & errorCode);
    static bool sameCEs(const int64_t ces1[], int32_t ces1Length,
  ...
  * The case bits are unused and available.
    ...
    static inline bool isTempCE(int64_t ce) {
    ...
    static inline bool isTempCE32(uint32_t ce32) {
        return
  ...
  * from a node with an explicit (root or default) weight.
    ...
    static inline bool nodeHasBefore2(int64_t node) {
        return (node & HAS_BEFORE2) != 0;
    }
    static inline bool nodeHasBefore3(int64_t node) {
        return (node & HAS_BEFORE3) != 0;
    }
    static inline bool nodeHasAnyBefore(int64_t node) {
        return (node & (HAS_BEFORE2 | HAS_BEFORE3)) != 0;
    }
    static inline bool isTailoredNode(int64_t node) {
