...
public:
	 ...
	 */
	CacheKeyBase(const CacheKeyBase &other) : UObject(other), fCreationStatus(other.fCreationStatus), fIsPrimary(false) 
	{
	}
	 ...
	 */
	virtual const SharedObject *createObject(const void * creationContext, UErrorCode & status) const = 0;
...
public:
	...
	virtual int32_t hashCode() const override {
		...
		return ustr_hashCharsN(s, static_cast<int32_t>(strlen(s)));
	}
...
public:
	...
	virtual const T *createObject(const void * creationContext, UErrorCode & status) const override;
	/**
...
public:
	 ...
	 */
	void setEvictionPolicy(int32_t count, int32_t percentageOfInUseItems, UErrorCode & status);
...
private:
	 ...
	 */
	void _get(const CacheKeyBase &key, const SharedObject *&value, const void * creationContext, UErrorCode & status) const;
	/**
	 ...
	 */
	bool _poll(const CacheKeyBase &key, const SharedObject *&value, UErrorCode & status) const;
	/**
	 ...
	 */
	void _putNew(const CacheKeyBase &key, const SharedObject * value, const UErrorCode creationStatus, UErrorCode & status) const;
	/**
	 ...
	 * threads notified.
	 */
	void _put(const UHashElement * element, const SharedObject * value, const UErrorCode status) const;
	/**
	 ...
	 */
	void _fetch(const UHashElement * element, const SharedObject *&value, UErrorCode & status) const;
	/**
	 * Determine if given hash entry is in progress.
	 * On entry, gCacheMutex must be held.
	 */
	bool _inProgress(const UHashElement * element) const;
	/**
	 * Determine if given hash entry is in progress.
	 * On entry, gCacheMutex must be held.
	 */
	bool _inProgress(const SharedObject * theValue, UErrorCode creationStatus) const;
	/**
	 * Determine if given hash entry is eligible for eviction.
	 * On entry, gCacheMutex must be held.
	 */
	bool _isEvictable(const UHashElement * element) const;
};
