...
public:
	...
	virtual ~BMPSet();
	virtual bool contains(UChar32 c) const;
	/*
...
private:
	...
	inline bool containsSlow(UChar32 c, int32_t lo, int32_t hi) const;
	bool latin1Contains[0x100]; // One byte 0 or 1 per Latin-1 character.
	bool containsFFFD; /* true if contains(U+FFFD). */
	/*
...
inline bool BMPSet::containsSlow(UChar32 c, int32_t lo, int32_t hi) const { return LOGIC(findCodePoint(c, lo, hi) & 1); }
