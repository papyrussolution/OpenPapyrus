...
public:
	...
	static UnicodeString getUnitPattern(const Locale & loc, const MeasureUnit& unit, UNumberUnitWidth width, StandardPlural::Form pluralForm, UErrorCode & status);
	static LongNameHandler* forCurrencyLongNames(const Locale &loc, const CurrencyUnit &currency, const PluralRules * rules,
	 ...
	 */
	static void forMeasureUnit(const Locale &loc, const MeasureUnit &unitRef, const UNumberUnitWidth &width,
	    const char * unitDisplayCase, const PluralRules * rules, const MicroPropsGenerator * parent, LongNameHandler * fillIn, UErrorCode & status);
	/**
...
private:
	SimpleModifier fModifiers[StandardPlural::Form::COUNT]; // A set of pre-computed modifiers, one for each plural form.
	const PluralRules * rules; // Not owned
	const MicroPropsGenerator * parent; // Not owned
	// Grammatical gender of the formatted result. Not owned: must point at
	// static or global strings.
	const char * gender = "";
	LongNameHandler(const PluralRules * rules, const MicroPropsGenerator * parent) : rules(rules), parent(parent) 
	{
	}
	LongNameHandler() : rules(nullptr), parent(nullptr) 
	{
	}
	friend class MemoryPool<LongNameHandler>; // Enables MemoryPool<LongNameHandler>::emplaceBack(): requires access to the private constructors.
	friend class NumberFormatterImpl; // Allow macrosToMicroGenerator to call the private default constructor.
	...
	static void processPatternTimes(MeasureUnitImpl &&productUnit, Locale loc, const UNumberUnitWidth &width,
	    const char * caseVariant, UnicodeString * outArray, UErrorCode & status);
