...
private:
	...
	int32_t resultCount;
	bool fStripTags; // If true, setText() will strip tags from input text.
	bool fFreshTextSet;
	static void setRecognizers(UErrorCode & status);

	bool * fEnabledRecognizers; // If not null, active set of charset recognizers had
...
public:
	...
	bool setStripTagsFlag(bool flag);
	bool getStripTagsFlag() const;
...
//    const char *getCharsetName(int32_t index, UErrorCode & status) const;
	...
	void setDetectableCharset(const char * encoding, bool enabled, UErrorCode & status);
};
