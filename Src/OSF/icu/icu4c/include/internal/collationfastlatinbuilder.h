...
private:
    ...
    bool loadGroups(const CollationData & data, UErrorCode & errorCode);
    bool inSameGroup(uint32_t p, uint32_t q) const;

    void resetCEs();
    void getCEs(const CollationData & data, UErrorCode & errorCode);
    bool getCEsFromCE32(const CollationData & data, UChar32 c, uint32_t ce32,
                         UErrorCode & errorCode);
    bool getCEsFromContractionCE32(const CollationData & data, uint32_t ce32,
                                    UErrorCode & errorCode);
    ...
    bool encodeUniqueCEs(UErrorCode & errorCode);
    bool encodeCharCEs(UErrorCode & errorCode);
    bool encodeContractions(UErrorCode & errorCode);
    uint32_t encodeTwoCEs(int64_t first, int64_t second) const;

    static bool isContractionCharCE(int64_t ce) {
