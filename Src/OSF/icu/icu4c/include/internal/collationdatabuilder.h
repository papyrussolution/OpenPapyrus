...
public:
  ...
  * Called for every non-special CE32 and every expansion CE.
    ...
    virtual bool isCompressibleLeadByte(uint32_t b) const;

    inline bool isCompressiblePrimary(uint32_t p) const {
  ...
  * @return true if this builder has mappings (e.g., add() has been called)
     */
    bool hasMappings() const { return modified; }

    /**
  * @return true if c has CEs in this builder
     */
    bool isAssigned(UChar32 c) const;
  ...
  * @return true if an OFFSET_TAG range was used for start..end
     */
    bool maybeSetPrimaryRange(UChar32 start, UChar32 end,
...
protected:
    ...
    uint32_t getCE32FromOffsetCE32(bool fromBase, UChar32 c, uint32_t ce32) const;
    ...
    static inline bool isBuilderContextCE32(uint32_t ce32) {
    ...
    uint32_t copyFromBaseCE32(UChar32 c, uint32_t ce32, bool withContext, UErrorCode & errorCode);
    /**
  ...
  * and returns the index of the last new item.
    ...
    bool getJamoCE32s(uint32_t jamoCE32s[], UErrorCode & errorCode);
