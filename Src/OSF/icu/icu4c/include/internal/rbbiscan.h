...
private:
	...
	bool fQuoteMode;          // Scan is in a 'quoted region'
	...
	bool fReverseRule;        // True if the rule currently being scanned
	//  is a reverse direction rule (if it
	//  starts with a '!')

	bool fLookAheadRule;     // True if the rule includes a '/'
	//   somewhere within it.
	bool fNoChainInRule;      // True if the current rule starts with a '^'.
