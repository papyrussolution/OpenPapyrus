...
private:
    ...
    static bool isValidKeyword(const UnicodeString & token);
...
class U_I18N_API FixedDecimal: public IFixedDecimal, public UObject {
   ...
   * @param c   Currently: an alias for param `e`.
    ...
    void init(double n);
    bool quickInit(double n);  // Try a fast-path only initialization,
...
public:
    ...
    bool negated = false;          // true for negated rules.
    bool integerOnly = false;      // true for 'within' rules.
    ...
    // bool isFulfilled(double number);
    bool isFulfilled(const IFixedDecimal &number);
};
...
public:
    ...
    bool           fDecimalSamplesUnbounded = false;
    bool           fIntegerSamplesUnbounded = false;
    // Internal error status, used for errors that occur during the copy constructor.
    UErrorCode      fInternalStatus = U_ZERO_ERROR;
