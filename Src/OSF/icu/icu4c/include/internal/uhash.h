...
struct UHashtable {
	...
	/* Function pointers */
	UHashFunction * keyHasher; /* Computes hash from key. Never null. */
	UKeyComparator * keyComparator; /* Compares keys for equality. Never null. */
 ...
 * @see uhash_openSize
 */
U_CAPI UHashtable * U_EXPORT2 uhash_init(UHashtable * hash,
 ...
 * @see uhash_openSize
 */
U_CAPI UHashtable * U_EXPORT2 uhash_initSize(UHashtable * hash,
    UHashFunction * keyHash,
    UKeyComparator * keyComp,
    UValueComparator * valueComp,
    int32_t size,
 ...
 * exist in the table.
 */
U_CAPI const UHashElement* U_EXPORT2 uhash_nextElement(const UHashtable * hash,
    int32_t * pos);
