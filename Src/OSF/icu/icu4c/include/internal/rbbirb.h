...
#include "uhash.h"
#include "unicode/symtable.h"// For UnicodeSet parsing, is the interface that looks up references to $variables within a set.
...
public:
	...
	RBBINode                ** fDefaultTree;// For rules not qualified with a ! the tree to which they belong to.
	bool fChainRules; // True for chained Unicode TR style rules. False for traditional regexp rules.
	bool fLBCMNoChain; // True:  suppress chaining of rules on chars with LineBreak property == CM.
	bool fLookAheadHardBreak; // True:  Look ahead matches cause an immediate break, no continuing for the longest match.
	RBBISetBuilder                * fSetBuilder; // Set and Character Category builder.
	UVector                       * fUSetNodes;  // Vector of all uset nodes.
	RBBITableBuilder              * fForwardTable;// State transition table, build time form.
	UVector                       * fRuleStatusVals;// The values that can be returned from getRuleStatus().
	RBBIDataHeader                * flattenData();// Create the flattened (runtime format) data tables..
private:
