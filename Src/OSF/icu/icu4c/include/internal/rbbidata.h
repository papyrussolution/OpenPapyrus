...
// set of RBBI run time data being shared by instances (clones) of RulesBasedBreakIterator.
//
 ...
 * @internal
 */
U_CAPI int32_t U_EXPORT2 ubrk_swap(const UDataSwapper * ds,
...
static const uint8 RBBI_DATA_FORMAT_VERSION[] = {6, 0, 0, 0};
...
typedef RBBIStateTableRowT<uint8> RBBIStateTableRow8;
typedef RBBIStateTableRowT<uint16> RBBIStateTableRow16;

constexpr uint16 ACCEPTING_UNCONDITIONAL = 1;   // Value constant for RBBIStateTableRow::fAccepting
...
public:
	...
	static bool          isDataVersionAcceptable(const UVersionInfo version);
