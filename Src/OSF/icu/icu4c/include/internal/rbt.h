...
/**
 * <code>RuleBasedTransliterator</code> is a transliterator built from a set of rules as defined for
 *Transliterator::createFromRules(). See the C++ class Transliterator documentation for the rule syntax.
 *
 * @author Alan Liu
...
private:
	/**
	 * The data object is immutable, so we can freely share it with other instances of RBT, as long as we do NOT own
	 *this object. TODO:  data is no longer immutable.  See bugs #1866, 2155
	 */
...
public:
	 ...
	 * @internal Use transliterator factory methods instead since this class will be removed in that release.
	 */
	/*RuleBasedTransliterator(const UnicodeString & id, const UnicodeString & rules, UTransDirection direction,
	   UnicodeFilter* adoptedFilter, UErrorCode & status);*/
	/**
...
private:
	 ...
	 */
	RuleBasedTransliterator(const UnicodeString & id, TransliterationRuleData* data, bool isDataAdopted);
public:
	/**
	 * Copy constructor.
	 * @internal Use transliterator factory methods instead since this class will be removed in that release.
...
public:
	/**
	 * Return a representation of this transliterator as source rules. These rules will produce an equivalent
	 *transliterator if used to construct a new transliterator.
	 * @param result the string to receive the rules.  Previous contents will be deleted.
	 * @param escapeUnprintable if true then convert unprintable character to their hex escape representations,
	 *\uxxxx or
	 * \Uxxxxxxxx.  Unprintable characters are those other than U+000A, U+0020..U+007E.
	 * @internal Use transliterator factory methods instead since this class will be removed in that release.
	 */
	virtual UnicodeString & toRules(UnicodeString & result, bool escapeUnprintable) const override;
protected:
	/**
...
public:
	...
	/**
	 * Return the class ID for this class.  This is useful only for comparing to a return value from
	 *getDynamicClassID().  For example:
	 * <pre>
	...
	/**
	 * Returns a unique class ID <b>polymorphically</b>.  This method is to implement a simple version of RTTI,
	 *since not all C++
	 * compilers support genuine RTTI.  Polymorphic operator == () and clone() methods call this method.
	 *
	 * @return The class ID for this object. All objects of a given class have the same class ID.  Objects of other
	 *classes have different class IDs.
	 */
