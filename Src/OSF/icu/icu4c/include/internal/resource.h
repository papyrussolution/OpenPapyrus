...
public:
	...
	ResourceArray(const uint16 * i16, const uint32_t * i32, int32_t len, const ResourceTracer& traceInfo) : items16(i16), items32(i32), length(len), fTraceInfo(traceInfo) 
	{
	}
	/**
...
public:
	...
	/** Only for implementation use. @internal */
	ResourceTable(const uint16 * k16, const int32_t * k32,
	    const uint16 * i16, const uint32_t * i32, int32_t len,
	 ...
	 * @return true if i is non-negative and less than getSize().
	 */
	bool getKeyAndValue(int32_t i, const char *&key, ResourceValue &value) const;
	 ...
	 */
	bool findValue(const char * key, ResourceValue &value) const;
...
public:
	 ...
	 */
	virtual bool isNoInheritanceMarker() const = 0;
