...
template <typename T> inline T * LocalMemory<T>::allocateInsteadAndReset(int32_t newCapacity) 
{
	if(newCapacity>0) {
		...
		if(p) {
			memzero(p, newCapacity*sizeof(T));
			uprv_free(LocalPointerBase<T>::ptr);
			LocalPointerBase<T>::ptr = p;
		}
		return p;
...
template <typename T, int32_t stackCapacity> icu::MaybeStackArray<T, stackCapacity>::MaybeStackArray(MaybeStackArray <T, stackCapacity>&& src) U_NOEXCEPT : 
...
template <typename T, int32_t stackCapacity> inline MaybeStackArray <T, stackCapacity>&MaybeStackArray<T,
    stackCapacity>::operator = (MaybeStackArray <T, stackCapacity>&& src) U_NOEXCEPT 
{
