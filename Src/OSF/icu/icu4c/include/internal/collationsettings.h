...
struct U_I18N_API CollationSettings : public SharedObject {
	...
	CollationSettings() : options((UCOL_DEFAULT_STRENGTH << STRENGTH_SHIFT) | (MAX_VAR_PUNCT << MAX_VARIABLE_SHIFT)),
		variableTop(0), reorderTable(NULL), minHighNoReorder(0), reorderRanges(NULL), reorderRangesLength(0),
		reorderCodes(NULL), reorderCodesLength(0), reorderCodesCapacity(0), fastLatinOptions(-1) 
	{
	}
	...
	inline bool operator !=(const CollationSettings &other) const { return !operator ==(other); }
	int32_t hashCode() const;
	void resetReordering();
	void aliasReordering(const CollationData & data, const int32_t * codes, int32_t length,
	    const uint32_t * ranges, int32_t rangesLength, const uint8 * table, UErrorCode & errorCode);
	...
	static int32_t getStrength(int32_t options) { return options >> STRENGTH_SHIFT; }
	...
	UColAttributeValue getFlag(int32_t bit) const { return ((options & bit) != 0) ? UCOL_ON : UCOL_OFF; }
	...
	UColAttributeValue getAlternateHandling() const { return ((options & ALTERNATE_MASK) == 0) ? UCOL_NON_IGNORABLE : UCOL_SHIFTED; }
	void setMaxVariable(int32_t value, int32_t defaultOptions, UErrorCode & errorCode);
	MaxVariable getMaxVariable() const { return (MaxVariable)((options & MAX_VARIABLE_MASK) >> MAX_VARIABLE_SHIFT); }
	/**
	 * Include case bits in the tertiary level if caseLevel=off and caseFirst!=off.
	 */
	static inline bool isTertiaryWithCaseBits(int32_t options) { return (options & (CASE_LEVEL | CASE_FIRST)) == CASE_FIRST; }
	...
	inline bool dontCheckFCD() const { return (options & CHECK_FCD) == 0; }
	inline bool hasBackwardSecondary() const { return (options & BACKWARD_SECONDARY) != 0; }
	inline bool isNumeric() const { return (options & NUMERIC) != 0; }
	/** CHECK_FCD etc. */
	int32_t options;
...
private:
	void setReorderArrays(const int32_t * codes, int32_t codesLength, const uint32_t * ranges, int32_t rangesLength, const uint8 * table, UErrorCode & errorCode);
