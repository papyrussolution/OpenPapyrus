...
public:
	 ...
	 * TimeZone API.
	 */
	virtual void getOffset(UDate date, bool local, int32_t& rawOffset,
	 ...
	 */
	virtual bool useDaylightTime() const override;

	/**
	 * TimeZone API.
	 */
	virtual bool inDaylightTime(UDate date, UErrorCode & ec) const override;
	 ...
	 */
	virtual bool hasSameRules(const TimeZone& other) const override;
	 ...
	 */
	virtual bool getNextTransition(UDate base, bool inclusive, TimeZoneTransition& result) const override;
	 ...
	 * @return  true if the transition is found.
	 */
	virtual bool getPreviousTransition(UDate base, bool inclusive, TimeZoneTransition& result) const override;
...
private:
	void constructEmpty();
	void getHistoricalOffset(UDate date, bool local, int32_t NonExistingTimeOpt, int32_t DuplicatedTimeOpt, int32_t& rawoff, int32_t& dstoff) const;
...
inline int16 OlsonTimeZone::transitionCount() const { return transitionCountPre32 + transitionCount32 + transitionCountPost32; }
inline double OlsonTimeZone::transitionTime(int16 transIdx) const { return (double)transitionTimeInSeconds(transIdx) * U_MILLIS_PER_SECOND; }

inline int32_t OlsonTimeZone::zoneOffsetAt(int16 transIdx) const 
{
	int16 typeIdx = (transIdx >= 0 ? typeMapData[transIdx] : 0) << 1;
	return typeOffsets[typeIdx] + typeOffsets[typeIdx + 1];
}

inline int32_t OlsonTimeZone::rawOffsetAt(int16 transIdx) const 
{
	int16 typeIdx = (transIdx >= 0 ? typeMapData[transIdx] : 0) << 1;
	return typeOffsets[typeIdx];
}

inline int32_t OlsonTimeZone::dstOffsetAt(int16 transIdx) const 
{
	int16 typeIdx = (transIdx >= 0 ? typeMapData[transIdx] : 0) << 1;
