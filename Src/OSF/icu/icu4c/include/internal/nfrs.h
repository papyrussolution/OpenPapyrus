...
public:
    ...
    bool isFractionRuleSet() const { return fIsFractionRuleSet; }

    void  getName(UnicodeString & result) const { result.setTo(name); }
    bool isNamed(const UnicodeString & _name) const { return this->name == _name; }
    ...
    bool parse(const UnicodeString & text, ParsePosition& pos, double upperBound, uint32_t nonNumericalExecutedRuleMask, Formattable& result) const;
