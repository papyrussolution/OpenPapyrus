...
public:
    ...
    bool isFieldEmpty(int32_t field) const;
...
public:
    ...
    bool         skeletonWasSpecified; // if specified in availableFormats, not derived
...
public:
    ...
    bool isPatternSeparator(const UnicodeString & field) const;
    static bool isQuoteLiteral(const UnicodeString & s);
    static int32_t getCanonicalIndex(const UnicodeString & s) { return getCanonicalIndex(s, true); }
    static int32_t getCanonicalIndex(const UnicodeString & s, bool strict);
...
public:
    ...
    bool equals(const DateTimeMatcher* other) const;
...
public:
    ...
    PatternMap();
    virtual  ~PatternMap();
    void  add(const UnicodeString & basePattern, const PtnSkeleton& skeleton, const UnicodeString & value, bool skeletonWasSpecified, UErrorCode & status);
    const UnicodeString * getPatternFromBasePattern(const UnicodeString & basePattern, bool& skeletonWasSpecified) const;
    ...
    bool equals(const PatternMap& other) const;
private:
    bool isDupAllowed;
...
private:
    int32_t pos;
    bool isCanonicalItem(const UnicodeString & item);
