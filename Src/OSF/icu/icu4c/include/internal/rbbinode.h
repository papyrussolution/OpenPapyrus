...
//  class RBBINode
//
//    Represents a node in the parse tree generated when reading a rule file.
//
U_NAMESPACE_BEGIN
...
public:
	...
	bool fNullable;                    // See Aho.
	int32_t fVal;                 // For leafChar nodes, the value.
	//   Values are the character category,
	//   corresponds to columns in the final
	//   state transition table.
	bool fLookAheadEnd;                // For endMark nodes, set true if marking the end of a look-ahead rule.
	bool fRuleRoot;                    // True if this node is the root of a rule.
	bool fChainIn;                     // True if chaining into this rule is allowed (no '^' present).
	...
	static void printTree(const RBBINode * n, bool withHeading);
#endif
private:
