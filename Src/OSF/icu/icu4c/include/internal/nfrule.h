...
public:
	...
	bool doParse(const UnicodeString & text, ParsePosition& pos, bool isFractional, double upperBound, uint32_t nonNumericalExecutedRuleMask, Formattable& result) const;
	bool shouldRollBack(int64_t number) const;
	void _appendRuleText(UnicodeString & result) const;
	int32_t findTextLenient(const UnicodeString & str, const UnicodeString & key, int32_t startingAt, int32_t* resultCount) const;
...
private:
	...
	double matchToDelimiter(const UnicodeString & text, int32_t startPos, double baseValue,
	    ...
	    uint32_t nonNumericalExecutedRuleMask, double upperBound) const;
	...
	bool allIgnorable(const UnicodeString & str, UErrorCode & status) const;
	int32_t findText(const UnicodeString & str, const UnicodeString & key, int32_t startingAt, int32_t* resultCount) const;
