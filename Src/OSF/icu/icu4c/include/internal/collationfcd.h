...
public:
    static inline bool hasLccc(UChar32 c) {
        // assert c <= 0xffff
    ...
    static inline bool hasTccc(UChar32 c) {
        // assert c <= 0xffff
        // c can be negative, e.g., U_SENTINEL from UCharIterator;
        // that is handled in the first test.
        int32_t i;
    ...
    static inline bool mayHaveLccc(UChar32 c) {
  ...
  * @return true if c is U+0F73, U+0F75 or U+0F81 or one of several other Tibetan characters
     */
    static inline bool maybeTibetanCompositeVowel(UChar32 c) {
