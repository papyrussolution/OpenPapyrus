...
public:
	...
	CompactUnicodeString(const UnicodeString & text, UErrorCode & status) : fBuffer(text.length() + 1, status) 
	{
		if(U_FAILURE(status)) {
			return;
		}
...
public:
	...
	/**
	 * The index of the last char consumed during parsing. If parsing started at index 0, this is equal to the
	 *number of chars consumed. This is NOT necessarily the same as the StringSegment offset;
	 ...
	 * after reading the number, even if there is other content following the number. For example, after parsing the
	 *string "123 " (123 followed by a space), the cursor should be set to 3, not 4, even though there are matchers
	 *that accept whitespace. In this example, the digits are strong, whereas the whitespace is weak. Grouping
	 *separators are weak, whereas decimal separators are strong. Most other chars are strong.
	 *
 ...
 *
 * Given a string, there should NOT be more than one way to consume the string with the same matcher applied multiple
 *times. If there is, the non-greedy parsing algorithm will be unhappy and may enter an exponential-time loop. For
 *example, consider the "A Matcher" that accepts "any number of As". Given the string "AAAA", there are 2^N = 8 ways to
 *apply the A Matcher to this string: you could have the A Matcher apply 4 times to each character; you could have it
 *apply just once to all the characters; you could have it apply to the first 2 characters and the second 2 characters;
 *and so on. A better version of the "A Matcher" would be for it to accept exactly one A, and allow the algorithm to run
 *it repeatedly to consume a string of multiple As. The A Matcher can implement the Flexible interface below to signal
 *that it can be applied multiple times in a row.
 *
 * @author sffc
 */
// Exported as U_I18N_API for tests
...
public:
	 ...
	 *
	 * @param segment The StringSegment to match against. Matches always start at the beginning of the segment. The
	 *segment is guaranteed to contain at least one char.
	...
	/**
	 * Performs a fast "smoke check" for whether or not this matcher could possibly match against the given string
	 *segment. The test should be as fast as possible but also as restrictive as possible. For example, matchers can
	 *maintain a UnicodeSet of all code points that count possibly start a match. Matchers should use the {@link
	 *StringSegment#startsWith} method in order to correctly handle case folding.
	 *
	 * @param segment The segment to check against.
	 * @return true if the matcher might be able to match against this segment; false if it definitely will not be
	 *able to match.
	 */
	virtual bool smokeTest(const StringSegment& segment) const = 0;
	/**
	 * Method called at the end of a parse, after all matchers have failed to consume any more chars. Allows a
	 *matcher to make final modifications to the result given the knowledge that no more matches are possible.
	 *
