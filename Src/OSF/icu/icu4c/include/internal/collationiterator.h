...
private:
    ...
    public:
        ...
        bool ensureAppendCapacity(int32_t appCap, UErrorCode & errorCode);

        inline bool incLength(UErrorCode & errorCode) {
...
public:
    CollationIterator(const CollationData *d, bool numeric)
            : trie(d->trie),
              data(d),
              cesIndex(0),
              skipped(NULL),
...
protected:
  ...
  * (Not needed in Java.)
     */
    virtual bool foundNULTerminator();
  ...
  *         or true if they map to CE(U+FFFD) (for UTF-8)
     */
    virtual bool forbidSurrogateCodePoints() const;
...
private:
  ...
  * The digits string must not be empty and must not have leading zeros.
     */
    void appendNumericCEs(uint32_t ce32, bool forward, UErrorCode & errorCode);
