...
public:
	...
	~DictionaryCache();
	void reset();
	bool following(int32_t fromPos, int32_t * pos, int32_t * statusIndex);
	bool preceding(int32_t fromPos, int32_t * pos, int32_t * statusIndex);
	/**
...
public:
	 ...
	 */
	bool populateNear(int32_t position, UErrorCode & status);
	 ...
	 */
	bool populatePreceding(UErrorCode & status);
	 ...
	 */
	bool seek(int32_t startPosition);
	void dumpCache();
private:
