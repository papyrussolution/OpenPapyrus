...
void TestIDNA::testAPI(const UChar * src, const UChar * expected, const char * testName,
	...
	if(status == U_BUFFER_OVERFLOW_ERROR) {
		...
		if(destLen+1 < MAX_DEST_SIZE) {
			...
			if(U_SUCCESS(status) && expectedStatus != U_IDNA_STD3_ASCII_RULES_ERROR&& (doCompare==TRUE) &&
			    u_strCaseCompare(dest, destLen, expected, expectedLen, 0, &status)!=0) {
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) {
			...
			if(destLen+1 < MAX_DEST_SIZE) {
				...
				if(U_SUCCESS(status) && (doCompare==TRUE) &&
				    u_strCaseCompare(dest, destLen, expected, expectedLen, 0, &status)!=0) {
					//errln("Did not get the expected result for %s null terminated source with both
					// options set.\n",testName);
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) {
			...
			if(destLen+1 < MAX_DEST_SIZE) {
				...
				if(U_SUCCESS(status) && (doCompare==TRUE) &&
					...
					//errln("Did not get the expected result for %s null terminated source with both
					// options set.\n",testName);
					errln("Did not get the expected result for "+UnicodeString(
						    testName) +" null terminated source with both options set. Expected: "+
					    prettify(UnicodeString(expected, expectedLen)));
				}
			}
			else {
...
void TestIDNA::testIDNToUnicode(const char * testName, TestFunc func) {
	...
	int32_t bufLen = 0;
	UParseError parseError;
	for(i = 0; i< SIZEOFARRAYi(domainNames); i++) {
...
/*
   void TestIDNA::testConformance(const char * toASCIIName, TestFunc toASCII,
    ...
    for(int32_t i=0;i< SIZEOFARRAYi(conformanceTestCases);i++) {
        ...
        if(U_FAILURE(status)) {
            errln(UnicodeString("Conversion of UTF8 source in conformanceTestCases[") + i +UnicodeString( "].in (
               ")+prettify(utf8Chars1) +UnicodeString(" ) failed. Error: ")+ UnicodeString(u_errorName(status)));
            continue;
        }
        if(utf8Chars2 != NULL) {
...
void TestIDNA::testChaining(const UChar * src, int32_t numIterations, const char * testName,
	...
	for(; i<=numIterations; i++) {
		if((i%2) ==0) {
			...
			if(U_FAILURE(status)) {
				errcheckln(status, "%s source with source length and options set failed - %s", testName,
				    u_errorName(status));
				break;
			}
		}
			...
			if(U_FAILURE(status)) {
				errcheckln(status,
				    "%s  source with source length and options set failed - %s",
				    testName,
				    u_errorName(status));
				break;
			}
...
void TestIDNA::runIndexedTest(int32_t index, bool exec, const char *& name, char * par)
{
...
void TestIDNA::TestIDNAMonkeyTest() {
	...
	/* for debugging */
	for(i = 0; i<SIZEOFARRAYi(failures); i++) {
		source.truncate(0);
	...
	{ // test deletion of code points
		UnicodeString source(
			"\\u043f\\u00AD\\u034f\\u043e\\u0447\\u0435\\u043c\\u0443\\u0436\\u0435\\u043e\\u043d\\u0438\\u043d\\u0435\\u0433\\u043e\\u0432\\u043e\\u0440\\u044f\\u0442\\u043f\\u043e\\u0440\\u0443\\u0441\\u0441\\u043a\\u0438\\u0000",
			-1,
			US_INV);
		source = source.unescape();
		UnicodeString expected(
			"\\u043f\\u043e\\u0447\\u0435\\u043c\\u0443\\u0436\\u0435\\u043e\\u043d\\u0438\\u043d\\u0435\\u0433\\u043e\\u0432\\u043e\\u0440\\u044f\\u0442\\u043f\\u043e\\u0440\\u0443\\u0441\\u0441\\u043a\\u0438\\u0000",
			-1,
			US_INV);
...
void TestIDNA::TestCompareReferenceImpl() {
	...
	if(U_FAILURE(dataStatus)) {
		dataerrln("Couldn't load test data: %s\n", u_errorName(dataStatus)); // save us from thousands and
		                                                                     // thousands of errors
		return;
	}
	uidna_close(uidna_openUTS46(0, &dataStatus)); // Fail quickly if no data.
	if(!assertSuccess("", dataStatus, true, __FILE__, __LINE__)) {
		return;
	}
