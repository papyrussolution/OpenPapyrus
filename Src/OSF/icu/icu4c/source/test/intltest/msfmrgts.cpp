...
bool MessageFormatRegressionTest::failure(UErrorCode status, const char * msg, bool possibleDataError)
{
	if(U_FAILURE(status)) {
...
void MessageFormatRegressionTest::Test4104976()
{
	...
	int32_t formats_length = SIZEOFARRAYi(formats);
	UErrorCode status = U_ZERO_ERROR;
	...
	for(int i = 0; i < 5; i++) {
		...
		if(objs == NULL)
			...
			if(objs[0].getType() == Formattable::kString)
				...
				logln((UnicodeString)"  " +
				    (objs[0].getType() == Formattable::kLong ? objs[0].getLong() : objs[0].getDouble()));
			delete[] objs;
		}
	}
