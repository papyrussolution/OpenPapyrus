...
bool CalendarRegressionTest::failure(UErrorCode status, const char * msg)
{
	if(U_FAILURE(status)) {
 ...
 * bug 4100311
 */
void CalendarRegressionTest::test4100311()
{
 ...
 * @bug 4074758
 */
void CalendarRegressionTest::test4074758()
...
void CalendarRegressionTest::test4028518()
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarRegressionTest::Test9019()
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarRegressionTest::printdate(GregorianCalendar * cal, const char * string)
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 * @bug 4031502
 */
void CalendarRegressionTest::test4031502()
{
 ...
 */
UDate CalendarRegressionTest::getAssociatedDate(UDate d, UErrorCode & status)
{
	GregorianCalendar * cal = new GregorianCalendar(status);
	cal->setTime(d, status);
...
void CalendarRegressionTest::dowTest(bool lenient)
{
	UErrorCode status = U_ZERO_ERROR;
	GregorianCalendar * cal = new GregorianCalendar(status);
	if(U_FAILURE(status)) {
 ...
 */
void CalendarRegressionTest::Test4149677()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarRegressionTest::Test4162587()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarRegressionTest::Test4165343()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarRegressionTest::Test4166109()
{
	/* Test month:
	 *
	 *      March 1998
 ...
 */
void CalendarRegressionTest::Test4167060()
{
	UErrorCode status = U_ZERO_ERROR;
		...
		for(int32_t j = 0; j < 3; ++j) {
			...
			for(int32_t i = 0; i < 4; i++) {
				...
				if(valid && newYear != years[i]) {
					errln(UnicodeString(
						    "  FAIL: ") + newYear + " should be valid; date, month and time shouldn't change");
				}
...
UDate CalendarRegressionTest::makeDate(int32_t y, int32_t m, int32_t d,
...
void CalendarRegressionTest::TestDeprecates(void)
{
	...
	c1->setTime(c2->getTime(status), status);
	c1->roll(Calendar::HOUR, (bool)FALSE, status);
	c2->roll(UCAL_HOUR, (bool)FALSE, status);

	if(U_FAILURE(status)) {
		errln("Error code when trying to roll(bool)");
	}
	else if(*c1 != *c2) {
		errln("roll(EDateField, bool) had different effect than roll(UCalendarField, bool)");
	}
