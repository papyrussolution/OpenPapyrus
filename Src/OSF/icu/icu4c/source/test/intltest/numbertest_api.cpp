...
void NumberFormatterApiTest::runIndexedTest(int32_t index, bool exec, const char *& name, char *) {
	if(exec) {
...
//            u"$5.43");
	...
	assertFormatSingle(
		...
		Locale("en-US"),
		3.65,
		UnicodeString("3 yd, 1 ft, 11.4 in"));
	...
	assertFormatSingle(u"Mixed Unit (Not Sorted) [imperial full integers]",                //
	    ...
	    Locale("en-US"),                                                                   //
	    4.36112,                                                                           //
	    UnicodeString(u"1 inch, 4 yards, 1 foot"));                                        //
	...
	assertFormatSingle(u"Mixed Unit (Not Sorted) [imperial full] with `And` in the end",   //
	    ...
	    Locale("fr-FR"),                                                                   //
	    4.38571,                                                                           //
	    UnicodeString(u"1,88556\u00A0pouce, 4\u00A0yards et 1\u00A0pied"));                //
	...
	assertFormatSingle(u"Mixed unit, Scientific [Not in Order]",                           //
	    ...
	    Locale("en-US"),                                                                   //
	    3.65,                                                                              //
	    UnicodeString("1 ft, 1.14E1 in, 3 yd"));                                                          //
...
void NumberFormatterApiTest::unitUsageErrorCodes() {
	...
	unloc_formatter = NumberFormatter::forSkeleton(UnicodeString(u"unit/foobar"), status);
	// This gives an error, because foobar is an invalid unit:
	status.expectErrorAndReset(U_NUMBER_SKELETON_SYNTAX_ERROR);

	unloc_formatter = NumberFormatter::forSkeleton(UnicodeString(u"usage/foobar"), status);
...
void NumberFormatterApiTest::unitUsageSkeletons() {
	...
	assertFormatSingle(
		...
		Locale("en-ZA"),
		987654321,
		UnicodeString(u"990 thousand km"));
	...
	assertFormatSingle(
		...
		Locale("en-ZA"),
		1e20,
		UnicodeString(u"1,5E28 kilometres"));
...
//                    null).unit(GBP), Locale::getEnglish(), 1234567.89, u"1234568 British pounds");
	...
	assertFormatSingle(
		...
		Locale("ca"),
		444444.55,
		UnicodeString(u"444.444,55 USD"));
	...
	assertFormatSingle(
		...
		Locale("pt-PT"),
		444444.55,
		UnicodeString(u"444,444$55 PTE"));
	...
	assertFormatSingle(
		...
		Locale("ro-RO"),
		24,
		UnicodeString(u"24,00 lei românești"));
...
void NumberFormatterApiTest::runUnitInflectionsTestCases(UnlocalizedNumberFormatter unf, UnicodeString skeleton,
    const UnitInflectionTestCase * cases, int32_t numCases, IcuTestErrorCode &status) 
{
	for(int32_t i = 0; i < numCases; i++) {
		...
		;
		UnicodeString skelString = UnicodeString("unit/") + UnicodeString(t.unitIdentifier) + UnicodeString(u" ") + skeleton;
		const char16_t * skel;
		...
		assertFormatSingle((UnicodeString("Unit: \"") + UnicodeString(t.unitIdentifier) + UnicodeString("\", \"") + skeleton +
		    UnicodeString(u"\", locale=\"") + UnicodeString(t.locale) + UnicodeString(u"\", case=\"") +
		    UnicodeString(t.unitDisplayCase ? t.unitDisplayCase : "") + UnicodeString(u"\", value=") + t.value)
		    .getTerminatedBuffer(),
		    skel, skel, unf, Locale(t.locale), t.value, UnicodeString(t.expected));
		status.assertSuccess();
	}
}
 ...
 * U_UNSUPPORTED_ERROR.
 */
void NumberFormatterApiTest::assertFormatDescending(const char16_t* umessage,
 ...
 * U_UNSUPPORTED_ERROR.
 */
void NumberFormatterApiTest::assertFormatDescendingBig(const char16_t* umessage,
    const char16_t* uskeleton,
    const char16_t* conciseSkeleton,
    const UnlocalizedNumberFormatter& f,
    Locale locale,
 ...
 * U_UNSUPPORTED_ERROR.
 */
FormattedNumber NumberFormatterApiTest::assertFormatSingle(const char16_t* umessage,
    const char16_t* uskeleton,
    const char16_t* conciseSkeleton,
    const UnlocalizedNumberFormatter& f,
    Locale locale,
