...
UnicodeString printOneEdit(const Edits::Iterator &ei) {
	if(ei.hasChange()) {
		return UnicodeString() + ei.oldLength() + UnicodeString(u"->") + ei.newLength();
	}
	else {
		return UnicodeString() + ei.oldLength() + UnicodeString(u"=") + ei.newLength();
	}
...
bool TestUtility::checkEqualEdits(IntlTest &test, const UnicodeString & name, const Edits &e1, const Edits &e2, UErrorCode &errorCode) 
{
	Edits::Iterator ei1 = e1.getFineIterator();
	Edits::Iterator ei2 = e2.getFineIterator();
	bool ok = TRUE;
	for(int32_t i = 0; ok; ++i) {
		bool ei1HasNext = ei1.next(errorCode);
		bool ei2HasNext = ei2.next(errorCode);
		ok &= test.assertEquals(name + UnicodeString(u" next()[") + i + UnicodeString(u"]") + __LINE__,
			ei1HasNext, ei2HasNext);
		ok &= test.assertSuccess(name + UnicodeString(u" errorCode[") + i + UnicodeString(u"]") + __LINE__, errorCode);
		ok &= test.assertEquals(name + UnicodeString(u" edit[") + i + UnicodeString(u"]") + __LINE__,
...
void TestUtility::checkEditsIter(IntlTest &test, const UnicodeString & name,
    Edits::Iterator ei1, Edits::Iterator ei2,      // two equal iterators
    const EditChange expected[], int32_t expLength, bool withUnchanged,
    UErrorCode &errorCode) 
{
	test.assertFalse(name + UnicodeString(u":") + __LINE__, ei2.findSourceIndex(-1, errorCode));
	test.assertFalse(name + UnicodeString(u":") + __LINE__, ei2.findDestinationIndex(-1, errorCode));
	...
	for(int32_t expIndex = 0; expIndex < expLength; ++expIndex) {
		...
		if(withUnchanged || expect.change) {
			test.assertTrue(msg + UnicodeString(u":") + __LINE__, ei1.next(errorCode));
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.change, ei1.hasChange());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.oldLength, ei1.oldLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.newLength, ei1.newLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expSrcIndex, ei1.sourceIndex());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expDestIndex, ei1.destinationIndex());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expReplIndex, ei1.replacementIndex());
		}

		if(expect.oldLength > 0) {
			test.assertTrue(msg + UnicodeString(u":") + __LINE__, ei2.findSourceIndex(expSrcIndex, errorCode));
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.change, ei2.hasChange());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.oldLength, ei2.oldLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.newLength, ei2.newLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expSrcIndex, ei2.sourceIndex());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expDestIndex, ei2.destinationIndex());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expReplIndex, ei2.replacementIndex());
			if(!withUnchanged) {
		...
		if(expect.newLength > 0) {
			test.assertTrue(msg + UnicodeString(u":") + __LINE__, ei2.findDestinationIndex(expDestIndex, errorCode));
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.change, ei2.hasChange());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.oldLength, ei2.oldLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expect.newLength, ei2.newLength());
			test.assertEquals(msg + UnicodeString(u":") + __LINE__, expSrcIndex, ei2.sourceIndex());
	...
	test.assertFalse(msg + UnicodeString(u":") + __LINE__, ei1.next(errorCode));
	test.assertFalse(msg + UnicodeString(u":") + __LINE__, ei1.hasChange());
	test.assertEquals(msg + UnicodeString(u":") + __LINE__, 0, ei1.oldLength());
	test.assertEquals(msg + UnicodeString(u":") + __LINE__, 0, ei1.newLength());
	test.assertEquals(msg + UnicodeString(u":") + __LINE__, expSrcIndex, ei1.sourceIndex());
	test.assertEquals(msg + UnicodeString(u":") + __LINE__, expDestIndex, ei1.destinationIndex());
	test.assertEquals(msg + UnicodeString(u":") + __LINE__, expReplIndex, ei1.replacementIndex());
	test.assertFalse(name + UnicodeString(u":") + __LINE__, ei2.findSourceIndex(expSrcIndex, errorCode));
	test.assertFalse(name + UnicodeString(u":") + __LINE__, ei2.findDestinationIndex(expDestIndex, errorCode));
	...
	for(auto i = 0; i < (int32_t)srcIndexes.size(); ++i) {
		for(int32_t ij = 0; ij < SIZEOFARRAYi(ZIG_ZAG); ++ij) {
			int32_t j = ZIG_ZAG[ij];
			if((i + j) < (int32_t)srcIndexes.size()) {
				int32_t si = srcIndexes[i + j];
				test.assertEquals(name + UnicodeString(u" destIndexFromSrc(") + si + UnicodeString(u"):") + __LINE__,
	...
	for(auto i = 0; i < (int32_t)destIndexes.size(); ++i) {
		for(int32_t ij = 0; ij < SIZEOFARRAYi(ZIG_ZAG); ++ij) {
			...
			if((i + j) < (int32_t)destIndexes.size()) {
				...
				test.assertEquals(name + UnicodeString(u" srcIndexFromDest(") + di + UnicodeString(u"):") + __LINE__,
