...
private:
	bool dataDriven(const TestCase &test, IcuTestErrorCode &errorCode);
};
...
void LocaleMatcherTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
	if(exec) {
...
void LocaleMatcherTest::testBasics() {
	...
	}
	{
		...
		LocaleMatcher matcher = LocaleMatcher::Builder().
			...
			ARRAY_RANGE(pointers), [](const Locale * p) -> const Locale & {
			return *p;
		}).
		    build(errorCode);
		const Locale * best = matcher.getBestMatch("en_GB", errorCode);
...
bool LocaleMatcherTest::dataDriven(const TestCase &test, IcuTestErrorCode &errorCode) {
	LocaleMatcher::Builder builder;
	...
	if(test.expDesired.isEmpty() && test.expCombined.isEmpty()) {
		...
		if(desired.getLength() == 1) {
			...
			bool ok = assertEquals("bestSupported from Locale",
				locString(expMatch), locString(bestSupported));
		...
		bool ok = assertEquals("result.getSupportedLocale from Locales",
			locString(expMatch), locString(result.getSupportedLocale()));
...
void LocaleMatcherTest::testDataDriven() {
	...
	while((p = ucbuf_readline(f.getAlias(), &lineLength, errorCode)) != nullptr &&
		...
		bool ok = dataDriven(test, errorCode);
