...
class RBBIMonkeyTest: public IntlTest {
  ...
  private:
    ...
    static bool getIntParam(UnicodeString name, UnicodeString & params, int64_t &val, UErrorCode & status);
    static bool getStringParam(UnicodeString name, UnicodeString & params, CharString &dest, UErrorCode & status);
    static bool getBoolParam(UnicodeString name, UnicodeString & params, bool &dest, UErrorCode & status);
...
class RBBIMonkeyImpl: public UObject {
  public:
    ...
    bool              fVerbose;                 // True to do long dump of failing data.
    int32_t fLoopCount;

    bool              fDumpExpansions;          // Debug flag to output epananded form of rules and sets.
