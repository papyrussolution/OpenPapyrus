...
void SelectFormatTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void SelectFormatTest::selectFormatUnitTest(/*char *par*/)
{
	...
	for(int32_t i = 0; i<SELECT_SYNTAX_DATA; ++i) {
		...
		if(U_SUCCESS(status)) {
			errln(
				"\nERROR: Unexpected result - SelectFormat Unit Test failed to detect syntax error with pattern: "+
				checkSyntaxData[i]);
		}
	}
	...
	for(int32_t i = 0; i<SELECT_PATTERN_DATA; ++i) {
		...
		for(int32_t j = 0; j<3; j++) {
			...
			if(U_FAILURE(status)) {
				errln("ERROR: SelectFormat Unit test failed in format() with argument: "+ formatArgs[j] + " and error is " + u_errorName(
					    status));
			}
			else {
