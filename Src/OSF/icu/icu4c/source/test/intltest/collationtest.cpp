...
private:
	...
	static bool isSectionStarter(char16_t c) {
		return c == 0x25 || c == 0x2a || c == 0x40;
	}                                                                                   // %*@
	...
	bool readNonEmptyLine(UCHARBUF * f, IcuTestErrorCode &errorCode);
	...
	bool needsNormalization(const UnicodeString & s, UErrorCode &errorCode) const;

	bool getSortKeyParts(const char16_t * s, int32_t length,
	    CharString &dest, int32_t partSize,
	    IcuTestErrorCode &errorCode);
	bool getCollationKey(const char * norm, const UnicodeString & line,
	    const char16_t * s, int32_t length,
	    CollationKey &key, IcuTestErrorCode &errorCode);
	bool getMergedCollationKey(const char16_t * s, int32_t length,
	    CollationKey &key, IcuTestErrorCode &errorCode);
	bool checkCompareTwo(const char * norm, const UnicodeString & prevFileLine,
...
void CollationTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
	if(exec) {
...
void CollationTest::TestIllegalUTF8() {
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(strings); i += 2) {
...
public:
	...
	UChar32 next() {
		return (pos < length) ? cp[pos++] : U_SENTINEL;
	}

	UChar32 previous() {
		return (pos > 0) ? cp[--pos] : U_SENTINEL;
	}
...
void CollationTest::TestFCD() {
	...
	CodePointIterator cpi(cp, SIZEOFARRAYi(cp));
	...
	cpi.resetToStart();
	UCharIterator iter;
	uiter_setString(&iter, s, SIZEOFARRAYi(s) - 1); // -1: without the terminating NUL
...
namespace {
bool isValidCE(const CollationRootElements &re, const CollationData &data,
...
bool isValidCE(const CollationRootElements &re, const CollationData &data, int64_t ce) {
...
bool CollationTest::readNonEmptyLine(UCHARBUF * f, IcuTestErrorCode &errorCode) {
	for(;;) {
		int32_t lineLength;
		...
		if(comment != NULL) {
			...
			while(lineLength > 0 && isCROrLF(line[lineLength - 1])) {
				--lineLength;
			}
		}
...
void CollationTest::parseAndSetAttribute(IcuTestErrorCode &errorCode) {
	...
	UColAttributeValue value;
	for(int32_t i = 0;; ++i) {
		if(i == SIZEOFARRAYi(attributeValues)) {
			errln("invalid attribute value name on line %d", (int)fileLineNumber);
			infoln(fileLine);
			errorCode.set(U_PARSE_ERROR);
			return;
...
void CollationTest::buildTailoring(UCHARBUF * f, IcuTestErrorCode &errorCode) {
	...
	if(errorCode.isFailure()) {
		...
		if(parseError.offset >= 0) {
			infoln("  rules offset: %d", (int)parseError.offset);
		}
...
bool CollationTest::needsNormalization(const UnicodeString & s, UErrorCode &errorCode) const {
	if(U_FAILURE(errorCode) || !fcd->isNormalized(s, errorCode)) {
		return TRUE;
	}
...
bool CollationTest::getSortKeyParts(const char16_t * s, int32_t length,
    CharString &dest, int32_t partSize,
    IcuTestErrorCode &errorCode) {
	if(errorCode.isFailure()) {
		return FALSE;
...
bool CollationTest::getCollationKey(const char * norm, const UnicodeString & line,
    const char16_t * s, int32_t length,
    CollationKey &key, IcuTestErrorCode &errorCode) {
	if(errorCode.isFailure()) {
		return FALSE;
	...
	for(int32_t psi = 0; psi < SIZEOFARRAYi(partSizes); ++psi) {
		...
		if(keyLength != parts.length() || memcmp(keyBytes, parts.data(), keyLength) != 0) {
			infoln(fileTestName);
 ...
 */
bool CollationTest::getMergedCollationKey(const char16_t * s, int32_t length,
    CollationKey &key, IcuTestErrorCode &errorCode) {
	if(errorCode.isFailure()) {
		return FALSE;
	}
...
int32_t getDifferenceLevel(const CollationKey &prevKey, const CollationKey &key,
    UCollationResult order, bool collHasCaseLevel) {
	if(order == UCOL_EQUAL) {
		return Collation::NO_LEVEL;
	}
...
bool CollationTest::checkCompareTwo(const char * norm, const UnicodeString & prevFileLine,
    const UnicodeString & prevString, const UnicodeString & s,
    UCollationResult expectedOrder, Collation::Level expectedLevel,
    IcuTestErrorCode &errorCode) {
	if(errorCode.isFailure()) {
	...
	CollationKey key;
	if(!getCollationKey(norm, fileLine, s.getBuffer(), s.length(), key, errorCode)) {
		return FALSE;
	}
	...
	bool containNUL = prevString.indexOf((char16_t)0) >= 0 || s.indexOf((char16_t)0) >= 0;
	if(!containNUL) {
	...
	bool collHasCaseLevel = coll->getAttribute(UCOL_CASE_LEVEL, errorCode) == UCOL_ON;
