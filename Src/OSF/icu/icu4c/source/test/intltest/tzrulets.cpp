...
static bool hasEquivalentTransitions(/*const*/ BasicTimeZone& tz1, /*const*/ BasicTimeZone& tz2,
    UDate start, UDate end, bool ignoreDstAmount, int32_t maxTransitionTimeDelta, UErrorCode & status);

class TestZIDEnumeration : public StringEnumeration {
public:
	TestZIDEnumeration(bool all = FALSE);
	~TestZIDEnumeration();
	virtual int32_t count(UErrorCode & /*status*/) const override { return len; }
...
TestZIDEnumeration::TestZIDEnumeration(bool all) : idx(0) 
{
...
const UnicodeString * TestZIDEnumeration::snext(UErrorCode & status) 
{
	if(tzenum != NULL) {
...
void TestZIDEnumeration::reset(UErrorCode & status) 
{
	if(tzenum != NULL) {
		tzenum->reset(status);
	}
...
void TimeZoneRuleTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
 ...
 */
void TimeZoneRuleTest::TestSimpleRuleBasedTimeZone() 
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void TimeZoneRuleTest::TestHistoricalRuleBasedTimeZone() {
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void TimeZoneRuleTest::TestOlsonTransition() 
{
 ...
 * transitions.
 */
void TimeZoneRuleTest::TestRBTZTransition() 
{
...
void TimeZoneRuleTest::TestHasEquivalentTransitions() {
 ...
 */
void TimeZoneRuleTest::TestVTimeZoneRoundTrip() 
{
			...
			if(U_FAILURE(status)) {
				...
				if(U_FAILURE(status)) {
					...
					if(raw1 + dst1 != raw2 + dst2) {
						errln("FAIL: VTimeZone for " + *tzid + " is not equivalent to its OlsonTimeZone corresponding at " + dateToString(startTime));
					}
					TimeZoneTransition trans;
					bool avail = tz->getNextTransition(startTime, FALSE, trans);
					if(avail) {
						if(!vtz_new->hasEquivalentTransitions(*tz, trans.getTime(),
							...
							if(!hasEquivalentTransitions(*vtz_new, *tz, trans.getTime() + maxDelta,
								...
								logln("VTimeZone for " + *tzid + "  differs from its OlsonTimeZone corresponding with maximum transition time delta - " +
								    maxDelta);
							}
						}
						if(U_FAILURE(status)) {
 ...
 */
void TimeZoneRuleTest::TestVTimeZoneRoundTripPartial() {
	const int32_t STARTYEARS[] = {
		1900,
		1950,
		2020,
							...
							if(!vtz_new->hasEquivalentTransitions(*tz, trans.getTime(),
								...
								if(!hasEquivalentTransitions(*vtz_new, *tz, trans.getTime() + maxDelta,
									...
									logln("VTimeZone for " + *tzid +
									    "  differs from its OlsonTimeZone corresponding with maximum transition time delta - " +
									    maxDelta);
								}
							}
							if(U_FAILURE(status)) {
 ...
 */
void TimeZoneRuleTest::TestVTimeZoneSimpleWrite() {
 ...
 */
void TimeZoneRuleTest::TestVTimeZoneHeaderProps() {
 ...
 * the expected format.
 */
void TimeZoneRuleTest::TestGetSimpleRules() {
	UDate testTimes[] = {
 ...
 */
void TimeZoneRuleTest::TestTimeZoneRuleCoverage() {
 ...
 */
void TimeZoneRuleTest::TestSimpleTimeZoneCoverage() {
 ...
 */
void TimeZoneRuleTest::TestVTimeZoneCoverage() {
	UErrorCode status = U_ZERO_ERROR;
	...
	// hasSameRules
	bool bSame = otz->hasSameRules(*vtz);
	...
	TimeZoneTransition tzt1, tzt2;
	bool btr1 = otz->getNextTransition(base, TRUE, tzt1);
	bool btr2 = vtz->getNextTransition(base, TRUE, tzt2);
	...
	bool equiv = vtz->hasEquivalentTransitions(*otz, time1, time2, FALSE, status);
	if(U_FAILURE(status)) {
...
void TimeZoneRuleTest::TestVTimeZoneParse() 
{
	UErrorCode status = U_ZERO_ERROR;
	...
	static const char16_t fooData[] = {
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3A, 0x32, 0x30, 0x30, 0x37, 0x31, 0x30, 0x31, 0x30, 0x54, 0x30, 0x31, 0x30,
		0x30, 0x30, 0x30, 0x0D, 0x0A,
		/* "RRULE:FREQ=YEARLY;BYDAY=WE;BYMONTHDAY=10,11,12,13,14,15,16;BYMONTH=10\x0D\x0A" */
		0x52, 0x52, 0x55, 0x4C, 0x45, 0x3A, 0x46, 0x52, 0x45, 0x51, 0x3D, 0x59, 0x45, 0x41, 0x52, 0x4C, 0x59, 0x3B, 0x42, 0x59,
		0x44, 0x41, 0x59, 0x3D, 0x57, 0x45, 0x3B, 0x42, 0x59, 0x4D, 0x4F, 0x4E, 0x54, 0x48, 0x44, 0x41, 0x59, 0x3D, 0x31, 0x30,
		0x2C, 0x31, 0x31, 0x2C, 0x31, 0x32, 0x2C, 0x31, 0x33, 0x2C, 0x31, 0x34, 0x2C, 0x31, 0x35, 0x2C, 0x31, 0x36, 0x3B, 0x42,
		0x59, 0x4D, 0x4F, 0x4E, 0x54, 0x48, 0x3D, 0x31, 0x30, 0x0D, 0x0A,
		/* "END:STANDARD\x0D\x0A" */
		0x45, 0x4E, 0x44, 0x3A, 0x53, 0x54, 0x41, 0x4E, 0x44, 0x41, 0x52, 0x44, 0x0D, 0x0A,
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3A, 0x32, 0x30, 0x30, 0x37, 0x30, 0x34, 0x31, 0x35, 0x54, 0x30, 0x31, 0x30,
		0x30, 0x30, 0x30, 0x0D, 0x0A,
		/* "RRULE:FREQ=YEARLY;BYMONTHDAY=15;BYMONTH=4\x0D\x0A" */
		0x52, 0x52, 0x55, 0x4C, 0x45, 0x3A, 0x46, 0x52, 0x45, 0x51, 0x3D, 0x59, 0x45, 0x41, 0x52, 0x4C, 0x59, 0x3B, 0x42, 0x59,
		0x4D, 0x4F, 0x4E, 0x54, 0x48, 0x44, 0x41, 0x59, 0x3D, 0x31, 0x35, 0x3B, 0x42, 0x59, 0x4D, 0x4F, 0x4E, 0x54, 0x48, 0x3D,
		0x34, 0x0D, 0x0A,
...
void TimeZoneRuleTest::TestT6216() {
	// Test case in #6216
	static const char16_t tokyoTZ[] = {
		...
		0x50, 0x52, 0x4f, 0x44, 0x49, 0x44, 0x3a, 0x2d, 0x2f, 0x2f, 0x50, 0x59, 0x56, 0x4f, 0x42, 0x4a, 0x45, 0x43, 0x54, 0x2f,
		0x2f, 0x4e, 0x4f, 0x4e, 0x53, 0x47, 0x4d, 0x4c, 0x20, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6f, 0x6e, 0x20, 0x31, 0x2f, 0x2f,
		0x45, 0x4e, 0x0d, 0x0a,
		/* "BEGIN:VTIMEZONE\r\n" */
		0x42, 0x45, 0x47, 0x49, 0x4e, 0x3a, 0x56, 0x54, 0x49, 0x4d, 0x45, 0x5a, 0x4f, 0x4e, 0x45, 0x0d, 0x0a,
		/* "TZID:Asia/Tokyo\r\n" */
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3a, 0x32, 0x30, 0x30, 0x30, 0x30, 0x31, 0x30, 0x31, 0x54, 0x30, 0x30, 0x30,
		0x30, 0x30, 0x30, 0x0d, 0x0a,
		/* "RRULE:FREQ=YEARLY;BYMONTH=1\r\n" */
		0x52, 0x52, 0x55, 0x4c, 0x45, 0x3a, 0x46, 0x52, 0x45, 0x51, 0x3d, 0x59, 0x45, 0x41, 0x52, 0x4c, 0x59, 0x3b, 0x42, 0x59,
		0x4d, 0x4f, 0x4e, 0x54, 0x48, 0x3d, 0x31, 0x0d, 0x0a,
		/* "TZNAME:Asia/Tokyo\r\n" */
	...
	static const char16_t finalOverlap[] = {
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3a, 0x32, 0x30, 0x30, 0x30, 0x31, 0x30, 0x32, 0x39, 0x54, 0x30, 0x32, 0x30,
		0x30, 0x30, 0x30, 0x0d, 0x0a,
		/* "RRULE:FREQ=YEARLY;BYDAY=-1SU;BYMONTH=10\r\n" */
		0x52, 0x52, 0x55, 0x4c, 0x45, 0x3a, 0x46, 0x52, 0x45, 0x51, 0x3d, 0x59, 0x45, 0x41, 0x52, 0x4c, 0x59, 0x3b, 0x42, 0x59,
		0x44, 0x41, 0x59, 0x3d, 0x2d, 0x31, 0x53, 0x55, 0x3b, 0x42, 0x59, 0x4d, 0x4f, 0x4e, 0x54, 0x48, 0x3d, 0x31, 0x30, 0x0d,
		0x0a,
		/* "END:STANDARD\r\n" */
		0x45, 0x4e, 0x44, 0x3a, 0x53, 0x54, 0x41, 0x4e, 0x44, 0x41, 0x52, 0x44, 0x0d, 0x0a,
		/* "BEGIN:DAYLIGHT\r\n" */
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3a, 0x31, 0x39, 0x39, 0x39, 0x30, 0x34, 0x30, 0x34, 0x54, 0x30, 0x32, 0x30,
		0x30, 0x30, 0x30, 0x0d, 0x0a,
		/* "RRULE:FREQ=YEARLY;BYDAY=1SU;BYMONTH=4;UNTIL=20050403T040000Z\r\n" */
		0x52, 0x52, 0x55, 0x4c, 0x45, 0x3a, 0x46, 0x52, 0x45, 0x51, 0x3d, 0x59, 0x45, 0x41, 0x52, 0x4c, 0x59, 0x3b, 0x42, 0x59,
		0x44, 0x41, 0x59, 0x3d, 0x31, 0x53, 0x55, 0x3b, 0x42, 0x59, 0x4d, 0x4f, 0x4e, 0x54, 0x48, 0x3d, 0x34, 0x3b, 0x55, 0x4e,
		0x54, 0x49, 0x4c, 0x3d, 0x32, 0x30, 0x30, 0x35, 0x30, 0x34, 0x30, 0x33, 0x54, 0x30, 0x34, 0x30, 0x30, 0x30, 0x30, 0x5a,
		0x0d, 0x0a,
		/* "END:DAYLIGHT\r\n" */
		0x45, 0x4e, 0x44, 0x3a, 0x44, 0x41, 0x59, 0x4c, 0x49, 0x47, 0x48, 0x54, 0x0d, 0x0a,
		/* "END:VTIMEZONE\r\n" */
	...
	static const char16_t finalNonOverlap[] = {
		...
		0x54, 0x5a, 0x49, 0x44, 0x3a, 0x46, 0x69, 0x6e, 0x61, 0x6c, 0x4e, 0x6f, 0x6e, 0x4f, 0x76, 0x65, 0x72, 0x6c, 0x61, 0x70,
		0x0d, 0x0a,
		/* "BEGIN:STANDARD\r\n" */
		0x42, 0x45, 0x47, 0x49, 0x4e, 0x3a, 0x53, 0x54, 0x41, 0x4e, 0x44, 0x41, 0x52, 0x44, 0x0d, 0x0a,
		/* "TZOFFSETFROM:-0200\r\n" */
		...
		0x52, 0x52, 0x55, 0x4c, 0x45, 0x3a, 0x46, 0x52, 0x45, 0x51, 0x3d, 0x59, 0x45, 0x41, 0x52, 0x4c, 0x59, 0x3b, 0x42, 0x59,
		0x44, 0x41, 0x59, 0x3d, 0x2d, 0x31, 0x53, 0x55, 0x3b, 0x42, 0x59, 0x4d, 0x4f, 0x4e, 0x54, 0x48, 0x3d, 0x31, 0x30, 0x3b,
		0x55, 0x4e, 0x54, 0x49, 0x4c, 0x3d, 0x32, 0x30, 0x30, 0x34, 0x31, 0x30, 0x33, 0x31, 0x54, 0x30, 0x34, 0x30, 0x30, 0x30,
		0x30, 0x5a, 0x0d, 0x0a,
		/* "END:STANDARD\r\n" */
		0x45, 0x4e, 0x44, 0x3a, 0x53, 0x54, 0x41, 0x4e, 0x44, 0x41, 0x52, 0x44, 0x0d, 0x0a,
		/* "BEGIN:DAYLIGHT\r\n" */
		...
		0x44, 0x54, 0x53, 0x54, 0x41, 0x52, 0x54, 0x3a, 0x32, 0x30, 0x30, 0x37, 0x31, 0x30, 0x32, 0x38, 0x54, 0x30, 0x32, 0x30,
		0x30, 0x30, 0x30, 0x0d, 0x0a,
		/* "RRULE:FREQ=YEARLY;BYDAY=-1SU;BYMONTH=10\r\n" */
		0x52, 0x52, 0x55, 0x4c, 0x45, 0x3a, 0x46, 0x52, 0x45, 0x51, 0x3d, 0x59, 0x45, 0x41, 0x52, 0x4c, 0x59, 0x3b, 0x42, 0x59,
		0x44, 0x41, 0x59, 0x3d, 0x2d, 0x31, 0x53, 0x55, 0x3b, 0x42, 0x59, 0x4d, 0x4f, 0x4e, 0x54, 0x48, 0x3d, 0x31, 0x30, 0x0d,
	...
	for(i = 0; !TestZones[i].isEmpty(); i++) {
		...
		for(j = 0; j < numTimes; j++) {
			...
			if(offset != Expected[i][j]) {
				errln(
					(UnicodeString)"FAIL: Invalid offset at time(" + times[j] + "):" + offset + " Expected:" +
					Expected[i][j]);
			}
		}
		delete vtz;
...
void TimeZoneRuleTest::TestT6669() {
	UErrorCode status = U_ZERO_ERROR;
	...
	bool avail = stz.getNextTransition(t, FALSE, tzt);
	if(!avail) {
...
void TimeZoneRuleTest::TestVTimeZoneWrapper() {
#if 0
	// local variables
	bool b;
	char16_t * data = NULL;
...
//
UDate TimeZoneRuleTest::getUTCMillis(int32_t y, int32_t m, int32_t d, int32_t hr, int32_t min, int32_t sec, int32_t msec) 
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void TimeZoneRuleTest::verifyTransitions(BasicTimeZone& icutz, UDate start, UDate end) 
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void TimeZoneRuleTest::compareTransitionsAscending(BasicTimeZone& z1, BasicTimeZone& z2, UDate start, UDate end, bool inclusive) 
{
	...
	UDate time = start;
	while(true) {
		const bool avail1 = z1.getNextTransition(time, inclusive, tzt1);
		const bool avail2 = z2.getNextTransition(time, inclusive, tzt2);
		bool inRange1 = false;
		bool inRange2 = false;
		if(avail1) {
 ...
 */
void TimeZoneRuleTest::compareTransitionsDescending(BasicTimeZone& z1, BasicTimeZone& z2, UDate start, UDate end, bool inclusive) 
{
	UnicodeString zid1, zid2;
	TimeZoneTransition tzt1, tzt2;
	z1.getID(zid1);
	...
	UDate time = end;
	while(true) {
		const bool avail1 = z1.getPreviousTransition(time, inclusive, tzt1);
		const bool avail2 = z2.getPreviousTransition(time, inclusive, tzt2);
		bool inRange1 = false;
		bool inRange2 = false;
		if(avail1) {
		...
		if(!inRange1) {
			errln((UnicodeString)"FAIL: " + zid1 + " does not have any transitions before " + dateToString(time) + " after " + dateToString(start));
			break;
		}
		if(!inRange2) {
			errln((UnicodeString)"FAIL: " + zid2 + " does not have any transitions before " + dateToString(time) + " after " + dateToString(start));
			break;
		}
		if(tzt1.getTime() != tzt2.getTime()) {
    ...
    UDate start, UDate end, bool ignoreDstAmount, int32_t maxTransitionTimeDelta, UErrorCode & status) 
{
	...
	while(true) {
		const bool avail1 = tz1.getNextTransition(time, FALSE, tr1);
		const bool avail2 = tz2.getNextTransition(time, FALSE, tr2);
		if(ignoreDstAmount) {
			...
			while(true) {
				if(avail2 && tr2.getTime() <= end && (tr2.getFrom()->getRawOffset() + tr2.getFrom()->getDSTSavings() == 
					tr2.getTo()->getRawOffset() + tr2.getTo()->getDSTSavings()) && (tr2.getFrom()->getDSTSavings() != 0 && tr2.getTo()->getDSTSavings() != 0)) {
		...
		const bool inRange1 = (avail1 && tr1.getTime() <= end);
		const bool inRange2 = (avail2 && tr2.getTime() <= end);
		if(!inRange1 && !inRange2) {
			// No more transition in the range
			break;
		}
		...
		const double delta = tr1.getTime() >= tr2.getTime() ? tr1.getTime() - tr2.getTime() : tr2.getTime() - tr1.getTime();
...
void TimeZoneRuleTest::TestT8943() 
{
	UErrorCode status = U_ZERO_ERROR;
