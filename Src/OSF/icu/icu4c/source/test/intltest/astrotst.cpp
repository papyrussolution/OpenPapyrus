...
void AstroTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void AstroTest::TestSolarLongitude() {
	...
	for(uint32_t i = 0; i < SIZEOFARRAYi(tests); i++) {
		...
		logln(
			(UnicodeString)"Sun position is " + result.toString() + (UnicodeString)";  " /* +
		                                                                                        result.toHmsString()*/   + " Sun longitude is " +
			longitude);
	}
	closeAstro(status);
	ASSERT_OK(status);
...
void AstroTest::TestMoonAge() {
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(testcase); i++) {
		...
		gc->set((int32_t)testcase[i][0],
		    (int32_t)testcase[i][1]-1,
		    (int32_t)testcase[i][2],
		    (int32_t)testcase[i][3],
