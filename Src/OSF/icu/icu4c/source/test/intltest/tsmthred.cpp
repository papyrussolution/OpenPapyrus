...
void MultithreadTest::runIndexedTest(int32_t index, bool exec,
...
public:
	...
	bool doStuff(int32_t offset, UnicodeString & appendErr, UErrorCode & status) const;
private:
...
bool ThreadSafeFormat::doStuff(int32_t offset, UnicodeString & appendErr, UErrorCode & status) const {
	bool okay = TRUE;
...
bool U_CALLCONV isAcceptable(void *, const char *, const char *, const UDataInfo *) {
	return TRUE;
}
		...
		for(iteration = 0; !IntlTest::gTest->getErrors() && iteration<kFormatThreadIterations; iteration++) {
			...
			switch((iteration+fOffset) % kNumberOfMessageTests)
			{
				...
				case 1:
				    ...
				    expected =
					u"1:A customer in Burkina Faso is receiving a #8 error - U_INDEX_OUTOFBOUNDS_ERROR. "
				...
				case 2:
				    ...
				    messageLocale =                      Locale("de", "AT@currency=ATS");// Austrian
				                                                     // German
...
public:
	...
	void setCollator(Collator * c, Line * l, int32_t nl, bool atLeastUCA62)
	{
		coll = c;
		lines = l;
		noLines = nl;
		...
		for(i = 0; i < noLines; i++) {
			...
			if(oldSk != NULL) {
				...
				if(cmpres != normalizeResult(skres)) {
					IntlTest::gTest->errln(UnicodeString(
						    u"Difference between coll->compare and sortkey compare on line ") + (i + 1));
					break;
				}
				...
				if(res > 0) {
					IntlTest::gTest->errln(UnicodeString(
						    u"Line is not greater or equal than previous line, for line ") + (i + 1));
					break;
				}
			}
...
public:
	UnifiedCacheThread(const UnifiedCache * cache, const char * loc, const char * loc2) : fCache(cache), fLoc(loc), fLoc2(loc2) 
	{
	}
	~UnifiedCacheThread() 
	{
