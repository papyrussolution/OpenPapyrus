...
bool TimeZoneRegressionTest::failure(UErrorCode status, const char * msg)
{
	if(U_FAILURE(status)) {
...
void TimeZoneRegressionTest:: Test4073215()
{
	...
	bool indt = cal.getTimeZone().inDaylightTime(jan31 = cal.getTime(status), status);
...
bool TimeZoneRegressionTest::checkCalendar314(GregorianCalendar * testCal, TimeZone * testTZ)
{
	UErrorCode status = U_ZERO_ERROR;
	...
	bool inDaylightTime = testTZ->inDaylightTime(testDate, status);
 ...
 */
void TimeZoneRegressionTest::Test4154525()
{
	...
	for(int32_t i = 0; i < 10; i += 2) {
		...
		for(int32_t j = 0; j < 2; ++j) {
			...
			if(U_FAILURE(status)) {
				if(valid) {
					errln(UnicodeString("Fail: DST savings of ") + savings + " to " + method + " gave " +
					    u_errorName(status));
				}
				else {
					logln(UnicodeString("Pass: DST savings of ") + savings + " to " + method + " gave " +
					    u_errorName(status));
				}
			}
			else {
 ...
 */
void TimeZoneRegressionTest::Test4154650()
{
	const int32_t GOOD = 1, BAD = 0;
 ...
 */
void TimeZoneRegressionTest::Test4162593()
{
	UErrorCode status = U_ZERO_ERROR;
...
void TimeZoneRegressionTest::TestJ449() {
	...
	if(zoneWith->getID(str) != UnicodeString(idWithLocaleData) ||
		...
		dataerrln(UnicodeString("Fail: Unable to create zones - wanted ") + idWithLocaleData + ", got " + zoneWith->getID(
			    str) + ", and wanted " + idWithoutLocaleData + " but got " + zoneWithout->getID(str));
	}
	else {
...
void TimeZoneRegressionTest::TestJDK12API()
{
	// TimeZone *pst = TimeZone::createTimeZone("PST");
