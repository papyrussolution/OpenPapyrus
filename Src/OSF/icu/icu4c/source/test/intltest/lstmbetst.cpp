...
void LSTMBETest::runIndexedTest(int32_t index, bool exec, const char *& name, char * params)
{
	fTestParams = params;
	TESTCASE_AUTO_BEGIN;
	...
	do {
		...
		if(line.length() > 0) {
			...
			if(key == "Model:") {
				...
				if(U_FAILURE(status)) {
					dataerrln("Could not utext_openConstUnicodeString for " + value +
...
const LanguageBreakEngine* LSTMBETest::createEngineFromTestData(const char * model, UScriptCode script, UErrorCode & status) {
