...
int32_t FieldsSet::parseFrom(const UnicodeString & str, const
	...
	for(int i = 0; i < destCount; i += 1) {
		...
		if(U_FAILURE(status)) {
			...
			it_errln(UnicodeString("Parse Failed: Field ") + UnicodeString(ch) + UnicodeString(", err ") +
		...
		if(field != -1) {
			...
			if(U_FAILURE(status)) {
				...
				u_UCharsToChars(u, ch, len);
				ch[len] = 0; /* include terminating \0 */
				it_errln(UnicodeString("Parse Failed: Value ") + UnicodeString(ch) + UnicodeString(
					    ", err ") + UnicodeString(u_errorName(status)));
				delete[] kv;
				delete[] dest;
				return -1;
			}
...
bool FieldsSet::isSameType(const FieldsSet& other) const {
	return((&other==this)||
...
bool FieldsSet::isSet(int32_t field) const {
	if(field<0|| field>=fieldCount()) {
		return FALSE;
	}
	return fIsSet[field];
...
int32_t FieldsSet::handleParseName(const FieldsSet* /* inheritFrom */,
    const UnicodeString & name,
    const UnicodeString & /* substr*/,
    UErrorCode & status) {
	if(fEnum > -1) {
...
void FieldsSet::parseValueDefault(const FieldsSet* inheritFrom, int32_t field, const UnicodeString & substr, UErrorCode & status) {
	...
	if(substr.length()==0) { // inherit requested
		...
		if((inheritFrom == NULL) || !inheritFrom->isSet((UCalendarDateFields)field)) {
			...
			it_errln(UnicodeString("Parse Failed: Couldn't inherit field ") + field + UnicodeString(" [") +
			    UnicodeString(udbg_enumName(fEnum, field)) + UnicodeString("]"));
			status = U_ILLEGAL_ARGUMENT_ERROR;
			return;
		}
...
void FieldsSet::parseValueEnum(UDebugEnumType type,
 ...
 */
bool CalendarFieldsSet::matches(Calendar * cal, CalendarFieldsSet &diffSet,
...
int32_t DateTimeStyleSet::handleParseName(const FieldsSet* /* inheritFrom */,
    const UnicodeString & name,
    const UnicodeString & /* substr */,
    UErrorCode & status) {
