...
void TransliteratorAPITest::runIndexedTest(int32_t index, bool exec,
...
void TransliteratorAPITest::TestGetDisplayName() {
	...
	for(uint32_t i = 0; i<UPRV_LENGTHOF(dispNames); i = i+2) {
		...
		//  doTest(message, name, dispNames[i+1]); //!!! This will obviously fail for any locale other than
		// english and its children!!!
		name = "";
...
void TransliteratorAPITest::TestTransliterate1() {
	UnicodeString Data[] = {
		...
		"Latin-Devanagari", UnicodeString("kra ksha khra gra cra dya dhya", ""), CharsToUnicodeString(
			"\\u0915\\u094D\\u0930 \\u0915\\u094D\\u0936 \\u0916\\u094D\\u0930 \\u0917\\u094D\\u0930 \\u091a\\u094D\\u0930 \\u0926\\u094D\\u092F \\u0927\\u094D\\u092F"),

		"Devanagari-Latin",    CharsToUnicodeString("\\u092D\\u093E\\u0930\\u0924"),        CharsToUnicodeString("bh\\u0101rata"),
		//  "Contracted-Expanded", CharsToUnicodeString("\\u00C0\\u00C1\\u0042"),
		//               CharsToUnicodeString("\\u0041\\u0300\\u0041\\u0301\\u0042") ,
		//  "Expanded-Contracted", CharsToUnicodeString("\\u0041\\u0300\\u0041\\u0301\\u0042"),
		// CharsToUnicodeString("\\u00C0\\u00C1\\u0042") ,
...
void TransliteratorAPITest::TestTransliterate2() {
	...
	UnicodeString Data2[] = {
		...
		"Any-Hex",         "hello! How are you?",  "0", "5", UnicodeString("\\u0068\\u0065\\u006C\\u006C\\u006F", ""),
		UnicodeString("\\u0068\\u0065\\u006C\\u006C\\u006F! How are you?", ""),
		"Any-Hex",         "hello! How are you?",  "7", "12", UnicodeString("\\u0048\\u006F\\u0077\\u0020\\u0061", ""),
		UnicodeString("hello! \\u0048\\u006F\\u0077\\u0020\\u0061re you?", ""),
		"Hex-Any",         CharsToUnicodeString("\\u0068\\u0065\\u006C\\u006C\\u006F\\u0021\\u0020"), "0", "5",  "hello", "hello! ",
		//  "Contracted-Expanded", CharsToUnicodeString("\\u00C0\\u00C1\\u0042"),        "1", "2",
		//  CharsToUnicodeString("\\u0041\\u0301"), CharsToUnicodeString("\\u00C0\\u0041\\u0301\\u0042") ,
		"Devanagari-Latin",    CharsToUnicodeString("\\u092D\\u093E\\u0930\\u0924"), "0", "1",  "bha", CharsToUnicodeString(
			"bha\\u093E\\u0930\\u0924"),
		"Devanagari-Latin",    CharsToUnicodeString("\\u092D\\u093E\\u0930\\u0924"), "1", "2",  CharsToUnicodeString(
			"\\u0314\\u0101"), CharsToUnicodeString("\\u092D\\u0314\\u0101\\u0930\\u0924")
	};
	...
	for(uint32_t i = 0; i<UPRV_LENGTHOF(Data2); i = i+6) {
		...
		doTest(
			t->getID() + ".transliterate(UnicodeString, int32_t, int32_t, UnicodeString):(" + start + "," + limit + ")  for \n\t source: " +
			prettify(Data2[i+1]),
			gotResBuf,
			Data2[i+4]);
		...
		doTest(
			t->getID() + ".transliterate(Replaceable, int32_t, int32_t, ):(" + start + "," + limit + ")  for \n\t source: " +
			prettify(Data2[i+1]),
			temp,
			Data2[i+5]);
		status = U_ZERO_ERROR;
...
void TransliteratorAPITest::TestSimpleKeyboardTransliterator() {
	...
	if(U_FAILURE(status))
		errln("FAIL: " + t->getID()+ ".translitere(Replaceable, int[], UnicodeString, UErrorCode)-->" +
	...
	for(uint32_t i = 0; i<UPRV_LENGTHOF(index1); i++) {
		...
		if(status == U_ILLEGAL_ARGUMENT_ERROR)
			...
			errln("FAIL: invalid index values didn't throw U_ILLEGAL_ARGUMENT_ERROR throw" +
			    (UnicodeString)u_errorName(status));
	}
...
void TransliteratorAPITest::TestKeyboardTransliterator1() {
	...
	for(i = 0; i<10; i = i+2) {
		...
		if(Data[i+0] != "") {
			...
			if(U_FAILURE(status)) {
				errln("FAIL: " + t->getID()+ ".transliterate(Replaceable, int32_t[], UnicodeString, UErrorCode)-->" +
				    (UnicodeString)u_errorName(status));
				continue;
			}
		}
...
void TransliteratorAPITest::TestKeyboardTransliterator2() {
	UnicodeString Data[] = {
		...
		//   CharsToUnicodeString("\\u00C1"), CharsToUnicodeString("Ad\\u0041\\u0301d here:"),             "1",
		//  "2",  "1" ,
		//   CharsToUnicodeString("\\u00C0"), CharsToUnicodeString("Ad\\u0041\\u0301d here:\\u0041\\u0300"),
		// "11", "11", "11",
...
void TransliteratorAPITest::TestKeyboardTransliterator3() {
	...
	for(uint32_t i = 0; i<UPRV_LENGTHOF(Data); i = i+4) {
		...
		if(U_FAILURE(status)) {
			errln("FAIL: " + t->getID()+ ".transliterate(Replaceable, int32_t[], UErrorCode)-->" +
			    (UnicodeString)u_errorName(status));
			continue;
		}
		t->finishTransliteration(s, index);
...
class TestFilter1 : public UnicodeFilter {
	...
	virtual TestFilter1* clone() const override { return new TestFilter1(*this); }
	virtual bool contains(UChar32 c) const override 
	{
		return oneof4(c, 0x63, 0x61, 0x43, 0x41) ? false : true;
	}
	// Stubs
	virtual UnicodeString & toPattern(UnicodeString & result, bool /*escapeUnprintable*/) const override 
	{
		return result;
	}
	virtual bool matchesIndexValue(uint8_t /*v*/) const override { return false; }
...
class TestFilter2 : public UnicodeFilter 
{
	...
	virtual TestFilter2* clone() const override { return new TestFilter2(*this); }
	virtual bool contains(UChar32 c) const override { return oneof2(c, 0x65, 0x6c) ? false : true; }
	// Stubs
	virtual UnicodeString & toPattern(UnicodeString & result, bool /*escapeUnprintable*/) const override { return result; }
	virtual bool matchesIndexValue(uint8_t /*v*/) const override { return false; }
	virtual void addMatchSetTo(UnicodeSet & /*toUnionTo*/) const override {}
};
...
class TestFilter3 : public UnicodeFilter {
	...
	virtual TestFilter3* clone() const override { return new TestFilter3(*this); }
	virtual bool contains(UChar32 c) const override 
	{
		if(c==0x6f || c==0x77)
			return FALSE;
	...
	virtual UnicodeString & toPattern(UnicodeString & result,
	    bool /*escapeUnprintable*/) const override {
		return result;
	}
...
void TransliteratorAPITest::displayOutput(const UnicodeString & got,
...
public:
	virtual UnicodeFunctor* clone() const override {
		return NULL;
	}
	static UClassID getStaticClassID() {
		return (UClassID)&MyUnicodeFunctorTestClassID;
	}
