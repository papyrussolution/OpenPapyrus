...
static bool isDevelopmentBuild = (U_ICU_VERSION_MINOR_NUM == 0);

void TimeZoneTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
 ...
 */
void TimeZoneTest::TestGenericAPI()
{
...
void TimeZoneTest::findTransition(const TimeZone& tz,
    UDate min, UDate max) {
	UErrorCode ec = U_ZERO_ERROR;
	UnicodeString id, s;
	bool startsInDST = tz.inDaylightTime(min, ec);
...
void TimeZoneTest::testUsingBinarySearch(const TimeZone& tz,
    UDate min, UDate max,
 ...
 */
void TimeZoneTest::TestPRTOffset()
{
 ...
 */
void TimeZoneTest::TestVariousAPI518()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void TimeZoneTest::TestGetAvailableIDs913()
{
	UErrorCode ec = U_ZERO_ERROR;
	int32_t i;
...
void TimeZoneTest::TestGetAvailableIDsNew()
{
	UErrorCode ec = U_ZERO_ERROR;
...
void TimeZoneTest::checkContainsAll(StringEnumeration * s1, const char * name1,
...
void TimeZoneTest::TestShortZoneIDs()
{
	...
	for(i = 0; kReferenceList[i].id[0]; i++) {
		...
		bool usesDaylight = tz->useDaylightTime();
...
void TimeZoneTest::TestAliasedNames()
{
	struct {
		const char * from;
		const char * to;
 ...
 * are present or not.
 */
void TimeZoneTest::TestDisplayName()
{
	UErrorCode status = U_ZERO_ERROR;
	int32_t i;
 ...
 * @bug 4107276
 */
void TimeZoneTest::TestDSTSavings()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 * @bug 4107570
 */
void TimeZoneTest::TestAlternateRules()
{
...
void TimeZoneTest::TestGetWindowsID() {
	static const struct {
		...
		{"Australia/Eucla",         "Aus Central W. Standard Time"},// formerly no Windows ID mapping, now has
		                                                     // one
		{"Bogus",                   ""},
		{0,                         0},
	};
