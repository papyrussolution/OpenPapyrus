...
U_NAMESPACE_USE
void CalendarLimitTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
bool CalendarLimitTest::withinErr(double a, double b, double err)
{
	return ( uprv_fabs(a - b) < uprv_fabs(a * err));
}

void CalendarLimitTest::TestCalendarExtremeLimit()
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarLimitTest::TestLimits() {
...
void CalendarLimitTest::doTheoreticalLimitsTest(Calendar& cal, bool leapMonth) {
...
void CalendarLimitTest::doLimitsTest(Calendar& cal, UDate startDate, int32_t endTime) {
	int32_t testTime = quick ? ( endTime / 40 ) : endTime;
	doLimitsTest(cal, NULL /*default fields*/, startDate, testTime);
}

void CalendarLimitTest::doLimitsTest(Calendar& cal,
...
UnicodeString &CalendarLimitTest::ymdToString(const Calendar& cal, UnicodeString & str) {
	UErrorCode status = U_ZERO_ERROR;
	str.remove();
