...
void ConversionTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
...
void ConversionTest::TestToUnicode() {
	ConversionCase cc;
...
void ConversionTest::TestFromUnicode() {
	ConversionCase cc;
	char charset[100], cbopt[4];
	const char * option;
		...
		if(U_SUCCESS(errorCode)) {
			for(i = 0; testData->nextCase(testCase, errorCode); ++i) {
				...
				if((index = s.indexOf((char16_t)0))>0) {
					...
					if(length<0 || length>=SIZEOFARRAYi(cc.subString)) {
						errorCode = U_ILLEGAL_ARGUMENT_ERROR;
					}
					else {
			...
			for(i = 0; testData->nextCase(testCase, errorCode); ++i) {
				...
				if(U_FAILURE(errorCode) || pos.getIndex()!=mapnot.length()) {
					errln("error creating the mapnot set for conversion/getUnicodeSet test case %d - %s\n"
					    "    error index %d  index %d  U+%04x",
					    i, u_errorName(errorCode), pos.getErrorIndex(), pos.getIndex(),
					    mapnot.char32At(pos.getIndex()));
					errorCode = U_ZERO_ERROR;
					continue;
				}
				...
				if(!diffSet.isEmpty()) {
					...
					if(s.length()>100) {
						s.replace(100, 0x7fffffff, ellipsis, SIZEOFARRAYi(ellipsis));
					}
				...
				if(!diffSet.isEmpty()) {
					...
					errln(
						"error: ucnv_getUnicodeSet(\"%s\") contains unexpected items - conversion/getUnicodeSet test case %d",
						charset,
						i);
					errln(s);
				}
...
void ConversionTest::TestGetUnicodeSet2() {
	...
	char buffer[1024];
	int32_t i;
	for(i = 0; i<SIZEOFARRAYi(cnvNames); ++i) {
		UErrorCode errorCode = U_ZERO_ERROR;
		...
		for(which = UCNV_ROUNDTRIP_SET; which<UCNV_SET_COUNT; which = (UConverterUnicodeSet)((int)which+1)) {
			...
			if(set!=expected) {
				...
				// added strings: The above conversion method does not tell us what strings might be
				// convertible.
			...
			if(set!=expected) {
				...
				if(!diffSet.isEmpty()) {
					...
					if(out.length()>100) {
						out.replace(100, 0x7fffffff, ellipsis, SIZEOFARRAYi(ellipsis));
					}
...
void ConversionTest::TestDefaultIgnorableCallback() {
	...
	while(iter.next()) {
		...
		outputLength = ucnv_fromUChars(cnv.getAlias(),
			output,
			10,
			UnicodeString::fromUTF32(input, 1).getTerminatedBuffer(),
			-1,
			&status);
...
void ConversionTest::TestUTF8ToUTF8Overflow() {
	...
	errorLength = SIZEOFARRAYi(errorBytes);
...
void ConversionTest::TestUTF8ToUTF8Streaming() {
...
UConverter * ConversionTest::cnv_open(const char * name, UErrorCode &errorCode) {
	if(name!=NULL && *name=='+') {
...
static char * printBytes(const uint8_t * bytes, int32_t length, char * out) {
	uint8_t b;
		...
		for(;;) {
			...
			if(source>sourceLimit || target>targetLimit) {
				...
				*pErrorCode = U_ZERO_ERROR;
				...
				flush = (bool)(cc.finalFlush && sourceLimit==bytesLimit);
			}
		}
	}
	else { /* step<0 */
		...
		for(;;) {
			...
			if((step&1)!=0 /* odd: -1, -3, -5, ... */) {
				...
				ucnv_toUnicode(cnv,
				    &target, targetLimit,
				    &source, sourceLimit,
				    NULL, (bool)(sourceLimit==bytesLimit), pErrorCode);
				...
				if(*pErrorCode==U_BUFFER_OVERFLOW_ERROR) {
					...
					*pErrorCode = U_ZERO_ERROR;
					...
					// we are done (flush==TRUE) but we continue, to get the index out of bounds
					// error above
				}
...
bool ConversionTest::ToUnicodeCase(ConversionCase &cc, UConverterToUCallback callback, const char * option) 
{
	...
	// with no data, the above crashes with "pointer being freed was not allocated" for charset "x11-compound-text",
	// see #13078
	...
	int32_t i, step;
	ok = TRUE;
	for(i = 0; i<SIZEOFARRAYi(steps) && ok; ++i) {
		step = steps[i].step;
		...
		if(step!=0) {
			...
			for(int32_t i = 0; i < SIZEOFARRAYi(resultOffsets); i++) {
				resultOffsets[i] = -1;
			}
		}
		for(int32_t i = 0; i < SIZEOFARRAYi(result); i++) {
			result[i] = -1;
		}
		errorCode.reset();
		resultLength = stepToUnicode(cc, cnv.getAlias(), result, SIZEOFARRAYi(result), step==0 ? resultOffsets : NULL, step, errorCode);
	...
	while(ok && cc.finalFlush) {
		...
		memzero(result, sizeof(result));
		errorCode.reset();
		resultLength = ucnv_toUChars(cnv.getAlias(), result, SIZEOFARRAYi(result), (const char *)cc.bytes, cc.bytesLength, errorCode);
...
bool ConversionTest::checkToUnicode(ConversionCase &cc, UConverter * cnv, const char * name,
	...
	if(msg==NULL) {
		...
		if((s-buffer)>(int32_t)sizeof(buffer)) {
			errln(
				"toUnicode[%d](%s cb=\"%s\" fb=%d flush=%d %s) fatal error: checkToUnicode() test output buffer overflow writing %d chars\n",
				cc.caseNr,
				cc.charset,
				cc.cbopt,
				cc.fallbacks,
...
static int32_t stepFromUTF8(ConversionCase &cc,
	...
	if(step==0) {
		...
		pivotLimit = pivotBuffer+SIZEOFARRAYi(pivotBuffer);
	}
	else {
		// start with empty partial buffers
		sourceLimit = source;
	...
	for(;;) {
		...
		if(source>sourceLimit || target>targetLimit) {
			...
			*pErrorCode = U_ZERO_ERROR;
			...
			flush = (bool)(cc.finalFlush && sourceLimit==utf8Limit);
		}
	}
...
static int32_t stepFromUnicode(ConversionCase &cc, UConverter * cnv,
    char * result, int32_t resultCapacity,
    int32_t * resultOffsets,            /* also resultCapacity */
    int32_t step,
    UErrorCode * pErrorCode) {
	...
	for(;;) {
		...
		if(source>sourceLimit || target>targetLimit) {
			...
			*pErrorCode = U_ZERO_ERROR;
			...
			flush = (bool)(cc.finalFlush && sourceLimit==unicodeLimit);
		}
	}
...
bool ConversionTest::FromUnicodeCase(ConversionCase &cc, UConverterFromUCallback callback, const char * option) {
	UConverter * cnv;
	UErrorCode errorCode;
	...
	char utf8[256];
	cc.utf8 = utf8;
	u_strToUTF8(utf8, SIZEOFARRAYi(utf8), &cc.utf8Length,
	...
	while(ok && cc.finalFlush) {
		...
		memzero(result, sizeof(result));
		errorCode = U_ZERO_ERROR;
		resultLength = ucnv_fromUChars(cnv, result, SIZEOFARRAYi(result), cc.unicode, cc.unicodeLength, &errorCode);
...
bool ConversionTest::checkFromUnicode(ConversionCase &cc, UConverter * cnv, const char * name,
	...
	errorCode = U_ZERO_ERROR;
	resultInvalidLength = SIZEOFARRAYi(resultInvalidUChars);
	...
	if(msg==NULL) {
		...
		if((s-buffer)>(int32_t)sizeof(buffer)) {
			errln(
				"fromUnicode[%d](%s cb=\"%s\" fb=%d flush=%d %s) fatal error: checkFromUnicode() test output buffer overflow writing %d chars\n",
				cc.caseNr,
				cc.charset,
				cc.cbopt,
				cc.fallbacks,
