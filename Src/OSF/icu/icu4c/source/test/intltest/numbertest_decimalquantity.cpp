...
void DecimalQuantityTest::runIndexedTest(int32_t index, bool exec, const char *&name, char *) {
	if(exec) {
...
void DecimalQuantityTest::assertDoubleEquals(UnicodeString message, double a, double b) {
	...
	if(diff > bound) {
		errln(message + u": " + DoubleToUnicodeString(a) + u" vs " + DoubleToUnicodeString(
			    b) + u" differ by " + DoubleToUnicodeString(diff));
	}
}
...
void DecimalQuantityTest::assertToStringAndHealth(const DecimalQuantity &fq, const UnicodeString & expected) {
...
void DecimalQuantityTest::testNickelRounding() {
	...
	for(const auto& cas : cases) {
		UnicodeString message = DoubleToUnicodeString(cas.input) + u" @ " + Int64ToUnicodeString(cas.magnitude) + u" / " +
		    Int64ToUnicodeString(cas.roundingMode);
