...
void IntlTestDecimalFormatAPI::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
// ======= Test constructors
	...
	status = U_ZERO_ERROR;
	{
		...
		assertEquals("pat 0: ", (bool)FALSE, (bool)df.isGroupingUsed());
	...
	}
	{
		...
		assertEquals("pat #,##0: ", (bool)TRUE, (bool)df.isGroupingUsed());
		df.setGroupingUsed(false);
...
// ======= Test various format() methods
	...
	FieldPosition pos1(FieldPosition::DONT_CARE), pos2(FieldPosition::DONT_CARE), pos3(FieldPosition::DONT_CARE), pos4(
...
// ======= Test getters and setters
	...
	bool tf = pat.isDecimalSeparatorAlwaysShown();
	...
	bool esas = pat.isExponentSignAlwaysShown();
	...
	bool sn = pat.isScientificNotation();
...
void IntlTestDecimalFormatAPI::testRounding(/*char *par*/)
{
	...
	for(mode = 0; mode < 7; mode++) {
		...
		message = (UnicodeString)"round(" + (double)Roundingnumber + UnicodeString(",") + mode + UnicodeString(
		...
		message = (UnicodeString)"round(" + (double)Roundingnumber1 + UnicodeString(",") + mode + UnicodeString(
			",FALSE) with RoundingIncrement=1.0==>");
		verify(message, resultStr, result[i++]);
		message.remove();
		resultStr.remove();
...
#if defined(_MSC_VER)
// Ignore the noisy warning 4805 (comparisons between int and bool) in the function below as we use the ICU TRUE/FALSE
// macros
