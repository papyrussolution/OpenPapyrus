...
#define CONFIRM_GE(actual, expected, myAction) UPRV_BLOCK_MACRO_BEGIN { \
		if((actual)>=(expected)) { \
			...
			record_fail( \
				myAction + (UnicodeString)" returned " + (actual) + (UnicodeString)" instead of x >= " + (expected) + "\n"); \
		} \
} UPRV_BLOCK_MACRO_END
...
#define CONFIRM_UErrorCode(actual, expected, myAction) UPRV_BLOCK_MACRO_BEGIN { \
		if((expected)==(actual)) { \
			...
			record_fail(myAction + (UnicodeString)" returned " + u_errorName(actual) + " instead of " + u_errorName( \
				    expected) + "\n"); \
		} \
} UPRV_BLOCK_MACRO_END
...
static struct {
	...
	E_Where where;
	bool like[e_Where_count];
	bool inherits[e_Where_count];
}
...
void ResourceBundleTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void ResourceBundleTest::TestResourceBundles()
{
...
void ResourceBundleTest::TestConstruction()
{
	...
	strcpy(versionID1, "44.0"); // hardcoded, please change if the default.txt file or
	                            // ResourceBundle::kVersionSeparater is changed.

	strcpy(versionID2, "55.0"); // hardcoded, please change if the te_IN.txt file or
	                            // ResourceBundle::kVersionSeparater is changed.
...
bool ResourceBundleTest::testTag(const char * frag,
    bool in_Root,
    bool in_te,
    bool in_te_IN)
{
	...
	bool is_in[] = { in_Root, in_te, in_te_IN };
	...
	for(i = 0; i<bundles_count; ++i) {
		...
		for(j = e_te_IN; j>=e_Root; --j) {
			if(is_in[j] && param[i].inherits[j]) {
				if(j == actual_bundle) /* it's in the same bundle OR it's a nonexistent=default bundle
					                  (5) */
...
void ResourceBundleTest::record_pass(UnicodeString passMessage)
{
	logln(passMessage);
	++pass;
}

void ResourceBundleTest::record_fail(UnicodeString errMessage)
{
	err(errMessage);
	++fail;
}

void ResourceBundleTest::TestExemplar() {
	...
	for(; locIndex<locCount; locIndex++) {
		...
		if(U_SUCCESS(status) && status!=U_USING_FALLBACK_WARNING && status!=U_USING_DEFAULT_WARNING) {
			...
			if(U_FAILURE(status)) {
				errln("Could not construct UnicodeSet from pattern for ExemplarCharacters in locale : %s. Error: %s",
				    locale,
				    u_errorName(status));
				status = U_ZERO_ERROR;
			}
...
void ResourceBundleTest::TestGetSize(void)
{
	const struct {
		const char * key;
		int32_t size;
...
void ResourceBundleTest::TestGetLocaleByType(void)
{
	const struct {
