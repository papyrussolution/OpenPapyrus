	...
	for(int32_t lineNumber = 0;; lineNumber++) { // Loop once per input line.
		...
		if(fClassDefMatcher->matches(status)) {
			UnicodeString className =
			    fClassDefMatcher->group(fClassDefMatcher->pattern().groupNumberFromName("ClassName", status), status);
			UnicodeString classDef  = fClassDefMatcher->group(fClassDefMatcher->pattern().groupNumberFromName("ClassDef",
				status),
				status);
			if(fMonkeyImpl->fDumpExpansions) {
		...
		if(fRuleDefMatcher->matches(status)) {
			UnicodeString ruleName = fRuleDefMatcher->group(fRuleDefMatcher->pattern().groupNumberFromName("RuleName", status),
				status);
			UnicodeString ruleDef  = fRuleDefMatcher->group(fRuleDefMatcher->pattern().groupNumberFromName("RuleDef", status),
				status);
			if(fMonkeyImpl->fDumpExpansions) {
	...
	while(strIdx < fString.length()) {
		...
		for(ruleNum = 0; ruleNum<rules->fBreakRules.size(); ruleNum++) {
			...
			if(rule->fRuleMatcher->lookingAt(strIdx, status)) {
				// A candidate rule match, check further to see if we take it or continue to check other
				// rules.
...
void MonkeyTestData::dump(int32_t around) const {
	...
	for(int charIdx = start; charIdx < end; charIdx = fString.moveIndex32(charIdx, 1)) {
		...
		if(f2ndRuleForPos.charAt(charIdx) > 0) {
			const BreakRule * secondRule =
			    static_cast<BreakRule *>(fBkRules->fBreakRules.elementAt(f2ndRuleForPos.charAt(charIdx)));
...
void RBBIMonkeyImpl::testIsBoundaryRandom(UErrorCode & status) 
{
	...
	for(int i = stringLen; i>=0; --i) {
		...
		if(fTestData->fExpectedBreaks.charAt(strIdx) != static_cast<char16_t>(fBI->isBoundary(strIdx))) {
...
bool RBBIMonkeyTest::getIntParam(UnicodeString name, UnicodeString & params, int64_t &val, UErrorCode & status) {
	name.append(" *= *(-?\\d+) *,? *");
...
bool RBBIMonkeyTest::getStringParam(UnicodeString name, UnicodeString & params, CharString &dest, UErrorCode & status) {
	name.append(" *= *([^ ,]*) *,? *");
	RegexMatcher m(name, params, 0, status);
	if(m.find()) {
		// The param exists.
...
bool RBBIMonkeyTest::getBoolParam(UnicodeString name, UnicodeString & params, bool &dest, UErrorCode & status) {
