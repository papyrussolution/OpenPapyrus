...
void UTextTest::runIndexedTest(int32_t index, bool exec,
...
//
void UTextTest::TestCopyMove(const UnicodeString & us, UText * ut, bool move,
...
void UTextTest::FreezeTest() {
	...
	TEST_SUCCESS(status);
	bool writable = utext_isWritable(ut);
...
U_CDECL_BEGIN
static bool U_CALLCONV fragTextAccess(UText * ut, int64_t index, bool forward) {
...
//   out of the tests.
//
UText * cloneFragmentedUnicodeString(UText * dest, const UText * src, bool deep, UErrorCode * status) {
	if(U_FAILURE(*status)) {
		return NULL;
	}
	if(deep) {
...
void UTextTest::Ticket12130() {
	...
	for(int32_t startIdx = 0; startIdx<str.length(); ++startIdx) {
		...
		if(0 != str.tempSubString(startIdx, 20).compare(UnicodeString(extractBuffer))) {
