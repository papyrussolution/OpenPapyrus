...
private:
	...
	void RunTest(const Locale & locale,
	...
	void RunTest(const Locale & locale,
	    ...
	    const WithoutQuantityExpected* expectedResults,
	    int32_t expectedResultLength);
	void RunTest(const RelativeDateTimeFormatter& fmt,
	    const WithQuantityExpected* expectedResults,
	    int32_t expectedResultLength,
	...
	void RunTest(const RelativeDateTimeFormatter& fmt,
	    ...
	    int32_t expectedResultLength,
	    const char * description);
	void CheckExpectedResult(const RelativeDateTimeFormatter& fmt,
	...
	void CheckExpectedResult(const RelativeDateTimeFormatter& fmt,
	    ...
	    const char * description);
	void VerifyIllegalArgument(const RelativeDateTimeFormatter& fmt,
	    UDateDirection direction,
	    UDateRelativeUnit unit);
	void VerifyIllegalArgument(const RelativeDateTimeFormatter& fmt,
	    UDateDirection direction,
...
void RelativeDateTimeFormatterTest::runIndexedTest(int32_t index, bool exec, const char *&name, char *) {
	if(exec) {
...
void RelativeDateTimeFormatterTest::TestEnglishCaps() {
	...
	if(U_FAILURE(status)) {
		dataerrln(
			"Failed call to RelativeDateTimeFormatter(\"en\", NULL, UDAT_STYLE_LONG, UDISPCTX_CAPITALIZATION_FOR_BEGINNING_OF_SENTENCE, status); : %s",
			u_errorName(status));
		return;
	}
...
void RelativeDateTimeFormatterTest::TestGetters() {
	...
	if(U_FAILURE(status)) {
		dataerrln(
			"Failed call to RelativeDateTimeFormatter(\"en\", NULL, UDAT_STYLE_NARROW, UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE, status);) : %s",
			u_errorName(status));
		return;
	}
	RelativeDateTimeFormatter fmt3(status);
...
void RelativeDateTimeFormatterTest::RunTest(const Locale & locale,
    const WithQuantityExpected* expectedResults,
    int32_t expectedResultLength) {
	UErrorCode status = U_ZERO_ERROR;
...
void RelativeDateTimeFormatterTest::RunTest(const RelativeDateTimeFormatter& fmt,
    const WithQuantityExpected* expectedResults,
    int32_t expectedResultLength,
    const char * description) {
...
{
	switch(unit) {
		case UDAT_REL_UNIT_SECOND: return kSeconds;
		case UDAT_REL_UNIT_MINUTE: return kMinutes;
		case UDAT_REL_UNIT_HOUR: return kHours;
		case UDAT_REL_UNIT_DAY: return kDays;
		case UDAT_REL_UNIT_WEEK: return kWeeks;
		case UDAT_REL_UNIT_MONTH: return kMonths;
		case UDAT_REL_UNIT_QUARTER: return kQuarters;
		case UDAT_REL_UNIT_YEAR: return kYears;
		case UDAT_REL_UNIT_SUNDAY: return kSundays;
		case UDAT_REL_UNIT_MONDAY: return kMondays;
		case UDAT_REL_UNIT_TUESDAY: return kTuesdays;
		case UDAT_REL_UNIT_WEDNESDAY: return kWednesdays;
		case UDAT_REL_UNIT_THURSDAY: return kThursdays;
		case UDAT_REL_UNIT_FRIDAY: return kFridays;
		case UDAT_REL_UNIT_SATURDAY: return kSaturdays;
...
{
	switch(unit) {
		case UDAT_ABSOLUTE_SUNDAY: return kSunday;
		case UDAT_ABSOLUTE_MONDAY: return kMonday;
		case UDAT_ABSOLUTE_TUESDAY: return kTuesday;
		case UDAT_ABSOLUTE_WEDNESDAY: return kWednesday;
		case UDAT_ABSOLUTE_THURSDAY: return kThursday;
		case UDAT_ABSOLUTE_FRIDAY: return kFriday;
		case UDAT_ABSOLUTE_SATURDAY: return kSaturday;
		case UDAT_ABSOLUTE_DAY: return kDay;
		case UDAT_ABSOLUTE_WEEK: return kWeek;
		case UDAT_ABSOLUTE_MONTH: return kMonth;
		case UDAT_ABSOLUTE_QUARTER: return kQuarter;
		case UDAT_ABSOLUTE_YEAR: return kYear;
		case UDAT_ABSOLUTE_NOW: return kNow;
		default: return kUndefined;
	}
	return kUndefined;
}
