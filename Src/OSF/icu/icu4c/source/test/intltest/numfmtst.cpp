...
void NumberFormatTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	TESTCASE_AUTO_BEGIN;
	...
	TESTCASE_AUTO(TestComplexCurrency); // This test removed because CLDR no longer uses choice formats in currency
	                                    // symbols.
...
void NumberFormatTest::TestAPI(void)
{
	logln("Test API");
...
public:
	...
	virtual void parse(const UnicodeString &, Formattable&, ParsePosition&) const override 
	{
	}
	virtual void parse(const UnicodeString &, Formattable&, UErrorCode&) const override 
	{
	}
...
void NumberFormatTest::TestCoverage() {
...
void NumberFormatTest::TestLocalizedPatternSymbolCoverage() {
	...
	dfs.setSymbol(DecimalFormatSymbols::kGroupingSeparatorSymbol, UnicodeString(u'⁖'));
	dfs.setSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol, UnicodeString(u'⁘'));
	dfs.setSymbol(DecimalFormatSymbols::kPatternSeparatorSymbol, UnicodeString(u'⁙'));
	dfs.setSymbol(DecimalFormatSymbols::kDigitSymbol, UnicodeString(u'▰'));
	dfs.setSymbol(DecimalFormatSymbols::kZeroDigitSymbol, UnicodeString(u'໐'));
	dfs.setSymbol(DecimalFormatSymbols::kSignificantDigitSymbol, UnicodeString(u'⁕'));
	dfs.setSymbol(DecimalFormatSymbols::kPlusSignSymbol, UnicodeString(u'†'));
	dfs.setSymbol(DecimalFormatSymbols::kMinusSignSymbol, UnicodeString(u'‡'));
	dfs.setSymbol(DecimalFormatSymbols::kPercentSymbol, UnicodeString(u'⁜'));
	dfs.setSymbol(DecimalFormatSymbols::kPerMillSymbol, UnicodeString(u'‱'));
	dfs.setSymbol(DecimalFormatSymbols::kExponentialSymbol, u"⁑⁑"); // tests multi-char sequence
	dfs.setSymbol(DecimalFormatSymbols::kPadEscapeSymbol, UnicodeString(u'⁂'));
	{
		...
		UnicodeString p2;
		assertEquals("toPattern should match on localizedPattern instance", standardPattern, df2.toPattern(p2));
		UnicodeString lp1;
		assertEquals("toLocalizedPattern should match on standardPattern instance", localizedPattern, df1.toLocalizedPattern(lp1));
	}
	{
...
void NumberFormatTest::TestPatterns(void)
{
	UErrorCode status = U_ZERO_ERROR;
	...
	for(int32_t i = 0; i<pat_length; ++i) {
		...
		if(U_FAILURE(status)) {
			errln((UnicodeString)"FAIL: DecimalFormat constructor failed for " + pat[i]); continue;
		}
...
void NumberFormatTest::TestExponential(void)
{
	UErrorCode status = U_ZERO_ERROR;
	DecimalFormatSymbols sym(Locale::getUS(), status);
	if(U_FAILURE(status)) {
...
void NumberFormatTest::TestScientific2() {
	// jb 2552
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::TestScientificGrouping() {
	// jb 2552
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::TestInt64() {
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::TestQuotes(void)
{
	UErrorCode status = U_ZERO_ERROR;
	UnicodeString * pat;
 ...
 */
void NumberFormatTest::TestCurrencySign(void)
{
	UErrorCode status = U_ZERO_ERROR;
	DecimalFormatSymbols* sym = new DecimalFormatSymbols(Locale::getUS(), status);
	UnicodeString pat;
...
static char16_t toHexString(int32_t i) {
	return (char16_t)(i + (i < 10 ? 0x30 : (0x41 - 10)));
}

UnicodeString &NumberFormatTest::escape(UnicodeString & s)
{
	UnicodeString buf;
 ...
 */
void NumberFormatTest::TestCurrency(void)
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void NumberFormatTest::TestParse(void)
{
	UErrorCode status = U_ZERO_ERROR;
	UnicodeString arg("0");
 ...
 */
void NumberFormatTest::TestLenientParse(void)
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void NumberFormatTest::TestRounding487(void)
{
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::TestSecondaryGrouping() {
	...
	if(out.length() != 14) {
		...
		for(int32_t i = 0; i<out.length(); ++i) {
			...
			bool isGroup = (out.charAt(i) == 0x002C);
...
void NumberFormatTest::roundingTest(NumberFormat& nf, double x, int32_t maxFractionDigits, const char * expected)
{
...
void NumberFormatTest::TestPatterns2() {
	...
	expectPad(fmt, "#$*^$", ILLEGAL);
	expectPad(fmt, "'pre'#,##0*x'post'", DecimalFormat::kPadBeforeSuffix, 12, (char16_t)0x0078 /*x*/);
	expectPad(fmt, "''#0*x", DecimalFormat::kPadBeforeSuffix, 3, (char16_t)0x0078 /*x*/);
	expectPad(fmt, "'I''ll'*a###.##", DecimalFormat::kPadAfterPrefix, 10, (char16_t)0x0061 /*a*/);
	fmt.applyPattern("AA#,##0.00ZZ", status);
	CHECK(status, "applyPattern");
	fmt.setPadCharacter(UnicodeString(hat));
...
void NumberFormatTest::TestSurrogateSupport() {
	...
	custom.setSymbol(DecimalFormatSymbols::kZeroDigitSymbol, UnicodeString((char16_t)0x30));
...
void NumberFormatTest::TestCurrencyNames() {
	...
	bool isChoiceFormat;
	int32_t len;
	const bool possibleDataError = TRUE;
...
void NumberFormatTest::TestCurrencyVariants() {
	...
	struct TestCase {
		...
		{"en-US", u"CDF", u"CDF", u"CDF", u"CDF", u"CDF", U_USING_FALLBACK_WARNING}, // narrow: fallback to
		                                                                             // short
...
void NumberFormatTest::TestCurrencyUnit() {
	...
	char16_t * EUR = (char16_t *)SAlloc::M(6);
	EUR[0] = u'E';
	EUR[1] = u'U';
	EUR[2] = u'R';
	char * EUR8 = (char *)SAlloc::M(3);
	EUR8[0] = 'E';
	EUR8[1] = 'U';
	EUR8[2] = 'R';
...
bool NumberFormatTest::equalValue(const Formattable& a, const Formattable& b) {
...
void NumberFormatTest::expect_rbnf(NumberFormat& fmt, const Formattable& n,
    const UnicodeString & exp, bool rt) {
	UnicodeString saw;
	FieldPosition pos;
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::expectCurrency(NumberFormat& nf, const Locale & locale,
	...
	UnicodeString s;
	fmt.format(value, s);
	s.findAndReplace(UnicodeString((UChar32)0x00A0), UnicodeString((UChar32)0x0020));
...
void NumberFormatTest::TestCurrencyFormat()
{
	...
	if(result != toFormat) {
		errln("measureObj does not round trip. Formatted string was \"" + str + "\" Got: " + toString(
			    result) + " Expected: " + toString(toFormat));
	}
	status = U_ZERO_ERROR;
	str.truncate(0);
	...
	if(result != toFormat) {
		errln("Clone does not round trip. Formatted string was \"" + str + "\" Got: " + toString(result) + " Expected: " +
		    toString(toFormat));
	}
...
void NumberFormatTest::TestRoundingPattern() {
	...
	for(int32_t i = 0; i < numOfTests; i++) {
		...
		if(result != tests[i].expected) {
			errln(
				"String Pattern Rounding Test Failed: Pattern: \"" + tests[i].pattern + "\" Number: " + tests[i].testCase + " - Got: " + result + " Expected: " +
				tests[i].expected);
		}
	}
...
void NumberFormatTest::TestNonpositiveMultiplier() {
	...
	//expect2(df, java.math.BigDecimal.valueOf(Long.MAX_VALUE),
	// java.math.BigDecimal.valueOf(Long.MAX_VALUE).negate().toString());
	//expect2(df, java.math.BigDecimal.valueOf(Long.MIN_VALUE),
...
void NumberFormatTest::TestSpaceParsing() {
	// the data are:
	...
	for(uint32_t i = 0; i < SIZEOFARRAYi(DATA); ++i) {
		...
		if(parsePosition.getIndex() != parsedPosition ||
			...
			errln(
				"FAILED parse " + stringToBeParsed + "; lenient: " + DATA[i].lenient + "; wrong position, expected: (" + parsedPosition + ", " + errorIndex + "); got (" + parsePosition.getIndex() + ", " + parsePosition.getErrorIndex() +
				")");
		}
...
void NumberFormatTest::TestNumberingSystems() {
	const TestNumberingSystemItem DATA[] = {
		...
		{ "th_TH@numbers=traditional", 1234.567, FALSE, "\\u0E51,\\u0E52\\u0E53\\u0E54.\\u0E55\\u0E56\\u0E57" }, // fall
		                                                                                                         // back
		                                                                                                         // to
		                                                                                                         // native
		                                                                                                         // per
		...
		{ "zh_TW@numbers=traditional", 1234.567, TRUE,
		  "\\u4E00\\u5343\\u4E8C\\u767E\\u4E09\\u5341\\u56DB\\u9EDE\\u4E94\\u516D\\u4E03" },
		{ "zh_TW@numbers=finance", 1234.567, TRUE,
...
void NumberFormatTest::TestMultiCurrencySign() {
	const char * DATA[][6] = {
		...
		{"en_US", "\\u00A4#,##0.00;-\\u00A4#,##0.00", "-1234.56", "-$1,234.56", "-USD\\u00A01,234.56",
		 "-US dollars\\u00A01,234.56"},
		{"en_US", "\\u00A4#,##0.00;-\\u00A4#,##0.00", "1", "$1.00", "USD\\u00A01.00", "US dollars\\u00A01.00"},
		// for CHINA locale
		{"zh_CN", "\\u00A4#,##0.00;(\\u00A4#,##0.00)", "1234.56", "\\u00A51,234.56", "CNY\\u00A01,234.56",
		 "\\u4EBA\\u6C11\\u5E01\\u00A01,234.56"},
		{"zh_CN", "\\u00A4#,##0.00;(\\u00A4#,##0.00)", "-1234.56", "(\\u00A51,234.56)", "(CNY\\u00A01,234.56)",
		 "(\\u4EBA\\u6C11\\u5E01\\u00A01,234.56)"},
...
void NumberFormatTest::TestCurrencyFormatForMixParsing() {
	UErrorCode status = U_ZERO_ERROR;
...
void NumberFormatTest::TestMismatchedCurrencyFormatFail() {
	...
	if(!assertSuccess("createCurrencyInstance() failed.", status, true, __FILE__, __LINE__)) {
		return;
	}
	UnicodeString pattern;
...
void NumberFormatTest::TestDecimalFormatCurrencyParse() {
	// Locale.US
	UErrorCode status = U_ZERO_ERROR;
	...
	char16_t currency = 0x00A4;
	// "\xA4#,##0.00;-\xA4#,##0.00"
	pat.append(currency).append(currency).append(currency).append("#,##0.00;-").append(currency).append(currency).append(currency).
	append("#,##0.00");
	DecimalFormat* fmt = new DecimalFormat(pat, sym, status);
	if(U_FAILURE(status)) {
		delete fmt;
...
void NumberFormatTest::TestCurrencyIsoPluralFormat() {
	static const char * DATA[][6] = {
		...
		{"ru_RU", "1", "RUB", "1,00\\u00A0\\u20BD", "1,00\\u00A0RUB",
		 "1,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E \\u0440\\u0443\\u0431\\u043B\\u044F"},
		{"ru_RU", "2", "RUB", "2,00\\u00A0\\u20BD", "2,00\\u00A0RUB",
		 "2,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E \\u0440\\u0443\\u0431\\u043B\\u044F"},
		{"ru_RU", "5", "RUB", "5,00\\u00A0\\u20BD", "5,00\\u00A0RUB",
		 "5,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E \\u0440\\u0443\\u0431\\u043B\\u044F"},
	...
	for(int32_t i = 0; i<SIZEOFARRAYi(DATA); ++i) {
		...
		for(int32_t kIndex = 0; kIndex < SIZEOFARRAYi(currencyStyles); ++kIndex) {
			...
			if(U_FAILURE(status)) {
				delete numFmt;
				dataerrln((UnicodeString)"can not create instance, locale:" + localeString + ", style: " + k + " - " + u_errorName(
					    status));
				continue;
			}
			...
			for(int j = 3; j < 6; ++j) {
				...
				if(U_FAILURE(status) ||
					...
					if(parseResult.getType() == Formattable::kDouble) {
						errln((UnicodeString)"expected: " + numberToBeFormat + "; actual: " +
...
void NumberFormatTest::TestCurrencyParsing() {
	static const char * DATA[][6] = {
		// the data are:
		// locale,
		// currency amount to be formatted,
		...
		// format result using PLURALCURRENCYSTYLE,
		{"en_US", "1", "USD", "$1.00", "USD\\u00A01.00", "1.00 US dollars"},
		{"pa_IN", "1", "USD", "US$\\u00A01.00", "USD\\u00A01.00",
		 "1.00 \\u0a2f\\u0a42.\\u0a10\\u0a38. \\u0a21\\u0a3e\\u0a32\\u0a30"},
		{"es_AR", "1", "USD", "US$\\u00A01,00", "USD\\u00A01,00", "1,00 d\\u00f3lar estadounidense"},
		{"ar_EG", "1", "USD", "\\u0661\\u066b\\u0660\\u0660\\u00a0US$", "\\u0661\\u066b\\u0660\\u0660\\u00a0USD",
		 "\\u0661\\u066b\\u0660\\u0660 \\u062f\\u0648\\u0644\\u0627\\u0631 \\u0623\\u0645\\u0631\\u064a\\u0643\\u064a"},
		{"fa_CA", "1", "USD", "\\u200e$\\u06f1\\u066b\\u06f0\\u06f0", "\\u200eUSD\\u06f1\\u066b\\u06f0\\u06f0",
		 "\\u06f1\\u066b\\u06f0\\u06f0 \\u062f\\u0644\\u0627\\u0631 \\u0622\\u0645\\u0631\\u06cc\\u06a9\\u0627"},
		{"he_IL", "1", "USD", "\\u200f1.00\\u00a0$", "\\u200f1.00\\u00a0USD",
		 "1.00 \\u05d3\\u05d5\\u05dc\\u05e8 \\u05d0\\u05de\\u05e8\\u05d9\\u05e7\\u05d0\\u05d9"},
			...
			for(int j = 3; j < 6; ++j) {
				...
				if(U_FAILURE(status) ||
					...
					errln((UnicodeString)"FAIL: NumberFormat(" + localeString +", " + currencyStyleNames[kIndex] +
					    "), Currency="+currencyISOCode+", parse("+DATA[i][j]+") returned error " + (UnicodeString)u_errorName(
						    status)+".  Testcase: data[" + i + "][" + currencyStyleNames[j-3] +"="+j+"]");
					if(parseResult.getType() == Formattable::kDouble) {
						errln(
							(UnicodeString)"expected: " + numberToBeFormat + "; actual (double): " +
							parseResult.getDouble());
					}
					else {
						errln(
							(UnicodeString)"expected: " + numberToBeFormat + "; actual (long): " +
							parseResult.getLong());
					}
...
void NumberFormatTest::TestParseCurrencyInUCurr() {
	const char * DATA[] = {
...
void NumberFormatTest::Test9087(void)
{
	...
	}
	{
		...
		if(U_FAILURE(status)) {
			...
			if(U_FAILURE(status)||expect!=result) {
				...
				logln("OK:  got expected '"+result+"' status "+UnicodeString(u_errorName(
					    status), "")+" on -9223372036854775808");
			}
		}
	}
	{
...
bool NumberFormatTest::testFormattableAsUFormattable(const char * file, int line, Formattable &f) {
		...
		case Formattable::kString:
		    expectUType = UFMT_STRING;
		    {
			    ...
			    if(U_SUCCESS(valueStatus)) {
				    ...
				    assertTrue("char16_t * NULL-terminated", uch[len]==0);
				    exactMatch = (str == str2);
			    }
			    triedExact = TRUE;
		    }
			    ...
			    if(exactMatch) {
				    for(int i = 0; U_SUCCESS(valueStatus) && i<count; i++) {
					    ...
					    if(*Formattable::fromUFormattable(uu) != (array2[i])) {
						    ...
						    if(!testFormattableAsUFormattable("(sub item)", i,
...
void NumberFormatTest::TestUFormattable() {
	...
	{
		Formattable f((int64_t)80994231587905127LL); // weight of the moon, in kilotons
		...
		if(U_SUCCESS(status)) {
			...
			if(ppos.getIndex() == itemPtr->numString.length()) {
				...
				if(U_FAILURE(status) || parsedValue != itemPtr->value) {
					errln(
						(UnicodeString)"FAIL: locale " + itemPtr->locale + ", lenient " + itemPtr->lenient + ", parse of \"" + itemPtr->numString + "\" gives value " +
						parsedValue);
				}
			}
			else {
				errln(
					(UnicodeString)"FAIL: locale " + itemPtr->locale + ", lenient " + itemPtr->lenient + ", parse of \"" + itemPtr->numString + "\" gives position " +
					ppos.getIndex());
			}
		}
		else {
...
void NumberFormatTest::verifyRounding(DecimalFormat& format,
    const double * values,
...
void NumberFormatTest::TestCurrencyFormatForMissingLocale() {
	...
	if(!assertTrue(WHERE, curFmt.isValid(), false, true)) {
		return;
	}
	;
...
void NumberFormatTest::TestDoubleLimit11439() {
	...
	for(int64_t num = MAX_INT64_IN_DOUBLE-10; num<=MAX_INT64_IN_DOUBLE; num++) {
		...
		if(num != rtNum) {
			errln("%s:%d MAX_INT64_IN_DOUBLE test, %lld did not round trip. Got %lld", __FILE__, __LINE__, (long long)num,
			    (long long)rtNum);
			return;
		}
	}
...
void NumberFormatTest::verifyFieldPositionIterator(NumberFormatTest_Attributes * expected, FieldPositionIterator &iter) {
	int32_t idx = 0;
	FieldPosition fp;
	while(iter.next(fp)) {
...
void NumberFormatTest::TestFormatFailIfMoreThanMaxDigits() {
	...
	assertEquals("Coverage for getter 1", (bool)FALSE, df.isFormatFailIfMoreThanMaxDigits());
	df.setFormatFailIfMoreThanMaxDigits(TRUE);
	assertEquals("Coverage for getter 2", (bool)TRUE, df.isFormatFailIfMoreThanMaxDigits());
...
void NumberFormatTest::TestParseCaseSensitive() {
	...
	assertEquals("Coverage for getter 1", (bool)FALSE, df.isParseCaseSensitive());
	df.setParseCaseSensitive(TRUE);
	assertEquals("Coverage for getter 1", (bool)TRUE, df.isParseCaseSensitive());
	Formattable result;
	ParsePosition ppos;
...
void NumberFormatTest::TestParseNoExponent() {
	...
	assertEquals("Coverage for getter 1", (bool)FALSE, df.isParseNoExponent());
	df.setParseNoExponent(TRUE);
	assertEquals("Coverage for getter 1", (bool)TRUE, df.isParseNoExponent());
	Formattable result;
	ParsePosition ppos;
	df.parse(u"1E2", result, ppos);
	assertEquals("Should parse only 1 digit", 1, ppos.getIndex());
...
void NumberFormatTest::TestSignAlwaysShown() {
	...
	assertEquals("Coverage for getter 1", (bool)FALSE, df.isSignAlwaysShown());
	df.setSignAlwaysShown(TRUE);
	assertEquals("Coverage for getter 1", (bool)TRUE, df.isSignAlwaysShown());
	UnicodeString result;
	df.format(1234, result, status);
	status.errIfFailureAndReset();
...
void NumberFormatTest::Test11839() {
	...
	if(!assertSuccess("", errorCode, true, __FILE__, __LINE__)) {
		return;
	}
...
void NumberFormatTest::Test13148_ParseGroupingSeparators() {
	...
	symbols.setSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol, UnicodeString(u'.'));
	symbols.setSymbol(DecimalFormatSymbols::kGroupingSeparatorSymbol, UnicodeString(u','));
