...
static UChar toHexString(int32_t i) {
...
void NumberFormatTest::TestPatterns2() {
	...
	expectPad(fmt, "#$*^$", ILLEGAL);
	expectPad(fmt, "'pre'#,##0*x'post'", DecimalFormat::kPadBeforeSuffix, 12, (UChar)0x0078 /*x*/);
	expectPad(fmt, "''#0*x", DecimalFormat::kPadBeforeSuffix, 3, (UChar)0x0078 /*x*/);
	expectPad(fmt, "'I''ll'*a###.##", DecimalFormat::kPadAfterPrefix, 10, (UChar)0x0061 /*a*/);
...
void NumberFormatTest::TestSurrogateSupport() {
	...
	custom.setSymbol(DecimalFormatSymbols::kZeroDigitSymbol, UnicodeString((UChar)0x30));
