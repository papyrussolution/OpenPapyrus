...
void IdnaConfTest::ExplainCodePointTag(UnicodeString & buf) {
	buf.append((char16_t)0); // add a terminal NULL
		...
		if(passfail == 0) {
			if(U_FAILURE(status)) {
				...
				if(namezone.compare(result, -1) == 0) {
					// expected
					logln(UnicodeString("namebase: ") + prettify(namebase) + UnicodeString(" result: ") +
					    prettify(result));
				}
				else {
			...
			if(U_FAILURE(status)) {
				...
				if(namebase.compare(result, -1) == 0) {
					...
					logln(UnicodeString(
						    "ICU will not recognize malformed ACE-Prefixes or incorrect ACE-Prefixes. ") +
					    UnicodeString("namebase: ") + prettify(namebase) + UnicodeString(" result: ") +
					    prettify(result));
				}
				else {
...
void IdnaConfTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/) 
{
	switch(index) {
		TESTCASE(0, Test);
		default: name = ""; break;
