...
void TestChoiceFormat::runIndexedTest(int32_t index, bool exec,
...
static bool chkstatus(UErrorCode & status, const char * msg = NULL)
{
	bool ok = U_SUCCESS(status);
	if(!ok) it_errln(msg);
	return ok;
}
...
void TestChoiceFormat::TestSimpleExample(void)
{
	...
	const bool * gotClosures = form->getClosures(count);
	...
	for(ix = 0; ix<count; ix++) {
		if(gotLimits[ix] != limits[ix]) {
			errln((UnicodeString)"getLimits didn't get the limits correctly.  Expected " + limits[ix] + " Got " +
			    gotLimits[ix]);
		}
	}
	//Testing getFormats()
	...
	for(ix = 0; ix<count; ix++) {
		if(gotFormats[ix] != monthNames[ix]) {
			errln(
				(UnicodeString)"getFormats didn't get the Formats correctly.  Expected " + monthNames[ix] + " Got " +
				gotFormats[ix]);
		}
	}
#endif
...
void TestChoiceFormat::TestComplexExample(void)
{
	...
	for(i = 0; i < 4; ++i) {
		...
		if(res2 != checkstr2[i]) {
			it_errln("***  test_complex_example res string");
			it_errln(UnicodeString("*** ") + i + UnicodeString(" -> '") + res2 + UnicodeString(
				    "' unlike '") + checkstr2[i] + UnicodeString("' ! "));
		}
	}
	...
	if(!form_A || !form_B || !form_A2) {
		...
		bool ok = (form_Assigned == *form_A) && (form_Assigned != *form_B);
...
void TestChoiceFormat::TestClosures() {
	...
	const bool * closures2 = fmt2.getClosures(count2);
...
void TestChoiceFormat::TestChoiceFormatToPatternOverflow()
{
	...
	ChoiceFormat fmt(limits, monthNames, SIZEOFARRAYi(limits));
