...
void IntlTestCollator::runIndexedTest(int32_t index, bool exec, const char *& name, char * par)
{
	if(exec) {
...
void IntlTestCollator::doTestVariant(Collator* col,
	...
	if(!quick) { /*!QUICK*/
		...
		if(U_FAILURE(status)) { /* probably buffer is not big enough */
			...
			if(U_SUCCESS(status)) { /* probably buffer is not big enough */
				UCollationResult compareResultUTF8 = (UCollationResult)result,
				    compareResultUTF8Norm = (UCollationResult)result;
				UCharIterator sIter, tIter;
				...
				if(compareResultUTF8 != compareResultIter) {
					errln(
						"different results in iterative comparison for UTF-16 and UTF-8 encoded strings. "+source+", "+
						target);
				}
				if(compareResultUTF8 != compareResultUTF8Norm) {
					errln(
						"different results in iterative when normalization is turned on with UTF-8 strings. "+source+", "+
						target);
				}
			}
			else {
		...
		for(i = 0; i < partialSizesSize; i++) {
			...
			if(norm != UCOL_ON && !quick) {
				...
				partialNormalizedSKResult =
				    compareUsingPartials(myCollation, src, sLen, trg, tLen, partialSizes[i], status);
				ucol_setAttribute(myCollation, UCOL_NORMALIZATION_MODE, norm, &status);
				if(partialSKResult != partialNormalizedSKResult) {
					errln(
						"Partial sortkey comparison gets different result when normalization is on: "+source+", "+target+" (size "+
						partialSizes[i]+")");
				}
			}
		}
...
void IntlTestCollator::doTest(Collator* col, const UnicodeString & source, const UnicodeString & target, Collator::EComparisonResult result)
{
	if(col) {
...
// (maybe moved later)
void IntlTestCollator::reportCResult(const UnicodeString & source, const UnicodeString & target,
...
UnicodeString &IntlTestCollator::appendCompareResult(Collator::EComparisonResult result,
