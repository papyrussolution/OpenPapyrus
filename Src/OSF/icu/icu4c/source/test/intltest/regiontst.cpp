...
void RegionTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * par)
{
	optionv = (par && *par=='v');
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(knownRegions); i++) {
		...
		if(r) {
			...
			if(nc > 0) {
				...
				if(*ncRegion != *r && nc != 891) { // 891 is special case - CS and YU both deprecated
					                           // codes for region 891
					errln("Creating region %s by its numeric code returned a different region. Got: %s instead.",
					    r->getRegionCode(),
					    ncRegion->getRegionCode());
				}
			}
		}
		else {
...
void RegionTest::TestGetInstanceString() {
	...
	if(status != U_ILLEGAL_ARGUMENT_ERROR) {
		errcheckln(status,
		    "Calling Region::getInstance(NULL) should have triggered an U_ILLEGAL_ARGUMENT_ERROR, but didn't. - %s",
		    u_errorName(status));
	}
	...
	if(status != U_ILLEGAL_ARGUMENT_ERROR) {
		errcheckln(status,
		    "Calling Region::getInstance(\"BOGUS\") should have triggered an U_ILLEGAL_ARGUMENT_ERROR, but didn't. - %s",
		    u_errorName(status));
	}
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(testData); i++) {
		...
		if(type != data.expectedType) {
			dataerrln("Unexpected region type for Region::getInstance(\"%s\"); Expected: %d Got: %d",
			    data.inputID,
			    data.expectedType,
			    type);
		}
...
void RegionTest::TestGetInstanceInt() {
	...
	if(status != U_ILLEGAL_ARGUMENT_ERROR) {
		errcheckln(status,
		    "Calling Region::getInstance(-123) should have triggered an U_ILLEGAL_ARGUMENT_ERROR, but didn't. - %s",
		    u_errorName(status));
	}
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(testData); i++) {
		...
		if(data.expectedType != type) {
			dataerrln("Unexpected region type for Region.getInstance(%d)); Expected: %d Got: %d",
			    data.inputID,
			    data.expectedType,
			    type);
		}
		...
		if(r) {
			...
			for(int32_t i = 0; i < containedRegions->count(status); i++) {
				...
				if(!containingRegion || *containingRegion != *r) {
					errln(
						"Region: %s contains region %s. Expected containing region of this region to be the original region, but got %s",
						r->getRegionCode(),
						cr->getRegionCode(),
						containingRegion ? containingRegion->getRegionCode() : "NULL");
				}
			}
...
void RegionTest::TestGetContainedRegionsWithType() {
	for(int32_t i = 0; i < SIZEOFARRAYi(knownRegions); i++) {
		...
		if(r) {
			...
			if(U_FAILURE(status)) {
				errln("%s->getContainedRegions(URGN_TERRITORY, status) failed: %s", r->getRegionCode(),
				    u_errorName(status));
				continue;
			}
			for(int32_t j = 0; j < containedRegions->count(status); j++) {
				...
				if(!containingRegion || *containingRegion != *r) {
					errln(
						"Continent: %s contains territory %s. Expected containing continent of this region to be the original region, but got %s",
						r->getRegionCode(),
						cr->getRegionCode(),
						containingRegion ? containingRegion->getRegionCode() : "NULL");
				}
...
void RegionTest::TestGetContainingRegion() {
	for(int32_t i = 0; i < SIZEOFARRAYi(knownRegions); i++) {
		...
		if(r) {
			...
			if(rd.parent == NULL) {
				if(c) {
					errln("Containing region for %s should have been NULL.  Got: %s",
					    r->getRegionCode(),
					    c->getRegionCode());
				}
			}
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(knownRegions); i++) {
		...
		if(r) {
			...
			if(rd.containingContinent == NULL) {
				if(c != NULL) {
					errln("Containing continent for %s should have been NULL.  Got: %s",
					    r->getRegionCode(),
					    c->getRegionCode());
				}
			}
...
void RegionTest::TestGetPreferredValues() {
	static const char * testData[6][17] = {
		...
		{ "AN", "CW", "SX", "BQ", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL }, // Netherlands
		                                                                                                          // Antilles
		{ "CS", "RS", "ME", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL }, // Serbia
		                                                                                                          // &
		                                                                                                          // Montenegro
		{ "FQ", "AQ", "TF", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL }, // French
		                                                                                                          // Southern
		                                                                                                          // and
		                                                                                                          // Antarctic
		                                                                                                          // Territories
		{ "NT", "IQ", "SA", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL }, // Neutral
		                                                                                                          // Zone
		{ "PC", "FM", "MH", "MP", "PW", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, }, // Pacific
		                                                                                                           // Islands
		                                                                                                           // Trust
		                                                                                                           // Territory
		{ "SU", "RU", "AM", "AZ", "BY", "EE", "GE", "KZ", "KG", "LV", "LT", "MD", "TJ", "TM", "UA", "UZ", NULL}, // Soviet
		                                                                                                         // Union
	};
	...
	for(int32_t i = 0; i < 6; i++) {
		...
		if(r) {
			...
			for(int i = 1; data[i]; i++) {
				...
				if(!found) {
					errln("Region::getPreferredValues() for region \"%s\" should have contained \"%s\" but it didn't.",
					    r->getRegionCode(),
					    data[i]);
				}
			}
...
void RegionTest::TestContains() {
	for(int32_t i = 0; i < SIZEOFARRAYi(knownRegions); i++) {
		...
		if(r) {
			...
			while(c) {
				if(!c->contains(*r)) {
					errln("Region \"%s\" should have contained \"%s\" but it didn't.",
					    c->getRegionCode(),
					    r->getRegionCode());
				}
