...
void CompoundTransliteratorTest::runIndexedTest(int32_t index, bool exec,
...
void CompoundTransliteratorTest::TestConstruction() {
	...
	}
	{
		...
		if(newID!=UnicodeString("Hangul-Latin")) {
			errln(UnicodeString(
				    "Test for Jitterbug 914 for cpdTrans(UnicodeString(\"Latin-Hangul\"),UTRANS_REVERSE,NULL,err) failed"));
		}
	}
	delete t1;
	delete t2;
...
void CompoundTransliteratorTest::TestTransliterate() {
	...
	if(U_FAILURE(status)) {
		...
		expectAux(ct1->getID() + ":String, index(0,0,0), incremental=FALSE",
		    rsource2 + "->" + rsource2,
	...
	UnicodeString Data[] = {
		...
		//"Devanagari-Latin;Latin-Devanagari",        CharsToUnicodeString("\\u092D\\u0948'\\u0930'\\u0935"),
		//  CharsToUnicodeString("\\u092D\\u0948\\u0930\\u0935"), // quotes lost
	...
	uint32_t i;
	for(i = 0; i<SIZEOFARRAYi(Data); i = i+3) {
		UErrorCode status = U_ZERO_ERROR;
