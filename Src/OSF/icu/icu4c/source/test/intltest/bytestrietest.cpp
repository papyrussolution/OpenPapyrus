...
public:
	...
	BytesTrie * buildTrie(const StringAndValue data[], int32_t dataLength, UStringTrieBuildOption buildOption);
...
extern IntlTest * createBytesTrieTest() { return new BytesTrieTest(); }
...
void BytesTrieTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
	if(exec) {
...
void BytesTrieTest::TestEmpty() {
	...
	checkData(data, SIZEOFARRAYi(data));
}
...
void BytesTrieTest::TestBranches() {
	...
	for(int32_t length = 2; length<=SIZEOFARRAYi(data); ++length) {
...
BytesTrie * BytesTrieTest::buildMonthsTrie(UStringTrieBuildOption buildOption) {
	...
	return buildTrie(data, SIZEOFARRAYi(data), buildOption);
}
...
void BytesTrieTest::TestGetNextBytes() {
	...
	char buffer[40];
	CheckedArrayByteSink sink(buffer, SIZEOFARRAYi(buffer));
...
void BytesTrieTest::TestIteratorFromBranch() {
	...
	checkIterator(iter, data, SIZEOFARRAYi(data));
	// Reset, and we should get the same result.
	logln("after iter.reset()");
	checkIterator(iter.reset(), data, SIZEOFARRAYi(data));
}
...
void BytesTrieTest::TestTruncatingIteratorFromLinearMatchShort() {
	...
	LocalPointer<BytesTrie> trie(buildTrie(data, SIZEOFARRAYi(data), USTRINGTRIE_BUILD_FAST));
	if(trie.isNull()) {
		return; // buildTrie() reported an error
	}
	// Go into a linear-match node.
	...
	checkIterator(iter, expected, SIZEOFARRAYi(expected));
	// Reset, and we should get the same result.
	logln("after iter.reset()");
	checkIterator(iter.reset(), expected, SIZEOFARRAYi(expected));
}
...
void BytesTrieTest::TestDelta() {
	...
	for(int32_t delta : sampleDeltas) {
		...
		memzero(intBytes0, sizeof(intBytes0));
		...
		for(int32_t j = 0; j<stringLength; ++j) {
			...
			if(result==USTRINGTRIE_INTERMEDIATE_VALUE) {
				...
				if(trie.current()!=USTRINGTRIE_INTERMEDIATE_VALUE) {
					errln("trie.getValue().current()!=USTRINGTRIE_INTERMEDIATE_VALUE before end of %s (at index %d)",
					    data[i].s,
					    j);
					break;
				}
...
void BytesTrieTest::checkIterator(BytesTrie::Iterator &iter,
	...
	for(int32_t i = 0; i<dataLength; ++i) {
		...
		bool hasNext = iter.next(errorCode);
