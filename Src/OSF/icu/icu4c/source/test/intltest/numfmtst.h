...
class NumberFormatDataDrivenTest : public DataDrivenNumberFormatTestSuite {
  public:
    void runIndexedTest( int32_t index, bool exec, const char *& name, char * par ) override;
...
class NumberFormatTest: public CalendarTimeZoneTest {
 ...
 private:
    bool testFormattableAsUFormattable(const char *file, int line, Formattable &f);

    void expectParseCurrency(const NumberFormat &fmt, const char16_t * currency, double amount, const char *text);

    static bool equalValue(const Formattable& a, const Formattable& b);
    ...
    void expect(NumberFormat& fmt, const Formattable& n,
                const UnicodeString & exp, bool rt=TRUE);

    void expect(NumberFormat& fmt, const Formattable& n,
                const char *exp, bool rt=TRUE) {
        expect(fmt, n, UnicodeString(exp, ""), rt);
    }

    void expect(NumberFormat* fmt, const Formattable& n,
                const UnicodeString & exp, bool rt, UErrorCode errorCode);

    void expect(NumberFormat* fmt, const Formattable& n,
                const char *exp, bool rt, UErrorCode errorCode) {
