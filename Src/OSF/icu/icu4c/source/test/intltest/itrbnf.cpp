...
void IntlTestRBNF::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	...
	// test rule constructor
	{
		...
		if(U_FAILURE(status)) {
			...
			while(ures_hasNext(ruleSets.getAlias())) {
				const char16_t * currentString = ures_getNextString(ruleSets.getAlias(), &len, NULL, &status);
				ruleLen += len;
			...
			if(U_FAILURE(status) || ruleLen == 0 || spelloutRules == NULL) {
				...
				if(!(ruleCtorResult == *formatter)) {
					errln(
						"Formatter constructed from the original rules should be semantically equivalent to the original!");
				}
...
void IntlTestRBNF::TestMultiplePluralRules() {
	...
	resultStr.remove();
	formatter.parse(formatter.format(1000.0, UnicodeString("%spellout-cardinal-feminine-genitive"), resultStr, pos, status),
	    result,
	    status);
...
void IntlTestRBNF::TestLLong()
{
	...
	// also test commutivity
	{
		...
		const llong* answers[VALUE_WIDTH*VALUE_WIDTH] = {
			...
			&ZERO, &TWO_TO_16, &NEG_TWO_TO_16, &TWO_TO_16X3, &NEG_TWO_TO_16X3, &TWO_TO_32, &NEG_TWO_TO_32, &TWO_TO_48,
			&NEG_TWO_TO_48,
			&ZERO, &NEG_TWO_TO_16, &TWO_TO_16, &NEG_TWO_TO_16X3, &TWO_TO_16X3, &NEG_TWO_TO_32, &TWO_TO_32, &NEG_TWO_TO_48,
			&TWO_TO_48,
...
void IntlTestRBNF::TestGermanSpellout()
{
	...
	if(U_FAILURE(status)) {
		...
		static const char * const testData[][2] = {
			...
			{ "2,345,678",
			  "zwei Millionen drei\\u00ADhundert\\u00ADf\\u00fcnf\\u00ADund\\u00ADvierzig\\u00ADtausend\\u00ADsechs\\u00ADhundert\\u00ADacht\\u00ADund\\u00ADsiebzig" },
			{ NULL, NULL}
		};
...
//        { "1,234,567,890.0987654", "one billion, two hundred and thirty-four million, five hundred and sixty-seven
// thousand, eight hundred and ninety point zero nine eight seven six five four" },
//        { "123,456,789.9876543", "one hundred and twenty-three million, four hundred and fifty-six thousand, seven
// hundred and eighty-nine point nine eight seven six five four three" },
//        { "12,345,678.87654321", "twelve million, three hundred and forty-five thousand, six hundred and seventy-eight
// point eight seven six five four three two one" },
			{ "1,234,567.7654321",
	...
	for(int i = 0; i < count; ++i) {
		...
		for(int j = 0; j < 2; ++j) {
			...
			for(unsigned int numidx = 0; numidx < SIZEOFARRAYi(numbers); numidx++) {
				...
				// We only check the spellout. The behavior is undefined for numbers < 1 and fractional
				// numbers.
				if(j == 0) {
					...
					else if(num.getType() == Formattable::kDouble &&
					    (int64_t)(num.getDouble() * 1000) != (int64_t)(n*1000)) {
...
void IntlTestRBNF::TestMultiplierSubstitution() {
...
void IntlTestRBNF::TestLargeNumbers() {
	...
	const char * const enTestFullData[][2] = {
		{"-9007199254740991",
		 "minus nine quadrillion seven trillion one hundred ninety-nine billion two hundred fifty-four million seven hundred forty thousand nine hundred ninety-one"},                  // Maximum
		                                                                                                                                                                                // precision
		                                                                                                                                                                                // in
		                                                                                                                                                                                // both
		                                                                                                                                                                                // a
		                                                                                                                                                                                ...
		                                                                                                                                                                                // long
		{"9007199254740991",
		 "nine quadrillion seven trillion one hundred ninety-nine billion two hundred fifty-four million seven hundred forty thousand nine hundred ninety-one"},                 // Maximum
		                                                                                                                                                                         // precision
		                                                                                                                                                                         // in
		                                                                                                                                                                         // both
		                                                                                                                                                                         // a
		                                                                                                                                                                         ...
		                                                                                                                                                                         // long
		{"-9007199254740992",
		 "minus nine quadrillion seven trillion one hundred ninety-nine billion two hundred fifty-four million seven hundred forty thousand nine hundred ninety-two"}, // Only precisely contained in a long
		{"9007199254740992",
		 "nine quadrillion seven trillion one hundred ninety-nine billion two hundred fifty-four million seven hundred forty thousand nine hundred ninety-two"}, // Only precisely contained in a long
		{"9999999999999998",
		...
		{"9223372036854775808", "9,223,372,036,854,775,808"}, // We've gone beyond 64-bit precision. This can
		                                                      // only be represented with BigDecimal.
		{ NULL, NULL }
	};
...
void IntlTestRBNF::TestParseFailure() {
	...
	if(assertSuccess("", status, true, __FILE__, __LINE__)) {
		for(int i = 0; i < SIZEOFARRAYi(testData); ++i) {
			...
			if(status != U_INVALID_FORMAT_ERROR) { // I would have expected U_PARSE_ERROR, but
				                               // NumberFormat::parse gives U_INVALID_FORMAT_ERROR
...
void IntlTestRBNF::doTest(RuleBasedNumberFormat* formatter, const char * const testData[][2], bool testParsing)
{
