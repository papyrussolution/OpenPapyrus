...
void TimeZoneBoundaryTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void TimeZoneBoundaryTest::findDaylightBoundaryUsingTimeZone(UDate d, bool startsInDST, UDate expectedBoundary)
{
...
void TimeZoneBoundaryTest::findDaylightBoundaryUsingTimeZone(UDate d, bool startsInDST, UDate expectedBoundary, TimeZone* tz)
{
	...
	while((max - min) > INTERVAL) {
		UDate mid = (min + max) / 2;
		bool isIn = tz->inDaylightTime(mid, status);
		if(failure(status, "TimeZone::inDaylightTime")) return;
		if(isIn == startsInDST) {
			min = mid;
		}
...
void TimeZoneBoundaryTest::verifyDST(UDate d,
    TimeZone* time_zone,
    bool expUseDaylightTime,
    bool expInDaylightTime,
    UDate expZoneOffset,
    UDate expDSTOffset)
{
	UnicodeString str;
	...
	if(time_zone->getRawOffset() == expZoneOffset)
		...
		dataerrln(UnicodeString("FAIL: getRawOffset() = ") + (time_zone->getRawOffset() / ONE_HOUR) + ";  expected " +
		    (expZoneOffset / ONE_HOUR));
	...
	int32_t offset = time_zone->getOffset((uint8_t)gc->get(UCAL_ERA, status),
		...
		(uint8_t)gc->get(UCAL_DAY_OF_WEEK, status),
		((gc->get(UCAL_HOUR_OF_DAY, status) * 60 + gc->get(UCAL_MINUTE, status)) * 60 + gc->get(UCAL_SECOND,
		status)) * 1000 +
...
void TimeZoneBoundaryTest::testUsingBinarySearch(SimpleTimeZone* tz, UDate d, UDate expectedBoundary)
{
	...
	UDate min = d;
	UDate max = min + SIX_MONTHS;
	bool startsInDST = tz->inDaylightTime(d, status);
...
void TimeZoneBoundaryTest::TestNewRules()
{
#if 1
	{
		...
		tz = new SimpleTimeZone(-8 * (int32_t)ONE_HOUR,
			"Test_1",
			UCAL_AUGUST,
			2,
			UCAL_TUESDAY,
...
void TimeZoneBoundaryTest::findBoundariesStepwise(int32_t year, UDate interval, TimeZone* z, int32_t expectedChanges)
{
	...
	while(time < limit) {
		d = time;
		bool state = z->inDaylightTime(d, status);
		if(failure(status, "TimeZone::inDaylightTime")) return;
		if(state != lastState) {
