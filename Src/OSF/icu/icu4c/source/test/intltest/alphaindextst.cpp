...
void AlphabeticIndexTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void AlphabeticIndexTest::APITest() {
	...
	if(index->getCollator().getReorderCodes(reorderCodes, SIZEOFARRAYi(reorderCodes), status) > 0) {
...
void AlphabeticIndexTest::HackPinyinTest() {
	...
	UnicodeString names[SIZEOFARRAYi(pinyinTestData)];
	int32_t nameCount;
...
static const char * localeAndIndexCharactersLists[][2] = {
	...
	/* Chinese (Traditional Han)*/  {"zh_Hant",
					 "1\\u5283:2\\u5283:3\\u5283:4\\u5283:5\\u5283:6\\u5283:7\\u5283:8\\u5283:9\\u5283:10\\u5283:11\\u5283:12\\u5283:13\\u5283:14\\u5283:15\\u5283:16\\u5283:17\\u5283:18\\u5283:19\\u5283:20\\u5283:21\\u5283:22\\u5283:23\\u5283:24\\u5283:25\\u5283:26\\u5283:27\\u5283:28\\u5283:29\\u5283:30\\u5283:31\\u5283:32\\u5283:33\\u5283:35\\u5283:36\\u5283:39\\u5283:48\\u5283"},
};

void AlphabeticIndexTest::TestIndexCharactersList() {
	UErrorCode status = U_ZERO_ERROR;
	for(int32_t i = 0; i < SIZEOFARRAYi(localeAndIndexCharactersLists); ++i) {
...
void AlphabeticIndexTest::TestHaniFirst() {
	...
	coll->setReorderCodes(reorderCodes, SIZEOFARRAYi(reorderCodes), status);
	TEST_CHECK_STATUS;
...
void AlphabeticIndexTest::TestNoLabels() {
	...
	AlphabeticIndex index(coll.orphan(), status);
	TEST_CHECK_STATUS;
	index.addRecord(UnicodeString((char16_t)0x897f), NULL, status);
	index.addRecord("i", NULL, status);
	index.addRecord(UnicodeString((char16_t)0x03B1), NULL, status);
...
void AlphabeticIndexTest::TestChineseZhuyin() {
	UErrorCode status = U_ZERO_ERROR;
	char loc[100];
	uloc_forLanguageTag("zh-u-co-zhuyin", loc, SIZEOFARRAYi(loc), NULL, &status);
	AlphabeticIndex index(loc, status);
	LocalPointer<AlphabeticIndex::ImmutableIndex> immIndex(index.buildImmutableIndex(status));
	TEST_CHECK_STATUS;
...
void AlphabeticIndexTest::TestJapaneseKanji() {
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(kanji); ++i) {
		char msg[40];
...
void AlphabeticIndexTest::checkHasBuckets(const Locale &locale, UScriptCode script) {
	...
	assertTrue(loc + UnicodeString(u" at least 3 buckets"), index->getBucketCount() >= 3);
	const AlphabeticIndex::Bucket * bucket = index->getBucket(1);
	assertEquals(loc + UnicodeString(u" real bucket"), U_ALPHAINDEX_NORMAL, bucket->getLabelType());
	assertEquals(loc + UnicodeString(u" expected script"), script, uscript_getScript(bucket->getLabel().char32At(0), errorCode));
}
