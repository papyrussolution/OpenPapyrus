...
void IntlTestSpoof::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
...
void IntlTestSpoof::testMixedNumbers() {
	...
	for(int32_t testNum = 0; testNum < SIZEOFARRAYi(tests); testNum++) {
		...
		bool mixedNumberFailure = ((uspoof_getCheckResultChecks(checkResult, &status) & USPOOF_MIXED_NUMBERS) != 0);
