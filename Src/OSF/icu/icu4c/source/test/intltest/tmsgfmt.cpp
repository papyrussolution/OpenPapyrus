...
void TestMessageFormat::runIndexedTest(int32_t index, bool exec,
...
IntlTest& operator<<(IntlTest & stream, const Formattable&  obj)
{
...
void TestMessageFormat::testMsgFormatPlural(/* char * par */)
{
	...
	UnicodeString t4(
		"There {argument, plural, one{is # zavod}few{are {argument, number,###.0} zavoda} other{are #zavodov}} in the directory.");
...
void TestMessageFormat::testMsgFormatSelect(/* char * par */)
{
	...
	MessageFormat* msgFmt1 = internalCreate(t1.unescape(),
		Locale("fr"),
		err,
	...
	if(!U_FAILURE(err)) {
		...
		for(int i = 0; i< 3; i++) {
			internalFormat(msgFmt1,
			    ...
			    2,
			    exp[i].unescape(),
			    (char *)"From TestMessageFormat::testSelectFormat format t1");
		}
	}
	delete msgFmt1;
	...
	MessageFormat* msgFmt2 = internalCreate(t2.unescape(),
		Locale("fr"),
		err,
	...
	if(!U_FAILURE(err)) {
		...
		for(int i = 0; i< 3; i++) {
			internalFormat(msgFmt2,
			    ...
			    2,
			    exp[i].unescape(),
			    (char *)"From TestMessageFormat::testSelectFormat format t2");
		}
	}
	delete msgFmt2;
	...
	MessageFormat* msgFmt4 = internalCreate(t4.unescape(),
		Locale("fr"),
		err,
	...
	if(!U_FAILURE(err)) {
		...
		//Format
		for(int i = 0; i< 3; i++) {
			internalFormat(msgFmt4,
			    testArgs[i],
			    3,
			    exp[i].unescape(),
			    (char *)"From TestMessageFormat::testSelectFormat format t4");
		}
	}
	delete msgFmt4;
	...
	MessageFormat* msgFmt5 = internalCreate(t5.unescape(),
		Locale("fr"),
		err,
		(char *)"From TestMessageFormat::TestSelectFormat create t5");
	// with no data the above should fail but it seems to construct an invalid MessageFormat with no reported error.
	// See #13079
	if(!U_FAILURE(err)) {
		//Arguments
		...
		//Format
		for(int i = 0; i< 4; i++) {
			internalFormat(msgFmt5,
			    testArgs[i],
			    3,
			    exp[i].unescape(),
			    (char *)"From TestMessageFormat::testSelectFormat format t5");
		}
	}
	delete msgFmt5;
	...
	if(!U_FAILURE(err)) {
		...
		//Format
		for(int i = 0; i< 14; i++) {
			internalFormat(msgFmt6.getAlias(),
			    testArgs[i],
			    4,
			    exp[i],
	...
	for(int i = 0; i < formats_count; ++i) {
		...
		for(int j = 0; j < values_count; ++j) {
			...
			if(U_FAILURE(ec)) {
				...
				logln((UnicodeString)"value: " + toString(args[0]) + " --> " + result + UnicodeString(
					    " ec: ") + u_errorName(ec));
