...
void DataDrivenNumberFormatTestSuite::run(const char * fileName, bool runAllTests) {
	fFileLineNumber = 0;
	fFormatTestNumber = 0;
	...
	while(U_SUCCESS(status)) {
		...
		if(state == 0) {
			...
			if(runAllTests || !breaksC()) {
				...
				bool shouldFail = (NFTT_GET_FIELD(fTuple, output, "") == "fail")
				    ? !breaksC()
				    : breaksC();
				bool actualSuccess = isPass(fTuple, errorMessage, status);
...
bool DataDrivenNumberFormatTestSuite::isPass(const NumberFormatTestTuple &tuple,
...
bool DataDrivenNumberFormatTestSuite::isFormatPass(const NumberFormatTestTuple & /* tuple */,
    UnicodeString & /*appendErrorMessage*/,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
...
UObject * DataDrivenNumberFormatTestSuite::newFormatter(UErrorCode & /*status*/) {
	return NULL;
}

bool DataDrivenNumberFormatTestSuite::isToPatternPass(const NumberFormatTestTuple & /* tuple */,
    UnicodeString & /*appendErrorMessage*/,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
...
bool DataDrivenNumberFormatTestSuite::isParsePass(const NumberFormatTestTuple & /* tuple */,
    UnicodeString & /*appendErrorMessage*/,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
...
bool DataDrivenNumberFormatTestSuite::isParseCurrencyPass(const NumberFormatTestTuple & /* tuple */,
    UnicodeString & /*appendErrorMessage*/,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
...
bool DataDrivenNumberFormatTestSuite::isSelectPass(const NumberFormatTestTuple & /* tuple */,
    UnicodeString & /*appendErrorMessage*/,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
