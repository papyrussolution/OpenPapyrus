...
void UCharsTrieTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
	if(exec) {
...
void UCharsTrieTest::TestNextForCodePoint() {
	...
	LocalPointer<UCharsTrie> trie(buildTrie(data, SIZEOFARRAYi(data), USTRINGTRIE_BUILD_FAST));
	if(trie.isNull()) {
	...
	if((result = trie->reset().nextForCodePoint(0x4dff))!=USTRINGTRIE_NO_VALUE || result!=trie->current() ||
	    (result = trie->nextForCodePoint(0x10000))!=USTRINGTRIE_NO_VALUE || result!=trie->current() ||
	    (result = trie->nextForCodePoint(0x9999))!=USTRINGTRIE_NO_VALUE || result!=trie->current() ||
	    (result = trie->nextForCodePoint(0x20222))!=USTRINGTRIE_NO_MATCH || result!=trie->current() // no match for
	                                                                                                // trail
	                                                                                                // surrogate
	    ) {
...
public:
	...
	int32_t countUniqueFirstChars() const {
		return set.size();
	}
...
void UCharsTrieTest::checkIterator(UCharsTrie::Iterator &iter,
	...
	for(int32_t i = 0; i<dataLength; ++i) {
		...
		if(iter.getString()!=expectedString) {
			...
			invString.extract(0, invString.length(), buffer, SIZEOFARRAYi(buffer), US_INV);
