...
void StringSearchTest::runIndexedTest(int32_t index, bool exec,
    const char *& name, char *)
{
#if !UCONFIG_NO_BREAK_ITERATION
	bool areBroken = FALSE;
...
Collator::ECollationStrength StringSearchTest::getECollationStrength(const UCollationStrength &strength) const
{
	switch(strength)
	{
		case UCOL_PRIMARY:
...
bool StringSearchTest::assertEqualWithStringSearch(StringSearch * strsrch,
    const SearchData * search)
{
	...
	int32_t nextStart;
	bool isOverlap = (strsrch->getAttribute(USEARCH_OVERLAP) == USEARCH_ON);
...
bool StringSearchTest::assertEqual(const SearchData * search)
{
	UErrorCode status = U_ZERO_ERROR;
...
bool StringSearchTest::assertCanonicalEqual(const SearchData * search)
{
	UErrorCode status = U_ZERO_ERROR;
	Collator      * collator = getCollator(search->collator);
	BreakIterator * breaker  = getBreakIterator(search->breaker);
...
bool StringSearchTest::assertEqualWithAttribute(const SearchData * search,
...
void StringSearchTest::TestPattern()
{
	char16_t temp[512];
	int templength;
	u_unescape(PATTERN_[0].text, temp, 512);
	UnicodeString text;
	text.setTo(temp, u_strlen(temp));
	u_unescape(PATTERN_[0].pattern, temp, 512);
	UnicodeString pattern;
	pattern.setTo(temp, u_strlen(temp));

	m_en_us_->setStrength(getECollationStrength(PATTERN_[0].strength));
	UErrorCode status = U_ZERO_ERROR;
	StringSearch * strsrch = new StringSearch(pattern, text, m_en_us_, NULL, status);
	if(U_FAILURE(status)) {
		errln("Error opening string search %s", u_errorName(status));
	...
	if(!assertEqualWithStringSearch(strsrch, &PATTERN_[0])) {
		m_en_us_->setStrength(getECollationStrength(UCOL_TERTIARY));
		if(strsrch != NULL) {
			delete strsrch;
		}
	...
	u_unescape(PATTERN_[1].pattern, temp, 512);
	pattern.setTo(temp, u_strlen(temp));
	...
	if(!assertEqualWithStringSearch(strsrch, &PATTERN_[1])) {
		m_en_us_->setStrength(getECollationStrength(UCOL_TERTIARY));
		if(strsrch != NULL) {
			delete strsrch;
		}
...
void StringSearchTest::TestPatternCanonical()
{
	...
	UnicodeString pattern;
	pattern.setTo(temp, u_strlen(temp));
	m_en_us_->setStrength(getECollationStrength(PATTERNCANONICAL[0].strength));
	UErrorCode status = U_ZERO_ERROR;
	StringSearch * strsrch = new StringSearch(pattern, text, m_en_us_, NULL, status);
	strsrch->setAttribute(USEARCH_CANONICAL_MATCH, USEARCH_ON, status);
	if(U_FAILURE(status)) {
...
public:
	...
	virtual SearchIterator* safeClone(void) const override {
		return NULL;
	}
	virtual int32_t handleNext(int32_t, UErrorCode &) override {
		return 0;
	}
	virtual int32_t handlePrev(int32_t, UErrorCode &) override {
		return 0;
	}
