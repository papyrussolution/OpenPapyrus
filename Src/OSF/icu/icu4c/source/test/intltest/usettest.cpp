...
void UnicodeSetTest::runIndexedTest(int32_t index, bool exec,
...
void UnicodeSetTest::TestToPattern() {
	...
	// whitespace.
	{
		...
		for(int32_t j = 0; OTHER_TOPATTERN_TESTS[j]!=NULL; ++j) {
			...
			if(U_FAILURE(ec)) {
				dataerrln((UnicodeString)"FAIL: bad pattern " + OTHER_TOPATTERN_TESTS[j] + " - " +
...
bool UnicodeSetTest::toPatternAux(UChar32 start, UChar32 end) {
...
bool UnicodeSetTest::checkPat(const UnicodeString & source,
...
void UnicodeSetTest::TestPatterns() {
	UnicodeSet set;
	...
	set.complement();
	UnicodeString exp;
	exp.append((char16_t)0x0000).append("aeeoouu").append((char16_t)(u'z'+1)).append(u'\uFFFF');
	expectPairs(set, exp);
}

void UnicodeSetTest::TestCategories() {
	...
	for(i = 0; i<0x200; ++i) {
		bool lu = (u_charType((char16_t)i) == U_UPPERCASE_LETTER);
...
void UnicodeSetTest::TestCloneEqualHash() {
	UErrorCode status = U_ZERO_ERROR;
...
void UnicodeSetTest::TestAddRemove() {
...
void UnicodeSetTest::TestAPI() {
	...
	if(set != exp) {
		errln("FAIL: (with cd).retain(\"cd\")"); return;
	}
	...
	exp.clear();
	if(set != exp) {
		errln("FAIL: (without cd).retain(\"cd\")"); return;
	}
...
void UnicodeSetTest::TestPropertySet() {
	static const char * const DATA[] = {
		...
		"[[:ccc=0:]-[:lccc=0:]-[:tccc=0:]]", // Weirdos. Complete canonical class is zero, but both lead and
		                                     // trail are not
		"\\u0F73\\u0F75\\u0F81",
		"abcd\\u0300\\u0301\\u00c0\\u00c5",
...
void UnicodeSetTest::copyWithIterator(UnicodeSet & t, const UnicodeSet & s, bool withRange) {
	t.clear();
...
bool UnicodeSetTest::checkEqual(const UnicodeSet & s, const UnicodeSet & t, const char * message) {
...
void UnicodeSetTest::expectContainment(const UnicodeString & pat,
...
void UnicodeSetTest::expectContainment(const UnicodeSet & set,
    const UnicodeString & charsIn,
    const UnicodeString & charsOut) {
	UnicodeString pat;
	set.toPattern(pat);
...
void UnicodeSetTest::expectPattern(UnicodeSet & set,
	...
	if(*tempset != set || getPairs(*tempset) != getPairs(set)) {
		errln(UnicodeString("FAIL: applyPattern(\""+ pattern + "\").toPattern() => " + temppattern + " => pairs \""+
		    escape(getPairs(*tempset)) + "\", expected pairs \"" +
		    escape(getPairs(set)) + "\""));
	}
	else {
		logln(UnicodeString("Ok:   applyPattern(\""+ pattern + "\").toPattern() => " + temppattern + " => pairs \"" +
		    escape(getPairs(*tempset)) + "\""));
	}
...
void UnicodeSetTest::doAssert(bool condition, const char * message)
{
	if(!condition) {
...
static int32_t appendUTF8(const char16_t * s, int32_t length, char * t, int32_t capacity) {
	UErrorCode errorCode = U_ZERO_ERROR;
	int32_t length8 = 0;
...
public:
	...
	const UnicodeSet &getSet() const { return set; }
	bool hasStrings() const { return (bool)(stringsLength>0); }
	bool hasStringsWithSurrogates() const { return hasSurrogates; }
private:
...
static inline bool matches16CPB(const char16_t * s, int32_t start, int32_t limit, const UnicodeString & t) {
	s += start;
	limit -= start;
...
static int32_t containsSpanUTF16(const UnicodeSetWithStrings &set, const char16_t * s, int32_t length, USetSpanCondition spanCondition) 
{
	...
	if(!set.hasStrings()) {
		...
		while((prev = start)<length) {
			...
			if(static_cast<USetSpanCondition>(realSet.contains(c)) != spanCondition) {
				break;
			}
		}
		return prev;
...
static int32_t containsSpanBackUTF16(const UnicodeSetWithStrings &set, const char16_t * s, int32_t length, USetSpanCondition spanCondition) 
{
	if(length==0) {
		return 0;
	}
...
static inline int32_t slen(const void * s, bool isUTF16) {
 ...
 */
static int32_t getSpans(const UnicodeSetWithStrings &set, bool isComplement,
    const void * s, int32_t length, bool isUTF16,
    uint32_t whichSpans,
...
void UnicodeSetTest::testSpan(const UnicodeSetWithStrings * sets[4],
	...
	for(i = 0; i<SET_COUNT; ++i) {
		...
		for(type = 0;; ++type) {
			limitsCount = getSpans(*sets[i], (bool)(i&1),
				s, length, isUTF16,
				whichSpans,
				type, typeName,
...
bool stringContainsUnpairedSurrogate(const char16_t * s, int32_t length) {
	char16_t c, c2;
...
static int32_t addAlternative(uint32_t whichSpans[], int32_t whichSpansCount,
