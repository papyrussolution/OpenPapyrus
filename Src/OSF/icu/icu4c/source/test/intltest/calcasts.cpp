...
bool CalendarCaseTest::checkField(Calendar * cal, UCalendarDateFields field, int32_t value, UErrorCode & status)
{
	...
	if(res != value) {
		errln((UnicodeString)"FAIL: Checking field " + fieldName(field) + " expected " + value + " and got " + res +
		    UnicodeString("\n"));
		return FALSE;
	}
	else {
...
void CalendarCaseTest::IslamicCivil()
{
	...
	if(result != expectedUChars) {
		errln((UnicodeString)"FAIL: DateFormatting failed. Got " + result + " and expected " + UnicodeString(
			    expectedUChars) + UnicodeString("\n"));
...
void CalendarCaseTest::Ethiopic() {
	static TestCase tests[] = {
		...
		{1725315.5,  1,     3,   13,   6,  SAT,    0,  0,  0},// Gregorian: 27/08/0011 - first ethiopic leap
		                                               // year
