...
#define mkcstr(U) u_austrcpy(SAlloc::C(8, u_strlen(U) + 1), U)
...
UnicodeString CalendarTest::calToStr(const Calendar & cal)
{
	...
	for(i = 0; i<UCAL_FIELD_COUNT; i++) {
		out +=
		    (UnicodeString("") + fieldName((UCalendarDateFields)i) + "=" +
		    cal.get((UCalendarDateFields)i, status) + UnicodeString(" "));
	}
...
void CalendarTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
 ...
 */
void CalendarTest::TestGenericAPI()
{
	UErrorCode status = U_ZERO_ERROR;
	UDate d;
	UnicodeString str;
	bool eq = FALSE, b4 = FALSE, af = FALSE;
	...
	cal->roll(UCAL_SECOND, (bool)TRUE, status);
	logln(UnicodeString("cal=")  +cal->getTime(status)  + UnicodeString(calToStr(*cal)));
	...
	for(i = 0; i<UCAL_FIELD_COUNT; ++i) {
		switch(i) {
			...
			case UCAL_EXTENDED_YEAR:
			    if(!cal->isSet((UCalendarDateFields)i)) errln("FAIL: Calendar::isSet F, should be T " +
					fieldName((UCalendarDateFields)i));
			    break;
			default:
			    if(cal->isSet((UCalendarDateFields)i)) errln("FAIL: Calendar::isSet = T, should be F  " +
					fieldName((UCalendarDateFields)i));
		}
	...
	if(count < 1 || loc == 0) {
		...
		for(i = 0; i<count; ++i) {
			...
			if(U_FAILURE(status)) {
				errcheckln(status,
				    UnicodeString(
					    "FAIL: Calendar::createInstance #3, locale ") +  loc[i].getName() + " , error " +
				    u_errorName(status));
				return;
			}
			delete cal;
	...
	if(gc->getTime(status) != (d = date(98, 10, 14, 21, 43) )|| U_FAILURE(status))
		errln("FAIL: new GregorianCalendar(ymdhm) failed with " + UnicodeString(u_errorName(status)) + ",  cal="  +
		    gc->getTime(status)  + UnicodeString(calToStr(*gc)) + ", d=" + d);
	else
 ...
 */
void CalendarTest::TestDOW943()
{
	dowTest(FALSE);
	dowTest(TRUE);
}

void CalendarTest::dowTest(bool lenient)
{
	UErrorCode status = U_ZERO_ERROR;
	...
	if(U_FAILURE(status)) {
		errln("Calendar::get failed [%s]", u_errorName(status)); return;
	}
 ...
 */
void CalendarTest::TestClonesUnique908()
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarTest::TestGregorianChange768()
{
	bool b;
	UErrorCode status = U_ZERO_ERROR;
	UnicodeString str;
 ...
 */
void CalendarTest::TestDisambiguation765()
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarTest::verify765(const UnicodeString & msg, Calendar* c, int32_t year, int32_t month, int32_t day)
{
	UnicodeString str;
	UErrorCode status = U_ZERO_ERROR;
	...
	if(y == year &&
		...
		errln("FAIL: " + msg + dateToString(c->getTime(
			    status), str) + "; expected " + (int32_t)year + "/" + (int32_t)(month + 1) + "/" + (int32_t)day +
		    "; got " + (int32_t)y + "/" + (int32_t)(m + 1) + "/" + (int32_t)d + " for Locale: " +
		    c->getLocaleID(ULOC_ACTUAL_LOCALE, status));
		if(U_FAILURE(status)) {
			errln("Calendar::getTime failed"); return;
		}
	}
...
void CalendarTest::verify765(const UnicodeString & msg /*, IllegalArgumentException e*/, UErrorCode status)
{
 ...
 */
void CalendarTest::TestGMTvsLocal4064654()
{
...
void CalendarTest::test4064654(int32_t yr, int32_t mo, int32_t dt, int32_t hr, int32_t mn, int32_t sc)
{
	UDate date;
	UErrorCode status = U_ZERO_ERROR;
	UnicodeString str;
 ...
 */
void CalendarTest::TestAddSetOrder621()
{
 ...
 */
void CalendarTest::TestAddRollExtensive()
{
	int32_t maxlimit = 40;
	...
	while(e < UCAL_FIELD_COUNT) {
		...
		for(i = 0; i < limit; i++) {
			...
			temp->roll(e, -1, status);
			if(U_FAILURE(status)) {
				errln(UnicodeString("GregorianCalendar::roll ") + CalendarTest::fieldName(e) + " count=" +
				    UnicodeString('@'+i) + " by -1 failed with " + u_errorName(status)); return;
			}
		}
		check520(temp, y, m, d, hr, min, sec, ms, e);
...
// -------------------------------------
void CalendarTest::check520(Calendar* c,
 ...
 */
void CalendarTest::TestFieldSet4781()
{
	// try {
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarTest::TestSecondsZero121()
{
	UErrorCode status = U_ZERO_ERROR;
...
UnicodeString CalendarTest::value(Calendar* calendar)
{
	UErrorCode status = U_ZERO_ERROR;
 ...
 */
void CalendarTest::TestFields060()
{
	UErrorCode status = U_ZERO_ERROR;
	int32_t year = 1997;
 ...
 */
void CalendarTest::TestEpochStartFields()
{
	UErrorCode status = U_ZERO_ERROR;
	...
	bool idt = gc->inDaylightTime(status);
	if(U_FAILURE(status)) {
 ...
 */
void CalendarTest::TestDOWProgression()
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarTest::TestDOW_LOCALandYEAR_WOY()
{
	...
	if(U_FAILURE(status)) {
		dataerrln("Couldn't create SimpleDateFormat - %s", u_errorName(status)); return;
	}
...
void CalendarTest::loop_addroll(Calendar * cal,
	...
	for(i = 0; i<times; i++) {
		...
		if(U_FAILURE(errorCode)) {
			errln("Error in add"); delete calclone; return;
		}
	...
	for(i = 0; i<times; i++) {
		...
		if(U_FAILURE(errorCode)) {
			errln("Error in roll"); delete calclone; return;
		}
...
void CalendarTest::doYEAR_WOYLoop(Calendar * cal, SimpleDateFormat * sdf,
...
void CalendarTest::marchByDelta(Calendar* cal, int32_t delta)
{
	UErrorCode status = U_ZERO_ERROR;
...
void CalendarTest::TestDebug()
{
	for(int32_t t = 0; t<=UDBG_ENUM_COUNT; t++) {
		...
		for(int32_t i = 0; i<=count; i++) {
			if(t<=UDBG_HIGHEST_CONTIGUOUS_ENUM && i<count) {
				if(i!=udbg_enumArrayValue((UDebugEnumType)t, i)) {
					errln("FAIL: udbg_enumArrayValue(%d,%d) returned %d, expected %d", t, i,
					    udbg_enumArrayValue((UDebugEnumType)t, i), i);
				}
			}
			else {
		...
		if(udbg_enumExpectedCount((UDebugEnumType)t) != count && t<=UDBG_HIGHEST_CONTIGUOUS_ENUM) {
			errln("FAIL: udbg_enumExpectedCount(%d): %d, != UCAL_FIELD_COUNT=%d ",
			    t,
			    udbg_enumExpectedCount((UDebugEnumType)t),
			    count);
		}
		else {
			logln("udbg_ucal_fieldCount: %d, UCAL_FIELD_COUNT=udbg_enumCount %d ",
			    udbg_enumExpectedCount((UDebugEnumType)t),
			    count);
		}
	}
...
static UDate doMinDateOfCalendar(Calendar* adopt, bool &isGregorian, UErrorCode & status) {
...
UDate CalendarTest::minDateOfCalendar(const Locale & locale, bool &isGregorian, UErrorCode & status) {
	if(U_FAILURE(status)) return 0.0;
	return doMinDateOfCalendar(Calendar::createInstance(locale, status), isGregorian, status);
}

UDate CalendarTest::minDateOfCalendar(const Calendar& cal, bool &isGregorian, UErrorCode & status) {
	if(U_FAILURE(status)) return 0.0;
	...
	for(int i = 0; TEST_LOCALES[i] != NULL; i++) {
		...
		for(int j = 0; TEST_DATA[j][0] != 0; j++) {
			...
			if(weekNum != TEST_DATA[j][1] || weekYear != TEST_DATA[j][2]) {
				errln(
					"Error: Incorrect week of year on January 1st, %d for locale %s: Returned [weekNum=%d, weekYear=%d], Expected [weekNum=%d, weekYear=%d]",
					TEST_DATA[j][0],
					TEST_LOCALES[i],
					weekNum,
					weekYear,
...
void CalendarTest::TestAmbiguousWallTimeAPIs() {
	UErrorCode status = U_ZERO_ERROR;
	Calendar* cal = Calendar::createInstance(status);
	if(U_FAILURE(status)) {
...
public:
	...
	bool operator != (const CalFields& rhs) const;
	bool isEquivalentTo(const Calendar& cal, UErrorCode & status) const;
...
void CalFields::setTo(Calendar& cal) const {
	cal.clear();
	cal.set(year, month - 1, day, hour, min, sec);
	cal.set(UCAL_MILLISECOND, ms);
}

char * CalFields::toString(char * buf, int32_t len) const {
	char local[32];
...
static const RepeatedWallTimeTestData RPDATA[] =
{
	...
	{"America/New_York",    CalFields(2011, 11, 6, 0, 59, 59),   CalFields(2011, 11, 6, 4, 59, 59),
	 CalFields(2011, 11, 6, 4, 59, 59)},
	...
	{"America/New_York",    CalFields(2011, 11, 6, 1, 59, 59),   CalFields(2011, 11, 6, 6, 59, 59),
	 CalFields(2011, 11, 6, 5, 59, 59)},
	...
	{"Australia/Lord_Howe", CalFields(2011, 4, 3, 1, 29, 59),    CalFields(2011, 4, 2, 14, 29, 59),
	 CalFields(2011, 4, 2, 14, 29, 59)},
	...
	{"Australia/Lord_Howe", CalFields(2011, 4, 3, 1, 59, 59),    CalFields(2011, 4, 2, 15, 29, 59),
	 CalFields(2011, 4, 2, 14, 59, 59)},
	...
	for(int32_t i = 0; RPDATA[i].tzid != NULL; i++) {
		...
		if(U_FAILURE(status)) {
			...
			if(outLastGMT != RPDATA[i].expLastGMT) {
				dataerrln(UnicodeString("Fail: UCAL_WALLTIME_LAST ") + RPDATA[i].in.toString(buf,
				    sizeof(buf)) + "[" + RPDATA[i].tzid + "] is parsed as "
				    + outLastGMT.toString(buf,
				    sizeof(buf)) + "[GMT]. Expected: " + RPDATA[i].expLastGMT.toString(buf, sizeof(buf)) + "[GMT]");
			}
		}
		...
		if(U_FAILURE(status)) {
			...
			if(outDefGMT != RPDATA[i].expLastGMT) {
				dataerrln(UnicodeString("Fail: (default) ") + RPDATA[i].in.toString(buf,
				    sizeof(buf)) + "[" + RPDATA[i].tzid + "] is parsed as "
				    + outDefGMT.toString(buf,
				    sizeof(buf)) + "[GMT]. Expected: " + RPDATA[i].expLastGMT.toString(buf, sizeof(buf)) + "[GMT]");
			}
		}
		...
		if(U_FAILURE(status)) {
			...
			if(outFirstGMT != RPDATA[i].expFirstGMT) {
				dataerrln(UnicodeString("Fail: UCAL_WALLTIME_FIRST ") + RPDATA[i].in.toString(buf,
				    sizeof(buf)) + "[" + RPDATA[i].tzid + "] is parsed as "
				    + outFirstGMT.toString(buf,
				    sizeof(buf)) + "[GMT]. Expected: " + RPDATA[i].expFirstGMT.toString(buf, sizeof(buf)) + "[GMT]");
			}
		}
		delete tz;
	}
...
{
	// Time zone           Input wall time                 valid?  WALLTIME_LAST in GMT            WALLTIME_FIRST in
	// GMT           WALLTIME_NEXT_VALID in GMT
	{"America/New_York",    CalFields(2011, 3, 13, 1, 59, 59),   TRUE,   CalFields(2011, 3, 13, 6, 59, 59),   CalFields(2011,
	     3,
	     13,
	     6,
	     59,
	     59),   CalFields(2011, 3, 13, 6, 59, 59)},
	{"America/New_York",    CalFields(2011, 3, 13, 2, 0, 0),     FALSE,  CalFields(2011, 3, 13, 7, 0, 0),     CalFields(2011,
	     3,
	     13,
	     6,
	     0,
	     0),     CalFields(2011, 3, 13, 7, 0, 0)},
	{"America/New_York",    CalFields(2011, 3, 13, 2, 1, 0),     FALSE,  CalFields(2011, 3, 13, 7, 1, 0),     CalFields(2011,
	     3,
	     13,
	     6,
	     1,
	     0),     CalFields(2011, 3, 13, 7, 0, 0)},
	{"America/New_York",    CalFields(2011, 3, 13, 2, 30, 0),    FALSE,  CalFields(2011, 3, 13, 7, 30, 0),    CalFields(2011,
	     3,
	     13,
	     6,
	     30,
	     0),    CalFields(2011, 3, 13, 7, 0, 0)},
	{"America/New_York",    CalFields(2011, 3, 13, 2, 59, 59),   FALSE,  CalFields(2011, 3, 13, 7, 59, 59),   CalFields(2011,
	     3,
	     13,
	     6,
	     59,
	     59),   CalFields(2011, 3, 13, 7, 0, 0)},
	{"America/New_York",    CalFields(2011, 3, 13, 3, 0, 0),     TRUE,   CalFields(2011, 3, 13, 7, 0, 0),     CalFields(2011,
	     3,
	     13,
	     7,
	     0,
	...
	{"Pacific/Apia",        CalFields(2011, 12, 29, 23, 59, 59), TRUE,   CalFields(2011, 12, 30, 9, 59, 59),  CalFields(2011,
	     12,
	     30,
	     9,
	     59,
	     59),  CalFields(2011, 12, 30, 9, 59, 59)},
	{"Pacific/Apia",        CalFields(2011, 12, 30, 0, 0, 0),    FALSE,  CalFields(2011, 12, 30, 10, 0, 0),   CalFields(2011,
	     12,
	     29,
	     10,
	     0,
	     0),   CalFields(2011, 12, 30, 10, 0, 0)},
	{"Pacific/Apia",        CalFields(2011, 12, 30, 12, 0, 0),   FALSE,  CalFields(2011, 12, 30, 22, 0, 0),   CalFields(2011,
	     12,
	     29,
	     22,
	     0,
	     0),   CalFields(2011, 12, 30, 10, 0, 0)},
	{"Pacific/Apia",        CalFields(2011, 12, 30, 23, 59, 59), FALSE,  CalFields(2011, 12, 31, 9, 59, 59),  CalFields(2011,
	     12,
	     30,
	     9,
	     59,
	     59),  CalFields(2011, 12, 30, 10, 0, 0)},
	{"Pacific/Apia",        CalFields(2011, 12, 31, 0, 0, 0),    TRUE,   CalFields(2011, 12, 30, 10, 0, 0),   CalFields(2011,
	     12,
	     30,
	     10,
	     0,
	...
	{NULL,                  CalFields(0, 0, 0, 0, 0, 0),         TRUE,   CalFields(0, 0, 0, 0, 0, 0),
	 CalFields(0, 0, 0, 0, 0, 0),         CalFields(0, 0, 0, 0, 0, 0)}
};
	...
	for(int32_t i = 0; SKDATA[i].tzid != NULL; i++) {
		...
		for(int32_t j = 0; j < 2; j++) {
			...
			if(bLenient || SKDATA[i].isValid) {
				...
				if(U_FAILURE(status)) {
					...
					if(outLastGMT != SKDATA[i].expLastGMT) {
						dataerrln(UnicodeString("Fail: UCAL_WALLTIME_LAST ") +
						    SKDATA[i].in.toString(buf, sizeof(buf)) + "[" + SKDATA[i].tzid + "] is parsed as "
						    + outLastGMT.toString(buf,
						    sizeof(buf)) + "[GMT]. Expected: " +
						    SKDATA[i].expLastGMT.toString(buf, sizeof(buf)) + "[GMT]");
					}
				}
			}
			...
			if(bLenient || SKDATA[i].isValid) {
				...
				if(U_FAILURE(status)) {
					...
					if(outDefGMT != SKDATA[i].expLastGMT) {
						dataerrln(UnicodeString("Fail: (default) ") + SKDATA[i].in.toString(buf,
						    sizeof(buf)) + "[" +
						    SKDATA[i].tzid + "] is parsed as "
						    + outDefGMT.toString(buf,
						    sizeof(buf)) + "[GMT]. Expected: " +
						    SKDATA[i].expLastGMT.toString(buf, sizeof(buf)) + "[GMT]");
					}
			...
			if(bLenient || SKDATA[i].isValid) {
				...
				if(U_FAILURE(status)) {
					...
					if(outFirstGMT != SKDATA[i].expFirstGMT) {
						dataerrln(UnicodeString("Fail: UCAL_WALLTIME_FIRST ") +
						    SKDATA[i].in.toString(buf, sizeof(buf)) + "[" + SKDATA[i].tzid + "] is parsed as "
						    + outFirstGMT.toString(buf,
						    sizeof(buf)) + "[GMT]. Expected: " +
						    SKDATA[i].expFirstGMT.toString(buf, sizeof(buf)) + "[GMT]");
					}
				}
			}
			else if(U_SUCCESS(status)) {
			...
			if(bLenient || SKDATA[i].isValid) {
				...
				if(U_FAILURE(status)) {
					errln(UnicodeString(
						    "Fail: Failed to get/set time calNextAvail/calGMT (UCAL_WALLTIME_NEXT_VALID) - ")
					    + SKDATA[i].in.toString(buf, sizeof(buf)) + "[" + SKDATA[i].tzid + "]");
				}
				else {
					if(outNextAvailGMT != SKDATA[i].expNextAvailGMT) {
						dataerrln(UnicodeString("Fail: UCAL_WALLTIME_NEXT_VALID ") +
						    SKDATA[i].in.toString(buf, sizeof(buf)) + "[" + SKDATA[i].tzid + "] is parsed as "
						    + outNextAvailGMT.toString(buf,
						    sizeof(buf)) + "[GMT]. Expected: " +
						    SKDATA[i].expNextAvailGMT.toString(buf, sizeof(buf)) + "[GMT]");
					}
				}
			}
			else if(U_SUCCESS(status)) {
...
void CalendarTest::TestIslamicUmAlQura() {
	...
	for(int x = 1; x<=loopCnt; x++) {
		...
		TEST_CHECK_STATUS;
		tstCal->roll(UCAL_DAY_OF_MONTH, (bool)TRUE, status);
		TEST_CHECK_STATUS;
	}
	...
	if(is_day != 24 || is_month != IslamicCalendar::RABI_2 || is_year != 1395)
		errln("unexpected conversion date month %i not %i or day %i not 24 or year %i not 1395",
		    is_month,
...
void CalendarTest::TestIslamicTabularDates() {
	...
	if(tbla_month != is_month || tbla_year != is_year)
		errln("unexpected difference between islamic and tbla month %d : %d and/or year %d : %d",
		    tbla_month,
		    is_month,
		    tbla_year,
		    is_year);
