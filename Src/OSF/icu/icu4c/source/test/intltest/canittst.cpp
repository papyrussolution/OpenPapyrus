...
void CanonicalIteratorTest::runIndexedTest(int32_t index, bool exec,
...
void CanonicalIteratorTest::TestBasic() {
	...
	if(U_SUCCESS(status)) {
		for(i = 0; i < UPRV_LENGTHOF(testArray); ++i) {
			...
			expectEqual(UnicodeString(i) + UnicodeString(": "), testStr, collectionToString(set),
...
void CanonicalIteratorTest::expectEqual(const UnicodeString & message,
...
U_CFUNC int U_CALLCONV compareUnicodeStrings(const void * s1, const void * s2) {
