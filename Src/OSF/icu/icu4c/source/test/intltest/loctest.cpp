...
static const char * const rawData[33][8] = {
	...
	{   "English (United States)", "French (France)", "Catalan (Spain)", "Greek (Greece)", "Norwegian (Norway, NY)", "Italian",
	    "xx (YY)", "Chinese (Simplified, China)" },
	...
	{   "",     "",     "",     "",     "NY",     "",     "",   "" },
	// display name (French)
	//{   "anglais (Etats-Unis)", "francais (France)", "catalan (Espagne)", "grec (Grece)", "norvegien
	// (Norvege,Nynorsk)", "italien", "xx (YY)" },
	{   "anglais (\\u00C9tats-Unis)", "fran\\u00E7ais (France)", "catalan (Espagne)", "grec (Gr\\u00E8ce)",
	    "norv\\u00E9gien (Norv\\u00E8ge, NY)", "italien", "xx (YY)", "chinois (simplifi\\u00E9, Chine)" },
	...
	{   "angl\\u00E8s (Estats Units)", "franc\\u00E8s (Fran\\u00E7a)", "catal\\u00E0 (Espanya)", "grec (Gr\\u00E8cia)",
	    "noruec (Noruega, NY)", "itali\\u00E0", "", "xin\\u00E8s (simplificat, Xina)" },
	...
	{   "",     "",     "",     "",     "Nynorsk",   "",     "", ""},
	// display name (<root>)
	//{   "English (United States)", "French (France)", "Catalan (Spain)", "Greek (Greece)", "Norwegian
	// (Norway,Nynorsk)", "Italian", "xx (YY)" },
	{   "English (United States)", "French (France)", "Catalan (Spain)", "Greek (Greece)", "Norwegian (Norway,NY)", "Italian",
	    "xx (YY)", "" }
};
...
void LocaleTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	TESTCASE_AUTO_BEGIN;
 ...
 */
void LocaleTest::TestDisplayNames()
{
...
void LocaleTest::TestSimpleObjectStuff() {
	...
	Locale test8 = Locale::createFromName("aa-aa-jupiTER"); // was "aa-aa.utf8@jupiter" but in 3.0 getName won't
	                                                        // normalize that
...
void LocaleTest::doTestDisplayNames(Locale & displayLocale, int32_t compareIndex) {
	...
	for(int32_t i = 0; i <= MAX_LOCALES; i++) {
		...
		if(testLang != expectedLang)
			dataerrln("Display language (" + UnicodeString(displayLocale.getName()) + ") of (" +
			    UnicodeString(testLocale.getName()) + ") got " + testLang + " expected " + expectedLang);
		if(testScript != expectedScript)
			dataerrln("Display script (" + UnicodeString(displayLocale.getName()) + ") of (" +
			    UnicodeString(testLocale.getName()) + ") got " + testScript + " expected " + expectedScript);
		if(testCtry != expectedCtry)
			dataerrln("Display country (" + UnicodeString(displayLocale.getName()) + ") of (" +
			    UnicodeString(testLocale.getName()) + ") got " + testCtry + " expected " + expectedCtry);
		if(testVar != expectedVar)
			dataerrln("Display variant (" + UnicodeString(displayLocale.getName()) + ") of (" +
			    UnicodeString(testLocale.getName()) + ") got " + testVar + " expected " + expectedVar);
		if(testName != expectedName)
			dataerrln("Display name (" + UnicodeString(displayLocale.getName()) + ") of (" + UnicodeString(
				    testLocale.getName()) + ") got " + testName + " expected " + expectedName);
	}
}
 ...
 * @bug 4011756 4011380
 */
void LocaleTest::TestISO3Fallback()
{
	Locale test("xx", "YY");
 ...
 * @bug 4106155 4118587
 */
void LocaleTest::TestGetLangsAndCountries()
{
 ...
 * @bug 4118587
 */
void LocaleTest::TestSimpleDisplayNames()
{
 ...
 * @bug 4118595
 */
void LocaleTest::TestUninstalledISO3Names()
{
 ...
 */
void LocaleTest::TestAtypicalLocales()
{
 ...
 */
void LocaleTest::TestThaiCurrencyFormat()
{
	UErrorCode status = U_ZERO_ERROR;
...
void LocaleTest::Test4139940()
{
	...
	if(U_FAILURE(status)) {
		dataerrln(UnicodeString("Could not create SimpleDateFormat object for locale hu. Error: ") +
...
UDate LocaleTest::date(int32_t y, int32_t m, int32_t d, int32_t hr, int32_t min, int32_t sec)
{
	UErrorCode status = U_ZERO_ERROR;
...
void LocaleTest::TestVariantParsing()
{
...
// Jitterbug 1735
void LocaleTest::TestSetIsBogus() {
	Locale l("en_US");
	l.setToBogus();
	if(l.isBogus() != TRUE) {
...
void LocaleTest::TestAddLikelySubtags() {
...
void LocaleTest::TestMinimizeSubtags() {
...
void LocaleTest::TestKeywordVariants() {
	static const struct {
		const char * localeID;
...
void LocaleTest::TestCreateUnicodeKeywords() {
...
void LocaleTest::TestKeywordVariantParsing() {
	static const struct {
		const char * localeID;
		const char * keyword;
...
void LocaleTest::TestCreateKeywordSet() {
...
void LocaleTest::TestCreateKeywordSetEmpty() {
...
void LocaleTest::TestCreateKeywordSetWithPrivateUse() {
...
void LocaleTest::TestCreateUnicodeKeywordSet() {
...
void LocaleTest::TestCreateUnicodeKeywordSetEmpty() {
...
void LocaleTest::TestCreateUnicodeKeywordSetWithPrivateUse() {
...
void LocaleTest::TestGetKeywordValueStdString() {
...
void LocaleTest::TestGetUnicodeKeywordValueStdString() {
...
void LocaleTest::TestSetKeywordValue() {
	static const struct {
		const char * keyword;
		const char * value;
	} testCases[] = {
...
void LocaleTest::TestSetKeywordValueStringPiece() {
...
void LocaleTest::TestSetUnicodeKeywordValueStringPiece() {
...
void LocaleTest::TestGetBaseName() {
	static const struct {
		const char * localeID;
		const char * baseName;
	} testCases[] = {
 ...
 * "eq" equal,                 e.g., en == en
 */
void LocaleTest::_checklocs(const char * label, const char * req, const Locale & validLoc, const Locale & actualLoc, const char * expReqValid, const char * expValidActual) 
{
	...
	if(((0 == uprv_strcmp(expReqValid, "gt") && reqValid > 0) ||
		...
		dataerrln("FAIL: %s; req=%s, valid=%s, actual=%s.  Require (R %s V) and (V %s A)", label, req, valid, actual, expReqValid, expValidActual);
	}
}
...
void LocaleTest::checkRegisteredCollators(const char * expectExtra) {
	...
	for(int32_t i = 0; i<count1; i++) {
		...
		if(expectExtra != NULL && !strcmp(expectExtra, oldList[i].getName())) {
			errln(
				"Inexplicably, Collator::getAvailableCollators(count) had registered collator %s. This shouldn't happen, so I am going to consider it an error.\n",
				expectExtra);
		}
	}
	...
	while((locStr = newEnum->snext(status)) && U_SUCCESS(status)) {
		...
		if(oldHash.geti(*locStr) == 0) {
			if(expectExtra != NULL && expectStr==*locStr) {
				logln(UnicodeString(
					    "As expected, Collator::getAvailableLocales(count) is missing registered collator ") +
				    expectStr);
			}
			else {
...
void LocaleTest::TestVariantWithOutCountry() {
	...
	if(0 != strcmp(loc.getVariant(), "POSIX")) {
		errln("FAIL: en__POSIX didn't get parsed correctly - name is %s - expected %s got %s",
		    loc.getName(),
		    "POSIX",
		    loc.getVariant());
	}
	...
	if(0 != strcmp(loc2.getVariant(), "FOUR")) {
		errln("FAIL: en__FOUR didn't get parsed correctly - name is %s - expected %s got %s",
		    loc2.getName(),
		    "FOUR",
		    loc2.getVariant());
	}
	...
	if(0 != strcmp(loc3.getVariant(), "FOUR")) {
		errln("FAIL: en_Latn__FOUR didn't get parsed correctly - name is %s - expected %s got %s",
		    loc3.getName(),
		    "FOUR",
		    loc3.getVariant());
	}
	...
	if(0 != strcmp(loc4.getVariant(), "FOUR")) {
		errln("FAIL: _Latn__FOUR didn't get parsed correctly - name is %s - expected %s got %s",
		    loc4.getName(),
		    "FOUR",
		    loc4.getVariant());
	}
	...
	if(0 != strcmp(loc5.getVariant(), "FOUR")) {
		errln("FAIL: _Latn_US_FOUR didn't get parsed correctly - name is %s - expected %s got %s",
		    loc5.getName(),
		    "FOUR",
		    loc5.getVariant());
	}
	...
	if(0 != strcmp(loc6.getVariant(), "1901")) {
		errln("FAIL: de-1901 didn't get parsed correctly - name is %s - expected %s got %s",
		    loc6.getName(),
		    "1901",
		    loc6.getVariant());
	}
...
void LocaleTest::TestCanonicalization(void)
{
	static const struct {
		...
		{ "no-no-ny.utf8@B", "no_NO_NY.utf8@B", "no_NO@b=ny" /* not: "nn_NO" [alan ICU3.0] */ }, /* @ ignored
		                                                                                            unless
		                                                                                            variant is
		                                                                                            empty */
...
void LocaleTest::TestCapturingTagConvertingIterator() {
	...
	Locale::ConvertingIterator<const char **, std::function<const Locale & (const char *)> > iter(
		ARRAY_RANGE(tags), [&](const char * tag) -> const Locale & {
		return locale = Locale(tag);
		});
