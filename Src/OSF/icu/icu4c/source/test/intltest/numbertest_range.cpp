...
void NumberRangeFormatterTest::runIndexedTest(int32_t index, bool exec, const char *& name, char *) {
	if(exec) {
...
void NumberRangeFormatterTest::test21683_StateLeak() {
	...
	for(auto range : ranges) {
		...
		while(true) {
			...
			if(status.errIfFailureAndReset("ufmtval_nextPosition")) {
				goto cleanup;
			}
			if(!hasMore) {
				break;
...
void NumberRangeFormatterTest::assertFormatRange(const char16_t* message,
...
FormattedNumberRange NumberRangeFormatterTest::assertFormattedRangeEquals(const char16_t* message,
