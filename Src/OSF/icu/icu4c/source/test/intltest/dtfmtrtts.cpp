...
#ifndef INFINITE_TEST_ // @sobolev INFINITE-->INFINITE_TEST_
	#define INFINITE_TEST_ 0
#endif
...
void DateFormatRoundTripTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * par)
{
	optionv = (par && *par=='v');
	switch(index) {
...
bool DateFormatRoundTripTest::failure(UErrorCode status, const char * msg)
{
	if(U_FAILURE(status)) {
...
bool DateFormatRoundTripTest::failure(UErrorCode status, const char * msg, const UnicodeString & str)
{
	if(U_FAILURE(status)) {
...
void DateFormatRoundTripTest::test(const Locale & loc)
{
	...
	//  4 time
	//  16 date-time
	bool TEST_TABLE [24];//= new boolean[24];
	int32_t i = 0;
	for(i = 0; i < 24; ++i)
		TEST_TABLE[i] = TRUE;
	...
	for(style = DateFormat::FULL; style <= DateFormat::SHORT; ++style) {
		if(TEST_TABLE[itable++]) {
			...
			if(df == NULL) {
				errln(UnicodeString("Could not DF::createDateInstance ") +
	...
	for(style = DateFormat::FULL; style <= DateFormat::SHORT; ++style) {
		if(TEST_TABLE[itable++]) {
			...
			if(df == NULL) {
				errln(UnicodeString("Could not DF::createTimeInstance ") +
				    UnicodeString(styleName((DateFormat::EStyle)style)) + " Locale: " + loc.getDisplayName(temp));
			}
			else {
				test(df, loc, TRUE);
	...
	for(int32_t dstyle = DateFormat::FULL; dstyle <= DateFormat::SHORT; ++dstyle) {
		for(int32_t tstyle = DateFormat::FULL; tstyle <= DateFormat::SHORT; ++tstyle) {
			if(TEST_TABLE[itable++]) {
				...
				DateFormat * df = DateFormat::createDateTimeInstance((DateFormat::EStyle)dstyle,
					(DateFormat::EStyle)tstyle,
					loc);
				if(df == NULL) {
					dataerrln(UnicodeString("Could not DF::createDateTimeInstance ") +
					    UnicodeString(styleName((DateFormat::EStyle)dstyle)) + ", tstyle" +
					    UnicodeString(styleName((DateFormat::EStyle)tstyle))    + "Locale: " +
					    loc.getDisplayName(temp));
				}
				else {
					test(df, loc);
					delete df;
...
void DateFormatRoundTripTest::test(DateFormat * fmt, const Locale &origLocale, bool timeOnly)
{
	UnicodeString pat;
	...
	bool hasEra = (pat.indexOf(UnicodeString("G")) != -1);
	bool hasZoneDisplayName = (pat.indexOf(UnicodeString("z")) != -1) || (pat.indexOf(UnicodeString("v")) != -1)
	...
	for(int i = 0; i < TRIALS; ++i) {
		...
		for(loop = 0; loop < DEPTH; ++loop) {
			...
			if(s[loop].length() == 0) {
				errln(
					"FAIL: fmt->format gave 0-length string in " + pat + " with number " + d[loop] + " in locale " +
					origLocale.getName());
			}

			if(loop > 0) {
				if(smatch == 0) {
					bool match = s[loop] == s[loop-1];
					if(smatch == 0) {
						if(match)
							smatch = loop;
					}
				...
				if(dmatch == 0) {
					...
					bool match = d[loop] /*.getTime()*/ == d[loop-1] /*.getTime()*/;
					if(dmatch == 0) {
						if(match)
							dmatch = loop;
					}
		...
		if(smatch > maxSmatch) { // Don't compute unless necessary
			...
			// unless timezone isn't used or we aren't close to the DST changover
				...
				((hasZoneDisplayName && (fmt->getTimeZone().inDaylightTime(d[0], status) == fmt->getTimeZone().inDaylightTime(d[dmatch], status) || 
				getField(d[0], UCAL_MONTH) == UCAL_APRIL || getField(d[0], UCAL_MONTH) == UCAL_OCTOBER)) || !hasZoneDisplayName)) {
				maxSmatch = 2;
			}
		...
		bool fail = (dmatch > maxDmatch || smatch > maxSmatch);
		if(optionv || fail) {
			if(fail) {
				errln(UnicodeString("\nFAIL: Pattern: ") + pat +
