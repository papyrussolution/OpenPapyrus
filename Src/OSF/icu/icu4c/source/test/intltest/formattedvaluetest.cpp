...
private:
	...
	void assertAllPartsEqual(UnicodeString messagePrefix,
...
void FormattedValueTest::runIndexedTest(int32_t index, bool exec, const char *&name, char *) {
	if(exec) {
 ...
 * 4 = UDAT_AM_PM_FIELD
 */
void FormattedValueTest::assertAllPartsEqual(UnicodeString messagePrefix,
    const ConstrainedFieldPosition& cfpos,
    int32_t matching,
    UFieldCategory category,
    int32_t field,
	...
	assertEquals(messagePrefix + u": integer field",
	    (bool)((matching & 1) != 0), cfpos.matchesField(UFIELD_CATEGORY_NUMBER, UNUM_INTEGER_FIELD));
	assertEquals(messagePrefix + u": compact field",
	    (bool)((matching & 2) != 0), cfpos.matchesField(UFIELD_CATEGORY_NUMBER, UNUM_COMPACT_FIELD));
	assertEquals(messagePrefix + u": date field",
	    (bool)((matching & 4) != 0), cfpos.matchesField(UFIELD_CATEGORY_DATE, UDAT_AM_PM_FIELD));
}
...
void IntlTestWithFieldPosition::checkMixedFormattedValue(const char16_t* message,
    const FormattedValue& fv,
    UnicodeString expectedString,
	...
	bool afterLoopResult = fv.nextPosition(cfpos, status);
