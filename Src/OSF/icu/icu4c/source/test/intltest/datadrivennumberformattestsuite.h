...
class DataDrivenNumberFormatTestSuite : public IntlTest {
 ...
 public:
   ...
   *  skips the tests that are known to break for ICU4C.
      */
     void run(const char *fileName, bool runAllTests);
  ...
  * Test parsing with currency.
     */
    virtual bool isParseCurrencyPass(
            const NumberFormatTestTuple &tuple,
            UnicodeString & appendErrorMessage,
            UErrorCode & status);
 ...
 private:
    ...
    void showLineInfo();
    bool breaksC();
    bool readLine(UCHARBUF *f, UErrorCode &);
    bool isPass(
            const NumberFormatTestTuple &tuple,
            UnicodeString & appendErrorMessage,
            UErrorCode & status);
