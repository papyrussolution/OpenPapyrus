...
#define TESTCLASSID_TRANSLIT(c, t) UPRV_BLOCK_MACRO_BEGIN { \
		delete testClass(Transliterator::createInstance(UnicodeString(t), UTRANS_FORWARD, parseError, status), \
		    #c, \
		    "Transliterator: " #t, \
		    c ::getStaticClassID()); \
		if(U_FAILURE(status)) { \
...
UObject * UObjectTest::testClass(UObject * obj, const char * className, const char * factory, UClassID staticID)
{
	uint32_t i;
...
//
//    find common i18n -name '*.h' -print | xargs fgrep ClassID | cut -d: -f1 | cut -d\/ -f2-  | sort | uniq | sed -e
// 's%.*%#include "&"%'
...
void UObjectTest::testIDs()
{
...
#endif
	//TESTCLASSID_FACTORY(CompoundTransliterator,
	// Transliterator::createInstance(UnicodeString("Any-Jex;Hangul-Jamo"), UTRANS_FORWARD, parseError, status));
	...
	TESTCLASSID_FACTORY(RuleBasedTransliterator,
	    Transliterator::createFromRules(UnicodeString("abcd"), UnicodeString("a>b;"), UTRANS_FORWARD, parseError, status));
...
void UObjectTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /* par */)
{
	switch(index) {
		TESTCASE(0, testIDs);
