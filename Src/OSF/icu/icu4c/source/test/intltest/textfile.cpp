...
bool TextFile::readLine(UnicodeString & line, UErrorCode & ec) {
...
bool TextFile::readLineSkippingComments(UnicodeString & line, UErrorCode & ec,
    bool trim) {
	for(;;) {
 ...
 * successful.
 */
bool TextFile::setBuffer(int32_t index, char c, UErrorCode & ec) {
	if(capacity <= index) {
...
bool TextFile::ensureCapacity(int32_t mincapacity) {
