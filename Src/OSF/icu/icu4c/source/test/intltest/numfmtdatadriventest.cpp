...
void NumberFormatDataDrivenTest::runIndexedTest(int32_t index, bool exec, const char *& name, char *) {
	if(exec) {
...
static DecimalQuantity&strToDigitList(const UnicodeString & str, DecimalQuantity& digitList, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return digitList;
	}
	if(str == "NaN") {
...
static UnicodeString &format(const DecimalFormat& fmt, const DecimalQuantity& digitList, UnicodeString & appendTo,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return appendTo;
	}
...
template <class T>
static UnicodeString &format(const DecimalFormat& fmt, T value, UnicodeString & appendTo, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return appendTo;
	}
	FieldPosition fpos(FieldPosition::DONT_CARE);
...
static DecimalFormat* newDecimalFormat(const Locale & locale, const UnicodeString & pattern, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
bool NumberFormatDataDrivenTest::isFormatPass(const NumberFormatTestTuple& tuple,
...
bool NumberFormatDataDrivenTest::isToPatternPass(const NumberFormatTestTuple& tuple,
    UnicodeString & appendErrorMessage, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
bool NumberFormatDataDrivenTest::isParsePass(const NumberFormatTestTuple& tuple,
    UnicodeString & appendErrorMessage, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
bool NumberFormatDataDrivenTest::isParseCurrencyPass(const NumberFormatTestTuple& tuple,
    UnicodeString & appendErrorMessage,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
