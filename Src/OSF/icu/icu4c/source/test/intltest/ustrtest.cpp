...
void UnicodeStringTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * par)
{
...
void UnicodeStringTest::TestBasicManipulation()
{
...

	{
		...
		UnicodeString s, t(buffer, -1, SIZEOFARRAYi(buffer));

		if(s.setTo(buffer, -1, SIZEOFARRAYi(buffer)).length()!=u_strlen(buffer)) {
		...
		UnicodeString u(buffer, -1, SIZEOFARRAYi(buffer));
		if(s.setTo(buffer, -1, SIZEOFARRAYi(buffer)).length()!=SIZEOFARRAYi(buffer)) {
			errln("UnicodeString.setTo(buffer without NUL, length, capacity) does not work with length==-1");
		}
		if(u.length()!=SIZEOFARRAYi(buffer)) {
...
#if U_CHARSET_IS_UTF8
	{
		...
		UnicodeString from16(FALSE, utf16, SIZEOFARRAYi(utf16));
		...
		if(length8!=((int32_t)sizeof(utf8)-1) || 0!=memcmp(buffer, utf8, sizeof(utf8))) {
			errln("UnicodeString::extract(char * U_CHARSET_IS_UTF8) failed");
		}
		length8 = from16.extract(1, 2, buffer, (uint32_t)sizeof(buffer));
		if(length8!=4 || buffer[length8]!=0 || 0!=memcmp(buffer, utf8+1, length8)) {
...
void UnicodeStringTest::TestCompare()
{
	...
	/* test caseCompare() */
	{
		static const char16_t _mixed[] =
		{ 0x61, 0x42, 0x131, 0x3a3, 0xdf,       0x130,       0x49,  0xfb03,           0xd93f, 0xdfff, 0 }, _otherDefault[] =
		{ 0x41, 0x62, 0x131, 0x3c3, 0x73, 0x53, 0x69, 0x307, 0x69,  0x46, 0x66, 0x49, 0xd93f, 0xdfff, 0 }, _otherExcludeSpecialI[] =
		{ 0x41, 0x62, 0x131, 0x3c3, 0x53, 0x73, 0x69,        0x131, 0x66, 0x46, 0x69, 0xd93f, 0xdfff, 0 }, _different[] =
		{ 0x41, 0x62, 0x131, 0x3c3, 0x73, 0x53, 0x130,       0x49,  0x46, 0x66, 0x49, 0xd93f, 0xdffd, 0 };

		UnicodeString mixed(TRUE, _mixed, -1),
...
void UnicodeStringTest::TestExtract()
{
	...
	}
	{
		...
		UErrorCode errorCode = U_ZERO_ERROR;
		int32_t length = s.extract((char16_t *)NULL, 0, errorCode);
		if(errorCode!=U_BUFFER_OVERFLOW_ERROR || length!=s.length()) {
			errln("UnicodeString.extract(NULL, 0)==%d (%s) expected %d (U_BUFFER_OVERFLOW_ERROR)",
			    length,
			    s.length(),
...

	{
		...
		if(U_SUCCESS(errorCode)) {
			...
			errorCode = U_ZERO_ERROR;
			// extract for real
			if((length = s.extract(buffer, sizeof(buffer), cnv, errorCode))!=13 || memcmp(buffer, expect, 13)!=0 || buffer[13]!=0 || U_FAILURE(errorCode)) {
				errln("UnicodeString::extract(UConverter) conversion failed (length=%ld, %s)", length, u_errorName(errorCode));
			}
			// Test again with just the converter name.
			if((length = s.extract(0, s.length(), buffer, sizeof(buffer), "UTF-8"))!=13 ||
			    memcmp(buffer, expect, 13)!=0 || buffer[13]!=0 || U_FAILURE(errorCode)) {
				errln("UnicodeString::extract(\"UTF-8\") conversion failed (length=%ld, %s)", length, u_errorName(errorCode));
			}
			// try the constructor
			UnicodeString t(expect, sizeof(expect), cnv, errorCode);
			if(U_FAILURE(errorCode) || s!=t) {
				errln("UnicodeString(UConverter) conversion failed (%s)", u_errorName(errorCode));
			}
			ucnv_close(cnv);
		}
	}
...
void UnicodeStringTest::TestRemoveReplace()
{
...
void UnicodeStringTest::TestSearching()
{
	...
	if(occurrences != 3)
		errln(
			(UnicodeString)"indexOf with character & start & end offsets failed: expected to find 2 occurrences, found " +
			occurrences);
	//---
	...
	if(occurrences != 3)
		errln(
			(UnicodeString)"lastIndexOf with character & start & end offsets failed: expected to find 3 occurrences, found " +
			occurrences);
...
void UnicodeStringTest::TestSpacePadding()
{
...
void UnicodeStringTest::TestPrefixAndSuffix()
{
	UnicodeString test1("Now is the time for all good men to come to the aid of their country.");
...
void UnicodeStringTest::TestStartsWithAndEndsWithNulTerminated() {
...
void UnicodeStringTest::TestFindAndReplace()
{
...
void UnicodeStringTest::TestReverse()
{
...
void UnicodeStringTest::TestMiscellaneous()
{
	UnicodeString test1("This is a test");
	UnicodeString test2("This is a test");
...
void UnicodeStringTest::TestStackAllocation()
{
	char16_t testString[] = {
		0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x63, 0x72, 0x61, 0x7a, 0x79, 0x20, 0x74, 0x65, 0x73, 0x74,
		0x2e, 0
	};
...
{
	UnicodeString _in("abc\\u4567 \\n\\r \\U00101234xyz\\x1\\x{5289}\\x1b", -1, US_INV);
	UnicodeString _out("abc");
	_out.append((char16_t)0x4567);
	_out.append(" ");
	_out.append((char16_t)0xA);
	_out.append((char16_t)0xD);
	_out.append(" ");
	_out.append((UChar32)0x00101234);
	_out.append("xyz");
	_out.append((UChar32)1).append((UChar32)0x5289).append((char16_t)0x1b);
	UnicodeString result = _in.unescape();
	if(result != _out) {
		errln("FAIL: " + prettify(_in) + ".unescape() -> " + prettify(result) + ", expected " + prettify(_out));
	}
...
static int32_t _refUnicodeStringHasMoreChar32Than(const UnicodeString & s, int32_t start, int32_t length, int32_t number) 
{
...
void UnicodeStringTest::_testUnicodeStringHasMoreChar32Than(const UnicodeString & s, int32_t start, int32_t length, int32_t number) 
{
	if(static_cast<int32_t>(s.hasMoreChar32Than(start, length, number)) != _refUnicodeStringHasMoreChar32Than(s, start, length, number)) {
		errln("hasMoreChar32Than(%d, %d, %d)=%hd is wrong\n", start, length, number, s.hasMoreChar32Than(start, length, number));
	}
}

void UnicodeStringTest::TestCountChar32() 
{
	{
		UnicodeString s = UNICODE_STRING("\\U0002f999\\U0001d15f\\u00c4\\u1ed0", 32).unescape();
	...
	UnicodeString string(str, SIZEOFARRAYi(str));
...
void UnicodeStringTest::TestBogus() {
	UnicodeString test1("This is a test");
	UnicodeString test2("This is a test");
	UnicodeString test3("Me too!");
	...
	UErrorCode errorCode = U_ZERO_ERROR;
	UnicodeString
	    test4((const char16_t*)NULL),
	test5(TRUE, (const char16_t*)NULL, 1),
	test6((char16_t *)NULL, 5, 5),
	...
	test4.setTo(NULL, 3);
	test5.setTo(TRUE, (const char16_t*)NULL, 1);
	test6.setTo((char16_t *)NULL, 5, 5);
...
public:
	...
	virtual int32_t count(UErrorCode & /*status*/) const override {
		return SIZEOFARRAYi(testEnumStrings);
	}

	virtual const UnicodeString * snext(UErrorCode & status) override {
		if(U_SUCCESS(status) && i<SIZEOFARRAYi(testEnumStrings)) {
...
void UnicodeStringTest::TestStringEnumeration() {
	UnicodeString s;
	TestEnumeration ten;
	int32_t i, length;
	UErrorCode status;
	...
	for(i = 0; i<SIZEOFARRAYi(testEnumStrings); ++i) {
		status = U_ZERO_ERROR;
...
void UnicodeStringTest::TestNameSpace() {
...
void UnicodeStringTest::TestUTF32() 
{
	...
	UnicodeString from32 = UnicodeString::fromUTF32(utf32, SIZEOFARRAYi(utf32));
...
void UnicodeStringTest::TestReadOnlyAlias() 
{
	...
	bool hasRVO = wrapUChars(abc).getBuffer()==abc;
...
void UnicodeStringTest::TestAppendable() {
	UnicodeString dest;
	SimpleAppendable app(dest);
	doTestAppendable(dest, app);
}

void UnicodeStringTest::TestUnicodeStringImplementsAppendable() {
	UnicodeString dest;
	UnicodeStringAppendable app(dest);
	doTestAppendable(dest, app);
}

void UnicodeStringTest::TestSizeofUnicodeString() {
...
void UnicodeStringTest::TestMoveSwap() {
	static const char16_t abc[3] = { 0x61, 0x62, 0x63 }; // "abc"
	UnicodeString s1(FALSE, abc, SIZEOFARRAYi(abc)); // read-only alias
...
void UnicodeStringTest::TestUInt16Pointers() {
...
void UnicodeStringTest::TestWCharPointers() {
...
void UnicodeStringTest::TestNullPointers() {
