...
void NewResourceBundleTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
#define CONFIRM_UErrorCode(actual, expected) UPRV_BLOCK_MACRO_BEGIN { \
		if((expected)==(actual)) { \
			...
			record_fail(); \
			errln(action + (UnicodeString)" returned " + (UnicodeString)u_errorName(actual) + (UnicodeString)" instead of " + \
			    (UnicodeString)u_errorName(expected)); \
		} \
} UPRV_BLOCK_MACRO_END
...
void NewResourceBundleTest::TestResourceBundles()
{
...
void NewResourceBundleTest::TestConstruction()
{
...
void NewResourceBundleTest::TestIteration()
{
	UErrorCode err = U_ZERO_ERROR;
	const char * testdatapath;
	const char * data[] = {
...
static bool equalRB(ResourceBundle &a, ResourceBundle &b) {
	UResType type;
	UErrorCode status;
						...
						if(ures_getSize(bundle) > 1) {
							...
							if(!U_FAILURE(err) && ures_getSize(rowbundle)>1) {
								...
								while(ures_hasNext(rowbundle)) {
									...
									UnicodeString bla2 = ures_getUnicodeStringByIndex(rowbundle,
										col,
										&err);
									if(!U_FAILURE(err)) {
										expected += itoa(row, buf);
...
bool NewResourceBundleTest::testTag(const char * frag,
    bool in_Root,
    bool in_te,
    bool in_te_IN)
{
...
void NewResourceBundleTest::record_pass()
{
	++pass;
}

void NewResourceBundleTest::record_fail()
{
	err();
	++fail;
}

void NewResourceBundleTest::TestNewTypes() {
	char action[256];
	const char * testdatapath;
	UErrorCode status = U_ZERO_ERROR;
...
void NewResourceBundleTest::TestGetByFallback() {
	UErrorCode status = U_ZERO_ERROR;
...
static void U_CALLCONV traceData(const void *,
    int32_t fnNumber,
    int32_t,
    const char *,
    va_list args) {
