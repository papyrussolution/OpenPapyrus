...
private:
	...
	void TestFallback();
	void check(UGender expected_neutral, UGender expected_mixed, UGender expected_taints, const UGender* genderList,
	    int32_t listLength);
	void checkLocale(const Locale & locale, UGender expected, const UGender* genderList, int32_t listLength);
};

void GenderInfoTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /* par */) {
	if(exec) {
...
void GenderInfoTest::TestGetListGender() {
	...
	check(UGENDER_FEMALE, UGENDER_FEMALE, UGENDER_FEMALE, kSingleFemale, SIZEOFARRAYi(kSingleFemale));
	check(UGENDER_MALE, UGENDER_MALE, UGENDER_MALE, kSingleMale, SIZEOFARRAYi(kSingleMale));
	check(UGENDER_OTHER, UGENDER_OTHER, UGENDER_OTHER, kSingleOther, SIZEOFARRAYi(kSingleOther));

	check(UGENDER_OTHER, UGENDER_FEMALE, UGENDER_FEMALE, kAllFemale, SIZEOFARRAYi(kAllFemale));
	check(UGENDER_OTHER, UGENDER_MALE, UGENDER_MALE, kAllMale, SIZEOFARRAYi(kAllMale));
	check(UGENDER_OTHER, UGENDER_OTHER, UGENDER_MALE, kAllOther, SIZEOFARRAYi(kAllOther));

	check(UGENDER_OTHER, UGENDER_OTHER, UGENDER_MALE, kFemaleMale, SIZEOFARRAYi(kFemaleMale));
	check(UGENDER_OTHER, UGENDER_OTHER, UGENDER_MALE, kFemaleOther, SIZEOFARRAYi(kFemaleOther));
	check(UGENDER_OTHER, UGENDER_OTHER, UGENDER_MALE, kMaleOther, SIZEOFARRAYi(kMaleOther));
}
...
void GenderInfoTest::check(UGender expected_neutral,
    UGender expected_mixed,
    UGender expected_taints,
    const UGender* genderList,
    int32_t listLength) {
...
void GenderInfoTest::checkLocale(const Locale & locale, UGender expected, const UGender* genderList, int32_t listLength) {
	UErrorCode status = U_ZERO_ERROR;
