...
void IntlTestDecimalFormatSymbols::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
...
void IntlTestDecimalFormatSymbols::testDigitSymbols() {
	...
	for(int32_t i = 0; i<=9; i++) {
		assertEquals(UnicodeString("i. ASCII Digit at index ") + Int64ToUnicodeString(i), UnicodeString(u'0' + i), symbols.getConstDigitSymbol(i));
	}
	for(int32_t i = 0; i<=9; i++) {
		DecimalFormatSymbols::ENumberFormatSymbol key = i == 0 ? DecimalFormatSymbols::kZeroDigitSymbol : 
			static_cast<DecimalFormatSymbols::ENumberFormatSymbol>(DecimalFormatSymbols::kOneDigitSymbol + i - 1);
...
void IntlTestDecimalFormatSymbols::testNumberingSystem() {
	...
	for(int i = 0; i<8; i++) {
		...
		assertEquals((UnicodeString)"Percent sign with " + cas.locid + " and " + cas.nsname, expected2, actual2);
	}
}
