...
public: // package
	...
	virtual void findDaylightBoundaryUsingTimeZone(UDate d, bool startsInDST, UDate expectedBoundary);
	virtual void findDaylightBoundaryUsingTimeZone(UDate d, bool startsInDST, UDate expectedBoundary, TimeZone* tz);
private:
...
public: // package
	 ...
	 */
	virtual void verifyDST(UDate d, TimeZone* time_zone, bool expUseDaylightTime, bool expInDaylightTime, UDate expZoneOffset, UDate expDSTOffset);
	/**
	...
	void verifyMapping(Calendar& cal, int year, int month, int dom, int hour, double epochHours);
private:
	const UDate ONE_SECOND;
	const UDate ONE_MINUTE;
	const UDate ONE_HOUR;
	const UDate ONE_DAY_MS;
