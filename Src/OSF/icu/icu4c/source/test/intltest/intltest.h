...
//UnicodeString operator+(const UnicodeString & left, int64_t num); // Some compilers don't allow this because of the
// long type.
...
//
//| void MyTest::runIndexedTest(int32_t index, bool exec,
...
public:
	...
	virtual bool runTest(char * name = NULL, char * par = NULL, char * baseName = NULL); // not to be overridden

	virtual bool setVerbose(bool verbose = TRUE);
	virtual bool setNoErrMsg(bool no_err_msg = TRUE);
	virtual bool setQuick(bool quick = TRUE);
	virtual bool setLeaks(bool leaks = TRUE);
	virtual bool setNotime(bool no_time = TRUE);
	 ...
	 */
	bool logKnownIssue(const char * ticket, const UnicodeString & message);
	/**
	 * Logs that an issue is known. Can be called multiple times.
	 * Usually used this way:
	 * <code>if( ... && logKnownIssue("ICU-12345", "some bug")) continue; </code>
	 ...
	 * @return true if test should be skipped
	 */
	bool logKnownIssue(const char * ticket);
	/**
	 ...
	 * @return true if test should be skipped
	 */
	bool logKnownIssue(const char * ticket, const char * fmt, ...);
...
private:
	...
	bool assertTrue(const char * message,
	    bool condition,
	    bool quiet = FALSE,
	    bool possibleDataError = FALSE,
	    const char * file = NULL,
	    int line = 0);
	bool assertFalse(const char * message, bool condition, bool quiet = FALSE, bool possibleDataError = FALSE);
	/**
	 * @param possibleDataError - if TRUE, use dataerrln instead of errcheckln on failure
	 * @return TRUE on success, FALSE on failure.
	 */
	bool assertSuccess(const char * message, UErrorCode ec, bool possibleDataError = FALSE, const char * file = NULL, int line = 0);
	bool assertEquals(const char * message, const UnicodeString & expected,
	    const UnicodeString & actual, bool possibleDataError = FALSE);
	bool assertEquals(const char * message, const char * expected, const char * actual);
	bool assertEquals(const char * message, bool expected, bool actual);
	bool assertEquals(const char * message, int32_t expected, int32_t actual);
	bool assertEquals(const char * message, int64_t expected, int64_t actual);
	bool assertEquals(const char * message, double expected, double actual);
	/**
	 ...
	 */
	bool assertEqualsNear(const char * message, double expected, double actual, double delta);
	bool assertEquals(const char * message, UErrorCode expected, UErrorCode actual);
	bool assertEquals(const char * message, const UnicodeSet & expected, const UnicodeSet & actual);
	bool assertEquals(const char * message,
	    const std::vector<std::string>& expected, const std::vector<std::string>& actual);

#if !UCONFIG_NO_FORMATTING
	bool assertEquals(const char * message, const Formattable& expected,
	    const Formattable& actual, bool possibleDataError = FALSE);
	bool assertEquals(const UnicodeString & message, const Formattable& expected,
	    const Formattable& actual);
#endif
	bool assertNotEquals(const char * message, int32_t expectedNot, int32_t actual);
	bool assertTrue(const UnicodeString & message, bool condition, bool quiet = FALSE, bool possibleDataError = FALSE);
	bool assertFalse(const UnicodeString & message, bool condition, bool quiet = FALSE, bool possibleDataError = FALSE);
	bool assertSuccess(const UnicodeString & message, UErrorCode ec);
	bool assertEquals(const UnicodeString & message, const UnicodeString & expected,
	    const UnicodeString & actual, bool possibleDataError = FALSE);
	bool assertEquals(const UnicodeString & message, const char * expected, const char * actual);
	bool assertEquals(const UnicodeString & message, bool expected, bool actual);
	bool assertEquals(const UnicodeString & message, int32_t expected, int32_t actual);
	bool assertEquals(const UnicodeString & message, int64_t expected, int64_t actual);
	bool assertEquals(const UnicodeString & message, double expected, double actual);
	/**
	 * Asserts that two doubles are equal to within a positive delta. Returns
	 * false if they are not.
	 *
...
public:
	bool run_phase2(char * name, char * par); // internally, supports reporting memory leaks
