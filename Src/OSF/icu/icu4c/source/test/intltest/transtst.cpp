...
{
	UnicodeString RULES = UnicodeString("abc>zyx;") +
	    "ab>yz;" +
	    "bc>zx;" +
	    "ca>xy;" +
	    "a>x;" +
...
void TransliteratorTest::TestKeyboard() {
	...
	keyboardAux(*t, DATA, SIZEOFARRAYi(DATA));
	delete t;
}
...
public:
	UClassID getDynamicClassID() const override {
		return (UClassID)&gTestFilterClassID;
	}
};
...
void TransliteratorTest::TestJ329() {
	struct { bool containsErrors; const char * rule; } DATA[] = {
		{ FALSE, "a > b; c > d" },
	...
	for(int32_t i = 0; i<DATA_length; ++i) {
		...
		Transliterator * rbt = Transliterator::createFromRules("<ID>",
			...
			parseError,
			status);
		bool gotError = U_FAILURE(status);
...
void TransliteratorTest::TestFilterIDs() {
	...
	enum { DATA_length = SIZEOFARRAYi(DATA) };
...
void TransliteratorTest::TestNormalizationTransliterator() {
	...
	const char * CANON[] = {
		...
		"\\u1e10\\u0307\\u0323", "D\\u0327\\u0323\\u0307", "\\u1e10\\u0323\\u0307", // D dot_below cedilla
		                                                                            // dot_above
		"D\\u0307\\u0328\\u0323", "D\\u0328\\u0323\\u0307", "\\u1e0c\\u0328\\u0307", // D dot_above ogonek
		                                                                             // dot_below
...
void TransliteratorTest::TestToRules() {
	...
	static const int32_t DATA_length = SIZEOFARRAYi(DATA);
...
//    Transliterator* latinToDevToLatin=Transliterator::createInstance("Latin-Devanagari;Devanagari-Latin",
// UTRANS_FORWARD, parseError, status);
//    Transliterator* devToLatinToDev=Transliterator::createInstance("Devanagari-Latin;Latin-Devanagari",
// UTRANS_FORWARD, parseError, status);
//    if(U_FAILURE(status)) {
...
void TransliteratorTest::TestCompoundFilterID() {
	...
	for(int32_t i = 0; DATA[i]; i += 4) {
		...
		bool ok = (t.isValid() && U_SUCCESS(ec));
...
void TransliteratorTest::TestCompoundLatinRT() {
	...
	const int MAX_LEN = SIZEOFARRAYi(source);
	const char * const expected[MAX_LEN] = {
		"\\u0930\\u094D\\u092E\\u094D\\u0915\\u094D\\u0937\\u0947\\u0924\\u094D",
		"\\u0936\\u094d\\u0930\\u0940\\u092e\\u0926\\u094d",
		"\\u092d\\u0917\\u0935\\u0926\\u094d\\u0917\\u0940\\u0924\\u093e",
	...
	if(MAX_LEN != SIZEOFARRAYi(expected)) {
	...
	Transliterator* latinToDevToLatin = Transliterator::createInstance("Latin-Devanagari;Devanagari-Latin",
		...
		parseError,
		status);
	Transliterator* devToTelToDev    = Transliterator::createInstance("Devanagari-Telugu;Telugu-Devanagari",
		UTRANS_FORWARD,
		parseError,
		status);
...
void TransliteratorTest::TestGurmukhiDevanagari() {
	...
	UErrorCode status = U_ZERO_ERROR;
	UnicodeSet vowel(UnicodeString("[\\u0905-\\u090A \\u090F\\u0910\\u0913\\u0914 \\u093e-\\u0942\\u0947\\u0948\\u094B\\u094C\\u094D]",
	    -1,
	    US_INV).unescape(), status);
...
void TransliteratorTest::TestSpecialCases() {
	...
	const UnicodeString testCases[] = {
		...
		/*
		   "Title", CharsToUnicodeString("\\uFB00i\\u0131I\\u0130 \\u01C7\\u01C8\\u01C9 ") + DESERET_dee +
		      DESERET_DEE,
	...
	for(i = 0; i < sources; i++) {
		...
		for(j = 0; j < targets; j++) {
			...
			for(k = 0; k< variants; k++) {
				...
				if(U_FAILURE(status)) {
					// The following are forward-only, it is OK that creating an inverse will not
					// work:
					// 1. Devanagari-Arabic
					// 2. Any-*/BGN
					// 2a. Any-*/BGN_1981
...
void TransliteratorTest::TestAnyX() {
	...
	expect(*anyLatin,
	    CharsToUnicodeString(
		    "greek:\\u03B1\\u03B2\\u03BA\\u0391\\u0392\\u039A hiragana:\\u3042\\u3076\\u304F cyrillic:\\u0430\\u0431\\u0446"),
...
void TransliteratorTest::TestAny() {
	...
	UParseError parseError;
	Transliterator* anyLatin = Transliterator::createInstance("Any-Latin", UTRANS_FORWARD, parseError, status);
	if(U_FAILURE(status)) {
		dataerrln("Failure: file %s, line %d, status = %s", __FILE__, __LINE__, u_errorName(status));
		return;
	}
...
void TransliteratorTest::TestAllCodepoints() 
{
	...
	for(uint32_t i = 0; i<=0x10ffff; i++) {
		...
		strcpy(id, myId);
		strcpy(abbr, uscript_getShortName(code));
		...
		if(strcmp(newAbbrId, oldAbbrId)!=0) {
...
void TransliteratorTest::TestAlternateSyntax() {
	...
	// U+2192 == >
	// U+2194 == <>
	expect(CharsToUnicodeString("a \\u2192 x; b \\u2190 y; c \\u2194 z"), "abc", "xbz");
...
/*
   ...
   static const int32_t BOGUS_BEGIN_END_RULES_length = SIZEOFARRAYi(BOGUS_BEGIN_END_RULES);
 */
...
static const int32_t BEGIN_END_TEST_CASES_length = SIZEOFARRAYi(BEGIN_END_TEST_CASES);
...
void TransliteratorTest::expectAux(const UnicodeString & tag, const UnicodeString & source, const UnicodeString & result, const UnicodeString & expectedResult) 
{
	expectAux(tag, source + " -> " + result, result == expectedResult, expectedResult);
}

void TransliteratorTest::expectAux(const UnicodeString & tag, const UnicodeString & summary, bool pass, const UnicodeString & expectedResult) 
{
	if(pass) {
