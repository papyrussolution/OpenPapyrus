...
void DataDrivenFormatTest::runIndexedTest(int32_t index, bool exec,
 ...
 *             Headers { "locale", "zone", "spec", "date", "str"}
            ...
            // date:   either an unsigned long (millis), or a calendar spec ERA=0,YEAR=1, etc.. applied to the calendar
             *type specified by the locale
...
void DataDrivenFormatTest::testConvertDate(TestData * testData,
    const DataMap * /* settings */, bool fmt) {
	...
	while(testData->nextCase(currentCase, status)) {
		...
		// Process: 'locale'
		locale.extract(0, locale.length(), calLoc, (const char *)0); // default codepage.  Invariant codepage
		                                                            // doesn't have '@'!
		Locale loc(calLoc);
		if(spec.startsWith(kPATTERN)) {
			...
			format = DateFormat::createDateTimeInstance((DateFormat::EStyle)styleSet.getDateStyle(),
				(DateFormat::EStyle)styleSet.getTimeStyle(),
				loc);
			if(format == NULL) {
		...
		if(date.startsWith(kMILLIS)) {
			...
			UnicodeString add = UnicodeString(date, kRELATIVE_ADD.length()); // "add" is a string indicating
			                                                                 // which fields to add
...
//            }
			...
			if(output == expectStr) {
				...
				errln(caseString+": format:  output!=expectStr, got " + *udbg_escape(output,
				    &result) + " expected " +
				    *udbg_escape(expectStr, &result2));
			}
		}
		else {
			cal->clear();
			...
			if(useDate) {
				...
				if(gotDate == fromDate) {
					...
					errln(
						caseString+": parse: FAIL. parsed '"+expectStr+"' and got "+gotDateStr+", expected " +
						expectDateStr);
				}
			}
			else {
