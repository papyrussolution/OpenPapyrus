...
void StringTest::Test_UNICODE_STRING() {
...
void StringTest::Test_UNICODE_STRING_SIMPLE() {
...
void StringTest::TestUpperOrdinal() {
	for(int32_t i = 0;; ++i) {
...
void StringTest::TestLowerOrdinal() {
	for(int32_t i = 0;; ++i) {
		char ic = nativeInvChars[i];
		uint8_t ac = static_cast<uint8_t>(asciiInvChars[i]);
		int32_t expected = ac - 'a';
...
void StringTest::Test_UTF8_COUNT_TRAIL_BYTES() {
...
void StringTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
	if(exec) {
...
void StringTest::TestStringPieceFind() {
	struct TestCase {
		const char * haystack;
		const char * needle;
		int32_t expected;
...
#ifdef U_HAVE_STRING_VIEW
void StringTest::TestStringPieceStringView() {
	static constexpr char msg[] = "Kapow!";
...
void StringTest::TestStringPieceU8() {
...
void StringTest::TestCheckedArrayByteSink() {
...
void StringTest::TestStringByteSink() {
...
void StringTest::TestStringByteSinkAppendU8() 
{
	// ICU-20984 "mitigate some C++20 char8_t breakages"
	// For the following APIs there are overloads for both
	// const char * and const char8_t *.
...
void StringTest::TestSTLCompatibility() 
{
#if defined(_MSC_VER)
...
void StringTest::TestCharString() 
{
	IcuTestErrorCode errorCode(*this, "TestCharString()");
	char expected[400];
	static const char longStr[] = "This is a long string that is meant to cause reallocation of the internal buffer of CharString.";
