...
void UVectorTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) 
...
#define TEST_CHECK_STATUS(status) UPRV_BLOCK_MACRO_BEGIN { \
		if(U_FAILURE(status)) { errln("UVectorTest failure at line %d.  status=%s\n", __LINE__, u_errorName(status)); return; } \
} UPRV_BLOCK_MACRO_END

#define TEST_ASSERT(expr) UPRV_BLOCK_MACRO_BEGIN { \
		if((expr)==FALSE) { errln("UVectorTest failure at line %d.\n", __LINE__); } \
} UPRV_BLOCK_MACRO_END

static int32_t U_CALLCONV UVectorTest_compareInt32(UElement key1, UElement key2) 
{
...
U_CDECL_BEGIN
static /*int8_t*/bool U_CALLCONV UVectorTest_compareCstrings(const UElement key1, const UElement key2) 
{
	return sstreq((const char *)key1.pointer, (const char *)key2.pointer);
}
U_CDECL_END
...
U_CDECL_BEGIN
static bool U_CALLCONV neverTRUE(const UElement /*key1*/, const UElement /*key2*/) { return FALSE; }
U_CDECL_END
