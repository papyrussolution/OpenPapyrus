...
private:
	void verifyOffsets(const int32_t * expected,
	    const int32_t * actual,
	    int32_t count);
};

void SimpleFormatterTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/) {
	TESTCASE_AUTO_BEGIN;
...
void SimpleFormatterTest::TestTextWithNoArguments() 
{
	...
	assertEquals("String output 1", " has no  arguments.", fmt.getTextWithNoArguments());
	// Test offset positions
	int32_t offsets[3];
	assertEquals("String output 2", u" has no  arguments.", fmt.getTextWithNoArguments(offsets, 3));
	assertEquals("Offset at 0", 0, offsets[0]);
	assertEquals("Offset at 1", 8, offsets[1]);
	assertEquals("Offset at 2", -1, offsets[2]);
}
...
void SimpleFormatterTest::verifyOffsets(const int32_t * expected, const int32_t * actual, int32_t count) {
