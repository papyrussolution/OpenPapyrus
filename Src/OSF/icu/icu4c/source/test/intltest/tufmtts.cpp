...
static bool tmaEqual(const TimeUnitAmount& left, const TimeUnitAmount& right) {
	...
	bool result = left.getNumber().getDouble(status) == right.getNumber().getDouble(status);
	if(U_FAILURE(status)) {
		return FALSE;
	}
	return result;
		...
		for(int style = UTMUTFMT_FULL_STYLE;
			...
			for(TimeUnit::UTimeUnitFields j = TimeUnit::UTIMEUNIT_YEAR;
				...
				for(unsigned int i = 0; i < SIZEOFARRAYi(tests); ++i) {
					...
					if(!assertSuccess("parseObject()", status)) return;
					if(!tmaEqual(*((TimeUnitAmount*)result_1.getObject()),
					    *((TimeUnitAmount*)formattable.getObject()))) {
						dataerrln("No round trip: ");
					}
				}
			}
...
void TimeUnitTest::testGreekWithFallback() {
	...
	TimeUnit::UTimeUnitFields tunits[] =
	{TimeUnit::UTIMEUNIT_SECOND, TimeUnit::UTIMEUNIT_MINUTE, TimeUnit::UTIMEUNIT_HOUR, TimeUnit::UTIMEUNIT_DAY,
	 TimeUnit::UTIMEUNIT_MONTH, TimeUnit::UTIMEUNIT_YEAR};
	UTimeUnitFormatStyle styles[] = {UTMUTFMT_FULL_STYLE, UTMUTFMT_ABBREVIATED_STYLE};
	const int numbers[] = {1, 7};

	const char16_t oneSecond[] =
			...
			for(unsigned int styleIndex = 0;
				...
				for(unsigned int unitIndex = 0;
					...
					u_strToUTF8(tmp1,
					    128,
					    &len,
					    expected[counter].unescape().getTerminatedBuffer(),
					    expected[counter].unescape().length(),
					    &status);
					std::cout <<  "Formatted string : " << tmp << " expected : " << tmp1 << "\n";
#endif
					if(!assertEquals("formatted time string is not expected, locale: " +
					    UnicodeString(locales[locIndex]) + " style: " + (int)styles[styleIndex] + " units: " +
					    (int)tunits[unitIndex], expected[counter], str)) {
						str.remove();
						return;
					}
					str.remove();
