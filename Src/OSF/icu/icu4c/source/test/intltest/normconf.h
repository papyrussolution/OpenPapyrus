...
class NormalizerConformanceTest : public IntlTest {
  ...
  * @return true if the test passes
     */
    bool checkConformance(const UnicodeString * field,
                           const char *line,
                           int32_t options,
                           UErrorCode & status);

    bool checkNorm(UNormalizationMode mode, int32_t options,
  ...
  * @param return true if got == exp
     */
    bool assertEqual(const char *op, const char *op2,
  ...
  * @return FALSE upon failure
     */
    bool hexsplit(const char *s, char delimiter,
