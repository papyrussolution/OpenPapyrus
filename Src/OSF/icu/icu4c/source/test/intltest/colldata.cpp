	...
	: ces(NULL), listMax(CELIST_BUFFER_SIZE), listSize(0)
{
	...
	bool toShift = ucol_getAttribute(coll, UCOL_ALTERNATE_HANDLING, &status) ==  UCOL_SHIFTED;
	...
	while((order = ucol_next(elems, &status)) != UCOL_NULLORDER) {
		bool cont = isContinuation(order);
...
bool CEList::matchesAt(int32_t offset, const CEList * other) const
{
...
U_CDECL_BEGIN
static void U_CALLCONV deleteStringList(void * obj)
{
