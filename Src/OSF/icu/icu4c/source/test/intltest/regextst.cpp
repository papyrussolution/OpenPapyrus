...
void RegexTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
//
//          The expected results are bool - TRUE or FALSE.
...
bool RegexTest::doRegexLMTest(const char * pat, const char * text, bool looking, bool match, int32_t line) {
...
bool RegexTest::doRegexLMTestUTF8(const char * pat, const char * text, bool looking, bool match, int32_t line) {
...
void RegexTest::API_Match() 
{
	...
	//
	// A pattern that doesn't save state should work with a minimal sized stack
	//
	{
		UErrorCode status = U_ZERO_ERROR;
	...
	//
	// find with zero length matches, match position should bump ahead to prevent loops.
	//
	{
		int32_t i;
		UErrorCode status = U_ZERO_ERROR;
		RegexMatcher m("(?= ?)", 0, status);  // This pattern will zero-length matches anywhere, using an always-true look-ahead.
		REGEX_CHECK_STATUS;
		UnicodeString s("    ");
		m.reset(s);
		for(i = 0;; i++) {
...
void RegexTest::API_Match_UTF8() 
{
	...
	//
	{
		...
		int32_t input1Len = static_cast<int32_t>(strlen("abcdef this is a test")); /* TODO: why not nativelen
		                                                                              (input1) ? */
		...
		REGEX_CHECK_STATUS;
		REGEX_ASSERT(m1->lookingAt(status) == TRUE);
		const char str_abcdefthisisatest[] =
		{ 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x20, 0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x74, 0x65,
		  0x73, 0x74, 0x00 };                                                                                                                                                /*
		                                                                                                                                                                        abcdef
		                                                                                                                                                                        this
		                                                                                                                                                                        is
	...
	//
	{
		...
		UErrorCode status = U_ZERO_ERROR;
		UText re = UTEXT_INITIALIZER;
		const char str_01234567_pat[] =
		{ 0x30, 0x31, 0x28, 0x32, 0x33, 0x28, 0x34, 0x35, 0x29, 0x36, 0x37, 0x29, 0x28, 0x2e, 0x2a, 0x29, 0x00 };                         /*
		                                                                                                                                     01(23(45)67)(.*)
		       */
	...
	//
	{
		...
		REGEX_CHECK_STATUS;
		UText input = UTEXT_INITIALIZER;
		const char str_abcabcabc[] =
		{ 0x2e, 0x61, 0x62, 0x63, 0x2e, 0x2e, 0x61, 0x62, 0x63, 0x2e, 0x2e, 0x2e, 0x61, 0x62, 0x63, 0x2e, 0x2e, 0x00 };                      /*
		                                                                                                                                        .abc..abc...abc..
		 */
	...
	//
	{
		...
		UErrorCode status = U_ZERO_ERROR;
		UText re = UTEXT_INITIALIZER;
		const char str_Gabcabc[] =
		{ 0x2e, 0x2a, 0x3f, 0x28, 0x3f, 0x3a, 0x28, 0x5c, 0x47, 0x61, 0x62, 0x63, 0x29, 0x7c, 0x28, 0x61, 0x62, 0x63, 0x29,
		  0x29, 0x00 };                                                                                                                                      /*
		                                                                                                                                                        .*?(?:(\\Gabc)|(abc))
		        */
...
void RegexTest::API_Replace_UTF8() 
{
	...
	REGEX_CHECK_STATUS;
	const char str_yzabcabc[] =
	{ 0x2e, 0x79, 0x7a, 0x2e, 0x2e, 0x61, 0x62, 0x63, 0x2e, 0x2e, 0x2e, 0x61, 0x62, 0x63, 0x2e, 0x2e, 0x00 };                         /*
	                                                                                                                                     .yz..abc...abc..
	       */
	...
	//  Plain vanilla non-matches.
	//
	const char str_abxabxabx[] =
	{ 0x2e, 0x61, 0x62, 0x78, 0x2e, 0x2e, 0x61, 0x62, 0x78, 0x2e, 0x2e, 0x2e, 0x61, 0x62, 0x78, 0x2e, 0x2e, 0x00 };                          /*
	                                                                                                                                            .abx..abx...abx..
	     */
	...
	const char str_v[24] =
	{ 0x54, 0x68, 0x65, 0x20, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20, 0x6f, 0x66, 0x20, 0x5c, 0x24, 0x31, 0x20, 0x69, 0x73, 0x20, 0x24,
	  0x31, 0x2e, 0x00 };                                                                                                                                                  /*
	                                                                                                                                                                          The
	                                                                                                                                                                          value
	                                                                                                                                                                          of
	...
	result = matcher2->replaceFirst(&replText, NULL, status);
	REGEX_CHECK_STATUS;
	const char str_Thevalueof1isbcdefg[] =
	{ 0x54, 0x68, 0x65, 0x20, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20, 0x6f, 0x66, 0x20, 0x24, 0x31, 0x20, 0x69, 0x73, 0x20, 0x62, 0x63,
	  0x2e, 0x64, 0x65, 0x66, 0x67, 0x00 };                                                                                                                                                                  /*
	                                                                                                                                                                                                            The
	                                                                                                                                                                                                            value
	                                                                                                                                                                                                            of
	                                                                                                                                                                                                            $1
	...
	const char str_byitselfnogroupnumber[] = { 0x5c, 0x24, 0x20, 0x62, 0x79, 0x20, 0x69, 0x74, 0x73, 0x65, 0x6c,
						   0x66, 0x2c, 0x20, 0x6e, 0x6f, 0x20, 0x67, 0x72, 0x6f, 0x75, 0x70, 0x20, 0x6e, 0x75, 0x6d,
						   0x62,
	...
	result = matcher2->replaceFirst(&replText, NULL, status);
	REGEX_CHECK_STATUS;
	const char str_byitselfnogroupnumberdefg[] =
	{ 0x24, 0x20, 0x62, 0x79, 0x20, 0x69, 0x74, 0x73, 0x65, 0x6c, 0x66, 0x2c, 0x20, 0x6e, 0x6f, 0x20, 0x67, 0x72, 0x6f, 0x75, 0x70,
	  0x20, 0x6e, 0x75, 0x6d, 0x62, 0x65, 0x72, 0x20, 0x24, 0x24, 0x24, 0x64, 0x65, 0x66, 0x67, 0x00 };                                                                                                                                                                        /*
	                                                                                                                                                                                                                                                                              $
	                                                                                                                                                                                                                                                                              by
	                                                                                                                                                                                                                                                                              itself,
	                                                                                                                                                                                                                                                                              no
	...
	unsigned char supplDigitChars[] =
	{ 0x53, 0x75, 0x70, 0x70, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x74, 0x61, 0x6c, 0x20, 0x44, 0x69, 0x67, 0x69, 0x74, 0x20, 0x31, 0x20,
	  0x24, 0x78, 0x78, 0x78, 0x78, 0x2e, 0x00 };                                                                                                                                                             /*
	                                                                                                                                                                                                             Supplemental
	                                                                                                                                                                                                             Digit
	                                                                                                                                                                                                             1
	                                                                                                                                                                                                             $xxxx.
	...
	result = matcher2->replaceFirst(&replText, NULL, status);
	REGEX_CHECK_STATUS;
	const char str_SupplementalDigit1bcdefg[] =
	{ 0x53, 0x75, 0x70, 0x70, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x74, 0x61, 0x6c, 0x20, 0x44, 0x69, 0x67, 0x69, 0x74, 0x20, 0x31, 0x20,
	  0x62, 0x63, 0x2e, 0x64, 0x65, 0x66, 0x67, 0x00 };                                                                                                                                                                       /*
	                                                                                                                                                                                                                             Supplemental
	                                                                                                                                                                                                                             Digit
	                                                                                                                                                                                                                             1
	                                                                                                                                                                                                                             bc.defg
	...
	REGEX_ASSERT(result == &destText);
	REGEX_ASSERT_UTEXT_UTF8(str_SupplementalDigit1bcdefg, result);
	const char str_badcapturegroupnumber5[] =
	{ 0x62, 0x61, 0x64, 0x20, 0x63, 0x61, 0x70, 0x74, 0x75, 0x72, 0x65, 0x20, 0x67, 0x72, 0x6f, 0x75, 0x70, 0x20, 0x6e, 0x75, 0x6d,
	  0x62, 0x65, 0x72, 0x20, 0x24, 0x35, 0x2e, 0x2e, 0x2e,  0x00 };                                                                                                                                                                     /*
	                                                                                                                                                                                                                                        bad
	                                                                                                                                                                                                                                        capture
	                                                                                                                                                                                                                                        group
	                                                                                                                                                                                                                                        number
...
//    REGEX_ASSERT_UTEXT_UTF8("abcdefg", result);
	...
	//
	{
		const char str_abc1abc2abc3[] =
		{ 0x61, 0x62, 0x63, 0x20, 0x31, 0x20, 0x61, 0x62, 0x63, 0x20, 0x32, 0x20, 0x61, 0x62, 0x63, 0x20, 0x33, 0x00 };                       /*
		                                                                                                                                         abc
		                                                                                                                                         1
		                                                                                                                                         abc
		                                                                                                                                         2
		...
		result = matcher->replaceAll(&replText, NULL, status);
		REGEX_CHECK_STATUS;
		const char str_C1C2C3[] =
		{ 0x2d, 0x2d, 0x43, 0x2d, 0x2d, 0x20, 0x31, 0x20, 0x2d, 0x2d, 0x43, 0x2d, 0x2d, 0x20, 0x32, 0x20, 0x2d, 0x2d, 0x43,
		  0x2d, 0x2d, 0x20, 0x33, 0x00 };                                                                                                                                     /*
		                                                                                                                                                                         --C--
		                                                                                                                                                                         1
		                                                                                                                                                                         --C--
		                                                                                                                                                                         2
	...
	//
	{
		...
		const char str_blah[] =
		{ 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77,
		  0x69, 0x74, 0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74,
		  0x68, 0x20, 0x65, 0x65, 0x20, 0x73, 0x73, 0x20, 0x73, 0x74, 0x75, 0x66, 0x66, 0x20, 0x65, 0x65, 0x20, 0x66,
		  0x69, 0x6e, 0x00 };                                                                                                                                                                                                                                                                                                                                                   /*
		                                                                                                                                                                                                                                                                                                                                                                           The
		                                                                                                                                                                                                                                                                                                                                                                           matches
		                                                                                                                                                                                                                                                                                                                                                                           start
		...
		REGEX_CHECK_STATUS;
		const char str_blah2[] =
		{ 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77,
		  0x69, 0x74, 0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74,
		  0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68, 0x00 };                                                                                                                                                                                                                                                /*
		                                                                                                                                                                                                                                                                                                            The
		                                                                                                                                                                                                                                                                                                            matches
		                                                                                                                                                                                                                                                                                                            start
		                                                                                                                                                                                                                                                                                                            with
		...
		{ 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77,
		      0x69, 0x74, 0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74,
		      0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68, 0x20, 0x66, 0x69, 0x6e, 0x00 };                                                                                                                                                                                                                                            /*
		                                                                                                                                                                                                                                                                                                                                    The
		                                                                                                                                                                                                                                                                                                                                    matches
		                                                                                                                                                                                                                                                                                                                                    start
		                                                                                                                                                                                                                                                                                                                                    with
...
static bool utextOffsetToNative(UText * utext, int32_t unistrOffset, int32_t& nativeIndex)
{
	bool couldFind = TRUE;
...
void RegexTest::regex_find(const UnicodeString & pattern,
	...
	bool isMatch        = FALSE, isUTF8Match = FALSE;
	bool failed         = FALSE;
	int32_t numFinds;
	int32_t i;
	bool useMatchesFunc   = FALSE;
	bool useLookingAtFunc = FALSE;
	...
	if(UTF8Pattern == NULL) {
		// UTF-8 does not allow unpaired surrogates, so this could actually happen without being a failure of
		// the engine
	...
	if(UTF8Pattern != NULL) {
		...
		if(UTF8Matcher == NULL) {
			// UTF-8 does not allow unpaired surrogates, so this could actually happen without being a
	...
	if(UTF8Matcher != NULL) {
		...
		for(i = 0; i<groupStarts.size(); i++) {
			...
			//  -1 means there was no UVector slot and we won't be requesting that capture group for this
			// test, don't bother inserting
			if(start >= 0) {
				int32_t startUTF8;
				if(!utextOffsetToNative(&inputText, start, startUTF8)) {
					errln("Error at line %d: could not find native index for group start %d.  UTF16 index %d",
					    line,
					    i,
					    start);
					failed = TRUE;
			...
			if(end >= 0) {
				...
				if(!utextOffsetToNative(&inputText, end, endUTF8)) {
					errln("Error at line %d: could not find native index for group end %d.  UTF16 index %d",
					    line,
					    i,
					    end);
					failed = TRUE;
	...
	else if(UTF8Matcher != NULL && (flags.indexOf((char16_t)0x59) >= 0) &&  //  'Y' flag:  RequireEnd() == false
	...
	else if(UTF8Matcher != NULL && (flags.indexOf((char16_t)0x79) >= 0) &&  //  'Y' flag:  RequireEnd() == false
	...
	else if(UTF8Matcher != NULL && (flags.indexOf((char16_t)0x5A) >= 0) &&  //  'Z' flag:  hitEnd() == false
	...
	else if(UTF8Matcher != NULL && (flags.indexOf((char16_t)0x7A) >= 0) &&  //  'z' flag:  hitEnd() == true
...
void RegexTest::PerlTests() 
{
	...
	while(lineMat->find()) {
		...
		while(perlExpr.length() > 0) {
			...
			else if(perlExpr.startsWith(UNICODE_STRING_SIMPLE("\\"))) { // \Escape.  Take following char as
				                                                    // a literal.
...
struct callBackContext {
	...
	int32_t numCalls;
	int32_t lastSteps;
	void reset(int32_t max) {
		maxCalls = max; numCalls = 0; lastSteps = 0;
	}
};

U_CDECL_BEGIN
static bool U_CALLCONV testCallBackFn(const void * context, int32_t steps) {
...
struct progressCallBackContext {
	...
	int32_t numCalls;
	void reset(int32_t max) {
		maxCalls = max; numCalls = 0; lastIndex = 0;
	}
};
...
U_CDECL_BEGIN
static bool U_CALLCONV testProgressCallBackFn(const void * context, int64_t matchIndex) {
...
void RegexTest::FindProgressCallbacks() {
...

	{
		...
		status = U_ZERO_ERROR;
		s = "aaaaaaaaaaaaaaaaaaab";
		cbInfo.reset(s.length()); //  Some upper limit for number of calls that is greater than size of our
		                          // input string
		matcher.reset(s);
		...
		// A longer running match that causes matcher.find() to invoke our callback which we cancel/interrupt at
		// some point.
		status = U_ZERO_ERROR;
...
void RegexTest::NamedCapture() 
{
	...
	status = U_ZERO_ERROR;
	replacedText  = m->replaceAll(UnicodeString("<$000016>"), status);// Consume leading zeroes. Don't consume
	                                                                  // digits
...
void RegexTest::Bug7651() 
{
	...
	// malloc instead of using fSmallData.
	//  It will cause a segfault if RegexMatcher tries to use fSmallData instead of malloc'ing the memory needed
	// (see init2) for the pattern operator stack allocation.
	UnicodeString pattern2(
		"((https?\\:\\/\\/|www\\.)\\S+(?<![\\!\\),\\.:;\\]\\u0080-\\uFFFF])|(?<![A-Za-z0-9_])[\\@\\uff20][A-Za-z0-9_]+(?:\\/[\\w\\-]+)?|(?<![A-Za-z0-9])[\\#\\uff03][A-Za-z0-9_][A-Za-z0-9_\\u00c0-\\u00d6\\u00c8-\\u00f6\\u00f8-\\u00ff]*|\\$[A-Za-z]+)");
...
void RegexTest::TestCase11049(const char * pattern, const char * data, bool expectMatch, int32_t lineNumber) {
	UErrorCode status = U_ZERO_ERROR;
