...
CollationMonkeyTest::CollationMonkeyTest() : source("-abcdefghijklmnopqrstuvwxyz#&^$@", ""), myCollator(0)
{
...
void CollationMonkeyTest::report(UnicodeString & s, UnicodeString & t, int32_t result, int32_t revResult)
{
	if(revResult != -result) {
		UnicodeString msg;
		msg += s;
...
int32_t CollationMonkeyTest::checkValue(int32_t value)
{
	if(value < 0) {
		return -value;
	}
...
void CollationMonkeyTest::TestCompare(/* char * par */)
{
	if(source.length() == 0) {
...
void CollationMonkeyTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
