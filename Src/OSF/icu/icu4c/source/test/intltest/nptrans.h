...
public :
    ...
    inline bool isProhibited(UChar32 ch);
  ...
  * @return TRUE is the code point is a label separator
     */
    bool isLabelSeparator(UChar32 ch, UErrorCode & status);

    inline bool isLDHChar(UChar32 ch);
...
inline bool NamePrepTransform::isLDHChar(UChar32 ch) {
    // high runner case
    if(ch>0x007A) {
        return FALSE;
    }
