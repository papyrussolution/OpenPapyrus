...
bool beVerbose = FALSE, haveCopyright = TRUE;

/* prototypes --------------------------------------------------------------- */

static void parseMappings(const char * filename, bool reportError, TestIDNA& test, UErrorCode * pErrorCode);
...
extern int testData(TestIDNA& test) 
{
	...
	char * filename = (char *)SAlloc::M(strlen(IntlTest::pathToDataDirectory())*1024);
...
static void U_CALLCONV strprepProfileLineFn(void * /*context*/,
...
static inline UStringPrepType getValues(uint32_t result, int32_t& value, bool& isIndex) {
...
static void compareMapping(uint32_t codepoint, uint32_t* mapping, int32_t mapLength,
    UStringPrepType type) {
	uint32_t result = 0;
		...
		for(int8_t i = 0; i< mapLength; i++) {
			if(mapping[i] <= 0xFFFF) {
				...
				if(mappingData[index+i] != lead ||
					...
					pTestIDNA->errln("Did not get the expected result. Expected: 0x%04X 0x%04X  Got: 0x%04X 0x%04X",
					    lead,
					    trail,
					    mappingData[index+i],
		...
		if(result > _SPREP_TYPE_THRESHOLD) {
			if(retType != type) {
				pTestIDNA->errln("FAIL: Did not get the expected type for 0x%06X. Expected: %s Got: %s\n",
				    start,
				    usprepTypeNames[type],
				    usprepTypeNames[retType]);
			}
		}
		else {
