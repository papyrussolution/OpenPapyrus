...
void IntlCalendarTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	...
	for(j = 0; locs[j]; j++) {
		...
		if(U_SUCCESS(status)) {
			...
			if(strcmp(c->getType(), types[j])) {
				dataerrln(UnicodeString(locs[j]) + UnicodeString(
	...
	for(int32_t i = 0; data[i]!=-1;) {
		...
		if(y == year && e == era) {
			logln((UnicodeString)"OK: " + D + " => " + cal.get(UCAL_ERA, status) + ":" +
			    ...
			    (cal.get(UCAL_MONTH,
			    status)+1) + "/" + cal.get(UCAL_DATE, status) +  " (" + UnicodeString(cal.getType()) + ")");
		}
		else {
...
#define JapaneseCalendar_HEISEI 235
	...
	int32_t data[] = {
		...
		JapaneseCalendar_SHOWA,     64,   1989,  UCAL_JANUARY, 7,// Test current era transition (different code
		                                                   // path than others)
...
void IntlCalendarTest::TestJapaneseFormat() {
	...
	if(otherDate != aDate) {
		...
		fmt.format(otherDate, str3);
		errln("Parse incorrect of " + expected + " - wanted " + aDate + " but got " +  " = " +   otherDate + ", " + str3 + " = " + CalendarTest::calToStr(
			    *cal2));
	}
	else {
		logln("Parsed OK: " + expected);
...
void IntlCalendarTest::TestForceGannenNumbering()
{
	...
	if(U_FAILURE(status)) {
		...
		if(testString1.length() < 3 || testString1.charAt(2) != 0x5143) {
			errln(UnicodeString("Formatting year 1 in created text style, got " + testString1 +
		...
		if(testString2.length() < 2 || testString2.charAt(1) != 0x0031) {
			errln(UnicodeString("Formatting year 1 in created numeric style, got " + testString2 +
		...
		if(testString1.length() < 2 || testString1.charAt(1) != 0x0031) {
			errln(UnicodeString("Formatting year 1 in applied numeric style, got " + testString1 +
			    " but expected 2nd char to be 1"));
		}
		...
		if(testString2.length() < 3 || testString2.charAt(2) != 0x5143) {
			errln(UnicodeString("Formatting year 1 in applied text style, got " + testString2 +
			    " but expected 3rd char to be 0x5143"));
		}
	}
		...
		if(U_FAILURE(status)) {
			...
			if(U_FAILURE(status)) {
				...
				if(testString3.length() < 3 || testString3.charAt(2) != 0x0031) {
					errln(UnicodeString("Formatting year 1 with Gannen disabled, got " + testString3 +
...
void IntlCalendarTest::simpleTest(const Locale & loc, const UnicodeString & expect, UDate expectDate, UErrorCode & status)
{
	...
	if(fmt2) {
		...
		if(d != expectDate) {
			fmt2->format(d, tmp);
			errln(UnicodeString("Failed to parse ") + escape(
				    expect) + ", " + loc.getName() + " expect " + (double)expectDate + " got " + (double)d  + " " +
			    escape(tmp));
			logln("wanted " +
			    escape(fmt0->format(expectDate, tmp.remove())) + " but got " + escape(fmt0->format(d, tmp.remove())));
		}
		delete fmt2;
	}
	else {
