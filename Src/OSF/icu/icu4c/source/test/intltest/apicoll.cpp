...
void CollationAPITest::doAssert(bool condition, const char * message)
{
	if(!condition) {
...
void CollationAPITest::TestProperty(/* char * par */)
{
	...
	doAssert((col->compare("black-bird", "blackbird", 5) == Collator::EQUAL),
	    "black-bird = blackbird with length of 5 comparison failed");
	doAssert((col->compare("black bird", "black-bird", 10) == Collator::LESS),
...
void CollationAPITest::TestRuleBasedColl()
{
...
void CollationAPITest::TestRules()
{
...
void CollationAPITest::TestDecomposition() {
	UErrorCode status = U_ZERO_ERROR;
...
void CollationAPITest::TestSafeClone() {
...
void CollationAPITest::TestHashCode(/* char * par */)
{
...
//
void CollationAPITest::TestCollationKey(/* char * par */)
{
...
//
void CollationAPITest::TestElemIter(/* char * par */)
{
...
void CollationAPITest::TestOperators(/* char * par */)
{
	UErrorCode success = U_ZERO_ERROR;
	UnicodeString ruleset1("&9 < a, A < b, B < c, C; ch, cH, Ch, CH < d, D, e, E");
	UnicodeString ruleset2("&9 < a, A < b, B < c, C < d, D, e, E");
	...
	*col1 = *col2;
	...
	if(U_FAILURE(success)) {
		errln(
			"The RuleBasedCollator constructor failed when building with the 2nd rule set with tertiary strength and Normalizer::NO_OP.");
		return;
	}
...
// test clone and copy
void CollationAPITest::TestDuplicate(/* char * par */)
{
	UErrorCode status = U_ZERO_ERROR;
...
void CollationAPITest::TestCompare(/* char * par */)
{
...
void CollationAPITest::TestGetAll(/* char * par */)
{
...
void CollationAPITest::TestSortKey()
{
	...
	doAssert(key1.compareTo(key2) == Collator::GREATER, "Result should be \"Abcda\" > \"abcda\"");
	doAssert(key2.compareTo(key1) == Collator::LESS, "Result should be \"abcda\" < \"Abcda\"");
	doAssert(key2.compareTo(key3) == Collator::EQUAL, "Result should be \"abcda\" ==  \"abcda\"");
	...
	col->getSortKey(UnicodeString(test1), sortkey1, 64);
	col->getSortKey(UnicodeString(test2), sortkey2, 64);
	col->getSortKey(UnicodeString(test3), sortkey3, 64);

	const uint8_t * tempkey = key1.getByteArray(keylength);
	doAssert(memcmp(tempkey, sortkey1, keylength) == 0, "Test1 string should have the same collation key and sort key");
	tempkey = key2.getByteArray(keylength);
	doAssert(memcmp(tempkey, sortkey2, keylength) == 0, "Test2 string should have the same collation key and sort key");
	tempkey = key3.getByteArray(keylength);
	doAssert(memcmp(tempkey, sortkey3, keylength) == 0, "Test3 string should have the same collation key and sort key");
	...
	col->getCollationKey(test2, u_strlen(test2), key2, status);
	col->getCollationKey(test3, u_strlen(test3), key3, status);
	doAssert(key1.compareTo(key2) == Collator::EQUAL, "Result should be \"Abcda\" == \"abcda\"");
	doAssert(key2.compareTo(key3) == Collator::EQUAL, "Result should be \"abcda\" ==  \"abcda\"");
	tempkey = key2.getByteArray(keylength);
	doAssert(memcmp(tempkey, key2primary.getAlias(), keylength - 1) == 0, "Binary format for 'abcda' sortkey different for secondary strength!");
	col->getSortKey(UnicodeString(test1), sortkey1, 64);
	col->getSortKey(UnicodeString(test2), sortkey2, 64);
	col->getSortKey(UnicodeString(test3), sortkey3, 64);
	tempkey = key1.getByteArray(keylength);
...
void CollationAPITest::TestGetLocale() {
	...
	coll = new RuleBasedCollator(UnicodeString(rlz), status);
	locale = coll->getLocale(ULOC_REQUESTED_LOCALE, status);
...
U_CDECL_BEGIN
static int U_CALLCONV compare_teststruct(const void * string1, const void * string2) {
...
UCollationResult TestCollator::compare(const UnicodeString & source, const UnicodeString & target, UErrorCode & status) const
{
	if(U_SUCCESS(status)) {
...
CollationKey& TestCollator::getCollationKey(const char16_t * source, int32_t sourceLength, CollationKey& key, UErrorCode & status) const
{
...
int32_t TestCollator::getSortKey(const UnicodeString & source, uint8_t* result, int32_t resultLength) const
{
	UErrorCode status = U_ZERO_ERROR;
	int32_t length = source.extract((char *)result, resultLength, NULL, status);
	result[length] = 0;
	return length;
}

int32_t TestCollator::getSortKey(const char16_t * source, int32_t sourceLength, uint8_t* result, int32_t resultLength) const
{
	UnicodeString str(source, sourceLength);
...
void CollationAPITest::TestGapTooSmall() {
	...
	{
		RuleBasedCollator(UnicodeString(u"&☺<*\u4E00-\u9FFF"), errorCode);
	...
	{
		RuleBasedCollator coll(UnicodeString(u"&[before 1]\uFDD1€<*\u4E00-\u9FFF"), errorCode);
		assertTrue("tailored Han before currency", coll.compare(UnicodeString(u"\u4E00"), UnicodeString(u"$"), errorCode) < 0);
		errorCode.errIfFailureAndReset("unexpected exception for tailoring many characters at the end of symbols");
	}
}
...
void CollationAPITest::dump(UnicodeString msg, RuleBasedCollator* c, UErrorCode & status) 
{
	...
	logln(msg + " " + c->compare(bigone, littleone) + " s: " + c->getStrength() + " u: " + c->getAttribute(UCOL_CASE_FIRST, status));
}

void CollationAPITest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par */)
{
