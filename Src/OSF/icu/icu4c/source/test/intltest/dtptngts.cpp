...
void IntlTestDateTimePatternGeneratorAPI::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void IntlTestDateTimePatternGeneratorAPI::testAPI(/*char *par*/)
{
	...
	UnicodeString patternResults_zh_Hans_CN[] = {
		...
		CharsToUnicodeString("1999\\u5E741\\u6708"),              // 01: yMMM  -> yyyy\u5E74MMM (fixed expected
		                                                          // result per ticket:6626:)
		CharsToUnicodeString("1999/1/13"),                        // 02: yMd
		CharsToUnicodeString("1999\\u5E741\\u670813\\u65E5"),     // 03: yMMMd -> yyyy\u5E74MMMd\u65E5 (fixed
		                                                          // expected result per ticket:6626:)
		UnicodeString("1/13"),                                    // 04: Md
		CharsToUnicodeString("1\\u670813\\u65E5"),                // 05: MMMd  -> M\u6708d\u65E5 (fixed expected
		                                                          // result per ticket:6626:)
		CharsToUnicodeString("1\\u670813\\u65E5"),                // 06: MMMMd  -> M\u6708d\u65E5
	...
	UnicodeString patternResults_ru[] = {
		...
		UnicodeString("23:58"),                                   // 10: jjmm  -> HH:mm
		UnicodeString("58:59"),                                   // 11: mmss  -> mm:ss
		CharsToUnicodeString("\\u044F\\u043D\\u0432\\u0430\\u0440\\u044C 1999 \\u0433."), // 12: yyyyMMMM ->
		                                                                                  // LLLL y
	...
	if(dateReturned != expectedResult) {
		errln(UnicodeString("ERROR: Simple test addPattern failed!: d\'. von\' MMMM   Got: ") + dateReturned +
...

	{
		...
		UnicodeString expR = CharsToUnicodeString("1999\\u5E741\\u6708"); // fixed expected result per
		                                                                  // ticket:6626:
		Locale loc("ja");
...

	{
		...
		UnicodeString expR = UnicodeString("h:mm a v"); // avail formats has hm -> "h:mm a" (fixed expected
		                                                // result per ticket:6626:)
		Locale loc("en");
		DateTimePatternGenerator * patGen = DateTimePatternGenerator::createInstance(loc, status);
		if(U_FAILURE(status)) {
	...
	while(localeIndex < MAX_LOCALE) {
		...
		while(patternData[dataIndex].length() > 0) {
			...
			if(resultDate != patternResults[localeIndex][resultIndex]) {
				...
				errln(UnicodeString("\nERROR: Test various skeletons[") + (dataIndex-1) + UnicodeString(
	...
	conflictingStatus = test->addPattern(UnicodeString("MMMMMdd"), true, conflictingPattern, status); //duplicate
	                                                                                                  // pattern
...
void IntlTestDateTimePatternGeneratorAPI::testOptions(/*char *par*/)
{
	DTPtnGenOptionsData testData[] = {
		...
		{ "en@calendar=chinese",  "YYYY",  "Y(Y)",  UDATPG_MATCH_NO_OPTIONS },// not a good result, want r(Y) or
		                                                                    // r(U)
		...
		{ "zh@calendar=chinese",  "YYYY",  "YY\\u5E74",    UDATPG_MATCH_NO_OPTIONS },// not a good result, may
		                                                                         // want r(Y) or r(U)
	...
	for(; count-- > 0; ++testDataPtr) {
		...
		if(U_FAILURE(status)) {
			dataerrln("Unable to create DateTimePatternGenerator instance for locale(%s): %s", locale.getName(),
			    u_errorName(status));
			delete dtpgen;
			continue;
		}
		...
		if(U_SUCCESS(status)) {
			...
			for(; itemCount-- > 0; ++testDataPtr) {
				...
				for(lenIndx = 0; lenIndx < FIELD_LENGTHS_COUNT; lenIndx++) {
					...
					if(U_FAILURE(status)) {
						errln(
							"DateTimePatternGenerator getBestPattern for locale %s, skelChar %c skelLength %d fails: %s",
							locale.getName(),
							testDataPtr->patternChar,
							skelLen,
							u_errorName(status));
					}
					else if(pattern.length() <= 0) {
						errln(
							"DateTimePatternGenerator getBestPattern for locale %s, skelChar %c skelLength %d produces 0-length pattern",
							locale.getName(),
							testDataPtr->patternChar,
							skelLen);
					}
		...
		if(U_SUCCESS(status)) {
			...
			for(i = 0; i < len; i++) {
				...
				if(U_FAILURE(status)) {
					...
					errln("ERROR: getBestPattern %s, expected %s, got %s", testItems[i][0], testItems[i][1],
					    charResult);
				}
			}
		}
...
void IntlTestDateTimePatternGeneratorAPI::testJjMapping() {
	...
	// First test that j maps correctly by region in a locale for which we do not have data.
	{
		...
		if(U_FAILURE(status)) {
			dataerrln("FAIL: DateTimePatternGenerator::createInstance failed for locale %s: %s", testLocaleID,
			    u_errorName(status));
		}
		else {
			UnicodeString jPattern = dtpg->getBestPattern(jSkeleton, UDATPG_MATCH_ALL_FIELDS_LENGTH, status); // get
			                                                                                                  // pattern
			                                                                                                  // with
			                                                                                                  // h
			                                                                                                  // e.g.
			                                                                                                  ...
			                                                                                                  // a"
			if(U_FAILURE(status)) {
				errln("FAIL: DateTimePatternGenerator::getBestPattern locale %s, pattern j: %s", testLocaleID,
				    u_errorName(status));
			}
			else {
				UnicodeString jPatSkeleton = DateTimePatternGenerator::staticGetSkeleton(jPattern, status); // strip
				                                                                                            // literals,
				                                                                                            // get
				                                                                                            // e.g.
				                                                                                            // "ah"
				if(U_FAILURE(status)) {
					errln("FAIL: DateTimePatternGenerator::staticGetSkeleton locale %s: %s", testLocaleID,
					    u_errorName(status));
				}
				else if(jPatSkeleton.indexOf(u'h') < 0) { // expect US preferred cycle 'h', not H or
					                                  // other cycle
					errln("ERROR: DateTimePatternGenerator::getBestPattern locale %s, pattern j did not use 'h'",
					    testLocaleID);
				}
			}
		}
		...
		for(; *charPtr != (UChar)0; charPtr++) {
			if(jPatSkeleton.indexOf(*charPtr) >= 0) {
				if(shortPatSkeleton.indexOf(*charPtr) < 0) {
					...
					errln(
						"ERROR: locale %s (valid %s), expected j resolved char %s to occur in short time pattern '%s' for %s (best pattern: '%s')",
						localeID,
						validLoc,
						jcBuf,
						spBuf,
...
void IntlTestDateTimePatternGeneratorAPI::test20640_HourCyclArsEnNH() {
	...
	const struct TestCase {
		...
		{"cn_ZH", u"HH", u"HH:mm", UDAT_HOUR_CYCLE_23 }, // Desired & now actual behavior (does this fix
		                                                 // ICU-20653?)
...
void IntlTestDateTimePatternGeneratorAPI::testFallbackWithDefaultRootLocale() {
	...
	if(status != U_USING_DEFAULT_WARNING) {
		errln(
			"ERROR: expected createInstance to return U_USING_DEFAULT_WARNING for invalid locale and default root locale. Status: %s",
			u_errorName(status));
	}
