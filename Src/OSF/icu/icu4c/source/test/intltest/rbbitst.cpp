...
void RBBITest::runIndexedTest(int32_t index, bool exec, const char *& name, char * params)
{
...
void RBBITest::TestBug3818() 
{
	UErrorCode status = U_ZERO_ERROR;
	// Four Thai words...
	static const char16_t thaiWordData[] = {  0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0 };
...
void RBBITest::executeTest(TestParams * t, UErrorCode & status) {
	...
	//
	prevBP = static_cast<int32_t>(utext_nativeLength(t->textToBreak) + 2); // start with a phony value for the last
	                                                                       // break pos seen.
	bp = t->bi->last();
	...
	for(i = 0; i < utext_nativeLength(t->textToBreak); i++) {
		bool boundaryExpected = (t->getExpectedBreak(i) != 0);
		...
		switch(parseState) {
			...
			case PARSE_RULES:
			    if(testString.compare(charIdx-1, 8, u"</rules>") == 0) {
				    ...
				    if(U_SUCCESS(ec)) {
					    errln(
						    "file rbbitst.txt: %d - Expected, but did not get, a failure creating break iterator from rules.",
...
bool RBBITest::testCaseIsKnownIssue(const UnicodeString & testCase, const char * fileName) {
		...
		for(int32_t boundary = bi->first(); boundary != BreakIterator::DONE; boundary = bi->next()) {
			...
			for(int32_t nonBoundary = prev + 1; nonBoundary < boundary; nonBoundary++) {
				if(bi->isBoundary(nonBoundary)) {
					...
					errln(
						"%s:%d isBoundary(nonBoundary) failed. (loop, prev, nonBoundary, boundary) = (%d, %d, %d, %d)",
						__FILE__,
						__LINE__,
						loop,
						prev,
...
void RBBITest::RunMonkey(BreakIterator * bi, RBBIMonkeyKind &mk, const char * name, uint32_t seed,
    int32_t numIterations, bool useUText) {
#if !UCONFIG_NO_REGULAR_EXPRESSIONS
...
void RBBITest::TestEmoji() 
{
	...
	if(U_FAILURE(status)) {
		errln("%s:%d %s while opening emoji-test.txt", __FILE__, __LINE__, u_errorName(status)); // @sobolev @fix "%s:%s %s"-->"%s:%d %s"
		return;
	}
	logln("Opening data file %s\n", testFileName.data());
	int len;
...
void RBBITest::TestTableRedundancies() {
	...
	bool in8Bits = fwtbl->fFlags & RBBI_8BITS_ROWS;
...
void RBBITest::TestReverse(std::unique_ptr<RuleBasedBreakIterator>bi) {
	...
	typedef std::pair<bool, int32_t> Result;
