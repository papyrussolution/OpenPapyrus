...
private:
	...
	void CheckLocale(const Locale & locale, UNumberCompactStyle style,
	...
	void CheckLocaleWithCurrency(const Locale & locale, UNumberCompactStyle style, const char16_t * currency,
	    const ExpectedResult* expectedResults, int32_t expectedResultLength);
	void CheckExpectedResult(const CompactDecimalFormat* cdf, const ExpectedResult* expectedResult,
...
void CompactDecimalFormatTest::runIndexedTest(int32_t index, bool exec, const char *&name, char *) {
	if(exec) {
...
void CompactDecimalFormatTest::TestAPIVariants() {
	...
	if(actual != expected || pos.getEndIndex() != 3 || status != U_ZERO_ERROR) {
		errln(UnicodeString(
			    "Fail format(double,UnicodeString &,FieldPosition&,UErrorCode&): Expected: \"") + expected + "\", pos 3, status U_ZERO_ERROR; " +
	...
	if(actual != expected || pos.getEndIndex() != 3 || status != U_ZERO_ERROR) {
		errln(UnicodeString(
			    "Fail format(int32_t,UnicodeString &,FieldPosition&,UErrorCode&): Expected: \"") + expected + "\", first pos 3, status U_ZERO_ERROR; " +
		    "Got: \"" + actual + "\", pos " + pos.getEndIndex() + ", status " + u_errorName(status));
	}
	...
	if(actual != expected || pos.getEndIndex() != 3 || status != U_ZERO_ERROR) {
		errln(UnicodeString(
			    "Fail format(int32_t,UnicodeString &,FieldPosition&,UErrorCode&): Expected: \"") + expected + "\", pos 3, status U_ZERO_ERROR; " +
		    "Got: \"" + actual + "\", pos " + pos.getEndIndex() + ", status " + u_errorName(status));
	}
	...
	if(actual != expected || pos.getEndIndex() != 3 || status != U_ZERO_ERROR) {
		errln(UnicodeString(
			    "Fail format(int64_t,UnicodeString &,FieldPosition&,UErrorCode&): Expected: \"") + expected + "\", pos 3, status U_ZERO_ERROR; " +
		    "Got: \"" + actual + "\", pos " + pos.getEndIndex() + ", status " + u_errorName(status));
	}
...
void CompactDecimalFormatTest::CheckLocale(const Locale & locale,
...
void CompactDecimalFormatTest::CheckLocaleWithCurrency(const Locale & locale, UNumberCompactStyle style,
    const char16_t * currency, const ExpectedResult* expectedResults, int32_t expectedResultLength) 
{
	UErrorCode status = U_ZERO_ERROR;
	LocalPointer<CompactDecimalFormat> cdf(createCDFInstance(locale, style, status));
	if(U_FAILURE(status)) {
...
extern IntlTest * createCompactDecimalFormatTest() { return new CompactDecimalFormatTest(); }
