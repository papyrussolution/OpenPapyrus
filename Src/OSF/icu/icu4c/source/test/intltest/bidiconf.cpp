...
private:
    bool parseLevels(const char *&start);
    bool parseOrdering(const char *start);
    bool parseInputStringFromBiDiClasses(const char *&start);

    bool checkLevels(const UBiDiLevel actualLevels[], int32_t actualCount);
    bool checkOrdering(UBiDi *ubidi);
...
void BiDiConformanceTest::runIndexedTest(int32_t index, bool exec, const char *&name, char * /*par*/) {
    if(exec) {
...
bool BiDiConformanceTest::parseLevels(const char *&start) {
    directionBits=0;
    levelsCount=0;
...
bool BiDiConformanceTest::parseOrdering(const char *start) {
    orderingCount=0;
    while(*start!=0 && *(start=u_skipWhitespace(start))!=0 && *start!=';') {
        char *end;
        uint32_t value=(uint32_t)strtoul(start, &end, 10);
...
bool BiDiConformanceTest::parseInputStringFromBiDiClasses(const char *&start) {
    inputString.remove();
    /*
...
bool BiDiConformanceTest::checkLevels(const UBiDiLevel actualLevels[], int32_t actualCount) {
    bool isOk=TRUE;
...
bool BiDiConformanceTest::checkOrdering(UBiDi *ubidi) {
    bool isOk=TRUE;
