...
void JamoTest::runIndexedTest(int32_t index, bool exec,
...
void JamoTest::TestRealText() {
	...
	Transliterator* tarray[4] = { hangulJamo, jamoLatin, latinJamo, jamoHangul };
	CompoundTransliterator rt(tarray, 4);

	UnicodeString buf;
	int32_t total = 0;
	int32_t errors = 0;
	for(int32_t i = 0; i < WHAT_IS_UNICODE_length; ++i) {
		++total;
...
void JamoTest::expectAux(const UnicodeString & tag,
 ...
 */
UnicodeString JamoTest::nameToJamo(const UnicodeString & input) {
	if(NAME_JAMO == 0) {
 ...
 */
UnicodeString JamoTest::jamoToName(const UnicodeString & input) {
	if(NAME_JAMO == 0) {
		errln("Failed to create NAME_JAMO");
		return input; /* failure! */
	}
