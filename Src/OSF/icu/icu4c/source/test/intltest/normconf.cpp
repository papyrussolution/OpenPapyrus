...
void NormalizerConformanceTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/) 
{
	TESTCASE_AUTO_BEGIN;
...
FileStream * NormalizerConformanceTest::openNormalizationTestFile(const char * filename) 
{
	...
	if(U_SUCCESS(errorCode)) {
		strcpy(unidataPath, folder);
		strcat(unidataPath, U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING "data" U_FILE_SEP_STRING "unidata" U_FILE_SEP_STRING);
		strcat(unidataPath, filename);
		input = T_FileStream_open(unidataPath, "rb");
		if(input) {
			return input;
...
void NormalizerConformanceTest::TestConformance(FileStream * input, int32_t options) {
	...
	for(c = 0; c<=0x10ffff; quick ? c += 113 : ++c) {
		...
		if(checkConformance(fields, lineBuf, options, status)) {
			...
			++failCount;
			if(status == U_FILE_ACCESS_ERROR) {
				dataerrln("Something is wrong with the normalizer, skipping the rest of the test.: %s",
				    u_errorName(status));
				break;
			}
		}
...
namespace {
bool isNormalizedUTF8(const Normalizer2 * norm2, const UnicodeString & s, UErrorCode &errorCode) {
	if(norm2 == nullptr) {
		return true;
	}
	std::string s8;
 ...
 */
bool NormalizerConformanceTest::checkConformance(const UnicodeString * field,
    const char * line,
    int32_t options,
    UErrorCode & status) {
...
bool NormalizerConformanceTest::checkNorm(UNormalizationMode mode, int32_t options,
	...
	bool pass = TRUE;
	pass &= assertEquals("edits.hasChanges()", (bool)(s8 != out8), edits.hasChanges());
...
bool NormalizerConformanceTest::assertEqual(const char * op, const char * op2,
 ...
 */
bool NormalizerConformanceTest::hexsplit(const char * s, char delimiter,
		...
		for(;;) {
			...
			if(*t == 0) {
				if((i + 1) == outputLength) {
					...
					errln(UnicodeString("Missing field(s) in ",
					    "") + s + " only " + (i + 1) + " out of " + outputLength);
					return FALSE;
				}
			}
		}
