...
static int32_t toEnum(const Numberformattesttuple_EnumConversion * table,
    int32_t tableLength,
...
static void fromEnum(const Numberformattesttuple_EnumConversion * table,
    int32_t tableLength,
    int32_t val,
...
static void identVal(const UnicodeString & str, void * strPtr, UErrorCode & /*status*/) {
	*static_cast<UnicodeString *>(strPtr) = str;
}

static void identStr(const void * strPtr, UnicodeString & appendTo) {
	appendTo.append(*static_cast<const UnicodeString *>(strPtr));
}

static void strToLocale(const UnicodeString & str, void * localePtr, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
...
static void localeToStr(const void * localePtr, UnicodeString & appendTo) {
	appendTo.append(
		UnicodeString(
			static_cast<const Locale *>(localePtr)->getName()));
}

static void strToInt(const UnicodeString & str, void * intPtr, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
...
static void intToStr(const void * intPtr, UnicodeString & appendTo) {
	UChar buffer[20];
...
static void strToDouble(const UnicodeString & str, void * doublePtr, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
	CharString buffer;
...
static void doubleToStr(const void * doublePtr, UnicodeString & appendTo) {
	char buffer[256];
...
static void eRoundingToStr(const void * roundPtr, UnicodeString & appendTo) {
...
static void currencyUsageToStr(const void * currencyUsagePtr, UnicodeString & appendTo) {
...
static void ePadPositionToStr(const void * padPositionPtr, UnicodeString & appendTo) {
...
static void formatStyleToStr(const void * formatStylePtr, UnicodeString & appendTo) {
...
bool NumberFormatTestTuple::setField(ENumberFormatTestTupleField fieldId,
...
bool NumberFormatTestTuple::clearField(ENumberFormatTestTupleField fieldId,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
void NumberFormatTestTuple::clear() {
...
ENumberFormatTestTupleField NumberFormatTestTuple::getFieldByName(const UnicodeString & name) {
	CharString buffer;
...
const void * NumberFormatTestTuple::getFieldAddress(int32_t fieldId) const {
	return reinterpret_cast<const char *>(this) + gFieldData[fieldId].offset;
}

void * NumberFormatTestTuple::getMutableFieldAddress(int32_t fieldId) {
	return reinterpret_cast<char *>(this) + gFieldData[fieldId].offset;
}

void NumberFormatTestTuple::setFlag(int32_t fieldId, bool value) {
	void * flagAddr = reinterpret_cast<char *>(this) + gFieldData[fieldId].flagOffset;
	*static_cast<bool *>(flagAddr) = value;
}

bool NumberFormatTestTuple::isFlag(int32_t fieldId) const {
	const void * flagAddr = reinterpret_cast<const char *>(this) + gFieldData[fieldId].flagOffset;
	return *static_cast<const bool *>(flagAddr);
}
