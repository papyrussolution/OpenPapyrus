...
bool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString & msg, const Locale & l, bool possibleDataError)
{
	if(U_FAILURE(status)) {
		if(possibleDataError) {
			dataerrln(UnicodeString("FAIL: ", "") + msg
			    + UnicodeString(" failed, error ",
			    "") + UnicodeString(u_errorName(status), "") + UnicodeString(l.getName(), ""));
		}
		else {
...
bool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString & msg, const char * l, bool possibleDataError)
{
	if(U_FAILURE(status)) {
		if(possibleDataError) {
			dataerrln(UnicodeString("FAIL: ", "") + msg
...
bool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString & msg, bool possibleDataError)
{
	if(U_FAILURE(status)) {
		if(possibleDataError) {
			dataerrln(UnicodeString("FAIL: ", "") + msg
...
float NumberFormatRegressionTest::assignFloatValue(float returnfloat)
{
...
void NumberFormatRegressionTest::Test4061302(void)
{
	...
	logln(
		(UnicodeString)"Before set ==> Currency : " + currency +(UnicodeString)" Intl Currency : " + intlCurrency + (UnicodeString)" Monetary Decimal Separator : " +
		monDecSeparatorStr);
	...
	monDecSeparatorStr.append(monDecSeparator);
	logln(
		"After set ==> Currency : " + currency + " Intl Currency : " + intlCurrency + " Monetary Decimal Separator : " +
		monDecSeparatorStr);
...
void NumberFormatRegressionTest::Test4062486(void)
{
	...
	if(field.getBeginIndex() != 0 && field.getEndIndex() != 5)
		errln(UnicodeString("Format 1234.5 failed. Begin index: ") /*+ field.getBeginIndex() + " End index: " +
		                                                              field.getEndIndex()*/                                          );
	field.setBeginIndex(7);
	field.setEndIndex(4);
	if(field.getBeginIndex() != 7 && field.getEndIndex() != 4)
		errln("Set begin/end field indexes failed. Begin index: " /*+ field.getBeginIndex() + " End index: " +
		                                                             field.getEndIndex()*/                                          );
...
void NumberFormatRegressionTest::Test4162198(void)
{
 ...
 */
void NumberFormatRegressionTest::Test4162852(void)
{
	UErrorCode status = U_ZERO_ERROR;
	for(int32_t i = 0; i < 2; ++i) {
