...
void TimeZoneFormatTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
...
void TimeZoneFormatTest::TestTimeZoneRoundTrip() {
				...
				for(int32_t datidx = 0; datidx < nDates; datidx++) {
					...
					if(uprv_strcmp(PATTERNS[patidx], "V") == 0) {
						...
						if(U_FAILURE(status)) {
							...
							if(outtzid.compare(ETC_UNKNOWN, -1) == 0) {
								...
								logln((UnicodeString)"Canonical round trip failed (probably as expected); tz=" + *tzid
								    + ", locale=" + LOCALES[locidx].getName() + ", pattern=" +
								    PATTERNS[patidx]
						...
						if(isOffsetFormat || tzstr == localGMTString) {
							// Localized GMT or ISO: total offset (raw + dst) must be
							// preserved.
...
static bool isSpecialTimeRoundTripCase(const char * loc,
...
void TimeZoneFormatTest::TestTimeRoundTrip() {
	UErrorCode status = U_ZERO_ERROR;
...
void TimeZoneFormatTest::RunTimeRoundTripTests(int32_t threadNumber) {
	...
	while(gLocaleData->nextTest(locidx, patidx)) {
		...
		bool minutesOffset = contains(MINUTES_OFFSET, PATTERNS[patidx]);
		tzids->reset(status);
		const UnicodeString * tzid;
		...
		while((tzid = tzids->snext(status))) {
			...
			while(t < gLocaleData->END_TIME) {
				if(!tztAvail) {
					...
					if(delta < 0) {
						bool isDstDecession = tzt.getFrom()->getDSTSavings() > 0 &&
				...
				for(int32_t testidx = 0; testidx < testLen; testidx++) {
					...
					if(U_FAILURE(status)) {
						errln(
							(UnicodeString)"Parse failure for text=" + text + ", tzid=" + *tzid + ", locale=" +
							gLocaleData->locales[locidx].getName()
					...
					if(!bTimeMatch) {
						UnicodeString msg = (UnicodeString)"Time round trip failed for " + "tzid=" + *tzid
						    ...
						    + ", text=" + text + ", time=" + testTimes[testidx] + ", restime=" + parsedDate +
...
void TimeZoneFormatTest::TestAdoptDefaultThreadSafe() {
...
void TimeZoneFormatTest::TestParse() {
	const ParseTestData DATA[] = {
		...
		{"AQTST",           0,      "en",       UTZFMT_STYLE_SPECIFIC_LONG,
		 UTZFMT_PARSE_OPTION_ALL_STYLES | UTZFMT_PARSE_OPTION_TZ_DATABASE_ABBREVIATIONS, "Asia/Aqtobe",  5,
	...
	for(int32_t i = 0; DATA[i].text; i++) {
		...
		if(errMsg.length() > 0) {
			errln(
				(UnicodeString)"Fail: " + errMsg + " [text=" + DATA[i].text + ", pos=" + DATA[i].inPos + ", style=" + DATA[i].style +
				"]");
		}
	}
}

void TimeZoneFormatTest::TestISOFormat() {
...
void TimeZoneFormatTest::TestFormat() {
...
void TimeZoneFormatTest::TestFormatTZDBNames() {
	UDate dateJan = 1358208000000.0; // 2013-01-15T00:00:00Z
	UDate dateJul = 1373846400000.0; // 2013-07-15T00:00:00Z
...
void TimeZoneFormatTest::TestFormatCustomZone() {
	struct {
		const char * id;
		int32_t offset;
		const char * expected;
...
void TimeZoneFormatTest::TestFormatTZDBNamesAllZoneCoverage() {
	UErrorCode status = U_ZERO_ERROR;
...
void TimeZoneFormatTest::TestCentralTime() 
{
