...
#define EXHAUSTIVE(id, test) case id:                            \
	    ...
	    break
void TransliteratorRoundTripTest::runIndexedTest(int32_t index, bool exec,
...
public:
	...
	inline Transliterator* operator->() {
		return t;
	}

	inline operator const Transliterator*() const { return t; }
	inline operator Transliterator*() {
		return t;
	}
};
...
public:
	...
	virtual bool is(const UnicodeString & /*sourceString*/) const {
		return TRUE;
	}
};
...
public:
	...
	virtual bool is(const UnicodeString & sourceString) const override;
	int   getType(char16_t c) const;
};

bool LegalJamo::is(const UnicodeString & sourceString) const {
	int t;
...
public:
	...
	static bool isVowel(char16_t c);

	static bool isRho(char16_t c);
};

bool LegalGreek::is(const UnicodeString & sourceString) const {
	UnicodeString decomp;
	UErrorCode ec = U_ZERO_ERROR;
	Normalizer::decompose(sourceString, FALSE, 0, decomp, ec);
...
bool LegalGreek::isVowel(char16_t c) {
	switch(c) {
		case 0x03B1:
		case 0x03B5:
		case 0x03B7:
...
bool LegalGreek::isRho(char16_t c) {
	switch(c) {
		case 0x03C1:
		case 0x03A1:
		    return TRUE;
...
private:
	...
	static bool isSame(const UnicodeString & a, const UnicodeString & b);

	static bool isCamel(const UnicodeString & a);

	bool checkIrrelevants(Transliterator * t, const UnicodeString & irrelevants);

	void test2(bool quick, int32_t density);
...
bool RTTest::isSame(const UnicodeString & a, const UnicodeString & b) {
	if(a == b) return TRUE;
...
bool RTTest::isCamel(const UnicodeString & a) {
    ...
    int32_t density)
{
	...
	if(errorCount > 0) {
		...
		str[length] = 0;
		parent->errln("FAIL: %s errors: %d %s", str, errorCount, (errorCount > errorLimit ? " (at least!)" : " ")); // +
		                                                                                                            // ",
		                                                                                                            // see
		                                                                                                            // "
		                                                                                                            // +
...
bool RTTest::checkIrrelevants(Transliterator * t,
...
void RTTest::test2(bool quickRt, int32_t density) {
...
void TransliteratorRoundTripTest::TestHan() {
	...
	UnicodeSet allMarks(UNICODE_STRING_SIMPLE("[\\u0304\\u0301\\u030C\\u0300\\u0306]"), status); // look only for
	                                                                                             // Pinyin tone
	                                                                                             // marks, not all
	                                                                                             // marks (there are
	                                                                                             // some others in
	...
	bool roundtripOK = (target3.compare(target) == 0);
...
void TransliteratorRoundTripTest::TestArabic() {
	UnicodeString ARABIC(
		"[\\u060C\\u061B\\u061F\\u0621\\u0627-\\u063A\\u0641-\\u0655\\u0660-\\u066C\\u067E\\u0686\\u0698\\u06A4\\u06AD\\u06AF\\u06CB-\\u06CC\\u06F0-\\u06F9]",
		-1,
		US_INV);
	Legal * legal = new Legal();
...
bool LegalHebrew::is(const UnicodeString & sourceString) const {
...
void TransliteratorRoundTripTest::TestHebrew() {
	...
	test.test(UNICODE_STRING_SIMPLE("[a-zA-Z\\u02BC\\u02BB]"),
	    UNICODE_STRING_SIMPLE("[[[:hebrew:]-[\\u05BD\\uFB00-\\uFBFF]]&[:Age=4.0:]]"), "[\\u05F0\\u05F1\\u05F2]", this, quick, legal);
...
bool LegalIndic::is(const UnicodeString & sourceString) const {
...
void TransliteratorRoundTripTest::TestDevanagariLatin() {
	{
		...
		TransliteratorPointer t1(Transliterator::createInstance(
			    "[\\u0964-\\u0965\\u0981-\\u0983\\u0985-\\u098C\\u098F-\\u0990\\u0993-\\u09A8\\u09AA-\\u09B0\\u09B2\\u09B6-\\u09B9\\u09BC\\u09BE-\\u09C4\\u09C7-\\u09C8\\u09CB-\\u09CD\\u09D7\\u09DC-\\u09DD\\u09DF-\\u09E3\\u09E6-\\u09FA];NFD;Bengali-InterIndic;InterIndic-Gujarati;NFC;",
			    UTRANS_FORWARD,
			    parseError,
			    status));
...
static const char * const interIndicArray[] = {
	...
	"[\\u0904\\u0951-\\u0954\\u0943-\\u0949\\u094a\\u0962\\u0963\\u090D\\u090e\\u0911\\u0912\\u0929\\u0933\\u0934\\u0935\\u093d\\u0950\\u0958\\u0959\\u095a\\u095b\\u095e\\u097d]", /*roundtrip
	                                                                                                                                                                                   exclusions*/

	"DEVANAGARI-BENGALI", "[[:Devanagari:]-[\\u0970]]", "[:BENGALI:]",
	"[\\u0951-\\u0954\\u0951-\\u0954\\u09D7\\u090D\\u090e\\u0911\\u0912\\u0929\\u0933\\u0934\\u0935\\u093d\\u0950\\u0958\\u0959\\u095a\\u095b\\u095e\\u09f0\\u09f1\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                                                                                                                           exclusions*/

	"GURMUKHI-DEVANAGARI", "[:GURMUKHI:]", "[[:Devanagari:]-[\\u0970]]",
	"[\\u0904\\u0901\\u0902\\u0936\\u0933\\u0951-\\u0954\\u0902\\u0903\\u0943-\\u0949\\u094a\\u0962\\u0963\\u090B\\u090C\\u090D\\u090e\\u0911\\u0912\\u0934\\u0937\\u093D\\u0950\\u0960\\u0961\\u097d]", /*roundtrip
	                                                                                                                                                                                                        exclusions*/

	"DEVANAGARI-GURMUKHI", "[[:Devanagari:]-[\\u0970]]", "[:GURMUKHI:]",
	"[\\u0904\\u0A02\\u0946\\u0A5C\\u0951-\\u0954\\u0A70\\u0A71\\u090B\\u090C\\u090D\\u090e\\u0911\\u0912\\u0934\\u0937\\u093D\\u0950\\u0960\\u0961\\u0a72\\u0a73\\u0a74]", /*roundtrip
	                                                                                                                                                                           exclusions*/
	...
	"[\\u0904\\u0943-\\u094a\\u0962\\u0963\\u0951-\\u0954\\u0950\\u090D\\u090e\\u0912\\u0911\\u0931\\u0935\\u097d]", /*roundtrip
	                                                                                                                    exclusions*/
	...
	"[\\u0901\\u0904\\u093c\\u0943-\\u094a\\u0951-\\u0954\\u0962\\u0963\\u090B\\u090C\\u090D\\u0911\\u0916\\u0917\\u0918\\u091B\\u091D\\u0920\\u0921\\u0922\\u0925\\u0926\\u0927\\u092B\\u092C\\u092D\\u0936\\u093d\\u0950[\\u0958-\\u0961]\\u097d]", /*roundtrip
	                                                                                                                                                                                                                                                     exclusions*/
	...
	"[\\u0904\\u093c\\u0950\\u0945\\u0949\\u0951-\\u0954\\u0962\\u0963\\u090D\\u0911\\u093d\\u0929\\u0934[\\u0958-\\u095f]\\u097d]", /*roundtrip
	                                                                                                                                    exclusions*/
	...
	"[\\u0901\\u0904\\u0946\\u093c\\u0950\\u0945\\u0949\\u0951-\\u0954\\u0962\\u0963\\u0950\\u090D\\u0911\\u093d\\u0929\\u0934[\\u0958-\\u095f]\\u097d]", /*roundtrip
	                                                                                                                                                         exclusions*/

	"DEVANAGARI-KANNADA", "[[:Devanagari:]-[\\u0970]]", "[:KANNADA:]",
	"[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0cde\\u0cd5\\u0cd6\\u0950\\u090D\\u0911\\u093d\\u0929\\u0934[\\u0958-\\u095f]]", /*roundtrip
	                                                                                                                         exclusions*/

	"MALAYALAM-DEVANAGARI", "[:MALAYALAM:]", "[[:Devanagari:]-[\\u0970]]",
	"[\\u0901\\u0904\\u094a\\u094b\\u094c\\u093c\\u0950\\u0944\\u0945\\u0949\\u0951-\\u0954\\u0962\\u0963\\u090D\\u0911\\u093d\\u0929\\u0934[\\u0958-\\u095f]\\u097d]", /*roundtrip
	                                                                                                                                                                       exclusions*/
	...
	"[\\u0981\\u0982\\u09b6\\u09e2\\u09e3\\u09c3\\u09c4\\u09d7\\u098B\\u098C\\u09B7\\u09E0\\u09E1\\u09F0\\u09F1\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                                                                        exclusions*/

	"BENGALI-GURMUKHI", "[:BENGALI:]", "[:GURMUKHI:]",
	"[\\u0A02\\u0a5c\\u0a47\\u0a70\\u0a71\\u0A33\\u0A35\\u0A59\\u0A5A\\u0A5B\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip
	                                                                                                           exclusions*/
	...
	"[\\u0981\\u09bc\\u09c3\\u09c4\\u09e2\\u09e3\\u09f0\\u09f1\\u098B\\u098C\\u0996\\u0997\\u0998\\u099B\\u099D\\u09A0\\u09A1\\u09A2\\u09A5\\u09A6\\u09A7\\u09AB\\u09AC\\u09AD\\u09B6\\u09DC\\u09DD\\u09DF\\u09E0\\u09E1\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                                                                                                                                                                                 exclusions*/

	"BENGALI-Tamil", "[:BENGALI:]", "[:tamil:]",
	"[\\u0bc6\\u0bc7\\u0bca\\u0B8E\\u0B92\\u0BA9\\u0BB1\\u0BB3\\u0BB4\\u0BB5\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip
	                                                                                                    exclusions*/

	"Telugu-BENGALI", "[:telugu:]", "[:BENGALI:]",
	"[\\u09e2\\u09e3\\u09bc\\u09d7\\u09f0\\u09f1\\u09dc\\u09dd\\u09df\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                              exclusions*/
	...
	"[\\u0981\\u09e2\\u09e3\\u09bc\\u09d7\\u09dc\\u09dd\\u09df\\u09f0\\u09f1\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                                     exclusions*/

	"BENGALI-KANNADA", "[:BENGALI:]", "[:KANNADA:]",
	"[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0cc6\\u0cca\\u0cd5\\u0cd6\\u0cc7\\u0C8E\\u0C92\\u0CB1\\u0cb3\\u0cb5\\u0cde]", /*roundtrip
	                                                                                                                      exclusions*/

	"MALAYALAM-BENGALI", "[:MALAYALAM:]", "[:BENGALI:]",
	"[\\u0981\\u09e2\\u09e3\\u09bc\\u09c4\\u09f0\\u09f1\\u09dc\\u09dd\\u09df\\u09dc\\u09dd\\u09df\\u09f2-\\u09fa\\u09ce]", /*roundtrip
	                                                                                                                          exclusions*/
	...
	"[\\u0A02\\u0ab3\\u0ab6\\u0A70\\u0a71\\u0a82\\u0a83\\u0ac3\\u0ac4\\u0ac5\\u0ac9\\u0a5c\\u0a72\\u0a73\\u0a74\\u0a8b\\u0a8d\\u0a91\\u0abd]", /*roundtrip
	                                                                                                                                              exclusions*/

	"GURMUKHI-GUJARATI", "[:GURMUKHI:]", "[:GUJARATI:]",
	"[\\u0a5c\\u0A70\\u0a71\\u0a72\\u0a73\\u0a74\\u0a82\\u0a83\\u0a8b\\u0a8c\\u0a8d\\u0a91\\u0ab3\\u0ab6\\u0ab7\\u0abd\\u0ac3\\u0ac4\\u0ac5\\u0ac9\\u0ad0\\u0ae0\\u0ae1]", /*roundtrip
	                                                                                                                                                                          exclusions*/

	"ORIYA-GURMUKHI", "[:ORIYA:]", "[:GURMUKHI:]",
	"[\\u0A01\\u0A02\\u0a5c\\u0a21\\u0a47\\u0a71\\u0b02\\u0b03\\u0b33\\u0b36\\u0b43\\u0b56\\u0b57\\u0B0B\\u0B0C\\u0B37\\u0B3D\\u0B5F\\u0B60\\u0B61\\u0a35\\u0a72\\u0a73\\u0a74]", /*roundtrip
	                                                                                                                                                                                 exclusions*/

	"GURMUKHI-ORIYA", "[:GURMUKHI:]", "[:ORIYA:]",
	"[\\u0b01\\u0b02\\u0b03\\u0b33\\u0b36\\u0b43\\u0b56\\u0b57\\u0B0B\\u0B0C\\u0B37\\u0B3D\\u0B5F\\u0B60\\u0B61\\u0b70\\u0b71]", /*roundtrip
	                                                                                                                                exclusions*/

	"TAMIL-GURMUKHI", "[:TAMIL:]", "[:GURMUKHI:]",
	"[\\u0A01\\u0A02\\u0a33\\u0a36\\u0a3c\\u0a70\\u0a71\\u0a47\\u0A16\\u0A17\\u0A18\\u0A1B\\u0A1D\\u0A20\\u0A21\\u0A22\\u0A25\\u0A26\\u0A27\\u0A2B\\u0A2C\\u0A2D\\u0A59\\u0A5A\\u0A5B\\u0A5C\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip
	                                                                                                                                                                                                                           exclusions*/

	"GURMUKHI-TAMIL", "[:GURMUKHI:]", "[:TAMIL:]",
	"[\\u0b82\\u0bc6\\u0bca\\u0bd7\\u0bb7\\u0bb3\\u0b83\\u0B8E\\u0B92\\u0BA9\\u0BB1\\u0BB4\\u0bb6\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip
	                                                                                                                         exclusions*/

	"TELUGU-GURMUKHI", "[:TELUGU:]", "[:GURMUKHI:]",
	"[\\u0A02\\u0a33\\u0a36\\u0a3c\\u0a70\\u0a71\\u0A59\\u0A5A\\u0A5B\\u0A5C\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip
	                                                                                                           exclusions*/

	"GURMUKHI-TELUGU", "[:GURMUKHI:]", "[:TELUGU:]",
	"[\\u0c01\\u0c02\\u0c03\\u0c33\\u0c36\\u0c44\\u0c43\\u0c46\\u0c4a\\u0c56\\u0c55\\u0C0B\\u0C0C\\u0C0E\\u0C12\\u0C31\\u0C37\\u0C60\\u0C61]", /*roundtrip
	                                                                                                                                              exclusions*/

	"KANNADA-GURMUKHI", "[:KANNADA:]", "[:GURMUKHI:]",
	"[\\u0A01\\u0A02\\u0a33\\u0a36\\u0a3c\\u0a70\\u0a71\\u0A59\\u0A5A\\u0A5B\\u0A5C\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip
	                                                                                                                  exclusions*/

	"GURMUKHI-KANNADA", "[:GURMUKHI:]", "[:KANNADA:]",
	"[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0c82\\u0c83\\u0cb3\\u0cb6\\u0cc4\\u0cc3\\u0cc6\\u0cca\\u0cd5\\u0cd6\\u0C8B\\u0C8C\\u0C8E\\u0C92\\u0CB1\\u0CB7\\u0cbd\\u0CE0\\u0CE1\\u0cde]", /*roundtrip
	                                                                                                                                                                                     exclusions*/

	"MALAYALAM-GURMUKHI", "[:MALAYALAM:]", "[:GURMUKHI:]",
	"[\\u0A01\\u0A02\\u0a4b\\u0a4c\\u0a33\\u0a36\\u0a3c\\u0a70\\u0a71\\u0A59\\u0A5A\\u0A5B\\u0A5C\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip
	                                                                                                                                exclusions*/

	"GURMUKHI-MALAYALAM", "[:GURMUKHI:]", "[:MALAYALAM:]",
	"[\\u0d02\\u0d03\\u0d33\\u0d36\\u0d43\\u0d46\\u0d4a\\u0d4c\\u0d57\\u0D0B\\u0D0C\\u0D0E\\u0D12\\u0D31\\u0D34\\u0D37\\u0D60\\u0D61]", /*roundtrip
	                                                                                                                                       exclusions*/
	...
	"[\\u0A81\\u0a8c\\u0abc\\u0ac3\\u0Ac4\\u0Ac5\\u0Ac9\\u0Ac7\\u0A8B\\u0A8D\\u0A91\\u0A96\\u0A97\\u0A98\\u0A9B\\u0A9D\\u0AA0\\u0AA1\\u0AA2\\u0AA5\\u0AA6\\u0AA7\\u0AAB\\u0AAC\\u0AAD\\u0AB6\\u0ABD\\u0AD0\\u0AE0\\u0AE1]", /*roundtrip
	                                                                                                                                                                                                                           exclusions*/
	...
	"[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0cc6\\u0cca\\u0cd5\\u0cd6\\u0C8C\\u0C8E\\u0C92\\u0CB1\\u0CDE\\u0CE1]", /*roundtrip
	                                                                                                               exclusions*/
	...
	"[\\u0B01\\u0b3c\\u0b43\\u0b56\\u0B0B\\u0B0C\\u0B16\\u0B17\\u0B18\\u0B1B\\u0B1D\\u0B20\\u0B21\\u0B22\\u0B25\\u0B26\\u0B27\\u0B2B\\u0B2C\\u0B2D\\u0B36\\u0B3D\\u0B5C\\u0B5D\\u0B5F\\u0B60\\u0B61\\u0b70\\u0b71]", /*roundtrip
	                                                                                                                                                                                                                    exclusions*/

	"ORIYA-TAMIL", "[:ORIYA:]", "[:TAMIL:]",
	"[\\u0bc6\\u0bca\\u0bc7\\u0B8E\\u0B92\\u0BA9\\u0BB1\\u0BB4\\u0BB5\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip
	                                                                                             exclusions*/
	...
	"[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0cc4\\u0cc6\\u0cca\\u0cd5\\u0cc7\\u0C8E\\u0C92\\u0CB1\\u0CB5\\u0CDE]", /*roundtrip
	                                                                                                               exclusions*/
	...
	"[\\u0C01\\u0c43\\u0c44\\u0c46\\u0c47\\u0c55\\u0c56\\u0c66\\u0C0B\\u0C0C\\u0C16\\u0C17\\u0C18\\u0C1B\\u0C1D\\u0C20\\u0C21\\u0C22\\u0C25\\u0C26\\u0C27\\u0C2B\\u0C2C\\u0C2D\\u0C36\\u0C60\\u0C61]", /*roundtrip
	                                                                                                                                                                                                      exclusions*/
	...
	"[\\u0cc3\\u0cc4\\u0cc6\\u0cc7\\u0cd5\\u0cd6\\u0C8B\\u0C8C\\u0C96\\u0C97\\u0C98\\u0C9B\\u0C9D\\u0CA0\\u0CA1\\u0CA2\\u0CA5\\u0CA6\\u0CA7\\u0CAB\\u0CAC\\u0CAD\\u0CB6\\u0cbc\\u0cbd\\u0CDE\\u0CE0\\u0CE1]", /*roundtrip
	                                                                                                                                                                                                             exclusions*/
	...
	"[\\u0d43\\u0d12\\u0D0B\\u0D0C\\u0D16\\u0D17\\u0D18\\u0D1B\\u0D1D\\u0D20\\u0D21\\u0D22\\u0D25\\u0D26\\u0D27\\u0D2B\\u0D2C\\u0D2D\\u0D36\\u0D60\\u0D61]", /*roundtrip
	                                                                                                                                                            exclusions*/
