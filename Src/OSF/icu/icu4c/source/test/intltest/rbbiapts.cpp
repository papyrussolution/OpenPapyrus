...
void RBBIAPITest::TestCloneEquals()
{
	...
	RuleBasedBreakIterator* bi1     = (RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(Locale::getDefault(),
		status);
	RuleBasedBreakIterator* biequal = (RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(Locale::getDefault(),
		status);
	RuleBasedBreakIterator* bi3     = (RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(Locale::getDefault(),
		status);
...
void RBBIAPITest::TestGetSetAdoptText()
{
	...
	LocalPointer<RuleBasedBreakIterator> charIter1((RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(
		    Locale::getDefault(),
		    status));
	LocalPointer<RuleBasedBreakIterator> wordIter1((RuleBasedBreakIterator*)RuleBasedBreakIterator::createWordInstance(
		    Locale::getDefault(),
		    status));
	if(status.isFailure()) {
	...
	if(wordIter1->current() != 0)
		errln(
			(UnicodeString)"ERROR:1 setText did not set the iteration position to the beginning of the text, it is" + wordIter1->current() +
			(UnicodeString)"\n");
	...
	if(wordIter1->current() != 0)
		errln(
			(UnicodeString)"ERROR:2 setText did not reset the iteration position to the beginning of the text, it is" + wordIter1->current() +
			(UnicodeString)"\n");
...
void RBBIAPITest::TestIteration()
{
	...
	RuleBasedBreakIterator* bi  =
	    (RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(Locale::getDefault(), status);
...
void RBBIAPITest::TestRegistration() 
{
	...
	{
		bool unreg = BreakIterator::unregister(key, status);
		if(!unreg) {
...
void RBBIAPITest::TestFilteredBreakIteratorBuilder() {
	...
	const UnicodeString text(
		"In the meantime Mr. Weston arrived with his small ship, which he had now recovered. Capt. Gorges, who informed the Sgt. here that one purpose of his going east was to meet with Mr. Weston, took this opportunity to call him to account for some abuses he had to lay to his charge.");            // (William
	                                                                                                                                                                                                                                                                                                              // Bradford,
	                                                                                                                                                                                                                                                                                                              // public
	                                                                                                                                                                                                                                                                                                              // domain.
	                                                                                                                                                                                                                                                                                                              // http://catalog.hathitrust.org/Record/008651224
	                                                                                                                                                                                                                                                                                                              // )
	                                                                                                                                                                                                                                                                                                              // -
	                                                                                                                                                                                                                                                                                                              // edited.
...
void RBBIAPITest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void RBBIAPITest::doBoundaryTest(BreakIterator& bi, UnicodeString & text, int32_t * boundaries) {
	...
	for(int32_t i = 0; i < text.length(); i++) {
		...
		if(i == boundaries[p]) {
			if(!isB)
				errln((UnicodeString)"Wrong result from isBoundary() for " + i +
				    (UnicodeString)": expected true, got false");
			p++;
		}
		else {
			if(isB)
				errln((UnicodeString)"Wrong result from isBoundary() for " + i +
				    (UnicodeString)": expected false, got true");
		}
	}
}
