...
void TimeZoneOffsetLocalTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
...
void TimeZoneOffsetLocalTest::TestGetOffsetAroundTransition() {
	...
	// Expected offsets by void getOffset(UDate date, bool local, int32_t& rawOffset,
	// int32_t& dstOffset, UErrorCode & ec) with local=TRUE
	// or void getOffsetFromLocal(UDate date, UTimeZoneLocalOption nonExistingTimeOpt, UTimeZoneLocalOption
	// duplicatedTimeOpt,
	...
	// Test getOffset(UDate date, bool local, int32_t& rawOffset,
	// int32_t& dstOffset, UErrorCode & ec) with local = TRUE
	for(int32_t i = 0; i < NUM_TIMEZONES; i++) {
	...
	// Test getOffsetFromLocal(UDate date, UTimeZoneLocalOption nonExistingTimeOpt, UTimeZoneLocalOption
	// duplicatedTimeOpt,
	...
	for(int32_t i = 0; i < NUM_TIMEZONES; i++) {
		for(int m = 0; m < NUM_DATES; m++) {
			...
			if(U_FAILURE(status)) {
				errln(
					(UnicodeString)"getOffsetFromLocal with UCAL_TZ_LOCAL_STANDARD_FORMER/UCAL_TZ_LOCAL_STANDARD_LATTER failed for TESTZONES[" + i +
					"]");
			}
			else if(rawOffset != OFFSETS2[m][0] || dstOffset != OFFSETS2[m][1]) {
				dateStr.remove();
	...
	for(int32_t i = 0; i < NUM_TIMEZONES; i++) {
		for(int m = 0; m < NUM_DATES; m++) {
			...
			if(U_FAILURE(status)) {
				errln(
					(UnicodeString)"getOffsetFromLocal with UCAL_TZ_LOCAL_DAYLIGHT_LATTER/UCAL_TZ_LOCAL_DAYLIGHT_FORMER failed for TESTZONES[" + i +
					"]");
			}
	...
	for(int32_t i = 0; i < NUM_TIMEZONES; i++) {
		for(int m = 0; m < NUM_DATES; m++) {
			...
			if(U_FAILURE(status)) {
				errln(
					(UnicodeString)"getOffsetFromLocal with UCAL_TZ_LOCAL_FORMER/UCAL_TZ_LOCAL_LATTER failed for TESTZONES[" + i +
					"]");
			}
			else if(rawOffset != OFFSETS2[m][0] || dstOffset != OFFSETS2[m][1]) {
				dateStr.remove();
