...
void DecimalFormatTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
const char * DecimalFormatTest::getPath(char * buffer, const char * filename) {
	UErrorCode status = U_ZERO_ERROR;
...
void DecimalFormatTest::DataDrivenTests() {
	...
	RegexMatcher formatLineMat(UnicodeString(
		    ...
		    "(ceiling|floor|down|up|halfeven|halfdown|halfup|default|unnecessary)\\s+" // Capture group 2:
	                                                                                       // Rounding Mode
