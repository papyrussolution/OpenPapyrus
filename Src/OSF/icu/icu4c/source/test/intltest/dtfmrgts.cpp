...
void DateFormatRegressionTest::Test4106807(void)
{
	...
	for(int32_t i = 0; i < 5; i++) {
		...
		const UnicodeString dateString(strings[i]);
		//try {
...
{
	UErrorCode status = U_ZERO_ERROR;
	const UnicodeString dateFormat("MM/dd/yy HH:mm:ss zzz");
 ...
 */
void DateFormatRegressionTest::Test4162071(void)
{
...
void DateFormatRegressionTest::Test4182066() 
{
	...
	int32_t STRINGS_COUNT = SIZEOFARRAYi(STRINGS);
	...
	for(int32_t i = 0; i<STRINGS_COUNT; ++i) {
		const UnicodeString str(STRINGS[i]);
		...
		if(expected == actual) {
			...
			out.append("FAIL: " + str + " => " + actStr + ", expected " + expStr + "\n");
			pass = FALSE;
		}
	}
	if(pass) {
 ...
 */
void DateFormatRegressionTest::Test4210209() 
{
	UErrorCode status = U_ZERO_ERROR;
	...
	UnicodeString s, ss;
	fmt.format(d, s);
	logln(disp.format(d, ss.remove()) + " f> " + pattern + " => \"" + s + "\"");
	ParsePosition pos(0);
	d = fmt.parse(s, pos);
	logln(UnicodeString("\"") + s + "\" p> " + pattern + " => " + disp.format(d, ss.remove()));
	logln(UnicodeString("Parse pos = ") + pos.getIndex() + ", error pos = " + pos.getErrorIndex());
	...
	logln(UnicodeString("Attempt to set Calendar to Feb 29 2000: ") + disp.format(cal.getTime(status), ss.remove()));
	if(U_FAILURE(status)) {
...
void DateFormatRegressionTest::Test714(void)
{
	...
	UnicodeString s;
	UnicodeString tests("7:25:43 AM");
	UErrorCode status = U_ZERO_ERROR;
	fmt->format(d, s);
	if(U_FAILURE(status)) {
...
public:
	...
	Test1684Data(int32_t xyear, int32_t xmonth, int32_t xdate, int32_t xwomyear, int32_t xwommon, int32_t xwom, int32_t xdow,
	    const char * xdata, const char * xnormalized) : year(xyear), month(xmonth-1), date(xdate), womyear(xwomyear),
		wommon(xwommon-1), wom(xwom), dow(xdow), data(xdata, ""), normalized((xnormalized==NULL) ? xdata : xnormalized, "")
	{
	}
};
...
#define kTest1684Count  SIZEOFARRAYi(tests)
...
void DateFormatRegressionTest::TestT10619() {
	...
	const TestDateFormatLeniencyItem items[] = {
		//locale    leniency    parse String                    pattern                             expected
		// result
		...
		{ "en",     true,       UnicodeString("6 Jan 08 2008"), UnicodeString("eee MMM dd yyyy"),
		  UnicodeString("Sat Jan 08 2008") },
		...
		for(itemPtr = items; itemPtr->locale != NULL; itemPtr++) {
			...
			if(pos.getErrorIndex() > -1) {
				if(itemPtr->expectedResult.length() != 0) {
					errln(
						"error: unexpected error - " + itemPtr->parseString + " - error index " + pos.getErrorIndex() +
...
void DateFormatRegressionTest::TestT10855() {
	...
	while(items[x].pattern.length() > 0) {
		...
		if(position.getErrorIndex() != -1 || position.getIndex() == items[x].initialParsePos) {
			...
			errln("Parse Succeeded...should have failed. Index is %d - ErrorIndex is %d",
			    position.getIndex(),
