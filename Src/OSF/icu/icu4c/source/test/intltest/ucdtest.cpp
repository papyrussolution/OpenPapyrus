...
void UnicodeTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	if(exec) {
...
static int32_t getTokenIndex(const char * const tokens[], int32_t countTokens, const char * s) 
{
	const char * t, * z;
	int32_t i, j;
...
U_CFUNC void U_CALLCONV derivedPropsLineFn(void * context, char * fields[][2], int32_t /* fieldCount */, UErrorCode * pErrorCode)
{
...
bool UnicodeTest::compareUSets(const UnicodeSet &a, const UnicodeSet &b,
...
void UnicodeTest::TestScriptMetadata() {
	...
	for(int32_t sci = 0; sci < USCRIPT_CODE_LIMIT; ++sci) {
		...
		if(usage == USCRIPT_USAGE_NOT_ENCODED) {
			...
			assertEquals("RTL vs. set", (bool)rtl.contains(firstChar), (bool)uscript_isRightToLeft(sc));
			assertEquals("cased vs. set", (bool)cased.contains(firstChar), (bool)uscript_isCased(sc));
			assertEquals("encoded, has characters", (bool)(sc == charScript), (bool)(!scriptSet.isEmpty()));
