...
bool checkEqual(const PluralRules &test, char * result, int32_t max);
bool testEquality(const PluralRules &test);

// This is an API test, not a unit test.  It doesn't test very many cases, and doesn't
// try to test the full functionality.  It just calls each function in the class and
// verifies that it works on a basic level.

void PluralRulesTest::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
...
void PluralRulesTest::assertRuleValue(const UnicodeString & rule, double expected) {
	assertRuleKeyValue("a:" + rule, "a", expected);
}

void PluralRulesTest::assertRuleKeyValue(const UnicodeString & rule,
			...
			for(int32_t j = 0; j < count; ++j) {
				if(values[j] == UPLRULES_NO_UNIQUE_VALUE) {
					...
					if(*keyword != resultKeyword) {
						errln(
							"file %s, line %d, Locale %s, sample for keyword \"%s\":  %g, select(%g) returns keyword \"%s\"",
							__FILE__,
							__LINE__,
							locales[i].getName(),
							US(*keyword).cstr(),
		...
		while(NULL != (keyword = keywords->snext(status))) {
			...
			for(int32_t j = 0; j < count; ++j) {
				if(values[j] == UPLRULES_NO_UNIQUE_VALUE_DECIMAL) {
					...
					// }
					if(*keyword != resultKeyword) {
						if(values[j].exponent == 0 ||
						    !logKnownIssue("21714", "PluralRules::select treats 1c6 as 1")) {
							UnicodeString valueString(values[j].toString());
							char valueBuf[16];
							valueString.extract(0, valueString.length(), valueBuf, sizeof(valueBuf));
							errln(
								"file %s, line %d, Locale %s, sample for keyword \"%s\":  %s, select(%s) returns keyword \"%s\"",
								__FILE__,
								__LINE__,
								locales[i].getName(),
								US(*keyword).cstr(),
...
void PluralRulesTest::testSamplesWithExponent() {
	...
	checkNewSamples(description,
	    test,
	    u"other",
	    u"@integer 2~17, 100, 1000, 10000, 100000, 2e5, 3e5, 4e5, 5e5, 6e5, 7e5, …",
	    FixedDecimal(2));
...
void PluralRulesTest::testSamplesWithCompactNotation() {
	...
	checkNewSamples(description,
	    test,
	    u"other",
	    u"@integer 2~17, 100, 1000, 10000, 100000, 2c5, 3c5, 4c5, 5c5, 6c5, 7c5, …",
	    FixedDecimal(2));
...
void PluralRulesTest::checkNewSamples(UnicodeString description,
	...
	if(!(firstInRange == actualFirstSample)) {
		...
		snprintf(errMsg,
		    sizeof(errMsg),
		    "First parsed sample FixedDecimal not equal to expected for samples: %s in rule string: %s\n",
		    descCstr(),
		    samplesCstr());
		errln(errMsg);
	}
...
void PluralRulesTest::testGetAllKeywordValues() {
	const char * data[] = {
...
void PluralRulesTest::testScientificPluralKeyword() {
...
void PluralRulesTest::testCompactDecimalPluralKeyword() {
...
void PluralRulesTest::testDoubleValue() {
...
void PluralRulesTest::testLongValue() {
...
UnicodeString PluralRulesTest::getPluralKeyword(const LocalPointer<PluralRules> &rules,
    Locale locale,
    double number,
    const char16_t* skeleton) {
...
void PluralRulesTest::testLocaleExtension() {
	...
	if(errorCode.errIfFailureAndReset("PluralRules::forLocale()")) {
		return;
	}
