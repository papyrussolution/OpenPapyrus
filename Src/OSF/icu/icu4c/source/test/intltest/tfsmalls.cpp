...
/*static bool chkstatus( UErrorCode & status, char * msg = NULL )
   {
    bool ok = (status == U_ZERO_ERROR);
    if(!ok) it_errln( msg );
    return ok;
   }*/
...
void TestFormatSmallClasses::runIndexedTest(int32_t index, bool exec, const char *& name, char * /*par*/)
{
	switch(index) {
		case 0: name = "pp";
