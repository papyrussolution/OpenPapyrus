...
static bool assertEqualWithAttribute(const SearchData search,
...
static void TestPattern(void)
{
	...
	status = U_ZERO_ERROR;
	u_unescape(PATTERN_[0].text, text, 128);
	u_unescape(PATTERN_[0].pattern, pattern, 32);
	ucol_setStrength(EN_US_, PATTERN_[0].strength);
	strsrch = usearch_openFromCollator(pattern, -1, text, -1, EN_US_, NULL, &status);
	if(status == U_FILE_ACCESS_ERROR) {
		log_data_err("Is your data around?\n");
		return;
	}
	...
	if(!assertEqualWithUStringSearch(strsrch, PATTERN_[0])) {
		goto ENDTESTPATTERN;
	}
	u_unescape(PATTERN_[1].pattern, pattern, 32);
	...
	if(!assertEqualWithUStringSearch(strsrch, PATTERN_[1])) {
		goto ENDTESTPATTERN;
	}
	u_unescape(PATTERN_[0].pattern, pattern, 32);
	usearch_setPattern(strsrch, pattern, -1, &status);
...
static void TestForwardBackward() {
	UErrorCode status = U_ZERO_ERROR;
	UCollator * coll = NULL;
					...
					if(U_SUCCESS(status) ) {
						...
						while(true) {
							...
							if(nextOffsetPtr < limitOffsetPtr) {
								...
								log_err(
									"error, locale %s, usearch_next returned more matches than expected\n",
									tuscItemPtr->locale);
							}
						}
						if(U_FAILURE(status) ) {
						...
						while(true) {
							...
							if(nextOffsetPtr > limitOffsetPtr) {
								...
								log_err(
									"error, locale %s, usearch_previous returned more matches than expected\n",
									tuscItemPtr->locale);
							}
						}
						if(U_FAILURE(status) ) {
...
void addSearchTest(TestNode** root)
{
	...
	addTest(root, &TestCompositeBoundaries, "tscoll/usrchtst/TestCompositeBoundaries");
	addTest(root, &TestGetSetOffset, "tscoll/usrchtst/TestGetSetOffset");
	addTest(root, &TestGetSetAttribute, "tscoll/usrchtst/TestGetSetAttribute");
	...
	addTest(root, &TestStrengthCanonical, "tscoll/usrchtst/TestStrengthCanonical");
	addTest(root, &TestBreakIteratorCanonical, "tscoll/usrchtst/TestBreakIteratorCanonical");
	addTest(root, &TestVariableCanonical, "tscoll/usrchtst/TestVariableCanonical");
	addTest(root, &TestOverlapCanonical, "tscoll/usrchtst/TestOverlapCanonical");
	addTest(root, &TestCollatorCanonical, "tscoll/usrchtst/TestCollatorCanonical");
	addTest(root, &TestPatternCanonical, "tscoll/usrchtst/TestPatternCanonical");
	addTest(root, &TestTextCanonical, "tscoll/usrchtst/TestTextCanonical");
	addTest(root, &TestCompositeBoundariesCanonical, "tscoll/usrchtst/TestCompositeBoundariesCanonical");
	addTest(root, &TestGetSetOffsetCanonical, "tscoll/usrchtst/TestGetSetOffsetCanonical");
	addTest(root, &TestSupplementaryCanonical, "tscoll/usrchtst/TestSupplementaryCanonical");
	addTest(root, &TestContractionCanonical, "tscoll/usrchtst/TestContractionCanonical");
