...
static const DateIntervalFormatTestItem testItems[] = {
	...
	{ "en", "yMMMEd",   CAP_NONE,  tzUSPacific, Date201009270800, Date201009270800 + 31.0*_DAY,
	  "Mon, Sep 27 \\u2013 Thu, Oct 28, 2010" },
	{ "en", "yMMMEd",   CAP_NONE,  tzUSPacific, Date201009270800, Date201009270800 + 410.0*_DAY,
	  "Mon, Sep 27, 2010 \\u2013 Fri, Nov 11, 2011" },
	{ "de", "Hm",       CAP_NONE,  tzUSPacific, Date201009270800, Date201009270800 + 12.0*_HOUR, "08:00\\u201320:00 Uhr" },
	{ "de", "Hm",       CAP_NONE,  tzUSPacific, Date201009270800, Date201009270800 + 31.0*_DAY,
	...
	{ "cs", "yMMMM",    CAP_NONE,  tzUSPacific, Date201009270800, Date201009270800 + 60.0*_DAY,
	  "z\\u00E1\\u0159\\u00ED\\u2013listopad 2010" },
	...
	{ "cs", "yMMMM",    CAP_BEGIN, tzUSPacific, Date201009270800, Date201009270800 + 60.0*_DAY,
	  "Z\\u00E1\\u0159\\u00ED\\u2013listopad 2010" },
	...
	{ "cs", "yMMMM",    CAP_LIST,  tzUSPacific, Date201009270800, Date201009270800 + 60.0*_DAY,
	  "Z\\u00E1\\u0159\\u00ED\\u2013listopad 2010" },
...
#endif
	{ "cs", "yMMMM",    CAP_ALONE, tzUSPacific, Date201009270800, Date201009270800 + 60.0*_DAY,
	  "z\\u00E1\\u0159\\u00ED\\u2013listopad 2010" },
...
static void TestDateIntervalFormat()
{
	...
	for(testItemPtr = testItems; testItemPtr->locale != NULL; ++testItemPtr) {
		...
		if(U_SUCCESS(status)) {
			...
			status = U_ZERO_ERROR;
			int32_t fmtLen = udtitvfmt_format(udtitvfmt,
				testItemPtr->from,
				testItemPtr->to,
				result,
				kFormatBufLen,
			...
			if(U_SUCCESS(status)) {
				...
				if(u_strcmp(result, resultExpected) != 0) {
					...
					log_err(
						"ERROR: udtitvfmt_format for locale %s, skeleton %s, tzid %s, from %.1f, to %.1f: expect %s, get %s\n",
						testItemPtr->locale,
						testItemPtr->skeleton,
						tzidForLog,
						testItemPtr->from,
						testItemPtr->to,
						testItemPtr->resultExpected,
						u_austrcpy(bcharBuf, result));
				}
			}
			else {
			...
			for(; *deltasPtr >= 0.0; deltasPtr++, expectedPtr++) {
				...
				if(U_FAILURE(status)) {
					...
					log_err("FAIL: udtitvfmt_format for locale %s, skeleton %s, delta %12.1f, expect %d-%d \"%s\", get %d-%d \"%s\"\n",
						locSkelItemPtr->locale, locSkelItemPtr->skeleton, *deltasPtr, expectedPtr->posBegin, expectedPtr->posEnd,
						bebuf, fpos.beginIndex, fpos.endIndex, bbuf);
				}
			}
