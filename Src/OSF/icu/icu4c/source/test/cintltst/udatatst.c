...
static void TestUDataOpen() {
	...
	char * path = (char *)SAlloc::M(sizeof(char) * (strlen(ctest_dataOutDir())
	...
	/* try again, adding /tmp */
	{
		 ...
		 *   missing, skip this test without error.
		 */
		icuDataFilePath = (char *)SAlloc::M(strlen(path) + 10);
		strcpy(icuDataFilePath, path);
		strcat(icuDataFilePath, ".dat");
		/* lots_of_mallocs(); */
		if(uprv_fileExists(icuDataFilePath)) {
	 ...
	 */
	/* lots_of_mallocs(); */
	icuDataFilePath = (char *)SAlloc::M(strlen(ctest_dataOutDir()) + 50);
...
/*      icuDataFilePath = (char *)SAlloc::M(strlen(u_getDataDirectory()) + 50); */
...
/*      SAlloc::F(icuDataFilePath); */
	...
	if(status==U_FILE_ACCESS_ERROR) {
		...
		log_err(
			"calling udata_open with non-existing file  [testdata | nonexist.tst] not handled correctly\n.  Expected: U_FILE_ACCESS_ERROR, Got: %s\n",
			myErrorName(status));
		if(U_SUCCESS(status)) {
			udata_close(result);
		}
...
static bool U_CALLCONV isAcceptable1(void * context,
...
static bool U_CALLCONV isAcceptable2(void * context, const char * type, const char * name, const UDataInfo * pInfo) 
{
	(void)context; // suppress compiler warnings about unused variable
...
static bool U_CALLCONV isAcceptable3(void * context, const char * type, const char * name, const UDataInfo * pInfo) 
{
	(void)context; // suppress compiler warnings about unused variable
	if(pInfo->size>=20 &&
	    pInfo->isBigEndian==U_IS_BIG_ENDIAN &&
...
static void TestErrorConditions() 
{
	...
	if(U_FAILURE(status)) {
		if(status != U_ILLEGAL_ARGUMENT_ERROR || result != NULL) {
			log_err(
				"FAIL: udata_open() with name=NULL should return NULL and errocode U_ILLEGAL_ARGUMENT_ERROR, GOT: errorcode=%s\n",
				myErrorName(status));
		}
		else {
		...
		log_err(
			"FAIL: udata_open() with data name=NULL is supposed to fail for path = %s, name=NULL type=%s errorcode=U_ZERO_ERROR \n",
			testPath,
			type);
		udata_close(result);
	}
	...
	if(U_FAILURE(status)) {
		if(status != U_ILLEGAL_ARGUMENT_ERROR || result != NULL) {
			log_err(
				"FAIL: udata_openChoice() with name=NULL should return NULL and errocode U_ILLEGAL_ARGUMENT_ERROR, GOT: errorcode=%s\n",
				myErrorName(status));
		}
		else {
		...
		log_err(
			"FAIL: udata_openChoice() with data name=NULL is supposed to fail for path = %s, name=NULL type=%s errorcode=U_ZERO_ERROR \n",
			testPath,
			type);
		udata_close(result);
	}
...
static void TestSwapCase(UDataMemory * pData, const char * name,
    UDataSwapFn * swapFn,
...
static void U_CALLCONV printErrorToString(void * context, const char * fmt, va_list args) {
...
static void TestSwapData() {
	...
	buffer = (uint8_t*)SAlloc::M(2*SWAP_BUFFER_SIZE);
	if(buffer==NULL) {
...
//        ICU_TIMEZONE_FILES_DIR=../testdata/out/build
// LD_LIBRARY_PATH=../../lib:../../stubdata:../../tools/ctestfw:$LD_LIBRARY_PATH  ./cintltst /udatatst/TestTZDataDir
