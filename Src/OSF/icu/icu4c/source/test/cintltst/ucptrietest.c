...
static int32_t skipSpecialValues(const CheckRange checkRanges[], int32_t countCheckRanges) {
	int32_t i;
...
static int32_t getSpecialValues(const CheckRange checkRanges[], int32_t countCheckRanges,
...
static uint32_t U_CALLCONV testFilter(const void * context, uint32_t value) {
	(void)context; // suppress compiler warnings about unused variable
	return value ^ 0x5555;
}

static bool doCheckRange(const char * name, const char * variant,
...
static void testTrieGetRanges(const char * testName, const UCPTrie * trie, const UMutableCPTrie * mutableTrie,
...
static void testTrieGetters(const char * testName, const UCPTrie * trie,
...
static void testBuilderGetters(const char * testName, const UMutableCPTrie * mutableTrie,
    const CheckRange checkRanges[], int32_t countCheckRanges) {
	uint32_t initialValue, errorValue;
	uint32_t value, value2;
	UChar32 start, limit;
...
static void testTrieUTF8(const char * testName,
    const UCPTrie * trie, UCPTrieValueWidth valueWidth,
    const CheckRange checkRanges[], int32_t countCheckRanges) {
...
static void testBuilder(const char * testName, const UMutableCPTrie * mutableTrie, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
...
static void trieTestGolden(const char * testName, const UCPTrie* trie, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	...
	snprintf(goldendatapath, 512, "%scodepointtrie%s%s.toml", testdatapath, U_FILE_SEP_STRING, testName);
	...
	void * memoryBuffer = SAlloc::M(fsize + 1);
...
cleanup:
	fclose(stream);
	SAlloc::F(memoryBuffer);
}

static uint32_t storage[120000];
static uint32_t swapped[120000];

static void testTrieSerialize(const char * testName, UMutableCPTrie * mutableTrie,
    UCPTrieType type, UCPTrieValueWidth valueWidth, bool withSwap, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	UCPTrie * trie;
...
static UMutableCPTrie * testTrieSerializeAllValueWidth(const char * testName,
    UMutableCPTrie * mutableTrie, bool withClone, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	char name[40];
...
static UMutableCPTrie * makeTrieWithRanges(const char * testName, bool withClone, const SetRange setRanges[], int32_t countSetRanges,
...
static void testTrieRanges(const char * testName, bool withClone, const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	uint32_t initialValue, errorValue;
	getSpecialValues(checkRanges, countCheckRanges, &initialValue, &errorValue);
	UMutableCPTrie * mutableTrie = makeTrieWithRanges(testName, withClone, setRanges, countSetRanges, initialValue, errorValue);
	if(mutableTrie) {
		mutableTrie = testTrieSerializeAllValueWidth(testName, mutableTrie, withClone, checkRanges, countCheckRanges);
		umutablecptrie_close(mutableTrie);
	}
}
...
static void GrowDataArrayTest() 
{
	...
	int32_t i;
	UErrorCode errorCode = U_ZERO_ERROR;
	UMutableCPTrie * mutableTrie = umutablecptrie_open(1, 0xad, &errorCode);
	if(U_FAILURE(errorCode)) {
		log_err("error: umutablecptrie_open(%s) failed: %s\n", testName, u_errorName(errorCode));
		return;
	}
...
static void ManyAllSameBlocksTest() 
{
	static const char * const testName = "many-all-same";
	int32_t i;
	CheckRange checkRanges[(0x110000 >> 12) + 1];
	UErrorCode errorCode = U_ZERO_ERROR;
	UMutableCPTrie * mutableTrie = umutablecptrie_open(0xff33, 0xad, &errorCode);
	if(U_FAILURE(errorCode)) {
		log_err("error: umutablecptrie_open(%s) failed: %s\n", testName, u_errorName(errorCode));
		return;
	}
...
static void testGetRangesFixedSurr(const char * testName, const UMutableCPTrie * mutableTrie,
    UCPMapRangeOption option, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
...
static void TrieTestGetRangesFixedSurr() {
	static const SetRange
	    setRangesFixedSurr[] = {
		{ 0xd000, 0xd7ff, 5 },
		{ 0xd7ff, 0xe001, 3 },
...
static void TestSmallNullBlockMatchesFast() {
