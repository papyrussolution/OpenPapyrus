...
static void TestUnescapeRepeatedSurrogateLead20725();
	...
	/* test u_strcmpCodePointOrder() */
	{
		...
		for(i = 0; i<(SIZEOFARRAYi(strings)-1); ++i) {
			...
			if(u_strncmpCodePointOrder(strings[i], strings[i+1], 2)!=u_memcmpCodePointOrder(strings[i], strings[i+1], 2)) {
				log_err(
					"error: u_strncmpCodePointOrder(2)!=u_memcmpCodePointOrder(2) for string %d and the following one\n",
					i);
			}
 ...
 */
static void compareIterators(UCharIterator * iter1, const char * n1,
	...
	for(i = 0; i<3; ++i) {
		...
		if(c1!=c2) {
			log_err("%s->next()=U+%04x != U+%04x=%s->next() at %d (started in middle)\n", n1, c1, c2, n2,
			    iter1->getIndex(iter1, UITER_CURRENT));
			return;
		}
	}
