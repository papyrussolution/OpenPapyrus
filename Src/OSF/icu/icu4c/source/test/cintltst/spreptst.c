...
static void Test_nfs4_cs_prep();
static void Test_nfs4_cis_prep();
static void Test_nfs4_mixed_prep();
static void TestBEAMWarning();
static void TestCoverage();
static void TestStringPrepProfiles();
...
static void Test_nfs4_cis_prep() 
{
	...
	for(i = 0; i< SIZEOFARRAYi(conformanceTestCases); i++) {
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) {
			status = U_ZERO_ERROR;
			dest = (char *)SAlloc::M(++destLen);
			destLen = nfs4_cis_prepare(src, (int32_t)strlen(src), dest, destLen, &parseError, &status);
		}

		if(expectedStatus != status) {
			log_data_err(
				"Did not get the expected status for nfs4_cis_prep at index %i. Expected: %s Got: %s - (Are you missing data?)\n",
				i,
...
static void Test_nfs4_cs_prep() {
...

	{
		...
		if(U_SUCCESS(status)) {
			...
			if(destLen != srcLen) {
				log_err(
					"Did not get the expected length for the output for case: BiDi Checking Turned OFF. Expected: %i Got: %i\n",
					srcLen,
					destLen);
			}
		}
	...
	}
	{
		...
		if(U_SUCCESS(status)) {
			...
			if(destLen != srcLen) {
				log_err(
					"Did not get the expected length for the output for case: Normalization Turned OFF. Expected: %i Got: %i\n",
					srcLen,
					destLen);
			}
		}
	...
	}
	{
		...
		if(U_SUCCESS(status)) {
			...
			if(destLen != srcLen) {
				log_err(
					"Did not get the expected length for the output for case: Case Mapping Turned OFF. Expected: %i Got: %i\n",
					srcLen,
					destLen);
			}
		}
	...
	}
	{
		...
		if(U_SUCCESS(status)) {
			...
			if(destLen != expLen) {
				log_err(
					"Did not get the expected length for the outputfor case: Case Mapping Turned On. Expected: %i Got: %i\n",
					strlen(expected),
					destLen);
			}
		}
