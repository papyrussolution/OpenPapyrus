...
static int findStringIndex(const char * const * sortedArr, int32_t sortedArrLen, const char * target, int32_t targetLen) {
...
static void getPrefixSuffix(const char * src, int32_t srcLength,
	...
	*suffixLen = srcLength - i;
	...
	if((findStringIndex(special_prefixes, SIZEOFARRAYi(special_prefixes), *prefix, *prefixLen-1) != -1) && (*suffix != NULL)) {
