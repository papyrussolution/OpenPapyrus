...
static uint32_t U_CALLCONV testEnumValue(const void * context, uint32_t value) {
	(void)context; // suppress compiler warnings about unused variable
	return value^0x5555;
}

/* utrie2_enum() callback, verifies a range */
static bool U_CALLCONV testEnumRange(const void * context, UChar32 start, UChar32 end, uint32_t value) {
...
static void testTrieEnum(const char * testName,
    const UTrie2 * trie,
...
static void testTrieGetters(const char * testName,
	...
	bool isFrozen = utrie2_isFrozen(trie);
...
static void testTrieUTF8(const char * testName,
	...
	while(p<limit) {
		...
		if(value!=values[i]) {
			log_err("error: wrong value from UTRIE2_U8_NEXT(%s)(from %d %lx->U+%04lx) (read %d bytes): 0x%lx instead of 0x%lx\n",
			    testName, (int)prev8, (unsigned long)bytes, (long)c, (int)((p-s)-prev8), (long)value, (long)values[i]);
		}
		if(i8!=(p-s)) {
...
static void testFrozenTrie(const char * testName,
...
static void testNewTrie(const char * testName, const UTrie2 * trie,
    const CheckRange checkRanges[], int32_t countCheckRanges) {
...
static void testTrieSerialize(const char * testName, UTrie2 * trie, UTrie2ValueBits valueBits,
    bool withSwap, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	...
	do {
		...
		if(errorCode!=U_ILLEGAL_ARGUMENT_ERROR) {
			log_err("error: utrie2_freeze(already-frozen with other valueBits %s) set %s != U_ILLEGAL_ARGUMENT_ERROR\n", testName, u_errorName(errorCode));
			break;
		}
		errorCode = U_ZERO_ERROR;
		if(withSwap) {
		...
		if(length1!=length2) {
			log_err("error: trie serialization (%s) lengths different: preflight vs. serialize\n", testName);
			break;
		}
		...
		if(withSwap) {
			...
			if(U_FAILURE(errorCode) || swappedLength!=length2) {
				log_err("error: utrie2_swap(%s to OE preflighting) failed (%s) or before/after lengths different\n", testName, u_errorName(errorCode));
			...
			if(U_FAILURE(errorCode) || swappedLength!=length2) {
				log_err("error: utrie2_swap(%s to PE preflighting) failed (%s) or before/after lengths different\n", testName, u_errorName(errorCode));
				udata_closeSwapper(ds);
				break;
			}
		...
		if(length2!=length3) {
			log_err("error: trie serialization (%s) lengths different: serialize vs. unserialize\n", testName);
			break;
		}
		...
		utrie2_freeze(trie, otherValueBits, &errorCode);
		if(errorCode!=U_ILLEGAL_ARGUMENT_ERROR) {
			log_err("error: utrie2_freeze(unserialized with other valueBits %s) set %s != U_ILLEGAL_ARGUMENT_ERROR\n", testName, u_errorName(errorCode));
			break;
		}
		errorCode = U_ZERO_ERROR;
		if(withSwap) {
		...
		testFrozenTrie(testName, trie, valueBits, checkRanges, countCheckRanges);
		{
			...
			if(U_FAILURE(errorCode) || utrie2_isFrozen(clone)) {
				log_err("error: utrie2_cloneAsThawed(unserialized %s) failed - %s (isFrozen: %d)\n", testName, u_errorName(errorCode), clone!=NULL && utrie2_isFrozen(trie));
				break;
			}
			else {
				utrie2_close(trie);
...
static UTrie2 * testTrieSerializeAllValueBits(const char * testName, UTrie2 * trie, bool withClone,
    const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	char name[40];
...
static UTrie2 * makeTrieWithRanges(const char * testName, bool withClone,
    const SetRange setRanges[], int32_t countSetRanges, const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	UTrie2 * trie;
	uint32_t initialValue, errorValue;
	uint32_t value;
...
static void testTrieRanges(const char * testName, bool withClone,
...
static void dummyTest(UTrie2ValueBits valueBits) {
	CheckRange
	    checkRanges[] = {
		{ -1,       0 },
		{ 0,        0 },
...
static UNewTrie * makeNewTrie1WithRanges(const char * testName,
    const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges) {
	UNewTrie * newTrie;
	uint32_t initialValue, errorValue;
	...
	errorCode = U_ZERO_ERROR;
	getSpecialValues(checkRanges, countCheckRanges, &initialValue, &errorValue);
	newTrie = utrie_open(NULL, NULL, 2000, initialValue, initialValue, FALSE);
	if(U_FAILURE(errorCode)) {
...
static void testTrie2FromTrie1(const char * testName, const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges) 
{
	...
	uint32_t initialValue, errorValue;
	UErrorCode errorCode;
	UNewTrie * newTrie1_16 = makeNewTrie1WithRanges(testName, setRanges, countSetRanges, checkRanges, countCheckRanges);
	if(newTrie1_16==NULL) {
		return;
	}
	...
	errorCode = U_ZERO_ERROR;
	length16 = utrie_serialize(newTrie1_16, memory1_16, sizeof(memory1_16), NULL, TRUE, &errorCode);
	length32 = utrie_serialize(newTrie1_32, memory1_32, sizeof(memory1_32), NULL, FALSE, &errorCode);
	...
	if(U_SUCCESS(errorCode)) {
		...
		for(lead = 0xd800; lead<0xdc00; ++lead) {
			...
			if(value1!=value2) {
				log_err("error: utrie2_fromUTrie(%s) wrong value %ld!=%ld from lead surrogate code unit U+%04lx\n", name, (long)value2, (long)value1, (long)lead);
				break;
			}
		}
	}
...
void addTrie2Test(TestNode** root) 
{
	...
	addTest(root, &EnumNewTrieForLeadSurrogateTest, "tsutil/trie2test/EnumNewTrieForLeadSurrogateTest");
