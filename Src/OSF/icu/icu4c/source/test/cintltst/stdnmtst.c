...
static bool doTestNames(const char * name, const char * standard, const char ** expected, int32_t size) {
	...
	while(repeatTimes-- > 0) {
		...
		if(enumCount >= 0) {
			...
			if(enumName != NULL || len != 0 || U_FAILURE(err)) {
				log_err(
					"FAIL: uenum_next(past the list) did not return NULL[0] with U_SUCCESS(). name=%s standard=%s len=%d err=%s\n",
					name,
					standard,
					len,
					u_errorName(err));
...
static bool doTestUCharNames(const char * name, const char * standard, const char ** expected, int32_t size) 
{
	UErrorCode err = U_ZERO_ERROR;
	UEnumeration * myEnum = ucnv_openStandardNames(name, standard, &err);
	int32_t enumCount = uenum_count(myEnum, &err);
