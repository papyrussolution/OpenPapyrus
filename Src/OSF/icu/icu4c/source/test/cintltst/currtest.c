...
static void expectInList(const char * isoCurrency, uint32_t currencyType, bool isExpected) {
...
static void TestFractionDigitOverride() {
	...
	unum_formatDouble(fmt, 123.456, buffer, SIZEOFARRAYi(buffer), NULL, &status);
...
static void TestNumericCode() {
	...
	for(i = 0; NUMCODE_TESTDATA[i].alphaCode; i++) {
		int32_t length = (int32_t)strlen(NUMCODE_TESTDATA[i].alphaCode);
