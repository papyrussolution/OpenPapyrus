		...
		if(ures_getType(subRootBundle) != ures_getType(subBundle)) {
			...
			if(U_SUCCESS(errorCode)
			 && (ures_getType(subSubBundle) == URES_ARRAY || ures_getType(subSubRootBundle) == URES_ARRAY)) {
				...
				if((subBundleKey == NULL || (subBundleKey != NULL && strcmp(subBundleKey, "LocaleScript") != 0 && !isCurrencyPreEuro(subBundleKey))) && 
					...
					log_err("Different size array with key \"%s\" in \"%s\" from root for locale \"%s\"\n\troot array size=%d, locale array size=%d\n",
					    subBundleKey, ures_getKey(currentBundle), locale, ures_getSize(subRootBundle), ures_getSize(subBundle));
				}
				/*
...
#if !UCONFIG_NO_FILE_IO && !UCONFIG_NO_LEGACY_CONVERSION
static void TestLocaleStructure() {
	...
	for(locIndex = 0; locIndex < locCount; locIndex++) {
		...
		if(errorCode != U_ZERO_ERROR) {
			if(U_SUCCESS(errorCode)) {
				...
				log_err("%%%%%%% Unexpected error %d in %s %%%%%%%", u_errorName(errorCode), uloc_getAvailable(locIndex));
			}
...
static void compareArrays(const char * keyName, UResourceBundle * fromArray, const char * fromLocale,
    UResourceBundle * toArray, const char * toLocale, int32_t start, int32_t end)
{
...
static void compareConsistentCountryInfo(const char * fromLocale, const char * toLocale) {
	UErrorCode errorCode = U_ZERO_ERROR;
...
static void TestConsistentCountryInfo() {
...
static void findSetMatch(UScriptCode * scriptCodes, int32_t scriptsLen, USet * exemplarSet, const char * locale) 
{
	USet * scripts[10] = {0};
	...
	for(locIndex = 0; locIndex < locCount; locIndex++) {
		...
		//}
			...
			}
			{
				...
				if((uprv_strncmp(currLoc, "lrc", 3) == 0 || uprv_strncmp(currLoc, "mzn", 3) == 0) && log_knownIssue("cldrbug:8899", "lrc and mzn locales don't have translated day names")) {
					end = 0;
				}
				if((uprv_strncmp(currLoc, "mai", 3) == 0 || uprv_strncmp(currLoc, "sd_Deva", 7) == 0) && log_knownIssue("cldrbug:14995", "mai/sd_Deva day names use chars not in exemplars")) {
					end = 0;
				}
			...
			/* test that the MeasurementSystem API works */
			{
				...
				if(U_FAILURE(errorCode)) {
					...
					if(strstr(fullLoc, "_US")!=NULL || strstr(fullLoc, "_LR")!=NULL) {
						if(measurementSystem != UMS_US) {
							log_err(
								"ulocdata_getMeasurementSystem did not return expected data for locale %s \n",
								currLoc);
						}
					}
				...
				if(U_FAILURE(errorCode)) {
					...
					if(strstr(fullLoc,
					    "_US")!=NULL ||
					    strstr(fullLoc, "_BZ")!=NULL || strstr(fullLoc, "_CA")!=NULL || strstr(fullLoc, "_CL")!=NULL ||
					    strstr(fullLoc,
					    "_CO")!=NULL ||
					    strstr(fullLoc, "_CR")!=NULL || strstr(fullLoc, "_GT")!=NULL || strstr(fullLoc, "_MX")!=NULL ||
					    strstr(fullLoc,
					    "_NI")!=NULL ||
					    strstr(fullLoc, "_PA")!=NULL || strstr(fullLoc, "_PH")!=NULL || strstr(fullLoc, "_PR")!=NULL ||
...
static void TestExemplarSet() {
	...
	for(i = 0; i<n; i++) {
		...
		for(k = 0; k<2; ++k) {
			...
			if(strcmp(locale,
			    "yi") == 0 && codeLen > 0 &&
			    log_knownIssue("11217", "Fix result of uscript_getCode for yi: USCRIPT_YI -> USCRIPT_HEBREW")) {
