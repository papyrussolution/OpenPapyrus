...
static bool gFnNameError   = FALSE;
static bool gFnFormatError = FALSE;
...
static void U_CALLCONV testTraceExit(const void * context, int32_t fnNumber, const char * fmt, va_list args) 
{
	(void)context; // suppress compiler warnings about unused variable
	char buf[1000];
	const char * fnName;
...
static void TestTraceAPI() 
{
	 ...
	 */
	utrace_getFunctions(&originalTContext, &originalTEntryFunc, &originalTExitFunc, &originalTDataFunc);
	 ...
	 */
	{
		...
		test_format("a 64 bit val %l", 50, 0, "a 64 bit val 123456780abcdef0", __LINE__, INT64_C(0x123456780abcdef0));
		if(sizeof(void *) == 4) {
			ptr = (void *)0xdeadbeef;
	...
	{
		char buf[100];
		int32_t x = utrace_format(buf, 100, 0, "%s", "Hello, World.");
	 ...
	 */
	{
		const char * name = utrace_functionName(UTRACE_U_INIT);
