...
static void releaseAvailableNames() {
	uprv_free((void *)gAvailableNames);
	gAvailableNames = NULL;
	gCountAvailable = 0;
}

static const char ** getEncodings(int32_t start, int32_t step, int32_t count, int32_t * pCount) {
	const char ** names;
	int32_t i;
 ...
 */
static const char ** getNoEncodings(int32_t * pCount) {
	*pCount = 0;
	return NULL;
}
...
static const char ** getOneEncoding(int32_t * pCount) {
	return getEncodings(1, 0, 1, pCount);
}

static const char ** getFirstEvenEncodings(int32_t * pCount) {
	return getEncodings(0, 2, 25, pCount);
}

static const char ** getMiddleEncodings(int32_t * pCount) {
	return getEncodings(gCountAvailable - 12, 1, 22, pCount);
}

static const char ** getLastEncodings(int32_t * pCount) {
	return getEncodings(gCountAvailable - 1, -1, 25, pCount);
}

static const char ** getSomeEncodings(int32_t * pCount) {
	/* 20 evenly distributed */
	return getEncodings(5, (gCountAvailable + 19)/ 20, 20, pCount);
}

static const char ** getEveryThirdEncoding(int32_t * pCount) {
	return getEncodings(2, 3, (gCountAvailable + 2 )/ 3, pCount);
}

static const char ** getAllEncodings(int32_t * pCount) {
...
static void text_reset(TestText * tt) {
	tt->limit = tt->text;
	tt->number = 0;
}

static char * text_nextString(TestText * tt, int32_t * pLength) 
{
	char * s = tt->limit;
	if(s == tt->textLimit) {
...
static bool text_open(TestText * tt) 
{
	FILE * f;
	char * s;
	int32_t length;
...
static void text_close(TestText * tt) {
	uprv_free(tt->text);
}
    ...
    const char * utf8, int32_t length, const USet* excludedCodePoints, const UConverterUnicodeSet whichSet) 
{
	...
	for(i = 0; i < num_encodings; i++) {
		 ...
		 *   (bool)(uset_spanUTF8(set, utf8, length, USET_SPAN_SIMPLE) == length);
...
static void verifyResult(UEnumeration* res, const bool * resultsManually) 
{
	bool* resultsFromSystem = (bool*)uprv_malloc(gCountAvailable * sizeof(bool));
	const char * name;
	UErrorCode status = U_ZERO_ERROR;
	int32_t i;
...
static UConverterSelector * serializeAndUnserialize(UConverterSelector * sel, char ** buffer, UErrorCode * status) 
{
	char * new_buffer;
	int32_t ser_len2;
	/* preflight */
	int32_t ser_len = ucnvsel_serialize(sel, NULL, 0, status);
