...
#define ALIGNOF(type) offsetof(struct { char c; type member; }, member) /* Similar to C++ alignof(type)  */
...
static UConverter * cnv_open(const char * name, UErrorCode * pErrorCode) 
{
	return (name && name[0]=='*') ? ucnv_openPackage(loadTestData(pErrorCode), name+1, pErrorCode) : ucnv_open(name, pErrorCode);
}

static void ListNames();
static void TestFlushCache();
static void TestDuplicateAlias();
static void TestCCSID();
static void TestJ932();
static void TestJ1968();
#if !UCONFIG_NO_FILE_IO && !UCONFIG_NO_LEGACY_CONVERSION
static void TestLMBCSMaxChar();
#endif
...
static void TestEBCDICSwapLFNL();
static void TestConvertEx();
static void TestConvertExFromUTF8();
static void TestConvertExFromUTF8_C5F0();
static void TestConvertAlgorithmic();
void TestDefaultConverterError();           /* defined in cctest.c */
void TestDefaultConverterSet();           /* defined in cctest.c */
static void TestToUCountPending();
static void TestFromUCountPending();
static void TestDefaultName();
static void TestCompareNames();
	...
	if(U_FAILURE(err)) {
		...
		if(U_FAILURE(err)) {
			...
			if(U_SUCCESS(err)) {
				...
				const char ** aliases;
				aliases = (const char **)SAlloc::M(count * sizeof(const char *));
				if(aliases != 0) {
...
static void TestConvert()
{
	...
	******************************************************************/
	...
	const char * CodePagesToTest[NUM_CODEPAGE] = { "ibm-949_P110-1999" };
	const uint16_t CodePageNumberToTest[NUM_CODEPAGE] = { 949 };
	const int8_t CodePagesMinChars[NUM_CODEPAGE] = { 1 };
	const int8_t CodePagesMaxChars[NUM_CODEPAGE] = { 2 };
	const uint16_t CodePagesSubstitutionChars[NUM_CODEPAGE] = { 0xAFFE };
	const char * CodePagesTestFiles[NUM_CODEPAGE] = { "uni-text.bin" };
	const UConverterPlatform CodePagesPlatform[NUM_CODEPAGE] = { UCNV_IBM };
	const char * CodePagesLocale[NUM_CODEPAGE] = { "ko_KR" };
	...
	{
		int32_t targetLimit = 0, i = 0, targetCapacity = 0;
		...
		char * target = 0;
		int32_t sourceLimit = SIZEOFARRAYi(source);
		err = U_ZERO_ERROR;
		targetLimit = 0;
		...
		if(err == U_BUFFER_OVERFLOW_ERROR) {
			...
			target = (char *)SAlloc::M(sizeof(char) * targetLimit);
		...
		if(U_FAILURE(err)) {
			...
			err = U_ILLEGAL_ARGUMENT_ERROR;
			sourceLimit = SIZEOFARRAYi(source);
	...
	for(codepage_index = 0; codepage_index <  NUM_CODEPAGE; ++codepage_index) {
		...
		//D:\Papyrus\Src\OSF\icu\icu4c\source\test\testdata\
		// @sobolev strcat(ucs_file_name, ".." U_FILE_SEP_STRING);
		strcat(ucs_file_name, "source/test/testdata/"); // @sobolev
#endif
		...
		/*call it first time for trapping the targetcapacity and size needed to allocate memory for the buffer
		   uchar2 */
		targetcapacity2 = 0;
		...
		ucnv_toUnicode(myConverter, &my_ucs_file_buffer_1, my_ucs_file_buffer + MAX_FILE_LEN, &tmp_mytarget_use,
		    mytarget_use + (mytarget_1 - mytarget), NULL, FALSE, &err);
	...
	SAlloc::F((void *)mytarget);
	SAlloc::F((void *)output_cp_buffer);
	SAlloc::F((void *)ucs_file_buffer);
	SAlloc::F((void *)my_ucs_file_buffer);
#endif
}
	...
	static UConverterFromUCallback otherUnicodeAction(UConverterFromUCallback MIA)
	{
		return (MIA==(UConverterFromUCallback)UCNV_FROM_U_CALLBACK_STOP) ? 
			(UConverterFromUCallback)UCNV_FROM_U_CALLBACK_SUBSTITUTE : (UConverterFromUCallback)UCNV_FROM_U_CALLBACK_STOP;
	}

	static UConverterToUCallback otherCharAction(UConverterToUCallback MIA)
	{
		return (MIA==(UConverterToUCallback)UCNV_TO_U_CALLBACK_STOP) ? 
			(UConverterToUCallback)UCNV_TO_U_CALLBACK_SUBSTITUTE : (UConverterToUCallback)UCNV_TO_U_CALLBACK_STOP;
	}
#endif
...
static void TestAlias() 
{
	...
	int32_t ISO_2022_NAMES_LENGTH = SIZEOFARRAYi(ISO_2022_NAMES);
	const char * UTF8_NAMES[] =
	{ "UTF-8", "utf-8", "utf8", "ibm-1208",
	  "utf_8", "ibm1208", "cp1208" };
	int32_t UTF8_NAMES_LENGTH = SIZEOFARRAYi(UTF8_NAMES);
	...
	int32_t CONVERTERS_NAMES_LENGTH = SIZEOFARRAYi(CONVERTERS_NAMES);
	...
	for(i = 0; i<ncnv; ++i) {
		...
		for(j = 1; j<na; ++j) {
			...
			if(alias == NULL) {
				log_err("FAIL: Converter \"%s\" -> alias[%d]=NULL\n", name, j);
				continue;
			}
			mapBack = ucnv_getAlias(alias, 0, &status);
			if(mapBack == NULL) {
				log_err("FAIL: Converter \"%s\" -> alias[%d]=\"%s\" -> alias[0]=NULL, exp. \"%s\"\n", name, j, alias, alias0);
				continue;
			}
...
{
	TSCCContext * newCtx = (TSCCContext*)SAlloc::M(sizeof(TSCCContext));
...
static void TSCC_toU(const void * context, UConverterToUnicodeArgs * toUArgs, const char * codeUnits, int32_t length,
...
/*************************************/
	...
	if(to2 != (&to1)) {
		SAlloc::F(to2); /* to1 is stack based */
	}
	if(from2 != (&from1)) {
		SAlloc::F(from2); /* from1 is stack based */
	}
}

#endif

static bool containsAnyOtherByte(uint8_t * p, int32_t length, uint8_t b) {
	while(length>0) {
		if(*p!=b) {
			return TRUE;
		}
...
static void TestConvertSafeClone()
{
	...
	int32_t actualSizes[SIZEOFARRAYi(names)];
	...
	for(j = 0; j < SIZEOFARRAYi(bufferSizes); ++j) {
		for(idx = 0; idx < SIZEOFARRAYi(names); idx++) {
			err = U_ZERO_ERROR;
			...
			ucnv_close(cnv);
			if(actualSizes[idx] <= (bufferSizes[j] - (int32_t)/*ALIGNOF*/alignof(UConverter)) && err == U_SAFECLONE_ALLOCATED_WARNING) {
			...
			if(bufferSize <= bufferSizes[j]) {
				...
				if(containsAnyOtherByte(buffer[0], (int32_t)sizeof(buffer[0]), 0xaa) ||
				    containsAnyOtherByte(buffer[1]+bufferSize, (int32_t)(sizeof(buffer)-(sizeof(buffer[0])+bufferSize)),
				    0xaa)
				    ) {
			...
			ucnv_fromUnicode(cnv2, &pCharBuffer, charBufferLimit, &pUniBuffer, uniBufferLimit, NULL, TRUE, &err);
			if(U_FAILURE(err)) {
				log_err("FAIL: cloned converter failed to do fromU conversion. Error: %s\n", u_errorName(err));
			}
			ucnv_toUnicode(cnv2, &pUCharTarget, pUCharTargetLimit, &pCharSource, pCharSourceLimit, NULL, TRUE, &err);
			if(U_FAILURE(err)) {
...
static void TestCCSID() 
{
	...
	for(i = 0; i<SIZEOFARRAYi(ccsids); ++i) {
		ccsid = ccsids[i];
...
static void TestConvertEx() 
{
	...
	if(errorCode!=U_ZERO_ERROR || target-targetBuffer!=sizeof(shiftJIS) || *target!=0 || memcmp(targetBuffer, shiftJIS, sizeof(shiftJIS))!=0) {
...
static bool isOneTruncatedUTF8(const char * s, int32_t length) {
	if(length==0) {
		return FALSE;
	}
	else if(length==1) {
...
static void testFromTruncatedUTF8(UConverter * utf8Cnv, UConverter * cnv, const char * converterName,
	...
	for(i = 0; i<SIZEOFARRAYi(badUTF8); ++i) {
		/* truncated sequence? */
		...
		ucnv_convertEx(cnv, utf8Cnv,
		    ...
		    pivotBuffer, &pivotSource, &pivotTarget, pivotBuffer+SIZEOFARRAYi(pivotBuffer),
...
static void testFromBadUTF8(UConverter * utf8Cnv, UConverter * cnv, const char * converterName, char charUTF8[4], int32_t charUTF8Length,
    char char0[8], int32_t char0Length, char char1[8], int32_t char1Length) 
{
	...
	convertExMultiStreaming(utf8Cnv, cnv, utf8, utf8Length, expect, expectLength, testName, U_ZERO_ERROR);
}
...
static void TestConvertExFromUTF8() 
{
	...
	for(i = 0; i<SIZEOFARRAYi(converterNames); ++i) {
		errorCode = U_ZERO_ERROR;
...
static void TestConvertExFromUTF8_C5F0() 
{
	...
	for(i = 0; i<SIZEOFARRAYi(converterNames); ++i) {
		...
		if(i==SIZEOFARRAYi(converterNames)-1) {
		...
		if(errorCode!=U_STRING_NOT_TERMINATED_WARNING || src!=bad_utf8+2 ||
		    target!=dest+expectedLength || 0!=memcmp(dest, expected, expectedLength) ||
...
static void TestConvertAlgorithmic() {
	...
	if(errorCode!=U_STRING_NOT_TERMINATED_WARNING ||
		...
		log_err(
			"ucnv_fromAlgorithmic(UTF-8 -> Shift-JIS) fails (%s expect U_STRING_NOT_TERMINATED_WARNING), returns %d expect %d\n",
			u_errorName(errorCode),
			length,
			sizeof(shiftJIS));
	}
	...
	if(errorCode!=U_STRING_NOT_TERMINATED_WARNING ||
		...
		log_err(
			"ucnv_fromAlgorithmic(UTF-16 only BOM -> Shift-JIS) fails (%s expect U_STRING_NOT_TERMINATED_WARNING), returns %d expect 0\n",
			u_errorName(errorCode),
			length);
	}
...
static void TestLMBCSMaxChar() {
	...
	int32_t idx;
	for(idx = 0; idx < SIZEOFARRAYi(converter); idx++) {
		UErrorCode status = U_ZERO_ERROR;
...
#if !UCONFIG_NO_LEGACY_CONVERSION
static void testSwap(const char * name, bool swap) {
	/*
	 * Test Unicode text.
	...
	pc = (char *)strstr(swappedName, UCNV_SWAP_LFNL_OPTION_STRING);
	if(swap != (pc!=NULL)) {
	...
	pcu = text;
	pc = normal;
	ucnv_fromUnicode(cnv, &pc, normal+SIZEOFARRAYi(normal), &pcu, text+SIZEOFARRAYi(text), NULL, TRUE, &errorCode);
	normalLength = (int32_t)(pc-normal);

	pcu = text;
	pc = swapped;
	ucnv_fromUnicode(swapCnv, &pc, swapped+SIZEOFARRAYi(swapped), &pcu, text+SIZEOFARRAYi(text), NULL, TRUE, &errorCode);
	...
	pc = normal;
	pu = uNormal;
	ucnv_toUnicode(cnv, &pu, uNormal+SIZEOFARRAYi(uNormal), (const char **)&pc, normal+normalLength, NULL, TRUE, &errorCode);
	normalLength = (int32_t)(pu-uNormal);

	pc = normal;
	pu = uSwapped;
	ucnv_toUnicode(swapCnv, &pu, uSwapped+SIZEOFARRAYi(uSwapped), (const char **)&pc, normal+swappedLength, NULL, TRUE, &errorCode);
...
/*       const char16_t expectedUnicode[] = { 0x20ac, 0x0005, 0x0006, 0x000b, 0xdbc4, 0xde34, 0xd84d, 0xdc56, 0xfffd}; */
	...
	for(i = 0; i<SIZEOFARRAYi(fromUnicodeTests); ++i) {
		char tgt[10];
		char * target = tgt;
...
static void compareNames(const char ** names) {
...
static void TestSubstString() {
	...
	ucnv_setSubstString(cnv, sub, SIZEOFARRAYi(sub), &errorCode);
	if(U_FAILURE(errorCode)) {
		...
		if(U_FAILURE(errorCode) || len8!=sizeof(subChars) || 0!=memcmp(buffer, subChars, len8)) {
	...
	if(U_FAILURE(errorCode)) {
		...
		len8 = sizeof(buffer);
		ucnv_getSubstChars(cnv, buffer, &len8, &errorCode);
		/* Stateful converter, we expect that the Unicode string was set and that we get an empty char * string
		   now. */
...
static void InvalidArguments() {
	...
	if(charBuffer[0] != 1 || charBuffer[1] != 1 || ucharAsCharBuffer[0] != 2 || ucharAsCharBuffer[1] != 2) {
...
static void TestGetName() {
	...
	for(int32_t i = 0; i < SIZEOFARRAYi(names); i += 2) {
		UErrorCode errorCode = U_ZERO_ERROR;
...
#if U_IS_BIG_ENDIAN
		{ 4, 0xfe, 0xff, 0, 0x61 }, { 4, 0xfe, 0xff, 0, 0x61 },
#else
		{ 4, 0xff, 0xfe, 0x61, 0 }, { 4, 0xff, 0xfe, 0x61, 0 },
#endif
		{ 2, 0, 0x61 }, { 4, 0xfe, 0xff, 0, 0x61 },
		{ 2, 0x61, 0 }, { 4, 0xff, 0xfe, 0x61, 0 }
	};
	char bytes[10];
	for(int32_t i = 0; i < SIZEOFARRAYi(names); ++i) {
		UErrorCode errorCode = U_ZERO_ERROR;
		UConverter * cnv = ucnv_open(names[i], &errorCode);
		int32_t length = 0;
