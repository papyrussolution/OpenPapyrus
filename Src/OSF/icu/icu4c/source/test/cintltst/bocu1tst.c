 ...
 * @see packDiff
 */
static int32_t writePacked(int32_t packed, uint8_t * p) {
 ...
 * @see writeDiff
 */
static int32_t unpackDiff(int32_t initialPrev, int32_t packed) {
	Bocu1Rx rx = { 0, 0, 0 };
	int32_t count;
 ...
 * @see unpackDiff
 */
static uint8_t * writeDiff(int32_t diff, uint8_t * p) {
 ...
 */
static void printBytes(uint8_t * bytes, char * out) {
	int i;
	uint8_t b;
...
cleanup:
	SAlloc::F(roundtripRef);
	SAlloc::F(roundtripICU);
	SAlloc::F(bocu1Ref);
	SAlloc::F(bocu1ICU);
}
