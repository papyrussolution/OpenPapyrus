...
static void * U_CALLCONV myMemAlloc(const void * context, size_t size) 
{
	...
	char * retPtr = (char *)SAlloc::M(size+sizeof(ctest_AlignedMemory));
	if(retPtr != NULL) {
...
static void * U_CALLCONV myMemRealloc(const void * context, void * mem, size_t size) 
{
	...
	retPtr = (char *)SAlloc::R(p, size+sizeof(ctest_AlignedMemory));
	if(retPtr != NULL) {
		p += sizeof(ctest_AlignedMemory);
	}
	return retPtr;
...
static void TestHeapFunctions() 
{
	...
	icuDataDir = safeGetICUDataDirectory(); /* save icu data dir, so we can put it back after doing u_cleanup(). */
