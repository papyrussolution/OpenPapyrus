...
static void toLimitTest(int64_t toLimit, int64_t fromLimit, UDateTimeScale scale)
{
	...
	if(result != fromLimit) {
		log_err(
			"toLimit failure: scale = %d, toLimit = %lld , utmscale_toInt64(toLimit, scale, &status) = %lld, fromLimit = %lld.\n",
			scale,
			toLimit,
			result,
			fromLimit);
...
static void TestFromInt64(void)
{
	int32_t scale;
	UErrorCode status = U_ZERO_ERROR;
	int64_t result = utmscale_fromInt64(0, (UDateTimeScale)-1, &status);
...
static void TestToInt64(void)
{
	int32_t scale;
	UErrorCode status = U_ZERO_ERROR;
	int64_t result = utmscale_toInt64(0, (UDateTimeScale)-1, &status);
	(void)result; /* suppress set but not used warning. */
	if(status != U_ILLEGAL_ARGUMENT_ERROR) {
