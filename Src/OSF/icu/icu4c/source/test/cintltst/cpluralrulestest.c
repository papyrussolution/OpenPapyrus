			...
			if(U_SUCCESS(status) ) {
				...
				if(u_strcmp(keyword, keywordExpected) != 0) {
					...
					log_data_err("ERROR: uplrules_select for locale %s, number %.1f: expect %s, get %s\n",
					    testItemPtr->locale, testItemPtr->number, testItemPtr->keywordExpected,
					    u_austrcpy(bcharBuf, keyword));
				}
			}
			else {
			...
			if(U_SUCCESS(status) ) {
				...
				if(U_SUCCESS(status) ) {
					...
					if(u_strcmp(keyword, keywordExpected) != 0) {
						char bcharBuf[kKeywordBufLen];
						log_data_err(
							"ERROR: uplrules_selectWithFormat for locale %s, number %.1f: expect %s, get %s\n",
							testItemPtr->locale,
							testItemPtr->number,
							testItemPtr->keywordExpectedForDecimals,
							u_austrcpy(bcharBuf, keyword));
					}
				}
				else {
...
static void TestGetKeywords() {
	...
	for(; itemPtr->locale != NULL; itemPtr++) {
		...
		UEnumeration* uenum;
		bool expectKeywords[kNumKeywords];
		bool getKeywords[kNumKeywords];
		int32_t i, iKnown;
		UErrorCode status = U_ZERO_ERROR;
...
static void TestSelectRange() 
{
	...
	unumrf = unumrf_openForSkeletonWithCollapseAndIdentityFallback(u"", 0, UNUM_RANGE_COLLAPSE_AUTO, UNUM_IDENTITY_FALLBACK_APPROXIMATELY, "sl", NULL, &ec);
