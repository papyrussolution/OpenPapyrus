...
static void TestCharNames();
static void TestUCharFromNameUnderflow();
static void TestMirroring();
static void TestUScriptRunAPI();
static void TestAdditionalProperties();
static void TestNumericProperties();
static void TestPropertyNames();
static void TestPropertyValues();
...
static void parseUCDFile(const char * filename,
 ...
 */
	...
	log_verbose("testing u_istitle\n");
	{
		...
		int32_t num = SIZEOFARRAYi(expected);
		for(i = 0; i<num; i++) {
...
static bool showADiffB(const USet * a, const USet * b,
    const char * a_name, const char * b_name,
    bool expect, bool diffIsError) {
	USet * aa;
...
static bool showAMinusB(const USet * a, const USet * b,
    const char * a_name, const char * b_name,
    bool diffIsError) {
	return showADiffB(a, b, a_name, b_name, TRUE, diffIsError);
}

static bool showAIntersectB(const USet * a, const USet * b,
    const char * a_name, const char * b_name,
    bool diffIsError) {
	return showADiffB(a, b, a_name, b_name, FALSE, diffIsError);
}

static bool compareUSets(const USet * a, const USet * b,
    const char * a_name, const char * b_name,
    bool diffIsError) {
	/*
...
static void testSampleCharProps(bool propFn(UChar32), const char * propName,
    const UChar32 * sampleChars, int32_t sampleCharsLength,
    bool expected) {
	int32_t i;
	for(i = 0; i < sampleCharsLength; ++i) {
		bool result = propFn(sampleChars[i]);
		if(result != expected) {
...
/* Tests for isDefined(u_isdefined)(, isBaseForm(u_isbase()), isSpaceChar(u_isspace()), isWhiteSpace(),
   u_CharDigitValue() */
static void TestMisc()
{
...
/*    static const UChar sampleChars[] = {0x000a, 0x0045, 0x4e00, 0xDC00, 0xFFE8, 0xFFF0};*/
	...
	testSampleCharProps(u_isspace, "u_isspace", sampleSpaces, SIZEOFARRAYi(sampleSpaces), TRUE);
	testSampleCharProps(u_isspace, "u_isspace", sampleNonSpaces, SIZEOFARRAYi(sampleNonSpaces), FALSE);

	testSampleCharProps(u_isJavaSpaceChar, "u_isJavaSpaceChar",
	    sampleSpaces, SIZEOFARRAYi(sampleSpaces), TRUE);
	testSampleCharProps(u_isJavaSpaceChar, "u_isJavaSpaceChar",
	    sampleNonSpaces, SIZEOFARRAYi(sampleNonSpaces), FALSE);

	testSampleCharProps(u_isWhitespace, "u_isWhitespace",
	    sampleWhiteSpaces, SIZEOFARRAYi(sampleWhiteSpaces), TRUE);
	testSampleCharProps(u_isWhitespace, "u_isWhitespace",
	    sampleNonWhiteSpaces, SIZEOFARRAYi(sampleNonWhiteSpaces), FALSE);

	testSampleCharProps(u_isdefined, "u_isdefined",
	    sampleDefined, SIZEOFARRAYi(sampleDefined), TRUE);
	testSampleCharProps(u_isdefined, "u_isdefined",
	    sampleUndefined, SIZEOFARRAYi(sampleUndefined), FALSE);

	testSampleCharProps(u_isbase, "u_isbase", sampleBase, SIZEOFARRAYi(sampleBase), TRUE);
	testSampleCharProps(u_isbase, "u_isbase", sampleNonBase, SIZEOFARRAYi(sampleNonBase), FALSE);

	testSampleCharProps(u_isdigit, "u_isdigit", sampleDigits, SIZEOFARRAYi(sampleDigits), TRUE);
	testSampleCharProps(u_isdigit, "u_isdigit", sampleNonDigits, SIZEOFARRAYi(sampleNonDigits), FALSE);

	for(i = 0; i < SIZEOFARRAYi(sampleDigits); i++) {
	...
	if(strncmp(icuVersion, U_ICU_VERSION, smin((int32_t)strlen(icuVersion), (int32_t)strlen(U_ICU_VERSION))) != 0) {
...
typedef bool U_EXPORT2 IsPOSIXClass (UChar32 c);
...
static const struct {
	...
	{ 0x200b,                                                        ISCN },/* ZWSP */
	/*{ 0x200b,                                         ISPR|ISSP           },*/    /* ZWSP */ /* ZWSP became a
	   control char in 4.0.1*/
	{ 0x200e,                                                        ISCN },/* LRM */
...
static void TestPOSIX() {
	...
	mask = 1;
	for(cl = 0; cl<12; ++cl) {
		for(i = 0; i<SIZEOFARRAYi(posixData); ++i) {
			expect = (bool)((posixData[i].posixResults&mask)!=0);
...
static void TestControlPrint()
{
	...
	UChar32 c;
	testSampleCharProps(u_iscntrl, "u_iscntrl", sampleControl, SIZEOFARRAYi(sampleControl), TRUE);
	testSampleCharProps(u_iscntrl, "u_iscntrl", sampleNonControl, SIZEOFARRAYi(sampleNonControl), FALSE);
	testSampleCharProps(u_isprint, "u_isprint", samplePrintable, SIZEOFARRAYi(samplePrintable), TRUE);
	testSampleCharProps(u_isprint, "u_isprint", sampleNonPrintable, SIZEOFARRAYi(sampleNonPrintable), FALSE);
...
static void TestIdentifier()
{
	...
	testSampleCharProps(u_isJavaIDStart, "u_isJavaIDStart", sampleJavaIDStart, SIZEOFARRAYi(sampleJavaIDStart), TRUE);
	testSampleCharProps(u_isJavaIDStart, "u_isJavaIDStart", sampleNonJavaIDStart, SIZEOFARRAYi(sampleNonJavaIDStart), FALSE);
	testSampleCharProps(u_isJavaIDPart, "u_isJavaIDPart", sampleJavaIDPart, SIZEOFARRAYi(sampleJavaIDPart), TRUE);
	testSampleCharProps(u_isJavaIDPart, "u_isJavaIDPart", sampleNonJavaIDPart, SIZEOFARRAYi(sampleNonJavaIDPart), FALSE);
	/* IDPart should imply IDStart */
	testSampleCharProps(u_isJavaIDPart, "u_isJavaIDPart", sampleJavaIDStart, SIZEOFARRAYi(sampleJavaIDStart), TRUE);
	testSampleCharProps(u_isIDStart, "u_isIDStart", sampleUnicodeIDStart, SIZEOFARRAYi(sampleUnicodeIDStart), TRUE);
	testSampleCharProps(u_isIDStart, "u_isIDStart", sampleNonUnicodeIDStart, SIZEOFARRAYi(sampleNonUnicodeIDStart), FALSE);
	testSampleCharProps(u_isIDPart, "u_isIDPart", sampleUnicodeIDPart, SIZEOFARRAYi(sampleUnicodeIDPart), TRUE);
	testSampleCharProps(u_isIDPart, "u_isIDPart", sampleNonUnicodeIDPart, SIZEOFARRAYi(sampleNonUnicodeIDPart), FALSE);
	/* IDPart should imply IDStart */
	testSampleCharProps(u_isIDPart, "u_isIDPart", sampleUnicodeIDStart, SIZEOFARRAYi(sampleUnicodeIDStart), TRUE);
	testSampleCharProps(u_isIDIgnorable, "u_isIDIgnorable", sampleIDIgnore, SIZEOFARRAYi(sampleIDIgnore), TRUE);
	testSampleCharProps(u_isIDIgnorable, "u_isIDIgnorable", sampleNonIDIgnore, SIZEOFARRAYi(sampleNonIDIgnore), FALSE);
}
...
static bool U_CALLCONV enumTypeRange(const void * context, UChar32 start, UChar32 limit, UCharCategory type) {
	...
	for(i = 0; i<count; ++i) {
		if(start<=test[i][0] && test[i][0]<limit) {
			...
			/* stop at the range that includes the last test code point (increases code coverage for
			   enumeration) */
...
static bool U_CALLCONV enumDefaultsRange(const void * context, UChar32 start, UChar32 limit, UCharCategory type) {
	...
	static const int32_t defaultBidi[][2] = { /* { limit, class } */
		...
		{ 0x20D0, U_EUROPEAN_NUMBER_TERMINATOR }, /* Unicode 6.3 changes the currency symbols block
		                                             U+20A0..U+20CF to default to ET not L */
	...
	if(type==U_UNASSIGNED || type==U_PRIVATE_USE_CHAR) {
		...
		c = start;
		for(i = 0; i<SIZEOFARRAYi(defaultBidi) && c<limit; ++i) {
...
static void TestCodePoint() {
	...
	int32_t i;
	for(i = 0; i<SIZEOFARRAYi(codePoint); i++) {
		...
		else if(i >=18 && i<SIZEOFARRAYi(codePoint)) {
			if(U_IS_SURROGATE(c) || U16_IS_SURROGATE(c)) {
				log_err("ERROR: isSurrogate() failed for U+%04x\n", c);
			}
			if(U_IS_UNICODE_CHAR(c)) {
...
{
	int32_t result = 0;
	const char * matchPosition = strstr(tagStrings, str);
	if(matchPosition == 0) {
...
static bool enumCharNamesFn(void * context,
	...
	++*pCount;
	for(i = 0; i<SIZEOFARRAYi(names); ++i) {
...
static bool enumExtCharNamesFn(void * context,
    UChar32 code, UCharNameChoice nameChoice,
    const char * name, int32_t length) {
...
static void TestMirroring() {
	...
	if(!(u_charMirror(0x3c)==0x3e && u_charMirror(0x5d)==0x5b && u_charMirror(0x208d)==0x208e && u_charMirror(0x3017)==0x3016 &&
	    u_charMirror(0xbb)==0xab && u_charMirror(0x2215)==0x29F5 && u_charMirror(0x29F5)==0x2215 && /* large delta
	                                                                                                   between the
	                                                                                                   code points
	         */
...
static void CheckScriptRuns(UScriptRun * scriptRun, int32_t * runStarts, const RunTestData * testData, int32_t nRuns, const char * prefix)
{
...
static void TestUScriptRunAPI()
{
	...
	static const struct {
		...
		{testData1, SIZEOFARRAYi(testData1)},
		{testData2, SIZEOFARRAYi(testData2)}
	};
...
static void TestAdditionalProperties() {
	...
	/* test u_charAge() */
	for(i = 0; i<SIZEOFARRAYi(charAges); ++i) {
	...
	if(u_getIntPropertyMinValue(UCHAR_DASH)!=0 ||
	    ...
	    u_getIntPropertyMinValue((UProperty)0x2345)!=0) {
	...
	/*JB#2410*/
	if(u_getIntPropertyMaxValue((UProperty)0x2345)!=-1) {
	...
	for(i = 0; i<SIZEOFARRAYi(props); ++i) {
		...
		if(result!=props[i][2]) {
			log_data_err(
				"error: u_getIntPropertyValue(U+%04lx, %s)=%d is wrong, should be %d (props[%d]) - (Are you missing data?)\n",
				c,
				whichName,
				result,
				props[i][2],
		...
		switch((UProperty)props[i][1]) {
			case UCHAR_ALPHABETIC:
			    if(u_isUAlphabetic((UChar32)props[i][0])!=(bool)props[i][2]) {
				    log_err("error: u_isUAlphabetic(U+%04lx)=%d is wrong (props[%d])\n",
					props[i][0], result, i);
			    }
			    break;
			case UCHAR_LOWERCASE:
			    if(u_isULowercase((UChar32)props[i][0])!=(bool)props[i][2]) {
				    log_err("error: u_isULowercase(U+%04lx)=%d is wrong (props[%d])\n",
					props[i][0], result, i);
			    }
			    break;
			case UCHAR_UPPERCASE:
			    if(u_isUUppercase((UChar32)props[i][0])!=(bool)props[i][2]) {
				    log_err("error: u_isUUppercase(U+%04lx)=%d is wrong (props[%d])\n",
					props[i][0], result, i);
			    }
			    break;
			case UCHAR_WHITE_SPACE:
			    if(u_isUWhiteSpace((UChar32)props[i][0])!=(bool)props[i][2]) {
...
static void testFold(UChar32 c, int which,
	...
	if((which&CF_FULL)!=0) {
		length2 = u_strFoldCase(t, SIZEOFARRAYi(t), s, length, 0, &errorCode);
	...
	if((which&CF_TURKIC)!=0) {
		...
		length2 = u_strFoldCase(t, SIZEOFARRAYi(t), s, length, U_FOLD_CASE_EXCLUDE_SPECIAL_I, &errorCode);
...
static void U_CALLCONV caseFoldingLineFn(void * context,
    char * fields[][2], int32_t fieldCount,
    UErrorCode * pErrorCode) {
	(void)fieldCount; // suppress compiler warnings about unused variable
...
static void TestBinaryCharacterPropertiesAPI() {
	...
	UErrorCode errorCode = U_ZERO_ERROR;
	const USet * set = u_getBinaryPropertySet((UProperty)-1, &errorCode);
	if(U_SUCCESS(errorCode)) {
...
static void TestIntCharacterPropertiesAPI() 
{
	// API test only. See intltest/ucdtest.cpp for functional test.
	UErrorCode errorCode = U_ZERO_ERROR;
	const UCPMap * map = u_getIntPropertyMap((UProperty)(UCHAR_INT_START - 1), &errorCode);
	if(U_SUCCESS(errorCode)) {
