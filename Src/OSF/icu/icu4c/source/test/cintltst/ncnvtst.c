...
static void TestAvailableConverters() {
	...
	for(i = 0; i < ucnv_countAvailable(); i++) {
		...
		if(U_FAILURE(status)) {
			log_err(
				"ERROR: converter creation failed. Failure in alias table or the data table for \n converter=%s. Error=%s\n",
    ...
    const char * codepage, UConverterFromUCallback callback, const int32_t * expectOffsets, bool testReset)
{
	...
	do {
		...
		doFlush = (bool)(sourceLimit == realSourceEnd);
    ...
    const char * codepage, UConverterToUCallback callback, const int32_t * expectOffsets, bool testReset)
{
	...
	do {
		...
		if(targ == realBufferEnd) {
			log_err("Error, the end would overflow the real output buffer while about to call toUnicode! tarjey=%08lx %s",
			    targ,
			    gNuConvTestName);
			return FALSE;
		}
		...
		status = U_ZERO_ERROR;
		doFlush = (bool)((gInBufferSize ==999 && gOutBufferSize==999) ? (srcLimit == realSourceEnd) : FALSE);
...
static void doTestTruncated(const char * cnvName, const uint8_t * bytes, int32_t length) {
	...
	if(errorCode!=U_TRUNCATED_CHAR_FOUND || target!=buffer) {
		log_err(
			"error TestTruncated(%s, 1b): no input (previously %d), flush=TRUE: %s (should be U_TRUNCATED_CHAR_FOUND), output %d\n",
			cnvName,
			(int)length,
	...
	if(errorCode!=U_TRUNCATED_CHAR_FOUND || source!=sourceLimit || target!=buffer) {
		log_err(
			"error TestTruncated(%s, 2): input bytes[%d], flush=TRUE: %s (should be U_TRUNCATED_CHAR_FOUND), input left %d, output %d\n",
			cnvName,
			length,
			u_errorName(errorCode),
			(int)(sourceLimit-source),
			(int)(target-buffer));
	}
