...
static void TestTextAttributeCrash();
static void TestNBSPInPattern();
static void TestInt64Parse();
static void TestParseCurrency();
static void TestMaxInt();
static void TestNoExponent();
static void TestSignAlwaysShown();
static void TestMinimumGroupingDigits();
static void TestParseCaseSensitive();
static void TestUFormattable();
static void TestUNumberingSystem();
static void TestCurrencyIsoPluralFormat();
static void TestContext();
static void TestCurrencyUsage();
static void TestCurrFmtNegSameAsPositive();
static void TestVariousStylesAndAttributes();
...
static void TestParseCases();
static void TestSetMaxFracAndRoundIncr();
static void TestIgnorePadding();
static void TestSciNotationMaxFracCap();
static void TestMinIntMinFracZero();
static void Test21479_ExactCurrency();
...
static void TestNumberFormat()
{
	...
	/*UNumberFormatSymbol*/int symType = UNUM_DECIMAL_SEPARATOR_SYMBOL;
	int32_t newvalue;
	...
	UNumberFormat * cur_fr;
	UNumberFormat * per_def;
	UNumberFormat * per_fr;
	UNumberFormat * cur_frpattern;
	UNumberFormat * myclone;
	UNumberFormat * spellout_def;
...
/*    for (i = 0; i < 100000; i++)*/
	...
	if(U_FAILURE(status)) {
		log_err("parse of '%s' failed. Parsepos=%d. The error is  : %s\n",
		    aescstrdup(result, u_strlen(result)),
		    parsepos,
		    myErrorName(status));
	}
...
/*    for (i = 0; i < 100000; i++) */
	...
	log_verbose("\nTesting unum_getSymbols and unum_setSymbols()\n");
	/*when we try to change the symbols of french to default we need to apply the pattern as well to fetch correct
	   results */
	resultlength = 0;
	...
	for(symType = UNUM_DECIMAL_SEPARATOR_SYMBOL; symType < UNUM_FORMAT_SYMBOL_COUNT; symType++) {
		status = U_ZERO_ERROR;
		unum_getSymbol(cur_def, (UNumberFormatSymbol)symType, temp1, sizeof(temp1), &status);
		unum_setSymbol(cur_frpattern, (UNumberFormatSymbol)symType, temp1, -1, &status);
		if(U_FAILURE(status)) {
	...
	for(symType = UNUM_DECIMAL_SEPARATOR_SYMBOL; symType < UNUM_FORMAT_SYMBOL_COUNT; symType++) {
		status = U_ZERO_ERROR;
		unum_getSymbol(cur_def, (UNumberFormatSymbol)symType, temp1, sizeof(temp1), &status);
		unum_getSymbol(cur_frpattern, (UNumberFormatSymbol)symType, temp2, sizeof(temp2), &status);
	 ...
	 * Then, remove the strcmp("1.5", ...) and this comment, and the include "cstring.h" at the beginning of
	 *this file.
	 */
	if(u_strcmp(result, temp1) != 0) {
	...
	for(i = 0; i < UNUM_FORMAT_SYMBOL_COUNT; ++i) {
		symbol[0] = (char16_t)(0x41 + i);
		symbol[1] = (char16_t)(0x61 + i);
	...
	for(i = 0; i < UNUM_FORMAT_SYMBOL_COUNT; ++i) {
		resultlength = unum_getSymbol(cur_frpattern, (UNumberFormatSymbol)i, symbol, SIZEOFARRAYi(symbol), &status);
		if(U_FAILURE(status)) {
	...
	unum_getSymbol(cur_frpattern, (UNumberFormatSymbol)i, symbol, SIZEOFARRAYi(symbol), &status);
	if(U_SUCCESS(status)) {
	...
	if(spellout_def) {
		...
		for(i = 0; i < SIZEOFARRAYi(values); ++i) {
			char16_t buffer[128];
			int32_t len;
			int32_t value = values[i];
			status = U_ZERO_ERROR;
			len = unum_format(spellout_def, value, buffer, SIZEOFARRAYi(buffer), NULL, &status);
			if(U_FAILURE(status)) {
				...
				int32_t pp = 0;
				int32_t parseResult;
				/*ustrToAstr(buffer, len, logbuf, SIZEOFARRAYi(logbuf));*/
	...
	{ /* Test for ticket #7079 */
		...
		char16_t groupingSep[] = { 0 };
		char16_t numPercent[] = { 0x0031, 0x0032, 0x0025, 0 }; /* "12%" */
	...
	/* Test invalid symbol argument */
	{
		...
		if(U_FAILURE(status)) {
			...
			if(U_SUCCESS(status)) log_err(
					"unum_getSymbol()'s status should be ILLEGAL_ARGUMENT with invalid symbol (> UNUM_FORMAT_SYMBOL_COUNT) argument\n");

			status = U_ZERO_ERROR;
			unum_getSymbol(fmt, (UNumberFormatSymbol)badsymbolSmall, NULL, 0, &status);
			if(U_SUCCESS(status)) log_err(
					"unum_getSymbol()'s status should be ILLEGAL_ARGUMENT with invalid symbol (less than 0) argument\n");

			status = U_ZERO_ERROR;
			unum_setSymbol(fmt, (UNumberFormatSymbol)badsymbolLarge, value, valueLength, &status);
			if(U_SUCCESS(status)) log_err(
					"unum_setSymbol()'s status should be ILLEGAL_ARGUMENT with invalid symbol (> UNUM_FORMAT_SYMBOL_COUNT) argument\n");

			status = U_ZERO_ERROR;
			unum_setSymbol(fmt, (UNumberFormatSymbol)badsymbolSmall, value, valueLength, &status);
			if(U_SUCCESS(status)) log_err(
					"unum_setSymbol()'s status should be ILLEGAL_ARGUMENT with invalid symbol (less than 0) argument\n");
...
static void TestParseZero(void)
{
	...
	char16_t input[] = {0x30, 0}; /*  Input text is decimal '0' */
	char16_t pat[] = {0x0023, 0x003b, 0x0023, 0}; /*  {'#', ';', '#', 0}; */
	double dbl;
...
static const char16_t dollars2Sym[] = { 0x24, 0x32, 0x2E, 0x30, 0x30, 0 }; /* $2.00 */
static const char16_t dollars4Sym[] = { 0x24, 0x34, 0 }; /* $4 */
static const char16_t dollarsUS4Sym[] = { 0x55, 0x53, 0x24, 0x34, 0 }; /* US$4 */
static const char16_t dollars9Sym[] = { 0x39, 0xA0, 0x24, 0 }; /* 9 $ */
static const char16_t pounds3Sym[]  = { 0xA3, 0x33, 0x2E, 0x30, 0x30, 0 }; /* [POUND]3.00 */
static const char16_t pounds5Sym[]  = { 0xA3, 0x35, 0 }; /* [POUND]5 */
static const char16_t pounds7Sym[]  = { 0x37, 0xA0, 0xA3, 0 }; /* 7 [POUND] */
static const char16_t euros4Sym[] = { 0x34, 0x2C, 0x30, 0x30, 0xA0, 0x20AC, 0 }; /* 4,00 [EURO] */
static const char16_t euros6Sym[] = { 0x36, 0xA0, 0x20AC, 0 }; /* 6 [EURO] */
static const char16_t euros8Sym[] = { 0x20AC, 0x38, 0 }; /* [EURO]8 */
static const char16_t dollars4PluEn[] = { 0x34, 0x20, 0x55, 0x53, 0x20, 0x64, 0x6F, 0x6C, 0x6C, 0x61, 0x72, 0x73, 0 }; /* 4
                                                                                                                       US
                                                                                                                       dollars*/
static const char16_t pounds5PluEn[]  =
{ 0x35, 0x20, 0x42, 0x72, 0x69, 0x74, 0x69, 0x73, 0x68, 0x20, 0x70, 0x6F, 0x75, 0x6E, 0x64, 0x73, 0x20, 0x73, 0x74, 0x65, 0x72, 0x6C,
  0x69, 0x6E, 0x67, 0 };                                                                                                                                                 /*
                                                                                                                                                                            5
                                                                                                                                                                            British
                                                                                                                                                                            pounds
                                                                                                                                                                            sterling
 */
static const char16_t euros8PluEn[] = { 0x38, 0x20, 0x65, 0x75, 0x72, 0x6F, 0x73, 0 }; /* 8 euros*/
static const char16_t euros6PluFr[] = { 0x36, 0x20, 0x65, 0x75, 0x72, 0x6F, 0x73, 0 }; /* 6 euros*/

typedef struct {
	const char * locale;
	const char * descrip;
	const char16_t * currStr;
	const char16_t * plurStr;
...
static const ParseCurrencyItem parseCurrencyItems[] = {
	...
	{ NULL,    NULL,       NULL,        NULL,           U_ZERO_ERROR,  0, 0.0, U_ZERO_ERROR, 0, 0.0, NULL  }
};
...
static void TestParseCurrency()
{
	...
	for(itemPtr = parseCurrencyItems; itemPtr->locale != NULL; ++itemPtr) {
		...
		double parseVal;
		int32_t parsePos;
		char16_t parseCurr[4];
		char parseCurrB[4];

		status = U_ZERO_ERROR;
		unum = unum_open(UNUM_CURRENCY, NULL, 0, itemPtr->locale, NULL, &status);
		if(U_SUCCESS(status)) {
			const char16_t * currStr = itemPtr->currStr;
			status = U_ZERO_ERROR;
			parsePos = 0;
			parseVal = unum_parseDouble(unum, currStr, -1, &parsePos, &status);
			if(status != itemPtr->parsDoubExpectErr || parsePos != itemPtr->parsDoubExpectPos ||
			    parseVal != itemPtr->parsDoubExpectVal) {
			...
			if(status != itemPtr->parsCurrExpectErr || parsePos != itemPtr->parsCurrExpectPos ||
			    parseVal != itemPtr->parsCurrExpectVal ||
				...
				log_err(
					...
					itemPtr->locale,
					itemPtr->descrip,
					u_errorName(itemPtr->parsCurrExpectErr),
					itemPtr->parsCurrExpectPos,
					itemPtr->parsCurrExpectVal,
					itemPtr->parsCurrExpectCurr,
					u_errorName(status),
					parsePos,
					parseVal,
					parseCurrB);
		...
		if(itemPtr->plurStr != NULL) {
			...
			if(U_SUCCESS(status)) {
				...
				if(status != itemPtr->parsCurrExpectErr || parseVal != itemPtr->parsCurrExpectVal ||
				    strncmp(parseCurrB, itemPtr->parsCurrExpectCurr, 4) != 0) {
					log_err(
						"UNUM_CURRENCY parseDoubleCurrency Plural %s/%s, expect %s val %.1f cur %s, get %s val %.1f cur %s\n",
						itemPtr->locale,
						itemPtr->descrip,
						u_errorName(itemPtr->parsCurrExpectErr),
						itemPtr->parsCurrExpectVal,
...
static const char16_t ustr_en0[] = {0x7A, 0x65, 0x72, 0x6F, 0}; /* zero */
static const char16_t ustr_123[] = {0x31, 0x32, 0x33, 0}; /* 123 */
static const char16_t ustr_en123[] = {0x6f, 0x6e, 0x65, 0x20, 0x68, 0x75, 0x6e, 0x64, 0x72, 0x65, 0x64,
				   0x20, 0x74, 0x77, 0x65, 0x6e, 0x74, 0x79,
				   0x2d, 0x74, 0x68, 0x72, 0x65, 0x65, 0}; /* one hundred twenty-three */
static const char16_t ustr_fr123[] = {0x63, 0x65, 0x6e, 0x74, 0x20, 0x76, 0x69, 0x6e, 0x67, 0x74, 0x2d,
				   0x74, 0x72, 0x6f, 0x69, 0x73, 0}; /* cent vingt-trois */
static const char16_t ustr_ja123[] = {0x767e, 0x4e8c, 0x5341, 0x4e09, 0}; /* kanji 100(+)2(*)10(+)3 */
...
static const SpelloutParseTest spelloutParseTests[] = {
	...
	{ NULL,    NULL, NULL,        0,   0,  0, U_ZERO_ERROR }/* terminator */
};
...
static void TestSpelloutNumberParse()
{
	...
	for(testPtr = spelloutParseTests; testPtr->testname != NULL; ++testPtr) {
		...
		if(value != testPtr->value || position != testPtr->endPos || status != testPtr->status) {
			log_err(
				"unum_parse SPELLOUT, locale %s, testname %s, startPos %d: for value / endPos / status, expected %d / %d / %s, got %d / %d / %s\n",
				testPtr->locale,
				testPtr->testname,
				testPtr->startPos,
				testPtr->value,
				testPtr->endPos,
				myErrorName(testPtr->status),
				value,
				position,
				myErrorName(status));
		}
...
static void TestSigDigRounding()
{
	...
	(void)unum_formatDouble(fmt, d, result, SIZEOFARRAYi(result), NULL, &status);
	if(U_FAILURE(status)) {
		log_err("Error in formatting using unum_formatDouble(.....): %s\n", myErrorName(status));
		return;
	}
...
/*        u_uastrcpy(temp1, "(xxxxxxx10,456.37)"); */
		...
		if(U_FAILURE(status)) {
			...
			if(pos1.beginIndex == 13 && pos1.endIndex == 15)
				...
				log_err(
					"Fail: Error in complete number Formatting using unum_formatDouble()\nGot: b=%d end=%d\nExpected: b=13 end=15\n",
					pos1.beginIndex,
					pos1.endIndex);
...
static bool withinErr(double a, double b, double err) {
...
static void test_fmt(UNumberFormat * fmt, bool isDecimal) {
	char temp[512];
	char16_t buffer[512];
	int32_t BUFSIZE = SIZEOFARRAYi(buffer);
	double vals[] = {
		-.2, 0, .2, 5.5, 15.2, 250, 123456789
	};
	int i;

	for(i = 0; i < SIZEOFARRAYi(vals); ++i) {
		UErrorCode status = U_ZERO_ERROR;
...
static void TestNonExistentCurrency() {
	...
	UErrorCode status = U_ZERO_ERROR;
	char16_t currencySymbol[8];
	static const char16_t QQQ[] = {0x51, 0x51, 0x51, 0};
...
static void TestRBNFFormat() {
	...
	UParseError perr;
	char16_t pat[1024];
	char16_t tempUChars[512];
	UNumberFormat * formats[5];
	int COUNT = SIZEOFARRAYi(formats);
	int i;
	...
	for(i = 0; i < COUNT; ++i) {
		log_verbose("\n\ntesting format %d\n", i);
		test_fmt(formats[i], (bool)(i == 0));
	}

    #define FORMAT_BUF_CAPACITY 64
	{
		char16_t fmtbuf[FORMAT_BUF_CAPACITY];
		int32_t len;
		...
		if(U_FAILURE(status)) {
			...
			char16_t nansym[] = { 0x4E, 0x61, 0x4E, 0 }; /* NaN */
...
static void TestRBNFRounding() {
	char16_t fmtbuf[FORMAT_BUF_CAPACITY];
	char16_t expectedBuf[FORMAT_BUF_CAPACITY];
	int32_t len;
	UErrorCode status = U_ZERO_ERROR;
...
static void TestTextAttributeCrash() {
	char16_t ubuffer[64] = {0x0049, 0x004E, 0x0052, 0};
	static const char16_t expectedNeg[] = {0x0049, 0x004E, 0x0052, 0x0031, 0x0032, 0x0033, 0x0034, 0x002E, 0x0035, 0};
	static const char16_t expectedPos[] = {0x0031, 0x0032, 0x0033, 0x0034, 0x002E, 0x0035, 0};
	int32_t used;
	UErrorCode status = U_ZERO_ERROR;
...
#define SPECIAL_PATTERN "\\u00A4\\u00A4'\\u062f.\\u0625.\\u200f\\u00a0'###0.00"
		...
		u_unescape(SPECIAL_PATTERN, pat, SIZEOFARRAYi(pat));
...
static void TestCloneWithRBNF() {
	...
	if(u_strcmp(buffer, buffer_cloned)) {
		log_data_err(
			"Result from cloned formatter not identical to the original. Original: %s Cloned: %s - (Are you missing data?)",
			u_austrcpy(temp1, buffer),
			u_austrcpy(temp2, buffer_cloned));
	}
}
...
static void TestMaxInt() {
	UErrorCode status = U_ZERO_ERROR;
	char16_t pattern_hash[] = { 0x23, 0x00 }; /* "#" */
	char16_t result1[1024] = { 0 }, result2[1024] = { 0 };
	int32_t len1, len2;
	char16_t expect[] = { 0x0039, 0x0037, 0 };
...
static void TestUFormattable() {
	...
	// simple test for API docs
	{
		...
		if(assertSuccessCheck("calling unum_open()", &status, TRUE)) {
			...
			const char16_t str[] = { 0x0031, 0x0032, 0x0033, 0x0000 }; /* 123 */
			int32_t result = 0;
...
static const char16_t latnDesc[] = {0x0030, 0x0031, 0x0032, 0x0033, 0x0034, 0x0035, 0x0036, 0x0037, 0x0038, 0x0039, 0}; // 0123456789
static const char16_t romanDesc[] = {0x25, 0x72, 0x6F, 0x6D, 0x61, 0x6E, 0x2D, 0x75, 0x70, 0x70, 0x65, 0x72, 0}; // %roman-upper
static const char16_t arabDesc[] = {0x0660, 0x0661, 0x0662, 0x0663, 0x0664, 0x0665, 0x0666, 0x0667, 0x0668, 0x0669, 0}; //
static const char16_t arabextDesc[] = {0x06F0, 0x06F1, 0x06F2, 0x06F3, 0x06F4, 0x06F5, 0x06F6, 0x06F7, 0x06F8, 0x06F9, 0}; //
static const char16_t hanidecDesc[] = {0x3007, 0x4E00, 0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0}; //
static const char16_t hantDesc[] = {0x7A, 0x68, 0x5F, 0x48, 0x61, 0x6E, 0x74, 0x2F, 0x53, 0x70, 0x65, 0x6C, 0x6C, 0x6F, 0x75, 0x74,
...
static void TestUNumberingSystem() {
	...
	for(itemPtr = numSysTestItems; itemPtr->locale != NULL; itemPtr++) {
		...
		if(U_SUCCESS(status)) {
			char16_t ubuf[kNumSysDescripBufMax];
			...
			if(strcmp(numsys,
			    itemPtr->numsys) != 0 || radix != itemPtr->radix || !isAlgorithmic != !itemPtr->isAlgorithmic) {
	...
	for(int i = 0; i<3; ++i) {
		...
		if(U_SUCCESS(status)) {
			...
			while((numsys = uenum_next(uenum, NULL, &status)) != NULL && U_SUCCESS(status)) {
				...
				if(U_SUCCESS(status)) {
					numsysCount++;
					if(strcmp(numsys, "latn") ) foundLatn = TRUE;
					if(strcmp(numsys, "arab") ) foundArab = TRUE;
					unumsys_close(unumsys);
				}
				else {
					log_err(
						"unumsys_openAvailableNames includes %s but unumsys_openByName on it fails with status %s\n",
						numsys,
						myErrorName(status));
				}
			}
...
static void TestCurrencyIsoPluralFormat() {
	static const char * DATA[][8] = {
		...
		// locale             amount     ISOcode CURRENCYSTYLE         CURRENCY_STANDARD     CURRENCY_ACCOUNTING
		//   ISOCURRENCYSTYLE          PLURALCURRENCYSTYLE
		{"en_US",             "1",        "USD", "$1.00",              "$1.00",              "$1.00",              "USD\\u00A01.00",
		 "1.00 US dollars"},
		{"en_US",             "1234.56",  "USD", "$1,234.56",          "$1,234.56",          "$1,234.56",
		 "USD\\u00A01,234.56",    "1,234.56 US dollars"},
		{"en_US@cf=account",  "1234.56",  "USD", "$1,234.56",          "$1,234.56",          "$1,234.56",
		 "USD\\u00A01,234.56",    "1,234.56 US dollars"},
		{"en_US",             "-1234.56", "USD", "-$1,234.56",         "-$1,234.56",         "($1,234.56)",
		 "-USD\\u00A01,234.56",   "-1,234.56 US dollars"},
		{"en_US@cf=account",  "-1234.56", "USD", "($1,234.56)",        "-$1,234.56",         "($1,234.56)",
		 "-USD\\u00A01,234.56",   "-1,234.56 US dollars"},
		{"en_US@cf=standard", "-1234.56", "USD", "-$1,234.56",         "-$1,234.56",         "($1,234.56)",
		 "-USD\\u00A01,234.56",   "-1,234.56 US dollars"},
		{"zh_CN",             "1",        "USD", "US$1.00",            "US$1.00",            "US$1.00",            "USD\\u00A01.00",
		 "1.00\\u00A0\\u7F8E\\u5143"},
		{"zh_CN",             "-1",       "USD", "-US$1.00",           "-US$1.00",           "(US$1.00)",
		 "-USD\\u00A01.00",       "-1.00\\u00A0\\u7F8E\\u5143"},
		{"zh_CN@cf=account",  "-1",       "USD", "(US$1.00)",          "-US$1.00",           "(US$1.00)",
		 "-USD\\u00A01.00",       "-1.00\\u00A0\\u7F8E\\u5143"},
		{"zh_CN@cf=standard", "-1",       "USD", "-US$1.00",           "-US$1.00",           "(US$1.00)",
		 "-USD\\u00A01.00",       "-1.00\\u00A0\\u7F8E\\u5143"},
		{"zh_CN",             "1234.56",  "USD", "US$1,234.56",        "US$1,234.56",        "US$1,234.56",
		 "USD\\u00A01,234.56",    "1,234.56\\u00A0\\u7F8E\\u5143"},
		// {"zh_CN",          "1",        "CHY", "CHY1.00",            "CHY1.00",            "CHY1.00",
		//            "CHY1.00",        "1.00 CHY"}, // wrong ISO code
		// {"zh_CN",          "1234.56",  "CHY", "CHY1,234.56",        "CHY1,234.56",        "CHY1,234.56",
		//        "CHY1,234.56",    "1,234.56 CHY"}, // wrong ISO code
		{"zh_CN",             "1",        "CNY", "\\u00A51.00",        "\\u00A51.00",        "\\u00A51.00",        "CNY\\u00A01.00",
		 "1.00\\u00A0\\u4EBA\\u6C11\\u5E01"},
		{"zh_CN",             "1234.56",  "CNY", "\\u00A51,234.56",    "\\u00A51,234.56",    "\\u00A51,234.56",
		 "CNY\\u00A01,234.56",    "1,234.56\\u00A0\\u4EBA\\u6C11\\u5E01"},
		{"ru_RU",             "1",        "RUB", "1,00\\u00A0\\u20BD", "1,00\\u00A0\\u20BD", "1,00\\u00A0\\u20BD", "1,00\\u00A0RUB",
		 "1,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E "
		 "\\u0440\\u0443\\u0431\\u043B\\u044F"},
		{"ru_RU",             "2",        "RUB", "2,00\\u00A0\\u20BD", "2,00\\u00A0\\u20BD", "2,00\\u00A0\\u20BD", "2,00\\u00A0RUB",
		 "2,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E "
		 "\\u0440\\u0443\\u0431\\u043B\\u044F"},
		{"ru_RU",             "5",        "RUB", "5,00\\u00A0\\u20BD", "5,00\\u00A0\\u20BD", "5,00\\u00A0\\u20BD", "5,00\\u00A0RUB",
		 "5,00 \\u0440\\u043E\\u0441\\u0441\\u0438\\u0439\\u0441\\u043A\\u043E\\u0433\\u043E "
		 "\\u0440\\u0443\\u0431\\u043B\\u044F"},
		// test locale without currency information
		{"root",              "-1.23",    "USD", "-US$\\u00A01.23",    "-US$\\u00A01.23",    "-US$\\u00A01.23",
		 "-USD\\u00A01.23", "-1.23 USD"},
		{"root@cf=account",   "-1.23",    "USD", "-US$\\u00A01.23",    "-US$\\u00A01.23",    "-US$\\u00A01.23",
		 "-USD\\u00A01.23", "-1.23 USD"},
		// test choice format
		{"es_AR",             "1",        "INR", "INR\\u00A01,00",     "INR\\u00A01,00",     "INR\\u00A01,00",     "INR\\u00A01,00",
		 "1,00 rupia india"},
	};
	...
	for(i = 0; i<SIZEOFARRAYi(DATA); ++i) {
		...
		for(sIndex = 0; sIndex < SIZEOFARRAYi(currencyStyles); ++sIndex) {
			UNumberFormatStyle style = currencyStyles[sIndex];
			UErrorCode status = U_ZERO_ERROR;
			char16_t currencyCode[4];
			char16_t ubufResult[kUBufMax];
			...
			if(U_FAILURE(status)) {
				...
				if(ulenRes != ulenExp || u_strncmp(ubufResult, ubufExpected, ulenExp) != 0) {
					log_err(
						"FAIL: unum_formatDouble, locale %s, UNUM_CURRENCY_CODE %s, expected %s, got something else\n",
						localeString,
						currencyISOCode,
						DATA[i][3 + sIndex]);
				}
...
static void TestCurrencyUsage() {
	...
	for(i = 0; i<2; i++) {
		...
		for(j = 0; j<SIZEOFARRAYi(DATA); ++j) {
			...
			expectLen = u_unescape(DATA[j][1], expect, SIZEOFARRAYi(expect));

			unum_setTextAttribute(unumFmt, UNUM_CURRENCY_CODE, currencyCode, 3, &status);
			assertSuccess("num_setTextAttribute()", &status);

			resultLen = unum_formatDouble(unumFmt, numberToBeFormat, result, SIZEOFARRAYi(result),
				&pos, &status);
...
static char16_t currFmtNegSameAsPos[] = /* "\u00A4#,##0.00;\u00A4#,##0.00" */
...
//static char16_t currFmtToPatExpected[] = /* "\u00A4#,##0.00" */
//    {0xA4,0x23,0x2C,0x23,0x23,0x30,0x2E,0x30,0x30,0};
static const char16_t * currFmtToPatExpected = currFmtNegSameAsPos;

static char16_t currFmtResultExpected[] = /* "$100.00" */
{0x24, 0x31, 0x30, 0x30, 0x2E, 0x30, 0x30, 0};

static char16_t emptyString[] = {0};
		...
		if(U_SUCCESS(status)) {
			...
			if(U_FAILURE(status)) {
				...
				log_err("unum_toPattern result wrong, expected %s, got %s\n",
				    aescstrdup(currFmtToPatExpected, -1),
				    aescstrdup(ubuf, ulen));
			}
	...
	for(lsaTestPtr = lsaTests; lsaTestPtr->locale != NULL; lsaTestPtr++) {
		...
		if(U_FAILURE(status)) {
			...
			for(veItemPtr = lsaTestPtr->veItems; veItemPtr->expected != NULL; veItemPtr++) {
				char16_t uexp[kUBufSize];
				char16_t uget[kUBufSize];
				...
				if(U_FAILURE(status)) {
					...
					log_err(
						"FAIL: unum_formatDouble style %d, locale %s, attr %d, value %.2f: expect \"%s\", get \"%s\"\n",
						(int)lsaTestPtr->style,
						lsaTestPtr->locale,
						lsaTestPtr->attribute,
						veItemPtr->value,
...
static const char16_t currpat[]  = { 0xA4, 0x23, 0x2C, 0x23, 0x23, 0x30, 0x2E, 0x30, 0x30, 0};
static const char16_t parsetxt[] = { 0x78, 0x30, 0x79, 0x24, 0 }; /* x0y$ */
...
static const char16_t patNoFields[] = { 0x0027, 0x0078, 0x0027, 0 }; /* "'x'", for UNUM_PATTERN_DECIMAL */
			...
			if(U_FAILURE(status)) {
				...
				if(U_FAILURE(status)) {
					...
					for(fptr = itemPtr->expectedFields; TRUE; fptr++) {
						...
						if(field != fptr->field ||
						    (field >= 0 && (beginPos != fptr->beginPos || endPos != fptr->endPos))) {
							if(fptr->field >= 0) {
								log_err(
									"unum_formatDoubleForFields for locale %s as \"%s\"; expect field %d range %d-%d, get field %d range %d-%d\n",
									itemPtr->locale,
									aescstrdup(ubuf, ulen),
									fptr->field,
									fptr->beginPos,
								...
								log_err(
									"unum_formatDoubleForFields for locale %s as \"%s\"; expect field < 0, get field %d range %d-%d\n",
									itemPtr->locale,
									aescstrdup(ubuf, ulen),
									field,
									beginPos,
...
static void Test12052_NullPointer() {
	UErrorCode status = U_ZERO_ERROR;
	static const char16_t input[] = u"199a";
	char16_t currency[200] = {0};
	UNumberFormat * theFormatter = unum_open(UNUM_CURRENCY, NULL, 0, "en_US", NULL, &status);
	if(!assertSuccessCheck("unum_open() failed", &status, TRUE)) {
		return;
	}
	status = U_ZERO_ERROR;
...
typedef struct {
	const char * locale;
	const char16_t * text;   // text to parse
	bool lenient;       // leniency to use
	bool intOnly;       // whether to set PARSE_INT_ONLY
...
static const ParseCaseItem parseCaseItems[] = {
	...
	{ NULL, NULL, 0, 0, U_ZERO_ERROR, 0, 0, U_ZERO_ERROR, 0, 0.0, U_ZERO_ERROR, 0, NULL }
};
...
static void TestParseCases() 
{
	...
	for(itemPtr = parseCaseItems; itemPtr->locale != NULL; itemPtr++) {
		...
		if(status != itemPtr->intStatus || parsePos != itemPtr->intPos || intValue != itemPtr->intValue) {
			...
			log_err(
				"locale %s, text \"%s\", lenient %d, intOnly %d;\n      parse        expected status %s, pos %d, value %d;\n                   got   %s, %d, %d\n",
				itemPtr->locale,
				btext,
				itemPtr->lenient,
				itemPtr->intOnly,
				u_errorName(itemPtr->intStatus),
				itemPtr->intPos,
				itemPtr->intValue,
				u_errorName(status),
				parsePos,
		...
		if(status != itemPtr->doubStatus || parsePos != itemPtr->doubPos || doubValue != itemPtr->doubValue) {
			...
			log_err(
				...
				itemPtr->lenient,
				itemPtr->intOnly,
				u_errorName(itemPtr->doubStatus),
				itemPtr->doubPos,
				itemPtr->doubValue,
				u_errorName(status),
				parsePos,
		...
		(void)dclen;
		if(status != itemPtr->decStatus || parsePos != itemPtr->decPos || strcmp(decstr, itemPtr->decString) != 0) {
			char btext[32];
			u_austrcpy(btext, itemPtr->text);
			log_err(
...
typedef struct {
	...
	int32_t maxFrac;
	double roundIncr;
	const char16_t * expPattern;
	double valueToFmt;
	const char16_t * expFormat;
...
static void TestSetMaxFracAndRoundIncr() {
	...
	for(itemPtr = maxFracAndRoundIncrItems; itemPtr->descrip != NULL; itemPtr++) {
		...
		bool roundIncrUsed = (itemPtr->roundIncr != 0.0 && !ignoreRoundingIncrement(itemPtr->roundIncr, itemPtr->maxFrac));
...
static void TestSciNotationMaxFracCap() 
{
	static const char16_t * pat1 = u"#.##E+00;-#.##E+00";
	UErrorCode status = U_ZERO_ERROR;
