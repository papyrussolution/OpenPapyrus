...
static void TestDefaultKeyword();
static void TestBengaliSortKey();
...
void TestGetSetAttr() {
	...
	struct attrTest {
		...
		{UCOL_ALTERNATE_HANDLING, {UCOL_NON_IGNORABLE, UCOL_SHIFTED}, 2, UCOL_OFF}, /* attribute for handling
		                                                                              variable elements*/
		{UCOL_CASE_FIRST, {UCOL_OFF, UCOL_LOWER_FIRST, UCOL_UPPER_FIRST}, 3, UCOL_SHIFTED}, /* who goes first,
		                                                                                      lower case or
		                                                                                      uppercase */
	...
	for(i = 0; i<SIZEOFARRAYi(attrs); i++) {
...
void TestProperty()
{
	...
	if(0!=memcmp(versionUCAArray, versionUCDArray, 4)) {
...
void TestSafeClone() {
	...
	doAssert( (ucol_greater(col, umlautUStr, u_strlen(umlautUStr), oeStr, u_strlen(oeStr))),
	    "Original German phonebook collation sorts differently than expected");
	doAssert( (ucol_greater(someClonedCollators[0], umlautUStr, u_strlen(umlautUStr), oeStr, u_strlen(oeStr))),
	...
	for(idx = 0; idx < CLONETEST_COLLATOR_COUNT; idx++) {
		...
		doAssert( (ucol_equal(someCollators[idx], test1, u_strlen(test1), test2, u_strlen(test2))),
		...
		doAssert( (ucol_greater(someClonedCollators[idx], test1, u_strlen(test1), test2, u_strlen(test2))),
		    "Result should be \"abCda\" >>> \"abcda\" ");
...
void TestCloneBinary() {
	...
	if(U_FAILURE(err)) {
		...
		k1 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * l1);
		k2 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * l2);
...
static void TestBengaliSortKey(void)
{
	...
	uint32_t sortKeyStrLen1 = SIZEOFARRAYi(sortKeyStr1);
	...
	sortKey1 = (uint8_t*)SAlloc::M(sortKeyLen1+1);
	...
	sortKey2 = (uint8_t*)SAlloc::M(sortKeyLen2+1);
...
void TestOpenVsOpenRules() {
	...
	uint32_t sortKeyStrLen1 = SIZEOFARRAYi(sortKeyStr1),
	    sortKeyStrLen2 = SIZEOFARRAYi(sortKeyStr2);
	ULocaleData * uld;
	int32_t x, y, z;
	USet * eSet;
	int32_t eSize;
	...
	for(x = 0; x < numLocales; x += adder) {
		...
		for(y = 0; y < 5; y++) {
			...
			/* if there are some non-normal characters left, put a few into the string, just to make sure we
			   have some */
			if(eSize > 0) {
			...
			sortKey1 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortKeyLen1 + 1));
			...
			sortKey2 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortKeyLen2 + 1));
...
void TestSortKey()
{
	...
	if(strcmpResult <= 0) {
		log_err(
			"ERR: expected strcmp(\"9f 00\", \"61 00\") to be >=0 (GREATER).. got %d. Calling strcmp() for sortkeys may not work! \n",
			strcmpResult);
	}
	...
	sortk1 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklen+1));
	...
	sortk2 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklen+1));
	...
	sortk3 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklen+1));
	...
	sortkEmpty = (uint8_t*)SAlloc::M(sizeof(uint8_t) * sortklen+1);
...
void TestElemIter()
{
	...
	if(U_FAILURE(status)) {
		...
		log_verbose("PASS: Default collationElement iterator1 creation passed\n");
	}
	...
	if(U_FAILURE(status)) {
		...
		log_verbose("PASS: Default collationElement iterator2 creation passed\n");
	}
	...
	if(U_FAILURE(status)) {
		...
		log_verbose("PASS: Default collationElement iterator3 creation passed\n");
	}
	 ...
	 */
	/*
	   doAssert( ((order1 & UCOL_SECONDARYMASK) != (order3 & UCOL_SECONDARYMASK)), "The secondary orders should be
	      different");
	 */
	doAssert( (order1 != UCOL_NULLORDER), "Unexpected end of iterator reached");

	SAlloc::F(testString1);
	SAlloc::F(testString2);
...
void TestGetLocale() {
	...
	static const struct {
		...
		{ "sr_RS", "sr_Cyrl_RS", "sr" },
		{ "sh_YU", "sr_Latn_RS", "sr_Latn" }, /* was sh, then aliased to hr, now sr_Latn via import per cldrbug
		                                         5647: */
		{ "en_BE_FOO", "en", "root" },
...
void TestMergeSortKeys() {
	...
	if(U_SUCCESS(status)) {
		...
		uint32_t casesSize = SIZEOFARRAYi(cases);
		const char * prefix = "foo";
		const char * suffix = "egg";
		char outBuff1[256], outBuff2[256];

		uint8_t ** sortkeys = (uint8_t**)SAlloc::M(casesSize*sizeof(uint8_t *));
		uint8_t ** mergedPrefixkeys = (uint8_t**)SAlloc::M(casesSize*sizeof(uint8_t *));
		uint8_t ** mergedSuffixkeys = (uint8_t**)SAlloc::M(casesSize*sizeof(uint8_t *));
		...
		for(i = 0; i<casesSize; i++) {
			sortkeys[i] = (uint8_t*)SAlloc::M(256*sizeof(uint8_t));
			mergedPrefixkeys[i] = (uint8_t*)SAlloc::M(256*sizeof(uint8_t));
			mergedSuffixkeys[i] = (uint8_t*)SAlloc::M(256*sizeof(uint8_t));
		}
			...
			for(i = 0; i<casesSize; i++) {
				...
				if(i>0) {
					if(tMemCmp(mergedPrefixkeys[i-1], mergedPrefixkeys[i]) >= 0) {
						...
						log_err("%s\n%s\n", ucol_sortKeyToString(coll, mergedPrefixkeys[i-1], outBuff1, l1), ucol_sortKeyToString(coll, mergedPrefixkeys[i], outBuff2, l2));
					}
					if(tMemCmp(mergedSuffixkeys[i-1], mergedSuffixkeys[i]) >= 0) {
						log_err("Error while comparing suffixed keys @ strength %s:\n", strengthsC[strength<=UCOL_QUATERNARY ? strength : 4]);
						log_err("%s\n%s\n", ucol_sortKeyToString(coll, mergedSuffixkeys[i-1], outBuff1, l1), ucol_sortKeyToString(coll, mergedSuffixkeys[i], outBuff2, l2));
					}
				}
			}
		...
		for(i = 0; i<casesSize; i++) {
			SAlloc::F(sortkeys[i]);
			SAlloc::F(mergedPrefixkeys[i]);
			SAlloc::F(mergedSuffixkeys[i]);
		}
		SAlloc::F(sortkeys);
		SAlloc::F(mergedPrefixkeys);
		SAlloc::F(mergedSuffixkeys);
		SAlloc::F(sortKeysLen);
		ucol_close(coll);
	...
	for(i = 0; i < SIZEOFARRAYi(testCases); i++) {
		...
		if(U_SUCCESS(status)) {
			...
			if(strcmp(fromShortBuffer, fromNormalizedBuffer)) {
				log_err(
					"Strings obtained from collators instantiated by short string ('%s') and from normalized string ('%s') differ\n",
					fromShortBuffer,
...
static void doSetsTest(const char * locale, const USet * ref, USet * set, const char * inSet, const char * outSet, UErrorCode * status) {
	...
	if(!uset_containsAll(ref, set)) {
		...
		bufLen = uset_toPattern(set, buffer, SIZEOFARRAYi(buffer), TRUE, status);
		log_info("    missing: %s\n", aescstrdup(buffer, bufLen));
		bufLen = uset_toPattern(ref, buffer, SIZEOFARRAYi(buffer), TRUE, status);
...
#define OPEN_BINARY_ACCEPTS_NULL_BASE 0
	...
	const char * wUCA[] = { "\\u0430", "\\u0452", "\\u0434", "\\u0433", "\\u0432", "\\u0435" }; /* a, dje, d, g, v,
	                                                                                               e */
#if OPEN_BINARY_ACCEPTS_NULL_BASE
	const char * noUCA[] = {"\\u0434", "\\u0433", "\\u0432", "\\u0430", "\\u0435", "\\u0452" }; /* d, g, v, a, e,
	                                                                                               dje */
#endif
	...
	if(U_FAILURE(status)) {
		image = (uint8_t*)SAlloc::M(imageSize*sizeof(uint8_t));
		status = U_ZERO_ERROR;
	...
	genericOrderingTest(coll, wUCA, SIZEOFARRAYi(wUCA));

	genericOrderingTest(cloneWUCA, wUCA, SIZEOFARRAYi(wUCA));
#if OPEN_BINARY_ACCEPTS_NULL_BASE
	genericOrderingTest(cloneNOUCA, noUCA, SIZEOFARRAYi(noUCA));
#endif
...
static void TestDefault() {
	...
	if(status != U_USING_DEFAULT_WARNING) {
		/* What do you mean that you know about using pinyin collation in Spanish!? This should be in the zh
		   locale. */
...
static bool uenum_contains(UEnumeration * e, const char * s, UErrorCode * status) {
	const char * t;
...
#define MAX_NUMBER_OF_KEYWORDS 9
	...
	for(i = 0; i < SIZEOFARRAYi(PREFERRED) && !errorOccurred; i++) {
