...
static void TestOpenFromSource() 
{
	...
	confusables = (char *)SAlloc::M(3000000);
	if(f != NULL) {
	...
	f = fopen(fileName, "rb");
	TEST_ASSERT_NE(f, NULL);
	confusablesWholeScript = (char *)SAlloc::M(1000000);
	if(f != NULL) {
		confusablesWholeScriptLength = (int)fread(confusablesWholeScript, 1, 1000000, f);
		fclose(f);
	}
	rsc = uspoof_openFromSource(confusables, confusablesLength, confusablesWholeScript, confusablesWholeScriptLength, &errType, &pe, &status);
	TEST_ASSERT_SUCCESS(status);
	...
	SAlloc::F(confusablesWholeScript);
	SAlloc::F(confusables);
	SAlloc::F(fileName);
	uspoof_close(rsc);
...
static void TestUSpoofCAPI() {
	...
	status = U_ZERO_ERROR;
	buf = (char *)SAlloc::M(serializedSize + 10);
