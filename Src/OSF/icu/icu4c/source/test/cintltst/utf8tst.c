...
static void TestCharLength()
{
	...
	bool multiple;
#endif
	for(i = 0; i<SIZEOFARRAYi(codepoint); i = (int16_t)(i+2)) {
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
		multiple = (bool)(codepoint[i] == 1 ? FALSE : TRUE);
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	for(i = SIZEOFARRAYi(codePoints)-1, offset = sizeof(input); offset > 0; --i) {
...
static void TestFwdBack() {
	...
	static const uint16_t fwd_N_safe[] = {0, 1, 6, 10, 11, 13, 14, 18}; /*safe macro keeps it at the end of the
	                                                                       string */
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	offsafe = 0;
	for(i = 0; i<SIZEOFARRAYi(Nvalue); i++) {
...
static void TestFwdBackUnsafe() {
	...
	int32_t i;
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	for(i = 1, offset = 0; offset<SIZEOFARRAYi(input); ++i) {
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	for(i = SIZEOFARRAYi(backBoundaries)-2, offset = SIZEOFARRAYi(input); offset>0; --i) {
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	for(i = 0; i<SIZEOFARRAYi(boundaries); ++i) {
		offset = 0;
...
#if !U_HIDE_OBSOLETE_UTF_OLD_H
	for(i = 0; i<SIZEOFARRAYi(backBoundaries); ++i) {
		int32_t j = SIZEOFARRAYi(backBoundaries)-1-i;
		offset = SIZEOFARRAYi(input);
...
static void TestSetChar() {
	...
	for(offset = 0; offset<=SIZEOFARRAYi(input); offset++) {
		if(offset<SIZEOFARRAYi(input)) {
#if !U_HIDE_OBSOLETE_UTF_OLD_H
			setOffset = offset;
...
static void TestAppendChar() {
	...
	static const uint8_t result[][11] = {
		...
		{0x61, 0xF0, 0xa0, 0x90, 0x82, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x00},
		{0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0xc2, 0x9f}, /*gets UTF8_ERROR_VALUE_2 which
		                                                                       takes 2 bytes 0xc0, 0x9f*/
	...
	for(i = 0; i<SIZEOFARRAYi(test); i = (uint16_t)(i+2)) {
		...
		if(count<13) {
			...
			if(offset != movedOffset[count]) {
				log_err(
					"ERROR: UTF8_APPEND_CHAR_UNSAFE failed to move the offset correctly for count=%d.\nExpectedOffset=%d  currentOffset=%d\n",
					count,
					movedOffset[count],
					offset);
			}
			if(memcmp(str, result[count], size) !=0) {
			...
			if(offset != movedOffset[count]) {
				log_err(
					"ERROR: UTF8_APPEND_CHAR_SAFE failed to move the offset correctly for count=%d.\nExpectedOffset=%d  currentOffset=%d\n",
					count,
					movedOffset[count],
					offset);
			}
			...
			/*call the API instead of MACRO
			   ...
			   if(offset != movedOffset[count]) {
			    log_err("ERROR: utf8_appendCharSafeBody() failed to move the offset correctly for
			       count=%d.\nExpectedOffset=%d  currentOffset=%d\n",
...
static void TestAppend() {
	...
	if(length!=SIZEOFARRAYi(expectUnsafe) || 0!=memcmp(buffer, expectUnsafe, length)) {
	...
	for(i = 0; i<SIZEOFARRAYi(codePoints); ++i) {
		...
		U8_APPEND(buffer, length, SIZEOFARRAYi(buffer), c, isError);
		wrongIsError |= isError!=expectIsError;
	}
	if(wrongIsError) {
		log_err("U8_APPEND did not set isError correctly\n");
	}
	if(length!=SIZEOFARRAYi(expectSafe) || 0!=memcmp(buffer, expectSafe, length)) {
...
static void TestSurrogates() 
{
	...
	static const UChar32 cp[] = { 0xdf, 0xd7ff, 0xd801, 0xdffe, 0xe000, 0x17ffe };
	UChar32 cu, cs, cl;
	int32_t i, j, k, iu, is, il, length;
	k = 0; /* index into cp[] */
	length = SIZEOFARRAYi(b);
	for(i = 0; i<length;) {
		j = i;
