...
static void parseMappings(const char * filename, UStringPrepProfile* data, bool reportError, UErrorCode * pErrorCode);

static void compareMapping(UStringPrepProfile* data, uint32_t codepoint, uint32_t* mapping, int32_t mapLength,
...
static UStringPrepType getValues(uint32_t result, int32_t* value, bool* isIndex) {
...
static void compareFlagsForRange(UStringPrepProfile* data,
		...
		if(result > _SPREP_TYPE_THRESHOLD) {
			if(retType != type) {
				log_err("FAIL: Did not get the expected type for 0x%06X. Expected: %s Got: %s\n", start,
				    usprepTypeNames[type], usprepTypeNames[retType]);
			}
		}
		else {
...
void doStringPrepTest(const char * binFileName, const char * txtFileName, int32_t options, UErrorCode * errorCode) {
	...
	filename = (char *)SAlloc::M(strlen(srcdatapath)+strlen(relativepath)+strlen(txtFileName)+10);
