...
static void TestPrimary(); /* perform test with strength PRIMARY */
static void TestSecondary(); /* perform test with strength SECONDARY */
static void TestTertiary(); /* perform test with strength tertiary */
static void TestIdentical(); /*perform tests with strength Identical */
static void TestExtra(); /* perform extra tests */
static void TestJB581(); /* Test jitterbug 581 */
static void TestJB1401(); /* Test jitterbug 1401 */
static void TestVariableTop(); /* Test [variable top] in the rule syntax */
static void TestSurrogates(); /* Test surrogates */
static void TestInvalidRules();
static void TestJitterbug1098();
static void TestFCDCrash();
static void TestJ5298();
static void TestBadKey();
...
static char * U_EXPORT2 sortKeyToString(const UCollator * coll, const uint8_t * sortkey, char * buffer, uint32_t * len) {
	...
	while(strength <= UCOL_QUATERNARY && strength <= ucol_getStrength(coll)) {
		...
		else if(ucol_getAttribute(coll, UCOL_CASE_LEVEL,
		    &errorCode) == UCOL_OFF || doneCase == TRUE || strength != UCOL_SECONDARY) {
			strength++;
		}
		if(*currentSk) {
...
bool hasCollationElements(const char * locName) {
...
static void doTestVariant(UCollator* myCollation, const UChar source[], const UChar target[], UCollationResult result)
{
	...
	/* convert the strings to UTF-8 and do try comparing with char iterator and ucol_strcollUTF8 */
	{
		...
		if(U_FAILURE(status)) { /* probably buffer is not big enough */
			...
			if(U_SUCCESS(status)) {
				{
					/* ucol_strcollUTF8 */
					compareResulta = ucol_strcollUTF8(myCollation,
						utf8Source,
						utf8SourceLen,
						utf8Target,
						utf8TargetLen,
					...
					if(U_FAILURE(status)) {
						...
						log_err(
							"ucol_strcollUTF8 with explicit length returned wrong result (%i exp. %i): %s, %s\n",
							compareResulta,
							result,
							aescstrdup(source, -1),
							aescstrdup(target, -1));
					...
					if(U_FAILURE(status)) {
						...
						log_err(
							"ucol_strcollUTF8 with null terminated strings returned wrong result (%i exp. %i): %s, %s\n",
							compareResulta,
							result,
							aescstrdup(source, -1),
							aescstrdup(target, -1));
...

				{
					...
					if(compareResultUTF8Iter != compareResultIter) {
						log_err(
							"different results in iterative comparison for UTF-16 and UTF-8 encoded strings. %s, %s\n",
							aescstrdup(source, -1),
							aescstrdup(target, -1));
					}
					if(compareResultUTF8Iter != compareResultUTF8IterNorm) {
						log_err(
							"different results in iterative when normalization is turned on with UTF-8 strings. %s, %s\n",
							aescstrdup(source, -1),
							aescstrdup(target, -1));
					}
				}
	...
	if(1) { /*!QUICK*/
		...
		for(i = 0; i < partialSizesSize; i++) {
			...
			if(getTestOption(QUICK_OPTION) <= 0 && norm != UCOL_ON) {
				/*log_verbose("N ");*/
				ucol_setAttribute(myCollation, UCOL_NORMALIZATION_MODE, UCOL_ON, &status);
				partialNormalizedSKResult = compareUsingPartials(myCollation,
					source,
					sLen,
					target,
					tLen,
				...
				if(partialSKResult != partialNormalizedSKResult) {
					log_err(
						"Partial sortkey comparison gets different result when normalization is on: %s, %s (size %i)\n",
						aescstrdup(source, -1),
						aescstrdup(target, -1),
						partialSizes[i]);
				}
	...
	sortKey1 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklenmax+1));
	sortKey1a = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklenmax+1));
	ucol_getSortKey(myCollation, source, sLen, sortKey1,  sortklen1+1);
	ucol_getSortKey(myCollation, source, -1,   sortKey1a, sortklen1+1);

	sortKey2 = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklenmax+1));
	sortKey2a = (uint8_t*)SAlloc::M(sizeof(uint8_t) * (sortklenmax+1));
...
OrderAndOffset* getOrders(UCollationElements * iter, int32_t * orderLength)
{
	...
	OrderAndOffset * temp;
	OrderAndOffset * orders = (OrderAndOffset*)SAlloc::M(sizeof(OrderAndOffset) * maxSize);
	UErrorCode status = U_ZERO_ERROR;
	while((order = ucol_next(iter, &status)) != UCOL_NULLORDER) {
		if(size == maxSize) {
			maxSize *= 2;
			temp = (OrderAndOffset*)SAlloc::M(sizeof(OrderAndOffset) * maxSize);
	...
	if(maxSize > size && size > 0) {
		temp = (OrderAndOffset*)SAlloc::M(sizeof(OrderAndOffset) * size);
		memcpy(temp, orders, size * sizeof(OrderAndOffset));
		SAlloc::F(orders);
		orders = temp;
	}
...
void genericRulesStarterWithOptionsAndResult(const char * rules,
    const char * const s[],
    uint32_t size,
    const UColAttribute * attrs,
...
void genericLocaleStarterWithOptionsAndResult(const char * locale,
    const char * const s[],
    uint32_t size,
    const UColAttribute * attrs,
    const UColAttributeValue * values,
...
void genericLocaleStarterWithOptions(const char * locale,
    const char * const s[],
    uint32_t size,
    const UColAttribute * attrs,
    const UColAttributeValue * values,
...
static void TestSurrogates(void)
{
	...
	int rlen = u_unescape(str, rules, len);
	UCollator * enCollation = ucol_open("en_US", &status);
	if(U_FAILURE(status)) {
		log_err_status(status, "ERROR: in creation of collator :%s\n", myErrorName(status));
		return;
	}
	myCollation = ucol_openRules(rules, rlen, UCOL_OFF, UCOL_TERTIARY, NULL, &status);
	if(U_FAILURE(status)) {
		ucol_close(enCollation);
		log_err("ERROR: in creation of rule based collator :%s\n", myErrorName(status));
		return;
