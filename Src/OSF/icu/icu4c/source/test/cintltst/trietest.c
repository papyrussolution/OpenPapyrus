...
static uint32_t U_CALLCONV _testFoldedValue32(UNewTrie * trie, UChar32 start, int32_t offset) {
...
static int32_t U_CALLCONV _testFoldingOffset32(uint32_t data) {
	return (int32_t)(data>>16);
}

static uint32_t U_CALLCONV _testFoldedValue16(UNewTrie * trie, UChar32 start, int32_t offset) {
	uint32_t foldedValue, value;
	UChar32 limit;
	bool inBlockZero;
...
static int32_t U_CALLCONV _testFoldingOffset16(uint32_t data) {
	if(data&0x8000) {
...
static uint32_t U_CALLCONV _testEnumValue(const void * context, uint32_t value) {
	(void)context; // suppress compiler warnings about unused variable
	return value^0x5555;
}

static bool U_CALLCONV _testEnumRange(const void * context, UChar32 start, UChar32 limit, uint32_t value) {
...
static void testTrieIteration(const char * testName,
	...
	while(p < limit) {
		...
		if(c2!=0) {
			...
			if(value!=values[i]) {
				log_err(
					"error: wrong value from UTRIE_GETXX_FROM_OFFSET_TRAIL(%s)(U+%04lx, U+%04lx): 0x%lx instead of 0x%lx\n",
					testName,
					c,
					c2,
					value,
...
static void testTrieRangesWithMalloc(const char * testName,
    const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges,
    bool dataIs32, bool latin1Linear) {
...
static void testTrieRanges(const char * testName,
    const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges,
    bool dataIs32, bool latin1Linear) {
	union {
...
static void testTrieRanges2(const char * testName,
    const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges,
    bool dataIs32) {
	char name[40];
...
static void testTrieRanges4(const char * testName,
    const SetRange setRanges[], int32_t countSetRanges,
    const CheckRange checkRanges[], int32_t countCheckRanges) {
	char name[40];
...
static int32_t U_CALLCONV dummyGetFoldingOffset(uint32_t data) {
...
static void dummyTest(bool make16BitTrie) {
