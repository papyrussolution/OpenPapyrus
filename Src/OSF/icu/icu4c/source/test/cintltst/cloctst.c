...
static void TestNonexistentLanguageExemplars();
static void TestLocDataErrorCodeChaining();
static void TestLocDataWithRgTag();
static void TestLanguageExemplarsFallbacks();
...
static const char * const rawData2[LOCALE_INFO_SIZE][LOCALE_SIZE] = {
	...
	{   "English (United States)", "French (France)", "Catalan (Spain)",
	    ...
	    "German (Germany, Sort Order=Phonebook Sort Order)", "Spanish (Sort Order=Traditional Sort Order)",
	...
	{   "anglais (\\u00C9tats-Unis)", "fran\\u00E7ais (France)", "catalan (Espagne)",
	    ...
	    "allemand (Allemagne, ordre de tri=ordre de l\\u2019annuaire)", "espagnol (ordre de tri=ordre traditionnel)",
	...
	{   "angl\\u00E8s (Estats Units)", "franc\\u00E8s (Fran\\u00E7a)", "catal\\u00E0 (Espanya)",
	    ...
	    "alemany (Alemanya, ordenaci\\u00F3=ordre de la guia telef\\u00F2nica)", "espanyol (ordenaci\\u00F3=ordre tradicional)",
	    "japon\\u00E8s (Jap\\u00F3, calendari=calendari japon\\u00e8s)" },
	...
	{   "", "", "", "", "NY", "", "", "", ""    },/* TODO: currently there is no translation for NY in Greek fix
	                                              this test when we have it */
	/* display name (Greek) */
	{
...
static void TestBasicGetters() 
{
	...
	for(i = 0; i < LOCALE_SIZE; i++) {
		testLocale = (char *)SAlloc::M(sizeof(char) * (strlen(rawData2[NAME][i])+1));
		strcpy(testLocale, rawData2[NAME][i]);
		log_verbose("Testing   %s  .....\n", testLocale);
		cap = uloc_getLanguage(testLocale, NULL, 0, &status);
		if(status==U_BUFFER_OVERFLOW_ERROR) {
			status = U_ZERO_ERROR;
			temp = (char *)SAlloc::M(sizeof(char) * (cap+1));
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) {
			status = U_ZERO_ERROR;
			temp = (char *)SAlloc::R(temp, sizeof(char) * (cap+1));
		...
		if(status==U_BUFFER_OVERFLOW_ERROR) {
			status = U_ZERO_ERROR;
			name = (char *)SAlloc::M(sizeof(char) * (cap+1));
...
static void TestNullDefault() 
{
	...
	}
	{
		...
		if(strcmp(n_en_US, "en_US") != 0) {
			log_err("uloc_setDefault altered previously default string. Expected \"en_US\", got \"%s\"\n",  n_en_US);
		}
...
static void TestPrefixes() {
	...
	static const char * const testData[][7] =
	{
		...
		{"x-piglatin", "", "ML", "", "x-piglatin_ML.MBE", "x-piglatin_ML.MBE", "x-piglatin_ML"}, /* Multibyte
		                                                                                            English */
...
static void TestSimpleResourceInfo() {
	...
	for(i = 0; i < LOCALE_SIZE; i++) {
		testLocale = (char *)SAlloc::R(testLocale, sizeof(char) * (u_strlen(dataTable[NAME][i])+1));
		...
		if(strcmp(temp2, rawData2[LCID_][i]) != 0) {
			log_err("LCID mismatch: %s versus %s\n", temp2, rawData2[LCID_][i]);
		}
	}
	SAlloc::F(expected);
	SAlloc::F(testLocale);
...
static int32_t UCharsToEscapedAscii(const UChar * utext, int32_t len, char * resultChars, int32_t buflen) {
	...
	static const int32_t ESCAPE_MAP_LENGTH = SIZEOFARRAYi(ESCAPE_MAP);
...
static void TestDisplayNames()
{
	...
	length = uloc_getDisplayLanguage(NULL, NULL, buffer, SIZEOFARRAYi(buffer), &errorCode);
	...
	if(U_FAILURE(errorCode) || (length<=3 && buffer[0]<=0x7f)) {
		...
		for(int32_t i = 0, count = uloc_countAvailable(); i < count; i++) {
			...
			if(strcmp(defaultLocale, uloc_getAvailable(i)) == 0) {
				log_data_err(
	...
	errorCode = U_ZERO_ERROR;
	length = uloc_getDisplayLanguage("qq", "rr", buffer, SIZEOFARRAYi(buffer), &errorCode);
	...
	errorCode = U_ZERO_ERROR;
	length = uloc_getDisplayName("qq_US_POSIX", "en_US", buffer, SIZEOFARRAYi(buffer), &errorCode);
...

	{
		...
		for(i = 0; i<SIZEOFARRAYi(testL); i++) {
			errorCode = U_ZERO_ERROR;
			uloc_getDisplayName(aLocale, testL[i], buffer, SIZEOFARRAYi(buffer), &errorCode);
			if(U_FAILURE(errorCode)) {
				...
				if(u_strcmp(buffer, expectBuffer)) {
					log_data_err(
						"FAIL in uloc_getDisplayName(%s,%s,..) expected '%s' got '%s' (Are you missing data?)\n",
						aLocale,
						testL[i],
						expect[i],
						austrdup(buffer));
	   ...
	   see ticket #8262. */
	{
		...
		len = uloc_getDisplayName(locale, displayLocale, result, SIZEOFARRAYi(result), &ec);
		if(U_FAILURE(ec)) {
			...
			if(len!=expectedLen) {
				...
				char * resultChars = (char *)SAlloc::M(cap);
				...
				for(i = len+1; i>=0; --i) {
					...
					/* There's no guarantee about what's in the buffer if we've overflowed, in
					   particular,
...
static void TestGetAvailableLocalesByType() 
{
	...
	assertIntEquals("countAvailable() should be same in old and new methods", uloc_countAvailable(), uenum_count(uenum, &status));
	for(int32_t i = 0; i < uloc_countAvailable(); i++) {
		const char * old = uloc_getAvailable(i);
		int32_t len = 0;
		const char * p_new = uenum_next(uenum, &len, &status);
		assertEquals("Old and new strings should equal", old, p_new);
		assertIntEquals("String length should be correct", strlen(old), len);
	}
	assertPtrEquals("Should get nullptr on the last string", NULL, uenum_next(uenum, NULL, &status));
	uenum_close(uenum);
	...
	assertIntEquals("Should return as many strings as claimed", count, uenum_count(uenum, &status));
	...
	assertIntEquals("After reset, should return as many strings as claimed", count, uenum_count(uenum, &status));
	uenum_close(uenum);
...
static void TestDisplayNameBrackets()
{
	...
	for(; itemPtr->displayLocale != NULL; itemPtr++) {
		...
		if(U_FAILURE(status) || u_strcmp(getName, expectRegionName) != 0) {
			log_data_err(
				"uloc_getDisplayCountry for displayLocale %s and namedLocale %s returns unexpected name or status %s\n",
				itemPtr->displayLocale,
				itemPtr->namedLocale,
				myErrorName(status));
		}
		...
		if(U_SUCCESS(status)) {
			...
			if(U_FAILURE(status) || u_strcmp(getName, expectRegionName) != 0) {
				log_data_err(
					"uldn_regionDisplayName for displayLocale %s and namedRegion %s returns unexpected name or status %s\n",
					itemPtr->displayLocale,
					itemPtr->namedRegion,
					myErrorName(status));
			}
			...
			if(U_FAILURE(status) || u_strcmp(getName, expectLocaleName) != 0) {
				log_data_err(
					"uldn_localeDisplayName for displayLocale %s and namedLocale %s returns unexpected name or status %s\n",
					itemPtr->displayLocale,
					itemPtr->namedLocale,
					myErrorName(status));
			}
...
static void cleanUpDataTable()
{
	...
	if(dataTable) {
		for(i = 0; i < LOCALE_INFO_SIZE; i++) {
			for(j = 0; j < LOCALE_SIZE; j++) {
				SAlloc::F(dataTable[i][j]);
			}
			SAlloc::F(dataTable[i]);
		}
		SAlloc::F(dataTable);
	}
	dataTable = NULL;
...
static void TestKeywordVariants(void)
{
	static const struct {
		...
		{
			"de_DE@euro;collation=phonebook", /* The POSIX style variant @euro cannot be combined with
			                                     key=value? */
	...
	for(i = 0; i < SIZEOFARRAYi(testCases); i++) {
		...
		*buffer = 0;
		...
		if(U_SUCCESS(status)) {
			...
			else if(strcmp(testCases[i].expectedLocaleIDNoKeywords, buffer) != 0) {
		...
		if(U_SUCCESS(status)) {
			...
			else if(strcmp(testCases[i].expectedCanonicalID, buffer) != 0) {
	...
	for(i = 0; i < SIZEOFARRAYi(testCases); i++) {
		*buffer = 0;
		...
		if(testCases[i].expectedValue) {
			...
			else if(strcmp(testCases[i].expectedValue, buffer) != 0) {
...
static const struct {
	...
	{ "en_US@calendar=slovakian", "calendar", "gregorian", "en_US@calendar=gregorian" }, /* don't know what this
	...
	/*32*/ { "en_US@ calendar = gregorian ; collation = phonebook", "calendar", "japanese",
		 "en_US@calendar=japanese;collation=phonebook" },
...
static void TestKeywordSet(void)
{
	...
	for(i = 0; i < SIZEOFARRAYi(kwSetTestCases); i++) {
		UErrorCode status = U_ZERO_ERROR;
		memset(buffer, '%', 1023);
		...
		if(kwSetTestCases[i].x != NULL) {
			...
			if(kwSetTestCases[i].v != NULL && kwSetTestCases[i].v[0] != 0) {
				...
				else if(resultLen != (int32_t)strlen(kwSetTestCases[i].v) ||
				    strcmp(buffer, kwSetTestCases[i].v) != 0) {
					log_err("FAIL: #%d getKeywordValue: got %s (%d) expected %s (%d)\n", i, buffer, resultLen,
					    kwSetTestCases[i].v, strlen(kwSetTestCases[i].v));
				}
			}
		}
		else {
...
static void TestCanonicalization(void)
{
	static const struct {
		...
		{ "no-no-ny.utf8@B", "no_NO_NY.utf8@B", "no_NO_NY_B" /* not: "nn_NO" [alan ICU3.0] */ }, /* @ ignored
		                                                                                            unless
		                                                                                            variant is
		                                                                                            empty */
		...
		{ "no-Hant-GB_NY@currency=$$$", "no_Hant_GB_NY@currency=$$$",
		  "no_Hant_GB_NY@currency=$$$" /* not: "nn_Hant_GB@currency=$$$" [alan ICU3.0] */ },
			...
			if(status==U_BUFFER_OVERFLOW_ERROR) {
				...
				if(U_FAILURE(status)) {
					log_err("uloc_getDisplayKeyword filed for keyword : %s in locale id: %s for display locale: %s \n",
					    ...
					    testCases[i].displayLocale,
					    u_errorName(status));
					SAlloc::F(displayKeyword);
					break;
				}
				if(u_strncmp(displayKeyword, testCases[i].displayKeyword, displayKeywordLen)!=0) {
					if(status == U_USING_DEFAULT_WARNING) {
						log_data_err(
							"uloc_getDisplayKeyword did not get the expected value for keyword : %s in locale id: %s for display locale: %s . Got error: %s. Perhaps you are missing data?\n",
							testCases[i].localeID,
							keyword,
							testCases[i].displayLocale,
							u_errorName(status));
						...
						log_err(
							"uloc_getDisplayKeyword did not get the expected value for keyword : %s in locale id: %s for display locale: %s \n",
							testCases[i].localeID,
							keyword,
							testCases[i].displayLocale);
					}
...
static void TestDisplayKeywordValues() {
	...
	static const struct {
		...
		{   "de_AT@currency=ATS",         "fr_FR",
		    {0x0073, 0x0063, 0x0068, 0x0069, 0x006c, 0x006c, 0x0069, 0x006e, 0x0067, 0x0020, 0x0061, 0x0075, 0x0074, 0x0072, 0x0069,
		     0x0063, 0x0068, 0x0069, 0x0065, 0x006e, 0x0000}},
		...
		{   "eu_ES@currency=ESP",         "it_IT",
		    {0x0070, 0x0065, 0x0073, 0x0065, 0x0074, 0x0061, 0x0020, 0x0073, 0x0070, 0x0061, 0x0067, 0x006e, 0x006f, 0x006c, 0x0061,
		     0x0000}},
		{   "de@collation=phonebook",     "es",
		    {0x006F, 0x0072, 0x0064, 0x0065, 0x006E, 0x0020, 0x0064, 0x0065, 0x0020, 0x006C, 0x0069, 0x0073, 0x0074, 0x00ED, 0x006E,
		     0x0020, 0x0074, 0x0065, 0x006C, 0x0065, 0x0066, 0x00F3, 0x006E, 0x0069, 0x0063, 0x006F, 0x0000}},

		{ "de_DE@collation=phonebook",  "es",
		  {0x006F, 0x0072, 0x0064, 0x0065, 0x006E, 0x0020, 0x0064, 0x0065, 0x0020, 0x006C, 0x0069, 0x0073, 0x0074, 0x00ED, 0x006E,
		   0x0020, 0x0074, 0x0065, 0x006C, 0x0065, 0x0066, 0x00F3, 0x006E, 0x0069, 0x0063, 0x006F, 0x0000}},
		{ "es_ES@collation=traditional", "de",
		  {0x0054, 0x0072, 0x0061, 0x0064, 0x0069, 0x0074, 0x0069, 0x006f, 0x006e, 0x0065, 0x006c, 0x006c, 0x0065, 0x0020, 0x0053,
		   0x006f, 0x0072, 0x0074, 0x0069, 0x0065, 0x0072, 0x0072, 0x0065, 0x0067, 0x0065, 0x006c, 0x006e,
		   0x0000}},
		{ "ja_JP@calendar=japanese",    "de",
		  {0x004a, 0x0061, 0x0070, 0x0061, 0x006e, 0x0069, 0x0073, 0x0063, 0x0068, 0x0065, 0x0072, 0x0020, 0x004b, 0x0061, 0x006c,
		   0x0065, 0x006e, 0x0064, 0x0065, 0x0072, 0x0000}},
	};
	for(i = 0; i < SIZEOFARRAYi(testCases); i++) {
		UErrorCode status = U_ZERO_ERROR;
		const char * keyword = NULL;
		...
		for(keywordCount = uenum_count(keywordEnum, &status); keywordCount > 0; keywordCount--) {
			...
			if(status==U_BUFFER_OVERFLOW_ERROR) {
				...
				if(U_FAILURE(status)) {
					log_err(
						"uloc_getDisplayKeywordValue failed for keyword : %s in locale id: %s for display locale: %s with error : %s \n",
						testCases[i].localeID,
						keyword,
						testCases[i].displayLocale,
						u_errorName(status));
					SAlloc::F(displayKeywordValue);
					break;
				}
				if(u_strncmp(displayKeywordValue, testCases[i].displayKeywordValue, displayKeywordValueLen)!=0) {
					if(status == U_USING_DEFAULT_WARNING) {
						log_data_err(
							"uloc_getDisplayKeywordValue did not return the expected value keyword : %s in locale id: %s for display locale: %s with error : %s Perhaps you are missing data\n",
							testCases[i].localeID,
							keyword,
							testCases[i].displayLocale,
							u_errorName(status));
						...
						log_err(
							"uloc_getDisplayKeywordValue did not return the expected value keyword : %s in locale id: %s for display locale: %s with error : %s \n",
							testCases[i].localeID,
							keyword,
							testCases[i].displayLocale,
							u_errorName(status));
	...
	}
	{
		...
		static const UChar expected[][50] = {
			...
			{0x0054, 0x0065, 0x006c, 0x0065, 0x0066, 0x006f, 0x006e, 0x0062, 0x0075, 0x0063, 0x0068, 0x002d, 0x0053, 0x006f,
			 0x0072, 0x0074, 0x0069, 0x0065, 0x0072, 0x0075, 0x006e, 0x0067, 0x0000},
		...
		for(keywordCount = 0; keywordCount < uenum_count(keywordEnum, &status); keywordCount++) {
			...
			if(status==U_BUFFER_OVERFLOW_ERROR) {
				...
				if(u_strncmp(displayKeywordValue, expected[keywordCount], displayKeywordValueLen)!=0) {
					if(status == U_USING_DEFAULT_WARNING) {
						log_data_err(
							"uloc_getDisplayKeywordValue did not return the expected value keyword : %s in locale id: %s for display locale: %s  got error: %s. Perhaps you are missing data?\n",
							localeID,
							keyword,
							displayLocale,
							u_errorName(status));
						...
						log_err(
							"uloc_getDisplayKeywordValue did not return the expected value keyword : %s in locale id: %s for display locale: %s \n",
							localeID,
							keyword,
							displayLocale);
					}
...
static void TestDisplayNameWarning() {
	...
	size = uloc_getDisplayLanguage("qqq", "kl", name, SIZEOFARRAYi(name), &status);
 ...
 */
static /*bool*/int8 _loccmp(const char * string, const char * prefix) 
{
...
static void _checklocs(const char * label, const char * req, const char * valid, const char * actual) 
{
	/* We want the valid to be strictly > the bogus requested locale,
	   and the valid to be >= the actual. */
	if(_loccmp(req, valid) > 0 && _loccmp(valid, actual) >= 0) {
...
static void TestGetLocale() 
{
...
#if !UCONFIG_NO_FORMATTING
	{
		...
		/* TODO MessageFormat is currently just storing the locale it is given.
		   ...
		   locale is invalid. */
		test = (_cmpversion("3.2") <= 0) ? /* Here is the weakened test for 3.0: */(_loccmp(req, valid) >= 0) : /* Here is what the test line SHOULD be: */(_loccmp(req, valid) > 0);
		if(test && _loccmp(valid, actual) >= 0) {
...
#if !UCONFIG_NO_BREAK_ITERATION
	{
		...
		obj = ubrk_open(UBRK_WORD, req, EMPTY, 0, &ec);
		if(U_FAILURE(ec)) {
...
static void TestAcceptLanguage() {
	...
	const int32_t numTests = SIZEOFARRAYi(tests);
	...
	for(i = 0; i<numTests; i++) {
		...
		if(status != tests[i].expectStatus) {
			...
			if((outResult>0)&&strcmp(tmp, tests[i].expect)) {
...
static bool isLocaleAvailable(UResourceBundle * resIndex, const char * loc) {
	UErrorCode status = U_ZERO_ERROR;
	int32_t len = 0;
...
static void TestGetLocaleForLCID() {
	...
	length = uloc_getLocaleForLCID(lcid, temp2, SIZEOFARRAYi(temp2), &status);
	if(U_FAILURE(status)) {
	...
	length = uloc_getLocaleForLCID(0x12345, temp2, SIZEOFARRAYi(temp2), &status);
	if(U_SUCCESS(status)) {
	...
	for(i = 0; i < LOCALE_SIZE; i++) {
		testLocale = rawData2[NAME][i];
		log_verbose("Testing   %s ......\n", testLocale);
		sscanf(rawData2[LCID_][i], "%x", &lcid);
		length = uloc_getLocaleForLCID(lcid, temp2, SIZEOFARRAYi(temp2), &status);
		if(U_FAILURE(status)) {
		...
		if(length != (int32_t)strlen(temp2)) {
		...
		length = uloc_getLanguage(temp2, temp3, SIZEOFARRAYi(temp3), &status);
		if(U_FAILURE(status)) {
		    ...
		    rawData2[LANG][i]) && !(strcmp(temp3, "nn") == 0 && strcmp(rawData2[VAR][i], "NY") == 0)) {
		...
		length = uloc_getScript(temp2, temp3, SIZEOFARRAYi(temp3), &status);
		if(U_FAILURE(status)) {
			log_err("  couldn't get script in uloc_getLocaleForLCID(%#04x) = %s, status %s\n", lcid, temp2,
			    u_errorName(status));
			status = U_ZERO_ERROR;
		}
		else if(strcmp(temp3, rawData2[SCRIPT][i])) {
		...
		length = uloc_getCountry(temp2, temp3, SIZEOFARRAYi(temp3), &status);
		if(U_FAILURE(status)) {
			log_err("  couldn't get country in uloc_getLocaleForLCID(%#04x) = %s, status %s\n", lcid, temp2,
			    u_errorName(status));
			status = U_ZERO_ERROR;
		}
		else if(strlen(rawData2[CTRY][i]) && strcmp(temp3, rawData2[CTRY][i])) {
...
static void TestLikelySubtags()
{
	...
	for(; i < SIZEOFARRAYi(basic_maximize_data); ++i) {
		UErrorCode status = U_ZERO_ERROR;
	...
	for(i = 0; i < SIZEOFARRAYi(basic_minimize_data); ++i) {
		UErrorCode status = U_ZERO_ERROR;
	...
	for(i = 0; i < SIZEOFARRAYi(full_data); ++i) {
		UErrorCode status = U_ZERO_ERROR;
	...
	for(i = 0; i < SIZEOFARRAYi(maximizeErrors); ++i) {
		UErrorCode status = U_ZERO_ERROR;
	...
	for(i = 0; i < SIZEOFARRAYi(minimizeErrors); ++i) {
		UErrorCode status = U_ZERO_ERROR;
...
const char * const locale_to_langtag[][3] = {
	...
	{"en_US_POSIX@va=posix2",   "en-US-u-va-posix2",  "en-US-u-va-posix2"},/* if keyword va=xxx already exists,
	                                                                        variant POSIX is simply dropped */
...
static void TestToLanguageTag() {
	...
	for(i = 0; locale_to_langtag[i][0] != NULL; i++) {
		...
		if(U_FAILURE(status)) {
			...
			if(expected == NULL) {
				log_err(
					"Error should be returned by uloc_toLanguageTag for locale id [%s], but [%s] is returned without errors\n",
					inloc,
					langtag);
			}
			else if(strcmp(langtag, expected) != 0) {
				log_data_err(
					"uloc_toLanguageTag returned language tag [%s] for input locale [%s] - expected: [%s]. Are you missing data?\n",
					langtag,
					inloc,
					expected);
			}
		...
		if(U_FAILURE(status)) {
			if(expected != NULL) {
				log_data_err(
					"Error returned by uloc_toLanguageTag {strict} for locale id [%s] - error: %s Are you missing data?\n",
					inloc,
					u_errorName(status));
			}
		}
			...
			if(expected == NULL) {
				log_err(
					"Error should be returned by uloc_toLanguageTag {strict} for locale id [%s], but [%s] is returned without errors\n",
					inloc,
					langtag);
			}
			else if(strcmp(langtag, expected) != 0) {
...
static void TestBug20132() {
	...
	const int32_t expected_len = (int32_t)strlen(expected);
...
static const struct {
	...
	{"ja-u-ijkl-efgh-abcd-ca-japanese-xx-yyy-zzz-kn",   "ja@attribute=abcd-efgh-ijkl;calendar=japanese;colnumeric=yes;xx=yyy-zzz",
	 FULL_LENGTH},
...
static void TestForLanguageTag() {
	...
	for(i = 0; i < SIZEOFARRAYi(langtag_to_locale); i++) {
		status = U_ZERO_ERROR;
		locale[0] = 0;
		expParsedLen = langtag_to_locale[i].len;
		if(expParsedLen == FULL_LENGTH) {
			expParsedLen = (int32_t)strlen(langtag_to_locale[i].bcpID);
		}
		...
		if(U_FAILURE(status)) {
			...
			if(strcmp(langtag_to_locale[i].locID, locale) != 0) {
			...
			if(parsedLen != expParsedLen) {
				log_err(
					"uloc_forLanguageTag parsed length of %d for input language tag [%s] - expected parsed length: %d\n",
					parsedLen,
...
static void TestLangAndRegionCanonicalize() {
	...
	int32_t i;
	UErrorCode status;
	for(i = 0; i < SIZEOFARRAYi(langtag_to_canonical); i++) {
		status = U_ZERO_ERROR;
		...
		if(U_FAILURE(status)) {
			...
			if(strcmp(expected_canonical, canonical) != 0) {
...
{
	// key = alphanum alpha ;
	static const char * invalid[] = { "a0", "00", "a@", "0@", "@a", "@a", "abc", "0bc", };
	for(int i = 0; i < SIZEOFARRAYi(invalid); i++) {
		const char * bcpKey = NULL;
	...
	static const char * valid[] = { "aa", "0a", };
	for(int i = 0; i < SIZEOFARRAYi(valid); i++) {
		const char * bcpKey = NULL;
...
static void TestToLegacyKey(void)
{
	...
	for(int32_t i = 0; DATA[i][0] != NULL; i++) {
		const char * keyword = DATA[i][0];
		const char * expected = DATA[i][1];
		const char * legacyKey = uloc_toLegacyKey(keyword);
		if(expected == NULL) {
			if(legacyKey != NULL) {
		...
		else if(strcmp(legacyKey, expected) != 0) {
...
static void TestToUnicodeLocaleType(void)
{
	...
	for(i = 0; DATA[i][0] != NULL; i++) {
		...
		const char * bcpType = uloc_toUnicodeLocaleType(keyword, value);
		if(expected == NULL) {
			if(bcpType != NULL) {
		...
		else if(strcmp(bcpType, expected) != 0) {
...
static void TestToLegacyType(void)
{
	...
	static const char * DATA[][3] = {
		...
		{"variableTop",     "wxyz",             "$IN"},/* invalid codepoints type - return as is for now */
		{"kr",              "space-punct",      "space-punct"},/* valid reordercode type */
		{"kr",              "digit-spacepunct", "digit-spacepunct"},/* invalid reordercode type, but ok for
		                                                        legacy syntax */
		{NULL,              NULL,               NULL}
	};
	for(int32_t i = 0; DATA[i][0] != NULL; i++) {
		...
		const char * value = DATA[i][1];
		const char * expected = DATA[i][2];
		const char * legacyType = uloc_toLegacyType(keyword, value);
		if(expected == NULL) {
			if(legacyType != NULL) {
		...
		else if(strcmp(legacyType, expected) != 0) {
...
static const UDisplayContext optStdMidLong[3] =
{UDISPCTX_STANDARD_NAMES, UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE,    UDISPCTX_LENGTH_FULL};
static const UDisplayContext optStdMidShrt[3] =
{UDISPCTX_STANDARD_NAMES, UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE,    UDISPCTX_LENGTH_SHORT};
static const UDisplayContext optDiaMidLong[3] =
{UDISPCTX_DIALECT_NAMES,  UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE,    UDISPCTX_LENGTH_FULL};
static const UDisplayContext optDiaMidShrt[3] =
{UDISPCTX_DIALECT_NAMES,  UDISPCTX_CAPITALIZATION_FOR_MIDDLE_OF_SENTENCE,    UDISPCTX_LENGTH_SHORT};
...
static const UldnLocAndOpts uldnLocAndOpts[] = {
	{ "en", optStdMidLong, en_StdMidLong, SIZEOFARRAYi(en_StdMidLong) },
	{ "en", optStdMidShrt, en_StdMidShrt, SIZEOFARRAYi(en_StdMidShrt) },
	{ "en", optDiaMidLong, en_DiaMidLong, SIZEOFARRAYi(en_DiaMidLong) },
	{ "en", optDiaMidShrt, en_DiaMidShrt, SIZEOFARRAYi(en_DiaMidShrt) },
	{ "ro", optStdMidLong, ro_StdMidLong, SIZEOFARRAYi(ro_StdMidLong) },
	{ "yi", optStdMidLong, yi_StdMidLong, SIZEOFARRAYi(yi_StdMidLong) },
	{ NULL, NULL, NULL, 0 }
};
...
static void TestUldnNameVariants() {
	...
	for(uloPtr = uldnLocAndOpts; uloPtr->displayLocale != NULL; uloPtr++) {
		UErrorCode status = U_ZERO_ERROR;
		ULocaleDisplayNames * uldn =
		    uldn_openForContext(uloPtr->displayLocale, (UDisplayContext*)uloPtr->displayOptions, 3, &status);
		if(U_FAILURE(status)) {
		...
		for(; itemCount-- > 0; itemPtr++) {
			...
			switch(itemPtr->nameType) {
				...
				case TEST_ULOC_LANGUAGE:
				    ulenget =
					uloc_getDisplayLanguage(itemPtr->localeToName, uloPtr->displayLocale, uget, kUNameBuf, &status);
				...
				case TEST_ULOC_REGION:
				    ulenget =
					uloc_getDisplayCountry(itemPtr->localeToName, uloPtr->displayLocale, uget, kUNameBuf, &status);
			...
			if(U_FAILURE(status)) {
				log_data_err("%s fails, displayLocale %s, contexts %03X %03X %03X, localeToName %s: %s\n",
				    typeString,
				    uloPtr->displayLocale,
				    uloPtr->displayOptions[0],
				    uloPtr->displayOptions[1],
				    uloPtr->displayOptions[2],
				    itemPtr->localeToName,
				    u_errorName(status));
				continue;
			}
			...
			if(ulenget != ulenexp || u_strncmp(uget, itemPtr->expectResult, ulenexp) != 0) {
				...
				log_data_err(
					"%s fails, displayLocale %s, contexts %03X %03X %03X, localeToName %s:\n    expect %2d: %s\n    get    %2d: %s\n",
					typeString,
					uloPtr->displayLocale,
					uloPtr->displayOptions[0],
					uloPtr->displayOptions[1],
...
static void TestUsingDefaultWarning() 
{
	...
	if(status != U_USING_DEFAULT_WARNING || u_strcmp(buff, u"jjj") != 0 || length != 3) {
	...
	if(status != U_USING_DEFAULT_WARNING ||
	    u_strcmp(buff, u"Lala") != 0 || length != 4) {
		u_UCharsToChars(buff, errorOutputBuff, length+1);
	...
	if(status != U_USING_DEFAULT_WARNING || u_strcmp(buff, u"WT") != 0 || length != 2) {
		u_UCharsToChars(buff, errorOutputBuff, length+1);
	...
	if(status != U_USING_DEFAULT_WARNING || u_strcmp(buff, u"postCODE") != 0 || length != 8) {
		u_UCharsToChars(buff, errorOutputBuff, length+1);
