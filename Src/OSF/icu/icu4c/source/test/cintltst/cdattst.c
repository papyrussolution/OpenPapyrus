...
static void TestParseErrorReturnValue();
static void TestFormatForFields();
static void TestForceGannenNumbering();
...
static void TestDateFormat()
{
	...
	if(U_FAILURE(status)) {
		log_data_err(
			"FAIL: error in creating the dateformat using full time style with french locale -> %s (Are you missing data?)\n",
			myErrorName(status));
		return;
	}
	...
	for(stylePtr = dateStylesList, monthPtnPtr = monthPatnsList; *stylePtr != UDAT_NONE; ++stylePtr, ++monthPtnPtr) {
		...
		if(U_FAILURE(status)) {
			...
			if(U_FAILURE(status)) {
				...
				log_err(
					"udat_applyPatternRelative timeStyle SHORT dateStyle (%d | UDAT_RELATIVE) didn't update time pattern\n",
					*stylePtr);
			}
		}
		...
		for(dayOffset = -2, limit = 2; dayOffset <= limit; ++dayOffset) {
			...
			if(U_FAILURE(status)) {
				...
				if(U_FAILURE(status) || dateDiff > minutesTolerance) {
					log_err(
						"udat_parse timeStyle SHORT dateStyle (%d | UDAT_RELATIVE) fails, error %s, expect approx %.1f, got %.1f, parsePos %d\n",
						*stylePtr,
						myErrorName(status),
						dateToUse,
						dateResult,
				...
				if(U_FAILURE(status)) {
					...
					log_err(
						"relative date string not found in udat_format timeStyle SHORT dateStyle (%d | UDAT_RELATIVE)\n",
						*stylePtr);
				}
				else {
					parsePos = 0;
					...
					if(U_FAILURE(status) || dateDiff > daysTolerance) {
						log_err(
							"udat_parse timeStyle NONE dateStyle (%d | UDAT_RELATIVE) fails, error %s, expect approx %.1f, got %.1f, parsePos %d\n",
							*stylePtr,
							myErrorName(status),
							dateToUse,
							dateResult,
				...
				if(strPtr != NULL) {
					...
					if(fp.beginIndex != beginIndex) {
						log_err(
							"UFieldPosition beginIndex %d, expected %d, in udat_format timeStyle SHORT dateStyle (%d | UDAT_RELATIVE)\n",
							fp.beginIndex,
							beginIndex,
							*stylePtr);
					}
...
static void TestSymbols()
{
	...
	if(U_FAILURE(status)) {
		log_data_err(
			"error in creating the dateformat using full date, no time, locale zh@calendar=chinese -> %s (Are you missing data?)\n",
			myErrorName(status));
		return;
	}
	...
	/*since the month names has the last string empty and week names are 1 based 1.e first string in the weeknames
	   array is empty */
...
static bool _aux2ExtremeDates(UDateFormat* fmt, UDate small__, UDate large, char16_t * buf, int32_t buflen, char * cbuf, int32_t count, UErrorCode * ec) 
{
	...
	return _aux1ExtremeDates(fmt, mid, buf, buflen, cbuf, ec) && _aux2ExtremeDates(fmt, small__, mid, buf, buflen, cbuf, count+1, ec) &&
...
static void TestExtremeDates() 
{
	...
	ec = U_ZERO_ERROR;
	fmt = udat_open(UDAT_LONG, UDAT_LONG, "en_US", 0, 0, 0, 0, &ec);
	if(U_FAILURE(ec)) {
...
static void TestRelativeCrash() 
{
	...
	if(U_SUCCESS(status)) {
		const char * what = "???";
		{
			...
			udat_set2DigitYearStart(icudf, aDate, &subStatus); // @sobolev
				...
				if(U_SUCCESS(status)) {
					...
					if(U_SUCCESS(status)) {
						...
						if(U_FAILURE(status)) {
							...
							char bbuf2[kBbufMax];
							log_err(
								"FAIL: udat_format for locale %s, capitalizationContext %d, expected %s, got %s\n",
								textContextItemPtr->locale,
								(int)textContextItemPtr->capitalizationContext,
								u_austrncpy(bbuf1, textContextItemPtr->expectedFormat, kUbufMax),
						...
						if(U_FAILURE(status)) {
							...
							log_err(
								"FAIL: udat_getContext for locale %s, capitalizationContext %d, got context %d\n",
								textContextItemPtr->locale,
								(int)textContextItemPtr->capitalizationContext,
								(int)getContext);
						}
			...
			if(U_SUCCESS(status)) {
				...
				if(U_SUCCESS(status)) {
					...
					if(U_FAILURE(status)) {
						...
						log_err(
							"FAIL: udat_format today for locale %s, capitalizationContext %d, expected %s, got %s\n",
							textRelContextItemPtr->locale,
							(int)textRelContextItemPtr->capitalizationContext,
							u_austrncpy(bbuf1, textRelContextItemPtr->expectedFormatToday, kUbufMax),
							u_austrncpy(bbuf2, ubuf, kUbufMax));
					}
					status = U_ZERO_ERROR;
					...
					if(U_SUCCESS(status)) {
						...
						if(U_FAILURE(status)) {
							...
							log_err(
								...
								textRelContextItemPtr->locale,
								(int)textRelContextItemPtr->capitalizationContext,
								u_austrncpy(bbuf1, textRelContextItemPtr->expectedFormatYesterday,
								kUbufMax),
								u_austrncpy(bbuf2, ubuf, kUbufMax));
						}
					}
...
static void TestOverrideNumberFormat() {
	...
	for(i = 0; i<SIZEOFARRAYi(overrideNumberFormat); i++) {
		...
		else if(strcmp(overrideNumberFormat[i][0], "mixed") == 0) { // set 1 field at first but then full
			                                                    // override, both(M & d) should be override
			...
			if(U_FAILURE(status)) {
				...
				for(fptr = expectedFields;; fptr++) {
					...
					if(field != fptr->field || (field >= 0 && (beginPos != fptr->beginPos || endPos != fptr->endPos))) {
						if(fptr->field >= 0) {
							log_err(
								"udat_formatForFields as \"%s\"; expect field %d range %d-%d, get field %d range %d-%d\n",
							...
							log_err(
								"udat_formatForFields as \"%s\"; expect field < 0, get field %d range %d-%d\n",
								aescstrdup(ubuf, ulen),
								field,
								beginPos,
								endPos);
			...
			if(U_FAILURE(status)) {
				...
				for(fptr = expectedFields;; fptr++) {
					...
					if(field != fptr->field || (field >= 0 && (beginPos != fptr->beginPos || endPos != fptr->endPos))) {
						if(fptr->field >= 0) {
							log_err(
								"udat_formatFudat_formatCalendarForFieldsorFields as \"%s\"; expect field %d range %d-%d, get field %d range %d-%d\n",
								aescstrdup(ubuf, ulen),
								fptr->field,
								fptr->beginPos,
								fptr->endPos,
							...
							log_err(
								"udat_formatCalendarForFields as \"%s\"; expect field < 0, get field %d range %d-%d\n",
								aescstrdup(ubuf, ulen),
								field,
								beginPos,
								endPos);
