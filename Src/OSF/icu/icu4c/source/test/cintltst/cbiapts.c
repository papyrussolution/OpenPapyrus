...
static void TestBreakIteratorSafeClone();
static void TestBreakIteratorClone();
#endif
static void TestBreakIteratorRules();
static void TestBreakIteratorRuleError();
static void TestBreakIteratorStatusVec();
static void TestBreakIteratorUText();
static void TestBreakIteratorTailoring();
static void TestBreakIteratorRefresh();
static void TestBug11665();
static void TestBreakIteratorSuppressions();
...
static void TestBreakIteratorSafeClone(void)
{
	...
	for(i = 0; i < CLONETEST_ITERATOR_COUNT; i++) {
		...
		if(NULL ==
		    (brk = ubrk_safeClone(someIterators[i], NULL, &bufferSize, &status)) || status != U_SAFECLONE_ALLOCATED_WARNING) {
...
static void TestBreakIteratorRules() 
{
	...
	if(bi) {
		...
		for(i = 0; i < (int)sizeof(breaks); i++) {
			...
			if(pos == i) {
				int32_t tag = ubrk_getRuleStatus(bi);
				int32_t expectedTag = tagMap[tags[i]&0xf];
				if(tag != expectedTag) {
					log_err("FAIL: incorrect tag value.  Position = %d;  expected tag %d, got %d", pos, expectedTag, tag);
					break;
				}
				pos = ubrk_next(bi);
			}
 ...
 */
	...
	if(U_SUCCESS(status)) {
		...
		if(parseErr.line != 3 || parseErr.offset != 8) {
			log_data_err(
				"FAIL: incorrect error position reported. Got line %d, char %d, expected line 3, char 7 (Are you missing data?)\n",
				parseErr.line,
				parseErr.offset);
		}
	}
...
static const UChar kmTest[] = { /* 00 */ 0x179F, 0x17BC, 0x1798, 0x1785, 0x17C6, 0x178E, 0x17B6, 0x1799, 0x1796, 0x17C1,
	                                 ...
	                                 /* 50 */ 0x1782, 0 };
static const int32_t kmTestOffs_kmFwd[] =  {  3, /*8,*/ 11, 17, 23, 31, /*33,*/ 40,  43, 51 };  /* TODO: Investigate
                                                                                                   failure to break at
                                                                                                   offset 8 */
