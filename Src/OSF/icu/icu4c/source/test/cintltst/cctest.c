...
#include "cintltst.h"
#include "ustr_cnv.h"
void TestDefaultConverterError(); /* keep gcc happy */
void TestDefaultConverterSet(); /* keep gcc happy */

/* This makes sure that a converter isn't leaked when an error is passed to
    u_getDefaultConverter */
void TestDefaultConverterError() 
{
	...
	if(U_FAILURE(status)) {
		...
		if(!strcmp(nameBeforeSet, nameAfterSet)) { /* changing the default didn't affect. */
			...
			if(strcmp(nameBeforeSet, nameAfterRestore)) {
				log_err(
					"Error: u_getDefaultConverter() is still returning %s (expected %s) even though default converter was set back to %s (was %s)\n",
					nameAfterRestore,
					nameBeforeSet,
					defaultName,
					SET_TO);
