...
static void AssertAllPartsEqual(const char * messagePrefix,
...
#define AAPE_MSG(suffix) (uprv_strncpy(message+prefixEnd, suffix, 256-prefixEnd)-prefixEnd)
	...
	UFieldCategory _category = (UFieldCategory)ucfpos_getCategory(ucfpos, &status);
	...
	bool _matchesInteger = ucfpos_matchesField(ucfpos, UFIELD_CATEGORY_NUMBER, UNUM_INTEGER_FIELD, &status);
	assertSuccess(AAPE_MSG("integer field"), &status);
	assertTrue(AAPE_MSG("integer field"),
	    ((matching & 1) != 0) ? _matchesInteger : !_matchesInteger);

	bool _matchesCompact = ucfpos_matchesField(ucfpos, UFIELD_CATEGORY_NUMBER, UNUM_COMPACT_FIELD, &status);
	assertSuccess(AAPE_MSG("compact field"), &status);
	assertTrue(AAPE_MSG("compact field"),
	    ((matching & 2) != 0) ? _matchesCompact : !_matchesCompact);

	bool _matchesDate = ucfpos_matchesField(ucfpos, UFIELD_CATEGORY_DATE, UDAT_AM_PM_FIELD, &status);
