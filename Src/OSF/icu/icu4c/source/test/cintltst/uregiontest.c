	...
	for(rd = knownRegions; rd->code != NULL; rd++) {
		...
		if(U_SUCCESS(status) ) {
			...
			if(e > 0) {
				...
				if(!uregion_areEqual(ncRegion, r) && e != 891) { // 891 is special case - CS and YU both
					                                         // deprecated codes for region 891
					log_err(
						"ERROR: Creating region %s by its numeric code returned a different region. Got: %s instead.\n",
						uregion_getRegionCode(r),
						uregion_getRegionCode(ncRegion));
				}
			}
		}
		else {
				...
				while((crID = uenum_next(containedRegions, NULL, &status)) != NULL && U_SUCCESS(status) ) {
					...
					if(!containingRegion || !uregion_areEqual(containingRegion, r) ) {
						log_err(
							...
							uregion_getRegionCode(r),
							uregion_getRegionCode(cr),
							(containingRegion) ? uregion_getRegionCode(containingRegion) : "NULL");
					}
				}
			...
			if(rd->parent == NULL) {
				...
				if(c == NULL || !uregion_areEqual(p, c) ) {
					log_err("ERROR: Expected containing continent of region %s to be %s. Got: %s\n",
					    ...
					    (c) ? uregion_getRegionCode(c) : "NULL");
				}
			}
		}
		else {
...
static const char * expectPrefRegions4[] = { "PC", "FM", "MH", "MP", "PW", NULL };   /* Pacific Islands Trust Territory
       */
static const char * expectPrefRegions5[] =
			...
			if(U_SUCCESS(status) ) {
				if(preferredRegions != NULL) {
					...
					while((preferredCode = *regionListPtr++) != NULL) {
						...
						if(!found) {
							log_err(
								"ERROR: uregion_getPreferredValues for region \"%s\" should have contained \"%s\" but it didn't.\n",
								uregion_getRegionCode(r),
								preferredCode);
						}
					}
