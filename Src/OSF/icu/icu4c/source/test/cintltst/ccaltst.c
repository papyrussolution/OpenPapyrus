...
static const UCalGetTypeTest ucalGetTypeTests[] = {
	...
	{ NULL, (UCalendarType)0, NULL } /* terminator */
};
				...
				if(u_strcmp(zone2, EUROPE_PARIS) != 0) {
					...
					if(U_FAILURE(status) || status == U_STRING_NOT_TERMINATED_WARNING) {
						...
						if(u_strcmp(zone1, zone2) != 0) {
							log_err(
								"FAIL: ucal_getHostTimeZone() should give the same host timezone even if the default changed. (Got '%s', Expected '%s').\n",
								u_austrcpy(tempMsgBuf, zone2),
								u_austrcpy(tempMsgBuf2, zone1));
						}
					}
				}
			}
...
static void TestGetSetDateAPI()
{
	...
	else if((hour-8 + 1) != ucal_get(caldef2, UCAL_HOUR_OF_DAY, &status)) /*because it is not in daylight savings
		                                                                 time */
	...
	for(count = SIZEOFARRAYi(testDates); count-- > 0; ++testDatesPtr) {
		...
		if(U_SUCCESS(status)) {
			...
			for(subCount = testDatesPtr->numDates; subCount--; ++weekendDatesPtr) {
				UDate dateToTest;
				bool isWeekend;
				char fmtDateBytes[kFormattedDateMax] = "<could not format test date>"; /* initialize for
				                                                                          failure */
	...
	for(tfdItemPtr = tfdItems; tfdItemPtr->timezone != NULL; tfdItemPtr++) {
		...
		if(U_FAILURE(status)) {
			...
			if(tfdItemPtr->progressive) {
				...
				if(U_FAILURE(status)) {
					log_err(
						"FAIL: for locale \"%s\", start %.1f, target %.1f, ucal_setMillis or ucal_getFieldDifference had status %s\n",
						tfdItemPtr->locale,
						tfdItemPtr->start,
						tfdItemPtr->target,
						u_errorName(status));
					...
					log_data_err(
						"FAIL: for locale \"%s\", start %.1f, target %.1f, expected y-M-d-H-m-s progressive diffs %d-%d-%d-%d-%d-%d, got %d-%d-%d-%d-%d-%d\n",
						tfdItemPtr->locale,
						tfdItemPtr->start,
						tfdItemPtr->target,
						tfdItemPtr->yDiff,
				...
				if(U_FAILURE(status)) {
					log_err(
						"FAIL: for locale \"%s\", start %.1f, target %.1f, ucal_setMillis or ucal_getFieldDifference (y-M-d-H-m) had status %s\n",
						tfdItemPtr->locale,
						tfdItemPtr->start,
						tfdItemPtr->target,
						u_errorName(status));
				    ...
				    mDf !=  tfdItemPtr->mDiff) {
					log_data_err(
						"FAIL: for locale \"%s\", start %.1f, target %.1f, expected y-M-d-H-m total diffs %d-%d-%d-%d-%d, got %d-%d-%d-%d-%d\n",
						tfdItemPtr->locale,
						tfdItemPtr->start,
						tfdItemPtr->target,
						tfdItemPtr->yDiff,
				...
				if(tfdItemPtr->sDiff != 0x7FFFFFFF) {
					if(U_FAILURE(status)) {
						log_err(
							"FAIL: for locale \"%s\", start %.1f, target %.1f, ucal_setMillis or ucal_getFieldDifference (seconds) had status %s\n",
							tfdItemPtr->locale,
							tfdItemPtr->start,
							tfdItemPtr->target,
							u_errorName(status));
						...
						log_data_err(
							"FAIL: for locale \"%s\", start %.1f, target %.1f, expected seconds progressive diff %d, got %d\n",
							tfdItemPtr->locale,
							tfdItemPtr->start,
							tfdItemPtr->target,
							tfdItemPtr->sDiff,
					...
					log_err(
						"FAIL: for locale \"%s\", start %.1f, target %.1f, for ucal_getFieldDifference (seconds) expected overflow error, got none\n",
						tfdItemPtr->locale,
						tfdItemPtr->start,
						tfdItemPtr->target);
				}
...
typedef struct {
	const char * locale;
	bool era0YearsGoBackwards; /* until we have API to get this, per #9393 */
} EraTestItem;
	...
	for(eraTestItemPtr = eraTestItems; eraTestItemPtr->locale != NULL; eraTestItemPtr++) {
		...
		if(U_SUCCESS(status)) {
			...
			if(eraTestItemPtr->era0YearsGoBackwards) {
				...
				if(U_FAILURE(status)) {
					log_err("FAIL: set era 0 year 1 then roll 1 year and get year,era for %s, error %s\n",
					    eraTestItemPtr->locale, u_errorName(status));
					/* all calendars with era0YearsGoBackwards have "unbounded" era0 year values, so
					   we should pin at yr 1 */
				}
				else if(eraAfter != 0 || yrAfter != 1) {
					log_err(
						"FAIL: era 0 roll 1 year from year 1 does not stay within era or pin to year 1 for %s (get era %d year %d)\n",
						eraTestItemPtr->locale,
						eraAfter,
						yrAfter);
				}
			...
			if(U_SUCCESS(status) && eraMax > 0) {
				...
				ucal_set(ucalTest, UCAL_YEAR, 1);
				ucal_set(ucalTest, UCAL_ERA, 1);
				yrMax = ucal_getLimit(ucalTest, UCAL_YEAR, UCAL_ACTUAL_MAXIMUM, &status); /* max year
				                                                                             value for
				                                                                             era 1 */
				ucal_roll(ucalTest, UCAL_YEAR, -1, &status); /* roll down which should pin or wrap to
				                                                end */
				yrAfter = ucal_get(ucalTest, UCAL_YEAR, &status);
				eraAfter = ucal_get(ucalTest, UCAL_ERA, &status);
				if(U_FAILURE(status)) {
					...
					if(eraAfter != 1 || yrAfter != 1) {
						log_err(
							"FAIL: era 1 roll -1 year from year 1 does not stay within era or pin to year 1 for %s (get era %d year %d)\n",
							eraTestItemPtr->locale,
							eraAfter,
							yrAfter);
					}
					...
					log_err(
						"FAIL: era 1 roll -1 year from year 1 does not stay within era or wrap to year %d for %s (get era %d year %d)\n",
						yrMax,
						eraTestItemPtr->locale,
						eraAfter,
						yrAfter);
					...
					ucal_roll(ucalTest, UCAL_YEAR, 1, &status); /* now roll up which should wrap to
					                                               beginning */
					yrAfter = ucal_get(ucalTest, UCAL_YEAR, &status);
					eraAfter = ucal_get(ucalTest, UCAL_ERA, &status);
					if(U_FAILURE(status)) {
						...
						log_err(
							"FAIL: era 1 roll 1 year from year %d does not stay within era or wrap to year 1 for %s (get era %d year %d)\n",
							yrMax,
							eraTestItemPtr->locale,
							eraAfter,
							yrAfter);
				...
				if(U_SUCCESS(status) && eraNow > 1) {
					...
					yrMax = ucal_getLimit(ucalTest, UCAL_YEAR, UCAL_ACTUAL_MAXIMUM, &status); /* max
					                                                                             year
					                                                                             value
					                                                                             for
					                                                                             this
					...
					if(U_FAILURE(status)) {
						...
						/* if yrMax is reasonable we should wrap to that, else we should pin at
						   yr 1 */
					}
					else if(yrMax >= 32768) {
						if(eraAfter != eraNow || yrAfter != 1) {
							log_err(
								"FAIL: era %d roll -1 year from year 1 does not stay within era or pin to year 1 for %s (get era %d year %d)\n",
								eraNow,
								eraTestItemPtr->locale,
								eraAfter,
								yrAfter);
						...
						log_err(
							"FAIL: era %d roll -1 year from year 1 does not stay within era or wrap to year %d for %s (get era %d year %d)\n",
							eraNow,
							yrMax,
							eraTestItemPtr->locale,
							eraAfter,
						...
						/* now roll up which should wrap to beginning */
						ucal_roll(ucalTest, UCAL_YEAR, 1, &status); /* now roll up which should
						                                               wrap to beginning */
						yrAfter = ucal_get(ucalTest, UCAL_YEAR, &status);
						eraAfter = ucal_get(ucalTest, UCAL_ERA, &status);
						if(U_FAILURE(status)) {
							...
							log_err(
								"FAIL: era %d roll 1 year from year %d does not stay within era or wrap to year 1 for %s (get era %d year %d)\n",
								eraNow,
								yrMax,
								eraTestItemPtr->locale,
								eraAfter,
...
static const TZTransitionItem tzTransitionItems[] = {
	...
	{ "Cairo     mid 2012", zoneCairo,     2012, UCAL_JULY, 1, TRUE, TRUE  }, /* DST cancelled since 2011 (Changed
	                                                                          since 2014c) */
		...
		for(itemPtr = tzTransitionItems; itemPtr->descrip != NULL; itemPtr++) {
			...
			if(U_SUCCESS(status)) {
				...
				if(U_FAILURE(status) || result != itemPtr->hasPrev) {
					log_data_err(
						"FAIL: %s ucal_getTimeZoneTransitionDate prev status %s, expected result %d but got %d\n",
						itemPtr->descrip,
						u_errorName(status),
						itemPtr->hasPrev,
						result);
					...
					result = ucal_getTimeZoneTransitionDate(ucal,
						UCAL_TZ_TRANSITION_PREVIOUS_INCLUSIVE,
						&transition2,
						&status);
					if(U_FAILURE(status) || !result || transition2 != transition1) {
						log_err(
							"FAIL: %s ucal_getTimeZoneTransitionDate prev_inc status %s, result %d, expected date %.1f but got %.1f\n",
							itemPtr->descrip,
							u_errorName(status),
							result,
							transition1,
				...
				if(U_FAILURE(status) || result != itemPtr->hasNext) {
					log_data_err(
						"FAIL: %s ucal_getTimeZoneTransitionDate next status %s, expected result %d but got %d\n",
						itemPtr->descrip,
						u_errorName(status),
						itemPtr->hasNext,
						result);
					...
					if(U_FAILURE(status) || !result || transition2 != transition1) {
						log_err(
							"FAIL: %s ucal_getTimeZoneTransitionDate next_inc status %s, result %d, expected date %.1f but got %.1f\n",
							itemPtr->descrip,
							u_errorName(status),
							result,
							transition1,
...
void TestJpnCalAddSetNextEra() {
	...
	if(U_FAILURE(status)) {
		...
		if(U_FAILURE(status)) {
			...
			int32_t iEra, eYear;
			int32_t startYears[4] = { 1926, 1989, 2019, 0 }; // start years for Showa, Heisei, Reiwa; 0
			                                                 // marks invalid era
				...
				if(U_FAILURE(status)) {
					...
					if(U_FAILURE(status)) {
						...
						if(U_FAILURE(status)) {
							...
							// If this is the last valid era, we expect adding an era to pin
							// to the current era
							int32_t nextEraStart =
...
void TestUcalOpenBufferRead() {
	...
	const char * localeID =
	    "x-privatebutreallylongtagfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobar-foobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoobarfoorbarfoobarfoo";
