...
static void TestUnicodeIDs() {
	...
	if(U_FAILURE(errorCode) || count<1) {
		...
		if(U_FAILURE(errorCode) || count<1) {
			...
			log_err(
				"uenum_unext(transliterator IDs) returned %d IDs but uenum_count() after uenum_reset() claims there are %d\n",
				count,
				count2);
		}
	}
...
static void TestGetRulesAndSourceSet() {
	...
	if(U_SUCCESS(status)) {
		...
		if(status != U_BUFFER_OVERFLOW_ERROR || ulen <= 50 || ulen >= 100) {
			log_err(
				"FAIL: utrans_toRules unescaped, expected U_BUFFER_OVERFLOW_ERROR and len 50-100, got error=%s and len=%d\n",
				u_errorName(status),
				ulen);
		}
