...
// COPYRIGHT: Copyright (c) 2001-2016, International Business Machines Corporation and others. All Rights Reserved.
//
...
static void Test_FromUTF8();
static void Test_FromUTF8Lenient();
...
static void Test_UChar_UTF8_API() 
{
...

	{
		...
		if(err == U_BUFFER_OVERFLOW_ERROR && u8Temp[0] == 0x12) {
			err = U_ZERO_ERROR;
			u8Target = (char *)SAlloc::M(sizeof(uint8_t) * (u8DestLen+1));
	...
	// Since ICU 60, each surrogate byte sequence is treated as 3 single-byte errors.
	{
		static const UChar
		    ...
		    withTrail16Sub50005[] = { 0x1800, 0xd900, 0xdc05, 0xd900, 0xdc05, 0xd900, 0xdc05, 0x0061, 0 }; /*
		                                                                                                      sub==U+50005
		   */
...
static void Test_FromUTF8Lenient() {
	...
	for(pb = (const char *)bytes, pu = uchars;
	    *pb!=(char)0xff;
		...
		if(errorCode!= (destLength0==0 ? U_STRING_NOT_TERMINATED_WARNING : U_BUFFER_OVERFLOW_ERROR) || pDest!=NULL || dest[0]!=0x1234 || destLength!=srcLength) {
		...
		if(srcLength>0) {
			...
			if(errorCode!=U_BUFFER_OVERFLOW_ERROR || dest[srcLength-1]!=0x1234 || destLength!=srcLength) {
...
static void Test_UChar_WCHART_API() 
{
	...
	int i = 0;
	{
		...
		if(err == U_BUFFER_OVERFLOW_ERROR) {
			err = U_ZERO_ERROR;
			wDest = (wchar_t*)SAlloc::M(sizeof(wchar_t) * (reqLen+1));
			wDestLen = reqLen+1;
			u_strToWCS(wDest, wDestLen, &reqLen, uSrc, uSrcLen-1, &err);
		}
		...
		if(!U_FAILURE(err)) {
			for(i = 0; i< uSrcLen; i++) {
				if(uDest[i] != src16WithNulls[i]) {
					log_verbose(
						"u_str*WCS() failed for string with nulls expected: \\u%04X got: \\u%04X at index: %i \n",
						src16WithNulls[i],
						uDest[i],
						i);
					failed = TRUE;
	...
	}
	{
		...
		if(!U_FAILURE(err)) {
			for(i = 0; i< uSrcLen; i++) {
				if(uDest[i] != src16j[i]) {
					log_verbose(
						"u_str*WCS() failed for null terminated string expected: \\u%04X got: \\u%04X at index: %i \n",
						src16j[i],
						uDest[i],
						i);
					failed = TRUE;
...
#else
	log_info(
		"Not testing u_str*WCS because (!UCONFIG_NO_CONVERSION && !UCONFIG_NO_LEGACY_CONVERSION) and wchar is neither utf16 nor utf32");
#endif
}
...
static void Test_strToJavaModifiedUTF8() {
	...
	errorCode = U_ZERO_ERROR;
	length = -5;
	p = u_strToJavaModifiedUTF8(dest, sizeof(dest), &length, NULL, -1, &errorCode);
	if(errorCode!=U_ILLEGAL_ARGUMENT_ERROR || dest[0]!=(char)0xff) {
...
static void Test_strFromJavaModifiedUTF8() 
{
	...
	errorCode = U_ZERO_ERROR;
	length = numSubstitutions = -5;
	p = u_strFromJavaModifiedUTF8WithSub(dest, (int32_t)sizeof(dest), &length, (const char *)src, -1, 0xfffd, &numSubstitutions, &errorCode);
	if(U_FAILURE(errorCode) || p!=dest || length!=expectedTerminatedLength || 0!=memcmp(dest, expected, length) || dest[length]!=0 || numSubstitutions!=0) {
		log_err("u_strFromJavaModifiedUTF8WithSub(NUL-terminated) failed - %s\n", u_errorName(errorCode));
	}
	memset(dest, 0xff, sizeof(dest));
	errorCode = U_ZERO_ERROR;
	length = numSubstitutions = -5;
	p = u_strFromJavaModifiedUTF8WithSub(dest, (int32_t)sizeof(dest), NULL, (const char *)src, -1, 0xfffd, &numSubstitutions, &errorCode);
	if(U_FAILURE(errorCode) || p!=dest || 0!=memcmp(dest, expected, expectedTerminatedLength) || dest[expectedTerminatedLength]!=0 || numSubstitutions!=0) {
		log_err("u_strFromJavaModifiedUTF8WithSub(NUL-terminated, pLength=NULL) failed - %s\n", u_errorName(errorCode));
	}
	memset(dest, 0xff, sizeof(dest));
	errorCode = U_ZERO_ERROR;
	length = numSubstitutions = -5;
	p = u_strFromJavaModifiedUTF8WithSub(dest, (int32_t)sizeof(dest), &length, (const char *)src, -1, 0xfffd, NULL, &errorCode);
	if(U_FAILURE(errorCode) || p!=dest || length!=expectedTerminatedLength || 0!=memcmp(dest, expected, length) || dest[length]!=0) {
