...
#pragma hdrstop
/*
 * paragraphLayout doesn't make much sense without BreakIterator...
 */
...
StyleRuns::StyleRuns(const RunArray * styleRunArrays[], le_int32 styleCount) : fStyleCount(styleCount), fRunCount(0), fRunLimits(NULL), fStyleIndices(NULL)
{
...
static void fillMissingCharToGlyphMapValues(le_int32 * charToGlyphMap, le_int32 charCount) 
{
	le_int32 lastValidGlyph = -1;
	for(le_int32 ch = 0; ch <= charCount; ch += 1) {
 ...
 *
 */
ParagraphLayout::ParagraphLayout(const LEUnicode chars[], le_int32 count, const FontRuns   * fontRuns, const ValueRuns  * levelRuns,
    const ValueRuns  * scriptRuns, const LocaleRuns * localeRuns, UBiDiLevel paragraphLevel, le_bool vertical, LEErrorCode &status) : 
	fChars(chars), fCharCount(count), fFontRuns(NULL), fLevelRuns(levelRuns), fScriptRuns(scriptRuns), fLocaleRuns(localeRuns),
	fVertical(vertical), fClientLevels(TRUE), fClientScripts(TRUE), fClientLocales(TRUE), fEmbeddingLevels(NULL),
	fAscent(0), fDescent(0), fLeading(0), fGlyphToCharMap(NULL), fCharToMinGlyphMap(NULL), fCharToMaxGlyphMap(NULL), fGlyphWidths(NULL), fGlyphCount(0),
	fParaBidi(NULL), fLineBidi(NULL), fStyleRunLimits(NULL), fStyleIndices(NULL), fStyleRunCount(0),
	   ...
	   fFirstVisualRun(-1), fLastVisualRun(-1),*/fVisualRunLastX(0), fVisualRunLastY(0)
{
	...
	if((fGlyphWidths == NULL) || (fGlyphToCharMap == NULL) || (fCharToMinGlyphMap == NULL) || (fCharToMaxGlyphMap == NULL)) {
		status = LE_MEMORY_ALLOCATION_ERROR;
		return;
	}
	le_int32 glyph;
	for(runStart = 0, run = 0; run < fStyleRunCount; run += 1) {
		...
		for(glyph = 0; glyph < glyphCount; glyph += 1) {
			fGlyphWidths[glyphBase + glyph] = fStyleRunInfo[run].positions[glyph * 2 + 2] -
			    fStyleRunInfo[run].positions[glyph * 2];
		}
...
ParagraphLayout::~ParagraphLayout()
{
	...
	if(fStyleRunCount > 0) {
		...
		LE_DELETE_ARRAY(fStyleIndices);
		for(le_int32 run = 0; run < fStyleRunCount; run += 1) {
...
{
	const le_int32 count = fFontRuns->getCount();
	le_int32 maxDL = 0;
...
static const le_int32 languageMapCount = SIZEOFARRAY(languageMap);
...
le_int32 ParagraphLayout::getLanguageCode(const Locale * locale)
{
	...
	if(sstreq(language, "zho") && country) {
		uprv_strcat(code, "_");
		uprv_strcat(code, country);
	}
	for(le_int32 i = 0; i < languageMapCount; i += 1) {
		if(sstreq(code, languageMap[i].localeCode)) {
...
{
	return (script < 0 || script >= SIZEOFARRAY(complexTable)) ? FALSE : complexTable[script];
}
...
le_int32 ParagraphLayout::previousBreak(le_int32 charIndex)
{
	...
	while(charIndex < fCharCount && (u_isWhitespace(fChars[charIndex]) || u_iscntrl(fChars[charIndex]))) {
		charIndex += 1;
	}
