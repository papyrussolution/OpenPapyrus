...
bool IcuTestErrorCode::errIfFailureAndReset() {
	if(isFailure()) {
...
bool IcuTestErrorCode::errIfFailureAndReset(const char * fmt, ...) {
	if(isFailure()) {
		char buffer[4000];
		va_list ap;
		va_start(ap, fmt);
...
bool IcuTestErrorCode::errDataIfFailureAndReset() {
	if(isFailure()) {
...
bool IcuTestErrorCode::errDataIfFailureAndReset(const char * fmt, ...) 
{
	if(isFailure()) {
		char buffer[4000];
		va_list ap;
...
bool IcuTestErrorCode::expectErrorAndReset(UErrorCode expectedError) {
	if(get() != expectedError) {
		errlog(FALSE, UnicodeString(u"expected: ") + UnicodeString(u_errorName(expectedError)), nullptr);
	}
	bool retval = isFailure();
	reset();
	return retval;
}

bool IcuTestErrorCode::expectErrorAndReset(UErrorCode expectedError, const char * fmt, ...) {
	if(get() != expectedError) {
		char buffer[4000];
		va_list ap;
		va_start(ap, fmt);
...
void IcuTestErrorCode::errlog(bool dataErr, const UnicodeString & mainMessage, const char * extraMessage) const 
{
	UnicodeString msg(testName, -1, US_INV);
	msg.append(u' ').append(mainMessage);
	msg.append(UnicodeString(u" but got error: ")).append(UnicodeString(errorName(), -1, US_INV));
	if(!scopeMessage.isEmpty()) {
		msg.append(UnicodeString(u" scope: ")).append(scopeMessage);
	}
	if(extraMessage != nullptr) {
		msg.append(UnicodeString(u" - ")).append(UnicodeString(extraMessage, -1, US_INV));
	}
...
bool RBTestDataModule::getInfo(const DataMap *& info, UErrorCode & /*status*/) const
{
	info = fInfo;
	if(fInfo) {
		return TRUE;
