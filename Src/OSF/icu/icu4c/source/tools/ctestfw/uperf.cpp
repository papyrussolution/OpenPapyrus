...
UPerfTest::UPerfTest(int32_t argc, const char * argv[], UOption addOptions[], int32_t addOptionsCount, const char * addUsage, UErrorCode & status) : 
	ArgC(argc), ArgV(argv), _addUsage(addUsage), ucharBuf(NULL), encoding(""), uselen(FALSE), fileName(NULL), sourceDir("."),
	lines(NULL), numLines(0), line_mode(TRUE), buffer(NULL), bufferLen(0), verbose(FALSE), bulk_mode(FALSE), passes(1), iterations(0), time(0), locale(NULL) 
{
...
UPerfFunction* UPerfTest::runIndexedTest(int32_t /*index*/, bool /*exec*/, const char * & /*name*/, char * /*par*/)
{
...
bool UPerfTest::runTestLoop(char * testname, char * par)
{
	int32_t index = 0;
	const char * name;
	bool run_this_test;
...
bool UPerfTest::callTest(UPerfTest& testToBeCalled, char * par)
{
