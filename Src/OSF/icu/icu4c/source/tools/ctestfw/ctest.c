...
static int strncmp_nullcheck(const char * s1, const char * s2, int n);
static void getNextLevel(const char * name, int* nameLen, const char ** nextName);
static void iterateTestsWithLevel(const TestNode * root, int depth, const TestNode** nodeList, TestMode mode);
...
static bool NO_KNOWN = FALSE;
static void * knownList = NULL;
static char gTestName[1024] = "";
static bool ON_LINE = FALSE; /* are we on the top line with our test name? */
static bool HANGING_OUTPUT = FALSE; /* did the user leave us without a trailing \n ? */
...
static TestNode * createTestNode(const char * name, int32_t nameLen)
{
	TestNode * newNode = (TestNode*)SAlloc::M(sizeof(TestNode) + (nameLen + 1));
	newNode->test = NULL;
...
void T_CTEST_EXPORT2 cleanUpTestTree(TestNode * tn)
{
	if(tn->child) {
...
void T_CTEST_EXPORT2 addTest(TestNode** root, TestFunctionPtr test, const char * name)
{
	TestNode * newNode;
 ...
 * @param nodeList an array of MAXTESTS depth that's used for keeping track of where we are. nodeList[depth] points to
 *the 'parent' at depth depth.
	...
	if(root->child) {
		...
		if(mode!=SHOWTESTS) {
			...
			if((depth>1) && (ERROR_COUNT > myERROR_COUNT)) {
				log_testinfo("[%d %s in %s] ",
				    ERROR_COUNT-myERROR_COUNT,
				    (ERROR_COUNT-myERROR_COUNT)==1 ? "error" : "errors",
				    pathToFunction);
			}
...
void T_CTEST_EXPORT2 showTests(const TestNode * root)
{
...
void T_CTEST_EXPORT2 runTests(const TestNode * root)
{
	int i;
	const TestNode * nodeList[MAXTESTS];
	/* make up one for them */
...
const char * T_CTEST_EXPORT2 getTestName(void)
{
	if(currentTest != NULL) {
...
const TestNode* T_CTEST_EXPORT2 getTest(const TestNode* root, const char * name)
{
	const char * nextName;
	TestNode * nextNode;
...
void T_CTEST_EXPORT2 vlog_info(const char * prefix, const char * pattern, va_list ap)
{
	first_line_info();
	fprintf(stdout, "%-*s", INDENT_LEVEL, "");
	if(prefix) {
...
void T_CTEST_EXPORT2 log_err(const char * pattern, ...)
{
	va_list ap;
	first_line_err();
	if(strchr(pattern, '\n') != NULL) {
...
bool T_CTEST_EXPORT2 log_knownIssue(const char * ticket, const char * pattern, ...) {
	va_list ap;
	va_start(ap, pattern);
	return vlog_knownIssue(ticket, pattern, ap);
}

void T_CTEST_EXPORT2 log_err_status(UErrorCode status, const char * pattern, ...)
{
	va_list ap;
	va_start(ap, pattern);
...
void T_CTEST_EXPORT2 log_info(const char * pattern, ...)
{
	va_list ap;
...
void T_CTEST_EXPORT2 log_verbose(const char * pattern, ...)
{
	va_list ap;
	va_start(ap, pattern);
...
void T_CTEST_EXPORT2 log_data_err(const char * pattern, ...)
{
	va_list ap;
	va_start(ap, pattern);
	go_offline_err();
...
static void U_CALLCONV TraceData(const void * context, int32_t fnNumber, int32_t level, const char * fmt, va_list args) 
{
...
int T_CTEST_EXPORT2 initArgs(int argc, const char * const argv[], ArgHandlerPtr argHandler, void * context)
{
	int i;
	int argSkip = 0;
	VERBOSITY = FALSE;
...
int T_CTEST_EXPORT2 runTestRequest(const TestNode* root, int argc, const char * const argv[])
{
	/**
...
int32_t T_CTEST_EXPORT2 getTestOption(int32_t testOption) {
	switch(testOption) {
...
void T_CTEST_EXPORT2 setTestOption(int32_t testOption, int32_t value) 
{
	...
	switch(testOption) {
		case VERBOSITY_OPTION: VERBOSITY = value; break;
		case WARN_ON_MISSING_DATA_OPTION: WARN_ON_MISSING_DATA = value; break;
		case QUICK_OPTION: QUICK = value; break;
		case REPEAT_TESTS_OPTION: REPEAT_TESTS = value; break;
		case ICU_TRACE_OPTION: ICU_TRACE = (UTraceLevel)value; break;
		case WRITE_GOLDEN_DATA_OPTION: WRITE_GOLDEN_DATA = value;
		default: break;
	}
}
...
int32_t T_CTEST_EXPORT2 ctest_xml_setFileName(const char * name) 
{
	XML_FILE_NAME = name;
	return 0;
}

int32_t T_CTEST_EXPORT2 ctest_xml_init(const char * rootName) 
{
...
int32_t T_CTEST_EXPORT2 ctest_xml_fini(void) 
{
...
int32_t T_CTEST_EXPORT2 ctest_xml_testcase(const char * classname, const char * name, const char * timeSeconds, const char * failMsg) 
{
	if(!XML_FILE) return 0;
	fprintf(XML_FILE, "\t<testcase classname=\"%s:%s\" name=\"%s:%s\" time=\"%s\"", XML_PREFIX, classname, XML_PREFIX, name,
	    timeSeconds);
	if(failMsg) {
