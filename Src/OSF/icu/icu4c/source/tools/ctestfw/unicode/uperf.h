...
public:
	bool run();
	bool runTest(char * name = NULL, char * par = NULL); // not to be overridden
...
protected:
	...
	virtual UPerfFunction* runIndexedTest(int32_t index, bool exec, const char *& name, char * par = NULL); // override!
	virtual bool runTestLoop(char * testname, char * par);
	virtual bool callTest(UPerfTest& testToBeCalled, char * par);
