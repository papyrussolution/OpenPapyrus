...
public:
	IcuTestErrorCode(TestLog &callingTestClass, const char * callingTestName) : testClass(callingTestClass), testName(callingTestName),
		scopeMessage()
	{
	}
	virtual ~IcuTestErrorCode();
	// Returns TRUE if isFailure().
	bool errIfFailureAndReset();
	bool errIfFailureAndReset(const char * fmt, ...);
	bool errDataIfFailureAndReset();
	bool errDataIfFailureAndReset(const char * fmt, ...);
	bool expectErrorAndReset(UErrorCode expectedError);
	bool expectErrorAndReset(UErrorCode expectedError, const char * fmt, ...);
...
private:
	...
	void errlog(bool dataErr, const UnicodeString & mainMessage, const char * extraMessage) const;
};
