...
TestData::TestData(const char * testName) : name(testName), fInfo(NULL), fCurrSettings(NULL), fCurrCase(NULL),
	fSettingsSize(0), fCasesSize(0), fCurrentSettings(0), fCurrentCase(0)
{
}
...
RBTestData::RBTestData(const char * testName) : TestData(testName), fData(NULL), fHeaders(NULL), fSettings(NULL), fCases(NULL)
{
}

RBTestData::RBTestData(UResourceBundle * data, UResourceBundle * headers, UErrorCode & status) : TestData(ures_getKey(data)),
	fData(data), fHeaders(headers), fSettings(NULL), fCases(NULL)
{
...
bool RBTestData::getInfo(const DataMap *& info, UErrorCode & /*status*/) const
{
	if(fInfo) {
		info = fInfo;
		return TRUE;
...
bool RBTestData::nextSettings(const DataMap *& settings, UErrorCode & status)
{
	UErrorCode intStatus = U_ZERO_ERROR;
...
bool RBTestData::nextCase(const DataMap *& nextCase, UErrorCode & status)
{
	UErrorCode intStatus = U_ZERO_ERROR;
