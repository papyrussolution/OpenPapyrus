...
static void printUsage(const char * pname, bool isHelp) 
{
...
static bool isPackageName(const char * filename) {
	int32_t len;

	len = (int32_t)strlen(filename)-4; /* -4: subtract the length of ".dat" */
	return (bool)(len>0 && 0==strcmp(filename+len, ".dat"));
}
...
extern int main(int argc, char * argv[]) {
	...
	char outType;
	bool isHelp, isModified, isPackage;
	int result = 0;
	...
	if(argc>=3) {
		...
		isModified = (bool)(sourcePath!=destPath);
	}
	...
	if(options[OPT_OUT_TYPE].doesOccur) {
		 ...
		 */
		isModified |= (bool)(!isPackage || outType!=pkg->getInType());
	}
	else if(isPackage) {
	...
	if(!isPackage) {
		 ...
		 */
		if(options[OPT_COMMENT].doesOccur || options[OPT_COPYRIGHT].doesOccur || options[OPT_MATCHMODE].doesOccur ||
		    options[OPT_REMOVE_LIST].doesOccur || options[OPT_ADD_LIST].doesOccur || options[OPT_EXTRACT_LIST].doesOccur ||
