...
const char * pkg_writeCharListWrap(FileStream * s, CharList * l, const char * delim, const char * brk, int32_t quote)
{
	...
	while(l != NULL) {
		if(l->str) {
			...
			T_FileStream_write(s, buffer, (int32_t)strlen(buffer));
			ln += (int32_t)strlen(l->str);
		}
		if(l->next && delim) {
			if(ln > 60 && brk) {
				ln  = 0;
				T_FileStream_write(s, brk, (int32_t)strlen(brk));
			}
			T_FileStream_write(s, delim, (int32_t)strlen(delim));
		}
		l = l->next;
	}
	return NULL;
...
char * convertToNativePathSeparators(char * path) 
{
	...
	char * itr;
	while((itr = sstrchr(path, U_FILE_ALT_SEP_CHAR))) {
		*itr = U_FILE_SEP_CHAR;
	}
#endif
	return path;
...
CharList * pkg_appendUniqueDirToList(CharList * l, CharList** end, const char * strAlias) 
{
	...
	if((rPtr-strAlias) >= SIZEOFARRAYi(aBuf)) {
...
#if 0
static CharList * pkg_appendFromStrings(CharList * l, CharList** end, const char * s, int32_t len)
{
...
bool  pkg_listContains(CharList * l, const char * str)
{
	for(; l; l = l->next) {
		if(!strcmp(l->str, str)) {
			return true;
