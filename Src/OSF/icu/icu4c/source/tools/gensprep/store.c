 ...
 *table
 *  indexes[_SPREP_THREE_UCHARS_MAPPING_INDEX_START] -- The starting index of 3 UChars mapping index in the mapping
 *table
 *  indexes[_SPREP_FOUR_UCHARS_MAPPING_INDEX_START]  -- The starting index of 4 UChars mapping index in the mapping
 *table
 *  indexes[_SPREP_OPTIONS]                          -- Bit set of options to turn on in the profile, e.g:
 *USPREP_NORMALIZATION_ON, USPREP_CHECK_BIDI_ON
 *
 *
 * StringPrep Trie :
 *
 ...
 *
 *      0       -  ON : The code point is prohibited (USPREP_PROHIBITED). This is to allow for codepoint that are both
 *prohibited and mapped.
...
{
	return (bool)(p1.integer != p2.integer);
}

static void storeMappingData() 
{
