...
static UChar32 getNextChar(UCHARBUF * buf, bool skipwhite, struct UString * token, UErrorCode * status);
...
static bool   isNewline(UChar32 c);
...
/* Read and return the next token from the stream.  If the token is of
   ...
   space. */
U_CFUNC enum ETokenType getNextToken(UCHARBUF* buf,
    struct UString * token,
		...
		switch(c) {
			...
			case ASTERISK: /* " / * " */
			    ...
			    if(c2 == ASTERISK) { /* "/ * *" */
				    ...
				    ucbuf_ungetc(c2, buf); /* c2 is the non-asterisk following "/ *".  Include c2  back
				                              in buffer.  */
