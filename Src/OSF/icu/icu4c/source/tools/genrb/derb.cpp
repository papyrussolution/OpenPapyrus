...
static bool opt_truncate = FALSE;
...
static bool suppressAliases = FALSE;
...
/*    int32_t noOfElements = ures_getSize(resource);*/
	...
	switch(ures_getType(resource)) {
		...
		case URES_BINARY:
	    {
		    ...
		    if(U_SUCCESS(*status)) {
			    static const char16_t openStr[] =
			    { 0x003A, 0x0062, 0x0069, 0x006E, 0x0061, 0x0072, 0x0079, 0x0020, 0x007B, 0x0020 };                    /*
				                                                                                                      ":binary
				                                                                                                      {
				                                                                                                      "
				   */
		...
		case URES_INT_VECTOR:
	    {
		    ...
		    if(U_SUCCESS(*status)) {
			    static const char16_t openStr[] =
			    { 0x003A, 0x0069, 0x006E, 0x0074, 0x0076, 0x0065, 0x0063, 0x0074, 0x006F, 0x0072, 0x0020, 0x007B,
				      0x0020 };                                                                                                            /*
				                                                                                                                              ":intvector
				                                                                                                                              {
				                                                                                                                              "
		...
		case URES_ARRAY:
	    {
		    ...
		    if(suppressAliases == FALSE) {
			    ...
			    bool isTable = (bool)(ures_getType(resource) == URES_TABLE);
			    for(i = 0; i < resSize; i++) {
