...
struct SRBRoot {
    SRBRoot(const UString *comment, bool isPoolBundle, UErrorCode &errorCode);
    ~SRBRoot();
...
public:
  ...
  bool fNoFallback; /* see URES_ATT_NO_FALLBACK */
...
struct SResource {
    ...
    bool isTable() const { return fType == URES_TABLE; }
    bool isString() const { return fType == URES_STRING; }
