 ...
 */
U_CAPI bool U_EXPORT2 isFileModTimeLater(const char * filePath, const char * checkAgainst, bool isDir) {
	bool isLatest = TRUE;
			...
			while((dirEntry = readdir(pDir)) != NULL) {
				if(uprv_strcmp(dirEntry->d_name, SKIP1) != 0 && uprv_strcmp(dirEntry->d_name, SKIP2) != 0) {
					...
					if((subDirp = opendir(newpath.data())) != NULL) {
						/* If this new path is a directory, make a recursive call with the
						   newpath. */
						closedir(subDirp);
