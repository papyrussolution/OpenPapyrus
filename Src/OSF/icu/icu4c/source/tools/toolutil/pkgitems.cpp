...
public:
	...
	const UDataInfo * getDataInfo() const { return pInfo; }
	...
	void setItem(const Item * item, UDataSwapFn * swap) 
	{
		...
		if(pInfo->isBigEndian==U_IS_BIG_ENDIAN && pInfo->charsetFamily==U_CHARSET_FAMILY) {
			...
			UDataSwapper * ds = udata_openSwapper((bool)pInfo->isBigEndian,
				pInfo->charsetFamily,
				U_IS_BIG_ENDIAN,
				U_CHARSET_FAMILY,
				&errorCode);
 ...
 * and a suffix
 */
static void makeTargetName(const char * itemName, const char * id, int32_t idLength, const char * suffix,
...
static void checkIDSuffix(const char * itemName, const char * id, int32_t idLength, const char * suffix,
...
static void checkParent(const char * itemName, CheckDependency check, void * context,
    UErrorCode * pErrorCode) {
 ...
 * aliases.
 */
static void ures_enumDependencies(const char * itemName,
...
static void ures_enumDependencies(const char * itemName, const UDataInfo * pInfo,
...
static void ucnv_enumDependencies(const UDataSwapper * ds,
		...
		switch(format) {
			...
			case FMT_CNV:
		    {
			    ...
			    UDataSwapper * ds = udata_openSwapper(
				    (bool)pInfo->isBigEndian, pInfo->charsetFamily,
