...
U_CAPI UNewDataMemory * U_EXPORT2 udata_create(const char * dir, const char * type, const char * name,
...
U_CAPI uint32_t U_EXPORT2 udata_finish(UNewDataMemory * pData, UErrorCode * pErrorCode) {
...
U_CAPI void U_EXPORT2 udata_createDummy(const char * dir, const char * type, const char * name, UErrorCode * pErrorCode) {
...
U_CAPI void U_EXPORT2 udata_write8(UNewDataMemory * pData, uint8_t byte) {
	if(pData && pData->file) {
		T_FileStream_write(pData->file, &byte, 1);
	}
}

U_CAPI void U_EXPORT2 udata_write16(UNewDataMemory * pData, uint16_t word) {
	if(pData && pData->file) {
		T_FileStream_write(pData->file, &word, 2);
	}
}

U_CAPI void U_EXPORT2 udata_write32(UNewDataMemory * pData, uint32_t wyde) {
	if(pData && pData->file) {
		T_FileStream_write(pData->file, &wyde, 4);
	}
}

U_CAPI void U_EXPORT2 udata_writeBlock(UNewDataMemory * pData, const void * s, int32_t length) {
	if(pData && pData->file) {
		if(length>0) {
...
U_CAPI void U_EXPORT2 udata_writePadding(UNewDataMemory * pData, int32_t length) {
...
U_CAPI void U_EXPORT2 udata_writeString(UNewDataMemory * pData, const char * s, int32_t length) 
{
	if(pData && pData->file) {
		if(length==-1) {
