...
struct U_TOOLUTIL_API UniProps {
    ...
    UChar32 start, end;
    bool binProps[UCHAR_BINARY_LIMIT];
...
public:
  ...
  * Splits the line by replacing each ';' with a NUL.
    ...
    bool lineHasPropertyValues() const {
  ...
  * Returns U_SUCCESS(errorCode).
     */
    bool getRangeForAlgNames(UChar32 &start, UChar32 &end, UErrorCode &errorCode);

private:
    bool isLineBufferAvailable(int32_t i) {
    ...
    bool parseProperty(UniProps &props, const char *field, UnicodeSet &newValues,
                        UErrorCode &errorCode);
    UChar32 parseCodePoint(const char *s, UErrorCode &errorCode);
    bool parseCodePointRange(const char *s, UChar32 &start, UChar32 &end, UErrorCode &errorCode);
