 ...
 */
static int32_t _udbg_enumCount(UDebugEnumType type, bool actual) {
	switch(type) {
...
typedef int32_t U_CALLCONV USystemParameterCallback (const USystemParams * param, char * target, int32_t targetCapacity,
    UErrorCode * status);
...
/* parameter types */
U_CAPI int32_t paramEmpty(const USystemParams * /* param */, char * target, int32_t targetCapacity, UErrorCode * status) {
...
U_CAPI int32_t paramInteger(const USystemParams * param, char * target, int32_t targetCapacity, UErrorCode * status) {
	if(U_FAILURE(*status)) return 0;
...
U_CAPI int32_t paramCldrVersion(const USystemParams * /* param */, char * target, int32_t targetCapacity, UErrorCode * status) {
	if(U_FAILURE(*status)) return 0;
	char str[200] = "";
	UVersionInfo icu;
...
#if !UCONFIG_NO_FORMATTING
U_CAPI int32_t paramTimezoneDefault(const USystemParams * /* param */, char * target, int32_t targetCapacity, UErrorCode * status) {
	if(U_FAILURE(*status)) return 0;
	char16_t buf[100];
	char buf2[100];
	int32_t len;
...
U_CAPI int32_t paramLocaleDefaultBcp47(const USystemParams * /* param */, char * target, int32_t targetCapacity, UErrorCode * status) {
	if(U_FAILURE(*status)) return 0;
...
public:
	KnownIssues();
	~KnownIssues();
	void add(const char * ticket, const char * where, const char16_t * msg, bool * firstForTicket, bool * firstForWhere);
	void add(const char * ticket, const char * where, const char * msg, bool * firstForTicket, bool * firstForWhere);
	bool print();
private:
	std::map< std::string,
...
void KnownIssues::add(const char * ticketStr, const char * where, const char16_t * msg, bool * firstForTicket, bool * firstForWhere)
{
...
void KnownIssues::add(const char * ticketStr, const char * where, const char * msg, bool * firstForTicket, bool * firstForWhere)
{
	const std::string ticket = mapTicketId(ticketStr);
	if(fTable.find(ticket) == fTable.end()) {
		if(firstForTicket!=NULL) *firstForTicket = TRUE;
