...
U_CAPI void U_EXPORT2 ucm_printTable(UCMTable * table, FILE * f, bool byUnicode) 
{
	int32_t i, length;
...
static int32_t compareUnicode(UCMTable * lTable, const UCMapping * l,
...
static int32_t compareBytes(UCMTable * lTable, const UCMapping * l,
    UCMTable * rTable, const UCMapping * r,
    bool lexical) {
	const uint8_t * lb, * rb;
	int32_t result, i, length;
...
static int32_t compareMappings(UCMTable * lTable, const UCMapping * l,
    UCMTable * rTable, const UCMapping * r,
    bool uFirst) {
	int32_t result;
...
static uint8_t checkBaseExtUnicode(UCMStates * baseStates, UCMTable * base, UCMTable * ext, bool moveToExt, bool intersectBase) 
{
	(void)baseStates;
	int32_t cmp;
	UCMapping * mb = base->mappings;
	UCMapping * mbLimit = mb+base->mappingsLength;
	UCMapping * me = ext->mappings;
	UCMapping * meLimit = me+ext->mappingsLength;
	uint8_t result = 0;
	for(;;) {
		...
		if(cmp < 0) {
			// @sobolev Судя по вызовам функции, условие (intersectBase!=2) всегда истинно - вызовы все с константными значениями intersectBase
...
U_CAPI bool U_EXPORT2 ucm_checkBaseExt(UCMStates * baseStates, UCMTable * base, UCMTable * ext, UCMTable * moveTarget, bool intersectBase) 
{
	uint8_t result;
	...
	ucm_sortTable(ext);
	// check 
	result = checkBaseExtUnicode(baseStates, base, ext, (moveTarget!=NULL), intersectBase) | checkBaseExtBytes(baseStates, base, ext, (moveTarget!=NULL), intersectBase);
...
U_CAPI void U_EXPORT2 ucm_mergeTables(UCMTable * fromUTable, UCMTable * toUTable,
	...
	while(fromUIndex<fromUTop && toUIndex<toUTop) {
		...
		if(cmp==0) {
			...
			if(!(toUMapping->uLen==1 && (toUMapping->u==0xfffd || toUMapping->u==0x1a))) {
				...
				ucm_addMapping(fromUTable, toUMapping, UCM_GET_CODE_POINTS(toUTable, toUMapping),
				    UCM_GET_BYTES(toUTable, toUMapping));
	...
	while(toUIndex<toUTop) {
		...
		if(!(toUMapping->uLen==1 && (toUMapping->u==0xfffd || toUMapping->u==0x1a))) {
			toUMapping->f = 3; /* reverse fallback */
			ucm_addMapping(fromUTable,
			    toUMapping,
			    UCM_GET_CODE_POINTS(toUTable, toUMapping),
			    UCM_GET_BYTES(toUTable, toUMapping));
		}
...
U_CAPI bool U_EXPORT2 ucm_separateMappings(UCMFile * ucm, bool isSISO) 
{
	int32_t type;
	UCMTable * table = ucm->base;
	UCMapping * m = table->mappings;
	UCMapping * mLimit = m+table->mappingsLength;
...
U_CAPI bool U_EXPORT2 ucm_parseMappingLine(UCMapping * m,
...
U_CAPI int32_t U_EXPORT2 ucm_mappingType(UCMStates * baseStates,
    UCMapping * m,
    UChar32 codePoints[UCNV_EXT_MAX_UCHARS],
    uint8_t bytes[UCNV_EXT_MAX_BYTES]) {
	(void)codePoints;
...
U_CAPI bool U_EXPORT2 ucm_addMappingAuto(UCMFile * ucm, bool forBase, UCMStates * baseStates,
    UCMapping * m,
    UChar32 codePoints[UCNV_EXT_MAX_UCHARS],
    uint8_t bytes[UCNV_EXT_MAX_BYTES]) {
	int32_t type;
...
U_CAPI bool U_EXPORT2 ucm_addMappingFromLine(UCMFile * ucm, const char * line, bool forBase, UCMStates * baseStates) {
...
U_CAPI void U_EXPORT2 ucm_readTable(UCMFile * ucm, FileStream* convFile,
    bool forBase, UCMStates * baseStates,
