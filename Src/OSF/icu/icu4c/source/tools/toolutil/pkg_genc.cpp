...
U_CAPI void U_EXPORT2 writeAssemblyCode(const char * filename, const char * destdir, const char * optEntryPoint,
    const char * optFilename, char * outFilePath, size_t outFilePathCapacity) 
{
	...
	for(;;) {
		memzero(buffer.uint32s, sizeof(buffer.uint32s));
...
static void getArchitecture(uint16_t * pCPU, uint16_t * pBits, bool * pIsBigEndian, const char * optMatchArch) {
	union {
		char bytes[2048];
#ifdef U_ELF
		Elf32_Ehdr header32;
...
#   error "Unknown platform for CAN_GENERATE_OBJECTS."
	...
	if(optMatchArch != NULL) {
		...
		*pCPU = EM_386;
		*pBits = 32;
		*pIsBigEndian = (bool)(U_IS_BIG_ENDIAN ? ELFDATA2MSB : ELFDATA2LSB);
#elif U_PLATFORM_HAS_WIN32_API
