...
struct UCHARBUF {
	...
	FileStream* in;
	UConverter * conv;
	bool showWarning; /* makes this API not produce any errors */
	bool isBuffered;
};

U_CAPI bool U_EXPORT2 ucbuf_autodetect_fs(FileStream* in, const char ** cp, UConverter ** conv, int32_t* signatureLength,
    UErrorCode * error) {
	char start[8];
	int32_t numRead;
...
static bool ucbuf_isCPKnown(const char * cp) {
...
U_CAPI FileStream * U_EXPORT2 ucbuf_autodetect(const char * fileName,
    const char ** cp,
    UConverter ** conv,
    int32_t* signatureLength,
    UErrorCode * error) {
...
U_CAPI int32_t U_EXPORT2 ucbuf_getc32(UCHARBUF* buf, UErrorCode * error) {
...
/* getc and escape it */
U_CAPI int32_t U_EXPORT2 ucbuf_getcx32(UCHARBUF* buf, UErrorCode * error) 
{
	int32_t length;
	int32_t offset;
	UChar32 c32, c1, c2;
...
U_CAPI UCHARBUF* U_EXPORT2 ucbuf_open(const char * fileName, const char ** cp, bool showWarning, bool buffered, UErrorCode * error) {
	FileStream* in = NULL;
	int32_t fileSize = 0;
	const char * knownCp;
	if(!error || U_FAILURE(*error)) {
 ...
 */
U_CAPI void U_EXPORT2 ucbuf_ungetc(int32_t c, UCHARBUF* buf) {
...
U_CAPI void U_EXPORT2 ucbuf_rewind(UCHARBUF* buf, UErrorCode * error) {
	if(!error || U_FAILURE(*error)) {
		return;
	}
	if(buf) {
...
U_CAPI const char * U_EXPORT2 ucbuf_resolveFileName(const char * inputDir,
    const char * fileName,
    char * target,
    int32_t* len,
    UErrorCode * status) {
