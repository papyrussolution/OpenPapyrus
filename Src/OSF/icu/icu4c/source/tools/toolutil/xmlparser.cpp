...
U_NAMESPACE_BEGIN UOBJECT_DEFINE_RTTI_IMPLEMENTATION(UXMLParser)
...
UXMLParser * UXMLParser::createParser(UErrorCode &errorCode) {
...
UXMLElement * UXMLParser::parseFile(const char * filename, UErrorCode &errorCode) {
	...
	if(U_SUCCESS(errorCode) && charset!=NULL) {
		...
		if(mXMLDecl.reset(src).lookingAt(0, errorCode)) {
			...
			while(pos<declEnd && mAttrValue.lookingAt(pos, errorCode)) { // loop runs once per attribute on
				                                                     // this element.
...
UXMLElement * UXMLParser::parse(const UnicodeString & src, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
//      for it.
//
UXMLElement * UXMLParser::createElement(RegexMatcher  &mEl, UErrorCode & status) {
...
//
void UXMLParser::parseMisc(UErrorCode & status)  {
	for(;;) {
...
//
UnicodeString UXMLParser::scanContent(UErrorCode & status) {
...
//
void UXMLParser::replaceCharRefs(UnicodeString & s, UErrorCode & status) {
	UnicodeString result;
...
void UXMLParser::error(const char * message, UErrorCode & status) {
...
const UnicodeString * UXMLParser::intern(const UnicodeString & s, UErrorCode &errorCode) {
...
const UnicodeString * UXMLParser::findName(const UnicodeString & s) const {
	const UHashElement * he = fNames.find(s);
	if(he!=NULL) {
...
const UnicodeString & UXMLElement::getTagName() const {
	return *fName;
}

UnicodeString UXMLElement::getText(bool recurse) const {
	UnicodeString text;
	appendText(text, recurse);
	return text;
}

void UXMLElement::appendText(UnicodeString & text, bool recurse) const {
	const UObject * node;
...
int32_t UXMLElement::countAttributes() const {
	return fAttNames.size();
}

const UnicodeString * UXMLElement::getAttribute(int32_t i, UnicodeString & name, UnicodeString & value) const {
...
const UnicodeString * UXMLElement::getAttribute(const UnicodeString & name) const {
...
int32_t UXMLElement::countChildren() const {
	return fChildren.size();
}

const UObject * UXMLElement::getChild(int32_t i, UXMLNodeType &type) const {
...
const UXMLElement * UXMLElement::nextChildElement(int32_t &i) const {
	if(i<0) {
		return NULL;
	}
...
const UXMLElement * UXMLElement::getChildElement(const UnicodeString & name) const {
