...
static uint16_t readSwapUInt16(uint16_t x) { return (uint16_t)((x<<8)|(x>>8)); }
...
static inline int32_t makeTypeEnum(uint8_t charset, bool isBigEndian) { return 2*(int32_t)charset+isBigEndian; }
...
static inline char makeTypeLetter(uint8_t charset, bool isBigEndian) { return types[makeTypeEnum(charset, isBigEndian)]; }
static inline char makeTypeLetter(int32_t typeEnum) { return types[typeEnum]; }

static void makeTypeProps(char type, uint8_t &charset, bool &isBigEndian) 
{
	const int32_t typeEnum = makeTypeEnum(type);
	charset = (uint8_t)(typeEnum>>1);
	isBigEndian = (bool)(typeEnum&1);
}
...
U_CFUNC const UDataInfo * getDataInfo(const uint8_t * data, int32_t length, int32_t &infoLength, int32_t &headerLength, UErrorCode * pErrorCode) 
{
	...
	pInfo = &pHeader->info;
	if((length>=0 && length<(int32_t)sizeof(DataHeader)) || pHeader->dataHeader.magic1!=0xda || pHeader->dataHeader.magic2!=0x27 || pInfo->sizeofUChar!=2) {
...
static int32_t getTypeEnumForInputData(const uint8_t * data, int32_t length, UErrorCode * pErrorCode) 
{
	...
	return makeTypeEnum(pInfo->charsetFamily, (bool)pInfo->isBigEndian);
}
 ...
 */
static void makeFullFilename(const char * path, const char * name,
...
Package::Package() : doAutoPrefix(FALSE), prefixEndsWithType(FALSE) 
{
	inPkgName[0] = 0;
	pkgPrefix[0] = 0;
	inData = NULL;
	...
	DataHeader * pHeader = (DataHeader*)header;
	...
	if(headerLength&0xf) {
		...
		memzero(header+headerLength, length-headerLength);
		headerLength = length;
	}
...
void Package::readPackage(const char * filename) {
	...
	inIsBigEndian = (bool)pInfo->isBigEndian;
	...
	if(itemCount<=0) {
		...
		memzero(items, itemCount*sizeof(Item));
		/*
...
void Package::setMatchMode(uint32_t mode) { matchMode = mode; }
void Package::addItem(const char * name) { addItem(name, NULL, 0, FALSE, U_ICUDATA_TYPE_LETTER[0]); }

void Package::addItem(const char * name, uint8_t * data, int32_t length, bool isDataOwned, char type) 
{
	int32_t idx = findItem(name);
	if(idx<0) {
...
void Package::addFile(const char * filesPath, const char * name) 
{
	int32_t length;
	char type;
	uint8_t * data = readFile(filesPath, name, length, type);
...
void Package::extractItem(const char * filesPath, const char * outName, int32_t idx, char outType) 
{
	...
	bool itemIsBigEndian, outIsBigEndian;
...
int32_t Package::getItemCount() const { return itemCount; }
...
char * Package::allocString(bool in, int32_t length) 
{
	char * p;
	int32_t top;
	if(in) {
...
void Package::setItemCapacity(int32_t max)
{
	if(max > itemMax) {
		...
		if(items && itemCount>0) {
			memcpy(newItems, items, (size_t)itemCount*sizeof(items[0]));
		}
		itemMax = max;
		items = newItems;
		uprv_free(oldItems);
