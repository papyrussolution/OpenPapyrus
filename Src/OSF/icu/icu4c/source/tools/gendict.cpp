...
public:
	...
	DataDict(bool isBytesTrie, UErrorCode & status) : bt(NULL), ut(NULL), transformConstant(0), transformType(DictionaryData::TRANSFORM_NONE) 
	{
		if(isBytesTrie) {
...
static bool readLine(UCHARBUF * f, UnicodeString & fileLine, IcuToolErrorCode &errorCode) 
{
	int32_t lineLength;
	...
	if(comment != NULL) {
		...
		while(lineLength > 0 && (line[lineLength - 1] == CARRIAGE_RETURN_CHARACTER || line[lineLength - 1] == LINEFEED_CHARACTER)) {
			--lineLength;
		}
	}
	while(lineLength > 0 && u_isspace(line[lineLength - 1])) {
		--lineLength;
	}
...
int main(int argc, char ** argv) 
{
	...
	bool hasValuelessContents = FALSE;
	int lineCount = 0;
	int wordCount = 0;
	int minlen = 255;
	int maxlen = 0;
	...
	if(status.isFailure()) {
		slfprintf_stderr("gendict: got failure of type %s while serializing, if U_ILLEGAL_ARGUMENT_ERROR possibly due to duplicate dictionary entries\n",
		    status.errorName());
		exit(status.reset());
	}
	if(verbose) {
