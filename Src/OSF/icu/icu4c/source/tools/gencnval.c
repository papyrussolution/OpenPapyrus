 ...
 *
 *   This program reads convrtrs.txt and writes a memory-mappable converter name alias table to cnvalias.dat .
 *
...
static bool standardTagsUsed = FALSE;
...
static int32_t chomp_Local(char * line);
static void addOfficialTaggedStandards(char * line, int32_t lineLen);
static uint16_t addAlias(const char * alias, uint16_t standard, uint16_t converter, bool defaultName);
		...
		if(line[pos] == '{') {
			...
			do {
				...
				if(start != limit) {
					...
					addAlias(alias, tag, cnv, (bool)(line[limit-1] == '*'));
				}
			...
			addAlias(alias, EMPTY_TAG_NUM, cnv, (bool)(tags[0].aliasList[cnv].aliasCount == 0));
		}
	}
}
...
static uint16_t addAlias(const char * alias, uint16_t standard, uint16_t converter, bool defaultName) {
	uint32_t idx, idx2;
	bool startEmptyWithoutDefault = FALSE;
	AliasList * aliasList;
		...
		for(idx = 0; idx < converterCount; idx++) {
			for(idx2 = 0; idx2 < tags[standard].aliasList[idx].aliasCount; idx2++) {
				...
				if(aliasNum && ucnv_compareNames(alias, GET_ALIAS_STR(aliasNum)) == 0) {
					if(idx == converter) {
						/*
						 * (alias, standard) duplicates are harmless if they map to the same
						 *converter.
						 * Only print a warning in verbose mode, or if the alias is a precise
						 *duplicate,
