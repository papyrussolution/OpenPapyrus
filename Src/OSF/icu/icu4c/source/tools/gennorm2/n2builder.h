...
extern bool beVerbose, haveCopyright;
...
private:
  ...
  * or the last starter combines-forward.
     */
    bool mappingHasCompBoundaryAfter(const BuilderReorderingBuffer &buffer,
                                      Norm::MappingType mappingType) const;
    /** Returns TRUE if the mapping by itself recomposes, that is, it is not comp-normalized. */
    bool mappingRecomposes(const BuilderReorderingBuffer &buffer) const;
