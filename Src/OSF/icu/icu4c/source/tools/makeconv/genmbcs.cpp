...
static bool MBCSStartMappings(MBCSData * mbcsData);
...
static bool setFallback(MBCSData * mbcsData, uint32_t offset, UChar32 c) {
...
static bool MBCSAddTable(NewConverter * cnvData, UCMTable * table, UConverterStaticData * staticData) 
{
	 ...
	 * indicators are used.
	 */
	mbcsData->utf8Friendly = utf8Friendly = (bool)((table->flagsType&UCM_FLAGS_EXPLICIT)!=0);
	if(utf8Friendly) {
...
U_CDECL_END
static bool transformEUC(MBCSData * mbcsData) {
	uint8_t * p8;
...
static uint32_t MBCSWrite(NewConverter * cnvData, const UConverterStaticData * staticData, UNewDataMemory * pData, int32_t tableType) 
{
	...
	if(mbcsData->ucm->states.maxCharLength==1) {
		...
		for(i = 0; i<stage1Top; ++i) {
			mbcsData->stage1[i] += (uint16_t)stage1Top/2; /* stage 2 contains 32-bit entries, stage 1 16-bit
			                                                 entries */
		}
