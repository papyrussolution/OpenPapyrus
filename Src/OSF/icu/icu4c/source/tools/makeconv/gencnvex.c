...
static bool CnvExtIsValid(NewConverter * cnvData, const uint8_t * bytes, int32_t length);
static bool CnvExtAddTable(NewConverter * cnvData, UCMTable * table, UConverterStaticData * staticData);
...
typedef struct CnvExtData {
	...
	/* fromUnicode */
	UToolMemory * fromUTableUChars;
	UToolMemory * fromUTableValues;
	UToolMemory * fromUBytes;
	...
	int32_t maxInBytes;
	int32_t maxOutBytes;
	int32_t maxBytesPerUChar;
	int32_t maxInUChars;
	int32_t maxOutUChars;
	int32_t maxUCharsPerByte;
} CnvExtData;

NewConverter * CnvExtOpen(UCMFile * ucm) 
{
	CnvExtData * extData = (CnvExtData*)uprv_malloc(sizeof(CnvExtData));
	if(extData==NULL) {
...
static uint32_t CnvExtWrite(NewConverter * cnvData, const UConverterStaticData * staticData,
 ...
 */
static bool generateToUTable(CnvExtData * extData, UCMTable * table,
 ...
 */
static bool makeToUTable(CnvExtData * extData, UCMTable * table) {
	int32_t toUCount;
 ...
 */
static bool generateFromUTable(CnvExtData * extData, UCMTable * table,
    int32_t start, int32_t limit, int32_t unitIndex,
    uint32_t defaultValue) {
	UCMapping * mappings, * m;
	int32_t * map;
...
static bool generateFromUTrie(CnvExtData * extData, UCMTable * table, int32_t mapLength) {
	UCMapping * mappings, * m;
	int32_t * map;
	uint32_t value;
 ...
 * This function will modify the table's reverseMap.
 */
static bool makeFromUTable(CnvExtData * extData, UCMTable * table) {
	uint16_t * stage1;
