...
static const char16_t ID_NUMBER[] = { 0x6E, 0x75, 0x6D, 0x62, 0x65, 0x72, 0 /* "number" */ };
static const char16_t ID_DATE[] = { 0x64, 0x61, 0x74, 0x65, 0 /* "date" */ };
static const char16_t ID_TIME[] = { 0x74, 0x69, 0x6D, 0x65, 0 /* "time" */ };
static const char16_t ID_SPELLOUT[]  = { 0x73, 0x70, 0x65, 0x6c, 0x6c, 0x6f, 0x75, 0x74, 0 /* "spellout" */ };
static const char16_t ID_ORDINAL[] = { 0x6f, 0x72, 0x64, 0x69, 0x6e, 0x61, 0x6c, 0 /* "ordinal" */ };
static const char16_t ID_DURATION[]  = { 0x64, 0x75, 0x72, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0 /* "duration" */ };
...
static const char16_t ID_EMPTY[] = { 0 /* empty string, used for default so that null can mark end of list */ };
static const char16_t ID_CURRENCY[]  = { 0x63, 0x75, 0x72, 0x72, 0x65, 0x6E, 0x63, 0x79, 0 /* "currency" */ };
static const char16_t ID_PERCENT[] = { 0x70, 0x65, 0x72, 0x63, 0x65, 0x6E, 0x74, 0    /* "percent" */ };
static const char16_t ID_INTEGER[] = { 0x69, 0x6E, 0x74, 0x65, 0x67, 0x65, 0x72, 0    /* "integer" */ };
...
static const char16_t ID_SHORT[] = { 0x73, 0x68, 0x6F, 0x72, 0x74, 0    /* "short" */ };
static const char16_t ID_MEDIUM[] = { 0x6D, 0x65, 0x64, 0x69, 0x75, 0x6D, 0 /* "medium" */ };
static const char16_t ID_LONG[] = { 0x6C, 0x6F, 0x6E, 0x67, 0          /* "long" */ };
static const char16_t ID_FULL[] = { 0x66, 0x75, 0x6C, 0x6C, 0          /* "full" */ };
...
static const char16_t NULL_STRING[] = { 0x6E, 0x75, 0x6C, 0x6C, 0 /*"null"*/ };
static const char16_t OTHER_STRING[] = { 0x6F, 0x74, 0x68, 0x65, 0x72, 0 /* "other" */ };

U_CDECL_BEGIN
	static bool U_CALLCONV equalFormatsForHash(const UHashTok key1, const UHashTok key2) 
	{
...
//
MessageFormat::MessageFormat(const UnicodeString & pattern, UErrorCode & success) : fLocale(Locale::getDefault()), // Uses the default locale
	msgPattern(success), formatAliases(NULL), formatAliasesCapacity(0), argTypes(NULL),
	argTypeCount(0), argTypeCapacity(0), hasArgTypeConflicts(FALSE), defaultNumberFormat(NULL), defaultDateFormat(NULL),
	cachedFormatters(NULL), customFormatArgStarts(NULL), pluralProvider(*this, UPLURAL_TYPE_CARDINAL),
...
MessageFormat::MessageFormat(const UnicodeString & pattern, const Locale & newLocale, UErrorCode & success) : 
	fLocale(newLocale), msgPattern(success), formatAliases(NULL), formatAliasesCapacity(0), argTypes(NULL),
	argTypeCount(0), argTypeCapacity(0), hasArgTypeConflicts(FALSE), defaultNumberFormat(NULL),
	defaultDateFormat(NULL), cachedFormatters(NULL), customFormatArgStarts(NULL),
	pluralProvider(*this, UPLURAL_TYPE_CARDINAL), ordinalProvider(*this, UPLURAL_TYPE_ORDINAL)
{
	setLocaleIDs(fLocale.getName(), fLocale.getName());
	applyPattern(pattern, success);
}
 ...
 */
bool MessageFormat::allocateArgTypes(int32_t capacity, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
void MessageFormat::setLocale(const Locale & theLocale)
{
	if(fLocale != theLocale) {
		ZDELETE(defaultNumberFormat);
		ZDELETE(defaultDateFormat);
		fLocale = theLocale;
		setLocaleIDs(fLocale.getName(), fLocale.getName());
...
void MessageFormat::setArgStartFormat(int32_t argStart, Format* formatter, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		...
		SETIFZQ(formatter, new DummyFormat());
...
Format* MessageFormat::getCachedFormatter(int32_t argumentNumber) const 
{
	...
	if(ptr && dynamic_cast<DummyFormat*>((Format*)ptr) == NULL)
		...
		return NULL; // Not cached, or a DummyFormat representing setFormat(NULL).
}
//
...
void MessageFormat::format(int32_t msgStart, const void * plNumber,
	...
	for(int32_t i = msgStart + 1; U_SUCCESS(success); ++i) {
		...
		if(noArg) {
			...
			// We arrive here if getCachedFormatter returned NULL, but there was actually an element in the
			// hash table.
			// This can only happen if the hash table contained a DummyFormat, so the if statement above is
			// a check
...
NumberFormat* MessageFormat::createIntegerFormat(const Locale & locale, UErrorCode & status) const 
{
	...
	DecimalFormat * temp2;
	if(temp && (temp2 = dynamic_cast<DecimalFormat*>(temp)) != NULL) {
...
bool MessageFormat::usesNamedArguments() const {
...
bool MessageFormat::equalFormats(const void * left, const void * right) {
...
UnicodeString MessageFormat::PluralSelectorProvider::select(void * ctx, double number, UErrorCode & ec) const 
{
	...
	if(context.numberArgIndex > 0 && msgFormat.cachedFormatters) {
