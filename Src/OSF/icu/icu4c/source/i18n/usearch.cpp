 ...
 */
static inline uint32_t getMask(UCollationStrength strength)
{
	switch(strength) {
		case UCOL_PRIMARY: return UCOL_PRIMARYORDERMASK;
		case UCOL_SECONDARY: return UCOL_SECONDARYORDERMASK | UCOL_PRIMARYORDERMASK;
		default: return UCOL_TERTIARYORDERMASK | UCOL_SECONDARYORDERMASK | UCOL_PRIMARYORDERMASK;
	}
}

U_CDECL_BEGIN
static bool U_CALLCONV usearch_cleanup() 
{
	g_nfcImpl = nullptr;
	return TRUE;
}
 ...
 */
static inline void initializeFCD(UErrorCode * status)
{
 ...
 * @return new destination array, destination if there was no new allocation
 */
static inline int64_t * addTouint64_tArray(int64_t    * destination, uint32_t offset, uint32_t   * destinationlength, uint64_t value,
    uint32_t increments, UErrorCode * status)
{
	uint32_t newlength = *destinationlength;
	if(offset + 1 == newlength) {
		newlength += increments;
		int64_t * temp = (int64_t*)allocateMemory(sizeof(int64_t) * newlength, status);
		if(U_FAILURE(*status)) {
			return nullptr;
		}
...
static inline void initializePatternCETable(UStringSearch * strsrch, UErrorCode * status)
{
	...
	if(coleiter == nullptr) {
		coleiter = ucol_openElements(strsrch->collator, pattern->text, patternlength, status);
	...
	while((ce = ucol_next(coleiter, status)) != UCOL_NULLORDER &&
		...
		if(newce) {
			int32_t * temp = addTouint32_tArray(cetable, offset, &cetablesize, newce, patternlength - ucol_getOffset(coleiter) + 1, status);
			if(U_FAILURE(*status)) {
				return;
			}
			offset++;
 ...
 *               method, status assumed to be success when passed in.
 */
static inline void initializePattern(UStringSearch * strsrch, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		return;
	}
	...
	if(strsrch->strength == UCOL_PRIMARY) {
		...
		pattern->hasPrefixAccents = getFCD(patterntext, &index, length) >> SECOND_LAST_BYTE_SHIFT_;
		index = length;
		U16_BACK_1(patterntext, 0, index);
		pattern->hasSuffixAccents = getFCD(patterntext, &index, length) & LAST_BYTE_MASK_;
	}
	// ** HACK **
...
static UBreakIterator* getBreakIterator(UStringSearch * strsrch, UErrorCode & status)
{
	...
	strsrch->search->internalBreakIter = ubrk_open(UBRK_CHARACTER, ucol_getLocaleByType(strsrch->collator, ULOC_VALID_LOCALE, &status),
 ...
 */
static
inline bool isOutOfBounds(int32_t textlength, int32_t offset)
{
 ...
 */
static
inline bool checkIdentical(const UStringSearch * strsrch, int32_t start, int32_t end)
{
...
{
	if(strsrch) {
		if(strsrch->pattern.ces != strsrch->pattern.cesBuffer && strsrch->pattern.ces) {
			uprv_free(strsrch->pattern.ces);
		}
		if(strsrch->pattern.pces != nullptr && strsrch->pattern.pces != strsrch->pattern.pcesBuffer) {
			uprv_free(strsrch->pattern.pces);
		}
...
namespace {
bool initTextProcessedIter(UStringSearch * strsrch, UErrorCode * status) 
{
	if(U_FAILURE(*status)) {
		return FALSE;
	}
...
CEIBuffer::CEIBuffer(UStringSearch * ss, UErrorCode * status) {
	...
	if(!initTextProcessedIter(ss, status)) {
		return;
	}
...
static bool isBreakBoundary(UStringSearch * strsrch, int32_t index, UErrorCode & status) {
	...
	gcProperty = u_getIntPropertyValue(c, UCHAR_GRAPHEME_CLUSTER_BREAK);
	bool combining =  !(gcProperty==U_GCB_CONTROL || gcProperty==U_GCB_LF || gcProperty==U_GCB_CR);
	return !combining;
#elif !UCONFIG_NO_BREAK_ITERATION
	UBreakIterator * breakiterator = getBreakIterator(strsrch, status);
	if(U_FAILURE(status)) {
...
#if 0
static bool onBreakBoundaries(const UStringSearch * strsrch, int32_t start, int32_t end, UErrorCode & status)
{
	if(U_FAILURE(status)) {
		return TRUE;
	}
...
static UCompareCEsResult compareCE64s(int64_t targCE, int64_t patCE, int16_t compareType) {
	...
	if(targLev2 != patLev2) {
		...
		return (patLev2 == U_CE_LEVEL2_BASE ||
		       (compareType == USEARCH_ANY_BASE_WEIGHT_IS_WILDCARD && targLev2 == U_CE_LEVEL2_BASE) ) ?
	...
	if(targLev3 != patLev3) {
		return (patLev3 == U_CE_LEVEL3_BASE ||
		       (compareType == USEARCH_ANY_BASE_WEIGHT_IS_WILDCARD && targLev3 == U_CE_LEVEL3_BASE) ) ?
		       U_CE_MATCH : U_CE_NO_MATCH;
	}
...
U_CAPI bool U_EXPORT2 usearch_search(UStringSearch  * strsrch,
    int32_t startIdx,
    int32_t * matchStart,
    int32_t * matchLimit,
    UErrorCode * status)
{
	...
	for(targetIx = 0;; targetIx++) {
		...
		if(strsrch->search->elementComparisonType == 0) {
			...
			for(;; ++targetIxOffset) {
				...
				if((((nextCEI->ce) >> 32) & 0xFFFF0000UL) == 0) {
					UCompareCEsResult ceMatch =
					    compareCE64s(nextCEI->ce, patCE, strsrch->search->elementComparisonType);
					if(ceMatch == U_CE_NO_MATCH || ceMatch == U_CE_SKIP_PATN) {
						found = FALSE;
						break;
					}
					// If lowIndex == highIndex, this target CE is part of an expansion of the last
					// matched
					...
					found = false;
					break;
					// Else the target CE is not part of an expansion of the last matched element,
					// match succeeds
				}
				else {
					break;
		...
		bool allowMidclusterMatch = FALSE;
	...
	ASSIGN_PTR(matchStart, mStart);
	ASSIGN_PTR(matchLimit, mLimit);
	return found;
}

U_CAPI bool U_EXPORT2 usearch_searchBackwards(UStringSearch  * strsrch, int32_t startIdx, int32_t * matchStart, int32_t * matchLimit, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		return FALSE;
	}
	...
	// Outer loop moves over match starting positions in the
	//      target CE space.
	// Here, targetIx values increase toward the beginning of the base text (i.e. we get the text CEs in reverse
	// order).
	// But  patIx is 0 at the beginning of the pattern and increases toward the end.
	// So this loop performs a comparison starting with the end of pattern, and prcessd toward the beginning of the
	// pattern
...
bool usearch_handleNextExact(UStringSearch * strsrch, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		setMatchNotFound(strsrch, *status);
		return FALSE;
...
bool usearch_handleNextCanonical(UStringSearch * strsrch, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		setMatchNotFound(strsrch, *status);
		return FALSE;
...
bool usearch_handlePreviousExact(UStringSearch * strsrch, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		setMatchNotFound(strsrch, *status);
		return FALSE;
...
bool usearch_handlePreviousCanonical(UStringSearch * strsrch,
    UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		setMatchNotFound(strsrch, *status);
