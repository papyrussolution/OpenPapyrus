...
DecimalFormatSymbols* DecimalFormatSymbols::createWithLastResortData(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
struct DecFmtSymDataSink : public ResourceSink {
	...
	bool seenSymbol[DecimalFormatSymbols::kFormatSymbolCount];
	// Constructor/Destructor
	DecFmtSymDataSink(DecimalFormatSymbols& _dfs) : dfs(_dfs) 
	{
		memzero(seenSymbol, sizeof(seenSymbol));
	}
...
struct CurrencySpacingSink : public ResourceSink {
	...
	virtual void put(const char * key, ResourceValue &value, bool /*noFallback*/,
		...
		for(int32_t i = 0; spacingTypesTable.getKeyAndValue(i, key, value); ++i) {
			...
			for(int32_t j = 0; patternsTable.getKeyAndValue(j, key, value); ++j) {
				...
				const UnicodeString & current = dfs.getPatternForCurrencySpacing(pattern, beforeCurrency, errorCode);
				if(current.isEmpty()) {
					dfs.setPatternForCurrencySpacing(pattern, beforeCurrency, value.getUnicodeString(errorCode));
				}
			}
		}
	}
	void resolveMissing() 
	{
		...
		if(!hasBeforeCurrency || !hasAfterCurrency) {
			for(int beforeCurrency = FALSE; beforeCurrency <= TRUE; beforeCurrency++) {
				for(int32_t pattern = 0; pattern < UNUM_CURRENCY_SPACING_COUNT; pattern++) {
					dfs.setPatternForCurrencySpacing((UCurrencySpacing)pattern, LOGIC(beforeCurrency), UnicodeString(defaults[pattern], -1, US_INV));
				}
			}
		}
	}
...
Locale DecimalFormatSymbols::getLocale(ULocDataLocaleType type, UErrorCode & status) const {
	U_LOCALE_BASED(locBased, *this);
	return locBased.getLocale(type, status);
}

const UnicodeString &DecimalFormatSymbols::getPatternForCurrencySpacing(UCurrencySpacing type,
    bool beforeCurrency,
...
void DecimalFormatSymbols::setPatternForCurrencySpacing(UCurrencySpacing type,
    bool beforeCurrency,
