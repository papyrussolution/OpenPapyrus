...
// MSVC VS2015 warns C4805 when comparing bool with bool, VS2017 no longer emits this warning.
...
DecimalFormat::DecimalFormat(UErrorCode & status) : DecimalFormat(nullptr, status) 
{
	if(U_FAILURE(status)) {
		return;
	}
	...
	UnicodeString patternString(utils::getPatternForStyle(localeName, ns->getName(), CLDR_PATTERN_STYLE_DECIMAL, status));
	setPropertiesFromPattern(patternString, IGNORE_ROUNDING_IF_CURRENCY, status);
	touch(status);
}

DecimalFormat::DecimalFormat(const UnicodeString & pattern, UErrorCode & status) : DecimalFormat(nullptr, status) 
{
	if(U_FAILURE(status)) {
		return;
	}
...
DecimalFormat::DecimalFormat(const UnicodeString & pattern, DecimalFormatSymbols* symbolsToAdopt, UErrorCode & status) : DecimalFormat(symbolsToAdopt, status) 
{
	if(U_FAILURE(status)) {
		return;
	}
...
DecimalFormat::DecimalFormat(const UnicodeString & pattern, DecimalFormatSymbols* symbolsToAdopt, UNumberFormatStyle style, UErrorCode & status) : DecimalFormat(symbolsToAdopt, status) 
{
	if(U_FAILURE(status)) {
		return;
	}
...
DecimalFormat&DecimalFormat::setAttribute(UNumberFormatAttribute attr, int32_t newValue, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return *this;
	}
	if(fields == nullptr) {
		// We only get here if an OOM error happened during construction, copy construction, assignment, or
		// modification.
		status = U_MEMORY_ALLOCATION_ERROR;
		return *this;
	}
	switch(attr) {
		...
		case UNUM_PARSE_NO_EXPONENT:
		    setParseNoExponent((bool)newValue);
		    break;
		case UNUM_PARSE_DECIMAL_MARK_REQUIRED:
		    setDecimalPatternMatchRequired((bool)newValue);
		    break;
		...
		case UNUM_PARSE_CASE_SENSITIVE:
		    setParseCaseSensitive(static_cast<bool>(newValue));
		    break;
		case UNUM_SIGN_ALWAYS_SHOWN:
		    setSignAlwaysShown(static_cast<bool>(newValue));
		    break;
		case UNUM_FORMAT_FAIL_IF_MORE_THAN_MAX_DIGITS:
		    setFormatFailIfMoreThanMaxDigits(static_cast<bool>(newValue));
		    break;
		default:
...
void DecimalFormat::setGroupingUsed(bool enabled) 
{
	...
	if(UBOOL_TO_BOOL(enabled) == fields->properties.groupingUsed) {
		return;
	}
...
void DecimalFormat::setParseIntegerOnly(bool value) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(value) == fields->properties.parseIntegerOnly) {
		return;
	}
...
void DecimalFormat::setLenient(bool enable) {
	...
	if(!fields->properties.parseMode.isNull() && mode == fields->properties.parseMode.getNoError()) {
		return;
	}
...
UnicodeString &DecimalFormat::format(double number, UnicodeString & appendTo, FieldPositionIterator* posIter,
...
UnicodeString &DecimalFormat::format(int32_t number, UnicodeString & appendTo, FieldPositionIterator* posIter,
    UErrorCode & status) const {
...
UnicodeString &DecimalFormat::format(int64_t number, UnicodeString & appendTo, FieldPositionIterator* posIter,
    UErrorCode & status) const {
	if(U_FAILURE(status)) {
		return appendTo; // don't overwrite status if it's already a failure.
	}
...
UnicodeString &DecimalFormat::format(StringPiece number, UnicodeString & appendTo, FieldPositionIterator* posIter,
    UErrorCode & status) const {
	if(U_FAILURE(status)) {
		return appendTo; // don't overwrite status if it's already a failure.
	}
...
void DecimalFormat::setPositivePrefix(const UnicodeString & newValue) {
	...
	if(newValue == fields->properties.positivePrefix) {
		return;
	}
...
void DecimalFormat::setNegativePrefix(const UnicodeString & newValue) {
	...
	if(newValue == fields->properties.negativePrefix) {
		return;
	}
...
void DecimalFormat::setPositiveSuffix(const UnicodeString & newValue) {
	...
	if(newValue == fields->properties.positiveSuffix) {
		return;
	}
...
void DecimalFormat::setNegativeSuffix(const UnicodeString & newValue) {
	...
	if(newValue == fields->properties.negativeSuffix) {
		return;
	}
...
bool DecimalFormat::isSignAlwaysShown() const {
...
void DecimalFormat::setSignAlwaysShown(bool value) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(value) == fields->properties.signAlwaysShown) {
		return;
	}
...
void DecimalFormat::setRoundingIncrement(double newValue) {
	...
	if(newValue == fields->properties.roundingIncrement) {
		return;
	}
...
void DecimalFormat::setFormatWidth(int32_t width) {
	...
	if(width == fields->properties.formatWidth) {
		return;
	}
...
void DecimalFormat::setPadCharacter(const UnicodeString & padChar) {
	...
	if(padChar == fields->properties.padString) {
		return;
	}
	if(padChar.length() > 0) {
...
bool DecimalFormat::isScientificNotation() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setScientificNotation(bool useScientific) {
	if(fields == nullptr) {
		return;
	}
	int32_t minExp = useScientific ? 1 : -1;
	if(fields->properties.minimumExponentDigits == minExp) {
		return;
	}
	if(useScientific) {
...
void DecimalFormat::setMinimumExponentDigits(int8 minExpDig) {
	...
	if(minExpDig == fields->properties.minimumExponentDigits) {
		return;
	}
...
bool DecimalFormat::isExponentSignAlwaysShown() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setExponentSignAlwaysShown(bool expSignAlways) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(expSignAlways) == fields->properties.exponentSignAlwaysShown) {
		return;
	}
...
void DecimalFormat::setGroupingSize(int32_t newValue) {
	...
	if(newValue == fields->properties.groupingSize) {
		return;
	}
...
void DecimalFormat::setSecondaryGroupingSize(int32_t newValue) {
	...
	if(newValue == fields->properties.secondaryGroupingSize) {
		return;
	}
...
void DecimalFormat::setMinimumGroupingDigits(int32_t newValue) {
	...
	if(newValue == fields->properties.minimumGroupingDigits) {
		return;
	}
...
bool DecimalFormat::isDecimalSeparatorAlwaysShown() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setDecimalSeparatorAlwaysShown(bool newValue) {
	...
	if(UBOOL_TO_BOOL(newValue) == fields->properties.decimalSeparatorAlwaysShown) {
		return;
	}
...
bool DecimalFormat::isDecimalPatternMatchRequired() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setDecimalPatternMatchRequired(bool newValue) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(newValue) == fields->properties.decimalPatternMatchRequired) {
		return;
	}
...
bool DecimalFormat::isParseNoExponent() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setParseNoExponent(bool value) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(value) == fields->properties.parseNoExponent) {
		return;
	}
...
bool DecimalFormat::isParseCaseSensitive() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setParseCaseSensitive(bool value) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(value) == fields->properties.parseCaseSensitive) {
		return;
	}
...
bool DecimalFormat::isFormatFailIfMoreThanMaxDigits() const {
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setFormatFailIfMoreThanMaxDigits(bool value) {
	if(fields == nullptr) {
		return;
	}
	if(UBOOL_TO_BOOL(value) == fields->properties.formatFailIfMoreThanMaxDigits) {
		return;
	}
...
void DecimalFormat::setMaximumIntegerDigits(int32_t newValue) {
	...
	if(newValue == fields->properties.maximumIntegerDigits) {
		return;
	}
...
void DecimalFormat::setMinimumIntegerDigits(int32_t newValue) {
	...
	if(newValue == fields->properties.minimumIntegerDigits) {
		return;
	}
	// For backwards compatibility, conflicting min/max need to keep the most recent setting.
...
void DecimalFormat::setMaximumFractionDigits(int32_t newValue) {
	...
	if(newValue == fields->properties.maximumFractionDigits) {
		return;
	}
...
void DecimalFormat::setMinimumFractionDigits(int32_t newValue) {
	...
	if(newValue == fields->properties.minimumFractionDigits) {
		return;
	}
	// For backwards compatibility, conflicting min/max need to keep the most recent setting.
...
void DecimalFormat::setMinimumSignificantDigits(int32_t value) {
	...
	if(value == fields->properties.minimumSignificantDigits) {
		return;
	}
...
void DecimalFormat::setMaximumSignificantDigits(int32_t value) {
	...
	if(value == fields->properties.maximumSignificantDigits) {
		return;
	}
...
bool DecimalFormat::areSignificantDigitsUsed() const {
	const DecimalFormatProperties* dfp;
	// Not much we can do to report an error.
	if(fields == nullptr) {
		// Fallback to using the default instance of DecimalFormatProperties.
...
void DecimalFormat::setSignificantDigitsUsed(bool useSignificantDigits) {
	if(fields == nullptr) {
		return;
	}
...
void DecimalFormat::formatToDecimalQuantity(double number, DecimalQuantity& output, UErrorCode & status) const {
	// don't overwrite status if it's already a failure.
	if(U_FAILURE(status)) {
		return;
	}
...
void DecimalFormat::touch(UErrorCode & status) {
	...
	if(fields == nullptr) {
		...
		// For copy construction, there is unfortunately nothing to report the error, so we need to guard
		// against
	...
	// In C++, fields->symbols (or, if it's null, the DecimalFormatSymbols owned by the underlying
	...
	fields->symbols.adoptInstead(nullptr); // the fields->symbols property is only temporary, until we can copy it
	                                       // into a new LocalizedNumberFormatter
...
void DecimalFormat::setupFastFormat() 
{
	...
	// Nontrivial affixes:
	bool trivialPP = fields->properties.positivePrefixPattern.isEmpty();
	bool trivialPS = fields->properties.positiveSuffixPattern.isEmpty();
	bool trivialNP = fields->properties.negativePrefixPattern.isBogus() || (
		fields->properties.negativePrefixPattern.length() == 1 &&
		fields->properties.negativePrefixPattern.charAt(0) == u'-');
	bool trivialNS = fields->properties.negativeSuffixPattern.isEmpty();
...
bool DecimalFormat::fastFormatDouble(double input, UnicodeString & output) const 
{
	...
	if(std::isnan(input) || uprv_trunc(input) != input || input <= INT32_MIN || input > INT32_MAX) {
		return false;
	}
