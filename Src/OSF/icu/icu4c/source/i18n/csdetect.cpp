...
U_CDECL_BEGIN
static bool U_CALLCONV csdet_cleanup(void)
{
	U_NAMESPACE_USE
...
bool CharsetDetector::setStripTagsFlag(bool flag)
{
	bool temp = fStripTags;
	fStripTags = flag;
	fFreshTextSet = TRUE;
	return temp;
}

bool CharsetDetector::getStripTagsFlag() const
{
	return fStripTags;
}
...
void CharsetDetector::setDetectableCharset(const char * encoding, bool enabled, UErrorCode & status)
{
	if(U_FAILURE(status)) {
		return;
	}
...
UEnumeration * CharsetDetector::getAllDetectableCharsets(UErrorCode & status)
{
	...
	memzero(en->context, sizeof(Context));
