    ...
    UErrorCode * status)
{
	...
	if(timeStyle != UDAT_PATTERN) {
		...
		UnicodeString pat((bool)(patternLength == -1), pattern, patternLength);
	...
	if(tzID != 0) {
		TimeZone * zone = TimeZone::createTimeZone(UnicodeString((bool)(tzIDLength == -1), tzID, tzIDLength));
		if(zone == 0) {
			*status = U_MEMORY_ALLOCATION_ERROR;
			delete fmt;
			return 0;
...
U_CAPI bool U_EXPORT2 udat_isLenient(const UDateFormat* fmt)
{
	return ((DateFormat*)fmt)->isLenient();
}

U_CAPI void U_EXPORT2 udat_setLenient(UDateFormat*    fmt,
    bool isLenient)
{
	((DateFormat*)fmt)->setLenient(isLenient);
}

U_CAPI bool U_EXPORT2 udat_getBooleanAttribute(const UDateFormat* fmt,
...
U_CAPI void U_EXPORT2 udat_setBooleanAttribute(UDateFormat * fmt,
    UDateFormatBooleanAttribute attr,
    bool newValue,
    UErrorCode * status)
{
...
U_CAPI void U_EXPORT2 udat_adoptNumberFormatForFields(UDateFormat*    fmt,
    const char16_t * fields, UNumberFormat * numberFormatToSet, UErrorCode *  status)
{
	verifyIsSimpleDateFormat(fmt, status);
	if(U_FAILURE(*status)) 
		return;
...
U_CAPI int32_t U_EXPORT2 udat_countAvailable()
{
...
// A: Yes. Of course.
U_CAPI void U_EXPORT2 udat_applyPattern(UDateFormat     * format, bool localized, const char16_t * pattern, int32_t patternLength)
{
...
U_CAPI int32_t U_EXPORT2 udat_getSymbols(const UDateFormat     * fmt, UDateFormatSymbolType type, int32_t index, char16_t * result, int32_t resultLength, UErrorCode * status)
{
...
U_CAPI void U_EXPORT2 udat_setSymbols(UDateFormat * format, UDateFormatSymbolType type, int32_t index, char16_t * value, int32_t valueLength, UErrorCode * status)
{
	verifyIsSimpleDateFormat(format, status);
	if(U_FAILURE(*status)) return;
...
U_CAPI void U_EXPORT2 udat_applyPatternRelative(UDateFormat * format, const char16_t * datePattern, int32_t datePatternLength,
    const char16_t * timePattern, int32_t timePatternLength, UErrorCode * status)
{
	verifyIsRelativeDateFormat(format, status);
	if(U_FAILURE(*status)) return;
	const UnicodeString datePat((bool)(datePatternLength == -1), datePattern, datePatternLength);
	const UnicodeString timePat((bool)(timePatternLength == -1), timePattern, timePatternLength);
