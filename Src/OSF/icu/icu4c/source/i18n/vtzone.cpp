...
U_CDECL_BEGIN
static void U_CALLCONV deleteTimeZoneRule(void * obj) { delete (TimeZoneRule*)obj; }
U_CDECL_END
 ...
 */
static void getDefaultTZName(const UnicodeString & tzid, bool isDST, UnicodeString & zonename) {
	zonename = tzid;
	if(isDST) {
...
static void parseRRULE(const UnicodeString & rrule, int32_t& month, int32_t& dow, int32_t& wim,
	...
	while(nextProp) {
		...
		if(attr.compare(ICAL_FREQ, -1) == 0) {
			...
			// 3 or 4-letter format is used for represeinging Nth day of week, for example, "-1SA" for last
			// Saturday
...
static TimeZoneRule* createRuleByRRULE(const UnicodeString & zonename, int rawOffset, int dstSavings, UDate start,
	...
	int32_t days[7];
	int32_t daysCount = SIZEOFARRAYi(days);
	UDate until;
	...
	if(dates->size() == 1) {
		...
		for(i = 1; i < dates->size(); i++) {
			...
			int32_t tmp_days[7];
			int32_t tmp_daysCount = SIZEOFARRAYi(tmp_days);
 ...
 * to the DateTimerule.
 */
static bool isEquivalentDateRule(int32_t month, int32_t weekInMonth, int32_t dayOfWeek, const DateTimeRule * dtrule) {
...
void VTZWriter::write(char16_t ch) {
	out->append(ch);
}

void VTZWriter::write(const char16_t * str) {
	out->append(str, -1);
}
...
void VTimeZone::getOffset(UDate date, bool local, int32_t& rawOffset,
...
int32_t VTimeZone::getRawOffset() const {
	return tz->getRawOffset();
}

bool VTimeZone::useDaylightTime() const {
	return tz->useDaylightTime();
}

bool VTimeZone::inDaylightTime(UDate date, UErrorCode & status) const {
	return tz->inDaylightTime(date, status);
}

bool VTimeZone::hasSameRules(const TimeZone& other) const {
	return tz->hasSameRules(other);
}

bool VTimeZone::getNextTransition(UDate base, bool inclusive, TimeZoneTransition& result) const {
	return tz->getNextTransition(base, inclusive, result);
}

bool VTimeZone::getPreviousTransition(UDate base, bool inclusive, TimeZoneTransition& result) const {
...
void VTimeZone::getTimeZoneRules(const InitialTimeZoneRule*& initial,
		...
		switch(state) {
			...
			case VTZ:
			    if(name.compare(ICAL_TZID, -1) == 0) {
				    ...
				    bool isDST = (value.compare(ICAL_DAYLIGHT, -1) == 0);
			...
			case TZI:
			    if(name.compare(ICAL_DTSTART, -1) == 0) {
				    ...
				    if(U_FAILURE(status) || rule == nullptr) {
					    ...
					    bool startAvail = rule->getFirstStart(fromOffset, 0, actualStart);
	...
	if(finalRuleCount == 1) {
		if(rules->size() == 1) {
			...
			if(start == finalStart) {
				...
				newRule = new TimeArrayTimeZoneRule(finalRule->getName(tznam), finalRule->getRawOffset(), finalRule->getDSTSavings(),
					&finalStart, 1, DateTimeRule::UTC_TIME);
			}
			else {
				// Update the end year
				int32_t y, m, d, dow, doy, mid;
				Grego::timeToFields(start, y, m, d, dow, doy, mid);
				newRule = new AnnualTimeZoneRule(finalRule->getName(tznam), finalRule->getRawOffset(), finalRule->getDSTSavings(),
...
void VTimeZone::write(VTZWriter& writer, UErrorCode & status) const 
{
	if(vtzlines != nullptr) {
		for(int32_t i = 0; i < vtzlines->size(); i++) {
			...
			if(line->startsWith(ICAL_TZURL, -1) && line->charAt(sstrleni(ICAL_TZURL)) == COLON) {
				writer.write(ICAL_TZURL);
				writer.write(COLON);
				writer.write(tzurl);
				writer.write(ICAL_NEWLINE);
			}
			else if(line->startsWith(ICAL_LASTMOD, -1) && line->charAt(sstrleni(ICAL_LASTMOD)) == COLON) {
...
void VTimeZone::writeZone(VTZWriter& w, BasicTimeZone& basictz,
			...
			if(finalDstRule == nullptr) {
				...
				if(dstCount == 1) {
					...
					if(isEquivalentDateRule(dstMonth, dstWeekInMonth, dstDayOfWeek, finalDstRule->getRule())) {
						...
						UDate nextStart;
						bool nextStartAvail = finalDstRule->getNextStart(dstUntilTime,
			...
			if(finalStdRule == nullptr) {
				...
				if(stdCount == 1) {
					...
					if(isEquivalentDateRule(stdMonth, stdWeekInMonth, stdDayOfWeek, finalStdRule->getRule())) {
						...
						UDate nextStart;
						bool nextStartAvail = finalStdRule->getNextStart(stdUntilTime,
 ...
 */
void VTimeZone::writeZonePropsByTime(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
 ...
 */
void VTimeZone::writeZonePropsByDOM(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
 ...
 */
void VTimeZone::writeZonePropsByDOW(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
    int32_t fromOffset, int32_t toOffset,
 ...
 */
void VTimeZone::writeZonePropsByDOW_GEQ_DOM(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
    int32_t fromOffset, int32_t toOffset,
 ...
 */
void VTimeZone::writeZonePropsByDOW_GEQ_DOM_sub(VTZWriter& writer, int32_t month, int32_t dayOfMonth,
	...
	bool isFeb = (month == UCAL_FEBRUARY);
 ...
 */
void VTimeZone::writeZonePropsByDOW_LEQ_DOM(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
    int32_t fromOffset, int32_t toOffset,
    int32_t month, int32_t dayOfMonth, int32_t dayOfWeek,
    UDate startTime, UDate untilTime, UErrorCode & status) const {
	if(U_FAILURE(status)) {
 ...
 */
void VTimeZone::writeFinalRule(VTZWriter& writer, bool isDst, const AnnualTimeZoneRule* rule,
    int32_t fromRawOffset, int32_t fromDSTSavings,
    UDate startTime, UErrorCode & status) const {
	if(U_FAILURE(status)) {
		return;
	}
	bool modifiedRule = TRUE;
 ...
 */
void VTimeZone::beginZoneProps(VTZWriter& writer, bool isDst, const UnicodeString & zonename,
 ...
 */
void VTimeZone::endZoneProps(VTZWriter& writer, bool isDst, UErrorCode & status) const {
	if(U_FAILURE(status)) {
		return;
	}
