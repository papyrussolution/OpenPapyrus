...
static bool U_CALLCONV ucol_res_cleanup() {
	rootRules = NULL;
	rootRulesLength = 0;
...
void U_CALLCONV CollationLoader::loadRootRules(UErrorCode & errorCode) {
...
void CollationLoader::appendRootRules(UnicodeString & s) {
...
void CollationLoader::loadRules(const char * localeID, const char * collationType,
...
template <> U_I18N_API
const CollationCacheEntry * LocaleCacheKey<CollationCacheEntry>::createObject(const void * creationContext,
...
const CollationCacheEntry * CollationLoader::createCacheEntry(UErrorCode & errorCode) {
...
const CollationCacheEntry * CollationLoader::loadFromCollations(UErrorCode & errorCode) {
	...
	bool actualAndValidLocalesAreDifferent = Locale(actualLocale) != Locale(vLocale);
...
const CollationCacheEntry * CollationLoader::loadFromData(UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return NULL;
	}
...
const CollationCacheEntry * CollationLoader::getCacheEntry(UErrorCode & errorCode) {
...
const CollationCacheEntry * CollationLoader::makeCacheEntryFromRoot(const Locale & /*loc*/,
    UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return NULL;
	}
...
U_CAPI UEnumeration* U_EXPORT2 ucol_openAvailableLocales(UErrorCode * status) {
...
U_CAPI UEnumeration* U_EXPORT2 ucol_getKeywordValues(const char * keyword, UErrorCode * status) {
	if(U_FAILURE(*status)) {
		return NULL;
	}
...
U_CAPI UEnumeration* U_EXPORT2 ucol_getKeywordValuesForLocale(const char * /*key*/, const char * locale,
...
U_CAPI int32_t U_EXPORT2 ucol_getFunctionalEquivalent(char * result, int32_t resultCapacity,
