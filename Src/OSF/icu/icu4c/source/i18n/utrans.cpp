...
ReplaceableGlue::ReplaceableGlue(UReplaceable * replaceable, const UReplaceableCallbacks * funcCallback) : Replaceable()
{
...
int32_t ReplaceableGlue::getLength() const { return (*func->length)(rep); }
char16_t ReplaceableGlue::getCharAt(int32_t offset) const { return (*func->charAt)(rep, offset); }
UChar32 ReplaceableGlue::getChar32At(int32_t offset) const { return (*func->char32At)(rep, offset); }
...
void ReplaceableGlue::copy(int32_t start, int32_t limit, int32_t dest) { (*func->copy)(rep, start, limit, dest); }
...
U_CAPI UTransliterator* U_EXPORT2 utrans_openU(const char16_t * id, int32_t idLength, UTransDirection dir, const char16_t * rules,
    int32_t rulesLength, UParseError * parseError, UErrorCode * status) 
{
	...
	UParseError temp;
	SETIFZQ(parseError, &temp);
	UnicodeString ID(idLength<0, id, idLength); // r-o alias
	if(rules==NULL) {
		Transliterator * trans = Transliterator::createInstance(ID, dir, *parseError, *status);
		if(U_FAILURE(*status)) {
			return NULL;
		}
		...
		UnicodeString ruleStr(rulesLength < 0, rules, rulesLength); // r-o alias
		Transliterator * trans = Transliterator::createFromRules(ID, ruleStr, dir, *parseError, *status);
		if(U_FAILURE(*status)) {
			return NULL;
		}
		return (UTransliterator*)trans;
...
U_CAPI UTransliterator* U_EXPORT2 utrans_open(const char * id, UTransDirection dir, const char16_t * rules/* may be Null */,
    int32_t rulesLength/* -1 if null-terminated */, UParseError* parseError/* may be Null */, UErrorCode * status) 
{
...
U_CAPI const char16_t * U_EXPORT2 utrans_getUnicodeID(const UTransliterator * trans, int32_t * resultLength) 
{
	// Transliterator keeps its ID NUL-terminated
	const UnicodeString & ID = ((Transliterator*)trans)->getID();
	ASSIGN_PTR(resultLength, ID.length());
	return ID.getBuffer();
}
...
U_CAPI void U_EXPORT2 utrans_setFilter(UTransliterator* trans, const char16_t * filterPattern, int32_t filterPatternLen, UErrorCode * status) 
{
	utrans_ENTRY(status);
...
U_CAPI int32_t U_EXPORT2 utrans_countAvailableIDs() { return Transliterator::countAvailableIDs(); }

U_CAPI int32_t U_EXPORT2 utrans_getAvailableID(int32_t index, char * buf/*may be NULL*/, int32_t bufCapacity) 
{
...
{
	return (pErrorCode && U_SUCCESS(*pErrorCode)) ? ((UTransEnumeration*)uenum)->count : 0;
}

static const char16_t * U_CALLCONV utrans_enum_unext(UEnumeration * uenum, int32_t* resultLength, UErrorCode * pErrorCode) 
{
...
static void U_CALLCONV utrans_enum_close(UEnumeration * uenum) { uprv_free(uenum); }
...
// Transliteration API
//
U_CAPI void U_EXPORT2 utrans_trans(const UTransliterator* trans, UReplaceable* rep, const UReplaceableCallbacks* repFunc,
...
U_CAPI void U_EXPORT2 utrans_transIncremental(const UTransliterator* trans, UReplaceable* rep, const UReplaceableCallbacks* repFunc,
...
U_CAPI void U_EXPORT2 utrans_transUChars(const UTransliterator* trans, char16_t * text, int32_t* textLength, int32_t textCapacity,
    int32_t start, int32_t* limit, UErrorCode * status) 
{
	utrans_ENTRY(status);
	if(trans == 0 || text == 0 || limit == 0) {
		*status = U_ILLEGAL_ARGUMENT_ERROR;
		...
		int32_t textLen = (textLength == NULL || *textLength < 0) ? sstrleni(text) : *textLength;
		...
		*limit = ((Transliterator*)trans)->transliterate(str, start, *limit);
		...
		ASSIGN_PTR(textLength, textLen);
	}
}

U_CAPI void U_EXPORT2 utrans_transIncrementalUChars(const UTransliterator* trans, char16_t * text, int32_t* textLength,
...
U_CAPI int32_t U_EXPORT2 utrans_toRules(const UTransliterator* trans, bool escapeUnprintable, char16_t * result, int32_t resultLength, UErrorCode * status) 
{
...
U_CAPI USet* U_EXPORT2 utrans_getSourceSet(const UTransliterator* trans, bool ignoreFilter, USet* fillIn, UErrorCode * status) 
{
	utrans_ENTRY(status) fillIn;
	SETIFZQ(fillIn, uset_openEmpty());
	if(ignoreFilter) {
