...
//
void TextTrieMap::put(const UnicodeString & key, void * value, ZNStringPool &sp, UErrorCode & status) {
...
// resource bundle.
void TextTrieMap::put(const char16_t * key, void * value, UErrorCode & status) {
	fIsEmpty = FALSE;
...
void TextTrieMap::putImpl(const UnicodeString & key, void * value, UErrorCode & status) 
{
	if(fNodes == NULL) {
		fNodesCapacity = 512;
...
CharacterNode* TextTrieMap::addChildNode(CharacterNode * parent, char16_t c, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
CharacterNode* TextTrieMap::getChildNode(CharacterNode * parent, char16_t c) const {
	// Linear search of the sorted list of children.
	uint16 nodeIndex = parent->fFirstChild;
	while(nodeIndex > 0) {
		CharacterNode * current = fNodes + nodeIndex;
...
void TextTrieMap::search(const UnicodeString & text, int32_t start,
...
void TextTrieMap::search(CharacterNode * node, const UnicodeString & text, int32_t start,
...
private:
	static void * createMetaZoneAndPutInCache(UHashtable * cache, const char16_t * names[], const UnicodeString & mzID, UErrorCode & status) 
	{
		if(U_FAILURE(status)) {
			return NULL;
		}
...
struct ZNames::ZNamesLoader : public ResourceSink {
	...
	static UTimeZoneNameTypeIndex nameTypeFromKey(const char * key) 
	{
		...
		if(c0 == 'l') {
			return c1 == 'g' ? UTZNM_INDEX_LONG_GENERIC : c1 == 's' ? UTZNM_INDEX_LONG_STANDARD : c1 == 'd' ? UTZNM_INDEX_LONG_DAYLIGHT : UTZNM_INDEX_UNKNOWN;
		}
		else if(c0 == 's') {
			return c1 == 'g' ? UTZNM_INDEX_SHORT_GENERIC : c1 == 's' ? UTZNM_INDEX_SHORT_STANDARD : c1 == 'd' ? UTZNM_INDEX_SHORT_DAYLIGHT : UTZNM_INDEX_UNKNOWN;
		}
		else if(c0 == 'e' && c1 == 'c') {
...
const UnicodeString * MetaZoneIDsEnumeration::snext(UErrorCode & status) 
{
	if(U_SUCCESS(status) && fMetaZoneIDs && fPos < fLen) {
...
void MetaZoneIDsEnumeration::reset(UErrorCode & /*status*/) {
	fPos = 0;
}

int32_t MetaZoneIDsEnumeration::count(UErrorCode & /*status*/) const {
	return fLen;
}
...
bool ZNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode * node, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
TimeZoneNames::MatchInfoCollection* ZNameSearchHandler::getMatches(int32_t& maxMatchLen) {
 ...
 * Deleter for ZNames
 */
static void U_CALLCONV deleteZNames(void * obj) {
	if(obj != EMPTY) {
		delete (ZNames*)obj;
	}
}
 ...
 */
static void U_CALLCONV deleteZNameInfo(void * obj) {
	uprv_free(obj);
}
...
void TimeZoneNamesImpl::initialize(const Locale & locale, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
 ...
 * except initializer.
 */
void TimeZoneNamesImpl::loadStrings(const UnicodeString & tzCanonicalID, UErrorCode & status) 
{
...
void TimeZoneNamesImpl::cleanup() 
{
	if(fZoneStrings) {
...
bool TimeZoneNamesImpl::operator == (const TimeZoneNames &other) const 
{
	if(this == &other) {
		return true;
	}
...
TimeZoneNamesImpl* TimeZoneNamesImpl::clone() const 
{
	UErrorCode status = U_ZERO_ERROR;
	return new TimeZoneNamesImpl(fLocale, status);
}

StringEnumeration * TimeZoneNamesImpl::getAvailableMetaZoneIDs(UErrorCode & status) const {
	return TimeZoneNamesImpl::_getAvailableMetaZoneIDs(status);
}

// static implementation of getAvailableMetaZoneIDs(UErrorCode&)
StringEnumeration * TimeZoneNamesImpl::_getAvailableMetaZoneIDs(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
StringEnumeration * TimeZoneNamesImpl::getAvailableMetaZoneIDs(const UnicodeString & tzID, UErrorCode & status) const {
...
StringEnumeration * TimeZoneNamesImpl::_getAvailableMetaZoneIDs(const UnicodeString & tzID, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return NULL;
	}
...
UnicodeString &TimeZoneNamesImpl::getMetaZoneID(const UnicodeString & tzID, UDate date, UnicodeString & mzID) const {
	return TimeZoneNamesImpl::_getMetaZoneID(tzID, date, mzID);
}

// static implementation of getMetaZoneID
UnicodeString &TimeZoneNamesImpl::_getMetaZoneID(const UnicodeString & tzID, UDate date, UnicodeString & mzID) {
	ZoneMeta::getMetazoneID(tzID, date, mzID);
	return mzID;
}

UnicodeString &TimeZoneNamesImpl::getReferenceZoneID(const UnicodeString & mzID, const char * region, UnicodeString & tzID) const {
	return TimeZoneNamesImpl::_getReferenceZoneID(mzID, region, tzID);
}

// static implementation of getReferenceZoneID
UnicodeString &TimeZoneNamesImpl::_getReferenceZoneID(const UnicodeString & mzID, const char * region, UnicodeString & tzID) {
	ZoneMeta::getZoneIdByMetazone(mzID, UnicodeString(region, -1, US_INV), tzID);
	return tzID;
}

UnicodeString &TimeZoneNamesImpl::getMetaZoneDisplayName(const UnicodeString & mzID,
    UTimeZoneNameType type,
...
UnicodeString &TimeZoneNamesImpl::getExemplarLocationName(const UnicodeString & tzID, UnicodeString & name) const 
{
	name.setToBogus(); // cleanup result.
 ...
 * This method updates the cache and must be called with a lock
 */
ZNames* TimeZoneNamesImpl::loadMetaZoneNames(const UnicodeString & mzID, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
 ...
 * This method updates the cache and must be called with a lock
 */
ZNames* TimeZoneNamesImpl::loadTimeZoneNames(const UnicodeString & tzID, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
TimeZoneNames::MatchInfoCollection* TimeZoneNamesImpl::find(const UnicodeString & text, int32_t start, uint32_t types,
...
TimeZoneNames::MatchInfoCollection* TimeZoneNamesImpl::doFind(ZNameSearchHandler& handler, const UnicodeString & text, int32_t start, UErrorCode & status) const 
{
	...
	if(matches && ((maxLen == (text.length() - start)) || fNamesTrieFullyLoaded)) {
...
void TimeZoneNamesImpl::addAllNamesIntoTrie(UErrorCode & status) 
{
	...
	while((element = uhash_nextElement(fMZNamesMap, &pos)) != NULL) {
		if(element->value.pointer == EMPTY) {
			continue;
		}
...
U_CDECL_BEGIN
static void U_CALLCONV deleteZNamesLoader(void * obj) {
	if(obj == DUMMY_LOADER) {
		return;
	}
...
struct TimeZoneNamesImpl::ZoneStringsLoader : public ResourceSink {
	...
	void load(UErrorCode & status) 
	{
		...
		while((element = uhash_nextElement(keyToLoader, &pos)) != NULL) {
			if(element->value.pointer == DUMMY_LOADER) {
				continue;
			}
	...
	void consumeNamesTable(const char * key, ResourceValue &value, bool noFallback,
	    UErrorCode & status) {
		if(U_FAILURE(status)) {
			return;
		}
...
TZDBNames::TZDBNames(const char16_t ** names, char ** regions, int32_t numRegions) : fNames(names), fRegions(regions), fNumRegions(numRegions) 
{
}
...
TZDBNames* TZDBNames::createInstance(UResourceBundle * rb, const char * key) 
{
	if(rb == NULL || key == NULL || *key == 0) {
		return NULL;
	}
	UErrorCode status = U_ZERO_ERROR;
	const char16_t ** names = NULL;
	char ** regions = NULL;
...
const char16_t * TZDBNames::getName(UTimeZoneNameType type) const 
{
	if(fNames == NULL) {
		return NULL;
	}
...
const char ** TZDBNames::getParseRegions(int32_t& numRegions) const {
	if(fRegions == NULL) {
		numRegions = 0;
	}
	else {
...
TZDBNameSearchHandler::TZDBNameSearchHandler(uint32_t types, const char * region) : fTypes(types), fMaxMatchLen(0), fResults(NULL), fRegion(region) 
{
}
...
bool TZDBNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode * node, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
TimeZoneNames::MatchInfoCollection* TZDBNameSearchHandler::getMatches(int32_t& maxMatchLen) {
	// give the ownership to the caller
	TimeZoneNames::MatchInfoCollection* results = fResults;
	maxMatchLen = fMaxMatchLen;
 ...
 */
static void U_CALLCONV deleteTZDBNames(void * obj) {
	if(obj != EMPTY) {
 ...
 */
static void U_CALLCONV deleteTZDBNameInfo(void * obj) 
{
	uprv_free(obj);
}
...
static void U_CALLCONV prepareFind(UErrorCode & status) 
{
	...
	if(U_SUCCESS(status)) {
		while((mzID = mzIDs->snext(status)) != 0 && U_SUCCESS(status)) {
			...
			bool ambiguousType = (std && dst && u_strcmp(std, dst) == 0);
...
bool TZDBTimeZoneNames::operator == (const TimeZoneNames& other) const {
	if(this == &other) {
		return true;
	}
	// No implementation for now
...
TZDBTimeZoneNames* TZDBTimeZoneNames::clone() const {
	return new TZDBTimeZoneNames(fLocale);
}

StringEnumeration * TZDBTimeZoneNames::getAvailableMetaZoneIDs(UErrorCode & status) const {
	return TimeZoneNamesImpl::_getAvailableMetaZoneIDs(status);
}

StringEnumeration * TZDBTimeZoneNames::getAvailableMetaZoneIDs(const UnicodeString & tzID, UErrorCode & status) const {
	return TimeZoneNamesImpl::_getAvailableMetaZoneIDs(tzID, status);
}

UnicodeString &TZDBTimeZoneNames::getMetaZoneID(const UnicodeString & tzID, UDate date, UnicodeString & mzID) const {
	return TimeZoneNamesImpl::_getMetaZoneID(tzID, date, mzID);
}

UnicodeString &TZDBTimeZoneNames::getReferenceZoneID(const UnicodeString & mzID, const char * region, UnicodeString & tzID) const {
	return TimeZoneNamesImpl::_getReferenceZoneID(mzID, region, tzID);
}

UnicodeString &TZDBTimeZoneNames::getMetaZoneDisplayName(const UnicodeString & mzID, UTimeZoneNameType type, UnicodeString & name) const 
{
	name.setToBogus();
	if(mzID.isEmpty()) {
		return name;
...
UnicodeString &TZDBTimeZoneNames::getTimeZoneDisplayName(const UnicodeString & /* tzID */, UTimeZoneNameType /* type */,
    UnicodeString & name) const {
...
TZDBTimeZoneNames::MatchInfoCollection* TZDBTimeZoneNames::find(const UnicodeString & text, int32_t start, uint32_t types, UErrorCode & status) const 
{
...
const TZDBNames* TZDBTimeZoneNames::getMetaZoneNames(const UnicodeString & mzID, UErrorCode & status) 
{
		...
		if(cacheVal == NULL) {
			...
			if(U_SUCCESS(status)) {
				...
				if(newKey) {
					...
					// It's not possible that we get a valid tzdbNames with unknown ID. But just in case..
					ZDELETE(tzdbNames);
				}
			}
