...
// For use in ICU 
// @sobolev #define malloc_Removed(a) uprv_malloc(a)
// @sobolev #define free_Removed(a) uprv_free(a)
...
/* and free routines.  */
// @sobolev #define malloc_Removed(a) decMalloc(a)
// @sobolev #define free_Removed(a) decFree(a)
...
U_CAPI decNumber * U_EXPORT2 uprv_decNumberFromString(decNumber * dn, const char chars[], decContext * set) 
{
	...
	if(allocres) 
		uprv_free(allocres); /* drop any storage used  */
	if(status!=0) 
	...
	do {                            /* protect allocated storage  */
		...
		if(decNumberIsNegative(rhs)) { /* rhs<0  */
			...
			if(needbytes>sizeof(bufb)) { /* need malloc space  */
				allocbufb = (decNumber*)uprv_malloc(needbytes);
...
U_CAPI decNumber * U_EXPORT2 uprv_decNumberPower(decNumber * res, const decNumber * lhs,
	...
	do {                       /* protect allocated storage  */
		...
		if(needbytes>sizeof(dacbuff)) {
			allocdac = (decNumber*)uprv_malloc(needbytes);
			...
			*dac->lsu = 1; /* ..  */
			...
			if(decNumberIsNegative(rhs)) { /* was a **-n [hence digits>0]  */
				...
				if(needbytes>sizeof(invbuff)) {
					allocinv = (decNumber*)uprv_malloc(needbytes);
...
/* This computes C = op(A, B), where op adjusts the coefficient of C (by rounding or shifting) such that the exponent (-scale)   */
/* of C has the value B.  The numerical value of C will equal A, except for the effects of any rounding that occurred.   */
/* res is C, the result.  C may be A or B         */
/* lhs is A, the number to adjust        */
...
U_CAPI decNumber * U_EXPORT2 uprv_decNumberSquareRoot(decNumber * res, const decNumber * rhs,
	...
	do {                       /* protect allocated storage  */
		...
		if(needbytes>(Int)sizeof(buff)) {
			allocbuff = (decNumber*)uprv_malloc(needbytes);
...
static decNumber * decDivideOp(decNumber * res, const decNumber * lhs, const decNumber * rhs, decContext * set, Flag op, uInt * status) 
{
	...
	do {                       /* protect allocated storage  */
		...
		if(acclength*sizeof(Unit)>sizeof(accbuff)) {
			...
			allocacc = (Unit*)uprv_malloc(acclength*sizeof(Unit));
			if(allocacc==NULL) { /* hopeless -- abandon  */
				*status |= DEC_Insufficient_storage;
				break;
			}
		...
		if((var1units+1)*sizeof(Unit)>sizeof(varbuff)) {
			...
			varalloc = (Unit*)uprv_malloc((var1units+1)*sizeof(Unit));
    ...
    #define NEEDTWO (DECDPUN*2)    /* within two decUnitAddSub calls  */
		if(rhs->digits>NEEDTWO) { /* use fastpath...  */
			...
			if(needbytes>(Int)sizeof(zlhibuff)) {
				alloclhi = (uInt*)uprv_malloc(needbytes);
				zlhi = alloclhi;
			}
			needbytes = irhs*sizeof(uInt);
			if(needbytes>(Int)sizeof(zrhibuff)) {
				allocrhi = (uInt*)uprv_malloc(needbytes);
				zrhi = allocrhi;
			}
			...
			if(needbytes>(Int)sizeof(zaccbuff)) {
				allocacc = (uLong*)uprv_malloc(needbytes);
				zacc = (uLong*)allocacc;
			}
			...
			*a->lsu = 2; /* not 1 but < exp(1)  */
			...
			if(rhs->exponent!=use) {
				...
				if(needbytes>sizeof(bufr)) { /* need malloc space  */
					allocrhs = (decNumber*)uprv_malloc(needbytes);
			...
			if(needbytes>sizeof(buft)) { /* need malloc space  */
				allocbuft = (decNumber*)uprv_malloc(needbytes);
...
decNumber * decLnOp(decNumber * res, const decNumber * rhs, decContext * set, uInt * status) 
{
	...
	#define LN2 "0.6931471805599453094172321214581765680755"
		...
		/* table (see above) indexed by the first two digits of f, truncated.  */
		...
		/* now get top two digits of rhs into b by simple truncate and force to integer  */
		...
		bset.emin = -DEC_MAX_MATH*2; /* adjustment calculation [see decExpOp call below]  */
...
static decNumber * decQuantizeOp(decNumber * res, const decNumber * lhs, const decNumber * rhs, decContext * set, Flag quant, uInt * status) 
{
    ...
    #endif
		if(reqexp==BADINT/* bad (rescale only) or ..  */ || reqexp==BIGODD || reqexp==BIGEVEN /* very big (ditto) or ..  */ || (reqexp<etiny) /* < lowest  */ || (reqexp>set->emax)) { /* > emax  */
			*status |= DEC_Invalid_operation;
			break;
		}
...
static decNumber * decRoundOperand(const decNumber * dn, decContext * set,
	...
	res = (decNumber*)uprv_malloc(sizeof(decNumber) + (D2U(set->digits)-1)*sizeof(Unit));
	if(res==NULL) {
		*status |= DEC_Insufficient_storage;
		return NULL;
	}
...
// coefficient constant.  The exponent and status are unchanged except if:
// 
// -- the coefficient was increased and is all nines (in which case Overflow could occur, and is handled directly here so
// the caller does not need to re-test for overflow)
// 
// -- the coefficient was decreased and becomes all nines (in which case Underflow could occur, and is also handled directly).
// 
// All fields in dn are updated as required.
// 
static void decApplyRound(decNumber * dn, decContext * set, Int residue, uInt * status) 
{
	Int bump; /* 1 if coefficient needs to be incremented -1 if coefficient needs to be decremented  */
	if(residue==0) 
...
//#define malloc_Removed(a) decMalloc(a)
//#define free_Removed(a) decFree(a)
#endif
