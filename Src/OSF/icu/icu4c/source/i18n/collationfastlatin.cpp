...
int32_t CollationFastLatin::compareUTF16(const uint16_t * table, const uint16_t * primaries, int32_t options,
	...
	if((options & CollationSettings::CASE_LEVEL) != 0) {
		bool strengthIsPrimary = CollationSettings::getStrength(options) == UCOL_PRIMARY;
		leftIndex = rightIndex = 0;
		leftPair = rightPair = 0;
		for(;;) {
			while(leftPair == 0) {
	...
	if(CollationSettings::getStrength(options) <= UCOL_TERTIARY) {
		return UCOL_EQUAL;
	}
...
uint32_t CollationFastLatin::getSecondaries(uint32_t variableTop, uint32_t pair) {
	if(pair <= 0xffff) {
		// one mini CE
		if(pair >= MIN_SHORT) {
...
uint32_t CollationFastLatin::getCases(uint32_t variableTop, bool strengthIsPrimary, uint32_t pair) {
...
uint32_t CollationFastLatin::getTertiaries(uint32_t variableTop, bool withCaseBits, uint32_t pair) {
	if(pair <= 0xffff) {
		// one mini CE
		if(pair >= MIN_SHORT) {
			// A high secondary weight means we really have two CEs,
...
uint32_t CollationFastLatin::getQuaternaries(uint32_t variableTop, uint32_t pair) {
