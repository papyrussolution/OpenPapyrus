...
const MicroPropsGenerator* NumberFormatterImpl::macrosToMicroGenerator(const MacroProps& macros, bool safe, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return nullptr;
	}
	...
	if(isCldrUnit) {
		...
		if(macros.usage.isSet()) {
			...
			if(!utils::unitIsBaseUnit(macros.perUnit)) {
				...
				if(strcmp(unit.getType(), "") == 0 && (strcmp(macros.unit.getType(), "") == 0 || strcmp(macros.perUnit.getType(), "") == 0)) {
...
const PluralRules* NumberFormatterImpl::resolvePluralRules(const PluralRules* rulesPtr, const Locale & locale,
    UErrorCode & status) {
	if(rulesPtr != nullptr) {
		return rulesPtr;
	}
