...
static bool gStaticZonesInitialized = FALSE; // Whether the static zones are initialized and ready to use.
...
U_CDECL_BEGIN
static bool U_CALLCONV timeZone_cleanup(void)
{
	U_NAMESPACE_USE
	delete DEFAULT_ZONE;
	DEFAULT_ZONE = NULL;
	...
	memzero(TZDATA_VERSION, sizeof(TZDATA_VERSION));
...
static UResourceBundle * getZoneByName(const UResourceBundle * top, const UnicodeString & id, UResourceBundle * oldbundle,
	...
	if((idx == -1) && U_SUCCESS(status)) {
		...
		U_DEBUG_TZ_MSG(("gzbn: loaded ZONES, size %d, type %d, path %s %s\n", ures_getSize(tmp), ures_getType(tmp),
		    ures_getPath(tmp), u_errorName(status)));
		...
		U_DEBUG_TZ_MSG(("gzbn: loaded z#%d, size %d, type %d, path %s, %s\n", idx, ures_getSize(oldbundle), ures_getType(oldbundle),
		    ures_getPath(oldbundle),  u_errorName(status)));
	}
	ures_close(tmp);
	if(U_FAILURE(status)) {
		//ures_close(oldbundle);
...
UResourceBundle * TimeZone::loadRule(const UResourceBundle * top, const UnicodeString & ruleid, UResourceBundle * oldbundle,
    UErrorCode & status) {
	char key[64];
...
static UResourceBundle * openOlsonResource(const UnicodeString & id, UResourceBundle& res, UErrorCode & ec)
{
	...
	// but it is 0 in 2.8 (?).
	U_DEBUG_TZ_MSG(("Loading zone '%s' (%s, size %d) - %s\n", buf, ures_getKey((UResourceBundle*)&res), ures_getSize(&res),
	    u_errorName(ec)));
...
TimeZone* U_EXPORT2 TimeZone::detectHostTimeZone()
{
	...
	int32_t rawOffset = 0;
	const char * hostID;
	bool hostDetectionSucceeded = TRUE;
 ...
 */
void TimeZone::getOffset(UDate date, bool local, int32_t& rawOffset,
...
const char16_t * TimeZone::findID(const UnicodeString & id) 
{
...
const char16_t * TimeZone::dereferOlsonLink(const UnicodeString & id) {
	const char16_t * result = NULL;
	UErrorCode ec = U_ZERO_ERROR;
	UResourceBundle * rb = ures_openDirect(NULL, kZONEINFO, &ec);
	...
	if(U_SUCCESS(ec)) {
		if(ures_getType(rb) == URES_INT) {
			...
			const char16_t * tmp = ures_getStringByIndex(names, deref, NULL, &ec);
			if(U_SUCCESS(ec)) {
				result = tmp;
			}
		}
...
const char16_t * TimeZone::getRegion(const UnicodeString & id) {
	UErrorCode status = U_ZERO_ERROR;
	return getRegion(id, status);
}

const char16_t * TimeZone::getRegion(const UnicodeString & id, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
	const char16_t * result = NULL;
	...
	const char16_t * tmp = ures_getStringByIndex(res, idx, NULL, &status);
	if(U_SUCCESS(status)) {
		result = tmp;
	}
...
bool TimeZone::parseCustomID(const UnicodeString & id, int32_t& sign,
