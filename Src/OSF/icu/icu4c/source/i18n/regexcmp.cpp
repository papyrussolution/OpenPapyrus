...
//  Constructor.
//
RegexCompile::RegexCompile(RegexPattern * rxp, UErrorCode & status) : fParenStack(status), fSetStack(uprv_deleteUObject, nullptr, status), fSetOpStack(status)
{
...
void RegexCompile::compile(UText * pat/*Source pat to be compiled*/, UParseError &pp/*Error position info*/, UErrorCode &e/*Error Code*/)
{
	...
	assert(fRXPat->fPattern == NULL || utext_nativeLength(fRXPat->fPattern) == 0);
...
//
bool RegexCompile::doParseActions(int32_t action)
{
	bool returnVal = TRUE;
	switch((Regex_PatternParseAction)action) {
		...
		case doSetEnd:
		    ...
		    setEval(setEnd);
		    assert(fSetOpStack.peeki()==setStart);
		    fSetOpStack.popi();
		    break;
		...
		case doSetFinish:
	    {
		    ...
		    assert(fSetOpStack.empty());
		    UnicodeSet * theSet = (UnicodeSet*)fSetStack.pop();
		    assert(fSetStack.empty());
		    compileSet(theSet);
		    break;
	    }
...
//
void RegexCompile::fixLiterals(bool split) 
{
...
//
int32_t RegexCompile::blockTopLoc(bool reserveLoc) 
{
	int32_t theLoc;
	...
	if(fRXPat->fCompiledPat->size() == fMatchCloseParen) {
		...
		assert(theLoc > 0);
		assert(URX_TYPE(((uint32_t)fRXPat->fCompiledPat->elementAti(theLoc))) == URX_NOP);
	}
	else {
...
bool RegexCompile::compileInlineInterval() {
...
// Machine Generated Data. Do not hand edit.
	...
	static const int16 RECaseFixStringOffsets[] = {
	...
	static const int16 RECaseFixCounts[] = {
...
void RegexCompile::matchStartType() 
{
	...
	bool atStart = TRUE;          // True if no part of the pattern yet encountered
	...
	for(loc = 3; loc<end; loc++) {
		...
		if(forwardedLength.elementAti(loc) < currentLen) {
			...
			assert(currentLen>=0 && currentLen < INT32_MAX);
		}
			...
			case URX_ONECHAR_I:
			    ...
			    if(currentLen == 0) {
				    ...
				    if(u_hasBinaryProperty(c, UCHAR_CASE_SENSITIVE)) {
					    ...
					    //   For ONECHAR_I, no need to worry about text chars that expand on folding
					    // into strings.
	...
	if(fRXPat->fStartType == START_START) {
		...
		assert(fRXPat->fInitialChar != (UChar32)-1);
	}
...
int32_t RegexCompile::minMatchLength(int32_t start, int32_t end) 
{
	...
	for(loc = start; loc<=end; loc++) {
		...
		// and that path has a shorter minimum length than the current accumulated value,
		// replace the current accumulated value.
		// assert(currentLen>=0 && currentLen < INT32_MAX);  // MinLength == INT32_MAX for some
...
void RegexCompile::error(UErrorCode e) 
{
	if(U_SUCCESS(*fStatus) || e == U_MEMORY_ALLOCATION_ERROR) {
		*fStatus = e;
		...
		memzero(fParseErr->preContext,  sizeof(fParseErr->preContext));
		memzero(fParseErr->postContext, sizeof(fParseErr->postContext));
		...
		if(c.fChar == chBackSlash) {
			...
			if(RegexStaticSets::gStaticSets->fUnescapeCharSet.contains(peekCharLL())) {
				...
				if(UTEXT_FULL_TEXT_IN_CHUNK(fRXPat->fPattern, fPatternLength)) {
					...
					c.fChar = u_unescapeAt(uregex_ucstr_unescape_charAt,
						&endIndex,
						(int32_t)fPatternLength,
						(void *)fRXPat->fPattern->chunkContents);
...
UnicodeSet * RegexCompile::scanPosixProp() 
{
	...
	bool sawPropSetTerminator = FALSE;
	for(;;) {
...
//
UnicodeSet * RegexCompile::createSetForProperty(const UnicodeString & propName, bool negated) {
	if(U_FAILURE(*fStatus)) {
		return nullptr;
	}
