...
int32_t PluralRules::getAllKeywordValues(const UnicodeString & /* keyword */, double * /* dest */,
 ...
 */
static int32_t getSamplesFromString(const UnicodeString & samples, double * destDbl,
...
int32_t PluralRules::getSamples(const UnicodeString & keyword, double * dest,
...
int32_t PluralRules::getSamples(const UnicodeString & keyword, FixedDecimal * dest,
    int32_t destCapacity, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return 0;
	}
...
bool PluralRules::isKeyword(const UnicodeString & keyword) const {
				    ...
				    if(curAndConstraint->rangeList == nullptr) {
					    ...
					    if(curAndConstraint->rangeList->elementAti(rangeLowIdx) == -1) {
							...
							curAndConstraint->rangeList->elementAti(rangeHiIdx)) {
							    ...
							    // U_UNEXPECTED_TOKEN seems a little funny, but it is
							    // consistently
...
bool AndConstraint::isFulfilled(const IFixedDecimal &number) 
{
	bool result = TRUE;
	if(digitsType == none) {
...
bool OrConstraint::isFulfilled(const IFixedDecimal &number) {
...
bool RuleChain::isKeyword(const UnicodeString & keywordParam) const {
...
tokenType PluralRuleParser::charType(char16_t ch) {
...
bool FixedDecimal::quickInit(double n) {
	bool success = FALSE;
	n = fabs(n);
