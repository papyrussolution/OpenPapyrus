...
static bool arrayEqual(const void * a1, const void * a2, int32_t size) 
{
	if(a1 == NULL && a2 == NULL) {
		return TRUE;
	}
	if((a1 && a2 == NULL) || (a1 == NULL && a2)) {
		return FALSE;
	}
	if(a1 == a2) {
		return TRUE;
	...
	BasicTimeZone(tzid), finalZone(NULL)
{
	...
	if(U_SUCCESS(ec)) {
		...
		transitionCountPre32 = static_cast<int16>(len >> 1);
		...
		transitionCount32 = static_cast<int16>(len);
		if(ec == U_MISSING_RESOURCE_ERROR) {
		...
		transitionCountPost32 = static_cast<int16>(len >> 1);
		if(ec == U_MISSING_RESOURCE_ERROR) {
			// No pre-32bit transitions
			...
			if(U_SUCCESS(ec)) {
				...
				if(U_SUCCESS(ec) && len == 11) {
					...
					finalZone = new SimpleTimeZone(
						...
						emptyStr,
						(int8)ruleData[0], (int8)ruleData[1], (int8)ruleData[2],
						ruleData[3] * U_MILLIS_PER_SECOND,
						(SimpleTimeZone::TimeMode)ruleData[4],
						(int8)ruleData[5], (int8)ruleData[6], (int8)ruleData[7],
					...
					if(finalZone == NULL) {
						...
						// Note: Setting finalStartYear to the finalZone is problematic.  When a
						// date is around
						// year boundary, SimpleTimeZone may return false result when DST is
						// observed at the
						// beginning of year.  We could apply safe margin (day or two), but when
						// one of recurrent
						// rules falls around year boundary, it could return false result.
						//  Without setting the
						// start year, finalZone works fine around the year boundary of the
						// start year.
 ...
 * TimeZone API.
 */
int32_t OlsonTimeZone::getOffset(uint8 era, int32_t year, int32_t month, int32_t dom, uint8 dow, int32_t millis, int32_t monthLength, UErrorCode & ec) const 
{
	if(U_FAILURE(ec)) {
		return 0;
	}
	if((era != GregorianCalendar::AD && era != GregorianCalendar::BC) || month < UCAL_JANUARY || month > UCAL_DECEMBER || dom < 1 || 
		dom > monthLength || dow < UCAL_SUNDAY || dow > UCAL_SATURDAY || millis < 0 || millis >= U_MILLIS_PER_DAY || monthLength < 28 || 
		monthLength > 31) {
		ec = U_ILLEGAL_ARGUMENT_ERROR;
		return 0;
	}
	...
	if(finalZone && year >= finalStartYear) {
 ...
 * TimeZone API.
 */
void OlsonTimeZone::getOffset(UDate date, bool local, int32_t& rawoff, int32_t& dstoff, UErrorCode & ec) const 
{
	if(U_FAILURE(ec)) {
		return;
	}
	if(finalZone && date >= finalStartMillis) {
...
void OlsonTimeZone::getOffsetFromLocal(UDate date, UTimeZoneLocalOption nonExistingTimeOpt, UTimeZoneLocalOption duplicatedTimeOpt, int32_t& rawoff, int32_t& dstoff, UErrorCode & ec) const 
{
	if(U_FAILURE(ec)) {
		return;
	}
...
void OlsonTimeZone::getHistoricalOffset(UDate date, bool local,
...
#endif
	int16 transCount = transitionCount();
		...
		if(!local && sec < transitionTimeInSeconds(0)) {
			...
			for(transIdx = transCount - 1; transIdx >= 0; transIdx--) {
				...
				if(local && (sec >= (transition - MAX_OFFSET_SECONDS))) {
					...
					bool dstBefore = dstOffsetAt(transIdx - 1) != 0;

					int32_t offsetAfter = zoneOffsetAt(transIdx);
					bool dstAfter = dstOffsetAt(transIdx) != 0;
 ...
 * TimeZone API.
 */
bool OlsonTimeZone::useDaylightTime() const 
{
 ...
 * TimeZone API.
 */
bool OlsonTimeZone::inDaylightTime(UDate date, UErrorCode & ec) const 
{
	int32_t raw, dst;
...
bool OlsonTimeZone::hasSameRules(const TimeZone &other) const 
{
	if(this == &other) {
		return TRUE;
	}
	...
	if((finalZone == NULL && z->finalZone) || (finalZone && z->finalZone == NULL) || (finalZone && z->finalZone && *finalZone != *z->finalZone)) {
		return FALSE;
	}
	if(finalZone) {
	...
	return
		...
		&& arrayEqual(transitionTimesPost32, z->transitionTimesPost32,
		    sizeof(transitionTimesPost32[0]) * transitionCountPost32 << 1)
...
void OlsonTimeZone::clearTransitionRules() 
{
	initialRule = NULL;
...
void OlsonTimeZone::deleteTransitionRules() 
{
	delete initialRule;
...
void OlsonTimeZone::initTransitionRules(UErrorCode & status) {
	...
	if(transCount > 0) {
		...
		if(transitionIdx == transCount) {
			...
			UDate* times = (UDate*)uprv_malloc(sizeof(UDate)*transCount); /* large enough to store all
			                                                                 transition times */
			if(times == NULL) {
				status = U_MEMORY_ALLOCATION_ERROR;
				deleteTransitionRules();
			...
			for(typeIdx = 0; typeIdx < typeCount; typeIdx++) {
				...
				for(transitionIdx = firstTZTransitionIdx; transitionIdx < transCount; transitionIdx++) {
					if(typeIdx == (int16)typeMapData[transitionIdx]) {
				...
				if(nTimes > 0) {
					...
					if(historicRules == NULL) {
						...
						historicRules = (TimeArrayTimeZoneRule**)uprv_malloc(
							sizeof(TimeArrayTimeZoneRule*)*historicRuleCount);
						if(historicRules == NULL) {
							status = U_MEMORY_ALLOCATION_ERROR;
							deleteTransitionRules();
							uprv_free(times);
			...
			typeIdx = (int16)typeMapData[firstTZTransitionIdx];
...
bool OlsonTimeZone::getNextTransition(UDate base, bool inclusive, TimeZoneTransition& result) const 
{
...
bool OlsonTimeZone::getPreviousTransition(UDate base, bool inclusive, TimeZoneTransition& result) const 
{
	UErrorCode status = U_ZERO_ERROR;
	checkTransitionRules(status);
	if(U_FAILURE(status)) {
...
void OlsonTimeZone::getTimeZoneRules(const InitialTimeZoneRule*& initial, const TimeZoneRule* trsrules[], int32_t& trscount, UErrorCode & status) const 
{
	...
	if(finalZoneWithStartYear && trscount > cnt) {
