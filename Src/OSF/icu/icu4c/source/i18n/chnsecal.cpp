...
U_CDECL_BEGIN
static bool calendar_chinese_cleanup() {
 ...
 * @stable ICU 2.8
 */
int32_t ChineseCalendar::handleComputeMonthStart(int32_t eyear, int32_t month, bool useMonth) const 
{
	...
	int32_t isLeapMonth = useMonth ? saveIsLeapMonth : 0; // Ignore IS_LEAP_MONTH field if useMonth is false
 ...
 * moon
 */
bool ChineseCalendar::hasNoMajorSolarTerm(int32_t newMoon) const 
{
 ...
 * of a new moon
 */
bool ChineseCalendar::isLeapMonthBetween(int32_t newMoon1, int32_t newMoon2) const {
#ifdef U_DEBUG_CHNSECAL
...
bool ChineseCalendar::inDaylightTime(UErrorCode & status) const
{
...
bool ChineseCalendar::haveDefaultCentury() const
{
	return TRUE;
}
