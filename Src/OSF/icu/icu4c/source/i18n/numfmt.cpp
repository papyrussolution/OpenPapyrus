 ...
 */
U_CDECL_BEGIN
static void U_CALLCONV deleteNumberingSystem(void * obj) {
	delete (icu::NumberingSystem*)obj;
}

static bool U_CALLCONV numfmt_cleanup() {
#if !UCONFIG_NO_SERVICE
...
bool SimpleNumberFormatFactory::visible() const {
	return _visible;
}

const UnicodeString * SimpleNumberFormatFactory::getSupportedIDs(int32_t &count, UErrorCode & status) const
{
	if(U_SUCCESS(status)) {
		count = 1;
		return &_id;
...
inline const UChar * ArgExtractor::iso() const {
	return save;
}
...
static ICULocaleService* getNumberFormatService(void)
{
...
NumberFormat* NumberFormat::internalCreateInstance(const Locale & loc, UNumberFormatStyle kind, UErrorCode & status) {
...
NumberFormat* U_EXPORT2 NumberFormat::createInstance(const Locale & loc, UNumberFormatStyle kind, UErrorCode & status) {
...
template <> U_I18N_API
const SharedNumberFormat * LocaleCacheKey<SharedNumberFormat>::createObject(const void * /*unused*/, UErrorCode & status) const {
...
const SharedNumberFormat* U_EXPORT2 NumberFormat::createSharedInstance(const Locale & loc, UNumberFormatStyle kind, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
	if(kind != UNUM_DECIMAL) {
...
bool NumberFormat::isStyleSupported(UNumberFormatStyle style) {
...
NumberFormat* NumberFormat::makeInstance(const Locale & desiredLocale,
	...
	if(ns->isAlgorithmic()) {
		...
		if(df.isValid()) {
			// if the DecimalFormat object was successfully new'ed, then it will own symbolsToAdopt, even if
