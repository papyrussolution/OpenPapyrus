...
U_CAPI bool U_EXPORT2 ucal_inDaylightTime(const UCalendar*      cal,
    UErrorCode *  status)
{
	if(U_FAILURE(*status)) return (bool) -1;
    ...
    int32_t newValue)
{
	switch(attr) {
		case UCAL_LENIENT:
		    ((Calendar*)cal)->setLenient((bool)newValue);
		    break;
...
U_CAPI int32_t U_EXPORT2 ucal_getFieldDifference(UCalendar* cal, UDate target,
    UCalendarDateFields field,
    UErrorCode * status)
{
	if(U_FAILURE(*status)) {
    ...
    UDate* transition, UErrorCode * status)
{
	...
	if(btz != nullptr && U_SUCCESS(*status)) {
		...
		bool inclusive = (type == UCAL_TZ_TRANSITION_NEXT_INCLUSIVE || type == UCAL_TZ_TRANSITION_PREVIOUS_INCLUSIVE);
		bool result = (type == UCAL_TZ_TRANSITION_NEXT || type == UCAL_TZ_TRANSITION_NEXT_INCLUSIVE) ?
