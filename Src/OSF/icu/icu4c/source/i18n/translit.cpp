...
U_NAMESPACE_BEGIN UOBJECT_DEFINE_ABSTRACT_RTTI_IMPLEMENTATION(Transliterator)

/**
 * Return TRUE if the given UTransPosition is valid for text of
 * the given length.
 */
static inline bool positionIsValid(UTransPosition& index, int32_t len) {
 ...
 */
Transliterator* U_EXPORT2 Transliterator::createFromRules(const UnicodeString & ID,
		...
		for(int32_t i = 0; i < limit; i++) {
			...
			if(!parser.dataVector.isEmpty()) {
				...
				RuleBasedTransliterator* temprbt =
					...
					data, TRUE);
				// Check if NULL before adding it to transliterators to avoid future usage of NULL
				// pointer.
				if(temprbt == NULL) {
					if(U_SUCCESS(status)) {
						status = U_MEMORY_ALLOCATION_ERROR;
				...
				// TODO: ICU-21701 the transliterators vector will leak its contents if anything goes
				// wrong.
 ...
 */
bool Transliterator::initializeRegistry(UErrorCode & status) {
	if(registry != 0) {
		return TRUE;
	}
	...
	_registerSpecialInverse(UNICODE_STRING_SIMPLE("Null"), UNICODE_STRING_SIMPLE("Null"), FALSE);
	_registerSpecialInverse(UNICODE_STRING_SIMPLE("Upper"), UNICODE_STRING_SIMPLE("Lower"), TRUE);
	_registerSpecialInverse(UNICODE_STRING_SIMPLE("Title"), UNICODE_STRING_SIMPLE("Lower"), FALSE);
 ...
 */
U_CFUNC bool utrans_transliterator_cleanup() 
{
	U_NAMESPACE_USE
