...
static bool validateRE(const RegularExpression * re, bool requiresText, UErrorCode * status) 
{
	if(U_FAILURE(*status)) {
		return FALSE;
	}
...
//    uregex_open
//
U_CAPI URegularExpression *  U_EXPORT2 uregex_open(const char16_t * pattern, int32_t patternLength, uint32_t flags, UParseError * pe, UErrorCode * status) 
{
	if(U_FAILURE(*status)) {
		return NULL;
	}
...
//    uregex_pattern
//
U_CAPI const char16_t * U_EXPORT2 uregex_pattern(const URegularExpression * regexp2, int32_t * patLength, UErrorCode * status) 
{
...
//    uregex_setText
//
U_CAPI void U_EXPORT2 uregex_setText(URegularExpression * regexp2, const char16_t * text, int32_t textLength, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return;
...
//    uregex_getText
//
U_CAPI const char16_t * U_EXPORT2 uregex_getText(URegularExpression * regexp2, int32_t * textLength, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return NULL;
...
//    uregex_matches
//
U_CAPI bool U_EXPORT2 uregex_matches(URegularExpression * regexp2, int32_t startIndex, UErrorCode * status) 
{
	return uregex_matches64(regexp2, (int64_t)startIndex, status);
}

U_CAPI bool U_EXPORT2 uregex_matches64(URegularExpression * regexp2, int64_t startIndex, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	bool result = FALSE;
...
//    uregex_lookingAt
//
U_CAPI bool U_EXPORT2 uregex_lookingAt(URegularExpression * regexp2, int32_t startIndex, UErrorCode * status) 
{
	return uregex_lookingAt64(regexp2, (int64_t)startIndex, status);
}

U_CAPI bool U_EXPORT2 uregex_lookingAt64(URegularExpression * regexp2, int64_t startIndex, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	bool result = FALSE;
	if(validateRE(regexp, TRUE, status) == FALSE) {
...
//    uregex_find
//
U_CAPI bool U_EXPORT2 uregex_find(URegularExpression * regexp2, int32_t startIndex, UErrorCode * status) 
{
	return uregex_find64(regexp2, (int64_t)startIndex, status);
}

U_CAPI bool U_EXPORT2 uregex_find64(URegularExpression * regexp2, int64_t startIndex, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	bool result = FALSE;
	if(validateRE(regexp, TRUE, status) == FALSE) {
...
//    uregex_findNext
//
U_CAPI bool U_EXPORT2 uregex_findNext(URegularExpression * regexp2, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return FALSE;
	}
	bool result = regexp->fMatcher->find(*status);
	return result;
}
//
//    uregex_groupCount
...
//
int32_t uregex_groupNumberFromName(URegularExpression * regexp2, const char16_t * groupName, int32_t nameLength, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return 0;
...
//    uregex_group
//
U_CAPI int32_t U_EXPORT2 uregex_group(URegularExpression * regexp2, int32_t groupNum, char16_t * dest, int32_t destCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return 0;
...
//
U_CAPI bool U_EXPORT2 uregex_hasTransparentBounds(const URegularExpression   * regexp2, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return FALSE;
...
//
U_CAPI void U_EXPORT2 uregex_useTransparentBounds(URegularExpression    * regexp2, bool b, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return;
...
//
U_CAPI bool U_EXPORT2 uregex_hasAnchoringBounds(const URegularExpression   * regexp2, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return FALSE;
...
//
U_CAPI void U_EXPORT2 uregex_useAnchoringBounds(URegularExpression    * regexp2, bool b, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status) == FALSE) {
		return;
...
//    uregex_hitEnd
//
U_CAPI bool U_EXPORT2 uregex_hitEnd(const URegularExpression   * regexp2, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return FALSE;
...
//    uregex_requireEnd
//
U_CAPI bool U_EXPORT2 uregex_requireEnd(const URegularExpression   * regexp2, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return FALSE;
...
//    uregex_getMatchCallback
//
U_CAPI void U_EXPORT2 uregex_getFindProgressCallback(const URegularExpression * regexp2, URegexFindProgressCallback  ** callback, const void   ** context, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, FALSE, status)) {
...
//    uregex_replaceAll
//
U_CAPI int32_t U_EXPORT2 uregex_replaceAll(URegularExpression * regexp2, const char16_t * replacementText, int32_t replacementLength, char16_t * destBuf, int32_t destCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return 0;
...
//    uregex_replaceFirst
//
U_CAPI int32_t U_EXPORT2 uregex_replaceFirst(URegularExpression  * regexp2, const char16_t * replacementText, int32_t replacementLength,
    char16_t * destBuf, int32_t destCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return 0;
...
class RegexCImpl {
public:
	inline static int32_t appendReplacement(RegularExpression * regexp, const char16_t * replacementText, int32_t replacementLength,
	    char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status);
	inline static int32_t appendTail(RegularExpression    * regexp, char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status);
	inline static int32_t split(RegularExpression    * regexp, char16_t * destBuf, int32_t destCapacity, int32_t * requiredCapacity, char16_t * destFields[], int32_t destFieldsCapacity, UErrorCode * status);
};
...
//
int32_t RegexCImpl::appendReplacement(RegularExpression * regexp, const char16_t * replacementText, int32_t replacementLength,
    char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status) 
{
	...
	bool pendingBufferOverflow = FALSE;
	...
	if(replacementText == NULL || replacementLength < -1 || destCapacity == NULL || destBuf == NULL || (*destBuf == NULL && *destCapacity > 0) || *destCapacity < 0) {
		*status = U_ILLEGAL_ARGUMENT_ERROR;
		return 0;
	}
	...
	while(replIdx < replacementLength && U_SUCCESS(*status)) {
		...
		if(c == BACKSLASH) {
			...
			if(c==0x55 /*U*/ || c==0x75 /*u*/) {
				...
				UChar32 escapedChar = u_unescapeAt(uregex_ucstr_unescape_charAt, &replIdx,  // Index is updated by unescapeAt
		...
		if(u_isdigit(c32)) {
			...
			while(U_SUCCESS(*status) && c32 != RIGHTBRACKET) {
				...
				if((c32 >= 0x41 && c32 <= 0x5a) /*A..Z*/ || (c32 >= 0x61 && c32 <= 0x7a) /*a..z*/|| (c32 >= 0x31 && c32 <= 0x39)/*0..9*/) { 
...
//
U_CAPI int32_t U_EXPORT2 uregex_appendReplacement(URegularExpression    * regexp2, const char16_t * replacementText,
    int32_t replacementLength, char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
...
//    uregex_appendTail
//
int32_t RegexCImpl::appendTail(RegularExpression    * regexp, char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status)
{
	// If we come in with a buffer overflow error, don't suppress the operation.
	//  A series of appendReplacements, appendTail need to correctly preflight
	//  the buffer size when an overflow happens somewhere in the middle.
	...
	//
	// Update the user's buffer ptr and capacity vars to reflect the amount used.
	//
	if(destIdx < destCap) {
		*destBuf      += destIdx;
		*destCapacity -= destIdx;
...
//
U_CAPI int32_t U_EXPORT2 uregex_appendTail(URegularExpression    * regexp2, char16_t ** destBuf, int32_t * destCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
...
//    uregex_split
//
int32_t RegexCImpl::split(RegularExpression * regexp, char16_t * destBuf, int32_t destCapacity, int32_t * requiredCapacity, char16_t * destFields[], int32_t destFieldsCapacity, UErrorCode * status) 
{
	//
...
//
U_CAPI int32_t U_EXPORT2 uregex_split(URegularExpression * regexp2, char16_t * destBuf, int32_t destCapacity, int32_t * requiredCapacity, 
	char16_t * destFields[], int32_t destFieldsCapacity, UErrorCode * status) 
{
	RegularExpression * regexp = (RegularExpression *)regexp2;
	if(validateRE(regexp, TRUE, status) == FALSE) {
		return 0;
