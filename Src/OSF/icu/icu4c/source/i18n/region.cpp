...
static void U_CALLCONV deleteRegion(void * obj) { delete (icu::Region*)obj; }

/**
 * Cleanup callback func
 */
static bool U_CALLCONV region_cleanup(void)
{
...
void U_CALLCONV Region::loadRegionData(UErrorCode & status) {
	...
	for(int32_t i = 0; i < allRegions->size(); i++) {
		...
		r->fType = URGN_TERRITORY; // Only temporary - figure out the real type later once the aliases are
		                           // known.
		...
		uhash_put(newRegionIDMap.getAlias(), idStrAlias, (void *)(r.orphan()), &status); // regionIDMap takes
		                                                                                // ownership
	}
			...
			for(int32_t i = 0; i < ures_getSize(groupingBundle); i++) {
				...
				if(U_SUCCESS(status)) {
					if(grouping->containedRegions == NULL) {
						grouping->containedRegions = new UVector(uprv_deleteUObject,
	...
	while(ures_hasNext(territoryAlias.getAlias())) {
		...
		if(aliasToRegion != NULL && aliasFromRegion == NULL) { // This is just an alias from some string to a
			                                               // region
			...
			if(aliasFromRegion == NULL) { // Deprecated region code not in the primary codes list - so need
				...
				aliasFromRegion->idStr.extract(0, aliasFromRegion->idStr.length(), aliasFromRegion->id,
				    sizeof(aliasFromRegion->id), US_INV);
		...
		for(int j = 0; j < ures_getSize(mapping.getAlias()); j++) {
			...
			if(parentRegion != NULL && childRegion != NULL) {
				...
				if(parentRegion->containedRegions == NULL) {
					parentRegion->containedRegions =
					    new UVector(uprv_deleteUObject, uhash_compareUnicodeString, status);
				}
...
Region::Region () : code(-1), fType(URGN_UNKNOWN), containingRegion(NULL), containedRegions(NULL), preferredValues(NULL) 
{
	id[0] = 0;
}
 ...
 * "SU" (Soviet Union) would return a list of the regions containing "RU" (Russia), "AM" (Armenia), "AZ" (Azerbaijan),
 *etc...
 */
...
int32_t RegionNameEnumeration::count(UErrorCode & /*status*/) const { return (fRegionNames==NULL) ? 0 : fRegionNames->size(); }
RegionNameEnumeration::~RegionNameEnumeration() { delete fRegionNames; }
