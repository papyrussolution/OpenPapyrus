...
bool ParseData::isMatcher(UChar32 ch) {
...
int32_t RuleHalf::parseSection(const UnicodeString & rule, int32_t pos, int32_t limit,
    UnicodeString & buf,
    const UnicodeString & illegal,
    bool isSegment, UErrorCode & status) {
	int32_t start = pos;
	ParsePosition pp;
 ...
 * contain quantifiers or other special input-only elements.
 */
bool RuleHalf::isValidOutput(TransliteratorParser& transParser) {
 ...
 * contain functions or other special output-only elements.
 */
bool RuleHalf::isValidInput(TransliteratorParser& transParser) {
	for(int32_t i = 0; i<text.length();) {
		UChar32 c = text.char32At(i);
		i += U16_LENGTH(c);
 ...
 */
bool TransliteratorParser::checkVariableRange(UChar32 ch) const {
...
static const char16_t PRAGMA_VARIABLE_RANGE[] =
{0x7E, 0x76, 0x61, 0x72, 0x69, 0x61, 0x62, 0x6C, 0x65, 0x20, 0x72, 0x61, 0x6E, 0x67, 0x65, 0x20, 0x23, 0x20, 0x23, 0x7E, 0x3B, 0};                         // "~variable
                                                                                                                                                           // range
                                                                                                                                                           // #
                                                                                                                                                           // #~;"

static const char16_t PRAGMA_MAXIMUM_BACKUP[] =
{0x7E, 0x6D, 0x61, 0x78, 0x69, 0x6D, 0x75, 0x6D, 0x20, 0x62, 0x61, 0x63, 0x6B, 0x75, 0x70, 0x20, 0x23, 0x7E, 0x3B, 0}; // "~maximum
                                                                                                                                                 // backup
                                                                                                                                                 // #~;"

static const char16_t PRAGMA_NFD_RULES[] = {0x7E, 0x6E, 0x66, 0x64, 0x20, 0x72, 0x75, 0x6C, 0x65, 0x73, 0x7E, 0x3B, 0}; // "~nfd
                                                                                                                     // rules~;"

static const char16_t PRAGMA_NFC_RULES[] = {0x7E, 0x6E, 0x66, 0x63, 0x20, 0x72, 0x75, 0x6C, 0x65, 0x73, 0x7E, 0x3B, 0}; // "~nfc
                                                                                                                     // rules~;"
 ...
 */
bool TransliteratorParser::resemblesPragma(const UnicodeString & rule, int32_t pos, int32_t limit) {
...
U_CAPI int32_t utrans_stripRules(const char16_t * source, int32_t sourceLen, char16_t * target, UErrorCode * status) 
{
	...
	bool quoted = FALSE;
	int32_t index;
	memzero(target, sourceLen*U_SIZEOF_UCHAR);
