...
UnicodeString & RelativeDateFormat::format(Calendar& cal,
	...
	if(fDatePattern.isEmpty()) {
		...
		if(relativeDayString.length() > 0) {
			...
			relativeDayString.findAndReplace(UNICODE_STRING("'", 1), UNICODE_STRING("''", 2)); // double any
			                                                                                   // existing
			                                                                                   // APOSTROPHE
...
UnicodeString &RelativeDateFormat::format(const Formattable& obj,
    UnicodeString & appendTo,
    FieldPosition& pos,
...
void RelativeDateFormat::parse(const UnicodeString & text,
	...
	if(fDatePattern.isEmpty()) {
		...
		// Adjust offsets
		bool noError = (pos.getErrorIndex() < 0);
...
UDate RelativeDateFormat::parse(const UnicodeString & text,
    ParsePosition& pos) const {
...
void RelativeDateFormat::setContext(UDisplayContext value, UErrorCode & status)
{
	...
	if(U_SUCCESS(status)) {
		...
		if(fCapitalizationBrkIter == NULL && (value==UDISPCTX_CAPITALIZATION_FOR_BEGINNING_OF_SENTENCE ||
			...
			if(U_FAILURE(status)) {
				ZDELETE(fCapitalizationBrkIter);
			}
		}
#endif
	}
...
void RelativeDateFormat::initCapitalizationContextInfo(const Locale & thelocale)
{
	...
	if(U_SUCCESS(status) && rb != NULL) {
		...
		if(U_SUCCESS(status) && intVector != NULL && len >= 2) {
			fCapitalizationOfRelativeUnitsForUIListMenu = static_cast<bool>(intVector[0]);
			fCapitalizationOfRelativeUnitsForStandAlone = static_cast<bool>(intVector[1]);
		}
	}
#endif
}
