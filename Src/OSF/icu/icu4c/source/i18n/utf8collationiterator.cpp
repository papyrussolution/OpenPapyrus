...
bool UTF8CollationIterator::foundNULTerminator() {
	if(length < 0) {
		length = --pos;
		return TRUE;
	}
...
bool UTF8CollationIterator::forbidSurrogateCodePoints() const {
	return TRUE;
}
...
char16_t FCDUTF8CollationIterator::handleGetTrailSurrogate() {
	if(state != IN_NORMALIZED) {
		return 0;
	}
...
bool FCDUTF8CollationIterator::foundNULTerminator() {
...
bool FCDUTF8CollationIterator::nextSegment(UErrorCode & errorCode) {
...
bool FCDUTF8CollationIterator::previousSegment(UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return FALSE;
	}
	U_ASSERT(state == CHECK_BWD && pos != 0);
