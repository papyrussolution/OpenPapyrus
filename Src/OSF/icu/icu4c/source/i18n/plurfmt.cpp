...
UnicodeString &PluralFormat::format(const Formattable& obj,
...
UnicodeString &PluralFormat::format(int32_t number,
    UnicodeString & appendTo,
    FieldPosition& pos,
    UErrorCode & status) const {
...
UnicodeString &PluralFormat::format(double number,
    UnicodeString & appendTo,
    FieldPosition& pos,
    UErrorCode & status) const {
	return format(Formattable(number), (double)number, appendTo, pos, status);
...
UnicodeString &PluralFormat::format(const Formattable& numberObject, double number,
    UnicodeString & appendTo,
    FieldPosition& pos,
    UErrorCode & status) const {
	if(U_FAILURE(status)) {
...
void PluralFormat::parseType(const UnicodeString & source, const NFRule * rbnfLenientScanner, Formattable& result,
