 ...
 */
U_CDECL_BEGIN
static bool U_CALLCONV collator_cleanup() 
{
#if !UCONFIG_NO_SERVICE
	ZDELETE(gService);
...
{
	bool retVal = !gServiceInitOnce.isReset() && (getService() != NULL);
	return retVal;
}
...
static bool isAvailableLocaleListInitialized(UErrorCode & status) {
...
bool Collator::equals(const UnicodeString & source,
    const UnicodeString & target) const
{
	UErrorCode ec = U_ZERO_ERROR;
...
bool Collator::greaterOrEqual(const UnicodeString & source,
    const UnicodeString & target) const
{
	UErrorCode ec = U_ZERO_ERROR;
...
bool Collator::greater(const UnicodeString & source,
    const UnicodeString & target) const
{
	UErrorCode ec = U_ZERO_ERROR;
...
/*void Collator::getVersion(UVersionInfo versionInfo) const
   {
   if(versionInfo != NULL)
    memcpy(versionInfo, fVersion, U_MAX_VERSION_LENGTH);
   }
 */
...
void Collator::setLocales(const Locale & /* requestedLocale */, const Locale & /* validLocale */, const Locale & /*actualLocale*/) {
}
...
UnicodeString &CFactory::getDisplayName(const UnicodeString & id, const Locale & locale, UnicodeString & result) const
{
	if((_coverage & 0x1) == 0) {
		UErrorCode status = U_ZERO_ERROR;
...
URegistryKey U_EXPORT2 Collator::registerFactory(CollatorFactory* toAdopt, UErrorCode & status)
{
	if(U_SUCCESS(status)) {
...
StringEnumeration * U_EXPORT2 Collator::getKeywords(UErrorCode & status) {
	return UStringEnumeration::fromUEnumeration(
		ucol_getKeywords(&status), status);
}

StringEnumeration * U_EXPORT2 Collator::getKeywordValues(const char * keyword, UErrorCode & status) {
	return UStringEnumeration::fromUEnumeration(
		ucol_getKeywordValues(keyword, &status), status);
}

StringEnumeration * U_EXPORT2 Collator::getKeywordValuesForLocale(const char * key, const Locale & locale,
...
Locale U_EXPORT2 Collator::getFunctionalEquivalent(const char * keyword, const Locale & locale,
...
Collator &Collator::setMaxVariable(UColReorderCode /*group*/, UErrorCode & errorCode) {
...
UColReorderCode Collator::getMaxVariable() const {
...
int32_t Collator::getEquivalentReorderCodes(int32_t reorderCode,
...
int32_t Collator::internalGetShortDefinitionString(const char * /*locale*/,
    char * /*buffer*/,
    int32_t /*capacity*/,
    UErrorCode & status) const {
	if(U_SUCCESS(status)) {
...
UCollationResult Collator::internalCompareUTF8(const char * left, int32_t leftLength,
