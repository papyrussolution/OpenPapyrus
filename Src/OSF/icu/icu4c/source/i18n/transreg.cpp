...
TransliteratorAlias::TransliteratorAlias(const UnicodeString & theID, const UnicodeString & idBlocks, UVector* adoptedTransliterators, const UnicodeSet* cpdFilter) :
	ID(theID), aliasesOrRules(idBlocks), transes(adoptedTransliterators), compoundFilter(cpdFilter), direction(UTRANS_FORWARD), type(TransliteratorAlias::COMPOUND) 
{
}

TransliteratorAlias::TransliteratorAlias(const UnicodeString & theID, const UnicodeString & rules, UTransDirection dir) :
	ID(theID), aliasesOrRules(rules), transes(0), compoundFilter(0), direction(dir), type(TransliteratorAlias::RULES) 
{
}

TransliteratorAlias::~TransliteratorAlias() { delete transes; }
...
Transliterator* TransliteratorAlias::create(UParseError& pe, UErrorCode & ec) 
{
	...
	switch(type) {
		...
		case COMPOUND:
	    {
		    // the total number of transliterators in the compound is the total number of anonymous
		    // transliterators
...
bool TransliteratorAlias::isRuleBased() const { return type == RULES; }
...
private:
	...
	bool isSpecLocale; // TRUE if spec is a locale
	bool isNextLocale; // TRUE if nextSpec is a locale
	ResourceBundle* res;
...
bool TransliteratorSpec::hasFallback() const {
...
bool TransliteratorSpec::isLocale() const {
	return isSpecLocale;
}
...
TransliteratorEntry::TransliteratorEntry() : compoundFilter(NULL), entryType(NONE)
{
	u.prototype = 0;
	DEBUG_newEntry(this);
}
...
TransliteratorEntry::~TransliteratorEntry() 
{
	...
	if(entryType == PROTOTYPE) {
		...
		while(u.dataVector && !u.dataVector->isEmpty())
	...
	switch(entry->entryType) {
		...
		case TransliteratorEntry::COMPOUND_RBT:
	    {
		    ...
		    for(int32_t i = 0; U_SUCCESS(status) && i < entry->u.dataVector->size(); i++) {
			    ...
			    Transliterator* tl = new RuleBasedTransliterator(UnicodeString(
					    CompoundTransliterator::PASS_STRING) + UnicodeString(passNumber++),
				    (TransliterationRuleData*)(entry->u.dataVector->elementAt(i)),
				    FALSE);
			    if(tl == 0)
				    status = U_MEMORY_ALLOCATION_ERROR;
			    else
