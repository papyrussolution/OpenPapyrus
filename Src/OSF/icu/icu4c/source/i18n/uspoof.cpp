...
namespace {
bool U_CALLCONV uspoof_cleanup() {
...
U_CAPI USpoofChecker * U_EXPORT2 uspoof_openFromSerialized(const void * data, int32_t length, int32_t * pActualLength,
    UErrorCode * status) {
	if(U_FAILURE(*status)) {
		return NULL;
	}
...
U_CAPI int32_t U_EXPORT2 uspoof_checkUTF8(const USpoofChecker * sc,
...
U_CAPI int32_t U_EXPORT2 uspoof_check2UTF8(const USpoofChecker * sc,
    const char * id, int32_t length,
    USpoofCheckResult* checkResult,
    UErrorCode * status) {
	if(U_FAILURE(*status)) {
	...
	UnicodeString idStr = UnicodeString::fromUTF8(StringPiece(id, length>=0 ? length : static_cast<int32_t>(strlen(id))));
	int32_t result = uspoof_check2UnicodeString(sc, idStr, checkResult, status);
	return result;
}
...
U_CAPI int32_t U_EXPORT2 uspoof_areConfusableUTF8(const USpoofChecker * sc,
	...
	UnicodeString id1Str = UnicodeString::fromUTF8(StringPiece(id1, length1>=0 ? length1 : static_cast<int32_t>(strlen(id1))));
	UnicodeString id2Str = UnicodeString::fromUTF8(StringPiece(id2, length2>=0 ? length2 : static_cast<int32_t>(strlen(id2))));
	int32_t results = uspoof_areConfusableUnicodeString(sc, id1Str, id2Str, status);
	return results;
}

U_CAPI int32_t U_EXPORT2 uspoof_areConfusableUnicodeString(const USpoofChecker * sc,
...
U_CAPI int32_t U_EXPORT2 uspoof_checkUnicodeString(const USpoofChecker * sc,
...
U_CAPI int32_t U_EXPORT2 uspoof_check2UnicodeString(const USpoofChecker * sc,
    const icu::UnicodeString & id,
    USpoofCheckResult* checkResult,
    UErrorCode * status) {
	const SpoofImpl * This = SpoofImpl::validateThis(sc, *status);
...
U_I18N_API UnicodeString & U_EXPORT2 uspoof_getSkeletonUnicodeString(const USpoofChecker * sc,
    uint32_t /*type*/,
...
U_CAPI int32_t U_EXPORT2 uspoof_getSkeletonUTF8(const USpoofChecker * sc,
    uint32_t type,
    const char * id,  int32_t length,
	...
	UnicodeString srcStr = UnicodeString::fromUTF8(StringPiece(id, length>=0 ? length : static_cast<int32_t>(strlen(id))));
	UnicodeString destStr;
