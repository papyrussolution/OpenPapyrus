...
bool CollationIterator::foundNULTerminator() {
	return FALSE;
}

bool CollationIterator::forbidSurrogateCodePoints() const {
	return FALSE;
}
...
int64_t CollationIterator::nextCEFromCE32(const CollationData * d, UChar32 c, uint32_t ce32,
...
void CollationIterator::appendCEsFromCE32(const CollationData * d, UChar32 c, uint32_t ce32,
    bool forward, UErrorCode & errorCode) {
...
uint32_t CollationIterator::nextCE32FromDiscontiguousContraction(const CollationData * d, UCharsTrie &suffixes, uint32_t ce32,
...
void CollationIterator::appendNumericCEs(uint32_t ce32, bool forward, UErrorCode & errorCode) {
	// Collect digits.
	CharString digits;
	if(forward) {
		for(;;) {
