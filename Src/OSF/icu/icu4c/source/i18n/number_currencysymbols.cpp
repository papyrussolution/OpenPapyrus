...
CurrencySymbols::CurrencySymbols(CurrencyUnit currency, const Locale & locale, UErrorCode & status) : fCurrency(currency), fLocaleName(locale.getName(), status) 
{
...
const char16_t* CurrencySymbols::getIsoCode() const { return fCurrency.getISOCurrency(); }
...
UnicodeString CurrencySymbols::getPluralName(StandardPlural::Form plural, UErrorCode & status) const {
	const char16_t* isoCode = fCurrency.getISOCurrency();
	int32_t symbolLen = 0;
	const char16_t* symbol = ucurr_getPluralName(isoCode, fLocaleName.data(), nullptr /* isChoiceFormat */, StandardPlural::getKeyword(plural),
		&symbolLen, &status);
	// If given an unknown currency, ucurr_getName returns the input string, which we can't alias safely!
	// Otherwise, symbol points to a resource bundle, and we can use readonly-aliasing constructor.
	if(symbol == isoCode) {
...
CurrencyUnit icu::number::impl::resolveCurrency(const DecimalFormatProperties& properties, const Locale & locale,
    UErrorCode & status) {
