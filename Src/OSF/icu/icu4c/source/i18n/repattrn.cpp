...
//
RegexPattern * U_EXPORT2 RegexPattern::compile(const UnicodeString & regex,
    UParseError         &pe,
    UErrorCode          &err)
{
...
//   flags
//
uint32_t RegexPattern::flags() const { return fFlags; }
//
...
//
bool U_EXPORT2 RegexPattern::matches(const UnicodeString & regex, const UnicodeString & input, UParseError & pe, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
	RegexPattern * pat     = RegexPattern::compile(regex, 0, pe, status);
	RegexMatcher * matcher = pat->matcher(input, status);
	bool retVal  = matcher->matches(status);
	delete matcher;
	delete pat;
	return retVal;
}
...
//   matches, UText mode
//
bool U_EXPORT2 RegexPattern::matches(UText * regex, UText * input, UParseError & pe, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
	bool retVal  = FALSE;
	RegexPattern * pat     = RegexPattern::compile(regex, 0, pe, status);
	RegexMatcher * matcher = pat->matcher(status);
	if(U_SUCCESS(status)) {
		matcher->reset(input);
...
void RegexPattern::dumpOp(int32_t index) const 
{
	...
	if((uint32_t)pinnedType >= SIZEOFARRAYi(opNames)) {
		pinnedType = 0;
	}
