...
static UColAttributeValue ucol_sit_letterToAttributeValue(char letter, UErrorCode * status) {
	uint32_t i = 0;
	for(i = 0; i < SIZEOFARRAYi(conversions); i++) {
...
U_CDECL_BEGIN
static const char * U_CALLCONV _processLocaleElement(CollatorSpec * spec, uint32_t value, const char * string,
    UErrorCode * status)
{
	do {
...
U_CDECL_BEGIN
static const char * U_CALLCONV _processCollatorOption(CollatorSpec * spec, uint32_t option, const char * string,
    UErrorCode * status)
{
...
static char16_t readHexCodeUnit(const char ** string, UErrorCode * status)
{
	char16_t result = 0;
	int32_t value = 0;
	char c;
...
static const char * ucol_sit_readSpecs(CollatorSpec * s, const char * string,
...
U_CAPI void U_EXPORT2 ucol_prepareShortStringOpen(const char * definition,
    bool,
...
U_CAPI UCollator* U_EXPORT2 ucol_openFromShortString(const char * definition,
    bool forceDefaults,
    UParseError * parseError,
    UErrorCode * status)
{
...
U_CAPI int32_t U_EXPORT2 ucol_getShortDefinitionString(const UCollator * coll,
    const char * locale,
    char * dst,
    int32_t capacity,
    UErrorCode * status)
...
U_CAPI int32_t U_EXPORT2 ucol_normalizeShortDefinitionString(const char * definition,
    char * destination, int32_t capacity, UParseError * parseError, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		return 0;
	}
	memzero(destination, capacity*sizeof(char));
	UParseError pe;
	if(!parseError) {
		parseError = &pe;
	}
 ...
 */
U_CAPI int32_t U_EXPORT2 ucol_getContractions(const UCollator * coll,
    USet * contractions,
    UErrorCode * status)
{
