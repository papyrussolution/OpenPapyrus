...
static bool U_CALLCONV uspoof_cleanupDefaultData() {
...
void * SpoofData::reserveSpace(int32_t numBytes,  UErrorCode & status) 
{
	...
	memzero((char *)fRawData + returnOffset, numBytes);
	initPtrs(status);
...
U_CAPI int32_t U_EXPORT2 uspoof_swap(const UDataSwapper * ds, const void * inData, int32_t length, void * outData,
	...
	//
	if(inBytes != outBytes) {
		memzero(outBytes, spoofDataLength);
	}
