...
#define MILLIS_PER_YEAR (365 * SlConst::SecsPerDay * 1000.0)
...
BasicTimeZone::BasicTimeZone(const BasicTimeZone& source) : TimeZone(source) 
{
}
...
bool BasicTimeZone::hasEquivalentTransitions(const BasicTimeZone& tz, UDate start, UDate end, bool ignoreDstAmount, UErrorCode & status) const 
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
	...
	while(true) {
		...
		bool inRange1 = (avail1 && tr1.getTime() <= end);
		bool inRange2 = (avail2 && tr2.getTime() <= end);
...
void BasicTimeZone::getSimpleRulesNear(UDate date, InitialTimeZoneRule*& initial,
				...
				if(firstStart > start) {
					...
					if(asize > 0) {
						...
						LocalPointer<TimeArrayTimeZoneRule> newTar(new TimeArrayTimeZoneRule(
							    name, tar->getRawOffset(), tar->getDSTSavings(), &newTimes[0], asize, timeType),
						    status);
