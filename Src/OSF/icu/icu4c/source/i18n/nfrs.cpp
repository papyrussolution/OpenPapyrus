 ...
 */
void NFRuleSet::setBestFractionRule(int32_t originalIndex, NFRule * newRule, bool rememberRule) {
	if(rememberRule) {
...
static bool util_equalRules(const NFRule* rule1, const NFRule* rule2)
{
	if(rule1) {
		if(rule2) {
			return *rule1 == *rule2;
...
bool NFRuleSet::parse(const UnicodeString & text, ParsePosition& pos, double upperBound, uint32_t nonNumericalExecutedRuleMask, Formattable& result) const
{
	...
	for(int i = 0; i < NON_NUMERICAL_RULE_LENGTH; i++) {
		if(nonNumericalRules[i] && ((nonNumericalExecutedRuleMask >> i) & 1) == 0) {
			...
			bool success = nonNumericalRules[i]->doParse(text,
				workingPos,
				0,
				upperBound,
	...
	// are less significant than the rule containing the substitutions)/
	{
		...
		for(int32_t i = rules.size(); --i >= 0 && highWaterMark.getIndex() < text.length();) {
			...
			Formattable tempResult;
			bool success = rules[i]->doParse(text,
				workingPos,
				fIsFractionRuleSet,
				upperBound,
				nonNumericalExecutedRuleMask,
...
uint32_t util64_toa(int64_t w, char * buf, uint32_t len, uint32_t radix, bool raw)
{
	if(radix > 36) {
		radix = 36;
	}
...
uint32_t util64_tou(int64_t w, char16_t * buf, uint32_t len, uint32_t radix, bool raw)
{
	if(radix > 36) {
		radix = 36;
	}
