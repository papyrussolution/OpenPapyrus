...
int64_t CollationData::getSingleCE(UChar32 c, UErrorCode & errorCode) const {
...
uint32_t CollationData::getFirstPrimaryForGroup(int32_t script) const {
	int32_t index = getScriptIndex(script);
	return index == 0 ? 0 : (uint32_t)scriptStarts[index] << 16;
}

uint32_t CollationData::getLastPrimaryForGroup(int32_t script) const {
	int32_t index = getScriptIndex(script);
	if(index == 0) {
		return 0;
	}
...
int32_t CollationData::getGroupForPrimary(uint32_t p) const {
	p >>= 16;
...
int32_t CollationData::getScriptIndex(int32_t script) const 
{
	if(script < 0) {
		return 0;
	}
...
int32_t CollationData::getEquivalentScripts(int32_t script, int32_t dest[], int32_t capacity, UErrorCode & errorCode) const 
{
	if(U_FAILURE(errorCode)) {
		return 0;
	}
...
void CollationData::makeReorderRanges(const int32_t * reorder, int32_t length,
    UVector32 &ranges, UErrorCode & errorCode) const {
	makeReorderRanges(reorder, length, FALSE, ranges, errorCode);
}

void CollationData::makeReorderRanges(const int32_t * reorder, int32_t length, bool latinMustMove, UVector32 &ranges, UErrorCode & errorCode) const 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
