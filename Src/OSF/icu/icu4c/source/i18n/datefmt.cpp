...
public:
	...
	DateFmtBestPattern(const UnicodeString & pattern) : fPattern(pattern) 
	{
	}
	~DateFmtBestPattern();
};
...
template <> U_I18N_API
const DateFmtBestPattern * LocaleCacheKey<DateFmtBestPattern>::createObject(const void * /*creationContext*/, UErrorCode & status) const {
...
DateFormat::DateFormat(const DateFormat& other) : Format(other), fCalendar(0), fNumberFormat(0), fCapitalizationContext(UDISPCTX_CAPITALIZATION_NONE)
{
	*this = other;
}
...
DateFormat& DateFormat::operator = (const DateFormat& other)
{
	if(this != &other) {
		...
		fCalendar = other.fCalendar ? other.fCalendar->clone() : 0;
		fNumberFormat = other.fNumberFormat ? other.fNumberFormat->clone() : 0;
...
UnicodeString &DateFormat::format(const Formattable& obj, UnicodeString & appendTo, FieldPosition& fieldPosition, UErrorCode & status) const
{
	...
	UDate date = 0;
	switch(obj.getType()) {
		case Formattable::kDate: date = obj.getDate(); break;
		case Formattable::kDouble: date = (UDate)obj.getDouble(); break;
		case Formattable::kLong: date = (UDate)obj.getLong(); break;
...
UnicodeString &DateFormat::format(Calendar& /* unused cal */,
...
DateFormat* U_EXPORT2 DateFormat::createInstanceForSkeleton(Calendar * calendarToAdopt, const UnicodeString & skeleton,
...
const Calendar* DateFormat::getCalendar() const { return fCalendar; }
...
const NumberFormat* DateFormat::getNumberFormat() const { return fNumberFormat; }
void DateFormat::adoptTimeZone(TimeZone* zone) { CALLPTRMEMB(fCalendar, adoptTimeZone(zone)); }
void DateFormat::setTimeZone(const TimeZone& zone) { CALLPTRMEMB(fCalendar, setTimeZone(zone)); }

const TimeZone&DateFormat::getTimeZone() const
{
	return fCalendar ? fCalendar->getTimeZone() : *(TimeZone::createDefault()); // If calendar doesn't exists, create default timezone. fCalendar is rarely null
}

void DateFormat::setLenient(bool lenient)
{
	CALLPTRMEMB(fCalendar, setLenient(lenient));
	UErrorCode status = U_ZERO_ERROR;
...
bool DateFormat::isLenient() const
{
	const bool lenient = fCalendar ? fCalendar->isLenient() : true;
	UErrorCode status = U_ZERO_ERROR;
...
void DateFormat::setCalendarLenient(bool lenient)
{
	CALLPTRMEMB(fCalendar, setLenient(lenient));
}

bool DateFormat::isCalendarLenient() const
{
	return fCalendar ? fCalendar->isLenient() : /*fCalendar is rarely null*/false;
}
...
{
	return static_cast<bool>(fBoolFlags.get(attr));
}
