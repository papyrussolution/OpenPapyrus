 ...
 */
static int32_t U_CALLCONV hashPartialLocationKey(const UHashTok key) {
	// <tzID>&<mzID>#[L|S]
 ...
 */
static bool U_CALLCONV comparePartialLocationKey(const UHashTok key1, const UHashTok key2) {
 ...
 */
static void U_CALLCONV deleteGNameInfo(void * obj) {
	uprv_free(obj);
}
...
UnicodeString &TZGNCore::getDisplayName(const TimeZone& tz, UTimeZoneGenericNameType type, UDate date, UnicodeString & name) const 
{
	name.setToBogus();
	switch(type) {
 ...
 */
const UChar * TZGNCore::getGenericLocationName(const UnicodeString & tzCanonicalID) 
{
...
UnicodeString &TZGNCore::formatGenericNonLocationName(const TimeZone& tz, UTimeZoneGenericNameType type, UDate date,
	...
	if(!mzID.isEmpty()) {
		...
		if(sav == 0) {
			...
			if(dynamic_cast<OlsonTimeZone *>(tmptz) || dynamic_cast<SimpleTimeZone *>(tmptz) || dynamic_cast<RuleBasedTimeZone *>(tmptz) || dynamic_cast<VTimeZone *>(tmptz)) {
				btz = (BasicTimeZone*)tmptz;
			}
			if(btz) {
				TimeZoneTransition before;
				bool beforTrs = btz->getPreviousTransition(date, TRUE, before);
				if(beforTrs && (date - before.getTime() < kDstCheckRange) && before.getFrom()->getDSTSavings() != 0) {
					useStandard = FALSE;
				}
				else {
...
UnicodeString &TZGNCore::getPartialLocationName(const UnicodeString & tzCanonicalID,
 ...
 * This method updates the cache and must be called with a lock
 */
const UChar * TZGNCore::getPartialLocationName(const UnicodeString & tzCanonicalID, const UnicodeString & mzID, bool isLong, const UnicodeString & mzDisplayName) 
{
	U_ASSERT(!tzCanonicalID.isEmpty());
 ...
 * except initializer.
 */
void TZGNCore::loadStrings(const UnicodeString & tzCanonicalID) {
...
int32_t TZGNCore::findBestMatch(const UnicodeString & text, int32_t start, uint32_t types, UnicodeString & tzID, UTimeZoneFormatTimeType& timeType, UErrorCode & status) const 
{
	...
	// bool isLongStandard = FALSE;   // workaround - see the comments below
	bool isStandard = FALSE;   // TODO: Temporary hack (on hack) for short standard name/location name conflict (found in zh_Hant), should be removed after CLDR 21m1 integration
	if(tznamesMatches) {
		UnicodeString mzID;
...
TimeZoneGenericNameMatchInfo* TZGNCore::findLocal(const UnicodeString & text, int32_t start, uint32_t types, UErrorCode & status) const {
	...
	if(results && ((maxLen == (text.length() - start)) || fGNamesTrieFullyLoaded)) {
		// perfect match
...
TimeZoneNames::MatchInfoCollection* TZGNCore::findTimeZoneNames(const UnicodeString & text,
    int32_t start,
    uint32_t types,
...
static bool gTZGNCoreCacheInitialized = FALSE;
 ...
 */
static bool U_CALLCONV tzgnCore_cleanup(void)
{
	if(gTZGNCoreCache) {
 ...
 */
static void U_CALLCONV deleteTZGNCoreRef(void * obj) {
...
TimeZoneGenericNames::TimeZoneGenericNames() : fRef(0) 
{
}
...
TimeZoneGenericNames* TimeZoneGenericNames::createInstance(const Locale & locale, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return NULL;
	}
...
TimeZoneGenericNames* TimeZoneGenericNames::clone() const {
...
UnicodeString &TimeZoneGenericNames::getDisplayName(const TimeZone& tz, UTimeZoneGenericNameType type,
    UDate date, UnicodeString & name) const {
	return fRef->obj->getDisplayName(tz, type, date, name);
}

UnicodeString &TimeZoneGenericNames::getGenericLocationName(const UnicodeString & tzCanonicalID, UnicodeString & name) const {
	return fRef->obj->getGenericLocationName(tzCanonicalID, name);
}

int32_t TimeZoneGenericNames::findBestMatch(const UnicodeString & text, int32_t start, uint32_t types,
