...
bool FormattedValueStringBuilderImpl::nextPosition(ConstrainedFieldPosition& cfpos, UErrorCode & status) const {
	// NOTE: MSVC sometimes complains when implicitly converting between bool and bool
	return nextPositionImpl(cfpos, fNumericField, status) ? true : false;
}

bool FormattedValueStringBuilderImpl::nextFieldPosition(FieldPosition& fp, UErrorCode & status) const {
	...
	if(rawField == UNUM_FRACTION_FIELD && fp.getEndIndex() == 0) {
		...
		for(; i < fString.fZero + fString.fLength; i++) {
			if(isIntOrGroup(fString.getFieldPtr()[i]) ||
			    fString.getFieldPtr()[i] == Field(UFIELD_CATEGORY_NUMBER, UNUM_DECIMAL_SEPARATOR_FIELD)) {
				inside = true;
			}
			else if(inside) {
				break;
...
bool FormattedValueStringBuilderImpl::nextPositionImpl(ConstrainedFieldPosition& cfpos, Field numericField, UErrorCode & /*status*/) const {
	...
	if(spanIndicesCount > 0) {
		...
		if(si > 0) {
			prevIsSpan = cfpos.getCategory() == spanIndices[static_cast<ptrdiff_t>(si-1)].category && cfpos.getField() == spanIndices[static_cast<ptrdiff_t>(si-1)].spanValue;
		}
	}
	...
	for(int32_t i = fString.fZero + cfpos.getLimit(); i <= fString.fZero + fString.fLength; i++) {
		...
		if(cfpos.matchesField(UFIELD_CATEGORY_NUMBER, UNUM_INTEGER_FIELD)
		 ...
		 && !isIntOrGroup(_field)) {
			...
			cfpos.setState(UFIELD_CATEGORY_NUMBER, UNUM_INTEGER_FIELD, j - fString.fZero + 1, i - fString.fZero);
			return true;
		}
		...
		if(!prevIsSpan && (_field == Field(UFIELD_CATEGORY_LIST, ULISTFMT_ELEMENT_FIELD) || i - fString.fZero == nextSpanStart)) {
			int64_t si = cfpos.getInt64IterationContext();
			if(si >= spanIndicesCount) {
				break;
			}
			UFieldCategory spanCategory = spanIndices[static_cast<ptrdiff_t>(si)].category;
			int32_t spanValue = spanIndices[static_cast<ptrdiff_t>(si)].spanValue;
			int32_t length = spanIndices[static_cast<ptrdiff_t>(si)].length;
			cfpos.setInt64IterationContext(si + 1);
			if(si + 1 < spanIndicesCount) {
				nextSpanStart = spanIndices[static_cast<ptrdiff_t>(si+1)].start;
			}
			if(cfpos.matchesField(spanCategory, spanValue)) {
				fieldStart = i - fString.fZero;
				int32_t end = fieldStart + length;
				cfpos.setState(spanCategory, spanValue, fieldStart, end);
				return true;
			}
...
void FormattedValueStringBuilderImpl::appendSpanInfo(UFieldCategory category,
    int32_t spanValue,
    int32_t start,
    int32_t length,
    UErrorCode & status) {
...
void FormattedValueStringBuilderImpl::prependSpanInfo(UFieldCategory category,
    int32_t spanValue,
    int32_t start,
    int32_t length,
    UErrorCode & status) {
