...
static int32_t U_CALLCONV ures_a_codepointSort(const void * context, const void * left, const void * right) {
	//CompareContext *cmp=(CompareContext *)context;
	return u_strcmp(((const UResAEntry*)left)->key, ((const UResAEntry*)right)->key);
}
...
static const char16_t Canonical_Items[] = {
	...
	CAP_D, CAP_F, LOW_D, LOW_A, // The UDATPG_x_FIELD constants and these fields have a different order than in
	                            // ICU4J
...
DateTimePatternGenerator*  U_EXPORT2 DateTimePatternGenerator::createInstance(UErrorCode & status) {
	return createInstance(Locale::getDefault(), status);
}

DateTimePatternGenerator* U_EXPORT2 DateTimePatternGenerator::createInstance(const Locale & locale, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return nullptr;
	}
...
DateTimePatternGenerator* U_EXPORT2 DateTimePatternGenerator::createInstanceNoStdPat(const Locale & locale, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return nullptr;
	}
	LocalPointer<DateTimePatternGenerator> result(
...
DateTimePatternGenerator*  U_EXPORT2 DateTimePatternGenerator::createEmptyInstance(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return nullptr;
	}
	LocalPointer<DateTimePatternGenerator> result(
...
DateTimePatternGenerator::DateTimePatternGenerator(const Locale & locale, UErrorCode & status, bool skipStdPatterns) :
	skipMatcher(nullptr),
	fAvailableFormatKeyHash(nullptr),
	fDefaultHourFormatChar(0),
	internalErrorCode(U_ZERO_ERROR)
...
DateTimePatternGenerator&DateTimePatternGenerator::operator = (const DateTimePatternGenerator& other) {
	// reflexive case
	if(&other == this) {
		return *this;
	}
...
bool DateTimePatternGenerator::operator == (const DateTimePatternGenerator& other) const {
	if(this == &other) {
		return true;
	}
...
bool DateTimePatternGenerator::operator != (const DateTimePatternGenerator& other) const { return !operator == (other); }
...
U_CFUNC bool U_CALLCONV allowedHourFormatsCleanup() {
...
void DateTimePatternGenerator::initData(const Locale & locale, UErrorCode & status, bool skipStdPatterns) {
...
struct AllowedHourFormatsSink : public ResourceSink {
	...
	AllowedHourFormat getHourFormatFromUnicodeString(const UnicodeString & s) {
		if(s.length() == 1) {
			...
			if(s[0] == LOW_H && s[1] == LOW_B) {
				return ALLOWED_HOUR_FORMAT_hb;
			}
			if(s[0] == LOW_H && s[1] == CAP_B) {
				return ALLOWED_HOUR_FORMAT_hB;
			}
			if(s[0] == CAP_K && s[1] == LOW_B) {
				return ALLOWED_HOUR_FORMAT_Kb;
			}
			if(s[0] == CAP_K && s[1] == CAP_B) {
				return ALLOWED_HOUR_FORMAT_KB;
			}
			if(s[0] == CAP_H && s[1] == LOW_B) {
				return ALLOWED_HOUR_FORMAT_Hb;
			}
			if(s[0] == CAP_H && s[1] == CAP_B) {
				return ALLOWED_HOUR_FORMAT_HB;
			}
		}
...
UDateFormatHourCycle DateTimePatternGenerator::getDefaultHourCycle(UErrorCode & status) const {
	if(U_FAILURE(status)) {
...
UnicodeString DateTimePatternGenerator::getSkeleton(const UnicodeString & pattern, UErrorCode&
/*status*/) {
	FormatParser fp2;
...
UnicodeString DateTimePatternGenerator::staticGetSkeleton(const UnicodeString & pattern, UErrorCode & /*status*/) {
	FormatParser fp;
	DateTimeMatcher matcher;
	PtnSkeleton localSkeleton;
...
UnicodeString DateTimePatternGenerator::getBaseSkeleton(const UnicodeString & pattern, UErrorCode & /*status*/) {
	FormatParser fp2;
	DateTimeMatcher matcher;
	PtnSkeleton localSkeleton;
	matcher.set(pattern, &fp2, localSkeleton);
...
UnicodeString DateTimePatternGenerator::staticGetBaseSkeleton(const UnicodeString & pattern, UErrorCode & /*status*/) {
	FormatParser fp;
	DateTimeMatcher matcher;
	PtnSkeleton localSkeleton;
	matcher.set(pattern, &fp, localSkeleton);
...
void DateTimePatternGenerator::addICUPatterns(const Locale & locale, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return;
	}
	...
	for(int32_t i = DateFormat::kFull; i<=DateFormat::kShort; i++) {
		...
		if(df && (sdf = dynamic_cast<SimpleDateFormat*>(df)) != nullptr) {
...
void DateTimePatternGenerator::consumeShortTimePattern(const UnicodeString & shortTimePattern,
    UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
...
{
	DateTimePatternGenerator& dtpg; // Destination for data, modified via setters.
	UnicodeString conflictingPattern; // Temporary variable, required for calling addPatternWithSkeleton.
	AvailableFormatsSink(DateTimePatternGenerator& _dtpg) : dtpg(_dtpg) 
	{
	}
	virtual ~AvailableFormatsSink();
	virtual void put(const char * key, ResourceValue &value, bool isRoot, UErrorCode & errorCode) override 
	{
...
void DateTimePatternGenerator::addCLDRData(const Locale & locale, UErrorCode & errorCode) 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
	...
	path.clear().append(DT_DateTimeCalendarTag, errorCode).append('/', errorCode).append(calendarTypeToUse, errorCode).
		append('/', errorCode).append(DT_DateTimeAppendItemsTag, errorCode); // i.e., calendar/xxx/appendItems
	if(U_FAILURE(errorCode)) {
		return;
	}
...
void DateTimePatternGenerator::initHashtable(UErrorCode & err) 
{
	if(U_FAILURE(err)) {
		return;
	}
...
void DateTimePatternGenerator::setAppendItemFormat(UDateTimePatternField field, const UnicodeString & value) 
{
...
const UnicodeString &DateTimePatternGenerator::getAppendItemFormat(UDateTimePatternField field) const 
{
	return appendItemFormats[field];
}

void DateTimePatternGenerator::setAppendItemName(UDateTimePatternField field, const UnicodeString & value) 
{
	setFieldDisplayName(field, UDATPG_WIDTH_APPENDITEM, value);
}

const UnicodeString &DateTimePatternGenerator::getAppendItemName(UDateTimePatternField field) const 
{
	return fieldDisplayNames[field][UDATPG_WIDTH_APPENDITEM];
}

void DateTimePatternGenerator::setFieldDisplayName(UDateTimePatternField field, UDateTimePGDisplayWidth width, const UnicodeString & value) 
{
...
UnicodeString DateTimePatternGenerator::getFieldDisplayName(UDateTimePatternField field, UDateTimePGDisplayWidth width) const 
{
	return fieldDisplayNames[field][width];
}

UnicodeString &DateTimePatternGenerator::getMutableFieldDisplayName(UDateTimePatternField field, UDateTimePGDisplayWidth width) 
{
	return fieldDisplayNames[field][width];
}

void DateTimePatternGenerator::getAppendName(UDateTimePatternField field, UnicodeString & value) 
{
	value = SINGLE_QUOTE;
...
UnicodeString DateTimePatternGenerator::getBestPattern(const UnicodeString & patternForm, UErrorCode & status) 
{
	return getBestPattern(patternForm, UDATPG_MATCH_NO_OPTIONS, status);
}

UnicodeString DateTimePatternGenerator::getBestPattern(const UnicodeString & patternForm, UDateTimePatternMatchOptions options, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
 ...
 */
UnicodeString DateTimePatternGenerator::mapSkeletonMetacharacters(const UnicodeString & patternForm, int32_t* flags, UErrorCode & status) 
{
	UnicodeString patternFormMapped;
	patternFormMapped.remove();
	bool inQuoted = FALSE;
	const int32_t patLen = patternForm.length();
	for(int32_t patPos = 0; patPos < patLen; patPos++) {
		...
		if(patChr == SINGLE_QUOTE) {
			...
			if(patChr == LOW_J || patChr == CAP_C) {
				...
				if(patChr == LOW_J) {
					...
					if(bestAllowed == ALLOWED_HOUR_FORMAT_H || bestAllowed == ALLOWED_HOUR_FORMAT_HB ||
					    bestAllowed == ALLOWED_HOUR_FORMAT_Hb) {
						hourChar = CAP_H;
					}
					else if(bestAllowed == ALLOWED_HOUR_FORMAT_K || bestAllowed == ALLOWED_HOUR_FORMAT_KB ||
					...
					if(bestAllowed == ALLOWED_HOUR_FORMAT_HB || bestAllowed == ALLOWED_HOUR_FORMAT_hB ||
					    bestAllowed == ALLOWED_HOUR_FORMAT_KB) {
						dayPeriodChar = CAP_B;
					}
					else if(bestAllowed == ALLOWED_HOUR_FORMAT_Hb || bestAllowed == ALLOWED_HOUR_FORMAT_hb ||
					    bestAllowed == ALLOWED_HOUR_FORMAT_Kb) {
						dayPeriodChar = LOW_B;
					}
				}
...
UnicodeString DateTimePatternGenerator::replaceFieldTypes(const UnicodeString & pattern, const UnicodeString & skeleton, UErrorCode & status) 
{
	return replaceFieldTypes(pattern, skeleton, UDATPG_MATCH_NO_OPTIONS, status);
}

UnicodeString DateTimePatternGenerator::replaceFieldTypes(const UnicodeString & pattern, const UnicodeString & skeleton, UDateTimePatternMatchOptions options, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return UnicodeString();
	}
...
void DateTimePatternGenerator::setDecimal(const UnicodeString & newDecimal) 
{
...
const UnicodeString &DateTimePatternGenerator::getDecimal() const { return decimal; }

void DateTimePatternGenerator::addCanonicalItems(UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return;
	}
...
void DateTimePatternGenerator::setDateTimeFormat(const UnicodeString & dtFormat) 
{
...
const UnicodeString &DateTimePatternGenerator::getDateTimeFormat() const { return dateTimeFormat; }

void DateTimePatternGenerator::setDateTimeFromCalendar(const Locale & locale, UErrorCode & status) 
{
	if(U_FAILURE(status)) {
		return;
	}
	...
	if(fCalendar->getType() && *fCalendar->getType() != '\0' && strcmp(fCalendar->getType(), DT_DateTimeGregorianTag) != 0) {
		dateTimePatterns.adoptInstead(ures_getByKeyWithFallback(calData.getAlias(), fCalendar->getType(), nullptr, &status));
		ures_getByKeyWithFallback(dateTimePatterns.getAlias(), DT_DateTimePatternsTag, dateTimePatterns.getAlias(), &status);
	}
	if(dateTimePatterns.isNull() || status == U_MISSING_RESOURCE_ERROR) {
		status = U_ZERO_ERROR;
		dateTimePatterns.adoptInstead(ures_getByKeyWithFallback(calData.getAlias(), DT_DateTimeGregorianTag, dateTimePatterns.orphan(), &status));
		ures_getByKeyWithFallback(dateTimePatterns.getAlias(), DT_DateTimePatternsTag, dateTimePatterns.getAlias(), &status);
	}
	if(U_FAILURE(status)) {
		return;
...
void DateTimePatternGenerator::setDecimalSymbols(const Locale & locale, UErrorCode & status) 
{
...
UDateTimePatternConflict DateTimePatternGenerator::addPattern(const UnicodeString & pattern, bool override, UnicodeString & conflictingPattern, UErrorCode & status)
{
	if(U_FAILURE(internalErrorCode)) {
		status = internalErrorCode;
...
// 2. If the new entry's skeleton or basePattern does match an existing entry but that entry also had a skeleton
// specified
// (i.e. it was also from availableFormats), then the new entry does not override it regardless of the value of the
// override
// parameter. This prevents later availableFormats entries from a parent locale overriding earlier ones from the actual
// specified locale. However, availableFormats entries *should* override entries with matching skeleton whose skeleton
// was
...
UDateTimePatternConflict DateTimePatternGenerator::addPatternWithSkeleton(const UnicodeString & pattern,
    const UnicodeString * skeletonToUse, bool override, UnicodeString & conflictingPattern, UErrorCode & status)
{
	if(U_FAILURE(internalErrorCode)) {
		status = internalErrorCode;
		return UDATPG_NO_CONFLICT;
	...
	bool entryHadSpecifiedSkeleton;
...
{
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
		if(sstreq(CLDR_FIELD_APPEND[i], field)) {
...
UDateTimePatternField DateTimePatternGenerator::getFieldAndWidthIndices(const char * key, UDateTimePGDisplayWidth* widthP) const 
{
	...
	*widthP = UDATPG_WIDE;
	...
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
		if(sstreq(CLDR_FIELD_NAME[i], cldrFieldKey)) {
			return (UDateTimePatternField)i;
		}
	}
	return UDATPG_FIELD_COUNT;
...
const UnicodeString * DateTimePatternGenerator::getBestRaw(DateTimeMatcher& source, int32_t includeMask, 
	DistanceInfo* missingFields, UErrorCode & status, const PtnSkeleton** specifiedSkeletonPtr) 
{
...
UnicodeString DateTimePatternGenerator::adjustFieldTypes(const UnicodeString & pattern, const PtnSkeleton* specifiedSkeleton, int32_t flags,
    UDateTimePatternMatchOptions options) 
{
	...
	for(int32_t i = 0; i < fp->itemNumber; i++) {
		...
		if(fp->isQuoteLiteral(field)) {
			...
			// handle day periods - with #13183, no longer need special handling here, integrated with
			// normal types
			...
			if((flags & kDTPGFixFractionalSeconds) != 0 && typeValue == UDATPG_SECOND_FIELD) {
				...
				// requested skeleton, except that in the following cases the length of the adjusted
				// field
				// should match that in the found pattern (i.e. the length of this pattern field should
				// not be adjusted):
				// 1. typeValue is UDATPG_HOUR_FIELD/MINUTE/SECOND and the corresponding bit in options
				// is
				//    not set (ticket #7180). Note, we may want to implement a similar change for other
				//    numeric fields (MM, dd, etc.) so the default behavior is to get locale preference
				// for
				//    field length, but options bits can be used to override this.
				// 2. There is a specified skeleton for the found pattern and one of the following is
				// true:
				...
				if((typeValue==UDATPG_HOUR_FIELD && (options & UDATPG_MATCH_HOUR_FIELD_LENGTH)==0) ||
					...
					// they have no minimum field length-- 'E' and 'EE' are equivalent to 'EEE', but
					// 'e' and 'ee' are not
					...
					bool patFieldIsNumeric = (row->type > 0);
					bool skelFieldIsNumeric = (specifiedSkeleton->type[typeValue] > 0);
...
UnicodeString DateTimePatternGenerator::getBestAppending(int32_t missingFields, int32_t flags, UErrorCode & status, UDateTimePatternMatchOptions options) 
{
	if(U_FAILURE(status)) {
		return UnicodeString();
	}
	...
	if(missingFields!=0) {
		...
		while(distanceInfo->missingFieldMask!=0) { // precondition: EVERY single field must work!
			...
			if(appendItemFormats[topField].length() != 0) {
				...
				const UnicodeString * values[3] = { &resultPattern, &tempPattern, &appendName };
...
int32_t DateTimePatternGenerator::getTopBitNumber(int32_t foundMask) const {
	if(foundMask==0) {
		return 0;
	}
	int32_t i = 0;
...
void DateTimePatternGenerator::setAvailableFormat(const UnicodeString & key, UErrorCode & err)
{
	fAvailableFormatKeyHash->puti(key, 1, err);
}

bool DateTimePatternGenerator::isAvailableFormatSet(const UnicodeString & key) const 
{
	return (bool)(fAvailableFormatKeyHash->geti(key) == 1);
}

void DateTimePatternGenerator::copyHashtable(Hashtable * other, UErrorCode & status) 
{
...
StringEnumeration * DateTimePatternGenerator::getSkeletons(UErrorCode & status) const 
{
	if(U_FAILURE(status)) {
		return nullptr;
	}
	...
	LocalPointer<StringEnumeration> skeletonEnumerator(new DTSkeletonEnumeration(*patternMap, DT_SKELETON, status), status);
	return U_SUCCESS(status) ? skeletonEnumerator.orphan() : nullptr;
}

const UnicodeString &DateTimePatternGenerator::getPatternForSkeleton(const UnicodeString & skeleton) const {
	PtnElem * curElem;
...
StringEnumeration * DateTimePatternGenerator::getBaseSkeletons(UErrorCode & status) const {
	if(U_FAILURE(status)) {
		return nullptr;
	}
	if(U_FAILURE(internalErrorCode)) {
...
StringEnumeration * DateTimePatternGenerator::getRedundants(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return nullptr;
	}
	if(U_FAILURE(internalErrorCode)) {
...
bool DateTimePatternGenerator::isCanonicalItem(const UnicodeString & item) const {
	if(item.length() != 1) {
		return FALSE;
	}
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
...
void PatternMap::copyFrom(const PatternMap& other, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
...
PtnElem* PatternMap::getHeader(char16_t baseChar) const {
	PtnElem* curElem;
...
void PatternMap::add(const UnicodeString & basePattern,
...
const UnicodeString * PatternMap::getPatternFromBasePattern(const UnicodeString & basePattern, bool& skeletonWasSpecified) const { // key
	                                                                                                                            // to
	                                                                                                                            // search
	                                                                                                                            // for
	PtnElem * curElem;
...
bool PatternMap::equals(const PatternMap& other) const {
	if(this==&other) {
		return TRUE;
	}
	for(int32_t bootIndex = 0; bootIndex < MAX_PATTERN_ENTRIES; ++bootIndex) {
...
PtnElem* PatternMap::getDuplicateElem(const UnicodeString & basePattern,
    const PtnSkeleton &skeleton,
    PtnElem * baseElem) {
	PtnElem * curElem;
...
void DateTimeMatcher::set(const UnicodeString & pattern, FormatParser* fp) {
	PtnSkeleton localSkeleton;
	return set(pattern, fp, localSkeleton);
}

void DateTimeMatcher::set(const UnicodeString & pattern, FormatParser* fp, PtnSkeleton& skeletonResult) {
	int32_t i;
	...
	if(!skeletonResult.original.isFieldEmpty(UDATPG_HOUR_FIELD)) {
		if(skeletonResult.original.getFieldChar(UDATPG_HOUR_FIELD)==LOW_H ||
		    skeletonResult.original.getFieldChar(UDATPG_HOUR_FIELD)==CAP_K) {
			// We have a skeleton with 12-hour-cycle format
			if(skeletonResult.original.isFieldEmpty(UDATPG_DAYPERIOD_FIELD)) {
				// But we do not have a day period in the skeleton; add the default DAYPERIOD (currently
				// "a")
				for(i = 0; dtTypes[i].patternChar != 0; i++) {
					if(dtTypes[i].field == UDATPG_DAYPERIOD_FIELD) {
						// first entry for UDATPG_DAYPERIOD_FIELD
						skeletonResult.original.populate(UDATPG_DAYPERIOD_FIELD,
						    dtTypes[i].patternChar,
						    dtTypes[i].minLen);
						skeletonResult.baseOriginal.populate(UDATPG_DAYPERIOD_FIELD,
						    dtTypes[i].patternChar,
						    dtTypes[i].minLen);
...
void DateTimeMatcher::getBasePattern(UnicodeString & result) {
	result.remove(); // Reset the result first.
	skeleton.baseOriginal.appendTo(result);
}

UnicodeString DateTimeMatcher::getPattern() {
	UnicodeString result;
	return skeleton.original.appendTo(result);
}

int32_t DateTimeMatcher::getDistance(const DateTimeMatcher& other, int32_t includeMask, DistanceInfo& distanceInfo) const {
	int32_t result = 0;
	distanceInfo.clear();
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
...
void DateTimeMatcher::copyFrom(const PtnSkeleton& newSkeleton) {
	skeleton.copyFrom(newSkeleton);
}

void DateTimeMatcher::copyFrom() {
	// same as clear
	skeleton.clear();
}

bool DateTimeMatcher::equals(const DateTimeMatcher* other) const {
	if(other==nullptr) {
		return FALSE;
	}
...
int32_t DateTimeMatcher::getFieldMask() const {
	int32_t result = 0;
...
PtnSkeleton* DateTimeMatcher::getSkeletonPtr() {
	return &skeleton;
}
...
FormatParser::TokenStatus FormatParser::setTokens(const UnicodeString & pattern, int32_t startPos, int32_t * len) {
...
void FormatParser::set(const UnicodeString & pattern) {
	int32_t startPos = 0;
...
bool FormatParser::isQuoteLiteral(const UnicodeString & s) {
	return (bool)(s.charAt(0) == SINGLE_QUOTE);
}

// This function assumes the current itemIndex points to the quote literal.
// Please call isQuoteLiteral prior to this function.
void FormatParser::getQuoteLiteral(UnicodeString & quote, int32_t * itemIndex) {
	int32_t i = *itemIndex;
...
bool FormatParser::isPatternSeparator(const UnicodeString & field) const {
...
void DistanceInfo::setTo(const DistanceInfo& other) {
...
void PatternMapIterator::set(PatternMap& newPatternMap) {
	this->patternMap = &newPatternMap;
}

PtnSkeleton* PatternMapIterator::getSkeleton() const {
	if(nodePtr == nullptr) {
		return nullptr;
	}
	else {
...
bool PatternMapIterator::hasNext() const {
...
DateTimeMatcher&PatternMapIterator::next() {
...
UnicodeString PtnSkeleton::getBaseSkeleton() const {
	UnicodeString result;
...
char16_t PtnSkeleton::getFirstChar() const {
...
const UnicodeString * DTSkeletonEnumeration::snext(UErrorCode & status) {
...
void DTSkeletonEnumeration::reset(UErrorCode & /*status*/) {
	pos = 0;
}

int32_t DTSkeletonEnumeration::count(UErrorCode & /*status*/) const {
	return (fSkeletons.isNull()) ? 0 : fSkeletons->size();
}

bool DTSkeletonEnumeration::isCanonicalItem(const UnicodeString & item) {
	if(item.length() != 1) {
		return FALSE;
	}
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
...
void DTRedundantEnumeration::add(const UnicodeString & pattern, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
	if(fPatterns.isNull()) {
...
const UnicodeString * DTRedundantEnumeration::snext(UErrorCode & status) {
...
void DTRedundantEnumeration::reset(UErrorCode & /*status*/) {
	pos = 0;
}

int32_t DTRedundantEnumeration::count(UErrorCode & /*status*/) const {
	return (fPatterns.isNull()) ? 0 : fPatterns->size();
}

bool DTRedundantEnumeration::isCanonicalItem(const UnicodeString & item) const {
	if(item.length() != 1) {
		return FALSE;
	}
	for(int32_t i = 0; i<UDATPG_FIELD_COUNT; ++i) {
