...
char * SortKeyByteSink::GetAppendBuffer(int32_t min_capacity,
...
bool SortKeyLevel::ensureCapacity(int32_t appendCapacity) {
	if(!ok) {
		return FALSE;
	}
...
bool
CollationKeys::LevelCallback::needToWrite(Collation::Level /*level*/) {
	return TRUE;
}
...
void CollationKeys::writeSortKeyUpToQuaternary(CollationIterator &iter,
    const bool * compressibleBytes,
    const CollationSettings &settings,
    SortKeyByteSink &sink,
    Collation::Level minLevel, LevelCallback &callback,
    bool preflight, UErrorCode & errorCode) {
	...
	for(;;) {
		...
		if(p > Collation::NO_CE_PRIMARY && (levels & Collation::PRIMARY_LEVEL_FLAG) != 0) {
			...
			bool isCompressible = compressibleBytes[p >> 24];
			if(settings.hasReordering()) {
				p = settings.reorder(p);
			}
			uint32_t p1 = p >> 24;
