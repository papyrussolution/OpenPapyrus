...
void Calendar::dump() const 
{
	...
	for(i = 0; i<UCAL_FIELD_COUNT; i++) {
		...
		slfprintf_stderr("  %25s: %-11ld", f, fFields[i]);
		if(fStamp[i] == kUnset) {
			slfprintf_stderr(" (unset) ");
		}
		else if(fStamp[i] == kInternallySet) {
			slfprintf_stderr(" (internally set) ");
			//} else if(fStamp[i] == kInternalDefault) {
			//    slfprintf_stderr(" (internal default) ");
		}
		else {
			slfprintf_stderr(" %%%d ", fStamp[i]);
		}
		slfprintf_stderr("\n");
	}
}
...
protected:
	//virtual bool isSupportedID( const UnicodeString & id, UErrorCode & status) const {
	//  if(U_FAILURE(status)) {
	//    return FALSE;
	//  }
...
public:
	CalendarService() : ICULocaleService(UNICODE_STRING_SIMPLE("Calendar"))
	{
		UErrorCode status = U_ZERO_ERROR;
	...
	virtual bool isDefault() const override { return countFactories() == 1; }
};
...
static inline bool isCalendarServiceUsed() { return !gServiceInitOnce.isReset(); }

static void U_CALLCONV initCalendarService(UErrorCode & status)
{
#ifdef U_DEBUG_CALSVC
	slfprintf_stderr("Spinning up Calendar Service\n");
#endif
...
#ifdef U_DEBUG_CALSVC
	slfprintf_stderr("Registering classes..\n");
#endif
	// Register all basic instances.
	gService->registerFactory(new BasicCalendarFactory(), status);
#ifdef U_DEBUG_CALSVC
	slfprintf_stderr("Done..\n");
#endif

	if(U_FAILURE(status)) {
#ifdef U_DEBUG_CALSVC
		slfprintf_stderr("err (%s) registering classes, deleting service.....\n", u_errorName(status));
#endif
		ZDELETE(gService);
	}
}

static ICULocaleService* getCalendarService(UErrorCode & status)
{
...
Calendar::Calendar(UErrorCode & success) : UObject(), fIsTimeSet(FALSE), fAreFieldsSet(FALSE), fAreAllFieldsSet(FALSE),
	fAreFieldsVirtuallySet(FALSE), fNextStamp((int32_t)kMinimumUserStamp), fTime(0), fLenient(TRUE),
	fZone(NULL), fRepeatedWallTime(UCAL_WALLTIME_LAST), fSkippedWallTime(UCAL_WALLTIME_LAST)
{
	validLocale[0] = 0;
	actualLocale[0] = 0;
	clear();
...
Calendar::Calendar(TimeZone* zone, const Locale & aLocale, UErrorCode & success) : UObject(), fIsTimeSet(FALSE), fAreFieldsSet(FALSE),
	fAreAllFieldsSet(FALSE), fAreFieldsVirtuallySet(FALSE), fNextStamp((int32_t)kMinimumUserStamp), fTime(0), fLenient(TRUE),
	fZone(NULL), fRepeatedWallTime(UCAL_WALLTIME_LAST), fSkippedWallTime(UCAL_WALLTIME_LAST)
{
	validLocale[0] = 0;
	actualLocale[0] = 0;
	...
	if(zone == 0) {
		...
		slfprintf_stderr("%s:%d: ILLEGAL ARG because timezone cannot be 0\n",
		    __FILE__, __LINE__);
#endif
...
Calendar::Calendar(const TimeZone& zone, const Locale & aLocale, UErrorCode & success) : UObject(), fIsTimeSet(FALSE),
	fAreFieldsSet(FALSE), fAreAllFieldsSet(FALSE), fAreFieldsVirtuallySet(FALSE), fNextStamp((int32_t)kMinimumUserStamp),
	fTime(0), fLenient(TRUE), fZone(NULL), fRepeatedWallTime(UCAL_WALLTIME_LAST), fSkippedWallTime(UCAL_WALLTIME_LAST)
{
	validLocale[0] = 0;
	actualLocale[0] = 0;
	if(U_FAILURE(success)) {
...
Calendar::Calendar(const Calendar &source) : UObject(source), fZone(NULL)
{
	*this = source;
}

Calendar & Calendar::operator = (const Calendar &right)
{
	if(this != &right) {
...
Calendar * U_EXPORT2 Calendar::makeInstance(const Locale & aLocale, UErrorCode & success) 
{
	if(U_FAILURE(success)) {
		return NULL;
	}
	...
	if(str) {
...
#ifdef U_DEBUG_CALSVC
		slfprintf_stderr("Calendar::createInstance(%s), looking up [%s]\n", aLocale.getName(), l.getName());
#endif
		Locale actualLoc2;
		ZDELETE(u);
		...
		if(str) {
			...
			slfprintf_stderr("err - recursed, 2nd lookup was unistring %s\n", tmp);
#endif
...
Calendar* U_EXPORT2 Calendar::createInstance(TimeZone* zone, const Locale & aLocale, UErrorCode & success)
{
...
Calendar* U_EXPORT2 Calendar::createInstance(const TimeZone& zone, const Locale & aLocale, UErrorCode & success)
{
...
void U_EXPORT2 Calendar::getCalendarTypeFromLocale(const Locale &aLocale,
    char * typeBuffer,
...
bool Calendar::equals(const Calendar& when, UErrorCode & status) const
{
	return (this == &when || getTime(status) == when.getTime(status));
}

bool Calendar::before(const Calendar& when, UErrorCode & status) const
{
	return (this != &when && getTimeInMillis(status) < when.getTimeInMillis(status));
}

bool Calendar::after(const Calendar& when, UErrorCode & status) const
{
	return (this != &when && getTimeInMillis(status) > when.getTimeInMillis(status));
}

const Locale* U_EXPORT2 Calendar::getAvailableLocales(int32_t& count)
{
	return Locale::getAvailableLocales(count);
}

StringEnumeration * U_EXPORT2 Calendar::getKeywordValuesForLocale(const char * key,
    const Locale & locale, bool commonlyUsed, UErrorCode & status)
{
...
UDate U_EXPORT2 Calendar::getNow()
{
 ...
 */
double Calendar::getTimeInMillis(UErrorCode & status) const
{
	if(U_FAILURE(status))
		return 0.0;
	if(!fIsTimeSet)
 ...
 */
void Calendar::setTimeInMillis(double millis, UErrorCode & status) {
	if(U_FAILURE(status))
		return;
...
void Calendar::set(int32_t year, int32_t month, int32_t date, int32_t hour, int32_t minute)
{
	set(UCAL_YEAR, year);
	set(UCAL_MONTH, month);
	set(UCAL_DATE, date);
...
void Calendar::set(int32_t year, int32_t month, int32_t date, int32_t hour, int32_t minute, int32_t second)
{
	set(UCAL_YEAR, year);
	set(UCAL_MONTH, month);
	set(UCAL_DATE, date);
...
bool Calendar::isSet(UCalendarDateFields field) const
{
...
void Calendar::complete(UErrorCode & status)
{
	if(!fIsTimeSet) {
		updateTime(status);
		// Test for buffer overflows
...
#if defined (U_DEBUG_CAL)
	//slfprintf_stderr("%s:%d- Hmm! Jules @ %d, as per %.0lf millis\n",
...
void Calendar::computeGregorianFields(int32_t julianDay, UErrorCode & /* ec */) {
	...
	Grego::dayToFields(julianDay - kEpochStartAsJulianDay,
	    fGregorianYear,
	    fGregorianMonth,
...
#if defined (U_DEBUG_CAL)
	if(fFields[UCAL_DAY_OF_WEEK_IN_MONTH]==0) slfprintf_stderr("%s:%d: DOWIM %d on %g\n",
...
void Calendar::roll(UCalendarDateFields field, int32_t amount, UErrorCode & status)
{
	...
	switch(field) {
		...
		case UCAL_YEAR_WOY:
	    {
		    ...
		    bool era0WithYearsThatGoBackwards = FALSE;
...
#if defined (U_DEBUG_CAL)
		    slfprintf_stderr("%s:%d: ILLEGAL ARG because of roll on non-rollable field %s\n",
...
void Calendar::add(UCalendarDateFields field, int32_t amount, UErrorCode & status)
{
	...
	switch(field) {
		...
		default:
#if defined (U_DEBUG_CAL)
		    slfprintf_stderr("%s:%d: ILLEGAL ARG because field %s not addable", __FILE__, __LINE__, fldName(field));
#endif
		    status = U_ILLEGAL_ARGUMENT_ERROR;
		    return;
				...
				if(newWallTime != prevWallTime) {
					...
					switch(fSkippedWallTime) {
						...
						case UCAL_WALLTIME_NEXT_VALID:
						    ...
						    bool hasTransition = getImmediatePreviousZoneTransition(tmpT,
							    &immediatePrevTrans,
							    status);
...
#if defined (U_DEBUG_CAL)
				slfprintf_stderr("%s:%d: ILLEGAL ARG because field %s's max too large for int32_t\n", __FILE__, __LINE__, fldName(field));
#endif
...
void Calendar::adoptTimeZone(TimeZone* zone)
{
...
TimeZone* Calendar::orphanTimeZone()
{
...
void Calendar::setRepeatedWallTimeOption(UCalendarWallTimeOption option)
{
...
UCalendarWallTimeOption Calendar::getRepeatedWallTimeOption(void) const
{
	return fRepeatedWallTime;
}

void Calendar::setSkippedWallTimeOption(UCalendarWallTimeOption option)
{
	fSkippedWallTime = option;
}

UCalendarWallTimeOption Calendar::getSkippedWallTimeOption(void) const
{
...
{
	if(fFirstDayOfWeek != value && value >= UCAL_SUNDAY && value <= UCAL_SATURDAY) {
...
//
UCalendarWeekdayType Calendar::getDayOfWeekType(UCalendarDaysOfWeek dayOfWeek, UErrorCode & status) const
{
	if(U_FAILURE(status)) {
		return UCAL_WEEKDAY;
	}
...
int32_t Calendar::getWeekendTransition(UCalendarDaysOfWeek dayOfWeek, UErrorCode & status) const
{
	if(U_FAILURE(status)) {
		return 0;
	}
...
bool Calendar::isWeekend(UDate date, UErrorCode & status) const
{
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
bool Calendar::isWeekend(void) const
{
	UErrorCode status = U_ZERO_ERROR;
...
int32_t Calendar::getMinimum(EDateFields field) const { return getLimit((UCalendarDateFields)field, UCAL_LIMIT_MINIMUM); }
int32_t Calendar::getMinimum(UCalendarDateFields field) const { return getLimit(field, UCAL_LIMIT_MINIMUM); }
int32_t Calendar::getMaximum(EDateFields field) const { return getLimit((UCalendarDateFields)field, UCAL_LIMIT_MAXIMUM); }
int32_t Calendar::getMaximum(UCalendarDateFields field) const { return getLimit(field, UCAL_LIMIT_MAXIMUM); }
int32_t Calendar::getGreatestMinimum(EDateFields field) const { return getLimit((UCalendarDateFields)field, UCAL_LIMIT_GREATEST_MINIMUM); }
int32_t Calendar::getGreatestMinimum(UCalendarDateFields field) const { return getLimit(field, UCAL_LIMIT_GREATEST_MINIMUM); }
int32_t Calendar::getLeastMaximum(EDateFields field) const { return getLimit((UCalendarDateFields)field, UCAL_LIMIT_LEAST_MAXIMUM); }
int32_t Calendar::getLeastMaximum(UCalendarDateFields field) const { return getLimit(field, UCAL_LIMIT_LEAST_MAXIMUM); }
int32_t Calendar::getActualMinimum(EDateFields field, UErrorCode & status) const { return getActualMinimum((UCalendarDateFields)field, status); }
...
int32_t Calendar::getActualMinimum(UCalendarDateFields field, UErrorCode & status) const
{
...
#if defined (U_DEBUG_CAL)
			    slfprintf_stderr("%s:%d: ILLEGAL ARG because DOW in month cannot be 0\n", __FILE__, __LINE__);
#endif
...
#if defined (U_DEBUG_CAL)
		slfprintf_stderr("%s:%d: ILLEGAL ARG because of field %s out of range %d..%d  at %d\n", __FILE__, __LINE__, fldName(field), min, max, value);
#endif
		status = U_ILLEGAL_ARGUMENT_ERROR;
		return;
	}
...
void Calendar::computeTime(UErrorCode & status) {
	...
	//    slfprintf_stderr("%s:%d- D'oh- computed jules %d, to mills (%s)%.lf, recomputed %d\n",
			...
			if(U_SUCCESS(status)) {
				...
				if(zoneOffset != (raw + dst)) {
					if(!isLenient()) {
						...
						bool hasTransition = getImmediatePreviousZoneTransition(tmpTime,
 ...
 */
bool Calendar::getImmediatePreviousZoneTransition(UDate base, UDate * transitionTime, UErrorCode & status) const {
	BasicTimeZone * btz = getBasicTimeZone();
	if(btz) {
		TimeZoneTransition trans;
		bool hasTransition = btz->getPreviousTransition(base, TRUE, trans);
		if(hasTransition) {
	...
	if(btz) {
		...
		if(fRepeatedWallTime == UCAL_WALLTIME_FIRST) {
			...
			if(offsetDelta < 0) {
				...
				// Note: If it does not fall into the repeated time range, offsets remain unchanged
				// below.
...
{
	bool useMonth = oneof3(bestField, UCAL_DAY_OF_MONTH, UCAL_WEEK_OF_MONTH, UCAL_DAY_OF_WEEK_IN_MONTH);
	int32_t year;
...
#if defined (U_DEBUG_CAL)
	slfprintf_stderr("%s:%d: bestField= %s - y=%d\n", __FILE__, __LINE__, fldName(bestField), year);
#endif
		...
		if(bestField == UCAL_WEEK_OF_YEAR) { // ------------------------------------- WOY -------------
			if(!isSet(UCAL_YEAR_WOY) || // YWOY not set at all or
			 ...
			 && (fStamp[UCAL_YEAR_WOY]!=kInternallySet) )) { // (excluding where all fields are internally set - then YWOY is used)
				...
				if(woy==1) { // FIRST WEEK ---------------------------------
#if defined (U_DEBUG_CAL)
					slfprintf_stderr("%s:%d - woy=%d, yp=%d, nj(%d)=%d, nf=%d", __FILE__, __LINE__,
					    internalGet(bestField), resolveFields(kYearPrecedence), year+1,
					    nextJulianDay, nextFirst);

					slfprintf_stderr(" next: %d DFW,  min=%d   \n", (7-nextFirst), getMinimalDaysInFirstWeek());
#endif
...
#if defined (U_DEBUG_CAL)
						slfprintf_stderr("%s:%d - date now %d, jd%d, ywoy%d\n",
...
int32_t Calendar::getDefaultMonthInYear(int32_t /*eyear*/)
{
	return 0;
}

int32_t Calendar::getDefaultDayInMonth(int32_t /*eyear*/, int32_t /*month*/)
{
	return 1;
}
...
int32_t Calendar::handleGetExtendedYearFromWeekFields(int32_t yearWoy, int32_t woy)
{
	...
	UCalendarDateFields bestField = resolveFields(kDatePrecedence); // !! Note: if subclasses have a different
	                                                                // table, they should override
	...
	bool jan1InPrevYear = FALSE; // January 1st in the year of WOY is the 1st week?  (i.e. first week is < minimal
	                              // )
	//bool nextJan1InPrevYear = FALSE; // January 1st of Year of WOY + 1 is in the first week?
 ...
 * @internal
 */
void Calendar::prepareGetActual(UCalendarDateFields field, bool isMinimum, UErrorCode & status)
{
...
#if defined (U_DEBUG_CAL)
		    slfprintf_stderr("prepareGetActualHelper(WOM/WOY) - dow=%d\n", dow);
#endif
...
{
#if defined (U_DEBUG_CAL)
	slfprintf_stderr("getActualHelper(%d,%d .. %d, %s)\n", field, startValue, endValue, u_errorName(status));
#endif
	...
	delete work;
#if defined (U_DEBUG_CAL)
	slfprintf_stderr("getActualHelper(%d) = %d\n", field, result);
#endif
	return result;
}

void Calendar::setWeekData(const Locale & desiredLocale, const char * type, UErrorCode & status)
{
	if(U_FAILURE(status)) 
		return;
	...
	fWeekendCeaseMillis = SlConst::SecsPerDay * 1000; // 24*60*60*1000
 ...
 */
void Calendar::updateTime(UErrorCode & status)
{
	computeTime(status);
	if(U_FAILURE(status))
		return;
...
void Calendar::recalculateStamp() 
{
	int32_t index;
...
void Calendar::internalSet(EDateFields field, int32_t value)
{
	internalSet((UCalendarDateFields)field, value);
}

BasicTimeZone* Calendar::getBasicTimeZone() const 
{
	if(dynamic_cast<const OlsonTimeZone *>(fZone) != NULL || dynamic_cast<const SimpleTimeZone *>(fZone) != NULL || 
		dynamic_cast<const RuleBasedTimeZone *>(fZone) != NULL || dynamic_cast<const VTimeZone *>(fZone) != NULL) {
