...
NumberingSystem* U_EXPORT2 NumberingSystem::createInstance(const Locale & inLocale, UErrorCode & status) {
	...
	if(!nsResolved) { // Resolve the numbering system ( default, native, traditional or finance ) into a "real"
		          // numbering system
...
NumberingSystem* U_EXPORT2 NumberingSystem::createInstanceByName(const char * name, UErrorCode & status) {
	...
	bool isAlgorithmic = ( algorithmic == 1 );
...
void NumberingSystem::setAlgorithmic(bool c) {
	algorithmic = c;
}
...
bool NumberingSystem::isAlgorithmic() const {
	return ( algorithmic );
}
...
U_CFUNC bool U_CALLCONV numSysCleanup() {
	delete gNumsysNames;
