...
const char16_t * CompactData::getPattern(int32_t magnitude,
...
void CompactData::getUniquePatterns(UVector &output, UErrorCode & status) const {
	...
	for(auto pattern : patterns) {
		...
		output.addElementX(const_cast<char16_t *>(pattern), status);
...
void CompactData::CompactDataSink::put(const char * key, ResourceValue &value, bool /*noFallback*/,
    UErrorCode & status) {
