...
public:
	...
	virtual double transformNumber(double number) const override {
		return number - uprv_floor(number);
	}
 ...
 */
void NFSubstitution::toString(UnicodeString & text) const
{
 ...
 */
void NFSubstitution::doSubstitution(int64_t number, UnicodeString & toInsertInto, int32_t _pos, int32_t recursionCount,
 ...
 * position to determine exactly where to insert the new text)
 */
void NFSubstitution::doSubstitution(double number, UnicodeString & toInsertInto, int32_t _pos, int32_t recursionCount,
    UErrorCode & status) const {
 ...
 * is left unchanged.
 */
bool NFSubstitution::doParse(const UnicodeString & text,
    ParsePosition& parsePosition,
    double baseValue,
    double upperBound,
    bool lenientParse,
 ...
 * routine.
 */
bool ModulusSubstitution::doParse(const UnicodeString & text,
    ParsePosition& parsePosition,
    double baseValue,
    double upperBound,
    bool lenientParse,
    ...
    Formattable& result) const
{
	...
	if(ruleToUse == NULL) {
		return NFSubstitution::doParse(text,
			   parsePosition,
			   baseValue,
			   upperBound,
			   lenientParse,
 ...
 * it'll produce the same result.
 */
void ModulusSubstitution::toString(UnicodeString & text) const
{
	// use tokenChar() to get the character at the beginning and
	// end of the substitutin token.  In between them will go
	// either the name of the rule set it uses, or the pattern of
 ...
 * toInsertInto
 */
void FractionalPartSubstitution::doSubstitution(double number, UnicodeString & toInsertInto,
    int32_t _pos, int32_t recursionCount, UErrorCode & status) const
{
	...
	if(!byDigits) {
		...
		//          int32_t numberToFormat = (int32_t)uprv_round(transformNumber(number) * uprv_pow(10,
		// kMaxDecimalDigits));
...
bool FractionalPartSubstitution::doParse(const UnicodeString & text,
    ParsePosition& parsePosition,
    double baseValue,
    double /*upperBound*/,
    bool lenientParse,
