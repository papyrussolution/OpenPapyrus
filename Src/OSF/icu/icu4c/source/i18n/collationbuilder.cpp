...
void BundleImporter::getRules(const char * localeID, const char * collationType,
    UnicodeString & rules,
...
void RuleBasedCollator::internalBuildTailoring(const UnicodeString & rules,
    int32_t strength,
    UColAttributeValue decompositionMode,
...
CollationTailoring * CollationBuilder::parseAndBuild(const UnicodeString & ruleString,
...
void CollationBuilder::addRelation(int32_t strength, const UnicodeString & prefix,
...
int32_t CollationBuilder::findOrInsertNodeForCEs(int32_t strength, const char *&parserErrorReason,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return 0;
	}
...
int32_t CollationBuilder::insertNodeBetween(int32_t index, int32_t nextIndex, int64_t node,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return 0;
	}
...
void CollationBuilder::setCaseBits(const UnicodeString & nfdString,
    const char *&parserErrorReason, UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
void CollationBuilder::suppressContractions(const UnicodeSet & set, const char *&parserErrorReason,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
void CollationBuilder::optimize(const UnicodeSet & set, const char *& /* parserErrorReason */,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
uint32_t CollationBuilder::addWithClosure(const UnicodeString & nfdPrefix, const UnicodeString & nfdString,
...
uint32_t CollationBuilder::addOnlyClosure(const UnicodeString & nfdPrefix, const UnicodeString & nfdString,
    const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return ce32;
	...
	if(nfdPrefix.isEmpty()) {
		...
		for(;;) {
			...
			bool samePrefix = prefix == nfdPrefix;
			for(;;) {
				UnicodeString str = stringIter.next();
				if(str.isBogus()) {
					break;
...
bool CollationBuilder::mergeCompositeIntoString(const UnicodeString & nfdString,
...
bool CollationBuilder::ignorePrefix(const UnicodeString & s, UErrorCode & errorCode) const {
	// Do not map non-FCD prefixes.
	return !isFCD(s, errorCode);
}

bool CollationBuilder::ignoreString(const UnicodeString & s, UErrorCode & errorCode) const {
...
bool CollationBuilder::isFCD(const UnicodeString & s, UErrorCode & errorCode) const {
...
uint32_t CollationBuilder::addIfDifferent(const UnicodeString & prefix, const UnicodeString & str,
    const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return ce32;
...
bool CollationBuilder::sameCEs(const int64_t ces1[], int32_t ces1Length,
					...
					if(strength == UCOL_SECONDARY) {
						if(isTailoredNode(node)) {
							...
							if(!sIsTailored) {
								...
								uint32_t sLimit;
								if(s == 0) {
									// Gap at the beginning of the secondary CE
									// range.
...
U_CAPI UCollator * U_EXPORT2 ucol_openRules(const char16_t * rules, int32_t rulesLength,
	...
	UnicodeString r((bool)(rulesLength < 0), rules, rulesLength);
...
U_CAPI int32_t U_EXPORT2 ucol_getUnsafeSet(const UCollator * coll,
    USet * unsafe,
    UErrorCode * status)
{
