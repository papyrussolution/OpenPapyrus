...
static const char16_t gPercentPercent[] = { 0x25, 0x25, 0 }; /* "%%" */
...
static const char16_t gLenientParse[] = { 0x25, 0x25, 0x6C, 0x65, 0x6E, 0x69, 0x65, 0x6E, 0x74, 0x2D, 0x70, 0x61, 0x72, 0x73, 0x65, 0x3A, 0 }; /* "%%lenient-parse:" */
static const char16_t gSemiColon = 0x003B;
static const char16_t gSemiPercent[] = { 0x3B, 0x25, 0 }; /* ";%" */
	...
	void add(void * elem, UErrorCode & status) 
	{
		if(U_SUCCESS(status)) {
			if(size == cap) {
				...
				memzero(start, count); // fill with nulls, just because
			}
			buf[size++] = elem;
		}
	}
...
#ifdef RBNF_DEBUG
	#define RETURN_ERROR(msg) do { parseError(msg); return NULL; } while(0)
	#define EXPLANATION_ARG explanationArg
#else
	#define RETURN_ERROR(msg) do { parseError(NULL); return NULL; } while(0)
...
StringLocalizationInfo* LocDataParser::doParse() {
	skipWhitespace();
	if(!checkInc(OPEN_ANGLE)) {
		RETURN_ERROR("Missing open angle");
	}
	else {
		...
		while(mightHaveNext) {
			...
			if(elem) {
				...
				RETURN_ERROR("Unexpected character");
			}
		}
		...
		if(!checkInc(CLOSE_ANGLE)) {
			if(check(OPEN_ANGLE)) {
				RETURN_ERROR("Missing comma in outer array");
			}
			else {
				RETURN_ERROR("Missing close angle bracket in outer array");
			}
		}

		skipWhitespace();
		if(p != e) {
			RETURN_ERROR("Extra text after close of localization data");
		}
...
char16_t ** LocDataParser::nextArray(int32_t& requiredLength) {
	...
	while(mightHaveNext) {
		...
		if(elem) {
			...
			RETURN_ERROR("Unexpected comma");
		}
	}
	skipWhitespace();
	if(!checkInc(CLOSE_ANGLE)) {
		if(check(OPEN_ANGLE)) {
			RETURN_ERROR("Missing close angle bracket in inner array");
		}
		else {
			RETURN_ERROR("Missing comma in inner array");
		}
	}
	array.add(NULL, ec);
	if(U_SUCCESS(ec)) {
...
char16_t * LocDataParser::nextString() 
{
	...
	if(p < e) {
		...
		if(haveQuote) {
			if(x != c) {
				RETURN_ERROR("Missing matching quote");
			}
			else if(p == start) {
				RETURN_ERROR("Empty string");
			}
			inc();
		}
		else if(x == OPEN_ANGLE || x == TICK || x == QUOTE) {
			RETURN_ERROR("Unexpected character in string");
		}
	}
...
{
	StringLocalizationInfo * p_result = 0;
	if(U_SUCCESS(status)) {
		int32_t len = info.length();
		if(len) { // no error if len == 0
...
{
	return (index >= 0 && index < getNumberOfRuleSets()) ? data[0][index] : NULL;
}

const char16_t * StringLocalizationInfo::getLocaleName(int32_t index) const 
{
	return (index >= 0 && index < getNumberOfDisplayLocales()) ? data[index+1][0] : NULL;
}
...
RuleBasedNumberFormat::RuleBasedNumberFormat(const UnicodeString & description, const UnicodeString & locs,
    const Locale & alocale, UParseError& perror, UErrorCode & status) : fRuleSets(NULL), ruleSetDescriptions(NULL), 
	numRuleSets(0), defaultRuleSet(NULL), locale(alocale), collator(NULL), decimalFormatSymbols(NULL), 
	defaultInfinityRule(NULL), defaultNaNRule(NULL), fRoundingMode(DecimalFormat::ERoundingMode::kRoundUnnecessary), 
	lenient(FALSE), lenientParseRules(NULL), localizations(NULL), capitalizationInfoSet(FALSE), 
	capitalizationForUIListMenu(FALSE), capitalizationForStandAlone(FALSE), capitalizationBrkIter(NULL)
{
...
RuleBasedNumberFormat::RuleBasedNumberFormat(const UnicodeString & description, const UnicodeString & locs,
    UParseError& perror, UErrorCode & status) : fRuleSets(NULL), ruleSetDescriptions(NULL), numRuleSets(0), 
	defaultRuleSet(NULL), locale(Locale::getDefault()), collator(NULL), decimalFormatSymbols(NULL), 
	defaultInfinityRule(NULL), defaultNaNRule(NULL), fRoundingMode(DecimalFormat::ERoundingMode::kRoundUnnecessary), 
	lenient(FALSE), lenientParseRules(NULL), localizations(NULL), capitalizationInfoSet(FALSE), 
	capitalizationForUIListMenu(FALSE), capitalizationForStandAlone(FALSE), capitalizationBrkIter(NULL)
{
...
int32_t RuleBasedNumberFormat::getNumberOfRuleSetDisplayNameLocales() const {
	if(localizations) {
...
UnicodeString &RuleBasedNumberFormat::format(const DecimalQuantity &number,
...
UnicodeString &RuleBasedNumberFormat::format(int64_t number,
    UnicodeString & toAppendTo,
    FieldPosition& /* pos */) const
{
	if(defaultRuleSet) {
...
UnicodeString &RuleBasedNumberFormat::format(double number,
    UnicodeString & toAppendTo,
    FieldPosition& /* pos */) const
{
	UErrorCode status = U_ZERO_ERROR;
...
void RuleBasedNumberFormat::format(double number,
    NFRuleSet& rs,
    UnicodeString & toAppendTo,
    UErrorCode & status) const
{
...
void RuleBasedNumberFormat::parse(const UnicodeString & text, Formattable& result, ParsePosition& parsePosition) const
{
	...
	void RuleBasedNumberFormat::setLenient(bool enabled)
	{
		lenient = enabled;
		if(!enabled)
			ZDELETE(collator);
...
void RuleBasedNumberFormat::init(const UnicodeString & rules, LocalizationInfo* localizationInfos,
    UParseError& pErr, UErrorCode & status)
{
	// TODO: implement UParseError
	memzero(&pErr, sizeof(UParseError));
	...
	if(lp != -1) {
		...
		if(lp == 0 || description.charAt(lp - 1) == gSemiColon) {
			...
			int lpStart = lp + sstrleni(gLenientParse);
...
void RuleBasedNumberFormat::initCapitalizationContextInfo(const Locale & thelocale)
{
	...
	if(U_SUCCESS(status) && rb != NULL) {
		...
		if(U_SUCCESS(status) && intVector != NULL && len >= 2) {
			capitalizationForUIListMenu = static_cast<bool>(intVector[0]);
			capitalizationForStandAlone = static_cast<bool>(intVector[1]);
		}
	}
	ures_close(rb);
#endif
...
#endif
	collator = NULL;
	ZDELETE(decimalFormatSymbols);
	ZDELETE(defaultInfinityRule);
	ZDELETE(defaultNaNRule);
	ZDELETE(lenientParseRules);
#if !UCONFIG_NO_BREAK_ITERATION
	ZDELETE(capitalizationBrkIter);
#endif
	if(localizations) {
