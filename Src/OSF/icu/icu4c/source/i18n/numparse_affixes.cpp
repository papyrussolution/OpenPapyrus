...
bool CodePointMatcher::smokeTest(const StringSegment& segment) const { return segment.startsWith(fCp); }

UnicodeString CodePointMatcher::toString() const { return UnicodeString(u"<CodePoint>"); }
...
UnicodeString AffixMatcher::toString() const 
{
	...
	return UnicodeString(u"<Affix") + UnicodeString(isNegative ? u":negative " : u" ") +
	       (fPrefix ? fPrefix->getPattern() : UnicodeString(u"null")) + UnicodeString(u"#") +
	       (fSuffix ? fSuffix->getPattern() : UnicodeString(u"null")) + UnicodeString(u">");
}
