...
private:
	...
	virtual bool Resize(int32_t appendCapacity, int32_t length) override;
};

FixedSortKeyByteSink::~FixedSortKeyByteSink() {
}

void FixedSortKeyByteSink::AppendBeyondCapacity(const char * bytes, int32_t /*n*/, int32_t length) {
...
bool FixedSortKeyByteSink::Resize(int32_t /*appendCapacity*/, int32_t /*length*/) {
	return FALSE;
}
}  // namespace
...
bool CollationKeyByteSink::Resize(int32_t appendCapacity, int32_t length) {
	if(buffer_ == NULL) {
...
void RuleBasedCollator::adoptTailoring(CollationTailoring * t, UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
...
RuleBasedCollator * RuleBasedCollator::clone() const {
...
bool RuleBasedCollator::operator == (const Collator &other) const 
{
	if(this == &other) {
		return true;
	}
	...
	if((thisIsRoot || !tailoring->rules.isEmpty()) &&
		...
		if(tailoring->rules == o.tailoring->rules) {
			return true;
		}
	}
...
void RuleBasedCollator::setLocales(const Locale &requested, const Locale &valid,
    const Locale &actual) {
...
Locale RuleBasedCollator::getLocale(ULocDataLocaleType type, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
...
const char * RuleBasedCollator::internalGetLocaleID(ULocDataLocaleType type, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return NULL;
	}
	const Locale * result;
...
UnicodeSet * RuleBasedCollator::getTailoredSet(UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return NULL;
	}
...
void RuleBasedCollator::internalGetContractionsAndExpansions(UnicodeSet * contractions, UnicodeSet * expansions,
    bool addPrefixes, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
void RuleBasedCollator::internalAddContractions(UChar32 c, UnicodeSet & set, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
const CollationSettings &RuleBasedCollator::getDefaultSettings() const {
	return *tailoring->settings;
}

UColAttributeValue RuleBasedCollator::getAttribute(UColAttribute attr, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return UCOL_DEFAULT;
	}
	int32_t option;
...
void RuleBasedCollator::setAttribute(UColAttribute attr, UColAttributeValue value,
...
Collator &RuleBasedCollator::setMaxVariable(UColReorderCode group, UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return *this;
	}
...
UColReorderCode RuleBasedCollator::getMaxVariable() const {
	return (UColReorderCode)(UCOL_REORDER_CODE_FIRST + settings->getMaxVariable());
}

uint32_t RuleBasedCollator::getVariableTop(UErrorCode & /*errorCode*/) const {
...
uint32_t RuleBasedCollator::setVariableTop(const char16_t * varTop, int32_t len, UErrorCode & errorCode) 
{
	...
	bool numeric = settings->isNumeric();
	int64_t ce1, ce2;
...
uint32_t RuleBasedCollator::setVariableTop(const UnicodeString & varTop, UErrorCode & errorCode) {
	return setVariableTop(varTop.getBuffer(), varTop.length(), errorCode);
}

void RuleBasedCollator::setVariableTop(uint32_t varTop, UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
int32_t RuleBasedCollator::getReorderCodes(int32_t * dest, int32_t capacity,
...
void RuleBasedCollator::setReorderCodes(const int32_t * reorderCodes, int32_t length,
    UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
UCollationResult RuleBasedCollator::internalCompareUTF8(const char * left, int32_t leftLength,
...
public:
	FCDUTF16NFDIterator(const Normalizer2Impl &nfcImpl, const char16_t * text, const char16_t * textLimit)
		...
		UErrorCode errorCode = U_ZERO_ERROR;
		const char16_t * spanLimit = nfcImpl.makeFCD(text, textLimit, NULL, errorCode);
		if(U_FAILURE(errorCode)) {
			return;
		}
...
protected:
	virtual UChar32 nextRawCodePoint() override {
		if(pos == length || (s[pos] == 0 && length < 0)) {
			return U_SENTINEL;
		}
		UChar32 c;
...
UCollationResult RuleBasedCollator::doCompare(const char16_t * left, int32_t leftLength,
...
UCollationResult RuleBasedCollator::compare(UCharIterator &left, UCharIterator &right,
    UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode) || &left == &right) {
		return UCOL_EQUAL;
	}
	bool numeric = settings->isNumeric();
...
CollationKey &RuleBasedCollator::getCollationKey(const UnicodeString & s, CollationKey &key,
    UErrorCode & errorCode) const {
	return getCollationKey(s.getBuffer(), s.length(), key, errorCode);
}

CollationKey &RuleBasedCollator::getCollationKey(const char16_t * s, int32_t length, CollationKey& key,
    UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
...
int32_t RuleBasedCollator::getSortKey(const char16_t * s, int32_t length,
    uint8 * dest, int32_t capacity) const {
...
void RuleBasedCollator::writeSortKey(const char16_t * s, int32_t length,
    SortKeyByteSink &sink, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return;
	}
	const char16_t * limit = (length >= 0) ? s + length : NULL;
	bool numeric = settings->isNumeric();
...
void RuleBasedCollator::writeIdenticalLevel(const char16_t * s, const char16_t * limit,
    SortKeyByteSink &sink, UErrorCode & errorCode) const {
	// NFD quick check
	const char16_t * nfdQCYesLimit = data->nfcImpl.decompose(s, limit, NULL, errorCode);
	if(U_FAILURE(errorCode)) {
		return;
	}
...
public:
	...
	virtual bool needToWrite(Collation::Level l) override {
		if(!sink.Overflowed()) {
	...
	Collation::Level getLevel() const {
		return level;
	}

	int32_t getLevelCapacity() const {
...
void RuleBasedCollator::internalGetCEs(const UnicodeString & str, UVector64 &ces,
    UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return;
	}
...
bool RuleBasedCollator::isUnsafe(UChar32 c) const {
	return data->isUnsafeBackward(c, settings->isNumeric());
}

void U_CALLCONV RuleBasedCollator::computeMaxExpansions(const CollationTailoring * t, UErrorCode & errorCode) {
	t->maxExpansions = CollationElementIterator::computeMaxExpansions(t->data, errorCode);
}

bool RuleBasedCollator::initMaxExpansions(UErrorCode & errorCode) const {
	umtx_initOnce(tailoring->maxExpansionsInitOnce, computeMaxExpansions, tailoring, errorCode);
	return U_SUCCESS(errorCode);
}

CollationElementIterator * RuleBasedCollator::createCollationElementIterator(const UnicodeString & source) const {
	UErrorCode errorCode = U_ZERO_ERROR;
	if(!initMaxExpansions(errorCode)) {
		return NULL;
	}
...
CollationElementIterator * RuleBasedCollator::createCollationElementIterator(const CharacterIterator& source) const {
	UErrorCode errorCode = U_ZERO_ERROR;
	if(!initMaxExpansions(errorCode)) {
		return NULL;
	}
