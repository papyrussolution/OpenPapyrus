...
DecimalFormatProperties PatternParser::parseToProperties(const UnicodeString & pattern, IgnoreRounding ignoreRounding,
    UErrorCode & status) {
...
void PatternParser::patternInfoToProperties(DecimalFormatProperties& properties, ParsedPatternInfo& patternInfo,
...
UnicodeString PatternStringUtils::convertLocalized(const UnicodeString & input, const DecimalFormatSymbols& symbols,
	...
	for(int32_t i = 0; i < LEN; i++) {
		table[i][localIdx].findAndReplace(UnicodeString(u'\''), UnicodeString(u'’'));
	}
