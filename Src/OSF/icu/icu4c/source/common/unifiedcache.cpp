...
U_CDECL_BEGIN
static bool U_CALLCONV unifiedcache_cleanup() 
{
...
U_CAPI int32_t U_EXPORT2 ucache_hashKeys(const UHashTok key) {
	const CacheKeyBase * ckey = (const CacheKeyBase*)key.pointer;
	return ckey->hashCode();
}

U_CAPI bool U_EXPORT2 ucache_compareKeys(const UHashTok key1, const UHashTok key2) {
	const CacheKeyBase * p1 = (const CacheKeyBase*)key1.pointer;
	const CacheKeyBase * p2 = (const CacheKeyBase*)key2.pointer;
	return *p1 == *p2;
}

U_CAPI void U_EXPORT2 ucache_deleteKey(void * obj) {
...
void UnifiedCache::setEvictionPolicy(int32_t count, int32_t percentageOfInUseItems, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return;
	}
...
const UHashElement * UnifiedCache::_nextElement() const 
{
...
bool UnifiedCache::_flush(bool all) const 
{
	bool result = FALSE;
...
void UnifiedCache::_putNew(const CacheKeyBase &key,
...
void UnifiedCache::_putIfAbsentAndGet(const CacheKeyBase &key,
    const SharedObject *&value,
    UErrorCode & status) const {
	std::lock_guard<std::mutex> lock(*gCacheMutex);
...
void UnifiedCache::_registerPrimary(const CacheKeyBase * theKey, const SharedObject * value) const {
...
void UnifiedCache::_fetch(const UHashElement * element,
    const SharedObject *&value,
    UErrorCode & status) const {
	const CacheKeyBase * theKey = (const CacheKeyBase*)element->key.pointer;
...
bool UnifiedCache::_inProgress(const UHashElement* element) const {
	UErrorCode status = U_ZERO_ERROR;
	const SharedObject * value = NULL;
	_fetch(element, value, status);
	bool result = _inProgress(value, status);
	removeHardRef(value);
	return result;
}

bool UnifiedCache::_inProgress(const SharedObject* theValue, UErrorCode creationStatus) const {
	return (theValue == fNoValue && creationStatus == U_ZERO_ERROR);
}

bool UnifiedCache::_isEvictable(const UHashElement * element) const
{
	const CacheKeyBase * theKey = (const CacheKeyBase*)element->key.pointer;
