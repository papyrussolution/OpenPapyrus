...
BMPSet::BMPSet(const int32_t * parentList, int32_t parentListLength) : list(parentList), listLength(parentListLength)
{
	memzero(latin1Contains, sizeof(latin1Contains));
	memzero(table7FF, sizeof(table7FF));
	memzero(bmpBlockBits, sizeof(bmpBlockBits));
	/*
...
bool BMPSet::contains(UChar32 c) const {
	if((uint32_t)c<=0xff) {
		return (bool)latin1Contains[c];
	}
	else if((uint32_t)c<=0x7ff) {
		return (bool)((table7FF[c&0x3f]&((uint32_t)1<<(c>>6)))!=0);
	}
	...
	if((int8)b<0) {
		...
		if(b<0xc0) {
			...
			if(length>=2 && (b = *(limit-2))>=0xe0) {
				limit -= 2;
				if(static_cast<int>(containsFFFD) != spanCondition) {
					limit0 = limit;
				}
			}
			...
			if(b<0xf0) {
				if( /* handle U+0000..U+FFFF inline */(t1 = (uint8)(s[0]-0x80)) <= 0x3f && (t2 = (uint8)(s[1]-0x80)) <= 0x3f) {
					...
					if(twoBits<=1) {
						...
						if(static_cast<int>(containsSlow(c, list4kStarts[b], list4kStarts[b+1])) != spanCondition) {
							return s-1;
						}
					}
					s += 2;
	...
	do {
		...
		if(c<=0x7ff) {
			...
			if(twoBits<=1) {
				...
				// Look up the code point in its 4k block of code points.
				if(static_cast<int>(containsSlow(c, list4kStarts[lead], list4kStarts[lead+1])) != spanCondition) {
					return prev+1;
				}
			}
		}
		else {
			if(static_cast<int>(containsSlow(c, list4kStarts[0x10], list4kStarts[0x11])) != spanCondition) {
				return prev+1;
			}
		}
	} while(length>0);
