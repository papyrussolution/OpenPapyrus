...
U_CAPI int32_t U_EXPORT2 uprv_stableBinarySearch(char * array, int32_t limit, void * item, int32_t itemSize,
...
static void doInsertionSort(char * array, int32_t length, int32_t itemSize,
	...
	for(j = 1; j<length; ++j) {
		...
		if(insertionPoint<j) {
			...
			memcpy(pv, item, itemSize); /* v=array[j] */
			memmove(dest+itemSize, dest, (j-insertionPoint)*(size_t)itemSize);
			memcpy(dest, pv, itemSize); /* array[insertionPoint]=v */
		}
	}
}

static void insertionSort(char * array, int32_t length, int32_t itemSize,
 ...
 * ISBN 3-519-02260-5
 */
static void subQuickSort(char * array, int32_t start, int32_t limit, int32_t itemSize,
    UComparator * cmp, const void * context,
    void * px, void * pw) {
	int32_t left, right;
	...
	do {
		...
		/* x=array[middle] */
		memcpy(px, array+(size_t)((start+limit)/2)*itemSize, itemSize);
		...
		do {
			...
			if(left<right) {
				...
				if(left<right) {
					memcpy(pw, array+(size_t)left*itemSize, itemSize);
					memcpy(array+(size_t)left*itemSize, array+(size_t)right*itemSize, itemSize);
					memcpy(array+(size_t)right*itemSize, pw, itemSize);
				}
...
static void quickSort(char * array, int32_t length, int32_t itemSize,
    UComparator * cmp, const void * context, UErrorCode * pErrorCode) {
