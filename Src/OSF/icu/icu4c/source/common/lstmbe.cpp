...
public:
	...
	{
		memzero(data_, d1_ * d2_ * sizeof(float));
		return *this;
	}
private:
	void * memory_;
...
void CodePointsVectorizer::vectorize(UText * text, int32_t startPos, int32_t endPos,
...
void GraphemeClusterVectorizer::vectorize(UText * text, int32_t startPos, int32_t endPos,
    UVector32 &offsets, UVector32 &indices, UErrorCode & status) const
{
	if(U_FAILURE(status)) return;
	if(!offsets.ensureCapacity(endPos - startPos, status) ||
...
int32_t LSTMBreakEngine::divideUpDictionaryRange(UText * text,
    int32_t startPos,
    int32_t endPos,
...
const char16_t * LSTMBreakEngine::name() const { return fData->fName; }
