...
#define IS_EVEN_(n) (((n)&1)==0) // @sobolev IS_EVEN conflicts with IS_EVEN is SLIB.H
#define IS_POINTER_EVEN(p) IS_EVEN_((size_t)p)
...
static bool U_CALLCONV noopHasNext(UCharIterator * /*iter*/) {
	return FALSE;
}
...
static int32_t U_CALLCONV stringIteratorGetIndex(UCharIterator * iter, UCharIteratorOrigin origin) 
{
	switch(origin) {
		...
		case UITER_START: return iter->start;
		case UITER_CURRENT: return iter->index;
		case UITER_LIMIT: return iter->limit;
		case UITER_LENGTH: return iter->length;
		default: return -1; /* not a valid origin */ /* Should never get here! */ 
	}
}
...
static int32_t U_CALLCONV stringIteratorMove(UCharIterator * iter, int32_t delta, UCharIteratorOrigin origin) 
{
	int32_t pos;
	switch(origin) {
		case UITER_ZERO: pos = delta; break;
		case UITER_START: pos = iter->start+delta; break;
		case UITER_CURRENT: pos = iter->index+delta; break;
		case UITER_LIMIT: pos = iter->limit+delta; break;
		case UITER_LENGTH: pos = iter->length+delta; break;
...
static bool U_CALLCONV stringIteratorHasNext(UCharIterator * iter) { return iter->index<iter->limit; }
static bool U_CALLCONV stringIteratorHasPrevious(UCharIterator * iter) { return iter->index>iter->start; }
...
static uint32_t U_CALLCONV stringIteratorGetState(const UCharIterator * iter) { return (uint32_t)iter->index; }
...
U_CAPI void U_EXPORT2 uiter_setString(UCharIterator * iter, const char16_t * s, int32_t length) 
{
	if(iter!=0) {
		if(s!=0 && length>=-1) {
			*iter = stringIterator;
...
static void U_CALLCONV characterIteratorSetState(UCharIterator * iter, uint32_t state, UErrorCode * pErrorCode) {
	...
	else if((int32_t)state<((CharacterIterator*)(iter->context))->startIndex() ||
...
static bool U_CALLCONV utf8IteratorHasNext(UCharIterator * iter) { return iter->start<iter->limit || iter->reservedField!=0; }
static bool U_CALLCONV utf8IteratorHasPrevious(UCharIterator * iter) { return iter->start>0; }
