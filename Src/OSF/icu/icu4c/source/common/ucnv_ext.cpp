 ...
 */
static int32_t ucnv_extMatchToU(const int32_t * cx, int8 sisoState,
...
#define UCNV_SISO_STATE(cnv) \
	((cnv)->sharedData->mbcs.outputType==MBCS_OUTPUT_2_SISO ? (int8)(cnv)->mode : \
...
U_CFUNC bool ucnv_extInitialMatchToU(UConverter * cnv, const int32_t * cx,
	...
	/* try to match */
	match = ucnv_extMatchToU(cx, (int8)UCNV_SISO_STATE(cnv),
	...
	if(match>0) {
		...
		*src += match-firstLength;
		...
		cnv->preToUFirstLength = (int8)firstLength;
...
U_CFUNC UChar32 ucnv_extSimpleMatchToU(const int32_t * cx,
 ...
 */
U_CFUNC void ucnv_extContinueMatchToU(UConverter * cnv,
    UConverterToUnicodeArgs * pArgs, int32_t srcIndex,
    UErrorCode * pErrorCode) {
	uint32_t value = 0; /* initialize output-only param to 0 to silence gcc */
	int32_t match, length;

	match = ucnv_extMatchToU(cnv->sharedData->mbcs.extIndexes, (int8)UCNV_SISO_STATE(cnv),
...
static inline bool extFromUUseMapping(bool useFallback, uint32_t value, UChar32 firstCP) {
	return
...
static int32_t ucnv_extMatchFromU(const int32_t * cx,
	...
	stage12 = UCNV_EXT_ARRAY(cx, UCNV_EXT_FROM_U_STAGE_12_INDEX, uint16);
	stage3 = UCNV_EXT_ARRAY(cx, UCNV_EXT_FROM_U_STAGE_3_INDEX, uint16);
 ...
 */
static inline void ucnv_extWriteFromU(UConverter * cnv, const int32_t * cx,
    uint32_t value,
 ...
 */
U_CFUNC int32_t ucnv_extSimpleMatchFromU(const int32_t * cx,
...
static bool extSetUseMapping(UConverterUnicodeSet which, int32_t minLength, uint32_t value) {
...
U_CFUNC void ucnv_extGetUnicodeSet(const UConverterSharedData * sharedData,
    ...
    UErrorCode * pErrorCode) {
	const int32_t * cx;
	const uint16 * stage12, * stage3, * ps2, * ps3;
	const uint32_t * stage3b;
						...
						if(value==0) {
							...
							switch(filter) {
								case UCNV_SET_FILTER_2022_CN:
								    if(!(UCNV_EXT_FROM_U_GET_LENGTH(value)==3 &&
									UCNV_EXT_FROM_U_GET_DATA(value)<=0x82ffff)) {
									    continue;
								    }
								    break;
								case UCNV_SET_FILTER_SJIS:
								    if(!(UCNV_EXT_FROM_U_GET_LENGTH(value)==2 &&
									(value = UCNV_EXT_FROM_U_GET_DATA(value))>=0x8140 &&
									value<=0xeffc)) {
									    continue;
								    }
								    break;
								case UCNV_SET_FILTER_GR94DBCS:
								    if(!(UCNV_EXT_FROM_U_GET_LENGTH(value)==2 &&
									(uint16)((value = UCNV_EXT_FROM_U_GET_DATA(value))-0xa1a1)<=
									(0xfefe - 0xa1a1) &&
								...
								default:
								     ...
								     * or UCNV_SET_FILTER_DBCS_ONLY which is handled via
								     *minLength
								     */
								    break;
							}
