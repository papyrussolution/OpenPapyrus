...
private:
	...
	int32_t compactData(int32_t fastILimit, uint32_t * newData, int32_t newDataCapacity,
...
UChar32 MutableCodePointTrie::getRange(UChar32 start, UCPMapValueFilter * filter, const void * context,
		...
		if(flags[i] == ALL_SAME) {
			...
			if(haveValue) {
				if(trieValue2 != trieValue) {
					if(filter == nullptr || maybeFilterValue(trieValue2, initialValue, nullValue,
...
private:
	...
	template <typename UIntA, typename UIntB> int32_t findEntry(const UIntA * data, const UIntB * blockData, int32_t blockStart,
 ...
 */
int32_t MutableCodePointTrie::compactData(int32_t fastILimit, uint32_t * newData, int32_t newDataCapacity,
    int32_t dataNullIndex, MixedBlocks &mixedBlocks, UErrorCode & errorCode) {
#ifdef UCPTRIE_DEBUG
...
U_CAPI UChar32 U_EXPORT2 umutablecptrie_getRange(const UMutableCPTrie * trie, UChar32 start,
...
U_CAPI void U_EXPORT2 umutablecptrie_setRange(UMutableCPTrie * trie, UChar32 start, UChar32 end,
...
U_CAPI UCPTrie * U_EXPORT2 umutablecptrie_buildImmutable(UMutableCPTrie * trie, UCPTrieType type, UCPTrieValueWidth valueWidth,
