 ...
 */
static inline uint32_t findConverter(const char * alias, bool * containsOption, UErrorCode * pErrorCode) 
{
	...
	for(;;) {
		...
		if(result < 0) {
			...
			/* State whether the canonical converter name contains an option.
			   This information is contained in this list in order to maintain backward & forward
			      compatibility. */
			if(containsOption) {
 ...
 */
static inline bool isAliasInList(const char * alias, uint32_t listOffset) {
	if(listOffset) {
		uint32_t currAlias;
			...
			for(idx = 0; idx < gMainTable.taggedAliasArraySize; idx++) {
				...
				if(listOffset && isAliasInList(alias, listOffset)) {
					...
					uint32_t tempListOffset =
					    gMainTable.taggedAliasArray[tagNum*gMainTable.converterListSize + currConvNum];
...
U_CAPI const char * ucnv_io_getConverterName(const char * alias, bool * containsOption, UErrorCode * pErrorCode) 
{
...
U_CAPI int32_t U_EXPORT2 ucnv_swapAliases(const UDataSwapper * ds, const void * inData, int32_t length, void * outData, UErrorCode * pErrorCode) 
{
	...
	/* compute offsets */
	memzero(offsets, sizeof(offsets));
