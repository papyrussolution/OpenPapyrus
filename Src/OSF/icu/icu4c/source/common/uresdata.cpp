...
static const uint16 gEmpty16 = 0;
...
static void res_init(ResourceData * pResData,
	...
	if(formatVersion[0]==1 && formatVersion[1]==0) {
		...
		if(indexLength>URES_INDEX_ATTRIBUTES) {
			...
			pResData->noFallback = (bool)(att&URES_ATT_NO_FALLBACK);
			pResData->isPoolBundle = (bool)((att&URES_ATT_IS_POOL_BUNDLE)!=0);
			pResData->usesPoolBundle = (bool)((att&URES_ATT_USES_POOL_BUNDLE)!=0);
...
U_CAPI void U_EXPORT2 res_read(ResourceData * pResData,
    const UDataInfo * pInfo, const void * inBytes, int32_t length,
    UErrorCode * errorCode) 
{
	UVersionInfo formatVersion;
	memzero(pResData, sizeof(ResourceData));
...
static const int8 gPublicTypes[URES_LIMIT] = {
	URES_STRING,
	URES_BINARY,
	URES_TABLE,
	URES_ALIAS,
 ...
 */
bool isNoInheritanceMarker(const ResourceData * pResData, Resource res) {
	uint32_t offset = RES_GET_OFFSET(res);
	if(offset == 0) {
		// empty string
	}
...
bool ResourceDataValue::isNoInheritanceMarker() const {
...
U_CAPI Resource U_EXPORT2 res_getTableItemByIndex(const ResourceData * pResData, Resource table,
    int32_t indexR, const char ** key) {
	uint32_t offset = RES_GET_OFFSET(table);
	int32_t length;
	if(indexR < 0) {
...
bool icu::ResourceTable::getKeyAndValue(int32_t i, const char *&key, icu::ResourceValue &value) const 
{
	if(0 <= i && i < length) {
...
bool icu::ResourceTable::findValue(const char * key, ResourceValue &value) const {
	icu::ResourceDataValue &rdValue = static_cast<icu::ResourceDataValue &>(value);
...
bool icu::ResourceArray::getValue(int32_t i, icu::ResourceValue &value) const {
	if(0 <= i && i < length) {
		icu::ResourceDataValue &rdValue = static_cast<icu::ResourceDataValue &>(value);
		// Note: the ResourceTracer keeps a reference to the field of this
