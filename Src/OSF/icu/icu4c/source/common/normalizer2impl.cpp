...
bool ReorderingBuffer::append(const char16_t * s, int32_t length, bool isNFD,
...
bool ReorderingBuffer::appendZeroCC(const char16_t * s, const char16_t * sLimit, UErrorCode & errorCode) {
	if(s==sLimit) {
		return TRUE;
	}
...
void Normalizer2Impl::init(const int32_t * inIndexes, const UCPTrie * inTrie,
	...
	minYesNo = static_cast<uint16>(inIndexes[IX_MIN_YES_NO]);
	minYesNoMappingsOnly = static_cast<uint16>(inIndexes[IX_MIN_YES_NO_MAPPINGS_ONLY]);
	minNoNo = static_cast<uint16>(inIndexes[IX_MIN_NO_NO]);
	minNoNoCompBoundaryBefore = static_cast<uint16>(inIndexes[IX_MIN_NO_NO_COMP_BOUNDARY_BEFORE]);
	minNoNoCompNoMaybeCC = static_cast<uint16>(inIndexes[IX_MIN_NO_NO_COMP_NO_MAYBE_CC]);
...
const char16_t * Normalizer2Impl::copyLowPrefixFromNulTerminated(const char16_t * src,
...
void Normalizer2Impl::decompose(const char16_t * src, const char16_t * limit,
    UnicodeString & dest,
...
const char16_t * Normalizer2Impl::decompose(const char16_t * src, const char16_t * limit, ReorderingBuffer * buffer, UErrorCode & errorCode) const 
{
...
bool Normalizer2Impl::decompose(UChar32 c, uint16 norm16,
    ReorderingBuffer &buffer,
    UErrorCode & errorCode) const {
...
const char16_t * Normalizer2Impl::getDecomposition(UChar32 c, char16_t buffer[4], int32_t &length) const {
	uint16 norm16;
...
const char16_t * Normalizer2Impl::getRawDecomposition(UChar32 c, char16_t buffer[30], int32_t &length) const {
	uint16 norm16;
	...
	if(firstUnit&MAPPING_HAS_RAW_MAPPING) {
		...
		const uint16 * rawMapping = mapping-((firstUnit>>7)&1)-1;
		uint16 rm0 = *rawMapping;
...
bool Normalizer2Impl::norm16HasDecompBoundaryBefore(uint16 norm16) const {
...
bool Normalizer2Impl::norm16HasDecompBoundaryAfter(uint16 norm16) const {
 ...
 */
int32_t Normalizer2Impl::combine(const uint16 * list, UChar32 trail) 
{
	uint16 key1, firstUnit;
	if(trail<COMP_1_TRAIL_LIMIT) {
		...
		key1 = (uint16)(COMP_1_TRAIL_LIMIT+
		    (((trail>>COMP_1_TRAIL_SHIFT))&
		    ~COMP_1_TRIPLE));
		uint16 key2 = (uint16)(trail<<COMP_2_TRAIL_SHIFT);
		uint16 secondUnit;
		for(;;) {
 ...
 */
void Normalizer2Impl::addComposites(const uint16 * list, UnicodeSet & set) const 
{
	uint16 firstUnit;
			...
			if(isJamoVT(norm16)) {
				...
				if(c<Hangul::JAMO_T_BASE) {
					...
					if(prev<Hangul::JAMO_L_COUNT) {
						pRemove = p-1;
						char16_t syllable = (char16_t)(Hangul::HANGUL_BASE + (prev*Hangul::JAMO_V_COUNT+(c-Hangul::JAMO_V_BASE)) * Hangul::JAMO_T_COUNT);
						char16_t t;
...
UChar32 Normalizer2Impl::composePair(UChar32 a, UChar32 b) const {
	uint16 norm16 = getNorm16(a); // maps an out-of-range 'a' to inert norm16
	const uint16 * list;
	if(isInert(norm16)) {
		return U_SENTINEL;
	}
...
bool Normalizer2Impl::compose(const char16_t * src, const char16_t * limit,
    bool onlyContiguous,
    bool doCompose,
    ReorderingBuffer &buffer,
    UErrorCode & errorCode) const {
...
const char16_t * Normalizer2Impl::composeQuickCheck(const char16_t * src, const char16_t * limit,
    bool onlyContiguous,
...
void Normalizer2Impl::composeAndAppend(const char16_t * src, const char16_t * limit,
	...
	if(!buffer.isEmpty()) {
		...
		if(src!=firstStarterInSrc) {
			const char16_t * lastStarterInDest = findPreviousCompBoundary(buffer.getStart(), buffer.getLimit(), onlyContiguous);
		...
		if(!isMaybeOrNonZeroCC(norm16)) { // minNoNo <= norm16 < minMaybeYes
			...
			if(isDecompNoAlgorithmic(norm16)) {
				...
				if(norm16HasCompBoundaryAfter(norm16, onlyContiguous) ||
					...
					const uint16 * mapping = getMapping(norm16);
					int32_t length = *mapping++ & MAPPING_LENGTH_MASK;
					if(!ByteSinkUtil::appendChange(prevSrc, src, (const char16_t *)mapping, length, *sink, edits, errorCode)) {
						break;
					}
					prevBoundary = src;
					continue;
...
uint8 Normalizer2Impl::getPreviousTrailCC(const char16_t * start, const char16_t * p) const {
	if(start == p) {
		return 0;
	}
...
uint16 Normalizer2Impl::getFCD16FromNormData(UChar32 c) const {
	...
	if(norm16 >= limitNoNo) {
		if(norm16>=MIN_NORMAL_MAYBE_YES) {
			...
			uint16 deltaTrailCC = norm16 & DELTA_TCCC_MASK;
...
const char16_t * Normalizer2Impl::makeFCD(const char16_t * src, const char16_t * limit, ReorderingBuffer * buffer, UErrorCode & errorCode) const 
{
...
void Normalizer2Impl::makeFCDAndAppend(const char16_t * src, const char16_t * limit, bool doMakeFCD,
    UnicodeString & safeMiddle, ReorderingBuffer &buffer, UErrorCode & errorCode) const 
{
	if(!buffer.isEmpty()) {
		...
		if(src!=firstBoundaryInSrc) {
			const char16_t * lastBoundaryInDest = findPreviousFCDBoundary(buffer.getStart(), buffer.getLimit());
...
void Normalizer2Impl::makeCanonIterDataFromNorm16(UChar32 start, UChar32 end, const uint16 norm16,
	...
	for(UChar32 c = start; c<=end; ++c) {
		...
		if(isMaybeOrNonZeroCC(norm16)) {
			...
			if(norm16_2 > minYesNo) {
				// c decomposes, get everything from the variable-length extra data
				const uint16 * mapping = getMapping(norm16_2);
				uint16 firstUnit = *mapping;
				int32_t length = firstUnit&MAPPING_LENGTH_MASK;
