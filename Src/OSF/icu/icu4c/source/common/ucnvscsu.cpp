		...
		while(source<sourceLimit) {
			...
			switch(state) {
				case readCommand:
				    ...
				    if((1UL<<b)&0x2601 /* binary 0010 0110 0000 0001, check for b==0xd || b==0xa || b==9
				 || b==0 */                                                               ) {
					    /* CR/LF/TAB/NUL */
					    *target++ = (UChar)b;
					    if(offsets) {
...
static bool isInOffsetWindowOrDirect(uint32_t offset, uint32_t c) {
	return (bool)(c<=offset+0x7f &&
	       (c>=offset || (c<=0x7f &&
	       (c>=0x20 || (1UL<<c)&0x2601))));
...
getTrailSingle:
				...
				if((window = getWindow(scsu->fromUDynamicOffsets, c))>=0) {
					/* there is a window defined that contains this character - switch to it or
					   quote from it? */
...
static UConverter * U_CALLCONV _SCSUSafeClone(const UConverter * cnv,
    void * stackBuffer,
    int32_t * pBufferSize,
    UErrorCode * status)
{
