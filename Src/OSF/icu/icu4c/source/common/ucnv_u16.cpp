...
static void U_CALLCONV _UTF16BEReset(UConverter * cnv, UConverterResetChoice choice) {
...
static void U_CALLCONV _UTF16BEOpen(UConverter * cnv,
...
static const char * U_CALLCONV _UTF16BEGetName(const UConverter * cnv) {
	if(UCNV_GET_VERSION(cnv)==0) {
		return "UTF-16BE";
	}
	else {
...
static void U_CALLCONV _UTF16LEOpen(UConverter * cnv,
    UConverterLoadArgs * pArgs,
    UErrorCode * pErrorCode) {
	(void)pArgs;
	if(UCNV_GET_VERSION(cnv)<=1) {
...
static const char * U_CALLCONV _UTF16LEGetName(const UConverter * cnv) {
	if(UCNV_GET_VERSION(cnv)==0) {
		return "UTF-16LE";
	}
	else {
 ...
 */
U_CDECL_BEGIN
static void U_CALLCONV _UTF16Reset(UConverter * cnv, UConverterResetChoice choice) {
	if(choice<=UCNV_RESET_TO_UNICODE) {
...
U_CDECL_BEGIN
static void U_CALLCONV _UTF16Open(UConverter * cnv,
    UConverterLoadArgs * pArgs,
    UErrorCode * pErrorCode) {
...
static UChar32 U_CALLCONV _UTF16GetNextUChar(UConverterToUnicodeArgs * pArgs,
    UErrorCode * pErrorCode) {
