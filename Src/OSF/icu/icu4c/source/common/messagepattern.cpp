...
template <typename T, int32_t stackCapacity> void MessagePatternList<T, stackCapacity>::copyFrom(
	const MessagePatternList<T, stackCapacity> &other, int32_t length, UErrorCode & errorCode) 
{
...
template <typename T, int32_t stackCapacity>
bool MessagePatternList<T, stackCapacity>::ensureCapacityForOneMore(int32_t oldLength, UErrorCode & errorCode) {
...
bool MessagePattern::init(UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return FALSE;
	}
...
bool MessagePattern::copyStorage(const MessagePattern &other, UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return FALSE;
	}
	parts = NULL;
					...
					for(;;) {
						...
						if(index>=0) {
							if(/*(index+1)<msg.length() &&*/ msg.charAt(index+1)==u_apos) {
								...
								// still encodes a single apostrophe, skip the second
								// one
								addPart(UMSGPAT_PART_TYPE_SKIP_SYNTAX, ++index, 1, 0, errorCode);
							}
							else {
...
int32_t MessagePattern::parsePluralOrSelectStyle(UMessagePatternArgType argType, int32_t index, int32_t nestingLevel,
    UParseError * parseError, UErrorCode & errorCode) 
{
	if(U_FAILURE(errorCode)) {
		return 0;
...
bool MessagePattern::isArgTypeChar(UChar32 c) {
	return (u_a<=c && c<=u_z) || (u_A<=c && c<=u_Z);
}

bool MessagePattern::isChoice(int32_t index) {
	char16_t c;
	return
...
bool MessagePattern::isPlural(int32_t index) {
	char16_t c;
	return
...
bool MessagePattern::isSelect(int32_t index) {
	char16_t c;
	return
