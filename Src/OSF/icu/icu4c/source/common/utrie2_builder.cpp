...
typedef struct NewTrieAndStatus {
	...
	bool exclusiveLimit; /* rather than inclusive range end */
} NewTrieAndStatus;

static bool U_CALLCONV copyEnumRange(const void * context, UChar32 start, UChar32 end, uint32_t value) {
...
static inline bool isInNullBlock(UNewTrie2 * trie, UChar32 c, bool forLSCP) {
	int32_t i2, block;
	...
	return (bool)(block==trie->dataNullOffset);
}
...
static int32_t getIndex2Block(UNewTrie2 * trie, UChar32 c, bool forLSCP) {
	int32_t i1, i2;
...
static inline bool isWritableBlock(UNewTrie2 * trie, int32_t block) {
	return (bool)(block!=trie->dataNullOffset && 1==trie->map[block>>UTRIE2_SHIFT_2]);
}
 ...
 * @internal
 */
static int32_t getDataBlock(UNewTrie2 * trie, UChar32 c, bool forLSCP) {
 ...
 */
static void set32(UNewTrie2 * trie,
    UChar32 c, bool forLSCP, uint32_t value,
...
U_CAPI void U_EXPORT2 utrie2_set32ForLeadSurrogateCodeUnit(UTrie2 * trie,
 ...
 * @internal
 */
static void fillBlock(uint32_t * block, UChar32 start, UChar32 limit,
...
U_CAPI void U_EXPORT2 utrie2_setRange32(UTrie2 * trie,
    UChar32 start, UChar32 end,
    uint32_t value, bool overwrite,
    UErrorCode * pErrorCode) {
	/*
...
static inline bool equal_int32(const int32_t * s, const int32_t * t, int32_t length) {
	while(length>0 && *s==*t) {
		++s;
		++t;
		--length;
