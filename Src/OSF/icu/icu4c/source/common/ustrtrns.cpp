...
U_CAPI char16_t * U_EXPORT2 u_strFromUTF32WithSub(char16_t * dest, int32_t destCapacity, int32_t * pDestLength,
    const UChar32 * src, int32_t srcLength, UChar32 subchar, int32_t * pNumSubstitutions, UErrorCode * pErrorCode) 
{
	...
	ASSIGN_PTR(pNumSubstitutions, numSubstitutions);
	/* Terminate the buffer */
	u_terminateUChars(dest, destCapacity, reqLength, pErrorCode);
	return dest;
}

U_CAPI char16_t * U_EXPORT2 u_strFromUTF32(char16_t * dest, int32_t destCapacity, int32_t * pDestLength,
    const UChar32 * src, int32_t srcLength, UErrorCode * pErrorCode) 
{
	return u_strFromUTF32WithSub(dest, destCapacity, pDestLength, src, srcLength, U_SENTINEL, NULL, pErrorCode);
}

U_CAPI UChar32* U_EXPORT2 u_strToUTF32WithSub(UChar32 * dest, int32_t destCapacity, int32_t * pDestLength,
    const char16_t * src, int32_t srcLength, UChar32 subchar, int32_t * pNumSubstitutions, UErrorCode * pErrorCode) 
{
...
U_CAPI UChar32* U_EXPORT2 u_strToUTF32(UChar32 * dest, int32_t destCapacity, int32_t * pDestLength,
    const char16_t * src, int32_t srcLength, UErrorCode * pErrorCode) 
{
	return u_strToUTF32WithSub(dest, destCapacity, pDestLength, src, srcLength, U_SENTINEL, NULL, pErrorCode);
}

U_CAPI char16_t * U_EXPORT2 u_strFromUTF8WithSub(char16_t * dest, int32_t destCapacity, int32_t * pDestLength,
    const char * src, int32_t srcLength, UChar32 subchar, int32_t * pNumSubstitutions, UErrorCode * pErrorCode) 
{
	...
	if(srcLength < 0) {
		...
		int32_t i;
		UChar32 c;
		for(i = 0; (c = (uint8)src[i]) != 0 && (pDest < pDestLimit);) {
...
U_CAPI char16_t * U_EXPORT2 u_strFromUTF8(char16_t * dest,
			...
			if(ch < 0xc0) {
				...
				*pDest++ = (char16_t)ch;
				...
				if((t1 = pSrc[1]) != 0 && (t2 = pSrc[2]) != 0) {
					/* no need for (ch & 0xf) because the upper bits are truncated after <<12 in the
					   cast to (char16_t) */
					/* 0x2080 = (0x80 << 6) + 0x80 */
...
static inline uint8 * _appendUTF8(uint8 * pDest, UChar32 c) 
{
	...
	if((c)<=0x7f) {
		...
		*pDest++ = (uint8)((c>>6)|0xc0);
		*pDest++ = (uint8)((c&0x3f)|0x80);
	}
	else if(c<=0xffff) {
		*pDest++ = (uint8)((c>>12)|0xe0);
		*pDest++ = (uint8)(((c>>6)&0x3f)|0x80);
		*pDest++ = (uint8)(((c)&0x3f)|0x80);
	}
	else { /* if((uint32_t)(c)<=0x10ffff) */
		*pDest++ = (uint8)(((c)>>18)|0xf0);
		*pDest++ = (uint8)((((c)>>12)&0x3f)|0x80);
		*pDest++ = (uint8)((((c)>>6)&0x3f)|0x80);
		*pDest++ = (uint8)(((c)&0x3f)|0x80);
	}
	return pDest;
...
U_CAPI char * U_EXPORT2 u_strToUTF8WithSub(char * dest, int32_t destCapacity, int32_t * pDestLength, const char16_t * pSrc,
    int32_t srcLength, UChar32 subchar, int32_t * pNumSubstitutions, UErrorCode * pErrorCode) 
{
	...
	if(srcLength == -1) {
		while((ch = *pSrc)!=0) {
			...
			if(ch <= 0x7f) {
				...
				if((pDestLimit - pDest) >= 2) {
					*pDest++ = (uint8)((ch>>6)|0xc0);
					*pDest++ = (uint8)((ch&0x3f)|0x80);
				}
				else {
					reqLength = 2;
					break;
				...
				if((pDestLimit - pDest) >= 3) {
					*pDest++ = (uint8)((ch>>12)|0xe0);
					*pDest++ = (uint8)(((ch>>6)&0x3f)|0x80);
					*pDest++ = (uint8)((ch&0x3f)|0x80);
				}
				else {
					reqLength = 3;
		...
		const char16_t * pSrcLimit = (pSrc != NULL) ? (pSrc+srcLength) : NULL;
		int32_t count;
	...
	reqLength += (int32_t)(pDest - (uint8 *)dest);
	ASSIGN_PTR(pNumSubstitutions, numSubstitutions);
	ASSIGN_PTR(pDestLength, reqLength);
	/* Terminate the buffer */
...
U_CAPI char * U_EXPORT2 u_strToUTF8(char * dest, int32_t destCapacity, int32_t * pDestLength, const char16_t * pSrc, int32_t srcLength, UErrorCode * pErrorCode) 
{
    ...
    int32_t srcLength, UChar32 subchar, int32_t * pNumSubstitutions, UErrorCode * pErrorCode) 
{
	...
	if(srcLength < 0) {
		 ...
		 */
		UChar32 c;
		while(((c = (uint8)*src) != 0) && c <= 0x7f && (pDest < pDestLimit)) {
			*pDest++ = (char16_t)c;
			++src;
		}
		if(!c) {
				...
				*pDest++ = (char16_t)ch;
				...
				if(ch >= 0xe0) {
					if( /* handle U+0000..U+FFFF inline */
						ch <= 0xef && (t1 = (uint8)(src[i] - 0x80)) <= 0x3f && (t2 = (uint8)(src[i+1] - 0x80)) <= 0x3f) {
					...
					if( /* handle U+0000..U+07FF inline */
						ch >= 0xc0 && (t1 = (uint8)(src[i] - 0x80)) <= 0x3f) {
				...
				if(subchar < 0) {
					*pErrorCode = U_INVALID_CHAR_FOUND;
					...
					/*
					 * We need to write two UChars, adjusted count for that, and ran out of space.
					 */
					--i; // back out byte ch
					break;
				}
	...
	while(i < srcLength && (pDest < pDestLimit)) {
		...
		if(U8_IS_SINGLE(ch)) {
			*pDest++ = (char16_t)ch;
			...
			if(ch >= 0xe0) {
				if( /* handle U+0000..U+FFFF inline */
					ch <= 0xef && (i+1) < srcLength && (t1 = (uint8)(src[i] - 0x80)) <= 0x3f && (t2 = (uint8)(src[i+1] - 0x80)) <= 0x3f) {
					// no need for (ch & 0xf) because the upper bits are truncated after <<12 in the cast to (char16_t)
					*pDest++ = (char16_t)((ch << 12) | (t1 << 6) | t2);
					i += 2;
					continue;
				...
				if( /* handle U+0000..U+07FF inline */
					ch >= 0xc0 && i < srcLength && (t1 = (uint8)(src[i] - 0x80)) <= 0x3f) {
					*pDest++ = (char16_t)(((ch & 0x1f) << 6) | t1);
					++i;
					continue;
				}
	...
	while(i < srcLength) {
		...
		if(U8_IS_SINGLE(ch)) {
			...
			if(ch >= 0xe0) {
				if( /* handle U+0000..U+FFFF inline */
					ch <= 0xef && (i+1) < srcLength && (uint8)(src[i] - 0x80) <= 0x3f && (uint8)(src[i+1] - 0x80) <= 0x3f) {
					reqLength++;
					i += 2;
					continue;
				}
				...
				if( /* handle U+0000..U+07FF inline */
					ch >= 0xc0 && i < srcLength && (uint8)(src[i] - 0x80) <= 0x3f) {
					reqLength++;
					++i;
					continue;
				}
...
U_CAPI char * U_EXPORT2 u_strToJavaModifiedUTF8(char * dest,
	...
	uint32_t ch = 0;
	uint8 * pDest = (uint8 *)dest;
	uint8 * pDestLimit = pDest + destCapacity;
