...
static bool gDefaultConverterContainsOption;
...
static bool U_CALLCONV isCnvAcceptable(void * /*context*/,
 ...
 */
static bool ucnv_deleteSharedConverterData(UConverterSharedData * deadSharedData)
{
 ...
 */
static void parseConverterOptions(const char * inName,
	...
	while((c = *inName)!=0) {
		...
		if(uprv_strncmp(inName, "locale=", 7)==0) {
			...
			*dest = 0;
			...
			/* add processing for new options here with another } else if(uprv_strncmp(inName,
			   "option-name=", XX)==0) { */
		}
		else {
 ...
 */
U_CFUNC UConverterSharedData * ucnv_loadSharedData(const char * converterName,
    UConverterNamePieces * pPieces,
    UConverterLoadArgs * pArgs,
    UErrorCode * err) {
	...
	bool mayContainOption = TRUE;
	bool checkForAlgorithmic = TRUE;
	...
	if(pArgs == NULL) {
		memzero(&stackArgs, sizeof(stackArgs));
...
U_CFUNC bool ucnv_canCreateConverter(const char * converterName, UErrorCode * err) {
    ...
    UConverterLoadArgs * pArgs, UErrorCode * err)
{
	...
	memzero(myUConverter, sizeof(UConverter));
...
U_CAPI int32_t U_EXPORT2 ucnv_swap(const UDataSwapper * ds,
	...
	if(inStaticData->conversionType==UCNV_MBCS) {
		...
		if(inMBCSHeader->version[0]==4 && inMBCSHeader->version[1]>=1) {
			...
			noFromU = (bool)((mbcsHeader.options&MBCS_OPT_NO_FROM_U)!=0);
		}
		else {
