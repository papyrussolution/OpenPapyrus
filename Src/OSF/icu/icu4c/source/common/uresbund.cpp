...
static bool U_CALLCONV compareEntries(const UHashTok p1, const UHashTok p2) 
{
	...
	return (bool)(uhash_compareChars(name1, name2) && uhash_compareChars(path1, path2));
}
 ...
 */
static bool mayHaveParent(char * name) { return (name[0] != 0 && uprv_strstr("nb nn", name) != nullptr); }
/**
 *  Internal function
 */
 ...
 */
static UResourceDataEntry * getFallbackData(const UResourceBundle * resBundle, const char ** resTag, Resource * res, UErrorCode * status) 
{
	...
	*res = RES_BOGUS;
	...
	if(i>1) {
		if(strcmp(dataEntry->fName, uloc_getDefault())==0 || strcmp(dataEntry->fName, kRootLocaleName)==0) {
...
#ifdef URES_DEBUG
	U_CAPI bool U_EXPORT2 ures_dumpCacheContents() 
	{
		bool cacheNotEmpty = FALSE;
...
static void initCache(UErrorCode * status) { umtx_initOnce(gCacheInitOnce, &createCache, *status); }
...
static UResourceDataEntry * getPoolEntry(const char * path, UErrorCode * status);
...
static UResourceDataEntry * init_entry(const char * localeID, const char * path, UErrorCode * status) 
{
	...
	if(!r) {
		...
		memzero(r, sizeof(UResourceDataEntry));
			...
			if(r->fData.usesPoolBundle) {
				...
				if(U_SUCCESS(*status)) {
					...
					if(r->fData.pRoot[1 + URES_INDEX_POOL_CHECKSUM] == poolIndexes[URES_INDEX_POOL_CHECKSUM]) {
						r->fData.poolBundleKeys =
						    (const char *)(poolIndexes + (poolIndexes[URES_INDEX_LENGTH] & 0xff));
...
/* INTERNAL: */
/* CAUTION:  resbMutex must be locked when calling this function! */
static UResourceDataEntry * findFirstExisting(const char * path, char * name, const char * defaultLocale, bool * isRoot, bool * hasChopped, bool * isDefault, UErrorCode * status) 
{
	UResourceDataEntry * r = NULL;
	...
	*hasChopped = TRUE; /* we're starting with a fresh name */
	while(*hasChopped && !hasRealData) {
		...
		*isDefault = (bool)(uprv_strncmp(name, defaultLocale, strlen(name)) == 0);
		hasRealData = (bool)(r->fBogus == U_ZERO_ERROR);
		if(!hasRealData) {
			...
			*status = U_USING_FALLBACK_WARNING;
			...
			strcpy(name, r->fName); /* this is needed for supporting aliases */
		}

		*isRoot = (bool)(strcmp(name, kRootLocaleName) == 0);
...
static void ures_setIsStackObject(UResourceBundle * resB, bool state) 
{
	if(state) {
		resB->fMagic1 = 0;
		resB->fMagic2 = 0;
...
{
	memzero(resB, sizeof(UResourceBundle));
	ures_setIsStackObject(resB, TRUE);
}

U_NAMESPACE_BEGIN
	StackUResourceBundle::StackUResourceBundle() { ures_initStackObject(&bundle); }
	StackUResourceBundle::~StackUResourceBundle() { ures_close(&bundle); }
U_NAMESPACE_END
...
static UResourceDataEntry * entryOpen(const char * path, const char * localeID, UResOpenType openType, UErrorCode * status) 
{
	...
	bool usingUSRData = U_USE_USRDATA && ( path == NULL || uprv_strncmp(path, U_ICUDATA_NAME, 8) == 0);
	...
	if(usingUSRData) {
		if(path == NULL) {
			strcpy(usrDataPath, U_USRDATA_NAME);
		}
		else {
	...
	if(r) { /* if there is one real locale, we can look for parents. */
		...
		if((hasChopped || mayHaveParent(name)) && !isRoot) {
			if(!loadParentsExceptRoot(t1, name, SIZEOFARRAYi(name), usingUSRData, usrDataPath, status)) {
				goto finish;
			}
		}
	}
...
static UResourceDataEntry * entryOpenDirect(const char * path, const char * localeID, UErrorCode * status) 
{
	...
	if(r && strcmp(localeID, kRootLocaleName) != 0 && // not root
	    r->fParent == NULL && !r->fData.noFallback && strlen(localeID) < ULOC_FULLNAME_CAPACITY) {
		char name[ULOC_FULLNAME_CAPACITY];
		strcpy(name, localeID);
		if(!chopLocale(name) || strcmp(name, kRootLocaleName) == 0 ||
		    loadParentsExceptRoot(t1, name, SIZEOFARRAYi(name), FALSE, NULL, status)) {
			if(strcmp(t1->fName, kRootLocaleName) != 0 && t1->fParent == NULL) {
...
U_CAPI void U_EXPORT2 ures_close(UResourceBundle * resB)
{
	ures_closeBundle(resB, TRUE);
}

namespace {
UResourceBundle * init_resb_result(UResourceDataEntry * dataEntry, Resource r, const char * key, int32_t idx,
    UResourceDataEntry * validLocaleDataEntry, const char * containerResPath, int32_t recursionDepth, UResourceBundle * resB, UErrorCode * status);
    ...
    int32_t recursionDepth, UResourceBundle * resB, UErrorCode * status) 
{
	...
	if(chAlias[0] == RES_PATH_SEPARATOR) {
		...
		if(strcmp(path, "LOCALE") == 0) {
			...
			if(strcmp(path, "ICUDATA") == 0) { /* want ICU data */
				path = nullptr;
			}
			if(sep == nullptr) {
	...
	if(keyPath == nullptr) {
		...
		for(;;) {
			...
			while(*myPath && U_SUCCESS(*status)) {
				...
				if(temp == nullptr || strcmp(keyPath, temp) != 0) {
					...
					ures_freeResPath(resB);
					ures_appendResPath(resB, keyPath, (int32_t)strlen(keyPath), status);
    ...
    UResourceDataEntry * validLocaleDataEntry, const char * containerResPath, int32_t recursionDepth, UResourceBundle * resB, UErrorCode * status) 
{
	...
	if(containerResPath != resB->fResPath) {
		ures_appendResPath(
			resB, containerResPath, static_cast<int32_t>(strlen(containerResPath)), status);
	}
	if(key) {
	...
	/* Make sure that Purify doesn't complain about uninitialized memory copies. */
	{
		...
		memzero(resB->fResBuf + usedLen, sizeof(resB->fResBuf) - usedLen);
	}
	resB->fVersion = NULL;
	resB->fRes = r;
...
UResourceBundle * init_resb_result(UResourceDataEntry * dataEntry, Resource r, const char * key, int32_t idx,
    ...
    const UResourceBundle * container, UResourceBundle * resB, UErrorCode * status) 
{
	return init_resb_result(dataEntry, r, key, idx, container->fValidLocaleDataEntry, container->fResPath, 0, resB, status);
}
}  // namespace
...
U_CAPI const uint8 * U_EXPORT2 ures_getBinary(const UResourceBundle * resB, int32_t* len, UErrorCode *   status) 
{
	const uint8 * p;
	if(!status || U_FAILURE(*status)) {
		return NULL;
...
U_CAPI const int32_t* U_EXPORT2 ures_getIntVector(const UResourceBundle * resB, int32_t* len, UErrorCode *   status) 
{
	const int32_t * p;
	if(!status || U_FAILURE(*status)) {
		return NULL;
...
{
	return resB ? res_getPublicType(resB->fRes) : URES_NONE;
}
...
U_CAPI bool U_EXPORT2 ures_hasNext(const UResourceBundle * resB) 
{
...
U_CAPI UResourceBundle * U_EXPORT2 ures_findResource(const char * path, UResourceBundle * fillIn, UErrorCode * status)
{
...
U_CAPI UResourceBundle * U_EXPORT2 ures_findSubResource(const UResourceBundle * resB, char * path, UResourceBundle * fillIn, UErrorCode * status)
{
...
static void createPath(const char * origResPath, int32_t origResPathLen, const char * resPath, int32_t resPathLen, const char * inKey, CharString& path, UErrorCode * status) 
{
	...
	if(resPathLen > 0) {
		...
		if(U_SUCCESS(*status)) {
			...
			// Remove from the beginning of resPath the number of segments that are contained in
			// origResPath.
		...
		// Or, to put it another way, calculate <#-segments-in-key> - (<#-segments-in-resPath> -
		// <#-segments-in-origResPath>),
		...
		if(res == RES_BOGUS) {
			...
			while(res == RES_BOGUS && (dataEntry->fParent || !didRootOnce)) { /* Otherwise, we'll look in parents */
				if(dataEntry->fParent) {
					...
					// We can't just stop when we get to a bundle whose fParent is NULL.  That'll
					// work most of the time,
					// but if the bundle that the caller passed to us was "root" (which happens in
					// getAllItemsWithFallback(),
					// this function will drop right out without doing anything if "root" doesn't
					// contain the exact key path
					// specified.  In that case, we need one extra time through this loop to make
					// sure we follow any
				...
				if(dataEntry->fBogus == U_ZERO_ERROR) {
					...
					do {
						...
						if(RES_GET_TYPE(res) == URES_ALIAS && *myPath) {
							...
							/*helper = init_resb_result(dataEntry, res, inKey, -1, resB,
							   helper, status);*/
							if(helper) {
			...
			if(res != RES_BOGUS) {
				...
				if(strcmp(dataEntry->fName,
				    uloc_getDefault())==0 || strcmp(dataEntry->fName, kRootLocaleName)==0) {
					*status = U_USING_DEFAULT_WARNING;
				}
				else {
					*status = U_USING_FALLBACK_WARNING;
...
namespace {
void getAllItemsWithFallback(const UResourceBundle * bundle, ResourceDataValue &value, ResourceSink &sink, UErrorCode & errorCode) 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
	...
	bool hasParent = parentEntry != NULL && U_SUCCESS(parentEntry->fBogus);
	...
	if(hasParent) {
		...
		if(bundle->fResPath == NULL || *bundle->fResPath == 0) {
			...
			rb = ures_getByKeyWithFallback(parentBundle.getAlias(), bundle->fResPath, containerBundle.getAlias(), &pathErrorCode);
		}
...
struct GetAllChildrenSink : public ResourceSink {
	...
	GetAllChildrenSink(ResourceSink& dest) : dest(dest) 
	{
	}
		...
		for(int32_t i = 0; itemsTable.getKeyAndValue(i, key, value); ++i) {
			if(value.getType() == URES_ALIAS) {
				...
				UResourceBundle * aliasRB = getAliasTargetAsResourceBundle(rdv.getData(), rdv.getResource(), nullptr, -1,
					rdv.getValidLocaleDataEntry(), nullptr, 0, stackTempBundle.getAlias(), &errorCode);
...
U_CAPI void U_EXPORT2 ures_getAllChildrenWithFallback(const UResourceBundle * bundle, const char * path, icu::ResourceSink &sink, UErrorCode & errorCode) 
{
...
U_CAPI void U_EXPORT2 ures_getValueWithFallback(const UResourceBundle * bundle, const char * path, UResourceBundle * tempFillIn, ResourceDataValue &value, UErrorCode & errorCode) 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
...
U_CAPI void U_EXPORT2 ures_getAllItemsWithFallback(const UResourceBundle * bundle, const char * path, icu::ResourceSink &sink, UErrorCode & errorCode) 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
 ...
 */
U_CAPI const char * U_EXPORT2 ures_getLocaleInternal(const UResourceBundle * resourceBundle, UErrorCode * status)
{
	if(!status || U_FAILURE(*status)) {
		return NULL;
	}
...
U_CFUNC const char * ures_getName(const UResourceBundle * resB) { return resB ? resB->fData->fName : NULL; }

#ifdef URES_DEBUG
U_CFUNC const char * ures_getPath(const UResourceBundle * resB) { return resB ? resB->fData->fPath : NULL; }
#endif
...
static UResourceBundle * ures_openWithType(UResourceBundle * r, const char * path, const char * localeID, UResOpenType openType, UErrorCode * status) 
{
	...
	if(openType != URES_OPEN_DIRECT) {
		...
		uloc_getBaseName(localeID, canonLocaleID, SIZEOFARRAYi(canonLocaleID), status);
	...
	memzero(r, sizeof(UResourceBundle));
	ures_setIsStackObject(r, isStackObject);
...
U_CAPI UResourceBundle * U_EXPORT2 ures_open(const char * path, const char * localeID, UErrorCode * status) 
{
	return ures_openWithType(NULL, path, localeID, URES_OPEN_LOCALE_DEFAULT_ROOT, status);
}

U_CAPI UResourceBundle * U_EXPORT2 ures_openNoDefault(const char * path, const char * localeID, UErrorCode * status) 
{
 ...
 */
U_CAPI UResourceBundle *  U_EXPORT2 ures_openDirect(const char * path, const char * localeID, UErrorCode * status) 
{
 ...
 */
U_CAPI void U_EXPORT2 ures_openDirectFillIn(UResourceBundle * r, const char * path, const char * localeID, UErrorCode * status) 
{
	if(U_SUCCESS(*status) && r == NULL) {
		*status = U_ILLEGAL_ARGUMENT_ERROR;
		return;
 ...
 */
U_CAPI int32_t U_EXPORT2 ures_countArrayItems(const UResourceBundle * resourceBundle, const char * resourceKey, UErrorCode * status)
{
 ...
 * @internal
 */
U_CAPI const char * U_EXPORT2 ures_getVersionNumberInternal(const UResourceBundle * resourceBundle)
{
	if(!resourceBundle) 
		return NULL;
	if(resourceBundle->fVersion == NULL) {
		...
		if(minor_len > 0) {
			...
			strcpy(resourceBundle->fVersion, kDefaultMinorVersion);
		}
	}
...
U_CAPI UEnumeration* U_EXPORT2 ures_openAvailableLocales(const char * path, UErrorCode * status)
{
...
#if defined(URES_TREE_DEBUG)
		slfprintf_stderr("%s open failed - %s\n", path, u_errorName(*status));
#endif
...
U_CAPI int32_t U_EXPORT2 ures_getFunctionalEquivalent(char * result, int32_t resultCapacity,
    const char * path, const char * resName, const char * keyword, const char * locid,
    bool * isAvailable, bool omitDefault, UErrorCode * status)
{
...
#if defined(URES_TREE_DEBUG)
	slfprintf_stderr("getFunctionalEquivalent: \"%s\" [%s=%s] in %s - %s\n",
	...
	do {
		...
		if(((subStatus == U_USING_FALLBACK_WARNING) || (subStatus == U_USING_DEFAULT_WARNING)) && isAvailable) {
			*isAvailable = FALSE;
		}
		...
		subStatus = U_ZERO_ERROR;
		if(res) {
			strcpy(found, ures_getLocaleByType(res, ULOC_VALID_LOCALE, &subStatus));
		}
		uloc_getParent(found, parent, sizeof(parent), &subStatus);
		ures_close(res);
	} while(!defVal[0] && *found && strcmp(found, "root") != 0 && U_SUCCESS(*status));
...
/**/ slfprintf_stderr("@%d [%s] %s\n", __LINE__, kwVal, u_errorName(subStatus));
				...
				if(subStatus == U_ZERO_ERROR) {
					...
					if(strlen(defLoc) > strlen(full)) {
						...
						/* look for default item */
#if defined(URES_TREE_DEBUG)
						slfprintf_stderr("%s;%s -> recalculating Default0\n",
						...
						slfprintf_stderr("No trim0,  %s <= %s\n", defLoc, full);
					}
#endif
				}
				else {
#if defined(URES_TREE_DEBUG)
					slfprintf_stderr("err=%s in %s looking for %s\n", u_errorName(subStatus), parent, kwVal);
#endif
				}
			}
		}
	...
	if((full[0]==0) && strcmp(kwVal, defVal)) {
#if defined(URES_TREE_DEBUG)
		slfprintf_stderr("Failed to locate kw %s - try default %s\n", kwVal, defVal);
#endif
		strcpy(kwVal, defVal);
		strcpy(parent, base);
		strcpy(found, base);
		do { /* search for 'default' named item */
			...
			isAvailable = NULL; /* only want to set this the first time around */
#if defined(URES_TREE_DEBUG)
			slfprintf_stderr("%s;%s -> %s (looking for default %s)\n", path ? path : "ICUDATA", parent, u_errorName(subStatus), kwVal);
#endif
			if(U_FAILURE(subStatus)) {
				*status = subStatus;
			}
				...
				if(subStatus == U_ZERO_ERROR) {
					...
					if(subStatus == U_ZERO_ERROR) {
#if defined(URES_TREE_DEBUG)
						slfprintf_stderr("%s;%s -> full1 %s=%s,  %s\n", path ? path : "ICUDATA", parent, keyword, kwVal, u_errorName(subStatus));
#endif
						strcpy(full, parent);
						if(*full == 0) {
							strcpy(full, "root");
						...
						if(strlen(defLoc) > strlen(full)) {
							...
							/* look for default item */
#if defined(URES_TREE_DEBUG)
							slfprintf_stderr("%s;%s -> recalculating Default1\n", path ? path : "ICUDATA", full);
#endif
							defUstr = ures_getStringByKey(&bund1, DEFAULT_TAG, &defLen, &subStatus);
							if(U_SUCCESS(subStatus) && defLen) {
								u_UCharsToChars(defUstr, defVal, sstrleni(defUstr));
	...
	if(U_SUCCESS(*status)) {
		if(!full[0]) {
#if defined(URES_TREE_DEBUG)
			slfprintf_stderr("Still could not load keyword %s=%s\n", keyword, kwVal);
#endif
			*status = U_MISSING_RESOURCE_ERROR;
		}
		else if(omitDefault) {
#if defined(URES_TREE_DEBUG)
			slfprintf_stderr("Trim? full=%s, defLoc=%s, found=%s\n", full, defLoc, found);
#endif
			if(strlen(defLoc) <= strlen(full)) {
...
U_CAPI UEnumeration* U_EXPORT2 ures_getKeywordValues(const char * path, const char * keyword, UErrorCode * status)
{
...
#define VALUES_LIST_SIZE 512
	...
	while((locale = uenum_next(locs, &locLen, status)) != 0) {
		...
		if(!bund || U_FAILURE(subStatus)) {
			slfprintf_stderr("%s-%s values: Can't open %s locale - skipping. (%s)\n", path ? path : "<ICUDATA>", keyword, locale, u_errorName(subStatus));
		}
#endif
		ures_getByKey(bund, keyword, &item, &subStatus);
		if(!bund || U_FAILURE(subStatus)) {
#if defined(URES_TREE_DEBUG)
			slfprintf_stderr("%s-%s values: Can't find in %s - skipping. (%s)\n", path ? path : "<ICUDATA>", keyword, locale, u_errorName(subStatus));
#endif
			ures_close(bund);
			bund = NULL;
			continue;
...
#endif
			if(k == NULL || *k == 0 ||
			    strcmp(k, DEFAULT_TAG) == 0 || uprv_strncmp(k, "private-", 8) == 0) {
			...
			if(k) {
				...
				if((valuesCount >= (VALUES_LIST_SIZE-1)) || /* no more space in list .. */
					...
					*status = U_ILLEGAL_ARGUMENT_ERROR; /* out of space.. */
					...
					valuesIndex += kLen;
#if defined(URES_TREE_DEBUG)
					slfprintf_stderr("%s | %s | %s | [%s]   (UNIQUE)\n", path ? path : "<ICUDATA>", keyword, locale, k);
#endif
...
U_CAPI bool U_EXPORT2 ures_equal(const UResourceBundle * res1, const UResourceBundle * res2) 
{
	...
	if(res1->fKey==NULL||  res2->fKey==NULL) {
		...
		if(strcmp(res1->fKey, res2->fKey)!=0) {
			return FALSE;
		}
	}
	if(strcmp(res1->fData->fName, res2->fData->fName)!=0) {
		return FALSE;
	}
	if(res1->fData->fPath == NULL||  res2->fData->fPath==NULL) {
		...
		if(strcmp(res1->fData->fPath, res2->fData->fPath)!=0) {
			return FALSE;
		}
	}
	if(strcmp(res1->fData->fParent->fName, res2->fData->fParent->fName)!=0) {
		return FALSE;
	}
	if(strcmp(res1->fData->fParent->fPath, res2->fData->fParent->fPath)!=0) {
		return FALSE;
	}
...
U_CAPI UResourceBundle * U_EXPORT2 ures_clone(const UResourceBundle * res, UErrorCode * status) 
{
...
U_CAPI const UResourceBundle * U_EXPORT2 ures_getParentBundle(const UResourceBundle * res) 
{
	return res ? res->fParentRes : 0;
}
