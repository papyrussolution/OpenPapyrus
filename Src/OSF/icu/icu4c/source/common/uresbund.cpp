...
static UResourceDataEntry * findFirstExisting(const char * path, char * name, const char * defaultLocale,
	...
	*hasChopped = TRUE; /* we're starting with a fresh name */
	...
	while(*hasChopped && !hasRealData) {
		...
		*isDefault = (bool)(uprv_strncmp(name, defaultLocale, uprv_strlen(name)) == 0);
		...
		*isRoot = (bool)(uprv_strcmp(name, kRootLocaleName) == 0);
...
U_CAPI UResourceBundle * U_EXPORT2 ures_getByIndex(const UResourceBundle * resB,
    int32_t indexR,
	...
	if(URES_IS_TABLE(type)) {
		...
		if(res == RES_BOGUS) {
			...
			if(res != RES_BOGUS) {
				...
				if(uprv_strcmp(dataEntry->fName,
				    uloc_getDefault())==0 || uprv_strcmp(dataEntry->fName, kRootLocaleName)==0) {
					*status = U_USING_DEFAULT_WARNING;
				}
				else {
					*status = U_USING_FALLBACK_WARNING;
