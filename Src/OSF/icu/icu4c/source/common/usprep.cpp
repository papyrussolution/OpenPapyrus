...
static bool U_CALLCONV isSPrepAcceptable(void * /* context */,
    const char * /* type */,
    const char * /* name */,
...
static bool U_CALLCONV compareEntries(const UHashTok p1, const UHashTok p2) {
	...
	return ((bool)(uhash_compareChars(name1, name2) &
...
static bool U_CALLCONV loadData(UStringPrepProfile* profile,
    const char * path,
    const char * name,
    const char * type,
	...
	if(normUniVer < sprepUniVer && /* the Unicode version of SPREP file must be less than the Unicode Version of the
	                                  normalization data */
	    normUniVer < normCorrVer && /* the Unicode version of the NormalizationCorrections.txt file should be less
	                                   than the Unicode Version of the normalization data */
...
U_CAPI int32_t U_EXPORT2 usprep_prepare(const UStringPrepProfile* profile,
	...
	for(int32_t b2Index = 0; b2Index<b2Len;) {
		...
		if(type == USPREP_PROHIBITED ||
		    ((result < _SPREP_TYPE_THRESHOLD) && (result & 0x01) /* first bit says it the code point is
		                                                            prohibited*/                                    )
		    ) {
...
U_CAPI int32_t U_EXPORT2 usprep_swap(const UDataSwapper * ds,
