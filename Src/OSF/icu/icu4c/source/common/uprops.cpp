...
bool U_CALLCONV ulayout_isAcceptable(void * /*context*/,
...
bool ulayout_ensureData(UErrorCode & errorCode) {
	if(U_FAILURE(errorCode)) {
		return FALSE;
	}
...
typedef bool BinaryPropertyContains (const BinaryProperty &prop, UChar32 c, UProperty which);
...
static bool defaultContains(const BinaryProperty &prop, UChar32 c, UProperty /*which*/) {
	/* systematic, directly stored properties */
	return (u_getUnicodeProperties(c, prop.column)&prop.mask)!=0;
}

static bool caseBinaryPropertyContains(const BinaryProperty & /*prop*/, UChar32 c, UProperty which) {
	return static_cast<bool>(ucase_hasBinaryProperty(c, which));
}

static bool isBidiControl(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return ubidi_isBidiControl(c);
}

static bool isMirrored(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return ubidi_isMirrored(c);
}

static bool isJoinControl(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return ubidi_isJoinControl(c);
}

#if UCONFIG_NO_NORMALIZATION
static bool hasFullCompositionExclusion(const BinaryProperty &, UChar32, UProperty) {
	return FALSE;
}

#else
static bool hasFullCompositionExclusion(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
...
#if UCONFIG_NO_NORMALIZATION
static bool isNormInert(const BinaryProperty &, UChar32, UProperty) {
	return FALSE;
}

#else
static bool isNormInert(const BinaryProperty & /*prop*/, UChar32 c, UProperty which) {
	UErrorCode errorCode = U_ZERO_ERROR;
...
#if UCONFIG_NO_NORMALIZATION
static bool changesWhenCasefolded(const BinaryProperty &, UChar32, UProperty) {
	return FALSE;
}

#else
static bool changesWhenCasefolded(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	UnicodeString nfd;
	UErrorCode errorCode = U_ZERO_ERROR;
...
#if UCONFIG_NO_NORMALIZATION
static bool changesWhenNFKC_Casefolded(const BinaryProperty &, UChar32, UProperty) {
	return FALSE;
}

#else
static bool changesWhenNFKC_Casefolded(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	UErrorCode errorCode = U_ZERO_ERROR;
...
#if UCONFIG_NO_NORMALIZATION
static bool isCanonSegmentStarter(const BinaryProperty &, UChar32, UProperty) {
	return FALSE;
}

#else
static bool isCanonSegmentStarter(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	UErrorCode errorCode = U_ZERO_ERROR;
	const Normalizer2Impl * impl = Normalizer2Factory::getNFCImpl(errorCode);
	return
...
static bool isPOSIX_alnum(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return u_isalnumPOSIX(c);
}

static bool isPOSIX_blank(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return u_isblank(c);
}

static bool isPOSIX_graph(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return u_isgraphPOSIX(c);
}

static bool isPOSIX_print(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return u_isprintPOSIX(c);
}

static bool isPOSIX_xdigit(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	return u_isxdigit(c);
}

static bool isRegionalIndicator(const BinaryProperty & /*prop*/, UChar32 c, UProperty /*which*/) {
	// Property starts are a subset of lb=RI etc.
	return 0x1F1E6<=c && c<=0x1F1FF;
}

static bool hasEmojiProperty(const BinaryProperty & /*prop*/, UChar32 c, UProperty which) {
...
U_CAPI bool U_EXPORT2 u_hasBinaryProperty(UChar32 c, UProperty which) {
...
U_CFUNC UPropertySource U_EXPORT2 uprops_getSource(UProperty which) {
...
U_CFUNC void U_EXPORT2 uprops_addPropertyStarts(UPropertySource src, const USetAdder * sa, UErrorCode * pErrorCode) {
...
U_CAPI int32_t U_EXPORT2 u_getFC_NFKC_Closure(UChar32 c, UChar * dest, int32_t destCapacity, UErrorCode * pErrorCode) {
	...
	if(folded1Length<0) {
		...
		if(nfkcImpl->getCompQuickCheck(nfkcImpl->getNorm16(c))!=UNORM_NO) {
			return u_terminateUChars(dest, destCapacity, 0, pErrorCode); // c does not change at all under
			                                                             // CaseFolding+NFKC
		}
