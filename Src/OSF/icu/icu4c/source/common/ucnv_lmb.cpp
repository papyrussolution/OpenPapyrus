    ...
    ulmbcs_byte_t OptGroup)
{
	...
	if(extraInfo != NULL) {
		...
		ulmbcs_byte_t i;
		memzero(extraInfo, sizeof(UConverterDataLMBCS));
    ...
    )
{
	...
	*lastConverterIndex = group;
	...
	switch(bytesConverted) {
		case 4: *pLMBCS++ = (ulmbcs_byte_t)(value >> 24); CXX_FALLTHROUGH;
		case 3: *pLMBCS++ = (ulmbcs_byte_t)(value >> 16); CXX_FALLTHROUGH;
		case 2: *pLMBCS++ = (ulmbcs_byte_t)(value >> 8); CXX_FALLTHROUGH;
		case 1: *pLMBCS++ = (ulmbcs_byte_t)value; CXX_FALLTHROUGH;
...
static void U_CALLCONV _LMBCSFromUnicode(UConverterFromUnicodeArgs * args, UErrorCode * err)
{
	...
	bool groups_tried[ULMBCS_GRP_LAST+1];
	...
	while(args->source < args->sourceLimit && !U_FAILURE(*err)) {
		...
		if(!bytes_written) {
			...
			if(!bytes_written) { /* the ambiguous group cases  (Strategy 3) */
				memzero(groups_tried, sizeof(groups_tried));
				...
				if((extraInfo->OptGroup != 1) && (ULMBCS_AMBIGUOUS_MATCH(group, extraInfo->OptGroup))) {
					...
					/*To make R5 compatible translation, look for exceptional group first for
					   non-DBCS*/
				...
				if(!bytes_written && (extraInfo->localeConverterIndex) &&
...
static UChar32 U_CALLCONV _LMBCSGetNextUCharWorker(UConverterToUnicodeArgs*   args,
    UErrorCode *   err)
{
 ...
 */
static void U_CALLCONV _LMBCSToUnicodeWithOffsets(UConverterToUnicodeArgs*    args,
    UErrorCode * err)
{
