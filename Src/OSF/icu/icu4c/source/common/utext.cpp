...
static bool utext_access(UText * ut, int64_t index, bool forward) 
{
...
{
	bool r = (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_LENGTH_IS_EXPENSIVE)) != 0;
	return r;
}
...
U_CAPI int32_t U_EXPORT2 utext_extract(UText * ut, int64_t start, int64_t limit, char16_t * dest, int32_t destCapacity, UErrorCode * status) 
{
...
{
	bool b = (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_WRITABLE)) != 0;
	return b;
}
...
{
	bool b = (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_HAS_META_DATA)) != 0;
	return b;
}
...
struct UTF8Buf {
	...
	uint8 mapToNative[UTF8_TEXT_CHUNK_SIZE+4]; // map char16_t index in buf to
...
static bool U_CALLCONV utf8TextAccess(UText * ut, int64_t index, bool forward) {
	//
...
//
static char16_t * utext_strFromUTF8(char16_t * dest,
...
static int32_t U_CALLCONV utf8TextExtract(UText * ut, int64_t start, int64_t limit, char16_t * dest, int32_t destCapacity, UErrorCode * pErrorCode) 
{
...
static UText * U_CALLCONV utf8TextClone(UText * dest, const UText * src, bool deep, UErrorCode * status)
{
...
static UText * U_CALLCONV repTextClone(UText * dest, const UText * src, bool deep, UErrorCode * status) {
	// First do a generic shallow clone.  Does everything needed for the UText struct itself.
	dest = shallowTextClone(dest, src, status);
...
static bool U_CALLCONV repTextAccess(UText * ut, int64_t index, bool forward) {
...
static int32_t U_CALLCONV repTextExtract(UText * ut,
...
static int32_t U_CALLCONV repTextReplace(UText * ut,
    int64_t start, int64_t limit,
	...
	UnicodeString replStr((bool)(length<0), src, length); // read-only alias
...
static void U_CALLCONV repTextCopy(UText * ut,
    int64_t start, int64_t limit,
    int64_t destIndex,
    bool move,
    UErrorCode * status)
...
static UText * U_CALLCONV unistrTextClone(UText * dest, const UText * src, bool deep, UErrorCode * status) {
	// First do a generic shallow clone.  Does everything needed for the UText struct itself.
	dest = shallowTextClone(dest, src, status);
...
static bool U_CALLCONV unistrTextAccess(UText * ut, int64_t index, bool forward) {
	int32_t length  = ut->chunkLength;
	ut->chunkOffset = pinIndex(index, length);

	// Check whether request is at the start or end
	bool retVal = (forward && index<length) || (!forward && index>0);
	return retVal;
}

static int32_t U_CALLCONV unistrTextExtract(UText * t,
    int64_t start, int64_t limit,
    char16_t * dest, int32_t destCapacity,
    UErrorCode * pErrorCode) {
...
static int32_t U_CALLCONV unistrTextReplace(UText * ut,
    int64_t start, int64_t limit,
    const char16_t * src, int32_t length,
    UErrorCode * pErrorCode) {
...
static void U_CALLCONV unistrTextCopy(UText * ut,
    int64_t start, int64_t limit,
    int64_t destIndex,
    bool move,
    UErrorCode * pErrorCode) {
...
static UText * U_CALLCONV ucstrTextClone(UText * dest, const UText * src, bool deep, UErrorCode * status) {
...
static bool U_CALLCONV ucstrTextAccess(UText * ut, int64_t index, bool forward) 
{
	const char16_t * str   = (const char16_t *)ut->context;
...
breakout:
	...
	// Check whether request is at the start or end
	bool retVal = (forward && index<ut->chunkNativeLimit) || (!forward && index>0);
	return retVal;
}

static int32_t U_CALLCONV ucstrTextExtract(UText * ut, int64_t start, int64_t limit, char16_t * dest, int32_t destCapacity, UErrorCode * pErrorCode)
{
	if(U_FAILURE(*pErrorCode)) {
		return 0;
	}
...
static bool U_CALLCONV charIterTextAccess(UText * ut, int64_t index, bool forward) 
{
...
static UText * U_CALLCONV charIterTextClone(UText * dest, const UText * src, bool deep, UErrorCode * status) 
{
	if(U_FAILURE(*status)) {
		return NULL;
	}
...
static int32_t U_CALLCONV charIterTextExtract(UText * ut, int64_t start, int64_t limit, char16_t * dest, int32_t destCapacity, UErrorCode * status)
{
	if(U_FAILURE(*status)) {
		return 0;
	}
