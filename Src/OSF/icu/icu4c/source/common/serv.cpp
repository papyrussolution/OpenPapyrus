...
bool ICUServiceKey::fallback()
{
	return FALSE;
}

bool ICUServiceKey::isFallbackOf(const UnicodeString & id) const
{
	return id == _id;
}
...
StringPair* StringPair::create(const UnicodeString & displayName,
...
public:
	inline XMutex(UMutex * mutex, bool reentering)
		: fMutex(mutex)
		, fActive(!reentering)
	{
...
UObject* ICUService::getKey(ICUServiceKey& key, UnicodeString * actualReturn, const ICUServiceFactory* factory, UErrorCode & status) const
{
	...
	CacheEntry* result = NULL;
	{
		...
		if(factory != NULL) {
			...
			if(startIndex == 0) {
				// throw new InternalError("Factory " + factory + "not registered with service: " +
				// this);
...
UVector&ICUService::getDisplayNames(UVector& result,
    const Locale & locale,
...
bool ICUService::unregister(URegistryKey rkey, UErrorCode & status)
{
...
bool ICUService::acceptsListener(const EventListener& l) const
{
