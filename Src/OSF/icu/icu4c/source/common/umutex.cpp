...
U_CDECL_BEGIN static bool U_CALLCONV umtx_cleanup() 
{
	initMutex->~mutex();
...
//
U_COMMON_API bool U_EXPORT2 umtx_initImplPreInit(UInitOnce &uio) {
	std::call_once(*pInitFlag, umtx_init);
...
U_DEPRECATED void U_EXPORT2 u_setMutexFunctions(const void * /*context */, UMtxInitFn *, UMtxFn *,
...
U_DEPRECATED void U_EXPORT2 u_setAtomicIncDecFunctions(const void * /*context */, UMtxAtomicFn *, UMtxAtomicFn *,
    UErrorCode * status) {
	if(U_SUCCESS(*status)) {
		*status = U_UNSUPPORTED_ERROR;
	}
