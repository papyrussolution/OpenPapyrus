 ...
 */
RuleBasedBreakIterator::RuleBasedBreakIterator(RBBIDataHeader* data, UErrorCode & status) : fSCharIter(UnicodeString())
{
	init(status);
...
//
RuleBasedBreakIterator::RuleBasedBreakIterator(UDataMemory* udm, UErrorCode & status) : fSCharIter(UnicodeString())
{
	init(status);
...
//                           Used when creating a RuleBasedBreakIterator from a set of rules.
//
RuleBasedBreakIterator::RuleBasedBreakIterator() : fSCharIter(UnicodeString())
{
...
//
RuleBasedBreakIterator::RuleBasedBreakIterator(const RuleBasedBreakIterator& other) : BreakIterator(other), fSCharIter(UnicodeString())
{
	UErrorCode status = U_ZERO_ERROR;
	this->init(status);
	*this = other;
...
RuleBasedBreakIterator& RuleBasedBreakIterator::operator = (const RuleBasedBreakIterator &that) 
{
	...
	if(fLanguageBreakEngines != NULL) {
		ZDELETE(fLanguageBreakEngines); // Just rebuild for now
	}
