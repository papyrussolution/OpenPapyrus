...
static bool _isExtlangSubtag(const char * s, int32_t len) {
	/*
...
static bool _isPrivateuseVariantSubtag(const char * s, int32_t len) {
	/*
...
static bool _isPrivateuseValueSubtag(const char * s, int32_t len) {
	/*
...
static bool _isTKey(const char * s, int32_t len)
{
	/*
	 * tkey = alpha digit ;
	 */
...
static bool _isTValue(const char * s, int32_t len)
{
	/*
...
static bool _isUnicodeExtensionSubtag(int32_t& state, const char * s, int32_t len)
{
...
static bool _isStatefulSepListOf(bool (*test)(int32_t&, const char *, int32_t), const char * s, int32_t len)
{
	int32_t state = 0;
	const char * p;
	const char * start = s;
...
static bool _addExtensionToList(ExtensionListEntry ** first, ExtensionListEntry * ext, bool localeToBCP) {
	bool bAdded = TRUE;
		...
		while(true) {
			...
			if(localeToBCP) {
				...
				if(len == 1 && curlen == 1) {
					...
					if(cmp != 0) {
						if(strcmp(cur->key, LOCALE_ATTRIBUTE_KEY) == 0) {
							cmp = 1;
						}
						else if(strcmp(ext->key, LOCALE_ATTRIBUTE_KEY) == 0) {
							cmp = -1;
						}
					}
				}
...
static void _appendLanguageToLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, UErrorCode * status) 
{
	...
	if(!len) {
		...
		/* resolve deprecated */
		for(i = 0; i < SIZEOFARRAYi(DEPRECATEDLANGS); i += 2) {
			...
			if(strlen(buf) < strlen(DEPRECATEDLANGS[i])) break;
...
static void _appendScriptToLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, UErrorCode * status) {
...
static void _appendRegionToLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, UErrorCode * status) 
{
	...
	if(len > 0) {
		if(!ultag_isRegionSubtag(buf, len)) {
			...
			sink.Append("-", 1);
			/* resolve deprecated */
			for(int i = 0; i < SIZEOFARRAYi(DEPRECATEDREGIONS); i += 2) {
				if(uprv_compareInvCharsAsAscii(buf, DEPRECATEDREGIONS[i]) == 0) {
					strcpy(buf, DEPRECATEDREGIONS[i + 1]);
					len = (int32_t)strlen(buf);
					break;
				}
			}
...
{
	for(VariantListEntry* var1 = first; var1; var1 = var1->next) {
		for(VariantListEntry* var2 = var1->next; var2; var2 = var2->next) {
...
static void _appendVariantsToLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, bool * hadPosix, UErrorCode * status) {
			...
			if(*p == SEP || *p == LOCALE_SEP || *p == 0) {
				...
				if(pVar == NULL) {
					...
					if(_isVariantSubtag(pVar, -1)) {
						if(strcmp(pVar, POSIX_VALUE) || len != (int32_t)strlen(POSIX_VALUE)) {
							...
							/* Special handling for POSIX variant, need to remember that we
							   had it and then */
...
static void _appendKeywordsToLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, bool hadPosix, UErrorCode * status) {
	...
	if(keywordEnum.isValid() || hadPosix) {
		...
		if(U_SUCCESS(*status) && (firstExt || firstAttr)) {
			bool startLDMLExtension = FALSE;
			for(ext = firstExt; ext; ext = ext->next) {
				if(!startLDMLExtension && strlen(ext->key) > 1) {
					// first LDML u singelton extension 
					sink.Append("-u", 2);
					startLDMLExtension = TRUE;
				}
				// write out the sorted BCP47 attributes, extensions and private use 
				if(strcmp(ext->key, LOCALE_ATTRIBUTE_KEY) == 0) {
					// write the value for the attributes 
					for(attr = firstAttr; attr; attr = attr->next) {
						sink.Append("-", 1);
						sink.Append(attr->attribute, static_cast<int32_t>(strlen(attr->attribute)));
					}
				}
				else {
					sink.Append("-", 1);
					sink.Append(ext->key, static_cast<int32_t>(strlen(ext->key)));
					if(strcmp(ext->value, "true") != 0 && strcmp(ext->value, "yes") != 0) {
						sink.Append("-", 1);
						sink.Append(ext->value, static_cast<int32_t>(strlen(ext->value)));
					}
				}
			}
		}
 ...
 */
static void _appendLDMLExtensionAsKeywords(const char * ldmlext, ExtensionListEntry** appendTo, 
	icu::MemoryPool<ExtensionListEntry>& extPool, icu::MemoryPool<icu::CharString>& kwdBuf, bool * posixVariant, UErrorCode * status) 
{
	...
	if(pKwds) {
		...
		while(!isDone) {
			...
			if(emitKeyword) {
				...
				if(pBcpType) {
					char bcpTypeBuf[128]; /* practically long enough even considering multiple
					                         subtag type */
				...
				pBcpKey = pNextBcpKey;
				bcpKeyLen = pNextBcpKey ? nextBcpKeyLen : 0;
				pBcpType = NULL;
				bcpTypeLen = 0;
			}
		}
...
static ULanguageTag* ultag_parse(const char * tag, int32_t tagLen, int32_t* parsedLen, UErrorCode * status) {
	...
	*(tagBuf + tagLen) = 0;
	...
	for(i = 0; i < SIZEOFARRAYi(LEGACY); i += 2) {
		int32_t checkLegacyLen = static_cast<int32_t>(strlen(LEGACY[i]));
		...
		if(uprv_strnicmp(LEGACY[i], tagBuf, checkLegacyLen) == 0) {
			...
			int32_t replacementLen = static_cast<int32_t>(strlen(LEGACY[i+1]));
	...
	if(legacyLen == 0) {
		for(i = 0; i < SIZEOFARRAYi(REDUNDANT); i += 2) {
			const char * redundantTag = REDUNDANT[i];
			size_t redundantTagLen = strlen(redundantTag);
		...
		if(next & PRIV) {
			if(uprv_tolower(*pSubtag) == PRIVATEUSE && subtagLen == 1) {
				...
				while(pNext) {
					...
					subtagLen = (int32_t)(pSep - pSubtag);
					if(uprv_strncmp(pSubtag, PRIVUSE_VARIANT_PREFIX, strlen(PRIVUSE_VARIANT_PREFIX)) == 0) {
						*pSep = 0;
						next = VART;
						privateuseVar = TRUE;
						break;
...
U_CAPI void U_EXPORT2 ulocimp_toLanguageTag(const char * localeID, icu::ByteSink& sink, bool strict, UErrorCode * status) 
{
	...
	int32_t resultCapacity = static_cast<int32_t>(strlen(localeID));
	if(resultCapacity > 0) {
		char * buffer;
		for(;;) {
...
U_CAPI void U_EXPORT2 ulocimp_forLanguageTag(const char * langtag,
    int32_t tagLen,
    icu::ByteSink& sink,
