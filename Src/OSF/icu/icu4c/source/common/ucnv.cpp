...
U_CAPI UConverter * U_EXPORT2 ucnv_openU(const char16_t * name, UErrorCode * err)
{
...
U_CAPI UConverter * U_EXPORT2 ucnv_safeClone(const UConverter * cnv, void * stackBuffer, int32_t * pBufferSize, UErrorCode * status)
{
	...
	if(localConverter==NULL || U_FAILURE(*status)) {
		if(allocatedConverter && allocatedConverter->subChars != (uint8 *)allocatedConverter->subUChars) {
...
U_CAPI void U_EXPORT2 ucnv_getSubstChars(const UConverter * converter, char * mySubChar, int8 * len, UErrorCode * err)
{
	if(U_FAILURE(*err))
		return;
...
U_CAPI void U_EXPORT2 ucnv_setSubstChars(UConverter * converter, const char * mySubChar, int8 len, UErrorCode * err)
{
	if(U_FAILURE(*err))
		return;
...
U_CAPI void U_EXPORT2 ucnv_setSubstString(UConverter * cnv, const char16_t * s, int32_t length, UErrorCode * err) 
{
	...
	if(length8 > UCNV_MAX_SUBCHAR_LEN) {
		...
		if(cnv->subChars == (uint8 *)cnv->subUChars) {
			...
			memzero(cnv->subChars, UCNV_ERROR_BUFFER_LENGTH * U_SIZEOF_UCHAR);
		}
	}
...
static void _reset(UConverter * converter, UConverterResetChoice choice, bool callCallback) 
{
	...
	if(converter->sharedData->impl->reset) {
...
U_CAPI void U_EXPORT2 ucnv_setToUCallBack(UConverter * converter, UConverterToUCallback newAction, const void * newContext,
...
U_CAPI void U_EXPORT2 ucnv_setFromUCallBack(UConverter * converter, UConverterFromUCallback newAction,
    const void * newContext, UConverterFromUCallback * oldAction, const void ** oldContext, UErrorCode * err)
{
	if(U_FAILURE(*err))
		return;
...
U_CAPI void U_EXPORT2 ucnv_fromUnicode(UConverter * cnv, char ** target, const char * targetLimit, const char16_t ** source, const char16_t * sourceLimit,
...
static void _toUnicodeWithCallback(UConverterToUnicodeArgs * pArgs, UErrorCode * err) 
{
	...
	for(;;) {
		...
		for(;;) {
			...
			{
				UErrorCode e;
				if(calledCallback || (e = *err)==U_BUFFER_OVERFLOW_ERROR || (e!=U_INVALID_CHAR_FOUND &&
				    e!=U_ILLEGAL_CHAR_FOUND && e!=U_TRUNCATED_CHAR_FOUND && e!=U_ILLEGAL_ESCAPE_SEQUENCE && e!=U_UNSUPPORTED_ESCAPE_SEQUENCE)) {
					/*
					 * the callback did not or cannot resolve the error:
					 * set output pointers and return
					 *
 ...
 * @return TRUE if overflow
 */
static bool ucnv_outputOverflowToUnicode(UConverter * cnv, char16_t ** target, const char16_t * targetLimit, int32_t ** pOffsets, UErrorCode * err) 
{
	int32_t * offsets;
	char16_t * overflow, * t;
	int32_t i, length;
...
U_CAPI void U_EXPORT2 ucnv_toUnicode(UConverter * cnv,
...
U_CAPI int32_t U_EXPORT2 ucnv_fromUChars(UConverter * cnv, char * dest, int32_t destCapacity, const char16_t * src, int32_t srcLength, UErrorCode * pErrorCode) 
{
...
U_CAPI int32_t U_EXPORT2 ucnv_toUChars(UConverter * cnv, char16_t * dest, int32_t destCapacity, const char * src, int32_t srcLength, UErrorCode * pErrorCode) 
{
	const char * srcLimit;
...
U_CAPI UChar32 U_EXPORT2 ucnv_getNextUChar(UConverter * cnv, const char ** source, const char * sourceLimit, UErrorCode * err) 
{
	...
	if(cnv->UCharErrorBufferLength>0) {
		char16_t * overflow = cnv->UCharErrorBuffer;
		i = 0;
		length = cnv->UCharErrorBufferLength;
	...
	if(c<0) {
		 ...
		 */
		if(cnv->toULength==0 && cnv->sharedData->impl->getNextUChar) {
			...
			*source = s = args.source;
			if(*err==U_INDEX_OUTOFBOUNDS_ERROR) {
				 ...
				 *   UCNV_GET_NEXT_UCHAR_USE_TO_U: the native function did not want to handle it after
				 *all
...
U_CAPI void U_EXPORT2 ucnv_convertEx(UConverter * targetCnv, UConverter * sourceCnv, char ** target, const char * targetLimit,
    const char ** source, const char * sourceLimit, char16_t * pivotStart, char16_t ** pivotSource, char16_t ** pivotTarget, const char16_t * pivotLimit,
...
static int32_t ucnv_internalConvert(UConverter * outConverter, UConverter * inConverter,
...
/* @internal */
static int32_t ucnv_convertAlgorithmic(bool convertToAlgorithmic,
...
U_CAPI void U_EXPORT2 ucnv_getStarters(const UConverter * converter, bool starters[256], UErrorCode * err)
{
	if(!err || U_FAILURE(*err)) {
		return;
	}
	if(converter->sharedData->impl->getStarters) {
...
U_CAPI bool U_EXPORT2 ucnv_isAmbiguous(const UConverter * cnv) { return (bool)(ucnv_getAmbiguous(cnv) != NULL); }
U_CAPI void U_EXPORT2 ucnv_setFallback(UConverter * cnv, bool usesFallback) { cnv->useFallback = usesFallback; }
U_CAPI bool U_EXPORT2 ucnv_usesFallback(const UConverter * cnv) { return cnv->useFallback; }
...
U_CAPI void U_EXPORT2 ucnv_getInvalidUChars(const UConverter * converter, char16_t * errChars, int8 * len, UErrorCode * err)
{
	if(!err || U_FAILURE(*err)) {
		return;
	}
