...
bool UVector::containsAll(const UVector& other) const {
...
bool UVector::containsNone(const UVector& other) const {
	for(int32_t i = 0; i<other.size(); ++i) {
		if(indexOf(other.elements[i]) >= 0) {
			return FALSE;
		}
...
bool UVector::removeAll(const UVector& other) {
	bool changed = FALSE;
	for(int32_t i = 0; i<other.size(); ++i) {
...
bool UVector::retainAll(const UVector& other) {
	bool changed = FALSE;
...
bool UVector::removeElement(void * obj) {
...
bool UVector::equals(const UVector &other) const {
	int i;
...
bool UVector::ensureCapacityX(int32_t minimumCapacity, UErrorCode & status) {
...
bool UVector::ensureCapacity(int32_t minimumCapacity, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return false;
	}
	if(minimumCapacity < 0) {
