...
bool RuleBasedBreakIterator::DictionaryCache::following(int32_t fromPos, int32_t * result, int32_t * statusIndex) {
...
bool RuleBasedBreakIterator::DictionaryCache::preceding(int32_t fromPos, int32_t * result, int32_t * statusIndex) {
...
void RuleBasedBreakIterator::BreakCache::following(int32_t startPos, UErrorCode & status) {
	...
	if(startPos == fTextIdx || seek(startPos) || populateNear(startPos, status)) {
		...
		//       So clear it here, for the case where seek() succeeded on an iterator that had previously run
		// off the end.
		fBI->fDone = false;
		next();
	}
...
bool RuleBasedBreakIterator::BreakCache::seek(int32_t pos) {
...
bool RuleBasedBreakIterator::BreakCache::populateNear(int32_t position, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
		...
		if(position > 20) {
			...
			if(backupPos > 0) {
				...
				if(aBoundary <= backupPos + 4) {
					...
					// Four being the length of the longest potential code point, a supplementary in
					// UTF-8
					utext_setNativeIndex(&fBI->fText, aBoundary);
					if(backupPos == utext_getPreviousNativeIndex(&fBI->fText)) {
						// The initial handleNext() only advanced by a single code point. Go
						// again.
...
bool RuleBasedBreakIterator::BreakCache::populateFollowing() {
...
bool RuleBasedBreakIterator::BreakCache::populatePreceding(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
