...
public:
	...
	char16_t charAt(int32_t index, const UnicodeString & strings) const { return strings[stringOffset+1+index]; }
...
char16_t UCharsTrieBuilder::getElementUnit(int32_t i, int32_t unitIndex) const {
...
int32_t UCharsTrieBuilder::indexOfElementWithNextUnit(int32_t i, int32_t unitIndex, char16_t unit) const {
	while(unit==elements[i].charAt(unitIndex, strings)) {
		++i;
	}
	return i;
...
StringTrieBuilder::Node * UCharsTrieBuilder::createLinearMatchNode(int32_t i, int32_t unitIndex, int32_t length,
    Node * nextNode) const {
...
bool UCharsTrieBuilder::ensureCapacity(int32_t length) {
	if(uchars==NULL) {
...
int32_t UCharsTrieBuilder::write(const char16_t * s, int32_t length) {
...
int32_t UCharsTrieBuilder::writeValueAndFinal(int32_t i, bool isFinal) {
...
int32_t UCharsTrieBuilder::writeValueAndType(bool hasValue, int32_t value, int32_t node) {
	if(!hasValue) {
		return write(node);
	}
	char16_t intUnits[3];
