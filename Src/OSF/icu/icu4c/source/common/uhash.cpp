...
// 
static UHashTok _uhash_setElement(UHashtable * hash, UHashElement* e, int32_t hashcode, UHashTok key, UHashTok value, int8 hint) 
{
 ...
 */
static void _uhash_allocate(UHashtable * hash,
    int32_t primeIndex,
    UErrorCode * status) {
...
static UHashTok _uhash_put(UHashtable * hash, UHashTok key, UHashTok value, int8 hint, UErrorCode * status) 
{
...
U_CAPI UHashtable * U_EXPORT2 uhash_init(UHashtable * fillinResult,
...
U_CAPI void * U_EXPORT2 uhash_get(const UHashtable * hash,
    const void * key) {
	UHashTok keyholder;
...
U_CAPI void * U_EXPORT2 uhash_iget(const UHashtable * hash,
    int32_t key) {
	UHashTok keyholder;
...
U_CAPI int32_t U_EXPORT2 uhash_geti(const UHashtable * hash,
    const void * key) {
	UHashTok keyholder;
	keyholder.pointer = (void *)key;
...
U_CAPI int32_t U_EXPORT2 uhash_igeti(const UHashtable * hash,
    int32_t key) {
	UHashTok keyholder;
	keyholder.integer = key;
	return _uhash_find(hash, keyholder, hash->keyHasher(keyholder))->value.integer;
...
U_CAPI int32_t U_EXPORT2 uhash_getiAndFound(const UHashtable * hash,
    const void * key,
    bool * found) {
	UHashTok keyholder;
	keyholder.pointer = (void *)key;
...
U_CAPI int32_t U_EXPORT2 uhash_igetiAndFound(const UHashtable * hash,
    int32_t key,
    bool * found) {
	UHashTok keyholder;
	keyholder.integer = key;
...
U_CAPI int32_t U_EXPORT2 uhash_puti(UHashtable * hash,
    void * key,
    int32_t value,
    UErrorCode * status) {
	UHashTok keyholder, valueholder;
...
U_CAPI int32_t U_EXPORT2 uhash_iputi(UHashtable * hash,
    int32_t key,
    int32_t value,
    UErrorCode * status) {
	UHashTok keyholder, valueholder;
...
U_CAPI int32_t U_EXPORT2 uhash_putiAllowZero(UHashtable * hash,
    void * key,
    int32_t value,
    UErrorCode * status) {
	UHashTok keyholder, valueholder;
 ...
 */
U_CAPI bool U_EXPORT2 uhash_icontainsKey(const UHashtable * hash, int32_t key) {
	UHashTok keyholder;
	keyholder.integer = key;
	const UHashElement * e = _uhash_find(hash, keyholder, hash->keyHasher(keyholder));
	return !IS_EMPTY_OR_DELETED(e->hashcode);
