 ...
 */
U_CDECL_BEGIN
static bool U_CALLCONV service_cleanup() {
...
const Hashtable* LocaleUtility::getAvailableLocaleNames(const UnicodeString & bundleID)
{
	...
	if(htp == NULL) {
		...
		if(htp && U_SUCCESS(status)) {
			...
			for(;;) {
				const char16_t * id = uenum_unext(uenum.getAlias(), NULL, &status);
				if(id == NULL) {
					break;
				}
...
bool LocaleUtility::isFallbackOf(const UnicodeString & root, const UnicodeString & child)
{
