...
static bool isPNJConsonant(UChar32 c) {
	if(c < 0xa00 || 0xa50 <= c) {
		return FALSE;
	}
	else {
		return (bool)(pnjMap[c - 0xa00] & 1);
	}
}

static bool isPNJBindiTippi(UChar32 c) {
	if(c < 0xa00 || 0xa50 <= c) {
		return FALSE;
	}
	else {
		...
		if((pArgs->options & UCNV_OPTIONS_VERSION_MASK) < 9) {
			...
			converterData->currentMaskFromUnicode
				= converterData->currentMaskToUnicode
				    = converterData->defMaskToUnicode =
...
static void U_CALLCONV UConverter_fromUnicode_ISCII_OFFSETS_LOGIC(UConverterFromUnicodeArgs * args, UErrorCode * err) {
	...
	while(source < sourceLimit) {
		...
		if(converterData->currentDeltaFromUnicode == PNJ_DELTA && tempContextFromUnicode == PNJ_ADHAK &&
		    isPNJConsonant((sourceChar + PNJ_DELTA))) {
			/* If the previous codepoint is Adhak and the current codepoint is a consonant, the
			   targetByteUnit should be C + Halant + C. */
			/* reset context char */
			converterData->contextCharFromUnicode = 0x0000;
...
static void U_CALLCONV UConverter_toUnicode_ISCII_OFFSETS_LOGIC(UConverterToUnicodeArgs * args, UErrorCode * err) 
{
	...
	char16_t * contextCharToUnicode = NULL;
	bool found;
	int i;
	int offset = 0;
		...
		if(target < targetLimit) {
			...
			if(*contextCharToUnicode==ATR) {
				...
				*contextCharToUnicode = NO_CHAR_MARKER;
						...
						if(validityTable[(uint8)targetUniChar] & data->currentMaskToUnicode) {
							*contextCharToUnicode = NO_CHAR_MARKER;
							...
							/* Write the previous toUnicodeStatus, this was delayed to
							   handle consonant clustering for Gurmukhi script. */
							if(data->prevToUnicodeStatus) {
								WRITE_TO_TARGET_TO_U(args, source, target, args->offsets,
								    (source-args->source -1), data->prevToUnicodeStatus, 0, err);
								data->prevToUnicodeStatus = 0x0000;
							}
							/* write to target */
							WRITE_TO_TARGET_TO_U(args,
							    source,
							    target,
							    args->offsets,
				...
				/* Write the previous toUnicodeStatus, this was delayed to handle consonant clustering
				   for Gurmukhi script. */
				if(data->prevToUnicodeStatus) {
					WRITE_TO_TARGET_TO_U(args,
					    source,
				...
				case ATR:
				    *contextCharToUnicode = (char16_t)sourceChar;
				    ...
				    if(*toUnicodeStatus != missingCharMarker) {
					    /* Write the previous toUnicodeStatus, this was delayed to handle consonant
					       clustering for Gurmukhi script. */
					    if(data->prevToUnicodeStatus) {
						    WRITE_TO_TARGET_TO_U(args,
							source,
							target,
				...
				case ISCII_NUKTA:
				    ...
				    if(*contextCharToUnicode == ISCII_HALANT) {
					    ...
					    *contextCharToUnicode = NO_CHAR_MARKER;
					    ...
					    /* We got here because ISCII_NUKTA was preceded by 0xc0 and we are
					       converting Gurmukhi.
						    ...
						    if(validityTable[(uint8)targetUniChar] & data->currentMaskToUnicode) {
							    ...
							    *toUnicodeStatus = missingCharMarker;
							    if(data->currentDeltaToUnicode == PNJ_DELTA) {
								    /* Write the previous toUnicodeStatus, this was
								       delayed to handle consonant clustering for
								       Gurmukhi script. */
								    if(data->prevToUnicodeStatus) {
									    WRITE_TO_TARGET_TO_U(args, source, target, args->offsets,
										(source-args->source -1), data->prevToUnicodeStatus, 0,
										err);
									    data->prevToUnicodeStatus = 0x0000;
								    }
								    WRITE_TO_TARGET_TO_U(args,
			...
			if(*toUnicodeStatus != missingCharMarker) {
				/* Check to make sure that consonant clusters are handled correct for Gurmukhi script.
				   */
				if(data->currentDeltaToUnicode == PNJ_DELTA && data->prevToUnicodeStatus != 0 &&
				    isPNJConsonant(data->prevToUnicodeStatus) &&
				    (*toUnicodeStatus + PNJ_DELTA) == PNJ_SIGN_VIRAMA &&
				    ((UChar32)(targetUniChar + PNJ_DELTA) == data->prevToUnicodeStatus)) {
					/* Consonant clusters C + HALANT + C should be encoded as ADHAK + C */
					offset = (int)(source-args->source - 3);
					tempTargetUniChar = PNJ_ADHAK; /* This is necessary to avoid some compiler
					                                  warnings. */
					WRITE_TO_TARGET_TO_U(args, source, target, args->offsets, offset, tempTargetUniChar, 0, err);
					WRITE_TO_TARGET_TO_U(args, source, target, args->offsets, offset, data->prevToUnicodeStatus, 0,
					    err);
					...
					*toUnicodeStatus = missingCharMarker;
					...
					/* Check to make sure that Bindi and Tippi are handled correctly for Gurmukhi
					   script.
					 * If 0xA2 is preceded by a codepoint in the PNJ_BINDI_TIPPI_SET then the target
					 *codepoint should be Tippi instead of Bindi.
					 */
					if(data->currentDeltaToUnicode == PNJ_DELTA && (targetUniChar + PNJ_DELTA) == PNJ_BINDI &&
						...
						/* Store the current toUnicodeStatus code point for later handling of
						   consonant cluster in Gurmukhi. */
...
static UConverter * U_CALLCONV _ISCII_SafeClone(const UConverter * cnv,
    void * stackBuffer,
    int32_t * pBufferSize,
    UErrorCode * status)
{
...
static void U_CALLCONV _ISCIIGetUnicodeSet(const UConverter * cnv,
    const USetAdder * sa,
