...
const UChar * UCharsTrie::findUniqueValueFromBranch(const UChar * pos, int32_t length,
...
bool UCharsTrie::findUniqueValue(const UChar * pos, bool haveUniqueValue, int32_t &uniqueValue) {
	int32_t node = *pos++;
	for(;;) {
		if(node<kMinLinearMatch) {
			if(node==0) {
