...
LocaleMatcher::Builder &LocaleMatcher::Builder::setSupportedLocalesFromListString(StringPiece locales) {
...
bool LocaleMatcher::Builder::copyErrorTo(UErrorCode &outErrorCode) const {
...
namespace {
	LSR getMaximalLsrOrUnd(const XLikelySubtags &likelySubtags, const Locale &locale, UErrorCode & errorCode) 
	{
	...
	bool compareLSRs(const UHashTok t1, const UHashTok t2) 
	{
...
int32_t LocaleMatcher::putIfAbsent(const LSR &lsr, int32_t i, int32_t suppLength, UErrorCode & errorCode) 
{
...
LocaleMatcher::LocaleMatcher(const Builder &builder, UErrorCode & errorCode) : likelySubtags(*XLikelySubtags::getSingleton(errorCode)), 
	localeDistance(*LocaleDistance::getSingleton(errorCode)), thresholdDistance(builder.thresholdDistance_),
	demotionPerDesiredLocale(0), favorSubtag(builder.favor_), direction(builder.direction_),
	supportedLocales(nullptr), lsrs(nullptr), supportedLocalesLength(0), supportedLsrToIndex(nullptr),
	supportedLSRs(nullptr), supportedIndexes(nullptr), supportedLSRsLength(0), ownedDefaultLocale(nullptr), defaultLocale(nullptr) 
{
	if(U_FAILURE(errorCode)) {
		return;
	}
	...
	if(supportedLocalesLength > 0) {
		...
		memzero(supportedLocales, supportedLocalesLength * sizeof(const Locale *));
		...
		supportedLsrToIndex = uhash_openSize(hashLSR, compareLSRs, uhash_compareLong, supportedLocalesLength, &errorCode);
		if(U_FAILURE(errorCode)) {
			return;
		}
		supportedLSRs = static_cast<const LSR **>(uprv_malloc(supportedLocalesLength * sizeof(const LSR *)));
		supportedIndexes = static_cast<int32_t *>(uprv_malloc(supportedLocalesLength * sizeof(int32_t)));
...
const Locale * LocaleMatcher::getBestMatchForListString(StringPiece desiredLocaleList, UErrorCode & errorCode) const {
...
LocaleMatcher::Result LocaleMatcher::getBestMatchResult(const Locale &desiredLocale, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
...
LocaleMatcher::Result LocaleMatcher::getBestMatchResult(Locale::Iterator &desiredLocales, UErrorCode & errorCode) const {
...
bool LocaleMatcher::isMatch(const Locale &desired, const Locale &supported,
    UErrorCode & errorCode) const {
...
public:
	...
	const Locale & operator()(const char * tag) {
...
U_CAPI int32_t U_EXPORT2 uloc_acceptLanguage(char * result, int32_t resultAvailable,
...
U_CAPI int32_t U_EXPORT2 uloc_acceptLanguageFromHTTP(char * result, int32_t resultAvailable,
    UAcceptResult * outResult,
