...
U_NAMESPACE_BEGIN UOBJECT_DEFINE_RTTI_IMPLEMENTATION(Locale)
...
Locale & Locale::operator = (const Locale & other) 
{
	...
	if(other.fullName == other.fullNameBuffer) {
		strcpy(fullNameBuffer, other.fullNameBuffer);
	}
...
{
	return (strcmp(other.fullName, fullName) == 0);
}
...
static bool U_CALLCONV cleanupKnownCanonicalized() 
{
...
static void U_CALLCONV loadKnownCanonicalized(UErrorCode & status) 
{
	...
	for(int32_t i = 0; U_SUCCESS(status) && i < SIZEOFARRAYi(KNOWN_CANONICALIZED); i++) {
...
public:
	...
	static bool U_CALLCONV cleanup();
...
bool U_CALLCONV AliasData::cleanup()
{
	gInitOnce.reset();
	delete gSingleton;
	return TRUE;
...
private:
	...
	inline bool notEmpty(const char * str) { return str && str[0] != NULL_CHAR; }
	/**
...
CharString&AliasReplacer::generateKey(const char * language, const char * region, const char * variant,
    ...
    const char *& replacedRegion, const char *& replacedVariant, const char *& replacedExtensions, UVector& toBeFreed, UErrorCode & status)
{
	...
	CharString* str = new CharString(replacement, (int32_t)strlen(replacement), status);
	if(U_FAILURE(status)) {
		return;
	}
	if(str == nullptr) {
...
bool AliasReplacer::replaceLanguage(bool checkLanguage, bool checkRegion,
    bool checkVariants, UVector& toBeFreed, UErrorCode & status)
{
	...
	for(int32_t variant_index = 0;
		...
		if(searchVariant != nullptr && strlen(searchVariant) < 4) {
		...
		parseLanguageReplacement(replacement,
		    ...
		    status);
		replacedLanguage =
		    (replacedLanguage != nullptr && strcmp(replacedLanguage, "und") == 0) ?
...
bool AliasReplacer::replaceTerritory(UVector& toBeFreed, UErrorCode & status)
{
	...
	if(firstSpace != nullptr) {
		...
		if(likelyRegion != nullptr && strlen(likelyRegion) > 0) {
			size_t len = strlen(likelyRegion);
...
bool AliasReplacer::replaceVariant(UErrorCode & status)
{
	...
	for(int32_t i = 0; i < variants.size(); i++) {
		...
		if(!same(variant, replacement)) {
			...
			if(strcmp(variant, "heploc") == 0) {
				for(int32_t j = 0; j < variants.size(); j++) {
					if(strcmp((const char *)(variants.elementAt(j)), "hepburn") == 0) {
...
bool AliasReplacer::replaceTransformedExtensions(CharString& transformedExtensions, CharString& output, UErrorCode & status)
{
	...
	if(tkey != nullptr) {
		...
		tfields.sort([](UElement e1, UElement e2) -> int32_t {
				return strcmp((const char *)e1.pointer, (const char *)e2.pointer);
			}, status);
...
bool AliasReplacer::replace(const Locale & locale, CharString& out, UErrorCode & status)
{
	...
	UVector stringsToBeFreed([](void * obj) {
	    delete ((CharString*)obj);
			},
	    nullptr, 10, status);
	...
	if(extensionsStr != nullptr) {
		...
		if(U_SUCCESS(status) && !iter.isNull()) {
			...
			while((key = iter->next(nullptr, status)) != nullptr) {
				if(strcmp("sd", key) == 0 || strcmp("rg", key) == 0 ||
				    strcmp("t", key) == 0) {
					CharString value;
	...
	if(strcmp(out.data(), locale.getName()) == 0) {
		out.clear();
		return false;
	}
	return true;
...
{
	if(strcmp(locale, "c") == 0 || strcmp(locale, "en") == 0 || strcmp(locale, "en_US") == 0) {
		return true;
	}
...
{
	*length = SIZEOFARRAYi(KNOWN_CANONICALIZED);
...
Locale & Locale::init(const char * localeID, bool canonicalize)
{
	fIsBogus = FALSE;
		...
		while(U_SUCCESS(status) && legacy_key != nullptr) {
			...
			if(key != nullptr) {
				if(resultLength != nullptr) {
					*resultLength = static_cast<int32_t>(strlen(key));
				}
				return key;
			}
