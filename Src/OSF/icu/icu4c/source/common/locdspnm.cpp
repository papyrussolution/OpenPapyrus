...
UnicodeString &DefaultLocaleDisplayNames::keyValueDisplayName(const char * /* key */,
    const char * value,
	...
	virtual void put(const char * key, ResourceValue &value, bool /*noFallback*/,
		...
		for(int i = 0; contexts.getKeyAndValue(i, key, value); ++i) {
			...
			int32_t titlecaseInt =
			    (parent.capitalizationContext == UDISPCTX_CAPITALIZATION_FOR_UI_LIST_OR_MENU) ? intVector[0] : intVector[1];
			if(titlecaseInt == 0) {
				continue;
			}
...
UnicodeString &LocaleDisplayNamesImpl::localeDisplayName(const char * localeId,
    UnicodeString & result) const {
	return localeDisplayName(Locale(localeId), result);
}

// private
UnicodeString &LocaleDisplayNamesImpl::localeIdName(const char * localeId,
    UnicodeString & result, bool substitute) const {
...
UnicodeString &LocaleDisplayNamesImpl::scriptDisplayName(const char * script,
...
UnicodeString &LocaleDisplayNamesImpl::scriptDisplayName(UScriptCode scriptCode,
    UnicodeString & result) const {
	return scriptDisplayName(uscript_getName(scriptCode), result, FALSE);
}

UnicodeString &LocaleDisplayNamesImpl::regionDisplayName(const char * region,
    UnicodeString & result,
    bool skipAdjust) const {
	if(nameLength == UDISPCTX_LENGTH_SHORT) {
...
UnicodeString &LocaleDisplayNamesImpl::variantDisplayName(const char * variant,
    UnicodeString & result,
    bool skipAdjust) const {
...
UnicodeString &LocaleDisplayNamesImpl::keyValueDisplayName(const char * key,
    const char * value,
    UnicodeString & result,
    bool skipAdjust) const {
...
U_CAPI ULocaleDisplayNames * U_EXPORT2 uldn_openForContext(const char * locale,
    UDisplayContext * contexts, int32_t length,
...
U_CAPI UDisplayContext U_EXPORT2 uldn_getContext(const ULocaleDisplayNames * ldn,
