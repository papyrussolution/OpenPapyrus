...
static void U_CALLCONV cloneUnicodeString(UElement * dst, UElement * src) { dst->pointer = new UnicodeString(*(UnicodeString *)src->pointer); }
...
bool UnicodeSet::hasStrings() const { return strings != nullptr && !strings->isEmpty(); }
int32_t UnicodeSet::stringsSize() const { return strings == nullptr ? 0 : strings->size(); }
bool UnicodeSet::stringsContains(const UnicodeString & s) const { return strings != nullptr && strings->contains((void *)&s); }
 ...
 */
bool UnicodeSet::contains(const UnicodeString & s) const {
 ...
 * @return true if the test condition is met
 */
bool UnicodeSet::containsAll(const UnicodeSet & c) const {
 ...
 * @return true if the test condition is met
 */
bool UnicodeSet::containsAll(const UnicodeString & s) const {
	return (bool)(span(s.getBuffer(), s.length(), USET_SPAN_CONTAINED) ==
	       s.length());
}
 ...
 * @return true if the test condition is met
 */
bool UnicodeSet::containsNone(const UnicodeSet & c) const {
	// The specified set is a subset if all of its pairs are contained in
	// this set.  It's possible to code this more efficiently in terms of
	// direct manipulation of the inversion lists if the need arises.
	int32_t n = c.getRangeCount();
 ...
 * @return true if the test condition is met
 */
bool UnicodeSet::containsNone(const UnicodeString & s) const {
	return (bool)(span(s.getBuffer(), s.length(), USET_SPAN_NOT_CONTAINED) ==
	       s.length());
}
...
const UnicodeString * UnicodeSet::getString(int32_t index) const { return (const UnicodeString *)strings->elementAt(index); }
/**
 ...
 */
bool UnicodeSet::allocateStrings(UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
 ...
 */
void UnicodeSet::_appendToPat(UnicodeString & buf, const UnicodeString & s, bool escapeUnprintable) {
	UChar32 cp;
	for(int32_t i = 0; i < s.length(); i += U16_LENGTH(cp)) {
 ...
 */
void UnicodeSet::_appendToPat(UnicodeString & buf, UChar32 c, bool escapeUnprintable) {
...
int32_t UnicodeSet::span(const char16_t * s, int32_t length, USetSpanCondition spanCondition) const 
{
	...
	do {
		...
		if(spanCondition != static_cast<int>(contains(c))) {
			break;
		}
