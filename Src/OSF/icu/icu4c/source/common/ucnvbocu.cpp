 ...
 */
static const int8 bocu1ByteToTrail[BOCU1_MIN] = {
/* 0     1     2     3     4     5     6     7    */
 ...
 */
static const int8 bocu1TrailToByte[BOCU1_TRAIL_CONTROLS_COUNT] = {
/* 0     1     2     3     4     5     6     7    */
 ...
 */
static inline int32_t bocu1Prev(int32_t c) {
	/* compute new prev */
...
getTrail:
				if(source<sourceLimit) {
					...
					/* no more input */
					c = -c; /* negative lead surrogate as "incomplete" indicator to avoid c=0
					           everywhere else */
					break;
				}
			}
 ...
 * @return (diff<<2)|count
 */
static inline int32_t decodeBocu1LeadByte(int32_t b) {
	int32_t diff, count;
 ...
 * @see decodeBocu1
 */
static inline int32_t decodeBocu1TrailByte(int32_t count, int32_t b) {
	if(b<=0x20) {
