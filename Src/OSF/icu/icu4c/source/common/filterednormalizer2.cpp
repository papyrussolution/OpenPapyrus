...
UnicodeString & FilteredNormalizer2::normalize(const UnicodeString & src,
    UnicodeString & dest,
...
void FilteredNormalizer2::normalizeUTF8(uint32_t options, StringPiece src, ByteSink &sink,
...
void FilteredNormalizer2::normalizeUTF8(uint32_t options, const char * src, int32_t length,
...
UnicodeString & FilteredNormalizer2::normalizeSecondAndAppend(UnicodeString & first,
    const UnicodeString & second,
    UErrorCode & errorCode) const {
	return normalizeSecondAndAppend(first, second, TRUE, errorCode);
}

UnicodeString & FilteredNormalizer2::append(UnicodeString & first,
    const UnicodeString & second,
    UErrorCode & errorCode) const {
...
bool FilteredNormalizer2::getDecomposition(UChar32 c, UnicodeString & decomposition) const {
	return set.contains(c) && norm2.getDecomposition(c, decomposition);
}

bool FilteredNormalizer2::getRawDecomposition(UChar32 c, UnicodeString & decomposition) const {
	return set.contains(c) && norm2.getRawDecomposition(c, decomposition);
}

UChar32 FilteredNormalizer2::composePair(UChar32 a, UChar32 b) const {
...
bool FilteredNormalizer2::isNormalized(const UnicodeString & s, UErrorCode & errorCode) const {
...
bool FilteredNormalizer2::isNormalizedUTF8(StringPiece sp, UErrorCode & errorCode) const {
	if(U_FAILURE(errorCode)) {
		return FALSE;
	}
...
UNormalizationCheckResult FilteredNormalizer2::quickCheck(const UnicodeString & s, UErrorCode & errorCode) const {
	uprv_checkCanGetBuffer(s, errorCode);
	if(U_FAILURE(errorCode)) {
		return UNORM_MAYBE;
	}
...
int32_t FilteredNormalizer2::spanQuickCheckYes(const UnicodeString & s, UErrorCode & errorCode) const {
	uprv_checkCanGetBuffer(s, errorCode);
	if(U_FAILURE(errorCode)) {
		return 0;
	}
...
bool FilteredNormalizer2::hasBoundaryBefore(UChar32 c) const {
	return !set.contains(c) || norm2.hasBoundaryBefore(c);
}

bool FilteredNormalizer2::hasBoundaryAfter(UChar32 c) const {
	return !set.contains(c) || norm2.hasBoundaryAfter(c);
}

bool FilteredNormalizer2::isInert(UChar32 c) const {
...
U_CAPI UNormalizer2 * U_EXPORT2 unorm2_openFiltered(const UNormalizer2 * norm2, const USet * filterSet, UErrorCode * pErrorCode) {
