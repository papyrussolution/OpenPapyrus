...
U_CAPI UConverterSelector* U_EXPORT2 ucnvsel_open(const char * const*  converterList, int32_t converterListSize, const USet* excludedCodePoints, const UConverterUnicodeSet whichSet, UErrorCode * status) 
{
	...
	memzero(newSelector.getAlias(), sizeof(UConverterSelector));
 ...
 */
static int32_t ucnvsel_swap(const UDataSwapper * ds,
...
U_CAPI UConverterSelector* U_EXPORT2 ucnvsel_openFromSerialized(const void * buffer, int32_t length, UErrorCode * status) {
	...
	memzero(sel, sizeof(UConverterSelector));
...
static bool intersectMasks(uint32_t* dest, const uint32_t* source1, int32_t len) {
	int32_t i;
	uint32_t oredDest = 0;
	for(i = 0; i < len; ++i) {
