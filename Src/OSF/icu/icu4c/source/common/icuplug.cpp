...
struct UPlugData {
	...
	bool dontUnload; /**< TRUE if plugin must stay resident (leak plugin and lib) */
...
U_CAPI void U_EXPORT2 uplug_setPlugNoUnload(UPlugData * data, bool dontUnload)
{
...
static UPlugData* uplug_initErrorPlug(const char * libName,
    const char * sym,
    const char * config,
...
static UPlugData* uplug_initPlugFromLibrary(const char * libName, const char * sym, const char * config, UErrorCode * status) 
{
	...
	if(lib!=NULL && U_SUCCESS(*status)) {
		...
		if(lib) { /* still need to close the lib */
			UErrorCode subStatus = U_ZERO_ERROR;
...
U_CAPI UPlugLevel U_EXPORT2 uplug_getCurrentLevel() { return gCurrentLevel; }

static bool U_CALLCONV uplug_cleanup(void)
{
	int32_t i;
