...
bool UVector32::containsAll(const UVector32& other) const {
...
bool UVector32::containsNone(const UVector32& other) const {
	for(int32_t i = 0; i<other.size(); ++i) {
		if(indexOf(other.elements[i]) >= 0) {
			return FALSE;
		}
...
bool UVector32::removeAll(const UVector32& other) {
	bool changed = FALSE;
	for(int32_t i = 0; i<other.size(); ++i) {
...
bool UVector32::retainAll(const UVector32& other) {
	bool changed = FALSE;
...
bool UVector32::equals(const UVector32 &other) const {
	int i;
...
bool UVector32::expandCapacity(int32_t minimumCapacity, UErrorCode & status) {
	if(U_FAILURE(status)) {
		return FALSE;
	}
...
void UVector32::sortedInsert(int32_t tok, UErrorCode & ec) {
	...
	while(min != max) {
		...
		//int8 c = (*compare)(elements[probe], tok);
		//if(c > 0) {
