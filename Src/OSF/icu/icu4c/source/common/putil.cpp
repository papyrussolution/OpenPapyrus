...
bool fakeClock_set = FALSE; /** True if fake clock has spun up **/
...
U_CAPI bool U_EXPORT2 uprv_isNaN(double number)
{
#if IEEE_754
...
U_CAPI bool U_EXPORT2 uprv_isInfinite(double number)
{
#if IEEE_754
	BitPatternConversion convertedNumber;
	convertedNumber.d64 = number;
...
U_CAPI bool U_EXPORT2 uprv_isPositiveInfinity(double number)
{
#if IEEE_754 || U_PLATFORM == U_PF_OS390
	return (bool)(number > 0 && uprv_isInfinite(number));
#else
	return uprv_isInfinite(number);
#endif
}

U_CAPI bool U_EXPORT2 uprv_isNegativeInfinity(double number)
{
#if IEEE_754 || U_PLATFORM == U_PF_OS390
	return (bool)(number < 0 && uprv_isInfinite(number));
...
static bool isValidOlsonID(const char * id) 
{
	...
	return (bool)(id[idx] == 0 || strcmp(id, "PST8PDT") == 0 || strcmp(id, "MST7MDT") == 0 || strcmp(id, "CST6CDT") == 0 || strcmp(id, "EST5EDT") == 0);
}
...
typedef struct OffsetZoneMapping {
	...
	int32_t daylightType; /* 0=U_DAYLIGHT_NONE, 1=daylight in June-U_DAYLIGHT_JUNE, 2=daylight in
	                         December=U_DAYLIGHT_DECEMBER*/
	const char * stdID;
	const char * dstID;
	const char * olsonID;
} OffsetZoneMapping;
 ...
 */
static bool compareBinaryFiles(const char * defaultTZFileName, const char * TZFileName, DefaultTZInfo* tzInfo) {
	FILE * file;
	int64_t sizeFile;
	...
	if(file != NULL && tzInfo->defaultTZFilePtr != NULL) {
		...
		if(sizeFile != tzInfo->defaultTZFileSize) {
			...
			while(sizeFileLeft > 0) {
				memzero(bufferFile, MAX_READ_SIZE);
...
#define SKIP1 "."
#define SKIP2 ".."
static bool U_CALLCONV putil_cleanup();
...
static bool U_CALLCONV putil_cleanup(void)
{
...
static BOOL U_CALLCONV getIcuDataDirectoryUnderWindowsDirectory(char * directoryBuffer, UINT bufferLength)
{
	...
	UINT length = GetSystemWindowsDirectoryW(windowsPath, SIZEOFARRAYi(windowsPath));
	if((length > 0) && (length < (SIZEOFARRAYi(windowsPath) - 1))) {
		...
		if(U_SUCCESS(status) && (status != U_STRING_NOT_TERMINATED_WARNING) && (windowsPathUtf8Len < (SIZEOFARRAYi(windowsPathUtf8) - 1))) {
			...
			if((windowsPathUtf8Len + SIZEOFARRAYi(ICU_DATA_DIR_WINDOWS)) < bufferLength) {
				strcpy(directoryBuffer, windowsPathUtf8);
...
#     if U_PLATFORM_HAS_WINUWP_API == 0  // Windows UWP does not support getenv
	...
	if(getIcuDataDirectoryUnderWindowsDirectory(datadir_path_buffer, SIZEOFARRAYi(datadir_path_buffer))) {
		path = datadir_path_buffer;
	}
#endif
...
static const char * uprv_getPOSIXIDForCategory(int category)
{
	...
	if(category == LC_MESSAGES || category == LC_CTYPE) {
		...
		if((posixID == 0) || (strcmp("C", posixID) == 0) || (strcmp("POSIX", posixID) == 0)) {
 ...
 */
	...
	char * correctedPOSIXLocale = static_cast<char *>(uprv_malloc(strlen(posixID) + 10 + 1));
...
#define POSIX_LOCALE_CAPACITY 64
	...
	if((strcmp("C", correctedLocale) == 0) ||
		...
		LocaleLen = strlen(correctedLocale);
		 ...
		 */
			...
			strcpy(correctedLocale, "zh_HK");
		}

		/* A special zh_CN_GBK locale...
		 */
		else if(strcmp(correctedLocale, "zh_CN_GBK") == 0) {
			strcpy(correctedLocale, "zh_CN");
		}
	}
...
static const char * remapPlatformDependentCodepage(const char * locale, const char * name) {
	...
	if(strcmp(name, "IBM-943") == 0) {
		/* Use the ASCII compatible ibm-943 */
		name = "Shift-JIS";
	}
	else if(strcmp(name, "IBM-1252") == 0) {
		/* Use the windows-1252 that contains the Euro */
		name = "IBM-5348";
	}
#elif U_PLATFORM == U_PF_SOLARIS
	if(locale && strcmp(name, "EUC") == 0) {
	...
	else if(strcmp(name, "CP949") == 0) {
		/* Remap CP949 to a similar codepage to avoid issues with backslash and won symbol. */
		name = "EUC-KR";
	}
	else if(locale && strcmp(locale, "en_US_POSIX") != 0 && strcmp(name, "US-ASCII") == 0) {
		/*
	...
	if(strcmp(name, "CP949") == 0) {
		/* Remap CP949 to a similar codepage to avoid issues with backslash and won symbol. */
		name = "EUC-KR";
	}
#elif U_PLATFORM == U_PF_HPUX
	if(locale && strcmp(locale, "zh_HK") == 0 && strcmp(name, "big5") == 0) {
...
static const char * int_getDefaultCodepage()
{
	...
	memzero(codesetName, sizeof(codesetName));
	   ...
	   nl_langinfo may use the same buffer as setlocale. */
	{
		 ...
		 * instead of ASCII.
		 */
		if(strcmp(localeName, "en_US_POSIX") != 0) {
