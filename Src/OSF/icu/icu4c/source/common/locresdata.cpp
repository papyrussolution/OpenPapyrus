		...
		if(U_SUCCESS(errorCode)) {
			...
			if(U_FAILURE(errorCode)) {
				...
				*pErrorCode = errorCode; /*save the errorCode*/
				...
				if(replacement && itemKey != replacement) {
...
static ULayoutType _uloc_getOrientationHelper(const char * localeId,
    const char * key,
    UErrorCode * status)
{
...
U_CAPI ULayoutType U_EXPORT2 uloc_getCharacterOrientation(const char * localeId,
    UErrorCode * status)
{
 ...
 */
U_CAPI ULayoutType U_EXPORT2 uloc_getLineOrientation(const char * localeId,
    UErrorCode * status)
{
