...
static void U_CALLCONV T_UConverter_fromUnicode_UTF32_BE(UConverterFromUnicodeArgs * args,
    UErrorCode * err)
{
...
static void U_CALLCONV T_UConverter_fromUnicode_UTF32_BE_OFFSET_LOGIC(UConverterFromUnicodeArgs * args,
    UErrorCode * err)
{
	const char16_t * mySource = args->source;
	uchar * myTarget;
    ...
    UErrorCode * err)
{
	...
	if(length < 4) {
		...
		memcpy(args->converter->toUBytes, mySource, length);
	...
	memcpy(args->converter->toUBytes, mySource, 4);
...
static void U_CALLCONV T_UConverter_fromUnicode_UTF32_LE_OFFSET_LOGIC(UConverterFromUnicodeArgs * args,
    UErrorCode * err)
{
	const char16_t * mySource = args->source;
	uchar * myTarget;
			    ...
			    if(*source==utf32BOM[state]) {
				  ...
				  */
				    ...
				    if(count==(state&3)) {
					    ...
					    bool oldFlush = pArgs->flush;

					    /* some of the bytes are from a previous buffer, replay those first */
					    pArgs->source = utf32BOM+(state&4); /* select the correct BOM */
					    pArgs->sourceLimit = pArgs->source+((state&3)-count); /* replay previous
					                                                             bytes */
					    pArgs->flush = FALSE; /* this sourceLimit is not the real source stream
					                             limit */
...
static UChar32 U_CALLCONV _UTF32GetNextUChar(UConverterToUnicodeArgs * pArgs,
