...
public:
	 ...
	 */
	inline bool contains(const UnicodeString & s) {
	 ...
	 */
	inline bool adopt(UnicodeString * str, UErrorCode & status) {
	 ...
	 */
	inline bool add(const UnicodeString & str, UErrorCode & status) {
	 ...
	 */
	inline bool remove(const UnicodeString & s, UErrorCode & status) {
		if(U_FAILURE(status)) return false;
...
public:
	...
	virtual BreakIterator &refreshInputText(UText * input, UErrorCode & status) override {
		fDelegate->refreshInputText(input, status); return *this;
	}
	...
	}                                                                       // we keep the delegate current, so this
	                                                                        // should be correct.
...
SimpleFilteredSentenceBreakIterator::SimpleFilteredSentenceBreakIterator(BreakIterator * adopt,
    UCharsTrie * forwards,
    UCharsTrie * backwards,
    UErrorCode & status) :
...
SimpleFilteredSentenceBreakIterator::EFBMatchResult SimpleFilteredSentenceBreakIterator::breakExceptionAt(int32_t n) {
...

	{
		...
		while((uch = utext_previous32(fText.getAlias()))!=U_SENTINEL) { // more to consume backwards
			...
			// utext_getNativeIndex(fText.getAlias()));
		}
	}

	//if(bestValue >= 0) {
	//if(debug2) u_printf("rev<+/%C/+end of seq.. r=%d, bestPosn=%d, bestValue=%d\n", (char16_t)uch, r, bestPosn,
	// bestValue);
	//}
...
int32_t SimpleFilteredSentenceBreakIterator::first() {
...
bool SimpleFilteredSentenceBreakIterator::isBoundary(int32_t offset) {
...
int32_t SimpleFilteredSentenceBreakIterator::last() {
...
public:
	...
	virtual bool suppressBreakAfter(const UnicodeString & exception, UErrorCode & status) override;
	virtual bool unsuppressBreakAfter(const UnicodeString & exception, UErrorCode & status) override;
...
bool SimpleFilteredBreakIteratorBuilder::suppressBreakAfter(const UnicodeString & exception, UErrorCode & status)
{
	bool r = fSet.add(exception, status);
	FB_TRACE("suppressBreakAfter", &exception, r, 0);
	return r;
}

bool SimpleFilteredBreakIteratorBuilder::unsuppressBreakAfter(const UnicodeString & exception, UErrorCode & status)
{
	bool r = fSet.remove(exception, status);
		...
		if(nn>-1 && (nn+1)!=ustrs[i].length()) {
			...
			for(int j = 0; j<subCount; j++) {
				...
				if(ustrs[i].compare(0, nn+1, ustrs[j], 0, nn+1)==0) {
					...
					//bool otherIsPartial = ((nn+1)!=ustrs[j].length());  // true if ustrs[j]
					// doesn't end at nn
