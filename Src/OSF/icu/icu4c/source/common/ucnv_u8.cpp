...
static bool hasCESU8Data(const UConverter * cnv)
{
#if UCONFIG_ONLY_HTML_CONVERSION
	return FALSE;
#else
	return (bool)(cnv->sharedData == &_CESU8Data);
#endif
}
...
morebytes:
			while(i < inBytes) {
				if(mySource < sourceLimit) {
					...
					if(!icu::UTF8::isValidTrail(ch, static_cast<uint8>(ch2), i, inBytes) &&
...
U_CFUNC void U_CALLCONV ucnv_fromUnicode_UTF8(UConverterFromUnicodeArgs * args, UErrorCode * err)
{
	...
	uint8 * myTarget = (uint8 *)args->target;
	const uint8 * targetLimit = (uint8 *)args->targetLimit;
	uint8 * tempPtr;
	UChar32 ch;
	uint8 tempBuf[4];
	...
	while(mySource < sourceLimit && myTarget < targetLimit) {
		...
		if(ch < 0x80) { /* Single byte */
			...
			*(myTarget++) = (uint8)((ch >> 6) | 0xc0);
			if(myTarget < targetLimit) {
				*(myTarget++) = (uint8)((ch & 0x3f) | 0x80);
			}
			else {
				cnv->charErrorBuffer[0] = (uint8)((ch & 0x3f) | 0x80);
...
lowsurrogate:
			...
			if(ch <= MAXIMUM_UCS2) {
				indexToWrite = 2;
				tempPtr[0] = (uint8)((ch >> 12) | 0xe0);
			}
			else {
				indexToWrite = 3;
				tempPtr[0] = (uint8)((ch >> 18) | 0xf0);
				tempPtr[1] = (uint8)(((ch >> 12) & 0x3f) | 0x80);
			}
			tempPtr[indexToWrite-1] = (uint8)(((ch >> 6) & 0x3f) | 0x80);
			tempPtr[indexToWrite] = (uint8)((ch & 0x3f) | 0x80);
...
static void U_CALLCONV ucnv_UTF8FromUTF8(UConverterFromUnicodeArgs * pFromUArgs,
...
moreBytes:
			while(toULength<toULimit) {
				if(source<sourceLimit) {
					...
					/* store the partial UTF-8 character, compatible with the regular UTF-8
					   converter */
