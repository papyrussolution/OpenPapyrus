...
UnicodeString &Locale::getDisplayLanguage(const Locale &displayLocale, UnicodeString & result) const 
{
...
UnicodeString &Locale::getDisplayCountry(UnicodeString & dispCntry) const
{
...
static int32_t _getDisplayNameForComponent(const char * locale, const char * displayLocale,
    char16_t * dest, int32_t destCapacity, UDisplayNameGetter * getter, const char * tag, UErrorCode * pErrorCode) 
{
    ...
    UErrorCode * pErrorCode)
{
	...
	do {
		...
		for(int32_t subi = 0, resti = 0; subi<2;) { /* iterate through patterns 0 and 1*/
			bool subdone = FALSE; /* set true when ready to move to next substitution */
					...
					switch(resti++) {
						...
						default: {
						    ...
						    if(kw == NULL) {
							    ...
							    /* incorporating this behavior into the loop made it even
							       more complex,
