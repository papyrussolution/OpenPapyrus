...
static inline bool equal_uint32(const uint32_t * s, const uint32_t * t, int32_t length) {
	while(length>0 && *s==*t) {
		++s;
		++t;
		--length;
 ...
 * @internal
 */
static int32_t utrie_getDataBlock(UNewTrie * trie, UChar32 c) {
 ...
 * code point lookups.
 */
static void utrie_fold(UNewTrie * trie, UNewTrieGetFoldedValue * getFoldedValue, UErrorCode * pErrorCode) {
 ...
 * - -1 if it is not used
 */
static void _findUnusedBlocks(UNewTrie * trie) 
{
	int32_t i;
...
static int32_t _findSameDataBlock(const uint32_t * data, int32_t dataLength, int32_t otherBlock, int32_t step) 
{
	int32_t block;
 ...
 */
static void utrie_compact(UNewTrie * trie, bool overlap, UErrorCode * pErrorCode) {
 ...
 */
static uint32_t U_CALLCONV defaultGetFoldedValue(UNewTrie * trie, UChar32 start, int32_t offset) {
...
U_CAPI int32_t U_EXPORT2 utrie_serialize(UNewTrie * trie, void * dt, int32_t capacity,
...
U_CAPI int32_t U_EXPORT2 utrie_unserialize(UTrie * trie, const void * data, int32_t length, UErrorCode * pErrorCode) 
{
	...
	if((options&UTRIE_OPTIONS_SHIFT_MASK)!=UTRIE_SHIFT || ((options>>UTRIE_OPTIONS_INDEX_SHIFT)&UTRIE_OPTIONS_SHIFT_MASK)!=UTRIE_INDEX_SHIFT) {
		*pErrorCode = U_INVALID_FORMAT_ERROR;
		return -1;
	}
	trie->isLatin1Linear = (bool)((options&UTRIE_OPTIONS_LATIN1_IS_LINEAR)!=0);
