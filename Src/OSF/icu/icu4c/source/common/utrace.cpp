...
U_CAPI void U_EXPORT2 utrace_data(int32_t fnNumber, int32_t level, const char * fmt, ...) {
...
static void outputUString(const char16_t * s, int32_t len, char * outBuf, int32_t * outIx, int32_t capacity, int32_t indent) 
{
	int32_t i = 0;
	char16_t c;
	if(s==NULL) {
...
U_CAPI int32_t U_EXPORT2 utrace_vformat(char * outBuf, int32_t capacity, int32_t indent, const char * fmt, va_list args) {
	int32_t outIx  = 0;
	int32_t fmtIx  = 0;
	char fmtC;
	char c;
	...
	for(;;) {
		...
		switch(fmtC) {
			...
			case 'S':
			    /* char16_t * string, with length, len==-1 for null terminated. */
			    ...
			    if(ptrPtr == NULL) {
				    ...
				    for(i = 0; i<vectorLen || vectorLen==-1; i++) {
					    switch(vectorType) {
							...
							longArg = *ptrPtr==NULL ? 0 : 1; /* test for null terminated
							                                    array. */
							ptrPtr++;
							break;
						    case 'c':
							...
							longArg = *ptrPtr==NULL ? 0 : 1; /* for test for null term.
							                                    array. */
							ptrPtr++;
							break;
...
U_CAPI int32_t U_EXPORT2 utrace_format(char * outBuf, int32_t capacity,
...
U_CAPI void U_EXPORT2 utrace_setFunctions(const void * context,
...
U_CAPI void U_EXPORT2 utrace_getFunctions(const void ** context,
    UTraceEntry ** e, UTraceExit ** x, UTraceData ** d) {
	*e = pTraceEntryFunc;
	*x = pTraceExitFunc;
	*d = pTraceDataFunc;
...
U_CAPI void U_EXPORT2 utrace_setLevel(int32_t level) 
{
	if(level < UTRACE_OFF) {
		level = UTRACE_OFF;
	}
...
U_CAPI int32_t U_EXPORT2 utrace_getLevel() { return utrace_level; }

U_CFUNC bool utrace_cleanup() 
{
