...
U_CDECL_BEGIN
static bool U_CALLCONV uset_cleanup();
...
static inline bool isNameOpen(const UnicodeString & pattern, int32_t pos) {
	return pattern.charAt(pos)==u'\\' && pattern.charAt(pos+1)==u'N';
}

static inline bool isPOSIXOpen(const UnicodeString & pattern, int32_t pos) {
...
void UnicodeSet::applyPatternIgnoreSpace(const UnicodeString & pattern,
    ParsePosition& pos,
...
static bool versionFilter(UChar32 ch, void * context) {
...
static bool intPropertyFilter(UChar32 ch, void * context) {
	IntPropertyContext* c = (IntPropertyContext*)context;
	return u_getIntPropertyValue((UChar32)ch, c->prop) == c->value;
}

static bool scriptExtensionsFilter(UChar32 ch, void * context) {
...
namespace {
static bool mungeCharName(char * dst, const char * src, int32_t dstCapacity) {
 ...
 */
bool UnicodeSet::resemblesPropertyPattern(const UnicodeString & pattern,
    int32_t pos) {
 ...
 */
bool UnicodeSet::resemblesPropertyPattern(RuleCharacterIterator& chars,
    int32_t iterOpts) {
	// NOTE: literal will always be FALSE, because we don't parse escapes.
	bool result = FALSE, literal;
