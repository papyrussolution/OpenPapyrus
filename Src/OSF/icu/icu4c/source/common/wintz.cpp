 ...
 *
 * We use the Win32 API GetDynamicTimeZoneInformation (which is available since Vista) to get the current time zone
 *info,
 ...
 * field of the struct might be NULL. This can happen with some 3rd party RDP clients, and also when using older
 *versions
 * of the RDP protocol, which don't send the newer TimeZoneKeyNamei information and only send the StandardName and
 *DaylightName.
 *
 * Since these 3rd party clients and older RDP clients only send the pre-Vista time zone information to the server, this
 *means that we
 * need to fallback on using the pre-Vista methods to determine the time zone. This unfortunately requires examining the
 *registry directly
 * in order to try and determine the current time zone.
 *
 * Note that this can however still fail in some cases though if the client and server are using different languages, as
 *the StandardName
 * that is sent by client is localized in the client's language. However, we must compare this to the names that are on
 *the server, which
...
{
	// We first try to obtain the time zone directly by using the TimeZoneKeyName field of the
	// DYNAMIC_TIME_ZONE_INFORMATION struct.
	DYNAMIC_TIME_ZONE_INFORMATION dynamicTZI;
	memzero(&dynamicTZI, sizeof(dynamicTZI));
	SYSTEMTIME systemTimeAllZero;
	memzero(&systemTimeAllZero, sizeof(systemTimeAllZero));
	...
	if(dynamicTZI.DynamicDaylightTimeDisabled != 0 &&
		...
		if(utcOffsetMins % 60 == 0) {
			...
			//   "In order to conform with the POSIX style, those zone names beginning with "Etc/GMT" have
			// their sign reversed
			//   from the standard ISO 8601 convention. In the "Etc" area, zones west of GMT have a positive
			// sign and those
			//   east have a negative sign in their name (e.g "Etc/GMT-14" is 14 hours ahead of GMT)."
			//
			// Regarding the POSIX style, from
			// https://www.gnu.org/software/libc/manual/html_node/TZ-Variable.html
			//   "The offset specifies the time value you must add to the local time to get a Coordinated
			// Universal Time value."
			//
			// However, the Bias value in DYNAMIC_TIME_ZONE_INFORMATION *already* follows the POSIX
			// convention.
			//
			// From
			...
			//      UTC = local time + bias"
			//
			// For example, a time zone that is 3 hours ahead of UTC (UTC+03:00) would have a Bias value of
			// -180, and the
	...
	// client sent the server incomplete info (some 3rd party RDP clients only send the StandardName and
	//  DaylightName,
	// but do not send the important TimeZoneKeyName), or if the RDP server has not appropriately populated the
	// struct correctly.
	//
	...
	//
	// Note that this can however still fail though if the client and server are using different languages, as the
	// StandardName
	// that is sent by client is *localized* in the client's language. However, we must compare this to the names
	// that are
	// on the server, which are *localized* in registry using the server's language.
	//
	// One other note is that this fallback method doesn't work for the UWP version, as we can't use the registry
	// APIs.

	// windowsTimeZoneName will point at timezoneSubKeyName if we had to fallback to using the registry, and we
	// found a match.
...
// We can't use the registry APIs in the UWP version.
		...
		// means
		// that we could fail to find a match if the RDP client and RDP server are using different languages,
		// but unfortunately
		...
		for(DWORD i = 0; i < numTimeZoneSubKeys; i++) {
			...
			if(wcscmp(reinterpret_cast<WCHAR *>(registryStandardName), dynamicTZI.StandardName) == 0) {
				...
				// languages might use
				// the same string for more than one time zone. Thus we need to examine the TZI data in
				// the registry to
				// compare the GMT offset (the bias), and the DST transition dates, to ensure it's the
				// same time zone
				// as the currently reported one.
				REG_TZI_FORMAT registryTziValue;
				memzero(&registryTziValue, sizeof(registryTziValue));
				// Note: RegQueryValueExW wants the size of the buffer in bytes.
