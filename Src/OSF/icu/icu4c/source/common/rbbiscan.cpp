...
#include "rbbirpt.h"   // Contains state table for the rbbi rules parser. generated by a Perl script.
#include "rbbirb.h"
#include "rbbinode.h"
#include "rbbiscan.h"
#include "rbbitblb.h"
...
//
bool RBBIRuleScanner::doParseActions(int32_t action)
{
	RBBINode * n       = NULL;
	bool returnVal = TRUE;
	switch(action) {
...
void RBBIRuleScanner::scanSet() 
{
	...
	if(U_SUCCESS(*fRB->fStatus)) {
		RBBINode * n = pushNewNode(RBBINode::setRef);
		if(U_FAILURE(*fRB->fStatus)) {
			return;
		}
