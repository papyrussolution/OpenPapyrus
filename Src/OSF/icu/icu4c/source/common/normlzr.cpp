...
Normalizer* Normalizer::clone() const
{
...
void U_EXPORT2 Normalizer::compose(const UnicodeString & source,
    bool compat, int32_t options,
    UnicodeString & result,
    UErrorCode & status) {
	normalize(source, compat ? UNORM_NFKC : UNORM_NFC, options, result, status);
}

void U_EXPORT2 Normalizer::decompose(const UnicodeString & source,
    bool compat, int32_t options,
    UnicodeString & result,
    UErrorCode & status) {
...
bool Normalizer::isNormalized(const UnicodeString & source,
    UNormalizationMode mode, int32_t options,
    UErrorCode & status) {
	const Normalizer2 * n2 = Normalizer2Factory::getInstance(mode, status);
	if(U_SUCCESS(status)) {
...
UnicodeString & U_EXPORT2 Normalizer::concatenate(const UnicodeString & left, const UnicodeString & right,
    UnicodeString & result,
    UNormalizationMode mode, int32_t options,
...
bool Normalizer::getOption(int32_t option) const
{
