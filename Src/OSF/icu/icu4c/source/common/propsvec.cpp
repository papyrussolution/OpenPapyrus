...
U_CAPI UPropsVectors * U_EXPORT2 upvec_open(int32_t columns, UErrorCode * pErrorCode) {
	...
	memzero(pv, sizeof(UPropsVectors));
	pv->v = v;
	pv->columns = columns;
...
U_CAPI uint32_t * U_EXPORT2 upvec_cloneArray(const UPropsVectors * pv,
...
U_CAPI void U_CALLCONV upvec_compactToUTrie2Handler(void * context,
