...
typedef struct {
	...
	int32_t targetIndex;
	int32_t sourceIndex;
	bool isEscapeAppended;
	bool isStateDBCS;
	bool isTargetUCharDBCS;
	bool isEmptySegment;
}UConverterDataHZ;
...
static void U_CALLCONV UConverter_toUnicode_HZ_OFFSETS_LOGIC(UConverterToUnicodeArgs * args,
    UErrorCode * err) {
	char tempBuf[2];
			...
			if(args->converter->mode == UCNV_TILDE) {
				...
				switch(mySourceChar) {
					...
					case UCNV_CLOSE_BRACE:
					    ...
					    if(myData->isEmptySegment) {
						    myData->isEmptySegment = FALSE; /* we are handling it, reset to
						                                       avoid future spurious errors */
					...
					default:
					     ...
					     */
					    myData->isEmptySegment = FALSE; /* different error here, reset this to avoid
					                                       spurious future error */
					    *err = U_ILLEGAL_ESCAPE_SEQUENCE;
					    args->converter->toUBytes[0] = UCNV_TILDE;
				...
				if(args->converter->toUnicodeStatus == 0x00) {
					...
					if(mySourceChar == UCNV_TILDE) {
						...
						/* add another bit to distinguish a 0 byte from not having seen a lead
						   byte */
						args->converter->toUnicodeStatus = (uint32_t)(mySourceChar | 0x100);
						myData->isEmptySegment = FALSE; /* the segment has something, either
						                                   valid or will produce a different
						                                   error, so reset this */
					}
					continue;
				}
					...
					if(leadIsOk && trailIsOk) {
						...
						/* report a single illegal byte and continue with the following DBCS
						   starter byte */
						--mySource;
...
static void U_CALLCONV UConverter_fromUnicode_HZ_OFFSETS_LOGIC(UConverterFromUnicodeArgs * args,
	...
	bool isTargetUCharDBCS = (bool)myConverterData->isTargetUCharDBCS;
	bool oldIsTargetUCharDBCS;
	int len = 0;
	...
	while(mySourceIndex < mySourceLength) {
		...
		if(myTargetIndex < targetLength) {
			...
			if(targetUniChar != missingCharMarker) {
				myConverterData->isTargetUCharDBCS = isTargetUCharDBCS = (bool)(targetUniChar>0x00FF);
...
static UConverter * U_CALLCONV _HZ_SafeClone(const UConverter * cnv,
    void * stackBuffer,
    int32_t * pBufferSize,
    UErrorCode * status)
{
