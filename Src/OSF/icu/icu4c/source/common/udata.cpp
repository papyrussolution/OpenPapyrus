...
#ifdef UDATA_DEBUG
	slfprintf_stderr("Cache: [%s] <<< %p : %s. vFunc=%p\n", newElement->name,
...
public:
	UDataPathIterator(const char * path, const char * pkg,
	    const char * item, const char * suffix, bool doCheckLastFour,
 ...
 *        the 'item' parameter is often the same as pkg. (Though sometimes might have a tree part as well, ex:
 *"icudt62l-curr").
 */
...
#ifdef UDATA_DEBUG
		slfprintf_stderr("rest of path (IDD) = %s\n", currentPath);
		slfprintf_stderr("                     ");
		{
			int32_t qqq;
...
#ifdef UDATA_DEBUG
		slfprintf_stderr("ocd: no basename in %s, bailing.\n", path);
#endif
...
*                                                                      *
*----------------------------------------------------------------------*/
static bool extendICUData(UErrorCode * pErr)
{
	UDataMemory   * pData;
	...
	didUpdate = findCommonICUDataByName(U_ICUDATA_NAME, *pErr); /* Return 'true' when a racing writes out the
	                                                               extended        */
...
#ifdef UDATA_DEBUG
				slfprintf_stderr("** Mapped file: %s\n", pathBuffer);
#endif
				return pEntryData;
			}
 ...
 * @return 0 if not loaded, 1 if loaded or err
 */
static UDataMemory * doLoadFromCommonData(bool isICUData, const char * /*pkgName*/,
    ...
    UErrorCode * pErrorCode)
{
	...
	bool checkedExtendedICUData = FALSE;
...
static UDataMemory * doOpenChoice(const char * path, const char * type, const char * name, UDataMemoryIsAcceptable * isAcceptable, void * context, UErrorCode * pErrorCode)
{
	...
	if(path) {
		if(uprv_strchr(path, U_FILE_ALT_SEP_CHAR) != NULL) {
			...
			slfprintf_stderr("Changed path from [%s] to [%s]\n", path, altSepPath.s);
#endif
	...
	if(isICUData && isTimeZoneFile(name, type)) {
		...
		if(tzFilesDir[0] != 0) {
#ifdef UDATA_DEBUG
			slfprintf_stderr("Trying Time Zone Files directory = %s\n", tzFilesDir);
#endif
...
U_CAPI UDataMemory * U_EXPORT2 udata_openChoice(const char * path, const char * type, const char * name,
    UDataMemoryIsAcceptable * isAcceptable, void * context,
    UErrorCode * pErrorCode) {
#ifdef UDATA_DEBUG
	slfprintf_stderr("udata_openChoice(): Opening: %s : %s . %s\n", (path ? path : "NULL"), name, type);
#endif
