...
void BytesTrieElement::setTo(StringPiece s, int32_t val,
...
int32_t BytesTrieBuilder::writeValueAndFinal(int32_t i, bool isFinal) 
{
	if(0 <= i && i <= BytesTrie::kMaxOneByteValue) {
		return write(((BytesTrie::kMinOneByteValueLead+i)<<1)|static_cast<int>(isFinal));
	}
	char intBytes[5];
	int32_t length = 1;
	if(i<0 || i>0xffffff) {
	...
	intBytes[0] = (char)((intBytes[0]<<1)|static_cast<char>(isFinal));
	return write(intBytes, length);
}

int32_t BytesTrieBuilder::writeValueAndType(bool hasValue, int32_t value, int32_t node) 
{
