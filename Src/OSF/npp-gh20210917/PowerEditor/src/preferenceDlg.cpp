		...
		case WM_COMMAND:
	    {
			    ...
			    case IDC_CHECK_DARKMODE_ENABLE:
			{
				...
				if(nppGUI._darkmode._isEnabled) {
					// For toolbar: if dark mode enabled & TB_STANDARD is selected, switch to
					// TB_SMALL
					bool isStandardChecked = false;
					::SendMessage(_hParent, PREF_MSG_ISCHECKED_GENERALPAGE, IDC_RADIO_STANDARD,
					    LPARAM(&isStandardChecked));
		    ...
		    if(changed) {
			    if(!doEnableCustomizedColorCtrls) {
					...
					nppGUI._darkmode._isEnabled ? NppDarkMode::getDarkerBackgroundColor() : ::GetSysColor(
					    COLOR_3DFACE);
...
void MarginsBorderEdgeSubDlg::initScintParam()
{
	...
	oldFunclstToolbarProc =
	    reinterpret_cast<WNDPROC>(::SetWindowLongPtr(::GetDlgItem(_hSelf, IDC_COLUMNPOS_EDIT), GWLP_WNDPROC,
	    reinterpret_cast<LONG_PTR>(editNumSpaceProc)));
}
	...
	switch(message)
	{
		...
		case WM_HSCROLL:
	    {
		    ...
		    if(reinterpret_cast<HWND>(lParam) == hBorderWidthSlider) {
			    ...
			    svp._paddingLeft = static_cast<uchar>(::SendMessage(hPaddingLeftSlider, TBM_GETPOS, 0, 0));
			    ...
			    svp._paddingRight = static_cast<uchar>(::SendMessage(hPaddingRightSlider, TBM_GETPOS, 0, 0));
			    ...
			    svp._distractionFreeDivPart =
				static_cast<uchar>(::SendMessage(hDistractionFreeSlider, TBM_GETPOS, 0, 0));
		...
		case WM_COMMAND:
	    {
		    ...
		    switch(wParam)
		    {
				...
				svp._lineNumberMarginShow =
				    (BST_CHECKED == ::SendDlgItemMessage(_hSelf, IDC_CHECK_LINENUMBERMARGE, BM_GETCHECK, 0, 0));
				::EnableWindow(::GetDlgItem(_hSelf, IDC_RADIO_DYNAMIC), svp._lineNumberMarginShow);
				::EnableWindow(::GetDlgItem(_hSelf, IDC_RADIO_CONSTANT), svp._lineNumberMarginShow);
				::SendMessage(_hParent, WM_COMMAND, IDM_VIEW_LINENUMBER, 0);
				return TRUE;
			    case IDC_RADIO_DYNAMIC:
				svp._lineNumberMarginDynamicWidth =
				    (BST_CHECKED == ::SendDlgItemMessage(_hSelf, IDC_RADIO_DYNAMIC, BM_GETCHECK, 0, 0));
				::SendMessage(_hParent, WM_COMMAND, IDM_VIEW_LINENUMBER, 0);
				return TRUE;
			    case IDC_RADIO_CONSTANT:
				svp._lineNumberMarginDynamicWidth =
				    !(BST_CHECKED == ::SendDlgItemMessage(_hSelf, IDC_RADIO_CONSTANT, BM_GETCHECK, 0, 0));
				::SendMessage(_hParent, WM_COMMAND, IDM_VIEW_LINENUMBER, 0);
				return TRUE;

			    case IDC_CHECK_BOOKMARKMARGE:
				svp._bookMarkMarginShow =
				    (BST_CHECKED == ::SendDlgItemMessage(_hSelf, IDC_CHECK_BOOKMARKMARGE, BM_GETCHECK, 0, 0));
...
INT_PTR CALLBACK MiscSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch(message) {
		...
		case WM_COMMAND:
	    {
			...
			{
				nppGUI._writeTechnologyEngine =
				    isCheckedOrNot(IDC_CHECK_DIRECTWRITE_ENABLE) ? directWriteTechnology : defaultTechnology;
				return TRUE;
			}
...
void NewDocumentSubDlg::makeOpenAnsiAsUtf8(bool doIt)
{
	if(!doIt)
	...
	switch(message)
	{
		case WM_INITDIALOG:
	    {
		    ...
		    for(size_t i = 0, encodingArraySize = sizeof(encodings)/sizeof(int); i < encodingArraySize; ++i) {
			    ...
			    if(cmdID != -1) {
				    ...
				    int index =
					static_cast<int32_t>(::SendDlgItemMessage(_hSelf, IDC_COMBO_OTHERCP, CB_ADDSTRING, 0,
		...
		case WM_COMMAND:
		    switch(wParam)
		    {
				...
				ndds._openAnsiAsUtf8 =
				    (BST_CHECKED == ::SendMessage(::GetDlgItem(_hSelf, IDC_CHECK_OPENANSIASUTF8), BM_GETCHECK, 0, 0));
				return TRUE;
			    ...
			    case IDC_RADIO_OTHERCP:
			{
				...
				ndds._codepage =
				    static_cast<int32_t>(::SendDlgItemMessage(_hSelf, IDC_COMBO_OTHERCP, CB_GETITEMDATA, index, 0));
				return TRUE;
			}
			    ...
			    default:
			{
				if(HIWORD(wParam) == CBN_SELCHANGE) {
					if(LOWORD(wParam) == IDC_COMBO_DEFAULTLANG) {
						...
						ndds._lang =
						    static_cast<LangType>(::SendDlgItemMessage(_hSelf, IDC_COMBO_DEFAULTLANG,
						...
						auto index = ::SendDlgItemMessage(_hSelf, IDC_COMBO_OTHERCP, CB_GETCURSEL, 0, 0);
						ndds._codepage =
						    static_cast<int32_t>(::SendDlgItemMessage(_hSelf, IDC_COMBO_OTHERCP, CB_GETITEMDATA,
						    index, 0));
						return TRUE;
					}
				}
...
INT_PTR CALLBACK DefaultDirectorySubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
	    {
		    ...
		    ::SendDlgItemMessage(_hSelf, IDC_OPENSAVEDIR_CHECK_DRROPFOLDEROPENFILES, BM_SETCHECK, nppGUI._isFolderDroppedOpenFiles ? BST_CHECKED : BST_UNCHECKED,
			0);
	    }
		...
		case WM_COMMAND:
	    {
			    ...
			    case IDC_MAXNBFILEVAL_STATIC:
			{
				generic_string staticText =
				    pNativeSpeaker->getLocalizedStrFromID("recent-file-history-maxfile", TEXT("Max File: "));
	...
	switch(message)
	{
		case WM_INITDIALOG:
	    {
			    ...
			    if(static_cast<LangType>(i) != L_USER) {
				    ...
				    if((cmdID != -1)) {
					    ...
					    if(str.length() > 0) {
						    ...
						    ::SendDlgItemMessage(_hSelf, IDC_LIST_ENABLEDLANG, LB_ADDSTRING, 0,
							reinterpret_cast<LPARAM>(str.c_str()));
					    }
				    }
			    }
		    ...
		    for(size_t i = 0, len = nppGUI._excludedLangList.size(); i < len; ++i) {
			    ::SendDlgItemMessage(_hSelf, IDC_LIST_DISABLEDLANG, LB_ADDSTRING, 0,
				reinterpret_cast<LPARAM>(nppGUI._excludedLangList[i]._langName.c_str()));
		    }
		...
		case WM_COMMAND:
	    {
		    if(HIWORD(wParam) == LBN_SELCHANGE) {
			    ...
			    if(LOWORD(wParam) == IDC_LIST_DISABLEDLANG || LOWORD(wParam) == IDC_LIST_ENABLEDLANG) {
				    ...
				    if(i != LB_ERR) {
					    ...
					    int idListbox2Disable =
						(LOWORD(wParam)== IDC_LIST_ENABLEDLANG) ? IDC_LIST_DISABLEDLANG : IDC_LIST_ENABLEDLANG;
			...
			{
				bool useDefaultTab = BST_CHECKED == ::SendMessage(::GetDlgItem(_hSelf, IDC_CHECK_DEFAULTTABVALUE),
					BM_GETCHECK,
					0,
					0);
				auto index = ::SendDlgItemMessage(_hSelf, IDC_LIST_TABSETTNG, LB_GETCURSEL, 0, 0);
	...
	switch(message)
	{
		case WM_INITDIALOG:
	    {
		    ...
		    for(size_t i = 0, len = fontlist.size(); i < len; ++i) {
			    auto j =
				::SendDlgItemMessage(_hSelf, IDC_COMBO_HFONTNAME, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(fontlist[i].c_str()));
		    ...
		    index =
			::SendDlgItemMessage(_hSelf, IDC_COMBO_FFONTNAME, CB_FINDSTRINGEXACT, static_cast<WPARAM>(-1), reinterpret_cast<LPARAM>(nppGUI._printSettings._footerFontName.c_str()));
		    if(index == CB_ERR)
			    index = 0;
		    ...
		    ::SendDlgItemMessage(_hSelf, IDC_COMBO_HFONTSIZE, CB_SELECTSTRING, static_cast<WPARAM>(-1),
			reinterpret_cast<LPARAM>(intStr));
		    wsprintf(intStr, TEXT("%d"), nppGUI._printSettings._footerFontSize);
		    ::SendDlgItemMessage(_hSelf, IDC_COMBO_FFONTSIZE, CB_SELECTSTRING, static_cast<WPARAM>(-1),
			reinterpret_cast<LPARAM>(intStr));
		...
		case WM_COMMAND:
	    {
		    if(HIWORD(wParam) == EN_CHANGE) {
				    ...
				    case IDC_COMBO_FFONTSIZE:
				{
					...
					int * pVal =
					    (LOWORD(wParam) ==
					    IDC_COMBO_HFONTSIZE) ? &(nppGUI._printSettings._headerFontSize) : &(nppGUI.
					    _printSettings._footerFontSize);
	...
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    switch(nppGUI._backup) {
			    case bak_simple: ID2CheckBackupOnSave = IDC_RADIO_BKSIMPLE; break;
			    case bak_verbose: ID2CheckBackupOnSave = IDC_RADIO_BKVERBOSE; break;
			    default: ID2CheckBackupOnSave = IDC_RADIO_BKNONE; //bak_none
		    }
		    ...
		    if(HIWORD(wParam) == EN_CHANGE) {
			    switch(LOWORD(wParam)) {
				    ...
				    case IDC_BACKUPDIR_RESTORESESSION_EDIT:
						{
							TCHAR str[16];
							::GetDlgItemText(_hSelf, IDC_BACKUPDIR_RESTORESESSION_EDIT, str, SIZEOFARRAY(str));
...
INT_PTR CALLBACK AutoCompletionSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch(message) {
		...
		case WM_COMMAND:
	    {
			...
			{
				nppGUI._autocIgnoreNumbers =
				    (BST_CHECKED == ::SendDlgItemMessage(_hSelf, IDD_AUTOC_IGNORENUMBERS, BM_GETCHECK, 0, 0));
				return TRUE;
			}
			    ...
			    case IDD_AUTOC_STATIC_N:
			{
				...
				NativeLangSpeaker * pNativeSpeaker = nppParam.getNativeLangSpeaker();
				generic_string strNbChar =
				    pNativeSpeaker->getLocalizedStrFromID("autocomplete-nb-char", TEXT("Nb char : "));
			...
			{
				bool isChecked =
				    (BST_CHECKED == ::SendDlgItemMessage(_hSelf, static_cast<int32_t>(wParam), BM_GETCHECK, 0, 0));
				const TCHAR * label;
...
INT_PTR CALLBACK MultiInstanceSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch(message) {
		...
		case WM_COMMAND:
		    ...
		    switch(wParam) {
			    case IDC_SESSIONININST_RADIO: nppGUI._multiInstSetting = multiInstOnSession; break;
			    case IDC_MULTIINST_RADIO: nppGUI._multiInstSetting = multiInst; break;
			    case IDC_MONOINST_RADIO: nppGUI._multiInstSetting = monoInst; break;
			    case IDD_DATETIMEFORMAT_REVERSEORDER_CHECK: nppGUI._dateTimeReverseDefaultOrder = isCheckedOrNot(IDD_DATETIMEFORMAT_REVERSEORDER_CHECK); break;
...
INT_PTR CALLBACK DelimiterSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    setWarningIfNeed();
		    NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
		    generic_string tip2show =
			pNativeSpeaker->getLocalizedStrFromID("word-chars-list-tip",
			    TEXT("This allows you to include additional character into current word characters while double clicking for selection or searching with \"Match whole word only\" option checked."));
	...
	if(HIWORD(wParam) == EN_CHANGE) {
		switch(LOWORD(wParam))
		{
			case  IDC_CLOUDPATH_EDIT:
		    {
			    ...
			    if(::PathFileExists(inputDirExpanded)) {
				    ...
				    if(nppParams.isCloudPathChanged()) {
					    message =
						pNativeSpeaker->getLocalizedStrFromID("cloud-restart-warning",
						    TEXT("Please restart Notepad++ to take effect."));
				    }
				    ...
				    if(isChecked) {
					    generic_string message =
						pNativeSpeaker->getLocalizedStrFromID("cloud-invalid-warning", TEXT("Invalid path."));
			...
			case IDC_URISCHEMES_EDIT:
		    {
			    ...
			    ::SendDlgItemMessage(_hSelf, IDC_URISCHEMES_EDIT, WM_GETTEXT, uriSchemesMaxLength,
				reinterpret_cast<LPARAM>(uriScheme));
	...
	switch(message) {
		...
		case WM_COMMAND:
	    {
			    ...
			    case IDC_NOCLOUD_RADIO:
			{
				...
				::SendDlgItemMessage(_hSelf, IDC_CLOUDPATH_EDIT, WM_SETTEXT, 0,
				    reinterpret_cast<LPARAM>(nppGUI._cloudPath.c_str()));
			...
			{
				generic_string message =
				    pNativeSpeaker->getLocalizedStrFromID("cloud-select-folder",
					TEXT("Select a folder from/to where Notepad++ reads/writes its settings"));
...
INT_PTR CALLBACK SearchingSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM)
{
	...
	switch(message) {
		...
		case WM_COMMAND:
		    switch(wParam) {
			    case IDC_CHECK_STOPFILLINGFINDFIELD: nppGUI._stopFillingFindField = isCheckedOrNot(IDC_CHECK_STOPFILLINGFINDFIELD); return TRUE;
			    case IDC_CHECK_MONOSPACEDFONT_FINDDLG: nppGUI._monospacedFontFindDlg = isCheckedOrNot(IDC_CHECK_MONOSPACEDFONT_FINDDLG); return TRUE;
			    case IDC_CHECK_FINDDLG_ALWAYS_VISIBLE: nppGUI._findDlgAlwaysVisible = isCheckedOrNot(IDC_CHECK_FINDDLG_ALWAYS_VISIBLE); return TRUE;
			    case IDC_CHECK_CONFIRMREPLOPENDOCS: nppGUI._confirmReplaceInAllOpenDocs = isCheckedOrNot(IDC_CHECK_CONFIRMREPLOPENDOCS); return TRUE;
			    case IDC_CHECK_REPLACEANDSTOP: nppGUI._replaceStopsWithoutFindingNext = isCheckedOrNot(IDC_CHECK_REPLACEANDSTOP); return TRUE;
			    default: return FALSE;
		    }
		    break;
	}
