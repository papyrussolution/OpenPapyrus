...
bool XmlMatchedTagsHighlighter::getXmlMatchedTagsPos(XmlMatchedTagsPos &xmlTags)
{
	...
	} while(openFound.success && (styleAt == SCE_H_DOUBLESTRING || styleAt == SCE_H_SINGLESTRING || styleAt == SCE_H_COMMENT ) &&
	    searchStartPoint > 0);
	...
	if(openFound.success && styleAt != SCE_H_CDATA) {
		...
		if(!closeFound.success) {
			...
			if('/' == nextChar) {
				...
				// XML
				while(position < docLength && !isWhitespace(nextChar) && nextChar != '/' && nextChar != '>' &&
				    nextChar != '\"' && nextChar != '\'') {
				...
				if(tagName.size() != 0) {
					/* Now we need to find the open tag.  The logic here is that we search for
					   "<TAGNAME",
					 * then check the next character - if it's one of '>', ' ', '\"' then we know
					 *we've found
					 * a relevant tag.
					 ...
					 *backwards
					 * from the first "<TAGNAME...", as we know there's a close tag for the opened
					 *tag.
					...
					do {
						...
						if(nextOpenTag.success) {
							--openTagsRemaining;
							// Open tag found
							// Now we need to check how many close tags there are between
							// the open tag we just found,
							// and our close tag
							// eg. (Cursor == | )
							// <TAGNAME
							// attrib="value"><TAGNAME>something</TAGNAME></TAGNAME></TAGNA|ME>
							...
							do {
								inbetweenCloseTag = findCloseTag(tagName,
							...
							// If we didn't find any close tags between the open and our
							// close,
							// and there's no open tags remaining to find
							// then the open we found was the right one, and we can return
							// it
							if(0 == closeTagsFound && 0 == openTagsRemaining) {
								xmlTags.tagOpenStart = nextOpenTag.start;
								xmlTags.tagOpenEnd = nextOpenTag.end + 1;
								xmlTags.tagNameEnd = nextOpenTag.start +
								    static_cast<int32_t>(tagName.size()) + 1; /* +1 to account for '<' */
								tagFound = true;
							}
							else {
								// Need to find the same number of opening tags, without
								// closing tags etc.
				...
				if(tagName.size() != 0) {
					...
					if(-1 != closeAnglePosition) {
						...
						if(_pEditView->execute(SCI_GETCHARAT, closeAnglePosition - 1) == '/') {
							...
							/* Now we need to find the close tag.  The logic here is that we
							   search for "</TAGNAME",
							 * then check the next character - if it's '>' or whitespace
							 *followed by '>' then we've
							...
							do {
								...
								if(nextCloseTag.success) {
									--closeTagsRemaining;
									// Open tag found
									// Now we need to check how many close tags
									// there are between the open tag we just found,
									// and our close tag
									// eg. (Cursor == | )
									// <TAGNAM|E
									// attrib="value"><TAGNAME>something</TAGNAME></TAGNAME></TAGNAME>
									//                                            ^^^^^^^^
									// we've found this guy
									//                         ^^^^^^^^^ Now we need
									// to find this fella
									...
									do {
										inbetweenOpenTag = findOpenTag(tagName,
											currentStartPosition,
											currentEndPosition);
									...
									// If we didn't find any open tags between our
									// open and the close,
									// and there's no close tags remaining to find
									// then the close we found was the right one,
									...
									if(0 == openTagsFound && 0 == closeTagsRemaining) {
										...
										// Need to find the same number of
...
void XmlMatchedTagsHighlighter::tagMatch(bool doHiliteAttr)
{
	...
	if(lang == L_PHP || lang == L_ASP || lang == L_JSP) {
		...
		const int caret = 1 + static_cast<int32_t>(_pEditView->execute(SCI_GETCURRENTPOS)); // +1 to deal with the case when the caret is between the angle and the
			// question mark in "<?" (or between '<' and '%').
