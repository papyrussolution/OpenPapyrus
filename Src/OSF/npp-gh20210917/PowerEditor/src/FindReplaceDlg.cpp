...
{       //query may equal to result, since it always gets smaller
	...
	int charLeft = length;
	TCHAR current;
	while(i < length) { //because the backslash escape quences always reduce the size of the generic_string, no
		            // overflow checks have to be made for target, assuming parameters are correct
		current = query[i];
		--charLeft;
...
/*static*/int FASTCALL Searching::buildSearchFlags(const FindOption * option) 
{
		...
		case SCN_DOUBLECLICK:
			{
				...
				// WM_LBUTTONUP can go to a "File not found" messagebox instead of Scintilla here, because the mouse
				// is not captured.
...
void Finder::gotoFoundLine()
{
	...
	if(!::SendMessage(_hParent, WM_DOOPEN, 0, reinterpret_cast<LPARAM>(fInfo._fullPath.c_str()))) 
		return;
...
void Finder::deleteResult()
{
	...
	_scintView.setLexer(SCLEX_SEARCHRESULT, L_SEARCHRESULT, 0); // Restore searchResult lexer in case the lexer was
	                                                            // changed to SCLEX_NULL in GotoFoundLine()
...
void FindInFinderDlg::writeOptions()
{
	...
	_options._searchType =
	    isCheckedOrNot(IDREGEXP_FIFOLDER) ? FindRegex : isCheckedOrNot(IDEXTENDED_FIFOLDER) ? FindExtended : FindNormal;
...
void FindReplaceDlg::resizeDialogElements(LONG newWidth)
{
	...
	const auto moveWindowIDs = {
		...
		IDREPLACE, IDREPLACEALL, IDC_REPLACE_OPENEDFILES, IDD_FINDINFILES_FIND_BUTTON, IDD_FINDINFILES_REPLACEINFILES, IDOK,
		IDCANCEL,
	...
	for(int id : resizeWindowIDs) {
		...
		// Combo box for some reasons selects text on resize. So let's check befor resize if selection is
...
INT_PTR CALLBACK FindReplaceDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		...
		case WM_INITDIALOG:
	    {
			...
			pNativeSpeaker->getLocalizedStrFromID("shift-change-direction-tip",
			    TEXT("Use Shift+Enter to search in the opposite direction."));
		    ...
		    generic_string findInFilesFilterTip =
			pNativeSpeaker->getLocalizedStrFromID("find-in-files-filter-tip",
			    TEXT(
				    "Find in cpp, cxx, h, hxx && hpp:\r*.cpp *.cxx *.h *.hxx *.hpp\r\rFind in all files except exe, obj && log:\r*.* !*.exe !*.obj !*.log"));
		    _filterTip =
			CreateToolTip(IDD_FINDINFILES_FILTERS_STATIC, _hSelf, _hInst, const_cast<PTSTR>(findInFilesFilterTip.c_str()));
		...
		case WM_ACTIVATE:
	    {
		    ...
		    if(isCheckedOrNot(IDC_TRANSPARENT_LOSSFOCUS_RADIO)) {
			    if(LOWORD(wParam) == WA_INACTIVE && isVisible()) {
				    int percent =
					static_cast<int32_t>(::SendDlgItemMessage(_hSelf, IDC_PERCENTAGE_SLIDER, TBM_GETPOS, 0, 0));
				    (NppParameters::getInstance()).SetTransparent(_hSelf, percent);
			    }
			    else {
			    ...
			    case IDOK:      // Find Next : only for FIND_DLG and REPLACE_DLG
			{
				...
				if((_options._whichDirection == DIR_UP) && (_options._searchType == FindRegex) &&
					...
					if(findStatus == FSEndReached) {
						generic_string msg =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-end-reached",
							TEXT(
						...
						generic_string msg =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-top-reached",
							TEXT(
				...
				// restore search direction which may have been overwritten because shift-key was
				// pressed
...
//Process actions
			    ...
			    case IDREPLACEALL:
			{
				...
				if(_currentStatus == REPLACE_DLG) {
					...
					if((*_ppEditView)->getCurrentBuffer()->isReadOnly()) {
						NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
						generic_string msg = pNativeSpeaker->getLocalizedStrFromID("find-status-replace-readonly",
							TEXT("Replace: Cannot replace text. The current document is read only."));
					...
					if(nbReplaced < 0) {
						result =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-re-malformed",
							TEXT("Replace All: The regular expression is malformed."));
					}
					else {
						if(nbReplaced == 1) {
							result = pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-1-replaced",
								TEXT("Replace All: 1 occurrence was replaced"));
						}
						else {
							result = pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-nb-replaced",
								TEXT("Replace All: $INT_REPLACE$ occurrences were replaced"));
			    ...
			    case IDCCOUNTALL:
			{
				if(_currentStatus == FIND_DLG) {
					...
					if(nbCounted < 0) {
						result =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-count-re-malformed",
							TEXT("Count: The regular expression to search is malformed."));
					}
					else {
						if(nbCounted == 1) {
							result =
							    pNativeSpeaker->getLocalizedStrFromID("find-status-count-1-match",
								TEXT("Count: 1 match"));
						}
						else {
							result =
							    pNativeSpeaker->getLocalizedStrFromID("find-status-count-nb-matches",
								TEXT("Count: $INT_REPLACE$ matches"));
			    ...
			    case IDCMARKALL:
			{
				if(_currentStatus == MARK_DLG) {
					...
					if(nbMarked < 0) {
						result =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-mark-re-malformed",
							TEXT("Mark: The regular expression to search is malformed."));
					}
					else {
						if(nbMarked == 1) {
							result =
							    pNativeSpeaker->getLocalizedStrFromID("find-status-mark-1-match",
								TEXT("Mark: 1 match"));
						}
						else {
							result =
							    pNativeSpeaker->getLocalizedStrFromID("find-status-mark-nb-matches",
								TEXT("Mark: $INT_REPLACE$ matches"));
...
//Option actions
			...
			{
				if(_currentStatus == FINDINFILES_DLG)
					findHistory._isFifInHiddenFolder = _options._isInHiddenDir = isCheckedOrNot(
						    IDD_FINDINFILES_INHIDDENDIR_CHECK);
			}
				return TRUE;
			    ...
			    case IDD_FINDINFILES_PROJECT3_CHECK:
			{
				if(_currentStatus == FINDINPROJECTS_DLG) {
					...
					bool enable = _options._isProjectPanel_1 || _options._isProjectPanel_2 ||
					    _options._isProjectPanel_3;
			...
			{
				if(_currentStatus == FINDINFILES_DLG)
					folderBrowser(_hSelf,
					    TEXT("Select a folder to search from"),
					    IDD_FINDINFILES_DIR_COMBO,
...
bool FindReplaceDlg::processFindNext(const TCHAR * txt2find, const FindOption * options, FindStatus * oFindStatus,
    FindNextType findNextType /* = FINDNEXTTYPE_FINDNEXT */)
{
	if(oFindStatus)
		*oFindStatus = FSFound;
	...
	if(posFind == -1) { //no match found in target, check if a new target should be used
		...
		if(posFind == -1) { // not found
			...
			if(NotIncremental == pOptions->_incrementalType) { //incremental search doesnt trigger messages
				...
				NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
				generic_string msg =
				    pNativeSpeaker->getLocalizedStrFromID("find-status-cannot-find",
					TEXT("Find: Can't find the text \"$STR_REPLACE$\""));
	...
	else if(posFind < -1) { // error
		NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
		generic_string msgGeneral;
		if(posFind == -2) {
			msgGeneral = pNativeSpeaker->getLocalizedStrFromID("find-status-invalid-re",
				TEXT("Find: Invalid regular expression"));
		}
		else {
...
bool FindReplaceDlg::processReplace(const TCHAR * txt2find, const TCHAR * txt2replace, const FindOption * options)
{
	...
	if((*_ppEditView)->getCurrentBuffer()->isReadOnly()) {
		NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
		generic_string msg =
		    pNativeSpeaker->getLocalizedStrFromID("find-status-replace-readonly",
			TEXT("Replace: Cannot replace text. The current document is read only."));
		setStatusbarMessage(msg, FSNotFound);
		return false;
	}
	...
	if(moreMatches) {
		...
		if(nextFind.cpMin == currentSelection.cpMin && nextFind.cpMax == currentSelection.cpMax) {
			...
			if(nppGui._replaceStopsWithoutFindingNext) {
				generic_string msg =
				    pNativeSpeaker->getLocalizedStrFromID("find-status-replaced-without-continuing",
					TEXT("Replace: 1 occurrence was replaced."));
				...
				if(status == FSEndReached) {
					generic_string msg =
					    pNativeSpeaker->getLocalizedStrFromID("find-status-replace-end-reached",
						TEXT(
							"Replace: Replaced the 1st occurrence from the top. The end of document has been reached."));
					setStatusbarMessage(msg, FSEndReached);
				}
				else if(status == FSTopReached) {
					generic_string msg =
					    pNativeSpeaker->getLocalizedStrFromID("find-status-replace-top-reached",
						TEXT(
							"Replace: Replaced the 1st occurrence from the bottom. The begin of document has been reached."));
					setStatusbarMessage(msg, FSTopReached);
				}
				else {
					generic_string msg;
					if(moreMatches)
						msg =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-replaced-next-found",
							TEXT("Replace: 1 occurrence was replaced. The next occurrence found."));
					else
						msg =
						    pNativeSpeaker->getLocalizedStrFromID("find-status-replaced-next-not-found",
							TEXT("Replace: 1 occurrence was replaced. No more occurrences were found."));
		...
		NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
		generic_string msg =
		    pNativeSpeaker->getLocalizedStrFromID("find-status-replace-not-found", TEXT("Replace: no occurrence was found."));
		setStatusbarMessage(msg, FSNotFound);
	}
...
int FindReplaceDlg::processAll(ProcessOperation op,
    const FindOption * opt,
    bool isEntire,
    const FindersInfo * pFindersInfo,
    int colourStyleID)
{
	if(op == ProcessReplaceAll && (*_ppEditView)->getCurrentBuffer()->isReadOnly()) {
		NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
		generic_string msg =
		    pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-readonly",
			TEXT("Replace All: Cannot replace text. The current document is read only."));
		setStatusbarMessage(msg, FSNotFound);
		return 0;
	}
...
int FindReplaceDlg::processRange(ProcessOperation op, FindReplaceInfo & findReplaceInfo, const FindersInfo * pFindersInfo,
    const FindOption * opt, int colourStyleID, ScintillaEditView * view2Process)
{
	int nbProcessed = 0;
	...
	if(pOptions->_searchType == FindExtended) {
		...
		if(op == ProcessReplaceAll)
			stringSizeReplace =
			    Searching::convertExtendedToString(pTextReplace, pTextReplace, static_cast<int32_t>(stringSizeReplace));
	}
	...
	while(targetStart >= 0) {
		...
		switch(op) {
			...
			case ProcessFindInFinder:
		    {
			    ...
			    srm._start = start_mark;
			    srm._end = end_mark;
			    processed = (!pOptions->_isMatchLineNumber) ||
			    ...
			    if(processed) {
				    ...
				    pFindersInfo->_pDestFinder->add(FoundInfo(targetStart, targetEnd, lineNumber + 1, pFileName),
					srm,
					line.c_str());
			    }
			    break;
		    ...
		    {
			    int replacedLength = isRegExp ? pEditView->replaceTargetRegExMode(pTextReplace) : pEditView->replaceTarget(pTextReplace);
			...
			case ProcessMarkAll:
		    {
			    ...
			    // At least in version 2.27, after calling INDICATORFILLRANGE with length 0, further
			    // indicators
			    // on the same line would simply not be shown.  This may have been fixed in later version of
			    // Scintilla.
			    if(foundTextLen > 0) {
	...
	if(nbProcessed > 0) {
		...
		if(op == ProcessFindAll) {
			...
			pFinder = (pFindersInfo && pFindersInfo->_pDestFinder) ? pFindersInfo->_pDestFinder : _pFinder;
		}
		CALLPTRMEMB(pFinder, addFileHitCount(nbProcessed));
	}
	return nbProcessed;
}
...
void FindReplaceDlg::enableReplaceFunc(bool isEnable)
{
	...
	::MoveWindow(::GetDlgItem(_hSelf, IDCANCEL),
	    pClosePos->left + _deltaWidth,
	    pClosePos->top,
	    pClosePos->right,
	    pClosePos->bottom,
	    TRUE);
	::MoveWindow(::GetDlgItem(_hSelf, IDC_IN_SELECTION_CHECK),
	    pInSectionCheckPos->left + _deltaWidth,
	    pInSectionCheckPos->top,
	    pInSectionCheckPos->right,
	    pInSectionCheckPos->bottom,
	    TRUE);
	::MoveWindow(::GetDlgItem(_hSelf, IDC_REPLACEINSELECTION),
	    pInSelectionFramePos->left + _deltaWidth,
	    pInSelectionFramePos->top,
	    pInSelectionFramePos->right,
	    pInSelectionFramePos->bottom,
	    TRUE);
...
void FindReplaceDlg::saveInMacro(size_t cmd, int cmdType)
{
	...
	::SendMessage(_hParent, WM_FRSAVE_STR, IDFINDWHAT,
	    reinterpret_cast<LPARAM>(cmd == IDC_CLEAR_ALL ? TEXT("") : _options._str2Search.c_str()));
...
void FindReplaceDlg::finishFilesSearch(int count, int searchedCount, bool searchedEntireNotSelection)
{
...
{
	return _pFinder ? _pFinder->_scintView.getHSelf() : NULL;
}
			...
			case IDC_FRCOMMAND_EXEC:
		    {
			    ...
			    switch(intValue) {
					...
					if((_env->_whichDirection == DIR_UP) && (_env->_searchType == FindRegex) &&
					    !nppParamInst.regexBackward4PowerUser()) {
						// regex upward search is disabled
						// this macro step could have been recorded in an earlier version before
						// it was not allowed, or hand-edited
				    ...
				    case IDC_FINDNEXT:
					// IDC_FINDNEXT will not be recorded into new macros recorded with 7.8.5 and
					// later
					// stay playback compatible with 7.5.5 - 7.8.4 where IDC_FINDNEXT was allowed
					// but unneeded/undocumented
					nppParamInst._isFindReplacing = true;
				    ...
				    case IDC_FINDPREV:
					// IDC_FINDPREV will not be recorded into new macros recorded with 7.8.5 and
					// later
					// stay playback compatible with 7.5.5 - 7.8.4 where IDC_FINDPREV was allowed
					// but unneeded/undocumented
				    ...
				    case IDREPLACE:
					if((_env->_whichDirection == DIR_UP) &&
					    (_env->_searchType == FindRegex && !nppParamInst.regexBackward4PowerUser())) {
						// regex upward search is disabled
						// this macro step could have been recorded in an earlier version before
						// it was disabled, or hand-edited
						// make this a no-action macro step
					}
					else {
						nppParamInst._isFindReplacing = true;
				    ...
				    case IDREPLACEALL:
				{
					...
					NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
					if(nbReplaced < 0) {
						result = pNativeSpeaker->getLocalizedStrFromID("find-status-replaceall-re-malformed",
							TEXT("Replace All: The regular expression is malformed."));
					}
					else {
						if(nbReplaced == 1) {
				    ...
				    case IDCCOUNTALL:
				{
					...
					NativeLangSpeaker * pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
					if(nbCounted < 0) {
						result = pNativeSpeaker->getLocalizedStrFromID("find-status-count-re-malformed",
							TEXT("Count: The regular expression to search is malformed."));
					}
					else {
						if(nbCounted == 1) {
							result = pNativeSpeaker->getLocalizedStrFromID("find-status-count-1-match",
								TEXT("Count: 1 match"));
						}
						else {
							result = pNativeSpeaker->getLocalizedStrFromID("find-status-count-nb-matches",
								TEXT("Count: $INT_REPLACE$ matches"));
							result = stringReplace(result, TEXT("$INT_REPLACE$"), std::to_wstring(nbCounted));
						}
						result += TEXT(" ");
...
void FindReplaceDlg::enableFindInFilesFunc()
{
	...
	gotoCorrectTab();
	::MoveWindow(::GetDlgItem(_hSelf, IDCANCEL), _findInFilesClosePos.left + _deltaWidth, _findInFilesClosePos.top,
	    _findInFilesClosePos.right, _findInFilesClosePos.bottom, TRUE);
	TCHAR label[MAX_PATH];
	_tab.getCurrentTitle(label, MAX_PATH);
	::SetWindowText(_hSelf, label);
...
void FindReplaceDlg::enableFindInProjectsFunc()
{
	...
	gotoCorrectTab();
	::MoveWindow(::GetDlgItem(_hSelf, IDCANCEL),
	    _findInFilesClosePos.left + _deltaWidth,
	    _findInFilesClosePos.top,
...
void FindReplaceDlg::enableMarkFunc()
{
	...
	::MoveWindow(::GetDlgItem(_hSelf, IDCANCEL), _findInFilesClosePos.left + _deltaWidth, _findInFilesClosePos.top,
	    _findInFilesClosePos.right, _findInFilesClosePos.bottom, TRUE);
	::MoveWindow(::GetDlgItem(_hSelf, IDC_IN_SELECTION_CHECK), _replaceInSelCheckPos.left + _deltaWidth, _replaceInSelCheckPos.top,
	    _replaceInSelCheckPos.right, _replaceInSelCheckPos.bottom, TRUE);
	::MoveWindow(::GetDlgItem(_hSelf, IDC_REPLACEINSELECTION), _replaceInSelFramePos.left + _deltaWidth, _replaceInSelFramePos.top, 
		_replaceInSelFramePos.right, _replaceInSelFramePos.bottom, TRUE);
...
void FindReplaceDlg::drawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	COLORREF bgColor = NppDarkMode::isEnabled() ? NppDarkMode::getBackgroundColor() : getCtrlBgColor(_statusBar.getHSelf());
	...
	if(_statusbarTooltipMsg.length() == 0) 
		return;
	SIZE size;
...
bool FindReplaceDlg::replaceInFilesConfirmCheck(generic_string directory, generic_string fileTypes)
{
	...
	generic_string msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-files-confirm-directory",
		TEXT("Are you sure you want to replace all occurrences in :"));
	msg += TEXT("\r\r");
	msg += directory;
	msg += TEXT("\r\r");
...
bool FindReplaceDlg::replaceInProjectsConfirmCheck()
{
	...
	generic_string msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-projects-confirm-message",
		TEXT("Do you want to replace all occurrences in all documents in the selected Project Panel(s)?"));
	int res = ::MessageBox(NULL, msg.c_str(), title.c_str(), MB_OKCANCEL | MB_DEFBUTTON2 | MB_TASKMODAL);
	if(res == IDOK) {
		confirmed = true;
	}
...
bool FindReplaceDlg::replaceInOpenDocsConfirmCheck(void)
{
	...
	generic_string msg = pNativeSpeaker->getLocalizedStrFromID("replace-in-open-docs-confirm-message",
		TEXT("Are you sure you want to replace all occurrences in all open documents?"));
	int res = ::MessageBox(NULL, msg.c_str(), title.c_str(), MB_OKCANCEL | MB_DEFBUTTON2 | MB_TASKMODAL);
	if(res == IDOK) {
		confirmed = true;
	}
...
void Finder::addSearchHitCount(int count, int countSearched, bool isMatchLines, bool searchedEntireNotSelection)
{
	...
	NativeLangSpeaker* pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
	generic_string text = pNativeSpeaker->getLocalizedStrFromID(searchedEntireNotSelection ? "find-result-title-info" : "find-result-title-info-selections", TEXT(""));
	if(text.empty()) {
		...
		// selection variations:  "(1 hit in 1 selection of 1 searched)"     " (0 hits in 0 selections of 1
		// searched)"
		// line filter variation: "(1 hit in 1 file of 1 searched) - Line Filter Mode: only display the filtered
		// results"
	...
	if(isMatchLines) {
		generic_string lineFilterModeInfo =
		    pNativeSpeaker->getLocalizedStrFromID("find-result-title-info-extra",
			TEXT(" - Line Filter Mode: only display the filtered results"));
...
void Finder::setFinderStyle()
{
	...
	if(pStyler) {
		const NppStyle * pStyle = pStyler->findByID(SCE_SEARCHRESULT_CURRENT_LINE);
		if(pStyle) {
	...
	if(pStyleDefault) {
		...
		if(go.isEnable()) {
			const NppStyle * pStyleGlobalOverride = stylers.findByName(TEXT("Global override"));
	...
	switch(message) {
		...
		case WM_CONTEXTMENU:
	    {
		    if(HWND(wParam) == _scintView.getHSelf()) {
			    ...
			    generic_string purgeForEverySearch = pNativeSpeaker->getLocalizedStrFromID("finder-purge-for-every-search",
				    TEXT("Purge for every search"));
			    generic_string openAll = pNativeSpeaker->getLocalizedStrFromID("finder-open-all", TEXT("Open all"));
			    generic_string wrapLongLines =
				pNativeSpeaker->getLocalizedStrFromID("finder-wrap-long-lines", TEXT("Word wrap long lines"));
				...
				NppParameters::getInstance().getNativeLangSpeaker()->isRTL() ? TPM_RIGHTALIGN | TPM_LAYOUTRTL : TPM_LEFTALIGN, p.x, p.y, 0, _hSelf, NULL);
			    return TRUE;
		    }
	...
	switch(message) {
		...
		case WM_COMMAND:
	    {
		    ...
		    if(updateSearch) {
			    ...
			    // now does not match; so if Not Found, clear selection and put caret at beginning of what
			    // was
...
{
	return _hwnd ? (::WaitForSingleObject(_hActiveState, 0) != WAIT_OBJECT_0) : false;
}

void Progress::setInfo(const TCHAR * info) const
{
	if(_hwnd)
...
int Progress::createProgressWindow()
{
	...
	_hwnd = ::CreateWindowEx(WS_EX_APPWINDOW | WS_EX_TOOLWINDOW | WS_EX_OVERLAPPEDWINDOW, cClassName, _header, WS_POPUP | WS_CAPTION,
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, _hInst, (LPVOID)this);
	if(!_hwnd)
		return -1;
	...
	_hPBar = ::CreateWindowEx(0, PROGRESS_CLASS, TEXT("Progress Bar"), WS_CHILD | WS_VISIBLE | PBS_SMOOTH,
		xStartPos, yTextPos + textHeight, width - widthPadding, cPBheight, _hwnd, NULL, _hInst, NULL);
	...
	_hBtn = ::CreateWindowEx(0, TEXT("BUTTON"), TEXT("Cancel"), WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | BS_TEXT,
