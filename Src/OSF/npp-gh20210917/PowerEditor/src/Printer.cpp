...
{
	return ::PrintDlg(&_pdlg) ? doPrint(true) : 0;
}
...
{/*
	...
	HFONT fontHeader = ::CreateFont(headerLineHeight, 0, 0, 0, fontWeight, isFontItalic, FALSE, 0, 0, 0, 0, 0, 0, fontFace);
	...
	HFONT fontFooter = ::CreateFont(footerLineHeight, 0, 0, 0, fontWeight, isFontItalic, FALSE, 0, 0, 0, 0, 0, 0, fontFace);
	...
	while(lengthPrinted < lengthDoc) {
		bool printPage = (!(_pdlg.Flags & PD_PAGENUMS) || (pageNum >= _pdlg.nFromPage) && (pageNum <= _pdlg.nToPage));
		if(!justDoIt)
			printPage = false;
		TCHAR pageString[32];
		...
		if(printPage) {
			...
			if(nppGUI._printSettings.isHeaderPresent()) {
				...
				RECT rcw = {frPrint.rc.left, frPrint.rc.top - headerLineHeight - headerLineHeight / 2, frPrint.rc.right, frPrint.rc.top - headerLineHeight / 2};
				...
				if(headerM[0] != '\0') {
					...
					::ExtTextOut(_pdlg.hDC, ((frPrint.rc.right - frPrint.rc.left)/2 + frPrint.rc.left) - (size.cx/2),
					    frPrint.rc.top - headerLineHeight / 2, ETO_CLIPPED, &rcw, headerMiddle.c_str(), static_cast<int>(headerMiddle.length()), NULL);
				}
				// Right part
				if(headerR[0] != '\0') {
		...
		if(printPage) {
			if(nppGUI._printSettings.isFooterPresent()) {
				...
				RECT rcw = {frPrint.rc.left, frPrint.rc.bottom + footerLineHeight / 2, frPrint.rc.right, frPrint.rc.bottom + footerLineHeight + footerLineHeight / 2};
				SIZE size;
				// Left part
				if(footerL[0] != '\0') {
				...
				if(footerR[0] != '\0') {
					...
					::GetTextExtentPoint32(_pdlg.hDC, footerRight.c_str(), static_cast<int>(footerRight.length()),
					    &size);
