...
Shortcut::Shortcut() : _canModifyName(false) 
{
	setName(TEXT(""));
...
Shortcut::Shortcut(const TCHAR * name, bool isCtrl, bool isAlt, bool isShift, UCHAR key) : _canModifyName(false) 
{
	_name[0] = '\0';
...
/*virtual*/INT_PTR Shortcut::doDialog() 
{ 
...
/*virtual*/bool Shortcut::isValid() const 
{
	if(_keyCombo._key == 0)
...
/*virtual*/bool Shortcut::isEnabled() const { return (_keyCombo._key != 0); }
...
size_t FASTCALL ScintillaKeyMap::toKeyDef(size_t index) const 
{
	KeyCombo kc = _keyCombos[index];
...
{
	if(combo._key == 0 && (_size > 1)) { //remove the item if possible
...
{       //returns index where key is added, or -1 when invalid
	...
	for(size_t i = 0; i < _size; ++i) { //if already in the list do not add it
...
bool ScintillaKeyMap::isEnabled() const { return (_keyCombos[0]._key != 0); }
size_t ScintillaKeyMap::getSize() const { return _size; }
...
void getKeyStrFromVal(UCHAR keyVal, generic_string & str)
{
	...
	str = found ? namedKeyArray[i].name : TEXT("Unlisted");
}
...
bool recordedMacroStep::isMacroable() const
{
	...
	switch(_message) {
		...
		case SCI_SEARCHPREV: // (int searchFlags, const char *text)
		    return (_macroType == mtUseSParameter);
		...
		case SCI_NEWLINE:
		    return (_macroType == mtUseLParameter);
...
void recordedMacroStep::PlayBack(Window* pNotepad, ScintillaEditView * pEditView)
{
	if(_macroType == mtMenuCommand) {
		...
		if(_macroType == mtUseSParameter) {
			int byteBufferLength = ::WideCharToMultiByte(static_cast<UINT>(pEditView->execute(SCI_GETCODEPAGE)),
				0, _sParameter.c_str(), -1, NULL, 0, NULL, NULL);
			auto byteBuffer = std::make_unique< char[] >(byteBufferLength);
			::WideCharToMultiByte(static_cast<UINT>(pEditView->execute(SCI_GETCODEPAGE)),
			    0, _sParameter.c_str(), -1, byteBuffer.get(), byteBufferLength, NULL, NULL);
		...
		// then notify Notepad++
		if((_message == SCI_SETTEXT) || (_message == SCI_REPLACESEL) || (_message == SCI_ADDTEXT) || 
			(_message == SCI_ADDSTYLEDTEXT) || (_message == SCI_INSERTTEXT) || (_message == SCI_APPENDTEXT)) {
			SCNotification scnN;
			scnN.nmhdr.code = SCN_CHARADDED;
			scnN.nmhdr.hwndFrom = pEditView->getHSelf();
			scnN.nmhdr.idFrom = 0;
			scnN.ch = _sParameter.empty() ? 0 : _sParameter.at(0);
...
//This procedure uses _keyCombo as a temp. variable to store current settings which can then later be applied (by
// pressing OK)
