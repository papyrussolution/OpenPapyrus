...
void ColourPicker::init(HINSTANCE hInst, HWND parent)
{
	...
	_hSelf = ::CreateWindowEx(0, TEXT("Button"), TEXT("F"), WS_CHILD |  WS_VISIBLE, 0, 0, 25, 25, _hParent, NULL, _hInst, (LPVOID)0);
	if(!_hSelf)
...
void ColourPicker::drawForeground(HDC hDC)
{
	...
	if((((_currentColour      ) & 0xFF) +
	    ...
	    ((_currentColour >> 16) & 0xFF)) < 200)             //check if the color is too dark, if so, use white
		                                                // strikeout
	...
	switch(Message) {
		...
		case WM_ENABLE:
	    {
		    if((BOOL)wParam == FALSE) {
			    _currentColour =
				NppDarkMode::isEnabled() ? NppDarkMode::getDarkerBackgroundColor() : ::GetSysColor(COLOR_3DFACE);
			    redraw();
		    }
		    return TRUE;
	    }
