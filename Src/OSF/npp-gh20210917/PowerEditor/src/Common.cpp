...
int filter(uint code, struct _EXCEPTION_POINTERS *)
{
...
const wchar_t * WcharMbcsConvertor::char2wchar(const char * mbcs2Convert, UINT codepage, int lenMbcs, int * pLenWc, int * pBytesNotProcessed)
{
	...
	// Otherwise, test if we are cutting a multi-byte character at end of buffer
	...
	else { // For other encodings, ask system if there are any invalid characters; note that it will not correctly
	      // know if last character is cut when there are invalid characters inside the text
		...
		if(lenWc == 0 && GetLastError() == ERROR_NO_UNICODE_TRANSLATION) {
			...
			if(lenWc == 0) { // don't have to check that the error is still ERROR_NO_UNICODE_TRANSLATION,
			                 // since only the length parameter changed
				// TODO: should warn user about incorrect loading due to invalid characters
				// We still load the file, but the system will either strip or replace invalid
				// characters (including the last character, if cut in half)
				lenWc = MultiByteToWideChar(codepage, 0, mbcs2Convert, lenMbcs, NULL, 0);
			}
			else {
...
generic_string GetLastErrorAsString(DWORD errorCode)
{
	...
	SETIFZQ(errorCode, GetLastError());
	if(errorCode == 0)
...
HWND CreateToolTip(int toolID, HWND hDlg, HINSTANCE hInst, const PTSTR pszText)
{
	...
	if(toolID && hDlg && pszText) {
		...
		if(hwndTool) {
			...
			hwndTip = CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_ALWAYSTIP | TTS_BALLOON, CW_USEDEFAULT, CW_USEDEFAULT,
...
HWND CreateToolTipRect(int toolID, HWND hWnd, HINSTANCE hInst, const PTSTR pszText, const RECT rc)
{
	...
	// Create the tooltip. g_hInst is the global instance handle.
	HWND hwndTip = CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_ALWAYSTIP | TTS_BALLOON,
...
bool isCertificateValidated(const generic_string & fullFilePath, const generic_string & subjectName2check)
{
	...
	try {
		...
		result = CryptQueryObject(CERT_QUERY_OBJECT_FILE, fullFilePath.c_str(), CERT_QUERY_CONTENT_FLAG_PKCS7_SIGNED_EMBED, CERT_QUERY_FORMAT_FLAG_BINARY,
...
bool isAssoCommandExisting(LPCTSTR FullPathName)
{
	...
	if(isFileExisting) {
		...
		HRESULT hres = AssocQueryString(ASSOCF_VERIFY|ASSOCF_INIT_IGNOREUNKNOWN, ASSOCSTR_COMMAND, ext, NULL, buffer, &bufferLen);
		isAssoCommandExisting = (hres == S_OK) /*check if association exist and no error*/ && (buffer != NULL) /*check if buffer is not NULL*/ && 
