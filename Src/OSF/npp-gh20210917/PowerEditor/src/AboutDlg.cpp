...
INT_PTR CALLBACK AboutDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    LPCTSTR bitness = nppParam.archType() ==
			IMAGE_FILE_MACHINE_I386 ? TEXT("(32-bit)") : (nppParam.archType() ==
			IMAGE_FILE_MACHINE_AMD64 ? TEXT("(64-bit)") : TEXT("(ARM 64-bit)"));
		    ...
		    //_emailLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME),
		    // TEXT("https://notepad-plus-plus.org/news/v781-free-uyghur-edition/"));
		    //_emailLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME),
		    // TEXT("https://notepad-plus-plus.org/news/v792-stand-with-hong-kong/"));
		    //_emailLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME),
		    // TEXT("https://notepad-plus-plus.org/news/v791-pour-samuel-paty/"));
		...
		case WM_DRAWITEM:
	    {
		    ...
		    //HICON hIcon = (HICON)::LoadImage(_hInst, MAKEINTRESOURCE(IDI_JESUISCHARLIE), IMAGE_ICON, 64, 64,
		    // LR_DEFAULTSIZE);
		    //HICON hIcon = (HICON)::LoadImage(_hInst, MAKEINTRESOURCE(IDI_GILETJAUNE), IMAGE_ICON, 64, 64,
		    // LR_DEFAULTSIZE);
		    //HICON hIcon = (HICON)::LoadImage(_hInst, MAKEINTRESOURCE(IDI_SAMESEXMARRIAGE), IMAGE_ICON, 64, 64,
		    // LR_DEFAULTSIZE);
...
INT_PTR CALLBACK DebugInfoDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM /*lParam*/)
{
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    _debugInfoStr += nppParam.archType() ==
			IMAGE_FILE_MACHINE_I386 ? TEXT("   (32-bit)") : (nppParam.archType() ==
			IMAGE_FILE_MACHINE_AMD64 ? TEXT("   (64-bit)") : TEXT("   (ARM 64-bit)"));
		    _debugInfoStr += TEXT("\r\n");
		    // Build time
		    ...
		    if(RegOpenKeyExW(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion"), 0, KEY_READ,
			&hKey) == ERROR_SUCCESS) {
				...
				NULL,
				NULL,
				reinterpret_cast<LPBYTE>(szCurrentBuildNumber),
				&dataSize);
			    szCurrentBuildNumber[sizeof(szCurrentBuildNumber) / sizeof(TCHAR) - 1] = '\0';

			    dataSize = sizeof(DWORD);
			    if(RegQueryValueExW(hKey, TEXT("UBR"), NULL, NULL, reinterpret_cast<LPBYTE>(&dwUBR),
...
void DebugInfoDlg::init(HINSTANCE hInst, HWND parent, bool isAdmin, const generic_string& loadedPlugins) 
{
	_isAdmin = isAdmin;
...
void DoSaveOrNotBox::init(HINSTANCE hInst, HWND parent, const TCHAR* fn, bool isMulti) 
{
	Window::init(hInst, parent);
	if(fn)
		_fn = fn;
...
{
	generic_string msg;
	generic_string defaultMessage = TEXT(
		"Are you sure you want to save all modified documents?\r\rChoose \"Always Yes\" if you don't want to see this dialog again.\rYou can re-activate this dialog in Preferences later.");
	NativeLangSpeaker* nativeLangSpeaker = NppParameters::getInstance().getNativeLangSpeaker();
