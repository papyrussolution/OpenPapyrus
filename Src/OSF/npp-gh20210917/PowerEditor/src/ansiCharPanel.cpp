...
INT_PTR CALLBACK AnsiCharPanel::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    generic_string htmlNumberStr =
			pNativeSpeaker->getAttrNameStr(TEXT("HTML Number"), "AsciiInsertion", "ColumnHtmlNumber");
			    ...
			    case NM_DBLCLK:
					{
						...
						if(j != 2)
							...
							insertChar(static_cast<uchar>(i));
						return TRUE;
					}
			    case LVN_KEYDOWN:
					{
...
void AnsiCharPanel::insertChar(uchar char2insert) const
{
	char charStr[2];
	...
	if(codepage == -1) {
		...
		if(isUnicode) {
			MultiByteToWideChar(0, 0, charStr, -1, wCharStr, SIZEOFARRAY(wCharStr));
		...
		MultiByteToWideChar(codepage, 0, charStr, -1, wCharStr, SIZEOFARRAY(wCharStr));
		WideCharToMultiByte(CP_UTF8, 0, wCharStr, -1, multiByteStr, sizeof(multiByteStr), NULL, NULL);
	}
...
/*virtual*/void AnsiCharPanel::setBackgroundColor(int bgColour) const 
{
...
/*virtual*/void AnsiCharPanel::setForegroundColor(int fgColour) const 
{
