...
void PluginsAdminDlg::create(int dialogID, bool isRTL, bool msgDestParent)
{
	...
	::MoveWindow(hResearchLabel, researchLabelRect.left, researchLabelRect.top, researchLabelRect.right, researchLabelRect.bottom,
	    TRUE);
...
bool loadFromJson(PluginViewList & pl, const json& j)
{
	...
	for(const auto& i : jArray) {
		...
		catch(...)  // Every field is mandatory. If one of property is missing, an exception is thrown then this
