...
void ShortcutMapper::doDialog(bool isRTL) 
{
	if(isRTL) {
		...
		::DialogBoxParam(_hInst, MAKEINTRESOURCE(IDD_SHORTCUTMAPPER_DLG), _hParent, dlgProc,
		...
		case WM_COMMAND:
	    {
			    ...
			    case IDM_BABYGRID_CLEAR:
			{
				...
				switch(_currentState) {
					...
					case STATE_PLUGIN:
				    {
					    ...
					    ::SendMessage(_hParent, NPPM_INTERNAL_PLUGINSHORTCUTMOTIFIED, cmdID,
			    ...
			    case IDM_BABYGRID_DELETE:
			{
				if(_babygrid.getNumberRows() < 1)
					return TRUE;
				NppParameters& nppParam = NppParameters::getInstance();
				int res = nppParam.getNativeLangSpeaker()->messageBox("SCMapperDoDeleteOrNot", _hSelf,
					TEXT("Are you sure you want to delete this shortcut?"), TEXT("Are you sure?"), MB_OKCANCEL);
				if(res == IDOK) {
			    ...
			    case IDD_BABYGRID_ID1:
			{
				switch(HIWORD(wParam)) {
					...
					case BGN_ROWCHANGED:
				    {
					    ...
					    // In case of using filter will make the filtered items change index, so
					    // here we get its real index
						    ...
						    case STATE_MENU:
						{
							...
							findKeyConflicts(&conflictInfo,
							    vShortcuts[realIndexOfSelectedItem].getKeyCombo(),
							    realIndexOfSelectedItem);
						}
						break;
						    ...
						    case STATE_SCINTILLA:
						{
							...
							for(size_t sciIndex = 0; sciIndex < sciCombos; ++sciIndex)
								findKeyConflicts(&conflictInfo,
								    vShortcuts[realIndexOfSelectedItem].getKeyComboByIndex(
									    sciIndex),
								    realIndexOfSelectedItem);
						}
						break;
					    ...
					    if(conflictInfo.empty())
						    ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0,
							reinterpret_cast<LPARAM>(_conflictInfoOk.c_str()));
					    else
						    ::SendDlgItemMessage(_hSelf, IDC_BABYGRID_INFO, WM_SETTEXT, 0,
							reinterpret_cast<LPARAM>(conflictInfo.c_str()));
