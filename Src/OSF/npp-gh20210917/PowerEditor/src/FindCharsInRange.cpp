			    ...
			    case ID_FINDCHAR_NEXT:
					{
						...
						if(!getRangeFromUI(startRange, endRange)) {
							...
							NppParameters::getInstance().getNativeLangSpeaker()->messageBox("FindCharRangeValueError",
								_hSelf, TEXT("You should type between 0 and 255."), TEXT("Range Value problem"), MB_OK);
							return TRUE;
						}
...
bool FindCharsInRangeDlg::findCharInRange(uchar beginRange, uchar endRange, int startPos, bool direction, bool wrap)
{
	...
	for(int i = startPos-(direction == dirUp ? 1 : 0);
		...
		if(static_cast<uchar>(content[i]) >= beginRange && static_cast<uchar>(content[i]) <= endRange) {
			found = i;
			break;
		}
	}
	if(found == -1) {
		if(wrap) {
			for(int i = (direction == dirUp ? totalSize-1 : 0); (direction == dirDown) ? i < totalSize : i >= 0; (direction == dirDown) ? (++i) : (--i)) {
				if(static_cast<uchar>(content[i]) >= beginRange && static_cast<uchar>(content[i]) <= endRange) {
					found = i;
					break;
				}
...
bool FindCharsInRangeDlg::getRangeFromUI(uchar & startRange, uchar & endRange)
{
	...
	if(isCheckedOrNot(IDC_MYRANGE_RADIO)) {
		...
		if(start > end)
			return false;
		startRange = static_cast<uchar>(start);
		endRange = static_cast<uchar>(end);
		return true;
	}
	return false;
}
