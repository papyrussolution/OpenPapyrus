...
LRESULT CALLBACK hookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	if((nCode >= 0) && (wParam == WM_RBUTTONUP)) {
		...
		//to avoid duplicate messages, only send this message to the list control if it comes from outside the
		// control window. if the message occurs whilst the mouse is inside the control, the control will have
		// receive the mouse wheel message itself
...
int TaskListDlg::doDialog(bool isRTL)
{
	...
	return static_cast<int32_t>(::DialogBoxParam(_hInst, MAKEINTRESOURCE(IDD_TASKLIST_DLG), _hParent, dlgProc,
