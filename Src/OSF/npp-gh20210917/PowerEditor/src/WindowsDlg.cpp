...
struct BufferEquivalent {
	...
	bool compare(int i1, int i2) const
	{
		if(_iColumn >= 0 && _iColumn <= 3) {
			...
			return _strequiv(s1, s2) < 0;   //we can compare the full path to sort on directory, since after
			                                // sorting directories sorting files is the second thing to do
			                                // (if directories are the same that is)
		}
		return false;
	}
};
		...
		case WM_CONTEXTMENU:
	    {
		    if(!_listMenu.isCreated()) {
			    ...
			    const std::vector <MenuItemUnit> itemUnitArray
			    {
				    ...
				    {IDM_WINDOW_COPY_PATH, pNativeSpeaker->getAttrNameStr(TEXT("Copy Pathname(s)"),
					 WD_ROOTNODE,
					 WD_MENUCOPYPATH)}
			    };
...
BOOL WindowsDlg::onInitDialog()
{
	...
	LVCOLUMN lvColumn;
	memzero(&lvColumn, sizeof(lvColumn));
