	...
	if(nCode == HC_ACTION && hWndServer) {
		...
		if(pDockingManager) {
			...
			if(pCwp->message == WM_KILLFOCUS) {
				...
				for(int i = 0; i < DOCKCONT_MAX; ++i) {
					vcontainer[i]->SetActive(IsChild(vcontainer[i]->getHSelf(), pCwp->hwnd)); //activate the container that contains
						// the window with focus, this can be none
				}
			}
		}
	}
...
void DockingManager::init(HINSTANCE hInst, HWND hWnd, Window ** ppWin)
{
	...
	_hSelf = ::CreateWindowEx(0, DSPC_CLASS_NAME, TEXT(""), WS_CHILD | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT,
		CW_USEDEFAULT, CW_USEDEFAULT, _hParent, NULL, _hInst, (LPVOID)this);
	if(!_hSelf) {
...
void FASTCALL DockingManager::setStyleCaption(BOOL captionOnTop) 
{
	_vContainer[CONT_TOP]->setCaptionTop(captionOnTop);
	_vContainer[CONT_BOTTOM]->setCaptionTop(captionOnTop);
}

void FASTCALL DockingManager::setTabStyle(BOOL orangeLine)
{
	for(size_t i = 0; i < _vContainer.size(); ++i)
