...
PluginsManager::PluginsManager() : _dynamicIDAlloc(ID_PLUGINS_CMD_DYNAMIC, ID_PLUGINS_CMD_DYNAMIC_LIMIT), _markerAlloc(MARKER_PLUGINS, MARKER_PLUGINS_LIMIT)   
{
}
...
int PluginsManager::loadPlugin(const TCHAR * pluginFilePath)
{
	...
	try {
		...
		const DWORD dwFlags = GetProcAddress(GetModuleHandle(TEXT("kernel32.dll")),
			"AddDllDirectory") != NULL ? LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS : 0;
...
bool PluginsManager::getShortcutByCmdID(int cmdID, ShortcutKey * sk)
{
	...
	for(size_t i = 0, len = pluginCmdSCList.size(); i < len; ++i) {
		if(pluginCmdSCList[i].getID() == (ulong)cmdID) {
...
void PluginsManager::pluginExceptionAlert(const TCHAR * pluginName, const std::exception& e)
{
...
void PluginsManager::addInLoadedDlls(const TCHAR * fullPath, const TCHAR * fn) 
{
