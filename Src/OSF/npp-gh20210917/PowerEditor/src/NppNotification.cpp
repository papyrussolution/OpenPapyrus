	...
	switch(notification->nmhdr.code) {
		    ...
		    if(doPeekOnTab) {
			    ...
			    if(pTabDocView) {
				    ...
				    if(pBuf != currentBufMain && pBuf != currentBufSub) {    // if hover on other tab
					    _documentPeeker.doDialog(p, pBuf,
						*(const_cast<ScintillaEditView*>(pTabDocView->getScintillaEditView())));
				    }
			    ...
			    if(hWin == _pEditView->getHSelf()) {    // In the same view group
					...
					_pPublicInterface->getClassName()) == 0 && hWinParent != _pPublicInterface->getHSelf()) { // another Notepad++
					    ...
					    if(buf->isDirty()) {
						    ...
						    _nativeLangSpeaker.messageBox("CannotMoveDoc", _pPublicInterface->getHSelf(),
								TEXT("Document is modified, save it then try again."), TEXT("Move to new Notepad++ Instance"),
							MB_OK);
					    }
					    else {
						    ::SendMessage(hWinParent, NPPM_INTERNAL_SWITCHVIEWFROMHWND, 0,
							reinterpret_cast<LPARAM>(hWin));
						    ::SendMessage(hWinParent, WM_COPYDATA,
								reinterpret_cast<WPARAM>(_pPublicInterface->getHinst()), reinterpret_cast<LPARAM>(&fileNamesData));
						    if(!isInCtrlStat) {
		...
		case NM_CLICK:
		    if(notification->nmhdr.hwndFrom == _statusBar.getHSelf()) {
			    ...
			    if(lpnm->dwItemSpec == DWORD(STATUSBAR_TYPING_MODE)) {
				    ...
				    _statusBar.setText((_pEditView->execute(SCI_GETOVERTYPE)) ? TEXT("OVR") : TEXT("INS"),
		...
		case NM_RCLICK:
	    {
		    ...
		    if(!_tabPopupMenu.isCreated()) {
			    ...
			    itemUnitArray.push_back(MenuItemUnit(IDM_FILE_CONTAININGFOLDERASWORKSPACE,
				TEXT("Open Containing Folder as Workspace")));
			    itemUnitArray.push_back(MenuItemUnit(0, NULL));
			    ...
			    // IMPORTANT: If list above is modified, you have to change the value of
			    // tabContextMenuItemPos[] in localization.cpp file
		...
		case SCN_DOUBLECLICK:
	    {
		    ...
		    if(notification->modifiers == SCMOD_CTRL) {
			    ...
			    size_t position_of_click = (notification->position != -1) ? notification->position : _pEditView->execute(SCI_GETCURRENTPOS);
			    // Anonymous scope to limit use of the buf pointer (much easier to deal with std::string).
			    {
				    ...
				    if(nppGUI._delimiterSelectionOnEntireDocument) {
					    ...
					    // Compute the position of the click (relative to the beginning of the
					    // line).
			    ...
			    if(nppGUI._rightmostDelimiter == nppGUI._leftmostDelimiter) {
				    // If the delimiters are the same (e.g. they are both a quotation mark), choose the
				    // ones
				    ...
				    // The pair where the distance from the left delimiter to position_of_click is at a
				    // minimum is the one we're looking for.
				    ...
				    std::stack<uint> leftmost_delimiter_positions;
				    for(uint i = 0; i < bufstring.length(); ++i) {
					    if(bufstring.at(i) == nppGUI._leftmostDelimiter)
						    leftmost_delimiter_positions.push(i);
					    else if(bufstring.at(i) == nppGUI._rightmostDelimiter && !leftmost_delimiter_positions.empty()) {
						    uint matching_leftmost = leftmost_delimiter_positions.top();
						    ...
						    // If it was possible, leftmost_position and rightmost_position
						    // should be of type optional<uint>.
						    if(matching_leftmost <= position_of_click && i >= position_of_click && (leftmost_position == -1 ||
								matching_leftmost > static_cast<uint>(leftmost_position))) {
		    ...
		    else { // Double click with no modifiers
			    ...
			    if((notification->position < startPos) || (notification->position > endPos))
				    break;
			    // WM_LBUTTONUP goes to opening browser instead of Scintilla here, because the mouse is not
			    // captured.
			    // The missing message causes mouse cursor flicker as soon as the mouse cursor is moved to a
			    // position outside the text editing area.
		...
		case SCN_MACRORECORD:
		    _macro.push_back(recordedMacroStep(notification->message, notification->wParam, 
				notification->lParam, static_cast<int32_t>(_pEditView->execute(SCI_GETCODEPAGE))));
		    break;
		case SCN_PAINTED:
		    if(!notifyView)
			    return FALSE;
				...
				// Restoring a position must done after SCN_PAINTED notification so that it works in every
				// circumstances (including wrapped large file)
