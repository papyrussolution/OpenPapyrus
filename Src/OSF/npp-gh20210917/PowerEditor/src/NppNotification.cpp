			    ...
			    if(hWin == _pEditView->getHSelf()) {    // In the same view group
					...
					_pPublicInterface->getClassName()) == 0 && hWinParent != _pPublicInterface->getHSelf()) { // another Notepad++
					    ...
					    if(buf->isDirty()) {
						    ...
						    ::SendMessage(hWinParent,
							WM_COPYDATA,
							reinterpret_cast<WPARAM>(_pPublicInterface->getHinst()),
		    ...
		    if(notification->modifiers == SCMOD_CTRL) {
			    ...
			    // Anonymous scope to limit use of the buf pointer (much easier to deal with std::string).
			    {
				    ...
				    if(nppGUI._delimiterSelectionOnEntireDocument) {
					    ...
					    // line).
					    const auto line_position = notifyView->execute(SCI_POSITIONFROMLINE,
						    notifyView->getCurrentLineNumber());
			    ...
			    if(nppGUI._rightmostDelimiter == nppGUI._leftmostDelimiter) {
				    ...
				    for(uint i = 0; i < bufstring.length(); ++i) {
					    ...
					    else if(bufstring.at(i) == nppGUI._rightmostDelimiter &&
						!leftmost_delimiter_positions.empty()) {
						    ...
						    if(matching_leftmost <= position_of_click && i >= position_of_click &&
							(leftmost_position == -1 ||
			    ...
			    // Open URL
			    generic_string url =
				notifyView->getGenericTextAsString(static_cast<size_t>(startPos), static_cast<size_t>(endPos));
