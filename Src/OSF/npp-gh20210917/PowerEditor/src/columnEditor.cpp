		...
		case WM_COMMAND:
	    {
			    ...
			    case IDOK:
			{
				...
				if(isTextMode) {
					...
					if(repeat == 0) {
						repeat = 1;         // Without this we might get an infinite loop while
							            // calculating the set "numbers" below.
					}
					...
					if((*_ppEditView)->execute(SCI_SELECTIONISRECTANGLE) ||
						...
						// If required a message can be shown to user, that select column
						// properly or something similar
						if(colInfos.size() > 0) {
							std::sort(colInfos.begin(), colInfos.end(), SortInPositionOrder());
							(*_ppEditView)->columnReplace(colInfos,
							    initialNumber,
							    increaseNumber,
							    repeat,
							    format);
