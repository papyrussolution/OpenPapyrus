...
/*virtual*/void FunctionListPanel::display(bool toShow) const 
{
    DockingDlgInterface::display(toShow);
}

/*virtual*/void FunctionListPanel::setBackgroundColor(COLORREF bgColour) 
{
	TreeView_SetBkColor(_treeView.getHSelf(), bgColour);
	TreeView_SetBkColor(_treeViewSearchResult.getHSelf(), bgColour);
}

/*virtual*/void FunctionListPanel::setForegroundColor(COLORREF fgColour) 
{
			...
			case TVN_KEYDOWN:
		    {
			    ...
			    if(ptvkd->wVKey == VK_RETURN) {
				    ...
				    PostMessage(_hParent, WM_COMMAND, SCEN_SETFOCUS << 16,
					    reinterpret_cast<LPARAM>((*_ppEditView)->getHSelf()));
			    }
...
INT_PTR CALLBACK FunctionListPanel::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		...
		case WM_INITDIALOG:
	    {
			...
			TBSTYLE_TRANSPARENT | BTNS_AUTOSIZE | BTNS_SEP | TBSTYLE_TOOLTIPS;
			...
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hToolbarMenu, GWLP_WNDPROC,
			reinterpret_cast<LONG_PTR>(funclstToolbarProc)));
		    ...
		    for(size_t i = 0; i < nbIcons; ++i) {
			    ...
			    HBITMAP hBmp =
				static_cast<HBITMAP>(::LoadImage(_hInst, MAKEINTRESOURCE(icoID), IMAGE_BITMAP, iconSizeDyn, iconSizeDyn,
			...
			MAKELONG(nppParams._dpiManager.scaleX(16), nppParams._dpiManager.scaleY(16)));
		    ...
		    oldFunclstSearchEditProc =
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hSearchEdit, GWLP_WNDPROC,
			reinterpret_cast<LONG_PTR>(funclstSearchEditProc)));
