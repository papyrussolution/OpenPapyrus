...
void RunMacroDlg::init(HINSTANCE hInst, HWND hPere /*, ScintillaEditView **ppEditView*/) 
{
		...
		case WM_COMMAND:
	    {
		    ...
		    switch(wParam)
		    {
				...
				if((HIWORD(wParam) == CBN_SELCHANGE) && (LOWORD(wParam) == IDC_MACRO_COMBO)) {
					_macroIndex =
					    static_cast<int32_t>(::SendDlgItemMessage(_hSelf, IDC_MACRO_COMBO, CB_GETCURSEL, 0, 0));
					return TRUE;
				}
		    }
	    }
