...
MenuItemUnit::MenuItemUnit(ulong cmdID, const TCHAR * itemName, const TCHAR * parentFolderName) : _cmdID(cmdID)
{
	if(!itemName)
		_itemName.clear();
	else
...
void ContextMenu::display(const POINT & p) const { ::TrackPopupMenu(_hMenu, TPM_LEFTALIGN, p.x, p.y, 0, _hParent, NULL); }
...
void ContextMenu::create(HWND hParent, const std::vector<MenuItemUnit> & menuItemArray, const HMENU mainMenuHandle, bool copyLink)
{
	...
	for(size_t i = 0, len = menuItemArray.size(); i < len; ++i) {
		...
		uint flag = MF_BYPOSITION | ((item._cmdID == 0) ? MF_SEPARATOR : 0);
		if(hParentFolder) {
