...
static LRESULT CALLBACK hookProcMouse(int nCode, WPARAM wParam, LPARAM lParam)
{
	if(nCode >= 0) {
		switch(wParam) {
			case WM_MOUSEMOVE:
			case WM_NCMOUSEMOVE: ::PostMessage(hWndServer, UINT(wParam), 0, 0); break;
			case WM_LBUTTONUP: 
			case WM_NCLBUTTONUP: ::PostMessage(hWndServer, UINT(wParam), 0, 0); break;
			default: break;
		}
	}
...
tTbData* DockingCont::findToolbarByName(TCHAR* pszName)
{
	...
	// find entry by handle
	for(size_t iTb = 0, len = _vTbData.size(); iTb < len; ++iTb) {
		if(sstreq(pszName, _vTbData[iTb]->pszName))
			pTbData = _vTbData[iTb];
	}
	return pTbData;
}
...
LRESULT DockingCont::runProcCaption(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(Message) {
		...
		case WM_MOUSEMOVE:
	    {
		    ...
		    else if((_bCapTTHover == TRUE) && (_hoverMPos != isInRect(hwnd, LOWORD(lParam), HIWORD(lParam)))) {
			    toolTip.destroy();
			    _bCapTTHover = FALSE;
		    }
		    return TRUE;
...
eMousePos DockingCont::isInRect(HWND hwnd, int x, int y)
{
	...
	if(_isTopCaption == TRUE) {
		...
		else if((x > rc.right - (_closeButtonWidth + _closeButtonPosLeftDynamic)) &&
		    (x < (rc.right - _closeButtonPosLeftDynamic)) &&
		    ...
		    if((_beginDrag == TRUE) && (wParam == MK_LBUTTON)) {
			    ...
			    if((_bTabTTHover == FALSE) && (iItem != iItemSel)) {
				    ...
				    if(iItem == iItemSel) {
					    ...
					    toolTip.init(_hInst, hwnd);
					    toolTip.Show(rc, (reinterpret_cast<tTbData*>(tcItem.lParam))->pszName, info.pt.x,
						info.pt.y + 20);
				    }
			    }
...
void DockingCont::drawTabItem(DRAWITEMSTRUCT * pDrawItemStruct)
{
	...
	if(((tTbData*)tcItem.lParam)->uMask & DWS_ICONTAB) {
		...
		int iPosImage =
		    static_cast<int32_t>(::SendMessage(_hParent, DMM_GETICONPOS, 0,
		    reinterpret_cast<LPARAM>(reinterpret_cast<tTbData*>(tcItem.lParam)->hClient)));
...
INT_PTR CALLBACK DockingCont::run_dlgProc(UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch(Message) {
		...
		case WM_INITDIALOG:
	    {
		    ...
		    _hDefaultCaptionProc =
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hCaption, GWLP_WNDPROC,
			reinterpret_cast<LONG_PTR>(wndCaptionProc)));

		    // intial subclassing of tab
		    ::SetWindowLongPtr(_hContTab, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
		    _hDefaultTabProc =
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hContTab, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(wndTabProc)));
...
void DockingCont::selectTab(int iTab)
{
	if(iTab != -1) {
		...
		if(static_cast<uint>(iTab) != _prevItem) {
...
bool DockingCont::updateCaption()
{
	...
	if((((tTbData*)tcItem.lParam)->uMask & DWS_ADDINFO) && (lstrlen(((tTbData*)tcItem.lParam)->pszAddInfo) != 0)) {
		_pszCaption += TEXT(" - ");
		_pszCaption += ((tTbData*)tcItem.lParam)->pszAddInfo;
	}
	::SetWindowText(_isFloating ? _hSelf : _hCaption, _pszCaption.c_str());
	return true;
}
...
void DockingCont::focusClient()
{
	...
	if(iItem != -1) {
		...
		if(tbData->pszAddInfo && sstreq(tbData->pszAddInfo, DM_NOFOCUSWHILECLICKINGCAPTION))
			return;
