...
template <class T> struct ComTraits {
	static const GUID uid;
};

template <class T> const GUID ComTraits<T>::uid = __uuidof(T);
...
public:
	...
	const generic_string& getLastUsedFolder() const {
...
private:
	...
	static BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM param)
	{
		...
		if(IsWindowEnabled(hwnd) && GetClassName(hwnd, buffer, bufferLen) != 0) {
			if(lstrcmpi(buffer, _T("ComboBox")) == 0) {
				...
				// We use the first combo box, but there might be the others (file type dropdown,
				// address bar, etc).
				...
				if(hwndChild && !inst->_hwndNameEdit && !GetWindowLongPtr(hwndChild, GWLP_USERDATA)) {
					...
					inst->_fileNameProc =
					    (WNDPROC)SetWindowLongPtr(hwndChild, GWLP_WNDPROC, reinterpret_cast<LPARAM>(&FileNameWndProc));
				...
				// 3. Find by text. Set the localized text for the OK button beforehand
				// (IFileDialog::SetOkButtonLabel).
				//    Problem: Localization might be not installed; need to use the OS language not the
				// app language.
				// 4. Just get the first button found. Save/Open button has control ID value lower than
				// Cancel button.
				//    Problem: It may work or may not, depending on the initialization order or other
				// environment factors.
	...
	static LRESULT CALLBACK FileNameWndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		switch(msg) {
			case WM_SETFOCUS: inst->_monitorKeyboard = true; break;
			case WM_KILLFOCUS: inst->_monitorKeyboard = false; break;
		}
...
public:
	...
	bool init(CLSID id)
	{
		...
		if(_fileTypeIndex >= static_cast<int>(_filterSpec.size()))
			_fileTypeIndex = 0;
		HRESULT hr = CoCreateInstance(id, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&_dialog));
		_customize = _dialog;
	...
	bool initSave() { return init(CLSID_FileSaveDialog); }
	bool initOpen() { return init(CLSID_FileOpenDialog); }
	...
	bool show()
	{
		...
		if(SUCCEEDED(hr)) {
			...
			if(okPressed && params.getNppGUI()._openSaveDir == dir_last) {
				...
				// At least, after it is hidden, the current directory is the same as before it was
				// shown.
...
void CustomFileDialog::setDefExt(const TCHAR* ext) { _impl->_defExt = ext; }
void CustomFileDialog::setDefFileName(const TCHAR* fn) { _impl->_initialFileName = fn; }
void CustomFileDialog::setFolder(const TCHAR* folder) { _impl->_initialFolder = folder ? folder : _T(""); }
...
void CustomFileDialog::setExtIndex(int extTypeIndex) { _impl->_fileTypeIndex = extTypeIndex; }
bool CustomFileDialog::getCheckboxState() const { return _impl->getCheckboxState(IDC_FILE_CUSTOM_CHECKBOX); }
bool CustomFileDialog::isReadOnly() const { return _impl->_hasReadonly; }
