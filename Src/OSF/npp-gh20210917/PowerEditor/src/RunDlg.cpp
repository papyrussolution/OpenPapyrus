...
{
	if(sstreq(fullCurrentPath, str))
		return FULL_CURRENT_PATH;
	else if(sstreq(currentDirectory, str))
		return CURRENT_DIRECTORY;
	else if(sstreq(onlyFileName, str))
		return FILE_NAME;
	else if(sstreq(fileNamePart, str))
		return NAME_PART;
	else if(sstreq(fileExtPart, str))
		return EXT_PART;
	else if(sstreq(currentWord, str))
		return CURRENT_WORD;
	else if(sstreq(nppDir, str))
		return NPP_DIRECTORY;
	else if(sstreq(nppFullFilePath, str))
		return NPP_FULL_FILE_PATH;
	else if(sstreq(currentLine, str))
		return CURRENT_LINE;
	else if(sstreq(currentColumn, str))
			    ...
			    case IDC_BUTTON_SAVE:
					{
						...
						if(uc.doDialog() != -1) {
							...
							int const posBase = 2;
							if(nbCmd == 0)
								::InsertMenu(hRunMenu, posBase - 1, MF_BYPOSITION, static_cast<uint>(-1), 0);
							...
							if(nbCmd == 0) {
								...
								::InsertMenu(hRunMenu, posBase + nbCmd + 1, MF_BYPOSITION, static_cast<uint>(-1), 0);
