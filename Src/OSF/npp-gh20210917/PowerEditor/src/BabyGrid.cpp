...
void DisplayColumn(HWND hWnd, int SI, int c, int offset, HFONT hfont, HFONT hcolumnheadingfont)
{
	...
	while(r<=BGHS[SI].bottomvisiblerow) {
		...
		if((c==0)&&(BGHS[SI].ROWSNUMBERED)) {
			...
			// this will reduce some unnecessary and 'heavy' message calls for getting iDataType and
			// iProtection separately
			iProperty =
			    static_cast<int32_t>(SendMessage(hWnd, BGM_GETCELLDATA, reinterpret_cast<WPARAM>(&BGcell),
...
void SetHomeRow(HWND hWnd, int SI, int row, int col)
{
	...
	if((cellrect.bottom > gridrect.bottom)&&
	    ((cellrect.bottom - cellrect.top)<(gridrect.bottom-(BGHS[SI].headerrowheight+BGHS[SI].titleheight)))) {
...
LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message) {
	    ...
	    break;
		case BGM_GETROWS: ReturnValue = BGHS[SelfIndex].rows; break;
		case BGM_GETCOLS: ReturnValue = BGHS[SelfIndex].cols; break;
		case BGM_GETCOLWIDTH: ReturnValue = BGHS[SelfIndex].columnwidths[wParam]; break;
		case BGM_GETROWHEIGHT: ReturnValue = BGHS[SelfIndex].rowheight; break;
		case BGM_GETHEADERROWHEIGHT: ReturnValue = BGHS[SelfIndex].headerrowheight; break;
		case BGM_GETOWNERDRAWITEM: ReturnValue = BGHS[SelfIndex].ownerdrawitem; break;
		case BGM_DRAWCURSOR: DrawCursor(hWnd, static_cast<int32_t>(wParam)); break;
		case BGM_SETCURSORPOS:
		    DrawCursor(hWnd, SelfIndex);
		    if((((int)wParam <= BGHS[SelfIndex].rows)&&((int)wParam > 0)) && (((int)lParam <= BGHS[SelfIndex].cols)&&((int)lParam > 0))) {
		...
		case BGM_SETLASTVIEW:
		    if((((int)wParam <= BGHS[SelfIndex].rows) && ((int)wParam > 0)) && (((int)lParam <= BGHS[SelfIndex].rows) && ((int)lParam > 0))) {
		    ...
		    break;
		case BGM_SETINITIALCONTENT: BGHS[SelfIndex].INITIALCONTENT = (BOOL)wParam; break;
		...
		case BGM_SHOWINTEGRALROWS:
		    ...
		    RefreshGrid(hWnd);
		    break;
		case BGM_SETCOLAUTOWIDTH: BGHS[SelfIndex].COLAUTOWIDTH = (BOOL)wParam; break;
		case BGM_SETALLOWCOLRESIZE: BGHS[SelfIndex].ALLOWCOLUMNRESIZING = (BOOL)wParam; break;
		case BGM_PROTECTCELL:
		    ...
		    break;
		case BGM_NOTIFYROWCHANGED: NotifyRowChanged(hWnd, SelfIndex); break;
		case BGM_NOTIFYCOLCHANGED: NotifyColChanged(hWnd, SelfIndex); break;
		case BGM_SETPROTECT:
		    if((BOOL)wParam) {
		...
		case BGM_SETCELLDATA:
		    ...
		    iDataType = 1;
		    if(iDataType==1) {
			    wcscat_s(buffer, TEXT("A"));
		    }
		    if(iDataType==2) {
			    wcscat_s(buffer, TEXT("N"));
		    }
		    if(iDataType==3) {
			    wcscat_s(buffer, TEXT("T"));
		    }
		    if(iDataType==4) {
			    wcscat_s(buffer, TEXT("F"));
		    }
		    if(iDataType==5) {
			    wcscat_s(buffer, TEXT("G"));
		    }

		    wcscat_s(buffer, TEXT("|"));
		    wcscat_s(buffer, (TCHAR*)lParam);
		    FindResult =
			static_cast<int32_t>(SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(buffer)));
		...
		case BGM_SETHEADERROWHEIGHT:
		    ...
		    break;
		case BGM_GETHOMEROW: ReturnValue = BGHS[SelfIndex].homerow; break;
		case BGM_GETROW: ReturnValue = BGHS[SelfIndex].cursorrow; break;
		case BGM_GETCOL: ReturnValue = BGHS[SelfIndex].cursorcol; break;
		case BGM_GETTYPE:
		    LPBGcell = (_BGCELL*)wParam;
		    if(OutOfRange(LPBGcell)) {
			    wParam = MAKEWPARAM(GetMenu(hWnd), BGN_OUTOFRANGE);
 ...
 */
		    ...
		    if(wParam == VK_NEXT) {
			    ...
			    //get rows per page
			    GetClientRect(hWnd, &gridrect);
			    rpp = (gridrect.bottom - (BGHS[SelfIndex].headerrowheight+BGHS[SelfIndex].titleheight))/
				BGHS[SelfIndex].rowheight;
			    DrawCursor(hWnd, SelfIndex);
		...
		case WM_SIZE:
	    {
		    ...
		    //For a dynamic position (e.g. sizing of the parenet window) an adjustment to this function is
		    // needed!
...
int BinarySearchListBox(HWND lbhWnd, TCHAR* searchtext)
{
	...
	if(lbcount < 12) {
		...
		FindResult = static_cast<int32_t>(SendMessage(lbhWnd, LB_FINDSTRING, static_cast<uint>(-1), reinterpret_cast<LPARAM>(searchtext)));
