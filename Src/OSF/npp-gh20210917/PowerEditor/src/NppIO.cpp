...
BufferID Notepad_plus::doOpen(const generic_string& fileName,
    bool isRecursive,
    bool isReadOnly,
    int encoding,
    const TCHAR * backupFileName,
    FILETIME fileNameTimestamp)
{
	...
	if(res == SavingStatus::SaveWrittingFailed) {
		...
		if(_isAdministrator) {
			...
			if(isSnapshotMode) { // if both rememberSession && backup mode are enabled
				             // Open the 2nd Notepad++ instance in Admin mode, then close the 1st
				             // instance.
				...
				int openInAdminModeRes = _nativeLangSpeaker.messageBox("OpenInAdminModeWithoutCloseCurrent",
					_pPublicInterface->getHSelf(),
					TEXT(
						"The file cannot be saved and it may be protected.\rDo you want to launch Notepad++ in Administrator mode?"),
					TEXT("Save failed"),
					MB_YESNO);
...
bool Notepad_plus::fileCloseAll(bool doDeleteBackup, bool isSnapshotMode)
{
	...
	for(size_t i = 0; i < _mainDocTab.nbItem() && !noSaveToAll; ++i) {
		...
		if(buf->isUntitled() && buf->docLength() == 0) {
			...
			if(isSnapshotMode) {
				if(buf->getBackupFileName() == TEXT("") || !::PathFileExists(buf->getBackupFileName().c_str())) { //backup
				                                                                                                  // file
				                                                                                                  // has
				                                                                                                  // been
				                                                                                                  // deleted
					...
					int res = _nativeLangSpeaker.messageBox("NoBackupDoSaveFile",
						_pPublicInterface->getHSelf(),
						TEXT(
							"Your backup file cannot be found (deleted from outside).\rSave it otherwise your data will be lost\rDo you want to save file \"$STR_REPLACE$\" ?"),
						TEXT("Save"),
	...
	if(bothActive()) {
		...
		for(int32_t i = static_cast<int32_t>(_pNonDocTab->nbItem()) - 1; i >= 0; i--) { //close all from right
		                                                                                // to left
...
bool Notepad_plus::fileCloseAllButCurrent()
{
	...
	for(size_t i = 0; i < _mainDocTab.nbItem() && !noSaveToAll; ++i) {
		...
		if(buf->isUntitled() && buf->docLength() == 0) {
			...
			if(res == IDCANCEL) {
				for(int32_t j = static_cast<int32_t>(mainSaveOpIndex.size()) - 1; j >= 0; j--) {        //close
				                                                                                        // all
				                                                                                        // from
				                                                                                        // right
				                                                                                        // to
	...
	for(size_t i = 0; i < _subDocTab.nbItem() && !noSaveToAll; ++i) {
		...
		if(buf->isUntitled() && buf->docLength() == 0) {
			...
			if(res == IDCANCEL) {
				...
				for(int32_t j = static_cast<int32_t>(subSaveOpIndex.size()) - 1; j >= 0; j--) { //close
				                                                                                // all
				                                                                                // from
				                                                                                // right
				                                                                                // to
		...
		if(backup != bak_none) {
			...
			if(!::CopyFileEx(fn, fn_bak.c_str(), nullptr, nullptr, &doCancel, COPY_FILE_NO_BUFFERING)) {
				int res = _nativeLangSpeaker.messageBox("FileBackupFailed",
					_pPublicInterface->getHSelf(),
					TEXT(
						"The previous version of the file could not be saved into the backup directory at \"$STR_REPLACE$\".\r\rDo you want to save the current file anyways?"),
...
bool Notepad_plus::loadSession(Session & session, bool isSnapshotMode, bool shouldLoadFileBrowser)
{
	...
	for(size_t i = 0; i < session.nbMainFiles();) {
		...
		if(PathFileExists(pFn)) {
			if(isSnapshotMode && session._mainViewFiles[i]._backupFilePath != TEXT(""))
				lastOpened = doOpen(pFn,
					false,
					false,
					session._mainViewFiles[i]._encoding,
		...
		if(lastOpened != BUFFER_INVALID) {
			...
			if(isSnapshotMode && session._mainViewFiles[i]._backupFilePath != TEXT("") &&
			    PathFileExists(session._mainViewFiles[i]._backupFilePath.c_str()))
				buf->setDirty(true);
	...
	for(size_t k = 0; k < session.nbSubFiles();) {
		...
		if(PathFileExists(pFn)) {
			if(isSnapshotMode && session._subViewFiles[k]._backupFilePath != TEXT(""))
				lastOpened = doOpen(pFn,
					false,
					false,
					session._subViewFiles[k]._encoding,
		...
		if(lastOpened != BUFFER_INVALID) {
			...
			if(isSnapshotMode && session._subViewFiles[k]._backupFilePath != TEXT("") &&
			    PathFileExists(session._subViewFiles[k]._backupFilePath.c_str()))
				buf->setDirty(true);
