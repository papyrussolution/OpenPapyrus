...
void allowWmCopydataMessages(Notepad_plus_Window& notepad_plus_plus, const NppParameters& nppParameters, winVer ver)
{
	...
	if(ver >= WV_VISTA || ver == WV_UNKNOWN) {
		...
		if(hDll) {
			// According to MSDN ChangeWindowMessageFilter may not be supported in future versions of
			// Windows,
			...
			if(nppParameters.getWinVersion() == WV_VISTA) {
				...
				typedef BOOL (WINAPI *MESSAGEFILTERFUNCEX)(HWND hWnd, UINT message, DWORD action,
...
// Only white space in double quotes will be kept, such as file path argument or "-settingsDir=" argument (ex.:
// -settingsDir="c:\my settings\my folder\")
			...
			switch(cmdLinePtr[i]) {
				case '\"': //quoted filename, ignore any following whitespace
					if(!isStringInArg && i > 0 && cmdLinePtr[i - 1] == '=') {
						...
						isInFile = false;
						//because we dont want to leave in any quotes in the filename, remove them now (with
						// zero terminator)
						cmdLinePtr[i] = 0;
					}
...
generic_string getEasterEggNameFromParam(ParamVector & params, uchar & type)
{
	generic_string EasterEggName;
	if(!getParamValFromString(TEXT("-qn="), params, EasterEggName)) { // get internal easter egg
		if(!getParamValFromString(TEXT("-qt="), params, EasterEggName)) { // get user quote from cmdline
			                                                          // argument
			if(!getParamValFromString(TEXT("-qf="), params, EasterEggName))  // get user quote from a
					                                                 // content of file
				return TEXT("");
			else {
...
{
	Win32Exception::removeHandler(); //disable exception handler after excpetion, we dont want corrupt data
		// structurs to crash the exception handler
	::MessageBox(Notepad_plus_Window::gNppHWND, TEXT("Notepad++ will attempt to save any unsaved data. However, dataloss is very likely."),
	...
	if(res) {
		...
		::MessageBox(Notepad_plus_Window::gNppHWND, TEXT("Unfortunatly, Notepad++ was not able to save your work. We are sorry for any lost data."),
...
// -notepadStyleCmdline is also considered an ignored parameter here, as we don't want it to be part of the assembled
// file name
...
//
// Adapted from
// http://www.codeproject.com/KB/debug/postmortemdebug_standalone1.aspx#_Reading_a_Minidump_with%20Visual%20Stud
...
{
	static LPCTSTR msgTitle = TEXT("Notepad++ crash analysis");
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR pCmdLine, int)
{
	...
	if(showHelp)
		::MessageBox(NULL, Notepad_plus_Window::P_CommandArgHelp, TEXT("Notepad++ Command Argument Help"), MB_OK);
	...
	if(doFunctionListExport || doPrintAndQuit) { // export functionlist feature will serialize fuctionlist on the
		// disk, then exit Notepad++. So it's important to not launch into existing instance, and keep it silent.
		isMultiInst = true;
	...
	if((!isMultiInst) && (!TheFirstOne)) {
		...
		if(hNotepad_plus) {
			...
			int sw = ::IsZoomed(hNotepad_plus) ? SW_MAXIMIZE : (::IsIconic(hNotepad_plus) ? SW_RESTORE : 0);
			if(sw != 0)
	...
	if(TheFirstOne && isUpExist && isGtXP && isSignatureOK) {
		...
		if(doUpdateNpp) {
			...
			nppGui._autoUpdateOpt._nextUpdateDate = NppDate(nppGui._autoUpdateOpt._intervalDays);
		}
		// to be removed
	...
	try {
		...
		while(going) {
			...
			if(going) {
				...
				if(!notepad_plus_plus.isDlgsMsg(&msg)) {
					if(::TranslateAccelerator(notepad_plus_plus.getHSelf(), notepad_plus_plus.getAccTable(),
					    &msg) == 0) {
		...
		wsprintf(message,
		    TEXT("An exception occured. Notepad++ cannot recover and must be shut down.\r\nThe exception details are as follows:\r\n")
