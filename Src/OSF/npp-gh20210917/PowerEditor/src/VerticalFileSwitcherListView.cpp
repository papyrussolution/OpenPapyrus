...
void VerticalFileSwitcherListView::initList()
{
	...
	if(isExtColumn) {
		...
		insertColumn(extStr.c_str(), nppParams._dpiManager.scaleX(nppParams.getNppGUI()._fileSwitcherExtWidth), ++colIndex); //2nd
		                                                                                                                     // column
	}
	if(isPathColumn) {
		generic_string pathStr = pNativeSpeaker->getAttrNameStr(TEXT("Path"), FS_ROOTNODE, FS_CLMNPATH);
		insertColumn(pathStr.c_str(), nppParams._dpiManager.scaleX(nppParams.getNppGUI()._fileSwitcherPathWidth), ++colIndex); //2nd
		                                                                                                                       // column
		                                                                                                                       // if
		                                                                                                                       // .ext
		                                                                                                                       // is
	...
	for(size_t i = 0, len = taskListInfo._tlfsLst.size(); i < len; ++i) {
		...
		TaskLstFnStatus * tl = new TaskLstFnStatus(fileNameStatus._iView,
