...
bool SecurityGard::verifySignedLibrary(const std::wstring& filepath, NppModule module2check)
{
	...
	try {
		...
		PCCERT_CONTEXT context = ::CertFindCertificateInStore(hStore,
		...
		if(::CertNameToStr(X509_ASN_ENCODING, &context->pCertInfo->Subject, CERT_X500_NAME_STR, subject_buffer.get(),
		    subject_sze) <= 1) {
	...
	//
	// fields verifications - if status is true, and string to compare (from the parameter) is not empty, then do
	// compare
	//
