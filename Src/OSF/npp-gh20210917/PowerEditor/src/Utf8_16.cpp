...
u78 Utf8_16_Read::utf8_7bits_8bits()
{
	...
	utf8 * endx      = sx + m_nLen;
	while(sx<endx) {
		if(*sx == '\0') { // For detection, we'll say that NUL means not UTF8
			ASCII7only = 0;
			rv = 0;
			break;
		}
		else if((*sx & 0x80) == 0x0) { // 0nnnnnnn If the byte's first hex code begins with 0-7, it is an ASCII character.
			++sx;
		}
		else if((*sx & (0x80+0x40)) == 0x80) { // 10nnnnnn 8 through B cannot be first hex codes
			ASCII7only = 0;
			rv = 0;
			break;
		}
		else if((*sx & (0x80+0x40+0x20)) == (0x80+0x40)) { // 110xxxvv 10nnnnnn, 11 bit character
			ASCII7only = 0;
		...
		else if((*sx & (0x80+0x40+0x20+0x10)) == (0x80+0x40+0x20)) {    // 1110qqqq 10xxxxvv 10nnnnnn, 16 bit character
			ASCII7only = 0;
		...
		else if((*sx & (0x80+0x40+0x20+0x10+0x8)) == (0x80+0x40+0x20+0x10)) { // 11110qqq 10xxxxvv 10nnnnnn
			                                                              // 10mmmmmm, 21 bit character
			ASCII7only = 0;
...
UniMode Utf8_16_Read::determineEncoding(const uchar * buf, size_t bufLen)
{
	// detect UTF-16 big-endian with BOM
...
size_t Utf8_16_Write::convert(char * p, size_t _size)
{
	...
	switch(m_eEncoding) {
	    ...
	    {
		    utf16* pCur = NULL;
		    if(oneof2(m_eEncoding, uni16BE, uni16LE)) {
			    // Write the BOM
...
void Utf16_Iter::operator++()
{
	...
	switch(m_eState) {
		...
		case eSurrogate:
		    read();
		    if((m_nCur16 >= 0xDC00) && (m_nCur16 < 0xE000)) { // valid surrogate pair
