...
public:
	...
	int scaledScreenWidth() const { return scaledSystemMetricX(SM_CXSCREEN); }
	int scaledScreenHeight() const { return scaledSystemMetricY(SM_CYSCREEN); }
	// Scale rectangle from raw pixels to relative pixels.
	void scaleRect(__inout RECT * pRect) const
	{
	...
	int pointsToPixels(int pt) const { return MulDiv(pt, _dpiY, 72); };
...
private:
	...
	int scaledSystemMetricX(int nIndex) const { return MulDiv(GetSystemMetrics(nIndex), 96, _dpiX); }
	// This returns a 96-DPI scaled-down equivalent value for nIndex
	// For example, the value 120 at 120 DPI setting gets scaled down to 96
	// X and Y versions are provided, though to date all Windows OS releases
	// have equal X and Y scale values
	int scaledSystemMetricY(int nIndex) const { return MulDiv(GetSystemMetrics(nIndex), 96, _dpiY); }
};
//
