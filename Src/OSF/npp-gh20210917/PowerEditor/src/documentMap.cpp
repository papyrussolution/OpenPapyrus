...
void DocumentMap::scrollMap()
{
	if(_pMapView && _ppEditView) {
		...
		LRESULT lowerPos = (*_ppEditView)->execute(SCI_POSITIONFROMPOINT,
			rcEditView.right - rcEditView.left,
			rcEditView.bottom - rcEditView.top);
		...
		if(!(*_ppEditView)->isWrap()) { // not wrapped: mimic height of edit view
		...
		else { // wrapped: ask Scintilla, since in the map view the current range of edit view might be wrapped
			// differently
...
DocumentMap::DocumentMap() : DockingDlgInterface(IDD_DOCUMENTMAP) 
{
}

void DocumentMap::create(tTbData * data, bool isRTL)
{
...
/*virtual*/void DocumentMap::display(bool toShow) const 
{
...
INT_PTR CALLBACK DocumentMap::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		case WM_INITDIALOG:
	    {
		    _hwndScintilla = reinterpret_cast<HWND>(::SendMessage(_hParent, NPPM_CREATESCINTILLAHANDLE, 0,
				reinterpret_cast<LPARAM>(_hSelf)));
		    _pMapView = reinterpret_cast<ScintillaEditView *>(::SendMessage(_hParent, NPPM_INTERNAL_GETSCINTEDTVIEW, 0,
				reinterpret_cast<LPARAM>(_hwndScintilla)));
