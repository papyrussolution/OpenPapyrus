...
const TCHAR* TiXmlBase::GetEntity(const TCHAR* p, TCHAR* value)
{
	...
	for(i = 0; i<NUM_ENTITY; ++i) {
		if(generic_strncmp(entity[i].str, p, entity[i].strLength) == 0) {
			assert(static_cast<uint>(lstrlen(entity[i].str)) == entity[i].strLength);
			*value = entity[i].chr;
...
const TCHAR* TiXmlBase::ReadText(const TCHAR* p, TIXML_STRING * text, bool trimWhiteSpace, const TCHAR* endTag, bool caseInsensitive)
{
	*text = TEXT("");
	if(!trimWhiteSpace/*certain tags always keep whitespace*/ || !condenseWhiteSpace/*if true, whitespace is always kept*/) {
...
//	TiXmlParsingData data( p, prevData );
	...
	if(*p == '\'') {
		...
		value = TEXT("");
		while(p && *p /*existence*/ && !_istspace(*p) && *p != '\n' && *p != '\r'/*whitespace*/ && *p != '/' && *p != '>'/*tag end*/) {
			value += *p;
			++p;
		}
	}
