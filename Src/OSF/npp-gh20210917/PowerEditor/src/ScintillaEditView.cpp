...
/*virtual*/ScintillaEditView::~ScintillaEditView()
{
	--_refCount;
...
/*virtual*/void ScintillaEditView::destroy()
{
...
void ScintillaEditView::init(HINSTANCE hInst, HWND hPere)
{
	...
	_hSelf = ::CreateWindowEx(0, TEXT("Scintilla"), TEXT("Notepad++"), WS_CHILD | WS_VSCROLL | WS_HSCROLL | WS_CLIPCHILDREN | WS_EX_RTLREADING,
		0, 0, 100, 100, _hParent, NULL, _hInst, NULL);
	if(!_hSelf) {
...
void ScintillaEditView::setSpecialStyle(const NppStyle & styleToSet)
{
...
void ScintillaEditView::setStyle(NppStyle styleToSet)
{
	GlobalOverride & go = NppParameters::getInstance().getGlobalOverrideStyle();
	if(go.isEnable()) {
		const NppStyle * pStyle = NppParameters::getInstance().getMiscStylerArray().findByName(TEXT("Global override"));
		if(pStyle) {
			if(go.enableFg) {
...
void ScintillaEditView::setUserLexer(const TCHAR * userLangName)
{
	...
	UserLangContainer * userLangContainer =
	    userLangName ? NppParameters::getInstance().getULCFromName(userLangName) : _userDefineDlg._pCurrentUserLang;
	...
	execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>("fold"), reinterpret_cast<LPARAM>("1"));
	execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>("userDefine.isCaseIgnored"),
	    reinterpret_cast<LPARAM>(userLangContainer->_isCaseIgnored ? "1" : "0"));
	execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>("userDefine.allowFoldOfComments"),
	    reinterpret_cast<LPARAM>(userLangContainer->_allowFoldOfComments ? "1" : "0"));
	execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>("userDefine.foldCompact"),
	    reinterpret_cast<LPARAM>(userLangContainer->_foldCompact ? "1" : "0"));

	char name[] = "userDefine.prefixKeywords0";
	for(int i = 0; i<SCE_USER_TOTAL_KEYWORD_GROUPS; ++i) {
		itoa(i+1, (name+25), 10);
		execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>(name),
		    reinterpret_cast<LPARAM>(userLangContainer->_isPrefix[i] ? "1" : "0"));
	}
	...
	for(int i = 0; i < SCE_USER_KWLIST_TOTAL; ++i) {
		...
		if(globalMappper().setLexerMapper.find(i) != globalMappper().setLexerMapper.end()) {
			execute(SCI_SETPROPERTY, reinterpret_cast<WPARAM>(globalMappper().setLexerMapper[i].c_str()),
			    reinterpret_cast<LPARAM>(keyWords_char));
		}
			...
			for(size_t j = 0, len = strlen(keyWords_char); j<len && index < (max_char-1); ++j) {
				...
				if(keyWords_char[j] == '\\' &&
				    (keyWords_char[j+1] == '"' || keyWords_char[j+1] == '\'' || keyWords_char[j+1] == '\\')) {
					++j;
				...
				if(inDoubleQuote || inSingleQuote) {
					...
					else if(nonWSFound == true && keyWords_char[j-1] != '"' && keyWords_char[j+1] != '"' &&
					    keyWords_char[j+1] > ' ') {
	...
	for(const NppStyle & style : userLangContainer->_styles) {
...
void ScintillaEditView::setExternalLexer(LangType typeDoc)
{
	...
	if(pStyler) {
		for(const NppStyle & style : *pStyler) {
			setStyle(style);
			...
			if(style._keywordClass >= 0 && style._keywordClass <= KEYWORDSET_MAX) {
				...
				execute(SCI_SETKEYWORDS, style._keywordClass,
				    reinterpret_cast<LPARAM>(getCompleteKeywordList(keywordList, typeDoc, style._keywordClass)));
			}
		}
	}
}
...
void ScintillaEditView::setJsLexer()
{
	...
	if(pNewStyler) { // New js styler is available, so we can use it do more modern styling
		for(const NppStyle & style : *pNewStyler) {
			setStyle(style);
		}
		...
		if(pOldStyler) {
			for(NppStyle style : *pOldStyler) { //not by reference, but copy
...
void ScintillaEditView::defineDocType(LangType typeDoc)
{
	...
	NppStyle * pStyleDefault = stylers.findByID(STYLE_DEFAULT);
	if(pStyleDefault) {
	...
	switch(typeDoc) {
		...
		case L_ASCII:
	    {
		    ...
		    if(pStyler) {
			    const NppStyle * pStyle = pStyler->findByName(TEXT("DEFAULT"));
			    if(pStyle) {
	    ...
	    break;
		case L_SQL: setSqlLexer(); break;
		case L_VB: setVBLexer(); break;
		case L_PASCAL: setPascalLexer(); break;
		case L_PERL: setPerlLexer(); break;
		case L_PYTHON: setPythonLexer(); break;
		case L_BATCH: setBatchLexer(); break;
		case L_TEX: setTeXLexer(); break;
		case L_NSIS: setNsisLexer(); break;
		case L_BASH: setBashLexer(); break;
		case L_FORTRAN: setFortranLexer(); break;
		case L_FORTRAN_77: setFortran77Lexer(); break;
		case L_LISP: setLispLexer(); break;
		case L_SCHEME: setSchemeLexer(); break;
		case L_ASM: setAsmLexer(); break;
		case L_DIFF: setDiffLexer(); break;
		case L_PROPS: setPropsLexer(); break;
		case L_PS: setPostscriptLexer(); break;
		case L_RUBY: setRubyLexer(); break;
		case L_SMALLTALK: setSmalltalkLexer(); break;
		case L_VHDL: setVhdlLexer(); break;
		case L_KIX: setKixLexer(); break;
		case L_CAML: setCamlLexer(); break;
		case L_ADA: setAdaLexer(); break;
		case L_VERILOG: setVerilogLexer(); break;
		case L_AU3: setAutoItLexer(); break;
		case L_MATLAB: setMatlabLexer(); break;
		case L_HASKELL: setHaskellLexer(); break;
		case L_INNO: setInnoLexer(); break;
		case L_CMAKE: setCmakeLexer(); break;
		case L_YAML: setYamlLexer(); break;
		case L_COBOL: setCobolLexer(); break;
		case L_GUI4CLI: setGui4CliLexer(); break;
		case L_D: setDLexer(); break;
		case L_POWERSHELL: setPowerShellLexer(); break;
		case L_R: setRLexer(); break;
		case L_COFFEESCRIPT: setCoffeeScriptLexer(); break;
		case L_BAANC: setBaanCLexer(); break;
		case L_SREC: setSrecLexer(); break;
		case L_IHEX: setIHexLexer(); break;
		case L_TEHEX: setTEHexLexer(); break;
		case L_ASN1: setAsn1Lexer(); break;
		case L_AVS: setAVSLexer(); break;
...
BufferID ScintillaEditView::attachDefaultDoc()
{
	...
	_currentBufferID = id;
	_currentBuffer = buf;
	bufferUpdated(buf, BufferChangeMask);   //make sure everything is in sync with the buffer, since no reference
	                                        // exists
...
void ScintillaEditView::saveCurrentPos()
{
	...
	int32_t docLine = static_cast<int32_t>(execute(SCI_DOCLINEFROMVISIBLE, displayedLine));         //linenumber of
	                                                                                                // the line
	                                                                                                // displayed in
	                                                                                                // the top
	int32_t offset = displayedLine - static_cast<int32_t>(execute(SCI_VISIBLEFROMDOCLINE, docLine));                //use
	                                                                                                                // this
	                                                                                                                // to
	                                                                                                                // calc
	                                                                                                                // offset
...
LRESULT ScintillaEditView::execute(UINT Msg, WPARAM wParam, LPARAM lParam) const 
{
	try {
...
void ScintillaEditView::bufferUpdated(Buffer * buffer, int mask)
{
	...
	if(buffer == _currentBuffer) {
		...
		if(mask & BufferChangeUnicode) {
			int enc = CP_ACP;
			if(buffer->getUnicodeMode() == uni8Bit) { //either 0 or CJK codepage
...
bool ScintillaEditView::isFoldIndentationBased() const
{
	...
	return oneof6(lexer, SCLEX_PYTHON, SCLEX_COFFEESCRIPT, SCLEX_HASKELL, SCLEX_NIMROD, SCLEX_VB, SCLEX_YAML);
}
...
void ScintillaEditView::getLine(size_t lineNumber, TCHAR * line, int lineBufferLen)
{
	...
	memzero(lineA, sizeof(char) * lineBufferLen);
...
{
	int mode = (meth == LINEWRAP_ALIGNED) ? SC_WRAPINDENT_SAME : (meth == LINEWRAP_INDENT) ? SC_WRAPINDENT_INDENT : SC_WRAPINDENT_FIXED;
...
void ScintillaEditView::performGlobalStyles()
{
	...
	const NppStyle * pStyle = stylers.findByName(TEXT("Current line background colour"));
	if(pStyle) {
		...
		case SENTENCECASE_BLEND:
	    {
		    ...
		    for(int i = 0; i < nbChars; ++i) {
			    if(::IsCharAlphaW(strWToConvert[i])) {
				    ...
				    wasEolN = false;
				    //An exception
				    if(strWToConvert[i] == L'i' && ((i < 1) ? false : (::iswspace(strWToConvert[i - 1]) || strWToConvert[i - 1] == L'(' ||
						strWToConvert[i - 1] == L'"')) && ((i + 1 == nbChars) ? false : (::iswspace(strWToConvert[i + 1]) ||
						strWToConvert[i + 1] == L'\''))) {
					    strWToConvert[i] = L'I';
				    }
			    }
...
TCHAR * int2str(TCHAR * str, int strLen, int number, int base, int nbChiffre, bool isZeroLeading)
{
	...
	if(base == 16)
		...
		const uint MASK_ULONG_BITFORT = 0x80000000;
		int nbBits = sizeof(uint) * 8;
		int nbBit2Shift = (nbChiffre >= nbBits) ? nbBits : (nbBits - nbChiffre);
		ulong mask = MASK_ULONG_BITFORT >> nbBit2Shift;
		int i = 0;
		for(; mask > 0; ++i) {
...
void ScintillaEditView::foldChanged(size_t line, int levelNow, int levelPrev)
{
	...
	else if(!(levelNow & SC_FOLDLEVELWHITEFLAG) && ((levelPrev & SC_FOLDLEVELNUMBERMASK) > (levelNow & SC_FOLDLEVELNUMBERMASK))) {
...
void FASTCALL ScintillaEditView::clearIndicator(int indicatorNumber) 
{
	int docStart = 0;
...
bool ScintillaEditView::getIndicatorRange(int indicatorNumber, int * from, int * to, int * cur)
{
	...
	if((curPos < startPos) || (curPos > endPos))
		return false;
	ASSIGN_PTR(from, startPos);
	ASSIGN_PTR(to, endPos);
	ASSIGN_PTR(cur, curPos);
	return true;
};
...
void ScintillaEditView::hideLines()
{
	...
	//Special func on buffer. If markers are added, create notification with location of start, and hide bool set to
	// true
	...
	if(scope != 0) { //something went wrong. Someone managed to make overlapping hidelines sections. We cant do anything since this isnt supposed to happen
	}
...
void ScintillaEditView::setBorderEdge(bool doWithBorderEdge)
{
	...
	if(NppDarkMode::isEnabled()) {
		...
		SETFLAG(style, WS_BORDER, doWithBorderEdge);
	}
	else {
		style &= ~WS_BORDER;
		SETFLAG(exStyle, WS_EX_CLIENTEDGE, doWithBorderEdge);
	}
...
{
	StyleArray & stylers = NppParameters::getInstance().getMiscStylerArray();
	const NppStyle * pStyle = stylers.findByName(TEXT("Fold"));
	if(pStyle) {
...
void ScintillaEditView::markedTextToClipboard(int indiStyle, bool doAll /*= false*/)
{
	...
	if(styledVect.size() > 0) {
		...
		const generic_string delim = (textContainsLineEndingChar && styledVect.size() > 1) ? TEXT("\r\n----\r\n") : TEXT("\r\n");
