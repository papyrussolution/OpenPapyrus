...
INT_PTR CALLBACK HashFromFilesDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) {
		case WM_INITDIALOG:
	    {
		    ...
		    _oldHashPathEditProc =
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(hHashPathEdit, GWLP_WNDPROC,
			reinterpret_cast<LONG_PTR>(HashPathEditStaticProc)));

		    ::SetWindowLongPtr(hHashResult, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
		    _oldHashResultProc =
			reinterpret_cast<WNDPROC>(::SetWindowLongPtr(hHashResult, GWLP_WNDPROC,
			reinterpret_cast<LONG_PTR>(HashResultStaticProc)));
	    }
		    return TRUE;
