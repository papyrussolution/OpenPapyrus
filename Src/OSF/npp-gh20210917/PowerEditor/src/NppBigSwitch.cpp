...
LRESULT CALLBACK Notepad_plus_Window::Notepad_plus_Proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message) {
		case WM_NCCREATE:
	    {
		    ...
		    Notepad_plus_Window * pM30ide =
			static_cast<Notepad_plus_Window *>((reinterpret_cast<LPCREATESTRUCT>(lParam))->lpCreateParams);
		    pM30ide->_hSelf = hwnd;
		...
		default:
	    {
		    return (reinterpret_cast<Notepad_plus_Window *>(::GetWindowLongPtr(hwnd, GWLP_USERDATA))->runProc(hwnd, message, wParam,
			   lParam));
	    }
	}
}
...
LRESULT Notepad_plus::process(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if(NppDarkMode::isDarkMenuEnabled() && NppDarkMode::isEnabled() &&
	    NppDarkMode::runUAHWndProc(hwnd, message, wParam, lParam, &result)) {
		return result;
	}
	switch(message) {
		case WM_NCACTIVATE:
		...
		case WM_FINDINFILES: return findInFiles();
		case WM_FINDINPROJECTS: return findInProjects();
	    ...
	    {
		    // Find in files function code should be here due to the number of parameters (2) cannot be passed
		    // via WM_COMMAND
...
#ifdef DEBUG
					printStr(TEXT(
						    "sizeof(CmdLineParams) != cmdLineParamsSize\rCmdLineParams is formed by an instance of another version,\rwhereas your CmdLineParams has been modified in this instance."));
#endif
				}
		...
		case NPPM_INTERNAL_CHANGETABBAEICONS:
		    _mainDocTab.changeIcons(static_cast<uchar>(lParam));
		    _subDocTab.changeIcons(static_cast<uchar>(lParam));
		    return TRUE;
		...
		case NPPM_GETEXTPART:
	    {
		    ...
		    // For the compability reason, if wParam is 0, then we assume the size of generic_string buffer
		    // (lParam) is large enough.
		...
		case NPPM_GETCURRENTLINE: return _pEditView->getCurrentLineNumber();
		case NPPM_GETCURRENTCOLUMN: return _pEditView->getCurrentColumnNumber();
		...
		case WM_FRSAVE_INT:
	    {
		    _macro.push_back(recordedMacroStep(static_cast<int32_t>(wParam), 0, static_cast<long>(lParam), NULL,
			recordedMacroStep::mtSavedSnR));
		    break;
	    }

		case WM_FRSAVE_STR:
	    {
		    _macro.push_back(recordedMacroStep(static_cast<int32_t>(wParam), 0, 0, reinterpret_cast<const TCHAR *>(lParam),
			recordedMacroStep::mtSavedSnR));
		    break;
	    }
		    ...
		    if(!_recordingMacro) {    // if we're not currently recording, then playback the recorded keystrokes
			    ...
			    for(;;) {
				    ...
				    if(times >= 0) {
					    ...
					    // Update the line count, but only if the number of lines remaining is
					    // shrinking.
		...
		case NPPM_INTERNAL_FINDKEYCONFLICTS:
	    {
		    ...
		    *reinterpret_cast<bool*>(lParam) =
			_pShortcutMapper->findKeyConflicts(nullptr, *reinterpret_cast<KeyCombo*>(wParam), (size_t)-1);
		...
		case WM_CONTEXTMENU:
		    ...
		    else if((HWND(wParam) == _mainEditView.getHSelf()) || (HWND(wParam) == _subEditView.getHSelf())) {
				switchEditViewTo(((HWND(wParam) == _mainEditView.getHSelf())) ? MAIN_VIEW : SUB_VIEW);
				POINT p;
				::GetCursorPos(&p);
		...
		case WM_UPDATESCINTILLAS:
	    {
		    ...
		    const NppStyle * pStyle = NppParameters::getInstance().getGlobalStylers().findByID(STYLE_DEFAULT);
		    if(pStyle) {
		...
		case WM_CLOSE:
	    {
		    if(_pPublicInterface->isPrelaunch()) {
			    ...
			    if(nppgui._cloudPath != TEXT("") && nppParam.isCloudPathChanged()) {
				    ...
				    if(!isOK) {
					    _nativeLangSpeaker.messageBox("SettingsOnCloudError", hwnd,
						...
						TEXT("Settings on Cloud"), MB_OK | MB_APPLMODAL);
			    ...
			    if(!updaterFullPath.empty()) {
				    Process updater(updaterFullPath.c_str(),
					nppParam.getWingupParams().c_str(),
					nppParam.getWingupDir().c_str());
				    updater.run(nppParam.shouldDoUAC());
			    }
		    }

		    // _isEndingSessionButNotReady is true means WM_QUERYENDSESSION is sent but no time to finish saving
		    // data
		...
		case NPPM_ADDTOOLBARICON_DEPRECATED:
	    {
			...
			reinterpret_cast<toolbarIcons*>(lParam),
	    ...
	    {
		    ::CheckMenuItem(_mainMenuHandle, static_cast<UINT>(wParam),
			MF_BYCOMMAND | (static_cast<BOOL>(lParam) ? MF_CHECKED : MF_UNCHECKED));
