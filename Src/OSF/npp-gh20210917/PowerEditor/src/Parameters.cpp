...
{
	if(sstreq(TEXT("instre1"), str)) return LANG_INDEX_INSTR;
	if(sstreq(TEXT("instre2"), str)) return LANG_INDEX_INSTR2;
	if(sstreq(TEXT("type1"), str)) return LANG_INDEX_TYPE;
	if(sstreq(TEXT("type2"), str)) return LANG_INDEX_TYPE2;
	if(sstreq(TEXT("type3"), str)) return LANG_INDEX_TYPE3;
	if(sstreq(TEXT("type4"), str)) return LANG_INDEX_TYPE4;
	if(sstreq(TEXT("type5"), str)) return LANG_INDEX_TYPE5;
	if(sstreq(TEXT("type6"), str)) return LANG_INDEX_TYPE6;
	if(sstreq(TEXT("type7"), str)) return LANG_INDEX_TYPE7;
...
int base64ToAscii(char * dest, const char * base64Str)
{
	...
	for(; i < nbLoop; i++) {
		size_t j = i * 4;
		UCHAR p0, p1;
		UCHAR uc0 = (UCHAR)base64IndexArray[base64Str[j]];
		UCHAR uc1 = (UCHAR)base64IndexArray[base64Str[j+1]];
		UCHAR uc2 = (UCHAR)base64IndexArray[base64Str[j+2]];
		UCHAR uc3 = (UCHAR)base64IndexArray[base64Str[j+3]];
...
static const LocalizationSwitcher::LocalizationDefinition localizationDefs[] = {
...
winVer NppParameters::getWindowsVersion()
{
	...
	switch(si.wProcessorArchitecture) {
		case PROCESSOR_ARCHITECTURE_IA64: _platForm = PF_IA64; break;
		case PROCESSOR_ARCHITECTURE_AMD64: _platForm = PF_X64; break;
		case PROCESSOR_ARCHITECTURE_INTEL: _platForm = PF_X86; break;
		default: _platForm = PF_UNKNOWN;
	}
...
bool NppParameters::reloadStylers(const TCHAR* stylePath)
{
	...
	if(!loadOkay) {
		if(!_pNativeLangSpeaker) {
			...
			_pNativeLangSpeaker->messageBox("LoadStylersFailed", NULL,
			    TEXT("Load \"$STR_REPLACE$\" failed!"), TEXT("Load stylers.xml failed"), MB_OK, 0, stylePathToLoad);
		}
		delete _pXmlUserStylerDoc;
...
bool NppParameters::load()
{
	...
	if(!_cmdSettingsDir.empty()) {
		if(!::PathIsDirectory(_cmdSettingsDir.c_str())) {
			...
			// _pNativeLangSpeaker is initialized AFTER _userPath being dterminated because nativeLang.xml
			// is from from _userPath.
...
bool NppParameters::isExistingExternalLangName(const TCHAR * newName) const
{
	if(isempty(newName))
		...
		for(int i = 0; i < _nbExternalLang; ++i) {
			if(sstreq(_externalLangArray[i]->_name, newName))
				return true;
		}
		return false;
	}
...
bool NppParameters::isExistingUserLangName(const TCHAR * newName) const
{
	if(isempty(newName))
		return true;
	else {
		for(int i = 0; i < _nbUserLang; ++i) {
			if(sstreq(_userLangArray[i]->_name.c_str(), newName))
				return true;
		}
		return false;
	}
...
const TCHAR * NppParameters::getUserDefinedLangNameFromExt(TCHAR * ext, TCHAR * fullName) const
{
	...
	for(int i = 0; i < _nbUserLang; ++i) {
		...
		for(size_t j = 0, len = extVect.size(); j < len; ++j) {
			if(!generic_stricmp(extVect[j].c_str(),
			    ext) || (_tcschr(fullName, '.') && !generic_stricmp(extVect[j].c_str(), fullName))) {
...
{
	for(int i = 0; i < _nbExternalLang; ++i) {
		if(sstreq(externalLangName, _externalLangArray[i]->_name))
			return i;
	}
	return -1;
}
...
{
	for(int i = 0; i < _nbUserLang; ++i) {
		if(sstreq(userLangName, _userLangArray[i]->_name.c_str()))
...
{
	const NppStyle * pStyle = _widgetStyleArray.findByName(TEXT("Current line background colour"));
	if(!pStyle)
		return COLORREF(-1);
...
{
	NppStyle * pStyle = _widgetStyleArray.findByName(TEXT("Current line background colour"));
	if(pStyle)
...
static int CALLBACK EnumFontFamExProc(const LOGFONT* lpelfe, const TEXTMETRIC*, DWORD, LPARAM lParam)
{
	...
	for(int i = vectSize - 1; i >= 0; i--) {
		if(sstreq(strVect[i].c_str(), lfFaceName))
...
std::pair<uchar, uchar> NppParameters::addUserDefineLangsFromXmlTree(TiXmlDocument * tixmldoc)
{
	if(!tixmldoc)
		return std::make_pair(static_cast<uchar>(0), static_cast<uchar>(0));
...
void NppParameters::initScintillaKeys()
{
	...
	for(int i = 0; i < SIZEOFARRAY(scintKeyDefs); ++i) {
		...
		if(skd.functionId == prevID) {
			...
			kc._key = static_cast<uchar>(skd.vKey);
			_scintillaKeyCommands[prevIndex].addKeyCombo(kc);
		}
		else {
			Shortcut s = Shortcut(skd.name, skd.isCtrl, skd.isAlt, skd.isShift, static_cast<uchar>(skd.vKey));
		...
		for(TiXmlNodeA * childNode = contextMenuRoot->FirstChildElement("Item"); childNode; childNode = childNode->NextSibling("Item")) {
			...
			if(idStr) {
				...
				if(!menuEntryName.empty() && !menuItemName.empty()) {
					...
					// if plugin menu existing plls the value of PluginEntryName and
					// PluginCommandItemName are valid
		...
		if(viewRoots[k]) {
			...
			for(TiXmlNode * childNode = viewRoots[k]->FirstChildElement(TEXT("File")); childNode; childNode = childNode->NextSibling(TEXT("File"))) {
				...
				if(fileName) {
					...
					if(boolStr)
						mapPosition._isWrap = sstreq(TEXT("yes"), boolStr);
					...
					(childNode->ToElement())->Attribute(TEXT("originalFileLastModifTimestamp"),
					    reinterpret_cast<int32_t*>(&fileModifiedTimestamp.dwLowDateTime));
					(childNode->ToElement())->Attribute(TEXT("originalFileLastModifTimestampHigh"),
					    reinterpret_cast<int32_t*>(&fileModifiedTimestamp.dwHighDateTime));
					...
					sessionFileInfo sfi(fileName, langName, encStr ? encoding : -1, isUserReadOnly,
					    position, backupFilePath, fileModifiedTimestamp, mapPosition);
					for(TiXmlNode * markNode = childNode->FirstChildElement(TEXT("Mark")); markNode; markNode = markNode->NextSibling(TEXT("Mark"))) {
						int lineNumber;
					...
					for(TiXmlNode * foldNode = childNode->FirstChildElement(TEXT("Fold")); foldNode; foldNode = foldNode->NextSibling(TEXT("Fold"))) {
						int lineNumber;
...
void NppParameters::feedFileListParameters(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = historyRoot->FirstChildElement(TEXT("File")); childNode && (_nbRecentFile < NB_MAX_LRF_FILE); childNode = childNode->NextSibling(TEXT("File"))) {
...
void NppParameters::feedFindHistoryParameters(TiXmlNode * node)
{
	...
	if(boolStr)
		_findHistory._dotMatchesNewline = sstreq(TEXT("yes"), boolStr);
	boolStr = (findHistoryRoot->ToElement())->Attribute(TEXT("isSearch2ButtonsMode"));
	if(boolStr)
		_findHistory._isSearch2ButtonsMode = sstreq(TEXT("yes"), boolStr);
...
void FASTCALL NppParameters::feedShortcut(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = shortcutsRoot->FirstChildElement(TEXT("Shortcut")); childNode; childNode = childNode->NextSibling(TEXT("Shortcut"))) {
		...
		if(idStr) {
			//find the commandid that matches this Shortcut sc and alter it, push back its index in the
			// modified list, if not present
			size_t len = _shortcuts.size();
			for(size_t i = 0; i < len; ++i) {
				if(_shortcuts[i].getID() == (ulong)id) { //found our match
...
void NppParameters::feedPluginCustomizedCmds(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = pluginCustomizedCmdsRoot->FirstChildElement(TEXT("PluginCommand")); childNode; childNode = childNode->NextSibling(TEXT("PluginCommand"))) {
		...
		for(size_t i = 0; i < len; ++i) {
			...
			if(!generic_strnicmp(pscOrig.getModuleName(), moduleName,
			    lstrlen(moduleName)) && pscOrig.getInternalID() == internalID) {
...
void NppParameters::feedScintKeys(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = scintKeysRoot->FirstChildElement(TEXT("ScintKey")); childNode; childNode = childNode->NextSibling(TEXT("ScintKey"))) {
		...
		for(int32_t i = 0; i < static_cast<int32_t>(len); ++i) {
			...
			if(skmOrig.getScintillaKeyID() == (ulong)scintKey && skmOrig.getMenuCmdID() == menuID) {
				//Found matching command
...
bool NppParameters::getShortcuts(TiXmlNode * node, Shortcut & sc)
{
	...
	if(!keyStr)
		return false;
	sc = Shortcut(name, isCtrl, isAlt, isShift, static_cast<uchar>(key));
	return true;
}
...
std::pair<uchar, uchar> NppParameters::feedUserLang(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = node->FirstChildElement(TEXT("UserLang")); childNode && (_nbUserLang < NB_MAX_USER_LANG); childNode = childNode->NextSibling(TEXT("UserLang"))) {
		...
		try {
			...
			for(int i = 0; i < SCE_USER_STYLE_TOTAL_STYLES; ++i) {
				const NppStyle * pStyle = _userLangArray[_nbUserLang - 1]->_styles.findByID(i);
				if(!pStyle)
	...
	return pair<uchar, uchar>(static_cast<uchar>(iBegin), static_cast<uchar>(iEnd));
}

bool NppParameters::isInBlackList(const TCHAR * fn) const
{
...
bool NppParameters::importUDLFromFile(const generic_string& sourceFile)
{
	...
	if(loadOkay) {
		...
		if(loadOkay) {
			...
			// imported UDL from xml file will be added into default udl, so we should make default udl
			// dirty
...
const TCHAR * NppParameters::getLangExtFromName(const TCHAR * langName) const
{
	for(int i = 0; i < _nbLang; ++i) {
...
const TCHAR * NppParameters::getLangExtFromLangType(LangType langType) const
{
	for(int i = 0; i < _nbLang; ++i) {
...
const TCHAR * NppParameters::getWordList(LangType langID, int typeIndex) const
{
	Lang * pLang = getLangFromID(langID);
	return pLang ? pLang->getWords(typeIndex) : nullptr;
}
	
Lang * NppParameters::getLangFromID(LangType langID) const
{
	for(int i = 0; i < _nbLang; ++i) {
...
Lang * NppParameters::getLangFromIndex(size_t i) const { return (i < size_t(_nbLang)) ? _langList[i] : nullptr; }
	...
	if(root) {
		...
		for(size_t k = 0; k < nbElem; ++k) {
			...
			for(size_t i = 0, len = viewElems[k].viewFiles->size(); i < len; ++i) {
				...
				(fileNameNode->ToElement())->SetAttribute(TEXT("userReadOnly"),
				    (viewSessionFiles[i]._isUserReadOnly && !viewSessionFiles[i]._isMonitoring) ? TEXT("yes") : TEXT("no"));
				(fileNameNode->ToElement())->SetAttribute(TEXT("filename"), viewSessionFiles[i]._fileName.c_str());
				(fileNameNode->ToElement())->SetAttribute(TEXT("backupFilePath"),
				    viewSessionFiles[i]._backupFilePath.c_str());
				(fileNameNode->ToElement())->SetAttribute(TEXT("originalFileLastModifTimestamp"),
				    static_cast<int32_t>(viewSessionFiles[i]._originalFileLastModifTimestamp.dwLowDateTime));
				(fileNameNode->ToElement())->SetAttribute(TEXT("originalFileLastModifTimestampHigh"),
				    static_cast<int32_t>(viewSessionFiles[i]._originalFileLastModifTimestamp.dwHighDateTime));

				// docMap
				(fileNameNode->ToElement())->SetAttribute(TEXT("mapFirstVisibleDisplayLine"),
				    viewSessionFiles[i]._mapPos._firstVisibleDisplayLine);
				(fileNameNode->ToElement())->SetAttribute(TEXT("mapFirstVisibleDocLine"),
				    viewSessionFiles[i]._mapPos._firstVisibleDocLine);
				(fileNameNode->ToElement())->SetAttribute(TEXT("mapLastVisibleDocLine"),
				    viewSessionFiles[i]._mapPos._lastVisibleDocLine);
				...
				(fileNameNode->ToElement())->SetAttribute(TEXT("mapKByteInDoc"),
				    static_cast<int>(viewSessionFiles[i]._mapPos._KByteInDoc));
				(fileNameNode->ToElement())->SetAttribute(TEXT("mapWrapIndentMode"),
				    viewSessionFiles[i]._mapPos._wrapIndentMode);
				fileNameNode->ToElement()->SetAttribute(TEXT("mapIsWrap"),
				    viewSessionFiles[i]._mapPos._isWrap ? TEXT("yes") : TEXT("no"));
...
void NppParameters::feedUserSettings(TiXmlNode * settingsRoot)
{
	...
	if(globalSettingNode) {
		...
		if(boolStr)
			_userLangArray[_nbUserLang - 1]->_isCaseIgnored = sstreq(TEXT("yes"), boolStr);

		boolStr = (globalSettingNode->ToElement())->Attribute(TEXT("allowFoldOfComments"));
		if(boolStr)
			_userLangArray[_nbUserLang - 1]->_allowFoldOfComments = sstreq(TEXT("yes"), boolStr);
		...
		if(boolStr)
			_userLangArray[_nbUserLang - 1]->_foldCompact = sstreq(TEXT("yes"), boolStr);
	}
	...
	if(prefixNode) {
		...
		if(sstreq(udlVersion, TEXT("2.1")) || !lstrcmp(udlVersion, TEXT("2.0"))) {
			for(int i = 0; i < SCE_USER_TOTAL_KEYWORD_GROUPS; ++i) {
				boolStr = (prefixNode->ToElement())->Attribute(
					globalMappper().keywordNameMapper[i+SCE_USER_KWLIST_KEYWORDS1]);
				if(boolStr)
					_userLangArray[_nbUserLang - 1]->_isPrefix[i] = sstreq(TEXT("yes"), boolStr);
			}
		}
...
void NppParameters::feedUserKeywordList(TiXmlNode * node)
{
	...
	for(TiXmlNode * childNode = node->FirstChildElement(TEXT("Keywords"));
		...
		if(valueNode) {
			...
			if(sstreq(udlVersion, TEXT("")) && !lstrcmp(keywordsName, TEXT("Delimiters"))) { // support for old style (pre 2.0)
			...
			else if(sstreq(keywordsName, TEXT("Comment"))) {
...
{
	int id = -1;
	for(TiXmlNode * childNode = node->FirstChildElement(TEXT("WordsStyle")); childNode; childNode = childNode->NextSibling(TEXT("WordsStyle"))) {
...
LexerStyler * LexerStylerArray::getLexerStylerByName(const TCHAR * lexerName) 
{
	if(lexerName) {
		for(size_t i = 0; i < _lexerStylerVect.size(); ++i) {
			if(sstreq(_lexerStylerVect[i].getLexerName(), lexerName))
...
void StyleArray::addStyler(int styleID, const generic_string & styleName)
{
...
NppStyle * StyleArray::findByName(const generic_string & name)
{
	for(size_t i = 0; i < _styleVect.size(); ++i) {
...
LangType NppParameters::getLangIDFromStr(const TCHAR * langName)
{
	...
	for(; lang < L_EXTERNAL; ++lang) {
		...
		if(sstreq(name, langName)) { //found lang?
...
generic_string NppParameters::getLocPathFromStr(const generic_string & localizationCode)
{
	...
	if(localizationCode == TEXT("ar") || localizationCode == TEXT("ar-dz") || localizationCode == TEXT("ar-bh") ||
	    localizationCode == TEXT("ar-eg") ||localizationCode == TEXT("ar-iq") || localizationCode == TEXT("ar-jo") ||
	    localizationCode == TEXT("ar-kw") || localizationCode == TEXT("ar-lb") || localizationCode == TEXT("ar-ly") ||
	    localizationCode == TEXT("ar-ma") || localizationCode == TEXT("ar-om") || localizationCode == TEXT("ar-qa") ||
	    localizationCode == TEXT("ar-sa") || localizationCode == TEXT("ar-sy") || localizationCode == TEXT("ar-tn") ||
	    localizationCode == TEXT("ar-ae") || localizationCode == TEXT("ar-ye"))
	...
	if(localizationCode == TEXT("fr") || localizationCode == TEXT("fr-be") || localizationCode == TEXT("fr-ca") ||
	    localizationCode == TEXT("fr-fr") || localizationCode == TEXT("fr-lu") || localizationCode == TEXT("fr-mc") ||
	    localizationCode == TEXT("fr-ch"))
	...
	if(localizationCode == TEXT("de") || localizationCode == TEXT("de-at") || localizationCode == TEXT("de-de") ||
	    localizationCode == TEXT("de-li") || localizationCode == TEXT("de-lu") || localizationCode == TEXT("de-ch"))
	...
	if(localizationCode == TEXT("es") || localizationCode == TEXT("es-bo") || localizationCode == TEXT("es-cl") ||
	    localizationCode == TEXT("es-co") || localizationCode == TEXT("es-cr") || localizationCode == TEXT("es-do") ||
	    localizationCode == TEXT("es-ec") || localizationCode == TEXT("es-sv") || localizationCode == TEXT("es-gt") ||
	    localizationCode == TEXT("es-hn") || localizationCode == TEXT("es-mx") || localizationCode == TEXT("es-ni") ||
	    localizationCode == TEXT("es-pa") || localizationCode == TEXT("es-py") || localizationCode == TEXT("es-pe") ||
	    localizationCode == TEXT("es-pr") || localizationCode == TEXT("es-es") || localizationCode == TEXT("es-uy") ||
	    localizationCode == TEXT("es-ve"))
	...
	for(TiXmlNode * childNode = GUIRoot->FirstChildElement(TEXT("GUIConfig")); childNode; childNode = childNode->NextSibling(TEXT("GUIConfig"))) {
		...
		if(sstreq(nm, TEXT("ToolBar"))) {
			...
			if(n) {
				val = n->Value();
				if(val) {
					if(sstreq(val, TEXT("small")))
						_nppGUI._toolBarStatus = TB_SMALL;
					else if(sstreq(val, TEXT("large")))
						_nppGUI._toolBarStatus = TB_LARGE;
					else if(sstreq(val, TEXT("small2")))
		...
		else if(sstreq(nm, TEXT("StatusBar"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
					if(sstreq(val, TEXT("hide")))
						_nppGUI._statusBarShow = false;
					else if(sstreq(val, TEXT("show")))
						_nppGUI._statusBarShow = true;
				}
			}
		}
		else if(sstreq(nm, TEXT("MenuBar"))) {
			const TiXmlNode * n = childNode->FirstChild();
			const TCHAR * val = n ? n->Value() : 0;
			if(val) {
				if(sstreq(val, TEXT("hide")))
			...
			if(val) {
				if(sstreq(val, TEXT("1")))
					_nppGUI._tabStatus |= TAB_ALTICONS;
				else if(sstreq(val, TEXT("0")))
					_nppGUI._tabStatus |= 0;
				else
					isFailed = true;
			}
		...
		else if(sstreq(nm, TEXT("RememberLastSession"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("DetectEncoding"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("SaveAllConfirm"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("MarkAll"))) {
		...
		// <GUIConfig name="SmartHighLight" matchCase="yes" wholeWordOnly="yes"
		// useFindSettings="no">yes</GUIConfig>
		else if(sstreq(nm, TEXT("SmartHighLight"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("TagsMatchHighLight"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("TaskList"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
					_nppGUI._doTaskList = (sstreq(val, TEXT("yes"))) ? true : false;
				}
			}
		}
		else if(sstreq(nm, TEXT("MRU"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val)
		...
		else if(sstreq(nm, TEXT("CheckHistoryFiles"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
		...
		else if(sstreq(nm, TEXT("ScintillaViewsSplitter"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val) {
					if(sstreq(val, TEXT("vertical")))
						_nppGUI._splitterPos = POS_VERTICAL;
					else if(sstreq(val, TEXT("horizontal")))
						_nppGUI._splitterPos = POS_HORIZOTAL;
				}
			}
		}

		else if(sstreq(nm, TEXT("UserDefineDlg"))) {
			bool isFailed = false;
			...
			if(val) {
				if(sstreq(val, TEXT("docked")))
					_nppGUI._userDefineDlgStatus |= UDD_DOCKED;
				else if(sstreq(val, TEXT("undocked")))
					_nppGUI._userDefineDlgStatus |= 0;
				else
					isFailed = true;
			}
		...
		else if(sstreq(nm, TEXT("TabSetting"))) {
			int i;
			...
			if(val)
				_nppGUI._tabReplacedBySpace = (sstreq(val, TEXT("yes")));
		}

		else if(sstreq(nm, TEXT("Caret"))) {
			int i;
		...
		else if(sstreq(nm, TEXT("ScintillaGlobalSettings"))) {
		...
		else if(sstreq(nm, TEXT("AppPosition"))) {
			...
			if(val)
				_nppGUI._isMaximized = sstreq(val, TEXT("yes"));
		}
		else if(sstreq(nm, TEXT("FindWindowPosition"))) {
		...
		else if(sstreq(nm, TEXT("FinderConfig"))) {
			const TCHAR* val = element->Attribute(TEXT("wrappedLines"));
			if(val) {
				_nppGUI._finderLinesAreCurrentlyWrapped = (sstreq(val, TEXT("yes")));
			}
			val = element->Attribute(TEXT("purgeBeforeEverySearch"));
			if(val) {
				_nppGUI._finderPurgeBeforeEverySearch = (sstreq(val, TEXT("yes")));
			}
		}

		else if(sstreq(nm, TEXT("NewDocDefaultSettings"))) {
			int i;
			...
			if(val)
				_nppGUI._newDocDefaultSettings._openAnsiAsUtf8 = sstreq(val, TEXT("yes"));
		}
		else if(sstreq(nm, TEXT("langsExcluded"))) {
			// TODO
			int g0 = 0; // up to 8
			int g1 = 0; // up to 16
			int g2 = 0; // up to 24
			...
			if(val)
				_nppGUI._isLangMenuCompact = (sstreq(val, TEXT("yes")));
		}
		else if(sstreq(nm, TEXT("Print"))) {
			const TCHAR* val = element->Attribute(TEXT("lineNumber"));
			if(val)
				_nppGUI._printSettings._printLineNumber = (sstreq(val, TEXT("yes")));
			int i;
		...
		else if(sstreq(nm, TEXT("DockingManager"))) {
			feedDockingManager(element);
		}
		else if(sstreq(nm, TEXT("globalOverride"))) {
			const TCHAR * bDir = element->Attribute(TEXT("fg"));
			if(bDir)
				_nppGUI._globalOverride.enableFg = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("bg"));
			if(bDir)
				_nppGUI._globalOverride.enableBg = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("font"));
			if(bDir)
				_nppGUI._globalOverride.enableFont = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("fontSize"));
			if(bDir)
				_nppGUI._globalOverride.enableFontSize = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("bold"));
			if(bDir)
				_nppGUI._globalOverride.enableBold = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("italic"));
			if(bDir)
				_nppGUI._globalOverride.enableItalic = sstreq(bDir, TEXT("yes"));
			bDir = element->Attribute(TEXT("underline"));
			if(bDir)
				_nppGUI._globalOverride.enableUnderLine = sstreq(bDir, TEXT("yes"));
		}
		else if(sstreq(nm, TEXT("auto-completion"))) {
			int i;
		...
		else if(sstreq(nm, TEXT("auto-insert"))) {
			...
			for(TiXmlNode * subChildNode = childNode->FirstChildElement(TEXT("UserDefinePair")); subChildNode; subChildNode = subChildNode->NextSibling(TEXT("UserDefinePair"))) {
				int open = -1;
				int openVal = 0;
		...
		else if(sstreq(nm, TEXT("sessionExt"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val)
		...
		else if(sstreq(nm, TEXT("workspaceExt"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val)
		...
		else if(sstreq(nm, TEXT("noUpdate"))) {
			TiXmlNode * n = childNode->FirstChild();
			if(n) {
				const TCHAR* val = n->Value();
				if(val)
					_nppGUI._autoUpdateOpt._doAutoUpdate = (sstreq(val, TEXT("yes"))) ? false : true;
				int i;
				...
				if(val)
					_nppGUI._autoUpdateOpt._nextUpdateDate = NppDate(val);
			}
		}
		else if(sstreq(nm, TEXT("openSaveDir"))) {
		...
		else if(sstreq(nm, TEXT("titleBar"))) {
		...
		else if(sstreq(nm, TEXT("delimiterSelection"))) {
			int leftmost = 0;
		...
		else if(sstreq(nm, TEXT("multiInst"))) {
			int val = 0;
		...
		else if(sstreq(nm, TEXT("searchEngine"))) {
			int i;
		...
		else if(sstreq(nm, TEXT("Searching"))) {
		...
		else if(sstreq(nm, TEXT("commandLineInterpreter"))) {
		...
		else if(sstreq(nm, TEXT("DarkMode"))) {
...
void NppParameters::feedScintillaParam(TiXmlNode * node)
{
	...
	if(nm) {
		if(sstreq(nm, TEXT("box")))
			_svp._folderStyle = FOLDER_STYLE_BOX;
		else if(sstreq(nm, TEXT("circle")))
			_svp._folderStyle = FOLDER_STYLE_CIRCLE;
		else if(sstreq(nm, TEXT("arrow")))
			_svp._folderStyle = FOLDER_STYLE_ARROW;
		else if(sstreq(nm, TEXT("simple")))
	...
	if(nm) {
		if(sstreq(nm, TEXT("default")))
			_svp._lineWrapMethod = LINEWRAP_DEFAULT;
		else if(sstreq(nm, TEXT("aligned")))
	...
	if(nm) {
		if(val >= 0 && val <= 30)
			_svp._paddingLeft = static_cast<uchar>(val);
	}
	nm = element->Attribute(TEXT("paddingRight"), &val);
	if(nm) {
		if(val >= 0 && val <= 30)
			_svp._paddingRight = static_cast<uchar>(val);
	}
	nm = element->Attribute(TEXT("distractionFreeDivPart"), &val);
	if(nm) {
		if(val >= 3 && val <= 9)
			_svp._distractionFreeDivPart = static_cast<uchar>(val);
	}
}
...
void NppParameters::duplicateDockingManager(TiXmlNode * dockMngNode, TiXmlElement* dockMngElmt2Clone)
{
	...
	for(TiXmlNode * childNode = dockMngNode->FirstChildElement(TEXT("FloatingWindow")); childNode; childNode = childNode->NextSibling(TEXT("FloatingWindow"))) {
		TiXmlElement * floatElement = childNode->ToElement();
		int cont;
		if(floatElement->Attribute(TEXT("cont"), &cont)) {
	...
	for(TiXmlNode * childNode = dockMngNode->FirstChildElement(TEXT("PluginDlg")); childNode; childNode = childNode->NextSibling(TEXT("PluginDlg"))) {
		TiXmlElement * dlgElement = childNode->ToElement();
		const TCHAR * name = dlgElement->Attribute(TEXT("pluginName"));
		...
		if(name && idStr) {
			...
			dlgElement->Attribute(TEXT("prev"), &prev);
			const TCHAR * val = dlgElement->Attribute(TEXT("isVisible"));
			const bool isVisible = sstreq(val, TEXT("yes"));
	...
	for(TiXmlNode * childNode = dockMngNode->FirstChildElement(TEXT("ActiveTabs")); childNode; childNode = childNode->NextSibling(TEXT("ActiveTabs"))) {
		TiXmlElement * dlgElement = childNode->ToElement();
...
bool NppParameters::writeScintillaParams()
{
	...
	const TCHAR * pWrapMethodStr = (_svp._lineWrapMethod == LINEWRAP_ALIGNED) ? TEXT("aligned") : (_svp._lineWrapMethod == LINEWRAP_INDENT) ? TEXT("indent") : TEXT("default");
...
void NppParameters::createXmlTreeFromGUIParams()
{
	...
	if(oldGUIRoot && _nppGUI._isCmdlineNosessionActivated) {
		for(TiXmlNode * childNode = oldGUIRoot->FirstChildElement(TEXT("GUIConfig")); childNode; childNode = childNode->NextSibling(TEXT("GUIConfig"))) {
			...
			if(nullptr == nm)
				continue;
			if(sstreq(nm, TEXT("DockingManager"))) {
	...
	// closeButton="yes" doubleClick2Close="no" vertical="no" multiLine="no" hide="no" quitOnEmpty="no"
	// iconSetNumber="0" />
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name = "NewDocDefaultSettings" format = "0" encoding = "0" lang = "3" codepage = "-1"
	// openAnsiAsUTF8 = "no" / >
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
		...
		GUIConfigElement->SetAttribute(TEXT("openAnsiAsUTF8"),
		    _nppGUI._newDocDefaultSettings._openAnsiAsUtf8 ? TEXT("yes") : TEXT("no"));
	}

	// <GUIConfig name = "langsExcluded" gr0 = "0" gr1 = "0" gr2 = "0" gr3 = "0" gr4 = "0" gr5 = "0" gr6 = "0" gr7 =
	// "0" langMenuCompact = "yes" / >
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name="Print" lineNumber="no" printOption="0" headerLeft="$(FULL_CURRENT_PATH)" headerMiddle=""
	// headerRight="$(LONG_DATE) $(TIME)" headerFontName="IBMPC" headerFontStyle="1" headerFontSize="8"
	// footerLeft="" footerMiddle="-$(CURRENT_PRINTING_PAGE)-" footerRight="" footerFontName="" footerFontStyle="0"
	// footerFontSize="9" margeLeft="0" margeTop="0" margeRight="0" margeBottom="0" />
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name = "globalOverride" fg = "no" bg = "no" font = "no" fontSize = "no" bold = "no" italic = "no"
	// underline = "no" / >
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name = "auto-insert" parentheses = "yes" brackets = "yes" curlyBrackets = "yes" quotes = "no"
	// doubleQuotes = "yes" htmlXmlTag = "yes" / >
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name="delimiterSelection" leftmostDelimiter="40" rightmostDelimiter="41"
	// delimiterSelectionOnEntireDocument="no" />
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
		...
		GUIConfigElement->SetAttribute(TEXT("delimiterSelectionOnEntireDocument"),
		    _nppGUI._delimiterSelectionOnEntireDocument ? TEXT("yes") : TEXT("no"));
	}
	...
	// isFolderDroppedOpenFiles="no" saveDlgExtFilterToAllTypes="no" />
	{
		TiXmlElement * GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// confirmReplaceOpenDocs="yes" confirmMacroReplaceOpenDocs="yes" confirmReplaceInFiles="yes"
	// confirmMacroReplaceInFiles="yes" replaceStopsWithoutFindingNext="no"/>
	{
		TiXmlElement* GUIConfigElement = (newGUIRoot->InsertEndChild(TiXmlElement(TEXT("GUIConfig"))))->ToElement();
	...
	// <GUIConfig name="SmartHighLight" matchCase="no" wholeWordOnly="yes" useFindSettings="no"
	// onAnotherView="no">yes</GUIConfig>
	{
	...
	// folderMarkStyle="box" lineWrapMethod="aligned" currentLineHilitingShow="show" scrollBeyondLastLine="no"
	// rightClickKeepsSelection="no" disableAdvancedScrolling="no" wrapSymbolShow="hide" Wrap="no" borderEdge="yes"
	// edge="no" edgeNbColumn="80" zoom="0" zoom2="0" whiteSpaceShow="hide" eolShow="hide" borderWidth="2"
	// smoothFont="no" />
...
void NppParameters::writeExcludedLangList(TiXmlElement * element)
{
	...
	for(size_t i = 0, len = _nppGUI._excludedLangList.size(); i < len; ++i) {
		...
		switch(nGrp) {
			case 0: g0 |= nMask; break;
			case 1: g1 |= nMask; break;
			case 2: g2 |= nMask; break;
			case 3: g3 |= nMask; break;
			case 4: g4 |= nMask; break;
			case 5: g5 |= nMask; break;
			case 6: g6 |= nMask; break;
			case 7: g7 |= nMask; break;
			case 8: g8 |= nMask; break;
			case 9: g9 |= nMask; break;
			case 10: g10 |= nMask; break;
			case 11: g11 |= nMask; break;
			case 12: g12 |= nMask; break;
		}
	}
...
int NppParameters::langTypeToCommandID(LangType lt) const
{
	int id;
	switch(lt) {
		case L_C: id = IDM_LANG_C; break;
		case L_CPP: id = IDM_LANG_CPP; break;
		case L_JAVA: id = IDM_LANG_JAVA;     break;
		case L_CS: id = IDM_LANG_CS; break;
		case L_OBJC: id = IDM_LANG_OBJC;     break;
		case L_HTML: id = IDM_LANG_HTML;     break;
		case L_XML: id = IDM_LANG_XML; break;
		case L_JS: 
		case L_JAVASCRIPT: id = IDM_LANG_JS; break;
		case L_JSON: id = IDM_LANG_JSON; break;
		case L_PHP: id = IDM_LANG_PHP; break;
		case L_ASP: id = IDM_LANG_ASP; break;
		case L_JSP: id = IDM_LANG_JSP; break;
		case L_CSS: id = IDM_LANG_CSS; break;
		case L_LUA: id = IDM_LANG_LUA; break;
		case L_PERL: id = IDM_LANG_PERL; break;
		case L_PYTHON: id = IDM_LANG_PYTHON; break;
		case L_BATCH: id = IDM_LANG_BATCH; break;
		case L_PASCAL: id = IDM_LANG_PASCAL; break;
		case L_MAKEFILE: id = IDM_LANG_MAKEFILE; break;
		case L_INI: id = IDM_LANG_INI; break;
		case L_ASCII: id = IDM_LANG_ASCII; break;
		case L_RC: id = IDM_LANG_RC; break;
		case L_TEX: id = IDM_LANG_TEX; break;
		case L_FORTRAN: id = IDM_LANG_FORTRAN; break;
		case L_FORTRAN_77: id = IDM_LANG_FORTRAN_77; break;
		case L_BASH: id = IDM_LANG_BASH; break;
		case L_FLASH: id = IDM_LANG_FLASH; break;
		case L_NSIS: id = IDM_LANG_NSIS; break;
		case L_USER: id = IDM_LANG_USER; break;
		case L_SQL: id = IDM_LANG_SQL; break;
		case L_VB: id = IDM_LANG_VB; break;
		case L_TCL: id = IDM_LANG_TCL; break;
		case L_LISP: id = IDM_LANG_LISP; break;
		case L_SCHEME: id = IDM_LANG_SCHEME; break;
		case L_ASM: id = IDM_LANG_ASM; break;
		case L_DIFF: id = IDM_LANG_DIFF; break;
		case L_PROPS: id = IDM_LANG_PROPS; break;
		case L_PS: id = IDM_LANG_PS; break;
		case L_RUBY: id = IDM_LANG_RUBY; break;
		case L_SMALLTALK: id = IDM_LANG_SMALLTALK; break;
		case L_VHDL: id = IDM_LANG_VHDL; break;
		case L_ADA: id = IDM_LANG_ADA; break;
		case L_MATLAB: id = IDM_LANG_MATLAB; break;
		case L_HASKELL: id = IDM_LANG_HASKELL; break;
		case L_KIX: id = IDM_LANG_KIX; break;
		case L_AU3: id = IDM_LANG_AU3; break;
		case L_VERILOG: id = IDM_LANG_VERILOG; break;
		case L_CAML: id = IDM_LANG_CAML; break;
		case L_INNO: id = IDM_LANG_INNO; break;
		case L_CMAKE: id = IDM_LANG_CMAKE; break;
		case L_YAML: id = IDM_LANG_YAML; break;
		case L_COBOL: id = IDM_LANG_COBOL; break;
		case L_D: id = IDM_LANG_D; break;
		case L_GUI4CLI: id = IDM_LANG_GUI4CLI; break;
		case L_POWERSHELL: id = IDM_LANG_POWERSHELL; break;
		case L_R: id = IDM_LANG_R; break;
		case L_COFFEESCRIPT: id = IDM_LANG_COFFEESCRIPT; break;
		case L_BAANC: id = IDM_LANG_BAANC; break;
		case L_SREC: id = IDM_LANG_SREC; break;
		case L_IHEX: id = IDM_LANG_IHEX; break;
		case L_TEHEX: id = IDM_LANG_TEHEX; break;
		case L_SWIFT: id = IDM_LANG_SWIFT; break;
		case L_ASN1: id = IDM_LANG_ASN1; break;
		case L_AVS: id = IDM_LANG_AVS; break;
		case L_BLITZBASIC: id = IDM_LANG_BLITZBASIC; break;
		case L_PUREBASIC: id = IDM_LANG_PUREBASIC; break;
		case L_FREEBASIC: id = IDM_LANG_FREEBASIC; break;
		case L_CSOUND: id = IDM_LANG_CSOUND; break;
		case L_ERLANG: id = IDM_LANG_ERLANG; break;
		case L_ESCRIPT: id = IDM_LANG_ESCRIPT; break;
		case L_FORTH: id = IDM_LANG_FORTH; break;
		case L_LATEX: id = IDM_LANG_LATEX; break;
		case L_MMIXAL: id = IDM_LANG_MMIXAL; break;
		case L_NIM: id = IDM_LANG_NIM; break;
		case L_NNCRONTAB: id = IDM_LANG_NNCRONTAB; break;
		case L_OSCRIPT: id = IDM_LANG_OSCRIPT; break;
		case L_REBOL: id = IDM_LANG_REBOL; break;
		case L_REGISTRY: id = IDM_LANG_REGISTRY; break;
		case L_RUST: id = IDM_LANG_RUST; break;
		case L_SPICE: id = IDM_LANG_SPICE; break;
		case L_TXT2TAGS: id = IDM_LANG_TXT2TAGS; break;
		case L_VISUALPROLOG: id = IDM_LANG_VISUALPROLOG; break;
		case L_TYPESCRIPT: id = IDM_LANG_TYPESCRIPT; break;
		case L_SEARCHRESULT: id = -1; break;
		case L_TEXT: id = IDM_LANG_TEXT;     break;
		default:
...
{
	switch(_platForm) {
		case PF_X86: return TEXT("32-bit");
		case PF_X64:
		case PF_IA64: return TEXT("64-bit");
		default: return TEXT("Unknown-bit");
	}
}
...
generic_string NppParameters::writeStyles(LexerStylerArray & lexersStylers, StyleArray & globalStylers)
{
	...
	for(TiXmlNode * childNode = lexersRoot->FirstChildElement(TEXT("LexerType"));
		...
		if(pLs) {
			...
			for(TiXmlNode * grChildNode = childNode->FirstChildElement(TEXT("WordsStyle")); grChildNode; grChildNode = grChildNode->NextSibling(TEXT("WordsStyle"))) {
				TiXmlElement * grElement = grChildNode->ToElement();
				const TCHAR * styleName = grElement->Attribute(TEXT("name"));
				const NppStyle * pStyle = pLs->findByName(styleName);
				NppStyle * pStyle2Sync = pLs2 ? pLs2->findByName(styleName) : nullptr;
	...
	for(TiXmlNode * childNode = globalStylesRoot->FirstChildElement(TEXT("WidgetStyle"));
		...
		const NppStyle * pStyle = _widgetStyleArray.findByName(styleName);
		NppStyle * pStyle2Sync = globalStylers.findByName(styleName);
		if(pStyle && pStyle2Sync) {
...
void NppParameters::writeStyle2Element(const NppStyle & style2Write, NppStyle & style2Sync, TiXmlElement * element)
{
...
void NppParameters::insertUserLang2Tree(TiXmlNode * node, UserLangContainer * userLang)
{
	...
	TiXmlElement * settingsElement = (rootElement->InsertEndChild(TiXmlElement(TEXT("Settings"))))->ToElement();
	{
		...
		globalElement->SetAttribute(TEXT("caseIgnored"),
		    userLang->_isCaseIgnored ? TEXT("yes") : TEXT("no"));
		...
		for(int i = 0; i < SCE_USER_TOTAL_KEYWORD_GROUPS; ++i)
			prefixElement->SetAttribute(globalMappper().keywordNameMapper[i+SCE_USER_KWLIST_KEYWORDS1],
			    userLang->_isPrefix[i] ? TEXT("yes") : TEXT("no"));
	}
	...
	for(const NppStyle & style2Write : userLang->_styles) {
	...
	if(kernel) {
		...
		if(IsWow64ProcessFunc) {
			...
			if(isWow64) {
				...
				LPFN_WOW64ENABLEWOW64FSREDIRECTION Wow64EnableWow64FsRedirectionFunc =
				    (LPFN_WOW64ENABLEWOW64FSREDIRECTION)GetProcAddress(kernel, "Wow64EnableWow64FsRedirection");
...
/*
   ...
   1[D2]                        1[D2]                        1[D2]                        0[D2]
                              [D2]
   2[D3]  [DUDL, <0,3>]         2[D3]  [DUDL, <0,3>]         2[D3]  [DUDL, <0,3>]         1[D3]  [DUDL, <0,2>]
               [D3]  [DUDL, <0,0>]
   3[U1]  [NUDL, <3,4>]         3[U1]  [NUDL, <3,4>]         3[U1]  [NUDL, <3,4>]         2[U1]  [NUDL, <2,3>]
               [U1]  [NUDL, <0,0>]
   4[U2]                        4[U2]                         [U2]                         [U2]
                              [U2]
   5[U2]  [NUDL, <4,6>]         5[U2]  [NUDL, <4,6>]         4[U2]  [NUDL, <4,5>]         3[U2]  [NUDL, <3,4>]
              0[U2]  [NUDL, <0,1>]
   6[C1]  [NULL, <6,7>]          [C1]  [NULL, <6,6>]          [C1]  [NULL, <5,5>]          [C1]  [NULL, <4,4>]
               [C1]  [NULL, <1,1>]
   7[I1]  [NULL, <7,8>]         6[I1]  [NULL, <6,7>]         5[I1]  [NULL, <5,6>]         4[I1]  [NULL, <4,5>]
              1[I1]  [NULL, <1,2>]
 */
...
NppDate::NppDate(const TCHAR * dateStr)
{
...
NppDate::NppDate(ulong year, ulong month, ulong day) : _year(year), _month(month), _day(day)
{
	assert(year > 0 && year <= 9999); // I don't think Notepad++ will last till AD 10000 :)
	assert(month > 0 && month <= 12);
	assert(day > 0 && day <= 31);
	assert(!(month == 2 && day > 29) && !(month == 4 && day > 30) && !(month == 6 && day > 30) && !(month == 9 && day > 30) && !(month == 11 && day > 30));
}
...
generic_string NppDate::toString() const // Return Notepad++ date format : YYYYMMDD
{
	TCHAR dateStr[16];
...
{
	switch(value) {
		case static_cast<LPARAM>(EolType::windows): return EolType::windows;
		case static_cast<LPARAM>(EolType::macos): return EolType::macos;
		case static_cast<LPARAM>(EolType::unix): return EolType::unix;
