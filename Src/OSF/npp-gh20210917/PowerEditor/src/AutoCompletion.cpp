...
void AutoCompletion::showPathCompletion()
{
	...
	generic_string autoCompleteEntries;
	{
		...
		if(hFind != INVALID_HANDLE_VALUE) {
			// Maximum number of entries to show. Without this it appears to the user like N++ hangs when
			// autocompleting
			// some really large directories (c:\windows\winxsys on my system for instance).
			const uint maxEntries = 2000;
			uint counter = 0;
			do {
				...
				if(data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)  // If directory, add trailing
					                                              // slash.
					autoCompleteEntries += TEXT("\\");
...
// if current pos > matchedStartSybol Pos and current pos is on the same line of matchedStartSybolPos, it'll be checked
// then removed
...
void AutoCompletion::insertMatchedChars(int character, const MatchedPairConf & matchedPairConf)
{
	...
	bool isInSandwich = (charPrev == '(' && charNext == ')') || (charPrev == '[' && charNext == ']') ||
	    (charPrev == '{' && charNext == '}');
...
void AutoCompletion::update(int character)
{
	...
	if(nppGUI._funcParams || _funcCalltip.isVisible()) {
		if(_funcCalltip.updateCalltip(character)) { //calltip visible because triggered by autocomplete, set
		                                            // mode
