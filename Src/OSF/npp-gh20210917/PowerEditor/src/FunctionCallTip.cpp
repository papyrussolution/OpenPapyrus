...
inline bool lower(TCHAR c) { return (c >= 'a' && c <= 'z'); }
...
void FunctionCallTip::showCalltip()
{
	...
	//Check if the current overload still holds. If the current param exceeds amounti n overload, see if another one
	// fits better (enough params)
