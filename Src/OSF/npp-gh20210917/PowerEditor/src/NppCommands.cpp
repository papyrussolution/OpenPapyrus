...
// The conversion must be implemented in classes which inherit from this, see prepareStringForConversion and
// convertStringToNumber.
//
template <typename T_Num> class NumericSorter : public ISorter {
public:
...
void Notepad_plus::command(int id)
{
	switch(id) {
		...
		case IDM_FILE_CONTAININGFOLDERASWORKSPACE:
	    {
		    ...
		    ::SendMessage(_pPublicInterface->getHSelf(), NPPM_GETFULLCURRENTPATH, CURRENTWORD_MAXLENGTH,
			reinterpret_cast<LPARAM>(currentFile));
		    ::SendMessage(_pPublicInterface->getHSelf(), NPPM_GETCURRENTDIRECTORY, CURRENTWORD_MAXLENGTH,
			reinterpret_cast<LPARAM>(currentDir));
		    if(!_pFileBrowser) {
		...
		case IDM_FILE_OPENFOLDERASWORSPACE:
			...
			break;
		case IDM_FILE_RELOAD: fileReload(); break;
		...
		case IDM_FILE_DELETE:
		    if(fileDelete())
			    checkDocState();
		    break;
		case IDM_FILE_RENAME: fileRename(); break;
		...
		case IDM_FILE_SAVEALL: fileSaveAll(); break;
		case IDM_FILE_SAVEAS: fileSaveAs(); break;
		case IDM_FILE_SAVECOPYAS: fileSaveAs(BUFFER_INVALID, true); break;
		case IDM_FILE_LOADSESSION: fileLoadSession(); break;
		case IDM_FILE_SAVESESSION: fileSaveSession(); break;
		case IDM_FILE_PRINTNOW: filePrint(false); break;
		case IDM_FILE_PRINT: filePrint(true); break;
		case IDM_FILE_EXIT: ::PostMessage(_pPublicInterface->getHSelf(), WM_CLOSE, 0, 0); break;
		case IDM_EDIT_UNDO:
			{
		...
		case IDM_EDIT_CUT_BINARY:
	    {
		    ...
		    HGLOBAL hglbCopy = GlobalAlloc(GMEM_MOVEABLE, (textLen + 1) * sizeof(uchar));
		    if(hglbCopy == NULL) {
			    CloseClipboard();
			    return;
		    }
		    ...
		    HGLOBAL hglbLenCopy = GlobalAlloc(GMEM_MOVEABLE, sizeof(ulong));
		    if(hglbLenCopy == NULL) {
			    CloseClipboard();
			    return;
		    }
		    // Lock the handle and copy the text to the buffer.
		    ulong * lpLenCopy = (ulong *)GlobalLock(hglbLenCopy);
		    *lpLenCopy = textLen;
		...
		case IDM_EDIT_SEARCHONINTERNET:
	    {
		    ...
		    if(nppGui._searchEngineChoice == nppGui.se_custom) {
			    ...
			    url.erase(std::remove_if(url.begin(), url.end(), [](_TUCHAR x) {
					return _istspace(x);
				}),
				url.end());
	    ...
	    {
		    ::CheckMenuItem(_mainMenuHandle, IDM_EDIT_BEGINENDSELECT,
			MF_BYCOMMAND | (_pEditView->beginEndSelectedIsStarted() ? MF_UNCHECKED : MF_CHECKED));
		...
		case IDM_EDIT_SORTLINES_RANDOMLY:
	    {
		    ...
		    bool isDescending = oneof5(id, IDM_EDIT_SORTLINES_LEXICOGRAPHIC_DESCENDING, IDM_EDIT_SORTLINES_INTEGER_DESCENDING, 
				IDM_EDIT_SORTLINES_DECIMALCOMMA_DESCENDING, IDM_EDIT_SORTLINES_DECIMALDOT_DESCENDING, IDM_EDIT_SORTLINES_LEXICO_CASE_INSENS_DESCENDING);
		    _pEditView->execute(SCI_BEGINUNDOACTION);
		    ...
		    try {
			    ...
			    _nativeLangSpeaker.messageBox("SortingError", _pPublicInterface->getHSelf(),
					TEXT("Unable to perform numeric sorting due to line $INT_REPLACE$."), TEXT("Sorting Error"), MB_OK | MB_ICONINFORMATION | MB_APPLMODAL, static_cast<int>(lineNo), 0);
		    }
		    _pEditView->execute(SCI_ENDUNDOACTION);
			    ...
			    if((id == IDM_SEARCH_FINDPREV) && (op._searchType == FindRegex) && !nppParams.regexBackward4PowerUser()) {
				    ...
				    if(status == FSEndReached) {
						...
						_nativeLangSpeaker.getLocalizedStrFromID("find-status-top-reached",
						    TEXT(
							    "Find: Found the 1st occurrence from the bottom. The beginning of the document has been reached."));
		...
		case IDM_SEARCH_GOTONEXTFOUND: _findReplaceDlg.gotoNextFoundResult(); break;
		case IDM_SEARCH_GOTOPREVFOUND: _findReplaceDlg.gotoNextFoundResult(-1); break;
		...
		case IDM_SEARCH_CLEARALLMARKS:
			...
			break;
		case IDM_SEARCH_STYLE1TOCLIP: _pEditView->markedTextToClipboard(SCE_UNIVERSAL_FOUND_STYLE_EXT1); break;
		case IDM_SEARCH_STYLE2TOCLIP: _pEditView->markedTextToClipboard(SCE_UNIVERSAL_FOUND_STYLE_EXT2); break;
		case IDM_SEARCH_STYLE3TOCLIP: _pEditView->markedTextToClipboard(SCE_UNIVERSAL_FOUND_STYLE_EXT3); break;
		case IDM_SEARCH_STYLE4TOCLIP: _pEditView->markedTextToClipboard(SCE_UNIVERSAL_FOUND_STYLE_EXT4); break;
		case IDM_SEARCH_STYLE5TOCLIP: _pEditView->markedTextToClipboard(SCE_UNIVERSAL_FOUND_STYLE_EXT5); break;
		case IDM_SEARCH_ALLSTYLESTOCLIP: _pEditView->markedTextToClipboard(-1, true); break;
	    ...
	    break;
		case IDM_SEARCH_TOGGLE_BOOKMARK: bookmarkToggle(-1); break;
		case IDM_SEARCH_NEXT_BOOKMARK: bookmarkNext(true); break;
		case IDM_SEARCH_PREV_BOOKMARK: bookmarkNext(false); break;
		case IDM_SEARCH_CLEAR_BOOKMARKS: bookmarkClearAll(); break;
		    ...
		    if(!udd->isCreated()) {
			    ...
			    if((isUDDlgDocked)&&(isUDDlgVisible)) {
				    ...
				    if(!_pMainSplitter) {
					    ...
					    _pMainSplitter->create(pWindow,
						ScintillaEditView::getUserDefineDlg(),
						splitterSizeDyn,
						SplitterMode::RIGHT_FIX,
						45);
				    }
	    ...
	    break;
		case IDM_EDIT_DUP_LINE: _pEditView->execute(SCI_LINEDUPLICATE); break;
		...
		case IDM_EDIT_JOIN_LINES:
			...
			break;
		case IDM_EDIT_LINE_UP: _pEditView->currentLinesUp(); break;
		case IDM_EDIT_LINE_DOWN: _pEditView->currentLinesDown(); break;
		...
		case IDM_EDIT_REMOVEEMPTYLINESWITHBLANK:
		    ...
		    _pEditView->execute(SCI_ENDUNDOACTION);
		    break;
		case IDM_EDIT_UPPERCASE: _pEditView->convertSelectedTextToUpperCase(); break;
		case IDM_EDIT_LOWERCASE: _pEditView->convertSelectedTextToLowerCase(); break;
		case IDM_EDIT_PROPERCASE_FORCE: _pEditView->convertSelectedTextToNewerCase(TITLECASE_FORCE); break;
		case IDM_EDIT_PROPERCASE_BLEND: _pEditView->convertSelectedTextToNewerCase(TITLECASE_BLEND); break;
		case IDM_EDIT_SENTENCECASE_FORCE: _pEditView->convertSelectedTextToNewerCase(SENTENCECASE_FORCE); break;
		case IDM_EDIT_SENTENCECASE_BLEND: _pEditView->convertSelectedTextToNewerCase(SENTENCECASE_BLEND); break;
		case IDM_EDIT_INVERTCASE: _pEditView->convertSelectedTextToNewerCase(INVERTCASE); break;

		case IDM_EDIT_RANDOMCASE: _pEditView->convertSelectedTextToNewerCase(RANDOMCASE); break;
		case IDM_EDIT_BLOCK_COMMENT: doBlockComment(cm_toggle); break;
		case IDM_EDIT_BLOCK_COMMENT_SET: doBlockComment(cm_comment); break;
		case IDM_EDIT_BLOCK_UNCOMMENT: doBlockComment(cm_uncomment); break;
		case IDM_EDIT_STREAM_COMMENT: doStreamComment(); break;
		case IDM_EDIT_STREAM_UNCOMMENT: undoStreamComment(); break;
		...
		case IDM_VIEW_WRAP:
	    {
		    ...
		    // ViewMoveAtWrappingDisableFix: Disable wrapping messes up visible lines. Therefore save view
		    // position before in IDM_VIEW_WRAP and restore after SCN_PAINTED, as Scintilla-Doc. says
		    if(!isWraped) {
		...
		case IDM_VIEW_WRAP_SYMBOL:
		    ...
		    break;
		case IDM_VIEW_HIDELINES: _pEditView->hideLines(); break;
		case IDM_VIEW_ZOOMIN: _pEditView->execute(SCI_ZOOMIN); break;
		case IDM_VIEW_ZOOMOUT: _pEditView->execute(SCI_ZOOMOUT); break;
		...
		case IDM_VIEW_SYNSCROLLH:
	    {
		    ...
		    if(_syncInfo._isSynScollH) {
			    ...
			    int pixel =
				static_cast<int32_t>(_mainEditView.execute(SCI_TEXTWIDTH, STYLE_DEFAULT,
		...
		case IDM_VIEW_SUMMARY:
	    {
		    ...
		    if(pNativeSpeaker) {
			    ...
			    generic_string nbCharLabel =
				pNativeSpeaker->getLocalizedStrFromID("summary-nbchar", TEXT("Characters (without line endings): "));
			    ...
			    generic_string nbSelLabel1 =
				pNativeSpeaker->getLocalizedStrFromID("summary-nbsel1", TEXT(" selected characters ("));
		...
		case IDM_VIEW_MONITORING:
	    {
		    ...
		    if(curBuf->isMonitoringOn()) {
			    ...
			    if(::PathFileExists(longFileName)) {
				    ...
				    _nativeLangSpeaker.messageBox("DocNoExistToMonitor", _pPublicInterface->getHSelf(),
						TEXT("The file should exist to be monitored."), TEXT("Monitoring problem"), MB_OK);
			    }
		    }
		    break;
	    }
	    ...
	    {
		    EolType newFormat = (id == IDM_FORMAT_TODOS) ? EolType::windows : (id == IDM_FORMAT_TOUNIX) ? EolType::unix : EolType::macos;
		    Buffer* buf = _pEditView->getCurrentBuffer();
		    if(!buf->isReadOnly()) {
			    std::lock_guard<std::mutex> lock(command_mutex);
		...
		case IDM_FORMAT_KOI8R_CYRILLIC:
	    {
		    ...
		    if(buf->isDirty()) {
			    ...
			    int answer = _nativeLangSpeaker.messageBox("SaveCurrentModifWarning",
				    _pPublicInterface->getHSelf(),
				    TEXT(
					    "You should save the current modification.\rAll the saved modifications can not be undone.\r\rContinue?"),
				    TEXT("Save Current Modification"),
				    MB_YESNO);
	    ...
	    {
		    GridState st = id==IDM_SETTING_SHORTCUT_MAPPER_MACRO ? STATE_MACRO : id==
			IDM_SETTING_SHORTCUT_MAPPER_RUN ? STATE_USER : STATE_MENU;
		...
		case IDM_TOOL_SHA256_GENERATEINTOCLIPBOARD:
		    ...
		    break;
		case IDM_DEBUGINFO: _debugInfoDlg.doDialog(); break;
		case IDM_ABOUT:
	    {
		...
		case IDM_CMDLINEARGUMENTS:
		    // No translattable
		    ::MessageBox(_pPublicInterface->getHSelf(), Notepad_plus_Window::P_CommandArgHelp, TEXT("Notepad++ Command Argument Help"), MB_OK | MB_APPLMODAL);
		    break;
		case IDM_FORUM:
	    ...
	    {
		    // wingup doesn't work with the obsolete security layer (API) under xp since downloadings are
		...
		case IDM_EDIT_AUTOCOMPLETE: showAutoComp(); break;
		case IDM_EDIT_AUTOCOMPLETE_CURRENTFILE: autoCompFromCurrentFile(); break;
		case IDM_EDIT_AUTOCOMPLETE_PATH: showPathCompletion(); break;
		case IDM_EDIT_FUNCCALLTIP: showFunctionComp(); break;
		...
		case IDM_LANG_USER:
			{
				...
				CALLPTRMEMB(_pDocMap, setSyntaxHiliting());
			}
			break;
