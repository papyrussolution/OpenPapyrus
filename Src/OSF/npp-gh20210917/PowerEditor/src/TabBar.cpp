...
void TabBar::init(HINSTANCE hInst, HWND parent, bool isVertical, bool isMultiLine)
{
	...
	int style = WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE | TCS_FOCUSNEVER | TCS_TABS | WS_TABSTOP | vertical | multiLine;
...
{
	::DeleteObject(_hFont);
	_hFont = ::CreateFont(fontSize, 0, (_isVertical) ? 900 : 0, (_isVertical) ? 900 : 0, FW_NORMAL, 0, 0, 0, 0, 0, 0, 0, 0, fontName);
	if(_hFont)
...
void TabBar::deletItemAt(size_t index)
{
	if((index == _nbItem-1)) {
		...
		if(_nbItem > 1) {
			...
			if(itemRect.left < 5) { //if last visible tab, scroll left once (no more than 5px away should be
			                        // safe, usually 2px depending on the drawing)
				//To scroll the tab control to the left, use the WM_HSCROLL notification
				//Doesn't really seem to be documented anywhere, but the values do match the message
				// parameters
				//The up/down control really is just some sort of scrollbar
				//There seems to be no negative effect on any internal state of the tab control or the
				// up/down control
...
void TabBarPlus::init(HINSTANCE hInst, HWND parent, bool isVertical, bool isMultiLine)
{
	...
	SETIFZQ(_hFont, (HFONT)::GetStockObject(DEFAULT_GUI_FONT));
	SETIFZQ(_hLargeFont, (HFONT)::GetStockObject(SYSTEM_FONT));
...
void TabBarPlus::doOwnerDrawTab()
{
	...
	for(int i = 0; i < _nbCtrl; ++i) {
		if(_hwndArray[i]) {
			...
			SETFLAG(style, TCS_OWNERDRAWFIXED, isOwnerDrawTab());
			...
			::SendMessage(_hwndArray[i], TCM_SETPADDING, 0,
			    MAKELPARAM(_drawTabCloseButton ? paddingSizePlusClosebuttonDynamicW : paddingSizeDynamicW, 0));
		}
	}
}
...
{
	switch(i) {
		case activeText: _activeTextColour = colour2Set; break;
		case activeFocusedTop: _activeTopBarFocusedColour = colour2Set; break;
		case activeUnfocusedTop: _activeTopBarUnfocusedColour = colour2Set; break;
		case inactiveText: _inactiveTextColour = colour2Set; break;
		case inactiveBg: _inactiveBgColour = colour2Set; break;
		default: return;
	}
	doOwnerDrawTab();
}
...
int32_t TabBarPlus::getTabIndexAt(int x, int y)
{
...
bool TabBarPlus::isPointInParentZone(POINT screenPoint) const
{
	RECT parentZone;
	::GetWindowRect(_hParent, &parentZone);
	return (((screenPoint.x >= parentZone.left) && (screenPoint.x <= parentZone.right)) && (screenPoint.y >= parentZone.top) && (screenPoint.y <= parentZone.bottom));
}
	...
	switch(Message) {
		...
		case WM_MOUSEMOVE:
	    {
		    ...
		    if(_isDragging) {
			    ...
			    else if(iTabNow != -1 && _currentHoverTabItem != -1 && _currentHoverTabItem != iTabNow) { 
			    ...
			    if(_drawTabCloseButton) {
				    ...
				    if(_currentHoverTabItem != -1) {    // is hovering
					    ::SendMessage(_hSelf, TCM_GETITEMRECT, _currentHoverTabItem,
						reinterpret_cast<LPARAM>(&_currentHoverTabRect));
		...
		case WM_LBUTTONUP:
	    {
		    ...
		    if(_drawTabCloseButton) {
			    if((_whichCloseClickDown == currentTabOn) && _closeButtonZone.isHit(xPos, yPos, _currentHoverTabRect, _isVertical)) {
				    ...
				    //	 update close hover flag (_isCloseHover), so that x will be highlighted or not
				    // based on new _currentHoverTabRect
				    int nextTab = getTabIndexAt(xPos, yPos);
				    if(nextTab != -1) {
					    ::SendMessage(_hSelf, TCM_GETITEMRECT, nextTab,
						reinterpret_cast<LPARAM>(&_currentHoverTabRect));
...
void TabBarPlus::drawItem(DRAWITEMSTRUCT * pDrawItemStruct, bool isDarkMode)
{
	...
	if(isSelected && !isDarkMode) {
		...
		// the active tab is also slightly higher by default (use this to shift the tab cotent up bx two pixels
		// if tobBar is not drawn)
		if(_isVertical) {
	...
	if(_drawTabCloseButton) {
		...
		if(_isCloseHover && (_currentHoverTabItem == nTab) && (_whichCloseClickDown == -1))  // hover
			...
			idCloseImg =
			    isSelected ? (isDarkMode ? IDR_CLOSETAB_DM : IDR_CLOSETAB) : (isDarkMode ? IDR_CLOSETAB_INACT_DM :
			    IDR_CLOSETAB_INACT);
	...
	// Tab's caption must be encoded like this because otherwise tab control would make tab too small or too big for
	// the text.
	...
	*out = '\0';
	...
	if(_isVertical) {
		// center text horizontally (rotated text is positioned as if it were unrotated, therefore manual
