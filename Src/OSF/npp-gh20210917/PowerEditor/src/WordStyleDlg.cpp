						...
						case CPN_COLOURPICKED:
						    if(reinterpret_cast<HWND>(lParam) == _pFgColour->getHSelf()) {
							    ...
							    if((tabColourIndex = whichTabColourIndex()) != -1) {
								    TabBarPlus::setColour(_pFgColour->getColour(),
									(TabBarPlus::tabColourIndex)tabColourIndex);
							    }
							    else if(isDocumentMapStyle()) {
								    ViewZoneDlg::setColour(_pFgColour->getColour(),
									ViewZoneDlg::ViewZoneColorIndex::focus);
							    }
							    apply();
							    return TRUE;
						    }
							    ...
							    if((tabColourIndex = whichTabColourIndex()) != -1) {
								    ...
								    TabBarPlus::setColour(_pBgColour->getColour(),
									(TabBarPlus::tabColourIndex)tabColourIndex);
							    }
							    else if(isDocumentMapStyle()) {
								    ViewZoneDlg::setColour(_pBgColour->getColour(),
									ViewZoneDlg::ViewZoneColorIndex::frost);
							    }
							    apply();
							    return TRUE;
						    }
...
NppStyle & WordStyleDlg::getCurrentStyler() 
{
...
void WordStyleDlg::setStyleListFromLexer(int index)
{
	...
	for(const NppStyle & style : lexerStyler) {
...
/*virtual*/void WordStyleDlg::redraw(bool forceUpdate) const 
{
