...
// (windowsx.h) #define GET_X_LPARAM(lp) static_cast<short>(LOWORD(lp))
// (windowsx.h) #define GET_Y_LPARAM(lp) static_cast<short>(HIWORD(lp))
...
{
	if(relatedRoot.empty() || subFolder.empty())
		return false;
	...
	switch(message) {
		...
		case WM_COMMAND:
		    switch(LOWORD(wParam)) { 
				case FB_CMD_AIMFILE: selectCurrentEditingFile(); break;
			    case FB_CMD_FOLDALL: _treeView.foldAll(); break;
			    case FB_CMD_EXPANDALL: _treeView.expandAll(); break;
			    default: popupMenuCmd(LOWORD(wParam));
		    }
		    break;
		case WM_DESTROY:
	...
	if(hFile != INVALID_HANDLE_VALUE) {
		if(foundData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			if(!isInHiddenDir && (foundData.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN)) {
				...
				if((OrdinalIgnoreCaseCompareStrings(foundData.cFileName,
				    TEXT(".")) != 0) && (OrdinalIgnoreCaseCompareStrings(foundData.cFileName, TEXT("..")) != 0)) {
	...
	for(const auto f : _folderUpdaters) {
		if(f->_rootFolder._rootPath == rootFolderPath)
			...
			if(isRelatedRootFolder(rootFolderPath, f->_rootFolder._rootPath)) {
				NppParameters::getInstance().getNativeLangSpeaker()->messageBox("FolderAsWorspaceSubfolderExists", _hParent,
				    ...
				    TEXT("Folder as Workspace adding folder problem"), MB_OK, 0/*not used*/, rootFolderPath.c_str());
				return;
			}
		}
	}
...
HTREEITEM FileBrowser::createFolderItemsFromDirStruct(HTREEITEM hParentItem, const FolderInfo & directoryStructure)
{
	...
	if(directoryStructure._parent == nullptr && hParentItem == nullptr) {
		...
		hFolderItem =
		    _treeView.addItem(directoryStructure._name.c_str(), TVI_ROOT, INDEX_CLOSE_ROOT, reinterpret_cast<LPARAM>(customData));
	}
	else {
		...
		hFolderItem =
		    _treeView.addItem(directoryStructure._name.c_str(), hParentItem, INDEX_CLOSE_NODE,
...
std::vector <FileBrowser::FilesToChange> FileBrowser::getFilesFromParam(LPARAM lParam) const
{
	...
	for(size_t i = 0; i < filesToChange.size(); i++) {
		...
		const auto it = std::find_if(groupedFiles.begin(), groupedFiles.end(), [&commonPath](const auto & group) {
			return group._commonPath == commonPath;
		});
...
LPCWSTR explainAction(DWORD dwAction)
{
	switch(dwAction) {
		...
		case FILE_ACTION_RENAMED_NEW_NAME: return L"Renamed ";
...
DWORD WINAPI FolderUpdater::watching(void * params)
{
	...
	if(dir2Watch[dir2Watch.length() - 1] != '\\')
		dir2Watch += TEXT("\\"); // CReadDirectoryChanges will add another '\' so we will get "\\" as a
	                                 // separator (of monitored root) in the notification
	...
	while(toBeContinued) {
		...
		switch(waitStatus) {
		    ...
		    {
			    static const uint MAX_BATCH_SIZE = 100;
			    ...
			    if(oneof2(dwAction, FILE_ACTION_ADDED, FILE_ACTION_REMOVED)) {
				    processChange(dwAction, filesToChange, thisFolderUpdater);
			    }
		    }
		    break;
...
void FolderUpdater::processChange(DWORD dwAction, std::vector <generic_string> filesToChange, FolderUpdater* thisFolderUpdater)
{
	...
	switch(dwAction) {
		case FILE_ACTION_ADDED:
			...
			reinterpret_cast<LPARAM>(&filesToChange));
		    oldName = TEXT("");
		    break;
		...
		case FILE_ACTION_RENAMED_NEW_NAME:
		    if(!oldName.empty()) {
				...
				FB_RNFILE,
				reinterpret_cast<WPARAM>(nullptr),
				reinterpret_cast<LPARAM>(&fileRename));
		    }
		    oldName = TEXT("");
		    break;
