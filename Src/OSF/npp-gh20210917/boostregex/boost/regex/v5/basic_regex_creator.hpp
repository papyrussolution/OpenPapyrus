...
protected:
	...
	std::vector<unsigned char>    m_recursion_checks;// notes which recursions we've followed while analysing this
			                               // expression
	typename traits::char_class_type m_word_mask;  // mask used to determine if a character is a word character
	typename traits::char_class_type m_mask_space; // mask used to determine if a character is a word character
	typename traits::char_class_type m_lower_mask;  // mask used to determine if a character is a lowercase
			                                // character
	typename traits::char_class_type m_upper_mask; // mask used to determine if a character is an uppercase
			                               // character
	typename traits::char_class_type m_alpha_mask; // mask used to determine if a character is an alphabetic
			                               // character
private:
...
basic_regex_creator<charT, traits>::basic_regex_creator(regex_data<charT, traits>* data)
	...
	m_has_backrefs(false), m_bad_repeats(0), m_has_recursions(false), m_word_mask(0), m_mask_space(0), m_lower_mask(0), m_upper_mask(0),
	m_alpha_mask(0)
{
...
template <class charT, class traits>
inline re_syntax_base* basic_regex_creator<charT, traits>::append_set(const basic_char_set<charT, traits>& char_set)
{
	...
	return char_set.has_digraphs()
	       ...
	       : append_set(char_set,
		   static_cast<truth_type*>(0));
}

template <class charT, class traits>
re_syntax_base* basic_regex_creator<charT, traits>::append_set(const basic_char_set<charT, traits>& char_set, std::integral_constant<bool,
    false>*)
{
	typedef typename traits::string_type string_type;
	...
	while(sfirst != slast) {
		charT* p =
		    static_cast<charT*>(this->m_pdata->m_data.extend(sizeof(charT) *
		    (sfirst->first == static_cast<charT>(0) ? 1 : sfirst->second ? 3 : 2)));
    ...
    true>*)
{
	...
	memzero(result->_map, sizeof(result->_map));
	//
	...
	while(first != last) {
		...
		if(flags() & regex_constants::collate) {
			...
			memset(result->_map + static_cast<unsigned char>(c1), true, static_cast<unsigned char>(1u) +
			    static_cast<unsigned char>(static_cast<unsigned char>(c2) - static_cast<unsigned char>(c1)));
		}
	}
	//
	// and now the classes:
...
void basic_regex_creator<charT, traits>::finalize(const charT* p1, const charT* p2)
{
	...
	memzero(m_pdata->m_startmap, sizeof(m_pdata->m_startmap));
	...
	while(state) {
		switch(state->type)
		{
			...
			case syntax_element_alt:
			    memzero(static_cast<re_alt*>(state)->_map, sizeof(static_cast<re_alt*>(state)->_map));
			    ...
			    if(idx < 0) {
				    ...
				    if(idx >= hash_value_mask) {
					    ...
					    if(idx <= 0) {
						    ...
						    if(0 == this->m_pdata->m_status) // update the error code if not
										     // already set
			    ...
			    if(l_map) {
				    ...
				    if(static_cast<re_set_long<m_type>*>(state)->singleton) {
					    ...
					    for(unsigned int i = 0; i < (1u << CHAR_BIT); ++i) {
						    ...
						    if(&c !=
							re_is_set_member(&c, &c + 1, static_cast<re_set_long<m_type>*>(state), *m_pdata,
							l_icase))
							    l_map[i] |= mask;
					    }
				    }
