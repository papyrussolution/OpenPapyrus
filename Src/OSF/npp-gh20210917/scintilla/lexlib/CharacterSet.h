...
constexpr bool IsAlphaNumeric(int ch) noexcept { return ((ch >= '0') && (ch <= '9')) || ((ch >= 'a') && (ch <= 'z')) || ((ch >= 'A') && (ch <= 'Z')); }
...
template <typename T> constexpr T MakeUpperCase(T ch) noexcept { return (ch < 'a' || ch > 'z') ? ch : (ch - 'a' + 'A'); }
template <typename T> constexpr T MakeLowerCase(T ch) noexcept { return (ch < 'A' || ch > 'Z') ? ch : (ch - 'A' + 'a'); }
