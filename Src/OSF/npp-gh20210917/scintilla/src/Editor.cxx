...
constexpr bool CanEliminate(const DocModification &mh) noexcept { return (mh.modificationType & (SC_MOD_BEFOREINSERT | SC_MOD_BEFOREDELETE)) != 0; }
...
void Editor::InsertCharacter(std::string_view sv, CharacterSource charSource) {
	...
	FilterSelections();
	{
		...
		std::sort(selPtrs.begin(), selPtrs.end(),
		    [](const SelectionRange * a, const SelectionRange * b) noexcept {
			return *a < *b;
		});
 ...
 */
Sci::Position Editor::FindText(uptr_t wParam,              ///< Search modes : @c SCFIND_MATCHCASE, @c SCFIND_WHOLEWORD,
 ...
 * @return The position of the found text, -1 if not found.
 */
Sci::Position Editor::SearchText(unsigned int iMessage,              ///< Accepts both @c SCI_SEARCHNEXT and @c SCI_SEARCHPREV.
