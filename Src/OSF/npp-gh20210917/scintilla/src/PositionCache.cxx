...
ScreenLine::ScreenLine(const LineLayout * ll_,
    int subLine,
    const ViewStyle &vs,
    XYPOSITION width_,
			...
			if(((nextBreak > 0) && (ll->styles[nextBreak] != ll->styles[nextBreak - 1])) ||
				...
				while((nextBreak >= saeNext) && (saeNext < lineRange.end)) {
					...
					saeNext =
					    ...
					    selAndEdge.size()) ? selAndEdge[saeCurrentPos] : lineRange.end);
				}
...
void PositionCache::MeasureWidths(Surface * surface, const ViewStyle &vstyle, unsigned int styleNumber,
	...
	if(len > BreakFinder::lengthStartSubdivision) {
		...
		while(startSegment < len) {
			const unsigned int lenSegment = pdoc->SafeSegment(s + startSegment,
				len - startSegment,
