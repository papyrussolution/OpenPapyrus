	...
	if(dbcsCodePage) {
		if(SC_CP_UTF8 == dbcsCodePage) {
			if(increment == 1) {
				...
				// If ch is not a trail byte then pos is valid intercharacter position
				if(UTF8IsTrailByte(ch)) {
					// If ch is a trail byte in a valid UTF-8 character then return start of
					// character
					Sci::Position startUTF = pos;
					Sci::Position endUTF = pos;
					if(InGoodUTF8(pos, startUTF, endUTF)) {
			...
			for(int step = 0; step < steps; step++) {
				...
				if(action.at == removeAction) {
					...
					if((coalescedRemoveLen > 0) &&
					    (action.position == prevRemoveActionPos ||
					    action.position == (prevRemoveActionPos + prevRemoveActionLen))) {
		...
		if(resr.increment == 1) {
			if(line == resr.lineRangeStart) {
				if((resr.startPos != startOfLine) && searchforLineStart)
					continue;       // Can't match start of line if start position after start of
				                        // line
	...
	for(Sci::Position j = 0; j < *length; j++) {
		if(text[j] == '\\') {
			if(text[j + 1] >= '0' && text[j + 1] <= '9') {
				...
				if(!search.pat[patNum].empty())         // Will be null if try for a match that did not
					                                // occur
