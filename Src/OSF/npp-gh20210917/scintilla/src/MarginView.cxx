					...
					if(stMargin.text && ValidStyledText(vs, vs.marginStyleOffset, stMargin)) {
						if(firstSubLine) {
							...
							if(vs.ms[margin].style == SC_MARGIN_RTEXT) {
								const int width = WidestLineWidth(surface,
									vs,
									vs.marginStyleOffset,
									stMargin);
							...
							// if we're displaying annotation lines, colour the margin to
							// match the associated document line
						...
						if(marks & 1) {
							...
							if((vs.ms[margin].mask & SC_MASK_FOLDERS) &&
								...
								if(highlightDelimiter.IsBodyOfFoldBlock(lineDoc)) {
									...
									if(firstSubLine) {
										part =
										    headWithTail ? LineMarker::FoldPart::headWithTail :
										    LineMarker
										    ::FoldPart::head;
									}
									else {
