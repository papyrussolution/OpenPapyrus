...
void SCI_METHOD LexerCPP::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument * pAccess) {
	...
	for(; sc.More();) {
		if(sc.atLineStart) {
			...
			// Inside inactive preprocessor declaration, state will be reset anyway at the end of this
			// block.
			    ...
			    if(sc.atLineStart || sc.atLineEnd || !setWord.Contains(sc.ch) || (sc.ch == '.')) {
				    ...
				    if(literalString || sc.ch == '\'') {
					    ...
					    if(valid) {
						    if(literalString) {
							    if(raw) {
								    // Set the style of the string prefix to
								    // SCE_C_STRINGRAW but then change to
								    // SCE_C_DEFAULT as that allows the raw string start
								    // code to run.
				...
				if(sc.atLineEnd) {
					...
					if(options.trackPreprocessor) {
						...
						if(sc.Match("ifdef") || sc.Match("ifndef")) {
							...
							// .. #endif
							// #elif is shown as active if either preceding or following
							// section is active
							// as that means that it contributed to the result.
							if(!preproc.CurrentIfTaken()) {
								// Inactive, if expression true then may become active
								// if parent scope active
								assert(sc.state == (SCE_C_PREPROCESSOR|inactiveFlag));
								// Similar to #if
			...
			if(it != preprocessorDefinitions.end()) {
				...
				if(it->second.IsMacro()) {
					if((i + 1 < tokens.size()) && (tokens.at(i + 1) == "(")) {
						...
						macroTokens.erase(std::remove_if(macroTokens.begin(), macroTokens.end(), OnlySpaceOrTab),
						    macroTokens.end());

						for(size_t iMacro = 0; iMacro < macroTokens.size();) {
							if(setWordStart.Contains(macroTokens[iMacro][0])) {
								std::map<std::string, std::string>::const_iterator itFind = arguments.find(
									macroTokens[iMacro]);
