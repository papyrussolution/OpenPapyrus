...
void SCI_METHOD LexerABL::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument * pAccess) {
	...
	// Look back to set variables that are actually invisible secondary states. The reason to avoid formal states is
	// to cut down on state's bits
	if(startPos > 0) {
		...
		while(back >= 0 && (checkCommentNestingLevel || checkIsSentenceStart)) {
			...
			if(checkIsSentenceStart && st != SCE_ABL_COMMENT && st != SCE_ABL_LINECOMMENT && st != SCE_ABL_CHARACTER &&
			    st != SCE_ABL_STRING) {
	...
	for(; sc.More();) {
		...
		switch(sc.state) {
			case SCE_ABL_OPERATOR:
			    ...
			    break;
			case SCE_ABL_NUMBER:
			    // We accept almost anything because of hex. and maybe number suffixes and scientific
			...
			case SCE_ABL_IDENTIFIER:
			    if(sc.atLineStart || sc.atLineEnd || (!setWord.Contains(sc.ch) && sc.ch != '-')) {
				    ...
				    bool isLastWordEnd = (s[0] == 'e' && s[1] =='n' && s[2] == 'd' && !IsAlphaNumeric(s[3]) && s[3] != '-'); // helps
				                                                                                                             // to
				                                                                                                             // identify
				                                                                                                             // "end
				                                                                                                             // trigger"
			...
			case SCE_ABL_PREPROCESSOR:
			    if(sc.atLineStart && !continuationLine) {
				    sc.SetState(SCE_ABL_DEFAULT);
				    // Force Scintilla to acknowledge changed stated even though this change might
				    // happen outside of the current line
		...
		if(sc.state == SCE_ABL_DEFAULT) {
			if(IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				...
				// Force Scintilla to acknowledge changed stated even though this change might happen
