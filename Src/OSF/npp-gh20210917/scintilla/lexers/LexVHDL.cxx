	...
	while(sc.More()) {
		...
		if(sc.state == SCE_VHDL_OPERATOR) {
			...
			if(!isExtendedId && (!IsAWordChar(sc.ch) || (sc.ch == '.'))) {
				...
				// extended identifiers are terminated by backslash, check for end of line in case we
				// have invalid syntax
				isExtendedId = false;
			...
			if(IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				...
				if(sc.GetRelative(2) == '\'') {
					if(sc.chNext != '(' || sc.GetRelative(4) != '\'') {
						...
						sc.ForwardSetState(SCE_VHDL_DEFAULT);
						advance = false;
					} // else can be a tick or a character literal, need more context, eg.:
					  // identifier'('x')
			...
			if(IsAWordChar(ch) && !IsAWordChar(chNext)) {
				...
				if(keywords.InList(s)) {
					if(
						...
						if(strcmp(prevWord, "end") != 0) { // check for instantiated unit by
							                           // backward searching for the colon.
							...
							// check for a colon (':') before the instantiated units
							// "entity", "component" or "configuration". Don't fold
							// thereafter.
							if(chAtPos != ':') {
								if(levelMinCurrentElse > levelNext) {
									levelMinCurrentElse = levelNext;
						...
						if(strcmp(prevWord, "end") != 0) { // check for "end procedure" etc.
							// This code checks to see if the procedure / function is a
							// definition within a "package"
					...
					else if(strcmp(s, "elsif") == 0) { // elsif is followed by then so folding
						                           // occurs correctly
						levelNext--;
					}
					else if(strcmp(s, "else") == 0) {
						if(strcmp(prevWord, "when") != 0) { // ignore a <= x when y else z;
							levelMinCurrentElse = levelNext - 1; // VHDL else is all on its
							                                     // own so just dec. the min
							                                     // level
						}
					}
					else if(
