				...
				if(sc.atLineEnd) {
					...
					if(sc.Match("protected")) {
						...
						if(sc.Match("ifdef") || sc.Match("ifndef")) {
							...
							// Ensure only one chosen out of `if .. `elsif .. `elsif ..
							// `else .. `endif
							if(!preproc.CurrentIfTaken()) {
								// Similar to `ifdef
							...
							if(options.updatePreprocessor && !preproc.IsInactive()) {
								...
								if((endName < restOfLine.length()) && (restOfLine.at(endName) == '(')) {
									...
									ppDefineHistory.push_back(PPDefinition(curLine, key, value, false,
									    args));
									definitionsChanged = true;
								}
								else {
							...
							if(options.updatePreprocessor && !preproc.IsInactive()) {
								...
								for(itDef = preprocessorDefinitions.begin();
								    itDef != preprocessorDefinitions.end();
								    ++itDef) {
									ppDefineHistory.push_back(PPDefinition(curLine, itDef->first, "",
									    true));
								}
...
void SCI_METHOD LexerVerilog::Fold(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument * pAccess)
{
	...
	for(Sci_PositionU i = startPos; i < endPos; i++) {
		...
		if(style == SCE_V_OPERATOR) {
			...
			// wait and disable statements containing '(' will not contain "fork" keyword, special
