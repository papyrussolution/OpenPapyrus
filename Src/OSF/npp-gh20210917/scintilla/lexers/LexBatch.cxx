...
static void ColouriseBatchDoc(Sci_PositionU startPos,
	...
	if(startPos > 0) {
		...
		while(startPos > 0) {
			ln--;
			if((styler.SafeGetCharAt(startPos-3) == '^' && styler.SafeGetCharAt(startPos-2) == '\r' &&
			    styler.SafeGetCharAt(startPos-1) == '\n')
	...
	for(Sci_PositionU i = startPos; i < startPos + length; i++) {
		...
		if(AtEOL(styler, i) || (linePos >= sizeof(lineBuffer) - 1) || (i==startPos + length-1)) {
			...
			bool stopLineProcessing = false;  // Used to stop line processing if Comment or Drive Change
			                                  // found
			...
			while(offset < lengthLine  && !stopLineProcessing) {
				...
				if(IsBSeparator(wordBuffer[0])) {
					...
					if((CompareCaseInsensitive(wordBuffer, "errorlevel") == 0) ||
						...
						// Identify External Command / Program Location for CALL, DO, LOADHIGH
						// and LH
					}
					...
					// Check for Special Keyword in list, External Command / Program, or Default
					// Text
				}
					...
					// Check for Argument  (%n), Environment Variable (%x...%) or Local Variable
					// (%%a)
				}
				else if(wordBuffer[0] == '%') {
					// Colorize Default Text
			...
			if(!continueProcessing && (i<startPos + length-1)) {
				if(linePos==1 || (linePos==2 && lineBuffer[1]=='\r'))  // empty line on Unix and Mac or
					                                               // on Windows
					continueProcessing = true;
				else {
