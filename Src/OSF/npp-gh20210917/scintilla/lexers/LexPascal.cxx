	...
	if(strcmp(s, "record") == 0) {
		...
		if(j < endPos) {
			...
			if(styler.SafeGetCharAt(j) == ';') {
				...
				// and object method declarations ("TNotifyEvent = procedure(Sender: TObject) of
				// object;")
				ignoreKeyword = true;
			}
				...
				if(styler.SafeGetCharAt(j) == '(') {
					// Handle simplified complete class declarations ("type TMyClass =
					// class(TObject);")
					...
					char s2[11];    // Size of the longest possible keyword + one additional
					                // character + null
