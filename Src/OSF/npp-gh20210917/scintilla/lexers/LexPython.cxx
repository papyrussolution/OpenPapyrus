...
/* Return the state to use for the string starting at i; *nextIndex will be set to the first index following the
   quote(s) */
int GetPyStringState(Accessor &styler, Sci_Position i, Sci_PositionU * nextIndex, literalsAllowed allowed) {
	...
	// Advance beyond r, u, or ur prefix (or r, b, or br in Python 2.7+ and r, f, or fr in Python 3.6+), but bail if
	// there are any unexpected chars
	if(ch == 'r' || ch == 'R') {
		i++;
...
void SCI_METHOD LexerPython::Fold(Sci_PositionU startPos, Sci_Position length, int /*initStyle - unused*/, IDocument * pAccess) {
	...
	while((lineCurrent <= docLines) && ((lineCurrent <= maxLines) || prevQuote)) {
		...
		if(lineNext <= docLines) {
			...
			const Sci_Position lookAtPos =
			    (styler.LineStart(lineNext) == styler.Length()) ? styler.Length() - 1 : styler.LineStart(lineNext);
