...
struct OptionSetBaan : public OptionSet<OptionsBaan> {
	OptionSetBaan() {
		...
		DefineProperty("fold.baan.keywords.based",
...
static inline int IsAnyOtherIdentifier(char * s, Sci_Position sLength) {
	...
	switch(sLength) {
		case 8:
		    ...
		    break;
		case 9:
		    if(s[0] == 't' && isalpha(s[1]) && isalpha(s[2]) && isalpha(s[3]) && isalpha(s[4]) && isalpha(s[5]) && IsADigit(s[6]) &&
			IsADigit(s[7]) && IsADigit(s[8])) {
			    //t^^^^^###
			    return(SCE_BAAN_TABLEDEF);
		    }
		    ...
		    break;
		case 13:
		    if(s[8] == '.' && isalpha(s[0]) && isalpha(s[1]) && isalpha(s[2]) && isalpha(s[3]) && isalpha(s[4]) && IsADigit(s[5]) &&
			IsADigit(s[6]) && IsADigit(s[7])) {
			    //^^^^^###.****
			    return(SCE_BAAN_TABLESQL);
		    }
			...
			if(styler.Match(i, "table") || styler.Match(i, "extern") || styler.Match(i, "long")
				...
				for(Sci_Position j = eol_pos; j > pos; j--) {
					...
					if(styleFromEnd == SCE_BAAN_COMMENT)
						...
						//Above conditions ensures, Declaration is not part of any function
						// parameters.
						return true;
					else
						return false;
				...
				if(styleNext != SCE_BAAN_WORD) {
					...
					if(strcmp(word, "for") == 0) {
						...
						if(styler.Match(j, "update")) {
							// Means this is a "for update" used by Select which is already
							// folded.
							foldStart = false;
						}
					}
						...
						if(!styler.Match(j, "case")) {
							// Means this is not a "on Case" statement... could be "on" used
							// by index.
							foldStart = false;
						}
					}
						...
						if(foldNextSelect) {
							// Next Selects are sub-clause till reach of selectCloseTags[]
							// array.
			...
			if(currLineStyle != 0 && currLineStyle != nextLineStyle) {
				...
				if(!beforeDeclarationSection) {
					...
					if(nextLineStyle == SCE_BAAN_WORD5 && priorSectionIsSubSection(lineCurrent-1, styler))
						...
						// On reaching the next main section, the previous main as well sub
						// section ends.
						levelCurrent--;
				}
			}
