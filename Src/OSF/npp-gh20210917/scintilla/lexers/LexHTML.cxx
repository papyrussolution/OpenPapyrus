...
void classifyWordHTPy(Sci_PositionU start,
    Sci_PositionU end,
...
Sci_Position FindPhpStringDelimiter(std::string &phpStringDelimiter,
    Sci_Position i,
...
void SCI_METHOD LexerHTML::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument * pAccess) {
	...
	// If inside a tag, it may be a script tag, so reread from the start of line starting tag to ensure any language
	// tags are seen
	if(InTagState(state)) {
	...
	// String can be heredoc, must find a delimiter first. Reread from beginning of line containing the string, to
	// get the correct lineState
		...
		if(fold) {
			switch(scriptLanguage) {
				...
				case eScriptPHP:
				    ...
				    else if(((state == SCE_HPHP_COMMENT) || (state == SCE_HJ_COMMENT)) && foldComment && (ch == '*') &&
					(chNext == '/')) {
					    levelCurrent--;
				    }
				    break;
...
//				StateToPrint = SCE_H_SGML_BLOCK_DEFAULT;
			...
			case SCE_H_ENTITY:
			    ...
			    if(ch != '#' && !(IsASCII(ch) && isalnum(ch)) // Should check that '#' follows '&', but it
			                                                  // is unlikely anyway...
				&& ch != '.' && ch != '-' && ch != '_' && ch != ':') { // valid in XML
				    if(!IsASCII(ch))    // Possibly start of a multibyte character so don't allow this
					                // byte to be in entity style
			...
			case SCE_HP_START:
				...
				styler.SafeGetCharAt(i + 3) == '-') {
				    ...
				    if(chNext == '\"' && chNext2 == '\"') {
					    ...
					    //					state =
					    // statePrintForState(SCE_HP_STRING,inScriptType);
					    state = SCE_HP_STRING;
				    }
			    }
			    else if(ch == '\'') {
