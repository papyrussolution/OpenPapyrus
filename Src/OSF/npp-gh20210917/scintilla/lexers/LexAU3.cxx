...
//                  Changed folding inside of #cs-#ce. Default is no keyword folding inside comment blocks when
// fold.comment=1
		...
		switch(sc.state)
		{
			case SCE_AU3_COMMENTBLOCK:
		    {
			    ...
			    if(!(IsAWordChar(sc.ch) || (sc.ch == '-' && strcmp(s, "#comments") == 0))) {
				    if((strcmp(s, "#ce")== 0 || strcmp(s, "#comments-end")== 0))
					    sc.SetState(SCE_AU3_COMMENT);              // set to comment line for the
					                                               // rest of the line
				    else
	...
	for(Sci_Position i = startPos; i < endPos; i++) {
		...
		if((ch == '\r' && chNext != '\n') || (ch == '\n') || (i == endPos)) {
			...
			if(szKeywordlen > 0 && (!(chPrev == '_')) &&
				...
				// create double Fold for select&switch because Case will subtract one of the current
				// level
			...
			visibleChars = 0;
			// if the last character is an Underscore then don't reset since the line continues on the next
			// line.
			if(!(chPrev == '_')) {
				szKeywordlen = 0;
				szThenlen = 0;
