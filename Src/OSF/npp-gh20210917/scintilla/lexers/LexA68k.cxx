	...
	************************************************************/
	...
	for(; sc.More(); sc.Forward()) {
		...
		if(sc.state != SCE_A68K_DEFAULT) {
			...
			************************************************************/
			...
			// In this case, colourise it if was a keyword.
				...
				sc.GetCurrentLowered(Buffer, sizeof(Buffer));           // Buffer the string of the
				                                                        // current context
				if(cpuInstruction.InList(Buffer)) {                     // And check if it belongs to a
					                                                // keyword list
		...
		************************************************************/
		...
		// is a local label (second case is used for macro local labels). We set it already as a label, it can't
		// be a macro/equ declaration
		...
		else if((sc.ch < 0x80) && (sc.ch == ';')) {                     // Default: alert in a comment. If it
			                                                        // doesn't match
			sc.SetState(SCE_A68K_COMMENT);                          // with an alert, it will be toggle to a
			                                                        // normal comment
		}
		...
		else if((sc.ch < 0x80) && (sc.ch == '\\') && (isdigit(sc.chNext))) { // Replacement symbols in macro are
			                                                             // prefixed with '\'
			sc.SetState(SCE_A68K_MACRO_ARG);
		}
		else if((sc.ch < 0x80) && IsIdentifierStart(sc.ch)) {           // An identifier: constant, label,
			                                                        // etc...
