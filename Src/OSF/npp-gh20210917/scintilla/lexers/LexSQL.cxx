...
void SCI_METHOD LexerSQL::Fold(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument * pAccess) {
	...
	if(lineCurrent > 0) {
		// Backtrack to previous line in case need to fix its fold status for folding block of single-line
		// comments (i.e. '--').
		...
		if(style == SCE_SQL_WORD && stylePrev != SCE_SQL_WORD) {
			...
			if(!options.foldOnlyBegin &&
				...
				if(endFound) {
					endFound = false;
					if(options.foldOnlyBegin && !isUnfoldingIgnored) {
						// this end isn't for begin block, but for loop block ("end loop;") or
						// case block ("end case;")
						// so ignore previous "end" by increment levelNext.
						levelNext++;
					}
				    ...
				    // and IF or CASE aren't on only one line with ELSE or ELSIF (with flag
				    // statementFound)
				...
				// Don't foldind when CASE and WHEN are on the same line (with flag statementFound) (eg.
				// "CASE selector WHEN expression1 THEN sequence_of_statements1;\n")
