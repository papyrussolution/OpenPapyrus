		...
		switch(MASKACTIVE(style))
		{
					    ...
					    if(MatchIgnoreCase(styler, i, "begin"))
						    ...
						    if(!foldOnlyBegin) {
							    ...
							    if(whileFound || loopFound || repeatFound || caseFound)
								    ...
								    // IF alone does not increase the fold level as it
								    // is also used in non-block'ed
								    // code like DROP PROCEDURE blah IF EXISTS.
								    // Instead THEN opens the new level (if not part of
								    // an ELSEIF or WHEN (case) branch).
								    if(MatchIgnoreCase(styler, i, "then")) {
									    ...
									    // Neither of
									    // if/then/while/loop/repeat/case, so check
									    // for
			...
			default:
			    if(!isspacechar(currentChar) && endPending) {
				    // END followed by a non-whitespace character (not covered by other cases like
				    // identifiers)
				    // also should end a folding block. Typical case: END followed by self defined
				    // delimiter.
				    levelNext--;
				    if(levelNext < SC_FOLDLEVELBASE)
					    levelNext = SC_FOLDLEVELBASE;
