...
int RecogniseErrorListLine(const char * lineBuffer, Sci_PositionU lengthLine, Sci_Position &startValue) {
	if(lineBuffer[0] == '>') {
		...
		enum { stInitial,
		       ...
		       stUnrecognized} state = stInitial;
		for(Sci_PositionU i = 0; i < lengthLine; i++) {
			...
			if(state == stInitial) {
				...
				if((ch == ' ') && (chNext == ':')) {
					...
					unsigned numstep;
					if(ch == ' ')
						numstep = 1; // ch was ' ', handle as if it's a delphi errorline, only
						             // add 1 to i.
					else
						numstep = 2; // otherwise add 2.
					Sci_PositionU chPos = 0;
					for(Sci_PositionU j = i + numstep;
					    j < lengthLine && IsAlphabetic(lineBuffer[j]) && chPos < sizeof(word) - 1;
					    j++)
...
void ColouriseErrorListLine(const std::string &lineBuffer,
