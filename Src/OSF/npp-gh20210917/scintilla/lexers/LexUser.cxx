...
// static vector<int> * foldVectorStatic;  // foldVectorStatic is used for debugging only, it should be commented out in
// production code !
...
struct forwardStruct {
	...
	forwardStruct() : _vec(0), _sceID(0), _maskID(0) {
			...
			for(iter1 = fwEndVectors[i]->begin(); iter1 != fwEndVectors[i]->end(); ++iter1) {
				...
				for(; iter2 != iter1->end(); ++iter2) {
					...
					for(;; ++iter3) {
						...
						b = ignoreCase ? toupper(sc.GetRelative(forward + index++)) : sc.GetRelative(
							forward + index++);
						if(a != b)
							break;
						if(iter3 != iter2->end())
...
static bool isInListForward3(vector<string> * tokens, StyleContext & sc, bool ignoreCase, Sci_Position offset, size_t & moveForward)
{
	...
	for(iter1 = tokens->begin(); iter1 != tokens->end(); ++iter1) {
		...
		for(iter2 = iter1->begin(); iter2 != iter1->end(); ++iter2) {
			...
			b =
			    static_cast<unsigned char>(ignoreCase ? toupper(sc.GetRelative(offset + indexb++)) : sc.GetRelative(offset +
			    indexb++));
			if(a != b) {
				isFound = false;
				break;
    ...
    bool ignoreCase, int decSeparator, size_t & moveForward)
{
	...
	for(;;) {
		...
		if(previousWasRange == true) { // prefix in the middle is an error case, so any number is treated as if
		                               // it had a prefix
			                       // the only acceptable position for prefix is immediatelly after range
			                       // char, e.g. 0x10--0x15
		...
		if(hasExp == false) {
			if(toupper(sc.GetRelative(offset)) == 'E') { // treat E as scientific notation only if it does
			                                             // not match extra chars!!
    ...
    int & moveForward, vvstring * fwEndVectors[], int & nlCount, size_t docLength)
{
	...
	// in case 'isInListBackward' finds such keywords it will set 'moveForward' parameter so algorythm could adjust
	// position
	...
	while(i >= 0) {
		while(static_cast<unsigned char>(ignoreCase ? toupper(list.WordAt(i)[0]) : list.WordAt(i)[0]) ==
		    (ignoreCase ? toupper(firstChar) : firstChar)) {
			a = 0;
			b = 0;
			bNext = 0;
			indexa = 0;
				...
				if(a == '\v' || a == '\b') {
					...
					if(isWhiteSpace2(b, nlCountTemp, wsChar, bNext)) {
						...
						while((sc.currentPos + offset + indexb) <= docLength &&
						    isWhiteSpace2(b, nlCountTemp, wsChar, bNext));

						a =
						    static_cast<unsigned char>(ignoreCase ? toupper(list.WordAt(i)[indexa++]) : list.WordAt(
							    i)
						    [indexa++]);
					}
			...
			if(!a) {
				...
				if(wsChar) {
					...
					// then the next word in the text file must be treated as part of multi-part
					// keyword
					// e.g. prefixed "else if" matches "else if nextWord", but not "else iffy"
					if(specialMode) {
						if(fwDelimiterFound == FORWARD_WHITESPACE_FOUND) { // there must be a
						                                                   // white space !!
							// skip whitespace (all of it)
							int savedPosition = indexb; // return here if whitespace is not
							                            // followed by another word
							for(;;) {
								if((sc.currentPos + offset + indexb) > docLength)
									break;
								if(!isWhiteSpace2(sc.GetRelative(offset + indexb), nlCountTemp, wsChar,
								    sc.GetRelative(offset + indexb + 1)))
									break;
								++indexb;
							}

							// skip next "word" (if next word is not found, go back to end
							// of multi-part keyword)
							// it is not necessary to check EOF position here, because
							// sc.GetRelative returns ' ' beyond EOF
							bool nextWordFound = false;
							while(!isWhiteSpace2(sc.GetRelative(indexb + offset), nlCountTemp, wsChar,
							    sc.GetRelative(offset + indexb + 1))) {
								if(isInListForward2(fwEndVectors, FW_VECTORS_TOTAL, sc, ignoreCase,
								    indexb + offset)) {
									break;
								}
								++indexb;
...
static void ColouriseUserDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList * kwLists[], Accessor &styler)
{
	...
	_itoa(SCE_USER_STYLE_DELIMITER1,    (nestingBuffer+19), 10);    int delim1Nesting       = styler.GetPropertyInt(nestingBuffer, 0);// one
	                                                                                                                                // byte
	                                                                                                                                // difference
	_itoa(SCE_USER_STYLE_DELIMITER2,    (nestingBuffer+19), 10);    int delim2Nesting       = styler.GetPropertyInt(nestingBuffer, 0);// for
	                                                                                                                                // two-digit
	                                                                                                                                // numbers
		...
		while(switchPerformed) {
			...
			for(int i = 0; i<MAPPER_TOTAL; ++i) { // for each keyword list object
				for(int j = 0; j<kwLists[i]->Length(); ++j) { // for each keyword within object
					...
					if(equal && isMultiPart && kwLists[i]->WordAt(j+1)[z]) { // perform switch only
					                                                         // if next word is
					                                                         // longer !
	...
	vvstring * numberDelimSeparators[SCE_USER_TOTAL_DELIMITERS+6][SCE_USER_TOTAL_DELIMITERS+6]; // TODO: define
	                                                                                            // hardcoded values
	                                                                                            // as constants
	                                                                                            // (syncy also for
	                                                                                            // FW_VECTORS_TOTAL)
	...
	for(; finished;) {
		...
		switch(sc.state)
		{
			...
			case SCE_USER_STYLE_DELIMITER8:
		    {
			    ...
			    while(loopEscape == true) {
				    ...
				    for(; iter != (*delimEscape)[openIndex].end(); ++iter) {
					    if(ignoreCase ? sc.MatchIgnoreCase2(iter->c_str()) : sc.Match(iter->c_str())) {
						    sc.Forward(iter->length() + 1); // escape is found, skip escape
							                            // string and one char after it.
						    loopEscape = true;
						    //break;
					    }
				    }
			    ...
			    for(; iter != (*delimClose)[openIndex].end(); ++iter) {
				    if(ignoreCase ? sc.MatchIgnoreCase2(iter->c_str()) : sc.Match(iter->c_str())) {
					    ...
					    nestedVector.push_back(*NI.Set(sc.currentPos + iter->length() - 1, nestedLevel--, openIndex,
						sc.state, NI_CLOSE));
					    // is there anything on the left side? (any backward keyword 'glued' with
					    // end of delimiter sequence)
					    setBackwards(kwLists,
						sc,
						prefixes,
						ignoreCase,
						delimNesting,
					    ...
					    // was current delimiter sequence nested, or do we start over from
					    // SCE_USER_STYLE_IDENTIFIER?
			    ...
			    if(isWhiteSpace(sc.ch) && !isWhiteSpace(sc.chPrev)) {
				    ...
				    // create new 'compare point' (AKA beginning of nested keyword) before checking for
				    // numbers
				    sc.SetState(prevState);
			    }
			...
			case SCE_USER_STYLE_COMMENT:
		    {
			    ...
			    for(; iter != commentClose[openIndex].end(); ++iter) {
				    if(ignoreCase ? sc.MatchIgnoreCase2(iter->c_str()) : sc.Match(iter->c_str())) {
					    ...
					    nestedVector.push_back(*NI.Set(sc.currentPos + iter->length() - 1, nestedLevel--, openIndex,
						SCE_USER_STYLE_COMMENT, NI_CLOSE));
					    // is there anything on the left side? (any backward keyword 'glued' with
					    ...
					    sc.Forward(iter->length());
					    // was current comment sequence nested, or do we start over from
					    // SCE_USER_STYLE_IDENTIFIER?
					    readLastNested(lastNestedGroup, newState, openIndex);
			...
			case SCE_USER_STYLE_COMMENTLINE:
		    {
			    ...
			    // first, check end of line comment sequence (in rare cases when line comments can end
			    // before new line char)
			    ...
			    for(; iter != commentLineClose[openIndex].end(); ++iter) {
				    if(ignoreCase ? sc.MatchIgnoreCase2(iter->c_str()) : sc.Match(iter->c_str())) {
					    ...
					    nestedVector.push_back(*NI.Set(sc.currentPos + iter->length() - 1, nestedLevel--, openIndex,
						SCE_USER_STYLE_COMMENTLINE, NI_CLOSE));
					    // is there anything on the left side? (any backward keyword 'glued' with
					    // end of line comment sequence)
					    setBackwards(kwLists,
						sc,
						prefixes,
						ignoreCase,
					    ...
					    sc.Forward(iter->length());
					    // was current line comment sequence nested, or do we start over from
					    // SCE_USER_STYLE_IDENTIFIER?
					    readLastNested(lastNestedGroup, newState, openIndex);
			    ...
			    if(sc.atLineEnd) {
				    ...
				    for(; iter != commentLineContinue[openIndex].end(); ++iter) {
					    ...
					    for(size_t i = 0; i < length; ++i) {
						    if(ignoreCase) {
							    if(toupper((*iter)[i]) !=
								toupper(styler.SafeGetCharAt(sc.currentPos - length + i - offset, 0))) {
					    ...
					    // if line comment continuation string is found at EOL, treat next line as a
					    // comment line
					    if(lineContinuation) {
						    isCommentLine = COMMENTLINE_YES;
						    break; // break out of 'for', not 'case'
				    ...
				    sc.ChangeState(SCE_USER_STYLE_COMMENTLINE); // no need to paint, only change state
					                                        // for now
				    if(!lineContinuation) {
					    ...
					    checkEOL = EOL_FORCE_CHECK;
					    // record end of line comment sequence (NI_CLOSE)
					    nestedVector.push_back(*NI.Set(sc.currentPos - 1, nestedLevel--, openIndex,
						SCE_USER_STYLE_COMMENTLINE, NI_CLOSE));
					    // was current line comment sequence nested, or do we start over from
					    // SCE_USER_STYLE_IDENTIFIER?
					    readLastNested(lastNestedGroup, newState, openIndex);
			...
			case SCE_USER_STYLE_DEFAULT:
		    {
			    ...
			    if(!commentLineOpen.empty()) {
					...
					(pureLC == PURE_LC_WSP && visibleChars == false)) {
					    if(isInListForward(commentLineOpen, sc, ignoreCase, openIndex, skipForward)) {
						    ...
						    nestedVector.push_back(*NI.Set(sc.currentPos, ++nestedLevel, openIndex,
							SCE_USER_STYLE_COMMENTLINE, NI_OPEN));
						    lastNestedGroup.push_back(NI);
			    ...
			    if(!commentOpen.empty()) {
				    if(isInListForward(commentOpen, sc, ignoreCase, openIndex, skipForward)) {
					    ...
					    nestedVector.push_back(*NI.Set(sc.currentPos, ++nestedLevel, openIndex, SCE_USER_STYLE_COMMENT,
						NI_OPEN));
					    lastNestedGroup.push_back(NI);
			    ...
			    for(int i = 0; i<SCE_USER_TOTAL_DELIMITERS; ++i) {
				    if(!delimStart[i]->empty()) {
					    if(isInListForward(*delimStart[i], sc, ignoreCase, openIndex, skipForward)) {
						    ...
						    // record start of delimiter sequence (NI_OPEN) in BOTH nesting
						    // vectors
						    nestedVector.push_back(*NI.Set(sc.currentPos, ++nestedLevel, openIndex,
							i+SCE_USER_STYLE_DELIMITER1, NI_OPEN));
						    lastNestedGroup.push_back(NI);
		...
		if((sc.atLineEnd == true && checkEOL != EOL_SKIP_CHECK) || (sc.atLineEnd == false && checkEOL == EOL_FORCE_CHECK)) {
			...
			for(int i = 0; i<nlCount; ++i) { // multi-line multi-part keyword
				// foldVector.push_back(levelNext | levelNext << 16);  // TODO: what about
				// SC_ISCOMMENTLINE?
...
static void FoldUserDoc(Sci_PositionU /* startPos */, Sci_Position /* length */, int /*initStyle*/, WordList *[],  Accessor & /*
                                                                                                                                 styler
                                                                                                                                 */       )
{
