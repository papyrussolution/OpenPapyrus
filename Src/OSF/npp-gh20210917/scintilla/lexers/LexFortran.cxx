...
static int classifyFoldPointFortran(const char * s, const char * prevWord, const char chNextNonBlank) {
	...
	if((strcmp(prevWord, "module") == 0 && strcmp(s, "subroutine") == 0)
	    ...
	    || (strcmp(prevWord, "module") == 0 && strcmp(s, "procedure") == 0)      // Take care of the "module
	                                                                             // procedure" statement
		...
		if(style == SCE_F_WORD) {
			if(iswordchar(ch) && !iswordchar(chNext)) {
				...
				if(strcmp(s, "forall") == 0 || (strcmp(s, "where") == 0 && strcmp(prevWord, "else") != 0)) {
					...
					// There are multiple forms of "do" loop. The older form with a label "do 100
					// i=1,10" would require matching
					// labels to ensure the folding level does not decrease too far when labels are
