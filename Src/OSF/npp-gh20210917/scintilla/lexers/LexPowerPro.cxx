...
static bool IsFunction(Accessor &styler, Sci_PositionU currentPos) {
	...
	for(Sci_PositionU j = 0; j < 16; j++) {                                         //check up to 16 preceeding
		                                                                        // characters
		char c = styler.SafeGetCharAt(position--, '\0');        //if can't read char, return NUL (past beginning
		                                                        // of document)
...
static void ColourisePowerProDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList * keywordlists[],
	...
	for(; sc.More(); sc.Forward()) {
		...
		switch(sc.state) {
			...
			case SCE_POWERPRO_IDENTIFIER:
			    ...
			    if((sc.ch > 0) && !setWord.Contains(sc.ch)) {      // || (sc.ch == '.')) { // use this line
				                                               // if you want to match keywords with a .
				                                               // ie: win.debug will only match
				                                               // win.debug neither win nor debug will
		...
		if(sc.state == SCE_POWERPRO_DEFAULT) {
			...
			else if(onlySpaces && sc.ch == ';') {                   //legacy comment that can only have
...
static void FoldPowerProDoc(Sci_PositionU startPos, Sci_Position length, int, WordList *[], Accessor &styler)
{
	...
	//used to tell if we're recursively folding the whole document, or just a small piece (ie: if statement or 1
	// function)
		...
		if((ch == '\r' && chNext != '\n')   // \r\n
			...
			if(firstWordLen > 0
				...
				if((strcmp(szFirstWord, "function") == 0) || (firstWordLen > 0 && szFirstWord[0] == '@')) {
					if(isFoldingAll) {  //if we're folding the whole document (recursivly by lua
						            // script)
						if(functionCount > 0) {
							levelCurrent--;
						}
					...
					else {   //if just folding a small piece (by clicking on the minus sign next to
						 // the word)
						levelCurrent--;
					}
				}
