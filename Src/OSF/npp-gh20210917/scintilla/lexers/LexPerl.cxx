...
void LexerPerl::InterpolateSegment(StyleContext &sc, int maxSeg, bool isPattern) {
	...
	while(maxSeg > 0) {
		...
		if((maxSeg > 1) && (sc.ch == '$' || sc.ch == '@')) {
			...
			while((maxSeg > sLen) && (sc.GetRelativeCharacter(sLen) == '$'))        // >0 $ dereference
				                                                                // within
				sLen++;
			...
			if(braces) {
				if((maxSeg > sLen) && (sc.GetRelativeCharacter(sLen) == '}')) {         // } end for
					                                                                // {word}
					sLen++;
				}
				else
			    ...
			    if(HereDoc.State == 0) { // '<<' encountered
				    ...
				    if(HereDoc.Quoted) { // a quoted here-doc delimiter
					    if(sc.ch == HereDoc.Quote) { // closing quote => end of delimiter
						    ...
						    if(sc.Match('\\', static_cast<char>(HereDoc.Quote))) { // escaped
							                                                   // quote
							    sc.Forward();
						    }
						    if(sc.ch != '\r') { // skip CR if CRLF
							    int i = 0;                  // else append char, possibly an
							                                // extended char
							    while(i < sc.width) {
								    HereDoc.Append(static_cast<unsigned char>(styler.SafeGetCharAt(sc.
									currentPos + i)));
								    i++;
							    }
						    }
					...
					switch(bkstyle) {
						...
						case SCE_PL_IDENTIFIER:
						    ...
						    if((bkstyle == 1) || (bkstyle == 2)) {
							    ...
							    if(sc.ch == '/') {
								    // if '/', /PATTERN/ unless digit/space immediately
								    // after '/'
								    // if '//', always expect defined-or operator to
								    // follow identifier
						...
						case SCE_PL_SCALAR:     // for $var<< case:
						    if(isHereDoc && hereDocSpace) // if SCALAR whitespace '<<', *always*
							                          // a HERE doc
							    preferRE = true;
						    break;
						case SCE_PL_WORD:
						    ...
						    if(sc.ch == '/') {
							    ...
							    // keywords always forced as /PATTERN/: split, if, elsif,
							    // while
							    // everything else /PATTERN/ unless digit/space immediately
							    // after '/'
