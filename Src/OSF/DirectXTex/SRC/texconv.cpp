...
HRESULT __cdecl LoadFromBMPEx(_In_z_ const wchar_t* szFile,
    _In_ WIC_FLAGS flags,
    _Out_opt_ TexMetadata* metadata,
    _Out_ ScratchImage& image) noexcept;

HRESULT __cdecl LoadFromPortablePixMap(_In_z_ const wchar_t* szFile,
    _Out_opt_ TexMetadata* metadata,
    _Out_ ScratchImage& image) noexcept;

HRESULT __cdecl SaveToPortablePixMap(_In_ const Image& image,
    _In_z_ const wchar_t* szFile) noexcept;

HRESULT __cdecl LoadFromPortablePixMapHDR(_In_z_ const wchar_t* szFile,
    _Out_opt_ TexMetadata* metadata,
    _Out_ ScratchImage& image) noexcept;

HRESULT __cdecl SaveToPortablePixMapHDR(_In_ const Image& image,
    _In_z_ const wchar_t* szFile) noexcept;
			...
			if(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if(findData.cFileName[0] != L'.') {
					...
					auto subfolder = (folder)
					    ? (std::wstring(folder) + std::wstring(findData.cFileName) +
	...
	*pFactory = nullptr;
	...
	if(!s_CreateDXGIFactory1) {
		...
		s_CreateDXGIFactory1 =
		    reinterpret_cast<pfn_CreateDXGIFactory1>(reinterpret_cast<void*>(GetProcAddress(hModDXGI,
		    "CreateDXGIFactory1")));
		if(!s_CreateDXGIFactory1)
			return false;
	}
	...
	*pDevice = nullptr;
	...
	if(!s_DynamicD3D11CreateDevice) {
		...
		s_DynamicD3D11CreateDevice =
		    reinterpret_cast<PFN_D3D11_CREATE_DEVICE>(reinterpret_cast<void*>(GetProcAddress(hModD3D11,
		    "D3D11CreateDevice")));
		if(!s_DynamicD3D11CreateDevice)
			return false;
	}
...
{
	const float ST2084 =
	    pow((0.8359375f + 18.8515625f *
		pow(abs(normalizedLinearValue), 0.1593017578f)) / (1.0f + 18.6875f * pow(abs(normalizedLinearValue), 0.1593017578f)),
		78.84375f);
	return ST2084;  // Don't clamp between [0..1], so we can still perform operations on scene values higher than
		        // 10,000 nits
}

inline float ST2084ToLinear(float ST2084)
{
	const float normalizedLinear =
	    pow(std::max(pow(abs(ST2084), 1.0f / 78.84375f) - 0.8359375f,
		0.0f) / (18.8515625f - 18.6875f * pow(abs(ST2084), 1.0f / 78.84375f)), 1.0f / 0.1593017578f);
...
int __cdecl wmain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[])
{
	...
	for(int iArg = 1; iArg < argc; iArg++) {
		...
		if(allowOpts
		    && ('-' == pArg[0]) && ('-' == pArg[1])) {
			if(pArg[2] == 0) {
				// "-- " is the POSIX standard for "end of options" marking to escape the '-' and '/'
				// characters at the start of filepaths.
				allowOpts = false;
			}
			...
			SearchForFiles(path.make_preferred().c_str(), conversion, (dwOptions & (uint64_t(1) << OPT_RECURSIVE)) != 0,
			    nullptr);
	...
	for(auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv) {
			...
			if(FAILED(hr)) {
				...
				if(hr == static_cast<HRESULT>(0xc00d5212) /* MF_E_TOPO_CODEC_NOT_FOUND */) {
					if(_wcsicmp(ext, L".heic") == 0 || _wcsicmp(ext, L".heif") == 0) {
						wprintf(
							L"INFO: This format requires installing the HEIF Image Extensions - https://aka.ms/heif\n");
					}
					else if(_wcsicmp(ext, L".webp") == 0) {
						wprintf(
							L"INFO: This format requires installing the WEBP Image Extensions - https://www.microsoft.com/p/webp-image-extensions/9pg2dk419drg\n");
					}
				}
				continue;
			}
		...
		if(IsCompressed(info.format)) {
			...
			if((info.width % 4) != 0 || (info.height % 4) != 0) {
				if(dwOptions & (uint64_t(1) << OPT_BCNONMULT4FIX)) {
					...
					// Fix by changing size but also have to trim any mip-levels which can be
					// invalid
			...
			if(IsCompressed(tformat)) {
				switch(tformat)
				{
					...
					case DXGI_FORMAT_BC5_SNORM:
					    nmfmt =
						(BitsPerColor(info.format) >
						8) ? DXGI_FORMAT_R16G16B16A16_SNORM : DXGI_FORMAT_R8G8B8A8_SNORM;
					    break;
					...
					default:
					    nmfmt =
						(BitsPerColor(info.format) >
						8) ? DXGI_FORMAT_R16G16B16A16_UNORM : DXGI_FORMAT_R8G8B8A8_UNORM;
					    break;
				}
			}
		...
		if((!tMips || info.mipLevels != tMips || preserveAlphaCoverage) && (info.mipLevels != 1)) {
			...
			if(cimage && (tMips == 1)) {
				// Special case for trimming mips off compressed images and keeping the original
				// compressed highest level mip
		...
		if(IsCompressed(tformat) && (FileType == CODEC_DDS)) {
			if(cimage && (cimage->GetMetadata().format == tformat)) {
				// We never changed the image and it was already compressed in our desired format, use
				// original data
		...
		// --- Save result -------------------------------------------------------------
		{
			...
			if(keepRecursiveDirs && *pConv->szFolder) {
				...
				if(!GetFullPathNameW(szDest, MAX_PATH, szPath, nullptr)) {
					wprintf(L" get full path FAILED (%08X%ls)\n",
					    ...
					    GetErrorDesc(HRESULT_FROM_WIN32(GetLastError())));
					retVal = 1;
					continue;
				}
			...
			switch(FileType)
			{
				...
				default:
			    {
				    const WICCodecs codec =
					(FileType == CODEC_HDP ||
					FileType == CODEC_JXR) ? WIC_CODEC_WMP : static_cast<WICCodecs>(FileType);
