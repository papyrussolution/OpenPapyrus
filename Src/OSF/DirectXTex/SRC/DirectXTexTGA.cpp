...
// Decodes TGA header
//
HRESULT DecodeTGAHeader(_In_reads_bytes_(size) const void* pSource, size_t size, TGA_FLAGS flags, _Out_ TexMetadata& metadata,
    size_t& offset, _Inout_opt_ uint32_t* convFlags) noexcept
{
	if(!pSource)
		return E_INVALIDARG;
    ...
    _In_ uint32_t convFlags) noexcept
{
	...
	switch(image->format) {
		...
		case DXGI_FORMAT_R8G8B8A8_UNORM:
	    {
		    ...
		    for(size_t y = 0; y < image->height; ++y) {
			    size_t offset = ((convFlags & CONV_FLAGS_INVERTX) ? (image->width - 1) : 0);
			    auto dPtr = reinterpret_cast<uint32_t*>(image->pixels + (image->rowPitch * ((convFlags & CONV_FLAGS_INVERTY) ? y : (image->height - y - 1)))) + offset;
			    for(size_t x = 0; x < image->width;) {
				    if(sPtr >= endPtr)
					    return E_FAIL;
				    if(*sPtr & 0x80) {
