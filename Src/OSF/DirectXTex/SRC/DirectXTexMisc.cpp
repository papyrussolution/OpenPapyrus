...
HRESULT EvaluateImage_(const Image& image, const std::function<void __cdecl(_In_reads_(width) const XMVECTOR* pixels, size_t width, size_t y)>& pixelFunc)
{
	if(!pixelFunc)
		return E_INVALIDARG;
	if(!image.pixels)
...
HRESULT TransformImage_(const Image& srcImage, const std::function<void __cdecl(_Out_writes_(width) XMVECTOR* outPixels,
    _In_reads_(width) const XMVECTOR* inPixels, size_t width, size_t y)>& pixelFunc, const Image& destImage)
{
	if(!pixelFunc)
		return E_INVALIDARG;
...
_Use_decl_annotations_ HRESULT DirectX::TransformImage(const Image& image,
    std::function<void __cdecl(_Out_writes_(width) XMVECTOR* outPixels, _In_reads_(width) const XMVECTOR* inPixels, size_t width, size_t y)> pixelFunc, ScratchImage& result)
{
