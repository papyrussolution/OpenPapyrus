...
void EvaluateRow(_In_reads_(width) const XMVECTOR* pSource, _Out_writes_(width + 2) float* pDest, size_t width, CNMAP_FLAGS flags) noexcept
{
	...
	for(size_t y = 0; y < height; ++y) {
		...
		for(size_t x = 0; x < width; ++x) {
			...
			if(convFlags & CONVF_UNORM) {
				...
				const XMVECTOR n1 = XMVectorMultiplyAdd((flags & CNMAP_INVERT_SIGN) ? g_XMNegativeOneHalf : g_XMOneHalf,
					normal,
					g_XMOneHalf);
...
//
_Use_decl_annotations_ HRESULT DirectX::ComputeNormalMap(const Image& srcImage, CNMAP_FLAGS flags, float amplitude,
    DXGI_FORMAT format, ScratchImage& normalMap) noexcept
{
...
_Use_decl_annotations_ HRESULT DirectX::ComputeNormalMap(const Image* srcImages, size_t nimages, const TexMetadata& metadata,
    CNMAP_FLAGS flags, float amplitude, DXGI_FORMAT format, ScratchImage& normalMaps) noexcept
{
	if(!srcImages || !nimages || !IsValid(format))
		return E_INVALIDARG;
	if(IsCompressed(format) || IsCompressed(metadata.format) || IsTypeless(format) || IsTypeless(metadata.format) || 
		IsPlanar(format) || IsPlanar(metadata.format) || IsPalettized(format) || IsPalettized(metadata.format))
		return HRESULT_E_NOT_SUPPORTED;
	static_assert(CNMAP_CHANNEL_RED == 0x1, "CNMAP_CHANNEL_ flag values don't match mask");
	switch(flags & 0xf) {
		case 0:
