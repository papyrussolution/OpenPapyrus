...
const LegacyDDS g_LegacyDDSMap[] = {
	...
	{ DXGI_FORMAT_B4G4R4A4_UNORM,     CONV_FLAGS_NOALPHA|CONV_FLAGS_4444,      DDSPF_X4R4G4B4 }, // D3DFMT_X4R4G4B4 (uses DXGI 1.2 format)
	{ DXGI_FORMAT_B4G4R4A4_UNORM,     CONV_FLAGS_EXPAND|CONV_FLAGS_44,        DDSPF_A4L4 }, // D3DFMT_A4L4 (uses DXGI 1.2 format)
    ...
    _Out_ TexMetadata& metadata, _Inout_ uint32_t& convFlags) noexcept
{
	...
	if(!(flags & DDS_FLAGS_ALLOW_LARGE_FILES)) {
		...
		if(metadata.arraySize > 2048u /* D3D12_REQ_TEXTURE1D_ARRAY_AXIS_DIMENSION,
    ...
    void* pDestination, size_t maxsize, size_t& required) noexcept
{
	...
	if(!(flags & DDS_FLAGS_FORCE_DX10_EXT)) {
		switch(metadata.format)
		{
			...
			case DXGI_FORMAT_BC2_UNORM:             memcpy(&ddpf, metadata.IsPMAlpha() ? (&DDSPF_DXT2) : (&DDSPF_DXT3),
				sizeof(DDS_PIXELFORMAT)); break;
			case DXGI_FORMAT_BC3_UNORM:             memcpy(&ddpf, metadata.IsPMAlpha() ? (&DDSPF_DXT4) : (&DDSPF_DXT5),
				sizeof(DDS_PIXELFORMAT)); break;
			...
			case DXGI_FORMAT_B8G8R8A8_UNORM:        memcpy(&ddpf, &DDSPF_A8R8G8B8, sizeof(DDS_PIXELFORMAT)); break;// DXGI
			                                                                                                 // 1.1
			case DXGI_FORMAT_B8G8R8X8_UNORM:        memcpy(&ddpf, &DDSPF_X8R8G8B8, sizeof(DDS_PIXELFORMAT)); break;// DXGI
			                                                                                                 // 1.1
			case DXGI_FORMAT_B4G4R4A4_UNORM:        memcpy(&ddpf, &DDSPF_A4R4G4B4, sizeof(DDS_PIXELFORMAT)); break;// DXGI
			                                                                                                 // 1.2
			case DXGI_FORMAT_YUY2:                  memcpy(&ddpf, &DDSPF_YUY2, sizeof(DDS_PIXELFORMAT)); break;// DXGI
			                                                                                    // 1.2
	...
	*static_cast<uint32_t*>(pDestination) = DDS_MAGIC;
	...
	if(ddpf.size == 0) {
		...
		if(flags & DDS_FLAGS_FORCE_DX10_EXT_MISC2) {
			// This was formerly 'reserved'. D3DX10 and D3DX11 will fail if this value is anything other
			// than 0
	...
	switch(inFormat)
	{
		case TEXP_LEGACY_R8G8B8:
		    ...
		    if(inSize >= 3 && outSize >= 4) {
				...
				((icount < (inSize - 2)) && (ocount < (outSize - 3)));
				icount += 3, ocount += 4) {
		    ...
		    switch(outFormat)
		    {
				...
				if(inSize >= 1 && outSize >= 4) {
					...
					for(size_t ocount = 0, icount = 0;
						...
						uint32_t t3 =
						    uint32_t(((t & 0x03) << 22) | ((t & 0x03) << 20) | ((t & 0x03) << 18) |
							((t & 0x03) << 16));
		...
		case TEXP_LEGACY_A8R3G3B2:
		    ...
		    if(inSize >= 2 && outSize >= 4) {
			    ...
			    for(size_t ocount = 0, icount = 0;
				((icount < (inSize - 1)) && (ocount < (outSize - 3)));
				icount += 2, ocount += 4) {
				    ...
				    uint32_t t3 =
					uint32_t(((t & 0x0003) << 22) | ((t & 0x0003) << 20) | ((t & 0x0003) << 18) |
					    ((t & 0x0003) << 16));
		    ...
		    switch(outFormat)
		    {
				...
				if(inSize >= 1 && outSize >= 4) {
					...
					for(size_t ocount = 0, icount = 0;
						...
						uint32_t ta = (tflags & TEXP_SCANLINE_SETALPHA) ? 0xff000000 : uint32_t(
							((t & 0xf0) << 24) | ((t & 0xf0) << 20));
		...
		case TEXP_LEGACY_B4G4R4A4:
		    ...
		    if(inSize >= 2 && outSize >= 4) {
				...
				icount += 2, ocount += 4) {
				    ...
				    uint32_t ta =
					uint32_t((tflags & TEXP_SCANLINE_SETALPHA) ? 0xff000000 : (((t & 0xf000) << 16) |
					    ((t & 0xf000) << 12)));
...
//
HRESULT CopyImage(_In_reads_bytes_(size) const void* pPixels, _In_ size_t size, _In_ const TexMetadata& metadata,
    _In_ CP_FLAGS cpFlags, _In_ uint32_t convFlags, _In_reads_opt_(256) const uint32_t * pal8, _In_ const ScratchImage& image) noexcept
{
	assert(pPixels);
					    ...
					    for(size_t h = 0; h < images[index].height; ++h) {
						    if(convFlags & CONV_FLAGS_EXPAND) {
							    if(convFlags & CONV_FLAGS_4444) {
									...
									pSrc, spitch,
									(convFlags &
									CONV_FLAGS_565) ? DXGI_FORMAT_B5G6R5_UNORM :
    ...
    TexMetadata& metadata) noexcept
{
	...
	// File is too big for 32-bit allocation, so reject read (4 GB should be plenty large enough for a valid DDS
	// file)
...
//
_Use_decl_annotations_ HRESULT DirectX::LoadFromDDSFile(const wchar_t* szFile, DDS_FLAGS flags, TexMetadata* metadata, ScratchImage& image) noexcept
{
	if(!szFile)
		return E_INVALIDARG;
	image.Release();
...
//
_Use_decl_annotations_ HRESULT DirectX::SaveToDDSMemory(const Image* images, size_t nimages, const TexMetadata& metadata, DDS_FLAGS flags, Blob& blob) noexcept
{
	if(!images || (nimages == 0))
		return E_INVALIDARG;
...
//
_Use_decl_annotations_ HRESULT DirectX::SaveToDDSFile(const Image* images, size_t nimages, const TexMetadata& metadata, DDS_FLAGS flags, const wchar_t* szFile) noexcept
{
	if(!szFile)
		return E_INVALIDARG;
	// Create DDS Header
...
#else
	ScopedHandle hFile(safe_handle(CreateFileW(szFile, GENERIC_WRITE | DELETE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr)));
#endif
	if(!hFile) {
		return HRESULT_FROM_WIN32(GetLastError());
	}
			    ...
			    for(size_t level = 0; level < metadata.mipLevels; ++level, ++index) {
				    ...
				    if((images[index].slicePitch == ddsSlicePitch) && (ddsSlicePitch <= UINT32_MAX)) {
					    ...
					    if(rowPitch < ddsRowPitch) {
						    // DDS uses 1-byte alignment, so if this is happening then the input
						    // pitch isn't actually a full line of data
						    return E_FAIL;
					    }
