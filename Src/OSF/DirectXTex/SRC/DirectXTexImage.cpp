...
//
_Use_decl_annotations_ HRESULT DirectX::Internal::DetermineImageArray(const TexMetadata& metadata, CP_FLAGS cpFlags, size_t& nImages, size_t& pixelSize) noexcept
{
		...
		case TEX_DIMENSION_TEXTURE3D:
	    {
		    ...
		    for(size_t level = 0; level < metadata.mipLevels; ++level) {
			    ...
			    for(size_t slice = 0; slice < d; ++slice) {
				    ...
				    // We use the same memory organization that Direct3D 11 needs for
