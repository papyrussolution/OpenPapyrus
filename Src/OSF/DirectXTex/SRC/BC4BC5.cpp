...
//------------------------------------------------------------------------------
void FindEndPointsBC4U(_In_reads_(BLOCK_SIZE) const float theTexelsU[],
...
void FindEndPointsBC4S(_In_reads_(BLOCK_SIZE) const float theTexelsU[],
...
_Use_decl_annotations_ void DirectX::D3DXEncodeBC5U(uint8_t * pBC, const XMVECTOR * pColor, uint32_t flags) noexcept
{
	...
	assert(pBC && pColor);
	static_assert(sizeof(BC4_UNORM) == 8, "BC4_UNORM should be 8 bytes");
	memzero(pBC, sizeof(BC4_UNORM) * 2);
