...
namespace {
const char g_Signature[] = "#?RADIANCE"; // This is the official header signature for the .HDR (RGBE) file format.
const char g_AltSignature[] = "#?RGBE"; // This is a common variant header signature that is otherwise exactly the same format.
...
// Decodes HDR header
//-------------------------------------------------------------------------------------
HRESULT DecodeHDRHeader(_In_reads_bytes_(size) const void* pSource,
    size_t size,
...
// FloatToRGBE
//-------------------------------------------------------------------------------------
inline void FloatToRGBE(_Out_writes_(width*4) uint8_t* pDestination, _In_reads_(width* fpp) const float* pSource, size_t width,
...
HRESULT DirectX::GetMetadataFromHDRFile(const wchar_t* szFile, TexMetadata& metadata) noexcept
{
	...
	// File is too big for 32-bit allocation, so reject read (4 GB should be plenty large enough for a valid HDR
	// file)
