...
//
HRESULT PerformFlipRotateUsingWIC(const Image& srcImage, TEX_FR_FLAGS flags, const WICPixelFormatGUID& pfGUID, const Image& destImage) noexcept
{
...
HRESULT PerformFlipRotateViaF32(const Image& srcImage,
    TEX_FR_FLAGS flags,
    const Image& destImage) noexcept
{
	if(!srcImage.pixels || !destImage.pixels)
    ...
    ScratchImage& result) noexcept
{
	...
	for(size_t index = 0; index < nimages; ++index) {
		...
		if(wicpf) {
			...
			// Case 2: Source format is not supported by WIC, so we have to convert, flip/rotate, and
			// convert back
