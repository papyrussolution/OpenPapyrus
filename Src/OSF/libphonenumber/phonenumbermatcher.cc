...
// Author: Tao Huang
//
// Implementation of a stateful class that finds and extracts telephone numbers from text.
//
#include <libphonenumber-internal.h>
#pragma hdrstop
#include "phonenumbermatcher.h"
...
public:
	...
	AlternateFormats() : format_data_(), calling_code_to_alternate_formats_map_() 
	{
		if(!LoadAlternateFormats(&format_data_)) {
			LOG(DFATAL) << "Could not parse compiled-in metadata.";
			return;
		}
		for(RepeatedPtrField<PhoneMetadata>::const_iterator it = format_data_.metadata().begin(); it != format_data_.metadata().end(); ++it) {
			calling_code_to_alternate_formats_map_.insert(std::make_pair(it->country_code(), &*it));
		}
	}
	const PhoneMetadata* GetAlternateFormatsForCountry(int country_calling_code)
	const {
		map<int, const PhoneMetadata*>::const_iterator it = calling_code_to_alternate_formats_map_.find(country_calling_code);
...
PhoneNumberMatcher::PhoneNumberMatcher(const PhoneNumberUtil& util, const string & text, const string & region_code,
	...
	last_match_(NULL), search_index_(0) 
{
}
...
/*static*/bool PhoneNumberMatcher::IsLatinLetter(char32 letter) 
{
	...
	const UBlockCode block = ublock_getCode(letter);
	return oneof6(block, UBLOCK_BASIC_LATIN, UBLOCK_LATIN_1_SUPPLEMENT, UBLOCK_LATIN_EXTENDED_A, UBLOCK_LATIN_EXTENDED_ADDITIONAL,
...
bool PhoneNumberMatcher::ParseAndVerify(const string & candidate, int offset, PhoneNumberMatch* match) 
{
	...
	if(!reg_exps_->matching_brackets_->FullMatch(candidate) || reg_exps_->pub_pages_->PartialMatch(candidate)) {
		return false;
	}
	...
	if(leniency_ >= VALID) {
		...
		if(offset > 0 && !reg_exps_->lead_class_pattern_->Consume(candidate_input.get())) {
			char32 previous_char;
			const char* previous_char_ptr = EncodingUtils::BackUpOneUTF8Character(text_.c_str(), text_.c_str() + offset);
			...
			// symbol.
			if(IsInvalidPunctuationSymbol(previous_char) || IsLatinLetter(previous_char)) {
				return false;
			}
		}
		...
		if(lastCharIndex < text_.length()) {
			char32 next_char;
			const char* next_char_ptr = EncodingUtils::AdvanceOneUTF8Character(text_.c_str() + lastCharIndex - 1);
...
bool PhoneNumberMatcher::VerifyAccordingToLeniency(Leniency leniency, const PhoneNumber& number, const string & candidate) const 
{
	switch(leniency) {
		...
		case PhoneNumberMatcher::STRICT_GROUPING: {
		    if(!phone_util_.IsValidNumber(number) || !ContainsOnlyValidXChars(number, candidate, phone_util_) || 
				ContainsMoreThanOneSlashInNationalNumber(number, candidate, phone_util_) || !IsNationalPrefixPresentIfRequired(number)) {
			    return false;
		    }
		...
		default:
		    LOG(ERROR) << "No implementation defined for verification for leniency " << static_cast<int>(leniency);
		    return false;
	}
}
...
bool PhoneNumberMatcher::ExtractMatch(const string & candidate, int offset, PhoneNumberMatch* match) 
{
	...
	if(reg_exps_->time_stamps_->PartialMatch(candidate)) {
		scoped_ptr<RegExpInput> following_text(reg_exps_->regexp_factory_->CreateInput(text_.substr(offset + candidate.size())));
...
bool PhoneNumberMatcher::HasNext() 
{
	if(state_ == NOT_READY) {
		...
		if(!Find(search_index_, &temp_match)) {
			...
			last_match_.reset(new PhoneNumberMatch(temp_match.start(), temp_match.raw_string(), temp_match.number()));
    ...
    ResultCallback4<bool, const PhoneNumberUtil&, const PhoneNumber&, const string &, const std::vector <string>&>* checker) const 
{
	...
	// try them instead.
	const PhoneMetadata* alternate_formats = alternate_formats_->GetAlternateFormatsForCountry(phone_number.country_code());
	if(alternate_formats) {
		string national_significant_number;
		phone_util_.GetNationalSignificantNumber(phone_number, &national_significant_number);
		for(RepeatedPtrField<NumberFormat>::const_iterator it = alternate_formats->number_format().begin(); it != alternate_formats->number_format().end(); ++it) {
			if(it->leading_digits_pattern_size() > 0) {
				std::unique_ptr<RegExpInput> nsn_input(reg_exps_->regexp_factory_->CreateInput(national_significant_number));
				// There is only one leading digits pattern for alternate formats.
				if(!reg_exps_->regexp_cache_.GetRegExp(it->leading_digits_pattern(0)).Consume(nsn_input.get())) {
					// Leading digits don't match; try another one.
					continue;
				}
			}
			formatted_number_groups.clear();
			GetNationalNumberGroupsForPattern(phone_number, &*it, &formatted_number_groups);
			if(checker->Run(phone_util_, phone_number, normalized_candidate, formatted_number_groups)) {
				return true;
			}
		}
	}
...
void PhoneNumberMatcher::GetNationalNumberGroups(const PhoneNumber& number, std::vector <string>* digit_blocks) const 
{
	...
	SplitStringUsing(rfc3966_format.substr(start_index, end_index - start_index), "-", digit_blocks);
}

void PhoneNumberMatcher::GetNationalNumberGroupsForPattern(const PhoneNumber& number, const NumberFormat* formatting_pattern, std::vector <string>* digit_blocks) const 
{
	string rfc3966_format;
	// We format the NSN only, and split that according to the separator.
	string national_significant_number;
	phone_util_.GetNationalSignificantNumber(number, &national_significant_number);
	phone_util_.FormatNsnUsingPattern(national_significant_number, *formatting_pattern, PhoneNumberUtil::RFC3966, &rfc3966_format);
	SplitStringUsing(rfc3966_format, "-", digit_blocks);
}

bool PhoneNumberMatcher::IsNationalPrefixPresentIfRequired(const PhoneNumber& number) const 
{
	...
	phone_util_.GetRegionCodeForCountryCode(number.country_code(), &phone_number_region);
	const PhoneMetadata* metadata = phone_util_.GetMetadataForRegion(phone_number_region);
	if(!metadata) {
		return true;
	}
	...
	const NumberFormat* format_rule = phone_util_.ChooseFormattingPatternForNumber(metadata->number_format(), national_number);
	...
	if(format_rule && !format_rule->national_prefix_formatting_rule().empty()) {
		...
		if(phone_util_.FormattingRuleHasFirstGroupOnly(format_rule->national_prefix_formatting_rule())) {
		...
		return phone_util_.MaybeStripNationalPrefixAndCarrierCode(*metadata, &raw_input_copy, NULL); // Don't need to keep the stripped carrier code.
	}
	return true;
}
    ...
    const string & normalized_candidate, const std::vector <string>& formatted_number_groups) const 
{
	...
	string digit_block;
	while(reg_exps_->capturing_ascii_digits_pattern_->FindAndConsume(candidate_number.get(), &digit_block)) {
		candidate_groups.push_back(digit_block);
	}
	// Set this to the last group, skipping it if the number has an extension.
	int candidate_number_group_index = static_cast<int>(phone_number.has_extension() ? candidate_groups.size() - 2 : candidate_groups.size() - 1);
	...
	if(candidate_groups.size() == 1 || candidate_groups.at(candidate_number_group_index).find(national_significant_number) != string::npos) {
		return true;
	}
...
/*static*/bool PhoneNumberMatcher::ContainsMoreThanOneSlashInNationalNumber(const PhoneNumber& number, const string & candidate, const PhoneNumberUtil& util) 
{
	...
	if(oneof2(number.country_code_source(), PhoneNumber::FROM_NUMBER_WITH_PLUS_SIGN, PhoneNumber::FROM_NUMBER_WITHOUT_PLUS_SIGN)) {
