...
namespace i18n {
	namespace phonenumbers {
		template <class R, class A1, class A2, class A3, class A4> class ResultCallback4;
		...
		private:
			...
			const PhoneNumberMatcherRegExps* reg_exps_; // Helper class holding useful regular expressions.
			const AlternateFormats* alternate_formats_; // Helper class holding loaded data containing alternate ways phone numbers might be formatted for certain regions.
			const PhoneNumberUtil& phone_util_; // The phone number utility;
			const string text_; // The text searched for phone numbers;
			const string preferred_region_; // The region(country) to assume for phone numbers without an international prefix.
			Leniency leniency_; // The degree of validation requested.
			int max_tries_; // The maximum number of retries after matching an invalid number.
			State state_; // The iteration tristate.
			scoped_ptr<PhoneNumberMatch> last_match_; // The last successful match, NULL unless in State.READY.
			int search_index_; // The next index to start searching at. Undefined in State.DONE.
