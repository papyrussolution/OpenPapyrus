...
namespace i18n {
	namespace phonenumbers {
		...
		public:
			...
			public:
				const_reverse_iterator(const_iterator it) : std::reverse_iterator<const_iterator>(it) 
				{
				}
		...
		inline bool UnicodeTextRangeIsEmpty(const UnicodeTextRange& r) { return r.first == r.second; }
		...
		//
		inline UnicodeText MakeUnicodeTextAcceptingOwnership(char* utf8_buffer, int byte_length, int byte_capacity) 
		{
		...
		//
		inline UnicodeText MakeUnicodeTextWithoutAcceptingOwnership(const char* utf8_buffer, int byte_length) 
		{
