...
public:
	...
	enum PhoneNumberType {
		FIXED_LINE,
		MOBILE,
		FIXED_LINE_OR_MOBILE, // In some regions (e.g. the USA), it is impossible to distinguish between fixed-line and mobile numbers by looking at the phone number itself.
		TOLL_FREE, // Freephone lines
		PREMIUM_RATE,
		...
		// company.
		UAN,
		VOICEMAIL, // Used for "Voice Mail Access Numbers".
		UNKNOWN // A phone number is of type UNKNOWN when it does not fit any of the known patterns for a specific region.
	};
	...
	void GetSupportedGlobalNetworkCallingCodes(std::set<int>* calling_codes) const;
	...
	void GetSupportedTypesForRegion(const string & region_code, std::set<PhoneNumberType>* types) const;
	...
	void GetSupportedTypesForNonGeoEntity(int country_calling_code, std::set<PhoneNumberType>* types) const;
	...
	void GetNationalSignificantNumber(const PhoneNumber& number, string* national_significant_num) const;
	...
	//   national_destination_code = national_significant_number.substr(0, national_destination_code_length);
	//   subscriber_number = national_significant_number.substr(national_destination_code_length, string::npos);
	// } else {
	...
	// abroad.
	void GetCountryMobileToken(int country_calling_code, string* mobile_token) const;
	...
	void Format(const PhoneNumber& number, PhoneNumberFormat number_format, string* formatted_number) const;

	// Formats a phone number in the specified format using client-defined
	// formatting rules.
	void FormatByPattern(const PhoneNumber& number, PhoneNumberFormat number_format, const RepeatedPtrField <NumberFormat> & user_defined_formats, string * formatted_number) const;
	...
	void FormatNationalNumberWithCarrierCode(const PhoneNumber& number, const string & carrier_code, string* formatted_number) const;
	...
	// when formatting.
	void FormatNationalNumberWithPreferredCarrierCode(const PhoneNumber& number, const string & fallback_carrier_code, string* formatted_number) const;
	...
	void FormatNumberForMobileDialing(const PhoneNumber& number, const string & region_calling_from, bool with_formatting, string* formatted_number) const;
	...
	void FormatOutOfCountryCallingNumber(const PhoneNumber& number, const string & calling_from, string* formatted_number) const;
	...
	void FormatInOriginalFormat(const PhoneNumber& number, const string & region_calling_from, string* formatted_number) const;
	...
	void FormatOutOfCountryKeepingAlphaChars(const PhoneNumber& number, const string & calling_from, string* formatted_number) const;
	...
	bool IsValidNumberForRegion(const PhoneNumber& number, const string & region_code) const;
	...
	void GetRegionCodeForNumber(const PhoneNumber& number, string* region_code) const;
	...
	// is left unchanged.
	void GetRegionCodesForCountryCallingCode(int country_calling_code, std::list<string>* region_codes) const;
	...
	void GetNddPrefixForRegion(const string & region_code, bool strip_non_digits, string* national_prefix) const;
	...
	//      length of area codes for fixed line numbers), it will return false for
	//      the subscriber-number-only version.
	ValidationResult IsPossibleNumberForTypeWithReason(const PhoneNumber& number, PhoneNumberType type) const;
	...
	// is needed for a call to connect, the number is not considered possible
	// without it.
	bool IsPossibleNumberForType(const PhoneNumber& number, PhoneNumberType type) const;
	...
	// it again.
	bool IsNumberGeographical(PhoneNumberType phone_number_type, int country_calling_code) const;
	...
	// passed in.
	bool GetInvalidExampleNumber(const string & region_code, PhoneNumber* number) const;
	...
	bool GetExampleNumberForType(const string & region_code, PhoneNumberType type, PhoneNumber* number) const;
	...
	bool GetExampleNumberForNonGeoEntity(int country_calling_code, PhoneNumber* number) const;
	...
	// +).
	ErrorType Parse(const string & number_to_parse, const string & default_region, PhoneNumber* number) const;
	...
	// field.
	ErrorType ParseAndKeepRawInput(const string & number_to_parse, const string & default_region, PhoneNumber* number) const;
	...
	MatchType IsNumberMatch(const PhoneNumber& first_number, const PhoneNumber& second_number) const;
	...
	// number.
	MatchType IsNumberMatchWithTwoStrings(const string & first_number, const string & second_number) const;
	...
	// number.
	MatchType IsNumberMatchWithOneString(const PhoneNumber& first_number, const string & second_number) const;
	...
	friend bool ConvertFromTelephoneNumberProto(const TelephoneNumber& proto_to_convert, PhoneNumber* new_proto);
	friend bool ConvertToTelephoneNumberProto(const PhoneNumber& proto_to_convert, TelephoneNumber* resulting_proto);
protected:
	bool IsNumberMatchingDesc(const string & national_number, const PhoneNumberDesc& number_desc) const;
