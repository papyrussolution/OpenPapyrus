...
namespace i18n {
	namespace phonenumbers {
		namespace gtl {
			...
			struct OrderByFirst {
				template <typename T> bool operator()(const T& p1, const T& p2) const { return p1.first < p2.first; }
			};

			// Deletes the second attribute (pointer type expected) of the pairs contained
			// in the provided range.
			template <typename ForwardIterator> void STLDeleteContainerPairSecondPointers(const ForwardIterator& begin, const ForwardIterator& end) 
			{
			...
			template <typename T> void STLDeleteElements(T* container) 
			{
