...
void AsYouTypeFormatter::SetShouldAddSpaceAfterNationalPrefix(const NumberFormat& format) {
...
void AsYouTypeFormatter::GetFormattingTemplate(const string & number_pattern, const string & number_format, UnicodeString* formatting_template) 
{
	...
	MatchAllGroups(number_pattern, longest_phone_number, *regexp_factory_, &regexp_cache_, &a_phone_number);
...
void AsYouTypeFormatter::InputDigitWithOptionToRememberPosition(char32 next_char,
...
void AsYouTypeFormatter::AttemptToChoosePatternWithPrefixExtracted(string* formatted_number) {
	able_to_format_ = true;
	is_expecting_country_code_ = false;
	possible_formats_.clear();
	last_match_position_ = 0;
...
void AsYouTypeFormatter::AttemptToFormatAccruedDigits(string* formatted_result) {
...
void AsYouTypeFormatter::AttemptToChooseFormattingPattern(string* formatted_number) {
...
void AsYouTypeFormatter::RemoveNationalPrefixFromNationalNumber(string* national_prefix) {
...
char AsYouTypeFormatter::NormalizeAndAccrueDigitsAndPlusSign(char32 next_char,
    bool remember_position) {
