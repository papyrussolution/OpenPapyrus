...
{
	if(conn->transport == connectdata::TRNSPRT_TCP) {
    ...
    curl_socket_t * sockfd)
{
	...
	addr->socktype = (conn->transport == connectdata::TRNSPRT_TCP) ? SOCK_STREAM : SOCK_DGRAM;
	addr->protocol = conn->transport != connectdata::TRNSPRT_TCP ? IPPROTO_UDP : ai->ai_protocol;
	...
	if(data->set.fopensocket) {
		...
		*sockfd = socket(addr->family, addr->socktype, addr->protocol); /* opensocket callback not set, so simply create the socket now */
	if(*sockfd == CURL_SOCKET_BAD)
		return CURLE_COULDNT_CONNECT; /* no socket, no connection */
	if(conn->transport == connectdata::TRNSPRT_QUIC) {
		(void)curlx_nonblock(*sockfd, TRUE); /* QUIC sockets need to be nonblocking */
	}
#if defined(ENABLE_IPV6) && defined(HAVE_SOCKADDR_IN6_SIN6_SCOPE_ID)
	if(conn->scope_id && (addr->family == AF_INET6)) {
		struct sockaddr_in6 * const sa6 = (struct sockaddr_in6 * const)&addr->sa_addr;
