 ...
 */
char * FASTCALL Curl_checkheaders(const struct connectdata * conn, const char * thisheader)
{
	...
	if(data->set.postfields)
		...
		if(data->set.seek_func) {
			...
			if(data->state.fread_func == (curl_read_callback)fread) {
				if(-1 != fseek((FILE *)data->state.in, 0, SEEK_SET))
					/* successful rewind */
					return CURLE_OK;
			}
		...
		if(!k->header && (nread > 0 || is_empty_data)) {
			...
			if(data->set.verbose) {
				if(k->badheader) {
					...
					if(k->badheader == SingleRequest::HEADER_PARTHEADER)
						Curl_debug(data, CURLINFO_DATA_IN,
						    k->str, (size_t)nread);
				}
				else
			...
			if((k->badheader == SingleRequest::HEADER_PARTHEADER) && !k->ignorebody) {
			...
			if(!k->chunk && (nread || k->badheader || is_empty_data)) {
				...
				if(k->badheader < SingleRequest::HEADER_ALLBAD) {
				...
				k->badheader = SingleRequest::HEADER_NORMAL; /* taken care of now */
 ...
 * authentication etc.
 */
CURLcode FASTCALL Curl_pretransfer(struct Curl_easy * data)
{
	CURLcode result;
	...
	if(data->state.httpreq == HTTPREQ_PUT)
		...
		if(data->set.postfields && (data->state.infilesize == -1))
			data->state.infilesize = (curl_off_t)strlen((const char *)data->set.postfields);
	}
	else
 ...
 */
CURLcode FASTCALL Curl_posttransfer(struct Curl_easy * data)
{
#if defined(HAVE_SIGNAL) && defined(SIGPIPE) && !defined(HAVE_MSG_NOSIGNAL)
 ...
 * upcoming transfer.
 */
void Curl_setup_transfer(struct Curl_easy * data,   /* transfer */
    ...
    )
{
	...
	struct connectdata * conn = data->conn;
	struct HTTP * http = (struct HTTP *)data->req.protop;
	bool httpsending = ((conn->handler->protocol&PROTO_FAMILY_HTTP) && (http->sending == HTTP::HTTPSEND_REQUEST));
