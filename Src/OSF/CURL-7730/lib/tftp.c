    ...
    tftp_event_t event)
{
	...
	switch(event) {
		...
		case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
		    ...
		    senddata = sendto(state->sockfd, (char *)state->spacket.data,
    ...
    tftp_event_t event)
{
	...
	switch(event) {
		case TFTP_EVENT_DATA:
		    ...
		    sbytes = sendto(state->sockfd, (char *)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr *)&state->remote_addr, state->remote_addrlen);
		    if(sbytes < 0) {
			    failf(data, "%s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
		...
		case TFTP_EVENT_ERROR:
		    ...
		    setpacketblock(&state->spacket, state->block);
		    (void)sendto(state->sockfd, (char *)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr *)&state->remote_addr, state->remote_addrlen);
		    ...
		    if(event == TFTP_EVENT_ACK) {
				...
				!(state->block == 0 && rblock == 65535)) {
				    ...
				    if(state->retries>state->retry_max) {
					    ...
					    sbytes = sendto(state->sockfd, (char *)state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr *)&state->remote_addr,
...
static CURLcode tftp_setup_connection(struct connectdata * conn)
{
	struct Curl_easy * data = conn->data;
	char * type;
	conn->transport = connectdata::TRNSPRT_UDP;
