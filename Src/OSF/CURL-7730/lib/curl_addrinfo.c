...
struct Curl_addrinfo * Curl_he2ai(const struct hostent * he, int port)                        {
	...
	for(i = 0; (curr = he->h_addr_list[i]) != NULL; i++) {
		...
		if(!firstai)
			firstai = ai; /* store the pointer we want to return from this function */
		if(prevai)
			prevai->ai_next = ai; /* make the previous entry point to this */
		ai->ai_family = he->h_addrtype;
		/* we return all names as STREAM, so when using this address for TFTP the type must be ignored and conn->socktype be used instead! */
...
#ifdef ENABLE_IPV6
			case AF_INET6:
			    addr6 = (sockaddr_in6 *)ai->ai_addr; /* storage area for this info */
