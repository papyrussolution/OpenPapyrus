	...
	switch(zp->zlib_init) {
		...
		case ZLIB_INIT:
	    {
		    ...
		    switch(check_gzip_header((uchar *)buf, nbytes, &hlen)) {
				...
				memcpy((void *)z->next_in, buf, z->avail_in); // @sobolev @badcast
		...
		case ZLIB_GZIP_HEADER:
	    {
		    ...
		    memcpy((void *)(z->next_in + z->avail_in - nbytes), buf, nbytes); // @sobolev @badcast
...
static CURLcode zstd_init_writer(struct connectdata * conn, struct contenc_writer * writer)
{
...
static CURLcode zstd_unencode_write(struct connectdata * conn, struct contenc_writer * writer, const char * buf, size_t nbytes)
{
	CURLcode result = CURLE_OK;
	struct zstd_params * zp = (struct zstd_params *)&writer->params;
	ZSTD_inBuffer in;
...
static void zstd_close_writer(struct connectdata * conn, struct contenc_writer * writer)
{
	struct zstd_params * zp = (struct zstd_params *)&writer->params;
	(void)conn;
	if(zp->decomp) {
...
static struct contenc_writer * new_unencoding_writer(struct connectdata * conn,
