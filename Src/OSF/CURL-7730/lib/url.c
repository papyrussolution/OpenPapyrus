    ...
    bool * waitpipe)
{
	...
	bool canmultiplex = LOGIC(IsMultiplexingPossible(data, needle));
...
#elif defined(USE_WIN32_IDN)
	SAlloc::F(host->encalloc); /* must be freed with SAlloc::F() since this was
...
static struct connectdata * allocate_conn(struct Curl_easy * data)
{
	   ...
	   data becomes proxy backend data). */
	{
		...
		char * ssl = (char *)SAlloc::C(4, sslsize);
		if(!ssl) {
			SAlloc::F(conn);
			return NULL;
		}
		conn->ssl_extra = ssl;
		conn->ssl[0].backend = (ssl_backend_data *)ssl;
		conn->ssl[1].backend = (ssl_backend_data *)(ssl + sslsize);
#ifndef CURL_DISABLE_PROXY
		conn->proxy_ssl[0].backend = (ssl_backend_data *)(ssl + 2 * sslsize);
		conn->proxy_ssl[1].backend = (ssl_backend_data *)(ssl + 3 * sslsize);
#endif
	}
#endif
	...
	conn->transport = connectdata::TRNSPRT_TCP; /* most of them are TCP streams */
 ...
 */
	    ...
	    struct connectdata * conn)
	{
		...
		uc = curl_url_get(uh, CURLUPART_OPTIONS, &data->state.up.options,
			CURLU_URLDECODE);
		if(!uc) {
			conn->options = sstrdup(data->state.up.options);
			if(!conn->options)
				return CURLE_OUT_OF_MEMORY;
		}
		...
		conn->host.rawalloc = sstrdup(hostname ? hostname : "");
	...
	static CURLcode create_conn_helper_init_proxy(struct connectdata * conn)
	{
		...
		*************************************************************/
		...
		if(data->set.str[STRING_PRE_PROXY]) {
			socksproxy = sstrdup(data->set.str[STRING_PRE_PROXY]);
 ...
 */
	    ...
	    int * port_result)
	{
		...
		*port_result = -1;
		...
		if(hostptr) {
			*hostname_result = sstrdup(hostptr);
			if(!*hostname_result) {
				SAlloc::F(host_dup);
				return CURLE_OUT_OF_MEMORY;
			}
 ...
 */
	    ...
	    struct curl_slist * conn_to_host)
	{
		...
		if(data->asi && !host && (port == -1) &&
			...
			if(hit) {
				char * hostd = sstrdup((char *)as->dst.host);
				if(!hostd)
					return CURLE_OUT_OF_MEMORY;
		...
		if(conn->bits.reuse)
			...
			*async = FALSE;
			...
			if(conn->unix_domain_socket) {
				...
				hostaddr = (struct Curl_dns_entry *)SAlloc::C(1, sizeof(struct Curl_dns_entry));
				if(!hostaddr)
					result = CURLE_OUT_OF_MEMORY;
				else {
					bool longpath = FALSE;
			...
			if(!conn->bits.proxy) {
				...
				conn->hostname_resolve = sstrdup(connhost->name);
				...
				/* resolve proxy */
				conn->hostname_resolve = sstrdup(host->name);
				if(!conn->hostname_resolve)
					return CURLE_OUT_OF_MEMORY;
				rc = Curl_resolv_timeout(conn, conn->hostname_resolve, (int)conn->port,
					&hostaddr, timeout_ms);
	...
	static CURLcode create_conn(struct Curl_easy * data, struct connectdata ** in_connect, bool * async)
	{
		...
		*in_connect = conn;
		...
		if(data->set.str[STRING_SASL_AUTHZID]) {
			conn->sasl_authzid = sstrdup(data->set.str[STRING_SASL_AUTHZID]);
		...
		if(data->set.str[STRING_UNIX_SOCKET_PATH]) {
			conn->unix_domain_socket = sstrdup(data->set.str[STRING_UNIX_SOCKET_PATH]);
