	...
	if(data->set.httpversion == CURL_HTTP_VERSION_3) {
		if(conn->handler->flags & PROTOPT_SSL)
			/* Only go HTTP/3 directly on HTTPS URLs. It needs a UDP socket and does
			   the QUIC dance. */
			conn->transport = connectdata::TRNSPRT_QUIC;
		else {
 ...
 */
static bool pickoneauth(struct auth * pick, ulong mask)
{
	bool picked;
	/* only deal with authentication we want */
	ulong avail = pick->avail & pick->want & mask;
	picked = TRUE;
...
#define TE_HEADER "TE: gzip\r\n"
	...
	if(conn->bits.httpproxy && !conn->bits.tunnel_proxy) {
		...
		if(strcasecompare("ftp", data->state.up.scheme)) {
			if(data->set.proxy_transfer_mode) {
				...
				char * type = (char *)strstr(path, ";type=");
	...
	if(result)
		return result;
	if(!postsize && (http->sending != HTTP::HTTPSEND_REQUEST))
...
static bool checkprefixmax(const char * prefix, const char * buffer, size_t len)
{
	...
	return LOGIC(curl_strnequal(prefix, buffer, ch)); // @sobolev LOGIC
}
/*
 * checkhttpprefix()
 *
