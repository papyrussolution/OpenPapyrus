	...
	switch(what) {
		...
		case CURLUPART_URL: {
		    ...
		    if(u->scheme && strcasecompare("file", u->scheme)) {
			    ...
			    url = aprintf("%s://%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s", scheme, u->user ? u->user : "", u->password ? ":" : "",
				    u->password ? u->password : "", options ? ";" : "", options ? options : "", (u->user || u->password || options) ? "@" : "",
    ...
    const char * part, unsigned int flags)
{
	...
	DEBUGASSERT(storep);
	{
		...
		if(urlencode) {
			...
			for(i = (const uchar *)part, o = enc; *i; i++) {
