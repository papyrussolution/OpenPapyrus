...
static size_t mime_subparts_read(char * buffer, size_t size, size_t nitems, void * instream, bool * hasread)
{
	...
	while(nitems) {
		size_t sz = 0;
		curl_mimepart * part = static_cast<curl_mimepart *>(mime->state.ptr);
...
curl_off_t Curl_mime_size(curl_mimepart * part)
{
	...
	if(part->kind == MIMEKIND_MULTIPART)
		part->datasize = multipart_size(static_cast<curl_mime *>(part->arg));
	size = part->datasize;
	if(part->encoder)
...
const char * Curl_mime_contenttype(const char * filename)
{
	...
	if(filename) {
		...
		for(uint i = 0; i < SIZEOFARRAY(ctts); i++) {
			const size_t len2 = strlen(ctts[i].extension);
