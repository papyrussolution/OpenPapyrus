...
struct Curl_addrinfo * Curl_doh(struct connectdata * conn, const char * hostname, int port, int * waitp)
{
	...
	*waitp = TRUE; /* this never returns synchronously */
	...
	memzero(&data->req.doh, sizeof(struct dohdata));
	conn->bits.doh = TRUE;
...
static ushort get16bit(const uchar * doh, int index)
{
	return (ushort)((doh[index] << 8) | doh[index + 1]);
}

static unsigned int get32bit(const uchar * doh, int index)
{
...
UNITTEST void de_init(struct dohentry * de)
{
	...
	de->ttl = INT_MAX;
	for(int i = 0; i < DOH_MAX_CNAME; i++)
		Curl_dyn_init(&de->cname[i], DYN_DOH_CNAME);
}

UNITTEST DOHcode doh_decode(const uchar * doh, size_t dohlen, DNStype dnstype, struct dohentry * d)
{
	uchar rcode;
	ushort qdcount;
	ushort ancount;
	...
	*dnsp = NULL; /* defaults to no response */
	...
	if(!data->req.doh.probe[DOH_PROBE_SLOT_IPADDR_V4].easy && !data->req.doh.probe[DOH_PROBE_SLOT_IPADDR_V6].easy) {
		...
		for(slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
			...
			if(!p->dnstype)
				continue;
			rc[slot] = doh_decode(Curl_dyn_uptr(&p->serverdoh), Curl_dyn_len(&p->serverdoh), static_cast<DNStype>(p->dnstype), &de);
			Curl_dyn_free(&p->serverdoh);
			if(rc[slot]) {
				infof(data, "DOH: %s type %s for %s\n", doh_strerror(rc[slot]), type2name(static_cast<DNStype>(p->dnstype)), data->req.doh.host);
			}
		} /* next slot */
