...
static CURLcode mqtt_send(struct connectdata * conn, char * buf, size_t len)
{
	...
	struct Curl_easy * data = conn->data;
	struct MQTT * mq = (struct MQTT *)data->req.protop;
	ssize_t n;
	...
	if(len != (size_t)n) {
		size_t nsend = len - n;
		char * sendleftovers = (char *)Curl_memdup(&buf[n], nsend);
		if(!sendleftovers)
			return CURLE_OUT_OF_MEMORY;
...
static CURLcode mqtt_connect(struct connectdata * conn)
{
	...
	result = Curl_rand_hex(conn->data, (uchar *)&client_id[clen],
...
{
	CURLcode result;
	char * payload = (char *)conn->data->set.postfields;
