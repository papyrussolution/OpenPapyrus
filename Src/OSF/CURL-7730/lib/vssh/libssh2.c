...
/* Local functions: */
static const char * sftp_libssh2_strerror(ulong err);
...
static CURLcode sftp_libssh2_error_to_CURLE(ulong err)
{
	switch(err) {
...
static CURLcode libssh2_session_error_to_CURLE(int err)
{
	switch(err) {
		...
		/* This is the error returned by libssh2_scp_recv2 on unknown file */
		case LIBSSH2_ERROR_SCP_PROTOCOL: return CURLE_REMOTE_FILE_NOT_FOUND;
 ...
 */
	...
	#define SFTP_SEEK(x, y) libssh2_sftp_seek64(x, (uint64)y) // @sobolev libssh2_uint64_t-->uint64
#else
...
#else
	#define SCP_SEND(a, b, c, d) libssh2_scp_send64(a, b, (int)(c), (uint64)d, 0, 0) // @sobolev libssh2_uint64_t-->uint64
#endif
...
#endif
				infof(data, "SSH host check: %d, key: %s\n", keycheck, (keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) ? host->key : "<none>");
				/* setup 'knownkey' */
				if(keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) {
					knownkey.key = host->key;
					knownkey.len = 0;
					knownkey.keytype = (keytype == LIBSSH2_HOSTKEY_TYPE_RSA) ? CURLKHTYPE_RSA : CURLKHTYPE_DSS;
					knownkeyp = &knownkey;
				}
				/* setup 'foundkey' */
				foundkey.key = remotekey;
				foundkey.len = keylen;
				foundkey.keytype = (keytype == LIBSSH2_HOSTKEY_TYPE_RSA) ? CURLKHTYPE_RSA : CURLKHTYPE_DSS;
				/*
		...
		switch(rc) {
			...
			case CURLKHSTAT_FINE_ADD_TO_FILE:
			    ...
			    if(keycheck != LIBSSH2_KNOWNHOST_CHECK_MATCH) {
				    ...
				    int addrc = libssh2_knownhost_add(sshc->kh, conn->host.name, NULL,
					    remotekey, keylen, LIBSSH2_KNOWNHOST_TYPE_PLAIN|LIBSSH2_KNOWNHOST_KEYENC_RAW|keybit, NULL);
				    if(addrc)
		...
		switch(sshc->state) {
			    ...
			    /* Send any quote commands */
		    {
			    ...
			    }
			    {
				    ...
				    if(strncasecompare(cmd, "chgrp ", 6) || strncasecompare(cmd, "chmod ", 6) ||
						strncasecompare(cmd, "chown ", 6) || strncasecompare(cmd, "atime ", 6) || strncasecompare(cmd, "mtime ", 6)) {
					    /* attribute change */
					    ...
					    memzero(&sshc->quote_attrs, sizeof(LIBSSH2_SFTP_ATTRIBUTES));
			...
			case SSH_SFTP_QUOTE_STAT:
		    {
			    ...
			    if(strncasecompare(cmd, "chgrp", 5)) {
				    ...
				    sshc->quote_attrs.atime = (ulong)date;
				    ...
				    sshc->quote_attrs.mtime = (ulong)date;
				    sshc->quote_attrs.flags = LIBSSH2_SFTP_ATTR_ACMODTIME;
			    }
			...
			case SSH_SFTP_QUOTE_SETSTAT:
			    rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2,
				    curlx_uztoui(strlen(sshc->quote_path2)), LIBSSH2_SFTP_SETSTAT, &sshc->quote_attrs);
			    if(rc == LIBSSH2_ERROR_EAGAIN) {
				    break;
			    }
			...
			case SSH_SFTP_QUOTE_RENAME:
			    rc = libssh2_sftp_rename_ex(sshc->sftp_session, sshc->quote_path1,
				    curlx_uztoui(strlen(sshc->quote_path1)), sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
			...
			case SSH_SFTP_UPLOAD_INIT:
		    {
			    ...
			    if(!sshc->sftp_handle) {
				    ...
				    failf(data, "Upload failed: %s (%d/%d)", sftperr != LIBSSH2_FX_OK ? sftp_libssh2_strerror(sftperr) : "ssh error", sftperr, rc);
				    break;
			    }
			    ...
			    if(data->state.resume_from > 0) {
				    ...
				    if(seekerr != CURL_SEEKFUNC_OK) {
					    ...
					    do {
						    size_t readthisamountnow = (data->state.resume_from - passed > data->set.buffer_size) ?
								(size_t)data->set.buffer_size : curlx_sotouz(data->state.resume_from - passed);
						    size_t actuallyread;
						    Curl_set_in_callback(data, true);
			...
			case SSH_SCP_UPLOAD_INIT:
			     ...
			     */
			    sshc->ssh_channel =
				SCP_SEND(sshc->ssh_session, sftp_scp->path, data->set.new_file_perms, data->state.infilesize);
			    if(!sshc->ssh_channel) {
				    int ssh_err;
				    char * err_msg = NULL;
			...
			case SSH_SESSION_FREE:
			    ...
			    ZFREE(sshc->readdir_filename);
			    ZFREE(sshc->readdir_longentry);
			    ZFREE(sshc->readdir_line);
			    Curl_dyn_free(&sshc->readdir);

			    /* the code we are about to return */
			    result = sshc->actualcode;
			    memzero(sshc, sizeof(struct ssh_conn));
...
static ssize_t ssh_tls_recv(libssh2_socket_t sock, void * buffer, size_t length, int flags, void ** abstract)
{
	...
	CURLcode result;
	(void)flags;
	result = Curl_read(conn, sock, (char *)buffer, length, &nread);
...
{
	switch(err) {
		case LIBSSH2_FX_NO_SUCH_FILE: return "No such file or directory";
		case LIBSSH2_FX_PERMISSION_DENIED: return "Permission denied";
		case LIBSSH2_FX_FAILURE: return "Operation failed";
		case LIBSSH2_FX_BAD_MESSAGE: return "Bad message from SFTP server";
		case LIBSSH2_FX_NO_CONNECTION: return "Not connected to SFTP server";
		case LIBSSH2_FX_CONNECTION_LOST: return "Connection to SFTP server lost";
		case LIBSSH2_FX_OP_UNSUPPORTED: return "Operation not supported by SFTP server";
		case LIBSSH2_FX_INVALID_HANDLE: return "Invalid handle";
		case LIBSSH2_FX_NO_SUCH_PATH: return "No such file or directory";
		case LIBSSH2_FX_FILE_ALREADY_EXISTS: return "File already exists";
		case LIBSSH2_FX_WRITE_PROTECT: return "File is write protected";
		case LIBSSH2_FX_NO_MEDIA: return "No media";
		case LIBSSH2_FX_NO_SPACE_ON_FILESYSTEM: return "Disk full";
		case LIBSSH2_FX_QUOTA_EXCEEDED: return "User quota exceeded";
		case LIBSSH2_FX_UNKNOWN_PRINCIPLE: return "Unknown principle";
		case LIBSSH2_FX_LOCK_CONFlICT: return "File lock conflict";
		case LIBSSH2_FX_DIR_NOT_EMPTY: return "Directory not empty";
		case LIBSSH2_FX_NOT_A_DIRECTORY: return "Not a directory";
		case LIBSSH2_FX_INVALID_FILENAME: return "Invalid filename";
		case LIBSSH2_FX_LINK_LOOP: return "Link points to itself";
	}
