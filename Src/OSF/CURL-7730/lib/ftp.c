...
static CURLcode AllowServerConnect(struct connectdata * conn, bool * connected)
{
	...
	*connected = FALSE;
	...
	if(*connected) {
		...
		if(*connected == FALSE) {
			Curl_expire(data, data->set.accepttimeout > 0 ? data->set.accepttimeout : DEFAULT_ACCEPT_TIMEOUT, static_cast<expire_id>(0));
		}
	}
...
static CURLcode ftp_state_use_port(struct connectdata * conn, /*ftpport*/int fcmd/*start with this*/) // @sobolev ftpport-->int
{
	CURLcode result = CURLE_OK;
	struct ftp_conn * ftpc = &conn->proto.ftpc;
	struct Curl_easy * data = conn->data;
	...
	if(data->set.str[STRING_FTPPORT] && (strlen(data->set.str[STRING_FTPPORT]) > 1)) {
...
#endif
				strncpy(addr, string_ftpport, ip_end - ip_start); /* (ipv4|domain|interface):port(-range) */
			}
			else
				strcpy(addr, string_ftpport); /* ipv4|interface */
		}
		...
		if(*addr != '\0') {
			...
			host = NULL; /* there was only a port(-range) given, default the host */
	} /* data->set.ftpport */
	if(!host) {
		const char * r;
	...
	if(h) {
		res = h->addr;
		// when we return from this function, we can forget about this entry to we can unlock it now already 
	...
	//
	// get a plain printable version of the numerical address to work with below 
	Curl_printable_address(ai, myhost, sizeof(myhost));
#ifdef ENABLE_IPV6
	if(!conn->bits.ftp_use_eprt && conn->bits.ipv6)
		// EPRT is disabled but we are connected to a IPv6 host, so we ignore the request and enable EPRT again! 
		conn->bits.ftp_use_eprt = TRUE;
#endif
	for(; fcmd != DONE; fcmd++) {
		if(!conn->bits.ftp_use_eprt && (EPRT == fcmd))
			continue; /* if disabled, goto next */
		if((PORT == fcmd) && sa->sa_family != AF_INET)
			continue; /* PORT is IPv4 only */
		switch(sa->sa_family) {
			case AF_INET:
	...
	ftpc->count1 = fcmd; /* store which command was sent */
	close_secondarysocket(conn);
...
{
	CURLcode result = CURLE_OK;
	struct FTP * ftp = (struct FTP *)conn->data->req.protop;
	struct Curl_easy * data = conn->data;
...
static CURLcode ftp_state_list(struct connectdata * conn)
{
	CURLcode result = CURLE_OK;
	struct Curl_easy * data = conn->data;
	struct FTP * ftp = (struct FTP *)data->req.protop;
		...
		if(ptr) {
			...
			if(5 == sscanf(ptr, "%c%c%c%u%c",
				...
				if(ptr) {
					...
					ftpc->newhost = sstrdup(control_address(conn));
					if(!ftpc->newhost)
						return CURLE_OUT_OF_MEMORY;
				}
			}
...
static CURLcode ftp_state_port_resp(struct connectdata * conn, int ftpcode)
{
	struct Curl_easy * data = conn->data;
	struct ftp_conn * ftpc = &conn->proto.ftpc;
	// @sobolev ftpport fcmd = (ftpport)ftpc->count1;
	int    fcmd = ftpc->count1; // @sobolev
	CURLcode result = CURLE_OK;
	...
	if(ftpcode / 100 != 2) {
		...
		if(fcmd == DONE) {
			...
			result = ftp_state_use_port(conn, static_cast<ftpport>(fcmd)); /* try next */
	}
	else {
		...
		case 213:
	    {
		    ...
		    if(data->set.opt_no_body && ftpc->file && data->set.get_filetime && (data->info.filetime >= 0)) {
			    ...
			    msnprintf(headerbuf, sizeof(headerbuf), "Last-Modified: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n",
					Curl_wkday[tm->tm_wday ? tm->tm_wday-1 : 6], tm->tm_mday, Curl_month[tm->tm_mon],
    ...
    bool premature)
{
	...
	if(result) {
		...
		if((data->set.ftp_filemethod == FTPFILE_NOCWD) && (rawPath[0] == '/'))
			SAlloc::F(rawPath); /* full path => no CWDs happened => keep ftpc->prevpath */
		else {
			SAlloc::F(ftpc->prevpath);
...
static void wc_data_dtor(void * ptr)
{
	struct ftp_wc * ftpwc = (struct ftp_wc *)ptr;
...
static CURLcode init_wc_data(struct connectdata * conn)
{
	...
	if(last_slash) {
		...
		wildcard->pattern = sstrdup(last_slash);
		if(!wildcard->pattern)
			return CURLE_OUT_OF_MEMORY;
	...
	ftpwc = (struct ftp_wc *)SAlloc::C(1, sizeof(struct ftp_wc));
	if(!ftpwc) {
	...
	wildcard->path = sstrdup(ftp->path);
	if(!wildcard->path) {
		result = CURLE_OUT_OF_MEMORY;
		goto fail;
	}
	...
	ftpwc->backup.file_descriptor = (FILE *)conn->data->set.out;
...
static CURLcode wc_statemach(struct connectdata * conn)
{
	...
	switch(wildcard->state) {
		...
		case CURLWC_MATCHING: {
		    ...
		    struct ftp_wc * ftpwc = (struct ftp_wc *)wildcard->protdata;
		...
		case CURLWC_DOWNLOADING: {
		    ...
		    struct ftp_conn * ftpc = &conn->proto.ftpc;
		    struct curl_fileinfo * finfo = (struct curl_fileinfo *)wildcard->filelist.head->ptr;
		    struct FTP * ftp = (struct FTP *)conn->data->req.protop;
	...
	switch(data->set.ftp_filemethod) {
		...
		case FTPFILE_SINGLECWD:
		    ...
		    if(slashPos) {
			    ...
			    ftpc->dirs[0] = (char *)SAlloc::C(1, dirlen + 1);
			    if(!ftpc->dirs[0]) {
				    SAlloc::F(rawPath);
				    return CURLE_OUT_OF_MEMORY;
			    }
		...
		case FTPFILE_MULTICWD: {
		    ...
		    if(dirAlloc > 0) {
			    ftpc->dirs = (char **)SAlloc::C(dirAlloc, sizeof(ftpc->dirs[0]));
			    if(!ftpc->dirs) {
				    SAlloc::F(rawPath);
				    return CURLE_OUT_OF_MEMORY;
			    }
			    ...
			    while((slashPos = strchr(curPos, '/')) != NULL) {
				    ...
				    if(compLen > 0) {
					    char * comp = (char *)SAlloc::C(1, compLen + 1);
					    if(!comp) {
						    SAlloc::F(rawPath);
						    return CURLE_OUT_OF_MEMORY;
					    }
...
static CURLcode ftp_setup_connection(struct connectdata * conn)
{
	...
	char * type;
	struct FTP * ftp;
	conn->data->req.protop = ftp = (struct FTP *)SAlloc::C(sizeof(struct FTP), 1);
	if(NULL == ftp)
		return CURLE_OUT_OF_MEMORY;
