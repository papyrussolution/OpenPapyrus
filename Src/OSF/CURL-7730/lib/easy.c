 ...
 */
CURLcode curl_global_init_mem(long flags, curl_malloc_callback m, curl_free_callback f, curl_realloc_callback r,
    curl_strdup_callback s, curl_calloc_callback c)
{
	...
	/* set memory functions before global_init() in case it wants memory functions */
	Curl_cmalloc = m;
	Curl_cfree = f;
	Curl_cstrdup = s;
	Curl_crealloc = r;
...
{
	CURLcode result = CURLE_OK;
	// @sobolev enum dupstring i;
	int    i; // @sobolev
	// Copy src->set into dst->set first, then deal with the strings afterwards 
	dst->set = src->set;
	...
	memzero(dst->set.blobs, BLOB_LAST * sizeof(struct curl_blob *)); // clear all blob pointers first 
	{
		//
		// duplicate all blobs 
		//
	...
	if(src->set.postfieldsize && src->set.str[i]) {
		...
		dst->set.str[i] = (char *)Curl_memdup(src->set.str[i], curlx_sotouz(src->set.postfieldsize));
		if(!dst->set.str[i])
			return CURLE_OUT_OF_MEMORY;
...
CURLcode curl_easy_pause(struct Curl_easy * data, int action)
{
	...
	if((k->keepon & ~newstate & KEEP_SEND_PAUSE) && (data->mstate == CURLM_STATE_PERFORM ||
	    data->mstate == CURLM_STATE_TOOFAST) && data->state.fread_func == (curl_read_callback)Curl_mime_read) {
		Curl_mime_unpause(static_cast<curl_mimepart *>(data->state.in));
	}
...
CURLcode curl_easy_recv(struct Curl_easy * data, void * buffer, size_t buflen, size_t * n)
{
	...
	if(result)
		return result;
	*n = 0;
	result = Curl_read(c, sfd, (char *)buffer, buflen, &n1);
	if(result)
		return result;
	*n = (size_t)n1;
	return CURLE_OK;
...
CURLcode curl_easy_upkeep(struct Curl_easy * data)
{
	...
	if(!GOOD_EASY_HANDLE(data))
		return CURLE_BAD_FUNCTION_ARGUMENT;
	if(data->multi_easy) {
		return upkeep(&data->multi_easy->conn_cache, data); /* Use the common function to keep connections alive. */
	}
	else {
		return CURLE_OK; /* No connections, so just return success */
	}
}
