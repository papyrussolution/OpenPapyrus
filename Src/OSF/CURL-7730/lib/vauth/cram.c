...
CURLcode Curl_auth_create_cram_md5_message(struct Curl_easy * data, const char * chlg, const char * userp, const char * passwdp, char ** outptr, size_t * outlen)
{
	...
	struct HMAC_context * ctxt = Curl_HMAC_init(Curl_HMAC_MD5, (const uchar *)passwdp, curlx_uztoui(strlen(passwdp)));
	if(!ctxt)
	...
	response = aprintf("%s %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
