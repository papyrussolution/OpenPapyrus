...
static CURLcode connect_init(struct connectdata * conn, bool reinit)
{
	...
	s->tunnel_state = http_connect_state::TUNNEL_INIT;
	s->keepon = TRUE;
	s->cl = 0;
...
static void connect_done(struct connectdata * conn)
{
	...
	s->tunnel_state = http_connect_state::TUNNEL_COMPLETE;
...
#define SELECT_ERROR   1
	...
	do {
		timediff_t check;
		if(http_connect_state::TUNNEL_INIT == s->tunnel_state) {
			/* BEGIN CONNECT PHASE */
			char * host_port;
			struct dynbuf req;
			...
			s->tunnel_state = http_connect_state::TUNNEL_CONNECT;
		} /* END CONNECT PHASE */
				...
				if(('\r' == linep[0]) ||
					...
					if((407 == k->httpcode) && !data->state.authproblem) {
						...
						if(s->cl) {
							...
							if(linep[1] == '\n')
								/* this can only be a LF if the letter at index 0 was a
								   CR */
								linep++;
		...
		if(data->req.newurl && (http_connect_state::TUNNEL_COMPLETE == s->tunnel_state)) {
