...
{
	DES_cblock key;
	extend_key_56_to_64(key_56, (char *)&key); /* Expand the 56-bit key to 64-bits */
	DES_set_odd_parity(&key); /* Set the key parity to odd */
	DES_set_key(&key, ks); /* Set the key */
}
...
{
	char key[8];
	extend_key_56_to_64(key_56, key); /* Expand the 56-bit key to 64-bits */
	Curl_des_set_odd_parity((uchar *)key, sizeof(key)); /* Set the key parity to odd */
	des_set_key(des, (const uint8_t*)key); /* Set the key */
}
 ...
 */
static bool encrypt_des(const uchar * in, uchar * out, const uchar * key_56)
{
	...
	if(SECSuccess == PK11_CipherOp(ctx, out, &out_len, /* outbuflen */ 8, (uchar *)in, /* inbuflen */ 8) && SECSuccess == PK11_Finalize(ctx))
		rv = /* all OK */ TRUE;
fail:
	/* cleanup */
	if(ctx)
...
static void ascii_uppercase_to_unicode_le(uchar * dest, const char * src, size_t srclen)
{
	size_t i;
	for(i = 0; i < srclen; i++) {
		dest[2 * i] = (uchar)(Curl_raw_toupper(src[i]));
		dest[2 * i + 1] = '\0';
	}
}
...
CURLcode Curl_ntlm_core_mk_nt_hash(struct Curl_easy * data, const char * password, uchar * ntbuffer /* 21 bytes */)
{
	...
	pw = len ? (uchar *)SAlloc::M(len * 2) : (uchar *)sstrdup("");
	if(!pw)
		return CURLE_OUT_OF_MEMORY;
 ...
 */
CURLcode Curl_ntlm_core_mk_ntlmv2_hash(const char * user, size_t userlen, const char * domain, size_t domlen,
    uchar * ntlmhash, uchar * ntlmv2hash)
{
	...
	identity = (uchar *)SAlloc::M(identity_len);
	if(!identity)
		return CURLE_OUT_OF_MEMORY;
 ...
 */
CURLcode Curl_ntlm_core_mk_ntlmv2_resp(uchar * ntlmv2hash, uchar * challenge_client, struct ntlmdata * ntlm,
    uchar ** ntresp, unsigned int * ntresp_len)
{
 ...
 */
	...
	unsigned int len = 0;
	uchar * ptr = NULL;
	uchar hmac_output[HMAC_MD5_LENGTH];
	curl_off_t tw;
