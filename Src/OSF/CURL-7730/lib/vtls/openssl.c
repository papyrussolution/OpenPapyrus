...
static int SSL_CTX_use_certificate_bio(SSL_CTX * ctx, BIO * in, int type,
...
static int SSL_CTX_use_PrivateKey_bio(SSL_CTX * ctx, BIO* in, int type,
    const char * key_passwd)
{
	int ret = 0;
	EVP_PKEY * pkey = NULL;
			...
			case SSL_FILETYPE_ENGINE:
			    ...
			    {   /* XXXX still needs some work */
				    ...
				    if(data->state.engine) {
					    ...
					    priv_key = (EVP_PKEY*)
						ENGINE_load_private_key((ENGINE *)data->state.engine, key_file,
						    ui_method,
						    key_passwd);
...
static CURLcode Curl_ossl_set_engine_default(struct Curl_easy * data)
{
	...
	if(data->state.engine) {
		if(ENGINE_set_default((ENGINE *)data->state.engine, ENGINE_METHOD_ALL) > 0) {
			infof(data, "set default crypto engine '%s'\n", ENGINE_get_id((const ENGINE *)data->state.engine));
		}
		else {
			failf(data, "set default crypto engine '%s' failed", ENGINE_get_id((const ENGINE *)data->state.engine));
...
static struct curl_slist * Curl_ossl_engines_list(struct Curl_easy * data)
{
	...
	for(ENGINE * e = ENGINE_get_first(); e; e = ENGINE_get_next(e)) {
		struct curl_slist * beg = curl_slist_append(list, ENGINE_get_id(e));
		if(!beg) {
...
static int select_next_proto_cb(SSL * ssl, uchar ** out, uchar * outlen, const uchar * in, unsigned int inlen, void * arg)
{
...
#if(OPENSSL_VERSION_NUMBER < 0x10100000L) /* 1.1.0 */
static CURLcode set_ssl_version_min_max_legacy(ctx_option_t * ctx_options,
...
static CURLcode Curl_ossl_md5sum(uchar * tmp, /* input */
    size_t tmplen,
    uchar * md5sum /* output */,
    size_t unused)
{
...
static CURLcode Curl_ossl_sha256sum(const uchar * tmp, /* input */
    size_t tmplen,
