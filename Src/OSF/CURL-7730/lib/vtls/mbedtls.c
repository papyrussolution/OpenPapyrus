...
static CURLcode mbed_connect_step2(struct connectdata * conn,
    int sockindex)
{
	int ret;
	struct Curl_easy * data = conn->data;
...
static CURLcode mbed_connect_step3(struct connectdata * conn,
    int sockindex)
{
    ...
    CURLcode * curlcode)
{
	...
	ret = mbedtls_ssl_read(&backend->ssl, (uchar *)buf, buffersize);
	if(ret <= 0) {
...
static CURLcode mbed_connect_common(struct connectdata * conn,
    int sockindex,
    bool nonblocking,
    bool * done)
{
...
static CURLcode Curl_mbedtls_sha256sum(const uchar * input,
    size_t inputlen,
    uchar * sha256sum,
    size_t sha256len CXX_UNUSED_PARAM)
{
	(void)sha256len;
