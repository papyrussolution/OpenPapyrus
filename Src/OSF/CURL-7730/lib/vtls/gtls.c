...
static CURLcode gtls_connect_step1(struct connectdata * conn,
    int sockindex)
{
	struct Curl_easy * data = conn->data;
	struct ssl_connect_data * connssl = &conn->ssl[sockindex];
	...
	*certverifyresult = 0;
	...
	if(SSL_SET_OPTION(primary.clientcert)) {
		if(SSL_SET_OPTION(key_passwd)) {
			...
			rc = gnutls_certificate_set_x509_key_file2(backend->cred, SSL_SET_OPTION(primary.clientcert),
				SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(primary.clientcert),
				do_file_type(SSL_SET_OPTION(cert_type)), SSL_SET_OPTION(key_passwd), supported_key_encryption_algorithms);
			if(rc != GNUTLS_E_SUCCESS) {
			...
			if(gnutls_certificate_set_x509_key_file(backend->cred, SSL_SET_OPTION(primary.clientcert),
				    SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(primary.clientcert),
...
static CURLcode gtls_connect_step3(struct connectdata * conn,
    int sockindex)
{
 ...
 */
static CURLcode gtls_connect_common(struct connectdata * conn,
    int sockindex,
    bool nonblocking,
    bool * done)
{
...
static CURLcode Curl_gtls_md5sum(uchar * tmp, /* input */
    size_t tmplen,
    uchar * md5sum,                             /* output */
    size_t md5len)
{
	...
	md5_init(&MD5pw);
	md5_update(&MD5pw, (uint)tmplen, tmp);
	md5_digest(&MD5pw, (uint)md5len, md5sum);
#elif defined(USE_GNUTLS)
	gcry_md_hd_t MD5pw;
...
static CURLcode Curl_gtls_sha256sum(const uchar * tmp, /* input */
    size_t tmplen,
...
static void * Curl_gtls_get_internals(struct ssl_connect_data * connssl,
    CURLINFO info CXX_UNUSED_PARAM)
{
	struct ssl_backend_data * backend = connssl->backend;
	(void)info;
