...
static CURLcode ssl_connect_init_proxy(struct connectdata * conn, int sockindex)
{
	...
	if(ssl_connection_complete == conn->ssl[sockindex].state && !conn->proxy_ssl[sockindex].use) {
		...
		memzero(pbdata, Curl_ssl->sizeof_ssl_backend_data);
...
CURLcode Curl_ssl_connect_nonblocking(struct connectdata * conn, int sockindex,
    bool * done)
{
	CURLcode result;
#ifndef CURL_DISABLE_PROXY
    ...
    uchar ** der, size_t * der_len)
{
	...
	begin_pos = (char *)strstr(pem, "-----BEGIN PUBLIC KEY-----");
	if(!begin_pos)
		return CURLE_BAD_CONTENT_ENCODING;
...
{
	return LOGIC(Curl_ssl->supports & SSLSUPP_TLS13_CIPHERSUITES); // @sobolev LOGIC
}
...
int Curl_none_shutdown(struct connectdata * conn CXX_UNUSED_PARAM,
    int sockindex CXX_UNUSED_PARAM)
{
	(void)conn;
	(void)sockindex;
	return 0;
...
int Curl_none_check_cxn(struct connectdata * conn CXX_UNUSED_PARAM)
{
	(void)conn;
	return -1;
}

CURLcode Curl_none_random(struct Curl_easy * data CXX_UNUSED_PARAM, uchar * entropy CXX_UNUSED_PARAM, size_t length CXX_UNUSED_PARAM)
{
	(void)data;
	(void)entropy;
	(void)length;
...
void Curl_none_close_all(struct Curl_easy * data CXX_UNUSED_PARAM)
{
	(void)data;
}

void Curl_none_session_free(void * ptr CXX_UNUSED_PARAM)
{
	(void)ptr;
}

bool Curl_none_data_pending(const struct connectdata * conn CXX_UNUSED_PARAM, int connindex CXX_UNUSED_PARAM)
{
	(void)conn;
	(void)connindex;
	return 0;
...
CURLcode Curl_none_set_engine(struct Curl_easy * data CXX_UNUSED_PARAM, const char * engine CXX_UNUSED_PARAM)
{
	(void)data;
	(void)engine;
	return CURLE_NOT_BUILT_IN;
...
CURLcode Curl_none_set_engine_default(struct Curl_easy * data CXX_UNUSED_PARAM)
{
	(void)data;
	return CURLE_NOT_BUILT_IN;
}

struct curl_slist * Curl_none_engines_list(struct Curl_easy * data CXX_UNUSED_PARAM)
{
	(void)data;
...
#ifndef CURL_DISABLE_CRYPTO_AUTH
CURLcode Curl_none_md5sum(uchar * input, size_t inputlen, uchar * md5sum, size_t md5len CXX_UNUSED_PARAM)
{
...
#else
CURLcode Curl_none_md5sum(uchar * input CXX_UNUSED_PARAM, size_t inputlen CXX_UNUSED_PARAM, uchar * md5sum CXX_UNUSED_PARAM, size_t md5len CXX_UNUSED_PARAM)
{
	(void)input;
	(void)inputlen;
	(void)md5sum;
...
static size_t Curl_multissl_version(char * buffer, size_t size)
{
	...
	const struct Curl_ssl * current = (Curl_ssl == &Curl_ssl_multi ? available_backends[0] : Curl_ssl);
	if(current != selected) {
		char * p = backends;
