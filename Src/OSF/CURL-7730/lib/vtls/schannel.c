...
static CURLcode Curl_schannel_random(struct Curl_easy * data CXX_UNUSED_PARAM,
...
static void Curl_schannel_checksum(const uchar * input, size_t inputlen, uchar * checksum,
    size_t checksumlen, DWORD provType, const unsigned int algId)
{
	HCRYPTPROV hProv = 0;
	HCRYPTHASH hHash = 0;
	DWORD cbHashSize = 0;
...
static CURLcode Curl_schannel_md5sum(uchar * input,
    size_t inputlen,
    uchar * md5sum,
    size_t md5len)
{
...
static CURLcode Curl_schannel_sha256sum(const uchar * input,
    size_t inputlen,
    uchar * sha256sum,
    size_t sha256len)
{
