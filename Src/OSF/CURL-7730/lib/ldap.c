...
static int ldap_win_bind_auth(LDAP * server, const char * user, const char * passwd, ulong authflags)
{
	ULONG method = 0;
	SEC_WINNT_AUTH_IDENTITY cred;
	int rc = LDAP_AUTH_METHOD_NOT_SUPPORTED;
	memzero(&cred, sizeof(cred));
...
#if defined(USE_WIN32_LDAP)
		ludp->lud_dn = curlx_convert_UTF8_to_tchar(unescaped); /* Convert the unescaped string to a tchar */
		curlx_unicodefree(unescaped); /* Free the unescaped string as we are done with it */
		if(!ludp->lud_dn) {
			rc = LDAP_NO_MEMORY;
			goto quit;
		}
...
#if defined(USE_WIN32_LDAP)
		ludp->lud_filter = curlx_convert_UTF8_to_tchar(unescaped); /* Convert the unescaped string to a tchar */
		curlx_unicodefree(unescaped); /* Free the unescaped string as we are done with it */
		if(!ludp->lud_filter) {
			rc = LDAP_NO_MEMORY;
			goto quit;
