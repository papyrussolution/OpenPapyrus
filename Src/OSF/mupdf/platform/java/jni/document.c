...
{
	SeekableStreamState * state = (SeekableStreamState *)stm->state;
	...
	n = env->CallIntMethod(state->stream, mid_SeekableInputStream_read, state->array);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	if(n > 0) {
		env->GetByteArrayRegion(state->array, 0, n, state->buffer);
		if(env->ExceptionCheck())
			fz_throw_java_and_detach_thread(ctx, env, detach);
...
static void SeekableInputStream_seek(fz_context * ctx, fz_stream * stm, int64_t offset, int whence)
{
	...
	pos = env->CallLongMethod(state->stream, mid_SeekableStream_seek, offset, whence);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	stm->pos = pos;
...
{
	SeekableStreamState * state = (SeekableStreamState *)streamState_;
	jboolean detach = JNI_FALSE;
	JNIEnv * env;
	env = jni_attach_thread(ctx, &detach);
	if(env == NULL) {
	...
	env->DeleteGlobalRef(state->stream);
	env->DeleteGlobalRef(state->array);
...
{
	SeekableStreamState * state = (SeekableStreamState *)streamState_;
	const jbyte * buffer = (const jbyte *)buffer_;
	jboolean detach = JNI_FALSE;
	JNIEnv * env;
	env = jni_attach_thread(ctx, &detach);
	if(env == NULL)
	...
	while(count > 0) {
		...
		env->SetByteArrayRegion(state->array, 0, n, buffer);
		if(env->ExceptionCheck())
			fz_throw_java_and_detach_thread(ctx, env, detach);
		buffer += n;
		count -= n;
		env->CallVoidMethod(state->stream, mid_SeekableOutputStream_write, state->array, 0, n);
		if(env->ExceptionCheck())
			fz_throw_java_and_detach_thread(ctx, env, detach);
	}
	jni_detach_thread(detach);
...
static int64_t SeekableOutputStream_tell(fz_context * ctx, void * streamState_)
{
	...
	pos = env->CallLongMethod(state->stream, mid_SeekableStream_position);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	jni_detach_thread(detach);
	return pos;
...
static void SeekableOutputStream_seek(fz_context * ctx, void * streamState_, int64_t offset, int whence)
{
	...
	(void)env->CallLongMethod(state->stream, mid_SeekableStream_seek, offset, whence);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	jni_detach_thread(detach);
}
...
JNIEXPORT void JNICALL FUN(Document_finalize)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document_safe(env, self);
	if(!ctx || !doc) return;
	env->SetLongField(self, fid_Document_pointer, 0);
...
JNIEXPORT jobject JNICALL FUN(Document_openNativeWithStream)(JNIEnv *env, jclass cls, jstring jmagic, jobject jdocument,
    jobject jaccelerator)
{
	fz_context * ctx = get_context(env);
	...
	if(!ctx) return NULL;
	if(jmagic) {
		magic = env->GetStringUTFChars(jmagic, NULL);
		if(!magic) jni_throw_run(env, "cannot get characters in magic string");
	}
	if(jdocument) {
		jdoc = env->NewGlobalRef(jdocument);
		if(!jdoc) {
			if(magic) env->ReleaseStringUTFChars(jmagic, magic);
			jni_throw_run(env, "cannot get reference to document stream");
		}
	}
	if(jaccelerator) {
		jacc = env->NewGlobalRef(jaccelerator);
		if(!jacc) {
			env->DeleteGlobalRef(jdoc);
			if(magic) env->ReleaseStringUTFChars(jmagic, magic);
			jni_throw_run(env, "cannot get reference to accelerator stream");
		}
	}

	docarray = env->NewByteArray(sizeof docstate->buffer);
	if(docarray)
		docarray = (jbyteArray)env->NewGlobalRef(docarray);
	if(!docarray) {
		env->DeleteGlobalRef(jacc);
		env->DeleteGlobalRef(jdoc);
		if(magic) env->ReleaseStringUTFChars(jmagic, magic);
		jni_throw_run(env, "cannot create internal buffer for document stream");
	}

	accarray = env->NewByteArray(sizeof accstate->buffer);
	if(accarray)
		accarray = (jbyteArray)env->NewGlobalRef(accarray);
	if(!accarray) {
		env->DeleteGlobalRef(docarray);
		env->DeleteGlobalRef(jacc);
		env->DeleteGlobalRef(jdoc);
		if(magic) env->ReleaseStringUTFChars(jmagic, magic);
	...
	fz_try(ctx)
	{
		if(jdoc) {
			...
			docstate = (SeekableStreamState *)Memento_label(fz_malloc(ctx, sizeof(SeekableStreamState)), "SeekableStreamState_docstate");
		...
		if(jacc) {
			...
			accstate = (SeekableStreamState *)Memento_label(fz_malloc(ctx, sizeof(SeekableStreamState)), "SeekableStreamState_accstate");
	...
	fz_always(ctx)
	{
		...
		if(magic)
			env->ReleaseStringUTFChars(jmagic, magic);
	}
	fz_catch(ctx)
	{
		env->DeleteGlobalRef(accarray);
		env->DeleteGlobalRef(docarray);
		env->DeleteGlobalRef(jacc);
		env->DeleteGlobalRef(jdoc);
		jni_rethrow(env, ctx);
...
JNIEXPORT jobject JNICALL FUN(Document_openNativeWithPath)(JNIEnv *env, jclass cls, jstring jfilename, jstring jaccelerator)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = NULL;
	...
	if(jaccelerator) {
		accelerator = env->GetStringUTFChars(jaccelerator, NULL);
	...
	{
		if(accelerator)
			env->ReleaseStringUTFChars(jaccelerator, accelerator);
		if(filename)
...
JNIEXPORT jobject JNICALL FUN(Document_openNativeWithPathAndStream)(JNIEnv *env, jclass cls, jstring jfilename, jobject jaccelerator)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = NULL;
	const char * filename = NULL;
	...
	if(jaccelerator) {
		jacc = env->NewGlobalRef(jaccelerator);
		if(!jacc) {
			if(jfilename) env->ReleaseStringUTFChars(jfilename, filename);
			jni_throw_run(env, "cannot get reference to accelerator stream");
		}
	}
	...
	fz_try(ctx)
	{
		...
		if(jacc) {
			/* No exceptions can occur from here to stream owning accstate, so we must not free accstate. */
			accstate = (SeekableStreamState *)Memento_label(fz_malloc(ctx, sizeof(SeekableStreamState)), "SeekableStreamState_accstate2");
			accstate->stream = jacc;
			accstate->array = accarray;
...
JNIEXPORT jobject JNICALL FUN(Document_openNativeWithBuffer)(JNIEnv *env, jclass cls, jstring jmagic, jobject jbuffer, jobject jaccelerator)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = NULL;
	const char * magic = NULL;
	...
	if(jbuffer) {
		n = env->GetArrayLength((jarray)jbuffer);
		buffer = env->GetByteArrayElements((jbyteArray)jbuffer, NULL);
		if(!buffer) {
			if(magic) env->ReleaseStringUTFChars(jmagic, magic);
	...
	if(jaccelerator) {
		m = env->GetArrayLength((jarray)jaccelerator);
		accelerator = env->GetByteArrayElements((jbyteArray)jaccelerator, NULL);
		if(!accelerator) {
			if(buffer) env->ReleaseByteArrayElements((jbyteArray)jbuffer, buffer, 0);
			if(magic) env->ReleaseStringUTFChars(jmagic, magic);
	...
	fz_always(ctx)
	{
		...
		if(accelerator) env->ReleaseByteArrayElements((jbyteArray)jaccelerator, accelerator, 0);
		if(buffer) env->ReleaseByteArrayElements((jbyteArray)jbuffer, buffer, 0);
		if(magic) env->ReleaseStringUTFChars(jmagic, magic);
	}
	fz_catch(ctx)
...
JNIEXPORT jboolean JNICALL FUN(Document_recognize)(JNIEnv *env, jclass cls, jstring jmagic)
{
	fz_context * ctx = get_context(env);
	const char * magic = NULL;
...
JNIEXPORT void JNICALL FUN(Document_saveAccelerator)(JNIEnv *env, jobject self, jstring jfilename)
{
	fz_context * ctx = get_context(env);
...
JNIEXPORT void JNICALL FUN(Document_outputAccelerator)(JNIEnv *env, jobject self, jobject jstream)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	...
	stream = env->NewGlobalRef(jstream);
	if(!stream)
		return;

	array = env->NewByteArray(sizeof state->buffer);
	if(array)
		array = (jbyteArray)env->NewGlobalRef(array);
	if(!array) {
	...
	fz_try(ctx)
	{
		state = (SeekableStreamState *)Memento_label(fz_malloc(ctx, sizeof(SeekableStreamState)), "SeekableStreamState_outputAccelerator");
		state->stream = stream;
		state->array = array;
...
JNIEXPORT jboolean JNICALL FUN(Document_needsPassword)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	int okay = 0;
...
JNIEXPORT jboolean JNICALL FUN(Document_authenticatePassword)(JNIEnv *env, jobject self, jstring jpassword)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
...
JNIEXPORT jint JNICALL FUN(Document_countChapters)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	int count = 0;
...
JNIEXPORT jint JNICALL FUN(Document_countPages)(JNIEnv *env, jobject self, jint chapter)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	int count = 0;
...
JNIEXPORT jboolean JNICALL FUN(Document_isReflowable)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	int is_reflowable = 0;
...
JNIEXPORT void JNICALL FUN(Document_layout)(JNIEnv *env, jobject self, jfloat w, jfloat h, jfloat em)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
...
JNIEXPORT jobject JNICALL FUN(Document_loadPage)(JNIEnv *env, jobject self, jint chapter, jint number)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_page * page = NULL;
...
JNIEXPORT jstring JNICALL FUN(Document_getMetaData)(JNIEnv *env, jobject self, jstring jkey)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	const char * key = NULL;
...
JNIEXPORT jboolean JNICALL FUN(Document_isUnencryptedPDF)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
...
JNIEXPORT jobjectArray JNICALL FUN(Document_loadOutline)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_outline * outline = NULL;
	/*jobject*/jobjectArray joutline = NULL;
	...
	fz_try(ctx)
	{
		...
		if(outline) {
			...
			if(!joutline && !env->ExceptionCheck())
...
JNIEXPORT jlong JNICALL FUN(Document_makeBookmark)(JNIEnv *env, jobject self, jint chapter, jint page)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_bookmark mark = 0;
...
JNIEXPORT jobject JNICALL FUN(Document_findBookmark)(JNIEnv *env, jobject self, jlong mark)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_location loc = { -1, -1 };
	...
	return env->NewObject(cls_Location, mid_Location_init, loc.chapter, loc.page, 0, 0);
}

JNIEXPORT jobject JNICALL FUN(Document_resolveLink)(JNIEnv *env, jobject self, jstring juri)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_location loc = { -1, -1 };
	...
	return env->NewObject(cls_Location, mid_Location_init, loc.chapter, loc.page, x, y);
}

JNIEXPORT jboolean JNICALL FUN(Document_hasPermission)(JNIEnv *env, jobject self, jint permission)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	jboolean result = JNI_FALSE;
	fz_try(ctx)
	result = fz_has_permission(ctx, doc, (fz_permission)permission);
	fz_catch(ctx)
	jni_rethrow(env, ctx);
	return result;
}

JNIEXPORT /*jobject*/jobjectArray JNICALL FUN(Document_search)(JNIEnv *env, jobject self, jint chapter, jint page, jstring jneedle)
{
	fz_context * ctx = get_context(env);
	fz_document * doc = from_Document(env, self);
	fz_quad hits[256];
	...
	needle = env->GetStringUTFChars(jneedle, NULL);
	if(!needle) return 0;
	fz_try(ctx)
	n = fz_search_chapter_page_number(ctx, doc, chapter, page, needle, hits, nelem(hits));
	fz_always(ctx)
		env->ReleaseStringUTFChars(jneedle, needle);
	fz_catch(ctx)
	jni_rethrow(env, ctx);
