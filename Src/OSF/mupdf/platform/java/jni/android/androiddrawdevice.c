...
static jlong newNativeAndroidDrawDevice(JNIEnv * env,
    jobject self,
    fz_context * ctx,
    jobject obj,
    jint width,
    jint height,
    NativeDeviceLockFn * lock,
    NativeDeviceUnlockFn * unlock,
    jint xOrigin,
    jint yOrigin,
    jint patchX0,
    jint patchY0,
    ...
    jint patchY1)
{
	...
	fz_try(ctx)
	{
		...
		ninfo->height = height;
		ninfo->object = obj;
		env->SetLongField(self, fid_NativeDevice_nativeInfo, jlong_cast(ninfo));
		env->SetObjectField(self, fid_NativeDevice_nativeResource, obj);
...
JNIEXPORT void JNICALL FUN(android_AndroidDrawDevice_invertLuminance)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
...
JNIEXPORT jlong JNICALL FUN(android_AndroidDrawDevice_newNative)(JNIEnv *env, jclass self, jobject jbitmap, jint xOrigin, jint yOrigin,
    jint pX0, jint pY0, jint pX1, jint pY1)
{
	fz_context * ctx = get_context(env);
	...
	fz_try(ctx)
	device = newNativeAndroidDrawDevice(env,
		self,
		ctx,
		jbitmap,
		info.width,
		info.height,
		androidDrawDevice_lock,
		androidDrawDevice_unlock,
		xOrigin,
		yOrigin,
		pX0,
		pY0,
