...
static pdf_pkcs7_signer * signer_keep(fz_context * ctx, pdf_pkcs7_signer * signer_)
{
	...
	return (pdf_pkcs7_signer *)fz_keep_imp(ctx, signer, &signer->refs);
}
...
static void signer_drop(fz_context * ctx, pdf_pkcs7_signer * signer_)
{
	...
	if(fz_drop_imp(ctx, signer, &signer->refs)) {
		...
		env->DeleteGlobalRef(signer->java_signer);
		fz_free(ctx, signer);
		jni_detach_thread(detach);
	}
}
...
static char * string_field_to_utfchars(fz_context * ctx, JNIEnv * env, jobject obj, jfieldID fid)
{
	...
	if(!jstr) 
		return NULL;
	str = env->GetStringUTFChars(static_cast<jstring>(jstr), NULL);
	if(!str)
...
static pdf_pkcs7_designated_name * signer_designated_name(fz_context * ctx, pdf_pkcs7_signer * signer_)
{
	...
	desname = env->CallObjectMethod((jobject)env, (jmethodID)signer->java_signer, mid_PKCS7Signer_name);
	...
	fz_try(ctx)
	{
		name = static_cast<pdf_pkcs7_designated_name *>(Memento_label(fz_calloc(ctx, 1, sizeof(*name)), "designated name"));
...
static size_t signer_max_digest_size(fz_context * ctx, pdf_pkcs7_signer * signer_)
{
	...
	len = env->CallIntMethod(reinterpret_cast<jobject>(env), (jmethodID)signer->java_signer, mid_PKCS7Signer_maxDigest);
	if(len < 0)
		len = 0;
...
static int signer_create_digest(fz_context * ctx, pdf_pkcs7_signer * signer_, fz_stream * stm, unsigned char * digest, size_t digest_len)
{
	...
	jdigest = env->CallObjectMethod((jobject)env, (jmethodID)jsigner, mid_PKCS7Signer_sign, jstm);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	if(!jdigest)
	...
	if(digest != NULL) {
		jbyte * src = NULL;
		int srclen = 0;
		src = env->GetByteArrayElements((jbyteArray)jdigest, 0);
		if(src == NULL)
			fz_throw_and_detach_thread(ctx, detach, FZ_ERROR_GENERIC, "cannot get digest");
		srclen = env->GetArrayLength((jarray)jdigest);
		if((size_t)srclen > digest_len) {
			env->ReleaseByteArrayElements((jbyteArray)jdigest, src, JNI_ABORT);
...
{
	java_pkcs7_signer * signer = (java_pkcs7_signer *)Memento_label(fz_calloc(ctx, 1, sizeof(*signer)), "java_pkcs7_signer");
	if(signer == NULL) 
		return NULL;
	...
	signer->refs = 1;
	signer->java_signer = env->NewGlobalRef(java_signer);
...
JNIEXPORT jlong JNICALL FUN(PKCS7Signer_newNative)(JNIEnv *env, jclass cls, jobject jsigner)
{
	...
	jsigner = env->NewGlobalRef(jsigner);
...
JNIEXPORT void JNICALL FUN(PKCS7Signer_finalize)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_pkcs7_signer * signer = from_PKCS7Signer_safe(env, self);
	if(!ctx || !signer) return;
	env->SetLongField(self, fid_PKCS7Signer_pointer, 0);
