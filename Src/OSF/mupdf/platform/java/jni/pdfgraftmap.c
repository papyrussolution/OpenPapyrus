...
JNIEXPORT void JNICALL FUN(PDFGraftMap_finalize)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_graft_map * map = from_PDFGraftMap_safe(env, self);
	if(!ctx || !map) return;
	env->SetLongField(self, fid_PDFGraftMap_pointer, 0);
	pdf_drop_graft_map(ctx, map);
}

JNIEXPORT jobject JNICALL FUN(PDFGraftMap_graftObject)(JNIEnv *env, jobject self, jobject jobj)
{
	fz_context * ctx = get_context(env);
...
JNIEXPORT void JNICALL FUN(PDFGraftMap_graftPage)(JNIEnv *env, jobject self, jint pageTo, jobject jobj, jint pageFrom)
{
	fz_context * ctx = get_context(env);
