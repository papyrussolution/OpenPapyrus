...
JNIEXPORT jobject JNICALL FUN(PDFPage_createAnnotation)(JNIEnv *env, jobject self, jint subtype)
{
...
JNIEXPORT void JNICALL FUN(PDFPage_deleteAnnotation)(JNIEnv *env, jobject self, jobject jannot)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
...
JNIEXPORT jboolean JNICALL FUN(PDFPage_update)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
...
JNIEXPORT jboolean JNICALL FUN(PDFPage_applyRedactions)(JNIEnv *env, jobject self, jboolean blackBoxes, jint imageMethod)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
...
JNIEXPORT /*jobject*/jobjectArray JNICALL FUN(PDFPage_getAnnotations)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
	pdf_annot * annot = NULL;
	pdf_annot * annots = NULL;
	/*jobject*/jobjectArray jannots = NULL;
	int count;
	int i;
	if(!ctx || !page) return NULL;
	...
	jannots = env->NewObjectArray(count, cls_PDFAnnotation, NULL);
	if(!jannots || env->ExceptionCheck()) jni_throw_null(env, "cannot wrap page annotations in object array");
	annot = annots;
	for(i = 0; annot && i < count; i++) {
		jobject jannot = to_PDFAnnotation_safe(ctx, env, annot);
		if(!jannot) return NULL;
		env->SetObjectArrayElement(jannots, i, jannot);
		if(env->ExceptionCheck()) return NULL;
		env->DeleteLocalRef(jannot);
		fz_try(ctx)
		annot = pdf_next_annot(ctx, annot);
		fz_catch(ctx)
		jni_rethrow(env, ctx);
...
JNIEXPORT jobjectArray JNICALL FUN(PDFPage_getWidgetsNative)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
	...
	jwidgets = env->NewObjectArray(count, cls_PDFWidget, NULL);
	if(!jwidgets || env->ExceptionCheck()) jni_throw_null(env, "cannot wrap page widgets in object array");
	...
	for(i = 0; widget && i < count; i++) {
		...
		env->SetObjectArrayElement(jwidgets, i, jwidget);
		if(env->ExceptionCheck()) return NULL;

		env->DeleteLocalRef(jwidget);
...
JNIEXPORT jobject JNICALL FUN(PDFPage_createSignature)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	pdf_page * page = from_PDFPage(env, self);
	pdf_widget * widget = NULL;
