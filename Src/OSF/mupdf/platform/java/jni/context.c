...
JNIEXPORT jint JNICALL FUN(Context_initNative)(JNIEnv *env, jclass cls)
{
	if(!check_enums())
		return -1;
...
JNIEXPORT void JNICALL FUN(Context_emptyStore)(JNIEnv *env, jclass cls)
{
...
JNIEXPORT void JNICALL FUN(Context_enableICC)(JNIEnv *env, jclass cls)
{
	fz_context * ctx = get_context(env);
	if(!ctx) return;
...
JNIEXPORT void JNICALL FUN(Context_disableICC)(JNIEnv *env, jclass cls)
{
	fz_context * ctx = get_context(env);
	if(!ctx) return;
...
JNIEXPORT void JNICALL FUN(Context_setAntiAliasLevel)(JNIEnv *env, jclass cls, jint level)
{
	fz_context * ctx = get_context(env);
	if(!ctx) return;
...
JNIEXPORT jobject JNICALL FUN(Context_getVersion)(JNIEnv *env, jclass cls)
{
	fz_context * ctx = get_context(env);
	...
	jvs = env->NewStringUTF(FZ_VERSION);
	if(!jvs || env->ExceptionCheck())
		return NULL;

	jversion = env->NewObject(cls_Context_Version, mid_Context_Version_init);
	if(!jversion || env->ExceptionCheck())
		return NULL;
	env->SetIntField(jversion, fid_Context_Version_major, FZ_VERSION_MAJOR);
	env->SetIntField(jversion, fid_Context_Version_minor, FZ_VERSION_MINOR);
	env->SetIntField(jversion, fid_Context_Version_patch, FZ_VERSION_PATCH);
	env->SetObjectField(jversion, fid_Context_Version_version, jvs);
	return jversion;
}
