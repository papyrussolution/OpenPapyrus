...
static inline jobject to_ColorSpace(fz_context * ctx, JNIEnv * env, fz_colorspace * cs)
{
	...
	jcs = env->CallStaticObjectMethod((jclass)env, cls_ColorSpace, mid_ColorSpace_fromPointer, jlong_cast(cs));
	if(!jcs)
...
static inline jobject to_FitzInputStream(fz_context * ctx, JNIEnv * env, fz_stream * stm)
{
	...
	jstm = env->NewObject(cls_FitzInputStream, mid_FitzInputStream_init, jlong_cast(stm));
	if(!jstm)
...
static inline jobject to_Image(fz_context * ctx, JNIEnv * env, fz_image * img)
{
	...
	jimg = env->NewObject(cls_Image, mid_Image_init, jlong_cast(img));
	if(!jimg)
...
static inline jobject to_Matrix(fz_context * ctx, JNIEnv * env, fz_matrix mat)
{
	jobject jctm;
	if(!ctx) return NULL;
	jctm = env->NewObject(cls_Matrix, mid_Matrix_init, mat.a, mat.b, mat.c, mat.d, mat.e, mat.f);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
	return jctm;
}
...
static inline jobject to_Path(fz_context * ctx, JNIEnv * env, const fz_path * path)
{
	...
	jpath = env->NewObject(cls_Path, mid_Path_init, jlong_cast(path));
	if(!jpath)
...
static inline jobject to_Rect(fz_context * ctx, JNIEnv * env, fz_rect rect)
{
	...
	jrect = env->NewObject(cls_Rect, mid_Rect_init, rect.x0, rect.y0, rect.x1, rect.y1);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
...
static inline jobject to_Shade(fz_context * ctx, JNIEnv * env, fz_shade * shd)
{
	...
	jshd = env->NewObject(cls_Shade, mid_Shade_init, jlong_cast(shd));
	if(!jshd)
...
static inline jobject to_StrokeState(fz_context * ctx, JNIEnv * env, const fz_stroke_state * state)
{
	...
	jstate = env->NewObject(cls_StrokeState, mid_StrokeState_init, jlong_cast(state));
	if(!jstate)
...
static inline jobject to_Text(fz_context * ctx, JNIEnv * env, const fz_text * text)
{
	...
	jtext = env->NewObject(cls_Text, mid_Text_init, jlong_cast(text));
	if(!jtext)
...
static inline jbyteArray to_byteArray(fz_context * ctx, JNIEnv * env, const unsigned char * arr, jint n)
{
	jbyteArray jarr;
	if(!ctx) return NULL;
	jarr = env->NewByteArray(n);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
	if(!jarr)
		fz_throw(ctx, FZ_ERROR_GENERIC, "cannot allocate byte array");
	env->SetByteArrayRegion(jarr, 0, n, (jbyte*)arr);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
	return jarr;
}
...
static inline jfloatArray to_floatArray(fz_context * ctx, JNIEnv * env, const float * arr, jint n)
{
	jfloatArray jarr;
	if(!ctx) return NULL;
	jarr = env->NewFloatArray(n);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
	if(!jarr)
		fz_throw(ctx, FZ_ERROR_GENERIC, "cannot allocate float array");
	env->SetFloatArrayRegion(jarr, 0, n, arr);
	if(env->ExceptionCheck())
		fz_throw_java(ctx, env);
	return jarr;
}
...
static inline jobject to_Font_safe(fz_context * ctx, JNIEnv * env, fz_font * font)
{
	...
	jfont = env->NewObject(cls_Font, mid_Font_init, jlong_cast(font));
	if(!jfont)
...
static inline jobject to_Matrix_safe(fz_context * ctx, JNIEnv * env, fz_matrix mat)
{
	if(!ctx) 
		return NULL;
	return env->NewObject(cls_Matrix, mid_Matrix_init, mat.a, mat.b, mat.c, mat.d, mat.e, mat.f);
}

static inline /*jobject*/jobjectArray to_Outline_safe(fz_context * ctx, JNIEnv * env, fz_document * doc, fz_outline * outline)
{
	jobject joutline = NULL;
	/*jobject*/jobjectArray jarr = NULL;
	jsize jindex = 0;
	jsize count = 0;
	...
	jarr = env->NewObjectArray(count, cls_Outline, NULL);
	if(!jarr || env->ExceptionCheck()) 
		return NULL;
	while(outline) {
		jstring jtitle = NULL;
		jstring juri = NULL;
		jobject jdown = NULL;
		if(outline->title) {
			jtitle = env->NewStringUTF(outline->title);
			if(!jtitle || env->ExceptionCheck()) return NULL;
		}
		if(outline->uri) {
			juri = env->NewStringUTF(outline->uri);
			if(!juri || env->ExceptionCheck()) return NULL;
		}
		if(outline->down) {
			jdown = to_Outline_safe(ctx, env, doc, outline->down);
			if(!jdown) return NULL;
		}
		joutline = env->NewObject(cls_Outline, mid_Outline_init, jtitle, juri, jdown);
		if(!joutline) return NULL;
		if(jdown)
			env->DeleteLocalRef(jdown);
		if(juri)
			env->DeleteLocalRef(juri);
		if(jtitle)
			env->DeleteLocalRef(jtitle);
		env->SetObjectArrayElement(jarr, jindex++, joutline);
		if(env->ExceptionCheck()) return NULL;

		env->DeleteLocalRef(joutline);
...
static inline jobject to_PDFAnnotation_safe(fz_context * ctx, JNIEnv * env, pdf_annot * annot)
{
	...
	jannot = env->NewObject(cls_PDFAnnotation, mid_PDFAnnotation_init, jlong_cast(annot));
	if(!jannot)
...
static inline jobject to_PDFObject_safe(fz_context * ctx, JNIEnv * env, pdf_obj * obj)
{
	...
	jobj = env->NewObject(cls_PDFObject, mid_PDFObject_init, jlong_cast(obj));
	if(!jobj)
...
static inline jobject to_Point_safe(fz_context * ctx, JNIEnv * env, fz_point point)
{
	...
	return env->NewObject(cls_Point, mid_Point_init, point.x, point.y);
}
...
static inline jobject to_Quad_safe(fz_context * ctx, JNIEnv * env, fz_quad quad)
{
	...
	return env->NewObject(cls_Quad, mid_Quad_init,
		   quad.ul.x, quad.ul.y,
		   quad.ur.x, quad.ur.y,
		   quad.ll.x, quad.ll.y,
		   quad.lr.x, quad.lr.y);
...
static inline jobjectArray to_QuadArray_safe(fz_context * ctx, JNIEnv * env, const fz_quad * quads, jint n)
{
	...
	arr = env->NewObjectArray(n, cls_Quad, NULL);
	if(!arr || env->ExceptionCheck()) return NULL;

	for(i = 0; i < n; i++) {
		jobject jquad = to_Quad_safe(ctx, env, quads[i]);
		if(!jquad) return NULL;

		env->SetObjectArrayElement(arr, i, jquad);
		if(env->ExceptionCheck()) return NULL;
...
static inline jobject to_Rect_safe(fz_context * ctx, JNIEnv * env, fz_rect rect)
{
	...
	return env->NewObject(cls_Rect, mid_Rect_init, rect.x0, rect.y0, rect.x1, rect.y1);
}
...
static inline jobjectArray to_StringArray_safe(fz_context * ctx, JNIEnv * env, const char ** strings, int n)
{
	...
	arr = env->NewObjectArray(n, cls_String, NULL);
	if(!arr || env->ExceptionCheck()) return NULL;

	for(i = 0; i < n; i++) {
		jstring jstring;

		jstring = env->NewStringUTF(strings[i]);
		if(!jstring || env->ExceptionCheck()) return NULL;

		env->SetObjectArrayElement(arr, i, jstring);
		if(env->ExceptionCheck()) return NULL;
...
static inline jobject to_PDFWidget_safe(fz_context * ctx, JNIEnv * env, pdf_widget * widget)
{
	...
	jwidget = env->NewObject(cls_PDFWidget, mid_PDFWidget_init, jlong_cast(widget));
	if(!jwidget || env->ExceptionCheck()) {
	...
	fz_try(ctx)
	{
		...
		env->SetIntField(jwidget, fid_PDFWidget_fieldType, fieldType);
		env->SetIntField(jwidget, fid_PDFWidget_fieldFlags, fieldFlags);
		if(fieldType == PDF_WIDGET_TYPE_TEXT) {
			env->SetIntField(jwidget, fid_PDFWidget_maxLen, pdf_text_widget_max_len(ctx, widget));
			env->SetIntField(jwidget, fid_PDFWidget_textFormat, pdf_text_widget_format(ctx, widget));
		}
		if(fieldType == PDF_WIDGET_TYPE_COMBOBOX || fieldType == PDF_WIDGET_TYPE_LISTBOX) {
			nopts = pdf_choice_widget_options(ctx, widget, 0, NULL);
			if(nopts > 0) {
				opts = (char **)Memento_label(fz_malloc(ctx, nopts * sizeof(*opts)), "to_PDFWidget");
				pdf_choice_widget_options(ctx, widget, 0, (const char**)opts);
				jopts = to_StringArray_safe(ctx, env, (const char**)opts, nopts);
				if(!jopts || env->ExceptionCheck())
					fz_throw_java(ctx, env);
			}
		}
	}
	...
	env->SetObjectField(jwidget, fid_PDFWidget_options, jopts);
...
static inline jobject to_Document_safe_own(fz_context * ctx, JNIEnv * env, fz_document * doc)
{
	...
	if(pdf)
		obj = env->NewObject(cls_PDFDocument, mid_PDFDocument_init, jlong_cast(pdf));
	else
		obj = env->NewObject(cls_Document, mid_Document_init, jlong_cast(doc));
	if(!obj)
...
static inline jobject to_Device_safe_own(fz_context * ctx, JNIEnv * env, fz_device * device)
{
	...
	jdev = env->NewObject(cls_DisplayList, mid_Device_init, jlong_cast(device));
	if(!jdev)
...
static inline jobject to_DisplayList_safe_own(fz_context * ctx, JNIEnv * env, fz_display_list * list)
{
	...
	jlist = env->NewObject(cls_DisplayList, mid_DisplayList_init, jlong_cast(list));
	if(!jlist)
...
static inline jobject to_Page_safe_own(fz_context * ctx, JNIEnv * env, fz_page * page)
{
	...
	if(ppage)
		jobj = env->NewObject(cls_PDFPage, mid_PDFPage_init, jlong_cast(page));
	else
		jobj = env->NewObject(cls_Page, mid_Page_init, jlong_cast(page));
	if(!jobj)
...
static inline jobject to_Link_safe_own(fz_context * ctx, JNIEnv * env, fz_link * link)
{
	...
	if(!jbounds || env->ExceptionCheck()) {
		fz_drop_link(ctx, link);
		return NULL;
	}
	juri = env->NewStringUTF(link->uri);
	if(!juri || env->ExceptionCheck()) {
		fz_drop_link(ctx, link);
		return NULL;
	}
	jobj = env->NewObject(cls_Link, mid_Link_init, jbounds, juri);
	if(!jobj)
		fz_drop_link(ctx, link);
	return jobj;
}
...
static inline jobject to_PDFGraftMap_safe_own(fz_context * ctx, JNIEnv * env, jobject pdf, pdf_graft_map * map)
{
	...
	jmap = env->NewObject(cls_PDFGraftMap, mid_PDFGraftMap_init, jlong_cast(map), pdf);
	if(!jmap)
...
static inline jobject to_Pixmap_safe_own(fz_context * ctx, JNIEnv * env, fz_pixmap * pixmap)
{
	...
	jobj = env->NewObject(cls_Pixmap, mid_Pixmap_init, jlong_cast(pixmap));
	if(!jobj)
...
static inline jobject to_StructuredText_safe_own(fz_context * ctx, JNIEnv * env, fz_stext_page * text)
{
	...
	jtext = env->NewObject(cls_StructuredText, mid_StructuredText_init, jlong_cast(text));
	if(!jtext)
...
static inline fz_buffer * from_Buffer(JNIEnv * env, jobject jobj)
{
	fz_buffer * buffer;
	if(!jobj) return NULL;
	buffer = CAST(fz_buffer *, env->GetLongField(jobj, fid_Buffer_pointer));
...
static inline fz_colorspace * from_ColorSpace(JNIEnv * env, jobject jobj)
{
	fz_colorspace * cs;
	if(!jobj) return NULL;
	cs = CAST(fz_colorspace *, env->GetLongField(jobj, fid_ColorSpace_pointer));
...
static inline fz_cookie * from_Cookie(JNIEnv * env, jobject jobj)
{
	fz_cookie * cookie;
	if(!jobj) return NULL;
	cookie = CAST(fz_cookie *, env->GetLongField(jobj, fid_Cookie_pointer));
...
static fz_device * from_Device(JNIEnv * env, jobject jobj)
{
	fz_device * dev;
	if(!jobj) return NULL;
	dev = CAST(fz_device *, env->GetLongField(jobj, fid_Device_pointer));
...
static inline fz_display_list * from_DisplayList(JNIEnv * env, jobject jobj)
{
	...
	if(!jobj) return NULL;
	list = CAST(fz_display_list *, env->GetLongField(jobj, fid_DisplayList_pointer));
...
static inline fz_document * from_Document(JNIEnv * env, jobject jobj)
{
	fz_document * doc;
	if(!jobj) return NULL;
	doc = CAST(fz_document *, env->GetLongField(jobj, fid_Document_pointer));
...
static inline fz_document_writer * from_DocumentWriter(JNIEnv * env, jobject jobj)
{
	...
	if(!jobj) return NULL;
	wri = CAST(fz_document_writer *, env->GetLongField(jobj, fid_DocumentWriter_pointer));
...
static inline fz_font * from_Font(JNIEnv * env, jobject jobj)
{
	fz_font * font;
	if(!jobj) return NULL;
	font = CAST(fz_font *, env->GetLongField(jobj, fid_Font_pointer));
...
static inline fz_image * from_Image(JNIEnv * env, jobject jobj)
{
	fz_image * image;
	if(!jobj) return NULL;
	image = CAST(fz_image *, env->GetLongField(jobj, fid_Image_pointer));
...
static inline fz_page * from_Page(JNIEnv * env, jobject jobj)
{
	fz_page * page;
	if(!jobj) return NULL;
	page = CAST(fz_page *, env->GetLongField(jobj, fid_Page_pointer));
...
static inline fz_path * from_Path(JNIEnv * env, jobject jobj)
{
	fz_path * path;
	if(!jobj) return NULL;
	path = CAST(fz_path *, env->GetLongField(jobj, fid_Path_pointer));
...
static inline pdf_annot * from_PDFAnnotation(JNIEnv * env, jobject jobj)
{
	pdf_annot * annot;
	if(!jobj) return NULL;
	annot = CAST(pdf_annot *, env->GetLongField(jobj, fid_PDFAnnotation_pointer));
...
static inline pdf_document * from_PDFDocument(JNIEnv * env, jobject jobj)
{
	pdf_document * pdf;
	if(!jobj) return NULL;
	pdf = CAST(pdf_document *, env->GetLongField(jobj, fid_PDFDocument_pointer));
...
static inline pdf_graft_map * from_PDFGraftMap(JNIEnv * env, jobject jobj)
{
	pdf_graft_map * map;
	if(!jobj) return NULL;
	map = CAST(pdf_graft_map *, env->GetLongField(jobj, fid_PDFGraftMap_pointer));
...
static inline pdf_obj * from_PDFObject(JNIEnv * env, jobject jobj)
{
	pdf_obj * obj;
	if(!jobj) return NULL;
	obj = CAST(pdf_obj *, env->GetLongField(jobj, fid_PDFObject_pointer));
	return obj;
}
...
static inline pdf_page * from_PDFPage(JNIEnv * env, jobject jobj)
{
	pdf_page * page;
	if(!jobj) return NULL;
	page = CAST(pdf_page *, env->GetLongField(jobj, fid_PDFPage_pointer));
...
static inline fz_pixmap * from_Pixmap(JNIEnv * env, jobject jobj)
{
	fz_pixmap * pixmap;
	if(!jobj) return NULL;
	pixmap = CAST(fz_pixmap *, env->GetLongField(jobj, fid_Pixmap_pointer));
...
static inline fz_shade * from_Shade(JNIEnv * env, jobject jobj)
{
	fz_shade * shd;
	if(!jobj) return NULL;
	shd = CAST(fz_shade *, env->GetLongField(jobj, fid_Shade_pointer));
...
static inline fz_stroke_state * from_StrokeState(JNIEnv * env, jobject jobj)
{
	...
	if(!jobj) return NULL;
	stroke = CAST(fz_stroke_state *, env->GetLongField(jobj, fid_StrokeState_pointer));
...
static inline fz_stext_page * from_StructuredText(JNIEnv * env, jobject jobj)
{
	fz_stext_page * stext;
	if(!jobj) return NULL;
	stext = CAST(fz_stext_page *, env->GetLongField(jobj, fid_StructuredText_pointer));
...
static inline fz_text * from_Text(JNIEnv * env, jobject jobj)
{
	fz_text * text;
	if(!jobj) return NULL;
	text = CAST(fz_text *, env->GetLongField(jobj, fid_Text_pointer));
...
static inline fz_matrix from_Matrix(JNIEnv * env, jobject jmat)
{
	...
	mat.a = env->GetFloatField(jmat, fid_Matrix_a);
	mat.b = env->GetFloatField(jmat, fid_Matrix_b);
	mat.c = env->GetFloatField(jmat, fid_Matrix_c);
	mat.d = env->GetFloatField(jmat, fid_Matrix_d);
	mat.e = env->GetFloatField(jmat, fid_Matrix_e);
	mat.f = env->GetFloatField(jmat, fid_Matrix_f);
...
static inline fz_point from_Point(JNIEnv * env, jobject jpt)
{
	...
	pt.x = env->GetFloatField(jpt, fid_Point_x);
	pt.y = env->GetFloatField(jpt, fid_Point_y);
...
static inline fz_rect from_Rect(JNIEnv * env, jobject jrect)
{
	...
	rect.x0 = env->GetFloatField(jrect, fid_Rect_x0);
	rect.x1 = env->GetFloatField(jrect, fid_Rect_x1);
	rect.y0 = env->GetFloatField(jrect, fid_Rect_y0);
	rect.y1 = env->GetFloatField(jrect, fid_Rect_y1);
	return rect;
}
...
static inline fz_quad from_Quad(JNIEnv * env, jobject jquad)
{
	...
	quad.ul.x = env->GetFloatField(jquad, fid_Quad_ul_x);
	quad.ul.y = env->GetFloatField(jquad, fid_Quad_ul_y);
	quad.ur.x = env->GetFloatField(jquad, fid_Quad_ur_x);
	quad.ur.y = env->GetFloatField(jquad, fid_Quad_ur_y);
	quad.ll.x = env->GetFloatField(jquad, fid_Quad_ll_x);
	quad.ll.y = env->GetFloatField(jquad, fid_Quad_ll_y);
	quad.lr.x = env->GetFloatField(jquad, fid_Quad_lr_x);
	quad.lr.y = env->GetFloatField(jquad, fid_Quad_lr_y);
...
{
	if(!jobj) return NULL;
	return CAST(fz_buffer *, env->GetLongField(jobj, fid_Buffer_pointer));
}
...
{
	if(!jobj) return NULL;
	return CAST(fz_colorspace *, env->GetLongField(jobj, fid_ColorSpace_pointer));
}

static inline fz_cookie * from_Cookie_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_cookie *, env->GetLongField(jobj, fid_Cookie_pointer));
}

static fz_device * from_Device_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_device *, env->GetLongField(jobj, fid_Device_pointer));
}

static inline fz_display_list * from_DisplayList_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_display_list *, env->GetLongField(jobj, fid_DisplayList_pointer));
}

static inline fz_document * from_Document_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_document *, env->GetLongField(jobj, fid_Document_pointer));
}

static inline fz_document_writer * from_DocumentWriter_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_document_writer *, env->GetLongField(jobj, fid_DocumentWriter_pointer));
}

static inline fz_font * from_Font_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_font *, env->GetLongField(jobj, fid_Font_pointer));
}

static inline fz_image * from_Image_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_image *, env->GetLongField(jobj, fid_Image_pointer));
}

static inline fz_page * from_Page_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_page *, env->GetLongField(jobj, fid_Page_pointer));
}

static inline fz_path * from_Path_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_path *, env->GetLongField(jobj, fid_Path_pointer));
}

static inline pdf_annot * from_PDFAnnotation_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_annot *, env->GetLongField(jobj, fid_PDFAnnotation_pointer));
}

static inline pdf_document * from_PDFDocument_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_document *, env->GetLongField(jobj, fid_PDFDocument_pointer));
}

static inline pdf_graft_map * from_PDFGraftMap_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_graft_map *, env->GetLongField(jobj, fid_PDFGraftMap_pointer));
}

static inline pdf_obj * from_PDFObject_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_obj *, env->GetLongField(jobj, fid_PDFObject_pointer));
}

static inline pdf_widget * from_PDFWidget_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_widget *, env->GetLongField(jobj, fid_PDFWidget_pointer));
}

static inline pdf_pkcs7_signer * from_PKCS7Signer_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(pdf_pkcs7_signer *, env->GetLongField(jobj, fid_PKCS7Signer_pointer));
}

static inline java_pkcs7_verifier * from_PKCS7Verifier_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(java_pkcs7_verifier *, env->GetLongField(jobj, fid_PKCS7Verifier_pointer));
}

static inline fz_stream * from_FitzInputStream_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_stream *, env->GetLongField(jobj, fid_FitzInputStream_pointer));
}

static inline fz_pixmap * from_Pixmap_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_pixmap *, env->GetLongField(jobj, fid_Pixmap_pointer));
}

static inline fz_shade * from_Shade_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_shade *, env->GetLongField(jobj, fid_Shade_pointer));
}

static inline fz_stroke_state * from_StrokeState_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_stroke_state *, env->GetLongField(jobj, fid_StrokeState_pointer));
}

static inline fz_stext_page * from_StructuredText_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_stext_page *, env->GetLongField(jobj, fid_StructuredText_pointer));
}

static inline fz_text * from_Text_safe(JNIEnv * env, jobject jobj)
{
	if(!jobj) return NULL;
	return CAST(fz_text *, env->GetLongField(jobj, fid_Text_pointer));
}
