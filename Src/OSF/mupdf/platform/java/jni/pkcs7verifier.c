...
static void java_pkcs7_drop_verifier(fz_context * ctx, pdf_pkcs7_verifier * verifier_)
{
	...
	env->DeleteGlobalRef(verifier->jverifier);
	fz_free(ctx, verifier);

	jni_detach_thread(detach);
}

static pdf_signature_error java_pkcs7_check_certificate(fz_context * ctx,
    ...
    size_t len)
{
	...
	result = env->CallIntMethod(reinterpret_cast<jobject>(env), (jmethodID)jverifier, mid_PKCS7Verifier_checkCertificate, jsignature);
...
static pdf_signature_error java_pkcs7_check_digest(fz_context * ctx,
    pdf_pkcs7_verifier * verifier,
    fz_stream * stm,
    unsigned char * signature,
    size_t len)
{
	...
	fz_catch(ctx)
	fz_rethrow_and_detach_thread(ctx, detach);
	result = env->CallIntMethod(reinterpret_cast<jobject>(env), (jmethodID)jverifier, mid_PKCS7Verifier_checkDigest, jstm, jsignature);
	if(env->ExceptionCheck())
		fz_throw_java_and_detach_thread(ctx, env, detach);
	jni_detach_thread(detach);
	return result;
...
JNIEXPORT jlong JNICALL FUN(PKCS7Verifier_newNative)(JNIEnv *env, jobject self, jobject jverifier)
{
	...
	jverifier = env->NewGlobalRef(jverifier);
...
JNIEXPORT void JNICALL FUN(PKCS7Verifier_finalize)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	java_pkcs7_verifier * verifier = from_PKCS7Verifier_safe(env, self);
	if(!ctx || !verifier) return;
	env->SetLongField(self, fid_PKCS7Verifier_pointer, 0);
