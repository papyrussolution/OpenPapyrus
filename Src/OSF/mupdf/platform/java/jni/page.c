...
JNIEXPORT void JNICALL FUN(Page_finalize)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page_safe(env, self);
	if(!ctx || !page) return;
	env->SetLongField(self, fid_Page_pointer, 0);
	fz_drop_page(ctx, page);
}

JNIEXPORT jobject JNICALL FUN(Page_toPixmap)(JNIEnv *env, jobject self, jobject jctm, jobject jcs, jboolean alpha, jboolean showExtra)
{
	fz_context * ctx = get_context(env);
...
JNIEXPORT jobject JNICALL FUN(Page_getBounds)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_rect rect;
...
JNIEXPORT void JNICALL FUN(Page_run)(JNIEnv *env, jobject self, jobject jdev, jobject jctm, jobject jcookie)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
...
JNIEXPORT void JNICALL FUN(Page_runPageContents)(JNIEnv *env, jobject self, jobject jdev, jobject jctm, jobject jcookie)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_device * dev = from_Device(env, jdev);
...
JNIEXPORT void JNICALL FUN(Page_runPageAnnots)(JNIEnv *env, jobject self, jobject jdev, jobject jctm, jobject jcookie)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_device * dev = from_Device(env, jdev);
...
JNIEXPORT void JNICALL FUN(Page_runPageWidgets)(JNIEnv *env, jobject self, jobject jdev, jobject jctm, jobject jcookie)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_device * dev = from_Device(env, jdev);
...
JNIEXPORT /*jobject*/jobjectArray JNICALL FUN(Page_getLinks)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_link * link = NULL;
	fz_link * links = NULL;
	/*jobject*/jobjectArray jlinks = NULL;
	int link_count;
	int i;
	if(!ctx || !page) return NULL;
	fz_var(links);
	...
	jlinks = env->NewObjectArray(link_count, cls_Link, NULL);
	if(!jlinks || env->ExceptionCheck()) {
		fz_drop_link(ctx, links);
		return NULL;
	}
	link = links;
	for(i = 0; link && i < link_count; i++) {
		...
		jlink = env->NewObject(cls_Link, mid_Link_init, jbounds, juri);
		if(!jlink || env->ExceptionCheck()) {
			fz_drop_link(ctx, links);
			return NULL;
		}
		env->DeleteLocalRef(juri);
		env->DeleteLocalRef(jbounds);
		env->SetObjectArrayElement(jlinks, i, jlink);
...
JNIEXPORT /*jobject*/jobjectArray JNICALL FUN(Page_search)(JNIEnv *env, jobject self, jstring jneedle)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_quad hits[256];
...
JNIEXPORT jobject JNICALL FUN(Page_toDisplayList)(JNIEnv *env, jobject self, jboolean showExtra)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
...
JNIEXPORT jobject JNICALL FUN(Page_toStructuredText)(JNIEnv *env, jobject self, jstring joptions)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
...
JNIEXPORT jbyteArray JNICALL FUN(Page_textAsHtml)(JNIEnv *env, jobject self)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
	fz_stext_page * text = NULL;
	...
	fz_try(ctx)
	{
		...
		env->SetByteArrayRegion(arr, 0, (jsize)len, (jbyte*)data);
		if(env->ExceptionCheck())
			fz_throw_java(ctx, env);
	}
	fz_always(ctx)
...
JNIEXPORT jobject JNICALL FUN(Page_createLink)(JNIEnv *env, jobject self, jobject jrect, jstring juri)
{
	fz_context * ctx = get_context(env);
	fz_page * page = from_Page(env, self);
