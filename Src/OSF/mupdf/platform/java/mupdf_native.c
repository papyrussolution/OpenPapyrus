...
static void fz_throw_java_and_detach_thread(fz_context * ctx, JNIEnv * env, jboolean detach)
{
	...
	if(ex) {
		jobject msg;
		env->ExceptionClear();
		msg = env->CallObjectMethod(ex, mid_Object_toString);
		if(env->ExceptionCheck())
			env->ExceptionClear();
		else if(msg) {
			const char * p = env->GetStringUTFChars(static_cast<jstring>(msg), NULL);
			if(p) {
				char buf[256];
				fz_strlcpy(buf, p, sizeof buf);
				env->ReleaseStringUTFChars((jstring)msg, p);
				if(detach)
...
static jclass get_class(int * failed, JNIEnv * env, const char * name)
{
	...
	current_class = (jclass)env->NewGlobalRef(local);
	if(!current_class) {
...
static JNIEnv * jni_attach_thread(fz_context * ctx, jboolean * detach)
{
	...
	int state;
	*detach = JNI_FALSE;
	state = jvm->GetEnv((void **)&env, MY_JNI_VERSION);
	if(state == JNI_EDETACHED) {
		*detach = JNI_TRUE;
		state = jvm->AttachCurrentThread((void **)&env, NULL);
	}
...
{
	env->DeleteGlobalRef(cls_Buffer);
	env->DeleteGlobalRef(cls_ColorSpace);
	env->DeleteGlobalRef(cls_Context_Version);
	env->DeleteGlobalRef(cls_Cookie);
	env->DeleteGlobalRef(cls_Device);
	env->DeleteGlobalRef(cls_DisplayList);
	env->DeleteGlobalRef(cls_Document);
	env->DeleteGlobalRef(cls_DocumentWriter);
	env->DeleteGlobalRef(cls_FitzInputStream);
	env->DeleteGlobalRef(cls_FloatArray);
	env->DeleteGlobalRef(cls_Font);
	env->DeleteGlobalRef(cls_IllegalArgumentException);
	env->DeleteGlobalRef(cls_Image);
	env->DeleteGlobalRef(cls_IndexOutOfBoundsException);
	env->DeleteGlobalRef(cls_IntegerArray);
	env->DeleteGlobalRef(cls_IOException);
	env->DeleteGlobalRef(cls_Link);
	env->DeleteGlobalRef(cls_Location);
	env->DeleteGlobalRef(cls_Matrix);
	env->DeleteGlobalRef(cls_NativeDevice);
	env->DeleteGlobalRef(cls_NullPointerException);
	env->DeleteGlobalRef(cls_Object);
	env->DeleteGlobalRef(cls_Outline);
	env->DeleteGlobalRef(cls_OutOfMemoryError);
	env->DeleteGlobalRef(cls_Page);
	env->DeleteGlobalRef(cls_Path);
	env->DeleteGlobalRef(cls_PathWalker);
	env->DeleteGlobalRef(cls_PDFAnnotation);
	env->DeleteGlobalRef(cls_PDFDocument);
	env->DeleteGlobalRef(cls_PDFDocument_JsEventListener);
	env->DeleteGlobalRef(cls_PDFPage);
	env->DeleteGlobalRef(cls_PDFGraftMap);
	env->DeleteGlobalRef(cls_PDFObject);
	env->DeleteGlobalRef(cls_Pixmap);
	env->DeleteGlobalRef(cls_Point);
	env->DeleteGlobalRef(cls_Quad);
	env->DeleteGlobalRef(cls_Rect);
	env->DeleteGlobalRef(cls_RuntimeException);
	env->DeleteGlobalRef(cls_SeekableStream);
	env->DeleteGlobalRef(cls_SeekableInputStream);
	env->DeleteGlobalRef(cls_SeekableOutputStream);
	env->DeleteGlobalRef(cls_Shade);
	env->DeleteGlobalRef(cls_String);
	env->DeleteGlobalRef(cls_StrokeState);
	env->DeleteGlobalRef(cls_StructuredText);
	env->DeleteGlobalRef(cls_StructuredTextWalker);
	env->DeleteGlobalRef(cls_Text);
	env->DeleteGlobalRef(cls_TextBlock);
	env->DeleteGlobalRef(cls_TextChar);
	env->DeleteGlobalRef(cls_TextLine);
	env->DeleteGlobalRef(cls_TextWalker);
	env->DeleteGlobalRef(cls_TryLaterException);
	env->DeleteGlobalRef(cls_UnsupportedOperationException);
	env->DeleteGlobalRef(cls_PDFWidget);
	env->DeleteGlobalRef(cls_PKCS7Signer);
	env->DeleteGlobalRef(cls_PKCS7Verifier);
	env->DeleteGlobalRef(cls_PKCS7DesignatedName);
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM * vm, void * reserved)
{
	JNIEnv * env;
	jint ret;
	ret = vm->GetEnv((void**)&env, MY_JNI_VERSION);
	if(ret != JNI_OK) {
...
{
	JNIEnv * env;
	jint ret = vm->GetEnv((void**)&env, MY_JNI_VERSION);
	if(ret != JNI_OK) {
