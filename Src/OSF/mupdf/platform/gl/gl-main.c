...
static void open_browser(const char * uri)
{
	...
	if(pid == 0) {
		if(fork() == 0) {
			...
			slfprintf_stderr("cannot exec '%s'\n", browser);
		}
		_exit(0);
	}
	waitpid(pid, NULL, 0);
...
static void process_sigs(fz_context * ctx_, pdf_obj * field, void * arg, pdf_obj ** ft)
{
				...
				fz_try(ctx)
				{
					if(pdf_signature_is_signed(ctx, pdf, field)) {
						...
						ui_label("Signature %d: CERT: %s, DIGEST: %s%s", i+1,
						    ...
						    pdf_signature_incremental_change_since_signing(ctx, pdf,
						    field) ? ", Changed since" : "");
...
{
	slfprintf_stderr("mupdf-gl version %s\n", FZ_VERSION);
	slfprintf_stderr("usage: %s [options] document [page]\n", argv0);
	slfprintf_stderr("\t-p -\tpassword\n");
	slfprintf_stderr("\t-r -\tresolution\n");
	slfprintf_stderr("\t-I\tinvert colors\n");
	slfprintf_stderr("\t-W -\tpage width for EPUB layout\n");
	slfprintf_stderr("\t-H -\tpage height for EPUB layout\n");
	slfprintf_stderr("\t-S -\tfont size for EPUB layout\n");
	slfprintf_stderr("\t-U -\tuser style sheet for EPUB layout\n");
	slfprintf_stderr("\t-X\tdisable document styles for EPUB layout\n");
	slfprintf_stderr("\t-J\tdisable javascript in PDF forms\n");
	slfprintf_stderr("\t-A -\tset anti-aliasing level (0-8,9,10)\n");
	slfprintf_stderr("\t-B -\tset black tint color (default: 303030)\n");
	slfprintf_stderr("\t-C -\tset white tint color (default: FFFFF0)\n");
	exit(1);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	char ** argv = fz_argv_from_wargv(argc, (const wchar_t **)(wargv));
