...
static void lock(curlstate * state)
{
...
static void unlock(curlstate * state)
{
	assert(locked == 1);
	locked = 0;
...
{
	struct curlstate * state = (struct curlstate *)state_;
	lock(state);
	if(strncmp((const char *)ptr, "Accept-Ranges: bytes", 20) == 0) {
...
static size_t on_curl_data(void * ptr, size_t size, size_t nmemb, void * state_)
{
	...
	if(state->data_arrived == 0) {
		...
		if(state->content_length == 0) {
			...
			state->map = (unsigned char *)fz_malloc_no_throw(state->ctx, (state->map_length+7)>>3);
			state->buffer = (unsigned char *)fz_malloc_no_throw(state->ctx, len);
			state->buffer_max = len;
			if(state->map == NULL || state->buffer == NULL) {
				unlock(state);
				return 0;
			}
			memzero(state->map, (state->map_length+7)>>3);
			...
			state->buffer = (unsigned char *)fz_malloc_no_throw(state->ctx, state->content_length);
...
{
	struct curlstate * state = (struct curlstate *)stream->state;
	size_t len_read = 0;
...
{
	struct curlstate * state = (struct curlstate *)stm->state;
	stm->wp = stm->rp;
	if(whence == SEEK_END) {
		size_t clen;
		int data_arrived;
...
static void fetcher_thread(curlstate * state)
{
