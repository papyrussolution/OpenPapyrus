...
static void lock(prog_state * state)
{
...
static void unlock(prog_state * state)
{
	assert(locked == 1);
	locked = 0;
...
static fz_stream * fz_open_file_ptr_progressive(fz_context * ctx, FILE * file, int kbps, void (*on_data)(void*, int), void * opaque)
{
	fz_stream * stm;
	prog_state * state;
...
fz_stream * fz_open_file_progressive(fz_context * ctx, const char * name, int kbps, void (*on_data)(void*, int), void * opaque)
{
	FILE * f;
#ifdef _WIN32
	f = (FILE *)fz_fopen_utf8(name, "rb");
#else
	f = fopen(name, "rb");
#endif
	if(!f)
