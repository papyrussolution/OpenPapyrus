...
{
	struct stat info;
	return (stat(path, &info) < 0) ? 0 : info.st_mtime;
}
...
static int convert_to_accel_path(fz_context * ctx, char outname[], char * absname, size_t len)
{
	...
	return (fz_snprintf(outname, len, "%s/%s.accel", tmpdir, absname) >= len) ? 0 : 1;
}
...
{
	char absname[PATH_MAX];
	if(doc && fz_document_supports_accelerator(ctx, doc) && get_accelerator_filename(ctx, absname, sizeof(absname), filename))
...
static int make_fake_doc(pdfapp_t * app)
{
	...
	fz_try(ctx)
	{
		fz_rect mediabox = { 0.0f, 0.0f, static_cast<float>(app->winw), static_cast<float>(app->winh) };
		int i;
...
static void pdfapp_showpage(pdfapp_t * app, int loadpage, int drawpage, int repaint, int transition, int searching)
{
	...
	if(drawpage) {
		char buf2[64];
		size_t len;
		sprintf(buf2, " - %d/%d (%g dpi)", app->pageno, app->pagecount, app->resolution);
...
void pdfapp_inverthit(pdfapp_t * app)
{
	fz_matrix ctm;
	pdfapp_viewctm(&ctm, app);
	for(int i = 0; i < app->hit_count; i++) {
		fz_rect bbox = fz_rect_from_quad(app->hit_bbox[i]);
...
void pdfapp_onkey(pdfapp_t * app, int c, int modifiers)
{
	...
	switch(c) {
		...
		case '<':
		    if(app->layout_em > 6) {
			    fz_bookmark mark =
				fz_make_bookmark(app->ctx, app->doc, fz_location_from_page_number(app->ctx, app->doc, app->pageno));
			    app->layout_em -= 1;
			    fz_layout_document(app->ctx, app->doc, app->layout_w, app->layout_h, app->layout_em);
			    app->pagecount = fz_count_pages(app->ctx, app->doc);
