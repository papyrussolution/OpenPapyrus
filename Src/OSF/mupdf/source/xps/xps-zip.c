...
static fz_document * xps_open_document_with_directory(fz_context * ctx, const char * directory)
{
	xps_document * doc;
...
fz_document * xps_open_document(fz_context * ctx, const char * filename)
{
	...
	const char * p = strstr(filename, "/_rels/.rels");
	if(!p)
...
static void xps_drop_document(fz_context * ctx, fz_document * doc_)
{
...
static int xps_lookup_metadata(fz_context * ctx, fz_document * doc_, const char * key, char * buf, int size)
{
