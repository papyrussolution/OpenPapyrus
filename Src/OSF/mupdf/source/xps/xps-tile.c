...
static void xps_paint_tiling_brush_clipped(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect viewbox, struct closure * c)
{
...
static void xps_paint_tiling_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect viewbox, int tile_mode, struct closure * c)
{
	fz_matrix ttm;
...
void xps_parse_tiling_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    ...
    void (*func)(fz_context * ctx, xps_document*, fz_matrix, fz_rect, char*, xps_resource*, fz_xml*, void*), void * user)
{
	...
	tile_mode = TILE_NONE;
	if(tile_mode_att) {
		if(sstreq(tile_mode_att, "None"))
			tile_mode = TILE_NONE;
		if(sstreq(tile_mode_att, "Tile"))
			tile_mode = TILE_TILE;
		if(sstreq(tile_mode_att, "FlipX"))
			tile_mode = TILE_FLIP_X;
		if(sstreq(tile_mode_att, "FlipY"))
			tile_mode = TILE_FLIP_Y;
		if(sstreq(tile_mode_att, "FlipXY"))
...
static void xps_paint_visual_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
...
void xps_parse_visual_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    char * base_uri, xps_resource * dict, fz_xml * root)
{
	fz_xml * node;
...
void xps_parse_canvas(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area, char * base_uri, xps_resource * dict,
    fz_xml * root)
{
	fz_device * dev = doc->dev;
...
void xps_run_page(fz_context * ctx, fz_page * page_, fz_device * dev, fz_matrix ctm, fz_cookie * cookie)
{
	xps_page * page = (xps_page*)page_;
	xps_document * doc = page->doc;
	fz_matrix page_ctm = fz_pre_scale(ctm, 72.0f / 96.0f, 72.0f / 96.0f);
	doc->cookie = cookie;
	doc->dev = dev;
