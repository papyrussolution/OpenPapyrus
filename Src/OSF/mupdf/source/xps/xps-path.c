...
static char * xps_parse_float_array(fz_context * ctx, xps_document * doc, char * s, int num, float * x)
{
	int k = 0;
	if(s == NULL || *s == 0)
		return NULL;
 ...
 */
static void xps_draw_arc(fz_context * ctx, xps_document * doc, fz_path * path,
...
static void xps_parse_arc_segment(fz_context * ctx, xps_document * doc, fz_path * path, fz_xml * root, int stroking, int * skipped_stroke)
{
	...
	is_stroked = 1;
	if(is_stroked_att && sstreq(is_stroked_att, "false"))
		is_stroked = 0;
	if(!is_stroked)
		*skipped_stroke = 1;
	...
	is_large_arc = sstreq(is_large_arc_att, "true");
	is_clockwise = sstreq(sweep_direction_att, "Clockwise");
...
static void xps_parse_poly_quadratic_bezier_segment(fz_context * ctx,
    xps_document * doc,
    fz_path * path,
    fz_xml * root,
    int stroking,
...
static void xps_parse_poly_bezier_segment(fz_context * ctx,
    xps_document * doc,
    fz_path * path,
    fz_xml * root,
    int stroking,
...
static void xps_parse_poly_line_segment(fz_context * ctx,
    xps_document * doc,
    fz_path * path,
    fz_xml * root,
    int stroking,
...
static void xps_parse_path_figure(fz_context * ctx, xps_document * doc, fz_path * path, fz_xml * root, int stroking)
{
	fz_xml * node;
	...
	if(is_closed_att)
		is_closed = sstreq(is_closed_att, "true");
	if(is_filled_att)
		is_filled = sstreq(is_filled_att, "true");
	if(start_point_att)
...
{
	if(attr) {
		if(sstreq(attr, "Flat")) return 0;
		if(sstreq(attr, "Round")) return 1;
		if(sstreq(attr, "Square")) return 2;
		if(sstreq(attr, "Triangle")) return 3;
	}
	return 0;
}
...
void xps_parse_path(fz_context * ctx, xps_document * doc, fz_matrix ctm, char * base_uri, xps_resource * dict, fz_xml * root)
{
	fz_device * dev = doc->dev;
	...
	if(stroke_att || stroke_tag) {
		...
		stroke->start_cap = (fz_linecap)xps_parse_line_cap(stroke_start_line_cap_att);
		stroke->dash_cap = (fz_linecap)xps_parse_line_cap(stroke_dash_cap_att);
		stroke->end_cap = (fz_linecap)xps_parse_line_cap(stroke_end_line_cap_att);
		stroke->linejoin = FZ_LINEJOIN_MITER_XPS;
		if(stroke_line_join_att) {
			if(sstreq(stroke_line_join_att, "Miter")) stroke->linejoin = FZ_LINEJOIN_MITER_XPS;
			if(sstreq(stroke_line_join_att, "Round")) stroke->linejoin = FZ_LINEJOIN_ROUND;
			if(sstreq(stroke_line_join_att, "Bevel")) stroke->linejoin = FZ_LINEJOIN_BEVEL;
		}
