...
#define REL_DOC_STRUCTURE               "http://schemas.microsoft.com/xps/2005/06/documentstructure"
...
#define REL_START_PART_OXPS             "http://schemas.openxps.org/oxps/v1.0/fixedrepresentation"
#define REL_DOC_STRUCTURE_OXPS          "http://schemas.openxps.org/oxps/v1.0/documentstructure"
 ...
 */
static void xps_add_fixed_document(fz_context * ctx, xps_document * doc, char * name)
{
	xps_fixdoc * fixdoc;
	/* Check for duplicates first */
	for(fixdoc = doc->first_fixdoc; fixdoc; fixdoc = fixdoc->next)
		if(sstreq(fixdoc->name, name))
			return;
...
static void xps_add_fixed_page(fz_context * ctx, xps_document * doc, char * name, int width, int height)
{
	xps_fixpage * page;
	/* Check for duplicates first */
	for(page = doc->first_page; page; page = page->next)
		if(sstreq(page->name, name))
			return;
...
static void xps_add_link_target(fz_context * ctx, xps_document * doc, char * name)
{
...
fz_location xps_lookup_link_target(fz_context * ctx, fz_document * doc_, const char * target_uri, float * xp, float * yp)
{
...
static void xps_drop_link_targets(fz_context * ctx, xps_document * doc)
{
...
static void xps_drop_fixed_pages(fz_context * ctx, xps_document * doc)
{
...
static void xps_drop_fixed_documents(fz_context * ctx, xps_document * doc)
{
...
static void xps_parse_metadata_imp(fz_context * ctx, xps_document * doc, fz_xml * item, xps_fixdoc * fixdoc)
{
	while(item) {
		if(fz_xml_is_tag(item, "Relationship")) {
			...
			if(target && type) {
				...
				if(sstreq(type, REL_START_PART) || sstreq(type, REL_START_PART_OXPS)) {
					fz_free(ctx, doc->start_part);
					doc->start_part = fz_strdup(ctx, tgtbuf);
				}
				if((sstreq(type, REL_DOC_STRUCTURE) || sstreq(type, REL_DOC_STRUCTURE_OXPS)) && fixdoc)
...
static void xps_parse_metadata(fz_context * ctx, xps_document * doc, xps_part * part, xps_fixdoc * fixdoc)
{
	fz_xml_doc * xml;
	char buf[1024];
	char * s;
...
int xps_count_pages(fz_context * ctx, fz_document * doc_, int chapter)
{
	xps_document * doc = (xps_document*)doc_;
	return doc->page_count;
}

static fz_xml_doc * xps_load_fixed_page(fz_context * ctx, xps_document * doc, xps_fixpage * page)
{
	xps_part * part;
	fz_xml_doc * xml = NULL;
	fz_xml * root;
...
static fz_rect xps_bound_page(fz_context * ctx, fz_page * page_)
{
...
static void xps_drop_page_imp(fz_context * ctx, fz_page * page_)
{
	xps_page * page = (xps_page*)page_;
...
fz_page * xps_load_page(fz_context * ctx, fz_document * doc_, int chapter, int number)
{
	xps_document * doc = (xps_document*)doc_;
	xps_page * page = NULL;
	xps_fixpage * fix;
...
static int xps_recognize(fz_context * ctx, const char * magic)
{
...
static const char * xps_extensions[] = { "oxps", "xps", NULL };
static const char * xps_mimetypes[] = { "application/oxps", "application/vnd.ms-xpsdocument", "application/xps", NULL };
