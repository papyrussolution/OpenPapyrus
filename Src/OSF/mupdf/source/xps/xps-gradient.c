...
{
	const struct stop * astop = (const struct stop *)a;
	const struct stop * bstop = (const struct stop *)b;
    ...
    fz_xml * root, int spread)
{
	...
	xrad = smax(0.01f, xrad);
	yrad = smax(0.01f, yrad);
	...
	ma = smax(ma, fceili(hypotf(area.x0 - x0, area.y0 - y0) / xrad));
	ma = smax(ma, fceili(hypotf(area.x1 - x0, area.y0 - y0) / xrad));
	ma = smax(ma, fceili(hypotf(area.x0 - x0, area.y1 - y0) / xrad));
	ma = smax(ma, fceili(hypotf(area.x1 - x0, area.y1 - y0) / xrad));
...
static void xps_draw_linear_gradient(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    struct stop * stops, int count,
    fz_xml * root, int spread)
{
	float x0, y0, x1, y1;
	...
	mi = smin(mi, ffloori(k)); ma = smax(ma, fceili(k));
...
static void xps_parse_gradient_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    ...
    void (*draw)(fz_context * ctx, xps_document *, fz_matrix, fz_rect, struct stop *, int, fz_xml *, int))
{
	...
	if(spread_att) {
		if(sstreq(spread_att, "Pad"))
			spread_method = SPREAD_PAD;
		if(sstreq(spread_att, "Reflect"))
			spread_method = SPREAD_REFLECT;
		if(sstreq(spread_att, "Repeat"))
...
void xps_parse_linear_gradient_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    char * base_uri, xps_resource * dict, fz_xml * root)
{
	xps_parse_gradient_brush(ctx, doc, ctm, area, base_uri, dict, root, xps_draw_linear_gradient);
}

void xps_parse_radial_gradient_brush(fz_context * ctx, xps_document * doc, fz_matrix ctm, fz_rect area,
    char * base_uri, xps_resource * dict, fz_xml * root)
{
