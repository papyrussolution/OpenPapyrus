...
int xps_encode_font_char(fz_context * ctx, fz_font * font, int code)
{
	FT_Face face = (FT_Face)fz_font_ft_face(ctx, font);
...
static fz_font * xps_lookup_font_imp(fz_context * ctx, xps_document * doc, char * name)
{
...
static void xps_insert_font(fz_context * ctx, xps_document * doc, char * name, fz_font * font)
{
 ...
 */
static void xps_deobfuscate_font_resource(fz_context * ctx, xps_document * doc, xps_part * part)
{
	uchar buf[33];
	uchar key[16];
	uchar * data;
...
static void xps_select_best_font_encoding(fz_context * ctx, xps_document * doc, fz_font * font)
{
...
fz_font * xps_lookup_font(fz_context * ctx, xps_document * doc, char * base_uri, char * font_uri, char * style_att)
{
	...
	if(style_att) {
		if(sstreq(style_att, "BoldSimulation"))
			fz_strlcat(fakename, "#Bold", sizeof fakename);
		else if(sstreq(style_att, "ItalicSimulation"))
			fz_strlcat(fakename, "#Italic", sizeof fakename);
		else if(sstreq(style_att, "BoldItalicSimulation"))
...
static char * xps_parse_real_num(char * s, float * number, int * override)
{
	char * tail;
	float v;
	v = fz_strtof(s, &tail);
...
static char * xps_parse_glyph_metrics(char * s, float * advance, float * uofs, float * vofs, int bidi_level)
{
	int override;
	if(*s == ',') {
	...
	fz_try(ctx)
	{
		while((us && un > 0) || (is && *is)) {
			...
			while(glyph_count--) {
				...
				fz_show_glyph(ctx, text, font, tm, glyph_index, char_code, is_sideways, bidi_level, (fz_bidi_direction)dir, FZ_LANG_UNSET);
...
void xps_parse_glyphs(fz_context * ctx, xps_document * doc, fz_matrix ctm,
    char * base_uri, xps_resource * dict, fz_xml * root)
{
	...
	if(is_sideways_att)
		is_sideways = sstreq(is_sideways_att, "true");
	if(bidi_level_att)
	...
	fz_try(ctx) {
		...
		text = xps_parse_glyphs_imp(ctx, doc, ctm, font, font_size, fz_atof(origin_x_att), fz_atof(origin_y_att), is_sideways, bidi_level, indices_att, unicode_att);
