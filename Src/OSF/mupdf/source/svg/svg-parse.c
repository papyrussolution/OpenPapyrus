...
fz_matrix svg_parse_transform(fz_context * ctx, svg_document * doc, const char * str, fz_matrix transform)
{
	...
	while(*str) {
		...
		else if(sstreq(keyword, "translate")) {
			if(nargs == 1)
...
float svg_parse_number_from_style(fz_context * ctx, svg_document * doc, const char * style, const char * att, float number)
{
	if(style) {
		char * end;
