...
static void svg_run_rect(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
...
static void svg_run_circle(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static void svg_run_ellipse(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static void svg_run_line(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static fz_path * svg_parse_polygon_imp(fz_context * ctx, svg_document * doc, fz_xml * node, int doclose)
{
	fz_path * path;
...
static void svg_run_polyline(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static void svg_run_polygon(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
	fz_path * path;
	svg_parse_common(ctx, doc, node, &local_state);
...
static void svg_add_arc(fz_context * ctx, fz_path * path, float size_x, float size_y, float rotation_angle,
    int is_large_arc, int is_clockwise, float point_x, float point_y)
{
		...
		while(*str) {
			...
			switch(cmd)
			{
				...
				case 'a':
				    if(nargs == 7) {
					    p = fz_currentpoint(ctx, path);
					    svg_add_arc(ctx, path, args[0], args[1], args[2], args[3], args[4], args[5] + p.x,
						args[6] + p.y);
					    nargs = 0;
				    }
				    break;
...
static void svg_run_path(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * node, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static void svg_parse_viewport(fz_context * ctx, svg_document * doc, fz_xml * node, svg_state * state)
{
	char * w_att = fz_xml_att(node, "width");
	char * h_att = fz_xml_att(node, "height");
	if(w_att)
...
static void svg_lex_viewbox(const char * s, float * x, float * y, float * w, float * h)
{
...
static int svg_parse_preserve_aspect_ratio(const char * att, int * x, int * y)
{
	*x = *y = 1;
...
static void svg_parse_viewbox(fz_context * ctx, svg_document * doc, fz_xml * node, svg_state * state)
{
...
static void svg_parse_common(fz_context * ctx, svg_document * doc, fz_xml * node, svg_state * state)
{
	...
	if(fill_rule_att) {
		if(sstreq(fill_rule_att, "nonzero"))
			state->fill_rule = 1;
		if(sstreq(fill_rule_att, "evenodd"))
			state->fill_rule = 0;
	}

	if(stroke_att) {
		if(sstreq(stroke_att, "none")) {
	...
	if(stroke_width_att) {
		if(sstreq(stroke_width_att, "inherit"))
			;
		else
	...
	if(stroke_linecap_att) {
		if(sstreq(stroke_linecap_att, "butt"))
			stroke->start_cap = FZ_LINECAP_BUTT;
		if(sstreq(stroke_linecap_att, "round"))
			stroke->start_cap = FZ_LINECAP_ROUND;
		if(sstreq(stroke_linecap_att, "square"))
	...
	if(stroke_linejoin_att) {
		if(sstreq(stroke_linejoin_att, "miter"))
			stroke->linejoin = FZ_LINEJOIN_MITER;
		if(sstreq(stroke_linejoin_att, "round"))
			stroke->linejoin = FZ_LINEJOIN_ROUND;
		if(sstreq(stroke_linejoin_att, "bevel"))
...
static void svg_parse_font_attributes(fz_context * ctx, svg_document * doc, fz_xml * node, svg_state * state, char * buf, int buf_size)
{
	char * style_att = fz_xml_att(node, "style");
	...
	if(font_weight_att) {
		...
		if(sstreq(font_weight_att, "bold")) state->is_bold = 1;
		if(sstreq(font_weight_att, "bolder")) state->is_bold = 1;
	}
	else {
	...
	if(font_style_att) {
		state->is_italic = 0;
		if(sstreq(font_style_att, "italic")) state->is_italic = 1;
		if(sstreq(font_style_att, "oblique")) state->is_italic = 1;
	}
	else {
	...
	if(text_anchor_att) {
		state->text_anchor = 0;
		if(sstreq(text_anchor_att, "middle")) state->text_anchor = 1;
		if(sstreq(text_anchor_att, "end")) state->text_anchor = 2;
	}
	else {
...
static void svg_run_svg(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * root, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
	fz_xml * node;
...
static void svg_run_g(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * root, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
	fz_xml * node;
...
static void svg_run_use_symbol(fz_context * ctx,
    fz_device * dev,
    svg_document * doc,
    fz_xml * use,
    fz_xml * symbol,
    const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
	fz_xml * node;
...
static void svg_run_use(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * root, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
	...
	if(href_att && href_att[0] == '#') {
		fz_xml * linked = (fz_xml *)fz_tree_lookup(ctx, doc->idmap, href_att + 1);
		if(linked) {
...
static void svg_run_image(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * root, const svg_state * inherit_state)
{
	svg_state local_state = *inherit_state;
...
static fz_font * svg_load_font(fz_context * ctx, const svg_state * state)
{
...
static fz_matrix svg_run_text_string(fz_context * ctx, fz_device * dev, fz_matrix trm, const char * s, const svg_state * state)
{
	fz_font * font = NULL;
	fz_text * text = NULL;
...
static void svg_collapse_whitespace(char * start, int is_first, int is_last)
{
...
static fz_matrix svg_run_text(fz_context * ctx, fz_device * dev, svg_document * doc, fz_xml * root, const svg_state * inherit_state,
