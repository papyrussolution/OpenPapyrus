...
static void infousage(void)
{
	slfprintf_stderr("usage: mutool info [options] file.pdf [pages]\n"
...
static void showglobalinfo(fz_context * ctx, globals * glo)
{
	pdf_obj * obj;
...
static void gatherdimensions(fz_context * ctx, globals * glo, int page, pdf_obj * pageref)
{
	fz_rect bbox;
	int j;
	pdf_obj * obj = pdf_dict_get(ctx, pageref, PDF_NAME(MediaBox));
	...
	glo->dim[glo->dims - 1].u.dim.bbox = (fz_rect *)fz_malloc(ctx, sizeof(fz_rect));
	memcpy(glo->dim[glo->dims - 1].u.dim.bbox, &bbox, sizeof(fz_rect));
	return;
}

static void gatherfonts(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i;
...
static void gatherimages(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i, n;
...
static void gatherforms(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i, n;
...
static void gatherpsobjs(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i, n;
...
static void gathershadings(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i, n;
...
static void gatherpatterns(fz_context * ctx, globals * glo, int page, pdf_obj * pageref, pdf_obj * dict)
{
	int i, n;
...
{
	pdf_obj * rsrc;
	pdf_obj * pageref = pdf_lookup_page_obj(ctx, glo->doc, page-1);
	if(!pageref)
		fz_throw(ctx, FZ_ERROR_GENERIC, "cannot retrieve info from page %d", page);
...
static void printinfo(fz_context * ctx, globals * glo, const char * filename, int show, int page)
{
	int i;
	int j;
	fz_output * out = glo->out;
...
static void showinfo(fz_context * ctx, globals * glo, const char * filename, int show, const char * pagelist)
{
...
int pdfinfo_main(int argc, const char * argv[])
{
	...
	int ret;
	fz_context * ctx;
	while((c = fz_getopt(argc, (char * const *)argv, "FISPXMp:")) != -1) {
		switch(c) {
