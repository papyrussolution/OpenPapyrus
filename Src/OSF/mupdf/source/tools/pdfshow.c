...
{
	fprintf(stderr, "usage: mutool show [options] file.pdf ( trailer | xref | pages | grep | outline | js | form | <path> ) *\n"
	    "\t-p -\tpassword\n"
	    "\t-o -\toutput file\n"
...
static void print_outline(fz_outline * outline, int level)
{
	int i;
	while(outline) {
		if(outline->down)
...
int pdfshow_main(int argc, const char * argv[])
{
	...
	while((c = fz_getopt(argc, const_cast<char * const *>(argv), "p:o:beg")) != -1) {
		switch(c) {
