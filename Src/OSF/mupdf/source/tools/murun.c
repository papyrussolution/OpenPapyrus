...
{
	js_newerror(J, fz_caught_message((fz_context *)js_getcontext(J)));
	js_throw(J);
}

FZ_NORETURN static void rethrow_as_fz(js_State * J)
{
	fz_throw((fz_context *)js_getcontext(J), FZ_ERROR_GENERIC, "%s", js_tostring(J, -1));
}

static void * alloc(void * actx, void * ptr, int n)
{
	return fz_realloc_no_throw((fz_context *)actx, ptr, n);
}
...
static void ffi_gc_fz_buffer(js_State * J, void * buf)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_try(ctx)
	fz_drop_buffer(ctx, (fz_buffer *)buf);
	fz_catch(ctx)
	rethrow(J);
}
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_drop_widget(ctx, (pdf_widget *)widget);
}

static void ffi_gc_pdf_annot(js_State * J, void * annot)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_drop_annot(ctx, (pdf_annot *)annot);
}

static void ffi_gc_pdf_document(js_State * J, void * doc)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_drop_document(ctx, (pdf_document *)doc);
}
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_drop_graft_map(ctx, (pdf_graft_map *)map);
}

static fz_document * ffi_todocument(js_State * J, int idx)
{
	if(js_isuserdata(J, idx, "pdf_document"))
		return (fz_document *)js_touserdata(J, idx, "pdf_document");
	return (fz_document *)js_touserdata(J, idx, "fz_document");
}
...
static fz_page * ffi_topage(js_State * J, int idx)
{
	if(js_isuserdata(J, idx, "pdf_page"))
		return (fz_page *)js_touserdata(J, idx, "pdf_page");
	return (fz_page *)js_touserdata(J, idx, "fz_page");
}
...
static struct color ffi_tocolor(js_State * J, int idx)
{
	...
	int n, i;
	fz_context * ctx = (fz_context *)js_getcontext(J);
	c.colorspace = (fz_colorspace *)js_touserdata(J, idx, "fz_colorspace");
	if(c.colorspace) {
...
{
	fz_buffer * buf = (fz_buffer *)buf_;
	int idx;
	unsigned char * data;
	size_t len = fz_buffer_storage((fz_context *)js_getcontext(J), buf, &data);
...
static void ffi_pushbuffer(js_State * J, fz_buffer * buf)
{
	...
	js_newuserdatax(J, "fz_buffer", buf, ffi_buffer_has, ffi_buffer_put, NULL, ffi_gc_fz_buffer);
}
...
static fz_buffer * ffi_tobuffer(js_State * J, int idx)
{
	...
	if(js_isuserdata(J, idx, "fz_buffer"))
		buf = fz_keep_buffer(ctx, (fz_buffer *)js_touserdata(J, idx, "fz_buffer"));
	else {
...
static void js_dev_fill_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
...
static void js_dev_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
    fz_rect scissor)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
...
static void js_dev_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path,
...
static void js_dev_clip_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke,
...
static void js_dev_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
...
static void js_dev_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke,
...
static void js_dev_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_clip_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke,
    fz_matrix ctm, fz_rect scissor)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
...
static void js_dev_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shade, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_fill_image(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
...
static void js_dev_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, fz_rect scissor)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_pop_clip(fz_context * ctx, fz_device * dev)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_begin_mask(fz_context * ctx, fz_device * dev, fz_rect bbox, int luminosity,
...
static void js_dev_end_mask(fz_context * ctx, fz_device * dev)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_begin_group(fz_context * ctx, fz_device * dev, fz_rect bbox,
...
static void js_dev_end_group(fz_context * ctx, fz_device * dev)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static int js_dev_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view,
...
static void js_dev_end_tile(fz_context * ctx, fz_device * dev)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_begin_layer(fz_context * ctx, fz_device * dev, const char * name)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
static void js_dev_end_layer(fz_context * ctx, fz_device * dev)
{
	js_State * J = ((js_device*)dev)->J;
	if(js_try(J))
		rethrow_as_fz(J);
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_device * dev = (fz_device *)js_touserdata(J, 0, "fz_device");
	fz_try(ctx)
...
static void ffi_Device_fillPath(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_device * dev = (fz_device *)js_touserdata(J, 0, "fz_device");
	fz_path * path = (fz_path *)js_touserdata(J, 1, "fz_path");
...
static void ffi_Device_fillText(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_device * dev = (fz_device *)js_touserdata(J, 0, "fz_device");
	fz_text * text = (fz_text *)js_touserdata(J, 1, "fz_text");
...
static void ffi_Device_fillShade(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_device * dev = (fz_device *)js_touserdata(J, 0, "fz_device");
	fz_shade * shade = (fz_shade *)js_touserdata(J, 1, "fz_shade");
	fz_matrix ctm = ffi_tomatrix(J, 2);
...
static void ffi_Device_fillImage(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_device * dev = (fz_device *)js_touserdata(J, 0, "fz_device");
	fz_image * image = (fz_image *)js_touserdata(J, 1, "fz_image");
	fz_matrix ctm = ffi_tomatrix(J, 2);
	float alpha = js_tonumber(J, 3);
	fz_color_params color_params = ffi_tocolorparams(J, 4);
	fz_try(ctx)
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_buffer * buf = (fz_buffer *)js_touserdata(J, 0, "fz_buffer");
...
static void ffi_Buffer_writeBuffer(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_buffer * buf = (fz_buffer *)js_touserdata(J, 0, "fz_buffer");
	fz_buffer * cat = (fz_buffer *)js_touserdata(J, 1, "fz_buffer");
	fz_try(ctx)
...
static void ffi_Document_hasPermission(js_State * J)
{
	...
	fz_try(ctx)
	result = fz_has_permission(ctx, doc, (fz_permission)flag);
	fz_catch(ctx)
	rethrow(J);
...
static void ffi_Page_run(js_State * J)
{
	...
	if(js_isuserdata(J, 1, "fz_device")) {
		device = (fz_device *)js_touserdata(J, 1, "fz_device");
		fz_try(ctx)
		if(no_annots)
...
static void ffi_Page_toPixmap(js_State * J)
{
	...
	fz_colorspace * colorspace = (fz_colorspace *)js_touserdata(J, 2, "fz_colorspace");
...
{
	fz_colorspace * colorspace = (fz_colorspace *)js_touserdata(J, 0, "fz_colorspace");
	fz_context * ctx = (fz_context *)js_getcontext(J);
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_colorspace * colorspace = (fz_colorspace *)js_touserdata(J, 1, "fz_colorspace");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_pixmap * pixmap = (fz_pixmap *)js_touserdata(J, 0, "fz_pixmap");
...
static void ffi_new_Image(js_State * J)
{
	...
	if(js_isuserdata(J, 2, "fz_image"))
		mask = (fz_image *)js_touserdata(J, 2, "fz_image");
	if(js_isuserdata(J, 1, "fz_pixmap")) {
		fz_pixmap * pixmap = (fz_pixmap *)js_touserdata(J, 1, "fz_pixmap");
		fz_try(ctx)
...
{
	fz_image * image = (fz_image *)js_touserdata(J, 0, "fz_image");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_shade * shade = (fz_shade *)js_touserdata(J, 0, "fz_shade");
	fz_matrix ctm = ffi_tomatrix(J, 1);
	fz_rect bounds;
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_font * font = (fz_font *)js_touserdata(J, 0, "fz_font");
...
{
	fz_text * text = (fz_text *)js_touserdata(J, 0, "fz_text");
	char buf[8];
	fz_text_span * span;
	fz_matrix trm;
	int i;
	...
	for(span = text->head; span; span = span->next) {
		...
		if(js_hasproperty(J, 1, "beginSpan")) {
			...
			js_pushstring(J, fz_string_from_text_language(buf, (fz_text_language)span->language));
			js_call(J, 6);
			js_pop(J, 1);
		}
...
static void ffi_Text_showGlyph(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_text * text = (fz_text *)js_touserdata(J, 0, "fz_text");
	fz_font * font = (fz_font *)js_touserdata(J, 1, "fz_font");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_path * path = (fz_path *)js_touserdata(J, 0, "fz_path");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_display_list * list = (fz_display_list *)js_touserdata(J, 0, "fz_display_list");
	fz_device * device = NULL;
	fz_matrix ctm = ffi_tomatrix(J, 2);
...
{
	fz_stext_page * page = (fz_stext_page *)js_touserdata(J, 0, "fz_stext_page");
...
static void ffi_new_DrawDevice(js_State * J)
{
	...
	fz_pixmap * pixmap = (fz_pixmap *)js_touserdata(J, 2, "fz_pixmap");
	fz_device * device = NULL;
	fz_try(ctx)
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	fz_document_writer * wri = (fz_document_writer *)js_touserdata(J, 0, "fz_document_writer");
...
static pdf_obj * ffi_toobj(js_State * J, pdf_document * pdf, int idx)
{
	...
	if(js_isuserdata(J, idx, "pdf_obj"))
		return pdf_keep_obj(ctx, (pdf_obj *)js_touserdata(J, idx, "pdf_obj"));
...
static int ffi_pdf_obj_has(js_State * J, void * obj, const char * key)
{
	...
	if(!strcmp(key, "length")) {
		fz_try(ctx)
		len = pdf_array_len(ctx, (pdf_obj *)obj);
		fz_catch(ctx)
		rethrow(J);
		js_pushnumber(J, len);
		return 1;
	...
	if(is_number(key, &idx)) {
		fz_try(ctx)
		val = pdf_array_get(ctx, (pdf_obj *)obj, idx);
		fz_catch(ctx)
		rethrow(J);
	}
	else {
		fz_try(ctx)
		val = pdf_dict_gets(ctx, (pdf_obj *)obj, key);
		fz_catch(ctx)
		rethrow(J);
	}
	if(val) {
...
static int ffi_pdf_obj_put(js_State * J, void * obj, const char * key)
{
	...
	fz_try(ctx)
	pdf = pdf_get_bound_document(ctx, (pdf_obj *)obj);
	fz_catch(ctx)
	rethrow(J);
	...
	if(is_number(key, &idx)) {
		fz_try(ctx)
		pdf_array_put(ctx, (pdf_obj *)obj, idx, val);
		fz_always(ctx)
		...
		fz_try(ctx)
		pdf_dict_puts(ctx, (pdf_obj *)obj, key, val);
		fz_always(ctx)
		pdf_drop_obj(ctx, val);
		fz_catch(ctx)
		rethrow(J);
...
static int ffi_pdf_obj_delete(js_State * J, void * obj, const char * key)
{
	...
	if(is_number(key, &idx)) {
		fz_try(ctx)
		pdf_array_delete(ctx, (pdf_obj *)obj, idx);
		fz_catch(ctx)
		rethrow(J);
	}
	else {
		fz_try(ctx)
		pdf_dict_dels(ctx, (pdf_obj *)obj, key);
		fz_catch(ctx)
		rethrow(J);
	}
	return 1;
...
static void ffi_pushobj(js_State * J, pdf_obj * obj)
{
	if(obj) {
		...
		js_newuserdatax(J, "pdf_obj", obj, ffi_pdf_obj_has, ffi_pdf_obj_put, ffi_pdf_obj_delete, ffi_gc_pdf_obj);
	}
	else {
		js_pushnull(J);
	}
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_document * pdf = (pdf_document *)js_touserdata(J, 0, "pdf_document");
...
static void ffi_PDFDocument_deleteObject(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_document * pdf = (pdf_document *)js_touserdata(J, 0, "pdf_document");
	pdf_obj * ind = js_isuserdata(J, 1, "pdf_obj") ? (pdf_obj *)js_touserdata(J, 1, "pdf_obj") : NULL;
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_document * dst = (pdf_document *)js_touserdata(J, 0, "pdf_document");
	pdf_obj * obj = (pdf_obj *)js_touserdata(J, 1, "pdf_obj");
	fz_try(ctx)
...
static void ffi_PDFDocument_graftPage(js_State * J)
{
	...
	pdf_document * src = (pdf_document *)js_touserdata(J, 2, "pdf_document");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_graft_map * map = (pdf_graft_map *)js_touserdata(J, 0, "pdf_graft_map");
	pdf_obj * obj = (pdf_obj *)js_touserdata(J, 1, "pdf_obj");
	fz_try(ctx)
...
{
	pdf_obj * obj = (pdf_obj *)js_touserdata(J, 0, "pdf_obj");
	const char * key = js_tostring(J, 1);
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_obj * ref = (pdf_obj *)js_touserdata(J, 0, "pdf_obj");
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_page * page = (pdf_page *)js_touserdata(J, 0, "pdf_page");
...
static void ffi_PDFPage_createAnnotation(js_State * J)
{
	...
	fz_try(ctx)
	{
		...
		annot = pdf_create_annot(ctx, page, (enum pdf_annot_type)subtype);
	}
	fz_catch(ctx)
	rethrow(J);
	js_getregistry(J, "pdf_annot");
...
static void ffi_PDFPage_deleteAnnotation(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_page * page = (pdf_page *)js_touserdata(J, 0, "pdf_page");
	pdf_annot * annot = (pdf_annot *)js_touserdata(J, 1, "pdf_annot");
	fz_try(ctx)
...
static void ffi_PDFAnnotation_getType(js_State * J)
{
	...
	fz_try(ctx)
	{
		...
		subtype = pdf_string_from_annot_type(ctx, (enum pdf_annot_type)type);
	}
	fz_catch(ctx)
	rethrow(J);
...
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_widget * widget = (pdf_widget *)js_touserdata(J, 0, "pdf_widget");
	int type;
	fz_try(ctx)
...
static void ffi_PDFWidget_getOptions(js_State * J)
{
	...
	for(i = 0; i < n; ++i) {
		fz_try(ctx)
		opt = pdf_choice_field_option(ctx, widget->obj, __export, i);
		fz_catch(ctx)
		rethrow(J);
		js_pushstring(J, opt);
	}
...
static void ffi_PDFWidget_checkCertificate(js_State * J)
{
	...
	fz_catch(ctx)
	rethrow(J);
	js_pushstring(J, pdf_signature_error_description((pdf_signature_error)val));
}
...
static void ffi_PDFWidget_checkDigest(js_State * J)
{
	...
	pdf_widget * widget = (pdf_widget *)js_touserdata(J, 0, "pdf_widget");
	pdf_pkcs7_verifier * verifier = NULL;
	pdf_signature_error val = PDF_SIGNATURE_ERROR_OKAY;
	fz_var(verifier);
	fz_try(ctx)
	{
		verifier = pkcs7_openssl_new_verifier(ctx);
...
static void ffi_PDFWidget_sign(js_State * J)
{
	fz_context * ctx = (fz_context *)js_getcontext(J);
	pdf_widget * widget = (pdf_widget *)js_touserdata(J, 0, "pdf_widget");
	pdf_pkcs7_signer * signer = (pdf_pkcs7_signer *)js_touserdata(J, 1, "pdf_pkcs7_signer");
	fz_try(ctx)
