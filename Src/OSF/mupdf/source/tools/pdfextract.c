...
{
	slfprintf_stderr("usage: mutool extract [options] file.pdf [object numbers]\n");
	slfprintf_stderr("\t-p\tpassword\n");
	slfprintf_stderr("\t-r\tconvert images to rgb\n");
	slfprintf_stderr("\t-N\tdo not use ICC color conversions\n");
	exit(1);
}
...
static void writejpeg(const unsigned char * data, size_t len, const char * file)
{
	char buf[1024];
	fz_output * out;
