...
time_t stat_mtime(const char * path)
{
	struct stat info;
...
{
	slfprintf_stderr("mudraw version " FZ_VERSION "\n"
...
{
	if(output_format == OUT_STEXT_XML || output_format == OUT_TRACE || output_format == OUT_BBOX ||
	    output_format == OUT_OCR_STEXT_XML || output_format == OUT_XMLTEXT)
...
{
	if(output_format == OUT_STEXT_XML || output_format == OUT_TRACE || output_format == OUT_OCR_TRACE || output_format == OUT_BBOX ||
...
static void drawband(fz_context * ctx,
    fz_page * page,
    fz_display_list * list,
    fz_matrix ctm,
    fz_rect tbounds,
    ...
    fz_bitmap ** bit)
{
	...
	*bit = NULL;
	...
	fz_try(ctx)
	{
		...
		if(((output_format == OUT_PCL || output_format == OUT_PWG) && out_cs == CS_MONO) || (output_format == OUT_PBM) ||
...
static void dodrawpage(fz_context * ctx, fz_page * page, fz_display_list * list, int pagenum, fz_cookie * cookie,
    int start, int interptime, const char * fname, int bg, fz_separations * seps)
{
	fz_rect mediabox;
	fz_device * dev = NULL;
	fz_var(dev);
	...
	else if(output_format == OUT_TEXT || output_format == OUT_HTML || output_format == OUT_XHTML || output_format == OUT_STEXT_XML ||
	    output_format == OUT_STEXT_JSON ||
	    output_format == OUT_OCR_TEXT || output_format == OUT_OCR_HTML || output_format == OUT_OCR_XHTML ||
	    output_format == OUT_OCR_STEXT_XML || output_format == OUT_OCR_STEXT_JSON) {
		...
		fz_try(ctx)
		{
			...
			if(num_workers > 0) {
				for(band = 0; band < fz_mini(num_workers, bands); band++) {
					...
					memzero(&workers[band].cookie, sizeof(fz_cookie));
			...
			for(band = 0; band < bands; band++) {
				...
				if(num_workers > 0 && band + num_workers < bands) {
					...
					w->ctm = ctm;
					w->tbounds = tbounds;
					memzero(&w->cookie, sizeof(fz_cookie));
					w->running = 1;
#ifndef DISABLE_MUTHREADS
	...
	if(showtime) {
		...
		if(bg) {
			...
			slfprintf_stderr(" %dms (interpretation) %dms (rendering) %dms (total)", interptime, diff, diff + interptime);
		}
		else {
			if(diff < timing.min) {
				timing.min = diff;
...
static void drawpage(fz_context * ctx, fz_document * doc, int pagenum)
{
	...
	if(bgprint.active) {
		...
		if(bgprint.error) {
			...
			if(!quiet || showfeatures || showtime || showmd5)
				slfprintf_stderr("page %s %d%s", filename, pagenum, features);
...
static void * trace_malloc(void * arg, size_t size)
{
	...
	p = (trace_header *)SAlloc::M(size + sizeof(trace_header));
	if(!p)
		return NULL;
	p[0].size = size;
	info->current += size;
...
{
	return oneof5(ch, '\011', '\012', '\014', '\015', '\040');
}
...
static void apply_layer_config(fz_context * ctx, fz_document * doc, const char * lc)
{
	...
	if(*lc == 0 || *lc == 'l') {
		...
		slfprintf_stderr("Layer configs:\n");
		for(config = 0; config < num_configs; config++) {
			slfprintf_stderr(" %s%d:", config < 10 ? " " : "", config);
			pdf_layer_config_info(ctx, pdoc, config, &info);
			if(info.name)
				slfprintf_stderr(" Name=\"%s\"", info.name);
			if(info.creator)
				slfprintf_stderr(" Creator=\"%s\"", info.creator);
			slfprintf_stderr("\n");
		}
		return;
	}
	...
	for(j = 0; j < n; j++) {
		...
		slfprintf_stderr("%s%d: ", j < 10 ? " " : "", j);
		while(ui.depth > 0) {
			ui.depth--;
			slfprintf_stderr("  ");
		}
		if(ui.type == PDF_LAYER_UI_CHECKBOX)
			slfprintf_stderr(" [%c] ", ui.selected ? 'x' : ' ');
		else if(ui.type == PDF_LAYER_UI_RADIOBOX)
			slfprintf_stderr(" (%c) ", ui.selected ? 'x' : ' ');
		if(ui.text)
			slfprintf_stderr("%s", ui.text);
		if(ui.type != PDF_LAYER_UI_LABEL && ui.locked)
			slfprintf_stderr(" <locked>");
		slfprintf_stderr("\n");
	}
#endif
}
...
#endif
{
	...
	fz_var(doc);
	while((c = fz_getopt(argc, (char * const *)argv, "qp:o:F:R:r:w:h:fB:c:e:G:Is:A:DiW:H:S:T:t:U:XLvPl:y:NO:am:")) != -1) {
		switch(c) {
			default: usage(); break;
			...
			case 'O': spots = fz_atof(fz_optarg);
			    ...
			    slfprintf_stderr("Spot rendering/Overprint/Overprint simulation not enabled in this build\n");
			    spots = SPOTS_NONE;
#endif
			    break;
			case 's':
...
#else
			    slfprintf_stderr("Threads not enabled in this build\n");
			    break;
#endif
			case 't':
#ifndef OCR_DISABLED
...
#else
			    slfprintf_stderr("OCR functionality not enabled in this build\n");
			    break;
#endif
			case 'm':
			...
			case 'v': slfprintf_stderr("mudraw version %s\n", FZ_VERSION); return 1;
		}
	}
	if(fz_optind == argc)
		usage();
	if(num_workers > 0) {
		if(uselist == 0) {
			slfprintf_stderr("cannot use multiple threads without using display list\n");
			exit(1);
		}
		if(band_height == 0) {
			slfprintf_stderr("Using multiple threads without banding is pointless\n");
		}
	}

	if(bgprint.active) {
		if(uselist == 0) {
			slfprintf_stderr("cannot bgprint without using display list\n");
			exit(1);
		}
	}
	...
	if(locks == NULL) {
		slfprintf_stderr("mutex initialisation failed\n");
		exit(1);
	}
#endif
	...
	fz_try(ctx)
	{
		...
		if(bgprint.active) {
			...
			if(fail) {
				slfprintf_stderr("bgprint startup failed\n");
				exit(1);
			}
		}
		...
		if(num_workers > 0) {
			...
			if(fail) {
				slfprintf_stderr("worker startup failed\n");
				exit(1);
			}
		}
		...
		if(band_height < 0) {
			slfprintf_stderr("Bandheight must be > 0\n");
			exit(1);
		}
			...
			for(i = 0; i < (int)nelem(suffix_table); i++) {
				if(!strcmp(format, suffix_table[i].suffix+1)) {
					...
					if(spots == SPOTS_FULL && suffix_table[i].spots == 0) {
						slfprintf_stderr("Output format '%s' does not support spot rendering.\nDoing overprint simulation instead.\n", suffix_table[i].suffix+1);
		...
		if(band_height) {
			if(!oneof12(output_format, OUT_PAM, OUT_PGM, OUT_PPM, OUT_PNM, OUT_PNG, OUT_PBM, OUT_PKM, OUT_PCL, OUT_PCLM, OUT_PS, OUT_PSD, OUT_OCR_PDF)) {
				slfprintf_stderr("Banded operation only possible with PxM, PCL, PCLM, PDFOCR, PS, PSD, and PNG outputs\n");
				exit(1);
			}
			if(showmd5) {
				slfprintf_stderr("Banded operation not compatible with MD5\n");
				exit(1);
			}
		}
		{
		...
		switch(out_cs) {
			...
			case CS_ICC:
			    ...
			    fz_catch(ctx)
			    {
				    slfprintf_stderr("Invalid ICC destination color space\n");
				    exit(1);
			    }
			    if(colorspace == NULL) {
				    slfprintf_stderr("Invalid ICC destination color space\n");
			    ...
			    break;
			default:
			    slfprintf_stderr("Unknown colorspace!\n");
			    exit(1);
			    break;
		}
		...
		if(out_cs != CS_ICC)
			...
			if(!okay) {
				slfprintf_stderr("ICC profile uses a colorspace that cannot be used for this format\n");
				exit(1);
			}
		}
		...
		fz_catch(ctx)
		{
			bgprint_flush();
			fz_drop_document(ctx, doc);
			slfprintf_stderr("error: cannot draw '%s'\n", filename);
			errored = 1;
		}
		if(!output_file_per_page)
			file_level_trailers(ctx);
		...
		if(showtime && timing.count > 0) {
			...
			if(files == 1) {
				slfprintf_stderr("total %dms (%dms layout) / %d pages for an average of %dms\n",
				    timing.total, timing.layout, timing.count, timing.total / timing.count);
				if(bgprint.active) {
					slfprintf_stderr("fastest page %d: %dms (interpretation) %dms (rendering) %dms(total)\n",
					    timing.minpage, timing.mininterp, timing.min - timing.mininterp, timing.min);
					slfprintf_stderr("slowest page %d: %dms (interpretation) %dms (rendering) %dms(total)\n",
					    timing.maxpage, timing.maxinterp, timing.max - timing.maxinterp, timing.max);
				}
				else {
					slfprintf_stderr("fastest page %d: %dms\n", timing.minpage, timing.min);
					slfprintf_stderr("slowest page %d: %dms\n", timing.maxpage, timing.max);
				}
			}
			else {
				slfprintf_stderr("total %dms (%dms layout) / %d pages for an average of %dms in %d files\n",
				    timing.total, timing.layout, timing.count, timing.total / timing.count, files);
				slfprintf_stderr("fastest layout: %dms (%s)\n", timing.minlayout, timing.minlayoutfilename);
				slfprintf_stderr("slowest layout: %dms (%s)\n", timing.maxlayout, timing.maxlayoutfilename);
				slfprintf_stderr("fastest page %d: %dms (%s)\n", timing.minpage, timing.min, timing.minfilename);
				slfprintf_stderr("slowest page %d: %dms (%s)\n", timing.maxpage, timing.max, timing.maxfilename);
			}
		}
	...
	{
		if(!errored) {
			slfprintf_stderr("Rendering failed\n");
			errored = 1;
		}
	}
	...
	if(trace_info.mem_limit || trace_info.alloc_limit || showmemory) {
		...
		fz_snprintf(buf, sizeof buf, "Memory use total=%zu peak=%zu current=%zu",
		    trace_info.total, trace_info.peak, trace_info.current);
