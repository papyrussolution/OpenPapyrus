...
{
	fprintf(stderr, "usage: mutool create [-o output.pdf] [-O options] page.txt [page2.txt ...]\n"
...
static void create_page(const char * input)
{
	...
	char * s, * p;
	pdf_obj * page;
	pdf_obj * resources = pdf_new_dict(ctx, doc, 2);
	fz_buffer * contents = fz_new_buffer(ctx, 1024);
	fz_stream * stm = fz_open_file(ctx, input);
