...
{
	fprintf(stderr, "usage: mutool pages [options] file.pdf [pages]\n"
...
static int showpage(fz_context * ctx, pdf_document * doc, fz_output * out, int page)
{
	pdf_obj * pageref;
	int failed = 0;
...
static int showpages(fz_context * ctx, pdf_document * doc, fz_output * out, const char * pagelist)
{
...
int pdfpages_main(int argc, const char * argv[])
{
	...
	int ret;
	fz_context * ctx;
	while((c = fz_getopt(argc, (char * const *)argv, "p:")) != -1) {
		switch(c) {
