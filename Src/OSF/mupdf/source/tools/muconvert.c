...
int muconvert_main(int argc, const char * argv[])
{
	...
	while((c = fz_getopt(argc, (char * const *)argv, "p:A:W:H:S:U:Xo:F:O:")) != -1) {
		switch(c) {
			default: usage(); break;
	...
	fz_catch(ctx)
	{
		slfprintf_stderr("cannot register document handlers: %s\n", fz_caught_message(ctx));
		fz_drop_context(ctx);
		return EXIT_FAILURE;
	}
	...
	fz_catch(ctx)
	{
		slfprintf_stderr("cannot create document: %s\n", fz_caught_message(ctx));
		fz_drop_context(ctx);
		return EXIT_FAILURE;
	}
