...
{
	fprintf(stderr, "Usage: mutool trace [options] file [pages]\n"
	    "\t-p -\tpassword\n"
	    "\n"
...
int mutrace_main(int argc, const char * argv[])
{
	...
	if(!ctx) {
		slfprintf_stderr("cannot create mupdf context\n");
		return EXIT_FAILURE;
	}
	fz_try(ctx)
	{
	...
	fz_catch(ctx)
	{
		slfprintf_stderr("cannot initialize mupdf: %s\n",  fz_caught_message(ctx));
		fz_drop_context(ctx);
		return EXIT_FAILURE;
	}
	...
	fz_catch(ctx)
	{
		slfprintf_stderr("cannot run document: %s\n", fz_caught_message(ctx));
		fz_drop_document(ctx, doc);
		fz_drop_context(ctx);
		return EXIT_FAILURE;
	}
