...
{
	fprintf(stderr, "usage: mutool merge [-o output.pdf] [-O options] input.pdf [pages] [input2.pdf] [pages2] ...\n"
...
int pdfmerge_main(int argc, const char * argv[])
{
	...
	if(!ctx) {
		slfprintf_stderr("error: Cannot initialize MuPDF context.\n");
		exit(1);
	}
	...
	fz_catch(ctx)
	{
		slfprintf_stderr("error: Cannot create destination document.\n");
	...
	while(fz_optind < argc) {
		...
		fz_catch(ctx)
		slfprintf_stderr("error: Cannot merge document '%s'.\n", input);
	}
	...
	if(fz_optind == argc) {
		...
		fz_catch(ctx)
		slfprintf_stderr("error: Cannot save output file: '%s'.\n", output);
	}
