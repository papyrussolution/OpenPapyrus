...
{
	fprintf(stderr, "usage: mutool poster [options] input.pdf [output.pdf]\n"
	    "\t-p -\tpassword\n"
...
static void intersect_box(fz_context * ctx, pdf_document * doc, pdf_obj * page, pdf_obj * box_name, fz_rect mb)
{
...
int pdfposter_main(int argc, const char * argv[])
{
	...
	pdf_document * doc;
	fz_context * ctx;
	while((c = fz_getopt(argc, (char * const *)argv, "x:y:p:")) != -1) {
		switch(c) {
	...
	if(argc - fz_optind > 0 && (strstr(argv[fz_optind], ".pdf") || strstr(argv[fz_optind], ".PDF"))) {
