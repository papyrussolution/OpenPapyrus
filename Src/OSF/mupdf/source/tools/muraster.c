...
{
	slfprintf_stderr("muraster version " FZ_VERSION "\n"
...
static int drawband(fz_context * ctx,
    fz_page * page,
    fz_display_list * list,
    fz_matrix ctm,
    fz_rect tbounds,
	...
	fz_try(ctx)
	{
		...
		for(band = 0; band < bands; band++) {
			...
			if(out) {
				...
				fz_write_band(ctx,
				    render->bander,
				    bit ? bit->stride : pix->stride,
				    draw_height,
				    bit ? bit->samples : pix->samples);
				errors_are_fatal = 0;
			}
...
static int try_render_page(fz_context * ctx,
    int pagenum,
    fz_cookie * cookie,
    int start,
    int interptime,
    ...
    render_details * render)
{
	...
	if(showtime) {
		...
		if(bg) {
			...
			slfprintf_stderr(" %dms (interpretation) %dms (rendering) %dms (total)\n", interptime, diff, diff + interptime);
		}
		else {
			if(diff < timing.min) {
				timing.min = diff;
...
static void initialise_banding(fz_context * ctx, render_details * render, int color)
{
	size_t min_band_mem;
	int bpp, h, w, reps;
...
static void drawpage(fz_context * ctx, fz_document * doc, int pagenum)
{
	...
	if(showtime) {
		slfprintf_stderr("page %s %d", filename, pagenum);
	}
	if(bgprint.active) {
		bgprint.started = 1;
		bgprint.solo = 0;
...
static void finish_bgprint(fz_context * ctx)
{
	int status;
...
static void bgprint_worker(void * arg)
{
	...
	do {
		...
		if(pagenum >= 0) {
			...
			bgprint.status = try_render_page(bgprint.ctx,
				pagenum,
				&cookie,
				start,
				bgprint.interptime,
...
static int read_rotation(const char * arg)
{
	int i;
	if(strcmp(arg, "auto")) {
		return -1;
...
int main(int argc, char ** argv)
{
	...
	while((c = fz_getopt(argc, argv, "p:o:F:R:r:w:h:fB:M:s:A:iW:H:S:T:U:XvP")) != -1) {
		switch(c) {
			...
			case 'P':
			    ...
			    break;
#endif
			case 'v': slfprintf_stderr("muraster version %s\n", FZ_VERSION); return 1;
		}
	}
	...
	if(min_band_height <= 0) {
		slfprintf_stderr("Require a positive minimum band height\n");
		exit(1);
	}
	...
	if(locks == NULL) {
		slfprintf_stderr("cannot initialise mutexes\n");
		exit(1);
	}
#endif
	...
	if(showtime && timing.count > 0) {
		slfprintf_stderr("total %dms / %d pages for an average of %dms\n", timing.total, timing.count, timing.total / timing.count);
