...
{
	fprintf(stderr, "usage: mutool clean [options] input.pdf [output.pdf] [pages]\n"
	    "\t-p -\tpassword\n"
...
int pdfclean_main(int argc, const char * argv[])
{
	...
	int errors = 0;
	fz_context * ctx;
	while((c = fz_getopt(argc, (char * const *)argv, "adfgilp:sczDAE:O:U:P:")) != -1) {
		switch(c) {
			...
			case 'P': opts.permissions = satoi(fz_optarg); break;
