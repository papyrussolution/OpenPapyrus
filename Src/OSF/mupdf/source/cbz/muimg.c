...
typedef struct {
	...
	const char * format;
	int page_count;
	fz_pixmap *(*load_subimage)(fz_context *ctx, const uchar *p, size_t total, int subimage);
} img_document;

static void img_drop_document(fz_context * ctx, fz_document * doc_)
{
	img_document * doc = (img_document*)doc_;
	fz_drop_buffer(ctx, doc->buffer);
}

static int img_count_pages(fz_context * ctx, fz_document * doc_, int chapter)
{
	img_document * doc = (img_document*)doc_;
	return doc->page_count;
}

static fz_rect img_bound_page(fz_context * ctx, fz_page * page_)
{
...
static void img_run_page(fz_context * ctx, fz_page * page_, fz_device * dev, fz_matrix ctm, fz_cookie * cookie)
{
	img_page * page = (img_page*)page_;
	fz_image * image = page->image;
	int xres, yres;
...
static void img_drop_page(fz_context * ctx, fz_page * page_)
{
	img_page * page = (img_page*)page_;
	fz_drop_image(ctx, page->image);
}

static fz_page * img_load_page(fz_context * ctx, fz_document * doc_, int chapter, int number)
{
	img_document * doc = (img_document*)doc_;
...
static int img_lookup_metadata(fz_context * ctx, fz_document * doc_, const char * key, char * buf, int size)
{
	img_document * doc = (img_document*)doc_;
...
static fz_document * img_open_document_with_stream(fz_context * ctx, fz_stream * file)
{
