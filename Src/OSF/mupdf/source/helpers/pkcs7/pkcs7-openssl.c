...
static inline const STACK_OF(PKCS12_SAFEBAG) * PKCS12_SAFEBAG_get0_safes(const PKCS12_SAFEBAG *bag)
{
	return (OBJ_obj2nid(bag->type) == NID_safeContentsBag) ? bag->value.safes : NULL;
}

static inline const PKCS8_PRIV_KEY_INFO * PKCS12_SAFEBAG_get0_p8inf(const PKCS12_SAFEBAG * bag)
{
...
static pdf_signature_error check_digest(fz_context * ctx,
...
static pdf_signature_error check_certificate(fz_context * ctx, pdf_pkcs7_verifier * verifier, unsigned char * sig, size_t sig_len)
{
	PKCS7 * pk7sig = NULL;
	PKCS7 * pk7cert = NULL;
	X509_STORE * st = NULL;
...
static void drop_openssl_verifier(fz_context * ctx, pdf_pkcs7_verifier * verifier)
{
