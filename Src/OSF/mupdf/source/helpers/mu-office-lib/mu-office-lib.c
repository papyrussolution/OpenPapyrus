 ...
 */
	...
	void * Pal_Mem_calloc(unsigned int num, size_t size) { return calloc(num, size); }
	void * Pal_Mem_malloc(size_t size) { return malloc(size); }
	void * Pal_Mem_realloc(void * ptr, size_t size) { return realloc(ptr, size); }
	void Pal_Mem_free(void * address) { free(address); }
#endif
/*
        All MuPDF's allocations are redirected through the
        following functions.
 */
static void * muoffice_malloc(void * arg, size_t size) { return Pal_Mem_malloc(size); }
static void * muoffice_realloc(void * arg, void * old, size_t size) { return Pal_Mem_realloc(old, size); }
static void muoffice_free(void * arg, void * ptr) { Pal_Mem_free(ptr); }
...
static void ensure_doc_loaded(MuOfficeDoc * doc)
{
	if(doc == NULL)
		return;
...
void MuOfficePage_destroy(MuOfficePage * page)
{
	if(page) {
		...
		MuOfficeDoc * doc = page->doc;
		MuOfficePage ** ptr = &doc->pages;
		while(*ptr && *ptr != page)
			ptr = &(*ptr)->next;
		assert(*ptr);
		*ptr = page->next;
...
MuError MuOfficePage_getSize(MuOfficePage * page, float * pWidth, float * pHeight)
{
	...
	/* MuPDF measures in points (72ths of an inch). This API wants 90ths of an inch, so adjust. */
	if(pWidth)
