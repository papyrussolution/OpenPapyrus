...
static void pcl_header(fz_context * ctx, fz_output * out, fz_pcl_options * pcl, int num_copies, int xres, int yres, int w, int h)
{
...
static int line_is_blank(uchar * dst, const uchar * sp, int w)
{
	int zero = 0;
...
static int delta_compression(uchar * curr, uchar * prev, uchar * comp, int ds, int space)
{
	int left = space;
	int x = ds;
...
typedef struct color_pcl_band_writer_s {
	...
	uchar * linebuf;
	uchar compbuf[32768];
	uchar compbuf2[32768];
} color_pcl_band_writer;

static void color_pcl_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
	...
	writer->linebuf = (uchar*)Memento_label(fz_malloc(ctx, w * 3 * 2), "color_pcl_linebuf");
...
static void flush_if_not_room(fz_context * ctx, fz_output * out, const uchar * comp, int * fill, int len)
{
	if(len + *fill >= 32767) {
...
static void color_pcl_compress_column(fz_context * ctx, color_pcl_band_writer * writer, const uchar * sp, int w, int h, int stride)
{
	fz_output * out = writer->super.out;
	int ss = w * 3;
	int seed_valid = 0;
	int fill = 0;
	int y = 0;
	uchar * prev = writer->linebuf + w * 3;
	uchar * curr = writer->linebuf;
	uchar * comp = writer->compbuf;
	uchar * comp2 = writer->compbuf2;
...
static void color_pcl_write_band(fz_context * ctx,
...
static void color_pcl_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
}

static void color_pcl_drop_band_writer(fz_context * ctx, fz_band_writer * writer_)
{
	color_pcl_band_writer * writer = (color_pcl_band_writer*)writer_;
 ...
 */
static int mode2compress(uchar * out, const uchar * in, int in_len)
{
	int x;
	int out_len = 0;
	int run;
 ...
 */
static int mode3compress(uchar * out, const uchar * in, uchar * prev, int in_len)
{
	uchar * compressed = out;
	const uchar * cur = in;
	const uchar * end = in + in_len;
...
static void mono_pcl_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
	...
	writer->prev = (uchar *)fz_calloc(ctx, line_size, sizeof(uchar));
	writer->mode2buf = (uchar *)fz_calloc(ctx, max_mode_2_size, sizeof(uchar));
	writer->mode3buf = (uchar *)fz_calloc(ctx, max_mode_3_size, sizeof(uchar));
...
static void mono_pcl_write_band(fz_context * ctx,
    fz_band_writer * writer_,
    int ss,
    int band_start,
    int band_height,
    const uchar * data)
{
	...
	for(y = 0; y < band_height; y++, data += ss) {
		const uchar * end_data = data + line_size;
...
static void mono_pcl_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
	mono_pcl_band_writer * writer = (mono_pcl_band_writer*)writer_;
	fz_output * out = writer->super.out;
...
static void mono_pcl_drop_band_writer(fz_context * ctx, fz_band_writer * writer_)
{
	mono_pcl_band_writer * writer = (mono_pcl_band_writer*)writer_;
...
static fz_device * pcl_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
	fz_pcl_writer * wri = (fz_pcl_writer*)wri_;
	return fz_new_draw_device_with_options(ctx, &wri->draw, mediabox, &wri->pixmap);
}

static void pcl_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	fz_pcl_writer * wri = (fz_pcl_writer*)wri_;
...
static void pcl_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pcl_writer * wri = (fz_pcl_writer*)wri_;
	fz_close_output(ctx, wri->out);
}

static void pcl_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pcl_writer * wri = (fz_pcl_writer*)wri_;
	fz_drop_pixmap(ctx, wri->pixmap);
...
{
	fz_pcl_writer * wri = fz_new_derived_document_writer(ctx,
		fz_pcl_writer,
		pcl_begin_page,
		pcl_end_page,
		pcl_close_writer,
