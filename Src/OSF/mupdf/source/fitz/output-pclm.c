...
void fz_write_pixmap_as_pclm(fz_context * ctx, fz_output * out, const fz_pixmap * pixmap, const fz_pclm_options * pclm)
{
...
static int new_obj(fz_context * ctx, pclm_band_writer * writer)
{
...
static void pclm_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
	...
	writer->compbuf = NULL;
	writer->stripbuf = (uchar*)Memento_label(fz_malloc(ctx, (size_t)w * sh * n), "pclm_stripbuf");
	writer->complen = fz_deflate_bound(ctx, (size_t)w * sh * n);
	writer->compbuf = (uchar *)Memento_label(fz_malloc(ctx, writer->complen), "pclm_compbuf");
...
static void flush_strip(fz_context * ctx, pclm_band_writer * writer, int fill)
{
	uchar * data = writer->stripbuf;
	fz_output * out = writer->super.out;
	int w = writer->super.w;
	int n = writer->super.n;
...
static void pclm_write_band(fz_context * ctx,
    fz_band_writer * writer_,
    int stride,
    int band_start,
    int band_height,
    const uchar * sp)
...
static void pclm_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
}

static void pclm_drop_band_writer(fz_context * ctx, fz_band_writer * writer_)
{
	pclm_band_writer * writer = (pclm_band_writer*)writer_;
	fz_output * out = writer->super.out;
	int i;
...
fz_band_writer * fz_new_pclm_band_writer(fz_context * ctx, fz_output * out, const fz_pclm_options * options)
{
	...
	if(options)
		...
		memzero(&writer->options, sizeof(writer->options));
...
void fz_save_pixmap_as_pclm(fz_context * ctx, fz_pixmap * pixmap, char * filename, int append, const fz_pclm_options * pclm)
{
...
static fz_device * pclm_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
	fz_pclm_writer * wri = (fz_pclm_writer*)wri_;
	return fz_new_draw_device_with_options(ctx, &wri->draw, mediabox, &wri->pixmap);
}

static void pclm_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	fz_pclm_writer * wri = (fz_pclm_writer*)wri_;
...
static void pclm_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pclm_writer * wri = (fz_pclm_writer*)wri_;
...
static void pclm_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pclm_writer * wri = (fz_pclm_writer*)wri_;
...
{
	fz_pclm_writer * wri = fz_new_derived_document_writer(ctx,
		fz_pclm_writer,
		pclm_begin_page,
		pclm_end_page,
		pclm_close_writer,
