...
fz_matrix fz_shear(float h, float v)
{
	fz_matrix m;
	m.a = 1; m.b = v;
	m.c = h; m.d = 1;
...
fz_matrix fz_pre_shear(fz_matrix m, float h, float v)
{
	float a = m.a;
	float b = m.b;
	m.a += v * m.c;
...
fz_matrix fz_pre_rotate(fz_matrix m, float theta)
{
	while(theta < 0)
		theta += 360;
	while(theta >= 360)
...
fz_matrix fz_invert_matrix(fz_matrix src)
{
	float a = src.a;
...
int fz_try_invert_matrix(fz_matrix * dst, fz_matrix src)
{
...
fz_rect fz_rect_from_irect(fz_irect a)
{
	fz_rect r;
