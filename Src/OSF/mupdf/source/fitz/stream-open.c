...
int fz_file_exists(fz_context * ctx, const char * path)
{
	FILE * file;
#ifdef _WIN32
	file = (FILE*)fz_fopen_utf8(path, "rb");
#else
	file = fopen(path, "rb");
#endif
	if(file)
...
{
	return (fz_stream *)fz_keep_imp(ctx, stm, &stm->refs);
}
...
{
	fz_file_stream * state = (fz_file_stream *)stm->state;
...
{
	fz_file_stream * state = (fz_file_stream *)state_;
...
static fz_stream * fz_open_file_ptr(fz_context * ctx, FILE * file)
{
	fz_stream * stm;
...
{
#ifdef _WIN32
	FILE * file = (FILE*)fz_fopen_utf8(name, "rb");
#else
...
#ifdef _WIN32
fz_stream * fz_open_file_w(fz_context * ctx, const wchar_t * name)
{
...
{
	fz_stream * stm = fz_new_stream(ctx, NULL, next_buffer, NULL);
	stm->seek = seek_buffer;
	stm->rp = (uchar*)data;
	stm->wp = (uchar*)data + len;
	stm->pos = (int64_t)len;
	return stm;
}
