...
static fz_output * start_def(fz_context * ctx, svg_device * sdev)
{
	sdev->def_count++;
	if(sdev->def_count == 2) {
		if(sdev->defs == NULL) {
...
static fz_output * end_def(fz_context * ctx, svg_device * sdev)
{
	if(sdev->def_count > 0)
		sdev->def_count--;
	if(sdev->def_count == 1)
...
static void svg_path_moveto(fz_context * ctx, void * arg, float x, float y)
{
...
static void svg_path_lineto(fz_context * ctx, void * arg, float x, float y)
{
	fz_output * out = (fz_output*)arg;
...
static void svg_path_curveto(fz_context * ctx, void * arg, float x1, float y1, float x2, float y2, float x3, float y3)
{
	fz_output * out = (fz_output*)arg;
...
static void svg_path_close(fz_context * ctx, void * arg)
{
	fz_output * out = (fz_output*)arg;
...
static void svg_dev_path(fz_context * ctx, svg_device * sdev, const fz_path * path)
{
...
static void svg_dev_ctm(fz_context * ctx, svg_device * sdev, fz_matrix ctm)
{
...
static void svg_dev_stroke_state(fz_context * ctx, svg_device * sdev, const fz_stroke_state * stroke_state, fz_matrix ctm)
{
	fz_output * out = sdev->out;
	float exp;
...
static uint svg_hex_color(fz_context * ctx, fz_colorspace * colorspace, const float * color, fz_color_params color_params)
{
	float rgb[3];
	int r, g, b;
...
static void svg_dev_fill_color(fz_context * ctx,
    svg_device * sdev,
...
static void svg_dev_stroke_color(fz_context * ctx,
    svg_device * sdev,
    fz_colorspace * colorspace,
    const float * color,
    float alpha,
...
static int find_first_char(fz_context * ctx, const fz_text_span * span, int i)
{
	for(; i < span->len; ++i)
...
static float svg_cluster_advance(fz_context * ctx, const fz_text_span * span, int i, int end)
{
	int n = 1;
...
static void svg_dev_text_span(fz_context * ctx, svg_device * sdev, fz_matrix ctm, const fz_text_span * span)
{
	fz_output * out = sdev->out;
	char font_family[100];
	int is_bold, is_italic;
	fz_matrix inv_tm, final_tm;
	SPoint2F p;
	int start, end, i;
	...
	if(span->len == 0) {
		...
		float font_size = fz_matrix_expansion(tm);
		...
		if(span->wmode != 0) 
			fz_write_printf(ctx, out, " writing-mode=\"tb\"");
		...
		while(start < span->len) {
			...
			for(i = start + 1; i < end; ++i) {
				const fz_text_item * it = &span->items[i];
				if(it->gid >= 0)
...
static font * svg_dev_text_span_as_paths_defs(fz_context * ctx, fz_device * dev, fz_text_span * span, fz_matrix ctm)
{
	...
	if(font_idx == sdev->num_fonts) {
		...
		if(font_idx == sdev->max_fonts) {
			...
			memzero(&sdev->fonts[font_idx], (newmax - font_idx) * sizeof(font));
	...
	for(i = 0; i < span->len; i++) {
		...
		if(gid >= fnt->max_sentlist) {
			...
			for(int j = fnt->max_sentlist; j <= gid; j++) {
...
static void svg_dev_text_span_as_paths_fill(fz_context * ctx, fz_device * dev, const fz_text_span * span, fz_matrix ctm,
...
static void svg_dev_text_span_as_paths_stroke(fz_context * ctx, fz_device * dev, const fz_text_span * span,
    const fz_stroke_state * stroke, fz_matrix ctm, fz_colorspace * colorspace, const float * color, float alpha, const font * fnt, fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	fz_matrix shift = { 1, 0, 0, 1, 0, 0};
...
static void svg_dev_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm, fz_rect scissor)
{
	svg_device * sdev = (svg_device*)dev;
	int num = sdev->id++;
	fz_output * out = start_def(ctx, sdev);
...
static void svg_dev_clip_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke, fz_matrix ctm, fz_rect scissor)
{
	svg_device * sdev = (svg_device*)dev;
	int num = sdev->id++;
	float white[3] = { 1, 1, 1 };
	fz_rect bounds = fz_bound_path(ctx, path, stroke, ctm);
	fz_output * out = start_def(ctx, sdev);
	fz_write_printf(ctx, out,
	    "<mask id=\"ma%d\" x=\"%g\" y=\"%g\" width=\"%g\" height=\"%g\" maskUnits=\"userSpaceOnUse\" maskContentUnits=\"userSpaceOnUse\">\n",
...
static void svg_dev_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	if(sdev->text_as_text) {
		...
		for(fz_text_span * span = text->head; span; span = span->next) {
			const font * fnt = svg_dev_text_span_as_paths_defs(ctx, dev, span, ctm);
			svg_dev_text_span_as_paths_stroke(ctx, dev, span, stroke, ctm, colorspace, color, alpha, fnt, color_params);
		}
	}
}

static void svg_dev_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	fz_rect bounds;
...
static void svg_dev_clip_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm, fz_rect scissor)
{
	svg_device * sdev = (svg_device*)dev;
	int num = sdev->id++;
...
static void svg_dev_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	fz_text_span * span;
...
static void svg_send_image(fz_context * ctx, svg_device * sdev, fz_image * img, fz_color_params color_params)
{
	fz_output * out = sdev->out;
	int i;
	int id;
...
static void svg_dev_fill_image(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shade, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	...
	fz_try(ctx)
	{
		...
		fz_write_printf(ctx,
		    out,
		    "<image x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" xlink:href=\"",
		    pix->x,
		    pix->y,
		    pix->w,
		    pix->h);
...
static void svg_dev_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out;
...
static void svg_dev_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, fz_rect scissor)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out;
	fz_matrix local_ctm = ctm;
...
static void svg_dev_pop_clip(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_begin_mask(fz_context * ctx,
    fz_device * dev,
    fz_rect bbox,
    int luminosity,
    fz_colorspace * colorspace,
...
static void svg_dev_end_mask(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	int mask = 0;
...
static void svg_dev_begin_group(fz_context * ctx,
    fz_device * dev,
    fz_rect bbox,
    fz_colorspace * cs,
    int isolated,
...
static void svg_dev_end_group(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static int svg_dev_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm,
    int id)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out;
...
static void svg_dev_end_tile(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
	int num, cp = -1;
...
static void svg_dev_begin_layer(fz_context * ctx, fz_device * dev, const char * name)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_end_layer(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_close_device(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	fz_output * out = sdev->out;
...
static void svg_dev_drop_device(fz_context * ctx, fz_device * dev)
{
	svg_device * sdev = (svg_device*)dev;
	int i;
