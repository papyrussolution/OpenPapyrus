...
{
	return ctx ? (fz_style_context *)fz_keep_imp(ctx, ctx->style, &ctx->style->refs) : NULL;
}
...
{
	return ctx ? (fz_tuning_context *)fz_keep_imp(ctx, ctx->tuning, &ctx->tuning->refs) : NULL;
}
...
static void fz_init_error_context(fz_context * ctx)
{
