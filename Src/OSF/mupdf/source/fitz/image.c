...
{
	return (fz_image *)fz_keep_key_storable(ctx, &image->key_storable);
}

fz_image * fz_keep_image_store_key(fz_context * ctx, fz_image * image)
{
	return (fz_image *)fz_keep_key_storable_key(ctx, &image->key_storable);
}
...
static int fz_make_hash_image_key(fz_context * ctx, fz_store_hash * hash, void * key_)
{
...
static void * fz_keep_image_key(fz_context * ctx, void * key_)
{
	fz_image_key * key = (fz_image_key*)key_;
	return fz_keep_imp(ctx, key, &key->refs);
}

static void fz_drop_image_key(fz_context * ctx, void * key_)
{
	fz_image_key * key = (fz_image_key*)key_;
...
static int fz_cmp_image_key(fz_context * ctx, void * k0_, void * k1_)
{
	fz_image_key * k0 = (fz_image_key*)k0_;
	fz_image_key * k1 = (fz_image_key*)k1_;
	return k0->image == k1->image && k0->l2factor == k1->l2factor && k0->rect.x0 == k1->rect.x0 && k0->rect.y0 == k1->rect.y0 &&
	       k0->rect.x1 == k1->rect.x1 && k0->rect.y1 == k1->rect.y1;
}

static void fz_format_image_key(fz_context * ctx, char * s, size_t n, void * key_)
{
	fz_image_key * key = (fz_image_key*)key_;
	fz_snprintf(s, n, "(image %d x %d sf=%d)", key->image->w, key->image->h, key->l2factor);
}

static int fz_needs_reap_image_key(fz_context * ctx, void * key_)
{
	fz_image_key * key = (fz_image_key*)key_;
...
static void fz_mask_color_key(fz_pixmap * pix, int n, const int * colorkey)
{
	uchar * p = pix->samples;
	int w;
	int k, t;
...
static void fz_unblend_masked_tile(fz_context * ctx, fz_pixmap * tile, fz_image * image, const fz_irect * isa)
{
	fz_pixmap * mask;
	uchar * s, * d = tile->samples;
	int n = tile->n;
	int k;
		...
		while(h--) {
			...
			do {
				if(*s == 0)
					...
					for(k = 0; k < image->n; k++)
						d[k] = sclamp(image->colorkey[k] + (d[k] - image->colorkey[k]) * 255 / *s, 0, 255);
				s++;
				d += n;
			}
			while(--w);
...
static int subarea_next(fz_context * ctx, fz_stream * stm, size_t len)
{
	subarea_state * state = (subarea_state *)stm->state;
	size_t n;
	stm->wp = stm->rp = NULL;
	while(state->nskip > 0) {
...
static void subarea_drop(fz_context * ctx, void * state)
{
	fz_free(ctx, state);
}

static fz_stream * subarea_stream(fz_context * ctx, fz_stream * stm, fz_image * image, const fz_irect * subarea, int l2factor)
{
	fz_stream * sstm = NULL;
	subarea_state * state;
	int f = 1<<l2factor;
...
static void subsample_drop(fz_context * ctx, void * state)
{
	fz_free(ctx, state);
}

static int subsample_next(fz_context * ctx, fz_stream * stm, size_t len)
{
...
static fz_stream * subsample_stream(fz_context * ctx, fz_stream * src, int w, int h, int n, int l2extra)
{
	fz_stream * stm;
	l2sub_state * state = (l2sub_state *)fz_malloc(ctx, sizeof(l2sub_state) + w*(size_t)(n<<l2extra));
	state->src = src;
	state->w = w;
	state->h = h;
	state->n = n;
...
fz_pixmap * fz_decomp_image_from_stream(fz_context * ctx,
    fz_stream * stm,
...
void fz_drop_image_imp(fz_context * ctx, fz_storable * image_)
{
...
static void drop_compressed_image(fz_context * ctx, fz_image * image_)
{
...
static void drop_pixmap_image(fz_context * ctx, fz_image * image_)
{
...
static fz_pixmap * compressed_image_get_pixmap(fz_context * ctx, fz_image * image_, fz_irect * subarea, int w, int h, int * l2factor)
{
	fz_compressed_image * image = (fz_compressed_image*)image_;
	int native_l2factor;
	fz_stream * stm;
	...
	switch(image->buffer->params.type)
	{
	    ...
	    {
		    uchar * s = image->buffer->buffer->data;
		    uchar * e = s + image->buffer->buffer->len;
		    uchar * d;
...
static fz_pixmap * pixmap_image_get_pixmap(fz_context * ctx, fz_image * image_, fz_irect * subarea, int w, int h, int * l2factor)
{
	fz_pixmap_image * image = (fz_pixmap_image*)image_;
...
fz_pixmap * fz_get_pixmap_from_image(fz_context * ctx, fz_image * image, const fz_irect * subarea, fz_matrix * ctm, int * dw, int * dh)
{
	fz_pixmap * tile;
	...
	fz_try(ctx)
	{
		...
		existing_tile = (fz_pixmap *)fz_store_item(ctx, keyp, tile, fz_pixmap_size(ctx, tile), &fz_image_store_type);
		if(existing_tile) {
...
fz_image * fz_new_image_of_size(fz_context * ctx, int w, int h, int bpc, fz_colorspace * colorspace,
    ...
    fz_drop_image_fn * drop)
{
	...
	image = (fz_image *)Memento_label(fz_calloc(ctx, 1, size), "fz_image");
...
fz_image * fz_new_image_from_compressed_buffer(fz_context * ctx, int w, int h,
...
static fz_pixmap * display_list_image_get_pixmap(fz_context * ctx, fz_image * image_, fz_irect * subarea, int w, int h, int * l2factor)
{
...
static size_t display_list_image_get_size(fz_context * ctx, fz_image * image_)
{
	fz_display_list_image * image = (fz_display_list_image*)image_;
