...
typedef struct {
	...
	uchar combining;
	uchar bidi_class;
	uchar east_asian_width;
	uchar script;
	uchar linebreak_class;
} UCDRecord;
...
static const ushort * get_decomp_record(uint32_t code)
{
	int index, offset;
	if(code >= 0x110000)
		index = 0;
...
static uint32_t decode_utf16(const ushort ** code_ptr)
{
	const ushort * code = *code_ptr;
	if(code[0] < 0xd800 || code[0] > 0xdc00) {
		...
		*code_ptr += 2;
		return 0x10000 + ((uint32_t)code[1] - 0xdc00) + (((uint32_t)code[0] - 0xd800) << 10);
	}
}
...
{
	int i, len;
	const ushort * rec = get_decomp_record(code);
	len = rec[0] >> 8;
	if(!len)
		return 0;
	rec++;
