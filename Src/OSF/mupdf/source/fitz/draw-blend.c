...
static void fz_hue_rgb(uchar * rr, uchar * rg, uchar * rb, int br, int bg, int bb, int sr, int sg, int sb)
{
	uchar tr, tg, tb;
	...
	do {
		...
		while(haa != 0) { /* Use while, so we can break out */
			...
			for(k = 0; k < first_spot; k++) {
				 ...
				 *
				 *  Ar * Cr = Cb * (Ar - alpha * As) + alpha * As * (1 - Ab) * Cs + alpha * As * Ab *
				 *B(Cb, Cs) ]
				 *
				 * And:
				 *
		...
		if(haa != 0) {
			...
			if(ba == 0 && alpha == 255) {
				...
				if(ra != 0) {
					...
					/* From the notes at the top:
					 *
					 *  Ar * Cr = Cb * (Ar - alpha * As) + alpha * As * (1 - Ab) * Cs + alpha * As *
					 *Ab * B(Cb, Cs) ]
					 *
					 * And:
					 *
