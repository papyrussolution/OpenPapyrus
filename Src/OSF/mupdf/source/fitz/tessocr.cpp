...
static bool load_file(const char* filename, GenericVector<char>* data)
{
	bool result = false;
...
static bool tess_file_reader(const char * fname, GenericVector<char> * out)
{
...
static void set_leptonica_mem(fz_context * ctx)
{
	int die = 0;
...
static void clear_leptonica_mem(fz_context * ctx)
{
	int die = 0;
...
static Pix * ocr_set_image(fz_context * ctx, tesseract::TessBaseAPI * api, fz_pixmap * pix)
{
...
static void ocr_clear_image(fz_context * ctx, Pix * image)
{
...
static bool do_cancel(void * arg, int dummy)
{
	return true;
}

static bool progress_callback(ETEXT_DESC * monitor, int l, int r, int t, int b)
{
