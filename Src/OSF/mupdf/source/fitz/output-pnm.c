 ...
 */
static void pnm_write_header(fz_context * ctx, fz_band_writer * writer, fz_colorspace * cs)
{
...
static void pnm_write_band(fz_context * ctx, fz_band_writer * writer, int stride, int band_start, int band_height, const uchar * p)
{
	fz_output * out = writer->out;
	int w = writer->w;
	int h = writer->h;
...
static void pam_write_header(fz_context * ctx, fz_band_writer * writer, fz_colorspace * cs)
{
	fz_output * out = writer->out;
	int w = writer->w;
	int h = writer->h;
...
static void pam_write_band(fz_context * ctx, fz_band_writer * writer, int stride, int band_start, int band_height, const uchar * sp)
{
	fz_output * out = writer->out;
	int w = writer->w;
	int h = writer->h;
