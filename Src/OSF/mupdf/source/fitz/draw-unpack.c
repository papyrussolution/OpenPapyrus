...
static uchar get1_tab_1[256][8];
static uchar get1_tab_1p[256][16];
static uchar get1_tab_255[256][8];
static uchar get1_tab_255p[256][16];
...
static const uchar mask[9] = { 0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff };
...
static void fz_unpack_mono_line_unscaled(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip)
{
	int w3 = w >> 3;
	int x;
	for(x = 0; x < w3; x++) {
...
static void fz_unpack_mono_line_scaled(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip)
{
	int w3 = w >> 3;
	int x;
	for(x = 0; x < w3; x++) {
...
static void fz_unpack_mono_line_unscaled_with_padding(uchar * dp,
    uchar * sp,
    int w,
    int n,
    int depth,
...
static void fz_unpack_mono_line_scaled_with_padding(uchar * dp,
    uchar * sp,
    int w,
    int n,
    int depth,
...
static void fz_unpack_line(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip)
{
	int len = w * n;
	while(len--)
		*dp++ = *sp++;
...
static void fz_unpack_line_with_padding(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip)
{
	int x, k;
...
static void fz_unpack_any_l2depth(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip)
{
	uchar * p = dp;
	int b = 0;
	int x, k;
...
typedef void (* fz_unpack_line_fn)(uchar * dp, uchar * sp, int w, int n, int depth, int scale, int pad, int skip);

void fz_unpack_tile(fz_context * ctx, fz_pixmap * dst, uchar * src, int n, int depth, size_t stride, int scale)
{
	uchar * sp = src;
...
static int unpack_next(fz_context * ctx, fz_stream * stm, size_t max)
{
...
static void unpack_drop(fz_context * ctx, void * state)
{
	fz_free(ctx, state);
}
...
fz_stream * fz_unpack_stream(fz_context * ctx, fz_stream * src, int depth, int w, int h, int n, int indexed, int pad, int skip)
{
	...
	state = (unpack_state*)fz_malloc(ctx, sizeof(unpack_state) + dst_stride + src_stride);
	state->src = src;
	state->depth = depth;
	state->w = w;
	state->h = h;
