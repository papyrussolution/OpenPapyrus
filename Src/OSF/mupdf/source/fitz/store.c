...
{
	fz_store * store = fz_malloc_struct(ctx, fz_store);
	fz_try(ctx) {
...
/*
        ...
        Drops FZ_LOCK_ALLOC.
 */
static void do_reap(fz_context * ctx)
{
...
static void evict(fz_context * ctx, fz_item * item)
{
	fz_store * store = ctx->store;
	int drop;
...
static size_t ensure_space(fz_context * ctx, size_t tofree)
{
	fz_item * item, * prev;
	size_t count;
	fz_store * store = ctx->store;
...
void * fz_store_item(fz_context * ctx, void * key, void * val_, size_t itemsize, const fz_store_type * type)
{
	fz_item * item = NULL;
	size_t size;
	 ...
	 * the item. */
	item = (fz_item*)Memento_label(fz_malloc_no_throw(ctx, sizeof(fz_item)), "fz_item");
	if(!item)
		return NULL;
	memzero(item, sizeof(fz_item));
	...
	if(use_hash) {
		...
		fz_try(ctx)
		{
			...
			existing = (fz_item *)fz_hash_insert(ctx, store->hash, &hash, item);
		}
		fz_catch(ctx)
		{
...
void * fz_find_item(fz_context * ctx, fz_store_drop_fn * drop, void * key, const fz_store_type * type)
{
	...
	if(use_hash) {
		...
		item = (fz_item *)fz_hash_find(ctx, store->hash, &hash);
	}
	else {
...
static void fz_debug_store_item(fz_context * ctx, void * state, void * key_, int keylen, void * item_)
{
...
static void fz_debug_store_locked(fz_context * ctx, fz_output * out)
{
	fz_item * item, * next;
	char buf[256];
	fz_store * store = ctx->store;
 ...
 */
static int scavenge(fz_context * ctx, size_t tofree)
{
	fz_store * store = ctx->store;
	size_t freed = 0;
	fz_item * item;
...
int fz_shrink_store(fz_context * ctx, uint percent)
{
	int success;
	fz_store * store;
	size_t new_size;
