...
struct info {
	...
	uint image_left, image_top;
	uint image_width, image_height;
	uint image_interlaced;
...
static const uchar * gif_read_subblocks(fz_context * ctx,
    struct info * info,
    const uchar * p,
    const uchar * end,
    fz_buffer * buf)
...
static const uchar * gif_read_header(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < 6)
...
static uint safe_load_u16(const uchar * p)
{
	return p[1] << 8 | p[0];
}

static const uchar * gif_read_lsd(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < 7)
...
static const uchar * gif_read_gct(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < info->gct_entries * 3)
		fz_throw(ctx, FZ_ERROR_GENERIC, "premature end in global color table in gif image");
	info->gct = (uchar*)Memento_label(fz_malloc(ctx, info->gct_entries * 3), "gif_gct");
	memmove(info->gct, p, info->gct_entries * 3);
	return p + info->gct_entries * 3;
}

static const uchar * gif_read_id(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < 10)
...
static const uchar * gif_read_lct(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < info->lct_entries * 3)
		fz_throw(ctx, FZ_ERROR_GENERIC, "premature end in local color table in gif image");
	info->lct = (uchar *)Memento_label(fz_malloc(ctx, info->lct_entries * 3), "gif_lct");
	memmove(info->lct, p, info->lct_entries * 3);

	return p + info->lct_entries * 3;
}

static void gif_read_line(fz_context * ctx, struct info * info, int ct_entries, const uchar * ct, uint y,
    uchar * sp)
{
	uint index = (info->image_top + y) * info->width + info->image_left;
	uchar * samples = fz_pixmap_samples(ctx, info->pix);
...
static const uchar * gif_read_tbid(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
...
static const uchar * gif_read_gce(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < 8)
...
static const uchar * gif_read_ce(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	return gif_read_subblocks(ctx, info, p + 2, end, NULL);
}

static const uchar* gif_read_pte(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
	if(end - p < 15)
...
static const uchar * gif_read_icc(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
#if FZ_ENABLE_ICC
 ...
 */
static const uchar * gif_read_ae(fz_context * ctx, struct info * info, const uchar * p, const uchar * end)
{
...
static void gif_mask_transparency(fz_context * ctx, struct info * info)
{
	uchar * mp = info->mask;
	uchar * dp = fz_pixmap_samples(ctx, info->pix);
	for(uint y = 0; y < info->height; y++)
		for(uint x = 0; x < info->width; x++, mp++, dp += 4)
			if(*mp == 0x00)
				dp[3] = 0;
}

static fz_pixmap * gif_read_image(fz_context * ctx, struct info * info, const uchar * p, size_t total, int only_metadata)
{
	const uchar * end = p + total;
	...
	fz_try(ctx)
	{
		info->mask = (uchar *)fz_calloc(ctx, (size_t)info->width * info->height, 1);
		/* Read optional global color table */
		if(info->has_gct) {
			uchar * bp, * dp = fz_pixmap_samples(ctx, info->pix);
			uint x, y, k;
		...
		while(1) {
			...
			/* Read extension */
				...
				if(p[1] == 0x01 && info->gif89a) {
					...
					/* Graphic control extension applies only to the graphic rendering block
					   following it */
					info->transparent = 0;
...
fz_pixmap * fz_load_gif(fz_context * ctx, const uchar * p, size_t total)
{
	fz_pixmap * image;
	struct info gif;
...
void fz_load_gif_info(fz_context * ctx,
    const uchar * p,
    size_t total,
    int * wp,
    int * hp,
