...
static void read_zip_dir_imp(fz_context * ctx, fz_zip_archive * zip, int64_t start_offset)
{
	...
	fz_try(ctx)
	{
		...
		for(i = 0; i < (int)count; i++) {
			...
			name = (char *)Memento_label(fz_malloc(ctx, namesize + 1), "zip_name");

			n = fz_read(ctx, file, (uchar*)name, namesize);
...
static fz_buffer * read_zip_entry(fz_context * ctx, fz_archive * arch, const char * name)
{
	...
	if(method == 0) {
		...
		fz_try(ctx)
		{
			cbuf = (uchar *)fz_malloc(ctx, ent->csize);
...
{
	const uchar signature[4] = { 'P', 'K', 0x03, 0x04 };
	uchar data[4];
	size_t n;
...
fz_archive * fz_open_zip_archive(fz_context * ctx, const char * filename)
{
	fz_archive * zip = NULL;
	fz_stream * file;
