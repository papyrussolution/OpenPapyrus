...
{
	fz_device * dev = (fz_device *)Memento_label(fz_calloc(ctx, 1, size), "fz_device");
	dev->refs = 1;
	return dev;
}

static void fz_disable_device(fz_context * ctx, fz_device * dev)
{
...
{
	return (fz_device *)fz_keep_imp(ctx, dev, &dev->refs);
}
...
static void pop_clip_stack(fz_context * ctx, fz_device * dev, int type)
{
...
static void pop_push_clip_stack(fz_context * ctx, fz_device * dev, int pop_type, int push_type)
{
...
void fz_fill_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
...
void fz_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	if(dev->stroke_path) {
		fz_try(ctx)
...
void fz_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	if(dev->fill_text) {
		fz_try(ctx)
...
void fz_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	if(dev->stroke_text) {
		fz_try(ctx)
...
void fz_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
