...
void fz_drop_pixmap_imp(fz_context * ctx, fz_storable * pix_)
{
    ...
    uchar * samples)
{
	...
	if(!samples && pix->h > 0 && pix->w > 0) {
		fz_try(ctx)
		{
			...
			pix->samples = (uchar*)Memento_label(fz_malloc(ctx, pix->h * pix->stride), "pixmap_data");
		}
		fz_catch(ctx)
		{
			fz_drop_separations(ctx, pix->seps);
...
fz_pixmap * fz_new_pixmap(fz_context * ctx, fz_colorspace * colorspace, int w, int h, fz_separations * seps, int alpha)
{
	int stride;
	int s = fz_count_active_separations(ctx, seps);
	int n;
...
{
	fz_pixmap * pix =
	    fz_new_pixmap_with_bbox(ctx, old->colorspace, fz_make_irect(old->x, old->y, old->w, old->h), old->seps, old->alpha);
...
fz_irect fz_pixmap_bbox_no_ctx(const fz_pixmap * pix)
{
	fz_irect bbox;
	bbox.x0 = pix->x;
	bbox.y0 = pix->y;
 ...
 */
#ifdef ARCH_ARM
static void clear_cmyka_bitmap_ARM(uint32_t * samples, int c, int value)
__attribute__((naked));
...
void fz_fill_pixmap_with_color(fz_context * ctx, fz_pixmap * pix, fz_colorspace * colorspace, float * color, fz_color_params color_params)
{
	...
	h = pix->h;
	for(y = 0; y < h; ++y) {
		uchar * p = pix->samples + y * pix->stride;
		for(x = 0; x < w; ++x) {
			for(i = 0; i < n; ++i)
				*p++ = colorbv[i];
			for(i = 0; i < s; ++i)
...
void fz_clear_pixmap_rect_with_value(fz_context * ctx, fz_pixmap * dest, int value, fz_irect b)
{
	uchar * destp;
	int x, y, w, k;
	size_t destspan;
...
static inline void invert_luminance(int type, uchar * s)
{
	int r, g, b, y;
...
#ifdef ARCH_ARM
static void fz_subsample_pixmap_ARM(uchar * ptr, int w, int h, int f, int factor,
...
void fz_subsample_pixmap(fz_context * ctx, fz_pixmap * tile, int factor)
{
	...
	tile->samples = (uchar *)fz_realloc(ctx, tile->samples, (size_t)tile->h * tile->w * tile->n);
}
