...
static void encode(uchar *, const uint *, const uint);
static void decode(uint *, const uchar *, const uint);
static void transform(uint state[4], const uchar block[64]);
...
static void encode(uchar * output, const uint * input, const uint len)
{
...
static void decode(uint * output, const uchar * input, const uint len)
{
	for(uint i = 0, j = 0; j < len; i++, j += 4) {
...
void fz_md5_final(fz_md5 * context, uchar digest[16])
{
	uchar bits[8];
	uint index, padlen;
	/* Save number of bits */
