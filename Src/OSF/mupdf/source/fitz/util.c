...
{
	int count = 0;
	fz_stext_page * text = fz_new_stext_page_from_display_list(ctx, list, NULL);
	fz_try(ctx)
...
int fz_search_page(fz_context * ctx, fz_page * page, const char * needle, fz_quad * hit_bbox, int hit_max)
{
	...
	int count = 0;
	fz_stext_page * text = fz_new_stext_page_from_page(ctx, page, &opts);
	fz_try(ctx)
		count = fz_search_stext_page(ctx, text, needle, hit_bbox, hit_max);
	fz_always(ctx)
		fz_drop_stext_page(ctx, text);
...
int fz_search_chapter_page_number(fz_context * ctx, fz_document * doc, int chapter, int number, const char * needle, fz_quad * hit_bbox, int hit_max)
{
	int count = 0;
	fz_page * page = fz_load_chapter_page(ctx, doc, chapter, number);
	fz_try(ctx)
		count = fz_search_page(ctx, page, needle, hit_bbox, hit_max);
	fz_always(ctx)
		fz_drop_page(ctx, page);
...
{
	fz_buffer * buf = NULL;
	fz_stext_page * text = fz_new_stext_page_from_display_list(ctx, list, options);
	fz_try(ctx)
