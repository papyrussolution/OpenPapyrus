...
static void * do_scavenging_malloc(fz_context * ctx, size_t size)
{
	int phase = 0;
...
static void * do_scavenging_realloc(fz_context * ctx, void * p, size_t size)
{
	int phase = 0;
	fz_lock(ctx, FZ_LOCK_ALLOC);
	do {
		void * q = ctx->alloc.realloc(ctx->alloc.user, p, size);
		if(q) {
			fz_unlock(ctx, FZ_LOCK_ALLOC);
			return q;
		}
...
{
	return size ? do_scavenging_malloc(ctx, size) : NULL;
}
...
static void * fz_malloc_default(void * opaque, size_t size)
{
	return SAlloc::M(size);
}

static void * fz_realloc_default(void * opaque, void * old, size_t size)
{
	return SAlloc::R(old, size);
}

static void fz_free_default(void * opaque, void * ptr)
{
	SAlloc::F(ptr);
}
...
static void fz_lock_default(void * user, int lock)
{
}

static void fz_unlock_default(void * user, int lock)
{
}
...
void fz_assert_lock_held(fz_context * ctx, int lock)
{
	...
	if(fz_locks_debug[idx][lock] == 0)
		slfprintf_stderr("Lock %d not held when expected\n", lock);
}
...
void fz_assert_lock_not_held(fz_context * ctx, int lock)
{
	...
	if(idx < 0)
		return;
	if(fz_locks_debug[idx][lock] != 0)
		slfprintf_stderr("Lock %d held when not expected\n", lock);
}
...
void fz_lock_debug_lock(fz_context * ctx, int lock)
{
	...
	if(idx < 0)
		return;
	if(fz_locks_debug[idx][lock] != 0) {
		slfprintf_stderr("Attempt to take lock %d when held already!\n", lock);
	}
	for(i = lock-1; i >= 0; i--) {
		if(fz_locks_debug[idx][i] != 0) {
			slfprintf_stderr("Lock ordering violation: Attempt to take lock %d when %d held already!\n", lock, i);
		}
	}
...
void fz_lock_debug_unlock(fz_context * ctx, int lock)
{
	...
	if(idx < 0)
		return;
	if(fz_locks_debug[idx][lock] == 0) {
		slfprintf_stderr("Attempt to release lock %d when not held!\n", lock);
	}
