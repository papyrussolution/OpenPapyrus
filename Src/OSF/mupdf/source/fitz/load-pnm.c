...
static const uchar * pnm_read_white(fz_context * ctx, const uchar * p, const uchar * e, int single_line)
{
	if(e - p < 1)
...
static const uchar * pnm_read_signature(fz_context * ctx, const uchar * p, const uchar * e, char * signature)
{
	if(e - p < 2)
...
static const uchar * pnm_read_number(fz_context * ctx, const uchar * p, const uchar * e, int * number)
{
	if(e - p < 1)
...
static const uchar * pnm_read_token(fz_context * ctx, const uchar * p, const uchar * e, int * token)
{
	const struct { int len; const char * str; int type; } tokens[] =
	{
		{5, "WIDTH", TOKEN_WIDTH},
...
static int map_color(fz_context * ctx, int color, int inmax, int outmax)
{
	float f = (float)color / inmax;
	return f * outmax;
}

static fz_pixmap * pnm_ascii_read_image(fz_context * ctx,
    struct info * pnm,
    const uchar * p,
    const uchar * e,
    int onlymeta,
    ...
    const uchar ** out)
{
	...
	if((uint)pnm->height > UINT_MAX / pnm->width / fz_colorspace_n(ctx, pnm->cs) / (pnm->bitdepth / 8 + 1))
...
static fz_pixmap * pnm_binary_read_image(fz_context * ctx,
    struct info * pnm,
    const uchar * p,
    const uchar * e,
    int onlymeta,
...
static const uchar * pam_binary_read_header(fz_context * ctx, struct info * pnm, const uchar * p, const uchar * e)
{
...
static fz_pixmap * pam_binary_read_image(fz_context * ctx,
    struct info * pnm,
    const uchar * p,
    const uchar * e,
    int onlymeta,
...
static fz_pixmap * pnm_read_image(fz_context * ctx, struct info * pnm, const uchar * p, size_t total, int onlymeta, int subimage)
{
	const uchar * e = p + total;
	char signature[3] = { 0 };
	fz_pixmap * pix = NULL;
	...
	while(p < e && ((!onlymeta && subimage >= 0) || onlymeta)) {
		...
		if(sstreq(signature, "P1")) {
			pnm->cs = fz_device_gray(ctx);
			pix = pnm_ascii_read_image(ctx, pnm, p, e, subonlymeta, 1, &p);
		}
		else if(sstreq(signature, "P2")) {
			pnm->cs = fz_device_gray(ctx);
			pix = pnm_ascii_read_image(ctx, pnm, p, e, subonlymeta, 0, &p);
		}
		else if(sstreq(signature, "P3")) {
			pnm->cs = fz_device_rgb(ctx);
			pix = pnm_ascii_read_image(ctx, pnm, p, e, subonlymeta, 0, &p);
		}
		else if(sstreq(signature, "P4")) {
			pnm->cs = fz_device_gray(ctx);
			pix = pnm_binary_read_image(ctx, pnm, p, e, subonlymeta, 1, &p);
		}
		else if(sstreq(signature, "P5")) {
			pnm->cs = fz_device_gray(ctx);
			pix = pnm_binary_read_image(ctx, pnm, p, e, subonlymeta, 0, &p);
		}
		else if(sstreq(signature, "P6")) {
			pnm->cs = fz_device_rgb(ctx);
			pix = pnm_binary_read_image(ctx, pnm, p, e, subonlymeta, 0, &p);
		}
		else if(sstreq(signature, "P7"))
			pix = pam_binary_read_image(ctx, pnm, p, e, subonlymeta, &p);
		else
			fz_throw(ctx, FZ_ERROR_GENERIC, "unsupported portable anymap signature (0x%02x, 0x%02x)", signature[0],
			    signature[1]);
...
fz_pixmap * fz_load_pnm(fz_context * ctx, const uchar * p, size_t total)
{
	struct info pnm = { 0 };
	return pnm_read_image(ctx, &pnm, p, total, 0, 0);
}

void fz_load_pnm_info(fz_context * ctx,
    const uchar * p,
    size_t total,
    int * wp,
    int * hp,
...
fz_pixmap * fz_load_pnm_subimage(fz_context * ctx, const uchar * p, size_t total, int subimage)
{
	struct info pnm = { 0 };
	return pnm_read_image(ctx, &pnm, p, total, 0, subimage);
}

int fz_load_pnm_subimage_count(fz_context * ctx, const uchar * p, size_t total)
{
	struct info pnm = { 0 };
