...
static void file_write(fz_context * ctx, void * opaque, const void * buffer, size_t count)
{
	...
	if(count == 0)
		return;
	if(count == 1) {
		int x = putc(((uchar*)buffer)[0], file);
...
static void stdout_write(fz_context * ctx, void * opaque, const void * buffer, size_t count)
{
...
static void stderr_write(fz_context * ctx, void * opaque, const void * buffer, size_t count)
{
...
static void file_seek(fz_context * ctx, void * opaque, int64_t off, int whence)
{
	FILE * file = (FILE *)opaque;
#ifdef _WIN32
...
static int64_t file_tell(fz_context * ctx, void * opaque)
{
	FILE * file = (FILE *)opaque;
#ifdef _WIN32
...
static void file_drop(fz_context * ctx, void * opaque)
{
	FILE * file = (FILE *)opaque;
	int n = fclose(file);
	if(n < 0)
...
static fz_stream * file_as_stream(fz_context * ctx, void * opaque)
{
	FILE * file = (FILE *)opaque;
	fflush(file);
    ...
    fz_output_drop_fn * drop)
{
	...
	fz_try(ctx)
	{
		...
		out->drop = drop;
		if(bufsiz > 0) {
			out->bp = (char *)Memento_label(fz_malloc(ctx, bufsiz), "output_buf");
			out->wp = out->bp;
...
fz_output * fz_new_output_with_path(fz_context * ctx, const char * filename, int append)
{
	...
	file = (FILE *)fz_fopen_utf8(filename, append ? "rb+" : "wb+");
	if(append) {
		if(!file)
			file = (FILE *)fz_fopen_utf8(filename, "wb+");
		else
...
static void buffer_write(fz_context * ctx, void * opaque, const void * data, size_t len)
{
	fz_buffer * buffer = (fz_buffer *)opaque;
	fz_append_data(ctx, buffer, data, len);
}

static void buffer_seek(fz_context * ctx, void * opaque, int64_t off, int whence)
{
	fz_throw(ctx, FZ_ERROR_GENERIC, "cannot seek in buffer: %s", strerror(errno));
}

static int64_t buffer_tell(fz_context * ctx, void * opaque)
{
	fz_buffer * buffer = (fz_buffer *)opaque;
	return (int64_t)buffer->len;
}

static void buffer_drop(fz_context * ctx, void * opaque)
{
	fz_buffer * buffer = (fz_buffer *)opaque;
...
fz_stream * fz_stream_from_output(fz_context * ctx, fz_output * out)
{
...
void fz_truncate_output(fz_context * ctx, fz_output * out)
{
...
static void fz_write_emit(fz_context * ctx, void * out, int c)
{
	fz_write_byte(ctx, (fz_output *)out, c);
}

void fz_write_vprintf(fz_context * ctx, fz_output * out, const char * fmt, va_list args)
{
...
void fz_write_base64(fz_context * ctx, fz_output * out, const uchar * data, size_t size, int newline)
{
	...
	for(i = 0; i + 3 <= size; i += 3) {
		...
		fz_write_byte(ctx, out, p_basis[c>>2]);
		fz_write_byte(ctx, out, p_basis[((c&3)<<4)|(d>>4)]);
		fz_write_byte(ctx, out, p_basis[((d&15)<<2)|(e>>6)]);
		fz_write_byte(ctx, out, p_basis[e&63]);
	}
	if(size - i == 2) {
		int c = data[i];
		int d = data[i+1];
		fz_write_byte(ctx, out, p_basis[c>>2]);
		fz_write_byte(ctx, out, p_basis[((c&3)<<4)|(d>>4)]);
		fz_write_byte(ctx, out, p_basis[((d&15)<<2)]);
		fz_write_byte(ctx, out, '=');
	}
	else if(size - i == 1) {
		int c = data[i];
		fz_write_byte(ctx, out, p_basis[c>>2]);
		fz_write_byte(ctx, out, p_basis[((c&3)<<4)]);
		fz_write_byte(ctx, out, '=');
		fz_write_byte(ctx, out, '=');
	}
}

void fz_write_base64_buffer(fz_context * ctx, fz_output * out, fz_buffer * buf, int newline)
{
	uchar * data;
...
{
	fz_band_writer * writer = (fz_band_writer *)fz_calloc(ctx, size, 1);
	writer->out = out;
	return writer;
}
...
void fz_write_band(fz_context * ctx, fz_band_writer * writer, int stride, int band_height, const uchar * samples)
{
	if(writer == NULL || writer->band == NULL)
		return;
