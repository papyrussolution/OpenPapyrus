...
static void fz_premultiply_row(fz_context * ctx, int n, int c, int w, uchar * s)
{
	int n1 = n-1;
	for(; w > 0; w--) {
		uchar a = s[n1];
...
static void fz_unmultiply_row(fz_context * ctx, int n, int c, int w, uchar * s, const uchar * in)
{
	int a, inva;
	int k;
	int n1 = n-1;
...
static void fz_lcms_log_error(cmsContext id, uint32 error_code, const char * error_text)
{
	fz_context * ctx = (fz_context*)cmsGetContextUserData(id);
	fz_warn(ctx, "lcms: %s.", error_text);
}

static void * fz_lcms_malloc(cmsContext id, uint size)
{
	fz_context * ctx = (fz_context *)cmsGetContextUserData(id);
	return Memento_label(fz_malloc_no_throw(ctx, size), "lcms");
}

static void * fz_lcms_realloc(cmsContext id, void * ptr, uint size)
{
	fz_context * ctx = (fz_context *)cmsGetContextUserData(id);
...
static void fz_lcms_log_error(FILE * fOut, cmsContext id, uint32 error_code, const char * error_text)
{
...
fz_icc_profile * fz_new_icc_profile(fz_context * ctx, uchar * data, size_t size)
{
	GLOINIT
	fz_icc_profile * profile = (fz_icc_profile *)cmsOpenProfileFromMem(GLO data, (uint32)size);
	if(profile == NULL)
...
{
	GLOINIT
	cmsMLU * descMLU = (cmsMLU *)cmsReadTag(GLO profile, cmsSigProfileDescriptionTag);
	name[0] = 0;
	cmsMLUgetASCII(GLO descMLU, "en", "US", name, (uint32)size);
}
...
void fz_icc_transform_pixmap(fz_context * ctx, fz_icc_link * link, const fz_pixmap * src, fz_pixmap * dst, int copy_spots)
{
	...
	uchar * inputpos, * outputpos, * buffer;
	int ss = src->stride;
	int ds = dst->stride;
	int sw = src->w;
	int dw = dst->w;
	...
	if(cmm_num_src != sc || cmm_num_dst != dc || cmm_extras != ssp+sa || sa != da || (copy_spots && ssp != dsp))
		fz_throw(ctx,
		    FZ_ERROR_GENERIC,
		    "bad setup in ICC pixmap transform: src: %d vs %d+%d+%d, dst: %d vs %d+%d+%d",
		    cmm_num_src,
		    sc,
		    ssp,
		    sa,
