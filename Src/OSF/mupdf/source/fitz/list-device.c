...
static void fz_append_display_node(fz_context * ctx,
    fz_device * dev,
    ...
    size_t private_data_len)
{
	 ...
	 * write it. */
	if(rect_for_updates || (rect != NULL &&
	    (writer->rect.x0 != rect->x0 || writer->rect.y0 != rect->y0 || writer->rect.x1 != rect->x1 || writer->rect.y1 != rect->y1))) {
		node.rect = 1;
		rect_off = size;
	...
	if(!color) {
		...
		if(colorspace != writer->colorspace) {
			if(colorspace == fz_device_gray(ctx)) {
				...
				if(i == n)
					color = NULL;
				memzero(writer->color, sizeof(float)*n);
			}
		}
		else {
	...
	if(ctm &&
	    (ctm->a != writer->ctm.a || ctm->b != writer->ctm.b || ctm->c != writer->ctm.c || ctm->d != writer->ctm.d ||
	    ctm->e != writer->ctm.e || ctm->f != writer->ctm.f)) {
		int ctm_flags;
	...
	*node_ptr = node;
	 ...
	 * this case */
	if(path_off) {
		my_path = (fz_path *)(&node_ptr[path_off]);
		(void)fz_pack_path(ctx, (uint8 *)my_path, path_size * sizeof(fz_display_node), path);
	}
...
static int fz_pack_color_params(fz_color_params color_params)
{
	int flags = 0;
...
static void fz_unpack_color_params(fz_color_params * color_params, int flags)
{
...
static void fz_list_fill_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
...
static void fz_list_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke,
...
static void fz_list_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm, fz_rect scissor)
{
	fz_rect rect = fz_bound_path(ctx, path, NULL, ctm);
...
static void fz_list_clip_stroke_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void fz_list_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
...
static void fz_list_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_text * cloned_text = fz_keep_text(ctx, text);
	fz_try(ctx)
...
static void fz_list_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	fz_text * cloned_text = fz_keep_text(ctx, text);
	fz_try(ctx)
	{
...
static void fz_list_clip_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void fz_list_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	fz_text * cloned_text = fz_keep_text(ctx, text);
	fz_try(ctx)
	{
...
static void fz_list_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shade, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
...
static void fz_list_fill_image(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
...
static void fz_list_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_image * image2 = fz_keep_image(ctx, image);
...
static void fz_list_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, fz_rect scissor)
{
	fz_image * image2 = fz_keep_image(ctx, image);
	fz_try(ctx)
	{
...
static void fz_list_begin_mask(fz_context * ctx,
    fz_device * dev,
    fz_rect rect,
    int luminosity,
    fz_colorspace * colorspace,
...
static void fz_list_begin_group(fz_context * ctx,
    fz_device * dev,
    fz_rect rect,
    fz_colorspace * colorspace,
    int isolated,
...
static int fz_list_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm,
    int id)
{
...
static void fz_list_render_flags(fz_context * ctx, fz_device * dev, int set, int clear)
{
	int flags;
...
static void fz_list_set_default_colorspaces(fz_context * ctx, fz_device * dev, fz_default_colorspaces * default_cs)
{
...
static void fz_list_begin_layer(fz_context * ctx, fz_device * dev, const char * layer_name)
{
	fz_append_display_node(
		ctx,
		dev,
...
static void fz_list_end_layer(fz_context * ctx, fz_device * dev)
{
	fz_append_display_node(
		ctx,
		dev,
...
static void fz_list_drop_device(fz_context * ctx, fz_device * dev)
{
	fz_list_device * writer = (fz_list_device*)dev;
	fz_drop_colorspace(ctx, writer->colorspace);
	fz_drop_stroke_state(ctx, writer->stroke);
...
{
	fz_list_device * dev = fz_new_derived_device(ctx, fz_list_device);
	...
	memzero(dev->color, sizeof(float)*FZ_MAX_COLORS);
	dev->top = 0;
	dev->tiled = 0;
	return &dev->super;
}
...
void fz_run_display_list(fz_context * ctx, fz_display_list * list, fz_device * dev, fz_matrix top_ctm, fz_rect scissor, fz_cookie * cookie)
{
...
visible:
		...
		fz_try(ctx)
		{
			switch(n.cmd)
			{
				...
				case FZ_CMD_BEGIN_GROUP:
				    align_node_for_pointer(&node);
				    fz_begin_group(ctx, dev, trans_rect, *(fz_colorspace**)node, (n.flags & ISOLATED) != 0,
					(n.flags & KNOCKOUT) != 0, (n.flags>>2), alpha);
				    break;
				case FZ_CMD_END_GROUP:
