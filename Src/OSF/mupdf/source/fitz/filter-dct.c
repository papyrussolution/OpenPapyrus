...
static void fz_dct_mem_init(struct jpeg_decompress_struct * cinfo, fz_dctd * state) { cinfo->client_data = state; }
...
static void * fz_dct_mem_alloc(j_common_ptr cinfo, size_t size)
{
	fz_dctd * state = JZ_DCT_STATE_FROM_CINFO(cinfo);
	return Memento_label(fz_malloc_no_throw(state->ctx, size), "dct_alloc");
}

static void fz_dct_mem_free(j_common_ptr cinfo, void * object, size_t size)
{
	fz_dctd * state = JZ_DCT_STATE_FROM_CINFO(cinfo);
...
{
	jpeg_cust_mem_data * custmptr = fz_malloc_struct(state->ctx, jpeg_cust_mem_data);
	if(!jpeg_cust_mem_init(custmptr, state, NULL, NULL, NULL, fz_dct_mem_alloc, fz_dct_mem_free, fz_dct_mem_alloc, fz_dct_mem_free, NULL)) {
...
static void fz_dct_mem_term(struct jpeg_decompress_struct * cinfo)
{
...
static void init_source_dct(j_decompress_ptr cinfo) { /* nothing to do */ }
static void term_source_dct(j_decompress_ptr cinfo) { /* nothing to do */ }
...
static int next_dctd(fz_context * ctx, fz_stream * stm, size_t max)
{
	...
	fz_try(ctx) {
		if(!state->init) {
			...
			state->scanline = (uchar*)Memento_label(fz_malloc(ctx, state->stride), "dct_scanline");
...
static void close_dctd(fz_context * ctx, void * state_)
{
