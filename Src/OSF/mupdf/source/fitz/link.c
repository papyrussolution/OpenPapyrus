...
{
	fz_link * link = fz_malloc_struct(ctx, fz_link);
	link->refs = 1;
	link->rect = bbox;
	link->next = NULL;
	link->uri = NULL;
...
{
	return (fz_link *)fz_keep_imp(ctx, link, &link->refs);
}
...
{
	/* Basically, this function returns true, if the URI starts with a valid 'scheme' followed by ':'. */
	...
	while(isasciialnum(*uri) || (*uri == '+') || (*uri == '-') || (*uri == '.'))
		++uri;
	return uri[0] == ':';
}
