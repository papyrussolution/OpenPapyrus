...
static inline fz_bresenham_core init_bresenham_core(int start, int end, int n)
{
	fz_bresenham_core b;
	int delta = end-start;
...
static inline fz_bresenham init_bresenham(int start, int end, int n)
{
	fz_bresenham b;
...
static inline void step(fz_bresenham * b)
{
	step_core(&b->c, b->n);
}
...
static inline void step_core(fz_bresenham_core * b, int n)
{
	b->i += b->di;
	b->f -= b->df;
	if(b->f <= 0) {
...
static inline fz_ipoint_bresenham init_ip_bresenham(fz_ipoint start, fz_ipoint end, int n)
{
...
static inline void step_ip(fz_ipoint_bresenham * b)
{
	step_core(&b->x, b->n);
	step_core(&b->y, b->n);
}

static inline fz_ipoint current_ip(const fz_ipoint_bresenham b)
{
	fz_ipoint ip;
...
static inline fz_ipoint2_bresenham init_ip2_bresenham(fz_ipoint ss, fz_ipoint se, fz_ipoint es, fz_ipoint ee, int n)
{
...
static inline void step_ip2(fz_ipoint2_bresenham * b)
{
	step_core(&b->sx, b->n);
	step_core(&b->sy, b->n);
	step_core(&b->ex, b->n);
...
static inline fz_ipoint start_ip(const fz_ipoint2_bresenham b)
{
	fz_ipoint ip;
...
static inline fz_ipoint end_ip(const fz_ipoint2_bresenham b)
{
	fz_ipoint ip;
...
static void interp_n(uchar * d, const uchar * s0,
    const uchar * s1, int f, int n)
{
	do {
		int a = *s0++;
		int b = *s1++ - a;
...
static void interp2_n(uchar * d, const uchar * s0,
...
static inline void copy_pixel(uchar * d, const fz_pixmap * src, fz_ipoint p)
{
	int u = p.x>>8;
	int v = p.y>>8;
	int fu = p.x & 255;
