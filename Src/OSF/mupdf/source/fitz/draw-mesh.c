...
static void fz_paint_triangle(fz_pixmap * pix, float * v[3], int n, fz_irect bbox)
{
	edge_data e0, e1;
	int top, mid, bot;
	float y, y1;
...
static void prepare_mesh_vertex(fz_context * ctx, void * arg, fz_vertex * v, const float * input)
{
...
static void do_paint_tri(fz_context * ctx, void * arg, fz_vertex * av, fz_vertex * bv, fz_vertex * cv)
{
	struct paint_tri_data * ptd = (struct paint_tri_data *)arg;
	float * vertices[3];
	fz_pixmap * dest;
	...
	fz_try(ctx)
	{
		...
		if(shade->use_function) {
			...
			if(fz_colorspace_is_device_n(ctx, colorspace)) {
				...
				conv = fz_clone_pixmap_area_with_different_seps(ctx,
					temp,
					NULL,
					dest->colorspace,
					dest->seps,
