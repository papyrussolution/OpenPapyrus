...
{
	int gid = FT_Get_Char_Index((FT_Face)face, cid);
	if(gid == 0)
		gid = FT_Get_Char_Index((FT_Face)face, 0xf000 + cid);
	/* some chinese fonts only ship the similarly looking 0x2026 */
	if(gid == 0 && cid == 0x22ef)
		gid = FT_Get_Char_Index((FT_Face)face, 0x2026);
	return gid;
}

int ft_name_index(void * face, const char * name)
{
	int code = FT_Get_Name_Index((FT_Face)face, (char *)name);
	if(code == 0) {
		int unicode = fz_unicode_from_glyph_name(name);
		if(unicode) {
			const char ** dupnames = fz_duplicate_glyph_names_from_unicode(unicode);
			while(*dupnames) {
				code = FT_Get_Name_Index((FT_Face)face, (char *)*dupnames);
				if(code)
					break;
				dupnames++;
			}
			if(code == 0) {
				...
				code = FT_Get_Name_Index((FT_Face)face, buf);
			}
		}
	}
	return code;
...
static fz_font * fz_new_font(fz_context * ctx, const char * name, int use_glyph_bbox, int glyph_count)
{
	fz_font * font;
	int i;
...
{
	return (fz_font *)fz_keep_imp(ctx, font, &font->refs);
}

static void free_resources(fz_context * ctx, fz_font * font)
{
	int i;
	if(font->t3resources) {
...
static fz_font * fz_load_fallback_math_font(fz_context * ctx)
{
	int size;
	if(!ctx->font->math) {
		const uchar * data = fz_lookup_noto_math_font(ctx, &size);
		if(data)
...
static fz_font * fz_load_fallback_music_font(fz_context * ctx)
{
	int size;
	if(!ctx->font->music) {
		const uchar * data = fz_lookup_noto_music_font(ctx, &size);
		if(data)
...
static fz_font * fz_load_fallback_symbol1_font(fz_context * ctx)
{
	int size;
	if(!ctx->font->symbol1) {
		const uchar * data = fz_lookup_noto_symbol1_font(ctx, &size);
		if(data)
...
static fz_font * fz_load_fallback_symbol2_font(fz_context * ctx)
{
	int size;
	if(!ctx->font->symbol2) {
		const uchar * data = fz_lookup_noto_symbol2_font(ctx, &size);
		if(data)
...
static fz_font * fz_load_fallback_emoji_font(fz_context * ctx)
{
	int size;
	if(!ctx->font->emoji) {
		const uchar * data = fz_lookup_noto_emoji_font(ctx, &size);
		if(data)
...
{
	for(const struct ft_error * e = ft_errors; e->str; e++)
		if(e->err == err)
			return e->str;
	return "Unknown error";
}

static void fz_keep_freetype(fz_context * ctx)
{
	int fterr;
	int maj, min, pat;
...
fz_font * fz_new_font_from_memory(fz_context * ctx, const char * name, const uchar * data, int len, int index, int use_glyph_bbox)
{
...
static int find_base14_index(const char * name)
{
	if(sstreq(name, "Courier")) return 0;
	if(sstreq(name, "Courier-Oblique")) return 1;
	if(sstreq(name, "Courier-Bold")) return 2;
	if(sstreq(name, "Courier-BoldOblique")) return 3;
	if(sstreq(name, "Helvetica")) return 4;
	if(sstreq(name, "Helvetica-Oblique")) return 5;
	if(sstreq(name, "Helvetica-Bold")) return 6;
	if(sstreq(name, "Helvetica-BoldOblique")) return 7;
	if(sstreq(name, "Times-Roman")) return 8;
	if(sstreq(name, "Times-Italic")) return 9;
	if(sstreq(name, "Times-Bold")) return 10;
	if(sstreq(name, "Times-BoldItalic")) return 11;
	if(sstreq(name, "Symbol")) return 12;
	if(sstreq(name, "ZapfDingbats")) return 13;
	return -1;
}
...
static fz_matrix * fz_adjust_ft_glyph_width(fz_context * ctx, fz_font * font, int gid, fz_matrix * trm)
{
...
static fz_glyph * glyph_from_ft_bitmap(fz_context * ctx, int left, int top, FT_Bitmap * bitmap)
{
	(void)Memento_label(bitmap->buffer, "ft_bitmap");
	if(bitmap->pixel_mode == FT_PIXEL_MODE_MONO)
		return fz_new_glyph_from_1bpp_data(ctx, left, top - bitmap->rows, bitmap->width, bitmap->rows,
			   bitmap->buffer + (bitmap->rows-1)*bitmap->pitch, -bitmap->pitch);
	else
		return fz_new_glyph_from_8bpp_data(ctx, left, top - bitmap->rows, bitmap->width, bitmap->rows,
			   bitmap->buffer + (bitmap->rows-1)*bitmap->pitch, -bitmap->pitch);
}

static fz_pixmap * pixmap_from_ft_bitmap(fz_context * ctx, int left, int top, FT_Bitmap * bitmap)
{
	(void)Memento_label(bitmap->buffer, "ft_bitmap");
	if(bitmap->pixel_mode == FT_PIXEL_MODE_MONO)
		return fz_new_pixmap_from_1bpp_data(ctx, left, top - bitmap->rows, bitmap->width, bitmap->rows,
			   bitmap->buffer + (bitmap->rows-1)*bitmap->pitch, -bitmap->pitch);
	else
		return fz_new_pixmap_from_8bpp_data(ctx, left, top - bitmap->rows, bitmap->width, bitmap->rows,
			   bitmap->buffer + (bitmap->rows-1)*bitmap->pitch, -bitmap->pitch);
}

/* Takes the freetype lock, and returns with it held */
static FT_GlyphSlot do_ft_render_glyph(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, int aa)
{
	FT_Face face = (FT_Face)font->ft_face;
	FT_Matrix m;
	FT_Vector v;
...
fz_pixmap * fz_render_ft_glyph_pixmap(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, int aa)
{
...
fz_glyph * fz_render_ft_glyph(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, int aa)
{
	FT_GlyphSlot slot = do_ft_render_glyph(ctx, font, gid, trm, aa);
	fz_glyph * glyph = NULL;
...
/* Takes the freetype lock, and returns with it held */
static FT_Glyph do_render_ft_stroked_glyph(fz_context * ctx,
    fz_font * font,
    int gid,
    fz_matrix trm,
    fz_matrix ctm,
...
fz_glyph * fz_render_ft_stroked_glyph(fz_context * ctx,
    fz_font * font,
    int gid,
    fz_matrix trm,
    fz_matrix ctm,
...
static fz_rect * fz_bound_ft_glyph(fz_context * ctx, fz_font * font, int gid)
{
	FT_Face face = (FT_Face)font->ft_face;
	FT_Error fterr;
	FT_BBox cbox;
...
{
	fz_font * font = fz_new_font(ctx, name, 1, 256);
	fz_try(ctx)
	{
		font->t3procs = (fz_buffer **)fz_calloc(ctx, 256, sizeof(fz_buffer*));
		font->t3lists = (fz_display_list **)fz_calloc(ctx, 256, sizeof(fz_display_list*));
		font->t3widths = (float *)fz_calloc(ctx, 256, sizeof(float));
		font->t3flags = (ushort *)fz_calloc(ctx, 256, sizeof(ushort));
	}
	fz_catch(ctx)
	{
...
static void fz_bound_t3_glyph(fz_context * ctx, fz_font * font, int gid)
{
	fz_device * dev;
	fz_display_list * list = font->t3lists[gid];
	if(!list) {
...
void fz_prepare_t3_glyph(fz_context * ctx, fz_font * font, int gid)
{
	fz_device * dev;
	fz_rect d1_rect;
	fz_buffer * contents = font->t3procs[gid];
	if(!contents)
		return;
	...
	dev->flags = FZ_DEVFLAG_FILLCOLOR_UNDEFINED |FZ_DEVFLAG_STROKECOLOR_UNDEFINED|FZ_DEVFLAG_STARTCAP_UNDEFINED |FZ_DEVFLAG_DASHCAP_UNDEFINED |
	    FZ_DEVFLAG_ENDCAP_UNDEFINED |FZ_DEVFLAG_LINEJOIN_UNDEFINED |FZ_DEVFLAG_MITERLIMIT_UNDEFINED |FZ_DEVFLAG_LINEWIDTH_UNDEFINED;
...
void fz_run_t3_glyph(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, fz_device * dev)
{
	fz_display_list * list = font->t3lists[gid];
	if(list) {
		fz_matrix ctm = fz_concat(font->t3matrix, trm);
		fz_run_display_list(ctx, list, dev, ctm, fz_infinite_rect, NULL);
	}
}

fz_pixmap * fz_render_t3_glyph_pixmap(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, fz_colorspace * model, const fz_irect * scissor, int aa)
{
...
fz_glyph * fz_render_t3_glyph(fz_context * ctx, fz_font * font, int gid, fz_matrix trm, fz_colorspace * model, const fz_irect * scissor, int aa)
{
...
void fz_render_t3_glyph_direct(fz_context * ctx, fz_device * dev, fz_font * font, int gid, fz_matrix trm, void * gstate, fz_default_colorspaces * def_cs)
{
	...
	fz_try(ctx)
	{
		...
		font->t3run(ctx, font->t3doc, font->t3resources, (fz_buffer *)contents, dev, ctm, gstate, def_cs);
	}
	fz_always(ctx)
	font->t3procs[gid] = (fz_buffer *)contents;
	fz_catch(ctx)
	fz_rethrow(ctx);
}
...
fz_rect fz_bound_glyph(fz_context * ctx, fz_font * font, int gid, fz_matrix trm)
{
	...
	if(font->bbox_table && gid < font->glyph_count && gid >= 0) {
		...
		rect = font->bbox; /* fall back to font bbox */
	}
...
{
	return font->ft_face ? fz_outline_ft_glyph(ctx, font, gid, ctm) : NULL;
}
...
static float fz_advance_ft_glyph(fz_context * ctx, fz_font * font, int gid, int wmode)
{
	FT_Error fterr;
	FT_Fixed adv = 0;
	int mask;
	...
	fz_lock(ctx, FZ_LOCK_FREETYPE);
	fterr = FT_Get_Advance((FT_Face)font->ft_face, gid, mask, &adv);
	fz_unlock(ctx, FZ_LOCK_FREETYPE);
	if(fterr && fterr != FT_Err_Invalid_Argument) {
		fz_warn(ctx, "FT_Get_Advance(%s,%d): %s", font->name, gid, ft_error_string(fterr));
		if(font->width_table) {
...
static float fz_advance_t3_glyph(fz_context * ctx, fz_font * font, int gid)
{
	return (gid >= 0 && gid <= 255) ? font->t3widths[gid] : 0;
}
...
float fz_advance_glyph(fz_context * ctx, fz_font * font, int gid, int wmode)
{
	if(font->ft_face) {
		if(wmode)
...
int fz_encode_character(fz_context * ctx, fz_font * font, int ucs)
{
	if(font->ft_face) {
		if(ucs >= 0 && ucs < 0x10000) {
			...
			if(!font->encoding_cache[pg]) {
				...
				for(i = 0; i < 256; ++i)
					font->encoding_cache[pg][i] = FT_Get_Char_Index((FT_Face)font->ft_face, (pg << 8) + i);
			}
			return font->encoding_cache[pg][ix];
		}
		return FT_Get_Char_Index((FT_Face)font->ft_face, ucs);
	}
	return ucs;
}
...
int fz_encode_character_sc(fz_context * ctx, fz_font * font, int unicode)
{
	if(font->ft_face) {
		...
		if(cat == UCDN_GENERAL_CATEGORY_LL || cat == UCDN_GENERAL_CATEGORY_LT) {
			...
			if(name) {
				glyph = FT_Get_Name_Index((FT_Face)font->ft_face, (char *)name);
				if(glyph > 0)
					return glyph;
			}
			sprintf(buf, "uni%04X.sc", unicode);
			glyph = FT_Get_Name_Index((FT_Face)font->ft_face, buf);
			if(glyph > 0)
				return glyph;
		}
	}
...
int fz_encode_character_with_fallback(fz_context * ctx, fz_font * user_font, int unicode, int script, int language, fz_font ** out_font)
{
	fz_font * font = 0;
	int gid = fz_encode_character(ctx, user_font, unicode);
	if(gid > 0)
		return *out_font = user_font, gid;
	SETIFZ(script, ucdn_get_script(unicode));
	/* Fix for ideographic/halfwidth/fullwidth punctuation forms. */
	if((unicode >= 0x3000 && unicode <= 0x303F) || (unicode >= 0xFF00 && unicode <= 0xFFEF)) {
		if(!oneof4(script, UCDN_SCRIPT_HANGUL, UCDN_SCRIPT_HIRAGANA, UCDN_SCRIPT_KATAKANA, UCDN_SCRIPT_BOPOMOFO))
...
int fz_font_is_bold(fz_context * ctx, fz_font * font) { return font ? font->flags.is_bold : 0; }
int fz_font_is_italic(fz_context * ctx, fz_font * font) { return font ? font->flags.is_italic : 0; }
int fz_font_is_serif(fz_context * ctx, fz_font * font) { return font ? font->flags.is_serif : 0; }
int fz_font_is_monospaced(fz_context * ctx, fz_font * font) { return font ? font->flags.is_mono : 0; }
const char * fz_font_name(fz_context * ctx, fz_font * font) { return font ? font->name : ""; }
fz_buffer ** fz_font_t3_procs(fz_context * ctx, fz_font * font) { return font ? font->t3procs : NULL; }
fz_rect fz_font_bbox(fz_context * ctx, fz_font * font) { return font->bbox; }
void * fz_font_ft_face(fz_context * ctx, fz_font * font) { return font ? font->ft_face : NULL; }
fz_font_flags_t * fz_font_flags(fz_font * font) { return font ? &font->flags : NULL; }
fz_shaper_data_t * fz_font_shaper_data(fz_context * ctx, fz_font * font) { return font ? &font->shaper_data : NULL; }

void fz_font_digest(fz_context * ctx, fz_font * font, uchar digest[16])
{
	if(!font->buffer)
