...
static inline void big32(uchar * buf, uint v)
{
	buf[0] = (v >> 24) & 0xff;
	buf[1] = (v >> 16) & 0xff;
	buf[2] = (v >> 8) & 0xff;
...
static void putchunk(fz_context * ctx, fz_output * out, const char * tag, uchar * data, size_t size)
{
	uint sum;
	...
	sum = crc32(0, NULL, 0);
	sum = crc32(sum, (uchar *)tag, 4);
	sum = crc32(sum, data, (uint)size);
...
static void png_write_icc(fz_context * ctx, png_band_writer * writer, fz_colorspace * cs)
{
#if FZ_ENABLE_ICC
	if(cs && !(cs->flags & FZ_COLORSPACE_IS_DEVICE) && (cs->flags & FZ_COLORSPACE_IS_ICC) && cs->u.icc.buffer) {
		...
		fz_try(ctx)
		{
			chunk = (uchar*)fz_calloc(ctx, size, 1);
			pos = chunk;
...
static void png_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
	...
	static const uchar pngsig[8] = { 137, 80, 78, 71, 13, 10, 26, 10 };
	uchar head[13];
	int color;
    ...
    const uchar * sp)
{
	...
	if(writer->udata == NULL) {
		...
		writer->udata = (uchar *)Memento_label(fz_malloc(ctx, writer->usize), "png_write_udata");
		writer->cdata = (uchar *)Memento_label(fz_malloc(ctx, writer->csize), "png_write_cdata");
	...
	do {
		...
		if(!finalband) {
			...
			if(err == Z_OK) {
				...
				writer->cdata = (uchar *)Memento_label(fz_realloc(ctx, writer->cdata, writer->csize << 2), "realloc png_write_cdata");
				writer->csize <<= 2;
				continue;
			}
			if(err != Z_STREAM_END)
...
static void png_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
	png_band_writer * writer = (png_band_writer*)(void *)writer_;
	fz_output * out = writer->super.out;
	uchar block[1];
...
static fz_buffer * png_from_pixmap(fz_context * ctx, fz_pixmap * pix, fz_color_params color_params, int drop)
{
	fz_buffer * buf = NULL;
	fz_output * out = NULL;
	fz_pixmap * pix2 = NULL;
...
fz_buffer * fz_new_buffer_from_pixmap_as_png(fz_context * ctx, fz_pixmap * pix, fz_color_params color_params)
{
