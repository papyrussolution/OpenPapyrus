...
{
	fz_separations * sep = fz_malloc_struct(ctx, fz_separations);
	sep->refs = 1;
...
{
	return (fz_separations *)fz_keep_imp(ctx, sep, &sep->refs);
}
...
void fz_set_separation_behavior(fz_context * ctx, fz_separations * sep, int separation, fz_separation_behavior beh)
{
	...
	separation >>= 4;
	old = (fz_separation_behavior)((sep->state[separation]>>shift) & 3);
...
static inline fz_separation_behavior sep_state(const fz_separations * sep, int i)
{
...
fz_separation_behavior fz_separation_current_behavior(fz_context * ctx, const fz_separations * sep, int separation)
{
	...
	return (fz_separation_behavior)beh;
}
...
fz_pixmap * fz_copy_pixmap_area_converting_seps(fz_context * ctx,
    fz_pixmap * src,
    fz_pixmap * dst,
    fz_colorspace * prf,
    fz_color_params color_params,
    fz_default_colorspaces * default_cs)
{
	...
	int sc = sn - ss - sa;
	int dc = dn - ds - da;
	const uchar * sdata = src->samples + sstride * (dst->y - src->y) + (dst->x - src->x) * sn;
	uchar * ddata = dst->samples;
	int x, y, i, j, k, n;
	uchar mapped[FZ_MAX_COLORS];
...
void fz_convert_separation_colors(fz_context * ctx,
...
found_process:
			...
			if(remaining == 0) {
				memzero(remainders, sizeof(float) * n);
				remaining = 1;
			}
...
void fz_separation_equivalent(fz_context * ctx,
    ...
    fz_color_params color_params)
{
	...
	memzero(colors, sizeof(float) * fz_colorspace_n(ctx, seps->cs[i]));
