...
static inline int getcomp(const uchar * line, int x, int bpc)
{
	switch(bpc)
	{
...
static inline void putcomp(uchar * line, int x, int bpc, int value)
{
	int maxval = (1 << bpc) - 1;
...
static const uchar png_signature[8] =
{
...
static void png_predict(uchar * samples, uint width, uint height, uint n, uint depth)
{
	uint stride = (width * n * depth + 7) / 8;
	uint bpp = (n * depth + 7) / 8;
	uint i, row;

	for(row = 0; row < height; row++) {
		uchar * src = samples + (uint)((stride + 1) * row);
		uchar * dst = samples + (uint)(stride * row);
...
static const uint adam7_ix[7] = { 0, 4, 0, 2, 0, 1, 0 };
static const uint adam7_dx[7] = { 8, 8, 4, 4, 2, 2, 1 };
static const uint adam7_iy[7] = { 0, 0, 4, 0, 2, 0, 1 };
static const uint adam7_dy[7] = { 8, 8, 8, 4, 4, 2, 2 };

static void png_deinterlace_passes(fz_context * ctx, struct info * info, uint * w, uint * h, uint * ofs)
{
...
static void png_deinterlace(fz_context * ctx, struct info * info, uint * passw, uint * passh, uint * passofs)
{
	uint n = info->n;
	...
	output = (uchar*)Memento_label(fz_malloc(ctx, info->height * stride), "png_deinterlace");

	for(p = 0; p < 7; p++) {
		uchar * sp = info->samples + (passofs[p]);
		uint w = passw[p];
		uint h = passh[p];
...
static void png_read_ihdr(fz_context * ctx, struct info * info, const uchar * p, uint size)
{
...
static void png_read_plte(fz_context * ctx, struct info * info, const uchar * p, uint size)
{
	int n = size / 3;
	int i;
...
static void png_read_trns(fz_context * ctx, struct info * info, const uchar * p, uint size)
{
	uint i;
...
static void png_read_icc(fz_context * ctx, struct info * info, const uchar * p, uint size)
{
#if FZ_ENABLE_ICC
...
static void png_read_phys(fz_context * ctx, struct info * info, const uchar * p, uint size)
{
	if(size != 9)
...
static void png_read_image(fz_context * ctx, struct info * info, const uchar * p, size_t total, int only_metadata)
{
	...
	if(!only_metadata) {
		...
		info->samples = (uchar *)Memento_label(fz_malloc(ctx, info->size), "png_samples");
...
static fz_pixmap * png_expand_palette(fz_context * ctx, struct info * info, fz_pixmap * src)
{
	fz_pixmap * dst = fz_new_pixmap(ctx, info->cs, src->w, src->h, NULL, info->transparency);
	uchar * sp = src->samples;
...
static void png_mask_transparency(struct info * info, fz_pixmap * dst)
{
	uint stride = (info->width * info->n * info->depth + 7) / 8;
	uint depth = info->depth;
	uint n = info->n;
	uint x, y, k, t;

	for(y = 0; y < info->height; y++) {
		uchar * sp = info->samples + (uint)(y * stride);
		uchar * dp = dst->samples + (uint)(y * dst->stride);
...
fz_pixmap * fz_load_png(fz_context * ctx, const uchar * p, size_t total)
{
