...
#include <..\OSF\LIBJPEG\jpeglib.h>
	...
	static void * fz_jpg_mem_alloc(j_common_ptr cinfo, size_t size)
	{
		fz_context * ctx = JZ_CTX_FROM_CINFO(cinfo);
		return fz_malloc_no_throw(ctx, size);
	}

	static void fz_jpg_mem_free(j_common_ptr cinfo, void * object, size_t size)
	{
		fz_context * ctx = JZ_CTX_FROM_CINFO(cinfo);
		fz_free(ctx, object);
	}
	...
	static void fz_jpg_mem_init(j_common_ptr cinfo, fz_context * ctx)
	{
		jpeg_cust_mem_data * custmptr = fz_malloc_struct(ctx, jpeg_cust_mem_data);
		if(!jpeg_cust_mem_init(custmptr, ctx, NULL, NULL, NULL, fz_jpg_mem_alloc, fz_jpg_mem_free, fz_jpg_mem_alloc, fz_jpg_mem_free, NULL)) {
	...
	static void fz_jpg_mem_term(j_common_ptr cinfo)
	{
...
{
	static uchar eoi[2] = { 0xFF, JPEG_EOI };
...
static inline int read_value(const uchar * data, int bytes, int is_big_endian)
{
	int value = 0;
	if(!is_big_endian)
		data += bytes;
...
static fz_colorspace * extract_icc_profile(fz_context * ctx, jpeg_saved_marker_ptr init_marker,
    int output_components, fz_colorspace * colorspace)
{
#if FZ_ENABLE_ICC
...
static int extract_exif_resolution(jpeg_saved_marker_ptr marker, int * xres, int * yres)
{
	...
	for(offset += 2; ifd_len > 0 && offset + 12 < marker->data_length; ifd_len--, offset += 12) {
		...
		uint value_off = read_value(data + offset + 8, 4, is_big_endian) + 6;
		switch(tag) {
			case 0x11A:
...
fz_pixmap * fz_load_jpeg(fz_context * ctx, const uchar * rbuf, size_t rlen)
{
	...
	fz_try(ctx)
	{
		...
		row[0] = (uchar*)fz_malloc(ctx, (size_t)cinfo.output_components * cinfo.output_width);
		dp = image->samples;
...
void fz_load_jpeg_info(fz_context * ctx, const uchar * rbuf, size_t rlen, int * xp,
    int * yp, int * xresp, int * yresp, fz_colorspace ** cspacep)
{
	struct jpeg_decompress_struct cinfo;
	struct jpeg_error_mgr err;
	struct jpeg_source_mgr src;
