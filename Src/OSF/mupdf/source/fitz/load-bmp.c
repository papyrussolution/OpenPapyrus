...
static const uchar web_palette[] = {
...
static const uchar vga_palette[] = {
...
static const uchar gray_palette[] = {
	0x00, 0x00, 0x00, 0x54, 0x54, 0x54,
	0xA8, 0xA8, 0xA8, 0xFF, 0xFF, 0xFF,
};

static const uchar bw_palette[] = {
...
#define is_win_bmp(info) ((info)->version == 12 || (info)->version == 40 || (info)->version == 52 || (info)->version == 56 || \
	(info)->version == 108 || (info)->version == 124)

#define is_valid_win_compression(info) (is_win_bmp(info) && \
	((info)->compression == _BI_NONE || (info)->compression == _BI_RLE8 || (info)->compression == _BI_RLE4 || \
	(info)->compression == _BI_BITFIELDS || (info)->compression == _BI_JPEG || (info)->compression == _BI_PNG || \
	(info)->compression == _BI_ALPHABITS || (info)->compression == _BI_RLE24))
#define is_valid_os2_compression(info) (is_os2_bmp(info) && \
	((info)->compression == _BI_NONE || (info)->compression == _BI_RLE8 || (info)->compression == _BI_RLE4 || \
	(info)->compression == _BI_HUFFMAN1D || (info)->compression == _BI_RLE24))
#define is_valid_compression(info) (is_valid_win_compression(info) || is_valid_os2_compression(info))

#define is_valid_rgb_bitcount(info) ((info)->compression == _BI_NONE && \
	((info)->bitcount == 1 || (info)->bitcount == 2 || (info)->bitcount == 4 || (info)->bitcount == 8 || (info)->bitcount == 16 || \
	(info)->bitcount == 24 || (info)->bitcount == 32))
#define is_valid_rle8_bitcount(info) ((info)->compression == _BI_RLE8 && (info)->bitcount == 8)
#define is_valid_rle4_bitcount(info) ((info)->compression == _BI_RLE4 && (info)->bitcount == 4)
#define is_valid_bitfields_bitcount(info) (is_win_bmp(info) && (info)->compression == _BI_BITFIELDS && \
	((info)->bitcount == 16 || (info)->bitcount == 32))
#define is_valid_jpeg_bitcount(info) (is_win_bmp(info) && (info)->compression == _BI_JPEG && (info)->bitcount == 0)
#define is_valid_png_bitcount(info) (is_win_bmp(info) && (info)->compression == _BI_PNG && (info)->bitcount == 0)
#define is_valid_alphabits_bitcount(info) (is_win_bmp(info) && (info)->compression == _BI_ALPHABITS && \
	((info)->bitcount == 16 || (info)->bitcount == 32))
#define is_valid_rle24_bitcount(info) (is_os2_bmp(info) && (info)->compression == _BI_RLE24 && (info)->bitcount == 24)
#define is_valid_huffman1d_bitcount(info) (is_os2_bmp(info) && (info)->compression == _BI_HUFFMAN1D && (info)->bitcount == 1)
#define is_valid_bitcount(info) (is_valid_rgb_bitcount(info) || is_valid_rle8_bitcount(info) || is_valid_rle4_bitcount(info) || \
	is_valid_bitfields_bitcount(info) || is_valid_jpeg_bitcount(info) || is_valid_png_bitcount(info) || \
	is_valid_alphabits_bitcount(info) || is_valid_rle24_bitcount(info) || is_valid_huffman1d_bitcount(info))

#define has_palette(info) ((info)->bitcount == 1 || (info)->bitcount == 2 || (info)->bitcount == 4 || (info)->bitcount == 8)
#define has_color_masks(info) (((info)->bitcount == 16 || (info)->bitcount == 32) && (info)->version == 40 && \
	((info)->compression == _BI_BITFIELDS || (info)->compression == _BI_ALPHABITS))
...
static uchar * bmp_decompress_rle24(fz_context * ctx, struct info * info, const uchar * p, const uchar ** end)
{
	...
	while(sp + 2 <= *end) {
		...
		else if(sp[0] == 0 && sp[1] == 1) { /* end of bitmap */
			sp += 2;
			break;
		}
		else if(sp[0] == 0 && sp[1] == 2) { /* delta */
			sp += 2;
			x += sp < *end ? *sp++ : 0;
			y += sp < *end ? *sp++ : 0;
		}
		else if(sp[0] == 0 && sp[1] >= 3) { /* absolute */
			int dn, sn, pad;
			dn = sp[1];
			sn = (dn * 3 + 1) / 2 * 2;
			pad = sn & 1;
	...
	info->compression = _BI_NONE;
	info->bitcount = 24;
	*end = ep;
	return decompressed;
}

static uchar * bmp_decompress_rle8(fz_context * ctx, struct info * info, const uchar * p, const uchar ** end)
{
	const uchar * sp;
	uchar * dp, * ep, * decompressed;
	uint32_t width = info->width;
...
static uchar * bmp_decompress_rle4(fz_context * ctx, struct info * info, const uchar * p, const uchar ** end)
{
	const uchar * sp;
	uchar * dp, * ep, * decompressed;
	uint32_t width = info->width;
    ...
    const uchar * p)
{
	...
	uint32_t sstride;
	int32_t dstride;
	uint rmult, gmult, bmult, amult;
	uint rtrunc, gtrunc, btrunc, atrunc;
	uint32_t x;
	int32_t y;
	if(info->compression == _BI_NONE)
		ssp = p;
	else if(info->compression == _BI_RLE4)
		ssp = decompressed = bmp_decompress_rle4(ctx, info, p, &end);
	else if(info->compression == _BI_RLE8)
		ssp = decompressed = bmp_decompress_rle8(ctx, info, p, &end);
	else if(is_win_bmp(info) && (info->compression == _BI_BITFIELDS || info->compression == _BI_ALPHABITS))
		ssp = p;
	else if(is_os2_bmp(info) && info->compression == _BI_RLE24)
		ssp = decompressed = bmp_decompress_rle24(ctx, info, p, &end);
	else if(is_os2_bmp(info) && info->compression == _BI_HUFFMAN1D)
		...
		switch(bitcount)
		{
			case 32:
			    for(x = 0; x < width; x++) {
				    ...
				    uint r = (sample & info->rmask) >> info->rshift;
				    uint g = (sample & info->gmask) >> info->gshift;
				    uint b = (sample & info->bmask) >> info->bshift;
				    uint a = info->abits == 0 ? 255 : (sample & info->amask) >> info->ashift;
				    *dp++ = (r * rmult) >> rtrunc;
				    *dp++ = (g * gmult) >> gtrunc;
				    *dp++ = (b * bmult) >> btrunc;
			...
			case 16:
			    for(x = 0; x < width; x++) {
				    uint sample = (sp[1] << 8) | sp[0];
				    uint r = (sample & info->rmask) >> info->rshift;
				    uint g = (sample & info->gmask) >> info->gshift;
				    uint b = (sample & info->bmask) >> info->bshift;
				    uint a = (sample & info->amask) >> info->ashift;
				    *dp++ = (r * rmult) >> rtrunc;
				    *dp++ = (g * gmult) >> gtrunc;
				    *dp++ = (b * bmult) >> btrunc;
...
static fz_colorspace * bmp_read_color_profile(fz_context * ctx, struct info * info, const uchar * begin, const uchar * end)
{
...
static void compute_mask_info(uint mask, uint * shift, uint * bits)
{
	*bits = 0;
	*shift = 0;
	while(mask && (mask & 1) == 0) {
...
static const uchar * bmp_read_color_masks(fz_context * ctx, struct info * info,
    const uchar * begin, const uchar * end, const uchar * p)
{
	int size = 0;
	if(info->compression == _BI_BITFIELDS) {
		size = 12;
		if(end - p < 12)
	...
	else if(info->compression == _BI_ALPHABITS) {
		size = 16;
		if(end - p < 16)
			fz_throw(ctx, FZ_ERROR_GENERIC, "premature end in mask header in bmp image");
		info->rmask = read32(p + 0);
...
static int bmp_palette_is_gray(fz_context * ctx, struct info * info, int readcolors)
{
	int i;
...
static fz_pixmap * bmp_read_image(fz_context * ctx, struct info * info, const uchar * begin,
    const uchar * end, const uchar * p, int only_metadata)
{
	const uchar * profilebegin;
	...
	if(!is_valid_bitcount(info))
		fz_throw(ctx,
		    FZ_ERROR_GENERIC,
		    "invalid bits per pixel (%u) for compression (%u) in bmp image",
		    info->bitcount,
		    info->compression);
	...
	if(is_win_bmp(info) && info->compression == _BI_JPEG) {
	...
	else if(is_win_bmp(info) && info->compression == _BI_PNG) {
		if((uint32_t)(end - p) < info->bitmapsize)
...
fz_pixmap * fz_load_bmp(fz_context * ctx, const uchar * p, size_t total)
{
	struct info info;
	fz_pixmap * image;
...
fz_pixmap * fz_load_bmp_subimage(fz_context * ctx, const uchar * buf, size_t len, int subimage)
{
	const uchar * begin = buf;
...
int fz_load_bmp_subimage_count(fz_context * ctx, const uchar * buf, size_t len)
{
	const uchar * begin = buf;
	const uchar * end = buf + len;
	int nextoffset = 0;
	int count = 0;
	do {
		const uchar * p = begin + nextoffset;
		if(is_bitmap_array(p)) {
			/* read16(p+0) == type */
			/* read32(p+2) == size of this header in bytes */
			nextoffset = read32(p + 6);
