...
static void jpx_ycc_to_rgb(fz_context * ctx, fz_pixmap * pix, int cbsign, int crsign)
{
	const int w = pix->w;
	const int h = pix->h;
	const int stride = pix->stride;
	for(int y = 0; y < h; y++) {
		uchar * row = &pix->samples[stride * y];
		for(int x = 0; x < w; x++) {
			int ycc[3];
			ycc[0] = row[x * 3 + 0];
			ycc[1] = row[x * 3 + 1];
 ...
 * though SAlloc::M etc, but no context is passed around.
 *
 * In order to ensure that allocations throughout mupdf
 * are done consistently, we implement SAlloc::M etc as
...
void * opj_malloc_Removed(size_t size)
{
	...
	assert(ctx != NULL);
	return Memento_label(fz_malloc_no_throw(ctx, size), "SAlloc::M");
}

void * opj_calloc_Removed(size_t n, size_t size)
{
	fz_context * ctx = get_opj_context();
	assert(ctx != NULL);
...
void * opj_realloc_Removed(void * ptr, size_t size)
{
	fz_context * ctx = get_opj_context();
	assert(ctx != NULL);
...
void opj_free_Removed(void * ptr)
{
	fz_context * ctx = get_opj_context();
	assert(ctx != NULL);
	fz_free(ctx, ptr);
...
// @sobolev void * opj_aligned_malloc(size_t size) { return opj_aligned_malloc_n(16, size); }
// @sobolev void * opj_aligned_32_malloc(size_t size) { return opj_aligned_malloc_n(32, size); }
...
static size_t fz_opj_stream_read(void * p_buffer, size_t p_nb_bytes, void * p_user_data)
{
	stream_block * sb = (stream_block*)p_user_data;
	size_t len = sb->size - sb->pos;
	if(!len)
		return (size_t)-1; /* End of file! */
	if(len > p_nb_bytes)
		len = p_nb_bytes;
...
static boolint fz_opj_stream_seek(OPJ_OFF_T seek_pos, void * p_user_data)
{
	stream_block * sb = (stream_block*)p_user_data;
...
static int32_t safe_mul32(fz_context * ctx, int32_t a, int32_t b)
{
...
static int32_t safe_mla32(fz_context * ctx, int32_t a, int32_t b, int32_t c)
{
...
static inline void template_copy_comp(uchar * dst0, int w, int h, int stride, const int32_t * src, int32_t ox,
    int32_t oy, uint32_t cdx, uint32_t cdy, uint32_t cw, uint32_t ch, uint32_t sgnd, uint32_t prec, int comps)
{
	int x, y;
...
static void copy_jpx_to_pixmap(fz_context * ctx, fz_pixmap * img, opj_image_t * jpx)
{
	int w = img->w;
	int h = img->h;
	int k;
	int stride = fz_pixmap_stride(ctx, img);
	int comps = fz_pixmap_components(ctx, img);
	uchar * dst = fz_pixmap_samples(ctx, img);
	for(k = 0; k < comps; k++) {
		opj_image_comp_t * comp = &(jpx->comps[k]);
		uint32_t cdx = comp->dx;
		uint32_t cdy = comp->dy;
		uint32_t cw = comp->w;
		uint32_t ch = comp->h;
		int32_t oy = safe_mul32(ctx, comp->y0, cdy) - jpx->y0;
		int32_t ox = safe_mul32(ctx, comp->x0, cdx) - jpx->x0;
		uchar * dst0 = dst + oy * stride;
		if(comp->data == NULL)
...
static fz_pixmap * jpx_read_image(fz_context * ctx, fz_jpxd * state, const uchar * data,
    size_t size, fz_colorspace * defcs, int onlymeta)
{
	fz_pixmap * img = NULL;
...
fz_pixmap * fz_load_jpx(fz_context * ctx, const uchar * data, size_t size, fz_colorspace * defcs)
{
	fz_jpxd state = { 0 };
	fz_pixmap * pix = NULL;
	fz_try(ctx)
...
void fz_load_jpx_info(fz_context * ctx, const uchar * data, size_t size, int * wp, int * hp, int * xresp, int * yresp, fz_colorspace ** cspacep)
{
	fz_jpxd state = { 0 };
	fz_try(ctx)
	{
