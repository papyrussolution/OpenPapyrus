...
static void fz_ocr_fill_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
...
static void fz_ocr_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke,
...
static void fz_ocr_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke,
    fz_matrix ctm, fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shade, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_fill_image(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm, fz_rect scissor)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_clip_stroke_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
...
static void fz_ocr_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_clip_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void fz_ocr_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, fz_rect scissor)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_pop_clip(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_begin_mask(fz_context * ctx,
    fz_device * dev,
    fz_rect rect,
    int luminosity,
...
static void fz_ocr_end_mask(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_begin_group(fz_context * ctx,
    fz_device * dev,
    fz_rect rect,
    fz_colorspace * cs,
    int isolated,
...
static void fz_ocr_end_group(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static int fz_ocr_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm, int id)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_end_tile(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_render_flags(fz_context * ctx, fz_device * dev, int set, int clear)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_set_default_colorspaces(fz_context * ctx, fz_device * dev, fz_default_colorspaces * cs)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_begin_layer(fz_context * ctx, fz_device * dev, const char * layer_name)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void fz_ocr_end_layer(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
...
static void drop_ocr_device(fz_context * ctx, fz_ocr_device * ocr)
{
	int i;
	if(ocr == NULL)
		return;
...
static void flush_word(fz_context * ctx, fz_ocr_device * ocr)
{
	float color = 1;
...
static fz_text_span * fz_clone_text_span(fz_context * ctx, const fz_text_span * span)
{
	fz_text_span * cspan;
...
#ifdef DEBUG_OCR
static void debug_word(fz_context * ctx, word_record * word)
{
	int i;
...
static fz_text_span * rewrite_span(fz_context * ctx, fz_rewrite_device * dev, fz_matrix ctm, const fz_text_span * span)
{
...
static fz_text * rewrite_text(fz_context * ctx, fz_rewrite_device * dev, fz_matrix ctm, const fz_text * text)
{
...
static void rewrite_fill_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
    int even_odd,
    fz_matrix ctm,
...
static void rewrite_stroke_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void rewrite_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm, fz_rect scissor)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_clip_stroke_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void rewrite_fill_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    fz_matrix ctm,
    fz_colorspace * cs,
...
static void rewrite_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void rewrite_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
	fz_text * rtext = rewrite_text(ctx, rewrite, ctm, text);
...
static void rewrite_clip_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void rewrite_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
	fz_text * rtext = rewrite_text(ctx, rewrite, ctm, text);
...
static void rewrite_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shd, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_fill_image(fz_context * ctx, fz_device * dev, fz_image * img, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_fill_image_mask(fz_context * ctx,
    fz_device * dev,
    fz_image * img,
    fz_matrix ctm,
    fz_colorspace * cs,
...
static void rewrite_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * img, fz_matrix ctm, fz_rect scissor)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_pop_clip(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_begin_mask(fz_context * ctx,
    fz_device * dev,
    fz_rect area,
    int luminosity,
    fz_colorspace * cs,
...
static void rewrite_end_mask(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_begin_group(fz_context * ctx,
    fz_device * dev,
    fz_rect area,
    fz_colorspace * cs,
    int isolated,
...
static void rewrite_end_group(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static int rewrite_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm,
    int id)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_end_tile(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_render_flags(fz_context * ctx, fz_device * dev, int set, int clear)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_set_default_colorspaces(fz_context * ctx, fz_device * dev, fz_default_colorspaces * cs)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_begin_layer(fz_context * ctx, fz_device * dev, const char * layer_name)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_end_layer(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
...
static void rewrite_close(fz_context * ctx, fz_device * dev)
{
	fz_rewrite_device * rewrite = (fz_rewrite_device*)dev;
	fz_font * font;
	fz_text * text = NULL;
...
static fz_device * new_rewrite_device(fz_context * ctx, fz_device * target, word_record ** words, int words_len)
{
...
static int fz_ocr_progress(fz_context * ctx, void * arg, int prog)
{
	fz_ocr_device * ocr = (fz_ocr_device*)arg;
...
static void fz_ocr_close_device(fz_context * ctx, fz_device * dev)
{
	fz_ocr_device * ocr = (fz_ocr_device*)dev;
	void * tessapi;
...
static void fz_ocr_drop_device(fz_context * ctx, fz_device * dev)
{
