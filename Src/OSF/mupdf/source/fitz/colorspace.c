...
void fz_enable_icc(fz_context * ctx) { ctx->icc_enabled = 1; }
void fz_disable_icc(fz_context * ctx) { ctx->icc_enabled = 0; }
...
{
	for(int i = 0; i < SIZEOFARRAYi(fz_intent_names); i++)
		if(sstreq(name, fz_intent_names[i]))
			return i;
...
{
	if(ri >= 0 && ri < SIZEOFARRAYi(fz_intent_names))
...
const char * FASTCALL fz_colorspace_name(fz_context * ctx, const fz_colorspace * cs)
{
	return cs ? cs->name : "None";
}

enum fz_colorspace_type FASTCALL fz_colorspace_type(fz_context * ctx, const fz_colorspace * cs)
{
	return cs ? cs->type : FZ_COLORSPACE_NONE;
}

int FASTCALL fz_colorspace_n(fz_context * ctx, const fz_colorspace * cs)
{
	return cs ? cs->n : 0;
}

int FASTCALL fz_colorspace_is_gray(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && cs->type == FZ_COLORSPACE_GRAY;
}

int FASTCALL fz_colorspace_is_rgb(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && cs->type == FZ_COLORSPACE_RGB;
}

int FASTCALL fz_colorspace_is_cmyk(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && cs->type == FZ_COLORSPACE_CMYK;
}

int FASTCALL fz_colorspace_is_lab(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && cs->type == FZ_COLORSPACE_LAB;
}

int FASTCALL fz_colorspace_is_indexed(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->type == FZ_COLORSPACE_INDEXED);
}

int FASTCALL fz_colorspace_is_device_n(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->type == FZ_COLORSPACE_SEPARATION);
}

int FASTCALL fz_colorspace_is_subtractive(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->type == FZ_COLORSPACE_CMYK || cs->type == FZ_COLORSPACE_SEPARATION);
}

int FASTCALL fz_colorspace_is_device(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->flags & FZ_COLORSPACE_IS_DEVICE);
}

int FASTCALL fz_colorspace_is_lab_icc(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->type == FZ_COLORSPACE_LAB) && (cs->flags & FZ_COLORSPACE_IS_ICC);
}

int FASTCALL fz_colorspace_is_device_gray(fz_context * ctx, const fz_colorspace * cs)
{
	return fz_colorspace_is_device(ctx, cs) && fz_colorspace_is_gray(ctx, cs);
}

int FASTCALL fz_colorspace_is_device_cmyk(fz_context * ctx, const fz_colorspace * cs)
{
	return fz_colorspace_is_device(ctx, cs) && fz_colorspace_is_cmyk(ctx, cs);
}

int FASTCALL fz_colorspace_device_n_has_only_cmyk(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && ((cs->flags & FZ_COLORSPACE_HAS_CMYK_AND_SPOTS) == FZ_COLORSPACE_HAS_CMYK);
}

int FASTCALL fz_colorspace_device_n_has_cmyk(fz_context * ctx, const fz_colorspace * cs)
{
	return cs && (cs->flags & FZ_COLORSPACE_HAS_CMYK);
}

int FASTCALL fz_is_valid_blend_colorspace(fz_context * ctx, const fz_colorspace * cs)
{
	return (!cs || oneof3(cs->type, FZ_COLORSPACE_GRAY, FZ_COLORSPACE_RGB, FZ_COLORSPACE_CMYK));
}

fz_colorspace * FASTCALL fz_keep_colorspace(fz_context * ctx, const fz_colorspace * cs)
{
...
fz_colorspace * fz_new_indexed_colorspace(fz_context * ctx, fz_colorspace * base, int high, uchar * lookup)
{
	fz_colorspace * cs;
	char name[100];
	if(high < 0 || high > 255)
...
void fz_colorspace_name_colorant(fz_context * ctx, fz_colorspace * cs, int i, const char * name)
{
	...
	if(sstreq(name, "Cyan") || sstreq(name, "Magenta") || sstreq(name, "Yellow") || sstreq(name, "Black"))
...
static int fz_cmp_link_key(fz_context * ctx, void * k0_, void * k1_)
{
	...
	return memcmp(k0->src_md5, k1->src_md5, 16) == 0 && memcmp(k0->dst_md5, k1->dst_md5, 16) == 0 &&
		k0->src_extras == k1->src_extras && k0->dst_extras == k1->dst_extras && k0->rend.bp == k1->rend.bp &&
		k0->rend.ri == k1->rend.ri && k0->copy_spots == k1->copy_spots && k0->format == k1->format &&
		k0->proof == k1->proof && k0->bgr == k1->bgr;
}

static void fz_format_link_key(fz_context * ctx, char * s, size_t n, void * key_)
{
	...
	int i;
	for(i = 0; i < 16; ++i) {
		sm[i*2+0] = SlConst::P_HxDigL[key->src_md5[i]>>4];
		sm[i*2+1] = SlConst::P_HxDigL[key->src_md5[i]&15];
		dm[i*2+0] = SlConst::P_HxDigL[key->dst_md5[i]>>4];
		dm[i*2+1] = SlConst::P_HxDigL[key->dst_md5[i]&15];
	}
	sm[32] = 0;
	dm[32] = 0;
...
fz_icc_link * fz_find_icc_link(fz_context * ctx, fz_colorspace * src, int src_extras, fz_colorspace * dst, int dst_extras,
    fz_colorspace * prf, fz_color_params rend, int format, int copy_spots)
{
	...
	link = (fz_icc_link *)fz_find_item(ctx, fz_drop_icc_link_imp, &key, &fz_link_store_type);
	if(!link) {
		...
		fz_try(ctx) {
			...
			old_link = (fz_icc_link *)fz_store_item(ctx, new_key, link, 1000, &fz_link_store_type);
			if(old_link) {
...
static void indexed_via_base(fz_context * ctx, fz_color_converter * cc, const float * src, float * dst)
{
	...
	const uchar * lookup = ss->u.indexed.lookup;
...
static void fz_init_process_color_converter(fz_context * ctx, fz_color_converter * cc, fz_colorspace * ss, fz_colorspace * ds, fz_colorspace * is, fz_color_params params)
{
...
{
	fz_cached_color_converter * cc = (fz_cached_color_converter *)cc_->opaque;
	float * val = (float *)fz_hash_find(ctx, cc->hash, ss);
...
void fz_fin_cached_color_converter(fz_context * ctx, fz_color_converter * cc_)
{
	...
	if(cc_ == NULL)
		return;
	cc = (fz_cached_color_converter *)cc_->opaque;
	if(cc == NULL)
		return;
	cc_->opaque = NULL;
...
void fz_convert_slow_pixmap_samples(fz_context * ctx,
    const fz_pixmap * src,
    fz_pixmap * dst,
    fz_colorspace * is,
    ...
    int copy_spots)
{
	...
	/* 1-d lookup table for single channel colorspaces */
		...
		uchar lookup[FZ_MAX_COLORS * 256];
		fz_color_converter cc;
	...
	/* Memoize colors using a hash table for the general case */
		...
		uchar * dold;
		fz_color_converter cc;
		fz_hash_table * lookup = fz_new_hash_table(ctx, 509, srcn+sa, -1, NULL);
		fz_find_color_converter(ctx, &cc, ss, ds, is, params);
		fz_try(ctx) {
			while(h--) {
				size_t ww = w;
				while(ww--) {
					if(*s == *sold && memcmp(sold, s, srcn+sa) == 0) {
						...
						sold = s;
						dold = d;
						color = (uchar*)fz_hash_find(ctx, lookup, s);
						if(color) {
...
void fz_convert_pixmap_samples(fz_context * ctx, const fz_pixmap * src, fz_pixmap * dst, fz_colorspace * prf, const fz_default_colorspaces * default_cs,
    fz_color_params params, int copy_spots)
{
#if FZ_ENABLE_ICC
	fz_colorspace * ss = src->colorspace;
	fz_colorspace * ds = dst->colorspace;
	...
	fz_try(ctx) {
		...
		/* Handle DeviceGray to CMYK as K only. See note in Section 6.3 of PDF spec 1.7. */
		else if((ss->flags & FZ_COLORSPACE_IS_DEVICE) && (ss->type == FZ_COLORSPACE_GRAY) && (ds->type == FZ_COLORSPACE_CMYK)) {
			fz_convert_fast_pixmap_samples(ctx, src, dst, copy_spots);
		}
