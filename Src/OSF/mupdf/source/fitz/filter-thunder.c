...
static int next_thunder(fz_context * ctx, fz_stream * stm, size_t max)
{
...
static void close_thunder(fz_context * ctx, void * state_)
{
...
fz_stream * fz_open_thunder(fz_context * ctx, fz_stream * chain, int w)
{
	...
	fz_try(ctx)
	{
		...
		state->lastpixel = 0;
		state->len = w / 2;
		state->buffer = (uchar*)Memento_label(fz_malloc(ctx, state->len), "thunder_buffer");
