...
static void split_at_script(const uint32_t * fragment,
    size_t fragment_len,
    int level,
    void * arg,
 ...
 */
static void classify_characters(const uint32_t * text,
    ...
    fz_bidi_flags flags)
{
	...
	if((flags & FZ_BIDI_CLASSIFY_WHITE_SPACE)!=0) {
		...
		for(i = 0; i < len; i++) {
			 ...
			 * as numbers
			 */
			slfprintf_stderr("%c", (text[i] <= 127 && text[i] >= 32) ?
			    text[i] : text[i] % 9 + '0');
		}
		slfprintf_stderr("\nTypes: ");
#endif
...
#ifdef DEBUG_BIDI_VERBOSE
			slfprintf_stderr("%c", char_from_types[(int)types[i]]);
#endif
		}
#ifdef DEBUG_BIDI_VERBOSE
		slfprintf_stderr("\n");
...
static void classify_quoted_blocks(const uint32_t * text, fz_bidi_chartype * types, size_t len)
{
	size_t i;
	int inQuote = FALSE;
	int pdfNeeded = FALSE;
	...
	for(i = 0; i < len; i++) {
		switch(direction_from_type(types[i])) {
			case FZ_BIDI_LTR: ltrFound = TRUE; break;
			case FZ_BIDI_RTL: rtlFound = TRUE; break;
			default: break;
		}
	}
 ...
 */
static fz_bidi_level * create_levels(fz_context * ctx, const uint32_t * text, size_t len, fz_bidi_direction * baseDir,
    int resolveWhiteSpace, int flags)
{
	...
	fz_bidi_level * levels = (fz_bidi_level *)Memento_label(fz_malloc(ctx, len * sizeof(*levels)), "bidi_levels");
	fz_var(types);
	fz_try(ctx)
	{
		types = (fz_bidi_chartype *)fz_malloc(ctx, len * sizeof(fz_bidi_chartype));
		classify_characters(text, types, len, (fz_bidi_flags)flags);
