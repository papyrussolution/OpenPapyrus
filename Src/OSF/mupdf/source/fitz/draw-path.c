...
static void line(fz_context * ctx, fz_rasterizer * rast, fz_matrix ctm, float x0, float y0, float x1, float y1)
{
...
static void bezier(fz_context * ctx, fz_rasterizer * rast, fz_matrix ctm, float flatness,
    float xa, float ya,
    float xb, float yb,
    float xc, float yc,
...
static void quad(fz_context * ctx, fz_rasterizer * rast, fz_matrix ctm, float flatness, float xa, float ya, float xb, float yb, float xc, float yc, int depth)
{
	float dmax;
	float xab, yab;
	float xbc, ybc;
...
static void flatten_moveto(fz_context * ctx, void * arg_, float x, float y)
{
...
static void flatten_lineto(fz_context * ctx, void * arg_, float x, float y)
{
	flatten_arg * arg = (flatten_arg*)arg_;
...
static void flatten_curveto(fz_context * ctx, void * arg_, float x1, float y1, float x2, float y2, float x3, float y3)
{
	flatten_arg * arg = (flatten_arg*)arg_;
...
static void flatten_quadto(fz_context * ctx, void * arg_, float x1, float y1, float x2, float y2)
{
	flatten_arg * arg = (flatten_arg*)arg_;
...
static void flatten_close(fz_context * ctx, void * arg_)
{
	flatten_arg * arg = (flatten_arg*)arg_;
...
static void flatten_rectto(fz_context * ctx, void * arg_, float x0, float y0, float x1, float y1)
{
	flatten_arg * arg = (flatten_arg*)arg_;
...
static void fz_add_horiz_rect(fz_context * ctx, sctx * s, float x0, float y0, float x1, float y1)
{
...
static void fz_add_vert_rect(fz_context * ctx, sctx * s, float x0, float y0, float x1, float y1)
{
	if(fz_antidropout_rasterizer(ctx, s->rast)) {
		if(s->ctm.b == 0 && s->ctm.c == 0) {
			float tx0 = s->ctm.a * x0 + s->ctm.e;
...
static void fz_add_line_join(fz_context * ctx, sctx * s, float ax, float ay, float bx, float by, float cx, float cy, int join_under)
{
	float miterlimit = s->miterlimit;
	float linewidth = s->linewidth;
	fz_linejoin linejoin = (fz_linejoin)s->linejoin;
	float dx0, dy0;
	float dx1, dy1;
	float dlx0, dly0;
	float dlx1, dly1;
...
static void fz_add_line_cap(fz_context * ctx, sctx * s, float ax, float ay, float bx, float by, fz_linecap linecap, int rev)
{
	...
	switch(linecap) {
		...
		case FZ_LINECAP_ROUND:
	    {
		    ...
		    float oy = by - dly;
		    for(i = 1; i < n; i++) {
			    float theta = SMathConst::Pi_f * i / n;
...
static void fz_add_line_dot(fz_context * ctx, sctx * s, float ax, float ay)
{
	float flatness = s->flatness;
	float linewidth = s->linewidth;
	...
	if(n < 3)
		n = 3;
	for(i = 1; i < n; i++) {
		float theta = SMathConst::Pi_f * 2 * i / n;
		float cth = cosf(theta);
		float sth = sinf(theta);
...
static void fz_stroke_flush(fz_context * ctx, sctx * s, fz_linecap start_cap, fz_linecap end_cap)
{
	if(s->sn == 2) {
...
static void fz_stroke_moveto(fz_context * ctx, void * s_, float x, float y)
{
...
static void fz_stroke_lineto(fz_context * ctx, sctx * s, float x, float y, int from_bezier)
{
...
static void fz_stroke_closepath(fz_context * ctx, sctx * s)
{
	if(s->sn == 2) {
...
static void fz_stroke_bezier(fz_context * ctx, struct sctx * s,
    float xa, float ya,
    float xb, float yb,
    float xc, float yc,
    float xd, float yd, int depth)
...
static void fz_stroke_quad(fz_context * ctx, struct sctx * s,
    float xa, float ya,
    float xb, float yb,
...
static void stroke_moveto(fz_context * ctx, void * s_, float x, float y)
{
	sctx * s = (sctx*)s_;
...
static void stroke_lineto(fz_context * ctx, void * s_, float x, float y)
{
	sctx * s = (sctx*)s_;
...
static void stroke_curveto(fz_context * ctx, void * s_, float x1, float y1, float x2, float y2, float x3, float y3)
{
	sctx * s = (sctx*)s_;
...
static void stroke_quadto(fz_context * ctx, void * s_, float x1, float y1, float x2, float y2)
{
	sctx * s = (sctx*)s_;
...
static void stroke_close(fz_context * ctx, void * s_)
{
	sctx * s = (sctx*)s_;
...
static void fz_dash_moveto(fz_context * ctx, struct sctx * s, float x, float y)
{
	s->toggle = 1;
	s->offset = 0;
	...
	if(s->toggle) {
		fz_stroke_flush(ctx, s, (fz_linecap)s->cap, s->stroke->end_cap);
...
a_moved_vertically:     /* d and dy have the same sign */
	...
	if(used != 0.0f) {
		...
		if(s->toggle) {
			...
			fz_stroke_flush(ctx, s, (fz_linecap)s->cap, s->stroke->dash_cap);
...
b_moved_vertically:     /* d and dy have the same sign */
	...
	while(total - used > s->dash_list[s->offset] - s->phase) {
		...
		if(s->toggle) {
			...
			fz_stroke_flush(ctx, s, (fz_linecap)s->cap, (fz_linecap)dash_cap);
			s->cap = dash_cap;
...
static void fz_dash_bezier(fz_context * ctx, struct sctx * s,
    float xa, float ya,
    float xb, float yb,
    float xc, float yc,
    float xd, float yd, int depth)
...
static void fz_dash_quad(fz_context * ctx, struct sctx * s,
    float xa, float ya,
    float xb, float yb,
    float xc, float yc, int depth)
{
...
static void dash_moveto(fz_context * ctx, void * s_, float x, float y)
{
	sctx * s = (sctx*)s_;
...
static void dash_lineto(fz_context * ctx, void * s_, float x, float y)
{
	sctx * s = (sctx*)s_;
...
static void dash_curveto(fz_context * ctx, void * s_, float x1, float y1, float x2, float y2, float x3, float y3)
{
	sctx * s = (sctx*)s_;
...
static void dash_quadto(fz_context * ctx, void * s_, float x1, float y1, float x2, float y2)
{
	sctx * s = (sctx*)s_;
...
static void dash_close(fz_context * ctx, void * s_)
{
	sctx * s = (sctx*)s_;
...
static int do_flatten_stroke(fz_context * ctx,
    fz_rasterizer * rast,
    const fz_path * path,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
    ...
    fz_irect * bbox)
{
	...
	fz_stroke_flush(ctx, &s, (fz_linecap)s.cap, stroke->end_cap);
	if(!bbox)
		return 0;
	*bbox = fz_bound_rasterizer(ctx, rast);
...
int fz_flatten_stroke_path(fz_context * ctx, fz_rasterizer * rast, const fz_path * path, const fz_stroke_state * stroke,
    fz_matrix ctm, float flatness, float linewidth, const fz_irect * scissor, fz_irect * bbox)
{
	if(fz_reset_rasterizer(ctx, rast, *scissor)) {
