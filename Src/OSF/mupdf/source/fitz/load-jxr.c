...
static const char * jxr_error_string(int rc)
{
	switch(rc)
	{
...
static inline float float32_from_int32_bits(int v)
{
	return *((float *)&v);
}

static inline float float32_from_float16(int v)
{
	int s = (v >> 15) & 0x1;
	int e = (v >> 10) & 0x1f;
	int m = (v >> 0) & 0x3ff;
...
static inline float sRGB_from_scRGB(float v)
{
	if(v <= 0.0031308f)
		return v * 12.92f;
...
static inline void jxr_unpack_sample(fz_context * ctx, struct info * info, jxr_image_t image, int * sp, uchar * dp)
{
...
static inline void jxr_unpack_alpha_sample(fz_context * ctx, struct info * info, jxr_image_t image, int * sp, uchar * dp)
{
...
static void jxr_decode_block(jxr_image_t image, int mx, int my, int * data)
{
...
static void jxr_decode_block_alpha(jxr_image_t image, int mx, int my, int * data)
{
	struct info * info = jxr_get_user_data(image);
	fz_context * ctx = info->ctx;
	uchar * p;
...
static void jxr_read_image(fz_context * ctx, const uchar * data, int size, struct info * info, int only_metadata)
{
	...
	fz_try(ctx)
	{
		...
		rc = jxr_read_image_container_memory(container, (uchar*)data, size);
		if(rc < 0)
		...
		if(!only_metadata) {
			...
			rc = jxr_read_image_bitstream_memory(image, (uchar*)data + image_offset, size - image_offset);
			if(rc < 0)
			...
			if(alpha_offset > 0) {
				...
				rc = jxr_read_image_bitstream_memory(alpha, (uchar*)data + alpha_offset, size - alpha_offset);
				if(rc < 0)
					fz_throw(ctx, FZ_ERROR_GENERIC, "cannot read jxr image: %s", jxr_error_string(rc));
			}
		}
...
fz_pixmap * fz_load_jxr(fz_context * ctx, const uchar * data, size_t size)
{
	struct info info = { 0 };
...
void fz_load_jxr_info(fz_context * ctx,
    const uchar * data,
    size_t size,
    int * wp,
    int * hp,
