...
#else
	#include "..\OSF\jbig2dec\memento.h"
#endif
#include <stdio.h>
#endif
#ifndef _MSC_VER
...
#define MEMENTO_PREFILL_USHORT (((ushort)MEMENTO_PREFILL_UBYTE) | (((ushort)MEMENTO_PREFILL_UBYTE)<<8))
...
#define MEMENTO_POSTFILL_USHORT (((ushort)MEMENTO_POSTFILL_UBYTE) | (((ushort)MEMENTO_POSTFILL_UBYTE)<<8))
...
#define MEMENTO_ALLOCFILL_USHORT (((ushort)MEMENTO_ALLOCFILL_UBYTE) | (((ushort)MEMENTO_ALLOCFILL_UBYTE)<<8))
...
#define MEMENTO_FREEFILL_USHORT (((ushort)MEMENTO_FREEFILL_UBYTE) | (((ushort)MEMENTO_FREEFILL_UBYTE)<<8))
...
#define MEMENTO_UINT32 uint
#define MEMENTO_UINT16 ushort

#define MEMENTO_PREFILL_UINT32  ((MEMENTO_UINT32)(MEMENTO_PREFILL  | (MEMENTO_PREFILL <<8) | (MEMENTO_PREFILL <<16) | \
	(MEMENTO_PREFILL <<24)))
#define MEMENTO_POSTFILL_UINT16 ((MEMENTO_UINT16)(MEMENTO_POSTFILL | (MEMENTO_POSTFILL<<8)))
#define MEMENTO_POSTFILL_UINT32 ((MEMENTO_UINT32)(MEMENTO_POSTFILL | (MEMENTO_POSTFILL<<8) | (MEMENTO_POSTFILL<<16) | \
	(MEMENTO_POSTFILL<<24)))
#define MEMENTO_FREEFILL_UINT16 ((MEMENTO_UINT16)(MEMENTO_FREEFILL | (MEMENTO_FREEFILL<<8)))
#define MEMENTO_FREEFILL_UINT32 ((MEMENTO_UINT32)(MEMENTO_FREEFILL | (MEMENTO_FREEFILL<<8) | (MEMENTO_FREEFILL<<16) | \
	(MEMENTO_FREEFILL<<24)))
...
typedef struct backtrace_state *(* backtrace_create_state_type)(const char * filename, int threaded,
...
static int init_libbt(void)
{
	...
	return 1;
fail:
	fprintf(stderr, "MEMENTO: libbacktrace.so failed to load; backtraces will be sparse.\nMEMENTO: See memento.h for how to rectify this.\n");
	libbt = NULL;
...
typedef BOOL (__stdcall *My_SymGetLineFromAddrType)(HANDLE hProcess, DWORD_NATIVESIZED dwAddr, PDWORD pdwDisplacement,
    MY_PIMAGEHLP_LINE Line);
...
void Memento_fin(void)
{
	...
	if(Memento_nonLeakBlocksLeaked() && memento.abortOnLeak) {
		slfprintf_stderr("Calling abort() because blocks were leaked and MEMENTO_ABORT_ON_LEAK is set.\n");
		abort();
	}
}
...
static int read_number(const char * text, int * out, int * relative, char ** end)
{
	...
	*out = (int)strtol(text, end, 0 /*base*/);
	if(errno || *end == text) {
		slfprintf_stderr("Failed to parse number at start of '%s'.\n", text);
		return -1;
	}
	if(0)
		slfprintf_stderr("text='%s': *out=%i *relative=%i\n",
		    text, *out, *relative);
	return 0;
}
...
static int read_number_delta(const char * text, int * out, char ** end)
{
	...
	if(e)
		return e;
	if(relative) {
		slfprintf_stderr("Base number should not start with '+' or '-' at start of '%s'.\n",
		    text);
		return -1;
	}
	if(*end) {
	...
	if(0) slfprintf_stderr("text='%s': *out=%i\n", text, *out);
...
static int read_number_range(const char * text, int * begin, int * end, char ** string_end)
{
	...
	if(*end < *begin) {
		slfprintf_stderr("Range %i..%i has negative extent, at start of '%s'.\n",
		    *begin, *end, text);
		return -1;
	}
	if(0) slfprintf_stderr("text='%s': *begin=%i *end=%i\n", text, *begin, *end);
...
static int Memento_add_squeezes(const char * text)
{
	...
	for(;;) {
		...
		if(*string_end && *string_end != ',') {
			slfprintf_stderr("Expecting comma at start of '%s'.\n", string_end);
			e = -1;
			break;
		}
		slfprintf_stderr("Adding squeeze range %i..%i.\n",
		    begin, end);
		...
		if(!memento.squeezes) {
			slfprintf_stderr("Failed to allocate memory for memento.squeezes_num=%i\n",
...
static void Memento_init(void)
{
	...
	if(env) {
		int e;
		slfprintf_stderr("Parsing squeeze ranges in MEMENTO_SQUEEZES=%s\n", env);
		e = Memento_add_squeezes(env);
		if(e) {
			slfprintf_stderr("Failed to parse MEMENTO_SQUEEZES=%s\n", env);
			exit(1);
		}
	}
	...
	if(!memento.squeezing && memento.squeezes_num) {
		...
		if(memento.squeezes_pos < memento.squeezes_num) {
			...
			if(memento.sequence >= begin && memento.sequence < end) {
				if(1) {
					fprintf(stderr, "squeezes match memento.sequence=%i: memento.squeezes_pos=%i/%i %i..%i\n", memento.sequence,
					    memento.squeezes_pos, memento.squeezes_num, memento.squeezes[memento.squeezes_pos].begin, memento.squeezes[memento.squeezes_pos].end);
				}
				return squeeze();
			}
		}
...
static void do_free(void * blk, int eventType)
{
	...
	if(checkBlock(memblk, "free")) {
		if(memento.abortOnCorruption) {
			slfprintf_stderr("*** memblk corrupted, calling abort()\n");
			abort();
		}
		return;
	}
...
int Memento_find(void * a)
{
	...
	Memento_appBlocks(&memento.free, Memento_containsAddr, &data);
	if(data.blk) {
		slfprintf_stderr("Address "FMTP " is in %sfreed block ",
		    data.addr,
		    (data.flags == 1 ? "" : (data.flags == 2 ?
		    "preguard of " : "postguard of ")));
		s = showBlock(data.blk, ' ');
...
void Memento_breakOnFree(void * a)
{
	...
	Memento_appBlocks(&memento.used, Memento_containsAddr, &data);
	if(data.blk) {
		slfprintf_stderr("Will stop when address "FMTP " (in %sallocated block ", data.addr,
...
char * Memento_strdup(const char * s)
{
	...
	char * ret = (char *)MEMENTO_UNDERLYING_MALLOC(len);
	if(ret)
		memcpy(ret, s, len);
	return ret;
}
...
int Memento_asprintf(char ** ret, const char * format, ...)
{
	...
	if(n < 0)
		return n;
	*ret = (char *)MEMENTO_UNDERLYING_MALLOC(n+1);
	if(*ret == NULL)
		return -1;
	va_start(va, format);
	n2 = vsnprintf(*ret, n + 1, format, va);
