...
#if SAVEICCPROFILE
static void save_profile(fz_context * ctx, fz_buffer * buf, const char * name)
{
...
static void fz_append_byte_n(fz_context * ctx, fz_buffer * buf, int c, int n)
{
	int k;
	for(k = 0; k < n; k++)
...
static void setdatetime(fz_context * ctx, icDateTimeNumber * datetime)
{
	datetime->day = 0;
	datetime->hours = 0;
	datetime->minutes = 0;
...
static void add_gammadata(fz_context * ctx, fz_buffer * buf, ushort gamma, icTagTypeSignature curveType)
{
...
static ushort float2u8Fixed8(fz_context * ctx, float number_in)
{
	return (ushort)(number_in * 256);
}

static void add_xyzdata(fz_context * ctx, fz_buffer * buf, icS15Fixed16Number temp_XYZ[])
{
	int j;
...
static icS15Fixed16Number double2XYZtype(fz_context * ctx, float number_in)
{
	short s;
	ushort m;
	if(number_in < 0)
		number_in = 0;
	s = (short)number_in;
	m = (ushort)((number_in - s) * 65536);
	return (icS15Fixed16Number)((s << 16) | m);
}

static void get_D50(fz_context * ctx, icS15Fixed16Number XYZ[])
{
...
static void get_XYZ_doubletr(fz_context * ctx, icS15Fixed16Number XYZ[], float vector[])
{
...
static void add_desc_tag(fz_context * ctx, fz_buffer * buf, const char text[], fz_icc_tag tag_list[], int curr_tag)
{
...
static void add_text_tag(fz_context * ctx, fz_buffer * buf, const char text[], fz_icc_tag tag_list[], int curr_tag)
{
...
static void add_common_tag_data(fz_context * ctx, fz_buffer * buf, fz_icc_tag tag_list[], const char * desc_name)
{
	add_desc_tag(ctx, buf, desc_name, tag_list, 0);
	add_text_tag(ctx, buf, copy_right, tag_list, 1);
}

static void init_common_tags(fz_context * ctx, fz_icc_tag tag_list[], int num_tags, int * last_tag, const char * desc_name)
{
	...
	/* temp_size = DATATYPE_SIZE + 4 (zeros) + 4 (len) + strlen(desc_name) + 1 (null) + 4 + 4 + 2 + 1 + 67 +
	   bytepad; */
...
static void copy_header(fz_context * ctx, fz_buffer * buffer, icHeader * header)
{
...
static void setheader_common(fz_context * ctx, icHeader * header)
{
	header->cmmId = 0;
...
static void matrixmult(fz_context * ctx, float leftmatrix[], int nlrow, int nlcol, float rightmatrix[], int nrrow, int nrcol,
    float result[])
{
	float * curr_row;
	int k, l, j;
...
static void apply_adaption(fz_context * ctx, float matrix[], float in[], float out[])
{
...
/*
        ...
        D50 white point
 */
static void gsicc_create_compute_cam(fz_context * ctx, float white_src[], float * cam)
{
...
fz_buffer * fz_new_icc_data_from_cal(fz_context * ctx, float wp[3], float bp[3], float gamma[3], float matrix[9], int n)
{
	...
	tag_list = (fz_icc_tag *)Memento_label(fz_malloc(ctx, sizeof(fz_icc_tag) * num_tags), "icc_tag_list");
