...
static fz_halftone * fz_new_halftone(fz_context * ctx, int comps)
{
	fz_halftone * ht;
	int i;
	ht = (fz_halftone *)Memento_label(fz_malloc(ctx, sizeof(fz_halftone) + (comps-1)*sizeof(fz_pixmap *)), "fz_halftone");
	ht->refs = 1;
	ht->n = comps;
	for(i = 0; i < comps; i++)
		ht->comp[i] = NULL;
...
fz_halftone * fz_keep_halftone(fz_context * ctx, fz_halftone * ht)
{
	return (fz_halftone *)fz_keep_imp(ctx, ht, &ht->refs);
}
...
static void make_ht_line(uchar * buf, fz_halftone * ht, int x, int y, int w)
{
	int k, n;
	n = ht->n;
	for(k = 0; k < n; k++) {
