...
static float triangle(fz_scale_filter * filter, float f)
{
	if(f >= 1)
		return 0;
	return 1-f;
...
static float box(fz_scale_filter * filter, float f)
{
	if(f >= 0.5f)
		return 0;
	return 1;
...
static float simple(fz_scale_filter * filter, float x)
{
	if(x >= 1)
		return 0;
	return 1 + (2*x - 3)*x*x;
...
static fz_weights * new_weights(fz_context * ctx,
    fz_scale_filter * filter,
    int src_w,
    float dst_w,
    int patch_w,
    ...
    int patch_l)
{
	 ...
	 */
	weights = (fz_weights *)fz_malloc(ctx, sizeof(*weights)+(size_t)(max_len+3)*(patch_w+1)*sizeof(int));
	if(!weights)
		return NULL;
	weights->count = -1;
...
static void init_weights(fz_weights * weights, int j)
{
	int index;
...
static void add_weight(fz_weights * weights, int j, int i, fz_scale_filter * filter,
...
static void reorder_weights(fz_weights * weights, int j, int src_w)
{
...
static void check_weights(fz_weights * weights, int j, int w, float x, float wf)
{
	int idx, len;
	int sum = 0;
	int max = -256;
...
static fz_weights * make_weights(fz_context * ctx,
    int src_w,
    float x,
    float dst_w,
    fz_scale_filter * filter,
...
fz_pixmap * fz_scale_pixmap_cached(fz_context * ctx,
    const fz_pixmap * src,
    float x,
    float y,
    float w,
	...
	* We also need to force the dst to have an alpha if x/y/w/h aren't ints. */
...
#endif /* SINGLE_PIXEL_SPECIALS */
		contrib_cols =
		    Memento_label(make_weights(ctx, src->w, x, w, filter, 0, dst_w_int, patch.x0, patch.x1, src->n, flip_x, cache_x),
			"contrib_cols");
#ifdef SINGLE_PIXEL_SPECIALS
		if(src->h == 1)
			contrib_rows = NULL;
...
#endif /* SINGLE_PIXEL_SPECIALS */
		contrib_rows =
		    Memento_label(make_weights(ctx, src->h, y, h, filter, 1, dst_h_int, patch.y0, patch.y1, src->n, flip_y, cache_y),
			"contrib_rows");
	...
	if(!contrib_rows) {
		...
		if(!contrib_cols) {
			...
			duplicate_single_pixel(output->samples,
			    src->samples,
			    src->n,
			    forcealpha,
			    patch.x1-patch.x0,
		...
		scale_single_col(output->samples,
		    output->stride,
		    src->samples,
		    src->stride,
		    contrib_rows,
...
#endif /* SINGLE_PIXEL_SPECIALS */
	{
		...
		fz_try(ctx)
		{
			temp = (uchar*)fz_calloc(ctx, (size_t)temp_span*temp_rows, sizeof(uchar));
		}
		fz_catch(ctx)
		{
		...
		for(row = 0; row < contrib_rows->count; row++) {
			...
			while(max_row < row_min+row_len) {
				...
				(*row_scale_in)(&temp[temp_span*(max_row % temp_rows)],
				    &src->samples[(flip_y ? (src->h-1-max_row) : max_row)*src->stride], contrib_cols);
				max_row++;
			}
...
void fz_drop_scale_cache(fz_context * ctx, fz_scale_cache * sc)
{
	if(!sc)
		return;
