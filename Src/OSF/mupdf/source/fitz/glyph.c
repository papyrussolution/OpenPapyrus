...
fz_glyph * fz_keep_glyph(fz_context * ctx, fz_glyph * glyph)
{
	return (fz_glyph *)fz_keep_storable(ctx, &glyph->storable);
}

void fz_drop_glyph(fz_context * ctx, fz_glyph * glyph)
{
	fz_drop_storable(ctx, &glyph->storable);
}

static void fz_drop_glyph_imp(fz_context * ctx, fz_storable * glyph_)
{
...
fz_irect fz_glyph_bbox_no_ctx(fz_glyph * glyph)
{
	fz_irect bbox;
	bbox.x0 = glyph->x;
	bbox.y0 = glyph->y;
...
fz_glyph * fz_new_glyph_from_8bpp_data(fz_context * ctx, int x, int y, int w, int h, uchar * sp, int span)
{
	fz_glyph * glyph = NULL;
	fz_pixmap * pix = NULL;
	int size, fill, yy;
	...
	fz_try(ctx)
	{
		...
		size = h * w;
		fill = h * sizeof(int);
		glyph = (fz_glyph*)Memento_label(fz_malloc(ctx, sizeof(fz_glyph) + size), "fz_glyph(8)");
		FZ_INIT_STORABLE(glyph, 1, fz_drop_glyph_imp);
		glyph->x = x;
		glyph->y = y;
		glyph->w = w;
		...
		if(fill != size) {
			glyph = (fz_glyph *)fz_realloc(ctx, glyph, sizeof(fz_glyph) + fill);
			size = fill;
		}
		glyph->size = size;
		break;
		 ...
		 * critical. */
try_pixmap:
		glyph = (fz_glyph *)Memento_label(fz_realloc(ctx, glyph, sizeof(fz_glyph)), "fz_glyph(8r)");
		FZ_INIT_STORABLE(glyph, 1, fz_drop_glyph_imp);
...
fz_glyph * fz_new_glyph_from_1bpp_data(fz_context * ctx, int x, int y, int w, int h, uchar * sp, int span)
{
	fz_pixmap * pix = NULL;
	fz_glyph * glyph = NULL;
	int size, fill, yy;
	...
	fz_try(ctx)
	{
		...
		size = h * w;
		fill = h * sizeof(int);
		glyph = (fz_glyph *)Memento_label(fz_malloc(ctx, sizeof(fz_glyph) + size), "fz_glyph(1)");
		FZ_INIT_STORABLE(glyph, 1, fz_drop_glyph_imp);
		glyph->x = x;
		glyph->y = y;
		glyph->w = w;
		 ...
		 * critical. */
try_pixmap:
		glyph = (fz_glyph *)fz_realloc(ctx, glyph, sizeof(fz_glyph));
		FZ_INIT_STORABLE(glyph, 1, fz_drop_glyph_imp);
