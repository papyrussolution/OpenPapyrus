...
size_t fz_read(fz_context * ctx, fz_stream * stm, uchar * buf, size_t len)
{
	size_t n;
	size_t count = 0;
	do {
...
char * fz_read_line(fz_context * ctx, fz_stream * stm, char * mem, size_t n)
{
	char * s = mem;
	int c = EOF;
	while(n > 1) {
