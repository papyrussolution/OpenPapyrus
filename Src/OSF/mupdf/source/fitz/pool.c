...
fz_pool * fz_new_pool(fz_context * ctx)
{
	...
	fz_try(ctx)
	{
		node = (fz_pool_node *)Memento_label(fz_calloc(ctx, offsetof(fz_pool_node, mem) + POOL_SIZE, 1), "fz_pool_block");
...
static void * fz_pool_alloc_oversize(fz_context * ctx, fz_pool * pool, size_t size)
{
	...
	fz_pool_node * node = (fz_pool_node *)Memento_label(fz_calloc(ctx, offsetof(fz_pool_node, mem) + size, 1), "fz_pool_oversize");
	node->next = pool->head;
	pool->head = node;
...
void * fz_pool_alloc(fz_context * ctx, fz_pool * pool, size_t size)
{
	...
	if(pool->pos + size > pool->end) {
		fz_pool_node * node = (fz_pool_node *)Memento_label(fz_calloc(ctx, offsetof(fz_pool_node, mem) + POOL_SIZE, 1), "fz_pool_block");
...
{
	size_t n = strlen(s) + 1;
	char * p = (char *)fz_pool_alloc(ctx, pool, n);
	memcpy(p, s, n);
	return p;
}
