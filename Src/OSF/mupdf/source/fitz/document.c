...
{
	return (ctx && ctx->handler) ? (fz_document_handler_context *)fz_keep_imp(ctx, ctx->handler, &ctx->handler->refs) : NULL;
}
...
{
	return doc ? ((doc->output_accelerator) != NULL) : 0;
}

void * fz_new_document_of_size(fz_context * ctx, int size)
{
	fz_document * doc = (fz_document *)fz_calloc(ctx, 1, size);
	doc->refs = 1;
	return doc;
}
...
{
	return (fz_document *)fz_keep_imp(ctx, doc, &doc->refs);
}
...
static void fz_ensure_layout(fz_context * ctx, fz_document * doc)
{
...
{
	return (doc && doc->make_bookmark) ? doc->make_bookmark(ctx, doc, loc) : ((loc.chapter<<16) + loc.page);
}

fz_location fz_lookup_bookmark(fz_context * ctx, fz_document * doc, fz_bookmark mark)
{
	return (doc && doc->lookup_bookmark) ? doc->lookup_bookmark(ctx, doc, mark) : fz_make_location((mark>>16) & 0xffff, mark & 0xffff);
}

int fz_needs_password(fz_context * ctx, fz_document * doc)
{
	return (doc && doc->needs_password) ? doc->needs_password(ctx, doc) : 0;
}

int fz_authenticate_password(fz_context * ctx, fz_document * doc, const char * password)
{
	return (doc && doc->authenticate_password) ? doc->authenticate_password(ctx, doc, password) : 1;
}

int fz_has_permission(fz_context * ctx, fz_document * doc, fz_permission p)
{
	return (doc && doc->has_permission) ? doc->has_permission(ctx, doc, p) : 1;
}

fz_outline * fz_load_outline(fz_context * ctx, fz_document * doc)
{
	fz_ensure_layout(ctx, doc);
	return (doc && doc->load_outline) ? doc->load_outline(ctx, doc) : NULL;
}
...
{
	fz_ensure_layout(ctx, doc);
	return (doc && doc->count_chapters) ? doc->count_chapters(ctx, doc) : 1;
}
...
{
	int n = 0;
	const int c = fz_count_chapters(ctx, doc);
	for(int i = 0; i < c; ++i)
...
{
	int n = fz_count_chapters(ctx, doc);
	int start = 0;
	for(int i = 0; i < n; ++i) {
...
{
	return (doc && doc->get_output_intent) ? doc->get_output_intent(ctx, doc) : NULL;
}

fz_page * fz_load_chapter_page(fz_context * ctx, fz_document * doc, int chapter, int number)
{
	fz_page * page;
	if(doc == NULL)
		return NULL;
...
{
	return (page && page->load_links) ? page->load_links(ctx, page) : NULL;
}

fz_rect fz_bound_page(fz_context * ctx, fz_page * page)
{
	return (page && page->bound_page) ? page->bound_page(ctx, page) : fz_empty_rect;
}
...
{
	fz_page * page = (fz_page *)Memento_label(fz_calloc(ctx, 1, size), "fz_page");
	page->refs = 1;
	return page;
}
...
{
	return (fz_page *)fz_keep_imp(ctx, page, &page->refs);
}
...
{
	return (page && page->separations) ? page->separations(ctx, page) : NULL;
}

int fz_page_uses_overprint(fz_context * ctx, fz_page * page)
{
	return (page && page->overprint) ? page->overprint(ctx, page) : 0;
}
