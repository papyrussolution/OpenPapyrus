...
static inline int getrun(const uchar * line, int x, int w, int c)
{
	int z = x;
	while(z < w) {
		int b = getbit(line, z);
...
static inline void putcode(fz_context * ctx, fz_buffer * out, const cfe_code * run)
{
	fz_append_bits(ctx, out, run->code, run->nbits);
}

static void putrun(fz_context * ctx, fz_buffer * out, int run, int c)
{
...
fz_buffer * fz_compress_ccitt_fax_g4(fz_context * ctx, const uchar * src, int columns, int rows)
{
	int stride = (columns + 7) >> 3;
	fz_buffer * out = fz_new_buffer(ctx, (stride * rows) >> 3);
	const uchar * ref = NULL;
...
fz_buffer * fz_compress_ccitt_fax_g3(fz_context * ctx, const uchar * src, int columns, int rows)
{
	int stride = (columns + 7) >> 3;
	fz_buffer * out = fz_new_buffer(ctx, (stride * rows) >> 3);
	int i;
