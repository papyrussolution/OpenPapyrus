...
static void FASTCALL xml_indent(int n)
{
	while(n--) {
		putchar(' ');
		putchar(' ');
...
fz_xml * FASTCALL fz_xml_prev(fz_xml * item) { return item ? item->prev : NULL; }
fz_xml * FASTCALL fz_xml_next(fz_xml * item) { return item ? item->next : NULL; }
fz_xml * FASTCALL fz_xml_up(fz_xml * item) { return item ? item->up : NULL; }
fz_xml * FASTCALL fz_xml_down(fz_xml * item) { return item && !FZ_TEXT_ITEM(item) ? item->down : NULL; }
char * FASTCALL fz_xml_text(fz_xml * item) { return (item && FZ_TEXT_ITEM(item)) ? item->u.text : NULL; }
char * FASTCALL fz_xml_tag(fz_xml * item) { return item && !FZ_TEXT_ITEM(item) && item->u.d.name[0] ? item->u.d.name : NULL; }

int FASTCALL fz_xml_is_tag(fz_xml * item, const char * name)
{
	if(!item || FZ_TEXT_ITEM(item))
		return 0;
	return sstreq(item->u.d.name, name);
}

char * FASTCALL fz_xml_att(fz_xml * item, const char * name)
{
	struct attribute * att;
	if(!item || FZ_TEXT_ITEM(item))
		return NULL;
	for(att = item->u.d.atts; att; att = att->next)
		if(sstreq(att->name, name))
			return att->value;
	return NULL;
}
...
{
	while(item) {
		if(!FZ_TEXT_ITEM(item) && sstreq(item->u.d.name, tag))
			return item;
		item = item->next;
	}
	return NULL;
...
static inline int isname(int c) { return c == '.' || c == '-' || c == '_' || c == ':' || (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'); }
static inline int iswhite(int c) { return c == ' ' || c == '\r' || c == '\n' || c == '\t'; }
...
static void xml_emit_att_name(fz_context * ctx, struct parser * parser, const char * a, const char * b)
{
	...
	size_t size = offsetof(struct attribute, name) + b-a+1;
	struct attribute * att = (struct attribute *)fz_pool_alloc(ctx, parser->pool, size);
...
static void xml_emit_att_value(fz_context * ctx, struct parser * parser, const char * a, const char * b)
{
	...
	s = att->value = (char *)fz_pool_alloc(ctx, parser->pool, b - a + 1);
	while(a < b) {
		if(*a == '&') {
...
static void FASTCALL xml_emit_close_tag(fz_context * ctx, struct parser * parser)
{
	parser->depth--;
...
parse_element:
	...
	if(*p == '!') {
		++p; goto parse_comment;
	}
	if(*p == '?') {
		++p; goto parse_processing_instruction;
	}
	while(iswhite(*p)) ++p;
	if(isname(*p))
...
static char * convert_to_utf8(fz_context * ctx, uchar * s, size_t n, int * dofree)
{
	const ushort * table;
	const uchar * e = s + n;
	char * dst, * d;
	...
	if(s[0] == 0xFE && s[1] == 0xFF) {
		s += 2;
		dst = d = (char *)Memento_label(fz_malloc(ctx, n * FZ_UTFMAX), "utf8_from_be");
		while(s + 1 < e) {
			c = s[0] << 8 | s[1];
			d += fz_runetochar(d, c);
			s += 2;
	...
	if(s[0] == 0xFF && s[1] == 0xFE) {
		s += 2;
		dst = d = (char *)Memento_label(fz_malloc(ctx, n * FZ_UTFMAX), "utf8_from_le");
		while(s + 1 < e) {
			c = s[0] | s[1] << 8;
			d += fz_runetochar(d, c);
			s += 2;
	...
	if(table) {
		dst = d = (char *)Memento_label(fz_malloc(ctx, n * FZ_UTFMAX), "utf8");
		while(*s) {
			c = table[*s++];
			d += fz_runetochar(d, c);
		}
...
fz_xml_doc * fz_parse_xml(fz_context * ctx, fz_buffer * buf, int preserve_white)
{
	...
	fz_try(ctx)
	{
		...
		xml = (fz_xml_doc *)fz_pool_alloc(ctx, parser.pool, sizeof *xml);
...
{
	struct mem_gumbo * mem = (struct mem_gumbo *)ctx;
	...
	switch(node->type) {
		case GUMBO_NODE_ELEMENT:
		    ...
		    for(i = 0; i < node->v.element.attributes.length; ++i) {
			    GumboAttribute * att = (GumboAttribute *)node->v.element.attributes.data[i];
...
fz_xml_doc * fz_parse_xml_from_html5(fz_context * ctx, fz_buffer * buf)
{
	...
	fz_try(ctx)
	{
		...
		// @sobolev opts.allocator = alloc_gumbo;
		// @sobolev opts.deallocator = dealloc_gumbo;
		opts.userdata = &mem;
		opts.tab_stop = 8;
