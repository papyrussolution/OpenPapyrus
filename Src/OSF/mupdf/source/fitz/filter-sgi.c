...
static inline int sgilog16val(fz_context * ctx, uint16_t v)
{
	int Le;
	float Y;
	Le = v & 0x7fff;
...
static int next_sgilog16(fz_context * ctx, fz_stream * stm, size_t max)
{
	...
	if(state->run < 0)
		return EOF;
	memzero(state->temp, state->w * sizeof(uint16_t));
...
static void close_sgilog16(fz_context * ctx, void * state_)
{
...
fz_stream * fz_open_sgilog16(fz_context * ctx, fz_stream * chain, int w)
{
	...
	fz_try(ctx)
	{
		...
		state->c = 0;
		state->w = w;
		state->temp = (uint16_t *)Memento_label(fz_malloc(ctx, w * sizeof(uint16_t)), "sgilog16_temp");
...
static void close_sgilog24(fz_context * ctx, void * state_)
{
...
static int next_sgilog32(fz_context * ctx, fz_stream * stm, size_t max)
{
	...
	if(state->run < 0)
		return EOF;
	memzero(state->temp, state->w * sizeof(uint32_t));
...
static void close_sgilog32(fz_context * ctx, void * state_)
{
...
fz_stream * fz_open_sgilog32(fz_context * ctx, fz_stream * chain, int w)
{
	...
	fz_try(ctx)
	{
		...
		state->c = 0;
		state->w = w;
		state->temp = (uint32_t *)Memento_label(fz_malloc(ctx, w * sizeof(uint32_t)), "sgilog32_temp");
		state->chain = fz_keep_stream(ctx, chain);
	}
	fz_catch(ctx)
	{
