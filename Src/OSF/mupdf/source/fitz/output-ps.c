...
static void ps_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
...
static void ps_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
	ps_band_writer * writer = (ps_band_writer*)writer_;
	fz_output * out = writer->super.out;
	int err;
...
static void ps_drop_band_writer(fz_context * ctx, fz_band_writer * writer_)
{
	ps_band_writer * writer = (ps_band_writer*)writer_;
...
static void ps_write_band(fz_context * ctx,
    ...
    const uchar * samples)
{
	...
	if(writer->input == NULL || writer->input_size < required_input) {
		fz_free(ctx, writer->input);
		writer->input = NULL;
		writer->input = (uchar*)Memento_label(fz_malloc(ctx, required_input), "pswriter_input");
		writer->input_size = required_input;
	}
	if(writer->output == NULL || writer->output_size < required_output) {
		fz_free(ctx, writer->output);
		writer->output = NULL;
		writer->output = (uchar *)Memento_label(fz_malloc(ctx, required_output), "pswriter_output");
...
static fz_device * ps_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
...
static void ps_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	fz_ps_writer * wri = (fz_ps_writer*)wri_;
...
static void ps_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_ps_writer * wri = (fz_ps_writer*)wri_;
...
static void ps_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_ps_writer * wri = (fz_ps_writer*)wri_;
	fz_drop_pixmap(ctx, wri->pixmap);
