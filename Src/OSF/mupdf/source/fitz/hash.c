...
static void fz_resize_hash(fz_context * ctx, fz_hash_table * table, int newsize)
{
	...
	newents = (fz_hash_entry *)fz_malloc_no_throw(ctx, newsize * sizeof(fz_hash_entry));
	if(table->lock == FZ_LOCK_ALLOC)
...
void * fz_hash_find(fz_context * ctx, fz_hash_table * table, const void * key)
{
	...
	unsigned pos = hash((const uchar *)key, table->keylen) % size;
	if(table->lock >= 0)
		fz_assert_lock_held(ctx, table->lock);
	while(1) {
		if(!ents[pos].val)
...
static void do_removal(fz_context * ctx, fz_hash_table * table, const void * key, unsigned hole)
{
	fz_hash_entry * ents = table->ents;
	unsigned size = table->size;
	unsigned look, code;
