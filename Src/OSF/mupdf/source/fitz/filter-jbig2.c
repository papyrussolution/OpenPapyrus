...
{
	return (fz_jbig2_globals *)fz_keep_storable(ctx, &globals->storable);
}
...
static void close_jbig2d(fz_context * ctx, void * state_)
{
...
static int next_jbig2d(fz_context * ctx, fz_stream * stm, size_t len)
{
	fz_jbig2d * state = (fz_jbig2d *)stm->state;
	uchar tmp[4096];
	uchar * buf = state->buffer;
	uchar * p = buf;
	uchar * ep;
	uchar * s;
	int x, w;
...
void fz_drop_jbig2_globals_imp(fz_context * ctx, fz_storable * globals_)
{
...
fz_stream * fz_open_jbig2d(fz_context * ctx, fz_stream * chain, fz_jbig2_globals * globals, int embedded)
{
	...
	options = (Jbig2Options)0;
	if(embedded)
		options |= JBIG2_OPTIONS_EMBEDDED;
	state->ctx = jbig2_ctx_new((Jbig2Allocator*)&state->alloc, (Jbig2Options)options, globals ? globals->gctx : NULL, error_callback, ctx);
	if(state->ctx == NULL) {
		fz_drop_jbig2_globals(ctx, state->gctx);
		fz_free(ctx, state);
