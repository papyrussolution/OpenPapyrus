...
static fz_draw_state * fz_knockout_begin(fz_context * ctx, fz_draw_device * dev)
{
	...
	if(state[0].group_alpha) {
		...
		state[1].group_alpha = fz_new_pixmap_with_bbox(ctx,
			state->group_alpha->colorspace,
			ga_bbox,
			state->group_alpha->seps,
			state->group_alpha->alpha);
	}
...
static int colors_supported(fz_context * ctx, fz_colorspace * cs, fz_pixmap * dest)
{
	...
	if(fz_colorspace_is_subtractive(ctx, dest->colorspace)) {
		...
		for(i = 0; i < n; i++) {
			...
			if(sstreq(name, "Cyan"))
				continue;
			if(sstreq(name, "Magenta"))
				continue;
			if(sstreq(name, "Yellow"))
				continue;
			if(sstreq(name, "Black"))
				continue;
			if(sstreq(name, "None"))
				continue;
			return 0;
...
static fz_overprint * set_op_from_spaces(fz_context * ctx, fz_overprint * op, const fz_pixmap * dest, fz_colorspace * src, int opm)
{
	int dn, sn, i, j, dc;
...
static fz_overprint * resolve_color(fz_context * ctx,
    fz_overprint * op,
    const float * color,
...
static fz_draw_state * push_group_for_separations(fz_context * ctx,
    fz_draw_device * dev,
    fz_color_params color_params,
    fz_default_colorspaces * default_cs)
{
	...
	/* Not needed if dest has the seps, and we are not using a proof or the target is the same as the proof and we
	   don't have an oi or the target is the same as the oi */
	if((clone == dev->stack[0].dest->seps) && (dev->proof_cs == NULL || dev->proof_cs == dev->stack[0].dest->colorspace) &&
	...
	fz_try(ctx)
	{
		...
		dev->stack[1].dest = fz_clone_pixmap_area_with_different_seps(ctx,
			dev->stack[0].dest,
			&dev->stack[0].scissor,
			dcs,
			clone,
...
static void fz_draw_fill_path(fz_context * ctx, fz_device * devp, const fz_path * path, int even_odd, fz_matrix in_ctm,
...
static void fz_draw_stroke_path(fz_context * ctx, fz_device * devp, const fz_path * path, const fz_stroke_state * stroke, fz_matrix in_ctm,
    fz_colorspace * colorspace_in, const float * color, float alpha, fz_color_params color_params)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix ctm = fz_concat(in_ctm, dev->transform);
...
static void fz_draw_clip_path(fz_context * ctx, fz_device * devp, const fz_path * path, int even_odd, fz_matrix in_ctm, fz_rect scissor)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix ctm = fz_concat(in_ctm, dev->transform);
	fz_rasterizer * rast = dev->rast;
...
static void fz_draw_clip_stroke_path(fz_context * ctx,
    fz_device * devp,
    const fz_path * path,
...
static void draw_glyph(uchar * colorbv, fz_pixmap * dst, fz_glyph * glyph,
    int xorig, int yorig, const fz_irect * scissor, fz_overprint * eop)
{
	...
	if(msk == NULL) {
		...
		uchar * mp = msk->samples + skip_y * msk->stride + skip_x;
		int da = dst->alpha;
...
static void fz_draw_fill_text(fz_context * ctx, fz_device * devp, const fz_text * text, fz_matrix in_ctm,
    fz_colorspace * colorspace_in, const float * color, float alpha, fz_color_params color_params)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix ctm = fz_concat(in_ctm, dev->transform);
	...
	for(span = text->head; span; span = span->next) {
		...
		for(i = 0; i < span->len; i++) {
			...
			glyph = fz_render_glyph(ctx,
				...
				&state->scissor,
				state->dest->alpha,
				fz_rasterizer_text_aa_level(rast));
			if(glyph) {
				...
				if(pixmap == NULL || pixmap->n == 1) {
					...
					fz_paint_image(ctx,
					    ...
					    mat,
					    alpha * 255,
					    !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES),
					    devp->flags & FZ_DEVFLAG_GRIDFIT_AS_TILED,
					    eop);
				}
...
static void fz_draw_stroke_text(fz_context * ctx, fz_device * devp, const fz_text * text, const fz_stroke_state * stroke,
...
static void fz_draw_clip_text(fz_context * ctx, fz_device * devp, const fz_text * text, fz_matrix in_ctm, fz_rect scissor)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix ctm = fz_concat(in_ctm, dev->transform);
	fz_irect bbox;
...
static void fz_draw_clip_stroke_text(fz_context * ctx,
    fz_device * devp,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix in_ctm,
...
static void fz_draw_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
}

static void fz_draw_fill_shade(fz_context * ctx,
    fz_device * devp,
    fz_shade * shade,
    fz_matrix in_ctm,
    float alpha,
		...
		if(shade->use_background) {
			...
			if(fz_overprint_required(eop)) {
				for(y = scissor.y0; y < scissor.y1; y++) {
					s = dest->samples + (uint)((y - dest->y) * dest->stride + (scissor.x0 - dest->x) * n);
			...
			if(shape) {
				for(y = scissor.y0; y < scissor.y1; y++) {
					s = shape->samples + (uint)((y - shape->y) * shape->stride + (scissor.x0 - shape->x));
					for(x = scissor.x0; x < scissor.x1; x++) {
						*s++ = 255;
					}
				}
			...
			if(group_alpha) {
				for(y = scissor.y0; y < scissor.y1; y++) {
					s = group_alpha->samples +
					    (uint)((y - group_alpha->y) * group_alpha->stride + (scissor.x0 - group_alpha->x));
					for(x = scissor.x0; x < scissor.x1; x++) {
						*s++ = alpha_byte;
					}
				}
...
static fz_pixmap * fz_transform_pixmap(fz_context * ctx,
    fz_draw_device * dev,
...
static fz_pixmap * convert_pixmap_for_painting(fz_context * ctx,
    fz_pixmap * pixmap,
    fz_colorspace * model,
...
static void fz_draw_fill_image(fz_context * ctx,
    fz_device * devp,
    fz_image * image,
    fz_matrix in_ctm,
    float alpha,
    fz_color_params color_params)
{
	...
	fz_try(ctx)
	{
		...
		if(!(devp->hints & FZ_DONT_INTERPOLATE_IMAGES) &&
			...
			fz_pixmap * scaled = fz_transform_pixmap(ctx,
				dev,
				pixmap,
				&local_ctm,
				state->dest->x,
			...
			if(!scaled) {
				...
				if(dy < 1)
					dy = 1;
				scaled =
				    fz_scale_pixmap_cached(ctx, pixmap, pixmap->x, pixmap->y, dx, dy, NULL, dev->cache_x, dev->cache_y);
			}
			if(scaled) {
				fz_drop_pixmap(ctx, pixmap);
				pixmap = scaled;
		...
		fz_paint_image(ctx, state->dest, &state->scissor, state->shape, state->group_alpha, pixmap, local_ctm, alpha * 255,
		    !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES), devp->flags & FZ_DEVFLAG_GRIDFIT_AS_TILED, eop);
...
static void fz_draw_fill_image_mask(fz_context * ctx, fz_device * devp, fz_image * image, fz_matrix in_ctm,
    fz_colorspace * colorspace_in, const float * color, float alpha, fz_color_params color_params)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix local_ctm = fz_concat(in_ctm, dev->transform);
...
static void fz_draw_clip_image_mask(fz_context * ctx, fz_device * devp, fz_image * image, fz_matrix in_ctm, fz_rect scissor)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_matrix local_ctm = fz_concat(in_ctm, dev->transform);
	fz_irect bbox;
	...
	fz_try(ctx)
	{
		...
		fz_paint_image(ctx, state[1].mask, &bbox, state[1].shape, state[1].group_alpha, pixmap, local_ctm, 255,
		    !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES), devp->flags & FZ_DEVFLAG_GRIDFIT_AS_TILED, 0);
...
static void fz_draw_pop_clip(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_draw_state * state;
...
static void fz_draw_begin_mask(fz_context * ctx,
    fz_device * devp,
    fz_rect area,
    int luminosity,
...
static void fz_draw_end_mask(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_pixmap * temp, * dest;
	fz_irect bbox;
...
static void fz_draw_begin_group(fz_context * ctx,
    fz_device * devp,
    fz_rect area,
    fz_colorspace * cs,
    int isolated,
    ...
    float alpha)
{
	...
	dump_spaces(dev->top-1, "");
	{
		...
		if(blendmode != 0)
			...
			btext[0] = 0;
		sprintf(text,
		    "Group begin%s%s%s%s: background is ",
		    isolated ? " (isolated)" : "",
		    knockout ? " (knockout)" : "",
		    atext,
		    btext);
...
static void fz_draw_end_group(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	int blendmode;
	int isolated;
	...
	if(state[0].dest->colorspace != state[1].dest->colorspace) {
		fz_pixmap * converted = fz_convert_pixmap(ctx,
			state[1].dest,
			state[0].dest->colorspace,
			NULL,
			dev->default_cs,
			fz_default_color_params,
			1);
...
static int fz_make_hash_tile_key(fz_context * ctx, fz_store_hash * hash, void * key_)
{
...
static void * fz_keep_tile_key(fz_context * ctx, void * key_)
{
	tile_key * key = (tile_key *)key_;
	return fz_keep_imp(ctx, key, &key->refs);
}

static void fz_drop_tile_key(fz_context * ctx, void * key_)
{
	tile_key * key = (tile_key *)key_;
...
static int fz_cmp_tile_key(fz_context * ctx, void * k0_, void * k1_)
{
...
static void fz_format_tile_key(fz_context * ctx, char * s, size_t n, void * key_)
{
	tile_key * key = (tile_key*)key_;
...
static void fz_drop_tile_record_imp(fz_context * ctx, fz_storable * storable)
{
...
static void fz_drop_tile_record(fz_context * ctx, tile_record * tile)
{
	fz_drop_storable(ctx, &tile->storable);
}

static tile_record * fz_new_tile_record(fz_context * ctx, fz_pixmap * dest, fz_pixmap * shape, fz_pixmap * group_alpha)
{
...
size_t fz_tile_size(fz_context * ctx, tile_record * tile)
{
	if(!tile)
		return 0;
...
static int fz_draw_begin_tile(fz_context * ctx,
    fz_device * devp,
    fz_rect area,
    fz_rect view,
    float xstep,
    ...
    int id)
{
	...
	if(id) {
		...
		tile = (tile_record *)fz_find_item(ctx, fz_drop_tile_record_imp, &tk, &fz_tile_store_type);
		if(tile) {
...
static void fz_draw_end_tile(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	float xstep, ystep;
	...
	fz_try(ctx)
	{
		...
		if(state[1].encache && state[1].id != 0) {
			...
			fz_try(ctx)
			{
				...
				existing_tile = (tile_record *)fz_store_item(ctx, key, tile, fz_tile_size(ctx, tile), &fz_tile_store_type);
				if(existing_tile) {
...
static void fz_draw_render_flags(fz_context * ctx, fz_device * devp, int set, int clear)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	dev->flags = (dev->flags | set ) & ~clear;
}

static void fz_draw_set_default_colorspaces(fz_context * ctx, fz_device * devp, fz_default_colorspaces * default_cs)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
...
static void fz_draw_close_device(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	...
	if(dev->resolve_spots && dev->top) {
		...
		fz_try(ctx)
		{
			fz_copy_pixmap_area_converting_seps(ctx,
			    state[1].dest,
			    state[0].dest,
			    dev->proof_cs,
			    fz_default_color_params,
...
static void fz_draw_drop_device(fz_context * ctx, fz_device * devp)
{
	fz_draw_device * dev = (fz_draw_device*)devp;
	fz_rasterizer * rast = dev->rast;
...
static fz_device * new_draw_device(fz_context * ctx,
    fz_matrix transform,
    fz_pixmap * dest,
    const fz_aa_context * aa,
    const fz_irect * clip,
    fz_colorspace * proof_cs)
{
...
fz_device * fz_new_draw_device_with_proof(fz_context * ctx, fz_matrix transform, fz_pixmap * dest, fz_colorspace * cs)
{
...
fz_draw_options * fz_parse_draw_options(fz_context * ctx, fz_draw_options * opts, const char * args)
{
	const char * val;
...
fz_device * fz_new_draw_device_with_options(fz_context * ctx, const fz_draw_options * opts, fz_rect mediabox, fz_pixmap ** pixmap)
{
