...
void fz_set_rasterizer_text_aa_level(fz_context * ctx, fz_aa_context * aa, int level)
{
#ifdef AA_BITS
	if(level != fz_aa_bits) {
		if(fz_aa_bits == 10)
...
void fz_set_rasterizer_graphics_aa_level(fz_context * ctx, fz_aa_context * aa, int level)
{
#ifdef AA_BITS
	if(level != fz_aa_bits) {
		if(fz_aa_bits == 10)
...
void fz_set_text_aa_level(fz_context * ctx, int level)
{
	fz_set_rasterizer_text_aa_level(ctx, &ctx->aa, level);
}

void fz_set_graphics_aa_level(fz_context * ctx, int level)
{
	fz_set_rasterizer_graphics_aa_level(ctx, &ctx->aa, level);
}
...
{
	fz_rasterizer * rast = (fz_rasterizer *)fz_calloc(ctx, 1, size);
	rast->fns = *fns;
	rast->clip.x0 = rast->clip.y0 = BBOX_MIN;
	rast->clip.x1 = rast->clip.y1 = BBOX_MAX;
	rast->bbox.x0 = rast->bbox.y0 = BBOX_MAX;
