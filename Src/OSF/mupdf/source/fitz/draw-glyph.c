...
{
	return glyph ? (sizeof(fz_glyph) + glyph->size + fz_pixmap_size(ctx, glyph->pixmap)) : 0;
}

void fz_new_glyph_cache_context(fz_context * ctx)
{
	fz_glyph_cache * cache = fz_malloc_struct(ctx, fz_glyph_cache);
	cache->total = 0;
	cache->refs = 1;
	ctx->glyph_cache = cache;
}

static void drop_glyph_cache_entry(fz_context * ctx, fz_glyph_cache_entry * entry)
{
...
static inline void move_to_front(fz_glyph_cache * cache, fz_glyph_cache_entry * entry)
{
    ...
    fz_colorspace * model, const fz_irect * scissor, int alpha, int aa)
{
	...
	hash = do_hash((uchar*)&key, sizeof(key)) % GLYPH_HASH_LEN;
	fz_lock(ctx, FZ_LOCK_GLYPHCACHE);
