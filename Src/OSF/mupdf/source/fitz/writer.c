 ...
 * key/value strings. */
static const char * fz_get_option(fz_context * ctx, const char ** key, const char ** val, const char * opts)
{
	if(!opts || *opts == 0)
		return NULL;
	if(*opts == ',')
...
int FASTCALL fz_has_option(fz_context * ctx, const char * opts, const char * key, const char ** val)
{
	const char * straw;
	size_t n = sstrlen(key);
...
int FASTCALL fz_option_eq(const char * a, const char * b)
{
	size_t n = sstrlen(b);
...
{
	fz_document_writer * wri = (fz_document_writer *)Memento_label(fz_calloc(ctx, 1, size), "fz_document_writer");
