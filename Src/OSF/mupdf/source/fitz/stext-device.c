...
fz_layout_block * fz_new_layout(fz_context * ctx)
{
	...
	fz_try(ctx)
	{
		block = (fz_layout_block *)fz_pool_alloc(ctx, pool, sizeof(fz_layout_block));
		block->pool = pool;
		block->head = NULL;
...
{
	fz_layout_line * line = (fz_layout_line *)fz_pool_alloc(ctx, block->pool, sizeof(fz_layout_line));
	line->x = x;
	line->y = y;
	line->h = h;
	line->p = p;
...
{
	fz_layout_char * ch = (fz_layout_char *)fz_pool_alloc(ctx, block->pool, sizeof(fz_layout_char));
	ch->x = x;
	ch->w = w;
	ch->p = p;
	ch->next = NULL;
...
fz_stext_page * fz_new_stext_page(fz_context * ctx, fz_rect mediabox)
{
	...
	fz_try(ctx)
	{
		page = (fz_stext_page *)fz_pool_alloc(ctx, pool, sizeof(*page));
		page->pool = pool;
...
static fz_stext_block * add_block_to_page(fz_context * ctx, fz_stext_page * page)
{
	fz_stext_block * block = (fz_stext_block *)fz_pool_alloc(ctx, page->pool, sizeof *page->first_block);
...
static fz_stext_block * add_text_block_to_page(fz_context * ctx, fz_stext_page * page)
{
...
static fz_stext_block * add_image_block_to_page(fz_context * ctx, fz_stext_page * page, fz_matrix ctm, fz_image * image)
{
	fz_stext_block * block = add_block_to_page(ctx, page);
...
static void remove_last_char(fz_context * ctx, fz_stext_line * line)
{
...
static int direction_from_bidi_class(int bidiclass, int curdir)
{
	switch(bidiclass)
	{
		/* strong */
...
static void fz_add_stext_char_imp(fz_context * ctx,
    fz_stext_device * dev,
    fz_font * font,
    int c,
    int glyph,
...
static void fz_add_stext_char(fz_context * ctx,
    fz_stext_device * dev,
    fz_font * font,
    int c,
    int glyph,
...
static void fz_stext_extract(fz_context * ctx, fz_stext_device * dev, fz_text_span * span, fz_matrix ctm)
{
...
static void fz_stext_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
	fz_text_span * span;
...
static void fz_stext_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
	fz_text_span * span;
	if(text == tdev->lasttext)
...
static void fz_stext_clip_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
...
static void fz_stext_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
	fz_text_span * span;
	if(text == tdev->lasttext)
...
static void fz_stext_fill_image(fz_context * ctx, fz_device * dev, fz_image * img, fz_matrix ctm, float alpha, fz_color_params color_params)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
...
static void fz_stext_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * img, fz_matrix ctm,
...
static fz_image * fz_new_image_from_shade(fz_context * ctx,
    fz_shade * shade,
...
static void fz_stext_fill_shade(fz_context * ctx,
    fz_device * dev,
    fz_shade * shade,
    fz_matrix ctm,
    float alpha,
...
static void fz_stext_close_device(fz_context * ctx, fz_device * dev)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
...
static void fz_stext_drop_device(fz_context * ctx, fz_device * dev)
{
	fz_stext_device * tdev = (fz_stext_device*)dev;
	fz_drop_text(ctx, tdev->lasttext);
}
...
fz_device * fz_new_stext_device(fz_context * ctx, fz_stext_page * page, const fz_stext_options * opts)
{
