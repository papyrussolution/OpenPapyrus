...
static int fz_reset_gel(fz_context * ctx, fz_rasterizer * rast)
{
...
static void fz_drop_gel(fz_context * ctx, fz_rasterizer * rast)
{
	fz_gel * gel = (fz_gel*)rast;
	if(gel == NULL)
		return;
...
static int clip_lerp_x(int val, int m, int x0, int y0, int x1, int y1, int * out)
{
...
static void fz_insert_gel_raw(fz_context * ctx, fz_rasterizer * ras, int x0, int y0, int x1, int y1)
{
...
static void fz_insert_gel(fz_context * ctx, fz_rasterizer * ras, float fx0, float fy0, float fx1, float fy1, int rev)
{
	int x0, y0, x1, y1;
	int d, v;
...
static void fz_insert_gel_rect(fz_context * ctx, fz_rasterizer * ras, float fx0, float fy0, float fx1, float fy1)
{
	int x0, y0, x1, y1;
	const int hscale = fz_rasterizer_aa_hscale(ras);
	const int vscale = fz_rasterizer_aa_vscale(ras);
...
{
	const fz_edge * a = (const fz_edge *)va;
	const fz_edge * b = (const fz_edge *)vb;
	return a->y - b->y;
}

static void sort_gel(fz_context * ctx, fz_gel * gel)
{
	fz_edge * a = gel->edges;
	int n = gel->len;
	int h, i, k;
...
static int fz_is_rect_gel(fz_context * ctx, fz_rasterizer * ras)
{
	fz_gel * gel = (fz_gel*)ras;
...
static void sort_active(fz_edge ** a, int n)
{
	int h, i, k;
	fz_edge * t;
...
static int insert_active(fz_context * ctx, fz_gel * gel, int y, int * e_)
{
	int h_min = INT_MAX;
	int e = *e_;
...
static void advance_active(fz_context * ctx, fz_gel * gel, int inc)
{
	fz_edge * edge;
	int i = 0;
...
static inline void add_span_aa(fz_context * ctx, fz_gel * gel, int * list, int x0, int x1, int xofs, int h)
{
	int x0pix, x0sub;
	int x1pix, x1sub;
	...
	x0pix = ((uint)x0) / hscale;
	x0sub = ((uint)x0) % hscale;
	x1pix = ((uint)x1) / hscale;
	x1sub = ((uint)x1) % hscale;
...
static inline void non_zero_winding_aa(fz_context * ctx, fz_gel * gel, int * list, int xofs, int h)
{
	int winding = 0;
	int x = 0;
	int i;
...
static inline void even_odd_aa(fz_context * ctx, fz_gel * gel, int * list, int xofs, int h)
{
	int even = 0;
	int x = 0;
	int i;
...
static inline void blit_aa(fz_pixmap * dst, int x, int y, uchar * mp, int w, uchar * color, void * fn, fz_overprint * eop)
{
	uchar * dp;
...
static void fz_scan_convert_aa(fz_context * ctx,
    fz_gel * gel,
    int eofill,
    const fz_irect * clip,
    fz_pixmap * dst,
    ...
    fz_overprint * eop)
{
	...
	if(bcap > gel->bcap) {
		...
		gel->alphas = NULL;
		gel->deltas = NULL;
		alphas = gel->alphas = Memento_label(fz_malloc_array(ctx, bcap, uchar), "gel_alphas");
		deltas = gel->deltas = Memento_label(fz_malloc_array(ctx, bcap, int), "gel_deltas");
	}
	alphas = gel->alphas;
	deltas = gel->deltas;
	memzero(deltas, (xmax - xmin + 1) * sizeof(int));
	gel->alen = 0;
	...
	while(gel->alen > 0 || e < gel->len) {
		...
		if(yc != yd) {
			...
			memzero(deltas, (skipx + clipn) * sizeof(int));
		}
		yd = yc;
		if(yd >= clip->y1)
			break;
...
static inline void non_zero_winding_sharp(fz_context * ctx, fz_gel * gel, int y, const fz_irect * clip,
    fz_pixmap * dst, uchar * color, fz_solid_color_painter_t * fn, fz_overprint * eop)
{
	int winding = 0;
	int x = 0;
...
static inline void even_odd_sharp(fz_context * ctx, fz_gel * gel, int y, const fz_irect * clip, fz_pixmap * dst,
    uchar * color, fz_solid_color_painter_t * fn, fz_overprint * eop)
{
	int even = 0;
	int x = 0;
	int i;
...
static void fz_scan_convert_sharp(fz_context * ctx, fz_gel * gel, int eofill, const fz_irect * clip,
    fz_pixmap * dst, uchar * color, fz_solid_color_painter_t * fn, fz_overprint * eop)
{
	int e = 0;
	int y = gel->edges[0].y;
...
static void fz_convert_gel(fz_context * ctx,
    fz_rasterizer * rast,
    int eofill,
    const fz_irect * clip,
    fz_pixmap * dst,
