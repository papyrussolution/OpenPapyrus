...
{
	return Memento_label(fz_malloc_no_throw((fz_context *)ctx, (size_t)items * size), "zlib_alloc");
}

void fz_zlib_free(void * ctx, void * ptr)
{
	fz_free((fz_context *)ctx, ptr);
}

static int next_flated(fz_context * ctx, fz_stream * stm, size_t required)
{
	fz_inflate_state * state = (fz_inflate_state *)stm->state;
	fz_stream * chain = state->chain;
	z_streamp zp = &state->z;
	int code;
	uchar * outbuf = state->buffer;
	...
	while(zp->avail_out > 0) {
		...
		else if(code == Z_DATA_ERROR && sstreq(zp->msg, "incorrect data check")) {
			fz_warn(ctx, "ignoring zlib error: %s", zp->msg);
			chain->rp = chain->wp;
			break;
		}
...
static void close_flated(fz_context * ctx, void * state_)
{
