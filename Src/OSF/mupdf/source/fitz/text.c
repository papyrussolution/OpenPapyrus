...
fz_text * fz_keep_text(fz_context * ctx, const fz_text * textc)
{
	fz_text * text = (fz_text*)textc; /* Explicit cast away of const */
	return (fz_text *)fz_keep_imp(ctx, text, &text->refs);
}

void fz_drop_text(fz_context * ctx, const fz_text * textc)
{
	fz_text * text = (fz_text*)textc; /* Explicit cast away of const */
...
static fz_text_span * fz_new_text_span(fz_context * ctx, fz_font * font, int wmode, int bidi_level, fz_bidi_direction markup_dir, fz_text_language language, fz_matrix trm)
{
...
static fz_text_span * fz_add_text_span(fz_context * ctx, fz_text * text, fz_font * font, int wmode, int bidi_level, fz_bidi_direction markup_dir, fz_text_language language, fz_matrix trm)
{
	if(!text->tail) {
...
void fz_show_glyph(fz_context * ctx, fz_text * text, fz_font * font, fz_matrix trm, int gid, int ucs, int wmode, int bidi_level, fz_bidi_direction markup_dir, fz_text_language lang)
{
	fz_text_span * span;
	if(text->refs != 1)
...
fz_matrix fz_show_string(fz_context * ctx, fz_text * text, fz_font * user_font, fz_matrix trm, const char * s,
...
fz_matrix fz_measure_string(fz_context * ctx, fz_font * user_font, fz_matrix trm, const char * s,
    int wmode, int bidi_level, fz_bidi_direction markup_dir, fz_text_language language)
{
	fz_font * font;
	int gid, ucs;
...
fz_text_language fz_text_language_from_string(const char * str)
{
	...
	if(sstreq(str, "zh-Hant") || sstreq(str, "zh-HK") || sstreq(str, "zh-MO") || sstreq(str, "zh-SG") || sstreq(str, "zh-TW"))
		return FZ_LANG_zh_Hant;
	if(sstreq(str, "zh-Hans") || sstreq(str, "zh-CN"))
		return FZ_LANG_zh_Hans;
	/* 1st char */
	if(str[0] >= 'a' && str[0] <= 'z')
		lang = (fz_text_language)(str[0] - 'a' + 1);
	else if(str[0] >= 'A' && str[0] <= 'Z')
		lang = (fz_text_language)(str[0] - 'A' + 1);
	else
		return (fz_text_language)0;
	/* 2nd char */
	if(str[1] >= 'a' && str[1] <= 'z')
		lang = (fz_text_language)(lang + (27*(str[1] - 'a' + 1)));
	else if(str[1] >= 'A' && str[1] <= 'Z')
		lang = (fz_text_language)(lang + (27*(str[1] - 'A' + 1)));
	else
		return (fz_text_language)0; /* There are no valid 1 char language codes */
	/* 3nd char */
	if(str[2] >= 'a' && str[2] <= 'z')
		lang = (fz_text_language)(lang + (27*27*(str[2] - 'a' + 1)));
