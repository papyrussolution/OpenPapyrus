...
static void transform256(uint state[8], uint data[16])
{
	const uint * K = SHA256_K;
	uint W[16];
	uint T[8];
	uint j;
...
void fz_sha256_update(fz_sha256 * context, const uchar * input, size_t inlen)
{
	...
	while(inlen > 0) {
		const uint copy_start = context->count[0] & 0x3F;
...
void fz_sha256_final(fz_sha256 * context, uchar digest[32])
{
	...
	memzero(context, sizeof(fz_sha256));
}
...
static void transform512(uint64_t state[8], uint64_t data[16])
{
...
void fz_sha512_update(fz_sha512 * context, const uchar * input, size_t inlen)
{
	/* Copy the input data into a properly aligned temporary buffer.
	 * This way we can be called with arbitrarily sized buffers
	 ...
	 * on architectures that don't allow unaligned memory access. */
	while(inlen > 0) {
		const uint copy_start = context->count[0] & 0x7F;
...
void fz_sha512_final(fz_sha512 * context, uchar digest[64])
{
	/* Add padding as described in RFC 3174 (it describes SHA-1 but
	 * the same padding style is used for SHA-512 too). */
	uint j = context->count[0] & 0x7F;
	context->buffer.u8[j++] = 0x80;
	while(j != 112) {
		if(j == 128) {
			transform512(context->state, context->buffer.u64);
	...
	memzero(context, sizeof(fz_sha512));
}
...
void fz_sha384_update(fz_sha384 * context, const uchar * input, size_t inlen)
{
	fz_sha512_update(context, input, inlen);
}

void fz_sha384_final(fz_sha384 * context, uchar digest[64])
{
