...
#define FROM_UNICODE(ENC) \
	int l = 0; \
	int r = SIZEOFARRAY(ENC ## _from_unicode) - 1; \
	if(u < 128) \
		return u; \
	while(l <= r) \
	{ \
...
{
	int l = 0;
	int r = SIZEOFARRAY(agl_dup_offsets) / 2 - 1;
	while(l <= r) {
		int m = (l + r) >> 1;
...
const char * fz_glyph_name_from_unicode_sc(int u)
{
	int l = 0;
	int r = SIZEOFARRAY(glyph_name_from_unicode_sc) / 2 - 1;
	while(l <= r) {
		int m = (l + r) >> 1;
