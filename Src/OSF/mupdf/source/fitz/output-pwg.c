...
{
	static const uchar pwgsig[4] = { 'R', 'a', 'S', '2' };
	/* Sync word */
	fz_write_data(ctx, out, pwgsig, 4);
}

static void pwg_page_header(fz_context * ctx, fz_output * out, const fz_pwg_options * pwg,
...
static void pwg_write_mono_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
...
static void pwg_write_mono_band(fz_context * ctx,
...
static void pwg_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
	pwg_band_writer * writer = (pwg_band_writer*)writer_;
...
static void pwg_write_band(fz_context * ctx,
    fz_band_writer * writer_,
    int stride,
    int band_start,
    int band_height,
...
static void warn_if_long(fz_context * ctx, const char * str, size_t ret)
{
	if(ret > 0)
...
static fz_device * pwg_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
	fz_pwg_writer * wri = (fz_pwg_writer*)wri_;
	return fz_new_draw_device_with_options(ctx, &wri->draw, mediabox, &wri->pixmap);
}

static void pwg_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	fz_pwg_writer * wri = (fz_pwg_writer*)wri_;
...
static void pwg_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pwg_writer * wri = (fz_pwg_writer*)wri_;
	fz_close_output(ctx, wri->out);
}

static void pwg_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pwg_writer * wri = (fz_pwg_writer*)wri_;
	fz_drop_pixmap(ctx, wri->pixmap);
...
{
	fz_pwg_writer * wri = fz_new_derived_document_writer(ctx,
		fz_pwg_writer,
		pwg_begin_page,
		pwg_end_page,
		pwg_close_writer,
