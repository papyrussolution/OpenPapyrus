...
static inline int getcomponent(uchar * line, int x, int bpc)
{
	switch(bpc) {
...
static inline void putcomponent(uchar * buf, int x, int bpc, int value)
{
	switch(bpc) {
...
static void fz_predict_tiff(fz_predict * state, uchar * out, uchar * in)
{
...
static void fz_predict_png(fz_context * ctx, fz_predict * state, uchar * out, uchar * in, size_t len, int predictor)
{
	int bpp = state->bpp;
	size_t i;
	uchar * ref = state->ref;
...
static int next_predict(fz_context * ctx, fz_stream * stm, size_t len)
{
	fz_predict * state = (fz_predict *)stm->state;
...
static void close_predict(fz_context * ctx, void * state_)
{
...
fz_stream * fz_open_predict(fz_context * ctx, fz_stream * chain, int predictor, int columns, int colors, int bpc)
{
	...
	fz_try(ctx)
	{
		...
		state->in = (uchar*)Memento_label(fz_malloc(ctx, state->stride + 1), "predict_in");
		state->out = (uchar*)Memento_label(fz_malloc(ctx, state->stride), "predict_out");
