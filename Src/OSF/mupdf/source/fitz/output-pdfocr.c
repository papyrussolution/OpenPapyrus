...
void fz_write_pixmap_as_pdfocr(fz_context * ctx, fz_output * out, const fz_pixmap * pixmap, const fz_pdfocr_options * pdfocr)
{
#ifdef OCR_DISABLED
	fz_throw(ctx, FZ_ERROR_GENERIC, "No OCR support in this build");
#else
...
static int new_obj(fz_context * ctx, pdfocr_band_writer * writer)
{
...
static void pdfocr_write_header(fz_context * ctx, fz_band_writer * writer_, fz_colorspace * cs)
{
...
static void flush_strip(fz_context * ctx, pdfocr_band_writer * writer, int fill)
{
...
static void pdfocr_write_band(fz_context * ctx,
	...
	if(n == 1) {
		...
		for(y = band_height; y > 0; y--) {
			...
			if(writer->ocrbitmap->w - w)
				memzero(d + w, writer->ocrbitmap->w - w);
...
static int pdfocr_progress(fz_context * ctx, void * arg, int prog)
{
	char_callback_data_t * cb = (char_callback_data_t*)arg;
	pdfocr_band_writer * writer = cb->writer;
...
static void pdfocr_write_trailer(fz_context * ctx, fz_band_writer * writer_)
{
	pdfocr_band_writer * writer = (pdfocr_band_writer*)writer_;
	fz_output * out = writer->super.out;
	int w = writer->super.w;
...
static void pdfocr_drop_band_writer(fz_context * ctx, fz_band_writer * writer_)
{
	pdfocr_band_writer * writer = (pdfocr_band_writer*)writer_;
	fz_output * out = writer->super.out;
	int i;
...
void fz_pdfocr_band_writer_set_progress(fz_context * ctx,
    fz_band_writer * writer_,
    int (*progress)(fz_context *, void *, int),
    void * progress_arg)
{
...
void fz_save_pixmap_as_pdfocr(fz_context * ctx, fz_pixmap * pixmap, char * filename, int append, const fz_pdfocr_options * pdfocr)
{
#ifdef OCR_DISABLED
	fz_throw(ctx, FZ_ERROR_GENERIC, "No OCR support in this build");
#else
...
static fz_device * pdfocr_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
	fz_pdfocr_writer * wri = (fz_pdfocr_writer*)wri_;
	return fz_new_draw_device_with_options(ctx, &wri->draw, mediabox, &wri->pixmap);
}

static void pdfocr_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	fz_pdfocr_writer * wri = (fz_pdfocr_writer*)wri_;
...
static void pdfocr_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pdfocr_writer * wri = (fz_pdfocr_writer*)wri_;
...
static void pdfocr_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_pdfocr_writer * wri = (fz_pdfocr_writer*)wri_;
...
fz_document_writer * fz_new_pdfocr_writer_with_output(fz_context * ctx, fz_output * out, const char * options)
{
	...
	fz_throw(ctx, FZ_ERROR_GENERIC, "No OCR support in this build");
#else
	fz_pdfocr_writer * wri = fz_new_derived_document_writer(ctx,
		fz_pdfocr_writer,
		pdfocr_begin_page,
		pdfocr_end_page,
		pdfocr_close_writer,
