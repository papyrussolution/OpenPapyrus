...
typedef struct {
	...
	uchar first_char;       /* first token of string */
} lzw_code;
...
static int next_lzwd(fz_context * ctx, fz_stream * stm, size_t len)
{
	fz_lzwd * lzw = (fz_lzwd*)stm->state;
	lzw_code * table = lzw->table;
	uchar * buf = lzw->buffer;
	uchar * p = buf;
	uchar * ep;
	uchar * s;
	int codelen;
...
static void close_lzwd(fz_context * ctx, void * state_)
{
