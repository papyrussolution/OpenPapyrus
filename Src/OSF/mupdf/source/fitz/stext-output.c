...
static void font_family_name(fz_context * ctx, fz_font * font, char * buf, int size, int is_mono, int is_serif)
{
...
static void fz_print_style_begin_html(fz_context * ctx, fz_output * out, fz_font * font, float size, int sup, int color)
{
	char family[80];
...
static void fz_print_style_end_html(fz_context * ctx, fz_output * out, fz_font * font, float size, int sup)
{
	int is_mono = fz_font_is_monospaced(ctx, font);
	int is_bold = fz_font_is_bold(ctx, font);
	int is_italic = fz_font_is_italic(ctx, font);
...
static void fz_print_stext_image_as_html(fz_context * ctx, fz_output * out, fz_stext_block * block)
{
	int x = block->bbox.x0;
	int y = block->bbox.y0;
...
void fz_print_stext_block_as_html(fz_context * ctx, fz_output * out, fz_stext_block * block)
{
	fz_stext_line * line;
	fz_stext_char * ch;
	int x, y;
...
static void fz_print_stext_image_as_xhtml(fz_context * ctx, fz_output * out, fz_stext_block * block)
{
	int w = block->bbox.x1 - block->bbox.x0;
	int h = block->bbox.y1 - block->bbox.y0;
...
static void fz_print_style_begin_xhtml(fz_context * ctx, fz_output * out, fz_font * font, int sup)
{
	int is_mono = fz_font_is_monospaced(ctx, font);
	int is_bold = fz_font_is_bold(ctx, font);
	int is_italic = fz_font_is_italic(ctx, font);
...
static void fz_print_style_end_xhtml(fz_context * ctx, fz_output * out, fz_font * font, int sup)
{
	int is_mono = fz_font_is_monospaced(ctx, font);
	int is_bold = fz_font_is_bold(ctx, font);
	int is_italic = fz_font_is_italic(ctx, font);
			...
			case FZ_STEXT_BLOCK_TEXT:
			    ...
			    for(line = block->u.t.first_line; line; line = line->next) {
				    ...
				    for(ch = line->first_char; ch; ch = ch->next) {
					    ...
					    fz_write_printf(ctx,
						out,
						"<char quad=\"%g %g %g %g %g %g %g %g\" x=\"%g\" y=\"%g\" color=\"#%06x\" c=\"",
						ch->quad.ul.x,
						ch->quad.ul.y,
						ch->quad.ur.x,
						ch->quad.ur.y,
...
static void text_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	fz_text_writer * wri = (fz_text_writer*)wri_;
	fz_drop_stext_page(ctx, wri->page);
...
fz_document_writer * fz_new_text_writer_with_output(fz_context * ctx, const char * format, fz_output * out, const char * options)
{
	...
	fz_try(ctx)
	{
		...
		if(sstreq(format, "text"))
			wri->format = FZ_FORMAT_TEXT;
		else if(sstreq(format, "html"))
			wri->format = FZ_FORMAT_HTML;
		else if(sstreq(format, "xhtml"))
			wri->format = FZ_FORMAT_XHTML;
		else if(sstreq(format, "stext"))
			wri->format = FZ_FORMAT_STEXT_XML;
		else if(sstreq(format, "stext.xml"))
			wri->format = FZ_FORMAT_STEXT_XML;
		else if(sstreq(format, "stext.json")) {
