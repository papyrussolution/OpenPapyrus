...
{
	fz_css_rule * rule = (fz_css_rule *)fz_pool_alloc(ctx, pool, sizeof *rule);
...
{
	fz_css_selector * sel = (fz_css_selector *)fz_pool_alloc(ctx, pool, sizeof *sel);
...
{
	fz_css_condition * cond = (fz_css_condition *)fz_pool_alloc(ctx, pool, sizeof *cond);
	cond->type = type;
...
static fz_css_property * fz_new_css_property(fz_context * ctx, fz_pool * pool, const char * name, fz_css_value * value, int spec)
{
	...
	if(info) {
		fz_css_property * prop = (fz_css_property *)fz_pool_alloc(ctx, pool, sizeof *prop);
		prop->name = info->key;
		prop->value = value;
		prop->spec = spec;
		prop->important = 0;
...
{
	fz_css_value * val = (fz_css_value *)fz_pool_alloc(ctx, pool, sizeof *val);
	val->type = type;
	val->data = NULL;
	val->args = NULL;
	val->next = NULL;
...
static fz_css_value * parse_term(struct lexbuf * buf)
{
	...
	if(buf->lookahead == '+' || buf->lookahead == '-') {
		...
		if(sign < 0) {
			...
			v->data = (char *)fz_pool_alloc(buf->ctx, buf->pool, strlen(buf->string) + 2);
			v->data[0] = '-';
...
static fz_css_rule * parse_stylesheet(struct lexbuf * buf, fz_css_rule * chain)
{
	...
	while(buf->lookahead != EOF) {
		if(accept(buf, '@')) {
			...
			else if(buf->lookahead == CSS_KEYWORD && sstreq(buf->string, "font-face")) {
				next(buf);
