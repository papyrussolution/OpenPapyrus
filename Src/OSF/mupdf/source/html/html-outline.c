...
static int has_same_href(fz_html_box * box, const char * old_href)
{
	while(box) {
		const char * href = box->href;
		if(href)
			return sstreq(old_href, href);
		box = box->up;
	}
	return 0;
}

static fz_link * load_link_flow(fz_context * ctx,
    fz_html_flow * flow,
    fz_link * head,
    int page,
    float page_h,
...
static fz_link * load_link_box(fz_context * ctx,
    fz_html_box * box,
    fz_link * head,
    int page,
    float page_h,
...
fz_link * fz_load_html_links(fz_context * ctx, fz_html * html, int page, const char * file)
{
	fz_link * link, * head;
	char dir[2048];
...
static fz_html_flow * find_first_content(fz_html_box * box)
{
	while(box) {
...
static float find_flow_target(fz_html_flow * flow, const char * id)
{
	while(flow) {
		if(flow->box->id && sstreq(id, flow->box->id))
			return flow->y;
		flow = flow->next;
	}
	return -1;
...
static float find_box_target(fz_html_box * box, const char * id)
{
	float y;
	while(box) {
		if(box->id && sstreq(id, box->id)) {
...
static fz_html_flow * make_flow_bookmark(fz_context * ctx, fz_html_flow * flow, float y)
{
	while(flow) {
		if(flow->y >= y)
			return flow;
...
static fz_html_flow * make_box_bookmark(fz_context * ctx, fz_html_box * box, float y)
{
	fz_html_flow * mark;
	while(box) {
		if(box->type == BOX_FLOW) {
...
static int lookup_flow_bookmark(fz_context * ctx, fz_html_flow * flow, fz_html_flow * mark)
{
	while(flow) {
		if(flow == mark)
			return 1;
...
static int lookup_box_bookmark(fz_context * ctx, fz_html_box * box, fz_html_flow * mark)
{
	while(box) {
		if(box->type == BOX_FLOW) {
...
static void cat_html_flow(fz_context * ctx, fz_buffer * cat, fz_html_flow * flow)
{
	while(flow) {
		switch(flow->type)
		{
...
static void cat_html_box(fz_context * ctx, fz_buffer * cat, fz_html_box * box)
{
	while(box) {
...
static const char * cat_html_text(fz_context * ctx, struct outline_parser * x, fz_html_box * box)
{
	if(!x->cat)
...
static void add_html_outline(fz_context * ctx, struct outline_parser * x, fz_html_box * box)
{
	fz_outline * node;
	char buf[100];
...
static void load_html_outline(fz_context * ctx, struct outline_parser * x, fz_html_box * box)
{
	while(box) {
		if(box->heading)
...
fz_outline * fz_load_html_outline(fz_context * ctx, fz_html * html)
{
