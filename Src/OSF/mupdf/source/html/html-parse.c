...
static fz_html_flow * add_flow(fz_context * ctx, fz_pool * pool, fz_html_box * top, fz_html_box * inline_box, int type, int extras)
{
	...
	fz_html_flow * flow = (fz_html_flow *)fz_pool_alloc(ctx, pool, size);
	flow->type = type;
	flow->expand = 0;
	flow->bidi_level = 0;
	flow->markup_lang = 0;
...
static void add_flow_word(fz_context * ctx,
    fz_pool * pool,
    fz_html_box * top,
    fz_html_box * inline_box,
    const char * a,
    const char * b,
    int lang)
{
...
static fz_html_flow * split_flow(fz_context * ctx, fz_pool * pool, fz_html_flow * flow, size_t offset)
{
	...
	len = strlen(text);
	new_flow = (fz_html_flow *)fz_pool_alloc(ctx, pool, offsetof(fz_html_flow, content) + len+1);
...
{
	return (fz_html *)fz_keep_storable(ctx, &html->storable);
}

static fz_html_box * new_box(fz_context * ctx, fz_pool * pool, fz_bidi_direction markup_dir)
{
	fz_html_box * box = (fz_html_box *)fz_pool_alloc(ctx, pool, sizeof *box);
...
{
	fz_html_box * box = (fz_html_box *)fz_pool_alloc(ctx, pool, offsetof(fz_html_box, padding));
	init_box(ctx, box, markup_dir);
	return box;
}
...
static void insert_inline_box(fz_context * ctx, fz_html_box * box, fz_html_box * top, int markup_dir, struct genstate * g)
{
	if(top->type == BOX_FLOW || top->type == BOX_INLINE) {
		...
		if(top->next && top->next->type == BOX_FLOW) {
			...
			fz_css_style style;
			fz_html_box * flow = new_short_box(ctx, g->pool, (fz_bidi_direction)markup_dir);
...
static fz_html_box * generate_boxes(fz_context * ctx,
    fz_xml * node,
    fz_html_box * top,
		...
		if(tag) {
			...
			if(tag[0]=='b' && tag[1]=='r' && tag[2]==0) {
				...
				if(top->type != BOX_INLINE) {
					...
					fz_css_style style;
					box = new_short_box(ctx, g->pool, (fz_bidi_direction)markup_dir);
					fz_default_css_style(ctx, &style);
				...
				if(src) {
					...
					if(display == DIS_BLOCK) {
						fz_html_box * imgbox;
						box = new_box(ctx, g->pool, (fz_bidi_direction)markup_dir);
						box->style = fz_css_enlist(ctx, &style, &g->styles, g->pool);
						top = insert_block_box(ctx, box, top);
						imgbox = new_short_box(ctx, g->pool, (fz_bidi_direction)markup_dir);
				...
				if(src && src[0] == '#') {
					fz_image * img = (fz_image *)fz_tree_lookup(ctx, g->images, src+1);
					if(display == DIS_BLOCK) {
						fz_html_box * imgbox;
						box = new_box(ctx, g->pool, (fz_bidi_direction)markup_dir);
						box->style = fz_css_enlist(ctx, &style, &g->styles, g->pool);
				...
				if(dir) {
					...
					else if(sstreq(dir, "rtl"))
						child_dir = FZ_BIDI_RTL;
					else if(sstreq(dir, "ltr"))
				...
				if(display == DIS_INLINE)
					box = new_short_box(ctx, g->pool, (fz_bidi_direction)child_dir);
				else
					box = new_box(ctx, g->pool, (fz_bidi_direction)child_dir);
				box->style = fz_css_enlist(ctx, &style, &g->styles, g->pool);
				...
				if(fz_xml_down(node)) {
					...
					if(sstreq(tag, "ul") || sstreq(tag, "ol"))
						child_counter = 0;
					else if(sstreq(tag, "section"))
						++child_section;
...
static void detect_box_directionality(fz_context * ctx, fz_pool * pool, uni_buf * buffer, fz_html_box * box)
{
	while(box) {
		if(box->flow_head)
			box->markup_dir = detect_flow_directionality(ctx, pool, buffer, (fz_bidi_direction)box->markup_dir, box->flow_head);
...
static void fix_nexts(fz_html_box * box)
{
	while(box) {
		if(box->down)
			fix_nexts(box->down);
...
static fz_html * fz_parse_html_imp(fz_context * ctx,
    ...
    int try_xml, int try_html5)
{
	...
	fz_try(ctx)
	{
		...
		html = (fz_html *)fz_pool_alloc(ctx, g.pool, sizeof *html);
...
fz_html * fz_parse_fb2(fz_context * ctx,
    fz_html_font_set * set,
    fz_archive * zip,
    const char * base_uri,
    fz_buffer * buf,
...
fz_html * fz_parse_html5(fz_context * ctx,
    fz_html_font_set * set,
    fz_archive * zip,
    const char * base_uri,
    fz_buffer * buf,
...
fz_html * fz_parse_xhtml(fz_context * ctx,
    fz_html_font_set * set,
    fz_archive * zip,
    const char * base_uri,
    fz_buffer * buf,
...
fz_html * fz_store_html(fz_context * ctx, fz_html * html, void * doc, int chapter)
{
	...
	fz_try(ctx)
	{
		...
		other_html = (fz_html *)fz_store_item(ctx, key, html, fz_html_size(ctx, html), &fz_html_store_type);
		if(other_html) {
			fz_drop_html(ctx, html);
			html = other_html;
		}
...
fz_html * fz_find_html(fz_context * ctx, void * doc, int chapter)
{
	...
	key.doc = doc;
	key.chapter_num = chapter;
	return (fz_html *)fz_find_item(ctx, &fz_drop_html_imp, &key, &fz_html_store_type);
}
