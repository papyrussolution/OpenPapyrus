...
static int quick_ligature_mov(fz_context * ctx, string_walker * walker, uint i, uint n, int unicode)
{
	for(uint k = i + n + 1; k < walker->glyph_count; ++k) {
...
static int quick_ligature(fz_context * ctx, string_walker * walker, uint i)
{
...
static void init_string_walker(fz_context * ctx, string_walker * walker, hb_buffer_t * hb_buf, int rtl,
    fz_font * font, int script, int language, int small_caps, const char * text)
{
	walker->ctx = ctx;
...
{
	fz_hb_lock(ctx);
	hb_font_destroy((hb_font_t *)handle);
	fz_hb_unlock(ctx);
}
...
static int walk_string(string_walker * walker)
{
	...
	fz_try(ctx)
	{
		face = (FT_Face)fz_font_ft_face(ctx, walker->font);
		...
		if(walker->language) {
			fz_string_from_text_language(lang, (fz_text_language)walker->language);
...
static void layout_line(fz_context * ctx, float indent, float page_w, float line_w, int align, fz_html_flow * start, fz_html_flow * end,
    fz_html_box * box, float baseline, float line_h)
{
	...
	if(align == _TA_JUSTIFY) {
		justify = slop / (i ? i : 1);
	}
	else if(align == _TA_RIGHT)
		x += slop;
	else if(align == _TA_CENTER)
		x += slop / 2;
...
static void flush_line(fz_context * ctx,
    fz_html_box * box,
    float page_h,
    float page_w,
    float line_w,
	...
	while(node) {
		...
		if(node->type == FLOW_BREAK || (line_w + nonbreak_w > box->w && candidate)) {
			...
			if(candidate->type == FLOW_BREAK)
				line_align = (align == _TA_JUSTIFY) ? _TA_LEFT : align;
			else
				line_align = align;
...
void fz_layout_html(fz_context * ctx, fz_html * html, float w, float h, float em)
{
	...
	fz_var(hb_buf);
	fz_var(unlocked);
	// If we're already laid out to the specifications we need, nothing to do. 
					...
					while(walker.start + k < walker.end) {
						...
						for(i = 0; i < walker.glyph_count; ++i) {
							if(walker.glyph_info[i].cluster == k) {
								...
								fz_show_glyph(ctx, text, walker.font, trm, walker.glyph_info[i].codepoint, c,
								    0, node->bidi_level, (fz_bidi_direction)box->markup_dir, (fz_text_language)node->markup_lang);
						...
						if(c != -1) {
							fz_show_glyph(ctx, text, walker.font, trm, -1, c,
							    0, node->bidi_level, (fz_bidi_direction)box->markup_dir, (fz_text_language)node->markup_lang);
						}
						k += n;
					}
...
static void draw_rect(fz_context * ctx,
    fz_device * dev,
    fz_matrix ctm,
    float page_top,
    fz_css_color color,
...
static void draw_block_box(fz_context * ctx,
    fz_html_box * box,
    float page_top,
    float page_bot,
    fz_device * dev,
...
static void draw_box(fz_context * ctx,
    fz_html_box * box,
    float page_top,
    float page_bot,
    fz_device * dev,
...
void fz_draw_html(fz_context * ctx, fz_device * dev, fz_matrix ctm, fz_html * html, int page)
{
	...
	fz_var(hb_buf);
	fz_var(unlocked);
	draw_rect(ctx, dev, ctm, 0, html->root->style->background_color, 0, 0, html->page_w + html->page_margin[L] + html->page_margin[R],
