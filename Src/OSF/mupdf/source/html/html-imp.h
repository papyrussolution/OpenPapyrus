...
struct fz_css_style_s {
	...
	/* Ensure the extra bits in the bitfield are copied on structure copies. */
...
struct fz_html_box_s {
	...
	float em;
	/* During construction, 'next' plays double duty; as well as its normal meaning of 'next sibling', the last
	   sibling has next meaning "the last of my children". We correct this as a post-processing pass after
	   construction. */
...
static inline int fz_html_box_has_boxes(fz_html_box * box)
{
...
struct fz_html_flow_s {
	unsigned int type : 3; /* What type of node */
	unsigned int expand : 1; /* Whether this should expand during justification */
	unsigned int breaks_line : 1; /* Whether this node is currently taken as a line break */
	unsigned int bidi_level : 7; /* Direction setting for text - UAX#9 says 125 is the max */
	unsigned int script : 8; /* The script detected by the bidi code. */
	unsigned int markup_lang : 15; /* Whether the markup specifies a given language. */
	float x, y, w, h;
...
/*
        Lookup style in the splay tree, returning a pointer to the found instance if there is one, creating and
