...
static fz_location epub_resolve_link(fz_context * ctx, fz_document * doc_, const char * dest, float * xp, float * yp)
{
...
static fz_rect epub_bound_page(fz_context * ctx, fz_page * page_)
{
	epub_page * page = (epub_page*)page_;
...
static char * find_metadata(fz_context * ctx, fz_xml * metadata, const char * key)
{
...
static int epub_lookup_metadata(fz_context * ctx, fz_document * doc_, const char * key, char * buf, int size)
{
	epub_document * doc = (epub_document*)doc_;
	if(sstreq(key, FZ_META_FORMAT))
		return 1 + (int)fz_strlcpy(buf, "EPUB", size);
	if(sstreq(key, FZ_META_INFO_TITLE) && doc->dc_title)
		return 1 + (int)fz_strlcpy(buf, doc->dc_title, size);
	if(sstreq(key, FZ_META_INFO_AUTHOR) && doc->dc_creator)
...
static fz_document * epub_open_accel_document_with_stream(fz_context * ctx, fz_stream * file, fz_stream * accel)
{
	return epub_init(ctx, fz_open_zip_archive_with_stream(ctx, file), accel);
}

static fz_document * epub_open_accel_document(fz_context * ctx, const char * filename, const char * accel)
{
...
static fz_document * epub_open_document_with_stream(fz_context * ctx, fz_stream * file)
{
	return epub_init(ctx, fz_open_zip_archive_with_stream(ctx, file), NULL);
}

static fz_document * epub_open_document(fz_context * ctx, const char * filename)
{
	return epub_open_accel_document(ctx, filename, NULL);
}

static int epub_recognize(fz_context * doc, const char * magic)
{
