...
static fz_font * fz_load_html_default_font(fz_context * ctx, fz_html_font_set * set, const char * family, int is_bold, int is_italic)
{
	int is_mono = sstreq(family, "monospace");
	int is_sans = sstreq(family, "sans-serif");
	...
	if(!set->fonts[idx]) {
		int size;
		const uchar * data = fz_lookup_builtin_font(ctx, real_family, is_bold, is_italic, &size);
		if(!data)
    ...
    const char * family, int is_bold, int is_italic, int is_small_caps)
{
	...
	for(custom = set->custom; custom; custom = custom->next) {
		if(sstreq(family, custom->family)) {
			int score =
	...
	if(sstreq(family, "monospace") || sstreq(family, "sans-serif") || sstreq(family, "serif"))
...
void fz_drop_html_font_set(fz_context * ctx, fz_html_font_set * set)
{
	...
	for(i = 0; i < SIZEOFARRAYi(set->fonts); ++i)
