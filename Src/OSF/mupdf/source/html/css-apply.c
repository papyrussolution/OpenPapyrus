...
void fz_match_css_at_page(fz_context * ctx, fz_css_match * match, fz_css * css)
{
	...
	for(rule = css->rule; rule; rule = rule->next) {
		sel = rule->selector;
		while(sel) {
			if(sel->name && sstreq(sel->name, "@page")) {
				for(prop = rule->declaration; prop; prop = prop->next)
					add_property(match, prop->name, prop->value, selector_specificity(sel, prop->important));
				break;
			}
...
void fz_add_css_font_face(fz_context * ctx, fz_html_font_set * set, fz_archive * zip, const char * base_uri, fz_css_property * declaration)
{
	...
	is_small_caps = sstreq(variant, "small-caps");
...
void fz_add_css_font_faces(fz_context * ctx, fz_html_font_set * set, fz_archive * zip, const char * base_uri, fz_css * css)
{
	...
	for(rule = css->rule; rule; rule = rule->next) {
		if(!rule->loaded) {
			...
			sel = rule->selector;
			while(sel) {
				if(sel->name && sstreq(sel->name, "@font-face")) {
...
static fz_css_value * value_from_inheritable_property(fz_css_match * match, int name)
{
	...
	if(match->up) {
		if(value && sstreq(value->data, "inherit"))
...
static fz_css_number border_width_from_property(fz_css_match * match, int property)
{
	...
	if(value) {
		if(sstreq(value->data, "thin"))
			return make_number(1, N_LENGTH);
		if(sstreq(value->data, "medium"))
			return make_number(2, N_LENGTH);
		if(sstreq(value->data, "thick"))
...
static int border_style_from_property(fz_css_match * match, int property)
{
	fz_css_value * value = value_from_property(match, property);
	if(value) {
		if(sstreq(value->data, "none")) return _BS_NONE;
		else if(sstreq(value->data, "hidden")) return _BS_NONE;
		else if(sstreq(value->data, "solid")) return _BS_SOLID;
	}
	return _BS_NONE;
}
...
hex_color:
	...
	if(value->type == '(' && sstreq(value->data, "rgb")) {
	...
	if(value->type == '(' && sstreq(value->data, "rgba")) {
...
int fz_get_css_match_display(fz_css_match * match)
{
	...
	if(value) {
		if(sstreq(value->data, "none"))
			return DIS_NONE;
		if(sstreq(value->data, "inline"))
			return DIS_INLINE;
		if(sstreq(value->data, "block"))
			return DIS_BLOCK;
		if(sstreq(value->data, "list-item"))
			return DIS_LIST_ITEM;
		if(sstreq(value->data, "inline-block"))
			return DIS_INLINE_BLOCK;
		if(sstreq(value->data, "table"))
			return DIS_TABLE;
		if(sstreq(value->data, "table-row"))
			return DIS_TABLE_ROW;
		if(sstreq(value->data, "table-cell"))
...
static int white_space_from_property(fz_css_match * match)
{
	...
	if(value) {
		if(sstreq(value->data, "normal")) return WS_NORMAL;
		else if(sstreq(value->data, "pre")) return WS_PRE;
		else if(sstreq(value->data, "nowrap")) return WS_NOWRAP;
		else if(sstreq(value->data, "pre-wrap")) return WS_PRE_WRAP;
		else if(sstreq(value->data, "pre-line")) return WS_PRE_LINE;
	}
	return WS_NORMAL;
}
...
static int visibility_from_property(fz_css_match * match)
{
	...
	if(value) {
		if(sstreq(value->data, "visible")) return V_VISIBLE;
		else if(sstreq(value->data, "hidden")) return V_HIDDEN;
		else if(sstreq(value->data, "collapse")) return V_COLLAPSE;
	}
	return V_VISIBLE;
}
...
static int page_break_from_property(fz_css_match * match, int prop)
{
	...
	if(value) {
		if(sstreq(value->data, "auto")) return PB_AUTO;
		else if(sstreq(value->data, "always")) return PB_ALWAYS;
		else if(sstreq(value->data, "avoid")) return PB_AVOID;
		else if(sstreq(value->data, "left")) return PB_LEFT;
		else if(sstreq(value->data, "right")) return PB_RIGHT;
	}
	return PB_AUTO;
}
...
void fz_apply_css_style(fz_context * ctx, fz_html_font_set * set, fz_css_style * style, fz_css_match * match)
{
	...
	if(value) {
		if(sstreq(value->data, "left")) style->text_align = _TA_LEFT;
		else if(sstreq(value->data, "right")) style->text_align = _TA_RIGHT;
		else if(sstreq(value->data, "center")) style->text_align = _TA_CENTER;
		else if(sstreq(value->data, "justify")) style->text_align = _TA_JUSTIFY;
	}

	value = value_from_property(match, PRO_VERTICAL_ALIGN);
	if(value) {
		if(sstreq(value->data, "baseline")) style->vertical_align = VA_BASELINE;
		else if(sstreq(value->data, "sub")) style->vertical_align = VA_SUB;
		else if(sstreq(value->data, "super")) style->vertical_align = VA_SUPER;
		else if(sstreq(value->data, "top")) style->vertical_align = VA_TOP;
		else if(sstreq(value->data, "bottom")) style->vertical_align = VA_BOTTOM;
		else if(sstreq(value->data, "text-top")) style->vertical_align = VA_TEXT_TOP;
		else if(sstreq(value->data, "text-bottom")) style->vertical_align = VA_TEXT_BOTTOM;
	}

	value = value_from_property(match, PRO_FONT_SIZE);
	if(value) {
		if(sstreq(value->data, "xx-large")) style->font_size = make_number(1.73f, N_SCALE);
		else if(sstreq(value->data, "x-large")) style->font_size = make_number(1.44f, N_SCALE);
		else if(sstreq(value->data, "large")) style->font_size = make_number(1.2f, N_SCALE);
		else if(sstreq(value->data, "medium")) style->font_size = make_number(1.0f, N_SCALE);
		else if(sstreq(value->data, "small")) style->font_size = make_number(0.83f, N_SCALE);
		else if(sstreq(value->data, "x-small")) style->font_size = make_number(0.69f, N_SCALE);
		else if(sstreq(value->data, "xx-small")) style->font_size = make_number(0.69f, N_SCALE);
		else if(sstreq(value->data, "larger")) style->font_size = make_number(1.2f, N_SCALE);
		else if(sstreq(value->data, "smaller")) style->font_size = make_number(1/1.2f, N_SCALE);
	...
	if(value) {
		if(sstreq(value->data, "none")) style->list_style_type = LST_NONE;
		else if(sstreq(value->data, "disc")) style->list_style_type = LST_DISC;
		else if(sstreq(value->data, "circle")) style->list_style_type = LST_CIRCLE;
		else if(sstreq(value->data, "square")) style->list_style_type = LST_SQUARE;
		else if(sstreq(value->data, "decimal")) style->list_style_type = LST_DECIMAL;
		else if(sstreq(value->data, "decimal-leading-zero")) style->list_style_type = LST_DECIMAL_ZERO;
		else if(sstreq(value->data, "lower-roman")) style->list_style_type = LST_LC_ROMAN;
		else if(sstreq(value->data, "upper-roman")) style->list_style_type = LST_UC_ROMAN;
		else if(sstreq(value->data, "lower-greek")) style->list_style_type = LST_LC_GREEK;
		else if(sstreq(value->data, "upper-greek")) style->list_style_type = LST_UC_GREEK;
		else if(sstreq(value->data, "lower-latin")) style->list_style_type = LST_LC_LATIN;
		else if(sstreq(value->data, "upper-latin")) style->list_style_type = LST_UC_LATIN;
		else if(sstreq(value->data, "lower-alpha")) style->list_style_type = LST_LC_ALPHA;
		else if(sstreq(value->data, "upper-alpha")) style->list_style_type = LST_UC_ALPHA;
		else if(sstreq(value->data, "armenian")) style->list_style_type = LST_ARMENIAN;
		else if(sstreq(value->data, "georgian")) style->list_style_type = LST_GEORGIAN;
	}
...

	{
		...
		style->small_caps = sstreq(font_variant, "small-caps");
...
const fz_css_style * fz_css_enlist(fz_context * ctx, const fz_css_style * style, fz_css_style_splay ** tree, fz_pool * pool)
{
	...
	if(*current == NULL) {
		x = *current = (fz_css_style_splay *)fz_pool_alloc(ctx, pool, sizeof(*y));
		x->style = *style;
		x->up = y;
		x->lt = NULL;
		x->gt = NULL;
