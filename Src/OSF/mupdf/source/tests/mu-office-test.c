...
static void load_progress(void * cookie, int pages_loaded, int complete)
{
	assert((intptr_t)cookie == 1234);
	slfprintf_stderr("load_progress: pages_loaded=%d complete=%d\n", pages_loaded, complete);
	if(complete)
		(void)ReleaseSemaphore(loaded, 1, NULL);
}

static void load_error(void * cookie, MuOfficeDocErrorType error)
{
	assert((intptr_t)cookie == 1234);
	slfprintf_stderr("load_error: error=%d\n", error);
}

static void render_progress(void * cookie, MuError error)
{
	assert((intptr_t)cookie == 5678);
	slfprintf_stderr("render_progress: error=%d\n", error);
	(void)ReleaseSemaphore(loaded, 1, NULL);
}

static int test_async(MuOfficeLib * mu)
{
	MuOfficeDoc * doc;
	MuError err;
	int count;
	...
	if(err) {
		slfprintf_stderr("Failed to count pages: error=%d\n", err);
		return EXIT_FAILURE;
	}
	slfprintf_stderr("%d Pages in document\n", count);

	/* Get a page */
	err = MuOfficeDoc_getPage(doc, 0, NULL, (void *)4321, &page);
	if(err) {
		slfprintf_stderr("Failed to get page: error=%d\n", err);
		return EXIT_FAILURE;
	}
	...
	if(err) {
		slfprintf_stderr("Failed to get page size: error=%d\n", err);
		return EXIT_FAILURE;
	}
	slfprintf_stderr("Page size = %g x %g\n", w, h);
	...
	if(err) {
		slfprintf_stderr("Page render failed: error=%d\n", err);
		return EXIT_FAILURE;
	}
...
static int test_sync(MuOfficeLib * mu)
{
	MuOfficeDoc * doc;
	MuError err;
	int count;
	...
	if(err) {
		slfprintf_stderr("Page render failed to complete: error=%d\n", err);
		return EXIT_FAILURE;
	}
...
int main(int argc, char ** argv)
{
	...
	if(err) {
		slfprintf_stderr("Failed to create lib instance: error=%d\n", err);
		return EXIT_FAILURE;
	}
...
static void save_png(const MuOfficeBitmap * bitmap, const char * filename)
{
	...
	fz_pixmap * pix;
	if(ctx == NULL) {
		slfprintf_stderr("save_png failed!\n");
		exit(EXIT_FAILURE);
	}
	pix = fz_new_pixmap_with_data(ctx, fz_device_rgb(ctx), bitmap->width, bitmap->height, NULL, 1, bitmap->lineSkip, (uchar *)bitmap->memptr);
	fz_try(ctx)
	{
