...
pdf_obj * pdf_find_font_resource(fz_context * ctx, pdf_document * doc, int type, int encoding, fz_font * item, uchar digest[16])
{
	pdf_obj * res;
	...
	digest[0] += type;
	digest[1] += encoding;
	res = (pdf_obj *)fz_hash_find(ctx, doc->resources.fonts, digest);
	if(res)
		pdf_keep_obj(ctx, res);
	return res;
}

pdf_obj * pdf_insert_font_resource(fz_context * ctx, pdf_document * doc, uchar digest[16], pdf_obj * obj)
{
	pdf_obj * res = (pdf_obj *)fz_hash_insert(ctx, doc->resources.fonts, digest, obj);
	if(res)
