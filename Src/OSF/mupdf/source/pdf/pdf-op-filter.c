...
static void copy_resource(fz_context * ctx, pdf_filter_processor * p, pdf_obj * key, const char * name)
{
	if(!isempty(name)) {
		pdf_obj * res = pdf_dict_get(ctx, p->old_rdb, key);
		pdf_obj * obj = pdf_dict_gets(ctx, res, name);
		if(obj) {
...
static void add_resource(fz_context * ctx, pdf_filter_processor * p, pdf_obj * key, const char * name, pdf_obj * val)
{
...
static void create_resource_name(fz_context * ctx, pdf_filter_processor * p, pdf_obj * key, const char * prefix, char * buf, int len)
{
	int i;
	pdf_obj * res = pdf_dict_get(ctx, p->new_rdb, key);
	if(!res)
...
static void filter_push(fz_context * ctx, pdf_filter_processor * p)
{
...
static int filter_pop(fz_context * ctx, pdf_filter_processor * p)
{
	filter_gstate * gstate = p->gstate;
...
static filter_gstate * gstate_to_update(fz_context * ctx, pdf_filter_processor * p)
{
	filter_gstate * gstate = p->gstate;
...
static void filter_flush(fz_context * ctx, pdf_filter_processor * p, int flush)
{
	...
	if(flush & FLUSH_COLOR_F) {
		if(gstate->pending.cs.cs == fz_device_gray(ctx) && !gstate->pending.sc.pat && !gstate->pending.sc.shd &&
		    gstate->pending.sc.n == 1 &&
		    (gstate->sent.cs.cs != fz_device_gray(ctx) || gstate->sent.sc.pat || gstate->sent.sc.shd || gstate->sent.sc.n != 1 ||
		    gstate->pending.sc.c[0] != gstate->sent.sc.c[0])) {
			if(p->chain->op_g)
				p->chain->op_g(ctx, p->chain, gstate->pending.sc.c[0]);
			goto done_sc;
		}
		if(gstate->pending.cs.cs == fz_device_rgb(ctx) && !gstate->pending.sc.pat && !gstate->pending.sc.shd &&
		    gstate->pending.sc.n == 3 &&
		    (gstate->sent.cs.cs != fz_device_rgb(ctx) || gstate->sent.sc.pat || gstate->sent.sc.shd || gstate->sent.sc.n != 3 ||
		    gstate->pending.sc.c[0] != gstate->sent.sc.c[0] ||
		...
		if(gstate->pending.cs.cs == fz_device_cmyk(ctx) && !gstate->pending.sc.pat && !gstate->pending.sc.shd &&
		    gstate->pending.sc.n == 4 &&
		    (gstate->sent.cs.cs != fz_device_cmyk(ctx) || gstate->sent.sc.pat || gstate->sent.sc.shd || gstate->pending.sc.n != 4 ||
		    gstate->pending.sc.c[0] != gstate->sent.sc.c[0] ||
		    gstate->pending.sc.c[1] != gstate->sent.sc.c[1] || gstate->pending.sc.c[2] != gstate->sent.sc.c[2] ||
		    gstate->pending.sc.c[3] != gstate->sent.sc.c[3])) {
			if(p->chain->op_k)
				p->chain->op_k(ctx,
				    p->chain,
				    gstate->pending.sc.c[0],
				    gstate->pending.sc.c[1],
				    gstate->pending.sc.c[2],
				    gstate->pending.sc.c[3]);
			goto done_sc;
		}
			...
			if(emit) {
				if(gstate->pending.sc.pat)
					if(p->chain->op_sc_pattern)
						p->chain->op_sc_pattern(ctx,
						    p->chain,
						    gstate->pending.sc.name,
						    gstate->pending.sc.pat,
						    gstate->pending.sc.n,
						    gstate->pending.sc.c);
...
done_sc:
	...
	if(flush & FLUSH_COLOR_S) {
		if(gstate->pending.CS.cs == fz_device_gray(ctx) && !gstate->pending.SC.pat && !gstate->pending.SC.shd &&
		    gstate->pending.SC.n == 1 &&
		    (gstate->sent.CS.cs != fz_device_gray(ctx) || gstate->sent.SC.pat || gstate->sent.SC.shd || gstate->sent.SC.n != 0 ||
		    gstate->pending.SC.c[0] != gstate->sent.SC.c[0])) {
			if(p->chain->op_G)
				p->chain->op_G(ctx, p->chain, gstate->pending.SC.c[0]);
			goto done_SC;
...
done_SC:
	...
	if(flush & FLUSH_TEXT) {
		...
		if(gstate->pending.text.font != gstate->sent.text.font ||
			...
			if(p->chain->op_Tf)
				p->chain->op_Tf(ctx, p->chain, (const char *)p->font_name, gstate->pending.text.font, gstate->pending.text.size);
		}
...
static int filter_show_char(fz_context * ctx, pdf_filter_processor * p, int cid, int * unicode)
{
	filter_gstate * gstate = p->gstate;
...
static void filter_show_space(fz_context * ctx, pdf_filter_processor * p, float tadj)
{
	filter_gstate * gstate = p->gstate;
	pdf_font_desc * fontdesc = gstate->pending.text.font;
...
static void walk_string(fz_context * ctx, int uni, int remove, editable_str * str)
{
	int rune;
 ...
 * the whole lot. */
static void mcid_char_imp(fz_context * ctx, pdf_filter_processor * p, tag_record * tr, int uni, int remove)
{
	if(tr->mcid_obj == NULL)
...
static void mcid_char(fz_context * ctx, pdf_filter_processor * p, int uni, int remove)
{
	tag_record * tr  = p->pending_tags;
	for(tr = p->pending_tags; tr; tr = tr->prev)
		mcid_char_imp(ctx, p, tr, uni, remove);
	for(tr = p->current_tags; tr; tr = tr->prev)
		mcid_char_imp(ctx, p, tr, uni, remove);
}

static void update_mcid(fz_context * ctx, pdf_filter_processor * p)
{
 ...
 * we hit the end).
 */
static void filter_string_to_segment(fz_context * ctx,
    pdf_filter_processor * p,
    uchar * buf,
    size_t len,
    size_t * pos,
    int * inc,
...
static void adjust_text(fz_context * ctx, pdf_filter_processor * p, float x, float y)
{
...
static void adjust_for_removed_space(fz_context * ctx, pdf_filter_processor * p)
{
	filter_gstate * gstate = p->gstate;
...
static void flush_adjustment(fz_context * ctx, pdf_filter_processor * p)
{
	pdf_obj * arr;
...
static void push_adjustment_to_array(fz_context * ctx, pdf_filter_processor * p, pdf_obj * arr)
{
	if(p->Tm_adjust == 0)
		return;
	pdf_array_push_real(ctx, arr, p->Tm_adjust * 1000);
...
static void filter_show_string(fz_context * ctx, pdf_filter_processor * p, uchar * buf, size_t len)
{
	filter_gstate * gstate = p->gstate;
	pdf_font_desc * fontdesc = gstate->pending.text.font;
...
static void filter_show_text(fz_context * ctx, pdf_filter_processor * p, pdf_obj * text)
{
	filter_gstate * gstate = p->gstate;
	pdf_font_desc * fontdesc = gstate->pending.text.font;
	int i, n;
	...
	if(pdf_is_string(ctx, text)) {
		filter_show_string(ctx, p, (uchar*)pdf_to_str_buf(ctx, text), pdf_to_str_len(ctx, text));
		return;
	}
	...
	fz_try(ctx)
	{
		for(i = 0; i < n; i++) {
			...
			if(pdf_is_string(ctx, item)) {
				uchar * buf = (uchar*)pdf_to_str_buf(ctx, item);
...
static void pdf_filter_w(fz_context * ctx, pdf_processor * proc, float linewidth)
{
...
static void pdf_filter_j(fz_context * ctx, pdf_processor * proc, int linejoin)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	gstate->pending.stroke.linejoin = (fz_linejoin)linejoin;
}

static void pdf_filter_J(fz_context * ctx, pdf_processor * proc, int linecap)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	gstate->pending.stroke.linecap = (fz_linecap)linecap;
}

static void pdf_filter_M(fz_context * ctx, pdf_processor * proc, float miterlimit)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
...
static void pdf_filter_d(fz_context * ctx, pdf_processor * proc, pdf_obj * array, float phase)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_ri(fz_context * ctx, pdf_processor * proc, const char * intent)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_ri)
...
static void pdf_filter_gs_OP(fz_context * ctx, pdf_processor * proc, int b)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_gs_OP)
...
static void pdf_filter_gs_OPM(fz_context * ctx, pdf_processor * proc, int i)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_gs_UseBlackPtComp(fz_context * ctx, pdf_processor * proc, pdf_obj * name)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_i(fz_context * ctx, pdf_processor * proc, float flatness)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_i)
...
static void pdf_filter_gs_begin(fz_context * ctx, pdf_processor * proc, const char * name, pdf_obj * extgstate)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_ALL);
...
static void pdf_filter_gs_BM(fz_context * ctx, pdf_processor * proc, const char * blendmode)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	if(p->chain->op_gs_BM)
...
static void pdf_filter_gs_CA(fz_context * ctx, pdf_processor * proc, float alpha)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	if(p->chain->op_gs_CA)
...
static void pdf_filter_gs_SMask(fz_context * ctx,
    pdf_processor * proc,
    pdf_obj * smask,
...
static void pdf_filter_gs_end(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_q(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_push(ctx, p);
}
...
static void pdf_filter_cm(fz_context * ctx, pdf_processor * proc, float a, float b, float c, float d, float e, float f)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	fz_matrix ctm;
...
static void pdf_filter_m(fz_context * ctx, pdf_processor * proc, float x, float y)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_l(fz_context * ctx, pdf_processor * proc, float x, float y)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_l)
...
static void pdf_filter_c(fz_context * ctx, pdf_processor * proc, float x1, float y1, float x2, float y2, float x3, float y3)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_c)
...
static void pdf_filter_v(fz_context * ctx, pdf_processor * proc, float x2, float y2, float x3, float y3)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_v)
...
static void pdf_filter_y(fz_context * ctx, pdf_processor * proc, float x1, float y1, float x3, float y3)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_y)
...
static void pdf_filter_h(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_h)
...
static void pdf_filter_re(fz_context * ctx, pdf_processor * proc, float x, float y, float w, float h)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_re)
...
static void pdf_filter_S(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_F(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_fstar(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_FILL);
	if(p->chain->op_fstar)
...
static void pdf_filter_B(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_ALL);
	if(p->chain->op_B)
...
static void pdf_filter_Bstar(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_ALL);
	if(p->chain->op_Bstar)
...
static void pdf_filter_n(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_n)
...
static void pdf_filter_W(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_W)
...
static void pdf_filter_Wstar(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_CTM);
	if(p->chain->op_Wstar)
...
static void pdf_filter_BT(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_ET(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_Tc(fz_context * ctx, pdf_processor * proc, float charspace)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Tw(fz_context * ctx, pdf_processor * proc, float wordspace)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Tz(fz_context * ctx, pdf_processor * proc, float scale)
{
...
static void pdf_filter_TL(fz_context * ctx, pdf_processor * proc, float leading)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Tf(fz_context * ctx, pdf_processor * proc, const char * name, pdf_font_desc * font, float size)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Tr(fz_context * ctx, pdf_processor * proc, int render)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Ts(fz_context * ctx, pdf_processor * proc, float rise)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
...
static void pdf_filter_Td(fz_context * ctx, pdf_processor * proc, float tx, float ty)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	p->Tm_adjust = 0;
...
static void pdf_filter_Tm(fz_context * ctx, pdf_processor * proc, float a, float b, float c, float d, float e, float f)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_Tstar(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	p->Tm_adjust = 0;
	filter_flush(ctx, p, FLUSH_ALL);
...
static void pdf_filter_TJ(fz_context * ctx, pdf_processor * proc, pdf_obj * array)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_show_text(ctx, p, array);
}

static void pdf_filter_Tj(fz_context * ctx, pdf_processor * proc, char * str, size_t len)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_show_string(ctx, p, (uchar*)str, len);
}

static void pdf_filter_squote(fz_context * ctx, pdf_processor * proc, char * str, size_t len)
{
...
static void pdf_filter_dquote(fz_context * ctx, pdf_processor * proc, float aw, float ac, char * str, size_t len)
{
...
static void pdf_filter_d0(fz_context * ctx, pdf_processor * proc, float wx, float wy)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_d0)
...
static void pdf_filter_d1(fz_context * ctx, pdf_processor * proc, float wx, float wy, float llx, float lly, float urx, float ury)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_d1)
...
static void pdf_filter_CS(fz_context * ctx, pdf_processor * proc, const char * name, fz_colorspace * cs)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
...
static void pdf_filter_SC_pattern(fz_context * ctx, pdf_processor * proc, const char * name, pdf_pattern * pat, int n, float * color)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	int i;
...
static void pdf_filter_SC_shade(fz_context * ctx, pdf_processor * proc, const char * name, fz_shade * shade)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	fz_strlcpy(gstate->pending.SC.name, name, sizeof gstate->pending.SC.name);
...
static void pdf_filter_SC_color(fz_context * ctx, pdf_processor * proc, int n, float * color)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_gstate * gstate = gstate_to_update(ctx, p);
	int i;
...
static void pdf_filter_G(fz_context * ctx, pdf_processor * proc, float g)
{
	float color[1] = { g };
...
static void pdf_filter_RG(fz_context * ctx, pdf_processor * proc, float r, float g, float b)
{
	float color[3] = { r, g, b };
...
static void pdf_filter_K(fz_context * ctx, pdf_processor * proc, float c, float m, float y, float k)
{
...
static void pdf_filter_BI(fz_context * ctx, pdf_processor * proc, fz_image * image, const char * colorspace)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_ALL);
	if(p->chain->op_BI) {
...
static void pdf_filter_sh(fz_context * ctx, pdf_processor * proc, const char * name, fz_shade * shade)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_ALL);
	if(p->chain->op_sh)
...
static void pdf_filter_Do_image(fz_context * ctx, pdf_processor * proc, const char * name, fz_image * image)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	fz_image * new_image;
	filter_flush(ctx, p, FLUSH_ALL);
...
static void pdf_filter_Do_form(fz_context * ctx, pdf_processor * proc, const char * name, pdf_obj * xobj, pdf_obj * page_resources)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	fz_matrix transform;
	filter_flush(ctx, p, FLUSH_ALL);
...
static void pdf_filter_MP(fz_context * ctx, pdf_processor * proc, const char * tag)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_MP)
...
static void pdf_filter_DP(fz_context * ctx, pdf_processor * proc, const char * tag, pdf_obj * raw, pdf_obj * cooked)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_DP)
...
static void pdf_filter_BMC(fz_context * ctx, pdf_processor * proc, const char * tag)
{
...
static void pdf_filter_BDC(fz_context * ctx, pdf_processor * proc, const char * tag, pdf_obj * raw, pdf_obj * cooked)
{
	/* Create a tag, and push it onto pending_tags. If it gets
	 * flushed to the stream, it'll be moved from there onto
	 * current_tags. */
...
static void pop_tag(fz_context * ctx, pdf_filter_processor * p, tag_record ** tags)
{
	tag_record * tag = *tags;
...
static void pdf_filter_EMC(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_filter_BX(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_BX)
...
static void pdf_filter_EX(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, 0);
	if(p->chain->op_EX)
...
static void pdf_filter_END(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
	filter_flush(ctx, p, FLUSH_TEXT);
	if(p->chain->op_END)
...
static void pdf_close_filter_processor(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
static void pdf_drop_filter_processor(fz_context * ctx, pdf_processor * proc)
{
	pdf_filter_processor * p = (pdf_filter_processor*)proc;
...
pdf_processor * pdf_new_filter_processor(fz_context * ctx,
    pdf_document * doc,
    pdf_processor * chain,
    pdf_obj * old_rdb,
    pdf_obj * new_rdb,
    ...
    pdf_filter_options * filter)
{
	...
	if(structparents != -1)
		proc->structarray =
		    pdf_keep_obj(ctx,
			pdf_lookup_number(ctx, pdf_dict_getp(ctx, pdf_trailer(ctx, doc), "Root/StructTreeRoot/ParentTree"), structparents));
	proc->chain = chain;
