...
static int pdf_field_dirties_document(fz_context * ctx, pdf_document * doc, pdf_obj * field)
{
	...
	if(ff & (PDF_FIELD_IS_NO_EXPORT|PDF_FIELD_IS_READ_ONLY)) 
		return 0;
	else
		return 1;
}
...
static void pdf_field_mark_dirty(fz_context * ctx, pdf_obj * field)
{
	...
	if(kids) {
		const int n = pdf_array_len(ctx, kids);
		for(int i = 0; i < n; i++)
...
static pdf_obj * lookup_field_sub(fz_context * ctx, pdf_obj * dict, const char * str)
{
	pdf_obj * kids;
	pdf_obj * name;
...
pdf_obj * pdf_lookup_field(fz_context * ctx, pdf_obj * arr, const char * str)
{
...
enum pdf_widget_type pdf_widget_type(fz_context * ctx, pdf_widget * widget)
{
	...
	if(pdf_name_eq(ctx, subtype, PDF_NAME(Widget)))
		return (enum pdf_widget_type)pdf_field_type(ctx, widget->obj);
	return PDF_WIDGET_TYPE_BUTTON;
}
...
void pdf_field_set_border_style(fz_context * ctx, pdf_obj * field, const char * text)
{
	...
	if(sstreq(text, "Solid"))
		val = PDF_NAME(S);
	else if(sstreq(text, "Dashed"))
		val = PDF_NAME(D);
	else if(sstreq(text, "Beveled"))
		val = PDF_NAME(B);
	else if(sstreq(text, "Inset"))
		val = PDF_NAME(I);
	else if(sstreq(text, "Underline"))
		val = PDF_NAME(U);
	else
		return;
...
void pdf_drop_widgets(fz_context * ctx, pdf_widget * widget)
{
	while(widget) {
...
int pdf_choice_widget_options(fz_context * ctx, pdf_widget * tw, int exportval, const char * opts[])
{
	...
	if(opts) {
		for(i = 0; i < n; i++) {
			...
			/* If it is a two element array, the second item is the one that we want if we want the listing
			   value. */
			if(m == 2)
				if(exportval)
...
const char * pdf_choice_field_option(fz_context * ctx, pdf_obj * field, int isExport, int i)
{
	pdf_obj * opt = pdf_dict_get_inheritable(ctx, field, PDF_NAME(Opt));
	pdf_obj * ent = pdf_array_get(ctx, opt, i);
	if(pdf_array_len(ctx, ent) == 2)
		return pdf_array_get_text_string(ctx, ent, isExport ? 0 : 1);
	else
...
static void validate_byte_ranges(fz_context * ctx, pdf_document * doc, fz_range * unsorted, int nranges)
{
	...
	fz_range * sorted;
	int i;
	sorted = (fz_range *)fz_calloc(ctx, nranges, sizeof(*sorted));
...
fz_stream * pdf_signature_hash_bytes(fz_context * ctx, pdf_document * doc, pdf_obj * signature)
{
	...
	fz_try(ctx)
	{
		...
		if(byte_range_len) {
			byte_range = (fz_range *)fz_calloc(ctx, byte_range_len, sizeof(*byte_range));
...
static fz_xml * get_xfa_resource(fz_context * ctx, pdf_document * doc, const char * str)
{
	fz_xml_doc * xfa;
...
static int find_name_component(char ** np, char ** sp, char ** ep)
{
	char * n = *np;
	char * s, * e;
	int idx = 0;
...
static pdf_obj * annot_from_name(fz_context * ctx, pdf_document * doc, const char * str)
{
...
static pdf_obj * get_locked_fields_from_xfa(fz_context * ctx, pdf_document * doc, pdf_obj * field)
{
...
void pdf_signature_set_value(fz_context * ctx, pdf_document * doc, pdf_obj * field, pdf_pkcs7_signer * signer, int64_t stime)
{
	...
	fz_try(ctx)
	{
		...
		buf = (char *)fz_calloc(ctx, max_digest_size, 1);
...
static void count_sigs(fz_context * ctx, pdf_obj * field, void * arg, pdf_obj ** ft)
{
	int * n = (int*)arg;
