...
static void move_to_root(cmap_splay * tree, uint x)
{
	if(x == EMPTY)
		return;
	do {
		uint z, zp;
		uint y = tree[x].parent;
		if(y == EMPTY)
			break;
		z = tree[y].parent;
		if(z == EMPTY) {
...
static uint delete_node(pdf_cmap * cmap, uint current)
{
	...
	cmap->tlen--;
	if(current != (uint)cmap->tlen) {
		if(replacement == (uint)cmap->tlen)
			replacement = current;
		tree[current] = tree[cmap->tlen];
		parent = tree[current].parent;
		if(parent == EMPTY)
			cmap->ttop = current;
		else if(tree[parent].left == (uint)cmap->tlen)
			tree[parent].left = current;
		else {
			assert(tree[parent].right == (uint)cmap->tlen);
			tree[parent].right = current;
		}
		if(tree[current].left != EMPTY) {
			assert(tree[tree[current].left].parent == (uint)cmap->tlen);
			tree[tree[current].left].parent = current;
		}
		if(tree[current].right != EMPTY) {
			assert(tree[tree[current].right].parent == (uint)cmap->tlen);
...
#ifdef DUMP_SPLAY
static void dump_splay(cmap_splay * tree, uint node, int depth, const char * pre)
{
	int i;
	...
	for(i = 0; i < depth; i++)
		slfprintf_stderr(" ");
	slfprintf_stderr("%s%d:", pre, node);
	if(tree[node].parent == EMPTY)
		slfprintf_stderr("^EMPTY");
	else
		slfprintf_stderr("^%d", tree[node].parent);
	if(tree[node].left == EMPTY)
		slfprintf_stderr("<EMPTY");
	else
		slfprintf_stderr("<%d", tree[node].left);
	if(tree[node].right == EMPTY)
		slfprintf_stderr(">EMPTY");
	else
		slfprintf_stderr(">%d", tree[node].right);
	slfprintf_stderr("(%x,%x,%x,%d)\n", tree[node].low, tree[node].high, tree[node].out, tree[node].many);
...
static void walk_splay(cmap_splay * tree, uint node, void (*fn)(cmap_splay *, void *), void * arg)
{
	int from = TOP;
...
static int tree_has_overlap(cmap_splay * tree, int node, int low, int high)
{
	if(tree[node].left != EMPTY)
...
static void do_check(cmap_splay * node, void * arg)
{
	cmap_splay * tree = arg;
	uint num = node - tree;
...
static void check_splay(cmap_splay * tree, uint node, int depth)
{
	if(node == EMPTY)
		return;
 ...
 * Add a range.
 */
static void add_range(fz_context * ctx,
    pdf_cmap * cmap,
    uint low,
    uint high,
    uint out,
			...
			do {
				...
				if(low <= tree[current].low && tree[current].low <= high) {
					...
					if(tree[current].low > tree[current].high) {
						/* update lt/gt references that will be moved/stale after deleting
						   current */
						if(gt == (uint)cmap->tlen - 1)
							gt = current;
						if(lt == (uint)cmap->tlen - 1)
							lt = current;
					...
					tree[current].high = low-1;
					add_range(ctx, cmap, high+1, new_high, tree[current].out + high + 1 - tree[current].low, 0,
					    tree[current].many);
					tree = cmap->tree;
				}
 ...
 */
static void add_mrange(fz_context * ctx, pdf_cmap * cmap, uint low, int * out, int len)
{
	int out_pos;
...
void pdf_map_range_to_range(fz_context * ctx, pdf_cmap * cmap, uint low, uint high, int out)
{
	add_range(ctx, cmap, low, high, out, 1, 0);
}

void pdf_map_one_to_many(fz_context * ctx, pdf_cmap * cmap, uint low, int * values, size_t len)
{
	if(len == 1) {
...
int pdf_lookup_cmap(pdf_cmap * cmap, uint cpt)
{
...
int pdf_lookup_cmap_full(pdf_cmap * cmap, uint cpt, int * out)
{
	pdf_range * ranges = cmap->ranges;
	pdf_xrange * xranges = cmap->xranges;
...
int pdf_decode_cmap(pdf_cmap * cmap, uchar * buf, uchar * end, uint * cpt)
{
	uint c;
	int k, n;
	int len = end - buf;
