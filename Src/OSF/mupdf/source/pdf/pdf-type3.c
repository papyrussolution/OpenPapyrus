...
{
	pdf_run_glyph(ctx, (pdf_document *)doc, (pdf_obj*)rdb, contents, dev, ctm, gstate, default_cs);
}

static void pdf_t3_free_resources(fz_context * ctx, void * doc, void * rdb_)
{
	pdf_obj * rdb = (pdf_obj*)rdb_;
	pdf_drop_obj(ctx, rdb);
}

pdf_font_desc * pdf_load_type3_font(fz_context * ctx, pdf_document * doc, pdf_obj * rdb, pdf_obj * dict)
{
	char buf[256];
	...
	fz_try(ctx)
	{
		...
		if(font->t3resources)
			pdf_keep_obj(ctx, (pdf_obj *)font->t3resources);
		if(!font->t3resources)
