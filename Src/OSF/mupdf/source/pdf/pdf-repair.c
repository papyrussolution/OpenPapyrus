	...
	* we only need to fully parse it if it is a dictionary. */
	...
	if(tok == PDF_TOK_STREAM) {
		...
		*stmofsp = fz_tell(ctx, file);
		...
		(void)fz_read(ctx, file, (uchar*)buf->scratch, 9);
...
static void pdf_repair_obj_stm(fz_context * ctx, pdf_document * doc, int stm_num)
{
	pdf_obj * obj;
	fz_stream * stm = NULL;
	pdf_token tok;
...
static void orphan_object(fz_context * ctx, pdf_document * doc, pdf_obj * obj)
{
...
void pdf_repair_xref(fz_context * ctx, pdf_document * doc)
{
	...
	fz_try(ctx)
	{
		...
		n = fz_read(ctx, doc->file, (uchar*)buf->scratch, fz_minz(buf->size, 1024));
