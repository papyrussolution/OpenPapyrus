...
static void devicen_eval(fz_context * ctx, void * tint, const float * sv, int sn, float * dv, int dn)
{
	pdf_eval_function(ctx, (pdf_function *)tint, sv, sn, dv, dn);
}

static void devicen_drop(fz_context * ctx, void * tint)
{
	pdf_drop_function(ctx, (pdf_function *)tint);
}

static fz_colorspace * load_devicen(fz_context * ctx, pdf_obj * array, int is_devn)
{
...
static fz_colorspace * load_indexed(fz_context * ctx, pdf_obj * array)
{
	...
	fz_try(ctx)
	{
		...
		if(pdf_is_string(ctx, lookupobj)) {
			...
			uchar * buf = (uchar*)pdf_to_str_buf(ctx, lookupobj);
			for(i = 0; i < sn; ++i)
				lookup[i] = buf[i];
			for(; i < n; ++i)
				lookup[i] = 0;
...
static void pdf_load_cal_common(fz_context * ctx, pdf_obj * dict, float * wp, float * bp, float * gamma)
{
	pdf_obj * obj;
	int i;
...
static fz_colorspace * pdf_load_cal_gray(fz_context * ctx, pdf_obj * dict)
{
	float wp[3];
	float bp[3] = { 0, 0, 0 };
	float gamma[3] = { 1, 1, 1 };
...
static fz_colorspace * pdf_load_cal_rgb(fz_context * ctx, pdf_obj * dict)
{
	pdf_obj * obj;
...
static fz_colorspace * pdf_load_colorspace_imp(fz_context * ctx, pdf_obj * obj)
{
...
{
	fz_colorspace * cs;
	if((cs = (fz_colorspace *)pdf_find_item(ctx, fz_drop_colorspace_imp, obj)) != NULL) {
		return cs;
	}
...
static fz_colorspace * pdf_load_output_intent(fz_context * ctx, pdf_document * doc)
{
