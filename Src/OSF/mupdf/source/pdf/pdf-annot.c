...
{
	return (pdf_annot *)fz_keep_imp(ctx, annot, &annot->refs);
}
...
void pdf_drop_annots(fz_context * ctx, pdf_annot * annot)
{
	while(annot) {
 ...
 */
static pdf_annot * pdf_new_annot(fz_context * ctx, pdf_page * page, pdf_obj * obj)
{
	pdf_annot * annot;
...
{
	if(sstreq("Text", subtype)) return PDF_ANNOT_TEXT;
	if(sstreq("Link", subtype)) return PDF_ANNOT_LINK;
	if(sstreq("FreeText", subtype)) return PDF_ANNOT_FREE_TEXT;
	if(sstreq("Line", subtype)) return PDF_ANNOT_LINE;
	if(sstreq("Square", subtype)) return PDF_ANNOT_SQUARE;
	if(sstreq("Circle", subtype)) return PDF_ANNOT_CIRCLE;
	if(sstreq("Polygon", subtype)) return PDF_ANNOT_POLYGON;
	if(sstreq("PolyLine", subtype)) return PDF_ANNOT_POLY_LINE;
	if(sstreq("Highlight", subtype)) return PDF_ANNOT_HIGHLIGHT;
	if(sstreq("Underline", subtype)) return PDF_ANNOT_UNDERLINE;
	if(sstreq("Squiggly", subtype)) return PDF_ANNOT_SQUIGGLY;
	if(sstreq("StrikeOut", subtype)) return PDF_ANNOT_STRIKE_OUT;
	if(sstreq("Redact", subtype)) return PDF_ANNOT_REDACT;
	if(sstreq("Stamp", subtype)) return PDF_ANNOT_STAMP;
	if(sstreq("Caret", subtype)) return PDF_ANNOT_CARET;
	if(sstreq("Ink", subtype)) return PDF_ANNOT_INK;
	if(sstreq("Popup", subtype)) return PDF_ANNOT_POPUP;
	if(sstreq("FileAttachment", subtype)) return PDF_ANNOT_FILE_ATTACHMENT;
	if(sstreq("Sound", subtype)) return PDF_ANNOT_SOUND;
	if(sstreq("Movie", subtype)) return PDF_ANNOT_MOVIE;
	if(sstreq("RichMedia", subtype)) return PDF_ANNOT_RICH_MEDIA;
	if(sstreq("Widget", subtype)) return PDF_ANNOT_WIDGET;
	if(sstreq("Screen", subtype)) return PDF_ANNOT_SCREEN;
	if(sstreq("PrinterMark", subtype)) return PDF_ANNOT_PRINTER_MARK;
	if(sstreq("TrapNet", subtype)) return PDF_ANNOT_TRAP_NET;
	if(sstreq("Watermark", subtype)) return PDF_ANNOT_WATERMARK;
	if(sstreq("3D", subtype)) return PDF_ANNOT_3D;
	if(sstreq("Projection", subtype)) return PDF_ANNOT_PROJECTION;
...
static void check_allowed_subtypes(fz_context * ctx, pdf_annot * annot, pdf_obj * property, pdf_obj ** allowed)
{
	...
	if(!is_allowed_subtype(ctx, annot, property, allowed))
		fz_throw(ctx, FZ_ERROR_GENERIC, "%s annotations have no %s property", pdf_to_name(ctx, subtype),
		    pdf_to_name(ctx, property));
}
...
static pdf_obj * pdf_add_popup_annot(fz_context * ctx, pdf_annot * annot)
{
...
pdf_annot * pdf_create_annot(fz_context * ctx, pdf_page * page, enum pdf_annot_type type)
{
	...
	switch(type)
	{
		...
		case PDF_ANNOT_LINE:
	    {
		    SPoint2F a = { 12, 12 }, b = { 12 + 100, 12 + 50 };
...
static int remove_from_tree(fz_context * ctx, pdf_obj * arr, pdf_obj * item)
{
	int i, n, res = 0;
...
enum pdf_line_ending pdf_line_ending_from_string(fz_context * ctx, const char * end){
	if(sstreq(end, "None")) return PDF_ANNOT_LE_NONE;
	else if(sstreq(end, "Square")) return PDF_ANNOT_LE_SQUARE;
	else if(sstreq(end, "Circle")) return PDF_ANNOT_LE_CIRCLE;
	else if(sstreq(end, "Diamond")) return PDF_ANNOT_LE_DIAMOND;
	else if(sstreq(end, "OpenArrow")) return PDF_ANNOT_LE_OPEN_ARROW;
	else if(sstreq(end, "ClosedArrow")) return PDF_ANNOT_LE_CLOSED_ARROW;
	else if(sstreq(end, "Butt")) return PDF_ANNOT_LE_BUTT;
	else if(sstreq(end, "ROpenArrow")) return PDF_ANNOT_LE_R_OPEN_ARROW;
	else if(sstreq(end, "RClosedArrow")) return PDF_ANNOT_LE_R_CLOSED_ARROW;
	else if(sstreq(end, "Slash")) return PDF_ANNOT_LE_SLASH;
	else return PDF_ANNOT_LE_NONE;
}
...
void pdf_annot_line_ending_styles(fz_context * ctx, pdf_annot * annot,
...
void pdf_set_annot_line_ending_styles(fz_context * ctx, pdf_annot * annot,
    enum pdf_line_ending start_style,
    enum pdf_line_ending end_style)
{
...
void pdf_set_annot_border(fz_context * ctx, pdf_annot * annot, float w)
{
...
static int pdf_annot_color_rgb(fz_context * ctx, pdf_obj * arr, float rgb[3])
{
	...
	if(!n) {
		...
		rgb[0] = 1.0f - smin(1.0f, color[0] + color[3]);
		rgb[1] = 1.0f - smin(1.0f, color[1] + color[3]);
		rgb[2] = 1.0f - smin(1.0f, color[2] + color[3]);
	}
	return 1;
}
...
void pdf_add_annot_ink_list(fz_context * ctx, pdf_annot * annot, int n, SPoint2F p[])
{
	fz_matrix page_ctm, inv_page_ctm;
	pdf_obj * ink_list, * stroke;
	int i;
 ...
 */
void pdf_set_annot_modification_date(fz_context * ctx, pdf_annot * annot, int64_t secs)
{
 ...
 */
void pdf_set_annot_creation_date(fz_context * ctx, pdf_annot * annot, int64_t secs)
{
	...
	*size = 12;
	...
	while((tok = fz_strsep(&p, " \n\r\t")) != NULL) {
		if(tok[0] == 0)
			...
			if(sstreq(tok+1, "Cour")) *font = "Cour";
			if(sstreq(tok+1, "Helv")) *font = "Helv";
			if(sstreq(tok+1, "TiRo")) *font = "TiRo";
			if(sstreq(tok+1, "Symb")) *font = "Symb";
			if(sstreq(tok+1, "ZaDb")) *font = "ZaDb";
		}
		else if(sstreq(tok, "Tf")) {
			*size = stack[0];
			top = 0;
		}
		else if(sstreq(tok, "g")) {
			color[0] = color[1] = color[2] = stack[0];
			top = 0;
		}
		else if(sstreq(tok, "rg")) {
			color[0] = stack[0];
			color[1] = stack[1];
			color[2] = stack[2];
			top = 0;
