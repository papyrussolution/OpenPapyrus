...
static int rune_from_utf16be(int * out, const uchar * s, const uchar * end)
{
	if(s + 2 <= end) {
		int a = s[0] << 8 | s[1];
...
static int rune_from_utf16le(int * out, const uchar * s, const uchar * end)
{
	if(s + 2 <= end) {
		int a = s[1] << 8 | s[0];
		if(a >= 0xD800 && a <= 0xDFFF && s + 4 <= end) {
...
static size_t skip_language_code_utf16le(const uchar * s, size_t n, size_t i)
{
...
static size_t skip_language_code_utf16be(const uchar * s, size_t n, size_t i)
{
	/* skip language escape codes */
...
static size_t skip_language_code_utf8(const uchar * s, size_t n, size_t i)
{
	/* skip language escape codes */
...
static int is_valid_utf8(const uchar * s, const uchar * end)
{
	for(; s < end; ++s) {
...
char * pdf_new_utf8_from_pdf_string(fz_context * ctx, const char * ssrcptr, size_t srclen)
{
	const uchar * srcptr = (const uchar*)ssrcptr;
	char * dstptr, * dst;
	size_t dstlen = 0;
	int ucs;
	size_t i, n;
	...
	if(srclen >= 2 && srcptr[0] == 254 && srcptr[1] == 255) {
		...
		dstptr = dst = (char *)Memento_label(fz_malloc(ctx, dstlen + 1), "utf8_from_utf16be");
		i = 2;
		while(i + 2 <= srclen) {
			n = skip_language_code_utf16be(srcptr, srclen, i);
			if(n)
	...
	/* UTF-16LE */
		...
		dstptr = dst = (char *)Memento_label(fz_malloc(ctx, dstlen + 1), "utf8_from_utf16le");
		i = 2;
		while(i + 2 <= srclen) {
			n = skip_language_code_utf16le(srcptr, srclen, i);
			if(n)
	...
	/* UTF-8 */
		...
		dstptr = dst = (char *)Memento_label(fz_malloc(ctx, dstlen + 1), "utf8_from_utf8");
		i = 3;
		while(i < srclen) {
			n = skip_language_code_utf8(srcptr, srclen, i);
			if(n)
	...
	/* PDFDocEncoding */
		...
		dstptr = dst = (char *)Memento_label(fz_malloc(ctx, dstlen + 1), "utf8_from_pdfdocenc");
		for(i = 0; i < srclen; i++) {
...
char * pdf_new_utf8_from_pdf_stream_obj(fz_context * ctx, pdf_obj * src)
{
	...
	srclen = fz_buffer_storage(ctx, stmbuf, (uchar**)&srcptr);
	fz_try(ctx)
...
static pdf_obj * pdf_new_text_string_utf16be(fz_context * ctx, const char * s)
{
	const char * ss;
	int c, i, n, a, b;
	uchar * p;
...
pdf_obj * pdf_parse_dict(fz_context * ctx, pdf_document * doc, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_obj * dict;
	pdf_obj * key = NULL;
	pdf_obj * val = NULL;
...
skip:
			...
			if(tok == PDF_TOK_KEYWORD && sstreq(buf->scratch, "ID"))
				break;
...
pdf_obj * pdf_parse_stm_obj(fz_context * ctx, pdf_document * doc, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
...
pdf_obj * pdf_parse_ind_obj(fz_context * ctx, pdf_document * doc,
