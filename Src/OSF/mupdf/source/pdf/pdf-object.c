...
{
	pdf_obj_num * obj = (pdf_obj_num *)Memento_label(fz_malloc(ctx, sizeof(pdf_obj_num)), "pdf_obj(int)");
	obj->super.refs = 1;
...
{
	pdf_obj_num * obj = (pdf_obj_num *)Memento_label(fz_malloc(ctx, sizeof(pdf_obj_num)), "pdf_obj(real)");
	obj->super.refs = 1;
...
pdf_obj * pdf_new_string(fz_context * ctx, const char * str, size_t len)
{
	...
	obj = (pdf_obj_string *)Memento_label(fz_malloc(ctx, offsetof(pdf_obj_string, buf) + len + 1), "pdf_obj(string)");
	obj->super.refs = 1;
...
pdf_obj * pdf_new_name(fz_context * ctx, const char * str)
{
	...
	obj = (pdf_obj_name *)Memento_label(fz_malloc(ctx, offsetof(pdf_obj_name, n) + strlen(str) + 1), "pdf_obj(name)");
	obj->super.refs = 1;
...
pdf_obj * pdf_new_indirect(fz_context * ctx, pdf_document * doc, int num, int gen)
{
	...
	obj = (pdf_obj_ref *)Memento_label(fz_malloc(ctx, sizeof(pdf_obj_ref)), "pdf_obj(indirect)");
	obj->super.refs = 1;
...
#define OBJ_IS_INT(obj) (obj >= PDF_LIMIT && obj->kind == PDF_INT)
#define OBJ_IS_REAL(obj) (obj >= PDF_LIMIT && obj->kind == PDF_REAL)
#define OBJ_IS_NUMBER(obj) (obj >= PDF_LIMIT && (obj->kind == PDF_REAL || obj->kind == PDF_INT))
#define OBJ_IS_STRING(obj) (obj >= PDF_LIMIT && obj->kind == PDF_STRING)
#define OBJ_IS_ARRAY(obj) (obj >= PDF_LIMIT && obj->kind == PDF_ARRAY)
...
{
	RESOLVE(obj);
	return OBJ_IS_STRING(obj) ? STRING(obj)->len : 0;
}
...
int STDCALL pdf_name_eq(fz_context * ctx, pdf_obj * a, pdf_obj * b)
{
	RESOLVE(a);
	RESOLVE(b);
	if(a <= PDF_FALSE || b <= PDF_FALSE)
	...
	if(a->kind == PDF_NAME && b->kind == PDF_NAME)
		return sstreq(NAME(a)->n, NAME(b)->n);
	return 0;
}
...
static char * pdf_objkindstr(pdf_obj * obj)
{
	if(obj == PDF_NULL)
		return "null";
	else if(oneof2(obj, PDF_TRUE, PDF_FALSE))
		return "boolean";
	else if(obj < PDF_LIMIT)
		return "name";
	else {
		switch(obj->kind) {
...
{
	int i;
	pdf_obj_array * obj = (pdf_obj_array *)Memento_label(fz_malloc(ctx, sizeof(pdf_obj_array)), "pdf_obj(array)");
	obj->super.refs = 1;
...
static void pdf_array_grow(fz_context * ctx, pdf_obj_array * obj)
{
	int i;
...
pdf_obj * pdf_copy_array(fz_context * ctx, pdf_obj * obj)
{
	pdf_document * doc;
	pdf_obj * arr;
	int i;
...
int STDCALL pdf_array_len(fz_context * ctx, pdf_obj * obj)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		return 0;
...
pdf_obj * STDCALL pdf_array_get(fz_context * ctx, pdf_obj * obj, int i)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		return NULL;
...
void pdf_array_put(fz_context * ctx, pdf_obj * obj, int i, pdf_obj * item)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		fz_throw(ctx, FZ_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
void pdf_array_put_drop(fz_context * ctx, pdf_obj * obj, int i, pdf_obj * item)
{
	fz_try(ctx)
...
void pdf_array_push(fz_context * ctx, pdf_obj * obj, pdf_obj * item)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		fz_throw(ctx, FZ_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
void pdf_array_push_drop(fz_context * ctx, pdf_obj * obj, pdf_obj * item)
{
	fz_try(ctx)
	pdf_array_push(ctx, obj, item);
	fz_always(ctx)
...
void pdf_array_insert(fz_context * ctx, pdf_obj * obj, pdf_obj * item, int i)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		fz_throw(ctx, FZ_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
void pdf_array_insert_drop(fz_context * ctx, pdf_obj * obj, pdf_obj * item, int i)
{
	fz_try(ctx)
...
void pdf_array_delete(fz_context * ctx, pdf_obj * obj, int i)
{
	RESOLVE(obj);
	if(!OBJ_IS_ARRAY(obj))
		fz_throw(ctx, FZ_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
int pdf_array_contains(fz_context * ctx, pdf_obj * arr, pdf_obj * obj)
{
...
int pdf_array_find(fz_context * ctx, pdf_obj * arr, pdf_obj * obj)
{
	int len = pdf_array_len(ctx, arr);
	for(int i = 0; i < len; i++)
		if(!pdf_objcmp(ctx, pdf_array_get(ctx, arr, i), obj))
...
{
	const struct keyval * a = (const struct keyval *)ap;
	const struct keyval * b = (const struct keyval *)bp;
	const char * an;
	const char * bn;
...
{
	int i;
	pdf_obj_dict * obj = (pdf_obj_dict *)Memento_label(fz_malloc(ctx, sizeof(pdf_obj_dict)), "pdf_obj(dict)");
	obj->super.refs = 1;
...
static void pdf_dict_grow(fz_context * ctx, pdf_obj * obj)
{
	int i;
...
pdf_obj * pdf_copy_dict(fz_context * ctx, pdf_obj * obj)
{
	pdf_document * doc;
	pdf_obj * dict;
	int i, n;
...
pdf_obj * pdf_dict_get_key(fz_context * ctx, pdf_obj * obj, int i)
{
	RESOLVE(obj);
	if(!OBJ_IS_DICT(obj))
		return NULL;
...
pdf_obj * pdf_dict_get_val(fz_context * ctx, pdf_obj * obj, int i)
{
	RESOLVE(obj);
	if(!OBJ_IS_DICT(obj))
		return NULL;
...
static int pdf_dict_finds(fz_context * ctx, pdf_obj * obj, const char * key)
{
	...
	if((obj->flags & PDF_FLAGS_SORTED) && len > 0) {
		...
		for(i = 0; i < len; i++) {
			...
			if(k < PDF_LIMIT) {
				...
				if(sstreq(PDF_NAME_LIST[(intptr_t)key], NAME(k)->n))
					return i;
			}
		}
...
pdf_obj * pdf_dict_getp(fz_context * ctx, pdf_obj * obj, const char * keys)
{
	char buf[256];
	char * k, * e;
	RESOLVE(obj);
...
pdf_obj * pdf_dict_getl(fz_context * ctx, pdf_obj * obj, ...)
{
	va_list keys;
	pdf_obj * key;
	va_start(keys, obj);
	while(obj && (key = va_arg(keys, pdf_obj *)) != NULL) {
...
pdf_obj * pdf_dict_getsa(fz_context * ctx, pdf_obj * obj, const char * key, const char * abbrev)
{
...
void pdf_dict_put_drop(fz_context * ctx, pdf_obj * obj, pdf_obj * key, pdf_obj * val)
{
	fz_try(ctx)
	pdf_dict_get_put(ctx, obj, key, val, NULL);
	fz_always(ctx)
...
void pdf_dict_get_put_drop(fz_context * ctx, pdf_obj * obj, pdf_obj * key, pdf_obj * val, pdf_obj ** old_val)
{
	fz_try(ctx)
...
void pdf_dict_puts(fz_context * ctx, pdf_obj * obj, const char * key, pdf_obj * val)
{
	pdf_obj * keyobj;
	RESOLVE(obj);
	if(!OBJ_IS_DICT(obj))
...
void pdf_dict_puts_drop(fz_context * ctx, pdf_obj * obj, const char * key, pdf_obj * val)
{
	pdf_obj * keyobj;
	RESOLVE(obj);
	if(!OBJ_IS_DICT(obj))
...
void pdf_dict_putp(fz_context * ctx, pdf_obj * obj, const char * keys, pdf_obj * val)
{
	pdf_document * doc;
	char buf[256];
	char * k, * e;
...
void pdf_dict_putl(fz_context * ctx, pdf_obj * obj, pdf_obj * val, ...)
{
	va_list keys;
	va_start(keys, val);
	fz_try(ctx)
...
void pdf_dict_dels(fz_context * ctx, pdf_obj * obj, const char * key)
{
	int i;
	RESOLVE(obj);
	if(!OBJ_IS_DICT(obj))
...
void pdf_dict_del(fz_context * ctx, pdf_obj * obj, pdf_obj * key)
{
	if(!OBJ_IS_NAME(key))
...
static void pdf_drop_array(fz_context * ctx, pdf_obj * obj)
{
	for(int i = 0; i < DICT(obj)->len; i++)
		pdf_drop_obj(ctx, ARRAY(obj)->items[i]);
	fz_free(ctx, DICT(obj)->items);
	fz_free(ctx, obj);
}

static void pdf_drop_dict(fz_context * ctx, pdf_obj * obj)
{
	for(int i = 0; i < DICT(obj)->len; i++) {
		pdf_drop_obj(ctx, DICT(obj)->items[i].k);
		pdf_drop_obj(ctx, DICT(obj)->items[i].v);
...
int pdf_obj_parent_num(fz_context * ctx, pdf_obj * obj)
{
	if(obj < PDF_LIMIT)
		return 0;
	switch(obj->kind) {
		case PDF_INDIRECT: return REF(obj)->num;
		case PDF_ARRAY: return ARRAY(obj)->parent_num;
		case PDF_DICT: return DICT(obj)->parent_num;
		default: return 0;
	}
}
...
static inline int iswhite(int ch) { return oneof6(ch, '\000', '\011', '\012', '\014', '\015', '\040'); }
static inline int isdelim(int ch) { return oneof10(ch, '(', ')', '<', '>', '[', ']', '{', '}', '/', '%'); }
...
static inline void fmt_putc(fz_context * ctx, struct fmt * fmt, int c)
{
	...
	if(fmt->len >= fmt->cap) {
		fmt->cap *= 2;
		if(fmt->buf == fmt->ptr) {
			fmt->ptr = (char *)Memento_label(fz_malloc(ctx, fmt->cap), "fmt_ptr");
			memcpy(fmt->ptr, fmt->buf, fmt->len);
		}
		else {
			fmt->ptr = (char *)fz_realloc(ctx, fmt->ptr, fmt->cap);
		}
	}
	fmt->ptr[fmt->len] = c;
	if(c == '\n')
...
{
	uchar * s = (uchar*)pdf_to_str_buf(ctx, obj);
...
{
	uchar * s = (uchar*)pdf_to_str_buf(ctx, obj);
	const size_t n = pdf_to_str_len(ctx, obj);
	size_t m = 0;
...
static void fmt_str_out(fz_context * ctx, void * fmt_, const uchar * s, size_t n)
{
...
static void fmt_hex_out(fz_context * ctx, void * arg, const uchar * s, size_t n)
{
...
{
	uchar * s = (uchar*)pdf_to_name(ctx, obj);
	int i, c;
	fmt_putc(ctx, fmt, '/');
	for(i = 0; s[i]; i++) {
...
static void fmt_obj(fz_context * ctx, struct fmt * fmt, pdf_obj * obj)
{
	...
	if(obj == PDF_NULL)
		...
		uchar * str = (uchar*)pdf_to_str_buf(ctx, obj);
		if(fmt->crypt || (fmt->ascii && is_binary_string(ctx, obj)) || (str[0]==0xff && str[1]==0xfe) || (str[0]==0xfe && str[1] == 0xff) || is_longer_than_hex(ctx, obj))
			fmt_hex(ctx, fmt, obj);
		else
			fmt_str(ctx, fmt, obj);
	}
...
static char * pdf_sprint_encrypted_obj(fz_context * ctx, char * buf, size_t cap, size_t * len, pdf_obj * obj, int tight, int ascii, pdf_crypt * crypt, int num, int gen)
{
	struct fmt fmt;
	fmt.indent = 0;
	fmt.col = 0;
	...
	if(!buf || cap == 0) {
		fmt.cap = 1024;
		fmt.buf = NULL;
		fmt.ptr = (char *)Memento_label(fz_malloc(ctx, fmt.cap), "fmt_buf");
	}
	else {
		fmt.cap = cap;
		fmt.buf = buf;
...
void pdf_print_encrypted_obj(fz_context * ctx, fz_output * out, pdf_obj * obj, int tight, int ascii, pdf_crypt * crypt, int num, int gen)
{
	char buf[1024];
	size_t n;
	char * ptr = pdf_sprint_encrypted_obj(ctx, buf, sizeof buf, &n, obj, tight, ascii, crypt, num, gen);
	fz_try(ctx)
...
pdf_obj * pdf_dict_get_inheritable(fz_context * ctx, pdf_obj * node, pdf_obj * key)
{
	pdf_obj * node2 = node;
	pdf_obj * val = NULL;
	pdf_obj * marked = NULL;
...
pdf_obj * pdf_dict_getp_inheritable(fz_context * ctx, pdf_obj * node, const char * path)
{
	pdf_obj * node2 = node;
	pdf_obj * val = NULL;
	pdf_obj * marked = NULL;
...
void   pdf_dict_put_bool(fz_context * ctx, pdf_obj * dict, pdf_obj * key, int x) { pdf_dict_put(ctx, dict, key, x ? PDF_TRUE : PDF_FALSE); }
void   pdf_dict_put_int(fz_context * ctx, pdf_obj * dict, pdf_obj * key, int64_t x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_int(ctx, x)); }
void   pdf_dict_put_real(fz_context * ctx, pdf_obj * dict, pdf_obj * key, double x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_real(ctx, x)); }
void   pdf_dict_put_name(fz_context * ctx, pdf_obj * dict, pdf_obj * key, const char * x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_name(ctx, x)); }
void   pdf_dict_put_string(fz_context * ctx, pdf_obj * dict, pdf_obj * key, const char * x, size_t n) { pdf_dict_put_drop(ctx, dict, key, pdf_new_string(ctx, x, n)); }
void   pdf_dict_put_text_string(fz_context * ctx, pdf_obj * dict, pdf_obj * key, const char * x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_text_string(ctx, x)); }
void   pdf_dict_put_rect(fz_context * ctx, pdf_obj * dict, pdf_obj * key, fz_rect x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_rect(ctx, NULL, x)); }
void   pdf_dict_put_matrix(fz_context * ctx, pdf_obj * dict, pdf_obj * key, fz_matrix x) { pdf_dict_put_drop(ctx, dict, key, pdf_new_matrix(ctx, NULL, x)); }
void   pdf_dict_put_date(fz_context * ctx, pdf_obj * dict, pdf_obj * key, int64_t time) { pdf_dict_put_drop(ctx, dict, key, pdf_new_date(ctx, NULL, time)); }
...
void pdf_array_push_bool(fz_context * ctx, pdf_obj * array, int x) { pdf_array_push(ctx, array, x ? PDF_TRUE : PDF_FALSE); }
void pdf_array_push_int(fz_context * ctx, pdf_obj * array, int64_t x) { pdf_array_push_drop(ctx, array, pdf_new_int(ctx, x)); }
void pdf_array_push_real(fz_context * ctx, pdf_obj * array, double x) { pdf_array_push_drop(ctx, array, pdf_new_real(ctx, x)); }
void pdf_array_push_name(fz_context * ctx, pdf_obj * array, const char * x) { pdf_array_push_drop(ctx, array, pdf_new_name(ctx, x)); }
void pdf_array_push_string(fz_context * ctx, pdf_obj * array, const char * x, size_t n) { pdf_array_push_drop(ctx, array, pdf_new_string(ctx, x, n)); }
void pdf_array_push_text_string(fz_context * ctx, pdf_obj * array, const char * x) { pdf_array_push_drop(ctx, array, pdf_new_text_string(ctx, x)); }
...
int    pdf_dict_get_bool(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_bool(ctx, pdf_dict_get(ctx, dict, key)); }
int    pdf_dict_get_int(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_int(ctx, pdf_dict_get(ctx, dict, key)); }
float  pdf_dict_get_real(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_real(ctx, pdf_dict_get(ctx, dict, key)); }
const char * pdf_dict_get_name(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_name(ctx, pdf_dict_get(ctx, dict, key)); }
const char * pdf_dict_get_string(fz_context * ctx, pdf_obj * dict, pdf_obj * key, size_t * sizep) { return pdf_to_string(ctx, pdf_dict_get(ctx, dict, key), sizep); }
const char * pdf_dict_get_text_string(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_text_string(ctx, pdf_dict_get(ctx, dict, key)); }
fz_rect pdf_dict_get_rect(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_rect(ctx, pdf_dict_get(ctx, dict, key)); }
fz_matrix pdf_dict_get_matrix(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_matrix(ctx, pdf_dict_get(ctx, dict, key)); }
int64_t pdf_dict_get_date(fz_context * ctx, pdf_obj * dict, pdf_obj * key) { return pdf_to_date(ctx, pdf_dict_get(ctx, dict, key)); }
int    pdf_array_get_bool(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_bool(ctx, pdf_array_get(ctx, array, index)); }
int    pdf_array_get_int(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_int(ctx, pdf_array_get(ctx, array, index)); }
float  pdf_array_get_real(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_real(ctx, pdf_array_get(ctx, array, index)); }
const char * pdf_array_get_name(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_name(ctx, pdf_array_get(ctx, array, index)); }
const char * pdf_array_get_string(fz_context * ctx, pdf_obj * array, int index, size_t * sizep) { return pdf_to_string(ctx, pdf_array_get(ctx, array, index), sizep); }
const char * pdf_array_get_text_string(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_text_string(ctx, pdf_array_get(ctx, array, index)); }
fz_rect    pdf_array_get_rect(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_rect(ctx, pdf_array_get(ctx, array, index)); }
fz_matrix  pdf_array_get_matrix(fz_context * ctx, pdf_obj * array, int index) { return pdf_to_matrix(ctx, pdf_array_get(ctx, array, index)); }
