...
static void expand_lists(fz_context * ctx, pdf_write_state * opts, int num)
{
	int i;
 ...
 */
static page_objects_list * page_objects_list_create(fz_context * ctx)
{
	page_objects_list * pol = (page_objects_list *)fz_calloc(ctx, 1, sizeof(*pol));
	pol->cap = 1;
	pol->len = 0;
	return pol;
}

static void page_objects_list_destroy(fz_context * ctx, page_objects_list * pol)
{
	int i;
...
static void page_objects_list_ensure(fz_context * ctx, page_objects_list ** pol, int newcap)
{
	int oldcap = (*pol)->cap;
	if(newcap <= oldcap)
		return;
...
static page_objects * page_objects_create(fz_context * ctx)
{
	int initial_cap = 8;
	page_objects * po = (page_objects *)fz_calloc(ctx, 1, sizeof(*po) + (initial_cap-1) * sizeof(int));
	po->cap = initial_cap;
	po->len = 0;
	return po;
}

static void page_objects_insert(fz_context * ctx, page_objects ** ppo, int i)
{
	page_objects * po;
	...
	if(po->len == po->cap) {
		po = (page_objects *)fz_realloc(ctx, po, sizeof(page_objects) + (po->cap*2 - 1)*sizeof(int));
		po->cap *= 2;
		*ppo = po;
	}
	po->object[po->len++] = i;
...
static void page_objects_list_insert(fz_context * ctx, pdf_write_state * opts, int page, int object)
{
...
static void page_objects_list_set_page_object(fz_context * ctx, pdf_write_state * opts, int page, int object)
{
	page_objects_list_ensure(ctx, &opts->page_object_lists, page+1);
	if(object >= opts->list_len)
		expand_lists(ctx, opts, object);
...
static void page_objects_sort(fz_context * ctx, page_objects * po)
{
	int i, j;
	int n = po->len;
...
static int order_ge(int ui, int uj)
{
	/*
...
static void heap_sort(int * list, int n, const int * val, int (*ge)(int, int))
{
	int i, j;

#ifdef DEBUG_HEAP_SORT
	slfprintf_stderr("Initially:\n");
	for(i = 0; i < n; i++) {
		slfprintf_stderr("%d: %d %x\n", i, list[i], val[list[i]]);
	}
#endif
	/* Step 1: Make a heap */
	/* Invariant: Valid heap in [0..i), unsorted elements in [i..n) */
...
#ifdef DEBUG_HEAP_SORT
	slfprintf_stderr("Valid heap:\n");
	for(i = 0; i < n; i++) {
		int k;
		slfprintf_stderr("%d: %d %x ", i, list[i], val[list[i]]);
		k = (i+1)*2-1;
		if(k < n) {
			if(ge(val[list[i]], val[list[k]]))
				slfprintf_stderr("OK ");
			else
				slfprintf_stderr("BAD ");
		}
		if(k+1 < n) {
...
static void page_objects_dedupe(fz_context * ctx, page_objects * po)
{
	int i, j;
	int n = po->len-1;
...
static void page_objects_list_sort_and_dedupe(fz_context * ctx, page_objects_list * pol)
{
	int i;
	int n = pol->len;
...
static void page_objects_dump(pdf_write_state * opts)
{
	...
	for(i = 0; i < pol->len; i++) {
		...
		slfprintf_stderr("Page %d\n", i+1);
		for(j = 0; j < p->len; j++) {
			int o = p->object[j];
			slfprintf_stderr("\tObject %d: use=%x\n", o, opts->use_list[o]);
		}
		slfprintf_stderr("Byte range=%d->%d\n", p->min_ofs, p->max_ofs);
		slfprintf_stderr("Number of objects=%d, Number of shared objects=%d\n", p->num_objects, p->num_shared);
		slfprintf_stderr("Page object number=%d\n", p->page_object_number);
	}
}

static void objects_dump(fz_context * ctx, pdf_document * doc, pdf_write_state * opts)
{
	int i;

	for(i = 0; i < pdf_xref_len(ctx, doc); i++) {
		slfprintf_stderr("Object %d use=%x offset=%d\n", i, opts->use_list[i], (int)opts->ofs_list[i]);
	}
}
...
static void renumberobjs(fz_context * ctx, pdf_document * doc, pdf_write_state * opts)
{
	...
	new_use_list = (int *)fz_calloc(ctx, pdf_xref_len(ctx, doc)+3, sizeof(int));
...
static void mark_all(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, pdf_obj * val, int flag, int page)
{
...
static int mark_pages(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, pdf_obj * val, int pagenum)
{
	if(pdf_mark_obj(ctx, val))
		return pagenum;
...
static void mark_root(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, pdf_obj * dict)
{
...
static void mark_trailer(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, pdf_obj * dict)
{
	int i, n = pdf_dict_len(ctx, dict);
...
static void add_linearization_objs(fz_context * ctx, pdf_document * doc, pdf_write_state * opts)
{
...
static void lpr_inherit_res_contents(fz_context * ctx, pdf_obj * res, pdf_obj * dict, pdf_obj * text)
{
	pdf_obj * o, * r;
	int i, n;
...
static void lpr_inherit_res(fz_context * ctx, pdf_obj * node, int depth, pdf_obj * dict)
{
	while(1) {
		pdf_obj * o;
...
static int lpr(fz_context * ctx, pdf_document * doc, pdf_obj * node, int depth, int page)
{
	pdf_obj * kids;
	pdf_obj * o = NULL;
	int i, n;
...
static void linearize(fz_context * ctx, pdf_document * doc, pdf_write_state * opts)
{
	...
	for(i = 0; i < pdf_xref_len(ctx, doc); i++) {
		slfprintf_stderr("%d: use=%d\n", i, opts->use_list[i]);
	}
#endif

	/* Allocate/init the structures used for renumbering the objects */
	reorder = (int *)fz_calloc(ctx, n, sizeof(int));
	rev_renumber_map = (int *)fz_calloc(ctx, n, sizeof(int));
	for(i = 0; i < n; i++) {
		reorder[i] = i;
	}
...
static int isbinarystream(fz_context * ctx, const uchar * data, size_t len)
{
	size_t i;
	for(i = 0; i < len; i++)
		if(isbinary(data[i]))
...
static fz_buffer * deflatebuf(fz_context * ctx, const uchar * p, size_t n)
{
	fz_buffer * buf;
	uLongf csize;
	int t;
	...
	data = (uchar *)Memento_label(fz_malloc(ctx, cap), "pdf_write_deflate");
...
{
	int is_hex = 0;
	pdf_obj * f = pdf_dict_get(ctx, dict, PDF_NAME(Filter));
	pdf_obj * dp = pdf_dict_get(ctx, dict, PDF_NAME(DecodeParms));
...
static fz_buffer * unhexbuf(fz_context * ctx, const uchar * p, size_t n)
{
	fz_stream * mstm = NULL;
	fz_stream * xstm = NULL;
	fz_buffer * out = NULL;
...
static void write_data(fz_context * ctx, void * arg, const uchar * data, size_t len)
{
	fz_write_data(ctx, (fz_output*)arg, data, len);
}

static void copystream(fz_context * ctx,
    pdf_document * doc,
...
static void expandstream(fz_context * ctx,
    pdf_document * doc,
    pdf_write_state * opts,
    pdf_obj * obj_orig,
    int num,
	...
	fz_try(ctx)
	{
		...
		if(!skip) {
			if(pdf_obj_num_is_stream(ctx, doc, num)) {
				...
				fz_write_printf(ctx, opts->out, "%d %d obj\n", num, gen);
				pdf_print_encrypted_obj(ctx,
				    opts->out,
				    obj,
				    opts->do_tight,
				    opts->do_ascii,
				    unenc ? NULL : opts->crypt,
				    num,
				    gen);
				fz_write_string(ctx, opts->out, "\nendobj\n\n");
			}
...
static void writexref(fz_context * ctx,
    pdf_document * doc,
    pdf_write_state * opts,
    int from,
    int to,
...
static void writexrefstreamsubsect(fz_context * ctx,
    pdf_document * doc,
    pdf_write_state * opts,
    pdf_obj * index,
    fz_buffer * fzbuf,
...
static void writexrefstream(fz_context * ctx,
    pdf_document * doc,
    pdf_write_state * opts,
    int from,
    int to,
...
static void padto(fz_context * ctx, fz_output * out, int64_t target)
{
...
static void dowriteobject(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, int num, int pass)
{
...
static void writeobjects(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, int pass)
{
	int num;
	int xref_len = pdf_xref_len(ctx, doc);
...
static void make_page_offset_hints(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, fz_buffer * buf)
{
	int i, j;
	...
	fz_try(ctx)
	{
		for(s = 0; s < doc->num_incremental_sections; s++) {
			...
			if(xref->unsaved_sigs) {
				...
				buf = (char *)fz_calloc(ctx, buf_size, 1);
				...
				for(usig = xref->unsaved_sigs; usig; usig = usig->next) {
					...
					size_t bytes_read;
					int pnum =
					    pdf_obj_parent_num(ctx,
						pdf_dict_getl(ctx, usig->field, PDF_NAME(V), PDF_NAME(ByteRange), NULL));
					...
					bytes_read = fz_read(ctx, stm, (uchar*)buf, buf_size);
					assert(bytes_read <= buf_size);

					bstr = (char *)fz_memmem(buf, bytes_read, SLASH_BYTE_RANGE, sizeof(SLASH_BYTE_RANGE)-1);
					cstr = (char *)fz_memmem(buf, bytes_read, SLASH_CONTENTS, sizeof(SLASH_CONTENTS)-1);
					fstr = (char *)fz_memmem(buf, bytes_read, SLASH_FILTER, sizeof(SLASH_FILTER)-1);
				...
				for(usig = xref->unsaved_sigs; usig; usig = usig->next)
					pdf_dict_putl_drop(ctx, usig->field, pdf_copy_array(ctx, byte_range), PDF_NAME(V),
					    PDF_NAME(ByteRange), NULL);
...
static void prepare_for_save(fz_context * ctx, pdf_document * doc, pdf_write_options * in_opts)
{
...
static pdf_obj * new_identity(fz_context * ctx, pdf_document * doc)
{
	uchar rnd[32];
	pdf_obj * id;
...
static void change_identity(fz_context * ctx, pdf_document * doc, pdf_obj * id)
{
	uchar rnd[16];
...
static void create_encryption_dictionary(fz_context * ctx, pdf_document * doc, pdf_crypt * crypt)
{
	uchar * o, * u;
	pdf_obj * encrypt;
	int r;
	...
	if(r < 4) {
		...
		uchar * oe = pdf_crypt_owner_encryption(ctx, crypt);
		uchar * ue = pdf_crypt_user_encryption(ctx, crypt);
		pdf_obj * cf;
...
static void ensure_initial_incremental_contents(fz_context * ctx, fz_stream * in, fz_output * out)
{
	fz_stream * verify;
	uchar buf0[256];
	uchar buf1[256];
...
static void do_pdf_save_document(fz_context * ctx, pdf_document * doc, pdf_write_state * opts, pdf_write_options * in_opts)
{
	int lastfree;
	int num;
	int xref_len;
...
{
	for(int s = 0; s < doc->num_incremental_sections; s++) {
		pdf_xref * xref = &doc->xref_sections[doc->num_incremental_sections - s - 1];
		if(xref->unsaved_sigs)
			return 1;
	}
...
#define ADD_OPT(S) do { if(!first) fz_strlcat(buffer, ",", buffer_len); fz_strlcat(buffer, (S), buffer_len); first = 0; } while(0)
	...
	*buffer = 0;
	...
	switch(opts->do_encrypt) {
		default:
		case PDF_ENCRYPT_UNKNOWN: break;
		case PDF_ENCRYPT_NONE: ADD_OPT("encrypt=no"); break;
		case PDF_ENCRYPT_KEEP: ADD_OPT("encrypt=keep"); break;
		case PDF_ENCRYPT_RC4_40: ADD_OPT("encrypt=rc4-40"); break;
		case PDF_ENCRYPT_RC4_128: ADD_OPT("encrypt=rc4-128"); break;
		case PDF_ENCRYPT_AES_128: ADD_OPT("encrypt=aes-128"); break;
		case PDF_ENCRYPT_AES_256: ADD_OPT("encrypt=aes-256"); break;
	}
	...
	switch(opts->do_garbage) {
		case 0: break;
		case 1: ADD_OPT("garbage=yes"); break;
		case 2: ADD_OPT("garbage=compact"); break;
		case 3: ADD_OPT("garbage=deduplicate"); break;
		default:
			{
				char temp[32];
	...
	switch(opts->do_appearance) {
		case 1: ADD_OPT("appearance=yes"); break;
		case 2: ADD_OPT("appearance=all"); break;
	}
#undef ADD_OPT
	return buffer;
}
...
static fz_device * pdf_writer_begin_page(fz_context * ctx, fz_document_writer * wri_, fz_rect mediabox)
{
...
static void pdf_writer_end_page(fz_context * ctx, fz_document_writer * wri_, fz_device * dev)
{
	pdf_writer * wri = (pdf_writer*)wri_;
	pdf_obj * obj = NULL;
	fz_var(obj);
...
static void pdf_writer_close_writer(fz_context * ctx, fz_document_writer * wri_)
{
	pdf_writer * wri = (pdf_writer*)wri_;
...
static void pdf_writer_drop_writer(fz_context * ctx, fz_document_writer * wri_)
{
	pdf_writer * wri = (pdf_writer*)wri_;
	fz_drop_buffer(ctx, wri->contents);
	pdf_drop_obj(ctx, wri->resources);
...
{
	pdf_writer * wri = fz_new_derived_document_writer(ctx, pdf_writer, pdf_writer_begin_page, pdf_writer_end_page,
		pdf_writer_close_writer, pdf_writer_drop_writer);
	fz_try(ctx) {
