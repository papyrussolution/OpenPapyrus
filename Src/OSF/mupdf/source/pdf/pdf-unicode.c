...
static void pdf_remap_cmap_range(fz_context * ctx, pdf_cmap * ucs_from_gid,
    uint cpt, uint gid, uint n, pdf_cmap * ucs_from_cpt)
{
	uint k;
	int ucsbuf[8];
	int ucslen;
...
void pdf_load_to_unicode(fz_context * ctx, pdf_document * doc, pdf_font_desc * font,
