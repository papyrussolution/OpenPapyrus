...
static int pdf_code_from_string(char * buf, size_t len)
{
	uint a = 0;
	while(len--)
		a = (a << 8) | *(uchar*)buf++;
...
static void pdf_parse_cmap_name(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
...
static void pdf_parse_wmode(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok = pdf_lex(ctx, file, buf);
	if(tok == PDF_TOK_INT)
...
static int is_keyword(pdf_token tok, pdf_lexbuf * buf, const char * word)
{
	/* Ignore trailing garbage when matching keywords */
	return (tok == PDF_TOK_KEYWORD && !strncmp(buf->scratch, word, strlen(word)));
}

static void pdf_parse_codespace_range(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
	int lo, hi;
	while(1) {
...
static void pdf_parse_cid_range(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
	int lo, hi, dst;
	while(1) {
...
static void pdf_parse_cid_char(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
	int src, dst;
...
static void pdf_parse_bf_range_array(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf, int lo, int hi)
{
	pdf_token tok;
	int dst[256];
...
static void pdf_parse_bf_range(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
	int lo, hi, dst;
...
static void pdf_parse_bf_char(fz_context * ctx, pdf_cmap * cmap, fz_stream * file, pdf_lexbuf * buf)
{
	pdf_token tok;
	int dst[256];
	int src;
	...
	fz_try(ctx)
	{
		while(1) {
			...
			if(tok == PDF_TOK_EOF)
				...
				if(sstreq(buf.scratch, "CMapName"))
					pdf_parse_cmap_name(ctx, cmap, file, &buf);
				else if(sstreq(buf.scratch, "WMode"))
