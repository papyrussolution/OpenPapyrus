...
static void pdf_run_xobject(fz_context * ctx,
...
static pdf_gstate * pdf_begin_group(fz_context * ctx, pdf_run_processor * pr, fz_rect bbox, softmask_save * softmask)
{
...
static void pdf_copy_pattern_gstate(fz_context * ctx, pdf_gstate * dst, const pdf_gstate * src)
{
	dst->ctm = src->ctm;
...
static pdf_gstate * pdf_show_pattern(fz_context * ctx, pdf_run_processor * pr, pdf_pattern * pat, int pat_gstate_num, fz_rect area,
    int what)
{
	pdf_gstate * gstate;
...
static void pdf_show_image_imp(fz_context * ctx, pdf_run_processor * pr, fz_image * image, fz_matrix image_ctm, fz_rect bbox)
{
	...
	if(image->colorspace) {
		...
		fz_fill_image_mask(ctx,
		    pr->dev,
		    image,
		    image_ctm,
		    gstate->fill.colorspace,
		    gstate->fill.v,
		    gstate->fill.alpha,
		    gstate->fill.color_params);
	}
		...
		fz_fill_shade(ctx,
		    pr->dev,
		    gstate->fill.shade,
		    pr->gstate[gstate->fill.gstate_num].ctm,
		    gstate->fill.alpha,
		    gstate->fill.color_params);
		fz_pop_clip(ctx, pr->dev);
	}
...
static void pdf_show_path(fz_context * ctx, pdf_run_processor * pr, int doclose, int dofill, int dostroke, int even_odd)
{
	...
	if(dostroke) {
		...
		else if(gstate->stroke_state->dash_len != 0 &&
			...
			switch(gstate->fill.kind)
			{
				...
				case PDF_MAT_PATTERN:
				    if(gstate->fill.pattern) {
					    ...
					    gstate =
						pdf_show_pattern(ctx, pr, gstate->fill.pattern, gstate->fill.gstate_num, bbox, PDF_FILL);
					    fz_pop_clip(ctx, pr->dev);
				    }
				    break;
				case PDF_MAT_SHADE:
				    if(gstate->fill.shade) {
					    fz_clip_path(ctx, pr->dev, path, even_odd, gstate->ctm, bbox);
					    /* The cluster and page 2 of patterns.pdf shows that fz_fill_shade should
					       NOT be called with gstate->ctm. */
					    fz_fill_shade(ctx,
						pr->dev,
						gstate->fill.shade,
						pr->gstate[gstate->fill.gstate_num].ctm,
			...
			switch(gstate->stroke.kind)
			{
				...
				case PDF_MAT_PATTERN:
				    if(gstate->stroke.pattern) {
					    ...
					    gstate = pdf_show_pattern(ctx,
						    pr,
						    gstate->stroke.pattern,
						    gstate->stroke.gstate_num,
						    bbox,
						    PDF_STROKE);
					    fz_pop_clip(ctx, pr->dev);
				    }
				...
				case PDF_MAT_SHADE:
				    if(gstate->stroke.shade) {
						...
						pr->dev,
						gstate->stroke.shade,
						pr->gstate[gstate->stroke.gstate_num].ctm,
						gstate->stroke.alpha,
						gstate->stroke.color_params);
					    fz_pop_clip(ctx, pr->dev);
				    }
				    break;
			}
		...
		if(dofill) {
			switch(gstate->fill.kind)
			{
				...
				case PDF_MAT_SHADE:
				    if(gstate->fill.shade) {
					    fz_clip_text(ctx, pr->dev, text, gstate->ctm, tb);
					    /* Page 2 of patterns.pdf shows that fz_fill_shade should NOT be called with
					       gstate->ctm */
					    fz_fill_shade(ctx,
						pr->dev,
						gstate->fill.shade,
...
static void show_string(fz_context * ctx, pdf_run_processor * pr, uchar * buf, size_t len)
{
	pdf_gstate * gstate = pr->gstate + pr->gtop;
	pdf_font_desc * fontdesc = gstate->text.font;
	uchar * end = buf + len;
...
static void pdf_show_string(fz_context * ctx, pdf_run_processor * pr, uchar * buf, size_t len)
{
	pdf_gstate * gstate = pr->gstate + pr->gtop;
	pdf_font_desc * fontdesc = gstate->text.font;
	...
	if(pdf_is_array(ctx, text)) {
		...
		for(i = 0; i < n; i++) {
			...
			if(pdf_is_string(ctx, item))
				show_string(ctx, pr, (uchar*)pdf_to_str_buf(ctx, item), pdf_to_str_len(ctx, item));
			else
				pdf_show_space(ctx, pr, -pdf_to_real(ctx, item) * gstate->text.size * 0.001f);
		}
	}
	else if(pdf_is_string(ctx, text)) {
		pdf_show_string(ctx, pr, (uchar*)pdf_to_str_buf(ctx, text), pdf_to_str_len(ctx, text));
	}
}
...
static void pdf_init_gstate(fz_context * ctx, pdf_gstate * gs, fz_matrix ctm)
{
	gs->ctm = ctm;
	gs->clip_depth = 0;
...
static void pdf_copy_gstate(fz_context * ctx, pdf_gstate * dst, pdf_gstate * src)
{
...
static void pdf_run_j(fz_context * ctx, pdf_processor * proc, int linejoin)
{
	...
	gstate->stroke_state = fz_unshare_stroke_state(ctx, gstate->stroke_state);
	gstate->stroke_state->linejoin = (fz_linejoin)linejoin;
}
...
static void pdf_run_J(fz_context * ctx, pdf_processor * proc, int linecap)
{
	...
	pr->dev->flags &= ~(FZ_DEVFLAG_STARTCAP_UNDEFINED | FZ_DEVFLAG_DASHCAP_UNDEFINED | FZ_DEVFLAG_ENDCAP_UNDEFINED);
	gstate->stroke_state = fz_unshare_stroke_state(ctx, gstate->stroke_state);
	gstate->stroke_state->start_cap = (fz_linecap)linecap;
	gstate->stroke_state->dash_cap = (fz_linecap)linecap;
	gstate->stroke_state->end_cap = (fz_linecap)linecap;
}
...
{
	pdf_run_processor * pr = (pdf_run_processor*)proc;
	pdf_show_string(ctx, pr, (uchar*)string, string_len);
}
    ...
    fz_cookie * cookie)
{
	...
	fz_try(ctx)
	{
		...
		proc->gcap = 64;
		proc->gstate = (pdf_gstate *)fz_calloc(ctx, proc->gcap, sizeof(pdf_gstate));
		proc->gtop = 0;
