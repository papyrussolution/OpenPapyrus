...
static void draw_circle(fz_context * ctx, fz_buffer * buf, float rx, float ry, float cx, float cy)
{
...
static void draw_circle_in_box(fz_context * ctx, fz_buffer * buf, float lw, float x0, float y0, float x1, float y1)
{
	float rx = (x1 - x0) / 2 - lw;
	float ry = (y1 - y0) / 2 - lw;
	float cx = x0 + lw + rx;
...
static void pdf_write_circle_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect)
{
	float lw;
	int sc;
	int ic;
...
static void pdf_write_ink_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect)
{
...
static void pdf_write_highlight_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect, pdf_obj ** res)
{
...
static void pdf_write_caret_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect, fz_rect * bbox)
{
...
static void pdf_write_icon_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect, fz_rect * bbox)
{
	const char * name;
	float xc = (rect->x0 + rect->x1) / 2;
	float yc = (rect->y0 + rect->y1) / 2;
	...
	else if(sstreq(name, "PushPin"))
		fz_append_string(ctx, buf, icon_push_pin);
	else if(sstreq(name, "Paperclip"))
...
static float measure_stamp_string(fz_context * ctx, fz_font * font, const char * text)
{
	float w = 0;
	while(*text) {
		int c, g;
...
static void write_stamp_string(fz_context * ctx, fz_buffer * buf, fz_font * font, const char * text)
{
...
static void write_stamp(fz_context * ctx, fz_buffer * buf, fz_font * font, const char * text, float y, float h)
{
...
static void pdf_write_stamp_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, fz_rect * rect, fz_rect * bbox, pdf_obj ** res)
{
	fz_font * font;
	pdf_obj * res_font;
	pdf_obj * name;
...
static void add_required_fonts(fz_context * ctx, pdf_document * doc, pdf_obj * res_font,
...
static void init_text_walk(fz_context * ctx,
...
static float measure_string(fz_context * ctx, fz_text_language lang, fz_font * font, const char * a)
{
	struct text_walk_state state;
	float w = 0;
...
static float break_string(fz_context * ctx,
    fz_text_language lang,
    fz_font * font,
    float size,
    const char * text,
...
static void write_string(fz_context * ctx, fz_buffer * buf,
...
static void write_string_with_quadding(fz_context * ctx, fz_buffer * buf,
...
static void write_comb_string(fz_context * ctx, fz_buffer * buf,
    fz_text_language lang, const char * fontname,
...
static void layout_comb_string(fz_context * ctx, fz_layout_block * out, float x, float y,
...
static void layout_string(fz_context * ctx, fz_layout_block * out,
...
static void layout_string_with_quadding(fz_context * ctx, fz_layout_block * out,
...
{
	if(sstreq(*name, "Cour")) return "Courier";
	if(sstreq(*name, "Helv")) return "Helvetica";
	if(sstreq(*name, "TiRo")) return "Times-Roman";
	if(sstreq(*name, "Symb")) return "Symbol";
	if(sstreq(*name, "ZaDb")) return "ZapfDingbats";
	return *name = "Helv", "Helvetica";
}

static void write_variable_text(fz_context * ctx, pdf_annot * annot, fz_buffer * buf, pdf_obj ** res,
...
static void layout_variable_text(fz_context * ctx, fz_layout_block * out,
    ...
    fz_rect * rect, fz_rect * bbox, fz_matrix * matrix, pdf_obj ** res)
{
	...
	*bbox = fz_make_rect(0, 0, w, h);
	...
	write_variable_text(ctx, annot, buf, res, (fz_text_language)lang, text, font, size, color, q, w, h, b*2,
	    0.8f, 1.2f, 1, 0, 0);
}

static void pdf_write_tx_widget_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf,
    fz_rect * rect, fz_rect * bbox, fz_matrix * matrix, pdf_obj ** res,
...
static void pdf_write_ch_widget_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf,
    fz_rect * rect, fz_rect * bbox, fz_matrix * matrix, pdf_obj ** res)
{
...
static void pdf_write_sig_widget_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf,
    fz_rect * rect, fz_rect * bbox, fz_matrix * matrix, pdf_obj ** res)
{
	float x0 = rect->x0 + 1;
	float y0 = rect->y0 + 1;
...
static void pdf_write_widget_appearance(fz_context * ctx, pdf_annot * annot, fz_buffer * buf,
    fz_rect * rect, fz_rect * bbox, fz_matrix * matrix, pdf_obj ** res)
{
