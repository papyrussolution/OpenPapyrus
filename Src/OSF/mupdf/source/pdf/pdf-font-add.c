...
#endif
	if(sstreq(kind, "TrueType")) return 2;
	if(sstreq(kind, "Type 1")) return 1;
	if(sstreq(kind, "CFF")) return 3;
	if(sstreq(kind, "CID Type 1")) return 1;
	return 0;
}
...
static pdf_obj* pdf_add_font_file(fz_context * ctx, pdf_document * doc, fz_font * font)
{
	...
	fz_try(ctx)
	{
		...
		switch(ft_font_file_kind((FT_Face)font->ft_face)) {
			case 1:
			...
			case 2:
			    break;
			case 3:
			    if(FT_Get_Sfnt_Table((FT_Face)font->ft_face, FT_SFNT_HEAD))
...
static void pdf_add_font_descriptor(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, fz_font * font)
{
...
static void pdf_add_simple_font_widths(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, fz_font * font, const char * const encoding[])
{
	int width_table[256];
	pdf_obj * widths;
	int i;
...
static void pdf_add_cid_system_info(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, const char * reg, const char * ord, int supp)
{
...
static void pdf_add_cid_font_widths(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, fz_font * font)
{
	FT_Face face = (FT_Face)font->ft_face;
...
static pdf_obj* pdf_add_descendant_cid_font(fz_context * ctx, pdf_document * doc, fz_font * font)
{
	FT_Face face = (FT_Face)font->ft_face;
	pdf_obj * fobj, * fref;
	const char * ps_name;
...
static void pdf_add_to_unicode(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, fz_font * font)
{
	FT_Face face = (FT_Face)font->ft_face;
	fz_buffer * buf;
	int * table;
	...
	/* Populate reverse cmap table */
	{
		FT_ULong ucs;
		FT_UInt gid;
		table = (int*)fz_calloc(ctx, face->num_glyphs, sizeof *table);
...
static void pdf_add_simple_font_encoding_imp(fz_context * ctx, pdf_document * doc, pdf_obj * font, const char * glyph_names[])
{
	pdf_obj * enc, * diff;
	int i, last;
...
static void pdf_add_simple_font_encoding(fz_context * ctx, pdf_document * doc, pdf_obj * fobj, int encoding)
{
	switch(encoding)
	{
		default:
...
int pdf_font_writing_supported(fz_font * font)
{
	...
	if(is_ttc(font))
		return 0;
	if(is_truetype((FT_Face)font->ft_face))
		return 1;
	if(is_postscript((FT_Face)font->ft_face))
		return 1;
	return 0;
}

pdf_obj * pdf_add_cjk_font(fz_context * ctx, pdf_document * doc, fz_font * fzfont, int script, int wmode, int serif)
{
