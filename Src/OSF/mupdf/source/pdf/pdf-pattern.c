...
static void pdf_drop_pattern_imp(fz_context * ctx, fz_storable * pat_)
{
...
static uint pdf_pattern_size(pdf_pattern * pat)
{
	if(pat == NULL)
		return 0;
	return sizeof(*pat);
...
pdf_pattern * pdf_load_pattern(fz_context * ctx, pdf_document * doc, pdf_obj * dict)
{
	pdf_pattern * pat;
	if((pat = (pdf_pattern *)pdf_find_item(ctx, pdf_drop_pattern_imp, dict)) != NULL) {
		return pat;
	}
