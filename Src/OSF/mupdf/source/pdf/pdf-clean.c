 ...
 */
static void pdf_filter_content_stream(fz_context * ctx,
    pdf_document * doc,
    pdf_obj * in_stm,
    pdf_obj * in_res,
    fz_matrix transform,
	...
	fz_try(ctx)
	{
		...
		for(i = 0; i < n; i++) {
			...
			fz_try(ctx)
			{
				...
				if(filter->sanitize) {
					proc_filter = pdf_new_filter_processor(ctx,
						doc,
						proc_buffer,
						in_res,
						out_res,
...
static void pdf_filter_xobject(fz_context * ctx, pdf_document * doc, pdf_obj * stm, pdf_obj * page_res, pdf_filter_options * filter)
{
...
static void pdf_redact_end_page(fz_context * ctx, fz_buffer * buf, void * opaque)
{
	pdf_page * page = (pdf_page *)opaque;
	pdf_annot * annot;
	pdf_obj * qp;
	int i, n;
...
static int pdf_redact_text_filter(fz_context * ctx, void * opaque, int * ucsbuf, int ucslen, fz_matrix trm, fz_matrix ctm, fz_rect bbox)
{
	pdf_page * page = (pdf_page *)opaque;
	pdf_annot * annot;
	pdf_obj * qp;
...
static fz_pixmap * pdf_redact_image_imp(fz_context * ctx, fz_matrix ctm, fz_image * image, fz_pixmap * pixmap, fz_quad q)
{
	fz_matrix inv_ctm;
	fz_irect r;
	int x, y, k, n, bpp;
...
static fz_image * pdf_redact_image_filter_remove(fz_context * ctx, void * opaque, fz_matrix ctm, const char * name, fz_image * image)
{
...
static fz_image * pdf_redact_image_filter_pixels(fz_context * ctx, void * opaque, fz_matrix ctm, const char * name, fz_image * image)
{
	fz_pixmap * redacted = NULL;
	pdf_page * page = (pdf_page *)opaque;
	pdf_annot * annot;
...
static int pdf_redact_page_link(fz_context * ctx, pdf_document * doc, pdf_page * page, fz_rect area)
{
	pdf_annot * annot;
	pdf_obj * qp;
	fz_quad q;
