...
{
	pdf_graft_map * map = fz_malloc_struct(ctx, pdf_graft_map);
...
{
	return (pdf_graft_map *)fz_keep_imp(ctx, map, &map->refs);
}
	...
	if(pdf_is_indirect(ctx, obj)) {
		...
		if(map->src == NULL) {
			fz_try(ctx)
			{
				...
				map->dst_from_src = (int*)fz_calloc(ctx, map->len, sizeof(int));
			}
			fz_catch(ctx)
			{
				pdf_drop_document(ctx, map->src);
