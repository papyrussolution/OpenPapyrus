...
struct pdf_crypt {
	...
	uchar key[32]; /* decryption key generated from password */
};

static void pdf_parse_crypt_filter(fz_context * ctx, pdf_crypt_filter * cf, pdf_crypt * crypt, pdf_obj * name);

pdf_crypt * pdf_new_crypt(fz_context * ctx, pdf_obj * dict, pdf_obj * id)
{
	pdf_crypt * crypt = fz_malloc_struct(ctx, pdf_crypt);
	/* Common to all security handlers (PDF 1.7 table 3.18) */
	pdf_obj * obj = pdf_dict_get(ctx, dict, PDF_NAME(Filter));
...
static void pdf_compute_encryption_key(fz_context * ctx, pdf_crypt * crypt, uchar * password, size_t pwlen, uchar * key)
{
	uchar buf[32];
	uint p;
	int i;
	...
	/* Step 4 - pass P value as uint, low-order byte first */
	p = (uint)crypt->p;
	buf[0] = (p) & 0xFF;
	buf[1] = (p >> 8) & 0xFF;
	buf[2] = (p >> 16) & 0xFF;
	...
	fz_md5_update(&md5, (uchar*)pdf_to_str_buf(ctx, crypt->id), pdf_to_str_len(ctx, crypt->id));
 ...
 */
static void pdf_compute_encryption_key_r5(fz_context * ctx, pdf_crypt * crypt, uchar * password,
    size_t pwlen, int ownerkey, uchar * validationkey)
{
	uchar buffer[128 + 8 + 48];
	fz_sha256 sha256;
	fz_aes aes;
	...
	memzero(buffer + 32, sizeof(buffer) - 32);
 ...
 */
static void pdf_compute_hardened_hash_r6(fz_context * ctx, uchar * password, size_t pwlen, uchar salt[8], uchar * ownerkey, uchar hash[32])
{
	uchar data[(128 + 64 + 48) * 64];
	uchar block[64];
	int block_size = 32;
	size_t data_len = 0;
	int i, j, sum;
...
static void pdf_compute_encryption_key_r6(fz_context * ctx, pdf_crypt * crypt, uchar * password, size_t pwlen, int ownerkey, uchar * validationkey)
{
	uchar hash[32];
	uchar iv[16];
	fz_aes aes;
	if(pwlen > 127)
		pwlen = 127;
	pdf_compute_hardened_hash_r6(ctx, password, pwlen, (ownerkey ? crypt->o : crypt->u) + 32, ownerkey ? crypt->u : NULL, validationkey);
	pdf_compute_hardened_hash_r6(ctx, password, pwlen, (ownerkey ? crypt->o : crypt->u) + 40, (ownerkey ? crypt->u : NULL), hash);
...
static int pdf_authenticate_user_password(fz_context * ctx, pdf_crypt * crypt, uchar * password, size_t pwlen)
{
	uchar output[32];
...
int pdf_authenticate_password(fz_context * ctx, pdf_document * doc, const char * pwd_utf8)
{
	char password[2048];
	int auth;
	...
	auth = 0;
	if(pdf_authenticate_user_password(ctx, doc->crypt, (uchar*)password, strlen(password)))
		auth = 2;
	if(pdf_authenticate_owner_password(ctx, doc->crypt, (uchar*)password, strlen(password)))
		auth |= 4;
	else if(auth & 2) {
		 ...
		 * stored keys. */
		(void)pdf_authenticate_user_password(ctx, doc->crypt, (uchar*)password, strlen(password));
	}
...
int pdf_crypt_length(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->length : 0; }
int pdf_crypt_permissions(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->p : 0; }
int pdf_crypt_encrypt_metadata(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->encrypt_metadata : 0; }
uchar * pdf_crypt_owner_password(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->o : NULL; }
uchar * pdf_crypt_user_password(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->u : NULL; }
uchar * pdf_crypt_owner_encryption(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->oe : NULL; }
uchar * pdf_crypt_user_encryption(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->ue : NULL; }
uchar * pdf_crypt_permissions_encryption(fz_context * ctx, pdf_crypt * crypt) { return crypt ? crypt->perms : 0; }
/*
...
static int pdf_compute_object_key(pdf_crypt * crypt, pdf_crypt_filter * cf, int num, int gen, uchar * key, int max_len)
{
	fz_md5 md5;
	uchar message[5];
...
static int is_signature(fz_context * ctx, pdf_obj * obj)
{
	if(pdf_dict_get(ctx, obj, PDF_NAME(Type)) == PDF_NAME(Sig))
		if(pdf_dict_get(ctx, obj,
		    PDF_NAME(Contents)) && pdf_dict_get(ctx, obj, PDF_NAME(ByteRange)) && pdf_dict_get(ctx, obj, PDF_NAME(Filter)))
			return 1;
	return 0;
}

static void pdf_crypt_obj_imp(fz_context * ctx, pdf_crypt * crypt, pdf_obj * obj, uchar * key, int keylen)
{
	uchar * s;
	int i;
	...
	if(pdf_is_string(ctx, obj)) {
		...
		s = (uchar*)pdf_to_str_buf(ctx, obj);
...
/* PDF 2.0 algorithm 9 */
static void pdf_compute_owner_password_r6(fz_context * ctx, pdf_crypt * crypt, uchar * password,
...
static void pdf_compute_permissions_r6(fz_context * ctx, pdf_crypt * crypt, uchar * output)
{
	uchar buf[16];
	uchar iv[16];
	fz_aes aes;
	/* Steps a) and b) - Extend permissions field and put into lower order bytes. */
	memcpy(buf, (uchar*)&crypt->p, 4);
	memset(&buf[4], 0xff, 4);
...
pdf_crypt * pdf_new_encrypt(fz_context * ctx, const char * opwd_utf8, const char * upwd_utf8, pdf_obj * id, int permissions, int algorithm)
{
	...
	if(pdf_is_string(ctx, id))
		...
		fz_warn(ctx, "missing file identifier, may not be able to do decryption");
	switch(algorithm) {
		case PDF_ENCRYPT_RC4_40: v = 1; r = 2; method = PDF_CRYPT_RC4; length = 40; break;
		case PDF_ENCRYPT_RC4_128: v = 2; r = 3; method = PDF_CRYPT_RC4; length = 128; break;
		case PDF_ENCRYPT_AES_128: v = 4; r = 4; method = PDF_CRYPT_AESV2; length = 128; break;
		case PDF_ENCRYPT_AES_256: v = 5; r = 6; method = PDF_CRYPT_AESV3; length = 256; break;
		default: fz_throw(ctx, FZ_ERROR_GENERIC, "invalid encryption method");
	}
	crypt->v = v;
	crypt->r = r;
	crypt->length = length;
	...
	memzero(crypt->o, sizeof(crypt->o));
	memzero(crypt->u, sizeof(crypt->u));
	memzero(crypt->oe, sizeof(crypt->oe));
	memzero(crypt->ue, sizeof(crypt->ue));
	if(crypt->r <= 4) {
