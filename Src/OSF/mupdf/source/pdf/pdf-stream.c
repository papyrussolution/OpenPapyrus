...
int pdf_is_stream(fz_context * ctx, pdf_obj * ref)
{
 ...
 */
static int pdf_stream_has_crypt(fz_context * ctx, pdf_obj * stm)
{
	pdf_obj * filters;
	pdf_obj * obj;
	int i;
...
static fz_jbig2_globals * pdf_load_jbig2_globals(fz_context * ctx, pdf_obj * dict)
{
	fz_jbig2_globals * globals;
	fz_buffer * buf = NULL;
	fz_var(buf);
	if((globals = (fz_jbig2_globals *)pdf_find_item(ctx, fz_drop_jbig2_globals_imp, dict)) != NULL)
		return globals;
...
static void build_compression_params(fz_context * ctx, pdf_obj * f, pdf_obj * p, fz_compression_params * params)
{
 ...
 */
static fz_stream * build_filter(fz_context * ctx,
    fz_stream * chain,
    pdf_document * doc,
    pdf_obj * f,
    pdf_obj * p,
...
static fz_stream * build_filter_drop(fz_context * ctx,
    fz_stream * tail,
    pdf_document * doc,
    pdf_obj * f,
    pdf_obj * p,
 ...
 */
static fz_stream * build_filter_chain_drop(fz_context * ctx,
    fz_stream * chain,
    pdf_document * doc,
    pdf_obj * fs,
    pdf_obj * ps,
...
static fz_stream * build_filter_chain(fz_context * ctx,
    fz_stream * chain,
    pdf_document * doc,
    pdf_obj * fs,
    pdf_obj * ps,
 ...
 */
static fz_stream * pdf_open_raw_filter(fz_context * ctx,
    fz_stream * file_stm,
    pdf_document * doc,
    pdf_obj * stmobj,
    int num,
 ...
 */
static fz_stream * pdf_open_filter(fz_context * ctx,
    pdf_document * doc,
    fz_stream * file_stm,
    pdf_obj * stmobj,
    int num,
    int64_t offset,
    fz_compression_params * imparams)
{
...
static fz_stream * pdf_open_image_stream(fz_context * ctx, pdf_document * doc, int num, fz_compression_params * params)
{
	pdf_xref_entry * x;
...
static int pdf_guess_filter_length(int len, const char * filter)
{
	if(sstreq(filter, "ASCIIHexDecode"))
		return len / 2;
	if(sstreq(filter, "ASCII85Decode"))
		return len * 4 / 5;
	if(sstreq(filter, "FlateDecode"))
		return len * 3;
	if(sstreq(filter, "RunLengthDecode"))
		return len * 3;
	if(sstreq(filter, "LZWDecode"))
		return len * 2;
	return len;
}
 ...
 * return 0. */
static int can_reuse_buffer(fz_context * ctx, pdf_xref_entry * entry, fz_compression_params * params)
{
	pdf_obj * f;
	pdf_obj * p;
...
static fz_buffer * pdf_load_image_stream(fz_context * ctx, pdf_document * doc, int num, fz_compression_params * params, int * truncated)
{
	fz_stream * stm = NULL;
	pdf_obj * dict, * obj;
	int i, len, n;
...
static fz_stream * pdf_open_object_array(fz_context * ctx, pdf_document * doc, pdf_obj * list)
{
	fz_stream * stm;
	int i, n;
...
fz_stream * pdf_open_contents_stream(fz_context * ctx, pdf_document * doc, pdf_obj * obj)
{
	...
	return fz_open_memory(ctx, (uchar*)"", 0);
}
