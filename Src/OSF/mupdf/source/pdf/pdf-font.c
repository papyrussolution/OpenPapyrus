...
void pdf_load_encoding(const char ** estrings, const char * encoding)
{
	...
	int i;
	if(sstreq(encoding, "StandardEncoding"))
		bstrings = fz_glyph_name_from_adobe_standard;
	if(sstreq(encoding, "MacRomanEncoding"))
		bstrings = fz_glyph_name_from_mac_roman;
	if(sstreq(encoding, "MacExpertEncoding"))
		bstrings = fz_glyph_name_from_mac_expert;
	if(sstreq(encoding, "WinAnsiEncoding"))
...
const uchar * pdf_lookup_substitute_font(fz_context * ctx, int mono, int serif, int bold, int italic, int * len)
{
	if(mono) {
		if(bold) {
...
{
	int i, k;
	for(i = 0; i < SIZEOFARRAYi(base_font_names); i++)
...
#endif
	if(sstreq(kind, "TrueType")) return TRUETYPE;
	if(sstreq(kind, "Type 1")) return TYPE1;
	if(sstreq(kind, "CFF")) return TYPE1;
	if(sstreq(kind, "CID Type 1")) return TYPE1;
	return UNKNOWN;
}
...
{
	int i;
	for(i = 0; i < SIZEOFARRAYi(mre_diff_table); ++i)
		if(sstreq(name, mre_diff_table[i].name))
			return mre_diff_table[i].code;
	for(i = 0; i < 256; i++)
		if(fz_glyph_name_from_mac_roman[i] && sstreq(name, fz_glyph_name_from_mac_roman[i]))
			return i;
	return -1;
}
...
static void pdf_load_builtin_font(fz_context * ctx, pdf_font_desc * fontdesc, const char * fontname, int has_descriptor)
{
	FT_Face face;
	...
	if(sstreq(clean_name, "Symbol") || sstreq(clean_name, "ZapfDingbats"))
		fontdesc->flags |= PDF_FD_SYMBOLIC;

	face = (FT_Face)fontdesc->font->ft_face;
	fontdesc->ascent = 1000.0f * face->ascender / face->units_per_EM;
	fontdesc->descent = 1000.0f * face->descender / face->units_per_EM;
}

static void pdf_load_substitute_font(fz_context * ctx,
...
static void pdf_load_substitute_cjk_font(fz_context * ctx, pdf_font_desc * fontdesc, const char * fontname, int ros, int serif)
{
...
static void pdf_load_system_font(fz_context * ctx, pdf_font_desc * fontdesc, const char * fontname, const char * collection)
{
	int bold = 0;
	int italic = 0;
	int serif = 0;
...
static void pdf_load_embedded_font(fz_context * ctx, pdf_document * doc, pdf_font_desc * fontdesc, const char * fontname, pdf_obj * stmref)
{
	fz_buffer * buf;
...
void pdf_drop_font(fz_context * ctx, pdf_font_desc * fontdesc)
{
	fz_drop_storable(ctx, &fontdesc->storable);
}

static void pdf_drop_font_imp(fz_context * ctx, fz_storable * fontdesc_)
{
...
static FT_CharMap select_type1_cmap(FT_Face face)
{
	int i;
...
static FT_CharMap select_truetype_cmap(FT_Face face, int symbolic)
{
	int i;
	...
	fz_try(ctx)
	{
		...
		if(descriptor && pdf_is_string(ctx, pdf_dict_get(ctx, descriptor, PDF_NAME(FontName))) &&
			...
			for(i = 0; cp936fonts[i]; i += 2)
				if(sstreq(basefont, cp936fonts[i]))
					break;
			if(cp936fonts[i]) {
		...
		etable = Memento_label(fz_malloc_array(ctx, 256, ushort), "cid_to_gid");
		fontdesc->size += 256 * sizeof(ushort);
		for(i = 0; i < 256; i++) {
			estrings[i] = NULL;
			etable[i] = 0;
		}
...
static int hail_mary_make_hash_key(fz_context * ctx, fz_store_hash * hash, void * key_)
{
	hash->u.pi.i = 0;
	hash->u.pi.ptr = NULL;
	return 1;
...
static void * hail_mary_keep_key(fz_context * ctx, void * key)
{
	return key;
}

static void hail_mary_drop_key(fz_context * ctx, void * key)
{
}

static int hail_mary_cmp_key(fz_context * ctx, void * k0, void * k1)
{
	return k0 == k1;
}

static void hail_mary_format_key(fz_context * ctx, char * s, size_t n, void * key_)
{
...
pdf_font_desc * pdf_load_hail_mary_font(fz_context * ctx, pdf_document * doc)
{
	...
	if((fontdesc = (pdf_font_desc *)fz_find_item(ctx, pdf_drop_font_imp, &hail_mary_store_key, &hail_mary_store_type)) != NULL) {
		return fontdesc;
	}
	/* FIXME: Get someone with a clue about fonts to fix this */
	fontdesc = pdf_load_simple_font(ctx, doc, NULL);
	existing = (pdf_font_desc *)fz_store_item(ctx, &hail_mary_store_key, fontdesc, fontdesc->size, &hail_mary_store_type);
...
static pdf_font_desc * load_cid_font(fz_context * ctx, pdf_document * doc, pdf_obj * dict, pdf_obj * encoding, pdf_obj * to_unicode)
{
	pdf_obj * widths;
	pdf_obj * descriptor;
	pdf_font_desc * fontdesc = NULL;
	...
	fz_try(ctx)
	{
		...
		if(pdf_is_stream(ctx, cidtogidmap)) {
			...
			fontdesc->cid_to_gid = Memento_label(fz_malloc_array(ctx, fontdesc->cid_to_gid_len, ushort), "cid_to_gid_map");
			fontdesc->size += fontdesc->cid_to_gid_len * sizeof(ushort);
		...
		/* unicode cmap to get a glyph id */
			...
			else if(sstreq(collection, "Adobe-Japan2"))
...
static pdf_font_desc * pdf_load_type0_font(fz_context * ctx, pdf_document * doc, pdf_obj * dict)
{
	pdf_obj * dfonts;
	pdf_obj * dfont;
	pdf_obj * subtype;
...
static void pdf_make_width_table(fz_context * ctx, pdf_font_desc * fontdesc)
{
...
pdf_font_desc * pdf_load_font(fz_context * ctx, pdf_document * doc, pdf_obj * rdb, pdf_obj * dict)
{
	pdf_obj * subtype;
	pdf_obj * dfonts;
	pdf_obj * charprocs;
	...
	if((fontdesc = (pdf_font_desc *)pdf_find_item(ctx, pdf_drop_font_imp, dict)) != NULL) {
		return fontdesc;
	}
	subtype = pdf_dict_get(ctx, dict, PDF_NAME(Subtype));
	dfonts = pdf_dict_get(ctx, dict, PDF_NAME(DescendantFonts));
