...
static void extend_xref_index(fz_context * ctx, pdf_document * doc, int newlen)
{
	int i;
...
static void ensure_solid_xref(fz_context * ctx, pdf_document * doc, int num, int which)
{
	...
	fz_try(ctx)
	{
		new_sub->table = (pdf_xref_entry *)fz_calloc(ctx, num, sizeof(pdf_xref_entry));
		new_sub->start = 0;
		new_sub->len = num;
		new_sub->next = NULL;
	}
...
static void ensure_incremental_xref(fz_context * ctx, pdf_document * doc)
{
	...
	if((doc->num_incremental_sections == 0 || doc->xref_sections[0].unsaved_sigs != NULL)
		...
		pdf_xref * pxref;
		pdf_xref_entry * new_table = (pdf_xref_entry *)fz_calloc(ctx, xref->num_objects, sizeof(pdf_xref_entry));
...
void pdf_replace_xref(fz_context * ctx, pdf_document * doc, pdf_xref_entry * entries, int n)
{
	...
	fz_try(ctx)
	{
		xref_index = (int *)fz_calloc(ctx, n, sizeof(int));
...
static void pdf_load_version(fz_context * ctx, pdf_document * doc)
{
	char buf[20];
...
static void pdf_read_start_xref(fz_context * ctx, pdf_document * doc)
{
	uchar buf[1024];
	size_t i, n;
	int64_t t;
...
static void fz_skip_space(fz_context * ctx, fz_stream * stm)
{
	do {
...
static int pdf_xref_size_from_old_trailer(fz_context * ctx, pdf_document * doc, pdf_lexbuf * buf)
{
	int len;
	char * s;
	int64_t t;
	...
	while(1) {
		...
		if(len > 0) {
			n = fz_read(ctx, doc->file, (uchar*)buf->scratch, 20);
			if(n < 19)
...
static pdf_xref_entry * pdf_xref_find_subsection(fz_context * ctx, pdf_document * doc, int start, int len)
{
	pdf_xref * xref = &doc->xref_sections[doc->num_xref_sections-1];
	pdf_xref_subsec * sub;
	int num_objects;
	...
	if(sub == NULL) {
		...
		fz_try(ctx)
		{
			sub->table = (pdf_xref_entry *)fz_calloc(ctx, len, sizeof(pdf_xref_entry));
			sub->start = start;
			sub->len = len;
...
static pdf_obj * pdf_read_old_xref(fz_context * ctx, pdf_document * doc, pdf_lexbuf * buf)
{
	...
	while(1) {
		...
		for(i = 0; i < len; i++) {
			...
			n = fz_read(ctx, file, (uchar*)buf->scratch + carried, 20-carried);
...
static void pdf_read_new_xref_section(fz_context * ctx, pdf_document * doc, fz_stream * stm, int i0, int i1, int w0, int w1, int w2)
{
	pdf_xref_entry * table;
	int i, n;
...
static pdf_obj * pdf_read_new_xref(fz_context * ctx, pdf_document * doc, pdf_lexbuf * buf)
{
	fz_stream * stm = NULL;
	pdf_obj * trailer = NULL;
	pdf_obj * index = NULL;
...
static pdf_obj * pdf_read_xref(fz_context * ctx, pdf_document * doc, int64_t ofs, pdf_lexbuf * buf)
{
	pdf_obj * trailer;
	int c;
...
static int64_t read_xref_section(fz_context * ctx, pdf_document * doc, int64_t ofs, pdf_lexbuf * buf)
{
	pdf_obj * trailer = NULL;
	pdf_obj * prevobj;
...
static void pdf_read_xref_sections(fz_context * ctx, pdf_document * doc, int64_t ofs, pdf_lexbuf * buf, int read_previous)
{
	int i, len, cap;
	int64_t * offsets;
...
static void pdf_prime_xref_index(fz_context * ctx, pdf_document * doc)
{
	int i, j;
...
static void pdf_load_xref(fz_context * ctx, pdf_document * doc, pdf_lexbuf * buf)
{
	int i;
	int xref_len;
	pdf_xref_entry * entry;
	...
	for(i = 0; i < xref_len; i++) {
		...
		if(entry->type == 'o') {
			...
			if(ofs <= 0 || ofs >= xref_len || pdf_get_xref_entry(ctx, doc, ofs)->type != 'n')
				fz_throw(ctx,
				    FZ_ERROR_GENERIC,
				    "invalid reference to an objstm that does not exist: %d (%d 0 R)",
				    (int)ofs,
				    i);
		}
	}
...
static void pdf_check_linear(fz_context * ctx, pdf_document * doc)
{
	pdf_obj * dict = NULL;
	pdf_obj * o;
	int num, gen;
...
static void pdf_load_linear(fz_context * ctx, pdf_document * doc)
{
	pdf_obj * dict = NULL;
	pdf_obj * hint = NULL;
	pdf_obj * o;
	...
	fz_try(ctx)
	{
		...
		memzero(doc->linear_page_refs, doc->linear_page_count * sizeof(pdf_obj*));
		doc->linear_obj = dict;
...
static void pdf_init_document(fz_context * ctx, pdf_document * doc)
{
	pdf_obj * encrypt, * id;
	pdf_obj * dict = NULL;
	pdf_obj * obj;
	...
	fz_try(ctx)
	{
		...
		if(repaired) {
			...
			if(doc->xref_index)
				memzero(doc->xref_index, sizeof(int) * doc->max_xref_len);
...
static void pdf_drop_document_imp(fz_context * ctx, pdf_document * doc)
{
	int i;
...
static pdf_xref_entry * pdf_load_obj_stm(fz_context * ctx, pdf_document * doc, int num, pdf_lexbuf * buf, int target)
{
	fz_stream * stm = NULL;
	pdf_obj * objstm = NULL;
	int * numbuf = NULL;
	...
	fz_try(ctx)
	{
		...
		numbuf = (int *)fz_calloc(ctx, count, sizeof(*numbuf));
		ofsbuf = (int64_t *)fz_calloc(ctx, count, sizeof(*ofsbuf));
			...
			if(entry->type == 'o' && entry->ofs == num) {
				...
				if(entry->obj) {
					if(pdf_objcmp(ctx, entry->obj, obj))
						fz_warn(ctx,
						    "Encountered new definition for object %d - keeping the original one",
						    numbuf[i]);
 ...
 * object loading
 */
static int pdf_obj_read(fz_context * ctx, pdf_document * doc, int64_t * offset, int * nump, pdf_obj ** page)
{
...
static void pdf_load_hinted_page(fz_context * ctx, pdf_document * doc, int pagenum)
{
	pdf_obj * page = NULL;
...
static int read_hinted_object(fz_context * ctx, pdf_document * doc, int num)
{
...
void pdf_update_stream(fz_context * ctx, pdf_document * doc, pdf_obj * obj, fz_buffer * newbuf, int compressed)
{
	int num;
	pdf_xref_entry * x;
...
int pdf_lookup_metadata(fz_context * ctx, pdf_document * doc, const char * key, char * buf, int size)
{
	...
	if(sstreq(key, FZ_META_ENCRYPTION)) {
		if(doc->crypt)
...
static fz_location pdf_resolve_link_imp(fz_context * ctx, fz_document * doc_, const char * uri, float * xp, float * yp)
{
...
static pdf_document * pdf_new_document(fz_context * ctx, fz_stream * file)
{
...
static void pdf_load_hints(fz_context * ctx, pdf_document * doc, int objnum)
{
	...
	fz_try(ctx)
	{
		...
		memzero(doc->hint_page, sizeof(*doc->hint_page) * (doc->linear_page_count+1));
		doc->hint_obj_offsets = fz_realloc_array(ctx, doc->hint_obj_offsets, max_object_num, int64_t);
		memzero(doc->hint_obj_offsets, sizeof(*doc->hint_obj_offsets) * max_object_num);
		...
		memzero(doc->hint_shared_ref, sizeof(*doc->hint_shared_ref) * shared);
		fz_sync_bits(ctx, stream);
		...
		memzero(doc->hint_shared, sizeof(*doc->hint_shared) * (shared_obj_count_total+1));
...
static void pdf_load_hint_object(fz_context * ctx, pdf_document * doc)
{
	pdf_lexbuf * buf = &doc->lexbuf.base;
	int64_t curr_pos;
...
{
	for(int x = 0; x < doc->num_xref_sections; x++) {
		pdf_xref * xref = &doc->xref_sections[x];
		for(pdf_xref_subsec * sub = xref->subsec; sub; sub = sub->next) {
			for(int e = 0; e < sub->len; e++) {
				pdf_xref_entry * entry = &sub->table[e];
				// We cannot drop objects if the stream buffer has been updated 
				if(entry->obj && entry->stm_buf == NULL) {
...
static int check_unchanged_between(fz_context * ctx, pdf_document * doc, pdf_changes * changes, pdf_obj * nobj, pdf_obj * oobj)
{
	int marked = 0;
	int changed = 0;
...
static void free_char_list(fz_context * ctx, char_list * c)
{
	if(c) {
...
void pdf_drop_locked_fields(fz_context * ctx, pdf_locked_fields * fl)
{
	if(fl) {
...
static void char_list_append(fz_context * ctx, char_list * list, const char * s)
{
...
static void filter_changes_accepted(fz_context * ctx, pdf_changes * changes, pdf_obj * obj, filter_fn filter)
{
	int obj_num;
	...
	fz_try(ctx)
	{
		...
		if(pdf_is_dict(ctx, obj)) {
			const int n = pdf_dict_len(ctx, obj);
			for(int i = 0; i < n; i++) {
			...
			const int n = pdf_array_len(ctx, obj);
			for(int i = 0; i < n; i++) {
...
static void check_field(fz_context * ctx, pdf_document * doc, pdf_changes * changes, pdf_obj * obj, pdf_locked_fields * locked, const char * name_prefix,
    pdf_obj * new_v, pdf_obj * old_v)
{
	...
	fz_try(ctx)
	{
		...
		if(t) {
			...
			field_name = (char *)fz_malloc(ctx, n);
			if(*name_prefix) {
...
static int pdf_obj_changed_in_version(fz_context * ctx, pdf_document * doc, int num, int version)
{
...
static void merge_lock_specification(fz_context * ctx, pdf_locked_fields * fields, pdf_obj * lock)
{
	int i, r, w;
	if(lock) {
			...
			if(pdf_name_eq(ctx, action, PDF_NAME(Include))) {
				if(fields->all) {
					...
					for(i = 0; i < len; i++) {
						...
						for(r = 0; r < fields->includes.len; r++) {
							if(sstreq(s, fields->includes.list[r]))
								break;
						}
				...
				if(fields->all) {
					...
					for(r = w = 0; r < fields->excludes.len; r++) {
						for(i = 0; i < len; i++) {
							const char * s = pdf_to_text_string(ctx, pdf_array_get(ctx, f, i));
							if(sstreq(s, fields->excludes.list[r]))
								break;
						}
...
static void find_locked_fields_value(fz_context * ctx, pdf_locked_fields * fields, pdf_obj * v)
{
	pdf_obj * ref = pdf_dict_get(ctx, v, PDF_NAME(Reference));
	if(ref) {
		const int n = pdf_array_len(ctx, ref);
		for(int i = 0; i < n; i++) {
			pdf_obj * sr = pdf_array_get(ctx, ref, i);
			pdf_obj * tm, * tp;
			/* Type is optional, but if it exists, it'd better be SigRef. */
			pdf_obj * type = pdf_dict_get(ctx, sr, PDF_NAME(Type));
			if(type && !pdf_name_eq(ctx, type, PDF_NAME(SigRef)))
				continue;
...
static void find_locked_fields_aux(fz_context * ctx, pdf_obj * field, pdf_locked_fields * fields, pdf_obj * inherit_v, pdf_obj * inherit_ft)
{
	int i, n;
	...
	fz_try(ctx)
	{
		...
		SETIFZQ(ft, inherit_ft);
		/* We are looking for Widget annotations of type Sig that are
		 * signed (i.e. have a 'V' field). */
		if(pdf_name_eq(ctx, pdf_dict_get(ctx, field, PDF_NAME(Subtype)), PDF_NAME(Widget)) && pdf_name_eq(ctx, ft, PDF_NAME(Sig)) &&
...
static int validate_locked_fields(fz_context * ctx, pdf_document * doc, int version, pdf_locked_fields * locked)
{
	int o_xref_base = doc->xref_base;
	int i, n;
	int all_indirects = 1;
	int num_objs = doc->max_xref_len;
	pdf_changes * changes = (pdf_changes *)Memento_label(fz_calloc(ctx, 1, sizeof(*changes) + sizeof(int)*(num_objs-1)), "pdf_changes");
	...
	fz_try(ctx)
	{
		...
		for(i = 1; i < num_objs; i++) {
			...
			if(pdf_name_eq(ctx, otype, PDF_NAME(ObjStm)) || pdf_name_eq(ctx, otype, PDF_NAME(XRef))) {
				changes->obj_changes[i] |= FIELD_CHANGE_VALID;
			}
	...
	for(i = 1; i < num_objs; i++) {
		if(changes->obj_changes[i] == FIELD_CHANGED)
			break; /* Change with no reason */
		if(changes->obj_changes[i] & FIELD_CHANGE_INVALID)
			break; /* Illegal Change */
	}
	fz_free(ctx, changes);
...
static int pdf_find_incremental_update_num_for_obj(fz_context * ctx, pdf_document * doc, pdf_obj * obj)
{
	pdf_xref * xref = NULL;
	pdf_xref_subsec * sub;
	int i, j;
