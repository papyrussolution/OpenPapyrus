...
static fz_image * pdf_load_jpx_imp(fz_context * ctx, pdf_document * doc, pdf_obj * rdb, pdf_obj * dict, fz_stream * cstm, int forcemask)
{
...
static fz_image * pdf_load_image_imp(fz_context * ctx, pdf_document * doc, pdf_obj * rdb, pdf_obj * dict, fz_stream * cstm, int forcemask)
{
	fz_image * image = NULL;
	pdf_obj * obj, * res;
	...
	fz_try(ctx)
	{
		...
		if(cstm == NULL) {
			...
			image = fz_new_image_from_compressed_buffer(ctx, w, h, bpc, colorspace,
				96, 96, interpolate, imagemask, decode, use_colorkey ? colorkey : NULL, buffer, mask);
			...
			stride = (w * n * bpc + 7) / 8;
			image = fz_new_image_from_compressed_buffer(ctx, w, h, bpc, colorspace,
				96, 96, interpolate, imagemask, decode, use_colorkey ? colorkey : NULL, NULL, mask);
			image->invert_cmyk_jpeg = 0;
...
int pdf_is_jpx_image(fz_context * ctx, pdf_obj * dict)
{
	...
	if(pdf_name_eq(ctx, filter, PDF_NAME(JPXDecode)))
		...
		const int n = pdf_array_len(ctx, filter);
		for(int i = 0; i < n; i++)
...
fz_image * pdf_load_image(fz_context * ctx, pdf_document * doc, pdf_obj * dict)
{
	fz_image * image;
	if((image = (fz_image *)pdf_find_item(ctx, fz_drop_image_imp, dict)) != NULL)
		return image;
...
static uint32_t getu32(const uchar * data)
{
	return ((uint32_t)data[0]<<24) | ((uint32_t)data[1]<<16) | ((uint32_t)data[2]<<8) | (uint32_t)data[3];
}

static size_t pdf_parse_jbig2_segment_header(fz_context * ctx,
...
static void pdf_copy_jbig2_segments(fz_context * ctx, fz_buffer * output, const uchar * data, size_t size, int page)
{
	struct jbig2_segment_header info;
...
static void pdf_copy_jbig2_random_segments(fz_context * ctx, fz_buffer * output, const uchar * data, size_t size, int page)
{
	struct jbig2_segment_header info;
	const uchar * header = data;
	const uchar * header_end;
	const uchar * end = data + size;
	size_t n;
	int type;
...
static fz_buffer * pdf_jbig2_stream_from_file(fz_context * ctx, fz_buffer * input, fz_jbig2_globals * globals_, int embedded, int page)
{
			...
			switch(cp->type)
			{
				...
				case FZ_IMAGE_FAX:
				    ...
				    if(cp->u.fax.damaged_rows_before_error)
					    pdf_dict_put_int(ctx, dp, PDF_NAME(DamagedRowsBeforeError),
						cp->u.fax.damaged_rows_before_error);
...
unknown_compression:
			...
			if(fz_is_pixmap_monochrome(ctx, pixmap)) {
				...
				int h = pixmap->h;
				int w = pixmap->w;
				uchar * s = pixmap->samples;
				uchar * d = (uchar*)fz_calloc(ctx, h, stride);
				...
				int h = pixmap->h;
				uchar * s = pixmap->samples;
				uchar * d = (uchar*)Memento_label(fz_malloc(ctx, size * h), "pdf_image_samples");
		...
		if(image->imagemask) {
			...
			switch(fz_colorspace_type(ctx, cs)) {
			    ...
			    {
				    fz_colorspace * basecs = cs->u.indexed.base;
				    int high = cs->u.indexed.high;
				    uchar * lookup = cs->u.indexed.lookup;
				    int basen = basecs->n;
				    pdf_obj * arr = pdf_dict_put_array(ctx, imobj, PDF_NAME(ColorSpace), 4);
