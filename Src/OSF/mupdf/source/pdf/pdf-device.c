...
/* Helper functions */
static void pdf_dev_stroke_state(fz_context * ctx, pdf_device * pdev, const fz_stroke_state * stroke_state)
{
	...
	else if(!gs->stroke_state || gs->stroke_state->dash_phase != stroke_state->dash_phase ||
...
static void pdf_dev_path_moveto(fz_context * ctx, void * arg, float x, float y)
{
	fz_buffer * buf = (fz_buffer*)arg;
	fz_append_printf(ctx, buf, "%g %g m\n", x, y);
}

static void pdf_dev_path_lineto(fz_context * ctx, void * arg, float x, float y)
{
	fz_buffer * buf = (fz_buffer*)arg;
	fz_append_printf(ctx, buf, "%g %g l\n", x, y);
}

static void pdf_dev_path_curveto(fz_context * ctx, void * arg, float x1, float y1, float x2, float y2, float x3, float y3)
{
	fz_buffer * buf = (fz_buffer*)arg;
	fz_append_printf(ctx, buf, "%g %g %g %g %g %g c\n", x1, y1, x2, y2, x3, y3);
}

static void pdf_dev_path_close(fz_context * ctx, void * arg)
{
	fz_buffer * buf = (fz_buffer*)arg;
...
static void pdf_dev_path(fz_context * ctx, pdf_device * pdev, const fz_path * path)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_ctm(fz_context * ctx, pdf_device * pdev, fz_matrix ctm)
{
	fz_matrix inverse;
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_alpha(fz_context * ctx, pdf_device * pdev, float alpha, int stroke)
{
	int i;
...
static int pdf_dev_add_font_res(fz_context * ctx, pdf_device * pdev, fz_font * font)
{
	pdf_obj * fres;
	char text[32];
	int k;
...
static void pdf_dev_font(fz_context * ctx, pdf_device * pdev, fz_font * font, fz_matrix trm)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_push_new_buf(fz_context * ctx,
    pdf_device * pdev,
    fz_buffer * buf,
    void (*on_pop)(fz_context*, pdf_device*, void*),
    void * on_pop_arg)
...
static void pdf_dev_push(fz_context * ctx, pdf_device * pdev)
{
	pdf_dev_push_new_buf(ctx, pdev, NULL, NULL, NULL);
}

static void * pdf_dev_pop(fz_context * ctx, pdf_device * pdev)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_text_span(fz_context * ctx, pdf_device * pdev, fz_text_span * span)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_trm(fz_context * ctx, pdf_device * pdev, int trm)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_begin_text(fz_context * ctx, pdf_device * pdev, int trm)
{
...
static void pdf_dev_end_text(fz_context * ctx, pdf_device * pdev)
{
	gstate * gs = CURRENT_GSTATE(pdev);
...
static int pdf_dev_new_form(fz_context * ctx,
    pdf_obj ** form_ref,
    pdf_device * pdev,
    fz_rect bbox,
    int isolated,
...
static void pdf_dev_fill_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm,
...
static void pdf_dev_stroke_path(fz_context * ctx, fz_device * dev, const fz_path * path, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_clip_path(fz_context * ctx, fz_device * dev, const fz_path * path, int even_odd, fz_matrix ctm, fz_rect scissor)
{
	pdf_device * pdev = (pdf_device*)dev;
	gstate * gs;
...
static void pdf_dev_clip_stroke_path(fz_context * ctx,
    fz_device * dev,
    const fz_path * path,
...
static void pdf_dev_fill_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
	fz_text_span * span;
...
static void pdf_dev_stroke_text(fz_context * ctx, fz_device * dev, const fz_text * text, const fz_stroke_state * stroke, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
	fz_text_span * span;
...
static void pdf_dev_clip_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm, fz_rect scissor)
{
	pdf_device * pdev = (pdf_device*)dev;
	fz_text_span * span;
...
static void pdf_dev_clip_stroke_text(fz_context * ctx,
    fz_device * dev,
    const fz_text * text,
    const fz_stroke_state * stroke,
    fz_matrix ctm,
...
static void pdf_dev_ignore_text(fz_context * ctx, fz_device * dev, const fz_text * text, fz_matrix ctm)
{
	pdf_device * pdev = (pdf_device*)dev;
	fz_text_span * span;
...
static void pdf_dev_add_image_res(fz_context * ctx, fz_device * dev, pdf_obj * im_res)
{
	char text[32];
	pdf_device * pdev = (pdf_device*)dev;
	int k;
...
static void pdf_dev_fill_image(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
	pdf_obj * im_res;
...
static void pdf_dev_fill_shade(fz_context * ctx, fz_device * dev, fz_shade * shade, fz_matrix ctm, float alpha,
    fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
...
static void pdf_dev_fill_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm,
    fz_colorspace * colorspace, const float * color, float alpha, fz_color_params color_params)
{
	pdf_device * pdev = (pdf_device*)dev;
	pdf_obj * im_res = NULL;
...
static void pdf_dev_clip_image_mask(fz_context * ctx, fz_device * dev, fz_image * image, fz_matrix ctm, fz_rect scissor)
{
	pdf_device * pdev = (pdf_device*)dev;
...
static void pdf_dev_pop_clip(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
...
static void pdf_dev_begin_mask(fz_context * ctx,
    fz_device * dev,
    fz_rect bbox,
    int luminosity,
    fz_colorspace * colorspace,
...
static void pdf_dev_end_mask(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
	pdf_document * doc = pdev->doc;
	gstate * gs = CURRENT_GSTATE(pdev);
...
static void pdf_dev_begin_group(fz_context * ctx,
    fz_device * dev,
    fz_rect bbox,
    fz_colorspace * cs,
    int isolated,
...
static void pdf_dev_end_group(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
	pdf_document * doc = pdev->doc;
	gstate * gs = CURRENT_GSTATE(pdev);
...
static int pdf_dev_begin_tile(fz_context * ctx, fz_device * dev, fz_rect area, fz_rect view, float xstep, float ystep, fz_matrix ctm,
    int id)
{
	pdf_device * pdev = (pdf_device*)dev;
...
static void pdf_dev_end_tile(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
...
static void pdf_dev_close_device(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
	pdf_dev_end_text(ctx, pdev);
}

static void pdf_dev_drop_device(fz_context * ctx, fz_device * dev)
{
	pdf_device * pdev = (pdf_device*)dev;
	int i;
