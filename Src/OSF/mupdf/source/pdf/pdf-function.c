...
pdf_function * pdf_keep_function(fz_context * ctx, pdf_function * func) { return (pdf_function *)fz_keep_storable(ctx, &func->storable); }
void pdf_drop_function(fz_context * ctx, pdf_function * func) { fz_drop_storable(ctx, &func->storable); }
size_t pdf_function_size(fz_context * ctx, const pdf_function * func) { return (func ? func->size : 0); }
...
static inline int ps_overflow(const ps_stack * st, int n) { return n < 0 || st->sp + n >= SIZEOFARRAYi(st->stack); }
static inline int ps_underflow(const ps_stack * st, int n) { return n < 0 || n > st->sp; }
static inline int ps_is_type(const ps_stack * st, int t) { return !ps_underflow(st, 1) && st->stack[st->sp - 1].type == t; }
static inline int ps_is_type2(const ps_stack * st, int t) { return !ps_underflow(st, 2) && st->stack[st->sp - 1].type == t && st->stack[st->sp - 2].type == t; }

static void ps_push_bool(ps_stack * st, int b)
{
	if(!ps_overflow(st, 1)) {
...
static void ps_push_int(ps_stack * st, int n)
{
	if(!ps_overflow(st, 1)) {
...
static void ps_push_real(ps_stack * st, float n)
{
	if(!ps_overflow(st, 1)) {
		st->stack[st->sp].type = PS_REAL;
		if(isnan(n)) {
			// Push 1.0, as it's a small known value that won't cause a divide by 0. Same reason as in fz_atof.
			n = 1.0f;
		}
...
static int ps_pop_bool(ps_stack * st)
{
...
static int ps_pop_int(ps_stack * st)
{
	if(!ps_underflow(st, 1)) {
...
static float ps_pop_real(ps_stack * st)
{
	if(!ps_underflow(st, 1)) {
		if(ps_is_type(st, PS_INT))
			return st->stack[--st->sp].u.i;
...
static void ps_copy(ps_stack * st, int n)
{
...
static void ps_roll(ps_stack * st, int n, int j)
{
...
static void ps_run(fz_context * ctx, psobj * code, ps_stack * st, int pc)
{
	int i1, i2;
	float r1, r2;
	int b1, b2;
...
static void resize_code(fz_context * ctx, pdf_function * func, int newsize)
{
...
static void parse_code(fz_context * ctx, pdf_function * func, fz_stream * stream, int * codeptr, pdf_lexbuf * buf)
{
	pdf_token tok;
		...
		switch(tok) {
			...
			case PDF_TOK_OPEN_BRACE:
			    ...
			    *codeptr += 4;
			    ...
			    else if(sstreq(buf->scratch, "ifelse")) {
				    if(elseptr < 0)
...
static void load_postscript_func(fz_context * ctx, pdf_function * func, pdf_obj * dict)
{
...
static void eval_postscript_func(fz_context * ctx, pdf_function * func, const float * in, float * out)
{
	ps_stack st;
	float x;
	int i;
...
static void load_sample_func(fz_context * ctx, pdf_function * func, pdf_obj * dict)
{
	fz_stream * stream;
	pdf_obj * obj;
	int samplecount;
...
static float interpolate_sample(pdf_function * func, int * scale, int * e0, int * e1, float * efrac, int dim, int idx)
{
	float a, b;
	const int idx0 = e0[dim] * scale[dim] + idx;
	const int idx1 = e1[dim] * scale[dim] + idx;
	if(dim == 0) {
...
static void eval_sample_func(fz_context * ctx, pdf_function * func, const float * in, float * out)
{
 ...
 */
static void load_exponential_func(fz_context * ctx, pdf_function * func, pdf_obj * dict)
{
	pdf_obj * obj;
	int i;
	if(func->m > 1)
	...
	if(func->u.e.n != (int)func->u.e.n) {
		...
		for(i = 0; i < func->m; i++)
			if(func->domain[i][0] == 0 || func->domain[i][1] == 0 || (func->domain[i][0] < 0 && func->domain[i][1] > 0))
...
static void eval_stitching_func(fz_context * ctx, pdf_function * func, float in, float * out)
{
	float low, high;
	int k = func->u.st.k;
...
static void pdf_drop_function_imp(fz_context * ctx, fz_storable * func_)
{
...
static void pdf_eval_function_imp(fz_context * ctx, pdf_function * func, const float * in, float * out)
{
	switch(func->type)
	{
...
pdf_function * pdf_load_function(fz_context * ctx, pdf_obj * dict, int in, int out)
{
	pdf_function * func;
	pdf_obj * obj;
	int i;
	...
	if((func = (pdf_function *)pdf_find_item(ctx, pdf_drop_function_imp, dict)) != NULL)
		return func;
