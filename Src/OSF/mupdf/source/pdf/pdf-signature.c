...
void pdf_write_digest(fz_context * ctx,
    ...
    pdf_obj * byte_range,
    pdf_obj * field,
    size_t hexdigest_offset,
    size_t hexdigest_length,
    pdf_pkcs7_signer * signer)
{
	...
	fz_try(ctx)
	{
		int i;
		size_t z;
		brange = (fz_range *)fz_calloc(ctx, brange_len, sizeof(*brange));
		...
		digest = (uchar *)fz_malloc(ctx, len);
...
static void check_field_locking(fz_context * ctx, pdf_obj * obj, void * data_, pdf_obj ** ff)
{
	...
	fz_try(ctx)
	{
		...
		prefix = (fieldname_prefix *)fz_calloc(ctx, 1, sizeof(*prefix)+n);
...
static void pop_field_locking(fz_context * ctx, pdf_obj * obj, void * data_)
{
	fieldname_prefix * prefix;
	sig_locking_data * data = (sig_locking_data*)data_;
...
void pdf_sign_signature(fz_context * ctx, pdf_widget * widget, pdf_pkcs7_signer * signer)
{
	...
	fz_try(ctx)
	{
		...
		if((sf & (PDF_SIGFLAGS_SIGSEXIST | PDF_SIGFLAGS_APPENDONLY)) != (PDF_SIGFLAGS_SIGSEXIST | PDF_SIGFLAGS_APPENDONLY))
			pdf_dict_put_drop(ctx, form, PDF_NAME(SigFlags),
			    pdf_new_int(ctx, sf | PDF_SIGFLAGS_SIGSEXIST | PDF_SIGFLAGS_APPENDONLY));
    ...
    pdf_obj * signature)
{
	...
	fz_try(ctx)
	dn = verifier->get_signatory(ctx, verifier, (uchar*)contents, contents_len);
	fz_always(ctx)
...
pdf_signature_error pdf_check_digest(fz_context * ctx, pdf_pkcs7_verifier * verifier, pdf_document * doc, pdf_obj * signature)
{
	...
	fz_try(ctx)
	{
		...
		result = verifier->check_digest(ctx, verifier, bytes, (uchar*)contents, contents_len);
	}
	fz_always(ctx)
	{
...
pdf_signature_error pdf_check_certificate(fz_context * ctx, pdf_pkcs7_verifier * verifier, pdf_document * doc, pdf_obj * signature)
{
	...
	pdf_signature_error result = PDF_SIGNATURE_ERROR_UNKNOWN;
	fz_try(ctx)
	result = verifier->check_certificate(ctx, verifier, (uchar*)contents, contents_len);
	fz_always(ctx)
	fz_free(ctx, contents);
	fz_catch(ctx)
	fz_rethrow(ctx);
...
int pdf_check_signature(fz_context * ctx,
    pdf_pkcs7_verifier * verifier,
    pdf_document * doc,
    pdf_obj * signature,
    char * ebuf,
