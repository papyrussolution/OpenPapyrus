...
static void pdf_load_name_tree_imp(fz_context * ctx, pdf_obj * dict, pdf_document * doc, pdf_obj * node)
{
	pdf_obj * kids = pdf_dict_get(ctx, node, PDF_NAME(Kids));
	pdf_obj * names = pdf_dict_get(ctx, node, PDF_NAME(Names));
	int i;
...
pdf_obj * pdf_lookup_number(fz_context * ctx, pdf_obj * node, int needle)
{
	pdf_obj * kids = pdf_dict_get(ctx, node, PDF_NAME(Kids));
...
static void pdf_walk_tree_kid(fz_context * ctx,
    pdf_obj * obj,
    pdf_obj * kid_name,
    ...
    pdf_obj ** inherit_vals)
{
	...
	fz_try(ctx)
	{
		if(pdf_is_array(ctx, obj)) {
			...
			for(i = 0; i < n; i++)
				pdf_walk_tree_kid(ctx, pdf_array_get(ctx, obj, i), kid_name, arrive, leave, arg, inherit_names,
				    inherit_vals);
		}
		else {
