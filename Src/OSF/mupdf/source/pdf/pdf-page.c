...
static int pdf_load_page_tree_imp(fz_context * ctx, pdf_document * doc, pdf_obj * node, int idx)
{
...
static int cmp_rev_page_map(const void * va, const void * vb)
{
	const pdf_rev_page_map * a = (const pdf_rev_page_map *)va;
	const pdf_rev_page_map * b = (const pdf_rev_page_map *)vb;
...
static pdf_obj * pdf_lookup_page_loc_imp(fz_context * ctx, pdf_document * doc, pdf_obj * node, int * skip, pdf_obj ** parentp, int * indexp)
{
	pdf_obj * kids;
	pdf_obj * hit = NULL;
	int i, len;
	...
	fz_try(ctx)
	{
		do {
			...
			for(i = 0; i < len; i++) {
				...
				if(type ? pdf_name_eq(ctx, type, PDF_NAME(Pages)) : pdf_dict_get(ctx, kid,
				    PDF_NAME(Kids)) && !pdf_dict_get(ctx, kid, PDF_NAME(MediaBox))) {
...
static int pdf_count_pages_before_kid(fz_context * ctx, pdf_document * doc, pdf_obj * parent, int kid_num)
{
...
static int pdf_lookup_page_number_slow(fz_context * ctx, pdf_document * doc, pdf_obj * node)
{
...
static int pdf_lookup_page_number_fast(fz_context * ctx, pdf_document * doc, int needle)
{
	int l = 0;
...
static int pdf_extgstate_uses_blending(fz_context * ctx, pdf_obj * dict)
{
...
static int pdf_pattern_uses_blending(fz_context * ctx, pdf_obj * dict)
{
	pdf_obj * obj;
...
static int pdf_xobject_uses_blending(fz_context * ctx, pdf_obj * dict)
{
...
static int pdf_extgstate_uses_overprint(fz_context * ctx, pdf_obj * dict)
{
...
static int pdf_pattern_uses_overprint(fz_context * ctx, pdf_obj * dict)
{
	pdf_obj * obj;
	obj = pdf_dict_get(ctx, dict, PDF_NAME(Resources));
...
static int pdf_xobject_uses_overprint(fz_context * ctx, pdf_obj * dict)
{
	pdf_obj * obj = pdf_dict_get(ctx, dict, PDF_NAME(Resources));
...
void pdf_page_transform(fz_context * ctx, pdf_page * page, fz_rect * page_mediabox, fz_matrix * page_ctm)
{
	pdf_page_obj_transform(ctx, page->obj, page_mediabox, page_ctm);
}

static void find_seps(fz_context * ctx, fz_separations ** seps, pdf_obj * obj, pdf_obj * clearme)
{
	int i, n;
	...
	if(pdf_name_eq(ctx, nameobj, PDF_NAME(Separation))) {
		...
		for(i = 0; i < n; i++) {
			if(sstreq(name, fz_separation_name(ctx, *seps, i)))
...
static void find_devn(fz_context * ctx, fz_separations ** seps, pdf_obj * obj, pdf_obj * clearme)
{
	int i, j, n, m;
	pdf_obj * arr;
	...
	for(j = 0; j < m; j++) {
		...
		/* Skip 'special' colorants. */
		if(sstreq(name, "Black") || sstreq(name, "Cyan") || sstreq(name, "Magenta") || sstreq(name, "Yellow") || sstreq(name, "All") || sstreq(name, "None"))
			continue;
		n = fz_count_separations(ctx, *seps);
		for(i = 0; i < n; i++) {
			if(sstreq(name, fz_separation_name(ctx, *seps, i)))
...
static void scan_page_seps(fz_context * ctx, pdf_obj * res, fz_separations ** seps, res_finder_fn * fn, pdf_obj * clearme)
{
	pdf_obj * dict;
	pdf_obj * obj;
	int i, n;
...
int pdf_page_uses_overprint(fz_context * ctx, pdf_page * page)
{
	return page ? page->overprint : 0;
}

static void pdf_drop_page_imp(fz_context * ctx, pdf_page * page)
{
...
static pdf_page * pdf_new_page(fz_context * ctx, pdf_document * doc)
{
...
static void pdf_load_default_colorspaces_imp(fz_context * ctx, fz_default_colorspaces * default_cs, pdf_obj * obj)
{
	pdf_obj * cs_obj;
...
fz_page * pdf_load_page_imp(fz_context * ctx, fz_document * doc_, int chapter, int number)
{
...
void pdf_delete_page(fz_context * ctx, pdf_document * doc, int at)
{
	pdf_obj * parent, * kids;
	int i;
...
void pdf_insert_page(fz_context * ctx, pdf_document * doc, int at, pdf_obj * page_ref)
{
	int count = pdf_count_pages(ctx, doc);
	pdf_obj * parent, * kids;
	int i;
