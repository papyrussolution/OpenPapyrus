...
static pdf_font_desc * pdf_try_load_font(fz_context * ctx, pdf_document * doc, pdf_obj * rdb, pdf_obj * font, fz_cookie * cookie)
{
...
static fz_image * parse_inline_image(fz_context * ctx, pdf_csi * csi, fz_stream * stm, char * csname, int cslen)
{
...
static void pdf_process_extgstate(fz_context * ctx, pdf_processor * proc, pdf_csi * csi, pdf_obj * dict)
{
	...
	if(pdf_is_int(ctx, obj) && proc->op_J)
		proc->op_J(ctx, proc, sclamp(pdf_to_int(ctx, obj), 0, 2));
...
static void pdf_process_Do(fz_context * ctx, pdf_processor * proc, pdf_csi * csi)
{
	...
	else if(sstreq(pdf_to_name(ctx, subtype), "PS"))
...
static void pdf_process_CS(fz_context * ctx, pdf_processor * proc, pdf_csi * csi, int stroke)
{
	...
	if(sstreq(csi->name, "Pattern")) {
		...
		if(sstreq(csi->name, "DeviceGray"))
			cs = fz_keep_colorspace(ctx, fz_device_gray(ctx));
		else if(sstreq(csi->name, "DeviceRGB"))
			cs = fz_keep_colorspace(ctx, fz_device_rgb(ctx));
		else if(sstreq(csi->name, "DeviceCMYK"))
...
static int is_known_bad_word(const char * word)
{
	switch(*word)
	{
		case 'I': return sstreq(word, "Infinity");
		case 'N': return sstreq(word, "NaN");
		case 'i': return sstreq(word, "inf");
		case 'n': return sstreq(word, "nan");
	}
	return 0;
}
...
static void pdf_process_keyword(fz_context * ctx, pdf_processor * proc, pdf_csi * csi, fz_stream * stm, char * word)
{
	...
	switch(key)
	{
		...
		case A('j'): if(proc->op_j) proc->op_j(ctx, proc, sclamp(s[0], 0.0f, 2.0f)); break;
		case A('J'): if(proc->op_J) proc->op_J(ctx, proc, sclamp(s[0], 0.0f, 2.0f)); break;
				...
				if(in_text_array) {
					switch(tok)
					{
						...
						case PDF_TOK_KEYWORD:
						    if(buf->scratch[0] == 'T' && (buf->scratch[1] == 'w' || buf->scratch[1] == 'c') &&
							buf->scratch[2] == 0) {
 ...
 * printed. */
static int pdf_should_print_annot(fz_context * ctx, pdf_annot * annot)
{
...
void pdf_process_annot(fz_context * ctx, pdf_processor * proc, pdf_document * doc, pdf_page * page, pdf_annot * annot, fz_cookie * cookie)
{
	...
	if(proc->usage) {
		if(sstreq(proc->usage, "Print")) {
			if(!(flags & PDF_ANNOT_IS_PRINT))
				return;
			if(!pdf_should_print_annot(ctx, annot))
				return;
		}
		if(sstreq(proc->usage, "View") && (flags & PDF_ANNOT_IS_NO_VIEW))
			return;
	}
