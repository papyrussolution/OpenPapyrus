...
static void lex_comment(fz_context * ctx, fz_stream * f)
{
	int c;
	do {
		c = lex_byte(ctx, f);
...
static int lex_number(fz_context * ctx, fz_stream * f, pdf_lexbuf * buf, int c)
{
	char * s = buf->scratch;
...
static void lex_name(fz_context * ctx, fz_stream * f, pdf_lexbuf * lb)
{
	char * s = lb->scratch;
...
static int lex_string(fz_context * ctx, fz_stream * f, pdf_lexbuf * lb)
{
	char * s = lb->scratch;
	char * e = s + lb->size;
	int bal = 1;
...
static int lex_hex_string(fz_context * ctx, fz_stream * f, pdf_lexbuf * lb)
{
	char * s = lb->scratch;
	char * e = s + lb->size;
	int a = 0, x = 0;
...
static pdf_token pdf_token_from_keyword(char * key)
{
	switch(*key) {
		case 'R':
		    ...
		    break;
		case 't':
		    if(sstreq(key, "true")) return PDF_TOK_TRUE;
		    if(sstreq(key, "trailer")) return PDF_TOK_TRAILER;
		    break;
		case 'f':
		    if(sstreq(key, "false")) return PDF_TOK_FALSE;
		    break;
		case 'n':
		    if(sstreq(key, "null")) return PDF_TOK_NULL;
		    break;
		case 'o':
		    if(sstreq(key, "obj")) return PDF_TOK_OBJ;
		    break;
		case 'e':
		    if(sstreq(key, "endobj")) return PDF_TOK_ENDOBJ;
		    if(sstreq(key, "endstream")) return PDF_TOK_ENDSTREAM;
		    break;
		case 's':
		    if(sstreq(key, "stream")) return PDF_TOK_STREAM;
		    if(sstreq(key, "startxref")) return PDF_TOK_STARTXREF;
		    break;
		case 'x':
...
ptrdiff_t pdf_lexbuf_grow(fz_context * ctx, pdf_lexbuf * lb)
{
	...
	if(lb->size == lb->base_size) {
		lb->scratch = (char *)Memento_label(fz_malloc(ctx, newsize), "pdf_lexbuf");
		memcpy(lb->scratch, lb->buffer, lb->size);
	}
	else {
		lb->scratch = (char *)fz_realloc(ctx, lb->scratch, newsize);
	}
	lb->size = newsize;
	return lb->scratch - old;
}

pdf_token pdf_lex(fz_context * ctx, fz_stream * f, pdf_lexbuf * buf)
{
	while(1) {
		...
		switch(c)
		{
			...
			case '<':
			    ...
			    if(c != EOF)
				    fz_unread_byte(ctx, f);
			    return (pdf_token)lex_hex_string(ctx, f, buf);
			case '>':
			    c = lex_byte(ctx, f);
			    if(c == '>')
...
pdf_token pdf_lex_no_string(fz_context * ctx, fz_stream * f, pdf_lexbuf * buf)
{
	while(1) {
		int c = lex_byte(ctx, f);
		switch(c)
