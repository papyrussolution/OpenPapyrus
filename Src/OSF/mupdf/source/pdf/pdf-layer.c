...
static int count_entries(fz_context * ctx, pdf_obj * obj)
{
...
static pdf_ocg_ui * get_ocg_ui(fz_context * ctx, pdf_ocg_descriptor * desc, int fill)
{
...
static int populate_ui(fz_context * ctx,
...
static void drop_ui(fz_context * ctx, pdf_ocg_descriptor * desc)
{
	if(!desc)
		return;
...
static void load_ui(fz_context * ctx, pdf_ocg_descriptor * desc, pdf_obj * ocprops, pdf_obj * occg)
{
	pdf_obj * order;
	pdf_obj * rbgroups;
	pdf_obj * locked;
	...
	if(desc->num_ui_entries == 0)
		return;
	desc->ui = (pdf_ocg_ui *)Memento_label(fz_calloc(ctx, count, sizeof(pdf_ocg_ui)), "pdf_ocg_ui");
	fz_try(ctx)
	{
...
void pdf_select_layer_config(fz_context * ctx, pdf_document * doc, int config)
{
	int i, j, len, len2;
	pdf_ocg_descriptor * desc = doc->ocg;
	pdf_obj * cobj;
	pdf_obj * name;
	pdf_obj * obj = pdf_dict_get(ctx, pdf_dict_get(ctx, pdf_trailer(ctx, doc), PDF_NAME(Root)), PDF_NAME(OCProperties));
	if(!obj) {
		if(config == 0)
			return;
		else
...
int pdf_count_layer_config_ui(fz_context * ctx, pdf_document * doc) { return (doc && doc->ocg) ? doc->ocg->num_ui_entries : 0; }
...
void pdf_toggle_layer_config_ui(fz_context * ctx, pdf_document * doc, int ui)
{
	...
	entry = &doc->ocg->ui[ui];
	if(entry->button_flags != PDF_LAYER_UI_RADIOBOX && entry->button_flags != PDF_LAYER_UI_CHECKBOX)
		return;
	if(entry->locked)
		return;
...
void pdf_layer_config_ui_info(fz_context * ctx, pdf_document * doc, int ui, pdf_layer_config_ui * info)
{
	...
	if(info) {
		...
		info->selected = 0;
		info->text = NULL;
		info->type = (pdf_layer_config_ui_type)0;
		if(doc == NULL || doc->ocg == NULL)
			return;
		if(ui < 0 || ui >= doc->ocg->num_ui_entries)
			fz_throw(ctx, FZ_ERROR_GENERIC, "Out of range UI entry selected");
		entry = &doc->ocg->ui[ui];
		info->type = (pdf_layer_config_ui_type)entry->button_flags;
...
static int ocg_intents_include(fz_context * ctx, pdf_ocg_descriptor * desc, const char * name)
{
	int i, len;
	if(sstreq(name, "All"))
		return 1;
...
void pdf_read_ocg(fz_context * ctx, pdf_document * doc)
{
	...
	fz_try(ctx)
	{
		...
		desc->len = len;
		desc->ocgs = (pdf_ocg_entry *)fz_calloc(ctx, len, sizeof(*desc->ocgs));
		desc->intent = NULL;
		for(i = 0; i < len; i++) {
