...
pdf_cmap * pdf_load_embedded_cmap(fz_context * ctx, pdf_document * doc, pdf_obj * stmobj)
{
	...
	fz_var(cmap);
	fz_var(usecmap);
	if((cmap = (pdf_cmap *)pdf_find_item(ctx, pdf_drop_cmap_imp, stmobj)) != NULL)
		return cmap;
	fz_try(ctx)
	{
...
{
	if(sstreq(name, "Identity-H")) return pdf_new_identity_cmap(ctx, 0, 2);
	if(sstreq(name, "Identity-V")) return pdf_new_identity_cmap(ctx, 1, 2);
	return NULL;
}
...
pdf_cmap * pdf_load_system_cmap(fz_context * ctx, const char * cmap_name)
{
	pdf_cmap * usecmap;
	pdf_cmap * cmap;
