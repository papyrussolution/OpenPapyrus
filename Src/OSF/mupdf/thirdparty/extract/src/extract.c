 ...
 */
{
	...
	for(p = 0; p<document->pages_num; ++p) {
		...
		for(i = 0; i<page->images_num; ++i) {
			image_t* image;
			if(extract_realloc2(&images.images, sizeof(image_t) * images.images_num, sizeof(image_t) * (images.images_num + 1))) 
				goto end;
			image = &page->images[i];
			...
			/* Add image type if we haven't seen it before. */
			{
				...
				for(it = 0; it<images.imagetypes_num; ++it) {
					outf("it=%i images.imagetypes[it]=%s image->type=%s", it, images.imagetypes[it], image->type);
...
compressed_error:
						outf("Unrecognised hex character '%x' at offset %lli in image data", cc,
						    (long long)(c-tag.text.chars));
						errno = EINVAL;
						goto end;
					}
