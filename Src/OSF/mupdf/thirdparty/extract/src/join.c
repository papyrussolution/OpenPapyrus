...
{
	static char ret[64];
	snprintf(ret, sizeof(ret), "{%f %f %f %f %f %f}", matrix->a, matrix->b, matrix->c, matrix->d, matrix->e, matrix->f);
	return ret;
}
...
static const char* line_string2(line_t* line)
{
	...
	int i;
	extract_astring_free(&ret);
	snprintf(buffer, sizeof(buffer), "line x=%f y=%f spans_num=%i:", line->spans[0]->chars[0].x, line->spans[0]->chars[0].y, line->spans_num);
...
static const char* paragraph_string(paragraph_t* paragraph)
{
	...
	if(paragraph->lines_num) {
		...
		if(paragraph->lines_num > 1) {
			extract_astring_cat(&ret, "..");
			extract_astring_cat(&ret, line_string2(paragraph->lines[paragraph->lines_num-1]));
		}
	}
	return ret.chars;
}
...
/* Creates representation of span_t's that consists of a list of line_t's, with
   ...
   On exit:
    ...
    undefined.
 */
static int make_lines(span_t**    spans,
    int spans_num,
    line_t***   o_lines,
    int*        o_lines_num
    )
 ...
 */
static double line_distance(double ax,
    double ay,
    double bx,
    double by,
    double angle
...
{
	const paragraph_t* const* a_paragraph = (const paragraph_t* const*)a;
	const paragraph_t* const* b_paragraph = (const paragraph_t* const*)b;
...
/* Creates a representation of line_t's that consists of a list of
   ...
   On exit:
    ...
    are undefined.
 */
static int make_paragraphs(line_t**        lines,
    int lines_num,
    ...
    )
{
...

	{
		...
		paragraphs_num = to;
		if(extract_realloc2(&paragraphs, sizeof(paragraph_t*) * paragraphs_num_old, sizeof(paragraph_t*) * paragraphs_num)) {
	...
	*o_paragraphs_num = paragraphs_num;
	...
	if(ret) {
		if(paragraphs) {
			for(a = 0; a<paragraphs_num; ++a) {
				if(paragraphs[a]) 
					extract_free(&paragraphs[a]->lines);
