...
/* Appends new char_t to an span_t with .ucs=c and all other fields zeroed. */
...
/* List of lines that are aligned and adjacent to each other so as to form a paragraph. */
...
/* Information about an image. <type> is as passed to extract_add_image();
   <name> and <id> are created to be unique identifiers for use in generated docx file. */
...
typedef struct {
	...
	line_t**    lines;
	int lines_num;
	/* These refer to items in .spans. Initially empty, then set by extract_join(). */

	paragraph_t**   paragraphs;
	int paragraphs_num;
	/* These refer to items in .lines. Initially empty, then set by extract_join(). */
} page_t;
