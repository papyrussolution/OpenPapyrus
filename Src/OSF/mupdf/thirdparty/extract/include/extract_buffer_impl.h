...
int extract_buffer_read_internal(extract_buffer_t*   buffer, void*               data, size_t numbytes, size_t*             o_actual);
/* Internal use only. */

static inline int extract_buffer_read(extract_buffer_t*   buffer, void* data, size_t numbytes, size_t* o_actual)
{
	extract_buffer_cache_t* cache = (extract_buffer_cache_t*)buffer;
...
int extract_buffer_write_internal(extract_buffer_t*   buffer, const void*         data, size_t numbytes, size_t*             o_actual);
/* Internal use only. */

static inline int extract_buffer_write(extract_buffer_t*   buffer, const void* data, size_t numbytes, size_t* o_actual)
{
	extract_buffer_cache_t* cache = (extract_buffer_cache_t*)buffer;
	if(cache->numbytes - cache->pos < numbytes) {
		/* Can't use just the cache. */
