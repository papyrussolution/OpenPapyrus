...
static void Ap_join(js_State * J)
{
	...
	for(k = 0; k < len; ++k) {
		...
		n += strlen(r);
		if(k == 0) {
			out = (char * volatile)js_malloc(J, n);
			strcpy(out, r);
		}
		else {
			n += seplen;
			out = (char * volatile)js_realloc(J, out, n);
			strcat(out, sep);
			strcat(out, r);
		}
		js_pop(J, 1);
...
{
	const struct sortslot * aslot = (const struct sortslot *)avoid;
	const struct sortslot * bslot = (const struct sortslot *)bvoid;
...
static void Ap_sort(js_State * J)
{
	...
	array = (struct sortslot *)js_malloc(J, len * sizeof *array);
...
static void Ap_indexOf(js_State * J)
{
	int k;
	int len = js_getlength(J, 0);
	int from = js_isdefined(J, 2) ? js_tointeger(J, 2) : 0;
...
static void Ap_lastIndexOf(js_State * J)
{
	int k;
	int len = js_getlength(J, 0);
	int from = js_isdefined(J, 2) ? js_tointeger(J, 2) : len - 1;
