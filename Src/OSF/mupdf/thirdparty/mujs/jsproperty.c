...
static js_Property * __delete(js_State * J, js_Object * obj, js_Property * node, const char * name)
{
	js_Property * temp, * succ;
	if(node != &sentinel) {
		int c = strcmp(name, node->name);
		if(c < 0) {
			node->left = __delete(J, obj, node->left, name);
		}
		else if(c > 0) {
			node->right = __delete(J, obj, node->right, name);
		}
		else {
			if(node->left == &sentinel) {
				...
				node->right = __delete(J, obj, node->right, succ->name);
			}
		}
...
{
	obj->properties = __delete(J, obj, obj->properties, name);
}
...
static js_Iterator * itwalk(js_State * J, js_Iterator * iter, js_Property * prop, js_Object * seen)
{
	...
	if(!(prop->atts & JS_DONTENUM)) {
		if(!seen || !jsV_getenumproperty(J, seen, prop->name)) {
			js_Iterator * head = (js_Iterator *)js_malloc(J, sizeof *head);
			head->name = prop->name;
			head->next = iter;
			iter = head;
		}
