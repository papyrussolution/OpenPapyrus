...
int js_isstring(js_State * J, int idx) {
	enum js_Type t = (enum js_Type)stackidx(J, idx)->type; return t == JS_TSHRSTR || t == JS_TLITSTR || t == JS_TMEMSTR;
}
...
int js_iscoercible(js_State * J, int idx) {
	js_Value * v = stackidx(J, idx); return v->type != JS_TUNDEFINED && v->type != JS_TNULL;
}
...
static int jsR_delproperty(js_State * J, js_Object * obj, const char * name)
{
	...
	if(obj->type == JS_CARRAY) {
		...
		if(obj->u.user.__delete && obj->u.user.__delete(J, obj->u.user.data, name))
			return 1;
	}
