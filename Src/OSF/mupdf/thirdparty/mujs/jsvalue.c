...
{
	js_Object * obj = jsV_newobject(J, (js_Class)type, NULL);
	obj->u.f.function = fun;
	obj->u.f.scope = scope;
	js_pushobject(J, obj);
}
...
void js_newuserdatax(js_State * J, const char * tag, void * data, js_HasProperty has, js_Put put, js_Delete __delete, js_Finalize finalize)
{
	js_Object * prototype = NULL;
	js_Object * obj;
	if(js_isobject(J, -1))
...
void js_concat(js_State * J)
{
	...
	if(js_isstring(J, -2) || js_isstring(J, -1)) {
		...
		char * sab = (char *)js_malloc(J, strlen(sa) + strlen(sb) + 1);
		strcpy(sab, sa);
		strcat(sab, sb);
		if(js_try(J)) {
			js_free(J, sab);
