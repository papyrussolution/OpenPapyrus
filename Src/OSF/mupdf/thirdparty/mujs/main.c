...
static char * read_stdin(void)
{
	...
	for(;;) {
		...
		if(!ss) {
			SAlloc::F(s);
			slfprintf_stderr("cannot allocate storage for stdin contents\n");
			return NULL;
		}
		s = ss;
	...
	if(ferror(stdin)) {
		SAlloc::F(s);
		slfprintf_stderr("error reading stdin\n");
		return NULL;
	}
...
{
	slfprintf_stderr("Usage: mujs [options] [script [scriptArgs*]]\n");
	slfprintf_stderr("\t-i: Enter interactive prompt after running code.\n");
	slfprintf_stderr("\t-s: Check strictness.\n");
	exit(1);
}
