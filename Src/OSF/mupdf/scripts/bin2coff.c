...
main(int argc, char * argv[])
{
	...
	if((argc < 3) || (argc > 5)) {
		...
		slfprintf_stderr("  bin  : source binary data\n");
		slfprintf_stderr("  obj  : target object file, in MS COFF format.\n");
		slfprintf_stderr("  label: identifier for the extern data. If not provided, the name of the\n");
		slfprintf_stderr("         binary file without extension is used.\n");
		slfprintf_stderr("  64bit:\n  Win32:\n  x64  : produce a 64 bit compatible object - symbols are generated without\n");
		slfprintf_stderr("         leading underscores and machine type is set to x86_x64.\n\n");
		slfprintf_stderr("With your linker set properly, typical access from a C source is:\n\n");
		slfprintf_stderr("    extern uint8_t  label[]     /* binary data         */\n");
		slfprintf_stderr("    extern uint32_t label_size  /* size of binary data */\n\n");
		exit(1);
	}

	if(((uint8_t*)&endian_test)[0] == 0xBE) {
		slfprintf_stderr("\nThis program is not compatible with Big Endian architectures.\n");
		slfprintf_stderr("You are welcome to modify the sourcecode (GPLv3+) to make it so.\n");
		exit(1);
	}
	...
	alloc_size = sizeof(IMAGE_FILE_HEADER) + sizeof(IMAGE_SECTION_HEADER) + size + sizeof(SIZE_TYPE) + 2*sizeof(IMAGE_SYMBOL) +
	    sizeof(IMAGE_STRINGS);
	if(!short_label) {
	...
	if(buffer == NULL) {
		slfprintf_stderr("Couldn't allocate buffer.\n");
		goto err;
	}
	...
	string_table =
	    (IMAGE_STRINGS*)&buffer[sizeof(IMAGE_FILE_HEADER) + sizeof(IMAGE_SECTION_HEADER) + size + sizeof(SIZE_TYPE) + 2*
	    sizeof(IMAGE_SYMBOL)];
	...
	STRNSCPY((char *)section_header->Name, ".data");
	...
	section_header->Characteristics = IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_ALIGN_16BYTES | IMAGE_SCN_MEM_READ |
	    IMAGE_SCN_MEM_WRITE;

	/* Populate data section */
	if(fread(&buffer[sizeof(IMAGE_FILE_HEADER) + sizeof(IMAGE_SECTION_HEADER)], 1, size, fd) != size) {
		slfprintf_stderr("Couldn't read file '%s'.\n", argv[1]);
		goto err;
	}
	fclose(fd); fd = NULL;
...
typedef struct {
	...
	*/
	...
	if(short_label) {
		...
		strcpy((char *)&symbol_table[0].N.ShortName[x86_32], label);
	}
	else {
#ifdef SLIBINCLUDED
		symbol_table[0].N.Name.Short = 0;
		symbol_table[0].N.Name.Long = sizeof(IMAGE_STRINGS);
#else
	...
	if(short_size) {
		...
		strcpy((char *)&symbol_table[1].N.ShortName[x86_32], label);
		strcpy((char *)&symbol_table[1].N.ShortName[x86_32+strlen(label)], SIZE_LABEL_SUFFIX);
	}
	else {
#ifdef SLIBINCLUDED
		symbol_table[1].N.Name.Short = 0;
		symbol_table[1].N.Name.Long = sizeof(IMAGE_STRINGS) + ((short_label) ? 0 : (x86_32 + (uint32_t)strlen(label) + 1));
#else
	...
	fd = fopen(argv[2], "wb");
	if(fd == NULL) {
		slfprintf_stderr("Couldn't create file '%s'.\n", argv[2]);
		goto err;
	}

	if(fwrite(buffer, 1, alloc_size, fd) != alloc_size) {
		slfprintf_stderr("Couldn't write file '%s'.\n", argv[2]);
		goto err;
	}
