...
*/
void pdf_map_range_to_range(fz_context *ctx, pdf_cmap *cmap, uint srclo, uint srchi, int dstlo);

/*
	Add a single one-to-many mapping.
*/
void pdf_map_one_to_many(fz_context *ctx, pdf_cmap *cmap, uint one, int *many, size_t len);
...
*/
int pdf_decode_cmap(pdf_cmap *cmap, uchar *s, uchar *e, uint *cpt);
