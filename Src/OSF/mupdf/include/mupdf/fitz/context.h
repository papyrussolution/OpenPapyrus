 ...
 */
#define fz_malloc_struct(CTX, TYPE) ((TYPE*)Memento_label(fz_calloc(CTX, 1, sizeof(TYPE)), #TYPE))
/**
        ...
        Throws exception in the event of failure to allocate.
 */
#define fz_malloc_array(CTX, COUNT, TYPE) ((TYPE*)Memento_label(fz_malloc(CTX, (COUNT)* sizeof(TYPE)), #TYPE "[]"))
#define fz_realloc_array(CTX, OLD, COUNT, TYPE) ((TYPE*)Memento_label(fz_realloc(CTX, OLD, (COUNT)* sizeof(TYPE)), #TYPE "[]"))
/**
        ...
        Throws exception in the event of failure to allocate.
 */
void * FASTCALL fz_malloc(fz_context * ctx, size_t size);
/**
 ...
 */
static inline void fz_lock(fz_context * ctx, int lock)
{
 ...
 */
static inline void fz_unlock(fz_context * ctx, int lock)
{
...
static inline void * fz_keep_imp(fz_context * ctx, void * p, int * refs)
{
	if(p) {
...
static inline void * fz_keep_imp8(fz_context * ctx, void * p, int8_t * refs)
{
	if(p) {
...
static inline void * fz_keep_imp16(fz_context * ctx, void * p, int16_t * refs)
{
	if(p) {
...
static inline int fz_drop_imp(fz_context * ctx, void * p, int * refs)
{
	if(p) {
		int drop;
		(void)Memento_checkIntPointerOrNull(refs);
...
static inline int fz_drop_imp8(fz_context * ctx, void * p, int8_t * refs)
{
	if(p) {
		int drop;
		(void)Memento_checkBytePointerOrNull(refs);
...
static inline int fz_drop_imp16(fz_context * ctx, void * p, int16_t * refs)
{
	if(p) {
		int drop;
		(void)Memento_checkShortPointerOrNull(refs);
