...
/**
        ...
        failure to allocate.
 */
fz_stream * fz_open_memory(fz_context * ctx, const uchar * data, size_t len);
 ...
 */
size_t fz_read(fz_context * ctx, fz_stream * stm, uchar * data, size_t len);
 ...
 */
static inline uint fz_read_bits(fz_context * ctx, fz_stream * stm, int n)
{
	int x;
...
/**
        ...
        Returns (uint)-1 for EOF, or the required number of bits.
 */
static inline uint fz_read_rbits(fz_context * ctx, fz_stream * stm, int n)
{
	int x;
