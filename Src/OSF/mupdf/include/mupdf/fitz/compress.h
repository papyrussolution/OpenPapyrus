...
/**
	...
	*compressed_length will be updated on exit to contain the size
	actually used.
 */
void fz_deflate(fz_context *ctx, uchar *dest, size_t *compressed_length, const uchar *source, size_t source_length, fz_deflate_level level);
...
*/
fz_buffer *fz_compress_ccitt_fax_g3(fz_context *ctx, const uchar *data, int columns, int rows);
...
*/
fz_buffer *fz_compress_ccitt_fax_g4(fz_context *ctx, const uchar *data, int columns, int rows);
