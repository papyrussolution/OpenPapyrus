...
void align_left_shift()
{
	...
	while(pc->IsNotNullChunk()) {
		...
		if(start->IsNotNullChunk() && ((pc->GetFlags() & PCF_IN_PREPROC) != (start->GetFlags() & PCF_IN_PREPROC))) {
		...
		else if(start->IsNotNullChunk() && pc->GetLevel() < start->GetLevel()) {
		...
		else if(!pc->TestFlags(PCF_IN_ENUM) && !pc->TestFlags(PCF_IN_TYPEDEF) && pc->IsString("<<")) {
