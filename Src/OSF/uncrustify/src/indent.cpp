...
void reindent_line(Chunk * pc, size_t column)
{
	...
	int col_delta = static_cast<int>(column - pc->GetColumn());
	size_t min_col   = column;
	pc->SetColumn(column);
	do {
...
static Chunk * candidate_chunk_first_on_line(Chunk * pc)
{
	...
	if(options::indent_inside_ternary_operator() && (  first->Is(CT_QUESTION) || first->Is(CT_COND_COLON))) {
...
void indent_text()
{
	...
	while(pc->IsNotNullChunk()) {
		...
		if(options::use_options_overriding_for_qt_macros() && (sstreq(pc->Text(), "SIGNAL") || sstreq(pc->Text(), "SLOT"))) {
		...
		const size_t parent_token_indent = (options::indent_brace_parent()) ? token_indent(pc->GetParentType()) : 0;
		...
		if(options::indent_func_def_force_col1()) {
			if(!in_func_def) {
				...
				if(pc->GetParentType() == CT_FUNC_DEF || (pc->Is(CT_COMMENT) && next->IsNotNullChunk() && next->GetParentType() == CT_FUNC_DEF)) {
					in_func_def = true;
		...
		do {
			...
			if(frm.top().level >= pc->GetLevel()) {
				...
				// End any assign operations with a semicolon on the same level
				if(pc->IsSemicolon() && ((frm.top().type == CT_IMPORT) || (frm.top().type == CT_USING))) {
					LOG_FMT(LINDLINE, "%s(%d): pc orig line is %zu, orig col is %zu, Text() is '%s', type is %s\n",
					    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->Text(),
					    get_token_name(pc->GetType()));
					frm.pop(__func__, __LINE__, pc);
				...
				// TODO: ideally formatting would know which opens occurred on a line and group closes
				// in the same manor
						...
						while(count-- > 0) {
							if(frm.top().type == CT_SQUARE_OPEN) {
								if(frm.paren_count == 0) {
									fprintf(stderr,
									    "%s(%d): frm.paren_count is ZERO, cannot be decremented, at line %zu, column %zu\n",
									    __func__,
									    __LINE__,
									    pc->GetOrigLine(),
									    pc->GetOrigCol());
							...
							LOG_FMT(LINDLINE,
							    "%s(%d): pc orig line is %zu, orig col is %zu, Text() is '%s', type is %s\n",
							    __func__,
							    __LINE__,
							    pc->GetOrigLine(),
							    pc->GetOrigCol(),
		...
		if(!pc->IsCommentOrNewline()
			...
			for(size_t ttidx = frm.size() - 1; ttidx > 0; ttidx--) {
				LOG_FMT(LINDPC,
				    "     [%zu %zu:%zu '%s' %s/%s tmp=%zu indent=%zu brace_indent=%zu indent_tab=%zu indent_cont=%d level=%zu pc brace level=%zu]\n",
				    ttidx,
		...
		if(pc->Is(CT_BRACE_CLOSE)) {
			if(language_is_set(LANG_OC)) {
				if(frm.top().type == CT_BRACE_OPEN
					...
					// TODO: ideally formatting would know which opens occurred on a line and group
					// closes in the same manor
					if(count > 0
					    && pc->Is(CT_BRACE_CLOSE)
					    && pc->GetParentType() == CT_OC_AT) {
						if(frm.top().ip.ref) {
			...
			if(!options::indent_macro_brace()
				...
				// 2a. If it's an assignment, check that both sides of the assignment operator are on
				// the same line
				...
				if(sameLine && ((isAssignSameLine) || (closureSameLineTopLevel))) {
					if(indent_size > frm.top().brace_indent) { // if
					                                           // options::indent_indent_columns()
					                                           // is too big
				...
				// Issue #1165
				LOG_FMT(LINDENT2,
				    "%s(%d): orig line is %zu, brace level is %zu, for '%s', pc->GetLevel() is %zu, pc(-1)->GetLevel() is %zu\n",
				    __func__,
				    __LINE__,
				    pc->GetOrigLine(),
				    pc->GetBraceLevel(),
			...
			// only to help the vim command }
				...
				if(frm.top().pc->GetParentType() == CT_OC_BLOCK_EXPR) {
					...
					if(options::indent_oc_block()
						...
						if(options::indent_oc_block_msg_xcode_style()) {
							Chunk * bbc           = pc->GetClosingParen();// block brace
							                                     // close '}'
				...
				// Issue # 1620, UNI-24090.cs
					...
					log_rule_B("indent_align_paren");
					// We are inside ({ ... }) -- where { and ( are on the same line, avoiding
					// double indentations.
					// only to help the vim command }
				    ...
				    && !pc->TestFlags(PCF_ONE_LINER)) {
					log_rule_B("indent_align_paren");
					// We are inside ({ ... }) -- where { and ( are on adjacent lines, avoiding
					// indentation of brace.
					// only to help the vim command }
					frm.top().brace_indent = frm.prev().indent - indent_size;
					indent_column_set(frm.top().brace_indent);
					frm.top().indent = frm.prev().indent_tmp;
			...
			if(options::indent_ignore_label()) {
				...
				if(val > 0) {
					...
					if(next->IsNotNullChunk()
					    && !next->IsNewline()
					    // label (+ 2, because there is colon and space after it) must fit into
					    // indent
			...
			if(pc->Is(CT_CLASS_COLON)) {
				if(options::indent_ignore_before_class_colon()) {
					...
					frm.top().indent_tmp = std::max<ptrdiff_t>(
						frm.top().indent_tmp + options::indent_before_class_colon(),
						0);
					log_indent_tmp();
				}
			}
			...
			if(options::indent_class_colon()
				...
				if(options::indent_constr_colon()) {
					...
					if(options::indent_ctor_init() != 0) {
						...
						/*
						 * If the std::max() calls were specialized with size_t (the type of the
						 *underlying variable),
						 * they would never actually do their job, because size_t is unsigned
						 *and therefore even
						 * a "negative" result would be always greater than zero.
						 * Using ptrdiff_t (a standard signed type of the same size as size_t)
						 *in order to avoid that.
						 */
						frm.top().indent = std::max<ptrdiff_t>(frm.top().indent + options::indent_ctor_init(), 0);
						log_indent();
						frm.top().indent_tmp = std::max<ptrdiff_t>(
							frm.top().indent_tmp + options::indent_ctor_init(),
							0);
						frm.top().indent_tab = std::max<ptrdiff_t>(
							frm.top().indent_tab + options::indent_ctor_init(),
							0);
						log_indent_tmp();
						indent_column_set(frm.top().indent_tmp);
					}
			...
			if(options::indent_inside_ternary_operator()
				...
				while(((idx > 0 && frm.at(idx).type != CT_BRACE_OPEN && frm.at(idx).type != CT_VBRACE_OPEN && frm.at(idx).type != CT_PAREN_OPEN && 
					frm.at(idx).type != CT_FPAREN_OPEN && frm.at(idx).type != CT_SPAREN_OPEN && frm.at(idx).type != CT_SQUARE_OPEN && 
					frm.at(idx).type != CT_ANGLE_OPEN && frm.at(idx).type != CT_CASE && frm.at(idx).type != CT_MEMBER && frm.at(idx).type != CT_QUESTION && 
					frm.at(idx).type != CT_COND_COLON && frm.at(idx).type != CT_LAMBDA && frm.at(idx).type != CT_ASSIGN_NL) || 
					frm.at(idx).pc->IsOnSameLine(frm.top().pc)) && (frm.at(idx).type != CT_CLASS_COLON && frm.at(idx).type != CT_CONSTR_COLON && 
					!(frm.at(idx).type == CT_LAMBDA && frm.at(idx).pc->GetPrevNc()->GetType() == CT_NEWLINE))) {
					if(idx == 0) {
			...
			if(pc->GetParentType() != CT_OC_AT
				...
				if(pc->GetLevel() == pc->GetBraceLevel()
					...
					if((options::use_indent_continue_only_once())
						...
						if(pc->Is(CT_SPAREN_OPEN) && options::indent_sparen_extra() != 0) {
		...
		else if(options::indent_member_single() && pc->Is(CT_MEMBER) && sstreq(pc->Text(), ".") && language_is_set(LANG_CS | LANG_CPP)) {
			log_rule_B("indent_member_single");
			if(frm.top().type != CT_MEMBER) {
				frm.push(pc, __func__, __LINE__);
			...
			if(tmp->IsNotNullChunk()) {
				...
				else if(tmp->Is(CT_WORD) || tmp->Is(CT_TYPE)) {
					tmp = tmp->GetNextNcNnlNpp();
				}
			}
			if(tmp->IsNotNullChunk() && (!sstreq(tmp->Text(), ".") || tmp->IsNot(CT_MEMBER))) {
				if(tmp->IsParenClose()) {
				...
				if(tmp->IsNotNullChunk() && tmp->GetPrev()->IsNewline()) {
				...
				if(options::indent_ignore_first_continue()) {
					...
					if(pc->GetLevel() == pc->GetBraceLevel()
						...
						if((options::use_indent_continue_only_once())
							...
							vardefcol = frm.top().indent; // use the same variable for the
							                              // next line
							frm.top().indent_cont = true;
						}
					}
			...
			if(pc->GetParentType() == CT_FUNC_DEF || pc->GetParentType() == CT_FUNC_PROTO || (pc->GetParentType() == CT_STRUCT && frm.top().type != CT_CLASS_COLON)) {
				indent_column_set(frm.top().indent + 4);
			}
		}
		else if(options::indent_inside_ternary_operator() && (pc->Is(CT_QUESTION) || pc->Is(CT_COND_COLON))) { // Issue #1130, #1715
			log_rule_B("indent_inside_ternary_operator");
			// Pop any colons before because they should already be processed
			while(pc->Is(CT_COND_COLON) && frm.top().type == CT_COND_COLON) {
				frm.pop(__func__, __LINE__, pc);
			}
			log_rule_B("indent_inside_ternary_operator");
			...
			if(next->Is(CT_COND_COLON)) {
				LOG_FMT(LINDLINE,
				    "%s(%d): Comment and COND_COLON: pc orig line is %zu, orig col is %zu, Text() is '%s', type is %s\n",
				    __func__,
				    __LINE__,
				    pc->GetOrigLine(),
				    pc->GetOrigCol(),
...
// #define ANYTHING_ELSE
		...
		if(options::indent_shift() == 1
			...
			if(prev2->Is(CT_NEWLINE)
				...
				// Setting frm.top().indent_cont = true in the top context when the indent is not also
				// set
		...
		if(did_newline
			...
			if(pc->TestFlags(PCF_DONT_INDENT)) {
				...
				if(frm.lastPopped().type == E_Token(pc->GetType() - 1)) {
					...
					LOG_FMT(LINDLINE,
					    "%s(%d): ck1 orig line is %zu, orig col is %zu, Text() is '%s', GetType() is %s\n",
					    __func__,
					    __LINE__,
					    ck1->GetOrigLine(),
					    ck1->GetOrigCol(),
					    ck1->Text(),
					    get_token_name(ck1->GetType()));
					Chunk * ck2 = ck1->GetPrev();
					LOG_FMT(LINDLINE,
					    "%s(%d): ck2 orig line is %zu, orig col is %zu, Text() is '%s', GetType() is %s\n",
					    __func__,
					    __LINE__,
					    ck2->GetOrigLine(),
					    ck2->GetOrigCol(),
					    ck2->Text(),
					    get_token_name(ck2->GetType()));
					...
					if(options::indent_paren_close() == -1) {
						...
						if(options::indent_paren_close() != 2) {
							...
							if(options::indent_paren_close() == 1) {
								...
								if(indent_column == 0) {
									fprintf(stderr,
									    "%s(%d): indent_column is ZERO, cannot be decremented, at line %zu, column %zu\n",
									    __func__,
									    __LINE__,
									    pc->GetOrigLine(),
									    pc->GetOrigCol());
							...
							LOG_FMT(LINDLINE, "%s(%d): indent_paren_close is 2\n",
							    __func__, __LINE__);
							LOG_FMT(LINDLINE,
							    "%s(%d): ck2 orig line is %zu, orig col is %zu, ck2->Text() is '%s'\n",
							    __func__,
							    __LINE__,
							    ck2->GetOrigLine(),
							    ck2->GetOrigCol(),
							...
							if(pc->GetPrev()->GetType() == CT_NEWLINE) {
								...
								// Issue #3407 - Skip over a possible 'noexcept' keyword
								...
								if(searchNext->GetType() == CT_SEMICOLON
									...
									if(options::indent_oc_inside_msg_sel()
									    ...
									    || frm.top().type == CT_OC_MSG_NAME)) { // Issue
									                                            // #2658
										log_rule_B("indent_oc_inside_msg_sel");
										// [Class Message:(...)<here>
									    ...
									    && (  frm.top().type == CT_QUESTION
									    || frm.top().type == CT_COND_COLON)) { // Issue
									                                           // #1130,
									                                           // #1715
										log_rule_B("indent_inside_ternary_operator");
										indent_column_set(frm.top().indent);
				...
				LOG_FMT(LINDENT,
				    "%s(%d): pc->line is %zu, pc->GetColumn() is %zu, pc->Text() is '%s, indent_column is %zu\n",
				    __func__,
				    __LINE__,
				    pc->GetOrigLine(),
				    pc->GetColumn(),
					...
					if((pc->Is(CT_QUESTION)// Issue #2101
						...
						if(options::indent_switch_break_with_case()
							...
							if(whereIsCase->IsNotNullChunk()) {
								LOG_FMT(LINDENT,
								    "%s(%d): orig line is %zu, orig col is %zu, Text() is '%s'\n",
								    __func__,
								    __LINE__,
								    whereIsCase->GetOrigLine(),
								    whereIsCase->GetOrigCol(),
								    whereIsCase->Text());
						...
						LOG_FMT(LINDPC, "%s(%d): Text() is '%s', (frm.at(frm_size - 1).pc)->GetType() is %s\n",
						    __func__, __LINE__, (frm.at(frm_size - 1).pc)->Text(),
						    get_token_name((frm.at(frm_size - 1).pc)->GetType()));
						...
						if(temp_ttidx == 0) {
							...
							LOG_FMT(LINDPC,
							    "%s(%d): Text() is '%s', token_before->GetType() is %s\n",
							    __func__,
							    __LINE__,
							    token_before->Text(),
							    get_token_name(token_before->GetType()));
							...
							if(token_before->Is(CT_ASSIGN)) {
								...
								if(before_Assign->IsNullChunk()) {
									...
									LOG_FMT(LINDPC,
									    "%s(%d): Text() is '%s', before_Assign->GetType() is %s, column is %zu\n",
									    __func__,
									    __LINE__,
									    before_Assign->Text(),
									    get_token_name(before_Assign->GetType()),
									    vor_col);
								...
								LOG_FMT(LINDPC,
								    "%s(%d): Text() is '%s', token_before->GetType() is %s, column is %zu\n",
								    __func__,
								    __LINE__,
								    token_before->Text(),
								    get_token_name(token_before->GetType()),
			...
			did_newline = false;
			if(pc->Is(CT_SQL_EXEC) || pc->Is(CT_SQL_BEGIN) || pc->Is(CT_SQL_END)) {
			...
			if(pc->TestFlags(PCF_VAR_TYPE)) {
				if(!frm.top().non_vardef && (frm.top().type == CT_BRACE_OPEN)) {
		...
		if(pc->IsNewline() || pc->Is(CT_COMMENT_MULTI) || pc->Is(CT_COMMENT_CPP)) {
			log_indent();
			frm.top().indent_tmp = frm.top().indent;
			log_indent_tmp();
			/*
			 ...
			 */
			if(pc->Is(CT_NL_CONT) && frm.top().indent_tmp <= indent_size && frm.top().type != CT_PP_DEFINE) {
...
bool ifdef_over_whole_file()
{
	...
	if(options::pp_indent_in_guard() || cpd.frag) {
		return false;
	}
	...
	for(Chunk * pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
		...
		if(IFstage == 0) {        // 0 is BEGIN
			...
			Chunk * next = pc->GetNext();
			if(next->IsNullChunk() || next->IsNot(CT_PP_IF)) {
				break;
			}
			...
			if(pc->Is(CT_PREPROC) || !pc->TestFlags(PCF_IN_PREPROC)) {
				IFstage = 0;
				break;
			}
		}
...
void indent_preproc()
{
	...
	for(Chunk * pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
		...
		const size_t pp_level = (pc->GetPpLevel() > pp_level_sub) ? pc->GetPpLevel() - pp_level_sub : 0;
		...
		if((options::pp_space_after() != IARF_IGNORE) && next->IsNotNullChunk()) {
		...
		bool at_file_level = pc->GetBraceLevel() <= ((pc->GetParentType() == CT_PP_DEFINE) ? 1U : 0U);
		if(((  at_file_level
