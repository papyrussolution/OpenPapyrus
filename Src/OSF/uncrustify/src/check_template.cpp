...
void check_template(Chunk * start, bool in_type_cast)
{
	...
	if(prev->Is(CT_TEMPLATE)) {
		...
		for(pc = start->GetNextNcNnl(E_Scope::PREPROC);
			...
			else if(in_if && (pc->Is(CT_BOOL) || pc->Is(CT_COMPARE))) {
				break;
			}
			else if(pc->Is(CT_BRACE_OPEN)) {
				if(!pc->TestFlags(PCF_IN_DECLTYPE) || !detect_cpp_braced_init_list(pc->GetPrev(), pc)) {
					break;
				}
			...
			else if(pc->Is(CT_BRACE_CLOSE) && pc->GetParentType() != CT_BRACED_INIT_LIST && !pc->TestFlags(PCF_IN_DECLTYPE)) {
				break;
			}
