/**
 * @file newlines.h prototypes for newlines.c
 *
 * @author  Ben Gardner
 * @license GPL v2+
 */
...
/**
 * Remove all extra newlines. Modify line breaks as needed.
 */
...
/** Step through all chunks, altering newlines inside parens of if/for/while/do as needed. Handles the style options:
   nl_multi_line_sparen_open, nl_multi_line_sparen_close, nl_before_if_closing_paren
 */
...
/**
 * Handle removal of extra blank lines in functions x <= 0: do nothing, x > 0: allow max x-1 blank lines
 */
void newlines_functions_remove_extra_blank_lines();
void newlines_squeeze_ifdef();
/**
 * In case of consecutive closing parens, which follow a newline, the closing paren are altered to different lines, as
 *per the respective opening parens. In the given example, first 2 opening paren are in same line, hence the respective
 *closing paren are put in the same line.
 * input:
 * func1(func2(
 *    func3(
 *       func4(
...
/**
 * Searches for a chunk of type chunk_type and moves them, if needed. Will not move tokens that are on their own line or
 *have other than exactly 1 newline before (UO_pos_comma == TRAIL) or after (UO_pos_comma == LEAD). We can't remove a
 *newline if it is right before a preprocessor.
 */
...
/**
 * Scans for newline tokens and changes the nl_count. A newline token has a minimum nl_count of 1. Note that a blank
 *line is actually 2 newlines, unless the newline is the first chunk. So, most comparisons have +1 below.
 */
void do_blank_lines();
/**
 * Clears the PCF_ONE_LINER flag on the current line. Done right before inserting a newline.
 */
...
/**
 * Add a newline before the chunk if there isn't already a newline present. Virtual braces are skipped, as they do not
 *contribute to the output.
 */
Chunk *newline_add_before(Chunk * pc);
/**
 * Add a newline after the chunk if there isn't already a newline present. Virtual braces are skipped, as they do not
 *contribute to the output.
 */
...
/**
 * Removes any CT_NEWLINE or CT_NL_CONT between start and end. Start must be before end on the chunk list. If the
 *'PCF_IN_PREPROC' status differs between two tags, we can't remove the newline.
 *
...
/**
 * Add a newline between two tokens. If there is already a newline between then, nothing is done. Otherwise a newline is
 *inserted.
 *
 * If end is CT_BRACE_OPEN and a comment and newline follow, then the brace open is moved instead of inserting a
 *newline.
 *
 * In this situation:
 *    if (...) { //comment
 ...
 * @param  newlines  reference in which the amount of newlines will be written to (will be initialized with 0)
