...
#define UNC_DECLARE_FLAGS(flag_type, enum_type) using flag_type = ::uncrustify::flags<enum_type>

#define UNC_DECLARE_OPERATORS_FOR_FLAGS(flag_type)                        \
	inline flag_type operator&(flag_type::enum_t f1, flag_type::enum_t f2) { return (flag_type{ f1 } &f2); }  \
	inline flag_type operator|(flag_type::enum_t f1, flag_type::enum_t f2) { return (flag_type{ f1 } | f2); } \
	inline flag_type operator|(flag_type::enum_t f1, flag_type f2)         { return (f2 | f1); }              \
...
public:
	...
	template <typename T> using integral = typename std::enable_if<std::is_integral<T>::value, bool>::type;
	inline flags() = default;
	inline flags(Enum flag) : m_i{static_cast<int_t>(flag)}
	{
	}

	inline bool operator==(Enum const &other) { return (m_i == static_cast<int_t>(other)); }
	...
	template <typename T, integral<T> = true> inline flags operator&(T mask) const
	{
		flags g; 
	...
	inline flags operator|(flags f) const
	{
		flags g; g.m_i = m_i | f.m_i; 
		return g;
	}
	inline flags operator|(Enum f) const
	{
		flags g; g.m_i = m_i | static_cast<int_t>(f); 
		return g;
	}
	inline flags operator^(flags f) const
	{
