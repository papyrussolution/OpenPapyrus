...
void newlines_sparens()
{
	...
	for(Chunk * sparen_open = Chunk::GetHead()->GetNextType(CT_SPAREN_OPEN, ANY_LEVEL);
		...
		// Add a newline before ')' if an if/for/while/switch condition spans multiple lines. Overrides
		// nl_before_if_closing_paren if both are specified.
		if(is_multiline
		...
		if(pc->IsNewline()) {
			...
			if(pc->GetNlCount() > 1
				...
				if((nl_opt & IARF_REMOVE)
					...
					// can keep using pc because anything other than newline stops loop, and we
					// delete if newline
...
static void blank_line_set(Chunk * pc, Option<unsigned> &opt)
{
	...
	if((optval > 0) && (pc->GetNlCount() != optval)) {
	...
	if(nl_opt & IARF_ADD) {
		...
		if(next->IsNot(CT_BRACE_CLOSE)) {
			...
			if((next = pc->GetNextNvb())->IsNewline()) {
				LOG_FMT(LNEWLINE,
				    "%s(%d): next->Text() is '%s', type %s, orig line %zu, orig col %zu\n",
				    __func__,
				    __LINE__,
				    next->Text(),
			...
			if(nl_count == 0) {
				...
				if(((next = pc->GetNext())->IsNotNullChunk())
					...
					pc = next;
					LOG_FMT(LNEWLINE,
					    "%s(%d): pc->Text() is '%s', type %s, orig line %zu, orig col %zu\n",
					    __func__,
					    __LINE__,
					    pc->Text(),
				...
				if(have_pre_vbrace_nl) {
					...
					LOG_FMT(LNEWLINE,
					    "%s(%d): prev->Text() is '%s', type %s, orig line %zu, orig col %zu\n",
					    __func__,
					    __LINE__,
					    prev->Text(),
					...
					if(pc->Is(CT_PREPROC)
						...
						LOG_FMT(LNEWLINE,
						    "%s(%d): cannot add newline after orig line %zu due to nl_squeeze_ifdef\n",
						    __func__,
						    __LINE__,
						    prev->GetOrigLine());
					}
			...
			if(is_it_possible) {
				...
				if(options::code_width() > 0) {
					...
					while(current->IsNotNullChunk()) {
						LOG_FMT(LNL1LINE,
						    "%s(%d): zu  kopieren: current orig line is %zu, orig col is %zu, Text() is '%s'\n",
						    __func__,
						    __LINE__,
						    current->GetOrigLine(),
						    current->GetOrigCol(),
				...
				if(options::code_width() > 0
					...
					for(count = 0; count < saved_chunk.size(); count++) {
						...
						if(tmp_2.GetOrigLine() != current->GetOrigLine()) {
							...
							LOG_FMT(LNEWLINE,
							    "%s(%d): %zu:%zu add newline before '%s'\n",
							    __func__,
							    __LINE__,
							    current->GetOrigLine(),
							    current->GetOrigCol(),
...
static void newline_func_multi_line(Chunk * start)
{
	...
	if(pc->Is(CT_FPAREN_CLOSE)
		...
		if(add_args) {
			// process the function in reverse and leave the first comma if the option to leave trailing
			// closure
		...
		if(pc->Is(CT_IF)
				...
				case CT_OC_CLASS:
			    {
				    if(pc->GetLevel() == pc->GetBraceLevel()) {
					    ...
					    for(Chunk * tmp = pc->GetPrev(); tmp->IsNotNullChunk(); tmp = tmp->GetPrev()) {
							...
							|| tmp->Is(CT_OC_IMPL)) {
							    LOG_FMT(LBLANK,
								"%s(%d): orig line is %zu, orig col is %zu, may be remove/force newline before {\n",
								__func__,
								__LINE__,
								pc->GetOrigLine(),
								pc->GetOrigCol());
							    ...
							    if(tmp->Is(CT_OC_INTF)) {
								    ...
								    newlines_do_else(pc->GetPrevNnl(),
									options::nl_oc_implementation_brace());
							    }
							    break;
						    }
					    }
			...
			if((options::nl_brace_struct_var() != IARF_IGNORE)
				...
				if(next->IsNot(CT_SEMICOLON)
				    ...
				    && next->IsNotNullChunk()) {
					// #1258
					// dont add newline between two consecutive braces closes, if the second is a
					// part of one liner.
			...
			if(options::nl_after_vbrace_open()
				...
				if(next->IsSemicolon()) {
					...
					log_rule_B("nl_after_vbrace_open");
					add_it = (  options::nl_after_vbrace_open() && next->IsNot(CT_VBRACE_CLOSE) && !next->IsCommentOrNewline());
				}
				if(add_it) {
					newline_iarf(pc, IARF_ADD);
				}
			...
			if(((pc->GetParentType() == CT_IF || pc->GetParentType() == CT_ELSEIF || pc->GetParentType() == CT_ELSE) && options::nl_create_if_one_liner())
			    || (pc->GetParentType() == CT_FOR && options::nl_create_for_one_liner()) || (pc->GetParentType() == CT_WHILE && options::nl_create_while_one_liner())) {
			...
			if(((pc->GetParentType() == CT_IF || pc->GetParentType() == CT_ELSEIF || pc->GetParentType() == CT_ELSE) && options::nl_split_if_one_liner())
			    || (pc->GetParentType() == CT_FOR && options::nl_split_for_one_liner()) || (pc->GetParentType() == CT_WHILE && options::nl_split_while_one_liner())) {
				if(pc->TestFlags(PCF_ONE_LINER)) {
					// split one-liner
			...
			if(pc->GetParentType() == CT_TEMPLATE) {
				...
				if(next->IsNotNullChunk()
					...
					Chunk * tmp = pc->GetPrevType(CT_ANGLE_OPEN, pc->GetLevel())->GetPrevNcNnlNi(); // Issue
					                                                                                // #2279
					...
					if(tmp->Is(CT_TEMPLATE)) {
						...
						else if(next->GetParentType() == CT_FUNC_PROTO) { // function
						                                                  // declaration
							iarf_e const action =
							    newline_template_option(
								pc,
			...
			if(next->IsNotNullChunk()) {
				...
				if(!next->Is(CT_ASSIGN)) {
					...
					if(braceOpen->IsNullChunk()) {
						// fatal error
						LOG_FMT(LERR,
						    "%s(%d): Missing BRACE_OPEN after namespace\n   orig line is %zu, orig col is %zu\n",
						    __func__,
						    __LINE__,
						    pc->GetOrigLine(),
						    pc->GetOrigCol());
		...
		if(pc->Is(tok)) {
			...
			if(pc->Is(CT_CONSTR_COLON)) {
				...
				if(with_acv
					...
					LOG_FMT(LBLANKD,
					    "%s(%d): paren_vor_value orig line is %zu, orig col is %zu, Text() '%s', type is %s\n",
					    __func__,
					    __LINE__,
					    paren_vor_value->GetOrigLine(),
					    paren_vor_value->GetOrigCol(),
					    paren_vor_value->Text(),
					    get_token_name(paren_vor_value->GetType()));
				...
				if(ncia & IARF_ADD) { // nl_class_init_args, nl_constr_init_args:
					if(pcc & TP_TRAIL) { // pos_class_comma, pos_constr_comma
						if(ncia == IARF_FORCE) { // nl_class_init_args, nl_constr_init_args: 5
							...
							// (ncia == IARF_ADD)         // nl_class_init_args,
							// nl_constr_init_args: 8
							newline_add_after(pc);
						}
						prev = pc->GetPrevNc();
...
void do_blank_lines()
{
	...
	for(Chunk * pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
		...
		// Change blanks after a try-catch-finally block
		if((options::nl_between_get_set() != 0) && (options::nl_between_get_set() != pc->GetNlCount()) && prev->IsNotNullChunk() && next->IsNotNullChunk()) {
			log_rule_B("nl_between_get_set");
			if(prev->GetParentType() == CT_GETSET && next->IsNot(CT_BRACE_CLOSE) && (  prev->Is(CT_BRACE_CLOSE) || prev->Is(CT_SEMICOLON))) {
				blank_line_set(pc, options::nl_between_get_set);
				log_rule_B("nl_between_get_set");
			}
		}
		// Change blanks after a try-catch-finally block
		if((options::nl_around_cs_property() != 0) && (options::nl_around_cs_property() != pc->GetNlCount()) && prev->IsNotNullChunk() && next->IsNotNullChunk()) {
			log_rule_B("nl_around_cs_property");
			if(prev->Is(CT_BRACE_CLOSE) && prev->GetParentType() == CT_CS_PROPERTY && next->IsNot(CT_BRACE_CLOSE)) {
				blank_line_set(pc, options::nl_around_cs_property);
				log_rule_B("nl_around_cs_property");
			}
			else if(next->GetParentType() == CT_CS_PROPERTY && next->TestFlags(PCF_STMT_START)) {
				blank_line_set(pc, options::nl_around_cs_property);
				log_rule_B("nl_around_cs_property");
			}
		}
		// Control blanks before an access spec
		if((options::nl_before_access_spec() > 0) && (options::nl_before_access_spec() != pc->GetNlCount()) && next->Is(CT_ACCESS)) {
			log_rule_B("nl_before_access_spec");
		...
		if(options::nl_before_whole_file_endif() != 0 && options::nl_before_whole_file_endif() != pc->GetNlCount() && next->Is(CT_PREPROC)
		    && next->GetParentType() == CT_PP_ENDIF && ifdef_over_whole_file() && next->TestFlags(PCF_WF_ENDIF)) {
		...
		if(options::nl_after_whole_file_endif() != 0 && options::nl_after_whole_file_endif() != pc->GetNlCount()) {
			Chunk * pp_start = prev->GetPpStart();
			if(pp_start->IsNotNullChunk() && pp_start->GetParentType() == CT_PP_ENDIF && ifdef_over_whole_file() && pp_start->TestFlags(PCF_WF_ENDIF)) {
				log_rule_B("nl_after_whole_file_endif");
				blank_line_set(pc, options::nl_after_whole_file_endif);
			}
		}
		if(line_added && pc->GetNlCount() > 1) {
			pc->SetNlCount(pc->GetNlCount() - 1);
			LOG_FMT(LBLANK, "%s(%d): orig line is %zu, orig col is %zu, text is '%s', new line count is now %zu\n",
			    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->Text(), pc->GetNlCount());
		}
...
void newlines_cleanup_dup()
{
	...
	while(pc->IsNotNullChunk()) {
		next = next->GetNext();
		if(pc->Is(CT_NEWLINE) && next->Is(CT_NEWLINE)) {
...
void annotations_newlines()
{
	...
	while((pc = pc->GetNextType(CT_ANNOTATION))->IsNotNullChunk()
		...
		LOG_FMT(LANNOT,
		    "%s(%d): orig line is %zu, orig col is %zu, annotation is '%s',  end @ orig line %zu, orig col %zu, is '%s'\n",
		    __func__,
		    __LINE__,
		    pc->GetOrigLine(),
		    pc->GetOrigCol(),
...
{
	if(pc_start->IsNullChunk() || pc_end->IsNullChunk()) {
		return false;
	}
	newlines = 0;
	Chunk * it = pc_start;
