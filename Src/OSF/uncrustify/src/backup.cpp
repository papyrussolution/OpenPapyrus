 ...
 *  3. Calculate the md5 over the output file. Create the .backup-md5 file.
 *
...
int backup_copy_file(const char * filename, const vector<UINT8> &data)
{
	...
	//snprintf(md5_str, sizeof(md5_str), "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
	   //dig[0], dig[1], dig[2], dig[3], dig[4], dig[5], dig[6], dig[7], dig[8], dig[9], dig[10], dig[11], dig[12], dig[13], dig[14], dig[15]);
	{
		SBinaryChunk bc_hash;
		SlHash::CalcBufferHash(SHASHF_MD5, data.data(), data.size(), bc_hash);
		bc_hash.Hex(md5_buf);
		assert(md5_buf.Len() == 32);
	}
	...
	if(md5_buf.IsEqiAscii(md5_str_in)) {
...
#endif // } 0
	SString temp_buf;
	SFile f(filename, SFile::mRead|SFile::mBinary);
	if(!f.IsValid()) {
		...
		SBinaryChunk bc_hash;
		if(f.CalcHash(0LL, SHASHF_MD5, bc_hash)) {
			assert(bc_hash.Len() == 16);
			temp_buf.Z().Cat(filename).Cat(UNC_BACKUP_MD5_SUFFIX);
			SFile f_hash(temp_buf, SFile::mWrite|SFile::mBinary);
			if(f_hash.IsValid()) {
				bc_hash.Hex(temp_buf).Space().Cat(path_basename(filename)).CR();
				f_hash.WriteLine(temp_buf);
				//fprintf(thefile, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x  %s\n",
					//dig[0], dig[1], dig[2], dig[3], dig[4], dig[5], dig[6], dig[7], dig[8], dig[9], dig[10], dig[11], dig[12], dig[13], dig[14], dig[15], path_basename(filename));
			}
		}
	}
#if 0 // {
