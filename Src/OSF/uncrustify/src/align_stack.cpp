...
void AlignStack::Add(Chunk * start, size_t seqnum)
{
	...
	LOG_FMT(LAS, "AlignStack::%s(%d): Candidate is '%s': orig line is %zu, column is %zu, type is %s, level is %zu\n",
	    __func__, __LINE__, start->Text(), start->GetOrigLine(), start->GetColumn(), get_token_name(start->GetType()),
	    start->GetLevel());
	...
	Chunk * prev = start;
	while((prev = prev->GetPrev())->IsNotNullChunk() && (prev->IsPointerOperator() || prev->Is(CT_TPAREN_OPEN))) {
	...
	if(m_star_style != SS_IGNORE) {
		...
		while(tmp_prev->IsStar() || tmp_prev->IsMsRef()) {
			ali      = tmp_prev;
		...
		if(tmp_prev->Is(CT_TPAREN_OPEN)) {
			...
			// ali->GetPrev(nav_e::ALL) to prev here, but that stored value is overwritten before it can be
			// used.
		}
	}
	...
	if(!options::align_keep_extra_space()) {
		...
		while(tmp->IsNotNullChunk() && tmp != start) {
			Chunk * next = tmp->GetNext();
			if(next->IsNotNullChunk()) {
				LOG_FMT(LAS,
				    "AlignStack::%s(%3d): GUY: next orig line is %zu, orig col is %zu, Text() '%s', level is %zu, type is %s\n",
				    __func__, __LINE__, next->GetOrigLine(), next->GetOrigCol(), next->Text(),
				    next->GetLevel(), get_token_name(next->GetType()));
	...
	if(m_max_col == 0 || m_thresh == 0 || (((start->GetColumn() + m_gap) <= (m_thresh + (m_absolute_thresh ? m_min_col : m_max_col))) // don't use
      // subtraction here to prevent underflow
		...
		if(ali->Is(CT_PTR_TYPE)) {
			LOG_FMT(LAS, "AlignStack::%s(%d): Add-[%s][%s]: ali orig line is %zu, column is %zu, type is %s, level is %zu\n",
			    __func__, __LINE__, ali->Text(), start->Text(), ali->GetOrigLine(), ali->GetColumn(),
			    get_token_name(ali->GetType()), ali->GetLevel());
		}
		else {
			LOG_FMT(LAS,
			    "AlignStack::%s(%3d): GUY: Add-[%s]: ali orig line is %zu, column is %zu, type is %s, level is %zu\n",
			    __func__,
			    __LINE__,
			    ali->Text(),
			    ali->GetOrigLine(),
			    ali->GetColumn(),
			    get_token_name(ali->GetType()),
			    ali->GetLevel());
		}
		...
		if(endcol > m_max_col) {
			...
			LOG_FMT(LAS,
			    "AlignStack::%s(%3d): GUY:    ali orig line is %zu, column is %zu, max_col old is %zu, new is %zu, m_min_col is %zu\n",
			    __func__,
			    __LINE__,
			    ali->GetOrigLine(),
			    ali->GetColumn(),
...
void AlignStack::Flush()
{
	...
	for(size_t idx = 0; idx < Len(); idx++) {
		...
		LOG_FMT(LAS,
		    "AlignStack::%s(%d):   idx is %zu, pc->Text() is '%s', orig line is %zu, orig col is %zu, alignment col_adj is %d\n",
		    __func__,
		    __LINE__,
		    idx,
		    pc->Text(),
	...
	for(size_t idx = 0; idx < Len(); idx++) {
		...
		if(m_star_style == SS_DANGLE) {
			...
			if(tmp->IsPointerOperator()) {
				...
				gap     = pc->GetAlignData().start->GetColumn() -
				    (pc->GetAlignData().ref->GetColumn() + pc->GetAlignData().ref->Len());
			}
		}
	...
	log_rule_B("align_on_tabstop");
	if(options::align_on_tabstop() && Len() > 1) {
	...
	for(size_t idx = 0; idx < Len(); idx++) {
		...
		if(idx == 0) {
			if(m_skip_first
				...
				LOG_FMT(LAS,
				    "AlignStack::%s(%d): orig line is %zu, orig col is %zu, dropping first item due to skip_first\n",
				    __func__,
				    __LINE__,
				    pc->GetOrigLine(),
				    pc->GetOrigCol());
	...
	if(length > 0) {
		...
		for(size_t idx = 0; idx < length; idx++) {
			...
			if(pc->Is(CT_PTR_TYPE)) {
				LOG_FMT(LAS,
				    "AlignStack::%s(%d): idx is %zu, [%s][%s]: orig line is %zu, orig col is %zu, type is %s, level is %zu, brace level is %zu\n",
				    __func__,
				    __LINE__,
				    idx,
				    pc->Text(),
				...
				LOG_FMT(LAS,
				    "AlignStack::%s(%d): idx is %zu, [%s]: orig line is %zu, orig col is %zu, type is %s, level is %zu, brace level is %zu\n",
				    __func__,
				    __LINE__,
				    idx,
				    pc->Text(),
...
const char * AlignStack::get_StarStyle_name(StarStyle star_style)
{
	// WITH_STACKID_DEBUG;
	switch(star_style) {
		case StarStyle::SS_IGNORE: return ("SS_IGNORE");
		case StarStyle::SS_INCLUDE: return ("SS_INCLUDE");
		case StarStyle::SS_DANGLE: return ("SS_DANGLE");
	}
	return ("?????");
