...
static bool while_needs_rewrite(Chunk * keyword, E_Token desired_type, const char * desired_condition)
{
	...
	if(!sstreq(condition->Text(), "true") && !sstreq(condition->Text(), "1")) {
		return false;
	}
	...
	if(sstreq(condition->Text(), "true") && !sstreq(desired_condition, "true")) {
		return true;
	}
	if(sstreq(condition->Text(), "1") && !sstreq(desired_condition, "1")) {
		return true;
	}
	return false;
} // while_needs_rewrite
...
static void rewrite_loop_condition(Chunk * &source, Chunk * &destination, E_Token desired_type, const char * desired_condition)
{
