	...
	while(pc->IsNotNullChunk()
		...
		if(next->IsNullChunk()
			...
			if(idx >= cpd.al_cnt) {
				...
				LOG_FMT(LSIB,
				    "%s(%d):   New idx is %2.1zu, pc->GetColumn() is %2.1zu, Text() '%s', token_width is %zu, type is %s\n",
				    __func__,
				    __LINE__,
				    idx,
				    pc->GetColumn(),
				...
				if(cpd.al[idx].type == pc->GetType()) {
					LOG_FMT(LSIB,
					    "%s(%d):   Match? idx is %2.1zu, orig line is %2.1zu, column is %2.1zu, token_width is %zu, type is %s\n",
					    __func__,
					    __LINE__,
					    idx,
					    pc->GetOrigLine(),
					...
					if(prev_match->IsNullChunk()) {
						...
						LOG_FMT(LSIB,
						    "%s(%d): prev_match '%s', orig line is %zu, orig col is %zu\n",
						    __func__,
						    __LINE__,
						    prev_match->Text(),
						    prev_match->GetOrigLine(),
						    prev_match->GetOrigCol());
...
Chunk * step_back_over_member(Chunk * pc)
{
	...
	while(tmp->IsNotNullChunk() && tmp->Is(CT_DC_MEMBER)) {
		pc  = tmp->GetPrevNcNnl();
		tmp = pc->GetPrevNcNnl();
	}
	return pc;
