		...
		if(pc->GetOrigLine() == actual_line) {
			...
			if(partNumber == 0
				...
				if(pc->Is(CT_COND_COLON)) {
					...
					LOG_FMT(LGUY,
					    "<> pa-type is %s, orig_line is %zu\n",
					    get_token_name(pa->GetType()),
					    pa->GetOrigLine());
				}
		...
		case 3:
		    ...
		    for(pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
			    if(pc->Is(CT_NEWLINE)) {
				    ...
				    LOG_FMT(LGUY, "(%zu)%s %s, col=%zu, column=%zu\n", pc->GetOrigLine(), pc->Text(),
					get_token_name(pc->GetType()), pc->GetOrigCol(), pc->GetColumn());
			    }
		    }
