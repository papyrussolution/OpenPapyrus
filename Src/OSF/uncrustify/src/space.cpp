...
static iarf_e do_space(Chunk * first, Chunk * second, int &min_sp)
{
	...
	if(first->Is(CT_SPAREN_CLOSE)) {
		if(second->Is(CT_VBRACE_OPEN)) {
			// Fix spacing between SPAREN_CLOSE and VBRACE_OPEN tokens as we don't want the default behavior
			// (which is ADD).
			log_rule("REMOVE");
			return (IARF_REMOVE);
		}
	...
	if(options::sp_before_tr_cmt() != IARF_IGNORE && second->GetParentType() == CT_COMMENT_END) {
	...
	if(first->Is(CT_PAREN_CLOSE) && second->Is(CT_DC_MEMBER) && second->GetNext()->GetType() == CT_FUNC_CALL) {
	...
	// c++17 structured bindings e.g., "auto [x, y, z]" vs. a[x, y, z]" or "auto const [x, y, z]" vs. "auto const[x,
	// y, z]"
...
void space_text()
{
	...
	while(pc->IsNotNullChunk()) {
		...
		if((options::use_options_overriding_for_qt_macros()) && ((strcmp(pc->Text(), "SIGNAL") == 0) || (strcmp(pc->Text(), "SLOT") == 0))) {
		...
		if(options::sp_skip_vbrace_tokens()) {
			next = pc->GetNext();
			while(next->IsEmptyText() && !next->IsNewline() && next->IsVBrace()) {
		...
		if(QT_SIGNAL_SLOT_found && options::sp_balance_nested_parens()) {
			...
			if(nn->IsNotNullChunk() && nn->Is(CT_SPACE)) {
		 ...
		 * of the next item
		 */
		if(pc->IsNewline() || pc->Is(CT_COMMENT_MULTI)) {
			...
			if((pc->Len() > 0)
				...
				if(tmp->IsNotNullChunk()
					...
					if(kw1
						...
						LOG_FMT(LSPACE,
						    "%s(%d): back-to-back words need a space: pc->Text() '%s', next->Text() '%s'\n",
						    __func__,
						    __LINE__,
						    pc->Text(),
						    next->Text());
					...
					// TODO:  what is the meaning of 4
						...
						if(ct != nullptr && (strlen(ct->tag) != pc->Len())) {
							...
							// 'foo<bar<int> >'. Note
							// that sp_angle_shift cannot remove the space without this
							// option.
							if(((  language_is_set(LANG_CPP) && options::sp_permit_cpp11_shift()) || (language_is_set(LANG_JAVA | LANG_CS | LANG_VALA | LANG_OC))) && pc->Is(CT_ANGLE_CLOSE) && next->Is(CT_ANGLE_CLOSE)) {
								// allow '>' and '>' to become '>>'
							}
			...
			switch(av) {
				...
				case IARF_ADD:
			    {
				    ...
				    if(pc->Is(CT_VBRACE_OPEN) && next->GetOrigCol() >= pc->GetPrev()->GetOrigCol()) {
				    ...
				    if(next->GetOrigCol() >= pc->GetOrigColEnd() && pc->GetOrigColEnd() != 0) {
			...
			if(next->IsComment()
				...
				if((  options::sp_before_tr_cmt() == IARF_IGNORE
					...
					if(options::indent_relative_single_line_comments()) {
						...
						LOG_FMT(LSPACE,
						    "%s(%d): pc is '%s', orig col is %zu, next orig col is %zu, pc orig col end is %zu\n",
						    __func__,
						    __LINE__,
						    pc->Text(),
						    pc->GetOrigCol(),
...
void space_text_balance_nested_parens()
{
	...
	while(first->IsNotNullChunk()) {
		...
		if(first->IsString("(") && next->IsString("(")) {
		...
		else if(first->IsString(")") && next->IsString(")")) {
...
size_t space_col_align(Chunk * first, Chunk * second)
{
	...
	LOG_FMT(LSPACE, "%s(%d):    => av is %s\n", __func__, __LINE__, (av == IARF_IGNORE) ? "IGNORE" : (av == IARF_ADD) ? "ADD" : (av == IARF_REMOVE) ? "REMOVE" : "FORCE");
	switch(av) {
		case IARF_ADD:
		case IARF_FORCE:
		    coldiff++;
		...
		case IARF_IGNORE:   // Issue #2064
		    ...
		    if(first->GetOrigLine() == second->GetOrigLine() && second->GetOrigCol() > (first->GetOrigCol() + first->Len())) {
			    coldiff++;
		    }
		    break;
		default:
...
void space_add_after(Chunk * pc, size_t count)
{
	...
	if(next->IsNullChunk() || next->IsNewline()) {
		return;
	}
	SETMIN(count, 16); // Limit to 16 spaces
