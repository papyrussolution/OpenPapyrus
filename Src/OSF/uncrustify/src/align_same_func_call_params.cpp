	...
	for(pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
		...
		if(pc->IsNot(CT_FUNC_CALL)) {
			if(pc->IsNewline()) {
				...
				if(align_root->IsNotNullChunk() && align_root->GetBraceLevel() > pc->GetBraceLevel()) {
		...
		while(prev->Is(CT_MEMBER) || prev->Is(CT_DC_MEMBER)) {
		...
		LOG_FMT(LASFCP, "%s(%3d): Func Call found at orig line is %zu, orig col is %zu, c_str() '%s'\n",
		    __func__, __LINE__, align_fcn->GetOrigLine(), align_fcn->GetOrigCol(), align_fcn_name.c_str());
		add_str = nullptr;
		if(align_root->IsNotNullChunk()) {
			...
			if(align_root->GetBraceLevel() == pc->GetBraceLevel() && align_root->GetLevel() == pc->GetLevel() && align_fcn_name.equals(align_root_name)) {
				//WITH_STACKID_DEBUG;
				fcn_as.Add(pc);
