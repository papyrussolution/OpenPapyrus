...
static int getLogTextUtf8Len(UncText::value_type &c0, size_t start, size_t end)
{
	...
	for(size_t i = start; i < end; ++i) {
		...
		if(ch < 0x80) {   // 1-byte sequence
			...
			throw out_of_range(string(__func__) + ":" + to_string(__LINE__) + " - ch value too big, can't convert to utf8");
		}
	}
	return (c1_idx);
} // getLogTextUTF8Len
...
int UncText::find(const char * search_txt, size_t start_idx) const
{
	...
	if(s_len < t_len/*search_txt longer than the string we are looking in*/ || start_idx + t_len - 1 >= s_len) { // starting position to high to find search_txt
		return -1;
	}
...
int UncText::rfind(const char * search_txt, size_t start_idx) const
{
	const size_t t_len = strlen(search_txt); // the length of 'text' we are looking for
	const size_t s_len = size();        // the length of the string we are looking in
	if(s_len < t_len/*search_txt longer than the string we are looking in*/ || start_idx < t_len - 1) { // starting position to low to find search_txt
		return -1;
	}
	const size_t end_idx = s_len - t_len;
...
void UncText::erase(size_t start_idx, size_t len)
{
	...
	if(end_idx >= m_chars.size()) {
		throw out_of_range(string(__func__) + ":" + to_string(__LINE__) + " - idx + len >= m_chars.size()");
	}
	const auto pos_s = getLogTextUtf8Len(m_chars, start_idx);
	const auto pos_e = pos_s + getLogTextUtf8Len(m_chars, start_idx, end_idx);
	m_logtext.pop_back();
	m_logtext.erase(std::next(std::begin(m_logtext), pos_s), std::next(std::begin(m_logtext), pos_e + 1));
	m_logtext.push_back('\0');
	m_chars.erase(std::next(std::begin(m_chars), start_idx), std::next(std::begin(m_chars), end_idx + 1));
}
...
int UncText::replace(const char * search_text, const UncText &replace_text)
{
	...
	while(fidx >= 0) {
		...
		(static_cast<size_t>(fidx) >= m_chars.size()) ? append(replace_text) : insert(static_cast<size_t>(fidx), replace_text);
