...
struct log_fcn_info {
	log_fcn_info(const char * name_, int line_) : name(name_), line(line_)
	{
	}
	const char * name;
	int line;
...
struct log_buf {
	log_buf() : log_file(nullptr), sev(LSYS), in_log(0), buf_len(0), show_hdr(false)
	{
		bufX.clear();
		bufX.resize(256);
	}
...
static void log_end()
{
	...
	if(!g_log.in_log || (g_log.buf_len > (g_log.bufX.size() / 2))) {
		log_flush(false);
	}
}
