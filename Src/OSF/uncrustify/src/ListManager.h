/**
 * @file ListManager.h Template class that manages items in a double-linked list. If C++ could do it, this would just be
 *a class that worked on an interface.
 *
 * @author  Ben Gardner
 * @license GPL v2+
 */
...
public:
	 ...
	 */
	Chunk *GetHead() const { return (m_head); }
	/**
	 * @return pointer to last element of the linked list
	 */
	Chunk *GetTail() const { return (m_tail); }
	/**
	...
	{
		if(obj1 != Chunk::NullChunkPtr && obj2 != Chunk::NullChunkPtr) {
			if(obj1->m_prev == obj2) {
				Remove(obj1);
				AddBefore(obj1, obj2);
			}
	...
	{
		if(obj != Chunk::NullChunkPtr && ref != Chunk::NullChunkPtr) {
