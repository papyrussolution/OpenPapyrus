/**
 * @file args.h Parses command line arguments.
 *
...
public:
	/**
	 * Initializes the argument library. Store the values and allocates enough memory for the 'used' flags. This
	 *keeps a reference to argv, so don't change it.
	 *
	...
	~Args();
	/**
	 * Checks to see if an arg w/o a value is present. Scans the args looking for an exact match.
	 *
	...
	/**
	 * Check for an arg with a value. Returns only the first match.
	 *
	 ...
	 *
	 * @param          token  the token string to match
	 * @param[in, out] index  search start position, in case that something is found, it will get the succeeding
	...
	/**
	 * This function retrieves all unused parameters. You must set the index before the first call. Set the index to
	 *1 to skip argv[0].
	 *
	 ...
	 * num_args is the maximum number of args split off. If there are more than num_args, the remaining text is
	 *ignored. Note that text is modified (zeroes are inserted)
	 *
