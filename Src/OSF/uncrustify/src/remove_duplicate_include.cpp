...
void remove_duplicate_include()
{
	...
	while(pc->IsNotNullChunk()) {
		//LOG_FMT(LRMRETURN, "%s(%d): orig line is %zu, orig col is %zu, Text() is '%s', type is %s, parent type
		// is %s\n",
		...
		if(pc->Is(CT_PREPROC)) {
			...
			//LOG_FMT(LRMRETURN, "%s(%d): orig line is %zu, orig col is %zu, Text() is '%s', type is %s,
			// parent type is %s\n",
