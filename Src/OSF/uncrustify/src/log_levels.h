 ...
 * @file log_levels.h
 *
 * Enum for log levels. Use these for the log severities in LOG_FMT(), etc.
 *
 * @author  Ben Gardner
 ...
 * -L or -log option.
 * use -L A to set all the levels All messages which have a level that is active will be stored to the log file. All
 *other log messages will be discarded. Different parts of the software use different log levels. This allows to log
 *only operations of a specific operation. This eases debugging. To get all log messages use the option -La By default
 *only system messages (level=LSYS) are logged.
 */
enum log_sev_t {
