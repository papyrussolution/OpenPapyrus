...
#pragma hdrstop
#include "generated/token_names.h" // don't move to uncrustify-internal.h
 ...
 */
static void do_source_file(const char * filename_in, const char * filename_out, const char * parsed_file, const char * dump_file,
...
int path_dirname_len(const char * filename)
{
	...
	return filename ? (static_cast<int>(path_basename(filename) - filename)) : 0;
}
...
static void tease()
{
	fprintf(stdout, "There are currently %zu options and minimal documentation.\nTry UniversalIndentGUI and good luck.\n", get_option_count());
}
	...
	struct local {
		static LONG WINAPI crash_filter(_In_ struct _EXCEPTION_POINTERS * exceptionInfo)
		{
			__try
			{
				...
				// treat an exception the same as a parse failure. exceptions can result from parse
				// failures where we
				// do not have specific handling (null-checks for particular parse paths etc.) and
				// callers generally
			...
			// safety - note that this will not flush like we need, but at least will get the right return
			// code
...
int main(int argc, char * argv[])
{
	...
	static_assert(SIZEOFARRAY(token_names) == CT_TOKEN_COUNT_, "");
#endif // DEBUG
	Args arg(argc, argv);
	if(arg.Present("--version") || arg.Present("-v")) {
		version_exit();
	}
	if(arg.Present("--help") || arg.Present("-h") || arg.Present("--usage") || arg.Present("-?")) {
		usage(argv[0]);
		return (EXIT_SUCCESS);
	}
	...
	const char * p_arg;
	if(((p_arg = arg.Param("-L")) != nullptr) || ((p_arg = arg.Param("--log")) != nullptr)) {
	...
	string cfg_file;
	if(((p_arg = arg.Param("--config")) != nullptr) || ((p_arg = arg.Param("-c")) != nullptr)) {
		cfg_file = p_arg;
	}
	...
	if(((parsed_file = arg.Param("--parsed")) != nullptr) || ((parsed_file = arg.Param("-p")) != nullptr)) {
	...
	if(((dump_file = arg.Param("--dump-steps")) != nullptr) || ((dump_file = arg.Param("-ds")) != nullptr)) {
	...
	if(((source_file = arg.Param("--file")) == nullptr) && ((source_file = arg.Param("-f")) == nullptr)) {
		// not using a single file, source_file is nullptr
	}
	// Get a source file list
	const char * source_list;
	if(((source_list = arg.Param("--files")) == nullptr) && ((source_list = arg.Param("-F")) == nullptr)) {
	...
	if(pfile_csv) {
		if(parsed_file == nullptr || (  parsed_file[0] == '-' && !parsed_file[1])) {
			fprintf(stderr, "FAIL: --debug-csv-format option must be used in combination with '-p FILE', where FILE\n      is not set to '-'\n");
			log_flush(true);
			exit(EX_CONFIG);
		}
	...
	const char * output_file = arg.Param("-o"); // Grab the output override
	const char * html_file = nullptr; // for debugging tracking
	...
	if(cpd.do_check && (output_file || replace || no_backup || keep_mtime || update_config || update_config_wd || detect || prefix || suffix || cpd.if_changed)) {
	...
	while((p_arg = arg.Params("--set", idx)) != nullptr) {
		...
		if(option != nullptr && value != nullptr && strtok(nullptr, "=") == nullptr) { // end of argument reached
	...
	idx = 1;
	if(source_file == nullptr && source_list == nullptr && arg.Unused(idx) == nullptr) {
		...
		LOG_FMT(LSYS, "%s(%d): Parsing: %zu bytes (%zu chars) from stdin as language %s\n", __func__, __LINE__, fm.raw.size(), fm.data.size(), language_name_from_flags(cpd.lang_flags));
		// Issue #3427
		...
		while((p_arg = arg.Unused(idx)) != nullptr) {
			char outbuf[1024];
			do_source_file(p_arg, make_output_filename(outbuf, sizeof(outbuf), p_arg, prefix, suffix), nullptr, nullptr, no_backup, keep_mtime);
		}
...
static void process_source_list(const char * source_list, const char * prefix, const char * suffix, bool no_backup, bool keep_mtime)
{
	...
	while(fgets(linebuf, sizeof(linebuf), p_file) != nullptr) {
		...
		if(fname[0] != '#') {
			char outbuf[1024];
			do_source_file(fname, make_output_filename(outbuf, sizeof(outbuf), fname, prefix, suffix), nullptr, nullptr, no_backup, keep_mtime);
		}
	}
	if(!from_stdin) {
		fclose(p_file);
...
static void make_folders(const string &filename)
{
	...
	for(int idx = 0; outname[idx] != 0; idx++) {
		...
		if(idx > last_idx && (outname[idx] == PATH_SEP)) {
			...
			if((strcmp(&outname[last_idx], ".") != 0) && (strcmp(&outname[last_idx], "..") != 0) && (!(  last_idx == 0 && idx == 2 && outname[1] == ':'))) {
				int status = mkdir(outname, 0750); // Coverity CID 75999
...
static int load_mem_file_config(const std::string &filename, file_mem &fm)
{
	int retval;
	char buf[1024];
	snprintf(buf, sizeof(buf), "%.*s%s", path_dirname_len(cpd.filename.c_str()), cpd.filename.c_str(), filename.c_str());
...
int load_header_files()
{
	...
	if(!options::cmt_insert_file_header().empty()) {
		...
		retval |= load_mem_file_config(options::cmt_insert_file_header(), cpd.file_hdr);
	}
	log_rule_B("cmt_insert_file_footer");
	if(!options::cmt_insert_file_footer().empty()) {
		retval |= load_mem_file_config(options::cmt_insert_file_footer(), cpd.file_ftr);
	}
	log_rule_B("cmt_insert_func_header");
	if(!options::cmt_insert_func_header().empty()) {
		retval |= load_mem_file_config(options::cmt_insert_func_header(), cpd.func_hdr);
	}
	log_rule_B("cmt_insert_class_header");
	if(!options::cmt_insert_class_header().empty()) {
		retval |= load_mem_file_config(options::cmt_insert_class_header(), cpd.class_hdr);
	}
	log_rule_B("cmt_insert_oc_msg_header");
	if(!options::cmt_insert_oc_msg_header().empty()) {
		retval |= load_mem_file_config(options::cmt_insert_oc_msg_header(), cpd.oc_msg_hdr);
	}
	log_rule_B("cmt_reflow_fold_regex_file");
	if(!options::cmt_reflow_fold_regex_file().empty()) {
		retval |= load_mem_file_config(options::cmt_reflow_fold_regex_file(), cpd.reflow_fold_regex);
	}
	return (retval);
} // load_header_files
...
static const char * make_output_filename(char * buf, size_t buf_size, const char * filename, const char * prefix, const char * suffix)
{
	...
	snprintf(&buf[len], buf_size - len, "%s%s", filename, (suffix != nullptr) ? suffix : "");
	return (buf);
}
...
static bool file_content_matches(const string &filename1, const string &filename2)
{
	...
	if((stat(filename1.c_str(), &st1) != 0) || (stat(filename2.c_str(), &st2) != 0) || st1.st_size != st2.st_size) {
		return false;
	}
...
static string fix_filename(const char * filename)
{
	...
	char   * tmp_file = new char[strlen(filename) + 16 + 1]; // + 1 for '//  + 1 for '/* + 1 for '\0' */' '
	if(tmp_file != nullptr) {
...
static bool bout_content_matches(const file_mem &fm, bool report_status)
{
	...
	if(cpd.bout->size() != fm.raw.size()) {
		if(report_status) {
			fprintf(stderr, "FAIL: %s (File size changed from %u to %u)\n", cpd.filename.c_str(), static_cast<int>(fm.raw.size()), static_cast<int>(cpd.bout->size()));
			log_flush(true);
		}
		is_same = false;
	}
...
static void do_source_file(const char * filename_in, const char * filename_out, const char * parsed_file,
    const char * dump_file, bool no_backup, bool keep_mtime)
{
	FILE * pfout = nullptr;
	bool did_open    = false;
	...
	if(did_open) {
		...
		if(filename_tmp != filename_out) {
			...
			if(!cpd.if_changed && file_content_matches(filename_tmp, filename_out)) {
				 ...
				 */
				if(!MoveFileExA(filename_tmp.c_str(), filename_out, MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED))
#else
...
static void add_func_header(E_Token type, file_mem &fm)
{
	...
	for(pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnlNpp()) {
		...
		// Check for one liners for functions. There'll be a closing brace w/o any newlines. Walk down the
		// chunks.
		ref = pc;
		...
		if(ref->Is(CT_FUNC_DEF)
			...
			while(ref->IsNot(CT_NEWLINE)
			    && (ref = ref->GetNext())) { // TODO: is the assignment of ref wanted here?, better move it
			                                 // to the loop
...
void uncrustify_file(const file_mem &fm, FILE * pfout, const char * parsed_file, const char * dump_file, bool defer_uncrustify_end)
{
	...
	if(options::utf8_force() || ((cpd.enc == char_encoding_e::e_BYTE) && options::utf8_byte())) {
	 ...
	 */
	{
		...
		if((language_is_set(LANG_C) || language_is_set(LANG_CPP)) && (options::mod_int_short() != IARF_IGNORE || options::mod_short_int() != IARF_IGNORE || 
			options::mod_int_long() != IARF_IGNORE || options::mod_long_int() != IARF_IGNORE || options::mod_int_signed() != IARF_IGNORE || 
			options::mod_signed_int() != IARF_IGNORE || options::mod_int_unsigned() != IARF_IGNORE || options::mod_unsigned_int() != IARF_IGNORE)) {
			change_int_types();
		}
...
//prot_the_line(__func__, __LINE__, 15, 4);
		...
		if(options::mod_sort_import() || options::mod_sort_include() || options::mod_sort_using()) {
			sort_imports();
		}
		...
		if((options::mod_add_long_ifdef_else_comment() > 0) || (options::mod_add_long_ifdef_endif_comment() > 0)) {
		...
		if(cpd.html_file == nullptr) {
			...
			FILE * t_file = fopen(cpd.html_file, "wb");
			if(t_file) {
				output_text(t_file);
				fclose(t_file);
				exit(EX_OK);
...
void uncrustify_end()
{
	...
	memzero(cpd.le_counts, sizeof(cpd.le_counts));
...
{
	if(token >= 0 && (token < static_cast<int> SIZEOFARRAY(token_names)) && (token_names[token] != nullptr)) {
...
E_Token find_token_name(const char * text)
{
	if(text != nullptr && (*text != 0)) {
		for(int idx = 1; idx < static_cast<int> SIZEOFARRAY(token_names); idx++) {
...
{
	int len1 = sstrleni(filename);
	int len2 = sstrleni(tag);
	return (len2 > 0 && len2 <= len1 && ((case_sensitive && (strcmp(&filename[len1-len2], tag) == 0)) || (!case_sensitive && (strcasecmp(&filename[len1-len2], tag) == 0))));
}
...
const char * get_file_extension(int &idx)
{
	...
	if(idx < static_cast<int> SIZEOFARRAY(language_exts)) {
