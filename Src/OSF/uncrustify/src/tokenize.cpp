...
struct TokenInfo {
	TokenInfo() : last_ch(0), idx(0), row(1), col(1)
	{
	}
	size_t last_ch;
	size_t idx;
...
struct TokenContext {
	TokenContext(const deque<int> &d) : data(d)
	{
	}
 ...
 * This is only for Xcode which sometimes inserts temporary code placeholder chunks, which in plaintext <#look like
 *this#>.
 *
 * @param pc  The structure to update, str is an input.
 *
...
static bool is_bin(int ch) { return ((ch == '0') || (ch == '1')); }
static bool is_bin_(int ch) { return (is_bin(ch) || ch == '_' || ch == '\''); }
static bool is_oct(int ch) { return ((ch >= '0') && (ch <= '7')); }
static bool is_oct_(int ch) { return (is_oct(ch) || ch == '_' || ch == '\''); }
static bool is_dec(int ch) { return ((ch >= '0') && (ch <= '9')); }
...
static bool parse_number(TokenContext &ctx, Chunk &pc)
{
	...
	if(((ctx.peek() == '3') && (ctx.peek(1) == '2')) || ((ctx.peek() == '6') && (ctx.peek(1) == '4'))) {
		pc.Str().append(ctx.get());
		pc.Str().append(ctx.get());
	}
...
static bool parse_cs_string(TokenContext &ctx, Chunk &pc)
{
	...
	while(ctx.more()) {
		if(parseState.top().braceDepth > 0) {
			// all we can do when in an expr is look for expr close with }, or a new string opening. must do
			// this first
			// so we can peek and potentially consume chars for new string openings, before the ch=get()
			// happens later,
		...
		if(ch == '\n') {
			...
			if(parseState.top().type & CS_STRING_VERBATIM) {
				if(!cpd.warned_unable_string_replace_tab_chars) {
					 ...
					 */
					LOG_FMT(warnlevel,
					    "%s(%d): %s: orig line is %zu, orig col is %zu, Detected non-replaceable tab char in literal string\n",
					    __func__,
					    __LINE__,
					    cpd.filename.c_str(),
					    pc.GetOrigLine(),
					    pc.GetOrigCol());
					LOG_FMT(warnlevel,
					    "%s(%d): Warning is given if doing tab-to-\\t replacement and we have found one in a C# verbatim string literal.\n",
					    __func__,
					    __LINE__);
			...
			// catch escaped quote in order to avoid ending string (but also must handle \\ to avoid
			// accidental 'escape' seq of `\\"`)
			if(ctx.peek() == '"'
			    || ctx.peek() == '\\') {
				pc.Str().append(ctx.get());
			}
...
static bool parse_cr_string(TokenContext &ctx, Chunk &pc, size_t q_idx)
{
	...
	pc.SetType(CT_STRING);
	while(ctx.more()) {
		if((ctx.peek() == ')') && (ctx.peek(tag_len + 1) == '"') && tag_compare(ctx.data, tag_idx, ctx.c.idx + 1, tag_len)) {
...
//ctx.restore(ctx.c);
	...
	if(language_is_set(LANG_D)) {
		...
		if((((ch == 'L') || (ch == 'S')) && ((ch1 == '"') || (ch1 == '\''))) || (ch == '"') || (ch == '\'') || ((ch == '<') && cpd.in_preproc == CT_PP_INCLUDE)) {
		...
		if((ch == '(') && (tail->IsNotNullChunk()) && (  tail->Is(CT_CNG_HASINC) || tail->Is(CT_CNG_HASINCN))) {
			parse_string(ctx, pc, 0, false);
			return true;
		}
	}
	// Check for Vala string templates
	if(language_is_set(LANG_VALA) && (ctx.peek() == '@')) {
	...
	if(language_is_set(LANG_OC) && (ctx.peek() == '@')) {
		size_t nc = ctx.peek(1);
		if(nc == 'R') { // Issue #2720
			if(ctx.peek(2) == '"') {
	...
	if(CharTable::IsKw1(ctx.peek()) || ((ctx.peek() == '\\') && (unc_tolower(ctx.peek(1)) == 'u')) || ((ctx.peek() == '@') && CharTable::IsKw1(ctx.peek(1)))) {
		parse_word(ctx, pc, false);
		return true;
	}
	// Check for C++11/14/17/20 attribute specifier sequences
	if(language_is_set(LANG_CPP) && ctx.peek() == '[') {
		if(!language_is_set(LANG_OC) || (  prev_pc->IsNotNullChunk() && !prev_pc->Is(CT_OC_AT))) {
	...
	char punc_txt[7];
	punc_txt[0] = static_cast<char>(ctx.peek());
	punc_txt[1] = static_cast<char>(ctx.peek(1));
	punc_txt[2] = static_cast<char>(ctx.peek(2));
	punc_txt[3] = static_cast<char>(ctx.peek(3));
	punc_txt[4] = static_cast<char>(ctx.peek(4));
	punc_txt[5] = static_cast<char>(ctx.peek(5));
	punc_txt[6] = '\0';
...
void tokenize(const deque<int> &data, Chunk * ref)
{
	...
	while(ctx.more()) {
		...
		if(pc->Is(CT_NEWLINE)) {
			...
			char copy[1000];
			LOG_FMT(LGUY,
			    "%s(%d): orig line is %zu, orig col is %zu, Text() '%s', type is %s, orig col end is %zu\n",
			    __func__,
			    __LINE__,
			    pc->GetOrigLine(),
			    pc->GetOrigCol(),
