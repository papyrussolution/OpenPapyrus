/**
 * @file token_enum.h List of the different tokens used in the program.
 * @author  Ben Gardner
 * @author  Guy Maurel since version 0.62 for uncrustify4Qt October 2015
 * @license GPL v2+
 */
 ...
 */
/**
 * This is an enum of all the different chunks/tokens/elements that the program can work with. The parser and scanner
 *assigns one of these to each chunk/token.
 */
enum E_Token {
	CT_NONE,
	CT_PARENT_NOT_SET,
	...
	CT_OC_BLOCK_TYPE, // ObjC: block declaration parent type, e.g. mainly the '(^block_t)' in 'void (^block_t)(int
	                  // arg);'
	CT_OC_BLOCK_EXPR, // ObjC: block expression with arg: '^(int arg) { arg++; };' and without (called a block
	                  // literal): '^{ ... };'
