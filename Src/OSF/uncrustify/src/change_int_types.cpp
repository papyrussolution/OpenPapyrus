...
{
	return (sstreq(pc->Text(), "auto") || sstreq(pc->Text(), "const") || sstreq(pc->Text(), "extern") || sstreq(pc->Text(), "mutable") || 
		sstreq(pc->Text(), "register") || sstreq(pc->Text(), "static") || sstreq(pc->Text(), "thread_local") || sstreq(pc->Text(), "typedef") || 
		sstreq(pc->Text(), "volatile") || sstreq(pc->Text(), "_Atomic") || sstreq(pc->Text(), "_Thread_local"));
}

static bool is_non_integer(const Chunk * pc)
{
	return (sstreq(pc->Text(), "char") || sstreq(pc->Text(), "double"));
}
...
void change_int_types()
{
	...
	for(Chunk * pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl()) {
		...
		else if(sstreq(pc->Text(), "unsigned")) {
			if(find_non_storage_siblings(pc, prev, next)) {
		...
		else if(!sstreq(pc->Text(), "int") && !is_storage_keyword(pc)) {
