...
#else
#define log_rule(rule) log_rule2(__func__, __LINE__, (rule), first, second)
#endif
// if you need more debug information, remove the comment at the next line
#define SUPER_LOG    1
#ifdef SUPER_LOG
	#define log_rule_B(rule) log_rule3(LCURRENT, __func__, __LINE__, (rule))
#else
	#define log_rule_B(rule) log_rule3(LCURRENT, __func__, (rule))
#endif
#ifdef SUPER_LOG
	#define log_rule_NL(rule) log_ruleNL((rule), pc)
#else
	#define log_rule_NL(rule) log_ruleNL(LCURRENT, __func__, (rule))
#endif
