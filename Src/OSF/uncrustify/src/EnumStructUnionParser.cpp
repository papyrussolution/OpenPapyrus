...
{
	LOG_FUNC_ENTRY();
	if(prev->IsNotNullChunk() && next->IsNotNullChunk()) {
	...
	if(prev->IsNotNullChunk() && next->IsNotNullChunk()) {
		...
		switch(prev_token_type) {
			case CT_ANGLE_CLOSE:
			     ...
			     * - an identifier
			     */
			    return (next->IsPointerOrReference() || next_token_type == CT_DC_MEMBER || next_token_type == CT_QUALIFIER || next_token_type == CT_WORD);
			case CT_ANGLE_OPEN:
			    /**
			     * assuming the previous token is possibly the opening angle of a
			     * templated type, just check to see if there's a matching closing
...
static bool chunk_is_macro_reference(Chunk * pc)
{
	...
	if((language_is_set(LANG_CPP) || language_is_set(LANG_C)) && pc->Is(CT_WORD) && !pc->TestFlags(PCF_IN_PREPROC)) {
		while(next->IsNotNullChunk()) {
			if(next->TestFlags(PCF_IN_PREPROC) && sstreq(pc->GetStr().c_str(), next->GetStr().c_str())) {
				return true;
			}
...
{
	LOG_FUNC_ENTRY();
	return (IsPointerOrReference() || (Is(CT_QUALIFIER) && !IsCppInheritanceAccessSpecifier()));
}
...
static std::pair<Chunk *, Chunk *> match_qualified_identifier(Chunk * pc)
{
	...
	if(end->IsNotNullChunk() && start->IsNotNullChunk()) {
		auto * double_colon = start->GetNextType(CT_DC_MEMBER);
		if(double_colon->IsNotNullChunk() && chunk_is_between(double_colon, start, end)) {
...
static std::tuple<Chunk *, Chunk *, Chunk *> match_variable(Chunk * pc, std::size_t level)
{
	...
	auto * identifier           =
	    identifier_end_pair.first->IsNotNullChunk() ? identifier_end_pair.first : start_identifier_pair.second;
...
void EnumStructUnionParser::mark_constructors()
{
	...
	if(body_detected()
		...
		for(auto * prev = body_start; next != body_end; prev = next) {
			 ...
			 */
			if(prev->IsNotNullChunk() && sstreq(prev->Text(), name) && prev->GetLevel() == level && next->IsParenOpen()) {
				prev->SetType(CT_FUNC_CLASS_DEF);
				LOG_FMT(LFTOR, "%s(%d): Constructor/destructor detected: '%s' at orig line is %zu, orig col is %zu, type is %s\n",
				    __unqualified_func__, __LINE__, name, prev->GetOrigLine(), prev->GetOrigCol(), get_token_name(prev->GetType()));
...
bool EnumStructUnionParser::where_clause_detected() const
{
	...
	auto * where_end   = get_where_end();
	auto * where_start = get_where_start();
	return (where_end->IsNotNullChunk() && where_start->IsNotNullChunk());
