...
void align_init_brace(Chunk * start)
{
	...
	if(pcSingle->IsNullChunk() || (  pcSingle->Is(CT_BRACE_CLOSE) && pcSingle->GetParentType() == CT_ASSIGN)) {
	...
	} while(pc->IsNotNullChunk() && pc->GetLevel() > start->GetLevel());
	// debug dump the current frame
	align_log_al(LALBR, start->GetOrigLine());
	log_rule_B("align_on_tabstop");
	if(options::align_on_tabstop() && cpd.al_cnt >= 1 && (cpd.al[0].type == CT_ASSIGN)) {
	...
	do {
		Chunk * tmp;
		if(idx == 0 && ((tmp = skip_c99_array(pc))->IsNotNullChunk())) {
			pc = tmp;
			...
			if(pc->Is(cpd.al[idx].type)) {
				...
				if(pc->Is(CT_COMMA)) {
					...
					if(!next->IsNewline()) {
						...
						if((idx < (cpd.al_cnt - 1)) && options::align_number_right() && (  next->Is(CT_NUMBER_FP) || 
							next->Is(CT_NUMBER) || next->Is(CT_POS) || next->Is(CT_NEG))) {
					...
					log_rule_B("align_number_right");
					if((idx < (cpd.al_cnt - 1)) && options::align_number_right()) {
						next = pc->GetNext();
						if(!next->IsNewline() && (  next->Is(CT_NUMBER_FP) || next->Is(CT_NUMBER) || next->Is(CT_POS) || next->Is(CT_NEG))) {
							// Need to wait until the next match to indent numbers
							num_token = next;
						}
					}
		...
		if(pc->IsNewline() || next->IsNewline()) {
			idx = 0;
		}
		pc = pc->GetNext();
	} while(pc->IsNotNullChunk() && pc->GetLevel() > start->GetLevel());
