...
Chunk * align_braced_init_list(Chunk * first, size_t span, size_t thresh, size_t * p_nl_count)
{
	...
	while(pc->IsNotNullChunk()) {
		LOG_FMT(LALASS, "%s(%d): orig line is %zu, check pc->Text() is '%s', type is %s, parent type is %s\n",
		    __func__, __LINE__, pc->GetOrigLine(), pc->ElidedText(copy), get_token_name(pc->GetType()),
		    get_token_name(pc->GetParentType()));

		// Don't check inside SPAREN, PAREN or SQUARE groups
		if(pc->Is(CT_SPAREN_OPEN) || pc->Is(CT_SQUARE_OPEN) || pc->Is(CT_PAREN_OPEN)) {
		...
		if(pc->Is(CT_BRACE_OPEN) && !(pc->GetParentType() == CT_BRACED_INIT_LIST)) {
			size_t myspan;
			size_t mythresh;
		...
		else if(equ_count == 0 && !pc->TestFlags(PCF_IN_TEMPLATE) && pc->Is(CT_BRACE_OPEN) && (pc->GetParentType() == CT_BRACED_INIT_LIST)) {
			equ_count++;
