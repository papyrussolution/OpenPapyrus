...
static void print_stack(log_sev_t logsev, const char * str, const ParsingFrame &frm)
{
	...
	for(size_t idx = 1; idx < frm.size(); idx++) {
		if(frm.at(idx).stage != E_BraceStage::NONE) {
			LOG_FMT(logsev, " [%s - %u]", get_token_name(frm.at(idx).type), (uint)frm.at(idx).stage);
		}
		else {
	...
	while(pc->IsNotNullChunk()) {
		...
		if(braceState.in_preproc != CT_NONE
			...
			if(braceState.in_preproc == CT_PP_DEFINE) {
				...
				if(options::pp_warn_unbalanced_if() && brace_level != 1) {
...
static void parse_cleanup(BraceState &braceState, ParsingFrame &frm, Chunk * pc)
{
		...
		if(pc->IsNot((E_Token)(frm.top().type + 1))) {
			if(pc->TestFlags(PCF_IN_PREPROC)) { // Issue #3113, #3283
				...
				if(frm.top().type != CT_EOF
				    && frm.top().type != CT_PP_DEFINE) {
					LOG_FMT(LWARN,
					    "%s(%d): File: %s, orig line is %zu, orig col is %zu, Error: Unexpected '%s' for '%s', which was on line %zu\n",
					    __func__,
					    __LINE__,
					    cpd.filename.c_str(),
					    pc->GetOrigLine(),
					    pc->GetOrigCol(),
					    pc->Text(),
					    get_token_name(frm.top().pc->GetType()),
	...
	if(pc->Is(CT_BRACE_OPEN)
		...
		if(pc->Is(CT_BRACE_OPEN)
			...
			LOG_FMT(LBCSPOP,
			    "%s(%d): pc orig line is %zu, orig col is %zu, Text() is '%s', type is %s, parent type is %s\n",
			    __func__,
			    __LINE__,
			    pc->GetOrigLine(),
			    pc->GetOrigCol(),
...
static bool check_complex_statements(ParsingFrame &frm, Chunk * pc, const BraceState &braceState)
{
	...
	if(frm.top().stage == E_BraceStage::ELSEIF) {
		...
		if(pc->Is(CT_IF) && ( !options::indent_else_if() || !pc->GetPrevNc()->IsNewline())) {
	...
	while(frm.top().stage == E_BraceStage::CATCH_) {
		if(pc->Is(CT_CATCH) || pc->Is(CT_FINALLY)) {
			...
			if(language_is_set(LANG_CS)) {
				...
				// historically this used OP_PAREN1; however, to my knowledge the expression after a
				// catch clause
				// is only optional for C# which has been handled above; therefore, this should now
				// always expect
				// a parenthetical expression after the catch keyword and brace after the finally
				// keyword
...
static bool handle_complex_close(ParsingFrame &frm, Chunk * pc, const BraceState &braceState)
{
	...
	if(frm.top().stage == E_BraceStage::PAREN1) {
		...
		if((frm.top().type == CT_IF)
			...
			frm.top().stage = E_BraceStage::CATCH_;
		...
		// PROBLEM
		LOG_FMT(LWARN, "%s(%d): %s:%zu Error: TOS.type='%s' TOS.stage=%u\n", __func__, __LINE__, cpd.filename.c_str(), pc->GetOrigLine(), get_token_name(frm.top().type), (uint)frm.top().stage);
		exit(EX_SOFTWARE);
	}
	return false;
...
bool close_statement(ParsingFrame &frm, Chunk * pc, const BraceState &braceState)
{
	LOG_FUNC_ENTRY();
	if(pc->IsNullChunk()) {
		throw invalid_argument(string(__func__) + ":" + to_string(__LINE__) + "args cannot be null chunk");
	}
	LOG_FMT(LTOK, "%s(%d): orig line is %zu, type is %s, '%s' type is %s, stage is %u\n",
	    __func__, __LINE__, pc->GetOrigLine(), get_token_name(pc->GetType()), pc->Text(), get_token_name(frm.top().type), (uint)frm.top().stage);
	if(braceState.consumed) {
		frm.stmt_count = 0;
		frm.expr_count = 0;
