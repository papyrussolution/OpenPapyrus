/**
 * @file keywords.h prototypes for keywords.c
 *
 * @author  Ben Gardner
 * @license GPL v2+
 */
 ...
 *
 * Let us have a look on the problem is caused by the tokens "in" and "out", used in the file "Issue_3353.h" under. The
 *strings representing the tokens ("in" and "out"), are found in the original table: static chunk_tag_t keywords[]
 *because they are used by other languages. They are tokenized as CT_IN and CT_OUT. The correct tokenization is
 *CT_FUNC_VAR.
 *
 * It is necessary to create (at run time) a new table with all the keywords proper to the used language.
 *
 */
/**
 * The file
