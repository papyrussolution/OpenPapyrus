...
void output_text(FILE * pfile)
{
	...
	for(pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNext()) {
		char copy[1000];
		LOG_FMT(LCONTTEXT,
		    "%s(%d): Text() is '%s', type is %s, orig line is %zu, column is %zu, nl is %zu\n",
		    __func__,
		    __LINE__,
		    pc->ElidedText(copy),
				...
				if(options::sp_before_nl_cont() & IARF_REMOVE) {
					...
					if(prev->Is(CT_PP_IGNORE)) {
						...
						if(prev->IsNotNullChunk()
							...
							// Add or remove space before a backslash-newline at the end of
							// a line.
							log_rule_B("sp_before_nl_cont");
...
void dump_step(const char * filename, const char * step_description)
{
	...
	if(sstrlen(filename) > 0) {
		...
		if(file_num == 0) {
			...
			++file_num;
			FILE * dump_file = fopen(buffer, "wb");
			if(dump_file) {
...
static Chunk * output_comment_c(Chunk * first)
{
	...
	while(can_combine_comment(pc, cmt)) {
		...
		// In case of reflow, original comment could contain trailing spaces before closing the comment, we
		// don't need them after reflow
	...
	// In case of reflow, original comment could contain trailing spaces before closing the comment, we don't need
	// them after reflow
	cmt_trim_whitespace(tmp, false);
	add_comment_text(tmp, cmt, false);
			...
			if(end_idx > start_idx) {
				...
				if(is_exception_tag
					...
					while(true) {
						...
						/**
						 * check for the possibility that comma-separated parameter names are
						 *present
						 */
						cmt_idx = eat_line_whitespace(pc->GetStr(),
							cmt_idx);
			...
			if(line_count == 1) {
				...
				if(line.size() == 0) {
					...
					if(options::cmt_star_cont()) {
						// The number of spaces to insert at the start of subsequent comment
						// lines.
						...
						// multiline comments can have empty lines with some spaces in them for
						// alignment
						// while adding * symbol and aligning them we don't want to keep these
						// trailing spaces
					...
					if(options::cmt_indent_multi()
						...
						if(options::cmt_star_cont()) {
							...
							add_text(cmt.cont_text);
							// The number of spaces to insert after the star on subsequent
							// comment lines.
							log_rule_B("cmt_sp_after_star_cont");
		...
		while((tmp = tmp->GetNext())->IsNotNullChunk()) {
			...
			if(tmp->IsNewline()) {
				...
				if(tmp->IsNullChunk()
					...
					LOG_FMT(LPPIF,
					    "#if / %s section candidate for augmenting when over NL threshold %zu != 0 (new line count=%zu)\n",
					    txt,
					    nl_min,
					    nl_count);

					if(nl_min > 0
					    && nl_count > nl_min) { // nl_count is 1 too large at all times as #if line
					                            // was counted too
						...
						LOG_FMT(LPPIF,
						    "#if / %s section over threshold %zu (new line count=%zu) --> insert comment after the %s: %s\n",
						    txt,
						    nl_min,
						    nl_count,
						    txt,
				...
				// checks both the #else and #endif for a given level, only then look further in the
				// main loop
				if(br_close->Is(CT_PP_ENDIF)) {
					break;
				}
