...
Chunk * fix_variable_definition(Chunk * start)
{
	...
	if(cs.Len() == 1 && end->Is(CT_BRACE_OPEN) && end->GetParentType() == CT_BRACED_INIT_LIST) {
		cs.Get(0)->m_pc->SetType(CT_TYPE);
	}
	// Function defs are handled elsewhere
	if((cs.Len() <= 1) || end->Is(CT_FUNC_DEF) || end->Is(CT_FUNC_PROTO) || end->Is(CT_FUNC_CLASS_DEF) || end->Is(CT_FUNC_CLASS_PROTO) || end->Is(CT_OPERATOR)) {
	...
	if((cs.Len() >= 3) && ((cs.Get(cs.Len() - 2)->m_pc->GetType() == CT_MEMBER) || (cs.Get(cs.Len() - 2)->m_pc->GetType() == CT_DC_MEMBER))) {
		idx = cs.Len() - 2;
		while(idx > 0) {
			tmp_pc = cs.Get(idx)->m_pc;
			if(tmp_pc->IsNot(CT_DC_MEMBER) && tmp_pc->IsNot(CT_MEMBER)) {
				break;
			}
			idx--;
			tmp_pc = cs.Get(idx)->m_pc;
			if(tmp_pc->IsNot(CT_WORD) && tmp_pc->IsNot(CT_TYPE)) {
				break;
			}
			make_type(tmp_pc);
			idx--;
...
void mark_cpp_constructor(Chunk * pc)
{
	...
	if(tmp->Is(CT_INV) || tmp->Is(CT_DESTRUCTOR)) {
	...
	if(!paren_open->IsString("(")) {
		LOG_FMT(LWARN, "%s:%zu Expected '(', got: [%s]\n", cpd.filename.c_str(), paren_open->GetOrigLine(), paren_open->Text());
		return;
	}
	// Mark parameters
	...
	while(tmp->IsNotNullChunk() && (  tmp->IsNot(CT_BRACE_OPEN) || tmp->GetLevel() != paren_open->GetLevel()) && !tmp->IsSemicolon()) {
		...
		if(tmp->IsString(":") && tmp->GetLevel() == paren_open->GetLevel()) {
			tmp->SetType(CT_CONSTR_COLON);
			hit_colon = true;
		}
		if(hit_colon && (tmp->IsParenOpen() || tmp->IsBraceOpen()) && tmp->GetLevel() == paren_open->GetLevel()) {
			var = skip_template_prev(tmp->GetPrevNcNnlNi()); // Issue #2279
			if(var->Is(CT_TYPE) || var->Is(CT_WORD)) {
...
{
	if(square_open->Is(CT_SQUARE_OPEN) && square_open->GetParentType() == CT_CPP_LAMBDA) {
...
void mark_define_expressions()
{
	...
	while(pc->IsNotNullChunk()) {
		if(!in_define) {
			if(pc->Is(CT_PP_DEFINE) || pc->Is(CT_PP_IF) || pc->Is(CT_PP_ELSE)) {
				in_define = true;
				first     = true;
			}
		}
		else {
			if(!pc->TestFlags(PCF_IN_PREPROC) || pc->Is(CT_PREPROC)) {
				in_define = false;
			}
			else {
				if(pc->IsNot(CT_MACRO) && (first || prev->Is(CT_PAREN_OPEN) || prev->Is(CT_ARITH) || prev->Is(CT_SHIFT) || prev->Is(CT_CARET) || 
					prev->Is(CT_ASSIGN) || prev->Is(CT_COMPARE) || prev->Is(CT_RETURN) || prev->Is(CT_GOTO) || prev->Is(CT_CONTINUE) || 
					prev->Is(CT_FPAREN_OPEN) || prev->Is(CT_SPAREN_OPEN) || prev->Is(CT_BRACE_OPEN) || prev->IsSemicolon() || prev->Is(CT_COMMA) || 
					prev->Is(CT_COLON) || prev->Is(CT_QUESTION))) {
					pc->SetFlagBits(PCF_EXPR_START);
					first = false;
				}
			}
...
void mark_exec_sql(Chunk * pc)
{
	...
	if(pc->IsNot(CT_SQL_BEGIN) || tmp->IsNullChunk() || tmp->IsNot(CT_SEMICOLON)) {
		return;
	}
	for(tmp = tmp->GetNext(); tmp->IsNotNullChunk() && tmp->IsNot(CT_SQL_END); tmp = tmp->GetNext()) {
	...
	if(pc->IsNotNullChunk()) {
		...
		while(pc->IsNotNullChunk()) {
			...
			if(pc->Is(CT_ANGLE_CLOSE)) {
				...
				if(pc->IsNullChunk()
				    || pc->Is(CT_TEMPLATE)) {
					//either expression is not complete or this is smth like 'template<T> void
					// func()'
					//  - we are not interested in 'template<T>' part
					break;
				}
				else {
					//this is smth like 'vector<int> func()' and 'pc' is currently on 'vector' -
					// just proceed
				}
			}
			if((!pc->IsTypeDefinition() && pc->IsNot(CT_OPERATOR) && pc->IsNot(CT_WORD) && pc->IsNot(CT_ADDR)) || pc->TestFlags(PCF_IN_PREPROC)) {
				break;
			}
			if(!pc->IsPointerOperator()) {
				first = pc;
		...
		if(pc->Is(CT_PAREN_CLOSE) && !pc->TestFlags(PCF_IN_PREPROC)) {
		...
		while(pc->IsNotNullChunk()) {
			...
			if(!is_return_tuple || pc->IsNot(CT_WORD) || (prev->IsNullChunk() && prev->IsNot(CT_TYPE))) {
				make_type(pc);
			}
			if(pc == start) {
				break;
		...
		if(parent_type != CT_NONE && first && first->TestFlags(PCF_IN_CLASS)) {
...
void mark_function(Chunk * pc)
{
	...
	if(pc->GetParentType() == CT_OPERATOR) {
		LOG_FMT(LFCN, "%s(%d): orig line is %zu, orig col is %zu, Text() '%s", __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->Text());
		log_pcf_flags(LGUY, pc->GetFlags());
		Chunk * pc_op = pc->GetPrevType(CT_OPERATOR, pc->GetLevel());
		if(pc_op->IsNotNullChunk() && pc_op->TestFlags(PCF_EXPR_START)) {
			...
			while((tmp = tmp->GetPrevNcNnlNi())->IsNotNullChunk()) { // Issue #2279
				...
				if(tmp->Is(CT_BRACE_OPEN)) {
					if(tmp->GetParentType() == CT_FUNC_DEF) {
						LOG_FMT(LFCN,
						    "%s(%d): (8) SET TO CT_FUNC_CALL: orig line is %zu, orig col is %zu, Text() '%s'\n",
						    __func__,
						    __LINE__,
						    pc->GetOrigLine(),
						    pc->GetOrigCol(),
					...
					if(tmp->GetParentType() == CT_CLASS
						...
						LOG_FMT(LFCN,
						    "%s(%d): (9) SET TO CT_FUNC_DEF: orig line is %zu, orig col is %zu, Text() '%s'\n",
						    __func__,
						    __LINE__,
						    pc->GetOrigLine(),
						    pc->GetOrigCol(),
	...
	if(tmp->IsNotNullChunk()
		...
		if(tmp3->IsString(")")
			...
			if(tmp2->IsNotNullChunk()) {
				LOG_FMT(LFCN,
				    "%s(%d): orig line is %zu, orig col is %zu, function variable '%s', changing '%s' into a type\n",
				    __func__,
				    __LINE__,
				    pc->GetOrigLine(),
				    pc->GetOrigCol(),
		...
		while(prev->IsNotNullChunk()) {
				...
				while(prev->Is(CT_DC_MEMBER)
					...
					if(prev->IsNullChunk()
						...
						LOG_FMT(LFCN, "%s(%d): --? skipped MEMBER and landed on %s\n",
						    __func__, __LINE__,
						    (prev->IsNullChunk()) ? "<null chunk>" : get_token_name(prev->GetType()));
						break;
					}
					...
					// Issue #1112
					// clarification: this will skip the CT_WORD, CT_TYPE or CT_THIS landing on
					// either
					// another CT_DC_MEMBER or CT_MEMBER or a token that indicates the context of
					// the
			...
			if(prev->Is(CT_TYPE)
				...
				if(!prev_prev->Is(CT_QUESTION)) { // Issue #1753
					...
					LOG_FMT(LFCN,
					    "%s(%d): prev is '%s', orig line is %zu, orig col is %zu, type is %s, parent type is %s\n",
					    __func__,
					    __LINE__,
					    prev->Text(),
					    prev->GetOrigLine(),
			...
			if(br_open->IsNotNullChunk()
				...
				if(!prev->IsString("*")
					...
					if(p_op->IsNotNullChunk()
						...
						pc->SetType(CT_FUNC_CTOR_VAR);
						LOG_FMT(LFCN,
						    "%s(%d):   4) Marked Text() is'%s', as FUNC_CTOR_VAR on orig line %zu, orig col %zu\n",
						    __func__,
						    __LINE__,
						    pc->Text(),
						    pc->GetOrigLine(),
...
PcfFlags mark_where_chunk(Chunk * pc, E_Token parent_type, PcfFlags flags)
{
	...
	if(pc->Is(CT_WHERE)) {
		...
		if(pc->IsString(":")) {
			...
			/* class/struct inside of a where-clause confuses parser for indentation; set it as a word so it
			   looks like the rest */
			pc->SetType(CT_WORD);
		}
	}
