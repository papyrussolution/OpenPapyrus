	...
	for(Chunk * pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl()) {
		...
		if(pc->Is(CT_MEMBER)
			...
			if(tmp->Is(CT_FPAREN_CLOSE)
				...
				// noptr-declarator ( parameter-list ) cv(optional) ref(optional) except(optional)
				// attr(optional) -> trailing
...
void tokenize_cleanup()
{
	...
	pc   = Chunk::GetHead();
	next = pc->GetNextNcNnl();
	while(pc->IsNotNullChunk() && next->IsNotNullChunk()) {
		if(pc->Is(CT_DOT) && language_is_set(LANG_ALLC)) {
			pc->SetType(CT_MEMBER);
		}
		if(pc->Is(CT_NULLCOND) && language_is_set(LANG_CS)) {
			pc->SetType(CT_MEMBER);
		}
		 ...
		 * var x = (T)base.y;
		 */
		if(pc->Is(CT_BASE) && (  next->Is(CT_PAREN_OPEN) || next->Is(CT_DOT))) {
			pc->SetType(CT_WORD);
		}
		if(pc->Is(CT_ENUM) && (  next->Is(CT_STRUCT) || next->Is(CT_CLASS))) {
		 ...
		 */
		if((next->Is(CT_STAR)) || (  language_is_set(LANG_CPP) && (next->Is(CT_CARET))) || (  language_is_set(LANG_CS | LANG_VALA) && (next->Is(CT_QUESTION)) && (strcmp(pc->Text(), "null") != 0))) {
			if(pc->Is(CT_TYPE) || pc->Is(CT_QUALIFIER) || pc->Is(CT_PTR_TYPE)) {
				next->SetType(CT_PTR_TYPE);
			}
		}
		if(pc->Is(CT_TYPE_CAST) && next->Is(CT_ANGLE_OPEN)) {
		...
		if(language_is_set(LANG_CPP)) {
			...
			if(pc->Is(CT_WORD) && next->Is(CT_DC_MEMBER)) {
				prev = pc->GetPrev();
			...
			if(prev->Is(CT_IF) && pc->Is(CT_QUALIFIER) && pc->IsString("constexpr")) {
...
static void mark_attributes_in_property_with_open_paren(Chunk * open_paren)
{
	...
	while(tmp->IsNot(CT_PAREN_CLOSE)) {
		...
		if((  tmp->Is(CT_COMMA) || tmp->Is(CT_PAREN_OPEN)) && (  next->Is(CT_WORD) || next->Is(CT_TYPE))) {
