...
/**
 * Checks to see if a series of chunks could be a C++ parameter FOO foo(5, &val);
 *
...
/*
 * Checks whether or not a given chunk has a parent cpp template, and if so returns the associated angle bracket nest
 *level with respect to the root parent template; returns 0 if the chunk is not part of a template parameter list
 */
size_t get_cpp_template_angle_nest_level(Chunk * pc);
/**
 * Parse off the types in the D template args, adds to cs returns the close_paren
 */
