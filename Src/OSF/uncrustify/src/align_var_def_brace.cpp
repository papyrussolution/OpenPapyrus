...
Chunk * align_var_def_brace(Chunk * start, size_t span, size_t * p_nl_count)
{
	...
	if(start->GetParentType() == CT_STRUCT || start->GetParentType() == CT_UNION) {
	...
	auto const align_mask = PCF_IN_FCN_DEF | PCF_VAR_1ST | (options::align_var_def_inline() ? PCF_NONE : PCF_VAR_INLINE);
	...
	while(pc->IsNotNullChunk()) {
		LOG_CHUNK(LAVDB, pc);
		if(pc->GetLevel() < start->GetLevel() && pc->GetLevel() != 0 && !pc->IsPreproc()) {
		...
		if(fp_active && !pc->TestFlags(PCF_IN_CLASS_BASE)) {
			// WARNING: Duplicate from the align_func_proto()
			log_rule_B("align_single_line_func");
			if(pc->Is(CT_FUNC_PROTO) || (pc->Is(CT_FUNC_DEF) && options::align_single_line_func())) {
				...
				fp_look_bro = (pc->Is(CT_FUNC_DEF)) && options::align_single_line_brace();
			}
			else if(fp_look_bro && pc->Is(CT_BRACE_OPEN) && pc->TestFlags(PCF_ONE_LINER)) {
				as_br.Add(pc);
				fp_look_bro = false;
			}
		}
		...
		if(!pc->TestFlags(PCF_IN_CLASS_BASE) && pc->IsNot(CT_FUNC_CLASS_DEF) && pc->IsNot(CT_FUNC_CLASS_PROTO) && 
			((pc->GetFlags() & align_mask) == PCF_VAR_1ST) && pc->IsNot(CT_FUNC_DEF)/*Issue 1452*/ && ((pc->GetLevel() == (start->GetLevel() + 1)) || 
            pc->GetLevel() == 0) && pc->GetPrev()->IsNot(CT_MEMBER)) {
			...
			LOG_FMT(LAVDB, "%s(%d): Text() is '%s', orig line is %zu, orig col is %zu, level is %zu\n",
			    __func__, __LINE__, pc->Text(), pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLevel());
			if(!did_this_line) {
				if(start->GetParentType() == CT_STRUCT && (as.m_star_style == AlignStack::SS_INCLUDE)) {
					// we must look after the previous token
					Chunk * prev_local = pc->GetPrev();
					while(prev_local->Is(CT_PTR_TYPE) || prev_local->Is(CT_ADDR)) {
					...
					LOG_FMT(LAVDB, "%s(%d): pc->Text() is '%s', level is %zu, brace level is %zu\n", __func__, __LINE__, pc->IsNewline() ? "Newline" : pc->Text(),
				...
				if(options::align_var_def_attribute()) {
					...
					while((next = next->GetNextNc())->IsNotNullChunk()) {
						...
						if(next->Is(CT_SEMICOLON) || next->IsNewline()) {
							break;
						}
					}
				}
