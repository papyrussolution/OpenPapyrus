...
Chunk * align_var_def_brace(Chunk * start, size_t span, size_t * p_nl_count)
{
	...
	while(pc->IsNotNullChunk()) {
		...
		if(pc->GetLevel() < start->GetLevel()
		    ...
		    && !pc->IsPreproc()) {
			LOG_FMT(LAVDB,
			    "%s(%d): orig line is %zu, orig col is %zu, Text() '%s', type is %s, PRE is %s\n",
			    __func__,
			    __LINE__,
			    pc->GetOrigLine(),
			    pc->GetOrigCol(),
		...
		if(!pc->TestFlags(PCF_IN_CLASS_BASE)
			...
			if(!did_this_line) {
				if(start->GetParentType() == CT_STRUCT
					...
					LOG_FMT(LAVDB,
					    "%s(%d): pc->Text() is '%s', level is %zu, brace level is %zu\n",
					    __func__,
					    __LINE__,
					    pc->IsNewline() ? "Newline" : pc->Text(),
					    pc->GetLevel(),
					    pc->GetBraceLevel());
				}
				// we must look after the previous token
