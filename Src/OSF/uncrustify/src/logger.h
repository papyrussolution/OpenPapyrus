...
#else
#define LOG_FMT(sev, ...) if(log_sev_on(sev)) { log_fmt(sev, __VA_ARGS__); }
#endif
...
#define LOG_CHUNK(sev, pc_current)                                                                                                                                                             \
	if(pc_current->Is(CT_NEWLINE)) {                                                                                                                                                            \
		...
		LOG_FMT(sev, "%s(%d): orig line is %zu, orig col is %zu, Text() '%s', type is %s, PRE is %s\n",                                                                                          \
		    __func__, __LINE__, pc_current->GetOrigLine(), pc_current->GetOrigCol(), pc_current->Text(), \
		    get_token_name(pc_current->GetType()), pc_current->IsPreproc() ? "true" : "false"); \
	}                                                                                                                                                                                           \
	else {                                                                                                                                                                                       \
		LOG_FMT(sev, "%s(%d): orig line is %zu, orig col is %zu, Text() '%s', type is %s, PRE is %s\n",                                                                                          \
		    __func__, __LINE__, pc_current->GetOrigLine(), pc_current->GetOrigCol(), pc_current->Text(), \
