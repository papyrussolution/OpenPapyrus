...
Chunk * Chunk::GetNext(const E_Scope scope) const
{
	...
	while(pc->IsNotNullChunk() && pc->TestFlags(PCF_IN_PREPROC)) {
		pc = pc->m_next;
	}
	return pc;
} // Chunk::GetNext
...
{
	return (Is(CT_TYPE) || Is(CT_PTR_TYPE) || Is(CT_BYREF) || Is(CT_DC_MEMBER) || Is(CT_QUALIFIER) || Is(CT_STRUCT) || Is(CT_ENUM) || Is(CT_UNION));
}
