...
void print_option_choices(FILE * pfile, uncrustify::GenericOption * option, char const * key = "Choices")
{
	fprintf(pfile, "%s=", key);
		...
		for(auto * const option : p_grp->options) {
			...
			while(*tmp != 0 && *(tmp + 1) != 0) {
				switch(*tmp) {
					...
					case '>': fputs("&gt;", pfile); break;
					case '&': fputs("&amp;", pfile); break;
					case '\n': fputs("<br/>", pfile); break;
					default: fputc(*tmp, pfile);
				}
				tmp++;
			}
				...
				switch(option->type()) {
					...
					case uncrustify::OT_IARF:
					    ...
					    fprintf(pfile, "ChoicesReadable=\"(%zu)Ignore %s|(%zu)Add %s|(%zu)Remove %s|(%zu)Force %s\"\n",
							optionNumber, optionNameReadable, optionNumber, optionNameReadable, optionNumber, optionNameReadable, optionNumber, optionNameReadable);
#else // DEBUG
					    //                                0         1      2         3
...
#if defined (DEBUG) && !defined (WIN32)
					    fprintf(pfile,
						"ChoicesReadable=\"(%zu)Newlines Unix|(%zu)Newlines Win|(%zu)Newlines Mac|(%zu)Newlines Auto\"\n",
						optionNumber,
						optionNumber,
						optionNumber,
						optionNumber);
...
#if defined (DEBUG) && !defined (WIN32)
					    fprintf(pfile,
						"ChoicesReadable=\"(%zu)Ignore %s|(%zu)Break %s|(%zu)Force %s|(%zu)Lead %s|(%zu)Trail %s|",
						optionNumber,
						optionNameReadable,
						optionNumber,
						optionNameReadable,
...
#if defined (DEBUG) && !defined (WIN32)
					    fprintf(pfile,
						"(%zu)Join %s|(%zu)Lead Break %s|(%zu)Lead Force %s|(%zu)Trail Break %s|(%zu)Trail Force %s\"\n",
						optionNumber,
						optionNameReadable,
						optionNumber,
						optionNameReadable,
