...
void do_symbol_check(Chunk * prev, Chunk * pc, Chunk * next)
{
	...
	if(next->Is(CT_PAREN_OPEN)
		...
		if(pc->Is(CT_DELEGATE)) {
			...
			for(tmp = pc->GetPrevNcNnlNi(); tmp->IsNotNullChunk(); tmp = tmp->GetPrevNcNnlNi()) { // Issue
			                                                                                      // #2279
				if(tmp->IsSemicolon()
	...
	if(next->Is(CT_PAREN_OPEN)) {
		...
		if(language_is_set(LANG_C | LANG_CPP | LANG_OC)
			...
			// This is the case where a block literal is passed as the first argument of a C-style method
			// invocation.
					...
					if(tmp->IsNotNullChunk()) {
						if(tmp->IsSemicolon()
							...
							if(!(  next->Is(CT_OPERATOR)
							    ...
							    || next->Is(CT_DC_MEMBER))) {
								LOG_FMT(LFCNR,
								    "%s(%d): orig line is %zu, orig col is %zu, Text() '%s', set PCF_VAR_1ST\n",
								    __func__,
								    __LINE__,
								    pc->GetOrigLine(),
								    pc->GetOrigCol(),
		...
		if(semi->Is(CT_NEWLINE)
			...
			if(mod_paren_value == IARF_REMOVE) {
				...
				for(temp = next; temp != cpar; temp = temp->GetNext()) {
					if(temp->GetLevel() == 0) {
						fprintf(stderr,
						    "%s(%d): temp->GetLevel() is ZERO, cannot be decremented, at line %zu, column %zu\n",
						    __func__,
						    __LINE__,
						    temp->GetOrigLine(),
						    temp->GetOrigCol());
...
static void handle_oc_class(Chunk * pc)
{
	...
	bool passed_name   = false;     // Did we pass the name of the class and now there can be only protocols, not
	                                // generics
...
static void handle_oc_block_literal(Chunk * pc)
{
	...
	if(apo->IsNotNullChunk()) {
		...
		if(apc->IsParenClose()) {
			LOG_FMT(LOCBLK,
			    " -- marking parens @ apo orig line is %zu, orig col is %zu and apc orig line is %zu, orig col is %zu\n",
			    apo->GetOrigLine(),
			    apo->GetOrigCol(),
			    apc->GetOrigLine(),
			    apc->GetOrigCol());
