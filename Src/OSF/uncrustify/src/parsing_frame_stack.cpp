		...
		if(pc->GetParentType() == CT_PP_IF) {
			...
			if(if_block) {
				...
				if(options::pp_warn_unbalanced_if()
					...
					LOG_FMT(LWARN,
					    "%s(%d): orig line is %zu, unbalanced #if block braces (1), in-level is %zu, out-level is %zu\n",
					    __func__,
					    __LINE__,
					    pc->GetOrigLine(),
					    base_brace_level,
				...
				// check if previous #else block has a different indentation than the corresponding #if
				// block
				...
				if(options::pp_warn_unbalanced_if()
					...
					LOG_FMT(LWARN,
					    "%s(%d): orig line is %zu, unbalanced #if-#else block braces (1), #else out-level is %zu, #if out-level is %zu\n",
					    __func__,
					    __LINE__,
					    pc->GetOrigLine(),
					    brace_level,
			...
			if(frm.in_ifdef == CT_PP_ELSE) {
				...
				if(options::pp_warn_unbalanced_if()
					...
					LOG_FMT(LWARN,
					    "%s(%d): orig line is %zu, unbalanced #if-#else block braces (2), #else out-level is %zu, #if out-level is %zu\n",
					    __func__,
					    __LINE__,
					    pc->GetOrigLine(),
					    brace_level,
				...
				if(options::pp_warn_unbalanced_if()
				    && brace_level != frm.GetBraceLevel()) {
					LOG_FMT(LWARN,
					    "%s(%d): orig line is %zu, unbalanced #if block braces (2), in-level is %zu, out-level is %zu\n",
					    __func__,
					    __LINE__,
					    pc->GetOrigLine(),
					    frm.GetBraceLevel(),
