/**
 * @file option.h Enumerations and data types for options.
 *
 * @author  Ben Gardner
...
class GenericOption {
public:
	GenericOption(const char * opt_name, const char * opt_desc) : m_name{opt_name}, m_desc{opt_desc}
	{
	}
...
public:
	BoundedOption(const char * opt_name, const char * opt_desc, T opt_val = T{}) : Option<T>{opt_name, opt_desc, opt_val}
	{
...
protected:
	bool validate(long val) override
	{
		if(val < static_cast<long>(min)) {
			...
			w("requested value %ld for option '%s' is less than the minimum value %ld", val, this->name(), static_cast<long>(min));
			return false;
		}
		if(val > static_cast<long>(max)) {
			OptionWarning w{ this };
			w("requested value %ld for option '%s' is greater than the maximum value %ld", val, this->name(), static_cast<long>(max));
			return false;
		}
		return true;
	}
...
/**
 * processes a single line string to extract configuration settings increments cpd.line_number and cpd.error_count
 *
 * @param config_line  single line string that will be processed
 * @param filename     for log messages, file from which the \p config_line param was extracted
