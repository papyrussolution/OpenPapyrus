...
Chunk * chunk_get_next_local(Chunk * pc, E_Scope scope = E_Scope::ALL)
{
	...
	} while(tmp->IsNotNullChunk() && (  tmp->IsComment() || tmp->Is(CT_NOEXCEPT)));
	return (tmp);
}
...
Chunk * chunk_get_prev_local(Chunk * pc, E_Scope scope = E_Scope::ALL)
{
	...
	} while(tmp->IsNotNullChunk() && (  tmp->IsCommentOrNewline() || tmp->Is(CT_NOEXCEPT)));
	return (tmp);
}
		...
		if(next->Is(CT_QUESTION)
			...
			if(cs_top_is_question(cs, next->GetLevel())
				...
				if(language_is_set(LANG_PAWN)) {
					...
					if(tmp->IsNotNullChunk()) {
						LOG_FMT(LFCN,
						    "%s(%d): tmp->Text() is '%s', orig line is %zu, orig col is %zu, type is %s\n",
						    __func__,
						    __LINE__,
						    tmp->Text(),
						    tmp->GetOrigLine(),
						...
						if(tmp->Is(CT_BASE)
							...
							LOG_FMT(LWARN,
							    "%s(%d): %s:%zu unexpected colon in col %zu n-parent=%s c-parent=%s l=%zu bl=%zu\n",
							    __func__,
							    __LINE__,
							    cpd.filename.c_str(),
							    next->GetOrigLine(),
