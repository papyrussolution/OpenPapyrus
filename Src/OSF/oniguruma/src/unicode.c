...
static const ushort EncUNICODE_ISO_8859_1_CtypeTable[256] = {
...
extern int onigenc_unicode_mbc_case_fold(OnigEncoding enc, OnigCaseFoldType flag, const uchar ** pp, const uchar * end, uchar * fold)
{
	...
	int i, rlen;
	const uchar * p = *pp;
	OnigCodePoint code = ONIGENC_MBC_TO_CODE(enc, p, end);
...
static int wb_get_next_main_code(OnigEncoding enc, uchar * p, const uchar * end, OnigCodePoint* rcode, enum WB_TYPE* rtype)
{
	OnigCodePoint code;
	enum WB_TYPE type;
	while(TRUE) {
...
extern int onigenc_egcb_is_break_position(OnigEncoding enc, uchar * p, uchar * prev,
...
extern int onigenc_unicode_is_code_ctype(OnigCodePoint code, uint ctype)
{
	if(
#ifdef USE_UNICODE_PROPERTIES
...
extern int onigenc_unicode_property_name_to_ctype(OnigEncoding enc, uchar * name, uchar * end)
{
	OnigCodePoint code;
