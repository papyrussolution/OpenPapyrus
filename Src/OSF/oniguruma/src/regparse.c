...
#define SET_ALL_MULTI_BYTE_RANGE(enc, pbuf) add_code_range_to_buf(pbuf, MBCODE_START_POS(enc), ~((OnigCodePoint)0))
...
typedef struct StStrEndKey {
	uchar * s;
	uchar * end;
} st_str_end_key;

static int str_end_cmp(/*st_str_end_key*/void * pX, /*st_str_end_key*/void * pY)
{
	st_str_end_key * x = (st_str_end_key *)pX;
	st_str_end_key * y = (st_str_end_key *)pY;
	uchar * p, * q;
	int c;
...
static int str_end_hash(/*st_str_end_key*/void * pX)
{
	st_str_end_key * x = (st_str_end_key *)pX;
	uint val = 0;
	uchar * p = x->s;
...
int onig_st_insert_strend(hash_table_type table, const uchar * str_key, const uchar * end_key, hash_data_type value)
{
	...
	key->s   = (uchar *)str_key;
	key->end = (uchar *)end_key;
	result = onig_st_insert((st_table *)table, (st_data_t)key, value);
	if(result) {
		SAlloc::F(key);
	}
	return result;
...
static int callout_name_table_cmp(/*st_callout_name_key*/void * pX, /*st_callout_name_key*/void * pY)
{
	st_callout_name_key * x = (st_callout_name_key *)pX;
	st_callout_name_key * y = (st_callout_name_key *)pY;
	uchar * p, * q;
	int c;
	if(x->enc  != y->enc) 
		return 1;
...
static int callout_name_table_hash(/*st_callout_name_key*/void * pX)
{
	st_callout_name_key * x = (st_callout_name_key *)pX;
	uint val = 0;
	uchar * p = x->s;
...
static int st_insert_callout_name_table(hash_table_type table, OnigEncoding enc, int type, uchar * str_key, uchar * end_key, hash_data_type value)
{
	int result;
	st_callout_name_key* key = (st_callout_name_key*)SAlloc::M(sizeof(st_callout_name_key));
	CHECK_NULL_RETURN_MEMERR(key);
...
static int names_clear(regex_t* reg)
{
	NameTable * t = (NameTable *)reg->name_table;
	if(IS_NOT_NULL(t)) {
		onig_st_foreach(t, (int (__cdecl *)(...))i_free_name_entry, 0);
	}
	return 0;
}
...
int onig_renumber_name_table(regex_t* reg, GroupNumMap* map)
{
	NameTable* t = (NameTable*)reg->name_table;
	if(IS_NOT_NULL(t)) {
		onig_st_foreach(t, (int (__cdecl *)(...))i_renumber_name, (HashDataType)map);
	}
	return 0;
}
	...
	if(IS_NOT_NULL(t)) {
		for(i = 0; i < t->num; i++) {
			...
			if(IS_NOT_NULL(e->name)) {
				...
				e->back_num   = 0;
				e->back_alloc = 0;
				if(IS_NOT_NULL(e->back_refs)) SAlloc::F(e->back_refs);
...
static int name_add(regex_t* reg, uchar * name, uchar * name_end, int backref, ScanEnv* env)
{
	...
	if(IS_NULL(e)) {
		...
		if(IS_NULL(t)) {
			...
			t = (NameTable*)SAlloc::M(sizeof(NameTable));
			CHECK_NULL_RETURN_MEMERR(t);
			t->e     = NULL;
			t->alloc = 0;
			t->num   = 0;

			t->e = (NameEntry*)SAlloc::M(sizeof(NameEntry) * alloc);
			if(IS_NULL(t->e)) {
				SAlloc::F(t);
				return ONIGERR_MEMORY;
			}
			...
			int i;
			alloc = t->alloc * 2;
			t->e = (NameEntry*)SAlloc::R(t->e, sizeof(NameEntry) * alloc);
...
clear:
	...
	if(e->back_num == 1) {
		...
		if(e->back_num == 2) {
			...
			e->back_refs = (int*)SAlloc::M(sizeof(int) * alloc);
			...
			if(e->back_num > e->back_alloc) {
				...
				e->back_refs = (int*)SAlloc::R(e->back_refs, sizeof(int) * alloc);
				CHECK_NULL_RETURN_MEMERR(e->back_refs);
				e->back_alloc = alloc;
			}
...
int onig_name_to_group_numbers(regex_t* reg, const uchar * name, const uchar * name_end, int** nums)
{
	...
	switch(e->back_num) {
		case 0: break;
		case 1: *nums = &(e->back_ref1); break;
		default: *nums = e->back_refs; break;
	}
	return e->back_num;
}

static int name_to_group_numbers(ScanEnv* env, const uchar * name, const uchar * name_end, int** nums)
{
	regex_t* reg = env->reg;
	NameEntry* e = name_find(reg, name, name_end);
	if(IS_NULL(e)) {
...
static int make_callout_func_list(CalloutNameListType** rs, int init_size)
{
	...
	*rs = 0;
	s = (CalloutNameListType*)SAlloc::M(sizeof(*s));
	if(IS_NULL(s)) return ONIGERR_MEMORY;
	v = (CalloutNameListEntry*)SAlloc::M(sizeof(CalloutNameListEntry) * init_size);
	if(IS_NULL(v)) {
		SAlloc::F(s);
		return ONIGERR_MEMORY;
	}
...
static int callout_func_list_add(CalloutNameListType* s, int* rid)
{
	if(s->n >= s->alloc) {
		...
		CalloutNameListEntry* nv = (CalloutNameListEntry*)SAlloc::R(s->v, sizeof(CalloutNameListEntry) * new_size);
		if(IS_NULL(nv)) 
			return ONIGERR_MEMORY;
		s->alloc = new_size;
		s->v = nv;
...
static int i_free_callout_name_entry(st_callout_name_key* key, CalloutNameEntry* e, void * arg ARG_UNUSED)
{
	...
	/*SAlloc::F(key->s); */ /* is same as e->name */
	SAlloc::F(key);
	SAlloc::F(e);
	return ST_DELETE;
}
...
{
	if(IS_NOT_NULL(t)) {
		onig_st_foreach(t, (int (__cdecl *)(...))i_free_callout_name_entry, 0);
	}
	return 0;
}
    ...
    int is_not_single, uchar * name, uchar * name_end)
{
	...
	*rentry = 0;
	...
	if(IS_NULL(e)) {
#ifdef USE_ST_LIBRARY
		if(IS_NULL(t)) {
			t = (CalloutNameTable *)onig_st_init_callout_name_table_with_size(INIT_NAMES_ALLOC_NUM);
			CHECK_NULL_RETURN_MEMERR(t);
		...
		r = st_insert_callout_name_table(t, enc, is_not_single, e->name, (e->name + (name_end - name)), (HashDataType)e);
		if(r < 0) return r;
#else
		int alloc;
		if(IS_NULL(t)) {
			alloc = INIT_NAMES_ALLOC_NUM;
			t = (CalloutNameTable*)SAlloc::M(sizeof(CalloutNameTable));
			CHECK_NULL_RETURN_MEMERR(t);
			t->e     = NULL;
			t->alloc = 0;
			t->num   = 0;
...
static int is_allowed_callout_name(OnigEncoding enc, uchar * name, uchar * name_end)
{
	uchar * p;
	OnigCodePoint c;
...
static int is_allowed_callout_tag_name(OnigEncoding enc, uchar * name, uchar * name_end)
{
	uchar * p;
	OnigCodePoint c;
...
static int get_callout_opt_arg_num_by_name_id(int name_id) { return GlobalCalloutNameList->v[name_id].opt_arg_num; }
...
static int setup_ext_callout_list_values(regex_t* reg)
{
	...
	if(IS_NOT_NULL(ext->tag_table)) {
		onig_st_foreach((CalloutTagTable*)ext->tag_table, (int (__cdecl *)(...))i_callout_callout_list_set, (st_data_t)ext);
	}
...
{
	if(IS_NOT_NULL(t)) {
		onig_st_foreach(t, (int (__cdecl *)(...))i_free_callout_tag_entry, 0);
	}
	return 0;
}
...
static CalloutTagVal callout_tag_find(CalloutTagTable* t, const uchar * name, const uchar * name_end)
{
	CalloutTagVal e;
...
static int callout_tag_table_new(CalloutTagTable** rt)
{
	CalloutTagTable * t;
	*rt = 0;
	t = (CalloutTagTable *)onig_st_init_strend_table_with_size(INIT_TAG_NAMES_ALLOC_NUM);
	CHECK_NULL_RETURN_MEMERR(t);
	*rt = t;
	return ONIG_NORMAL;
}

static int callout_tag_entry_raw(ScanEnv* env, CalloutTagTable* t, uchar * name, uchar * name_end, CalloutTagVal entry_val)
{
	int r;
	CalloutTagVal val;
	if(name_end - name <= 0)
...
static void scan_env_clear(ScanEnv* env)
{
	...
	memzero(env->mem_env_static, sizeof(env->mem_env_static));
	env->parse_depth      = 0;
#ifdef ONIG_DEBUG_PARSE
	...
	if(need >= SCANENV_MEMENV_SIZE) {
		if(env->mem_alloc <= need) {
			if(IS_NULL(env->mem_env_dynamic)) {
				...
				p = (MemEnv*)SAlloc::R(env->mem_env_dynamic, sizeof(MemEnv) * alloc);
				CHECK_NULL_RETURN_MEMERR(p);
			}
	...
	switch(NODE_TYPE(node)) {
		...
		case NODE_BACKREF:
		    if(IS_NOT_NULL(BACKREF_(node)->back_dynamic))
			    SAlloc::F(BACKREF_(node)->back_dynamic);
		    break;
...
{
	Node * node = (Node*)SAlloc::M(sizeof(Node));
    ...
    ScanEnv* env)
{
	...
	if(back_num <= NODE_BACKREFS_SIZE) {
		...
		int* p = (int*)SAlloc::M(sizeof(int) * back_num);
		if(IS_NULL(p)) {
			onig_node_free(node);
			return NULL;
		}
			...
			for(j = 0; j < list[i].u.arg.passed_num; j++) {
				if(list[i].u.arg.types[j] == ONIG_TYPE_STRING) {
					if(IS_NOT_NULL(list[i].u.arg.vals[j].s.start))
						SAlloc::F(list[i].u.arg.vals[j].s.start);
				}
			}
		}
...
#define INIT_CALLOUT_LIST_NUM  3
	...
	CalloutListEntry* e;
	RegexExt * ext = onig_get_regex_ext(env->reg);
	CHECK_NULL_RETURN_MEMERR(ext);
	if(IS_NULL(ext->callout_list)) {
		list = (CalloutListEntry *)SAlloc::M(sizeof(*list) * INIT_CALLOUT_LIST_NUM);
	...
	if(num > ext->callout_list_alloc) {
		...
		list = (CalloutListEntry *)SAlloc::R(ext->callout_list, sizeof(CalloutListEntry) * alloc);
		CHECK_NULL_RETURN_MEMERR(list);
		ext->callout_list       = list;
...
static int FASTCALL node_set_str(Node * node, const uchar * s, const uchar * end)
{
	int r;
...
static Node* node_new_str_with_options(const uchar * s, const uchar * end, OnigOptionType options)
{
...
static int scan_hexadecimal_number(uchar ** src, uchar * end, int minlen, int maxlen, OnigEncoding enc, OnigCodePoint* rcode)
{
	OnigCodePoint code = 0;
	OnigCodePoint c;
	uint val;
	int n = 0;
	uchar * p = *src;
...
static int scan_octal_number(uchar ** src, uchar * end, int minlen, int maxlen, OnigEncoding enc, OnigCodePoint* rcode)
{
	OnigCodePoint code = 0;
	OnigCodePoint c;
	uint val;
...
static int scan_number_of_base(uchar ** src, uchar * end, int minlen, OnigEncoding enc, OnigCodePoint* rcode, int base)
{
	int r;
	if(base == 16)
...
static int check_code_point_sequence_cc(uchar * p, uchar * end, int base, OnigEncoding enc, int state)
{
	int r;
	int n = 0;
	int end_digit = FALSE;
...
static int check_code_point_sequence(uchar * p, uchar * end, int base, OnigEncoding enc)
{
	int r;
	int n = 0;
	int end_digit = FALSE;
...
static int get_next_code_point(uchar ** src, uchar * end, int base, OnigEncoding enc, int in_cc, OnigCodePoint* rcode)
{
	int r;
	OnigCodePoint c;
	uchar * p = *src;
...
#define BB_WRITE_CODE_POINT(bbuf, pos, code) BB_WRITE(bbuf, pos, &(code), SIZE_CODE_POINT)
...
static int and_code_range_buf(BBuf* bbuf1, int not1, BBuf* bbuf2, int not2, BBuf** pbuf)
{
	int r;
...
static OnigCodePoint conv_backslash_value(OnigCodePoint c, ScanEnv* env)
{
...
static void ptoken_init(PToken* tok)
{
	tok->code_point_continue = 0;
}

static int fetch_interval(uchar ** src, uchar * end, PToken* tok, ScanEnv* env)
{
...
/* \M-, \C-, \c, or \... */
static int fetch_escaped_value_raw(uchar ** src, uchar * end, ScanEnv* env, OnigCodePoint* val)
{
	int v;
	OnigCodePoint c;
	OnigEncoding enc = env->enc;
...
static int fetch_escaped_value(uchar ** src, uchar * end, ScanEnv* env, OnigCodePoint* val)
{
	int len;
...
/*
   ...
   \k<+num+n>, \k<+num-n>
 */
static int fetch_name_with_level(OnigCodePoint start_code, uchar ** src, uchar * end,
...
static uchar * find_str_position(OnigCodePoint s[], int n, uchar * from, uchar * to,
...
static int str_exist_check_with_esc(OnigCodePoint s[], int n, uchar * from, uchar * to,
...
static int fetch_token_cc(PToken* tok, uchar ** src, uchar * end, ScanEnv* env, int state)
{
	int r;
	OnigCodePoint code;
	OnigCodePoint c, c2;
...
static int prs_posix_bracket(CClassNode* cc, uchar ** src, uchar * end, ScanEnv* env)
{
...
static int cc_cprop_next(CClassNode* cc, OnigCodePoint* pcode, CVAL* val, CSTATE* state,
    ScanEnv* env)
{
	int r;
...
static long prs_long(OnigEncoding enc, uchar * s, uchar * end, int sign_on, long max, long* rl)
{
	long v;
	long d;
	int flag;
...
static void clear_callout_args(int n, uint types[], OnigValue vals[])
{
	int i;
...
static int prs_callout_args(int skip_mode, int cterm, uchar ** src, uchar * end, int max_arg_num, uint types[], OnigValue vals[], ScanEnv* env)
{
			...
			case '(':
			    ...
			    if(IS_SYNTAX_OP2(env->syntax, ONIG_SYN_OP2_QMARK_LPAREN_IF_ELSE)) {
					...
					|| c == '-' || c == '+' || c == '<' || c == '\'') {
						...
						ONIG_SYN_OP2_QMARK_BRACE_CALLOUT_CONTENTS)) {
						    if(!PEND && PPEEK_IS('{')) {
							    /* condition part is callouts of contents:
							       (?(?{...})THEN|ELSE) */
					    ...
					    case 'y': /* y{g}, y{w} */
					{
						...
						switch(c) {
							case 'g':
							    ...
							    OPTION_NEGATE(option, ONIG_OPTION_TEXT_SEGMENT_EXTENDED_GRAPHEME_CLUSTER,
								FALSE);
...
static int assign_quantifier_body(Node* qnode, Node* target, int group, ScanEnv* env)
{
	QuantNode* qn;
	...
	if(to_len == 1) {
		...
		if(onig_is_code_in_cc(enc, from, cc)
			...
			for(i = 0; i < to_len; i++) {
				...
				CClassNode* cs_cc;
				index = 0;
				if(ONIGENC_IS_UNICODE_ENCODING(enc) && (index = onigenc_unicode_fold1_key(&to[i])) >= 0) {
