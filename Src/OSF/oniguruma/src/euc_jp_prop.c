...
/* Command-line: gperf -pt -T -L ANSI-C -N onigenc_euc_jp_lookup_property_name --output-file gperf1.tmp
   euc_jp_prop.gperf  */
...
#endif
#endif
static uint hash(const char * str, size_t len)
{
	static uchar asso_values[] = {
	...
	return (uint)len + asso_values[(uchar)str[2]] + asso_values[(uchar)str[0]];
}

struct PropertyNameCtype * onigenc_euc_jp_lookup_property_name(const char * str, size_t len)                           
{
