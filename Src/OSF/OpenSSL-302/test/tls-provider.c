...
int tls_provider_init(const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * in, const OSSL_DISPATCH ** out, void ** provctx);
...
static int tls_prov_get_capabilities(void * provctx, const char * capability, OSSL_CALLBACK * cb, void * arg)
{
	int ret;
	int i;
	...
	if(strcmp(capability, "TLS-GROUP") != 0) {
		return 0; /* We don't support this capability */
	}
	...
	for(i = 0; i < NUM_DUMMY_GROUPS; i++) {
		OSSL_PARAM dummygroup[SIZEOFARRAY(xor_group_params)];
		memcpy(dummygroup, xor_group_params, sizeof(xor_group_params));
		/* Give the dummy group a unique name */
		if(dummy_group_names[i] == NULL) {
			dummy_group_names[i] = (char*)OPENSSL_zalloc(dummy_name_max_size);
			if(dummy_group_names[i] == NULL)
				return 0;
			BIO_snprintf(dummy_group_names[i], dummy_name_max_size, "%s%d", dummy_base, i);
		}
...
{
	PROV_XOR_CTX * pxorctx = (PROV_XOR_CTX *)OPENSSL_zalloc(sizeof(PROV_XOR_CTX));
	if(pxorctx == NULL)
		return NULL;
...
static int xor_init(void * vpxorctx, void * vkey, ossl_unused const OSSL_PARAM params[])
{
	PROV_XOR_CTX * pxorctx = (PROV_XOR_CTX*)vpxorctx;
	if(pxorctx == NULL || vkey == NULL)
		return 0;
	pxorctx->key = (XORKEY *)vkey;
	return 1;
}
...
static int xor_set_peer(void * vpxorctx, void * vpeerkey)
{
	...
	if(pxorctx == NULL || vpeerkey == NULL)
		return 0;
	pxorctx->peerkey = (XORKEY *)vpeerkey;
	return 1;
}

static int xor_derive(void * vpxorctx, unsigned char * secret, size_t * secretlen, size_t outlen)
{
	PROV_XOR_CTX * pxorctx = (PROV_XOR_CTX*)vpxorctx;
	int i;
...
static void * xor_dupctx(void * vpxorctx)
{
	...
	PROV_XOR_CTX * dstctx = (PROV_XOR_CTX *)OPENSSL_zalloc(sizeof(*srcctx));
	if(dstctx == NULL)
		return NULL;
	*dstctx = *srcctx;
	return dstctx;
    ...
    unsigned char * ss, size_t * sslen)
{
	...
	if(genctx == NULL)
		goto end;
	ourkey = (XORKEY *)xor_gen(genctx, NULL, NULL);
	if(ourkey == NULL)
		goto end;
	...
	*ctlen = XOR_KEY_SIZE;
	...
	if(derivectx == NULL || !xor_init(derivectx, ourkey, NULL) || !xor_set_peer(derivectx, pxorctx->key) || !xor_derive(derivectx, ss, sslen, XOR_KEY_SIZE))
		goto end;
	rv = 1;
end:
    ...
    const unsigned char * ct, size_t ctlen)
{
	...
	if(ctlen != XOR_KEY_SIZE)
		return 0;
	peerkey = (XORKEY *)xor_newdata(pxorctx->provctx);
	if(peerkey == NULL)
		goto end;
	...
	/* Derive ss via KEX */
	derivectx = xor_newctx(pxorctx->provctx);
	if(derivectx == NULL || !xor_init(derivectx, pxorctx->key, NULL) || !xor_set_peer(derivectx, peerkey) || !xor_derive(derivectx, ss, sslen, XOR_KEY_SIZE))
		goto end;
	rv = 1;
end:
...
{
	const XORKEY * key = (const XORKEY *)vkey;
	int ok = 0;
	if(key) {
		ok = 1;
...
{
	XORKEY * tokey = (XORKEY *)xor_newdata(NULL);
	const XORKEY * fromkey = (const XORKEY *)vfromkey;
	int ok = 0;
...
{
	XORKEY * key = (XORKEY *)vkey;
	OSSL_PARAM * p;
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_BITS)) != NULL && !OSSL_PARAM_set_int(p, XOR_KEY_SIZE))
		return 0;

	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_SECURITY_BITS)) != NULL && !OSSL_PARAM_set_int(p, xor_group.secbits))
		return 0;
...
static void * xor_gen_init(void * provctx, int selection, const OSSL_PARAM params[])
{
	struct xor_gen_ctx * gctx = NULL;
	if((selection & (OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS)) == 0)
		return NULL;
	if((gctx = (struct xor_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL)
...
{
	struct xor_gen_ctx * gctx = (struct xor_gen_ctx *)genctx;
	const OSSL_PARAM * p;
	if(gctx == NULL)
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_GROUP_NAME);
	if(p) {
		if(p->data_type != OSSL_PARAM_UTF8_STRING || (strcmp((const char *)p->data, XORGROUP_NAME_INTERNAL) != 0 &&  
			strcmp((const char *)p->data, XORKEMGROUP_NAME_INTERNAL) != 0))
			return 0;
	}
	return 1;
}

static const OSSL_PARAM * xor_gen_settable_params(ossl_unused void * genctx, ossl_unused void * provctx)
{
...
{
	struct xor_gen_ctx * gctx = (struct xor_gen_ctx *)genctx;
	XORKEY * key = (XORKEY *)OPENSSL_zalloc(sizeof(*key));
	size_t i;
	if(!key)
		return NULL;
...
static int xor_import(void * vkey, int select, const OSSL_PARAM params[])
{
	...
	if((param_priv_key != NULL && !OSSL_PARAM_get_octet_string(param_priv_key, &pprivkey, sizeof(privkey), &priv_len)) || 
		(param_pub_key != NULL && !OSSL_PARAM_get_octet_string(param_pub_key, &ppubkey, sizeof(pubkey), &pub_len)))
		goto err;
	if(priv_len > 0) {
...
static int xor_export(void * vkey, int select, OSSL_CALLBACK * param_cb, void * cbarg)
{
	XORKEY * key = (XORKEY *)vkey;
	OSSL_PARAM params[3], * p = params;
	if(key == NULL || (select & OSSL_KEYMGMT_SELECT_KEYPAIR) == 0)
		return 0;
	*p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PRIV_KEY, key->privkey,
		sizeof(key->privkey));
	*p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PUB_KEY, key->pubkey, sizeof(key->pubkey));
