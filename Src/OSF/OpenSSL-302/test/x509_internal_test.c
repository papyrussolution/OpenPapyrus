 ...
 *
 ***/
#include "internal/crypto/ext_dat.h"
#include "internal/crypto/standard_exts.h"

static int test_standard_exts(void)
{
	size_t i;
	int prev = -1, good = 1;
	const X509V3_EXT_METHOD ** tmp = standard_exts;
	for(i = 0; i < SIZEOFARRAY(standard_exts); i++, tmp++) {
		if((*tmp)->ext_nid < prev)
			good = 0;
		prev = (*tmp)->ext_nid;
	}
...
int setup_tests()
{
	...
	ADD_ALL_TESTS(test_a2i_ipaddress, SIZEOFARRAY(a2i_ipaddress_tests));
	return 1;
}
