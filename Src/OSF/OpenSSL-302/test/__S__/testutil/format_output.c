    ...
    const BIGNUM * bn1, const BIGNUM * bn2)
{
	...
	if(len > MEM_BUFFER_SIZE && (bufp = (unsigned char *)OPENSSL_malloc(len * 2)) == NULL) {
		bufp = buffer;
		len = MEM_BUFFER_SIZE;
 ...
 */
void test_fail_memory_message(const char * prefix, const char * file, int line, const char * type, const char * left, const char * right,
    const char * op, const unsigned char * m1, size_t l1, const unsigned char * m2, size_t l2)
{
