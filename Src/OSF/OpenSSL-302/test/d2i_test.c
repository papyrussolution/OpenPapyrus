...
static int test_bad_asn1(void)
{
	...
	int derlen;
	int len;
	BIO * bio = BIO_new_file(test_file, "r");
	if(!TEST_ptr(bio))
		return 0;
...
int setup_tests()
{
	...
	if(!TEST_ptr(test_type_name = test_get_argument(0)) || !TEST_ptr(expected_error_string = test_get_argument(1)) || !TEST_ptr(test_file = test_get_argument(2)))
		return 0;
	...
	for(i = 0; i < SIZEOFARRAY(expected_errors); i++) {
