...
static uchar pbe_salt[] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, };
static const int pbe_iter = 1000;
static uchar pbe_plaintext[] = {
...
static const uchar pbe_ciphertext_rc4_md5[] = {
...
static const uchar pbe_ciphertext_des_sha1[] = {
...
static int test_pkcs5_pbe(const EVP_CIPHER * cipher, const EVP_MD * md, const uchar * exp, const int exp_len)
{
	...
	if(!TEST_true(PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher), pbe_iter, pbe_salt, sizeof(pbe_salt))))
		goto err;
	if(!TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter, cipher, md, 1)))
		goto err;
	if(!TEST_true(EVP_CipherUpdate(ctx, out, &i, pbe_plaintext, sizeof(pbe_plaintext))))
		goto err;
	outlen = i;
