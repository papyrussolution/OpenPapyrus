...
#define NUM_TESTS 34
static uchar ecb_data[NUM_TESTS][8] = {
...
static uchar plain_data[NUM_TESTS][8] = {
	{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
	{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
...
static uchar cipher_data[NUM_TESTS][8] = {
...
static uchar cbc_key[16] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xf0, 0xe1, 0xd2, 0xc3, 0xb4, 0xa5, 0x96, 0x87 };
static uchar cbc_iv[8] = { 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10 };
static char cbc_data[40] = "7654321 Now is the time for ";
static uchar cbc_ok[32] = {
...
static uchar key_test[KEY_TEST_NUM] = {
...
static uchar key_data[8] = { 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10 };

static uchar key_out[KEY_TEST_NUM][8] = {
...
static int test_bf_ecb_raw(int n)
{
	...
	BF_KEY key;
	BF_LONG data[2];
	BF_set_key(&key, strlen(bf_key[n]), (uchar *)bf_key[n]);
	data[0] = bf_plain[n][0];
	data[1] = bf_plain[n][1];
	BF_encrypt(data, &key);
...
{
	uchar cbc_in[40], cbc_out[40], iv[8];
	int ret = 1;
	BF_KEY key;
	BF_LONG len = strlen(cbc_data) + 1;
	BF_set_key(&key, 16, cbc_key);
	memzero(cbc_in, sizeof(cbc_in));
	memzero(cbc_out, sizeof(cbc_out));
	memcpy(iv, cbc_iv, sizeof(iv));
	BF_cbc_encrypt((uchar *)cbc_data, cbc_out, len, &key, iv, BF_ENCRYPT);
...
static int test_bf_cfb64(void)
{
	...
	memcpy(iv, cbc_iv, 8);
	n = 0;
	BF_cfb64_encrypt((uchar *)cbc_data, cbc_out, (long)13, &key, iv, &n, BF_ENCRYPT);
	BF_cfb64_encrypt((uchar *)&(cbc_data[13]), &(cbc_out[13]), len - 13, &key, iv, &n, BF_ENCRYPT);
...
static int test_bf_ofb64(void)
{
	...
	memcpy(iv, cbc_iv, 8);
	n = 0;
	BF_ofb64_encrypt((uchar *)cbc_data, cbc_out, (long)13, &key, iv, &n);
	BF_ofb64_encrypt((uchar *)&(cbc_data[13]), &(cbc_out[13]), len - 13, &key, iv, &n);
...
{
	static const OPTIONS test_options[] = { OPT_TEST_OPTIONS_DEFAULT_USAGE, { "print", OPT_PRINT, '-', "Output test tables instead of running tests"}, { NULL } };
	return test_options;
}
