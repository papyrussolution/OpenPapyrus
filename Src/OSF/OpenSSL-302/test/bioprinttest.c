...
static int dofptest(int test, int sub, double val, const char * width, int prec)
{
	...
	int ret = 1, i;
	for(i = 0; i < SIZEOFARRAY(fspecs); i++) {
...
{
	static const OPTIONS options[] = { OPT_TEST_OPTIONS_DEFAULT_USAGE, { "expected", OPT_PRINT, '-', "Output values" }, { NULL } };
	return options;
}
...
int setup_tests()
{
	...
	ADD_TEST(test_big);
	ADD_ALL_TESTS(test_fp, SIZEOFARRAY(pw_params));
	ADD_ALL_TESTS(test_zu, SIZEOFARRAY(zu_data));
	ADD_ALL_TESTS(test_j, SIZEOFARRAY(jf_data));
	return 1;
}
// 
// Replace testutil output routines.  We do this to eliminate possible sources of BIO error
// 
BIO * bio_out = NULL;
BIO * bio_err = NULL;
 ...
 */
int test_vprintf_stdout(const char * fmt, va_list ap) { return fprintf(stdout, "%*s# ", tap_level, "") + vfprintf(stdout, fmt, ap); }
int test_vprintf_stderr(const char * fmt, va_list ap) { return slfprintf_stderr("%*s# ", tap_level, "") + vfprintf(stderr, fmt, ap); }
int test_flush_stdout(void) { return fflush(stdout); }
int test_flush_stderr(void) { return fflush(stderr); }
int test_vprintf_tapout(const char * fmt, va_list ap) { return fprintf(stdout, "%*s", tap_level, "") + vfprintf(stdout, fmt, ap); }
int test_vprintf_taperr(const char * fmt, va_list ap) { return slfprintf_stderr("%*s", tap_level, "") + vfprintf(stderr, fmt, ap); }
