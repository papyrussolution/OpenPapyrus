...
static int tls_dump_gets(BIO * bio, char * buf, int size) { return -1; /* We don't support this - not needed anyway */ }
static int tls_dump_puts(BIO * bio, const char * str) { return tls_dump_write(bio, str, strlen(str)); }
...
static int mempacket_test_new(BIO * bio)
{
	...
	if(!TEST_ptr(ctx = (MEMPACKET_TEST_CTX *)OPENSSL_zalloc(sizeof(*ctx))))
		return 0;
...
{
	MEMPACKET_TEST_CTX * ctx = (MEMPACKET_TEST_CTX *)BIO_get_data(bio);
...
int mempacket_test_inject(BIO * bio, const char * in, int inl, int pktnum, int type)
{
	...
	int i, duprec;
	const uchar * inu = (const uchar*)in;
	...
	for(i = 0; i < (duprec ? 3 : 1); i++) {
		if(!TEST_ptr(allpkts[i] = (MEMPACKET *)OPENSSL_malloc(sizeof(*thispkt))))
			goto err;
		thispkt = allpkts[i];
		if(!TEST_ptr(thispkt->data = (uchar *)OPENSSL_malloc(inl)))
			goto err;
		/*
	...
	for(i = 0; (looppkt = sk_MEMPACKET_value(ctx->pkts, i)) != NULL; i++) {
		...
		if(looppkt->num > thispkt->num) {
			...
			do {
				...
				if(nextpkt && nextpkt->num == ctx->lastpkt)
					ctx->lastpkt++;
				else
					return inl;
			} while(1);
    ...
    int max_proto_version, SSL_CTX ** sctx, SSL_CTX ** cctx, char * certfile, char * privkeyfile)
{
	...
	if(sctx) {
		if(*sctx)
			serverctx = *sctx;
		else if(!TEST_ptr(serverctx = SSL_CTX_new_ex(libctx, NULL, sm)) || !TEST_true(SSL_CTX_set_options(serverctx,
	...
	/*
	 * There are no usable built-in TLSv1.3 groups if ec and dh are both disabled
	 */
	if(max_proto_version == 0 && (sm == TLS_server_method() || cm == TLS_client_method()))
		max_proto_version = TLS1_2_VERSION;
#endif
	if(serverctx && ((min_proto_version > 0 && !TEST_true(SSL_CTX_set_min_proto_version(serverctx, min_proto_version))) || 
		(max_proto_version > 0 && !TEST_true(SSL_CTX_set_max_proto_version(serverctx, max_proto_version)))))
		goto err;
	if(clientctx && ((min_proto_version > 0 && !TEST_true(SSL_CTX_set_min_proto_version(clientctx, min_proto_version))) || 
		(max_proto_version > 0 && !TEST_true(SSL_CTX_set_max_proto_version(clientctx, max_proto_version)))))
		goto err;
	if(serverctx && certfile && privkeyfile) {
		if(!TEST_int_eq(SSL_CTX_use_certificate_file(serverctx, certfile, SSL_FILETYPE_PEM), 1) || 
			!TEST_int_eq(SSL_CTX_use_PrivateKey_file(serverctx, privkeyfile, SSL_FILETYPE_PEM), 1) || !TEST_int_eq(SSL_CTX_check_private_key(serverctx), 1))
			goto err;
	}
	if(sctx)
		*sctx = serverctx;
 ...
 */
int create_ssl_objects(SSL_CTX * serverctx, SSL_CTX * clientctx, SSL ** sssl, SSL ** cssl, BIO * s_to_c_fbio, BIO * c_to_s_fbio)
{
	SSL * serverssl = NULL, * clientssl = NULL;
	BIO * s_to_c_bio = NULL, * c_to_s_bio = NULL;
	if(*sssl)
	...
	if(SSL_is_dtls(clientssl)) {
		if(!TEST_ptr(s_to_c_bio = BIO_new(bio_s_mempacket_test())) || !TEST_ptr(c_to_s_bio = BIO_new(bio_s_mempacket_test())))
			goto error;
	}
	else {
	...
	if(s_to_c_fbio && !TEST_ptr(s_to_c_bio = BIO_push(s_to_c_fbio, s_to_c_bio)))
		goto error;
	if(c_to_s_fbio && !TEST_ptr(c_to_s_bio = BIO_push(c_to_s_fbio, c_to_s_bio)))
		goto error;
...
/*
 * Create an SSL connection including any post handshake NewSessionTicket messages.
 */
