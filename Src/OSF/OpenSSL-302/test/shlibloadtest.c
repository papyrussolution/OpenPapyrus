...
static int test_lib(void)
{
	...
	switch(test_type) {
		...
		case CRYPTO_FIRST:
		    if(!sd_load(path_crypto, &cryptolib, SD_SHLIB)) {
			    slfprintf_stderr("Failed to load libcrypto\n");
			    goto end;
		    }
		...
		case SSL_FIRST:
		    if(!sd_load(path_ssl, &ssllib, SD_SHLIB)) {
			    slfprintf_stderr("Failed to load libssl\n");
			    goto end;
		    }
	...
	if(test_type != JUST_CRYPTO
		...
		if(!sd_sym(ssllib, "TLS_method", &symbols[0].sym)
			...
			slfprintf_stderr("Failed to load libssl symbols\n");
			goto end;
		}
		...
		if(ctx == NULL) {
			slfprintf_stderr("Failed to create SSL_CTX\n");
			goto end;
		}
		mySSL_CTX_free(ctx);
	}
	...
	if(!sd_sym(cryptolib, "ERR_get_error", &symbols[0].sym)
		...
		slfprintf_stderr("Failed to load libcrypto symbols\n");
		goto end;
	}
	myERR_get_error = (ERR_get_error_t)symbols[0].func;
	if(myERR_get_error() != 0) {
		slfprintf_stderr("Unexpected ERR_get_error() response\n");
		goto end;
	}
	...
	if(myOPENSSL_version_major() != OPENSSL_VERSION_MAJOR || myOPENSSL_version_minor() != OPENSSL_VERSION_MINOR || myOPENSSL_version_patch() != OPENSSL_VERSION_PATCH) {
		slfprintf_stderr("Invalid library version number\n");
		goto end;
	}
	myOPENSSL_atexit = (OPENSSL_atexit_t)symbols[4].func;
	if(!myOPENSSL_atexit(atexit_handler)) {
		slfprintf_stderr("Failed to register atexit handler\n");
		goto end;
	}
...
# ifdef DSO_DLFCN
		...
		if(!sd_sym(cryptolib, "DSO_dsobyaddr", &symbols[0].sym) || !sd_sym(cryptolib, "DSO_free", &symbols[1].sym)) {
			slfprintf_stderr("Unable to load DSO symbols\n");
			goto end;
		}
		...
		myDSO_free = (DSO_free_t)symbols[1].func;
		{
			...
			if(hndl == NULL) {
				slfprintf_stderr("DSO_dsobyaddr() failed\n");
				goto end;
			}
			myDSO_free(hndl);
		}
# endif /* DSO_DLFCN */
	...
	if(!sd_close(cryptolib)) {
		slfprintf_stderr("Failed to close libcrypto\n");
		goto end;
	}
	cryptolib = SD_INIT;
	if(test_type == CRYPTO_FIRST || test_type == SSL_FIRST) {
		if(!sd_close(ssllib)) {
			slfprintf_stderr("Failed to close libssl\n");
			goto end;
		}
		ssllib = SD_INIT;
	}
...
int main(int argc, char * argv[])
{
	const char * p;
	if(argc != 5) {
		slfprintf_stderr("Incorrect number of arguments\n");
		return 1;
	}
	p = argv[1];
	if(strcmp(p, "-crypto_first") == 0) {
		...
		slfprintf_stderr("Unrecognised argument\n");
		return 1;
	}
	path_crypto = argv[2];
	path_ssl = argv[3];
	...
	if(path_crypto == NULL || path_ssl == NULL) {
		slfprintf_stderr("Invalid libcrypto/libssl path\n");
		return 1;
	}
#ifdef SD_INIT
	if(!test_lib())
