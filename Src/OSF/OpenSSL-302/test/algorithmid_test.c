...
static int test_spki_aid(X509_PUBKEY * pubkey, const char * filename)
{
	...
	uchar * algid_legacy = NULL;
	int algid_legacy_len = 0;
	static uchar algid_prov[OSSL_MAX_ALGORITHM_ID_SIZE];
	size_t algid_prov_len = 0;
	const OSSL_PARAM * gettable_params = NULL;
	OSSL_PARAM params[] = {
		OSSL_PARAM_octet_string(ALGORITHMID_NAME, &algid_prov, sizeof(algid_prov)),
		OSSL_PARAM_END
	};
	int ret = 0;
	if(!TEST_true(X509_PUBKEY_get0_param(NULL, NULL, NULL, &alg, pubkey)) || !TEST_ptr(pkey = X509_PUBKEY_get0(pubkey)))
		goto end;
	 ...
	 * in this function.
	 */
	if((keydata = evp_pkey_export_to_provider(pkey, NULL, &keymgmt, NULL)) == NULL) {
		TEST_info("The public key found in '%s' doesn't have provider support.  Skipping...", filename);
		ret = 1;
		goto end;
	}

	if(!TEST_true(EVP_KEYMGMT_is_a(keymgmt, name))) {
		TEST_info("The AlgorithmID key type (%s) for the public key found in '%s' doesn't match the key type of the extracted public key.", name, filename);
		ret = 1;
		goto end;
	}
	if(!TEST_ptr(gettable_params = EVP_KEYMGMT_gettable_params(keymgmt)) || !TEST_ptr(OSSL_PARAM_locate_const(gettable_params, ALGORITHMID_NAME))) {
		TEST_info("The %s provider keymgmt appears to lack support for algorithm-id.  Skipping...", name);
		ret = 1;
		goto end;
	}
	algid_prov[0] = '\0';
	...
	if(TEST_mem_eq(algid_legacy, algid_legacy_len, algid_prov, algid_prov_len))
		ret = 1;
end:
...
static int test_x509_sig_aid(X509 * eecert, const char * ee_filename, X509 * cacert, const char * ca_filename)
{
	...
	size_t algid_prov_len = 0;
	const OSSL_PARAM * gettable_params = NULL;
	OSSL_PARAM params[] = { OSSL_PARAM_octet_string("algorithm-id", &algid_prov, sizeof(algid_prov)), OSSL_PARAM_END };
	int ret = 0;
	...
	if(!TEST_true(EVP_PKEY_is_a(pkey, OBJ_nid2sn(pkey_nid)))) {
		TEST_info("The '%s' pubkey can't be used to verify the '%s' signature", ca_filename, ee_filename);
		TEST_info("Signature algorithm is %s (pkey type %s, hash type %s)", OBJ_nid2sn(sig_nid), OBJ_nid2sn(pkey_nid), OBJ_nid2sn(dig_nid));
		TEST_info("Pkey key type is %s", EVP_PKEY_get0_type_name(pkey));
		goto end;
	}
	if(!TEST_int_ge(algid_legacy_len = i2d_X509_ALGOR(alg, &algid_legacy), 0))
		goto end;
	if(!TEST_ptr(mdctx = EVP_MD_CTX_new()) || !TEST_true(EVP_DigestVerifyInit_ex(mdctx, &pctx, OBJ_nid2sn(dig_nid), NULL, NULL, pkey, NULL))) {
		TEST_info("Couldn't initialize a DigestVerify operation with pkey type %s and hash type %s", OBJ_nid2sn(pkey_nid), OBJ_nid2sn(dig_nid));
		goto end;
	}
	if(!TEST_ptr(gettable_params = EVP_PKEY_CTX_gettable_params(pctx)) || !TEST_ptr(OSSL_PARAM_locate_const(gettable_params, ALGORITHMID_NAME))) {
		TEST_info("The %s provider keymgmt appears to lack support for algorithm-id  Skipping...", OBJ_nid2sn(pkey_nid));
		ret = 1;
		goto end;
	}
	algid_prov[0] = '\0';
...
const OPTIONS * test_get_options(void)
{
	static const OPTIONS test_options[] = {
		...
		{ OPT_HELP_STR, 1, '-', "file...\tFile(s) to run tests on.  All files must be PEM encoded.\n" },
		{ NULL }
	};
	return test_options;
}
