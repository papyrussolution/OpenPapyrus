...
#if !defined(OPENSSL_NO_DH) || !defined(OPENSSL_NO_DSA) || !defined(OPENSSL_NO_EC)
static EVP_PKEY * make_key(const char * type, EVP_PKEY * pTemplate, OSSL_PARAM * genparams)
{
	EVP_PKEY * pkey = NULL;
	EVP_PKEY_CTX * ctx = pTemplate ? EVP_PKEY_CTX_new_from_pkey(keyctx, pTemplate, testpropq) : EVP_PKEY_CTX_new_from_name(keyctx, type, testpropq);
	/*
	 * No real need to check the errors other than for the cascade
	 * effect.  |pkey| will simply remain NULL if something goes wrong.
	 */
	(void)(ctx && EVP_PKEY_keygen_init(ctx) > 0 && (genparams == NULL || EVP_PKEY_CTX_set_params(ctx, genparams) > 0) && EVP_PKEY_keygen(ctx, &pkey) > 0);
	EVP_PKEY_CTX_free(ctx);
	return pkey;
}
    ...
    const char * pass, const char * pcipher)
{
	...
	if(!TEST_FL_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, output_type, output_structure, testpropq))
	    ...
	    || (pass && !TEST_FL_true(OSSL_ENCODER_CTX_set_passphrase(ectx, upass, strlen(pass))))
	    || (pcipher && !TEST_FL_true(OSSL_ENCODER_CTX_set_cipher(ectx, pcipher, NULL)))
    ...
    const char * pass)
{
	...
	for(i = 0; i < 3; i++) {
		...
		if(!TEST_FL_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&testpkey,
		    ...
		    selection,
		    testctx, testpropq))
		    || (pass && !OSSL_DECODER_CTX_set_passphrase(dctx, upass, strlen(pass)))
...
static int encode_EVP_PKEY_legacy_PEM(const char * file, const int line, void ** encoded, long * encoded_len,
    void * object, ossl_unused int selection, ossl_unused const char * output_type, ossl_unused const char * output_structure, const char * pass, const char * pcipher)
{
	EVP_PKEY * pkey = (EVP_PKEY *)object;
    ...
    ossl_unused const char * pcipher)
{
	...
	if(!TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) || !TEST_FL_ptr(*encoded = mem_buf->data) || !TEST_FL_long_gt(*encoded_len = mem_buf->length, 0))
		goto end;
	/* Detach the encoded output */
	mem_buf->data = NULL;
	mem_buf->length = 0;
...
{
	OPENSSL_strlcpy(buf, (const char *)userdata, size);
	return strlen((const char *)userdata);
}

static int encode_EVP_PKEY_PVK(const char * file, const int line, void ** encoded, long * encoded_len, void * object, int selection,
    ossl_unused const char * output_type, ossl_unused const char * output_structure, const char * pass, ossl_unused const char * pcipher)
{
	EVP_PKEY * pkey = (EVP_PKEY *)object;
	BIO * mem_ser = NULL;
	BUF_MEM * mem_buf = NULL;
...
static int test_text(const char * file, const int line, const void * data1, size_t data1_len, const void * data2, size_t data2_len)
{
	return TEST_FL_strn2_eq((const char *)data1, data1_len, (const char *)data2, data2_len);
}

static int test_mem(const char * file, const int line, const void * data1, size_t data1_len, const void * data2, size_t data2_len)
{
...
static void collect_name(const char * name, void * arg)
{
	...
	new_namelist = (char *)OPENSSL_realloc(*namelist, space);
...
{
	test_output_memory(label, (const unsigned char *)data, data_len);
}

static void dump_pem(const char * label, const void * data, size_t data_len)
{
	test_output_string(label, (const char *)data, data_len - 1);
}

static int check_unprotected_PKCS8_DER(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	const unsigned char * datap = (const unsigned char *)data;
...
static int check_unprotected_PKCS8_PEM(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	static const char expected_pem_header[] = "-----BEGIN " PEM_STRING_PKCS8INF "-----";
	return TEST_FL_strn_eq((const char *)data, expected_pem_header, sizeof(expected_pem_header) - 1);
}

static int test_unprotected_via_PEM(const char * type, EVP_PKEY * key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
		"PEM", "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_text, check_unprotected_PKCS8_PEM, dump_pem, 0);
}
...
static int check_params_PEM(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	...
	return TEST_FL_int_gt(BIO_snprintf(expected_pem_header, sizeof(expected_pem_header), "-----BEGIN %s PARAMETERS-----", type), 0) && 
		TEST_FL_strn_eq((const char *)data, expected_pem_header, strlen(expected_pem_header));
}

static int test_params_via_DER(const char * type, EVP_PKEY * key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "DER", "type-specific", NULL, NULL,
		encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_mem, check_params_DER, dump_der, FLAG_DECODE_WITH_TYPE);
}

static int test_params_via_PEM(const char * type, EVP_PKEY * key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PEM", "type-specific", NULL, NULL,
		encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_text, check_params_PEM, dump_pem, 0);
}
...
{
	static char expected_pem_header[80];
	return TEST_FL_int_gt(BIO_snprintf(expected_pem_header, sizeof(expected_pem_header), "-----BEGIN %s PRIVATE KEY-----", type), 0) && 
		TEST_FL_strn_eq((const char *)data, expected_pem_header, strlen(expected_pem_header));
}
...
static int test_unprotected_via_legacy_PEM(const char * type, EVP_PKEY * key)
{
	...
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		   "PEM", "type-specific", NULL, NULL, encode_EVP_PKEY_legacy_PEM, decode_EVP_PKEY_prov, test_text, check_unprotected_legacy_PEM, dump_pem, 0);
}
...
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		   "MSBLOB", NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB, decode_EVP_PKEY_prov, test_mem, check_MSBLOB, dump_der, 0);
}
...
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		   "PVK", NULL, NULL, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov, test_mem, check_PVK, dump_der, 0);
}

static const char * pass_cipher = "AES-256-CBC";
static const char * pass = "the holy handgrenade of antioch";

static int check_protected_PKCS8_DER(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	const unsigned char * datap = (const unsigned char *)data;
...
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		"DER", "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_mem, check_protected_PKCS8_DER, dump_der, 0);
}

static int check_protected_PKCS8_PEM(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
...
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		"PEM", "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_text, check_protected_PKCS8_PEM, dump_pem, 0);
}

static int check_protected_legacy_PEM(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	static char expected_pem_header[80];
	return TEST_FL_int_gt(BIO_snprintf(expected_pem_header, sizeof(expected_pem_header), "-----BEGIN %s PRIVATE KEY-----", type), 0) && 
		TEST_FL_strn_eq((const char *)data, expected_pem_header, strlen(expected_pem_header)) && TEST_FL_ptr(strstr((const char *)data, "\nDEK-Info: "));
}
...
static int test_protected_via_legacy_PEM(const char * type, EVP_PKEY * key)
{
	if(!default_libctx || is_fips)
		return TEST_skip("Test not available if using a non-default library context or FIPS provider");
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
	   "PEM", "type-specific", pass, pass_cipher, encode_EVP_PKEY_legacy_PEM, decode_EVP_PKEY_prov, test_text, check_protected_legacy_PEM, dump_pem, 0);
}
...
static int test_protected_via_PVK(const char * type, EVP_PKEY * key)
{
	...
	ret = test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		"PVK", NULL, pass, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov, test_mem, check_PVK, dump_der, 0);
...
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_PUBLIC_KEY|OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
		"DER", "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_mem, check_public_DER, dump_der, 0);
}
...
static int check_public_PEM(const char * file, const int line, const char * type, const void * data, size_t data_len)
{
	...
	return
		TEST_FL_strn_eq((const char *)data, expected_pem_header, sizeof(expected_pem_header) - 1);
}

static int test_public_via_PEM(const char * type, EVP_PKEY * key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_PUBLIC_KEY|OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
		"PEM", "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_text, check_public_PEM, dump_pem, 0);
}
...
static int test_public_via_MSBLOB(const char * type, EVP_PKEY * key)
{
	return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_PUBLIC_KEY|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
		"MSBLOB", NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB, decode_EVP_PKEY_prov, test_mem, check_public_MSBLOB, dump_der, 0);
}

#define KEYS(KEYTYPE) static EVP_PKEY *key_ ## KEYTYPE = NULL
#define MAKE_KEYS(KEYTYPE, KEYTYPEstr, params) ok = ok && TEST_ptr(key_ ## KEYTYPE = make_key(KEYTYPEstr, NULL, params))
