...
static const uchar kExampleDHPrivateKeyDER[] = {
...
static const uchar kExampleECKey2DER[] = {
...
static const uchar kExampleECXKey2DER[] = {
...
static int test_rsa_tofrom_data_select(void)
{
	int ret;
	EVP_PKEY * key = NULL;
	const uchar * pdata = kExampleRSAKeyDER;
...
static int do_check_bn(OSSL_PARAM params[], const char * key,
    const uchar * expected, size_t expected_len)
{
	OSSL_PARAM * p;
	BIGNUM * bn = NULL;
	uchar buffer[256 + 1];
...
static int test_pkey_todata_null(void)
{
	...
	EVP_PKEY * pkey = NULL;
	int ret = 0;
	const uchar * pdata = keydata[0].kder;
...
int setup_tests()
{
	...
	ADD_ALL_TESTS(test_d2i_AutoPrivateKey_ex, SIZEOFARRAY(keydata));
#ifndef OPENSSL_NO_EC
...
#endif
	ADD_ALL_TESTS(test_PEM_read_bio_negative, SIZEOFARRAY(keydata));
