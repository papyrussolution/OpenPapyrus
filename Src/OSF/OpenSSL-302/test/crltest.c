...
static const char ** unknown_critical_crls[] = { kUnknownCriticalCRL, kUnknownCriticalCRL2 };
 ...
 */
static int verify(X509 * leaf, X509 * root, STACK_OF(X509_CRL) * crls, unsigned long flags)
{
	...
	if(!TEST_ptr(ctx) || !TEST_ptr(store) || !TEST_ptr(param) || !TEST_ptr(roots))
		goto err;
	/* Create a stack; upref the cert because we free it below. */
	X509_up_ref(root);
	if(!TEST_true(sk_X509_push(roots, root)) || !TEST_true(X509_STORE_CTX_init(ctx, store, leaf, NULL)))
		goto err;
	...
	param = NULL;
	ERR_clear_error();
	status = X509_verify_cert(ctx) == 1 ? X509_V_OK : X509_STORE_CTX_get_error(ctx);
err:
...
static int test_basic_crl(void)
{
	...
	int r = TEST_ptr(basic_crl)
	    && TEST_ptr(revoked_crl)
	    && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(basic_crl, NULL), X509_V_FLAG_CRL_CHECK), X509_V_OK)
	    && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(basic_crl, revoked_crl), X509_V_FLAG_CRL_CHECK), X509_V_ERR_CERT_REVOKED);
...
{
	return TEST_int_eq(verify(test_leaf, test_root, NULL, X509_V_FLAG_CRL_CHECK), X509_V_ERR_UNABLE_TO_GET_CRL);
}

static int test_bad_issuer_crl(void)
{
	X509_CRL * bad_issuer_crl = CRL_from_strings(kBadIssuerCRL);
	int r = TEST_ptr(bad_issuer_crl) && 
		TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(bad_issuer_crl, NULL), X509_V_FLAG_CRL_CHECK), X509_V_ERR_UNABLE_TO_GET_CRL);
...
static int test_known_critical_crl(void)
{
	...
	int r = TEST_ptr(known_critical_crl)
	    && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(known_critical_crl, NULL), X509_V_FLAG_CRL_CHECK), X509_V_OK);
...
static int test_unknown_critical_crl(int n)
{
	...
	int r = TEST_ptr(unknown_critical_crl)
	    && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(unknown_critical_crl, NULL), X509_V_FLAG_CRL_CHECK), X509_V_ERR_UNHANDLED_CRITICAL_CRL_EXTENSION);
...
int setup_tests()
{
	if(!TEST_ptr(test_root = X509_from_strings(kCRLTestRoot)) || !TEST_ptr(test_leaf = X509_from_strings(kCRLTestLeaf)))
		return 0;
	ADD_TEST(test_no_crl);
	ADD_TEST(test_basic_crl);
	ADD_TEST(test_bad_issuer_crl);
	ADD_TEST(test_known_critical_crl);
	ADD_ALL_TESTS(test_unknown_critical_crl, SIZEOFARRAY(unknown_critical_crls));
