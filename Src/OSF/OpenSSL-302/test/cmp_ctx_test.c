...
#define X509_new() X509_dup(test_cert) // Avoid using X509_new() via the generic macros below
...
static OSSL_CMP_CTX_TEST_FIXTURE * set_up(const char * const test_case_name)
{
	...
	if(!TEST_ptr(fixture = (OSSL_CMP_CTX_TEST_FIXTURE*)OPENSSL_zalloc(sizeof(*fixture))))
		return NULL;
...
static BIO * test_http_cb(BIO * bio, void * arg, int use_ssl, int detail) { return NULL; }
static OSSL_CMP_MSG * test_transfer_cb(OSSL_CMP_CTX * ctx, const OSSL_CMP_MSG * req) { return NULL; }
static int test_certConf_cb(OSSL_CMP_CTX * ctx, X509 * cert, int fail_info, const char ** txt) { return 0; }
...
#define DEFINE_SET_GET_TEST(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE) \
	DEFINE_SET_GET_BASE_TEST(OSSL_CMP ## _ ## CTX, set ## N, get ## M, DUP, FIELD, TYPE *, NULL, IS_0, TYPE ## _new(), TYPE ## _free)

#define DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FIELD, ELEM_TYPE, DEFAULT, NEW, FREE) \
	DEFINE_SET_GET_BASE_TEST(OSSL_CMP ## _ ## CTX, set ## N, get ## M, 1, FIELD, STACK_OF(ELEM_TYPE)*, NULL, DEFAULT, NEW, FREE)
...
#define DEFINE_SET_GET_SK_X509_TEST(OSSL_CMP, CTX, N, M, FNAME) \
	DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FNAME, X509, EMPTY_SK_X509, sk_X509_new_1(), sk_X509_pop_X509_free)

#define DEFINE_SET_GET_TEST_DEFAULT(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE, DEFAULT) \
	DEFINE_SET_GET_BASE_TEST(OSSL_CMP ## _ ## CTX, set ## N, get ## M, DUP, FIELD, TYPE *, NULL, DEFAULT, TYPE ## _new(), TYPE ## _free)
...
#define DEFINE_SET_GET_ARG_FN(SETN, GETN, FIELD, ARG, T) \
	static int OSSL_CMP_CTX_ ## SETN ## _ ## FIELD ## _ ## ARG(CMP_CTX *ctx, T val) { return OSSL_CMP_CTX_ ## SETN ## _ ## FIELD(ctx, ARG, val); } \
	static T OSSL_CMP_CTX_ ## GETN ## _ ## FIELD ## _ ## ARG(const CMP_CTX *ctx) { return OSSL_CMP_CTX_ ## GETN ## _ ## FIELD(ctx, ARG); }

#define DEFINE_SET_GET1_STR_FN(SETN, FIELD) \
	static int OSSL_CMP_CTX_ ## SETN ## _ ## FIELD ## _str(CMP_CTX *ctx, char * val) { return OSSL_CMP_CTX_ ## SETN ## _ ## FIELD(ctx, (uchar *)val, strlen(val)); } \
...
#define DEFINE_PUSH_TEST(N, DUP, FIELD, ELEM, TYPE) DEFINE_PUSH_BASE_TEST(push ## N, DUP, FIELD, ELEM, TYPE *, TYPE, IS_0, TYPE ## _new(), TYPE ## _free)
...
DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set0, get0, 0, trustedStore, X509_STORE *, NULL, DEFAULT_STORE, X509_STORE_new_1(), X509_STORE_free)
...
int setup_tests()
{
	...
	if(!TEST_ptr(cert_file = test_get_argument(0)) || !TEST_ptr(test_cert = load_cert_pem(cert_file, NULL)))
		return 0;
