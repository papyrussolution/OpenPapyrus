...
{
	FIXTURE * fixture;
	if(!TEST_ptr(fixture = (FIXTURE*)OPENSSL_zalloc(sizeof(*fixture)))
...
static int get_ulong_via_BN(const OSSL_PARAM * p, unsigned long * goal)
{
	...
	if(!TEST_true(OSSL_PARAM_get_BN(p, &n)) || !TEST_int_ge(BN_bn2nativepad(n, (unsigned char *)goal, sizeof(*goal)), 0))
		ret = 0;
	BN_free(n);
	return ret;
}
...
{
	unsigned long * keydata = (unsigned long *)arg;
...
static int test_pass_rsa(FIXTURE * fixture)
{
	...
	static unsigned long keydata[SIZEOFARRAY(expected)] = { 0, };
	...
	while(dup_pk == NULL) {
		...
		for(i = 0; i < SIZEOFARRAY(expected); i++) {
