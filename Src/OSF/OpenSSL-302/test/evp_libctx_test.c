 ...
 */
/*
 * DSA/DH low level APIs are deprecated for public use, but still ok for internal use.
 */
...
static int dhx_cert_load(void)
{
	...
	if(!TEST_ptr(bio = BIO_new_mem_buf(dhx_cert, sizeof(dhx_cert))) || !TEST_ptr(cert = X509_new_ex(libctx, NULL)) || !TEST_ptr(d2i_X509_bio(bio, &cert)))
		goto err;
	ret = 1;
err:
	X509_free(cert);
...
{
	STACK_OF(OPENSSL_STRING) *names = (STACK_OF(OPENSSL_STRING) *)cipher_names_list;
	const char * name = EVP_CIPHER_get0_name(cipher);
	char * namedup = NULL;
	assert(name != NULL);
	/* the cipher will be freed after returning, strdup is needed */
	if((namedup = OPENSSL_strdup(name)) != NULL && !sk_OPENSSL_STRING_push(names, namedup))
