...
//
// Test wrapping old style PEM password callback in a UI method through the use of UI utility functions
//
static int test_old(void)
{
	...
	if(!UI_add_input_string(ui, "prompt", UI_INPUT_FLAG_DEFAULT_PWD, pass, 0, sizeof(pass) - 1))
		goto err;
...
static int test_new_ui(void)
{
	...
	if(TEST_int_gt(password_callback(pass, sizeof(pass), 0, &cb_data), 0) && TEST_str_eq(pass, (const char *)cb_data.password))
		ok = 1;
	destroy_ui_method();
	return ok;
}
