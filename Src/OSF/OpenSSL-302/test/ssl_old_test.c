...
static int servername_cb(SSL * s, int * ad, void * arg)
{
	...
	if(servername) {
		if(s_ctx2 && sn_server2 && !strcasecmp(servername, sn_server2)) {
...
static int cb_server_alpn(SSL * s, const unsigned char ** out, unsigned char * outlen, const unsigned char * in, unsigned int inlen, void * arg)
{
	...
	size_t protos_len;
	char* alpn_str = (char *)arg;
	protos = next_protos_parse(&protos_len, alpn_str);
	if(protos == NULL) {
		slfprintf_stderr("failed to parser ALPN server protocol string: %s\n",
		    alpn_str);
		abort();
	}
	 ...
	 * verify_alpn.
	 */
	alpn_selected = (unsigned char *)OPENSSL_malloc(*outlen);
...
static int verify_alpn(SSL * client, SSL * server)
{
	...
	if(client_proto && memcmp(client_proto, server_proto, client_proto_len) != 0) {
		BIO_printf(bio_stdout, "ALPN selected protocols differ!\n");
		goto err;
	}
	...
	if(alpn_expected && (client_proto_len != strlen(alpn_expected) || memcmp(client_proto, alpn_expected, client_proto_len) != 0)) {
...
{
	slfprintf_stderr("usage: ssltest [args ...]\n");
	slfprintf_stderr("\n");
	slfprintf_stderr(" -server_auth  - check server certificate\n");
	slfprintf_stderr(" -client_auth  - do client authentication\n");
	slfprintf_stderr(" -v            - more output\n");
	slfprintf_stderr(" -d            - debug output\n");
	slfprintf_stderr(" -reuse        - use session-id reuse\n");
	slfprintf_stderr(" -num <val>    - number of connections to perform\n");
	slfprintf_stderr(" -bytes <val>  - number of bytes to swap between client/server\n");
#ifndef OPENSSL_NO_DH
	slfprintf_stderr(" -dhe512       - use 512 bit key for DHE (to test failure)\n");
	slfprintf_stderr(" -dhe1024dsa   - use 1024 bit key (with 160-bit subprime) for DHE\n");
	slfprintf_stderr(" -dhe2048      - use 2048 bit key (safe prime) for DHE (default, no-op)\n");
	slfprintf_stderr(" -dhe4096      - use 4096 bit key (safe prime) for DHE\n");
#endif
	slfprintf_stderr(" -no_dhe       - disable DHE\n");
#ifndef OPENSSL_NO_EC
	slfprintf_stderr(" -no_ecdhe     - disable ECDHE\n");
#endif
#ifndef OPENSSL_NO_PSK
	slfprintf_stderr(" -psk arg      - PSK in hex (without 0x)\n");
#endif
#ifndef OPENSSL_NO_SSL3
	slfprintf_stderr(" -ssl3         - use SSLv3\n");
#endif
#ifndef OPENSSL_NO_TLS1
	slfprintf_stderr(" -tls1         - use TLSv1\n");
#endif
#ifndef OPENSSL_NO_TLS1_1
	slfprintf_stderr(" -tls1_1       - use TLSv1.1\n");
#endif
#ifndef OPENSSL_NO_TLS1_2
	slfprintf_stderr(" -tls1_2       - use TLSv1.2\n");
#endif
#ifndef OPENSSL_NO_DTLS
	slfprintf_stderr(" -dtls         - use DTLS\n");
#ifndef OPENSSL_NO_DTLS1
	slfprintf_stderr(" -dtls1        - use DTLSv1\n");
#endif
#ifndef OPENSSL_NO_DTLS1_2
	slfprintf_stderr(" -dtls12       - use DTLSv1.2\n");
#endif
#endif
	slfprintf_stderr(" -CApath arg   - PEM format directory of CA's\n");
	slfprintf_stderr(" -CAfile arg   - PEM format file of CA's\n");
	slfprintf_stderr(" -cert arg     - Server certificate file\n");
	slfprintf_stderr(" -key arg      - Server key file (default: same as -cert)\n");
	slfprintf_stderr(" -c_cert arg   - Client certificate file\n");
	slfprintf_stderr(" -c_key arg    - Client key file (default: same as -c_cert)\n");
	slfprintf_stderr(" -cipher arg   - The TLSv1.2 and below cipher list\n");
	slfprintf_stderr(" -ciphersuites arg   - The TLSv1.3 ciphersuites\n");
	slfprintf_stderr(" -bio_pair     - Use BIO pairs\n");
	slfprintf_stderr(" -ipv4         - Use IPv4 connection on localhost\n");
	slfprintf_stderr(" -ipv6         - Use IPv6 connection on localhost\n");
	slfprintf_stderr(" -f            - Test even cases that can't work\n");
	slfprintf_stderr(" -time         - measure processor time used by client and server\n");
	slfprintf_stderr(" -zlib         - use zlib compression\n");
#ifndef OPENSSL_NO_NEXTPROTONEG
	slfprintf_stderr(" -npn_client - have client side offer NPN\n");
	slfprintf_stderr(" -npn_server - have server side offer NPN\n");
	slfprintf_stderr(" -npn_server_reject - have server reject NPN\n");
#endif
	slfprintf_stderr(" -serverinfo_file file - have server use this file\n");
	slfprintf_stderr(" -serverinfo_sct  - have client offer and expect SCT\n");
	slfprintf_stderr(" -serverinfo_tack - have client offer and expect TACK\n");
	slfprintf_stderr(" -custom_ext - try various custom extension callbacks\n");
	slfprintf_stderr(" -alpn_client <string> - have client side offer ALPN\n");
	slfprintf_stderr(" -alpn_server <string> - have server side offer ALPN\n");
	slfprintf_stderr(" -alpn_server1 <string> - alias for -alpn_server\n");
	slfprintf_stderr(" -alpn_server2 <string> - have server side context 2 offer ALPN\n");
	slfprintf_stderr(" -alpn_expected <string> - the ALPN protocol that should be negotiated\n");
	slfprintf_stderr(" -server_min_proto <string> - Minimum version the server should support\n");
	slfprintf_stderr(" -server_max_proto <string> - Maximum version the server should support\n");
	slfprintf_stderr(" -client_min_proto <string> - Minimum version the client should support\n");
	slfprintf_stderr(" -client_max_proto <string> - Maximum version the client should support\n");
	slfprintf_stderr(" -should_negotiate <string> - The version that should be negotiated, fail-client or fail-server\n");
#ifndef OPENSSL_NO_CT
	slfprintf_stderr(" -noct         - no certificate transparency\n");
	slfprintf_stderr(" -requestct    - request certificate transparency\n");
	slfprintf_stderr(" -requirect    - require certificate transparency\n");
#endif
	slfprintf_stderr(" -sn_client <string>  - have client request this servername\n");
	slfprintf_stderr(" -sn_server1 <string> - have server context 1 respond to this servername\n");
	slfprintf_stderr(" -sn_server2 <string> - have server context 2 respond to this servername\n");
	slfprintf_stderr(" -sn_expect1          - expected server 1\n");
	slfprintf_stderr(" -sn_expect2          - expected server 2\n");
	slfprintf_stderr(" -server_sess_out <file>    - Save the server session to a file\n");
	slfprintf_stderr(" -server_sess_in <file>     - Read the server session from a file\n");
	slfprintf_stderr(" -client_sess_out <file>    - Save the client session to a file\n");
	slfprintf_stderr(" -client_sess_in <file>     - Read the client session from a file\n");
	slfprintf_stderr(" -should_reuse <number>     - The expected state of reusing the session\n");
	slfprintf_stderr(" -no_ticket    - do not issue TLS session ticket\n");
	slfprintf_stderr(" -client_ktls  - try to enable client KTLS\n");
	slfprintf_stderr(" -server_ktls  - try to enable server KTLS\n");
	slfprintf_stderr(" -provider <name>    - Load the given provider into the library context\n");
	slfprintf_stderr(" -config <cnf>    - Load the given config file into the library context\n");
}
...
static void print_details(SSL * c_ssl, const char * prefix)
{
	...
	BIO_printf(bio_stdout, "%s%s, cipher %s %s", prefix, SSL_get_version(c_ssl), SSL_CIPHER_get_version(ciph), SSL_CIPHER_get_name(ciph));
...
int main(int argc, char * argv[])
{
	...
	SSL_CONF_CTX_set_flags(s_cctx, SSL_CONF_FLAG_CMDLINE | SSL_CONF_FLAG_SERVER |SSL_CONF_FLAG_CERTIFICATE |SSL_CONF_FLAG_REQUIRE_PRIVATE);
	...
	SSL_CONF_CTX_set_flags(c_cctx, SSL_CONF_FLAG_CMDLINE | SSL_CONF_FLAG_CLIENT|SSL_CONF_FLAG_CERTIFICATE|SSL_CONF_FLAG_REQUIRE_PRIVATE);
	...
	while(argc >= 1) {
		if(strcmp(*argv, "-F") == 0) {
			slfprintf_stderr("not compiled with FIPS support, so exiting without running.\n");
			EXIT(0);
		}
...
bad:
	...
	if(ssl3 + tls1 + tls1_1 + tls1_2 + dtls + dtls1 + dtls12 > 1) {
		slfprintf_stderr("At most one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, -dtls1 or -dtls12 should "
		    "be requested.\n");
		EXIT(1);
	}
	 ...
	 */
	if(no_protocol) {
		slfprintf_stderr("Testing was requested for a disabled protocol. "
		    "Skipping tests.\n");
		ret = EXIT_SUCCESS;
		goto end;
	}
	if(!ssl3 && !tls1 && !tls1_1 && !tls1_2 && !dtls && !dtls1 && !dtls12 && number > 1 && !reuse && !force) {
		slfprintf_stderr("This case cannot work.  Use -f to perform the test anyway (and\n-d to see what happens), "
		    "or add one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, -dtls1, -dtls12, -reuse\nto avoid protocol mismatch.\n");
		EXIT(1);
	}
	if(print_time) {
		if(bio_type != BIO_PAIR) {
			slfprintf_stderr("Using BIO pair (-bio_pair)\n");
			bio_type = BIO_PAIR;
		}
		if(number < 50 && !force)
			slfprintf_stderr("Warning: For accurate timings, use more connections (e.g. -num 1000)\n");
	}
#ifndef OPENSSL_NO_COMP
	if(comp == COMP_ZLIB)
		cm = COMP_zlib();
	if(cm) {
		if(COMP_get_type(cm) != NID_undef) {
			if(SSL_COMP_add_compression_method(comp, cm) != 0) {
				slfprintf_stderr("Failed to add compression method\n");
				ERR_print_errors_fp(stderr);
			}
		}
		else {
			slfprintf_stderr("Warning: %s compression not supported\n", comp == COMP_ZLIB ? "zlib" : "unknown");
			ERR_print_errors_fp(stderr);
		}
	}
...
#endif
	if(provider && !test_get_libctx(&libctx, &defctxnull, config, &thisprov, provider))
		goto end;
	...
	if(cipher) {
		if(strcmp(cipher, "") == 0) {
			if(!SSL_CTX_set_cipher_list(c_ctx, cipher)) {
				...
				slfprintf_stderr("CLEARING ALL TLSv1.2 CIPHERS SHOULD FAIL\n");
				goto end;
			}
			...
			if(!SSL_CTX_set_cipher_list(c_ctx, cipher) || !SSL_CTX_set_cipher_list(s_ctx, cipher) || !SSL_CTX_set_cipher_list(s_ctx2, cipher)) {
				ERR_print_errors(bio_err);
				goto end;
			}
		}
	...
	if(ciphersuites) {
		if(!SSL_CTX_set_ciphersuites(c_ctx, ciphersuites) || !SSL_CTX_set_ciphersuites(s_ctx, ciphersuites) || !SSL_CTX_set_ciphersuites(s_ctx2, ciphersuites)) {
			ERR_print_errors(bio_err);
			goto end;
		}
	}
	...
	}
	{
		...
		if(!SSL_CTX_set_session_id_context(s_ctx, (const unsigned char *)&session_id_context, sizeof(session_id_context)) ||
		    !SSL_CTX_set_session_id_context(s_ctx2, (const unsigned char *)&session_id_context, sizeof(session_id_context))) {
			ERR_print_errors(bio_err);
			goto end;
		}
	}
	...
	if(server_sess_out || server_sess_in) {
		/* Use a fixed key so that we can decrypt the ticket. */
		long size = SSL_CTX_set_tlsext_ticket_keys(s_ctx, NULL, 0);
		char * keys = (char*)OPENSSL_zalloc(size);
	...
	if(should_negotiate && ret == EXIT_SUCCESS &&
		...
		if(SSL_version(c_ssl) != version) {
			BIO_printf(bio_err, "Unexpected version negotiated. Expected: %s, got %s\n", should_negotiate, SSL_get_version(c_ssl));
			ret = EXIT_FAILURE;
			goto err;
		}
	}
	if(should_reuse != -1) {
		if(SSL_session_reused(s_ssl) != should_reuse || SSL_session_reused(c_ssl) != should_reuse) {
			BIO_printf(bio_err, "Unexpected session reuse state. Expected: %d, server: %d, client: %d\n", should_reuse, SSL_session_reused(s_ssl), SSL_session_reused(c_ssl));
			ret = EXIT_FAILURE;
			goto err;
		}
	}
    ...
    clock_t * s_time, clock_t * c_time)
{
			...
			if(cw_num > 0) {
				...
				if(r < 0) {
					if(!BIO_should_retry(c_ssl_bio)) {
						slfprintf_stderr("ERROR in CLIENT\n");
						err_in_client = 1;
						goto err;
					}
					/*
					...
					slfprintf_stderr("SSL CLIENT STARTUP FAILED\n");
					goto err;
				}
				else {
					if(debug)
			...
			if(sw_num > 0) {
				...
				if(r < 0) {
					if(!BIO_should_retry(s_ssl_bio)) {
						slfprintf_stderr("ERROR in SERVER\n");
						err_in_server = 1;
						goto err;
					}
					...
					slfprintf_stderr("SSL SERVER STARTUP FAILED\n");
					goto err;
				}
				else {
					if(debug)
	...
	if(verify_serverinfo() < 0) {
		slfprintf_stderr("Server info verify error\n");
		goto err;
	}
	if(verify_alpn(c_ssl, s_ssl) < 0 || verify_servername(c_ssl, s_ssl) < 0)
		goto err;
	if(custom_ext_error) {
		slfprintf_stderr("Custom extension error\n");
		goto err;
	}
#ifndef OPENSSL_NO_NEXTPROTONEG
end:
...
err:
	...
	BIO_free(s_ssl_bio);
	BIO_free(c_ssl_bio);
	if(should_negotiate && strcmp(should_negotiate, "fail-client") == 0)
		ret = (err_in_client != 0) ? EXIT_SUCCESS : EXIT_FAILURE;
	else if(should_negotiate && strcmp(should_negotiate, "fail-server") == 0)
...

		{
			...
			do {
				...
				if(num) {
					...
					if(r != (int)num) { /* can't happen */
						slfprintf_stderr("ERROR: BIO_write could not write BIO_ctrl_get_write_guarantee() bytes");
						goto err;
					}
					progress = 1;
					if(debug)
			...
			/* io2 to io1 */
			{
				...
				if(num) {
					...
					if(r != (int)num) { /* can't happen */
						slfprintf_stderr("ERROR: BIO_read could not read BIO_ctrl_pending() bytes");
						goto err;
					}
					progress = 1;
					r = BIO_nwrite(io1, &dataptr, (int)num);
					if(r != (int)num) { /* can't happen */
						slfprintf_stderr("ERROR: BIO_nwrite() did not accept BIO_nwrite0() bytes");
						goto err;
					}
					if(debug)
			...
			if(!progress && !prev_progress)
				if(cw_num > 0 || cr_num > 0 || sw_num > 0 || sr_num > 0) {
					slfprintf_stderr("ERROR: got stuck\n");
					slfprintf_stderr(" ERROR.\n");
					goto err;
				}
...
int doit(SSL * s_ssl, SSL * c_ssl, long count)
{
	...
	if((cbuf = (char *)OPENSSL_zalloc(bufsiz)) == NULL)
		goto err;
	if((sbuf = (char *)OPENSSL_zalloc(bufsiz)) == NULL)
		goto err;
		...
		if(do_server && !(done & S_DONE)) {
			if(!s_write) {
				...
				if(i < 0) {
					...
					ERR_print_errors(bio_err);
					slfprintf_stderr("SSL SERVER STARTUP FAILED in SSL_read\n");
					goto err;
				}
				else {
					if(debug)
				...
				if(i < 0) {
					...
					ERR_print_errors(bio_err);
					slfprintf_stderr("SSL SERVER STARTUP FAILED in SSL_write\n");
					goto err;
				}
				else {
					if(debug)
...
{
	char buf[256];
	char * s = X509_NAME_oneline(X509_get_subject_name(X509_STORE_CTX_get_current_cert(ctx)), buf, sizeof(buf));
	if(s) {
		if(ok)
			printf("depth=%d %s\n", X509_STORE_CTX_get_error_depth(ctx), buf);
		else {
			slfprintf_stderr("depth=%d error=%d %s\n", X509_STORE_CTX_get_error_depth(ctx), X509_STORE_CTX_get_error(ctx), buf);
		}
	}
	if(ok == 0) {
		...
		switch(i) {
			default:
			    slfprintf_stderr("Error string: %s\n", X509_verify_cert_error_string(i));
			    break;
...
static unsigned int psk_client_callback(SSL * ssl, const char * hint, char * identity, unsigned int max_identity_len, unsigned char * psk, unsigned int max_psk_len)
{
	...
	int ret = BIO_snprintf(identity, max_identity_len, "Client_identity");
	if(ret < 0)
		goto out_err;
	if(debug)
		slfprintf_stderr("client: created identity '%s' len=%d\n", identity, ret);
