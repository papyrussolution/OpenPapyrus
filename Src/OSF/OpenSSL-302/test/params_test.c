...
static int raw_set_params(void * vobj, const OSSL_PARAM * params)
{
	...
	for(; params->key != NULL; params++)
		if(strcmp(params->key, "p1") == 0) {
			...
			BN_free(obj->p3);
			if(!TEST_ptr(obj->p3 = BN_native2bn((const unsigned char *)params->data, params->data_size, NULL)))
				return 0;
		}
		...
		if(strcmp(params->key, "p1") == 0) {
			...
			*(double*)params->data = obj->p2;
			...
			if(!TEST_size_t_ge(params->data_size, params->return_size))
				return 0;
			BN_bn2nativepad(obj->p3, (unsigned char *)params->data, params->return_size);
		}
		else if(strcmp(params->key, "p4") == 0) {
...
int setup_tests()
{
	ADD_ALL_TESTS(test_case, SIZEOFARRAY(test_cases));
	ADD_ALL_TESTS(test_allocate_from_text, SIZEOFARRAY(int_from_text_test_cases));
	return 1;
}
