    ...
    EVP_RAND_CTX * ctx)
{
	...
	if(!TEST_ptr(tmp = BN_new())
	    || !TEST_int_lt(fbytes_counter, SIZEOFARRAY(numbers))
	...
	fbytes_counter = (fbytes_counter + 1) % SIZEOFARRAY(numbers);
	ret = 1;
err:
	BN_free(tmp);
	return ret;
...
static int test_builtin(int n, int as)
{
	...
	if(!TEST_int_ge(temp, 0)
	    || !TEST_ptr(sig = (unsigned char *)OPENSSL_malloc(sig_len = (size_t)temp))
	    /* create a signature */
...
#endif
	ADD_ALL_TESTS(x9_62_tests, SIZEOFARRAY(ecdsa_cavs_kats));
#endif
	return 1;
}
