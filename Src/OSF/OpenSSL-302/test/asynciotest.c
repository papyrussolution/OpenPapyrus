...
{
	struct async_ctrs * ctrs = (struct async_ctrs *)OPENSSL_zalloc(sizeof(struct async_ctrs));
	if(!ctrs)
		return 0;
...
static int async_free(BIO * bio)
{
	...
	if(!bio)
		return 0;
	ctrs = (struct async_ctrs *)BIO_get_data(bio);
	OPENSSL_free(ctrs);
	...
	if(ctrs->wctr > 0) {
		...
		if(fragment) {
			...
			while(PACKET_remaining(&pkt) > 0) {
				...
				if(!PACKET_get_1(&pkt, &contenttype) || !PACKET_get_1(&pkt, &versionhi) || !PACKET_get_1(&pkt, &versionlo) || !PACKET_get_length_prefixed_2(&pkt, &payload))
					return -1;
				...
				if(msgtype == SSL3_MT_SERVER_HELLO) {
					if(!PACKET_forward(&wholebody, SSL3_HM_HEADER_LENGTH - 1) || !PACKET_get_net_2(&wholebody, &negversion)
					...
					while(PACKET_remaining(&extensions)) {
						unsigned int type;
						PACKET extbody;
						if(!PACKET_get_net_2(&extensions, &type) || !PACKET_get_length_prefixed_2(&extensions, &extbody))
							return -1;
						if(type == TLSEXT_TYPE_supported_versions && (!PACKET_get_net_2(&extbody, &negversion) || PACKET_remaining(&extbody) != 0))
							return -1;
					}
				}
				 ...
				 */
				if(contenttype == SSL3_RT_CHANGE_CIPHER_SPEC || (negversion == TLS1_3_VERSION && msgtype == SSL3_MT_SERVER_HELLO)) {
					fragment = 0;
					break;
				}
			}
...
static int async_gets(BIO * bio, char * buf, int size) { return -1; /* We don't support this - not needed anyway */ }
static int async_puts(BIO * bio, const char * str) { return async_write(bio, str, strlen(str)); }
...
static int test_asyncio(int test)
{
	...
	if(!TEST_true(create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &serverctx, &clientctx, cert, privkey)))
		goto end;
	/*
	...
	if(!TEST_true(create_ssl_objects(serverctx, clientctx, &serverssl, &clientssl, s_to_c_fbio, c_to_s_fbio)) || !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)))
		goto end;
	/*
