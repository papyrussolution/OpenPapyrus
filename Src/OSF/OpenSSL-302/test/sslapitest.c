...
static int test_keylog(void)
{
	...
	memzero(client_log_buffer, sizeof(client_log_buffer));
	memzero(server_log_buffer, sizeof(server_log_buffer));
...
static int verify_retry_cb(X509_STORE_CTX * ctx, void * arg)
{
	...
	if(idx < 0 || (ssl = (SSL *)X509_STORE_CTX_get_ex_data(ctx, idx)) == NULL)
		return 0;
...
static int full_client_hello_callback(SSL * s, int * al, void * arg)
{
...
#else
	const unsigned char expected_ciphers[] = {0x00, 0x9d, 0xc0, 0x2c, 0x00, 0xff};
#endif
	...
	if(!SSL_client_hello_get1_extensions_present(s, &exts, &len))
		return SSL_CLIENT_HELLO_ERROR;
	if(len != SIZEOFARRAY(expected_extensions) ||
...
static int ocsp_server_cb(SSL * s, void * arg)
{
	...
	if(!TEST_ptr(copy = (unsigned char *)OPENSSL_memdup(orespder, sizeof(orespder))))
		return SSL_TLSEXT_ERR_ALERT_FATAL;
    ...
    SSL_CTX ** cctx)
{
	...
	if(!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
	    ...
	    || !TEST_true(SSL_CTX_set_session_id_context(*sctx, (const unsigned char *)&sess_id_ctx, sizeof(sess_id_ctx))))
		return 0;
...
static int test_tickets(int stateful, int idx)
{
	...
	SSL_free(serverssl);
	SSL_free(clientssl);
	for(j = 0; j < SIZEOFARRAY(sesscache); j++) {
...
static int test_psk_tickets(void)
{
	...
	int testresult = 0;
	int sess_id_ctx = 1;
	if(!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx, &cctx, NULL, NULL))
	    || !TEST_true(SSL_CTX_set_session_id_context(sctx, (const unsigned char *)&sess_id_ctx, sizeof(sess_id_ctx))))
		goto end;
	...
	if(!TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) || 
		!TEST_int_eq(1, find_session_cb_cnt) || !TEST_int_eq(1, use_session_cb_cnt) 
...
static const sigalgs_list testsigalgs[] = {
	{validlist1, SIZEOFARRAY(validlist1), NULL, 1, 1},
#ifndef OPENSSL_NO_EC
	{validlist2, SIZEOFARRAY(validlist2), NULL, 1, 1},
	{validlist3, SIZEOFARRAY(validlist3), NULL, 1, 0},
#endif
...
#endif
	{invalidlist1, SIZEOFARRAY(invalidlist1), NULL, 0, 0},
	{invalidlist2, SIZEOFARRAY(invalidlist2), NULL, 0, 0},
	{invalidlist3, SIZEOFARRAY(invalidlist3), NULL, 0, 0},
	{invalidlist4, SIZEOFARRAY(invalidlist4), NULL, 0, 0},
	{NULL, 0, "RSA", 0, 0},
	{NULL, 0, "SHA256", 0, 0},
...
static int test_set_sigalgs(int idx)
{
	...
	int testctx;
	/* Should never happen */
	if(!TEST_size_t_le((size_t)idx, SIZEOFARRAY(testsigalgs) * 2))
		return 0;
	testctx = ((size_t)idx < SIZEOFARRAY(testsigalgs));
	curr = testctx ? &testsigalgs[idx] : &testsigalgs[idx - SIZEOFARRAY(testsigalgs)];
	if(!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx, &cctx, cert, privkey)))
		return 0;
	SSL_CTX_set_max_proto_version(cctx, TLS1_2_VERSION);
	if(testctx) {
		int ret;
...
static int test_key_exchange(int idx)
{
	...
	switch(idx) {
		...
		case 0:
		    ...
		    kexch_groups_size = SIZEOFARRAY(ecdhe_kexch_groups);
		...
		case 6:
		    ...
		    kexch_groups_size = SIZEOFARRAY(ffdhe_kexch_groups);
		    kexch_name0 = "ffdhe2048";
		    break;
		case 7:
    ...
    int isecdhe, int idx)
{
	...
	numec = SIZEOFARRAY(ecdhe_kexch_groups);
	numff = SIZEOFARRAY(ffdhe_kexch_groups);
	if(isecdhe)
...
static int test_tls13_ciphersuite(int idx)
{
	...
	for(max_ver = TLS1_2_VERSION; max_ver <= TLS1_3_VERSION; max_ver++) {
		...
		if(max_ver == TLS1_2_VERSION)
			continue;
#endif
		for(i = 0; i < SIZEOFARRAY(t13_ciphers); i++) {
    ...
    size_t * outlen, int * al, void * add_arg)
{
	...
	if(SSL_is_server(s))
		...
		clntaddoldcb++;
	if(*server != SSL_is_server(s) || (data = (unsigned char *)OPENSSL_malloc(sizeof(*data))) == NULL)
		return -1;
	*data = 1;
	*out = data;
	*outlen = sizeof(char);
...
static int test_export_key_mat(int tst)
{
	...
	OPENSSL_assert(tst >= 0 && (size_t)tst < SIZEOFARRAY(protocols));
...
{
	char * gNid = NULL;
	OPENSSL_STRING * row = (OPENSSL_STRING *)OPENSSL_zalloc(sizeof(row) * (DB_NUMBER + 1));
	TXT_DB * db = NULL;
	int ret = 0;
...
static int create_new_vbase(char * userid, char * password)
{
	...
	if(!TEST_true(SRP_create_verifier_BN_ex(userid, password, &salt, &verifier, lgN->N, lgN->g, libctx, NULL)))
		goto end;
	user_pwd = (SRP_user_pwd *)OPENSSL_zalloc(sizeof(*user_pwd));
...
static int int_test_ssl_get_shared_ciphers(int tst, int clnt)
{
	...
	if(!TEST_true(SSL_CTX_set_cipher_list(cctx,
		...
		goto end;
	if(!TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) || !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)))
		goto end;
	if(!TEST_ptr(SSL_get_shared_ciphers(serverssl, buf, sizeof(buf))) || !TEST_int_eq(strcmp(buf, is_fips ? shared_ciphers_data[tst].fipsshared : shared_ciphers_data[tst].shared), 0)) {
...
static SSL_TICKET_RETURN dec_tick_cb(SSL * s, SSL_SESSION * ss, const unsigned char * keyname,
    size_t keyname_length, SSL_TICKET_STATUS status, void * arg)
{
	...
	if(!TEST_true(SSL_SESSION_get0_ticket_appdata(ss, &tickdata, &tickdlen)) || !TEST_size_t_eq(tickdlen, strlen(appdata)) || !TEST_int_eq(memcmp(tickdata, appdata, tickdlen), 0))
		return SSL_TICKET_RETURN_ABORT;
	if(tick_key_cb_called) {
		/* Don't change what the ticket key callback wanted to do */
		switch(status) {
			case SSL_TICKET_NO_DECRYPT: return SSL_TICKET_RETURN_IGNORE_RENEW;
			case SSL_TICKET_SUCCESS: return SSL_TICKET_RETURN_USE;
			case SSL_TICKET_SUCCESS_RENEW: return SSL_TICKET_RETURN_USE_RENEW;
			default: return SSL_TICKET_RETURN_ABORT;
		}
	}
	return tick_dec_ret;
}
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static int tick_key_cb(SSL * s, unsigned char key_name[16], unsigned char iv[EVP_MAX_IV_LENGTH], EVP_CIPHER_CTX * ctx, HMAC_CTX * hctx, int enc)
{
	const unsigned char tick_aes_key[] = "0123456789abcdef"; // @sobolev tick_aes_key[16] --> p_tick_aes_key[]
	const unsigned char tick_hmac_key[] = "0123456789abcdef"; // @sobolev tick_hmac_key[16] --> tick_hmac_key[]
	EVP_CIPHER * aes128cbc = EVP_CIPHER_fetch(libctx, "AES-128-CBC", NULL);
	EVP_MD * sha256 = EVP_MD_fetch(libctx, "SHA-256", NULL);
	int ret;
	tick_key_cb_called = 1;
	memzero(iv, AES_BLOCK_SIZE);
	memzero(key_name, 16);
	if(aes128cbc == NULL || sha256 == NULL || !EVP_CipherInit_ex(ctx, aes128cbc, NULL, tick_aes_key, iv, enc) || 
		!HMAC_Init_ex(hctx, tick_hmac_key, sstrlen(tick_hmac_key)/* @sobolev sizeof()-->sstrlen()*/, sha256, NULL))
		ret = -1;
	else
...
static int tick_key_evp_cb(SSL * s, unsigned char key_name[16], unsigned char iv[EVP_MAX_IV_LENGTH], EVP_CIPHER_CTX * ctx, EVP_MAC_CTX * hctx, int enc)
{
	const unsigned char tick_aes_key[] = "0123456789abcdef"; // @sobolev tick_aes_key[16]-->tick_aes_key[]
	unsigned char tick_hmac_key[] = "0123456789abcdef"; // @sobolev tick_hmac_key[16]-->tick_hmac_key[]
	OSSL_PARAM params[2];
	EVP_CIPHER * aes128cbc = EVP_CIPHER_fetch(libctx, "AES-128-CBC", NULL);
	int ret;
	tick_key_cb_called = 1;
	...
	if(aes128cbc == NULL || !EVP_CipherInit_ex(ctx, aes128cbc, NULL, tick_aes_key, iv, enc) || !EVP_MAC_init(hctx, tick_hmac_key, sstrlen(tick_hmac_key)/* @sobolev sizeof()-->sstrlen()*/, params))
		ret = -1;
	else
		ret = tick_key_renew ? 2 : 1;
	EVP_CIPHER_free(aes128cbc);
...
int setup_tests()
{
...
#if !defined(OPENSSL_NO_TLS1_2) || defined(OSSL_NO_USABLE_TLS1_3)
	ADD_ALL_TESTS(test_set_sigalgs, SIZEOFARRAY(testsigalgs) * 2);
	...
	ADD_ALL_TESTS(test_negotiated_group,
	    4 * (SIZEOFARRAY(ecdhe_kexch_groups)
	    + SIZEOFARRAY(ffdhe_kexch_groups)));
#endif
#else
	...
	ADD_ALL_TESTS(test_max_fragment_len_ext, SIZEOFARRAY(max_fragment_len_test));
#if !defined(OPENSSL_NO_SRP) && !defined(OPENSSL_NO_TLS1_2)
	ADD_ALL_TESTS(test_srp, 6);
#endif
	ADD_ALL_TESTS(test_info_callback, 6);
	ADD_ALL_TESTS(test_ssl_pending, 2);
	ADD_ALL_TESTS(test_ssl_get_shared_ciphers, SIZEOFARRAY(shared_ciphers_data));
...
#ifndef OPENSSL_NO_TLS1_2
	ADD_ALL_TESTS(test_multiblock_write, SIZEOFARRAY(multiblock_cipherlist_data));
#endif
