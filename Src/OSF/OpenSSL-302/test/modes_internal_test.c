...
static const unsigned char cts128_test_input[/*64*/] = "I would like the General Gau's Chicken, please, and wonton soup.";
...
typedef struct {
	...
	size_t (* encrypt_block)(const unsigned char * in, unsigned char * out, size_t len, const void * key, unsigned char ivec[16], block128_f block);
	size_t (* encrypt_stream)(const unsigned char * in, unsigned char * out, size_t len, const void * key, unsigned char ivec[16], cbc128_f cbc);
	size_t (* decrypt_block)(const unsigned char * in, unsigned char * out, size_t len, const void * key, unsigned char ivec[16], block128_f block);
	size_t (* decrypt_stream)(const unsigned char * in, unsigned char * out, size_t len, const void * key, unsigned char ivec[16], cbc128_f cbc);
} CTS128_FIXTURE;
...
static int test_gcm128(int idx)
{
	...
	if(P.data != NULL)
		if(!TEST_int_ge(CRYPTO_gcm128_encrypt(&ctx, P.data, out, P.size), 0))
			return 0;
	if(!TEST_false(CRYPTO_gcm128_finish(&ctx, T.data, 16)) || (C.data != NULL && !TEST_mem_eq(out, P.size, C.data, P.size)))
		return 0;
	CRYPTO_gcm128_setiv(&ctx, IV.data, IV.size);
	memzero(out, P.size);
	if(A.data != NULL)
...
int setup_tests()
{
	ADD_ALL_TESTS(test_aes_cts128, SIZEOFARRAY(aes_cts128_vectors));
	ADD_ALL_TESTS(test_aes_cts128_nist, SIZEOFARRAY(aes_cts128_vectors));
	ADD_ALL_TESTS(test_gcm128, SIZEOFARRAY(gcm128_vectors));
	return 1;
}
