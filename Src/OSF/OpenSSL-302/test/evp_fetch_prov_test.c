...
static int calculate_digest(const EVP_MD * md, const char * msg, size_t len, const uchar * exptd)
{
	uchar out[SHA256_DIGEST_LENGTH];
	EVP_MD_CTX * ctx;
	int ret = 0;
...
static int test_implicit_EVP_MD_fetch(void)
{
	...
	int ret = (use_default_ctx == 0 || load_providers(&ctx, prov)) && test_md(EVP_sha256());
 ...
 */
static int encrypt_decrypt(const EVP_CIPHER * cipher, const uchar * msg, size_t len)
{
...
{
	const uchar testmsg[] = "Hello world";
...
static int test_implicit_EVP_CIPHER_fetch(void)
{
	OSSL_LIB_CTX * ctx = NULL;
	OSSL_PROVIDER * prov[2] = {NULL, NULL};
	int ret = (use_default_ctx == 0 || load_providers(&ctx, prov)) && test_cipher(EVP_aes_128_cbc());
	unload_providers(&ctx, prov);
	return ret;
}
