...
static int pkcs12_create_test(void)
{
	...
	if(!TEST_int_eq(ERR_peek_error(), 0))
		goto err;
	p12 = PKCS12_create(NULL, NULL, pkey, NULL, NULL, NID_pbe_WithSHA1And3_Key_TripleDES_CBC, NID_pbe_WithSHA1And3_Key_TripleDES_CBC, 2, 1, 0);
	if(!TEST_ptr(p12))
		goto err;
	if(!TEST_int_eq(ERR_peek_error(), 0))
		goto err;
...
int setup_tests()
{
	 ...
	 */
	if(!default_libctx) {
		if(!TEST_false(OSSL_PROVIDER_available(NULL, "default")) || !TEST_false(OSSL_PROVIDER_available(NULL, "fips")))
			return 0;
	}
	ADD_TEST(test_single_cert_no_attrs);
	if(lgcyprov == NULL) {
		ADD_ALL_TESTS(test_single_key_enc_alg, SIZEOFARRAY(enc_nids_no_legacy));
		ADD_ALL_TESTS(test_single_secret_enc_alg, SIZEOFARRAY(enc_nids_no_legacy));
	}
	else {
		ADD_ALL_TESTS(test_single_key_enc_alg, SIZEOFARRAY(enc_nids_all));
		ADD_ALL_TESTS(test_single_secret_enc_alg, SIZEOFARRAY(enc_nids_all));
	}
#ifndef OPENSSL_NO_DES
	if(default_libctx)
		ADD_TEST(pkcs12_create_test);
#endif
	ADD_ALL_TESTS(test_single_key_enc_pass, SIZEOFARRAY(passwords));
	ADD_ALL_TESTS(test_single_key_enc_iter, SIZEOFARRAY(iters));
	ADD_TEST(test_single_key_with_attrs);
	ADD_ALL_TESTS(test_single_cert_mac_alg, SIZEOFARRAY(mac_nids));
	ADD_ALL_TESTS(test_single_cert_mac_pass, SIZEOFARRAY(passwords));
	ADD_ALL_TESTS(test_single_cert_mac_iter, SIZEOFARRAY(iters));
