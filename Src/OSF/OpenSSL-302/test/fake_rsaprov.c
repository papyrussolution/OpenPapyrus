...
{
	unsigned char * keydata = (unsigned char *)OPENSSL_zalloc(1);
	TEST_ptr(keydata);
	/* clear test globals */
	has_selection = 0;
...
{
	unsigned char * fake_rsa_key = (unsigned char *)keydata;
	/* key was imported */
	*fake_rsa_key = 1;
	return 1;
}
...
{
	unsigned char * sigctx = (unsigned char *)OPENSSL_zalloc(1);
	TEST_ptr(sigctx);
	return sigctx;
}
...
{
	unsigned char * sigctx = (unsigned char *)ctx;
	unsigned char * keydata = (unsigned char *)provkey;
	/* we must have a ctx */
	if(!TEST_ptr(sigctx))
		return 0;
...
static int fake_rsa_sig_sign(void * ctx, unsigned char * sig, size_t * siglen, size_t sigsize, const unsigned char * tbs, size_t tbslen)
{
	unsigned char * sigctx = (unsigned char *)ctx;
...
static const OSSL_ALGORITHM * fake_rsa_query(void * provctx, int operation_id, int * no_cache)
{
	*no_cache = 0;
	switch(operation_id) {
		case OSSL_OP_SIGNATURE: return fake_rsa_sig_algs;
		case OSSL_OP_KEYMGMT: return fake_rsa_keymgmt_algs;
	}
	return NULL;
}
...
static int fake_rsa_provider_init(const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * in, const OSSL_DISPATCH ** out, void ** provctx)
{
...
{
	OSSL_PROVIDER * p;
	if(!TEST_true(OSSL_PROVIDER_add_builtin(libctx, "fake-rsa", fake_rsa_provider_init)) || !TEST_ptr(p = OSSL_PROVIDER_try_load(libctx, "fake-rsa", 1)))
		return NULL;
	return p;
}
