...
static int watchccs_write(BIO * bio, const char * in, int inl)
{
	...
	while(PACKET_remaining(&pkt)) {
		if(!PACKET_get_1(&pkt, &rectype) || !PACKET_get_net_2(&pkt, &recvers) || !PACKET_get_length_prefixed_2(&pkt, &msg))
			return 0;
		...
		if(rectype == SSL3_RT_HANDSHAKE) {
			...
			if(msgtype == SSL3_MT_CLIENT_HELLO) {
				chseen++;
				/*
				 * Skip legacy_version (2 bytes) and Random (32 bytes) to read session_id.
				 */
				...
				if(chseen == 1) {
					 ...
					 */
					if(PACKET_remaining(&sessionid) != chsessidlen || (chsessidlen > 0 && memcmp(chsessid, PACKET_data(&sessionid), chsessidlen) != 0))
						badsessid = 1;
				}
			}
...
static int watchccs_gets(BIO * bio, char * buf, int size) { return -1; /* We don't support this - not needed anyway */ }
static int watchccs_puts(BIO * bio, const char * str) { return watchccs_write(bio, str, strlen(str)); }
...
static int test_tls13ccs(int tst)
{
	...
	chsessidlen = 0;
	if(!TEST_true(create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx, &cctx, cert, privkey)) || 
