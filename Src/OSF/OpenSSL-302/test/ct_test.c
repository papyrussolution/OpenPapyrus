...
#define CT_TEST_MAX_FILE_SIZE 8096 // Used when declaring buffers to read text files into 
...
static CT_TEST_FIXTURE * set_up(const char * const test_case_name)
{
	...
	if(!TEST_ptr(fixture = (CT_TEST_FIXTURE *)OPENSSL_zalloc(sizeof(*fixture))))
		goto end;
...
static int compare_extension_printout(X509_EXTENSION * extension, const char * expected_output)
{
	...
	char * actual_output = NULL;
	int result = 0;
	if(!TEST_ptr(text_buffer = BIO_new(BIO_s_mem())) || !TEST_true(X509V3_EXT_print(text_buffer, extension,
...
static int execute_cert_test(CT_TEST_FIXTURE * fixture)
{
	...
	if(fixture->sct_text_file != NULL) {
		sct_text_len = read_text_file(fixture->sct_dir, fixture->sct_text_file, expected_sct_text, CT_TEST_MAX_FILE_SIZE - 1);
	...
	if(fixture->certificate_file != NULL) {
		...
		if(fixture->expected_sct_count > 0) {
			...
			if(fixture->sct_text_file && !compare_extension_printout(sct_extension, expected_sct_text))
				goto end;
			scts = (STACK_OF(SCT) *)X509V3_EXT_d2i(sct_extension);
			for(i = 0; i < sk_SCT_num(scts); ++i) {
				SCT * sct_i = sk_SCT_value(scts, i);
...
static int test_ctlog_from_base64(void)
{
	...
	if(!TEST_true(!CTLOG_new_from_base64(&ctlogp, notb64, name)) || !TEST_true(!CTLOG_new_from_base64(&ctlogp, pad, name)))
		return 0;
	return 1;
}
