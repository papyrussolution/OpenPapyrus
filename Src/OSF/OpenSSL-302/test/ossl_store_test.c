...
{
	OSSL_STORE_SEARCH * search = NULL;
	int ret = TEST_ptr_null(search = OSSL_STORE_SEARCH_by_key_fingerprint(EVP_sha256(), NULL, 0));
	OSSL_STORE_SEARCH_free(search);
	return ret;
}
...
static int get_params(const char * uri, const char * type)
{
	...
	int ret = 0;
	OSSL_STORE_CTX * ctx = OSSL_STORE_open_ex(uri, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	if(!TEST_ptr(ctx))
		goto err;
...
static int test_store_get_params(int idx)
{
	...
	switch(idx) {
		...
		case 0: type = "DH"; break;
		case 1: type = "DHX"; break;
#else
		case 0:
		case 1: return 1;
#endif
...
{
	static const OPTIONS test_options[] = { OPT_TEST_OPTIONS_DEFAULT_USAGE, { "dir", OPT_INPUTDIR, '/' }, { "in", OPT_INFILE, '<' }, { "sm2", OPT_SM2FILE, '<' }, { "data", OPT_DATADIR, 's' }, { NULL } };
	return test_options;
}
...
int setup_tests()
{
	...
	while((o = (OPTION_CHOICE)opt_next()) != OPT_EOF) {
		switch(o) {
			case OPT_INPUTDIR: inputdir = opt_arg(); break;
			case OPT_INFILE: infile = opt_arg(); break;
			case OPT_SM2FILE: sm2file = opt_arg(); break;
			case OPT_DATADIR: datadir = opt_arg(); break;
			case OPT_TEST_CASES: break;
			default:
