...
static int tls_corrupt_write(BIO * bio, const char * in, int inl)
{
	...
	char * copy;
	if(docorrupt) {
		if(!TEST_ptr(copy = (char*)OPENSSL_memdup(in, inl)))
			return 0;
...
static int tls_corrupt_gets(BIO * bio, char * buf, int size) { return -1; /* We don't support this - not needed anyway */ }
static int tls_corrupt_puts(BIO * bio, const char * str) { return -1; /* We don't support this - not needed anyway */ }
...
static int setup_cipher_list(void)
{
	 ...
	 */
	cipher_list = (const char **)OPENSSL_malloc(sk_SSL_CIPHER_num(sk_ciphers) * sizeof(cipher_list[0]));
