...
{
	FAKE_RAND * r = (FAKE_RAND *)OPENSSL_zalloc(sizeof(*r));
	if(r)
...
static int fake_rand_instantiate(void * vrng, ossl_unused unsigned int strength, ossl_unused int prediction_resistance,
    ossl_unused const unsigned char * pstr, size_t pstr_len, ossl_unused const OSSL_PARAM params[])
{
...
static int fake_rand_generate(void * vrng, unsigned char * out, size_t outlen, unsigned int strength, int prediction_resistance, const unsigned char * adin, size_t adinlen)
{
	FAKE_RAND * frng = (FAKE_RAND*)vrng;
	size_t l;
	uint32_t r;
...
static int fake_rand_get_ctx_params(ossl_unused void * vrng, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_int(p, frng->state))
		return 0;

	p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_STRENGTH);
	if(p && !OSSL_PARAM_set_int(p, 256))
		return 0;
