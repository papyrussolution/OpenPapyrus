...
static int test_provider(OSSL_PROVIDER * prov, const char * expected_greeting)
{
	...
	ret =
	    ...
	    && TEST_ptr(greeting = (const char *)greeting_request[0].data)
...
static const char * expected_greeting1(const char * name)
{
	...
	BIO_snprintf(expected_greeting, sizeof(expected_greeting), "Hello OpenSSL %.20s, greetings from %s!", OPENSSL_VERSION_STR, name);
...
static int test_builtin_provider(void)
{
	...
	ret = TEST_ptr(prov = ossl_provider_new(NULL, name, PROVIDER_INIT_FUNCTION_NAME, 0)) && test_provider(prov, expected_greeting1(name));
...
static int test_loaded_provider(void)
{
	...
	OSSL_PROVIDER * prov = NULL;
	return TEST_ptr(prov = ossl_provider_new(NULL, name, NULL, 0)) && test_provider(prov, expected_greeting1(name));
}
...
static int test_configured_provider(void)
{
	...
	return TEST_ptr(prov = ossl_provider_find(NULL, name, 0)) && test_provider(prov, expected_greeting);
}
