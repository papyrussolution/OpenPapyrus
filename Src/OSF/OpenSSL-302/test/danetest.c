...
static STACK_OF(X509) *load_chain(BIO *fp, int nelem)
{
	...
	STACK_OF(X509) *chain;
	typedef X509 *(* d2i_X509_t)(X509 **, const uchar **, long);
	if(!TEST_ptr(chain = sk_X509_new_null()))
		goto err;
	for(count = 0; count < nelem && errtype == 0 && PEM_read_bio(fp, &name, &header, &data, &len) == 1; ++count) {
		if(strcmp(name, PEM_STRING_X509) == 0 || strcmp(name, PEM_STRING_X509_TRUSTED) == 0 || strcmp(name, PEM_STRING_X509_OLD) == 0) {
...
{
	uchar ** out = (uchar**)result;
	uchar * ret;
	uchar * cp;
	uint8_t byte;
	int nibble = 0;
	if(!TEST_ptr(ret = (uchar *)OPENSSL_malloc(strlen(in) / 2)))
		return -1;
	cp = ret;
	for(byte = 0; *in; ++in) {
		int x;
...
static int test_tlsafile(SSL_CTX * ctx, const char * base_name, BIO * f, const char * path)
{
	...
	while(ret > 0 && (line = read_to_eol(f)) != NULL) {
		...
		if(*line == '\0' || *line == '#')
			continue;
		++testno;
		if(sscanf(line, "%d %d %d %d %d%n", &ntlsa, &ncert, &noncheck, &want, &want_depth, &off) != 5 || !allws(line + off)) {
...
int setup_tests()
{
	...
	if(!TEST_ptr(basedomain = test_get_argument(0)) || !TEST_ptr(CAfile = test_get_argument(1)) || !TEST_ptr(tlsafile = test_get_argument(2)))
		return 0;
