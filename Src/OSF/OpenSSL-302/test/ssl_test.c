...
#define MAX_TESTCASE_NAME_LENGTH 100 /* Currently the section names are of the form test-<number>, e.g. test-15. */
...
static int check_result(HANDSHAKE_RESULT * result, SSL_TEST_CTX * test_ctx)
{
	if(!TEST_int_eq(result->result, test_ctx->expected_result)) {
		TEST_info("ExpectedResult mismatch: expected %s, got %s.", ssl_test_result_name(test_ctx->expected_result), ssl_test_result_name(result->result));
		return 0;
	}
	return 1;
}
...
static int check_alerts(HANDSHAKE_RESULT * result, SSL_TEST_CTX * test_ctx)
{
	if(!TEST_int_eq(result->client_alert_sent,
		...
		TEST_info("Client sent alert %s but server received %s.", print_alert(result->client_alert_sent), print_alert(result->client_alert_received));
		/*
	...
	if(test_ctx->expected_client_alert
		...
		TEST_error("ClientAlert mismatch: expected %s, got %s.", print_alert(test_ctx->expected_client_alert), print_alert(result->client_alert_sent));
		return 0;
	}
	if(test_ctx->expected_server_alert && (result->server_alert_sent & 0xff) != test_ctx->expected_server_alert) {
		TEST_error("ServerAlert mismatch: expected %s, got %s.", print_alert(test_ctx->expected_server_alert), print_alert(result->server_alert_sent));
		return 0;
	}
...
static int check_protocol(HANDSHAKE_RESULT * result, SSL_TEST_CTX * test_ctx)
{
	if(!TEST_int_eq(result->client_protocol, result->server_protocol)) {
		TEST_info("Client has protocol %s but server has %s.", ssl_protocol_name(result->client_protocol), ssl_protocol_name(result->server_protocol));
		return 0;
	}
...
static int check_servername(HANDSHAKE_RESULT * result, SSL_TEST_CTX * test_ctx)
{
	if(!TEST_int_eq(result->servername, test_ctx->expected_servername)) {
		TEST_info("Client ServerName mismatch, expected %s, got %s.", ssl_servername_name(test_ctx->expected_servername), ssl_servername_name(result->servername));
		return 0;
	}
	return 1;
}
...
static int check_session_ticket(HANDSHAKE_RESULT * result, SSL_TEST_CTX * test_ctx)
{
	...
	if(!TEST_int_eq(result->session_ticket, test_ctx->session_ticket_expected)) {
		TEST_info("Client SessionTicketExpected mismatch, expected %s, got %s.", ssl_session_ticket_name(test_ctx->session_ticket_expected), ssl_session_ticket_name(result->session_ticket));
		return 0;
	}
	return 1;
}
...
static int test_handshake(int idx)
{
	...
	SSL_CTX * server2_ctx = NULL;
	SSL_CTX * client_ctx = NULL;
	SSL_CTX * resume_server_ctx = NULL;
	SSL_CTX * resume_client_ctx = NULL;
	...
	if(test_ctx->method == SSL_TEST_METHOD_DTLS) {
		...
		if(!TEST_true(SSL_CTX_set_options(server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)) || !TEST_true(SSL_CTX_set_max_proto_version(server_ctx, 0)))
			goto err;
		if(test_ctx->extra.server.servername_callback != SSL_TEST_SERVERNAME_CB_NONE) {
			if(!TEST_ptr(server2_ctx = SSL_CTX_new_ex(libctx, NULL, DTLS_server_method())) || !TEST_true(SSL_CTX_set_options(server2_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
				goto err;
		}
		...
		if(test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
			...
			if(!TEST_true(SSL_CTX_set_max_proto_version(resume_server_ctx, 0)) || !TEST_true(SSL_CTX_set_options(resume_server_ctx,
	...
	if(test_ctx->method == SSL_TEST_METHOD_TLS) {
		...
		if(!TEST_true(SSL_CTX_set_max_proto_version(server_ctx, maxversion)) || !TEST_true(SSL_CTX_set_options(server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
			goto err;
		/* SNI on resumption isn't supported/tested yet. */
		if(test_ctx->extra.server.servername_callback != SSL_TEST_SERVERNAME_CB_NONE) {
			if(!TEST_ptr(server2_ctx = SSL_CTX_new_ex(libctx, NULL, TLS_server_method())) || !TEST_true(SSL_CTX_set_options(server2_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
				goto err;
		...
		if(test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME) {
			...
			if(!TEST_true(SSL_CTX_set_max_proto_version(resume_server_ctx, maxversion)) || !TEST_true(SSL_CTX_set_options(resume_server_ctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
				goto err;
	...
	if(!TEST_ptr(server_ctx) || !TEST_ptr(client_ctx) || !TEST_int_gt(CONF_modules_load(conf, test_app, 0),  0))
		goto err;
	if(!SSL_CTX_config(server_ctx, "server") || !SSL_CTX_config(client_ctx, "client")) {
		goto err;
	}
	if(server2_ctx != NULL && !SSL_CTX_config(server2_ctx, "server2"))
		goto err;
	if(resume_server_ctx != NULL && !SSL_CTX_config(resume_server_ctx, "resume-server"))
		goto err;
