...
HANDSHAKE_RESULT * HANDSHAKE_RESULT_new(void)
{
	...
	TEST_ptr(ret = (HANDSHAKE_RESULT *)OPENSSL_zalloc(sizeof(*ret)));
	return ret;
}
...
{
	unsigned char * resp;
	resp = (unsigned char *)OPENSSL_malloc(1);
	if(resp == NULL)
		return SSL_TLSEXT_ERR_ALERT_FATAL;
	/*
...
static int broken_session_ticket_cb(SSL * s, unsigned char * key_name, unsigned char * iv, EVP_CIPHER_CTX * ctx, EVP_MAC_CTX * hctx, int enc)
{
	return 0;
}
...
static int parse_protos(const char * protos, unsigned char ** out, size_t * outlen)
{
	...
	if(!TEST_ptr_null(*out)
	    ...
	    || !TEST_ptr(*out = (unsigned char *)OPENSSL_malloc(len + 1)))
		return 0;
	*outlen = len + 1;
...
{
	CTX_DATA * server_ctx_data = (CTX_DATA *)arg;
    ...
    CTX_DATA * client_ctx_data)
{
	...
	if(!TEST_ptr(ticket_keys = (unsigned char *)OPENSSL_zalloc(ticket_key_len))
...
static int create_peer(PEER * peer, SSL_CTX * ctx)
{
	...
	if(!TEST_ptr(ssl = SSL_new(ctx)) || !TEST_ptr(write_buf = (unsigned char *)OPENSSL_zalloc(peer_buffer_size)) || !TEST_ptr(read_buf = (unsigned char *)OPENSSL_zalloc(peer_buffer_size)))
		goto err;
...
static char * dup_str(const unsigned char * in, size_t len)
{
	...
	if(TEST_size_t_eq(/*OPENSSL_strnlen*/sstrnlen((const char *)(in), len), len))
...
static int set_sock_as_sctp(int sock)
{
	 ...
	 */
	memzero(&rto_info, sizeof(struct sctp_rtoinfo));
	...
	memzero(&assocparams, sizeof(struct sctp_assocparams));
    ...
    SSL_SESSION ** session_out, SSL_SESSION ** serv_sess_out)
{
	...
	if(!ret)
		return NULL;
	memzero(&server_ctx_data, sizeof(server_ctx_data));
	memzero(&server2_ctx_data, sizeof(server2_ctx_data));
	memzero(&client_ctx_data, sizeof(client_ctx_data));
	memzero(&server, sizeof(server));
	memzero(&client, sizeof(client));
	memzero(&server_ex_data, sizeof(server_ex_data));
	memzero(&client_ex_data, sizeof(client_ex_data));
	if(!configure_handshake_ctx(server_ctx, server2_ctx, client_ctx, test_ctx, extra, &server_ctx_data, &server2_ctx_data, &client_ctx_data)) {
