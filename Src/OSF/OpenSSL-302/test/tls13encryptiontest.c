...
static unsigned char * multihexstr2buf(const char * str[3], size_t * len)
{
	...
	totlen /= 2;
	outbuf = (unsigned char *)OPENSSL_malloc(totlen);
	if(outbuf == NULL)
		return NULL;
    ...
    unsigned char * iv, size_t ivlen, unsigned char * seq)
{
	...
	*key = OPENSSL_hexstr2buf(recd->key, NULL);
	...
	if(*key == NULL || ivtmp == NULL || sq == NULL || pt == NULL)
		goto err;
	rec->data = rec->input = (unsigned char *)OPENSSL_malloc(ptlen + EVP_GCM_TLS_TAG_LEN);
	if(rec->data == NULL)
		goto err;
	rec->length = ptlen;
...
static int test_tls13_encryption(void)
{
	...
	for(ctr = 0; ctr < SIZEOFARRAY(refdata); ctr++) {
		/* Load the record */
