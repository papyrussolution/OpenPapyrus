		...
		if(strcmp(p->key, "greeting") == 0) {
			...
			if(c_get_params(hand, counter_request)) {
				if(greeting) {
					...
					const char * versionp = *(const char **)counter_request[0].data;
					const char * namep = *(const char **)counter_request[1].data;
					sprintf(buf, "Hello OpenSSL %.20s, greetings from %s!", versionp, namep);
				}
			}
			else {
...
int OSSL_provider_init(const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * oin, const OSSL_DISPATCH ** out, void ** provctx)
{
	P_TEST_CTX * ctx;
	 ...
	 */
	ctx = (P_TEST_CTX *)malloc(sizeof(*ctx));
	if(!ctx)
		return 0;
...
#endif
	SAlloc::F(ctx->thisfile);
	SAlloc::F(ctx->thisfunc);
	SAlloc::F(ctx);
}
