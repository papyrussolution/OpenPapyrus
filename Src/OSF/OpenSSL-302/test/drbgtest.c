...
static int test_drbg_reseed(int expect_success, EVP_RAND_CTX * primary, EVP_RAND_CTX * pPublic, EVP_RAND_CTX * pPrivate,
    unsigned char * public_random, unsigned char * private_random, int expect_primary_reseed, int expect_public_reseed,
    int expect_private_reseed, time_t reseed_when)
{
	...
	if(!TEST_int_eq(rand_bytes((unsigned char *)public_random, RANDOM_SIZE), expect_success) || 
		!TEST_int_eq(rand_priv_bytes((unsigned char *)private_random, RANDOM_SIZE), expect_success))
		return 0;
	...
	if(expect_public_reseed >= 0) {
		...
		if(!TEST_int_ge(reseed_counter(pPublic), public_reseed) || !TEST_uint_ge(reseed_counter(pPublic),
		    reseed_counter(primary)))
			return 0;
	}
	if(expect_private_reseed >= 0) {
		/* Test whether pPublic DRBG was reseeded as expected */
		if(!TEST_int_ge(reseed_counter(pPrivate), private_reseed) || !TEST_uint_ge(reseed_counter(pPrivate),
		    reseed_counter(primary)))
			return 0;
	}
	if(expect_success == 1) {
