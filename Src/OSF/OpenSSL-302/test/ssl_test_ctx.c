...
{
	int ret_value;
	if(!parse_enum(ssl_test_results, SIZEOFARRAY(ssl_test_results), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_test_results, SIZEOFARRAY(ssl_test_results), result);
}
...
{
	return parse_enum(ssl_alerts, SIZEOFARRAY(ssl_alerts), alert, value);
}
...
{
	return enum_name(ssl_alerts, SIZEOFARRAY(ssl_alerts), alert);
}
...
{
	return parse_enum(ssl_protocols, SIZEOFARRAY(ssl_protocols),
		   &test_ctx->expected_protocol, value);
}

const char * ssl_protocol_name(int protocol)
{
	return enum_name(ssl_protocols, SIZEOFARRAY(ssl_protocols), protocol);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_verify_callbacks, SIZEOFARRAY(ssl_verify_callbacks), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_verify_callbacks, SIZEOFARRAY(ssl_verify_callbacks), callback);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_servername, SIZEOFARRAY(ssl_servername),
	    &ret_value, value)) {
		return 0;
	}
...
{
	int ret_value;
	if(!parse_enum(ssl_servername, SIZEOFARRAY(ssl_servername), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_servername, SIZEOFARRAY(ssl_servername), server);
}
...
__owur static int parse_servername_callback(SSL_TEST_SERVER_CONF * server_conf, const char * value)
{
	...
	if(!parse_enum(ssl_servername_callbacks,
	    SIZEOFARRAY(ssl_servername_callbacks), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_servername_callbacks, SIZEOFARRAY(ssl_servername_callbacks), callback);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_session_ticket, SIZEOFARRAY(ssl_session_ticket), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_session_ticket, SIZEOFARRAY(ssl_session_ticket), server);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_session_id, SIZEOFARRAY(ssl_session_id), &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_session_id, SIZEOFARRAY(ssl_session_id), server);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_test_methods, SIZEOFARRAY(ssl_test_methods),
	    &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_test_methods, SIZEOFARRAY(ssl_test_methods), method);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_handshake_modes, SIZEOFARRAY(ssl_handshake_modes),
	    &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_handshake_modes, SIZEOFARRAY(ssl_handshake_modes), mode);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_key_update_types, SIZEOFARRAY(ssl_key_update_types),
	    &ret_value, value)) {
		return 0;
	}
...
{
	int ret_value;
	if(!parse_enum(ssl_ct_validation_modes, SIZEOFARRAY(ssl_ct_validation_modes),
	    &ret_value, value)) {
		return 0;
	}
...
{
	return enum_name(ssl_ct_validation_modes, SIZEOFARRAY(ssl_ct_validation_modes),
		   mode);
}
...
{
	int ret_value;
	if(!parse_enum(ssl_certstatus, SIZEOFARRAY(ssl_certstatus), &ret_value,
	    value)) {
		return 0;
	}
...
{
	return enum_name(ssl_certstatus, SIZEOFARRAY(ssl_certstatus), cert_status);
}
    ...
    const char * value)
{
	...
	if(!parse_enum(ssl_max_fragment_len_mode,
	    SIZEOFARRAY(ssl_max_fragment_len_mode), &ret_value, value)) {
		return 0;
	}
...
const char * ssl_max_fragment_len_name(int MFL_mode)
{
	return enum_name(ssl_max_fragment_len_mode,
		   SIZEOFARRAY(ssl_max_fragment_len_mode), MFL_mode);
}
    ...
    const char * client_section)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		...
		for(j = 0; j < SIZEOFARRAY(ssl_test_client_options); j++) {
    ...
    const char * server_section)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		int found = 0;
		const CONF_VALUE * option = sk_CONF_VALUE_value(sk_conf, i);
		for(j = 0; j < SIZEOFARRAY(ssl_test_server_options); j++) {
...
SSL_TEST_CTX * SSL_TEST_CTX_create(const CONF * conf, const char * test_section, OSSL_LIB_CTX * libctx)
{
	...
	for(i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
		...
		if(strcmp(option->name, "client") == 0) {
			...
			for(j = 0; j < SIZEOFARRAY(ssl_test_ctx_options); j++) {
