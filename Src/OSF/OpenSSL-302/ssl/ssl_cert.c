...
CERT * ssl_cert_dup(CERT * cert)
{
	...
	for(i = 0; i < SSL_PKEY_NUM; i++) {
		...
		if(cert->pkeys[i].serverinfo != NULL) {
			...
			ret->pkeys[i].serverinfo = (uchar *)OPENSSL_malloc(cert->pkeys[i].serverinfo_length);
	...
	if(cert->conf_sigalgs) {
		ret->conf_sigalgs = (uint16_t *)OPENSSL_malloc(cert->conf_sigalgslen * sizeof(*cert->conf_sigalgs));
	...
	if(cert->client_sigalgs) {
		ret->client_sigalgs = (uint16_t *)OPENSSL_malloc(cert->client_sigalgslen * sizeof(*cert->client_sigalgs));
...
{
	int level, pfs_mask;
	int minbits = ssl_get_security_level_bits(s, ctx, &level);
	if(level == 0) {
		/*
...
int ssl_cert_lookup_by_nid(int nid, size_t * pidx)
{
	...
	for(i = 0; i < SIZEOFARRAY(ssl_cert_info); i++) {
