...
static int ssl_set_pkey(CERT * c, EVP_PKEY * pkey)
{
	...
	if(c->pkeys[i].x509 && !X509_check_private_key(c->pkeys[i].x509, pkey))
		return 0;
...
static int use_certificate_chain_file(SSL_CTX * ctx, SSL * ssl, const char * file)
{
	...
	if(ret) {
		...
		while(1) {
			...
			if(PEM_read_bio_X509(in, &ca, passwd_callback, passwd_callback_userdata)) {
				if(ctx)
...
static int serverinfo_find_extension(const uchar * serverinfo, size_t serverinfo_length, unsigned int extension_type, const uchar ** extension_data, size_t * extension_length)
{
	PACKET pkt, data;
	*extension_data = NULL;
	*extension_length = 0;
...
static int serverinfoex_srv_parse_cb(SSL * s, unsigned int ext_type, unsigned int context, const uchar * in, size_t inlen, X509 * x, size_t chainidx, int * al, void * arg)
{
	if(inlen != 0) {
...
static int serverinfo_srv_parse_cb(SSL * s, unsigned int ext_type, const uchar * in, size_t inlen, int * al, void * arg)
{
	return serverinfoex_srv_parse_cb(s, ext_type, 0, in, inlen, NULL, 0, al, arg);
}

static int serverinfoex_srv_add_cb(SSL * s, unsigned int ext_type, unsigned int context, const uchar ** out, size_t * outlen, X509 * x, size_t chainidx, int * al, void * arg)
{
...
static int serverinfo_srv_add_cb(SSL * s, unsigned int ext_type, const uchar ** out, size_t * outlen, int * al, void * arg)
{
 ...
 */
static int serverinfo_process_buffer(unsigned int version, const uchar * serverinfo, size_t serverinfo_length, SSL_CTX * ctx)
{
	PACKET pkt;
	if(serverinfo == NULL || serverinfo_length == 0)
		return 0;
	...
	while(PACKET_remaining(&pkt)) {
		...
		PACKET data;
		if((version == SSL_SERVERINFOV2 && !PACKET_get_net_4(&pkt, &context)) || !PACKET_get_net_2(&pkt, &ext_type) || !PACKET_get_length_prefixed_2(&pkt, &data))
			return 0;
		if(!ctx)
			continue;
		/*
...
int SSL_CTX_use_serverinfo_ex(SSL_CTX * ctx, unsigned int version, const uchar * serverinfo, size_t serverinfo_length)
{
...
int SSL_CTX_use_serverinfo_file(SSL_CTX * ctx, const char * file)
{
	...
	for(num_extensions = 0;; num_extensions++) {
		...
		memcpy(serverinfo + serverinfo_length + contextoff, extension, extension_length);
...
static int ssl_set_cert_and_key(SSL * ssl, SSL_CTX * ctx, X509 * x509, EVP_PKEY * privatekey, STACK_OF(X509) * chain, int override)
{
	...
	if(!override && (c->pkeys[i].x509 || c->pkeys[i].privatekey || c->pkeys[i].chain)) {
	...
	// @openssl305 (will be) OSSL_STACK_OF_X509_free(c->pkeys[i].chain); 
