...
{
	SSL_SESSION * dest = (SSL_SESSION *)OPENSSL_malloc(sizeof(*dest));
	if(dest == NULL) {
		goto err;
	}
	...
	if(ticket != 0 && src->ext.tick != NULL) {
		dest->ext.tick = (uchar *)OPENSSL_memdup(src->ext.tick, src->ext.ticklen);
	...
	if(src->ext.alpn_selected != NULL) {
		dest->ext.alpn_selected = (uchar *)OPENSSL_memdup(src->ext.alpn_selected, src->ext.alpn_selected_len);
	...
	if(src->ticket_appdata != NULL) {
		dest->ticket_appdata = (uchar *)OPENSSL_memdup(src->ticket_appdata, src->ticket_appdata_len);
...
const uchar * SSL_SESSION_get_id(const SSL_SESSION * s, unsigned int * len)
{
	if(len)
...
const uchar * SSL_SESSION_get0_id_context(const SSL_SESSION * s, unsigned int * len)
{
	if(len != NULL)
...
void SSL_SESSION_get0_alpn_selected(const SSL_SESSION * s, const uchar ** alpn, size_t * len)
{
...
int SSL_SESSION_set1_alpn_selected(SSL_SESSION * s, const uchar * alpn, size_t len)
{
	...
	s->ext.alpn_selected = (uchar *)OPENSSL_memdup(alpn, len);
...
int SSL_SESSION_set1_id_context(SSL_SESSION * s, const uchar * sid_ctx, unsigned int sid_ctx_len)
{
...
int SSL_SESSION_is_resumable(const SSL_SESSION * s)
{
	 ...
	 * session ID.
	 */
	return !s->not_resumable && (s->session_id_length > 0 || s->ext.ticklen > 0);
}
...
int SSL_set_session_ticket_ext(SSL * s, const void * ext_data, int ext_len)
{
	if(s->version >= TLS1_VERSION) {
		...
		s->ext.session_ticket = (TLS_SESSION_TICKET_EXT *)OPENSSL_malloc(sizeof(TLS_SESSION_TICKET_EXT) + ext_len);
...
void SSL_CTX_set_cookie_generate_cb(SSL_CTX * ctx, int (*cb)(SSL * ssl, uchar * cookie, unsigned int * cookie_len))
{
	ctx->app_gen_cookie_cb = cb;
}

void SSL_CTX_set_cookie_verify_cb(SSL_CTX * ctx, int (*cb)(SSL * ssl, const uchar * cookie, unsigned int cookie_len))
{
...
int SSL_SESSION_set1_ticket_appdata(SSL_SESSION * ss, const void * data, size_t len)
{
	...
	ss->ticket_appdata = (uchar *)OPENSSL_memdup(data, len);
...
void SSL_CTX_set_stateless_cookie_generate_cb(SSL_CTX * ctx, int (*cb)(SSL * ssl, uchar * cookie, size_t * cookie_len))
{
	ctx->gen_stateless_cookie_cb = cb;
}

void SSL_CTX_set_stateless_cookie_verify_cb(SSL_CTX * ctx, int (*cb)(SSL * ssl, const uchar * cookie, size_t cookie_len))
{
