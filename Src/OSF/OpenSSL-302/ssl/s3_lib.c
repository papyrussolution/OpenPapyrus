...
long ssl3_ctrl(SSL * s, int cmd, long larg, void * parg)
{
	...
	switch(cmd) {
		...
		case SSL_CTRL_SET_GROUPS:
		    return tls1_set_groups(&s->ext.supportedgroups, &s->ext.supportedgroups_len, (int *)parg, larg);
		case SSL_CTRL_SET_GROUPS_LIST:
		    return tls1_set_groups_list(s->ctx, &s->ext.supportedgroups, &s->ext.supportedgroups_len, (const char *)parg);
		...
		case SSL_CTRL_SET_CLIENT_CERT_TYPES:
		    if(!s->server)
			    return 0;
		    return ssl3_set_req_cert_type(s->cert, (const uchar *)parg, larg);
	...
	switch(cmd) {
		...
		case SSL_CTRL_GET_TLSEXT_TICKET_KEYS:
	    {
		    ...
		    if(cmd == SSL_CTRL_SET_TLSEXT_TICKET_KEYS) {
			    ...
			    memcpy(ctx->ext.secure->tick_hmac_key, keys + sizeof(ctx->ext.tick_key_name), sizeof(ctx->ext.secure->tick_hmac_key));
			    memcpy(ctx->ext.secure->tick_aes_key, keys + sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key), sizeof(ctx->ext.secure->tick_aes_key));
		    }
		    else {
			    ...
			    memcpy(keys + sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key), ctx->ext.secure->tick_aes_key, sizeof(ctx->ext.secure->tick_aes_key));
		    }
		    return 1;
	    }
		case SSL_CTRL_GET_TLSEXT_STATUS_REQ_TYPE:
...
#endif
		case SSL_CTRL_SET_GROUPS:
		    return tls1_set_groups(&ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, (int *)parg, larg);
		case SSL_CTRL_SET_GROUPS_LIST:
		    return tls1_set_groups_list(ctx, &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, (const char *)parg);
		case SSL_CTRL_SET_SIGALGS:
		...
		case SSL_CTRL_SET_CLIENT_CERT_TYPES:
		    return ssl3_set_req_cert_type(ctx->cert, (const uchar *)parg, larg);
		case SSL_CTRL_BUILD_CERT_CHAIN:
...
int SSL_CTX_set_tlsext_ticket_key_evp_cb(SSL_CTX * ctx, int (*fp)(SSL *, uchar *, uchar *,
...
static int ssl3_read_internal(SSL * s, void * buf, size_t len, int peek, size_t * readbytes)
{
	...
	ret = s->method->ssl_read_bytes(s, SSL3_RT_APPLICATION_DATA, NULL, (uchar *)buf, len, peek, readbytes);
...
int ssl_gensecret(SSL * s, uchar * pms, size_t pmslen)
{
	int rv = 0;
...
int ssl_encapsulate(SSL * s, EVP_PKEY * pubkey,
    uchar ** ctp, size_t * ctlenp,
    int gensecret)
{
	int rv = 0;
	uchar * pms = NULL, * ct = NULL;
