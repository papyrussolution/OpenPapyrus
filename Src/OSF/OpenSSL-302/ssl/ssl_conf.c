...
static void ssl_set_option(SSL_CONF_CTX * cctx, unsigned int name_flags, uint64_t option_value, int onoff)
{
	uint32_t * pflags;
...
static int cmd_VerifyMode(SSL_CONF_CTX * cctx, const char * value)
{
	static const ssl_flag_tbl ssl_vfy_list[] = {
		...
		SSL_FLAG_VFY_SRV("RequirePostHandshake", SSL_VERIFY_PEER | SSL_VERIFY_POST_HANDSHAKE | SSL_VERIFY_FAIL_IF_NO_PEER_CERT),
	};
	if(value == NULL)
		return -3;
...
{
	if(ssl_conf_cmd_skip_prefix(cctx, &cmd)) {
		const ssl_conf_cmd_tbl * runcmd = ssl_conf_cmd_lookup(cctx, cmd);
		if(runcmd)
