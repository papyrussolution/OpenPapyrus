...
{
	struct provider_group_data_st * pgd = (struct provider_group_data_st *)data;
	SSL_CTX * ctx = pgd->ctx;
	...
	ginf->tlsname = OPENSSL_strdup((const char *)p->data);
	...
	ginf->realname = OPENSSL_strdup((const char *)p->data);
	...
	ginf->algorithm = OPENSSL_strdup((const char *)p->data);
	...
	if(p && (!OSSL_PARAM_get_uint(p, &is_kem) || is_kem > 1)) {
		ERR_raise(ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT);
		goto err;
	}
...
int ssl_load_groups(SSL_CTX * ctx)
{
	...
	uint16_t tmp_supp_groups[SIZEOFARRAY(supported_groups_default)];
	if(!OSSL_PROVIDER_do_all(ctx->libctx, discover_provider_groups, ctx))
		return 0;
	for(i = 0; i < SIZEOFARRAY(supported_groups_default); i++) {
	...
	if(num_deflt_grps == 0)
		return 1;
	ctx->ext.supported_groups_default = (uint16_t *)OPENSSL_malloc(sizeof(uint16_t) * num_deflt_grps);
...
int tls1_group_id2nid(uint16_t group_id, int include_unknown)
{
	 ...
	 */
	for(i = 0; i < SIZEOFARRAY(nid_to_group); i++) {
    ...
    int isec, int * okfortls13)
{
	...
	if(SSL_IS_DTLS(s)) {
		...
		if(ret && okfortls13 && maxversion == TLS1_3_VERSION)
    ...
    const char * str)
{
	...
	gcb.gid_arr = (uint16_t *)OPENSSL_malloc(gcb.gidmax * sizeof(*gcb.gid_arr));
	if(gcb.gid_arr == NULL)
		return 0;
	gcb.ctx = ctx;
	 ...
	 * the result
	 */
	tmparr = (uint16_t *)OPENSSL_memdup(gcb.gid_arr, gcb.gidcnt * sizeof(*tmparr));
	if(tmparr == NULL)
		goto end;
	*pext = tmparr;
	*pextlen = gcb.gidcnt;
...
int ssl_setup_sig_algs(SSL_CTX * ctx)
{
	...
	SIGALG_LOOKUP * cache = (SIGALG_LOOKUP *)OPENSSL_malloc(sizeof(*lu) * SIZEOFARRAY(sigalg_lookup_tbl));
	EVP_PKEY * tmpkey = EVP_PKEY_new();
	int ret = 0;
	if(cache == NULL || tmpkey == NULL)
		goto err;
	ERR_set_mark();
	for(i = 0, lu = sigalg_lookup_tbl; i < SIZEOFARRAY(sigalg_lookup_tbl); lu++, i++) {
		EVP_PKEY_CTX * pctx;
		cache[i] = *lu;
		/*
...
int tls12_check_peer_sigalg(SSL * s, uint16_t sig, EVP_PKEY * pkey)
{
	...
	if(secbits == 0 || !ssl_security(s, SSL_SECOP_SIGALG_CHECK, secbits, md ? EVP_MD_get_type(md) : NID_undef, (void*)sigalgstr)) {
		SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_WRONG_SIGNATURE_TYPE);
		return 0;
	}
    ...
    size_t sesslen, SSL_SESSION ** psess)
{
	...
	if(tctx->ext.ticket_key_evp_cb || tctx->ext.ticket_key_cb)
#else
	if(tctx->ext.ticket_key_evp_cb)
#endif
	{
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
		else if(tctx->ext.ticket_key_cb)
...
static int tls1_check_sig_alg(SSL * s, X509 * x, int default_nid)
{
	...
	for(i = 0; i < sigalgslen; i++) {
		...
		if(sigalg && sig_nid == sigalg->sigandhash)
			return 1;
	}
	return 0;
}
...
SSL_HMAC * ssl_hmac_new(const SSL_CTX * ctx)
{
	...
	if(!ret)
		return NULL;
#ifndef OPENSSL_NO_DEPRECATED_3_0
	if(ctx->ext.ticket_key_evp_cb == NULL && ctx->ext.ticket_key_cb) {
...
int ssl_hmac_init(SSL_HMAC * ctx, void * key, size_t len, char * md)
{
	...
	if(ctx->ctx) {
		...
		*p = OSSL_PARAM_construct_end();
		if(EVP_MAC_init(ctx->ctx, (const uchar *)key, len, params))
			return 1;
	}
#ifndef OPENSSL_NO_DEPRECATED_3_0
	if(ctx->old_ctx)
...
int ssl_hmac_update(SSL_HMAC * ctx, const uchar * data, size_t len)
{
	if(ctx->ctx)
...
int ssl_hmac_final(SSL_HMAC * ctx, uchar * md, size_t * len, size_t max_size)
{
	if(ctx->ctx)
...
__owur int tls13_set_encoded_pub_key(EVP_PKEY * pkey, const uchar * enckey, size_t enckeylen)
{
	if(EVP_PKEY_is_a(pkey, "DH")) {
		...
		if(enckeylen < 3 /* point format and at least 1 byte for x and y */ || enckey[0] != 0x04)
			return 0;
	}
