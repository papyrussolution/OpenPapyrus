...
#pragma hdrstop
#define OPENSSL_SUPPRESS_DEPRECATED // We need to use some engine and HMAC deprecated APIs 
...
int ssl_hmac_old_update(SSL_HMAC * ctx, const uchar * data, size_t len)
{
	return HMAC_Update(ctx->old_ctx, data, len);
}

int ssl_hmac_old_final(SSL_HMAC * ctx, uchar * md, size_t * len)
{
	unsigned int l;
