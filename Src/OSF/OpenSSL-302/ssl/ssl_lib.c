...
static int ssl_undefined_function_2(SSL * ssl, SSL3_RECORD * r, uchar * s, int t)
{
	return ssl_undefined_function(ssl);
}

static int ssl_undefined_function_3(SSL * ssl, uchar * r, uchar * s, size_t t, size_t * u)
{
	return ssl_undefined_function(ssl);
}
...
static size_t ssl_undefined_function_5(SSL * ssl, const char * r, size_t s, uchar * t)
{
	return ssl_undefined_function(ssl);
}
...
static int ssl_undefined_function_7(SSL * ssl, uchar * r, size_t s, const char * t, size_t u, const uchar * v, size_t w, int x)
{
	return ssl_undefined_function(ssl);
}
...
static int dane_tlsa_add(SSL_DANE * dane,
    uint8_t usage,
    uint8_t selector,
    uint8_t mtype, const uchar * data, size_t dlen)
{
	danetls_record * t;
	...
	t->selector = selector;
	t->mtype = mtype;
	t->data = (uchar *)OPENSSL_malloc(dlen);
	if(t->data == NULL) {
		tlsa_free(t);
		ERR_raise(ERR_LIB_SSL, ERR_R_MALLOC_FAILURE);
		return -1;
...
SSL * SSL_new(SSL_CTX * ctx)
{
	...
	if(ctx->ext.ecpointformats) {
		s->ext.ecpointformats = (uchar *)OPENSSL_memdup(ctx->ext.ecpointformats, ctx->ext.ecpointformats_len);
	...
	if(ctx->ext.supportedgroups) {
		s->ext.supportedgroups = (uint16_t *)OPENSSL_memdup(ctx->ext.supportedgroups, ctx->ext.supportedgroups_len * sizeof(*ctx->ext.supportedgroups));
...
#endif
	if(s->ctx->ext.alpn) {
		s->ext.alpn = (uchar *)OPENSSL_malloc(s->ctx->ext.alpn_len);
		if(s->ext.alpn == NULL) {
			s->ext.alpn_len = 0;
			goto err;
		}
...
int SSL_CTX_set_session_id_context(SSL_CTX * ctx, const uchar * sid_ctx,
...
int SSL_set_session_id_context(SSL * ssl, const uchar * sid_ctx,
    unsigned int sid_ctx_len)
{
	if(sid_ctx_len > SSL_MAX_SID_CTX_LENGTH) {
		ERR_raise(ERR_LIB_SSL, SSL_R_SSL_SESSION_ID_CONTEXT_TOO_LONG);
	...
	if(!ctx) {
		switch(cmd) {
			case SSL_CTRL_SET_GROUPS_LIST:
			    return tls1_set_groups_list(ctx, NULL, NULL, (const char *)parg);
 ...
 */
int SSL_select_next_proto(uchar ** out, uchar * outlen,
    const uchar * server,
    unsigned int server_len,
    const uchar * client, unsigned int client_len)
{
	unsigned int i, j;
	const uchar * result;
 ...
 */
void SSL_get0_next_proto_negotiated(const SSL * s, const uchar ** data,
    unsigned * len)
{
	*data = s->ext.npn;
	if(*data == NULL) {
...
static int alpn_value_ok(const uchar * protos, unsigned int protos_len)
{
	unsigned int idx;
    ...
    unsigned int protos_len)
{
	...
	if(!alpn_value_ok(protos, protos_len))
		return 1;
	alpn = (uchar *)OPENSSL_memdup(protos, protos_len);
	if(alpn == NULL) {
		ERR_raise(ERR_LIB_SSL, ERR_R_MALLOC_FAILURE);
		return 1;
	}
 ...
 * length-prefixed strings). Returns 0 on success.
 */
int SSL_set_alpn_protos(SSL * ssl, const uchar * protos, unsigned int protos_len)
{
	uchar * alpn;
	if(protos_len == 0 || protos == NULL) {
...
int SSL_export_keying_material(SSL * s, uchar * out, size_t olen,
    ...
    const uchar * context, size_t contextlen,
    int use_context)
{
...
int SSL_export_keying_material_early(SSL * s, uchar * out, size_t olen,
    const char * label, size_t llen, const uchar * context, size_t contextlen)
{
...
int ssl_get_server_cert_serverinfo(SSL * s, const uchar ** serverinfo,
...
void   SSL_CTX_set_quiet_shutdown(SSL_CTX * ctx, int mode) { ctx->quiet_shutdown = mode; }
...
int    SSL_client_version(const SSL * s) { return s->client_version; }
...
{
	X509_LOOKUP * lookup = X509_STORE_add_lookup(ctx->cert_store, X509_LOOKUP_store());
	if(!lookup)
		return 0;
	/* We ignore errors, in case the directory doesn't exist */
	ERR_set_mark();
...
int ssl_handshake_hash(SSL * s, uchar * out, size_t outlen,
    size_t * hashlen)
{
	EVP_MD_CTX * ctx = NULL;
...
int SSL_client_hello_get0_ext(SSL * s, unsigned int type, const uchar ** out, size_t * outlen)
{
	size_t i;
	RAW_EXTENSION * r;
	if(s->clienthello == NULL)
...
int ssl_cache_cipherlist(SSL * s, PACKET * cipher_suites, int sslv2format)
{
	...
	if(sslv2format) {
		 ...
		 * problem.
		 */
		raw = (uchar *)OPENSSL_malloc(numciphers * TLS_CIPHER_LEN);
...
int SSL_bytes_to_cipher_list(SSL * s, const uchar * bytes, size_t len,
