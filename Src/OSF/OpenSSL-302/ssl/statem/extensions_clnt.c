    ...
    size_t chainidx)
{
	...
	if(psksess != NULL) {
		...
		s->psksession_id = (uchar *)OPENSSL_memdup(id, idlen);
    ...
    X509 * x, size_t chainidx)
{
	...
	if(!s->hit) {
		...
		s->ext.peer_ecpointformats = (uchar *)OPENSSL_malloc(ecpointformats_len);
...
{
	if(s->ext.session_ticket_cb != NULL && !s->ext.session_ticket_cb(s, PACKET_data(pkt),
    ...
    size_t chainidx)
{
	...
	if(s->ct_validation_callback != NULL) {
		...
		if(size > 0) {
			s->ext.scts = (uchar *)OPENSSL_malloc(size);
			if(s->ext.scts == NULL) {
				s->ext.scts_len = 0;
    ...
    size_t chainidx)
{
	...
	s->ext.npn = (uchar *)OPENSSL_malloc(selected_len);
	if(s->ext.npn == NULL) {
		s->ext.npn_len = 0;
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		return 0;
    ...
    size_t chainidx)
{
	...
	if(!ginf->is_kem) {
		...
		/* KEM Mode */
		const uchar * ct = PACKET_data(&encoded_pt);
