...
static WRITE_TRAN ossl_statem_server13_write_transition(SSL * s)
{
	...
	switch(st->hand_state) {
		...
		case TLS_ST_SW_SRVR_HELLO:
		    if((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0 && s->hello_retry_request != ssl_st::SSL_HRR_COMPLETE)
...
WORK_STATE ossl_statem_server_post_work(SSL * s, WORK_STATE wst)
{
	...
	switch(st->hand_state) {
		...
		case TLS_ST_SW_CHANGE:
		    ...
		    if(SSL_IS_TLS13(s)) {
			    if(!s->method->ssl3_enc->setup_key_block(s) || !s->method->ssl3_enc->change_cipher_state(s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_SERVER_WRITE)) {
				    /* SSLfatal() already called */
				    return WORK_ERROR;
			    }
			    if(s->ext.early_data != SSL_EARLY_DATA_ACCEPTED && !s->method->ssl3_enc->change_cipher_state(s, SSL3_CC_HANDSHAKE |SSL3_CHANGE_CIPHER_SERVER_READ)) {
				    /* SSLfatal() already called */
				    return WORK_ERROR;
			    }
			    /*
...
int dtls_raw_hello_verify_request(WPACKET * pkt, uchar * cookie,
    size_t cookie_len)
{
...
static void ssl_check_for_safari(SSL * s, const CLIENTHELLO_MSG * hello)
{
	...
	s->s3.is_probably_safari = PACKET_equal(&tmppkt, kSafariExtensionsBlock, ext_len);
}

#define RENEG_OPTIONS_OK(options) ((options & SSL_OP_NO_RENEGOTIATION) == 0 && (options & SSL_OP_ALLOW_CLIENT_RENEGOTIATION) != 0)
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL * s, PACKET * pkt)
{
	...
	if(s->renegotiate == 0 && !SSL_IS_FIRST_HANDSHAKE(s)) {
		...
		if(!RENEG_OPTIONS_OK(s->options) || (!s->s3.send_connection_binding && (s->options & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION) == 0)) {
	...
	if(clienthello->isv2) {
		unsigned int mt;
		if(!SSL_IS_FIRST_HANDSHAKE(s) || s->hello_retry_request != ssl_st::SSL_HRR_NONE) {
...
int tls_handle_alpn(SSL * s)
{
	...
	if(s->ctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
		int r = s->ctx->ext.alpn_select_cb(s, &selected, &selected_len, s->s3.alpn_proposed, (unsigned int)s->s3.alpn_proposed_len, s->ctx->ext.alpn_select_cb_arg);
		...
		if(r == SSL_TLSEXT_ERR_OK) {
			...
			if(s->session->ext.alpn_selected == NULL
				...
				if(!s->hit) {
					...
					s->session->ext.alpn_selected = (uchar *)OPENSSL_memdup(selected, selected_len);
					if(s->session->ext.alpn_selected == NULL) {
						SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
						return 0;
					}
...
int tls_construct_server_hello(SSL * s, WPACKET * pkt)
{
	...
	if(!WPACKET_put_bytes_u16(pkt, version)
	     ...
	     */
	    || !WPACKET_memcpy(pkt, s->hello_retry_request == ssl_st::SSL_HRR_PENDING ? hrrrandom : s->s3.server_random,
	    SSL3_RANDOM_SIZE)) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		return 0;
	}
	...
	if(!tls_construct_extensions(s, pkt, s->hello_retry_request == ssl_st::SSL_HRR_PENDING ? SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST : 
		(SSL_IS_TLS13(s) ? SSL_EXT_TLS1_3_SERVER_HELLO : SSL_EXT_TLS1_2_SERVER_HELLO), NULL, 0)) {
		/* SSLfatal() already called */
		return 0;
	}
	if(s->hello_retry_request == ssl_st::SSL_HRR_PENDING) {
...
static int tls_process_cke_rsa(SSL * s, PACKET * pkt)
{
	...
	rsa_decrypt = (uchar *)OPENSSL_malloc(outlen);
	if(rsa_decrypt == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	 ...
	 */
	if(EVP_PKEY_decrypt_init(ctx) <= 0 || EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_WITH_TLS_PADDING) <= 0) {
    ...
    uchar * tick_nonce)
{
	...
	SSL_HMAC * hctx = NULL;
	uchar * p, * encdata1, * encdata2, * macdata1, * macdata2;
	const uchar * const_p;
	...
	senc = (uchar *)OPENSSL_malloc(slen_full);
	if(senc == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
#endif
	{
		...
		EVP_CIPHER * cipher = EVP_CIPHER_fetch(s->ctx->libctx, "AES-256-CBC", s->ctx->propq);
		if(cipher == NULL) {
		...
		if(iv_len < 0 || RAND_bytes_ex(s->ctx->libctx, iv, iv_len, 0) <= 0 || !EVP_EncryptInit_ex(ctx, cipher, NULL, tctx->ext.secure->tick_aes_key, iv)
		    || !ssl_hmac_init(hctx, tctx->ext.secure->tick_hmac_key, sizeof(tctx->ext.secure->tick_hmac_key), "SHA256")) {
...
int tls_construct_new_session_ticket(SSL * s, WPACKET * pkt)
{
	...
	if(SSL_IS_TLS13(s)) {
		...
		if(s->s3.alpn_selected != NULL) {
			...
			s->session->ext.alpn_selected = (uchar *)OPENSSL_memdup(s->s3.alpn_selected, s->s3.alpn_selected_len);
			if(s->session->ext.alpn_selected == NULL) {
