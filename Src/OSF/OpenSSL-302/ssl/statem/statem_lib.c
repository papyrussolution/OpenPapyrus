...
int tls_construct_cert_verify(SSL * s, WPACKET * pkt)
{
	...
	if(s->version == SSL3_VERSION) {
		 ...
		 */
		if(EVP_DigestSignUpdate(mctx, hdata, hdatalen) <= 0 || EVP_MD_CTX_ctrl(mctx, EVP_CTRL_SSL3_MASTER_SECRET,
		    (int)s->session->master_key_length, s->session->master_key) <= 0 || EVP_DigestSignFinal(mctx, NULL, &siglen) <= 0) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
			goto err;
		}
		 ...
		 */
		if(EVP_DigestSign(mctx, NULL, &siglen, (const uchar *)hdata, hdatalen) <= 0) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
			goto err;
		}
		sig = (uchar *)OPENSSL_malloc(siglen);
		if(sig == NULL || EVP_DigestSign(mctx, sig, &siglen, (const uchar *)hdata, hdatalen) <= 0) {
			SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
			goto err;
		}
	}
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL * s, PACKET * pkt)
{
	...
	if(s->version == SSL3_VERSION) {
		...
		j = EVP_DigestVerify(mctx, data, len, (const uchar *)hdata, hdatalen);
		if(j <= 0) {
			SSLfatal(s, SSL_AD_DECRYPT_ERROR, SSL_R_BAD_SIGNATURE);
			goto err;
		}
...
size_t construct_key_exchange_tbs(SSL * s, uchar ** ptbs, const void * param, size_t paramlen)
{
	...
	uchar * tbs = (uchar *)OPENSSL_malloc(tbslen);
	if(tbs == NULL) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_MALLOC_FAILURE);
		return 0;
	}
