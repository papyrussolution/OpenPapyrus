...
static WRITE_TRAN ossl_statem_client13_write_transition(SSL * s)
{
	...
	switch(st->hand_state) {
		...
		case TLS_ST_CR_FINISHED:
		    ...
		    else if((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0 && s->hello_retry_request == ssl_st::SSL_HRR_NONE)
...
int tls_construct_client_hello(SSL * s, WPACKET * pkt)
{
	...
	if(s->new_session || s->session->ssl_version == TLS1_3_VERSION) {
		if(s->version == TLS1_3_VERSION
			...
			if(s->hello_retry_request == ssl_st::SSL_HRR_NONE && RAND_bytes_ex(s->ctx->libctx, s->tmp_session_id, sess_id_len, 0) <= 0) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
				return 0;
			}
		}
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL * s, PACKET * pkt)
{
	...
	if(s->version == TLS1_3_VERSION && sversion == TLS1_2_VERSION && PACKET_remaining(pkt) >= SSL3_RANDOM_SIZE && memcmp(hrrrandom, PACKET_data(pkt), SSL3_RANDOM_SIZE) == 0) {
...
MSG_PROCESS_RETURN tls_process_new_session_ticket(SSL * s, PACKET * pkt)
{
	...
	s->session->ext.tick = (uchar *)OPENSSL_malloc(ticklen);
...
int tls_process_cert_status_body(SSL * s, PACKET * pkt)
{
	...
	s->ext.ocsp.resp = (uchar *)OPENSSL_malloc(resplen);
...
static int tls_construct_cke_gost(SSL * s, WPACKET * pkt)
{
	...
	if(EVP_PKEY_encrypt_init(pkey_ctx) <= 0 /* Generate session key*/ || RAND_bytes_ex(s->ctx->libctx, pms, pmslen, 0) <= 0) {
		SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	;
...
int ossl_gost_ukm(const SSL * s, uchar * dgst_buf)
{
