...
{
	size_t i, num_exts, builtin_num = SIZEOFARRAY(ext_defs), offset;
    ...
    RAW_EXTENSION ** res, size_t * len, int init)
{
	...
	*res = NULL;
	...
	num_exts = SIZEOFARRAY(ext_defs) + (exts != NULL ? exts->meths_count : 0);
...
static ossl_inline void ssl_tsan_decr(const SSL_CTX * ctx, TSAN_QUALIFIER int * stat)
{
	if(ssl_tsan_lock(ctx)) {
		tsan_decr(reinterpret_cast<volatile long *>(stat));
		ssl_tsan_unlock(ctx);
	}
}
...
static int final_server_name(SSL * s, unsigned int context, int sent)
{
	 ...
	 */
	if(SSL_IS_FIRST_HANDSHAKE(s) && s->ctx != s->session_ctx && s->hello_retry_request == ssl_st::SSL_HRR_NONE) { 
    ...
    int external)
{
	...
	if(s->hello_retry_request == ssl_st::SSL_HRR_PENDING) {
		...
		if(s->server) {
			...
			if(!PACKET_buf_init(&hashprefix, (const uchar *)hdata, hdatalen) || !PACKET_forward(&hashprefix, 1) || 
				!PACKET_get_length_prefixed_3(&hashprefix, &msg) || !PACKET_forward(&hashprefix, 1) || !PACKET_get_length_prefixed_3(&hashprefix, &msg)) {
				SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
				goto err;
			}
