...
#pragma hdrstop
#define OPENSSL_SUPPRESS_DEPRECATED // We need to use the SRP deprecated APIs in order to implement the SSL SRP APIs - which are themselves deprecated.
...
int ssl_srp_ctx_free_intern(SSL * s)
{
	...
	BN_free(s->srp_ctx.b);
	BN_free(s->srp_ctx.v);
	memzero(&s->srp_ctx, sizeof(s->srp_ctx));
...
int ssl_srp_ctx_init_intern(SSL * s)
{
	...
	if((ctx->srp_ctx.N && ((s->srp_ctx.N = BN_dup(ctx->srp_ctx.N)) == NULL)) ||
	    (ctx->srp_ctx.g && ((s->srp_ctx.g = BN_dup(ctx->srp_ctx.g)) == NULL)) ||
	    (ctx->srp_ctx.s && ((s->srp_ctx.s = BN_dup(ctx->srp_ctx.s)) == NULL)) ||
	    (ctx->srp_ctx.B && ((s->srp_ctx.B = BN_dup(ctx->srp_ctx.B)) == NULL)) ||
	    (ctx->srp_ctx.A && ((s->srp_ctx.A = BN_dup(ctx->srp_ctx.A)) == NULL)) ||
	    (ctx->srp_ctx.a && ((s->srp_ctx.a = BN_dup(ctx->srp_ctx.a)) == NULL)) ||
	    (ctx->srp_ctx.v && ((s->srp_ctx.v = BN_dup(ctx->srp_ctx.v)) == NULL)) ||
	    (ctx->srp_ctx.b && ((s->srp_ctx.b = BN_dup(ctx->srp_ctx.b)) == NULL))) {
		ERR_raise(ERR_LIB_SSL, ERR_R_BN_LIB);
		goto err;
	}
	if(ctx->srp_ctx.login && ((s->srp_ctx.login = OPENSSL_strdup(ctx->srp_ctx.login)) == NULL)) {
		ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
		goto err;
	}
	if(ctx->srp_ctx.info && ((s->srp_ctx.info = OPENSSL_strdup(ctx->srp_ctx.info)) == NULL)) {
		ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
		goto err;
	}
...
int ssl_srp_server_param_with_username_intern(SSL * s, int * ad)
{
	...
	*ad = SSL_AD_UNKNOWN_PSK_IDENTITY;
	if(s->srp_ctx.TLS_ext_srp_username_callback && ((al = s->srp_ctx.TLS_ext_srp_username_callback(s, ad, s->srp_ctx.SRP_cb_arg)) != SSL_ERROR_NONE))
		return al;
	*ad = SSL_AD_INTERNAL_ERROR;
	...
	return ((s->srp_ctx.B = SRP_Calc_B_ex(s->srp_ctx.b, s->srp_ctx.N, s->srp_ctx.g, s->srp_ctx.v, s->ctx->libctx, s->ctx->propq)) != NULL) ? SSL_ERROR_NONE : SSL3_AL_FATAL;
}
...
int SSL_set_srp_server_param_pw(SSL * s, const char * user, const char * pass, const char * grp)
{
	...
	s->srp_ctx.s = NULL;
	// @openssl3.0.5 if(!SRP_create_verifier_BN_ex(user, pass, &s->srp_ctx.s, &s->srp_ctx.v, GN->N, GN->g, s->ctx->libctx, s->ctx->propq))
	if(!SRP_create_verifier_BN_ex(user, pass, &s->srp_ctx.s, &s->srp_ctx.v, s->srp_ctx.N, s->srp_ctx.g, s->ctx->libctx, s->ctx->propq)) // @openssl3.0.5 
		return -1;
	return 1;
}
...
int srp_verify_server_param(SSL * s)
{
	...
	/*
	 * Sanity check parameters: we can quickly check B % N == 0 by checking B != 0 since B < N
	 */
