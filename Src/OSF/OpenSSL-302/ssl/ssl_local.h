...
#define SSL_kRSA                0x00000001U /* RSA key exchange */
#define SSL_kDHE                0x00000002U /* tmp DH key no DH cert */
#define SSL_kEDH                SSL_kDHE /* synonym */
#define SSL_kECDHE              0x00000004U /* ephemeral ECDH */
#define SSL_kEECDH              SSL_kECDHE /* synonym */
#define SSL_kPSK                0x00000008U /* PSK */
#define SSL_kGOST               0x00000010U /* GOST key exchange */
#define SSL_kSRP                0x00000020U /* SRP */
#define SSL_kRSAPSK             0x00000040U
#define SSL_kECDHEPSK           0x00000080U
#define SSL_kDHEPSK             0x00000100U
#define SSL_kGOST18             0x00000200U /* GOST KDF key exchange, draft-smyshlyaev-tls12-gost-suites */

/* all PSK */
#define SSL_PSK     (SSL_kPSK | SSL_kRSAPSK | SSL_kECDHEPSK | SSL_kDHEPSK)
#define SSL_kANY                0x00000000U /* Any appropriate key exchange algorithm (for TLS 1.3 ciphersuites) */
/* Bits for algorithm_auth (server authentication) */
#define SSL_aRSA                0x00000001U /* RSA auth */
#define SSL_aDSS                0x00000002U /* DSS auth */
#define SSL_aNULL               0x00000004U /* no auth (i.e. use ADH or AECDH) */
#define SSL_aECDSA              0x00000008U /* ECDSA auth*/
#define SSL_aPSK                0x00000010U /* PSK auth */
#define SSL_aGOST01             0x00000020U /* GOST R 34.10-2001 signature auth */
#define SSL_aSRP                0x00000040U /* SRP auth */
#define SSL_aGOST12             0x00000080U /* GOST R 34.10-2012 signature auth */
#define SSL_aANY                0x00000000U /* Any appropriate signature auth (for TLS 1.3 ciphersuites) */
#define SSL_aCERT (SSL_aRSA | SSL_aDSS | SSL_aECDSA | SSL_aGOST01 | SSL_aGOST12) /* All bits requiring a certificate */
...
typedef struct sigalg_lookup_st {
	const char * name; /* TLS 1.3 signature scheme name */
	uint16_t sigalg; /* Raw value used in extension */
	int hash; /* NID of hash algorithm or NID_undef if no hash */
	int hash_idx; /* Index of hash algorithm or -1 if no hash algorithm */
	int sig; /* NID of signature algorithm */
	int sig_idx; /* Index of signature algorithm */
	int sigandhash; /* Combined hash and signature NID, if any */
	int curve; /* Required public key curve (ECDSA only) */
	int enabled; /* Whether this signature algorithm is actually available for use */
} SIGALG_LOOKUP;
/*
...
#endif
#define DTLS1_MAX_MTU_OVERHEAD                   48 /* Max MTU overhead we know about so far is 40 for IPv6 + 8 for UDP */
/*
...
__owur size_t ssl3_final_finish_mac(SSL * s, const char * sender, size_t slen, unsigned char * p);
__owur int ssl3_finish_mac(SSL * s, const unsigned char * buf, size_t len);
void ssl3_free_digest_list(SSL * s);
__owur unsigned long ssl3_output_cert_chain(SSL * s, WPACKET * pkt, CERT_PKEY * cpk);
__owur const SSL_CIPHER * ssl3_choose_cipher(SSL * ssl, STACK_OF(SSL_CIPHER) *clnt, STACK_OF(SSL_CIPHER) *srvr);
...
__owur int ssl_version_supported(const SSL * s, int version, const SSL_METHOD ** meth);
__owur int ssl_set_client_hello_version(SSL * s);
__owur int ssl_check_version_downgrade(SSL * s);
__owur int ssl_set_version_bound(int method_version, int version, int * bound);
__owur int ssl_choose_server_version(SSL * s, CLIENTHELLO_MSG * hello, DOWNGRADE * dgrd);
__owur int ssl_choose_client_version(SSL * s, int version, RAW_EXTENSION * extensions);
__owur int ssl_get_min_max_version(const SSL * s, int * min_version, int * max_version, int * real_max);
__owur long tls1_default_timeout(void);
__owur int dtls1_do_write(SSL * s, int type);
void dtls1_set_message_header(SSL * s, unsigned char mt, size_t len, size_t frag_off, size_t frag_len);
int dtls1_write_app_data_bytes(SSL * s, int type, const void * buf_, size_t len, size_t * written);
...
__owur int tls1_export_keying_material(SSL * s, unsigned char * out, size_t olen,
    ...
    int use_context);
__owur int tls13_export_keying_material(SSL * s, unsigned char * out, size_t olen, const char * label, size_t llen, const unsigned char * context, size_t contextlen, int use_context);
__owur int tls13_export_keying_material_early(SSL * s, unsigned char * out, size_t olen, const char * label, size_t llen, const unsigned char * context, size_t contextlen);
...
__owur int tls1_set_groups(uint16_t ** pext, size_t * pextlen, int * curves, size_t ncurves);
__owur int tls1_set_groups_list(SSL_CTX * ctx, uint16_t ** pext, size_t * pextlen, const char * str);
__owur EVP_PKEY * ssl_generate_pkey_group(SSL * s, uint16_t id);
__owur int tls_valid_group(SSL * s, uint16_t group_id, int minversion, int maxversion, int isec, int * okfortls13);
...
__owur SSL_TICKET_STATUS tls_get_ticket_from_client(SSL * s, CLIENTHELLO_MSG * hello, SSL_SESSION ** ret);
__owur SSL_TICKET_STATUS tls_decrypt_ticket(SSL * s, const unsigned char * etick, size_t eticklen, const unsigned char * sess_id, size_t sesslen, SSL_SESSION ** psess);
...
__owur int tls1_set_raw_sigalgs(CERT * c, const uint16_t * psigs, size_t salglen, int client);
...
__owur int tls12_copy_sigalgs(SSL * s, WPACKET * pkt, const uint16_t * psig, size_t psiglen);
__owur int tls1_save_u16(PACKET * pkt, uint16_t ** pdest, size_t * pdestlen);
__owur int tls1_save_sigalgs(SSL * s, PACKET * pkt, int cert);
__owur int tls1_process_sigalgs(SSL * s);
__owur int tls1_set_peer_legacy_sigalg(SSL * s, const EVP_PKEY * pkey);
__owur int tls1_lookup_md(SSL_CTX * ctx, const SIGALG_LOOKUP * lu, const EVP_MD ** pmd);
...
__owur int ssl_handshake_hash(SSL * s, unsigned char * out, size_t outlen, size_t * hashlen);
 ...
 * |encrypted_premaster|.
 */
__owur int ssl_log_rsa_client_key_exchange(SSL * ssl, const uint8_t * encrypted_premaster, size_t encrypted_premaster_len,
 ...
 */
__owur int ssl_log_secret(SSL * ssl, const char * label, const uint8_t * secret, size_t secret_len);
...
custom_ext_method * custom_ext_find(const custom_ext_methods * exts, ENDPOINT role, unsigned int ext_type, size_t * idx);
void custom_ext_init(custom_ext_methods * meths);

__owur int custom_ext_parse(SSL * s, unsigned int context, unsigned int ext_type, const unsigned char * ext_data, size_t ext_size, X509 * x, size_t chainidx);
__owur int custom_ext_add(SSL * s, int context, WPACKET * pkt, X509 * x, size_t chainidx, int maxversion);
__owur int custom_exts_copy(custom_ext_methods * dst, const custom_ext_methods * src);
__owur int custom_exts_copy_flags(custom_ext_methods * dst, const custom_ext_methods * src);
...
int tls_provider_set_tls_params(SSL * s, EVP_CIPHER_CTX * ctx, const EVP_CIPHER * ciph, const EVP_MD * md);
...
static ossl_unused ossl_inline void ssl_tsan_counter(const SSL_CTX * ctx, TSAN_QUALIFIER int * stat)
{
	if(ssl_tsan_lock(ctx)) {
		tsan_counter(reinterpret_cast<volatile long *>(stat));
		ssl_tsan_unlock(ctx);
	}
}
