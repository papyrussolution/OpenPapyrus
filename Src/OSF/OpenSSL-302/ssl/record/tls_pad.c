...
static int ssl3_cbc_copy_mac(size_t * reclen, size_t origreclen, unsigned char * recdata, unsigned char ** mac,
    int * alloced, size_t block_size, size_t mac_size, size_t good, OSSL_LIB_CTX * libctx);
    ...
    OSSL_LIB_CTX * libctx)
{
	...
	*reclen -= mac_size;
	...
	if(!ossl_assert(mac != NULL && alloced != NULL))
		return 0;
	*mac = out = (unsigned char *)OPENSSL_malloc(mac_size);
	if(*mac == NULL)
		return 0;
	*alloced = 1;
	...
	for(i = 0; i < mac_size; i++) {
		...
		out[j++] = constant_time_select_8((unsigned char)(good & 0xff), rotated_mac[rotate_offset++], randmac[i]);
