...
int ssl3_get_record(SSL * s)
{
	...
	do {
		...
		if((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
			...
			if(rret <= 0) {
				...
				switch(errno) {
					case EBADMSG:
					    SSLfatal(s, SSL_AD_BAD_RECORD_MAC, SSL_R_DECRYPTION_FAILED_OR_BAD_RECORD_MAC);
					    break;
					case EMSGSIZE:
					    SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_R_PACKET_LENGTH_TOO_LONG);
					    break;
					case EINVAL:
					    SSLfatal(s, SSL_AD_PROTOCOL_VERSION, SSL_R_WRONG_VERSION_NUMBER);
					    break;
					default:
					    break;
				}
			...
			if(s->server && RECORD_LAYER_is_first_record(&s->rlayer)
				...
				if(thisrr->length > SSL3_BUFFER_get_len(rbuf) - SSL2_RT_HEADER_LENGTH) {
					SSLfatal(s, SSL_AD_RECORD_OVERFLOW, SSL_R_PACKET_LENGTH_TOO_LONG);
					return -1;
				}
				...
				if(!PACKET_get_1(&pkt, &type) || !PACKET_get_net_2(&pkt, &version) || !PACKET_get_net_2_len(&pkt, &thisrr->length)) {
					if(s->msg_callback)
				 ...
				 * that explicitly
				 */
				if(!s->first_packet && !SSL_IS_TLS13(s) && s->hello_retry_request != ssl_st::SSL_HRR_PENDING && version != (unsigned int)s->version) {
					if((s->version & 0xFF00) == (version & 0xFF00) && !s->enc_write_ctx && !s->write_hash) {
						if(thisrr->type == SSL3_RT_ALERT) {
							 ...
							 * end.
							 */
							SSLfatal(s, SSL_AD_NO_ALERT, SSL_R_WRONG_VERSION_NUMBER);
							return -1;
						}
						/*
				...
				if((version >> 8) != SSL3_VERSION_MAJOR) {
					if(RECORD_LAYER_is_first_record(&s->rlayer)) {
						...
						if(strncmp((char*)p, "GET ", 4) == 0 || strncmp((char*)p, "POST ", 5) == 0 || strncmp((char*)p, "HEAD ", 5) == 0 || strncmp((char*)p, "PUT ", 4) == 0) {
							...
							SSLfatal(s, SSL_AD_NO_ALERT, SSL_R_HTTPS_PROXY_REQUEST);
							return -1;
						}
				...
				if(SSL_IS_TLS13(s) && s->enc_read_ctx != NULL) {
					...
					if(thisrr->rec_version != TLS1_2_VERSION) {
						SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_WRONG_VERSION_NUMBER);
						return -1;
					}
				}
		...
		if(thisrr->rec_version == SSL2_VERSION) {
			more = thisrr->length + SSL2_RT_HEADER_LENGTH - SSL3_RT_HEADER_LENGTH;
		}
		else {
			more = thisrr->length;
		}
	...
	if(num_recs == 1 && thisrr->type == SSL3_RT_CHANGE_CIPHER_SPEC && (SSL_IS_TLS13(s) || s->hello_retry_request != ssl_st::SSL_HRR_NONE) && SSL_IS_FIRST_HANDSHAKE(s)) {
		...
		if(thisrr->length != 1 || thisrr->data[0] != 0x01) {
			SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_INVALID_CCS_MESSAGE);
			return -1;
		}
		/*
		...
		if(RECORD_LAYER_get_empty_record_count(&s->rlayer) > MAX_EMPTY_RECORDS) {
			SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_R_UNEXPECTED_CCS_MESSAGE);
			return -1;
		}
		thisrr->read = 1;
	...
	if(mac_size > 0) {
		macbufs = (SSL_MAC_BUF *)OPENSSL_zalloc(sizeof(*macbufs) * num_recs);
		if(macbufs == NULL) {
	...
	if(enc_err == 0) {
		...
		if(num_recs == 1 && ossl_statem_skip_early_data(s)) {
			 ...
			 */
			thisrr = &rr[0];
			if(!early_data_count_ok(s, thisrr->length, EARLY_DATA_CIPHERTEXT_OVERHEAD, 0)) {
