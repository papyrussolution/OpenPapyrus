...
start:
	...
	if(SSL3_RECORD_get_type(rr) == SSL3_RT_ALERT) {
		...
		uchar * alert_bytes = SSL3_RECORD_get_data(rr) + SSL3_RECORD_get_off(rr);
		PACKET alert;
		if(!PACKET_buf_init(&alert, alert_bytes, SSL3_RECORD_get_length(rr)) || !PACKET_get_1(&alert, &alert_level) || 
...
int dtls1_write_bytes(SSL * s, int type, const void * buf, size_t len, size_t * written)
{
	...
	s->rwstate = SSL_NOTHING;
	i = do_dtls1_write(s, type, (const uchar *)buf, len, 0, written);
	return i;
}
...
DTLS1_BITMAP * dtls1_get_bitmap(SSL * s, SSL3_RECORD * rr, unsigned int * is_next_epoch)
{
	*is_next_epoch = 0;
	 ...
	 * epoch
	 */
	else if(rr->epoch == (ulong)(s->rlayer.d->r_epoch + 1) && s->rlayer.d->unprocessed_rcds.epoch != s->rlayer.d->r_epoch && 
