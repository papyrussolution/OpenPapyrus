...
{
	return ossl_prov_bio_read_ex((OSSL_CORE_BIO*)BIO_get_data(bio), data, data_len, bytes_read);
}

static int bio_core_write_ex(BIO * bio, const char * data, size_t data_len, size_t * written)
{
	return ossl_prov_bio_write_ex((OSSL_CORE_BIO *)BIO_get_data(bio), data, data_len, written);
}

static long bio_core_ctrl(BIO * bio, int cmd, long num, void * ptr)
{
	return ossl_prov_bio_ctrl((OSSL_CORE_BIO*)BIO_get_data(bio), cmd, num, ptr);
}

static int bio_core_gets(BIO * bio, char * buf, int size)
{
	return ossl_prov_bio_gets((OSSL_CORE_BIO*)BIO_get_data(bio), buf, size);
}

static int bio_core_puts(BIO * bio, const char * str)
{
	return ossl_prov_bio_puts((OSSL_CORE_BIO*)BIO_get_data(bio), str);
}
...
{
	BIO_set_init(bio, 0);
	ossl_prov_bio_free((OSSL_CORE_BIO*)BIO_get_data(bio));
	return 1;
}
