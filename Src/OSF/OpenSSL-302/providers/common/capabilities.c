...
#define TLS_GROUP_ENTRY(tlsname, realname, algorithm, idx) \
	{ \
		...
		OSSL_PARAM_uint(OSSL_CAPABILITY_TLS_GROUP_ID, (unsigned int*)&group_list[idx].group_id), \
		OSSL_PARAM_uint(OSSL_CAPABILITY_TLS_GROUP_SECURITY_BITS, (unsigned int*)&group_list[idx].secbits), \
		OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MIN_TLS, (unsigned int*)&group_list[idx].mintls), \
		OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MAX_TLS, (unsigned int*)&group_list[idx].maxtls), \
		OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MIN_DTLS, (unsigned int*)&group_list[idx].mindtls), \
		OSSL_PARAM_int(OSSL_CAPABILITY_TLS_GROUP_MAX_DTLS, (unsigned int*)&group_list[idx].maxdtls), \
		OSSL_PARAM_END \
	}
...
{
#if !defined(OPENSSL_NO_EC) || !defined(OPENSSL_NO_DH)
	for(size_t i = 0; i < SIZEOFARRAY(param_group_list); i++)
