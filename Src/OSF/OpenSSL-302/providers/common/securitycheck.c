...
int ossl_rsa_check_key(OSSL_LIB_CTX * ctx, const RSA * rsa, int operation)
{
	...
	if(ossl_securitycheck_enabled(ctx)) {
		...
		if(protect ? (sz < 2048) : (sz < 1024)) {
			ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH, "operation: %d", operation);
			return 0;
		}
	}
#else
...
int ossl_ec_check_key(OSSL_LIB_CTX * ctx, const EC_KEY * ec, int protect)
{
	...
	if(ossl_securitycheck_enabled(ctx)) {
		...
		if(nid == NID_undef) {
			ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_CURVE, "Explicit curves are not allowed in fips mode");
			return 0;
		}
		curve_name = EC_curve_nid2nist(nid);
		if(curve_name == NULL) {
			ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_CURVE, "Curve %s is not approved in FIPS mode", curve_name);
			return 0;
		}
		/*
		...
		if(protect && strength < 112) {
			ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_CURVE, "Curve %s cannot be used for signing", curve_name);
			return 0;
		}
	}
#endif /* OPENSSL_NO_FIPS_SECURITYCHECKS */
