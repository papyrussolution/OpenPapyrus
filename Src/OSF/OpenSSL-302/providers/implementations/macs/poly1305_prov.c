...
static void * poly1305_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (struct poly1305_data_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
		ctx->provctx = provctx;
	return ctx;
}
...
{
	struct poly1305_data_st * src = (struct poly1305_data_st *)vsrc;
	struct poly1305_data_st * dst;
	if(!ossl_prov_is_running())
		return NULL;
	dst = (struct poly1305_data_st *)poly1305_new(src->provctx);
	if(dst == NULL)
		return NULL;
...
{
	struct poly1305_data_st * ctx = (struct poly1305_data_st *)vmacctx;
...
static int poly1305_set_ctx_params(void * vmacctx, const OSSL_PARAM * params)
{
	struct poly1305_data_st * ctx = (struct poly1305_data_st *)vmacctx;
	const OSSL_PARAM * p;
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL && !poly1305_setkey(ctx, (const unsigned char *)p->data, p->data_size))
		return 0;
	return 1;
}
