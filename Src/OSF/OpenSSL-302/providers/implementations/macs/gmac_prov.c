...
static void * gmac_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((macctx = (struct gmac_data_st *)OPENSSL_zalloc(sizeof(*macctx))) == NULL || (macctx->ctx = EVP_CIPHER_CTX_new()) == NULL) {
		gmac_free(macctx);
		return NULL;
	}
...
static int gmac_set_ctx_params(void * vmacctx, const OSSL_PARAM params[])
{
	struct gmac_data_st * macctx = (struct gmac_data_st *)vmacctx;
	EVP_CIPHER_CTX * ctx = macctx->ctx;
	OSSL_LIB_CTX * provctx = PROV_LIBCTX_OF((PROV_CTX*)macctx->provctx);
	const OSSL_PARAM * p;
	if(!params)
		return 1;
	if(!ctx || !ossl_prov_cipher_load_from_params(&macctx->cipher, params, provctx))
		return 0;
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL)
		if(p->data_type != OSSL_PARAM_OCTET_STRING || !gmac_setkey(macctx, (const unsigned char *)p->data, p->data_size))
			return 0;

	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_IV)) != NULL) {
		if(p->data_type != OSSL_PARAM_OCTET_STRING)
			return 0;
		if(!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, p->data_size, NULL) || !EVP_EncryptInit_ex(ctx, NULL, NULL, NULL, (const unsigned char *)p->data))
			return 0;
	}
	return 1;
}
