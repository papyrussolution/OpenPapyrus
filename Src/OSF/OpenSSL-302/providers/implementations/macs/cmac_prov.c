...
static void * cmac_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((macctx = (struct cmac_data_st *)OPENSSL_zalloc(sizeof(*macctx))) == NULL || (macctx->ctx = CMAC_CTX_new()) == NULL) {
		OPENSSL_free(macctx);
		macctx = NULL;
	}
	else {
...
{
	struct cmac_data_st * macctx = (struct cmac_data_st *)vmacctx;
	OSSL_LIB_CTX * ctx = PROV_LIBCTX_OF((PROV_CTX*)macctx->provctx);
	const OSSL_PARAM * p;
	if(!params)
		return 1;
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL) {
		if(p->data_type != OSSL_PARAM_OCTET_STRING)
			return 0;
		return cmac_setkey(macctx, (const unsigned char *)p->data, p->data_size);
	}
	return 1;
}
