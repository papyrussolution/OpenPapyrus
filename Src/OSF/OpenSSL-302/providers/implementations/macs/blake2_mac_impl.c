...
static int blake2_mac_set_ctx_params(void * vmacctx, const OSSL_PARAM params[])
{
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL && !blake2_setkey(macctx, (const unsigned char *)p->data, p->data_size))
		return 0;
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_CUSTOM)) != NULL) {
		...
		BLAKE2_PARAM_SET_PERSONAL(&macctx->params, (const uint8_t *)p->data, p->data_size);
	}
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_SALT)) != NULL) {
		...
		BLAKE2_PARAM_SET_SALT(&macctx->params, (const uint8_t *)p->data, p->data_size);
	}
	return 1;
}
