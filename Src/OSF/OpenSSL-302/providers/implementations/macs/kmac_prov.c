...
static struct kmac_data_st * kmac_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((kctx = (struct kmac_data_st *)OPENSSL_zalloc(sizeof(*kctx))) == NULL || (kctx->ctx = EVP_MD_CTX_new()) == NULL) {
		kmac_free(kctx);
		return NULL;
	}
...
static void * kmac_fetch_new(void * provctx, const OSSL_PARAM * params)
{
	...
	if(!ossl_prov_digest_load_from_params(&kctx->digest, params,
	    PROV_LIBCTX_OF((PROV_CTX*)provctx))) {
		kmac_free(kctx);
		return 0;
	}
...
static int kmac_final(void * vmacctx, unsigned char * out, size_t * outl, size_t outsize)
{
	...
	ok = right_encode(encoded_outlen, sizeof(encoded_outlen), &len, lbits) && EVP_DigestUpdate(ctx, encoded_outlen, len) && EVP_DigestFinalXOF(ctx, out, kctx->out_len);
	*outl = kctx->out_len;
	return ok;
}
...
static int kmac_set_ctx_params(void * vmacctx, const OSSL_PARAM * params)
{
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL && !kmac_setkey(kctx, (const unsigned char *)p->data, p->data_size))
		return 0;
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_CUSTOM)) != NULL) {
		...
		if(!encode_string(kctx->custom, sizeof(kctx->custom), &kctx->custom_len, (const unsigned char *)p->data, p->data_size))
			return 0;
	}
	return 1;
}
