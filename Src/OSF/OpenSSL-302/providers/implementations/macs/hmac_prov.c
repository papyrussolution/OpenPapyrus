...
int ssl3_cbc_digest_record(const EVP_MD * md, unsigned char * md_out, size_t * md_out_size, const unsigned char * header/*[13]*/,
...
static void * hmac_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((macctx = (struct hmac_data_st *)OPENSSL_zalloc(sizeof(*macctx))) == NULL || (macctx->ctx = HMAC_CTX_new()) == NULL) {
		OPENSSL_free(macctx);
		return NULL;
	}
...
static void * hmac_dup(void * vsrc)
{
	...
	*dst = *src;
	...
	if(src->key != NULL) {
		...
		dst->key = (unsigned char *)OPENSSL_secure_malloc(src->keylen > 0 ? src->keylen : 1);
		if(dst->key == NULL) {
			hmac_free(dst);
			return 0;
		}
    ...
    const unsigned char * key, size_t keylen)
{
	...
	macctx->key = (unsigned char *)OPENSSL_secure_malloc(keylen > 0 ? keylen : 1);
	if(macctx->key == NULL)
		return 0;
...
static int hmac_set_ctx_params(void * vmacctx, const OSSL_PARAM params[])
{
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_KEY)) != NULL) {
		...
		macctx->key = (unsigned char *)OPENSSL_secure_malloc(p->data_size > 0 ? p->data_size : 1);
		if(macctx->key == NULL)
			return 0;
