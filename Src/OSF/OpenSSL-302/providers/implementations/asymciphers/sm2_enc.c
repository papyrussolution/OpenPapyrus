...
{
	PROV_SM2_CTX * psm2ctx = (PROV_SM2_CTX *)OPENSSL_zalloc(sizeof(PROV_SM2_CTX));
	if(psm2ctx == NULL)
		return NULL;
	psm2ctx->libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
	return psm2ctx;
}
...
static int sm2_init(void * vpsm2ctx, void * vkey, const OSSL_PARAM params[])
{
	...
	if(psm2ctx == NULL || vkey == NULL || !EC_KEY_up_ref((EC_KEY *)vkey))
		return 0;
	EC_KEY_free(psm2ctx->key);
	psm2ctx->key = (EC_KEY*)vkey;
...
static void * sm2_dupctx(void * vpsm2ctx)
{
	...
	PROV_SM2_CTX * dstctx = (PROV_SM2_CTX *)OPENSSL_zalloc(sizeof(*srcctx));
	if(dstctx == NULL)
		return NULL;
	*dstctx = *srcctx;
