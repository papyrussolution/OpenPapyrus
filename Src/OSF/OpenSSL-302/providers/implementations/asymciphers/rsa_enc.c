...
static int rsa_encrypt(void * vprsactx, uchar * out, size_t * outlen, size_t outsize, const uchar * in, size_t inlen)
{
	PROV_RSA_CTX * prsactx = (PROV_RSA_CTX*)vprsactx;
	int ret;
	if(!ossl_prov_is_running())
	...
	if(prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
		...
		uchar * tbuf;
		if((tbuf = (uchar *)OPENSSL_malloc(rsasize)) == NULL) {
...
static int rsa_decrypt(void * vprsactx, uchar * out, size_t * outlen, size_t outsize, const uchar * in, size_t inlen)
{
	PROV_RSA_CTX * prsactx = (PROV_RSA_CTX*)vprsactx;
	int ret;
	size_t len = RSA_size(prsactx->rsa);
	...
	if(prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING || prsactx->pad_mode == RSA_PKCS1_WITH_TLS_PADDING) {
		uchar * tbuf;
		if((tbuf = (uchar *)OPENSSL_malloc(len)) == NULL) {
			ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
			return 0;
		}
		...
		if(prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
			...
			ret = RSA_padding_check_PKCS1_OAEP_mgf1(out, outsize, tbuf, len, len, prsactx->oaep_label, prsactx->oaep_labellen, prsactx->oaep_md, prsactx->mgf1_md);
		}
		else {
...
static void * rsa_dupctx(void * vprsactx)
{
	...
	if(dstctx == NULL)
		return NULL;
	*dstctx = *srcctx;
	if(dstctx->rsa && !RSA_up_ref(dstctx->rsa)) {
		OPENSSL_free(dstctx);
		return NULL;
	}
	if(dstctx->oaep_md && !EVP_MD_up_ref(dstctx->oaep_md)) {
		RSA_free(dstctx->rsa);
		OPENSSL_free(dstctx);
		return NULL;
	}
	if(dstctx->mgf1_md && !EVP_MD_up_ref(dstctx->mgf1_md)) {
		RSA_free(dstctx->rsa);
...
static int rsa_get_ctx_params(void * vprsactx, OSSL_PARAM * params)
{
	...
	if(p && !OSSL_PARAM_set_utf8_string(p, prsactx->oaep_md == NULL ? "" : EVP_MD_get0_name(prsactx->oaep_md)))
		return 0;
	...
	if(p && !OSSL_PARAM_set_octet_ptr(p, prsactx->oaep_label, prsactx->oaep_labellen))
		return 0;
	p = OSSL_PARAM_locate(params, OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION);
	if(p && !OSSL_PARAM_set_uint(p, prsactx->client_version))
		return 0;

	p = OSSL_PARAM_locate(params, OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION);
	if(p && !OSSL_PARAM_set_uint(p, prsactx->alt_version))
		return 0;
...
static int rsa_set_ctx_params(void * vprsactx, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		if(!OSSL_PARAM_get_octet_string(p, &tmp_label, 0, &tmp_labellen))
			return 0;
		OPENSSL_free(prsactx->oaep_label);
		prsactx->oaep_label = (uchar *)tmp_label;
