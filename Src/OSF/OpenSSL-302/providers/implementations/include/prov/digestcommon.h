...
#define PROV_DISPATCH_FUNC_DIGEST_GET_PARAMS(name)                             \
	...
	{ OSSL_FUNC_DIGEST_GETTABLE_PARAMS, (void (*)(void)) ossl_digest_default_gettable_params }

#define PROV_FUNC_DIGEST_FINAL(name, CTX, dgstsize, fin)                           \
	static OSSL_FUNC_digest_final_fn name ## _internal_final;                        \
	static int name ## _internal_final(void * ctx, unsigned char * out, size_t * outl, size_t outsz) \
	{                                                                              \
		if(ossl_prov_is_running() && outsz >= dgstsize && fin(out, (CTX *)ctx)) {  \
			*outl = dgstsize;                                                      \
			return 1;                                                              \
		}                                                                          \
		return 0;                                                                  \
...
# define PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_START(name, CTX, blksize, dgstsize, flags, upd, fin) \
	...
	{                                                                              \
		CTX * ctx = ossl_prov_is_running() ? (CTX *)OPENSSL_zalloc(sizeof(* ctx)) : NULL;   \
		return ctx;                                                                \
	}                                                                              \
	...
	{                                                                              \
		CTX * in = (CTX*)ctx;                                                      \
		CTX * ret = ossl_prov_is_running() ? (CTX *)OPENSSL_malloc(sizeof(* ret)) : NULL;   \
		if(ret != NULL)                                                           \
			* ret = * in;                                                            \
		return ret;                                                                \
	}                                                                              \
	PROV_FUNC_DIGEST_FINAL(name, CTX, dgstsize, fin)                                    \
...
# define IMPLEMENT_digest_functions(name, CTX, blksize, dgstsize, flags, init, upd, fin)                       \
	...
	static int name ## _internal_init(void * ctx, ossl_unused const OSSL_PARAM params[])         \
	{                                                                              \
		return ossl_prov_is_running() && init((CTX *)ctx);                                \
	}                                                                              \
	PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_START(name, CTX, blksize, dgstsize, flags, upd, fin), \
	{ OSSL_FUNC_DIGEST_INIT, (void (*)(void)) name ## _internal_init },           \
	PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_END

# define IMPLEMENT_digest_functions_with_settable_ctx(name, CTX, blksize, dgstsize, flags, init, upd, fin, settable_ctx_params, set_ctx_params) \
	static OSSL_FUNC_digest_init_fn name ## _internal_init;                          \
	static int name ## _internal_init(void * ctx, const OSSL_PARAM params[])          \
	{                                                                              \
		return ossl_prov_is_running() && init((CTX *)ctx) && set_ctx_params(ctx, params); \
	}                                                                              \
	PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_START(name, CTX, blksize, dgstsize, flags, upd, fin), \
	{ OSSL_FUNC_DIGEST_INIT, (void (*)(void)) name ## _internal_init },           \
