...
static void free_file_ctx(struct file_ctx_st * ctx)
{
	...
	if(ctx->type != file_ctx_st::IS_DIR) {
...
static struct file_ctx_st * new_file_ctx(int type, const char * uri, void * provctx)
{
	...
	if((ctx = (struct file_ctx_st *)OPENSSL_zalloc(sizeof(*ctx))) != NULL && (uri == NULL || (ctx->uri = OPENSSL_strdup(uri)) != NULL)) {
		ctx->type = (file_ctx_st::T)type;
		ctx->provctx = provctx;
		return ctx;
	}
	free_file_ctx(ctx);
...
static struct file_ctx_st * file_open_stream(BIO * source, const char * uri, void * provctx)
{
	...
	if((ctx = new_file_ctx(file_ctx_st::IS_FILE, uri, provctx)) == NULL) {
...
{
	struct file_ctx_st * ctx;
	if((ctx = new_file_ctx(file_ctx_st::IS_DIR, uri, provctx)) == NULL) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
	...
	if(ctx->_.dir.last_entry == NULL) {
		if(ctx->_.dir.last_errno != 0) {
			ERR_raise_data(ERR_LIB_SYS, ctx->_.dir.last_errno, "Calling OPENSSL_DIR_read(\"%s\")", path);
			goto err;
		}
...
static void * file_open(void * provctx, const char * uri)
{
	...
	if(S_ISDIR(st.st_mode))
		ctx = (struct file_ctx_st *)file_open_dir(path, uri, provctx);
	else if((bio = BIO_new_file(path, "rb")) == NULL || (ctx = file_open_stream(bio, uri, provctx)) == NULL)
		BIO_free_all(bio);
	return ctx;
}
...
{
	struct file_ctx_st * ctx;
	BIO * new_bio = ossl_bio_new_from_core_bio((PROV_CTX *)provctx, cin);
	if(new_bio == NULL)
		return NULL;
...
{
	struct file_ctx_st * ctx = (struct file_ctx_st *)loaderctx;
	const OSSL_PARAM * p;
	if(!params)
		return 1;
	if(ctx->type != file_ctx_st::IS_DIR) {
	...
	if(p && !OSSL_PARAM_get_int(p, &ctx->expected_type))
		return 0;
	...
	if(p) {
		...
		if(!OSSL_PARAM_get_octet_string_ptr(p, (const void**)&der, &der_len) || (x509_name = d2i_X509_NAME(NULL, &der, der_len)) == NULL)
			return 0;
		hash = X509_NAME_hash_ex(x509_name, ossl_prov_ctx_get0_libctx((PROV_CTX *)ctx->provctx), NULL, &ok);
		BIO_snprintf(ctx->_.dir.search_name, sizeof(ctx->_.dir.search_name), "%08lx", hash);
...
{
	OSSL_LIB_CTX * libctx = ossl_prov_ctx_get0_libctx((PROV_CTX *)ctx->provctx);
	...
	if(ctx->_.file.decoderctx == NULL) {
		...
		for(to_algo = ossl_any_to_obj_algorithm; to_algo->algorithm_names != NULL; to_algo++) {
			 ...
			 */
			to_obj = (OSSL_DECODER *)ossl_decoder_from_algorithm(0, to_algo, NULL);
			if(to_obj != NULL)
				to_obj_inst = ossl_decoder_instance_new(to_obj, ctx->provctx);
			OSSL_DECODER_free(to_obj);
			if(to_obj_inst == NULL)
				goto err;
			if(!ossl_decoder_ctx_add_decoder_inst(ctx->_.file.decoderctx, to_obj_inst)) {
		...
		if(!OSSL_DECODER_CTX_add_extra(ctx->_.file.decoderctx, libctx, ctx->_.file.propq)) {
			ERR_raise(ERR_LIB_PROV, ERR_R_OSSL_DECODER_LIB);
			goto err;
		}
 ...
 *  -------------------------
 */
static int file_load(void * loaderctx, OSSL_CALLBACK * object_cb, void * object_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	struct file_ctx_st * ctx = (struct file_ctx_st *)loaderctx;
	switch(ctx->type) {
		case file_ctx_st::IS_FILE:
		    return file_load_file(ctx, object_cb, object_cbarg, pw_cb, pw_cbarg);
		case file_ctx_st::IS_DIR:
...
static int file_eof(void * loaderctx)
{
	...
	switch(ctx->type) {
		...
		case file_ctx_st::IS_FILE:
		     ...
		     */
		    return !BIO_pending(ctx->_.file.file) && BIO_eof(ctx->_.file.file);
	}
	/* ctx->type has an unexpected value */
	assert(0);
	return 1;
