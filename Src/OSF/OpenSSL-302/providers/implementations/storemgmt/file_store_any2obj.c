...
static int any2obj_decode_final(void * provctx, int objtype, BUF_MEM * mem, OSSL_CALLBACK * data_cb, void * data_cbarg)
{
	/*
	...
	if(mem != NULL) {
		...
		params[1] = OSSL_PARAM_construct_octet_string(OSSL_OBJECT_PARAM_DATA, mem->data, mem->length);
...
{
	BIO * in = ossl_bio_new_from_core_bio((PROV_CTX*)provctx, cin);
	BUF_MEM * mem = NULL;
	int ok;
	if(!in)
		return 0;
...
static int msblob2obj_decode(void * provctx, OSSL_CORE_BIO * cin, int selection, OSSL_CALLBACK * data_cb, void * data_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	BIO * in = ossl_bio_new_from_core_bio((PROV_CTX*)provctx, cin);
	BUF_MEM * mem = NULL;
	...
	if((mem = BUF_MEM_new()) == NULL || !BUF_MEM_grow(mem, mem_want)) {
...
next:
	...
	/* any2obj_decode_final() frees |mem| for us */
	return any2obj_decode_final(provctx, OSSL_OBJECT_PKEY, mem, data_cb, data_cbarg);
err:
	BIO_free(in);
	BUF_MEM_free(mem);
	return 0;
