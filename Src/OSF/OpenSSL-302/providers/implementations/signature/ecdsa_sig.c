...
static void * ecdsa_newctx(void * provctx, const char * propq)
{
	...
	if(!ctx)
		return NULL;
	ctx->flag_allow_md = 1;
	ctx->libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
...
static int ecdsa_signverify_init(void * vctx, void * ec, const OSSL_PARAM params[], int operation)
{
	...
	if(ec) {
		if(!ossl_ec_check_key(ctx->libctx, (EC_KEY *)ec, operation == EVP_PKEY_OP_SIGN))
			return 0;
		if(!EC_KEY_up_ref((EC_KEY *)ec))
			return 0;
		EC_KEY_free(ctx->ec);
		ctx->ec = (EC_KEY *)ec;
	}
...
static int ecdsa_get_ctx_params(void * vctx, OSSL_PARAM * params)
{
	...
	if(p && !OSSL_PARAM_set_octet_string(p, ctx->aid, ctx->aid_len))
		return 0;
...
static const OSSL_PARAM * ecdsa_settable_ctx_params(void * vctx, ossl_unused void * provctx)
{
	PROV_ECDSA_CTX * ctx = (PROV_ECDSA_CTX*)vctx;
...
const OSSL_DISPATCH ossl_ecdsa_signature_functions[] = {
	...
	{ OSSL_FUNC_SIGNATURE_GET_CTX_MD_PARAMS, (void (*)(void))ecdsa_get_ctx_md_params },
	{ OSSL_FUNC_SIGNATURE_GETTABLE_CTX_MD_PARAMS, (void (*)(void))ecdsa_gettable_ctx_md_params },
	{ OSSL_FUNC_SIGNATURE_SET_CTX_MD_PARAMS, (void (*)(void))ecdsa_set_ctx_md_params },
	{ OSSL_FUNC_SIGNATURE_SETTABLE_CTX_MD_PARAMS, (void (*)(void))ecdsa_settable_ctx_md_params },
	{ 0, NULL }
};
