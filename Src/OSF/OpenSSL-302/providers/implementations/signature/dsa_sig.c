...
static void * dsa_newctx(void * provctx, const char * propq)
{
	...
	if(pdsactx == NULL)
		return NULL;
	pdsactx->libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
...
static int dsa_signverify_init(void * vpdsactx, void * vdsa, const OSSL_PARAM params[], int operation)
{
	...
	if(vdsa != NULL) {
		if(!ossl_dsa_check_key(pdsactx->libctx, (const DSA *)vdsa, operation == EVP_PKEY_OP_SIGN)) {
			ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH);
			return 0;
		}
		if(!DSA_up_ref((DSA *)vdsa))
			return 0;
		DSA_free(pdsactx->dsa);
		pdsactx->dsa = (DSA *)vdsa;
	}
...
static int dsa_get_ctx_params(void * vpdsactx, OSSL_PARAM * params)
{
	...
	if(p && !OSSL_PARAM_set_utf8_string(p, pdsactx->mdname))
		return 0;
	return 1;
}
