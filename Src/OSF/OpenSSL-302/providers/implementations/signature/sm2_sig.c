...
#pragma hdrstop
/*
 * ECDSA low level APIs are deprecated for public use, but still ok for internal use - SM2 implemetation uses ECDSA_size() function.
 */
...
{
	PROV_SM2_CTX * ctx = (PROV_SM2_CTX *)OPENSSL_zalloc(sizeof(PROV_SM2_CTX));
	if(!ctx)
		return NULL;
...
static void * sm2sig_dupctx(void * vpsm2ctx)
{
	...
	PROV_SM2_CTX * dstctx;
	dstctx = (PROV_SM2_CTX *)OPENSSL_zalloc(sizeof(*srcctx));
	if(dstctx == NULL)
		return NULL;
...
static int sm2sig_get_ctx_params(void * vpsm2ctx, OSSL_PARAM * params)
{
	...
	if(p && !OSSL_PARAM_set_utf8_string(p, psm2ctx->md == NULL ? psm2ctx->mdname : EVP_MD_get0_name(psm2ctx->md)))
		return 0;
	return 1;
}
