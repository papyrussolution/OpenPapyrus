	...
	switch(prsactx->pad_mode) {
		...
		case RSA_PKCS1_PSS_PADDING:
		    if(rsa_pss_restricted(prsactx))
			    if((mdname && !EVP_MD_is_a(prsactx->md, mdname))
				|| (mgf1_mdname && !EVP_MD_is_a(prsactx->mgf1_md, mgf1_mdname))) {
...
static void * rsa_newctx(void * provctx, const char * propq)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((prsactx = (PROV_RSA_CTX *)OPENSSL_zalloc(sizeof(PROV_RSA_CTX))) == NULL || (propq && (propq_copy = OPENSSL_strdup(propq)) == NULL)) {
		OPENSSL_free(prsactx);
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
	prsactx->libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
    ...
    const OSSL_PARAM params[], int operation)
{
	...
	if(vrsa) {
		if(!ossl_rsa_check_key(prsactx->libctx, (const RSA *)vrsa, operation))
			return 0;
		if(!RSA_up_ref((RSA *)vrsa))
			return 0;
...
static int rsa_digest_signverify_init(void * vprsactx, const char * mdname, void * vrsa, const OSSL_PARAM params[], int operation)
{
	...
	if(!rsa_signverify_init(vprsactx, vrsa, params, operation))
		return 0;
	if(mdname /* was rsa_setup_md already called in rsa_signverify_init()? */ && (mdname[0] == '\0' || strcasecmp(prsactx->mdname, mdname) != 0) && !rsa_setup_md(prsactx, mdname, prsactx->propq))
		return 0;
...
static void * rsa_dupctx(void * vprsactx)
{
	...
	*dstctx = *srcctx;
	...
	dstctx->tbuf = NULL;
	dstctx->propq = NULL;
	if(srcctx->rsa && !RSA_up_ref(srcctx->rsa))
		goto err;
	dstctx->rsa = srcctx->rsa;
	if(srcctx->md && !EVP_MD_up_ref(srcctx->md))
		goto err;
	dstctx->md = srcctx->md;
	if(srcctx->mgf1_md && !EVP_MD_up_ref(srcctx->mgf1_md))
		goto err;
...
static int rsa_get_ctx_params(void * vprsactx, OSSL_PARAM * params)
{
	...
	if(p) {
		...
		size_t aid_len;
		unsigned char * aid = rsa_generate_signature_aid(prsactx, aid_buf, sizeof(aid_buf), &aid_len);
	...
	if(p && !OSSL_PARAM_set_utf8_string(p, prsactx->mdname))
		return 0;

	p = OSSL_PARAM_locate(params, OSSL_SIGNATURE_PARAM_MGF1_DIGEST);
	if(p && !OSSL_PARAM_set_utf8_string(p, prsactx->mgf1_mdname))
		return 0;
	...
	if(p) {
		if(p->data_type == OSSL_PARAM_INTEGER) {
			...
			switch(prsactx->saltlen) {
				...
				default:
			    {
				    int len = BIO_snprintf((char *)p->data, p->data_size, "%d", prsactx->saltlen);
				    if(len <= 0)
					    return 0;
				    p->return_size = len;
				    break;
			...
			if(value && !OSSL_PARAM_set_utf8_string(p, value))
				return 0;
		}
	}
	return 1;
...
bad_pad:
	...
	if(pmgf1mdname && !rsa_setup_mgf1_md(prsactx, pmgf1mdname, pmgf1mdprops))
		return 0;
	if(pmdname) {
...
{
	PROV_RSA_CTX * prsactx = (PROV_RSA_CTX*)vprsactx;
	if(prsactx && !prsactx->flag_allow_md)
