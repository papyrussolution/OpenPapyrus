...
static void * mac_newctx(void * provctx, const char * propq, const char * macname)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	pmacctx = (PROV_MAC_CTX *)OPENSSL_zalloc(sizeof(PROV_MAC_CTX));
	if(pmacctx == NULL)
		return NULL;
	pmacctx->libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
    ...
    const OSSL_PARAM params[])
{
	...
	if(vkey != NULL) {
		if(!ossl_mac_key_up_ref((MAC_KEY *)vkey))
			return 0;
		ossl_mac_key_free(pmacctx->key);
		pmacctx->key = (MAC_KEY *)vkey;
	}
...
static void * mac_dupctx(void * vpmacctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	dstctx = (PROV_MAC_CTX *)OPENSSL_zalloc(sizeof(*srcctx));
	if(dstctx == NULL)
		return NULL;
	*dstctx = *srcctx;
	dstctx->propq = NULL;
...
static const OSSL_PARAM * mac_settable_ctx_params(ossl_unused void * ctx,
    ...
    const char * macname)
{
	EVP_MAC * mac = EVP_MAC_fetch(PROV_LIBCTX_OF((PROV_CTX*)provctx), macname,
		NULL);
	const OSSL_PARAM * params;
