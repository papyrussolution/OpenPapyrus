...
{
	struct pem2der_ctx_st * ctx = (struct pem2der_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
...
{
	struct pem2der_ctx_st * ctx = (struct pem2der_ctx_st *)vctx;
	OPENSSL_free(ctx);
}
...
{
	struct pem2der_pass_data_st * pass_data = (struct pem2der_pass_data_st *)data;
	size_t plen;
    ...
    OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	...
	for(i = 0; i < SIZEOFARRAY(pem_name_map); i++)
		if(strcmp(pem_name, pem_name_map[i].pem_name) == 0)
			break;

	if(i < SIZEOFARRAY(pem_name_map)) {
