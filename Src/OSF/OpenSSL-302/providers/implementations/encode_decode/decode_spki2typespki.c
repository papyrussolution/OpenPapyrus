...
{
	struct spki2typespki_ctx_st * ctx = (struct spki2typespki_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
...
{
	struct spki2typespki_ctx_st * ctx = (struct spki2typespki_ctx_st *)vctx;
	OPENSSL_free(ctx);
}

static int spki2typespki_decode(void * vctx, OSSL_CORE_BIO * cin, int selection, OSSL_CALLBACK * data_cb, void * data_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	struct spki2typespki_ctx_st * ctx = (struct spki2typespki_ctx_st *)vctx;
