...
{
	struct pvk2key_ctx_st * ctx = (struct pvk2key_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
...
{
	struct pvk2key_ctx_st * ctx = (struct pvk2key_ctx_st *)vctx;
	OPENSSL_free(ctx);
}

static int pvk2key_decode(void * vctx, OSSL_CORE_BIO * cin, int selection, OSSL_CALLBACK * data_cb, void * data_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	struct pvk2key_ctx_st * ctx = (struct pvk2key_ctx_st *)vctx;
	...
	if((selection == 0 || (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) && ctx->desc->read_private_key != NULL) {
		...
		int err, lib, reason;
		memzero(&pwdata, sizeof(pwdata));
		if(!ossl_pw_set_ossl_passphrase_cb(&pwdata, pw_cb, pw_cbarg))
			goto end;
		key = ctx->desc->read_private_key(in, ossl_pw_pvk_password, &pwdata, PROV_LIBCTX_OF(ctx->provctx), NULL);
		/*
...
static int pvk2key_export_object(void * vctx, const void * reference, size_t reference_sz, OSSL_CALLBACK * export_cb, void * export_cbarg)
{
	struct pvk2key_ctx_st * ctx = (struct pvk2key_ctx_st *)vctx;
...
#define IMPLEMENT_MS(KEYTYPE, keytype)                                  \
	...
	const OSSL_DISPATCH ossl_ ## pvk_to_ ## keytype ## _decoder_functions[] = { \
		{ OSSL_FUNC_DECODER_NEWCTX, (void (*)(void))pvk2 ## keytype ## _newctx },                     \
		{ OSSL_FUNC_DECODER_FREECTX, (void (*)(void))pvk2key_freectx },                            \
		{ OSSL_FUNC_DECODER_DECODE, (void (*)(void))pvk2key_decode },                             \
		{ OSSL_FUNC_DECODER_EXPORT_OBJECT, (void (*)(void))pvk2key_export_object },                      \
		{ 0, NULL }                                                     \
	}
