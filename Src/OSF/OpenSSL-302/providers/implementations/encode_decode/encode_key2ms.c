...
{
	struct key2ms_ctx_st * ctx = (struct key2ms_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
...
{
	struct key2ms_ctx_st * ctx = (struct key2ms_ctx_st *)vctx;
...
{
	struct key2ms_ctx_st * ctx = (struct key2ms_ctx_st *)vctx;
	const OSSL_PARAM * p = OSSL_PARAM_locate_const(params, OSSL_ENCODER_PARAM_ENCRYPT_LEVEL);
	if(p && !OSSL_PARAM_get_int(p, &ctx->pvk_encr_level))
		return 0;
	return 1;
}
    ...
    OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	...
	if((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == 0)
		return 0;        /* Error */
	if((pkey = EVP_PKEY_new()) != NULL && set1_key(pkey, key) && (pw_cb == NULL || ossl_pw_set_ossl_passphrase_cb(&ctx->pwdata, pw_cb, pw_cbarg)))
