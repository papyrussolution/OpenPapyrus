...
struct key2any_ctx_st {
	...
	int cipher_intent; /* Set to 1 if intending to encrypt/decrypt, otherwise 0 */
	EVP_CIPHER * cipher;
...
typedef int key_to_der_fn (BIO * out, const void * key, int key_nid, const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx);
...
static PKCS8_PRIV_KEY_INFO * key_to_p8info(const void * key, int key_nid, void * params, int params_type, i2d_of_void * k2d)
{
	...
	if((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL || (derlen = k2d(key, &der)) <= 0 || !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, params_type, params, der, derlen)) {
...
static X509_PUBKEY * key_to_pubkey(const void * key, int key_nid, void * params, int params_type, i2d_of_void k2d)
{
	...
	if((xpk = X509_PUBKEY_new()) == NULL || (derlen = k2d(key, &der)) <= 0 || !X509_PUBKEY_set0_param(xpk, OBJ_nid2obj(key_nid), params_type, params, der, derlen)) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
 ...
 * key data itself.
 */
static int key_to_epki_der_priv_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
...
static int key_to_epki_pem_priv_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
	int strtype = V_ASN1_UNDEF;
...
static int key_to_pki_der_priv_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
	int strtype = V_ASN1_UNDEF;
...
static int key_to_pki_pem_priv_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
	int strtype = V_ASN1_UNDEF;
...
static int key_to_spki_der_pub_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
	int strtype = V_ASN1_UNDEF;
...
static int key_to_spki_pem_pub_bio(BIO * out, const void * key, int key_nid, ossl_unused const char * pemname, key_to_paramstring_fn * p2s, i2d_of_void * k2d, struct key2any_ctx_st * ctx)
{
	int ret = 0;
	void * str = NULL;
	int strtype = V_ASN1_UNDEF;
...
static int dh_spki_pub_to_der(const void * dh, unsigned char ** pder)
{
	...
	int ret;
	if((bn = DH_get0_pub_key((const DH *)dh)) == NULL) {
...
static int dh_pki_priv_to_der(const void * dh, unsigned char ** pder)
{
	...
	if((bn = DH_get0_priv_key((const DH *)dh)) == NULL) {
...
{
	if(DH_test_flags((const DH *)dh, DH_FLAG_TYPE_DHX))
		return i2d_DHxparams((const DH *)dh, pder);
	return i2d_DHparams((const DH *)dh, pder);
}
...
{
	int type = DH_test_flags((const DH *)dh, DH_FLAG_TYPE_DHX) ? EVP_PKEY_DHX : EVP_PKEY_DH;
...
static int dsa_spki_pub_to_der(const void * dsa, unsigned char ** pder)
{
	...
	ASN1_INTEGER * pub_key = NULL;
	int ret;
	if((bn = DSA_get0_pub_key((const DSA *)dsa)) == NULL) {
		ERR_raise(ERR_LIB_PROV, PROV_R_NOT_A_PUBLIC_KEY);
		return 0;
	}
	if((pub_key = BN_to_ASN1_INTEGER(bn, NULL)) == NULL) {
...
static int dsa_pki_priv_to_der(const void * dsa, unsigned char ** pder)
{
	...
	ASN1_INTEGER * priv_key = NULL;
	int ret;
	if((bn = DSA_get0_priv_key((const DSA *)dsa)) == NULL) {
		ERR_raise(ERR_LIB_PROV, PROV_R_NOT_A_PRIVATE_KEY);
		return 0;
	}
	if((priv_key = BN_to_ASN1_INTEGER(bn, NULL)) == NULL) {
...
static int prepare_ec_explicit_params(const void * eckey, void ** pstr, int * pstrtype)
{
	...
	params->length = i2d_ECParameters((const EC_KEY *)eckey, &params->data);
	if(params->length <= 0) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		ASN1_STRING_free(params);
		return 0;
...
{
	if(EC_KEY_get0_public_key((const EC_KEY *)eckey) == NULL) {
		ERR_raise(ERR_LIB_PROV, PROV_R_NOT_A_PUBLIC_KEY);
		return 0;
	}
	return i2o_ECPublicKey((const EC_KEY *)eckey, pder);
}
...
static int prepare_rsa_params(const void * rsa, int nid, int save, void ** pstr, int * pstrtype)
{
	...
	*pstr = NULL;
	switch(RSA_test_flags((const RSA *)rsa, RSA_FLAG_TYPE_MASK)) {
			    ...
			    *pstrtype = V_ASN1_UNDEF;
			    ...
			    for(i = 0; i < 2; i++) {
				    switch(i) {
						...
						break;
					    case 1:
						if((str = (unsigned char *)OPENSSL_malloc(str_sz)) == NULL
...
{
	switch(RSA_test_flags((const RSA *)rsa, RSA_FLAG_TYPE_MASK)) {
		case RSA_FLAG_TYPE_RSA: return expected_type == EVP_PKEY_RSA;
		case RSA_FLAG_TYPE_RSASSAPSS: return expected_type == EVP_PKEY_RSA_PSS;
	}
	/* Currently unsupported RSA key type */
...
{
	struct key2any_ctx_st * ctx = (struct key2any_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
...
static int key2any_encode(struct key2any_ctx_st * ctx, OSSL_CORE_BIO * cout, const void * key, int type, const char * pemname,
    check_key_type_fn * checker, key_to_der_fn * writer, OSSL_PASSPHRASE_CALLBACK * pwcb, void * pwcbarg,
...
#define DO_PRIVATE_KEY(impl, type, kind, output)                            \
	if((selection & DO_PRIVATE_KEY_selection_mask) != 0)                   \
		return key2any_encode((struct key2any_ctx_st *)ctx, cout, key, impl ## _evp_type,              \
...
#define MAKE_ENCODER(impl, type, evp_type, kind, output)                    \
	...
	static int impl ## _to_ ## kind ## _ ## output ## _encode(void * ctx, OSSL_CORE_BIO *cout, const void * key, const OSSL_PARAM key_abstract[],   \
