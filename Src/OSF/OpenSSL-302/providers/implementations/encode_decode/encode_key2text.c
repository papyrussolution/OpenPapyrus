...
static int ec_param_explicit_gen_to_text(BIO * out, const EC_GROUP * group, BN_CTX * ctx)
{
	...
	if(point == NULL)
		return 0;
	switch(form) {
		case POINT_CONVERSION_COMPRESSED: glabel = "Generator (compressed):"; break;
		case POINT_CONVERSION_UNCOMPRESSED: glabel = "Generator (uncompressed):"; break;
		case POINT_CONVERSION_HYBRID: glabel = "Generator (hybrid):"; break;
		default: return 0;
	}
...
{
	const EC_KEY * ec = (const EC_KEY*)key;
	const char * type_label = NULL;
...
{
	const ECX_KEY * ecx = (const ECX_KEY*)key;
	const char * type_label = NULL;
...
{
	BIO * out = ossl_bio_new_from_core_bio((PROV_CTX *)vctx, cout);
	int ret;
	if(!out)
		return 0;
