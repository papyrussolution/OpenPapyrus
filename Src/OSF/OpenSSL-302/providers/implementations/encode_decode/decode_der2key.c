...
struct keytype_desc_st {
	...
	const char * structure_name; /* The input structure name */
	/*
	 ...
	 */
	int evp_type;
	int selection_mask; /* The selection mask for OSSL_FUNC_decoder_does_selection() */
...
struct der2key_ctx_st {
	...
	int selection; /* The selection that is passed to der2key_decode() */
	unsigned int flag_fatal : 1; /* Flag used to signal that a failure is fatal */
};

typedef void * key_from_pkcs8_t (const PKCS8_PRIV_KEY_INFO * p8inf, OSSL_LIB_CTX * libctx, const char * propq);

static void * der2key_decode_p8(const unsigned char ** input_der, long input_der_len, struct der2key_ctx_st * ctx, key_from_pkcs8_t * key_from_pkcs8)
{
	PKCS8_PRIV_KEY_INFO * p8inf = NULL;
	const X509_ALGOR * alg = NULL;
	void * key = NULL;
	if((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, input_der, input_der_len)) != NULL && PKCS8_pkey_get0(NULL, NULL, NULL, &alg, p8inf) && OBJ_obj2nid(alg->algorithm) == ctx->desc->evp_type)
...
static struct der2key_ctx_st * der2key_newctx(void * provctx, const struct keytype_desc_st * desc)                                 
{
	struct der2key_ctx_st * ctx = (struct der2key_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
		ctx->provctx = (PROV_CTX *)provctx;
		ctx->desc = desc;
	}
	return ctx;
}
...
static int der2key_check_selection(int selection, const struct keytype_desc_st * desc)
{
	...
	for(i = 0; i < SIZEOFARRAY(checks); i++) {
...
static int der2key_decode(void * vctx, OSSL_CORE_BIO * cin, int selection, OSSL_CALLBACK * data_cb, void * data_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	struct der2key_ctx_st * ctx = (struct der2key_ctx_st *)vctx;
	 ...
	 */
	if(key != NULL && ctx->desc->check_key != NULL && !ctx->desc->check_key(key, ctx)) {
...
static int der2key_export_object(void * vctx, const void * reference, size_t reference_sz, OSSL_CALLBACK * export_cb, void * export_cbarg)
{
	struct der2key_ctx_st * ctx = (struct der2key_ctx_st *)vctx;
	OSSL_FUNC_keymgmt_export_fn * f_export = ossl_prov_get_keymgmt_export(ctx->desc->fns);
	void * keydata;
	if(reference_sz == sizeof(keydata) && f_export != NULL) {
		/* The contents of the reference is the address to our object */
		keydata = *(void**)reference;
		return f_export(keydata, ctx->selection, export_cb, export_cbarg);
	}
	return 0;
}
...
{
	ossl_dh_set0_libctx((DH *)key, PROV_LIBCTX_OF(ctx->provctx));
}
...
static void * dsa_d2i_PKCS8(void ** key, const unsigned char ** der, long der_len, struct der2key_ctx_st * ctx)
{
	return der2key_decode_p8(der, der_len, ctx, (key_from_pkcs8_t*)ossl_dsa_key_from_pkcs8);
}
...
{
	ossl_dsa_set0_libctx((DSA *)key, PROV_LIBCTX_OF(ctx->provctx));
}
...
static void * ec_d2i_PKCS8(void ** key, const unsigned char ** der, long der_len, struct der2key_ctx_st * ctx)
{
	return der2key_decode_p8(der, der_len, ctx, (key_from_pkcs8_t*)ossl_ec_key_from_pkcs8);
}
...
static int ec_check(void * key, struct der2key_ctx_st * ctx)
{
	...
	int sm2 = (EC_KEY_get_flags((EC_KEY *)key) & EC_FLAG_SM2_RANGE) != 0;
	return sm2 == (ctx->desc->evp_type == EVP_PKEY_SM2);
}

static void ec_adjust(void * key, struct der2key_ctx_st * ctx)
{
	ossl_ec_key_set0_libctx((EC_KEY *)key, PROV_LIBCTX_OF(ctx->provctx));
}
...
static void * ecx_d2i_PKCS8(void ** key, const unsigned char ** der, long der_len, struct der2key_ctx_st * ctx)
{
	return der2key_decode_p8(der, der_len, ctx, (key_from_pkcs8_t*)ossl_ecx_key_from_pkcs8);
}

static void ecx_key_adjust(void * key, struct der2key_ctx_st * ctx)
{
	ossl_ecx_key_set0_libctx((ECX_KEY *)key, PROV_LIBCTX_OF(ctx->provctx));
}
...
{
	switch(RSA_test_flags((RSA *)key, RSA_FLAG_TYPE_MASK)) {
		case RSA_FLAG_TYPE_RSA: return ctx->desc->evp_type == EVP_PKEY_RSA;
		case RSA_FLAG_TYPE_RSASSAPSS: return ctx->desc->evp_type == EVP_PKEY_RSA_PSS;
	}
...
{
	ossl_rsa_set0_libctx((RSA *)key, PROV_LIBCTX_OF(ctx->provctx));
}
...
#define MAKE_DECODER(keytype_name, keytype, type, kind)                 \
	static const struct keytype_desc_st kind ## _ ## keytype ## _desc = { keytype_name, ossl_ ## keytype ## _keymgmt_functions, DO_ ## kind(keytype) }; \
                                                                        \
	...
	static int kind ## _der2 ## keytype ## _does_selection(void * provctx, int selection)     \
	{                                                                   \
		return der2key_check_selection(selection, &kind ## _ ## keytype ## _desc);       \
	}                                                                   \
	const OSSL_DISPATCH ossl_ ## kind ## _der_to_ ## keytype ## _decoder_functions[] = { \
		{ OSSL_FUNC_DECODER_NEWCTX, (void (*)(void))kind ## _der2 ## keytype ## _newctx },              \
		{ OSSL_FUNC_DECODER_FREECTX, (void (*)(void))der2key_freectx },                            \
		{ OSSL_FUNC_DECODER_DOES_SELECTION, (void (*)(void))kind ## _der2 ## keytype ## _does_selection },      \
		{ OSSL_FUNC_DECODER_DECODE, (void (*)(void))der2key_decode },                             \
		{ OSSL_FUNC_DECODER_EXPORT_OBJECT, (void (*)(void))der2key_export_object },                      \
		{ 0, NULL }                                                     \
	}
