...
{
	struct epki2pki_ctx_st * ctx = (struct epki2pki_ctx_st *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
...
{
	struct epki2pki_ctx_st * ctx = (struct epki2pki_ctx_st *)vctx;
	OPENSSL_free(ctx);
}
 ...
 * PrivateKeyInfo.
 */
static int epki2pki_decode(void * vctx, OSSL_CORE_BIO * cin, int selection, OSSL_CALLBACK * data_cb, void * data_cbarg, OSSL_PASSPHRASE_CALLBACK * pw_cb, void * pw_cbarg)
{
	struct epki2pki_ctx_st * ctx = (struct epki2pki_ctx_st *)vctx;
	BUF_MEM * mem = NULL;
