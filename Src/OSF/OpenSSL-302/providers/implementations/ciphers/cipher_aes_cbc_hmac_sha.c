...
static int aes_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		hw->init_mac_key(ctx, (const unsigned char *)p->data, p->data_size);
	}

#if !defined(OPENSSL_NO_MULTIBLOCK)
	p = OSSL_PARAM_locate_const(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_SEND_FRAGMENT);
	if(p && !OSSL_PARAM_get_size_t(p, &ctx->multiblock_max_send_fragment)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GET_PARAMETER);
		return 0;
	}
	/*
	...
	if(p) {
		...
		mb_param.inp = (const unsigned char *)p->data;
	...
	if(p) {
		...
		mb_param.out = (unsigned char *)p->data;
		mb_param.inp = (unsigned char *)pin->data;
	...
	if(p) {
		...
		if(hw->set_tls1_aad(ctx, (unsigned char *)p->data, (int)p->data_size) <= 0)
			return 0;
	}
...
static int aes_get_ctx_params(void * vctx, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_uint(p, ctx->multiblock_interleave)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD_PACKLEN);
	if(p && !OSSL_PARAM_set_uint(p, ctx->multiblock_aad_packlen)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_LEN);
	if(p && !OSSL_PARAM_set_size_t(p, ctx->multiblock_encrypt_len)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
#endif /* !defined(OPENSSL_NO_MULTIBLOCK) */
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_AEAD_TLS1_AAD_PAD);
	if(p && !OSSL_PARAM_set_size_t(p, ctx->tls_aad_pad)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_KEYLEN);
	if(p && !OSSL_PARAM_set_size_t(p, ctx->base.keylen)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
...
static void * aes_cbc_hmac_sha1_newctx(void * provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (PROV_AES_HMAC_SHA1_CTX *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
...
static void * aes_cbc_hmac_sha256_newctx(void * provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (PROV_AES_HMAC_SHA256_CTX *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
