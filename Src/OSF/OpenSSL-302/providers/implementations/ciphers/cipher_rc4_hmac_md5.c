...
#pragma hdrstop
/*
 * MD5 and RC4 low level APIs are deprecated for public use, but still ok for internal use.
 */
...
static void * rc4_hmac_md5_newctx(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (PROV_RC4_HMAC_MD5_CTX *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx)
		ossl_cipher_generic_initkey(ctx, RC4_HMAC_MD5_KEY_BITS, RC4_HMAC_MD5_BLOCK_BITS, RC4_HMAC_MD5_IV_BITS,
		    RC4_HMAC_MD5_MODE, RC4_HMAC_MD5_FLAGS, ossl_prov_cipher_hw_rc4_hmac_md5(RC4_HMAC_MD5_KEY_BITS), NULL);
	return ctx;
}
...
static int rc4_hmac_md5_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		sz = GET_HW(ctx)->tls_init(&ctx->base, (unsigned char *)p->data, p->data_size);
		if(sz == 0) {
	...
	if(p) {
		...
		GET_HW(ctx)->init_mackey(&ctx->base, (const unsigned char *)p->data, p->data_size);
	}
