...
static void * rc5_dupctx(void * ctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ret = (PROV_RC5_CTX *)OPENSSL_malloc(sizeof(*ret));
	if(ret == NULL) {
...
static int rc5_einit(void * ctx, const uchar * key, size_t keylen, const uchar * iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int rc5_dinit(void * ctx, const uchar * key, size_t keylen,
...
#define IMPLEMENT_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ) \
	...
	const OSSL_DISPATCH ossl_ ## alg ## kbits ## lcmode ## _functions[] = {                \
		...
		{ OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))rc5_settable_ctx_params }, \
		{ 0, NULL }                                                                \
	};
