 ...
 *       data with them."
 */
static int aes_xts_check_keys_differ(const unsigned char * key, size_t bytes, int enc)
{
	if((!ossl_aes_xts_allow_insecure_decrypt || enc) && CRYPTO_memcmp(key, key + bytes, bytes) == 0) {
    ...
    const OSSL_PARAM params[], int enc)
{
	...
	ctx->enc = enc;
	if(iv) {
		if(!ossl_cipher_generic_initiv((PROV_CIPHER_CTX *)vctx, iv, ivlen))
			return 0;
	}
	if(key) {
...
static int aes_xts_dinit(void * vctx, const unsigned char * key, size_t keylen, const unsigned char * iv, size_t ivlen, const OSSL_PARAM params[])
{
	return aes_xts_init(vctx, key, keylen, iv, ivlen, params, 0);
}

static void * aes_xts_newctx(void * provctx, unsigned int mode, uint64_t flags, size_t kbits, size_t blkbits, size_t ivbits)
{
	PROV_AES_XTS_CTX * ctx = (PROV_AES_XTS_CTX *)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
		ossl_cipher_generic_initkey(&ctx->base, kbits, blkbits, ivbits, mode, flags, ossl_prov_cipher_hw_aes_xts(kbits), NULL);
	}
	return ctx;
}
...
static int aes_xts_cipher(void * vctx, unsigned char * out, size_t * outl, size_t outsize, const unsigned char * in, size_t inl)
{
	PROV_AES_XTS_CTX * ctx = (PROV_AES_XTS_CTX*)vctx;
	if(!ossl_prov_is_running() || ctx->xts.key1 == NULL || ctx->xts.key2 == NULL || !ctx->base.iv_set || 
		out == NULL || in == NULL || inl < AES_BLOCK_SIZE)
		return 0;
	/*
	...
	if(ctx->stream != NULL)
		(*ctx->stream)(in, out, inl, (const AES_KEY *)ctx->xts.key1, (const AES_KEY *)ctx->xts.key2, ctx->base.iv);
...
static int aes_xts_stream_final(void * vctx, unsigned char * out, size_t * outl, size_t outsize)
{
	if(!ossl_prov_is_running())
		return 0;
	*outl = 0;
