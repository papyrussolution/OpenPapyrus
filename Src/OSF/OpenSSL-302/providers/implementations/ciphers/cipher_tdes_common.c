    ...
    size_t ivbits, uint64_t flags, const PROV_CIPHER_HW * hw)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	tctx = (PROV_TDES_CTX*)OPENSSL_zalloc(sizeof(*tctx));
	if(tctx != NULL)
...
void * ossl_tdes_dupctx(void * ctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ret = (PROV_TDES_CTX *)OPENSSL_malloc(sizeof(*ret));
	if(!ret) {
...
int ossl_tdes_einit(void * vctx, const unsigned char * key, size_t keylen, const unsigned char * iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int tdes_generatekey(PROV_CIPHER_CTX * ctx, void * ptr)
{
	...
	if(kl == 0 || RAND_priv_bytes_ex(ctx->libctx, (unsigned char *)ptr, kl, 0) <= 0)
		return 0;
...
int ossl_tdes_get_ctx_params(void * vctx, OSSL_PARAM params[])
{
	...
	if(p && !tdes_generatekey(ctx, p->data)) {
