...
const char * ossl_cipher_cbc_cts_mode_id2name(unsigned int id)
{
	...
	for(i = 0; i < SIZEOFARRAY(cts_modes); ++i) {
    ...
    unsigned char * out, size_t len)
{
	...
	in += len;
	out += len;
	memzero(tmp_in.c, sizeof(tmp_in));
...
int ossl_cipher_cbc_cts_block_update(void * vctx, unsigned char * out, size_t * outl, size_t outsize, const unsigned char * in, size_t inl)
{
	...
	/*
	 * Return an error if the update is called multiple times, only one shot is supported.
	 */
	if(ctx->updated == 1)
		return 0;
