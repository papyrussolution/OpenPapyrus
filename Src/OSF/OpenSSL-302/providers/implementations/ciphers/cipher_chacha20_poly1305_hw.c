...
static int chacha20_poly1305_initiv(PROV_CIPHER_CTX * bctx)
{
	...
	if(ctx->nonce_len <= CHACHA_CTR_SIZE) {
		memcpy(tempiv + CHACHA_CTR_SIZE - ctx->nonce_len, bctx->oiv, ctx->nonce_len);
		if(bctx->enc)
...
static int chacha20_poly1305_tls_cipher(PROV_CIPHER_CTX * bctx, unsigned char * out, size_t * out_padlen, const unsigned char * in, size_t len)
{
	...
	if(plen <= 3 * CHACHA_BLK_SIZE) {
		...
		if(plen) {
			if(bctx->enc)
				ctr = (unsigned char *)xor128_encrypt_n_pad(out, in, ctr, plen);
			else
				ctr = (unsigned char *)xor128_decrypt_n_pad(out, in, ctr, plen);
			in += plen;
			out += plen;
