...
static void * chacha20_poly1305_newctx(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (PROV_CHACHA20_POLY1305_CTX*)OPENSSL_zalloc(sizeof(*ctx));
	if(ctx) {
...
static int chacha20_poly1305_get_ctx_params(void * vctx, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_size_t(p, CHACHA20_POLY1305_KEYLEN)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_AEAD_TAGLEN);
	if(p && !OSSL_PARAM_set_size_t(p, ctx->tag_len)) {
		ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
		return 0;
	}
    ...
    const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		len = hw->tls_init(&ctx->base, (unsigned char *)p->data, p->data_size);
		if(!len) {
	...
	if(p) {
		...
		if(hw->tls_iv_set_fixed(&ctx->base, (unsigned char *)p->data, p->data_size) == 0) {
			ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_IV_LENGTH);
			return 0;
		}
	}
