...
size_t ossl_crngt_get_entropy(PROV_DRBG * drbg, unsigned char ** pout, int entropy, size_t min_len, size_t max_len, int prediction_resistance)
{
	...
	CRNG_TEST_GLOBAL * crngt_glob = (CRNG_TEST_GLOBAL *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_RAND_CRNGT_INDEX, &rand_crng_ossl_ctx_method);
	...
	if(!crngt_glob->preloaded) {
		if(!crngt_get_entropy(drbg->provctx, crngt_glob->md, buf, crngt_glob->crngt_prev, NULL)) {
	...
	if(bytes_needed > max_len)
		goto unlock_return;
	entp = ent = (unsigned char *)OPENSSL_secure_malloc(bytes_needed);
	if(ent == NULL)
		goto unlock_return;
