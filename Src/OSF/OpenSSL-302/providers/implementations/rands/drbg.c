    ...
    const unsigned char * adin, size_t adin_len)
{
	...
	/* Allocate storage */
	buffer = (unsigned char *)OPENSSL_secure_malloc(bytes_needed);
	if(buffer == NULL) {
...
{
	PROV_DRBG_NONCE_GLOBAL * dngbl = (PROV_DRBG_NONCE_GLOBAL *)OPENSSL_zalloc(sizeof(*dngbl));
	if(dngbl == NULL)
		return NULL;
...
{
	PROV_DRBG_NONCE_GLOBAL * dngbl = (PROV_DRBG_NONCE_GLOBAL *)vdngbl;
	if(dngbl == NULL)
		return;
    ...
    size_t min_len, size_t max_len)
{
	...
	PROV_DRBG_NONCE_GLOBAL * dngbl = (PROV_DRBG_NONCE_GLOBAL *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_DRBG_NONCE_INDEX,
	...
	if(drbg->parent != NULL && drbg->parent_nonce != NULL) {
		...
		if(n > 0 && (buf = (unsigned char *)OPENSSL_malloc(n)) != NULL) {
    ...
    const unsigned char * pers, size_t perslen)
{
	...
	drbg->state = (DRBG_STATUS)EVP_RAND_STATE_ERROR;
	if(drbg->min_noncelen > 0) {
		if(drbg->parent_nonce != NULL) {
			...
			nonce = (unsigned char *)OPENSSL_malloc(noncelen);
			if(nonce == NULL) {
				ERR_raise(ERR_LIB_PROV, PROV_R_ERROR_RETRIEVING_NONCE);
				goto end;
			}
	...
	cleanup_entropy(drbg, entropy, entropylen);
	drbg->state = (DRBG_STATUS)EVP_RAND_STATE_READY;
...
{
	drbg->state = (DRBG_STATUS)EVP_RAND_STATE_UNINITIALISED;
	return 1;
}
 ...
 *
 */
int ossl_prov_drbg_generate(PROV_DRBG * drbg, unsigned char * out, size_t outlen, unsigned int strength, int prediction_resistance,
    const unsigned char * adin, size_t adinlen)
{
	int fork_id;
...
int ossl_drbg_get_ctx_params(PROV_DRBG * drbg, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_uint(p, drbg->reseed_interval))
		return 0;
	p = OSSL_PARAM_locate(params, OSSL_DRBG_PARAM_RESEED_TIME);
	if(p && !OSSL_PARAM_set_time_t(p, drbg->reseed_time))
		return 0;
...
int ossl_drbg_set_ctx_params(PROV_DRBG * drbg, const OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_get_uint(p, &drbg->reseed_interval))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_DRBG_PARAM_RESEED_TIME_INTERVAL);
	if(p && !OSSL_PARAM_get_time_t(p, &drbg->reseed_time_interval))
		return 0;
	return 1;
}
