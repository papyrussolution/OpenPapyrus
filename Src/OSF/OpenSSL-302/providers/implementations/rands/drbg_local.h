...
#define TSC_READ_COUNT                 4 /* How many times to read the TSC as a randomness source. */
...
int ossl_prov_drbg_generate(PROV_DRBG * drbg, unsigned char * out, size_t outlen, unsigned int strength, int prediction_resistance, const unsigned char * adin, size_t adinlen);
...
#define PROV_DRBG_VERYIFY_ZEROIZATION(v)    \
	{                                       \
		for(size_t i = 0; i < SIZEOFARRAY(v); i++) \
			if((v)[i] != 0)                \
				return 0;                   \
	}
