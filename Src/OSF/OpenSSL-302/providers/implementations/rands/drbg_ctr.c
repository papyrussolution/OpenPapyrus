...
{
	if(ctr->bltmp_pos) {
		memzero(ctr->bltmp + ctr->bltmp_pos, 16 - ctr->bltmp_pos);
		if(!ctr_BCC_blocks(ctr, ctr->bltmp))
			return 0;
	}
	return 1;
...
static int drbg_ctr_instantiate(PROV_DRBG * drbg, const unsigned char * entropy, size_t entropylen, const unsigned char * nonce, size_t noncelen,
    const unsigned char * pers, size_t perslen)
{
	PROV_DRBG_CTR * ctr = (PROV_DRBG_CTR*)drbg->data;
	if(entropy == NULL)
		return 0;
	memzero(ctr->K, sizeof(ctr->K));
	memzero(ctr->V, sizeof(ctr->V));
...
{
	PROV_DRBG_CTR * ctr = (PROV_DRBG_CTR*)OPENSSL_secure_zalloc(sizeof(*ctr));
	if(ctr == NULL) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		return 0;
	}
...
static void drbg_ctr_free(void * vdrbg)
{
	...
	if(drbg && (ctr = (PROV_DRBG_CTR*)drbg->data) != NULL) {
		EVP_CIPHER_CTX_free(ctr->ctx_ecb);
		EVP_CIPHER_CTX_free(ctr->ctx_ctr);
...
static int drbg_ctr_get_ctx_params(void * vdrbg, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_int(p, ctr->use_df))
		return 0;
