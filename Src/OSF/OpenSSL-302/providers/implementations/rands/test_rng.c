...
{
	PROV_TEST_RNG * t = (PROV_TEST_RNG *)OPENSSL_zalloc(sizeof(*t));
	if(t == NULL)
		return NULL;
...
static int test_rng_get_ctx_params(void * vtest, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_int(p, t->state))
		return 0;

	p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_STRENGTH);
	if(p && !OSSL_PARAM_set_int(p, t->strength))
		return 0;

	p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_MAX_REQUEST);
	if(p && !OSSL_PARAM_set_size_t(p, t->max_request))
		return 0;
	return 1;
}
...
static int test_rng_set_ctx_params(void * vtest, const OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_get_uint(p, &t->strength))
		return 0;
