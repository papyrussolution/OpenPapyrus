...
{
	KECCAK1600_CTX * ctx = (KECCAK1600_CTX*)vctx;
	return SHA3_absorb(ctx->A, (const unsigned char *)inp, len, ctx->block_size);
}
...
#define SHA3_newctx(typ, uname, name, bitlen, pad)                             \
	...
	{                                                                              \
		KECCAK1600_CTX * ctx = ossl_prov_is_running() ? (KECCAK1600_CTX*)OPENSSL_zalloc(sizeof(*ctx)) : NULL; \
		if(!ctx)                                                           \
			return NULL;                                                           \
...
static void * keccak_dupctx(void * ctx)
{
	...
	KECCAK1600_CTX * ret = ossl_prov_is_running() ? (KECCAK1600_CTX*)OPENSSL_malloc(sizeof(*ret))
	    : NULL;
...
static int shake_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_get_size_t(p, &ctx->md_size)) {
