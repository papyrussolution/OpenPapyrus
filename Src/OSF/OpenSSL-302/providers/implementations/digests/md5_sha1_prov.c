...
#pragma hdrstop
/*
 * MD5 and SHA-1 low level APIs are deprecated for public use, but still ok for internal use.
 */
...
static const OSSL_PARAM * md5_sha1_settable_ctx_params(ossl_unused void * ctx, ossl_unused void * provctx)
{
...
static int md5_sha1_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	...
	if(p && p->data_type == OSSL_PARAM_OCTET_STRING)
		return ossl_md5_sha1_ctrl(ctx, EVP_CTRL_SSL3_MASTER_SECRET, p->data_size, p->data);
	return 1;
}
