...
static void * kdf_pkcs12_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (KDF_PKCS12 *)OPENSSL_zalloc(sizeof(*ctx));
	if(!ctx) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
static const OSSL_PARAM * kdf_pkcs12_settable_ctx_params(ossl_unused void * ctx, ossl_unused void * provctx)
{
...
static const OSSL_PARAM * kdf_pkcs12_gettable_ctx_params(ossl_unused void * ctx, ossl_unused void * provctx)
{
