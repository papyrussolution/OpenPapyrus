    ...
    uint64_t iter, const EVP_MD * md_type, unsigned char * out, size_t n)
{
	...
	if(!EVP_DigestInit_ex(ctx, md_type, NULL) || !EVP_DigestUpdate(ctx, pass, passlen) || !EVP_DigestUpdate(ctx, salt, saltlen) || !EVP_DigestFinal_ex(ctx, md_tmp, NULL))
		goto err;
...
static void * kdf_pbkdf1_new(void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	ctx = (KDF_PBKDF1 *)OPENSSL_zalloc(sizeof(*ctx));
	if(!ctx) {
		ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
static int kdf_pbkdf1_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	const OSSL_PARAM * p;
	KDF_PBKDF1 * ctx = (KDF_PBKDF1 *)vctx;
	OSSL_LIB_CTX * libctx = (OSSL_LIB_CTX *)PROV_LIBCTX_OF((PROV_CTX *)ctx->provctx);
