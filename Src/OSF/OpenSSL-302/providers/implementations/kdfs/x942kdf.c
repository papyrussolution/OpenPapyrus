    ...
    const char * propq, size_t * id)
{
	...
	if(cipher != NULL) {
		for(i = 0; i < SIZEOFARRAY(kek_algs); i++) {
    ...
    unsigned char ** out_ctr)
{
	...
	WPACKET_cleanup(&pkt);
	/* Alloc the buffer */
	der_buf = (unsigned char *)OPENSSL_zalloc(der_buflen);
	if(der_buf == NULL)
		goto err;
...
static int x942kdf_set_ctx_params(void * vctx, const OSSL_PARAM params[])
{
	...
	KDF_X942 * ctx = (KDF_X942 *)vctx;
	OSSL_LIB_CTX * provctx = PROV_LIBCTX_OF((PROV_CTX*)ctx->provctx);
	...
	if(p != NULL && !x942kdf_set_buffer(&ctx->supp_privinfo, &ctx->supp_privinfo_len, p))
		return 0;
	...
	if(p) {
		 ...
		 */
		if(pq != NULL)
			propq = (const char *)p->data;
		if(find_alg_id(provctx, (const char *)p->data, propq, &id) == 0)
			return 0;
...
static const OSSL_PARAM * x942kdf_settable_ctx_params(ossl_unused void * ctx, ossl_unused void * provctx)
{
