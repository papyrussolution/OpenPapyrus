...
static int dsa_gen_type_name2id(const char * name)
{
	...
	for(i = 0; i < SIZEOFARRAY(dsatype2id); ++i) {
...
static void * dsa_newdata(void * provctx)
{
	if(!ossl_prov_is_running())
		return NULL;
	return ossl_dsa_new(PROV_LIBCTX_OF((PROV_CTX*)provctx));
}
...
{
	const DSA * dsa = (const DSA *)keydata;
	int ok = 1;
...
static void * dsa_gen_init(void * provctx, int selection, const OSSL_PARAM params[])
{
	...
	if(!ossl_prov_is_running() || (selection & DSA_POSSIBLE_SELECTIONS) == 0)
		return NULL;
	if((gctx = (struct dsa_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
...
static int dsa_set_gen_seed(struct dsa_gen_ctx * gctx, const unsigned char * seed, size_t seedlen)
{
...
static int dsa_gen_set_params(void * genctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		if(p->data_type != OSSL_PARAM_UTF8_STRING || ((gctx->gen_type = dsa_gen_type_name2id((const char *)p->data)) == -1)) {
	...
	if(p && (p->data_type != OSSL_PARAM_OCTET_STRING || !dsa_set_gen_seed(gctx, (const unsigned char *)p->data, p->data_size)))
		return 0;
...
{
	struct dsa_gen_ctx * gctx = (struct dsa_gen_ctx *)BN_GENCB_get_arg(cb);
...
{
	if(ossl_prov_is_running())
		return ossl_dsa_dup((const DSA *)keydata_from, selection);
	return NULL;
}
