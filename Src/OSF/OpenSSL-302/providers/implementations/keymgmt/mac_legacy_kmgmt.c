...
MAC_KEY * ossl_mac_key_new(OSSL_LIB_CTX * libctx, int cmac)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	mackey = (MAC_KEY *)OPENSSL_zalloc(sizeof(*mackey));
	if(mackey == NULL)
		return NULL;
...
{
	return ossl_mac_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), 0);
}

static void * mac_new_cmac(void * provctx)
{
	return ossl_mac_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), 1);
}
...
{
	const MAC_KEY * key = (const MAC_KEY *)keydata;
	int ok = 0;
...
{
	const MAC_KEY * key1 = (const MAC_KEY *)keydata1;
	const MAC_KEY * key2 = (const MAC_KEY *)keydata2;
	int ok = 1;
	if(!ossl_prov_is_running())
		return 0;
	if((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
...
static int mac_key_fromdata(MAC_KEY * key, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		key->priv_key = (unsigned char *)OPENSSL_secure_malloc(p->data_size > 0 ? p->data_size : 1);
	...
	if(p) {
		...
		key->properties = OPENSSL_strdup((const char *)p->data);
...
{
	return key_to_params((MAC_KEY *)key, NULL, params);
}
...
static void * mac_gen_init_common(void * provctx, int selection)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((gctx = (struct mac_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
		gctx->libctx = libctx;
...
{
	struct mac_gen_ctx * gctx = (struct mac_gen_ctx *)mac_gen_init_common(provctx, selection);
...
{
	struct mac_gen_ctx * gctx = (struct mac_gen_ctx *)genctx;
	const OSSL_PARAM * p;
	if(gctx == NULL)
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_PRIV_KEY);
	if(p) {
		...
		gctx->priv_key = (unsigned char *)OPENSSL_secure_malloc(p->data_size);
