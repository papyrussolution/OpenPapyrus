...
{
	const RSA * rsa = (const RSA *)keydata;
	int ok = 1;
...
static int rsa_get_params(void * key, OSSL_PARAM params[])
{
	 ...
	 */
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_DEFAULT_DIGEST)) != NULL && (rsa_type != RSA_FLAG_TYPE_RSASSAPSS
	 ...
	 * With RSA-OAEP keys, this may need to be amended.
	 */
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_MANDATORY_DIGEST)) != NULL && rsa_type == RSA_FLAG_TYPE_RSASSAPSS && !ossl_rsa_pss_params_30_is_unrestricted(pss_params)) {
...
static void * rsa_dup(const void * keydata_from, int selection)
{
	if(ossl_prov_is_running()
		...
		return ossl_rsa_dup((RSA *)keydata_from, selection);
	return NULL;
}
