...
#pragma hdrstop
/*
 * ECDH/ECDSA low level APIs are deprecated for public use, but still ok for internal use.
 */
    ...
    unsigned char ** pub_key)
{
	...
	if(pub_point != NULL) {
		...
		if(p != NULL || tmpl != NULL) {
			...
			if((pub_key_len = EC_POINT_point2buf(ecg, pub_point, POINT_CONVERSION_COMPRESSED, pub_key, bnctx)) == 0 || 
...
void * ec_newdata(void * provctx)
{
	if(!ossl_prov_is_running())
		return NULL;
	return EC_KEY_new_ex(PROV_LIBCTX_OF((PROV_CTX*)provctx), NULL);
}
...
static void * sm2_newdata(void * provctx)
{
	if(!ossl_prov_is_running())
		return NULL;
	return EC_KEY_new_by_curve_name_ex(PROV_LIBCTX_OF((PROV_CTX*)provctx), NULL, NID_sm2);
}
#endif
#endif
...
{
	const EC_KEY * ec = (const EC_KEY *)keydata;
	int ok = 1;
...
static int ec_set_params(void * key, const OSSL_PARAM params[])
{
	...
	if(!params)
		return 1;
	if(!ossl_ec_group_set_params((EC_GROUP*)EC_KEY_get0_group((EC_KEY *)key), params))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY);
	if(p) {
		BN_CTX * ctx = BN_CTX_new_ex(ossl_ec_key_get_libctx((EC_KEY *)key));
		int ret = 1;
		if(!ctx || p->data_type != OSSL_PARAM_OCTET_STRING || !EC_KEY_oct2key((EC_KEY *)key, (const unsigned char *)p->data, p->data_size, ctx))
			ret = 0;
		BN_CTX_free(ctx);
		if(!ret)
			return 0;
...
{
	const EC_KEY * eck = (const EC_KEY *)keydata;
	int ok = 1;
	BN_CTX * ctx = NULL;
    ...
    const OSSL_PARAM params[])
{
	...
	if(!ossl_prov_is_running() || (selection & (EC_POSSIBLE_SELECTIONS)) == 0)
		return NULL;
	if((gctx = (struct ec_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
		gctx->libctx = libctx;
...
static void * sm2_gen_init(void * provctx, int selection,
    const OSSL_PARAM params[])
{
	struct ec_gen_ctx * gctx = (struct ec_gen_ctx *)ec_gen_init(provctx, selection, params);
	if(gctx != NULL) {
...
#define COPY_UTF8_PARAM(params, key, val)                                      \
	...
	if(p) {                                                               \
		if(p->data_type != OSSL_PARAM_UTF8_STRING)                                \
			goto err;                                                              \
		OPENSSL_free(val);                                                         \
		val = OPENSSL_strdup((const char *)p->data);                               \
		if(val == NULL)                                                           \
			goto err;                                                              \
	}
...
{
	if(ossl_prov_is_running())
		return ossl_ec_key_dup((const EC_KEY *)keydata_from, selection);
	return NULL;
}
