...
static void * x25519_new_key(void * provctx)
{
	if(!ossl_prov_is_running())
		return 0;
	return ossl_ecx_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), ECX_KEY_TYPE_X25519, 0,
		   NULL);
}
...
static void * x448_new_key(void * provctx)
{
	if(!ossl_prov_is_running())
		return 0;
	return ossl_ecx_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), ECX_KEY_TYPE_X448, 0,
		   NULL);
}
...
static void * ed25519_new_key(void * provctx)
{
	if(!ossl_prov_is_running())
		return 0;
	return ossl_ecx_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), ECX_KEY_TYPE_ED25519, 0,
		   NULL);
}
...
static void * ed448_new_key(void * provctx)
{
	if(!ossl_prov_is_running())
		return 0;
	return ossl_ecx_key_new(PROV_LIBCTX_OF((PROV_CTX*)provctx), ECX_KEY_TYPE_ED448, 0,
		   NULL);
}

static int ecx_has(const void * keydata, int selection)
{
	const ECX_KEY * key = (const ECX_KEY *)keydata;
	int ok = 0;
...
static int ecx_set_params(void * key, const OSSL_PARAM params[])
{
	...
	if(p) {
		if(p->data_type != OSSL_PARAM_UTF8_STRING || !set_property_query(ecxkey, (const char *)p->data))
			return 0;
	}
	return 1;
}
    ...
    const OSSL_PARAM params[], ECX_KEY_TYPE type)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	if((gctx = (struct ecx_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
		gctx->libctx = libctx;
		gctx->type = type;
...
static int ecx_gen_set_params(void * genctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		gctx->propq = OPENSSL_strdup((const char *)p->data);
		if(gctx->propq == NULL)
			return 0;
	}
...
{
	if(ossl_prov_is_running())
		return ossl_ecx_key_dup((const ECX_KEY *)keydata_from, selection);
	return NULL;
}
...
{
	const ECX_KEY * ecx = (const ECX_KEY *)keydata;
