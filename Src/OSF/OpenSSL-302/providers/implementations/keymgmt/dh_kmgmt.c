...
static void * dh_newdata(void * provctx)
{
	...
	if(ossl_prov_is_running()) {
		dh = ossl_dh_new_ex(PROV_LIBCTX_OF((PROV_CTX*)provctx));
		if(dh != NULL) {
...
static ossl_inline int dh_get_params(void * key, OSSL_PARAM params[])
{
	DH * dh = (DH *)key;
	OSSL_PARAM * p;
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_BITS)) != NULL && !OSSL_PARAM_set_int(p, DH_bits(dh)))
		return 0;
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_SECURITY_BITS)) != NULL && !OSSL_PARAM_set_int(p, DH_security_bits(dh)))
		return 0;
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_MAX_SIZE)) != NULL && !OSSL_PARAM_set_int(p, DH_size(dh)))
		return 0;
	if((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY)) != NULL) {
		if(p->data_type != OSSL_PARAM_OCTET_STRING)
			return 0;
		p->return_size = ossl_dh_key2buf(dh, (unsigned char**)&p->data, p->data_size, 0);
		if(p->return_size == 0)
			return 0;
	}
	return ossl_dh_params_todata(dh, NULL, params) && ossl_dh_key_todata(dh, NULL, params, 1);
}
...
static int dh_set_params(void * key, const OSSL_PARAM params[])
{
	...
	if(p && (p->data_type != OSSL_PARAM_OCTET_STRING || !ossl_dh_buf2key(dh, (const unsigned char *)p->data, p->data_size)))
		return 0;
	return 1;
}
...
{
	OSSL_LIB_CTX * libctx = PROV_LIBCTX_OF((PROV_CTX*)provctx);
	struct dh_gen_ctx * gctx = NULL;
	if(!ossl_prov_is_running())
		return NULL;
	if((selection & (OSSL_KEYMGMT_SELECT_KEYPAIR|OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS)) == 0)
		return NULL;
	if((gctx = (struct dh_gen_ctx *)OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
...
static int dh_set_gen_seed(struct dh_gen_ctx * gctx, unsigned char * seed, size_t seedlen)
{
	...
	if(seed != NULL && seedlen > 0) {
		gctx->seed = (unsigned char *)OPENSSL_memdup(seed, seedlen);
		if(gctx->seed == NULL)
			return 0;
...
static int dh_gen_common_set_params(void * genctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		if(p->data_type != OSSL_PARAM_UTF8_STRING || ((gctx->gen_type = dh_gen_type_name2id_w_default((const char *)p->data, gctx->dh_type)) == -1)) {
	...
	if(p) {
		...
		if(p->data_type != OSSL_PARAM_UTF8_STRING || p->data == NULL || (group = ossl_ffc_name_to_dh_named_group((const char *)p->data)) == NULL || 
			((gctx->group_nid = ossl_ffc_named_group_get_uid(group)) == NID_undef)) {
			ERR_raise(ERR_LIB_PROV, ERR_R_PASSED_INVALID_ARGUMENT);
			return 0;
		}
	}
	if((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_PBITS)) != NULL && !OSSL_PARAM_get_size_t(p, &gctx->pbits))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_DH_PRIV_LEN);
	if(p && !OSSL_PARAM_get_int(p, &gctx->priv_len))
		return 0;
	return 1;
}
...
static int dhx_gen_set_params(void * genctx, const OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_get_int(p, &gctx->gindex))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_PCOUNTER);
	if(p && !OSSL_PARAM_get_int(p, &gctx->pcounter))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_H);
	if(p && !OSSL_PARAM_get_int(p, &gctx->hindex))
		return 0;
	p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_FFC_SEED);
	if(p && (p->data_type != OSSL_PARAM_OCTET_STRING || !dh_set_gen_seed(gctx, (unsigned char *)p->data, p->data_size)))
		return 0;
	...
	if(p) {
		...
		gctx->mdname = OPENSSL_strdup((const char *)p->data);
		if(gctx->mdname == NULL)
			return 0;
	}
	...
	if(p) {
		...
		gctx->mdprops = OPENSSL_strdup((const char *)p->data);
...
{
	struct dh_gen_ctx * gctx = (struct dh_gen_ctx *)BN_GENCB_get_arg(cb);
...
{
	if(ossl_prov_is_running())
		return ossl_dh_dup((const DH *)keydata_from, selection);
	return NULL;
}
...
const OSSL_DISPATCH ossl_dh_keymgmt_functions[] = {
	...
	{ OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS, (void (*)(void))dh_gen_settable_params },
