...
static void * ecdh_newctx(void * provctx)
{
	...
	if(pectx == NULL)
		return NULL;
	pectx->libctx = PROV_LIBCTX_OF((PROV_CTX *)provctx);
...
static int ecdh_init(void * vpecdhctx, void * vecdh, const OSSL_PARAM params[])
{
	...
	if(!ossl_prov_is_running() || pecdhctx == NULL || vecdh == NULL || !EC_KEY_up_ref((EC_KEY *)vecdh))
		return 0;
	EC_KEY_free(pecdhctx->k);
	pecdhctx->k = (EC_KEY *)vecdh;
	pecdhctx->cofactor_mode = -1;
	pecdhctx->kdf_type = PROV_ECDH_KDF_NONE;
	return ecdh_set_ctx_params(pecdhctx, params) && ossl_ec_check_key(pecdhctx->libctx, (EC_KEY *)vecdh, 1);
}
...
{
	PROV_ECDH_CTX * pecdhctx = (PROV_ECDH_CTX*)vpecdhctx;
	if(!ossl_prov_is_running() || pecdhctx == NULL || vecdh == NULL || 
		!ecdh_match_params(pecdhctx->k, (EC_KEY *)vecdh) || !ossl_ec_check_key(pecdhctx->libctx, (EC_KEY *)vecdh, 1) || !EC_KEY_up_ref((EC_KEY *)vecdh))
		return 0;
...
static void * ecdh_dupctx(void * vpecdhctx)
{
	...
	*dstctx = *srcctx;
	...
	if(srcctx->kdf_ukm != NULL && srcctx->kdf_ukmlen > 0) {
		dstctx->kdf_ukm = (unsigned char *)OPENSSL_memdup(srcctx->kdf_ukm,
			srcctx->kdf_ukmlen);
...
int ecdh_set_ctx_params(void * vpecdhctx, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		pectx->kdf_ukm = (unsigned char *)tmp_ukm;
...
int ecdh_get_ctx_params(void * vpecdhctx, OSSL_PARAM params[])
{
	...
	if(p != NULL && !OSSL_PARAM_set_utf8_string(p, pectx->kdf_md == NULL ? "" : EVP_MD_get0_name(pectx->kdf_md))) {
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_EXCHANGE_PARAM_KDF_OUTLEN);
	if(p && !OSSL_PARAM_set_size_t(p, pectx->kdf_outlen))
		return 0;
	p = OSSL_PARAM_locate(params, OSSL_EXCHANGE_PARAM_KDF_UKM);
	if(p && !OSSL_PARAM_set_octet_ptr(p, pectx->kdf_ukm, pectx->kdf_ukmlen))
		return 0;
	return 1;
}
...
static const OSSL_PARAM * ecdh_gettable_ctx_params(ossl_unused void * vpecdhctx, ossl_unused void * provctx)
{
	return known_gettable_ctx_params;
}

static ossl_inline size_t ecdh_size(const EC_KEY * k)
{
	size_t degree = 0;
...
static ossl_inline int ecdh_plain_derive(void * vpecdhctx, unsigned char * secret, size_t * psecretlen, size_t outlen)
{
	PROV_ECDH_CTX * pecdhctx = (PROV_ECDH_CTX*)vpecdhctx;
	int retlen, ret = 0;
	...
	if((group = EC_KEY_get0_group(pecdhctx->k)) == NULL || (cofactor = EC_GROUP_get0_cofactor(group)) == NULL)
		return 0;
	/*
	...
	if(pecdhctx->cofactor_mode != -1 && pecdhctx->cofactor_mode != key_cofactor_mode && !BN_is_one(cofactor)) {
...
static ossl_inline int ecdh_X9_63_kdf_derive(void * vpecdhctx, unsigned char * secret, size_t * psecretlen, size_t outlen)
{
	PROV_ECDH_CTX * pecdhctx = (PROV_ECDH_CTX*)vpecdhctx;
    ...
    size_t * psecretlen, size_t outlen)
{
	...
	switch(pecdhctx->kdf_type) {
		case PROV_ECDH_KDF_NONE: return ecdh_plain_derive(vpecdhctx, secret, psecretlen, outlen);
		case PROV_ECDH_KDF_X9_63: return ecdh_X9_63_kdf_derive(vpecdhctx, secret, psecretlen, outlen);
		default: break;
	}
	return 0;
}
...
const OSSL_DISPATCH ossl_ecdh_keyexch_functions[] = {
	...
	{ OSSL_FUNC_KEYEXCH_SETTABLE_CTX_PARAMS, (void (*)(void))ecdh_settable_ctx_params },
	{ OSSL_FUNC_KEYEXCH_GET_CTX_PARAMS, (void (*)(void))ecdh_get_ctx_params },
	{ OSSL_FUNC_KEYEXCH_GETTABLE_CTX_PARAMS, (void (*)(void))ecdh_gettable_ctx_params },
	{ 0, NULL }
};
