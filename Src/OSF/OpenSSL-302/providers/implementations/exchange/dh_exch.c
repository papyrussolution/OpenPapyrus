...
static void * dh_newctx(void * provctx)
{
	...
	if(pdhctx == NULL)
		return NULL;
	pdhctx->libctx = PROV_LIBCTX_OF((PROV_CTX *)provctx);
...
static int dh_init(void * vpdhctx, void * vdh, const OSSL_PARAM params[])
{
	...
	pdhctx->dh = (DH *)vdh;
	pdhctx->kdf_type = PROV_DH_KDF_NONE;
	return dh_set_ctx_params(pdhctx, params) && ossl_dh_check_key(pdhctx->libctx, (const DH *)vdh);
}
...
static int dh_match_params(DH * priv, DH * peer)
{
	...
	ret = dhparams_priv != NULL && dhparams_peer != NULL && ossl_ffc_params_cmp(dhparams_priv, dhparams_peer, 1);
	if(!ret)
...
{
	PROV_DH_CTX * pdhctx = (PROV_DH_CTX*)vpdhctx;
	if(!ossl_prov_is_running() || pdhctx == NULL || vdh == NULL || !dh_match_params((DH *)vdh, pdhctx->dh) || !DH_up_ref((DH *)vdh))
		return 0;
...
static int dh_plain_derive(void * vpdhctx, unsigned char * secret, size_t * secretlen, size_t outlen, unsigned int pad)
{
	PROV_DH_CTX * pdhctx = (PROV_DH_CTX*)vpdhctx;
	int ret;
	size_t dhsize;
	...
	if(p) {
		...
		if(p->data != NULL && p->data_size != 0) {
			if(!OSSL_PARAM_get_octet_string(p, &tmp_ukm, 0, &tmp_ukmlen))
				return 0;
			pdhctx->kdf_ukm = (unsigned char *)tmp_ukm;
...
static int dh_get_ctx_params(void * vpdhctx, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_utf8_string(p, pdhctx->kdf_md == NULL ? "" : EVP_MD_get0_name(pdhctx->kdf_md))) {
		return 0;
	}
	p = OSSL_PARAM_locate(params, OSSL_EXCHANGE_PARAM_KDF_OUTLEN);
	if(p && !OSSL_PARAM_set_size_t(p, pdhctx->kdf_outlen))
		return 0;
	p = OSSL_PARAM_locate(params, OSSL_EXCHANGE_PARAM_KDF_UKM);
	if(p && !OSSL_PARAM_set_octet_ptr(p, pdhctx->kdf_ukm, pdhctx->kdf_ukmlen))
		return 0;
	p = OSSL_PARAM_locate(params, OSSL_KDF_PARAM_CEK_ALG);
	if(p && !OSSL_PARAM_set_utf8_string(p, pdhctx->kdf_cekalg == NULL ? "" :  pdhctx->kdf_cekalg))
		return 0;
	return 1;
}
