...
static void * kdf_newctx(const char * kdfname, void * provctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	kdfctx = (PROV_KDF_CTX *)OPENSSL_zalloc(sizeof(PROV_KDF_CTX));
	if(kdfctx == NULL)
		return NULL;
	kdfctx->provctx = provctx;
	kdf = EVP_KDF_fetch(PROV_LIBCTX_OF((PROV_CTX*)provctx), kdfname, NULL);
	if(kdf == NULL)
		goto err;
...
static int kdf_init(void * vpkdfctx, void * vkdf, const OSSL_PARAM params[])
{
	...
	if(!ossl_prov_is_running() || pkdfctx == NULL || vkdf == NULL || !ossl_kdf_data_up_ref((KDF_DATA *)vkdf))
		return 0;
	pkdfctx->kdfdata = (KDF_DATA *)vkdf;
...
static void * kdf_dupctx(void * vpkdfctx)
{
	...
	if(!ossl_prov_is_running())
		return NULL;
	dstctx = (PROV_KDF_CTX *)OPENSSL_zalloc(sizeof(*srcctx));
	if(dstctx == NULL)
		return NULL;
	*dstctx = *srcctx;
...
static const OSSL_PARAM * kdf_settable_ctx_params(ossl_unused void * vpkdfctx,
    ...
    const char * kdfname)
{
	EVP_KDF * kdf = EVP_KDF_fetch(PROV_LIBCTX_OF((PROV_CTX*)provctx), kdfname,
		NULL);
