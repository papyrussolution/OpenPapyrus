...
static int deflt_get_params(void * provctx, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_utf8_ptr(p, "OpenSSL Default Provider"))
		return 0;
...
static OSSL_ALGORITHM exported_ciphers[SIZEOFARRAY(deflt_ciphers)];
...
{
	BIO_meth_free(ossl_prov_ctx_get0_core_bio_method((PROV_CTX*)provctx));
	ossl_prov_ctx_free((PROV_CTX *)provctx);
}
...
int ossl_default_provider_init(const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * in, const OSSL_DISPATCH ** out, void ** provctx)
{
	...
	ossl_prov_ctx_set0_libctx((PROV_CTX *)*provctx, (OSSL_LIB_CTX*)c_get_libctx(handle));
	ossl_prov_ctx_set0_handle((PROV_CTX *)*provctx, handle);
	ossl_prov_ctx_set0_core_bio_method((PROV_CTX *)*provctx, corebiometh);
