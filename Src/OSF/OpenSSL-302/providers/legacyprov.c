...
static int legacy_get_params(void * provctx, OSSL_PARAM params[])
{
	...
	if(p && !OSSL_PARAM_set_utf8_ptr(p, "OpenSSL Legacy Provider"))
		return 0;
...
static const OSSL_ALGORITHM * legacy_query(void * provctx, int operation_id, int * no_cache)
{
	*no_cache = 0;
	switch(operation_id) {
		case OSSL_OP_DIGEST: return legacy_digests;
		case OSSL_OP_CIPHER: return legacy_ciphers;
		case OSSL_OP_KDF: return legacy_kdfs;
	}
	return NULL;
}
...
{
	OSSL_LIB_CTX_free(PROV_LIBCTX_OF((PROV_CTX*)provctx));
