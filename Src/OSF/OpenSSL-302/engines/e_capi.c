...
static int capi_rsa_sign(int dtype, const uchar * m, unsigned int m_len, uchar * sigret, unsigned int * siglen, const RSA * rsa);
...
#endif
static int capi_load_ssl_client_cert(ENGINE * e, SSL * ssl, STACK_OF(X509_NAME) * ca_dn, X509 ** pcert, EVP_PKEY ** pkey, STACK_OF(X509) ** pother, UI_METHOD * ui_method, void * callback_data);
...
static const ENGINE_CMD_DEFN capi_cmd_defns[] = {
	...
	{CAPI_CMD_LIST_OPTIONS, "list_options", "Set list options (1=summary,2=friendly name, 4=full printout, 8=PEM output, 16=XXX, 32=private key info)", ENGINE_CMD_FLAG_NUMERIC},
...
int capi_rsa_priv_enc(int flen, const uchar * from, uchar * to, RSA * rsa, int padding)
{
	CAPIerr(CAPI_F_CAPI_RSA_PRIV_ENC, CAPI_R_FUNCTION_NOT_SUPPORTED);
	return -1;
}

int capi_rsa_sign(int dtype, const uchar * m, unsigned int m_len,
    uchar * sigret, unsigned int * siglen, const RSA * rsa)
{
	ALG_ID alg;
	HCRYPTHASH hash;
	DWORD slen;
	...
	switch(dtype) {
		...
		case NID_md5: alg = CALG_MD5; break;
		case NID_md5_sha1: alg = CALG_SSL3_SHAMD5; break;
		default:
	    {
		    char algstr[10];
...
int capi_rsa_priv_dec(int flen, const uchar * from, uchar * to, RSA * rsa, int padding)
{
	int i;
	uchar * tmpbuf;
	CAPI_KEY * capi_key;
...
static int capi_rsa_free(RSA * rsa)
{
	CAPI_KEY * capi_key = (CAPI_KEY *)RSA_get_ex_data(rsa, rsa_capi_idx);
...
static int capi_list_containers(CAPI_CTX * ctx, BIO * out)
{
	...
	if(ctx->cspname != NULL) {
		if((clen = MultiByteToWideChar(CP_ACP, 0, ctx->cspname, -1, NULL, 0))) {
			cspname = (LPWSTR)alloca(clen * sizeof(WCHAR));
...
static int capi_ctx_set_provname(CAPI_CTX * ctx, LPSTR pname, DWORD type, int check)
{
	...
	if(check) {
		...
		if((len = MultiByteToWideChar(CP_ACP, 0, pname, -1, NULL, 0))) {
			name = (LPWSTR)alloca(len * sizeof(WCHAR));
...
#include <openssl/engine.h>
	...
	OPENSSL_EXPORT int bind_engine(ENGINE * e, const char * id, const dynamic_fns * fns) { return 0; }
	IMPLEMENT_DYNAMIC_CHECK_FN()
#else
	void engine_load_capi_int(void);
	void engine_load_capi_int(void) {}
