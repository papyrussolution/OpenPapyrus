    ...
    unsigned short old_id)
{
	...
	unsigned int ndx = (type + hash) % SIZEOFARRAY(hash_table);
...
static int handle_symlink(const char * filename, const char * fullpath)
{
	...
	if(filename[i++] != '.')
		return -1;
	for(type = SIZEOFARRAY(suffixes) - 1; type > 0; type--) {
...
static int do_file(const char * filename, const char * fullpath, enum Hash h)
{
	...
	if((ext = strrchr(filename, '.')) == NULL)
		goto end;
	for(i = 0; i < SIZEOFARRAY(extensions); i++) {
		if(strcasecmp(extensions[i], ext + 1) == 0)
			break;
	}
	if(i >= SIZEOFARRAY(extensions))
		goto end;
...
static int do_dir(const char * dirname, enum Hash h)
{
	...
	for(i = 0; i < SIZEOFARRAY(hash_table); i++) {
