...
const OPTIONS dgst_options[] = {
	...
	{"fips-fingerprint", OPT_FIPS_FINGERPRINT, '-', "Compute HMAC with the key used in OpenSSL-FIPS fingerprint"},
...
opthelp:
	...
	if(sigfile != NULL && sigkey != NULL) {
		...
		sigbuf = (unsigned char *)app_malloc(siglen, "signature buffer");
...
static const char * newline_escape_filename(const char * file, int * backslash)
{
	...
	file_cpy = (char *)app_malloc(mem_len, file);
	i = 0;
	while(e < length) {
		const char c = file[e];
		if(c == '\n') {
    ...
    const char * file)
{
	...
	if(key) {
		...
		if(tmplen > BUFSIZE) {
			len = tmplen;
			allocated_buf = (unsigned char *)app_malloc(len, "Signature buffer");
			buf = allocated_buf;
		}
		...
		len = xoflen;
		if(len > BUFSIZE) {
			allocated_buf = (unsigned char *)app_malloc(len, "Digest buffer");
			buf = allocated_buf;
		}
		BIO_get_md_ctx(bp, &ctx);
