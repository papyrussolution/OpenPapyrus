...
opthelp:
			...
			case OPT_CRL_CA_COMPROMISE:
			    rev_arg = opt_arg();
			    rev_type = (REVINFO_TYPE)((o - OPT_CRL_REASON) + REV_CRL_REASON);
			    break;
			case OPT_ENGINE:
...
again2:
	...
	row[DB_exp_date] = (OPENSSL_STRING)app_malloc(tm->length + 1, "row expdate");
	...
	irow = (OPENSSL_STRING *)app_malloc(sizeof(*irow) * (DB_NUMBER + 1), "row space");
	for(i = 0; i < DB_NUMBER; i++)
		irow[i] = row[i];
	irow[DB_NUMBER] = NULL;
    ...
    const char * value)
{
	...
	if(rrow == NULL) {
		...
		row[DB_exp_date] = (char *)app_malloc(tm->length + 1, "row exp_data");
		memcpy(row[DB_exp_date], tm->data, tm->length);
		row[DB_exp_date][tm->length] = '\0';
		row[DB_rev_date] = NULL;
		row[DB_file] = OPENSSL_strdup("unknown");
		...
		irow = (char **)app_malloc(sizeof(*irow) * (DB_NUMBER + 1), "row ptr");
		for(i = 0; i < DB_NUMBER; i++)
			irow[i] = row[i];
		irow[DB_NUMBER] = NULL;
		if(!TXT_DB_insert(db->db, irow)) {
...
static int get_certificate_status(const char * serial, CA_DB * db)
{
	...
	row[DB_serial] = (char *)app_malloc(serial_len + 2, "row serial#");
	if(serial_len % 2) {
		/*
...
#define NUM_REASONS SIZEOFARRAY(crl_reasons)
...
static char * make_revocation_str(REVINFO_TYPE rev_type, const char * rev_arg)
{
	...
	if(other)
		i += strlen(other) + 1;
	str = (char *)app_malloc(i, "revocation reason");
