...
int req_main(int argc, char ** argv)
{
	...
	char * p_template = default_config_file;
	char * keyout = NULL;
...
opthelp:
	...
	if((req_conf = app_load_config_verbose(p_template, verbose)) == NULL)
		goto end;
	if(addext_bio != NULL) {
		if(verbose)
	...
	if(p_template != default_config_file && !app_load_modules(req_conf))
		goto end;
	if(req_conf != NULL) {
...
static int prompt_info(X509_REQ * req, STACK_OF(CONF_VALUE) * dn_sk, const char * dn_sect,
    STACK_OF(CONF_VALUE) * attr_sk, const char * attr_sect, int attribs, unsigned long chtype)
{
	int i;
	char * p, * q;
	char buf[100];
...
{
	char buf[1024];
	int ret = build_data(text, def, value, n_min, n_max, buf, sizeof(buf), "DN value", "DN default");
	if((ret == 0) || (ret == 1))
		return ret;
	ret = 1;
