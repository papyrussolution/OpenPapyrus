...
static int process(const char * uri, const UI_METHOD * uimeth, PW_CB_DATA * uidata, int expected, int criterion, OSSL_STORE_SEARCH * search,
    int text, int noout, int recursive, int indent, BIO * out, const char * prog, OSSL_LIB_CTX * libctx);
...
opthelp:
			...
			case OPT_CRITERION_ISSUER:
			    if(criterion != 0 || (criterion == OSSL_STORE_SEARCH_BY_ISSUER_SERIAL && issuer != NULL)) {
				    BIO_printf(bio_err, "%s: criterion already given.\n", prog);
				    goto end;
			    }
			...
			case OPT_CRITERION_SERIAL:
			    if(criterion != 0 || (criterion == OSSL_STORE_SEARCH_BY_ISSUER_SERIAL && serial != NULL)) {
				    BIO_printf(bio_err, "%s: criterion already given.\n", prog);
				    goto end;
			    }
			    criterion = OSSL_STORE_SEARCH_BY_ISSUER_SERIAL;
    ...
    const char * prog, OSSL_LIB_CTX * libctx)
{
	...
	for(;;) {
		...
		if(!info) {
			...
			if(OSSL_STORE_eof(store_ctx))
				break;
			BIO_printf(bio_err, "ERROR: OSSL_STORE_load() returned NULL without eof or error indications\n");
