...
static int do_raw_keyop(int pkey_op, EVP_MD_CTX * mctx, EVP_PKEY * pkey, BIO * in, int filesize, unsigned char * sig, int siglen, unsigned char ** out, size_t * poutlen);
...
opthelp:
	...
	if(rawin) {
		...
		if(rv > 0 && buf_outlen != 0) {
			buf_out = (unsigned char *)app_malloc(buf_outlen, "buffer output");
    ...
    unsigned char ** out, size_t * poutlen)
{
	...
	if(EVP_PKEY_get_id(pkey) == EVP_PKEY_ED25519 || EVP_PKEY_get_id(pkey) == EVP_PKEY_ED448) {
		...
		mbuf = (unsigned char *)app_malloc(filesize, "oneshot sign/verify buffer");
		switch(pkey_op) {
			case EVP_PKEY_OP_VERIFY:
			...
			case EVP_PKEY_OP_SIGN:
			    ...
			    if(rv == 1 && out != NULL) {
				    *out = (unsigned char *)app_malloc(*poutlen, "buffer output");
