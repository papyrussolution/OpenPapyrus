...
const OPTIONS x509_options[] = {
	...
	{"inform", OPT_INFORM, 'f', "CSR input file format (DER or PEM) - default PEM"},
...
opthelp:
	...
	if(subj && (fsubj = parse_name(subj, chtype, multirdn, "subject")) == NULL)
		goto end;
	if(CAkeyfile == NULL)
		CAkeyfile = CAfile;
	if(CAfile) {
	...
	if(reqfile || newcert) {
		...
		if(req && ext_copy != EXT_COPY_UNSET) {
	...
	if((fsubj || req) && !X509_set_subject_name(x, fsubj ? fsubj : X509_REQ_get_subject_name(req)))
		goto end;
	if((pubkey || privkey || req) && !X509_set_pubkey(x, pubkey ? pubkey : privkey ? privkey : X509_REQ_get0_pubkey(req)))
		goto end;
	if(CAfile) {
	...
	for(i = X509_get_ext_count(x) - 1; i >= 0; i--) {
		...
		if(clrext || (ext_names && strstr(ext_names, sn) == NULL))
	...
	if(sno && !X509_set_serialNumber(x, sno))
		goto end;
	if(reqfile || newcert || privkey || CAfile) {
...
static ASN1_INTEGER * x509_load_serial(const char * CAfile, const char * serialfile, int create)
{
	...
	if(serialfile == NULL) {
		...
		size_t len = p ? (size_t)(p - CAfile) : strlen(CAfile);
		buf = (char *)app_malloc(len + sizeof(POSTFIX), "serial# buffer");
...
static int purpose_print(BIO * bio, X509 * cert, X509_PURPOSE * pt)
{
	...
	const char * pname = X509_PURPOSE_get0_name(pt);
	for(int i = 0; i < 2; i++) {
...
static int print_x509v3_exts(BIO * bio, X509 * x, const char * ext_names)
{
	...
	if((names = (const char **)OPENSSL_malloc(sizeof(char *) * nn)) == NULL)
		goto end;
