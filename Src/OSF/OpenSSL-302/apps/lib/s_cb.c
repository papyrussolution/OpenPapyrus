...
static uchar cookie_secret[COOKIE_SECRET_LENGTH];
...
int verify_callback(int ok, X509_STORE_CTX * ctx)
{
	...
	if(!verify_args.quiet || !ok) {
		...
		if(err_cert) {
			X509_NAME_print_ex(bio_err, X509_get_subject_name(err_cert), 0, get_nameopt());
			BIO_puts(bio_err, "\n");
		}
		else {
...
static const char * get_sigtype(int nid)
{
	switch(nid) {
		...
		case NID_ED25519: return "Ed25519";
		case NID_ED448: return "Ed448";
		case NID_id_GostR3410_2001: return "gost2001";
		case NID_id_GostR3410_2012_256: return "gost2012_256";
		case NID_id_GostR3410_2012_512: return "gost2012_512";
		default: return NULL;
	}
}
...
{
	int i, nsig;
	int client = SSL_is_server(s) ? 0 : 1;
	if(shared)
...
{
	int i, * groups, nid;
	int ngroups = SSL_get1_groups(s, NULL);
	if(ngroups <= 0)
		return 1;
...
long bio_dump_callback(BIO * bio, int cmd, const char * argp, size_t len, int argi, long argl, int ret, size_t * processed)
{
...
void msg_cb(int write_p, int version, int content_type, const void * buf, size_t len, SSL * ssl, void * arg)
{
	...
	const uchar* bp = (const uchar*)buf;
	if(oneof7(version, SSL3_VERSION, TLS1_VERSION, TLS1_1_VERSION, TLS1_2_VERSION, TLS1_3_VERSION, DTLS1_VERSION, DTLS1_BAD_VER)) {
	...
	if(len > 0) {
		...
		for(i = 0; i < num; i++) {
			if(i % 16 == 0 && i > 0)
				BIO_printf(bio, "\n   ");
			BIO_printf(bio, " %02x", ((const uchar*)buf)[i]);
		}
		if(i < len)
			BIO_printf(bio, " ...");
		BIO_printf(bio, "\n");
...
void tlsext_cb(SSL * s, int client_server, int type, const uchar * data, int len, void * arg)
{
	BIO * bio = (BIO *)arg;
...
#ifndef OPENSSL_NO_SOCK
int generate_stateless_cookie_callback(SSL * ssl, uchar * cookie, size_t * cookie_len)
{
	uchar * buffer = NULL;
	size_t length = 0;
	unsigned short port;
	...
	length += sizeof(port);
	buffer = (uchar *)app_malloc(length, "cookie generate buffer");
	memcpy(buffer, &port, sizeof(port));
	BIO_ADDR_rawaddress(peer, buffer + sizeof(port), NULL);
	if(EVP_Q_mac(NULL, "HMAC", NULL, "SHA1", NULL, cookie_secret, COOKIE_SECRET_LENGTH, buffer, length, cookie, DTLS1_COOKIE_LENGTH, cookie_len) == NULL) {
		BIO_printf(bio_err, "Error calculating HMAC-SHA1 of buffer with secret\n");
		goto end;
	}
	res = 1;
end:
...
int verify_stateless_cookie_callback(SSL * ssl, const uchar * cookie, size_t cookie_len)
{
	uchar result[EVP_MAX_MD_SIZE];
	size_t resultlength;
	/* Note: we check cookie_initialized because if it's not,
	 * it cannot be valid */
	if(cookie_initialized && generate_stateless_cookie_callback(ssl, result, &resultlength) && 
		cookie_len == resultlength && memcmp(result, cookie, resultlength) == 0)
		return 1;
	return 0;
}

int generate_cookie_callback(SSL * ssl, uchar * cookie, unsigned int * cookie_len)
{
	size_t temp = 0;
...
int verify_cookie_callback(SSL * ssl, const uchar * cookie, unsigned int cookie_len)
{
...
{
	SSL_EXCERT * exc = (SSL_EXCERT *)app_malloc(sizeof(*exc), "prepend cert");
	memzero(exc, sizeof(*exc));
	exc->next = *pexc;
	*pexc = exc;
	if(exc->next) {
...
int load_excert(SSL_EXCERT ** pexc)
{
	...
	for(; exc; exc = exc->next) {
		...
		exc->cert = load_cert(exc->certfile, exc->certform, "Server Certificate");
		if(exc->cert == NULL)
			return 0;
		if(exc->keyfile) {
			exc->key = load_key(exc->keyfile, exc->keyform, 0, NULL, NULL, "server key");
		}
		else {
			exc->key = load_key(exc->certfile, exc->certform, 0, NULL, NULL, "server key");
		}
		if(exc->key == NULL)
			return 0;
		if(exc->chainfile) {
...
{
	const uchar * rlist;
	static const uchar scsv_id[] = { 0, 0xFF };
 ...
 */
static char * hexencode(const uchar * data, size_t len)
{
...
#define TLSA_TAIL_SIZE 12
		...
		BIO_printf(bio, "DANE TLSA %d %d %d %s%s %s at depth %d\n",
		    usage, selector, mtype, (dlen > TLSA_TAIL_SIZE) ? "..." : "", hexdata, (mspki) ? "signed the certificate" : mdpth ? "matched TA certificate" : "matched EE certificate", mdpth);
...
void print_ssl_summary(SSL * s)
{
	...
	if(peer) {
		...
		X509_NAME_print_ex(bio_err, X509_get_subject_name(peer), 0, get_nameopt());
		BIO_puts(bio_err, "\n");
    ...
    STACK_OF(X509_CRL) * crls, int crl_download)
{
	...
	int rv = 0;
	if(vfyCApath || vfyCAfile || vfyCAstore) {
		vfy = X509_STORE_new();
		if(vfy == NULL)
			goto err;
		if(vfyCAfile && !X509_STORE_load_file(vfy, vfyCAfile))
			goto err;
		if(vfyCApath && !X509_STORE_load_path(vfy, vfyCApath))
			goto err;
		if(vfyCAstore && !X509_STORE_load_store(vfy, vfyCAstore))
			goto err;
	...
	if(chCApath || chCAfile || chCAstore) {
		ch = X509_STORE_new();
		if(ch == NULL)
			goto err;
		if(chCAfile && !X509_STORE_load_file(ch, chCAfile))
			goto err;
		if(chCApath && !X509_STORE_load_path(ch, chCApath))
			goto err;
		if(chCAstore && !X509_STORE_load_store(ch, chCAstore))
			goto err;
...
static int security_callback_debug(const SSL * s, const SSL_CTX * ctx, int op, int bits, int nid, void * other, void * ex)
{
	security_debug_ex * sdb = (security_debug_ex *)ex;
	int show_bits = 1, cert_md = 0;
	const char * nm;
	int show_nm;
	int rv = sdb->old_cb(s, ctx, op, bits, nid, other, ex);
	...
	switch(op & SSL_SECOP_OTHER_TYPE) {
		case SSL_SECOP_OTHER_CIPHER:
		    BIO_puts(sdb->out, SSL_CIPHER_get_name((const SSL_CIPHER *)other));
		    break;
	    ...
	    {
		    if(cert_md) {
			    int sig_nid = X509_get_signature_nid((const X509 *)other);
			    BIO_puts(sdb->out, OBJ_nid2sn(sig_nid));
		    }
		    else {
			    EVP_PKEY * pkey = X509_get0_pubkey((const X509 *)other);
			    if(!pkey) {
	    ...
	    {
		    const uchar * salg = (const uchar *)other;
