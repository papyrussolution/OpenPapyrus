...
int chopup_args(ARGS * arg, char * buf)
{
	...
	if(arg->size == 0) {
		arg->size = 20;
		arg->argv = (char **)app_malloc(sizeof(*arg->argv) * arg->size, "argv space");
	}
	for(p = buf;;) {
		/* Skip whitespace. */
		...
		if(arg->argc >= arg->size) {
			char ** tmp;
			arg->size += 20;
			tmp = (char **)OPENSSL_realloc(arg->argv, sizeof(*arg->argv) * arg->size);
			if(tmp == NULL)
				return 0;
			arg->argv = tmp;
		}
...
int ctx_set_verify_locations(SSL_CTX * ctx, const char * CAfile, int noCAfile, const char * CApath, int noCApath, const char * CAstore, int noCAstore)
{
	...
	if(CApath && !SSL_CTX_load_verify_dir(ctx, CApath))
		return 0;
	if(CAstore && !SSL_CTX_load_verify_store(ctx, CAstore))
		return 0;
	return 1;
}
...
char * get_passwd(const char * pass, const char * desc)
{
	...
	if(pass && result == NULL) {
		BIO_printf(bio_err, "Trying plain input string (better precede with 'pass:')\n");
...
{
	int same = (arg1 && arg2 && strcmp(arg1, arg2) == 0);
	if(arg1) {
	...
	if(!keepbio || pwdbio == NULL) {
		if(strncmp(arg, "file:", 5) == 0) {
			...
			if(tmp == NULL || tmp - arg > PASS_SOURCE_SIZE_MAX)
				...
				BIO_printf(bio_err, "Invalid password argument, starting with \"%.*s\"\n", (int)(tmp - arg + 1), arg);
			return NULL;
		}
	}
...
{
	long errorline = -1;
	CONF * conf = NCONF_new_ex(app_get0_libctx(), NULL);
	...
	if(errorline <= 0) {
		...
		BIO_printf(bio_err, "%s: Error on line %ld of ", opt_getprog(), errorline);
	}
	if(filename)
...
#define IS_HTTP(uri) ((uri) != NULL && strncmp(uri, OSSL_HTTP_PREFIX, strlen(OSSL_HTTP_PREFIX)) == 0)
#define IS_HTTPS(uri) ((uri) != NULL && strncmp(uri, OSSL_HTTPS_PREFIX, strlen(OSSL_HTTPS_PREFIX)) == 0)

X509 * load_cert_pass(const char * uri, int format, int maybe_stdin, const char * pass, const char * desc)
{
	X509 * cert = NULL;
	if(!desc)
		desc = "certificate";
    ...
    const char * pass, ENGINE * e, const char * desc)
{
	...
	(void)load_key_certs_crls(uri, format, may_stdin, pass, desc, &pkey, NULL, NULL, NULL, NULL, NULL, NULL);
	OPENSSL_free(allocated_uri);
	return pkey;
}

EVP_PKEY * load_pubkey(const char * uri, int format, int maybe_stdin, const char * pass, ENGINE * e, const char * desc)
{
	EVP_PKEY * pkey = NULL;
	char * allocated_uri = NULL;
	if(!desc)
	...
	(void)load_key_certs_crls(uri, format, maybe_stdin, pass, desc, NULL, &pkey, NULL, NULL, NULL, NULL, NULL);
	OPENSSL_free(allocated_uri);
	return pkey;
}

EVP_PKEY * load_keyparams_suppress(const char * uri, int format, int maybe_stdin, const char * keytype, const char * desc, int suppress_decode_errors)
{
	EVP_PKEY * params = NULL;
	if(!desc)
		desc = "key parameters";
	(void)load_key_certs_crls_suppress(uri, format, maybe_stdin, NULL, desc, NULL, NULL, &params, NULL, NULL, NULL, NULL, suppress_decode_errors);
	if(params && keytype && !EVP_PKEY_is_a(params, keytype)) {
		if(!suppress_decode_errors) {
			BIO_printf(bio_err, "Unable to load %s from %s (unexpected parameters type)\n", desc, uri);
			ERR_print_errors(bio_err);
		}
...
EVP_PKEY * load_keyparams(const char * uri, int format, int maybe_stdin, const char * keytype, const char * desc)
{
...
static void warn_cert(const char * uri, X509 * cert, int warn_EE, X509_VERIFY_PARAM * vpm)
{
	uint32_t ex_flags = X509_get_extension_flags(cert);
	int res = X509_cmp_timeframe(vpm, X509_get0_notBefore(cert), X509_get0_notAfter(cert));
	if(res != 0)
...
static void warn_certs(const char * uri, STACK_OF(X509) * certs, int warn_EE, X509_VERIFY_PARAM * vpm)
{
	for(int i = 0; i < sk_X509_num(certs); i++)
		warn_cert(uri, sk_X509_value(certs, i), warn_EE, vpm);
}

int load_cert_certs(const char * uri, X509 ** pcert, STACK_OF(X509) ** pcerts, int exclude_http, const char * pass, const char * desc, X509_VERIFY_PARAM * vpm)
{
	int ret = 0;
	char * pass_string;
	if(exclude_http && (strncasecmp(uri, "http://", 7) == 0 || strncasecmp(uri, "https://", 8) == 0)) {
		BIO_printf(bio_err, "error: HTTP retrieval not allowed for %s\n", desc);
		return ret;
	}
	pass_string = get_passwd(pass, desc);
	ret = load_key_certs_crls(uri, FORMAT_UNDEF, 0, pass_string, desc, NULL, NULL, NULL, pcert, pcerts, NULL, NULL);
...
STACK_OF(X509) *load_certs_multifile(char * files, const char * pass, const char * desc, X509_VERIFY_PARAM *vpm)
{
	...
	while(files) {
		...
		if(!load_cert_certs(files, NULL, &certs, 0, pass, desc, vpm))
			goto err;
		if(!X509_add_certs(result, certs, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
			goto oom;
...
static X509_STORE * sk_X509_to_store(X509_STORE * store /* may be NULL */, const STACK_OF(X509) * certs /* may NULL */)
{
	int i;
	if(!store)
 ...
 */
X509_STORE * load_certstore(char * input, const char * pass, const char * desc, X509_VERIFY_PARAM * vpm)
{
 ...
 */
int load_certs(const char * uri, int maybe_stdin, STACK_OF(X509) ** certs, const char * pass, const char * desc)
{
	int was_NULL = *certs == NULL;
	int ret = load_key_certs_crls(uri, FORMAT_UNDEF, maybe_stdin, pass, desc, NULL, NULL, NULL, NULL, certs, NULL, NULL);
	if(!ret && was_NULL) {
		sk_X509_pop_free(*certs, X509_free);
		*certs = NULL;
	}
 ...
 */
int load_crls(const char * uri, STACK_OF(X509_CRL) ** crls, const char * pass, const char * desc)
{
	int was_NULL = *crls == NULL;
	int ret = load_key_certs_crls(uri, FORMAT_UNDEF, 0, pass, desc, NULL, NULL, NULL, NULL, NULL, NULL, crls);
	if(!ret && was_NULL) {
...
{
	switch(format) {
		case FORMAT_PEM: return "PEM";
		case FORMAT_ASN1: return "DER";
	}
	return NULL;
}
 ...
 */
static int load_key_certs_crls_suppress(const char * uri, int format, int maybe_stdin, const char * pass, const char * desc,
    EVP_PKEY ** ppkey, EVP_PKEY ** ppubkey, EVP_PKEY ** pparams, X509 ** pcert, STACK_OF(X509) ** pcerts, X509_CRL ** pcrl, STACK_OF(X509_CRL) ** pcrls, int suppress_decode_errors)
{
	PW_CB_DATA uidata;
	...
	int ncerts = 0;
	int ncrls = 0;
	const char * failed = ppkey ? "key" : (ppubkey ? "public key" : (pparams ? "params" : (pcert ? "cert" : (pcrl ? "CRL" : (pcerts ? "certs" : (pcrls ? "CRLs" : NULL))))));
	...
	if((input_type = format2string(format)) != NULL) {
		itp[0] = OSSL_PARAM_construct_utf8_string(OSSL_STORE_PARAM_INPUT_TYPE, (char*)input_type, 0);
	...
	if(!uri) {
		...
		if(bio) {
			ctx = OSSL_STORE_attach(bio, "file", libctx, propq, get_ui_method(), &uidata, params, NULL, NULL);
			BIO_free(bio);
		}
	}
	else {
		ctx = OSSL_STORE_open_ex(uri, libctx, propq, get_ui_method(), &uidata, params, NULL, NULL);
	}
	if(!ctx) {
	...
	if(failed == NULL) {
		int any = 0;
		if((ppkey && *ppkey == NULL) || (ppubkey && *ppubkey == NULL)) {
			failed = "key";
		}
		else if(pparams && *pparams == NULL) {
			failed = "params";
		}
		else if((pcert || pcerts) && ncerts == 0) {
			if(pcert == NULL)
				any = 1;
			failed = "cert";
		}
		else if((pcrl || pcrls) && ncrls == 0) {
			if(pcrl == NULL)
				any = 1;
			failed = "CRL";
		}
	...
	if(!suppress_decode_errors && failed) {
		if(desc && strstr(desc, failed)) {
...
X509_STORE * setup_verify(const char * CAfile, int noCAfile, const char * CApath, int noCApath, const char * CAstore, int noCAstore)
{
...
{
	char buf[2][BSIZE];
	int i = strlen(serialfile) + strlen(old_suffix);
	int j = strlen(serialfile) + strlen(new_suffix);
	if(i > j)
		j = i;
	if(j + 1 >= BSIZE) {
		BIO_printf(bio_err, "File name too long\n");
...
CA_DB * load_index(const char * dbfile, DB_ATTR * db_attr)
{
	...
	retdb = (CA_DB *)app_malloc(sizeof(*retdb), "new DB");
	retdb->db = tmpdb;
	tmpdb = NULL;
	if(db_attr)
...
X509_NAME * parse_name(const char * cp, int chtype, int canmulti, const char * desc)
{
	...
	X509_NAME * n;
	if(*cp++ != '/') {
		BIO_printf(bio_err, "%s: %s name is expected to be in the format /type0=value0/type1=value1/type2=... where characters may be escaped by \\. This name is not in that format: '%s'\n",
		    opt_getprog(), desc, --cp);
		return NULL;
	}
	n = X509_NAME_new();
	...
	while(*cp) {
		...
		*bp++ = '\0';
		if(*cp == '\0') {
			BIO_printf(bio_err, "%s: Missing '=' after RDN type string '%s' in %s name string\n", opt_getprog(), typestr, desc);
			goto err;
		}
		++cp;
		/* Collect the value. */
		...
		for(; *cp != '\0' && *cp != '/'; *bp++ = *cp++) {
			...
			if(*cp == '\\' && *++cp == '\0') {
				BIO_printf(bio_err, "%s: Escape character at end of %s name string\n", opt_getprog(), desc);
				goto err;
			}
		}
		*bp++ = '\0';
		...
		if(nid == NID_undef) {
			BIO_printf(bio_err, "%s: Skipping unknown %s name attribute \"%s\"\n", opt_getprog(), desc, typestr);
			if(ismulti)
				BIO_printf(bio_err, "Hint: a '+' in a value string needs be escaped using '\\' else a new member of a multi-valued RDN is expected\n");
			continue;
		}
		if(*valstr == '\0') {
			BIO_printf(bio_err, "%s: No value provided for %s name attribute \"%s\", skipped\n", opt_getprog(), desc, typestr);
			continue;
		}
		if(!X509_NAME_add_entry_by_NID(n, nid, chtype,
		    ...
		    -1, ismulti ? -1 : 0)) {
			ERR_print_errors(bio_err);
			BIO_printf(bio_err, "%s: Error adding %s name attribute \"/%s=%s\"\n", opt_getprog(), desc, typestr, valstr);
			goto err;
		}
	}
	OPENSSL_free(work);
...
unsigned char * next_protos_parse(size_t * outlen, const char * in)
{
	...
	if(len == 0 || len >= 65535)
		return NULL;
	out = (unsigned char *)app_malloc(len + 1, "NPN buffer");
	for(i = 0; i <= len; ++i) {
		if(i == len || in[i] == ',') {
			/*
...
void print_cert_checks(BIO * bio, X509 * x, const char * checkhost, const char * checkemail, const char * checkip)
{
	if(x == NULL)
		return;
	if(checkhost) {
		BIO_printf(bio, "Hostname %s does%s match certificate\n", checkhost, X509_check_host(x, checkhost, 0, 0, NULL) == 1 ? "" : " NOT");
	}
	if(checkemail) {
...
static int do_pkey_ctx_init(EVP_PKEY_CTX * pkctx, STACK_OF(OPENSSL_STRING) * opts)
{
	...
	for(int i = 0; i < sk_OPENSSL_STRING_num(opts); i++) {
		const char * opt = sk_OPENSSL_STRING_value(opts, i);
...
static int do_sign_init(EVP_MD_CTX * ctx, EVP_PKEY * pkey, const char * md, STACK_OF(OPENSSL_STRING) * sigopts)
{
	 ...
	 * for this algorithm.
	 */
	if(EVP_PKEY_get_default_digest_name(pkey, def_md, sizeof(def_md)) == 2 && strcmp(def_md, "UNDEF") == 0) {
		/* The signing algorithm requires there to be no digest */
		md = NULL;
	}
	return EVP_DigestSignInit_ex(ctx, &pkctx, md, app_get0_libctx(), app_get0_propq(), pkey, NULL) && do_pkey_ctx_init(pkctx, sigopts);
}

static int adapt_keyid_ext(X509 * cert, X509V3_CTX * ext_ctx, const char * name, const char * value, int add_default)
{
...
int do_X509_sign(X509 * cert, EVP_PKEY * pkey, const char * md, STACK_OF(OPENSSL_STRING) * sigopts, X509V3_CTX * ext_ctx)
{
	const STACK_OF(X509_EXTENSION) *exts = X509_get0_extensions(cert);
	...
	if(sk_X509_EXTENSION_num(exts /* may be NULL */) > 0) {
		...
		ERR_pop_to_mark();
		if(!adapt_keyid_ext(cert, ext_ctx, "authorityKeyIdentifier", "keyid, issuer", !self_sign))
			goto end;
	}
	if(mctx && do_sign_init(mctx, pkey, md, sigopts) > 0)
...
BIO * app_http_tls_cb(BIO * bio, void * arg, int connect, int detail)
{
	...
	if(connect && detail) { /* connecting with TLS */
		...
		if((info->use_proxy && !OSSL_HTTP_proxy_connect(bio, info->server, info->port, NULL, NULL,/* no proxy credentials */ info->timeout, bio_err, opt_getprog()))
    ...
    const ASN1_ITEM * it)
{
	...
	if(!OSSL_HTTP_parse_url(url, &use_ssl, NULL /* userinfo */, &server, &port, NULL /* port_num, */, NULL, NULL, NULL))
		return NULL;
	...
	resp = (ASN1_VALUE *)ASN1_item_d2i_bio(it, mem, NULL);
	BIO_free(mem);
end:
	OPENSSL_free(server);
	OPENSSL_free(port);
    ...
    long timeout, const ASN1_ITEM * rsp_it)
{
	...
	rsp = OSSL_HTTP_transfer(NULL, host, port, path, use_ssl,
		...
		0 /* keep_alive */);
	BIO_free(req_mem);
	res = (ASN1_VALUE *)ASN1_item_d2i_bio(rsp_it, rsp, NULL);
	BIO_free(rsp);
	return res;
}
...
{
	BIO * b = BIO_new_fp(stdout, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
	void * prefix = NULL;
...
{
	BIO * b = BIO_new_fp(stderr, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
#ifdef OPENSSL_SYS_VMS
	if(FMT_istext(format))
		b = BIO_push(BIO_new(BIO_f_linebuffer()), b);
#endif
...
static const char * modestr(char mode, int format)
{
	...
	switch(mode) {
		case 'a': return FMT_istext(format) ? "a" : "ab";
		case 'r': return FMT_istext(format) ? "r" : "rb";
		case 'w': return FMT_istext(format) ? "w" : "wb";
	}
	return NULL; /* The assert above should make sure we never reach this point */
}
...
BIO * bio_open_owner(const char * filename, int format, int _private)
{
	...
	int fd = -1, mode;
#endif
	if(!_private || filename == NULL || strcmp(filename, "-") == 0)
...
OSSL_PARAM * app_params_new_from_opts(STACK_OF(OPENSSL_STRING) * opts, const OSSL_PARAM * paramdefs)
{
	...
	if(opts == NULL)
		return NULL;
	params = (OSSL_PARAM *)OPENSSL_zalloc(sizeof(OSSL_PARAM) * (sz + 1));
	if(!params)
		return NULL;

	for(params_n = 0; params_n < sz; params_n++) {
		opt = sk_OPENSSL_STRING_value(opts, (int)params_n);
		if((stmp = OPENSSL_strdup(opt)) == NULL || (vtmp = strchr(stmp, ':')) == NULL)
			goto err;
		/* Replace ':' with 0 to terminate the string pointed to by stmp */
		*vtmp = 0;
		/* Skip over the separator so that vmtp points to the value */
		vtmp++;
		if(!OSSL_PARAM_allocate_from_text(&params[params_n], paramdefs, stmp, vtmp, strlen(vtmp), &found))
			goto err;
		OPENSSL_free(stmp);
	}
