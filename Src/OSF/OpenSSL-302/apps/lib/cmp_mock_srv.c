...
{
	mock_srv_ctx * ctx = (mock_srv_ctx*)OPENSSL_zalloc(sizeof(mock_srv_ctx));
	if(!ctx)
		goto err;
...
{
	mock_srv_ctx * ctx = (mock_srv_ctx *)OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
	if(!ctx) {
...
int ossl_cmp_mock_srv_set1_chainOut(OSSL_CMP_SRV_CTX * srv_ctx, STACK_OF(X509) * chain)
{
	...
	if(chain && (chain_copy = X509_chain_up_ref(chain)) == NULL)
		return 0;
	sk_X509_pop_free(ctx->chainOut, X509_free);
...
int ossl_cmp_mock_srv_set1_caPubsOut(OSSL_CMP_SRV_CTX * srv_ctx, STACK_OF(X509) * caPubs)
{
	...
	if(caPubs && (caPubs_copy = X509_chain_up_ref(caPubs)) == NULL)
		return 0;
	sk_X509_pop_free(ctx->caPubsOut, X509_free);
    ...
    STACK_OF(X509) ** caPubs)
{
	...
	if(!ctx || cert_req == NULL || certOut == NULL || chainOut == NULL || caPubs == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return NULL;
	}
	if(ctx->sendError) {
	...
	*caPubs = NULL;
	...
	if(OSSL_CMP_MSG_get_bodytype(cert_req) == OSSL_CMP_KUR && crm && ctx->certOut) {
		...
		if(issuer && X509_NAME_cmp(issuer, OSSL_CRMF_CERTID_get0_issuer(cid)) != 0) {
			ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_CERTID);
			return NULL;
		}
		if(serial && ASN1_INTEGER_cmp(serial, OSSL_CRMF_CERTID_get0_serialNumber(cid)) != 0) {
			ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_CERTID);
			return NULL;
		}
	}
	if(ctx->certOut && (*certOut = X509_dup(ctx->certOut)) == NULL)
		goto err;
	if(ctx->chainOut && (*chainOut = X509_chain_up_ref(ctx->chainOut)) == NULL)
		goto err;
	if(ctx->caPubsOut && (*caPubs = X509_chain_up_ref(ctx->caPubsOut)) == NULL)
		goto err;
	if(ctx->statusOut && (si = OSSL_CMP_PKISI_dup(ctx->statusOut)) == NULL)
		goto err;
	return si;
err:
	X509_free(*certOut);
...
{
	mock_srv_ctx * ctx = (mock_srv_ctx *)OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
	if(!ctx || genm == NULL || in == NULL || out == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return 0;
	}
	if(ctx->sendError) {
...
static void process_error(OSSL_CMP_SRV_CTX * srv_ctx, const OSSL_CMP_MSG * error,
    const OSSL_CMP_PKISI * statusInfo, const ASN1_INTEGER * errorCode, const OSSL_CMP_PKIFREETEXT * errorDetails)
{
	mock_srv_ctx * ctx = (mock_srv_ctx *)OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    ...
    const ASN1_OCTET_STRING * certHash, const OSSL_CMP_PKISI * si)
{
	...
	if(!ctx || certConf == NULL || certHash == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return 0;
	}
	if(ctx->sendError || ctx->certOut == NULL) {
...
{
	mock_srv_ctx * ctx = (mock_srv_ctx *)OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
	if(!ctx || pollReq == NULL || certReq == NULL || check_after == NULL) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return 0;
	}
	if(ctx->sendError) {
...
OSSL_CMP_SRV_CTX * ossl_cmp_mock_srv_new(OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	if(srv_ctx && ctx && OSSL_CMP_SRV_CTX_init(srv_ctx, ctx, process_cert_request, process_rr, process_genm, process_error, process_certConf, process_pollReq))
		return srv_ctx;
	mock_srv_ctx_free(ctx);
...
{
	if(srv_ctx)
		mock_srv_ctx_free((mock_srv_ctx *)OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx));
	OSSL_CMP_SRV_CTX_free(srv_ctx);
}
