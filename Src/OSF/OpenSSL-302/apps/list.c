...
{
	if(select_name != NULL && (c == NULL || strcasecmp(select_name,  EVP_CIPHER_get0_name(c)) != 0))
		return;
	if(c) {
...
DEFINE_STACK_OF(EVP_MD) static int md_cmp(const EVP_MD * const * a, const EVP_MD * const * b)
{
...
{
	STACK_OF(EVP_MAC) *mac_stack = (STACK_OF(EVP_MAC) *)stack;
...
{
	STACK_OF(EVP_KDF) *kdf_stack = (STACK_OF(EVP_KDF) *)stack;
...
{
	STACK_OF(EVP_RAND) *rand_stack = (STACK_OF(EVP_RAND) *)stack;
...
static void list_random_generators(void)
{
	...
	for(i = 0; i < sk_EVP_RAND_num(rands); i++) {
		...
		if(select_name != NULL && strcasecmp(EVP_RAND_get0_name(m), select_name) != 0)
			continue;
...
static void display_random(const char * name, EVP_RAND_CTX * drbg)
{
	...
	if(drbg != NULL) {
		...
		switch(EVP_RAND_get_state(drbg)) {
			case EVP_RAND_STATE_UNINITIALISED: p = "uninitialised"; break;
			case EVP_RAND_STATE_READY: p = "ready"; break;
			case EVP_RAND_STATE_ERROR: p = "error"; break;
...
{
	STACK_OF(OSSL_ENCODER) *encoder_stack = (STACK_OF(OSSL_ENCODER) *)stack;
...
{
	STACK_OF(OSSL_DECODER) *decoder_stack = (STACK_OF(OSSL_DECODER) *)stack;
...
DEFINE_STACK_OF(EVP_SIGNATURE) static int signature_cmp(const EVP_SIGNATURE * const * a, const EVP_SIGNATURE * const * b)
{
...
{
	STACK_OF(EVP_SIGNATURE) *sig_stack = (STACK_OF(EVP_SIGNATURE) *)stack;
...
{
	STACK_OF(EVP_ASYM_CIPHER) *asym_cipher_stack = (STACK_OF(EVP_ASYM_CIPHER) *)stack;
...
{
	STACK_OF(EVP_KEYEXCH) *kex_stack = (STACK_OF(EVP_KEYEXCH) *)stack;
...
DEFINE_STACK_OF(OSSL_STORE_LOADER) static int store_cmp(const OSSL_STORE_LOADER * const * a, const OSSL_STORE_LOADER * const * b)
{
...
{
	STACK_OF(OSSL_STORE_LOADER) *store_stack = (STACK_OF(OSSL_STORE_LOADER) *)stack;
...
{
	STACK_OF(OSSL_PROVIDER) *provider_stack = (STACK_OF(OSSL_PROVIDER) *)stack;
...
int list_main(int argc, char ** argv)
{
	...
	while((o = (HELPLIST_CHOICE)opt_next()) != OPT_EOF) {
		switch(o) {
