...
#pragma hdrstop
//#if defined(_WIN32)
//#include <windows.h> /* Included before async.h to avoid some warnings */
//#endif
...
opthelp:
	...
	if(!SSL_CTX_set_session_id_context(ctx, (const unsigned char *)&s_server_session_id_context,
	...
	if(ctx2 != NULL) {
		...
		if(!SSL_CTX_set_session_id_context(ctx2, (const unsigned char *)&s_server_session_id_context, sizeof(s_server_session_id_context))) {
			BIO_printf(bio_err, "error setting session id context\n");
			ERR_print_errors(bio_err);
			goto end;
		}
...
static int sv_body(int s, int stype, int prot, unsigned char * context)
{
	...
	for(;;) {
		...
		if(!read_from_sslcon) {
			...
			timeout.tv_sec = 1;
			timeout.tv_usec = 0;
			i = select(width, (fd_set *)&readfds, NULL, NULL, &timeout);
...
static int www_body(int s, int stype, int prot, unsigned char * context)
{
	...
	buf = (char *)app_malloc(bufsize + 1, "server www buffer");
	...
	for(;;) {
		...
		if(((www == 1) && (strncmp("GET ", buf, 4) == 0)) ||
			...
			if(www == 1 && strncmp("GET /reneg", buf, 10) == 0) {
				...
				FD_ZERO(&readfds);
				openssl_fdset(s, &readfds);
				i = select(width, (fd_set *)&readfds, NULL, NULL, NULL);
...
static int rev_body(int s, int stype, int prot, unsigned char * context)
{
	...
	/* as we use BIO_gets(), and it always null terminates data, we need
	 * to allocate 1 byte longer buffer to fit the full 2^14 byte record */
	buf = (char *)app_malloc(bufsize + 1, "server rev buffer");
	io = BIO_new(BIO_f_buffer());
	ssl_bio = BIO_new(BIO_f_ssl());
	if((io == NULL) || (ssl_bio == NULL))
		goto err;
