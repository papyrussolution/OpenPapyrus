...
static void warn_deprecated(const FUNCTION * fp)
{
	if(fp->deprecated_version != NULL)
		BIO_printf(bio_err, "The command %s was deprecated in version %s.", fp->name, fp->deprecated_version);
	else
...
static void setup_trace_category(int category)
{
	...
	if(trace_data == NULL || bio == NULL || (trace_data->bio = channel) == NULL || OSSL_trace_set_callback(category, internal_trace_cb, trace_data) == 0 || 
		sk_tracedata_push(trace_data_stack, trace_data) == 0) {
		slfprintf_stderr("warning: unable to setup trace callback for category '%s'.\n", OSSL_trace_get_category_name(category));
	...
	if(val != NULL) {
		...
		for(valp = val; (item = strtok(valp, ",")) != NULL; valp = NULL) {
			...
			if(category == OSSL_TRACE_CATEGORY_ALL) {
				...
				slfprintf_stderr("warning: unknown trace category: '%s'.\n", item);
		}
	}
	OPENSSL_free(val);
}
...
int main(int argc, char * argv[])
{
...
#endif
	if((fname = "apps_startup", !apps_startup()) || (fname = "prog_init", (prog = prog_init()) == NULL)) {
	...
	if(fp == NULL) {
		...
		global_help = argc > 1 && (strcmp(argv[1], "-help") == 0 || strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--h") == 0);
		argc--;
		argv++;
	...
	ret = argc == 0 || global_help ? do_cmd(prog, 1, help_argv) : do_cmd(prog, argc, argv);
end:
...
int help_main(int argc, char ** argv)
{
	...
	while((o = (HELP_CHOICE)opt_next()) != OPT_hEOF) {
		switch(o) {
			case OPT_hERR:
			case OPT_hEOF:
