...
#define SIZE_NUM         SIZEOFARRAY(lengths_list)
...
#define opt_found(value, pairs, result) \
	opt_found(value, result, pairs, SIZEOFARRAY(pairs))
...
static int have_md(const char * name)
{
	...
	if(opt_md_silent(name, &md)) {
		...
		if(ctx && EVP_DigestInit(ctx, md) > 0)
			ret = 1;
		EVP_MD_CTX_free(ctx);
		EVP_MD_free(md);
	}
...
static int have_cipher(const char * name)
{
	...
	if(opt_cipher_silent(name, &cipher)) {
		...
		if(ctx && EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, 1) > 0)
			ret = 1;
...
static int _Speed_Decrypt = 0; // @sobolev decrypt-->_Speed_Decrypt
...
#define stop_it(do_it, test_num) memzero(do_it + test_num, SIZEOFARRAY(do_it) - test_num);
...
opterr:
	...
	loopargs = (loopargs_t *)app_malloc(loopargs_len * sizeof(loopargs_t), "array of loopargs");
	memzero(loopargs, loopargs_len * sizeof(loopargs_t));
	for(i = 0; i < loopargs_len; i++) {
		...
		loopargs[i].buf_malloc = (unsigned char *)app_malloc(buflen, "input buffer");
		loopargs[i].buf2_malloc = (unsigned char *)app_malloc(buflen, "input buffer");
		memzero(loopargs[i].buf_malloc, buflen);
		memzero(loopargs[i].buf2_malloc, buflen);
		/* Align the start of buffers on a 64 byte boundary */
		loopargs[i].buf = loopargs[i].buf_malloc + misalign;
		loopargs[i].buf2 = loopargs[i].buf2_malloc + misalign;
		loopargs[i].buflen = buflen - misalign;
		loopargs[i].sigsize = buflen - misalign;
		loopargs[i].secret_a = (unsigned char *)app_malloc(MAX_ECDH_SIZE, "ECDH secret a");
		loopargs[i].secret_b = (unsigned char *)app_malloc(MAX_ECDH_SIZE, "ECDH secret b");
#ifndef OPENSSL_NO_DH
		loopargs[i].secret_ff_a = (unsigned char *)app_malloc(MAX_FFDH_SIZE, "FFDH secret a");
		loopargs[i].secret_ff_b = (unsigned char *)app_malloc(MAX_FFDH_SIZE, "FFDH secret b");
#endif
	}
#ifndef NO_FORK
	...
	if(doit[D_HMAC]) {
		...
		if(mac == NULL || evp_mac_mdname == NULL)
			goto end;
		evp_hmac_name = (char *)app_malloc(sizeof("hmac()") + strlen(evp_mac_mdname), "HMAC name");
		...
		if(evp_cipher) {
			...
			if(EVP_CIPHER_get_mode(evp_cipher) == EVP_CIPH_CCM_MODE) {
				...
				if(lengths == lengths_list) {
					...
					size_num = SIZEOFARRAY(aead_lengths_list);
				}
			}
			for(testnum = 0; testnum < size_num; testnum++) {
				...
				for(k = 0; k < loopargs_len; k++) {
					...
					if(!EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL, iv, _Speed_Decrypt ? 0 : 1)) {
					...
					loopargs[k].key = (unsigned char *)app_malloc(keylen, "evp_cipher key");
	...
	if(doit[D_EVP_CMAC]) {
		...
		evp_cmac_name = (char *)app_malloc(sizeof("cmac()") + strlen(evp_mac_ciphername), "CMAC name");
	...
	for(testnum = 0; testnum < RSA_NUM; testnum++) {
		...
		if(primes > RSA_DEFAULT_PRIME_NUM) {
			...
			bn = BN_new();
			st = bn && BN_set_word(bn, RSA_F4) && init_gen_str(&genctx, "RSA", NULL, 0, NULL, NULL) && 
...
show_res:
	...
	testnum = 1;
	for(k = 0; k < SIZEOFARRAY(ecdsa_doit); k++) {
		if(!ecdsa_doit[k])
			continue;
		if(testnum && !mr) {
	...
	testnum = 1;
	for(k = 0; k < SIZEOFARRAY(eddsa_doit); k++) {
		if(!eddsa_doit[k])
			continue;
		if(testnum && !mr) {
			printf("%30ssign    verify    sign/s verify/s\n", " ");
...
#ifndef OPENSSL_NO_SM2
	testnum = 1;
	for(k = 0; k < SIZEOFARRAY(sm2_doit); k++) {
		if(!sm2_doit[k])
			continue;
		if(testnum && !mr) {
			printf("%30ssign    verify    sign/s verify/s\n", " ");
	...
	for(i = 0; i < loopargs_len; i++) {
		...
		for(k = 0; k < SM2_NUM; k++) {
			EVP_PKEY_CTX * pctx = NULL;
			/* free signing ctx */
			if(loopargs[i].sm2_ctx[k] && (pctx = EVP_MD_CTX_get_pkey_ctx(loopargs[i].sm2_ctx[k])) != NULL)
				EVP_PKEY_CTX_free(pctx);
			EVP_MD_CTX_free(loopargs[i].sm2_ctx[k]);
			/* free verification ctx */
			if(loopargs[i].sm2_vfy_ctx[k] && (pctx = EVP_MD_CTX_get_pkey_ctx(loopargs[i].sm2_vfy_ctx[k])) != NULL)
				EVP_PKEY_CTX_free(pctx);
...
static char * sstrsep(char ** string, const char * delim)
{
	...
	if(**string == 0)
		return NULL;
	memzero(isdelim, sizeof(isdelim));
	isdelim[0] = 1;
	while(*delim) {
    ...
    const openssl_speed_sec_t * seconds)
{
	...
	int j, count, keylen, num = SIZEOFARRAY(mblengths_list);
	const char * alg_name;
	...
	inp = (unsigned char *)app_malloc(mblengths[num - 1], "multiblock input buffer");
	out = (unsigned char *)app_malloc(mblengths[num - 1] + 1024, "multiblock output buffer");
	if((ctx = EVP_CIPHER_CTX_new()) == NULL)
	...
	key = (unsigned char *)app_malloc(keylen, "evp_cipher key");
	...
	for(j = 0; j < num; j++) {
		...
		for(count = 0; run && count < INT_MAX; count++) {
			...
			int packlen;
			memzero(aad, 8); /* avoid uninitialized values */
