...
#include <internal\openssl-app-internal.h>
#pragma hdrstop
...
static int truststore_set_host_etc(X509_STORE * ts, const char * host)
{
	...
	if(!X509_VERIFY_PARAM_set1_host(ts_vpm, NULL, 0) || !X509_VERIFY_PARAM_set1_ip(ts_vpm, NULL, 0) || !X509_VERIFY_PARAM_set1_email(ts_vpm, NULL, 0))
		return 0;
	X509_VERIFY_PARAM_set_hostflags(ts_vpm, X509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT|X509_CHECK_FLAG_NO_PARTIAL_WILDCARDS);
	return (host && X509_VERIFY_PARAM_set1_ip_asc(ts_vpm, host)) || X509_VERIFY_PARAM_set1_host(ts_vpm, host, 0);
}
    ...
    const OSSL_CMP_MSG * req)
{
	...
	if(opt_reqin && opt_rspin == NULL) {
		 ...
		 */
		if(opt_reqin_new_tid && !OSSL_CMP_MSG_update_transactionID(ctx, req_new))
			goto err;
	}
	if(opt_rspin) {
		...
		res = opt_use_mock_srv ? OSSL_CMP_CTX_server_perform(ctx, actual_req) : OSSL_CMP_MSG_http_perform(ctx, actual_req);
	}
	if(res == NULL)
		goto err;
	if(opt_reqin || prev_opt_rspin) {
		...
		if(!OSSL_CMP_CTX_set1_senderNonce(ctx, nonce) || !OSSL_CMP_CTX_set1_transactionID(ctx, tid)) {
...
static int set_name(const char * str, int (*set_fn)(OSSL_CMP_CTX * ctx, const X509_NAME * name), OSSL_CMP_CTX * ctx, const char * desc)
{
	if(str) {
...
static int set_gennames(OSSL_CMP_CTX * ctx, char * names, const char * desc)
{
	...
	for(; names; names = next) {
		...
		if(!n)
			n = a2i_GENERAL_NAME(NULL, NULL, NULL, strchr(names, ':') ? GEN_URI : GEN_DNS, names, 0);
...
#endif
	if(opt_keyform_s && !opt_format(opt_keyform_s, FORMAT_OPTIONS, &opt_keyform)) {
		CMP_err("unknown option given for key loading format");
		return 0;
	}
#undef FORMAT_OPTIONS
	if(opt_certform_s && !opt_format(opt_certform_s, OPT_FMT_PEMDER, &opt_certform)) {
...
static OSSL_CMP_SRV_CTX * setup_srv_ctx(ENGINE * engine)
{
	...
	OSSL_CMP_SRV_CTX * srv_ctx = ossl_cmp_mock_srv_new(app_get0_libctx(), app_get0_propq());
	if(srv_ctx == NULL)
		return NULL;
	...
	if(opt_srv_secret) {
		...
		if(pass_str) {
			...
			res = OSSL_CMP_CTX_set1_secretValue(ctx, (unsigned char *)pass_str, strlen(pass_str));
			clear_free(pass_str);
			if(res == 0)
				goto err;
		}
	...
	if(opt_srv_secret == NULL && ((opt_srv_cert == NULL) != (opt_srv_key == NULL))) {
		CMP_err("must give both -srv_cert and -srv_key options or neither");
		goto err;
	}
	if(opt_srv_cert) {
		X509 * srv_cert = load_cert_pwd(opt_srv_cert, opt_srv_keypass, "certificate of the mock server");
	...
	if(opt_srv_key) {
		EVP_PKEY * pkey = load_key_pwd(opt_srv_key, opt_keyform, opt_srv_keypass, engine, "private key for mock server cert");
	...
	if(!setup_certs(opt_srv_untrusted, "untrusted certificates for mock server", ctx, (add_X509_stack_fn_t)OSSL_CMP_CTX_set1_untrusted))
		goto err;
	if(opt_rsp_cert == NULL) {
		CMP_warn("no -rsp_cert given for mock server");
	}
	else {
		X509 * cert = load_cert_pwd(opt_rsp_cert, opt_keypass, "cert to be returned by the mock server");
		if(!cert)
			goto err;
...
{
	if(!setup_certs(opt_untrusted, "untrusted certificates", ctx, (add_X509_stack_fn_t)OSSL_CMP_CTX_set1_untrusted))
		return 0;
	if(opt_srvcert || opt_trusted) {
		...
		if(opt_srvcert) {
			...
			srvcert = load_cert_pwd(opt_srvcert, opt_otherpass, "directly trusted CMP server certificate");
			ok = srvcert && OSSL_CMP_CTX_set1_srvCert(ctx, srvcert);
			X509_free(srvcert);
			if(!ok)
				return 0;
		}
	...
	if(opt_unprotected_errors)
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_UNPROTECTED_ERRORS, 1);
	if(opt_out_trusted) { /* for use in OSSL_CMP_certConf_cb() */
		X509_VERIFY_PARAM * out_vpm = NULL;
		X509_STORE * out_trusted = load_trusted(opt_out_trusted, 1, "trusted certs for verifying newly enrolled cert");
		if(out_trusted == NULL)
			return 0;
 ...
 * Returns pointer on success, NULL on error
 */
static SSL_CTX * setup_ssl_ctx(OSSL_CMP_CTX * ctx, const char * host, ENGINE * engine)
{
	...
	if(opt_tls_cert && opt_tls_key) {
		...
		int ok;
		if(!load_cert_certs(opt_tls_cert, &cert, &certs, 0, opt_tls_keypass, "TLS client certificate (optionally with chain)", vpm))
		...
		if(!ok || !SSL_CTX_set0_chain(ssl_ctx, certs)) {
			CMP_err1("unable to use client TLS certificate file '%s'", opt_tls_cert);
			sk_X509_pop_free(certs, X509_free);
			goto err;
		}
		...
		if(opt_tls_extra) {
			STACK_OF(X509) *tls_extra = load_certs_multifile(opt_tls_extra, opt_otherpass, "extra certificates for TLS", vpm);
			int res = 1;
			if(tls_extra == NULL)
				goto err;
	...
	if(opt_tls_trusted) {
		...
		if(!truststore_set_host_etc(trust_store, opt_tls_host ? opt_tls_host : host))
			goto err;
...
static int setup_protection_ctx(OSSL_CMP_CTX * ctx, ENGINE * engine)
{
	...
	if(opt_secret) {
		...
		if(pass_string) {
			cleanse(opt_secret);
			res = OSSL_CMP_CTX_set1_secretValue(ctx, (unsigned char *)pass_string, strlen(pass_string));
			clear_free(pass_string);
			if(res == 0)
				return 0;
		}
	...
	if(opt_ref && !OSSL_CMP_CTX_set1_referenceValue(ctx, (unsigned char *)opt_ref, strlen(opt_ref)))
		return 0;
	if(opt_key) {
		EVP_PKEY * pkey = load_key_pwd(opt_key, opt_keyform, opt_keypass, engine, "private key for CMP client certificate");
		if(pkey == NULL || !OSSL_CMP_CTX_set1_pkey(ctx, pkey)) {
			EVP_PKEY_free(pkey);
			return 0;
		}
	...
	if(opt_cert) {
		...
		int ok;
		if(!load_cert_certs(opt_cert, &cert, &certs, 0, opt_keypass, "CMP client certificate (optionally with chain)", vpm))
		...
		if(!ok) {
			...
			if(opt_own_trusted) {
				own_trusted = load_trusted(opt_own_trusted, 0, "trusted certs for verifying own CMP signer cert");
				ok = own_trusted != NULL;
			}
	...
	if(!setup_certs(opt_extracerts, "extra certificates for CMP", ctx, (add_X509_stack_fn_t)OSSL_CMP_CTX_set1_extraCertsOut))
		return 0;
	...
	if(opt_digest) {
		...
		if(!OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_DIGEST_ALGNID, digest) || !OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_OWF_ALGNID, digest)) {
...
static int setup_request_ctx(OSSL_CMP_CTX * ctx, ENGINE * engine)
{
	...
	X509V3_CTX ext_ctx;
	if(opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL && opt_cert == NULL && opt_cmd != CMP_RR && opt_cmd != CMP_GENM)
	...
	if(opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
		...
		if(!set_name(opt_subject, OSSL_CMP_CTX_set1_subjectName, ctx, "subject") || !set_name(opt_issuer, OSSL_CMP_CTX_set1_issuer, ctx, "issuer"))
			return 0;
	}
	else {
	...
	if(opt_cmd == CMP_KUR) {
		...
		if(opt_subject)
			CMP_warn2("given -subject '%s' overrides the subject of '%s' for KUR", opt_subject, ref_cert ? ref_cert : opt_csr);
	}
	if(opt_cmd == CMP_RR) {
	...
	if(opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL && opt_oldcert == NULL && opt_cert == NULL)
	...
	if(opt_days > 0 && !OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_VALIDITY_DAYS, opt_days)) {
		CMP_err("could not set requested cert validity period");
		return 0;
	}
	if(opt_policies && opt_policy_oids) {
	...
	if(opt_reqexts || opt_policies) {
		if((exts = sk_X509_EXTENSION_new_null()) == NULL)
			goto oom;
		X509V3_set_ctx(&ext_ctx, NULL, NULL, csr, NULL, X509V3_CTX_REPLACE);
		X509V3_set_nconf(&ext_ctx, conf);
		if(opt_reqexts && !X509V3_EXT_add_nconf_sk(conf, &ext_ctx, opt_reqexts, &exts)) {
			CMP_err1("cannot load certificate request extension section '%s'", opt_reqexts);
			goto exts_err;
		}
		if(opt_policies && !X509V3_EXT_add_nconf_sk(conf, &ext_ctx, opt_policies, &exts)) {
	...
	if(OSSL_CMP_CTX_reqExtensions_have_SAN(ctx) && opt_sans) {
	...
	if(opt_oldcert) {
		if(opt_cmd == CMP_GENM) {
			...
			X509 * oldcert = load_cert_pwd(opt_oldcert, opt_keypass, opt_cmd == CMP_KUR ? "certificate to be updated" : opt_cmd == CMP_RR ? "certificate to be revoked" : "reference certificate (oldcert)");
...
static int setup_client_ctx(OSSL_CMP_CTX * ctx, ENGINE * engine)
{
	...
	if(!OSSL_HTTP_parse_url(opt_server, &ssl, NULL /* user */, &host, &port, &portnum, &path, NULL /* q */, NULL /* frag */)) {
	...
	if(opt_path == NULL)
		used_path = path;
	if(!OSSL_CMP_CTX_set1_server(ctx, host) || !OSSL_CMP_CTX_set_serverPort(ctx, portnum))
		goto oom;
	if(opt_proxy && !OSSL_CMP_CTX_set1_proxy(ctx, opt_proxy))
		goto oom;
	if(opt_no_proxy && !OSSL_CMP_CTX_set1_no_proxy(ctx, opt_no_proxy))
		goto oom;
...
set_path:
	...
	if(!setup_verification_ctx(ctx))
		goto err;
	if(opt_keep_alive != 1)
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_KEEP_ALIVE, opt_keep_alive);
	if(opt_total_timeout > 0 && opt_msg_timeout > 0 && opt_total_timeout < opt_msg_timeout) {
		CMP_err2("-total_timeout argument = %d must not be < %d (-msg_timeout)", opt_total_timeout, opt_msg_timeout);
		goto err;
	}
	if(opt_msg_timeout >= 0) /* must do this before setup_ssl_ctx() */
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_MSG_TIMEOUT, opt_msg_timeout);
	if(opt_total_timeout >= 0)
		(void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_TOTAL_TIMEOUT, opt_total_timeout);
	...
	if(opt_reqin || opt_reqout || opt_rspin || opt_rspout || opt_use_mock_srv)
	...
	if(opt_tls_used) {
		...
		if((info = (APP_HTTP_TLS_INFO *)OPENSSL_zalloc(sizeof(*info))) == NULL)
			goto err;
	...
	if(!setup_request_ctx(ctx, engine))
		goto err;
	if(!set_name(opt_recipient, OSSL_CMP_CTX_set1_recipient, ctx, "recipient") || !set_name(opt_expect_sender, OSSL_CMP_CTX_set1_expected_sender, ctx, "expected sender"))
		goto err;
	if(opt_geninfo && !handle_opt_geninfo(ctx))
		goto err;
...
{
	if((opt_certform == FORMAT_PEM && PEM_write_bio_X509(bio, cert)) || (opt_certform == FORMAT_ASN1 && i2d_X509_bio(bio, cert)))
		return 1;
 ...
 */
static int save_free_certs(OSSL_CMP_CTX * ctx, STACK_OF(X509) * certs, char * destFile, char * desc)
{
	BIO * bio = NULL;
	int i;
...
static int conf_get_number_e(const CONF * conf_, const char * groups, const char * name, long * result)
{
...
static int read_config(void)
{
	 ...
	 */
	int n_options = SIZEOFARRAY(cmp_options) - 1;
	for(opt = &cmp_options[start_opt], i = start_idx; opt->name; i++, opt++)
		if(!strcmp(opt->name, OPT_SECTION_STR) || !strcmp(opt->name, OPT_MORE_STR))
			n_options--;
	OPENSSL_assert(SIZEOFARRAY(cmp_vars) == n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST + OPT_R__FIRST + 1 - OPT_R__LAST + OPT_V__FIRST + 1 - OPT_V__LAST);
	for(opt = &cmp_options[start_opt], i = start_idx; opt->name; i++, opt++) {
		int provider_option = (OPT_PROV__FIRST <= opt->retval && opt->retval < OPT_PROV__LAST);
		int rand_state_option = (OPT_R__FIRST <= opt->retval && opt->retval < OPT_R__LAST);
		int verification_option = (OPT_V__FIRST <= opt->retval && opt->retval < OPT_V__LAST);
		if(strcmp(opt->name, OPT_SECTION_STR) == 0 || strcmp(opt->name, OPT_MORE_STR) == 0) {
			i--;
			continue;
		}
...
static int cmp_server(OSSL_CMP_CTX * srv_cmp_ctx) {
	...
	while(opt_max_msgs <= 0 || msgs < opt_max_msgs) {
		...
		if(req) {
			...
			ret = http_server_send_asn1_resp(cbio, keep_alive, "application/pkixcmp", ASN1_ITEM_rptr(OSSL_CMP_MSG), (const ASN1_VALUE*)resp);
...
int cmp_main(int argc, char ** argv)
{
	...
	if(configfile && configfile[0] != '\0' /* non-empty string */ && (configfile != default_config_file || access(configfile, F_OK) != -1)) {
		CMP_info2("using section(s) '%s' of OpenSSL configuration file '%s'", opt_section, configfile);
		...
		if(conf == NULL) {
			...
			if(strcmp(opt_section, CMP_SECTION) == 0) { /* default */
				if(!NCONF_get_section(conf, opt_section))
					CMP_info2("no [%s] section found in config file '%s'; will thus use just [default] and unnamed section if present", opt_section, configfile);
			}
			else {
...
#ifndef OPENSSL_NO_SOCK
	if((opt_tls_cert || opt_tls_key || opt_tls_keypass || opt_tls_extra || opt_tls_trusted || opt_tls_host) && !opt_tls_used)
...
#ifndef OPENSSL_NO_SOCK
	if(opt_tls_used && (opt_use_mock_srv || opt_rspin)) {
		CMP_warn("ignoring -tls_used option since -use_mock_srv or -rspin is given");
		opt_tls_used = 0;
	}
	if(opt_port) { /* act as very basic CMP HTTP server */
	...
	for(i = 0; i < opt_repeat; i++) {
			...
			goto err; /* we got no response, maybe even did not send request */
		{
			...
			char * buf = (char *)app_malloc(OSSL_CMP_PKISI_BUFLEN, "PKIStatusInfo buf");
...
#endif
			CMP_print(bio_err, status == OSSL_CMP_PKISTATUS_accepted ? OSSL_CMP_LOG_INFO : status == OSSL_CMP_PKISTATUS_rejection || status == OSSL_CMP_PKISTATUS_waiting ? 
				OSSL_CMP_LOG_ERR : OSSL_CMP_LOG_WARNING, status == OSSL_CMP_PKISTATUS_accepted ? "info" :
			    status == OSSL_CMP_PKISTATUS_rejection ? "server error" : status == OSSL_CMP_PKISTATUS_waiting ? "internal error" : "warning",
			    "received%s%s %s", from, server, string != NULL ? string : "<unknown PKIStatus>");
			OPENSSL_free(buf);
		}
		if(save_free_certs(cmp_ctx, OSSL_CMP_CTX_get1_extraCertsIn(cmp_ctx), opt_extracertsout, "extra") < 0)
			ret = 0;
		if(!ret)
			goto err;
		ret = 0;
		...
		if(save_free_certs(cmp_ctx, OSSL_CMP_CTX_get1_newChain(cmp_ctx), opt_chainout, "chain") < 0)
			goto err;
...
err:
	...
	if(ret != 1)
		OSSL_CMP_CTX_print_errors(cmp_ctx);
	ossl_cmp_mock_srv_free((OSSL_CMP_SRV_CTX *)OSSL_CMP_CTX_get_transfer_cb_arg(cmp_ctx));
#ifndef OPENSSL_NO_SOCK
	APP_HTTP_TLS_INFO_free((APP_HTTP_TLS_INFO *)OSSL_CMP_CTX_get_http_cb_arg(cmp_ctx));
#endif
	X509_STORE_free((X509_STORE *)OSSL_CMP_CTX_get_certConf_cb_arg(cmp_ctx));
