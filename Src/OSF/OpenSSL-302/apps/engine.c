...
static int append_buf(char ** buf, int * size, const char * s)
{
	...
	if(p == NULL) {
		*size = ((len + expand - 1) / expand) * expand;
		p = *buf = (char *)app_malloc(*size, "engine buffer");
	}
	else {
	...
	do {
		...
		if(!(flags & ENGINE_CMD_FLAG_INTERNAL) || verbose >= 4) {
			...
			if((len = ENGINE_ctrl(e, ENGINE_CTRL_GET_NAME_LEN_FROM_CMD, num, NULL, NULL)) <= 0)
				goto err;
			name = (char *)app_malloc(len + 1, "name buffer");
			...
			if((len = ENGINE_ctrl(e, ENGINE_CTRL_GET_DESC_LEN_FROM_CMD, num,
				...
				goto err;
			if(len > 0) {
				desc = (char *)app_malloc(len + 1, "description buffer");
...
{
	struct util_store_cap_data * ctx = (struct util_store_cap_data *)arg;
