...
{
	tlsextnextprotoctx * ctx = (tlsextnextprotoctx *)arg;
	if(!c_quiet) {
...
static ossl_ssize_t hexdecode(const char ** inptr, void * result)
{
	...
	const char * in = *inptr;
	unsigned char * ret = (unsigned char *)app_malloc(strlen(in) / 2, "hexdecode");
...
/* Known false-positive of MemorySanitizer. */
	...
	cbuf = (char *)app_malloc(BUFSIZZ, "cbuf");
	sbuf = (char *)app_malloc(BUFSIZZ, "sbuf");
	mbuf = (char *)app_malloc(BUFSIZZ, "mbuf");
...
opthelp:
	...
	if(key_file == NULL)
		key_file = cert_file;
	if(key_file != NULL) {
		key = load_key(key_file, key_format, 0, pass, e, "client certificate private key");
		if(!key)
			goto end;
	}
	if(cert_file != NULL) {
		cert = load_cert_pass(cert_file, cert_format, 1, pass, "client certificate");
		if(!cert)
			goto end;
	}
	if(chain_file != NULL) {
	...
	switch((PROTOCOL_CHOICE)starttls_proto) {
		...
		case PROTO_IRC:
	    {
		    ...
		    do {
				...
				&& !BIO_pending(fbio)
				&& !BIO_pending(sbio)
				&& select(width, (fd_set *)&readfds, NULL, NULL,
				&timeout) < 1) {
				    BIO_printf(bio_err,
		...
		if(!ssl_pending) {
			...
			if(!write_tty) {
				if(read_tty) {
					tv.tv_sec = 1;
					tv.tv_usec = 0;
					i = select(width, (fd_set *)&readfds, (fd_set *)&writefds, NULL, &tv);
					if(!i && (!has_stdin_waiting() || !read_tty))
						continue;
				}
				else
					i = select(width, (fd_set *)&readfds, (fd_set *)&writefds, NULL, timeoutp);
			}
#else
