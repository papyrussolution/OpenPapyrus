...
void ossl_ec_GFp_nistp_points_make_affine_internal(size_t num, void * point_array,
    ...
    void (*felem_contract)
    (void * out, const void * in));
void ossl_ec_GFp_nistp_recode_scalar_bits(uchar * sign,
    uchar * digit,
    unsigned char in);
#endif
...
int ossl_ec_key_simple_oct2priv(EC_KEY * eckey, const uchar * buf,
    size_t len);
...
struct ec_key_method_st {
	...
	int (* compute_key)(uchar ** pout, size_t * poutlen,
	    const EC_POINT * pub_key, const EC_KEY * ecdh);
	int (* sign)(int type, const uchar * dgst, int dlen, unsigned char
...
int ossl_ecdsa_simple_sign_setup(EC_KEY * eckey, BN_CTX * ctx_in, BIGNUM ** kinvp,
    BIGNUM ** rp);
ECDSA_SIG * ossl_ecdsa_simple_sign_sig(const uchar * dgst, int dgst_len,
    const BIGNUM * in_kinv, const BIGNUM * in_r,
    EC_KEY * eckey);
int ossl_ecdsa_simple_verify_sig(const uchar * dgst, int dgst_len,
    const ECDSA_SIG * sig, EC_KEY * eckey);
