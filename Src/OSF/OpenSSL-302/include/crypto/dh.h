...
int ossl_dh_compute_key(uchar * key, const BIGNUM * pub_key, DH * dh);
...
int ossl_dh_buf2key(DH * key, const uchar * buf, size_t len);
size_t ossl_dh_key2buf(const DH * dh, uchar ** pbuf, size_t size, int alloc);
int ossl_dh_kdf_X9_42_asn1(uchar * out, size_t outlen, const uchar * Z, size_t Zlen, const char * cek_alg, const uchar * ukm, size_t ukmlen, const EVP_MD * md, OSSL_LIB_CTX * libctx, const char * propq);
