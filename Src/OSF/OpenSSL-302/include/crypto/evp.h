...
struct evp_pkey_ctx_st {
	...
	void * app_data; /* Application specific data, usually used by the callback */
	EVP_PKEY_gen_cb * pkey_gencb; /* Keygen callback */
	/* implementation specific keygen data */
	int * keygen_info;
	int keygen_info_count;
	/* Legacy fields below */
	int legacy_keytype; /* EVP_PKEY identity */
	...
	uint flag_call_digest_custom : 1; /* Indicator if digest_custom needs to be called */
	/*
...
struct evp_pkey_method_st {
	...
	int (* digestsign) (EVP_MD_CTX * ctx, uchar * sig, size_t * siglen, const uchar * tbs, size_t tbslen);
	int (* digestverify) (EVP_MD_CTX * ctx, const uchar * sig, size_t siglen, const uchar * tbs, size_t tbslen);
...
struct evp_cipher_st {
	...
	int origin;
	/* init key */
	int (* init) (EVP_CIPHER_CTX * ctx, const uchar * key,
	    const uchar * iv, int enc);
	/* encrypt/decrypt data */
	int (* do_cipher) (EVP_CIPHER_CTX * ctx, uchar * out,
...
int evp_cipher_param_to_asn1_ex(EVP_CIPHER_CTX * c, ASN1_TYPE * type, evp_cipher_aead_asn1_params * params);
int evp_cipher_asn1_to_param_ex(EVP_CIPHER_CTX * c, ASN1_TYPE * type, evp_cipher_aead_asn1_params * params);
/*
...
struct evp_pkey_st {
	...
	union legacy_pkey_st pkey; /* Union to store the reference to an origin legacy key */
	union legacy_pkey_st legacy_cache_pkey; /* Union to store the reference to a non-origin legacy key */
#endif
	/* == Common attributes == */
	...
	int save_parameters;
	uint foreign : 1; /* the low-level key is using an engine or an app-method */
...
void evp_encode_ctx_set_flags(EVP_ENCODE_CTX * ctx, uint flags);
/* EVP_ENCODE_CTX flags */
#define EVP_ENCODE_CTX_NO_NEWLINES          1 /* Don't generate new lines when encoding */
#define EVP_ENCODE_CTX_USE_SRP_ALPHABET     2 /* Use the SRP base64 alphabet instead of the standard one */
...
int ossl_pkcs5_pbkdf2_hmac_ex(const char * pass, int passlen, const uchar * salt, int saltlen, int iter, const EVP_MD * digest, int keylen, uchar * out, OSSL_LIB_CTX * libctx, const char * propq);
