...
int ossl_asn1_item_digest_ex(const ASN1_ITEM * it, const EVP_MD * type,
    void * data, uchar * md, unsigned int * len,
...
X509_PUBKEY * ossl_d2i_X509_PUBKEY_INTERNAL(const uchar ** pp,
    long len, OSSL_LIB_CTX * libctx);
void ossl_X509_PUBKEY_INTERNAL_free(X509_PUBKEY * xpub);

RSA * ossl_d2i_RSA_PSS_PUBKEY(RSA ** a, const uchar ** pp, long length);
int ossl_i2d_RSA_PSS_PUBKEY(const RSA * a, uchar ** pp);
...
#endif /* OPENSSL_NO_EC */
EVP_PKEY * ossl_d2i_PUBKEY_legacy(EVP_PKEY ** a, const uchar ** pp,
    long length);
