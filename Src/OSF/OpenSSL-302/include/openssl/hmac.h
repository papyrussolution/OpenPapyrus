...
extern "C" {
	...
	OSSL_DEPRECATEDIN_3_0 int HMAC_Update(HMAC_CTX * ctx, const uchar * data, size_t len);
	OSSL_DEPRECATEDIN_3_0 int HMAC_Final(HMAC_CTX * ctx, uchar * md, unsigned int * len);
...
#endif
uchar * HMAC(const EVP_MD * evp_md, const void * key, int key_len, const uchar * data, size_t data_len, uchar * md, unsigned int * md_len);
