...
#define i2d_DSAparams_fp(fp, x) \
	ASN1_i2d_fp(i2d_DSAparams, (fp), (uchar *)(x))
...
OSSL_DEPRECATEDIN_3_0 DSA_SIG *DSA_do_sign(const uchar * dgst, int dlen,
    DSA *dsa);
OSSL_DEPRECATEDIN_3_0 int DSA_do_verify(const uchar * dgst, int dgst_len,
    DSA_SIG * sig, DSA * dsa);
...
OSSL_DEPRECATEDIN_3_0 int DSA_sign(int type, const uchar * dgst,
    int dlen, uchar * sig,
    unsigned int * siglen, DSA * dsa);
OSSL_DEPRECATEDIN_3_0 int DSA_verify(int type, const uchar * dgst,
    int dgst_len, const uchar * sigbuf,
    int siglen, DSA * dsa);
...
DSA * DSA_generate_parameters(int bits, uchar * seed, int seed_len,
