...
static int julian_adj(const struct tm * tm, int off_day, long offset_sec, long * pday, int * psec)
{
	...
	offset_day = offset_sec / SlConst::SecsPerDay;
	// Avoid sign issues with % operator
	offset_hms = offset_sec - (offset_day * SlConst::SecsPerDay);
	offset_day += off_day;
	// Add current time seconds to offset 
	offset_hms += tm->tm_hour * 3600 + tm->tm_min * 60 + tm->tm_sec;
	// Adjust day seconds if overflow 
	if(offset_hms >= SlConst::SecsPerDay) {
		offset_day++;
		offset_hms -= SlConst::SecsPerDay;
	}
