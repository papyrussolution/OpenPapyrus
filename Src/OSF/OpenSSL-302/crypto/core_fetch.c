...
static int ossl_method_construct_precondition(OSSL_PROVIDER * provider, int operation_id, void * cbdata, int * result)
{
...
static int ossl_method_construct_postcondition(OSSL_PROVIDER * provider, int operation_id, int no_store, void * cbdata, int * result)
{
	...
	*result = 1;
	return no_store != 0 || ossl_provider_set_operation_bit(provider, operation_id);
}
...
static void ossl_method_construct_this(OSSL_PROVIDER * provider, const OSSL_ALGORITHM * algo, int no_store, void * cbdata)
{
	...
	if(data->force_store || !no_store) {
		...
		data->mcm->put(NULL, method, provider, algo->algorithm_names, algo->property_definition, data->mcm_data);
	}
	else {
		/*
		 ...
		 */
		if((data->store = (OSSL_METHOD_STORE *)data->mcm->get_tmp_store(data->mcm_data)) == NULL)
			return;
...
{
	void * method = NULL;
	if((method = mcm->get(NULL, (const OSSL_PROVIDER**)provider_rw, mcm_data)) == NULL) {
