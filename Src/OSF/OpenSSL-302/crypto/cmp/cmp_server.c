...
{
	OSSL_CMP_SRV_CTX * ctx = (OSSL_CMP_SRV_CTX*)OPENSSL_zalloc(sizeof(OSSL_CMP_SRV_CTX));
	if(!ctx)
		goto err;
...
OSSL_CMP_MSG * OSSL_CMP_CTX_server_perform(OSSL_CMP_CTX * client_ctx, const OSSL_CMP_MSG * req)
{
	...
	if((srv_ctx = (OSSL_CMP_SRV_CTX *)OSSL_CMP_CTX_get_transfer_cb_arg(client_ctx)) == NULL) {
