...
{
	OSSL_CMP_CTX * ctx = (OSSL_CMP_CTX*)OPENSSL_zalloc(sizeof(*ctx));
	if(!ctx)
		goto err;
    ...
    int category, int cmd, void * vdata)
{
	...
	if(!ctx->log_cb(func != NULL ? func : "(no func)", file ? file : "(no file)", line, level, msg))
		cnt = 0;
end:
	OPENSSL_free(func);
	OPENSSL_free(file);
...
{
	if(ctx && OSSL_CMP_LOG_ERR > ctx->log_verbosity)
 ...
 */
int OSSL_CMP_CTX_set1_referenceValue(OSSL_CMP_CTX * ctx, const uchar * ref, int len)
{
	if(!ctx) {
		ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
		return 0;
...
int OSSL_CMP_CTX_set1_secretValue(OSSL_CMP_CTX * ctx, const uchar * sec, const int len)
{
 ...
 * of the OSSL_CMP_CTX structure so that they may be retrieved later.
 */
int ossl_cmp_ctx_set1_extraCertsIn(OSSL_CMP_CTX * ctx, STACK_OF(X509) * extraCertsIn)
{
	if(!ossl_assert(ctx != NULL))
		return 0;
	sk_X509_pop_free(ctx->extraCertsIn, X509_free);
	ctx->extraCertsIn = NULL;
	return extraCertsIn == NULL || (ctx->extraCertsIn = X509_chain_up_ref(extraCertsIn)) != NULL;
}
...
int OSSL_CMP_CTX_set1_extraCertsOut(OSSL_CMP_CTX * ctx, STACK_OF(X509) * extraCertsOut)
{
	...
	return extraCertsOut == NULL || (ctx->extraCertsOut = X509_chain_up_ref(extraCertsOut)) != NULL;
}
...
int OSSL_CMP_CTX_push0_policy(OSSL_CMP_CTX * ctx, POLICYINFO * pinfo)
{
	...
	if(ctx->policies == NULL && (ctx->policies = CERTIFICATEPOLICIES_new()) == NULL)
		return 0;
...
#define DEFINE_OSSL_CMP_CTX_set1(FIELD, TYPE) /* this uses _dup */ \
    ...
    \
		if(val && (val_dup = TYPE ## _dup(val)) == NULL) \
			return 0; \
...
DEFINE_OSSL_CMP_CTX_set1(recipient, X509_NAME) /* Set the X509 name of the recipient. Set in the PKIHeader */
DEFINE_OSSL_CMP_CTX_set1(expected_sender, X509_NAME) /* Store the X509 name of the expected sender in the PKIHeader of responses */
DEFINE_OSSL_CMP_CTX_set1(issuer, X509_NAME) /* Set the X509 name of the issuer. Set in the PKIHeader */
/*
...
int OSSL_CMP_CTX_set0_reqExtensions(OSSL_CMP_CTX * ctx, X509_EXTENSIONS * exts)
{
	...
	if(sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 && exts && X509v3_get_ext_by_NID(exts, NID_subject_alt_name, -1) >= 0) {
...
int OSSL_CMP_CTX_reqExtensions_have_SAN(OSSL_CMP_CTX * ctx)
{
	...
	return ctx->reqExtensions && X509v3_get_ext_by_NID(ctx->reqExtensions, NID_subject_alt_name, -1) >= 0;
}
...
int OSSL_CMP_CTX_get_option(const OSSL_CMP_CTX * ctx, int opt)
{
	...
	switch(opt) {
		case OSSL_CMP_OPT_LOG_VERBOSITY: return ctx->log_verbosity;
		case OSSL_CMP_OPT_IMPLICIT_CONFIRM: return ctx->implicitConfirm;
		case OSSL_CMP_OPT_DISABLE_CONFIRM: return ctx->disableConfirm;
		case OSSL_CMP_OPT_UNPROTECTED_SEND: return ctx->unprotectedSend;
		case OSSL_CMP_OPT_UNPROTECTED_ERRORS: return ctx->unprotectedErrors;
		case OSSL_CMP_OPT_VALIDITY_DAYS: return ctx->days;
		case OSSL_CMP_OPT_SUBJECTALTNAME_NODEFAULT: return ctx->SubjectAltName_nodefault;
		case OSSL_CMP_OPT_SUBJECTALTNAME_CRITICAL: return ctx->setSubjectAltNameCritical;
		case OSSL_CMP_OPT_POLICIES_CRITICAL: return ctx->setPoliciesCritical;
		case OSSL_CMP_OPT_IGNORE_KEYUSAGE: return ctx->ignore_keyusage;
		case OSSL_CMP_OPT_POPO_METHOD: return ctx->popoMethod;
		case OSSL_CMP_OPT_DIGEST_ALGNID: return EVP_MD_get_type(ctx->digest);
		case OSSL_CMP_OPT_OWF_ALGNID: return EVP_MD_get_type(ctx->pbm_owf);
		case OSSL_CMP_OPT_MAC_ALGNID: return ctx->pbm_mac;
		case OSSL_CMP_OPT_KEEP_ALIVE: return ctx->keep_alive;
		case OSSL_CMP_OPT_MSG_TIMEOUT: return ctx->msg_timeout;
		case OSSL_CMP_OPT_TOTAL_TIMEOUT: return ctx->total_timeout;
		case OSSL_CMP_OPT_PERMIT_TA_IN_EXTRACERTS_FOR_IR: return ctx->permitTAInExtraCertsForIR;
		case OSSL_CMP_OPT_REVOCATION_REASON: return ctx->revocationReason;
		default: ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_OPTION); return -1;
	}
}
