...
{
	const char * end_level = sstrchr(level, ':');
	int len;
...
const char * ossl_cmp_log_parse_metadata(const char * buf, OSSL_CMP_severity * level, char ** func, char ** file, int * line)
{
	...
	const char * p_file = buf == NULL ? NULL : sstrchr(buf, ':');
	...
	*line = 0;
	if(p_file != NULL) {
		const char * p_line = sstrchr(++p_file, ':');
