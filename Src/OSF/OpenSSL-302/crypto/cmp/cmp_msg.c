...
{
	OSSL_CMP_MSG * msg = (OSSL_CMP_MSG*)ASN1_item_new_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx, propq);
...
static const X509_NAME * determine_subj(OSSL_CMP_CTX * ctx, const X509_NAME * ref_subj, int for_KUR)
{
...
OSSL_CRMF_MSG * OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX * ctx, int for_KUR, int rid)
{
	...
	X509 * refcert = ctx->oldCert ? ctx->oldCert : ctx->cert;
	/* refcert defaults to current client cert */
	EVP_PKEY * rkey = OSSL_CMP_CTX_get0_newPkey(ctx, 0);
	STACK_OF(GENERAL_NAME) *default_sans = NULL;
	const X509_NAME * ref_subj = ctx->p10CSR ? X509_REQ_get_subject_name(ctx->p10CSR) : (refcert ? X509_get_subject_name(refcert) : NULL);
	const X509_NAME * subject = determine_subj(ctx, ref_subj, for_KUR);
	const X509_NAME * issuer = (ctx->issuer || !refcert) ? (IS_NULL_DN(ctx->issuer) ? NULL : ctx->issuer) : X509_get_issuer_name(refcert);
	...
	if(ctx->p10CSR != NULL && (exts = X509_REQ_get_extensions(ctx->p10CSR)) == NULL)
		goto err;
	if(!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) && refcert != NULL && (default_sans = (STACK_OF(GENERAL_NAME) *)X509V3_get_d2i(X509_get0_extensions(refcert), 
		NID_subject_alt_name, NULL, NULL)) != NULL && !add1_extension(&exts, NID_subject_alt_name, crit, default_sans))
		goto err;
	if(ctx->reqExtensions != NULL /* augment/override existing ones */ && !add_extensions(&exts, ctx->reqExtensions))
		goto err;
	if(sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 && !add1_extension(&exts, NID_subject_alt_name,
	    crit, ctx->subjectAltNames))
		goto err;
	if(ctx->policies != NULL && !add1_extension(&exts, NID_certificate_policies,
    ...
    int unprotected)
{
	...
	if(errorCode > 0
		...
		lib = ERR_lib_error_string((ulong)errorCode);
		reason = ERR_reason_error_string((ulong)errorCode);
	}
 ...
 */
OSSL_CMP_CERTRESPONSE * ossl_cmp_certrepmessage_get0_certresponse(const OSSL_CMP_CERTREPMESSAGE * crm,
    int rid)
{
...
OSSL_CMP_MSG * d2i_OSSL_CMP_MSG_bio(BIO * bio, OSSL_CMP_MSG ** msg)
{
	...
	return (OSSL_CMP_MSG *)ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), bio, msg, libctx, propq);
}
