...
int BIO_sock_error(int sock)
{
	 ...
	 * (char *) or (void *).
	 */
	int i = getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)&j, &size);
	if(i < 0)
...
int BIO_sock_init(void)
{
	...
	if(!wsa_init_done) {
		wsa_init_done = 1;
		memzero(&wsa_state, sizeof(wsa_state));
		/*
		...
		if(WSAStartup(0x0202, &wsa_state) != 0) {
			ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling wsastartup()");
...
#endif /* __DJGPP__ */
	if(i < 0)
		ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling ioctlsocket()");
	return i;
}
