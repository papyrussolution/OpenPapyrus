...
#endif
			{
				ERR_raise_data(ERR_LIB_BIO, ERR_R_SYS_LIB, gai_strerrorA(ret)); // @sobolev gai_strerror-->gai_strerrorA
			}
			return 0;
		}
    ...
    int family, int socktype, int protocol, BIO_ADDRINFO ** res)
{
	...
	if(1) {
		...
		// Note that |res| SHOULD be a 'struct addrinfo **' thanks to macro magic in bio_local.h
...
#endif
			    ERR_raise_data(ERR_LIB_BIO, ERR_R_SYS_LIB, gai_strerrorA(old_ret ? old_ret : gai_ret)); // @sobolev gai_strerror-->gai_strerrorA
			    break;
		}
	}
	else {
