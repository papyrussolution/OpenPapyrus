...
BIO * BIO_new_file(const char * filename, const char * mode)
{
	...
	if(file == NULL) {
		ERR_raise_data(ERR_LIB_SYS, get_last_sys_error(), "calling fopen(%s, %s)", filename, mode);
		if(errno == ENOENT
#ifdef ENXIO
		    || errno == ENXIO
#endif
...
static int file_read(BIO * b, char * out, int outl)
{
	...
	if(b->init && (out != NULL)) {
		...
		if(ret == 0 && (b->flags & BIO_FLAGS_UPLINK_INTERNAL ? UP_ferror((FILE*)b->ptr) : ferror((FILE*)b->ptr))) {
			ERR_raise_data(ERR_LIB_SYS, get_last_sys_error(), "calling fread()");
			ERR_raise(ERR_LIB_BIO, ERR_R_SYS_LIB);
			ret = -1;
		}
	}
...
#define _IOB_ENTRIES 20
		...
		case BIO_C_SET_FILENAME:
		    ...
		    if(fp == NULL) {
			    ERR_raise_data(ERR_LIB_SYS, get_last_sys_error(), "calling fopen(%s, %s)", ptr, p);
			    ERR_raise(ERR_LIB_BIO, ERR_R_SYS_LIB);
			    ret = 0;
			    break;
		    }
		...
		case BIO_CTRL_SET_CLOSE:
		    ...
		    break;
		case BIO_CTRL_FLUSH:
		    st = b->flags & BIO_FLAGS_UPLINK_INTERNAL ? UP_fflush((FILE *)b->ptr) : fflush((FILE*)b->ptr);
		    if(st == EOF) {
			    ERR_raise_data(ERR_LIB_SYS, get_last_sys_error(), "calling fflush()");
			    ERR_raise(ERR_LIB_BIO, ERR_R_SYS_LIB);
			    ret = 0;
		    }
		    break;
...
static int file_write(BIO * b, const char * in, int inl) { return -1; }
static int file_read(BIO * b, char * out, int outl) { return -1; }
static int file_puts(BIO * bp, const char * str) { return -1; }
static int file_gets(BIO * bp, char * buf, int size) { return 0; }
static long file_ctrl(BIO * b, int cmd, long num, void * ptr) { return 0; }
static int file_new(BIO * bi) { return 0; }
static int file_free(BIO * a) { return 0; }
