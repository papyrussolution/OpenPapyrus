...
{
	return (BIO_CORE_GLOBALS *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_BIO_CORE_INDEX, &bio_core_globals_method);
}
...
static int bio_core_read_ex(BIO * bio, char * data, size_t data_len, size_t * bytes_read)
{
	...
	if(bcgbl == NULL || bcgbl->c_bio_read_ex == NULL)
		return 0;
	return bcgbl->c_bio_read_ex((OSSL_CORE_BIO *)BIO_get_data(bio), data, data_len, bytes_read);
}
...
static int bio_core_write_ex(BIO * bio, const char * data, size_t data_len, size_t * written)
{
	...
	if(bcgbl == NULL || bcgbl->c_bio_write_ex == NULL)
		return 0;
	return bcgbl->c_bio_write_ex((OSSL_CORE_BIO *)BIO_get_data(bio), data, data_len, written);
}
...
static long bio_core_ctrl(BIO * bio, int cmd, long num, void * ptr)
{
	...
	if(bcgbl == NULL || bcgbl->c_bio_ctrl == NULL)
		return -1;
	return bcgbl->c_bio_ctrl((OSSL_CORE_BIO *)BIO_get_data(bio), cmd, num, ptr);
}
...
static int bio_core_gets(BIO * bio, char * buf, int size)
{
	...
	if(bcgbl == NULL || bcgbl->c_bio_gets == NULL)
		return -1;
	return bcgbl->c_bio_gets((OSSL_CORE_BIO *)BIO_get_data(bio), buf, size);
}
...
static int bio_core_puts(BIO * bio, const char * str)
{
	...
	if(bcgbl == NULL || bcgbl->c_bio_puts == NULL)
		return -1;
	return bcgbl->c_bio_puts((OSSL_CORE_BIO *)BIO_get_data(bio), str);
}
...
static int bio_core_free(BIO * bio)
{
	...
	if(bcgbl == NULL)
		return 0;
	BIO_set_init(bio, 0);
	bcgbl->c_bio_free((OSSL_CORE_BIO *)BIO_get_data(bio));
	return 1;
}
