		...
		case BIO_C_SET_CONNECT:
		    if(ptr) {
			    ...
			    if(num == 0) { /* BIO_set_conn_hostname */
				    ...
				    ret = BIO_parse_hostserv((const char *)ptr, &data->param_hostname, &data->param_service, BIO_PARSE_PRIO_HOST);
				    if(hold_service != data->param_service)
					    OPENSSL_free(hold_service);
			    }
			    else if(num == 1) { /* BIO_set_conn_port */
				    OPENSSL_free(data->param_service);
				    if((data->param_service = OPENSSL_strdup((const char *)ptr)) == NULL)
					    ret = 0;
			    }
