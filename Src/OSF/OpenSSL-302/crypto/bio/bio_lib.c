 ...
 */
#define HAS_LEN_OPER(o) ((o) == BIO_CB_READ || (o) == BIO_CB_WRITE || (o) == BIO_CB_GETS)
 ...
 */
static long bio_call_callback(BIO * b, int oper, const char * argp, size_t len, int argi, long argl, long inret, size_t * processed)
{
	long ret = inret;
#ifndef OPENSSL_NO_DEPRECATED_3_0
	int bareoper;
...
void * BIO_get_data(BIO * a) { return a->ptr; }
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
	BIO_callback_fn BIO_get_callback(const BIO * b) { return b->callback; }
	void BIO_set_callback(BIO * b, BIO_callback_fn cb) { b->callback = cb; }
#endif
BIO_callback_fn_ex BIO_get_callback_ex(const BIO * b) { return b->callback_ex; }
void BIO_set_callback_ex(BIO * b, BIO_callback_fn_ex cb) { b->callback_ex = cb; }
...
static int bio_read_intern(BIO * b, void * data, size_t dlen, size_t * readbytes)
{
	...
	if(HAS_CALLBACK(b) && ((ret = (int)bio_call_callback(b, BIO_CB_READ, (const char *)data, dlen, 0, 0L, 1L, NULL)) <= 0))
		return ret;
	if(!b->init) {
	...
	if(HAS_CALLBACK(b))
		ret = (int)bio_call_callback(b, BIO_CB_READ | BIO_CB_RETURN, (const char *)data, dlen, 0, 0L, ret, readbytes);
	/* Shouldn't happen */
...
static int bio_write_intern(BIO * b, const void * data, size_t dlen, size_t * written)
{
	...
	if(HAS_CALLBACK(b) && ((ret = (int)bio_call_callback(b, BIO_CB_WRITE, (const char *)data, dlen, 0, 0L, 1L, NULL)) <= 0))
		return ret;
	...
	ret = b->method->bwrite(b, (const char *)data, dlen, &local_written);
	if(ret > 0)
		b->num_write += (uint64_t)local_written;
	if(HAS_CALLBACK(b))
		ret = (int)bio_call_callback(b, BIO_CB_WRITE | BIO_CB_RETURN, (const char *)data, dlen, 0, 0L, ret, &local_written);
	if(written)
...
{
	return bio_write_intern(b, data, dlen, written) > 0 || (b && dlen == 0); /* order is important for *written */
}
...
long BIO_ctrl(BIO * b, int cmd, long larg, void * parg)
{
	...
	if(HAS_CALLBACK(b)) {
		ret = bio_call_callback(b, BIO_CB_CTRL, (const char *)parg, 0, cmd, larg, 1L, NULL);
		if(ret <= 0)
			return ret;
	}
	ret = b->method->ctrl(b, cmd, larg, parg);
	if(HAS_CALLBACK(b))
		ret = bio_call_callback(b, BIO_CB_CTRL | BIO_CB_RETURN, (const char *)parg, 0, cmd, larg, ret, NULL);
	return ret;
}
...
long BIO_callback_ctrl(BIO * b, int cmd, BIO_info_cb * fp)
{
	...
	if(HAS_CALLBACK(b)) {
		ret = bio_call_callback(b, BIO_CB_CTRL, (const char *)&fp, 0, cmd, 0, 1L, NULL);
		if(ret <= 0)
			return ret;
	}
	ret = b->method->callback_ctrl(b, cmd, fp);
	if(HAS_CALLBACK(b))
		ret = bio_call_callback(b, BIO_CB_CTRL | BIO_CB_RETURN, (const char *)&fp, 0, cmd, 0, ret, NULL);
	return ret;
}
/*
...
static int bio_wait(BIO * bio, time_t max_time, unsigned int nap_milliseconds)
{
	...
	if(sec_diff == 0) { /* we are below the 1 seconds resolution of max_time */
		...
		if((ulong)sec_diff * 1000 < nap_milliseconds)
