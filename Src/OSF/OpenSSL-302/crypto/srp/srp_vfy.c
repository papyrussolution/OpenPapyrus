...
static int t_fromb64(uchar * a, size_t alen, const char * src)
{
	...
	int outl = 0, outl2 = 0;
	size_t size, padsize;
	const uchar * pad = (const unsigned char*)"00";
...
static int t_tob64(char * dst, const uchar * src, int size)
{
	...
	outl += outl2;
	EVP_EncodeFinal(ctx, (uchar *)dst + outl, &outl2);
	outl += outl2;
...
static SRP_user_pwd * srp_user_pwd_dup(SRP_user_pwd * src)
{
	...
	if(!SRP_user_pwd_set1_ids(ret, src->id, src->info) || !SRP_user_pwd_set0_sv(ret, BN_dup(src->s), BN_dup(src->v))) {
