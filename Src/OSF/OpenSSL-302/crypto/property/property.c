...
{
	OSSL_GLOBAL_PROPERTIES * globp = (OSSL_GLOBAL_PROPERTIES *)vglobp;
	if(globp != NULL) {
...
OSSL_PROPERTY_LIST ** ossl_ctx_global_properties(OSSL_LIB_CTX * libctx, int loadconfig)
{
	...
	if(loadconfig && !OPENSSL_init_crypto(OPENSSL_INIT_LOAD_CONFIG, NULL))
		return NULL;
#endif
	globp = (OSSL_GLOBAL_PROPERTIES *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_GLOBAL_PROPERTIES, &ossl_ctx_global_properties_method);
...
{
	OSSL_GLOBAL_PROPERTIES * globp = (OSSL_GLOBAL_PROPERTIES *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_GLOBAL_PROPERTIES, &ossl_ctx_global_properties_method);
...
{
	OSSL_METHOD_STORE * res = (OSSL_METHOD_STORE *)OPENSSL_zalloc(sizeof(*res));
	if(res != NULL) {
		res->ctx = ctx;
...
int ossl_method_store_add(OSSL_METHOD_STORE * store, const OSSL_PROVIDER * prov, int nid, const char * properties, void * method,
    int (*method_up_ref)(void *), void (*method_destruct)(void *))
{
	ALGORITHM * alg = NULL;
	IMPLEMENTATION * impl;
	int ret = 0;
	...
	if(alg == NULL) {
		if((alg = (ALGORITHM *)OPENSSL_zalloc(sizeof(*alg))) == NULL || (alg->impls = sk_IMPLEMENTATION_new_null()) == NULL || (alg->cache = lh_QUERY_new(&query_hash, &query_cmp)) == NULL)
			goto err;
		alg->nid = nid;
...
{
	struct alg_do_each_data_st * data = (struct alg_do_each_data_st *)arg;
...
{
	SPARSE_ARRAY_OF(ALGORITHM) * algs = (SPARSE_ARRAY_OF(ALGORITHM) *)arg;
