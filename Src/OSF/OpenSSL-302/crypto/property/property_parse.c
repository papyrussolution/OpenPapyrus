    ...
    STACK_OF(OSSL_PROPERTY_DEFINITION) * sk)
{
	...
	int i;
	OSSL_PROPERTY_LIST * r = (OSSL_PROPERTY_LIST*)OPENSSL_malloc(sizeof(*r) + (n <= 0 ? 0 : n - 1) * sizeof(r->properties[0]));
	if(r) {
...
OSSL_PROPERTY_LIST * ossl_parse_property(OSSL_LIB_CTX * ctx, const char * defn)
{
	...
	while(!done) {
		const char * start = s;
		prop = (OSSL_PROPERTY_DEFINITION *)OPENSSL_malloc(sizeof(*prop));
		if(prop == NULL)
			goto err;
		memzero(&prop->v, sizeof(prop->v));
		prop->optional = 0;
		if(!parse_name(ctx, &s, 1, &prop->name_idx))
			goto err;
		prop->oper = OSSL_PROPERTY_OPER_EQ;
		if(prop->name_idx == 0) {
			ERR_raise_data(ERR_LIB_PROP, PROP_R_PARSE_FAILED, "Unknown name HERE-->%s", start);
			goto err;
		}
		if(match_ch(&s, '=')) {
			if(!parse_value(ctx, &s, prop, 1)) {
				ERR_raise_data(ERR_LIB_PROP, PROP_R_NO_VALUE, "HERE-->%s", start);
				goto err;
			}
		}
		else {
...
skip_value:
	...
	if(*s != '\0') {
		ERR_raise_data(ERR_LIB_PROP, PROP_R_TRAILING_CHARACTERS, "HERE-->%s", s);
		goto err;
	}
	res = stack_to_property_list(ctx, sk);
err:
    ...
    const OSSL_PROPERTY_LIST * b)
{
	...
	r = (OSSL_PROPERTY_LIST*)OPENSSL_malloc(sizeof(*r) + (t == 0 ? 0 : t - 1) * sizeof(r->properties[0]));
	if(r == NULL)
		return NULL;
...
int ossl_property_parse_init(OSSL_LIB_CTX * ctx)
{
	...
	for(i = 0; i < SIZEOFARRAY(predefined_names); i++)
