...
static OSSL_PROPERTY_IDX ossl_property_string(CRYPTO_RWLOCK * lock, PROP_TABLE * t, OSSL_PROPERTY_IDX * pidx, const char * s)
{
...
{
	struct find_str_st * findstr = (struct find_str_st *)vfindstr;
...
static const char * ossl_property_str(int name, OSSL_LIB_CTX * ctx, OSSL_PROPERTY_IDX idx)
{
	...
	PROPERTY_STRING_DATA * propdata = (PROPERTY_STRING_DATA *)ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_PROPERTY_STRING_INDEX, &property_string_data_method);
	if(propdata == NULL)
		return NULL;
	findstr.str = NULL;
	findstr.idx = idx;
...
OSSL_PROPERTY_IDX ossl_property_name(OSSL_LIB_CTX * ctx, const char * s, int create)
{
	...
	if(propdata == NULL)
		return 0;
	return ossl_property_string(propdata->lock, propdata->prop_names, create ? &propdata->prop_name_idx : NULL, s);
}
...
OSSL_PROPERTY_IDX ossl_property_value(OSSL_LIB_CTX * ctx, const char * s, int create)
{
	...
	if(propdata == NULL)
		return 0;
	return ossl_property_string(propdata->lock, propdata->prop_values, create ? &propdata->prop_value_idx : NULL, s);
}
