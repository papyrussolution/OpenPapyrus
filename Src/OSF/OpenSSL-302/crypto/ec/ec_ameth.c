	...
	switch(op) {
		...
		case ASN1_PKEY_CTRL_SET1_TLS_ENCPT:
		    ...
		    if(!ossl_assert(evp_pkey_is_legacy(pkey)))
			    return 0;
		    return EC_KEY_oct2key(evp_pkey_get0_EC_KEY_int(pkey), (const uchar *)arg2, arg1, NULL);
		case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
		    return EC_KEY_key2buf(EVP_PKEY_get0_EC_KEY(pkey), POINT_CONVERSION_UNCOMPRESSED, (uchar **)arg2, NULL);
...
static int ec_pkey_export_to(const EVP_PKEY * from, void * to_keydata, OSSL_FUNC_keymgmt_import_fn * importer, OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	uchar * pub_key_buf = NULL, * gen_buf = NULL;
	...
	if(pub_point != NULL) {
		...
		if((pub_key_buflen = EC_POINT_point2buf(ecg, pub_point, POINT_CONVERSION_COMPRESSED, &pub_key_buf, bnctx)) == 0 || 
...
{
	int _private = EC_KEY_get0_private_key(x) != NULL;
	return do_EC_KEY_print(bp, x, off, _private ? EC_KEY_PRINT_PRIVATE : EC_KEY_PRINT_PUBLIC);
}
