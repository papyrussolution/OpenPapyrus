...
#pragma hdrstop
/*
 * ECDSA low level APIs are deprecated for public use, but still ok for internal use.
 */
...
static int ecx_pub_encode(X509_PUBKEY * pk, const EVP_PKEY * pkey)
{
	...
	penc = (uchar *)OPENSSL_memdup(ecxkey->pubkey, KEYLEN(pkey));
	if(penc == NULL) {
		ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	if(!X509_PUBKEY_set0_param(pk, OBJ_nid2obj(pkey->ameth->pkey_id), V_ASN1_UNDEF, NULL, penc, KEYLEN(pkey))) {
		OPENSSL_free(penc);
		ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
		return 0;
	}
...
static int ecx_pub_decode(EVP_PKEY * pkey, const X509_PUBKEY * pubkey)
{
	...
	if(!X509_PUBKEY_get0_param(NULL, &p, &pklen, &palg, pubkey))
		return 0;
	ecx = ossl_ecx_key_op(palg, p, pklen, pkey->ameth->pkey_id, KEY_OP_PUBLIC, NULL, NULL);
	if(ecx) {
		ret = 1;
...
static int ecx_priv_decode_ex(EVP_PKEY * pkey, const PKCS8_PRIV_KEY_INFO * p8, OSSL_LIB_CTX * libctx, const char * propq)
{
	int ret = 0;
...
static int ecx_ctrl(EVP_PKEY * pkey, int op, long arg1, void * arg2)
{
	switch(op) {
		case ASN1_PKEY_CTRL_SET1_TLS_ENCPT: {
		    ECX_KEY * ecx = ossl_ecx_key_op(NULL, (const uchar *)arg2, arg1, pkey->ameth->pkey_id, KEY_OP_PUBLIC, NULL, NULL);
		    if(ecx) {
			    EVP_PKEY_assign(pkey, pkey->ameth->pkey_id, ecx);
			    return 1;
		    }
		...
		case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
		    if(pkey->pkey.ecx != NULL) {
			    ...
			    *ppt = (uchar *)OPENSSL_memdup(pkey->pkey.ecx->pubkey, KEYLEN(pkey));
			    if(*ppt != NULL)
				    return KEYLEN(pkey);
		    }
		    return 0;
    ...
    OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	if(key->privkey != NULL) {
		if(!OSSL_PARAM_BLD_push_octet_string(tmpl, OSSL_PKEY_PARAM_PRIV_KEY, key->privkey, key->keylen))
			goto err;
...
static int validate_ecx_derive(EVP_PKEY_CTX * ctx, uchar * key,
    size_t * keylen,
    const uchar ** privkey,
    const uchar ** pubkey)
{
	...
	ecxkey = (const ECX_KEY *)evp_pkey_get_legacy(ctx->pkey);
	peerkey = (const ECX_KEY *)evp_pkey_get_legacy(ctx->peerkey);
	if(ecxkey == NULL || ecxkey->privkey == NULL) {
		ERR_raise(ERR_LIB_EC, EC_R_INVALID_PRIVATE_KEY);
		return 0;
	}
...
static int pkey_ecd_digestsign25519(EVP_MD_CTX * ctx, uchar * sig, size_t * siglen, const uchar * tbs, size_t tbslen)
{
	const ECX_KEY * edkey = (const ECX_KEY *)evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
	if(!sig) {
	...
	if(ossl_ed25519_sign(sig, tbs, tbslen, edkey->pubkey, edkey->privkey, NULL, NULL) == 0)
		return 0;
	*siglen = ED25519_SIGSIZE;
	return 1;
}

static int pkey_ecd_digestsign448(EVP_MD_CTX * ctx, uchar * sig, size_t * siglen, const uchar * tbs, size_t tbslen)
{
	const ECX_KEY * edkey = (const ECX_KEY *)evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
	if(!sig) {
	...
	if(ossl_ed448_sign(edkey->libctx, sig, tbs, tbslen, edkey->pubkey, edkey->privkey, NULL, 0, edkey->propq) == 0)
		return 0;
	*siglen = ED448_SIGSIZE;
	return 1;
}

static int pkey_ecd_digestverify25519(EVP_MD_CTX * ctx, const uchar * sig, size_t siglen, const uchar * tbs, size_t tbslen)
{
	const ECX_KEY * edkey = (const ECX_KEY *)evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
	if(siglen != ED25519_SIGSIZE)
		return 0;
...
static int pkey_ecd_digestverify448(EVP_MD_CTX * ctx, const uchar * sig, size_t siglen, const uchar * tbs, size_t tbslen)
{
	const ECX_KEY * edkey = (const ECX_KEY *)evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
	if(siglen != ED448_SIGSIZE)
		return 0;
	return ossl_ed448_verify(edkey->libctx, tbs, tbslen, sig, edkey->pubkey, NULL, 0, edkey->propq);
}
...
static int s390x_pkey_ecx_derive25519(EVP_PKEY_CTX * ctx, uchar * key,
    size_t * keylen)
{
	const uchar * privkey, * pubkey;
...
static int s390x_pkey_ecx_derive448(EVP_PKEY_CTX * ctx, uchar * key,
    size_t * keylen)
{
	const uchar * privkey, * pubkey;
