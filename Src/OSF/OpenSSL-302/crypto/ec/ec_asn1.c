...
static int ec_asn1_group2curve(const EC_GROUP * group, X9_62_CURVE * curve)
{
	...
	if((a_buf = (uchar *)OPENSSL_malloc(len)) == NULL
	    || (b_buf = (uchar *)OPENSSL_malloc(len)) == NULL) {
		ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
#else
	{
		X9_62_CHARACTERISTIC_TWO * char_two = params->fieldID->p.char_two;
...
int i2d_ECPrivateKey(const EC_KEY * a, uchar ** out)
{
	int ret = 0, ok = 0;
...
int i2d_ECParameters(const EC_KEY * a, uchar ** out)
{
	if(!a) {
		ERR_raise(ERR_LIB_EC, ERR_R_PASSED_NULL_PARAMETER);
		return 0;
...
ECDSA_SIG * d2i_ECDSA_SIG(ECDSA_SIG ** psig, const uchar ** ppin, long len)
{
	ECDSA_SIG * sig;
	if(len < 0)
		return NULL;
...
int i2d_ECDSA_SIG(const ECDSA_SIG * sig, uchar ** ppout)
{
	BUF_MEM * buf = NULL;
	size_t encoded_len;
	WPACKET pkt;
