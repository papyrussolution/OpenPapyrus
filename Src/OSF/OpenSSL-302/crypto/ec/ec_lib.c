...
#pragma hdrstop
/*
 * EC_GROUP low level APIs are deprecated for public use, but still ok for internal use.
 */
...
EC_GROUP * ossl_ec_group_new_ex(OSSL_LIB_CTX * libctx, const char * propq, const EC_METHOD * meth)
{
	EC_GROUP * ret;
	if(meth == NULL) {
...
int EC_GROUP_copy(EC_GROUP * dest, const EC_GROUP * src)
{
	...
	if(src->seed) {
		...
		if((dest->seed = (uchar *)OPENSSL_malloc(src->seed_len)) == NULL) {
			ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
			return 0;
		}
...
static EC_GROUP * group_new_from_name(const OSSL_PARAM * p, OSSL_LIB_CTX * libctx, const char * propq)
{
	int ok = 0, nid;
...
int ossl_ec_group_set_params(EC_GROUP * group, const OSSL_PARAM params[])
{
	...
	if(p) {
		...
		if(p->data_type != OSSL_PARAM_OCTET_STRING || !EC_GROUP_set_seed(group, (const uchar *)p->data, p->data_size)) {
    ...
    OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	if(strcasecmp((const char *)ptmp->data, SN_X9_62_prime_field) == 0) {
		is_prime_field = 1;
	}
	else if(strcasecmp((const char *)ptmp->data, SN_X9_62_characteristic_two_field) == 0) {
		is_prime_field = 0;
	}
	else {
		/* Invalid field */
	...
	if(ptmp) {
		...
		if(!EC_GROUP_set_seed(group, (const uchar *)ptmp->data, ptmp->data_size))
			goto err;
	}
	/* generator base point */
	ptmp = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_EC_GENERATOR);
	if(ptmp == NULL || ptmp->data_type != OSSL_PARAM_OCTET_STRING) {
	...
	if((point = EC_POINT_new(group)) == NULL)
		goto err;
	EC_GROUP_set_point_conversion_form(group, (point_conversion_form_t)(buf[0] & ~0x01));
