 ...
 * Must have extra <= 1
 */
static void sc_subx(curve448_scalar_t out, const c448_word_t accum[C448_SCALAR_LIMBS], const curve448_scalar_t sub, const curve448_scalar_t p, c448_word_t extra)
{
...
static void sc_montmul(curve448_scalar_t out, const curve448_scalar_t a, const curve448_scalar_t b)
{
	uint i, j;
...
void ossl_curve448_scalar_mul(curve448_scalar_t out, const curve448_scalar_t a, const curve448_scalar_t b)
{
	sc_montmul(out, a, b);
	sc_montmul(out, out, sc_r2);
}

void ossl_curve448_scalar_sub(curve448_scalar_t out, const curve448_scalar_t a, const curve448_scalar_t b)
{
	sc_subx(out, a->limb, b, sc_p, 0);
}

void ossl_curve448_scalar_add(curve448_scalar_t out, const curve448_scalar_t a, const curve448_scalar_t b)
{
	c448_dword_t chain = 0;
	uint i;
	for(i = 0; i < C448_SCALAR_LIMBS; i++) {
...
static ossl_inline void scalar_decode_short(curve448_scalar_t s, const uchar * ser, size_t nbytes)
{
	size_t i, j, k = 0;
	for(i = 0; i < C448_SCALAR_LIMBS; i++) {
		c448_word_t out = 0;
...
c448_error_t ossl_curve448_scalar_decode(curve448_scalar_t s, const uchar ser[C448_SCALAR_BYTES])
{
	uint i;
...
void ossl_curve448_scalar_encode(uchar ser[C448_SCALAR_BYTES], const curve448_scalar_t s)
{
	uint i, j, k = 0;
	for(i = 0; i < C448_SCALAR_LIMBS; i++) {
