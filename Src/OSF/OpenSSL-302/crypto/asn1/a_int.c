...
static size_t c2i_ibuf(uchar * b, int * pneg, const uchar * p, size_t plen)
{
	int neg, pad;
...
int ossl_i2c_ASN1_INTEGER(ASN1_INTEGER * a, uchar ** pp)
{
...
ASN1_INTEGER * ossl_c2i_ASN1_INTEGER(ASN1_INTEGER ** a, const uchar ** pp,
    long len)
{
...
static ASN1_STRING * bn_to_asn1_string(const BIGNUM * bn, ASN1_STRING * ai, int atype)
{
	ASN1_INTEGER * ret;
	int len;
	if(ai == NULL) {
...
int ossl_i2c_uint64_int(uchar * p, uint64_t r, int neg)
{
	unsigned char buf[sizeof(uint64_t)];
	size_t off;
