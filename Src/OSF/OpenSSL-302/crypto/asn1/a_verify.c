...
int ASN1_verify(i2d_of_void * i2d, X509_ALGOR * a, ASN1_BIT_STRING * signature, char * data, EVP_PKEY * pkey)
{
	...
	buf_in = (uchar *)OPENSSL_malloc((unsigned int)inl);
	if(buf_in == NULL) {
		ERR_raise(ERR_LIB_ASN1, ERR_R_MALLOC_FAILURE);
		goto err;
	}
	...
	ret = -1;
	if(EVP_VerifyFinal(ctx, (uchar *)signature->data, (unsigned int)signature->length, pkey) <= 0) {
		ERR_raise(ERR_LIB_ASN1, ERR_R_EVP_LIB);
		ret = 0;
		goto err;
	}
...
int ASN1_item_verify_ex(const ASN1_ITEM * it, const X509_ALGOR * alg, const ASN1_BIT_STRING * signature, const void * data,
    const ASN1_OCTET_STRING * id, EVP_PKEY * pkey, OSSL_LIB_CTX * libctx, const char * propq)
{
	EVP_MD_CTX * ctx;
	int rv = -1;
...
int ASN1_item_verify_ctx(const ASN1_ITEM * it, const X509_ALGOR * alg, const ASN1_BIT_STRING * signature, const void * data, EVP_MD_CTX * ctx)
{
	...
	inl = ASN1_item_i2d((const ASN1_VALUE *)data, &buf_in, it);
	if(inl <= 0) {
		ERR_raise(ERR_LIB_ASN1, ERR_R_INTERNAL_ERROR);
		goto err;
	}
