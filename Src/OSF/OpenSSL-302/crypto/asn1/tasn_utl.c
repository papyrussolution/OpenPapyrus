...
int ossl_asn1_enc_save(ASN1_VALUE ** pval, const uchar * in, int inlen,
    const ASN1_ITEM * it)
{
	ASN1_ENCODING * enc = asn1_get_enc_ptr(pval, it);
	...
	if(inlen <= 0)
		return 0;
	if((enc->enc = (uchar *)OPENSSL_malloc(inlen)) == NULL) {
		ERR_raise(ERR_LIB_ASN1, ERR_R_MALLOC_FAILURE);
		return 0;
	}
...
int ossl_asn1_enc_restore(int * len, uchar ** out, const ASN1_VALUE ** pval,
    const ASN1_ITEM * it)
{
