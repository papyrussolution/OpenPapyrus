...
static int asn1_type_get_int_oct(ASN1_OCTET_STRING * oct, int32_t anum,
    long * num, uchar * data, int max_len)
{
...
int ASN1_TYPE_set_int_octetstring(ASN1_TYPE * a, long num, uchar * data,
    int len)
{
	asn1_int_oct atmp;
	ASN1_OCTET_STRING oct;
...
int ossl_asn1_type_get_octetstring_int(const ASN1_TYPE * a, long * num, uchar * data, int max_len)
{
	asn1_oct_int * atmp = NULL;
	int ret = -1;
	if((a->type != V_ASN1_SEQUENCE) || (a->value.sequence == NULL))
		goto err;
	atmp = (asn1_oct_int *)ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(asn1_oct_int), a);
	if(atmp == NULL)
		goto err;
	ret = asn1_type_get_int_oct(atmp->oct, atmp->num, num, data, max_len);
	if(ret == -1) {
