    ...
    OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	ret = ASN1_item_d2i_ex((ASN1_VALUE **)x, &p, len, it, libctx, propq);
err:
	BUF_MEM_free(b);
	return ret;
}
...
void * ASN1_item_d2i_fp_ex(const ASN1_ITEM * it, FILE * in, void * x, OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	BIO_set_fp(b, in, BIO_NOCLOSE);
	ret = (char*)ASN1_item_d2i_bio_ex(it, b, x, libctx, propq);
	BIO_free(b);
	return ret;
}
...
int asn1_d2i_read_bio(BIO * in, BUF_MEM ** pb)
{
	...
	for(;;) {
		...
		if(inf & 0x80) {
			unsigned long e = ERR_GET_REASON(ERR_peek_last_error());
