...
int ASN1_sign(i2d_of_void * i2d, X509_ALGOR * algor1, X509_ALGOR * algor2, ASN1_BIT_STRING * signature, char * data, EVP_PKEY * pkey, const EVP_MD * type)
{
	...
	inll = (size_t)inl;
	buf_in = (uchar *)OPENSSL_malloc(inll);
	outll = outl = EVP_PKEY_get_size(pkey);
	buf_out = (uchar *)OPENSSL_malloc(outll);
	...
	if(!EVP_SignInit_ex(ctx, type, NULL)
	    || !EVP_SignUpdate(ctx, (uchar *)buf_in, inl)
	    || !EVP_SignFinal(ctx, (uchar *)buf_out,
...
int ASN1_item_sign_ctx(const ASN1_ITEM * it, X509_ALGOR * algor1, X509_ALGOR * algor2, ASN1_BIT_STRING * signature, const void * data, EVP_MD_CTX * ctx)
{
	...
	if(pkey->ameth == NULL) {
		...
		size_t aid_len = 0;
		if(pctx == NULL || !EVP_PKEY_CTX_IS_SIGNATURE_OP(pctx)) {
			ERR_raise(ERR_LIB_ASN1, ASN1_R_CONTEXT_NOT_INITIALISED);
			goto err;
		}
	...
	buf_len = ASN1_item_i2d((const ASN1_VALUE *)data, &buf_in, it);
	if(buf_len <= 0) {
		outl = 0;
		ERR_raise(ERR_LIB_ASN1, ERR_R_INTERNAL_ERROR);
		goto err;
