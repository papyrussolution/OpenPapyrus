...
static int asn1_item_embed_d2i(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it, int tag, int aclass, char opt, ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq);
...
static int asn1_template_ex_d2i(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_TEMPLATE * tt, char opt, ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq);
static int asn1_template_noexp_d2i(ASN1_VALUE ** val, const uchar ** in, long len, const ASN1_TEMPLATE * tt, char opt, ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq);
 ...
 */
static int asn1_item_ex_d2i_intern(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it, int tag, 
	int aclass, char opt, ASN1_TLC * ctx, OSSL_LIB_CTX * libctx, const char * propq)
{
	int rv;
...
int ASN1_item_ex_d2i(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it, int tag, int aclass, char opt, ASN1_TLC * ctx)
{
	return asn1_item_ex_d2i_intern(pval, in, len, it, tag, aclass, opt, ctx, NULL, NULL);
}

ASN1_VALUE * ASN1_item_d2i_ex(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it, OSSL_LIB_CTX * libctx, const char * propq)
{
	ASN1_TLC c;
	ASN1_VALUE * ptmpval = NULL;
	SETIFZQ(pval, &ptmpval);
	asn1_tlc_clear_nc(&c);
	if(asn1_item_ex_d2i_intern(pval, in, len, it, -1, 0, 0, &c, libctx, propq) > 0)
		return *pval;
	return NULL;
}

ASN1_VALUE * ASN1_item_d2i(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it)
{
 ...
 */
static int asn1_item_embed_d2i(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it,
    int tag, int aclass, char opt, ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	switch(it->itype) {
		...
		case ASN1_ITYPE_EXTERN:
		    ...
		    if(ef->asn1_ex_d2i_ex)
			    return ef->asn1_ex_d2i_ex(pval, in, len, it, tag, aclass, opt, ctx, libctx, propq);
		...
		case ASN1_ITYPE_CHOICE:
		    ...
		    for(i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
			     ...
			     */
			    ret = asn1_template_ex_d2i(pchptr, &p, len, tt, 1, ctx, depth, libctx, propq);
		...
		case ASN1_ITYPE_SEQUENCE:
		    ...
		    if(*pval == NULL && !ossl_asn1_item_ex_new_intern(pval, it, libctx, propq)) {
			    ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
			    goto err;
		    }
		    if(asn1_cb && !asn1_cb(ASN1_OP_D2I_PRE, pval, it, NULL))
		    ...
		    for(i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
			    if(tt->flags & ASN1_TFLG_ADB_MASK) {
				    ASN1_VALUE ** pseqval;
				    const ASN1_TEMPLATE * seqtt = ossl_asn1_do_adb(*pval, tt, 0);
				    if(seqtt == NULL)
					    continue;
		    ...
		    for(i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
			    ASN1_VALUE ** pseqval;
			    const ASN1_TEMPLATE * seqtt = ossl_asn1_do_adb(*pval, tt, 1);
			    if(seqtt == NULL)
				    goto err;
			    pseqval = ossl_asn1_get_field_ptr(pval, seqtt);
			     ...
			     */
			    ret = asn1_template_ex_d2i(pseqval, &p, len, seqtt, isopt, ctx, depth, libctx, propq);
			    if(!ret) {
				    errtt = seqtt;
				    goto err;
			    }
		    ...
		    for(; i < it->tcount; tt++, i++) {
			    ...
			    if(seqtt->flags & ASN1_TFLG_OPTIONAL) {
				    ASN1_VALUE ** pseqval = ossl_asn1_get_field_ptr(pval, seqtt);
				    ossl_asn1_template_free(pseqval, seqtt);
			    }
			    else {
				    errtt = seqtt;
 ...
 */
static int asn1_template_ex_d2i(ASN1_VALUE ** val, const uchar ** in, long inlen, const ASN1_TEMPLATE * tt, char opt, ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq)
{
	int flags, aclass;
	int ret;
	long len;
	...
	if(flags & ASN1_TFLG_EXPTAG) {
		...
		return asn1_template_noexp_d2i(val, in, inlen, tt, opt, ctx, depth, libctx, propq);
	*in = p;
	return 1;
err:
	return 0;
...
static int asn1_template_noexp_d2i(ASN1_VALUE ** val, const uchar ** in, long len, const ASN1_TEMPLATE * tt, char opt,
    ASN1_TLC * ctx, int depth, OSSL_LIB_CTX * libctx, const char * propq)
{
	int flags, aclass;
	int ret;
	ASN1_VALUE * tval;
...
static int asn1_d2i_ex_primitive(ASN1_VALUE ** pval, const uchar ** in, long inlen, const ASN1_ITEM * it, int tag, int aclass, char opt, ASN1_TLC * ctx)
{
	int ret = 0, utype;
	long plen;
 ...
 */
static int asn1_check_tlen(long * olen, int * otag, uchar * oclass, char * inf, char * cst,
    const uchar ** in, long len, int exptag, int expclass, char opt, ASN1_TLC * ctx)
{
	int i;
	int ptag, pclass;
	long plen;
	...
	if(i & 1)
		plen = len - (p - q);
	ASSIGN_PTR(inf, (i & 1));
	ASSIGN_PTR(cst, (i & V_ASN1_CONSTRUCTED));
	ASSIGN_PTR(olen, plen);
	ASSIGN_PTR(oclass, pclass);
	ASSIGN_PTR(otag, ptag);
	*in = p;
	return 1;
err:
	asn1_tlc_clear(ctx);
