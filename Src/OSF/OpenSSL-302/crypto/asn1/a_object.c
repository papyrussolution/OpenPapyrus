...
int i2d_ASN1_OBJECT(const ASN1_OBJECT * a, uchar ** pp)
{
	...
	if(pp == NULL || objsize == -1)
		return objsize;
	if(*pp == NULL) {
		if((p = allocated = (uchar *)OPENSSL_malloc(objsize)) == NULL) {
...
ASN1_OBJECT * ossl_c2i_ASN1_OBJECT(ASN1_OBJECT ** a, const uchar ** pp,
    long len)
{
...
{
	ASN1_OBJECT * ret = (ASN1_OBJECT *)OPENSSL_zalloc(sizeof(*ret));
	if(!ret) {
		ERR_raise(ERR_LIB_ASN1, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
