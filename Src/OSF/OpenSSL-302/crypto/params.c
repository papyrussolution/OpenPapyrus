...
{
	return p && p->return_size != OSSL_PARAM_UNMODIFIED;
}
...
{
	const uchar * n = (const unsigned char*)number;
	DECLARE_IS_ENDIAN;
	return 0x80 & (IS_BIG_ENDIAN ? n[0] : n[s - 1]);
}

/* Check that all the bytes specified match the expected sign byte */
static int check_sign_bytes(const uchar * p, size_t n, unsigned char s)
{
...
{
	return copy_integer((uchar *)dest, dest_len, (const uchar *)src, src_len, is_negative(src, src_len) ? 0xff : 0, 1);
}
...
{
	return copy_integer((uchar *)dest, dest_len, (const uchar *)src, src_len, 0, 1);
}
...
static int unsigned_from_signed(void * dest, size_t dest_len, const void * src, size_t src_len)
{
	if(is_negative(src, src_len))
		return 0;
	return copy_integer((uchar *)dest, dest_len, (const uchar *)src, src_len, 0, 0);
}
...
int OSSL_PARAM_get_int(const OSSL_PARAM * p, int * val)
{
	...
	switch(sizeof(int)) {
		case sizeof(int32_t): return OSSL_PARAM_get_int32(p, (int32_t*)val);
		case sizeof(int64_t): return OSSL_PARAM_get_int64(p, (int64_t*)val);
	}
#endif
...
#ifndef OPENSSL_SMALL_FOOTPRINT
	switch(sizeof(int)) {
		case sizeof(int32_t): return OSSL_PARAM_set_int32(p, (int32_t)val);
		case sizeof(int64_t): return OSSL_PARAM_set_int64(p, (int64_t)val);
	}
#endif
...
int OSSL_PARAM_get_uint(const OSSL_PARAM * p, unsigned int * val)
{
	...
	switch(sizeof(unsigned int)) {
		case sizeof(uint32_t): return OSSL_PARAM_get_uint32(p, (uint32_t*)val);
		case sizeof(uint64_t): return OSSL_PARAM_get_uint64(p, (uint64_t*)val);
	}
#endif
...
#ifndef OPENSSL_SMALL_FOOTPRINT
	switch(sizeof(unsigned int)) {
		case sizeof(uint32_t): return OSSL_PARAM_set_uint32(p, (uint32_t)val);
		case sizeof(uint64_t): return OSSL_PARAM_set_uint64(p, (uint64_t)val);
	}
#endif
...
int OSSL_PARAM_get_BN(const OSSL_PARAM * p, BIGNUM ** val)
{
	...
	if(val == NULL || p == NULL || p->data_type != OSSL_PARAM_UNSIGNED_INTEGER)
		return 0;
	b = BN_native2bn((const uchar *)p->data, (int)p->data_size, *val);
	if(b) {
		*val = b;
		return 1;
	}
...
int OSSL_PARAM_set_BN(OSSL_PARAM * p, const BIGNUM * val)
{
	...
	if(p->data_size >= bytes) {
		...
		return BN_bn2nativepad(val, (uchar *)p->data, p->data_size) >= 0;
	}
	return 0;
}

OSSL_PARAM OSSL_PARAM_construct_BN(const char * key, uchar * buf,
    size_t bsize)
{
    ...
    unsigned int type)
{
	...
	if(val == NULL)
		return 1;
	if(*val == NULL) {
		char * const q = (char * const)OPENSSL_malloc(alloc_sz);
		if(q == NULL)
			return 0;
		*val = q;
		*max_len = alloc_sz;
