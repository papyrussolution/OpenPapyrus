...
static int ossl_param_buf_alloc(OSSL_PARAM_BUF * out, size_t extra_blocks, int is_secure)
{
	...
	out->alloc = (OSSL_PARAM_ALIGNED_BLOCK *)(is_secure ? OPENSSL_secure_zalloc(sz) : OPENSSL_zalloc(sz));
	if(out->alloc == NULL) {
...
OSSL_PARAM * OSSL_PARAM_merge(const OSSL_PARAM * p1, const OSSL_PARAM * p2)
{
	...
	/* Copy p1 to list1 */
	if(p1) {
		for(p = p1; p->key && list1_sz < OSSL_PARAM_MERGE_LIST_MAX; p++)
			list1[list1_sz++] = p;
	}
	list1[list1_sz] = NULL;
	/* copy p2 to a list2 */
	if(p2) {
		for(p = p2; p->key && list2_sz < OSSL_PARAM_MERGE_LIST_MAX; p++)
			list2[list2_sz++] = p;
	}
	list2[list2_sz] = NULL;
	...
	params = (OSSL_PARAM *)OPENSSL_zalloc((list1_sz + list2_sz + 1) * sizeof(*p1));
	if(!params) {
