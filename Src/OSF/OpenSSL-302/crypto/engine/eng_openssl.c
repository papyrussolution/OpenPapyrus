...
#pragma hdrstop
/*
 * RC4 and SHA-1 low level APIs and EVP _meth_ APISs are deprecated for public use, but still ok for internal use.
 */
...
static int openssl_ciphers(ENGINE * e, const EVP_CIPHER ** cipher, const int ** nids, int nid)
{
	...
	if(nid == NID_rc4)
		...
		*cipher = test_r4_40_cipher();
		...
		slfprintf_stderr("(TEST_ENG_OPENSSL_RC4) returning NULL for nid %d\n", nid);
#endif
		*cipher = NULL;
		return 0;
	}
...
#endif
	return SHA1_Init((SHA_CTX*)EVP_MD_CTX_get0_md_data(ctx));
}
...
#endif
	return SHA1_Update((SHA_CTX*)EVP_MD_CTX_get0_md_data(ctx), data, count);
}
...
#endif
	return SHA1_Final(md, (SHA_CTX*)EVP_MD_CTX_get0_md_data(ctx));
}
...
static int ossl_hmac_signctx(EVP_PKEY_CTX * ctx, uchar * sig, size_t * siglen, EVP_MD_CTX * mctx)
{
	unsigned int hlen;
	OSSL_HMAC_PKEY_CTX * hctx = EVP_PKEY_CTX_get_data(ctx);
