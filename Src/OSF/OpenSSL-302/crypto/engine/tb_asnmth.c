...
const EVP_PKEY_ASN1_METHOD * ENGINE_get_pkey_asn1_meth_str(ENGINE * e, const char * str, int len)
{
	...
	for(i = 0; i < nidcount; i++) {
		...
		if(ameth != NULL && ((int)strlen(ameth->pem_str) == len) && strncasecmp(ameth->pem_str, str, len) == 0)
			return ameth;
	}
	return NULL;
}
...
static void look_str_cb(int nid, STACK_OF(ENGINE) * sk, ENGINE * def, void * arg)
{
	...
	for(i = 0; i < sk_ENGINE_num(sk); i++) {
		...
		if(ameth != NULL && ((int)strlen(ameth->pem_str) == lk->len) && strncasecmp(ameth->pem_str, lk->str, lk->len) == 0) {
			lk->e = e;
			lk->ameth = ameth;
			return;
		}
