...
int CRYPTO_secure_allocated(const void * ptr)
{
	 ...
	 */
	return sh_allocated((const char *)ptr);
#else
	return 0;
#endif /* OPENSSL_NO_SECURE_MEMORY */
}
...
static int sh_init(size_t size, size_t minsize)
{
	...
	sh.bittable = (uchar *)OPENSSL_zalloc(sh.bittable_size >> 3);
	OPENSSL_assert(sh.bittable != NULL);
	if(sh.bittable == NULL)
		goto err;

	sh.bitmalloc = (uchar *)OPENSSL_zalloc(sh.bittable_size >> 3);
	...
	if(sh.map_result == MAP_FAILED)
		goto err;
#else
	sh.map_result = (char *)VirtualAlloc(NULL, sh.map_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
