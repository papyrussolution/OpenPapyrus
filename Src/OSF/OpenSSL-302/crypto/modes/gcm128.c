...
extern "C" void gcm_gmult_4bit_mmx(u64 Xi[2], const u128 Htable[16]);
extern "C" void gcm_ghash_4bit_mmx(u64 Xi[2], const u128 Htable[16], const u8 * inp, size_t len);
extern "C" void gcm_gmult_4bit_x86(u64 Xi[2], const u128 Htable[16]);
extern "C" void gcm_ghash_4bit_x86(u64 Xi[2], const u128 Htable[16], const u8 * inp, size_t len);
#endif
...
void CRYPTO_gcm128_setiv(GCM128_CONTEXT * ctx, const uchar * iv,
    size_t len)
{
	DECLARE_IS_ENDIAN;
	unsigned int ctr;
...
int CRYPTO_gcm128_aad(GCM128_CONTEXT * ctx, const uchar * aad,
    size_t len)
{
	size_t i;
	unsigned int n;
...
int CRYPTO_gcm128_finish(GCM128_CONTEXT * ctx, const uchar * tag, size_t len)
{
	...
	void (*gcm_gmult_p)(u64 Xi[2], const u128 Htable[16]) = ctx->gmult;
#if defined(GHASH) && !defined(OPENSSL_SMALL_FOOTPRINT)
	void (*gcm_ghash_p)(u64 Xi[2], const u128 Htable[16], const u8 *inp, size_t len) = ctx->ghash;
#endif
#endif
#if defined(GHASH) && !defined(OPENSSL_SMALL_FOOTPRINT)
	u128 bitlen;
	...
	if(mres) {
		...
		memzero(ctx->Xn + mres, blocks - mres);
		mres = blocks;
		if(mres == sizeof(ctx->Xn)) {
			GHASH(ctx, ctx->Xn, mres);
			mres = 0;
