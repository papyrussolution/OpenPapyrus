...
typedef int fixup_args_fn (enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx);
typedef int cleanup_args_fn (enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx);
...
struct translation_ctx_st {
	...
	const char * ctrl_str;
	int ishex;
	int p1; /* the ctrl-style int argument. */
	void * p2; /* the ctrl-style void* argument. */
	size_t sz; /* a size, for passing back the |p2| size where applicable */
	OSSL_PARAM * params; /* pointer to the OSSL_PARAM-style params array. */
	/*-
    ...
    struct translation_ctx_st * ctx)
{
	...
	switch(state) {
				...
				*ctx->params = OSSL_PARAM_construct_int(translation->param_key,
				...
				if(ctx->p2 != NULL) {
					if(ctx->action_type == SET) {
						ctx->buflen = BN_num_bytes((const BIGNUM *)ctx->p2);
						if((ctx->allocated_buf = OPENSSL_malloc(ctx->buflen)) == NULL) {
							ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
							return 0;
						}
						if(BN_bn2nativepad((const BIGNUM *)ctx->p2, (uchar *)ctx->allocated_buf, ctx->buflen) < 0) {
							OPENSSL_free(ctx->allocated_buf);
							ctx->allocated_buf = NULL;
							return 0;
						}
						*ctx->params = OSSL_PARAM_construct_BN(translation->param_key, (uchar *)ctx->allocated_buf, ctx->buflen);
					}
					else {
						/*
						 ...
						 */
						ERR_raise_data(ERR_LIB_EVP, ERR_R_UNSUPPORTED, "[action:%d, state:%d] trying to get a BIGNUM via ctrl call", ctx->action_type, state);
						return 0;
					}
				}
				else {
					*ctx->params = OSSL_PARAM_construct_uint(translation->param_key, (unsigned int*)&ctx->p1);
				}
				break;
			    case OSSL_PARAM_UTF8_STRING:
				*ctx->params = OSSL_PARAM_construct_utf8_string(translation->param_key, (char*)ctx->p2, (size_t)ctx->p1);
				break;
			    case OSSL_PARAM_UTF8_PTR:
				*ctx->params = OSSL_PARAM_construct_utf8_ptr(translation->param_key, (char **)ctx->p2, (size_t)ctx->p1);
				break;
			    case OSSL_PARAM_OCTET_STRING:
				*ctx->params = OSSL_PARAM_construct_octet_string(translation->param_key, ctx->p2, (size_t)ctx->p1);
				break;
			    case OSSL_PARAM_OCTET_PTR:
				*ctx->params = OSSL_PARAM_construct_octet_ptr(translation->param_key, (void **)ctx->p2, (size_t)ctx->p1);
				break;
		    }
		    break;
		...
		case PRE_CTRL_STR_TO_PARAMS:
	    {
		    ...
		    const char * orig_value = (const char *)ctx->p2;
		    const OSSL_PARAM * settable = NULL;
		    int exists = 0;
		    /* Only setting is supported here */
		    if(ctx->action_type != SET) {
			    ERR_raise_data(ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED, "[action:%d, state:%d] only setting allowed", ctx->action_type, state);
			    return 0;
		    }
		    /*
		    ...
		    if(!OSSL_PARAM_allocate_from_text(ctx->params, settable, tmp_ctrl_str, (const char *)ctx->p2, strlen((const char *)ctx->p2), &exists)) {
			    if(!exists) {
				    ERR_raise_data(ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED, "[action:%d, state:%d] name=%s, value=%s",
						ctx->action_type, state, orig_ctrl_str, orig_value);
				    return -2;
			    }
			    return 0;
		    }
		...
		case PRE_PARAMS_TO_CTRL:
	    {
		    ...
		    if(state == PRE_PARAMS_TO_CTRL && ctx->action_type == SET) {
			    ...
			    switch(translation->param_data_type) {
					...
					if(ctx->p2 != NULL) {
						...
						if(!OSSL_PARAM_get_BN(ctx->params, (BIGNUM **)ctx->p2))
							return 0;
					}
					else {
					...
					return 1;
				    case OSSL_PARAM_UTF8_STRING:
					return OSSL_PARAM_get_utf8_string(ctx->params, (char **)ctx->p2, ctx->sz);
				    case OSSL_PARAM_OCTET_STRING:
					return OSSL_PARAM_get_octet_string(ctx->params, (void **)ctx->p2, ctx->sz, &ctx->sz);
				    case OSSL_PARAM_OCTET_PTR:
					return OSSL_PARAM_get_octet_ptr(ctx->params, (const void **)ctx->p2, &ctx->sz);
				    default:
					ERR_raise_data(ERR_LIB_EVP, ERR_R_UNSUPPORTED, "[action:%d, state:%d] unknown OSSL_PARAM data type %d", ctx->action_type, state, translation->param_data_type);
					return 0;
			    }
		    }
			...
			&& ctx->action_type == GET) {
			    ...
			    switch(param_data_type) {
					...
					if(ctx->p2 != NULL) {
						/* BIGNUM passed back */
						return OSSL_PARAM_set_BN(ctx->params, (const BIGNUM *)ctx->p2);
					}
					else {
						/* Normal C unsigned int passed back */
						return OSSL_PARAM_set_uint(ctx->params, (unsigned int)ctx->p1);
					}
					return 0;
				    case OSSL_PARAM_UTF8_STRING:
					return OSSL_PARAM_set_utf8_string(ctx->params, (const char *)ctx->p2);
				    case OSSL_PARAM_OCTET_STRING:
					return OSSL_PARAM_set_octet_string(ctx->params, ctx->p2, size);
				    case OSSL_PARAM_OCTET_PTR:
					return OSSL_PARAM_set_octet_ptr(ctx->params, ctx->p2, size);
				    default:
					ERR_raise_data(ERR_LIB_EVP, ERR_R_UNSUPPORTED, "[action:%d, state:%d] unsupported OSSL_PARAM data type %d",
					    ctx->action_type, state, translation->param_data_type);
					return 0;
			    }
		    }
	    }
...
{
	return EVP_CIPHER_get0_name((EVP_CIPHER *)cipher);
}

static const char * get_md_name(void * md)
{
	return EVP_MD_get0_name((EVP_MD *)md);
}
...
static int fix_cipher_md(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx,
    const char *(*get_name)(void * algo), const void *(*get_algo_by_name)(OSSL_LIB_CTX * libctx, const char * name))
{
	int ret = 1;
	if((ret = default_check(state, translation, ctx)) <= 0)
		return ret;
	if(state == PRE_CTRL_TO_PARAMS && ctx->action_type == GET) {
		 ...
		 */
		ctx->p2 = (char*)(ctx->p2 == NULL ? OBJ_nid2sn(ctx->p1) : get_name(ctx->p2));
	...
	if(state == POST_CTRL_TO_PARAMS && ctx->action_type == GET) {
		 ...
		 */
		*(void**)ctx->orig_p2 = (void*)get_algo_by_name(ctx->pctx->libctx, (const char *)ctx->p2);
		ctx->p1 = 1;
	}
	else if(state == PRE_PARAMS_TO_CTRL && ctx->action_type == SET) {
		ctx->p2 = (void*)get_algo_by_name(ctx->pctx->libctx, (const char *)ctx->p2);
		ctx->p1 = 0;
	}
    ...
    const struct kdf_type_map_st * kdf_type_map)
{
	...
	if((ret = default_fixup_args(state, translation, ctx)) <= 0)
		return ret;
	if((state == POST_CTRL_TO_PARAMS && ctx->action_type == GET) || (state == PRE_PARAMS_TO_CTRL && ctx->action_type == SET)) {
		ctx->p1 = ret = -1;
		/* Convert KDF type strings to numbers */
		for(; kdf_type_map->kdf_type_str != NULL; kdf_type_map++)
			if(strcasecmp((const char *)ctx->p2, kdf_type_map->kdf_type_str) == 0) {
...
static int fix_dh_kdf_type(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
    ...
    struct translation_ctx_st * ctx)
{
	...
	if((state == PRE_CTRL_TO_PARAMS && ctx->action_type == SET)
		 ...
		 */
		OBJ_obj2txt(ctx->name_buf, sizeof(ctx->name_buf), (const ASN1_OBJECT *)ctx->p2, 0);
	...
	if((state == PRE_PARAMS_TO_CTRL && ctx->action_type == SET)
		 ...
		 */
		ctx->p2 = (ASN1_OBJECT*)OBJ_txt2obj((const char *)ctx->p2, 0);
	}
...
static int fix_dh_paramgen_type(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	int ret;
	if((ret = default_check(state, translation, ctx)) <= 0)
		return ret;
...
static int fix_ec_param_enc(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	int ret;
	if((ret = default_check(state, translation, ctx)) <= 0)
		return ret;
	...
	if(state == PRE_PARAMS_TO_CTRL) {
		if(strcmp((const char *)ctx->p2, OSSL_PKEY_EC_ENCODING_EXPLICIT) == 0)
			ctx->p1 = OPENSSL_EC_EXPLICIT_CURVE;
		else if(strcmp((const char *)ctx->p2, OSSL_PKEY_EC_ENCODING_GROUP) == 0)
    ...
    struct translation_ctx_st * ctx)
{
	...
	if((ret = default_fixup_args(state, translation, ctx)) <= 0)
		return ret;
	if(state == PRE_PARAMS_TO_CTRL) {
		ctx->p1 = OBJ_sn2nid((const char *)ctx->p2);
		ctx->p2 = NULL;
	}
	return ret;
}
...
static int fix_ecdh_cofactor(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	/*
    ...
    struct translation_ctx_st * ctx)
{
	...
	if(state == PRE_CTRL_TO_PARAMS && ctx->action_type == GET) {
		...
		for(i = 0; i < SIZEOFARRAY(str_value_map); i++) {
			if(ctx->p1 == (int)str_value_map[i].id)
				break;
		}
		if(i == SIZEOFARRAY(str_value_map)) {
			ERR_raise_data(ERR_LIB_RSA, RSA_R_UNKNOWN_PADDING_TYPE, "[action:%d, state:%d] padding number %d", ctx->action_type, state, ctx->p1);
			return -2;
		}
		/*
		...
		ctx->p2 = const_cast<void *>(str_value_map[i].ptr); // @badcast
		ctx->p1 = strlen((const char *)ctx->p2);
	}
	if((ret = default_fixup_args(state, translation, ctx)) <= 0)
		return ret;
	if((ctx->action_type == SET && state == PRE_PARAMS_TO_CTRL) || (ctx->action_type == GET && state == POST_CTRL_TO_PARAMS)) {
		size_t i;
		for(i = 0; i < SIZEOFARRAY(str_value_map); i++) {
		...
		if(i == SIZEOFARRAY(str_value_map)) {
			ERR_raise_data(ERR_LIB_RSA, RSA_R_UNKNOWN_PADDING_TYPE, "[action:%d, state:%d] padding name %s", ctx->action_type, state, ctx->p1);
			ctx->p1 = ret = -2;
		}
...
static int fix_hkdf_mode(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	static const OSSL_ITEM str_value_map[] = {
	...
	if((ret = default_check(state, translation, ctx)) <= 0)
		return ret;
	if((ctx->action_type == SET && state == PRE_CTRL_TO_PARAMS) || (ctx->action_type == GET && state == POST_PARAMS_TO_CTRL)) {
		size_t i;
		for(i = 0; i < SIZEOFARRAY(str_value_map); i++) {
			if(ctx->p1 == (int)str_value_map[i].id)
				break;
...
static int get_payload_group_name(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	EVP_PKEY * pkey = (EVP_PKEY *)ctx->p2;
	ctx->p2 = NULL;
...
static int get_payload_private_key(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	EVP_PKEY * pkey = (EVP_PKEY *)ctx->p2;
	ctx->p2 = NULL;
...
static int get_payload_public_key(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	EVP_PKEY * pkey = (EVP_PKEY *)ctx->p2;
	uchar * buf = NULL;
	int ret;
	...
	switch(EVP_PKEY_get_base_id(pkey)) {
		...
		case EVP_PKEY_EC:
		    if(ctx->params->data_type == OSSL_PARAM_OCTET_STRING) {
			    ...
			    if(!bnctx)
				    return 0;
			    ctx->sz = EC_POINT_point2buf(ecg, point, POINT_CONVERSION_COMPRESSED, &buf, bnctx);
			    ctx->p2 = buf;
			    BN_CTX_free(bnctx);
			    break;
		    }
...
static int get_dh_dsa_payload_q(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	const BIGNUM * bn = NULL;
	switch(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2)) {
#ifndef OPENSSL_NO_DH
		case EVP_PKEY_DH:
		    bn = DH_get0_q(EVP_PKEY_get0_DH((EVP_PKEY *)ctx->p2));
		    break;
#endif
#ifndef OPENSSL_NO_DSA
		case EVP_PKEY_DSA:
		    bn = DSA_get0_q(EVP_PKEY_get0_DSA((EVP_PKEY *)ctx->p2));
		    break;
#endif
	}
...
static int get_dh_dsa_payload_g(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	const BIGNUM * bn = NULL;
	switch(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2)) {
#ifndef OPENSSL_NO_DH
		case EVP_PKEY_DH:
		    bn = DH_get0_g(EVP_PKEY_get0_DH((EVP_PKEY *)ctx->p2));
		    break;
#endif
#ifndef OPENSSL_NO_DSA
		case EVP_PKEY_DSA:
		    bn = DSA_get0_g(EVP_PKEY_get0_DSA((EVP_PKEY *)ctx->p2));
		    break;
#endif
	}
...
static int get_ec_decoded_from_explicit_params(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	int val = 0;
	EVP_PKEY * pkey = (EVP_PKEY *)ctx->p2;
...
static int get_rsa_payload_n(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	const BIGNUM * bn = NULL;
	if(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2) != EVP_PKEY_RSA)
		return 0;
	bn = RSA_get0_n(EVP_PKEY_get0_RSA((const EVP_PKEY *)ctx->p2));
	return get_payload_bn(state, translation, ctx, bn);
}

static int get_rsa_payload_e(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	const BIGNUM * bn = NULL;
	if(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2) != EVP_PKEY_RSA)
		return 0;
	bn = RSA_get0_e(EVP_PKEY_get0_RSA((const EVP_PKEY *)ctx->p2));
	return get_payload_bn(state, translation, ctx, bn);
}

static int get_rsa_payload_d(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx)
{
	const BIGNUM * bn = NULL;
	if(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2) != EVP_PKEY_RSA)
		return 0;
	bn = RSA_get0_d(EVP_PKEY_get0_RSA((const EVP_PKEY *)ctx->p2));
	return get_payload_bn(state, translation, ctx, bn);
}

static int get_rsa_payload_factor(enum state state, const struct translation_st * translation,
    struct translation_ctx_st * ctx, size_t factornum)
{
	const RSA * r = EVP_PKEY_get0_RSA((const EVP_PKEY *)ctx->p2);
	const BIGNUM * bn = NULL;
	switch(factornum) {
		case 0:
		    bn = RSA_get0_p(r);
    ...
    size_t exponentnum)
{
	...
	switch(exponentnum) {
		...
		default:
	    {
		    ...
		    if(exponentnum - 2 < pnum && RSA_get0_multi_prime_crt_params(r, exps, coeffs))
...
static int get_rsa_payload_coefficient(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx, size_t coefficientnum)
{
	const RSA * r = EVP_PKEY_get0_RSA((const EVP_PKEY *)ctx->p2);
	const BIGNUM * bn = NULL;
	switch(coefficientnum) {
		...
		default:
	    {
		    size_t pnum = RSA_get_multi_prime_extra_count(r);
		    const BIGNUM * exps[10], * coeffs[10];
		    if(coefficientnum - 1 < pnum && RSA_get0_multi_prime_crt_params(r, exps, coeffs))
...
#define IMPL_GET_RSA_PAYLOAD_EXPONENT(n)                                \
	static int get_rsa_payload_e ## n(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx) \
	{                                                                   \
		if(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2) != EVP_PKEY_RSA)              \
			return 0;                                                   \
		return get_rsa_payload_exponent(state, translation, ctx, n - 1); \
	}

#define IMPL_GET_RSA_PAYLOAD_COEFFICIENT(n)                             \
	static int get_rsa_payload_c ## n(enum state state, const struct translation_st * translation, struct translation_ctx_st * ctx) \
	{                                                                   \
		if(EVP_PKEY_get_base_id((EVP_PKEY *)ctx->p2) != EVP_PKEY_RSA)              \
			return 0;                                                   \
		return get_rsa_payload_coefficient(state, translation, ctx, n - 1); \
	}
...
static const struct translation_st * lookup_translation(struct translation_st * tmpl,
...
static const struct translation_st * lookup_evp_pkey_ctx_translation(struct translation_st * tmpl)
{
	return lookup_translation(tmpl, evp_pkey_ctx_translations, SIZEOFARRAY(evp_pkey_ctx_translations));
}

static const struct translation_st * lookup_evp_pkey_translation(struct translation_st * tmpl)                                      
{
	return lookup_translation(tmpl, evp_pkey_translations, SIZEOFARRAY(evp_pkey_translations));
}

/* This must ONLY be called for provider side operations */
int evp_pkey_ctx_ctrl_to_param(EVP_PKEY_CTX * pctx, int keytype, int optype, int cmd, int p1, void * p2)
{
	...
	if(pctx->pmeth != NULL && pctx->pmeth->pkey_id != translation->keytype1 && pctx->pmeth->pkey_id != translation->keytype2)
		return -1;
...
static int evp_pkey_setget_params_to_ctrl(const EVP_PKEY * pkey, enum action action_type, OSSL_PARAM * params)
{
	int ret = 1;
	for(; params != NULL && params->key != NULL; params++) {
		struct translation_ctx_st ctx = { 0, };
		 ...
		 * support getting.
		 */
		if(!ossl_assert(translation != NULL) || !ossl_assert(translation->action_type == GET) || !ossl_assert(translation->fixup_args != NULL)) {
			return -2;
		}
