...
static int des_ecb_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	...
	DES_ecb_encrypt((DES_cblock*)(in + i), (DES_cblock*)(out + i), (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx),
	    EVP_CIPHER_CTX_is_encrypting(ctx));
	return 1;
}

static int des_ofb_cipher(EVP_CIPHER_CTX * ctx, uchar * out,
    const uchar * in, size_t inl)
{
	while(inl >= EVP_MAXCHUNK) {
		int num = EVP_CIPHER_CTX_get_num(ctx);
		DES_ofb64_encrypt(in, out, (long)EVP_MAXCHUNK, (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx),
	...
	if(inl) {
		int num = EVP_CIPHER_CTX_get_num(ctx);
		DES_ofb64_encrypt(in, out, (long)inl, (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx),
		    (DES_cblock*)ctx->iv, &num);
		EVP_CIPHER_CTX_set_num(ctx, num);
	}
	return 1;
...
static int des_cbc_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t inl)
{
	...
	while(inl >= EVP_MAXCHUNK) {
		DES_ncbc_encrypt(in, out, (long)EVP_MAXCHUNK, (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx),
...
static int des_cfb8_cipher(EVP_CIPHER_CTX * ctx, uchar * out,
    const uchar * in, size_t inl)
{
	while(inl >= EVP_MAXCHUNK) {
		DES_cfb_encrypt(in, out, 8, (long)EVP_MAXCHUNK, (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx),
		    (DES_cblock*)ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
		inl -= EVP_MAXCHUNK;
		in += EVP_MAXCHUNK;
		out += EVP_MAXCHUNK;
	...
	if(inl)
		DES_cfb_encrypt(in, out, 8, (long)inl, (DES_key_schedule *)EVP_CIPHER_CTX_get_cipher_data(ctx), 
			(DES_cblock*)ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
	return 1;
}
...
static int des_ctrl(EVP_CIPHER_CTX * c, int type, int arg, void * ptr)
{
	switch(type) {
		case EVP_CTRL_RAND_KEY:
		    if(RAND_priv_bytes((uchar *)ptr, 8) <= 0)
			    return 0;
