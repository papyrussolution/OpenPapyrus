...
#include "crypto/ec.h"
/*
 * This file is meant to contain functions to provide EVP_PKEY support for EC keys.
 */
...
int EVP_PKEY_CTX_set_ecdh_cofactor_mode(EVP_PKEY_CTX * ctx, int cofactor_mode)
{
	...
	int ret = evp_pkey_ctx_getset_ecdh_param_checks(ctx);
	if(ret != 1)
		return ret;
	/*
...
int EVP_PKEY_CTX_get0_ecdh_kdf_ukm(EVP_PKEY_CTX * ctx, uchar ** pukm)
{
	size_t ukmlen;
	OSSL_PARAM params[2], * p = params;
	int ret = evp_pkey_ctx_getset_ecdh_param_checks(ctx);
...
{
	return EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_EC, EVP_PKEY_OP_TYPE_GEN, EVP_PKEY_CTRL_EC_PARAMGEN_CURVE_NID, nid, NULL);
}
