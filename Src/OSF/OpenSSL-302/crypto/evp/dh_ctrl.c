...
static int dh_paramgen_check(EVP_PKEY_CTX * ctx)
{
	...
	if(evp_pkey_ctx_is_legacy(ctx) && ctx->pmeth->pkey_id != EVP_PKEY_DH && ctx->pmeth->pkey_id != EVP_PKEY_DHX)
		return -1;
	return 1;
}
...
{
	if(!ctx || !EVP_PKEY_CTX_IS_DERIVE_OP(ctx)) {
		ERR_raise(ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED);
		/* Uses the same return values as EVP_PKEY_CTX_ctrl */
		return -2;
	}
...
int EVP_PKEY_CTX_set_dh_paramgen_seed(EVP_PKEY_CTX * ctx, const uchar * seed, size_t seedlen)
{
	int ret;
	OSSL_PARAM params[2], * p = params;
	if((ret = dh_paramgen_check(ctx)) <= 0)
		return ret;
	*p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_FFC_SEED, (void*)seed, seedlen);
	*p = OSSL_PARAM_construct_end();
	return evp_pkey_ctx_set_params_strict(ctx, params);
}
...
int EVP_PKEY_CTX_set0_dh_kdf_ukm(EVP_PKEY_CTX * ctx, uchar * ukm, int len)
{
	int ret;
	OSSL_PARAM params[2], * p = params;
...
int EVP_PKEY_CTX_get0_dh_kdf_ukm(EVP_PKEY_CTX * ctx, uchar ** pukm)
{
	int ret;
	size_t ukmlen;
	OSSL_PARAM params[2], * p = params;
