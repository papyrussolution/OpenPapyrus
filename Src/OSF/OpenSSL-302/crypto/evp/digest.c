...
int EVP_DigestUpdate(EVP_MD_CTX * ctx, const void * data, size_t count)
{
	...
	return ctx->digest->dupdate(ctx->algctx, (const uchar *)data, count);
	/* Code below to be removed when legacy support is dropped. */
legacy:
...
int EVP_DigestFinal(EVP_MD_CTX * ctx, uchar * md, unsigned int * size)
{
...
/* The caller can assume that this removes any secret data from the context */
int EVP_DigestFinal_ex(EVP_MD_CTX * ctx, uchar * md, unsigned int * isize)
{
	int ret, sz;
	size_t size = 0;
	size_t mdsize = 0;
...
int EVP_Digest(const void * data, size_t count,
    uchar * md, unsigned int * size, const EVP_MD * type,
    ENGINE * impl)
{
...
int EVP_MD_CTX_set_params(EVP_MD_CTX * ctx, const OSSL_PARAM params[])
{
	...
	if(pctx != NULL && (pctx->operation == EVP_PKEY_OP_VERIFYCTX || pctx->operation == EVP_PKEY_OP_SIGNCTX) && pctx->op.sig.algctx != NULL && pctx->op.sig.signature->set_ctx_md_params != NULL)
...
const OSSL_PARAM * EVP_MD_CTX_settable_params(EVP_MD_CTX * ctx)
{
	...
	/* If we have a pctx then we should try that first */
	pctx = ctx->pctx;
	if(pctx != NULL && (pctx->operation == EVP_PKEY_OP_VERIFYCTX || pctx->operation == EVP_PKEY_OP_SIGNCTX) && pctx->op.sig.algctx != NULL && pctx->op.sig.signature->settable_ctx_md_params != NULL)
...
int EVP_MD_CTX_get_params(EVP_MD_CTX * ctx, OSSL_PARAM params[])
{
	EVP_PKEY_CTX * pctx = ctx->pctx;
	/* If we have a pctx then we should try that first */
	if(pctx != NULL && (pctx->operation == EVP_PKEY_OP_VERIFYCTX || pctx->operation == EVP_PKEY_OP_SIGNCTX) && pctx->op.sig.algctx != NULL && pctx->op.sig.signature->get_ctx_md_params != NULL)
...
const OSSL_PARAM * EVP_MD_CTX_gettable_params(EVP_MD_CTX * ctx)
{
	...
	/* If we have a pctx then we should try that first */
	pctx = ctx->pctx;
	if(pctx != NULL && (pctx->operation == EVP_PKEY_OP_VERIFYCTX || pctx->operation == EVP_PKEY_OP_SIGNCTX) && pctx->op.sig.algctx != NULL && pctx->op.sig.signature->gettable_ctx_md_params != NULL)
...
static void set_legacy_nid(const char * name, void * vlegacy_nid)
{
	...
	if(legacy_method == NULL)
		return;
	nid = EVP_MD_nid((EVP_MD *)legacy_method);
...
{
	EVP_MD * md = (EVP_MD *)evp_generic_fetch(ctx, OSSL_OP_DIGEST, algorithm, properties, evp_md_from_algorithm, evp_md_up_ref, evp_md_free);
	return md;
}
