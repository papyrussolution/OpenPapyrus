...
extern "C" void aesni256_cbc_sha1_dec(const void * inp, void * out, size_t blocks, const AES_KEY * key, unsigned char iv[16], SHA_CTX * ctx, const void * in0);
...
static int aesni_cbc_hmac_sha1_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_AES_HMAC_SHA1 * key = data(ctx);
	unsigned int l;
	...
	if(EVP_CIPHER_CTX_is_encrypting(ctx)) {
		...
		if(plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
			...
			} * data = (aesni_cbc_hmac_sha1_cipher_DataBlock *)key->md.data;
