...
int EVP_PKEY_decapsulate(EVP_PKEY_CTX * ctx, uchar * secret, size_t * secretlen, const uchar * in, size_t inlen)
{
	if(!ctx || (in == NULL || inlen == 0) || (secret == NULL && secretlen == NULL))
		return 0;
...
{
	EVP_KEM * kem = (EVP_KEM *)OPENSSL_zalloc(sizeof(EVP_KEM));
	if(kem == NULL) {
...
{
	return (EVP_KEM *)evp_generic_fetch(ctx, OSSL_OP_KEM, algorithm, properties, evp_kem_from_algorithm, (int (*)(void *))EVP_KEM_up_ref, (void (*)(void *))EVP_KEM_free);
}

EVP_KEM * evp_kem_fetch_from_prov(OSSL_PROVIDER * prov, const char * algorithm, const char * properties)
{
	return (EVP_KEM *)evp_generic_fetch_from_prov(prov, OSSL_OP_KEM, algorithm, properties, evp_kem_from_algorithm, (int (*)(void *))EVP_KEM_up_ref, (void (*)(void *))EVP_KEM_free);
}
