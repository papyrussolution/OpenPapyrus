...
int EVP_PBE_scrypt_ex(const char * pass, size_t passlen, const uchar * salt, size_t saltlen,
    uint64_t N, uint64_t r, uint64_t p, uint64_t maxmem, uchar * key, size_t keylen, OSSL_LIB_CTX * ctx, const char * propq)
{
	const char * empty = "";
	int rv = 1;
	EVP_KDF * kdf;
	...
	if(kctx == NULL)
		return 0;
	*z++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (uchar *)pass, passlen);
	*z++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (uchar *)salt, saltlen);
