...
#pragma hdrstop
/*
 * This file uses the low level AES functions (which are deprecated for non-internal use) in order to implement the EVP AES ciphers.
 */
    ...
    const uchar * iv, int enc)
{
	...
	if((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc) {
		ret = aesni_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
		dat->block = (block128_f)aesni_decrypt;
		dat->stream.cbc = mode == EVP_CIPH_CBC_MODE ? (cbc128_f)aesni_cbc_encrypt : NULL;
	}
	else {
		ret = aesni_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
...
static int aesni_ccm_init_key(EVP_CIPHER_CTX * ctx, const uchar * key, const uchar * iv, int enc)
{
	...
	if(!iv && !key)
		return 1;
	if(key) {
		aesni_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &cctx->ks.ks);
...
static int aesni_ocb_init_key(EVP_CIPHER_CTX * ctx, const uchar * key, const uchar * iv, int enc)
{
	...
	if(key) {
		do {
			 ...
			 */
			aesni_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksenc.ks);
			aesni_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksdec.ks);
...
#ifdef VPAES_CAPABLE
	if(VPAES_CAPABLE) {
		ret = vpaes_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
...
#endif
	{
		ret = AES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
		dat->block = (block128_f)AES_encrypt;
...
static int aes_cbc_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_AES_KEY * dat = EVP_C_DATA(EVP_AES_KEY, ctx);
	if(dat->stream.cbc)
		(*dat->stream.cbc)(in, out, len, &dat->ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
	...
	switch(type) {
		...
		case EVP_CTRL_AEAD_SET_IVLEN:
		    ...
		    if((arg > EVP_MAX_IV_LENGTH) && (arg > gctx->ivlen)) {
			    if(gctx->iv != c->iv)
				    OPENSSL_free(gctx->iv);
			    if((gctx->iv = (uchar *)OPENSSL_malloc(arg)) == NULL) {
				    ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
				    return 0;
			    }
		    }
		...
		case EVP_CTRL_COPY:
	    {
		    ...
		    if(gctx->iv == c->iv)
			    ...
			    if((gctx_out->iv = (uchar *)OPENSSL_malloc(gctx->ivlen)) == NULL) {
				    ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
				    return 0;
			    }
...
static int aes_xts_cipher(EVP_CIPHER_CTX * ctx, uchar * out,
    const uchar * in, size_t len)
{
	EVP_AES_XTS_CTX * xctx = EVP_C_DATA(EVP_AES_XTS_CTX, ctx);
	...
	if(xctx->stream)
		(*xctx->stream)(in, out, len, (const AES_KEY *)xctx->xts.key1, (const AES_KEY *)xctx->xts.key2, ctx->iv);
	else if(CRYPTO_xts128_encrypt(&xctx->xts, ctx->iv, in, out, len, EVP_CIPHER_CTX_is_encrypting(ctx)))
		return 0;
	return 1;
}
...
#ifdef HWAES_CAPABLE
			if(HWAES_CAPABLE) {
				HWAES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &cctx->ks.ks);
...
#ifdef VPAES_CAPABLE
			if(VPAES_CAPABLE) {
				vpaes_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &cctx->ks.ks);
...
#endif
			AES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &cctx->ks.ks);
...
#ifdef VPAES_CAPABLE
			if(VPAES_CAPABLE) {
				vpaes_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksenc.ks);
				vpaes_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksdec.ks);
				if(!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)vpaes_encrypt, (block128_f)vpaes_decrypt, NULL))
					return 0;
				break;
			}
#endif
			AES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksenc.ks);
			AES_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksdec.ks);
    ...
    const uchar * in, size_t len)
{
	...
	if(in != NULL) {
		 ...
		 */
		if(*buf_len > 0) {
			unsigned int remaining = AES_BLOCK_SIZE - (*buf_len);
			if(remaining > len) {
