...
static int aria_init_key(EVP_CIPHER_CTX * ctx, const uchar * key, const uchar * iv, int enc)
{
	...
	if(enc || (mode != EVP_CIPH_ECB_MODE && mode != EVP_CIPH_CBC_MODE))
		ret = ossl_aria_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, (ARIA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx));
	else
		ret = ossl_aria_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, (ARIA_KEY *)EVP_CIPHER_CTX_get_cipher_data(ctx));
	if(ret < 0) {
...
static void aria_cbc_encrypt(const uchar * in, uchar * out, size_t len, const ARIA_KEY * key, uchar * ivec, const int enc)
{
	if(enc)
...
static void aria_cfb8_encrypt(const uchar * in, uchar * out, size_t length, const ARIA_KEY * key,
    uchar * ivec, int * num, const int enc)
{
	CRYPTO_cfb128_8_encrypt(in, out, length, key, ivec, num, enc, (block128_f)ossl_aria_encrypt);
}

static void aria_ecb_encrypt(const uchar * in, uchar * out, const ARIA_KEY * key, const int enc)
{
	ossl_aria_encrypt(in, out, key);
}

static void aria_ofb128_encrypt(const uchar * in, uchar * out, size_t length, const ARIA_KEY * key, uchar * ivec, int * num)
{
