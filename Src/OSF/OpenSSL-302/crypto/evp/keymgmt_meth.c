...
static void * keymgmt_new(void)
{
	...
	if((keymgmt = (EVP_KEYMGMT *)OPENSSL_zalloc(sizeof(*keymgmt))) == NULL || (keymgmt->lock = CRYPTO_THREAD_lock_new()) == NULL) {
...
static void * keymgmt_from_algorithm(int name_id, const OSSL_ALGORITHM * algodef, OSSL_PROVIDER * prov)
{
	...
	for(; fns->function_id != 0; fns++) {
		switch(fns->function_id) {
			case OSSL_FUNC_KEYMGMT_NEW:
			    if(keymgmt->F_New == NULL)
				    keymgmt->F_New = OSSL_FUNC_keymgmt_new(fns);
			    break;
			...
			case OSSL_FUNC_KEYMGMT_EXPORT:
			    if(keymgmt->F_Export == NULL) {
				    exportfncnt++;
				    keymgmt->F_Export = OSSL_FUNC_keymgmt_export(fns);
			    }
			    break;
	 ...
	 */
	if(keymgmt->free == NULL || (keymgmt->F_New == NULL && keymgmt->gen == NULL && keymgmt->load == NULL) || 
		keymgmt->has == NULL || (getparamfncnt != 0 && getparamfncnt != 2) || (setparamfncnt != 0 && setparamfncnt != 2) || 
		(setgenparamfncnt != 0 && setgenparamfncnt != 2) || (importfncnt != 0 && importfncnt != 2) || 
		(exportfncnt != 0 && exportfncnt != 2) || (keymgmt->gen != NULL && (keymgmt->gen_init == NULL || keymgmt->gen_cleanup == NULL))) {
		EVP_KEYMGMT_free(keymgmt);
...
EVP_KEYMGMT * evp_keymgmt_fetch_by_number(OSSL_LIB_CTX * ctx, int name_id, const char * properties)
{
	return (EVP_KEYMGMT *)evp_generic_fetch_by_number(ctx, OSSL_OP_KEYMGMT, name_id, properties, keymgmt_from_algorithm,
		   (int (*)(void *))EVP_KEYMGMT_up_ref, (void (*)(void *))EVP_KEYMGMT_free);
}

EVP_KEYMGMT * evp_keymgmt_fetch_from_prov(OSSL_PROVIDER * prov, const char * name, const char * properties)
{
	return (EVP_KEYMGMT *)evp_generic_fetch_from_prov(prov, OSSL_OP_KEYMGMT, name, properties,
		   keymgmt_from_algorithm, (int (*)(void *))EVP_KEYMGMT_up_ref, (void (*)(void *))EVP_KEYMGMT_free);
}

EVP_KEYMGMT * EVP_KEYMGMT_fetch(OSSL_LIB_CTX * ctx, const char * algorithm, const char * properties)
{
	return (EVP_KEYMGMT *)evp_generic_fetch(ctx, OSSL_OP_KEYMGMT, algorithm, properties, keymgmt_from_algorithm,
		   (int (*)(void *))EVP_KEYMGMT_up_ref, (void (*)(void *))EVP_KEYMGMT_free);
}
...
const OSSL_PROVIDER * EVP_KEYMGMT_get0_provider(const EVP_KEYMGMT * keymgmt) { return keymgmt->prov; }
int evp_keymgmt_get_number(const EVP_KEYMGMT * keymgmt) { return keymgmt->name_id; }
const char * EVP_KEYMGMT_get0_description(const EVP_KEYMGMT * keymgmt) { return keymgmt->description; }
const char * EVP_KEYMGMT_get0_name(const EVP_KEYMGMT * keymgmt) { return keymgmt->type_name; }
int EVP_KEYMGMT_is_a(const EVP_KEYMGMT * keymgmt, const char * name) { return evp_is_a(keymgmt->prov, keymgmt->name_id, NULL, name); }

void EVP_KEYMGMT_do_all_provided(OSSL_LIB_CTX * libctx, void (*fn)(EVP_KEYMGMT * keymgmt, void * arg), void * arg)
{
	evp_generic_do_all(libctx, OSSL_OP_KEYMGMT, (void (*)(void *, void *))fn, arg, keymgmt_from_algorithm,
	    (int (*)(void *))EVP_KEYMGMT_up_ref, (void (*)(void *))EVP_KEYMGMT_free);
}
...
int evp_keymgmt_gen_set_template(const EVP_KEYMGMT * keymgmt, void * genctx, void * pTemplate)
{
	/*
	...
	if(keymgmt->gen_set_template == NULL)
		return 1;
	return keymgmt->gen_set_template(genctx, pTemplate);
}
...
int evp_keymgmt_match(const EVP_KEYMGMT * keymgmt, const void * keydata1, const void * keydata2, int selection)
{
...
int evp_keymgmt_import(const EVP_KEYMGMT * keymgmt, void * keydata, int selection, const OSSL_PARAM params[])
{
	if(keymgmt->import == NULL)
		return 0;
...
int evp_keymgmt_export(const EVP_KEYMGMT * keymgmt, void * keydata, int selection, OSSL_CALLBACK * param_cb, void * cbarg)
{
	if(keymgmt->F_Export == NULL)
		return 0;
	return keymgmt->F_Export(keydata, selection, param_cb, cbarg);
}
