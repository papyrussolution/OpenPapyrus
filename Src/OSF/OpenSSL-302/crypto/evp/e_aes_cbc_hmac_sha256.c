...
static size_t tls1_1_multi_block_encrypt(EVP_AES_HMAC_SHA256 * key, uchar * out, const uchar * inp, size_t inp_len, int n4x)
{                               /* n4x is 1 or 2 */
...
static int aesni_cbc_hmac_sha256_cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, size_t len)
{
	EVP_AES_HMAC_SHA256 * key = data(ctx);
	unsigned int l;
	...
	if(EVP_CIPHER_CTX_is_encrypting(ctx)) {
		...
		if(plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
			...
			} * data = (aesni_cbc_hmac_sha256_cipher_DataBlock *)key->md.data;
