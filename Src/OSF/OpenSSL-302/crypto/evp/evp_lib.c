...
#pragma hdrstop
/*
 * EVP _meth_ APIs are deprecated for public use, but still ok for internal use.
 */
...
int EVP_CIPHER_set_asn1_iv(EVP_CIPHER_CTX * c, ASN1_TYPE * type)
{
	...
	if(type != NULL) {
		oiv = (uchar *)EVP_CIPHER_CTX_original_iv(c);
	...
	if(cipher->set_asn1_parameters != NULL) {
		...
		*p = OSSL_PARAM_construct_end();
		...
		if(OSSL_PARAM_modified(params)
		    ...
		    && (der = (uchar *)OPENSSL_malloc(params[0].return_size)) != NULL) {
			params[0].data = der;
...
int evp_cipher_cache_constants(EVP_CIPHER * cipher)
{
	...
	if(ok) {
		...
		if(OSSL_PARAM_locate_const(EVP_CIPHER_gettable_ctx_params(cipher), OSSL_CIPHER_PARAM_ALGORITHM_ID_PARAMS))
			cipher->flags |= EVP_CIPH_FLAG_CUSTOM_ASN1;
	}
	return ok;
}

int EVP_CIPHER_get_block_size(const EVP_CIPHER * cipher) { return cipher->block_size; }
int EVP_CIPHER_CTX_get_block_size(const EVP_CIPHER_CTX * ctx) { return EVP_CIPHER_get_block_size(ctx->cipher); }
int EVP_CIPHER_impl_ctx_size(const EVP_CIPHER * e) { return e->ctx_size; }

int EVP_Cipher(EVP_CIPHER_CTX * ctx, uchar * out, const uchar * in, unsigned int inl)
{
...
{
	int ok;
	const uchar * v = ctx->oiv;
	OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
	params[0] = OSSL_PARAM_construct_octet_ptr(OSSL_CIPHER_PARAM_IV, (void**)&v, sizeof(ctx->oiv));
...
{
	int ok;
	const uchar * v = ctx->iv;
	OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
	params[0] = OSSL_PARAM_construct_octet_ptr(OSSL_CIPHER_PARAM_UPDATED_IV, (void**)&v, sizeof(ctx->iv));
	ok = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params);
	return ok != 0 ? v : NULL;
}
...
int EVP_PKEY_CTX_set_group_name(EVP_PKEY_CTX * ctx, const char * name)
{
	...
	if(!ctx || !EVP_PKEY_CTX_IS_GEN_OP(ctx)) {
    ...
    const char * type, ...)
{
	...
	EVP_PKEY * ret = NULL;
	va_start(args, type);
	if(sstreqi_ascii(type, "RSA")) {
		bits = va_arg(args, size_t);
		params[0] = OSSL_PARAM_construct_size_t(OSSL_PKEY_PARAM_RSA_BITS, &bits);
	}
	else if(sstreqi_ascii(type, "EC")) {
		name = va_arg(args, char *);
		params[0] = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, name, 0);
	}
	else if(!sstreqi_ascii(type, "ED25519") && !sstreqi_ascii(type, "X25519") && !sstreqi_ascii(type, "ED448") && !sstreqi_ascii(type, "X448")) {
