...
EVP_MAC_CTX * EVP_MAC_CTX_new(EVP_MAC * mac)
{
	...
	if(!ctx || (ctx->algctx = mac->newctx(ossl_provider_ctx(mac->prov))) == NULL || !EVP_MAC_up_ref(mac)) {
...
static int evp_mac_final(EVP_MAC_CTX * ctx, int xof, uchar * out, size_t * outl, size_t outsize)
{
	size_t l;
	int res;
	OSSL_PARAM params[2];
	size_t macsize;
	if(!ctx || ctx->meth == NULL) {
...
uchar * EVP_Q_mac(OSSL_LIB_CTX * libctx,
    const char * name, const char * propq,
    const char * subalg, const OSSL_PARAM * params,
    const void * key, size_t keylen,
    const uchar * data, size_t datalen,
    uchar * out, size_t outsize, size_t * outlen)
{
	...
	if((ctx = EVP_MAC_CTX_new(mac)) != NULL && EVP_MAC_CTX_set_params(ctx, subalg_param) && 
		EVP_MAC_CTX_set_params(ctx, params) && EVP_MAC_init(ctx, (const uchar *)key, keylen, params) && 
		EVP_MAC_update(ctx, data, datalen) && EVP_MAC_final(ctx, out, &len, outsize)) {
		if(!out) {
			out = (uchar *)OPENSSL_malloc(len);
