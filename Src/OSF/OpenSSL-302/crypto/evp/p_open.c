...
int EVP_OpenInit(EVP_CIPHER_CTX * ctx, const EVP_CIPHER * type, const uchar * ek, int ekl, const uchar * iv, EVP_PKEY * priv)
{
	uchar * key = NULL;
	size_t keylen = 0;
	int ret = 0;
	...
	if(EVP_PKEY_decrypt_init(pctx) <= 0 || EVP_PKEY_decrypt(pctx, NULL, &keylen, ek, ekl) <= 0)
		goto err;
	if((key = (uchar *)OPENSSL_malloc(keylen)) == NULL) {
		ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
		goto err;
	}
