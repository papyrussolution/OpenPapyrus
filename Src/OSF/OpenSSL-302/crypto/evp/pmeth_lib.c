...
int evp_pkey_ctx_state(const EVP_PKEY_CTX * ctx)
{
	...
	if((EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.algctx) || (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.algctx) || 
		(EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.algctx) || (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->op.keymgmt.genctx) || 
		(EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.algctx))
...

{
	...
	/* BEGIN legacy */
	if(id == -1) {
		if(pkey && !evp_pkey_is_provided(pkey)) {
			id = pkey->type;
		}
		else {
			if(pkey) {
...
common:
	 ...
	 */
	if(e == NULL && app_pmeth == NULL && keytype) {
		/*
...
void evp_pkey_ctx_free_old_ops(EVP_PKEY_CTX * ctx)
{
	if(EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)) {
		if(ctx->op.sig.algctx && ctx->op.sig.signature)
		...
		if(ctx->op.kex.algctx && ctx->op.kex.exchange)
		...
		if(ctx->op.encap.algctx && ctx->op.encap.kem)
		...
		if(ctx->op.ciph.algctx && ctx->op.ciph.cipher)
		...
		if(ctx->op.keymgmt.genctx && ctx->keymgmt)
...
int EVP_PKEY_CTX_set_params(EVP_PKEY_CTX * ctx, const OSSL_PARAM * params)
{
	switch(evp_pkey_ctx_state(ctx)) {
		case EVP_PKEY_STATE_PROVIDER:
		    if(EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange && ctx->op.kex.exchange->set_ctx_params)
			    return ctx->op.kex.exchange->set_ctx_params(ctx->op.kex.algctx, params);
		    if(EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.signature && ctx->op.sig.signature->set_ctx_params)
			    return ctx->op.sig.signature->set_ctx_params(ctx->op.sig.algctx, params);
		    if(EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.cipher && ctx->op.ciph.cipher->set_ctx_params)
			    return ctx->op.ciph.cipher->set_ctx_params(ctx->op.ciph.algctx, params);
		    if(EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt && ctx->keymgmt->gen_set_params)
			    return evp_keymgmt_gen_set_params(ctx->keymgmt, ctx->op.keymgmt.genctx, params);
		    if(EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem && ctx->op.encap.kem->set_ctx_params)
...
int EVP_PKEY_CTX_get_params(EVP_PKEY_CTX * ctx, OSSL_PARAM * params)
{
	switch(evp_pkey_ctx_state(ctx)) {
		case EVP_PKEY_STATE_PROVIDER:
		    if(EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange && ctx->op.kex.exchange->get_ctx_params)
			    return ctx->op.kex.exchange->get_ctx_params(ctx->op.kex.algctx, params);
		    if(EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.signature && ctx->op.sig.signature->get_ctx_params)
			    return ctx->op.sig.signature->get_ctx_params(ctx->op.sig.algctx, params);
		    if(EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.cipher && ctx->op.ciph.cipher->get_ctx_params)
			    return ctx->op.ciph.cipher->get_ctx_params(ctx->op.ciph.algctx, params);
		    if(EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem && ctx->op.encap.kem->get_ctx_params)
...
const OSSL_PARAM * EVP_PKEY_CTX_gettable_params(const EVP_PKEY_CTX * ctx)
{
	...
	if(EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange && ctx->op.kex.exchange->gettable_ctx_params) {
	...
	if(EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.signature && ctx->op.sig.signature->gettable_ctx_params) {
	...
	if(EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.cipher && ctx->op.ciph.cipher->gettable_ctx_params) {
	...
	if(EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem && ctx->op.encap.kem->gettable_ctx_params) {
...
const OSSL_PARAM * EVP_PKEY_CTX_settable_params(const EVP_PKEY_CTX * ctx)
{
	...
	if(EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.cipher && ctx->op.ciph.cipher->settable_ctx_params) {
		provctx = ossl_provider_ctx(EVP_ASYM_CIPHER_get0_provider(ctx->op.ciph.cipher));
		return ctx->op.ciph.cipher->settable_ctx_params(ctx->op.ciph.algctx, provctx);
	}
	if(EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt && ctx->keymgmt->gen_settable_params) {
	...
	if(EVP_PKEY_CTX_IS_KEM_OP(ctx) && ctx->op.encap.kem && ctx->op.encap.kem->settable_ctx_params) {
		provctx = ossl_provider_ctx(EVP_KEM_get0_provider(ctx->op.encap.kem));
...
int EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX * ctx, const EVP_MD ** md)
{
	...
	if(!ctx || !EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)) {
    ...
    const char * name, const char * value)
{
	...
	switch(evp_pkey_ctx_state(ctx)) {
		...
		case EVP_PKEY_STATE_UNKNOWN:
		case EVP_PKEY_STATE_LEGACY:
		    if(!ctx || ctx->pmeth == NULL || ctx->pmeth->ctrl_str == NULL) {
			    ERR_raise(ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED);
			    return -2;
		    }
...
int evp_pkey_ctx_use_cached_data(EVP_PKEY_CTX * ctx)
{
	...
	if(ret && ctx->cached_parameters.dist_id_set) {
		...
		if(name)
			ret = evp_pkey_ctx_ctrl_str_int(ctx, name, (const char *)val);
		else
