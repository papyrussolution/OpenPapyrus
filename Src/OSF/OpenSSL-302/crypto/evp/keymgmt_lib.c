...
{
	struct evp_keymgmt_util_try_import_data_st * data = (struct evp_keymgmt_util_try_import_data_st *)arg;
...
int evp_keymgmt_util_cache_keydata(EVP_PKEY * pk, EVP_KEYMGMT * keymgmt, void * keydata)
{
	...
	if(keydata != NULL) {
		...
		p = (OP_CACHE_ELEM *)OPENSSL_malloc(sizeof(*p));
		if(!p)
			return 0;
		p->keydata = keydata;
		p->keymgmt = keymgmt;
