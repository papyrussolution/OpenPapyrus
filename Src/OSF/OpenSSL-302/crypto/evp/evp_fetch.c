...
{
	return (OSSL_METHOD_STORE *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_EVP_METHOD_STORE_INDEX, &evp_method_store_method);
}
...
static void * get_evp_method_from_store(void * store, const OSSL_PROVIDER ** prov, void * data)
{
	...
	if(store == NULL && (store = get_evp_method_store(methdata->libctx)) == NULL)
		return NULL;
	if(!ossl_method_store_fetch((OSSL_METHOD_STORE *)store, meth_id, methdata->propquery, prov, &method))
		return NULL;
	return method;
}
...
static int put_evp_method_in_store(void * store, void * method, const OSSL_PROVIDER * prov, const char * names, const char * propdef, void * data)
{
	...
	if(store == NULL && (store = get_evp_method_store(methdata->libctx)) == NULL)
		return 0;
	return ossl_method_store_add((OSSL_METHOD_STORE *)store, prov, meth_id, propdef, method, methdata->refcnt_up_method, methdata->destruct_method);
}
...
static void * inner_evp_generic_fetch(struct evp_method_data_st * methdata, OSSL_PROVIDER * prov, int operation_id,
    int name_id, const char * name, const char * properties, void *(*new_method)(int name_id, const OSSL_ALGORITHM * algodef,
    OSSL_PROVIDER * prov), int (*up_ref_method)(void *), void (*free_method)(void *))
{
	...
	// 
	// If there's ever an operation_id == 0 passed, we have an internal programming error.
	// 
	...
	// 
	// If we have been passed both a name_id and a name, we have an internal programming error.
	// 
	...
	if((name_id != 0 || name != NULL) && method == NULL) {
		...
		ERR_raise_data(ERR_LIB_EVP, code, "%s, Algorithm (%s : %d), Properties (%s)",
		    ossl_lib_ctx_get_descriptor(methdata->libctx), name == NULL ? "<null>" : name, name_id, properties == NULL ? "<null>" : properties);
	}
	return method;
}
    ...
    int mirrored)
{
	...
	if(plp != NULL && store != NULL) {
		...
		if(strsz > 0)
			propstr = (char*)OPENSSL_malloc(strsz);
		if(propstr == NULL) {
			ERR_raise(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR);
			return 0;
		}
...
char * evp_get_global_properties_str(OSSL_LIB_CTX * libctx, int loadconfig)
{
	...
	propstr = (char*)OPENSSL_malloc(sz);
	if(propstr == NULL) {
		ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
{
	struct filter_data_st * data = (struct filter_data_st *)arg;
