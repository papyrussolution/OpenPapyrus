...
static void * evp_keyexch_from_algorithm(int name_id, const OSSL_ALGORITHM * algodef, OSSL_PROVIDER * prov)
{
...
{
	return (EVP_KEYEXCH *)evp_generic_fetch(ctx, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm, (int (*)(void *))EVP_KEYEXCH_up_ref, (void (*)(void *))EVP_KEYEXCH_free);
}

EVP_KEYEXCH * evp_keyexch_fetch_from_prov(OSSL_PROVIDER * prov, const char * algorithm, const char * properties)
{
	return (EVP_KEYEXCH *)evp_generic_fetch_from_prov(prov, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm, (int (*)(void *))EVP_KEYEXCH_up_ref, (void (*)(void *))EVP_KEYEXCH_free);
}
...
int EVP_PKEY_derive_init_ex(EVP_PKEY_CTX * ctx, const OSSL_PARAM params[])
{
	...
	if(ctx->pkey == NULL) {
		...
		if(pkey == NULL || !EVP_PKEY_set_type_by_keymgmt(pkey, ctx->keymgmt) || (pkey->keydata = evp_keymgmt_newdata(ctx->keymgmt)) == NULL) {
			ERR_clear_last_mark();
			EVP_PKEY_free(pkey);
