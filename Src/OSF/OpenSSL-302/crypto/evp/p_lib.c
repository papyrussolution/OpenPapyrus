...
static int pkey_set_type(EVP_PKEY * pkey, ENGINE * e, int type, const char * str, int len, EVP_KEYMGMT * keymgmt);
...
int EVP_PKEY_eq(const EVP_PKEY * a, const EVP_PKEY * b)
{
	...
	if(!a || !b)
		return 0;
	if(a->keymgmt != NULL || b->keymgmt != NULL)
		return evp_pkey_cmp_any(a, b, (SELECT_PARAMETERS | OSSL_KEYMGMT_SELECT_KEYPAIR));
	/* All legacy keys */
	if(a->type != b->type)
		return -1;
	if(a->ameth != NULL) {
...
static EVP_PKEY * new_raw_key_int(OSSL_LIB_CTX * libctx, const char * strtype, const char * propq, int nidtype, ENGINE * e, const uchar * key, size_t len, int key_is_priv)
{
	EVP_PKEY * pkey = NULL;
	...
	if(e == NULL && ameth == NULL) {
		...
		if(EVP_PKEY_fromdata_init(ctx) == 1) {
			...
			ERR_clear_last_mark();
			params[0] = OSSL_PARAM_construct_octet_string(key_is_priv ? OSSL_PKEY_PARAM_PRIV_KEY : OSSL_PKEY_PARAM_PUB_KEY, (void*)key, len);
...
static int get_raw_key_details(const OSSL_PARAM params[], void * arg)
{
	...
	struct raw_key_details_st * raw_key = (struct raw_key_details_st *)arg;
	if(raw_key->selection == OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
		if((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_PRIV_KEY)) != NULL)
			return OSSL_PARAM_get_octet_string(p, (void**)raw_key->key, raw_key->key == NULL ? 0 : *raw_key->len, raw_key->len);
	}
...
static void detect_foreign_key(EVP_PKEY * pkey)
{
	switch(pkey->type) {
		...
		case EVP_PKEY_DH:
		    pkey->foreign = pkey->pkey.dh != NULL && ossl_dh_is_foreign(pkey->pkey.dh);
		    break;
#endif
		default:
		    pkey->foreign = 0;
...
const uchar * EVP_PKEY_get0_hmac(const EVP_PKEY * pkey, size_t * len)
{
	...
	os = (const ASN1_OCTET_STRING *)evp_pkey_get_legacy((EVP_PKEY*)pkey);
	if(os != NULL) {
		*len = os->length;
		return os->data;
	}
...
static DSA * evp_pkey_get0_DSA_int(const EVP_PKEY * pkey)
{
	...
	return (DSA *)evp_pkey_get_legacy((EVP_PKEY*)pkey);
}
...
static const ECX_KEY * evp_pkey_get0_ECX_KEY(const EVP_PKEY * pkey, int type)
{
	...
	return (const ECX_KEY *)evp_pkey_get_legacy((EVP_PKEY*)pkey);
}
...
DH * evp_pkey_get0_DH_int(const EVP_PKEY * pkey)
{
	...
	return (DH *)evp_pkey_get_legacy((EVP_PKEY*)pkey);
}
...
const char * evp_pkey_type2name(int type)
{
	...
	for(i = 0; i < SIZEOFARRAY(standard_name2type); i++) {
		if(type == (int)standard_name2type[i].id)
			return (const char *)standard_name2type[i].ptr;
	}
...
int EVP_PKEY_set1_encoded_public_key(EVP_PKEY * pkey, const uchar * pub,
    size_t publen)
{
...
size_t EVP_PKEY_get1_encoded_public_key(EVP_PKEY * pkey, uchar ** ppub)
{
	...
	if(pkey != NULL && evp_pkey_is_provided(pkey)) {
		...
		*ppub = (uchar *)OPENSSL_malloc(return_size);
		if(*ppub == NULL)
			return 0;
...
static void find_ameth(const char * name, void * data)
{
	 ...
	 */
	ERR_set_mark();
	if(pkey_set_type(NULL, NULL, EVP_PKEY_NONE, name, strlen(name), NULL)) {
		if(str[0] == NULL)
			str[0] = name;
		else if(str[1] == NULL)
			str[1] = name;
...
#define EVP_PKEY_TYPE_STRLEN (str[0] == NULL ? -1 : (int)strlen(str[0]))
	...
	if(!EVP_KEYMGMT_names_do_all(keymgmt, find_ameth, &str) || str[1] != NULL) {
		ERR_raise(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR);
		return 0;
	}
#else
...
int evp_pkey_copy_downgraded(EVP_PKEY ** dest, const EVP_PKEY * src)
{
	...
	if(evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
		...
		if(!ossl_assert(type != EVP_PKEY_NONE)) {
			ERR_raise_data(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR, "keymgmt key type = %s but legacy type = EVP_PKEY_NONE", keytype);
			return 0;
		}
		...
		if(EVP_PKEY_set_type(*dest, type)) {
			...
			if((*dest)->ameth->import_from == NULL) {
				ERR_raise_data(ERR_LIB_EVP, EVP_R_NO_IMPORT_FUNCTION, "key type = %s", keytype);
			}
			else {
				/*
				...
				if(pctx == NULL)
					ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
				if(pctx != NULL && evp_keymgmt_export(keymgmt, keydata, OSSL_KEYMGMT_SELECT_ALL, (*dest)->ameth->import_from, pctx)) {
					/* Synchronize the dirty count */
			...
			ERR_raise_data(ERR_LIB_EVP, EVP_R_KEYMGMT_EXPORT_FAILURE, "key type = %s", keytype);
		}
	}
...
int EVP_PKEY_get_bn_param(const EVP_PKEY * pkey, const char * key_name, BIGNUM ** bn)
{
	...
	if(!EVP_PKEY_get_params(pkey, params)) {
		 ...
		 */
		buf = (uchar *)OPENSSL_zalloc(buf_sz);
		if(buf == NULL)
			return 0;
		params[0].data = buf;
...
int EVP_PKEY_get_octet_string_param(const EVP_PKEY * pkey, const char * key_name, uchar * buf, size_t max_buf_sz, size_t * out_len)
{
	OSSL_PARAM params[2];
	int ret1 = 0, ret2 = 0;
	if(key_name == NULL)
...
int EVP_PKEY_get_int_param(const EVP_PKEY * pkey, const char * key_name, int * out)
{
	...
	params[1] = OSSL_PARAM_construct_end();
	return EVP_PKEY_get_params(pkey, params) && OSSL_PARAM_modified(params);
}
...
int EVP_PKEY_set_bn_param(EVP_PKEY * pkey, const char * key_name, const BIGNUM * bn)
{
	...
	unsigned char buffer[2048];
	int bsize = 0;
	if(key_name == NULL || bn == NULL || pkey == NULL || !evp_pkey_is_assigned(pkey))
		return 0;
...
int EVP_PKEY_set_octet_string_param(EVP_PKEY * pkey, const char * key_name, const uchar * buf, size_t bsize)
{
	OSSL_PARAM params[2];
	if(key_name == NULL)
		return 0;
	params[0] = OSSL_PARAM_construct_octet_string(key_name, (uchar *)buf, bsize);
	params[1] = OSSL_PARAM_construct_end();
	return EVP_PKEY_set_params(pkey, params);
}
...
{
	return (pkey != NULL && evp_pkey_is_provided(pkey)) ? EVP_KEYMGMT_settable_params(pkey->keymgmt) : NULL;
}
...
{
	return (pkey && evp_pkey_is_provided(pkey)) ? EVP_KEYMGMT_gettable_params(pkey->keymgmt) : NULL;
}
