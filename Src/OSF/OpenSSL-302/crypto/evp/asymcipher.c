...
static int evp_pkey_asym_cipher_init(EVP_PKEY_CTX * ctx, int operation, const OSSL_PARAM params[])
{
	 ...
	 *
	 * 1.  Do the normal asym cipher fetch, using the fetching data given by the EVP_PKEY_CTX.
	 * 2.  Do the provider specific asym cipher fetch, from the same provider as |ctx->keymgmt|
	 *
...
{
	return (EVP_ASYM_CIPHER *)evp_generic_fetch_from_prov(prov, OSSL_OP_ASYM_CIPHER,
...
{
	evp_generic_do_all(libctx, OSSL_OP_ASYM_CIPHER, (void (*)(void *, void *))fn, arg, evp_asym_cipher_from_algorithm, (int (*)(void *))EVP_ASYM_CIPHER_up_ref, (void (*)(void *))EVP_ASYM_CIPHER_free);
}
