...
DSA_SIG * d2i_DSA_SIG(DSA_SIG ** psig, const uchar ** ppin, long len)
{
	DSA_SIG * sig;
...
int i2d_DSA_SIG(const DSA_SIG * sig, uchar ** ppout)
{
	BUF_MEM * buf = NULL;
	size_t encoded_len;
	WPACKET pkt;
...
int ossl_dsa_sign_int(int type, const uchar * dgst, int dlen, uchar * sig, unsigned int * siglen, DSA * dsa)
{
	DSA_SIG * s;
...
int DSA_sign(int type, const uchar * dgst, int dlen, uchar * sig, unsigned int * siglen, DSA * dsa)
{
