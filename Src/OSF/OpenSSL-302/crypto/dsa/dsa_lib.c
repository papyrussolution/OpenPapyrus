...
void * DSA_get_ex_data(const DSA * d, int idx) { return CRYPTO_get_ex_data(&d->ex_data, idx); }
...
{
	return (d->params.p && d->params.q) ? BN_security_bits(BN_num_bits(d->params.p), BN_num_bits(d->params.q)) : -1;
}

int DSA_bits(const DSA * dsa)
{
	return dsa->params.p ? BN_num_bits(dsa->params.p) : -1;
}
