...
struct ossl_http_req_ctx_st {
	...
	uchar * buf;    /* Buffer to write request or read response */
	...
	uchar * pos;    /* Current position sending data */
...
OSSL_HTTP_REQ_CTX * OSSL_HTTP_REQ_CTX_new(BIO * wbio, BIO * rbio, int buf_size)
{
	...
	if((rctx = (OSSL_HTTP_REQ_CTX *)OPENSSL_zalloc(sizeof(*rctx))) == NULL)
		return NULL;
	rctx->state = OHS_ERROR;
	rctx->buf_size = buf_size > 0 ? buf_size : OSSL_HTTP_DEFAULT_MAX_LINE_LEN;
	rctx->buf = (uchar *)OPENSSL_malloc(rctx->buf_size);
	rctx->wbio = wbio;
	rctx->rbio = rbio;
	if(rctx->buf == NULL) {
		OPENSSL_free(rctx);
...
static int set1_content(OSSL_HTTP_REQ_CTX * rctx, const char * content_type, BIO * req)
{
	...
	if(((req_len = BIO_ctrl(req, BIO_CTRL_INFO, 0, NULL)) <= 0 || BIO_printf(rctx->mem, "Content-Length: %ld\r\n", req_len) > 0) && BIO_up_ref(req)) {
		rctx->req = req;
		return 1;
	}
	return 0;
...
static OSSL_HTTP_REQ_CTX * http_req_ctx_new(int free_wbio, BIO * wbio, BIO * rbio, OSSL_HTTP_bio_cb_t bio_update_fn,
    void * arg, int use_ssl, const char * proxy, const char * server, const char * port, int buf_size, int overall_timeout)
{
    ...
    int buf_size, int overall_timeout)
{
	...
	if(rbio == NULL && BIO_do_connect_retry(cbio, overall_timeout, -1) <= 0) {
		if(!bio) /* cbio was not provided by caller */
			BIO_free_all(cbio);
		goto end;
	}
	...
	if(rctx)
		(void)ERR_pop_to_mark(); // remove any spurious error queue entries by ssl_add_cert_chain()
	else
		(void)ERR_clear_last_mark();
	return rctx;
}
...
BIO * OSSL_HTTP_exchange(OSSL_HTTP_REQ_CTX * rctx, char ** redirection_url)
{
	...
	if(!resp) {
		if(rctx->redirection_url != NULL) {
			...
			if(lib == ERR_LIB_SSL || lib == ERR_LIB_HTTP || (lib == ERR_LIB_BIO && reason == BIO_R_CONNECT_TIMEOUT) || (lib == ERR_LIB_BIO && reason == BIO_R_CONNECT_ERROR)
				...
				if(err == 0) {
					BIO_snprintf(buf, sizeof(buf), " peer has disconnected%s", rctx->use_ssl ? " violating the protocol" : ", likely because it requires the use of TLS");
					ERR_add_error_data(1, buf);
				}
			}
		}
...
static char * base64encode(const void * buf, size_t len)
{
	...
	if(len % 3 > 0)
		outl++;
	outl <<= 2;
	out = (char *)OPENSSL_malloc(outl + 1);
	if(!out)
		return 0;
	i = EVP_EncodeBlock((uchar *)out, (const uchar *)buf, len);
 ...
 */
int OSSL_HTTP_proxy_connect(BIO * bio, const char * server, const char * port, const char * proxyuser, const char * proxypass,
    int timeout, BIO * bio_err, const char * prog)
{
#undef BUF_SIZE
#define BUF_SIZE (8 * 1024)
	char * mbuf = (char *)OPENSSL_malloc(BUF_SIZE);
	char * mbufp;
	int read_len = 0;
	int ret = 0;
	...
	if(proxyuser != NULL) {
		...
		proxyauth = (char *)OPENSSL_malloc(len + 1);
		if(proxyauth == NULL)
			goto end;
