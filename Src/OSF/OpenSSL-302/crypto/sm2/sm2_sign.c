...
static int sm2_sig_verify(const EC_KEY * key, const ECDSA_SIG * sig, const BIGNUM * e)
{
	...
	const BIGNUM * s = NULL;
	OSSL_LIB_CTX * libctx = ossl_ec_key_get_libctx(key);
	BN_CTX * ctx = BN_CTX_new_ex(libctx);
...
int ossl_sm2_internal_sign(const uchar * dgst, int dgstlen, uchar * sig, unsigned int * siglen, EC_KEY * eckey)
{
	ECDSA_SIG * s = NULL;
	int sigleni;
	int ret = -1;
	BIGNUM * e = BN_bin2bn(dgst, dgstlen, NULL);
	if(!e) {
		ERR_raise(ERR_LIB_SM2, ERR_R_BN_LIB);
		goto done;
	}
...
int ossl_sm2_internal_verify(const uchar * dgst, int dgstlen, const uchar * sig, int sig_len, EC_KEY * eckey)
{
	BIGNUM * e = NULL;
	const uchar * p = sig;
	uchar * der = NULL;
