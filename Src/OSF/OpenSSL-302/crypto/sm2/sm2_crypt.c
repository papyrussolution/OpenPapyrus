...
int ossl_sm2_plaintext_size(const uchar * ct, size_t ct_size,
    size_t * pt_size)
{
...
int ossl_sm2_encrypt(const EC_KEY * key, const EVP_MD * digest, const uint8 * msg, size_t msg_len, uint8 * ciphertext_buf, size_t * ciphertext_len)
{
	...
	memzero(ciphertext_buf, *ciphertext_len);
	...
	if(!ossl_ecdh_kdf_X9_63(msg_mask, msg_len, x2y2, 2 * field_size, NULL, 0, digest, libctx, propq)) {
