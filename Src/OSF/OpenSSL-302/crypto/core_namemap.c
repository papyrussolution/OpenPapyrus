...
int ossl_namemap_doall_names(const OSSL_NAMEMAP * namemap, int number, void (*fn)(const char * name, void * data), void * data)
{
	...
	cbdata.names = (const char **)OPENSSL_malloc(sizeof(*cbdata.names) * num_names);
...
static int namemap_name2num_n(const OSSL_NAMEMAP * namemap, const char * name, size_t name_len)
{
	...
	return namenum_entry ? namenum_entry->number : 0;
}
...
{
	return name ? ossl_namemap_name2num_n(namemap, name, strlen(name)) : 0;
}
    ...
    void * arg)
{
	...
	if(base_nid != NID_undef) {
		num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, OBJ_nid2sn(base_nid));
		num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, OBJ_nid2ln(base_nid));
	}
	if(nid != NID_undef) {
		num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, OBJ_nid2sn(nid));
		num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, OBJ_nid2ln(nid));
		if((obj = OBJ_nid2obj(nid)) != NULL) {
			char txtoid[OSSL_MAX_NAME_SIZE];
			if(OBJ_obj2txt(txtoid, sizeof(txtoid), obj, 1) > 0)
				num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, txtoid);
		}
	}
	if(pem_name)
		num = ossl_namemap_add_name((OSSL_NAMEMAP *)arg, num, pem_name);
}

static void get_legacy_cipher_names(const OBJ_NAME * on, void * arg)
{
	const EVP_CIPHER * cipher = (const EVP_CIPHER *)OBJ_NAME_get(on->name, on->type);
	if(cipher)
...
{
	const EVP_MD * md = (const EVP_MD *)OBJ_NAME_get(on->name, on->type);
	if(md)
...
OSSL_NAMEMAP * ossl_namemap_stored(OSSL_LIB_CTX * libctx)
{
	...
	int nms;
#endif
	OSSL_NAMEMAP * namemap = (OSSL_NAMEMAP *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_NAMEMAP_INDEX, &stored_namemap_method);
	if(namemap == NULL)
		return NULL;
#ifndef FIPS_MODULE
...
OSSL_NAMEMAP * ossl_namemap_new(void)
{
	...
	if((namemap = (OSSL_NAMEMAP *)OPENSSL_zalloc(sizeof(*namemap))) != NULL && (namemap->lock = CRYPTO_THREAD_lock_new()) != NULL && 
