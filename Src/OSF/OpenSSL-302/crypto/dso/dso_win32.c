...
static int win32_load(DSO * dso)
{
	...
	p = (HINSTANCE *)OPENSSL_malloc(sizeof(*p));
	if(!p) {
...
static int win32_unload(DSO * dso)
{
	...
	if(sk_void_num(dso->meth_data) < 1)
		return 1;
	p = (HINSTANCE *)sk_void_pop(dso->meth_data);
	if(!p) {
...
static DSO_FUNC_TYPE win32_bind_func(DSO * dso, const char * symname)
{
	...
	ptr = (HINSTANCE *)sk_void_value(dso->meth_data, sk_void_num(dso->meth_data) - 1);
	if(ptr == NULL) {
		ERR_raise(ERR_LIB_DSO, DSO_R_NULL_HANDLE);
		return NULL;
	}
...
static struct file_st * win32_splitter(DSO * dso, const char * filename, int assume_last_is_dir)
{
	...
	result = (struct file_st *)OPENSSL_zalloc(sizeof(*result));
	if(!result) {
		ERR_raise(ERR_LIB_DSO, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
...
static char * win32_joiner(DSO * dso, const struct file_st * file_split)
{
	...
	result = (char*)OPENSSL_malloc(len + 1);
	if(!result) {
		ERR_raise(ERR_LIB_DSO, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
