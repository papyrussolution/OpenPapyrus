...
static int load_iv(char ** fromp, uchar * to, int num);
...
static int check_pem(const char * nm, const char * name)
{
	...
	if(strcmp(nm, PEM_STRING_DHXPARAMS) == 0 && strcmp(name, PEM_STRING_DHPARAMS) == 0)
		return 1;
...
{
	return (flags & PEM_FLAG_SECURE) ? OPENSSL_secure_malloc(num) : OPENSSL_malloc(num);
}

static int pem_bytes_read_bio_flags(uchar ** pdata, long * plen, char ** pnm, const char * name, BIO * bp,
    pem_password_cb * cb, void * u, unsigned int flags)
{
...
int PEM_bytes_read_bio_secmem(uchar ** pdata, long * plen, char ** pnm,
    const char * name, BIO * bp, pem_password_cb * cb,
    void * u) {
	return pem_bytes_read_bio_flags(pdata, plen, pnm, name, bp, cb, u,
    ...
    pem_password_cb * callback, void * u)
{
	...
	data = (uchar *)OPENSSL_malloc((unsigned int)dsize + 20);
	if(!data) {
...
int PEM_do_header(EVP_CIPHER_INFO * cipher, uchar * data, long * plen,
    pem_password_cb * callback, void * u)
{
	int ok;
	int keylen;
...
int PEM_get_EVP_CIPHER_INFO(char * header, EVP_CIPHER_INFO * cipher)
{
	...
	char * dekinfostart, c;
	cipher->cipher = NULL;
	memzero(cipher->iv, sizeof(cipher->iv));
...
#ifndef OPENSSL_NO_STDIO
int PEM_read(FILE * fp, char ** name, char ** header, uchar ** data,
    long * len)
{
	BIO * b;
	int ret;
...
int PEM_read_bio_ex(BIO * bp, char ** name_out, char ** header,
    uchar ** data, long * len_out, unsigned int flags)
{
	...
	*data = (uchar *)pem_malloc(len, flags);
...
int PEM_read_bio(BIO * bp, char ** name, char ** header, uchar ** data,
    long * len)
{
