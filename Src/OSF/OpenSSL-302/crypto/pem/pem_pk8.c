...
static int do_pk8pkey(BIO * bp, const EVP_PKEY * x, int isder, int nid, const EVP_CIPHER * enc, const char * kstr, int klen, pem_password_cb * cb, void * u, const char * propq);

#ifndef OPENSSL_NO_STDIO
static int do_pk8pkey_fp(FILE * bp, const EVP_PKEY * x, int isder, int nid, const EVP_CIPHER * enc, const char * kstr, int klen,
	...
	if(nid == -1 && OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0) {
		...
		if(enc != NULL) {
			...
			if(OSSL_ENCODER_CTX_set_cipher(ctx, EVP_CIPHER_get0_name(enc),
			    NULL)) {
				const uchar * ukstr = (const unsigned char*)kstr;
...
EVP_PKEY * d2i_PKCS8PrivateKey_bio(BIO * bp, EVP_PKEY ** x, pem_password_cb * cb, void * u)
{
	...
	X509_SIG * p8 = d2i_PKCS8_bio(bp, NULL);
	if(p8 == NULL)
		return NULL;
	if(cb)
...
static int do_pk8pkey_fp(FILE * fp, const EVP_PKEY * x, int isder, int nid, const EVP_CIPHER * enc, const char * kstr, int klen, pem_password_cb * cb, void * u, const char * propq)
{
	BIO * bp;
	int ret;
