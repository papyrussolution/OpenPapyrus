...
#pragma hdrstop
/*
 * RSA and DSA low level APIs are deprecated for public use, but still ok for internal use.
 */
 ...
 */
static unsigned int read_ledword(const uchar ** in)
{
	const uchar * p = *in;
	unsigned int ret;
	ret = (unsigned int)*p++;
...
static int read_lebn(const uchar ** in, unsigned int nbyte, BIGNUM ** r)
{
 ...
 * created.
 */
#define isdss_to_evp_type(isdss) (isdss == 0 ? EVP_PKEY_RSA : isdss == 1 ? EVP_PKEY_DSA : EVP_PKEY_NONE)
...
static EVP_PKEY * evp_pkey_new0_key(void * key, int evp_type)
{
	...
	if((pkey = EVP_PKEY_new()) != NULL) {
		switch(evp_type) {
			case EVP_PKEY_RSA:
			    if(EVP_PKEY_set1_RSA(pkey, (struct rsa_st *)key))
				    break;
			    EVP_PKEY_free(pkey);
			    pkey = NULL;
			    break;
			...
			case EVP_PKEY_DSA:
			    if(EVP_PKEY_set1_DSA(pkey, (struct dsa_st *)key))
				    break;
			    EVP_PKEY_free(pkey);
			    pkey = NULL;
			    break;
 ...
 *      header data.
 */
int ossl_do_blob_header(const uchar ** in, unsigned int length,
...
static void * do_b2i_key(const uchar ** in, unsigned int length,
    int * isdss, int * ispub)
{
	const uchar * p = *in;
...
EVP_PKEY * ossl_b2i(const uchar ** in, unsigned int length, int * ispub)
{
	int isdss = -1;
...
#ifndef OPENSSL_NO_DSA
DSA * ossl_b2i_DSA_after_header(const uchar ** in, unsigned int bitlen,
    int ispub)
{
	const uchar * p = *in;
	DSA * dsa = NULL;
...
RSA * ossl_b2i_RSA_after_header(const uchar ** in, unsigned int bitlen,
    int ispub)
{
	const uchar * pin = *in;
...
static void write_ledword(uchar ** out, unsigned int dw)
{
	uchar * p = *out;
...
static void write_rsa(uchar ** out, const RSA * rsa, int ispub);

#ifndef OPENSSL_NO_DSA
static int check_bitlen_dsa(const DSA * dsa, int ispub, unsigned int * magic);
static void write_dsa(uchar ** out, const DSA * dsa, int ispub);
#endif

static int do_i2b(uchar ** out, const EVP_PKEY * pk, int ispub)
{
	uchar * p;
	...
	if(*out)
		...
		if((p = (uchar *)OPENSSL_malloc(outlen)) == NULL) {
			ERR_raise(ERR_LIB_PEM, ERR_R_MALLOC_FAILURE);
			outlen = -1;
			goto end;
		}
...
int ossl_do_PVK_header(const uchar ** in, unsigned int length, int skip_magic, unsigned int * psaltlen, unsigned int * pkeylen)
{
	const uchar * p = *in;
...
static int derive_pvk_key(uchar * key,
    const uchar * salt, unsigned int saltlen,
...
static void * do_PVK_body_key(const uchar ** in,
    ...
    OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	if(saltlen) {
		...
		if(!derive_pvk_key(keybuf, p, saltlen,
		    (uchar *)psbuf, inlen, libctx, propq))
			goto err;
		p += saltlen;
...
#ifndef OPENSSL_NO_DSA
DSA * b2i_DSA_PVK_bio_ex(BIO * in, pem_password_cb * cb, void * u, OSSL_LIB_CTX * libctx, const char * propq)
{
	int isdss = 1;
	int ispub = 0;           /* PVK keys are always private */
	return (DSA *)do_PVK_key_bio(in, cb, u, &isdss, &ispub, libctx, propq);
}
...
RSA * b2i_RSA_PVK_bio_ex(BIO * in, pem_password_cb * cb, void * u, OSSL_LIB_CTX * libctx, const char * propq)
{
	int isdss = 0;
	int ispub = 0;           /* PVK keys are always private */
	return (RSA *)do_PVK_key_bio(in, cb, u, &isdss, &ispub, libctx, propq);
}
...
static int i2b_PVK(uchar ** out, const EVP_PKEY * pk, int enclevel,
    pem_password_cb * cb, void * u, OSSL_LIB_CTX * libctx,
    const char * propq)
{
	int ret = -1;
	int outlen = 24, pklen;
	uchar * p = NULL, * start = NULL;
