...
int OSSL_ENCODER_to_data(OSSL_ENCODER_CTX * ctx, uchar ** pdata,
    size_t * pdata_len)
{
	BIO * out;
	BUF_MEM * buf = NULL;
...
static OSSL_ENCODER_INSTANCE * ossl_encoder_instance_new(OSSL_ENCODER * encoder, void * encoderctx)
{
	...
	if((encoder_inst = (OSSL_ENCODER_INSTANCE *)OPENSSL_zalloc(sizeof(*encoder_inst))) == NULL) {
	...
	if(props == NULL) {
		ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION, "there are no property definitions with encoder %s", OSSL_ENCODER_get0_name(encoder));
		goto err;
	}
...
static int encoder_process(struct encoder_process_data_st * data)
{
	...
	for(i = data->current_encoder_inst_index; i-- > 0;) {
		...
		current_encoder_ctx = (OSSL_ENCODER_CTX *)OSSL_ENCODER_INSTANCE_get_encoder_ctx(current_encoder_inst);
		...
		OSSL_TRACE_BEGIN(ENCODER) {
			BIO_printf(trc_out, "[%d] (ctx %p) Considering encoder instance %p (encoder %p)\n", data->level, (void*)data->ctx, (void*)current_encoder_inst, (void*)current_encoder);
		} OSSL_TRACE_END(ENCODER);
		/*
	...
	if(allocated_out != NULL) {
		...
		data->running_output = (uchar *)buf->data;
