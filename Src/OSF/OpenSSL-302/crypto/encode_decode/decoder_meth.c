...
static OSSL_DECODER * ossl_decoder_new(void)
{
	...
	if((decoder = (OSSL_DECODER *)OPENSSL_zalloc(sizeof(*decoder))) == NULL || (decoder->base.lock = CRYPTO_THREAD_lock_new()) == NULL) {
...
{
	struct decoder_data_st * methdata = (struct decoder_data_st *)data;
...
{
	return (OSSL_METHOD_STORE *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_DECODER_STORE_INDEX, &decoder_store_method);
}
...
static int put_decoder_in_store(void * store, void * method, const OSSL_PROVIDER * prov, const char * names, const char * propdef, void * data)
{
	struct decoder_data_st * methdata = (struct decoder_data_st *)data;
	...
	if((namemap = ossl_namemap_stored(methdata->libctx)) == NULL || (id = ossl_namemap_name2num_n(namemap, names, l)) == 0)
		return 0;
	if(store == NULL && (store = get_decoder_store(methdata->libctx)) == NULL)
		return 0;
	return ossl_method_store_add((OSSL_METHOD_STORE *)store, prov, id, propdef, method, (int (*)(void *))OSSL_DECODER_up_ref, (void (*)(void *))OSSL_DECODER_free);
}
 ...
 */
static void * construct_decoder(const OSSL_ALGORITHM * algodef, OSSL_PROVIDER * prov, void * data)
{
	/*
...
OSSL_DECODER * ossl_decoder_fetch_by_number(OSSL_LIB_CTX * libctx, int id, const char * properties)
{
	struct decoder_data_st methdata;
	void * method;
	methdata.libctx = libctx;
...
static void do_one(ossl_unused int id, void * method, void * arg)
{
	...
	data->user_fn((OSSL_DECODER *)method, data->user_arg);
}

void OSSL_DECODER_do_all_provided(OSSL_LIB_CTX * libctx, void (*user_fn)(OSSL_DECODER * decoder, void * arg), void * user_arg)
{
	struct decoder_data_st methdata;
...
OSSL_DECODER_CTX * OSSL_DECODER_CTX_new(void)
{
	...
	if((ctx = (OSSL_DECODER_CTX *)OPENSSL_zalloc(sizeof(*ctx))) == NULL)
		ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_MALLOC_FAILURE);
	return ctx;
}
...
int OSSL_DECODER_CTX_set_params(OSSL_DECODER_CTX * ctx, const OSSL_PARAM params[])
{
	...
	for(i = 0; i < l; i++) {
		...
		OSSL_DECODER * decoderctx = (OSSL_DECODER *)OSSL_DECODER_INSTANCE_get_decoder_ctx(decoder_inst);
