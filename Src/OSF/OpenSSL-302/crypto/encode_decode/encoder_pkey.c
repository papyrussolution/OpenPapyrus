    ...
    const char * propquery)
{
	...
	params[0] = OSSL_PARAM_construct_utf8_string(OSSL_ENCODER_PARAM_CIPHER, (char*)cipher_name, 0);
	params[1] = OSSL_PARAM_construct_utf8_string(OSSL_ENCODER_PARAM_PROPERTIES, (char*)propquery, 0);
...
{
	struct collected_encoder_st * data = (struct collected_encoder_st *)arg;
	size_t i, end_i;
...
{
	struct collected_names_st * data = (struct collected_names_st *)arg;
	if(data->error_occurred)
		return;
	data->error_occurred = 1;     /* Assume the worst */
...
{
	struct construct_data_st * construct_data = (struct construct_data_st *)arg;
...
{
	struct construct_data_st * data = (struct construct_data_st *)arg;
	if(data->obj == NULL) {
		OSSL_ENCODER * encoder = OSSL_ENCODER_INSTANCE_get_encoder(encoder_inst);
		...
		if(k_prov != e_prov) {
			...
			if(!evp_keymgmt_export(pk->keymgmt, pk->keydata, data->selection, &encoder_import_cb, data))
				return NULL;
 ...
 */
static int ossl_encoder_ctx_setup_for_pkey(OSSL_ENCODER_CTX * ctx, const EVP_PKEY * pkey, int selection, const char * propquery)
{
	...
	if(pkey->keymgmt != NULL) {
		...
		if((data = (struct construct_data_st *)OPENSSL_zalloc(sizeof(*data))) == NULL) {
...
OSSL_ENCODER_CTX * OSSL_ENCODER_CTX_new_for_pkey(const EVP_PKEY * pkey, int selection, const char * output_type, const char * output_struct, const char * propquery)
{
	...
	if(!evp_pkey_is_assigned(pkey)) {
		ERR_raise_data(ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_INVALID_ARGUMENT, "The passed EVP_PKEY must be assigned a key");
		return NULL;
	}
	...
	OSSL_TRACE_BEGIN(ENCODER) {
		BIO_printf(trc_out, "(ctx %p) Looking for %s encoders with selection %d\n", (void*)ctx, EVP_PKEY_get0_type_name(pkey), selection);
	...
	if(OSSL_ENCODER_CTX_set_output_type(ctx, output_type)
		...
		params[0] = OSSL_PARAM_construct_int(OSSL_ENCODER_PARAM_SAVE_PARAMETERS, &save_parameters);
