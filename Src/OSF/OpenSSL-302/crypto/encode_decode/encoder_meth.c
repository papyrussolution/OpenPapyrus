...
static OSSL_ENCODER * ossl_encoder_new(void)
{
	...
	if((encoder = (OSSL_ENCODER *)OPENSSL_zalloc(sizeof(*encoder))) == NULL || (encoder->base.lock = CRYPTO_THREAD_lock_new()) == NULL) {
...
{
	struct encoder_data_st * methdata = (struct encoder_data_st *)data;
...
{
	return (OSSL_METHOD_STORE *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_ENCODER_STORE_INDEX, &encoder_store_method);
}
...
static int put_encoder_in_store(void * store, void * method, const OSSL_PROVIDER * prov, const char * names, const char * propdef, void * data)
{
	...
	if(store == NULL && (store = get_encoder_store(methdata->libctx)) == NULL)
		return 0;
	return ossl_method_store_add((OSSL_METHOD_STORE *)store, prov, id, propdef, method, (int (*)(void *))OSSL_ENCODER_up_ref, (void (*)(void *))OSSL_ENCODER_free);
}
...
static void do_one(ossl_unused int id, void * method, void * arg)
{
	...
	data->user_fn((OSSL_ENCODER *)method, data->user_arg);
}
...
OSSL_ENCODER_CTX * OSSL_ENCODER_CTX_new(void)
{
	...
	if((ctx = (OSSL_ENCODER_CTX *)OPENSSL_zalloc(sizeof(*ctx))) == NULL)
		ERR_raise(ERR_LIB_OSSL_ENCODER, ERR_R_MALLOC_FAILURE);
	return ctx;
}
