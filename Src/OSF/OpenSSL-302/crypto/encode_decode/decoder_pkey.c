...
int OSSL_DECODER_CTX_set_passphrase(OSSL_DECODER_CTX * ctx, const uchar * kstr, size_t klen)
{
...
static int decoder_construct_pkey(OSSL_DECODER_INSTANCE * decoder_inst, const OSSL_PARAM * params, void * construct_data)
{
	struct decoder_pkey_data_st * data = (struct decoder_pkey_data_st *)construct_data;
	...
	const OSSL_PARAM * p = OSSL_PARAM_locate_const(params, OSSL_OBJECT_PARAM_DATA_TYPE);
	if(p) {
	...
	for(i = 0; i < end; i++) {
		...
		if(keymgmt_prov == decoder_prov && evp_keymgmt_has_load(keymgmt) && EVP_KEYMGMT_is_a(keymgmt, data->object_type))
			break;
	}
	if(i < end) {
	...
	else if((keymgmt = EVP_KEYMGMT_fetch(data->libctx, data->object_type, data->propq)) != NULL) {
		keymgmt_prov = EVP_KEYMGMT_get0_provider(keymgmt);
	}
	if(keymgmt) {
		EVP_PKEY * pkey = NULL;
		...
		if(keymgmt_prov == decoder_prov) {
			 ...
			 */
			(void)decoder->export_object(decoderctx, object_ref, object_ref_sz, &evp_keymgmt_util_try_import, &import_data);
			keydata = import_data.keydata;
			import_data.keydata = NULL;
		}
		if(keydata && (pkey = evp_keymgmt_util_make_pkey(keymgmt, keydata)) == NULL)
...
{
	STACK_OF(EVP_KEYMGMT) *keymgmts = (STACK_OF(EVP_KEYMGMT) *)arg;
...
{
	struct collect_decoder_data_st * data = (struct collect_decoder_data_st *)arg;
	size_t i, end_i;
	 ...
	 */
	if(decoder->does_selection && !decoder->does_selection(provctx, data->ctx->selection))
		return;
	OSSL_TRACE_BEGIN(DECODER) {
		BIO_printf(trc_out, "(ctx %p) Checking out decoder %p:\n    %s with %s\n", (void*)data->ctx, (void*)decoder,
		    OSSL_DECODER_get0_name(decoder), OSSL_DECODER_get0_properties(decoder));
...
int ossl_decoder_ctx_setup_for_pkey(OSSL_DECODER_CTX * ctx, EVP_PKEY ** pkey, const char * keytype, OSSL_LIB_CTX * libctx, const char * propquery)
{
	...
	int isecoid = 0;
	int i, end;
	if(keytype && (strcmp(keytype, "id-ecPublicKey") == 0 || strcmp(keytype, "1.2.840.10045.2.1") == 0))
		isecoid = 1;
	OSSL_TRACE_BEGIN(DECODER) {
		BIO_printf(trc_out, "(ctx %p) Looking for decoders producing %s%s%s%s%s%s\n",
		    (void*)ctx, keytype ? keytype : "", keytype ? " keys" : "keys of any type",
		    input_type ? " from " : "", input_type ? input_type : "", input_structure ? " with " : "",
		    input_structure ? input_structure : "");
	} OSSL_TRACE_END(DECODER);
	if((process_data = (struct decoder_pkey_data_st *)OPENSSL_zalloc(sizeof(*process_data))) == NULL
	    || (propquery && (process_data->propq = OPENSSL_strdup(propquery)) == NULL)
...
OSSL_DECODER_CTX * OSSL_DECODER_CTX_new_for_pkey(EVP_PKEY ** pkey,
    ...
    OSSL_LIB_CTX * libctx, const char * propquery)
{
	...
	OSSL_TRACE_BEGIN(DECODER) {
		BIO_printf(trc_out, "(ctx %p) Looking for %s decoders with selection %d\n", (void*)ctx, keytype, selection);
