...
int OSSL_DECODER_from_bio(OSSL_DECODER_CTX * ctx, BIO * in)
{
	...
	if(!data.flag_construct_called) {
		const char * spaces = (ctx->start_input_type && ctx->input_structure) ? " " : "";
		const char * input_type_label = ctx->start_input_type ? "Input type: " : "";
		const char * input_structure_label = ctx->input_structure ? "Input structure: " : "";
		const char * comma = (ctx->start_input_type && ctx->input_structure) ? ", " : "";
		const char * input_type = ctx->start_input_type ? ctx->start_input_type : "";
		const char * input_structure = ctx->input_structure ? ctx->input_structure : "";
		if(ERR_peek_last_error() == lasterr || ERR_peek_error() == 0)
			/* Prevent spurious decoding error but add at least something */
			ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_UNSUPPORTED, "No supported data to decode. %s%s%s%s%s%s",
			    spaces, input_type_label, input_type, comma, input_structure_label, input_structure);
		ok = 0;
	}
...
int OSSL_DECODER_from_data(OSSL_DECODER_CTX * ctx, const uchar ** pdata,
    size_t * pdata_len)
{
	BIO * membio;
	int ret = 0;
...
OSSL_DECODER_INSTANCE * ossl_decoder_instance_new(OSSL_DECODER * decoder, void * decoderctx)
{
	...
	if((decoder_inst = (OSSL_DECODER_INSTANCE *)OPENSSL_zalloc(sizeof(*decoder_inst))) == NULL) {
	...
	if(props == NULL) {
		ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION, "there are no property definitions with decoder %s", OSSL_DECODER_get0_name(decoder));
		goto err;
	}
	...
	if(decoder_inst->input_type == NULL) {
		ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION, "the mandatory 'input' property is missing for decoder %s (properties: %s)",
...
{
	STACK_OF(OSSL_DECODER) *skdecoders = (STACK_OF(OSSL_DECODER) *)arg;
	if(OSSL_DECODER_up_ref(decoder))
...
{
	struct collect_extra_decoder_data_st * data = (struct collect_extra_decoder_data_st *)arg;
	size_t j;
	...
	if(OSSL_DECODER_is_a(decoder, data->output_type)) {
		...
		switch(data->type_check) {
			case collect_extra_decoder_data_st::IS_SAME:
			    ...
			    break;
			case collect_extra_decoder_data_st::IS_DIFFERENT:
    ...
    OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	do {
		 ...
		 */
		for(data.type_check = collect_extra_decoder_data_st::IS_SAME; data.type_check <= collect_extra_decoder_data_st::IS_DIFFERENT; 
			CENUMINCR(collect_extra_decoder_data_st::TypeCheck, data.type_check)) {
...
{
	return ctx ? ctx->construct : NULL;
}

void * OSSL_DECODER_CTX_get_construct_data(OSSL_DECODER_CTX * ctx)
{
	return ctx ? ctx->construct_data : NULL;
}

OSSL_DECODER_CLEANUP * OSSL_DECODER_CTX_get_cleanup(OSSL_DECODER_CTX * ctx)
{
	return ctx ? ctx->cleanup : NULL;
}
...
static int decoder_process(const OSSL_PARAM params[], void * arg)
{
	struct decoder_process_data_st * data = (struct decoder_process_data_st *)arg;
...
#define LEVEL (new_data.recursion < sizeof(LEVEL_STR) ? &LEVEL_STR[sizeof(LEVEL_STR) - new_data.recursion - 1] : LEVEL_STR "...")
	if(!params) {
		...
		if(p && !OSSL_PARAM_get_utf8_string_ptr(p, &data_structure))
			goto end;
	...
	for(i = data->current_decoder_inst_index; i-- > 0;) {
		 ...
		 */
		if(decoder == NULL && ctx->start_input_type && strcasecmp(ctx->start_input_type, new_input_type) != 0) {
			OSSL_TRACE_BEGIN(DECODER) {
				BIO_printf(trc_out,
		 ...
		 */
		if(decoder && !OSSL_DECODER_is_a(decoder, new_input_type)) {
			OSSL_TRACE_BEGIN(DECODER) {
				BIO_printf(trc_out,
		 ...
		 */
		if(data_type && !OSSL_DECODER_is_a(new_decoder, data_type)) {
			OSSL_TRACE_BEGIN(DECODER) {
				BIO_printf(trc_out,
		 ...
		 */
		if(data_structure && (new_input_structure == NULL || strcasecmp(data_structure, new_input_structure) != 0)) {
			OSSL_TRACE_BEGIN(DECODER) {
				BIO_printf(trc_out, "(ctx %p) %s [%u] the previous decoder's data structure doesn't match the input structure of the considered decoder, skipping...\n",
				    (void*)new_data.ctx, LEVEL, (unsigned int)i);
			} OSSL_TRACE_END(DECODER);
			continue;
		}
		 ...
		 */
		if(!data->flag_input_structure_checked && ctx->input_structure && new_input_structure) {
			data->flag_input_structure_checked = 1;
			if(strcasecmp(new_input_structure, ctx->input_structure) != 0) {
				OSSL_TRACE_BEGIN(DECODER) {
					BIO_printf(trc_out, "(ctx %p) %s [%u] the previous decoder's data structure doesn't match the input structure given by the user, skipping...\n",
					    (void*)new_data.ctx, LEVEL, (unsigned int)i);
				} OSSL_TRACE_END(DECODER);
				continue;
			}
