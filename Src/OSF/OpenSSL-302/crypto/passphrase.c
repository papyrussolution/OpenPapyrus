...
{
	if(data) {
		if(data->type == ossl_passphrase_data_st::is_expl_passphrase)
			OPENSSL_clear_free(data->_.expl_passphrase.passphrase_copy, data->_.expl_passphrase.passphrase_len);
...
int ossl_pw_set_passphrase(struct ossl_passphrase_data_st * data, const uchar * passphrase, size_t passphrase_len)
{
	...
	data->type = ossl_passphrase_data_st::is_expl_passphrase;
	data->_.expl_passphrase.passphrase_copy =
	    (char*)(passphrase_len != 0 ? OPENSSL_memdup(passphrase, passphrase_len) : OPENSSL_malloc(1));
...
int ossl_pw_set_pem_password_cb(struct ossl_passphrase_data_st * data, pem_password_cb * cb, void * cbarg)
{
	...
	ossl_pw_clear_passphrase_data(data);
	data->type = ossl_passphrase_data_st::is_pem_password;
	data->_.pem_password.password_cb = cb;
	data->_.pem_password.password_cbarg = cbarg;
	return 1;
}

int ossl_pw_set_ossl_passphrase_cb(struct ossl_passphrase_data_st * data, OSSL_PASSPHRASE_CALLBACK * cb, void * cbarg)
{
	if(!ossl_assert(data != NULL && cb != NULL)) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_PASSED_NULL_PARAMETER);
		return 0;
	...
	ossl_pw_clear_passphrase_data(data);
	data->type = ossl_passphrase_data_st::is_ossl_passphrase;
	data->_.ossl_passphrase.passphrase_cb = cb;
	data->_.ossl_passphrase.passphrase_cbarg = cbarg;
	return 1;
}

int ossl_pw_set_ui_method(struct ossl_passphrase_data_st * data, const UI_METHOD * ui_method, void * ui_data)
{
	...
	ossl_pw_clear_passphrase_data(data);
	data->type = ossl_passphrase_data_st::is_ui_method;
 ...
 */
static int do_ui_passphrase(char * pass, size_t pass_size, size_t * pass_len, const char * prompt_info, int verify, const UI_METHOD * ui_method, void * ui_data)
{
	...
	ipass = (char*)OPENSSL_zalloc(pass_size + 1);
	if(ipass == NULL) {
		ERR_raise(ERR_LIB_CRYPTO, ERR_R_MALLOC_FAILURE);
		goto end;
	}
	...
	if(verify) {
		/* Get a buffer for verification prompt */
		vpass = (char*)OPENSSL_zalloc(pass_size + 1);
		if(vpass == NULL) {
			ERR_raise(ERR_LIB_CRYPTO, ERR_R_MALLOC_FAILURE);
			goto end;
		}
...
int ossl_pw_get_passphrase(char * pass, size_t pass_size, size_t * pass_len, const OSSL_PARAM params[], int verify, struct ossl_passphrase_data_st * data)
{
	...
	if(data->type == ossl_passphrase_data_st::is_ossl_passphrase) {
	...
	if((p = OSSL_PARAM_locate_const(params, OSSL_PASSPHRASE_PARAM_INFO)) != NULL) {
		if(p->data_type != OSSL_PARAM_UTF8_STRING) {
			ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT, "Prompt info data type incorrect");
			return 0;
		}
		prompt_info = (const char *)p->data;
	}
	if(data->type == ossl_passphrase_data_st::is_pem_password) {
	...
	else if(data->type == ossl_passphrase_data_st::is_ui_method) {
...
do_cache:
	if(ret && data->flag_cache_passphrase) {
		if(data->cached_passphrase == NULL
			...
			data->cached_passphrase = (char*)new_cache;
		}
    ...
    void * userdata, const char * info)
{
	...
	if(ossl_pw_get_passphrase(buf, (size_t)size, &password_len, params, rwflag, (struct ossl_passphrase_data_st *)userdata))
...
int ossl_pw_passphrase_callback_enc(char * pass, size_t pass_size, size_t * pass_len, const OSSL_PARAM params[], void * arg)
{
	return ossl_pw_get_passphrase(pass, pass_size, pass_len, params, 1, (struct ossl_passphrase_data_st *)arg);
}

int ossl_pw_passphrase_callback_dec(char * pass, size_t pass_size, size_t * pass_len, const OSSL_PARAM params[], void * arg)
{
	return ossl_pw_get_passphrase(pass, pass_size, pass_len, params, 0, (struct ossl_passphrase_data_st *)arg);
}
