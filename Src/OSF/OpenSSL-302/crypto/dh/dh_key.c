...
int ossl_dh_buf2key(DH * dh, const uchar * buf, size_t len)
{
...
size_t ossl_dh_key2buf(const DH * dh, uchar ** pbuf_out, size_t size, int alloc)
{
	const BIGNUM * pubkey;
	uchar * pbuf = NULL;
	const BIGNUM * p;
	...
	if(pbuf_out != NULL && (alloc || *pbuf_out != NULL)) {
		if(!alloc) {
			...
			pbuf = (uchar *)OPENSSL_malloc(p_size);
		}
