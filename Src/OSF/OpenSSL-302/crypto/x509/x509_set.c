...
{
	return x ? ossl_x509_set1_time(&x->cert_info.validity.notBefore, tm) : 0;
}

int X509_set1_notAfter(X509 * x, const ASN1_TIME * tm)
{
	return x ? ossl_x509_set1_time(&x->cert_info.validity.notAfter, tm) : 0;
}
...
{
	ASSIGN_PTR(mdnid, siginf->mdnid);
	ASSIGN_PTR(pknid, siginf->pknid);
	ASSIGN_PTR(secbits, siginf->secbits);
	ASSIGN_PTR(flags, siginf->flags);
...
static int x509_sig_info_init(X509_SIG_INFO * siginf, const X509_ALGOR * alg, const ASN1_STRING * sig)
{
	int pknid, mdnid;
	const EVP_MD * md;
