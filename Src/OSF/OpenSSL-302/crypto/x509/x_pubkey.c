...
X509_PUBKEY *ossl_d2i_X509_PUBKEY_INTERNAL(const uchar ** pp, long len, OSSL_LIB_CTX *libctx)
{
	X509_PUBKEY * xpub = (X509_PUBKEY *)OPENSSL_zalloc(sizeof(*xpub));
	if(xpub == NULL)
		return NULL;
	return (X509_PUBKEY*)ASN1_item_d2i_ex((ASN1_VALUE**)&xpub, pp, len, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), libctx, NULL);
}
...
static int x509_pubkey_ex_populate(ASN1_VALUE ** pval, const ASN1_ITEM * it)
{
	...
	return (pubkey->algor != NULL || (pubkey->algor = X509_ALGOR_new()) != NULL) && (pubkey->public_key != NULL || (pubkey->public_key = ASN1_BIT_STRING_new()) != NULL);
}

static int x509_pubkey_ex_new_ex(ASN1_VALUE ** pval, const ASN1_ITEM * it, OSSL_LIB_CTX * libctx, const char * propq)
{
	X509_PUBKEY * ret;
	if((ret = (X509_PUBKEY *)OPENSSL_zalloc(sizeof(*ret))) == NULL || !x509_pubkey_ex_populate((ASN1_VALUE**)&ret, NULL) || 
...
static int x509_pubkey_ex_d2i_ex(ASN1_VALUE ** pval, const uchar ** in, long len, const ASN1_ITEM * it, int tag, int aclass,
    char opt, ASN1_TLC * ctx, OSSL_LIB_CTX * libctx, const char * propq)
{
	const uchar * in_saved = *in;
	size_t publen;
	X509_PUBKEY * pubkey;
	int ret;
	...
	if((ret = ASN1_item_ex_d2i(pval, in, len, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), tag, aclass, opt, ctx)) <= 0)
		return ret;
	publen = *in - in_saved;
	...
	if(ret <= 0 && !pubkey->flag_force_legacy) {
		...
		if(aclass != V_ASN1_UNIVERSAL) {
			tmpbuf = (uchar *)OPENSSL_memdup(in_saved, publen);
			if(tmpbuf == NULL) {
				ERR_raise(ERR_LIB_ASN1, ERR_R_MALLOC_FAILURE);
				return 0;
			}
		...
		p = in_saved;
		if(OBJ_obj2txt(txtoidname, sizeof(txtoidname), pubkey->algor->algorithm, 0) <= 0) {
			ERR_clear_last_mark();
			goto end;
		}
		if((dctx = OSSL_DECODER_CTX_new_for_pkey(&pubkey->pkey, "DER", "SubjectPublicKeyInfo", txtoidname, EVP_PKEY_PUBLIC_KEY, pubkey->libctx, pubkey->propq)) != NULL)
			/*
...
static int x509_pubkey_ex_i2d(const ASN1_VALUE ** pval, uchar ** out, const ASN1_ITEM * it, int tag, int aclass)
{
	return ASN1_item_ex_i2d(pval, out, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), tag, aclass);
}

static int x509_pubkey_ex_print(BIO * out, const ASN1_VALUE ** pval, int indent, const char * fname, const ASN1_PCTX * pctx)
{
	return ASN1_item_print(out, *pval, indent, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), pctx);
}
...
{
	X509_PUBKEY * pubkey = (X509_PUBKEY*)ASN1_item_new_ex(X509_PUBKEY_it(), libctx, propq);
...
{
	X509_PUBKEY * pubkey = (X509_PUBKEY *)OPENSSL_zalloc(sizeof(*pubkey));
	if(pubkey == NULL || !x509_pubkey_set0_libctx(pubkey, a->libctx, a->propq) || 
		(pubkey->algor = X509_ALGOR_dup(a->algor)) == NULL || (pubkey->public_key = ASN1_BIT_STRING_new()) == NULL || 
		!ASN1_BIT_STRING_set(pubkey->public_key, a->public_key->data, a->public_key->length)) {
		x509_pubkey_ex_free((ASN1_VALUE**)&pubkey, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL));
...
int X509_PUBKEY_set(X509_PUBKEY ** x, EVP_PKEY * pkey)
{
	...
	if(pkey->ameth != NULL) {
		...
		uchar * der = NULL;
		size_t derlen = 0;
		OSSL_ENCODER_CTX * ectx = OSSL_ENCODER_CTX_new_for_pkey(pkey, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
		if(OSSL_ENCODER_to_data(ectx, &der, &derlen)) {
			const uchar * pder = der;
...
{
	EVP_PKEY * pkey;
	int nid = OBJ_obj2nid(key->algor->algorithm);
	if(!key->flag_force_legacy) {
#ifndef OPENSSL_NO_ENGINE
		ENGINE * e = ENGINE_get_pkey_meth_engine(nid);
		if(!e)
			return 0;
		ENGINE_finish(e);
#else
 ...
 */
static EVP_PKEY * d2i_PUBKEY_int(EVP_PKEY ** a, const uchar ** pp, long length, OSSL_LIB_CTX * libctx, const char * propq,
    unsigned int force_legacy, X509_PUBKEY * (*d2i_x509_pubkey)(X509_PUBKEY **a, const uchar ** in, long len))
{
	...
	if(libctx != NULL || propq != NULL || force_legacy) {
		xpk2 = (X509_PUBKEY *)OPENSSL_zalloc(sizeof(*xpk2));
		if(xpk2 == NULL) {
			ERR_raise(ERR_LIB_X509, ERR_R_MALLOC_FAILURE);
			return NULL;
		}
...
EVP_PKEY * ossl_d2i_PUBKEY_legacy(EVP_PKEY ** a, const uchar ** pp, long length)
{
	return d2i_PUBKEY_int(a, pp, length, NULL, NULL, 1, d2i_X509_PUBKEY);
}

EVP_PKEY * d2i_PUBKEY_ex(EVP_PKEY ** a, const uchar ** pp, long length, OSSL_LIB_CTX * libctx, const char * propq)
{
	return d2i_PUBKEY_int(a, pp, length, libctx, propq, 0, d2i_X509_PUBKEY);
}

EVP_PKEY * d2i_PUBKEY(EVP_PKEY ** a, const uchar ** pp, long length)
{
	return d2i_PUBKEY_ex(a, pp, length, NULL, NULL);
}

int i2d_PUBKEY(const EVP_PKEY * a, uchar ** pp)
{
	int ret = -1;
	if(!a)
		return 0;
	if(a->ameth != NULL) {
		...
		OSSL_ENCODER_CTX * ctx = OSSL_ENCODER_CTX_new_for_pkey(a, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
		BIO * out = BIO_new(BIO_s_mem());
		BUF_MEM * buf = NULL;
		if(OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0 && out != NULL && OSSL_ENCODER_to_bio(ctx, out) && BIO_get_mem_ptr(out, &buf) > 0) {
			ret = static_cast<int>(buf->length);
			if(pp != NULL) {
				if(*pp == NULL) {
					*pp = (uchar *)buf->data;
					buf->length = 0;
 ...
 */
RSA * d2i_RSA_PUBKEY(RSA ** a, const uchar ** pp, long length)
{
	RSA * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
	if(!pkey)
		return NULL;
...
int i2d_RSA_PUBKEY(const RSA * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
DH * ossl_d2i_DH_PUBKEY(DH ** a, const uchar ** pp, long length)
{
	DH * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_DH_PUBKEY(const DH * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
DH * ossl_d2i_DHx_PUBKEY(DH ** a, const uchar ** pp, long length)
{
	DH * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_DHx_PUBKEY(const DH * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
DSA * d2i_DSA_PUBKEY(DSA ** a, const uchar ** pp, long length)
{
	DSA * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int i2d_DSA_PUBKEY(const DSA * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
EC_KEY * d2i_EC_PUBKEY(EC_KEY ** a, const uchar ** pp, long length)
{
	EC_KEY * key = NULL;
	int type;
	const uchar * q = *pp;
...
int i2d_EC_PUBKEY(const EC_KEY * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
ECX_KEY * ossl_d2i_ED25519_PUBKEY(ECX_KEY ** a, const uchar ** pp, long length)
{
	ECX_KEY * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_ED25519_PUBKEY(const ECX_KEY * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
ECX_KEY * ossl_d2i_ED448_PUBKEY(ECX_KEY ** a, const uchar ** pp, long length)
{
	ECX_KEY * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_ED448_PUBKEY(const ECX_KEY * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
ECX_KEY * ossl_d2i_X25519_PUBKEY(ECX_KEY ** a, const uchar ** pp, long length)
{
	ECX_KEY * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_X25519_PUBKEY(const ECX_KEY * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
ECX_KEY * ossl_d2i_X448_PUBKEY(ECX_KEY ** a, const uchar ** pp, long length)
{
	ECX_KEY * key = NULL;
	const uchar * q = *pp;
	EVP_PKEY * pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
...
int ossl_i2d_X448_PUBKEY(const ECX_KEY * a, uchar ** pp)
{
	EVP_PKEY * pktmp;
	int ret;
	if(!a)
...
int X509_PUBKEY_get0_param(ASN1_OBJECT ** ppkalg, const uchar ** pk, int * ppklen, X509_ALGOR ** pa, const X509_PUBKEY * pub)
{
	if(ppkalg)
...
{
	return x ? x->cert_info.key->public_key : NULL;
}
...
int X509_PUBKEY_eq(const X509_PUBKEY * a, const X509_PUBKEY * b)
{
	...
	if(!a || !b)
		return 0;
	if(!X509_PUBKEY_get0_param(NULL, NULL, NULL, &algA, a) || algA == NULL || !X509_PUBKEY_get0_param(NULL, NULL, NULL, &algB, b) || algB == NULL)
		return -2;
	if(X509_ALGOR_cmp(algA, algB) != 0)
		return 0;
	if((pA = X509_PUBKEY_get0(a)) == NULL || (pB = X509_PUBKEY_get0(b)) == NULL)
		return -2;
	return EVP_PKEY_eq(pA, pB);
}

int ossl_x509_PUBKEY_get0_libctx(OSSL_LIB_CTX ** plibctx, const char ** ppropq, const X509_PUBKEY * key)
{
	if(plibctx)
		*plibctx = key->libctx;
	if(ppropq)
