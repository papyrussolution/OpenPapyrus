    ...
    STACK_OF(CONF_VALUE) * values)
{
	...
	if(ctx && (ctx->flags & X509V3_CTX_TEST) != 0)
		return akeyid;
	...
	if(keyid == 2 || (keyid == 1 && !ss)) {
		...
		if(i >= 0 && (ext = X509_get_ext(issuer_cert, i)) != NULL && !(same_issuer && !ss)) 
		...
		if((keyid == 2 || issuer == 0) && (ikeyid == NULL || ASN1_STRING_length(ikeyid) <= 2) /* indicating "none" */) {
