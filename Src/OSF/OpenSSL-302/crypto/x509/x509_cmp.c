...
unsigned long X509_issuer_and_serial_hash(X509 * a)
{
	...
	if(!EVP_DigestUpdate(ctx, (uchar *)f, strlen(f)))
		goto err;
	OPENSSL_free(f);
	if(!EVP_DigestUpdate(ctx, (uchar *)a->cert_info.serialNumber.data, (ulong)a->cert_info.serialNumber.length))
		goto err;
	if(!EVP_DigestFinal_ex(ctx, &(md[0]), NULL))
		goto err;
	ret = (((ulong)md[0]) | ((ulong)md[1] << 8L) | ((ulong)md[2] << 16L) | ((ulong)md[3] << 24L)) & 0xffffffffL;
err:
	EVP_MD_free(digest);
	EVP_MD_CTX_free(ctx);
	return ret;
...
/* compiler would allow 'const' for the certs, yet they may get up-ref'ed */
{
	const int n = sk_X509_num(certs /* may be NULL */);
	for(int i = 0; i < n; i++) {
...
unsigned long X509_NAME_hash_ex(const X509_NAME * x, OSSL_LIB_CTX * libctx, const char * propq, int * ok)
{
	...
	if(ok)
		*ok = 0;
	if(sha1 != NULL && EVP_Digest(x->canon_enc, x->canon_enclen, md, NULL, sha1, NULL)) {
		ret = (((ulong)md[0]) | ((ulong)md[1] << 8L) | ((ulong)md[2] << 16L) | ((ulong)md[3] << 24L)) & 0xffffffffL;
		if(ok)
			*ok = 1;
	}
...
EVP_PKEY * X509_get0_pubkey(const X509 * x) { return x ? X509_PUBKEY_get0(x->cert_info.key) : NULL; }
EVP_PKEY * X509_get_pubkey(X509 * x) { return x ? X509_PUBKEY_get(x->cert_info.key) : NULL; }
