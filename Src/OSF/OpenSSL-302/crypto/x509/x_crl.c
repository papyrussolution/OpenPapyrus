    ...
    void * exarg)
{
	...
	switch(operation) {
		...
		case ASN1_OP_D2I_POST:
		    ...
		    crl->idp = (ISSUING_DIST_POINT *)X509_CRL_get_ext_d2i(crl, NID_issuing_distribution_point, &i, NULL);
		    if(crl->idp != NULL) {
		    ...
		    crl->akid = (AUTHORITY_KEYID *)X509_CRL_get_ext_d2i(crl, NID_authority_key_identifier, &i, NULL);
		    if(crl->akid == NULL && i != -1)
			    crl->flags |= EXFLAG_INVALID;
		    crl->crl_number = (ASN1_INTEGER *)X509_CRL_get_ext_d2i(crl, NID_crl_number, &i, NULL);
		    if(crl->crl_number == NULL && i != -1)
			    crl->flags |= EXFLAG_INVALID;
		    crl->base_crl_number = (ASN1_INTEGER *)X509_CRL_get_ext_d2i(crl, NID_delta_crl, &i, NULL);
