...
static int verify_cb_cert(X509_STORE_CTX * ctx, X509 * x, int depth, int err)
{
	...
	ctx->current_cert = x ? x : sk_X509_value(ctx->chain, depth);
	if(err != X509_V_OK)
		ctx->error = err;
...
static int verify_chain(X509_STORE_CTX * ctx)
{
	...
	ok = ctx->verify ? ctx->verify(ctx) : internal_verify(ctx);
	if(ok <= 0)
		return ok;
...
static int get_issuer_sk(X509 ** issuer, X509_STORE_CTX * ctx, X509 * x)
{
	*issuer = (X509 *)find_issuer(ctx, (STACK_OF(X509) *)ctx->other_ctx, x);
	if(*issuer)
...
static int check_extensions(X509_STORE_CTX * ctx)
{
	...
	for(i = 0; i < num; i++) {
		...
		if((ctx->param->flags & X509_V_FLAG_X509_STRICT) != 0
			...
			CB_FAIL_IF(((x->ex_flags & EXFLAG_CA) != 0 || (x->ex_kusage & KU_CRL_SIGN) != 0 || x->altname == NULL) && X509_NAME_entry_count(X509_get_subject_name(x)) == 0,
			    ctx, x, i, X509_V_ERR_SUBJECT_NAME_EMPTY);
			CB_FAIL_IF(X509_NAME_entry_count(X509_get_subject_name(x)) == 0 && x->altname && (x->ex_flags & EXFLAG_SAN_CRITICAL) == 0, ctx, x, i, X509_V_ERR_EMPTY_SUBJECT_SAN_NOT_CRITICAL);
			/* Check SAN is non-empty according to RFC 5280 section 4.2.1.6 */
			CB_FAIL_IF(x->altname && sk_GENERAL_NAME_num(x->altname) <= 0, ctx, x, i, X509_V_ERR_EMPTY_SUBJECT_ALT_NAME);
			...
			CB_FAIL_IF(x->akid && (x->ex_flags & EXFLAG_AKID_CRITICAL) != 0, ctx, x, i, X509_V_ERR_AUTHORITY_KEY_IDENTIFIER_CRITICAL);
			CB_FAIL_IF(x->skid && (x->ex_flags & EXFLAG_SKID_CRITICAL) != 0, ctx, x, i, X509_V_ERR_SUBJECT_KEY_IDENTIFIER_CRITICAL);
...
static int internal_verify(X509_STORE_CTX * ctx)
{
	...
	while(n >= 0) {
		 ...
		 */
		if(xi && (xs != xi || ((ctx->param->flags & X509_V_FLAG_CHECK_SS_SIGNATURE) != 0 && (xi->ex_flags & EXFLAG_SS) != 0))) {
			EVP_PKEY * pkey;
			/*
    ...
    const ASN1_TIME * start, const ASN1_TIME * end)
{
	...
	if(end && X509_cmp_time(end, time) < 0)
		return 1;
	if(start && X509_cmp_time(start, time) > 0)
		return -1;
	return 0;
}
...
ASN1_TIME * X509_time_adj_ex(ASN1_TIME * s, int offset_day, long offset_sec, time_t * in_tm)
{
	...
	if(in_tm)
		...
		time(&t);
	if(s && (s->flags & ASN1_STRING_FLAG_MSTRING) == 0) {
    ...
    EVP_PKEY * skey, const EVP_MD * md, unsigned int flags)
{
	...
	for(i = 0; i < sk_X509_REVOKED_num(revs); i++) {
		X509_REVOKED * rvtmp;
		X509_REVOKED * rvn = sk_X509_REVOKED_value(revs, i);
		/*
...
void * X509_STORE_CTX_get_ex_data(const X509_STORE_CTX * ctx, int idx) { return CRYPTO_get_ex_data(&ctx->ex_data, idx); }
...
int X509_STORE_CTX_get_error_depth(const X509_STORE_CTX * ctx) { return ctx->error_depth; }
void X509_STORE_CTX_set_error_depth(X509_STORE_CTX * ctx, int depth) { ctx->error_depth = depth; }
X509 * X509_STORE_CTX_get_current_cert(const X509_STORE_CTX * ctx) { return ctx->current_cert; }
void X509_STORE_CTX_set_current_cert(X509_STORE_CTX * ctx, X509 * x) { ctx->current_cert = x; }
STACK_OF(X509) *X509_STORE_CTX_get0_chain(const X509_STORE_CTX *ctx) { return ctx->chain; }
STACK_OF(X509) *X509_STORE_CTX_get1_chain(const X509_STORE_CTX *ctx) { return ctx->chain ? X509_chain_up_ref(ctx->chain) : NULL; }
X509 * X509_STORE_CTX_get0_current_issuer(const X509_STORE_CTX * ctx) { return ctx->current_issuer; }
X509_CRL * X509_STORE_CTX_get0_current_crl(const X509_STORE_CTX * ctx) { return ctx->current_crl; }
X509_STORE_CTX * X509_STORE_CTX_get0_parent_ctx(const X509_STORE_CTX * ctx) { return ctx->parent; }
void X509_STORE_CTX_set_cert(X509_STORE_CTX * ctx, X509 * x) { ctx->cert = x; }
void X509_STORE_CTX_set0_crls(X509_STORE_CTX * ctx, STACK_OF(X509_CRL) * sk) { ctx->crls = sk; }
...
{
	/*
	 * We need to be idempotent because, unfortunately, SAlloc::F() also calls
	 * cleanup(), so the natural call sequence new(), init(), cleanup(), SAlloc::F()
...
void X509_STORE_CTX_set_verify_cb(X509_STORE_CTX * ctx, X509_STORE_CTX_verify_cb verify_cb) { ctx->verify_cb = verify_cb; }
X509_STORE_CTX_verify_cb X509_STORE_CTX_get_verify_cb(const X509_STORE_CTX * ctx) { return ctx->verify_cb; }
void X509_STORE_CTX_set_verify(X509_STORE_CTX * ctx, X509_STORE_CTX_verify_fn verify) { ctx->verify = verify; }
X509_STORE_CTX_verify_fn X509_STORE_CTX_get_verify(const X509_STORE_CTX * ctx) { return ctx->verify; }
X509_STORE_CTX_get_issuer_fn X509_STORE_CTX_get_get_issuer(const X509_STORE_CTX * ctx) { return ctx->get_issuer; }
X509_STORE_CTX_check_issued_fn X509_STORE_CTX_get_check_issued(const X509_STORE_CTX * ctx) { return ctx->check_issued; }
X509_STORE_CTX_check_revocation_fn X509_STORE_CTX_get_check_revocation(const X509_STORE_CTX * ctx) { return ctx->check_revocation; }
X509_STORE_CTX_get_crl_fn X509_STORE_CTX_get_get_crl(const X509_STORE_CTX * ctx) { return ctx->get_crl; }
X509_STORE_CTX_check_crl_fn X509_STORE_CTX_get_check_crl(const X509_STORE_CTX * ctx) { return ctx->check_crl; }
X509_STORE_CTX_cert_crl_fn X509_STORE_CTX_get_cert_crl(const X509_STORE_CTX * ctx) { return ctx->cert_crl; }
X509_STORE_CTX_check_policy_fn X509_STORE_CTX_get_check_policy(const X509_STORE_CTX * ctx) { return ctx->check_policy; }
X509_STORE_CTX_lookup_certs_fn X509_STORE_CTX_get_lookup_certs(const X509_STORE_CTX * ctx) { return ctx->lookup_certs; }
X509_STORE_CTX_lookup_crls_fn X509_STORE_CTX_get_lookup_crls(const X509_STORE_CTX * ctx) { return ctx->lookup_crls; }
X509_STORE_CTX_cleanup_fn X509_STORE_CTX_get_cleanup(const X509_STORE_CTX * ctx) { return ctx->cleanup; }
X509_POLICY_TREE * X509_STORE_CTX_get0_policy_tree(const X509_STORE_CTX * ctx) { return ctx->tree; }
int X509_STORE_CTX_get_explicit_policy(const X509_STORE_CTX * ctx) { return ctx->explicit_policy; }
int X509_STORE_CTX_get_num_untrusted(const X509_STORE_CTX * ctx) { return ctx->num_untrusted; }
...
static int check_dane_issuer(X509_STORE_CTX * ctx, int depth)
{
	...
	if(cert && (matched = dane_match(ctx, cert, depth)) < 0)
		return matched;
	if(matched > 0) {
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
	 ...
	 */
	search = ctx->untrusted ? S_DOUNTRUSTED : 0;
	 ...
	 */
	if(DANETLS_ENABLED(dane) && dane->certs && !X509_add_certs(sk_untrusted, dane->certs, X509_ADD_FLAG_DEFAULT))
		goto memerr;
	/*
...
static int check_curve(X509 * cert)
{
	...
	if(EVP_PKEY_get_id(pkey) == EVP_PKEY_EC) {
		int val;
		int ret = EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_EC_DECODED_FROM_EXPLICIT_PARAMS, &val);
