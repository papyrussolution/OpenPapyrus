...
static X509_EXTENSION * X509V3_EXT_nconf_int(CONF * conf, X509V3_CTX * ctx, const char * section, const char * name, const char * value)
{
	int crit;
	int ext_type;
	X509_EXTENSION * ret;
...
static X509_EXTENSION * do_ext_i2d(const X509V3_EXT_METHOD * method, int ext_nid, int crit, void * ext_struc)
{
	...
	if(method->it) {
		ext_der = NULL;
		ext_len = ASN1_item_i2d((const ASN1_VALUE *)ext_struc, &ext_der, ASN1_ITEM_ptr(method->it));
		if(ext_len < 0)
			goto merr;
	}
	else {
