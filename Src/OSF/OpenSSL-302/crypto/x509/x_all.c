    ...
    int timeout, const ASN1_ITEM * it)
{
	...
	ASN1_VALUE * res = (ASN1_VALUE *)ASN1_item_d2i_bio(it, mem, NULL);
	BIO_free(mem);
	return res;
}
...
PKCS7 * d2i_PKCS7_fp(FILE * fp, PKCS7 ** p7)
{
	...
	ret = (PKCS7 *)ASN1_item_d2i_fp_ex(ASN1_ITEM_rptr(PKCS7), fp, p7, libctx, propq);
	if(ret)
...
PKCS7 * d2i_PKCS7_bio(BIO * bp, PKCS7 ** p7)
{
	...
	ret = (PKCS7 *)ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(PKCS7), bp, p7, libctx, propq);
	if(ret)
		ossl_pkcs7_resolve_libctx(ret);
	return ret;
}
...
X509_REQ * d2i_X509_REQ_bio(BIO * bp, X509_REQ ** req)
{
	...
	return (X509_REQ *)ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(X509_REQ), bp, req, libctx, propq);
}
...
int X509_digest(const X509 * cert, const EVP_MD * md, uchar * data,
    unsigned int * len)
{
    ...
    EVP_MD ** md_used, int * md_is_fallback)
{
	...
	if(!X509_digest(cert, md, hash, &len) || (p_new = ASN1_OCTET_STRING_new()) == NULL)
		goto err;
	if((ASN1_OCTET_STRING_set(p_new, hash, len))) {
		if(md_used != NULL)
			*md_used = md;
		else
			EVP_MD_free(md);
...
int X509_CRL_digest(const X509_CRL * data, const EVP_MD * type, uchar * md, unsigned int * len)
{
	if(type == NULL) {
		ERR_raise(ERR_LIB_X509, ERR_R_PASSED_NULL_PARAMETER);
		return 0;
...
EVP_PKEY * d2i_PrivateKey_ex_bio(BIO * bp, EVP_PKEY ** a, OSSL_LIB_CTX * libctx, const char * propq)
{
	...
	const uchar * p;
	void * ret = NULL;
	int len = asn1_d2i_read_bio(bp, &b);
	if(len < 0)
		goto err;
	p = (uchar *)b->data;
