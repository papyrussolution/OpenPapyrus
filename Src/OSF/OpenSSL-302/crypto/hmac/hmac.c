...
int HMAC_Init_ex(HMAC_CTX * ctx, const void * key, int len, const EVP_MD * md, ENGINE * impl)
{
	...
	if(key) {
		...
		if(j < 0)
			return 0;
		if(j < len) {
			if(!EVP_DigestInit_ex(ctx->md_ctx, md, impl) || !EVP_DigestUpdate(ctx->md_ctx, key, len) || !EVP_DigestFinal_ex(ctx->md_ctx, keytmp, &keytmp_length))
				return 0;
		}
		else {
...
int HMAC_Final(HMAC_CTX * ctx, uchar * md, unsigned int * len)
{
	unsigned int i;
...
uchar * HMAC(const EVP_MD * evp_md, const void * key, int key_len,
    const uchar * data, size_t data_len, uchar * md, unsigned int * md_len)
{
