...
{
	struct child_prov_globals * gbl = (struct child_prov_globals *)vgbl;
...
static int ossl_child_provider_init(const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * in, const OSSL_DISPATCH ** out, void ** provctx)
{
	...
	gbl = (struct child_prov_globals *)ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
	if(gbl == NULL)
		return 0;
...
{
	OSSL_LIB_CTX * ctx = (OSSL_LIB_CTX*)cbdata;
	struct child_prov_globals * gbl;
	const char * provname;
	OSSL_PROVIDER * cprov;
	int ret = 0;
...
int ossl_provider_init_as_child(OSSL_LIB_CTX * ctx, const OSSL_CORE_HANDLE * handle, const OSSL_DISPATCH * in)
{
	struct child_prov_globals * gbl;
	if(!ctx)
		return 0;
	...
	for(; in->function_id != 0; in++) {
		switch(in->function_id) {
			...
			case OSSL_FUNC_PROVIDER_REGISTER_CHILD_CB:
			    gbl->c_provider_register_child_cb = OSSL_FUNC_provider_register_child_cb(in);
			    break;
			case OSSL_FUNC_PROVIDER_DEREGISTER_CHILD_CB:
			    gbl->c_provider_deregister_child_cb = OSSL_FUNC_provider_deregister_child_cb(in);
			    break;
			case OSSL_FUNC_PROVIDER_NAME:
			    gbl->c_prov_name = OSSL_FUNC_provider_name(in);
			    break;
			case OSSL_FUNC_PROVIDER_GET0_PROVIDER_CTX:
			    gbl->c_prov_get0_provider_ctx = OSSL_FUNC_provider_get0_provider_ctx(in);
			    break;
	...
	if(gbl->c_get_libctx == NULL || gbl->c_provider_register_child_cb == NULL || gbl->c_prov_name == NULL || 
		gbl->c_prov_get0_provider_ctx == NULL || gbl->c_prov_get0_dispatch == NULL || gbl->c_prov_up_ref == NULL || gbl->c_prov_free == NULL)
		return 0;
	gbl->lock = CRYPTO_THREAD_lock_new();
	if(gbl->lock == NULL)
		return 0;
	if(!gbl->c_provider_register_child_cb(gbl->handle, provider_create_child_cb, provider_remove_child_cb, provider_global_props_cb, ctx))
		return 0;
	return 1;
}
...
{
	struct child_prov_globals * gbl = (struct child_prov_globals *)ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
	if(gbl == NULL)
		return;
...
{
	struct child_prov_globals * gbl;
	gbl = (struct child_prov_globals *)ossl_lib_ctx_get_data(ossl_provider_libctx(prov), OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
	if(gbl == NULL)
		return 0;
