...
int ossl_encode_der_integer(WPACKET * pkt, const BIGNUM * n)
{
	...
	if(!WPACKET_start_sub_packet(pkt)
	    ...
	    || !WPACKET_close(pkt))
		return 0;
	if(bnbytes && BN_bn2binpad(n, bnbytes, (int)cont_len) != (int)cont_len)
		return 0;
	return 1;
}
 ...
 * were consumed.
 */
size_t ossl_decode_der_dsa_sig(BIGNUM * r, BIGNUM * s, const uchar ** ppin, size_t len)
{
	size_t consumed;
	PACKET pkt, contpkt;
	uint tag;
	if(!PACKET_buf_init(&pkt, *ppin, len) || !PACKET_get_1(&pkt, &tag) || tag != ID_SEQUENCE || 
		!ossl_decode_der_length(&pkt, &contpkt) || !ossl_decode_der_integer(&contpkt, r) || !ossl_decode_der_integer(&contpkt, s) || PACKET_remaining(&contpkt) != 0)
		return 0;
