...
static int ossl_cms_signerinfo_get_signing_cert(const CMS_SignerInfo * si, ESS_SIGNING_CERT ** psc)
{
	...
	if(!str)
		return 0;
	sc = (ESS_SIGNING_CERT *)ASN1_item_unpack(str, ASN1_ITEM_rptr(ESS_SIGNING_CERT));
	if(!sc)
		return -1;
	if(psc)
		*psc = sc;
...
static int ossl_cms_signerinfo_get_signing_cert_v2(const CMS_SignerInfo * si, ESS_SIGNING_CERT_V2 ** psc)
{
	...
	if(!str)
		return 0;
	sc = (ESS_SIGNING_CERT_V2 *)ASN1_item_unpack(str, ASN1_ITEM_rptr(ESS_SIGNING_CERT_V2));
	if(!sc)
		return -1;
	if(psc)
		*psc = sc;
...
int ossl_cms_check_signing_certs(const CMS_SignerInfo * si, const STACK_OF(X509) * chain)
{
	...
	int ret = ossl_cms_signerinfo_get_signing_cert(si, &ss) >= 0 && ossl_cms_signerinfo_get_signing_cert_v2(si, &ssv2) >= 0 && OSSL_ESS_check_signing_certs(ss, ssv2, chain, 1) > 0;
	ESS_SIGNING_CERT_free(ss);
	ESS_SIGNING_CERT_V2_free(ssv2);
	return ret;
}

CMS_ReceiptRequest * CMS_ReceiptRequest_create0_ex(uchar * id, int idlen, int allorfirst,
    STACK_OF(GENERAL_NAMES) * receiptList, STACK_OF(GENERAL_NAMES) * receiptsTo, OSSL_LIB_CTX * libctx)
{
...
CMS_ReceiptRequest * CMS_ReceiptRequest_create0(uchar * id, int idlen, int allorfirst,
...
static int cms_msgSigDigest(CMS_SignerInfo * si, uchar * dig, unsigned int * diglen)
{
	const EVP_MD * md = EVP_get_digestbyobj(si->digestAlgorithm->algorithm);
	if(!md)
		return 0;
	if(!ossl_asn1_item_digest_ex(ASN1_ITEM_rptr(CMS_Attributes_Verify), md, si->signedAttrs, dig, diglen, ossl_cms_ctx_get0_libctx(si->cms_ctx), ossl_cms_ctx_get0_propq(si->cms_ctx)))
		return 0;
	return 1;
}
