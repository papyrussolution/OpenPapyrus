...
static int cms_copy_content(BIO * out, BIO * in, uint flags)
{
...
CMS_ContentInfo * CMS_data_create_ex(BIO * in, uint flags, OSSL_LIB_CTX * libctx, const char * propq)
{
...
CMS_ContentInfo * CMS_digest_create_ex(BIO * in, const EVP_MD * md, uint flags, OSSL_LIB_CTX * ctx, const char * propq)
{
	CMS_ContentInfo * cms;
	/*
...
CMS_ContentInfo * CMS_EncryptedData_encrypt_ex(BIO * in, const EVP_CIPHER * cipher, const uchar * key, size_t keylen, uint flags, OSSL_LIB_CTX * libctx, const char * propq)
{
	CMS_ContentInfo * cms;
	if(cipher == NULL) {
...
{
	return CMS_EncryptedData_encrypt_ex(in, cipher, key, keylen, flags, NULL, NULL);
}

static int cms_signerinfo_verify_cert(CMS_SignerInfo * si, X509_STORE * store, STACK_OF(X509) * certs, STACK_OF(X509_CRL) * crls, STACK_OF(X509) ** chain, const CMS_CTX * cms_ctx)
{
	X509_STORE_CTX * ctx;
	X509 * signer;
	int i, j, r = 0;
	ctx = X509_STORE_CTX_new_ex(ossl_cms_ctx_get0_libctx(cms_ctx), ossl_cms_ctx_get0_propq(cms_ctx));
	if(!ctx) {
		ERR_raise(ERR_LIB_CMS, ERR_R_MALLOC_FAILURE);
		goto err;
	}
...
int CMS_verify(CMS_ContentInfo * cms, STACK_OF(X509) * certs, X509_STORE * store, BIO * dcont, BIO * out, uint flags)
{
	CMS_SignerInfo * si;
	...
	if((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 || cadesVerify) {
		if(cadesVerify) {
			...
			si_chains = (STACK_OF(X509) **)OPENSSL_zalloc(scount * sizeof(si_chains[0]));
			if(si_chains == NULL) {
				ERR_raise(ERR_LIB_CMS, ERR_R_MALLOC_FAILURE);
				goto err;
			}
		...
		for(i = 0; i < scount; i++) {
			si = sk_CMS_SignerInfo_value(sinfos, i);
			if(!cms_signerinfo_verify_cert(si, store, cms_certs, crls, si_chains ? &si_chains[i] : NULL, ctx))
				goto err;
		}
	}
...
CMS_ContentInfo * CMS_sign_ex(X509 * signcert, EVP_PKEY * pkey, STACK_OF(X509) * certs, BIO * data, uint flags, OSSL_LIB_CTX * libctx, const char * propq)
{
	CMS_ContentInfo * cms;
	int i;
	cms = CMS_ContentInfo_new_ex(libctx, propq);
...
CMS_ContentInfo * CMS_encrypt_ex(STACK_OF(X509) * certs, BIO * data, const EVP_CIPHER * cipher, uint flags, OSSL_LIB_CTX * libctx, const char * propq)
{
	CMS_ContentInfo * cms;
	int i;
	X509 * recip;
	cms = (EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER) ? CMS_AuthEnvelopedData_create_ex(cipher, libctx, propq) : CMS_EnvelopedData_create_ex(cipher, libctx, propq);
	if(cms == NULL)
		goto merr;
	for(i = 0; i < sk_X509_num(certs); i++) {
...
CMS_ContentInfo * CMS_encrypt(STACK_OF(X509) * certs, BIO * data, const EVP_CIPHER * cipher, uint flags)
{
	return CMS_encrypt_ex(certs, data, cipher, flags, NULL, NULL);
}

static int cms_kari_set1_pkey_and_peer(CMS_ContentInfo * cms, CMS_RecipientInfo * ri, EVP_PKEY * pk, X509 * cert, X509 * peer)
{
	int i;
	...
	for(i = 0; i < sk_CMS_RecipientEncryptedKey_num(reks); i++) {
		...
		if(cert && CMS_RecipientEncryptedKey_cert_cmp(rek, cert))
			continue;
...
int CMS_decrypt(CMS_ContentInfo * cms, EVP_PKEY * pk, X509 * cert, BIO * dcont, BIO * out, uint flags)
{
	int r;
	BIO * cont;
