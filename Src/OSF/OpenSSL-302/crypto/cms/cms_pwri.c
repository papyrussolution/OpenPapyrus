    ...
    EVP_CIPHER_CTX * ctx)
{
	...
	if((tmp = (uchar *)OPENSSL_malloc(inlen)) == NULL) {
		ERR_raise(ERR_LIB_CMS, ERR_R_MALLOC_FAILURE);
		return 0;
	}
...
static int kek_wrap_key(uchar * out, size_t * outlen,
    const uchar * in, size_t inlen,
    ...
    CMS_RecipientInfo * ri, int en_de)
{
	...
	kekcipher = EVP_CIPHER_fetch(ossl_cms_ctx_get0_libctx(cms_ctx), name, ossl_cms_ctx_get0_propq(cms_ctx));
	if(kekcipher == NULL) {
