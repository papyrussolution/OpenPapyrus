...
CMS_ContentInfo *d2i_CMS_ContentInfo(CMS_ContentInfo **a, const uchar ** in, long len)
{
...
int i2d_CMS_ContentInfo(const CMS_ContentInfo * a, uchar ** out)
{
...
STACK_OF(X509) *CMS_get1_certs(CMS_ContentInfo *cms)
{
	...
	CMS_CertificateChoices * cch;
	int i;
	STACK_OF(CMS_CertificateChoices) ** pcerts = cms_get0_certificate_choices(cms);
	if(pcerts == NULL)
		return NULL;
	for(i = 0; i < sk_CMS_CertificateChoices_num(*pcerts); i++) {
		cch = sk_CMS_CertificateChoices_value(*pcerts, i);
...
STACK_OF(X509_CRL) *CMS_get1_crls(CMS_ContentInfo *cms)
{
	...
	CMS_RevocationInfoChoice * rch;
	int i;
	STACK_OF(CMS_RevocationInfoChoice) ** pcrls = cms_get0_revocation_choices(cms);
	if(pcrls == NULL)
		return NULL;
