...
int ossl_ffc_params_fromdata(FFC_PARAMS * ffc, const OSSL_PARAM params[])
{
	...
	if(prm) {
		...
		if(prm->data_type != OSSL_PARAM_UTF8_STRING
		    || prm->data == NULL
		    || (group = ossl_ffc_name_to_dh_named_group((const char *)prm->data)) == NULL
	...
	if(prm) {
		if(prm->data_type != OSSL_PARAM_OCTET_STRING)
			goto err;
		if(!ossl_ffc_params_set_seed(ffc, (const uchar *)prm->data, prm->data_size))
			goto err;
	}
	...
	if(prm) {
		...
		if(!ossl_ffc_set_digest(ffc, (const char *)prm->data, props))
			goto err;
	}
