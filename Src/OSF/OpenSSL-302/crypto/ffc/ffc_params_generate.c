...
static int generate_canonical_g(BN_CTX * ctx, BN_MONT_CTX * mont,
    ...
    int gindex, uchar * seed, size_t seedlen)
{
	int ret = 0;
	int counter = 1;
	...
	for(counter = 1; counter <= 0xFFFF; ++counter) {
		...
		md[0] = (uchar)(gindex & 0xff);
		md[1] = (uchar)((counter >> 8) & 0xff);
		md[2] = (uchar)(counter & 0xff);
...
static int generate_p(BN_CTX * ctx, const EVP_MD * evpmd, int max_counter, int n,
    uchar * buf, size_t buf_len, const BIGNUM * q,
...
static int generate_q_fips186_4(BN_CTX * ctx, BIGNUM * q, const EVP_MD * evpmd,
    int qsize, uchar * seed, size_t seedlen,
    ...
    BN_GENCB * cb)
{
	...
	*res = 0;
	...
	seed_tmp = (uchar *)OPENSSL_malloc(seedlen);
	if(seed_tmp == NULL)
		goto err;
	...
	if(seed == NULL) {
		...
		seed = (uchar *)OPENSSL_malloc(seedlen);
		if(seed == NULL)
			goto err;
	}
