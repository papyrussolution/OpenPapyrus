...
int ossl_ffc_params_validate_unverifiable_g(BN_CTX * ctx, BN_MONT_CTX * mont, const BIGNUM * p, const BIGNUM * q,
...
int ossl_ffc_params_full_validate(OSSL_LIB_CTX * libctx, const FFC_PARAMS * params, int paramstype, int * res)
{
	...
	if(res == NULL)
		res = &tmpres;
#ifdef FIPS_MODULE
	return ossl_ffc_params_FIPS186_4_validate(libctx, params, paramstype, res, NULL);
#else
	if(params->seed != NULL) {
		if(params->flags & FFC_PARAM_FLAG_VALIDATE_LEGACY)
			return ossl_ffc_params_FIPS186_2_validate(libctx, params, paramstype, res, NULL);
		else
			return ossl_ffc_params_FIPS186_4_validate(libctx, params, paramstype, res, NULL);
	}
	else {
		int ret = ossl_ffc_params_simple_validate(libctx, params, paramstype, res);
		if(ret) {
			BN_CTX * ctx;
