...
static int prepare_from_text(const OSSL_PARAM * paramdefs, const char * key, const char * value, size_t value_n,
	/* Output parameters */ const OSSL_PARAM ** paramdef, int * ishex, size_t * buf_n, BIGNUM ** tmpbn, int * found)
{
	const OSSL_PARAM * p;
	size_t buf_bits;
	int r;
	...
	if(buf_n > 0) {
		switch(paramdef->data_type) {
			...
			case OSSL_PARAM_UNSIGNED_INTEGER:
			    ...
			    BN_bn2nativepad(tmpbn, (uchar *)buf, buf_n);
			...
			case OSSL_PARAM_OCTET_STRING:
			    if(ishex) {
				    size_t l = 0;
				    if(!OPENSSL_hexstr2buf_ex((uchar *)buf, buf_n, &l, value, ':'))
					    return 0;
			    }
			    else {
