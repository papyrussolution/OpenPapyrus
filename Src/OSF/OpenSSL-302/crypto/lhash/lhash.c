...
void * OPENSSL_LH_retrieve(OPENSSL_LHASH * lh, const void * data)
{
	...
	rn = getrn(lh, data, &hash);
	if(tsan_lock(lh)) {
		tsan_counter(reinterpret_cast<volatile long *>(*rn == NULL ? &lh->num_retrieve_miss : &lh->num_retrieve));
		tsan_unlock(lh);
	}
...
static void contract(OPENSSL_LHASH * lh)
{
	...
	if(lh->p == 0) {
		n = (OPENSSL_LH_NODE **)OPENSSL_realloc(lh->b, (unsigned int)(sizeof(OPENSSL_LH_NODE *) * lh->pmax));
		if(!n) {
...
static OPENSSL_LH_NODE ** getrn(OPENSSL_LHASH * lh, const void * data, unsigned long * rhash)
{
	...
	hash = (*(lh->hash))(data);
	if(do_tsan)
		tsan_counter(reinterpret_cast<volatile long *>(&lh->num_hash_calls));
	*rhash = hash;
	nn = hash % lh->pmax;
	if(nn < lh->p)
	...
	for(n1 = *ret; n1 != NULL; n1 = n1->next) {
		if(do_tsan)
			tsan_counter(reinterpret_cast<volatile long *>(&lh->num_hash_comps));
		if(n1->hash != hash) {
			ret = &(n1->next);
			continue;
		}
		if(do_tsan)
			tsan_counter(reinterpret_cast<volatile long *>(&lh->num_comp_calls));
...
unsigned long OPENSSL_LH_strhash(const char * c)
{
	...
	while(*c) {
		...
		ret = (ret << r) | (ulong)((uint64_t)ret >> (32 - r));
		ret &= 0xFFFFFFFFL;
		ret ^= v * v;
		c++;
	}
