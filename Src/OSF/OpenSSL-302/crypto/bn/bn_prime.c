...
static int probable_prime(BIGNUM * rnd, int bits, int safe, prime_t * mods, BN_CTX * ctx);
static int probable_prime_dh(BIGNUM * rnd, int bits, int safe, prime_t * mods, const BIGNUM * add, const BIGNUM * rem, BN_CTX * ctx);
static int bn_is_prime_int(const BIGNUM * w, int checks, BN_CTX * ctx, int do_trial_division, BN_GENCB * cb);
...
#define BN_SMALL_PRIME_FACTORS_TOP SIZEOFARRAY(small_prime_factors)
...
static int calc_trial_divisions(int bits)
{
	if(bits <= 512)
		...
		GetPrimeTab(&prime_tab_count);
		return /*NUMPRIMES*/prime_tab_count;
	}
}
/*
...
int BN_generate_prime_ex2(BIGNUM * ret, int bits, int safe, const BIGNUM * add, const BIGNUM * rem, BN_GENCB * cb, BN_CTX * ctx)
{
	...
	mods = (prime_t*)OPENSSL_zalloc(sizeof(*mods) * /*NUMPRIMES*/prime_tab_count);
	if(mods == NULL) {
...
int ossl_bn_check_prime(const BIGNUM * w, int checks, BN_CTX * ctx, int do_trial_division, BN_GENCB * cb)
{
...
static int bn_is_prime_int(const BIGNUM * w, int checks, BN_CTX * ctx, int do_trial_division, BN_GENCB * cb)
{
	...
	if(do_trial_division) {
		...
		for(i = 1; i < trial_divisions; i++) {
			BN_ULONG mod = BN_mod_word(w, /*primes[i]*/p_prime_tab[i]);
			if(mod == (BN_ULONG)-1)
				return -1;
			if(mod == 0)
				return BN_is_word(w, /*primes[i]*/p_prime_tab[i]);
		}
...
static int probable_prime(BIGNUM * rnd, int bits, int safe, prime_t * mods, BN_CTX * ctx)
{
	...
	size_t prime_tab_count = 0;
	const ushort * p_prime_tab = GetPrimeTab(&prime_tab_count);
	BN_ULONG maxdelta = BN_MASK2 - /*primes*/p_prime_tab[trial_divisions - 1];
again:
	if(!BN_priv_rand_ex(rnd, bits, BN_RAND_TOP_TWO, BN_RAND_BOTTOM_ODD, 0, ctx))
		return 0;
	if(safe && !BN_set_bit(rnd, 1))
		return 0;
	/* we now have a random number 'rnd' to test. */
	for(i = 1; i < trial_divisions; i++) {
		BN_ULONG mod = BN_mod_word(rnd, (BN_ULONG)/*primes*/p_prime_tab[i]);
		if(mod == (BN_ULONG)-1)
			return 0;
		mods[i] = (prime_t)mod;
	}
...
loop:
	for(i = 1; i < trial_divisions; i++) {
		 ...
		 */
		if(bits <= 31 && delta <= 0x7fffffff && square(/*primes*/p_prime_tab[i]) > BN_get_word(rnd) + delta)
			break;
		if(safe ? (mods[i] + delta) % /*primes*/p_prime_tab[i] <= 1 : (mods[i] + delta) % /*primes*/p_prime_tab[i] == 0) {
			delta += safe ? 4 : 2;
			if(delta > maxdelta)
				goto again;
			goto loop;
...
again:
	...
	if(BN_num_bits(rnd) < bits || BN_get_word(rnd) < (safe ? 5u : 3u)) {
