 ...
 * multiple.
 */
#define MOD_EXP_CTIME_ALIGN(x_) ((uchar *)(x_) + (MOD_EXP_CTIME_MIN_CACHE_LINE_WIDTH - (((size_t)(x_)) & (MOD_EXP_CTIME_MIN_CACHE_LINE_MASK))))
// @sobolev Вынесено из тела функции BN_mod_exp_mont_consttime() {
#if defined(OPENSSL_BN_ASM_MONT5)
	// 
	// Dedicated window==4 case improves 512-bit RSA sign by ~15%, but as
	// 512-bit RSA is hardly relevant, we omit it to spare size...
	// 
	extern "C" void bn_mul_mont_gather5(BN_ULONG * rp, const BN_ULONG * ap, const void * table, const BN_ULONG * np, const BN_ULONG * n0, int num, int power);
	extern "C" void bn_scatter5(const BN_ULONG * inp, size_t num, void * table, size_t power);
	extern "C" void bn_gather5(BN_ULONG * out, size_t num, void * table, size_t power);
	extern "C" void bn_power5(BN_ULONG * rp, const BN_ULONG * ap, const void * table, const BN_ULONG * np, const BN_ULONG * n0, int num, int power);
	extern "C" int bn_get_bits5(const BN_ULONG * ap, int off);
	extern "C" int bn_from_montgomery(BN_ULONG * rp, const BN_ULONG * ap, const BN_ULONG * not_used, const BN_ULONG * np, const BN_ULONG * n0, int num);
#endif
// } @sobolev 
/*
...
int BN_mod_exp_mont_consttime(BIGNUM * rr, const BIGNUM * a, const BIGNUM * p, const BIGNUM * m, BN_CTX * ctx, BN_MONT_CTX * in_mont)
{
	...
	if(window == 5 && top > 1) {
		 ...
		 * 512-bit RSA is hardly relevant, we omit it to spare size...
		 */
		/* @sobolev (moved up out of function body) 
		extern "C" void bn_mul_mont_gather5(BN_ULONG * rp, const BN_ULONG * ap, const void * table, const BN_ULONG * np, const BN_ULONG * n0, int num, int power);
		extern "C" void bn_scatter5(const BN_ULONG * inp, size_t num, void * table, size_t power);
		extern "C" void bn_gather5(BN_ULONG * out, size_t num, void * table, size_t power);
		extern "C" void bn_power5(BN_ULONG * rp, const BN_ULONG * ap, const void * table, const BN_ULONG * np, const BN_ULONG * n0, int num, int power);
 ...
 */
int BN_mod_exp_mont_consttime_x2(BIGNUM * rr1, const BIGNUM * a1, const BIGNUM * p1, const BIGNUM * m1, BN_MONT_CTX * in_mont1,
    BIGNUM * rr2, const BIGNUM * a2, const BIGNUM * p2, const BIGNUM * m2, BN_MONT_CTX * in_mont2, BN_CTX * ctx)
{
	int ret = 0;
#ifdef RSAZ_ENABLED
	...
	if(ossl_rsaz_avx512ifma_eligible() && ((a1->top == 16) && (p1->top == 16) && (BN_num_bits(m1) == 1024) && (a2->top == 16) && (p2->top == 16) && (BN_num_bits(m2) == 1024))) {
		...
		ret = ossl_rsaz_mod_exp_avx512_x2(rr1->d, a1->d, p1->d, m1->d, mont1->RR.d, mont1->n0[0], rr2->d, a2->d, p2->d, m2->d, mont2->RR.d, mont2->n0[0], 1024 /* factor bit size */);
		rr1->top = 16;
		rr1->neg = 0;
		bn_correct_top(rr1);
		bn_check_top(rr1);
