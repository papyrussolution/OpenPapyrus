...
void BN_clear_free(BIGNUM * a)
{
	if(!a)
		return;
	if(a->d && !BN_get_flags(a, BN_FLG_STATIC_DATA))
		bn_free_d(a, 1);
...
/* ignore negative */
static
int bn2binpad(const BIGNUM * a, uchar * to, int tolen, endianess_t endianess)
{
	int n;
...
BIGNUM * BN_native2bn(const uchar * s, int len, BIGNUM * ret)
{
	DECLARE_IS_ENDIAN;
	if(IS_LITTLE_ENDIAN)
...
int BN_bn2nativepad(const BIGNUM * a, uchar * to, int tolen)
{
	DECLARE_IS_ENDIAN;
	if(IS_LITTLE_ENDIAN)
