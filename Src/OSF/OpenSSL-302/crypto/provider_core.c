...
{
	INFOPAIR * dest = (INFOPAIR *)OPENSSL_zalloc(sizeof(*dest));
	if(dest == NULL)
		return NULL;
	if(src->name) {
...
{
	struct provider_store_st * store = (struct provider_store_st *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_PROVIDER_STORE_INDEX, &provider_store_method);
	if(!store)
...
int ossl_provider_info_add_to_store(OSSL_LIB_CTX * libctx, OSSL_PROVIDER_INFO * entry)
{
	...
	if(store->provinfosz == 0) {
		store->provinfo = (OSSL_PROVIDER_INFO *)OPENSSL_zalloc(sizeof(*store->provinfo) * BUILTINS_BLOCK_SIZE);
		...
		tmpbuiltins = (OSSL_PROVIDER_INFO *)OPENSSL_realloc(store->provinfo, sizeof(*store->provinfo) * newsz);
		if(tmpbuiltins == NULL) {
			ERR_raise(ERR_LIB_CRYPTO, ERR_R_MALLOC_FAILURE);
			goto err;
		}
...
#ifndef FIPS_MODULE
		/*
		 * Make sure any providers are loaded from config before we try to find them.
		 */
		if(!noconfig) {
...
{
	struct provider_store_st * store = NULL;
	OSSL_PROVIDER_INFO _template;
	OSSL_PROVIDER * prov = NULL;
	if((store = get_provider_store(libctx)) == NULL)
		return NULL;
	memzero(&_template, sizeof(_template));
	if(init_function == NULL) {
		const OSSL_PROVIDER_INFO * p;
		size_t i;
		/* Check if this is a predefined builtin provider */
		for(p = ossl_predefined_providers; p->name; p++) {
		...
		_template.init = init_function;
	}
	/* provider_new() generates an error, so no need here */
	if((prov = provider_new(name, _template.init, _template.parameters)) == NULL)
		return NULL;
	prov->libctx = libctx;
#ifndef FIPS_MODULE
...
static int infopair_add(STACK_OF(INFOPAIR) ** infopairsk, const char * name, const char * value)
{
	...
	if((pair = (INFOPAIR *)OPENSSL_zalloc(sizeof(*pair))) != NULL
	    && (*infopairsk || (*infopairsk = sk_INFOPAIR_new_null()) != NULL)
 ...
 */
/*static*/extern const OSSL_DISPATCH * core_dispatch; /* Define further down */
...
static int provider_init(OSSL_PROVIDER * prov)
{
	...
	if(prov->init_function == NULL || !prov->init_function((OSSL_CORE_HANDLE*)prov, core_dispatch, &provider_dispatch, &tmp_provctx)) {
	...
	if(p_get_reason_strings) {
		...
		prov->error_strings = (ERR_STRING_DATA *)OPENSSL_zalloc(sizeof(ERR_STRING_DATA) * (cnt + 1));
...
{
	return prov ? prov->provctx : NULL;
}

const OSSL_DISPATCH * ossl_provider_get0_dispatch(const OSSL_PROVIDER * prov)
{
	return prov ? prov->dispatch : NULL;
}

OSSL_LIB_CTX * ossl_provider_libctx(const OSSL_PROVIDER * prov)
{
	return prov ? prov->libctx : NULL;
}
...
int ossl_provider_clear_all_operation_bits(OSSL_LIB_CTX * libctx)
{
	...
	if((store = get_provider_store(libctx)) != NULL) {
		...
		for(i = 0; i < num; i++) {
			...
			if(provider->operation_bits)
				memzero(provider->operation_bits, provider->operation_bits_sz);
...
int ossl_provider_set_operation_bit(OSSL_PROVIDER * provider, size_t bitnum)
{
	...
	if(provider->operation_bits_sz <= byte) {
		...
		memzero(provider->operation_bits + provider->operation_bits_sz, byte + 1 - provider->operation_bits_sz);
    ...
    void * cbdata)
{
	...
	if((store = get_provider_store(libctx)) == NULL)
		return 0;
	child_cb = (OSSL_PROVIDER_CHILD_CB *)OPENSSL_malloc(sizeof(*child_cb));
	if(child_cb == NULL)
		return 0;
...
static int core_obj_add_sigid(const OSSL_CORE_HANDLE * prov, const char * sign_name, const char * digest_name, const char * pkey_name)
{
	...
	if(digest_name && digest_name[0] != '\0' && (digest_nid = OBJ_txt2nid(digest_name)) == NID_undef)
		return 0;
...
static const OSSL_DISPATCH core_dispatch_[] = {
	...
	{ OSSL_FUNC_PROVIDER_NAME, (void (*)(void))OSSL_PROVIDER_get0_name },
	{ OSSL_FUNC_PROVIDER_GET0_PROVIDER_CTX, (void (*)(void))OSSL_PROVIDER_get0_provider_ctx },
	{ OSSL_FUNC_PROVIDER_GET0_DISPATCH, (void (*)(void))OSSL_PROVIDER_get0_dispatch },
	{ OSSL_FUNC_PROVIDER_UP_REF, (void (*)(void))provider_up_ref_intern },
	{ OSSL_FUNC_PROVIDER_FREE, (void (*)(void))provider_free_intern },
