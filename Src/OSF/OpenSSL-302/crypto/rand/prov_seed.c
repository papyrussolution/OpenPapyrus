...
size_t ossl_rand_get_nonce(ossl_unused OSSL_CORE_HANDLE * handle,
    uchar ** pout, size_t min_len, size_t max_len,
    const void * salt, size_t salt_len)
{
	size_t ret = 0;
	RAND_POOL * pool = ossl_rand_pool_new(0, 0, min_len, max_len);
	if(pool == NULL) {
		ERR_raise(ERR_LIB_RAND, ERR_R_MALLOC_FAILURE);
		return 0;
	}
	...
	if(salt != NULL && !ossl_rand_pool_add(pool, (const unsigned char*)salt, salt_len, 0))
		goto err;
	ret   = ossl_rand_pool_length(pool);
	*pout = ossl_rand_pool_detach(pool);
err:
