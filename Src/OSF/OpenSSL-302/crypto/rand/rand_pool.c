...
RAND_POOL * ossl_rand_pool_new(int entropy_requested, int secure, size_t min_len, size_t max_len)
{
	...
	if(secure)
		pool->buffer = (uchar *)OPENSSL_secure_zalloc(pool->alloc_len);
	else
		pool->buffer = (uchar *)OPENSSL_zalloc(pool->alloc_len);
	if(pool->buffer == NULL) {
		ERR_raise(ERR_LIB_RAND, ERR_R_MALLOC_FAILURE);
		goto err;
	}
 ...
 */
RAND_POOL * ossl_rand_pool_attach(const uchar * buffer, size_t len, size_t entropy)
{
	RAND_POOL * pool = (RAND_POOL *)OPENSSL_zalloc(sizeof(*pool));
	if(pool == NULL) {
		ERR_raise(ERR_LIB_RAND, ERR_R_MALLOC_FAILURE);
 ...
 */
const uchar * ossl_rand_pool_buffer(RAND_POOL * pool)
{
	return pool->buffer;
}
 ...
 */
uchar * ossl_rand_pool_detach(RAND_POOL * pool)
{
 ...
 */
void ossl_rand_pool_reattach(RAND_POOL * pool, uchar * buffer)
{
	pool->buffer = buffer;
...
static int rand_pool_grow(RAND_POOL * pool, size_t len)
{
	if(len > pool->alloc_len - pool->len) {
		...
		if(pool->secure)
			p = (uchar *)OPENSSL_secure_zalloc(newlen);
		else
			p = (uchar *)OPENSSL_zalloc(newlen);
		if(!p) {
			ERR_raise(ERR_LIB_RAND, ERR_R_MALLOC_FAILURE);
			return 0;
		}
 ...
 */
uchar * ossl_rand_pool_add_begin(RAND_POOL * pool, size_t len)
{
	if(!len)
		return NULL;
