...
#define OPENSSL_NO_DEPRECATED_3_0 // @sobolev

static int ossl_store_close_it(OSSL_STORE_CTX * ctx);

static int loader_set_params(OSSL_STORE_LOADER * loader, OSSL_STORE_LOADER_CTX * loader_ctx, const OSSL_PARAM params[], const char * propq)
{
	if(params) {
    ...
    const OSSL_PARAM params[], OSSL_STORE_post_process_info_fn post_process, void * post_process_data)
{
...
#endif
		if(loader == NULL && (fetched_loader = OSSL_STORE_LOADER_fetch(libctx, scheme, propq)) != NULL) {
			const OSSL_PROVIDER * provider = OSSL_STORE_LOADER_get0_provider(fetched_loader);
			void * provctx = OSSL_PROVIDER_get0_provider_ctx(provider);
			no_loader_found = 0;
			loader_ctx = (OSSL_STORE_LOADER_CTX *)fetched_loader->p_open(provctx, uri);
			if(loader_ctx == NULL) {
				OSSL_STORE_LOADER_free(fetched_loader);
				fetched_loader = NULL;
			}
			else if(!loader_set_params(fetched_loader, loader_ctx, params, propq)) {
	...
	if((propq != NULL && (propq_copy = OPENSSL_strdup(propq)) == NULL) || (ctx = (OSSL_STORE_CTX *)OPENSSL_zalloc(sizeof(*ctx))) == NULL) {
		ERR_raise(ERR_LIB_OSSL_STORE, ERR_R_MALLOC_FAILURE);
		goto err;
	}
	if(ui_method != NULL && (!ossl_pw_set_ui_method(&ctx->pwdata, ui_method, ui_data) || !ossl_pw_enable_passphrase_caching(&ctx->pwdata))) {
...
OSSL_STORE_CTX * OSSL_STORE_open(const char * uri, const UI_METHOD * ui_method, void * ui_data, OSSL_STORE_post_process_info_fn post_process,
    void * post_process_data)
{
	return OSSL_STORE_open_ex(uri, NULL, NULL, ui_method, ui_data, NULL, post_process, post_process_data);
}
...
OSSL_STORE_SEARCH * OSSL_STORE_SEARCH_by_key_fingerprint(const EVP_MD * digest, const uchar * bytes, size_t len)
{
	...
	if(digest != NULL && len != (size_t)EVP_MD_get_size(digest)) {
		ERR_raise_data(ERR_LIB_OSSL_STORE, OSSL_STORE_R_FINGERPRINT_SIZE_DOES_NOT_MATCH_DIGEST,
		    "%s size is %d, fingerprint size is %zu", EVP_MD_get0_name(digest), EVP_MD_get_size(digest), len);
		OPENSSL_free(search);
		return NULL;
	}
    ...
    void * post_process_data)
{
	...
	if(loader == NULL && (fetched_loader = OSSL_STORE_LOADER_fetch(libctx, scheme, propq)) != NULL) {
		...
		if(cbio == NULL || (loader_ctx = (OSSL_STORE_LOADER_CTX *)fetched_loader->p_attach(provctx, cbio)) == NULL) {
			OSSL_STORE_LOADER_free(fetched_loader);
			fetched_loader = NULL;
		}
		else if(!loader_set_params(fetched_loader, loader_ctx, params, propq)) {
	...
	if(ui_method != NULL && !ossl_pw_set_ui_method(&ctx->pwdata, ui_method, ui_data)) {
		ERR_clear_last_mark();
		OPENSSL_free(ctx);
		return NULL;
	}
