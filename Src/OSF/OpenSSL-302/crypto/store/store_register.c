...
{
	OSSL_STORE_LOADER _template;
	OSSL_STORE_LOADER * loader = NULL;
	_template.scheme = scheme;
	_template.open = NULL;
	_template.load = NULL;
	_template.eof = NULL;
	_template.closefn = NULL;
	_template.open_ex = NULL;
	if(!RUN_ONCE(&registry_init, do_registry_init)) {
		ERR_raise(ERR_LIB_OSSL_STORE, ERR_R_MALLOC_FAILURE);
		return NULL;
	}
	...
	else if((loader = lh_OSSL_STORE_LOADER_retrieve(loader_register, &_template)) == NULL)
...
OSSL_STORE_LOADER * ossl_store_unregister_loader_int(const char * scheme)
{
	...
	if(!ossl_store_register_init())
		ERR_raise(ERR_LIB_OSSL_STORE, ERR_R_INTERNAL_ERROR);
	else if((loader = lh_OSSL_STORE_LOADER_delete(loader_register, &_template)) == NULL)
		ERR_raise_data(ERR_LIB_OSSL_STORE, OSSL_STORE_R_UNREGISTERED_SCHEME, "scheme=%s", scheme);
	CRYPTO_THREAD_unlock(registry_lock);
	return loader;
}
