...
static OSSL_STORE_LOADER * new_loader(OSSL_PROVIDER * prov)
{
	...
	if((loader = (OSSL_STORE_LOADER *)OPENSSL_zalloc(sizeof(*loader))) == NULL || (loader->lock = CRYPTO_THREAD_lock_new()) == NULL) {
		OPENSSL_free(loader);
		return NULL;
	}
	loader->prov = prov;
...
{
	return OSSL_STORE_LOADER_up_ref((OSSL_STORE_LOADER *)method);
}
...
{
	struct loader_data_st * methdata = (struct loader_data_st *)data;
...
{
	return (OSSL_METHOD_STORE *)ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_STORE_LOADER_STORE_INDEX, &loader_store_method);
}
...
static void * get_loader_from_store(void * store, const OSSL_PROVIDER ** prov, void * data)
{
	...
	if(store == NULL && (store = get_loader_store(methdata->libctx)) == NULL)
		return NULL;
	if(!ossl_method_store_fetch((OSSL_METHOD_STORE *)store, id, methdata->propquery, prov, &method))
		return NULL;
	return method;
}

static int put_loader_in_store(void * store, void * method, const OSSL_PROVIDER * prov, const char * scheme, const char * propdef, void * data)
{
	struct loader_data_st * methdata = (struct loader_data_st *)data;
	OSSL_NAMEMAP * namemap;
	int id;
	if((namemap = ossl_namemap_stored(methdata->libctx)) == NULL || (id = ossl_namemap_name2num(namemap, scheme)) == 0)
		return 0;
	if(store == NULL && (store = get_loader_store(methdata->libctx)) == NULL)
		return 0;
	return ossl_method_store_add((OSSL_METHOD_STORE *)store, prov, id, propdef, method, up_ref_loader, free_loader);
}

static void * loader_from_algorithm(int scheme_id, const OSSL_ALGORITHM * algodef, OSSL_PROVIDER * prov)
{
 ...
 */
static void * construct_loader(const OSSL_ALGORITHM * algodef, OSSL_PROVIDER * prov, void * data)
{
	/*
    ...
    const char * scheme, const char * properties)
{
	...
	return (OSSL_STORE_LOADER *)method;
}
...
OSSL_STORE_LOADER * ossl_store_loader_fetch_by_number(OSSL_LIB_CTX * libctx, int scheme_id, const char * properties)
{
	struct loader_data_st methdata;
	void * method;
	methdata.libctx = libctx;
...
{
	struct do_one_data_st * data = (struct do_one_data_st *)arg;
	data->user_fn((OSSL_STORE_LOADER *)method, data->user_arg);
}

void OSSL_STORE_LOADER_do_all_provided(OSSL_LIB_CTX * libctx, void (*user_fn)(OSSL_STORE_LOADER * loader, void * arg), void * user_arg)
{
	struct loader_data_st methdata;
