...
static int setup_tbuf(RSA_PKEY_CTX * ctx, EVP_PKEY_CTX * pk)
{
	if(ctx->tbuf != NULL)
		return 1;
	if((ctx->tbuf = (uchar *)OPENSSL_malloc(RSA_size(EVP_PKEY_get0_RSA(pk->pkey)))) == NULL) {
...
bad_pad:
		...
		case EVP_PKEY_CTRL_MD:
		    if(!check_padding_md((const EVP_MD *)p2, rctx->pad_mode))
			    return 0;
		    if(rsa_pss_restricted(rctx)) {
			    if(EVP_MD_get_type(rctx->md) == EVP_MD_get_type((const EVP_MD *)p2))
				    return 1;
		...
		case EVP_PKEY_CTRL_GET_RSA_MGF1_MD:
		    ...
		    if(type == EVP_PKEY_CTRL_GET_RSA_MGF1_MD) {
			    ...
			    if(rsa_pss_restricted(rctx)) {
				    if(EVP_MD_get_type(rctx->mgf1md) == EVP_MD_get_type((const EVP_MD *)p2))
					    return 1;
...
static int pkey_rsa_ctrl_str(EVP_PKEY_CTX * ctx, const char * type, const char * value)
{
	...
	if(sstreq(type, "rsa_pss_saltlen")) {
		int saltlen;
		if(sstreq(value, "digest"))
			saltlen = RSA_PSS_SALTLEN_DIGEST;
		else if(sstreq(value, "max"))
			saltlen = RSA_PSS_SALTLEN_MAX;
		else if(sstreq(value, "auto"))
