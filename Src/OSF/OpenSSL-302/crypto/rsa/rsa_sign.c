...
const uchar * ossl_rsa_digestinfo_encoding(int md_nid, size_t *len)
{
	switch(md_nid) {
#ifndef FIPS_MODULE
#ifndef OPENSSL_NO_MDC2
 ...
 */
static int encode_pkcs1(uchar ** out, size_t * out_len, int type, const uchar * m, size_t m_len)
{
	size_t di_prefix_len, dig_info_len;
	const uchar * di_prefix;
	uchar * dig_info;
	if(type == NID_undef) {
	...
	dig_info = (uchar *)OPENSSL_malloc(dig_info_len);
	if(dig_info == NULL) {
...
int RSA_sign(int type, const uchar * m, unsigned int m_len,
    uchar * sigret, unsigned int * siglen, RSA * rsa)
{
 ...
 */
int ossl_rsa_verify(int type, const uchar * m, unsigned int m_len,
...
int RSA_verify(int type, const uchar * m, unsigned int m_len,
    const uchar * sigbuf, unsigned int siglen, RSA * rsa)
{
