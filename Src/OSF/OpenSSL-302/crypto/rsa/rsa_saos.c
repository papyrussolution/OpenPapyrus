 ...
 */
int RSA_sign_ASN1_OCTET_STRING(int type, const uchar * m, unsigned int m_len, uchar * sigret, unsigned int * siglen, RSA * rsa)
{
	...
	s = (uchar *)OPENSSL_malloc((unsigned int)j + 1);
	if(!s) {
...
int RSA_verify_ASN1_OCTET_STRING(int dtype,
    const uchar * m,
    unsigned int m_len, uchar * sigbuf,
    unsigned int siglen, RSA * rsa)
{
	...
	s = (uchar *)OPENSSL_malloc((unsigned int)siglen);
	if(!s) {
		ERR_raise(ERR_LIB_RSA, ERR_R_MALLOC_FAILURE);
		goto err;
	}
