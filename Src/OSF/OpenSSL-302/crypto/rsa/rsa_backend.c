    ...
    OSSL_LIB_CTX * libctx)
{
	...
	if(param_propq != NULL) {
		if(param_propq->data_type == OSSL_PARAM_UTF8_STRING)
			propq = (const char *)param_propq->data;
	}
	/*
	...
	if(param_mgf != NULL) {
		...
		if(param_mgf->data_type == OSSL_PARAM_UTF8_STRING)
			mgfname = (const char *)param_mgf->data;
		else if(!OSSL_PARAM_get_utf8_ptr(param_mgf, &mgfname))
			return 0;
		if(strcasecmp((const char *)param_mgf->data, ossl_rsa_mgf_nid2name(default_maskgenalg_nid)) != 0)
			return 0;
	}
	/*
	...
	if(param_md != NULL) {
		...
		if(param_md->data_type == OSSL_PARAM_UTF8_STRING)
			mdname = (const char *)param_md->data;
	...
	if(param_mgf1md != NULL) {
		...
		if(param_mgf1md->data_type == OSSL_PARAM_UTF8_STRING)
			mgf1mdname = (const char *)param_mgf1md->data;
		else if(!OSSL_PARAM_get_utf8_ptr(param_mgf, &mgf1mdname))
			goto err;
		if((mgf1md = EVP_MD_fetch(libctx, mgf1mdname, propq)) == NULL || !ossl_rsa_pss_params_30_set_maskgenhashalg(
...
RSA * ossl_rsa_dup(const RSA * rsa, int selection)
{
	...
	if((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0
		...
		for(i = 0; i < pnum; i++) {
			...
			if((duppinfo = (RSA_PRIME_INFO *)OPENSSL_zalloc(sizeof(*duppinfo))) == NULL) {
...
{
	RSA_PSS_PARAMS * pss = (RSA_PSS_PARAMS *)ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(RSA_PSS_PARAMS), alg->parameter);
	if(pss == NULL)
		return NULL;
