    ...
    const EVP_MD * mgf1md)
{
	...
	dbmask = (uchar *)OPENSSL_malloc(dbmask_len);
	if(dbmask == NULL) {
...
int RSA_padding_check_PKCS1_OAEP(uchar * to, int tlen, const uchar * from, int flen, int num,
    const uchar * param, int plen)
{
	return RSA_padding_check_PKCS1_OAEP_mgf1(to, tlen, from, flen, num, param, plen, NULL, NULL);
}

int RSA_padding_check_PKCS1_OAEP_mgf1(uchar * to, int tlen, const uchar * from, int flen,
    int num, const uchar * param, int plen, const EVP_MD * md, const EVP_MD * mgf1md)
{
	 ...
	 */
	uchar * db = NULL, * em = NULL, seed[EVP_MAX_MD_SIZE], phash[EVP_MAX_MD_SIZE];
	int mdlen;
	if(!md) {
#ifndef FIPS_MODULE
		md = EVP_sha1();
