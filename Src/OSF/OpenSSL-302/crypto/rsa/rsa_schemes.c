...
static int meth2nid(const void * meth, int (* meth_is_a)(const void * meth, const char * name), const OSSL_ITEM * items, size_t items_n)
{
	size_t i;
	if(meth != NULL)
		for(i = 0; i < items_n; i++)
			if(meth_is_a(meth, (const char *)items[i].ptr))
...
{
	return EVP_MD_is_a((const EVP_MD *)md, name);
}

int ossl_rsa_oaeppss_md2nid(const EVP_MD * md)
{
	return meth2nid(md, md_is_a, oaeppss_name_nid_map, SIZEOFARRAY(oaeppss_name_nid_map));
}

const char * ossl_rsa_oaeppss_nid2name(int md)
{
	return nid2name(md, oaeppss_name_nid_map, SIZEOFARRAY(oaeppss_name_nid_map));
}
