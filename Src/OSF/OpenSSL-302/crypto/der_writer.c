...
int ossl_DER_w_octet_string(WPACKET * pkt, int tag,
    const uchar * data, size_t data_n)
{
	return int_start_context(pkt, tag)
	       && WPACKET_start_sub_packet(pkt)
...
static int int_put_bytes_ulong(WPACKET * pkt, const void * v, unsigned int * top_byte)
{
	const unsigned long * value = (const unsigned long *)v;
...
{
	uchar * p = NULL;
	size_t n = BN_num_bytes((const BIGNUM *)v);
	/* The BIGNUM limbs are in LE order */
	*top_byte = ((bn_get_words((const BIGNUM *)v) [(n - 1) / BN_BYTES]) >> (8 * ((n - 1) % BN_BYTES))) & 0xFF;
	if(!WPACKET_allocate_bytes(pkt, n, &p))
		return 0;
	if(p)
		BN_bn2bin((const BIGNUM *)v, p);
	return 1;
}
