...
static int zlib_stateful_compress_block(COMP_CTX * ctx, uchar * out, unsigned int olen, uchar * in, unsigned int ilen);
static int zlib_stateful_expand_block(COMP_CTX * ctx, uchar * out, unsigned int olen, uchar * in, unsigned int ilen);
...
#endif
	ctx = (BIO_ZLIB_CTX *)OPENSSL_zalloc(sizeof(*ctx));
	if(!ctx) {
...
static int bio_zlib_free(BIO * bi)
{
	...
	if(!bi)
		return 0;
	ctx = (BIO_ZLIB_CTX *)BIO_get_data(bi);
	if(ctx->ibuf) {
...
static int bio_zlib_read(BIO * b, char * out, int outl)
{
	...
	if(!ctx->ibuf) {
		ctx->ibuf = (uchar *)OPENSSL_malloc(ctx->ibufsize);
		if(ctx->ibuf == NULL) {
			ERR_raise(ERR_LIB_COMP, ERR_R_MALLOC_FAILURE);
			return 0;
		}
