    ...
    EVP_PKEY * pkey)
{
	...
	uchar * ek = NULL; /* decrypted symmetric encryption key */
	size_t eksize = 0; /* size of decrypted symmetric encryption key */
	EVP_CIPHER * cipher = NULL; /* used cipher */
	int cikeysize = 0; /* key size from cipher */
	uchar * iv = NULL; /* initial vector for symmetric encryption */
	uchar * outbuf = NULL; /* decryption output buffer */
	const uchar * p = NULL; /* needed for decoding ASN1 */
	int n, outlen = 0;
	...
	if(pkctx != NULL && EVP_PKEY_decrypt_init(pkctx)) {
		...
		if(EVP_PKEY_decrypt(pkctx, NULL, &eksize,
		    ...
		    || (ek = (uchar *)OPENSSL_malloc(eksize)) == NULL)
			goto end;
	...
	if((iv = (uchar *)OPENSSL_malloc(EVP_CIPHER_get_iv_length(cipher))) == NULL)
		goto end;
	 ...
	 */
	if((p = outbuf = (uchar *)OPENSSL_malloc(ecert->encValue->length +
