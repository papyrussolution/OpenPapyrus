...
{
	ASSIGN_PTR(malloc_fn, malloc_impl);
	ASSIGN_PTR(realloc_fn, realloc_impl);
	ASSIGN_PTR(free_fn, free_impl);
}

#if !defined(OPENSSL_NO_CRYPTO_MDEBUG) && !defined(FIPS_MODULE)
void CRYPTO_get_alloc_counts(int * mcount, int * rcount, int * fcount)
{
	ASSIGN_PTR(mcount, LOAD(malloc_count));
	ASSIGN_PTR(rcount, LOAD(realloc_count));
	ASSIGN_PTR(fcount, LOAD(free_count));
}
