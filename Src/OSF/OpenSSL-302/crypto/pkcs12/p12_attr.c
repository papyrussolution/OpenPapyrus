...
int PKCS12_add_friendlyname_asc(PKCS12_SAFEBAG * bag, const char * name, int namelen)
{
	if(X509at_add1_attr_by_NID(&bag->attrib, NID_friendlyName,
	    MBSTRING_ASC, (uchar *)name, namelen) != NULL)
		return 1;
	else
		return 0;
}
...
{
	if(X509at_add1_attr_by_NID(&bag->attrib, NID_friendlyName, MBSTRING_UTF8, (uchar *)name, namelen) != NULL)
		return 1;
	else
		return 0;
}

int PKCS12_add_friendlyname_uni(PKCS12_SAFEBAG * bag, const uchar * name, int namelen)
{
...
{
	if(X509at_add1_attr_by_NID(&bag->attrib, NID_ms_csp_name, MBSTRING_ASC, (uchar *)name, namelen) != NULL)
		return 1;
	else
		return 0;
}

int PKCS12_add1_attr_by_NID(PKCS12_SAFEBAG * bag, int nid, int type, const uchar * bytes, int len)
{
...
int PKCS12_add1_attr_by_txt(PKCS12_SAFEBAG * bag, const char * attrname, int type, const uchar * bytes, int len)
{
