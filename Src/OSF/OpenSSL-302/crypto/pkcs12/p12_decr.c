 ...
 */
uchar * PKCS12_pbe_crypt_ex(const X509_ALGOR * algor, const char * pass, int passlen, const uchar * in, int inlen,
    uchar ** data, int * datalen, int en_de, OSSL_LIB_CTX * libctx, const char * propq)
{
	uchar * out = NULL;
	int outlen, i;
	...
	if((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx)) & EVP_CIPH_FLAG_CIPHER_WITH_MAC) != 0) {
		...
		if(EVP_CIPHER_CTX_is_encrypting(ctx)) {
			...
			inlen -= mac_len;
			if(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)mac_len, (uchar *)in+inlen) < 0) {
				ERR_raise(ERR_LIB_PKCS12, ERR_R_INTERNAL_ERROR);
				goto err;
			}
		}
	}
	if((out = (uchar *)OPENSSL_malloc(max_out_len)) == NULL) {
		ERR_raise(ERR_LIB_PKCS12, ERR_R_MALLOC_FAILURE);
		goto err;
	}
