...
#ifdef HAVE_ZLIB_H
		case AG_GZIP:
		    memzero(&cookie->stream.gz, sizeof cookie->stream.gz);
...
/*
 * Return number of bytes into buf, 0 on EOF, -1 on error.  Update stream offset.
 */
static ssize_t zfile_read(void * cookie_, char * buf, size_t size) 
{
	...
	do {
		...
		while(CNEXT_OUT(cookie) >
			...
			size_t left = CNEXT_OUT(cookie) - &cookie->outbuf[cookie->outbuf_start];
...
static int zfile_seek(void * cookie_, off64_t * offset_, int whence) 
{
	struct zfile * cookie = cookie_;
