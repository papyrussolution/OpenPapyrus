...
pthread_cond_t files_ready = PTHREAD_COND_INITIALIZER; // @global
pthread_mutex_t stats_mtx = PTHREAD_MUTEX_INITIALIZER; // @global
pthread_mutex_t work_queue_mtx = PTHREAD_MUTEX_INITIALIZER; // @global
symdir_t * symhash = NULL; // @global
...
ssize_t search_buf(const char * buf, const size_t buf_len, const char * dir_full_path) 
{
	...
	if(!opts.literal && opts.query_len == 1 && opts.query[0] == '.') {
		matches_size = 1;
		matches = (matches == NULL) ? (match_t *)ag_malloc(matches_size * sizeof(match_t)) : matches;
		matches[0].start = 0;
...
/* hash_strnstr only for little-endian platforms that allow unaligned access */
			...
			if(opts.word_regexp) {
				...
				// Check whether both start and end of the match lie on a word boundary
				if((start == buf || is_wordchar(*(start - 1)) != opts.literal_starts_wordchar) && (end == buf + buf_len || is_wordchar(*end) != opts.literal_ends_wordchar)) {
					/* It's a match */
				}
				else {
					/* It's not a match */
		...
		int offset_vector[3];
		if(opts.multiline) {
			while(buf_offset < buf_len && (pcre_exec(opts.re, opts.re_extra, buf, buf_len, buf_offset, 0, offset_vector, 3)) >= 0) {
...
void search_file(const char * file_full_path) 
{
	...
	int matches_count = -1;
	FILE * fp = NULL;
	int rv = stat(file_full_path, &statbuf);
	if(rv != 0) {
	...
	fd = _open(file_full_path, O_RDONLY);
	if(fd < 0) {
	...
	if(opts.mmap) {
		...
		if(!opts.search_binary_files) {
			...
			// Optimization: If skipping binary files, don't read the whole buffer before checking if binary
			// or not.
...
static int check_symloop_enter(const char * path, dirkey_t * outkey) 
{
	...
	memzero(outkey, sizeof(dirkey_t));
	outkey->dev = 0;
	outkey->ino = 0;
...
//
//
int ag_scandir(const char * pDirName, struct dirent *** pppNameList, filter_fp filter, void * pBaton) 
{
	struct dirent ** names = NULL;
	struct dirent * entry, * d;
	int names_len = 32;
	int results_len = 0;
	DIR * dirp = opendir(pDirName);
	if(dirp == NULL) {
		goto fail;
	}
	names = (dirent **)SAlloc::M(sizeof(struct dirent *) * names_len);
	if(names == NULL) {
		goto fail;
	}
	while((entry = readdir(dirp)) != NULL) {
		if((*filter)(pDirName, entry, pBaton) == FALSE) {
			continue;
		}
		if(results_len >= names_len) {
			struct dirent ** tmp_names = names;
			names_len *= 2;
			names = (dirent **)SAlloc::R(names, sizeof(struct dirent *) * names_len);
			if(names == NULL) {
				SAlloc::F(tmp_names);
				goto fail;
			}
		...
		d = (dirent *)SAlloc::M(sizeof(struct dirent));
#else
		d = (dirent *)SAlloc::M(entry->d_reclen);
#endif
		if(d == NULL) {
			goto fail;
		}
...
void search_dir(ignores * ig, const char * base_path, const char * path, const int depth, dev_t original_dev) 
{
	...
	work_queue_t * queue_item = 0;
	int symres = check_symloop_enter(path, &current_dirkey);
		...
		if(results == 0) {
			...
			if(errno == ENOTDIR) {
				...
				if(depth == 0 && opts.paths_len == 1) {
					...
					/* If we're only searching one file and --only-matching is specified, disable line
					   numbers too. */
			...
			for(i = 0; i < results; i++) {
				...
				if(!is_directory(path, dir)) {
					if(opts.file_search_regex) {
						rc = pcre_exec(opts.file_search_regex, NULL, dir_full_path, strlen(dir_full_path), 0, 0, offset_vector, 3);
						if(rc < 0) { /* no match */
					...
					if(depth < opts.max_search_depth || opts.max_search_depth == -1) {
						...
						ignores * child_ig = init_ignore(ig, dir->d_name, dir->d_namlen);
		#else
						ignores * child_ig = init_ignore(ig, dir->d_name, strlen(dir->d_name));
		#endif
						search_dir(child_ig, base_path, dir_full_path, depth + 1, original_dev);
						cleanup_ignore(child_ig);
					}
					else {
						if(opts.max_search_depth == DEFAULT_MAX_SEARCH_DEPTH) {
							// 
							// If the user didn't intentionally specify a particular depth, this is a warning...
							// 
