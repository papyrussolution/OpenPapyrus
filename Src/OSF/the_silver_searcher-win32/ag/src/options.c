...
size_t get_lang_count() { return sizeof(langs) / sizeof(lang_spec_t); }
...
void parse_options(int argc, char ** argv, char ** base_paths[], char ** paths[]) 
{
	...
	memzero(ext_index, sizeof(size_t) * lang_count);
	for(i = 0; i < lang_count; i++) {
		option_t opt = { langs[i].name, no_argument, NULL, 0 };
		longopts[i + longopts_len] = opt;
	}
	// @sobolev longopts[full_len - 1] = (option_t){NULL, 0, NULL, 0 };
	memzero(longopts+full_len-1, sizeof(longopts[full_len-1])); // @sobolev
	if(argc < 2) {
		usage();
	...
	rv = fstat(_fileno(stdin), &statbuf);
	if(rv == 0) {
	...
	// turn off colors print filenames on every line
	// 
	if(!_isatty(_fileno(stdout))) {
		opts.color = 0;
		group = 0;
		// Don't search the file that stdout is redirected to 
		rv = fstat(_fileno(stdout), &statbuf);
		if(rv != 0) {
	...
	if(list_file_types) {
		...
		for(lang_index = 0; lang_index < lang_count; lang_index++) {
			...
			for(int j = 0; j < MAX_EXTENSIONS && langs[lang_index].extensions[j]; j++) {
...
#ifdef _WIN32
		gitconfig_file = _popen("git config -z --path --get core.excludesfile 2>NUL", "r");
#else
		...
		for(i = 0; i < (size_t)argc; i++) {
			...
			if(base_path) {
				...
				if(base_path_len > 1 && base_path[base_path_len - 1] != '/') {
					base_path = (char *)ag_realloc(base_path, base_path_len + 2);
