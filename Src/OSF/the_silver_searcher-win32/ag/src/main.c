...
int main(int argc, char ** argv) 
{
...
#endif
	SLS.Init("TheSilverSearcher"); // @sobolev
	pthread_win32_process_attach_np(); // @sobolev
	...
	if(opts.search_stream) {
		...
		for(i = 0; paths[i] != NULL; i++) {
			...
			struct stat s; // @sobolev = { .st_dev = 0 };
			memzero(&s, sizeof(s)); // @sobolev
#ifndef _WIN32
	...
	if(opts.stats) {
		...
		double time_diff = ((long)stats.time_end.tv_sec * 1000000 + stats.time_end.tv_usec) - ((long)stats.time_start.tv_sec * 1000000 + stats.time_start.tv_usec);
		time_diff /= 1000000;
	...
	for(i = 0; paths[i] != NULL; i++) {
		SAlloc::F(paths[i]);
		SAlloc::F(base_paths[i]);
	}
	SAlloc::F(base_paths);
	SAlloc::F(paths);
	SAlloc::F(find_skip_lookup);
