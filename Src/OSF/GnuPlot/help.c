...
static int LoadHelp(char * path)
{
	...
	if((helpfp = fopen(path, "r")) == NULL) {
		return (H_ERROR); // can't open help file, so error exit 
	}
	...
	while(!feof(helpfp)) {
		...
		//
		// Now store the text for this entry. buf already contains the first line of text.
		//
		firsthead = storeline(buf);
		head = firsthead;
		while(fgets(buf, MAX_LINE_LEN - 1, helpfp) && (buf[0] != KEYFLAG)) {
			// save text line 
...
static LINEBUF * storeline(char * text)
{
	LINEBUF * p_new = (LINEBUF*)malloc(sizeof(LINEBUF), "p_new line buffer");
	if(text)
...
static LINKEY * storekey(char * key)
{
	...
	p_new = (LINKEY*)malloc(sizeof(LINKEY), "p_new key list");
	if(key)
...
static void sortkeys()
{
	...
	/* allocate the array */
	keys = (KEY*)malloc((keycount + 1) * sizeof(KEY), "key array");
 ...
 * illegal! */
static int keycomp(const void * arg1, const void * arg2)
{
	const KEY * a = arg1;
	const KEY * b = arg2;
