...
struct poly_struct {
	edge_struct * edge[3];   /* As we do triangolation here... */
	poly_struct * next; /* To chain lists. */
};
//
// Contours are saved using this struct list
//
struct cntr_struct : public RPoint { // The coordinates of this vertex
...
static RPoint3 /*x_min, y_min, z_min*/ContourMin; // Minimum values of x, y, and z
static RPoint3 /*x_max, y_max, z_max*/ContourMax; // Maximum values of x, y, and z 
...
static void gen_triangle(int num_isolines, iso_curve * iso_lines, poly_struct **p_polys, edge_struct **p_edges);
static void calc_min_max(int num_isolines, iso_curve * iso_lines, double * xx_min, double * yy_min, double * zz_min, double * xx_max, double * yy_max, double * zz_max);
...
gnuplot_contours * contour(int num_isolines, iso_curve * iso_lines)
{
	...
	if(contour_levels_kind == LEVELS_AUTO) {
		...
		z = floor(ContourMin.z / dz) * dz;
		num_of_z_levels = (int)floor((ContourMax.z - z) / dz);
	}
	for(i = 0; i < num_of_z_levels; i++) {
		...
		gen_contours(p_edges, z, ContourMin.x, ContourMax.x, ContourMin.y, ContourMax.y);
...
static void end_crnt_cntr()
{
	gnuplot_contours * cntr = (gnuplot_contours *)malloc(sizeof(gnuplot_contours));
	cntr->coords = (GpCoordinate *)malloc(sizeof(GpCoordinate) * crnt_cntr_pt_index);
	for(int i = 0; i < crnt_cntr_pt_index; i++) {
		cntr->coords[i].Set(crnt_cntr[i * 2], crnt_cntr[i * 2 + 1], contour_level);
	}
...
{
	cntr_struct * p_cntr;
	int num_active = update_all_edges(p_edges, z_level); /* Number of edges marked ACTIVE. */ /* Do pass 1. */
...
static cntr_struct * gen_one_contour(edge_struct * p_edges/* list of edges input */,
	...
	bool * contr_isclosed/* open or closed contour, in/out */, int * num_active/* number of active edges in/out */)
{
	edge_struct * pe_temp;
	if(!*contr_isclosed) {
...
static cntr_struct * update_cntr_pt(edge_struct * p_edge, double z_level)
{
	...
	t = (t < 0.0 ? 0.0 : t);
	t = (t > 1.0 ? 1.0 : t);
	p_cntr = (cntr_struct *)malloc(sizeof(cntr_struct));
...
// calculating the relative error (< GpEpsilon)
//
static int fuzzy_equal(cntr_struct * p_cntr1, cntr_struct * p_cntr2)
{
	double unit_x = fabs(ContourMax.x - ContourMin.x); // reference 
	double unit_y = fabs(ContourMax.y - ContourMin.y);
	return ((fabs(p_cntr1->x - p_cntr2->x) < unit_x * GpEpsilon) && (fabs(p_cntr1->y - p_cntr2->y) < unit_y * GpEpsilon));
}
 ...
 */
static void gen_triangle(int num_isolines,           /* number of iso-lines input */
    iso_curve * iso_lines,        /* iso-lines input */
    ...
    edge_struct ** p_edges)      /* list of edges output */
{
	...
	for(i = 1; i < num_isolines; i++) {
		...
		for(j = 0; j < grid_x_max - 1; j++) {
			...
			add_poly(edge0, edge1, edge2, p_polys, &pp_tail); // generate lower triangle 
			edge0 = edge1; // copy diagonal edge for upper triangle 
			edge1 = add_edge(p_vrtx2 + j, p_vrtx2 + j + 1, &p_edge2, &pe_tail2); // generate upper edge 
			edge2 = add_edge(p_vrtx1 + j + 1, p_vrtx2 + j + 1, p_edges, &pe_tail); // generate vertical edge
			add_poly(edge0, edge1, edge2, p_polys, &pp_tail); // generate upper triangle 
		}
		if(p_edge2) {
 ...
 */
static void calc_min_max(int num_isolines,           /* number of iso-lines input */
    iso_curve * iso_lines, /* iso-lines input */
    ...
    double * xx_max, double * yy_max, double * zz_max) /* min/max values in/out */
{
	int i, j;
	GpCoordinate * vertex;
	int grid_x_max = iso_lines->p_count; /* number of vertices per iso_line */
	(*xx_min) = (*yy_min) = (*zz_min) = GPVL;  /* clear min/max values */
	(*xx_max) = (*yy_max) = (*zz_max) = -GPVL;
...
//
static edge_struct * add_edge(GpCoordinate * point0/* 2 vertices input */,
    GpCoordinate * point1, edge_struct ** p_edge/* pointers to edge list in/out */, edge_struct ** pe_tail)
{
...
#endif
	{
		pe_temp = (edge_struct *)malloc(sizeof(edge_struct));
 ...
 */
static poly_struct * add_poly(edge_struct * edge0, edge_struct * edge1,
    edge_struct * edge2 /* 3 edges input */, poly_struct ** p_poly, poly_struct ** pp_tail/* pointers to polygon list in/out */)
{
	poly_struct * pp_temp = NULL;
	if(edge0 && edge1 && edge2) {
		pp_temp = (poly_struct *)malloc(sizeof(poly_struct));
 ...
 */
static void put_contour(cntr_struct * p_cntr /* contour structure input */, double xx_min, double xx_max,
	double yy_min, double yy_max, /* minimum/maximum values input */ bool contr_isclosed/* contour line closed? (input) */)
{
	if(p_cntr) { // Nothing to do if it is empty contour
		switch(interp_kind) {
			...
			case CONTOUR_KIND_CUBIC_SPL: /* Cubic spline interpolation. */
				put_contour_cubic(p_cntr, xx_min, xx_max, yy_min, yy_max, chk_contour_kind(p_cntr, contr_isclosed) ? true : false);
				break;
			case CONTOUR_KIND_BSPLINE: /* Bspline approximation. */
				put_contour_bspline(p_cntr, chk_contour_kind(p_cntr, contr_isclosed) ? true : false);
				break;
		}
		free_contour(p_cntr);
	}
...
static void put_contour_cubic(cntr_struct * p_cntr, double xx_min, double xx_max, double yy_min, double yy_max, bool contr_isclosed)
{
	...
	delta_t = (double *)malloc(num_pts * sizeof(double));
	d2x = (double *)malloc(num_pts * sizeof(double));
	d2y = (double *)malloc(num_pts * sizeof(double));
	...
	unit_x = (unit_x > GpGg.Zero ? unit_x : GpGg.Zero); /* should not be zero */
	unit_y = (unit_y > GpGg.Zero ? unit_y : GpGg.Zero);
	if(num_pts > 2) {
    ...
    double unit_x, double unit_y) /* Unit length in x and y (norm=1), input */
{
	...
	double norm;
	cntr_struct * pc_temp;
	tri_diag * m = (tri_diag *)malloc(num_pts * sizeof(tri_diag)); // The tri-diagonal matrix is saved here
	/*
	...
	for(i = 0; i < num_pts - 1; i++) {
		...
		//
		// The norm of a linear stroke is calculated in "normal coordinates" and used as interval length:
		//
...
static void gen_bspline_approx(cntr_struct * p_cntr, int num_of_points, int order, bool contr_isclosed)
{
	...
	eval_bspline(t_max - GpEpsilon, pc_temp, num_of_points, order, knot_index, contr_isclosed, &x, &y);
...
static void eval_bspline(double t, cntr_struct * p_cntr,
    int num_of_points, int order, int j, bool contr_isclosed, double * x, double * y)
{
	int i, p;
	double ti, tikp, * dx, * dy; /* Copy p_cntr into it to make it faster. */
	dx = (double *)malloc((order + j) * sizeof(double));
	dy = (double *)malloc((order + j) * sizeof(double));
