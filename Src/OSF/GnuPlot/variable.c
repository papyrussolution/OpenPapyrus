...
static bool fontpath_init_done = false;

/*
 * char *fontpath_handler (int, char *)
 *
 */
char * fontpath_handler(int action, char * path)
{
	/* fontpath variable
	 * the path elements are '\0' separated (!)
...
#  endif
						    ...
						    if(envbeg) {
							    ...
							    tmpdir = (char *)malloc(strlen(currdir)+envlen +envbeg-envend+1);
...
#  if defined(PIPES)
						    /* Read environment variables */
							    ...
							    tmpdir = malloc(strlen(currdir)+envlen+cmdbeg-envend+1);
...
#  endif
					    ...
					    if(existdir(currdir) ) {
						    ...
						    if(fontpath) {
							    ...
							    fontpath = (char *)gp_realloc(fontpath, elen + 1 + plen + 1, "expand fontpath");
							    last = fontpath+elen;
							    *last = PATHSEP;
							    ++last;
							    *last = '\0';
...
#else
		    current_locale = (char *)gp_realloc(current_locale, strlen(newlocale) + 1, "locale");
