...
#endif
#if defined(_MSC_VER)
// @sobolev int ms_vsnprintf(char *str, size_t size, const char *format, va_list ap);
// @sobolev int ms_snprintf(char *str, size_t size, const char * format, ...);
#endif
#ifndef GP_GETCWD
		...
		#else
			#define GP_GETCWD(path,len) _getcwd(path, len)
		#endif /* __EMX__ */
	#else
...
/*
 * Note about GPVL:  This is the upper bound double (or float, if PC)
...
#define GPVL 1e37
 *
 * To get an appropriate value for GPVL, we can use DBL_MAX (or
 ...
 * DBL_MAX).
 */
/* Now define GPVL. This is usually DBL_MAX/2 - 1. On MS-DOS however
 * we use floats for memory considerations and thus use FLT_MAX.
 */

#ifndef COORDVAL_FLOAT
	#ifdef DBL_MAX
		#define GPVL (DBL_MAX/2-1)
	#endif
#else /* COORDVAL_FLOAT */
	#ifdef FLT_MAX
		#define GPVL (FLT_MAX/2-1)
	#endif
#endif /* COORDVAL_FLOAT */
#ifndef GPVL
	#ifdef HUGE
		#define GPVL (HUGE/2-1)
	#elif defined(HUGE_VAL)
		#define GPVL (HUGE_VAL/2-1)
	#else
	/* as a last resort */
		#define GPVL (1e37)
	/* #  warning "using last resort 1e37 as GPVL define, please check your headers" */
...
#ifndef inrange
	#define inrange(z_,lo_,up_) (((lo_)<(up_)) ? (((z_)>=(lo_)) && ((z_)<=(up_))) : (((z_)>=(up_)) && ((z_)<=(lo_))))
#endif
//
// HBB 20030117: new macro to simplify clipping operations in the presence of possibly reversed axes
//
#ifndef cliptorange
