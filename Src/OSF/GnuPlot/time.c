...
static size_t xstrftime(char * buf, size_t bufsz, const char * fmt, struct tm * tm, double usec, double fulltime);

/* days in year */
static int gdysize(int yr)
{
	if(!(yr % 4))
		return ((!(yr % 100)) && yr % 400) ? 365 : 366;
	else
		return (365);
}
...
char * gstrptime(char * s, char * fmt, struct tm * tm, double * usec)
{
	int    yday = 0;
...
static size_t xstrftime(char * str /* output buffer */, size_t bsz/* space available */, const char * fmt, struct tm * tm, double usec, double fulltime)
{
		...
		CHECK_SPACE(incr);			    \
				...
				case 'S':
				    ...
				    if(p > 0) {
					    ...
					    int msec = (int)floor(0.5 + base * usec);
					    char * f = &s[strlen(s)];
					    CHECK_SPACE(p+1);
				...
				case 't': /* Time (as opposed to Date) formats */
			    {
				    ...
				    switch(*fmt++) {
						...
						tminute = (int)floor(60. * (fabs(fulltime/3600.) - floor(fabs(fulltime/3600.))));
						if(fulltime < 0) {
							if(!sign_printed) {
								sign_printed = true;
								*s++ = '-';
					    ...
					    case 'S':
						/* +/- fractional seconds */
						tsecond = (int)floor(60. * (fabs(fulltime/60.) - floor(fabs(fulltime/60.))));
						if(fulltime < 0) {
							if(usec > 0)
								usec = 1.0 - usec;
							if(!sign_printed) {
...
double gtimegm(struct tm * tm)
{
	...
	if(tm->tm_year < ZERO_YEAR) {
		for(int i = tm->tm_year; i < ZERO_YEAR; i++) {
			dsec -= (double)gdysize(i);
		}
	}
	else {
		for(int i = ZERO_YEAR; i < tm->tm_year; i++) {
			dsec += (double)gdysize(i);
		}
	}
	if(tm->tm_mday > 0) {
	...
	FPRINTF((stderr, "broken-down time : %02d/%02d/%d:%02d:%02d:%02d = %g seconds\n", tm->tm_mday, tm->tm_mon + 1, tm->tm_year,
		    tm->tm_hour, tm->tm_min, tm->tm_sec, dsec));
	return (dsec);
}
...
size_t gstrftime(char * buf, size_t bufsz, const char * fmt, double l_clock)
{
...
char * gstrptime(char * s, char * fmt, struct tm * tm)
{
