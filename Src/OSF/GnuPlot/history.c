...
void GpHistory::AddHistory(char * line)
{
	static GpHistEntry * first_entry = NULL; // @global
	// this points to first entry in history list, whereas "history" points to last entry
	static long hist_count = 0; // @global
	// number of entries in history list 
	GpHistEntry * entry = history;
	if(!history_full)
		while(entry != NULL) {
...
#endif
	entry = (GpHistEntry *)malloc(sizeof(GpHistEntry));
...
void GpHistory::WriteHistoryN(const int n, const char * filename, const char * mode)
{
	GpHistEntry * entry = history;
	GpHistEntry * start = NULL;
	FILE * out = stdout;
#ifdef PIPES
	...
	if(entry) {
		...
		entry = start;
		hist_index = (n > 0) ? MAX(hist_entries - n, 0) + 1 : 1;
...
void GpHistory::WriteHistory(char * filename)
{
	WriteHistoryN(0, filename, "w");
}
//
...
void GpHistory::ReadHistory(char * filename)
{
	FILE * hist_file;
...
const char * GpHistory::HistoryFindByNumber(int n)
{
	if(history && n > 0) {
		GpHistEntry * entry = history;
		int hist_entries = 0;
...
const char * GpHistory::HistoryFind(char * cmd)
{
	GpHistEntry * entry = history;
	size_t len;
	char * line;
...
int GpHistory::HistoryFindAll(char * cmd)
{
	GpHistEntry * entry = history;
	int hist_index = 1;
	char res = 0;
...
void GpHistory::WriteHistoryList(const int num, const char * const filename, const char * mode)
{
	 ...
	 */
	istart = (num > 0) ? (history_length - num) : 1;
...
void GpHistory::WriteHistoryN(const int n, const char * filename, const char * mode)
{
	WriteHistoryList(n, filename, mode);
}
//
// finds and returns a command from the history list by number 
//
