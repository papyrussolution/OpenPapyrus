...
#define EQ(X, Y)  (fabs( (X)-(Y) ) < GpEpsilon) // X == Y
#define GR(X, Y)  ((X) >  (Y) + GpEpsilon) // X >  Y 
#define GE(X, Y)  ((X) >= (Y)-GpEpsilon) // X >= Y
#define SIGN(X)   (((X)<-GpEpsilon) ? -1 : ((X)>GpEpsilon))
//
...
#endif
#if HIDDEN3D_GRIDBOX
	ulong xbits;    /* x coverage mask of bounding box */
	ulong ybits;    /* y coverage mask of bounding box */
#endif
};
...
#if HIDDEN3D_GRIDBOX
	#define UINT_BITS                           (CHAR_BIT * sizeof(uint))
	#define COORD_TO_BITMASK(x, shift)          (~0U << (uint)((((x) / surface_scale) + 1.0) / 2.0 * UINT_BITS + (shift)))
	#define CALC_BITRANGE(range_min, range_max) ((~COORD_TO_BITMASK((range_max), 1)) & COORD_TO_BITMASK(range_min, 0))
#endif
 ...
 *
 * Directions are interpreted in a pseudo-geographical GpCoordinate
...
static int coord_to_treecell(double x)
{
	int    index = ((((x) / surface_scale) + 1.0) / 2.0) * QUADTREE_GRANULARITY;
...
static void color_edges(long new_edge, long old_edge, long new_poly, long old_poly, int style_above, int style_below);
static void build_networks(SurfacePoints * plots, int pcount);
static int compare_edges_by_zmin(const void * p1, const void * p2);
static int compare_polys_by_zmax(const void * p1, const void * p2);
static void sort_edges_by_z();
static void sort_polys_by_z();
static bool get_plane(p_polygon p, t_plane plane);
static long split_line_at_ratio(long vnum1, long vnum2, double w);
static GP_INLINE double area2D(GpVertex * v1, GpVertex * v2, GpVertex * v3);
//static void draw_vertex(GpVertex * v);
//static GP_INLINE void draw_edge(Edge * e, GpVertex * v1, GpVertex * v2);
...
void GpGadgets::SetHidden3DOptions(GpCommand & rC)
{
	int tmp;
	while(!rC.EndOfCommand()) {
		switch(rC.LookupTable(&set_hidden3d_tbl[0], rC.CToken)) {
			case S_HI_DEFAULTS:
			...
			case S_HI_OFFSET:
			    rC.CToken++;
			    hiddenBacksideLinetypeOffset = rC.IntExpression();
			    rC.CToken--;
			    break;
			case S_HI_NOOFFSET:
			...
			case S_HI_TRIANGLEPATTERN:
			    rC.CToken++;
			    hiddenTriangleLinesdrawnPattern = rC.IntExpression();
			    rC.CToken--;
			    break;
			case S_HI_UNDEFINED:
			    rC.CToken++;
			    tmp = rC.IntExpression();
...
void save_hidden3doptions(FILE * fp)
{
	if(!GpGg.hidden3d) {
		...
		fprintf(fp, "set hidden3d %s offset %d trianglepattern %ld undefined %d %saltdiagonal %sbentover\n",
			GpGg.hidden3d_layer == LAYER_BACK ? "back" : "front", hiddenBacksideLinetypeOffset, hiddenTriangleLinesdrawnPattern,
...
void init_hidden_line_removal()
{
	...
	if(hiddenHandleUndefinedPoints < OUTRANGE)
		hiddenHandleUndefinedPoints = UNHANDLED;
	vertices.Init(sizeof(GpVertex), 100, 100);
	edges.Init(sizeof(GpEdge), 100, 100);
	polygons.Init(sizeof(mesh_triangle), 100, 100);
#if HIDDEN3D_QUADTREE
	qtree.Init(sizeof(qtreelist), 100, 100);
#endif
}
...
static long store_vertex(GpCoordinate * point, lp_style_type * lp_style, bool color_from_column)
{
	GpVertex * thisvert = (GpVertex *)vertices.GetNext();
	thisvert->lp_style = lp_style;
	if((int)point->type >= hiddenHandleUndefinedPoints) {
		thisvert->SetAsUndefined();
		return (-1);
	}
	else {
		GpGg.Map3DXYZ(point->x, point->y, point->z, thisvert);
...
static long make_edge(long vnum1, long vnum2, lp_style_type * lp, int style, int next)
{
	GpEdge * thisedge = (GpEdge *)edges.GetNext();
...
static GP_INLINE double intersect_line_plane(GpVertex * v1, GpVertex * v2, t_plane p)
{
	...
	if(numerator == 0)
		...
		return (denominator==0 ? (numerator>0 ? GPVL : -GPVL) : numerator/denominator);
	}
}
...
static long store_polygon(long vnum1, polygon_direction direction, long crvlen)
{
	...
	if(v1->IsEqual(*v2) || v2->IsEqual(*v3) || v3->IsEqual(*v1))
		return (-2);
	// All else OK, fill in the polygon: 
	p = (p_polygon)polygons.GetNext();
...
#define GET_MIN(poly, var, min)			\
	do {					    \
		...
		for(int i = 1; i< POLY_NVERT; i++, v++)    \
			if(vlist[*v].var < min)		   \
				min = vlist[*v].var;		\
		if(min < -GpGg.surface_scale) disable_mouse_z = true;       \
	} while(0)
...
#define GET_MAX(poly, var, max)			\
	do {					    \
		...
		for(i = 1; i< POLY_NVERT; i++, v++)    \
			if(vlist[*v].var > max)		   \
				max = vlist[*v].var;		\
		if(max > GpGg.surface_scale) disable_mouse_z = true;	       \
	} while(0)
...
static void color_edges(long new_edge,          /* index of 'new', conflictless edge */
    long old_edge,          /* index of 'old' edge, may conflict */
    long new_poly,          /* index of current polygon */
    long old_poly,          /* index of poly sharing old_edge */
 ...
 * */
static void build_networks(SurfacePoints * pPlots, int pcount)
{
	long i;
	SurfacePoints * p_plot;

	int surface;            /* count the surfaces (i.e. sub-pPlots) */
	...
	nv = ne = np = 0;
	max_crvlen = -1;
	for(p_plot = pPlots, surface = 0; surface < pcount; p_plot = p_plot->next_sp, surface++) {
		long crvlen;
		// Quietly skip empty pPlots 
		if(p_plot->plot_type == NODATA)
			continue;
		crvlen = p_plot->iso_crvs->p_count;
		// Allow individual pPlots to opt out of hidden3d calculations 
		if(p_plot->opt_out_of_hidden3d)
			continue;

		/* register maximal isocurve length. Only necessary for
		 * grid-topology pPlots that will create polygons, so I can do
		 * it here, already. */
		...
		if(p_plot->plot_type == FUNC3D) {
			...
			ncrvs = p_plot->num_iso_read;
			if(p_plot->has_grid_topology)
				nverts += ncrvs * crvlen;
			else if(p_plot->plot_style == VECTOR)
				nverts += p_plot->iso_crvs->p_count;
			else {
				/* have to check each isoline separately: */
				for(icrvs = p_plot->iso_crvs; icrvs; icrvs = icrvs->next)
	...
	// above the current one, to allow easy access to them from the current isoline 
	north_polygons = (long *)malloc(2 * max_crvlen * sizeof(long));
	these_polygons = (long *)malloc(2 * max_crvlen * sizeof(long));
	north_edges = (long *)malloc(3 * max_crvlen * sizeof(long));
	these_edges = (long *)malloc(3 * max_crvlen * sizeof(long));
	...
	for(p_plot = pPlots, surface = 0; surface < pcount; p_plot = p_plot->next_sp, surface++) {
		bool color_from_column = p_plot->pm3d_color_from_column;
		long crvlen;
		lp = &(p_plot->lp_properties);
		// Quietly skip empty pPlots 
		if(p_plot->plot_type == NODATA)
			continue;
		crvlen = p_plot->iso_crvs->p_count;
		...
		above = p_plot->hidden3d_top_linetype;
		...
		if(p_plot->plot_style == VECTOR) {
			...
			if(p_plot->arrow_properties.head == NOHEAD) {
				p_plot->arrow_properties.head_length = 1;
				p_plot->arrow_properties.head_angle = 0;
			}
			GpGg.ApplyHeadProperties(p_plot->arrow_properties);
		}
		...
		if(!p_plot->has_grid_topology) {
			for(crv = 0, icrvs = p_plot->iso_crvs; icrvs; crv++, icrvs = icrvs->next) {
				...
				if(p_plot->plot_style == LABELPOINTS) {
					...
					for(GpTextLabel * label = p_plot->labels->next; label != NULL; label = label->next) {
						labelpoint.x = label->place.x;
						labelpoint.y = label->place.y;
						labelpoint.z = label->place.z;
						labelpoint.CRD_COLOR = (label->textcolor.type == TC_Z) ? label->textcolor.value : label->textcolor.lt;
						const long thisvertex = store_vertex(&labelpoint, &(p_plot->lp_properties), color_from_column);
						if(thisvertex >= 0) {
					...
					for(i = 0; i < icrvs->p_count; i++) {
						...
						switch(p_plot->plot_style) {
							    ...
							    /* set second vertex to the low end of zrange */
						    {
							    ...
							    points[i].z = GpGg[FIRST_Z_AXIS].Range.low;
		 ...
		 * edges properly */
		for(i = 0; i < p_plot->iso_crvs->p_count; i++) {
		...
		for(crv = 0, icrvs = p_plot->iso_crvs; icrvs; crv++, icrvs = icrvs->next) {
			...
			for(i = 0; i < icrvs->p_count; i++) {
				...
				these_polygons[2 * i] = these_polygons[2 * i + 1] = 
					these_edges[3 * i] = these_edges[3 * i + 1] = these_edges[3 * i + 2] = -3;
				switch(p_plot->plot_style) {
					case PM3DSURFACE:
					case LINESPOINTS:
					case STEPS:
					    ...
					    if(i > 0) {
						    ...
						    if(thisvertex < 0) {
							    if((crv > 0) && (hiddenShowAlternativeDiagonal)) {
								     ...
								     * */
								    these_edges[i*3+2] = e3 = store_edge(vertices.end - 1, edir_NE, crvlen,
								    lp, above);
								    if(e3 > -2) {
						    ...
						    if(crv > 0) { /* vertices to the north exist */
							    ...
							    if(e3 > -2) {
								    // diagonal edge of this cell is OK, so try to build both the polygons:
								    if(e1 > -2) {
									    // one pair of edges is valid: put first polygon, which points towards the southwest 
						    ...
						    // We're at the west border of the grid, but not on the north one: put vertical end-wall edge:
					...
					case IMPULSES:
					    ...
					    // set second vertex to the low end of zrange
					    {
						    const double remember_z = points[i].z;
						    points[i].z = (p_plot->plot_style == IMPULSES) ? 0.0 : GpGg[FIRST_Z_AXIS].Range.low;
...
static void sort_edges_by_z()
{
	if(edges.end) {
		long   i;
		long * sortarray = (long *)malloc(sizeof(long) * edges.end);
...
static void sort_polys_by_z()
{
	...
	p_polygon p_this;
	if(polygons.end) {
		long * sortarray = (long *)malloc(sizeof(long) * polygons.end);
		// initialize sortarray with an identity mapping 
		...
		// traverse plist in the order given by sortarray, and set the 'next' pointers 
#if HIDDEN3D_QUADTREE
		// HBB 20000716: Loop backwards, to ease construction of linked lists from the head:
		{
			int grid_x, grid_y;
			...
			for(i = polygons.end - 1; i >= 0; i--) {
				...
				for(grid_x = grid_x_low; grid_x <= grid_x_high; grid_x++) {
					for(grid_y = grid_y_low; grid_y <= grid_y_high; grid_y++) {
						qtreelist * newhead = qtree.GetNext();
...
//
//static void draw_vertex(GpVertex * v)
void GpGadgets::DrawVertex(GpTermEntry * pT, GpVertex & rV)
{
	uint x, y;
	if(rV.lp_style) {
		const int p_type = rV.lp_style->p_type;
		TermCoord(rV, x, y);
		if((p_type >= -1 || p_type == PT_CHARACTER || p_type == PT_VARIABLE) && !ClipPoint(x, y)) {
			t_colorspec * tc = &rV.lp_style->pm3d_color;
			if(rV.label) {
				WriteLabel(pT, x, y, rV.label);
				rV.lp_style = NULL;
			}
			else {
				if(tc->type == TC_LINESTYLE && tc->lt == LT_COLORFROMCOLUMN) {
					lp_style_type style = *(rV.lp_style);
					load_linetype(&style, (int)rV.real_z);
					tc = &style.pm3d_color;
					ApplyPm3DColor(pT, tc);
				}
				else if(tc->type == TC_RGB && tc->lt == LT_COLORFROMCOLUMN)
					SetRgbColorVar(pT, (uint)rV.real_z);
				else if(tc->type == TC_RGB)
					SetRgbColorConst(pT, tc->lt);
				else if(tc->type == TC_CB)
					pT->SetColor(CB2Gray(rV.real_z) );
				else if(tc->type == TC_Z)
					pT->SetColor(CB2Gray(Z2CB(rV.real_z)) );
#ifdef HIDDEN3D_VAR_PTSIZE
				if(rV.lp_style->p_size == PTSZ_VARIABLE)
					(pT->pointsize)(pointsize * rV.original->CRD_PTSIZE);
#endif
				if(p_type == PT_CHARACTER)
					(pT->put_text)(x, y, rV.lp_style->p_char);
#ifdef HIDDEN3D_VAR_PTSIZE
				else if(p_type == PT_VARIABLE)
					(pT->point)(x, y, (int)(rV.original->CRD_PTTYPE) - 1);
#endif
				else
					(pT->point)(x, y, p_type);
				rV.lp_style = NULL; // vertex has been drawn --> flag it as done 
			}
		}
	}
}
...
//
//static void draw_edge(Edge * e, GpVertex * v1, GpVertex * v2)
void GpGadgets::DrawEdge(GpTermEntry * pT, GpEdge * pE, GpVertex * pV1, GpVertex * pV2)
{
	// It used to be that pE contained style as a integer linetype.
	// This destroyed any style attributes set in the splot command.
	// We really just want to extract a colorspec.
	t_colorspec color = pE->lp->pm3d_color;
	lp_style_type lptemp = *(pE->lp);
	bool recolor = false;
	bool arrow = oneof2(lptemp.p_type, PT_ARROWHEAD, PT_BACKARROW);
	int varcolor = (int)((arrow && (pE->style == PT_BACKARROW)) ? pV2->real_z : pV1->real_z);
	if(color.type == TC_RGB && color.lt == LT_COLORFROMCOLUMN) { // This handles 'lc rgb variable' 
		recolor = true;
	...
	else if(color.type == TC_RGB && pE->lp == &BorderLp) {
		lptemp.pm3d_color.lt = varcolor;
	}
		...
		lptemp.p_type = pE->style;
	}
	else if((hiddenBacksideLinetypeOffset != 0) && (pE->lp->pm3d_color.type != TC_Z)) { // This is the default style: color top and bottom in successive colors 
		recolor = true;
		load_linetype(&lptemp, pE->style + 1);
	...
	if(arrow) {
		if(pE->v2 != pV2-vlist && pE->v1 != pV1-vlist) {
			lptemp.p_type = 0;
		}
		else if(pE->style == PT_BACKARROW) {
			if(pE->v2 == pV2-vlist && pE->v1 != pV1-vlist)
				lptemp.p_type = 0;
		}
		else {
			if(pE->v1 == pV1-vlist && pE->v2 != pV2-vlist)
				lptemp.p_type = 0;
		}
	}
	Draw3DLineUnconditional(pT, pV1, pV2, &lptemp, color);
	if(pE->lp->flags & LP_SHOW_POINTS) {
		DrawVertex(pT, *pV1);
		DrawVertex(pT, *pV2);
	}
}
...
{
	// Create a new vertex 
	GpVertex * v = (GpVertex *)vertices.GetNext();
	...
	if(v->IsEqual(vlist[vnum1])) {
		droplast_dynarray(&vertices);
		return vnum1;
	}
	else if(v->IsEqual(vlist[vnum2])) {
		droplast_dynarray(&vertices);
		return vnum2;
	}
	else
...
static GP_INLINE double area2D(GpVertex * v1, GpVertex * v2, GpVertex * v3)
{
	const double dx12 = v2->x - v1->x; /* x/y components of (v2-v1) and (v3-v1) */
	const double dx13 = v3->x - v1->x;
	const double dy12 = v2->y - v1->y;
	const double dy13 = v3->y - v1->y;
    ...
    long * firstpoly/* first plist index to consider */)
{
	...
	double xmin, xmax;    /* all of these are for the edge */
	double ymin, ymax;
	double zmin;
#if HIDDEN3D_GRIDBOX
	uint xextent;   /* extent bitmask in x direction */
	uint yextent;   /* same, in y direction */

#define SET_XEXTENT xextent = CALC_BITRANGE(xmin, xmax);
#define SET_YEXTENT yextent = CALC_BITRANGE(ymin, ymax);
#else
	 ...
	 * allow modifying '*firstpoly', without moving it too far to the front. */
	double first_zmin;
...
#define setup_edge(vert1, vert2)		\
	...
	for(polynum = *firstpoly; polynum >=0; polynum = p->next)
#endif /* HIDDEN3D_QUADTREE */
			{
				...
				if(0
				    ...
				    || (!(xextent & p->xbits)) || (!(yextent & p->ybits))
#endif
				    || (p->xmax < xmin) || (p->xmin > xmax) || (p->ymax < ymin) || (p->ymin > ymax))
					continue;
							...
							if(covA == 2 && covB == 2)
								...
								if(i == 0) {
									...
									if(newvert[1] != vnum1) {
										...
										in_front(edgenum, newvert[1], vnum2, &plist[polynum].next);
#endif
	...
	GpGg.DrawEdge(term, elist + edgenum, vlist + vnum1, vlist + vnum2);
	while(vertices.end > enter_vertices)
		droplast_dynarray(&vertices);
	return 1;
}
 ...
 * visible surface. */
/* NB: The GpVertex * arguments are not allowed to be pointers into the
...
void GpGadgets::DrawLineHidden(GpTermEntry * pT, GpVertex * v1, GpVertex * v2/* pointers to the end vertices */, lp_style_type * lp/* line and point style to draw in */)
{
	...
	if(!polygons.end) {
		Draw3DLineUnconditional(pT, v1, v2, lp, lp->pm3d_color);
	}
	else {
...
//
// Externally callable function to draw a label, but hide it behind any visible occluding surfaces. 
//
void draw_label_hidden(GpVertex * v, lp_style_type * lp, int x, int y)
{
	// If there is no surface to hide behind, just draw the label 
	if(!polygons.end) {
		GpGg.WriteLabel(term, x, y, v->label);
	}
	else {
...
void plot3d_hidden(SurfacePoints * plots, int pcount)
{
	...
	if(!polygons.end) {
		...
		sort_edges_by_z();
		while(efirst >= 0) {
			GpGg.DrawEdge(term, elist+efirst, vlist + elist[efirst].v1, vlist + elist[efirst].v2);
			efirst = elist[efirst].next;
		}
	}
	else {
		long temporary_pfirst;
...
void reset_hidden3doptions()
{
	...
	GpGg.hidden3d_layer = LAYER_BACK;
}
