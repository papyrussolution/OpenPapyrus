...
static GP_LPPRINT PrintFind(HDC hdc)
{
	GP_LPPRINT p_this;
	p_this = prlist;
	while(p_this && (p_this->hdcPrn!=hdc)) {
		p_this = p_this->next;
	}
	return p_this;
}
...
{
	GP_LPPRINT p_this, prev;
	prev = (GP_LPPRINT)NULL;
	p_this = prlist;
	while(p_this && (p_this!=lpr)) {
		prev = p_this;
		p_this = p_this->next;
	}
	if(p_this && (p_this == lpr)) {
		/* unhook it */
		if(prev)
			prev->next = p_this->next;
		else
			prlist = p_this->next;
	}
}
...
void WDPROC DumpPrinter(HWND hwnd, LPSTR szAppName, LPSTR szFileName)
{
	...
	if(PrintDlg(&pd)) {
		...
		if((buf = (char *)malloc(4096 + 2)) != NULL) {
			bufcount = (WORD*)buf;
