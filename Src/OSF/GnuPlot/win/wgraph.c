...
static void     GetPlotRect(GW * lpgw, LPRECT rect);
...
static uint WDPROC GraphGetTextLength(GW * lpgw, HDC hdc, LPCSTR text);
static void     draw_text_justify(HDC hdc, int justify);
static void     draw_put_text(GW * lpgw, HDC hdc, int x, int y, char * str);
...
static void DestroyBlocks(GW * lpgw)
{
	...
	while(p_this) {
		next = p_this->next;
		if(!p_this->gwop) {
			p_this->gwop = (struct GWOP*)GlobalLock(p_this->hblk);
		}
		if(p_this->gwop) {
...
void WDPROC GraphOp(GW * lpgw, UINT op, UINT x, UINT y, LPCSTR str)
{
	if(str)
...
void WDPROC GraphOpSize(GW * lpgw, UINT op, UINT x, UINT y, LPCSTR str, DWORD size)
{
	struct GWOP * gwop;
	char * npstr;
...
void WDPROC GraphInitStruct(GW * lpgw)
{
	if(!lpgw->initialized) {
		int i;
		...
		if(lpgw != listgraphs) {
			...
			lpgw->Title = _strdup(titlestr);
		}
		else {
			lpgw->Title = _strdup(WINGRAPHTITLE);
		}
		lpgw->sampling = 1;
		lpgw->fontscale = 1.;
		lpgw->linewidth = 1.;
...
void WDPROC GraphInit(GW * lpgw)
{
	...
	/* resize to match requested GpGg.Canvas size */
	if(lpgw->Canvas.x != 0) {
...
void WDPROC GraphUpdateWindowPosSize(GW * lpgw)
{
	/* resize to match requested GpGg.Canvas size */
	if(lpgw->Canvas.x != 0) {
		lpgw->Size.x = lpgw->Canvas.x + lpgw->Decoration.x;
...
void WDPROC GraphClose(GW * lpgw)
{
#ifdef USE_MOUSE
...
void WDPROC GraphStart(GW * lpgw, double pointsize)
{
	lpgw->locked = true;
	lpgw->buffervalid = false;
	DestroyBlocks(lpgw);
...
/* shige */
void WDPROC GraphChangeTitle(GW * lpgw)
{
	if(GraphHasWindow(lpgw))
		SetWindowText(lpgw->hWndGraph, lpgw->Title);
}

void WDPROC GraphResume(GW * lpgw)
{
	lpgw->locked = true;
}

void WDPROC GraphPrint(GW * lpgw)
{
	if(GraphHasWindow(lpgw))
		SendMessage(lpgw->hWndGraph, WM_COMMAND, M_PRINT, 0L);
}

void WDPROC GraphRedraw(GW * lpgw)
{
	lpgw->buffervalid = false;
	if(GraphHasWindow(lpgw))
...
static bool TryCreateFont(GW * lpgw, char * fontface, HDC hdc)
{
	if(fontface != NULL)
...
static void MakeFonts(GW * lpgw, LPRECT lprect, HDC hdc)
{
	...
	lpgw->lf.lfHeight = -MulDiv((int)(lpgw->fontsize * lpgw->fontscale), GetDeviceCaps(hdc, LOGPIXELSY), 72) * lpgw->sampling;
		...
		if(strcmp(lpgw->fontname, lpgw->deffontname) != 0) {
			...
			if(!TryCreateFont(lpgw, lpgw->deffontname, hdc)) {
				if(strcmp(lpgw->deffontname, GraphDefaultFont()) != 0) {
					fprintf(stderr,
					    "Warning:  font \"%s\" not available, trying \"%s\" instead.\n",
					    lpgw->deffontname,
					    GraphDefaultFont());
			...
			if(strcmp(lpgw->fontname, GraphDefaultFont()) != 0) {
				...
				fprintf(stderr,
				    "Error:  font \"%s\" not available, but don't know which font to substitute.\n",
				    lpgw->fontname);
			}
		}
	}
...
static void SelFont(GW * lpgw)
{
	...
	if((p = _fstrstr(lpgw->fontname, " Bold")) != (LPSTR)NULL) {
		...
		lf.lfFaceName[ (uint)(p-lpgw->fontname) ] = '\0';
	}
...
void WDPROC GraphEnhancedOpen(char * fontname, double fontsize, double base, BOOL widthflag, BOOL showflag, int overprint)
{
	...
	if(!enhstate.opened_string) {
		...
		/* Select font */
		const char * fontstring = sstrlen(fontname) ? fontname : enhstate.lpgw->deffontname;
		strcpy(enhstate.fontname, fontstring);
		enhstate.fontsize = fontsize;
		GraphChangeFont(enhstate.lpgw, enhstate.fontname, (int)enhstate.fontsize, enhstate.hdc, *(enhstate.rect));
		...
		enhstate.base = win_scale * base * enhstate.lpgw->sampling * enhstate.lpgw->fontscale * enhstate.res_scale;
	}
}
...
int draw_enhanced_text(GW * lpgw, HDC hdc, LPRECT rect, int x, int y, const char * str)
{
	...
	if((GetDeviceCaps(hdc, TECHNOLOGY) == DT_RASPRINTER)) {
		...
		enhstate.res_scale = (double)GetDeviceCaps(hdc, VERTRES) / (double)GetDeviceCaps(hdc_screen, VERTRES);
	...
	enhstate.x += (int)(sin(lpgw->angle * M_PI/180) * enhstate.shift);
	enhstate.y += (int)(cos(lpgw->angle * M_PI/180) * enhstate.shift);
	...
	for(pass = 1; pass <= num_passes; pass++) {
		 ...
		 */
		while(*(str = enhanced_recursion(str, true, save_fontname, save_fontsize, 0.0, true, true, 0))) {
			GraphEnhancedFlush();
			if(!*++str)
				break;  /* end of string */
...
{
	switch(justify) {
		case LEFT: SetTextAlign(hdc, TA_LEFT|TA_BOTTOM); break;
		case RIGHT: SetTextAlign(hdc, TA_RIGHT|TA_BOTTOM); break;
		case CENTRE: SetTextAlign(hdc, TA_CENTER|TA_BOTTOM); break;
	}
}
...
static void draw_new_pens(GW * lpgw, HDC hdc, LOGPEN cur_penstruct)
{
	...
	if(cur_penstruct.lopnWidth.x <= 1) {
		...
		lpgw->hsolid = ExtCreatePen(PS_GEOMETRIC|PS_SOLID|
		    (lpgw->rounded ? PS_ENDCAP_ROUND | PS_JOIN_ROUND : PS_ENDCAP_SQUARE | PS_JOIN_MITER),
		    cur_penstruct.lopnWidth.x, &lb, 0, 0);
	}
...
void draw_update_keybox(GW * lpgw, unsigned plotno, unsigned x, unsigned y)
{
	LPRECT bb;
	if(plotno) {
		if(plotno > lpgw->maxkeyboxes) {
			...
			for(int i = plotno - 1; i < (int)lpgw->maxkeyboxes; i++) {
		...
		if((int)x < bb->left) bb->left = x;
		if((int)x > bb->right) bb->right = x;
		if((int)y < bb->bottom) bb->bottom = y;
		if((int)y > bb->top) bb->top = y;
	}
}
...
static void draw_grey_out_key_box(GW * lpgw, HDC hdc, int plotno)
{
	...
	oldbrush = (HBRUSH)SelectObject(memdc, (HBRUSH)GetStockObject(LTGRAY_BRUSH));
...
void draw_image(GW * lpgw, HDC hdc, char * image, POINT corners[4], uint width, uint height, int color_mode)
{
	BITMAPINFO bmi;
	HRGN hrgn;
	int rc;
	...
	/* create clip region */
	hrgn = CreateRectRgn(MIN(corners[2].x, corners[3].x), MIN(corners[2].y, corners[3].y),
		...
		if(!lpgw->color) {
			...
			for(y = 0; y < (int)height; y++) {
				for(x = 0; x < (int)width; x++) {
					...
					// convert to gray 
					const unsigned luma = luma_from_color(p[2], p[1], p[0]);
					p[0] = p[1] = p[2] = luma;
				}
			}
		}
...
static void drawgraph(GW * lpgw, HDC hdc, LPRECT rect)
{
	 ...
	 */
	isColor = (((GetDeviceCaps(hdc, PLANES) * GetDeviceCaps(hdc, BITSPIXEL)) > 2) || (GetDeviceCaps(hdc, TECHNOLOGY) == DT_METAFILE) || 
		(GetDeviceCaps(hdc, TECHNOLOGY) == DT_PLOTTER) || (GetDeviceCaps(hdc, TECHNOLOGY) == DT_RASPRINTER));
	if(isColor) {
	...
	ps_caching = !((GetDeviceCaps(hdc, TECHNOLOGY) == DT_METAFILE) || (GetDeviceCaps(hdc, TECHNOLOGY) == DT_PLOTTER) || (GetDeviceCaps(hdc, TECHNOLOGY) == DT_RASPRINTER));
	...
	lpgw->numplots = 0;
	lpgw->hasgrid = false;
	for(i = 0; i < (int)lpgw->maxkeyboxes; i++) {
		lpgw->keyboxes[i].left = INT_MAX;
		lpgw->keyboxes[i].right = 0;
		lpgw->keyboxes[i].bottom = INT_MAX;
		lpgw->keyboxes[i].top = 0;
			...
			switch(curptr->op) {
				    ...
				    switch(boxedtext.option) {
						...
						if((boxedtext.angle % 90) == 0) {
							...
							if(boxedtext.option == TEXTBOX_OUTLINE)
								/* FIXME: Shouldn't we use the current color brush
								  lpgw->hcolorbrush? */
...
#endif
							{
								...
								if(boxedtext.option == TEXTBOX_OUTLINE) {
									...
									/* FIXME: Shouldn't we use the current color
									  brush lpgw->hcolorbrush? */
								...
								if(boxedtext.option == TEXTBOX_OUTLINE) {
									rect[4].x = rect[0].x;
									rect[4].y = rect[0].y;
									gdiplusPolylineEx(hdc, rect, 5, PS_SOLID, line_width, RGB(0,
										    0,
										    0) /* last_color */, 1.);
								}
								else {
									gdiplusSolidFilledPolygonEx(hdc,
									    rect,
									    4,
									    lpgw->background,
									    1.,
				...
				case W_boxfill: { /* ULIG */
				    ...
				    if(lpgw->antialiasing && lpgw->patternaa &&
					    ...
					    ppt[4].x = ppt[0].x;
					    ppt[4].y = ppt[0].y;
					    gdiplusPatternFilledPolygonEx(hdc,
						    ppt,
						    5,
						    fill_color,
						    1.,
				...
				case W_setcolor: {
				    ...
				    if(curptr->htext != NULL) { /* TC_LT */
					    const int pen = (int)curptr->x % WGNUMPENS;
					    if(pen <= LT_NODRAW)
						    color = lpgw->background;
					    else
							color = lpgw->color ? lpgw->colorpen[pen + 2].lopnColor : lpgw->monopen[pen + 2].lopnColor;
					    alpha_c = 1.;
				    }
				    else {                                      /* TC_RGB */
					    rgb255_color rgb255;
					    ...
					    if(lpgw->color || ((rgb255.r == rgb255.g) && (rgb255.r == rgb255.b))) {
						    color = RGB(rgb255.r, rgb255.g, rgb255.b); // Use colors or this is already gray scale 
					    }
					    else {
						    // convert to gray 
				...
				case W_filled_polygon_draw: {
				    ...
				    if(!transparent) {
					    ...
					    else if(lpgw->antialiasing && lpgw->patternaa &&
						    ((fillstyle & 0x0f) == FS_TRANSPARENT_PATTERN)) {
						    gdiplusPatternFilledPolygonEx(hdc,
							    ppt,
							    polyi,
...
static void SaveAsEMF(GW * lpgw)
{
	...
	if(GetSaveFileName(&Ofn) != 0) {
		...
		antialiasing = lpgw->antialiasing ? true : false;
...
static void CopyPrint(GW * lpgw)
{
	...
	if(StartDoc(printer, &docInfo) > 0) {
		bool aa = lpgw->antialiasing ? true : false;
		lpgw->sampling = 1;
...
static COLORREF GetColor(HWND hwnd, COLORREF ref)
{
	...
	cc.rgbResult = ref;
	cc.Flags = CC_RGBINIT;
	return ChooseColor(&cc) ? cc.rgbResult : ref;
}
...
static BOOL LineStyle(GW * lpgw)
{
	...
	if(DialogBox(hdllInstance, "LineStyleDlgBox", lpgw->hWndGraph, LineStyleDlgProc) == IDOK) {
...
#endif
	lpgw = (GW *)GetWindowLongPtr(hwnd, 0);
#ifdef USE_MOUSE
	/*  mouse events first */
	if((lpgw == graphwin) && GpGg.Mse.Cfg.on) {
		switch(message) {
			case WM_MOUSEMOVE:
#if 1
			...
			case WM_LBUTTONDOWN: 
				{
					...
					const int y = GET_Y_LPARAM(lParam) - lpgw->ToolbarHeight;
					for(uint i = 0; (i < lpgw->numplots) && (i < lpgw->maxkeyboxes) && (i < lpgw->maxhideplots); i++) {
						if((lpgw->keyboxes[i].left != INT_MAX) && (x >= lpgw->keyboxes[i].left) &&
							(x <= lpgw->keyboxes[i].right) && (y <= lpgw->keyboxes[i].top) && (y >= lpgw->keyboxes[i].bottom)) {
...
void GraphChangeFont(GW * lpgw, LPCSTR font, int fontsize, HDC hdc, RECT rect)
{
	int newfontsize;
...
void WDPROC win_close_terminal_window(GW * lpgw)
{
	if(GraphHasWindow(lpgw))
...
void WDPROC Graph_set_cursor(GW * lpgw, int c, int x, int y)
{
	switch(c) {
		...
		case 0: /* standard cross-hair cursor */
		    SetCursor((hptrCurrent = GpGg.Mse.Cfg.on ? hptrCrossHair : hptrDefault));
...
void WDPROC Graph_set_ruler(GW * lpgw, int x, int y)
{
 ...
 */
void WDPROC Graph_put_tmptext(GW * lpgw, int where, LPCSTR text)
{
...
void WDPROC Graph_set_clipboard(GW * lpgw, LPCSTR s)
{
	size_t length;
	HGLOBAL memHandle;
	LPSTR clipText;
 ...
 * Toggle active plots
 */
void WDPROC GraphModifyPlots(GW * lpgw, uint ops, int plotno)
{
	bool changed = false;
