...
char * appdata_directory(void)
{
	...
	if(dir[0])
		...
		if(hShell32) {
			pSHGetSpecialFolderPath = (SHGETSPECIALFOLDERPATH)GetProcAddress(hShell32, TEXT("SHGetSpecialFolderPathA"));
...
static char * LocalisedFile(const char * name, const char * ext, const char * defaultname)
{
	char * lang;
	char * filename;
...
static void ReadMainIni(LPSTR file, LPSTR section)
{
	char   profile[81] = "";
	...
	szLanguageCode = profile[0] ? _strdup(profile) : NULL;
	// help file name 
...
#endif
{
	...
	// locate ini file 
	{
		...
		char * inifile = gp_strdup("~\\wgnuplot.ini");
	...
	// 
	for(i = 1; i < argc; i++) {
		if(_stricmp(argv[i], "/noend") != 0) {
			if((argv[i][0] != '-') || (argv[i][1] == 'e')) {
				GpGg.IsInteractive = false;
				break;
			}
		}
	}
...
{
	return (isterm(file)) ? MyGetChE() : fgetc(file);
}
...
int fake_pclose(FILE * stream)
{
	...
	if(pipe_type == 'w') {
		char * cmd;
		cmd = (char*)malloc(strlen(pipe_command) + strlen(pipe_filename) + 10, "fake_pclose");
...
void win_raise_terminal_group(void)
{
	LPGW lpgw = listgraphs;
	while(lpgw != NULL) {
		ShowWindow(lpgw->hWndGraph, SW_SHOWNORMAL);
