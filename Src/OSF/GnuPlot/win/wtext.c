...
void CreateTextClass(LPTW lptw)
{
	...
	lptw->hbrBackground = CreateSolidBrush(lptw->bSysColors ? GetSysColor(COLOR_WINDOW) : RGB(0, 0, 0));
...
int WDPROC TextInit(LPTW lptw)
{
	...
	SETIFZ(lptw->Attr, 0xf0); // black on white 
	...
	lptw->hWndParent = CreateWindow(szParentClass, lptw->Title,
	    WS_OVERLAPPEDWINDOW, lptw->Origin.x, lptw->Origin.y, lptw->Size.x, lptw->Size.y, NULL, NULL, lptw->hInstance, lptw);
	...
	lptw->hWndText = CreateWindow(szTextClass, lptw->Title, WS_CHILD | WS_VSCROLL | WS_HSCROLL,
	    0, lptw->ButtonHeight, rect.right, rect.bottom - lptw->ButtonHeight,
	...
	lptw->hStatusbar = CreateWindowEx(0, STATUSCLASSNAME, (LPSTR)NULL, WS_CHILD | SBARS_SIZEGRIP,
	    0, 0, 0, 0, lptw->hWndParent, (HMENU)ID_TEXTSTATUS, lptw->hInstance, lptw);
	if(lptw->hStatusbar) {
		RECT rect;
		/* auto-adjust size */
 ...
 */
	AppendMenu(lptw->hPopMenu, MF_STRING | (lptw->bWrap ? MF_CHECKED : MF_UNCHECKED), M_WRAP, "&Wrap long lines");
...
int WDPROC TextPutCh(LPTW lptw, BYTE ch)
{
	switch(ch) {
		case '\r':
		    lptw->CursorPos.x = 0;
...
void TextPutStr(LPTW lptw, LPSTR str)
{
	...
	while(*str) {
		...
		for(count = 0, n = 0; *str && (isprint((uchar)*str) || (*str == '\t')); str++) {
			if(*str == '\t') {
				uint tab;
		...
		else if(*str && !isprint((uchar)*str) && (*str != '\t')) {
			TextPutCh(lptw, *str++);
		}
	}
}
...
static void DoLine(LPTW lptw, HDC hdc, int xpos, int ypos, int x, int y, int count)
{
	...
	int idx = 0;
	int num = count;
	if(y <= (int)sb_length(&(lptw->ScreenBuffer))) {
	...
	switch(message) {
		...
		case WM_SIZE: {
		    ...
		    if(!caret_visible) {
			    ...
			    if((int)((ynew + 1)* lptw->CharSize.y) > new_screensize_y)
		    ...
		    if(GetKeyState(VK_SHIFT) < 0) {
			    ...
			    switch(wParam) {
					...
					if(count < (long)(lptw->KeyBufSize-2)) {
						*lptw->KeyBufIn++ = 0;
						if((lptw->KeyBufIn - lptw->KeyBuf) >= (SSIZE_T)lptw->KeyBufSize)
		    ...
		    if(LOWORD(wParam) < NUMMENU)
				    ...
				    case M_PASTE:
				{
					...
					if(!caret_visible) {
						...
						lptw->ScrollPos.x = MAX((xnew * lptw->CharSize.x) - deltax, 0);
						if((LONG)((ynew + 1) * lptw->CharSize.y) > lptw->ScreenSize.y)
...
int WDPROC TextKBHit(LPTW lptw)
{
...
int WDPROC TextGetCh(LPTW lptw)
{
	...
	int    ch = *lptw->KeyBufOut++;
	if(ch=='\r')
		ch = '\n';
	if((lptw->KeyBufOut - lptw->KeyBuf) >= (SSIZE_T)lptw->KeyBufSize)
...
int WDPROC TextGetChE(LPTW lptw)
{
...
LPSTR WDPROC TextGetS(LPTW lptw, LPSTR str, uint size)
{
	LPSTR next = str;
	while(--size>0) {
...
void DragFunc(LPTW lptw, HDROP hdrop)
{
	...
	if(lptw->DragPre && lptw->DragPost) {
		int cFiles = DragQueryFile(hdrop, (UINT)-1, (LPSTR)NULL, 0);
...
void WriteTextIni(LPTW lptw)
{
	...
	char profile[80];
	if(file && section) {
		int iconic = IsIconic(lptw->hWndParent);
		if(iconic)
...
void ReadTextIni(LPTW lptw)
{
	...
	char profile[81];
	LPSTR p;
	BOOL bOKINI = (file != (LPSTR)NULL) && (section != (LPSTR)NULL);
	profile[0] = '\0';
	if(bOKINI)
