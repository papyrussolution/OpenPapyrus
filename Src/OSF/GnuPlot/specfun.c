...
void f_erf(GpArgument * arg)
{
	t_value a;
	double x;
	(void)arg;                      
	x = GpE.PopOrConvertFromString(a).Real();
	x = erf(x);
	GpE.Push(a.SetComplex(x, 0.0));
}

void f_erfc(GpArgument * arg)
{
	t_value a;
	double x;
	(void)arg;                      
...
void f_ibeta(GpArgument * arg)
{
	...
	(void)arg;                      
	x = GpE.PopOrConvertFromString(a).Real();
	arg2 = GpE.PopOrConvertFromString(a).Real();
	arg1 = GpE.PopOrConvertFromString(a).Real();
	x = ibeta(arg1, arg2, x);
	if(x == -1.0) {
		GpE.undefined = true;
		GpE.Push(a.SetInt(0));
...
void f_gamma(GpArgument * arg)
{
	double y;
	t_value a;
	(void)arg;                      
	y = GAMMA(GpE.PopOrConvertFromString(a).Real());
	if(y > E_MAXEXP) {
		GpE.undefined = true;
		GpE.Push(a.SetInt(0));
	}
	else
		GpE.Push(a.SetComplex(signgam * gp_exp(y), 0.0));
}

void f_lgamma(GpArgument * arg)
{
	t_value a;
	(void)arg;                      
	GpE.Push(a.SetComplex(GAMMA(GpE.PopOrConvertFromString(a).Real()), 0.0));
}

#ifndef BADRAND

void f_rand(GpArgument * arg)
{
	t_value a;
	(void)arg;                      
	GpE.Push(a.SetComplex(ranf(GpE.PopOrConvertFromString(a)), 0.0));
}
...
void f_rand(GpArgument * arg)
{
	...
	(void)pop__(&a)->Real();
	y = (781 * y + 387) % maxran;
	GpE.Push(a.SetComplex((double)y / maxran, 0.0));
}
 ...
 */
#ifndef HAVE_LIBCERF
void f_voigt(GpArgument * arg)
{
	t_value a;
	double x, y;
	(void)arg;                      
	y = GpE.PopOrConvertFromString(a).Real();
	x = GpE.PopOrConvertFromString(a).Real();
	GpE.Push(a.SetComplex(humlik(x, y), 0.0));
}
...
void f_inverse_normal(GpArgument * arg)
{                               /* Inverse normal distribution function */
	...
	if(x <= 0.0 || x >= 1.0) {
		GpE.undefined = true;
		GpE.Push(a.SetComplex(0.0, 0.0));
	}
	else {
		GpE.Push(a.SetComplex(inverse_normal_func(x), 0.0));
	}
}

void f_inverse_erf(GpArgument * arg)
{                               /* Inverse error function */
	...
	if(fabs(x) >= 1.0) {
		...
		GpE.Push(a.SetComplex(inverse_error_func(x), 0.0));
	}
}
...
void f_lambertw(GpArgument * arg)
{
	t_value a;
	double x;
	(void)arg;                     
	...
	x = lambertw(x);
	if(x <= -1)
		GpE.undefined = true; // Error return from lambertw --> flag 'undefined' 
	GpE.Push(a.SetComplex(x, 0.0));
}
...
void f_expint(GpArgument * arg)
{
	t_value a;
	double n, x;
	(void)arg;
	x = GpE.PopOrConvertFromString(a).Real();
	n = GpE.PopOrConvertFromString(a).Real();
	x = expint(n, x);
	if(x <= -1)
		GpE.undefined = true; // Error return from expint --> flag 'undefined'
	GpE.Push(a.SetComplex(x, 0.0));
}
