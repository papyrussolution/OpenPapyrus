...
double * vec(int n)
{
	return (n < 1) ? 0 : (double *)malloc(n * sizeof(double));
}
...
double ** matr(int rows, int cols)
{
	double ** m = 0;
	if(rows >= 1 && cols >= 1) {
		m = (double **)malloc(rows * sizeof(m[0]));
		m[0] = (double *)malloc(rows * cols * sizeof(m[0][0]));
...
{
	*v = (n < 1) ? NULL : (double *)gp_realloc(*v, n * sizeof((*v)[0]), "vec");
	return *v;
}
...
void Givens(double ** C, double * d, double * x, int N, int n)
{
	...
	for(j = 0; j < n; j++) {
		for(i = j + 1; i < N; i++) {
			if(C[i][j]) {
				if(fabs(C[j][j]) < (SMathConst::Epsilon * fabs(C[i][j]))) {
					...
					if(w == 0) {
						GpF.ErrorEx(NO_CARET, "w = 0 in Givens();  Cjj = %g,  Cij = %g", C[j][j], C[i][j]);
					}
					gamma = C[j][j] / w;
					sigma = -C[i][j] / w;
	...
	if(d) { // stop here if no d was specified
		...
		for(i = n - 1; i >= 0; i--) {
			...
			for(k = i + 1; k < n; k++)
				s += C[i][k] * x[k];
			if(C[i][i] == 0) {
				GpF.ErrorEx(NO_CARET, "Singular matrix in Givens()");
			}
			x[i] = -s / C[i][i];
		}
	}
...
void Invert_RtR(double ** R, double ** I, int n)
{
	...
	for(i = 0; i < n; i++) {
		...
		I[i][i] = 1;
		if(!R[i][i]) {
			GpF.ErrorEx(NO_CARET, "Singular matrix in Invert_RtR");
		}
	}
