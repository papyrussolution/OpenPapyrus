...
char * GpCommand::ReadLine_ipc(const char* prompt)
{
	...
	if(direction == 0) {
		...
		if((dir = opendir(search))) {
			...
			while((entry = readdir(dir)) != NULL) {
				if(entry->d_name[0] != '.') { // ignore files and directories starting with a dot 
			...
			ZFREE(search);
			ZFREE(name);
			return (n_completions > 0) ? completions[0] : 0;
		}
		free(search);
		ZFREE(name);
	}
		...
		if(n_completions > 0) {
			completion_idx = (direction > 0) ? ((completion_idx + 1) % n_completions) : ((completion_idx + n_completions - 1) % n_completions);
...
static void tab_completion(bool forward)
{
	...
	if(max_pos > (last_tab_pos - last_completion_len))
		memmove(cur_line + last_tab_pos + completion_len, cur_line + last_tab_pos + last_completion_len, max_pos  - last_tab_pos - last_completion_len);
...
char * GpCommand::ReadLine(const char * pPrompt)
{
	int cur_char;
	char * p_new_line;
	...
	cur_line = (char*)malloc(MAXBUF);
	line_len = MAXBUF;
	set_termio(); // set the termio so we can do our own input processing 
	// print the pPrompt 
	...
	for(;; ) {
		...
		if((isprint(cur_char) || (((cur_char & 0x80) != 0) && (cur_char != EOF))) && (cur_char != '\t') /* TAB is a printable character in some locales */) {
			size_t i;
			...
			if(cur_pos < max_pos) {
				switch(encoding) {
					case S_ENC_UTF8:
					    ...
					    else if(((cur_char & 0xc0) == 0x80) && ((uchar)(cur_line[cur_pos-2]) >= 0xe0))
			...
			switch(cur_char) {
...
#endif
				case 013: /* ^K */
				    clear_eoline(pPrompt);
				    max_pos = cur_pos;
				    break;
				case 020: // ^P 
				    if(H.history) {
					    if(H.cur_entry == NULL) {
						    H.cur_entry = H.history;
						    clear_line(pPrompt);
						    copy_line(H.cur_entry->line);
					    }
					    else if(H.cur_entry->prev) {
						    H.cur_entry = H.cur_entry->prev;
						    clear_line(pPrompt);
						    copy_line(H.cur_entry->line);
					    }
				    }
				...
				case '\r': /* ^M */
				     ...
				     */
				    p_new_line = (char*)gp_realloc(cur_line, strlen(cur_line) + 1,
				    "line resize");
				    if(p_new_line)
					    cur_line = p_new_line;
...
static void delete_previous_word()
{
	...
	if(cur_pos != save_pos) {
		...
		memzero(cur_line + cur_pos + m, save_pos - cur_pos);
...
static int msdos_getch()
{
	...
	if(term && term->waitforinput && GpGg.IsInteractive)
		c = term->waitforinput(0);
	else
	#endif // not USE_MOUSE 
	c = getch();
