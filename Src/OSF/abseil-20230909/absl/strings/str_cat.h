...
struct Hex {
	...
	friend void AbslStringify(S& sink, Hex hex) {
		...
		if(real_width >= hex.width) {
			...
			memset(end - 32, hex.fill, 16);
			// Patch up everything else up to the real_width.
			memset(end - real_width - 16, hex.fill, 16);
...
template <typename T>
ABSL_MUST_USE_RESULT inline std::string StrCat(strings_internal::EnableIfFastCase<T> a) {
...
ABSL_MUST_USE_RESULT inline std::string StrCat(const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d,
...
inline strings_internal::AlphaNumBuffer<
	numbers_internal::kSixDigitsToBufferSize>SixDigits(double d) {
