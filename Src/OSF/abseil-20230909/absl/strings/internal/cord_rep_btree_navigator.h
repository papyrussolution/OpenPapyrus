...
inline CordRepBtree* CordRepBtreeNavigator::btree() const { return height_ >= 0 ? node_[height_] : nullptr; }
...
inline CordRep* CordRepBtreeNavigator::InitFirst(CordRepBtree* tree) { return Init<CordRepBtree::kFront>(tree); }
inline CordRep* CordRepBtreeNavigator::InitLast(CordRepBtree* tree) { return Init<CordRepBtree::kBack>(tree); }

template <CordRepBtree::EdgeType edge_type> inline CordRep* CordRepBtreeNavigator::Init(CordRepBtree* tree) 
{
	assert(tree != nullptr);
	assert(tree->size() > 0);
...
inline CordRepBtreeNavigator::Position CordRepBtreeNavigator::Seek(size_t offset) {
...
inline CordRepBtreeNavigator::Position CordRepBtreeNavigator::InitOffset(CordRepBtree* tree, size_t offset) {
	assert(tree != nullptr);
	assert(tree->height() <= CordRepBtree::kMaxHeight);
