...
public:
	...
	template <typename T, typename = void>Condition(bool (*func)(T*), typename absl::internal::identity<T>::type* arg);
	...
	template <typename T>Condition(T* object, bool (absl::internal::identity<T>::type::* method)());

	// Same as above, for const members
	template <typename T>Condition(const T* object,
...
private:
	...
	inline void StoreCallback(T callback) {
		...
		memcpy(callback_, &callback, sizeof(callback));
	}

	template <typename T>
	inline void ReadCallback(T* callback) const {
		memcpy(callback, callback_, sizeof(*callback));
	}
