...
public:
		...
		explicit param_type(result_type lo = 0,
...
private:
	...
	explicit uniform_int_distribution(result_type lo,
	    result_type hi = (std::numeric_limits<result_type>::max)())
		: param_(lo, hi) {
	}
	...
	template <typename URBG>
	result_type operator()(URBG& gen, const param_type& param) { // NOLINT(runtime/references)
