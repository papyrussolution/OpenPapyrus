...
public:
	...
	template <class SeedSequence, typename = typename absl::enable_if_t<
	...
	randen_engine(const randen_engine &other) : next_(other.next_), impl_(other.impl_) 
	{
		memcpy(state(), other.state(), kStateSizeT * sizeof(result_type));
	}
	...
	typename absl::enable_if_t<
		!std::is_convertible<SeedSequence, result_type>::value>seed(SeedSequence&& seq) {
		// Zeroes the state.
		seed();
		reseed(seq);
	}
	...
	friend std ::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, // NOLINT(runtime/references)
	...
	template <class CharT, class Traits>
	friend std ::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, // NOLINT(runtime/references)
		...
		memcpy(engine.state(), state, sizeof(state));
		engine.next_ = next;
		return is;
	}
