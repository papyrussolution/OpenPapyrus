...
struct RequireUsableKey {
	...
	std::pair<
		...
		std::declval<const PassedKey&>()))>* operator()(const PassedKey&, const Args& ...) const;
};
...
public:
	...
	CommonFieldsGenerationInfoEnabled& operator=(CommonFieldsGenerationInfoEnabled&&) = default;
...
public:
	...
	CommonFieldsGenerationInfoDisabled(CommonFieldsGenerationInfoDisabled&&) =
	    default;
	CommonFieldsGenerationInfoDisabled& operator=(CommonFieldsGenerationInfoDisabled&&) = default;
...
public:
	...
	explicit HashSetIteratorGenerationInfoEnabled(const GenerationType* generation_ptr)
...
template <typename = void> inline FindInfo find_first_non_full(const CommonFields& common, size_t hash) 
{
...
inline void ResetCtrl(CommonFields& common, size_t slot_size) {
	...
	memset(ctrl, static_cast<int8_t>(ctrl_t::kEmpty), capacity + 1 + NumClonedBytes());
	ctrl[capacity] = ctrl_t::kSentinel;
...
private:
	...
	ABSL_ATTRIBUTE_NOINLINE explicit raw_hash_set(size_t bucket_count, const hasher& hash = hasher(),
...
private:
	...
	template <class K = key_type>
	std::pair<const_iterator, const_iterator> equal_range(const key_arg<K>& key) const ABSL_ATTRIBUTE_LIFETIME_BOUND {
		auto it = find(key);
		if(it != end()) return {it, std::next(it)};
		return {it, it};
	}
	...
	friend typename std::enable_if<H::template is_hashable<value_type>::value,
	    H>::type AbslHashValue(H h, const raw_hash_set& s) {
