							    ...
							    void SetInlinedSize(SizeType<A> size) {
								    ...
								    void AddSize(SizeType<A> count) {
									    ...
									    void SubtractSize(SizeType<A> count) {
										    ...
										    void MemcpyFrom(const Storage& other_storage) { // Assumption check: it
												// doesn't make sense to memcpy inlined elements unless we know the allocator
												// doesn't do anything fancy, and one of the following holds:
												//
												//  *  The elements are trivially relocatable. 
												//  *  It's possible to trivially assign the elements and then destroy the source.
												//  *  It's possible to trivially copy construct/assign the elements.
											    {
												    ...
												    ABSL_HARDENING_ASSERT(
													    ...
													    ||
													    (std::is_same<A,
													    std::allocator<V> >::value &&
													    (
														    // First
														    // case
														    // above
														    absl::
														    is_trivially_relocatable<V>
														    ::value ||
														    // Second
														    // case
														    // above
														    (absl::
														    is_trivially_move_assignable<V>
														    ::value &&
														    absl::
														    ...
														    // above
														    (absl::
														    is_trivially_copy_constructible<V>
														    ::value ||
														    absl::
														    is_trivially_copy_assignable<V>
														    ::value))));
											    }

											    GetSizeAndIsAllocated() =
											    other_storage.GetSizeAndIsAllocated();
											    data_ = other_storage.data_;
										    }

										    void DeallocateIfAllocated() {
											    if(GetIsAllocated()) {
												    MallocAdapter<A>::Deallocate(
													    GetAllocator(),
													    GetAllocatedData(),
													    GetAllocatedCapacity());
											    }
...
private:
										    ...
										    using Metadata = container_internal::CompressedTuple<A,
										    SizeType<A> >;
										    ...
										    struct Allocated {
											    ...
											    SizeType<A> allocated_capacity;
                                                                                    ...
                                                                                    // `kOptimalInlinedSize` is an
                                                                                    // automatically adjusted inlined
                                                                                    // capacity of the
                                                                                    // `InlinedVector`. Sometimes, it is
                                                                                    // possible to increase the capacity
                                                                                    // (from
                                                                                    // the user requested `N`) without
										    ...
										    void SwapN(ElementwiseSwapPolicy, Storage* other,
										    SizeType<A> n);
										    void SwapN(ElementwiseConstructPolicy, Storage* other,
										    SizeType<A> n);

										    void SwapInlinedElements(MemcpyPolicy, Storage* other);
										    template <typename NotMemcpyPolicy>
										    void SwapInlinedElements(NotMemcpyPolicy,
										    Storage* other);

										    template <typename ... Args>
										    ABSL_ATTRIBUTE_NOINLINE Reference<A> EmplaceBackSlow(
											    Args&&... args);
										    ...
										    void Storage<T, N, A>::DestroyContents() {
											    Pointer<A> data =
											    GetIsAllocated() ? GetAllocatedData()
											    : GetInlinedData();
											    DestroyAdapter<A>::DestroyElements(GetAllocator(),
											    data, GetSize());
											    DeallocateIfAllocated();
										    }
										    ...
										    void Storage<T, N, A>::InitFrom(const Storage& other) {
											    ...
											    if(!other.GetIsAllocated()) {
												    ...
												    MallocAdapter<A>::Allocate(GetAllocator(),
												    requested_capacity);
												    ...
												    src = other.GetAllocatedData();
                                                                                            ...
                                                                                            // Fast path: if the value
                                                                                            // type is trivially copy
                                                                                            // constructible and we know
                                                                                            // the allocator doesn't do
                                                                                            // anything fancy, then we
                                                                                            // know it is legal for us
                                                                                            // to
                                                                                            // simply memcpy the other
                                                                                            // vector's elements.
											    if(absl::is_trivially_copy_constructible<ValueType<A> >
											    ::value &&
											    std::is_same<A,
											    std::allocator<ValueType<A> > >::
											    value) {
												    memcpy(reinterpret_cast<char*>(dst),
												    reinterpret_cast<const char*>(src),
												    n * sizeof(ValueType<A>));
											    }
											    else {
												    auto values = IteratorValueAdapter<A,
												    ConstPointer<A> >(src);
												    ConstructElements<A>(GetAllocator(),
												    dst, values, n);
											    }
										    ...
										    ->void {
											    ...
											    if(new_size > GetInlinedCapacity()) { // Because this is
                                                    // only called from the `InlinedVector` constructors, it's safe to take on
                                                    // the allocation with size `0`. If `ConstructElements(...)` throws,
                                                    // deallocation will be automatically handled by `~Storage()`.
												    SizeType<A> requested_capacity =
												    ComputeCapacity(GetInlinedCapacity(),
												    new_size);
												    Allocation<A> allocation =
												    MallocAdapter<A>::Allocate(GetAllocator(),
												    requested_capacity);
											    ...
											    ConstructElements<A>(GetAllocator(), construct_data, values, new_size);
													...
													// already correct for either case, *adding* `new_size` gives us the correct result faster than setting it directly.
											    AddSize(new_size);
										    }
										    ...
										    auto Storage<T, N, A>::Assign(ValueAdapter values, SizeType<A> new_size)->void {
											    ...
											    if(new_size > storage_view.capacity) {
												    SizeType<A> requested_capacity =
												    ComputeCapacity(storage_view.capacity,
												    new_size);
												    construct_loop =
												    {allocation_tx.Allocate(
													     requested_capacity),
												     new_size};
												    destroy_loop =
												    {storage_view.data,
											    ...
											    AssignElements<A>(assign_loop.data(), values,
											    assign_loop.size());

											    ConstructElements<A>(GetAllocator(),
											    construct_loop.data(), values,
											    construct_loop.size());
										    ...
										    ->void {
											    ...
											    if(new_size <= size) {
                                                                                                    // Destroy extra old
                                                                                                    // elements.
												    DestroyAdapter<A>::DestroyElements(alloc,
												    base + new_size,
												    size - new_size);
											    }
                                                                                                    ...
                                                                                                    // elements in
                                                                                                    // place.
												    ConstructElements<A>(alloc, base + size,
												    values,
												    new_size - size);
											    }
											    else {
                                                                                                    ...
                                                                                                    // due to
                                                                                                    // exceptions.
												    AllocationTransaction<A> allocation_tx(
													    alloc);
												    SizeType<A> requested_capacity =
												    ComputeCapacity(storage_view.capacity,
												    new_size);
												    ...
												    ConstructionTransaction<A>
												    construction_tx(alloc);
												    construction_tx.Construct(new_data +
												    size, values,
												    new_size - size);

												    IteratorValueAdapter<A,
												    MoveIterator<A> > move_values(
													    (MoveIterator<A>(base)));
												    ConstructElements<A>(alloc, new_data,
												    move_values, size);
										    ...
										    template <typename T, size_t N, typename A>
										    template <typename ValueAdapter>
										    auto Storage<T, N, A>::Insert(ConstIterator<A> pos,
										    ValueAdapter values,
										    SizeType<A> insert_count)->Iterator<A> {
											    StorageView<A> storage_view = MakeStorageView();

											    auto insert_index = static_cast<SizeType<A>>(
												    std::distance(ConstIterator<A>(
													    storage_view.data), pos));
											    SizeType<A> insert_end_index = insert_index +
											    insert_count;
											    SizeType<A> new_size = storage_view.size +
											    insert_count;
											    ...
											    if(new_size > storage_view.capacity) {
												    ...
												    move_construction_tx.Construct(new_data,
												    move_values,
												    insert_index);
												    ...
												    Pointer<A> move_assignment_values =
												    storage_view.data +
												    insert_index;
												    absl::Span<ValueType<A> >
												    move_assignment = {
												    ...
												    move_construction_tx.Construct(
													    move_construction.data(),
													    move_construction_values,
													    move_construction.size());

												    for(Pointer<A>
												    destination = move_assignment.data() +
												    move_assignment.size(),
												    last_destination =
												    move_assignment.data(),
												    source = move_assignment_values +
												    move_assignment.size();
												    ;) {
													    --destination;
													    --source;
													    if(destination <
													    last_destination) break;
													    * destination =
													    std::move(* source);
												    }

												    AssignElements<A>(insert_assignment.data(),
												    values,
												    insert_assignment.size());

												    ConstructElements<A>(GetAllocator(),
												    insert_construction.data(),
												    values,
												    ...
												    return Iterator<A>(storage_view.data +
												    insert_index);
											    }
											    }
											    ...
											    ... args)->Reference<A> {
												    ...
												    storage_view.capacity)) {
													    ...
													    return * last_ptr;
                                                                                                    ...
                                                                                                    // prevent
                                                                                                    // regression.
												    return EmplaceBackSlow(std::forward<Args>(
													    args)...);
											    }
											    ...
											    ... args)->Reference<A> {
												    ...
												    allocation_tx.Allocate(
													    requested_capacity);
												    Pointer<A> last_ptr = construct_data +
												    storage_view.size;
											    ...
											    ->Iterator<A> {
												    ...
												    AssignElements<A>(storage_view.data +
												    erase_index,
												    move_values,
												    ...
												    typename A>
												    auto Storage<T, N,
												    A>::Swap(Storage* other_storage_ptr)
												    ->void {
													    using std::swap;
													    ...
													    other_storage_ptr
													    ->GetIsAllocated()) {
														    swap(data_.allocated,
														    other_storage_ptr
														    ->data_.
														    allocated);
													    }
													    else if(!GetIsAllocated() &&
													    !other_storage_ptr
													    ->GetIsAllocated()) {
														    SwapInlinedElements(
												    ...
												    typename A>
												    void Storage<T, N,
												    A>::SwapN(ElementwiseSwapPolicy,
												    Storage* other,
												    SizeType<A> n) {
													    std::swap_ranges(GetInlinedData(),
													    GetInlinedData()
													    + n,
												    ...
												    typename A>
												    void Storage<T, N,
												    A>::SwapN(ElementwiseConstructPolicy,
												    Storage* other,
												    SizeType<A> n) {
												    ...
												    A>::SwapInlinedElements(
													    ...
													    ConstructElements<A>(large_ptr->
													    GetAllocator(),
													    small_ptr->GetInlinedData() +
													    small_size,
													    move_values,
													    diff);
