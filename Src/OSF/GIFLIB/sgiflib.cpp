...
#define READ(_gif, _buf, _len) (((GifFilePrivateType*)_gif->Private)->Read ? ((GifFilePrivateType*)_gif->Private)->Read(_gif, _buf, _len) : \
...
// 
static int FASTCALL DGifGetWord(GifFileType * GifFile, GifWord * Word)
{
	uint8 c[2];
			    ...
			    while(ExtData != NULL) {
				    ...
				    if(ExtData != NULL)
					    if(GifAddExtensionBlock(&GifFile->ExtensionBlockCount,
						    &GifFile->ExtensionBlocks, CONTINUE_EXT_FUNC_CODE, ExtData[0], &ExtData[1]) == GIF_ERROR)
						    return (GIF_ERROR);
			    }
			    break;
...
{
	GifFilePrivateType * p_private = (GifFilePrivateType*)GifFileOut->Private;
...
static int EGifBufferedOutput(GifFileType * GifFile, GifByteType * Buf, int c)
{
	if(c == FLUSH_OUTPUT) {
		...
		if(Buf[0] != 0 && InternalWrite(GifFile, Buf, Buf[0] + 1) != (unsigned)(Buf[0] + 1)) {
...
int EGifPutScreenDesc(GifFileType * GifFile, const int Width, const int Height, const int ColorRes, const int BackGround, const ColorMapObject * ColorMap)
{
	...
	if(InternalWrite(GifFile, (uint8*)write_version, strlen(write_version)) != strlen(write_version)) {
		GifFile->Error = E_GIF_ERR_WRITE_FAILED;
		return GIF_ERROR;
	}
...
int EGifGCBToSavedExtension(const GraphicsControlBlock * GCB, GifFileType * GifFile, int ImageIndex)
{
	...
	if(GifAddExtensionBlock(&GifFile->SavedImages[ImageIndex].ExtensionBlockCount,
		    &GifFile->SavedImages[ImageIndex].ExtensionBlocks, GRAPHICS_EXT_FUNC_CODE,
		    Len, (uint8*)buf) == GIF_ERROR)
		return (GIF_ERROR);
	return (GIF_OK);
}
...
int EGifPutCodeNext(GifFileType * GifFile, const GifByteType * CodeBlock)
{
	...
	GifFilePrivateType * Private = (GifFilePrivateType*)GifFile->Private;
	if(CodeBlock != NULL) {
		if(InternalWrite(GifFile, CodeBlock, CodeBlock[0] + 1) != (unsigned)(CodeBlock[0] + 1)) {
			GifFile->Error = E_GIF_ERR_WRITE_FAILED;
			return GIF_ERROR;
		}
	}
...
void GifDrawBox(SavedImage * Image, const int x, const int y, const int w, const int d, const int color)
{
...
void GifDrawRectangle(SavedImage * Image, const int x, const int y, const int w, const int d, const int color)
{
...
ColorMapObject * GifMakeMapObject(int ColorCount, const GifColorType * ColorMap)
{
	...
	Object = (ColorMapObject*)SAlloc::M(sizeof(ColorMapObject));
...
ColorMapObject * GifUnionColorMap(const ColorMapObject * ColorIn1, const ColorMapObject * ColorIn2, GifPixelType ColorTransIn2[])
{
	 ...
	 * 16 colors.
	 */
	while(ColorIn1->Colors[CrntSlot - 1].Red == 0 && ColorIn1->Colors[CrntSlot - 1].Green == 0 && ColorIn1->Colors[CrntSlot - 1].Blue == 0)
		CrntSlot--;
...
// 
int GifAddExtensionBlock(int * ExtensionBlockCount, ExtensionBlock ** ExtensionBlocks, int Function, uint Len, uint8 ExtData[])
{
	ExtensionBlock * ep;
	...
	ep->ByteCount = Len;
	ep->Bytes = (GifByteType*)SAlloc::M(ep->ByteCount);
	if(ep->Bytes == NULL)
		return (GIF_ERROR);
	if(ExtData != NULL) {
