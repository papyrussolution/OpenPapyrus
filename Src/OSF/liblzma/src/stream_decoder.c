...
struct lzma_stream_decoder_coder {
	...
	bool ignore_check; /// If true, we will tell the Block decoder to skip calculating and verifying the integrity check.
    ...
    size_t * out_pos, size_t out_size, lzma_action action)
{
	...
	while(true)
		switch(coder->sequence) {
			case lzma_stream_decoder_coder::SEQ_STREAM_HEADER: {
			    ...
			    coder->sequence = lzma_stream_decoder_coder::SEQ_BLOCK_HEADER;
			...
			case lzma_stream_decoder_coder::SEQ_BLOCK_HEADER: {
			    ...
			    if(coder->pos == 0) {
				    ...
				    if(in[*in_pos] == 0x00) {
					    coder->sequence = lzma_stream_decoder_coder::SEQ_INDEX;
					    break;
				    }
			...
			case lzma_stream_decoder_coder::SEQ_INDEX: {
			    // If we don't have any input, don't call
			    // lzma_index_hash_decode() since it would return
			    // LZMA_BUF_ERROR, which we must not do here.
			    if(*in_pos >= in_size)
				    return LZMA_OK;
			    // Decode the Index and compare it to the hash calculated from the sizes of the Blocks (if any).
...
{
	const lzma_stream_decoder_coder * coder = (const lzma_stream_decoder_coder *)coder_ptr;
