...
static size_t powerpc_code(void * simple lzma_attribute((__unused__)), uint32_t now_pos, bool is_encoder, uint8 * buffer, size_t size)
{
	size_t i;
	for(i = 0; i + 4 <= size; i += 4) {
		// PowerPC branch 6(48) 24(Offset) 1(Abs) 1(Link)
		if((buffer[i] >> 2) == 0x12 && ((buffer[i + 3] & 3) == 1)) {
			const uint32_t src = (((uint32_t)(buffer[i + 0]) & 3) << 24) | ((uint32_t)(buffer[i+1]) << 16) | ((uint32_t)(buffer[i+2]) << 8) | ((uint32_t)(buffer[i + 3]) & ~UINT32_C(3));
			uint32_t dest;
			if(is_encoder)
...
static lzma_ret powerpc_coder_init(lzma_next_coder * next, const lzma_allocator * allocator,
