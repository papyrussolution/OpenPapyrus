    ...
    const uint8 *in, size_t in_size, uint8 *out, size_t *out_pos_ptr, size_t out_size)
{
	...
	// Use a local copy. We update *out_pos_ptr only if everything succeeds.
	size_t out_pos = *out_pos_ptr;
	// Check that there's enough space for both Stream Header and Stream Footer.
	...
	if(lzma_stream_footer_encode(&stream_flags, out + out_pos) != LZMA_OK)
		return LZMA_PROG_ERROR;
	out_pos += LZMA_STREAM_HEADER_SIZE;
	// Everything went fine, make the new output position available to the application.
	*out_pos_ptr = out_pos;
	return LZMA_OK;
}
