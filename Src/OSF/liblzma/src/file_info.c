...
/// we aren't trying to seek too far backwards (too close or beyond the beginning of the file).
//
    ...
    lzma_action action lzma_attribute((__unused__)))
{
	...
	while(true)
		switch(coder->sequence) {
			case lzma_file_info_coder::SEQ_MAGIC_BYTES:
			...
			case lzma_file_info_coder::SEQ_PADDING_DECODE: {
			    ...
			    if(new_padding == coder->temp_size) {
				    ...
				    coder->sequence = lzma_file_info_coder::SEQ_PADDING_SEEK;
				    break;
			    }
			    ...
			    coder->sequence = lzma_file_info_coder::SEQ_FOOTER;
			...
			case lzma_file_info_coder::SEQ_FOOTER:
			    ...
			    if(coder->temp_size >= coder->footer_flags.backward_size) {
				    ...
				    // of the Index.
				    coder->temp_pos = static_cast<size_t>(coder->temp_size - coder->footer_flags.backward_size);
			    }
			    else {
			...
			case lzma_file_info_coder::SEQ_INDEX_DECODE: {
			    ...
			    if(coder->temp_size != 0) {
				    assert(coder->temp_size - coder->temp_pos == coder->index_remaining);
				    ret = decode_index(coder, allocator, coder->temp, &coder->temp_pos, coder->temp_size, false);
			    }
			    else {
				    ...
				    if(in_size - *in_pos > coder->index_remaining)
					    in_stop = *in_pos + (size_t)(coder->index_remaining);

				    ret = decode_index(coder, allocator, in, in_pos, in_stop, true);
			    }
			    ...
			    coder->sequence = lzma_file_info_coder::SEQ_HEADER_DECODE;
			    ...
			    assert(coder->temp_size == 0 || coder->temp_size >= coder->footer_flags.backward_size);
			    ...
			    if(coder->temp_size != 0 && coder->temp_size - coder->footer_flags.backward_size >= seek_amount) {
				    ...
				    // coder->temp_size - LZMA_STREAM_HEADER_SIZE].
				    coder->temp_pos = static_cast<size_t>(coder->temp_size - coder->footer_flags.backward_size - seek_amount + LZMA_STREAM_HEADER_SIZE);
			...
			case lzma_file_info_coder::SEQ_HEADER_COMPARE:
			    ...
			    if(coder->file_target_pos == 0) {
				    ...
				    assert(lzma_index_file_size(coder->combined_index) == coder->file_size);
			    ...
			    coder->sequence = coder->temp_size > 0 ? lzma_file_info_coder::SEQ_PADDING_DECODE : lzma_file_info_coder::SEQ_PADDING_SEEK;
			    break;
...
static lzma_ret file_info_decoder_memconfig(void * coder_ptr,  uint64 * memusage,  uint64 * old_memlimit,  uint64 new_memlimit)
{
	...
	else if(coder->sequence == lzma_file_info_coder::SEQ_INDEX_DECODE) {
    ...
    lzma_index ** dest_index,  uint64 memlimit,  uint64 file_size)
{
	...
	if(dest_index == NULL)
		return LZMA_PROG_ERROR;
	lzma_file_info_coder * coder = (lzma_file_info_coder *)next->coder;
	if(!coder) {
		coder = (lzma_file_info_coder *)lzma_alloc(sizeof(lzma_file_info_coder), allocator);
		if(!coder)
	...
	coder->sequence = lzma_file_info_coder::SEQ_MAGIC_BYTES;
