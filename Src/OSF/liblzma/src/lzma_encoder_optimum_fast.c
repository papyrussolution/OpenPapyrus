...
{
	const uint32 nice_len = mf->nice_len;
	uint32 len_main;
	uint32 matches_count;
	if(mf->read_ahead == 0) {
	...
	const uint8 * buf = mf_ptr(mf) - 1;
	const uint32 buf_avail = MIN(mf_avail(mf) + 1, MATCH_LEN_MAX);
	if(buf_avail < 2) {
	...
	for(uint32 i = 0; i < REPS; ++i) {
		...
		const uint32 len = lzma_memcmplen(buf, buf_back, 2, buf_avail);
	...
	++buf;
	const uint32 limit = MAX(2, len_main - 1);
	for(uint32 i = 0; i < REPS; ++i) {
