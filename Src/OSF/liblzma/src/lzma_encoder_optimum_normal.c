...
// Prices //
//
static uint32_t get_literal_price(const lzma_lzma1_encoder * const coder, const uint32_t pos, const uint32_t prev_byte, const bool match_mode,
...
static inline uint32_t get_short_rep_price(const lzma_lzma1_encoder * const coder,
    const lzma_lzma_state state, const uint32_t pos_state)
{
	return rc_bit_0_price(coder->is_rep0[state]) + rc_bit_0_price(coder->is_rep0_long[state][pos_state]);
}

static inline uint32_t get_pure_rep_price(const lzma_lzma1_encoder * const coder, const uint32_t rep_index,
...
static inline uint32_t get_rep_price(const lzma_lzma1_encoder * const coder, const uint32_t rep_index,
...
static inline uint32_t get_dist_len_price(const lzma_lzma1_encoder * const coder, const uint32_t dist,
    const uint32_t len, const uint32_t pos_state)
{
...
static void backward(lzma_lzma1_encoder * coder, uint32_t * len_res,
...
// Main //
//
static inline uint32_t helper1(lzma_lzma1_encoder * coder, lzma_mf * mf,
    uint32_t * back_res, uint32_t * len_res, uint32_t position)
{
...
static inline uint32_t helper2(lzma_lzma1_encoder * coder, uint32_t * reps, const uint8_t * buf,
    ...
    const uint32_t nice_len, const uint32_t buf_avail_full)
{
	...
	if(!next_is_literal && match_byte != current_byte) {  // speed optimization
		...
		if(len_test >= 2) {
			...
			const uint32_t cur_and_len_price = next_rep_match_price + get_rep_price(coder, 0, len_test, state_2, pos_state_next);
...
extern void lzma_lzma_optimum_normal(lzma_lzma1_encoder * coder,
    lzma_mf * mf, uint32_t * back_res, uint32_t * len_res, uint32_t position)
{
