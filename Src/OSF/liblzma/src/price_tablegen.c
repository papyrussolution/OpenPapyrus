...
{
	for(uint32_t i = (UINT32_C(1) << RC_MOVE_REDUCING_BITS) / 2; i < RC_BIT_MODEL_TOTAL; i += (UINT32_C(1) << RC_MOVE_REDUCING_BITS)) {
		...
		rc_prices[i >> RC_MOVE_REDUCING_BITS] = (RC_BIT_MODEL_TOTAL_BITS << cycles_bits) - 15 - bit_count;
	}
}
...
static void print_price_table(void)
{
	...
	const size_t array_size = sizeof(lzma_rc_prices) / sizeof(lzma_rc_prices[0]);
