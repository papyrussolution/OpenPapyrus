...
static size_t x86_code(void * simple_ptr, uint32_t now_pos, bool is_encoder, uint8_t * buffer, size_t size)
{
	...
	lzma_simple_x86 * simple = (lzma_simple_x86 *)simple_ptr;
...
static lzma_ret x86_coder_init(lzma_next_coder * next, const lzma_allocator * allocator,
    const lzma_filter_info * filters, bool is_encoder)
{
	const lzma_ret ret = lzma_simple_coder_init(next, allocator, filters, &x86_code, sizeof(lzma_simple_x86), 5, 1, is_encoder);
	if(ret == LZMA_OK) {
		lzma_simple_coder * coder = (lzma_simple_coder *)next->coder;
		lzma_simple_x86 * simple = (lzma_simple_x86 *)coder->simple;
		simple->prev_mask = 0;
