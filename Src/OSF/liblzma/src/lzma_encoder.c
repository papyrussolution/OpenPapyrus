...
static inline void length(lzma_range_encoder * rc, lzma_length_encoder * lc, const uint32_t pos_state, uint32_t len, const bool fast_mode)
{
	...
	// Only getoptimum uses the prices so don't update the table when in fast mode.
	if(!fast_mode)
...
static bool is_options_valid(const lzma_options_lzma * options)
{
	...
	return is_lclppb_valid(options) && options->nice_len >= MATCH_LEN_MIN && options->nice_len <= MATCH_LEN_MAX && (options->mode == LZMA_MODE_FAST || options->mode == LZMA_MODE_NORMAL);
}

static void set_lz_options(lzma_lz_encoder_options * lz_options, const lzma_options_lzma * options)
{
...
extern lzma_ret lzma_lzma_encoder_create(void ** coder_ptr, const lzma_allocator * allocator,
    const lzma_options_lzma * options, lzma_lz_encoder_options * lz_options)
{
	...
	lzma_lzma1_encoder * coder = (lzma_lzma1_encoder *)(*coder_ptr);
	...
	switch(options->mode) {
		...
		case LZMA_MODE_NORMAL: {
		    ...
		    coder->match_len_encoder.table_size = options->nice_len + 1 - MATCH_LEN_MIN;
		    coder->rep_len_encoder.table_size = options->nice_len + 1 - MATCH_LEN_MIN;
		    break;
	    }
		default:
		    return LZMA_OPTIONS_ERROR;
...
static lzma_ret lzma_encoder_init(lzma_lz_encoder * lz, const lzma_allocator * allocator, const void * options, lzma_lz_encoder_options * lz_options)
{
	lz->code = &lzma_encode;
	return lzma_lzma_encoder_create(&lz->coder, allocator, (const lzma_options_lzma *)options, lz_options);
}
