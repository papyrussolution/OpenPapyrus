...
struct worker_thread_s {
	...
	mythread_cond cond;
	mythread thread_id; /// The ID of this thread is used to join the thread when it's not needed anymore.
};
...
struct lzma_stream_coder_s {
	...
	size_t block_size; /// Start a new Block every block_size bytes of input unless LZMA_FULL_FLUSH or LZMA_FULL_BARRIER is used earlier.
	...
	lzma_ret thread_error; /// Error code from a worker thread
	worker_thread * threads; /// Array of allocated thread-specific structures
	uint32_t threads_max; /// Number of structures in "threads" above. This is also the number of threads that will be created at maximum.
	uint32_t threads_initialized; /// Number of thread structures that have been initialized, and thus the number of worker threads actually created so far.
...
static worker_state worker_encode(worker_thread * thr, worker_state state)
{
	...
	/* @sobolev thr->block_options = (lzma_block) {
		.version = 0,
		.check = thr->coder->stream_flags.check,
		.compressed_size = thr->coder->outq.buf_size_max,
		.uncompressed_size = thr->coder->block_size,
		// TODO: To allow changing the filter chain, the filters array must be copied to each worker_thread.
		.filters = thr->coder->filters,
	};*/
	// @sobolev {
	memzero(&thr->block_options, sizeof(thr->block_options));
...
{
	worker_thread * thr = (worker_thread *)thr_ptr;
	...
	for(uint32_t i = 0; i < coder->threads_initialized; ++i) {
		mythread_sync(coder->threads[i].mutex) {
			while(coder->threads[i].state != THR_IDLE)
				mythread_cond_wait(&coder->threads[i].cond, &coder->threads[i].mutex);
		}
	}
}
...
static lzma_ret initialize_new_thread(lzma_stream_coder * coder, const lzma_allocator * allocator)
{
	...
	thr->progress_in = 0;
	thr->progress_out = 0;
	thr->block_encoder.SetDefault();// = LZMA_NEXT_CODER_INIT;
    ...
    size_t * out_pos, size_t out_size, lzma_action action)
{
	...
	switch(coder->sequence) {
		case lzma_stream_coder_s::SEQ_STREAM_HEADER:
		...
		case lzma_stream_coder_s::SEQ_BLOCK: {
		    ...
		    return_if_error(lzma_index_encoder_init(
				...
				coder->index));
		    coder->sequence = lzma_stream_coder_s::SEQ_INDEX;
...
//              next->update = &stream_encoder_mt_update;
	...
	coder->sequence = lzma_stream_coder_s::SEQ_STREAM_HEADER;
	...
	if(coder->threads_max != options->threads) {
		...
		coder->threads = (worker_thread *)lzma_alloc(options->threads * sizeof(worker_thread), allocator);
