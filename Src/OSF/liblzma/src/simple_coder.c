    ...
    size_t in_size, uint8_t * out, size_t * out_pos, size_t out_size, lzma_action action)
{
	...
	if(coder->size > 0) {
		{
			const lzma_ret ret = copy_or_code(coder, allocator, in, in_pos, in_size, coder->buffer, &coder->size, coder->allocated, action);
			assert(ret != LZMA_STREAM_END);
			if(ret != LZMA_OK)
				return ret;
		}
...
static lzma_ret simple_coder_update(void * coder_ptr, const lzma_allocator * allocator,
    ...
    size_t simple_size, size_t unfiltered_max, uint32_t alignment, bool is_encoder)
{
	...
	if(filters[0].options != NULL) {
		const lzma_options_bcj * simple = (const lzma_options_bcj *)(filters[0].options);
...
extern lzma_ret lzma_simple_props_decode(void ** options, const lzma_allocator * allocator, const uint8_t * props, size_t props_size)
{
	...
	if(props_size != 4)
		return LZMA_OPTIONS_ERROR;
	lzma_options_bcj * opt = (lzma_options_bcj *)lzma_alloc(sizeof(lzma_options_bcj), allocator);
	if(opt == NULL)
		return LZMA_MEM_ERROR;
