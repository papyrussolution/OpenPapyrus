...
#define checkPureFunction(code) checkPureFunction_(code, __FILE__, __LINE__)
	...
	void incorrectLogicOperator2() {
		...
		check("void f(int x) {\n"
		    ...
		    "        a++;\n"
		    "}");
		ASSERT_EQUALS("", errout.str()); // float comparisons with == and != are not checked right now - such
	...
	void oppositeInnerConditionEmpty() {
		...
		check("void f1(const std::string &s) { if(s.size() < 0) if(s.empty()) {}} "); // <- CheckOther reports:
		...
		ASSERT_EQUALS("", errout.str()); //We don't know the type of T so we don't know the relationship between
		                                 // size() and empty(). e.g. s might be a 50 tonne truck with nothing in
		                                 // it.
		...
		check("void f1(const std::string &s) { if(s.size() >= 0) if(s.empty()) {}} "); // CheckOther says:
		                                                                               // Unsigned expression
		                                                                               // 's.size()' can't be
		                                                                               // negative so it is
		                                                                               // unnecessary to test
	...
	void identicalConditionAfterEarlyExit() {
		...
		check("void f(int x) {\n" // #8217 - crash for incomplete code
		    ...
		    "  if (x > 100) {}\n"
		    "}");
		// TODO: we should probably throw unknownMacro InternalError. Complain that the macro X must be defined.
		// We can't check the code well without the definition.
	...
	void clarifyCondition7() {
		...
		check("void f(bool error) {\n"
		    "    bool & withoutSideEffects=found.first->second;\n" // Declaring a reference to a boolean; & is
		                                                           // no operator at all
...
#define MSG(EXPR, \
	    RESULT)   "[test.cpp:1]: (warning) Invalid test for overflow '" EXPR \
	"'; signed integer overflow is undefined behavior. Some mainstream compilers remove such overflow tests when optimising the code and assume it's always " \
	RESULT ".\n"
...
#define MSG(EXPR, \
	    RESULT)   "[test.cpp:1]: (warning) Invalid test for overflow '" EXPR \
	"'; signed integer overflow is undefined behavior. Some mainstream compilers removes handling of overflows when optimising the code and change the code to '" \
	RESULT "'.\n"
