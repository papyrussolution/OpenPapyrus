...
#define check(...) check_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void run() override {
		...
		TEST_CASE(array_index_same_struct_and_var_name); // #4751 - not handled well when struct name and var
		                                                 // name is same
	...
	/*
	    void countSprintfLength() const {
	        ...
	        ASSERT_EQUALS(10, CheckBufferOverrun::countSprintfLength("\\\\\\\\Hello%d \\0Text\\\\\\\\",
	           unknownParameter));
	...
	void arrayIndexThenCheck() {
		...
		check("void f(const char s[]) {\n"
		    ...
		    "    }"
		    "}");
		ASSERT_EQUALS("", errout.str()); // No message because i is unknown and thus gets no varid. Avoid an
		                                 // internalError here.
	...
	void negativeArraySize() {
		...
		// don't warn for constant sizes -> this is a compiler error so this is used for static assertions for
		// instance
		check("int x, y;\n"
		    "int a[-1];\n"
		    "int b[x?1:-1];\n"
