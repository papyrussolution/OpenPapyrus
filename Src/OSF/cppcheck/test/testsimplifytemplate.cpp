...
private:
	...
	void run() override {
		...
		TEST_CASE(template_specialization_1); // #7868 - template specialization template <typename T> struct
		                                      // S<C<T>> {..};
		TEST_CASE(template_specialization_2); // #7868 - template specialization template <typename T> struct
		                                      // S<C<T>> {..};
		...
		TEST_CASE(templateParameterWithoutName); // #8602 Template default parameter without name yields syntax
		                                         // error
...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void template43() { // #5097 - Assert due to '>>' in 'B<A<C>>' not being treated as end of template
		            // instantiation
	...
	void template_default_type() {
		...
		//ASSERT_EQUALS("[file1.cpp:15]: (error) Internal error: failed to instantiate template. The checking
		// continues anyway.\n", errout.str());
		ASSERT_EQUALS("", errout.str());
	}
	...
	void templateNamePosition() {
		...
		ASSERT_EQUALS(4,
		    templateNamePositionHelper("template<class T> std::string foo() { static str::string str; return str; }", 4));
		ASSERT_EQUALS(5,
		    templateNamePositionHelper("template<class T> std::string & foo() { static str::string str; return str; }", 4));
		ASSERT_EQUALS(6,
		    templateNamePositionHelper("template<class T> const std::string & foo() { static str::string str; return str; }", 4));

		ASSERT_EQUALS(9,
		    templateNamePositionHelper("template<class T> std::map<int, int> foo() { static std::map<int, int> m; return m; }", 4));
		ASSERT_EQUALS(10,
		    templateNamePositionHelper("template<class T> std::map<int, int> & foo() { static std::map<int, int> m; return m; }",
		    4));
		ASSERT_EQUALS(11,
		    templateNamePositionHelper(
			    "template<class T> const std::map<int, int> & foo() { static std::map<int, int> m; return m; }",
			    4));
	...
	void expandSpecialized1() {
		...
		ASSERT_EQUALS(
			...
			tok(
				"template<> class A<int> { A(); A(const A &) ; A foo(); }; A<int>::A() { } A<int>::A(const A &) { } A<int> A<int>::foo() { A a; return a; }"));
	}
...
#define instantiateMatch(code, numberOfArguments, patternAfter) instantiateMatch_(code, numberOfArguments, patternAfter, __FILE__, __LINE__)
	bool instantiateMatch_(const char code[], const std::size_t numberOfArguments, const char patternAfter[], const char* file,
	    int line) {
		Tokenizer tokenizer(&settings, this);
