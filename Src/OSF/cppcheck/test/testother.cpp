...
#define checkInvalidPointerCast(...) checkInvalidPointerCast_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void passedByValue_nonConst() {
		...
		check("class X {\n"
		    "    virtual void func(std::string str) {}\n" // Do not warn about virtual functions, if 'str' is
	...
	void constVariable() {
		...
		check("struct T : public U { void dostuff() const {}};\n"
		    ...
		    "    y->mutate();\n" //to avoid warnings that y can be const
		    "}");
		TODO_ASSERT_EQUALS("can be const", errout.str(), ""); //Currently taking the address is treated as a
		                                                      // non-const operation when it should depend on
		                                                      // what we do with it
		check("struct T : public U { void dostuff() const {}};\n"
		    "void a(T& x) {\n"
		    "    x.dostuff();\n"
	...
	void constParameterCallback() {
		...
		ASSERT_EQUALS(
			"[test.cpp:2] -> [test.cpp:1]: (style) Parameter 'x' can be declared as reference to const. However it seems that 'callback' is a callback function, if 'x' is declared with const you might also need to cast function pointer(s).\n",
			errout.str());
		...
		check("class EventEngine : public IEventEngine {\n"
		    ...
		    "}");
		ASSERT_EQUALS(
			"[test.cpp:10] -> [test.cpp:13]: (style) Parameter 'signal' can be declared as reference to const. However it seems that 'signalEvent' is a callback function, if 'signal' is declared with const you might also need to cast function pointer(s).\n",
			errout.str());
	}
	...
	void duplicateBranch() {
		...
		check("int f(int signed, unsigned char value) {\n"
		    "    int ret;\n"
		    "    if (signed)\n"
		    "        ret = (signed char)value;\n" // cast must be kept so the simplifications and verification
		                                          // is skipped
		    "    else\n"
	...
	void incompleteArrayFill() {
		check("void f() {\n"
		    ...
		    "    memmove(a, b, 5);\n"
		    "}");
		ASSERT_EQUALS(// TODO "[test.cpp:4] -> [test.cpp:5]: (performance) Buffer 'a' is being written before
		              // its old content has been used.\n"
	...
	void redundantVarAssignment() {
		...
		check("void f() {\n"
		    "    static int i;\n"
		    "    i = 1;\n"
		    "    bar();\n" // bar() might call f() recursively. This could be a false positive in more complex
		                   // examples (when value of i is used somewhere. See #4229)
		    "    i = 2;\n"
		    "}");
		ASSERT_EQUALS("", errout.str());
	...
	void redundantVarAssignment_7133() {
		...
		check("class C { void operator=(int x); };\n" // #8368 - assignment operator might have side effects =>
		                                              // inconclusive
		    "void f() {\n"
		    "    C c;\n"
		    "    c = x;\n"
