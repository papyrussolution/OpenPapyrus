...
#define getSyntaxError(code) getSyntaxError_(code, __FILE__, __LINE__)
	...
	void garbageCode65() { // #6741
		ASSERT_THROW(checkCode("{ } { } typedef int u_array[]; typedef u_array &u_array_ref; (u_array_ref arg) { } u_array_ref"),
		    InternalError);
	}
	...
	void garbageCode78() { // #6756
		ASSERT_THROW(checkCode("( ) { [ ] } ( ) { } const_array_of_int ( ) { } typedef int A [ ] [ ] ; A a = { { } { } }"),
		    InternalError);
	}
	...
	void garbageCode84() { // #6780
		ASSERT_THROW(checkCode("int main ( [ ] ) { " " [ ] ; int i = 0 ; do { } ; } ( [ ] ) { }"), InternalError); // do
		                                                                                                           // not
		                                                                                                           // crash
	}

	void garbageCode85() { // #6784
		ASSERT_THROW(checkCode("{ } { } typedef void ( *VoidFunc() ) ( ) ; VoidFunc"), InternalError); // do not
		                                                                                               // crash
	}
	...
	void garbageCode88() { // #6786
		ASSERT_THROW(checkCode("( ) { ( 0 ) { ( ) } } g ( ) { i( ( false ?) ( ) : 1 ) ; } ;"), InternalError); // do
		                                                                                                       // not
		                                                                                                       // crash
	}

	void garbageCode90() { // #6790
		ASSERT_THROW(checkCode("{ } { } typedef int u_array [[ ] ; typedef u_array & u_array_ref] ( ) { } u_array_ref_gbl_obj0"),
		    InternalError);                                                                                                               // do
		                                                                                                                                  // not
		                                                                                                                                  // crash
	}
	...
	void garbageCode91() { // #6791
		ASSERT_THROW(checkCode("typedef __attribute__((vector_size (16))) { return[ (v2df){ } ;] }"), InternalError); // throw
		                                                                                                              // syntax
		                                                                                                              // error
	}

	void garbageCode92() { // #6792
		ASSERT_THROW(checkCode("template < typename _Tp ( ( ) ; _Tp ) , decltype > { } { ( ) ( ) }"), InternalError); // do
		                                                                                                              // not
		                                                                                                              // crash
	}
	...
	void garbageCode96() { // #6807
		ASSERT_THROW(checkCode("typedef J J[ ; typedef ( ) ( ) { ; } typedef J J ;] ( ) ( J cx ) { n } ;"), InternalError); // throw
		                                                                                                                    // syntax
		                                                                                                                    // error
	}
	...
	void garbageCode101() { // #6835
		// Reported case
		ASSERT_THROW(checkCode("template < class , =( , int) X = 1 > struct A { } ( ) { = } [ { } ] ( ) { A < void > 0 }"),
		    InternalError);
		// Reduced case
	...
	void garbageCode108() { //  #6895 "segmentation fault (invalid code) in CheckCondition::isOppositeCond"
		ASSERT_THROW(checkCode("A( ) { } bool f( ) { ( ) F; ( ) { ( == ) if ( !=< || ( !A( ) && r[2] ) ) ( !A( ) ) ( ) } }"),
		    InternalError);
	}
	...
	void syntaxErrorFuzzerCliType1() {
		...
		ASSERT_THROW(checkCode("void f() { x= y{ } name5 y[ ] + y ^ name5 ^ name5 for ( ( y y y && y y y && name5 ++ int )); }"),
		    InternalError);                                                                                                               // #9069
	}
