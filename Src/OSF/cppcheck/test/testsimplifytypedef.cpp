...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
	std::string tok_(const char* file,
	    int line,
	    const char code[],
	    bool simplify = true,
...
#define checkSimplifyTypedef(code) checkSimplifyTypedef_(code, __FILE__, __LINE__)
	...
	void simplifyTypedef14() {
		...
		// Tokenize and check output..
		TODO_ASSERT_THROW(tok(code, true, Settings::Native, false), InternalError); // TODO: Do not throw
		                                                                            // exception
		//ASSERT_EQUALS("", errout.str());
	}
	...
	void simplifyTypedef129() {
...

		{
			...
			const char exp[] =
			    "class c { char _a [ 4 ] ; const constexpr char ( & c_str ( ) const noexcept ) [ 4 ] { return _a ; } } ;";
			ASSERT_EQUALS(exp, tok(code, false));
		}

		{
			...
			const char actual[] =
			    "class c { char _a [ 4 ] ; constexpr operatorchar ( & ( ) const noexcept ) [ 4 ] { return _a ; } } ;";
			const char exp[] =
			    "class c { char _a [ 4 ] ; const operator char ( & ( ) const noexcept ) [ 4 ] { return _a ; } } ;";
...

		{
			...
			const char actual[] =
			    "class c { char _a [ 4 ] ; constexpr operatorconstchar ( & ( ) const noexcept ) [ 4 ] { return _a ; } } ;";
			const char exp[] =
			    "class c { char _a [ 4 ] ; const operator const char ( & ( ) const noexcept ) [ 4 ] { return _a ; } } ;";
			TODO_ASSERT_EQUALS(exp, actual, tok(code, false));
		}
	}
	...
	void simplifyTypedef137() { // #10054 debug: Executable scope 'x' with unknown function.
		...
		{
			// using "namespace external::ns1;" without redundant qualification on declaration and
			// definition
			const char code[] = "namespace external {\n"
			    "    namespace ns1 {\n"
			    "        template <int size> struct B { };\n"
	...
	void simplifyTypedefMacro() {
		const char code[] = "typedef uint32_t index_t;\n"
		    ...
		    "    if(prev_segment==NO_SEGMENT) {}\n"         // <- test that index_t is replaced with uint32_t in
		                                                    // the expanded tokens
		    "}";
