...
private:
	...
	void run() override {
		...
		TEST_CASE(virtualDestructor6); // only report error if base class pointer that points at derived class
		                               // is deleted
...
#define checkCopyCtorAndEqOperator(code) checkCopyCtorAndEqOperator_(code, __FILE__, __LINE__)
	...
	void copyCtorAndEqOperator() {
		...
		TODO_ASSERT_EQUALS("[test.cpp:1]: (warning) The class 'A' has 'copy constructor' but lack of 'operator='.\n",
		    "",
		    errout.str());
		// TODO the error message should be clarified. It should say something like 'copy constructor is empty
		// and will not assign i and therefore the behaviour is different to the default assignment operator'
		...
		TODO_ASSERT_EQUALS("[test.cpp:1]: (warning) The class 'A' has 'operator=' but lack of 'copy constructor'.\n",
		    "",
		    errout.str());
		// TODO the error message should be clarified. It should say something like 'assignment operator does
		// not assign i and therefore the behaviour is different to the default copy constructor'
...
#define checkOpertorEqToSelf(code) checkOpertorEqToSelf_(code, __FILE__, __LINE__)
	...
	void operatorEqToSelf3() {
		// this test has multiple inheritance so there is no trivial way to test for self assignment but doesn't
		// need it
		checkOpertorEqToSelf(
		...
		// this test has multiple inheritance and needs an assignment test but there is no trivial way to test
		// for it
		checkOpertorEqToSelf(
			"class A : public B, public C\n"
			"{\n"
...
#define checkVirtualDestructor(...) checkVirtualDestructor_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void virtualDestructor4() {
		// Derived class doesn't have a destructor => undefined behaviour according to paragraph 3 in
		// [expr.delete]
...
#define checkConst(...) checkConst_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void const1() {
		...
		// functions with a call to a member function can only be const, if that member function is const, too..
		// (#1305)
	...
	void const61() { // ticket #5606 - don't crash
		...
		// Based on an example from SVN source code causing an endless recursion within
		// CheckClass::isConstMemberFunc()
...
#define checkInitializationListUsage(code) checkInitializationListUsage_(code, __FILE__, __LINE__)
	...
	void initializerListUsage() {
		...
		checkInitializationListUsage("class Fred {\n"
		    "    std::string& s;\n"                  // Message is invalid for references, since their
		                                             // initialization in initializer list is required anyway
		                                             // and behaves different from assignment (#5004)
		...
		checkInitializationListUsage("class C;\n"
		    "class Fred {\n"
		    "    C a; int b;\n"
		    "    Fred() : b(5) { a = b; }\n"                  // Don't issue a message here: You actually could
		                                                      // move it to the initialization list, but it
		                                                      // would cause problems if you change the order of
