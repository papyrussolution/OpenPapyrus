...
private:
	...
	void preprocess(const char* code, std::map<std::string, std::string>& actual, const char filename[] = "file.c") {
		...
		for(const std::string & config : configs) {
			try {
				const std::string &cfgcode = preprocessor0.getcode(tokens,
					config,
					files,
					std::string(code).find("#file") != std::string::npos);
	...
	void ticket_3699() {
		...
		// First, it must not hang. Second, inline must becomes inline, and __forceinline must become
		// __forceinline.
	...
	void macro_simple17() const { // # 5074 - the Token::isExpandedMacro() doesn't always indicate properly if token
		                      // comes from macro
