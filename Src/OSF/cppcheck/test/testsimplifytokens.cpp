...
private:
	...
	void run() override {
		...
		TEST_CASE(simplifyKnownVariables35); // ticket #2353 - False positive: Division by zero 'if (x == 0)
		                                     // return 0; return 10 / x;'
		...
		TEST_CASE(simplifyKnownVariablesClassMember); // #2815 - value of class member may be changed by
		                                              // function call
...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
	std::string tok_(const char* file, int line, const char code[], bool simplify = true,
...
#define tokWithWindows(...) tokWithWindows_(__FILE__, __LINE__, __VA_ARGS__)
	std::string tokWithWindows_(const char* file,
	    int line,
	    const char code[],
	    bool simplify = true,
	    Settings::PlatformType type = Settings::Native) {
...
#define tokenizeDebugListing(...) tokenizeDebugListing_(__FILE__, __LINE__, __VA_ARGS__)
	std::string tokenizeDebugListing_(const char* file,
	    int line,
	    const char code[],
	    bool simplify = false,
	    const char filename[] = "test.cpp") {
	...
	void sizeof_array() {
		...
		code = "char(*Helper())[1];\n"
		    "sizeof(*Helper());\n";
		TODO_ASSERT_EQUALS("char ( * Helper ( ) ) [ 1 ] ; 1 ;", "char ( * Helper ( ) ) [ 1 ] ; sizeof ( * Helper ( ) ) ;",
		    tok(code));
	}
...
#define simplifyIfAndWhileAssign(code) simplifyIfAndWhileAssign_(code, __FILE__, __LINE__)
	...
	void remove_comma() {
		...
		{
			ASSERT_EQUALS("{ return a ? ( b = c , d ) : e ; }", tok("{ return a ? b = c , d : e ; }")); // Keep
			                                                                                            // comma
		}
	...
	void calculations() {
		...
		ASSERT_EQUALS("x ( 0 & 4 != 1 ) ;", tok("x(4%1<<1&4!=1);")); // #4931 (can be simplified further but
		                                                             // it's not a problem)
	...
	void flowControl() {
		{
			...
			ASSERT_EQUALS("void f ( ) { exit ( 0 ) ; }",
			    tokWithStdLib("void f() { exit(0); dead(); switch (n) { case 1: deadcode () ; default: deadcode (); } }"));
	...
	void simplifyAttribute() {
		...
		ASSERT_EQUALS("int f ( ) ;",
		    tok("__attribute__ ((visibility(\"default\"))) __attribute__ ((warn_unused_result)) int f();", true));
		ASSERT_EQUALS("blah :: blah f ( ) ;", tok("__attribute__ ((visibility(\"default\"))) blah::blah f();", true));
		ASSERT_EQUALS("template < T > Result < T > f ( ) ;",
		    tok("template<T> __attribute__ ((warn_unused_result)) Result<T> f();", true));
		ASSERT_EQUALS("template < T , U > Result < T , U > f ( ) ;",
		    tok("template<T, U> __attribute__ ((warn_unused_result)) Result<T, U> f();", true));
	}
...
#define simplifyKnownVariables(code) simplifyKnownVariables_(code, __FILE__, __LINE__)
	...
	void simplifyKnownVariables55() { // pointer alias
		ASSERT_EQUALS("void f ( ) { int a ; int * p ; if ( a > 0 ) { } }",
		...
		ASSERT_EQUALS("void f ( ) { int a ; int * p ; if ( x > a ) { } }",
