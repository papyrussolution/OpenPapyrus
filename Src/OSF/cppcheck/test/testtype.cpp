...
#define check(...) check_(__FILE__, __LINE__, __VA_ARGS__)
	void check_(const char* file,
	    ...
	    const std::string& standard = "c++11") {
	...
	void checkTooBigShift_Unix32() {
		...
		// and unsigned types having already a size of 4 bytes
		{
			...
			{"unsigned char", /*[unsigned]*/ "char", "bool", "unsigned short", "unsigned int", "unsigned long"};
		...
		{
			const std::string types[] =
			{"signed char", "signed short", /*[signed]*/ "short", "wchar_t", /*[signed]*/ "int", "signed int",
		...
		// 64 bit width types
		{
			...
			ASSERT_EQUALS("[test.cpp:1]: (error) Shifting 64-bit value by 64 bits is undefined behaviour\n", errout.str());
			check("signed long long f(long long x) { return (x = (signed long long)x << 64); }", &settings, "test.cpp",
			    "c++14");
			ASSERT_EQUALS("[test.cpp:1]: (error) Shifting 64-bit value by 64 bits is undefined behaviour\n", errout.str());
