...
#define check(...) check_(__FILE__, __LINE__, __VA_ARGS__)
	...
	void invalidFunctionUsageStrings() {
		...
		check("FILE* f(void) {\n"
		    "  const char fileName[] = { \'0\' + 42 };\n" // no size is explicitly defined, no implicit '\0' is
		                                                  // added
		    "  return fopen(fileName, \"r\"); \n"
		    "}");
		ASSERT_EQUALS("[test.cpp:3]: (error) Invalid fopen() argument nr 1. A nul-terminated string is required.\n", errout.str());
		...
		check("FILE* f(void) {\n"
		    "  const char fileName[] = { \'0\' + 42, \'x\' };\n" // no size is explicitly defined, no implicit
		                                                         // '\0' is added
		    "  return fopen(fileName, \"r\"); \n"
		    "}");
		ASSERT_EQUALS("[test.cpp:3]: (error) Invalid fopen() argument nr 1. A nul-terminated string is required.\n", errout.str());
		...
		check("FILE* f(void) {\n"
		    "  const char fileName[2] = { \'0\' + 42 };\n" // implicitly '\0' added at the end because size is
		                                                   // set to 2
		    "  return fopen(fileName, \"r\"); \n"
		    "}");
		ASSERT_EQUALS("", errout.str());
	...
	void checkIgnoredReturnValue() {
		...
		check("bool mystrcmp(char* a, char* b);\n" // cppcheck sees a custom strcmp definition, but it returns a
		                                           // value. Assume it is the one specified in the library.
		    "void foo() {\n"
		    "    mystrcmp(a, b);\n"
		    "}", "test.cpp", &settings2);
		ASSERT_EQUALS("[test.cpp:3]: (warning) Return value of function mystrcmp() is not used.\n", errout.str());

		check("void mystrcmp(char* a, char* b);\n" // cppcheck sees a custom strcmp definition which returns
		                                           // void!
		    "void foo() {\n"
		    "    mystrcmp(a, b);\n"
		    "}", "test.cpp", &settings2);
