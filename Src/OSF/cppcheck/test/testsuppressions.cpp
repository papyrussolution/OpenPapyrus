...
private:
	...
	void inlinesuppress() {
		...
		ASSERT_EQUALS(
			"Bad suppression attribute 'some'. You can write comments in the comment after a ; or //. Valid suppression attributes; symbolName=sym",
			msg);
	}
	...
	void multi_inlinesuppress_comment() {
		...
		errMsg = "";
		suppressions = Suppressions::parseMultiSuppressComment("//cppcheck-suppress[errorId1, errorId2 symbolName=arr] some text",
			&errMsg);
		ASSERT_EQUALS(2, suppressions.size());
		ASSERT_EQUALS(true, errMsg.empty());
	...
	void globalSuppressions() { // Testing that Cppcheck::useGlobalSuppressions works (#8515)
		...
		CppCheck cppCheck(*this, false, nullptr); // <- do not "use global suppressions". pretend this is a
		                                          // thread that just checks a file.
		Settings& settings = cppCheck.settings();
		...
		ASSERT_EQUALS("[test.c:1]: (error) Uninitialized variable: a\n", errout.str()); // <- report error so
		                                                                                // ThreadExecutor can
		                                                                                // suppress it and make
		                                                                                // sure the global
		                                                                                // suppression is
		                                                                                // matched.
	}

	void inlinesuppress_unusedFunction() const { // #4210, #4946 - wrong report of "unmatchedSuppression" for
		                                     // "unusedFunction"
		Suppressions suppressions;
	...
	void globalsuppress_unusedFunction() const { // #4946 - wrong report of "unmatchedSuppression" for
		                                     // "unusedFunction"
		Suppressions suppressions;
	...
	void suppressingSyntaxErrorAndExitCode() {
		...
		file3["test.cpp"] = "void f(int x, int y){\n"
		    ...
		    "}\n"
		    "f(0, 1);\n";
		ASSERT_EQUALS(1, checkSuppression(file3, "zerodiv:test.cpp:3")); // suppress 'errordiv' at line 3 of
		                                                                 // test.cpp
	}
};
