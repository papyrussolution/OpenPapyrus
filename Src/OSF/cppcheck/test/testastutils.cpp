...
#define isVariableChangedByFunctionCall(code, pattern, inconclusive) isVariableChangedByFunctionCall_(code, \
	    pattern, \
	    inconclusive, \
	    __FILE__, \
	    __LINE__)
...
#define nextAfterAstRightmostLeaf(code, parentPattern, rightPattern) nextAfterAstRightmostLeaf_(code, \
	    parentPattern, \
	    rightPattern, \
	    __FILE__, \
	    __LINE__)
	bool nextAfterAstRightmostLeaf_(const char code[], const char parentPattern[], const char rightPattern[], const char* file,
	    int line) {
		Settings settings;
		Tokenizer tokenizer(&settings, this);
		std::istringstream istr(code);
