...
#define getErrorPathForX(code, linenr) getErrorPathForX_(code, linenr, __FILE__, __LINE__)
	...
	bool testValueOfX_(const char* file,
	    int line,
	    const char code[],
	    unsigned int linenr,
	    const char value[],
	    ValueFlow::Value::ValueType type) {
		// Tokenize..
		Tokenizer tokenizer(&settings, this);
		std::istringstream istr(code);
		ASSERT_LOC(tokenizer.tokenize(istr, "test.cpp"), file, line);
...
#define testLifetimeOfX(...) testLifetimeOfX_(__FILE__, __LINE__, __VA_ARGS__)
	bool testLifetimeOfX_(const char* file,
	    int line,
	    const char code[],
	    unsigned int linenr,
	    const char value[],
	    ValueFlow::Value::LifetimeScope lifetimeScope = ValueFlow::Value::LifetimeScope::Local) {
		// Tokenize..
		Tokenizer tokenizer(&settings, this);
		std::istringstream istr(code);
		ASSERT_LOC(tokenizer.tokenize(istr, "test.cpp"), file, line);
		...
		for(const Token * tok = tokenizer.tokens(); tok; tok = tok->next()) {
			if(tok->str() == "x" && tok->linenr() == linenr) {
				for(const ValueFlow::Value &v : tok->values()) {
					if(v.isLifetimeValue() && v.lifetimeScope == lifetimeScope &&
					    Token::simpleMatch(v.tokvalue, value, strlen(value)))
						return true;
				}
			}
		}
...
#define tokenValues(...) tokenValues_(__FILE__, __LINE__, __VA_ARGS__)
	std::list<ValueFlow::Value> tokenValues_(const char* file,
	    int line,
	    const char code[],
	    const char tokstr[],
...
#define lifetimeValues(...) lifetimeValues_(__FILE__, __LINE__, __VA_ARGS__)
	std::vector<std::string> lifetimeValues_(const char* file,
	    int line,
	    const char code[],
	    const char tokstr[],
	    const Settings * s = nullptr) {
...
#define valueOfTok(code, tokstr) valueOfTok_(code, tokstr, __FILE__, __LINE__)
	...
	void valueFlowNumber() {
		...
		ASSERT_EQUALS_DOUBLE(192.0, valueOfTok("x=0x0.3p10;", "0x0.3p10").floatValue, 1e-5); // 3 * 16^-1 * 2^10
		                                                                                     // = 192
	...
	void valueFlowCalculations() {
		...
		ASSERT(tokenValues("(UNKNOWN_TYPE)1000;", "(").empty()); // don't know if there is truncation, sign
		                                                         // extension
...
#undef CHECK
	...
	void valueFlowBeforeConditionLoop() { // while, for, do-while
		...
		code = "void f(int x) {\n" // conditions inside loop, x is assigned inside do-while => don't use
		                           // condition
		    "    a = x;\n"
		    "    do {\n"
	...
	void valueFlowContainerSize() {
		...
		code = "void f(const std::vector<int> &ints) {\n"
		    ...
		    "}";
		ASSERT_EQUALS("",
		    isKnownContainerSizeValue(tokenValues(code, "ints . front", ValueFlow::Value::ValueType::CONTAINER_SIZE), 3));
		...
		code = "void f(const std::vector<int> &ints) {\n"
		    ...
		    "}";
		ASSERT_EQUALS("",
		    isKnownContainerSizeValue(tokenValues(code, "ints . front", ValueFlow::Value::ValueType::CONTAINER_SIZE), 4));
		...
		code = "void f(const std::vector<int> &ints) {\n"
		    ...
		    "}";
		ASSERT_EQUALS("",
		    isKnownContainerSizeValue(tokenValues(code, "ints . front", ValueFlow::Value::ValueType::CONTAINER_SIZE), 2));
		...
		code = "bool f() {\n"
		    ...
		    "}\n";
		ASSERT_EQUALS("",
		    isInconclusiveContainerSizeValue(tokenValues(code, "x1 . empty", ValueFlow::Value::ValueType::CONTAINER_SIZE), 0));
		ASSERT_EQUALS("",
		    isInconclusiveContainerSizeValue(tokenValues(code, "x2 . empty", ValueFlow::Value::ValueType::CONTAINER_SIZE), 0));
