...
void CheckUninitVar::checkScope(const Scope* scope, const std::set<std::string> &arrayTypeDefs)
{
	for(const Variable &var : scope->varlist) {
		if((mTokenizer->isCPP() && var.type() && !var.isPointer() &&
		    var.type()->needInitialization != Type::NeedInitialization::True) ||
		...
		if(Token::Match(var.nameToken(), "%name% ) (") && Token::simpleMatch(var.nameToken()->linkAt(2), ") =")) { // Function
			                                                                                                   // pointer
			                                                                                                   // is
			                                                                                                   // initialized,
			                                                                                                   // but
			...
			if(arg.declarationId() && Token::Match(arg.typeStartToken(), "%type% * %name% [,)]")) {
				...
				for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
					if(!Token::Match(tok, "[;{}] %varid% =", arg.declarationId()))
						continue;
					const Token * allocFuncCallToken = findAllocFuncCallToken(tok->tokAt(2)->astOperand2(),
						mSettings->library);
...
{
	const bool suppressErrors(possibleInit && *possibleInit); // Assume that this is a variable declaration, rather
	                                                          // than a fundef
	...
	for(; tok; tok = tok->next()) {
		...
		// Inner scope..
			...
			if(tok->str() == "{") {
				...
				const bool initif = !alwaysFalse && checkScopeForVariable(tok->next(),
					var,
					&possibleInitIf,
					&noreturnIf,
					alloc,
				...
				if(!alwaysTrue && noreturnIf && number_of_if > 0) {
					if(printDebug) {
						...
						reportError(tok,
						    Severity::debug,
						    "bailoutUninitVar",
						    "bailout uninitialized variable checking for '" + var.name() + "'. can't determine if this condition can be false when previous condition is false: " +
						    condition);
					}
					return true;
				}
				...
				if(!alwaysFalse && !initif && !noreturnIf) {
					for(const Token * tok2 = tok; tok2 && tok2 != tok->link(); tok2 = tok2->next()) {
						if(Token::Match(tok2, "[;{}.] %name% = - %name% ;"))
							...
							varValueIf[tok2->next()->varId()] =
							    VariableValue(MathLib::toLongNumber(tok2->strAt(3)));
					}
				}
				...
				if(!Token::simpleMatch(tok, "} else {")) {
					...
					const bool initelse = !alwaysTrue && checkScopeForVariable(tok->next(),
						var,
						&possibleInitElse,
						&noreturnElse,
						alloc,
		...
		// = { .. }
			...
			visitAstNodes(tok->next(),
				...
				if(child->str() == "." &&
				Token::Match(child->astOperand1(), "%varid%",
				var.declarationId()) && child->astOperand2() && child->astOperand2()->str() == membervar) {
					errorToken = child;
			...
			while(tok && tok->str() != ";") {
				...
				if(tok->varId() == var.declarationId()) {
					if(!membervar.empty()) {
						if(!suppressErrors &&
						    Token::Match(tok,
						    "%name% . %name%") && tok->strAt(2) == membervar &&
						    Token::Match(tok->next()->astParent(), "%cop%|return|throw|?"))
							uninitStructMemberError(tok, tok->str() + "." + membervar);
						else if(mTokenizer->isCPP() && !suppressErrors &&
						    Token::Match(tok, "%name%") && Token::Match(tok->astParent(), "return|throw|?"))
							uninitStructMemberError(tok, tok->str() + "." + membervar);
					}
		...
		if(tok->varId() == var.declarationId()) {
			...
			if(mTokenizer->isCPP() && var.isPointer() &&
			    (var.typeStartToken()->isStandardType() || var.typeStartToken()->isEnumType() ||
			    (var.type() && var.type()->needInitialization == Type::NeedInitialization::True)) &&
			    Token::simpleMatch(tok->next(), "= new")) {
				*alloc = CTOR_CALL;
			...
			if(!membervar.empty()) {
				...
				if(!suppressErrors && isVariableUsage(tok, var.isPointer(), *alloc)) {
					...
					while(parent->astParent() &&
					    ((astIsLHS(parent) && parent->astParent()->str() == "[") ||
					    parent->astParent()->isUnaryOp("*"))) {
						parent = parent->astParent();
						if(parent->str() == "[") {
							if(const Token * errorToken =
							    checkExpr(parent->astOperand2(), var, *alloc, number_of_if==0)) {
								if(!suppressErrors)
			...
			if(isMemberVariableAssignment(tok, membervar)) {
				...
				for(const Token * tok2 = tok->next(); tok2; tok2 = tok2->next()) {
					...
					if(rhs && tok2->varId() == var.declarationId() &&
					    isMemberVariableUsage(tok2, var.isPointer(), alloc, membervar)) {
						assign = false;
						break;
					}
				}
...
static bool isVoidCast(const Token * tok)
{
	return Token::simpleMatch(tok,
		   "(") && tok->isCast() && tok->valueType() && tok->valueType()->type == ValueType::Type::VOID_T &&
	       tok->valueType()->pointer == 0;
}

const Token* CheckUninitVar::isVariableUsage(bool cpp, const Token * vartok, const Library& library, bool pointer, Alloc alloc,
    int indirect)
{
	...
	while(valueExpr->astParent()) {
		// *&x
		if(valueExpr->astParent()->isUnaryOp("&") && valueExpr->astParent()->astParent() &&
		    valueExpr->astParent()->astParent()->isUnaryOp("*"))
			valueExpr = valueExpr->astParent()->astParent();
		// (type &)x
		else if(valueExpr->astParent()->isCast() && valueExpr->astParent()->isUnaryOp("(") &&
		    Token::simpleMatch(valueExpr->astParent()->link()->previous(), "& )"))
			valueExpr = valueExpr->astParent();
		else
			break;
	}
	if(!pointer) {
		if(Token::Match(vartok, "%name% [.(]") && vartok->variable() && !vartok->variable()->isPointer())
			return nullptr;
		while(Token::simpleMatch(valueExpr->astParent(),
		    ".") && astIsLhs(valueExpr) && valueExpr->astParent()->valueType() && valueExpr->astParent()->valueType()->pointer == 0)
			valueExpr = valueExpr->astParent();
	}
	...
	if(Token::simpleMatch(valueExpr->astParent(),
	    "&") && astIsRhs(valueExpr) && Token::Match(valueExpr->astParent()->tokAt(-3), "( %name% ) &"))
		return nullptr;
	...
	if(cpp && Token::Match(valueExpr->astParent(), "<<|>>")) {
		...
		if(const auto* vt = valueExpr->valueType()) {
			if(vt->type == ValueType::Type::VOID_T)
				return nullptr;
			// passing a char* to a stream will dereference it
			if((alloc == CTOR_CALL || alloc == ARRAY) && vt->pointer && vt->type != ValueType::Type::CHAR &&
			    vt->type != ValueType::Type::WCHAR_T)
				return nullptr;
		}
	}
		...
		if(func) {
			...
			if(arg) {
				...
				if(Token::Match(argStart, "const %type% & %name% [,)]")) {
					...
					if(vartok->variable() && vartok->variable()->valueType() &&
					    vartok->variable()->valueType()->type == ValueType::Type::RECORD)
						return -1;
					return 1;
				}
				if((pointer || address) &&
				    Token::Match(argStart, "const %type% %name% [") && Token::Match(argStart->linkAt(3), "] [,)]"))
					return 1;
			}
		}
		...
		if(Token::Match(tok->tokAt(-2), "[(,] & %name%")) {
			...
			if(Token::Match(ftok, "%name% (")) {
				...
				if(!function && mSettings) {
					// Function definition not seen, check if direction is specified in the library
					// configuration
					const Library::ArgumentChecks::Direction argDirection = mSettings->library.getArgDirection(ftok,
						1 + argumentNumber);
...
bool CheckUninitVar::isMemberVariableUsage(const Token * tok, bool isPointer, Alloc alloc, const std::string &membervar) const
{
	...
	if(Token::Match(tok,
	    ...
	    ".") && Token::Match(tok->previous(), "[(,] %name% [,)]") && isVariableUsage(tok, isPointer, alloc))
		return true;
...
void CheckUninitVar::uninitstringError(const Token * tok, const std::string &varname, bool strncpy_)
{
	reportError(tok,
	    ...
	    (strncpy_ ? " (strncpy doesn't always null-terminate it)." : " (not null-terminated)."),
	    CWE_USE_OF_POTENTIALLY_DANGEROUS_FUNCTION,
	    Certainty::normal);
}
