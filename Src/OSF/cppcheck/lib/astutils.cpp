...
bool astIsIterator(const Token * tok) { return tok && tok->valueType() && tok->valueType()->type == ValueType::Type::ITERATOR; }
bool astIsContainer(const Token* tok) { return getLibraryContainer(tok) != nullptr && !astIsIterator(tok); }
...
bool astIsContainerOwned(const Token* tok) { return astIsContainer(tok) && !astIsContainerView(tok); }
bool astIsRangeBasedForDecl(const Token* tok) { return Token::simpleMatch(tok->astParent(), ":") && Token::simpleMatch(tok->astParent()->astParent(), "("); }
...
bool isTemporary(bool cpp, const Token* tok, const Library* library, bool unknown)
{
	...
	if(Token::simpleMatch(tok, "."))
		return (tok->originalName() != "->" && isTemporary(cpp, tok->astOperand1(), library)) || isTemporary(cpp, tok->astOperand2(), library);
	if(Token::Match(tok, ",|::"))
...
Token* getInitTok(Token* tok) { return getInitTokImpl(tok); }
const Token* getInitTok(const Token* tok) { return getInitTokImpl(tok); }
Token* getStepTok(Token* tok) { return getStepTokImpl(tok); }
const Token* getStepTok(const Token* tok) { return getStepTokImpl(tok); }
    ...
    MathLib::bigint * const lastValue)
{
	...
	*partialCond = Token::Match(condExpr, "%oror%|&&");
	visitAstNodes(condExpr, [varid, &condExpr](const Token * tok) {
		...
		if(Token::Match(tok,
		"<|<=") && tok->isBinaryOp() && tok->astOperand1()->varId() == *varid && tok->astOperand2()->hasKnownIntValue()) {
			if(Token::Match(condExpr,
	...
	if(!Token::Match(condExpr,
	    "<|<=") || !condExpr->isBinaryOp() || condExpr->astOperand1()->varId() != *varid ||
...
bool exprDependsOnThis(const Token* expr, bool onVar, nonneg int depth)
{
	...
	if(Token::Match(expr,
	    "%name% (") && expr->function() && expr->function()->nestedIn && expr->function()->nestedIn->isClassOrStruct() &&
	    !expr->function()->isStatic()) {
...
bool isStructuredBindingVariable(const Variable* var)
{
	...
	while(tok && Token::Match(tok->astParent(), "[|,|:"))
		tok = tok->astParent();
	return tok && (tok->str() == "[" || Token::simpleMatch(tok->previous(), "] :")); // TODO: remove workaround when
	                                                                                 // #11105 is fixed
}
...
static const Token * followVariableExpression(const Token * tok, bool cpp, const Token * end = nullptr)
{
	...
	const Token * endToken =
	    (isInLoopCondition(tok) || isInLoopCondition(varTok) || var->scope() != tok->scope()) ? var->scope()->bodyEnd : lastTok;
...
static bool compareKnownValue(const Token * const tok1, const Token * const tok2, std::function<bool(const ValueFlow::Value&,
    const ValueFlow::Value&, bool)> compare)
{
    ...
    ErrorPath* errors)
{
	...
	bool noncommutativeEquals = isSameExpression(cpp, macro, tok1->astOperand1(), tok2->astOperand1(), library, pure, followVar, errors);
	noncommutativeEquals = noncommutativeEquals && isSameExpression(cpp, macro, tok1->astOperand2(), tok2->astOperand2(), library, pure, followVar, errors);
	...
	if(cpp && tok1->str() == "+" && tok1->isBinaryOp()) {
		...
		if(!(vt1 && (vt1->type >= ValueType::VOID_T || vt1->pointer) && vt2 && (vt2->type >= ValueType::VOID_T || vt2->pointer)))
			return false;
	}
	const bool commutative = tok1->isBinaryOp() && Token::Match(tok1, "%or%|%oror%|+|*|&|&&|^|==|!=");
	bool commutativeEquals = commutative && isSameExpression(cpp, macro, tok1->astOperand2(), tok2->astOperand1(), library, pure, followVar, errors);
	commutativeEquals = commutativeEquals && isSameExpression(cpp, macro, tok1->astOperand1(), tok2->astOperand2(), library, pure, followVar, errors);
		...
		for(const Token* tok1: {
			...
			for(const Token* tok2: {
				...
				if(isSameExpression(cpp, true, tok1, tok2, library, pure, followVar, errors)) {
					if(isOppositeCond(isNot, cpp, tok1->astSibling(), tok2->astSibling(), library, pure, followVar,
					    errors))
						return true;
				}
			}
...
bool isWithoutSideEffects(bool cpp, const Token* tok, bool checkArrayAccess, bool checkReference)
{
	...
	if(tok && tok->varId()) {
		const Variable* var = tok->variable();
		return var &&
		       ((!var->isClass() && (checkReference || !var->isReference())) || var->isPointer() ||
		       (checkArrayAccess ? var->isStlType() && !var->isStlType(CheckClass::stl_containers_not_const) : var->isStlType()));
	}
	return true;
}
...
static bool hasNoreturnFunction(const Token* tok, const Library* library, const Token** unknownFunc)
{
	...
	if(ftok) {
		...
		return hasNoreturnFunction(tok->astOperand1(), library, unknownFunc) || hasNoreturnFunction(tok->astOperand2(),
			   library,
			   unknownFunc);
	}
...
bool isVariableChanged(const Token * tok, int indirect, const Settings * settings, bool cpp, int depth)
{
	...
	while(Token::simpleMatch(tok2->astParent(), "*") ||
	    ...
	    (tok2->astParent() && tok2->astParent()->isUnaryOp("&") && !tok2->astParent()->astOperand2() &&
	    Token::simpleMatch(tok2->astParent()->astParent(), ".") && tok2->astParent()->astParent()->originalName()=="->") ||
	    (Token::simpleMatch(tok2->astParent(), "[") && tok2 == tok2->astParent()->astOperand1())) {
		if(tok2->astParent() &&
		    (tok2->astParent()->isUnaryOp("*") || (astIsLHS(tok2) && tok2->astParent()->originalName() == "->")))
			derefs++;
		if(derefs > indirect)
			break;
	...
	auto skipRedundantPtrOp = [](const Token* tok, const Token* parent) {
		    ...
		    while(parent && gparent &&
			((parent->isUnaryOp("*") && gparent->isUnaryOp("&")) || (parent->isUnaryOp("&") && gparent->isUnaryOp("*")))) {
			    tok = gparent;
	...
	if(tok->variable() &&
	    ...
	    ". %name%") && isFunctionCall(tok2->astParent()->next()) && tok2->astParent()->astOperand1() == tok2) {
		const Variable * var = tok->variable();
	...
	if(Token::Match(tok2->astParent(),
	    ":|=") && tok2 == tok2->astParent()->astOperand2() && Token::simpleMatch(tok2->astParent()->previous(), "]")) {
	...
	if(Token::simpleMatch(tok2->astParent(),
	    ":") && tok2->astParent()->astParent() && Token::simpleMatch(tok2->astParent()->astParent()->previous(), "for (")) {
...
static bool isExpressionChangedAt(const F& getExprTok, const Token* tok, int indirect, const nonneg int exprid, bool globalvar,
    const Settings* settings, bool cpp, int depth)
{
	if(depth < 0)
		return true;
	if(tok->exprId() != exprid) {
			...
			aliased = findAstNode(getExprTok(), [&](const Token* childTok) {
				for(const ValueFlow::Value& val : tok->values()) {
					...
					if(val.isLocalLifetimeValue() || (pointer && val.isSymbolicValue() && val.intvalue == 0)) {
						if(findAstNode(val.tokvalue, [&](const Token* aliasTok) { return aliasTok->exprId() == childTok->exprId(); }))
							return true;
					}
				}
				return false;
...
std::vector<const Token*> getArguments(const Token* ftok) { return astFlatten(getArgumentStart(ftok), ","); }
...
{
	return tok && Token::simpleMatch(tok->previous(), "> (") && tok->astOperand2() && tok->astOperand1() && isCPPCastKeyword(
		tok->astOperand1());
}
...
bool isConstVarExpression(const Token * tok, const char* skipMatch)
{
	...
	if(tok->str() == "?" && tok->astOperand2() && tok->astOperand2()->str() == ":") // ternary operator
		return isConstVarExpression(tok->astOperand2()->astOperand1()) && isConstVarExpression(tok->astOperand2()->astOperand2()); // left
	                                                                                                                                   // and
	                                                                                                                                   // right
	                                                                                                                                   // of
	                                                                                                                                   // ":"
...
{
	return !var || (!var->isLocal() && !var->isArgument()) || (deref && var->isArgument() && var->isPointer()) || var->isStatic() ||
	       var->isReference() || var->isExtern();
}
...
bool isGlobalData(const Token * expr, bool cpp)
{
	...
	visitAstNodes(expr,
		...
		if(tok->originalName() == "->") {
			...
			else if(lhsvar->isArgument() && (!lhstype || (lhstype->type <= ValueType::Type::VOID_T && !lhstype->container))) {
				globalData = true;
				return ChildrenToVisit::none;
			}
		}
...
struct FwdAnalysis::Result FwdAnalysis::checkRecursive(const Token * expr, const Token * startToken, const Token * endToken,
    const std::set<nonneg int> &exprVarIds, bool local, bool inInnerClass, int depth){
	...
	for(const Token* tok = startToken; precedes(tok, endToken); tok = tok->next()) {
		...
		if(const Token * lambdaEndToken = findLambdaEndToken(tok)) {
			tok = lambdaEndToken;
			const Result lambdaResult = checkRecursive(expr, lambdaEndToken->link()->next(), lambdaEndToken,
				exprVarIds, local, inInnerClass, depth);
		...
		if(mWhat == What::Reassign &&
			...
			// TODO: This is a quick bailout to avoid FP #9420, there are false negatives
			// (TODO_ASSERT_EQUALS)
			return Result(Result::Type::BAILOUT);
		...
		if(exprVarIds.find(tok->varId()) != exprVarIds.end()) {
			...
			while(!same && Token::Match(parent->astParent(), "*|.|::|[|(|%cop%")) {
				...
				if(Token::Match(parent, ". %var%") && parent->next()->varId() && exprVarIds.find(parent->next()->varId()) == exprVarIds.end() &&
				    isSameExpression(mCpp, false, expr->astOperand1(), parent->astOperand1(), mLibrary, true, false,
				    nullptr)) {
					other = true;
					break;
				}
			}
			if(mWhat != What::ValueFlow && same &&
			    Token::simpleMatch(parent->astParent(), "[") && parent == parent->astParent()->astOperand2()) {
...
std::set<nonneg int> FwdAnalysis::getExprVarIds(const Token* expr, bool* localOut, bool* unknownVarIdOut) const
{
	...
	visitAstNodes(expr,
		...
		if(tok->varId() > 0) {
			...
			if(!Token::simpleMatch(tok->previous(), ".")) {
				const Variable * var = tok->variable();
				if(var && var->isReference() && var->isLocal() &&
				Token::Match(var->nameToken(), "%var% [=(]") && !isGlobalData(var->nameToken()->next()->astOperand2()))
					return ChildrenToVisit::none;
...
bool FwdAnalysis::unusedValue(const Token * expr, const Token * startToken, const Token * endToken)
{
	...
	Result result = check(expr, startToken, endToken);
	return (result.type == FwdAnalysis::Result::Type::NONE || result.type == FwdAnalysis::Result::Type::RETURN) && !possiblyAliased(
		expr,
		startToken);
}
			...
			for(int argnr = 0; argnr < args.size(); ++argnr) {
				...
				for(const Token * subexpr = expr; subexpr; subexpr = subexpr->astOperand1()) {
					if(isSameExpression(mCpp, macro, subexpr, args[argnr], mLibrary, pure, followVar)) {
						const Scope* scope = expr->scope(); // if there is no other variable,
						                                    // assume no aliasing
