...
static const Token * checkMissingReturnScope(const Token * tok, const Library &library)
{
	...
	while((tok = tok->previous()) != nullptr) {
		...
		if(tok->str() == "}") {
			for(const Token * prev = tok->link()->previous();
			    prev && prev->scope() == tok->scope() && !Token::Match(prev, "[;{}]");
			    prev = prev->previous()) {
			...
			if(tok->scope()->type == Scope::ScopeType::eSwitch) {
				...
				for(const Token * switchToken = tok->link()->next(); switchToken != tok;
				    switchToken = switchToken->next()) {
					if(reachable && Token::simpleMatch(switchToken, "break ;")) {
						if(Token::simpleMatch(switchToken->previous(),
						    "}") && !checkMissingReturnScope(switchToken->previous(), library))
							reachable = false;
						else
							return switchToken;
					}
					...
					if(Token::simpleMatch(switchToken, "default :"))
						hasDefault = true;
					else if(switchToken->str() == "{" &&
					    (switchToken->scope()->isLoopScope() ||
					    switchToken->scope()->type == Scope::ScopeType::eSwitch))
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(printWarnings && Token::Match(tok, "%name% ( !!)")) {
				...
				// fmod ( x , y) If y is zero, then either a range error will occur or the function will
				// return zero (implementation-defined).
...
//     </arg>
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok && (tok != scope->bodyEnd); tok = tok->next()) {
			...
			if(printWarning && secondParamTok->isNumber()) { // Check if the second parameter is a literal
				                                         // and is out of range
