...
{
	return tokenizer->list.file(function->tokenDef) + ':' + MathLib::toString(function->tokenDef->linenr()) + ':' + MathLib::toString(
		function->tokenDef->column());
}
		...
		for(const Token * tok = scope.bodyStart; tok != scope.bodyEnd; tok = tok->next()) {
			...
			for(int argnr = 0; argnr < args.size(); ++argnr) {
				...
				if(argtok->variable() && argtok->variable()->isArray() && argtok->variable()->dimensions().size()==1 &&
					...
					functionCall.callArgNr = argnr + 1;
					functionCall.callArgumentExpression = argtok->expressionString();
					functionCall.callArgValue = argtok->variable()->dimension(0) * argtok->valueType()->typeSize(
...
static std::list<std::pair<const Token *, MathLib::bigint> > getUnsafeFunction(const Tokenizer * tokenizer,
    ...
    bool (*isUnsafeUsage)(const Check * check, const Token * argtok, MathLib::bigint * value))
{
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		for(int argnr = 0; argnr < function->argCount(); ++argnr) {
			for(const std::pair<const Token *,
			    MathLib::bigint> &v : getUnsafeFunction(tokenizer, settings, &scope, argnr, check, isUnsafeUsage)) {
    ...
    bool warning)
{
	...
	for(int index = 9; index >= 0; index--) {
		...
		if(functionCall) {
			...
			std::copy(functionCall->callValuePath.cbegin(), functionCall->callValuePath.cend(),
			    std::back_inserter(locationList));
		}

		ErrorMessage::FileLocation fileLoc(path[index]->location.fileName,
		    path[index]->location.lineNumber,
		    path[index]->location.column);
		fileLoc.setinfo("Calling function " + path[index]->callFunctionName + ", " + MathLib::toString(
			    path[index]->callArgNr) + getOrdinalText(path[index]->callArgNr) + " argument is " + value1);
