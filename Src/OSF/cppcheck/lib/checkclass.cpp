...
{
	switch(type) {
		case Function::eConstructor: return "constructor";
		case Function::eCopyConstructor: return "copy constructor";
		case Function::eMoveConstructor: return "move constructor";
		case Function::eDestructor: return "destructor";
		case Function::eFunction: return "function";
		case Function::eOperatorEqual: return "operator=";
		case Function::eLambda: return "lambda";
	}
	return "";
}
...
void CheckClass::constructors()
{
	...
	for(const Scope * scope : mSymbolDatabase->classAndStructScopes) {
		...
		for(const Function &func : scope->functionList) {
			...
			// Bail: If initializer list is not recognized as a variable or type then skip since parsing is
			// incomplete
			...
			for(Usage &usage : usageList) {
				...
				if(func.type == Function::eOperatorEqual) {
					...
					if(classNameUsed &&
					    mSettings->library.getTypeCheck("operatorEqVarError",
					    var.getTypeName()) != Library::TypeCheck::suppress)
					...
					// If constructor is not in scope then we maybe using a constructor from a
...
void CheckClass::checkExplicitConstructors()
{
	...
	for(const Scope * scope : mSymbolDatabase->classAndStructScopes) {
		...
		for(const Function &func : scope->functionList) {
			...
			//  4) Constructor is not marked as delete (programmer can mark the default constructor as
			// deleted, which is ok)
	...
	for(const Scope * scope : mSymbolDatabase->classAndStructScopes) {
		...
		for(const Function &func : scope->functionList) {
			...
			for(tok = func.functionScope->bodyStart; tok != func.functionScope->bodyEnd; tok = tok->next()) {
				if(Token::Match(tok, "%var% = new|malloc|g_malloc|g_try_malloc|realloc|g_realloc|g_try_realloc")) {
				    ...
				    "%var% = %name% . %name% ;") && allocatedVars.find(tok->varId()) != allocatedVars.end()) {
					copiedVars.insert(tok);
				}
			}
			break;
		...
		if(copyCtor && !copiedVars.empty()) {
			...
			/* FIXME: This doesn't work. See #4154
			   for (std::map<int, const Token*>::const_iterator i = allocatedVars.begin(); i !=
			      allocatedVars.end(); ++i) {
...
{
	reportError(alloc, Severity::warning, "noCopyConstructor",
	    noMemberErrorMessage(scope, "copy constructor", isdefault),
	    CWE398, inconclusive ? Certainty::inconclusive : Certainty::normal);
}

void CheckClass::noOperatorEqError(const Scope * scope, bool isdefault, const Token * alloc, bool inconclusive)
{
	reportError(alloc, Severity::warning, "noOperatorEq",
	    noMemberErrorMessage(scope, "operator=", isdefault), CWE398, inconclusive ? Certainty::inconclusive : Certainty::normal);
}

void CheckClass::noDestructorError(const Scope * scope, bool isdefault, const Token * alloc)
{
	reportError(alloc, Severity::warning, "noDestructor", noMemberErrorMessage(scope, "destructor", isdefault),
	    CWE398, Certainty::normal);
}
    ...
    const Scope * scope, std::vector<Usage> &usage)
{
	...
	for(; ftok && ftok != func.functionScope->bodyEnd; ftok = ftok->next()) {
		...
		if(initList) {
			if(level == 0 && Token::Match(ftok, "%name% {|(") && Token::Match(ftok->linkAt(1), "}|) ,|{")) {
				if(ftok->str() != func.name()) {
					...
					// there is a called member function, but it has no implementation, so we assume
			...
			else if(level != 0 && Token::Match(ftok, "%name% =")) // assignment in the initializer:
				                                              // var(value = x)
		...
		if(func.type == Function::eOperatorEqual &&
		    Token::Match(ftok, "[;{}] %name% (") && Token::Match(ftok->linkAt(2), ") . %name% ( *| this ) ;")) {
			assignAllVar(usage);
			break;
		}
		...
		// Calling member function?
			...
			if(ftok->function()) {
				...
				// there is a called member function, but it has no implementation, so we assume it
			...
			// not member function
				...
				if(!func.isConstructor() && isBaseClassFunc(ftok, scope)) {
					/** @todo False Negative: we should look at the base class functions to see if
					   they
...
void CheckClass::noConstructorError(const Token * tok, const std::string &classname, bool isStruct)
{
	...
	reportError(tok, Severity::style, "noConstructor",
	    "$symbol:" + classname + "\n" + "The " + std::string(
		    isStruct ? "struct" : "class") + " '$symbol' does not declare a constructor although it has private member variables which likely require initialization.\n"
...
void CheckClass::uninitVarError(const Token * tok, bool isprivate, Function::Type functionType, const std::string &classname, const std::string &varname,
    bool derived, bool inconclusive)
{
	std::string ctor;
...
{
	reportError(tok, Severity::performance, "useInitializationList",
...
static bool checkFunctionUsage(const Function * privfunc, const Scope* scope)
{
	...
	for(std::list<Function>::const_iterator func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
		if(func->functionScope) {
			...
			for(const Token * ftok = func->functionScope->classDef->linkAt(1);
			    ftok != func->functionScope->bodyEnd;
			    ftok = ftok->next()) {
...
void CheckClass::privateFunctions()
{
	...
	for(const Scope * scope : mSymbolDatabase->classAndStructScopes) {
		...
		for(const Function &func : scope->functionList) {
			...
			if(func.type == Function::eFunction && func.access == AccessControl::Private && !func.isOperator()) // TODO:
				                                                                                            // There
				                                                                                            // are
				                                                                                            // smarter
				                                                                                            // ways
		...
		if(!scope->definedType->derivedFrom.empty()) {
			...
			for(std::list<const Function*>::iterator it = privateFuncs.begin(); it != privateFuncs.end();) {
				if((*it)->isImplicitlyVirtual(true)) // Give true as default value to be returned if we
					                             // don't see all base classes
...
{
	reportError(tok, Severity::style, "unusedPrivateFunction",
		...
		for(const Token * tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
			if(Token::Match(tok, "memset|memcpy|memmove (")) {
				...
				if(Token::Match(arg3, "sizeof ( %type% ) )"))
					...
					int numIndirToVariableType = 0; // Offset to the actual type in terms of
			    ...
			    Token::Match(tok, "%var% = calloc|malloc|realloc|g_malloc|g_try_malloc|g_realloc|g_try_realloc (")) {
				const std::set<const Scope *> parsedTypes;
...
{
	reportError(tok, Severity::portability, "memsetClassFloat",
					...
					if(it->tokenDef->previous()->str() == "&" && it->tokenDef->strAt(-2) == scope->className) {
						...
						if(!it->isConst()) {
							...
							if(analyzedFunctions.find(&*it) == analyzedFunctions.end()) {
								...
								checkReturnPtrThis(scope, &*it, it->arg->link()->next(), it->arg->link()->next()->link(), analyzedFunctions);
							}
							// just bail for now
							else
								return;
...
{
	reportError(tok, Severity::style, "operatorEqRetRefThis",
	    "'operator=' should return reference to 'this' instance.", CWE398, Certainty::normal);
}
...
{
	if(error) {
		reportError(tok, Severity::error, "operatorEqMissingReturnStatement",
		    "No 'return' statement in non-void function causes undefined behavior.", CWE398, Certainty::normal);
	}
	else {
...
void CheckClass::operatorEqToSelf()
{
	...
	for(const Scope * scope : mSymbolDatabase->classAndStructScopes) {
		...
		for(const Function &func : scope->functionList) {
			if(func.type == Function::eOperatorEqual && func.hasBody()) {
				// make sure that the operator takes an object of the same type as *this, otherwise we
				// can't detect self-assignment checks
...
CheckClass::Bool CheckClass::isInverted(const Token * tok, const Token * rhs)
{
	...
	for(const Token * itr = tok; itr && itr->str()!="("; itr = itr->astParent()) {
		if(Token::simpleMatch(itr, "!=") && (isTrueKeyword(itr->astOperand1()) || isTrueKeyword(itr->astOperand2()))) {
		    ...
		    "!=") &&
		    ((Token::simpleMatch(itr->astOperand1(),
		    "this") &&
		    Token::simpleMatch(itr->astOperand2(),
		...
		for(const Type::BaseInfo & j : scope->definedType->derivedFrom) {
			...
			if(j.access != AccessControl::Private && j.type) {
				...
				// If this pattern is not seen then bailout the checking of these base/derived classes
				{
					...
					// No deletion of derived class instance through base class pointer found => the
					// code is ok
					bool ok = true;
				...
				if(!baseDestructor) {
					if(derivedFrom->derivedFrom.empty()) {
						virtualDestructorError(derivedFrom->classDef,
						    derivedFrom->name(),
						    derivedClass->str(),
						    false);
					}
					...
					if(derivedFrom->derivedFrom.empty()) {
						...
						if(baseDestructor->access == AccessControl::Public) {
							virtualDestructorError(baseDestructor->token,
							    derivedFrom->name(),
							    derivedClass->str(),
							    false);
							// check for duplicate error and remove it if found
							const std::list<const Function *>::iterator found = find(inconclusiveErrors.begin(),
								inconclusiveErrors.end(),
								baseDestructor);
...
void CheckClass::virtualDestructorError(const Token * tok, const std::string &Base, const std::string &Derived, bool inconclusive)
{
	if(inconclusive) {
		if(mSettings->severity.isEnabled(Severity::warning))
			reportError(tok, Severity::warning, "virtualDestructor",
			    "$symbol:" + Base + "\nClass '$symbol' which has virtual members does not have a virtual destructor.",
			    CWE404, Certainty::inconclusive);
	}
	else {
		...
		if(var.name() == tok->str()) {
			...
			if(isMember) {
				if(tok->varId() == 0)
					mSymbolDatabase->debugMessage(tok,
					    "varid0",
					    "CheckClass::isMemberVar found used member variable \'" + tok->str() + "\' with varid 0");
	...
	for(const Token * tok1 = func->functionScope->bodyStart; tok1 && tok1 != func->functionScope->bodyEnd; tok1 = tok1->next()) {
		if(tok1->isName() && isMemberVar(scope, tok1)) {
			...
			if(tok1->valueType() && tok1->valueType()->pointer > 0 && tok1->astParent() && tok1->astParent()->isCast() &&
			    !Token::simpleMatch(tok1->astParent(), "( const"))
				return false;
			...
			if(lhs->str() == "&") {
				...
				// TODO: We could additionally check what is done with the elements to avoid false
				// negatives. Here we just rely on "const" keyword being used.
			...
			for(;;) {
				if(Token::Match(end->next(), ". %name%")) {
					...
					if(end->varId()) {
						...
						if(var && assignTok && assignTok->isAssignmentOp() && assignTok->astOperand1() &&
						    assignTok->astOperand1()->variable()) {
							...
							if(assignVar->isPointer() && !assignVar->isConst() && var->typeScope()) {
								...
								// if there is no operator that is const and returns a
								// non-const pointer, func cannot be const
								if(std::none_of(funcMap.begin(), funcMap.end(),
								    [](const std::pair<std::string, const Function*>& fm) {
									return fm.second->isConst() && fm.first == "operator[]" &&
									!Function::returnsConst(fm.second);
								}))
									return false;
							}
						}
			...
			if(end->strAt(1) == "(") {
				...
				if((var->isStlType() // assume all std::*::size() and std::*::empty() are const
				    ...
				    ((lastVarTok->valueType()->container->getYield(end->str()) ==
				    ...
				    (tok1->previous()->isAssignmentOp() && tok1->tokAt(-2)->variable() &&
				    Token::Match(tok1->tokAt(-2)->variable()->typeEndToken(), "const_iterator|const_reverse_iterator")))))
					;
			...
			tok1 = jumpBackToken ? jumpBackToken : end; // Jump back to first [ to check inside, or jump to
			                                            // end of expression
		...
		// function/constructor call, return init list
		    ...
		    "%name% (|{") || Token::simpleMatch(tok1->astParent(), "return {")) && !tok1->isStandardType() &&
			...
			for(const Token* tok2 = lpar->next(); tok2 && tok2 != tok1->next()->link(); tok2 = tok2->next()) {
				if(tok2->str() == "(")
				    ...
				    isMemberVar(scope, tok2)) || (tok2->isUnaryOp("&") && (tok2 = tok2->astOperand1()))) {
					const Variable* var = tok2->variable();
					if(!var || !var->isMutable())
						return false; // TODO: Only bailout if function takes argument as
					                      // non-const reference
				}
			}
		}
		    ...
		    "> (") &&
		    (!tok1->link() || !Token::Match(tok1->link()->previous(), "static_cast|const_cast|dynamic_cast|reinterpret_cast"))) {
			return false;
		}
	}
...
namespace { // avoid one-definition-rule violation
	struct ClsVarInfo {
		ClsVarInfo(const Variable * _var, const Token * _tok) : var(_var), tok(_tok) 
		{
		}
		const Variable * var;
		const Token * tok;
				...
				if(tok->str() == ":") {
					...
					for(int j = 1; j < vars.size(); j++) {
						...
						if(vars[j].var->index() < vars[j - 1].var->index())
							initializerListError(vars[j].tok,
							    vars[j].var->nameToken(),
							    scope->className,
							    vars[j].var->name());
					}
				}
			...
			if(Token::Match(tok, "[:,] %var% (|{")) {
				...
				if(Token::Match(varTok->astParent(), "(|{")) {
					if(const Token* initTok = varTok->astParent()->astOperand2()) {
						...
						else if(initTok->isCast() &&
						    ((initTok->astOperand1() && initTok->astOperand1()->varId() == varTok->varId()) ||
						    (initTok->astOperand2() && initTok->astOperand2()->varId() == varTok->varId())))
							selfInitializationError(tok, varTok->str());
					}
				}
			}
...
{
	reportError(tok, Severity::error, "selfInitialization",
    ...
    std::map<const Function *, std::list<const Token *> > & virtualFunctionCallsMap)
{
	...
	for(const Token * tok = function.arg->link(); tok != function.functionScope->bodyEnd; tok = tok->next()) {
		...
		if(!callFunction ||
		    ...
		    !(tok->astParent() &&
		    (tok->astParent()->str() == "(" ||
		    (tok->astParent()->str() == "::" && Token::simpleMatch(tok->astParent()->astParent(), "(")))))
			continue;
...
void CheckClass::getFirstVirtualFunctionCallStack(std::map<const Function *, std::list<const Token *> > & virtualFunctionCallsMap,
    const Token * callToken, std::list<const Token *> & pureFuncStack)
{
...
void CheckClass::virtualFunctionCallInConstructorError(const Function * scopeFunction,
    const std::list<const Token *> & tokStack, const std::string &funcname)
{
	...
	reportError(errorPath, Severity::style, "virtualCallInConstructor",
	    "Virtual function '" + funcname + "' is called from " + scopeFunctionTypeName + " '" + constructorName + "' at line " + MathLib::toString(
	    lineNumber) + ". Dynamic binding is not used.", CWE(0U), Certainty::normal);
}

void CheckClass::pureVirtualFunctionCallInConstructorError(const Function * scopeFunction,
    const std::list<const Token *> & tokStack, const std::string &purefuncname)
{
	const char * scopeFunctionTypeName = scopeFunction ? getFunctionTypeName(scopeFunction->type) : "constructor";
	ErrorPath errorPath;
	for(const Token * tok : tokStack)
	...
	reportError(errorPath, Severity::warning, "pureVirtualCall",
	    ...
	    "Call of pure virtual function '$symbol' in " + scopeFunctionTypeName + ". The call will fail during runtime.",
	    CWE(0U),
	    Certainty::normal);
}
//
			...
			for(const Variable &parentClassVarIt : parentClassIt.type->classScope->varlist) {
				if(classVarIt.name() == parentClassVarIt.name() && !parentClassVarIt.isPrivate()) { // Check if the class and its parent have a common variable
					duplInheritedMembersError(classVarIt.nameToken(), parentClassVarIt.nameToken(),
					    ...
					    typeCurrent->classScope->type == Scope::eStruct, parentClassIt.type->classScope->type == Scope::eStruct);
				}
			}
		}
...
void CheckClass::overrideError(const Function * funcInBase, const Function * funcInDerived)
{
	...
	reportError(errorPath, Severity::style, "missingOverride",
	    ...
	    CWE(0U) /* Unknown CWE! */, Certainty::normal);
}
			...
			if(!var.isStatic()) {
				...
				for(const Function &func : classScope->functionList) {
					if(func.type != Function::Type::eFunction || !func.hasBody())
						continue;
					for(const Token * tok = func.functionScope->bodyStart;
					    tok != func.functionScope->bodyEnd;
					    tok = tok->next()) {
...
bool CheckClass::checkThisUseAfterFreeRecursive(const Scope * classScope, const Function * func, const Variable * selfPointer,
	...
	for(const Token * tok = bodyStart; tok != bodyEnd; tok = tok->next()) {
		...
		if(Token::Match(tok, "delete %var% ;") && selfPointer == tok->next()->variable()) {
			...
			*freeToken = tok;
		    ...
		    "!!. %name%") && tok->variable() && tok->variable()->scope() == classScope && !tok->variable()->isStatic() &&
...
void CheckClass::thisUseAfterFree(const Token * self, const Token * free, const Token * use)
{
	...
	const ErrorPath errorPath = { ErrorPathItem(self, "Assuming '" + selfPointer + "' is used as 'this'"), ErrorPathItem(free,
					  "Delete '" + selfPointer + "', invalidating 'this'"), ErrorPathItem(use,
			...
			while(Token::Match(initList, "[:,] %name% (")) {
				if(Token::Match(initList->tokAt(2), "( %var% )")) {
					...
					if(memberVar && argVar && memberVar->isConst() && memberVar->isReference() &&
						...
						unsafeClassRefMemberError(initList->next(),
						    classScope->className + "::" + memberVar->name());
				}
...
bool CheckClass::analyseWholeProgram(const CTU::FileInfo * ctu, const std::list<Check::FileInfo*> &fileInfo,
    const Settings& settings, ErrorLogger &errorLogger)
{
	...
	for(Check::FileInfo * fi1 : fileInfo) {
		...
		for(const MyFileInfo::NameLoc &nameLoc : fi->classDefinitions) {
			...
			const ErrorMessage errmsg(locationList, emptyString, Severity::error,
			    "$symbol:" + nameLoc.className + "\nThe one definition rule is violated, different classes/structs have the same name '$symbol'",
			    "ctuOneDefinitionRuleViolation", CWE_ONE_DEFINITION_RULE, Certainty::normal);
