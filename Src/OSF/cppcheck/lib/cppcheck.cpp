    ...
    std::function<bool(std::string, std::vector<std::string>, std::string, std::string*)> executeCommand)
{
	...
	if(!addonInfo.executable.empty())
		...
		for(const char* py_exe : py_exes) {
			std::string out;
			if(executeCommand(py_exe, split("--version"), redirect,
			    &out) && out.compare(0, 7, "Python ") == 0 && std::isdigit(out[7])) {
				pythonExe = py_exe;
				break;
			}
		}
...
unsigned int CppCheck::check(const std::string &path)
{
	if(mSettings.clang) {
		...
		try {
			...
			ValueFlow::setValues(&tokenizer.list, const_cast<SymbolDatabase *>(tokenizer.getSymbolDatabase()), this,
			    &mSettings);
			...
			if(fdump.is_open()) {
				...
				for(const ErrorMessage& errmsg: compilerWarnings)
					fdump << "  <clang-warning file=\"" << toxml(errmsg.callStack.front().getfile()) << "\" line=\"" <<
					    errmsg.callStack.front().line << "\" column=\"" << errmsg.callStack.front().column <<
					    "\" message=\"" << toxml(errmsg.shortMessage()) << "\"/>\n";
...
unsigned int CppCheck::check(const ImportProject::FileSettings &fs)
{
	...
	if(mSettings.clang) {
		temp.mSettings.includePaths.insert(temp.mSettings.includePaths.end(),
		    fs.systemIncludePaths.cbegin(),
		    fs.systemIncludePaths.cend());
...
unsigned int CppCheck::checkFile(const std::string& filename, const std::string &cfgname, std::istream& fileStream)
{
	...
	try {
		...
		if(!mSettings.plistOutput.empty()) {
			...
			filename2 = mSettings.plistOutput +
			    filename2.substr(0, filename2.find('.')) + "_" + std::to_string(fileNameHash) + ".plist";
		...
		if(mSettings.dump || !mSettings.addons.empty()) {
			...
			for(unsigned int i = 0; i < files.size(); ++i)
				dumpProlog << "    <file index=\"" << i << "\" name=\"" << ErrorLogger::toxml(files[i]) << "\"/>" <<
				    std::endl;
		...
		for(const std::string &currCfg : configurations) {
			...
			try {
				...
				configurationError.push_back(
					(mCurrentConfig.empty() ? "\'\'" : mCurrentConfig) + " : [" + o.location.file() + ':' +
					MathLib::toString(o.location.line) + "] " + o.msg);
...
static const char * pcreErrorCodeToString(const int pcreExecRet)
{
	switch(pcreExecRet) {
		...
		case PCRE_ERROR_BADLENGTH:
		    return
			    "This error is given if pcre_exec() is called with a negative value for the length argument (PCRE_ERROR_BADLENGTH)";
...
void CppCheck::analyseClangTidy(const ImportProject::FileSettings &fileSettings)
{
	...
	const std::string args = "-quiet -checks=*,-clang-analyzer-*,-llvm* \"" + fileSettings.filename + "\" -- " + allIncludes +
	    allDefines;
	std::string output;
