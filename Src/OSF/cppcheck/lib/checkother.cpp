...
static const struct CWE CWE362(362U);   // Concurrent Execution using Shared Resource with Improper Synchronization
                                        // ('Race Condition')
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			    ...
			    "EOF %comp% ( %var% = fclose|fflush|fputc|fputs|fscanf|getchar|getc|fgetc|putchar|putc|puts|scanf|sscanf|ungetc ("))
			{
			    ...
			    (Token::Match(tok,
			    "EOF %comp% ( %var% = std :: cin . get (") || Token::Match(tok, "EOF %comp% ( %var% = cin . get ("))) {
				tok = tok->tokAt(3);
				const Variable * var = tok->variable();
				if(var && var->typeEndToken()->str() == "char" && !var->typeEndToken()->isSigned()) {
			    ...
			    (Token::Match(tok, "%var% = std :: cin . get (") || Token::Match(tok, "%var% = cin . get ("))) {
				const Variable * var = tok->variable();
				if(var && var->typeEndToken()->str() == "char" && !var->typeEndToken()->isSigned()) {
...
void CheckOther::clarifyCalculation()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(tok->astOperand1()->tokType() == Token::eBitOp &&
			    Token::Match(tok->astOperand2()->astOperand1(),
			    "%char%") && Token::Match(tok->astOperand2()->astOperand2(), "%char%"))
				continue;
...
void CheckOther::warningOldStylePointerCast()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(; tok && tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(!Token::Match(tok,
			    "( const|volatile| const|volatile|class|struct| %type% * *| *| const|&| ) (| %name%|%num%|%bool%|%char%|%str%"))
				continue;
...
void CheckOther::invalidPointerCast()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(fromType->type != toType->type && fromType->type >= ValueType::Type::BOOL &&
			    toType->type >= ValueType::Type::BOOL && (toType->type != ValueType::Type::CHAR || printInconclusive)) {
				if(toType->isIntegral() && fromType->isIntegral())
					continue;

				invalidPointerCastError(tok,
				    fromType->str(),
				    toType->str(),
				    toType->type == ValueType::Type::CHAR,
				    toType->isIntegral());
			}
		}
	}
...
void CheckOther::invalidPointerCastError(const Token* tok, const std::string& from, const std::string& to, bool inconclusive, bool toIsInt)
{
	if(toIsInt) { // If we cast something to int*, this can be useful to play with its binary data representation
		reportError(tok,
		    ...
		    "invalidPointerCast",
		    "Casting from " + from + " to " + to + " is not portable due to different binary data representations on different platforms.",
		    CWE704,
		...
		reportError(tok,
		    Severity::portability,
		    "invalidPointerCast",
		    "Casting between " + from + " and " + to + " which have an incompatible binary data representation.",
		    CWE704,
		    Certainty::normal);
}
	...
	for(const Scope &switchScope : symbolDatabase->scopeList) {
		...
		for(const Token * tok2 = switchScope.bodyStart->next(); tok2 != switchScope.bodyEnd; tok2 = tok2->next()) {
			if(tok2->str() == "{") {
				...
				//   case 3: b = 1;
				//   case 4: if (a) { b = 2; }    // Doesn't make the b=1 redundant because it's
				// conditional
			...
			//    case 4: b |= 1;
				...
				// This variable has not had a bit operation performed on it yet, so just make a note of
				// it
				...
				// The same bit operation has been performed on the same variable twice, so report an
				// error
			...
			// Not a simple assignment so there may be good reason if this variable is assigned to twice.
			// E.g.:
			//    case 3: b = 1;
			//    case 4: b++;
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(tok->link() && Token::Match(tok, "(|[|<"))
			    ...
			    "%name% (") && mSettings->library.isnoreturn(tok) && !Token::Match(tok->next()->astParent(), "?|:")) {
			...
			if(secondBreak && (printInconclusive || !inconclusive)) {
				if(Token::Match(secondBreak, "continue|goto|throw|return") && secondBreak->isKeyword()) {
				...
				else if(secondBreak->str() == "break") { // break inside switch as second break
					                                 // statement should not issue a warning
					if(tok->str() == "break") // If the previous was a break, too: Issue warning
						duplicateBreakError(secondBreak, inconclusive);
					else {
						if(tok->scope()->type != Scope::eSwitch) // Check, if the enclosing
							                                 // scope is a switch
							duplicateBreakError(secondBreak, inconclusive);
					}
					tok = Token::findmatch(secondBreak, "[}:]");
				}
				else if(!Token::Match(secondBreak, "return|}|case|default") && secondBreak->strAt(1) != ":") { // TODO:
					                                                                                       // No
					                                                                                       // bailout
					                                                                                       // for
					                                                                                       // unconditional
					                                                                                       // scopes
					// If the goto label is followed by a loop construct in which the label is
					// defined it's quite likely
					// that the goto jump was intended to skip some code on the first loop
					// iteration.
					...
					if(labelName && Token::Match(secondBreak, "while|do|for")) {
						...
						if(scope2) {
							for(const Token * tokIter = scope2;
							    tokIter != scope2->link() && tokIter;
							    tokIter = tokIter->next()) {
								if(Token::Match(tokIter,
								    "[;{}] %any% :") && labelName->str() == tokIter->strAt(1)) {
					...
					// hide FP for statements that just hide compiler warnings about unused function
					// arguments
...
void CheckOther::checkVariableScope()
{
	...
	for(const Variable* var : symbolDatabase->variableList()) {
		...
		const bool isSimpleType = var->typeStartToken()->isStandardType() || var->typeStartToken()->isEnumType() ||
		    (mTokenizer->isC() && var->type() && var->type()->isStructType());
		...
		auto isSimpleExpr = [](const Token* tok) {
			    return tok &&
				   (tok->isNumber() || tok->tokType() == Token::eString || tok->tokType() == Token::eChar ||
				   tok->isBoolean());
		    };
		...
		for(; tok && tok != var->scope()->bodyEnd; tok = tok->next()) {
			if(tok->str() == "{" && tok->scope() != tok->previous()->scope() && !tok->isExpandedMacro() &&
			    !isWithinScope(tok, var, Scope::ScopeType::eLambda)) {
				if(used) {
					bool used2 = false;
...
bool CheckOther::checkInnerScope(const Token * tok, const Variable* var, bool& used)
{
	...
	if(scope->type == Scope::eUnconditional && (tok->strAt(-1) == ")" || tok->previous()->isName())) // Might be an
		                                                                                         // unknown
		                                                                                         // macro like
		                                                                                         // BOOST_FOREACH
		loopVariable = true;
	...
	for(; tok && tok != end; tok = tok->next()) {
		...
		if(loopVariable && noContinue && tok->scope() == scope && !forHeadEnd && scope->type != Scope::eSwitch &&
		    Token::Match(tok, "%varid% =", var->declarationId())) {                                                                                                // Assigned
			                                                                                                                                                   // in
			                                                                                                                                                   // outer
			                                                                                                                                                   // scope.
			loopVariable = false;
			int indent = 0;
			for(const Token* tok2 = tok->tokAt(2); tok2; tok2 = tok2->next()) { // Ensure that variable
				                                                            // isn't used on right side
				                                                            // of =, too
				if(tok2->str() == "(")
					indent++;
				else if(tok2->str() == ")") {
		...
		if(!bFirstAssignment && Token::Match(tok, "* %varid%", var->declarationId())) // dereferencing means
			                                                                      // access to previous
			                                                                      // content
			return false;

		if(Token::Match(tok, "= %varid%",
		    var->declarationId()) && (var->isArray() || var->isPointer() || (var->valueType() && var->valueType()->container)))                        // Create
			                                                                                                                                       // a
			                                                                                                                                       // copy
			                                                                                                                                       // of
			                                                                                                                                       // array/pointer.
		...
		if(tok->varId() == var->declarationId()) {
			...
			if(scope == tok->scope()) {
				if(scope->type == Scope::eSwitch)
					return false; // Used in outer switch scope - unsafe or impossible to reduce
				                      // scope
...
{
	{
		...
		if(func_scope->type == Function::Type::eConstructor) {
			...
			if(func_scope->arg->link()->next()->str() == ":") {
				for(const Token* tok2 = func_scope->arg->link()->next()->next();
				    tok2 != var->scope()->bodyStart;
				    tok2 = tok2->next()) {
...
void CheckOther::checkPassByReference()
{
	...
	for(const Variable* var : symbolDatabase->variableList()) {
		if(!var || !var->isArgument() || !var->isClass() || var->isPointer() || var->isArray() || var->isReference() ||
		    var->isEnumType())
			continue;
		...
		if((var->declEndToken() && var->declEndToken()->isExternC()) ||
		    (var->scope() && var->scope()->function && var->scope()->function->tokenDef &&
		    var->scope()->function->tokenDef->isExternC()))
			continue; // references cannot be used in functions in extern "C" blocks

		bool inconclusive = false;

		const bool isContainer = var->valueType() && var->valueType()->type == ValueType::Type::CONTAINER &&
		    var->valueType()->container && !var->valueType()->container->view;
		if(!isContainer) {
	...
	for(const Variable * var : symbolDatabase->variableList()) {
		...
		if(function && var->isArgument()) {
			...
			if(function->isConstructor() &&
			    isVariableMutableInInitializer(function->constructorMemberInitialization(), scope->bodyStart,
			    var->declarationId()))
				continue;
		}
		if(var->isGlobal())
		...
		if(var->isReference()) {
			...
			for(const Token* tok = var->nameToken(); tok != scope->bodyEnd && tok != nullptr; tok = tok->next()) {
				if(tok->variable() == var) {
					...
					if(var->isStlType() && Token::Match(tok, "%var% [")) { // containers whose
						                                               // operator[] is
						                                               // non-const
...
void CheckOther::checkConstPointer()
{
	...
	for(const Variable * p: pointers) {
		if(p->isArgument()) {
			if(!p->scope() || !p->scope()->function || p->scope()->function->isImplicitlyVirtual(true) ||
			    p->scope()->function->hasVirtualSpecifier())
				continue;
		}
		if(std::find(nonConstPointers.begin(), nonConstPointers.end(), p) == nonConstPointers.end()) {
			...
			if(p->isArgument() && p->typeStartToken() && p->typeStartToken()->isSimplifiedTypedef() &&
			    !(Token::simpleMatch(p->typeEndToken(), "*") && !p->typeEndToken()->isSimplifiedTypedef()))
				continue;
...
void CheckOther::constVariableError(const Variable * var, const Function * function)
{
	...
	if(var && var->isArgument() && function && function->functionPointerUsage) {
		...
		id += "Callback";
		message += ". However it seems that '" + function->name() +
...
static bool isConstStatement(const Token * tok, bool cpp)
{
	...
	if(isCPPCast(tok)) {
		if(Token::simpleMatch(tok->astOperand1(),
		    "dynamic_cast") && Token::simpleMatch(tok->astOperand1()->linkAt(1)->previous(), "& >"))
			return false;
	...
	if(Token::simpleMatch(tok, ",")) {
		if(tok->astParent()) // warn about const statement on rhs at the top level
			...
			const Token* lml = previousBeforeAstLeftmostLeaf(tok); // don't warn about matrix/vector
			                                                       // assignment (e.g. Eigen)
			if(lml)
				lml = lml->next();
	...
	if(Token::simpleMatch(tok, "?") && Token::simpleMatch(tok->astOperand2(), ":")) // ternary operator
		return isConstStatement(tok->astOperand1(),
			   cpp) && isConstStatement(tok->astOperand2()->astOperand1(), cpp) && isConstStatement(
...
void CheckOther::checkIncompleteStatement()
{
	...
	for(const Token * tok = mTokenizer->tokens(); tok; tok = tok->next()) {
		...
		if(isVoidStmt(tok))
			continue;
		if(mTokenizer->isCPP() && tok->str() == "&" &&
		    !(tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->isIntegral()))
			// Possible archive
			continue;
...
void CheckOther::zerodivError(const Token * tok, const ValueFlow::Value * value)
{
	if(!tok && !value) {
		...
		reportError(tok,
		    Severity::error,
		    "zerodivcond",
		    ValueFlow::eitherTheConditionIsRedundant(nullptr) + " or there is division by zero.",
		    CWE369,
		    Certainty::normal);
		return;
	}
...
void CheckOther::checkMisusedScopedObject()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
			if((tok->next()->type() || (tok->next()->function() && tok->next()->function()->isConstructor())) // TODO:
			                                                                                                  // The
			                                                                                                  // rhs
			                                                                                                  // of
			                                                                                                  // ||
	...
	for(const Scope & scope : symbolDatabase->scopeList) {
		...
		if(Token::simpleMatch(scope.bodyEnd, "} else {")) {
			...
			if(branchTop1->str() != branchTop2->str())
				continue;
			if(isSameExpression(mTokenizer->isCPP(), false, branchTop1->astOperand1(), branchTop2->astOperand1(),
			    mSettings->library, true, true, &errorPath) &&
			    isSameExpression(mTokenizer->isCPP(), false, branchTop1->astOperand2(), branchTop2->astOperand2(),
			    mSettings->library, true, true, &errorPath))
				duplicateBranchError(scope.classDef, scope.bodyEnd->next(), errorPath);
		}
	}
...
void CheckOther::invalidFreeError(const Token * tok, const std::string &allocation, bool inconclusive)
{
	...
	reportError(tok,
	    Severity::error,
	    "invalidFree",
	    "Mismatching address is " + deallocated + ". The address you get from " + alloc + " must be " + deallocated + " without offset.",
	    CWE(0U),
	    inconclusive ? Certainty::inconclusive : Certainty::normal);
}
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			if(tok->str() == "=" && Token::Match(tok->astOperand1(), "%var%")) {
				...
				if(Token::Match(endStatement, "%var% %assign%")) {
					...
					if(var1 && var2 &&
					    ...
					    isSameExpression(mTokenizer->isCPP(), true, tok->next(), nextAssign->next(), mSettings->library,
					    true, false) &&
					    isSameExpression(mTokenizer->isCPP(), true, tok->astOperand2(), nextAssign->astOperand2(),
					    mSettings->library, true, false) &&
						...
						for(const Token * assignTok = Token::findsimplematch(var2, ";");
						    assignTok && assignTok != varScope->bodyEnd;
						    assignTok = assignTok->next()) {
			...
			if(tok->isOp() && tok->astOperand1() && !Token::Match(tok, "+|*|<<|>>|+=|*=|<<=|>>=")) {
				...
				const bool followVar =
				    (!isConstVarExpression(tok) || Token::Match(tok, "%comp%|%oror%|&&")) && !pointerDereference;
				if(isSameExpression(mTokenizer->isCPP(),
						...
						if(assignment && warningEnabled)
							...
							if(mTokenizer->isCPP() && mSettings->standards.cpp >= Standards::CPP11 &&
								...
								if(parent && parent->previous() &&
								    parent->previous()->str() == "static_assert") {
									continue;
								}
							}
					...
					if(styleEnabled && tok->astOperand2() && tok->str() == tok->astOperand1()->str() &&
						...
						duplicateExpressionError(tok->astOperand2(),
						    tok->astOperand1()->astOperand2(),
						    tok,
						    errorPath);
					else if(tok->astOperand2() &&
					    isConstExpression(tok->astOperand1(), mSettings->library, true, mTokenizer->isCPP())) {
						auto checkDuplicate = [&](const Token* exp1, const Token* exp2, const Token* ast1) {
							    if(isSameExpression(mTokenizer->isCPP(), true, exp1, exp2, mSettings->library,
								true, true, &errorPath) &&
								isWithoutSideEffects(mTokenizer->isCPP(), exp1) &&
								isWithoutSideEffects(mTokenizer->isCPP(), ast1->astOperand2()))
								    duplicateExpressionError(exp1,
									exp2,
									tok,
									errorPath,
								        /*hasMultipleExpr*/ true);
						    };
						const Token * ast1 = tok->astOperand1();
						while(ast1 && tok->str() == ast1->str()) { // chain of identical
							                                   // operators
							checkDuplicate(ast1->astOperand2(), tok->astOperand2(), ast1);
							if(ast1->astOperand1() && ast1->astOperand1()->str() != tok->str()) // check
								                                                            // first
								                                                            // condition
								                                                            // in
								                                                            // the
			...
			else if(styleEnabled && tok->astOperand1() && tok->astOperand2() && tok->str() == ":" && tok->astParent() &&
			    tok->astParent()->str() == "?") {
				if(!tok->astOperand1()->values().empty() && !tok->astOperand2()->values().empty() &&
				    isEqualKnownValue(tok->astOperand1(), tok->astOperand2()) &&
				    !isVariableChanged(tok->astParent(), /*indirect*/ 0, mSettings, mTokenizer->isCPP()) &&
				    isConstStatement(tok->astOperand1(),
				    mTokenizer->isCPP()) && isConstStatement(tok->astOperand2(), mTokenizer->isCPP()))
					duplicateValueTernaryError(tok);
				else if(isSameExpression(mTokenizer->isCPP(), true, tok->astOperand1(), tok->astOperand2(),
				    mSettings->library, false, true, &errorPath))
    ...
    bool hasMultipleExpr)
{
	...
	const std::string& op = opTok ? opTok->str() : "&&";
	std::string msg = "Same expression " +
	    (hasMultipleExpr ? "\'" + expr1 + "\'" + " found multiple times in chain of \'" + op + "\' operators" : "on both sides of \'" +
	    op + "\'");
	...
	reportError(errors, Severity::style, id, msg +
	    ...
	    (hasMultipleExpr ? "more than once in a condition" : "on both sides of an operator")) +
...
void CheckOther::checkComparisonFunctionIsAlwaysTrueOrFalse()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			if(tok->isName() &&
			    Token::Match(tok, "isgreater|isless|islessgreater|isgreaterequal|islessequal ( %var% , %var% )")) {
...
void CheckOther::checkRedundantCopy()
{
	...
	for(const Variable* var : symbolDatabase->variableList()) {
		if(!var || var->isReference() || !var->isConst() || var->isPointer() || (!var->type() && !var->isStlType())) // bailout
			                                                                                                     // if
			                                                                                                     // var
			                                                                                                     // is
			                                                                                                     // of
		...
		if(Token::simpleMatch(dot,
		    ".") && dot->astOperand1() && isVariableChanged(dot->astOperand1()->variable(), mSettings, mTokenizer->isCPP()))
			continue;
		...
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			if(Token::Match(tok, "memset|memcpy|memmove ( %var% ,") && Token::Match(tok->linkAt(1)->tokAt(-2), ", %num% )")) {
				...
				if(MathLib::toLongNumber(tok->linkAt(1)->strAt(-1)) == var->dimension(0)) {
					...
					else if(var->valueType()->type == ValueType::Type::BOOL && printPortability) // sizeof(bool)
						                                                                     // is
						                                                                     // not
						                                                                     // 1
						                                                                     // on
		...
		for(const Token* tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
			...
			for(const Token * tok2 = tok;; tok2 = tok2->astParent()) {
				...
				if(tok2 == tok &&
				    tok->str() == "=" &&
				    parent->str() == "=" &&
				    isSameExpression(mTokenizer->isCPP(), false, tok->astOperand1(), parent->astOperand1(),
				    mSettings->library, true, false)) {
					if(mSettings->severity.isEnabled(Severity::warning) &&
					    isSameExpression(mTokenizer->isCPP(), true, tok->astOperand1(), parent->astOperand1(),
					    mSettings->library, true, false))
...
void CheckOther::checkFuncArgNamesDifferent()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		if(warning) {
			...
			if(order_different) {
				funcArgOrderDifferent(function->name(),
				    function->argDef->next(),
				    function->arg->next(),
				    declarations,
				    definitions);
				continue;
...
void CheckOther::checkShadowVariables()
{
	...
	for(const Scope & scope : symbolDatabase->scopeList) {
		if(!scope.isExecutable() || scope.type == Scope::eLambda)
			continue;
		const Scope * functionScope = &scope;
		while(functionScope && functionScope->type != Scope::ScopeType::eFunction &&
		    functionScope->type != Scope::ScopeType::eLambda)
			functionScope = functionScope->nestedIn;
		for(const Variable &var : scope.varlist) {
			...
			if(functionScope->functionOf && functionScope->functionOf->isClassOrStructOrUnion() && functionScope->function &&
			    functionScope->function->isStatic() &&
		...
		for(const Token * tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
			...
			auto setVarExpr = [&varexpr, &isVariableExprHidden](const Token * child) {
				    if(Token::Match(child, "%var%|.|[")) {
					    if(child->valueType() && child->valueType()->pointer == 0 && child->valueType()->isIntegral() &&
						child->values().empty()) {
				    ...
				    if(!isVariableExprHidden) {
						...
						(Token::simpleMatch(child->astOperand1(), "0") || Token::simpleMatch(child->astOperand2(), "0")))
						    return ChildrenToVisit::none;
...
void CheckOther::comparePointersError(const Token * tok, const ValueFlow::Value * v1, const ValueFlow::Value * v2)
{
	...
	reportError(
		errorPath, Severity::error, "comparePointers", verb + " pointers that point to different objects", CWE570,
		Certainty::normal);
}
			...
			if(tok->isAssignmentOp()) {
				...
				if(nonOverlappingData->sizeArg <= 0 || nonOverlappingData->sizeArg > args.size()) {
					if(nonOverlappingData->sizeArg == -1) {
						...
						if(!isSameExpression(mTokenizer->isCPP(), macro, ptr1, ptr2, mSettings->library, pure,
						    follow, &errorPath))
							continue;
