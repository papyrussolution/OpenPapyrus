...
public:
	...
	public:
		...
		template <typename T> Error(ErrorCode e, T&& r) : errorcode(e), reason(r) 
		{
		}
		ErrorCode errorcode;
		std::string reason;
	...
	void setnoreturn(const std::string& funcname, bool noreturn) { mNoReturn[funcname] = noreturn ? FalseTrueMaybe::True : FalseTrueMaybe::False; }
...
public:
		...
		enum class Direction {
			...
			DIR_OUT, ///< Output to caller. Data is passed by reference or address and is potentially
			         // written.
			DIR_INOUT, ///< Input to called function, and output to caller. Data is passed by reference or
			           // address and is potentially modified.
	...
	struct PlatformType {
		PlatformType() : mSigned(false), mUnsigned(false), mLong(false), mPointer(false), mPtrPtr(false), mConstPtr(false) 
		{
		}
		bool operator ==(const PlatformType & type) const 
		{
			return (mSigned == type.mSigned && mUnsigned == type.mUnsigned && mLong == type.mLong && mPointer == type.mPointer && mPtrPtr == type.mPtrPtr &&
			       mConstPtr == type.mConstPtr && mType == type.mType);
		}
