    ...
    nonneg int recursiveCount)
{
	...
	if(++recursiveCount > recursiveLimit) // maximum number of "else if ()"
		throw InternalError(startToken,
	...
	for(const Token * tok = startToken; tok && tok != endToken; tok = tok->next()) {
		...
		if(const Token* const tokAssignOp = isAssignment(varTok)) {
			...
			while(tokRightAstOperand && tokRightAstOperand->isCast())
				tokRightAstOperand =
				    tokRightAstOperand->astOperand2() ? tokRightAstOperand->astOperand2() : tokRightAstOperand->astOperand1();
			...
			if(Token::Match(fTok, "%type% (")) {
				...
				const bool arrayNew =
				    (tok2 &&
				    (tok2->str() == "[" ||
				    (tok2->str() == "(" && tok2->astOperand1() && tok2->astOperand1()->str() == "[")));
				VarInfo::AllocInfo& varAlloc = alloctype[varTok->varId()];
		...
		// if/else
			...
			for(const Token * innerTok = tok->tokAt(2); innerTok && innerTok != closingParenthesis;
			    innerTok = innerTok->next()) {
				// TODO: replace with checkTokenInsideExpression()

				if(!isLocalVarNoAutoDealloc(innerTok, mTokenizer->isCPP()))
					continue;

				// Check assignments in the if-statement. Skip multiple assignments since we don't track
				// those
				if(Token::Match(innerTok, "%var% =") && innerTok->astParent() == innerTok->next() &&
					...
					while(tokRightAstOperand && tokRightAstOperand->isCast())
						tokRightAstOperand =
						    tokRightAstOperand->astOperand2() ? tokRightAstOperand->astOperand2() :
						    tokRightAstOperand
						    ->astOperand1();
					if(tokRightAstOperand && Token::Match(tokRightAstOperand->previous(), "%type% (")) {
						const Library::AllocFunc* f = mSettings->library.getAllocFuncInfo(
							tokRightAstOperand->previous());
						if(f && f->arg == -1) {
...
const Token * CheckLeakAutoVar::checkTokenInsideExpression(const Token * const tok, VarInfo * varInfo)
{
	...
	if(tok->varId() > 0) {
		...
		if(var != varInfo->alloctype.end()) {
			bool unknown = false;
			if(var->second.status == VarInfo::DEALLOC &&
			    CheckNullPointer::isPointerDeRef(tok, unknown, mSettings) && !unknown) {
...
void CheckLeakAutoVar::changeAllocStatus(VarInfo * varInfo, const VarInfo::AllocInfo& allocation, const Token* tok, const Token* arg)
{
	...
	if(var != alloctype.end()) {
	    ...
	    !Token::simpleMatch(tok->astTop(), "return")) {
    ...
    const Library::AllocFunc* af)
{
	...
	for(const Token * funcArg = tokFirstArg; funcArg; funcArg = funcArg->nextArgument()) {
		...
		// Check smart pointer
			...
			const VarInfo::AllocInfo sp_allocation(sp_af ? sp_af->groupId : (arrayDelete ? NEW_ARRAY : NEW),
			    VarInfo::OWNED,
			    allocTok);
