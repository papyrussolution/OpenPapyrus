...
struct ScopeInfo2 {
	ScopeInfo2(const std::string &name_, const Token * bodyEnd_,
	    const std::set<std::string> &usingNamespaces_ = std::set<std::string>()) : name(name_), bodyEnd(bodyEnd_), usingNamespaces(
			usingNamespaces_) {
	}
 ...
 * The reason the Token class is needed (instead of using the string class) is that some extra functionality is also
 *needed for tokens:
...
public:
	...
	std::string stringifyList(const stringifyOptions& options,
...
private:
	...
	enum : uint64_t {
		...
		fIsCast                 = (1 << 6),
		fIsAttributeConstructor = (1 << 7), // __attribute__((constructor))
		...
		fConstexpr              = (1 << 27),
		fExternC                = (1 << 28),
		fIsSplitVarDeclComma    = (1 << 29),// set to true when variable declarations are split up ('int a,b;'
		                                  // => 'int a; int b;')
		fIsSplitVarDeclEq       = (1 << 30),// set to true when variable declaration with initialization is
		                               // split up ('int a=5;' => 'int a; a=5;')
