		...
		if(Token::Match(parent, "+|==|!=") && parent->astOperand1() && parent->astOperand2()) {
			for(const ValueFlow::Value &value1 : parent->astOperand1()->values()) {
				...
				for(const ValueFlow::Value &value2 : parent->astOperand2()->values()) {
					...
					else if(value1.isContainerSizeValue() && value2.isTokValue() &&
					    value2.tokvalue->tokType() == Token::eString)
						result.intvalue =
						    calculate(parent->str(), value1.intvalue,
							MathLib::bigint(Token::getStrLength(value2.tokvalue)));
					else if(value2.isContainerSizeValue() && value1.isTokValue() &&
					    value1.tokvalue->tokType() == Token::eString)
						result.intvalue =
						    calculate(parent->str(),
							MathLib::bigint(Token::getStrLength(value1.tokvalue)),
							value2.intvalue);
					else
						continue;
		...
		for(const ValueFlow::Value &value1 : parent->astOperand1()->values()) {
			...
			for(const ValueFlow::Value &value2 : parent->astOperand2()->values()) {
				...
				if(Token::Match(parent, "==|!=")) {
					...
					if(error)
						continue;
					// If the bound comes from the second value then invert the bound when
					// subtracting
...
static Token * valueFlowSetConstantValue(Token * tok, const Settings * settings, bool cpp)
{
	if((tok->isNumber() && MathLib::isInt(tok->str())) || (tok->tokType() == Token::eChar)) {
		...
		if(tok->next()->astOperand2() && !tok->next()->astOperand2()->isLiteral() && tok->next()->astOperand2()->valueType() &&
		    tok->next()->astOperand2()->valueType()->pointer == 0 && // <- TODO this is a bailout, abort when
		                                                             // there are array->pointer conversions
		    !tok->next()->astOperand2()->valueType()->isEnum()) { // <- TODO this is a bailout, handle enum with
			                                                  // non-int types
...
static void valueFlowSameExpressions(TokenList * tokenlist)
{
	for(Token * tok = tokenlist->front(); tok; tok = tok->next()) {
		...
		if(isSameExpression(tokenlist->isCPP(), false, tok->astOperand1(), tok->astOperand2(), tokenlist->getSettings()->library,
		    true, true, &val.errorPath)) {
    ...
    int depth = 20)
{
	...
	if(var && var->declarationId() == tok->varId()) {
		...
		if(f) {
				...
				for(LifetimeToken& lt : getLifetimeTokens(returnTok, escape, errorPath, pred, depth - returns.size())) {
					...
					if(argvar->isArgument() && (argvar->isReference() || argvar->isRValueReference())) {
						...
						std::vector<LifetimeToken> arglts = LifetimeToken::setInconclusive(
							getLifetimeTokens(argTok, escape, std::move(lt.errorPath), pred,
							depth - returns.size()),
							returns.size() > 1);
		    ...
		    ". %name% (") && tok->tokAt(-2)->originalName() != "->" && astIsContainer(tok->tokAt(-2)->astOperand1())) {
...
struct LifetimeStore {
	...
	static LifetimeStore fromFunctionArg(const Function * f,
	    Token * tok,
	    const Variable * var,
	    TokenList * tokenlist,
	    ErrorLogger * errorLogger) {
...
static void valueFlowLifetimeFunction(Token * tok, TokenList * tokenlist, ErrorLogger * errorLogger, const Settings * settings)
{
	...
	if(returnContainer >= 0) {
		...
		for(const Token* returnTok : returns) {
			...
			if(returnVar && returnVar->isArgument() &&
			    (returnVar->isConst() || !isVariableChanged(returnVar, settings, tokenlist->isCPP()))) {
	...
	for(Token * tok = tokenlist->front(); tok; tok = tok->next()) {
		...
		if(lam.isLambda()) {
			...
			auto captureVariable = [&](const Token* tok2, LifetimeCapture c, std::function<bool(const Token*)> pred) {
				    ...
				    if(c == LifetimeCapture::ByReference) {
					    LifetimeStore ls{
						    tok2, "Lambda captures variable by reference here.",
...
static bool isStdMoveOrStdForwarded(Token * tok, ValueFlow::Value::MoveKind * moveKind, Token ** varTok = nullptr)
{
	...
	if(variableToken->strAt(2) == ".") // Only partially moved
		return false;
	if(variableToken->valueType() && variableToken->valueType()->type >= ValueType::Type::VOID_T)
		return false;
	if(moveKind != nullptr)
		*moveKind = kind;
	if(varTok != nullptr)
		...
		for(Token* tok = const_cast<Token*>(start); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok, "%var% . reset|clear (") && tok->next()->originalName().empty()) {
				...
				value.errorPath.emplace_back(tok,
				    "Calling " + tok->next()->expressionString() + " makes " + tok->str() + " 'non-moved'");
				value.setKnown();
				std::list<ValueFlow::Value> values;
			...
			if(parent &&
			    ...
			    parent->str() == "(")) // MOVED in self assignment,
				                   // isOpenParenthesisMemberFunctionCallOfVarId == true
				continue;
...
//
static void valueFlowConditionExpressions(TokenList * tokenlist,
		...
		for(const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(Token::simpleMatch(startTok->link(), "} else {")) {
				...
				for(const Token* condTok2:conds) {
					...
					if(is1) {
						OppositeExpressionAnalyzer a2(true,
						    condTok2,
						    makeConditionValue(1, condTok2, false),
						    tokenlist);
						valueFlowGenericForward(startTok, startTok->link(), a2, settings);
					}
				}
    ...
    const Settings* const settings)
{
	...
	if(tok->astParent()) {
		for(ValueFlow::Value& value : values) {
			...
			const std::string info = "Assignment '" + tok->astParent()->expressionString() + "', assigned value is " +
			    valueKind + value.infoString();
			value.errorPath.emplace_back(tok, info);
		}
	}
...
static bool isVariableInit(const Token * tok)
{
	return (tok->str() == "(" || tok->str() == "{") &&
	       ...
	       tok->astOperand1()->variable()->valueType()->type >= ValueType::Type::VOID_T &&
    ...
    const Settings * settings)
{
	for(const Scope * scope : symboldatabase->functionScopes) {
		...
		for(Token* tok = const_cast<Token*>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(values.empty())
				continue;
			const bool init = vars.size() == 1 &&
			    (vars.front()->nameToken() == tok->astOperand1() || tok->isSplittedVarDeclEq());
			...
			if(valueFlowForLoop2(tok, &mem1, &mem2, &memAfter)) {
				for(const auto& p : mem1) {
					...
					if(p.first.tok->varId() == 0)
						continue;
					valueFlowForLoopSimplify(bodyStart,
					    p.first.tok,
					    false,
					    p.second.intvalue,
					    tokenlist,
...
static void valueFlowSwitchVariable(TokenList * tokenlist,
    SymbolDatabase* symboldatabase,
    ErrorLogger * errorLogger,
    const Settings * settings)
{
		...
		for(Token * tok = scope.bodyStart->next(); tok != scope.bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok, "case %num% :")) {
				...
				const std::string info(
					"case " + tok->next()->str() + ": " + vartok->str() + " is " + tok->next()->str() + " here.");
				values.back().errorPath.emplace_back(tok, info);
				bool known = false;
				if((Token::simpleMatch(tok->previous(),
				    "{") || Token::simpleMatch(tok->tokAt(-2), "break ;")) && !Token::Match(tok->tokAt(3), ";| case"))
					known = true;
				while(Token::Match(tok->tokAt(3), ";| case %num% :")) {
					...
					const std::string info2(
						"case " + tok->next()->str() + ": " + vartok->str() + " is " + tok->next()->str() +
						" here.");
		...
		for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			for(int argnr = 0U; argnr < callArguments.size(); ++argnr) {
				...
				if(argtok->variable() && !argtok->variable()->isPointer() && argvalues.size() == 1 &&
					...
					if(CheckUninitVar::isVariableUsage(tokenlist->isCPP(), argtok, settings->library, false,
					    CheckUninitVar::Alloc::NO_ALLOC, 0))
						continue;
				}
...
static void valueFlowFunctionReturn(TokenList * tokenlist, ErrorLogger * errorLogger)
{
	for(Token * tok = tokenlist->back(); tok; tok = tok->previous()) {
		...
		if(!functionScope || !Token::simpleMatch(functionScope->bodyStart, "{ return")) {
			if(functionScope && tokenlist->getSettings()->debugwarnings &&
			    Token::findsimplematch(functionScope->bodyStart, "return", functionScope->bodyEnd))
    ...
    ValueFlow::Value& value)
{
	...
	// If its on the rhs, then -1 multiplication is needed, which is not possible with simple delta analysis used
	// currently for symbolic values
...
static void valueFlowDynamicBufferSize(TokenList* tokenlist, SymbolDatabase* symboldatabase, const Settings* settings)
{
		    ...
		    switch(allocFunc->bufferSize) {
				...
				if(arg1 && arg1->hasKnownValue()) {
					...
					if(value.isTokValue() && value.tokvalue->tokType() == Token::eString)
						sizeValue = Token::getStrLength(value.tokvalue) + 1; // Add one for the
				                                                                     // null terminator
				}
				break;
		    }
	...
	auto getBufferSizeFromNew = [&](const Token* newTok) -> MathLib::bigint {
		    ...
		    if(numElem >= 0 && newTok->astParent() && newTok->astParent()->isAssignmentOp()) { // size of the
			                                                                               // allocated type
			    const Token* typeTok = newTok->astParent()->astOperand1(); // TODO: implement fallback for
			                                                               // e.g. "auto p = new Type;"
			    if(!typeTok || !typeTok->varId())
				    typeTok = newTok->astParent()->previous(); // hack for "int** z = ..."
			    if(typeTok && typeTok->valueType()) {
				    const MathLib::bigint typeSize = typeTok->valueType()->typeSize(*settings,
					    typeTok->valueType()->pointer > 1);
				    if(typeSize >= 0)
	...
	for(const Scope * functionScope : symboldatabase->functionScopes) {
		for(const Token * tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
			...
			value.errorPath.emplace_back(tok->tokAt(2),
			    "Assign " + tok->strAt(1) + ", buffer with size " + MathLib::toString(sizeValue));
...
static bool getMinMaxValues(const ValueType * vt, const cppcheck::Platform &platform, MathLib::bigint * minValue,
	...
	for(const Scope * functionScope : symboldatabase->functionScopes) {
		...
		for(const Variable &arg : function->argumentList) {
			...
			if((!isLow || !isHigh) && all) {
				...
				else if(arg.valueType()->type == ValueType::Type::FLOAT ||
				    arg.valueType()->type == ValueType::Type::DOUBLE ||
				    arg.valueType()->type == ValueType::Type::LONGDOUBLE) {
					std::list<ValueFlow::Value> argValues;
					argValues.emplace_back(0);
					argValues.back().valueType = ValueFlow::Value::ValueType::FLOAT;
					argValues.back().floatValue = isLow ? low : -1E25f;
					argValues.back().errorPath.emplace_back(arg.nameToken(),
					    "Safe checks: Assuming argument has value " + MathLib::toString(argValues.back().floatValue));
					argValues.back().safe = true;
					argValues.emplace_back(0);
					argValues.back().valueType = ValueFlow::Value::ValueType::FLOAT;
			...
			if(isLow) {
				...
				argValues.back().errorPath.emplace_back(arg.nameToken(),
				    std::string(safeLow ? "Safe checks: " : "") + "Assuming argument has value " + MathLib::toString(low));
				argValues.back().safe = safeLow;
			}
			if(isHigh) {
				argValues.emplace_back(high);
				argValues.back().errorPath.emplace_back(arg.nameToken(),
				    std::string(safeHigh ? "Safe checks: " : "") + "Assuming argument has value " +
				    MathLib::toString(high));
