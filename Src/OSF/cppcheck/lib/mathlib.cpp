	...
	if(isCharLiteral(str)) {
		try {
			...
			throw InternalError(nullptr,
				  "Internal Error. MathLib::toLongNumber: characterLiteralToLL(" + str + ") => " + e.what());
		}
	}
	if(isIntHex(str))
...
bool MathLib::isDecimalFloat(const std::string &str)
{
	if(str.empty())
		return false;
	enum class State {
		START, BASE_DIGITS1, LEADING_DECIMAL, TRAILING_DECIMAL, BASE_DIGITS2, E, MANTISSA_PLUSMINUS, MANTISSA_DIGITS, SUFFIX_F,
		SUFFIX_L
	} state = State::START;
	...
	return (state==State::BASE_DIGITS2 || state==State::MANTISSA_DIGITS || state==State::TRAILING_DECIMAL || state==State::SUFFIX_F ||
	       state==State::SUFFIX_L);
}
...
std::string MathLib::incdec(const std::string & var, const std::string & op)
{
	...
	throw InternalError(nullptr,
		  std::string("Unexpected operation '") + op + "' in MathLib::incdec(). Please report this to Cppcheck developers.");
}
