...
void CheckIO::checkFileUsage()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			if(tok->str() == "{")
			...
			else if(tok->str() == "return" || tok->str() == "continue" || tok->str() == "break" ||
			    mSettings->library.isnoreturn(tok)) {                                                                           // Reset
				                                                                                                            // upon
				                                                                                                            // return,
				                                                                                                            // continue
				                                                                                                            // or
			    ...
			    "%name% (") && tok->previous() &&
			    (!tok->previous()->isName() || Token::Match(tok->previous(), "return|throw"))) {
				std::string mode;
				...
				if((tok->str() == "fopen" || tok->str() == "freopen" || tok->str() == "tmpfile" ||
				...
				else if((tok->str() == "rewind" || tok->str() == "fseek" || tok->str() == "fsetpos" ||
				    tok->str() == "fflush") ||
				    ...
				    "if|for|while|catch|switch") && !mSettings->library.isFunctionConst(tok->str(), true)) {
					const Token* const end2 = tok->linkAt(1);
					if(scope->functionOf && scope->functionOf->isClassOrStruct() && !scope->function->isStatic() &&
					    ((tok->strAt(-1) != "::" && tok->strAt(-1) != ".") || tok->strAt(-2) == "this")) {
						if(!tok->function() ||
						    (tok->function()->nestedIn && tok->function()->nestedIn->isClassOrStruct())) {
							for(std::pair<const int, Filepointer>& filepointer : filepointers) {
								const Variable* var =
								    symbolDatabase->getVariableFromVarId(filepointer.first);
								if(!var || !(var->isLocal() || var->isGlobal() || var->isStatic())) {
									filepointer.second.mode = OpenMode::UNKNOWN_OM;
									filepointer.second.mode_indent = 0;
									filepointer.second.op_indent = indent;
									filepointer.second.lastOperation =
									    Filepointer::Operation::UNKNOWN_OP;
								}
							}
							continue;
						}
					...
					for(const Token* tok2 = tok->tokAt(2); tok2 != end2; tok2 = tok2->next()) {
						if(tok2->varId() && filepointers.find(tok2->varId()) != filepointers.end()) {
							fileTok = tok2;
							operation = Filepointer::Operation::UNKNOWN_OP; // Assume that
							                                                // repositioning
							                                                // was last
							                                                // operation and
							                                                // that the file
				...
				if(filepointers.find(fileTok->varId()) == filepointers.end()) { // function call
				...
				switch(operation) {
					case Filepointer::Operation::OPEN:
					    if(fileNameTok) {
						    for(std::map<int, Filepointer>::const_iterator it = filepointers.cbegin();
							it != filepointers.cend();
							++it) {
							    const Filepointer &fptr = it->second;
							    if(fptr.filename == fileNameTok->str() &&
								(fptr.mode == OpenMode::RW_MODE || fptr.mode == OpenMode::WRITE_MODE))
					...
					case Filepointer::Operation::POSITIONING:
					    ...
					    else if(f.append_mode == Filepointer::AppendMode::APPEND && tok->str() != "fflush" &&
						printWarnings)
		...
		for(const Token * tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			for(std::size_t i = 1; i < formatstr.length(); i++) {
				if(formatstr[i] == '%')
					...
					if(formatstr[i] == 's' || formatstr[i] == '[' || formatstr[i] == 'S' ||
					    (formatstr[i] == 'l' && formatstr[i+1] == 's'))                                                   // #3490
						                                                                                              // -
						                                                                                              // field
						                                                                                              // width
						                                                                                              // limits
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(formatStringArgNo >= 0) {
				// formatstring found in library. Find format string and first argument belonging to
				// format string.
				...
				// template <size_t size> int _snprintf_s(char (&buffer)[size], size_t count, const char
				// *format, ...);
				if(findFormat(2, tok->tokAt(2), &formatStringTok, &argListTok)) {
					if(!formatStringTok)
						continue;
    ...
    const bool scanf_s)
{
	...
	for(std::string::const_iterator i = formatString.begin(); i != formatString.end(); ++i) {
		if(*i == '%') {
			...
			if(scan || *i != 'm') { // %m is a non-standard extension that requires no parameter on print
				                // functions.
				++numFormat;
						...
						while(!done) {
							switch(*i) {
								...
								case ']': // charset
									...
									(*i == 's' ||
									bracketBeg ==
									formatString.end()) ? std::string{ 's' } : std::string{ bracketBeg,
																i + 1 };
								    if(argInfo.variableInfo && argInfo.isKnownType() &&
									argInfo.variableInfo->isArray() &&
									(argInfo.variableInfo->dimensions().size() == 1) &&
									argInfo.variableInfo->dimensions()[0].known) {
									    if(!width.empty()) {
										    const int numWidth = std::atoi(width.c_str());
										    if(numWidth != (argInfo.variableInfo->dimension(0) - 1))
											    invalidScanfFormatWidthError(tok,
												numFormat,
												numWidth,
												argInfo.variableInfo,
												specifier);
									    }
								    }
									...
									argInfo.typeToken->strAt(-1) == "const")) {
										...
										!argInfo.typeToken->isStandardType()))
										    invalidScanfArgTypeError_s(tok,
											numFormat,
											specifier,
											&argInfo);
								    }
								...
								case 'o':
								    ...
								    if(argInfo.typeToken->tokType() == Token::eString)
									    invalidScanfArgTypeError_int(tok,
										numFormat,
										specifier,
										&argInfo,
										true);
										...
										argInfo.typeToken->strAt(-1) == "const") {
										    ...
										    switch(specifier[0]) {
												...
												if(specifier[1] == 'l') {
													if(argInfo.typeToken->str() !=
														...
														invalidScanfArgTypeError_int(
															...
															&argInfo,
															true);
													else if(typesMatch(argInfo.typeToken
													    ->originalName(), "size_t") ||
													    typesMatch(argInfo.typeToken->
													    originalName(), "ptrdiff_t") ||
													    typesMatch(argInfo.typeToken->
													    originalName(), "uintmax_t"))
														invalidScanfArgTypeError_int(
															tok,
															numFormat,
															specifier,
												...
												else if(argInfo.typeToken->str() !=
												    "long" || argInfo.typeToken->isLong())
													invalidScanfArgTypeError_int(tok,
													    numFormat,
													    specifier,
													    &argInfo,
												...
												break;
											    case 'j':
												if(!typesMatch(argInfo.typeToken->
												    originalName(), "uintmax_t"))
													invalidScanfArgTypeError_int(tok,
													    numFormat,
													    specifier,
										...
										argInfo.typeToken->strAt(-1) == "const") {
										    ...
										    switch(specifier[0]) {
												...
												if(argInfo.typeToken->str() != "int")
												    ...
												    originalName(), "ptrdiff_t") ||
												    argInfo.typeToken->originalName() ==
												    "intmax_t")
													invalidScanfArgTypeError_int(tok,
													    numFormat,
													    specifier,
								...
								case 'a':
								    specifier += *i;
								    if(argInfo.typeToken->tokType() == Token::eString)
									    invalidScanfArgTypeError_float(tok,
										numFormat,
										specifier,
										&argInfo);
								    else if(argInfo.isKnownType()) {
										...
										argInfo.typeToken->strAt(-1) == "const") {
										    ...
										    switch(specifier[0]) {
											    case 'l':
												if(argInfo.typeToken->str() != "double" ||
												    argInfo.typeToken->isLong())
													invalidScanfArgTypeError_float(tok,
													    numFormat,
													    specifier,
								...
								case 'z':
								case 'L':
								    // Expect an alphabetical character after these
								    // specifiers
								    if((i + 1) != formatString.end() && !isalpha(*(i+1))) {
									    specifier += *i;
									    invalidLengthModifierError(tok, numFormat, specifier);
						...
						while(!done) {
							...
							switch(*i) {
								case 's':
									...
									argInfo.isKnownType() && !argInfo.isArrayOrPointer()) {
									    if(!Token::Match(argInfo.typeToken, "char|wchar_t")) {
										    if(!argInfo.element)
											    invalidPrintfArgTypeError_s(tok,
												numFormat,
												&argInfo);
									    }
								    }
								    ...
								    break;
								case 'n':
								    if((argInfo.isKnownType() &&
									(!argInfo.isArrayOrPointer() ||
									argInfo.typeToken->strAt(-1) == "const")) ||
									argListTok->tokType() == Token::eString)
								...
								case 'o':
								    specifier += *i;
								    if(argInfo.typeToken->tokType() == Token::eString)
									    invalidPrintfArgTypeError_uint(tok,
										numFormat,
										specifier,
										&argInfo);
									    ...
									    if(!Token::Match(argInfo.typeToken,
										"bool|short|long|int|char|wchar_t")) {
										    if(!(!argInfo.isArrayOrPointer() && argInfo.element))
											    invalidPrintfArgTypeError_uint(tok,
												numFormat,
												specifier,
										    ...
										    switch(specifier[0]) {
												...
												break;
											    case 'j':
												if(argInfo.typeToken->originalName() !=
												    "uintmax_t")
													invalidPrintfArgTypeError_uint(tok,
													    numFormat,
													    specifier,
								...
								case 'i':
								    specifier += *i;
								    if(argInfo.typeToken->tokType() == Token::eString) {
									    invalidPrintfArgTypeError_sint(tok,
										numFormat,
										specifier,
										&argInfo);
								    }
									    ...
									    if(argInfo.typeToken->isUnsigned() &&
										!Token::Match(argInfo.typeToken, "char|short")) {
										    if(!(!argInfo.isArrayOrPointer() && argInfo.element))
											    invalidPrintfArgTypeError_sint(tok,
												numFormat,
												specifier,
									    ...
									    else if(!Token::Match(argInfo.typeToken,
								...
								case 'u':
								    ...
								    if(argInfo.typeToken->tokType() == Token::eString) {
									    ...
									    if(!argInfo.typeToken->isUnsigned() &&
										!Token::Match(argInfo.typeToken, "bool|_Bool")) {
										    if(!(!argInfo.isArrayOrPointer() && argInfo.element))
											    invalidPrintfArgTypeError_uint(tok,
												numFormat,
												specifier,
								...
								case 'p':
								    if(argInfo.typeToken->tokType() == Token::eString)
									    ; // string literals are passed as pointers
								              // to literal start, okay
								...
								case 'G':
								    specifier += *i;
								    if(argInfo.typeToken->tokType() == Token::eString)
									    invalidPrintfArgTypeError_float(tok,
										numFormat,
										specifier,
										&argInfo);
								    else if(argInfo.isArrayOrPointer() && !argInfo.element) {
								    ...
								    break;
								case 'h': // Can be 'hh' (signed char or unsigned char)
								          // or 'h' (short int or unsigned short int)
								case 'l': { // Can be 'll' (long long int or unsigned
									    // long long int) or 'l' (long int or
									    // unsigned long int)
								    // If the next character is the same (which makes
								    // 'hh' or 'll') then expect another alphabetical
								    // character
								    if((i + 1) != formatString.end() && *(i + 1) == *i) {
							    ...
							    break;
								case 'I': // Microsoft extension: I for size_t and
								          // ptrdiff_t, I32 for __int32, and I64 for
								          // __int64
								    if((*(i+1) == '3' && *(i+2) == '2') ||
									(*(i+1) == '6' && *(i+2) == '4')) {
									    specifier += *i++;
	...
	if((numFormat + numSecure) != numFunction)
		wrongPrintfScanfArgumentsError(tok,
		    tok->originalName().empty() ? tok->str() : tok->originalName(),
		    numFormat + numSecure,
		    numFunction);
}
	...
	, isCPP(_isCPP)
{
	...
	// Use AST type info
	// TODO: This is a bailout so that old code is used in simple cases. Remove the old code and always use the AST
	// type.
	if(!Token::Match(arg, "%str% ,|)") && !(arg->variable() && arg->variable()->isArray())) {
		...
		if(valuetype && valuetype->type >= ValueType::Type::BOOL) {
			...
			if(top->isBinaryOp() && valuetype->pointer == 1 &&
			    (valuetype->type == ValueType::CHAR || valuetype->type == ValueType::WCHAR_T))
	...
	if(arg->tokType() == Token::eString) {
		...
		for(; tok1; tok1 = tok1->next()) {
			...
			// check for some common well known functions
			    ...
			    ((Token::Match(tok1->previous(), "%var% . size|empty|c_str ( ) [,)]") && isStdContainer(tok1->previous())) ||
			    (Token::Match(tok1->previous(),
			    "] . size|empty|c_str ( ) [,)]") && isStdContainer(tok1->previous()->link()->previous())))) {
				tempToken = new Token();
				tempToken->fileIndex(tok1->fileIndex());
				tempToken->linenr(tok1->linenr());
		...
		if(varTok) {
			...
			if(variableInfo) {
				if(element && isStdVectorOrString()) { // isStdVectorOrString sets type token if true
					...
					if(variableInfo->type() && variableInfo->type()->classScope &&
					    variableInfo->type()->classScope->enumType)
...
bool CheckIO::ArgumentInfo::isComplexType() const
{
	...
	return ((variableInfo->isStlStringType() ||
	       (varTypeTok->strAt(1) == "<" && varTypeTok->linkAt(1) && varTypeTok->linkAt(1)->strAt(1) != "::")) &&
		...
		if(type->tokType() == Token::eString) {
			...
			if(type->originalName().empty()) {
				...
				if(type->strAt(1) == "*" && !argInfo->element)
					...
					os << " *";
				else if(type->strAt(1) == "*" && argInfo->variableInfo && argInfo->element &&
				    argInfo->variableInfo->isArray())
					os << " *";
				if(argInfo->address)
					os << " *";
...
void CheckIO::invalidLengthModifierError(const Token* tok, nonneg int numFormat, const std::string& modifier)
{
	if(!mSettings->severity.isEnabled(Severity::warning))
		return;
	std::ostringstream errmsg;
	errmsg << "'" << modifier << "' in format string (no. " << numFormat <<
    ...
    const std::string& specifier)
{
	...
	if(arrlen > width) {
		if(tok != nullptr &&
		    (!mSettings->certainty.isEnabled(Certainty::inconclusive) || !mSettings->severity.isEnabled(Severity::warning)))
			return;
		...
		errmsg << "Width " << width << " given in format string (no. " << numFormat << ") is larger than destination buffer '"
		       << varname << "[" << arrlen << "]', use %" <<
		(specifier == "c" ? arrlen : (arrlen - 1)) << specifier << " to prevent overflowing it.";
