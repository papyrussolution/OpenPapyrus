...
public:
	...
	struct AllocInfo {
		...
		explicit AllocInfo(int type_ = 0, AllocStatus status_ = NOALLOC, const Token* allocTok_ = nullptr) : status(status_), type(
				type_), allocTok(allocTok_) {
		}
...
private:
	 ...
	 * @return next token to process (if no other checks needed for this token). NULL if other checks could be
	 *performed.
	 */
	const Token * checkTokenInsideExpression(const Token * const tok, VarInfo * varInfo);

	/** parse function call */
	void functionCall(const Token * tokName,
