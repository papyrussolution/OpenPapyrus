		...
		for(const Token * tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			for(const Token * tok2 = tok; tok2 != end2; tok2 = tok2->next()) {
				...
				// Variable is assigned -> Bail out
					...
					if(throwToken) // For non-inconclusive checking, wait until we find an
						       // assignment to it. Otherwise we assume it is safe to leave a
						       // dead pointer.
						deallocThrowError(throwToken, tok2->str());
					break;
				}
				// Variable passed to function. Assume it becomes assigned -> Bail out
				else if(Token::Match(tok2, "[,(] &| %varid% [,)]", varid)) // TODO: No bailout if passed
					                                                   // by value or as const
					                                                   // reference
					break;
			}
		}
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		if(varid) {
			for(const Token* tok = scope.bodyStart->next(); tok && tok != scope.bodyEnd; tok = tok->next()) {
				if(Token::simpleMatch(tok, "catch (") && tok->next()->link() && tok->next()->link()->next()) { // Don't
					                                                                                       // check
					                                                                                       // inner
					                                                                                       // catch
					                                                                                       // -
					...
					if(Token::Match(parent->astParent(),
					    "%assign%|++|--|(") && parent == parent->astParent()->astOperand1())
						break;
				}
