...
template <typename T> static void skipEnumBody(T ** tok)
{
	T * defStart = *tok;
...
Tokenizer::Tokenizer() : list(nullptr), mSettings(nullptr), mErrorLogger(nullptr), mSymbolDatabase(nullptr), mTemplateSimplifier(nullptr),
	mVarId(0), mUnnamedCount(0), mCodeWithTemplates(false), //is there any templates?
...
Tokenizer::Tokenizer(const Settings * settings, ErrorLogger * errorLogger) : list(settings), mSettings(settings), mErrorLogger(errorLogger), mSymbolDatabase(nullptr),
	mTemplateSimplifier(nullptr), mVarId(0), mUnnamedCount(0),
	...
	if(tok2->next() && tok2->next()->str() != ")" &&
		...
		if(Token::Match(tok2->next(), "( * %type% ) ("))
			...
			if(tok2->next()->str() == "(")
				...
				if(!tok2)
					return nullptr;
				if(tok2->str() == "(" && tok2->link()->next() && tok2->link()->next()->str() == "(") {
					tok2 = tok2->link();
					if(tok2->next()->str() == "(")
						tok2 = tok2->next()->link();
				}
			...
			for(Token * tok2 = tok; tok2; tok2 = tok2->next()) {
				...
				if(simplifyType) {
					...
					if(removed1 == classPath && !removed1.empty()) {
						for(std::vector<Space>::const_reverse_iterator it = spaceInfo.crbegin();
						    it != spaceInfo.crend();
						    ++it) {
...
bool usingMatch(const Token * nameToken,
		...
		if(addlength || Token::Match(tok, "%var% [ ] = %str% ;")) {
			...
			for(Token * tok2 = tok->tokAt(4); tok2 && tok2 != end; tok2 = tok2->next()) {
				if(tok2->link() && Token::Match(tok2, "{|(|[|<")) {
					if(tok2->str() == "[" && tok2->link()->strAt(1) == "=") { // designated
						                                                  // initializer
	...
	for(Token* tok = list.front(); tok; tok = tok->next()) {
		if(tok == list.front() || !tok->scopeInfo()) {
			...
			if(Token::simpleMatch(tok, "{")) {
				...
				std::shared_ptr<ScopeInfo2> newScopeInfo = std::make_shared<ScopeInfo2>(tok->scopeInfo()->name,
					tok->link(),
					tok->scopeInfo()->usingNamespaces);
	...
	while(tok2) {
		if(tok2->isName()) {
			...
			if(Token::Match(tok2, "struct|union|enum") || (!c && Token::Match(tok2, "class|typename"))) {
				...
				if(tok2->str() != "void" || Token::Match(tok2, "void const| *|(")) // just "void" cannot
					                                                           // be a variable type
					++typeCount;
				++singleNameCount;
			}
		...
		else if(singleNameCount >= 1 && tok2->previous() && tok2->previous()->isStandardType() &&
		...
		if(tok->str() == "{") {
			...
			if(indentlevel > 0 || initList) {
				if(Token::Match(tok->previous(),
				    "::|.") && tok->strAt(-2) != "this" && !Token::simpleMatch(tok->tokAt(-5), "( * this ) ."))
					continue;
				if(!tok->next())
					syntaxError(nullptr);
				if(tok->next()->str() == "::") {
				...
				if(!inEnum) {
					const std::map<std::string,
					    nonneg int>::const_iterator it = variableMap.map(false).find(tok->str());
					if(it != variableMap.map(false).end()) {
						tok->varId(it->second);
...
static const std::unordered_set<std::string> notstart_cpp = { NOTSTART_C, "delete", "friend", "new", "throw", "using", "virtual", "explicit",
	"const_cast", "dynamic_cast", "reinterpret_cast", "static_cast", "template"};
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(tok == functionDeclEndToken) {
			...
			if(tok->str() == ":")
				...
				scopeStack.push(VarIdScopeInfo(true, scopeStack.top().isStructInit || tok->strAt(-1) == "=",
				    /*isEnum=*/ false, *variableMap.getVarId()));
				...
				if(ifToken && ifToken->str() == "if") {
					// open another scope to differentiate between variables declared in the "if"
					// condition and in the "if" body
			...
			if(!scopeStack.top().isExecutable)
				...
				if(tokenLinkNext && tokenLinkNext->str() == "{") // might be for- or while-loop or
					                                         // if-statement
			...
			if(!Token::Match(startToken->previous(), "union|struct|enum|namespace {") &&
			    !(initlist &&
			    Token::Match(startToken->previous(), "%name%|>|>>") && Token::Match(startToken->link(), "} ,|{"))) {
				if(tok->str() == "{") {
					bool isExecutable;
					...
					initlist = false;
					scopeStack.push(VarIdScopeInfo(isExecutable, scopeStack.top().isStructInit || tok->strAt(-1) == "=",
					...
					if(!scopeStack.top().isStructInit) {
						...
						// check if this '}' is an end of an "else" body or an "if" body without
						// an "else" part
						...
						if(ifToken && ifToken->str() == ")")
							...
							ifToken = nullptr;
						if(startToken->strAt(-1) == "else" ||
						    (ifToken && ifToken->str() == "if" && tok->strAt(1) != "else")) {
							// leave the extra scope used to differentiate between variables
							// declared in the "if" condition and in the "if" body
							variableMap.leaveScope();
						}
					}
			...
			if(decl) {
				if(isCPP()) {
					if(Token * declTypeTok = Token::findsimplematch(tok, "decltype (", tok2)) {
						for(Token * declTok = declTypeTok->linkAt(1);
						    declTok != declTypeTok;
				...
				if(Token::Match(prev2, "%type% [;[=,)]") && tok2->previous()->str() != "const")
					...
					if(!tok3->isStandardType() && tok3->str() != "void" &&
					    !Token::Match(tok3,
					    "struct|union|class %type%") && tok3->str() != "." &&
					    !Token::Match(tok2->link()->previous(), "[&*]")) {
						if(!scopeStack.top().isExecutable) {
							// Detecting initializations with () in non-executable scope is
							// hard and often impossible to be done safely. Thus, only treat
							// code as a variable that definitely is one.
							decl = false;
							bool rhs = false;
				...
				else if(isCPP() && Token::Match(prev2, "%type% {") && Token::simpleMatch(tok2->link(), "} ;")) { // C++11
					                                                                                         // initialization
					                                                                                         // style
					...
					if(tok && tok->str() == "<") {
						...
						while(tok != end) {
							if(tok->isName() && !(Token::simpleMatch(tok->next(), "<") &&
								...
								const std::map<std::string,
								    nonneg int>::const_iterator it =
								    variableMap.map(false).find(tok->str());
								if(it != variableMap.map(false).end())
									tok->varId(it->second);
							}
							tok = tok->next();
		...
		if(tok->isName()) {
			...
			// function declaration inside executable scope? Function declaration is of form: type name "("
			// args ")"
			if(scopeStack.top().isExecutable && Token::Match(tok, "%name% [,)]")) {
				...
				// there are tokens which can't appear at the begin of a function declaration such as
				// "return"
				...
				if(Token::Match(start,
				    "[;{}] %type% %name%|*") && par && Token::simpleMatch(end, ") ;") && !isNotstartKeyword)
					// function declaration => don't set varid
					continue;
			}

			if(!scopeStack.top().isEnum || !(Token::Match(tok->previous(), "{|,") && Token::Match(tok->next(), ",|=|}"))) {
				const std::map<std::string, nonneg int>::const_iterator it = variableMap.map(globalNamespace).find(
					tok->str());
...
void Tokenizer::setVarIdPass2()
{
	...
	if(!isC()) {
		...
		for(Token * tok = list.front(); tok; tok = tok->next()) {
			...
			else if(usingnamespaces.empty() || tok->varId() || !tok->isName() || tok->isStandardType() ||
			    tok->tokType() == Token::eKeyword || tok->tokType() == Token::eBoolean ||
			    Token::Match(tok->previous(),
			    ".|namespace|class|struct|&|&&|*|> %name%") ||
			    Token::Match(tok->previous(),
			    "%type%| %name% ( %type%|)") || Token::Match(tok, "public:|private:|protected:") ||
...
static void linkBrackets(const Tokenizer * const tokenizer, std::stack<const Token*>& type, std::stack<Token*>& links,
    Token * const token, const char open, const char close)
{
	if(token->str()[0] == open) {
		links.push(token);
		type.push(token);
	...
	for(Token * token = list.front(); token; token = token->next()) {
		...
		if(token->link()) {
			...
			while(!type.empty() && type.top()->str() == "<") {
				...
				if(Token::Match(end,
				    "> %var% ;") && (type.top()->tokAt(-2) == nullptr || Token::Match(type.top()->tokAt(-2), ";|}|{")))
					break;
				type.pop();
			}
		}
		else if(token->str() == "<" && ((token->previous() && (token->previous()->isTemplate() ||
...
void Tokenizer::sizeofAddParentheses()
{
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(tok->next()->isLiteral() || Token::Match(tok->next(), "%name%|*|~|!|&")) {
			...
			while(Token::Match(endToken->next(),
			    "%name%|%num%|%str%|[|(|.|::|++|--|!|~") || (Token::Match(endToken, "%type% * %op%|?|:|const|;|,"))) {
			...
			if(derefs != 0 && varid != 0 && declTokOfVar.find(varid) != declTokOfVar.end()) {
				...
				if(Token::Match(decltok, "%name% [") && Token::simpleMatch(decltok->linkAt(1), "] [")) {
					...
					for(int i = 0; i < derefs; i++)
						tok2 = tok2->linkAt(1); // Skip all dimensions that are dereferenced
					                                // before the sizeof call
					while(Token::Match(tok2, "] [ %num% ]")) {
...
bool Tokenizer::simplifyTokenList1(const char FileName[])
{
	...
	for(const Token * tok = list.front(); tok; tok = tok->next()) {
		if(Token::simpleMatch(tok, "if (")) {
			tok = tok->next()->link();
			if(Token::Match(tok, ") %name% (") && tok->next()->isUpperCaseName() && Token::Match(tok->linkAt(2), ") {|else")) {
...
bool Tokenizer::simplifyTokenList2()
{
	...
	simplifyCasts();
	// Simplify simple calculations before replace constants, this allows the replacement of constants that are
	// calculated
		...
		for(const Variable * var : mSymbolDatabase->variableList()) {
			...
			if(typetok != var->typeEndToken()) {
				reportError(var->typeStartToken(),
				    Severity::debug,
				    "debug",
				    "Variable::typeStartToken() of variable '" + var->name() + "' is not located before Variable::typeEndToken(). The location of the typeStartToken() is '" + var->typeStartToken()->str() + "' at line " +
				    MathLib::toString(var->typeStartToken()->linenr()));
			}
		}
	}
}
...
void Tokenizer::simplifyHeadersAndUnusedTemplates()
{
	...
	const std::set<std::string> functionStart{"static", "const", "unsigned", "signed", "void", "bool", "char", "short", "int", "long",
						  "float", "*"};
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(isIncluded && !mSettings->checkHeaders && tok->str() == "{") {
			// TODO: We probably need to keep the executable code if this function is called from the source
			// file.
			const Token * prev = tok->previous();
		...
		if(!tok->previous() || Token::Match(tok->previous(), "[;{}]")) {
			...
			if(isIncluded && removeUnusedIncludedFunctions) {
				while(true) {
					...
					if(Token::Match(start,
					    "%name% (") &&
					    Token::Match(start->linkAt(1), ") const| ;") && keep.find(start->str()) == keep.end()) {
			...
			if(removeUnusedTemplates || (isIncluded && removeUnusedIncludedTemplates)) {
				if(Token::Match(tok, "template < %name%")) {
					const Token * closingBracket = tok->next()->findClosingBracket();
					if(Token::Match(closingBracket,
					    "> class|struct %name% [;:{]") && keep.find(closingBracket->strAt(2)) == keep.end()) {
					...
					else if(Token::Match(closingBracket,
					    "> %type% %name% (") &&
					    Token::simpleMatch(closingBracket->linkAt(3),
					    ") {") && keep.find(closingBracket->strAt(2)) == keep.end()) {
...
void Tokenizer::splitTemplateRightAngleBrackets(bool check)
{
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(tok->previous() && tok->str() == "<" && TemplateSimplifier::templateParameters(tok) &&
		    vars.find(tok->previous()->str()) == vars.end()) {
			...
			if(check) {
				if(Token::Match(endTok, ">>|>>="))
					reportError(tok,
					    ...
					    "bad closing bracket for !!!<!!!: " + getExpression(tok),
					    false);
				continue;
			}
			if(endTok && endTok->str() == ">>") {
		    ...
		    "class|struct|union|=|:|public|protected|private %name% <") && vars.find(tok->next()->str()) == vars.end()) {
...
void Tokenizer::removeMacrosInGlobalScope()
{
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(Token::Match(tok, "%type%") && tok->isUpperCaseName() &&
		    (!tok->previous() ||
		    Token::Match(tok->previous(), "[;{}]") || (tok->previous()->isName() && endsWith(tok->previous()->str(), ':')))) {
			const Token * tok2 = tok->next();
			if(tok2 && tok2->str() == "(")
				tok2 = tok2->link()->next();
			...
			if(Token::Match(tok,
			    "%name% (") &&
			    Token::Match(tok2,
			    "%name% *|&|::|<| %name%") && !Token::Match(tok2, "namespace|class|struct|union|private:|protected:|public:"))
				unknownMacroError(tok);

			if(Token::Match(tok,
			    "%type% (") &&
			    Token::Match(tok2, "%type% (") && !Token::Match(tok2, "noexcept|throw") && isFunctionHead(tok2->next(), ":;{"))
				unknownMacroError(tok);
		...
		for(Token * tok = begin; tok && tok != end; tok = tok->next()) {
			...
			if(tok->str() == "{") {
				...
				if(stilldead) {
					...
					if(indentLevel == 1 || tok->next()->str() != "}" ||
					    !Token::Match(tok->next()->link()->previous(), ";|{|}|do {"))
						stilldead = false;
					continue;
				}
			}
			...
			if(tok->next()->isNumber()) {
				...
				if(cmp == "==" || cmp == "!=") {
					...
					if(MathLib::isInt(op1) && MathLib::isInt(op2))
						...
						// It is inconclusive whether two unequal float representations are
						// numerically equal
...
void Tokenizer::simplifyCasts()
{
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		while((Token::Match(tok->next(),
		    "( %type% *| *| *|&| ) *|&| %name%") && (tok->str() != ")" || tok->tokAt(2)->isStandardType())) ||
				...
				if(varName->str() != "operator") {
					...
					if(tok2->str() == "=" && (isstatic || (isconst && !isPointerOrRef))) {
						...
						while(tok2 && tok2->str() != "," && tok2->str() != ";") {
							...
							const Token * tok3 = tok2;
							if(!isC() && tok2->str() == "<" &&
							    TemplateSimplifier::templateParameters(tok2) > 0) {
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(cOpIt != cAlternativeTokens.end()) {
			...
			if(Token::Match(tok->next(),
			    "%assign%|%or%|%oror%|&&|*|/|%|^") && !Token::Match(tok->previous(), "%num%|%char%|) %name% *"))
				continue;
...
Token * Tokenizer::initVar(Token * tok)
{
	...
	if(tok->tokAt(2)->isStandardType() || tok->strAt(2) == "void")
		return tok;
	else if(!tok->tokAt(2)->isNumber() &&
	    !Token::Match(tok->tokAt(2), "%type% (") && tok->strAt(2) != "&" && tok->tokAt(2)->varId() == 0)
		return tok;
	...
	// constants..
	{
		...
		for(Token * tok = list.front(); tok; tok = tok->next()) {
			...
			if(tok->isName() && (Token::Match(tok, "static| const| static| %type% const| %name% = %any% ;") ||
				...
				// find the entry for the known variable, if any.  Exclude the location where the
				// variable is assigned with next == "="
		...
		for(auto constantVar = constantVars.rbegin(); constantVar != constantVars.rend(); constantVar++) {
			...
			if(!referenceFound) {
				...
				// #8579 if we can we want another token to delete startTok. if we can't it doesn't
				// matter
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		for(; tok2; tok2 = tok2->next()) {
			...
			if(tok2->str() == "{")
				...
				// there could be a hang here if tok2 is moved back by the function calls below for some
				// reason
				if(Settings::terminated())
					return false;

				if(!simplifyKnownVariablesGetData(varid, &tok2, &tok3, value, valueVarId, valueIsPointer,
				...
				ret |= simplifyKnownVariablesSimplify(&tok2,
					tok3,
					varid,
					structname,
					value,
				...
				if(tok2->str() == "sprintf") {
					...
					while((n = value.find("%%", n)) != std::string::npos) {
						// Replace "%%" with "%" - erase the first '%' and continue past the
						// second '%'
						value.erase(n, 1);
						++n;
					}
				...
				// there could be a hang here if tok2 was moved back by the function call above for some
				// reason
				if(Settings::terminated())
					return false;
			}
...
bool Tokenizer::simplifyKnownVariablesGetData(nonneg int varid,
    Token ** _tok2,
    Token ** _tok3,
    std::string &value,
    nonneg int &valueVarId,
    bool &valueIsPointer,
    bool floatvar)
{
	Token * tok2 = *_tok2;
    ...
    int indentlevel) const
{
	...
	for(; tok3; tok3 = tok3->next()) {
		...
		if(((tok3->previous()->varId() > 0) && Token::Match(tok3, ("& " + structname + " %varid%").c_str(), varid)) ||
		    (Token::Match(tok3, ("[=+-*/%^|[] " + structname + " %varid% [=?+-*/%^|;])]").c_str(),
		    varid) && !Token::Match(tok3, ("= " + structname + " %name% =").c_str())) ||
...
bool Tokenizer::simplifyRedundantParentheses()
{
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(isCPP() && Token::simpleMatch(tok->previous(), "} (")) {
			...
			if(Token::Match(plp,
			    "%name%|>|] {") || (Token::simpleMatch(plp, ")") && Token::simpleMatch(plp->link()->previous(), "]")))
				continue;
		}
...
bool Tokenizer::isScopeNoReturn(const Token * endScopeToken, bool * unknown) const
{
	...
	if(!unknownFunc.empty() && mSettings->checkLibrary && mSettings->severity.isEnabled(Severity::information)) {
		...
		// don't warn for nonglobal functions (class methods, functions hidden in namespaces) since they can't
		// be configured yet
		...
		if(Token::Match(tok, "[{};] switch (")) {
			...
			if(!checklabel) {
				...
				if(indentlevel < indentcheck) {
					...
					if(indentswitch &&
					    Token::simpleMatch(tok, ") {") && Token::Match(tok->link()->tokAt(-2), "[{};] switch ("))
...
{
	printDebugOutput(0);
	throw InternalError(tok, "Unmatched '" + tok->str() + "'. Configuration: '" + mConfiguration + "'.", InternalError::SYNTAX);
}
...
{
	printDebugOutput(0);
	throw InternalError(tok1,
		  "There is an unknown macro here somewhere. Configuration is required. If " + tok1->str() + " is a macro then please configure it.",
		  InternalError::UNKNOWN_MACRO);
}
...
void Tokenizer::reportUnknownMacros() const
{
	...
	for(const Token * tok = tokens(); tok; tok = tok->next()) {
		...
		if(Token::Match(tok,
		    "%name% (") && tok->isUpperCaseName() &&
		    Token::simpleMatch(tok->linkAt(1), ") (") && Token::simpleMatch(tok->linkAt(1)->linkAt(1), ") {")) {
			// A keyword is not an unknown macro
			if(tok->isKeyword())
				continue;
		...
		else if(Token::Match(tok, "%name% (") && tok->isUpperCaseName() &&
		    Token::Match(tok->linkAt(1), ") %name% (") && Token::Match(tok->linkAt(1)->linkAt(2), ") [;{]")) {
...
void Tokenizer::findGarbageCode() const
{
	...
	static const std::unordered_set<std::string> nonConsecutiveKeywords{ "break", "continue", "for", "goto", "if", "return", "switch", "throw", "typedef", "while" };
	for(const Token * tok = tokens(); tok; tok = tok->next()) {
		// initialization: = {
		if(Token::simpleMatch(tok, "= {") && Token::simpleMatch(tok->linkAt(1), "} ("))
		...
		if(tok->isControlFlowKeyword() && Token::Match(tok, "if|while|for|switch")) { // if|while|for|switch
			                                                                      // (EXPR) { ... }
		...
		if(tok->isKeyword() && nonConsecutiveKeywords.count(tok->str()) != 0) {
			...
			if(Token::Match(prev, "%op%|%num%|%str%|%char%")) {
				if(!Token::simpleMatch(tok->tokAt(-2), "operator \"\" if") &&
					...
					syntaxError(tok,
					    prev ==
					    tok->previous() ? (prev->str() + " " + tok->str()) : (prev->str() + " .. " + tok->str()));
			}
		}
	}
	...
	if((list.back()->str()==")" || list.back()->str()=="}") && list.back()->previous() &&
	    list.back()->previous()->isControlFlowKeyword())
...
{
	const bool cpp = isCPP();
	int count = 0; // A counter that is used when giving unique names for anonymous structs.
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		// check for named struct/union
			...
			if(Token::Match(tok->next(), "const|static|volatile| *|&| const| (| %type% )| ,|;|[|=|(|{")) {
				...
				if(!tok)
					break; // see #4869 segmentation fault in Tokenizer::simplifyStructDecl (invalid
				               // code)
...
void Tokenizer::simplifyCallingConvention()
{
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		while(Token::Match(tok,
		    "__cdecl|__stdcall|__fastcall|__thiscall|__clrcall|__syscall|__pascal|__fortran|__far|__near") ||
		    (windows && Token::Match(tok, "WINAPI|APIENTRY|CALLBACK"))) {
			tok->deleteThis();
		}
	}
}
		...
		while(Token::Match(tok, "__attribute__|__attribute (")) {
			...
			if(Token::Match(after, "%name%|*|&|(")) {
				...
				if(Token::simpleMatch(prev, ")") && Token::Match(prev->link()->previous(), "%name% ("))
				    ...
				    ")") && Token::Match(prev->link()->tokAt(-2), "operator %op% (") && isCPP())
		...
		if(vartok->isName()) {
			if(Token::Match(tok->previous(), "__cppcheck_low__ ( %num% )"))
				vartok->setCppcheckAttribute(TokenImpl::CppcheckAttributes::Type::LOW,
				    MathLib::toLongNumber(tok->next()->str()));
...
static const std::unordered_set<std::string> keywords = { "inline" , "_inline" , "__inline" , "__forceinline" , "register" , "__restrict" , "__restrict__" , "__thread" };
		...
		if(!Token::Match(tok->previous(), ".|::")) {
			if(Token::Match(tok, "%name% (")) {
				if(isFunctionHead(tok->next(), "{"))
					...
					while(Token::Match(start->previous(), "%type%|*|&"))
						start = start->previous();
					if(start != tok && start->isName() &&
					    (!start->previous() || Token::Match(start->previous(), "[;{}]")))
						userFunctions.insert(tok->str());
				}
				if(userFunctions.find(tok->str()) == userFunctions.end() &&
				    stdFunctions.find(tok->str()) != stdFunctions.end())
					insert = true;
			}
			else if(Token::Match(tok, "%name% <") && stdTemplates.find(tok->str()) != stdTemplates.end())
				insert = true;
			else if(tok->isName() && !tok->varId() &&
			    !Token::Match(tok->next(), "(|<") && stdTypes.find(tok->str()) != stdTypes.end())
				insert = true;
		}

		if(insert) {
			tok->previous()->insertToken("std");
			tok->previous()->linenr(tok->linenr()); // For stylistic reasons we put the std:: in the same
			                                        // line as the following token
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		...
		if(Token::Match(tok, "CopyMemory|RtlCopyMemory|RtlCopyBytes")) {
			...
			if(tok1) {
				...
				if(tok2)
					Token::move(tok1->previous(), tok2->tokAt(-2), tok->next()->link()->previous()); // Swap
				                                                                                         // third
				                                                                                         // with
				                                                                                         // second
				                                                                                         // argument
	...
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		if(Token::Match(tok, "class|struct|namespace %type% :|{") &&
			...
			if(tok == classInfo.back().bodyEnd)
				...
				const bool isConstructorOrDestructor = Token::Match(tok, "%type% :: ~| %type%") && (tok->strAt(2) == tok->str() || (tok->strAt(2) == "~" && tok->strAt(3) == tok->str()));
				if(!isConstructorOrDestructor) {
					...
					if(!isPrependedByType) {
						// It's not a constructor declaration and it's not a function
						// declaration so
						// this is a function call which can have all the qualifiers just fine -
						// skip.
						continue;
					}
				}
...
{
	for(Token * tok = list.front(); tok; tok = tok->next()) {
		if(Token::simpleMatch(tok, "return strncat (") && Token::simpleMatch(tok->linkAt(2), ") ;") && tok->strAt(3) != ")" && tok->strAt(3) != ",") {
			//first argument
			Token * tok2 = tok->tokAt(3);
...
void Tokenizer::prepareTernaryOpForAST()
{
	...
	//       "The expression in the middle of the conditional operator (between ? and :) is parsed as if
	// parenthesized: its precedence relative to ?: is ignored."
				...
				if(Token::simpleMatch(tok2, "{"))
					...
					if(Token::Match(tok2->previous(), "namespace %name% =")) {
						...
						if(sameTokens(tokNameStart, tokNameEnd, tok2->tokAt(2))) {
							...
							// cppcheck-suppress duplicateBranch - remove when TODO below is
							// addressed
							if(endScope == scope) {
...
bool Tokenizer::hasIfdef(const Token * start, const Token * end) const
{
	...
	for(const Directive &d: mPreprocessor->getDirectives()) {
		if(d.str.compare(0, 3, "#if") == 0 && d.linenr >= start->linenr() && d.linenr <= end->linenr() &&
		    start->fileIndex() < list.getFiles().size() && d.file == list.getFiles()[start->fileIndex()])
			return true;
	}
	return false;
}
