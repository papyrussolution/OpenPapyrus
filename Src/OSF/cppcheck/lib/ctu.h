...
public:
	...
	struct Location {
		...
		Location(const std::string &fileName, nonneg int lineNumber, nonneg int column) : fileName(fileName),
	...
	struct UnsafeUsage {
		...
		UnsafeUsage(const std::string &myId,
		    ...
		    MathLib::bigint value) : myId(myId), myArgNr(myArgNr), myArgumentName(myArgumentName), location(location),
			value(value) {
		}
