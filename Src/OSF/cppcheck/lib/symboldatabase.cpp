		...
		// check if in class or structure or union
			...
			if(isFunction(tok, scope, &funcStart, &argStart, &declEnd)) {
				...
				// function prototype?
					...
					for(std::multimap<std::string, const Function*>::const_iterator it = range.first;
					    it != range.second;
					    ++it) {
			...
			if(tok->isKeyword() && Token::Match(tok, "else|try|do {")) {
			    ...
			    Token::Match(tok, "if|for|while|catch|switch (") && Token::simpleMatch(tok->next()->link(), ") {")) {
				...
				if(scope->type == Scope::eFor)
					scope->checkVariable(tok->tokAt(2), AccessControl::Local, mSettings); // check
				                                                                              // for
				                                                                              // variable
				                                                                              // declaration
				                                                                              // and add
					...
					scope->checkVariable(tok->tokAt(2), AccessControl::Throw, mSettings); // check
				                                                                              // for
				                                                                              // variable
				                                                                              // declaration
				                                                                              // and add
		...
		do {
			...
			for(Scope& scope : scopeList) {
				...
				if(scope.isClassOrStruct() && scope.definedType->needInitialization == Type::NeedInitialization::Unknown) {
					...
					// User defined types with user defined default constructor doesn't need
					// initialization.
					// We assume the default constructor initializes everything.
					// Another check will figure out if the constructor actually initializes
					// everything.
						...
						for(const Variable& var: scope.varlist) {
							if(var.isClass()) {
								if(var.type()) {
									...
									if(var.type()->needInitialization ==
									    Type::NeedInitialization::True && !var.hasDefault())
										needInitialization = true;
									else if(var.type()->needInitialization ==
									    Type::NeedInitialization::Unknown) {
										if(!(var.valueType() &&
										    var.valueType()->type == ValueType::CONTAINER))
											unknown = true;
									}
								}
							}
				    ...
				    scope.definedType->needInitialization == Type::NeedInitialization::Unknown)
					scope.definedType->needInitialization = Type::NeedInitialization::True;
			}
		    ...
		    for(Token* tok = const_cast<Token *>(bodyStart); tok != bodyEnd; tok = tok->next()) {
			    if(bodyStart != bodyEnd && tok->str() == "{") {
				    ...
				    for(Scope* innerScope: scope.nestedList) {
					    ...
					    if(std::find(list.begin(), list.end(), tok) != list.end()) { // Is begin of
						                                                         // inner scope
						    tok = tok->link();
...
void SymbolDatabase::createSymbolDatabaseSetFunctionPointers(bool firstPass)
{
	...
	for(const Token* tok = mTokenizer->list.front(); tok != mTokenizer->list.back(); tok = tok->next()) {
		if(tok->isName() && !tok->function() && tok->varId() == 0 &&
		    Token::Match(tok, "%name% [(,)>;]") && !isReservedName(tok->str())) {
			...
			if(tok->next()->str() != "(") {
				...
				while(Token::Match(start->tokAt(-2), "%name% ::"))
					start = start->tokAt(-2);
				if(!Token::Match(start->previous(),
				    "[(,<=]") && !Token::Match(start->tokAt(-2), "[(,<=] &") && !Token::Match(start, "%name% ;"))
					continue;
			}
...
void SymbolDatabase::fixVarId(VarIdMap & varIds, const Token * vartok, Token * membertok, const Variable * membervar)
{
	...
	if(memberId == varId->second.end()) {
		if(membertok->varId() == 0) {
			varId->second.insert(std::make_pair(membervar->nameToken()->varId(),
			    const_cast<Tokenizer *>(mTokenizer)->newVarId()));
			mVariableList.push_back(membervar);
...
void SymbolDatabase::createSymbolDatabaseSetVariablePointers()
{
	...
	for(const Token* tok = mTokenizer->list.front(); tok != mTokenizer->list.back(); tok = tok->next()) {
		...
		const bool isVar = tok->variable() &&
		    (tok->variable()->typeScope() || tok->variable()->isSmartPointer() ||
		    (tok->valueType() && tok->valueType()->type == ValueType::CONTAINER));
		...
		const bool isDerefAccess = isVar && !isDirectAccess && Token::simpleMatch(tok->astParent(), "*") && Token::simpleMatch(
						...
						if(membervar) {
							...
							if(membertok->varId() == 0 || mVariableList[membertok->varId()] == nullptr) {
								if(tok->function()->retDef)
									fixVarId(varIds, tok->function()->retDef,
									    const_cast<Token *>(membertok), membervar);
							}
						}
					}
				}
    ...
    const Token** declEnd) const
{
	...
	// regular function?
	    ...
	    (Token::Match(tok->previous(), "%name%|>|&|*|::|~") ||  // Either a return type or scope qualifier in front
	                                                            // of tok
		...
		if(!Token::Match(tok1, "{|}|;|public:|protected:|private:") && tok1) {
			...
			while(Token::Match(tok1,
			    "%type%|*|&") && !endsWith(tok1->str(), ':') && (!isReservedName(tok1->str()) || tok1->str() == "const"))
				tok1 = tok1->previous();
	...
	// regular C function with missing return or invalid C++ ?
	    ...
	    (!tok->previous() || Token::Match(tok->previous(), ";|}"))) {
		if(mTokenizer->isC()) {
			debugMessage(tok, "debug",
			    "SymbolDatabase::isFunction found C function '" + tok->str() + "' without a return type.");
			*funcStart = tok;
			*argStart = tok->next();
			*declEnd = tok->linkAt(1)->next();
			return true;
		...
		if(var) {
			if(!var->scope()) {
				...
				if(!var->isArgument() || (function && function->hasBody())) {
					throw InternalError(
						      var->nameToken(),
						      "Analysis failed (variable without scope). If the code is valid then please report this failure.",
						      InternalError::INTERNAL);
					//std::cout << "!!!Variable found without scope: " << var->nameToken()->str() <<
					// std::endl;
				}
			}
		}
...
void Variable::evaluate(const Settings* settings)
{
	...
	while(tok != end) {
		if(tok->str() == "static")
			...
			setFlag(fIsPointer,
			    !isArray() ||
			    (isContainer && !Token::Match(tok->next(), "%name% [")) || Token::Match(tok->previous(), "( * %name% )"));
	...
	if(mTypeStartToken) {
		...
		setFlag(fIsClass,
		    !lib->podtype(
			    strtype) && !mTypeStartToken->isStandardType() && !isEnumType() && !isPointer() && !isReference() &&
		    strtype != "...");
		setFlag(fIsStlType, Token::simpleMatch(mTypeStartToken, "std ::"));
		setFlag(fIsStlString,
		    isStlType() &&
		    (Token::Match(mTypeStartToken->tokAt(2),
		    "string|wstring|u16string|u32string !!::") ||
		    (Token::simpleMatch(mTypeStartToken->tokAt(2),
		    "basic_string <") && !Token::simpleMatch(mTypeStartToken->linkAt(3), "> ::"))));
...
bool Function::argsMatch(const Scope * scope, const Token * first, const Token * second, const std::string &path,
    nonneg int path_length) const
{
...
bool Function::isSafe(const Settings * settings) const
{
	...
	if(settings->safeChecks.classes && access == AccessControl::Public &&
	    (nestedIn->type == Scope::ScopeType::eClass || nestedIn->type == Scope::ScopeType::eStruct))
		return true;
...
void SymbolDatabase::printOut(const char * title) const
{
	...
	for(std::list<Scope>::const_iterator scope = scopeList.cbegin(); scope != scopeList.cend(); ++scope) {
		...
		for(auto func = scope->functionList.cbegin(); func != scope->functionList.cend(); ++func) {
			...
			if(func->retDef) {
				std::cout << "           ";
				for(const Token * tok = func->retDef;
				    tok && tok != func->tokenDef && !Token::Match(tok, "{|;|override|final");
				    tok = tok->next())
					std::cout << " " << tokenType(tok);
				std::cout << std::endl;
			}
	...
	for(std::list<Scope>::const_iterator scope = scopeList.begin(); scope != scopeList.end(); ++scope) {
		...
		if(scope->functionList.empty() && scope->varlist.empty())
			...
			if(!scope->functionList.empty()) {
				...
				for(std::list<Function>::const_iterator function = scope->functionList.begin();
				    function != scope->functionList.end();
				    ++function) {
					...
					if(function->argCount() == 0U)
						...
						for(unsigned int argnr = 0; argnr < function->argCount(); ++argnr) {
							...
							out << "          <arg nr=\"" << argnr+1 << "\" variable=\"" << arg << "\"/>" <<
								std::endl;
...
void Function::addArguments(const SymbolDatabase * symbolDatabase, const Scope * scope)
{
	...
	for(const Token* tok = start->next(); tok; tok = tok->next()) {
		...
		if(!endTok) {
			if(tok->previous()->isName() && !Token::Match(tok->tokAt(-1), "const|volatile")) {
				if(tok->previous() != typeTok) {
					...
					if(hasBody())
						symbolDatabase->debugMessage(nameTok,
						    "varid0",
						    "Function::addArguments found argument \'" + nameTok->str() + "\' with varid 0.");
				}
				else
					endTok = typeTok;
			}
		...
		argumentList.emplace_back(nameTok,
		    startTok,
		    endTok,
		    count++,
		    AccessControl::Argument,
		    argType,
		    functionScope,
		    symbolDatabase->mSettings);
	...
	for(const Token* tok = argDef->next(); tok && tok != argDef->link(); tok = tok->next()) {
		if(tok->str() == "=") {
			...
			if(tok->strAt(1) == "[") {
				...
				if(!tok)
					throw InternalError(lambdaStart,
						  "Analysis failed (lambda not recognized). If the code is valid then please report this failure.",
						  InternalError::INTERNAL);
			}
		}
	}
...
bool Function::isImplicitlyVirtual(bool defaultVal) const
{
	...
	if(getOverriddenFunction(&foundAllBaseClasses)) //If it overrides a base class's method then it's virtual
		return true;
	if(foundAllBaseClasses) //If we've seen all the base classes and none of the above were true then it must not be
		                // virtual
		return false;
...
const Function * Function::getOverriddenFunctionRecursive(const ::Type* baseType, bool * foundAllBaseClasses) const
{
	...
	for(const ::Type::BaseInfo & i : baseType->derivedFrom) {
		...
		if(!derivedFromType->derivedFrom.empty() && !derivedFromType->hasCircularDependencies()) {
			// avoid endless recursion, see #5289 Crash: Stack overflow in isImplicitlyVirtual_rec when
			// checking SVN and
	...
	enumClass(false)
{
	...
	if(nameTok && ((type == Scope::eEnum && Token::Match(nameTok, ":|{")) || nameTok->str() != "{")) // anonymous
		                                                                                         // and unnamed
		                                                                                         // structs/unions
		                                                                                         // don't have a
		                                                                                         // name
...
const Token * Scope::checkVariable(const Token * tok, AccessControl varaccess, const Settings* settings)
{
	...
	if(tok && isVariableDeclaration(tok, vartok, typetok)) {
		...
		if(vartok->varId() == 0) {
			if(!vartok->isBoolean())
				check->debugMessage(vartok,
				    "varid0",
				    "Scope::checkVariable found variable \'" + vartok->str() + "\' with varid 0.");
			return tok;
		}
...
bool Scope::isVariableDeclaration(const Token* const tok, const Token*& vartok, const Token*& typetok) const
{
	...
	if(Token::Match(localTypeTok, "%type% <")) {
		...
		if(isCPP11 && Token::simpleMatch(localTypeTok, "decltype (") && Token::Match(localTypeTok->linkAt(1), ") %name%|*|&|&&"))
			...
			if(isCPP11 &&
			    Token::simpleMatch(localVarTok, "decltype (") && Token::Match(localVarTok->linkAt(1), ") %name%|*|&|&&"))
		...
		if(scope) {
			...
			if(scope) {
				...
				// enum
					...
					for(std::vector<Scope *>::const_iterator it = scope->nestedList.begin(),
					    end = scope->nestedList.end();
					    it != end;
					    ++it) {
...
const Type* SymbolDatabase::findVariableType(const Scope * start, const Token * typeTok) const
{
	...
	// check for a qualified name and use it when given
		...
		while(Token::Match(tok1->tokAt(-2), "%type% ::") ||
		    (Token::simpleMatch(tok1->tokAt(-2),
		    "> ::") && tok1->linkAt(-2) && Token::Match(tok1->linkAt(-2)->tokAt(-1), "%type%"))) {
			if(tok1->strAt(-1) == "::")
				tok1 = tok1->tokAt(-2);
			else
...
void Scope::findFunctionInBase(const std::string & name, nonneg int args, std::vector<const Function *> & matches) const
{
	if(isClassOrStruct() && definedType && !definedType->derivedFrom.empty()) {
		...
		for(const Type::BaseInfo & i : derivedFrom) {
			const Type * base = i.type;
			if(base && base->classScope) {
				if(base->classScope == this) // Ticket #5120, #5125: Recursive class; tok should have
					                     // been found already
					continue;

				auto range = base->classScope->functionMap.equal_range(name);
				for(std::multimap<std::string, const Function*>::const_iterator it = range.first; it != range.second;
				    ++it) {
					const Function * func = it->second;
...
static void checkVariableCallMatch(const Variable* callarg, const Variable* funcarg, size_t& same, size_t& fallback1, size_t& fallback2)
{
	if(callarg) {
		...
		bool constEquals = !callarg->isArrayOrPointer() ||
		    ((callarg->typeStartToken()->strAt(-1) == "const") == (funcarg->typeStartToken()->strAt(-1) == "const"));
...
const Function* Scope::findFunction(const Token * tok, bool requireConst) const
{
	...
	for(std::size_t i = 0; i < matches.size();) {
		...
		for(std::size_t j = 0; j < args; ++j) {
			...
			else if(funcarg->isStlStringType() && arguments[j]->valueType() && arguments[j]->valueType()->pointer == 1 &&
			    arguments[j]->valueType()->type == ValueType::Type::CHAR)
				fallback2++;
		...
		while(Token::Match(tok1->tokAt(-2), ">|%type% ::")) {
			if(tok1->strAt(-2) == ">") {
				if(tok1->linkAt(-2))
					...
					if(mSettings->debugwarnings)
						debugMessage(tok1->tokAt(-2),
						    "debug",
						    "SymbolDatabase::findFunction found '>' without link.");
					return nullptr;
				}
			}
			else
...
void SymbolDatabase::setValueType(Token * tok, const ValueType &valuetype)
{
	...
	if(vt1 && vt1->smartPointerType &&
	    Token::Match(parent, ". %name% (") && parent->originalName() == "->" && !parent->next()->function()) {
	...
	if(parent->str() == "[" && (!mIsCpp || parent->astOperand1() == tok) && valuetype.pointer > 0U &&
	    !Token::Match(parent->previous(), "[{,]")) {
	...
	if(vt2 &&
	    parent->str() == ":" &&
	    Token::Match(parent->astParent(), "( const| auto *|&| %var% :") && // TODO: east-const, multiple const, ref
	                                                                       // to ptr, rvalue ref
		...
		if(vt2->pointer) {
			...
			if(setType) {
				// Type of "auto" has been determined.. set type information for "auto" and variable
				// tokens
				setValueType(autoToken, autovt);
				setAutoTokenProperties(autoToken);
	...
	if(ternary || parent->isArithmeticalOp() || parent->tokType() == Token::eIncDecOp) {
		...
		if(parent->str() == "+" && vt1->type == ValueType::Type::CONTAINER && vt2 && vt2->type == ValueType::Type::CONTAINER &&
		    vt1->container == vt2->container) {
			setValueType(parent, *vt1);
			return;
		}
	}
	...
	if(vt1->isIntegral() && vt1->pointer == 0U &&
	    ...
	    (ternary || parent->isArithmeticalOp() || parent->tokType() == Token::eBitOp || parent->tokType() == Token::eIncDecOp ||
	    parent->isAssignmentOp())) {
		ValueType vt;
...
static const Token * parsedecl(const Token * type, ValueType * const valuetype, ValueType::Sign defaultSignedness, const Settings* settings)
{
		...
		if(valuetype->type == ValueType::Type::UNKNOWN_TYPE &&
			...
			if(container) {
				...
				if(valuetype->typeScope)
					valuetype->type =
					    (scope->type == Scope::ScopeType::eClass) ? ValueType::Type::RECORD : ValueType::Type::NONSTD;
			}
		}
	...
	return (type &&
	       (valuetype->type != ValueType::Type::UNKNOWN_TYPE || valuetype->pointer > 0 ||
	       valuetype->reference != Reference::None)) ? type : nullptr;
}
		...
		if(tok->isNumber()) {
			if(MathLib::isFloat(tok->str())) {
				...
				if(mSettings->platformType != cppcheck::Platform::Unspecified) {
					...
					else if(type <= ValueType::Type::INT && !MathLib::isDec(tokStr) &&
					    mSettings->isIntValue(value >> 2)) {
						type = ValueType::Type::INT;
						sign = ValueType::Sign::UNSIGNED;
					}
					else if(type <= ValueType::Type::LONG &&
					    mSettings->isLongValue(unsignedSuffix ? (value >> 1) : value))
						type = ValueType::Type::LONG;
					else if(type <= ValueType::Type::LONG && !MathLib::isDec(tokStr) &&
					    mSettings->isLongValue(value >> 2)) {
						type = ValueType::Type::LONG;
						sign = ValueType::Sign::UNSIGNED;
					}
			...
			// library type/function
				...
				if(tok->astParent() && Token::Match(tok->astOperand1(), "%name%|::")) {
					...
					if((e == "std::make_shared" || e == "std::make_unique") &&
					    Token::Match(tok->astOperand1(), ":: %name% < %name%")) {
						ValueType vt;
				...
				if(typestr.empty() || typestr == "iterator") {
					if(Token::simpleMatch(tok->astOperand1(), ".") &&
						...
						const std::map<std::string,
						    Library::Container::Function>::const_iterator it = cont->functions.find(
							tok->astOperand1()->astOperand2()->str());
			...
			while(functionScope && functionScope->isExecutable() && functionScope->type != Scope::eLambda &&
			    functionScope->type != Scope::eFunction)
...
std::string ValueType::str() const
{
	...
	if(type == VOID_T)
		...
		while(scope && scope->type != Scope::eGlobal) {
			...
			scope =
			    (scope->definedType &&
			    scope->definedType->enclosingScope) ? scope->definedType->enclosingScope : scope->nestedIn;
		}
		ret += ' ' + className;
	}
...
ValueType::MatchResult ValueType::matchParameter(const ValueType * call, const ValueType * func)
{
	...
	if(call->pointer != func->pointer) {
		if(call->pointer > 1 && func->pointer == 1 && func->type == ValueType::Type::VOID_T)
			return ValueType::MatchResult::FALLBACK1;
		if(call->pointer == 1 && func->pointer == 0 && func->isIntegral() && func->sign != ValueType::Sign::SIGNED)
			return ValueType::MatchResult::FALLBACK1;
		if(call->pointer == 1 && call->type == ValueType::Type::CHAR && func->pointer == 0 && func->container &&
		    func->container->stdStringLike)
	...
	if(call->type != func->type) {
		if(call->type == ValueType::Type::VOID_T || func->type == ValueType::Type::VOID_T)
			return ValueType::MatchResult::FALLBACK1;
		if(call->pointer > 0)
	...
	if(func->typeScope != nullptr && func->container != nullptr) {
		if(func->type < ValueType::Type::VOID_T || func->type == ValueType::Type::UNKNOWN_INT)
			return ValueType::MatchResult::UNKNOWN;
	}

	if(call->isIntegral() && func->isIntegral() && call->sign != ValueType::Sign::UNKNOWN_SIGN &&
	    func->sign != ValueType::Sign::UNKNOWN_SIGN && call->sign != func->sign)
		return ValueType::MatchResult::FALLBACK1;
