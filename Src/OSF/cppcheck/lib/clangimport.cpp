...
struct Data {
	struct Decl {
		...
		Decl(Token * def, Enumerator * enumerator) : def(def), enumerator(enumerator), function(nullptr), scope(nullptr), var(
				nullptr) {
		}
...
public:
	...
	AstNodePtr getChild(int c) {
		if(c >= children.size()) {
			...
			err << "ClangImport: AstNodePtr::getChild(" << c << ") out of bounds. children.size=" << children.size() << " " <<
				    nodeType;
...
void clangimport::AstNode::createTokensFunctionDecl(TokenList * tokenList)
{
	...
	for(int i = 0; i < children.size(); ++i) {
		...
		if(!prev) {
			function->argumentList.emplace_back(vartok,
			    child->getType(),
			    nullptr,
			    typeEndToken,
			    i,
...
void clangimport::AstNode::createTokensForCXXRecord(TokenList * tokenList)
{
	...
	if(isDefinition()) {
		...
		Scope * scope = createScope(tokenList,
			isStruct ? Scope::ScopeType::eStruct : Scope::ScopeType::eClass,
			children2,
			classToken);
