...
static bool isNonReferenceArg(const Token * tok)
{
	...
	return (var && var->isArgument() && !var->isReference() &&
	       (var->isPointer() || (var->valueType() && var->valueType()->type >= ValueType::Type::CONTAINER) || var->type()));
}
...
static bool variableIsUsedInScope(const Token* start, nonneg int varId, const Scope * scope)
{
	...
	for(const Token * tok = start; tok && tok != scope->bodyEnd; tok = tok->next()) {
		...
		if(scopeType == Scope::eFor || scopeType == Scope::eDo || scopeType == Scope::eWhile) // In case of
			                                                                              // loops, better
			                                                                              // checking would
			                                                                              // be necessary
			return true;
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok, "[;{}] %var% = & %var%") && isRefPtrArg(tok->next()) && isAutoVar(tok->tokAt(4))) {
			    ...
			    "[;{}] * %var% =") && isPtrArg(tok->tokAt(2)) && isAddressOfLocalVariable(tok->tokAt(3)->astOperand2())) {
				checkAutoVariableAssignment(tok->next(), false);
			}
			else if(Token::Match(tok,
			    "[;{}] %var% . %var% =") && isPtrArg(tok->next()) && isAddressOfLocalVariable(tok->tokAt(4)->astOperand2())) {
				checkAutoVariableAssignment(tok->next(), false);
			}
			    ...
			    (isPtrArg(tok->next()) || isArrayArg(tok->next())) &&
			    isAddressOfLocalVariable(tok->linkAt(2)->next()->astOperand2())) {
				...
				if(isArrayVar(tok) || tok->tokType() == Token::eString)
					...
					for(const ValueFlow::Value &v : tok->values()) {
						...
						if(isArrayVar(v.tokvalue) ||
						    ((v.tokvalue->tokType() == Token::eString) && !v.isImpossible())) {
...
bool CheckAutoVariables::checkAutoVariableAssignment(const Token * expr, bool inconclusive, const Token * startToken)
{
	...
	for(const Token * tok = startToken; tok; tok = tok->next()) {
		...
		if(Token::simpleMatch(tok, "if (")) {
			...
			return checkAutoVariableAssignment(expr, inconclusive, ifStart) || checkAutoVariableAssignment(expr,
				   inconclusive,
				   ifStart->link()->next());
		}
...
{
	reportError(tok, Severity::error, "returnAddressOfAutoVariable", "Address of an auto-variable returned.", CWE562,
	    Certainty::normal);
}
...
static bool isDeadTemporary(bool cpp, const Token* tok, const Token* expr, const Library* library)
{
	...
	if(expr) {
		...
		if(astIsRHS(tok) &&
		    ...
		    ":") && Token::simpleMatch(parent->astParent(), "(") && Token::simpleMatch(parent->astParent()->previous(), "for (")) {
				...
				if(val.isLocalLifetimeValue()) {
					if(escape) {
						...
						if(!isLifetimeBorrowed(tok, mSettings))
							continue;
						if(tokvalue->exprId() == tok->exprId() &&
						    !(tok->variable() && tok->variable()->isArray()) &&
