...
ErrorMessage::ErrorMessage(const std::list<FileLocation> &callStack,
...
std::string ErrorMessage::serialize() const
{
	...
	for(std::list<ErrorMessage::FileLocation>::const_iterator loc = callStack.begin(); loc != callStack.end(); ++loc) {
		...
		smallStream << (*loc).line << '\t' << (*loc).column << '\t' << (*loc).getfile(false) << '\t' << loc->getOrigFile(false) <<
		    '\t' << loc->getinfo();
...
bool ErrorMessage::deserialize(const std::string &data)
{
	...
	while(iss.good()) {
		...
		// (*loc).line << '\t' << (*loc).column << '\t' << (*loc).getfile(false) << '\t' <<
		// loc->getOrigFile(false) << '\t' << loc->getinfo();
...
std::string ErrorMessage::toString(bool verbose, const std::string &templateFormat, const std::string &templateLocation) const
{
	...
	// Support a few special characters to allow to specific formatting, see
	// http://sourceforge.net/apps/phpbb/cppcheck/viewtopic.php?f=4&t=494&sid=21715d362c0dbafd3791da4d9522f814
	...
	while(pos1 != std::string::npos) {
		...
		const std::string replaceWith =
		    (certainty == Certainty::inconclusive) ? result.substr(pos1+14, pos2-pos1-14) : std::string();
	...
	if(!callStack.empty()) {
		...
		if(result.find("{code}") != std::string::npos) {
			...
			if(pos == std::string::npos)
				...
				endl = "\r";
			findAndReplace(result, "{code}",
			    readCode(callStack.back().getOrigFile(), callStack.back().line, callStack.back().column, endl));
		}
	}
	else {
...
bool ErrorLogger::reportUnmatchedSuppressions(const std::list<Suppressions::Suppression> &unmatched)
{
	...
	for(const Suppressions::Suppression &s : unmatched) {
		...
		reportErr(ErrorMessage(callStack, emptyString, Severity::information, "Unmatched suppression: " + s.errorId,
		    "unmatchedSuppression", Certainty::normal));
		err = true;
	}
	return err;
}
...
ErrorMessage::FileLocation::FileLocation(const Token* tok, const TokenList* tokenList)
	: fileIndex(tok->fileIndex()), line(tok->linenr()), column(tok->column()), mOrigFileName(tokenList->getOrigFile(tok)), mFileName(tokenList->file(
		    tok))
{
}
