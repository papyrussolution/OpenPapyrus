...
void CheckInternal::checkRedundantNextPrevious()
{
	...
	for(const Scope* scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok,
			    "previous ( ) . next|tokAt|strAt|linkAt (") || Token::Match(tok, "next ( ) . previous|tokAt|strAt|linkAt (") ||
			    (Token::simpleMatch(tok,
			    "tokAt (") && Token::Match(tok->linkAt(1), ") . previous|next|tokAt|strAt|linkAt|str|link ("))) {
...
void CheckInternal::checkStlUsage()
{
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			//if (Token::simpleMatch(tok, ". back ( )") && tok->astOperand1() &&
			// tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->container &&
			// Token::simpleMatch(tok->astOperand1()->valueType()->container, "std :: string"))
			//  reportError(tok, Severity::error, "internalStlUsage", "The 'std::string::back()' function
			// shall be avoided for now.");
		}
	}
}
