...
private:
	...
	void argumentSize();
	void argumentSizeError(const Token * tok,
	...
	void negativeMemoryAllocationSizeError(const Token* tok); // provide a negative value to memory allocation
	                                                          // function
...
public:
	...
	static bool analyseWholeProgram1(const std::map<std::string, std::list<const CTU::FileInfo::CallBase *> > &callsMap,
