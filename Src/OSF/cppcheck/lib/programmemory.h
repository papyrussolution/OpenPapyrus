...
struct ExprIdToken {
	...
	bool operator==(const ExprIdToken& rhs) const { return getExpressionId() == rhs.getExpressionId(); }
	template <class T, class U> friend bool operator!=(const T& lhs, const U& rhs) { return !(lhs == rhs); }
	struct Hash {
...
struct ProgramMemory {
	...
	Map::iterator begin() { return mValues.begin(); }
	Map::iterator end() { return mValues.end(); }
	Map::const_iterator begin() const { return mValues.begin(); }
	Map::const_iterator end() const { return mValues.end(); }
private:
	Map mValues;
};
...
void execute(const Token* expr, ProgramMemory* const programMemory, MathLib::bigint* result, bool* error, const Settings* settings = nullptr);
/**
