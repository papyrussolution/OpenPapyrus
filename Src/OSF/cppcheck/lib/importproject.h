...
namespace cppcheck {
struct stricmp {
	bool operator()(const std::string &lhs, const std::string &rhs) const { return caseInsensitiveStringCompare(lhs, rhs) < 0; }
};
}
...
public:
	...
	struct CPPCHECKLIB FileSettings {
		...
		void setIncludePaths(const std::string &basepath, const std::list<std::string> &in, std::map<std::string, std::string,
...
private:
	...
	bool importVcxproj(const std::string &filename,
