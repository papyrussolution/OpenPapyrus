...
void TokenList::determineCppC()
{
	if(!mSettings) {
		...
		mIsC = mSettings->enforcedLang == Settings::C ||
		    (mSettings->enforcedLang == Settings::None && Path::isC(getSourceFilePath()));
		mIsCpp = mSettings->enforcedLang == Settings::CPP ||
		    (mSettings->enforcedLang == Settings::None && Path::isCPP(getSourceFilePath()));
	}
...
struct AST_state {
	...
	explicit AST_state(bool cpp) : depth(0), inArrayAssignment(0), cpp(cpp), assign(0), inCase(false), stopAtColon(false),
		functionCallEndPar(nullptr) {
	}
};
...
static void compileTerm(Token *&tok, AST_state& state)
{
	...
	if(Token::Match(tok, "L %str%|%char%"))
		tok = tok->next();
	if(state.inArrayAssignment && Token::Match(tok->previous(), "[{,] . %name%")) { // Jump over . in C style struct
		                                                                        // initialization
		state.op.push(tok);
	...
	if(tok->isLiteral()) {
		...
		if(Token::Match(tok, "return|case") || (state.cpp && tok->str() == "throw")) {
			...
			if(Token::Match(tok, "%name% <") && tok->linkAt(1))
			    ...
			    "%name% ...") ||
			    (state.op.size() == 1 && state.depth == 0 && Token::Match(tok->tokAt(-3), "!!& ) ( %name% ) =")))
				tok = tok->next();
			tok = tok->next();
	...
	while(tok) {
		if(tok->tokType() == Token::eIncDecOp && !isPrefixUnary(tok, state.cpp)) {
			...
			if((oldOpSize > 0 && Token::simpleMatch(tok->previous(), "} ("))
			    ...
			    !Token::Match(tok->previous(), "return|case") && (!state.cpp || !Token::Match(tok->previous(), "throw|delete")))
			    || (tok->strAt(-1) == "]" && (!state.cpp || !Token::Match(tok->linkAt(-1)->previous(), "new|delete")))
			    || (tok->strAt(-1) == ">" && tok->linkAt(-1))
			    || (tok->strAt(-1) == ")" && !iscast(tok->linkAt(-1), state.cpp)) // Don't treat brackets to
			                                                                      // clarify precedence as
			                                                                      // function calls
		    ...
		    state.cpp) && Token::simpleMatch(tok->link(), ") {") && Token::simpleMatch(tok->link()->linkAt(1), "} [")) {
			Token * cast = tok;
			tok = tok->link()->next();
			Token * tok1 = tok;
			compileUnaryOp(tok, state, compileExpression);
	...
	while(tok) {
		if((Token::Match(tok, "[+-!~*&]") || tok->tokType() == Token::eIncDecOp) &&
			...
			if(tok->str() == "(") {
				if(Token::Match(tok,
				    "( &| %name%") &&
				    Token::Match(tok->link(), ") ( %type%") && Token::simpleMatch(tok->link()->linkAt(1), ") ("))
					tok = tok->link()->next();
...
static void compileAssignTernary(Token *&tok, AST_state& state)
{
	...
	while(tok) {
		if(tok->isAssignmentOp()) {
			...
			// as if parenthesized: its precedence relative to ?: is ignored."
			// Hence, we rely on Tokenizer::prepareTernaryOpForAST() to add such parentheses where
			// necessary.
			const bool stopAtColon = state.stopAtColon;
...
static Token * createAstAtToken(Token * tok, bool cpp)
{
		...
		if(tok2 == tok->tokAt(2) && Token::Match(tok2, "%op%|(")) {
			...
			while(tok2 && tok2 != endPar && tok2->str() != ";") {
				if(tok2->str() == "<" && tok2->link()) {
				    ...
				    "%name% )| %op%|(|[|.|:|::") || Token::Match(tok2->previous(), "[(;{}] %cop%|(")) {
					init1 = tok2;
					AST_state state1(cpp);
					compileExpression(tok2, state1);
					if(Token::Match(tok2, ";|)"))
	...
	if(Token::Match(tok, "return|case") ||
		...
		if(cpp &&
		    (Token::Match(tok->tokAt(-2), "[;{}] new|delete %name%") || Token::Match(tok->tokAt(-3), "[;{}] :: new|delete %name%")))
			tok = tok->previous();
...
void TokenList::validateAst() const
{
	...
	for(const Token * tok = mTokensFrontBack.front; tok; tok = tok->next()) {
		...
		if((tok->isAssignmentOp() || tok->isComparisonOp() ||
		    Token::Match(tok, "[|^/%]")) && tok->astOperand1() && !tok->astOperand2())
			...
			do {
				...
				if(astTokens.find(parent) != astTokens.end())
					throw InternalError(tok,
						  "AST broken: endless recursion from '" + tok->str() + "'",
						  InternalError::AST);
		...
		if(Token::Match(tok, "%or%|%oror%|%assign%|%comp%")) {
			...
			if(!tok->astOperand1() || !tok->astOperand2())
				throw InternalError(tok,
					  "Syntax Error: AST broken, binary operator '" + tok->str() + "' doesn't have two operands.",
					  InternalError::AST);
		}
		...
		if(Token::Match(tok, "%var% .")) {
			if(!tok->astParent()) {
				throw InternalError(
					      tok,
					      "Syntax Error: AST broken, '" + tok->str() + "' doesn't have a parent.",
					      InternalError::AST);
			}
...
void TokenList::simplifyStdType()
{
	for(Token * tok = front(); tok; tok = tok->next()) {
		...
		if(Token::Match(tok,
		    "char|short|int|long|unsigned|signed|double|float") ||
		    (mSettings->standards.c >= Standards::C99 && Token::Match(tok, "complex|_Complex"))) {
			bool isFloat = false;
			bool isSigned = false;
			...
			for(; tok2->next(); tok2 = tok2->next()) {
				if(tok2->str() == "long") {
					...
					isComplex = !isFloat || tok2->str() == "_Complex" || Token::Match(tok2->next(), "*|&|%name%"); // Ensure
				                                                                                                       // that
				                                                                                                       // "complex"
				                                                                                                       // is
				                                                                                                       // not
			...
			if(!typeSpec) { // unsigned i; or similar declaration
				...
				while(tok3 != tok2) {
					if(tok2 != typeSpec &&
					    (isComplex || !Token::Match(tok2, "complex|_Complex"))) // Ensure that
						                                                    // "complex" is not
						                                                    // the variables
						                                                    // name
						tok2->deleteThis();
