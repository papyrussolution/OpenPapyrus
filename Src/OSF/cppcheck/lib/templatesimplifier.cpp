		...
		if(Token::simpleMatch(tok, "template <")) {
			...
			if(tok->strAt(1) == "(") {
				...
				for(auto pos = range.first; pos != range.second; ++pos) {
					// look for declaration with same qualification or constructor with same
					// qualification
					if(pos->second->fullName() == fullName ||
						...
						for(size_t i = 0; i < declarationParams.size(); ++i) {
							...
							bool isArgLiteral = Token::Match(instantiationArgs[i],
								"%num%|%str%|%char%|%bool% ,|)");
							if(isArgLiteral &&
							    Token::Match(declarationParams[i], "const| %type% &| %name%| ,|)")) {
								match++;
								...
								if(templateParams[0]->str() == declarationParams[i]->str())
									...
									// todo: check if non-template args match for
									// function overloads
								}
							}
						}
						...
						if(match == declarationParams.size()) {
							...
							switch(arg->tokType()) {
								...
								case Token::eNumber: {
								    ...
								    if(num.isFloat()) {
									    // MathLib::getSuffix doesn't work for
					...
					if(it != mTemplateDeclarations.end()) {
						...
						for(const auto & nameSpace :  tok->scopeInfo()->usingNamespaces) {
							...
							const std::list<TokenAndName>::const_iterator it1 = std::find_if(
								mTemplateDeclarations.begin(),
								mTemplateDeclarations.end(),
								FindFullName(newFullName));
							if(it1 != mTemplateDeclarations.end()) {
								...
								while((pos = nameSpace.substr(offset).find(' ')) != std::string::npos) {
									qualificationTok->insertToken(nameSpace.substr(offset, pos),
									    "",
									    true);
									offset = offset + pos + 1;
								}
...
void TemplateSimplifier::simplifyTemplateAliases()
{
	for(std::list<TokenAndName>::iterator it1 = mTemplateDeclarations.begin(); it1 != mTemplateDeclarations.end();) {
		...
		for(std::list<TokenAndName>::iterator it2 = mTemplateInstantiations.begin(); it2 != mTemplateInstantiations.end();) {
			...
			Token * end = TokenList::copyTokens(dst,
				aliasDeclaration.aliasStartToken(),
				aliasDeclaration.aliasEndToken()->previous(),
				false)->next();
...
void TemplateSimplifier::addNamespace(const TokenAndName &templateDeclaration, const Token * tok)
{
	...
	if(token != tokStart->str() || tok->strAt(-1) != "::") {
		if(insert) {
			...
			if(!inTemplate)
				mTokenList.addtoken(templateDeclaration.scope().substr(start),
				    tok->linenr(),
				    tok->column(),
				    tok->fileIndex());
			else
    ...
    bool copy)
{
	...
	if(copy && isClass) {
		...
		while(start && start != end) {
			...
			if(itype < typeParametersInDeclaration.size() && itype < mTypesUsedInTemplateInstantiation.size() &&
				...
				const bool isVariadicTemplateArg = templateDeclaration.isVariadic() &&
				    itype + 1 == typeParametersInDeclaration.size();
				...
				for(const Token * typetok = mTypesUsedInTemplateInstantiation[itype].token();
					...
					if(Token::Match(typetok,
					    "%name% <") && (typetok->strAt(2) == ">" || templateParameters(typetok->next())))
						++typeindentlevel;
	...
	for(Token * tok3 = mTokenList.front(); tok3; tok3 = tok3 ? tok3->next() : nullptr) {
		...
		const std::string lastName =
		    (templateInstantiation.name().find(' ') != std::string::npos) ? templateInstantiation.name().substr(
			templateInstantiation.name().rfind(' ')+1) : templateInstantiation.name();
			...
			// link() newly tokens manually
				...
				if(tok3->str() == "{") {
					...
					if(tok3->strAt(1) == ";") {
						...
						mTokenList.addtoken(tokSemicolon,
						    tokSemicolon->linenr(),
						    tokSemicolon->column(),
						    tokSemicolon->fileIndex());
					}
					brackets.pop();
...
bool TemplateSimplifier::simplifyNumericCalculations(Token * tok, bool isTemplate)
{
	...
	while(tok->tokAt(3) && tok->isNumber() && tok->tokAt(2)->isNumber()) { // %any% %num% %any% %num% %any%
		...
		if(Token::Match(before, "* %num% /") && (num2 != "0") && num1 == MathLib::multiply(num2, MathLib::divide(num1, num2))) {
		    ...
		    (Token::Match(op, "[/%]") && isLowerThanMulDiv(before) && isLowerEqualThanMulDiv(after)) ||  // NOT
		                                                                                                 // associative
		    (Token::Match(op, "[+-]") && isLowerThanMulDiv(before) && isLowerThanMulDiv(after)) ||  // Only
		                                                                                            ...
		                                                                                            // handled
		                                                                                            // later
		    (Token::Match(op, ">>|<<") && isLowerThanShift(before) && isLowerThanPlusMinus(after)) ||  // NOT
		                                                                                               // associative
		...
		if(Token::Match(op, "[/%] 0")) {
			if(isTemplate) throw InternalError(op,
					  "Instantiation error: Divide by zero in template instantiation.",
					  InternalError::INSTANTIATION);
			else return ret;
		}
    ...
    std::set<std::string> &expandedtemplates)
{
	...
	for(const TokenAndName &instantiation : mTemplateInstantiations) {
		...
		if(numberOfTemplateInstantiations != mTemplateInstantiations.size()) {
			...
			if(recursiveCount > mSettings->maxTemplateRecursion) {
				std::list<std::string> typeStringsUsedInTemplateInstantiation;
				const std::string typeForNewName = templateDeclaration.name() + "<" + getNewName(instantiation.token(),
		...
		if(Token::Match(startToken->previous(), ";|{|}|=|const") &&
		    (!specialized &&
		    !instantiateMatch(tok2, typeParametersInDeclaration.size(), templateDeclaration.isVariadic(),
	...
	if(!instantiated && specialized) {
		...
		if(expandedtemplates.insert(newFullName).second) {
			expandTemplate(templateDeclaration,
			    templateDeclaration,
				...
				if(forwardDecl.fullName() == decl.fullName()) {
					...
					for(size_t k = 0; k < params1.size(); k++) {
						...
						if(params1[k]->strAt(1) == "=" && params2[k]->strAt(1) != "=") {
							...
							if(end)
								TokenList::copyTokens(const_cast<Token *>(params2[k]),
								    params1[k]->next(),
								    end->previous());
						}
					}
		...
		while(!mMemberFunctionsToDelete.empty()) {
			...
			if(it != mTemplateDeclarations.end()) {
				...
				const std::list<TokenAndName>::iterator it1 = std::find_if(mTemplateForwardDeclarations.begin(),
					mTemplateForwardDeclarations.end(),
					FindToken(mMemberFunctionsToDelete.begin()->token()));
				// multiple functions can share the same declaration so make sure it hasn't already been
				// deleted
				if(it1 != mTemplateForwardDeclarations.end()) {
