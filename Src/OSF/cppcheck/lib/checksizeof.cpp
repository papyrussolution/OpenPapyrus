		...
		for(const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok->tokAt(2), "malloc|alloca|calloc (")) {
				if(Token::Match(tok, "%var% ="))
				    ...
				    Token::Match(tok, "> ( malloc|alloca|calloc (") && Token::Match(tok->link()->tokAt(-3), "%var% ="))
				...
				for(const Token* tok2 = tokSize; tok2 != paramsListEndTok; tok2 = tok2->next()) {
					if(Token::simpleMatch(tok2, "/ sizeof")) {
						...
						auto hasMultiplication = [](const Token* parTok) -> bool {
							    while(parTok) { // Allow division if followed by
								            // multiplication
								    if(parTok->isArithmeticalOp() && parTok->str() == "*") {
									    for(const Token* szTok :
										{ parTok->astOperand1(), parTok->astOperand2() })
										    if(Token::simpleMatch(szTok,
											"(") &&
											Token::simpleMatch(szTok->previous(), "sizeof"))
											    return true;
								    }
			...
			for(;
			    tokSize && tokSize->str() != ")" && tokSize->str() != "," && tokSize->str() != "sizeof";
			...
			if(tokSize->linkAt(1)->strAt(-1) == "*") {
				if(variable && variable->valueType() && variable->valueType()->pointer == 1 && variable->valueType()->type != ValueType::VOID_T)
					sizeofForPointerError(variable, variable->str());
				else if(variable2 && variable2->valueType() && variable2->valueType()->pointer == 1 && variable2->valueType()->type != ValueType::VOID_T)
			...
			// Now check for the sizeof usage again. Once here, everything using sizeof(varid) or
			// sizeof(&varid)
			// looks suspicious
		...
		if(Token::simpleMatch(tok, "sizeof (")) {
			...
			if(lPar && lPar->str() == "(") {
				...
				if(lPar->astParent() && lPar->astParent()->str() == "/") {
					...
					if(var && var->isPointer() && !var->isArray() &&
					    !(var->valueType() && var->valueType()->pointer <= derefCount))
...
void CheckSizeof::sizeofVoid()
{
	...
	for(const Token * tok = mTokenizer->tokens(); tok; tok = tok->next()) {
		if(Token::simpleMatch(tok, "sizeof ( void )")) {
			...
			if(vt && vt->type == ValueType::Type::VOID_T && vt->pointer == 0U)
			...
			const bool op1IsvoidPointer = (vt1 && vt1->type == ValueType::Type::VOID_T && vt1->pointer == 1U);
			...
			const bool voidpointer1 = (vt1 && vt1->type == ValueType::Type::VOID_T && vt1->pointer == 1U);
			const bool voidpointer2 = (vt2 && vt2->type == ValueType::Type::VOID_T && vt2->pointer == 1U);
