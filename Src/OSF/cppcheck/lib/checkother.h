...
public:
	...
	/** @brief %Check for suspicious code where if and else branch are the same (e.g "if (a) b = true; else b =
	   true;") */
	...
	/** @brief %Check that variadic function calls don't use NULL. If NULL is \#defined as 0 and the function
	   expects a pointer, the behaviour is undefined. */
...
private:
	...
	void duplicateExpressionError(const Token * tok1,
	    const Token * tok2,
	    const Token * opTok,
	    ErrorPath errors,
	...
	void funcArgOrderDifferent(const std::string & functionName, const Token * declaration, const Token * definition,
	    const std::vector<const Token*> & declarations, const std::vector<const Token*> & definitions);
