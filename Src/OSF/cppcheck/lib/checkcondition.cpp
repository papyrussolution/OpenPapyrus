    ...
    const MathLib::bigint num)
{
	...
	for(const Token * tok2 = startTok; tok2; tok2 = tok2->next()) {
		if((bitop == '&') &&
		    Token::Match(tok2->tokAt(2), "%varid% %cop% %num% ;", varid) && tok2->strAt(3) == std::string(1U, bitop)) {
		...
		if(Token::Match(tok2, "if|while (")) {
			...
			if(tok2->str() == "while") {
				...
				if(!bodyEnd || bodyEnd->str() != "}" ||
				    isVariableChanged(bodyStart, bodyEnd, varid, !islocal, mSettings, mTokenizer->isCPP()))
					continue;
			}
	...
	for(const Token * tok = mTokenizer->tokens(); tok; tok = tok->next()) {
		if(tok->str() == "|" && tok->astOperand1() && tok->astOperand2() && tok->astParent()) {
			...
			const bool isBoolean = Token::Match(parent, "&&|%oror%") ||
			    ...
			    (parent->str() == "=" && parent->astOperand2() == tok && parent->astOperand1() &&
			    parent->astOperand1()->variable() &&
			    Token::Match(parent->astOperand1()->variable()->typeStartToken(), "bool|_Bool")) ||
			    (parent->str() == "(" && Token::Match(parent->astOperand1(), "if|while")) ||
			    (parent->str() == "return" && parent->astOperand1() == tok && inBooleanFunction(tok));

			const bool isTrue =
			    (tok->astOperand1()->hasKnownIntValue() && tok->astOperand1()->values().front().intvalue != 0) ||
			...
			if((isZero1 || isZero2) && !tok->isExpandedMacro() && !(isZero1 && tok->astOperand1()->isExpandedMacro()) &&
			    !(isZero2 && tok->astOperand2()->isExpandedMacro()))
...
void CheckCondition::multiCondition()
{
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		for(;;) {
			...
			if(tok2->astOperand2()) {
				ErrorPath errorPath;
				if(isOverlappingCond(cond1, tok2->astOperand2(),
				    true) && !isExpressionChanged(cond1, cond1, tok2->astOperand2(), mSettings, mTokenizer->isCPP()))
					overlappingElseIfConditionError(tok2->astOperand2(), cond1->linenr());
				else if(isOppositeCond(true, mTokenizer->isCPP(), cond1, tok2->astOperand2(), mSettings->library, true,
				    true, &errorPath) &&
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		visitAstNodes(condTok,
			...
			if(cond->varId()) {
				...
				nonlocal =
				Token::Match(cond->astParent(),
				"%cop%|(|[") || Token::Match(cond, "%name% .") || (mTokenizer->isCPP() && cond->str() == "this");
			}
			else {
		...
		for(MULTICONDITIONTYPE type:types) {
			...
			for(; tok && tok != endToken; tok = tok->next()) {
				...
				if(Token::Match(tok, "if|return")) {
					...
					const Token * condEndToken = tok->str() == "if" ? condStartToken->link() : Token::findsimplematch(
						condStartToken,
						";");
					...
					// Condition..
					const Token * cond2 = tok->str() ==
					    "if" ? condStartToken->astOperand2() : condStartToken->astOperand1();
					...
					if(type == MULTICONDITIONTYPE::INNER) {
						visitAstNodes(cond1, [&](const Token* firstCondition) {
							...
							if(firstCondition->str() == "&&") {
								if(!isOppositeCond(false, mTokenizer->isCPP(), firstCondition, cond2,
								mSettings->library, true, true))
									return ChildrenToVisit::op1_and_op2;
							}
							if(!firstCondition->hasKnownIntValue()) {
								if(!isReturnVar &&
								isOppositeCond(false,
								mTokenizer->isCPP(),
								firstCondition,
								cond2,
								mSettings->library,
								...
								isSameExpression(mTokenizer->isCPP(), true, firstCondition, cond2,
						...
						visitAstNodes(cond2, [&](const Token * secondCondition) {
							...
							isSameExpression(mTokenizer->isCPP(), true, cond1, secondCondition,
							mSettings->library, true, true, &errorPath)) {
				...
				if(Token::Match(tok, "%type% (") && nonlocal && isNonConstFunctionCall(tok, mSettings->library)) // non
					                                                                                         // const
					                                                                                         // function
					                                                                                         // call
					                                                                                         // ->
				...
				if((tok->varId() && vars.find(tok->varId()) != vars.end()) ||
					...
					if(mTokenizer->isCPP() &&
					    Token::Match(tok, "%name% <<") && (!tok->valueType() || !tok->valueType()->isIntegral()))
						break;
					if(isLikelyStreamRead(mTokenizer->isCPP(),
					    tok->next()) || isLikelyStreamRead(mTokenizer->isCPP(), tok->previous()))
						break;
					if(Token::Match(tok, "%name% [")) {
		...
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(printStyle &&
				...
				if(isOppositeCond(true, mTokenizer->isCPP(), tok->astOperand1(), tok2, mSettings->library, true, false)) {
					...
					const std::string cond1 = expr1 + " " + tok->str() + " (" + expr2 + " " +
					    tok->astOperand2()->str() + " " + expr3 + ")";
					const std::string cond2 = expr1 + " " + tok->str() + " " + expr3;

					const std::string cond1VerboseMsg = expr1VerboseMsg + " " + tok->str() + " " + expr2VerboseMsg +
					    " " + tok->astOperand2()->str() + " " + expr3VerboseMsg;
				...
				else if(isSameExpression(mTokenizer->isCPP(), false, tok->astOperand1(), tok2, mSettings->library, true,
				    true)) {
					std::string expr1(tok->astOperand1()->expressionString());
					std::string expr2(tok->astOperand2()->astOperand1()->expressionString());
					std::string expr3(tok->astOperand2()->astOperand2()->expressionString());
			...
			const bool isfloat =
			    astIsFloat(expr1, true) || MathLib::isFloat(value1) || astIsFloat(expr2, true) || MathLib::isFloat(value2);
			...
			if(!isfloat &&
			    isOppositeCond(isLogicalOr, mTokenizer->isCPP(), tok->astOperand1(), tok->astOperand2(), mSettings->library,
			    true, true, &errorPath)) {
			...
			const bool useUnsignedInt = (std::numeric_limits<MathLib::bigint>::max()==i1) ||
			    (std::numeric_limits<MathLib::bigint>::max()==i2);
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			if(Token::Match(tok, "( %name% [=&|^]")) {
				...
				if(tok->astOperand2() && tok->astOperand2()->variable() &&
				    tok->astOperand2()->variable()->nameToken() == tok->astOperand2())
					continue;
				...
				if(vt1 && vt1->type == ValueType::BOOL &&
				    !Token::Match(tok->astOperand1(), "%name%|(|[|::|.") && countPar(tok->astOperand1(), tok) == 0)
					clarifyConditionError(tok, false, true);
				else if(vt2 && vt2->type == ValueType::BOOL &&
				    !Token::Match(tok->astOperand2(), "%name%|(|[|::|.") && countPar(tok, tok->astOperand2()) == 0)
					clarifyConditionError(tok, false, true);
			}
		}
	}
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			// do not write alwaysTrueFalse for these comparisons.
			{
				...
				if(CheckOther::comparisonNonZeroExpressionLessThanZero(tok, &zeroValue,
...
void CheckCondition::checkInvalidTestForOverflow()
{
	...
	for(const Token * tok = mTokenizer->tokens(); tok; tok = tok->next()) {
		...
		for(const Token * lhs: lhsTokens) {
			...
			const bool isSignedInteger = lhs->valueType() && lhs->valueType()->isIntegral() &&
			    lhs->valueType()->sign == ValueType::Sign::SIGNED;
			...
			for(const Token * expr: exprTokens) {
				...
				if((other->isNumber() && other->getKnownIntValue() > 0) ||
				    (!other->isNumber() && other->valueType() && other->valueType()->isIntegral() &&
				    other->valueType()->sign == ValueType::Sign::UNSIGNED)) {
					bool result;
					if(lhs->str() == "+")
						result = (cmp == ">" || cmp == ">=");
					else
...
void CheckCondition::invalidTestForOverflow(const Token* tok, const ValueType * valueType, const std::string &replace)
{
	...
	if(replace == "false" || replace == "true")
		errmsg += " Some mainstream compilers remove such overflow tests when optimising the code and assume it's always " +
		    replace + ".";
	else
		errmsg += " Some mainstream compilers removes handling of overflows when optimising the code and change the code to '" +
		    replace + "'.";
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(nextAfterAstRightmostLeaf(assignTok) != blockTok->link()->previous())
				continue;
			if(!isSameExpression(
				    mTokenizer->isCPP(), true, condTok->astOperand1(), assignTok->astOperand1(), mSettings->library, true,
				    true))
				continue;
			if(!isSameExpression(
				    mTokenizer->isCPP(), true, condTok->astOperand2(), assignTok->astOperand2(), mSettings->library, true,
				    true))
				continue;
...
void CheckCondition::duplicateConditionalAssignError(const Token * condTok, const Token* assignTok)
{
	...
	if(condTok && assignTok) {
		if(condTok->str() == "==") {
			msg = "Assignment '" + assignTok->expressionString() + "' is redundant with condition '" +
			    condTok->expressionString() + "'.";
			...
			msg = "The statement 'if (" + condTok->expressionString() + ") " + assignTok->expressionString() +
			    "' is logically equivalent to '" + assignTok->expressionString() + "'.";
		...
		for(const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			for(int i = 0; i < 2; ++i) {
				...
				if(!valueTok->hasKnownIntValue() || !typeTok->valueType() || typeTok->valueType()->pointer)
					continue;
				if(valueTok->getKnownIntValue() < 0 && valueTok->valueType() &&
				    valueTok->valueType()->sign != ValueType::Sign::SIGNED)
					continue;
				int bits = 0;
				...
				const auto typeMinValue =
				    (typeTok->valueType()->sign == ValueType::Sign::UNSIGNED) ? 0 : (-(1LL << (bits-1)));
				...
				else if(bits >= mSettings->int_bit &&
				    (!valueTok->valueType() || valueTok->valueType()->sign != ValueType::Sign::SIGNED))
					typeMaxValue = unsignedTypeMaxValue;
				else
