...
static bool isRaiiClass(const ValueType * valueType, bool cpp, bool defaultReturn = true)
{
	...
	if((valueType->smartPointerType && isRaiiClassScope(valueType->smartPointerType->classScope)) ||
	    (!valueType->smartPointerType && valueType->type == ValueType::Type::SMART_POINTER))
		return true;
						...
						if(addressOf ||
							...
							// not in same scope as declaration
								...
								if(var1->_assignments.find(scope) == var1->_assignments.end() ||
									...
									// nothing to replace
									// cppcheck-suppress duplicateBranch - remove
									// when TODO below is address
									if(var1->_assignments.empty())
										replace = false;

									// this variable has previous assignments
									else {
										// TODO: determine if existing aliases
								...
								// assignment in this scope
									...
									// replace when only one other assignment, merge
									// them otherwise
...
static bool isPartOfClassStructUnion(const Token* tok)
{
	for(; tok; tok = tok->previous()) {
		if(tok->str() == "}" || tok->str() == ")")
			...
			return (tok->strAt(-1) == "struct" || tok->strAt(-2) == "struct" || tok->strAt(-1) == "class" ||
			       tok->strAt(-2) == "class" || tok->strAt(-1) == "union" || tok->strAt(-2) == "union");
		}
	}
	return false;
}
	...
	if(scope->isExecutable()) {
		...
		for(std::list<Variable>::const_iterator i = scope->varlist.begin(); i != scope->varlist.end(); ++i) {
			...
			if(i->isArray() && (i->nameToken()->previous()->str() == "*" || i->nameToken()->strAt(-2) == "*"))
			    ...
			    !(i->valueType() && i->valueType()->type == ValueType::UNKNOWN_TYPE &&
			    Token::simpleMatch(i->typeStartToken(), "auto")))
			...
			if(Token::Match(i->nameToken()->previous(), "* %var% ) (")) // function pointer. Jump behind
				                                                    // parameter list.
			...
			if(i->isArray() && i->isClass() && // Array of class/struct members. Initialized by ctor except
			                                   // for std::array
			    !(i->isStlType() && i->valueType() && i->valueType()->containerTypeToken &&
			    i->valueType()->containerTypeToken->isStandardType()))
				variables.write(i->declarationId(), i->nameToken());
			if(i->isArray() && Token::Match(i->nameToken(), "%name% [ %var% ]")) // Array index variable
				                                                             // read.
			...
			if(defValTok && defValTok->next()) {
				...
				if(defValTok->str() == "=" && defValTok->next()->str() != "{") {
					...
					if(tokBraceStart) {
						for(const Token* tok = tokBraceStart->next();
						    tok && tok != tokBraceStart->link();
						    tok = tok->next()) {
							if(tok->varId()) {
	...
	for(; tok && tok != scope->bodyEnd; tok = tok->next()) {
		if(tok->str() == "{" && tok != scope->bodyStart && !tok->previous()->varId()) {
			for(const Scope * i : scope->nestedList) {
				if(i->bodyStart == tok) { // Find associated scope
					checkFunctionVariableUsage_iterateScopes(tok->scope(), variables); // Scan child
					                                                                   // scope
					tok = tok->link();
					break;
				}
		...
		if(tok->isName() && endsWith(tok->str(), '>')) {
			...
			for(const Token* tok2 = tok->next(); tok2; tok2 = tok2->next()) {
				if(tok2->varId()) {
					...
					else if(var->typeEndToken()->str() == ">") // Be careful with types like
						                                   // std::vector
						tok = tok->previous();
					break;
				}
		...
		// assignment
			...
			if(dereference) {
				...
				// Consider allocating memory separately because allocating/freeing alone does not
				// constitute using the variable
					...
					const Token * allocFuncCallToken = findAllocFuncCallToken(start->next()->astOperand2(),
						mSettings->library);
		...
		// assignment
			...
			const bool deref = eq && eq->astOperand1() && eq->astOperand1()->valueType() &&
			    eq->astOperand1()->valueType()->pointer == 0U;
			...
			if(var) {
				...
				if(var->mType == Variables::pointer &&
				    Token::Match(skipBrackets(tok->next()),
				    "= new|malloc|calloc|kmalloc|kzalloc|kcalloc|strdup|strndup|vmalloc|g_new0|g_try_new|g_new|g_malloc|g_malloc0|g_try_malloc|g_try_malloc0|g_strdup|g_strndup|g_strdup_printf"))
				{
		...
		// ++|--
		else if(tok->next() && tok->next()->tokType() == Token::eIncDecOp && tok->next()->astOperand1() &&
		    tok->next()->astOperand1()->varId()) {
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
			...
			const bool isInitialization =
			    (Token::Match(tok, "%var% (|{") && tok->variable() && tok->variable()->nameToken() == tok);
			...
			if(tok->str() == "=" && !(tok->valueType() && tok->valueType()->pointer) &&
			    isRaiiClass(tok->valueType(), mTokenizer->isCPP(), false))
				continue;
			...
			if(iteratorToken && iteratorToken->variable() &&
			    iteratorToken->variable()->typeEndToken()->str().find("iterator") != std::string::npos)
				continue;
			...
			if(op1Var) {
				...
				// Bailout for unknown template classes, we have no idea what side effects such
				// assignments have
				if(mTokenizer->isCPP() &&
				    op1Var->isClass() &&
			...
			if(tok->previous() && tok->previous()->variable() &&
			    tok->previous()->variable()->nameToken()->scope()->type == Scope::eUnion)
				continue;
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		if(const Preprocessor * preprocessor = mTokenizer->getPreprocessor()) {
			...
			for(const Directive &d: preprocessor->getDirectives()) {
				if(d.str == "#pragma pack(1)" && d.file == mTokenizer->list.getFiles().front() &&
				    d.linenr < scope.bodyStart->linenr()) {
					isPacked = true;
					break;
				}
			}
		...
		for(const Variable* var : symbolDatabase->variableList()) {
			if(var && (var->isExtern() || (var->isGlobal() && !var->isStatic())) &&
			    var->typeEndToken()->str() == scope.className) {
				bailout = true;
				break;
			}
			if(var && (var->typeStartToken()->str() == scope.className || var->typeEndToken()->str() == scope.className)) {
				const std::string addressPattern("!!" + scope.className + " & " + var->name()); // cast
				                                                                                // from
				                                                                                // struct
		...
		for(const Variable &var : scope.varlist) {
			...
			if(!var.typeStartToken()->isStandardType() && !var.isPointer() && !astIsContainer(var.nameToken()) &&
			    !isRecordTypeWithoutSideEffects(var.type()))
				continue;
...
bool CheckUnusedVar::isRecordTypeWithoutSideEffects(const Type* type)
{
	...
	for(const Function& f : type->classScope->functionList) {
		...
		if(Token::simpleMatch(nextToken, ") :")) {
			...
			for(const Token * initListToken = nextToken;
			    Token::Match(initListToken, "[:,] %var% [({]");
			    initListToken = initListToken->linkAt(2)->next()) {
				...
				for(const Token* valueToken = initListToken->tokAt(3);
				    valueToken != valueEnd;
	...
	for(Token* bodyToken = func.functionScope->bodyStart->next(); bodyToken != func.functionScope->bodyEnd;
		...
		if(bodyVariable) {
			...
			if(bodyVariable->isGlobal() || (pointersToGlobals.find(bodyVariable) != pointersToGlobals.end())) {
				...
				// check if pointer to global variable assigned to another variable (another_var =
				// &global_var)
		...
		if(bodyFunction) {
			if(std::find(checkedFuncs.begin(), checkedFuncs.end(), bodyFunction) != checkedFuncs.end()) { // recursion
				                                                                                      // found
				continue;
			}
