...
CheckMemoryLeak::AllocType CheckMemoryLeak::getAllocationType(const Token * tok2, nonneg int varid,
    std::list<const Function*> * callstack) const
{
	// What we may have...
	//     * var = (char *)malloc(10);
	//     * var = new char[10];
	...
	if(!Token::Match(tok2, "%name% ::|. %type%")) {
		...
		if(mTokenizer_->isCPP() && tok2->str() == "new") {
			...
			if(tok2->astOperand1() &&
			    ...
			    (tok2->astOperand1()->astOperand1() && tok2->astOperand1()->astOperand1()->str() == "[")))
				return NewArray;
...
void CheckMemoryLeak::memleakUponReallocFailureError(const Token * tok, const std::string &reallocfunction,
    const std::string &varname) const
{
	reportErr(tok,
	    Severity::error,
	    "memleakOnRealloc",
	    "$symbol:" + varname + "\nCommon " + reallocfunction + " mistake: \'$symbol\' nulled but not freed upon failure",
	    CWE(401U));
}
...
{
	return ((call_func_white_list.find(funcname)!=call_func_white_list.end()) || settings->library.isLeakIgnore(funcname) ||
	       (cpp && funcname == "delete"));
}
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(const Token * tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			if(tok->varId() > 0 && Token::Match(tok, "%name% =")) {
				...
				// Check that the allocation isn't followed immediately by an 'if (!var) { error(); }'
				// that might handle failure
...
void CheckMemoryLeakInClass::variable(const Scope * scope, const Token * tokVarname)
{
	...
	for(const Function &func : scope->functionList) {
		...
		if(!func.hasBody()) {
			if(destructor) { // implementation for destructor is not seen => assume it deallocates all
				         // variables properly
			...
			if(tok == func.functionScope->bodyStart)
				...
				if(!body || Token::Match(tok, "%varid% =|[", varid)) {
					...
					if(alloc != CheckMemoryLeak::No) {
						...
						if(alloc != CheckMemoryLeak::Many && memberDealloc != CheckMemoryLeak::No &&
						    memberDealloc != CheckMemoryLeak::Many && memberDealloc != alloc) {
							std::list<const Token *> callstack;
							callstack.push_back(tok);
				...
				if(dealloc != CheckMemoryLeak::No) {
					...
					if(dealloc != CheckMemoryLeak::Many && memberAlloc != CheckMemoryLeak::No && memberAlloc != Many &&
					    memberAlloc != dealloc) {
						std::list<const Token *> callstack;
						callstack.push_back(tok);
						mismatchAllocDealloc(callstack, classname + "::" + varname);
		...
		// Struct member is allocated => check if it is also properly deallocated..
			...
			for(const Token * tok3 = tok2; tok3; tok3 = tok3->next()) {
				...
				// Deallocating the struct member..
					...
					// If the deallocation happens at the base level, don't check this member
					// anymore
					if(indentlevel3 == 0)
						break;
				...
				// Returning from function..
					...
					if(!Token::Match(tok3, "return %varid% ;", structid) &&
					    ...
					    !(Token::Match(tok3, "return %varid% . %var%",
					    structid) && tok3->tokAt(3)->varId() == structmemberid) &&
					    !(Token::Match(tok3,
					    "return %name% (") && tok3->astOperand1() &&
					    deallocInFunction(tok3->astOperand1(), structid))) {
						memoryLeak(tok3, variable->name() + "." + tok2->strAt(2), Malloc);
					}
					break;
				}
...
void CheckMemoryLeakNoVar::check()
{
	...
	// only check functions
	for(const Scope * scope : symbolDatabase->functionScopes) {
		// Checks if a call to an allocation function like malloc() is made and its return value is not
		// assigned.
		...
		// Check for leaks where a the return value of an allocation function like malloc() is an input
		// argument,
...
void CheckMemoryLeakNoVar::checkForUnusedReturnValue(const Scope * scope)
{
	for(const Token * tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
		...
		if(!parent && warn) {
			...
			if(closingBrace->str() == "}" &&
			    Token::Match(closingBrace->link()->tokAt(-1), "%name%") && (!isNew && precedes(tok, closingBrace->link())))
				continue;
...
void CheckMemoryLeakNoVar::checkForUnsafeArgAlloc(const Scope * scope)
{
	...
	if(!mTokenizer->isCPP() || !mSettings->certainty.isEnabled(Certainty::inconclusive) ||
		...
		if(Token::Match(tok, "%name% (")) {
			...
			if(pointerType && functionCalled) {
				...
				if(functionCalled->strAt(1) == "<") {
					functionName += '<';
					for(const Token* tok2 = functionCalled->tokAt(2);
					    tok2 != functionCalled->next()->link();
					    tok2 = tok2->next())
...
void CheckMemoryLeakNoVar::unsafeArgAllocError(const Token * tok,
