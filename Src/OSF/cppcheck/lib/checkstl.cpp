    ...
    const ValueFlow::Value * indexValue)
{
	...
	if(!containerSize) {
		...
		if(containerSize->condition)
			errmsg = ValueFlow::eitherTheConditionIsRedundant(containerSize->condition) + " or expression '" + expression +
			    "' cause access out of bounds.";
		else if(indexValue == nullptr && !index.empty())
			errmsg = "Out of bounds access in expression '" + expression + "' because '$symbol' is empty and '" + index +
			    "' may be non-zero.";
		else
		...
		if(containerSize->condition)
			errmsg = ValueFlow::eitherTheConditionIsRedundant(containerSize->condition) + " or $symbol size can be " +
			    MathLib::toString(containerSize->intvalue) + ". Expression '" + expression + "' cause access out of bounds.";
		else if(indexValue->condition)
			errmsg = ValueFlow::eitherTheConditionIsRedundant(indexValue->condition) + " or '" + index +
			    "' can have the value " + indexValueString(*indexValue) + ". Expression '" + expression +
			    "' cause access out of bounds.";
		else
			errmsg = "Out of bounds access in '" + expression + "', if '$symbol' size is " + MathLib::toString(
				containerSize->intvalue) + " and '" + index + "' is " + indexValueString(*indexValue);
	}
	else {
		// should not happen
		return;
	...
	reportError(errorPath,
	    ...
	    CWE398,
	    (containerSize && containerSize->isInconclusive()) ||
	    (indexValue && indexValue->isInconclusive()) ? Certainty::inconclusive : Certainty::normal);
}
...
bool CheckStl::isContainerSize(const Token * containerToken, const Token * expr) const
{
	...
	if(!Token::Match(expr->astOperand1(), ". %name% ("))
		return false;
	if(!isSameExpression(mTokenizer->isCPP(), false, containerToken, expr->astOperand1()->astOperand1(), mSettings->library, false,
	    false))
		return false;
...
void CheckStl::iteratorsError(const Token* tok, const std::string& containerName1, const std::string& containerName2)
{
	reportError(tok,
	    ...
	    "Same iterator is used with different containers '" + containerName1 + "' and '" + containerName2 + "'.",
	    CWE664,
	    Certainty::normal);
}
	...
	for(const Variable* var : symbolDatabase->variableList()) {
		...
		for(const Token * tok2 = var->nameToken(); tok2 && tok2 != var->scope()->bodyEnd; tok2 = tok2->next()) {
			...
			if(containerAssignScope && tok2 == containerAssignScope->bodyEnd)
				containerToken = nullptr; // We don't know which containers might be used with the
			                                  // iterator
					...
					if(tok2->strAt(2) == "insert") {
						...
						while(par2->str() != ")") {
							...
							if(isIterator(par2->variable(), inconclusiveType2))
								break; // TODO: check if iterator points at same
							               // container
							if(par2->str() == "(")
								par2 = par2->link();
							par2 = par2->next();
...
void CheckStl::mismatchingContainers()
{
					...
					for(ArgIteratorInfo iter1 : cargs) {
						for(ArgIteratorInfo iter2 : cargs) {
							...
							if(iter1.info->first && iter2.info->last &&
							    isSameExpression(true, false, iter1.tok, iter2.tok, mSettings->library, false,
							    false))
	...
	for(const Variable * var : symbolDatabase->variableList()) {
		if(var && var->isStlStringType() && Token::Match(var->nameToken(), "%var% (") &&
		    Token::Match(var->nameToken()->tokAt(2),
...
static bool if_findCompare(const Token * const tokBack, bool stdStringLike)
{
	...
	if(tok->str() == ".")
		return true; // Dereferencing is OK, the programmer might know that the element exists - TODO: An
	                     // inconclusive warning might be appropriate
			...
			// check also for vector-like or pointer containers
				...
				if(tok->variable()->isArrayOrPointer())
					...
					container = mSettings->library.detectContainer(tok->variable()->typeStartToken()); // outer
					                                                                                   // container
					tok2 = Token::findsimplematch(tok->variable()->typeStartToken(),
						"<",
						tok->variable()->typeEndToken());
					if(container && container->type_templateArgNo >= 0 && tok2) {
						...
						container = mSettings->library.detectContainer(tok2); // innner
						                                                      // container
					}
					else
						container = nullptr;
...
static const Token * findInsertValue(const Token * tok, const Token * containerTok, const Token * keyTok, const Library &library)
{
	...
	if(Token::simpleMatch(top,
	    "(") && Token::Match(top->astOperand1(), ". insert|emplace (") && !astIsIterator(top->astOperand1()->tokAt(2))) {
		icontainerTok = top->astOperand1()->astOperand1();
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token * tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			...
			if(mSettings->standards.cpp < Standards::CPP17 &&
			    !(keyTok && keyTok->valueType() && (keyTok->valueType()->isIntegral() || keyTok->valueType()->pointer > 0)))
				continue;
...
void CheckStl::checkFindInsertError(const Token * tok)
{
	...
	if(tok &&
	    ...
	    "=") && tok == tok->astParent()->astOperand2() && Token::simpleMatch(tok->astParent()->astOperand1(), "[")) {
	...
	reportError(
		...
		"stlFindInsert",
		"Searching before insertion is not necessary." + replaceExpr,
		CWE398,
		Certainty::normal);
}
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		for(const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
			if(Token::Match(tok, "%var% . size ( )") ||
				...
				if((!tok->previous()->isArithmeticalOp() &&
				    Token::Match(end, ">=|< 1") && !end->tokAt(2)->isArithmeticalOp()) ||
				    (end->next() && !end->next()->isArithmeticalOp() &&
				    Token::Match(tok->tokAt(-2), "1 <=|>") && !tok->tokAt(-3)->isArithmeticalOp())) {
					if(isCpp03ContainerSizeSlow(varTok))
						sizeError(varTok);
				}
	...
	for(const Scope &scope : symbolDatabase->scopeList) {
		...
		for(const Token * tok = scope.bodyStart; tok && tok != scope.bodyEnd; tok = tok->next()) {
			...
			if(Token::Match(tok, "throw %var% . c_str|data ( ) ;") && isLocal(tok->next()) &&
				...
				const std::pair<std::multimap<const Function*, int>::const_iterator, std::multimap<const Function*,
				    int>::const_iterator> range = c_strFuncParam.equal_range(tok->function());
				for(std::multimap<const Function*, int>::const_iterator i = range.first; i != range.second; ++i) {
					...
					if(tok2 && Token::Match(tok2->tokAt(-4), ". c_str|data ( )")) {
						...
						else if(Token::Match(tok2->tokAt(-9), "%name% . str ( )")) { // Check
							                                                     // ss.str().c_str()
							                                                     // as
							                                                     // parameter
			...
			// Using c_str() to get the return value is only dangerous if the function returns a char*
			    ...
			    (printPerformance && (returnType == stdString || returnType == stdStringConstRef))) && tok->str() == "return") {
				bool err = false;
				...
				if(Token::Match(tok2, "std :: string|wstring (") &&
					...
					for(const Token * search_tok = tok2->next();
					    search_tok != search_end;
...
void CheckStl::dereferenceInvalidIteratorError(const Token* tok, const ValueFlow::Value * value, bool inconclusive)
{
	...
	const std::string errmsgcond("$symbol:" + varname + '\n' + ValueFlow::eitherTheConditionIsRedundant(
		    value ? value->condition : nullptr) + " or there is possible dereference of an invalid iterator: $symbol.");
	if(!tok || !value) {
	...
	if(value->condition) {
		reportError(errorPath, Severity::warning, "derefInvalidIteratorRedundantCheck", errmsgcond, CWE825,
		...
		for(const Token * tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
			...
			if(condBodyTok) {
				...
				if(isEarlyExit(condBodyTok)) {
					const Token * loopVar2 = Token::findmatch(condBodyTok,
						"%varid%",
						condBodyTok->link(),
						loopVar->varId());
					std::string algo;
