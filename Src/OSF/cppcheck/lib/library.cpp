...
Library::Error Library::load(const tinyxml2::XMLDocument &doc)
{
	...
	int format = rootnode->IntAttribute("format", 1); // Assume format version 1 if nothing else is specified (very
	                                                  // old .cfg files had no 'format' attribute)
	...
	for(const tinyxml2::XMLElement * node = rootnode->FirstChildElement(); node; node = node->NextSiblingElement()) {
		...
		if(nodename == "memory" || nodename == "resource") {
			...
			int allocationId = 0;
			for(const tinyxml2::XMLElement * memorynode = node->FirstChildElement();
			    memorynode;
			...
			for(const tinyxml2::XMLElement * reflectionnode = node->FirstChildElement();
			    reflectionnode;
			...
			for(const tinyxml2::XMLElement * markupnode = node->FirstChildElement();
			    markupnode;
			    markupnode = markupnode->NextSiblingElement()) {
				const std::string markupnodename = markupnode->Name();
				if(markupnodename == "keywords") {
					for(const tinyxml2::XMLElement * librarynode = markupnode->FirstChildElement();
					    librarynode;
					...
					for(const tinyxml2::XMLElement * exporter = markupnode->FirstChildElement();
					    exporter;
						...
						for(const tinyxml2::XMLElement * e = exporter->FirstChildElement();
						    e;
					...
					for(const tinyxml2::XMLElement * blocknode = markupnode->FirstChildElement();
					    blocknode;
			...
			if(inherits) {
				...
				if(i != containers.end())
					container = i->second; // Take values from parent and overwrite them if
				                               // necessary
				else
			...
			for(const tinyxml2::XMLElement * containerNode = node->FirstChildElement();
			    containerNode;
				...
				if(containerNodeName == "size" || containerNodeName == "access" || containerNodeName == "other") {
					for(const tinyxml2::XMLElement * functionNode = containerNode->FirstChildElement();
					    functionNode;
					...
					for(const tinyxml2::XMLElement* memberNode = node->FirstChildElement();
					    memberNode;
					    memberNode = memberNode->NextSiblingElement()) {
						...
						member.templateParameter =
						    memberTemplateParameter ? std::atoi(memberTemplateParameter) : -1;
			...
			for(const tinyxml2::XMLElement * checkNode = node->FirstChildElement();
			    checkNode;
			    checkNode = checkNode->NextSiblingElement()) {
				const std::string &checkName = checkNode->Name();
				for(const tinyxml2::XMLElement * checkTypeNode = checkNode->FirstChildElement();
				    checkTypeNode;
			...
			for(const tinyxml2::XMLElement * typenode = node->FirstChildElement();
			    typenode;
    ...
    std::set<std::string> &unknown_elements)
{
	...
	for(const tinyxml2::XMLElement * functionnode = node->FirstChildElement();
	    functionnode;
	    functionnode = functionnode->NextSiblingElement()) {
		...
		if(functionnodename == "noreturn") {
			...
			for(const tinyxml2::XMLElement * argnode = functionnode->FirstChildElement();
			    argnode;
...
bool Library::isIntArgValid(const Token * ftok, int argnr, const MathLib::bigint argvalue) const
{
	...
	for(const Token * tok = tokenList.front(); tok; tok = tok->next()) {
		if(tok->isNumber() && argvalue == MathLib::toLongNumber(tok->str()))
			return true;
		if(Token::Match(tok,
		    "%num% : %num%") && argvalue >= MathLib::toLongNumber(tok->str()) && argvalue <= MathLib::toLongNumber(tok->strAt(2)))
			return true;
		if(Token::Match(tok, "%num% : ,") && argvalue >= MathLib::toLongNumber(tok->str()))
			return true;
		if((!tok->previous() || tok->previous()->str() == ",") &&
		    Token::Match(tok, ": %num%") && argvalue <= MathLib::toLongNumber(tok->strAt(1)))
			return true;
	}
	return false;
}
...
bool Library::isFloatArgValid(const Token * ftok, int argnr, double argvalue) const
{
	...
	for(const Token * tok = tokenList.front(); tok; tok = tok->next()) {
		if(Token::Match(tok,
		    "%num% : %num%") && argvalue >= MathLib::toDoubleNumber(tok->str()) &&
		    argvalue <= MathLib::toDoubleNumber(tok->strAt(2)))
			return true;
		if(Token::Match(tok, "%num% : ,") && argvalue >= MathLib::toDoubleNumber(tok->str()))
			return true;
		if((!tok->previous() || tok->previous()->str() == ",") &&
		    Token::Match(tok, ": %num%") && argvalue <= MathLib::toDoubleNumber(tok->strAt(1)))
			return true;
