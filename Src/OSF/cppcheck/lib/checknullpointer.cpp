...
void CheckNullPointer::parseFunctionCall(const Token &tok, std::list<const Token *> &var, const Library * library)
{
	...
	if(library && library->formatstr_function(&tok)) {
		...
		// 1st parameter..
		if(Token::Match(&tok,
		    "snprintf|vsnprintf|fnprintf|vfnprintf") && args.size() > 1 &&
		    !(args[1] && args[1]->hasKnownIntValue() && args[1]->getKnownIntValue() == 0))                                                                           // Only
			                                                                                                                                                     // if
			                                                                                                                                                     // length
			                                                                                                                                                     // (second
			                                                                                                                                                     // parameter)
...
bool CheckNullPointer::isPointerDeRef(const Token * tok, bool &unknown, const Settings * settings)
{
	...
	if(Token::Match(parent, "<<|>>") && !firstOperand) {
		...
		if(var && var->isPointer() && Token::Match(var->typeStartToken(), "char|wchar_t")) { // Only outputting
			                                                                             // or reading to
			                                                                             // char* can cause
			                                                                             // problems
	...
	for(const Scope * scope : symbolDatabase->functionScopes) {
		...
		for(; tok != scope->bodyEnd; tok = tok->next()) {
			if(Token::Match(tok, "sizeof|decltype|typeid|typeof ("))
			    ...
			    "0 [") && (tok->previous()->str() != "&" || !Token::Match(tok->next()->link()->next(), "[.(]")))
				nullPointerError(tok);

			else if(Token::Match(tok->previous(),
			    "!!. %name% (|{") && (tok->previous()->str() != "::" || tok->strAt(-2) == "std")) {
				if(Token::Match(tok->tokAt(2), "0|NULL|nullptr )|}") && tok->varId()) { // constructor
					                                                                // call
					const Variable * var = tok->variable();
					if(var && !var->isPointer() && !var->isArray() && var->isStlStringType())
						nullPointerError(tok);
			...
			else if(Token::Match(tok->previous(), ">> 0|NULL|nullptr")) { // Only checking input stream
				                                                      // operations is safe here,
				                                                      // because otherwise 0 can be an
				                                                      // integer as well
...
void CheckNullPointer::nullPointerError(const Token * tok, const std::string &varname, const ValueFlow::Value * value, bool inconclusive)
{
	const std::string errmsgcond("$symbol:" + varname + '\n' + ValueFlow::eitherTheConditionIsRedundant(
		    value ? value->condition : nullptr) + " or there is possible null pointer dereference: $symbol.");
	const std::string errmsgdefarg(
		"$symbol:" + varname + "\nPossible null pointer dereference if the default parameter value is used: $symbol");

	if(!tok) {
		reportError(tok, Severity::error, "nullPointer", "Null pointer dereference", CWE_NULL_POINTER_DEREFERENCE,
		    Certainty::normal);
		reportError(tok, Severity::warning, "nullPointerDefaultArg", errmsgdefarg, CWE_NULL_POINTER_DEREFERENCE, Certainty::normal);
		reportError(tok, Severity::warning, "nullPointerRedundantCheck", errmsgcond, CWE_NULL_POINTER_DEREFERENCE,
		    Certainty::normal);
		return;
	}
