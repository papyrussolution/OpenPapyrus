...
private:
	 ...
	 */
	void reportErr(const Token * tok, Severity::SeverityType severity, const std::string &id, const std::string &msg,
	    const CWE &cwe) const;
	 ...
	 * @param cwe cwe number
	 */
	void reportErr(const std::list<const Token *> &callstack,
...
public:
	...
	/** @brief What type of allocation are used.. the "Many" means that several types of allocation and deallocation
	   are used */
...
/**
 * @brief %Check class variables, variables that are allocated in the constructor should be deallocated in the
 *destructor
 */
