...
int ThreadExecutor::handleRead(int rpipe, unsigned int &result)
{
	...
	if(read(rpipe, &type, 1) <= 0) {
		...
		// need to increment so a missing pipe (i.e. premature exit of forked process) results in an error
		// exitcode
		++result;
		return -1;
	}

	if(type != PipeWriter::REPORT_OUT && type != PipeWriter::REPORT_ERROR && type != PipeWriter::REPORT_INFO &&
...
unsigned int ThreadExecutor::check()
{
	...
	for(;;) {
		...
		if((iFile != mFiles.end() || iFileSettings != mSettings.project.fileSettings.end()) && nchildren < mSettings.jobs &&
		    checkLoadAverage(nchildren)) {
			int pipes[2];
			if(pipe(pipes) == -1) {
...
public:
	SyncLogForwarder(ThreadExecutor &threadExecutor)
		...
		for(std::map<std::string, std::size_t>::const_iterator i = threadExecutor.mFiles.begin();
		    i != threadExecutor.mFiles.end();
		    ++i) {
...
private:
	...
	void report(const ErrorMessage &msg, MessageType msgType)
	{
...

		{
			...
			if(std::find(mThreadExecutor.mErrorList.begin(), mThreadExecutor.mErrorList.end(),
			    errmsg) == mThreadExecutor.mErrorList.end()) {
...
unsigned int STDCALL ThreadExecutor::threadProc(SyncLogForwarder* logForwarder)
{
	...
	for(;;) {
		if(itFile == logForwarder->mThreadExecutor.mFiles.end() &&
		    itFileSettings == logForwarder->mThreadExecutor.mSettings.project.fileSettings.end()) {
		...
		if(!logForwarder->mThreadExecutor.mSettings.quiet) {
			...
			CppCheckExecutor::reportStatus(logForwarder->mProcessedFiles,
			    logForwarder->mTotalFiles,
			    logForwarder->mProcessedSize,
			    logForwarder->mTotalFileSize);
		}
	}
	return result;
}
