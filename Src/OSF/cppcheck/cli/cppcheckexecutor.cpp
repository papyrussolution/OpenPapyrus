...
bool CppCheckExecutor::parseFromArgs(CppCheck * cppcheck, int argc, const char* const argv[])
{
	...
	// Check that all include paths exist
	{
		for(std::list<std::string>::iterator iter = settings.includePaths.begin();
			...
			if(FileLister::isDirectory(path))
				...
				// If the include path is not found, warn user and remove the non-existing path from the
				// list.
	...
	if(!mSettings->project.fileSettings.empty() && !mSettings->fileFilters.empty()) {
		...
		for(const std::string &pathname : pathnames) {
			std::string err = FileLister::recursiveAddFiles(mFiles,
				Path::toNativeSeparators(pathname),
				mSettings->library.markupExtensions(),
				matcher);
			if(!err.empty()) {
			...
			for(int i = offset; i < maxdepth; ++i) {
				...
				if(demangling && firstBracketName) {
					...
					if(plus && (plus>(firstBracketName+1))) {
						...
						int status = 0;
						// We're violating the specification - passing stack address instead of
						// malloc'ed heap.
						// Benefit is that no further heap is required, while there is
						// sufficient stack...
						realnameString = abi::__cxa_demangle(input_buffer, demangle_buffer, &length, &status); // non-NULL
						                                                                                       // on
						                                                                                       // success
					}
				}
...
static bool bStackBelowHeap = false; // lame attempt to locate heap vs. stack address space. See
                                     // CppCheckExecutor::check_wrapper()
...
static void CppcheckSignalHandler(int signo, siginfo_t * info, void * context)
{
	...
	bool lowMem = false; // was low-memory condition detected? Be careful then! Avoid allocating much more memory
	                     // then.
...
int CppCheckExecutor::check_internal(CppCheck& cppcheck)
{
	...
	if(!settings.checkConfiguration) {
		...
		if(settings.checks.isEnabled(Checks::missingInclude) &&
		    (Preprocessor::missingIncludeFlag || Preprocessor::missingSystemIncludeFlag)) {
			const std::list<ErrorMessage::FileLocation> callStack;
