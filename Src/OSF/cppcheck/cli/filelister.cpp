...
std::string FileLister::recursiveAddFiles(std::map<std::string, std::size_t> &files,
...
std::string FileLister::addFiles(std::map<std::string, std::size_t> &files,
    const std::string &path,
    const std::set<std::string> &extra,
    bool recursive,
    const PathMatch& ignored)
    ...
    )
{
	...
	if(stat(path.c_str(), &file_stat) != -1) {
		if((file_stat.st_mode & S_IFMT) == S_IFDIR) {
			...
			std::string new_path;
			new_path.reserve(path.length() + 100);// prealloc some memory to avoid constant new/deletes in
			                                      // loop

			while((SUPPRESS_DEPRECATED_WARNING(readdir_r(dir, &dir_result_buffer.entry,
			    &dir_result)) == 0) && (dir_result != nullptr)) {
				...
				bool path_is_directory =
				    ...
				    (dir_result->d_type == DT_UNKNOWN && FileLister::isDirectory(new_path)));
#else
