	...
	for(int i = 1; i < argc; i++) {
		if(argv[i][0] == '-') {
			...
			// --project
				...
				mSettings->checkAllConfigurations = false; // Can be overridden with --max-configs or
				                                           // --force
				...
				if(projType == ImportProject::Type::CPPCHECK_GUI) {
					...
					if(platform == "win32A")
						...
						;
					else if(!mSettings->loadPlatformFile(projectFile.c_str(),
					    platform) && !mSettings->loadPlatformFile(argv[0], platform)) {
						std::string message("unrecognized platform: \"");
						message += platform;
						message += "\".";
						printError(message);
				...
				if(projType == ImportProject::Type::VS_SLN || projType == ImportProject::Type::VS_VCXPROJ) {
					...
					if(!CppCheckExecutor::tryLoadLibrary(mSettings->library, argv[0], "windows.cfg")) {
						...
						printError(
							"failed to load 'windows.cfg'. Your Cppcheck installation is broken. Please re-install.");
						return false;
					}
				}
			...
			// --project-configuration
				...
				mVSConfig = argv[i] + 24;
				if(!mVSConfig.empty() &&
				    (mSettings->project.projectType == ImportProject::Type::VS_SLN ||
			...
			// Filter errors
				...
				if(!f.is_open()) {
					...
					if(std::count(filename.begin(), filename.end(), ',') > 0 ||
						...
						// print more detailed error message to tell user how he can solve the
						// problem
