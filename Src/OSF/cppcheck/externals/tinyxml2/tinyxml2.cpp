...
static const char LINE_FEED                             = (char)0x0a;                   // all line endings are
                                                                                        // normalized to LF
...
char* XMLAttribute::ParseDeep(char* p, bool processEntities, int* curLineNumPtr)
{
	...
	p =
	    _value.ParseText(p, endTag, processEntities ? StrPair::ATTRIBUTE_VALUE : StrPair::ATTRIBUTE_VALUE_LEAVE_ENTITIES,
		curLineNumPtr);
	return p;
}
...
XMLNode* XMLElement::ShallowClone(XMLDocument* doc) const
{
	...
	XMLElement* element = doc->NewElement(Value() );                                // fixme: this will always
		                                                                        // allocate memory. Intern?
	for(const XMLAttribute* a = FirstAttribute(); a; a = a->Next() ) {
		element->SetAttribute(a->Name(), a->Value() );                          // fixme: this will always
			                                                                // allocate memory. Intern?
	}
	return element;
}
