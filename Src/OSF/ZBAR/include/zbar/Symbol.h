...
public:
	...
	const ZBarSymbolIterator symbol_end() const;
...
protected:
	...
	void init(const zbar_symbol_t * sym = NULL)
	{
		...
		if(sym) {
			...
			_data = std::string(zbar_symbol_get_data(sym), zbar_symbol_get_data_length(sym));
		}
		else {
			_type = ZBAR_NONE;
			_data = "";
...
class ZBarSymbolIterator : public std::iterator<std::input_iterator_tag, Symbol> {
public:
	/// default constructor.
	ZBarSymbolIterator ()
	{
	...
	/// constructor.
	ZBarSymbolIterator (const SymbolSet &syms) : _syms(syms)
	{
	...
	/// copy constructor.
	ZBarSymbolIterator (const ZBarSymbolIterator& iter) : _syms(iter._syms)
	{
		const zbar_symbol_set_t * zsyms = _syms;
		if(zsyms)
			_sym = zbar_symbol_set_first_symbol(zsyms);
