...
static /*inline*/ zbar_symbol_t * FASTCALL cache_lookup(zbar_image_scanner_t * iscn, zbar_symbol_t * sym)
{
	...
	while(*entry) {
		if((*entry)->type == sym->type && (*entry)->datalen == sym->datalen && !memcmp((*entry)->P_Data_, sym->P_Data_, sym->datalen))
			break;
...
static /*inline*/ void FASTCALL cache_sym(zbar_image_scanner_t * iscn, zbar_symbol_t * sym)
{
	if(iscn->enable_cache) {
		...
		if(!entry) {
			// FIXME reuse sym 
			entry = _zbar_image_scanner_alloc_sym(iscn, sym->type, sym->datalen + 1);
			entry->configs = sym->configs;
			entry->modifiers = sym->modifiers;
			memcpy(entry->P_Data_, sym->P_Data_, sym->datalen);
		...
		if((!dup && !near_thresh) || far_thresh) {
			//int type = sym->type;
			int h = _zbar_get_symbol_hash(/*type*/sym->type);
...
static /*inline*/ void FASTCALL qr_handler(zbar_image_scanner_t * iscn)
{
	...
	line->eoffs = zbar_scanner_get_edge(iscn->scn, line->eoffs, QR_FINDER_SUBPREC) - line->len;
	line->len -= u;
...
int zbar_image_scanner_set_config(zbar_image_scanner_t * iscn, zbar_symbol_type_t sym, zbar_config_t cfg, int val)
{
	...
	if(cfg < ZBAR_CFG_UNCERTAINTY)
		...
		if(cfg > ZBAR_CFG_UNCERTAINTY)
			...
			int c = cfg - ZBAR_CFG_UNCERTAINTY;
			if(sym > ZBAR_PARTIAL) {
		...
		cfg = (zbar_config_t)(((int)cfg) - (int)ZBAR_CFG_POSITION);
		if(!val)
			iscn->config &= ~(1 << cfg);
		else if(val == 1)
			iscn->config |= (1 << cfg);
	...
	if(syms->nsyms) {
		...
		for(symp = &syms->head; *symp; ) {
			...
			if(sym->cache_count <= 0 &&
				...
				else if(sym->type < ZBAR_COMPOSITE && sym->type != ZBAR_ISBN10) {
		...
		if(nean == 1 && naddon == 1 && iscn->ean_config) {
			...
			zbar_symbol_t * ean_sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_COMPOSITE, datalen);
			ean_sym->orient = ean->orient;
			ean_sym->syms = _zbar_symbol_set_create();
			memcpy(ean_sym->P_Data_, ean->P_Data_, ean->datalen);
			memcpy(ean_sym->P_Data_ + ean->datalen, addon->P_Data_, addon->datalen + 1);
