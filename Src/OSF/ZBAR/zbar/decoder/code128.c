...
static const uchar characters[NUM_CHARS] = {
...
static const uchar lo_base[8] = {
	0x00, 0x07, 0x0c, 0x19, 0x24, 0x32, 0x40, 0x47
};

static const uchar lo_offset[0x80] = {
...
static inline int8 decode_lo(int sig)
{
	uchar offset = (((sig >> 1) & 0x01) | ((sig >> 3) & 0x06) | ((sig >> 5) & 0x18) | ((sig >> 7) & 0x60));
	uchar idx = lo_offset[offset];
	uchar base, c;
	if(sig & 1)
		idx &= 0xf;
	else
...
{
	uchar rev = (sig & 0x4400) != 0;
	uchar idx, c;
	if(rev)
		sig = (((sig >> 12) & 0x000f) | ((sig >>  4) & 0x00f0) | ((sig <<  4) & 0x0f00) | ((sig << 12) & 0xf000));
...
static inline int8 decode6(zbar_decoder_t * dcode)
{
	...
	bars = (get_color(dcode) == ZBAR_BAR) ? (get_width(dcode, 0) + get_width(dcode, 2) + get_width(dcode, 4))
...
static inline uchar validate_checksum(zbar_decoder_t * dcode)
{
	...
	for(i = dcode128->character - 3; i; i--) {
		//zassert(sum < 103, -1, "dir=%x i=%x sum=%x acc=%x %s\n", dcode128->direction, i, sum, acc, _zbar_decoder_buf_dump(dcode->buf, dcode128->character));
		assert(sum < 103);
...
static inline uint postprocess_c(zbar_decoder_t * dcode, uint start, uint end, uint dst)
{
	...
	for(i = 0, j = dst; i < delta; i++, j += 2) {
		...
		uchar code = dcode->buf[start + delta + i];
		dcode->buf[j] = '0';
		if(code >= 50) {
			code -= 50;
			dcode->buf[j] += 5;
		...
		//zassert(dcode->buf[j] <= '9', delta, "start=%x end=%x i=%x j=%x %s\n", start, end, i, j, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
...
static inline uchar postprocess(zbar_decoder_t * dcode)
{
	...
	if(dcode128->direction) {
		...
		//zassert(dcode->buf[dcode128->character - 1] == STOP_REV, 1, "dir=%x %s\n", dcode128->direction, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
		assert(dcode->buf[dcode128->character - 1] == STOP_REV);
	}
	else {
		//zassert(dcode->buf[dcode128->character - 1] == STOP_FWD, 1, "dir=%x %s\n", dcode128->direction, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
	...
	for(i = 1, j = 0; (int)i < (dcode128->character - 2); i++) {
		...
		if((charset & 0x2) && (code < 100))
			...
			if((!charset || (charset == 0x81)) && (code >= 0x60))
				code -= 0x60; /* convert character set A to ASCII */
			dcode->buf[j++] = code;
			if(charset & 0x80)
				charset &= 0x7f;
		}
			...
			if(code < CODE_C) {
				...
				uchar newset = CODE_A - code;
				//zassert(code >= CODE_C && code <= CODE_A, 1, "i=%x j=%x code=%02x charset=%x cexp=%x %s\n", i, j, code, charset, cexp, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
	...
	//zassert(j < dcode->buf_alloc, 1, "j=%02x %s\n", j, _zbar_decoder_buf_dump(dcode->buf, dcode->code128.character));
	assert(j < dcode->buf_alloc);
	dcode->buflen = j;
	dcode->buf[j] = '\0';
...
zbar_symbol_type_t _zbar_decode_code128(zbar_decoder_t * dcode)
{
	...
	if((dcode128->character < 0) ? get_color(dcode) != ZBAR_SPACE : ( /* process every 6th element of active symbol */
	    ++dcode128->element != 6 || /* decode color based on direction */ get_color(dcode) != dcode128->direction))
		return ZBAR_NONE;
	dcode128->element = 0;
	...
	dcode128->width = dcode128->s6;
	//zassert(dcode->buf_alloc > dcode128->character, 0, "alloc=%x idx=%x c=%02x %s\n", dcode->buf_alloc, dcode128->character, c, _zbar_decoder_buf_dump(dcode->buf, dcode->buf_alloc));
	assert((int)dcode->buf_alloc > dcode128->character);
	if(dcode128->character == 1) {
	...
	if(dcode128->character > 2 && ((dcode128->direction) ? c >= START_A && c <= START_C : c == STOP_FWD)) {
		...
		if(validate_checksum(dcode) || postprocess(dcode))
			sym = ZBAR_NONE;
		else if(dcode128->character < CFG(*dcode128, ZBAR_CFG_MIN_LEN) || (CFG(*dcode128, ZBAR_CFG_MAX_LEN) > 0 &&
