...
static const int8 codabar_lo[12] = {
	0x0, 0x1, 0x4, 0x5, 0x2, 0xa, 0xb, 0x9,
	0x6, 0x7, 0x8, 0x3
};

static const uchar codabar_hi[8] = {
	0x1, 0x4, 0x7, 0x6, 0x2, 0x3, 0x0, 0x5
};

static const uchar codabar_characters[/*20*/] = "0123456789-$:/.+ABCD";

static int FASTCALL check_width(uint ref, uint w)
{
	const uint dref = ref;
	ref *= 4;
	w *= 4;
...
static int8 FASTCALL codabar_decode7(zbar_decoder_t * dcode)
{
	...
	if(s < 7)
		return -1;
	else if(!check_width(codabar->width, s)) { // check width 
		dbprintf(2, " [width]");
		return -1;
	}
	else {
		// extract min/max bar 
		uint ibar = decode_sortn(dcode, 4, 1);
		dbprintf(2, " bar=%04x", ibar);
		uint wbmax = get_width(dcode, ibar & 0xf);
		uint wbmin = get_width(dcode, ibar >> 12);
		if((8 * wbmin) < wbmax || (3 * wbmin) > (2 * wbmax)) {
			...
			const ulong b0b3 = wbmin * wbmax;
			const ulong b1b2 = wb1 * wb2;
			if((b1b2 + b1b2 / 8) < b0b3) {
				// single wide bar combinations 
				if((8 * wbmin) < (5 * wb1) || (8 * wb1) < (5 * wb2) || (4 * wb2) > (3 * wbmax) || (wb2 * wb2) >= (wb1 * wbmax)) {
				...
				if((4 * wbmin) > (3 * wb1) || (8 * wb1) < (5 * wb2) || (8 * wb2) < (5 * wbmax) || (wbmin * wb2) >= (wb1 * wb1)) {
			...
			uint ispc = decode_sort3(dcode, 2);
			dbprintf(2, "(%x) spc=%03x", ibar, ispc);
			const uint wsmax = get_width(dcode, ispc & 0xf);
			const uint wsmid = get_width(dcode, (ispc >> 4) & 0xf);
			const uint wsmin = get_width(dcode, (ispc >> 8) & 0xf);
			if(ibar >> 2) {
				if((8 * wsmin) < wsmax || (8 * wsmin) < (5 * wsmid) || (8 * wsmid) < (5 * wsmax)) { // verify no wide spaces 
				...
				const ulong s0s2 = wsmin * wsmax;
				const ulong s1s1 = wsmid * wsmid;
				if((s1s1 + s1s1 / 8) < s0s2) {
					if((8 * wsmin) < (5 * wsmid) || (4 * wsmid) > (3 * wsmax)) { // single wide space 
...
static inline int8 codabar_decode_start(zbar_decoder_t * dcode)
{
	codabar_decoder_t * codabar = &dcode->codabar;
	const uint s = codabar->s7;
	if(s < 8)
		return ZBAR_NONE;
	dbprintf(2, "      codabar: s=%d", s);
	// check leading quiet zone - spec is 10x 
	uint qz = get_width(dcode, 8);
	...
	uint wsmin = get_width(dcode, ispc >> 8);
	...
	uint ibar = decode_sortn(dcode, 4, 1);
	dbprintf(2, " bar=%04x", ibar);
	const uint wbmax = get_width(dcode, ibar & 0xf);
...
zbar_symbol_type_t _zbar_decode_codabar(zbar_decoder_t * dcode)
{
	...
	if(get_color(dcode) != ZBAR_SPACE)
		return ZBAR_NONE;
	else if(codabar->character < 0)
		return (zbar_symbol_type_t)codabar_decode_start(dcode);
	else if(codabar->character < 2 && codabar_decode_start(dcode))
		return ZBAR_PARTIAL;
	else if(--codabar->element)
		return ZBAR_NONE;
	else {
		codabar->element = 4;
		dbprintf(1, "      codabar[%c%02d+%x]", (codabar->direction) ? '<' : '>', codabar->character, codabar->element);
		int8 c = codabar_decode7(dcode);
		dbprintf(1, " %d", c);
		if(c < 0) {
			dbprintf(1, " [aborted]\n");
			goto reset;
		...
		if(codabar->character == NIBUF && acquire_lock(dcode, ZBAR_CODABAR)) {
			...
			if(c & 0x10) {
				...
				uint n = codabar->character;
				if((int)n < CFG(*codabar, ZBAR_CFG_MIN_LEN) || (CFG(*codabar, ZBAR_CFG_MAX_LEN) > 0 && (int)n > CFG(*codabar, ZBAR_CFG_MAX_LEN))) {
