...
static short FASTCALL pdf417_decode8(const zbar_decoder_t * dcode)
{
	...
	if(s < 8)
		...
		int8 e;
		for(uchar i = 0; i < 7; i++) {
			e = (get_color(dcode) == ZBAR_SPACE) ? 
				decode_e(get_width(dcode, i) + get_width(dcode, i + 1), s, 17) : 
				decode_e(get_width(dcode, 7 - i) + get_width(dcode, 6 - i), s, 17);
			dbprintf(4, "%x", e);
			if(e < 0 || e > 8)
				return -1;
			sig = (sig << 3) ^ e;
		...
		int clst = ((sig & 7) - ((sig >> 3) & 7) + ((sig >> 12) & 7) - ((sig >> 15) & 7));
		if(clst < 0)
			clst += 9;
		dbprintf(2, " k=%d", clst);
		{
		...
		if(clst != 0 && clst != 3 && clst != 6) {
			...
			// zassert(g[0] >= 0 && g[1] >= 0 && g[2] >= 0, -1, "dir=%x sig=%lx k=%x g0=%03x g1=%03x g2=%03x %s\n", dcode->pdf417.direction, sig, clst, g[0], g[1], g[2], _zbar_decoder_buf_dump(dcode->buf, dcode->pdf417.character));
			assert(g[0] >= 0 && g[1] >= 0 && g[2] >= 0);
			ushort c = (g[0] + g[1] + g[2]) & PDF417_HASH_MASK;
			dbprintf(2, " g0=%x g1=%x g2=%x c=%03d(%d)", g[0], g[1], g[2], c & 0x3ff, c >> 10);
			return c;
		}
	}
}
...
{
	zbar_symbol_type_t result = ZBAR_NONE;
	pdf417_decoder_t * dcode417 = &dcode->pdf417;
	...
	if(dcode417->character < 0) {
		...
		if(dcode417->element)
			...
			dcode417->element = 0;
			dbprintf(2, "      pdf417[%c%02d]:", (dcode417->direction) ? '<' : '>', dcode417->character);
			if(get_color(dcode) != dcode417->direction) {
				int c = dcode417->character;
				release_lock(dcode, ZBAR_PDF417);
				dcode417->character = -1;
				//zassert(get_color(dcode) == dcode417->direction, ZBAR_NONE, "color=%x dir=%x char=%d elem=0 %s\n", get_color(dcode), dcode417->direction, c, _zbar_decoder_buf_dump(dcode->buf, c));
				if(get_color(dcode) != dcode417->direction)
					return ZBAR_NONE;
			}
			short c = pdf417_decode8(dcode);
			if(c < 0 || size_buf(dcode, dcode417->character + 1)) {
				...
				if(c == PDF417_STOP) {
					...
					release_lock(dcode, ZBAR_PDF417);
					dcode417->character = -1;
					// @sobolev result = ZBAR_NONE;
					result = ZBAR_PDF417; // @sobolev
				}
				dbprintf(2, "\n");
			}
		}
