...
static const int8 finder_hash[0x20] = {
...
static const uchar exp_sequences[] = {
	/* sequence Group 1 */
	0x01,
	0x23,
	0x25, 0x07,
...
static const uchar exp_checksums[] = { 1, 189, 62, 113, 46, 43, 109, 134, 6, 79, 161, 45 };
...
static void decode10(uchar * buf, ulong n, int i)
{
	buf += i;
	while(--i >= 0) {
		const uchar d = (uchar)(n % 10);
		n /= 10;
		*--buf = '0' + d;
	}
}
		...
		for(i0 = 0; i0 < csegs; i0++) {
			...
			for(uint i1 = 0; i1 < csegs; i1++) {
				...
				dbprintf(2, "\n\t[%d,%d] f=%d(0%xx)/%d(%x%x%x)",
					i0, i1, seg->finder, seg->color, s1->finder, s1->exp, s1->color, s1->side);
				chkf = (seg->color) ? (seg->finder + s1->finder * 9) : (s1->finder + seg->finder * 9);
				if(chkf > 72)
					chkf--;
				if(chkf > 8)
					chkf--;
...
static inline zbar_symbol_type_t match_segment_exp(zbar_decoder_t * dcode, databar_segment_t * seg, int dir)
{
	databar_decoder_t * db = &dcode->databar;
	...
	for(i = 0;; i--) {
		...
		if(maxcnt > (int)cnt || (maxcnt == cnt && maxage <= age))
			continue;
		dbprintf(2, " !");
		maxcnt = cnt;
		maxage = age;
...
static zbar_symbol_type_t decode_char(zbar_decoder_t * dcode, databar_segment_t * seg, int off, int dir)
{
	...
	assert(i < sizeof(groups) / sizeof(*groups));
	...
	for(i = 0; i < csegs; i++) {
		...
		if(seg->finder < 0) {
			...
			if(age >= 128 && seg->count < 2) {
				...
				age = (age > seg->count) ? (age - seg->count + 1) : 1;
				if(maxage < age) {
					maxage = age;
					old = i;
...
zbar_symbol_type_t _zbar_decode_databar(zbar_decoder_t * dcode)
{
	...
	db->chars[i] = -1;
	seg = db->segs + iseg;
	dbprintf(2, "        databar: i=%d part=%d f=%d(%x%x%x)", iseg, seg->partial, seg->finder, seg->exp, seg->color, seg->side);
