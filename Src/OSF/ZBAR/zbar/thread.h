...
typedef struct zbar_thread_s {
	...
	static inline zbar_thread_id_t _zbar_thread_self() { return (GetCurrentThreadId()); }
	static inline int _zbar_thread_is_self(zbar_thread_id_t tid) { return (tid == GetCurrentThreadId()); }
#elif defined(HAVE_LIBPTHREAD)
	static inline void _zbar_thread_init(zbar_thread_t *thr)
	{
		sigset_t sigs;
	...
	static inline zbar_thread_id_t _zbar_thread_self() { return (pthread_self()); }
	static inline int _zbar_thread_is_self (zbar_thread_id_t tid) { return (pthread_equal(tid, pthread_self())); }
#else
