...
const int _zbar_num_formats = sizeof(_zbar_formats) / sizeof(uint32);
...
static inline int verify_format_sort(void)
{
	...
	uint32 ordered[num_format_defs];
	for(i = 0; i < num_format_defs; i++)
		sorted[i] = format_defs[i].format;
	qsort(sorted, num_format_defs, sizeof(uint32), intsort);
...
static inline ulong uvp_size(const zbar_image_t * img,
...
static inline uint32 convert_read_rgb(const uint8 * srcp, int bpp)
{
	uint32 p;
	if(bpp == 3) {
		p = *srcp;
 ...
 */
static inline void convert_y_resize(zbar_image_t * dst, const zbar_format_def_t * dstfmt,
    const zbar_image_t * src, const zbar_format_def_t * srcfmt, size_t n)
{
	uint8 * psrc, * pdst;
...
{
	ulong srcm, srcn;
	uint8 flags, * srcy, * dstp;
	const uint8 * srcu, * srcv;
	uint srcl, xmask, ymask, x, y;
...
static void convert_rgb_to_yuvp(zbar_image_t * dst, const zbar_format_def_t * dstfmt, const zbar_image_t * src, const zbar_format_def_t * srcfmt)
{
	...
	for(y = 0; y < dst->height; y++) {
		...
		for(x = 0; x < dst->width; x++) {
			if(x < src->width) {
				uint8 r, g, b;
				uint32 p = convert_read_rgb(srcp, srcfmt->p.rgb.bpp);
				srcp += srcfmt->p.rgb.bpp;
				// FIXME endianness? 
				r = (uint8)(((p >> rbit0) << rbits) & 0xff);
				g = (uint8)(((p >> gbit0) << gbits) & 0xff);
				b = (uint8)(((p >> bbit0) << bbits) & 0xff);
				// FIXME color space? 
...
{
	uint8 * dstp;
	ulong dstn = dst->width * dst->height;
	int drbits, drbit0, dgbits, dgbit0, dbbits, dbbit0;
	const uint8 * srcp;
	uint srcl, x, y;
	uint32 p = 0;
	...
	for(y = 0; y < dst->height; y++) {
		...
		for(x = 0; x < dst->width; x++) {
			if(x < src->width) {
				...
				srcp += srcfmt->p.rgb.bpp;
				// FIXME endianness? 
				r = (uint8)((p >> srbit0) << srbits);
				g = (uint8)((p >> sgbit0) << sgbits);
				b = (uint8)((p >> sbbit0) << sbbits);
				p = (((r >> drbits) << drbit0) | ((g >> dgbits) << dgbit0) | ((b >> dbbits) << dbbit0));
			}
			convert_write_rgb(dstp, p, dstfmt->p.rgb.bpp);
			dstp += dstfmt->p.rgb.bpp;
		}
...
const zbar_format_def_t * _zbar_format_lookup(uint32 fmt)
{
...
zbar_image_t * zbar_image_convert_resize(const zbar_image_t * src, ulong fmt, uint width, uint height)
{
...
zbar_image_t * zbar_image_convert(const zbar_image_t * src, ulong fmt)
{
	return zbar_image_convert_resize(src, fmt, src->width, src->height);
}

static inline int has_format(uint32 fmt, const uint32 * fmts)
{
	for(; *fmts; fmts++)
		if(*fmts == fmt)
			return 1;
...
int _zbar_best_format(uint32 src, uint32 * dst, const uint32 * dsts)
{
	const zbar_format_def_t * srcfmt;
	uint min_cost = -1;
	ASSIGN_PTR(dst, 0);
...
{
	static const uint32 y800[2] = { fourcc('Y', '8', '0', '0'), 0 };
	errinfo_t * errdst;
	const uint32 * srcs, * dsts;
	uint min_cost = -1;
	uint32 min_fmt = 0;
	const uint32 * fmt;
	if(!vdo && !win)
