...
#if 0 // @sobolev (не понятно что делать с iconv) {
...
static int text_is_latin1(const uchar * _text, int _len)
{
	for(int i = 0; i < _len; i++) {
		// The following line fails to compile correctly with gcc 3.4.4 on ARM with any optimizations enabled
						...
						case QR_MODE_KANJI: {
						    ...
						    if(eci<0) {
							    ...
							    for(ei = 0; ei<3; ei++) if(enc_list[ei]!=(iconv_t)-1) {
									    ...
									    err =
										    iconv(enc_list[ei], &in, &inleft, &out,
										    &outleft)==(size_t)-1;
									    if(!err) {
										    sa_ntext = out-sa_text;
