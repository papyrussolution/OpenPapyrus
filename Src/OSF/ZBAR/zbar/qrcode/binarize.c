	...
	for(y = -2; y<_height; y++) {
		...
		if(y>=0) for(x = 0; x<_width; x++) {
				...
				if(sn2) {
					...
					else _img[y*_width+x] = (uchar)(((m<<1)+9)/18);
				}
			}
	}
	SAlloc::F(sn2_buf[0]);
...
// This compares the current pixel value to the mean value of a (large) window surrounding it.
//
uchar * qr_binarize(const uchar * _img, int _width, int _height)
{
	uchar * mask = NULL;
	if(_width>0&&_height>0) {
		uint * col_sums;
