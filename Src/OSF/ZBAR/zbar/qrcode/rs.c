...
static void rs_poly_mul_x(uchar * _p, const uchar * _q, int _dp1)
{
...
/*Divide the polynomial by the free variable, x (shift the coefficients).
   The number of coefficients, _dp1, must be non-zero.*/
static void rs_poly_div_x(uchar * _p, const uchar * _q, int _dp1)
{
...
/*Compute the first (d+1) coefficients of the product of a degree e and a
   degree f polynomial.*/
static void rs_poly_mult(const rs_gf256 * _gf, uchar * _p, int _dp1,
    const uchar * _q, int _ep1, const uchar * _r, int _fp1)
{
	int m;
	int i;
...
static void rs_init_lambda(const rs_gf256 * _gf, uchar * _lambda, int _npar,
    const uchar * _erasures, int _nerasures, int _ndata)
{
	int i;
	int j;
...
static int rs_modified_berlekamp_massey(const rs_gf256 * _gf,
    uchar * _lambda, const uchar * _s, uchar * _omega, int _npar,
    const uchar * _erasures, int _nerasures, int _ndata)
{
	uchar tt[256];
	int n;
