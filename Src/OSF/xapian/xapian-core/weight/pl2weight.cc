...
PL2Weight * PL2Weight::clone() const
{
	return new PL2Weight(param_c);
}

void PL2Weight::init(double factor_)
{
	if(factor_ == 0.0) {
...
double PL2Weight::get_maxpart() const
{
	return upper_bound;
}
...
double PL2Weight::get_maxextra() const
{
	return 0;
}
...
PL2Weight * PL2Weight::create_from_parameters(const char * p) const
{
	if(*p == '\0')
