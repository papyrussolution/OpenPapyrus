...
namespace Xapian {
DLHWeight * DLHWeight::clone() const
{
	return new DLHWeight();
}

void DLHWeight::init(double factor)
{
	if(factor == 0.0) {
...
double DLHWeight::get_maxpart() const
{
	return upper_bound;
}
...
double DLHWeight::get_maxextra() const
{
	return 0;
}

DLHWeight * DLHWeight::create_from_parameters(const char * p) const
{
	if(*p != '\0')
