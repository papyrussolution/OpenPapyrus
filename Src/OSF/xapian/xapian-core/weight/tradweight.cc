...
void TradWeight::init(double factor)
{
	if(factor == 0.0) {
...
double TradWeight::get_maxpart() const
{
...
double TradWeight::get_maxextra() const
{
	return 0;
}
...
TradWeight * TradWeight::create_from_parameters(const char * p) const
{
	if(*p == '\0')
