...
PL2PlusWeight * PL2PlusWeight::clone() const
{
	return new PL2PlusWeight(param_c, param_delta);
}

void PL2PlusWeight::init(double factor_)
{
	if(factor_ == 0.0) {
...
PL2PlusWeight * PL2PlusWeight::unserialise(const string & s) const
{
...
double PL2PlusWeight::get_sumpart(Xapian::termcount wdf, Xapian::termcount len,
...
double PL2PlusWeight::get_maxpart() const
{
	return upper_bound;
}
...
double PL2PlusWeight::get_maxextra() const
{
	return 0;
}
...
PL2PlusWeight * PL2PlusWeight::create_from_parameters(const char * p) const
{
	if(*p == '\0')
