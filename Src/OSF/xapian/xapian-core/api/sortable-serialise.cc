...
size_t Xapian::sortable_serialise_(double value, char* buf) noexcept
{
	...
	if(exponent < 8) {
		next ^= 0x20;
		next |= static_cast<uchar>(exponent << 2);
		...
		// first byte:
		next |= static_cast<uchar>(exponent >> 6);
	...
	word1 &= 0x03ffffff;
	next |= static_cast<uchar>(word1 >> 24);
	buf[len++] = next;
...
static inline uchar numfromstr(const std::string & str, std::string::size_type pos)
{
...
double Xapian::sortable_unserialise(const std::string & value) noexcept
{
	...
	uchar first = numfromstr(value, 0);
	size_t i = 0;

	first ^= static_cast<uchar>(first & 0xc0) >> 1;
