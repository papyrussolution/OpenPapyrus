...
void Compactor::set_status(const string & table, const string & status)
{
	(void)table;
	(void)status;
}

string Compactor::resolve_duplicate_metadata(const string & key, size_t num_tags, const std::string tags[])
{
	(void)key;
	(void)num_tags;
	return tags[0];
...
[[noreturn]] static void backend_mismatch(const Xapian::Database::Internal* db, int backend1, const string &dbpath2, int backend2)
{
	string dbpath1;
...
namespace Xapian {
void Database::compact_(const string * output_ptr, int fd, uint flags, int block_size, Xapian::Compactor * compactor) const
{
	...
	Xapian::Compactor::compaction_level compaction =
	    static_cast<Xapian::Compactor::compaction_level>(flags &
	    (Xapian::Compactor::STANDARD|Xapian::Compactor::FULL|Xapian::Compactor::FULLER));
#else
	(void)compactor;
	(void)block_size;
#endif
