...
public:
	...
	Result(double weight_, Xapian::docid did_, std::string && collapse_key_, Xapian::doccount collapse_count_, std::string && sort_key_) : 
		weight(weight_), did(did_), collapse_count(collapse_count_), collapse_key(std::move(collapse_key_)), sort_key(std::move(sort_key_)) 
	{
	}
	void swap(Result& o);
	...
	void unshard_docid(Xapian::doccount shard, Xapian::doccount n_shards) { did = unshard(did, shard, n_shards); }
