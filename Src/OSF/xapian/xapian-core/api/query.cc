...
size_t Query::get_num_subqueries() const noexcept
{
	return internal.get() ? internal->get_num_subqueries() : 0;
}

const Query Query::get_subquery(size_t n) const
{
	return internal->get_subquery(n);
}

Xapian::termcount Query::get_leaf_wqf() const
{
	return internal->get_wqf();
}

Xapian::termpos Query::get_leaf_pos() const
{
