...
void DatabaseMaster::write_changesets_to_fd(int fd, const string & start_revision, ReplicationInfo * info) const
{
	...
	try {
		...
		conn.send_message(REPL_REPLY_FAIL, "Can't open database: " + e.get_msg(), 0.0);
		return;
	}
...
class DatabaseReplica::Internal : public Xapian::Internal::intrusive_base {
	...
	bool live_db_corrupt = false; /// Do we need to heal the replica?
	...
	bool need_copy_next; // Flag to indicate that the only valid operation next is a full copy.
	string offline_revision; // The revision that the secondary database has been updated to.
	string offline_uuid; // The UUID of the secondary database.
	string offline_needed_revision; // The revision that the secondary database must reach before it can be made live.
	...
	RemoteConnection * conn; /// The remote connection we're using.
...
DatabaseReplica::DatabaseReplica(const string & path) : internal(new DatabaseReplica::Internal(path))
{
		...
		switch(type) {
			...
			case REPL_REPLY_CHANGESET:
			    ...
			    if(!have_offline_db) {
				    ...
				    // changeset.
				    {
					    unique_ptr<DatabaseReplicator> replicator(DatabaseReplicator::open(replica_path));
