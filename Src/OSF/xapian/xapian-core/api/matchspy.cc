...
void ValueCountMatchSpy::operator()(const Document &doc, double) {
	Assert(internal.get());
	++(internal->total);
...
MatchSpy * ValueCountMatchSpy::unserialise(const string & s, const Registry &) const
{
	const char * p = s.data();
...
void ValueCountMatchSpy::merge_results(const string & s) {
