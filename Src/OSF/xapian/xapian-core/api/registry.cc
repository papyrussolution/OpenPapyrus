...
void Registry::register_weighting_scheme(const Xapian::Weight &wt)
{
...
const Xapian::Weight * Registry::get_weighting_scheme(const string & name) const
{
...
void Registry::register_posting_source(const Xapian::PostingSource &source)
{
	LOGCALL_VOID(API, "Xapian::Registry::register_posting_source", source.name());
	register_object(internal->postingsources, source);
}

const Xapian::PostingSource * Registry::get_posting_source(const string & name) const
{
...
void Registry::register_key_maker(Xapian::KeyMaker* keymaker)
{
