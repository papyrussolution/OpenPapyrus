...
struct Among {
	...
	uint s; // offset in pool to search string */
...
protected:
	symbol * p;
	int c, l, lb, bra, ket;
	static int SIZE(const symbol* p) { return alignment_cast<const int *>(p)[-1]; }
	static void SET_SIZE(symbol* p, int n) { alignment_cast<int *>(p)[-1] = n; }
	static int CAPACITY(const symbol* p) { return alignment_cast<const int *>(p)[-2]; }
	static void SET_CAPACITY(symbol* p, int n) { alignment_cast<int *>(p)[-2] = n; }
	...
	int find_among(const symbol * pool, const Among * v, int v_size, const unsigned char * fnum, const among_function * f);
	int find_among_b(const symbol * pool, const Among * v, int v_size, const unsigned char * fnum, const among_function * f);
	...
	void insert_v(int c_bra, int c_ket, const symbol * v) { insert_s(c_bra, c_ket, SIZE(v), v); }
...
public:
	...
	SnowballStemImplementation() : p(create_s()), c(0), l(0), lb(0), bra(0), ket(0) 
	{
	}
