...
//
// To add a zero terminated string to a block. If p = 0 the block is created. 
//
extern symbol * FASTCALL add_s_to_b(symbol * p, const char * s) 
{
	...
	k = SIZE(p);
	{
		const int x = k + n - CAPACITY(p);
		if(x > 0) 
			p = increase_capacity(p, x);
	}
	{
		for(int i = 0; i < n; i++) 
			p[i+k] = s[i];
...
{
	struct str * output = (struct str *)SAlloc::M(sizeof(struct str));
...
extern void str_append_b(struct str * str, const symbol * q) { str->data = add_to_b(str->data, SIZE(q), q); }
// Append a (char *, null terminated) string to a str
extern void str_append_string(struct str * str, const char * s) { str->data = add_s_to_b(str->data, s); }
...
/* Clear a string */
extern void str_clear(struct str * str) { SIZE(str->data) = 0; }
...
extern symbol * str_data(struct str * str) { return str->data; }
/* Get the length of the str. */
extern int str_len(struct str * str) { return SIZE(str->data); }

/* Get the last character of the str.
 *
 * Or -1 if the string is empty.
 */
extern int str_back(struct str * str) { return SIZE(str->data) ? str->data[SIZE(str->data) - 1] : -1; }
