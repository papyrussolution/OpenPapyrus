...
static int set_errno_from_getlasterror()
{
	int e;
	ulong winerr = GetLastError();
	switch(winerr) {
...
int posixy_unlink(const char * pFileName)
{
	// We must use DeleteFile as this can delete files that are open
	if(DeleteFileA(pFileName) != 0) {
		return 0;
	}
...
int posixy_open(const char * filename, int flags)
{
	...
	HANDLE handleWin = CreateFileA(filename, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, FILE_ATTRIBUTE_NORMAL, NULL);
