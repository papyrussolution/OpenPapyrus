...
template <typename T, typename U> static constexpr inline T safe_shl(T x, U shift) { return (shift >= sizeof(T) * 8 ? 0 : x << shift); }
...
Xapian::termpos BitReader::decode(Xapian::termpos outof, bool force)
{
	(void)force;
...
Xapian::termpos BitReader::read_bits(int count)
{
	...
	while(n_bits < count) {
		Assert(p < end);
		acc |= Xapian::termpos(static_cast<uchar>(*p++)) << n_bits;
		n_bits += 8;
	}
...
void BitReader::decode_interpolative(int j, int k, Xapian::termpos pos_j, Xapian::termpos pos_k)
{
...
Xapian::termpos BitReader::decode_interpolative_next()
{
	Assert(di_current.is_initialized());
