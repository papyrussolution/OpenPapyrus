...
public:
	...
	template <class U> intrusive_ptr_nonnull(intrusive_ptr_nonnull<U> const & rhs) : px(rhs.get() )
	{
		++px->_refs;
	}
	...
	template <class U> intrusive_ptr_nonnull(intrusive_ptr_nonnull<U> && rhs) : px(rhs.px)
	{
		rhs.px = 0;
	}
...
public:
	...
	template <class U> opt_intrusive_ptr(opt_intrusive_ptr<U> const & rhs) : px(rhs.get() ), counting(rhs.counting)
	{
		if(counting) 
			++px->_refs;
	}
	opt_intrusive_ptr(opt_intrusive_ptr const & rhs) : px(rhs.px), counting(rhs.counting)
	{
		if(counting) 
			++px->_refs;
	}
	~opt_intrusive_ptr()
	{
		if(counting && --px->_refs == 1) 
			delete px;
	}
	opt_intrusive_ptr(opt_intrusive_ptr && rhs) : px(rhs.px), counting(rhs.counting)
	{
		rhs.px = 0;
		rhs.counting = 0;
	}
	...
	template <class U> opt_intrusive_ptr(opt_intrusive_ptr<U> && rhs) : px(rhs.px), counting(rhs.counting)
	{
		rhs.px = 0;
		rhs.counting = 0;
	}
