...
public:
	 ...
	 */
	XAPIAN_VISIBILITY_INTERNAL void set_max_weight_cached_flag_ptr_(bool* flag_ptr) 
	{
...
public:
	 ...
	 */
	explicit ValuePostingSource(Xapian::valueno slot_) noexcept : slot(slot_) 
	{
	}
 ...
 */
class XAPIAN_VISIBILITY_DEFAULT DecreasingValueWeightPostingSource : public Xapian::ValueWeightPostingSource {
protected:
	...
	double curr_weight; /// Weight at current position.
	bool items_at_end; /// Flag, set to true if there are docs after the end of the range.
...
class XAPIAN_VISIBILITY_DEFAULT ValueMapPostingSource : public ValuePostingSource {
	double default_weight; /// The default weight
	double max_weight_in_map; /// The maximum weight in weight_map.
	std::map<std::string, double> weight_map; /// The value -> weight map
public:
...
class XAPIAN_VISIBILITY_DEFAULT FixedWeightPostingSource : public PostingSource {
	Xapian::Database db; /// The database we're reading documents from.
	Xapian::doccount termfreq; /// Number of documents in the posting source.
	Xapian::PostingIterator it; /// Iterator over all documents.
	bool started; /// Flag indicating if we've started (true if we have).
	Xapian::docid check_docid; /// The docid last passed to check() (0 if check() wasn't the last move).
public:
