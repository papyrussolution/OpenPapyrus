...
#define XAPIAN_VERSION "1.4.19" /// The version of Xapian as a C string literal.
#define XAPIAN_MAJOR_VERSION 1 /// The major component of the Xapian version. E.g. for Xapian 1.0.14 this would be: 1
#define XAPIAN_MINOR_VERSION 4 /// The minor component of the Xapian version. E.g. for Xapian 1.0.14 this would be: 0
#define XAPIAN_REVISION 19 /// The revision component of the Xapian version. E.g. for Xapian 1.0.14 this would be: 14
#define XAPIAN_DOCID_BASE_TYPE long long  /// Base (signed) type for Xapian::docid and related types.
#define XAPIAN_TERMCOUNT_BASE_TYPE long long /// Base (signed) type for Xapian::termcount and related types.
#define XAPIAN_TERMPOS_BASE_TYPE long long // int /// Base (signed) type for Xapian::termpos.
#define XAPIAN_TOTALLENGTH_TYPE unsigned long long /// Type for returning total document length.
#define XAPIAN_REVISION_TYPE unsigned long long /// Underlying type for Xapian::rev.
#define XAPIAN_HAS_CHERT_BACKEND  /// XAPIAN_HAS_CHERT_BACKEND Defined if the chert backend is enabled.
#define XAPIAN_HAS_GLASS_BACKEND  /// XAPIAN_HAS_GLASS_BACKEND Defined if the glass backend is enabled.
#define XAPIAN_HAS_INMEMORY_BACKEND  /// XAPIAN_HAS_INMEMORY_BACKEND Defined if the inmemory backend is enabled.
#define XAPIAN_HAS_REMOTE_BACKEND  /// XAPIAN_HAS_REMOTE_BACKEND Defined if the remote backend is enabled.
...
#define XAPIAN_AT_LEAST(A,B,C) (XAPIAN_MAJOR_VERSION > (A) || (XAPIAN_MAJOR_VERSION == (A) && (XAPIAN_MINOR_VERSION > (B) || (XAPIAN_MINOR_VERSION == (B) && XAPIAN_REVISION >= (C)))))
...
/// then get a compilation failure if the compiler lacks suitable support.
	...
	#if __cplusplus >= 201103L || (defined _MSC_VER && _MSC_VER >= 1900) || defined XAPIAN_LIB_BUILD
