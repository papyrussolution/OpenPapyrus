...
private:
	void operator = (const MatchSpy &) = delete; /// Don't allow assignment.
	MatchSpy(const MatchSpy &) = delete; /// Don't allow copying.
public:
...
public:
	...
	/// @private @internal
	struct XAPIAN_VISIBILITY_DEFAULT Internal : public Xapian::Internal::intrusive_base {
		Xapian::valueno slot; /// The slot to count.
		Xapian::doccount total; /// Total number of documents seen by the match spy.
		std::map<std::string, Xapian::doccount> values; /// The values seen so far, together with their frequency.
...
public:
	...
	explicit ValueCountMatchSpy(Xapian::valueno slot_) : internal(new Internal(slot_)) 
	{
	}
	...
	TermIterator values_end() const noexcept { return TermIterator(); }
	...
	TermIterator top_values_end(size_t) const noexcept { return TermIterator(); }
