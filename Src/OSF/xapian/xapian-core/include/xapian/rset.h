...
public:
	 ...
	 *  If @a did is already marked as relevant, nothing happens.
	 */
	void add_document(const Xapian::MSetIterator& it) { add_document(*it); }
	 ...
	 *  If @a did is not marked as relevant, nothing happens.
	 */
	void remove_document(const Xapian::MSetIterator& it) { remove_document(*it); }
	/// Check if a document is marked as relevant.
	bool contains(Xapian::docid did) const;
	/// Check if a document is marked as relevant.
	bool contains(const Xapian::MSetIterator& it) const { return contains(*it); }
