...
inline const Query operator&(const Query & a, const Query & b)
{
 ...
 */
inline const Query operator*(double factor, const Query & q)
{
 ...
 *  @param factor Non-negative real number to multiply weights by.
 */
inline const Query operator*(const Query & q, double factor)
{
	return Query(factor, q);
}
...
public:
	...
	operator Query() const { return Query(Query::OP_AND_NOT, Query(std::string()), query); }
 ...
 */
inline const Query operator&(const Query & a, const InvertedQuery_ & b)
{
 ...
 *  E.g. q1 &=~ q2;
 */
inline const Query operator&=(Query & a, const InvertedQuery_ & b) { return (a = Query(Query::OP_AND_NOT, a, b.query)); }

#ifndef DOXYGEN /* @internal doesn't seem to avoid a warning here. */
/** @internal Helper to allow q1 &~ q2 to work. */
inline const InvertedQuery_ operator~(const Query &q) { return InvertedQuery_(q); }
...
public:
	...
	virtual void postlist_sub_xor(Xapian::Internal::XorContext& ctx, Xapian::Internal::QueryOptimiser* qopt, double factor) const;
...
inline const Query Query::operator&=(const Query & o)
{
	if(o.empty()) {
		// q &= empty_query sets q to empty_query.
		*this = o;
	}
	else if(this != &o && internal.get() && internal->_refs == 1 && get_type() == OP_AND) {
...
inline const Query Query::operator|=(const Query & o)
{
	...
	else if(this != &o && internal.get() && internal->_refs == 1 && get_type() == OP_OR) {
