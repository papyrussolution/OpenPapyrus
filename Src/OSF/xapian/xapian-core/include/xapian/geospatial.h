...
double miles_to_metres(double miles) noexcept XAPIAN_CONST_FUNCTION;

/** Convert from miles to metres.
 *
 *  Experimental - see https://xapian.org/docs/deprecation#experimental-features
 */
inline double miles_to_metres(double miles) noexcept
{
	return 1609.344 * miles;
}

double metres_to_miles(double metres) noexcept XAPIAN_CONST_FUNCTION;

/** Convert from metres to miles.
 *
 *  Experimental - see https://xapian.org/docs/deprecation#experimental-features
 */
inline double metres_to_miles(double metres) noexcept
{
...
public:
	...
	const LatLongCoord & operator*() const { return *iter; }
...
public:
	...
	LatLongCoordsIterator begin() const { return LatLongCoordsIterator(coords.begin()); }
	/// Get an end iterator for the coordinates.
	LatLongCoordsIterator end() const { return LatLongCoordsIterator(coords.end()); }
...
class XAPIAN_VISIBILITY_DEFAULT LatLongDistancePostingSource : public ValuePostingSource {
	double dist; /// Current distance from centre.
	LatLongCoords centre; /// Centre, to compute distance from.
	const LatLongMetric * metric; /// Metric to compute the distance with.
	double max_range; /// Maximum range to allow.  If set to 0, there is no maximum range.
	double k1; /// Constant used in weighting function.
	double k2; /// Constant used in weighting function.
...
public:
	 ...
	 *  away.
	 */
	LatLongDistanceKeyMaker(Xapian::valueno slot_, const LatLongCoord & centre_) : slot(slot_), centre(), metric(new Xapian::GreatCircleMetric()), defkey(9, '\xff')
	{
		centre.append(centre_);
	}
