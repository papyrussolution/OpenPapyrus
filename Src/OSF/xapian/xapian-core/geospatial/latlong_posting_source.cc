...
static void validate_postingsource_params(double k1, double k2) {
	...
	if(k2 <= 0) {
		string msg("k2 parameter to LatLongDistancePostingSource must be greater than 0; was ");
		msg += str(k2);
		throw InvalidArgumentError(msg);
	}
}
...
double LatLongDistancePostingSource::get_weight() const
{
...
LatLongDistancePostingSource * LatLongDistancePostingSource::unserialise_with_registry(const string &s,
...
string LatLongDistancePostingSource::get_description() const
{
