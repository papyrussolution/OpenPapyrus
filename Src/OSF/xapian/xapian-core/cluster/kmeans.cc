...
void KMeans::initialise_clusters(ClusterSet& cset, doccount num_of_points)
{
	...
	for(uint i = 0; i < k; ++i) {
		uint x = (i * num_of_points) / k;
		cset.add_cluster(Cluster(Centroid(points[x])));
	}
}

void KMeans::initialise_points(const MSet& source)
{
...
ClusterSet KMeans::cluster(const MSet& mset)
{
