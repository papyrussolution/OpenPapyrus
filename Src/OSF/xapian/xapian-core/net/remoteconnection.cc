...
RemoteConnection::RemoteConnection(int fdin_, int fdout_, const string & context_) : fdin(fdin_), fdout(fdout_), context(context_)
{
	...
	if(!overlapped.hEvent)
		throw Xapian::NetworkError("Failed to setup OVERLAPPED", context, -int(GetLastError()));
...
void RemoteConnection::send_message(char type, const string &message,
    double end_time)
{
...
void RemoteConnection::send_file(char type, int fd, double end_time)
{
...
int RemoteConnection::get_message(string &result, double end_time)
{
	...
	size_t len = static_cast<uchar>(buffer[1]);
	if(len < 128) {
...
int RemoteConnection::get_message_chunked(double end_time)
{
	...
	// This code assume things about the pack_uint() encoding in order to
	// handle partial reads.
	uint_least64_t len = static_cast<uchar>(buffer[1]);
	if(len < 128) {
...
int RemoteConnection::get_message_chunk(string &result, size_t at_least,
    double end_time)
{
...
int RemoteConnection::receive_file(const string &file, double end_time)
{
...
#ifdef __WIN32__
DWORD RemoteConnection::calc_read_wait_msecs(double end_time)
{
	if(end_time == 0.0)
		return INFINITE;
