...
void unserialise_stats(const char* p, const char* p_end, Xapian::Weight::Internal& stat)
{
	...
	if(!unpack_uint(&p, p_end, &stat.total_length) || !unpack_uint(&p, p_end, &stat.collection_size) ||
	    !unpack_uint(&p, p_end, &stat.rset_size) || !unpack_uint(&p, p_end, &dummy) ||
	    !unpack_bool(&p, p_end, &stat.have_max_part) || !unpack_uint(&p, p_end, &n)) {
	...
	while(n--) {
		...
		if(!unpack_string(&p, p_end, term) || !unpack_uint(&p, p_end, &termfreq) ||
		    (stat.rset_size != 0 && !unpack_uint(&p, p_end, &reltermfreq)) || !unpack_uint(&p, p_end, &collfreq)) {
			unpack_throw_serialisation_error(p);
		}
		double max_part = 0.0;
		if(stat.have_max_part)
			max_part = unserialise_double(&p, p_end);
		stat.termfreqs.insert(make_pair(term, TermFreqs(termfreq, reltermfreq, collfreq, max_part)));
	}
}
