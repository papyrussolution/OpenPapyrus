...
struct FieldInfo {
	filter_type type; /// The type of this field.
	std::string grouping;
	std::vector <std::string> prefixes; /// Field prefix strings.
	/// Field processor.  Currently only one is supported.
	Xapian::Internal::opt_intrusive_ptr<Xapian::FieldProcessor> proc;
	FieldInfo(filter_type type_, const std::string & prefix, const std::string & grouping_ = std::string()) : type(type_), grouping(grouping_)
	{
		prefixes.push_back(prefix);
	}
	FieldInfo(filter_type type_, Xapian::FieldProcessor* proc_, const std::string & grouping_ = std::string()) : type(type_), grouping(grouping_), proc(proc_)
	{
	}
};
...
struct RangeProc {
	...
	RangeProc(RangeProcessor * range_proc, const std::string* grouping_) : proc(range_proc),
		grouping(grouping_ ? *grouping_ : std::string()), default_grouping(grouping_ == NULL) 
	{
	}
};
...
class QueryParser::Internal : public Xapian::Internal::intrusive_base {
	...
	uint min_wildcard_prefix_len = 0;
	uint min_partial_prefix_len = 2;
	void add_prefix(const std::string & field, const std::string & prefix);
	void add_prefix(const std::string & field, Xapian::FieldProcessor* proc);
	void add_boolean_prefix(const std::string & field, const std::string & prefix, const std::string* grouping);
	void add_boolean_prefix(const std::string & field, Xapian::FieldProcessor* proc, const std::string* grouping);
	std::string parse_term(Utf8Iterator& it, const Utf8Iterator& end, bool cjk_enable, uint flags,
	    bool& is_cjk_term, bool& was_acronym, size_t& first_wildcard, size_t& char_count, uint & edit_distance);
public:
	Internal() : stem_action(STEM_SOME), stopper(NULL), default_op(Query::OP_OR), errmsg(NULL) 
	{
	}
	Query parse_query(const std::string & query_string, uint flags, const std::string & default_prefix);
};
}
