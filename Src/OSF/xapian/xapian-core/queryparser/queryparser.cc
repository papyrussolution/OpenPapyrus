...
string SimpleStopper::get_description() const
{
	...
	for(unordered_set<string>::const_iterator i = stop_words.begin(); i != stop_words.end(); ++i) {
		if(i != stop_words.begin()) 
			desc += ' ';
		desc += *i;
	}
	desc += ')';
...
void QueryParser::set_stopper(const Stopper * stopper)
{
...
void QueryParser::add_prefix(const string &field, const string &prefix)
{
	Assert(internal.get());
	internal->add_prefix(field, prefix);
}

void QueryParser::add_prefix(const string &field, Xapian::FieldProcessor * proc)
{
	Assert(internal.get());
