...
#define ISSPACE(X) isspace((uchar)(X))
#define ISDIGIT(X) isdigit((uchar)(X))
#define ISALNUM(X) isalnum((uchar)(X))
#define ISALPHA(X) isalpha((uchar)(X))
#define ISUPPER(X) isupper((uchar)(X))
#define ISLOWER(X) islower((uchar)(X))
#ifndef __WIN32__
...
*/
// @sobolev #define lemonStrlen(X)   ((int)strlen(X))
/*
...
**
*/
static void lemon_addtext(char * zBuf,           /* The buffer to which text is added */
	...
	for(i = j = 0; (c = zFormat[i])!=0; i++) {
		if(c=='%') {
			...
			if(c=='d') {
				...
				slfprintf_stderr("illegal format\n");
				exit(1);
			}
			j = i+1;
		}
...
//static void lemon_strcpy(char * dest, const char * src) { while( (*(dest++) = *(src++))!=0) {} }
...
static struct action * Action_new();
...
static struct action * Action_new()
{
	...
	if(freelist==0) {
		int amt = 100;
		freelist = (struct action *)SAlloc::C(amt, sizeof(struct action));
		if(freelist==0) {
			slfprintf_stderr("Unable to allocate memory for a new parser action.");
			exit(1);
		}
...
{
	acttab * p = (acttab*)SAlloc::C(1, sizeof(*p) );
	if(p==0) {
		slfprintf_stderr("Unable to allocate memory for a new acttab.");
		exit(1);
	}
	memzero(p, sizeof(*p));
	p->nsymbol = nsymbol;
...
void acttab_action(acttab * p, int lookahead, int action)
{
	if(p->nLookahead>=p->nLookaheadAlloc) {
		...
		p->aLookahead = (struct lookahead_action *)SAlloc::R(p->aLookahead, sizeof(p->aLookahead[0])*p->nLookaheadAlloc);
		if(p->aLookahead==0) {
			slfprintf_stderr("malloc failed\n");
			exit(1);
		}
	}
	if(p->nLookahead==0) {
...
int acttab_insert(acttab * p, int makeItSafe)
{
	...
	*/
	...
	if(p->nAction + n >= p->nActionAlloc) {
		...
		p->aAction = (struct lookahead_action *)SAlloc::R(p->aAction, sizeof(p->aAction[0])*p->nActionAlloc);
		if(p->aAction==0) {
			slfprintf_stderr("malloc failed\n");
			exit(1);
		}
...
{
	for(struct rule * rp = xp->rule; rp; rp = rp->next) {
		if(rp->precsym==0) {
			for(int i = 0; i<rp->nrhs && rp->precsym==0; i++) {
...
void FindStates(struct lemon * lemp)
{
	...
	if(lemp->start) {
		...
		if(sp==0) {
			ErrorMsg(lemp->filename, 0, "The specified start symbol \"%s\" is not in a nonterminal of the grammar.  \"%s\" will be used as the start symbol instead.", 
...
void FindActions(struct lemon * lemp)
{
	...
	** start nonterminal.  */
	...
	for(i = 0; i<lemp->nstate; i++) {
		for(struct action * ap = lemp->sorted[i]->ap; ap; ap = ap->next) {
...
static int resolve_conflict(struct action * apx, struct action * apy)
{
	...
	if(apx->type==SHIFT && apy->type==REDUCE) {
		...
		assert(oneof10(apx->type, SH_RESOLVED, RD_RESOLVED, SSCONFLICT, SRCONFLICT, RRCONFLICT, SH_RESOLVED, RD_RESOLVED, SSCONFLICT, SRCONFLICT, RRCONFLICT));
...
PRIVATE struct config * newconfig()
{
	...
	if(freelist==0) {
		int amt = 3;
		freelist = (struct config *)SAlloc::C(amt, sizeof(struct config) );
		if(freelist==0) {
			slfprintf_stderr("Unable to allocate memory for a new configuration.");
			exit(1);
		}
		for(int i = 0; i<amt-1; i++) 
			freelist[i].next = &freelist[i+1];
...
struct config * Configlist_add(struct rule * rp/* The rule */, int dot/* Index into the RHS of the rule where the dot goes */)
{
...
{
	va_list ap;
	slfprintf_stderr("%s:%d: ", filename, lineno);
	va_start(ap, format);
...
void memory_error()
{
	slfprintf_stderr("Out of memory.  Aborting...\n");
	exit(1);
}
...
static void handle_D_option(char * z)
{
	char ** paz;
	nDefine++;
	azDefine = (char**)SAlloc::R(azDefine, sizeof(azDefine[0])*nDefine);
	if(azDefine==0) {
...
int main(int argc, char ** argv)
{
	...
	static const struct s_options options[] = {
		...
		{OPT_FLAG, "p", (void*)&showPrecedenceConflict, 0, "Show conflicts resolved by precedence rules"},
		{OPT_FLAG, "q", (void*)&quiet, 0, "(Quiet) Don't print the report file."},
		{OPT_FLAG, "r", (void*)&noResort, 0, "Do not sort or renumber states"},
		{OPT_FLAG, "s", (void*)&statistics, 0, "Print parser stats to standard output."},
	...
	if(OptNArgs()!=1) {
		slfprintf_stderr("Exactly one filename argument is required.\n");
		exit(1);
	}
	memzero(&lem, sizeof(lem));
	lem.errorcnt = 0;
	...
	if(lem.nrule==0) {
		slfprintf_stderr("Empty grammar.\n");
		exit(1);
	}
	...
	assert(sstreq(lem.symbols[i-1]->name, "{default}"));
	lem.nsymbol = i - 1;
	...
	*/
	...
	if(lem.nconflict > 0) {
		slfprintf_stderr("%d parsing conflicts.\n", lem.nconflict);
	}
...
static void errline(int n, int k, FILE * err)
{
	...
	spcnt = sstrleni(argv[0]) + 1;
	for(i = 1; i<n && argv[i]; i++) {
		fprintf(err, " %s", argv[i]);
		spcnt += sstrleni(argv[i])+1;
	}
	spcnt += k;
...
static int handleflags(int i, FILE * err)
{
	...
	for(j = 0; op[j].label; j++) {
		if(strncmp(&argv[i][1], op[j].label, sstrlen(op[j].label))==0) 
			break;
	}
	v = argv[i][0]=='-' ? 1 : 0;
	if(op[j].label==0) {
		...
		*((int*)op[j].arg) = v;
		...
		(op[j].func)(reinterpret_cast<char *>(v));
	}
		...
		switch(op[j].type) {
			...
			case OPT_DBL:
			    *(double*)(op[j].arg) = dv;
			    break;
			case OPT_FDBL:
			    (op[j].func)(reinterpret_cast<char *>(&dv)); // @sobolev / was: (op[j].func)(dv)
			    break;
			case OPT_INT:
			    *(int*)(op[j].arg) = lv;
			    break;
			case OPT_FINT:
			    (op[j].func)(reinterpret_cast<char *>(lv));
			    break;
			case OPT_STR:
			    *(char**)(op[j].arg) = sv;
			    break;
...
void OptPrint()
{
	...
	for(i = 0; op[i].label; i++) {
		switch(op[i].type) {
			...
			case OPT_INT:
			case OPT_FINT:
			    fprintf(errstream, "  -%s<integer>%*s  %s\n", op[i].label, (int)(max-sstrleni(op[i].label)-9), "", op[i].message);
			    break;
			case OPT_DBL:
			case OPT_FDBL:
			    fprintf(errstream, "  -%s<real>%*s  %s\n", op[i].label, (int)(max-sstrleni(op[i].label)-6), "", op[i].message);
			    break;
			case OPT_STR:
			case OPT_FSTR:
			    fprintf(errstream, "  -%s<string>%*s  %s\n", op[i].label, (int)(max-sstrleni(op[i].label)-8), "", op[i].message);
			    break;
		}
	}
}
	...
	switch(psp->state) {
		...
		case LHS_ALIAS_3:
		    if(x[0]==':' && x[1]==':' && x[2]=='=') {
			    ...
			    ErrorMsg(psp->filename, psp->tokenlineno, "Missing \"->\" following: \"%s(%s)\".",
		...
		case IN_RHS:
		    if(x[0]=='.') {
			    ...
			    if(msp->type!=MULTITERMINAL) {
				    ...
				    msp = (struct symbol *)SAlloc::C(1, sizeof(*msp));
				    MemoryCheck(msp);
				    memzero(msp, sizeof(*msp));
				    msp->type = MULTITERMINAL;
				    msp->nsubsym = 1;
				    msp->subsym = (struct symbol **)SAlloc::C(1, sizeof(struct symbol*));
			    ...
			    msp->nsubsym++;
			    msp->subsym = (struct symbol **)SAlloc::R(msp->subsym, sizeof(struct symbol*)*msp->nsubsym);
			    MemoryCheck(msp->subsym);
		...
		case WAITING_FOR_DECL_KEYWORD:
		    if(ISALPHA(x[0])) {
			    ...
			    else if(sstreq(x, "include")) {
				    psp->declargslot = &(psp->gp->include);
			    }
			    else if(sstreq(x, "code")) {
				    psp->declargslot = &(psp->gp->extracode);
			    }
			    else if(sstreq(x, "token_destructor")) {
				    psp->declargslot = &psp->gp->tokendest;
			    }
			    else if(sstreq(x, "default_destructor")) {
				    psp->declargslot = &psp->gp->vardest;
			    }
			    else if(sstreq(x, "token_prefix")) {
				    psp->declargslot = &psp->gp->tokenprefix;
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "syntax_error")) {
				    psp->declargslot = &(psp->gp->error);
			    }
			    else if(sstreq(x, "parse_accept")) {
				    psp->declargslot = &(psp->gp->accept);
			    }
			    else if(sstreq(x, "parse_failure")) {
				    psp->declargslot = &(psp->gp->failure);
			    }
			    else if(sstreq(x, "stack_overflow")) {
				    psp->declargslot = &(psp->gp->overflow);
			    }
			    else if(sstreq(x, "extra_argument")) {
				    psp->declargslot = &(psp->gp->arg);
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "token_type")) {
				    psp->declargslot = &(psp->gp->tokentype);
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "default_type")) {
				    psp->declargslot = &(psp->gp->vartype);
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "stack_size")) {
				    psp->declargslot = &(psp->gp->stacksize);
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "start_symbol")) {
				    psp->declargslot = &(psp->gp->start);
				    psp->insertLineMacro = 0;
			    }
			    else if(sstreq(x, "left")) {
				    psp->preccounter++;
				    psp->declassoc = LEFT;
				    psp->state = WAITING_FOR_PRECEDENCE_SYMBOL;
			    }
			    else if(sstreq(x, "right")) {
				    psp->preccounter++;
				    psp->declassoc = RIGHT;
				    psp->state = WAITING_FOR_PRECEDENCE_SYMBOL;
			    }
			    else if(sstreq(x, "nonassoc")) {
				    psp->preccounter++;
				    psp->declassoc = NONE;
				    psp->state = WAITING_FOR_PRECEDENCE_SYMBOL;
			    }
			    else if(sstreq(x, "destructor")) {
				    psp->state = WAITING_FOR_DESTRUCTOR_SYMBOL;
			    }
			    else if(sstreq(x, "type")) {
				    psp->state = WAITING_FOR_DATATYPE_SYMBOL;
			    }
			    else if(sstreq(x, "fallback")) {
				    psp->fallback = 0;
				    psp->state = WAITING_FOR_FALLBACK_ID;
			    }
			    else if(sstreq(x, "token")) {
				    psp->state = WAITING_FOR_TOKEN_NAME;
			    }
			    else if(sstreq(x, "wildcard")) {
				    psp->state = WAITING_FOR_WILDCARD_ID;
			    }
			    else if(sstreq(x, "token_class")) {
...
static void preprocess_input(char * z){
	...
	for(i = 0; z[i]; i++) {
		...
		else if( (strncmp(&z[i], "%ifdef", 6)==0 && ISSPACE(z[i+6])) || (strncmp(&z[i], "%ifndef", 7)==0 && ISSPACE(z[i+7]))) {
			if(exclude) {
				exclude++;
			}
			else {
				...
				exclude = 1;
				for(k = 0; k<nDefine; k++) {
					if(strncmp(azDefine[k], &z[j], n)==0 && sstrleni(azDefine[k])==n) {
						exclude = 0;
						break;
					}
				}
	...
	if(exclude) {
		slfprintf_stderr("unterminated %%ifdef starting on line %d\n", start_lineno);
		exit(1);
	}
}
...
void Parse(struct lemon * gp)
{
	...
	SAlloc::F(filebuf); /* Release the buffer after parsing */
...
{
	FILE * fp;
	SAlloc::F(lemp->outname);
	lemp->outname = file_makename(lemp, suffix);
	fp = fopen(lemp->outname, mode);
	if(fp==0 && *mode=='w') {
		slfprintf_stderr("Can't open file \"%s\".\n", lemp->outname);
		lemp->errorcnt++;
		return 0;
	}
	return fp;
...
PRIVATE int compute_action(struct lemon * lemp, struct action * ap)
{
	...
	switch(ap->type) {
		...
		case ERROR_: act = lemp->errAction;                            break;
...
PRIVATE FILE * tplt_open(struct lemon * lemp)
{
	...
	if(user_templatename != 0) {
		if(_access(user_templatename, 004)==-1) {
			slfprintf_stderr("Can't find the parser driver template file \"%s\".\n",
			    user_templatename);
			lemp->errorcnt++;
			return 0;
		}
		...
		if(in==0) {
			slfprintf_stderr("Can't open the template file \"%s\".\n",
			    user_templatename);
			lemp->errorcnt++;
			return 0;
		}
...
** symbol sp
*/
void emit_destructor_code(FILE * out,
    struct symbol * sp,
    struct lemon * lemp,
    int * lineno
    ){
...
PRIVATE int translate_code(struct lemon * lemp, struct rule * rp)
{
	...
	if(rp->nrhs==0) {
	...
	else if(sstreq(rp->lhsalias, rp->rhsalias[0])) {
		...
		** direct writing is allowed */
		...
		if(rp->lhs->dtnum!=rp->rhs[0]->dtnum) {
			ErrorMsg(lemp->filename, rp->ruleline, "%s(%s) and %s(%s) share the same label but have different datatypes.",
	...
	for(cp = (char *)rp->code; *cp; cp++) {
		...
		if(ISALPHA(*cp) && (cp==rp->code || (!ISALNUM(cp[-1]) && cp[-1]!='_'))) {
			...
			saved = *xp;
			*xp = 0;
			if(rp->lhsalias && sstreq(cp, rp->lhsalias)) {
				...
				for(i = 0; i<rp->nrhs; i++) {
					if(rp->rhsalias[i] && sstreq(cp, rp->rhsalias[i])) {
						if(i==0 && dontUseRhs0) {
	...
	*/
	for(i = 0; i<rp->nrhs; i++) {
		if(rp->rhsalias[i]) {
			if(i>0) {
				int j;
				if(rp->lhsalias && sstreq(rp->lhsalias, rp->rhsalias[i])) {
				...
				for(j = 0; j<i; j++) {
					if(rp->rhsalias[j] && sstreq(rp->rhsalias[j], rp->rhsalias[i])) {
...
*/
PRIVATE void emit_code(FILE * out,
    struct rule * rp,
    struct lemon * lemp,
    int * lineno
    ){
...
void print_stack_union(FILE * out,                  /* The output stream */
	...
	maxdtlength = 0;
	if(lemp->vartype) {
		maxdtlength = sstrleni(lemp->vartype);
	}
	for(i = 0; i<lemp->nsymbol; i++) {
		int len;
	...
	*/
	for(i = 0; i<lemp->nsymbol; i++) {
		...
		stddt[j] = 0;
		if(lemp->tokentype && sstreq(stddt, lemp->tokentype)) {
			sp->dtnum = 0;
			continue;
		}
		hash = 0;
...
void ReportTable(struct lemon * lemp,
	...
	*/
	ax = (struct axset *)SAlloc::C(lemp->nxstate*2, sizeof(ax[0]));
	if(ax==0) {
		slfprintf_stderr("malloc failed\n");
		exit(1);
	}
	...
	*/
	...
	for(i = 0; i<lemp->nsymbol; i++) {
		...
		for(j = i+1; j<lemp->nsymbol; j++) {
			...
			if(sp2 && sp2->type!=TERMINAL && sp2->destructor && sp2->dtnum==sp->dtnum && sstreq(sp->destructor, sp2->destructor)) {
...
void ReportHeader(struct lemon * lemp)
{
	...
	if(output_header_filename!=0) {
		char * tmp = lemp->filename;
		const char * ext = strrchr(output_header_filename, '.');
		SETIFZ(ext, ".h");
	...
	if(in) {
		...
		if(i==lemp->nterminal && nextChar==EOF) {
			return; // No change in the file.  Don't rewrite it.
		}
	}
	if(output_header_filename!=0) {
		char * tmp = lemp->filename;
...
void Strsafe_init()
{
	if(x1a) 
		return;
	x1a = (struct s_x1*)SAlloc::M(sizeof(struct s_x1) );
	if(x1a) {
		x1a->size = 1024;
		x1a->count = 0;
		x1a->tbl = (x1node*)SAlloc::C(1024, sizeof(x1node) + sizeof(x1node*));
		if(x1a->tbl==0) {
			memory_error();
		}
		else {
...
int Strsafe_insert(const char * data)
{
	...
	np = x1a->ht[h];
	while(np) {
		if(sstreq(np->data, data)) {
			// An existing entry with the same key is found. Fail because overwrite is not allows. 
			return 0;
		}
		np = np->next;
	}
	if(x1a->count>=x1a->size) {
		...
		array.tbl = (x1node*)SAlloc::C(arrSize, sizeof(x1node) + sizeof(x1node*));
...
struct symbol * Symbol_new(const char * x)
{
	struct symbol * sp = Symbol_find(x);
	if(sp==0) {
		sp = (struct symbol *)SAlloc::C(1, sizeof(struct symbol) );
		MemoryCheck(sp);
		sp->name = Strsafe(x);
...
{
	if(!x2a) {
		x2a = (struct s_x2*)SAlloc::M(sizeof(struct s_x2) );
		if(x2a) {
			x2a->size = 128;
			x2a->count = 0;
			x2a->tbl = (x2node*)SAlloc::C(128, sizeof(x2node) + sizeof(x2node*));
			if(x2a->tbl==0) {
				memory_error();
			}
			else {
...
int Symbol_insert(struct symbol * data, const char * key)
{
	...
	if(x2a->count>=x2a->size) {
		...
		array.tbl = (x2node*)SAlloc::C(arrSize, sizeof(x2node) + sizeof(x2node*));
		MemoryCheck(array.tbl);
		array.ht = (x2node**)&(array.tbl[arrSize]);
		for(i = 0; i<arrSize; i++) array.ht[i] = 0;
		for(i = 0; i<x2a->count; i++) {
			x2node * newnp;
			x2node * oldnp = &(x2a->tbl[i]);
...
{
	struct symbol * data = (x2a && n > 0 && n <= x2a->count) ? x2a->tbl[n-1].data : 0;
	return data;
}
...
struct symbol ** Symbol_arrayof()
{
	...
	if(x2a==0) 
		return 0;
	arrSize = x2a->count;
	array = (struct symbol **)SAlloc::C(arrSize, sizeof(struct symbol *));
	if(array) {
		for(i = 0; i<arrSize; i++) 
...
struct state * State_new()
{
	struct state * newstate = (struct state *)SAlloc::C(1, sizeof(struct state) );
...
{
	if(!x3a) {
		x3a = (struct s_x3*)SAlloc::M(sizeof(struct s_x3) );
		if(x3a) {
			x3a->size = 128;
			x3a->count = 0;
			x3a->tbl = (x3node*)SAlloc::C(128, sizeof(x3node) + sizeof(x3node*));
			if(x3a->tbl==0) {
				memory_error();
			}
			else {
...
int State_insert(struct state * data, struct config * key)
{
	...
	if(x3a->count>=x3a->size) {
		...
		array.tbl = (x3node*)SAlloc::C(arrSize, sizeof(x3node) + sizeof(x3node*));
		MemoryCheck(array.tbl);
...
struct state ** State_arrayof()
{
	struct state ** array = 0;
	if(x3a) {
		int arrSize = x3a->count;
		array = (struct state **)SAlloc::C(arrSize, sizeof(struct state *));
		if(array) {
...
{
	if(!x4a) {
		x4a = (struct s_x4*)SAlloc::M(sizeof(struct s_x4) );
		if(x4a) {
			x4a->size = 64;
			x4a->count = 0;
			x4a->tbl = (x4node*)SAlloc::C(64, sizeof(x4node) + sizeof(x4node*));
			if(x4a->tbl==0) {
				memory_error();
			}
			else {
...
int Configtable_insert(struct config * data)
{
	...
	if(x4a->count>=x4a->size) {
		...
		array.tbl = (x4node*)SAlloc::C(arrSize, sizeof(x4node) + sizeof(x4node*));
		MemoryCheck(array.tbl);
