...
TermList* EmptyDatabase::open_term_list_direct(Xapian::docid) const
{
	no_subdatabases();
	return NULL;
}
...
Xapian::termcount EmptyDatabase::get_unique_terms(Xapian::docid did) const
{
	Assert(did != 0);
	(void)did;
	no_subdatabases();
...
Xapian::termcount EmptyDatabase::get_wdfdocmax(Xapian::docid did) const
{
	Assert(did != 0);
	(void)did;
	no_subdatabases();
...
Xapian::Document::Internal* EmptyDatabase::open_document(Xapian::docid did, bool) const
{
	Assert(did != 0);
	(void)did;
	no_subdatabases();
...
int EmptyDatabase::get_backend_info(string*) const
{
	// This method should only be called on a single shard.
	Assert(false);
...
void EmptyDatabase::add_spelling(const string &, Xapian::termcount) const
{
	no_subdatabases();
}

Xapian::termcount EmptyDatabase::remove_spelling(const string &, Xapian::termcount) const
{
	no_subdatabases();
	return 0;
}
