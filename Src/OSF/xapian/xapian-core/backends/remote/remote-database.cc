...
static inline bool is_intermediate_reply(int reply_code)
{
...
void RemoteDatabase::send_message(message_type type, const string &message) const
{
	double end_time = RealTime::end_time(timeout);
	while(pending_reply) {
		string dummy;
...
void RemoteDatabase::get_remote_stats(Xapian::Weight::Internal& out) const
{
	string message;
...
Xapian::MSet RemoteDatabase::get_mset(const vector<opt_ptr_spy>& matchspies) const
{
	string message;
