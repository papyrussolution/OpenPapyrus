...
class RemoteTermList : public TermList {
	void operator =(const RemoteTermList &) = delete; /// Don't allow assignment.
	RemoteTermList(const RemoteTermList &) = delete; /// Don't allow copying.
	...
	const char * p = NULL;
public:
	RemoteTermList(Xapian::termcount num_entries_, Xapian::termcount doclen_, Xapian::doccount db_size_,
	    const RemoteDatabase* db_, Xapian::docid did_, std::string && data_) : 
		num_entries(num_entries_), doclen(doclen_), db_size(db_size_), db(db_), did(did_), data(data_) 
	{
	}
