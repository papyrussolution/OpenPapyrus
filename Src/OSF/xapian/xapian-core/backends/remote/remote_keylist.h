...
class RemoteKeyList : public AllTermsList {
	void operator =(const RemoteKeyList&) = delete; /// Don't allow assignment.
	RemoteKeyList(const RemoteKeyList&) = delete; /// Don't allow copying.
	std::string current_term;
	std::string data;
	const char * p = NULL;
public:
	RemoteKeyList(const std::string & prefix, std::string && data_) : current_term(prefix), data(data_) 
	{
	}
