...
class RemoteDocument : public Xapian::Document::Internal {
	void operator =(const RemoteDocument &); /// Don't allow assignment.
	RemoteDocument(const RemoteDocument &); /// Don't allow copying.
	friend class RemoteDatabase; /// RemoteDocument::open_document() needs to call our private constructor.
	...
	RemoteDocument(const Xapian::Database::Internal * db, Xapian::docid did_, std::string && data_, std::map<Xapian::valueno, std::string>&& values_) : 
		Xapian::Document::Internal(db, did_, std::move(data_), std::move(values_)) 
	{
	}
protected:
