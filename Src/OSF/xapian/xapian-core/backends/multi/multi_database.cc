...
Xapian::Document::Internal* MultiDatabase::open_document(Xapian::docid did, bool lazy) const
{
	Assert(did != 0);
...
int MultiDatabase::get_backend_info(string*) const
{
	// This method should only be called on a single shard.
	Assert(false);
...
void MultiDatabase::replace_document(Xapian::docid did, const Xapian::Document& doc)
{
	auto n_shards = shards.size();
	auto shard = shards[shard_number(did, n_shards)];
	shard->replace_document(shard_docid(did, n_shards), doc);
