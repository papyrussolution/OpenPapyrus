...
struct SubValueList {
	ValueList * valuelist;
	unsigned shard;
	SubValueList(ValueList * vl, unsigned shard_) : valuelist(vl), shard(shard_) 
	{
	}
	~SubValueList() 
	{
	...
	Xapian::docid get_docid() const { return valuelist->get_docid(); }
	Xapian::docid get_merged_docid(unsigned n_shards) const { return unshard(valuelist->get_docid(), shard, n_shards); }
	std::string get_value() const { return valuelist->get_value(); }
	void next() { valuelist->next(); }
...
public:
	...
	MultiValueList(Xapian::doccount n_shards_, SubValueList** valuelists_, Xapian::valueno slot_) : 
		count(n_shards_), valuelists(valuelists_), slot(slot_), n_shards(n_shards_)
	{
	}
	~MultiValueList();
