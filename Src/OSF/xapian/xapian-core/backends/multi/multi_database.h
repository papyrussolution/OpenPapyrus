...
public:
	explicit MultiDatabase(size_type reserve_size, bool read_only) : Xapian::Database::Internal(read_only ? TRANSACTION_READONLY : TRANSACTION_NONE), shards(reserve_size) 
	{
	}
	size_type size() const;
	void reserve(size_type new_size) 
	{
		shards.reserve(new_size);
	}
	...
	void get_freqs(const std::string & term, Xapian::doccount* tf_ptr, Xapian::termcount* cf_ptr) const;
