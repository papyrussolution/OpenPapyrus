...
class SlowValueList : public ValueList {
	...
	Xapian::docid last_docid; /// The last docid in the database, or 0 if we're at_end.
	Xapian::valueno slot; /// The value slot we're iterating over.
	std::string current_value; /// The value at the current position.
	Xapian::docid current_did; /// The document id at the current position.
public:
	SlowValueList(const Xapian::Database::Internal * db_, Xapian::valueno slot_) : db(db_), last_docid(db_->get_lastdocid()), slot(slot_), current_did(0)
	{
	}
