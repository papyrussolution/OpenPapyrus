...
class PrefixCompressedStringItor {
	 ...
	 */
	int tail = 0;
	PrefixCompressedStringItor(PrefixCompressedStringItor& o) : p(o.p), left(o.left), current(o.current), tail(o.tail) 
	{
	}
public:
	/** Construct for glass.
	...
	PrefixCompressedStringItor & operator++() 
	{
		if(left == 0) {
			...
			current.replace(keep, current.size() - tail - keep, reinterpret_cast<const char *>(p + 1), add);
			p += add + 1;
			left -= add + 1;
		}
		return *this;
...
struct PrefixCompressedStringItorGt {
	...
	bool operator()(const PrefixCompressedStringItor * a, const PrefixCompressedStringItor * b) const { return (**a > **b); }
};
