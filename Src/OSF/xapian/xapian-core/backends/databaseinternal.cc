...
void Database::Internal::keep_alive()
{
	// No-op except for remote databases.
}

void Database::Internal::readahead_for_query(const Xapian::Query &) const
{
}

Xapian::termcount Database::Internal::get_unique_terms_lower_bound() const
{
	return 1;
}

Xapian::termcount Database::Internal::get_unique_terms_upper_bound() const
{
...
void Database::Internal::dtor_called_()
{
	try {
...
void Database::Internal::begin_transaction(bool flushed)
{
	if(state != TRANSACTION_NONE) {
		if(transaction_active()) {
			invalid_operation("WritableDatabase::begin_transaction(): already in a transaction");
		}
		if(is_read_only()) {
			invalid_operation("WritableDatabase::begin_transaction(): called with a read-only shard");
		}
...
void Database::Internal::end_transaction(bool do_commit)
{
	if(!transaction_active()) {
		if(state != TRANSACTION_NONE) {
			if(is_read_only()) {
				invalid_operation(do_commit ? "WritableDatabase::commit_transaction(): called with a read-only shard" :
				    "WritableDatabase::cancel_transaction(): called with a read-only shard");
			}
			throw UnimplementedError("This backend doesn't implement transactions");
		}
		invalid_operation(do_commit ? "WritableDatabase::commit_transaction(): not in a transaction" :
		    "WritableDatabase::cancel_transaction(): not in a transaction");
	}
...
Xapian::docid Database::Internal::add_document(const Xapian::Document &)
{
	// Writable databases should override this method, but this can get called
	// if a read-only shard gets added to a WritableDatabase.
	invalid_operation("WritableDatabase::add_document() called with a read-only shard");
}

void Database::Internal::delete_document(Xapian::docid)
{
	// Writable databases should override this method, but this can get called
	// if a read-only shard gets added to a WritableDatabase.
	invalid_operation("WritableDatabase::delete_document() called with a read-only shard");
}

void Database::Internal::delete_document(const string &unique_term)
{
	// Default implementation - overridden for remote and sharded databases.
	if(is_read_only()) {
		// This can happen if a read-only shard gets added to a WritableDatabase.
		invalid_operation("WritableDatabase::delete_document() called with a read-only shard");
	}
...
void Database::Internal::replace_document(Xapian::docid, const Xapian::Document &)
{
	// Writable databases should override this method, but this can get called
	// if a read-only shard gets added to a WritableDatabase.
	invalid_operation("WritableDatabase::replace_document() called with a read-only shard");
}

Xapian::docid Database::Internal::replace_document(const string & unique_term, const Xapian::Document & document)
{
	// Default implementation - overridden for remote and sharded databases.
	if(is_read_only()) {
...
ValueList * Database::Internal::open_value_list(Xapian::valueno slot) const
{
	return new SlowValueList(this, slot);
}

TermList * Database::Internal::open_spelling_termlist(const string &) const
{
...
TermList * Database::Internal::open_spelling_wordlist() const
{
	// Only implemented for some database backends - others will just not
	// suggest spelling corrections (or not contribute to them in a multiple
	// database situation).
...
Xapian::doccount Database::Internal::get_spelling_frequency(const string &) const
{
	// Only implemented for some database backends - others will just not
	// suggest spelling corrections (or not contribute to them in a multiple
	// database situation).
...
void Database::Internal::add_spelling(const string &, Xapian::termcount) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement spelling correction");
}

Xapian::termcount Database::Internal::remove_spelling(const string &, Xapian::termcount) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement spelling correction");
}

TermList * Database::Internal::open_synonym_termlist(const string &) const
{
	// Only implemented for some database backends - others will just not
	// expand synonyms (or not contribute to them in a multiple database situation).
	return NULL;
}

TermList * Database::Internal::open_synonym_keylist(const string &) const
{
	// Only implemented for some database backends - others will just not
...
void Database::Internal::add_synonym(const string &, const string &) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement synonyms");
}

void Database::Internal::remove_synonym(const string &, const string &) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement synonyms");
}

void Database::Internal::clear_synonyms(const string &) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement synonyms");
}

string Database::Internal::get_metadata(const string &) const
{
	return string();
}

TermList * Database::Internal::open_metadata_keylist(const string &) const
{
...
void Database::Internal::set_metadata(const string &, const string &)
{
	throw Xapian::UnimplementedError("This backend doesn't implement metadata");
}

bool Database::Internal::reopen()
{
...
void Database::Internal::request_document(Xapian::docid) const
{
}

void Database::Internal::write_changesets_to_fd(int, const string &, bool, ReplicationInfo*)
{
	throw Xapian::UnimplementedError("This backend doesn't provide changesets");
}

Xapian::rev Database::Internal::get_revision() const
{
	throw Xapian::UnimplementedError("This backend doesn't provide access to revision information");
}

string Database::Internal::get_uuid() const
{
	return string();
}

void Database::Internal::invalidate_doc_object(Xapian::Document::Internal*) const
{
	// Do nothing, by default.
}

void Database::Internal::get_used_docid_range(Xapian::docid &, Xapian::docid &) const
{
	throw Xapian::UnimplementedError("This backend doesn't implement get_used_docid_range()");
}

bool Database::Internal::locked() const
{
	return false;
}

Database::Internal * Database::Internal::update_lock(int flags)
{
...
class Pos {
	...
	string term;
public:
	Pos(string && term_, PositionList* p_) : p(p_), term(term_) 
	{
		pos = p->get_position();
	}
	~Pos() 
    ...
    const string & end, vector<unique_ptr<Pos> >& heap, size_t prefix_size = 0)
{
	...
	while(!termlist->at_end()) {
		...
		if((start_pos ? poslist->skip_to(start_pos) : poslist->next()) && (end_pos == LAST_POS || poslist->get_position() <= end_pos)) {
...
string Database::Internal::reconstruct_text(Xapian::docid did, size_t length, const std::string & prefix,
    Xapian::termpos start_pos, Xapian::termpos end_pos) const
{
	if(end_pos == 0) {
	...
	if(LIKELY(termlist.get())) {
		if(prefix.empty()) {
			termlist->next();
			reconstruct_open_poslists(termlist.get(), start_pos, end_pos, "A", heap);
			termlist->skip_to("[");
			reconstruct_open_poslists(termlist.get(), start_pos, end_pos, prefix, heap);
		}
		else {
			...
			if(!termlist->at_end()) {
				...
				term_ub.resize(i + 1);
				if(i != string::npos) {
					term_ub[i] = (uchar)term_ub[i] + 1;
				}
				reconstruct_open_poslists(termlist.get(), start_pos, end_pos, term_ub, heap, prefix.size());
			}
		}
