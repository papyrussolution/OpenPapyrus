...
GlassChanges * GlassChanges::start(glass_revision_number_t old_rev, glass_revision_number_t rev, int flags)
{
	if(rev == 0) {
...
void GlassChanges::write_block(const char * p, size_t len)
{
	io_write(changes_fd, p, len);
}

void GlassChanges::commit(glass_revision_number_t new_rev, int flags)
{
	if(changes_fd < 0)
		return;
...
void GlassChanges::check(const string & changes_file)
{
