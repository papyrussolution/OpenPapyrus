...
public:
	 ...
	 */
	GlassPositionListTable(const std::string & dbdir, bool readonly) : GlassLazyTable("position", dbdir + "/position.", readonly) 
	{
	}
	GlassPositionListTable(int fd, off_t offset_, bool readonly_) : GlassLazyTable("position", fd, offset_, readonly_) 
	{
	}
	 ...
	 */
	void set_positionlist(Xapian::docid did, const std::string & tname, const std::string & s) 
	{
...
protected:
	BitReader rd; /// Interpolative decoder.
	Xapian::termpos current_pos; /// Current entry.
	Xapian::termpos last; /// Last entry.
	Xapian::termcount size; /// Number of entries.
...
class GlassPositionList : public GlassBasePositionList {
	std::string pos_data; /// The encoded positional data being read by rd.
	/// Copying is not allowed.
...
public:
	...
	explicit GlassPositionList(std::string && data);
	/// Construct and initialise with data.
	GlassPositionList(const GlassTable* table, Xapian::docid did, const std::string & term);
};

/** A reusable position list in a glass database. */
class GlassRePositionList : public GlassBasePositionList {
	GlassCursor cursor; /// Cursor for locating multiple entries efficiently.
	GlassRePositionList(const GlassRePositionList&) = delete; /// Copying is not allowed.
	GlassRePositionList& operator =(const GlassRePositionList&) = delete; /// Assignment is not allowed.
public:
	explicit GlassRePositionList(const GlassTable* table) : cursor(table) 
	{
	}
