...
static inline string make_slot_key(Xapian::docid did)
{
...
static inline string make_valuestats_key(Xapian::valueno slot)
{
...
void ValueChunkReader::assign(const char * p_, size_t len, Xapian::docid did_)
{
	p = p_;
	end = p_ + len;
	did = did_;
...
void GlassValueManager::remove_value(Xapian::docid did, Xapian::valueno slot)
{
	auto i = changes.find(slot);
	if(i == changes.end()) {
		i = changes.insert(make_pair(slot, map<Xapian::docid, string>())).first;
...
void GlassValueManager::add_document(Xapian::docid did, const Xapian::Document &doc,
...
string GlassValueManager::get_value(Xapian::docid did, Xapian::valueno slot) const
{
	auto i = changes.find(slot);
	if(i != changes.end()) {
...
void GlassValueManager::get_all_values(map<Xapian::valueno, string> & values,
...
void GlassValueManager::get_value_stats(Xapian::valueno slot, ValueStats & stats) const
{
...
void GlassValueManager::set_value_stats(map<Xapian::valueno, ValueStats> & value_stats)
{
