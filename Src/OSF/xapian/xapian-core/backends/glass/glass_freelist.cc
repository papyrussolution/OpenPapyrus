...
// testsuite.
	...
	#define FREELIST_END_ (8 + (GLASS_FREELIST_SIZE < 3 ? 3 : GLASS_FREELIST_SIZE) * 4)
...
void GlassFreeList::write_block(const GlassTable * B, uint4 n, uint8 * ptr, uint4 rev)
{
...
uint4 GlassFreeList::get_block(const GlassTable * B, uint4 block_size, uint4 * blk_to_free)
{
	if(fl == fl_end) {
...
uint4 GlassFreeList::walk(const GlassTable * B, uint4 block_size, bool inclusive)
{
	if(fl == fl_end) {
		return UNUSED; // It's expected that the caller checks !empty() first.
	}
	if(p == 0) {
		if(fl.n == UNUSED) {
			throw Xapian::DatabaseCorruptError("Freelist pointer invalid");
...
void GlassFreeList::mark_block_unused(const GlassTable * B, uint4 block_size, uint4 blk)
{
	...
	if(flw.c == 0) {
		...
		if(block_size != FREELIST_END) {
			memzero(pw + FREELIST_END, block_size - FREELIST_END);
		}
#endif
...
void GlassFreeList::commit(const GlassTable * B, uint4 block_size)
{
	if(pw && flw.c != 0) {
...
uint4 GlassFreeListChecker::count_set_bits(uint4 * p_first_bad_blk) const
{
	...
	for(uint4 i = 0; i < bitmap_size; ++i) {
		...
		if(c == 0 && p_first_bad_blk) {
			...
			else if(sizeof(elt_type) == sizeof(ulong)) {
				first_bad_blk += __builtin_ctzl(elt);
#endif
#if HAVE_DECL___BUILTIN_CTZLL
			}
			else if(sizeof(elt_type) == sizeof(ulong long)) {
