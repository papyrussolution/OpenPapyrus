...
public:
	...
	GlassValueManager(GlassPostListTable * postlist_table_, GlassTermListTable * termlist_table_) : 
		mru_slot(Xapian::BAD_VALUENO), postlist_table(postlist_table_), termlist_table(termlist_table_) 
	{
	}
	...
	bool is_modified() const { return !changes.empty(); }
	void cancel() 
	{
