...
class GlassFLCursor {
public:
	uint4 n; /// Block number of current freelist chunk.
	uint  c; /// Current offset in block.
...
protected:
	...
	bool flw_appending;
private:
	uint8 * p; /// Current freelist block.
	uint8 * pw; /// Current freelist block we're writing.
public:
	GlassFreeList() 
	{
		revision = 0;
	...
	{
		bool r = unpack_uint(pstart, end, &revision) && unpack_uint(pstart, end, &first_unused_block) &&
		    fl.unpack(pstart, end) && flw.unpack(pstart, end);
		if(r) {
			fl_end = flw;
			flw_appending = false;
		}
...
public:
	...
	bool mark_used(uint4 n) 
	{
		...
		n /= BITS_PER_ELT;
		if(UNLIKELY(n >= bitmap_size)) return false;
