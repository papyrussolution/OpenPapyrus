...
void GlassVersion::read()
{
	...
	if(version != GLASS_FORMAT_VERSION) {
		...
		msg += str(VERSION_TO_YEAR(version) * 10000 + VERSION_TO_MONTH(version) * 100 + VERSION_TO_DAY(version));
		msg += " but I only understand ";
		msg += str(VERSION_TO_YEAR(GLASS_FORMAT_VERSION) * 10000 + VERSION_TO_MONTH(GLASS_FORMAT_VERSION) * 100 + VERSION_TO_DAY(GLASS_FORMAT_VERSION));
	...
	for(uint table_no = 0; table_no < Glass::MAX_; ++table_no) {
...
const string GlassVersion::write(glass_revision_number_t new_rev, int flags)
{
...
bool GlassVersion::sync(const string & tmpfile,
