...
static inline bool get_tname_from_key(const char ** src, const char * end, string &tname)
{
	return unpack_string_preserving_sort(src, end, tname);
}

static inline bool check_tname_in_key_lite(const char ** keypos, const char * keyend, const string &tname)
{
	string tname_in_key;
...
static inline void read_did_increase(const char ** posptr, const char * end,
...
static inline void read_wdf(const char ** posptr, const char * end, Xapian::termcount * wdf_ptr)
{
    ...
    bool * is_last_chunk_ptr)
{
	LOGCALL_STATIC(DB,
	    Xapian::docid,
	    "read_start_of_chunk",
	    reinterpret_cast<const void*>(posptr) | reinterpret_cast<const void*>(end) | first_did_in_chunk |
...
void GlassPostListTable::get_freqs(const string & term,
...
void PostlistChunkWriter::append(GlassTable * table, Xapian::docid did,
    Xapian::termcount wdf)
{
	if(!started) {
		started = true;
 ...
 */
static inline string make_start_of_first_chunk(Xapian::doccount entries,
...
static void write_start_of_chunk(string & chunk,
    uint start_of_chunk_header,
    uint end_of_chunk_header,
    bool is_last_chunk,
    Xapian::docid first_did_in_chunk,
...
LeafPostList * GlassPostList::open_nearby_postlist(const std::string & term_,
...
PostList * GlassPostList::skip_to(Xapian::docid desired_did, double w_min)
{
