...
void GlassTableCheck::print_key(const uint8 * p, int c, int j) const
{
	if(j == 0) {
		LeafItem item(p, c);
		string key;
...
void GlassTableCheck::print_tag(const uint8 * p, int c, int j) const
{
	if(j == 0) {
		LeafItem item(p, c);
		string tag;
...
void GlassTableCheck::report_block_full(int m, int n, const uint8 * p) const
{
	int j = GET_LEVEL(p);
...
int GlassTableCheck::block_usage(const uint8 * p) const
{
 ...
 */
void GlassTableCheck::report_block(int m, int n, const uint8 * p) const
{
	int j = GET_LEVEL(p);
	int dir_end = DIR_END(p);
	int c;
...
[[noreturn]] static void failure(const char * msg, uint4 n, int c = 0)
{
	string e = "Block ";
	e += str(n);
	if(c) {
...
void GlassTableCheck::block_check(Glass::Cursor * C_, int j, int opts,
