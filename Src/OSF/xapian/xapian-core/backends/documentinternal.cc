...
namespace Xapian {
void Document::Internal::ensure_terms_fetched() const
{
	if(!terms) {
...
void Document::Internal::ensure_values_fetched() const
{
	if(values)
		return;
...
string Document::Internal::fetch_data() const
{
	return string();
}

void Document::Internal::fetch_all_values(map<Xapian::valueno, string>&values_) const
{
	values_.clear();
}

string Document::Internal::fetch_value(Xapian::valueno) const
{
	return string();
}
...
TermList* Document::Internal::open_term_list() const
{
	if(terms)
...
Xapian::ValueIterator Document::Internal::values_begin() const
{
...
string Document::Internal::get_description() const
{
