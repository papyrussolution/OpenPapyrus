...
class InMemoryPositionList : public PositionList {
	Xapian::VecCOW<Xapian::termpos> positions; /// Sorted list of term positions.
	/** Current index into @a positions.
	 *
	 *  Or size_t(-1) if we've not yet started.
	 */
	size_t index = size_t(-1);
	void operator =(const InMemoryPositionList&) = delete; /// Don't allow assignment.
	InMemoryPositionList(const InMemoryPositionList&) = delete; /// Don't allow copying.
public:
	...
	explicit InMemoryPositionList(Xapian::VecCOW<Xapian::termpos>&& positions_) : positions(std::move(positions_)) 
	{
	}
	/// Construct with copied positional data.
	explicit InMemoryPositionList(const Xapian::VecCOW<Xapian::termpos>& positions_) : positions(positions_.copy()) 
	{
	}
