...
class InMemoryDocument : public Xapian::Document::Internal {
	void operator =(const InMemoryDocument &); /// Don't allow assignment.
	InMemoryDocument(const InMemoryDocument &); /// Don't allow copying.
	friend class InMemoryDatabase; /// InMemoryDatabase::open_document() needs to call our private constructor.
	/// Private constructor - only called by InMemoryDatabase::open_document().
	InMemoryDocument(const Xapian::Database::Internal * db, Xapian::docid did_) : Xapian::Document::Internal(db, did_) 
	{
	}
protected:
