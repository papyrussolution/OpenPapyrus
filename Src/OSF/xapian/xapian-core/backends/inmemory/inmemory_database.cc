...
PostList * InMemoryPostList::skip_to(Xapian::docid did, double w_min)
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
...
Xapian::doccount InMemoryAllDocsPostList::get_termfreq() const
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
	return db->totdocs;
}

Xapian::docid InMemoryAllDocsPostList::get_docid() const
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
	Assert(did > 0);
...
Xapian::termcount InMemoryAllDocsPostList::get_wdf() const
{
	return 1;
}

PositionList * InMemoryAllDocsPostList::read_position_list()
{
	throw Xapian::UnimplementedError("Can't open position list for all docs iterator");
}

PositionList * InMemoryAllDocsPostList::open_position_list() const
{
	throw Xapian::UnimplementedError("Can't open position list for all docs iterator");
}

PostList * InMemoryAllDocsPostList::next(double /*w_min*/)
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
	Assert(!at_end());
	do {
...
PostList * InMemoryAllDocsPostList::skip_to(Xapian::docid did_, double /*w_min*/)
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
	Assert(!at_end());
	if(did <= did_) {
...
bool InMemoryAllDocsPostList::at_end() const
{
	if(db->is_closed()) InMemoryDatabase::throw_database_closed();
...
Xapian::termcount InMemoryDatabase::get_wdf_upper_bound(const string & term) const
{
	// Not a very tight bound in general, but InMemory isn't really built for
	// performance.
	Xapian::termcount cf;
...
Xapian::Document::Internal * InMemoryDatabase::open_document(Xapian::docid did, bool lazy) const
{
	if(closed) InMemoryDatabase::throw_database_closed();
	Assert(did != 0);
...
TermList * InMemoryDatabase::open_metadata_keylist(const string &) const
{
	if(closed) InMemoryDatabase::throw_database_closed();
