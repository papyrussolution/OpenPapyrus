...
// Helper function.
static inline void handle_prune(TermList *& old, TermList * result)
{
	if(result) {
		delete old;
		old = result;
...
TermList * OrTermList::skip_to(const string & term)
{
...
PositionList* OrTermList::positionlist_begin() const
{
	Assert(false);
	return NULL;
}
