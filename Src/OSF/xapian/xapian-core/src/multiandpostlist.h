...
class MultiAndPostList : public PostList {
	...
	void operator =(const MultiAndPostList &); /// Don't allow assignment.
	MultiAndPostList(const MultiAndPostList &); /// Don't allow copying.
	...
	double * max_wt; /// Array of maximum weights for the sub-postlists.
	double max_total; /// Total maximum weight (== sum of max_wt values).
	...
	double new_min(double w_min, size_t n) const { return w_min - (max_total - max_wt[n]); }
...
public:
	 ...
	 */
	template <class RandomItor> MultiAndPostList(RandomItor pl_begin, RandomItor pl_end, PostListTree * matcher_, Xapian::doccount db_size_) : 
		did(0), n_kids(pl_end - pl_begin), plist(NULL), max_wt(NULL), max_total(0), db_size(db_size_), matcher(matcher_)
	{
		...
		std::partial_sort_copy(pl_begin, pl_end, plist, plist + n_kids, ComparePostListTermFreqAscending());
	}
	/** Construct as the decay product of an OrPostList or AndMaybePostList. */
	MultiAndPostList(PostList * l, PostList * r, double lmax, double rmax, PostListTree * matcher_, Xapian::doccount db_size_) : 
		did(0), n_kids(2), plist(NULL), max_wt(NULL), max_total(lmax + rmax), db_size(db_size_), matcher(matcher_)
	{
