...
template <class T> static inline string tostring_unsigned(T value)
{
...
template <class T> static inline string tostring(T value)
{
	// Special case single digit positive numbers.
	// FIXME: is this actually worthwhile?
	if(value < 10 && value >= 0) 
		return string(1, '0' + char(value));
...
namespace Xapian {
	namespace Internal {
		string str(int value) { return tostring(value); }
		string str(uint value) { return tostring_unsigned(value); }
		string str(long value) { return tostring(value); }
		string str(ulong value) { return tostring_unsigned(value); }
		string str(long long value) { return tostring(value); }
		string str(uint64 value) { return tostring_unsigned(value); }

		template <class T> static inline string format(const char * fmt, T value)
		{
			char buf[128];
		#ifdef SNPRINTF
