...
void OrContext::select_elite_set(size_t set_size, size_t out_of)
{
...
PostList * OrContext::postlist_max()
{
	switch(pls.size()) {
		case 0:
		    return NULL;
...
Query::op QueryPostingSource::get_type() const noexcept
{
	return Query::LEAF_POSTING_SOURCE;
}

string QueryPostingSource::get_description() const
{
...
Query::op QueryScaleWeight::get_type() const noexcept
{
	return Query::OP_SCALE_WEIGHT;
}

size_t QueryScaleWeight::get_num_subqueries() const noexcept
{
	return 1;
}

const Query QueryScaleWeight::get_subquery(size_t) const
{
	return subquery;
}

string QueryScaleWeight::get_description() const
{
...
bool QueryTerm::postlist_sub_and_like(AndContext& ctx,
    QueryOptimiser* qopt,
    double factor) const
{
...
void QueryValueRange::serialise(string & result) const
{
	if(slot < 15) {
...
void QueryValueLE::serialise(string & result) const
{
...
void QueryValueGE::serialise(string & result) const
{
	if(slot < 15) {
...
termcount QueryWildcard::get_length() const noexcept
{
...
void QueryWildcard::serialise(string & result) const
{
	result += static_cast<char>(0x0b);
	pack_uint(result, max_expansion);
	result += static_cast<uchar>(flags);
	result += static_cast<uchar>(combiner);
...
termcount QueryEditDistance::get_length() const noexcept
{
	// We currently assume wqf is 1 for calculating the synonym's weight
	// since conceptually the synonym is one "virtual" term.  If we were
	// to combine multiple occurrences of the same synonym expansion into
...
void QueryEditDistance::serialise(string & result) const
{
...
Xapian::termcount QueryBranch::get_length() const noexcept
{
...
#define MULTIWAY(X) static_cast<uchar>(0x80 | (X) << 3)
#define MISC(X) static_cast<uchar>(X)
void QueryBranch::serialise_(string & result, Xapian::termcount parameter) const
{
	static const uchar first_byte[] = {
		MULTIWAY(0), // OP_AND
		MULTIWAY(1), // OP_OR
...
void QueryBranch::serialise(string & result) const
{
	QueryBranch::serialise_(result);
}

void QueryNear::serialise(string & result) const
{
	// FIXME: window - subqueries.size() ?
	QueryBranch::serialise_(result, window);
}

void QueryPhrase::serialise(string & result) const
{
	// FIXME: window - subqueries.size() ?
	QueryBranch::serialise_(result, window);
}

void QueryEliteSet::serialise(string & result) const
{
	// FIXME: set_size - subqueries.size() ?
	QueryBranch::serialise_(result, set_size);
}

void QueryBranch::gather_terms(void * void_terms) const
{
...
void QueryBranch::do_or_like(OrContext& ctx, QueryOptimiser * qopt, double factor,
...
PostList * QueryBranch::do_synonym(QueryOptimiser * qopt, double factor) const
{
...
PostList * QueryBranch::do_max(QueryOptimiser * qopt, double factor) const
{
...
Query::Internal * QueryWindowed::done()
{
...
void QueryScaleWeight::gather_terms(void * void_terms) const
{
...
void QueryAndLike::add_subquery(const Xapian::Query & subquery)
{
...
Query::Internal * QueryAndLike::done()
{
...
bool QueryAndLike::postlist_sub_and_like(AndContext& ctx, QueryOptimiser * qopt, double factor) const
{
	QueryVector::const_iterator i;
	for(i = subqueries.begin(); i != subqueries.end(); ++i) {
		// MatchNothing subqueries should have been removed by done().
...
void QueryOrLike::add_subquery(const Xapian::Query & subquery)
{
...
Query::Internal * QueryOrLike::done()
{
...
void QueryAndNot::add_subquery(const Xapian::Query & subquery)
{
	if(!subqueries.empty()) {
...
Query::Internal * QueryAndNot::done()
{
...
void QueryAndMaybe::add_subquery(const Xapian::Query & subquery)
{
...
Query::Internal * QueryAndMaybe::done()
{
	// Any MatchNothing right subqueries get discarded by add_subquery() - if
	// that leaves just the left subquery, return that.
	//
...
void QueryXor::postlist_sub_xor(XorContext& ctx, QueryOptimiser * qopt, double factor) const
{
	QueryVector::const_iterator i;
	for(i = subqueries.begin(); i != subqueries.end(); ++i) {
		// MatchNothing subqueries should have been removed by done().
...
bool QueryAndMaybe::postlist_sub_and_like(AndContext& ctx,
    QueryOptimiser* qopt,
    double factor) const
{
...
bool QueryFilter::postlist_sub_and_like(AndContext& ctx, QueryOptimiser * qopt, double factor) const
{
	QueryVector::const_iterator i;
	for(i = subqueries.begin(); i != subqueries.end(); ++i) {
		// MatchNothing subqueries should have been removed by done().
...
bool QueryWindowed::postlist_windowed(Query::op op, AndContext& ctx, QueryOptimiser * qopt, double factor) const
{
...
bool QueryPhrase::postlist_sub_and_like(AndContext & ctx, QueryOptimiser * qopt, double factor) const
{
	constexpr auto OP_PHRASE = Query::OP_PHRASE;
	return QueryWindowed::postlist_windowed(OP_PHRASE, ctx, qopt, factor);
}

bool QueryNear::postlist_sub_and_like(AndContext & ctx, QueryOptimiser * qopt, double factor) const
{
...
Query::Internal * QuerySynonym::done()
{
