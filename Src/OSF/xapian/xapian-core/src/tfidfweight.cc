...
static TfIdfWeight::wdf_norm decode_wdf_norm(const string & normalizations)
{
	...
	switch(normalizations[0]) {
		case 'b': return TfIdfWeight::wdf_norm::BOOLEAN;
		case 's': return TfIdfWeight::wdf_norm::SQUARE;
		case 'l': return TfIdfWeight::wdf_norm::LOG;
		case 'P': return TfIdfWeight::wdf_norm::PIVOTED;
		case 'L': return TfIdfWeight::wdf_norm::LOG_AVERAGE;
		case 'n': return TfIdfWeight::wdf_norm::NONE;
		case 'm': return TfIdfWeight::wdf_norm::MAX;
		case 'a': return TfIdfWeight::wdf_norm::AUG;
	}
	throw Xapian::InvalidArgumentError("Normalization string is invalid");
}
...
static TfIdfWeight::idf_norm decode_idf_norm(const string & normalizations)
{
	...
	switch(normalizations[1]) {
		case 'n': return TfIdfWeight::idf_norm::NONE;
		case 's': return TfIdfWeight::idf_norm::SQUARE;
		case 'f': return TfIdfWeight::idf_norm::FREQ;
		case 'P': return TfIdfWeight::idf_norm::PIVOTED;
		case 'p': return TfIdfWeight::idf_norm::PROB;
		case 't': return TfIdfWeight::idf_norm::TFIDF;
	}
	throw Xapian::InvalidArgumentError("Normalization string is invalid");
}
...
string TfIdfWeight::serialise() const
{
	...
	result += static_cast<uchar>(wdf_norm_);
	result += static_cast<uchar>(idf_norm_);
	result += static_cast<uchar>(wt_norm_);
	return result;
}

TfIdfWeight * TfIdfWeight::unserialise(const string & s) const
{
...
// and N are constants.
double TfIdfWeight::get_maxpart() const
{
...
double TfIdfWeight::get_sumextra(Xapian::termcount, Xapian::termcount, Xapian::termcount) const
{
	return 0;
}

double TfIdfWeight::get_maxextra() const
{
	return 0;
}

// Return normalized wdf, idf and weight depending on the normalization string.
double TfIdfWeight::get_wdfn(Xapian::termcount wdf, Xapian::termcount doclen, Xapian::termcount uniqterms, Xapian::termcount wdfdocmax, wdf_norm wdf_normalization) const
{
...
double TfIdfWeight::get_idfn(idf_norm idf_normalization) const
{
...
double TfIdfWeight::get_wtn(double wt, wt_norm wt_normalization) const
{
...
TfIdfWeight * TfIdfWeight::create_from_parameters(const char * p) const
{
	if(*p == '\0')
