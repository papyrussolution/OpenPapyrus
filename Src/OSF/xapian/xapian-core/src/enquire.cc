...
void Enquire::set_query(const Query& query, termcount query_length)
{
...
void Enquire::set_sort_by_key(KeyMaker* sorter, bool reverse)
{
	if(sorter == NULL) {
...
void Enquire::set_sort_by_value_then_relevance(valueno sort_key, bool reverse)
{
...
void Enquire::set_sort_by_relevance_then_value(valueno sort_key, bool reverse)
{
...
void Enquire::set_collapse_key(valueno collapse_key, doccount collapse_max)
{
	internal->collapse_key = collapse_key;
	internal->collapse_max = collapse_max;
}

void Enquire::set_cutoff(int percent_threshold, double weight_threshold)
{
	internal->percent_threshold = percent_threshold;
	internal->weight_threshold = weight_threshold;
}

void Enquire::add_matchspy(MatchSpy* spy)
{
...
TermIterator Enquire::get_matching_terms_begin(docid did) const
{
	return internal->get_matching_terms_begin(did);
}

void Enquire::set_expansion_scheme(const std::string &eweightname, double expand_k) const
{
	if(eweightname == "bo1") {
...
std::string Enquire::get_description() const
{
