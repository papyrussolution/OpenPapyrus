...
namespace Xapian {
	...
	bool   Database::reopen() { return internal->reopen(); }
	void   Database::close() { internal->close(); }
	size_t Database::size() const { return internal->size(); }
	...
	Xapian::termcount Database::get_unique_terms(Xapian::docid did) const
	{
		if(did == 0)
			docid_zero_invalid();
	...
	Xapian::termcount Database::get_wdfdocmax(Xapian::docid did) const
	{
		if(did == 0)
			docid_zero_invalid();
	...
	bool Database::term_exists(const string & term) const { return internal->term_exists(term); } // NB Internal::term_exists() handles term.empty().
	void Database::keep_alive() { internal->keep_alive(); }

	string Database::get_description() const
	{
	...
	string WritableDatabase::get_description() const
	{
