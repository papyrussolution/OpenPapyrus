...
namespace Xapian {
	...
	BB2Weight * BB2Weight::clone() const { return new BB2Weight(param_c); }
	void BB2Weight::init(double factor)
	{
		if(factor == 0.0) {
	...
	double BB2Weight::get_sumpart(Xapian::termcount wdf, Xapian::termcount len, Xapian::termcount, Xapian::termcount) const
	{
		if(wdf == 0) 
			return 0.0;
		...
		double stirling = stirling_value(wdfn + 1.0, y1, stirling_constant_1) - stirling_value(wdfn, y2, stirling_constant_2);
	...
	double BB2Weight::get_maxpart() const { return upper_bound; }
	double BB2Weight::get_sumextra(Xapian::termcount, Xapian::termcount, Xapian::termcount) const { return 0; }
	double BB2Weight::get_maxextra() const { return 0; }
	static inline void parameter_error(const char * message)
	{
		Xapian::Weight::Internal::parameter_error(message, "bb2");
	}
	BB2Weight * BB2Weight::create_from_parameters(const char * p) const
	{
		if(*p == '\0')
