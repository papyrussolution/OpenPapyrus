...
string LMWeight::serialise() const
{
	...
	result += static_cast<uchar>(select_smoothing);
	result += serialise_double(param_smoothing1);
	result += serialise_double(param_smoothing2);
	return result;
}

LMWeight * LMWeight::unserialise(const string & s) const
{
...
double LMWeight::get_sumpart(Xapian::termcount wdf, Xapian::termcount len,
    Xapian::termcount uniqterm, Xapian::termcount) const
{
	...
	if(select_smoothing == JELINEK_MERCER_SMOOTHING) {
		...
		weight_sum =
		    ((((wdf_double - param_smoothing1) >
		    0) ? (wdf_double - param_smoothing1) : 0) / len_double) +
		    ((param_smoothing1 * weight_collection * uniqterm_double) / len_double);
	}
	else {
		weight_sum =
		    (((1 - param_smoothing1) * (wdf_double + (param_smoothing2 * weight_collection)) /
		    (len_double + param_smoothing2)) + (param_smoothing1 * weight_collection));
	}
...
double LMWeight::get_maxpart() const
{
	...
	if(select_smoothing == JELINEK_MERCER_SMOOTHING) {
		...
		upper_bound = (get_doclength_upper_bound() + (param_smoothing1 * weight_collection)) /
		...
		upper_bound =
		    (((1 - param_smoothing1) * (get_doclength_upper_bound() + (param_smoothing2 * weight_collection)) /
		    (get_doclength_upper_bound() + param_smoothing2)) + (param_smoothing1 * weight_collection));
	}
...
double LMWeight::get_maxextra() const
{
	if(select_smoothing == DIRICHLET_PLUS_SMOOTHING) {
...
LMWeight * LMWeight::create_from_parameters(const char * p) const
{
	if(*p == '\0')
