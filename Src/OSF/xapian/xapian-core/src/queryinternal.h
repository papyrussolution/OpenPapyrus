...
namespace Xapian {
	namespace Internal {
		...
		public:
			...
			QueryTerm(const std::string & term_, Xapian::termcount wqf_, Xapian::termpos pos_) : term(term_), wqf(wqf_), pos(pos_) 
			{
			}
		...
		protected:
			Xapian::valueno slot;
		public:
			explicit QueryValueBase(Xapian::valueno slot_) : slot(slot_) 
			{
			}
		...
		protected:
			...
			void do_bool_or_like(BoolOrContext& ctx, QueryOptimiser* qopt, size_t first = 0) const;
		...
		protected:
			explicit QueryAndLike(size_t num_subqueries_) : QueryBranch(num_subqueries_) 
			{
			}
		public:
		...
		protected:
			explicit QueryOrLike(size_t num_subqueries_) : QueryBranch(num_subqueries_) 
			{
			}
		public:
			void add_subquery(const Xapian::Query & subquery);
		...
		protected:
			...
			QueryWindowed(size_t n_subqueries, Xapian::termcount window_) : QueryAndLike(n_subqueries), window(window_) 
			{
			}
			bool postlist_windowed(Xapian::Query::op op, AndContext& ctx, QueryOptimiser * qopt, double factor) const;
		public:
		...
		class QueryNear : public QueryWindowed {
			Xapian::Query::op get_op() const;
		public:
			QueryNear(size_t n_subqueries, Xapian::termcount window_) : QueryWindowed(n_subqueries, window_) 
			{
			}
			void serialise(std::string & result) const;
			bool postlist_sub_and_like(AndContext& ctx, QueryOptimiser* qopt, double factor) const;
		...
		class QueryPhrase : public QueryWindowed {
			Xapian::Query::op get_op() const;
		public:
			QueryPhrase(size_t n_subqueries, Xapian::termcount window_) : QueryWindowed(n_subqueries, window_) 
			{
			}
			void serialise(std::string & result) const;
			bool postlist_sub_and_like(AndContext& ctx, QueryOptimiser* qopt, double factor) const;
		...
		public:
			QueryEliteSet(size_t n_subqueries, Xapian::termcount set_size_) : QueryOrLike(n_subqueries), set_size(set_size_ ? set_size_ : DEFAULT_ELITE_SET_SIZE) 
			{
			}
			void serialise(std::string & result) const;
			PostList * postlist(QueryOptimiser * qopt, double factor) const;
		...
		public:
			QueryWildcard(const std::string &pattern_, Xapian::termcount max_expansion_, int flags_, Query::op combiner_);
		...
		public:
			QueryEditDistance(const std::string & pattern_, Xapian::termcount max_expansion_, int flags_,
				Query::op combiner_, uint edit_distance_ = 2, size_t fixed_prefix_len_ = 0) : pattern(pattern_),
				max_expansion(max_expansion_), flags(flags_), combiner(combiner_), edcalc(pattern), edit_distance(edit_distance_), fixed_prefix_len(fixed_prefix_len_) 
			{
			}
			...
			QueryEditDistance* change_combiner(Xapian::Query::op new_op) 
			{
				...
				return new QueryEditDistance(pattern, max_expansion, flags, new_op, edit_distance, fixed_prefix_len);
			}
			std::string get_description() const;
		};
