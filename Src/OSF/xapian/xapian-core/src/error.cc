...
Xapian::Error::Error(const std::string &msg_, const std::string &context_, const char * type_, const char * error_string_) : 
	msg(msg_), context(context_), error_string(), type(type_), my_errno(0)
{
	if(error_string_) 
...
Xapian::Error::get_error_string() const
{
	if(error_string.empty()) {
		...
		if(my_errno < 0 || my_errno >= WSABASEERR) {
			...
			DWORD len;
			char * error;
			len = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM|FORMAT_MESSAGE_ALLOCATE_BUFFER, 0, e, 0, (CHAR*)&error, 0, 0);
			if(error) {
