...
namespace Xapian {
	...
	string MultiValueKeyMaker::operator()(const Xapian::Document & doc) const
	{
		...
		while(true) {
			...
			if(++i == slots.end() && !reverse_sort) {
				if(v.empty()) {
					result.resize(last_not_empty_forwards); // Trim off all the trailing empty forwards values.
				}
				else {
					result += v; // No need to adjust the last value if it's sorted forwards.
				}
				break;
			}
			if(reverse_sort) {
	...
	KeyMaker* MultiValueKeyMaker::unserialise(const string & serialised, const Registry&) const
	{
