...
Xapian::doccount BoolOrPostList::get_termfreq_min() const
{
	Assert(n_kids != 0);
...
double BoolOrPostList::get_weight(Xapian::termcount, Xapian::termcount, Xapian::termcount) const
{
	return 0;
}
...
Xapian::doccount BoolOrPostList::get_termfreq_max() const
{
	Assert(n_kids != 0);
...
TermFreqs BoolOrPostList::get_termfreq_est_using_stats(const Xapian::Weight::Internal& stats) const
{
	Assert(n_kids != 0);
	// We calculate the estimate assuming independence.  The simplest
	// way to calculate this seems to be a series of (n_kids - 1) pairwise
