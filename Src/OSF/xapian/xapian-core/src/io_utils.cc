...
int io_open_block_wr(const char * fname, bool anew)
{
...
#else
	{
		...
		memzero(toclose, sizeof(toclose));
		fd = badfd;
		do {
			toclose[fd] = 1;
			fd = _dup(fd);
...
bool io_readahead_block(int fd, size_t n, off_t b, off_t o)
{
	o += b * n;
...
retry:
	...
	if(posixy_rename(tmp_file.c_str(), real_file.c_str()) < 0) {
		...
		if(_unlink(tmp_file.c_str()) == 0 || errno != ENOENT) {
			errno = saved_errno;
			return false;
		}
	}
