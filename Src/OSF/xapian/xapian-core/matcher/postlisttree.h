...
class PostListTree {
	...
	bool need_wdfdocmax;
	double max_weight;
	Xapian::doccount current_shard = 0; /// The current shard.
	...
	Xapian::doccount n_shards = 0; /// The number of shards.
...
public:
	PostListTree(ValueStreamDocument& vsdoc_, Xapian::Database& db_, const Xapian::Weight& wtscheme) : 
		need_doclength(wtscheme.get_sumpart_needs_doclength_()), need_unique_terms(wtscheme.get_sumpart_needs_uniqueterms_()),
		need_wdfdocmax(wtscheme.get_sumpart_needs_wdfdocmax_()), vsdoc(vsdoc_), db(db_) 
	{
	}
	~PostListTree() 
	{
	...
	Xapian::docid get_docid() const { return unshard(pl->get_docid(), current_shard, n_shards); }
	Xapian::termcount get_doclength(Xapian::docid shard_did) const { return shard_db->get_doclength(shard_did); }
	...
	Xapian::termcount count_matching_subqs() const { return pl->count_matching_subqs(); }
