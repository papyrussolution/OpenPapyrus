...
public:
	...
	CollapseData(Xapian::doccount item, Xapian::docid did) : items(1, {item, did}), next_best_weight(0), collapse_count(0) 
	{
	}
...
class Collapser {
	std::unordered_map<std::string, CollapseData> table; /// Map from collapse key values to the items we're keeping for them.
	Xapian::doccount entry_count = 0; /// How many items we're currently keeping in @a table.
...
public:
	CollapserLite(Xapian::doccount collapse_max_) : collapse_max(collapse_max_) 
	{
	}
	/// Return true if collapsing is active for this match.
	operator bool() const { return collapse_max != 0; }
