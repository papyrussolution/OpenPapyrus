...
class ValueStreamDocument : public Xapian::Document::Internal {
	...
	ValueStreamDocument(const Xapian::Database& db_, Xapian::doccount n_shards_) : 
		Internal(n_shards_ == 1 ? db_.internal.get() : static_cast<MultiDatabase*>(db_.internal.get())->shards[0], 0), db(db_), n_shards(n_shards_) 
	{
	}

public:
	explicit ValueStreamDocument(const Xapian::Database& db_) : ValueStreamDocument(db_, db_.internal->size()) 
	{
	}
