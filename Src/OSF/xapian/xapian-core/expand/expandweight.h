...
public:
	Xapian::doccount dbsize; /// Size of the subset of a multidb to which the value in termfreq applies.
	Xapian::doccount termfreq; /// Term frequency (for a multidb, may be for a subset of the databases).
	Xapian::termcount rcollection_freq; /// The number of times the term occurs in the rset.
	Xapian::doccount rtermfreq; /// The number of documents from the RSet indexed by the current term (r).
	double multiplier; /// The multiplier to be used in TradWeight query expansion.
	 ...
	 */
	ExpandStats(Xapian::doclength avlen_, double expand_k_ = 0.0) : avlen(avlen_), expand_k(expand_k_) 
	{
	}
...
class ExpandWeight {
	const Xapian::Database db; /// The combined database.
	Xapian::doccount dbsize; /// The number of documents in the whole database.
	Xapian::doccount rsize; /// The number of documents in the RSet.
	Xapian::termcount collection_freq; /// The collection frequency of the term.
	Xapian::totallength collection_len; /// The total length of the database.
...
public:
	 ...
	 */
	ExpandWeight(const Xapian::Database& db_, Xapian::doccount rsize_, bool use_exact_termfreq_, double expand_k_ = 0.0) : db(db_), dbsize(db.get_doccount()),
		rsize(rsize_), collection_freq(0), collection_len(db.get_total_length()), use_exact_termfreq(use_exact_termfreq_), stats(db.get_average_length(), expand_k_) 
	{
	}
...
public:
	 ...
	 */
	TradEWeight(const Xapian::Database& db_, Xapian::doccount rsize_, bool use_exact_termfreq_, double expand_k_) : ExpandWeight(db_, rsize_, use_exact_termfreq_, expand_k_) 
	{
	}
...
public:
	 ...
	 *  All the parameters are passed to the parent ExpandWeight object.
	 */
	Bo1EWeight(const Xapian::Database& db_, Xapian::doccount rsize_, bool use_exact_termfreq_) : ExpandWeight(db_, rsize_, use_exact_termfreq_) 
	{
	}
	double get_weight() const;
};
