...
namespace Xapian {
namespace Internal {
void ExpandWeight::collect_stats(TermList * merger, const std::string & term)
{
	...
	if(stats.dbsize == dbsize) {
		...
		if(use_exact_termfreq) {
			...
			LOGLINE(EXPAND, "termfreq is approx " << stats.termfreq << " * " << dbsize << " / " << stats.dbsize << " = " << tf);
			...
			if(stats.termfreq > termfreq_upper_bound) {
				LOGLINE(EXPAND, "termfreq can't be more than dbsize - (rsize + rtermfreq)");
