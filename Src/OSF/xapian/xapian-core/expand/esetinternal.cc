...
Xapian::termcount ESet::get_ebound() const
{
	return internal->ebound;
}

std::string ESet::get_description() const
{
	string desc = "ESet(";
	desc += ')';
	return desc;
...
std::string ESetIterator::operator*() const
{
...
double ESetIterator::get_weight() const
{
	Assert(off_from_end != 0);
	AssertRel(off_from_end, <=, eset.internal->items.size());
...
std::string ESetIterator::get_description() const
{
