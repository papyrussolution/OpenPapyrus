...
static int64 le4(const uchar * p) { return ((p[0] << 16) + (((int64)p[1]) << 24) + (p[2] << 0) + (p[3] << 8)); }
static int64 be4(const uchar * p) { return ((((int64)p[0]) << 24) + (p[1] << 16) + (p[2] << 8) + (p[3])); }
...
int archive_read_support_format_cpio(Archive * _a)
{
	...
	r = __archive_read_register_format(a, cpio, "cpio", archive_read_format_cpio_bid, archive_read_format_cpio_options,
...
static int archive_read_format_cpio_read_header(ArchiveRead * a, ArchiveEntry * entry)
{
	...
	if(archive_entry_filetype(entry) == AE_IFLNK) {
		...
		hl = __archive_read_ahead(a, (size_t)cpio->entry_bytes_remaining, NULL);
		if(hl == NULL)
			return ARCHIVE_FATAL;
		if(archive_entry_copy_symlink_l(entry, (const char *)hl, (size_t)cpio->entry_bytes_remaining, sconv) != 0) {
			if(errno == ENOMEM) {
			...
			switch(p[5]) {
				...
				case '2':
				    if(memcmp("07070", p, 5) == 0 && is_hex(p, newc_header_size)) {
					    skip = p - static_cast<const char *>(h);
...
static int header_newc(ArchiveRead * a, struct cpio * cpio, ArchiveEntry * entry, size_t * namelength, size_t * name_pad)
{
	...
	*name_pad = (2 - *namelength) & 3;
	...
	if(*name_pad > SIZE_MAX - *namelength) {
		archive_set_error(&a->archive, ARCHIVE_ERRNO_FILE_FORMAT, "cpio archive has invalid namelength");
		return ARCHIVE_FATAL;
	}
	/*
...
static int is_afio_large(const char * h, size_t len)
{
	if(len < afiol_header_size)
		return 0;
	if(h[afiol_ino_m_offset] != 'm' || h[afiol_mtime_n_offset] != 'n' || h[afiol_xsize_s_offset] != 's' || h[afiol_filesize_c_offset] != ':')
		return 0;
    ...
    ArchiveEntry * entry, size_t * namelength, size_t * name_pad)
{
	...
	archive_entry_set_nlink(entry, (uint)atol8(header + odc_nlink_offset, odc_nlink_size));
 ...
 */
static int header_afiol(ArchiveRead * a, struct cpio * cpio, ArchiveEntry * entry, size_t * namelength, size_t * name_pad)
{
	const void * h;
	const char * header;
	a->archive.archive_format = ARCHIVE_FORMAT_CPIO_AFIO_LARGE;
	...
	archive_entry_set_nlink(entry, (uint)atol16(header + afiol_nlink_offset, afiol_nlink_size));
