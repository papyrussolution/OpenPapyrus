...
static int older(BY_HANDLE_FILE_INFORMATION *, ArchiveEntry *);
static int permissive_name_w(struct archive_write_disk *);
static int restore_entry(struct archive_write_disk *);
static int set_acls(struct archive_write_disk *, HANDLE h, const wchar_t *, archive_acl *);
...
#define bhfi_size(bhfi) ((((int64)(bhfi)->nFileSizeHigh) << 32) + (bhfi)->nFileSizeLow)
...
static int la_CreateHardLinkW(wchar_t * linkname, wchar_t * target)
{
	...
	if(!set) {
		set = 1;
		f = static_cast<BOOLEAN (WINAPI *)(LPWSTR, LPWSTR, LPSECURITY_ATTRIBUTES)>(la_GetFunctionKernel32("CreateHardLinkW"));
	}
	if(!f) {
		errno = ENOTSUP;
		return 0;
 ...
 * the link target
 */
static int la_CreateSymbolicLinkW(const wchar_t * linkname, const wchar_t * target, int linktype) 
{
 ...
 */
static int older(BY_HANDLE_FILE_INFORMATION * st, ArchiveEntry * entry)
{
	time_t sec;
	long nsec;
