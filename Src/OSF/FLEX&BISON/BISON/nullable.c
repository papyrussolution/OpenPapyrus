...
void nullable_compute(void)
{
	...
	symbol_number * s2;
	rule_list * p;
	symbol_number * squeue = (symbol_number *)xnmalloc(nvars, sizeof *squeue);
	size_t * rcount = (size_t *)xcalloc(nrules, sizeof *rcount);
	/* RITEM contains all the rules, including useless productions.
	   Hence we must allocate room for useless nonterminals too.  */
	rule_list ** rsets = (rule_list **)xcalloc(nvars, sizeof *rsets);
	/* This is said to be more elements than we actually use.
	   Supposedly NRITEMS - NRULES is enough.  But why take the risk?  */
	rule_list * relts = (rule_list *)xnmalloc(nritems + nvars + 1, sizeof *relts);
	nullable = (bool *)xcalloc(nvars, sizeof *nullable);
	s1 = s2 = squeue;
	p = relts;
