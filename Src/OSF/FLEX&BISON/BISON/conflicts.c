...
static inline void log_resolution(rule * r, symbol_number token,
...
void conflicts_solve(void)
{
	...
	conflicts = (char*)xcalloc(nstates, sizeof *conflicts);
...
static size_t count_state_sr_conflicts(state * s)
{
	int i;
	transitions * trans = s->transitions;
	reductions * reds = s->reductions;
...
static size_t count_state_rr_conflicts(state * s, bool one_per_token)
{
	int i;
	reductions * reds = s->reductions;
	size_t res = 0;
...
static size_t count_rr_conflicts(bool one_per_token)
{
	size_t res = 0;
	state_number i;
