...
{
	symbol_list * res = (symbol_list *)xmalloc(sizeof *res);
	res->content_type = symbol_list::SYMLIST_SYMBOL;
	res->content.sym = sym;
...
{
	symbol_list * res = (symbol_list *)xmalloc(sizeof *res);
	res->content_type = symbol_list::SYMLIST_TYPE;
	res->content.sem_type = (semantic_type *)xmalloc(sizeof(semantic_type));
...
void symbol_list_syms_print(const symbol_list * l, FILE * f)
{
	...
	for(/* Nothing. */; l && l->content.sym; l = l->next) {
		fputs(sep, f);
		fputs(l->content_type == symbol_list::SYMLIST_SYMBOL ? "symbol: " : l->content_type == symbol_list::SYMLIST_TYPE ? "type: " : "invalid content_type: ", f);
...
void symbol_list_free(symbol_list * list)
{
	...
	for(node = list; node; node = next) {
		...
		if(node->content_type == symbol_list::SYMLIST_TYPE)
...
symbol_list * symbol_list_n_get(symbol_list * l, int n)
{
	...
	aver(l->content_type == symbol_list::SYMLIST_SYMBOL);
	aver(l->content.sym);
	return l;
}
...
{
	return (!node || (node->content_type == symbol_list::SYMLIST_SYMBOL && !node->content.sym));
}
