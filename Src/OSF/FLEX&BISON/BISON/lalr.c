...
{
	state_number s;
	goto_number * temp_map = (goto_number *)xnmalloc(nvars + 1, sizeof *temp_map);
	goto_map = (goto_number *)xcalloc(nvars + 1, sizeof *goto_map);
	ngotos = 0;
	for(s = 0; s < nstates; ++s) {
...
static void initialize_F(void)
{
	...
	goto_number * edge = (goto_number *)xnmalloc(ngotos + 1, sizeof *edge);
...
static void add_lookback_edge(state * s, rule * r, goto_number gotono)
{
	...
	goto_list * sp = (goto_list *)xmalloc(sizeof *sp);
...
{
	goto_number * edge = (goto_number *)xnmalloc(ngotos + 1, sizeof *edge);
	state_number * states1 = (state_number *)xnmalloc(ritem_longest_rhs() + 1, sizeof *states1);
	goto_number i;
	includes = (goto_number **)xnmalloc(ngotos, sizeof *includes);
	for(i = 0; i < ngotos; i++) {
		int nedges = 0;
		...
		if(nedges == 0)
			...
			int j;
			includes[i] = (goto_number *)xnmalloc(nedges + 1, sizeof includes[i][0]);
			for(j = 0; j < nedges; j++)
				includes[i][j] = edge[j];
...
static int state_lookahead_tokens_count(state * s, bool default_reduction_only_for_accept)
{
...
void lalr(void)
{
	...
	set_goto_map();
	initialize_F();
	lookback = (goto_list **)xcalloc(nLA, sizeof *lookback);
	build_relations();
	compute_FOLLOWS();
