...
static void warnings_print_categories(warnings warn_flags, FILE * f)
{
	/* Display only the first match, the second is "-Wall".  */
	size_t i;
	for(i = 0; warnings_args[i]; ++i)
		if(warn_flags & warnings_types[i]) {
			severity s = warning_severity((warnings)warnings_types[i]);
			fprintf(f, " [-W%s%s]", s == severity_error ? "error=" : "", warnings_args[i]);
			return;
		}
}
 ...
 */
static void error_message(const BFLocation * loc, warnings flags, const char * prefix, const char * message, va_list args)
{
	unsigned pos = 0;
	if(loc)
...
static void complains(const BFLocation * loc, warnings flags, const char * message, va_list args)
{
...
void complain(BFLocation const * loc, warnings flags, const char * message, ...)
{
	va_list args;
...
void complain_indent(BFLocation const * loc, warnings flags, unsigned * indent, const char * message, ...)
{
	va_list args;
	indent_ptr = indent;
	va_start(args, message);
...
void complain_args(BFLocation const * loc, warnings w, unsigned * indent, int argc, char * argv[])
{
	switch(argc) {
		case 1:
...
void deprecated_directive(BFLocation const * loc, char const * old, char const * upd)
{
	if(feature_flag & feature_caret)
		complain(loc, Wdeprecated, _("deprecated directive, use %s"), quote_n(1, upd));
	else
