...
static void record_merge_function_type(int merger, uniqstr type, BFLocation declaration_loc)
{
	int merger_find;
...
static symbol_list * grammar_symbol_append(symbol * sym, BFLocation loc)
{
...
void grammar_current_rule_begin(symbol * lhs, BFLocation loc, named_ref * lhs_name)
{
	symbol_list* p;
	...
	if(lhs->_Cls == unknown_sym) {
		lhs->_Cls = nterm_sym;
		lhs->number = nvars;
		++nvars;
	}
	else if(lhs->_Cls == token_sym)
...
static void grammar_rule_check(const symbol_list * r)
{
	/* Type check.
	...
	if(!r->action_props.code && r->content.sym->type_name) {
		...
		if(first_rhs) {
			...
			if(!UNIQSTR_EQ(lhs_type, rhs_type))
				complain(&r->location, Wother, _("type clash on default action: <%s> != <%s>"), lhs_type, rhs_type);
		}
...
void grammar_midrule_action(void)
{
	...
	BFLocation dummy_location = current_rule->action_props.location;
...
void grammar_current_rule_dprec_set(int dprec, BFLocation loc)
{
	...
	if(dprec <= 0)
		...
		duplicate_directive("%dprec", current_rule->dprec_location, loc);
	else{
...
void grammar_current_rule_symbol_append(symbol * sym, BFLocation loc, named_ref * name)
{
	symbol_list * p;
...
void grammar_current_rule_action_append(const char * action, BFLocation loc, named_ref * name, bool is_predicate)
{
	if(current_rule->action_props.code)
		grammar_midrule_action();
...

		{
			...
			for(p = p->next; p->content.sym; p = p->next) {
				...
				/* A rule gets by default the precedence and associativity
				   of its last token.  */
				if(p->content.sym->_Cls == token_sym && default_prec)
...
void reader(void)
{
	...
	accept->_Cls = nterm_sym;
	accept->number = nvars++;

	/* Construct the error token */
	errtoken = symbol_get("error", empty_location);
	errtoken->_Cls = token_sym;
	errtoken->number = ntokens++;

	/* Construct a token that represents all undefined literal tokens.
	   It is always token number 2.  */
	undeftoken = symbol_get("$undefined", empty_location);
	undeftoken->_Cls = token_sym;
