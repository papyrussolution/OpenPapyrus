 ...
 *       same.
 */
static void ielr_compute_internal_follow_edges(bitset ritem_sees_lookahead_set,
 ...
 */
static void ielr_compute_follow_kernel_items(bitset ritem_sees_lookahead_set,
 ...
 */
static void ielr_compute_always_follows(goto_number *** edgesp,
...
static state *** ielr_compute_predecessors(void)
{
	...
	state *** result = (state ***)xnmalloc(nstates, sizeof *result);
	for(i = 0; i < nstates; ++i)
	...
	for(i = 0; i < nstates; ++i) {
		result[i] = (state **)xnmalloc(predecessor_counts[i]+1, sizeof *result[i]);
 ...
 */
static void ielr_compute_auxiliary_tables(bitsetv * follow_kernel_itemsp,
    bitsetv * always_followsp,
    state **** predecessorsp)
{
	goto_number ** edges;
 ...
 *     state.
 */
static void ielr_compute_annotation_lists(bitsetv follow_kernel_items,
