...
static void allocate_itemsets(void)
{
	...
	kernel_base = (item_number **)xnmalloc(nsyms, sizeof *kernel_base);
	kernel_items = (item_number *)xnmalloc(count, sizeof *kernel_items);
...
static void allocate_storage(void)
{
	...
	state_hash_new();
	shift_symbol = (symbol_number *)xnmalloc(nsyms, sizeof *shift_symbol);
}
