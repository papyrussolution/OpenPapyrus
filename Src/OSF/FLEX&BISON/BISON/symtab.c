...
BFLocation startsymbol_location;
...
static symbol * symbol_new(uniqstr tag, BFLocation loc)
{
...
static void locations_sort(BFLocation * first, BFLocation * second)
{
	if(0 < location_cmp(*first, *second)) {
		BFLocation tmp = *first;
		*first = *second;
		*second = tmp;
	}
}
...
static semantic_type * semantic_type_new(uniqstr tag, const BFLocation * loc)
{
...
static void symbol_redeclaration(symbol * s, const char * what, BFLocation first, BFLocation second)
{
	unsigned i = 0;
...
static void semantic_type_redeclaration(semantic_type * s, const char * what, BFLocation first, BFLocation second)
{
	unsigned i = 0;
	locations_sort(&first, &second);
	complain_indent(&second, complaint, &i,
...
void symbol_precedence_set(symbol * sym, int prec, assoc a, BFLocation loc)
{
	if(a != undef_assoc) {
		if(sym->prec)
			symbol_redeclaration(sym, assoc_to_string(a), sym->prec_location, loc);
		else{
			sym->prec = prec;
			sym->assoc = a;
...
{
	bool warned = false;
	if(sym->_Cls != unknown_sym && sym->_Cls != _cls) {
		complain(&loc, complaint, _("symbol %s redefined"), sym->tag);
		/* Don't report both "redefined" and "redeclared".  */
		warned = true;
	}
	if(_cls == nterm_sym && sym->_Cls != nterm_sym)
		sym->number = nvars++;
	else if(_cls == token_sym && sym->number == NUMBER_UNDEFINED)
		sym->number = ntokens++;
	sym->_Cls = _cls;
	if(declaring) {
...
void symbol_user_token_number_set(symbol * sym, int user_token_number, BFLocation loc)
{
	...
	if(*user_token_numberp != USER_NUMBER_UNDEFINED && *user_token_numberp != user_token_number)
...
static inline bool symbol_check_defined(symbol * sym)
{
	if(sym->_Cls == unknown_sym) {
		assert(sym->status != declared);
		complain(&sym->location,
		    sym->status == needed ? complaint : Wother, _("symbol %s is used, but is not defined as a token and has no rules"), sym->tag);
		sym->_Cls = nterm_sym;
		sym->number = nvars++;
	}
	{
...
static inline bool semantic_type_check_defined(semantic_type * sem_type)
{
...
static bool symbol_check_defined_processor(void * sym, void * null ATTRIBUTE_UNUSED)
{
	return symbol_check_defined((symbol *)sym);
}

static bool semantic_type_check_defined_processor(void * sem_type, void * null ATTRIBUTE_UNUSED)
{
	return semantic_type_check_defined((semantic_type *)sem_type);
}
...
static inline void symbol_check_alias_consistency(symbol * pThis)
{
	symbol * sym = pThis;
	symbol * str = pThis->alias;
	/* Check only the symbol in the symbol-string pair.  */
	if(!(pThis->alias && pThis->user_token_number == USER_NUMBER_HAS_STRING_ALIAS))
		return;
	...
	}
	{
		...
		for(i = 0; i < CODE_PROPS_SIZE; ++i)
			if(str->props[i].code)
				symbol_code_props_set(sym, (code_props_type)i, &str->props[i]);
			else if(sym->props[i].code)
				symbol_code_props_set(str, (code_props_type)i, &sym->props[i]);
	}
...
static bool symbol_check_alias_consistency_processor(void * pThis, void * null ATTRIBUTE_UNUSED)
{
	symbol_check_alias_consistency((symbol *)pThis);
	return true;
}
...
static inline bool symbol_pack(symbol * pThis)
{
	aver(pThis->number != NUMBER_UNDEFINED);
	if(pThis->_Cls == nterm_sym)
		pThis->number += ntokens;
	else if(pThis->user_token_number == USER_NUMBER_HAS_STRING_ALIAS)
		return true;
	symbols[pThis->number] = pThis;
	return true;
}

static bool symbol_pack_processor(void * pThis, void * null ATTRIBUTE_UNUSED)
{
	return symbol_pack((symbol *)pThis);
}
...
static void user_token_number_redeclaration(int num, symbol * first, symbol * second)
{
	...
	complain_indent(&second->location, complaint, &i, _("user token number %d redeclaration for %s"), num, second->tag);
	i += SUB_INDENT;
	complain_indent(&first->location, complaint, &i, _("previous declaration for %s"), first->tag);
}
...
static inline bool symbol_translation(symbol * pThis)
{
	/* Non-terminal? */
	if(pThis->_Cls == token_sym && pThis->user_token_number != USER_NUMBER_HAS_STRING_ALIAS) {
		/* A token which translation has already been set? */
		if(token_translations[pThis->user_token_number] != undeftoken->number)
			user_token_number_redeclaration(pThis->user_token_number, symbols[token_translations[pThis->user_token_number]], pThis);
		else
			token_translations[pThis->user_token_number] = pThis->number;
	}
	return true;
}

static bool symbol_translation_processor(void * pThis, void * null ATTRIBUTE_UNUSED)
{
	return symbol_translation((symbol *)pThis);
}
...
{
	return hash_compare_semantic_type((semantic_type *)m1, (semantic_type *)m2);
}
...
symbol * symbol_from_uniqstr(const uniqstr key, BFLocation loc)
{
	...
	symbol * entry;
	probe.tag = key;
	entry = (symbol *)hash_lookup(symbol_table, &probe);
	if(!entry) {
...
semantic_type * semantic_type_from_uniqstr(const uniqstr key, const BFLocation * loc)
{
	...
	semantic_type * entry;
	probe.tag = key;
	entry = (semantic_type *)hash_lookup(semantic_type_table, &probe);
	if(!entry) {
		/* First insertion in the hash. */
		entry = (semantic_type *)semantic_type_new(key, loc);
...
static void symbols_do(Hash_processor processor, void * processor_data,
    struct hash_table * table, symbol *** sorted)
{
	size_t count = hash_get_n_entries(table);
	if(!*sorted) {
		*sorted = (symbol **)xnmalloc(count, sizeof **sorted);
...
static void symbols_token_translations_init(void)
{
	...
	for(i = 0; i < ntokens; ++i) {
		symbol * p_this = symbols[i];
		if(p_this->user_token_number != USER_NUMBER_UNDEFINED) {
			if(p_this->user_token_number > max_user_token_number)
				max_user_token_number = p_this->user_token_number;
			if(p_this->user_token_number == 256)
	...
	for(i = 0; i < ntokens; ++i) {
		symbol * p_this = symbols[i];
		if(p_this->user_token_number == USER_NUMBER_UNDEFINED)
			p_this->user_token_number = ++max_user_token_number;
		if(p_this->user_token_number > max_user_token_number)
			max_user_token_number = p_this->user_token_number;
	}

	token_translations = (symbol_number *)xnmalloc(max_user_token_number + 1, sizeof *token_translations);
...
{
	symbols_do(symbol_check_alias_consistency_processor, NULL, symbol_table, &symbols_sorted);
	...
	if(startsymbol->_Cls == unknown_sym)
		complain(&startsymbol_location, fatal, _("the start symbol %s is undefined"), startsymbol->tag);
	else if(startsymbol->_Cls == token_sym)
		complain(&startsymbol_location, fatal, _("the start symbol %s is a token"), startsymbol->tag);
}
...
{
	symgraphlink * l = (symgraphlink *)xmalloc(sizeof *l);
	l->id = id;
	l->next = next;
	return l;
}
...
{
	graphid i;
	used_assoc = (signed char *)xcalloc(nsyms, sizeof *used_assoc);
	for(i = 0; i < nsyms; ++i)
		used_assoc[i] = false;
}
