...
void relation_digraph(relation r, relation_node size, bitsetv * function)
{
	relation_node i;
	infinity = size + 2;
	INDEX = (relation_nodes)xcalloc(size + 1, sizeof *INDEX);
...
void relation_transpose(relation * R_arg, relation_node n)
{
	relation r = *R_arg;
	/* The result. */
	relation new_R = (relation)xnmalloc(n, sizeof *new_R);
	/* END_R[I] -- next entry of NEW_R[I]. */
	relation end_R = (relation)xnmalloc(n, sizeof *end_R);
	/* NEDGES[I] -- total size of NEW_R[I]. */
	size_t * nedges = (size_t *)xcalloc(n, sizeof *nedges);
	relation_node i;
	relation_node j;
