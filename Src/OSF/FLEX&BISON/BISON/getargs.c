 ...
 */
static void flags_argmatch(const char * option, const char * const keys[], const int values[], int all, int * flags, char * args)
{
	if(args) {
		args = strtok(args, ",");
		while(args) {
...
| Process the options.  |
   `----------------------*/
/* 
	Shorts options. Should be computed from long_options.  
*/
		...
		switch(c) {
		    ...
		    case 'F': /* -FNAME[=VALUE]. */
		    {
			    ...
			    muscle_percent_define_insert(name, command_line_location(),
				    value ? value : "", c == 'D' ? MUSCLE_PERCENT_DEFINE_D : MUSCLE_PERCENT_DEFINE_F);
		    }
		    break;

		    case 'I': include = AS_FILE_NAME(optarg); break;
		    case 'L': language_argmatch(optarg, command_line_prio, command_line_location()); break;
		    case 'S': skeleton_arg(AS_FILE_NAME(optarg), command_line_prio, command_line_location()); break;
		    case 'T': FLAGS_ARGMATCH(trace, optarg); break;
		    case 'V': version(); exit(EXIT_SUCCESS);
		    case 'W': FLAGS_ARGMATCH(warnings, optarg); break;
		    case 'b': spec_file_prefix = AS_FILE_NAME(optarg); break;
		    case 'd':
