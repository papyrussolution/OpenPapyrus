...
static int state_number_as_int(state_number s)
{
	return s;
}
...
#define TRANSITION_IS_ERROR(Transitions, Num)    (TRANSITION_SYMBOL(Transitions, Num) == errtoken->number)
/* 
...
#define FOR_EACH_SHIFT(Transitions, Iter)                       \
	for(Iter = 0; Iter < Transitions->num && (TRANSITION_IS_DISABLED(Transitions, Iter) \
		|| TRANSITION_IS_SHIFT(Transitions, Iter)); ++Iter)                     \
...
state * state_new(symbol_number accessing_symbol, size_t core_size, item_number * core);
