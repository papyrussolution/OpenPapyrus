...
static AnnotationList* AnnotationList__alloc_on_obstack(ContributionIndex contribution_count, struct obstack * annotations_obstackp)
{
	...
	size_t contributions_size = contribution_count * sizeof result->contributions[0];
	result = (AnnotationList*)obstack_alloc(annotations_obstackp, offsetof(AnnotationList, contributions) + contributions_size);
	result->next = NULL;
 ...
 */
static bool AnnotationList__insertInto(AnnotationList * self, AnnotationList ** list, size_t nitems)
{
	AnnotationList ** node;
...
static bool AnnotationList__compute_lhs_contributions(state * s, rule * the_rule,
	...
	for(predecessor = predecessors[s->number]; *predecessor; ++predecessor) {
		...
		annotation_node->inadequacyNode = self->inadequacyNode;
		{
			...
			for(ci = 0; ci < self->inadequacyNode->contributionCount; ++ci) {
				symbol_number contribution_token = InadequacyList__getContributionToken(self->inadequacyNode, ci)->number;
				if(AnnotationList__isContributionAlways(self, ci)) {
					annotation_node->contributions[ci] = NULL;
					continue;
				}
				annotation_node->contributions[ci] = Sbitset__new_on_obstack((*predecessor)->nitems, annotations_obstackp);
				{
					size_t predecessor_item = 0;
					Sbitset sbiter_item;
					Sbitset__Index self_item;
					SBITSET__FOR_EACH(self->contributions[ci], s->nitems, sbiter_item, self_item)
					{
						...
						if(item_number_is_rule_number(ritem[s->items[self_item] - 2])) {
							Sbitset items;
							unsigned int rulei;
							for(rulei = s->items[self_item]; !item_number_is_rule_number(ritem[rulei]); ++rulei)
								;
							if(AnnotationList__compute_lhs_contributions(*predecessor, &rules[item_number_as_rule_number(ritem[rulei])],
								    contribution_token, follow_kernel_items, always_follows, predecessors, item_lookahead_sets, &items, annotations_obstackp)) {
								obstack_free(annotations_obstackp, annotation_node->contributions[ci]);
								annotation_node->contributions[ci] = NULL;
								break;
							}
							else{
				...
				if(annotation_node->contributions[ci]) {
					Sbitset biter;
					Sbitset__Index i;
					SBITSET__FOR_EACH(annotation_node->contributions[ci], (*predecessor)->nitems, biter, i)
					{
 ...
 */
static bool AnnotationList__stateMakesContribution(AnnotationList const * self, size_t nitems, ContributionIndex ci, bitset * lookaheads)
{
	if(AnnotationList__isContributionAlways(self, ci))
		return true;
	if(!lookaheads)
		return false;
	{
		symbol_number token = InadequacyList__getContributionToken(self->inadequacyNode, ci)->number;
		Sbitset__Index item;
		Sbitset biter;
		SBITSET__FOR_EACH(self->contributions[ci], nitems, biter, item)
...
ContributionIndex AnnotationList__computeDominantContribution(AnnotationList const * self, size_t nitems, bitset * lookaheads, bool require_split_stable)
{
	...
	if(ci_shift != ContributionIndex__none) {
		...
		bool find_stable_error_action_domination = false;
		{
			...
			for(ci = 0, actioni = bitset_first(self->inadequacyNode->inadequacy.conflict.actions); ci < self->inadequacyNode->contributionCount; ++ci, actioni = bitset_next(self->inadequacyNode->inadequacy.conflict.actions, actioni+1)) {
				int reduce_precedence = 0;
				if(ci == ci_shift)
					continue;
				{
					rule * r = self->inadequacyNode->manifestingState->reductions->rules[actioni];
					if(r->prec)
				...
				if(reduce_precedence && (reduce_precedence < shift_precedence || (reduce_precedence == shift_precedence && token->assoc == right_assoc)))
					continue;
				...
				/* This uneliminated reduction contributes, so see if it can cause
				   an error action.  */
				if(reduce_precedence == shift_precedence && token->assoc == non_assoc) {
