...
static inline vector_number state_number_to_vector_number(state_number s)
{
	return s;
}
   ...
   `----------------------------------------*/
	...
	static void save_row(state_number s)
	{
		...
		if(count) {
			...
			unsigned int * sp3 = conflict_tos[s] = nondeterministic_parser ? (unsigned int *)xnmalloc(count, sizeof *sp3) : NULL;
...
| putting into YYTABLE later.                                       |
   `------------------------------------------------------------------*/
	...
	static void token_actions(void)
	{
		...
		conflrow = (unsigned int *)xnmalloc(ntokens, sizeof *conflrow);
		conflict_list = (unsigned int *)xnmalloc(1 + 2 * nconflict, sizeof *conflict_list);
...
| putting into YYTABLE later.                                        |
   `-------------------------------------------------------------------*/
	...
	static void goto_actions(void)
	{
		...
		yydefgoto = (state_number *)xnmalloc(nvars, sizeof *yydefgoto);
...
| parsers.                                                     |
   `-------------------------------------------------------------*/
	...
	static void pack_table(void)
	{
		...
		pos = (base_number *)xnmalloc(nentries, sizeof *pos);
		table = (base_number *)xcalloc(table_size, sizeof *table);
		conflict_table = (unsigned int *)xcalloc(table_size, sizeof *conflict_table);
...
| and yycheck.                                                     |
   `-----------------------------------------------------------------*/
	...
	void tables_generate(void)
	{
		...
		verify(sizeof nstates <= sizeof nvectors && sizeof nvars <= sizeof nvectors);
		nvectors = state_number_as_int(nstates) + nvars;
		froms = (base_number **)xcalloc(nvectors, sizeof *froms);
		tos = (base_number **)xcalloc(nvectors, sizeof *tos);
		conflict_tos = (unsigned int **)xcalloc(nvectors, sizeof *conflict_tos);
