...
static BFLocation current_lhs_location;
 ...
 */
static void current_lhs(symbol * sym, BFLocation loc, named_ref * ref);
...
static char const * translate_code(char * code, BFLocation loc, bool plain);
...
static char const * translate_code_braceless(char * code, BFLocation loc);

static void version_check(BFLocation const * loc, char const * version);

static void gram_error(BFLocation const *, char const *);
 ...
 */
static void add_param(param_type type, char * decl, BFLocation loc);
...
static void yy_symbol_value_print(FILE * yyoutput, int yytype, YYSTYPE const * const yyvaluep, YYLTYPE const * const yylocationp)
{
	FILE * yyo = yyoutput;
	YYUSE(yyo);
	YYUSE(yylocationp);
...
static int yy_lac_stack_realloc(YYSIZE_T * yycapacity, YYSIZE_T yyadd,
#if GRAM_DEBUG
...
static int yy_lac(yytype_int16 * yyesa, yytype_int16 ** yyes,
...
//#line 290 "src/parse-gram.y" /* yacc.c:1646  */
			    {
				    muscle_percent_define_insert((yyvsp[-1].uniqstr),
				    (yylsp[-1]),
				    (yyvsp[0].value).kind,
				    (yyvsp[0].value).chars,
...
//#line 327 "src/parse-gram.y" /* yacc.c:1646  */
			    {
				    ...
				    if(strchr(skeleton_user, '/')) {
					    ...
					    while(dir_length && current_file[dir_length - 1] == '/')
						    --dir_length;
					    skeleton_build = (char *)xmalloc(dir_length + 1 + strlen(skeleton_user) + 1);
					    if(dir_length > 0) {
...
static
char const * translate_code_braceless(char * code, BFLocation loc)
{
...
static void add_param(param_type type, char * decl, BFLocation loc)
{
	static char const alphanum[/*26 + 26 + 1 + 10*/] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789";
	...
	char const * name_start = NULL;
	{
		...
		for(p = decl; p[1]; p++)
			if((p == decl || !memchr(alphanum, p[-1], sizeof alphanum)) && memchr(alphanum, p[0], sizeof alphanum - 10))
				name_start = p;
...
static void gram_error(BFLocation const * loc, char const * msg)
{
...
static
void current_lhs(symbol * sym, BFLocation loc, named_ref * ref)
{
