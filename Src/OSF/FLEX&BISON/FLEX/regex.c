...
#define GET_BUFFER_SPACE(n) while((b-bufp->buffer+(n)) > (long)bufp->allocated) EXTEND_BUFFER() // @sobolev (long)
...
static reg_errcode_t regex_compile(const char * pattern, int size, reg_syntax_t syntax, struct re_pattern_buffer * bufp)
{
	...
	const char * p1; /* A random tempory spot in PATTERN.  */
	register unsigned char * b; /* Points to the end of the buffer, where we should append.  */
	compile_stack_type compile_stack; /* Keeps track of unclosed groups.  */
	/* Points to the current (ending) position in the pattern.  */
	const char * p = pattern;
	const char * pend = pattern+size;
	char * translate = bufp->translate; /* How to translate the characters in the pattern.  */
	...
	unsigned char * begalt; /* Address of beginning of regexp, or inside of last group.  */
...
int re_match_2(struct re_pattern_buffer * bufp, const char * string1, int size1, const char * string2, int size2, int pos, struct re_registers * regs, int stop)
{
	...
	/* Pointers into string1 and string2, just past the last characters in each to consider matching.  */
