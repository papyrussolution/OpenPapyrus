...
static const void * gl_linked_node_value(gl_list_t list, gl_list_node_t node)
{
	return node->value;
}

static int gl_linked_node_nx_set_value(gl_list_t list, gl_list_node_t node,
	const void * elt)
{
#if WITH_HASHTABLE
	if(elt != node->value) {
...
static const void * gl_linked_get_at(gl_list_t list, size_t position)
{
...
static gl_list_node_t gl_linked_nx_set_at(gl_list_t list, size_t position, const void * elt)
{
	size_t count = list->count;
	gl_list_node_t node;
	if(!(position < count))
...
static gl_list_node_t gl_linked_search_from_to(gl_list_t list, size_t start_index, size_t end_index,
	const void * elt)
{
	size_t count = list->count;
...
static gl_list_node_t gl_linked_nx_add_last(gl_list_t list, const void * elt)
{
	gl_list_node_t node =
	        (struct gl_list_node_impl *)malloc(sizeof(struct gl_list_node_impl));
	if(node == NULL)
...
static gl_list_node_t gl_linked_nx_add_before(gl_list_t list, gl_list_node_t node, const void * elt)
{
...
static gl_list_node_t gl_linked_nx_add_at(gl_list_t list, size_t position, const void * elt)
{
	size_t count = list->count;
	gl_list_node_t new_node;
	if(!(position <= count))
...
static bool gl_linked_remove_node(gl_list_t list, gl_list_node_t node)
{
	gl_list_node_t prev;
	gl_list_node_t next;
...
static bool gl_linked_remove_at(gl_list_t list, size_t position)
{
	size_t count = list->count;
...
static bool gl_linked_remove(gl_list_t list, const void * elt)
{
...
static gl_list_iterator_t gl_linked_iterator_from_to(gl_list_t list,
...
static bool gl_linked_iterator_next(gl_list_iterator_t * iterator,
...
static gl_list_node_t gl_linked_sortedlist_search_from_to(gl_list_t list,
...
static size_t gl_linked_sortedlist_indexof_from_to(gl_list_t list,
	gl_listelement_compar_fn compar,
	size_t low, size_t high,
	const void * elt)
{
...
static bool gl_linked_sortedlist_remove(gl_list_t list, gl_listelement_compar_fn compar,
	const void * elt)
{
	gl_list_node_t node;
