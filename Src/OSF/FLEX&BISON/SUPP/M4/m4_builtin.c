...
void define_user_macro(const char * name, const char * text, symbol_lookup mode)
{
	...
	if(macro_sequence_inuse && text) {
		...
		while((offset = re_search(&macro_sequence_buf, defn, len, offset, len-offset, &macro_sequence_regs)) >= 0) {
			/* Skip empty matches.  */
...
void m4_placeholder(struct obstack * obs M4_GNUC_UNUSED, int argc,
	token_data ** argv)
{
	M4ERROR((warning_status, 0, "\
