...
static ebitset_elt * ebitset_elt_calloc(void)
{
	ebitset_elt * elt;
...
static void ebitset_elt_free(ebitset_elt * elt)
{
...
static void ebitset_elt_remove(bitset bset, bitset_windex eindex)
{
	ebitset_elts * elts;
	ebitset_elt * elt;
...
static void ebitset_elt_add(bitset bset, ebitset_elt * elt, bitset_windex eindex)
{
	ebitset_elts * elts;
...
static bool ebitset_elt_zero_p(ebitset_elt * elt)
{
	int i;
...
static ebitset_elt * ebitset_elt_find(bitset bset, bitset_bindex bindex,
...
static bitset_windex ebitset_weed(bitset bset)
{
	ebitset_elts * elts;
	bitset_windex j;
	bitset_windex count;
...
static void ebitset_zero(bitset bset)
{
	ebitset_elts * elts;
	bitset_windex j;
...
static bool ebitset_equal_p(bitset dst, bitset src)
{
	ebitset_elts * selts;
	ebitset_elts * delts;
	bitset_windex j;
...
static void ebitset_copy_(bitset dst, bitset src)
{
	ebitset_elts * selts;
	ebitset_elts * delts;
	bitset_windex j;
...
/* Copy bits from bitset SRC to bitset DST.  Return true if
   bitsets different.  */
static bool ebitset_copy_cmp(bitset dst, bitset src)
{
	if(src == dst)
		return false;
...
static bool ebitset_test(bitset src, bitset_bindex bitno)
{
	bitset_windex windex = bitno / BITSET_WORD_BITS;
...
static bitset_bindex ebitset_list_reverse(bitset bset, bitset_bindex * list,
...
/* Find list of up to NUM bits set in BSET starting from and including
   *NEXT and store in array LIST.  Return with actual number of bits
   found and with *NEXT indicating where search stopped.  */
static bitset_bindex ebitset_list(bitset bset, bitset_bindex * list,
    bitset_bindex num, bitset_bindex * next)
{
	bitset_bindex bitno;
	bitset_windex windex;
...
static void ebitset_unused_clear(bitset dst)
{
...
static bool ebitset_empty_p(bitset dst)
{
	ebitset_elts * elts;
	bitset_windex j;
...
/* Is DST == DST | SRC?  */
static bool ebitset_subset_p(bitset dst, bitset src)
{
	bitset_windex j;
	ebitset_elts * selts;
	ebitset_elts * delts;
...
/* Is DST & SRC == 0?  */
static bool ebitset_disjoint_p(bitset dst, bitset src)
{
	bitset_windex j;
	ebitset_elts * selts;
	ebitset_elts * delts;
...
static bool ebitset_op3_cmp(bitset dst, bitset src1, bitset src2, enum bitset_ops op)
{
...
static bool ebitset_and_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed;
...
static bool ebitset_andn_cmp(bitset dst, bitset src1, bitset src2)
{
	bool changed;
...
static bool ebitset_or_cmp(bitset dst, bitset src1, bitset src2)
{
	if(EBITSET_ZERO_P(src2)) {
		return ebitset_copy_cmp(dst, src1);
	}
...
static bool ebitset_xor_cmp(bitset dst, bitset src1, bitset src2)
{
	if(EBITSET_ZERO_P(src2)) {
		return ebitset_copy_cmp(dst, src1);
	}
...
size_t ebitset_bytes(bitset_bindex n_bits ATTRIBUTE_UNUSED)
{
	return sizeof(bitset_union::ebitset_struct);
}
...
bitset ebitset_init(bitset bset, bitset_bindex n_bits)
{
	...
	EBITSET_ZERO_SET(bset);
	size = n_bits ? (n_bits + EBITSET_ELT_BITS - 1) / EBITSET_ELT_BITS : EBITSET_INITIAL_SIZE;
