...
template <uint16_t divisor> struct fast_divide_t<uint16_t, divisor, false> {
	static FORCEINLINE uint16_t divide(uint16_t n) {
...
template <int16_t divisor> struct fast_divide_t<int16_t, divisor, false> {
	static FORCEINLINE int16_t divide(int16_t n) {
...
// Power of 2 test
template <typename T, T N> struct is_power_of_two {
	static constexpr bool val = N!=0 && (N & (N - 1))==0;
};
}

// Public API.
template <typename T, T divisor> FORCEINLINE T fast_divide(T n) 
{
