 ...
 */
#define RETURN(conn, val) { if(conn && (val) < SASL_OK) (conn)->error_code = (val); return (val); }
#define MEMERROR(conn) { if(conn) sasl_seterror((conn), 0, "Out of Memory in " __FILE__ " near line %d", __LINE__); RETURN(conn, SASL_NOMEM) }
#define PARAMERROR(conn) { if(conn) sasl_seterror((conn), SASL_NOLOG, "Parameter error in " __FILE__ " near line %d", __LINE__); RETURN(conn, SASL_BADPARAM) }
#define INTERROR(conn, val) { if(conn) sasl_seterror((conn), 0, "Internal Error %d in " __FILE__ " near line %d", (val), __LINE__); RETURN(conn, (val)) }
...
typedef int sasl_plaintext_verifier (sasl_conn_t * conn, const char * userid, const char * passwd, const char * service, const char * user_realm);
 ...
 * common.c
 */
/*LIBSASL_API*/ extern const sasl_utils_t * sasl_global_utils;
...
extern int (* _sasl_client_cleanup_hook)();
extern int (* _sasl_server_cleanup_hook)();
...
extern int _sasl_is_equal_mech(const char * req_mech, const char * plug_mech, size_t req_mech_len, int * plus);
...
#define sasl_CALLOC(__nelem__, __size__) (_sasl_allocation_utils.calloc((__nelem__), (__size__)))
#define sasl_REALLOC(__ptr__, __size__) (_sasl_allocation_utils.realloc((__ptr__), (__size__)))
