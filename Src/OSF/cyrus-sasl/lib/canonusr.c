...
int sasl_canonuser_add_plugin(const char * plugname, sasl_canonuser_init_t * canonuserfunc)
{
	...
	if(!plugname || strlen(plugname) > (PATH_MAX - 1)) {
		sasl_seterror(NULL, 0, "bad plugname passed to sasl_canonuser_add_plugin\n");
		return SASL_BADPARAM;
	}
	result = canonuserfunc(sasl_global_utils, SASL_CANONUSER_PLUG_VERSION, &out_version, &plug, plugname);
	if(result != SASL_OK) {
	...
	if(!plug->canon_user_server && !plug->canon_user_client) {
		...
		_sasl_log(NULL, SASL_LOG_ERR, "canonuser plugin '%s' without either client or server side", plugname);
		return SASL_BADPROT;
	}
	new_item = (canonuser_plug_list_t *)sasl_ALLOC(sizeof(canonuser_plug_list_t));
	if(!new_item) 
		return SASL_NOMEM;
    ...
    uint flags __attribute__((unused)), char * out_user, uint out_umax, uint * out_ulen)
{
	...
	in_buf = (char *)sasl_ALLOC((ulen + 2) * sizeof(char));
...
int internal_canonuser_init(const sasl_utils_t * utils __attribute__((unused)), int max_version, int * out_version, sasl_canonuser_plug_t ** plug, const char * plugname __attribute__((unused)))
{
	if(!out_version || !plug) 
		return SASL_BADPARAM;
