    ...
    int rdwr, DB ** mbdb)
{
	...
	if(ret != 0) {
		...
		utils->log(conn, SASL_LOG_ERR, "unable to open Berkeley db %s: %s", path, db_strerror(ret));
...
static void berkeleydb_close(const sasl_utils_t * utils, DB * mbdb)
{
	...
	if(ret!=0) {
		/* error closing! */
		utils->log(NULL, SASL_LOG_ERR, "error closing sasldb: %s", db_strerror(ret));
	}
}
/*
 ...
 * SASL_OK on success.
 */
int _sasldb_getdata(const sasl_utils_t * utils, sasl_conn_t * context, const char * auth_identity,
    const char * realm, const char * propName, char * out, const size_t max_out, size_t * out_len)
{
	...
	if(!auth_identity || !realm || !propName || !out || !max_out) {
		utils->seterror(context, 0, "Bad parameter in db_berkeley.c: _sasldb_getdata");
		return SASL_BADPARAM;
	}
	if(!db_ok) {
    ...
    size_t data_len)
{
	...
	if(data_in) { /* putting secret */
		...
		if(result != 0) {
			utils->log(NULL, SASL_LOG_ERR, "error updating sasldb: %s", db_strerror(result));
		...
		if(result != 0) {
			utils->log(NULL, SASL_LOG_ERR, "error deleting entry from sasldb: %s", db_strerror(result));
			utils->seterror(context, SASL_NOLOG, "Couldn't update db");
