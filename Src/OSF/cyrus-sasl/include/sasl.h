...
typedef struct sasl_security_properties {
	...
	sasl_ssf_t min_ssf;
	sasl_ssf_t max_ssf;
	unsigned maxbufsize; /* Maximum security layer receive buffer size. 0=security layer not supported */
	unsigned security_flags; /* bitfield for attacks to protect against */
 ...
 *  SASL_FAIL   -- error
 */
typedef int sasl_log_t (void * context, int level, const char * message);
...
typedef int sasl_verifyfile_t (void * context, const char * file, sasl_verify_type_t type);
 ...
 *  SASL_FAIL   -- error
 */
typedef int sasl_getconfpath_t (void * context, char ** path);
 ...
 * returns SASL_OK
 */
typedef int sasl_chalprompt_t (void * context, int id, const char * challenge, const char * prompt, const char * defresult, const char ** result, unsigned * len);
