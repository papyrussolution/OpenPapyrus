...
LIBSASL_API int sasl_decode64(const char *in, uint inlen,
			      char *out, uint outmax, uint * outlen);
 ...
 */
LIBSASL_API int sasl_encode64(const char *in, uint inlen, char *out, uint outmax, uint * outlen);
 ...
 */
LIBSASL_API int sasl_mkchal(sasl_conn_t *conn, char *buf, unsigned maxlen, unsigned hostflag);

/* verify a string is valid UTF-8
 * if len == 0, strlen(str) will be used.
 * returns SASL_BADPROT on error, SASL_OK on success
 */
LIBSASL_API int sasl_utf8verify(const char *str, uint len);
...
LIBSASL_API void sasl_randseed(sasl_rand_t *rpool, const char *seed, uint len);

/* generate random octets */
LIBSASL_API void sasl_rand(sasl_rand_t *rpool, char *buf, uint len);

/* churn data into random number generator */
LIBSASL_API void sasl_churn(sasl_rand_t *rpool, const char *data, uint len);

/* erase a security sensitive buffer or password.
 *   Implementation may use recovery-resistant erase logic.  
 */
LIBSASL_API void sasl_erasebuffer(char *pass, uint len);
