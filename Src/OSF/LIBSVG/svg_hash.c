 ...
 *         or NULL on failure.
 */
static void * _svg_hash_aryalloc_(size_t esize)
{
 ...
 *         or NULL on failure.
 */
static void * _svg_hash_aryallocsize_(size_t esize, size_t size)
{
 ...
 */
static void _svg_hash_aryfree(void * ary)
{
 ...
 */
static int _svg_hash_aryreserve(void * ary, size_t max)
{
	if(_svg_hash_arymax(ary) == max)
		return 0;
	else {
		void * temp = realloc( *(svg_array_t **)ary-1, sizeof(svg_array_t)+max*_svg_hash_aryesize(ary) );
		if(temp == 0)
			return 1;
 ...
 * @return 0 on success; non-zero on failure
 */
static int _svg_hash_aryresize(void * ary, size_t size)
{
 ...
 * @return 0 on success; non-zero on failure
 */
static int _svg_hash_aryappend(void * ary, const void * element)
{
	if((_svg_hash_arysize(ary)+1 > _svg_hash_arymax(ary) && _svg_hash_aryreserve(ary, _svg_hash_arysize(ary)*2) != 0) ||
	   _svg_hash_aryresize(ary, _svg_hash_arysize(ary)+1) != 0) {
		return 1;
	}
	memcpy(*(char **)ary+(_svg_hash_arysize(ary)-1)*_svg_hash_aryesize(ary), element, _svg_hash_aryesize(ary));
	return 0;
}
...
INLINE size_t _svg_hash_next_prime(size_t size)
{
	size_t i;
...
INLINE size_t _svg_hash_str_hash(const svg_xml_hash_table_t * map, const char * s)
{
	size_t i;
	for(i = 0; *s; s++)
		i = 31*i+*s;
...
INLINE char _svg_hash_streq(const char * s1, const char * s2)
{
	if(s1 == s2) return 1;
	if(!s1 || !s2) return 0;
	while(*s1 != '\0' && *s1 == *s2)
 ...
 * @return 0 on success; non-zero on failure
 */
static int _svg_hash_table_realloc(svg_xml_hash_table_t * hashmap, size_t size)
{
	size_t i;
	size_t hash;
	...
	if(_svg_hash_aryresize(&hashmap->hash_tbl, _svg_hash_next_prime(size)))
		return 1;
	memset(hashmap->hash_tbl, 0xFF, _svg_hash_arysize(&hashmap->hash_tbl)*sizeof(size_t));
...
INLINE int _str_hash_insert_new(svg_xml_hash_table_t * hashmap, const char * key, const void * item, size_t hash)
{
	size_t newitemnum;
