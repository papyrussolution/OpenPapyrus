...
asn1_default:
			...
			case SCE_ASN1_IDENTIFIER: // An identifier (keyword, attribute, descriptor or type)
			    if(!isAsn1Char(sc.ch)) {
				    // The end of identifier is here: we can look for it in lists by now and change its
				    // state
				    char s[100];
			...
			case SCE_ASN1_SCALAR:   // A plain number
			    if(!isAsn1Number(sc.ch))
				    ...
				    sc.SetState(SCE_ASN1_DEFAULT);
			    break;
			case SCE_ASN1_OPERATOR: // The affectation operator ::= and wath follows (eg: ::= { org 6 } OID
			                        // or ::= 12 trap)
			    if(sc.ch == '{') {
