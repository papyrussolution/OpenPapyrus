...
struct OptionSetSQL : public OptionSet<OptionsSQL> {
	OptionSetSQL()
	{
		...
		DefineProperty("fold.sql.at.else", &OptionsSQL::foldAtElse, "This option enables SQL folding on a \"ELSE\" and \"ELSIF\" line of an IF statement.");
		...
		DefineProperty("lexer.sql.numbersign.comment", &OptionsSQL::sqlNumbersignComment, "If \"lexer.sql.numbersign.comment\" property is set to 0 a line beginning with '#' will not be a comment.");
		DefineProperty("sql.backslash.escapes", &OptionsSQL::sqlBackslashEscapes, "Enables backslash as an escape character in SQL.");
		DefineProperty("lexer.sql.allow.dotted.word", &OptionsSQL::sqlAllowDottedWord, "Set to 1 to colourise recognized words with dots (recommended for Oracle PL/SQL objects).");
...
Sci_Position SCI_METHOD LexerSQL::WordListSet(int n, const char * wl)
{
	...
	switch(n) {
		...
		case 2: wordListN = &kw_pldoc; break;
		case 3: wordListN = &kw_sqlplus; break;
		case 4: wordListN = &kw_user1; break;
		case 5: wordListN = &kw_user2; break;
		case 6: wordListN = &kw_user3; break;
		...
		if(style == SCE_SQL_WORD && stylePrev != SCE_SQL_WORD) {
			...
			if(!options.foldOnlyBegin && sstreq(s, "select")) {
				...
				if(endFound) {
					endFound = false;
					if(options.foldOnlyBegin && !isUnfoldingIgnored) {
						// this end isn't for begin block, but for loop block ("end loop;") or case block ("end
						// case;")
						// so ignore previous "end" by increment levelNext.
						levelNext++;
					}
				...
				// expression1 THEN sequence_of_statements1;\n")
