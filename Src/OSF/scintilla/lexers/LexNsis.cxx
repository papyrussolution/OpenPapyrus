...
static int calculateFoldNsis(Sci_PositionU start, Sci_PositionU end, int foldlevel, Accessor & styler, bool bElse, bool foldUtilityCmd)
{
	...
	if(s[0] == '!') {
		if(NsisCmp(s, "!ifndef",
		    ...
		    NsisCmp(s, "!ifdef",
			    bIgnoreCase) == 0 ||
		    NsisCmp(s, "!ifmacrodef",
			    bIgnoreCase) == 0 ||
		    NsisCmp(s, "!ifmacrondef",
			    bIgnoreCase) == 0 || NsisCmp(s, "!if", bIgnoreCase) == 0 || NsisCmp(s, "!macro", bIgnoreCase) == 0)
			newFoldlevel++;
		...
		if(NsisCmp(s, "Section",
		    ...
		    NsisCmp(s, "SectionGroup",
			    bIgnoreCase) == 0 ||
		    NsisCmp(s, "Function",
			    bIgnoreCase) == 0 || NsisCmp(s, "SubSection", bIgnoreCase) == 0 || NsisCmp(s, "PageEx", bIgnoreCase) == 0)
			newFoldlevel++;
		else if(NsisCmp(s, "SectionGroupEnd",
			    bIgnoreCase) == 0 ||
		    NsisCmp(s, "SubSectionEnd",
			    bIgnoreCase) == 0 ||
		    NsisCmp(s, "FunctionEnd",
			    bIgnoreCase) == 0 || NsisCmp(s, "SectionEnd", bIgnoreCase) == 0 || NsisCmp(s, "PageExEnd", bIgnoreCase) == 0)
			newFoldlevel--;
	}
...
static int classifyWordNsis(Sci_PositionU start, Sci_PositionU end, WordList * keywordLists[], Accessor & styler)
{
	...
	if(NsisCmp(s, "!ifmacrodef", bIgnoreCase) == 0 || NsisCmp(s, "!ifmacrondef", bIgnoreCase)  == 0)     // Covers
		                                                                                             // !ifmacrodef
		                                                                                             // and
		                                                                                             // !ifnmacrodef
		return SCE_NSIS_IFDEFINEDEF;

	if(NsisCmp(s, "SectionGroup", bIgnoreCase) == 0 || NsisCmp(s, "SectionGroupEnd", bIgnoreCase) == 0) // Covers
		                                                                                            // SectionGroup
		                                                                                            // and
		                                                                                            // SectionGroupEnd
		return SCE_NSIS_SECTIONGROUP;

	if(NsisCmp(s, "Section", bIgnoreCase) == 0 || NsisCmp(s, "SectionEnd", bIgnoreCase) == 0)    // Covers Section
		                                                                                     // and SectionEnd
		return SCE_NSIS_SECTIONDEF;

	if(NsisCmp(s, "SubSection", bIgnoreCase) == 0 || NsisCmp(s, "SubSectionEnd", bIgnoreCase) == 0)   // Covers
		                                                                                          // SubSection
		                                                                                          // and
		                                                                                          // SubSectionEnd
		return SCE_NSIS_SUBSECTIONDEF;

	if(NsisCmp(s, "PageEx", bIgnoreCase) == 0 || NsisCmp(s, "PageExEnd", bIgnoreCase) == 0) // Covers PageEx and
		                                                                                // PageExEnd
		return SCE_NSIS_PAGEEX;

	if(NsisCmp(s, "Function", bIgnoreCase) == 0 || NsisCmp(s, "FunctionEnd", bIgnoreCase) == 0)   // Covers Function
		                                                                                      // and FunctionEnd
...
static void ColouriseNsisDoc(Sci_PositionU startPos, Sci_Position length, int, WordList * keywordLists[], Accessor & styler)
{
	...
	if(startPos > 0)
		state = styler.StyleAt(startPos-1);  // Use the style from the previous line, usually default, but could
	                                             // be commentbox
