...
static int disambiguateBareword(LexAccessor &styler, Sci_PositionU bk, Sci_PositionU fw,
	...
	bk = backPos - 1;
	int ch = static_cast<uchar>(styler.SafeGetCharAt(bk));
	if (ch == '{' && !moreback) {
...
static int styleBeforeBracePair(LexAccessor &styler, Sci_PositionU bk) {
	...
	while (--bk > 0) {
		if (styler.StyleAt(bk) == SCE_PL_OPERATOR) {
			int bkch = static_cast<uchar>(styler.SafeGetCharAt(bk));
			if (bkch == ';') {	// early out
				break;
			} else if (bkch == '}') {
				braceCount++;
...
static int podLineScan(LexAccessor &styler, Sci_PositionU &pos, Sci_PositionU endPos) {
	...
	int state = -1;
	while (pos < endPos) {
		int ch = static_cast<uchar>(styler.SafeGetCharAt(pos));
		if (ch == '\n' || ch == '\r') {
...
static int PodHeadingLevel(Sci_Position pos, LexAccessor &styler) {
	int lvl = static_cast<uchar>(styler.SafeGetCharAt(pos + 5));
	if (lvl >= '1' && lvl <= '4') {
		return lvl - '0';
	}
	return 0;
			...
			if (HereDoc.State == 0) { // '<<' encountered
				...
				if (IsASpaceOrTab(delim_ch)) {
					...
					while ((i < endPos) && IsASpaceOrTab(delim_ch)) {
						i++;
						delim_ch = static_cast<uchar>(styler.SafeGetCharAt(i));
					}
				...
				if (HereDoc.Quoted) { // a quoted here-doc delimiter
					if (sc.ch == HereDoc.Quote) { // closing quote => end of delimiter
						...
						if (sc.ch != '\r') {	// skip CR if CRLF
							...
							while (i < sc.width) {
								HereDoc.Append(static_cast<uchar>(styler.SafeGetCharAt(sc.currentPos + i)));
								i++;
							}
						}
					}
		...
		if (sc.state == SCE_PL_DEFAULT) {
			if (IsADigit(sc.ch) ||
				...
				if (sc.state == SCE_PL_WORD) {
					while (setWord.Contains(static_cast<uchar>(styler.SafeGetCharAt(fw))))
						fw++;
