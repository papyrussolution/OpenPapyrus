    ...
    char * prevWord)
{
	...
	if(sstreq(prevWord, "undef") || sstreq(prevWord, "def") || sstreq(prevWord, "alias")) {
...
static bool keywordIsAmbiguous(const char * prevWord)
{
	...
	if(sstreq(prevWord, "if") || sstreq(prevWord, "do") || sstreq(prevWord, "while") || 
		sstreq(prevWord, "unless") || sstreq(prevWord, "until") || sstreq(prevWord, "for")) {
		return true;
	}
	else {
		return false;
	...
	while(--pos >= lineStartPosn) {
		...
		if(style == SCE_RB_DEFAULT) {
			...
			*dst = 0;
			...
			if(sstreq(prevWord, WHILE_BACKWARDS) || sstreq(prevWord, UNTIL_BACKWARDS) || sstreq(prevWord, FOR_BACKWARDS)) {
				return true;
			}
    ...
    WordList *[], Accessor & styler)
{
	...
	for(Sci_PositionU i = startPos; i < endPos; i++) {
		...
		if(style == SCE_RB_COMMENTLINE) {
			...
			else if(sstreq(prevWord, "if") || sstreq(prevWord, "def") || sstreq(prevWord, "class") || 
				sstreq(prevWord, "module") || sstreq(prevWord, "begin") || sstreq(prevWord, "case") || 
				sstreq(prevWord, "do") || sstreq(prevWord, "while") || sstreq(prevWord, "unless") || sstreq(prevWord, "until") || sstreq(prevWord, "for")) {
				levelCurrent++;
			}
		}
