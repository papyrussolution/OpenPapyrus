...
static void ColouriseTCMDLine(char * lineBuffer,
...
//	WordList &keywords2 = *keywordlists[1];     // Aliases (optional)
	...
	while(offset < lengthLine) {
		...
		if(IsBSeparator(wordBuffer[0])) {
		...
		else if((wordBuffer[0] != '%') && (wordBuffer[0] != '!') && (!IsBOperator(wordBuffer[0])) && (!inString) &&
		    (continueProcessing)) {
			...
			if(((isdec(wordBuffer[1])) || (wordBuffer[1] == '*')) && (wordBuffer[wbo] != '%')) {
				while(( wordBuffer[n] ) && ( sstrchr("%0123456789*#$", wordBuffer[n]) != NULL ))
					n++;
ColorizeArg:
				// Colorize Argument
