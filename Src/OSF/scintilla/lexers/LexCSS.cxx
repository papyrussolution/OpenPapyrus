...
static bool FASTCALL IsAWordChar(const uint ch) {
	 ...
	 * that our byte is between U+0080 - U+00A0 (to return false), so we have to allow all characters U+0080 and
	 *higher
	 */
...
static void ColouriseCssDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList * keywordlists[], Accessor &styler) {
	...
	for(; sc.More(); sc.Forward()) {
		if(sc.state == SCE_CSS_COMMENT &&
		    ((comment_mode == eCommentBlock && sc.Match('*', '/')) || (comment_mode == eCommentLine && sc.atLineEnd))) {
			if(lastStateC == -1) {
		...
		if(sc.ch != '.' && sc.ch != ':' && sc.ch != '#' && (
			    ...
			    (sc.ch != '(' && sc.ch != ')' && ( /* This line of the condition makes it possible to extend
		...
		if(sc.Match('/', '*')) {
			...
			// note that we've had to treat ([...]// as the start of a URL not a comment, e.g.
