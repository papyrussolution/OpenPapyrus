...
static int8 ClassificationTable[256] = { // @sobolev int-->int8
...
static bool FASTCALL isDecDigit(int ch) { return (ch > 0) && (ch < 0x80) && ((ClassificationTable[ch] & 1) != 0); }
static bool FASTCALL isLetter(int ch) { return (ch > 0) && (ch < 0x80) && ((ClassificationTable[ch] & 2) != 0); }
static bool FASTCALL isSpecial(int ch) { return (ch > 0) && (ch < 0x80) && ((ClassificationTable[ch] & 4) != 0); }
static bool FASTCALL isUpper(int ch) { return (ch > 0) && (ch < 0x80) && ((ClassificationTable[ch] & 8) != 0); }
static bool FASTCALL isBinSel(int ch) { return (ch > 0) && (ch < 0x80) && ((ClassificationTable[ch] & 16) != 0); }

// autogenerated }}}}

static bool FASTCALL isAlphaNumeric(int ch) { return isDecDigit(ch) || isLetter(ch); }

static bool FASTCALL isDigitOfRadix(int ch, int radix)
{
	if(isDecDigit(ch))
		return (ch - '0') < radix;
	else if(!isUpper(ch))
...
static void handleLetter(StyleContext& sc, WordList* specialSelectorList)
{
	...
	if(specialSelectorList->InList(ident))
		...
		if(sstreq(ident, "self"))
			state = SCE_ST_SELF;
		else if(sstreq(ident, "super"))
			state = SCE_ST_SUPER;
		else if(sstreq(ident, "nil"))
			state = SCE_ST_NIL;
		else if(sstreq(ident, "true") || sstreq(ident, "false"))
			state = SCE_ST_BOOL;
		else
