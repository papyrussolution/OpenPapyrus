...
/*inline*/ bool FASTCALL IsAHexCharacter(const int iChar, bool bCaseSensitive)
{
	// Case insensitive.
	if(!bCaseSensitive) {
		if(sstrchr("ABCDEFabcdef", iChar)) {
			return true;
		}
	}
	// Case sensitive
	else {
		if(sstrchr("ABCDEF", iChar)) {
			return true;
		}
	}
	return false;
			...
			if(scDoc.state == SCE_CLW_DEFAULT) {
				...
				// else the character is an @ (ampersand)
					...
					if(!bCaseSensitive) {
						...
						if(sstrchr("DEKNPSTdeknpst", scDoc.chNext)) {
					...
					// Case sensitive
						...
						// If character is a valid picture token character
						if(sstrchr("DEKNPST", scDoc.chNext)) {
