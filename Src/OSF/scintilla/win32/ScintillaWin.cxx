...
class IMContext {
public:
	IMContext(HWND hwnd_) : hwnd(hwnd_), hIMC(::ImmGetContext(hwnd_))
	{
	}
	~IMContext()
	{
...
private:
	...
	virtual sptr_t DefWndProc(uint iMessage, uptr_t wParam, sptr_t lParam);
...
public:
	...
	STDMETHODIMP DragEnter(LPDATAOBJECT pIDataSource, DWORD grfKeyState, POINTL pt, PDWORD pdwEffect);
	STDMETHODIMP DragOver(DWORD grfKeyState, POINTL pt, PDWORD pdwEffect);
	STDMETHODIMP DragLeave();
	STDMETHODIMP Drop(LPDATAOBJECT pIDataSource, DWORD grfKeyState, POINTL pt, PDWORD pdwEffect);
...
ScintillaWin::ScintillaWin(HWND hwnd) : lastHighSurrogateChar(0), TrackMouseEventFn(0), SetCoalescableTimerFn(0),
	linesPerScroll(0), wheelDelta(0) /*Wheel delta from roll*/, hRgnUpdate(0), hrOle(E_FAIL), sysCaretBitmap(0), sysCaretWidth(0), sysCaretHeight(0)
{
	...
	cfLineSelect = static_cast<CLIPFORMAT>(::RegisterClipboardFormat(TEXT("MSDEVLineSelect")));
	cfVSLineTag = static_cast<CLIPFORMAT>(::RegisterClipboardFormat(TEXT("VisualStudioEditorOperationsLineCutCopyClipboardTag")));
	wMain = hwnd;
	dob.sci = this;
	ds.sci = this;
	dt.sci = this;
...
void ScintillaWin::Initialise()
{
	...
	if(TrackMouseEventFn == NULL) {
		...
		if(commctrl32 != NULL) {
			TrackMouseEventFn = (TrackMouseEventSig)::GetProcAddress(commctrl32, "_TrackMouseEvent");
		}
	}
...
void ScintillaWin::EnsureRenderTarget(HDC hdc)
{
	...
	if(pD2DFactory && !pRenderTarget) {
		...
		if(technology == SC_TECHNOLOGY_DIRECTWRITEDC) {
			...
			dhrtp.presentOptions = (technology == SC_TECHNOLOGY_DIRECTWRITERETAIN) ? D2D1_PRESENT_OPTIONS_RETAIN_CONTENTS : D2D1_PRESENT_OPTIONS_NONE;
...
bool ScintillaWin::DragThreshold(Point ptStart, Point ptNow)
{
	...
	return (xMove > ::GetSystemMetrics(SM_CXDRAG)) || (yMove > ::GetSystemMetrics(SM_CYDRAG));
}
...
static bool BoundsContains(PRectangle rcBounds, HRGN hRgnBounds, PRectangle rcCheck)
{
	...
	if(!rcCheck.Empty()) {
		if(!rcBounds.Contains(rcCheck)) {
			...
			HRGN hRgnCheck = ::CreateRectRgn(static_cast<int>(rcCheck.left), static_cast<int>(rcCheck.top), static_cast<int>(rcCheck.right), static_cast<int>(rcCheck.bottom));
			if(hRgnCheck) {
...
{
	const int charsConverted = ::LCMapStringW(LOCALE_SYSTEM_DEFAULT, mapFlags, ws.c_str(), static_cast<int>(ws.length()), NULL, 0);
	std::wstring wsConverted(charsConverted, 0);
	if(charsConverted) {
		::LCMapStringW(LOCALE_SYSTEM_DEFAULT, mapFlags, ws.c_str(), static_cast<int>(ws.length()), &wsConverted[0], charsConverted);
	}
	return wsConverted;
}
...
int ScintillaWin::TargetAsUTF8(char * text)
{
	...
	if(IsUnicodeMode()) {
		...
		const int utf8Len = ::WideCharToMultiByte(CP_UTF8, 0, characters.c_str(),
		    static_cast<int>(characters.length()), NULL, 0, 0, 0);
		if(text) {
...
int ScintillaWin::EncodedFromUTF8(char * utf8, char * encoded) const
{
	...
	if(IsUnicodeMode()) {
		...
		int encodedLen = ::WideCharToMultiByte(CodePageOfDocument(), 0, &characters[0], charsLen, NULL, 0, 0, 0);
		if(encoded) {
...
void ScintillaWin::AddCharUTF16(wchar_t const * wcs, uint wclen)
{
	if(IsUnicodeMode()) {
		char utfval[maxLenInputIME * 3];
		uint len = UTF8Length(wcs, wclen);
...
bool ScintillaWin::KoreanIME()
{
	...
	return oneof2(codePage, 949, 1361);
}
...
void ScintillaWin::EscapeHanja()
{
	...
	uint const safeLength = UTF8MaxBytes+1;
...
std::vector<int> MapImeIndicators(std::vector<BYTE> inputStyle)
{
	...
	for(size_t i = 0; i < inputStyle.size(); i++) {
		switch(static_cast<int>(inputStyle.at(i))) {
			...
			case ATTR_TARGET_CONVERTED: imeIndicator[i] = SC_INDICATOR_TARGET; break;
			case ATTR_CONVERTED: imeIndicator[i] = SC_INDICATOR_CONVERTED; break;
			default: imeIndicator[i] = SC_INDICATOR_UNKNOWN; break;
		}
	}
	return imeIndicator;
}
...
void ScintillaWin::AddWString(std::wstring wcs)
{
	if(!wcs.empty()) {
		...
		for(size_t i = 0; i < wcs.size(); ) {
			...
			AddCharUTF(docChar.c_str(), static_cast<uint>(docChar.size()));
			i += ucWidth;
		}
	}
}
		...
		*ptr = L'\0';
		...
		if(IsUnicodeMode()) {
			...
			if(lParam == 0)
				...
				size_t uLen = UTF16FromUTF8(&docBytes[0], docBytes.size(), ptr, static_cast<int>(wParam) - 1);
				ptr[uLen] = L'\0';
				return uLen;
			}
		}
			...
			int lengthUTF16 = ::MultiByteToWideChar(cpSrc, 0, &docBytes[0], static_cast<int>(docBytes.size()), NULL, 0);
			if(lengthUTF16 >= static_cast<int>(wParam))
				lengthUTF16 = static_cast<int>(wParam)-1;
			::MultiByteToWideChar(cpSrc, 0, &docBytes[0], static_cast<int>(docBytes.size()), ptr, lengthUTF16);
			ptr[lengthUTF16] = L'\0';
			return lengthUTF16;
		}
	}
...
sptr_t ScintillaWin::WndProc(uint iMessage, uptr_t wParam, sptr_t lParam)
{
	try {
		...
		switch(iMessage) {
			...
			case WM_MOUSEWHEEL:
			    ...
			    wheelDelta -= static_cast<short>(_HiWord(wParam));
...
#pragma warning(suppress: 28159)
			    ...
			    break;
			case WM_GETMINMAXINFO: return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
			case WM_LBUTTONDOWN: 
				{
			...
			case WM_MOUSEMOVE: 
				{
					...
					if(ptMouseLast.x != pt.x || ptMouseLast.y != pt.y) {
						...
						ButtonMoveWithModifiers(pt, ((wParam & MK_SHIFT) != 0 ? SCI_SHIFT : 0) | 
							((wParam & MK_CONTROL) != 0 ? SCI_CTRL : 0) | (Platform::IsKeyDown(VK_MENU) ? SCI_ALT : 0));
					}
				}
				break;
			case WM_MOUSELEAVE:
			...
			case WM_KEYDOWN: 
				{
					//Platform::DebugPrintf("S keydown %d %x %x %x %x\n",iMessage, wParam, lParam, ::IsKeyDown(VK_SHIFT),
					// ::IsKeyDown(VK_CONTROL));
					lastKeyDownConsumed = false;
					int ret = KeyDown(KeyTranslate(static_cast<int>(wParam)), Platform::IsKeyDown(VK_SHIFT),
						Platform::IsKeyDown(VK_CONTROL), Platform::IsKeyDown(VK_MENU), &lastKeyDownConsumed);
			...
			case WM_SETTINGCHANGE:
			    ...
			    GetIntelliMouseParameters();
			    break;
			case WM_GETDLGCODE: return (DLGC_HASSETSEL | DLGC_WANTALLKEYS);
			case WM_KILLFOCUS: 
				{
			...
			case WM_INPUTLANGCHANGE:
			    ...
			    return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
			case WM_INPUTLANGCHANGEREQUEST: return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
			case WM_ERASEBKGND: return 1; // Avoid any background erasure as whole window painted.
			...
			case WM_WINDOWPOSCHANGED: return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
			case WM_GETTEXTLENGTH: return GetTextLength();
			case WM_GETTEXT: return GetText(wParam, lParam);
...
{
	return oneof7(codePage, 0, SC_CP_UTF8, 932, 936, 949, 950, 1361);
}

sptr_t ScintillaWin::DefWndProc(uint iMessage, uptr_t wParam, sptr_t lParam)
{
	return ::DefWindowProc(MainHWND(), iMessage, wParam, lParam);
}
/**
...
{
	return (paintState == painting) ? BoundsContains(rcPaint, hRgnUpdate, rc) : true;
}
...
bool ScintillaWin::ModifyScrollBars(int nMax, int nPage)
{
	...
	if((sci.nMin != 0) || (sci.nMax != vertEndPreferred) || (sci.nPage != static_cast<uint>(nPage)) || (sci.nPos != 0)) {
		sci.fMask = SIF_PAGE | SIF_RANGE;
		sci.nMin = 0;
...
{
	::SendMessage(::GetParent(MainHWND()), WM_COMMAND, MAKELONG(GetCtrlID(), SCEN_CHANGE), reinterpret_cast<LPARAM>(MainHWND()));
}

void ScintillaWin::NotifyFocus(bool focus)
{
	::SendMessage(::GetParent(MainHWND()), WM_COMMAND, MAKELONG(GetCtrlID(), focus ? SCEN_SETFOCUS : SCEN_KILLFOCUS), reinterpret_cast<LPARAM>(MainHWND()));
...
void ScintillaWin::NotifyDoubleClick(Point pt, int modifiers)
{
	...
	::SendMessage(MainHWND(), WM_LBUTTONDBLCLK, (modifiers & SCI_SHIFT) ? MK_SHIFT : 0, MAKELPARAM(pt.x, pt.y));
}
...
std::string ScintillaWin::CaseMapString(const std::string &s, int caseMapping)
{
	...
	const DWORD mapFlags = LCMAP_LINGUISTIC_CASING | ((caseMapping == cmUpper) ? LCMAP_UPPERCASE : LCMAP_LOWERCASE);
	// Change case
...
bool ScintillaWin::CanPaste()
{
	if(!Editor::CanPaste())
		...
		return IsUnicodeMode() ? (::IsClipboardFormatAvailable(CF_UNICODETEXT) != 0) : false;
}
	...
	if(::OpenClipboardRetry(MainHWND())) {
		...
		if(memUSelection) {
			...
			if(memSelection) {
				...
				if(ptr) {
					uint bytes = static_cast<uint>(memSelection.Size());
					uint len = bytes;
...
void ScintillaWin::CreateCallTipWindow(PRectangle)
{
	if(!ct.wCallTip.Created()) {
		ct.wCallTip = ::CreateWindow(callClassName, TEXT("ACallTip"), WS_POPUP, 100, 100, 150, 20, MainHWND(), 0, GetWindowInstance(MainHWND()), this);
...
STDMETHODIMP FormatEnumerator_Next(FormatEnumerator * fe, ULONG celt, FORMATETC * rgelt, ULONG * pceltFetched)
{
	...
	ASSIGN_PTR(pceltFetched, putPos);
...
FormatEnumerator::FormatEnumerator(int pos_, CLIPFORMAT formats_[], size_t formatsLen_) :
	vtbl(vtFormatEnumerator), ref(0) /* First QI adds first reference...*/, pos(pos_)
{
...
LRESULT ScintillaWin::ImeOnReconvert(LPARAM lParam)
{
	...
	for(size_t r = 0; r<sel.Count(); r++) {
		...
		if(EditModelFlags & fInOverstrike) {  // the docCompLen of bytes will be overstriked.
			...
			pdoc->DeleteChars(docCompStart, (overflow > 0) ? (docCompLen - overflow) : docCompLen);
		}
	}
...
void ScintillaWin::ScrollMessage(WPARAM wParam)
{
	...
	switch(_LoWord(wParam)) {
		...
		case SB_PAGEUP: topLineNew -= LinesToScroll(); break;
...
{
	bool formatOK = (pFEIn->cfFormat == CF_TEXT) || ((pFEIn->cfFormat == CF_UNICODETEXT) && IsUnicodeMode());
	if(!formatOK || pFEIn->ptd != 0 || (pFEIn->dwAspect & DVASPECT_CONTENT) == 0 ||
	    pFEIn->lindex != -1 || (pFEIn->tymed & TYMED_HGLOBAL) == 0) {
	...
	if(pFEIn->cfFormat == CF_UNICODETEXT) {
		...
		if(text) {
			UTF16FromUTF8(drag.Data(), drag.LengthWithTerminator(), static_cast<wchar_t *>(text.ptr), uchars);
		}
	}
	else {
...
{
	if(((0 != vs.caretWidth) && (sysCaretWidth != vs.caretWidth)) || ((0 != vs.lineHeight) && (sysCaretHeight != vs.lineHeight))) {
		return true;
	}
	return false;
}
...
BOOL ScintillaWin::CreateSystemCaret()
{
	...
	if(technology == SC_TECHNOLOGY_DEFAULT)
		::ShowCaret(MainHWND()); // System caret interferes with Direct2D drawing so only show it for GDI.
	return retval;
}
...
LRESULT PASCAL ScintillaWin::CTWndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
			...
			if(iMessage == WM_NCDESTROY) {
				...
				if(surfaceWindow) {
					...
					if(sciThis->technology == SC_TECHNOLOGY_DEFAULT) {
						...
						dhrtp.presentOptions = (sciThis->technology == SC_TECHNOLOGY_DIRECTWRITERETAIN) ? D2D1_PRESENT_OPTIONS_RETAIN_CONTENTS : D2D1_PRESENT_OPTIONS_NONE;
						D2D1_RENDER_TARGET_PROPERTIES drtp;
						drtp.type = D2D1_RENDER_TARGET_TYPE_DEFAULT;
						drtp.pixelFormat.format = DXGI_FORMAT_UNKNOWN;
						drtp.pixelFormat.alphaMode = D2D1_ALPHA_MODE_UNKNOWN;
...
#if defined(USE_D2D)
					CALLPTRMEMB(pCTRenderTarget, EndDraw());
#endif
					surfaceWindow->Release();
					delete surfaceWindow;
#if defined(USE_D2D)
					CALLPTRMEMB(pCTRenderTarget, Release());
#endif
				}
				::EndPaint(hWnd, &ps);
				return 0;
...
LRESULT PASCAL ScintillaWin::SWndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
