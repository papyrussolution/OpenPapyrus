...
namespace Scintilla {
	...
	public:
		...
		XYPOSITION PRectangle::Width() const { return (right - left); }
		XYPOSITION PRectangle::Height() const { return (bottom - top); }
		bool Empty() const;
	...
	public:
		...
		ColourDesired(uint red, uint green, uint blue) { Set(red, green, blue); }
		bool FASTCALL operator == (const ColourDesired &other) const { return co == other.co; }
		void FASTCALL Set(long lcol) { co = lcol; }
		...
		uint GetRed() const { return co & 0xff; }
		uint GetGreen() const { return (co >> 8) & 0xff; }
		uint GetBlue() const { return (co >> 16) & 0xff; }
	private:
		long co;
	};
	// 
	...
	public:
		...
		static short LowShortFromLong(long x) { return static_cast<short>(x & 0xffff); }
	...
	enum CharacterCategory {
		ccLu, ccLl, ccLt, ccLm, ccLo, ccMn, ccMc, ccMe, ccNd, ccNl, ccNo, ccPc, ccPd, ccPs, ccPe, ccPi, ccPf, ccPo,
		ccSm, ccSc, ccSk, ccSo, ccZs, ccZl, ccZp, ccCc, ccCf, ccCs, ccCo, ccCn
	};
	...
	private:
		...
		int    documentVersion;
		char   buf[bufferSize+4]; // @sobolev +1-->+4 (alignment)
