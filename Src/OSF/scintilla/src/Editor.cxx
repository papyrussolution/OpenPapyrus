...
*/
static bool FASTCALL CanDeferToLastStep(const DocModification &mh)
{
...
static bool FASTCALL CanEliminate(const DocModification &mh)
{
...
Editor::Idler::Idler() : state(false), idlerID(0)
{
}
...
Editor::SelectionText::SelectionText() : rectangular(false), lineCopy(false), codePage(0), characterSet(0)
{
}

Editor::SelectionText::~SelectionText()
{
}

void Editor::SelectionText::Clear()
{
	s.clear();
	rectangular = false;
	lineCopy = false;
...
void Editor::SelectionText::Copy(const std::string &s_, int codePage_, int characterSet_, bool rectangular_, bool lineCopy_)
{
	s = s_;
	codePage = codePage_;
...
void FASTCALL Editor::WrapPending::Wrapped(int line)
{
	if(start == line)
		start++;
}
...
//
//
Editor::WorkNeeded::WorkNeeded() : items(workNone), upTo(0)
{
}

void Editor::WorkNeeded::Reset()
{
	items = workNone;
	upTo = 0;
}

void Editor::WorkNeeded::Need(workItems items_, Position pos)
{
...
{
	return wMargin.GetID() ? 0 : topLine;
}
...
SelectionPosition Editor::SPositionFromLocation(const SciPoint & rPt, bool canReturnInvalid, bool charPosition, bool virtualSpace)
{
	...
	if(canReturnInvalid) {
		...
		else if(rPt.x < vs.textStart)
			return SelectionPosition(INVALID_POSITION);
		else if(rPt.y < 0)
			return SelectionPosition(INVALID_POSITION);
	}
	PointDocument ptdoc = DocumentPointFromView(rPt);
...
void Editor::RedrawRect(PRectangle rc)
{
	...
	PRectangle rcClient = GetClientRectangle();
	SETMAX(rc.top, rcClient.top);
	SETMIN(rc.bottom, rcClient.bottom);
	SETMAX(rc.left, rcClient.left);
	SETMIN(rc.right, rcClient.right);
...
void Editor::RedrawSelMargin(int line, bool allAfter)
{
	...
	if(line != -1) {
		...
		if(vs.largestMarkerHeight > vs.lineHeight) {
			...
			rcLine.top -= delta;
			rcLine.bottom += delta;
			SETMAX(rcLine.top, rcMarkers.top);
			SETMIN(rcLine.bottom, rcMarkers.bottom);
		}
...
PRectangle Editor::RectangleFromRange(Range r, int overlap)
{
	...
	SETMAX(rc.top, rcClientDrawing.top);
...
int Editor::CurrentPosition() const { return Sel.MainCaret(); }
bool Editor::SelectionEmpty() const { return Sel.Empty(); }
...
{
	if(!invalidateWholeSelection && (Sel.Count() > 1 || Sel.RangeMain().anchor != newMain.anchor || Sel.IsRectangular()))
		invalidateWholeSelection = true;
	int first_affected = smin(Sel.RangeMain().Start().Position(), newMain.Start().Position());
	// +1 for lastAffected ensures caret repainted
	int last_affected = smax(newMain.caret.Position()+1, newMain.anchor.Position());
	last_affected = smax(last_affected, Sel.RangeMain().End().Position());
	if(invalidateWholeSelection) {
		for(size_t r = 0; r < Sel.Count(); r++) {
			const SelectionRange & r_range = Sel.Range(r);
			const int caret_pos = r_range.caret.Position();
			const int anchor_pos = r_range.anchor.Position();
			first_affected = smin(first_affected, caret_pos);
			first_affected = smin(first_affected, anchor_pos);
			last_affected = smax(last_affected, caret_pos+1);
			last_affected = smax(last_affected, anchor_pos);
		}
	}
	ContainerNeedsUpdate(SC_UPDATE_SELECTION);
	InvalidateRange(first_affected, last_affected);
}
...
void Editor::SetSelection(SelectionPosition currentPos_, SelectionPosition anchor_)
{
	...
	// For Line selection - ensure the anchor and caret are always at the beginning and end of the region lines. 
...
void Editor::SetSelection(SelectionPosition currentPos_)
{
	...
	if(Sel.IsRectangular()) {
		Sel.Rectangular() = SelectionRange(SelectionPosition(currentPos_), Sel.Rectangular().anchor);
		SetRectangularRange();
	}
	else {
		Sel.RangeMain() = SelectionRange(SelectionPosition(currentPos_), Sel.RangeMain().anchor);
	}
	ClaimSelection();
	SetHoverIndicatorPosition(Sel.MainCaret());
	if(marginView.highlightDelimiter.NeedsDrawing(currentLine)) {
	...
	if(vs.ProtectionActive()) {
		if(moveDir > 0) {
			if((pos.Position() > 0) && vs.styles[pdoc->StyleIndexAt(pos.Position() - 1)].IsProtected()) {
				while((pos.Position() < pdoc->Length()) && (vs.styles[pdoc->StyleIndexAt(pos.Position())].IsProtected()))
					pos.Add(1);
			}
		}
		else if(moveDir < 0) {
			if(vs.styles[pdoc->StyleIndexAt(pos.Position())].IsProtected()) {
				while((pos.Position() > 0) && (vs.styles[pdoc->StyleIndexAt(pos.Position() - 1)].IsProtected()))
					pos.Add(-1);
			}
		}
	}
...
void Editor::MovedCaret(SelectionPosition newPos, SelectionPosition previousPos, bool ensureVisible)
{
	...
	if(ensureVisible) {
		...
		if(currentLine >= wrapPending.start)
			WrapLines(wsAll);
		XYScrollPosition newXY = XYScrollToMakeVisible(SelectionRange(posDrag.IsValid() ? posDrag : newPos), xysDefault);
...
{
	const SelectionPosition spCaret = ((Sel.Count() == 1) && Sel.Empty()) ? Sel.Last() : SelectionPosition(INVALID_POSITION);
	...
	if((options & xysVertical) && (pt.y < rcClient.top || ptBottomCaret.y >= rcClient.bottom || (caretYPolicy & CARET_STRICT))) {
		...
		if(!(range.caret == range.anchor)) {
			...
			if(lineAnchor < lineCaret) {
				...
				newXY.topLine = smin(newXY.topLine, lineAnchor);
				newXY.topLine = smax(newXY.topLine, lineCaret - LinesOnScreen());
			}
			else {
				// Shift down to show anchor or as much of range as possible
				newXY.topLine = smax(newXY.topLine, lineAnchor - LinesOnScreen());
	...
	if((options & xysHorizontal) && !Wrapping()) {
		...
		if(bSlop) {     // A margin is defined
			...
			if(bStrict) {
				...
				else if(pt.x >= rcClient.right) { // Caret is on the right of the display
					newXY.xOffset += xMoveR;
				}
			}
		}
		...
		if(!(range.caret == range.anchor)) {
			if(ptAnchor.x < pt.x) {
				...
				newXY.xOffset = smin(newXY.xOffset, maxOffset);
				newXY.xOffset = smax(newXY.xOffset, minOffset);
			}
			else {
...
void Editor::CaretSetPeriod(int period)
{
	if(caret.period != period) {
		caret.period = period;
		caret.Flags |= Caret::fOn;
		if(FineTickerAvailable()) {
			FineTickerCancel(tickCaret);
			if((caret.Flags & Caret::fActive) && (caret.period > 0))
				FineTickerStart(tickCaret, caret.period, caret.period/10);
		}
		InvalidateCaret();
	}
	...
	if(!Wrapping()) {
		if(wrapWidth != LineLayout::wrapWidthInfinite) {
			...
			for(int lineDoc = 0; lineDoc < pdoc->LinesTotal(); lineDoc++) {
				cs.SetHeight(lineDoc, 1 + (vs.annotationVisible ? pdoc->AnnotationLines(lineDoc) : 0));
			}
			wrapOccurred = true;
		}
		wrapPending.Reset();
		...
		wrapPending.start = smin(wrapPending.start, pdoc->LinesTotal());
		if(!SetIdle(true)) {
		...
		int lineToWrapEnd = smin(wrapPending.end, pdoc->LinesTotal());
		...
		const int lineEndNeedWrap = smin(wrapPending.end, pdoc->LinesTotal());
		lineToWrapEnd = smin(lineToWrapEnd, lineEndNeedWrap);
...
// lineToWrapEnd);
				...
				goodTopLine = cs.DisplayFromDoc(lineDocTop) + smin(subLineTop, cs.GetHeight(lineDocTop)-1);
			}
		}
...
void Editor::LinesSplit(int pixelWidth)
{
	if(!RangeContainsProtected(targetStart, targetEnd)) {
		...
		for(int line = lineStart; line <= lineEnd; line++) {
			...
			if(surface && ll) {
				uint posLineStart = pdoc->LineStart(line);
...
void Editor::Paint(SciSurface * surfaceWindow, PRectangle rcArea)
{
	...
	if(Flags & fHorizontalScrollBarVisible && (EditModelFlags & fTrackLineWidth) && (view.lineWidthMaxSeen > scrollWidth)) {
		if(FineTickerAvailable()) {
...
int Editor::RealizeVirtualSpace(int position, uint virtualSpace)
{
	if(virtualSpace > 0) {
...
void Editor::AddCharUTF(const char * s, uint len, bool treatAsDBCS)
{
	FilterSelections();
	{
		...
		for(std::vector <SelectionRange *>::reverse_iterator rit = selPtrs.rbegin(); rit != selPtrs.rend(); ++rit) {
			SelectionRange * currentSel = *rit;
			if(!RangeContainsProtected(currentSel->Start().Position(), currentSel->End().Position())) {
				...
				if(Sel.Range(r).caret.VirtualSpace()) {
					...
					if(allowLineStartDeletion || (pdoc->LineStart(lineCurrentPos) != Sel.Range(r).caret.Position())) {
						if(pdoc->GetColumn(Sel.Range(r).caret.Position()) <= pdoc->GetLineIndentation(lineCurrentPos) &&
							...
							const int posSelect = pdoc->SetLineIndentation(lineCurrentPos,
...
{
	return (shift ? SCI_SHIFT : 0) | (ctrl ? SCI_CTRL : 0) | (alt ? SCI_ALT : 0) | (meta ? SCI_META : 0) | (super ? SCI_SUPER : 0);
}
...
{
	SCNotification scn; // = {};
	scn.nmhdr.code = isSavePoint ? SCN_SAVEPOINTREACHED : isSavePoint;
	NotifyParent(scn);
}
...
void Editor::NotifyMacroRecord(uint iMessage, uptr_t wParam, sptr_t lParam)
{
...
void Editor::PageMove(int direction, Selection::selTypes selt, bool stuttered)
{
	...
	if(stuttered && (direction < 0 && currentLine > topStutterLine)) {
		topLineNew = topLine;
		pt__.y = vs.lineHeight * caretYSlop;
		newPos = SPositionFromLocation(pt__, false, false, UserVirtualSpace());
	}
	else if(stuttered && (direction > 0 && currentLine < bottomStutterLine)) {
		topLineNew = topLine;
		pt__.y = vs.lineHeight * (LinesToScroll() - caretYSlop);
		newPos = SPositionFromLocation(pt__, false, false, UserVirtualSpace());
	}
	else {
...
void Editor::LineTranspose()
{
	...
	if(line > 0) {
		...
		startCurrent += pdoc->InsertString(startPrevious, lineCurrent.c_str(), static_cast<int>(lineCurrent.length()));
		pdoc->InsertString(startCurrent, linePrevious.c_str(), static_cast<int>(linePrevious.length()));
...
void Editor::CursorUpOrDown(int direction, Selection::selTypes selt)
{
	...
	if(selt == Selection::selRectangle) {
		...
		const SelectionPosition posNew = MovePositionSoVisible(PositionUpOrDown(caretToUse, direction, lastXChosen), direction);
		...
		for(size_t r = 0; r < Sel.Count(); r++) {
			...
			const SelectionPosition posNew = MovePositionSoVisible(PositionUpOrDown(spCaretNow, direction, lastX), direction);
			Sel.Range(r) = selt == Selection::selStream ? SelectionRange(posNew, Sel.Range(r).anchor) : SelectionRange(posNew);
		}
		Sel.RemoveDuplicates();
...
namespace {
uint WithExtends(uint iMessage)
{
	switch(iMessage) {
...
int NaturalDirection(uint iMessage)
{
	switch(iMessage) {
		case SCI_CHARLEFT:
		case SCI_CHARLEFTEXTEND:
...
bool IsRectExtend(uint iMessage)
{
	switch(iMessage) {
		case SCI_CHARLEFTRECTEXTEND:
		case SCI_CHARRIGHTRECTEXTEND:
...
int Editor::HorizontalMove(uint iMessage)
{
	...
	if(IsRectExtend(iMessage)) {
		...
		const SelectionPosition selAtLimit = (NaturalDirection(iMessage) > 0) ? Sel.Limits().end : Sel.Limits().start;
		Sel.selType = Selection::selStream;
		...
		for(size_t r = 0; r < Sel.Count(); r++) {
			...
			switch(iMessage) {
				...
				case SCI_LINEEND:
				case SCI_LINEENDDISPLAY:
				case SCI_LINEENDWRAP: Sel.Range(r) = SelectionRange(spCaret); break;
				case SCI_CHARLEFTEXTEND:
				case SCI_CHARRIGHTEXTEND:
				case SCI_WORDLEFTEXTEND:
				case SCI_WORDRIGHTEXTEND:
...
int Editor::DelWordOrLine(uint iMessage)
{
	...
	for(size_t r = 0; r < Sel.Count(); r++) {
		...
		switch(iMessage) {
			case SCI_DELWORDLEFT:
			    rangeDelete = Range(pdoc->NextWordStart(Sel.Range(r).caret.Position(), -1), Sel.Range(r).caret.Position());
			    break;
			case SCI_DELWORDRIGHT:
			    rangeDelete = Range(Sel.Range(r).caret.Position(), pdoc->NextWordStart(Sel.Range(r).caret.Position(), 1));
			    break;
			case SCI_DELWORDRIGHTEND:
			    rangeDelete = Range(Sel.Range(r).caret.Position(), pdoc->NextWordEnd(Sel.Range(r).caret.Position(), 1));
			    break;
			case SCI_DELLINELEFT:
			    rangeDelete = Range(pdoc->LineStart(pdoc->LineFromPosition(Sel.Range(r).caret.Position())), Sel.Range(r).caret.Position());
			    break;
			case SCI_DELLINERIGHT:
			    rangeDelete = Range(Sel.Range(r).caret.Position(), pdoc->LineEnd(pdoc->LineFromPosition(Sel.Range(r).caret.Position())));
			    break;
		}
...
int Editor::KeyCommand(uint iMessage)
{
	switch(iMessage) {
		...
		case SCI_LINEENDDISPLAYEXTEND:
		case SCI_LINEENDWRAP:
		case SCI_LINEENDWRAPEXTEND: return HorizontalMove(iMessage);
		case SCI_DOCUMENTSTART:
		    MovePositionTo(0);
		    SetLastXChosen();
		    break;
		...
		case SCI_DOCUMENTENDEXTEND:
		    ...
		    SetLastXChosen();
		    break;
		case SCI_STUTTEREDPAGEUP: PageMove(-1, Selection::noSel, true); break;
		case SCI_STUTTEREDPAGEUPEXTEND: PageMove(-1, Selection::selStream, true); break;
		case SCI_STUTTEREDPAGEDOWN: PageMove(1, Selection::noSel, true); break;
		case SCI_STUTTEREDPAGEDOWNEXTEND: PageMove(1, Selection::selStream, true); break;
		case SCI_PAGEUP: PageMove(-1); break;
		case SCI_PAGEUPEXTEND: PageMove(-1, Selection::selStream); break;
		case SCI_PAGEUPRECTEXTEND: PageMove(-1, Selection::selRectangle); break;
		case SCI_PAGEDOWN: PageMove(1); break;
		case SCI_PAGEDOWNEXTEND: PageMove(1, Selection::selStream); break;
		case SCI_PAGEDOWNRECTEXTEND: PageMove(1, Selection::selRectangle); break;
		case SCI_EDITTOGGLEOVERTYPE:
		    //inOverstrike = !inOverstrike;
			INVERSEFLAG(EditModelFlags, fInOverstrike);
		    ShowCaretAtCurrentPosition();
		    ContainerNeedsUpdate(SC_UPDATE_CONTENT);
		    NotifyUpdateUI();
		    break;
		...
		case SCI_DELWORDRIGHTEND:
		case SCI_DELLINELEFT:
		case SCI_DELLINERIGHT: return DelWordOrLine(iMessage);
		case SCI_LINECOPY: 
			{
		...
		case SCI_LINEDELETE: 
			...
			break;
		case SCI_LINETRANSPOSE: LineTranspose(); break;
		case SCI_LINEDUPLICATE: Duplicate(true); break;
		case SCI_SELECTIONDUPLICATE: Duplicate(false); break;
		case SCI_LOWERCASE: ChangeCaseOfSelection(cmLower); break;
		case SCI_UPPERCASE: ChangeCaseOfSelection(cmUpper); break;
		case SCI_SCROLLTOSTART: ScrollTo(0); break;
		case SCI_SCROLLTOEND: ScrollTo(MaxScrollPos()); break;
	}
	return 0;
}
...
int Editor::KeyDownWithModifiers(int key, int modifiers, bool * consumed)
{
	...
	if(msg) {
		ASSIGN_PTR(consumed, true);
		return static_cast<int>(WndProc(msg, 0, 0));
	}
	else {
		ASSIGN_PTR(consumed, false);
		...
		if(lineOfAnchor == lineCurrentPos) {
			if(forwards) {
				...
				if(pdoc->GetColumn(caretPosition) <= pdoc->GetColumn(pdoc->GetLineIndentPosition(lineCurrentPos)) && pdoc->IsDocFlag(pdoc->dfTabIndents)) {
					int indentation = pdoc->GetLineIndentation(lineCurrentPos);
					int indentationStep = pdoc->IndentSize();
					const int posSelect = pdoc->SetLineIndentation(lineCurrentPos, indentation + indentationStep - indentation % indentationStep);
					Sel.Range(r) = SelectionRange(posSelect);
				}
				else {
					if(pdoc->IsDocFlag(pdoc->dfUseTabs)) {
						...
						const int lengthInserted = pdoc->InsertString(caretPosition, spaceText.c_str(), static_cast<int>(spaceText.length()));
						Sel.Range(r) = SelectionRange(caretPosition + lengthInserted);
					}
				}
			}
				...
				if(pdoc->GetColumn(caretPosition) <= pdoc->GetLineIndentation(lineCurrentPos) && pdoc->IsDocFlag(pdoc->dfTabIndents)) {
					...
					int newColumn = ((pdoc->GetColumn(caretPosition) - 1) / pdoc->tabInChars) * pdoc->tabInChars;
					SETMAX(newColumn, 0);
 ...
 */
long Editor::FindText(uptr_t wParam,              ///< Search modes : @c SCFIND_MATCHCASE, @c SCFIND_WHOLEWORD,
    ...
    sptr_t lParam)      ///< @c Sci_TextToFind structure: The text to search for in the given range.
{
	...
	if(!pdoc->HasCaseFolder())
		pdoc->SetCaseFolder(CaseFolderForEncoding());
	try {
		long pos = pdoc->FindText(static_cast<int>(ft->chrg.cpMin), static_cast<int>(ft->chrg.cpMax), ft->lpstrText, static_cast<int>(wParam), &lengthFound);
		if(pos != -1) {
 ...
 * @return The position of the found text, -1 if not found.
 */
long Editor::SearchText(uint iMessage,              ///< Accepts both @c SCI_SEARCHNEXT and @c SCI_SEARCHPREV.
    ...
    sptr_t lParam)                      ///< The text to search for.
{
	...
	try {
		if(iMessage == SCI_SEARCHNEXT) {
			pos = pdoc->FindText(searchAnchor, pdoc->Length(), txt, static_cast<int>(wParam), &lengthFound);
		}
		else {
...
long Editor::SearchInTarget(const char * text, int length)
{
	...
	if(!pdoc->HasCaseFolder())
		pdoc->SetCaseFolder(CaseFolderForEncoding());
	try {
		long pos = pdoc->FindText(targetStart, targetEnd, text, searchFlags, &lengthFound);
		if(pos != -1) {
...
void Editor::CopySelectionRange(SelectionText * ss, bool allowLineCopy)
{
	if(Sel.Empty()) {
		...
		ss->Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, Sel.IsRectangular(), Sel.selType == Selection::selLines);
	}
}
...
void Editor::CopyRangeToClipboard(int start, int end)
{
	...
	SelectionText selectedText;
	std::string text = RangeText(start, end);
	selectedText.Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, false, false);
	CopyToClipboard(selectedText);
}
...
{
	SelectionText selectedText;
	selectedText.Copy(std::string(text, length), pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, false, false);
	CopyToClipboard(selectedText);
}
...
void Editor::SetDragPosition(SelectionPosition newPos)
{
	...
	if(!(posDrag == newPos)) {
		...
		if(FineTickerAvailable()) {
			FineTickerCancel(tickCaret);
			if((caret.Flags & Caret::fActive) && (caret.period > 0) && (newPos.Position() < 0))
				FineTickerStart(tickCaret, caret.period, caret.period/10);
		}
		else {
			SetTicking(true);
...
bool Editor::PointInSelection(SciPoint pt)
{
	...
	for(size_t r = 0; r < Sel.Count(); r++) {
		...
		if(range.Contains(pos)) {
			bool hit = true;
			if(pos == range.Start()) {
				if(pt.x < ptPos.x) // see if just before selection
					hit = false;
			}
			if(pos == range.End()) {
				if(pt.x > ptPos.x) // see if just after selection
					hit = false;
			}
			if(hit)
				return true;
...
void Editor::ButtonDownWithModifiers(SciPoint pt, uint curTime, int modifiers)
{
	...
	if(((curTime - lastClickTime) < Platform::DoubleClickTime()) && Close(pt, lastClick, doubleClickCloseThreshold)) {
		...
		else if(oneof2(selectionType, selSubLine, selWholeLine)) {
		...
		if(inSelMargin) {
			...
			if(!shift)
				inDragDrop = (PointInSelection(pt) && !SelectionEmpty()) ? ddInitial : ddNone;
			SetMouseCapture(true);
			if(FineTickerAvailable())
				FineTickerStart(tickScroll, 100, 10);
			if(inDragDrop != ddInitial) {
				...
				if(shift)
					anchorCurrent = Sel.IsRectangular() ? Sel.Rectangular().anchor : Sel.RangeMain().anchor;
				Sel.selType = alt ? Selection::selRectangle : Selection::selStream;
				selectionType = selChar;
				originalAnchorPos = Sel.MainCaret();
...
void Editor::ButtonMoveWithModifiers(SciPoint pt, int modifiers)
{
	...
	SelectionPosition movePos = SPositionFromLocation(pt, false, false, AllowVirtualSpace(virtualSpaceOptions, Sel.IsRectangular()));
		...
		if(inDragDrop == ddDragging) {
			...
			if(selStart < selEnd) {
				if(drag.Length()) {
					...
					if(ctrl) {
						const int lengthInserted = pdoc->InsertString(newPos.Position(), drag.Data(), length);
						if(lengthInserted > 0) {
...
void Editor::Tick()
{
	...
	if(caret.period > 0) {
		...
		if(timer.ticksToWait <= 0) {
			INVERSEFLAG(caret.Flags, Caret::fOn);
...
bool Editor::Idle()
{
	...
	if(needWrap) {
		WrapLines(wsIdle); // Wrap lines during idle.
		needWrap = wrapPending.NeedsWrap(); // No more wrapping
	}
...
int Editor::PositionAfterArea(PRectangle rcArea) const
{
	...
	return (lineAfter < cs.LinesDisplayed()) ? pdoc->LineStart(cs.DocFromDisplay(lineAfter) + 1) : pdoc->Length();
}
...
void Editor::IdleStyling()
{
	...
	const int endGoal = (idleStyling >= SC_IDLESTYLING_AFTERVISIBLE) ? pdoc->Length() : posAfterArea;
...
{
	return rc.Empty() ? true : rcPaint.Contains(rc);
}

bool Editor::PaintContainsMargin()
{
	if(wMargin.GetID()) {
		return false; // With separate margin view, paint of text view never contains margin.
	}
	else {
		PRectangle rcSelMargin = GetClientRectangle();
...
void Editor::CheckForChangeOutsidePaint(Range r)
{
	if(paintState == painting && !(Flags & fPaintingAllText)) {
		...
		if(r.Valid()) {
			...
			PRectangle rcText = GetTextRectangle();
			SETMAX(rcRange.top, rcText.top);
			SETMIN(rcRange.bottom, rcText.bottom);
...
int Editor::ExpandLine(int line)
{
	...
	while(line <= lineMaxSubord) {
		...
		if(level & SC_FOLDLEVELHEADERFLAG)
			line = cs.GetExpanded(line) ? ExpandLine(line) : pdoc->GetLastChild(line);
		line++;
	}
		...
		if(action == SC_FOLDACTION_CONTRACT) {
			...
			if(lineMaxSubord > line) {
				...
				if(lineCurrent > line && lineCurrent <= lineMaxSubord) {
					EnsureCaretVisible(); // This does not re-expand the fold
				}
			}
		}
		else {
...
void Editor::FoldChanged(int line, int levelNow, int levelPrev)
{
	if(levelNow & SC_FOLDLEVELHEADERFLAG) {
		...
		if(!cs.GetExpanded(line)) {
			...
			// Combining two blocks where the second one is collapsed (e.g. by adding characters in the line which separates the
			// two blocks)
			FoldExpand(line, SC_FOLDACTION_EXPAND, levelPrev);
		}
	}
...
{
	return pdoc ? pdoc->dbcsCodePage : 0;
}
...
void Editor::AddStyledText(const char * buffer, int appendLength)
{
	...
	pdoc->StartStyling(CurrentPosition(), static_cast<uchar>(0xff));
...
static char * FASTCALL CharPtrFromSPtr(sptr_t lParam)
{
	return reinterpret_cast<char *>(lParam);
}

void Editor::StyleSetMessage(uint iMessage, uptr_t wParam, sptr_t lParam)
{
...
sptr_t Editor::StyleGetMessage(uint iMessage, uptr_t wParam, sptr_t lParam)
{
	vs.EnsureStyle(wParam);
	switch(iMessage) {
		case SCI_STYLEGETFORE: return vs.styles[wParam].fore.AsLong();
		case SCI_STYLEGETBACK: return vs.styles[wParam].back.AsLong();
...
void Editor::SetSelectionNMessage(uint iMessage, uptr_t wParam, sptr_t lParam)
{
	InvalidateRange(Sel.Range(wParam).Start().Position(), Sel.Range(wParam).End().Position());
	switch(iMessage) {
		case SCI_SETSELECTIONNCARET: Sel.Range(wParam).caret.SetPosition(static_cast<int>(lParam)); break;
		case SCI_SETSELECTIONNANCHOR: Sel.Range(wParam).anchor.SetPosition(static_cast<int>(lParam)); break;
		case SCI_SETSELECTIONNCARETVIRTUALSPACE: Sel.Range(wParam).caret.SetVirtualSpace(static_cast<int>(lParam)); break;
		case SCI_SETSELECTIONNANCHORVIRTUALSPACE: Sel.Range(wParam).anchor.SetVirtualSpace(static_cast<int>(lParam)); break;
		case SCI_SETSELECTIONNSTART: Sel.Range(wParam).anchor.SetPosition(static_cast<int>(lParam)); break;
		case SCI_SETSELECTIONNEND: Sel.Range(wParam).caret.SetPosition(static_cast<int>(lParam)); break;
	}
	InvalidateRange(Sel.Range(wParam).Start().Position(), Sel.Range(wParam).End().Position());
	ContainerNeedsUpdate(SC_UPDATE_SELECTION);
}
...
sptr_t Editor::BytesResult(sptr_t lParam, const uchar * val, size_t len)
{
...
sptr_t Editor::WndProc(uint iMessage, uptr_t wParam, sptr_t lParam)
{
	...
	switch(iMessage) {
		...
		case SCI_UNDO:
		    ...
		    SetLastXChosen();
		    break;
		case SCI_CANUNDO: return BIN(pdoc->CanUndo() && !pdoc->IsReadOnly());
		case SCI_EMPTYUNDOBUFFER:
		    pdoc->DeleteUndoHistory();
		    return 0;
		case SCI_GETFIRSTVISIBLELINE: return topLine;
		...
		case SCI_GETLINECOUNT: return (pdoc->LinesTotal() == 0) ? 1 : pdoc->LinesTotal();
		case SCI_GETMODIFY: return !pdoc->IsSavePoint();
		case SCI_SETSEL: 
			{
		...
		case SCI_LINELENGTH:
		    if((static_cast<int>(wParam) < 0) || (static_cast<int>(wParam) > pdoc->LineFromPosition(pdoc->Length())))
			    return 0;
		...
		case SCI_GETTARGETTEXT: 
			{
				...
				return BytesResult(lParam, reinterpret_cast<const uchar *>(text.c_str()), text.length());
			}
		...
		case SCI_GETMARGINLEFT: return vs.leftMarginWidth;
		case SCI_GETMARGINRIGHT: return vs.rightMarginWidth;
		...
		case SCI_CHANGEINSERTION:
		    ...
		    return 0;
		case SCI_APPENDTEXT: pdoc->InsertString(pdoc->Length(), CharPtrFromSPtr(lParam), static_cast<int>(wParam)); return 0;
		case SCI_CLEARALL: ClearAll(); return 0;
		case SCI_DELETERANGE: pdoc->DeleteChars(static_cast<int>(wParam), static_cast<int>(lParam)); return 0;
		case SCI_CLEARDOCUMENTSTYLE: ClearDocumentStyle(); return 0;
		case SCI_SETUNDOCOLLECTION: pdoc->SetUndoCollection(wParam != 0); return 0;
		case SCI_GETUNDOCOLLECTION: return pdoc->IsCollectingUndo();
		case SCI_BEGINUNDOACTION: pdoc->BeginUndoAction(); return 0;
		case SCI_ENDUNDOACTION: pdoc->EndUndoAction(); return 0;
		case SCI_GETCARETPERIOD: return caret.period;
		case SCI_SETCARETPERIOD: CaretSetPeriod(static_cast<int>(wParam)); break;
		...
		case SCI_GETCHARAT: return pdoc->CharAt(static_cast<int>(wParam));
		case SCI_SETCURRENTPOS:
		    ...
		    break;
		case SCI_GETCURRENTPOS: return Sel.IsRectangular() ? Sel.Rectangular().caret.Position() : Sel.MainCaret();
		case SCI_SETANCHOR:
		    if(Sel.IsRectangular()) {
		...
		case SCI_SETSELECTIONSTART: SetSelection(smax(Sel.MainCaret(), static_cast<int>(wParam)), static_cast<int>(wParam)); break;
		case SCI_GETSELECTIONSTART: return Sel.LimitsForRectangularElseMain().start.Position();
		case SCI_SETSELECTIONEND: SetSelection(static_cast<int>(wParam), smin(Sel.MainAnchor(), static_cast<int>(wParam))); break;
		...
		case SCI_SETPRINTMAGNIFICATION: view.printParameters.magnification = static_cast<int>(wParam); break;
		case SCI_GETPRINTMAGNIFICATION: return view.printParameters.magnification;
		case SCI_SETPRINTCOLOURMODE: view.printParameters.colourMode = static_cast<int>(wParam); break;
		case SCI_GETPRINTCOLOURMODE: return view.printParameters.colourMode;
		case SCI_SETPRINTWRAPMODE:
		    view.printParameters.wrapState = (wParam == SC_WRAP_WORD) ? eWrapWord : eWrapNone;
		    break;
		case SCI_GETPRINTWRAPMODE: return view.printParameters.wrapState;
		case SCI_GETSTYLEAT: return (static_cast<int>(wParam) >= pdoc->Length()) ? 0 : pdoc->StyleAt(static_cast<int>(wParam));
		case SCI_REDO: Redo(); break;
		case SCI_SELECTALL: SelectAll(); break;
		case SCI_SETSAVEPOINT: pdoc->SetSavePoint(); break;
		...
		case SCI_CANREDO: return (pdoc->CanRedo() && !pdoc->IsReadOnly()) ? 1 : 0;
		case SCI_MARKERLINEFROMHANDLE: return pdoc->LineFromHandle(static_cast<int>(wParam));
		case SCI_MARKERDELETEHANDLE: pdoc->DeleteMarkFromHandle(static_cast<int>(wParam)); break;
		case SCI_GETVIEWWS: return vs.viewWhitespace;
		case SCI_SETVIEWWS:
		    vs.viewWhitespace = static_cast<WhiteSpaceVisibility>(wParam);
		    Redraw();
		    break;
		case SCI_GETTABDRAWMODE: return vs.tabDrawMode;
		case SCI_SETTABDRAWMODE:
		    vs.tabDrawMode = static_cast<TabDrawMode>(wParam);
		    Redraw();
		    break;
		case SCI_GETWHITESPACESIZE: return vs.whitespaceSize;
		...
		case SCI_SETEOLMODE: pdoc->eolMode = static_cast<int>(wParam); break;
		case SCI_SETLINEENDTYPESALLOWED:
		    ...
		    break;
		case SCI_GETLINEENDTYPESALLOWED: return pdoc->GetLineEndTypesAllowed();
		case SCI_GETLINEENDTYPESACTIVE: return pdoc->GetLineEndTypesActive();
		case SCI_STARTSTYLING: pdoc->StartStyling(static_cast<int>(wParam), static_cast<char>(lParam)); break;
		case SCI_SETSTYLING:
		    if(static_cast<int>(wParam) < 0)
		...
		case SCI_SETTWOPHASEDRAW:
		    if(view.SetTwoPhaseDraw(wParam != 0))
			    InvalidateStyleRedraw();
		    break;
		case SCI_GETPHASESDRAW: return view.phasesDraw;
		...
		case SCI_SETFONTQUALITY:
		    ...
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETFONTQUALITY: return (vs.extraFontFlag & SC_EFF_QUALITY_MASK);
		case SCI_SETTABWIDTH:
		    ...
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETTABWIDTH: return pdoc->tabInChars;
		...
		case SCI_SETUSETABS:
		    ...
		    InvalidateStyleRedraw();
		    break;
		case SCI_SETLINEINDENTATION: pdoc->SetLineIndentation(static_cast<int>(wParam), static_cast<int>(lParam)); break;
		case SCI_SETTABINDENTS: pdoc->SetDocFlag(pdoc->dfTabIndents, wParam); break;
		case SCI_SETBACKSPACEUNINDENTS: pdoc->SetDocFlag(pdoc->dfBackspaceUnindents, wParam); break;
		...
		case SCI_SETIDLESTYLING:
		    ...
		    break;
		case SCI_GETMOUSEDWELLTIME: return dwellDelay;
		case SCI_WORDSTARTPOSITION: return pdoc->ExtendWordSelect(static_cast<int>(wParam), -1, lParam != 0);
		case SCI_WORDENDPOSITION: return pdoc->ExtendWordSelect(static_cast<int>(wParam), 1, lParam != 0);
		case SCI_ISRANGEWORD: return pdoc->IsWordAt(static_cast<int>(wParam), static_cast<int>(lParam));
		case SCI_GETIDLESTYLING: return idleStyling;
		...
		case SCI_SETWRAPVISUALFLAGSLOCATION:
		    if(vs.SetWrapVisualFlagsLocation(static_cast<int>(wParam)))
			    InvalidateStyleRedraw();
		    break;
		case SCI_GETWRAPMODE: return vs.wrapState;
		case SCI_GETWRAPVISUALFLAGS: return vs.wrapVisualFlags;
		case SCI_GETWRAPVISUALFLAGSLOCATION: return vs.wrapVisualFlagsLocation;
		...
		case SCI_SETWRAPINDENTMODE:
		    ...
		    break;
		case SCI_SETLAYOUTCACHE: view.llc.SetLevel(static_cast<int>(wParam)); break;
		case SCI_SETPOSITIONCACHE: view.posCache.SetSize(wParam); break;
		case SCI_GETWRAPSTARTINDENT: return vs.wrapVisualStartIndent;
		case SCI_GETWRAPINDENTMODE: return vs.wrapIndentMode;
		case SCI_GETLAYOUTCACHE: return view.llc.GetLevel();
		case SCI_GETPOSITIONCACHE: return view.posCache.GetSize();
		case SCI_SETSCROLLWIDTH:
		    PLATFORM_ASSERT(wParam > 0);
		    if((wParam > 0) && (wParam != static_cast<uint >(scrollWidth))) {
			    view.lineWidthMaxSeen = 0;
			    scrollWidth = static_cast<int>(wParam);
			    SetScrollBars();
		    }
		    break;
		case SCI_SETSCROLLWIDTHTRACKING: SETFLAG(EditModelFlags, fTrackLineWidth, wParam); break;
		case SCI_GETSCROLLWIDTH: return scrollWidth;
		case SCI_GETSCROLLWIDTHTRACKING: return BIN(EditModelFlags & fTrackLineWidth);
		case SCI_LINESJOIN: LinesJoin(); break;
		case SCI_LINESSPLIT: LinesSplit(static_cast<int>(wParam)); break;
		case SCI_TEXTWIDTH:
		...
		case SCI_SETCARETSTICKY:
		    ...
		    break;
		case SCI_TOGGLECARETSTICKY: caretSticky = !caretSticky; break;
		case SCI_GETCARETSTICKY: return caretSticky;
		...
		case SCI_SETINDENTATIONGUIDES:
		    ...
		    Redraw();
		    break;
		case SCI_GETINDENTATIONGUIDES: return vs.viewIndentationGuides;
		case SCI_SETHIGHLIGHTGUIDE:
		    ...
		    break;
		case SCI_GETHIGHLIGHTGUIDE: return highlightGuideColumn;
		case SCI_GETLINEENDPOSITION: return pdoc->LineEnd(static_cast<int>(wParam));
		case SCI_SETCODEPAGE:
		    ...
		    break;
		case SCI_GETCODEPAGE: return pdoc->dbcsCodePage;
		case SCI_SETIMEINTERACTION:
		    imeInteraction = static_cast<EditModel::IMEInteraction>(wParam);
		    break;
		case SCI_GETIMEINTERACTION: return imeInteraction;
		...
		case SCI_MARKERDEFINE:
		    ...
		    RedrawSelMargin();
		    break;
		case SCI_MARKERSYMBOLDEFINED: return (wParam <= MARKER_MAX) ? vs.markers[wParam].markType : 0;
		...
		case SCI_MARKERADDSET:
		    ...
		    break;
		case SCI_MARKERDELETE: pdoc->DeleteMark(static_cast<int>(wParam), static_cast<int>(lParam)); break;
		case SCI_MARKERDELETEALL: pdoc->DeleteAllMarks(static_cast<int>(wParam)); break;
		case SCI_MARKERGET: return pdoc->GetMark(static_cast<int>(wParam));
		case SCI_MARKERNEXT: return pdoc->MarkerNext(static_cast<int>(wParam), static_cast<int>(lParam));
		...
		case SCI_MARKERDEFINEPIXMAP:
		    ...
		    RedrawSelMargin();
		    break;
		case SCI_RGBAIMAGESETWIDTH: sizeRGBAImage.x = static_cast<XYPOSITION>(wParam); break;
		case SCI_RGBAIMAGESETHEIGHT: sizeRGBAImage.y = static_cast<XYPOSITION>(wParam); break;
		case SCI_RGBAIMAGESETSCALE: scaleRGBAImage = static_cast<float>(wParam); break;
		...
		case SCI_SETMARGINBACKN:
		    ...
		    break;
		case SCI_GETMARGINTYPEN: return ValidMargin(wParam) ? vs.ms[wParam].style : 0;
		case SCI_GETMARGINWIDTHN: return ValidMargin(wParam) ? vs.ms[wParam].width : 0;
		case SCI_GETMARGINMASKN: return ValidMargin(wParam) ? vs.ms[wParam].mask : 0;
		case SCI_GETMARGINSENSITIVEN: return ValidMargin(wParam) ? (vs.ms[wParam].sensitive ? 1 : 0) : 0;
		case SCI_GETMARGINCURSORN: return ValidMargin(wParam) ? vs.ms[wParam].cursor : 0;
		case SCI_GETMARGINBACKN: return ValidMargin(wParam) ? vs.ms[wParam].back.AsLong() : 0;
		case SCI_SETMARGINS:
		    if(wParam < 1000)
			    vs.ms.resize(wParam);
		    break;
		case SCI_GETMARGINS: return vs.ms.size();
		...
		case SCI_STYLESETVISIBLE:
		case SCI_STYLESETCHANGEABLE:
		case SCI_STYLESETHOTSPOT: StyleSetMessage(iMessage, wParam, lParam); break;
		...
		case SCI_STYLEGETVISIBLE:
		case SCI_STYLEGETCHANGEABLE:
		case SCI_STYLEGETHOTSPOT: return StyleGetMessage(iMessage, wParam, lParam);
		...
		case SCI_SETSTYLEBITS:
		    vs.EnsureStyle(0xff);
		    break;
		case SCI_GETSTYLEBITS: return 8;
		case SCI_SETLINESTATE: return pdoc->SetLineState(static_cast<int>(wParam), static_cast<int>(lParam));
		case SCI_GETLINESTATE: return pdoc->GetLineState(static_cast<int>(wParam));
		case SCI_GETMAXLINESTATE: return pdoc->GetMaxLineState();
		case SCI_GETCARETLINEVISIBLE: return vs.showCaretLineBackground;
		case SCI_SETCARETLINEVISIBLE:
		    vs.showCaretLineBackground = wParam != 0;
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETCARETLINEVISIBLEALWAYS: return vs.alwaysShowCaretLineBackground;
		case SCI_SETCARETLINEVISIBLEALWAYS:
		    vs.alwaysShowCaretLineBackground = wParam != 0;
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETCARETLINEBACK: return vs.caretLineBackground.AsLong();
		case SCI_SETCARETLINEBACK:
		    vs.caretLineBackground = static_cast<int>(wParam);
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETCARETLINEBACKALPHA: return vs.caretLineAlpha;
		case SCI_SETCARETLINEBACKALPHA:
		    vs.caretLineAlpha = static_cast<int>(wParam);
		    InvalidateStyleRedraw();
		    break;
		// Folding messages
		case SCI_VISIBLEFROMDOCLINE: return cs.DisplayFromDoc(static_cast<int>(wParam));
		case SCI_DOCLINEFROMVISIBLE: return cs.DocFromDisplay(static_cast<int>(wParam));
		case SCI_WRAPCOUNT: return WrapCount(static_cast<int>(wParam));
		...
		case SCI_GETFOLDLEVEL: return pdoc->GetLevel(static_cast<int>(wParam));
		case SCI_GETLASTCHILD: return pdoc->GetLastChild(static_cast<int>(wParam), static_cast<int>(lParam));
		case SCI_GETFOLDPARENT: return pdoc->GetFoldParent(static_cast<int>(wParam));
		case SCI_SHOWLINES:
		...
		case SCI_HIDELINES:
		    ...
		    Redraw();
		    break;
		case SCI_GETLINEVISIBLE: return cs.GetVisible(static_cast<int>(wParam));
		case SCI_GETALLLINESVISIBLE: return cs.HiddenLines() ? 0 : 1;
		case SCI_SETFOLDEXPANDED:
		    SetFoldExpanded(static_cast<int>(wParam), lParam != 0);
		    break;
		case SCI_GETFOLDEXPANDED: return cs.GetExpanded(static_cast<int>(wParam));
		case SCI_SETAUTOMATICFOLD:
		    foldAutomatic = static_cast<int>(wParam);
		    break;
		case SCI_GETAUTOMATICFOLD: return foldAutomatic;
		...
		case SCI_EXPANDCHILDREN:
		    ...
		    break;
		case SCI_CONTRACTEDFOLDNEXT: return ContractedFoldNext(static_cast<int>(wParam));
		...
		case SCI_SEARCHANCHOR:
		    ...
		    break;
		case SCI_SEARCHNEXT:
		case SCI_SEARCHPREV: return SearchText(iMessage, wParam, lParam);
		...
		case SCI_SETVISIBLEPOLICY:
		    ...
		    break;
		case SCI_LINESONSCREEN: return LinesOnScreen();
		case SCI_SETSELFORE:
		...
		case SCI_SETSELALPHA:
		    ...
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETSELALPHA: return vs.selAlpha;
		case SCI_GETSELEOLFILLED: return vs.selEOLFilled;
		...
		case SCI_SETCARETWIDTH:
		    ...
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETCARETWIDTH: return vs.caretWidth;
		case SCI_ASSIGNCMDKEY:
		    kmap.AssignCmdKey(Platform::LowShortFromLong(static_cast<long>(wParam)),
		    Platform::HighShortFromLong(static_cast<long>(wParam)), static_cast<uint>(lParam));
		    break;
		...
		case SCI_INDICSETSTYLE:
		    ...
		    break;
		case SCI_INDICGETSTYLE: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].sacNormal.style : 0;
		case SCI_INDICSETFORE:
		    ...
		    break;
		case SCI_INDICGETFORE: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].sacNormal.fore.AsLong() : 0;
		case SCI_INDICSETHOVERSTYLE:
		    ...
		    break;
		case SCI_INDICGETHOVERSTYLE: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].sacHover.style : 0;
		case SCI_INDICSETHOVERFORE:
		    ...
		    break;
		case SCI_INDICGETHOVERFORE: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].sacHover.fore.AsLong() : 0;
		case SCI_INDICSETFLAGS:
		    ...
		    break;
		case SCI_INDICGETFLAGS: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].Flags() : 0;
		case SCI_INDICSETUNDER:
		    ...
		    break;
		case SCI_INDICGETUNDER: return (wParam <= INDIC_MAX) ? vs.indicators[wParam].under : 0;
		...
		case SCI_PAGEUPRECTEXTEND:
		case SCI_PAGEDOWNRECTEXTEND:
		case SCI_SELECTIONDUPLICATE: return KeyCommand(iMessage);
		...
		case SCI_GETVIEWEOL: return vs.viewEOL;
		case SCI_SETVIEWEOL:
		    vs.viewEOL = wParam != 0;
		    InvalidateStyleRedraw();
		    break;
		...
		case SCI_GETEDGECOLUMN: return vs.theEdge.column;
		case SCI_SETEDGECOLUMN:
		    vs.theEdge.column = static_cast<int>(wParam);
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETEDGEMODE: return vs.edgeState;
		case SCI_SETEDGEMODE:
		    vs.edgeState = static_cast<int>(wParam);
		    InvalidateStyleRedraw();
		    break;
		case SCI_GETEDGECOLOUR: return vs.theEdge.colour.AsLong();
		...
		case SCI_SETMODEVENTMASK:
		    ...
		    return 0;
		case SCI_GETMODEVENTMASK: return modEventMask;
		...
		case SCI_SETLENGTHFORENCODE:
		    ...
		    return 0;
		case SCI_SELECTIONISRECTANGLE: return Sel.selType == Selection::selRectangle ? 1 : 0;
		case SCI_SETSELECTIONMODE: {
		    switch(wParam) {
			    case SC_SEL_STREAM:
		...
		case SCI_GETSELECTIONMODE:
		    switch(Sel.selType) {
			    case Selection::selStream: return SC_SEL_STREAM;
			    case Selection::selRectangle: return SC_SEL_RECTANGLE;
			    case Selection::selLines: return SC_SEL_LINES;
			    case Selection::selThin: return SC_SEL_THIN;
			    default: return SC_SEL_STREAM; // ?!
		    }
		...
		case SCI_SETOVERTYPE: SETFLAG(EditModelFlags, fInOverstrike, wParam); break;
		...
		case SCI_GETADDITIONALCARETFORE: return vs.additionalCaretColour.AsLong();
		case SCI_SETVIRTUALSPACEOPTIONS: virtualSpaceOptions = static_cast<int>(wParam); break;
		...
		case SCI_SWAPMAINANCHORCARET:
		    ...
		    Sel.RangeMain().Swap();
		    break;
		case SCI_MULTIPLESELECTADDNEXT: MultipleSelectAdd(addOne); break;
		case SCI_MULTIPLESELECTADDEACH: MultipleSelectAdd(addEach); break;
		case SCI_CHANGELEXERSTATE: pdoc->ChangeLexerState(static_cast<int>(wParam), static_cast<int>(lParam)); break;
		case SCI_SETIDENTIFIER: SetCtrlID(static_cast<int>(wParam)); break;
		case SCI_GETADDITIONALSELALPHA: return vs.selAdditionalAlpha;
		case SCI_GETIDENTIFIER: return GetCtrlID();
		case SCI_GETTECHNOLOGY: return technology;
		case SCI_COUNTCHARACTERS: return pdoc->CountCharacters(static_cast<int>(wParam), static_cast<int>(lParam));
		case SCI_SETTECHNOLOGY:
		    // No action by default
		    break;
		default: return DefWndProc(iMessage, wParam, lParam);
	}
