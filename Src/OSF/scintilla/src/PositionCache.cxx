...
void LineLayout::Resize(int maxLineLength_)
{
	if(maxLineLength_ > maxLineLength) {
		...
		styles = new uchar[maxLineLength_ + 1];
...
void FASTCALL LineLayoutCache::Allocate(size_t length_)
{
...
void SpecialRepresentations::SetRepresentation(const char * charBytes, const char * value)
{
	...
	if(it == mapReprs.end()) {
		...
		startByteHasReprs[static_cast<uchar>(charBytes[0])]++;
	}
...
bool PositionCacheEntry::Retrieve(uint styleNumber_, const char * s_,
    uint len_, XYPOSITION * positions_) const
{
...
uint PositionCacheEntry::Hash(uint styleNumber_, const char * s, uint len_)
{
	uint ret = s[0] << 7;
    ...
    const char * s, uint len, XYPOSITION * positions, Document * pdoc)
{
	...
	if((!pces.empty()) && (len < 30)) {
		...
		uint hashValue = PositionCacheEntry::Hash(styleNumber, s, len);
		probe = hashValue % pces.size();
		if(pces[probe].Retrieve(styleNumber, s, len, positions)) {
			return;
		}
		uint probe2 = (hashValue * 37) % pces.size();
	...
	if(len > BreakFinder::lengthStartSubdivision) {
		...
		while(startSegment < len) {
			uint lenSegment = pdoc->SafeSegment(s + startSegment, len - startSegment, BreakFinder::lengthEachSubdivision);
			...
			for(uint inSeg = 0; inSeg < lenSegment; inSeg++) {
