...
public:
	...
	size_t CaseConvertString(char * converted, size_t sizeConverted, const char * mixed, size_t lenMixed)
	{
		...
		size_t mixedPos = 0;
		uchar bytes[UTF8MaxBytes + 1];
		while(mixedPos < lenMixed) {
			const uchar leadByte = static_cast<uchar>(mixed[mixedPos]);
...
CaseConverter caseConvFold; // @global
...
void AddSymmetric(enum CaseConversion conversion, int lower, int upper)
{
	...
	switch(conversion) {
		case CaseConversionFold: caseConvFold.Add(upper, lowerUTF8); break;
		case CaseConversionUpper: caseConvUp.Add(lower, upperUTF8); break;
		case CaseConversionLower: caseConvLow.Add(upper, lowerUTF8); break;
	}
}
...
void SetupConversions(enum CaseConversion conversion)
{
	...
	while(*sComplex) {
		...
		sComplex++;
		lowerUTF8[i] = 0;
		int character = UnicodeFromUTF8(reinterpret_cast<uchar *>(originUTF8));
	...
	switch(conversion) {
		case CaseConversionFold: caseConvFold.FinishedAdding(); break;
		case CaseConversionUpper: caseConvUp.FinishedAdding(); break;
		case CaseConversionLower: caseConvLow.FinishedAdding(); break;
	}
}
...
CaseConverter * ConverterForConversion(enum CaseConversion conversion)
{
	switch(conversion) {
		...
		case CaseConversionUpper: return &caseConvUp;
		case CaseConversionLower: return &caseConvLow;
	}
	return 0;
}
}
...
std::string CaseConvertString(const std::string &s, enum CaseConversion conversion)
{
	...
	size_t lenMapped = CaseConvertString(&retMapped[0], retMapped.length(), s.c_str(), s.length(), conversion);
