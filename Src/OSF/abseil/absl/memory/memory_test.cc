...
struct MoveOnly {
	MoveOnly() = default;
	explicit MoveOnly(int i1) : ip1{new int {
				i1
			}} {}
	MoveOnly(int i1, int i2) : ip1{new int {
				i1
			}}, ip2{new int{i2}} {}
...
struct PointerWith {
	...
	static PointerWith pointer_to(element_type& r) { // NOLINT(runtime/references)
...
struct TestValue {
	...
	explicit TestValue(int* trace) : trace(trace) 
	{
		++*trace;
	}
	~TestValue() 
