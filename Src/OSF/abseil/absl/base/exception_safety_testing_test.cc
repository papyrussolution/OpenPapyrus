...
TEST(ThrowingValueTest, ThrowingCtors) {
	...
	TestOp([]() {
				ThrowingValue<> bomb(1);
			});
	TestOp([&]() {
				ThrowingValue<> bomb1 = bomb;
			});
	TestOp([&]() {
				ThrowingValue<> bomb1 = std::move(bomb);
			});
}
...
TEST(ThrowingAllocatorTest, NonThrowingConstruction) {
...

	{
		...
		SetCountdown();
		ExpectNoThrow([&]() {
					ThrowingAllocator<double> a1 = a;
				});

		SetCountdown();
		ExpectNoThrow([&]() {
					ThrowingAllocator<double> a1 = std::move(a);
				});
...
struct NonNegative {
	bool operator==(const NonNegative& other) const {
		return i == other.i;
	}
