...
private:
		result_type mean_;
		result_type stddev_;
		static_assert(std::is_floating_point<RealType>::value, "Class-template absl::gaussian_distribution<> must be parameterized using a floating-point type.");
	};
...
template <typename RealType> template <typename URBG> typename gaussian_distribution<RealType>::result_type gaussian_distribution<RealType>::operator()(URBG& g,  // NOLINT(runtime/references)
    const param_type& p) {
...
template <typename URBG>
inline double gaussian_distribution_base::zignor(URBG& g) {  // NOLINT(runtime/references)
	using random_internal::GeneratePositiveTag;
	using random_internal::GenerateRealFromBits;
