...
{
	return countl_zero(static_cast<T>(~x)); // Avoid integer promotion to a wider type
}
...
{
	return countr_zero(static_cast<T>(~x)); // Avoid integer promotion to a wider type
}
...
template <class T> ABSL_INTERNAL_CONSTEXPR_CLZ inline typename std::enable_if<std::is_unsigned<T>::value, T>::type bit_width(T x) noexcept 
{
