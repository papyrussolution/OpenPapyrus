...
std::ostream & operator<<(std::ostream & os, uint128 v) 
{
	...
	if(static_cast<size_t>(width) > rep.size()) {
		...
		if(adjustfield == std::ios::left) {
			rep.append(static_cast<size_t>(width - rep.size()), os.fill());
		}
		else if(adjustfield == std::ios::internal && (flags & std::ios::showbase) && (flags & std::ios::basefield) == std::ios::hex && v != 0) {
			rep.insert(2, static_cast<size_t>(width - rep.size()), os.fill());
		}
		else {
			rep.insert(0, static_cast<size_t>(width - rep.size()), os.fill());
		}
	}
	return os << rep;
}
		...
		switch(flags & std::ios::adjustfield) {
			...
			case std::ios::internal:
			    if(print_as_decimal && (rep[0] == '+' || rep[0] == '-')) {
				    rep.insert(1, static_cast<size_t>(width - rep.size()), os.fill());
			    }
