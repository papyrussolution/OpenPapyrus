...
// @v11.4.0 (replaced with IsPowerOfTwo() from SLIB) constexpr bool IsPowerOf2(unsigned int x) noexcept { return x != 0 && (x & (x - 1)) == 0; }
...
template <typename T> ABSL_ATTRIBUTE_ALWAYS_INLINE ABSL_INTERNAL_CONSTEXPR_CLZ inline int CountLeadingZeroes(T x) 
{
	...
	return sizeof(T) <= sizeof(uint16_t) ? CountLeadingZeroes16(static_cast<uint16_t>(x)) - (std::numeric_limits<uint16_t>::digits - std::numeric_limits<T>::digits) : 
		(sizeof(T) <= sizeof(uint32_t) ? CountLeadingZeroes32(static_cast<uint32_t>(x)) - (std::numeric_limits<uint32_t>::digits - std::numeric_limits<T>::digits) : 
...
template <class T> ABSL_ATTRIBUTE_ALWAYS_INLINE ABSL_INTERNAL_CONSTEXPR_CTZ inline int CountTrailingZeroes(T x) noexcept 
{
	...
	static_assert(IsPowerOfTwo(std::numeric_limits<T>::digits), "T must have a power-of-2 size");
	static_assert(sizeof(T) <= sizeof(uint64_t), "T too large");
	return x == 0 ? std::numeric_limits<T>::digits : (sizeof(T) <= sizeof(uint16_t) ? CountTrailingZeroesNonzero16(static_cast<uint16_t>(x)) : 
