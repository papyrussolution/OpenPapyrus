...
static inline const char * RemainingInput(State * state) { return &state->mangled_begin[state->parse_state.mangled_idx]; }
...
static bool IdentifierIsAnonymousNamespace(State * state, int length) 
{
	...
	return (length > static_cast<int>(sizeof(anon_prefix) - 1) && StrPrefix(RemainingInput(state), anon_prefix));
}
