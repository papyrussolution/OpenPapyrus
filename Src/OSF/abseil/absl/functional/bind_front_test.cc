...
namespace {
char CharAt(const char* s, size_t index) {
	return s[index];
}
...
struct NonCopyableFunctor {
	...
	const NonCopyableFunctor* operator()() const {
		return this;
	}
};
...
const std::string& GetNonCopyableValueByWrapper(std::reference_wrapper<NonCopyable> n) {
	return n.get().value;
}
...
int GetMember(FalseCopyable<std::unique_ptr<int> > x) {
	return *x.m;
}
...
struct ManglingCall {
	int operator()(int, double, std::string) const {
		return 0;
	}
};
