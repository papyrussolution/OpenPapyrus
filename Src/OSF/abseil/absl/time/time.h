...
template <typename T> Duration operator/(Duration lhs, T rhs) { return lhs /= rhs; }
inline int64_t operator/(Duration lhs, Duration rhs) { return time_internal::IDivDuration(true, lhs, rhs, &lhs); } // trunc towards zero
inline Duration operator%(Duration lhs, Duration rhs) { return lhs %= rhs; }
...
inline int64_t IDivDuration(Duration num, Duration den, Duration* rem) { return time_internal::IDivDuration(true, num, den, rem); } // trunc towards zero
...
inline TimeZone LocalTimeZone() { return TimeZone(time_internal::cctz::local_time_zone()); }
...
inline CivilSecond ToCivilSecond(Time t, TimeZone tz) { return tz.At(t).cs; } // already a CivilSecond
inline CivilMinute ToCivilMinute(Time t, TimeZone tz) { return CivilMinute(tz.At(t).cs); }
inline CivilHour ToCivilHour(Time t, TimeZone tz) { return CivilHour(tz.At(t).cs); }
inline CivilDay ToCivilDay(Time t, TimeZone tz) { return CivilDay(tz.At(t).cs); }
inline CivilMonth ToCivilMonth(Time t, TimeZone tz) { return CivilMonth(tz.At(t).cs); }
inline CivilYear ToCivilYear(Time t, TimeZone tz) { return CivilYear(tz.At(t).cs); }
...
{
	return GetRepHi(d) < 0 ? MakeDuration((std::numeric_limits<int64_t>::max)(), ~0U) : MakeDuration((std::numeric_limits<int64_t>::min)(), ~0U);
}
...
constexpr Duration FromInt64(int64_t v, std::ratio<60>) 
{
	return (v <= (std::numeric_limits<int64_t>::max)() / 60 && v >= (std::numeric_limits<int64_t>::min)() / 60)
	       ? MakeDuration(v * 60) : v > 0 ? InfiniteDuration() : -InfiniteDuration();
}

constexpr Duration FromInt64(int64_t v, std::ratio<3600>) 
{
	return (v <= (std::numeric_limits<int64_t>::max)() / 3600 && v >= (std::numeric_limits<int64_t>::min)() / 3600)
	       ? MakeDuration(v * 3600) : v > 0 ? InfiniteDuration() : -InfiniteDuration();
}
...
constexpr Duration InfiniteDuration() { return time_internal::MakeDuration((std::numeric_limits<int64_t>::max)(), ~0U); }
