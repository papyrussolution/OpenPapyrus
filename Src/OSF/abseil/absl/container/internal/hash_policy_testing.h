...
template <class Derived> struct WithId {
	...
	WithId(WithId&& that) : id_(that.id_) 
	{
		that.id_ = 0;
	}
	...
	friend bool operator==(const WithId& a, const WithId& b) { return a.id_ == b.id_; }
...
struct NonStandardLayout {
	...
	friend bool operator!=(const NonStandardLayout& a, const NonStandardLayout& b) { return a.value != b.value; }
	template <typename H> friend H AbslHashValue(H h, const NonStandardLayout& v) { return H::combine(std::move(h), v.value); }

	std::string value;
};

struct StatefulTestingHash : absl::container_internal::hash_testing_internal::WithId<StatefulTestingHash> {
	template <class T> size_t operator()(const T& t) const { return absl::Hash<T>{} (t); }
};
...
template <class T = int> struct Alloc : std::allocator<T> {
	...
	template <class U> Alloc(const Alloc<U>& that) : std::allocator<T>(that), id_(that.id()) 
	{
	}
	...
	size_t id() const { return id_; }
	friend bool operator==(const Alloc& a, const Alloc& b) { return a.id_ == b.id_; }
