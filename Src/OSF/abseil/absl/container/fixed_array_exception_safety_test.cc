...
using MoveThrowAlloc = testing::ThrowingAllocator<MoveThrower, testing::AllocSpec::kEverythingThrows>;
...
using MoveFixedArrWithAlloc = absl::FixedArray<MoveThrower, kInlined, MoveThrowAlloc>;

TEST(FixedArrayExceptionSafety, CopyConstructor) {
	auto _small = FixedArr(kSmallSize);
	TestThrowingCtor<FixedArr>(_small);
...
TEST(FixedArrayExceptionSafety, CopyConstructorWithAlloc) {
	auto _small = FixedArrWithAlloc(kSmallSize);
	TestThrowingCtor<FixedArrWithAlloc>(_small);
...
TEST(FixedArrayExceptionSafety, IteratorConstructor) {
	auto _small = FixedArr(kSmallSize);
	TestThrowingCtor<FixedArr>(_small.begin(), _small.end());
...
TEST(FixedArrayExceptionSafety, IteratorConstructorWithAlloc) {
	auto _small = FixedArrWithAlloc(kSmallSize);
	TestThrowingCtor<FixedArrWithAlloc>(_small.begin(), _small.end());
...
TEST(FixedArrayExceptionSafety, InitListConstructorWithAlloc) {
	constexpr int small_inlined = 3;
	using SmallFixedArrWithAlloc = absl::FixedArray<Thrower, small_inlined, ThrowAlloc>;
	TestThrowingCtor<SmallFixedArrWithAlloc>(std::initializer_list<Thrower>{});
	// Test inlined allocation
	TestThrowingCtor<SmallFixedArrWithAlloc>(std::initializer_list<Thrower>{Thrower{}, Thrower{}});
	// Test out of line allocation
	TestThrowingCtor<SmallFixedArrWithAlloc>(std::initializer_list<Thrower>{ Thrower{}, Thrower{}, Thrower{}, Thrower{}, Thrower{} });
}

template <typename FixedArrT> testing::AssertionResult ReadMemory(FixedArrT* fixed_arr) 
{
	int sum = 0;
