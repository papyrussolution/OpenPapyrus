...
TEST_P(CordTest, RemoveSuffixWithExternalOrSubstring) {
	absl::Cord cord = absl::MakeCordFromExternal(
		"foo bar baz", [](absl::string_view s) {
			DoNothing(s, nullptr);
		});
...
TEST_P(CordTest, ConstructFromExternalReleaserInvoked) {
	...
	{
		bool invoked = false;
		auto releaser = [&invoked](absl::string_view) {
			    invoked = true;
		    };
		{
...
TEST_P(CordTest, ConstructFromExternalNoArgLambda) {
	bool invoked = false;
	(void)MaybeHardened(
		absl::MakeCordFromExternal("dummy", [&invoked]() {
			invoked = true;
		}));
	EXPECT_TRUE(invoked);
}
...
TEST_P(CordTest, ConstructFromExternalStringViewArgLambda) {
	...
	(void)MaybeHardened(absl::MakeCordFromExternal(
		    "dummy", [&invoked](absl::string_view) {
			invoked = true;
		}));
	EXPECT_TRUE(invoked);
}
...
CordMutator cord_mutators[] = {
	...
	},
	{
		...
		[](absl::Cord& c) {
			c.RemoveSuffix(c.size() / 2);
		}
	},
	{
		"prepend string",
		[](absl::Cord& c) {
			c.Prepend("9876543210");
		},
		[](absl::Cord& c) {
			c.RemovePrefix(10);
		}
	...
	},
	{
		...
		[](absl::Cord& c) {
			c.RemovePrefix(c.size() / 2);
		}
	},
	{"remove prefix", [](absl::Cord& c) {
		 c.RemovePrefix(2);
	 }},
	{"remove suffix", [](absl::Cord& c) {
		 c.RemoveSuffix(2);
	 }},
	{"subcord", [](absl::Cord& c) {
		 c = c.Subcord(1, c.size() - 2);
	 }},
	{
		"swap inline",
		[](absl::Cord& c) {
