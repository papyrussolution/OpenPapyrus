...
public:
	...
	void PrintAsHexLower(T v) {
		...
		do {
			...
			memcpy(p, table + 2 * (static_cast<size_t>(v) & 0xFF), 2);
			if(sizeof(T) == 1) break;
			v >>= 8;
		} while(v);
		if(p[0] == '0') {
	...
	string_view with_neg_and_zero() const {
		return {start_, size_};
	}
...
// ==================== Raw pointers ====================
ArgConvertResult<FormatConversionCharSetInternal::p> FormatConvertImpl(VoidPtr v, const FormatConversionSpecImpl conv,
    FormatSinkImpl * sink) {
	if(!v.value) {
		sink->Append("(nil)");
		return {true};
