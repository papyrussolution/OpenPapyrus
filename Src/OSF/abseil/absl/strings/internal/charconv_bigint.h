...
constexpr int kMaxSmallPowerOfFive = 13; // The largest power that 5 that can be raised to, and still fit in a uint32_t.
constexpr int kMaxSmallPowerOfTen = 9; // The largest power that 10 that can be raised to, and still fit in a uint32_t.
...
public:
	...
	explicit constexpr BigUnsigned(uint64_t v) : size_((v >> 32) ? 2 : v ? 1 : 0), words_{static_cast<uint32_t>(v & 0xffffffffu), static_cast<uint32_t>(v >> 32)} 
	{
	}
