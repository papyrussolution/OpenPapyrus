...
// Returns the maximum number of significant digits we will read for a float in the given base.
...
template <> unsigned ToDigit<10>(char ch) { return ch - '0'; }
template <> unsigned ToDigit<16>(char ch) { return kAsciiToInt[static_cast<unsigned char>(ch)]; }
...
// a NaN.
bool IsNanChar(char v) { return (v == '_') || (v >= '0' && v <= '9') || (v >= 'a' && v <= 'z') || (v >= 'A' && v <= 'Z'); }
