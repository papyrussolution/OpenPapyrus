...
{
	const std::string formats[] = { "", "a", "%80d", "%d %u %c %s %f %g",
#if !defined(_MSC_VER) && !defined(__ANDROID__) && !defined(__native_client__)
		// MSVC, NaCL and Android don't support positional syntax.
	...
	for(const auto& fmt : formats) {
		...
		int fmt_result = snprintf(&*buf.begin(), buf.size(), fmt.c_str(), 123, 3, 49, "multistreaming!!!", 1.01, 1.01);
		ASSERT_TRUE(oss) << fmt;
...
public:
	...
	std::string ReadFile() 
	{
		...
		int read_bytes = static_cast<int>(std::fread(&str[0], 1, str.size(), file_));
...
struct Point {
	friend absl::FormatConvertResult<absl::FormatConversionCharSet::kString | absl::FormatConversionCharSet::kIntegral> AbslFormatConvert(
		const Point& p, const absl::FormatConversionSpec& spec, absl::FormatSink* s) 
	{
...
std::string CodegenAbslStrFormatInt(int i) { return absl::StrFormat("%d", i); } // NOLINT
std::string CodegenAbslStrFormatIntStringInt64(int i, const std::string& s, int64_t i64) { return absl::StrFormat("%d %s %d", i, s, i64); } // NOLINT
void CodegenAbslStrAppendFormatInt(std::string* out, int i) { absl::StrAppendFormat(out, "%d", i); } // NOLINT
void CodegenAbslStrAppendFormatIntStringInt64(std::string* out, int i, const std::string& s, int64_t i64) { absl::StrAppendFormat(out, "%d %s %d", i, s, i64); } // NOLINT
