...
class MutexRelock {
public:
	explicit MutexRelock(absl::Mutex& mu) : mu_(mu) 
	{
		mu_.Unlock();
	}
	~MutexRelock() 
...
public:
	template <typename V>
	FlagState(FlagImpl& flag_impl, const V& v, bool modified, bool on_command_line, int64_t counter) : flag_impl_(flag_impl), value_(v),
		modified_(modified), on_command_line_(on_command_line), counter_(counter) 
	{
	}
	~FlagState() override 
	{
		if(flag_impl_.ValueStorageKind() != FlagValueStorageKind::kAlignedBuffer && flag_impl_.ValueStorageKind() != FlagValueStorageKind::kSequenceLocked)
			return;
	...
	switch(ValueStorageKind()) {
		...
		case FlagValueStorageKind::kOneWordAtomic: {
		    ...
		    if(def_kind == FlagDefaultKind::kGenFunc) {
			    ...
			    memcpy(buf.data(), &default_value_, Sizeof(op_));
		    }
...
void FlagImpl::StoreValue(const void* src) {
	switch(ValueStorageKind()) {
		...
		case FlagValueStorageKind::kOneWordAtomic: {
		    ...
		    memcpy(&one_word_val, src, Sizeof(op_));
...
void FlagImpl::Read(void* dst) const {
	...
	switch(ValueStorageKind()) {
		...
		case FlagValueStorageKind::kOneWordAtomic: {
			...
			OneWordValue().load(std::memory_order_acquire);
		    memcpy(dst, &one_word_val, Sizeof(op_));
		    break;
	    }
		case FlagValueStorageKind::kSequenceLocked: {
