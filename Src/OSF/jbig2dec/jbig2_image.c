...
Jbig2Image * jbig2_image_new(Jbig2Ctx * ctx, uint32 width, uint32 height)
{
	...
	image->data = jbig2_new(ctx, uint8, (size_t)height * stride);
	if(image->data == NULL) {
...
Jbig2Image * jbig2_image_resize(Jbig2Ctx * ctx, Jbig2Image * image, uint32 width, uint32 height, int value)
{
	if(width == image->width) {
		...
		data = jbig2_renew(ctx, image->data, uint8, (size_t)height * image->stride);
		if(!data) {
		...
		if(height > image->height) {
			const uint8 fill = value ? 0xFF : 0x00;
		...
		Jbig2Image * newimage = jbig2_image_new(ctx, width, height);
		if(newimage == NULL) {
    ...
    uint8 leftmask, uint8 rightmask, uint32 bytewidth_, uint32 h, uint32 shift, uint32 dstride, uint32 sstride, Jbig2ComposeOp op)
{
	...
	if(bytewidth == 1) {
		for(j = 0; j < h; j++) {
			/* Only 1 byte! */
			uint8 v = (((early ? 0 : ss[0]<<8) | (late ? 0 : ss[1]))>>shift);
...
static void jbig2_image_compose_opt_OR(const uint8 * s, uint8 * d, int early, int late, uint8 mask, uint8 rightmask, uint32 bytewidth,
    uint32 h, uint32 shift, uint32 dstride, uint32 sstride)
{
	if(early || late)
...
static void jbig2_image_compose_opt_AND(const uint8 * s, uint8 * d, int early, int late, uint8 mask, uint8 rightmask, uint32 bytewidth,
    uint32 h, uint32 shift, uint32 dstride, uint32 sstride)
{
	if(early || late)
...
static void jbig2_image_compose_opt_XOR(const uint8 * s, uint8 * d, int early, int late, uint8 mask, uint8 rightmask, uint32 bytewidth,
    uint32 h, uint32 shift, uint32 dstride, uint32 sstride)
{
	if(early || late)
...
static void jbig2_image_compose_opt_XNOR(const uint8 * s, uint8 * d, int early, int late, uint8 mask, uint8 rightmask, uint32 bytewidth,
    uint32 h, uint32 shift, uint32 dstride, uint32 sstride)
{
	if(early || late)
...
static void jbig2_image_compose_opt_REPLACE(const uint8 * s, uint8 * d, int early, int late, uint8 mask, uint8 rightmask, uint32 bytewidth,
    uint32 h, uint32 shift, uint32 dstride, uint32 sstride)
{
	if(early || late)
...
int jbig2_image_compose(Jbig2Ctx * ctx, Jbig2Image * dst, Jbig2Image * src, int x, int y, Jbig2ComposeOp op)
{
	...
	if(!src)
		return 0;
	if((UINT32_MAX - src->width  < (uint32)(x > 0 ? x : -x)) || (UINT32_MAX - src->height < (uint32)(y > 0 ? y : -y))) {
#ifdef JBIG2_DEBUG
	...
	if((uint32)x + w > dst->width) {
		if(dst->width < (uint32)x)
			w = 0;
		else
			w = dst->width - x;
	}
	if((uint32)y + h > dst->height) {
		if(dst->height < (uint32)y)
			h = 0;
		else
			h = dst->height - y;
	}
	...
	bytewidth = (((uint32)x + w - 1) >> 3) - leftbyte + 1;
	leftmask = 255>>(x&7);
	...
	switch(op) {
		...
		case JBIG2_COMPOSE_REPLACE:
		    jbig2_image_compose_opt_REPLACE(ss, dd, early, late, leftmask, rightmask, bytewidth, h, shift, dst->stride,
			src->stride);
		    break;
	}
...
// returns 0 outside the image frame for the convenience of the template code
// 
