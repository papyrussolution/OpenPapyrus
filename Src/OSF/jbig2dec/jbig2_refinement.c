...
static int jbig2_decode_refinement_template1(Jbig2Ctx * ctx,
    Jbig2Segment * segment,
    const Jbig2RefinementRegionParams * params, Jbig2ArithState * as, Jbig2Image * image, Jbig2ArithCx * GR_stats)
{
	const int GRW = image->width;
	...
	for(y = 0; y < GRH; y++) {
		...
		for(x = 0; x < padded_width; x += 8) {
			...
			for(x_minor = 0; x_minor < minor_width; x_minor++) {
				int bit = jbig2_arith_decode(ctx, as, &GR_stats[CONTEXT]);
				if(bit < 0)
				...
				CONTEXT = ((CONTEXT & 0x0d6) << 1) | bit |
				    ...
				    ((refline_1 >> (9 - x_minor)) & 0x010) | ((refline_0 >> (9 - x_minor)) & 0x040) |
...
static int jbig2_decode_refinement_TPGRON(Jbig2Ctx * ctx, const Jbig2RefinementRegionParams * params, Jbig2ArithState * as,
    Jbig2Image * image, Jbig2ArithCx * GR_stats)
{
	const int GRW = image->width;
	const int GRH = image->height;
	int x, y, iv, LTP = 0;
 ...
 **/
int jbig2_decode_refinement_region(Jbig2Ctx * ctx, Jbig2Segment * segment, const Jbig2RefinementRegionParams * params, Jbig2ArithState * as, Jbig2Image * image, Jbig2ArithCx * GR_stats)
{
	jbig2_error(ctx, JBIG2_SEVERITY_DEBUG, segment->number, "decoding generic refinement region with offset %d,%x, GRTEMPLATE=%d, TPGRON=%d",
 ...
 */
static Jbig2Segment * jbig2_region_find_referred(Jbig2Ctx * ctx, Jbig2Segment * segment)
{
	const int nsegments = segment->referred_to_segment_count;
	for(int index = 0; index < nsegments; index++) {
		Jbig2Segment * rsegment = jbig2_find_segment(ctx, segment->referred_to_segments[index]);
		if(rsegment == NULL) {
...
int jbig2_refinement_region(Jbig2Ctx * ctx, Jbig2Segment * segment, const byte * segment_data)
{
	...
	jbig2_error(ctx, JBIG2_SEVERITY_INFO, segment->number, "segment flags = %02x %s%s", seg_flags, params.GRTEMPLATE ? " GRTEMPLATE" : "", params.TPGRON ? " TPGRON" : "");
	if(seg_flags & 0xFC)
	...
	if(!params.GRTEMPLATE) {
		...
		jbig2_error(ctx, JBIG2_SEVERITY_INFO, segment->number, "grat1: (%d, %d) grat2: (%d, %d)", params.grat[0], params.grat[1], params.grat[2], params.grat[3]);
		offset += 4;
	}
	/* 7.4.7.4 - set up the reference image */
	if(segment->referred_to_segment_count) {
		Jbig2Segment * ref = jbig2_region_find_referred(ctx, segment);
		if(ref == NULL)
	...
	params.GRREFERENCEDY = 0;
	{
		...
		int stats_size;
		Jbig2Image * image = jbig2_image_new(ctx, rsi.width, rsi.height);
		if(image == NULL) {
		...
		if((segment->flags & 63) == 40) {
			...
			jbig2_error(ctx, JBIG2_SEVERITY_DEBUG, segment->number, "composing %dx%d decoded refinement region onto page at (%d, %d)", rsi.width, rsi.height, rsi.x, rsi.y);
