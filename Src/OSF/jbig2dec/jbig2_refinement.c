		...
		for(x = 0; x < GRW; x++) {
			...
			if(bit < 0)
				return jbig2_error(ctx,
					   ...
					   segment->number,
					   "failed to decode arithmetic code when handling refinement template0");
			...
			for(x_minor = 0; x_minor < minor_width; x_minor++) {
				...
				if(bit < 0)
					return jbig2_error(ctx,
						   JBIG2_SEVERITY_WARNING,
						   segment->number,
						   "failed to decode arithmetic code when handling refinement template1");
...
static int jbig2_decode_refinement_TPGRON(Jbig2Ctx * ctx,
	...
	for(y = 0; y < GRH; y++) {
		...
		if(bit < 0)
			return jbig2_error(ctx,
				   ...
				   "failed to decode arithmetic code when handling refinement TPGRON1");
		LTP ^= bit;
		if(!LTP) {
			for(x = 0; x < GRW; x++) {
 ...
 **/
int jbig2_decode_refinement_region(Jbig2Ctx * ctx,
    Jbig2Segment * segment,
    const Jbig2RefinementRegionParams * params, Jbig2ArithState * as, Jbig2Image * image, Jbig2ArithCx * GR_stats)
{
	...
	for(index = 0; index < nsegments; index++) {
		...
		if(rsegment == NULL) {
			jbig2_error(ctx,
			    JBIG2_SEVERITY_WARNING,
			    segment->number,
			    "failed to find referred to segment %d",
