...
void GraphOpSize(LPGW lpgw, UINT op, UINT x, UINT y, LPCSTR str, DWORD size)
{
	...
	if( (p_this==NULL) || (p_this->used >= GWOPMAX) ) {
...
static bool TryCreateFont(LPGW lpgw, LPTSTR fontface, HDC hdc)
{
	if(fontface != NULL)
...
void GraphEnhancedOpen(char * fontname, double fontsize, double base, bool widthflag, bool showflag, int overprint)
{
...
static void draw_grey_out_key_box(LPGW lpgw, HDC hdc, int plotno)
{
	...
	ftn.BlendFlags = 0;
	AlphaBlend(hdc, bb->left, bb->bottom, width, height, memdc, 0, 0, width, height, ftn);
...
static void draw_image(LPGW lpgw, HDC hdc, char * image, POINT corners[4], uint width, uint height, int color_mode)
{
	...
	if(color_mode != IC_RGBA) {
		...
		StretchDIBits(hdc, MIN(corners[0].x, corners[1].x), MIN(corners[0].y, corners[1].y),
		    ...
		    0, 0, width, height, dibimage, &bmi, DIB_RGB_COLORS, SRCCOPY);
		if(!lpgw->color) 
			SAlloc::F(dibimage);
	}
	else {
			...
			switch(curptr->op) {
				...
				case W_filled_polygon_draw: {
				    ...
				    if(!transparent) {
					    ...
					    COLORREF bkColor = RGB((transparentColor >> 16) & 0xff,
...
LRESULT CALLBACK WndGraphProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message) {
		...
		case WM_KEYUP:
	    {
		    ...
		    modifier_mask = ((GetKeyState(VK_SHIFT) < 0) ? Mod_Shift : 0 ) | ((GetKeyState(VK_CONTROL) < 0) ? Mod_Ctrl : 0) | ((GetKeyState(VK_MENU) < 0) ? Mod_Alt : 0);
 ...
 */
void Graph_put_tmptext(LPGW lpgw, int where, LPCSTR text)
{
 ...
 * Toggle active plots
 */
void GraphModifyPlots(LPGW lpgw, uint ops, int plotno)
{
	int i;
	bool changed = FALSE;
