...
double ** matr(int rows, int cols)
{
	double ** m = 0;
	if(rows > 0 && cols > 0) {
		m = (double **)gp_alloc(rows * sizeof(m[0]), "matrix row pointers");
		m[0] = (double *)gp_alloc(rows * cols * sizeof(m[0][0]), "matrix elements");
...
double * redim_vec(double ** v, int n)
{
	if(n < 1)
		...
		*v = (double *)gp_realloc(*v, n * sizeof((*v)[0]), "vec");
	return *v;
}
		...
		for(i = j + 1; i < N; i++) {
			if(C[i][j]) {
				if(fabs(C[j][j]) < epsilon * fabs(C[i][j])) {
					...
					w = fsignz(C[j][j]) * sqrt(C[j][j] * C[j][j] + C[i][j] * C[i][j]);
					if(w == 0)
