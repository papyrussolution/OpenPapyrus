...
lp_style_type default_pm3d_border(lp_style_type::defCommon); // = DEFAULT_LP_STYLE_TYPE;
/* Set by plot styles that use pm3d quadrangles even in non-pm3d mode */
bool track_pm3d_quadrangles;
/*
			...
			if((interp_i <= 1 && interp_j <= 1) || pm3d.direction == PM3D_DEPTH) {
				{
					...
					if(pm3d_shade.strength > 0) {
						if(at_which_z == PM3D_AT_SURFACE) {
							bool gray_is_rgb = color_from_rgbvar;
							if(private_colormap) {
...
static void filled_polygon(gpdPoint * corners, int fillstyle, int nv)
{
	...
	if(nv > max_vertices) {
		max_vertices = nv;
		icorners = (gpiPoint *)gp_realloc(icorners, (2*max_vertices) * sizeof(gpiPoint), "filled_polygon");
		ocorners = (gpiPoint *)gp_realloc(ocorners, (2*max_vertices) * sizeof(gpiPoint), "filled_polygon");
...
static gpdPoint * get_polygon(int size)
{
	...
	if(next_polygon + size >= polygonlistsize) {
		...
		polygonlist = (gpdPoint *)gp_realloc(polygonlist, polygonlistsize * sizeof(gpdPoint), NULL);
	}
