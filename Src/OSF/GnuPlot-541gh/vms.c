...
#pragma hdrstop
#if 0 // @sobolev {
#ifndef VWS_XMAX // these are needed to modify terminal characteristics 
...
static void create_mbx(ushort int * chan, struct dsc$descriptor_s * namdsc)
{
	static ulong mbxbufsiz;
...
static void popen_completion_ast(ulong unused)
{
	if(waitpid_asleep) {
		waitpid_asleep = 0;
		sys$wake(0, 0);
...
FILE * popen(char * cmd, char * mode)
{
	...
	ushort chan;
	ulong flags = 1; /* nowait - gnu c doesn't allow &1 */
...
int pclose(FILE * fp)
{
	...
	ulong abort = SS$_TIMEOUT, retsts;
	unsigned long sts;
...
/*{{{ulong waitpid(ulong pid, int *statusp, int flags)*/
ulong waitpid(ulong pid, int * statusp, int flags)
{
	struct pipe_details * info;
	ulong abort = SS$_TIMEOUT;
	ulong sts;
	...
	if(info != NULL) { /* we know about this child */
		...
		*statusp = info->completion;
		...
		ulong ownercode = JPI$_OWNER, ownerpid, mypid;
		ulong interval[2];
