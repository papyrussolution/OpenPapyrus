...
static bool setting_zoom_region = FALSE;
...
static void MousePosToGraphPosReal(int xx, int yy, double * x, double * y, double * x2, double * y2)
{
	...
	if(is_3d_plot) {
		...
		if(abs(axis3d_x_dx) > abs(axis3d_x_dy)) {
			*x = GPO.AxS[FIRST_X_AXIS].min + ((double)xx) / axis3d_x_dx * (GPO.AxS[FIRST_X_AXIS].max - GPO.AxS[FIRST_X_AXIS].min);
		}
		else if(axis3d_x_dy != 0) {
			*x = GPO.AxS[FIRST_X_AXIS].min + ((double)yy) / axis3d_x_dy * (GPO.AxS[FIRST_X_AXIS].max - GPO.AxS[FIRST_X_AXIS].min);
		}
		else {
		...
		if(abs(axis3d_y_dx) > abs(axis3d_y_dy)) {
			*y = GPO.AxS[FIRST_Y_AXIS].min + ((double)xx) / axis3d_y_dx * (GPO.AxS[FIRST_Y_AXIS].max - GPO.AxS[FIRST_Y_AXIS].min);
		}
		else if(axis3d_y_dy != 0) {
			if(splot_map)
				*y = GPO.AxS[FIRST_Y_AXIS].max + ((double)yy) / axis3d_y_dy * (GPO.AxS[FIRST_Y_AXIS].min - GPO.AxS[FIRST_Y_AXIS].max);
			else
				*y = GPO.AxS[FIRST_Y_AXIS].min + ((double)yy) / axis3d_y_dy * (GPO.AxS[FIRST_Y_AXIS].max - GPO.AxS[FIRST_Y_AXIS].min);
		}
		else {
	...
	if(plot_bounds.ytop == plot_bounds.ybot)
		...
		*y2 = GPO.AxS[SECOND_Y_AXIS].MapBack(yy);
	}
...
static char * GetAnnotateString(char * s, double x, double y, int mode, char * fmt)
{
	if(GPO.AxS[FIRST_X_AXIS].datatype == DT_DMS || GPO.AxS[FIRST_Y_AXIS].datatype == DT_DMS) {
		static char dms_format[16];
		sprintf(dms_format, "%%D%s%%.2m'", degree_sign);
		if(GPO.AxS[FIRST_X_AXIS].datatype == DT_DMS)
		...
		double xrange = GPO.AxS[FIRST_X_AXIS].max - GPO.AxS[FIRST_X_AXIS].min;
		double yrange = GPO.AxS[FIRST_Y_AXIS].max - GPO.AxS[FIRST_Y_AXIS].min;
		/* calculate fractional coordinates.
		 * prevent division by zero */
		if(xrange) {
			char format[0xff] = "/";
			strcat(format, mouse_setting.fmt);
			sprintf(s, format, (x - GPO.AxS[FIRST_X_AXIS].min) / xrange);
		}
		else {
			sprintf(s, "/(undefined)");
		}
		...
		if(yrange) {
			...
			strcat(format, mouse_setting.fmt);
			strcat(format, "/");
			sprintf(s, format, (y - GPO.AxS[FIRST_Y_AXIS].min) / yrange);
		}
		else {
			sprintf(s, ", (undefined)/");
		}
		...
		double rmin = (GPO.AxS.__R().autoscale & AUTOSCALE_MIN) ? 0.0 : GPO.AxS.__R().set_min;
		...
		if(nonlinear(&GPO.AxS.__R()))
			r = GPO.EvalLinkFunction(&GPO.AxS.__R(), x/cos(phi) + GPO.AxS.__R().linked_to_primary->min);
		else if(GPO.AxS.__R().log)
			r = rmin + x/cos(phi);
		...
		GpValue readout;
		udvt_entry * plot_x = add_udv_by_name("x");
		udvt_entry * plot_y = add_udv_by_name("y");
		...
		GPO.EvaluateAt(mouse_readout_function.at, &readout);
		plot_x->udv_value = original_x;
		plot_y->udv_value = original_y;
		if(readout.type != STRING) {
			GPO.IntWarn(NO_CARET, "mouseformat function did not return a string");
		}
		else {
...
static void apply_zoom(struct t_zoom * z)
{
	...
	if(zoom_now) { // remember the current zoom 
		zoom_now->xmin = GPO.AxS[FIRST_X_AXIS].set_min;
		zoom_now->xmax = GPO.AxS[FIRST_X_AXIS].set_max;
		zoom_now->x2min = GPO.AxS[SECOND_X_AXIS].set_min;
		zoom_now->x2max = GPO.AxS[SECOND_X_AXIS].set_max;
		zoom_now->ymin = GPO.AxS[FIRST_Y_AXIS].set_min;
		zoom_now->ymax = GPO.AxS[FIRST_Y_AXIS].set_max;
		zoom_now->y2min = GPO.AxS[SECOND_Y_AXIS].set_min;
		zoom_now->y2max = GPO.AxS[SECOND_Y_AXIS].set_max;
	}
	...
	if(zoom_now == zoom_head && z != zoom_head) {
		GPO.AxS[FIRST_X_AXIS].autoscale = AUTOSCALE_NONE;
		GPO.AxS[FIRST_Y_AXIS].autoscale = AUTOSCALE_NONE;
		GPO.AxS[SECOND_X_AXIS].autoscale = AUTOSCALE_NONE;
		GPO.AxS[SECOND_Y_AXIS].autoscale = AUTOSCALE_NONE;
	}
	zoom_now = z;
	if(zoom_now == NULL) {
		alert();
	...
	set_explicit_range(&GPO.AxS[FIRST_X_AXIS], zoom_now->xmin, zoom_now->xmax);
	set_explicit_range(&GPO.AxS[FIRST_Y_AXIS], zoom_now->ymin, zoom_now->ymax);
	...
	if(!is_3d_plot && (zoom_now->x2min < VERYLARGE && zoom_now->x2max > -VERYLARGE)) {
		set_explicit_range(&GPO.AxS[SECOND_X_AXIS], zoom_now->x2min, zoom_now->x2max);
	}
	if(!is_3d_plot && (zoom_now->y2min < VERYLARGE && zoom_now->y2max > -VERYLARGE)) {
		set_explicit_range(&GPO.AxS[SECOND_Y_AXIS], zoom_now->y2min, zoom_now->y2max);
	}
	...
	if(zoom_now == zoom_head) {
		for(int i = 0; i < AXIS_ARRAY_SIZE; i++) {
			axis_array_copy[i].label = GPO.AxS[i].label;
			axis_array_copy[i].ticdef.def.user = GPO.AxS[i].ticdef.def.user;
			axis_array_copy[i].ticdef.font = GPO.AxS[i].ticdef.font;
			axis_array_copy[i].ticfmt = GPO.AxS[i].ticfmt;
			axis_array_copy[i].formatstring = GPO.AxS[i].formatstring;
			GPO.AxS[i] = axis_array_copy[i]; // @sobolev
		}
		...
		if(nonlinear(&GPO.AxS[FIRST_X_AXIS]))
			clone_linked_axes(&GPO.AxS[FIRST_X_AXIS], GPO.AxS[FIRST_X_AXIS].linked_to_primary);
		if(nonlinear(&GPO.AxS[FIRST_Y_AXIS]))
			clone_linked_axes(&GPO.AxS[FIRST_Y_AXIS], GPO.AxS[FIRST_Y_AXIS].linked_to_primary);
...
#define SET_AXIS(axis, name, minmax, condition)                         \
	z->name ## minmax = (GPO.AxS[axis].minmax condition) ? name ## minmax : GPO.AxS[axis].minmax
...
/* This suppresses mouse coordinate update after a ^C */
	...
	if(!ALMOST2D) {
	...
	else if(!TICS_ON(GPO.AxS[SECOND_X_AXIS].ticmode) && !TICS_ON(GPO.AxS[SECOND_Y_AXIS].ticmode)) {
		...
		sp = s0;
		if(TICS_ON(GPO.AxS[FIRST_X_AXIS].ticmode)) {
			sp = stpcpy(sp, "x=");
			sp = mkstr(sp, real_x, FIRST_X_AXIS);
			*sp++ = ' ';
		}
		if(TICS_ON(GPO.AxS[FIRST_Y_AXIS].ticmode)) {
			sp = stpcpy(sp, "y=");
			sp = mkstr(sp, real_y, FIRST_Y_AXIS);
			*sp++ = ' ';
		}
		if(TICS_ON(GPO.AxS[SECOND_X_AXIS].ticmode)) {
			sp = stpcpy(sp, "x2=");
			sp = mkstr(sp, real_x2, SECOND_X_AXIS);
			*sp++ = ' ';
		}
		if(TICS_ON(GPO.AxS[SECOND_Y_AXIS].ticmode)) {
			sp = stpcpy(sp, "y2=");
...
static char * builtin_toggle_log(struct gp_event_t * ge)
{
	...
	if(volatile_data)
		...
		do_string_replot(GPO.AxS.__CB().log ? "unset log cb" : "set log cb");
	else if(is_3d_plot && !splot_map)
		do_string_replot(GPO.AxS.__Z().log ? "unset log z" : "set log z");
	else
		do_string_replot(GPO.AxS[FIRST_Y_AXIS].log ? "unset log y" : "set log y");
 ...
 */
static double rescale(int AXIS, double w1, double w2)
{
	double newlimit;
	GpAxis * axis = &GPO.AxS[AXIS];
	double axmin = axis->min;
	double axmax = axis->max;
	if(nonlinear(axis)) {
		axmin = GPO.EvalLinkFunction(axis->linked_to_primary, axmin);
		axmax = GPO.EvalLinkFunction(axis->linked_to_primary, axmax);
	}
	newlimit = w1*axmin + w2*axmax;
	if(nonlinear(axis))
		newlimit = GPO.EvalLinkFunction(axis->linked_to_primary->linked_to_secondary, newlimit);
	return newlimit;
}
 ...
 */
static void rescale_around_mouse(double * newmin, double * newmax, int AXIS, double mouse_pos, double scale)
{
	GpAxis * axis = &GPO.AxS[AXIS];
	...
	double axmax = axis->max;
	if(nonlinear(axis)) {
		axmin = GPO.EvalLinkFunction(primary, axmin);
		axmax = GPO.EvalLinkFunction(primary, axmax);
		mouse_pos = GPO.EvalLinkFunction(primary, mouse_pos);
	}
	*newmin = mouse_pos + (axmin - mouse_pos) * scale;
	*newmax = mouse_pos + (axmax - mouse_pos) * scale;
	if(nonlinear(axis)) {
		*newmin = GPO.EvalLinkFunction(primary->linked_to_secondary, *newmin);
		*newmax = GPO.EvalLinkFunction(primary->linked_to_secondary, *newmax);
	}
}
...
static void zoom_in_X(int zoom_key)
{
	...
	if(is_mouse_outside_plot()) {
		...
		zoom_rescale_xyx2y2(w1, w2, 1, 0, w1, w2, 1, 0,  w2, w1, 0, 1, w2, w1, 0, 1, (zoom_key=='+' ? "zoom in X" : "zoom out X"));
	}
	else {
...
static void do_zoom_in_X() { zoom_in_X('+'); }
static void do_zoom_out_X() { zoom_in_X('-'); }
		...
		if(!setting_zoom_region) {
			if(3 == b &&
				...
				if(term->set_cursor) {
					...
					mv_mouse_y =
					    (term->xmax == term->ymax) ? mv_mouse_x : (int)((mv_mouse_x * (double)term->ymax) / term->xmax);
					mv_mouse_x += mouse_x;
					mv_mouse_y += mouse_y;
...
#define rev(a1, a2, A) if(sgn(a2-a1) != sgn(GPO.AxS[A].max-GPO.AxS[A].min)) \
...
void event_plotdone()
{
	if(needreplot) {
		needreplot = FALSE;
		do_save_3dplot(first_3dplot, plot3d_num, ((modifier_mask & Mod_Ctrl) != 0) ? AXIS_ONLY_ROTATE : NORMAL_REPLOT);
	}
	else
		allowmotion = TRUE;
}
...
static void do_save_3dplot(surface_points * plots, int pcount, REPLOT_TYPE quick)
{
	if(!plots || (E_REFRESH_NOT_OK == refresh_ok)) {
		...
		do_3dplot(term, plots, pcount, quick);
	}
}
...
static void recalc_ruler_pos()
{
	...
	if(is_3d_plot) {
		...
		GPO.Map3D_XY(ruler.x, ruler.y, dummy, &ppx, &ppy);
		ruler.px = ppx;
		ruler.py = ppy;
		return;
	}
	// It is 2D plot. 
	if(GPO.AxS[FIRST_X_AXIS].log && ruler.x < 0)
		ruler.px = -1;
	else {
		P = ruler.x;
		ruler.px = GPO.AxS[FIRST_X_AXIS].MapI(P);
	}
	if(GPO.AxS[FIRST_Y_AXIS].log && ruler.y < 0)
		ruler.py = -1;
	else {
		P = ruler.y;
		ruler.py = GPO.AxS[FIRST_Y_AXIS].MapI(P);
	}
