...
//int lookup_table(const struct gen_table * tbl, int find_token)
int FASTCALL GpProgram::LookupTableForCurrentToken(const struct gen_table * pTbl) const
{
	while(pTbl->key) {
		if(AlmostEquals(CToken, pTbl->key))
			return pTbl->value;
		pTbl++;
	}
