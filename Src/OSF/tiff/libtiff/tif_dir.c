...
static int _TIFFVSetField(TIFF* tif, uint32 tag, va_list ap)
{
	...
	switch(standard_tag) {
		case TIFFTAG_SUBFILETYPE: td->td_subfiletype = (uint32)va_arg(ap, uint32); break;
		case TIFFTAG_IMAGEWIDTH: td->td_imagewidth = (uint32)va_arg(ap, uint32); break;
		case TIFFTAG_IMAGELENGTH: td->td_imagelength = (uint32)va_arg(ap, uint32); break;
		...
		case TIFFTAG_COMPRESSION:
		    ...
		    if( (status = TIFFSetCompressionScheme(tif, v)) != 0)
			    ...
			    status = 0;
		    break;
		case TIFFTAG_PHOTOMETRIC: td->td_photometric = (uint16)va_arg(ap, uint16_vap); break;
		case TIFFTAG_THRESHHOLDING: td->td_threshholding = (uint16)va_arg(ap, uint16_vap); break;
		...
		case TIFFTAG_SAMPLESPERPIXEL:
		    ...
		    if(v != td->td_samplesperpixel) {
			    ...
			    if(td->td_sminsamplevalue != NULL) {
				    TIFFWarningExt(tif->tif_clientdata, module, "SamplesPerPixel tag value is changing, but SMinSampleValue tag was read with a different value. Cancelling it");
				    TIFFClrFieldBit(tif, FIELD_SMINSAMPLEVALUE);
				    SAlloc::F(td->td_sminsamplevalue);
				    td->td_sminsamplevalue = NULL;
			    }
			    if(td->td_smaxsamplevalue != NULL) {
				    TIFFWarningExt(tif->tif_clientdata, module, "SamplesPerPixel tag value is changing, but SMaxSampleValue tag was read with a different value. Cancelling it");
				    TIFFClrFieldBit(tif, FIELD_SMAXSAMPLEVALUE);
				    SAlloc::F(td->td_smaxsamplevalue);
		...
		case TIFFTAG_ROWSPERSTRIP:
		    ...
		    break;
		case TIFFTAG_MINSAMPLEVALUE: td->td_minsamplevalue = (uint16)va_arg(ap, uint16_vap); break;
		case TIFFTAG_MAXSAMPLEVALUE: td->td_maxsamplevalue = (uint16)va_arg(ap, uint16_vap); break;
		...
		case TIFFTAG_TILEWIDTH:
		    ...
		    if(v32 % 16) {
			    if(tif->tif_mode != O_RDONLY)
				    goto badvalue32;
			    TIFFWarningExt(tif->tif_clientdata, tif->tif_name, "Nonstandard tile width %d, convert file", v32);
		    }
		    td->td_tilewidth = v32;
		...
		case TIFFTAG_TILELENGTH:
		    ...
		    if(v32 % 16) {
			    if(tif->tif_mode != O_RDONLY)
				    goto badvalue32;
			    TIFFWarningExt(tif->tif_clientdata, tif->tif_name, "Nonstandard tile length %d, convert file", v32);
		    }
		    td->td_tilelength = v32;
		    tif->tif_flags |= TIFF_ISTILED;
		    break;
		...
		case TIFFTAG_TRANSFERFUNCTION:
		    ...
		    for(i = 0; i < v; i++)
			    _TIFFsetShortArray(&td->td_transferfunction[i], va_arg(ap, uint16*), 1U<<td->td_bitspersample);
		    break;
		...
		default: {
		    ...
		    if(fip->field_type == TIFF_ASCII) {
			    ...
			    if(fip->field_passcount)
				    tv->count = (fip->field_writecount == TIFF_VARIABLE2) ? (uint32)va_arg(ap, uint32) : (int)va_arg(ap, int);
			    else if(oneof2(fip->field_writecount, TIFF_VARIABLE, TIFF_VARIABLE2))
				    tv->count = 1;
			    ...
			    if(tv->count == 0) {
				    status = 0;
				    TIFFErrorExt(tif->tif_clientdata, module, "%s: Null count for \"%s\" (type %d, writecount %d, passcount %d)", 
						tif->tif_name, fip->field_name, fip->field_type, fip->field_writecount, fip->field_passcount);
				    goto end;
			    }
			    ...
			    if(fip->field_tag == TIFFTAG_DOTRANGE && sstreq(fip->field_name, "DotRange")) {
				    ...
				    memcpy(tv->value, &v2, 4);
			    }
			    else if(fip->field_passcount || oneof3(fip->field_writecount, TIFF_VARIABLE, TIFF_VARIABLE2, TIFF_SPP) || tv->count > 1) {
...
static int _TIFFVGetField(TIFF* tif, uint32 tag, va_list ap)
{
	...
	switch(standard_tag) {
		...
		case TIFFTAG_THRESHHOLDING: *va_arg(ap, uint16*) = td->td_threshholding; break;
		...
		case TIFFTAG_SMINSAMPLEVALUE:
		    if(tif->tif_flags & TIFF_PERSAMPLE)
			    *va_arg(ap, double**) = td->td_sminsamplevalue;
			    ...
			    for(uint16 i = 1; i < td->td_samplesperpixel; ++i)
		...
		case TIFFTAG_SMAXSAMPLEVALUE:
		    ...
		    break;
		case TIFFTAG_XRESOLUTION: *va_arg(ap, float*) = td->td_xresolution; break;
		case TIFFTAG_YRESOLUTION: *va_arg(ap, float*) = td->td_yresolution; break;
		case TIFFTAG_PLANARCONFIG: *va_arg(ap, uint16*) = td->td_planarconfig; break;
		case TIFFTAG_XPOSITION: *va_arg(ap, float*) = td->td_xposition; break;
		case TIFFTAG_YPOSITION: *va_arg(ap, float*) = td->td_yposition; break;
		case TIFFTAG_RESOLUTIONUNIT: *va_arg(ap, uint16*) = td->td_resolutionunit; break;
		case TIFFTAG_PAGENUMBER:
		...
		case TIFFTAG_DATATYPE:
		    switch(td->td_sampleformat) {
			    ...
			    case SAMPLEFORMAT_INT: *va_arg(ap, uint16*) = DATATYPE_INT; break;
			    case SAMPLEFORMAT_IEEEFP: *va_arg(ap, uint16*) = DATATYPE_IEEEFP; break;
			    case SAMPLEFORMAT_VOID: *va_arg(ap, uint16*) = DATATYPE_VOID; break;
		    }
		    break;
		...
		case TIFFTAG_SUBIFD:
		    ...
		    *va_arg(ap, uint64**) = td->td_subifd;
		    break;
		case TIFFTAG_YCBCRPOSITIONING: *va_arg(ap, uint16*) = td->td_ycbcrpositioning; break;
		case TIFFTAG_YCBCRSUBSAMPLING:
		...
		default:
	    {
		    ...
		    for(i = 0; i < td->td_customValueCount; i++) {
			    ...
			    if(fip->field_passcount) {
			    ...
			    else if(fip->field_tag == TIFFTAG_DOTRANGE && sstreq(fip->field_name, "DotRange")) {
				    /* TODO: This is an evil exception and should not have been
				       handled this way ... likely best if we move it into
				       the directory structure with an explicit field in
				       libtiff 4.1 and assign it a FIELD_ value */
				    ...
				    *va_arg(ap, uint16*) = ((uint16 *)tv->value)[1];
				    ...
				    if(fip->field_type == TIFF_ASCII || oneof3(fip->field_readcount, TIFF_VARIABLE, TIFF_VARIABLE2, TIFF_SPP) || tv->count > 1) {
					    *va_arg(ap, void **) = tv->value;
					    ret_val = 1;
				    }
				    else {
...
#define CleanupField(member) { SAlloc::F(td->member); td->member = 0; }
/*
...
void TIFFFreeDirectory(TIFF* tif)
{
	TIFFDirectory * td = &tif->tif_dir;
	int i;
	memzero(td->td_fieldsset, FIELD_SETLONGS);
	...
	for(i = 0; i < td->td_customValueCount; i++) {
		SAlloc::F(td->td_customValues[i].value);
	}
...
int TIFFDefaultDirectory(TIFF* tif)
{
	...
	if(tif->tif_nfieldscompat > 0) {
		for(uint32 i = 0; i < tif->tif_nfieldscompat; i++) {
...
static int TIFFAdvanceDirectory(TIFF* tif, uint64* nextdir, uint64* off)
{
	...
	if(isMapped(tif)) {
		...
		if(!(tif->tif_flags&TIFF_BIGTIFF)) {
			...
			memcpy(&dircount, tif->tif_base+poffa, sizeof(uint16));
			if(tif->tif_flags&TIFF_SWAB)
			...
			*nextdir = nextdir32;
			...
			memcpy(&dircount64, tif->tif_base+poffa, sizeof(uint64));
			if(tif->tif_flags&TIFF_SWAB)
...
uint16 TIFFNumberOfDirectories(TIFF* tif)
{
	...
	while(nextdir != 0 && TIFFAdvanceDirectory(tif, &nextdir, NULL)) {
		if(n != 65535) {
			...
			TIFFErrorExt(tif->tif_clientdata, module, "Directory count exceeded 65535 limit, giving up on counting.");
			return (65535);
		}
	}
	return (n);
