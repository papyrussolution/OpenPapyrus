...
int TIFFCheckTile(TIFF* tif, uint32 x, uint32 y, uint32 z, uint16 s)
{
	...
	if(x >= td->td_imagewidth) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name, "%lu: Col out of range, max %lu", (ulong)x, (ulong)(td->td_imagewidth - 1));
		return 0;
	}
	if(y >= td->td_imagelength) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name, "%lu: Row out of range, max %lu", (ulong)y, (ulong)(td->td_imagelength - 1));
		return 0;
	}
	if(z >= td->td_imagedepth) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name, "%lu: Depth out of range, max %lu", (ulong)z, (ulong)(td->td_imagedepth - 1));
		return 0;
	}
	if(td->td_planarconfig == PLANARCONFIG_SEPARATE && s >= td->td_samplesperpixel) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name, "%lu: Sample out of range, max %lu", (ulong)s, (ulong)(td->td_samplesperpixel - 1));
		return 0;
	}
	return 1;
}
...
uint32 TIFFNumberOfTiles(TIFF* tif)
{
	...
	if(td->td_planarconfig == PLANARCONFIG_SEPARATE)
		ntiles = _TIFFMultiply32(tif, ntiles, td->td_samplesperpixel, "TIFFNumberOfTiles");
	return (ntiles);
}
...
uint64 TIFFTileRowSize64(TIFF* tif)
{
	...
	if(td->td_planarconfig == PLANARCONFIG_CONTIG) {
		...
		rowsize = _TIFFMultiply64(tif, rowsize, td->td_samplesperpixel, "TIFFTileRowSize");
	}
...
uint64 TIFFVTileSize64(TIFF* tif, uint32 nrows)
{
	...
	if((td->td_planarconfig==PLANARCONFIG_CONTIG)&& (td->td_photometric==PHOTOMETRIC_YCBCR)&& (td->td_samplesperpixel==3)&& (!isUpSampled(tif))) {
		...
		if((ycbcrsubsampling[0] != 1 && ycbcrsubsampling[0] != 2 && ycbcrsubsampling[0] != 4)
			...
			uint16 samplingblock_samples = ycbcrsubsampling[0]*ycbcrsubsampling[1]+2;
			uint32 samplingblocks_hor = TIFFhowmany_32(td->td_tilewidth, ycbcrsubsampling[0]);
			uint32 samplingblocks_ver = TIFFhowmany_32(nrows, ycbcrsubsampling[1]);
			uint64 samplingrow_samples = _TIFFMultiply64(tif, samplingblocks_hor, samplingblock_samples, module);
			uint64 samplingrow_size = TIFFhowmany8_64(_TIFFMultiply64(tif, samplingrow_samples, td->td_bitspersample, module));
