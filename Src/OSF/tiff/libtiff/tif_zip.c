...
static int ZIPDecode(TIFF* tif, uint8 * op, tmsize_t occ, uint16 s)
{
	...
	do {
		...
		if(state == Z_DATA_ERROR) {
			TIFFErrorExt(tif->tif_clientdata, module, "Decoding error at scanline %lu, %s", (ulong)tif->tif_row, SAFE_MSG(sp));
...
int TIFFInitZIP(TIFF* tif, int scheme)
{
	 ...
	 */
	tif->tif_data = static_cast<uint8 *>(SAlloc::M(sizeof(ZIPState)));
