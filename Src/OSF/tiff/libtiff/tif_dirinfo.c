...
static const TIFFField tiffFields[] = {
	...
	{ TIFFTAG_SUBIFD, -1, -1, TIFF_IFD8, 0, TIFF_SETGET_C16_IFD8, TIFF_SETGET_UNDEFINED, FIELD_SUBIFD, 1, 1, "SubIFD", (TIFFFieldArray*)/*&tiffFieldArray*/_GetTiffFieldArray },
	...
	{ TIFFTAG_XMLPACKET, -3, -3, TIFF_BYTE, 0, TIFF_SETGET_C32_UINT8, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 1, "XMLPacket", NULL }, /* begin SGI tags */
	...
	{ TIFFTAG_EXIFIFD, 1, 1, TIFF_IFD8, 0, TIFF_SETGET_IFD8, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 0, "EXIFIFDOffset", (TIFFFieldArray*)/*&exifFieldArray*/_GetExifFieldArray() },
	...
	{ TIFFTAG_GLOBALPARAMETERSIFD, 1, 1, TIFF_IFD8, 0, TIFF_SETGET_IFD8, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 0,
	  "GlobalParametersIFD", NULL },
	...
	{ TIFFTAG_IMAGEBASECOLOR, -1, -1, TIFF_SHORT, 0, TIFF_SETGET_C16_UINT16, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 1,
	  "ImageBaseColor", NULL },
	{ TIFFTAG_T82OPTIONS, 1, 1, TIFF_LONG, 0, TIFF_SETGET_UINT32, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 0, "T82Options", NULL },
	{ TIFFTAG_STRIPROWCOUNTS, -1, -1, TIFF_LONG, 0, TIFF_SETGET_C16_UINT32, TIFF_SETGET_UNDEFINED, FIELD_CUSTOM, 0, 1, "StripRowCounts",
	  NULL },
...
int _TIFFMergeFields(TIFF * tif, const TIFFField info[], uint32 n)
{
	...
	if(tif->tif_fields && tif->tif_nfields > 0) {
		...
		tif->tif_fields = static_cast<TIFFField **>(_TIFFCheckMalloc(tif, n, sizeof(TIFFField *), reason));
	}
	if(!tif->tif_fields) {
...
uint32 TIFFFieldTag(const TIFFField* fip) { return fip->field_tag; }
...
{
	const TIFFField * fld = TIFFFindField(tif, tag, dt);
	if(fld == NULL) {
...
{
	(void)tif;
	TIFFField * fld = static_cast<TIFFField *>(SAlloc::M(sizeof(TIFFField)));
	if(fld == NULL)
		return NULL;
	memzero(fld, sizeof(TIFFField));
	fld->field_tag = tag;
...
int TIFFMergeFieldInfo(TIFF * tif, const TIFFFieldInfo info[], uint32 n)
{
	...
	if(tif->tif_nfieldscompat > 0) {
		...
		tif->tif_fieldscompat = static_cast<TIFFFieldArray *>(_TIFFCheckMalloc(tif, 1, sizeof(TIFFFieldArray), reason));
	}
	...
	tif->tif_fieldscompat[nfields].fields = static_cast<TIFFField *>(_TIFFCheckMalloc(tif, n, sizeof(TIFFField), reason));
