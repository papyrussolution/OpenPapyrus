...
public:
    ...
    Vec64cb(bool b) {
        m64 = __mmask64(-int64_t(b));    
    }
    // Constructor to make from two halves
    Vec64cb (Vec32cb const & x0, Vec32cb const & x1) {
        __m512i z = _mm512_inserti64x4(_mm512_castsi256_si512(x0), x1, 1);
        m64 = _mm512_cmpneq_epi8_mask(z, _mm512_setzero_epi32());
        //m64 = __mmask64((uint64_t)to_bits(x0) | ((uint64_t)to_bits(x1) << 32));
    }
    ...
    Vec32cb get_low() const {
        return Vec32cb(Vec32c(_mm512_castsi512_si256(_mm512_maskz_set1_epi8(m64, -1))));
    }
    Vec32cb get_high() const {
        return Vec64cb(m64 >> 32).get_low();
    } 
    // Member function to change a single element in vector
    Vec64cb & insert (uint32_t index, bool a) {
        uint64_t mask = uint64_t(1) << index;
        m64 = (m64 & ~mask) | uint64_t(a) << index;
        return *this;
    }    
    // Member function extract a single element from vector
    bool extract(int index) const { 
        return ((m64 >> index) & 1) != 0;
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
...
static inline int horizontal_find_first(Vec64cb const & x) {
    uint64_t b = uint64_t(__mmask64(x));
    if (b) {
...
static inline uint32_t horizontal_count(Vec64cb const & x) {
    return (uint32_t)vml_popcnt(uint64_t(__mmask64(x)));
}
...
public:
    ...
    Vec32s(int16_t i0, int16_t i1, int16_t i2, int16_t i3, int16_t i4, int16_t i5, int16_t i6, int16_t i7,
        ...
        // _mm512_set_epi16 missing in GCC 7.4.0
        zmm = _mm512_setr_epi32(i0+(i1<<16), i2+(i3<<16), i4+(i5<<16), i6+(i7<<16), 
            i8+(i9<<16), i10+(i11<<16), i12+(i13<<16), i14+(i15<<16), 
            i16+(i17<<16), i18+(i19<<16), i20+(i21<<16), i22+(i23<<16), 
            i24+(i25<<16), i26+(i27<<16), i28+(i29<<16), i30+(i31<<16));
#else
    ...
    Vec32s & load_partial(int n, void const * p) {
        zmm = _mm512_maskz_loadu_epi16(__mmask32((1 << n) - 1), p);
        return *this;
    }
    // Partial store. Store n elements
    void store_partial(int n, void * p) const {
...
public:
    ...
    Vec16sb get_low() const {
        return Vec16sb(Vec16s(_mm512_castsi512_si256(_mm512_maskz_set1_epi16(m32, -1))));
    }
    Vec16sb get_high() const {
        return Vec32sb(m32 >> 16).get_low();
    } 
    // Member function to change a single element in vector
    Vec32sb & insert (uint32_t index, bool a) {
        uint32_t mask = 1u << index;
        m32 = (m32 & ~mask) | uint32_t(a) << index;
        return *this;
    }    
    // Member function extract a single element from vector
    bool extract(int index) const { 
        return ((m32 >> index) & 1) != 0;
    }
    // Extract a single element. Use store function if extracting more than one element.
    // Operator [] can only read an element, not write.
    bool operator [] (int index) const {
...
static inline Vec32sb andnot (Vec32sb const & a, Vec32sb const & b) {
    //return  _kxnor_mask32(b, a);
    return __mmask32(a) & ~ __mmask32(b);
}

// horizontal_and. Returns true if all bits are 1
static inline bool horizontal_and (Vec32sb const & a) {
    return int32_t(__mmask32(a)) == -1;
}

// horizontal_or. Returns true if at least one bit is 1
static inline bool horizontal_or (Vec32sb const & a) {
    return int32_t(__mmask32(a)) != 0;
}

// Get index to the first element that is true. Return -1 if all are false
static inline int horizontal_find_first(Vec32sb const & x) {
    uint32_t b = uint32_t(__mmask32(x));
    if (b) {        
        return (int)bit_scan_forward(b);
    }
    else {
...
// Count number of true elements
static inline uint32_t horizontal_count(Vec32sb const & x) {
    return (uint32_t)vml_popcnt(uint32_t(__mmask32(x)));
}

// to_bits: convert boolean vector to integer bitfield
static inline uint32_t to_bits(Vec32sb x) {
    return uint32_t(__mmask32(x));
}
...
static inline Vec64c compress (Vec32s const & low, Vec32s const & high) {
    ...
    __m512i in    = constant16i<0,0,2,0,4,0,6,0,1,0,3,0,5,0,7,0>();
...
static inline Vec64c operator / (Vec64c const & a, Divisor_s const & d) {
    ...
    Vec32s  even = _mm512_sra_epi16(_mm512_sll_epi16(a, eight),eight);
    Vec32s  odd  = _mm512_sra_epi16(a, eight);
    Vec32s  evend = even / d;         // divide even-numbered elements
    Vec32s  oddd  = odd  / d;         // divide odd-numbered  elements
            oddd  = _mm512_sll_epi16(oddd, eight); // shift left to put back in place
...
static inline Vec64uc operator / (Vec64uc const & a, Divisor_us const & d) {
    ...
    Vec32us  odd  = _mm512_srl_epi16(a, eight);
...
static inline Vec32s divide_by_i(Vec32s const & x) {
    ...
    if ((d1 & (d1-1)) == 0) {
        ...
        __m512i sign;
        if (k > 1) sign = _mm512_srai_epi16(x, k-1); else sign = x;  // k copies of sign bit
...
static inline Vec32us divide_by_ui(Vec32us const & x) {
    ...
    const __m512i multv = _mm512_set1_epi16(mult);                   // broadcast mult
    ...
    if (round_down) {
        ...
        return select(overfl, Vec32us(uint32_t(mult >> b)), q);      // deal with overflow (rarely needed)
    }
    else {
