 ...
 * -------------------------------------------------------------------
 */
static INLINE int __ptw32_cancelable_wait(HANDLE waitHandle, DWORD timeout)
{
	int result;
	pthread_t self;
	__ptw32_thread_t * sp;
	...
	status = WaitForMultipleObjects(nHandles, handles,  FALSE, timeout);
...
int pthreadCancelableWait(HANDLE waitHandle) { return (__ptw32_cancelable_wait(waitHandle, INFINITE)); }
int pthreadCancelableTimedWait(HANDLE waitHandle, DWORD timeout) { return (__ptw32_cancelable_wait(waitHandle, timeout)); }
