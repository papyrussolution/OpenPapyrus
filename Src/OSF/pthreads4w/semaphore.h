...
__PTW32_BEGIN_C_DECLS
	// Function prototypes: some are implemented as stubs, which always fail; (FIXME: identify them).
	__PTW32_DLLPORT int __PTW32_CDECL sem_init(sem_t * sem, int pshared, unsigned int value);
	...
	__PTW32_DLLPORT int __PTW32_CDECL sem_timedwait(sem_t * sem, const struct timespec * abstime);
	__PTW32_DLLPORT int __PTW32_CDECL sem_post(sem_t * sem);
	__PTW32_DLLPORT int __PTW32_CDECL sem_post_multiple(sem_t * sem, int count);
