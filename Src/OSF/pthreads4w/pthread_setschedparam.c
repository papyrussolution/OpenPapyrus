...
/* Everything else */
	...
	else if(THREAD_PRIORITY_TIME_CRITICAL > prio && THREAD_PRIORITY_HIGHEST < prio) {
	...
	if(0 == SetThreadPriority(tp->threadH, prio)) {
		...
		tp->sched_priority = priority; // Must record the thread's sched_priority as given, not as finally adjusted.
	}
