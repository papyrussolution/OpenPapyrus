	...
	if(kind >= 0) {
		...
		if(PTHREAD_MUTEX_NORMAL == kind) {
			if((__PTW32_INTERLOCKED_LONG)__PTW32_INTERLOCKED_EXCHANGE_LONG((__PTW32_INTERLOCKED_LONGPTR)&mx->lock_idx, (__PTW32_INTERLOCKED_LONG)1) != 0) {
				while((__PTW32_INTERLOCKED_LONG)__PTW32_INTERLOCKED_EXCHANGE_LONG((__PTW32_INTERLOCKED_LONGPTR)&mx->lock_idx, (__PTW32_INTERLOCKED_LONG)-1) != 0) {
			...
			if((__PTW32_INTERLOCKED_LONG)__PTW32_INTERLOCKED_COMPARE_EXCHANGE_LONG((__PTW32_INTERLOCKED_LONGPTR)&mx->lock_idx, (__PTW32_INTERLOCKED_LONG)1, (__PTW32_INTERLOCKED_LONG)0) == 0) {
		...
		if((__PTW32_INTERLOCKED_LONG)__PTW32_ROBUST_NOTRECOVERABLE ==  __PTW32_INTERLOCKED_EXCHANGE_ADD_LONG((__PTW32_INTERLOCKED_LONGPTR)statePtr, (__PTW32_INTERLOCKED_LONG)0)) {
			...
			if(PTHREAD_MUTEX_NORMAL == kind) {
				...
				if((__PTW32_INTERLOCKED_LONG)__PTW32_INTERLOCKED_COMPARE_EXCHANGE_LONG((__PTW32_INTERLOCKED_LONGPTR)&mx->lock_idx, (__PTW32_INTERLOCKED_LONG)1,
					...
					if(pthread_equal(mx->ownerThread, self)) {
						...
						if(0 == result || EOWNERDEAD == result) {
							...
							/*
							 * Add mutex to the per-thread robust mutex currently-held list.
							 * If the thread terminates, all mutexes in this list will be
							 *unlocked.
							 */
							__ptw32_robust_mutex_add(mutex, self);
						}
...
int pthread_mutex_trylock(pthread_mutex_t * mutex)
{
	...
	if(kind >= 0) {
		/* Non-robust */
		if(0 ==  (__PTW32_INTERLOCKED_LONG)__PTW32_INTERLOCKED_COMPARE_EXCHANGE_LONG((__PTW32_INTERLOCKED_LONGPTR)&mx->lock_idx, (__PTW32_INTERLOCKED_LONG)1, (__PTW32_INTERLOCKED_LONG)0)) {
