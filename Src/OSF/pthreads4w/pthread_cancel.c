 ...
 */
DWORD __ptw32_Registercancellation(PAPCFUNC unused1, HANDLE threadH, DWORD unused2)
{
	CONTEXT context;
 ...
 */
{
	...
	if(tp->cancelType == PTHREAD_CANCEL_ASYNCHRONOUS && tp->cancelState == PTHREAD_CANCEL_ENABLE && tp->state < PThreadStateCanceling) {
		if(cancel_self) {
