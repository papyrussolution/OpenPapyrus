...
const static std::array<short, 0x40> CHARSETS[] = {
	...
	{ // set 2 (C)
		...
		0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA,  ECI,   FS,   GS,   RS,   NS, // Note
				                                                                                // that
				                                                                                // in
				                                                                                // original
				                                                                                // code
...
static void GetMessage(const ByteArray& bytes, int start, int len, Content& result, StructuredAppendInfo& sai)
{
	...
	for(int i = start; i < start + len; i++) {
		...
		switch(c) {
			...
			case NS:
			    result.append(
				    ToString((bytes[i + 1] << 24) + (bytes[i + 2] << 18) + (bytes[i + 3] << 12) + (bytes[i + 4] << 6) +
				    bytes[i + 5], 9));
			    i += 5;
			    break;
...
ZXING_EXPORT_TEST_ONLY DecoderResult Decode(ByteArray&& bytes, const int mode)
{
	Content result;
