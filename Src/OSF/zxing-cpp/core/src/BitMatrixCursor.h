...
public:
	...
	POINT direction(Direction dir) const noexcept {
		return static_cast<int>(dir) * right();
	}
	...
	void turn(Direction dir) noexcept {
		d = direction(dir);
	}
	...
	Value edgeAt(Direction dir) const noexcept {
		return edgeAt(direction(dir));
	}

	this_t& setDirection(PointF dir) {
		return d = bresenhamDirection(dir), *this;
	}

	this_t& setDirection(PointI dir) {
		return d = dir, *this;
	}
	...
	this_t movedBy(POINT o) const noexcept {
		return {*img, p + o, d};
	}

	this_t turnedBack() const noexcept {
		return {*img, p, back()};
	}
