...
public:
	...
	double modules(PointF beg, PointF end)
	{
		...
		modSizes.front() = 0; // the first element is an invalid sumBack value, would be pop_front() if vector
			              // supported this
		auto lineLength = distance(beg, end) - unitPixelDist;
		auto [iMin, iMax] = std::minmax_element(modSizes.begin() + 1, modSizes.end());
		auto meanModSize = average(modSizes, [](double dist){
				return dist > 0;
			});
		...
		if(*iMax > 2 * *iMin) {
			...
			meanModSize = average(modSizes, [](double dist) {
					return dist > 0;
				});
		}
...
public:
	...
	bool traceLine(PointF dEdge, RegressionLine& line)
	{
		...
		do {
			...
			if(stepResult != StepResult::FOUND)
				return stepResult == StepResult::OPEN_END && line.points().size() > 1 && updateDirectionFromLineCentroid(
					line);
		} while(true);
	}
	...
	bool traceGaps(PointF dEdge, RegressionLine& line, int maxStepSize, const RegressionLine& finishLine = {}, double minDist = 0)
	{
		...
		do {
			...
			// if we are drifting towards the inside of the code, pull the current position back out onto
			// the line
			if(line.isValid() && line.signedDistance(p) > 3) {
				// The current direction d and the line we are tracing are supposed to be roughly
				// parallel.
				// In case the 'go outward' step in traceStep lead us astray, we might end up with a
				// line
				...
				// consider a 90deg corner, rotated 45deg. we step away perpendicular from the line and
				// get
				// back projected where we left off the line.
				// The 'while' instead of 'if' was introduced to fix the issue with #245. It turns out
				// that
				// np can actually be behind the projection of the last line point and we need 2 steps
				// in d
...
# define CHECK(A) if(!(A)) continue
		...
		// at this point we found a plausible L-shape and are now looking for the b/w pattern at the top and
		// right:
		...
		// if we have an almost square (invalid rectangular) data matrix dimension, we try to parse it by
		// assuming a
		// square. we use the dimension that is closer to an integral value. all valid rectangular symbols
		// differ in
		// their dimension by at least 10. Note: this is currently not required for the black-box tests to
		// complete.
		...
		// shrink shape by half a pixel to go from center of white pixel outside of code to the edge between
		// white and black
		QuadrilateralF sourcePoints = {
			movedTowardsBy(tl, tr, bl, 0.5f),
			// move the tr point a little less because the jagged top and right line tend to be
			// statistically slightly
...
//	tryRotate = tryHarder = false;
	...
	// a history log to remember where the tracing already passed by to prevent a later trace from doing the same
	// work twice
	ByteMatrix history;
	if(tryHarder)
...
{
#ifdef __cpp_impl_coroutine
	// First try the very fast DetectPure() path. Also because DetectNew() generally fails with pure module size 1
	// symbols
