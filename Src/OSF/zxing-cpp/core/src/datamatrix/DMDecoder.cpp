...
struct Shift128 {
	...
	char operator()(int val) {
		return static_cast<char>(val + std::exchange(set, false) * 128);
	}
};
...
std::optional<std::array<int, 3> > DecodeNextTriple(BitSource& bits)
{
	...
	// If there is less than 2 bytes left or the next byte is the unlatch codeword then the current segment has
	// ended
...
ZXING_EXPORT_TEST_ONLY DecoderResult Decode(ByteArray&& bytes, const bool isDMRE)
{
	...
	try {
		while(!done && bits.available() >= 8) {
			...
			switch(oneByte) {
				...
				case 232: // FNC1
				    // Only recognizing an FNC1 as first/second by codeword position (aka symbol
				    // character position), not
				    // by decoded character position, i.e. not recognizing a C40/Text encoded FNC1
				    ...
				    if(bits.byteOffset() == firstFNC1Position)
					    ...
					    result.symbology.modifier = '3'; // AIM, note no AIM Application Indicator
						                             // format defined, ISO 16022:2006 11.2
				    else
				...
				default:
				    if(oneByte <= 128) { // ASCII data (ASCII value + 1)
					    ...
					    // work around encoders that use unlatch to ASCII as last code word (ask
					    // upstream)
