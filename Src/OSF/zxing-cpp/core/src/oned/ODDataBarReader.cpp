...
{
	auto checksum = [](Pair p) {
		    return p.left.checksum + 4 * p.right.checksum;
	    };
...
{
	auto value = [](Pair p) {
		    return 1597 * p.left.value + p.right.value;
	    };
	auto res = 4537077LL * value(leftPair) + value(rightPair);
	if(res >= 10000000000000LL) {  // Strip 2D linkage flag (GS1 Composite) if any (ISO/IEC 24724:2011 Section
			               // 5.2.3)
