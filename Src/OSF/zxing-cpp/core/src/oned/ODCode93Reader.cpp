...
static bool CheckOneChecksum(const std::string& result, int checkPosition, int weightMax)
{
	int weight = 1;
	int checkSum = 0;
...
static bool CheckChecksums(const std::string& result)
{
...
static bool IsStartGuard(const PatternView& window, int spaceInPixel)
{
	...
	// Use only the first 4 elements which results in more than a 2x speedup. This is counter-intuitive since we
	// save at
	// most 1/3rd of the loop iterations in FindPattern. The reason might be a successful vectorization with the
	// limited
