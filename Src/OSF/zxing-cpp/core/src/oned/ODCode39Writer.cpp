		...
		switch(character) {
			...
			default:
			    if(character > 0 && character < 27) {
				    ...
				    throw std::invalid_argument("Requested content contains a non-encodable character: '" +
...
BitMatrix Code39Writer::encode(const std::wstring& contents, int width, int height) const
{
	size_t length = contents.length();
	if(length == 0) {
	...
	for(size_t i = 0; i < length; i++) {
		...
		if(indexInString < 0) {
			...
			if(length > 80) {
				throw std::invalid_argument("Requested contents should be less than 80 digits long, but got " + std::to_string(
