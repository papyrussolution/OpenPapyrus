		...
		if(codeSet == CODE_CODE_C) {
			...
			switch(code) {
				...
				case CODE_FNC_2:
				    ...
				    break;
				case CODE_FNC_3:
				    _readerInit = true; // Can occur anywhere in the symbol (ISO/IEC 15417:2007 4.3.4.2
					                // (c))
				    break;
				case CODE_SHIFT:
				    if(shift)
...
static auto E2E_PATTERNS = [] {
	    // This creates an array of ints for fast IndexOf lookup of the edge-2-edge patterns (ISO/IEC 15417:2007(E)
	    // Table 2)
	    // e.g. a code pattern of { 2, 1, 2, 2, 2, 2 } becomes the e2e pattern { 3, 3, 4, 4 } and the value
	    // 0b11100011110000.
...
Result Code128Reader::decodePattern(int rowNumber, PatternView& next, std::unique_ptr<DecodingState>&) const
{
	...
	auto decodePattern = [](const PatternView& view, bool start = false) {
		    ...
		    if(code == -1 && !start) // if the reference algo fails, give the original upstream version a try
				             // (required to decode a few samples)
