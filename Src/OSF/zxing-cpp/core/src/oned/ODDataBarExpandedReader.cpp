...
{
	auto checksum = TransformReduce(pairs, 0, [](auto p) {
			return p.left.checksum + p.right.checksum;
		}) % 211 +
...
static bool FindValidSequence(const PairMap& all, ITER begin, ITER end, Pairs& stack)
{
	...
	if(auto ppairs = all.find(*begin); ppairs != all.end()) {
		// only try the N most common pairs, this means the absolute maximum number of ChecksumIsValid()
		// evaluations
		...
		for(auto p = pairs.begin(), pend = pairs.end(); p != pend && n < N; ++p, ++n) {
			...
			if(!p->right && std::next(begin) != end)
				continue;
			// to lower the chance of a misread, one can require each pair to have been seen at least N
			// times.
