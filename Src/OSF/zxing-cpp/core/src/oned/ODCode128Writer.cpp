...
BitMatrix Code128Writer::encode(const std::wstring& contents, int width, int height) const
{
	// Check length
			...
			switch(contents[position]) {
				...
				default:
				    ...
				    if(codeSet == CODE_CODE_A) {
					    ...
					    if(patternIndex < 0) {
						    // everything below a space character comes behind the underscore in
						    // the code patterns table
						    patternIndex += '`';
					    }
				    }
					    ...
					    patternIndex =
						(contents[position] - '0') * 10 +
						(position + 1 < length ? contents[position + 1] - '0' : 0);
