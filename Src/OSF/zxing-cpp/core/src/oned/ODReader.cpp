    ...
    bool tryHarder, bool rotate, bool isPure, int maxSymbols, int minLineCount, bool returnErrors)
{
	...
	for(int i = 0; i < maxLines; i++) {
		...
		// 'surprising' and inconsistent. It also requires the decoderState to be shared between normal and
		// reversed
		// scans, which makes no sense in general because it would mix partial detection data from two codes of
		// the same
		...
		for(bool upsideDown : {false, true}) {
			...
			for(size_t r = 0; r < readers.size(); ++r) {
				// If this is a pure symbol, then checking a single non-empty line is sufficient for all
				// but the stacked
				// DataBar codes. They are the only ones using the decodingState, which we can use as a
				// flag here.
