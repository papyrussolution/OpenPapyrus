...
GenericGFPoly&GenericGFPoly::divide(const GenericGFPoly& other, GenericGFPoly& quotient)
{
	assert(_field == other._field); // "GenericGFPolys do not have same GenericGF field"
	...
	// we use the memory from this (the dividend) and swap it with quotient, which will then accumulate the result
	// as
	...
	for(int i = 0; i < Size(result) - (Size(divisor) - 1); ++i) {
		...
		// we always skip the first coefficient of the divisor, because it's only used to normalize the dividend
		// coefficient
		for(int j = 1; j < Size(divisor); ++j)
			result[i + j] ^= _field->multiply(divisor[j], ci); // equivalent to: result[i + j] +=
			                                                   // -divisor[j] * ci
	}

	// extract the normalized remainder from result
	auto firstNonZero = std::find_if(result.end() - other.degree(), result.end(), [](int c){
			return c != 0;
		});
...
{
	auto firstNonZero = FindIf(_coefficients, [](int c){
			return c != 0;
		});
