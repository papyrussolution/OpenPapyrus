...
template <typename P> static LumImage ExtractLum(const ImageView& iv, P projection)
{
...
public:
	...
	{
		layers.push_back(iv);
		// TODO: if only matrix codes were considered, then using smin would be sufficient (see #425)
		while(threshold > 0 && smax(layers.back().width(), layers.back().height()) > threshold &&
		    smin(layers.back().width(), layers.back().height()) >= factor)
			addLayer(factor);
#if 0
		// Reversing the layers means we'd start with the smallest. that can make sense if we are only looking
		// for a
		// single symbol. If we start with the higher resolution, we get better (high res) position information.
		// TODO: see if masking out higher res layers based on found symbols in lower res helps overall
		// performance.
...
ImageView SetupLumImageView(ImageView iv, LumImage& lum, const DecodeHints& hints)
{
	...
	if(hints.binarizer() == Binarizer::GlobalHistogram || hints.binarizer() == Binarizer::LocalAverage) {
		if(iv.format() != ImageFormat::Lum) {
			lum = ExtractLum(iv, [r = RedIndex(iv.format()), g = GreenIndex(iv.format()), b = BlueIndex(iv.format())](const uint8_t* src) 
				{ return RGBToLum(src[r], src[g], src[b]); });
		}
