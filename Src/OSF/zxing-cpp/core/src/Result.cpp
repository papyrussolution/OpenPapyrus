...
Result::Result(DecoderResult&& decodeResult, Position&& position, BarcodeFormat format) : _content(std::move(decodeResult).content()),
	_error(std::move(decodeResult).error()), _position(std::move(position)), _sai(decodeResult.structuredAppend()), _format(format),
	_lineCount(decodeResult.lineCount()), _isMirrored(decodeResult.isMirrored()), _readerInit(decodeResult.readerInit())
{
...
bool Result::isValid() const { return format() != BarcodeFormat::None && _content.symbology.code != 0 && !error(); }
...
std::string Result::symbologyIdentifier() const { return _content.symbology.toString(); }
...
Result MergeStructuredAppendSequence(const Results& results)
{
	...
	if(allResults.back().sequenceSize() != Size(allResults) ||
	    ...
	    [&](Result& it) {
			return it.sequenceId() == allResults.front().sequenceId();
		}))
