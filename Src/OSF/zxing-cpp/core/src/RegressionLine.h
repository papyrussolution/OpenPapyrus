...
protected:
	...
	template <typename T> bool evaluate(const PointT<T>* begin, const PointT<T>* end)
	{
		...
		return dot(_directionInward, normal()) > 0.5f; // angle between original and new direction is at most 60
			                                       // degree
	}

	template <typename T> bool evaluate(const std::vector<PointT<T> >& points) {
		return evaluate(&points.front(), &points.back() + 1);
	}

	template <typename T> static auto distance(PointT<T> a, PointT<T> b) {
		return ZXing::distance(a, b);
	}
...
public:
	...
	int length() const {
		return _points.size() >= 2 ? int(distance(_points.front(), _points.back())) : 0;
	}
	...
	PointF normal() const {
		return isValid() ? PointF(a, b) : _directionInward;
	}

	auto signedDistance(PointF p) const {
		return dot(normal(), p) - c;
	}

	template <typename T> auto distance(PointT<T> p) const {
		return std::abs(signedDistance(PointF(p)));
	}

	PointF project(PointF p) const {
		return p - signedDistance(p) * normal();
	}

	PointF centroid() const {
		return std::accumulate(_points.begin(), _points.end(), PointF()) / _points.size();
	}
