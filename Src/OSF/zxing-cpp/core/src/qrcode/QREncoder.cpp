 ...
 */
ZXING_EXPORT_TEST_ONLY int GetAlphanumericCode(int code)
{
	return (code < Size(ALPHANUMERIC_TABLE)) ? ALPHANUMERIC_TABLE[code] : -1;
}
/**
 * Choose the best mode by examining the content. Note that 'encoding' is used as a hint;
 * if it is Shift_JIS, and the input is only double-byte Kanji, then we return {@link Mode#KANJI}.
 */
ZXING_EXPORT_TEST_ONLY CodecMode ChooseMode(const std::wstring& content, CharacterSet encoding)
{
 ...
 */
ZXING_EXPORT_TEST_ONLY void AppendModeInfo(CodecMode mode, BitArray& bits) { bits.appendBits(static_cast<int>(mode), 4); }

/**
 * Append length info. On success, store the result in "bits".
 */
ZXING_EXPORT_TEST_ONLY void AppendLengthInfo(int numLetters, const Version& version, CodecMode mode, BitArray& bits)
{
...
ZXING_EXPORT_TEST_ONLY void AppendNumericBytes(const std::wstring& content, BitArray& bits)
{
...
ZXING_EXPORT_TEST_ONLY void AppendAlphanumericBytes(const std::wstring& content, BitArray& bits)
{
	size_t length = content.length();
	size_t i = 0;
	while(i < length) {
...
ZXING_EXPORT_TEST_ONLY void Append8BitBytes(const std::wstring& content, CharacterSet encoding, BitArray& bits)
{
...
ZXING_EXPORT_TEST_ONLY void AppendKanjiBytes(const std::wstring& content, BitArray& bits)
{
	std::string bytes = TextEncoder::FromUnicode(content, CharacterSet::Shift_JIS);
 ...
 */
ZXING_EXPORT_TEST_ONLY void AppendBytes(const std::wstring& content, CodecMode mode, CharacterSet encoding, BitArray& bits)
{
	switch(mode) {
 ...
 */
ZXING_EXPORT_TEST_ONLY void TerminateBits(int numDataBytes, BitArray& bits)
{
 ...
 * JISX0510:2004 (p.30)
 */
ZXING_EXPORT_TEST_ONLY void GetNumDataBytesAndNumECBytesForBlockID(int numTotalBytes, int numDataBytes, int numRSBlocks, int blockID,
...
ZXING_EXPORT_TEST_ONLY void GenerateECBytes(const ByteArray& dataBytes, int numEcBytes, ByteArray& ecBytes)
{
	...
	std::transform(message.end() - numEcBytes, message.end(), ecBytes.begin(), [](auto c) {
 ...
 */
ZXING_EXPORT_TEST_ONLY BitArray InterleaveWithECBytes(const BitArray& bits, int numTotalBytes, int numDataBytes, int numRSBlocks)
{
	...
	if(numTotalBytes != output.sizeInBytes()) {   // Should be same.
		throw std::invalid_argument("Interleaving error: " + std::to_string(numTotalBytes) + " and " +
			  std::to_string(output.sizeInBytes())
			  + " differ.");
	}
	return output;
}
