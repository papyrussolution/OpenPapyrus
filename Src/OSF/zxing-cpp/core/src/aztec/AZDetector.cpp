...
static std::vector<ConcentricPattern> FindPureFinderPattern(const BitMatrix& image)
{
	...
	if(!image.findBoundingBox(left, top, width, height, 11))  // 11 is the size of an Aztec Rune, see ISO/IEC
			                                          // 24778:2008(E) Annex A
		return {};
...
#else // own algorithm based on PatternRow processing (between 0% and 100% faster than reference algo depending on
		// input)
...
DetectorResults Detect(const BitMatrix& image, bool isPure, bool tryHarder, int maxSymbols)
{
	...
	for(const auto & fp : fps) {
		...
		[&]() {
			...
			// In the outer layer of the Core Symbol, the 12 orientation bits at the corners are bitwise
			// compared against the specified
			// pattern in each of four possible orientations and their four mirror inverse orientations as
			// well. If in any of the 8
			// cases checked as many as 9 of the 12 bits correctly match, that is deemed to be the correct
			// orientation, otherwise
			// decoding fails.
			// Unfortunately, this seems to be wrong: there are 12-bit patterns in those 8 cases that differ
			// only in 4 bits like
			// 011'100'000'111 (rot90 && !mirror) and 111'000'001'110 (rot0 && mirror), meaning if two of
			// those are wrong, both cases
			// have a hamming distance of 2, meaning only 1 bit errors can be relyable recovered from. The
			// following code therefore
			// incorporates the complete set of mode message bits to help determine the orientation of the
			// symbol. This is still not
			// sufficient for the ErrorInModeMessageZero test case in AZDecoderTest.cpp but good enough for
			// the author.
			for(radius = 5; radius <= 7; radius += 2) {
				...
				for(mirror = 0; mirror <= 1; ++mirror) {
					...
					modeMessage =
					    ModeMessage(image,
						PerspectiveTransform(srcQuad, RotatedCorners(*fpQuad, rotate, mirror)),
						radius);
					if(modeMessage != -1)
						return;
				}
...
#if 1
		// improve prescision of sample grid by extrapolating from outer square of white pixels (5 edges away
		// from center)
		if(radius == 7) {
