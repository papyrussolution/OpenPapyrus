...
static EncodingState AddBinaryShiftChar(const EncodingState& state, int index)
{
	...
	int deltaBitCount =
	    (state.binaryShiftByteCount == 0 || state.binaryShiftByteCount == 31) ? 18 : (state.binaryShiftByteCount == 62) ? 9 : 8;
...
static bool IsBetterThanOrEqualTo(const EncodingState& state, const EncodingState& other)
{
	...
	if(state.binaryShiftByteCount < other.binaryShiftByteCount) {
		...
		// maximum possible additional cost (we end up exceeding the 31 byte boundary and other state can stay
		// beneath it)
		newModeBitCount += 10;
	}
...
static void UpdateStateForPair(const EncodingState& state, int index, int pairCode, std::list<EncodingState>& result)
{
	...
	if(pairCode == 3 || pairCode == 4) {
		...
		auto digitState = LatchAndAppend(stateNoBinary, MODE_DIGIT, 16 - pairCode);     // period or comma in
			                                                                        // DIGIT
		result.push_back(LatchAndAppend(digitState, MODE_DIGIT, 1));                            // space in
			                                                                                // DIGIT
	}
	if(state.binaryShiftByteCount > 0) {
 ...
 */
BitArray HighLevelEncoder::Encode(const std::string& text)
{
	...
	EncodingState minState = *std::min_element(states.begin(), states.end(), [](const EncodingState& a, const EncodingState& b) {
