...
ZXING_EXPORT_TEST_ONLY void GenerateModeMessage(bool compact, int layers, int messageSizeInWords, BitArray& modeMessage)
{
...
ZXING_EXPORT_TEST_ONLY void StuffBits(const BitArray& bits, int wordSize, BitArray& out)
{
	out = BitArray();
	int n = bits.size();
 ...
 */
EncodeResult Encoder::Encode(const std::string& data, int minECCPercent, int userSpecifiedLayers)
{
	// High-level encode
