...
static void DecodeContent(const BitArray& bits, Content& res)
{
	...
	while(remBits.size() >= (shiftTable == Table::DIGIT ? 4 : 5)) {  // see ISO/IEC 24778:2008 7.3.1.2 regarding
			                                                 // padding bits
		if(shiftTable == Table::BINARY) {
			...
			if(std::strncmp(str, "CTRL_", 5) == 0) {
				// Table changes
				// ISO/IEC 24778:2008 prescibes ending a shift sequence in the mode from which it was
				// invoked.
				// That's including when that mode is a shift.
				// Our test case dlusbs.png for issue #642 exercises that.
				latchTable = shiftTable;  // Latch the current mode, so as to return to Upper after U/S
					                  // B/S
...
ZXING_EXPORT_TEST_ONLY DecoderResult Decode(const BitArray& bits)
{
	Content res;
	res.symbology = {'z', '0', 3};
	try {
	...
	if(sai.index != -1)
		res.symbology.modifier += 6; // TODO: this is wrong as long as we remove the sai info from the content
		                             // in ParseStructuredAppend
