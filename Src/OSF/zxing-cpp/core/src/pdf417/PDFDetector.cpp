...
// A PDF471 barcode should have at least 3 rows, with each row being >= 3 times the module width. Therefore it should be
// at least
 ...
 */
static bool FindGuardPattern(const BitMatrix& matrix,
    int column,
    int row,
    int width,
    bool whiteFirst,
...
static std::array<Nullable<ResultPoint>, 4>&FindRowsWithPattern(const BitMatrix& matrix,
    int height,
    int width,
    int startRow,
    int startColumn,
    const std::vector<int>& pattern,
    std::array<Nullable<ResultPoint>, 4>& result)
{
	bool found = false;
	int startPos, endPos;
	...
	if(found) {
		...
		for(; stopRow < height; stopRow++) {
			...
			// a found pattern is only considered to belong to the same barcode if the start and end
			// positions
			// don't differ too much. Pattern drift should be not bigger than two for consecutive rows. With
			// a higher number of skipped rows drift could be larger. To keep it simple for now, we allow a
			// slightly
			// larger drift and don't check for skipped rows.
			if(found && std::abs(previousRowStart - startPos) < MAX_PATTERN_DRIFT &&
			    std::abs(previousRowEnd - endPos) < MAX_PATTERN_DRIFT) {
...
static void CopyToResult(std::array<Nullable<ResultPoint>, 8>& result,
    const std::array<Nullable<ResultPoint>, 4>& tmpResult,
...
#if 1 // 2x speed improvement for images with no PDF417 symbol by not looking for symbols without start guard (which are
				// not conforming to spec anyway)
		CopyToResult(result,
		    FindRowsWithPattern(matrix, height, width, startRow, startColumn, STOP_PATTERN, tmp),
...
static std::list<std::array<Nullable<ResultPoint>, 8> > DetectBarcode(const BitMatrix& bitMatrix, bool multiple)
{
	...
	while(row < bitMatrix.height()) {
		...
		if(vertices[0] == nullptr && vertices[3] == nullptr) {
			...
			// we didn't find a barcode starting at the given column and row. Try again from the first
			// column and slightly
		...
		// if we didn't find a right row indicator column, then continue the search for the next barcode after
		// the
