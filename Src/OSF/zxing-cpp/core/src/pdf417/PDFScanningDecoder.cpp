...
static bool GetModuleBitCount(const BitMatrix& image,
    int minColumn,
    int maxColumn,
    bool leftToRight,
    int startColumn,
    ...
    ModuleBitCountType& moduleBitCount)
{
	...
	return moduleNumber == moduleBitCount.size() ||
	       (imageColumn == (leftToRight ? maxColumn : minColumn) && moduleNumber == moduleBitCount.size() - 1);
}
...
static Nullable<Codeword> DetectCodeword(const BitMatrix& image,
    int minColumn,
    int maxColumn,
    bool leftToRight,
    int startColumn,
    ...
    int maxCodewordWidth)
{
	...
	// we usually know fairly exact now how long a codeword is. We should provide minimum and maximum expected
	// length
	...
	if(!CheckCodewordSkew(codewordBitCount, minCodewordWidth, maxCodewordWidth)) {
		// We could try to use the startX and endX position of the codeword in the same column in the previous
		// row,
...
static DetectionResultColumn GetRowIndicatorColumn(const BitMatrix& image,
    ...
    int maxCodewordWidth)
{
	DetectionResultColumn rowIndicatorColumn(boundingBox,
	    leftToRight ? DetectionResultColumn::RowIndicator::Left : DetectionResultColumn::RowIndicator::Right);
	for(int i = 0; i < 2; i++) {
		int increment = i == 0 ? 1 : -1;
		int startColumn = (int)startPoint.x();
		for(int imageRow = (int)startPoint.y();
		    imageRow <= boundingBox.maxY() && imageRow >= boundingBox.minY();
		    imageRow += increment) {
			auto codeword = DetectCodeword(image,
				0,
				image.width(),
				leftToRight,
				startColumn,
...
static bool GetBarcodeMetadata(Nullable<DetectionResultColumn>& leftRowIndicatorColumn,
    Nullable<DetectionResultColumn>& rightRowIndicatorColumn,
    BarcodeMetadata& result)
{
...
static bool AdjustBoundingBox(Nullable<DetectionResultColumn>& rowIndicatorColumn, Nullable<BoundingBox>& result)
{
	...
	BoundingBox box;
	if(BoundingBox::AddMissingRows(rowIndicatorColumn.value().boundingBox(), missingStartRows, missingEndRows,
	    rowIndicatorColumn.value().isLeftRowIndicator(), box)) {
		result = box;
		return true;
	}
	return false;
...
static bool Merge(Nullable<DetectionResultColumn>& leftRowIndicatorColumn,
    Nullable<DetectionResultColumn>& rightRowIndicatorColumn,
    DetectionResult& result)
{
	if(leftRowIndicatorColumn != nullptr || rightRowIndicatorColumn != nullptr) {
		...
		if(GetBarcodeMetadata(leftRowIndicatorColumn, rightRowIndicatorColumn, barcodeMetadata)) {
			...
			if(AdjustBoundingBox(leftRowIndicatorColumn,
			    leftBox) &&
			    AdjustBoundingBox(rightRowIndicatorColumn,
			    rightBox) && BoundingBox::Merge(leftBox, rightBox, mergedBox)) {
				...
				if(codeword != nullptr) {
					...
					if(rowNumber >= 0) {
						if(rowNumber >= Size(barcodeMatrix)) {
							// We have more rows than the barcode metadata allows for,
							// ignore them.
							continue;
						}
...
static bool AdjustCodewordCount(const DetectionResult& detectionResult, std::vector<std::vector<BarcodeValue> >& barcodeMatrix)
{
	...
	int calculatedNumberOfCodewords = detectionResult.barcodeColumnCount() * detectionResult.barcodeRowCount() - GetNumberOfECCodeWords(
 ...
 */
ZXING_EXPORT_TEST_ONLY bool DecodeErrorCorrection(std::vector<int>& received, int numECCodewords, const std::vector<int>& erasures [[maybe_unused]], int& nbErrors)
{
	const ModulusGF& field = GetModulusGF();
    ...
    int maxCodewordWidth)
{
	...
	for(int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
		...
		if(detectionResult.column(barcodeColumn) != nullptr) {
			// This will be the case for the opposite row indicator column, which doesn't need to be decoded
			// again.
			continue;
		}
		DetectionResultColumn::RowIndicator rowIndicator = barcodeColumn ==
		    0 ? DetectionResultColumn::RowIndicator::Left : (barcodeColumn ==
		    maxBarcodeColumn ? DetectionResultColumn::RowIndicator::Right : DetectionResultColumn::RowIndicator::
		    None);
		detectionResult.setColumn(barcodeColumn, DetectionResultColumn(boundingBox, rowIndicator));
		int startColumn = -1;
		int previousStartColumn = startColumn;
		// TODO start at a row for which we know the start position, then detect upwards and downwards from
		// there.
		for(int imageRow = boundingBox.minY(); imageRow <= boundingBox.maxY(); imageRow++) {
			...
			Nullable<Codeword> codeword = DetectCodeword(image,
				boundingBox.minX(),
				boundingBox.maxX(),
				leftToRight,
				startColumn,
