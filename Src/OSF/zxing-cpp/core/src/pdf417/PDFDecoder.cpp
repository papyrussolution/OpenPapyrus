...
static int ProcessECI(const std::vector<int>& codewords, int codeIndex, const int length, const int code, Content& result)
{
	if(codeIndex < length && IsECI(code)) {
		if(code == ECI_CHARSET)
			...
			codeIndex += code == ECI_GENERAL_PURPOSE ? 2 : 1; // Don't currently handle non-character set
			                                                  // ECIs so just ignore
	}
			...
			switch(code) {
				case MODE_SHIFT_TO_BYTE_COMPACTION_MODE:
				    ...
				    while(codeIndex < codewords[0] && IsECI(codewords[codeIndex])) {
					    codeIndex =
						ProcessTextECI(textCompactionData, index, codewords, codeIndex + 1,
						    codewords[codeIndex]);
				    }
				    if(codeIndex < codewords[0])
...
static int NumericCompaction(const std::vector<int>& codewords, int codeIndex, Content& result)
{
	...
	while(codeIndex < codewords[0]) {
		...
		if(code >= TEXT_COMPACTION_MODE_LATCH) {
			if(IsECI(code)) {
				// As operating in Basic Channel Mode (i.e. not embedding backslashed ECIs and doubling
				// backslashes)
				// allow ECIs anywhere in Numeric Compaction (i.e. ISO/IEC 15438:2015 5.5.3.4 doesn't
				// apply).
...
ZXING_EXPORT_TEST_ONLY int DecodeMacroBlock(const std::vector<int>& codewords, int codeIndex, DecoderResultExtra& resultMetadata)
{
	...
	if(optionalFieldsStart != -1) {
		...
		resultMetadata.setOptionalData(
			std::vector<int>(codewords.begin() + optionalFieldsStart,
	...
	try {
		for(int codeIndex = 1; codeIndex < codewords[0];) {
			...
			switch(code) {
				...
				// This should only be encountered once in this loop, when default Text Compaction mode
				// applies
				// (see default case below)
				case MODE_SHIFT_TO_BYTE_COMPACTION_MODE: codeIndex = TextCompaction(codewords, codeIndex - 1, result);
				    break;
				case BYTE_COMPACTION_MODE_LATCH:
				...
				case BEGIN_MACRO_PDF417_CONTROL_BLOCK: codeIndex = DecodeMacroBlock(codewords, codeIndex, *resultMetadata);
				    break;
				case BEGIN_MACRO_PDF417_OPTIONAL_FIELD:
				case MACRO_PDF417_TERMINATOR:
				    ...
				    break;
				case READER_INIT:
				    if(codeIndex != 2) // Must be first codeword after symbol length (ISO/IEC 15438:2015
						       // 5.4.1.4)
					    throw FormatError();
				    else
					    readerInit = true;
				    break;
				case LINKAGE_EANUCC:
				    if(codeIndex != 2) // Must be first codeword after symbol length (GS1 Composite
						       // ISO/IEC 24723:2010 4.3)
					    throw FormatError();
				    // TODO: handle else case
				    break;
				case LINKAGE_OTHER:
				    // Allowed to treat as invalid by ISO/IEC 24723:2010 5.4.1.5 and 5.4.6.1 when in
				    // Basic Channel Mode
				    throw UnsupportedError("LINKAGE_OTHER, see ISO/IEC 15438:2015 5.4.1.5");
				    break;
				default:
				    if(code >= TEXT_COMPACTION_MODE_LATCH) { // Reserved codewords (all others in
						                             // switch)
					    // Allowed to treat as invalid by ISO/IEC 24723:2010 5.4.6.1 when in Basic
					    // Channel Mode
					    throw UnsupportedError("Reserved codeword, see ISO/IEC 15438:2015 5.4.6.1");
				    }
				    else {
					    // Default mode is Text Compaction mode Alpha sub-mode (ISO/IEC 15438:2015
					    // 5.4.2.1)
