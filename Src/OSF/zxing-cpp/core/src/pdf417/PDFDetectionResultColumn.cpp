...
Nullable<Codeword>DetectionResultColumn::codewordNearby(int imageRow) const
{
...
void DetectionResultColumn::adjustCompleteIndicatorColumnRowNumbers(const BarcodeMetadata& barcodeMetadata)
{
	if(!isRowIndicator()) {
		return;
	}
...
bool DetectionResultColumn::getBarcodeMetadata(BarcodeMetadata& result)
{
	...
	if(cc.empty() || rcu.empty() || rcl.empty() || ec.empty() || cc[0] < 1 || rcu[0] + rcl[0] < MIN_ROWS_IN_BARCODE ||
	    rcu[0] + rcl[0] > MAX_ROWS_IN_BARCODE) {
		return false;
	}
