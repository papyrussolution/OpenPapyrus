...
std::vector<int> ReadCodeWords(BitMatrixCursor<POINT> topCur, SymbolInfo info)
{
	...
	auto print = [](CodeWord c [[maybe_unused]]) {
		    printf("%4d.%d ", c.code, c.cluster);
	    };
...
static Result DecodePure(const BinaryBitmap& image_)
{
	...
	// counter intuitively, using a floating point cursor is about twice as fast an integer one (on an AVX
	// architecture)
