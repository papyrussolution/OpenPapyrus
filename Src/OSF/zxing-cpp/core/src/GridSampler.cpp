...
DetectorResult SampleGrid(const BitMatrix& image, int width, int height, const ROIs& rois)
{
	...
	for(auto && [x0, x1, y0, y1, mod2Pix] : rois) {
		...
		auto isInside = [&mod2Pix = mod2Pix, &image](int x, int y) {
			    return image.isIn(mod2Pix(centered(PointI(x, y))));
		    };
	...
	for(auto && [x0, x1, y0, y1, mod2Pix] : rois) {
		for(int y = y0; y < y1; ++y)
			for(int x = x0; x < x1; ++x) {
				...
				// Due to a "numerical instability" in the PerspectiveTransform generation/application
				// it has been observed
				// that even though all boundary grid points get projected inside the image, it can
				// still happen that an
				// inner grid points is not. See #563. A true perspective transformation cannot have
				// this property.
				// The following check takes 100% care of the issue and turned out to be less of a
				// performance impact than feared.
				// TODO: Check some mathematical/numercial property of mod2Pix to determine if it is a
