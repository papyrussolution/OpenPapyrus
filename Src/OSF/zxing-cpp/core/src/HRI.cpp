...
{
	//TODO: c++20
	auto starts_with = [](std::string_view str, std::string_view pre) {
	...
	while(rem.size()) {
		const AiInfo* i = FindIf(aiInfos, [&](const AiInfo& i) {
				return starts_with(rem, i.aiPrefix);
			});
		...
		// See General Specification v22.0 Section 7.8.6.3: "...the processing routine SHALL tolerate a single
		// separator character
