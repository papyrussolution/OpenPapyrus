...
{
	// range is the approximate width/height of the nth ring, if nth>1 then it would be plausible to limit the
	// search radius
	// to approximately range / 2 * sqrt(2) == range * 0.75 but it turned out to be too limiting with
	// realworld/noisy data.
	int radius = range;
	bool inner = nth < 0;
	nth = std::abs(nth);
	log(center, 3);
