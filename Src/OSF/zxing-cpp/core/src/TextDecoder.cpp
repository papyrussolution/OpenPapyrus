...
void TextDecoder::Append(std::string& str, const uint8_t* bytes, size_t length, CharacterSet charset, bool sjisASCII)
{
	...
	const uint flags = ZUECI_FLAG_SB_STRAIGHT_THRU | (sjisASCII ? ZUECI_FLAG_SJIS_STRAIGHT_THRU : 0);
	int utf8_len;
	...
	uchar * utf8_buf = reinterpret_cast<uchar *>(str.data()) + str_len;
...
CharacterSet TextDecoder::GuessEncoding(const uint8_t* bytes, size_t length, CharacterSet fallback)
{
	...
	// Easy -- if there is BOM or at least 1 valid not-single byte character (and no evidence it can't be UTF-8),
	// done
	...
	// Easy -- if assuming Shift_JIS or at least 3 valid consecutive not-ascii characters (and no evidence it can't
	// be), done
...
CharacterSet TextDecoder::DefaultEncoding()
{
	return CharacterSet::ISO8859_1;
}
} // ZXing
