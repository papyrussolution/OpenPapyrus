 ...
 */
static void FASTCALL arith_encode(j_compress_ptr cinfo, uchar * pSt, int val)
{
	arith_entropy_ptr e = (arith_entropy_ptr)cinfo->entropy;
	// 
	...
	// 
	int sv = *pSt;
	INT32 qe = jpeg_aritab[sv & 0x7F]; // => Qe_Value 
	uchar nl = qe & 0xFF; 
	...
	if(val != (sv >> 7)) {
		...
		if(e->a >= qe) {
			...
			// the two symbols for coding efficiency, otherwise code the LPS as usual: 
			e->c += e->a;
			e->a = qe;
		}
		*pSt = (sv & 0x80) ^ nl; /* Estimate_after_LPS */
	}
	else {
		// Encode the more probable symbol 
		if(e->a >= 0x8000L) // A >= 0x8000 -> ready, no renormalization required 
			return; 
		if(e->a < qe) {
			// If the interval size (qe) for the less probable symbol (LPS)
			// is larger than the interval size for the MPS, then exchange
		...
		*pSt = (sv & 0x80) ^ nm; /* Estimate_after_MPS */
	}
...
METHODDEF(void) start_pass(j_compress_ptr cinfo, boolean gather_statistics)
{
	...
	if(gather_statistics) {
		...
		// We are fully adaptive here and need no extra statistics gathering pass!
	...
	if(cinfo->progressive_mode)
		if(cinfo->Ah == 0)
			...
			entropy->pub.encode_mcu = (cinfo->Ss == 0) ? encode_mcu_DC_refine : encode_mcu_AC_refine;
	else
	...
	for(ci = 0; ci < cinfo->comps_in_scan; ci++) {
		...
		if(cinfo->Ss == 0 && cinfo->Ah == 0) {
			...
			if(entropy->dc_stats[tbl] == NULL)
				entropy->dc_stats[tbl] = (uchar *)(*cinfo->mem->alloc_small)(reinterpret_cast<j_common_ptr>(cinfo), JPOOL_IMAGE, DC_STAT_BINS);
		...
		if(cinfo->Se) {
			...
			if(entropy->ac_stats[tbl] == NULL)
				entropy->ac_stats[tbl] = (uchar *)(*cinfo->mem->alloc_small)(reinterpret_cast<j_common_ptr>(cinfo), JPOOL_IMAGE, AC_STAT_BINS);
