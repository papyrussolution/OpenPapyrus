...
// 
// Descr: Set or change the 'quality' (quantization) setting, using default tables
//   and straight percentage-scaling quality scales.
//   This entry point allows different scalings for luminance and chrominance.
// 
GLOBAL(void) jpeg_default_qtables(j_compress_ptr cinfo, boolean force_baseline)
{
...
{
	quality = jpeg_quality_scaling(quality); // Convert user 0-100 rating to percentage scaling 
	jpeg_set_linear_quality(cinfo, quality, force_baseline); // Set up standard quality tables 
}
// 
// Huffman table setup routines
// 
//
// Descr: Define a Huffman table 
//
static void add_huff_table(j_compress_ptr cinfo, JHUFF_TBL **htblptr, const uint8 *bits, const uint8 *val)
{
	...
	(*htblptr)->sent_table = FALSE; // Initialize sent_table FALSE so table will be written to JPEG file. 
}
//
...
GLOBAL(void) jpeg_set_defaults(j_compress_ptr cinfo)
{
	...
	std_huff_tables(cinfo); // Set up two Huffman tables 
	...
	cinfo->scan_info = NULL; /* Default is no multiple-scan output */
	cinfo->num_scans = 0;
	cinfo->raw_data_in = FALSE; /* Expect normal source image, not raw downsampled data */
	...
	cinfo->optimize_coding = FALSE; /* By default, don't do extra passes to optimize entropy coding */
	cinfo->CCIR601_sampling = FALSE; /* By default, use the simpler non-cosited sampling alignment */
	cinfo->do_fancy_downsampling = TRUE; /* By default, apply fancy downsampling */
	cinfo->smoothing_factor = 0; /* No input smoothing */
	cinfo->dct_method = JDCT_DEFAULT; /* DCT algorithm preference */
	/* No restart markers */
...
GLOBAL(void) jpeg_default_colorspace(j_compress_ptr cinfo)
{
	switch(cinfo->in_color_space) {
		case JCS_UNKNOWN: jpeg_set_colorspace(cinfo, JCS_UNKNOWN); break;
		case JCS_GRAYSCALE: jpeg_set_colorspace(cinfo, JCS_GRAYSCALE); break;
		case JCS_RGB: jpeg_set_colorspace(cinfo, JCS_YCbCr); break;
		case JCS_YCbCr: jpeg_set_colorspace(cinfo, JCS_YCbCr); break;
		case JCS_CMYK: jpeg_set_colorspace(cinfo, JCS_CMYK); /* By default, no translation */ break;
		case JCS_YCCK: jpeg_set_colorspace(cinfo, JCS_YCCK); break;
		case JCS_BG_RGB: /* No translation for now -- conversion to BG_YCC not yet supportet */ jpeg_set_colorspace(cinfo, JCS_BG_RGB); break;
		case JCS_BG_YCC: jpeg_set_colorspace(cinfo, JCS_BG_YCC); break;
		default: ERREXIT(cinfo, JERR_BAD_IN_COLORSPACE);
	}
}
/*
...
#define SET_COMP(index, id, hsamp, vsamp, quant, dctbl, actbl)	\
	(compptr = &cinfo->comp_info[index], compptr->component_id = (id), compptr->h_samp_factor = (hsamp), \
	compptr->v_samp_factor = (vsamp), compptr->quant_tbl_no = (quant), compptr->dc_tbl_no = (dctbl), compptr->ac_tbl_no = (actbl) )
	...
	switch(colorspace) {
		...
		case JCS_RGB:
		    ...
		    SET_COMP(0, 0x52 /* 'R' */, 1, 1, 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0);
		    SET_COMP(1, 0x47 /* 'G' */, 1, 1, 0, 0, 0);
		    SET_COMP(2, 0x42 /* 'B' */, 1, 1, 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0);
		    break;
		case JCS_YCbCr:
		    cinfo->write_JFIF_header = TRUE; /* Write a JFIF marker */
		    cinfo->num_components = 3;
		...
		case JCS_BG_RGB:
		    ...
		    SET_COMP(0, 0x72 /* 'r' */, 1, 1, 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0);
		    SET_COMP(1, 0x67 /* 'g' */, 1, 1, 0, 0, 0);
		    SET_COMP(2, 0x62 /* 'b' */, 1, 1, 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0, cinfo->color_transform == JCT_SUBTRACT_GREEN ? 1 : 0);
		    break;
		case JCS_BG_YCC:
		    cinfo->write_JFIF_header = TRUE; /* Write a JFIF marker */
		    cinfo->JFIF_major_version = 2; /* Set JFIF major version = 2 */
...
GLOBAL(void) jpeg_simple_progression(j_compress_ptr cinfo)
{
	...
	if(ncomps == 3 && (cinfo->jpeg_color_space == JCS_YCbCr || cinfo->jpeg_color_space == JCS_BG_YCC)) {
	...
	if(ncomps == 3 && (cinfo->jpeg_color_space == JCS_YCbCr || cinfo->jpeg_color_space == JCS_BG_YCC)) {
		/* Custom script for YCC color images. */
		scanptr = fill_dc_scans(scanptr, ncomps, 0, 1); /* Initial DC scan */
		scanptr = fill_a_scan(scanptr, 0, 1, 5, 0, 2); /* Initial AC scan: get some luma data out in a hurry */
		scanptr = fill_a_scan(scanptr, 2, 1, 63, 0, 1); /* Chroma data is too small to be worth expending many scans on */
		scanptr = fill_a_scan(scanptr, 1, 1, 63, 0, 1);
		scanptr = fill_a_scan(scanptr, 0, 6, 63, 0, 2); /* Complete spectral selection for luma AC */
		scanptr = fill_a_scan(scanptr, 0, 1, 63, 2, 1); /* Refine next bit of luma AC */
		scanptr = fill_dc_scans(scanptr, ncomps, 1, 0); /* Finish DC successive approximation */
		scanptr = fill_a_scan(scanptr, 2, 1, 63, 1, 0); /* Finish AC successive approximation */
		scanptr = fill_a_scan(scanptr, 1, 1, 63, 1, 0);
		scanptr = fill_a_scan(scanptr, 0, 1, 63, 1, 0); /* Luma bottom bit comes last since it's usually largest scan */
	}
	else {
		/* All-purpose script for other color spaces. */
		scanptr = fill_dc_scans(scanptr, ncomps, 0, 1); /* Successive approximation first pass */
		scanptr = fill_scans(scanptr, ncomps, 1, 5, 0, 2);
		scanptr = fill_scans(scanptr, ncomps, 6, 63, 0, 2);
		scanptr = fill_scans(scanptr, ncomps, 1, 63, 2, 1); /* Successive approximation second pass */
		scanptr = fill_dc_scans(scanptr, ncomps, 1, 0); /* Successive approximation final pass */
