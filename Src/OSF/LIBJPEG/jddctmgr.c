...
GLOBAL(void) jinit_inverse_dct(j_decompress_ptr cinfo)
{
	int ci;
	jpeg_component_info * compptr;
	my_idct_ptr idct = (my_idct_ptr)(*cinfo->mem->alloc_small)(reinterpret_cast<j_common_ptr>(cinfo), JPOOL_IMAGE, SIZEOF(my_idct_controller));
	cinfo->idct = &idct->pub;
	idct->pub.start_pass = start_pass;
	for(ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
		/* Allocate and pre-zero a multiplier table for each component */
		compptr->dct_table = (*cinfo->mem->alloc_small)(reinterpret_cast<j_common_ptr>(cinfo), JPOOL_IMAGE, SIZEOF(multiplier_table));
