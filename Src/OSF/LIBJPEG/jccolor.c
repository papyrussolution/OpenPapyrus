...
METHODDEF(void) rgb_ycc_start(j_compress_ptr cinfo)
{
	...
	cconvert->rgb_ycc_tab = rgb_ycc_tab = (INT32*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, (TABLE_SIZE * SIZEOF(INT32)));
...
METHODDEF(void) rgb_ycc_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf, JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		for(JDIMENSION col = 0; col < num_cols; col++) {
			int r = GETJSAMPLE(inptr[RGB_RED]);
			int g = GETJSAMPLE(inptr[RGB_GREEN]);
			int b = GETJSAMPLE(inptr[RGB_BLUE]);
...
METHODDEF(void) rgb_gray_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf, JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		JSAMPROW inptr = *input_buf++;
		JSAMPROW outptr = output_buf[0][output_row++];
		for(JDIMENSION col = 0; col < num_cols; col++) {
			int r = GETJSAMPLE(inptr[RGB_RED]);
			int g = GETJSAMPLE(inptr[RGB_GREEN]);
			int b = GETJSAMPLE(inptr[RGB_BLUE]);
...
METHODDEF(void) cmyk_ycck_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf, JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	...
	while(--num_rows >= 0) {
		...
		JSAMPROW outptr1 = output_buf[1][output_row];
		JSAMPROW outptr2 = output_buf[2][output_row];
		JSAMPROW outptr3 = output_buf[3][output_row];
		output_row++;
		for(JDIMENSION col = 0; col < num_cols; col++) {
			int r = MAXJSAMPLE - GETJSAMPLE(inptr[0]);
			int g = MAXJSAMPLE - GETJSAMPLE(inptr[1]);
			int b = MAXJSAMPLE - GETJSAMPLE(inptr[2]);
 ...
 */
METHODDEF(void) rgb_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf, JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows)
{
	const JDIMENSION num_cols = cinfo->image_width;
	while(--num_rows >= 0) {
		JSAMPROW inptr = *input_buf++;
...
{
	const int nc = cinfo->num_components;
	const JDIMENSION num_cols = cinfo->image_width;
	while(--num_rows >= 0) {
		// It seems fastest to make a separate pass for each component. 
		for(int ci = 0; ci < nc; ci++) {
			JSAMPROW inptr = input_buf[0] + ci;
			JSAMPROW outptr = output_buf[ci][output_row];
			for(JDIMENSION col = 0; col < num_cols; col++) {
...
{
	my_cconvert_ptr cconvert = (my_cconvert_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(my_color_converter));
