...
static void expand_right_edge(JSAMPARRAY image_data, int num_rows, JDIMENSION input_cols, JDIMENSION output_cols)
{
	...
	if(numcols > 0) {
		for(int row = 0; row < num_rows; row++) {
			...
			JSAMPLE pixval = ptr[-1]; /* don't need GETJSAMPLE() here */
			for(int count = numcols; count > 0; count--)
				*ptr++ = pixval;
		}
	}
}
...
{
	my_downsample_ptr downsample = (my_downsample_ptr)cinfo->downsample;
	int inrow, outrow, h, v;
	JDIMENSION outcol, outcol_h; /* outcol_h == outcol*h_expand */
	JDIMENSION output_cols = compptr->width_in_blocks * compptr->DCT_h_scaled_size;
	JSAMPROW inptr, outptr;
	INT32 outvalue;
	int h_expand = downsample->h_expand[compptr->component_index];
	int v_expand = downsample->v_expand[compptr->component_index];
...
METHODDEF(void) fullsize_downsample(j_compress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY output_data)
{
	...
	/* Edge-expand */
	expand_right_edge(output_data, cinfo->max_v_samp_factor, cinfo->image_width, compptr->width_in_blocks * compptr->DCT_h_scaled_size);
}
/*
 * Downsample pixel values of a single component.
...
METHODDEF(void) h2v2_smooth_downsample(j_compress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY output_data)
{
	...
	while(inrow < cinfo->max_v_samp_factor) {
		...
		neighsum = GETJSAMPLE(*above_ptr) + GETJSAMPLE(above_ptr[1]) + GETJSAMPLE(*below_ptr) + 
			GETJSAMPLE(below_ptr[1]) + GETJSAMPLE(*inptr0) + GETJSAMPLE(inptr0[2]) + GETJSAMPLE(*inptr1) + GETJSAMPLE(inptr1[2]);
		neighsum += neighsum;
