...
// 
METHODDEF(JDIMENSION) get_8bit_row (j_compress_ptr cinfo, cjpeg_source_ptr sinfo) // @v9c(done)
...
METHODDEF(JDIMENSION) preload_image(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
	...
	switch(source->bits_per_pixel) {
		case 8: source->pub.get_pixel_rows = get_8bit_row; break;
		case 24: source->pub.get_pixel_rows = get_24bit_row; break;
		case 32: source->pub.get_pixel_rows = get_32bit_row; break;
		default: ERREXIT(cinfo, JERR_BMP_BADDEPTH);
	}
...
#define GET_2B(array, offset)  ((uint)UCH(array[offset]) + (((uint)UCH(array[offset+1])) << 8))
#define GET_4B(array, offset)  ((INT32)UCH(array[offset]) + (((INT32)UCH(array[offset+1])) << 8) + \
	(((INT32)UCH(array[offset+2])) << 16) + (((INT32)UCH(array[offset+3])) << 24))
	INT32 bfOffBits;
	INT32 headerSize;
	INT32 biWidth;
	INT32 biHeight;
	...
	bfOffBits = GET_4B(bmpfileheader, 10); // @v9c /*(INT32)*/
	// We ignore the remaining fileheader fields 
	// The infoheader might be 12 bytes (OS/2 1.x), 40 bytes (Windows),
	// or 64 bytes (OS/2 2.x).  Check the first 4 bytes to find out which.
	if(!ReadOK(source->pub.input_file, bmpinfoheader, 4))
		ERREXIT(cinfo, JERR_INPUT_EOF);
	headerSize = GET_4B(bmpinfoheader, 0); // @v9c /*(INT32)*/
	...
	switch((int)headerSize) {
		case 12:
		    ...
		    switch(source->bits_per_pixel) {
			    case 8: // colormapped image 
					...
					break;
			    case 24: // RGB image 
				case 32: // RGB image + Alpha channel // @v9c 
					// @v9c TRACEMS2(cinfo, 1, JTRC_BMP_OS2, (int)biWidth, (int)biHeight);
					TRACEMS3(cinfo, 1, JTRC_BMP_OS2, (int) biWidth, (int) biHeight, source->bits_per_pixel); // @v9c 
					break;
			    default:
		...
		case 64:
		    ...
		    switch(source->bits_per_pixel) {
				...
				case 24: // RGB image 
				case 32: // RGB image + Alpha channel 
					TRACEMS3(cinfo, 1, JTRC_BMP, (int)biWidth, (int)biHeight, source->bits_per_pixel);
					break;
				// } @v9c 
			    default:
					ERREXIT(cinfo, JERR_BMP_BADDEPTH);
