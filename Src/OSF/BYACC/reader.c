 ...
 */
static int matchec(const char * name)
{
	const char * p = cache;
	const char * q = name;
...
{
	char * s;
	if(strcmp(name, ".") == 0 || strcmp(name, "$accept") == 0 || strcmp(name, "$end") == 0)
		return 1;
...
static char * get_tag(void)
{
	...
	if(ntags >= tagmax) {
		tagmax += 16;
		tag_table = (char **)(tag_table ? REALLOC(tag_table, (unsigned)tagmax*sizeof(char *)) : MALLOC((unsigned)tagmax*sizeof(char *)));
		NO_SPACE(tag_table);
	}
	s = (char *)MALLOC(cinc);
	NO_SPACE(s);
...
static void pack_grammar(void)
{
	...
	rassoc = (Assoc_t *)REALLOC(rassoc, nrules);
	NO_SPACE(rassoc);
