...
#define CREATE_FILE_NAME(dest, suffix) \
        dest = (char *)MALLOC(len+strlen(suffix)+1); \
        NO_SPACE(dest); \
...
static void create_file_names(void)
{
	size_t len;
	char * prefix = NULL;
	const char * defines_suffix = DEFINES_SUFFIX;
	const char * externs_suffix = EXTERNS_SUFFIX;
	/* compute the file_prefix from the user provided output_file_name */
	if(output_file_name != 0) {
		if(!(prefix = strstr(output_file_name, ".tab.c")) && (prefix = strstr(output_file_name, ".c"))) {
			defines_suffix = ".h";
			externs_suffix = ".i";
		}
	}
