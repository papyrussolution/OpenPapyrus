...
{
	fprintf(stderr, "%s: e - line %d of \"%s\", illegal use of reserved symbol %s\n", myname, lineno, input_file_name, s);
	done(1);
}

void tokenized_start(char * s)
{
	fprintf(stderr, "%s: e - line %d of \"%s\", the start symbol %s cannot be declared to be a token\n", myname, lineno, input_file_name, s);
	done(1);
}
...
{
	fprintf(stderr, "%s: w - line %d of \"%s\", the precedence of %s has been redeclared\n", myname, lineno, input_file_name, s);
}
...
{
	fprintf(stderr, "%s: e - line %d of \"%s\", the start symbol %s is a token\n", myname, lineno, input_file_name, s);
	done(1);
}
...
{
	fprintf(stderr, "%s: w - line %d of \"%s\", the default action assigns an undefined value to $$\n", myname, lineno, input_file_name);
}
