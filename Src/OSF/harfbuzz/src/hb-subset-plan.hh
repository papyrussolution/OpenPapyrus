...
struct hb_subset_plan_t {
	...
	bool retain_gids : 1;
	bool name_legacy : 1;
	hb_set_t * unicodes; // For each cp that we'd like to retain maps to the corresponding gid.
	hb_set_t * name_ids; // name_ids we would like to retain
	hb_set_t * name_languages; // name_languages we would like to retain
	hb_set_t * glyphs_requested; //glyph ids requested to retain
	hb_set_t * drop_tables; // Tables which should be dropped.
	hb_map_t * codepoint_to_glyph; // The glyph subset
...
public:
	 ...
	 */
	inline const hb_set_t * glyphset() const
	{
		return _glyphset;
	}
	 ...
	 * The set of input glyph ids which will be retained in the subset.
	 */
	inline const hb_set_t * glyphset_gsub() const
	{
	 ...
	 */
	inline uint num_output_glyphs() const
	{
	...
	inline bool add_table(hb_tag_t tag,
	    hb_blob_t * contents)
	{
...
HB_INTERNAL hb_subset_plan_t * hb_subset_plan_create(hb_face_t           * face,
    hb_subset_input_t   * input);

HB_INTERNAL void hb_subset_plan_destroy(hb_subset_plan_t * plan);
