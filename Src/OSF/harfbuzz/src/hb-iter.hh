...
typename Iter::item_t> {
	...
	void __next__() {
		do ++it; while(it && !hb_has(p.get(), hb_get(f.get(), *it)));
	}
...
template <typename A, typename B> struct hb_zip_iter_t : hb_iter_t<hb_zip_iter_t<A, B>, hb_pair_t<typename A::item_t, typename B::item_t>> { 
	...
	bool operator != (const hb_zip_iter_t &o) const { return a != o.a && b != o.b; }
private:
	A a;
	B b;
};
...
struct {
	template <typename Sink> hb_sink_t<Sink> operator() (Sink&& s) const { return hb_sink_t<Sink> (s); }
	template <typename Sink> hb_sink_t<Sink&> operator() (Sink *s) const { return hb_sink_t<Sink&> (*s); }
}
...
struct {
	...
	operator() (Sink1&& s1, Sink2&& s2) const { return hb_unzip_t<Sink1, Sink2> (s1, s2); }
