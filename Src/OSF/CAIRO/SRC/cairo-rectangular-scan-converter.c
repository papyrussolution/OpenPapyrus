...
static inline void add_cell(sweep_line_t * sweep, int x, int covered, int uncovered)
{
	...
	if(x != cell->x) {
		SweepLine::Coverage::Cell * c;
		sweep->coverage.count++;
		c = (SweepLine::Coverage::Cell *)_cairo_freepool_alloc(&sweep->coverage.pool);
		if(unlikely(c == NULL)) {
...
static void render_rows(sweep_line_t * sweep_line,
...
static cairo_status_t generate(cairo_rectangular_scan_converter_t * self,
    cairo_span_renderer_t * renderer,
...
static cairo_status_t generate_box(cairo_rectangular_scan_converter_t * self,
    cairo_span_renderer_t     * renderer)
{
	const rectangle_t * r = (const rectangle_t *)self->chunks.base;
...
static cairo_status_t _cairo_rectangular_scan_converter_generate(void * converter, cairo_span_renderer_t * renderer)
{
	...
	rectangle_t ** rectangles;
	_cairo_rectangular_scan_converter::_cairo_rectangular_scan_converter_chunk * chunk;
	cairo_status_t status;
	int i, j;
...
static rectangle_t * _allocate_rectangle(cairo_rectangular_scan_converter_t * self)
{
	...
	if(chunk->count == chunk->size) {
		...
		chunk->next = (_cairo_rectangular_scan_converter::_cairo_rectangular_scan_converter_chunk *)_cairo_malloc_ab_plus_c(size,
			sizeof(rectangle_t), sizeof(_cairo_rectangular_scan_converter::_cairo_rectangular_scan_converter_chunk));
