...
struct SweepLineCoverageCell {
	struct SweepLineCoverageCell * prev;
	struct SweepLineCoverageCell * next;
...
typedef struct {
	...
	struct coverage {
		struct SweepLineCoverageCell head, tail, * cursor;			
		unsigned int count;
...
static inline rectangle_t * peek_stop(sweep_line_t * sweep)
{
...
static void sweep_line_init(sweep_line_t * sweep)
{
	...
	_cairo_freepool_init(&sweep->coverage.pool, sizeof(struct SweepLineCoverageCell));
...
static void sweep_line_fini(sweep_line_t * sweep)
{
...
{
	struct SweepLineCoverageCell * cell = sweep->coverage.cursor;
	if(cell->x > x) {
		do {
			UNROLL3({ if(cell->prev->x < x) break; cell = cell->prev; })
		} while(TRUE);
	}
	else {
		if(cell->x == x)
			goto found;
		do {
			UNROLL3({ cell = cell->next; if(cell->x >= x) break; })
		} while(TRUE);
	}
	if(x != cell->x) {
		struct SweepLineCoverageCell * c;
		sweep->coverage.count++;
		c = (struct SweepLineCoverageCell *)_cairo_freepool_alloc(&sweep->coverage.pool);
		if(unlikely(c == NULL)) {
			longjmp(sweep->jmpbuf, _cairo_error(CAIRO_STATUS_NO_MEMORY));
		}
		cell->prev->next = c;
...
static inline void _active_edges_to_spans(sweep_line_t * sweep)
{
	...
	int prev_x;
	struct SweepLineCoverageCell * cell;
	sweep->num_spans = 0;
	 ...
	 */
	for(rectangle = sweep->head.next; rectangle != &sweep->tail; rectangle = rectangle->next) {
		int height;
		int frac, i;
	...
	if(2*sweep->coverage.count >= sweep->size_spans) {
		unsigned size = sweep->size_spans;
...
static inline void sweep_line_insert(sweep_line_t * sweep, rectangle_t * rectangle)
{
	...
	if(pos->left != rectangle->left) {
		if(pos->left > rectangle->left) {
			do {
				UNROLL3({ if(pos->prev->left < rectangle->left) break; pos = pos->prev; })
			} while(TRUE);
		}
		else {
			do {
				UNROLL3({ pos = pos->next; if(pos->left >= rectangle->left) break; });
			} while(TRUE);
		}
	}
...
static void render_rows(sweep_line_t * sweep_line, cairo_span_renderer_t * renderer, int height)
{
...
static cairo_status_t generate(cairo_rectangular_scan_converter_t * self, cairo_span_renderer_t * renderer, rectangle_t ** rectangles)
{
	...
	if((status = (cairo_status_t)setjmp(sweep_line.jmpbuf)))
		goto BAIL;
...
out:
	status =  renderer->render_rows(renderer, sweep_line.current_y, self->ymax-sweep_line.current_y, NULL, 0);
...
static cairo_status_t _cairo_rectangular_scan_converter_generate(void * converter, cairo_span_renderer_t * renderer)
{
	cairo_rectangular_scan_converter_t * self = (cairo_rectangular_scan_converter_t *)converter;
	...
	if(unlikely(self->num_rectangles == 0)) {
		return renderer->render_rows(renderer, self->ymin, self->ymax-self->ymin, NULL, 0);
	}
...
static rectangle_t * _allocate_rectangle(cairo_rectangular_scan_converter_t * self)
{
	rectangle_t * rectangle;
	struct _cairo_rectangular_scan_converter_chunk * chunk = (struct _cairo_rectangular_scan_converter_chunk *)self->tail;
	if(chunk->count == chunk->size) {
		int size = chunk->size*2;
		chunk->next = (struct _cairo_rectangular_scan_converter_chunk *)_cairo_malloc_ab_plus_c(
			size, sizeof(rectangle_t), sizeof(struct _cairo_rectangular_scan_converter_chunk));
	...
	rectangle = (rectangle_t *)chunk->base;
...
static void _cairo_rectangular_scan_converter_destroy(void * converter)
{
	cairo_rectangular_scan_converter_t * self = (cairo_rectangular_scan_converter_t *)converter;
