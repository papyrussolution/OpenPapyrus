...
cairo_surface_t * _cairo_raster_source_pattern_acquire(const cairo_pattern_t * abstract_pattern, cairo_surface_t * target, const CairoIRect * extents)
{
	cairo_raster_source_pattern_t * pattern = (cairo_raster_source_pattern_t*)abstract_pattern;
	if(pattern->acquire == NULL)
		return NULL;
	SETIFZ(extents, &pattern->extents);
...
{
	cairo_raster_source_pattern_t * pattern = (cairo_raster_source_pattern_t*)abstract_pattern;
	return pattern->snapshot ? pattern->snapshot(&pattern->base, pattern->user_data) : CAIRO_STATUS_SUCCESS;
}
