...
cairo_surface_t * cairo_recording_surface_create(cairo_content_t content, const cairo_rectangle_t * extents)
{
	...
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)malloc(sizeof(cairo_recording_surface_t));
	if(unlikely(recording_surface == NULL))
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
	_cairo_surface_init(&recording_surface->base, &cairo_recording_surface_backend, NULL, /* device */ content);
	...
	if(extents != NULL) {
		...
		recording_surface->extents.x = (int)floor(extents->x);
		recording_surface->extents.y = (int)floor(extents->y);
		recording_surface->extents.width = (int)ceil(extents->x+extents->width)-recording_surface->extents.x;
		recording_surface->extents.height = (int)ceil(extents->y+extents->height)-recording_surface->extents.y;
		status = _cairo_clip_rectangle(&recording_surface->clip, &recording_surface->extents);
		if(unlikely(status)) {
...
{
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	int i;
	int num_elements = recording_surface->commands.num_elements;
	cairo_command_t ** elements = (cairo_command_t **)_cairo_array_index(&recording_surface->commands, 0);
	...
	cairo_image_surface_t ** image_out, void ** image_extra)
{
	...
	cairo_surface_t * image = _cairo_surface_has_snapshot(&surface->base, &_cairo_image_surface_backend);
	if(image != NULL) {
		*image_out = (cairo_image_surface_t *)cairo_surface_reference(image);
		*image_extra = NULL;
		return CAIRO_STATUS_SUCCESS;
	}
	image = _cairo_image_surface_create_with_content(surface->content, surface->extents.width, surface->extents.height);
...
static cairo_status_t _command_init(cairo_recording_surface_t * recording_surface,
	cairo_command_header_t * command, cairo_command_type_t type,
	cairo_operator_t op, cairo_clip_t * clip)
{
	...
	cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	cairo_command_paint_t * command = (cairo_command_paint_t *)malloc(sizeof(cairo_command_paint_t));
	if(unlikely(command == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _command_init(recording_surface, &command->header, CAIRO_COMMAND_PAINT, op, clip);
	if(unlikely(status))
	...
	cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	cairo_command_mask_t * command = (cairo_command_mask_t *)malloc(sizeof(cairo_command_mask_t));
	if(unlikely(command == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _command_init(recording_surface, &command->header, CAIRO_COMMAND_MASK, op, clip);
	if(unlikely(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(unlikely(status))
	...
	cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	cairo_command_stroke_t * command = (cairo_command_stroke_t *)malloc(sizeof(cairo_command_stroke_t));
	if(unlikely(command == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _command_init(recording_surface, &command->header, CAIRO_COMMAND_STROKE, op, clip);
	if(unlikely(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(unlikely(status))
	...
	cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	cairo_command_fill_t * command = (cairo_command_fill_t *)malloc(sizeof(cairo_command_fill_t));
	if(unlikely(command == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _command_init(recording_surface, &command->header, CAIRO_COMMAND_FILL, op, clip);
	if(unlikely(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(unlikely(status))
	...
	cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_recording_surface_t * recording_surface = (cairo_recording_surface_t *)abstract_surface;
	cairo_command_show_text_glyphs_t * command = (cairo_command_show_text_glyphs_t *)malloc(sizeof(cairo_command_show_text_glyphs_t));
	if(unlikely(command == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = _command_init(recording_surface, &command->header, CAIRO_COMMAND_SHOW_TEXT_GLYPHS, op, clip);
	if(unlikely(status))
		goto CLEANUP_COMMAND;
	status = _cairo_pattern_init_snapshot(&command->source.base, source);
	if(unlikely(status))
...
static cairo_surface_t * _cairo_recording_surface_snapshot(void * abstract_other)
{
	...
	if(unlikely(recording_surface == NULL))
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
	_cairo_surface_init(&recording_surface->base, &cairo_recording_surface_backend, NULL, /* device */ other->base.content);
 ...
 **/
cairo_bool_t _cairo_surface_is_recording(const cairo_surface_t * surface)
{
...
cairo_int_status_t _cairo_recording_surface_get_path(cairo_surface_t * surface,
	cairo_path_fixed_t * path)
{
	...
	elements = (cairo_command_t **)_cairo_array_index(&recording_surface->commands, 0);
		...
		switch(command->header.type) {
		    ...
		    case CAIRO_COMMAND_STROKE:
		    {
			    ...
			    status = _cairo_path_fixed_stroke_to_traps(&command->stroke.path,
				    &command->stroke.style, &command->stroke.ctm, &command->stroke.ctm_inverse,
				    command->stroke.tolerance, &traps);
...
static cairo_status_t _cairo_recording_surface_replay_internal(cairo_surface_t * surface,
	...
	for(i = recording_surface->replay_start_idx; i < num_elements; i++) {
		...
		switch(command->header.type) {
		    ...
		    case CAIRO_COMMAND_SHOW_TEXT_GLYPHS:
		    {
			     ...
			     */
			    cairo_glyph_t * glyphs_copy = (cairo_glyph_t *)_cairo_malloc_ab(num_glyphs, sizeof(cairo_glyph_t));
...
{
	return _cairo_recording_surface_replay_internal(surface, NULL, target, CAIRO_RECORDING_REPLAY, CAIRO_RECORDING_REGION_ALL);
}
...
{
	return _cairo_recording_surface_replay_internal(surface, NULL, target, CAIRO_RECORDING_CREATE_REGIONS, CAIRO_RECORDING_REGION_ALL);
}

cairo_status_t _cairo_recording_surface_replay_region(cairo_surface_t * surface,
	const cairo_rectangle_int_t * surface_extents, cairo_surface_t * target, cairo_recording_region_type_t region)
{	
	return _cairo_recording_surface_replay_internal(surface, surface_extents,
		target, CAIRO_RECORDING_REPLAY, region);
}
...
{
	cairo_status_t status;
	cairo_surface_t * null_surface = _cairo_null_surface_create(surface->content);
