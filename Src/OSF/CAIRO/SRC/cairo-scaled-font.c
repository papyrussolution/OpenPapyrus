...
static void _cairo_scaled_glyph_fini(cairo_scaled_font_t * scaled_font,
...
static const cairo_scaled_font_t _cairo_scaled_font_nil = {
	...
	{ CAIRO_ANTIALIAS_DEFAULT, /* options */
	  ...
	  CAIRO_LCD_FILTER_DEFAULT,  // @sobolev CAIRO_HINT_STYLE_DEFAULT-->CAIRO_LCD_FILTER_DEFAULT
	  CAIRO_HINT_STYLE_DEFAULT}, // @sobolev CAIRO_HINT_METRICS_DEFAULT-->CAIRO_HINT_STYLE_DEFAULT
	FALSE,                  /* placeholder */
	FALSE,                  /* holdover */
	TRUE,                   /* finished */
	{ 1., 0., 0., 1., 0, 0}, /* scale */
...
{
	return (CAIRO_REFERENCE_COUNT_IS_INVALID(&scaled_font->ref_count)) ? CAIRO_FONT_TYPE_TOY : scaled_font->backend->type;
}
...
static void _cairo_scaled_glyph_page_destroy(void * closure)
{
	cairo_scaled_glyph_page_t * page = (cairo_scaled_glyph_page_t *)closure;
	unsigned int n;
	cairo_scaled_font_t * scaled_font = (cairo_scaled_font_t *)page->cache_entry.hash;
...
cairo_status_t _cairo_scaled_font_register_placeholder_and_unlock_font_map(cairo_scaled_font_t * scaled_font)
{
	...
	status = _cairo_scaled_font_init(placeholder_scaled_font,
		scaled_font->font_face, &scaled_font->font_matrix,
		&scaled_font->ctm, &scaled_font->options, NULL);
	if(unlikely(status))
...
static cairo_bool_t _cairo_scaled_font_keys_equal(const void * abstract_key_a, const void * abstract_key_b)
{
	...
	if(key_a->hash_entry.hash != key_b->hash_entry.hash)
		return FALSE;
	return key_a->font_face == key_b->font_face && memcmp((unsigned char *)(&key_a->font_matrix.xx),
		(unsigned char *)(&key_b->font_matrix.xx), sizeof(cairo_matrix_t)) == 0 && memcmp((unsigned char *)(&key_a->ctm.xx),
		(unsigned char *)(&key_b->ctm.xx), sizeof(cairo_matrix_t)) == 0 && cairo_font_options_equal(&key_a->options, &key_b->options);
}

static cairo_bool_t _cairo_scaled_font_matches(const cairo_scaled_font_t * scaled_font,
	const cairo_font_face_t * font_face, const cairo_matrix_t * font_matrix,
	const cairo_matrix_t * ctm, const cairo_font_options_t * options)
{
	return scaled_font->original_font_face == font_face && memcmp((unsigned char *)(&scaled_font->font_matrix.xx),
		(unsigned char *)(&font_matrix->xx), sizeof(cairo_matrix_t)) == 0 &&
		memcmp((unsigned char *)(&scaled_font->ctm.xx), (unsigned char *)(&ctm->xx), sizeof(cairo_matrix_t)) == 0 &&
		cairo_font_options_equal(&scaled_font->options, options);
}

static cairo_bool_t _cairo_scaled_glyphs_equal(const void * abstract_a, const void * abstract_b)
{
	const cairo_scaled_glyph_t * a = (const cairo_scaled_glyph_t *)abstract_a;
	const cairo_scaled_glyph_t * b = (const cairo_scaled_glyph_t *)abstract_b;
	...
	const cairo_scaled_font_backend_t * backend)
{
	...
	if(unlikely(status))
		return status;
	_cairo_scaled_font_init_key(scaled_font, font_face, font_matrix, ctm, options);
...
void _cairo_scaled_font_reset_cache(cairo_scaled_font_t * scaled_font)
{
	...
	while(!cairo_list_is_empty(&scaled_font->glyph_pages)) {
		_cairo_cache_remove(&cairo_scaled_glyph_page_cache, &cairo_list_first_entry(&scaled_font->glyph_pages, cairo_scaled_glyph_page_t, link)->cache_entry);
	}
	CAIRO_MUTEX_UNLOCK(_cairo_scaled_glyph_page_cache_mutex);
}
...
static void _cairo_scaled_font_fini_internal(cairo_scaled_font_t * scaled_font)
{
	...
	if(scaled_font->surface_backend != NULL && scaled_font->surface_backend->scaled_font_fini != NULL)
...
void _cairo_scaled_font_revoke_ownership(cairo_scaled_font_t * scaled_font)
{
	...
	cairo_text_extents_t * extents)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		status = _cairo_scaled_glyph_lookup(scaled_font,
			glyphs[i].index, CAIRO_SCALED_GLYPH_INFO_METRICS, &scaled_glyph);
		if(unlikely(status)) {
			status = _cairo_scaled_font_set_error(scaled_font, status);
			goto UNLOCK;
		}
	...
	cairo_text_cluster_flags_t * cluster_flags)
{
	...
	if(cluster_flags) {
		*cluster_flags = CAIRO_TEXT_CLUSTER_FLAG_UNDEF;
	}
...
void _cairo_scaled_font_glyph_approximate_extents(cairo_scaled_font_t * scaled_font,
	const cairo_glyph_t * glyphs, int num_glyphs, cairo_rectangle_int_t * extents)
{
	...
	if(x0 <= x1 && y0 <= y1) {
		extents->x = (int)floor(x0-scaled_font->extents.max_x_advance);
		extents->width = (int)ceil(x1+scaled_font->extents.max_x_advance);
		extents->width -= extents->x;
		extents->y = (int)floor(y0-scaled_font->extents.ascent);
		extents->height = (int)ceil(y1+scaled_font->extents.descent);
...
static cairo_status_t _trace_mask_to_path(cairo_image_surface_t * mask,
	cairo_path_fixed_t * path, double tx, double ty)
{
	const uint8_t * row;
	...
	const cairo_glyph_t * glyphs, int num_glyphs, cairo_path_fixed_t * path)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(status == CAIRO_STATUS_SUCCESS) {
			status = _cairo_path_fixed_append(path, scaled_glyph->path, CAIRO_DIRECTION_FORWARD,
	...
	cairo_scaled_font_t * scaled_font, cairo_text_extents_t * fs_metrics)
{
	...
	for(hm = 0.0; hm <= 1.0; hm += 1.0)
		for(wm = 0.0; wm <= 1.0; wm += 1.0) {
			...
			double x = fs_metrics->x_bearing+fs_metrics->width*wm;
			double y = fs_metrics->y_bearing+fs_metrics->height*hm;
...
void _cairo_scaled_glyph_set_path(cairo_scaled_glyph_t * scaled_glyph,
	cairo_scaled_font_t * scaled_font, cairo_path_fixed_t * path)
{
	if(scaled_glyph->path != NULL)
		_cairo_path_fixed_destroy(scaled_glyph->path);
...
static cairo_status_t _cairo_scaled_font_allocate_glyph(cairo_scaled_font_t * scaled_font,
	cairo_scaled_glyph_t ** scaled_glyph)
{
	...
	if(scaled_font->global_cache_frozen == FALSE) {
		if(unlikely(cairo_scaled_glyph_page_cache.hash_table == NULL)) {
			status = _cairo_cache_init(&cairo_scaled_glyph_page_cache,
				NULL, _cairo_scaled_glyph_page_can_remove, _cairo_scaled_glyph_page_destroy, MAX_GLYPH_PAGES_CACHED);
			if(unlikely(status)) {
				CAIRO_MUTEX_UNLOCK(_cairo_scaled_glyph_page_cache_mutex);
				free(page);
				return status;
...
cairo_int_status_t _cairo_scaled_glyph_lookup(cairo_scaled_font_t * scaled_font,
	unsigned long index, cairo_scaled_glyph_info_t info, cairo_scaled_glyph_t ** scaled_glyph_ret)
{
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_scaled_glyph_t * scaled_glyph;
