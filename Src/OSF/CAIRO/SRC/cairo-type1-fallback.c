 ...
 */
static void charstring_encode_integer(cairo_array_t * data,
	int i,
...
static void charstring_encrypt(cairo_array_t * data)
{
	...
	unsigned char * end = d+_cairo_array_num_elements(data);
	while(d < end) {
		p = *d;
		c = p^(r>>8);
...
static cairo_int_status_t cairo_type1_font_create_charstring(cairo_type1_font_t * font,
	int subset_index, int glyph_index, cairo_charstring_type_t type, cairo_array_t * data)
{
	...
	status = _cairo_scaled_glyph_lookup(font->type1_scaled_font,
		glyph_index, CAIRO_SCALED_GLYPH_INFO_METRICS|CAIRO_SCALED_GLYPH_INFO_PATH, &scaled_glyph);
	...
	if(glyph_index == 0 && status == CAIRO_INT_STATUS_UNSUPPORTED) {
		emit_path = FALSE;
		status = _cairo_scaled_glyph_lookup(font->type1_scaled_font,
			glyph_index, CAIRO_SCALED_GLYPH_INFO_METRICS, &scaled_glyph);
	}
	if(unlikely(status))
		return status;
	...
	cairo_output_stream_t * encrypted_output)
{
	...
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		...
		status = cairo_type1_font_create_charstring(font, i,
			font->scaled_font_subset->glyphs[i], CAIRO_CHARSTRING_TYPE1, &data);
		if(unlikely(status))
			break;
		...
		if(font->scaled_font_subset->glyph_names != NULL) {
			_cairo_output_stream_printf(encrypted_output, "/%s %d RD ",
				font->scaled_font_subset->glyph_names[i], length);
		}
		else if(i == 0) {
...
static cairo_status_t cairo_type1_write_stream_encrypted(void * closure, const unsigned char * data, unsigned int length)
{
	uint16_t c, p;
...
static cairo_int_status_t cairo_type1_font_write_private_dict(cairo_type1_font_t * font,
	const char * name)
{
	cairo_int_status_t status;
	...
	encrypted_output = _cairo_output_stream_create(
		cairo_type1_write_stream_encrypted, NULL, font);
...
static cairo_status_t cairo_type1_write_stream(void * closure, const unsigned char * data, unsigned int length)
{
	cairo_type1_font_t * font = (cairo_type1_font_t *)closure;
...
static cairo_status_t _cairo_type1_fallback_init_internal(cairo_type1_subset_t * type1_subset,
	const char * name, cairo_scaled_font_subset_t * scaled_font_subset, cairo_bool_t hex_encode)
{
...
cairo_status_t _cairo_type1_fallback_init_binary(cairo_type1_subset_t * type1_subset,
	const char * name, cairo_scaled_font_subset_t * scaled_font_subset)
{
	...
	cairo_scaled_font_subset_t * scaled_font_subset)
{
	...
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		...
		status = cairo_type1_font_create_charstring(font, i,
			...
			CAIRO_CHARSTRING_TYPE2, &charstring);
		if(unlikely(status))
			goto fail2;
...
void _cairo_type2_charstrings_fini(cairo_type2_charstrings_t * type2_subset)
{
	unsigned int i;
	cairo_array_t * charstring;
	unsigned int num_charstrings = _cairo_array_num_elements(&type2_subset->charstrings);
