...
cairo_private void _cairo_box_from_doubles(cairo_box_t * box,
...
cairo_bool_t _cairo_rectangle_intersect(cairo_rectangle_int_t * dst, const cairo_rectangle_int_t * src)
{
	 ...
	 */
	int x2 = MIN(dst->x+(int)dst->width,  src->x+(int)src->width);
	int y2 = MIN(dst->y+(int)dst->height, src->y+(int)src->height);
	if(x1 >= x2 || y1 >= y2) {
		dst->x = 0;
		dst->y = 0;
		dst->width  = 0;
...
cairo_bool_t _cairo_box_intersects_line_segment(cairo_box_t * box, cairo_line_t * line)
{
	cairo_fixed_t t1 = 0, t2 = 0, t3 = 0, t4 = 0;
	cairo_int64_t t1y, t2y, t3x, t4x;
	cairo_fixed_t xlen, ylen;
	if(_cairo_box_contains_point(box, &line->p1) || _cairo_box_contains_point(box, &line->p2))
		return TRUE;
	xlen = P2x-P1x;
	ylen = P2y-P1y;
	if(xlen) {
...
cairo_bool_t _cairo_box_contains_point(cairo_box_t * box, const cairo_point_t * point)
{
	return (point->x < box->p1.x || point->x > box->p2.x || point->y < box->p1.y || point->y > box->p2.y) ? FALSE : TRUE;
}
