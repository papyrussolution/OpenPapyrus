...
static const cairo_t _cairo_nil[] = {
    ...
    DEFINE_NIL_CONTEXT(CAIRO_STATUS_PNG_ERROR), // 1.16.0
    DEFINE_NIL_CONTEXT(CAIRO_STATUS_FREETYPE_ERROR), // 1.16.0
    DEFINE_NIL_CONTEXT(CAIRO_STATUS_WIN32_GDI_ERROR), // 1.16.0
    DEFINE_NIL_CONTEXT(CAIRO_STATUS_TAG_ERROR) // 1.16.0
};
...
cairo_t * _cairo_create_in_error(cairo_status_t status)
{
	assert(status != CAIRO_STATUS_SUCCESS);
	cairo_t * cr = (cairo_t*)&_cairo_nil[status - CAIRO_STATUS_NO_MEMORY];
...
void _cairo_init(cairo_t * cr, const cairo_backend_t * backend)
{
...
{
	return (CAIRO_REFERENCE_COUNT_IS_INVALID(&cr->ref_count)) ? cr->status : _cairo_user_data_array_set_data(&cr->user_data, key, user_data, destroy);
}
...
{
	return (!cr || CAIRO_REFERENCE_COUNT_IS_INVALID(&cr->ref_count)) ? 0 : CAIRO_REFERENCE_COUNT_GET_VALUE(&cr->ref_count);
}
...
void cairo_restore(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->restore(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_push_group_with_content(cairo_t * cr, cairo_content_t content)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->push_group(cr, content);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
cairo_pattern_t * cairo_pop_group(cairo_t * cr)
{
	...
	cairo_pattern_t * group_pattern = cr->backend->pop_group(cr);
...
{
	cairo_pattern_t * group_pattern = cairo_pop_group(cr);
...
void cairo_set_operator(cairo_t * cr, cairo_operator_t op)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->set_operator(cr, op);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_set_source_rgb(cairo_t * cr, double red, double green, double blue)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->set_source_rgba(cr, red, green, blue, 1.);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_set_source_rgba(cairo_t * cr, double red, double green, double blue, double alpha)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->set_source_rgba(cr, red, green, blue, alpha);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
 ...
 * cairo_set_fill_rule:
 * @cr: a #cairo_t
 * @fill_rule: a fill rule, specified as a #CairoFillRule
 *
 * Set the current fill rule within the cairo context. The fill rule
 * is used to determine which regions are inside or outside a complex
 * (potentially self-intersecting) path. The current fill rule affects
 * both cairo_fill() and cairo_clip(). See #CairoFillRule for details
...
void FASTCALL cairo_new_sub_path(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->new_sub_path(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
void FASTCALL cairo_move_to(cairo_t * cr, double x, double y)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->move_to(cr, x, y);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void FASTCALL cairo_line_to(cairo_t * cr, double x, double y)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->line_to(cr, x, y);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_curve_to(cairo_t * cr, double x1, double y1, double x2, double y2, double x3, double y3)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->curve_to(cr, x1, y1, x2, y2, x3, y3);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_rel_move_to(cairo_t * cr, double dx, double dy)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->rel_move_to(cr, dx, dy);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_rel_line_to(cairo_t * cr, double dx, double dy)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->rel_line_to(cr, dx, dy);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_rectangle(cairo_t * cr, double x, double y, double width, double height)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->rectangle(cr, x, y, width, height);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_stroke_to_path(cairo_t * cr)
{
	...
	cairo_status_t status = _cairo_gstate_stroke_path(cr->gstate);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void FASTCALL cairo_close_path(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->close_path(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_paint(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->paint(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_paint_with_alpha(cairo_t * cr, double alpha)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->paint_with_alpha(cr, alpha);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_mask(cairo_t * cr, cairo_pattern_t * pattern)
{
	...
	cairo_status_t status = cr->backend->mask(cr, pattern);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_stroke(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->stroke(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_stroke_preserve(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->stroke_preserve(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_fill(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->fill(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
void cairo_fill_preserve(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->fill_preserve(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_copy_page(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->copy_page(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_show_page(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->show_page(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
void cairo_stroke_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	cairo_status_t status = cr->backend->stroke_extents(cr, x1, y1, x2, y2);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_fill_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	cairo_status_t status = cr->backend->fill_extents(cr, x1, y1, x2, y2);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_clip(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->clip(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
void cairo_clip_preserve(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->clip_preserve(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_reset_clip(cairo_t * cr)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->reset_clip(cr);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
void cairo_clip_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->clip_extents(cr, x1, y1, x2, y2);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
/**
...
{
	return (unlikely(cr->status)) ? _cairo_rectangle_list_create_in_error(cr->status) : cr->backend->clip_copy_rectangle_list(cr);
}
...
void cairo_font_extents(cairo_t * cr, cairo_font_extents_t * extents)
{
	...
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->font_extents(cr, extents);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
{
	return (unlikely(cr->status)) ? (cairo_font_face_t*)&_cairo_font_face_nil : cr->backend->get_font_face(cr);
}
/**
 * cairo_set_font_size:
 * @cr: a #cairo_t
...
void cairo_set_font_size(cairo_t * cr, double size)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->set_font_size(cr, size);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_set_font_matrix(cairo_t * cr, const cairo_matrix_t * matrix)
{
	if(unlikely(cr->status))
		return;
	cairo_status_t status = cr->backend->set_font_matrix(cr, matrix);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
{
	return (unlikely(cr->status)) ? _cairo_scaled_font_create_in_error(cr->status) : cr->backend->get_scaled_font(cr);
}
...
void cairo_text_extents(cairo_t * cr, const char * utf8, cairo_text_extents_t * extents)
{
	...
	int num_glyphs = 0;
	double x, y;
	extents->Clear(); // @v9.1.12
	/* @v9.1.12 extents->x_bearing = 0.0;
...
void cairo_show_text(cairo_t * cr, const char * utf8)
{
	...
	cairo_get_current_point(cr, &x, &y);
	status = cairo_scaled_font_text_to_glyphs(scaled_font, x, y, utf8, utf8_len, &glyphs, &num_glyphs,
	    has_show_text_glyphs ? &clusters : NULL, &num_clusters, &cluster_flags);
	if(unlikely(status))
		goto BAIL;
	if(num_glyphs == 0)
		return;
 ...
 * Since: 1.8
 **/
void cairo_show_text_glyphs(cairo_t * cr, const char * utf8,
    int utf8_len, const cairo_glyph_t * glyphs, int num_glyphs,
...
{
	return (unlikely(cr->status)) ? CAIRO_GSTATE_OPERATOR_DEFAULT : cr->backend->get_operator(cr);
}
...
{
	return (unlikely(cr->status)) ? 1.0 : cr->backend->get_opacity(cr);
}
...
{
	return (unlikely(cr->status)) ? CAIRO_GSTATE_TOLERANCE_DEFAULT : cr->backend->get_tolerance(cr);
}
...
{
	return (unlikely(cr->status)) ? CAIRO_ANTIALIAS_DEFAULT : cr->backend->get_antialias(cr);
}
...
{
	return (unlikely(cr->status)) ? FALSE : cr->backend->has_current_point(cr);
}
...
void cairo_get_current_point(cairo_t * cr, double * x_ret, double * y_ret)
{
	double x = 0.0;
	double y = 0.0;
	if(cr->status == CAIRO_STATUS_SUCCESS && cr->backend->has_current_point(cr)) {
...
{
	return (unlikely(cr->status)) ? CAIRO_GSTATE_FILL_RULE_DEFAULT : cr->backend->get_fill_rule(cr);
}
...
{
	return (unlikely(cr->status)) ? CAIRO_GSTATE_LINE_WIDTH_DEFAULT : cr->backend->get_line_width(cr);
}
