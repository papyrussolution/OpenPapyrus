...
static const cairo_t _cairo_nil = {
	...
	NULL,                   /* gstate */
	{{ (cairo_operator_t)0 }, { (cairo_operator_t)0 }}, /* gstate_tail */
...
static cairo_t * _context_get(void)
{
	int avail = ffs(~_context_stash.occupied)-1;
...
static void _context_put(cairo_t * cr)
{
	if(cr < &_context_stash.pool[0] || cr >= &_context_stash.pool[CAIRO_STASH_SIZE]) {
		free(cr);
		return;
	}
...
static cairo_t * _cairo_create_in_error(cairo_status_t status)
{
	cairo_t * cr;
	assert(status != CAIRO_STATUS_SUCCESS);
	/* special case OOM in order to avoid another allocation */
	switch((int)status) {
	    case CAIRO_STATUS_NO_MEMORY: return (cairo_t *)&_cairo_nil;
	    case CAIRO_STATUS_NULL_POINTER: return (cairo_t *)&_cairo_nil__null_pointer;
	}
...
void _cairo_reset_static_data(void)
{
	int status;
	CAIRO_MUTEX_LOCK(_cairo_error_mutex);
	...
	void * user_data, cairo_destroy_func_t destroy)
{
	if(CAIRO_REFERENCE_COUNT_IS_INVALID(&cr->ref_count))
		return cr->status;
	return _cairo_user_data_array_set_data(&cr->user_data, key, user_data, destroy);
}
...
void cairo_push_group_with_content(cairo_t * cr, cairo_content_t content)
{
	...
	if(clip->all_clipped) {
		...
		cairo_bool_t is_empty = _cairo_surface_get_extents(parent_surface, &extents);
		const cairo_rectangle_int_t * clip_extents = _cairo_clip_get_extents(_cairo_gstate_get_clip(cr->gstate));
		if(clip_extents != NULL)
			is_empty = _cairo_rectangle_intersect(&extents, clip_extents);
		group_surface = _cairo_surface_create_similar_solid(parent_surface,
			content, extents.width, extents.height, CAIRO_COLOR_TRANSPARENT, TRUE);
		status = group_surface->status;
		if(unlikely(status))
			goto bail;
...
cairo_pattern_t * cairo_pop_group(cairo_t * cr)
{
	...
	cairo_matrix_multiply(&device_transform_matrix, &_cairo_gstate_get_target(cr->gstate)->device_transform,
...
static cairo_bool_t _current_source_matches_solid(cairo_t * cr, double red, double green, double blue, double alpha)
{
	cairo_color_t color;
	const cairo_pattern_t * current = cr->gstate->source;
	...
	return _cairo_color_equal(&color, &((cairo_solid_pattern_t *)current)->color);
}
...
void cairo_set_dash(cairo_t * cr, const double * dashes, int num_dashes, double offset)
{
	...
	if(unlikely(cr->status))
		return;
	status = _cairo_gstate_set_dash(cr->gstate, dashes, num_dashes, offset);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_curve_to(cairo_t * cr, double x1, double y1, double x2, double y2, double x3, double y3)
{
	...
	status = _cairo_path_fixed_curve_to(cr->path, x1_fixed, y1_fixed, x2_fixed, y2_fixed, x3_fixed, y3_fixed);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_rel_curve_to(cairo_t * cr, double dx1, double dy1, double dx2, double dy2, double dx3, double dy3)
{
	...
	status = _cairo_path_fixed_rel_curve_to(cr->path, dx1_fixed, dy1_fixed, dx2_fixed, dy2_fixed, dx3_fixed, dy3_fixed);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
cairo_bool_t cairo_in_stroke(cairo_t * cr, double x, double y)
{
	...
	if(unlikely(cr->status))
		return FALSE;
	status = _cairo_gstate_in_stroke(cr->gstate, cr->path, x, y, &inside);
	if(unlikely(status))
		_cairo_set_error(cr, status);
	return inside;
}
...
void cairo_stroke_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	status = _cairo_gstate_stroke_extents(cr->gstate, cr->path, x1, y1, x2, y2);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_fill_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	status = _cairo_gstate_fill_extents(cr->gstate, cr->path, x1, y1, x2, y2);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_clip_extents(cairo_t * cr, double * x1, double * y1, double * x2, double * y2)
{
	...
	else if(!_cairo_gstate_clip_extents(cr->gstate, x1, y1, x2, y2)) {
		ASSIGN_PTR(x1, -INFINITY);
		ASSIGN_PTR(y1, -INFINITY);
		ASSIGN_PTR(x2, +INFINITY);
		ASSIGN_PTR(y2, +INFINITY);
	}
}
...
void cairo_text_extents(cairo_t * cr, const char * utf8, cairo_text_extents_t * extents)
{
	...
	status = _cairo_gstate_text_to_glyphs(cr->gstate,
		x, y, utf8, strlen(utf8), &glyphs, &num_glyphs, NULL, NULL, NULL);
...
void cairo_show_text(cairo_t * cr, const char * utf8)
{
	...
	status = _cairo_gstate_text_to_glyphs(cr->gstate, x, y, utf8, utf8_len, &glyphs, &num_glyphs,
		has_show_text_glyphs ? &clusters : NULL, &num_clusters, &cluster_flags);
	if(unlikely(status))
		goto BAIL;
	if(num_glyphs == 0)
		return;
	status = _cairo_gstate_show_text_glyphs(cr->gstate, utf8, utf8_len, glyphs, num_glyphs, clusters, num_clusters, cluster_flags);
	if(unlikely(status))
		goto BAIL;
	last_glyph = &glyphs[num_glyphs-1];
	status = _cairo_gstate_glyph_extents(cr->gstate, last_glyph, 1, &extents);
	if(unlikely(status))
		goto BAIL;
...
void cairo_show_glyphs(cairo_t * cr, const cairo_glyph_t * glyphs, int num_glyphs)
{
	...
	status = _cairo_gstate_show_text_glyphs(cr->gstate, NULL, 0, glyphs, num_glyphs, NULL, 0, (cairo_text_cluster_flags_t)0);
	if(unlikely(status))
		_cairo_set_error(cr, status);
}
...
void cairo_text_path(cairo_t * cr, const char * utf8)
{
	...
	glyphs = stack_glyphs;
	num_glyphs = ARRAY_LENGTH(stack_glyphs);
	status = _cairo_gstate_text_to_glyphs(cr->gstate, x, y, utf8, strlen(utf8), &glyphs, &num_glyphs, NULL, NULL, NULL);
	if(unlikely(status))
		goto BAIL;
	if(num_glyphs == 0)
		return;
	status = _cairo_gstate_glyph_path(cr->gstate, glyphs, num_glyphs, cr->path);
	if(unlikely(status))
		goto BAIL;
	last_glyph = &glyphs[num_glyphs-1];
	status = _cairo_gstate_glyph_extents(cr->gstate, last_glyph, 1, &extents);
...
void cairo_get_current_point(cairo_t * cr, double * x_ret, double * y_ret)
{
	cairo_fixed_t x_fixed, y_fixed;
	double x, y;
	if(cr->status == CAIRO_STATUS_SUCCESS && _cairo_path_fixed_get_current_point(cr->path, &x_fixed, &y_fixed)) {
