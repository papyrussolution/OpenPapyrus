...
static void edge_end_box(sweep_line_t * sweep_line, edge_t * left, int32_t bot, cairo_bool_t do_traps, void * container)
{
	...
	if(likely(left->top < bot)) {
		if(do_traps) {
			...
			_cairo_traps_add_trap((cairo_traps_t *)container, left->top, bot, &_left, &_right);
			...
			box.p2.y = bot;
			status = _cairo_boxes_add((cairo_boxes_t *)container, &box);
		}
	}
	if(unlikely(status))
...
static inline void sweep_line_delete_edge(sweep_line_t * sweep_line,
	edge_t * edge,
	cairo_bool_t do_traps,
	void * container)
{
...
static inline cairo_bool_t sweep_line_delete(sweep_line_t * sweep,
...
static inline cairo_bool_t sweep_line_insert(sweep_line_t * sweep, rectangle_t * rectangle)
{
	edge_t * pos;
	/* right edge */
...
static cairo_status_t _cairo_bentley_ottmann_tessellate_rectangular(rectangle_t ** rectangles,
	int num_rectangles, cairo_fill_rule_t fill_rule, cairo_bool_t do_traps, void * container)
{
...
cairo_status_t _cairo_bentley_ottmann_tessellate_rectangular_traps(cairo_traps_t * traps, cairo_fill_rule_t fill_rule)
{
	...
	if(traps->num_traps > ARRAY_LENGTH(stack_rectangles)) {
		rectangles = (rectangle_t *)_cairo_malloc_ab_plus_c(traps->num_traps, sizeof(rectangle_t)+sizeof(rectangle_t *), sizeof(rectangle_t *));
	...
	status = _cairo_bentley_ottmann_tessellate_rectangular(rectangles_ptrs, i, fill_rule, TRUE, traps);
...
cairo_status_t _cairo_bentley_ottmann_tessellate_boxes(const cairo_boxes_t * in, cairo_fill_rule_t fill_rule, cairo_boxes_t * out)
{
	...
	if(in->num_boxes > ARRAY_LENGTH(stack_rectangles)) {
		rectangles = (rectangle_t *)_cairo_malloc_ab_plus_c(in->num_boxes, sizeof(rectangle_t)+sizeof(rectangle_t *), sizeof(rectangle_t *));
		if(unlikely(rectangles == NULL))
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	...
	status = _cairo_bentley_ottmann_tessellate_rectangular(rectangles_ptrs, j, fill_rule, FALSE, out);
	if(rectangles != stack_rectangles)
		free(rectangles);
	return status;
}
