...
static cairo_bool_t _pattern_is_supported(uint32_t flags, const cairo_pattern_t * pattern)
{
...
static void _cairo_xcb_picture_set_matrix(cairo_xcb_picture_t * picture,
    const cairo_matrix_t * matrix, cairo_filter_t filter, double xc, double yc)
{
	...
	ignored = _cairo_matrix_to_pixman_matrix_offset(matrix, filter, xc, yc,
	    pixman_transform, &picture->x, &picture->y);
	(void)ignored;
...
static cairo_xcb_picture_t * _cairo_xcb_surface_picture(cairo_xcb_surface_t * target,
    const cairo_surface_pattern_t * pattern, const CairoIRect * extents)
{
	cairo_surface_t * source = pattern->surface;
	cairo_xcb_picture_t * picture = (cairo_xcb_picture_t*)_cairo_surface_has_snapshot(source, &_cairo_xcb_picture_backend);
	if(picture != NULL) {
	...
	if(picture == NULL) {
		cairo_image_surface_t * image;
		void * image_extra;
		cairo_status_t status = _cairo_surface_acquire_source_image(source, &image, &image_extra);
		if(unlikely(status))
			return (cairo_xcb_picture_t*)_cairo_surface_create_in_error(status);
		if(image->format != CAIRO_FORMAT_INVALID) {
			...
			// XXX XRenderPutImage! 
			cairo_image_surface_t * conv = _cairo_image_surface_coerce(image);
			_cairo_surface_release_source_image(source, image, image_extra);
...
static cairo_xcb_picture_t * _cairo_xcb_picture_for_pattern(cairo_xcb_surface_t * target, const cairo_pattern_t * pattern, const CairoIRect * extents)
{
	if(pattern == NULL)
		return _cairo_xcb_white_picture(target);
	...
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_LINEAR:
		    return _cairo_xcb_linear_picture(target, (cairo_linear_pattern_t*)pattern, extents);
		case CAIRO_PATTERN_TYPE_RADIAL:
		    return _cairo_xcb_radial_picture(target, (cairo_radial_pattern_t*)pattern, extents);
		case CAIRO_PATTERN_TYPE_SURFACE:
		    return _cairo_xcb_surface_picture(target, (cairo_surface_pattern_t*)pattern, extents);
		default:
		    ASSERT_NOT_REACHED;
...
static cairo_status_t _render_fill_boxes(void * abstract_dst, cairo_operator_t op,
    const cairo_color_t * color, cairo_boxes_t * boxes)
{
