...
void _cairo_traps_add_trap(cairo_traps_t * traps, cairo_fixed_t top, cairo_fixed_t bottom, cairo_line_t * left, cairo_line_t * right)
{
...
cairo_status_t _cairo_traps_tessellate_rectangle(cairo_traps_t * traps,
	const cairo_point_t * top_left, const cairo_point_t * bottom_right)
{
	cairo_line_t left;
	cairo_line_t right;
 ...
 **/
cairo_int_status_t _cairo_traps_extract_region(cairo_traps_t * traps, cairo_region_t ** region)
{
...
static void _sanitize_trap(cairo_trapezoid_t * t)
{
	...
	//
#define FIX(lr, tb, p) if(t->lr.p.y != t->tb) { \
