...
static cairo_surface_t * gallium_surface_create_internal(gallium_device_t * device, enum pipe_format format, int width,
	int height);

static inline gallium_device_t * gallium_device(gallium_surface_t * surface)
{
	return (gallium_device_t *)surface->drm.base.device;
}

static cairo_format_t _cairo_format_from_pipe_format(enum pipe_format format)
{
	switch((int)format) {
...
static enum pipe_format pipe_format_from_format(cairo_format_t format)
{
	switch((int)format) {
...
static enum pipe_format pipe_format_from_content(cairo_content_t content)
{
...
static cairo_bool_t format_is_supported_destination(gallium_device_t * device,
...
#if 0
static cairo_bool_t format_is_supported_source(gallium_device_t * device,
	enum pipe_format format)
{
	return device->screen->is_format_supported(device->screen,
		format,
...
static cairo_surface_t * gallium_surface_create_similar(void * abstract_src,
...
static cairo_status_t gallium_surface_finish(void * abstract_surface)
{
...
static cairo_surface_t * gallium_surface_map_to_image(gallium_surface_t * surface)
{
	gallium_device_t * device = gallium_device(surface);
	cairo_status_t status;
	void * ptr = NULL;
...
static cairo_status_t gallium_surface_acquire_source_image(void * abstract_surface,
...
static void gallium_surface_release_source_image(void * abstract_surface,
...
static cairo_status_t gallium_surface_flush(void * abstract_surface)
{
	gallium_surface_t * surface = abstract_surface;
	gallium_device_t * device = gallium_device(surface);
	cairo_status_t status;
...
static cairo_int_status_t gallium_surface_paint(void * abstract_surface,
	cairo_operator_t op,
...
static cairo_int_status_t gallium_surface_mask(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
...
static cairo_int_status_t gallium_surface_stroke(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
...
static cairo_int_status_t gallium_surface_fill(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
...
static cairo_int_status_t gallium_surface_glyphs(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
...
static int gallium_format_stride_for_width(enum pipe_format format, int width)
{
	int stride;
...
static cairo_drm_bo_t * _gallium_fake_bo_create(uint32_t size, uint32_t name)
{
	cairo_drm_bo_t * bo;
...
static void _gallium_fake_bo_release(void * dev, void * bo)
{
	free(bo);
}

static cairo_surface_t * gallium_surface_create_internal(gallium_device_t * device,
	enum pipe_format pipe_format,
...
static cairo_surface_t * gallium_surface_create(cairo_drm_device_t * base_dev,
	cairo_format_t format,
	int width, int height)
{
...
#if 0
static cairo_surface_t * gallium_surface_create_for_name(cairo_drm_device_t * base_dev,
	unsigned int name,
	cairo_format_t format,
...
static cairo_int_status_t gallium_surface_flink(void * abstract_surface)
{
	gallium_surface_t * surface = abstract_surface;
	gallium_device_t * device;
...
static void gallium_device_destroy(void * abstract_device)
{
...
cairo_drm_device_t * _cairo_drm_gallium_device_create(int fd, dev_t dev, int vendor_id, int chip_id)
{
	gallium_device_t * device;
	cairo_status_t status;
	void * handle;
