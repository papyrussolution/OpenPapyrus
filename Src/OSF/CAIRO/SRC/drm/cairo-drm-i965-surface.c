...
static void i965_stream_init(i965_stream_t * stream,
...
static void i965_add_relocation(i965_device_t * device,
	intel_bo_t * bo,
...
void i965_emit_relocation(i965_device_t * device,
...
static void i965_stream_reset(i965_stream_t * stream)
{
	stream->used = stream->committed = 0;
	stream->num_relocations = 0;
	stream->num_pending_relocations = 0;
...
void i965_stream_commit(i965_device_t * device,
	i965_stream_t * stream)
{
	intel_bo_t * bo;
	int n;
...
static void sf_states_pluck(void * entry, void * closure)
{
...
static void cc_offsets_pluck(void * entry, void * closure)
{
	i965_device_t * device = closure;
...
static void wm_kernels_pluck(void * entry, void * closure)
{
	i965_device_t * device = closure;
...
static void wm_states_pluck(void * entry, void * closure)
{
	i965_device_t * device = closure;
...
static void wm_bindings_pluck(void * entry, void * closure)
{
	i965_device_t * device = closure;
...
static void samplers_pluck(void * entry, void * closure)
{
	i965_device_t * device = closure;
...
static void i965_device_reset(i965_device_t * device)
{
	device->exec.count = 0;
...
static cairo_status_t i965_exec(i965_device_t * device, uint32_t offset)
{
...
static inline uint32_t next_bo_size(uint32_t v)
{
	v = (v+8191)/8192;
...
static void _copy_to_bo_and_apply_relocations(i965_device_t * device,
	intel_bo_t * bo,
	i965_stream_t * stream,
	uint32_t offset)
{
...
static cairo_surface_t * i965_surface_create_similar(void * abstract_other,
...
static cairo_status_t i965_surface_finish(void * abstract_surface)
{
...
static cairo_status_t i965_surface_flush(void * abstract_surface)
{
	i965_surface_t * surface = abstract_surface;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _composite_boxes_spans(void * closure,
...
cairo_status_t i965_fixup_unbounded(i965_surface_t * dst,
...
static cairo_status_t i965_fixup_unbounded_boxes(i965_surface_t * dst,
	const cairo_composite_rectangles_t * extents,
	cairo_clip_t * clip,
	cairo_boxes_t * boxes)
{
...
static cairo_status_t _composite_boxes(i965_surface_t * dst,
	cairo_operator_t op,
...
static cairo_status_t _clip_and_composite_boxes(i965_surface_t * dst,
	cairo_operator_t op,
...
static cairo_int_status_t i965_surface_paint(void * abstract_dst,
	cairo_operator_t op,
...
static cairo_int_status_t i965_surface_mask(void * abstract_dst,
	cairo_operator_t op,
	const cairo_pattern_t * source,
...
static cairo_status_t _composite_polygon_spans(void * closure,
	cairo_span_renderer_t * renderer,
	const cairo_rectangle_int_t * extents)
{
...
static cairo_int_status_t i965_surface_stroke(void * abstract_dst,
	cairo_operator_t op,
	const cairo_pattern_t * source,
...
static cairo_int_status_t i965_surface_fill(void * abstract_dst,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule,
...
static void i965_surface_init(i965_surface_t * surface,
...
static inline int cairo_const i965_tiling_stride(uint32_t tiling, int stride)
{
...
static inline int cairo_const i965_tiling_height(uint32_t tiling, int height)
{
	switch(tiling) {
	    default:
...
cairo_surface_t * i965_surface_create_internal(cairo_drm_device_t * base_dev,
	cairo_format_t format,
	int width, int height,
	uint32_t tiling,
...
static cairo_surface_t * i965_surface_create(cairo_drm_device_t * device,
...
static cairo_surface_t * i965_surface_create_for_name(cairo_drm_device_t * base_dev,
	unsigned int name,
	cairo_format_t format,
...
static cairo_status_t i965_surface_enable_scan_out(void * abstract_surface)
{
	i965_surface_t * surface = abstract_surface;
	intel_bo_t * bo;
	if(unlikely(surface->intel.drm.bo == NULL))
...
static cairo_int_status_t _i965_device_flush(cairo_drm_device_t * device)
{
	cairo_status_t status;
...
static cairo_int_status_t _i965_device_throttle(cairo_drm_device_t * device)
{
	cairo_status_t status;
...
static void _i965_device_destroy(void * base)
{
...
static cairo_bool_t hash_equal(const void * A, const void * B)
{
	const cairo_hash_entry_t * a = A, * b = B;
	return a->hash == b->hash;
}

cairo_drm_device_t * _cairo_drm_i965_device_create(int fd, dev_t dev, int vendor_id, int chip_id)
{
	i965_device_t * device;
	uint64_t gtt_size;
	cairo_status_t status;
