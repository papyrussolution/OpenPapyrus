...
int intel_get(int fd, int param)
{
...
cairo_bool_t intel_info(int fd, uint64_t * gtt_size)
{
...
void intel_bo_write(const intel_device_t * device, intel_bo_t * bo, unsigned long offset, unsigned long size, const void * data)
{
...
void intel_bo_read(const intel_device_t * device,
	intel_bo_t * bo,
...
void * intel_bo_map(const intel_device_t * device, intel_bo_t * bo)
{
...
void intel_bo_unmap(intel_bo_t * bo)
{
	munmap(bo->virtual, bo->base.size);
	bo->virtual = NULL;
}

cairo_bool_t intel_bo_is_inactive(const intel_device_t * device, intel_bo_t * bo)
{
...
cairo_bool_t intel_bo_wait(const intel_device_t * device, const intel_bo_t * bo)
{
	struct drm_i915_gem_set_domain set_domain;
	int ret;
...
static void intel_bo_cache_remove(intel_device_t * device,
	intel_bo_t * bo,
	int bucket)
{
...
cairo_bool_t intel_bo_madvise(intel_device_t * device,
	intel_bo_t * bo,
	int advice)
{
...
static void intel_bo_set_real_size(intel_device_t * device,
	intel_bo_t * bo,
	size_t size)
{
...
static void intel_bo_cache_purge(intel_device_t * device)
{
	int bucket;
...
intel_bo_t * intel_bo_create(intel_device_t * device,
	uint32_t max_size,
	uint32_t real_size,
...
intel_bo_t * intel_bo_create_for_name(intel_device_t * device, uint32_t name)
{
...
static void intel_bo_release(void * _dev, void * _bo)
{
...
void intel_bo_set_tiling(const intel_device_t * device,
	intel_bo_t * bo)
{
...
cairo_surface_t * intel_bo_get_image(const intel_device_t * device,
	intel_bo_t * bo,
...
static cairo_status_t _intel_bo_put_a1_image(intel_device_t * device,
	intel_bo_t * bo,
...
cairo_status_t intel_bo_put_image(intel_device_t * device,
	intel_bo_t * bo,
	cairo_image_surface_t * src,
	int src_x, int src_y,
	int width, int height,
...
static void _intel_device_init_bo_cache(intel_device_t * device)
{
	int i;
...
static cairo_bool_t _intel_snapshot_cache_entry_can_remove(const void * closure)
{
	return TRUE;
}

static void _intel_snapshot_cache_entry_destroy(void * closure)
{
...
cairo_status_t intel_device_init(intel_device_t * device, int fd)
{
...
static void _intel_bo_cache_fini(intel_device_t * device)
{
	int bucket;
...
static void _intel_gradient_cache_fini(intel_device_t * device)
{
	unsigned int n;
...
static void _intel_glyph_cache_fini(intel_device_t * device, intel_buffer_cache_t * cache)
{
...
void intel_device_fini(intel_device_t * device)
{
...
void intel_throttle(intel_device_t * device)
{
	ioctl(device->base.fd, DRM_IOCTL_I915_GEM_THROTTLE);
}

void intel_glyph_cache_unpin(intel_device_t * device)
{
	int n;
...
static cairo_status_t intel_glyph_cache_add_glyph(intel_device_t * device,
...
void intel_scaled_glyph_fini(cairo_scaled_glyph_t * scaled_glyph,
...
void intel_scaled_font_fini(cairo_scaled_font_t * scaled_font)
{
	cairo_list_del(&scaled_font->link);
}

static cairo_status_t intel_get_glyph_cache(intel_device_t * device,
...
cairo_int_status_t intel_get_glyph(intel_device_t * device,
	cairo_scaled_font_t * scaled_font,
	cairo_scaled_glyph_t * scaled_glyph)
{
...
cairo_status_t intel_buffer_cache_init(intel_buffer_cache_t * cache,
...
cairo_status_t intel_snapshot_cache_insert(intel_device_t * device,
...
void intel_snapshot_cache_thaw(intel_device_t * device)
{
	if(device->snapshot_cache.freeze_count)
		_cairo_cache_thaw(&device->snapshot_cache);
}

static cairo_bool_t _gradient_color_stops_equal(const cairo_gradient_pattern_t * a,
...
static int intel_gradient_sample_width(const cairo_gradient_pattern_t * gradient)
{
	unsigned int n;
	int width;
...
cairo_status_t intel_gradient_render(intel_device_t * device,
