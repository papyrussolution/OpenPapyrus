...
void _cairo_path_fixed_fini(cairo_path_fixed_t * path)
{
	...
	while(buf != cairo_path_head(path)) {
		cairo_path_buf_t * p_this_buf = buf;
		buf = cairo_path_buf_next(buf);
		_cairo_path_buf_destroy(p_this_buf);
	}
...
static cairo_path_op_t _cairo_path_last_op(cairo_path_fixed_t * path)
{
...
static inline void _cairo_path_fixed_extents_add(cairo_path_fixed_t * path, const cairo_point_t * point)
{
...
cairo_status_t _cairo_path_fixed_move_to(cairo_path_fixed_t * path, cairo_fixed_t x, cairo_fixed_t y)
{
	...
	if(_cairo_path_last_op(path) == CAIRO_PATH_OP_MOVE_TO) {
		...
		if(path->has_current_point && path->is_rectilinear) {
			...
			path->is_rectilinear = path->current_point.x == path->last_move_point.x || path->current_point.y == path->last_move_point.y;
			path->maybe_fill_region &= path->is_rectilinear;
		}
		if(path->maybe_fill_region) {
			path->maybe_fill_region = _cairo_fixed_is_integer(path->last_move_point.x) && _cairo_fixed_is_integer(path->last_move_point.y);
		}
	}
...
FLAGS:
	if(path->is_rectilinear) {
		path->is_rectilinear = path->current_point.x == x || path->current_point.y == y;
		path->maybe_fill_region &= path->is_rectilinear;
	}
	if(path->maybe_fill_region) {
		path->maybe_fill_region = _cairo_fixed_is_integer(x) && _cairo_fixed_is_integer(y);
	}
	if(path->is_empty_fill) {
		path->is_empty_fill = path->current_point.x == x && path->current_point.y == y;
	}
	path->current_point = point;
...
cairo_status_t _cairo_path_fixed_close_path(cairo_path_fixed_t * path)
{
	...
	if(_cairo_path_last_op(path) == CAIRO_PATH_OP_LINE_TO) {
		if(path->current_point.x == path->last_move_point.x && path->current_point.y == path->last_move_point.y) {
			cairo_point_t * p;
			cairo_path_buf_t * buf = cairo_path_tail(path);
			if(likely(buf->num_points >= 2)) {
				p = &buf->points[buf->num_points-2];
	...
	if(unlikely(status))
		return status;
	return _cairo_path_fixed_move_to(path, path->last_move_point.x, path->last_move_point.y);
}
...
static cairo_status_t _cairo_path_fixed_add(cairo_path_fixed_t * path,
	cairo_path_op_t op, const cairo_point_t * points, int num_points)
{
	cairo_path_buf_t * buf = cairo_path_tail(path);
	...
	if(WATCH_PATH) {
		...
		for(i = 0; i < num_points; i++) {
			...
			len += snprintf(buf+len, sizeof(buf), "(%f, %f)",
				_cairo_fixed_to_double(points[i].x), _cairo_fixed_to_double(points[i].y));
		}
		len += snprintf(buf+len, sizeof(buf), "]");
		fprintf(stderr, "_cairo_path_fixed_add (%s, %s)\n", op_str[(int)op], buf);
	}
...
static cairo_status_t _append_curve_to(void * abstract_closure,
	const cairo_point_t * p0, const cairo_point_t * p1, const cairo_point_t * p2)
{
	cairo_path_fixed_append_closure_t * closure = (cairo_path_fixed_append_closure_t *)abstract_closure;
	return _cairo_path_fixed_curve_to(closure->path, p0->x+closure->offset.x, p0->y+closure->offset.y, p1->x+closure->offset.x,
		p1->y+closure->offset.y, p2->x+closure->offset.x, p2->y+closure->offset.y);
}
...
cairo_bool_t _cairo_path_fixed_is_equal(const cairo_path_fixed_t * path,
	const cairo_path_fixed_t * other)
{
...
static cairo_status_t _cpf_curve_to(void * closure,
...
cairo_bool_t _cairo_path_fixed_iter_is_fill_box(cairo_path_fixed_iter_t * _iter, cairo_box_t * box)
{
	...
	else if(!(iter.buf->op[iter.n_op] == CAIRO_PATH_OP_CLOSE_PATH || iter.buf->op[iter.n_op] == CAIRO_PATH_OP_MOVE_TO)) {
		return FALSE;
	}
	if(!_cairo_path_fixed_iter_next_op(&iter))
		return FALSE;
