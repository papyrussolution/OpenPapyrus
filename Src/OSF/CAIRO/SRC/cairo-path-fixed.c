    ...
    cairo_fixed_t dx0, cairo_fixed_t dy0, cairo_fixed_t dx1, cairo_fixed_t dy1, cairo_fixed_t dx2, cairo_fixed_t dy2)
{
	...
	return _cairo_path_fixed_curve_to(path, path->current_point.x + dx0, path->current_point.y + dy0, 
		path->current_point.x + dx1, path->current_point.y + dy1, path->current_point.x + dx2,
...
cairo_bool_t _cairo_path_fixed_is_simple_quad(const cairo_path_fixed_t * path)
{
	if(!_path_is_quad(path))
		...
		const cairo_point_t * points = cairo_path_head(path)->points;
		if(_points_form_rect(points))
			return TRUE;
		else if(_lines_intersect_or_are_coincident(points[0], points[1], points[3], points[2]))
			return FALSE;
		else if(_lines_intersect_or_are_coincident(points[0], points[3], points[1], points[2]))
			return FALSE;
		else
			return TRUE;
	}
...
cairo_bool_t _cairo_path_fixed_is_rectangle(const cairo_path_fixed_t * path, cairo_box_t * box)
{
	...
	buf = cairo_path_head(path);
	return (buf->num_ops > 4) ? TRUE : FALSE;
}
