...
static cairo_status_t _cairo_gstate_transform_glyphs_to_backend(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs,
	int num_glyphs, const cairo_text_cluster_t * clusters, int num_clusters,
...
cairo_status_t _cairo_gstate_redirect_target(cairo_gstate_t * gstate, cairo_surface_t * child)
{
	...
	_cairo_clip_reset(&gstate->clip);
	return _cairo_clip_init_copy_transformed(&gstate->clip, &gstate->next->clip, &matrix);
}
 ...
 **/
cairo_bool_t _cairo_gstate_is_redirected(cairo_gstate_t * gstate)
{
 ...
 **/
cairo_surface_t * _cairo_gstate_get_parent_target(cairo_gstate_t * gstate)
{
...
void _cairo_gstate_backend_to_user_rectangle(cairo_gstate_t * gstate,
	double * x1, double * y1, double * x2, double * y2, cairo_bool_t * is_tight)
{
...
static void _cairo_gstate_copy_pattern(cairo_pattern_t * pattern, const cairo_pattern_t * original)
{
	...
	if(_cairo_pattern_is_clear(original)) {
		_cairo_pattern_init_solid((cairo_solid_pattern_t *)pattern, CAIRO_COLOR_TRANSPARENT);
		return;
	}
	if(original->type == CAIRO_PATTERN_TYPE_LINEAR || original->type == CAIRO_PATTERN_TYPE_RADIAL) {
		...
		if(_cairo_gradient_pattern_is_solid((cairo_gradient_pattern_t *)original, NULL, &color)) {
			_cairo_pattern_init_solid((cairo_solid_pattern_t *)pattern, &color);
			return;
		}
	}
...
static void _cairo_gstate_copy_transformed_pattern(cairo_gstate_t * gstate,
	cairo_pattern_t * pattern, const cairo_pattern_t * original, const cairo_matrix_t * ctm_inverse)
{
...
static cairo_bool_t _clipped(cairo_gstate_t * gstate)
{
...
cairo_status_t _cairo_gstate_paint(cairo_gstate_t * gstate)
{
	...
	status = _cairo_surface_paint(gstate->target, op, pattern, _gstate_get_clip(gstate, &clip));
...
#undef M
		...
		status = _cairo_surface_paint(gstate->target, op, &source_pattern.base, _gstate_get_clip(gstate, &clip));
	}
	else {
		status = _cairo_surface_mask(gstate->target, op, source, &mask_pattern.base, _gstate_get_clip(gstate, &clip));
	}
	_cairo_clip_fini(&clip);
	return status;
}
...
cairo_status_t _cairo_gstate_fill(cairo_gstate_t * gstate, cairo_path_fixed_t * path)
{
	...
	if(_cairo_path_fixed_fill_is_empty(path)) {
		if(_cairo_operator_bounded_by_mask(gstate->op))
			return CAIRO_STATUS_SUCCESS;
		status = _cairo_surface_paint(gstate->target, CAIRO_OPERATOR_CLEAR, &_cairo_pattern_clear.base, _gstate_get_clip(gstate, &clip));
	}
	else {
		cairo_pattern_union_t source_pattern;
		const cairo_pattern_t * pattern;
...
static void _cairo_gstate_traps_extents_to_user_rectangle(cairo_gstate_t * gstate,
	cairo_traps_t * traps, double * x1, double * y1, double * x2, double * y2)
{
	cairo_box_t extents;
	...
	cairo_path_fixed_t * path, double * x1, double * y1, double * x2, double * y2)
{
	...
	_cairo_traps_init(&traps);
	status = _cairo_path_fixed_stroke_to_traps(path,
		&gstate->stroke_style, &gstate->ctm, &gstate->ctm_inverse, gstate->tolerance, &traps);
...
{
	return _cairo_clip_clip(&gstate->clip, path, gstate->fill_rule, gstate->tolerance, gstate->antialias);
}
...
static cairo_bool_t _cairo_gstate_int_clip_extents(cairo_gstate_t * gstate, cairo_rectangle_int_t * extents)
{
	...
	const cairo_rectangle_int_t * clip_extents = _cairo_clip_get_extents(&gstate->clip);
	if(clip_extents != NULL) {
		cairo_bool_t is_empty = _cairo_rectangle_intersect(extents, clip_extents);
		is_bounded = TRUE;
	}
	return is_bounded;
}
...
cairo_status_t _cairo_gstate_select_font_face(cairo_gstate_t * gstate,
...
static cairo_status_t _cairo_gstate_ensure_scaled_font(cairo_gstate_t * gstate)
{
	...
	scaled_font = cairo_scaled_font_create(gstate->font_face, &gstate->font_matrix, &gstate->ctm, &options);
...
cairo_status_t _cairo_gstate_text_to_glyphs(cairo_gstate_t * gstate,
	double x,
	double y,
	const char * utf8,
	int utf8_len,
	   ...
	   _cairo_scaled_font_get_max_scale(gstate->scaled_font) <= 10240) {
		...
		if(status == CAIRO_STATUS_SUCCESS) {
			status = _cairo_surface_fill(gstate->target, op, pattern, &path, CAIRO_FILL_RULE_WINDING,
				gstate->tolerance, gstate->scaled_font->options.antialias, _gstate_get_clip(gstate, &clip));
		}
...
cairo_status_t _cairo_gstate_glyph_path(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs, int num_glyphs, cairo_path_fixed_t * path)
{
	...
	status = _cairo_gstate_transform_glyphs_to_backend(gstate,
		glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_UNDEF, transformed_glyphs, NULL, NULL);
	if(unlikely(status))
		goto CLEANUP_GLYPHS;
	status = _cairo_scaled_font_glyph_path(gstate->scaled_font, transformed_glyphs, num_glyphs, path);
CLEANUP_GLYPHS:
 ...
 **/
static cairo_status_t _cairo_gstate_transform_glyphs_to_backend(cairo_gstate_t * gstate,
	const cairo_glyph_t * glyphs,
	int num_glyphs,
	const cairo_text_cluster_t * clusters,
	int num_clusters,
