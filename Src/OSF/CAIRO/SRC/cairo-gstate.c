...
static cairo_status_t FASTCALL _cairo_gstate_init_copy(cairo_gstate_t * gstate, cairo_gstate_t * other);
static cairo_status_t FASTCALL _cairo_gstate_ensure_font_face(cairo_gstate_t * gstate);
static cairo_status_t FASTCALL _cairo_gstate_ensure_scaled_font(cairo_gstate_t * gstate);
...
cairo_status_t _cairo_gstate_redirect_target(cairo_gstate_t * gstate, cairo_surface_t * child)
{
	...
	gstate->clip = _cairo_clip_copy_with_translation(gstate->next->clip,
	    (int)(child->device_transform.x0 - gstate->parent_target->device_transform.x0),
	    (int)(child->device_transform.y0 - gstate->parent_target->device_transform.y0));
	return CAIRO_STATUS_SUCCESS;
}
...
cairo_status_t _cairo_gstate_set_fill_rule(cairo_gstate_t * gstate, CairoFillRule fill_rule)
{
	gstate->fill_rule = fill_rule;
	return CAIRO_STATUS_SUCCESS;
}

CairoFillRule _cairo_gstate_get_fill_rule(cairo_gstate_t * gstate)
{
...
cairo_status_t _cairo_gstate_stroke(cairo_gstate_t * gstate, cairo_path_fixed_t * path)
{
	...
	if(_cairo_stroke_style_dash_can_approximate(&gstate->stroke_style, &aggregate_transform, gstate->tolerance)) {
		style.dash = dash;
		_cairo_stroke_style_dash_approximate(&gstate->stroke_style, &gstate->ctm, gstate->tolerance, &style.dash_offset,
		    style.dash, &style.num_dashes);
	}
	_cairo_gstate_copy_transformed_source(gstate, &source_pattern.base);
	return _cairo_surface_stroke(gstate->target, gstate->op,
	    &source_pattern.base, path, &style, &aggregate_transform,
...
cairo_status_t _cairo_gstate_in_stroke(cairo_gstate_t * gstate,
    cairo_path_fixed_t * path, double x, double y, cairo_bool_t * inside_ret)
{
	cairo_status_t status;
	CairoIRect extents;
	cairo_box_t limit;
	 ...
	 */
	_cairo_path_fixed_approximate_stroke_extents(path, &gstate->stroke_style, &gstate->ctm, &extents);
	...
	status = _cairo_path_fixed_stroke_polygon_to_traps(path, &gstate->stroke_style,
	    &gstate->ctm, &gstate->ctm_inverse, gstate->tolerance, &traps);
	if(unlikely(status))
		goto BAIL;
...
static void _cairo_gstate_extents_to_user_rectangle(cairo_gstate_t   * gstate,
    const cairo_box_t * extents, double * x1, double * y1, double * x2, double * y2)
{
    ...
    cairo_path_fixed_t * path, double * x1, double * y1, double * x2, double * y2)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		status = _cairo_path_fixed_stroke_to_polygon(path, &gstate->stroke_style,
		    &gstate->ctm, &gstate->ctm_inverse, gstate->tolerance, &polygon);
...
static cairo_bool_t FASTCALL _cairo_gstate_int_clip_extents(cairo_gstate_t * gstate, CairoIRect * extents)
{
...
cairo_status_t _cairo_gstate_show_text_glyphs(cairo_gstate_t * gstate, const cairo_glyph_t * glyphs,
    int num_glyphs, cairo_glyph_text_info_t * info)
{
...
cairo_status_t _cairo_gstate_glyph_path(cairo_gstate_t * gstate,
...
static void _cairo_gstate_transform_glyphs_to_backend(cairo_gstate_t * gstate,
    const cairo_glyph_t  * glyphs, int num_glyphs, const cairo_text_cluster_t * clusters, int num_clusters,
    cairo_text_cluster_flags_t cluster_flags, cairo_glyph_t * transformed_glyphs, int * num_transformed_glyphs,
    cairo_text_cluster_t * transformed_clusters)
{
