...
static cairo_status_t _cairo_win32_device_flush(void * device)
{
	GdiFlush();
	return CAIRO_STATUS_SUCCESS;
}

static void _cairo_win32_device_finish(void * device)
{
}
...
{
	static cairo_device_t * __cairo_win32_device = 0;
	cairo_device_t * p_result = 0;
	if(__cairo_win32_device)
		...
		if(_cairo_atomic_ptr_cmpxchg((void**)&__cairo_win32_device, NULL, device))
			p_result = cairo_device_reference(&device->base);
		else {
...
unsigned _cairo_win32_flags_for_dc(HDC dc)
{
	uint32_t flags = 0;
