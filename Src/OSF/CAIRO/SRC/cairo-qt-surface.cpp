...
static cairo_status_t _cairo_path_to_qpainterpath_curve_to(void * closure, const cairo_point_t * p0,
...
static inline QPainterPath path_to_qt(cairo_path_fixed_t * path,
...
static cairo_status_t _cairo_qt_surface_acquire_dest_image(void * abstract_surface,
...
static void _cairo_qt_surface_release_dest_image(void * abstract_surface,
	cairo_rectangle_int_t * interest_rect,
	cairo_image_surface_t * image,
	cairo_rectangle_int_t * image_rect,
	void * image_extra)
...
static cairo_status_t _cairo_qt_surface_clone_similar(void * abstract_surface,
	cairo_surface_t * src,
	int src_x,
	int src_y,
	int width,
...
struct PatternToBrushConverter {
		...
		mAcquiredImageExtra(0)
	{
		if(pattern->type == CAIRO_PATTERN_TYPE_SOLID) {
			...
			if(surface->type == CAIRO_SURFACE_TYPE_QT) {
				...
				if(surface->type == CAIRO_SURFACE_TYPE_IMAGE) {
					...
					void * image_extra;
					if(_cairo_surface_acquire_source_image(surface, &isurf,
						   &image_extra) == CAIRO_STATUS_SUCCESS) {
			...
			if(pattern->type == CAIRO_PATTERN_TYPE_LINEAR) {
				...
				if(rpat->r1 == rpat->r2) {
					 ...
					 */
					if((extend == CAIRO_EXTEND_REFLECT || extend ==
					    CAIRO_EXTEND_REPEAT) && r0 > 0.0) {
						double r_org = r1;
						double r, x, y;
		...
		if(mBrush.style() != Qt::NoBrush  && pattern->type != CAIRO_PATTERN_TYPE_SOLID && !_cairo_matrix_is_identity(&pattern->matrix)) {
...
struct PatternToPenConverter {
	PatternToPenConverter (const cairo_pattern_t * source, const cairo_stroke_style_t * style) :
...
static cairo_int_status_t _cairo_qt_surface_composite(cairo_operator_t op,
...
cairo_surface_t * cairo_qt_surface_create_with_qimage(cairo_format_t format, int width, int height)
{
	...
	qs->image_equiv = cairo_image_surface_create_for_data(image->bits(),
		format, width, height, image->bytesPerLine());
	...
	int height)
{
	...
	_cairo_surface_init(&qs->base, &cairo_qt_surface_backend, NULL, /* device */ content);
	_cairo_surface_clipper_init(&qs->clipper, _cairo_qt_surface_clipper_intersect_clip_path);
