...
static cairo_font_face_t * _cairo_ft_resolve_pattern(FcPattern * pattern, const cairo_matrix_t * font_matrix,
...
static void _font_map_release_face_lock_held(cairo_ft_unscaled_font_map_t * font_map,
	cairo_ft_unscaled_font_t * unscaled)
{
	if(unscaled->face) {
 ...
 */
static cairo_status_t _cairo_ft_unscaled_font_create_internal(cairo_bool_t from_face,
	char * filename, int id, FT_Face font_face, cairo_ft_unscaled_font_t ** out)
{
	...
	unscaled = _cairo_hash_table_lookup(font_map->hash_table, &key.base.hash_entry);
	if(unscaled != NULL) {
...
#if CAIRO_HAS_FC_FONT
static cairo_status_t _cairo_ft_unscaled_font_create_for_pattern(FcPattern * pattern,
...
static void _cairo_ft_unscaled_font_destroy(void * abstract_font)
{
 ...
 */
cairo_warn FT_Face _cairo_ft_unscaled_font_lock_face(cairo_ft_unscaled_font_t * unscaled)
{
	cairo_ft_unscaled_font_map_t * font_map;
	FT_Face face = NULL;
 ...
 */
void _cairo_ft_unscaled_font_unlock_face(cairo_ft_unscaled_font_t * unscaled)
{
 ...
 */
static cairo_status_t _get_bitmap_surface(FT_Bitmap * bitmap,
...
static cairo_status_t _cairo_ft_font_face_scaled_font_create(void * abstract_font_face,
 ...
 */
static void _cairo_ft_scaled_glyph_vertical_layout_bearing_fix(void * abstract_font,
	FT_GlyphSlot glyph)
{
	cairo_ft_scaled_font_t * scaled_font = abstract_font;
	FT_Vector vector;
...
static cairo_int_status_t _cairo_ft_scaled_glyph_init(void * abstract_font,
	...
	cairo_scaled_glyph_info_t info)
{
	...
	if(info&CAIRO_SCALED_GLYPH_INFO_PATH) {
		 ...
		 */
		if((info&CAIRO_SCALED_GLYPH_INFO_SURFACE) != 0) {
			error = FT_Load_Glyph(face, _cairo_scaled_glyph_index(scaled_glyph), load_flags|FT_LOAD_NO_BITMAP);
			/* XXX ignoring all other errors for now.  They are not fatal, typically
			 * just a glyph-not-found. */
			if(error == FT_Err_Out_Of_Memory) {
				status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static unsigned long _cairo_ft_ucs4_to_index(void * abstract_font,
	uint32_t ucs4)
{
	cairo_ft_scaled_font_t * scaled_font = abstract_font;
	cairo_ft_unscaled_font_t * unscaled = scaled_font->unscaled;
...
static cairo_int_status_t _cairo_ft_load_truetype_table(void * abstract_font,
	unsigned long tag, long offset, unsigned char * buffer, unsigned long * length)
{
	cairo_ft_scaled_font_t * scaled_font = abstract_font;
	cairo_ft_unscaled_font_t * unscaled = scaled_font->unscaled;
	FT_Face face;
...
#if CAIRO_HAS_FC_FONT
static cairo_status_t _cairo_ft_font_face_create_for_pattern(FcPattern * pattern, cairo_font_face_t ** out);
...
static void _cairo_ft_font_face_destroy(void * abstract_face)
{
...
static cairo_font_face_t * _cairo_ft_font_face_get_implementation(void * abstract_face,
	const cairo_matrix_t * font_matrix,
	const cairo_matrix_t * ctm,
	const cairo_font_options_t * options)
{
...
#if CAIRO_HAS_FC_FONT
static cairo_status_t _cairo_ft_font_face_create_for_pattern(FcPattern * pattern,
...
cairo_font_face_t * cairo_ft_font_face_create_for_pattern(FcPattern * pattern)
{
	...
	cairo_font_face_t * font_face;
	cairo_ft_options_t ft_options;
	cairo_status_t status = _cairo_ft_unscaled_font_create_for_pattern(pattern, &unscaled);
	if(unlikely(status))
		return (cairo_font_face_t *)&_cairo_font_face_nil;
	if(unlikely(unscaled == NULL)) {
 ...
 */
cairo_unscaled_font_t * _cairo_ft_scaled_font_get_unscaled_font(cairo_scaled_font_t * abstract_font)
{
	cairo_ft_scaled_font_t * scaled_font = (cairo_ft_scaled_font_t *)abstract_font;
	return &scaled_font->unscaled->base;
}

cairo_bool_t _cairo_ft_scaled_font_is_vertical(cairo_scaled_font_t * scaled_font)
{
