...
static cairo_status_t _ft_to_cairo_error(FT_Error error) // @v1.14.6
{
	// Currently we don't get many (any?) useful statuses here. Populate as needed.
	switch (error) {
...
static cairo_warn FT_Face _cairo_ft_unscaled_font_lock_face(cairo_ft_unscaled_font_t * unscaled)
{
	...
	font_map = _cairo_ft_unscaled_font_map_lock();
	{
		...
		while(font_map->num_open_faces >= MAX_OPEN_FACES) {
			...
			entry = _cairo_hash_table_random_entry(font_map->hash_table, _has_unlocked_face);
			if(entry == NULL)
				break;
...
static cairo_status_t _get_bitmap_surface(FT_Bitmap * bitmap,
    FT_Library library, cairo_bool_t own_buffer, cairo_font_options_t * font_options, cairo_image_surface_t  ** surface)
{
	uint width, height;
	uchar * data;
	...
	height = bitmap->rows;
	if(width == 0 || height == 0) {
		*surface = (cairo_image_surface_t*)cairo_image_surface_create_for_data(NULL, format, 0, 0, 0);
...
static cairo_status_t _render_glyph_outline(FT_Face face, cairo_font_options_t * font_options, cairo_image_surface_t ** surface)
{
	...
	uint width, height;
	cairo_status_t status;
	FT_Error error; // @v1.14.6 fterror-->error
...
static cairo_bool_t _cairo_ft_is_synthetic(void * abstract_font)
{
	cairo_ft_scaled_font_t * scaled_font = abstract_font;
...
static cairo_int_status_t _cairo_index_to_glyph_name(void * abstract_font,
    char ** glyph_names, int num_glyph_names, ulong glyph_index, ulong * glyph_array_index)
{
	cairo_ft_scaled_font_t * scaled_font = abstract_font;
	cairo_ft_unscaled_font_t * unscaled = scaled_font->unscaled;
...
static cairo_bool_t _cairo_ft_font_face_destroy(void * abstract_face)
{
	...
	if(font_face->unscaled) {
		...
		for(tmp_face = font_face->unscaled->faces; tmp_face; tmp_face = tmp_face->next) {
