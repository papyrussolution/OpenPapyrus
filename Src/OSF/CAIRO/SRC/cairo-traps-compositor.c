    ...
    uint16 coverage)
{
	...
	info->compositor->fill_boxes(info->dst, CAIRO_OPERATOR_IN, &color, &info->boxes);
}

static void add_rect_with_offset(cairo_boxes_t * boxes, int x1, int y1, int x2, int y2, int dx, int dy)
{
	cairo_box_t box;
...
static cairo_int_status_t combine_clip_as_traps(const cairo_traps_compositor_t * compositor,
...
static cairo_status_t __clip_to_surface(const cairo_traps_compositor_t * compositor,
...
static cairo_surface_t * create_composite_mask(const cairo_traps_compositor_t * compositor,
    cairo_surface_t          * dst,
    void * draw_closure,
 ...
 */
static cairo_status_t clip_and_composite_combine(const cairo_traps_compositor_t * compositor,
    const cairo_composite_rectangles_t* extents,
    draw_func_t draw_func,
    void * draw_closure,
    cairo_operator_t op,
 ...
 */
static cairo_status_t clip_and_composite_source(const cairo_traps_compositor_t       * compositor,
    cairo_surface_t                      * dst,
    draw_func_t draw_func,
    draw_func_t mask_func,
    void * draw_closure,
...
static cairo_status_t fixup_unbounded_with_mask(const cairo_traps_compositor_t * compositor,
    const cairo_composite_rectangles_t * extents)
{
	cairo_surface_t * dst = extents->surface;
	cairo_surface_t * mask;
...
static void add_rect(cairo_boxes_t * boxes, int x1, int y1, int x2, int y2)
{
	cairo_box_t box;
	cairo_int_status_t status;
    ...
    uint need_clip)
{
	...
	if(need_clip & NEED_CLIP_REGION) {
		const CairoIRect * limit = ((need_clip & FORCE_CLIP_REGION) == 0) ? &extents->unbounded : &extents->destination;
...
static cairo_int_status_t composite_traps(const cairo_traps_compositor_t * compositor,
    cairo_surface_t                * dst,
    void * closure,
    cairo_operator_t op,
    cairo_surface_t                * src,
...
static cairo_status_t composite_aligned_boxes(const cairo_traps_compositor_t * compositor,
...
static cairo_int_status_t trim_extents_to_tristrip(cairo_composite_rectangles_t * extents,
    cairo_tristrip_t * strip)
{
	cairo_box_t box;
...
static cairo_int_status_t boxes_for_traps(cairo_boxes_t * boxes, cairo_traps_t * traps, cairo_antialias_t antialias)
{
	int i, j;
	...
	if(antialias != CAIRO_ANTIALIAS_NONE) {
		for(i = j = 0; i < traps->num_traps; i++) {
			...
			if(boxes->is_pixel_aligned) {
				boxes->is_pixel_aligned = _cairo_fixed_is_integer(x1) && _cairo_fixed_is_integer(y1) &&
		...
		for(i = j = 0; i < traps->num_traps; i++) {
			...
			j += (boxes->chunks.base[j].p1.x != boxes->chunks.base[j].p2.x && boxes->chunks.base[j].p1.y != boxes->chunks.base[j].p2.y);
		}
	}
...
static cairo_status_t clip_and_composite_boxes(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes);

static cairo_status_t clip_and_composite_polygon(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents,
    cairo_polygon_t * polygon,
    cairo_antialias_t antialias,
    CairoFillRule fill_rule,
...
static cairo_int_status_t composite_opacity_boxes(const cairo_traps_compositor_t * compositor,
    cairo_surface_t                * dst,
    void * closure,
    cairo_operator_t op,
    cairo_surface_t                * src,
    ...
    cairo_clip_t                   * clip)
{
	const cairo_solid_pattern_t * mask = (const cairo_solid_pattern_t *)closure;
...
static cairo_int_status_t composite_boxes(const cairo_traps_compositor_t * compositor,
    cairo_surface_t                * dst,
    void * closure,
    cairo_operator_t op,
    cairo_surface_t                * src,
    ...
    cairo_clip_t                   * clip)
{
	...
	TRACE((stderr, "%s\n", __FUNCTION__));
	status = _cairo_traps_init_boxes(&traps, (cairo_boxes_t *)closure);
	if(unlikely(status))
		return status;
...
static cairo_int_status_t composite_traps_as_boxes(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents,
...
static cairo_int_status_t clip_and_composite_traps(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents,
    composite_traps_info_t * info,
    unsigned flags)
{
...
static cairo_int_status_t clip_and_composite_tristrip(const cairo_traps_compositor_t * compositor,
    cairo_composite_rectangles_t * extents,
...
static cairo_int_status_t composite_mask(const cairo_traps_compositor_t * compositor,
    cairo_surface_t                 * dst,
    void * closure,
    cairo_operator_t op,
    cairo_surface_t                 * src,
...
static cairo_int_status_t _cairo_traps_compositor_stroke(const cairo_compositor_t * _compositor,
    cairo_composite_rectangles_t * extents,
    ...
    cairo_antialias_t antialias)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED && path->has_curve_to && antialias == CAIRO_ANTIALIAS_NONE) {
		...
		if(likely(status == CAIRO_INT_STATUS_SUCCESS))
			status = clip_and_composite_polygon(compositor, extents, &polygon, CAIRO_ANTIALIAS_NONE, CAIRO_FILL_RULE_WINDING, TRUE);
		_cairo_polygon_fini(&polygon);
	}

	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		cairo_int_status_t (* func)(const cairo_path_fixed_t * path, const cairo_stroke_style_t  * stroke_style,
...
static cairo_int_status_t _cairo_traps_compositor_fill(const cairo_compositor_t * _compositor,
    cairo_composite_rectangles_t * extents, const cairo_path_fixed_t  * path, CairoFillRule fill_rule,
    double tolerance, cairo_antialias_t antialias)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		cairo_polygon_t polygon;
#if 0
		if(extents->mask.width  > extents->unbounded.width || extents->mask.height > extents->unbounded.height) {
			cairo_box_t limits;
...
#endif
		if(likely(status == CAIRO_INT_STATUS_SUCCESS)) {
			status = clip_and_composite_polygon(compositor, extents, &polygon, antialias, fill_rule, path->has_curve_to);
		}
		_cairo_polygon_fini(&polygon);
	}
	return status;
...
static cairo_int_status_t composite_glyphs(const cairo_traps_compositor_t * compositor,
    cairo_surface_t * dst, void * closure, cairo_operator_t op, cairo_surface_t * src,
    int src_x, int src_y, int dst_x, int dst_y, const CairoIRect * extents, cairo_clip_t * clip)
{
	...
	if(op == CAIRO_OPERATOR_ADD && (dst->content & CAIRO_CONTENT_COLOR) == 0)
		info->use_mask = 0;
	return compositor->composite_glyphs(dst, op, src, src_x, src_y, dst_x, dst_y, info);
}

static cairo_int_status_t _cairo_traps_compositor_glyphs(const cairo_compositor_t * _compositor,
    cairo_composite_rectangles_t* extents, cairo_scaled_font_t * scaled_font,
    cairo_glyph_t * glyphs, int num_glyphs, cairo_bool_t overlap)
{
	const cairo_traps_compositor_t * compositor = (cairo_traps_compositor_t*)_compositor;
	cairo_int_status_t status;
	TRACE((stderr, "%s\n", __FUNCTION__));
	...
	status = compositor->check_composite_glyphs(extents, scaled_font, glyphs, &num_glyphs);
	if(likely(status == CAIRO_INT_STATUS_SUCCESS)) {
		...
		status = clip_and_composite(compositor, extents, composite_glyphs, NULL, &info, need_bounded_clip(extents) | FORCE_CLIP_REGION);
	}
	_cairo_scaled_font_thaw_cache(scaled_font);
	return status;
}

void _cairo_traps_compositor_init(cairo_traps_compositor_t * compositor, const cairo_compositor_t  * delegate)
{
