...
cairo_int_status_t _cairo_image_info_get_jpeg_info(cairo_image_info_t * info, const uchar * data, long length)
{
	const uchar * p = data;
...
static const uchar * FASTCALL _jpx_get_box_contents(const uchar * p)
{
	return p + 8;
}

static cairo_bool_t FASTCALL _jpx_match_box(const uchar * p, const uchar * end, uint32_t type)
{
	uint32_t length;
	if(p + 8 < end) {
...
cairo_int_status_t _cairo_image_info_get_jpx_info(cairo_image_info_t * info, const uchar * data, ulong length)
{
	const uchar * p = data;
	...
	if(!p)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	// Step into the JP2 header box. First box must be the Image Header 
...
cairo_int_status_t _cairo_image_info_get_png_info(cairo_image_info_t * info, const uchar * data, ulong length)
{
	const uchar * p = data;
	const uchar * end = data + length;
...
static const uchar * _jbig2_find_data_end(const uchar * p, const uchar * end, int type)
{
	uchar end_seq[2];
	int mmr;
...
static const uchar * _jbig2_get_next_segment(const uchar * p, const uchar  * end, int * type,
    const uchar ** data, ulong * data_len)
{
	ulong seg_num;
...
cairo_int_status_t _cairo_image_info_get_jbig2_info(cairo_image_info_t * info, const uchar   * data, ulong length)
{
	const uchar * p = data;
	const uchar * end = data + length;
	int seg_type;
