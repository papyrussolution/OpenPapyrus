...
static void _add_edge(cairo_polygon_t * polygon, const cairo_point_t * p1, const cairo_point_t * p2, int top, int bottom, int dir)
{
	cairo_edge_t * edge;
	assert(top < bottom);
...
static void _add_clipped_edge(cairo_polygon_t * polygon, const cairo_point_t * p1, const cairo_point_t * p2, const int top, const int bottom, const int dir)
{
	...
	for(n = 0; n < polygon->num_limits; n++) {
		...
		if(bottom <= limits->p1.y)
			continue;
		if(p1->x >= limits->p1.x && p2->x >= limits->p1.x && p1->x <= limits->p2.x && p2->x <= limits->p2.x) {
			top_y = top;
			...
			int p1_y, p2_y;
			int left_y = _cairo_edge_compute_intersection_y_for_x(p1, p2, limits->p1.x);
			int right_y = _cairo_edge_compute_intersection_y_for_x(p1, p2, limits->p2.x);
...
static void _cairo_polygon_add_edge(cairo_polygon_t * polygon, const cairo_point_t * p1, const cairo_point_t * p2)
{
	int dir;
...
{
	_cairo_polygon_add_edge((cairo_polygon_t *)polygon, p1, p2);
...
cairo_status_t _cairo_polygon_move_to(cairo_polygon_t * polygon, const cairo_point_t * point)
{
	if(polygon->has_current_edge) {
		_cairo_polygon_add_edge(polygon, &polygon->last_point, &polygon->current_point);
		polygon->has_current_edge = FALSE;
	}
...
cairo_status_t _cairo_polygon_line_to(cairo_polygon_t * polygon, const cairo_point_t * point)
{
	/* squash collinear edges */
	if(polygon->has_current_edge) {
		if(polygon->current_point.x != point->x || polygon->current_point.y != point->y) {
			cairo_slope_t this_slope;
			_cairo_slope_init(&this_slope, &polygon->current_point, point);
			if(_cairo_slope_equal(&polygon->current_edge, &this_slope)) {
				polygon->current_point = *point;
				return CAIRO_STATUS_SUCCESS;
			}
			_cairo_polygon_add_edge(polygon, &polygon->last_point, &polygon->current_point);
			polygon->last_point = polygon->current_point;
			polygon->current_edge = this_slope;
		}
	}
	else if(polygon->has_current_point) {
		if(polygon->current_point.x != point->x || polygon->current_point.y != point->y) {
			polygon->last_point = polygon->current_point;
			_cairo_slope_init(&polygon->current_edge, &polygon->last_point, point);
...
cairo_status_t _cairo_polygon_close(cairo_polygon_t * polygon)
{
