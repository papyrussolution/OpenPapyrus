...
cairo_status_t _cairo_clip_combine_with_surface(const cairo_clip_t * clip, cairo_surface_t * dst, int dst_x, int dst_y)
{
	cairo_clip_path_t * clip_path;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_clip_t * copy = _cairo_clip_copy_with_translation(clip, -dst_x, -dst_y);
	cairo_clip_path_t * copy_path = copy->path;
	copy->path = NULL;
	if(copy->boxes) {
		status = _cairo_surface_paint(dst, CAIRO_OPERATOR_IN, &_cairo_pattern_white.base, copy);
	}
	clip = NULL;
...
cairo_surface_t * _cairo_clip_get_surface(const cairo_clip_t * clip, cairo_surface_t * target, int * tx, int * ty)
{
	...
	if(clip->num_boxes) {
		...
		if(status == CAIRO_STATUS_SUCCESS)
			status = _cairo_surface_fill(surface, CAIRO_OPERATOR_ADD,
			    &_cairo_pattern_white.base, &path, CAIRO_FILL_RULE_WINDING, 1., CAIRO_ANTIALIAS_DEFAULT, 0);
...
cairo_surface_t * _cairo_clip_get_image(const cairo_clip_t * clip, cairo_surface_t * target, const CairoIRect * extents)
{
	cairo_status_t status;
	cairo_surface_t * surface = cairo_surface_create_similar_image(target, CAIRO_FORMAT_A8, extents->width, extents->height);
	if(unlikely(surface->status))
		return surface;
	status = _cairo_surface_paint(surface, CAIRO_OPERATOR_SOURCE, &_cairo_pattern_white.base, 0);
	if(likely(status == CAIRO_STATUS_SUCCESS))
		status = _cairo_clip_combine_with_surface(clip, surface, extents->x, extents->y);
	if(unlikely(status)) {
		cairo_surface_destroy(surface);
