...
static void _cairo_device_observer_lock(void * _device)
{
	...
	cairo_status_t ignored = cairo_device_acquire(device->target);
}

static void _cairo_device_observer_unlock(void * _device)
{
	cairo_device_observer_t * device = (cairo_device_observer_t*)_device;
	cairo_device_release(device->target);
}

static cairo_status_t _cairo_device_observer_flush(void * _device)
{
	cairo_device_observer_t * device = (cairo_device_observer_t*)_device;
...
static void _cairo_device_observer_finish(void * _device)
{
	cairo_device_observer_t * device = (cairo_device_observer_t*)_device;
	log_fini(&device->log);
...
static void _cairo_device_observer_destroy(void * _device)
{
	cairo_device_observer_t * device = (cairo_device_observer_t*)_device;
...
static cairo_device_observer_t * to_device(cairo_surface_observer_t * suface)
{
...
static cairo_surface_t * _cairo_surface_create_observer_internal(cairo_device_t * device, cairo_surface_t * target)
{
	...
	_cairo_surface_init(&surface->base, &_cairo_surface_observer_backend, device, target->content);
...
static inline void do_callbacks(cairo_surface_observer_t * surface, cairo_list_t * head)
{
...
static cairo_status_t _cairo_surface_observer_finish(void * abstract_surface)
{
...
static cairo_surface_t * _cairo_surface_observer_create_similar(void * abstract_other, cairo_content_t content, int width, int height)
{
	cairo_surface_observer_t * other = (cairo_surface_observer_t *)abstract_other;
	cairo_surface_t * surface;
	cairo_surface_t * target = NULL;
	if(other->target->backend->create_similar)
		target = other->target->backend->create_similar(other->target, content, width, height);
	if(target == NULL)
...
static cairo_surface_t * _cairo_surface_observer_create_similar_image(void * other, cairo_format_t format, int width, int height)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)other;
...
static cairo_image_surface_t * _cairo_surface_observer_map_to_image(void * abstract_surface, const CairoIRect * extents)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	return _cairo_surface_map_to_image(surface->target, extents);
}

static cairo_int_status_t _cairo_surface_observer_unmap_image(void * abstract_surface, cairo_image_surface_t * image)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	return _cairo_surface_unmap_image(surface->target, image);
}

static void record_target(cairo_observation_record_t * r, cairo_surface_t * target)
{
	CairoIRect extents;
...
static cairo_observation_record_t * record_paint(cairo_observation_record_t * r, cairo_surface_t * target,
    cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip, cairo_time_t elapsed)
{
...
static cairo_observation_record_t * record_fill(cairo_observation_record_t * r,
    cairo_surface_t            * target,
    cairo_operator_t op,
    const cairo_pattern_t      * source,
...
static cairo_observation_record_t * record_stroke(cairo_observation_record_t * r,
    cairo_surface_t          * target,
    cairo_operator_t op,
    const cairo_pattern_t    * source,
    const cairo_path_fixed_t * path,
...
static cairo_observation_record_t * record_glyphs(cairo_observation_record_t * r,
    cairo_surface_t          * target,
    cairo_operator_t op,
    const cairo_pattern_t    * source,
...
static void FASTCALL add_record(cairo_observation_t * log, cairo_observation_record_t * r)
{
...
static cairo_int_status_t _cairo_surface_observer_paint(void * abstract_surface, cairo_operator_t op,
...
static cairo_int_status_t _cairo_surface_observer_mask(void * abstract_surface, cairo_operator_t op,
...
static void add_record_fill(cairo_observation_t * log, cairo_surface_t * target, cairo_operator_t op,
    const cairo_pattern_t * source, const cairo_path_fixed_t * path, CairoFillRule fill_rule,
    double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip, cairo_time_t elapsed)
{
	cairo_observation_record_t record;
	cairo_int_status_t status;
	add_record(log, record_fill(&record, target, op, source, path, fill_rule, tolerance, antialias, clip, elapsed));
	if(log->record) {
		status = log->record->base.backend->fill(&log->record->base, op, source, path, fill_rule, tolerance, antialias, clip);
		assert(status == CAIRO_INT_STATUS_SUCCESS);
	}
...
static cairo_int_status_t _cairo_surface_observer_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t     * source,
    const cairo_path_fixed_t  * path,
    CairoFillRule fill_rule,
...
static void add_record_stroke(cairo_observation_t * log,
    cairo_surface_t * target,
    cairo_operator_t op,
    const cairo_pattern_t          * source,
    const cairo_path_fixed_t       * path,
    ...
    cairo_time_t elapsed)
{
	cairo_observation_record_t record;
	cairo_int_status_t status;
	add_record(log, record_stroke(&record, target, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip, elapsed));
	if(log->record) {
		status = log->record->base.backend->stroke(&log->record->base, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
		assert(status == CAIRO_INT_STATUS_SUCCESS);
	}
...
static cairo_int_status_t _cairo_surface_observer_stroke(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t           * source,
    const cairo_path_fixed_t        * path,
    const cairo_stroke_style_t      * style,
    ...
    const cairo_clip_t              * clip)
{
	...
	status = _cairo_composite_rectangles_init_for_stroke(&composite, surface->target, op, source, path, style, ctm, clip);
	if(unlikely(status)) {
	...
	_cairo_composite_rectangles_fini(&composite);
	t = _cairo_time_get();
	status = _cairo_surface_stroke(surface->target, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
	if(unlikely(status))
		return status;
	_cairo_surface_sync(surface->target, x, y);
	t = _cairo_time_get_delta(t);
	add_record_stroke(&surface->log, surface->target, op, source, path, style, ctm, ctm_inverse,
	    tolerance, antialias, clip, t);
	add_record_stroke(&device->log, surface->target, op, source, path, style, ctm, ctm_inverse,
	    tolerance, antialias, clip, t);
	do_callbacks(surface, &surface->stroke_callbacks);
	return CAIRO_STATUS_SUCCESS;
}

static void add_record_glyphs(cairo_observation_t  * log,
    cairo_surface_t      * target,
    cairo_operator_t op,
    const cairo_pattern_t* source,
    cairo_glyph_t        * glyphs,
    ...
    cairo_time_t elapsed)
{
	cairo_observation_record_t record;
	cairo_int_status_t status;
	add_record(log, record_glyphs(&record, target, op, source, glyphs, num_glyphs, scaled_font, clip, elapsed));
	if(log->record) {
		status = log->record->base.backend->show_text_glyphs(&log->record->base,
		    op, source, NULL, 0, glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font, clip);
		assert(status == CAIRO_INT_STATUS_SUCCESS);
	}
...
static cairo_int_status_t _cairo_surface_observer_glyphs(void * abstract_surface,
    ...
    cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	status = _cairo_composite_rectangles_init_for_glyphs(&composite, surface->target,
	    op, source, scaled_font, glyphs, num_glyphs, clip, NULL);
	if(unlikely(status)) {
	...
	_cairo_composite_rectangles_fini(&composite);
	// XXX We have to copy the glyphs, because the backend is allowed to modify! 
	...
	t = _cairo_time_get();
	status = _cairo_surface_show_text_glyphs(surface->target, op, source, NULL, 0, dev_glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font, clip);
	SAlloc::F(dev_glyphs);
	if(unlikely(status))
		return status;
	_cairo_surface_sync(surface->target, x, y);
	t = _cairo_time_get_delta(t);
	add_record_glyphs(&surface->log, surface->target, op, source, glyphs, num_glyphs, scaled_font, clip, t);
	add_record_glyphs(&device->log, surface->target, op, source, glyphs, num_glyphs, scaled_font, clip, t);
	do_callbacks(surface, &surface->glyphs_callbacks);
	return CAIRO_STATUS_SUCCESS;
}

static cairo_status_t _cairo_surface_observer_flush(void * abstract_surface, unsigned flags)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
...
static cairo_status_t _cairo_surface_observer_mark_dirty(void * abstract_surface, int x, int y, int width, int height)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	cairo_status_t status;
	...
	if(surface->target->backend->mark_dirty_rectangle)
		status = surface->target->backend->mark_dirty_rectangle(surface->target, x, y, width, height);
	return status;
}

static cairo_int_status_t _cairo_surface_observer_copy_page(void * abstract_surface)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
...
static cairo_int_status_t _cairo_surface_observer_show_page(void * abstract_surface)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
...
static cairo_bool_t _cairo_surface_observer_get_extents(void * abstract_surface, CairoIRect * extents)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	return _cairo_surface_get_extents(surface->target, extents);
}

static void _cairo_surface_observer_get_font_options(void * abstract_surface, cairo_font_options_t * options)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
...
static cairo_surface_t * _cairo_surface_observer_source(void * abstract_surface, CairoIRect * extents)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	return _cairo_surface_get_source(surface->target, extents);
}

static cairo_status_t _cairo_surface_observer_acquire_source_image(void * abstract_surface, cairo_image_surface_t ** image_out, void ** image_extra)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	surface->log.num_sources_acquired++;
	to_device(surface)->log.num_sources_acquired++;
	return _cairo_surface_acquire_source_image(surface->target, image_out, image_extra);
}

static void _cairo_surface_observer_release_source_image(void * abstract_surface, cairo_image_surface_t  * image, void * image_extra)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	_cairo_surface_release_source_image(surface->target, image, image_extra);
}

static cairo_surface_t * _cairo_surface_observer_snapshot(void * abstract_surface)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)abstract_surface;
	/* XXX hook onto the snapshot so that we measure number of reads */
	return surface->target->backend->snapshot ? surface->target->backend->snapshot(surface->target) : NULL;
}

static cairo_t * _cairo_surface_observer_create_context(void * target)
{
	cairo_surface_observer_t * surface = (cairo_surface_observer_t *)target;
	if(_cairo_surface_is_subsurface(&surface->base))
		surface = (cairo_surface_observer_t*)_cairo_surface_subsurface_get_target(&surface->base);
...
cairo_surface_t * cairo_surface_create_observer(cairo_surface_t * target, cairo_surface_observer_mode_t mode)
{
	...
	else if(unlikely(target->finished))
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_SURFACE_FINISHED));
	else {
		cairo_bool_t record = mode & CAIRO_SURFACE_OBSERVER_RECORD_OPERATIONS;
		cairo_device_t * device = _cairo_device_create_observer_internal(target->device, record);
		if(unlikely(device->status))
			return _cairo_surface_create_in_error(device->status);
		else {
			cairo_surface_t * surface = _cairo_surface_create_observer_internal(device, target);
...
cairo_status_t cairo_surface_observer_add_paint_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	...
	else if(!_cairo_surface_is_observer(abstract_surface))
		return _cairo_error(CAIRO_STATUS_SURFACE_TYPE_MISMATCH);
	else {
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
		return _cairo_surface_observer_add_callback(&surface->paint_callbacks, func, data);
	}
}
...
cairo_status_t cairo_surface_observer_add_mask_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	...
	return _cairo_surface_observer_add_callback(&surface->mask_callbacks, func, data);
}
...
cairo_status_t cairo_surface_observer_add_fill_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	if(unlikely(CAIRO_REFERENCE_COUNT_IS_INVALID(&abstract_surface->ref_count)))
		...
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
		return _cairo_surface_observer_add_callback(&surface->fill_callbacks, func, data);
	}
}
...
cairo_status_t cairo_surface_observer_add_stroke_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	if(unlikely(CAIRO_REFERENCE_COUNT_IS_INVALID(&abstract_surface->ref_count)))
		...
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
		return _cairo_surface_observer_add_callback(&surface->stroke_callbacks, func, data);
	}
}
...
cairo_status_t cairo_surface_observer_add_glyphs_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	if(unlikely(CAIRO_REFERENCE_COUNT_IS_INVALID(&abstract_surface->ref_count)))
		...
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
		return _cairo_surface_observer_add_callback(&surface->glyphs_callbacks, func, data);
	}
}
...
cairo_status_t cairo_surface_observer_add_flush_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	if(unlikely(CAIRO_REFERENCE_COUNT_IS_INVALID(&abstract_surface->ref_count)))
		...
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
		return _cairo_surface_observer_add_callback(&surface->flush_callbacks, func, data);
	}
}
...
cairo_status_t cairo_surface_observer_add_finish_callback(cairo_surface_t * abstract_surface, cairo_surface_observer_callback_t func, void * data)
{
	...
	if(!_cairo_surface_is_observer(abstract_surface))
		return _cairo_error(CAIRO_STATUS_SURFACE_TYPE_MISMATCH);
	surface = (cairo_surface_observer_t*)abstract_surface;
	return _cairo_surface_observer_add_callback(&surface->finish_callbacks, func, data);
}

static void print_extents(cairo_output_stream_t * stream, const struct extents * e)
{
	_cairo_output_stream_printf(stream, "  extents: total %g, avg %g [unbounded %d]\n",
	    e->area.sum, e->area.sum / e->area.count, e->unbounded);
}

static inline int ordercmp(int a, int b, const void * pArray)
{
	/* high to low */
	return ((const uint *)pArray)[b] - ((const uint *)pArray)[a];
}

CAIRO_COMBSORT_DECLARE_WITH_DATA(sort_order, int, ordercmp)

static void print_array(cairo_output_stream_t * stream, const uint * array, const char ** names, int count)
{
	int order[64];
	int i, j;
...
static void print_pattern(cairo_output_stream_t * stream, const char * name, const struct pattern * p)
{
...
{
	/* Fake %.1f */
	return _cairo_round(_cairo_time_to_s(a) * 1000 / _cairo_time_to_s(b)) / 10;
}

static cairo_bool_t replay_record(cairo_observation_t * log, cairo_observation_record_t * r, cairo_device_t * script)
{
#if CAIRO_HAS_SCRIPT_SURFACE
	cairo_surface_t * surface;
	cairo_int_status_t status;
	if(log->record == NULL || script == NULL)
		return FALSE;
	surface = cairo_script_surface_create(script, r->target_content, r->target_width, r->target_height);
	status = _cairo_recording_surface_replay_one(log->record, r->index, surface);
...
static cairo_time_t _cairo_observation_total_elapsed(cairo_observation_t * log)
{
	cairo_time_t total = log->paint.elapsed;
...
static void _cairo_observation_print(cairo_output_stream_t * stream, cairo_observation_t * log)
{
	...
	_cairo_output_stream_printf(stream, "paint: count %d [no-op %d], elapsed %f [%f%%]\n",
	    log->paint.count, log->paint.noop, _cairo_time_to_ns(log->paint.elapsed), percent(log->paint.elapsed, total));
	if(log->paint.count) {
...
cairo_status_t cairo_surface_observer_print(cairo_surface_t * abstract_surface, cairo_write_func_t write_func, void * closure)
{
	if(unlikely(abstract_surface->status))
		return abstract_surface->status;
	else if(unlikely(!_cairo_surface_is_observer(abstract_surface)))
		return _cairo_error(CAIRO_STATUS_SURFACE_TYPE_MISMATCH);
	else {
		cairo_surface_observer_t * surface = (cairo_surface_observer_t*)abstract_surface;
...
double cairo_device_observer_mask_elapsed(cairo_device_t * abstract_device)
{
	if(unlikely(CAIRO_REFERENCE_COUNT_IS_INVALID(&abstract_device->ref_count)))
		return -1;
	else if(!_cairo_device_is_observer(abstract_device))
		return -1;
	else {
		cairo_device_observer_t * device = (cairo_device_observer_t*)abstract_device;
