...
static int _cairo_path_count(cairo_path_t * path, cairo_path_fixed_t * path_fixed, double tolerance, cairo_bool_t flatten)
{
	...
	if(flatten) {
		status = _cairo_path_fixed_interpret_flat(path_fixed,
			CAIRO_DIRECTION_FORWARD, _cpc_move_to, _cpc_line_to, _cpc_close_path, &cpc, tolerance);
	}
	else {
		status = _cairo_path_fixed_interpret(path_fixed,
			CAIRO_DIRECTION_FORWARD, _cpc_move_to, _cpc_line_to, _cpc_curve_to, _cpc_close_path, &cpc);
	}
	if(unlikely(status))
		return -1;
	return cpc.count;
...
static cairo_status_t _cairo_path_populate(cairo_path_t * path,
	cairo_path_fixed_t * path_fixed, cairo_gstate_t * gstate, cairo_bool_t flatten)
{
	cairo_status_t status;
	cpp_t cpp;
	cpp.data = path->data;
...
cairo_path_t * _cairo_path_create_in_error(cairo_status_t status)
{
	cairo_path_t * path;
...
static cairo_path_t * _cairo_path_create_internal(cairo_path_fixed_t * path_fixed, cairo_gstate_t * gstate, cairo_bool_t flatten)
{
	...
	path->num_data = _cairo_path_count(path, path_fixed, _cairo_gstate_get_tolerance(gstate), flatten);
	if(path->num_data < 0) {
		free(path);
		return (cairo_path_t *)&_cairo_path_nil;
	}
	if(path->num_data) {
		...
		path->status = _cairo_path_populate(path, path_fixed, gstate, flatten);
	}
	else {
		path->data = NULL;
 ...
 * data==%NULL.
 **/
cairo_path_t * _cairo_path_create(cairo_path_fixed_t * path, cairo_gstate_t * gstate)
{
 ...
 * data==%NULL.
 **/
cairo_path_t * _cairo_path_create_flat(cairo_path_fixed_t * path, cairo_gstate_t * gstate)
{
...
cairo_status_t _cairo_path_append_to_context(const cairo_path_t * path, cairo_t * cr)
{
	...
	cairo_status_t status;
	double x, y;
	cairo_matrix_t user_to_backend = cr->gstate->ctm;
	cairo_matrix_multiply(&user_to_backend, &user_to_backend, &cr->gstate->target->device_transform);
