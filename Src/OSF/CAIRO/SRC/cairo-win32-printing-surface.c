...
static void _cairo_win32_printing_surface_init_ps_mode(cairo_win32_surface_t * surface)
{
	DWORD word;
...
static void _cairo_win32_printing_surface_init_image_support(cairo_win32_surface_t * surface)
{
	DWORD word;
 ...
 * and argument 0.
 */
static void _cairo_win32_printing_surface_init_language_pack(cairo_win32_surface_t * surface)
{
...
static cairo_int_status_t analyze_surface_pattern_transparency(cairo_surface_pattern_t * pattern)
{
...
static cairo_bool_t pattern_supported(cairo_win32_surface_t * surface, const cairo_pattern_t * pattern)
{
...
static cairo_int_status_t _cairo_win32_printing_surface_analyze_operation(cairo_win32_surface_t * surface,
	cairo_operator_t op,
...
static cairo_bool_t _cairo_win32_printing_surface_operation_supported(cairo_win32_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * pattern)
{
...
static void _cairo_win32_printing_surface_init_clear_color(cairo_win32_surface_t * surface,
...
static COLORREF _cairo_win32_printing_surface_flatten_transparency(cairo_win32_surface_t * surface,
...
static cairo_status_t _cairo_win32_printing_surface_select_solid_brush(cairo_win32_surface_t * surface,
	const cairo_pattern_t * source)
{
	...
	surface->old_brush = (HBRUSH)SelectObject(surface->dc, surface->brush);
	return CAIRO_STATUS_SUCCESS;
}

static void _cairo_win32_printing_surface_done_solid_brush(cairo_win32_surface_t * surface)
{
...
static cairo_status_t _cairo_win32_printing_surface_get_ctm_clip_box(cairo_win32_surface_t * surface, RECT * clip)
{
	XFORM xform;
...
static cairo_status_t _cairo_win32_printing_surface_paint_solid_pattern(cairo_win32_surface_t * surface,
	const cairo_pattern_t * pattern)
{
	RECT clip;
...
static cairo_status_t _cairo_win32_printing_surface_paint_recording_pattern(cairo_win32_surface_t * surface,
...
static cairo_int_status_t _cairo_win32_printing_surface_check_jpeg(cairo_win32_surface_t * surface,
...
static cairo_int_status_t _cairo_win32_printing_surface_check_png(cairo_win32_surface_t * surface,
	cairo_surface_t * source, const unsigned char ** data, unsigned long * length, cairo_image_info_t * info)
{
	const unsigned char * mime_data;
	unsigned long mime_data_length;
	cairo_int_status_t status;
	...
	if(!(surface->flags&CAIRO_WIN32_SURFACE_CAN_CHECK_PNG))
		return CAIRO_INT_STATUS_UNSUPPORTED;
	cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_PNG, &mime_data, &mime_data_length);
	if(mime_data == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	...
	if(status)
		return status;
	result = 0;
	if(ExtEscape(surface->dc, CHECKPNGFORMAT, mime_data_length, (char *)mime_data, sizeof(result), (char *)&result) <= 0)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	if(result != 1)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	*data = mime_data;
...
static cairo_status_t _cairo_win32_printing_surface_paint_image_pattern(cairo_win32_surface_t * surface, cairo_surface_pattern_t * pattern)
{
	cairo_status_t status;
	cairo_extend_t extend;
	cairo_image_surface_t * image;
	...
	mime_type = BI_JPEG;
	status = _cairo_win32_printing_surface_check_jpeg(surface, pattern->surface, &mime_data, &mime_size, &mime_info);
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		mime_type = BI_PNG;
		status = _cairo_win32_printing_surface_check_png(surface, pattern->surface, &mime_data, &mime_size, &mime_info);
	}
...
static cairo_status_t _cairo_win32_printing_surface_paint_surface_pattern(cairo_win32_surface_t * surface,
	cairo_surface_pattern_t * pattern)
{
	if(_cairo_surface_is_recording(pattern->surface)) {
...
static cairo_int_status_t _cairo_win32_printing_surface_paint_linear_pattern(cairo_win32_surface_t * surface,
...
static cairo_int_status_t _cairo_win32_printing_surface_paint_pattern(cairo_win32_surface_t * surface,
	const cairo_pattern_t * pattern)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_win32_printing_surface_path_move_to(void * closure, const cairo_point_t * point)
{
	...
	if(path_info->surface->has_ctm) {
		...
		MoveToEx(path_info->surface->dc, _cairo_fixed_integer_part(point->x), _cairo_fixed_integer_part(point->y), NULL);
	}
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _cairo_win32_printing_surface_emit_path(cairo_win32_surface_t * surface, cairo_path_fixed_t * path)
{
...
{
	cairo_win32_surface_t * surface = cairo_container_of(clipper, cairo_win32_surface_t, clipper);
	cairo_status_t status;
...
static cairo_int_status_t _cairo_win32_printing_surface_paint(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	cairo_win32_surface_t * surface = (cairo_win32_surface_t *)abstract_surface;
	...
	if(cairo_scaled_font_get_type(scaled_font) == CAIRO_FONT_TYPE_WIN32 &&
	   source->type == CAIRO_PATTERN_TYPE_SOLID) {
		...
		if(_cairo_win32_scaled_font_is_type1(scaled_font)) {
			type1_glyphs = (cairo_glyph_t *)_cairo_malloc_ab(num_glyphs, sizeof(cairo_glyph_t));
...
cairo_surface_t * cairo_win32_printing_surface_create(HDC hdc)
{
	...
	paginated = _cairo_paginated_surface_create(&surface->base, CAIRO_CONTENT_COLOR_ALPHA, &cairo_win32_surface_paginated_backend);
	/* paginated keeps the only reference to surface now, drop ours */
	cairo_surface_destroy(&surface->base);
	return paginated;
}

cairo_bool_t _cairo_surface_is_win32_printing(cairo_surface_t * surface)
{
