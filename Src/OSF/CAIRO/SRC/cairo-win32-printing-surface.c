...
static void FASTCALL _cairo_win32_printing_surface_init_ps_mode(cairo_win32_printing_surface_t * surface)
{
...
static void FASTCALL _cairo_win32_printing_surface_init_image_support(cairo_win32_printing_surface_t * surface)
{
 ...
 * and argument 0.
 */
static void FASTCALL _cairo_win32_printing_surface_init_language_pack(cairo_win32_printing_surface_t * surface)
{
...
static cairo_int_status_t FASTCALL analyze_surface_pattern_transparency(cairo_surface_pattern_t * pattern)
{
	cairo_image_surface_t * image;
	void * image_extra;
	cairo_int_status_t status = _cairo_surface_acquire_source_image(pattern->surface, &image, &image_extra);
	if(!status) {
		cairo_image_transparency_t transparency = _cairo_image_analyze_transparency(image);
		switch(transparency) {
...
static cairo_bool_t FASTCALL surface_pattern_supported(const cairo_surface_pattern_t * pattern)
{
	if(_cairo_surface_is_recording(pattern->surface))
		return TRUE;
	else if(cairo_surface_get_type(pattern->surface) != CAIRO_SURFACE_TYPE_WIN32 && pattern->surface->backend->acquire_source_image == NULL)
		return FALSE;
	else
		return TRUE;
}

static cairo_bool_t FASTCALL pattern_supported(cairo_win32_printing_surface_t * surface, const cairo_pattern_t * pattern)
{
    ...
    cairo_operator_t op, const cairo_pattern_t * pattern)
{
	...
	if(!oneof3(op, CAIRO_OPERATOR_SOURCE, CAIRO_OPERATOR_OVER, CAIRO_OPERATOR_CLEAR))
		return CAIRO_INT_STATUS_UNSUPPORTED;
...
static void FASTCALL _cairo_win32_printing_surface_init_clear_color(cairo_win32_printing_surface_t * surface, cairo_solid_pattern_t * color)
{
...
static COLORREF FASTCALL _cairo_win32_printing_surface_flatten_transparency(cairo_win32_printing_surface_t * surface, const cairo_color_t * color)
{
...
static cairo_status_t FASTCALL _cairo_win32_printing_surface_select_solid_brush(cairo_win32_printing_surface_t * surface, const cairo_pattern_t * source)
{
	...
	if(!surface->brush)
		...
		surface->old_brush = (HBRUSH)SelectObject(surface->win32.dc, surface->brush);
		return CAIRO_STATUS_SUCCESS;
	}
}

static void FASTCALL _cairo_win32_printing_surface_done_solid_brush(cairo_win32_printing_surface_t * surface)
{
...
static cairo_status_t FASTCALL _cairo_win32_printing_surface_get_ctm_clip_box(cairo_win32_printing_surface_t * surface, RECT * clip)
{
	XFORM xform;
...
static cairo_status_t FASTCALL _cairo_win32_printing_surface_paint_solid_pattern(cairo_win32_printing_surface_t * surface, const cairo_pattern_t * pattern)
{
	RECT clip;
    ...
    cairo_surface_pattern_t * pattern)
{
	...
	cairo_status_t status = cairo_matrix_invert(&p2d);
...
static cairo_status_t _cairo_win32_printing_surface_paint_image_pattern(cairo_win32_printing_surface_t   * surface,
    cairo_surface_pattern_t * pattern)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_win32_printing_surface_paint_surface_pattern(cairo_win32_printing_surface_t * surface, cairo_surface_pattern_t * pattern)
{
	if(_cairo_surface_is_recording(pattern->surface))
		return _cairo_win32_printing_surface_paint_recording_pattern(surface, pattern);
	else
		return _cairo_win32_printing_surface_paint_image_pattern(surface, pattern);
}
...
static cairo_int_status_t _cairo_win32_printing_surface_paint_linear_pattern(cairo_win32_printing_surface_t * surface,
...
static cairo_int_status_t _cairo_win32_printing_surface_paint_pattern(cairo_win32_printing_surface_t * surface,
...
static cairo_status_t _cairo_win32_printing_surface_clipper_intersect_clip_path(cairo_surface_clipper_t * clipper,
    cairo_path_fixed_t * path, CairoFillRule fill_rule,
...
static void _cairo_win32_printing_surface_get_font_options(void * abstract_surface, cairo_font_options_t  * options)
{
...
static cairo_int_status_t _cairo_win32_printing_surface_paint(void * abstract_surface,
...
static int _cairo_win32_line_cap(cairo_line_cap_t cap)
{
	switch(cap) {
...
static int _cairo_win32_line_join(cairo_line_join_t join)
{
	switch(join) {
...
static cairo_int_status_t _cairo_win32_printing_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
    ...
    const cairo_clip_t    * clip)
{
	...
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
		...
		return (style->num_dashes > 0 && style->dash_offset != 0.0) ? CAIRO_INT_STATUS_UNSUPPORTED : _cairo_win32_printing_surface_analyze_operation(surface, op, source);
	}
	assert(_cairo_win32_printing_surface_operation_supported(surface, op, source));
	...
	pen = ExtCreatePen(pen_style, (DWORD)(scale * style->line_width), &brush, style->num_dashes, dash_array);
	if(pen == NULL)
...
static cairo_int_status_t _cairo_win32_printing_surface_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t       * source,
    const cairo_path_fixed_t    * path,
...
static cairo_int_status_t _cairo_win32_printing_surface_emit_win32_glyphs(cairo_win32_printing_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t  * source,
...
static cairo_int_status_t _cairo_win32_printing_surface_show_glyphs(void * abstract_surface,
    ...
    int num_glyphs, cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
		...
		if(cairo_scaled_font_get_type(scaled_font) == CAIRO_FONT_TYPE_WIN32) {
			return _cairo_win32_scaled_font_is_bitmap(scaled_font) ? CAIRO_INT_STATUS_UNSUPPORTED : _cairo_win32_printing_surface_analyze_operation(surface, op, source);
		}
#endif
		...
		return status ? status : _cairo_win32_printing_surface_analyze_operation(surface, op, source);
	}
	if(source->type == CAIRO_PATTERN_TYPE_SOLID) {
		cairo_solid_pattern_t * solid = (cairo_solid_pattern_t*)source;
	...
	if(cairo_scaled_font_get_type(scaled_font) == CAIRO_FONT_TYPE_WIN32 && source->type == CAIRO_PATTERN_TYPE_SOLID) {
		return _cairo_win32_printing_surface_emit_win32_glyphs(surface, op, source, glyphs, num_glyphs, scaled_font, clip);
	}
#endif
	SaveDC(surface->win32.dc);
	old_ctm = surface->ctm;
...
static const char ** _cairo_win32_printing_surface_get_supported_mime_types(void * abstract_surface)
{
...
cairo_surface_t * cairo_win32_printing_surface_create(HDC hdc)
{
...
#endif
	_cairo_surface_clipper_init(&surface->clipper, _cairo_win32_printing_surface_clipper_intersect_clip_path);
	surface->win32.format = CAIRO_FORMAT_RGB24;
	surface->content = CAIRO_CONTENT_COLOR_ALPHA; // @v1.14.6 surface->win32.base.content-->surface->content
	...
	_cairo_surface_init(&surface->win32.base, &cairo_win32_printing_surface_backend, NULL, /* device */ CAIRO_CONTENT_COLOR_ALPHA);
