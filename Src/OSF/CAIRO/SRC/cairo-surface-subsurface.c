...
static cairo_surface_t * _cairo_surface_subsurface_create_similar(void * other,
	...
	cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	...
	if(unlikely(status))
		goto CLEANUP;
	status = _cairo_surface_offset_paint(surface->target, -surface->extents.x, -surface->extents.y, op, source, &target_clip);
CLEANUP:
	...
	cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, cairo_clip_t * clip)
{
	...
	if(unlikely(status))
		goto CLEANUP;
	status = _cairo_surface_offset_mask(surface->target, -surface->extents.x, -surface->extents.y, op, source, mask, &target_clip);
CLEANUP:
	_cairo_clip_fini(&target_clip);
	return status;
}
...
static cairo_int_status_t _cairo_surface_subsurface_fill(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule,
	double tolerance, cairo_antialias_t antialias, cairo_clip_t * clip)
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
	cairo_rectangle_int_t rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_status_t status;
	...
	if(unlikely(status))
		goto CLEANUP;
	status = _cairo_surface_offset_fill(surface->target, -surface->extents.x, -surface->extents.y, op, source, path, fill_rule, tolerance, antialias, &target_clip);
CLEANUP:
	_cairo_clip_fini(&target_clip);
	return status;
}
...
static cairo_int_status_t _cairo_surface_subsurface_stroke(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, cairo_path_fixed_t * path,
	...
	double tolerance, cairo_antialias_t antialias, cairo_clip_t * clip)
{
	...
	if(unlikely(status))
		goto CLEANUP;
	status = _cairo_surface_offset_stroke(surface->target, -surface->extents.x, -surface->extents.y,
	...
	int num_glyphs, cairo_scaled_font_t * scaled_font, cairo_clip_t * clip, int * remaining_glyphs)
{
	...
	if(unlikely(status))
		goto CLEANUP;
	status = _cairo_surface_offset_glyphs(surface->target, -surface->extents.x, -surface->extents.y,
...
static cairo_status_t _cairo_surface_subsurface_flush(void * abstract_surface)
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
...
static void cairo_surface_paint_to_target(cairo_surface_t * target, cairo_surface_subsurface_t * subsurface)
{
	cairo_t * cr = cairo_create(target);
	cairo_set_source_surface(cr, subsurface->target, -subsurface->extents.x, -subsurface->extents.y);
	...
	cairo_image_surface_t ** image_out, void ** extra_out)
{
	...
	if(surface->target->type == CAIRO_SURFACE_TYPE_RECORDING) {
		...
		cairo_surface_t * snapshot = _cairo_surface_has_snapshot(&surface->base, &_cairo_image_surface_backend);
		if(snapshot != NULL) {
			*image_out = (cairo_image_surface_t *)cairo_surface_reference(snapshot);
			*extra_out = NULL;
			return CAIRO_STATUS_SUCCESS;
		}
		if(!_cairo_surface_has_snapshot(&meta->base, &_cairo_image_surface_backend)) {
			image = (cairo_image_surface_t *)_cairo_image_surface_create_with_content(meta->content,
	...
	extra = (struct extra *)malloc(sizeof(struct extra));
	...
	if(PIXMAN_FORMAT_BPP(extra->image->pixman_format) >= 8 &&
	   target_extents.x <= surface->extents.x && target_extents.y <= surface->extents.y &&
	   ...
	   surface->extents.y+surface->extents.height <= target_extents.y+target_extents.height) {
		...
		image = (cairo_image_surface_t *)_cairo_image_surface_create_with_pixman_format(data,
			extra->image->pixman_format, surface->extents.width, surface->extents.height, extra->image->stride);
		if(unlikely((status = image->base.status)))
			goto CLEANUP_IMAGE;
		image->base.is_clear = FALSE;
	}
	else {
		image = (cairo_image_surface_t *)_cairo_image_surface_create_with_pixman_format(NULL,
			extra->image->pixman_format, surface->extents.width, surface->extents.height, 0);
		if(unlikely((status = image->base.status)))
			goto CLEANUP_IMAGE;
		cairo_surface_paint_to_target(&image->base, surface);
	}
...
static void _cairo_surface_subsurface_release_source_image(void * abstract_surface, cairo_image_surface_t * image, void * abstract_extra)
{
	...
	if(abstract_extra != NULL) {
		struct extra * extra = (struct extra *)abstract_extra;
		_cairo_surface_release_source_image(surface->target, extra->image, extra->image_extra);
		free(extra);
	}
...
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
	cairo_surface_subsurface_t * snapshot = (cairo_surface_subsurface_t *)malloc(sizeof(cairo_surface_subsurface_t));
	if(unlikely(snapshot == NULL))
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
	_cairo_surface_init(&snapshot->base, &_cairo_surface_subsurface_backend, NULL, /* device */ surface->target->content);
	snapshot->target = _cairo_surface_snapshot(surface->target);
	if(unlikely(snapshot->target->status)) {
		cairo_status_t status = snapshot->target->status;
		free(snapshot);
...
cairo_surface_t * cairo_surface_create_for_rectangle(cairo_surface_t * target, double x, double y, double width, double height)
{
	...
	surface = (cairo_surface_subsurface_t *)malloc(sizeof(cairo_surface_subsurface_t));
	if(unlikely(surface == NULL))
		return _cairo_surface_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
	assert(_cairo_matrix_is_translation(&target->device_transform));
	x += target->device_transform.x0;
	y += target->device_transform.y0;
	_cairo_surface_init(&surface->base, &_cairo_surface_subsurface_backend, NULL, /* device */ target->content);
