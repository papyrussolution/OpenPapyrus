...
static void _cairo_arc_in_direction(cairo_t * cr, double xc, double yc, double radius, double angle_min, double angle_max, /*cairo_direction_t dir*/int reverse)
{
	if(cairo_status(cr))
		return;
	...
	if(angle_max - angle_min > M_PI) {
		...
		if(/*dir == CAIRO_DIRECTION_FORWARD*/!reverse) {
			_cairo_arc_in_direction(cr, xc, yc, radius, angle_min, angle_mid, /*dir*/reverse); // @recursion
			_cairo_arc_in_direction(cr, xc, yc, radius, angle_mid, angle_max, /*dir*/reverse); // @recursion
		}
		else {
			_cairo_arc_in_direction(cr, xc, yc, radius, angle_mid, angle_max, /*dir*/reverse);
			_cairo_arc_in_direction(cr, xc, yc, radius, angle_min, angle_mid, /*dir*/reverse);
		}
	}
		...
		segments -= 1;
		if(/*dir == CAIRO_DIRECTION_REVERSE*/reverse) {
			/* @sobolev double t = angle_min;
			angle_min = angle_max;
			angle_max = t;*/
			Exchange(&angle_min, &angle_max); // @sobolev
			step = -step;
		}
...
{
	_cairo_arc_in_direction(cr, xc, yc, radius, angle1, angle2, /*CAIRO_DIRECTION_FORWARD*/0);
}
/**
...
{
	_cairo_arc_in_direction(cr, xc, yc, radius, angle2, angle1, /*CAIRO_DIRECTION_REVERSE*/1);
}
