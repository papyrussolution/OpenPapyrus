...
static cairo_int_status_t _cairo_image_surface_fill(void * dst, cairo_operator_t op, const cairo_pattern_t * source,
	cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, double tolerance, cairo_antialias_t antialias,
	cairo_clip_t * clip);

static pixman_image_t * _pixman_image_for_solid(const cairo_solid_pattern_t * pattern);
...
{
	/*cairo_content_t*/ int content = CAIRO_CONTENT_UNDEF;
	if(PIXMAN_FORMAT_RGB(pixman_format))
		content |= CAIRO_CONTENT_COLOR;
	if(PIXMAN_FORMAT_A(pixman_format))
		content |= CAIRO_CONTENT_ALPHA;
	return (cairo_content_t)content;
}
...
static cairo_surface_t * _cairo_image_surface_create_similar(void * abstract_other,
	cairo_content_t content, int width, int height)
{
	...
	if(content == other->base.content) {
		return _cairo_image_surface_create_with_pixman_format(NULL,
			other->pixman_format, width, height, 0);
	}
	return _cairo_image_surface_create_with_content(content, width, height);
}

static cairo_status_t _cairo_image_surface_finish(void * abstract_surface)
{
...
{
	*image_out = (cairo_image_surface_t *)abstract_surface;
	*image_extra = NULL;
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _cairo_image_surface_set_clip_region(cairo_image_surface_t * surface,
...
static void _cairo_image_surface_unset_clip_region(cairo_image_surface_t * surface)
{
...
static cairo_bool_t _nearest_sample(cairo_filter_t filter, double * tx, double * ty)
{
...
static pixman_image_t * _pixman_image_for_gradient(const cairo_gradient_pattern_t * pattern,
	const cairo_rectangle_int_t * extents, int * ix, int * iy)
{
	...
	if(pattern->base.type == CAIRO_PATTERN_TYPE_LINEAR) {
		...
		cairo_fixed_t xdim = (cairo_fixed_t)fabs((double)(linear->p2.x-linear->p1.x));
		cairo_fixed_t ydim = (cairo_fixed_t)fabs((double)(linear->p2.y-linear->p1.y));
		/*
		 ...
		 */
		if(_cairo_fixed_integer_ceil(xdim) > PIXMAN_MAX_INT || _cairo_fixed_integer_ceil(ydim) > PIXMAN_MAX_INT) {
			double sf;
			if(xdim > ydim)
...
static void _acquire_source_cleanup(pixman_image_t * pixman_image, void * closure)
{
	struct acquire_source_cleanup * data = (struct acquire_source_cleanup *)closure;
	_cairo_surface_release_source_image(data->surface, data->image, data->image_extra);
	free(data);
}

static cairo_filter_t sampled_area(const cairo_surface_pattern_t * pattern,
	const cairo_rectangle_int_t * extents, cairo_rectangle_int_t * sample)
{
	cairo_filter_t filter;
	double pad;
	double x1 = extents->x;
	double y1 = extents->y;
	double x2 = extents->x+(int)extents->width;
	double y2 = extents->y+(int)extents->height;
	_cairo_matrix_transform_bounding_box(&pattern->base.matrix, &x1, &y1, &x2, &y2, NULL);
	filter = _cairo_pattern_analyze_filter(&pattern->base, &pad);
	sample->x = (int)floor(x1-pad);
	sample->y = (int)floor(y1-pad);
	sample->width  = (int)ceil(x2+pad)-sample->x;
	sample->height = (int)ceil(y2+pad)-sample->y;
	return filter;
}

static uint16_t expand_channel(uint16_t v, uint32_t bits)
{
	int offset = 16-bits;
	while(offset > 0) {
		v |= v>>bits;
...
static pixman_image_t * _pixman_image_for_surface(const cairo_surface_pattern_t * pattern,
	...
	int * ix, int * iy)
{
	...
	if(pattern->surface->type == CAIRO_SURFACE_TYPE_IMAGE &&
	   (!is_mask || !pattern->base.has_component_alpha ||
		...
		if(type == CAIRO_SURFACE_TYPE_IMAGE) {
			...
			if(extend == CAIRO_EXTEND_NONE && _cairo_matrix_is_translation(&pattern->base.matrix) && _nearest_sample(filter, &tx, &ty)) {
				*ix = (int)tx;
				*iy = (int)ty;
			...
			if(is_contained && _cairo_matrix_is_translation(&pattern->base.matrix) && _nearest_sample(filter, &tx, &ty)) {
				*ix = (int)(tx+sub->extents.x);
				*iy = (int)(ty+sub->extents.y);
				return pixman_image_ref(source->pixman_image);
			}
			...
			if(PIXMAN_FORMAT_BPP(source->pixman_format) >= 8) {
				pixman_image = pixman_image_create_bits(source->pixman_format,
					sub->extents.width,
					sub->extents.height,
					(uint32_t *)(source->data+sub->extents.x*
					             PIXMAN_FORMAT_BPP(source->pixman_format)/8+sub->extents.y*
					             source->stride),
					source->stride);
				if(unlikely(pixman_image == NULL))
					return NULL;
	...
	if(pixman_image == NULL) {
		...
		if(unlikely(status))
			return NULL;
		if(sample.width == 1 && sample.height == 1) {
			if(sample.x < 0 || sample.y < 0 || sample.x >= image->width || sample.y >= image->height) {
				if(extend == CAIRO_EXTEND_NONE) {
	...
	*ix = (int)tx;
	*iy = (int)ty;
	if(_cairo_matrix_has_unity_scale(&pattern->base.matrix) && tx == pattern->base.matrix.x0 && ty == pattern->base.matrix.y0) {
...
static pixman_image_t * _pixman_image_for_pattern(const cairo_pattern_t * pattern,
	cairo_bool_t is_mask,
	const cairo_rectangle_int_t * extents,
	int * tx, int * ty)
{
...
static cairo_status_t _cairo_image_surface_fixup_unbounded(cairo_image_surface_t * dst,
...
static cairo_status_t _cairo_image_surface_fixup_unbounded_boxes(cairo_image_surface_t * dst,
...
static cairo_bool_t reduce_alpha_op(cairo_image_surface_t * dst,
	cairo_operator_t op,
...
static pixman_image_t * _create_composite_mask_pattern(cairo_clip_t * clip,
 ...
 */
static cairo_status_t _clip_and_composite_with_mask(cairo_clip_t * clip,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	image_draw_func_t draw_func,
	void * draw_closure,
 ...
 */
static cairo_status_t _clip_and_composite_combine(cairo_clip_t * clip,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	image_draw_func_t draw_func,
	void * draw_closure,
 ...
 */
static cairo_status_t _clip_and_composite_source(cairo_clip_t * clip,
	const cairo_pattern_t * pattern,
	image_draw_func_t draw_func,
	void * draw_closure,
	cairo_image_surface_t * dst,
...
static cairo_status_t _clip_and_composite(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	image_draw_func_t draw_func,
	void * draw_closure,
...
static void _project_line_x_onto_16_16(const cairo_line_t * line,
	cairo_fixed_t top,
...
static void _pixman_image_add_traps(pixman_image_t * image,
	int dst_x, int dst_y,
...
static cairo_status_t _composite_traps(void * closure,
	pixman_image_t * dst,
	pixman_format_code_t dst_format,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	...
	cairo_region_t * clip_region)
{
	composite_traps_info_t * info = (composite_traps_info_t *)closure;
...
static inline uint32_t color_to_uint32(const cairo_color_t * color)
{
	return
...
static inline cairo_bool_t color_to_pixel(const cairo_color_t * color,
	pixman_format_code_t format,
	uint32_t * pixel)
{
	uint32_t c;
...
static inline cairo_bool_t pattern_to_pixel(const cairo_solid_pattern_t * solid,
	cairo_operator_t op,
	pixman_format_code_t format,
	uint32_t * pixel)
{
...
static cairo_status_t _fill_span(void * abstract_renderer,
	int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	fill_span_renderer_t * renderer = (fill_span_renderer_t *)abstract_renderer;
	uint8_t * row;
	unsigned i;
	if(num_spans == 0)
		return CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _fill_unaligned_boxes(cairo_image_surface_t * dst,
	const cairo_pattern_t * pattern,
	uint32_t pixel,
...
static cairo_status_t _cairo_image_surface_span(void * abstract_renderer,
	int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_surface_span_renderer_t * renderer = (cairo_image_surface_span_renderer_t *)abstract_renderer;
	uint8_t * row;
	unsigned i;
	if(num_spans == 0)
		return CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _composite_unaligned_boxes(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	const cairo_boxes_t * boxes,
	const cairo_composite_rectangles_t * extents)
...
static cairo_status_t _composite_boxes(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	cairo_boxes_t * boxes,
	cairo_antialias_t antialias,
...
static cairo_status_t _clip_and_composite_boxes(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_boxes_t * boxes,
	cairo_antialias_t antialias,
...
static cairo_status_t _clip_and_composite_trapezoids(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_traps_t * traps,
	cairo_antialias_t antialias,
...
static cairo_clip_path_t * _clip_get_single_path(cairo_clip_t * clip)
{
...
static cairo_int_status_t _cairo_image_surface_paint(void * abstract_surface,
...
static cairo_status_t _composite_mask(void * closure,
	pixman_image_t * dst,
	pixman_format_code_t dst_format,
	cairo_operator_t op,
	...
	cairo_region_t * clip_region)
{
	const cairo_pattern_t * mask_pattern = (const cairo_pattern_t *)closure;
...
static cairo_int_status_t _cairo_image_surface_mask(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, cairo_clip_t * clip)
{
	...
	cairo_bool_t have_clip = FALSE;
	cairo_status_t status;
	status = _cairo_composite_rectangles_init_for_mask(&extents, surface->width, surface->height, op, source, mask, clip);
	if(unlikely(status))
		return status;
	if(_cairo_clip_contains_extents(clip, &extents))
		clip = NULL;
...
static cairo_status_t _composite_spans(void * closure,
	pixman_image_t * dst,
	pixman_format_code_t dst_format,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	...
	cairo_region_t * clip_region)
{
	...
	composite_spans_info_t * info = (composite_spans_info_t *)closure;
	cairo_image_surface_span_renderer_t renderer;
...
static cairo_status_t _clip_and_composite_polygon(cairo_image_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_polygon_t * polygon,
	cairo_fill_rule_t fill_rule,
...
static cairo_int_status_t _cairo_image_surface_stroke(void * abstract_surface,
	cairo_operator_t op,
...
static cairo_int_status_t _cairo_image_surface_fill(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule,
...
static cairo_status_t _composite_glyphs_via_mask(void * closure,
	pixman_image_t * dst,
	pixman_format_code_t dst_format,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	...
	cairo_region_t * clip_region)
{
	composite_glyphs_info_t * info = (composite_glyphs_info_t *)closure;
	...
	pixman_format_code_t mask_format = (pixman_format_code_t)0; /* silence gcc */
	cairo_status_t status;
	int src_x, src_y;
	int i;
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(glyph_surface->pixman_format != mask_format &&
		   ...
		   PIXMAN_FORMAT_BPP(glyph_surface->pixman_format)) {
			...
			pixman_image_composite32(PIXMAN_OP_SRC, white, mask, new_mask, 0, 0, 0, 0, 0, 0, extents->width, extents->height);
			pixman_image_unref(mask);
			mask = new_mask;
			if(PIXMAN_FORMAT_RGB(mask_format))
				pixman_image_set_component_alpha(mask, TRUE);
		...
		if(glyph_surface->pixman_format == mask_format) {
			pixman_image_composite32(PIXMAN_OP_ADD, glyph_surface->pixman_image, NULL, mask,
				0, 0, 0, 0, x-extents->x, y-extents->y, glyph_surface->width, glyph_surface->height);
		}
		else {
			pixman_image_composite32(PIXMAN_OP_ADD, white, glyph_surface->pixman_image, mask,
				0, 0, 0, 0, x-extents->x, y-extents->y, glyph_surface->width, glyph_surface->height);
		}
	}
	pixman_image_composite32(_pixman_operator(op), src, mask, dst,
		extents->x+src_x, extents->y+src_y, 0, 0, extents->x-dst_x, extents->y-dst_y, extents->width,     extents->height);
...
static cairo_status_t _composite_glyphs(void * closure,
	pixman_image_t * dst,
	pixman_format_code_t dst_format,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
...
static cairo_int_status_t _cairo_image_surface_glyphs(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_glyph_t * glyphs,
	int num_glyphs,
...
static cairo_bool_t _cairo_image_surface_get_extents(void * abstract_surface,
...
static void _cairo_image_surface_get_font_options(void * abstract_surface,
...
static cairo_int_status_t _cairo_image_surface_composite(cairo_operator_t op,
	const cairo_pattern_t * src_pattern,
	...
	cairo_region_t * clip_region)
{
	cairo_image_surface_t * dst = (cairo_image_surface_t *)abstract_dst;
	cairo_composite_rectangles_t extents;
	pixman_image_t * src;
...
static cairo_int_status_t _cairo_image_surface_fill_rectangles(void * abstract_surface,
	cairo_operator_t op, const cairo_color_t * color, cairo_rectangle_int_t * rects, int num_rects)
{
	cairo_image_surface_t * surface = (cairo_image_surface_t *)abstract_surface;
	...
	if(num_rects > ARRAY_LENGTH(stack_boxes)) {
		pixman_boxes = (pixman_box32_t *)_cairo_malloc_ab(num_rects, sizeof(pixman_box32_t));
	...
	status = CAIRO_STATUS_SUCCESS;
	if(!pixman_image_fill_boxes(_pixman_operator(op), surface->pixman_image, &pixman_color, num_rects, pixman_boxes)) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
	}
...
static cairo_int_status_t _cairo_image_surface_composite_trapezoids(cairo_operator_t op,
	const cairo_pattern_t * pattern,
	void * abstract_dst,
	cairo_antialias_t antialias,
	int src_x,
...
static cairo_status_t _cairo_image_surface_span_renderer_render_rows(
	void * abstract_renderer, int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	legacy_image_surface_span_renderer_t * renderer = (legacy_image_surface_span_renderer_t *)abstract_renderer;
	while(height--)
		_cairo_image_surface_span_render_row(y++, spans, num_spans, renderer->mask_data, renderer->mask_stride);
	return CAIRO_STATUS_SUCCESS;
}

static void _cairo_image_surface_span_renderer_destroy(void * abstract_renderer)
{
	legacy_image_surface_span_renderer_t * renderer = (legacy_image_surface_span_renderer_t *)abstract_renderer;
	if(renderer == NULL)
		return;
...
static cairo_status_t _cairo_image_surface_span_renderer_finish(void * abstract_renderer)
{
	legacy_image_surface_span_renderer_t * renderer = (legacy_image_surface_span_renderer_t *)abstract_renderer;
...
static cairo_bool_t _cairo_image_surface_check_span_renderer(cairo_operator_t op,
	const cairo_pattern_t * pattern, void * abstract_dst, cairo_antialias_t antialias)
{
	return TRUE;
	(void)op;
	(void)pattern;
...
static cairo_span_renderer_t * _cairo_image_surface_create_span_renderer(cairo_operator_t op,
	const cairo_pattern_t * pattern,
	void * abstract_dst,
	cairo_antialias_t antialias,
	const cairo_composite_rectangles_t * rects,
...
{
	cairo_image_surface_t * dst = (cairo_image_surface_t *)abstract_dst;
	legacy_image_surface_span_renderer_t * renderer = (legacy_image_surface_span_renderer_t *)calloc(1, sizeof(*renderer));
	...
	/* TODO: support rendering to A1 surfaces (or: go add span
	 * compositing to pixman.) */
	renderer->mask = pixman_image_create_bits(PIXMAN_a8, rects->bounded.width, rects->bounded.height, NULL, 0);
 ...
 **/
cairo_bool_t _cairo_surface_is_image(const cairo_surface_t * surface)
{
...
cairo_image_transparency_t _cairo_image_analyze_transparency(cairo_image_surface_t * image)
{
	...
	if(image->transparency != CAIRO_IMAGE_UNKNOWN)
		return (cairo_image_transparency_t)image->transparency;
	if((image->base.content&CAIRO_CONTENT_ALPHA) == 0)
		return (cairo_image_transparency_t)(image->transparency = CAIRO_IMAGE_IS_OPAQUE);
	if((image->base.content&CAIRO_CONTENT_COLOR) == 0) {
		if(image->format == CAIRO_FORMAT_A1)
			return (cairo_image_transparency_t)(image->transparency = CAIRO_IMAGE_HAS_BILEVEL_ALPHA);
		else
