...
cairo_image_surface_t * _cairo_image_surface_map_to_image(void * abstract_other, const CairoIRect * extents)
{
	cairo_image_surface_t * other = (cairo_image_surface_t *)abstract_other;
	cairo_surface_t * surface;
	...
	surface = _cairo_image_surface_create_with_pixman_format(data,
	    other->pixman_format, extents->width, extents->height, other->stride);
...
cairo_surface_t * _cairo_image_surface_source(void * abstract_surface, CairoIRect * extents)
{
	cairo_image_surface_t * surface = (cairo_image_surface_t *)abstract_surface;
	if(extents) {
...
cairo_bool_t _cairo_image_surface_get_extents(void * abstract_surface, CairoIRect   * rectangle)
{
	cairo_image_surface_t * surface = (cairo_image_surface_t *)abstract_surface;
	rectangle->x = 0;
	rectangle->y = 0;
...
cairo_int_status_t _cairo_image_surface_paint(void * abstract_surface, cairo_operator_t op,
    ...
    const cairo_clip_t         * clip)
{
	...
	return _cairo_compositor_stroke(surface->compositor, &surface->base,
	    op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
}
    ...
    const cairo_clip_t           * clip)
{
	...
	return _cairo_compositor_fill(surface->compositor, &surface->base,
	    op, source, path, fill_rule, tolerance, antialias, clip);
}

cairo_int_status_t _cairo_image_surface_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t      * source,
...
cairo_image_surface_t * _cairo_image_surface_create_from_image(cairo_image_surface_t * other,
    pixman_format_code_t format,
    int x, int y,
    int width, int height, int stride)
{
	...
	image = (pixman_image_t *)pixman_image_create_bits(format, width, height, (uint32_t *)mem, stride);
...
cairo_image_transparency_t _cairo_image_analyze_transparency(cairo_image_surface_t * image)
{
	...
	else if(image->format == CAIRO_FORMAT_RGB16_565) {
		image->transparency = CAIRO_IMAGE_IS_OPAQUE;
		return CAIRO_IMAGE_IS_OPAQUE;
	}
	else if(image->format != CAIRO_FORMAT_ARGB32)
		return (cairo_image_transparency_t)(image->transparency = CAIRO_IMAGE_HAS_ALPHA);
	else {
		image->transparency = CAIRO_IMAGE_IS_OPAQUE;
		for(y = 0; y < image->height; y++) {
...
cairo_image_color_t _cairo_image_analyze_color(cairo_image_surface_t      * image)
{
	...
	if(image->color != CAIRO_IMAGE_UNKNOWN_COLOR)
		return (cairo_image_color_t)image->color;
	else if(image->format == CAIRO_FORMAT_A1)
		return (cairo_image_color_t)(image->color = CAIRO_IMAGE_IS_MONOCHROME);
	else if(image->format == CAIRO_FORMAT_A8)
		return (cairo_image_color_t)(image->color = CAIRO_IMAGE_IS_GRAYSCALE);
	else if(image->format == CAIRO_FORMAT_ARGB32) {
