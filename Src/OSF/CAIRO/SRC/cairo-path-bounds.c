...
static void _cairo_path_bounder_add_point(cairo_path_bounder_t * bounder, const cairo_point_t * point)
{
	...
	const cairo_point_t * b, const cairo_point_t * c, const cairo_point_t * d)
{
	...
	if(!bounder->has_point ||
	   ...
	   d->y < bounder->extents.p1.y || d->y > bounder->extents.p2.y) {
		return _cairo_spline_bound(_cairo_path_bounder_line_to, bounder, &bounder->current_point, b, c, d);
	}
	else {
...
void _cairo_path_fixed_fill_extents(const cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule, double tolerance, cairo_rectangle_int_t * extents)
{
	cairo_path_bounder_t bounder;
	cairo_status_t status;
	if(!path->has_curve_to) {
	...
	cairo_rectangle_int_t * extents)
{
	...
	status = _cairo_path_fixed_stroke_to_traps(path, stroke_style, ctm, ctm_inverse, tolerance, &traps);
...
cairo_bool_t _cairo_path_fixed_extents(const cairo_path_fixed_t * path, cairo_box_t * box)
{
	...
	_cairo_path_bounder_init(&bounder);
	status = _cairo_path_fixed_interpret(path, CAIRO_DIRECTION_FORWARD,
		_cairo_path_bounder_move_to, _cairo_path_bounder_line_to,
