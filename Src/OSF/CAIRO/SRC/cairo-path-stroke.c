...
typedef struct cairo_stroker {
	...
	cairo_status_t (* add_convex_quad)(void * closure, const cairo_point_t quad[4]);
	cairo_pen_t pen;
...
static void _cairo_stroker_limit(cairo_stroker_t * stroker, const cairo_path_fixed_t * path, const cairo_box_t * boxes, int num_boxes)
{
	double dx, dy;
	 ...
	 */
	_cairo_stroke_style_max_distance_from_path(&stroker->style, path, stroker->ctm, &dx, &dy);
...
static cairo_status_t _cairo_stroker_init(cairo_stroker_t * stroker, const cairo_path_fixed_t   * path,
    const cairo_stroke_style_t * stroke_style, const cairo_matrix_t * ctm,
    const cairo_matrix_t * ctm_inverse, double tolerance, const cairo_box_t * limits, int num_limits)
{
...
static void FASTCALL _cairo_stroker_fini(cairo_stroker_t * stroker)
{
	_cairo_pen_fini(&stroker->pen);
}

/*static void FASTCALL _translate_point(cairo_point_t * point, const cairo_point_t * offset)
{
	point->x += offset->x;
	point->y += offset->y;
}*/

static int FASTCALL _cairo_stroker_join_is_clockwise(const cairo_stroke_face_t * in, const cairo_stroke_face_t * out)
{
...
/*static inline int _range_step(int i, int step, int maxVal)
{
	i += step;
	if(i < 0)
		i = maxVal - 1;
 ...
 * inpt and outpt.
 */
static cairo_status_t _tessellate_fan(cairo_stroker_t * stroker, const cairo_slope_t * in_vector,
    const cairo_slope_t * out_vector, const cairo_point_t * midpt, const cairo_point_t * inpt,
    const cairo_point_t * outpt, cairo_bool_t clockwise)
{
	...
	if(clockwise) {
		...
		if(stroker->add_external_edge) {
			...
			while(start != stop) {
				...
				CairoTranslatePoint(&points[num_points], &pen->vertices[start].point);
				num_points++;
				if(start-- == 0)
					start += pen->num_vertices;
			}
...
static cairo_status_t _cairo_stroker_join(cairo_stroker_t * stroker, const cairo_stroke_face_t * in, const cairo_stroke_face_t * out)
{
	int clockwise = _cairo_stroker_join_is_clockwise(out, in);
	const cairo_point_t * inpt, * outpt;
	cairo_point_t points[4];
	cairo_status_t status;
	if(in->cw.x == out->cw.x  && in->cw.y == out->cw.y && in->ccw.x == out->ccw.x && in->ccw.y == out->ccw.y) {
		return CAIRO_STATUS_SUCCESS;
	}
	if(clockwise) {
	...
	switch(stroker->style.line_join) {
		...
		default: {
		    ...
		    double in_dot_out = -in->usr_vector.x * out->usr_vector.x + -in->usr_vector.y * out->usr_vector.y;
			    ...
			    if(CairoSlopeCompareSgn(fdx1, fdy1, mdx, mdy) != CairoSlopeCompareSgn(fdx2, fdy2, mdx, mdy)) {
				    if(stroker->add_external_edge) {
					    ...
					    if(clockwise) {
						    ...
						    status = stroker->add_external_edge(stroker->closure, outpt, &points[0]);
						    if(unlikely(status))
							    return status;
						    status = stroker->add_external_edge(stroker->closure, &points[0], inpt);
						    if(unlikely(status))
							    return status;
					    }
					    return CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _cairo_stroker_add_cap(cairo_stroker_t * stroker, const cairo_stroke_face_t * f)
{
	switch(stroker->style.line_cap) {
		...
		default:
		    if(stroker->add_external_edge) {
			    return stroker->add_external_edge(stroker->closure, &f->ccw, &f->cw);
		    }
		    else {
			    return CAIRO_STATUS_SUCCESS;
		    }
...
static cairo_status_t _cairo_stroker_add_leading_cap(cairo_stroker_t * stroker, const cairo_stroke_face_t * face)
{
	cairo_point_t t;
...
static cairo_status_t _cairo_stroker_add_trailing_cap(cairo_stroker_t * stroker, const cairo_stroke_face_t * face)
{
	return _cairo_stroker_add_cap(stroker, face);
}

static /*inline*/ cairo_bool_t FASTCALL _compute_normalized_device_slope(double * dx, double * dy, const cairo_matrix_t * ctm_inverse, double * mag_out)
{
	double dx0 = *dx, dy0 = *dy;
	double mag;
	cairo_matrix_transform_distance(ctm_inverse, &dx0, &dy0);
	if(dx0 == 0.0 && dy0 == 0.0) {
		ASSIGN_PTR(mag_out, 0.0);
		return FALSE;
	}
	if(dx0 == 0.0) {
		*dx = 0.0;
	...
	ASSIGN_PTR(mag_out, mag);
	return TRUE;
}

static void FASTCALL _compute_face(const cairo_point_t * point, const cairo_slope_t * dev_slope,
    double slope_dx, double slope_dy, cairo_stroker_t * stroker, cairo_stroke_face_t * face)
{
...
static cairo_status_t FASTCALL _cairo_stroker_add_caps(cairo_stroker_t * stroker)
{
	cairo_status_t status;
	...
	if(stroker->has_initial_sub_path && !stroker->has_first_face && !stroker->has_current_face && stroker->style.line_cap == CAIRO_LINE_CAP_ROUND) {
		...
		_compute_normalized_device_slope(&dx, &dy, stroker->ctm_inverse, 0);
...
static cairo_status_t _cairo_stroker_add_sub_edge(cairo_stroker_t * stroker, const cairo_point_t * p1,
    const cairo_point_t * p2, cairo_slope_t * dev_slope, double slope_dx, double slope_dy,
    cairo_stroke_face_t * start, cairo_stroke_face_t * end)
{
	...
	*end = *start;
	...
	if(stroker->add_external_edge) {
		...
		if(unlikely(status))
			return status;
		status = stroker->add_external_edge(stroker->closure, &start->ccw, &end->ccw);
		if(unlikely(status))
			return status;
		return CAIRO_STATUS_SUCCESS;
	}
...
static cairo_status_t _cairo_stroker_line_to(void * closure, const cairo_point_t * point)
{
	...
	_compute_normalized_device_slope(&slope_dx, &slope_dy, stroker->ctm_inverse, 0);
 ...
 */
static cairo_status_t _cairo_stroker_line_to_dashed(void * closure,
    const cairo_point_t * p2)
{
	...
	if(stroker->has_bounds && (!_cairo_box_contains_point(&stroker->bounds, p1) ||
	...
	while(remain) {
		...
		if(stroker->dash.dash_on && (fully_in_bounds || (!stroker->has_first_face && stroker->dash.dash_starts_on) ||
			...
			status = _cairo_stroker_add_sub_edge(stroker, &segment.p1, &segment.p2, &dev_slope,
			    slope_dx, slope_dy, &sub_start, &sub_end);
			if(unlikely(status))
				return status;
			if(stroker->has_current_face) {
				/* Join with final face from previous segment */
	...
	if(stroker->dash.dash_on && !stroker->has_current_face) {
		 ...
		 */
		_compute_face(p2, &dev_slope, slope_dx, slope_dy, stroker, &stroker->current_face);
...
static cairo_status_t _cairo_stroker_curve_to(void * closure,
    ...
    const cairo_point_t * d)
{
	...
	line_to = stroker->dash.dashed ? (cairo_spline_add_point_func_t)_cairo_stroker_line_to_dashed : (cairo_spline_add_point_func_t)_cairo_stroker_line_to;
	/* spline_to is only capable of rendering non-degenerate splines. */
	spline_to = stroker->dash.dashed ? (cairo_spline_add_point_func_t)_cairo_stroker_line_to_dashed : (cairo_spline_add_point_func_t)_cairo_stroker_spline_to;
	if(!_cairo_spline_init(&spline, spline_to, stroker, &stroker->current_point, b, c, d)) {
	...
	if(!stroker->dash.dashed || stroker->dash.dash_on) {
		...
		if(_compute_normalized_device_slope(&slope_dx, &slope_dy, stroker->ctm_inverse, NULL)) {
			_compute_face(&stroker->current_point, &spline.initial_slope, slope_dx, slope_dy, stroker, &face);
		}
		if(stroker->has_current_face) {
...
cairo_status_t _cairo_path_fixed_stroke_to_shaper(cairo_path_fixed_t  * path, const cairo_stroke_style_t * stroke_style,
    const cairo_matrix_t * ctm, const cairo_matrix_t * ctm_inverse, double tolerance,
    cairo_status_t (* add_triangle)(void * closure, const cairo_point_t triangle[3]),
    cairo_status_t (* add_triangle_fan)(void * closure, const cairo_point_t * midpt, const cairo_point_t * points, int npoints),
    cairo_status_t (* add_convex_quad)(void * closure, const cairo_point_t quad[4]), void * closure)
{
	...
	status = _cairo_path_fixed_interpret(path, _cairo_stroker_move_to, 
		stroker.dash.dashed ? _cairo_stroker_line_to_dashed : _cairo_stroker_line_to,
