...
static void _cairo_stroker_dash_start(cairo_stroker_dash_t * dash)
{
	double offset;
...
static void _cairo_stroker_dash_step(cairo_stroker_dash_t * dash, double step)
{
...
static void _cairo_stroker_dash_init(cairo_stroker_dash_t * dash, const cairo_stroke_style_t * style)
{
...
static cairo_status_t _cairo_stroker_init(cairo_stroker_t * stroker, const cairo_stroke_style_t * stroke_style, 
	const cairo_matrix_t * ctm, const cairo_matrix_t * ctm_inverse, double tolerance)
{
	...
	status = _cairo_pen_init(&stroker->pen, stroke_style->line_width/2.0, tolerance, ctm);
	if(unlikely(status))
		return status;
...
static void _cairo_stroker_limit(cairo_stroker_t * stroker, const cairo_box_t * boxes, int num_boxes)
{
	double dx, dy;
...
static void _cairo_stroker_fini(cairo_stroker_t * stroker)
{
	_cairo_pen_fini(&stroker->pen);
}

static void _translate_point(cairo_point_t * point, const cairo_point_t * offset)
{
	point->x += offset->x;
	point->y += offset->y;
}
...
static inline int _range_step(int i, int step, int max)
{
	i += step;
	if(i < 0)
		i = max-1;
	...
	cairo_bool_t clockwise)
{
	...
	if(npoints > ARRAY_LENGTH(stack_points)) {
		points = (cairo_point_t *)_cairo_malloc_ab(npoints, sizeof(cairo_point_t));
...
static cairo_status_t _cairo_stroker_add_cap(cairo_stroker_t * stroker,
...
static cairo_status_t _cairo_stroker_add_leading_cap(cairo_stroker_t * stroker,
...
static cairo_status_t _cairo_stroker_add_trailing_cap(cairo_stroker_t * stroker,
	const cairo_stroke_face_t * face)
{
...
static void _compute_face(const cairo_point_t * point, cairo_slope_t * dev_slope,
...
static cairo_status_t _cairo_stroker_add_caps(cairo_stroker_t * stroker)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_stroker_add_sub_edge(cairo_stroker_t * stroker,
...
static cairo_status_t _cairo_stroker_curve_to(void * closure, const cairo_point_t * b, const cairo_point_t * c, const cairo_point_t * d)
{
	cairo_stroker_t * stroker = (cairo_stroker_t *)closure;
	...
	if(!_cairo_spline_init(&spline, line_to, stroker, &stroker->current_point, b, c, d)) {
	...
	void * closure)
{
	...
	cairo_status_t status = _cairo_stroker_init(&stroker, stroke_style, ctm, ctm_inverse, tolerance);
	if(unlikely(status))
		return status;
	...
	status = _cairo_path_fixed_interpret(path, CAIRO_DIRECTION_FORWARD, _cairo_stroker_move_to,
...
cairo_status_t _cairo_path_fixed_stroke_to_traps(const cairo_path_fixed_t * path,
	const cairo_stroke_style_t * stroke_style,
	const cairo_matrix_t * ctm,
	const cairo_matrix_t * ctm_inverse,
	double tolerance,
	cairo_traps_t * traps)
{
	...
	if(path->is_rectilinear) {
		status = _cairo_path_fixed_stroke_rectilinear_to_traps(path, stroke_style, ctm, traps);
...
static void _cairo_rectilinear_stroker_limit(cairo_rectilinear_stroker_t * stroker,
	const cairo_box_t * boxes, int num_boxes)
{
	stroker->has_bounds = TRUE;
	_cairo_boxes_get_extents(boxes, num_boxes, &stroker->bounds);
...
static void _cairo_rectilinear_stroker_fini(cairo_rectilinear_stroker_t * stroker)
{
...
static cairo_status_t _cairo_rectilinear_stroker_emit_segments(cairo_rectilinear_stroker_t * stroker)
{
	cairo_status_t status;
	...
	for(i = 0; i < stroker->num_segments; i++) {
		...
		if(stroker->do_traps) {
			status = _cairo_traps_tessellate_rectangle((cairo_traps_t *)stroker->container, a, b);
		}
		else {
			cairo_box_t box;
			box.p1 = *a;
			box.p2 = *b;
			status = _cairo_boxes_add((cairo_boxes_t *)stroker->container, &box);
		}
		if(unlikely(status))
			return status;
	}
...
static cairo_status_t _cairo_rectilinear_stroker_emit_segments_dashed(cairo_rectilinear_stroker_t * stroker)
{
	...
	for(i = 0; i < stroker->num_segments; i++) {
		cairo_point_t * a = &stroker->segments[i].p1;
		cairo_point_t * b = &stroker->segments[i].p2;
		cairo_bool_t is_horizontal = stroker->segments[i].is_horizontal;
		/* Handle the joins for a potentially degenerate segment. */
		if(line_cap == CAIRO_LINE_CAP_BUTT && stroker->segments[i].has_join &&
		   (i != stroker->num_segments-1 || (!stroker->open_sub_path && stroker->dash.dash_starts_on))) {
			...
			if(stroker->do_traps) {
				status = _cairo_traps_tessellate_rectangle((cairo_traps_t *)stroker->container, &p1, &p2);
			}
			else {
				cairo_box_t box;
				box.p1 = p1;
...
static cairo_status_t _cairo_rectilinear_stroker_line_to(void * closure, const cairo_point_t * b)
{
	...
	if(a->x == b->x && a->y == b->y)
		return CAIRO_STATUS_SUCCESS;
	status = _cairo_rectilinear_stroker_add_segment(stroker, a, b, a->y == b->y, TRUE);
...
static cairo_status_t _cairo_rectilinear_stroker_line_to_dashed(void * closure, const cairo_point_t * point)
{
	...
	while(remain > 0.) {
		double step_length = MIN(stroker->dash.dash_remain, remain);
		remain -= step_length;
		...
		if(stroker->dash.dash_on && (fully_in_bounds || _cairo_box_intersects_line_segment(&stroker->bounds, &segment))) {
			status = _cairo_rectilinear_stroker_add_segment(stroker, &segment.p1, &segment.p2, is_horizontal, remain <= 0.);
			if(unlikely(status))
				return status;
			dash_on = TRUE;
		}
...
cairo_int_status_t _cairo_path_fixed_stroke_rectilinear_to_traps(const cairo_path_fixed_t * path,
	const cairo_stroke_style_t * stroke_style, const cairo_matrix_t * ctm, cairo_traps_t * traps)
{
	cairo_rectilinear_stroker_t rectilinear_stroker;
	cairo_int_status_t status;
	assert(path->is_rectilinear);
	if(!_cairo_rectilinear_stroker_init(&rectilinear_stroker, stroke_style, ctm, TRUE, traps)) {
		return CAIRO_INT_STATUS_UNSUPPORTED;
	}
	if(traps->num_limits) {
		_cairo_rectilinear_stroker_limit(&rectilinear_stroker, traps->limits, traps->num_limits);
	}
	status = _cairo_path_fixed_interpret(path,
		CAIRO_DIRECTION_FORWARD,
	...
	cairo_boxes_t * boxes)
{
	...
	cairo_int_status_t status;
	assert(path->is_rectilinear);
	if(!_cairo_rectilinear_stroker_init(&rectilinear_stroker, stroke_style, ctm, FALSE, boxes)) {
		return CAIRO_INT_STATUS_UNSUPPORTED;
	}
	if(boxes->num_limits) {
		_cairo_rectilinear_stroker_limit(&rectilinear_stroker, boxes->limits, boxes->num_limits);
	}
	status = _cairo_path_fixed_interpret(path, CAIRO_DIRECTION_FORWARD, _cairo_rectilinear_stroker_move_to,
		rectilinear_stroker.dash.dashed ? _cairo_rectilinear_stroker_line_to_dashed : _cairo_rectilinear_stroker_line_to,
		NULL, _cairo_rectilinear_stroker_close_path, &rectilinear_stroker);
	if(unlikely(status))
		goto BAIL;
	if(rectilinear_stroker.dash.dashed)
		status = _cairo_rectilinear_stroker_emit_segments_dashed(&rectilinear_stroker);
	...
	if(unlikely(status))
		goto BAIL;
	/* As we incrementally tessellate, we do not eliminate self-intersections */
	status = _cairo_bentley_ottmann_tessellate_boxes(boxes, CAIRO_FILL_RULE_WINDING, boxes);
	if(unlikely(status))
		goto BAIL;
	_cairo_rectilinear_stroker_fini(&rectilinear_stroker);
	return CAIRO_STATUS_SUCCESS;
