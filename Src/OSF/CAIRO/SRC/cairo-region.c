 ...
 **/
static cairo_status_t _cairo_region_set_error(cairo_region_t * region, cairo_status_t status)
{
...
cairo_status_t cairo_region_subtract(cairo_region_t * dst, const cairo_region_t * other)
{
	...
	if(!pixman_region32_subtract(&dst->rgn, &dst->rgn, CONST_CAST&other->rgn)) {
...
{
	if(region->status == CAIRO_STATUS_SUCCESS)
...
{
	pixman_box32_t box;
	return (region->status) ? FALSE : pixman_region32_contains_point(CONST_CAST&region->rgn, x, y, &box);
}
