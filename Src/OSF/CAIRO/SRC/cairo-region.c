...
void FASTCALL _cairo_region_init(cairo_region_t * region)
{
...
void _cairo_region_init_rectangle(cairo_region_t * region, const CairoIRect * rectangle)
{
	VG(VALGRIND_MAKE_MEM_UNDEFINED(region, sizeof(cairo_region_t)));
	region->status = CAIRO_STATUS_SUCCESS;
	CAIRO_REFERENCE_COUNT_INIT(&region->ref_count, 0);
...
cairo_region_t * _cairo_region_create_from_boxes(const cairo_box_t * boxes, int count)
{
	...
	CAIRO_REFERENCE_COUNT_INIT(&region->ref_count, 1);
	region->status = CAIRO_STATUS_SUCCESS;
	if(!pixman_region32_init_rects(&region->rgn, (pixman_box32_t*)boxes, count)) {
		SAlloc::F(region);
		return _cairo_region_create_in_error(_cairo_error(CAIRO_STATUS_NO_MEMORY));
	}
	return region;
...
cairo_box_t * _cairo_region_get_boxes(const cairo_region_t * region, int * nbox)
{
	if(region->status) {
		nbox = 0;
		return NULL;
 ...
 * Since: 1.10
 **/
cairo_region_t * cairo_region_create_rectangle(const CairoIRect * rectangle)
{
	cairo_region_t * region = (cairo_region_t *)_cairo_malloc(sizeof(cairo_region_t));
	if(unlikely(region == NULL))
		return (cairo_region_t*)&_cairo_region_nil;
...
{
	if(region && !CAIRO_REFERENCE_COUNT_IS_INVALID(&region->ref_count)) {
		assert(CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&region->ref_count));
...
{
	return region->status ? 0 : pixman_region32_n_rects(CONST_CAST &region->rgn);
}
 ...
 * @rectangle: return location for a #CairoIRect
 *
 * Stores the @nth rectangle from the region in @rectangle.
 *
 * Since: 1.10
 **/
void cairo_region_get_rectangle(const cairo_region_t * region, int nth, CairoIRect * rectangle)
{
	if(region->status) {
		...
		pixman_box32_t * pbox = pixman_region32_rectangles(CONST_CAST &region->rgn, NULL) + nth;
		rectangle->x = pbox->x1;
		rectangle->y = pbox->y1;
 ...
 *
 * Gets the bounding rectangle of @region as a #CairoIRect
 *
 * Since: 1.10
 **/
...
cairo_status_t cairo_region_subtract(cairo_region_t * dst, const cairo_region_t * other)
{
	...
	else if(!pixman_region32_subtract(&dst->rgn, &dst->rgn, CONST_CAST &other->rgn))
 ...
 * Since: 1.10
 **/
cairo_status_t cairo_region_subtract_rectangle(cairo_region_t * dst, const CairoIRect * rectangle)
{
...
cairo_status_t cairo_region_intersect(cairo_region_t * dst, const cairo_region_t * other)
{
	if(dst->status)
		...
		return _cairo_region_set_error(dst, other->status);
	else if(!pixman_region32_intersect(&dst->rgn, &dst->rgn, CONST_CAST &other->rgn))
		return _cairo_region_set_error(dst, CAIRO_STATUS_NO_MEMORY);
	else 
		return CAIRO_STATUS_SUCCESS;
}
 ...
 * Since: 1.10
 **/
cairo_status_t cairo_region_intersect_rectangle(cairo_region_t * dst, const CairoIRect * rectangle)
{
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	if(dst->status)
		status = dst->status;
 ...
 * Since: 1.10
 **/
cairo_status_t cairo_region_union_rectangle(cairo_region_t * dst, const CairoIRect * rectangle)
{
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	if(dst->status)
		status = dst->status;
 ...
 * Since: 1.10
 **/
cairo_status_t cairo_region_xor_rectangle(cairo_region_t * dst, const CairoIRect * rectangle)
{
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
