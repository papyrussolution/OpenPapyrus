...
static ulong _cairo_gl_shader_cache_hash(const cairo_shader_cache_entry_t * entry)
{
...
static cairo_gl_var_type_t cairo_gl_operand_get_var_type(cairo_gl_operand_t * operand)
{
	switch(operand->type) {
		default:
...
static void cairo_gl_shader_dcl_coverage(cairo_output_stream_t * stream)
{
	_cairo_output_stream_printf(stream, "varying float coverage;\n");
}

static void cairo_gl_shader_def_coverage(cairo_output_stream_t * stream)
{
...
static void compile_shader(cairo_gl_context_t * ctx,
    GLuint * shader,
    GLenum type,
    const char * source)
{
...
static void link_shader_program(cairo_gl_context_t * ctx,
    GLuint * program,
    GLuint vert,
    GLuint frag)
{
...
static GLint _cairo_gl_get_op_uniform_location(cairo_gl_context_t * ctx,
    cairo_gl_shader_t  * shader,
 ...
 * compile time.
 */
static void _cairo_gl_shader_set_samplers(cairo_gl_context_t * ctx,
    cairo_gl_shader_t * shader)
{
	cairo_gl_dispatch_t * dispatch = &ctx->dispatch;
	GLint location;
...
void _cairo_gl_shader_bind_vec4(cairo_gl_context_t * ctx, GLint location, float value0, float value1, float value2, float value3)
{
	cairo_gl_dispatch_t * dispatch = &ctx->dispatch;
	assert(location != -1);
...
void _cairo_gl_shader_bind_matrix(cairo_gl_context_t * ctx, GLint location, const cairo_matrix_t* m)
{
	cairo_gl_dispatch_t * dispatch = &ctx->dispatch;
	float gl_m[9] = { 
		m->xx, m->yx, 0, // @v1.14.6 @fix
		m->xy, m->yy, 0, // @v1.14.6 @fix
		m->x0, m->y0, 1  // @v1.14.6 @fix
	};
	assert(location != -1);
	dispatch->UniformMatrix3fv(location, 1, GL_FALSE, gl_m); // @v1.14.6 @fix GL_TRUE->GL_FALSE
}

void _cairo_gl_shader_bind_matrix4f(cairo_gl_context_t * ctx, GLint location, GLfloat* gl_m)
{
	cairo_gl_dispatch_t * dispatch = &ctx->dispatch;
	assert(location != -1);
