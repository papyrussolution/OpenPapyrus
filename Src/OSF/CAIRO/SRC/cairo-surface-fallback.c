 ...
 **/
static cairo_int_status_t _fallback_init(fallback_state_t * state,
	cairo_surface_t * dst, int x, int y, int width, int height)
{
...
static void _fallback_fini(fallback_state_t * state)
{
	_cairo_surface_release_dest_image(state->dst, &state->extents, state->image, &state->image_rect, state->image_extra);
}

typedef cairo_status_t (*cairo_draw_func_t)(void * closure,
	cairo_operator_t op, const cairo_pattern_t * src, cairo_surface_t * dst,
	int dst_x, int dst_y, const cairo_rectangle_int_t * extents, cairo_region_t * clip_region);

static cairo_status_t _create_composite_mask_pattern(cairo_surface_pattern_t * mask_pattern,
	cairo_clip_t * clip,
	...
	const cairo_rectangle_int_t * extents)
{
	...
	if(clip != NULL) {
		...
		if(unlikely(_cairo_status_is_error(status) || status == CAIRO_INT_STATUS_NOTHING_TO_DO)) {
			return status;
		}
	 ...
	 */
	mask = _cairo_surface_create_similar_solid(dst, CAIRO_CONTENT_ALPHA, extents->width, extents->height, CAIRO_COLOR_TRANSPARENT, TRUE);
	...
	if(clip_region && (extents->x || extents->y)) {
		...
		cairo_region_translate(fallback_region, -extents->x, -extents->y);
		clip_region = fallback_region;
	}
	status = draw_func(draw_closure, CAIRO_OPERATOR_ADD, &_cairo_pattern_white.base, mask,
		extents->x, extents->y, extents, clip_region);
	if(unlikely(status))
		goto CLEANUP_SURFACE;
	if(clip_surface)
	...
	const cairo_rectangle_int_t * extents)
{
	...
	cairo_status_t status = _create_composite_mask_pattern(&mask_pattern, clip, draw_func, draw_closure, dst, extents);
	...
	const cairo_rectangle_int_t * extents)
{
	 ...
	 */
	cairo_surface_t * intermediate = _cairo_surface_create_similar_scratch(dst, dst->content, extents->width, extents->height);
	if(intermediate == NULL)
		intermediate = _cairo_image_surface_create_with_content(dst->content, extents->width, extents->width);
	...
	if(unlikely(status))
		goto CLEANUP_SURFACE;
	status = (*draw_func)(draw_closure, op, src, intermediate, extents->x, extents->y, extents, NULL);
	if(unlikely(status))
		goto CLEANUP_SURFACE;
	...
	status = _cairo_surface_composite(CAIRO_OPERATOR_DEST_IN, &clip_pattern.base, NULL, intermediate,
		extents->x-clip_x, extents->y-clip_y, 0, 0, 0, 0, extents->width, extents->height, NULL);
	if(unlikely(status))
		goto CLEANUP_CLIP;
	/* Punch the clip out of the destination */
	status = _cairo_surface_composite(CAIRO_OPERATOR_DEST_OUT,
		&clip_pattern.base, NULL, dst, extents->x-clip_x, extents->y-clip_y,
		0, 0, extents->x, extents->y, extents->width, extents->height, NULL);
	if(unlikely(status))
		goto CLEANUP_CLIP;
...
static int _cairo_rectangle_empty(const cairo_rectangle_int_t * rect)
{
 ...
 **/
static cairo_status_t _clip_and_composite(cairo_clip_t * clip,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_draw_func_t draw_func,
	void * draw_closure,
	...
	if(op == CAIRO_OPERATOR_SOURCE) {
		...
		if(clip_surface) {
			if(_cairo_operator_bounded_by_mask(op)) {
				...
				status = _clip_and_composite_combine(clip, op, src, draw_func, draw_closure, dst, extents);
			}
		}
		else {
			status = draw_func(draw_closure, op, src, dst, 0, 0, extents, clip_region);
		}
	}
	return status;
}
 ...
 */
static cairo_status_t _composite_trap_region(cairo_clip_t * clip,
	const cairo_pattern_t * src,
	cairo_operator_t op,
	cairo_surface_t * dst,
...
static cairo_status_t _composite_traps_draw_func(void * closure,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_surface_t * dst,
	int dst_x,
...
{
	cairo_composite_traps_info_t * info = (cairo_composite_traps_info_t *)closure;
	cairo_status_t status;
...
static cairo_status_t _clip_and_composite_region(const cairo_pattern_t * src,
	cairo_operator_t op,
	cairo_surface_t * dst,
	cairo_region_t * trap_region,
	cairo_clip_t * clip,
	cairo_rectangle_int_t * extents)
{
	...
	if((src->type == CAIRO_PATTERN_TYPE_SOLID || op == CAIRO_OPERATOR_CLEAR) &&
	   clip == NULL) {
		 ...
		 */
		status = _composite_trap_region(clip, src, op, dst, trap_region, extents);
	}
...
static cairo_status_t _fill_rectangles(cairo_surface_t * dst,
	cairo_operator_t op, const cairo_pattern_t * src, cairo_traps_t * traps, cairo_clip_t * clip)
{
	const cairo_color_t * color;
...
static cairo_status_t _composite_rectangle(cairo_surface_t * dst,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_traps_t * traps,
	cairo_clip_t * clip)
...
static cairo_status_t _clip_and_composite_trapezoids(const cairo_pattern_t * src,
	cairo_operator_t op,
	cairo_surface_t * dst,
	cairo_traps_t * traps,
	cairo_antialias_t antialias,
	...
	cairo_rectangle_int_t * extents)
{
	 ...
	 */
	if(!clip_surface || (_cairo_operator_bounded_by_mask(op) && op != CAIRO_OPERATOR_SOURCE)) {
		...
		if(trap_region != NULL) {
			...
			status = _clip_and_composite_region(src, op, dst, trap_region, clip_surface ? clip : NULL, extents);
			cairo_region_destroy(trap_region);
	...
	return _clip_and_composite(clip, op, src, _composite_traps_draw_func, &traps_info, dst, extents);
}

cairo_status_t _cairo_surface_fallback_paint(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	...
	if(!_cairo_surface_get_extents(surface, &rect))
		ASSERT_NOT_REACHED;
	status = _cairo_composite_rectangles_init_for_paint(&extents, rect.width, rect.height, op, source, clip);
	if(unlikely(status))
		return status;
	 ...
	 * operation to a fill...
	 */
	if(clip != NULL && clip_path->prev == NULL && _cairo_operator_bounded_by_mask(op)) {
		return _cairo_surface_fill(surface, op, source, &clip_path->path, clip_path->fill_rule, clip_path->tolerance, clip_path->antialias, NULL);
	}
	...
	if(unlikely(status))
		goto CLEANUP_BOXES;
	status = _clip_and_composite_trapezoids(source, op, surface, &traps, CAIRO_ANTIALIAS_DEFAULT, clip,
...
static cairo_status_t _cairo_surface_mask_draw_func(void * closure,
	cairo_operator_t op, const cairo_pattern_t * src, cairo_surface_t * dst,
	int dst_x, int dst_y, const cairo_rectangle_int_t * extents, cairo_region_t * clip_region)
{
	cairo_pattern_t * mask = (cairo_pattern_t *)closure;
	cairo_status_t status;
	cairo_region_t * extents_region = NULL;
	if(clip_region == NULL && !_cairo_operator_bounded_by_source(op)) {
		extents_region = cairo_region_create_rectangle(extents);
		if(unlikely(extents_region->status))
			return extents_region->status;
		cairo_region_translate(extents_region, -dst_x, -dst_y);
...
cairo_status_t _cairo_surface_fallback_mask(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, cairo_clip_t * clip)
{
	...
	if(!_cairo_surface_get_extents(surface, &rect))
		ASSERT_NOT_REACHED;
	status = _cairo_composite_rectangles_init_for_mask(&extents, rect.width, rect.height, op, source, mask, clip);
	if(unlikely(status))
		return status;
	if(_cairo_clip_contains_extents(clip, &extents))
		clip = NULL;
	...
	return _clip_and_composite(clip, op, source, _cairo_surface_mask_draw_func, (void *)mask,
		surface, extents.is_bounded ? &extents.bounded : &extents.unbounded);
}

cairo_status_t _cairo_surface_fallback_stroke(cairo_surface_t * surface,
	cairo_operator_t op,
	...
	cairo_clip_t * clip)
{
	...
	status = _cairo_composite_rectangles_init_for_stroke(&extents,
		rect.width, rect.height, op, source, path, stroke_style, ctm, clip);
	if(unlikely(status))
		return status;
	if(_cairo_clip_contains_extents(clip, &extents))
		clip = NULL;
...
DO_TRAPS:
	status = _clip_and_composite_trapezoids(source, op, surface,
		&traps, antialias, clip, extents.is_bounded ? &extents.bounded : &extents.unbounded);
CLEANUP:
	_cairo_traps_fini(&traps);
...
cairo_status_t _cairo_surface_fallback_fill(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	...
	cairo_clip_t * clip)
{
	...
	status = _cairo_composite_rectangles_init_for_fill(&extents,
		rect.width, rect.height, op, source, path, clip);
	if(unlikely(status))
		return status;
	if(_cairo_clip_contains_extents(clip, &extents))
		clip = NULL;
...
static cairo_status_t _cairo_surface_old_show_glyphs_draw_func(void * closure,
	cairo_operator_t op,
	const cairo_pattern_t * src,
	cairo_surface_t * dst,
	int dst_x,
	...
	cairo_region_t * clip_region)
{
	cairo_show_glyphs_info_t * glyph_info = (cairo_show_glyphs_info_t *)closure;
	cairo_status_t status;
	cairo_region_t * extents_region = NULL;
	if(clip_region == NULL &&
	   !_cairo_operator_bounded_by_source(op)) {
...
cairo_status_t _cairo_surface_fallback_show_glyphs(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_glyph_t * glyphs,
	int num_glyphs,
...
cairo_surface_t * _cairo_surface_fallback_snapshot(cairo_surface_t * surface)
{
...
cairo_status_t _cairo_surface_fallback_composite(cairo_operator_t op,
	const cairo_pattern_t * src,
	...
	cairo_region_t * clip_region)
{
	...
	cairo_status_t status = _fallback_init(&state, dst, dst_x, dst_y, width, height);
	if(unlikely(status))
		return status;
	...
	if(clip_region != NULL && (state.image_rect.x || state.image_rect.y)) {
		...
		if(unlikely(status))
			goto FAIL;
		cairo_region_translate(fallback_region, -state.image_rect.x, -state.image_rect.y);
		clip_region = fallback_region;
	}
...
cairo_status_t _cairo_surface_fallback_fill_rectangles(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_color_t * color, cairo_rectangle_int_t * rects, int num_rects)
{
	...
	if(state.image_rect.x != 0 || state.image_rect.y != 0) {
		offset_rects = (cairo_rectangle_int_t *)_cairo_malloc_ab(num_rects, sizeof(cairo_rectangle_int_t));
	...
	status = _cairo_surface_fill_rectangles(&state.image->base, op, color, rects, num_rects);
	free(offset_rects);
...
cairo_status_t _cairo_surface_fallback_composite_trapezoids(cairo_operator_t op,
	...
	cairo_region_t * clip_region)
{
	...
	if(state.image_rect.x != 0 || state.image_rect.y != 0) {
		offset_traps = (cairo_trapezoid_t *)_cairo_malloc_ab(num_traps, sizeof(cairo_trapezoid_t));
		if(offset_traps == NULL) {
			status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
			goto FAIL;
		}
		_cairo_trapezoid_array_translate_and_scale(offset_traps, traps, num_traps,
			-state.image_rect.x, -state.image_rect.y, 1.0, 1.0);
		traps = offset_traps;
	...
	status = _cairo_surface_composite_trapezoids(op, pattern, &state.image->base,
		antialias, src_x, src_y, dst_x-state.image_rect.x, dst_y-state.image_rect.y,
		width, height, traps, num_traps, clip_region);
FAIL:
	if(offset_traps != NULL)
		free(offset_traps);
	if(fallback_region != NULL)
...
cairo_status_t _cairo_surface_fallback_clone_similar(cairo_surface_t * surface,
	cairo_surface_t * src,
	int src_x,
	int src_y,
	int width,
	...
	cairo_surface_t ** clone_out)
{
	cairo_surface_pattern_t pattern;
	cairo_status_t status;
	cairo_surface_t * new_surface = _cairo_surface_create_similar_scratch(surface, src->content, width, height);
	if(new_surface == NULL)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	...
	status = _cairo_surface_paint(new_surface, CAIRO_OPERATOR_SOURCE, &pattern.base, NULL);
	_cairo_pattern_fini(&pattern.base);
	if(unlikely(status)) {
