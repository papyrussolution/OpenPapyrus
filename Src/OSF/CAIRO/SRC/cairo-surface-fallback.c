    ...
    const cairo_clip_t      * clip)
{
	return _cairo_compositor_stroke(&_cairo_fallback_compositor,
	    (cairo_surface_t *)surface, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
}

cairo_int_status_t _cairo_surface_fallback_fill(void * surface,
    cairo_operator_t op,
    const cairo_pattern_t      * source,
    const cairo_path_fixed_t   * path,
    ...
    const cairo_clip_t         * clip)
{
	return _cairo_compositor_fill(&_cairo_fallback_compositor,
	    (cairo_surface_t *)surface, op, source, path,
	    fill_rule, tolerance, antialias, clip);
}

cairo_int_status_t _cairo_surface_fallback_glyphs(void * surface,
    cairo_operator_t op,
    const cairo_pattern_t   * source,
