...
{
	cairo_image_surface_t * surface = (cairo_image_surface_t *)_surface;
...
static cairo_int_status_t fill_rectangles(void * _dst, cairo_operator_t op, const cairo_color_t * color,
    CairoIRect  * rects, int num_rects)
{
	cairo_image_surface_t * dst = (cairo_image_surface_t *)_dst;
	uint32_t pixel;
	int i;
...
static cairo_int_status_t fill_boxes(void * _dst, cairo_operator_t op, const cairo_color_t * color, cairo_boxes_t * boxes)
{
	cairo_image_surface_t * dst = (cairo_image_surface_t *)_dst;
	_cairo_boxes_t::_cairo_boxes_chunk * chunk;
	uint32_t pixel;
...
static cairo_int_status_t composite(void * _dst, cairo_operator_t op,
    cairo_surface_t * abstract_src, cairo_surface_t * abstract_mask,
...
static cairo_int_status_t composite_boxes(void * _dst,
    cairo_operator_t op,
    cairo_surface_t        * abstract_src,
    cairo_surface_t        * abstract_mask,
    int src_x,
...
{
	pixman_image_t * dst = to_pixman_image((cairo_surface_t *)_dst);
	...
	if(((cairo_surface_t*)_dst)->is_clear &&
	    (op == CAIRO_OPERATOR_SOURCE || op == CAIRO_OPERATOR_OVER || op == CAIRO_OPERATOR_ADD)) {
...
#if PIXMAN_HAS_OP_LERP
			op = (cairo_operator_t)PIXMAN_OP_LERP_SRC;
#else
...
static void project_line_x_onto_16_16(const cairo_line_t * line, cairo_fixed_t top, cairo_fixed_t bottom, pixman_line_fixed_t * out)
{
	...
	RPoint p1, p2;
	p1.Set(_cairo_fixed_to_double(line->p1.x), _cairo_fixed_to_double(line->p1.y));
	p2.Set(_cairo_fixed_to_double(line->p2.x), _cairo_fixed_to_double(line->p2.y));
...
void _pixman_image_add_tristrip(pixman_image_t * image, int dst_x, int dst_y, cairo_tristrip_t * strip)
{
...
static inline pixman_glyph_cache_t * get_glyph_cache(void)
{
	SETIFZ(global_glyph_cache, pixman_glyph_cache_create());
...
static cairo_int_status_t composite_one_glyph(void * _dst,
    cairo_operator_t op,
    cairo_surface_t                    * _src,
    int src_x,
    int src_y,
    ...
    int src_x, int src_y, int dst_x, int dst_y, cairo_composite_glyphs_info_t * info)
{
	...
	if(i * info->extents.height > (int)sizeof(buf)) {
		...
		memzero(buf, i * info->extents.height);
		mask = pixman_image_create_bits(format, info->extents.width, info->extents.height, (uint32_t*)buf, i);
	}
	if(unlikely(mask == NULL)) {
		pixman_image_unref(white);
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	...
	pixman_image_composite32(_pixman_operator(op), ((cairo_image_source_t*)_src)->pixman_image, mask,
	    to_pixman_image((cairo_surface_t *)_dst), info->extents.x + src_x, info->extents.y + src_y,
	    0, 0, info->extents.x - dst_x, info->extents.y - dst_y, info->extents.width, info->extents.height);
	pixman_image_unref(mask);
	pixman_image_unref(white);
	return CAIRO_STATUS_SUCCESS;
}
    ...
    int src_x, int src_y, int dst_x, int dst_y, cairo_composite_glyphs_info_t * info)
{
	...
	op = (cairo_operator_t)_pixman_operator(op);
	dst = to_pixman_image((cairo_surface_t *)_dst);
...
const cairo_compositor_t * _cairo_image_traps_compositor_get(void)
{
	static cairo_traps_compositor_t compositor;
	if(compositor.base.delegate == NULL) {
		_cairo_traps_compositor_init(&compositor, &__cairo_no_compositor);
...
const cairo_compositor_t * _cairo_image_mask_compositor_get(void)
{
	static cairo_mask_compositor_t compositor;
	if(compositor.base.delegate == NULL) {
		_cairo_mask_compositor_init(&compositor, _cairo_image_traps_compositor_get());
		compositor.acquire = acquire;
		compositor.release = release;
		compositor.set_clip_region = set_clip_region;
		compositor.pattern_to_surface = _cairo_image_source_create_for_pattern;
...
static cairo_status_t _cairo_image_spans(void * abstract_renderer,
    int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	...
	do {
		...
		if(spans[0].coverage) {
			*row++ = (uint8)(r->opacity * spans[0].coverage);
			if(--len)
    ...
    int y, int height, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	...
	if(num_spans == 0) {
		memzero(mask, height * r->u.mask.stride);
	}
	else {
		uint8 * row = mask;
...
static cairo_status_t _cairo_image_finish_spans_and_zero(void * abstract_renderer)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(r->u.mask.extents.y < r->u.mask.extents.height)
		memzero(r->u.mask.data, (r->u.mask.extents.height - r->u.mask.extents.y) * r->u.mask.stride);
	return CAIRO_STATUS_SUCCESS;
}

static cairo_status_t _fill8_spans(void * abstract_renderer, int y, int h,
...
static cairo_status_t _fill16_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
...
static cairo_status_t _fill32_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
...
#if 0
static cairo_status_t _fill_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = abstract_renderer;
...
static cairo_status_t _mono_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
	...
	do {
		if(spans[0].coverage) {
			pixman_image_composite32((pixman_op_t)r->op,
...
static cairo_status_t _mono_unbounded_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0) {
...
static cairo_status_t _mono_finish_unbounded_spans(void * abstract_renderer)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
...
#define G_SHIFT 8
static inline uint32_t mul8x2_8(uint32_t a, uint8 b)
{
	uint32_t t = (a & RB_MASK) * b + RB_ONE_HALF;
	return ((t + ((t >> G_SHIFT) & RB_MASK)) >> G_SHIFT) & RB_MASK;
}

static inline uint32_t add8x2_8x2(uint32_t a, uint32_t b)
{
	uint32_t t = a + b;
...
static inline uint8 mul8_8(uint8 a, uint8 b)
{
	uint16 t = a * (uint16)b + ONE_HALF;
	return ((t >> G_SHIFT) + t) >> G_SHIFT;
}

static inline uint32_t lerp8x4(uint32_t src, uint8 a, uint32_t dst)
{
...
static cairo_status_t _fill_xrgb32_lerp_opaque_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
...
static cairo_status_t _blit_xrgb32_lerp_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	if(num_spans == 0)
...
static cairo_status_t _inplace_spans(void * abstract_renderer,
    int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * mask;
	int x0, x1;
	...
	if(num_spans == 2 && spans[0].coverage == 0xff) {
		pixman_image_composite32((pixman_op_t)r->op, r->src, NULL, r->u.composite.dst,
	...
	mask = (uint8*)pixman_image_get_data(r->mask);
	x1 = x0 = spans[0].x;
	do {
		int len = spans[1].x - spans[0].x;
		*mask++ = spans[0].coverage;
		if(len > 1) {
			if(len >= r->u.composite.run_length && spans[0].coverage == 0xff) {
				if(x1 != x0) {
					pixman_image_composite32((pixman_op_t)r->op, r->src, r->mask, r->u.composite.dst,
...
static cairo_status_t _inplace_opacity_spans(void * abstract_renderer, int y, int h,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * mask;
...
static cairo_status_t _inplace_src_spans(void * abstract_renderer, int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * m;
	int x0;
	...
	do {
		...
		if(len >= r->u.composite.run_length && spans[0].coverage == 0xff) {
			...
			pixman_image_composite32(PIXMAN_OP_SRC, r->src, NULL, r->u.composite.dst,
			    spans[0].x + r->u.composite.src_x, y + r->u.composite.src_y,
			    0, 0, spans[0].x, y, spans[1].x - spans[0].x, h);
			m = r->_buf;
			x0 = spans[1].x;
		}
		else if(spans[0].coverage == 0x0) {
...
static cairo_status_t _inplace_src_opacity_spans(void * abstract_renderer, int y, int h, const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_image_span_renderer_t * r = (cairo_image_span_renderer_t *)abstract_renderer;
	uint8 * mask;
	int x0;
...
static cairo_int_status_t inplace_renderer_init(cairo_image_span_renderer_t * r,
    const cairo_composite_rectangles_t * composite, cairo_antialias_t antialias, cairo_bool_t needs_clip)
{
	cairo_image_surface_t * dst = (cairo_image_surface_t*)composite->surface;
	uint8 * buf;
	if(composite->mask_pattern.base.type != CAIRO_PATTERN_TYPE_SOLID)
	...
	else if(oneof2(dst->format, CAIRO_FORMAT_ARGB32, CAIRO_FORMAT_RGB24) &&
	    (composite->op == CAIRO_OPERATOR_SOURCE || (composite->op == CAIRO_OPERATOR_OVER &&
			    (dst->base.is_clear || (dst->base.content & CAIRO_CONTENT_ALPHA) == 0))) &&
	    composite->source_pattern.base.type == CAIRO_PATTERN_TYPE_SURFACE &&
	    composite->source_pattern.surface.surface->backend->type == CAIRO_SURFACE_TYPE_IMAGE &&
	    to_image_surface(composite->source_pattern.surface.surface)->format == dst->format) {
		cairo_image_surface_t * src = to_image_surface(composite->source_pattern.surface.surface);
		int tx, ty;
	...
	if(r->base.render_rows == NULL) {
		...
		if(oneof2(src->type, CAIRO_PATTERN_TYPE_LINEAR, CAIRO_PATTERN_TYPE_RADIAL))
			r->u.composite.run_length = 256;
		if(dst->base.is_clear && oneof3(composite->op, CAIRO_OPERATOR_SOURCE, CAIRO_OPERATOR_OVER, CAIRO_OPERATOR_ADD)) {
			r->op = PIXMAN_OP_SRC;
		}
		else if(composite->op == CAIRO_OPERATOR_SOURCE) {
		...
		r->src = _pixman_image_for_pattern(dst, src, FALSE,
		    &composite->bounded, &composite->source_sample_area, &r->u.composite.src_x, &r->u.composite.src_y);
		if(unlikely(r->src == NULL))
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
    ...
    cairo_bool_t needs_clip)
{
	...
	if(op == CAIRO_OPERATOR_CLEAR) {
#if PIXMAN_HAS_OP_LERP
		op = (cairo_operator_t)PIXMAN_OP_LERP_CLEAR;
#else
		...
		if(_cairo_pattern_is_opaque(&composite->source_pattern.base, &composite->source_sample_area)) {
			op = (cairo_operator_t)PIXMAN_OP_OVER;
		}
		else {
#if PIXMAN_HAS_OP_LERP
			op = (cairo_operator_t)PIXMAN_OP_LERP_SRC;
	...
	r->op = op;
	r->src = _pixman_image_for_pattern(dst, source, FALSE,
	    &composite->unbounded, &composite->source_sample_area,
	...
	if(composite->mask_pattern.base.type == CAIRO_PATTERN_TYPE_SOLID) {
		...
		int mask_x, mask_y;
		pixman_image_t * mask = _pixman_image_for_pattern(dst, &composite->mask_pattern.base, TRUE,
	...
	if(r->u.mask.extents.height * r->u.mask.stride > SZ_BUF) {
		r->mask = pixman_image_create_bits(PIXMAN_a8, r->u.mask.extents.width, r->u.mask.extents.height, NULL, 0);
		r->base.render_rows = _cairo_image_spans;
		r->base.finish = NULL;
	}
	else {
		r->mask = pixman_image_create_bits(PIXMAN_a8, r->u.mask.extents.width, r->u.mask.extents.height, (uint32_t*)r->_buf, r->u.mask.stride);
...
static void span_renderer_fini(cairo_abstract_span_renderer_t * _r, cairo_int_status_t status)
{
	...
	if(likely(status == CAIRO_INT_STATUS_SUCCESS && r->bpp == 0)) {
		...
		pixman_image_composite32((pixman_op_t)r->op, r->src, r->mask,
...
const cairo_compositor_t * _cairo_image_spans_compositor_get(void)
{
	static cairo_spans_compositor_t spans;
	static cairo_compositor_t shape;
	if(spans.base.delegate == NULL) {
		_cairo_shape_mask_compositor_init(&shape, _cairo_image_traps_compositor_get());
		shape.glyphs = NULL;
