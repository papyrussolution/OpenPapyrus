...
static void _gl_lock(void * device)
{
	cairo_gl_context_t * ctx = (cairo_gl_context_t*)device;
	ctx->acquire(ctx);
}

static void _gl_unlock(void * device)
{
	cairo_gl_context_t * ctx = (cairo_gl_context_t*)device;
...
static cairo_status_t _gl_flush(void * device)
{
...
static void _gl_finish(void * device)
{
...
static void _gl_destroy(void * device)
{
	cairo_gl_context_t * ctx = device;
...
cairo_status_t _cairo_gl_context_init(cairo_gl_context_t * ctx)
{
	cairo_status_t status;
	...
	memzero(ctx->glyph_cache, sizeof(ctx->glyph_cache));
	...
	_cairo_array_init(&ctx->tristrip_indices, sizeof(ushort));
...
void _cairo_gl_context_activate(cairo_gl_context_t * ctx,
...
static GLenum _get_depth_stencil_format(cairo_gl_context_t * ctx)
{
...
#if CAIRO_HAS_GLESV2_SURFACE
static void _cairo_gl_ensure_msaa_gles_framebuffer(cairo_gl_context_t * ctx,
...
static void _cairo_gl_ensure_framebuffer(cairo_gl_context_t * ctx,
    cairo_gl_surface_t * surface)
{
	GLenum status;
	cairo_gl_dispatch_t * dispatch = &ctx->dispatch;
...
#if CAIRO_HAS_GL_SURFACE
static void _cairo_gl_ensure_multisampling(cairo_gl_context_t * ctx,
    cairo_gl_surface_t * surface)
{
 ...
 * mesa source code.
 */
static void _gl_identity_ortho(GLfloat * m,
...
void _cairo_gl_context_set_destination(cairo_gl_context_t * ctx,
    cairo_gl_surface_t * surface,
    cairo_bool_t multisampling)
{
...
void cairo_gl_device_set_thread_aware(cairo_device_t        * device,
