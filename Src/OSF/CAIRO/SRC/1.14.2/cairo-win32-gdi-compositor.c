...
static COLORREF FASTCALL color_to_rgb(const cairo_color_t * c)
{
...
static cairo_status_t alpha_blend_boxes(cairo_win32_display_surface_t * dst,
    const cairo_pattern_t * source, cairo_boxes_t * boxes, uint8_t alpha)
{
	const cairo_surface_pattern_t * pattern;
	struct copy_box cb;
	cairo_surface_t * surface;
...
static cairo_int_status_t _cairo_win32_gdi_compositor_stroke(const cairo_compositor_t    * compositor,
    ...
    cairo_bool_t overlap)
{
	...
	if(check_blit(composite) && check_glyphs(composite, scaled_font)) {
		...
		if(status)
			return status;
		status = _cairo_win32_surface_emit_glyphs(&dst->win32, &composite->source_pattern.base,
		    glyphs, num_glyphs, scaled_font, TRUE);
