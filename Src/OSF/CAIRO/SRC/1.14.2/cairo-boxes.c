...
static void FASTCALL _cairo_boxes_add_internal(cairo_boxes_t * boxes, const cairo_box_t * box)
{
	...
	if(unlikely(chunk->count == chunk->size)) {
		...
		chunk->next = (_cairo_boxes_t::_cairo_boxes_chunk *)_cairo_malloc_ab_plus_c(size, sizeof(cairo_box_t), sizeof(_cairo_boxes_t::_cairo_boxes_chunk));
...
cairo_box_t * _cairo_boxes_to_array(const cairo_boxes_t * boxes, int * num_boxes, cairo_bool_t force_allocation)
{
...
{
	struct cairo_box_renderer * r = (struct cairo_box_renderer *)abstract_renderer;
