...
static cairo_bool_t _cairo_clip_contains_rectangle_box(const cairo_clip_t * clip, const CairoIRect * rect, const cairo_box_t * box)
{
	int i;
...
cairo_clip_t * _cairo_clip_intersect_rectilinear_path(cairo_clip_t * clip,
...
static cairo_clip_t * _cairo_clip_intersect_rectangle_box(cairo_clip_t * clip, const CairoIRect * r, const cairo_box_t * box)
{
...
cairo_clip_t * _cairo_clip_intersect_box(cairo_clip_t * clip, const cairo_box_t * box)
{
	CairoIRect r;
	if(_cairo_clip_is_all_clipped(clip))
		return clip;
...
static cairo_status_t _reduce_move_to(void * closure, const cairo_point_t * point)
{
	...
	cairo_status_t status = _reduce_close(closure);
...
cairo_clip_t * _cairo_clip_from_boxes(const cairo_boxes_t * boxes)
{
	cairo_box_t extents;
