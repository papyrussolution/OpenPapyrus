...
static cairo_status_t _cairo_xcb_connection_find_visual_formats(cairo_xcb_connection_t * connection,
...
static cairo_status_t _cairo_xcb_connection_parse_xrender_formats(cairo_xcb_connection_t * connection,
    const xcb_render_query_pict_formats_reply_t * formats)
{
...
static xcb_render_query_version_reply_t * _render_restrict_env(xcb_render_query_version_reply_t * version)
{
	const char * env;
	if(version == NULL)
		return NULL;
...
static cairo_status_t _cairo_xcb_connection_query_render(cairo_xcb_connection_t * connection)
{
	xcb_connection_t * c = connection->xcb_connection;
...
#if 0
static void _cairo_xcb_connection_query_cairo(cairo_xcb_connection_t * connection)
{
	xcb_connection_t * c = connection->xcb_connection;
...
static void _cairo_xcb_connection_query_shm(cairo_xcb_connection_t * connection)
{
	xcb_connection_t * c = connection->xcb_connection;
...
static cairo_status_t _device_flush(void * device)
{
...
static void _pluck_xrender_format(void * entry,
    void * closure)
{
...
static void _device_destroy(void * device)
{
	cairo_xcb_connection_t * connection = device;
...
cairo_xcb_connection_t * _cairo_xcb_connection_get(xcb_connection_t * xcb_connection)
{
...
xcb_render_pictformat_t _cairo_xcb_connection_get_xrender_format_for_visual(cairo_xcb_connection_t * connection,
...
void _cairo_xcb_connection_put_xid(cairo_xcb_connection_t * connection, uint32_t xid)
{
...
void cairo_xcb_device_debug_cap_xrender_version(cairo_device_t * device, int major_version, int minor_version)
{
	...
	/* clear any flags that are inappropriate for the desired version */
	if(major_version < 0 && minor_version < 0) {
		connection->flags &= ~(CAIRO_XCB_HAS_RENDER | CAIRO_XCB_RENDER_HAS_COMPOSITE |
		    CAIRO_XCB_RENDER_HAS_COMPOSITE_GLYPHS | CAIRO_XCB_RENDER_HAS_FILL_RECTANGLES |
		    CAIRO_XCB_RENDER_HAS_COMPOSITE_TRAPEZOIDS | CAIRO_XCB_RENDER_HAS_PICTURE_TRANSFORM |
		    CAIRO_XCB_RENDER_HAS_FILTERS | CAIRO_XCB_RENDER_HAS_FILTER_GOOD |
		    CAIRO_XCB_RENDER_HAS_FILTER_BEST | CAIRO_XCB_RENDER_HAS_PDF_OPERATORS |
