	...
	switch(have_dx_adx_bdx) {
		...
		case HAVE_DX_ADX:
		    ...
		    if((-adx ^ dx) < 0) {
			    ...
			    cairo_int64_t dy_adx = _cairo_int32x32_64_mul(a->line.p1.y - y, adx);
...
static int edges_compare_x_for_y(const cairo_edge_t * a, const cairo_edge_t * b, int32_t y)
{
	...
	switch(have_ax_bx) {
		default:
		case HAVE_NEITHER: return edges_compare_x_for_y_general(a, b, y);
		case HAVE_AX: return -edge_compare_for_y_against_x(b, y, ax);
		case HAVE_BX: return edge_compare_for_y_against_x(a, y, bx);
		case HAVE_BOTH: return ax - bx;
	}
}

static inline int slope_compare(const edge_t * a, const edge_t * b)
{
	cairo_int64_t L, R;
	int cmp = a->dxdy.quo - b->dxdy.quo;
	if(cmp)
		return cmp;
	if(a->dxdy.rem == 0)
		return -b->dxdy.rem;
...
static inline int line_equal(const cairo_line_t * a, const cairo_line_t * b)
{
	return a->p1.x == b->p1.x && a->p1.y == b->p1.y && a->p2.x == b->p2.x && a->p2.y == b->p2.y;
}

static inline int sweep_line_compare_edges(const edge_t  * a, const edge_t  * b, cairo_fixed_t y)
{
	int cmp;
...
static cairo_bool_t intersect_lines(const edge_t * a, const edge_t * b, cairo_bo_intersect_point_t * intersection)
{
	...
	if(_cairo_int64_eq(qr.rem, den_det))
		return FALSE;
#if 0
	intersection->x.exactness = _cairo_int64_is_zero(qr.rem) ? _cairo_bo_intersect_ordinate::EXACT : _cairo_bo_intersect_ordinate::INEXACT;
#else
	intersection->x.exactness = _cairo_bo_intersect_ordinate::EXACT;
	...
	if(_cairo_int64_eq(qr.rem, den_det))
		return FALSE;
#if 0
	intersection->y.exactness = _cairo_int64_is_zero(qr.rem) ? _cairo_bo_intersect_ordinate::EXACT : _cairo_bo_intersect_ordinate::INEXACT;
#else
...
static int bo_intersect_ordinate_32_compare(int32_t a, int32_t b, int exactness)
{
	...
	return -(_cairo_bo_intersect_ordinate::INEXACT == exactness);
}
...
static cairo_bool_t bo_edge_contains_intersect_point(const edge_t * edge, cairo_bo_intersect_point_t * point)
{
	 ...
	 */
	cmp_top = bo_intersect_ordinate_32_compare(point->y.ordinate, edge->edge.top, point->y.exactness);
	if(cmp_top < 0)
		return FALSE;
	cmp_bottom = bo_intersect_ordinate_32_compare(point->y.ordinate, edge->edge.bottom, point->y.exactness);
	if(cmp_bottom > 0)
		return FALSE;
	...
	if(cmp_top == 0) {
		cairo_fixed_t top_x = line_compute_intersection_x_for_y(&edge->edge.line, edge->edge.top);
		return bo_intersect_ordinate_32_compare(top_x, point->x.ordinate, point->x.exactness) < 0;
	}
	else { /* cmp_bottom == 0 */
		cairo_fixed_t bot_x = line_compute_intersection_x_for_y(&edge->edge.line, edge->edge.bottom);
...
static inline int event_compare(const event_t * a, const event_t * b)
{
	return a->y - b->y;
}

static void FASTCALL pqueue_init(pqueue_t * pq)
{
...
static void FASTCALL pqueue_fini(pqueue_t * pq)
{
	if(pq->elements != pq->elements_embedded)
		SAlloc::F(pq->elements);
}

static cairo_bool_t FASTCALL pqueue_grow(pqueue_t * pq)
{
...
static /*inline*/ void FASTCALL pqueue_push(sweep_line_t * sweep_line, event_t * event)
{
	event_t ** elements;
	int i, parent;
...
static /*inline*/ void FASTCALL pqueue_pop(pqueue_t * pq)
{
...
static /*inline*/ event_t * FASTCALL event_next(sweep_line_t * sweep_line)
{
...
static /*inline*/ void FASTCALL event_insert_stop(sweep_line_t * sweep_line, edge_t * edge)
{
...
inline static void coverage_rewind(_sweep_line::Coverage * cells)
{
	cells->cursor = &cells->head;
}

static void coverage_init(_sweep_line::Coverage * cells)
{
	_cairo_freepool_init(&cells->pool, sizeof(struct cell));
...
static void coverage_fini(_sweep_line::Coverage * cells)
{
	_cairo_freepool_fini(&cells->pool);
}

inline static void coverage_reset(_sweep_line::Coverage * cells)
{
	cells->head.next = &cells->tail;
	cells->tail.prev = &cells->head;
	cells->count = 0;
...
inline static void full_inc_edge(edge_t * edge)
{
...
static void full_step(cairo_botor_scan_converter_t * self, sweep_line_t * sweep_line, cairo_fixed_t row, cairo_span_renderer_t * renderer)
{
	...
	if(cairo_list_is_empty(&sweep_line->active)) {
		cairo_status_t status = renderer->render_rows(renderer, top, bottom - top, NULL, 0);
		if(unlikely(status))
			longjmp(sweep_line->unwind, status);
		return;
	}
...
cairo_always_inline static void sub_inc_edge(edge_t * edge, cairo_fixed_t height)
{
	if(height == 1) {
...
static cairo_status_t botor_generate(cairo_botor_scan_converter_t     * self,
    event_t                         ** start_events,
    cairo_span_renderer_t            * renderer)
{
	cairo_status_t status;
...
{
	_cairo_botor_scan_converter::_cairo_botor_scan_converter_chunk * chunk = self->tail;
	if(chunk->count == chunk->size) {
		int size = chunk->size * 2;
		chunk->next = (_cairo_botor_scan_converter::_cairo_botor_scan_converter_chunk *)_cairo_malloc_ab_plus_c(size, sizeof(edge_t),
		    sizeof(_cairo_botor_scan_converter::_cairo_botor_scan_converter_chunk));
...
void _cairo_botor_scan_converter_init(cairo_botor_scan_converter_t * self, const cairo_box_t * extents, CairoFillRule fill_rule)
{
