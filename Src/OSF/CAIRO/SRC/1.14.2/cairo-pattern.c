...
static void FASTCALL _cairo_pattern_notify_observers(cairo_pattern_t * pattern, uint flags)
{
...
void _cairo_pattern_init(cairo_pattern_t * pattern, cairo_pattern_type_t type)
{
	...
	if(oneof2(type, CAIRO_PATTERN_TYPE_SURFACE, CAIRO_PATTERN_TYPE_RASTER_SOURCE))
...
static cairo_status_t _cairo_gradient_pattern_init_copy(cairo_gradient_pattern_t * pattern, const cairo_gradient_pattern_t * other)
{
	...
	if(other->stops == other->stops_embedded)
		...
		memcpy(pattern->stops, other->stops, other->n_stops * sizeof(cairo_gradient_stop_t));
	}
	return CAIRO_STATUS_SUCCESS;
}

static cairo_status_t FASTCALL _cairo_mesh_pattern_init_copy(cairo_mesh_pattern_t * pattern, const cairo_mesh_pattern_t * other)
{
	*pattern = *other;
...
cairo_status_t _cairo_pattern_create_copy(cairo_pattern_t ** pattern_out, const cairo_pattern_t  * other)
{
	...
	switch(other->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_solid_pattern_t));
		    break;
		case CAIRO_PATTERN_TYPE_SURFACE:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_surface_pattern_t));
		    break;
		case CAIRO_PATTERN_TYPE_LINEAR:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_linear_pattern_t));
		    break;
		case CAIRO_PATTERN_TYPE_RADIAL:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_radial_pattern_t));
		    break;
		case CAIRO_PATTERN_TYPE_MESH:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_mesh_pattern_t));
		    break;
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
		    pattern = (cairo_pattern_t *)SAlloc::M(sizeof(cairo_raster_source_pattern_t));
		    break;
		default:
		    ASSERT_NOT_REACHED;
...
static void FASTCALL _cairo_pattern_init_gradient(cairo_gradient_pattern_t * pattern, cairo_pattern_type_t type)
{
...
{
	cairo_solid_pattern_t * pattern = (cairo_solid_pattern_t *)_freed_pool_get(&freed_pattern_pool[CAIRO_PATTERN_TYPE_SOLID]);
	if(unlikely(pattern == NULL)) {
		/* None cached, need to create a new pattern. */
		pattern = (cairo_solid_pattern_t *)SAlloc::M(sizeof(cairo_solid_pattern_t));
		if(unlikely(pattern == NULL)) {
...
cairo_pattern_t * cairo_pattern_create_for_surface(cairo_surface_t * surface)
{
	...
	pattern = (cairo_surface_pattern_t *)_freed_pool_get(&freed_pattern_pool[CAIRO_PATTERN_TYPE_SURFACE]);
	if(unlikely(pattern == NULL)) {
		pattern = (cairo_surface_pattern_t *)SAlloc::M(sizeof(cairo_surface_pattern_t));
		if(unlikely(pattern == NULL)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
cairo_pattern_t * cairo_pattern_create_linear(double x0, double y0, double x1, double y1)
{
	...
	if(unlikely(pattern == NULL)) {
		pattern = (cairo_linear_pattern_t *)SAlloc::M(sizeof(cairo_linear_pattern_t));
		if(unlikely(pattern == NULL)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
{
	cairo_radial_pattern_t * pattern = (cairo_radial_pattern_t *)_freed_pool_get(&freed_pattern_pool[CAIRO_PATTERN_TYPE_RADIAL]);
	if(unlikely(pattern == NULL)) {
		pattern = (cairo_radial_pattern_t *)SAlloc::M(sizeof(cairo_radial_pattern_t));
		if(unlikely(pattern == NULL)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
...
cairo_pattern_t * cairo_pattern_create_mesh(void)
{
	...
	pattern = _freed_pool_get(&freed_pattern_pool[CAIRO_PATTERN_TYPE_MESH]);
	if(unlikely(pattern == NULL)) {
		pattern = (cairo_mesh_pattern_t *)SAlloc::M(sizeof(cairo_mesh_pattern_t));
		if(unlikely(pattern == NULL)) {
			_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
			return (cairo_pattern_t*)&_cairo_pattern_nil.base;
		}
    ...
    const cairo_user_data_key_t * key, void * user_data, cairo_destroy_func_t destroy)
{
	if(CAIRO_REFERENCE_COUNT_IS_INVALID(&pattern->ref_count))
		return pattern->status;
	return _cairo_user_data_array_set_data(&pattern->user_data, key, user_data, destroy);
}
...
static void _calc_control_point(cairo_mesh_patch_t * patch, int control_point)
{
...
void cairo_mesh_pattern_end_patch(cairo_pattern_t * pattern)
{
	...
	while(mesh->current_side < 3) {
		int corner_num;
		cairo_mesh_pattern_line_to(pattern, current_patch->points[0][0].x, current_patch->points[0][0].y);
 ...
 * Since: 1.12
 **/
void cairo_mesh_pattern_set_control_point(cairo_pattern_t * pattern, uint point_num, double x, double y)
{
	cairo_mesh_pattern_t * mesh;
	int i, j;
	if(unlikely(pattern->status))
...
static cairo_status_t _cairo_pattern_gradient_grow(cairo_gradient_pattern_t * pattern)
{
	...
	if(pattern->stops == pattern->stops_embedded) {
		new_stops = (cairo_gradient_stop_t *)_cairo_malloc_ab(new_size, sizeof(cairo_gradient_stop_t));
		if(new_stops)
...
static void _cairo_mesh_pattern_set_corner_color(cairo_mesh_pattern_t * mesh,
    uint corner_num, double red, double green, double blue, double alpha)
{
	assert(mesh->current_patch);
	assert(corner_num <= 3);
	cairo_color_t * color = &mesh->current_patch->colors[corner_num];
	color->red   = red;
	color->green = green;
	color->blue  = blue;
	color->alpha = alpha;
 ...
 * Since: 1.12
 **/
void cairo_mesh_pattern_set_corner_color_rgb(cairo_pattern_t * pattern, uint corner_num, double red, double green, double blue)
{
...
static void _cairo_pattern_add_color_stop(cairo_gradient_pattern_t * pattern, double offset, double red, double green, double blue, double alpha)
{
	...
	for(i = 0; i < pattern->n_stops; i++) {
		if(offset < stops[i].offset) {
			memmove(&stops[i + 1], &stops[i], sizeof(cairo_gradient_stop_t) * (pattern->n_stops - i));
			break;
		}
	}
 ...
 **/
cairo_bool_t FASTCALL _cairo_pattern_is_opaque_solid(const cairo_pattern_t * pattern)
{
...
static cairo_bool_t _surface_is_opaque(const cairo_surface_pattern_t * pattern, const CairoIRect * sample)
{
	CairoIRect extents;
...
static cairo_bool_t _raster_source_is_opaque(const cairo_raster_source_pattern_t * pattern, const CairoIRect * sample)
{
...
static cairo_bool_t _gradient_is_opaque(const cairo_gradient_pattern_t * gradient,
 ...
 **/
cairo_bool_t FASTCALL _cairo_pattern_is_opaque(const cairo_pattern_t * abstract_pattern, const CairoIRect * sample)
{
	if(abstract_pattern->has_component_alpha)
		return FALSE;
	else {
		const cairo_pattern_union_t * pattern = (cairo_pattern_union_t*)abstract_pattern;
 ...
 */
static int use_bilinear(double x, double y, double t)
{
	/* This is the inverse matrix! */
	double h = x*x + y*y;
	if(h < 1.0 / (0.75 * 0.75))
		return TRUE;  /* scale > .75 */
	if((h > 3.99 && h < 4.01) /* scale is 1/2 */ && !_cairo_fixed_from_double(x*y) /* parallel to an axis */ && _cairo_fixed_is_integer(_cairo_fixed_from_double(t)))
		return TRUE;
	return FALSE;
}
		...
		case CAIRO_FILTER_FAST:
		    ...
		    if(_cairo_matrix_is_pixel_exact(&pattern->matrix)) {
			    ...
			    if(pattern->filter == CAIRO_FILTER_GOOD &&
					use_bilinear(pattern->matrix.xx, pattern->matrix.xy, pattern->matrix.x0) &&
					use_bilinear(pattern->matrix.yx, pattern->matrix.yy, pattern->matrix.y0))
 ...
 **/
static inline double _cairo_hypot(double x, double y)
{
	return hypot(x, y);
}
 ...
 **/
void _cairo_pattern_sampled_area(const cairo_pattern_t * pattern, const CairoIRect * extents, CairoIRect * sample)
{
	double x1, x2, y1, y2;
	double padx, pady;
 ...
 **/
cairo_int_status_t _cairo_pattern_get_ink_extents(const cairo_pattern_t * pattern, CairoIRect * extents)
{
...
static ulong FASTCALL _cairo_solid_pattern_hash(ulong hash, const cairo_solid_pattern_t * solid)
{
	hash = _cairo_hash_bytes(hash, &solid->color, sizeof(solid->color));
	return hash;
}

static ulong FASTCALL _cairo_gradient_color_stops_hash(ulong hash, const cairo_gradient_pattern_t * gradient)
{
	hash = _cairo_hash_bytes(hash, &gradient->n_stops, sizeof(gradient->n_stops));
	for(uint n = 0; n < gradient->n_stops; n++) {
		hash = _cairo_hash_bytes(hash, &gradient->stops[n].offset, sizeof(double));
		hash = _cairo_hash_bytes(hash, &gradient->stops[n].color, sizeof(cairo_color_stop_t));
	}
	return hash;
}

ulong _cairo_linear_pattern_hash(ulong hash, const cairo_linear_pattern_t * linear)
{
...
ulong _cairo_radial_pattern_hash(ulong hash, const cairo_radial_pattern_t * radial)
{
...
static ulong FASTCALL _cairo_mesh_pattern_hash(ulong hash, const cairo_mesh_pattern_t * mesh)
{
	const cairo_mesh_patch_t * patch = (const cairo_mesh_patch_t *)_cairo_array_index_const(&mesh->patches, 0);
	const uint n = _cairo_array_num_elements(&mesh->patches);
	for(uint i = 0; i < n; i++)
		hash = _cairo_hash_bytes(hash, patch + i, sizeof(cairo_mesh_patch_t));
	return hash;
}

static ulong FASTCALL _cairo_surface_pattern_hash(ulong hash, const cairo_surface_pattern_t * surface)
{
	hash ^= surface->surface->unique_id;
	return hash;
}

static ulong FASTCALL _cairo_raster_source_pattern_hash(ulong hash, const cairo_raster_source_pattern_t * raster)
{
...
ulong _cairo_pattern_hash(const cairo_pattern_t * pattern)
{
	...
	if(pattern->type != CAIRO_PATTERN_TYPE_SOLID) {
		...
		hash = _cairo_hash_bytes(hash, &pattern->has_component_alpha, sizeof(pattern->has_component_alpha));
	}
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
...
static cairo_bool_t _cairo_mesh_pattern_equal(const cairo_mesh_pattern_t * a, const cairo_mesh_pattern_t * b)
{
	...
	uint i;
	uint num_patches_a = _cairo_array_num_elements(&a->patches);
	uint num_patches_b = _cairo_array_num_elements(&b->patches);
	if(num_patches_a != num_patches_b)
		return FALSE;
	for(i = 0; i < num_patches_a; i++) {
		patch_a = (const cairo_mesh_patch_t *)_cairo_array_index_const(&a->patches, i);
		patch_b = (const cairo_mesh_patch_t *)_cairo_array_index_const(&b->patches, i);
...
cairo_bool_t _cairo_pattern_equal(const cairo_pattern_t * a, const cairo_pattern_t * b)
{
	...
	switch(a->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		    return _cairo_solid_pattern_equal((cairo_solid_pattern_t*)a, (cairo_solid_pattern_t*)b);
		case CAIRO_PATTERN_TYPE_LINEAR:
		    return _cairo_linear_pattern_equal((cairo_linear_pattern_t*)a, (cairo_linear_pattern_t*)b);
		case CAIRO_PATTERN_TYPE_RADIAL:
		    return _cairo_radial_pattern_equal((cairo_radial_pattern_t*)a, (cairo_radial_pattern_t*)b);
		case CAIRO_PATTERN_TYPE_MESH:
		    return _cairo_mesh_pattern_equal((cairo_mesh_pattern_t*)a, (cairo_mesh_pattern_t*)b);
		case CAIRO_PATTERN_TYPE_SURFACE:
		    return _cairo_surface_pattern_equal((cairo_surface_pattern_t*)a, (cairo_surface_pattern_t*)b);
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
		    return _cairo_raster_source_pattern_equal((cairo_raster_source_pattern_t*)a, (cairo_raster_source_pattern_t*)b);
		default:
		    ASSERT_NOT_REACHED;
		    return FALSE;
	}
...
cairo_status_t cairo_pattern_get_surface(cairo_pattern_t * pattern, cairo_surface_t ** surface)
{
	...
	if(pattern->type != CAIRO_PATTERN_TYPE_SURFACE)
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	ASSIGN_PTR(surface, spat->surface);
	return CAIRO_STATUS_SUCCESS;
}
/**
    ...
    int index, double * offset, double * red, double * green, double * blue, double * alpha)
{
	...
	if(!oneof2(pattern->type, CAIRO_PATTERN_TYPE_LINEAR, CAIRO_PATTERN_TYPE_RADIAL))
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	if(index < 0 || (uint)index >= gradient->n_stops)
		return _cairo_error(CAIRO_STATUS_INVALID_INDEX);
	ASSIGN_PTR(offset, gradient->stops[index].offset);
	ASSIGN_PTR(red, gradient->stops[index].color.red);
	ASSIGN_PTR(green, gradient->stops[index].color.green);
	ASSIGN_PTR(blue, gradient->stops[index].color.blue);
	ASSIGN_PTR(alpha, gradient->stops[index].color.alpha);
	return CAIRO_STATUS_SUCCESS;
}
...
cairo_status_t cairo_pattern_get_color_stop_count(cairo_pattern_t * pattern, int * count)
{
	...
	if(!oneof2(pattern->type, CAIRO_PATTERN_TYPE_LINEAR, CAIRO_PATTERN_TYPE_RADIAL))
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	ASSIGN_PTR(count, gradient->n_stops);
	return CAIRO_STATUS_SUCCESS;
}
...
cairo_status_t cairo_pattern_get_linear_points(cairo_pattern_t * pattern, double * x0, double * y0, double * x1, double * y1)
{
	...
	if(pattern->type != CAIRO_PATTERN_TYPE_LINEAR)
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	ASSIGN_PTR(x0, linear->pd1.x);
	ASSIGN_PTR(y0, linear->pd1.y);
	ASSIGN_PTR(x1, linear->pd2.x);
	ASSIGN_PTR(y1, linear->pd2.y);
	return CAIRO_STATUS_SUCCESS;
}
    ...
    double * x0, double * y0, double * r0, double * x1, double * y1, double * r1)
{
	...
	if(pattern->type != CAIRO_PATTERN_TYPE_RADIAL)
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	ASSIGN_PTR(x0, radial->cd1.center.x);
	ASSIGN_PTR(y0, radial->cd1.center.y);
	ASSIGN_PTR(r0, radial->cd1.radius);
	ASSIGN_PTR(x1, radial->cd2.center.x);
	ASSIGN_PTR(y1, radial->cd2.center.y);
	ASSIGN_PTR(r1, radial->cd2.radius);
	return CAIRO_STATUS_SUCCESS;
}
 ...
 * Since: 1.12
 **/
cairo_path_t * cairo_mesh_pattern_get_path(cairo_pattern_t * pattern, uint patch_num)
{
	cairo_mesh_pattern_t * mesh = (cairo_mesh_pattern_t*)pattern;
	const cairo_mesh_patch_t * patch;
	cairo_path_t * path;
	...
	for(l = 0; l < 4; l++) {
		...
		for(int k = 1; k < 4; k++) {
			...
			int i = mesh_path_point_i[current_point];
			int j = mesh_path_point_j[current_point];
...
cairo_status_t cairo_mesh_pattern_get_corner_color_rgba(cairo_pattern_t * pattern,
    uint patch_num, uint corner_num, double * red, double * green, double * blue, double * alpha)
{
	cairo_mesh_pattern_t * mesh = (cairo_mesh_pattern_t*)pattern;
	uint patch_count;
	const cairo_mesh_patch_t * patch;
	...
	ASSIGN_PTR(green, patch->colors[corner_num].green);
	ASSIGN_PTR(blue, patch->colors[corner_num].blue);
	ASSIGN_PTR(alpha, patch->colors[corner_num].alpha);
	return CAIRO_STATUS_SUCCESS;
}
 ...
 * Since: 1.12
 **/
cairo_status_t cairo_mesh_pattern_get_control_point(cairo_pattern_t * pattern, uint patch_num, uint point_num, double * x, double * y)
{
	cairo_mesh_pattern_t * mesh = (cairo_mesh_pattern_t*)pattern;
	const cairo_mesh_patch_t * patch;
	uint patch_count;
...
static void _cairo_debug_print_surface_pattern(FILE * file, const cairo_surface_pattern_t * pattern)
{
...
static void _cairo_debug_print_radial_pattern(FILE * file, const cairo_radial_pattern_t * pattern)
{
}

static void _cairo_debug_print_mesh_pattern(FILE * file, const cairo_mesh_pattern_t * pattern)
{
}
