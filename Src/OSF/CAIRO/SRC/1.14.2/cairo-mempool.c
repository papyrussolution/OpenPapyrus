...
//
// a simple buddy allocator for memory pools XXX fragmentation? use Doug Lea's SAlloc::M?
//
...
{
	_cairo_mempool::_cairo_memblock * block;
	if(clear)
...
static _cairo_mempool::_cairo_memblock * get_buddy(cairo_mempool_t * pool, size_t offset, int bits)                                
{
	_cairo_mempool::_cairo_memblock * block;
...
static void merge_buddies(cairo_mempool_t * pool, _cairo_mempool::_cairo_memblock * block, int max_bits)
{
...
static int merge_bits(cairo_mempool_t * pool, int max_bits)
{
	...
	for(bits = 0; bits < max_bits - 1; bits++) {
		cairo_list_foreach_entry_safe(block, next, _cairo_mempool::_cairo_memblock, &pool->free[bits], link)
		{
			...
			if(buddy == NULL)
				continue;
			if(buddy == next) {
				next = cairo_container_of(buddy->link.next, _cairo_mempool::_cairo_memblock, link);
			}
			cairo_list_del(&block->link);
...
static void * buddy_malloc(cairo_mempool_t * pool, int bits)
{
	...
	for(b = bits; b <= pool->max_free_bits; b++) {
		if(!cairo_list_is_empty(&pool->free[b])) {
			block = cairo_list_first_entry(&pool->free[b], _cairo_mempool::_cairo_memblock, link);
			break;
		}
	}
	assert(block != NULL);
