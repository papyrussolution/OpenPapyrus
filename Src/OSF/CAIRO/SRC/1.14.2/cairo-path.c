...
static cairo_status_t _cpp_curve_to(void * closure, const cairo_point_t * p1, const cairo_point_t * p2, const cairo_point_t * p3)
{
	...
	double x3 = _cairo_fixed_to_double(p3->x);
	double y3 = _cairo_fixed_to_double(p3->y);
...
static cairo_status_t _cairo_path_populate(cairo_path_t * path, cairo_path_fixed_t * path_fixed, cairo_t * cr, cairo_bool_t flatten)
{
	cairo_status_t status;
	cpp_t cpp;
	cpp.data = path->data;
...
cairo_status_t _cairo_path_append_to_context(const cairo_path_t * path, cairo_t * cr)
{
	...
	for(const cairo_path_data_t * p = &path->data[0]; p < end; p += p->header.length) {
