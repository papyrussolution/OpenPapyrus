...
static void _cairo_rectilinear_stroker_limit(cairo_rectilinear_stroker_t * stroker, const cairo_box_t * boxes, int num_boxes)
{
    ...
    cairo_antialias_t antialias, cairo_boxes_t * boxes)
{
	...
	if(stroke_style->miter_limit < SMathConst::Sqrt2)
		return FALSE;
	if(!(stroke_style->line_cap == CAIRO_LINE_CAP_BUTT || stroke_style->line_cap == CAIRO_LINE_CAP_SQUARE)) {
		return FALSE;
	}
	...
	stroker->half_line_x = _cairo_fixed_from_double(fabs(ctm->xx) * stroke_style->line_width / 2.0);
...
static cairo_status_t _cairo_rectilinear_stroker_add_segment(cairo_rectilinear_stroker_t * stroker,
    const cairo_point_t * p1, const cairo_point_t * p2, unsigned flags)
{
...
static cairo_status_t _cairo_rectilinear_stroker_line_to(void * closure, const cairo_point_t * b)
{
	...
	if(a->x == b->x && a->y == b->y)
		return CAIRO_STATUS_SUCCESS;
	status = _cairo_rectilinear_stroker_add_segment(stroker, a, b, (((a->y == b->y) ? 1 : 0) | JOIN));
    ...
    cairo_antialias_t antialias, cairo_boxes_t * boxes)
{
	...
	if(!_cairo_rectilinear_stroker_init(&rectilinear_stroker, stroke_style, ctm, antialias, boxes)) {
		return CAIRO_INT_STATUS_UNSUPPORTED;
	}
	if(!rectilinear_stroker.dash.dashed && _cairo_path_fixed_is_stroke_box(path, &box) &&
	    /* if the segments overlap we need to feed them into the tessellator */
	    box.p2.x - box.p1.x > 2* rectilinear_stroker.half_line_x && box.p2.y - box.p1.y > 2* rectilinear_stroker.half_line_y) {
		cairo_box_t b;
		/* top */
	...
	status = _cairo_path_fixed_interpret(path, _cairo_rectilinear_stroker_move_to,
