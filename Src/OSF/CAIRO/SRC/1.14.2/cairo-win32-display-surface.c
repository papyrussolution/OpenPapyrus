...
static cairo_surface_t * _cairo_win32_display_surface_create_for_dc(HDC original_dc,
    cairo_format_t format, int width, int height)
{
	...
	if(status)
		goto FAIL;
	_cairo_image_surface_set_parent(to_image_surface(surface->image), &surface->win32.base);
	...
	_cairo_surface_init(&surface->win32.base, &cairo_win32_display_surface_backend, device, _cairo_content_from_format(format));
...
static cairo_surface_t * _cairo_win32_display_surface_create_similar(void * abstract_src, cairo_content_t content, int width, int height)
{
	cairo_win32_display_surface_t * src = (cairo_win32_display_surface_t *)abstract_src;
...
static cairo_surface_t * _cairo_win32_display_surface_create_similar_image(void * abstract_other,
    cairo_format_t format, int width, int height)
{
	cairo_win32_display_surface_t * surface = (cairo_win32_display_surface_t *)abstract_other;
	cairo_image_surface_t * image;
	surface = (cairo_win32_display_surface_t*)_cairo_win32_display_surface_create_for_dc(surface->win32.dc, format, width, height);
...
static cairo_image_surface_t * _cairo_win32_display_surface_map_to_image(void * abstract_surface, const CairoIRect * extents)
{
	cairo_win32_display_surface_t * surface = (cairo_win32_display_surface_t *)abstract_surface;
	cairo_status_t status;
	...
	if(surface->fallback == NULL) {
		surface->fallback = _cairo_win32_display_surface_create_for_dc(surface->win32.dc,
		    surface->win32.format, surface->win32.extents.x + surface->win32.extents.width,
		...
		if(!BitBlt(to_win32_surface(surface->fallback)->dc, surface->win32.extents.x, surface->win32.extents.y,
			...
			surface->win32.extents.x, surface->win32.extents.y, SRCCOPY)) {
...
static cairo_int_status_t _cairo_win32_display_surface_unmap_image(void * abstract_surface, cairo_image_surface_t * image)
{
	...
	if(surface->fallback) {
		...
		surface->fallback->damage = _cairo_damage_add_rectangle(surface->fallback->damage, &r);
		surface = to_win32_display_surface(surface->fallback);
	}
...
static cairo_status_t _cairo_win32_display_surface_flush(void * abstract_surface, unsigned flags)
{
	...
	if(surface->fallback->damage) {
		...
		cairo_damage_t * damage = _cairo_damage_reduce(surface->fallback->damage);
		surface->fallback->damage = NULL;
		fallback = to_win32_display_surface(surface->fallback);
		assert(fallback->image);
		TRACE((stderr, "%s: flushing damage x %d\n", __FUNCTION__, damage->region ? cairo_region_num_rectangles(damage->region) : 0));
		if(damage->status) {
			if(!BitBlt(surface->win32.dc, 0, 0, surface->win32.extents.width, surface->win32.extents.height, fallback->win32.dc, 0, 0, SRCCOPY))
				status = _cairo_win32_print_gdi_error(__FUNCTION__);
		}
			...
			for(i = 0; i < n; i++) {
				...
				if(!BitBlt(surface->win32.dc, rect.x, rect.y, rect.width, rect.height, fallback->win32.dc, rect.x, rect.y, SRCCOPY)) {
					status = _cairo_win32_print_gdi_error(__FUNCTION__);
					break;
				}
			}
...
static cairo_int_status_t _cairo_win32_save_initial_clip(HDC hdc, cairo_win32_display_surface_t * surface)
{
	RECT rect;
	int clipBoxType;
	int gm;
...
cairo_status_t _cairo_win32_display_surface_set_clip(cairo_win32_display_surface_t * surface, cairo_clip_t * clip)
{
	char stack[512];
	CairoIRect extents;
	int num_rects;
...
void _cairo_win32_display_surface_unset_clip(cairo_win32_display_surface_t * surface)
{
	XFORM saved_xform;
...
void _cairo_win32_display_surface_discard_fallback(cairo_win32_display_surface_t * surface)
{
	if(surface->fallback) {
		TRACE((stderr, "%s (surface=%d)\n", __FUNCTION__, surface->win32.base.unique_id));
...
static cairo_int_status_t _cairo_win32_display_surface_paint(void * surface, cairo_operator_t op,
...
static cairo_int_status_t _cairo_win32_display_surface_stroke(void * surface,
    cairo_operator_t op,
    ...
    const cairo_clip_t         * clip)
{
	cairo_win32_device_t * device = to_win32_device_from_surface(surface);
	TRACE((stderr, "%s (surface=%d)\n", __FUNCTION__, to_win32_surface(surface)->base.unique_id));
	return _cairo_compositor_stroke(device->compositor, (cairo_surface_t *)surface,
	    op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
}

static cairo_int_status_t _cairo_win32_display_surface_fill(void * surface,
    cairo_operator_t op, const cairo_pattern_t * source,
    ...
    double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	cairo_win32_device_t * device = to_win32_device_from_surface(surface);
	TRACE((stderr, "%s (surface=%d)\n", __FUNCTION__, to_win32_surface(surface)->base.unique_id));
	return _cairo_compositor_fill(device->compositor, (cairo_surface_t *)surface,
	    op, source, path, fill_rule, tolerance, antialias, clip);
}

static cairo_int_status_t _cairo_win32_display_surface_glyphs(void * surface,
    cairo_operator_t op, const cairo_pattern_t * source,
