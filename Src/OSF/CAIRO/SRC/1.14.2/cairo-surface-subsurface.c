...
static cairo_image_surface_t * _cairo_surface_subsurface_map_to_image(void * abstract_surface, const CairoIRect * extents)
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
	CairoIRect target_extents;
...
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
	CairoIRect rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_clip_t * target_clip = _cairo_clip_copy_intersect_rectangle(clip, &rect);
	cairo_status_t status = _cairo_surface_offset_paint(surface->target,
    ...
    cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, const cairo_clip_t * clip)
{
	...
	CairoIRect rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_clip_t * target_clip = _cairo_clip_copy_intersect_rectangle(clip, &rect);
	cairo_status_t status = _cairo_surface_offset_mask(surface->target,
	    -surface->extents.x, -surface->extents.y,
    ...
    const cairo_clip_t              * clip)
{
	...
	CairoIRect rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_clip_t * target_clip = _cairo_clip_copy_intersect_rectangle(clip, &rect);
	cairo_status_t status = _cairo_surface_offset_fill(surface->target,
	    -surface->extents.x, -surface->extents.y,
    ...
    const cairo_clip_t                    * clip)
{
	...
	CairoIRect rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_clip_t * target_clip = _cairo_clip_copy_intersect_rectangle(clip, &rect);
	cairo_status_t status = _cairo_surface_offset_stroke(surface->target,
	    -surface->extents.x, -surface->extents.y,
	...
	int num_glyphs, cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	CairoIRect rect = { 0, 0, surface->extents.width, surface->extents.height };
	cairo_clip_t * target_clip = _cairo_clip_copy_intersect_rectangle(clip, &rect);
	cairo_status_t status = _cairo_surface_offset_glyphs(surface->target,
	    -surface->extents.x, -surface->extents.y,
	    op, source,
...
static cairo_bool_t _cairo_surface_subsurface_get_extents(void * abstract_surface, CairoIRect * extents)
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
	extents->x = 0;
	extents->y = 0;
...
static cairo_surface_t * _cairo_surface_subsurface_source(void * abstract_surface, CairoIRect * extents)
{
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)abstract_surface;
...
cairo_surface_t * cairo_surface_create_for_rectangle(cairo_surface_t * target, double x, double y, double width, double height)
{
	...
	cairo_surface_subsurface_t * surface = (cairo_surface_subsurface_t *)SAlloc::M(sizeof(cairo_surface_subsurface_t));
	...
	cairo_surface_set_device_scale(&surface->base, target->device_transform.xx, target->device_transform.yy);
	return &surface->base;
}

cairo_surface_t * _cairo_surface_create_for_rectangle_int(cairo_surface_t * target, const CairoIRect * extents)
{
	if(unlikely(target->status))
		return _cairo_surface_create_in_error(target->status);
	if(unlikely(target->finished))
	...
	surface->extents.x = (int)(surface->extents.x * target->device_transform.xx);
	surface->extents.y = (int)(surface->extents.y * target->device_transform.yy);
	surface->extents.width = (int)(surface->extents.width * target->device_transform.xx);
	surface->extents.height = (int)(surface->extents.height * target->device_transform.yy);
	surface->extents.x = (int)(surface->extents.x + target->device_transform.x0);
	surface->extents.y = (int)(surface->extents.y + target->device_transform.y0);
	surface->target = cairo_surface_reference(target);
	surface->base.type = surface->target->type;
	surface->snapshot = NULL;
	cairo_surface_set_device_scale(&surface->base, target->device_transform.xx, target->device_transform.yy);
...
void _cairo_surface_subsurface_set_snapshot(cairo_surface_t * surface, cairo_surface_t * snapshot)
{
	...
	_cairo_surface_attach_snapshot(ss->target, &ss->base, _cairo_surface_subsurface_detach_snapshot);
}
