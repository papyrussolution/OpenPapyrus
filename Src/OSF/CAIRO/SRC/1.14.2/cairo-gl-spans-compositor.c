...
static cairo_status_t _cairo_gl_unbounded_spans(void * abstract_renderer, int y, int height,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_gl_span_renderer_t * r = abstract_renderer;
	cairo_gl_emit_span_t emit = r->emit;
	if(y > r->ymin) {
		emit(r->ctx, r->xmin, r->ymin, r->xmax, y, 0);
	}
	if(num_spans == 0) {
		emit(r->ctx, r->xmin, y, r->xmax, y + height, 0);
	}
	else {
		if(spans[0].x != r->xmin) {
			emit(r->ctx, r->xmin, y, spans[0].x, y + height, 0);
		}
		do {
			emit(r->ctx, spans[0].x, y, spans[1].x, y + height, r->opacity * spans[0].coverage);
			spans++;
		} while(--num_spans > 1);
		if(spans[0].x != r->xmax) {
			emit(r->ctx, spans[0].x, y, r->xmax, y + height, 0);
		}
	}
	r->ymin = y + height;
	return CAIRO_STATUS_SUCCESS;
...
/* XXX */
static cairo_status_t _cairo_gl_clipped_spans(void * abstract_renderer, int y, int height,
    const cairo_half_open_span_t * spans, unsigned num_spans)
{
	cairo_gl_span_renderer_t * r = abstract_renderer;
	cairo_gl_emit_span_t emit = r->emit;
...
static cairo_status_t _cairo_gl_finish_unbounded_spans(void * abstract_renderer)
{
	...
	cairo_gl_emit_span_t emit = r->emit;
	if(r->ymax > r->ymin) {
		emit(r->ctx, r->xmin, r->ymin, r->xmax, r->ymax, 0);
	}
...
static cairo_int_status_t draw_image_boxes(void * _dst, cairo_image_surface_t * image,
    cairo_boxes_t * boxes, int dx, int dy)
{
	...
	for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
		for(i = 0; i < chunk->count; i++) {
			...
			cairo_status_t status = _cairo_gl_surface_draw_image(dst, image, x + dx, y + dy, w, h, x, y, TRUE);
			if(unlikely(status))
				return status;
		}
	}
...
static cairo_int_status_t copy_boxes(void * _dst, cairo_surface_t * _src, cairo_boxes_t * boxes, const CairoIRect * extents, int dx, int dy)
{
	cairo_gl_surface_t * dst = _dst;
...
static cairo_int_status_t composite_boxes(void * _dst, cairo_operator_t op,
    ...
    int src_x, int src_y, int mask_x, int mask_y, int dst_x, int dst_y, cairo_boxes_t * boxes, const CairoIRect  * extents)
{
	...
	TRACE((stderr, "%s mask=(%d,%d), dst=(%d, %d)\n", __FUNCTION__, mask_x, mask_y, dst_x, dst_y));
	if(abstract_mask) {
	...
	if(unlikely(status))
		goto FAIL;
	_cairo_gl_composite_set_source_operand(&setup, src_operand);
	_cairo_gl_operand_translate(&setup.src, -src_x, -src_y);
	_cairo_gl_composite_set_mask_operand(&setup, source_to_operand(abstract_mask));
    ...
    const cairo_composite_rectangles_t * composite, cairo_antialias_t antialias, cairo_bool_t needs_clip)
{
	...
	if(op == CAIRO_OPERATOR_CLEAR) {
	...
	else if(composite->surface->is_clear && oneof3(op, CAIRO_OPERATOR_SOURCE, CAIRO_OPERATOR_OVER, CAIRO_OPERATOR_ADD)) {
		...
		return CAIRO_INT_STATUS_UNSUPPORTED;
	status = _cairo_gl_composite_init(&r->setup, op, (cairo_gl_surface_t*)composite->surface, FALSE);
	if(unlikely(status))
		goto FAIL;
	status = _cairo_gl_composite_set_source(&r->setup, source, &composite->source_sample_area, &composite->unbounded, TRUE);
	if(unlikely(status))
		goto FAIL;
	r->opacity = 1.0;
	if(composite->mask_pattern.base.type == CAIRO_PATTERN_TYPE_SOLID) {
		...
		status = _cairo_gl_composite_set_mask(&r->setup, &composite->mask_pattern.base,
		    &composite->mask_sample_area, &composite->unbounded, TRUE);
		if(unlikely(status))
			goto FAIL;
	}
...
static void _cairo_gl_span_renderer_fini(cairo_abstract_span_renderer_t * _r, cairo_int_status_t status)
{
	cairo_gl_span_renderer_t * r = (cairo_gl_span_renderer_t*)_r;
...
const cairo_compositor_t * _cairo_gl_span_compositor_get(void)
{
	...
	if(spans.base.delegate == NULL) {
		...
		_cairo_shape_mask_compositor_init(&shape, _cairo_gl_traps_compositor_get());
		shape.glyphs = NULL;
