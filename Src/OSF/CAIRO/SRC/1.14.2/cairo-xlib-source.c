...
static cairo_xlib_surface_t * unwrap_source(const cairo_surface_pattern_t * pattern)
{
	CairoIRect limits;
	return (cairo_xlib_surface_t*)_cairo_pattern_get_source(pattern, &limits);
}

static cairo_status_t _cairo_xlib_source_finish(void * abstract_surface)
{
...
static cairo_status_t _cairo_xlib_proxy_finish(void * abstract_surface)
{
...
static cairo_surface_t * source(cairo_xlib_surface_t * dst, Picture picture, Pixmap pixmap)
{
...
static cairo_status_t picture_set_filter(Display * dpy,
    Picture picture,
    cairo_filter_t filter)
{
...
static int extend_to_repeat(cairo_extend_t extend)
{
	switch(extend) {
		default:
		    ASSERT_NOT_REACHED;
...
static cairo_surface_t * color_source(cairo_xlib_surface_t * dst, const cairo_color_t * color)
{
	Display * dpy = dst->display->display;
	XRenderColor xcolor;
	Picture picture;
...
static cairo_surface_t * white_source(cairo_xlib_surface_t * dst)
{
	cairo_xlib_display_t * display = dst->display;
...
static cairo_surface_t * solid_source(cairo_xlib_surface_t * dst,
...
static cairo_surface_t * embedded_source(cairo_xlib_surface_t * dst,
    ...
    int * src_x, int * src_y)
{
	...
	if(_cairo_surface_is_image(src)) {
		...
		cairo_image_surface_t * image;
		CairoIRect map_extents = { 0, 0, upload.width, upload.height };
