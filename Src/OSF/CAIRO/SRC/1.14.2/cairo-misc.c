...
cairo_glyph_t * cairo_glyph_allocate(int num_glyphs)
{
	if(num_glyphs <= 0)
		return NULL;
	return (cairo_glyph_t *)_cairo_malloc_ab(num_glyphs, sizeof(cairo_glyph_t));
}
...
cairo_text_cluster_t * cairo_text_cluster_allocate(int num_clusters)
{
	if(num_clusters <= 0)
		return NULL;
	return (cairo_text_cluster_t *)_cairo_malloc_ab(num_clusters, sizeof(cairo_text_cluster_t));
}
...
int _cairo_lround(double d)
{
	 ...
	 * care about, is OK.
	 */
#if ( defined(FLOAT_WORDS_BIGENDIAN) && !defined(WORDS_BIGENDIAN)) || (!defined(FLOAT_WORDS_BIGENDIAN) &&  defined(WORDS_BIGENDIAN))
	{
...
static ulong FASTCALL _intern_string_hash(const char * str, int len)
{
