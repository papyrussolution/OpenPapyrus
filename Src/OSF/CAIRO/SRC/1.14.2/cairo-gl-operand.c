    ...
    const CairoIRect * extents)
{
	...
	surface = (cairo_gl_surface_t*)_cairo_gl_surface_create_scratch(ctx, CAIRO_CONTENT_COLOR_ALPHA, extents->width, extents->height);
	map_extents = *extents;
	...
	if(src_is_gl_surface) {
		status = _cairo_gl_context_release(ctx, status);
		if(unlikely(status)) {
			_cairo_surface_unmap_image(&surface->base, image); // @v1.14.6
			goto fail;
		}
	}
...
void _cairo_gl_solid_operand_init(cairo_gl_operand_t * operand,
...
void _cairo_gl_operand_destroy(cairo_gl_operand_t * operand)
{
	switch(operand->type) {
		case CAIRO_GL_OPERAND_CONSTANT:
		    break;
...
cairo_int_status_t _cairo_gl_operand_init(cairo_gl_operand_t * operand,
    const cairo_pattern_t * pattern,
    cairo_gl_surface_t * dst,
    const CairoIRect * sample,
    const CairoIRect * extents,
...
cairo_filter_t _cairo_gl_operand_get_filter(cairo_gl_operand_t * operand)
{
...
GLint _cairo_gl_operand_get_gl_filter(cairo_gl_operand_t * operand)
{
...
cairo_extend_t _cairo_gl_operand_get_extend(cairo_gl_operand_t * operand)
{
...
void _cairo_gl_operand_bind_to_shader(cairo_gl_context_t * ctx,
...
cairo_bool_t _cairo_gl_operand_needs_setup(cairo_gl_operand_t * dest,
...
uint _cairo_gl_operand_get_vertex_size(const cairo_gl_operand_t * operand)
{
	switch(operand->type) {
		default:
		case CAIRO_GL_OPERAND_COUNT:
...
void _cairo_gl_operand_emit(cairo_gl_operand_t * operand,
    GLfloat ** vb,
    GLfloat x,
    GLfloat y)
{
