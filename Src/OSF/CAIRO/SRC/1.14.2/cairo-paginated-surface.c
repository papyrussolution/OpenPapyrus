...
static cairo_surface_t * _cairo_paginated_surface_create_similar(void * abstract_surface,
    ...
    const cairo_paginated_surface_backend_t        * backend)
{
	...
	_cairo_surface_init(&surface->base, &cairo_paginated_surface_backend, NULL/* device */, content);
...
cairo_surface_t * _cairo_paginated_surface_get_target(cairo_surface_t * surface)
{
	...
	cairo_paginated_surface_t * paginated_surface = (cairo_paginated_surface_t*)surface;
...
cairo_status_t _cairo_paginated_surface_set_size(cairo_surface_t * surface, int width, int height)
{
	...
	paginated_surface->recording_surface = cairo_recording_surface_create(paginated_surface->content, &recording_extents);
...
{
	cairo_paginated_surface_t * surface = (cairo_paginated_surface_t *)abstract_surface;
...
static cairo_surface_t * _cairo_paginated_surface_create_image_surface(void * abstract_surface, int width, int height)
{
	cairo_paginated_surface_t * surface = (cairo_paginated_surface_t *)abstract_surface;
	cairo_font_options_t options;
	cairo_surface_t * image = _cairo_image_surface_create_with_content(surface->content, width, height);
	cairo_surface_get_font_options(&surface->base, &options);
	_cairo_surface_set_font_options(image, &options);
	return image;
}

static cairo_surface_t * _cairo_paginated_surface_source(void * abstract_surface, CairoIRect * extents)
{
	return _cairo_surface_get_source(((cairo_paginated_surface_t *)abstract_surface)->target, extents);
}

static cairo_status_t _cairo_paginated_surface_acquire_source_image(void * abstract_surface, cairo_image_surface_t ** image_out, void ** image_extra)
{
	cairo_paginated_surface_t * surface = (cairo_paginated_surface_t *)abstract_surface;
	cairo_surface_t * image;
	cairo_status_t status;
	CairoIRect extents;
	cairo_bool_t is_bounded = _cairo_surface_get_extents(surface->target, &extents);
	if(!is_bounded)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	image = _cairo_paginated_surface_create_image_surface(surface, extents.width, extents.height);
...
static cairo_int_status_t _paint_fallback_image(cairo_paginated_surface_t * surface, CairoIRect * rect)
{
	...
	cairo_surface_t * image = _cairo_paginated_surface_create_image_surface(surface, (int)ceil(width  * x_scale), (int)ceil(height * y_scale));
	...
	cairo_matrix_init(&pattern.base.matrix, x_scale, 0, 0, y_scale, -x*x_scale, -y*y_scale);
...
static cairo_int_status_t _paint_page(cairo_paginated_surface_t * surface)
{
	...
	status = _cairo_recording_surface_replay_and_create_regions(surface->recording_surface, analysis);
	if(status)
		goto FAIL;
	...
	if(surface->backend->set_fallback_images_required) {
		...
		status = surface->backend->set_fallback_images_required(surface->target, has_fallbacks);
		if(unlikely(status))
			goto FAIL;
	}
	// Finer grained fallbacks are currently only supported for some surface types 
	if(surface->backend->supports_fine_grained_fallbacks != NULL && surface->backend->supports_fine_grained_fallbacks(surface->target)) {
...
static cairo_bool_t _cairo_paginated_surface_get_extents(void * abstract_surface, CairoIRect * rectangle)
{
	return _cairo_surface_get_extents(((cairo_paginated_surface_t *)abstract_surface)->target, rectangle);
}

static void _cairo_paginated_surface_get_font_options(void * abstract_surface, cairo_font_options_t * options)
{
	cairo_surface_get_font_options(((cairo_paginated_surface_t *)abstract_surface)->target, options);
}
...
{
	return _cairo_surface_paint(((cairo_paginated_surface_t *)abstract_surface)->recording_surface, op, source, clip);
}

static cairo_int_status_t _cairo_paginated_surface_mask(void * abstract_surface,
    cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, const cairo_clip_t * clip)
{
	return _cairo_surface_mask(((cairo_paginated_surface_t *)abstract_surface)->recording_surface, op, source, mask, clip);
}
...
{
	return _cairo_surface_stroke(((cairo_paginated_surface_t *)abstract_surface)->recording_surface, op, source, path, style, ctm, ctm_inverse, tolerance, antialias, clip);
}

static cairo_int_status_t _cairo_paginated_surface_fill(void * abstract_surface, cairo_operator_t op,
...
{
	return _cairo_surface_fill(((cairo_paginated_surface_t *)abstract_surface)->recording_surface, op, source, path, fill_rule, tolerance, antialias, clip);
}

static cairo_bool_t _cairo_paginated_surface_has_show_text_glyphs(void * abstract_surface)
{
	return cairo_surface_has_show_text_glyphs(((cairo_paginated_surface_t *)abstract_surface)->target);
}

static cairo_int_status_t _cairo_paginated_surface_show_text_glyphs(void * abstract_surface, cairo_operator_t op,
...
{
	return _cairo_surface_show_text_glyphs(((cairo_paginated_surface_t *)abstract_surface)->recording_surface, op, source,
