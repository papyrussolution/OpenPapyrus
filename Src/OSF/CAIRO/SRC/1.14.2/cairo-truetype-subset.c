    ...
    cairo_bool_t is_pdf)
{
	...
	if(length) {
		truetype_subset->data = (uchar *)malloc(length);
	...
	if(num_strings) {
		...
		truetype_subset->string_offsets = (ulong *)malloc(offsets_length);
...
static cairo_status_t find_name(tt_name_t * name, int name_id, int platform, int encoding, int language, char ** str_out)
{
	...
	for(i = 0; i < be16_to_cpu(name->num_records); i++) {
		...
		if(be16_to_cpu(record->name) == name_id &&
			...
			str = (char *)malloc(be16_to_cpu(record->length) + 1);
			if(str == NULL)
				return _cairo_error(CAIRO_STATUS_NO_MEMORY);
