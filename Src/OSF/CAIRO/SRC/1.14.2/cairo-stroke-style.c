    ...
    const cairo_path_fixed_t * path, const cairo_matrix_t * ctm, double * dx, double * dy)
{
	...
	if(style->line_cap == CAIRO_LINE_CAP_SQUARE)
		style_expansion = SMathConst::Sqrt1_2; // M_SQRT1_2;
	if(style->line_join == CAIRO_LINE_JOIN_MITER && !path->stroke_is_rectilinear && style_expansion < SMathConst::Sqrt2 * style->miter_limit) {
		style_expansion = SMathConst::Sqrt2 * style->miter_limit;
	}
...
{
	double style_expansion = 0.5;
	if(style->line_join == CAIRO_LINE_JOIN_MITER && !path->stroke_is_rectilinear &&
	    style_expansion < SMathConst::Sqrt2 * style->miter_limit) {
		style_expansion = SMathConst::Sqrt2 * style->miter_limit;
	}
	style_expansion *= style->line_width;
...
cairo_bool_t _cairo_stroke_style_dash_can_approximate(const cairo_stroke_style_t * style, const cairo_matrix_t * ctm, double tolerance)
{
	if(!style->num_dashes)
		return FALSE;
	double period = _cairo_stroke_style_dash_period(style);
    ...
    const cairo_matrix_t * ctm, double tolerance, double * dash_offset, double * dashes, uint * num_dashes)
{
	...
	uint i = 0;
	double coverage = _cairo_stroke_style_dash_stroked(style) / _cairo_stroke_style_dash_period(style);
