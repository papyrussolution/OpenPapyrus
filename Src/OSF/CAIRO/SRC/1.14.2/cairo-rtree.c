...
void _cairo_rtree_node_destroy(cairo_rtree_t * rtree, cairo_rtree_node_t * node)
{
	int i;
...
void _cairo_rtree_node_collapse(cairo_rtree_t * rtree, cairo_rtree_node_t * node)
{
	int i;
	do {
...
cairo_status_t _cairo_rtree_node_insert(cairo_rtree_t * rtree, cairo_rtree_node_t * node, int width, int height, cairo_rtree_node_t ** out)
{
	int w, h, i;
	assert(node->state == CAIRO_RTREE_NODE_AVAILABLE);
	assert(node->pinned == FALSE);
	if(node->width  - width  > rtree->min_size || node->height - height > rtree->min_size) {
		w = node->width  - width;
...
void _cairo_rtree_node_remove(cairo_rtree_t * rtree, cairo_rtree_node_t * node)
{
...
cairo_int_status_t _cairo_rtree_insert(cairo_rtree_t * rtree, int width, int height, cairo_rtree_node_t ** out)
{
	cairo_rtree_node_t * node;
	cairo_list_foreach_entry(node, cairo_rtree_node_t, &rtree->available, link)
	{
...
cairo_int_status_t _cairo_rtree_evict_random(cairo_rtree_t * rtree, int width, int height, cairo_rtree_node_t ** out)
{
	...
	cairo_list_foreach_entry_safe(node, next, cairo_rtree_node_t, &rtree->pinned, link) {
		node = node->parent;
	...
	cnt = 0;
	cairo_list_foreach_entry(node, cairo_rtree_node_t, &rtree->evictable, link)
	{
		if(node->width >= width && node->height >= height)
			cnt++;
	}
...
void _cairo_rtree_unpin(cairo_rtree_t * rtree)
{
	while(!cairo_list_is_empty(&rtree->pinned)) {
		cairo_rtree_node_t * node = cairo_list_first_entry(&rtree->pinned, cairo_rtree_node_t, link);
		node->pinned = 0;
		cairo_list_move(&node->link, &rtree->evictable);
	}
}

void _cairo_rtree_init(cairo_rtree_t * rtree, int width, int height, int min_size, int node_size, void (* destroy)(cairo_rtree_node_t *))
{
	...
	memzero(&rtree->root, sizeof(rtree->root));
...
void _cairo_rtree_reset(cairo_rtree_t * rtree)
{
	int i;
...
static void _cairo_rtree_node_foreach(cairo_rtree_node_t * node, void (* func)(cairo_rtree_node_t *, void * data), void * data)
{
	for(int i = 0; i < 4 && node->children[i] != NULL; i++)
		_cairo_rtree_node_foreach(node->children[i], func, data);
	func(node, data);
}

void _cairo_rtree_foreach(cairo_rtree_t * rtree, void (* func)(cairo_rtree_node_t *, void * data), void * data)
{
	if(rtree->root.state == CAIRO_RTREE_NODE_OCCUPIED) {
		func(&rtree->root, data);
	}
	else {
		for(int i = 0; i < 4 && rtree->root.children[i] != NULL; i++)
			_cairo_rtree_node_foreach(rtree->root.children[i], func, data);
	}
}

void _cairo_rtree_fini(cairo_rtree_t * rtree)
{
	if(rtree->root.state == CAIRO_RTREE_NODE_OCCUPIED) {
		rtree->destroy(&rtree->root);
	}
