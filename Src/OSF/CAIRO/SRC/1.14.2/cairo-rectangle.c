 ...
 * corners, expressed in fixed point numbers.  A #CairoIRect is
 * also a rectangular area, but represented by the upper left corner
 * and the width and the height, as integer numbers.
 *
 * This function converts a #cairo_box_t to a #CairoIRect by
 * increasing the area to the nearest integer coordinates.  We should
 * standardize on #cairo_rectangle_fixed_t and #CairoIRect, and
...
static cairo_status_t _cairo_box_add_spline_point(void * closure, const cairo_point_t * point, const cairo_slope_t * tangent)
{
    ...
    const cairo_point_t * b, const cairo_point_t * c, const cairo_point_t * d)
{
	...
	if(!_cairo_box_contains_point(extents, b) || !_cairo_box_contains_point(extents, c)) {
		cairo_status_t status = _cairo_spline_bound(_cairo_box_add_spline_point, extents, a, b, c, d);
		assert(status == CAIRO_STATUS_SUCCESS);
	}
}

void _cairo_rectangle_int_from_double(CairoIRect * recti, const cairo_rectangle_t * rectf)
{
