...
static void quartz_font_ensure_symbols(void)
{
...
static cairo_status_t _cairo_quartz_font_face_create_for_toy(cairo_toy_font_face_t   * toy_face,
    cairo_font_face_t      ** font_face)
{
	...
	full_name = SAlloc::M(strlen(family) + 64); // give us a bit of room to tack on Bold, Oblique, etc.
    ...
    cairo_scaled_font_t ** font_out)
{
	...
	if(!_cairo_quartz_font_symbols_present)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	font = SAlloc::M(sizeof(cairo_quartz_scaled_font_t));
	if(font == NULL)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	memzero(font, sizeof(cairo_quartz_scaled_font_t));
...
cairo_font_face_t * cairo_quartz_font_face_create_for_cgfont(CGFontRef font)
{
	...
	font_face = SAlloc::M(sizeof(cairo_quartz_font_face_t));
	if(!font_face) {
...
static void _cairo_quartz_scaled_font_fini(void * abstract_font)
{
}

#define INVALID_GLYPH 0x00

static inline CGGlyph _cairo_quartz_scaled_glyph_index(cairo_scaled_glyph_t * scaled_glyph) {
	ulong index = _cairo_scaled_glyph_index(scaled_glyph);
	if(index > 0xffff)
		return INVALID_GLYPH;
	return (CGGlyph)index;
}

static cairo_int_status_t _cairo_quartz_init_glyph_metrics(cairo_quartz_scaled_font_t * font,
...
static void _cairo_quartz_path_apply_func(void * info, const CGPathElement * el)
{
...
static cairo_int_status_t _cairo_quartz_init_glyph_path(cairo_quartz_scaled_font_t * font,
    cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_quartz_font_face_t * font_face = _cairo_quartz_scaled_to_face(font);
	CGGlyph glyph = _cairo_quartz_scaled_glyph_index(scaled_glyph);
	...
	textMatrix = CGAffineTransformMake(font->base.scale.xx, font->base.scale.yx,
	    -font->base.scale.xy, -font->base.scale.yy, 0, 0);
	ctFont = CTFontCreateWithGraphicsFont(font_face->cgFont, 1.0, NULL, NULL); // @v1.14.6 0.0-->1.0
...
static cairo_int_status_t _cairo_quartz_init_glyph_surface(cairo_quartz_scaled_font_t * font,
    cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	...
	//fprintf (stderr, "glyphRect[n]: %f %f %f %f\n", glyphRect.origin.x, glyphRect.origin.y, glyphRect.size.width,
...
static cairo_int_status_t _cairo_quartz_scaled_glyph_init(void * abstract_font,
    cairo_scaled_glyph_t * scaled_glyph,
