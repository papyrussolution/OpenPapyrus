...
cairo_int_status_t __cairo_contour_add_point(cairo_contour_t * contour, const cairo_point_t * point)
{
	...
	next = (cairo_contour_chain_t *)_cairo_malloc_ab_plus_c(tail->size_points*2, sizeof(cairo_point_t), sizeof(cairo_contour_chain_t));
...
static void first_inc(cairo_contour_t * contour, cairo_point_t ** p, cairo_contour_chain_t ** chain)
{
...
static void last_dec(cairo_contour_t * contour, cairo_point_t ** p, cairo_contour_chain_t ** chain)
{
	if(*p == (*chain)->points) {
...
cairo_int_status_t _cairo_contour_add(cairo_contour_t * dst, const cairo_contour_t * src)
{
...
static void iter_init(cairo_contour_iter_t * iter, cairo_contour_t * contour)
{
	iter->chain = &contour->chain;
	iter->point = &contour->chain.points[0];
}

static void iter_init_last(cairo_contour_iter_t * iter, cairo_contour_t * contour)
{
...
cairo_int_status_t _cairo_contour_add_reversed(cairo_contour_t * dst, const cairo_contour_t * src)
{
	if(src->chain.num_points == 0)
		return CAIRO_INT_STATUS_SUCCESS;
	else {
		for(const cairo_contour_chain_t * last = src->tail; last; last = prev_const_chain(src, last)) {
			for(int i = last->num_points-1; i >= 0; i--) {
				cairo_int_status_t status = _cairo_contour_add_point(dst, &last->points[i]);
				if(unlikely(status))
					return status;
			}
		}
...
static cairo_bool_t _cairo_contour_simplify_chain(cairo_contour_t * contour, const double tolerance,
    const cairo_contour_iter_t * first, const cairo_contour_iter_t * last)
{
	cairo_contour_iter_t furthest;
	cairo_contour_iter_t iter = *first;
	iter_next(&iter);
	if(iter_equal(&iter, last))
		return FALSE;
	else {
		int x0 = first->point->x;
		int y0 = first->point->y;
		int nx = last->point->y - y0;
		int ny = x0 - last->point->x;
		int count = 0;
		...
		if(count == 0)
			return FALSE;
		else if(max_error > tolerance * ((uint64_t)nx * nx + (uint64_t)ny * ny)) {
			cairo_bool_t simplified = FALSE;
...
void _cairo_contour_simplify(cairo_contour_t * contour, double tolerance)
{
	...
	do {
		...
		simplified |= _cairo_contour_simplify_chain(contour, tolerance, &iter, &furthest);
		iter_init_last(&iter, contour);
		if(!iter_equal(&furthest, &iter))
			simplified |= _cairo_contour_simplify_chain(contour, tolerance, &furthest, &iter);
	} while(simplified);
	iter_init(&iter, contour);
	for(chain = &contour->chain; chain; chain = chain->next) {
...
void _cairo_contour_reset(cairo_contour_t * contour)
{
	_cairo_contour_fini(contour);
	_cairo_contour_init(contour, contour->direction);
}

void _cairo_contour_fini(cairo_contour_t * contour)
{
...
void _cairo_debug_print_contour(FILE * file, cairo_contour_t * contour)
{
	cairo_contour_chain_t * chain;
	int num_points = 0;
	int size_points = 0;
	...
	for(chain = &contour->chain; chain; chain = chain->next) {
		for(int i = 0; i < chain->num_points; i++) {
			fprintf(file, "  [%d] = (%f, %f)\n", num_points++, _cairo_fixed_to_double(chain->points[i].x), _cairo_fixed_to_double(chain->points[i].y));
		}
	}
}

void __cairo_contour_remove_last_chain(cairo_contour_t * contour)
{
	cairo_contour_chain_t * chain;
