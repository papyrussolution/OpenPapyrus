...
static cairo_status_t _cairo_gl_gradient_render(const cairo_gl_context_t    * ctx,
    uint n_stops,
    const cairo_gradient_stop_t * stops,
    void * bytes,
    int width)
...
cairo_int_status_t _cairo_gl_gradient_create(cairo_gl_context_t           * ctx,
    uint n_stops,
    const cairo_gradient_stop_t  * stops,
    cairo_gl_gradient_t         ** gradient_out)
{
	...
	if((uint)ctx->max_texture_size / 2 <= n_stops)
...
cairo_gl_gradient_t * _cairo_gl_gradient_reference(cairo_gl_gradient_t * gradient)
{
...
void _cairo_gl_gradient_destroy(cairo_gl_gradient_t * gradient)
{
