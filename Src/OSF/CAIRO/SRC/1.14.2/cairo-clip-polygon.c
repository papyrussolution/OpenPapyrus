...
cairo_int_status_t _cairo_clip_get_polygon(const cairo_clip_t * clip,
    cairo_polygon_t * polygon, CairoFillRule * fill_rule, cairo_antialias_t * antialias)
{
...
cairo_bool_t _cairo_clip_is_polygon(const cairo_clip_t * clip)
{
	if(_cairo_clip_is_all_clipped(clip))
		return TRUE;
	/* If there is no clip, we need an infinite polygon */
