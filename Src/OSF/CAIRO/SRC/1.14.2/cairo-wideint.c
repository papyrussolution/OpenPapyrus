...
{
	cairo_int64_t s = _cairo_uint32x32_64_mul((uint32_t)a, (uint32_t)b);
	if(a < 0)
		s.hi -= b;
	if(b < 0)
		s.hi -= a;
...
{
	cairo_uint64_t s = _cairo_uint32x32_64_mul(a.lo, b.lo);
...
cairo_uquorem64_t _cairo_uint64_divrem(cairo_uint64_t num, cairo_uint64_t den)
{
	...
	cairo_uint64_t quo;
	cairo_uint64_t bit = _cairo_uint32_to_uint64(1);
...
{
	cairo_uint128_t s;
	uint32_t al = (uint32_t)uint64_lo32(a);
	uint32_t ah = (uint32_t)uint64_hi32(a);
	uint32_t bl = (uint32_t)uint64_lo32(b);
	uint32_t bh = (uint32_t)uint64_hi32(b);
...
{
	int cmp = _cairo_uint64_cmp(a.hi, b.hi);
	if(cmp)
		return cmp;
