...
static void FASTCALL _cairo_scaled_font_fini_internal(cairo_scaled_font_t * scaled_font);
...
static void _cairo_scaled_glyph_fini(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t * scaled_glyph)
{
	while(!cairo_list_is_empty(&scaled_glyph->dev_privates)) {
		cairo_scaled_glyph_private_t * p_private = cairo_list_first_entry(&scaled_glyph->dev_privates,
...
static cairo_scaled_font_map_t * _cairo_scaled_font_map_lock(void)
{
	...
	if(cairo_scaled_font_map == NULL) {
		cairo_scaled_font_map = (cairo_scaled_font_map_t *)SAlloc::M(sizeof(cairo_scaled_font_map_t));
...
cairo_status_t _cairo_scaled_font_register_placeholder_and_unlock_font_map(cairo_scaled_font_t * scaled_font)
{
	...
	if(unlikely(status))
		return status;
	placeholder_scaled_font = (cairo_scaled_font_t *)SAlloc::M(sizeof(cairo_scaled_font_t));
...
static cairo_bool_t _cairo_scaled_font_keys_equal(const void * abstract_key_a, const void * abstract_key_b)
{
	...
	return key_a->original_font_face == key_b->original_font_face && memcmp((uchar*)(&key_a->font_matrix.xx),
		(uchar*)(&key_b->font_matrix.xx), sizeof(cairo_matrix_t)) == 0 &&
		memcmp((uchar*)(&key_a->ctm.xx), (uchar*)(&key_b->ctm.xx), sizeof(cairo_matrix_t)) == 0 &&
		cairo_font_options_equal(&key_a->options, &key_b->options);
}

static cairo_bool_t _cairo_scaled_font_matches(const cairo_scaled_font_t * scaled_font,
    const cairo_font_face_t * font_face, const cairo_matrix_t * font_matrix, const cairo_matrix_t * ctm, const cairo_font_options_t * options)
{
	return scaled_font->original_font_face == font_face && memcmp((uchar*)(&scaled_font->font_matrix.xx),
...
void FASTCALL _cairo_scaled_font_reset_cache(cairo_scaled_font_t * scaled_font)
{
	...
	while(!cairo_list_is_empty(&scaled_font->glyph_pages)) {
		cairo_scaled_glyph_page_t * page = cairo_list_first_entry(&scaled_font->glyph_pages, cairo_scaled_glyph_page_t, link);
...
cairo_status_t FASTCALL _cairo_scaled_font_set_metrics(cairo_scaled_font_t * scaled_font, cairo_font_extents_t * fs_metrics)
{
	cairo_status_t status;
...
void _cairo_scaled_font_attach_private(cairo_scaled_font_t * scaled_font,
    cairo_scaled_font_private_t * pPrivate, const void * key,
    ...
    const cairo_font_options_t * options)
{
	...
	double det;
	cairo_status_t status = font_face->status;
	if(unlikely(status))
	...
	if(scaled_font) {
		...
		if(!CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&scaled_font->ref_count)) {
			if(scaled_font->holdover) {
				for(int i = 0; i < font_map->num_holdovers; i++) {
					if(font_map->holdovers[i] == scaled_font) {
						font_map->num_holdovers--;
						memmove(&font_map->holdovers[i], &font_map->holdovers[i+1], (font_map->num_holdovers - i) * sizeof(cairo_scaled_font_t*));
						break;
					}
				}
				scaled_font->holdover = FALSE;
...
cairo_scaled_font_t * _cairo_scaled_font_create_in_error(cairo_status_t status)
{
	...
	if(unlikely(scaled_font == NULL)) {
		scaled_font = (cairo_scaled_font_t *)SAlloc::M(sizeof(cairo_scaled_font_t));
		if(unlikely(scaled_font == NULL)) {
...
{
	return (!scaled_font || CAIRO_REFERENCE_COUNT_IS_INVALID(&scaled_font->ref_count)) ? 0 : CAIRO_REFERENCE_COUNT_GET_VALUE(&scaled_font->ref_count);
}
...
{
	return (CAIRO_REFERENCE_COUNT_IS_INVALID(&scaled_font->ref_count)) ? scaled_font->status : _cairo_user_data_array_set_data(&scaled_font->user_data, key, user_data, destroy);
}
...
void cairo_scaled_font_text_extents(cairo_scaled_font_t   * scaled_font, const char  * utf8, cairo_text_extents_t  * extents)
{
	...
	if(utf8 == NULL)
		goto ZERO_EXTENTS;
	status = cairo_scaled_font_text_to_glyphs(scaled_font, 0., 0., utf8, -1, &glyphs, &num_glyphs, 0, 0, 0);
	if(unlikely(status)) {
...
void cairo_scaled_font_glyph_extents(cairo_scaled_font_t   * scaled_font, const cairo_glyph_t * glyphs, int num_glyphs, cairo_text_extents_t * extents)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(scaled_glyph->metrics.width != 0 && scaled_glyph->metrics.height != 0) {
			double left = scaled_glyph->metrics.x_bearing + glyphs[i].x;
			double right = left + scaled_glyph->metrics.width;
			double top = scaled_glyph->metrics.y_bearing + glyphs[i].y;
			double bottom = top + scaled_glyph->metrics.height;
			if(!visible) {
				visible = TRUE;
				min_x = left;
				max_x = right;
	...
	if(num_glyphs) {
		...
		double y0 = glyphs[0].y;
		/* scaled_glyph contains the glyph for num_glyphs - 1 already. */
		double x1 = glyphs[num_glyphs - 1].x + scaled_glyph->metrics.x_advance;
		double y1 = glyphs[num_glyphs - 1].y + scaled_glyph->metrics.y_advance;
...
static inline cairo_bool_t _range_contains_glyph(const cairo_box_t * extents,
    cairo_fixed_t left, cairo_fixed_t top, cairo_fixed_t right, cairo_fixed_t bottom)
{	
...
static cairo_status_t _cairo_scaled_font_single_glyph_device_extents(cairo_scaled_font_t * scaled_font,
    const cairo_glyph_t * glyph, CairoIRect * extents)
{
	cairo_scaled_glyph_t * scaled_glyph;
	cairo_status_t status;
	_cairo_scaled_font_freeze_cache(scaled_font);
...
cairo_status_t _cairo_scaled_font_glyph_device_extents(cairo_scaled_font_t * scaled_font,
    const cairo_glyph_t * glyphs, int num_glyphs, CairoIRect * extents, cairo_bool_t * overlap_out)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		cairo_fixed_t x1 = x + scaled_glyph->bbox.p1.x;
		cairo_fixed_t x2 = x + scaled_glyph->bbox.p2.x;
		cairo_fixed_t y1 = y + scaled_glyph->bbox.p1.y;
		cairo_fixed_t y2 = y + scaled_glyph->bbox.p2.y;
		if(overlap == FALSE)
...
cairo_bool_t _cairo_scaled_font_glyph_approximate_extents(cairo_scaled_font_t * scaled_font,
    const cairo_glyph_t * glyphs, int num_glyphs, CairoIRect * extents)
{
	double x0, x1, y0, y1, pad;
	int i;
	// If any of the factors are suspect (i.e. the font is broken), bail 
	if(scaled_font->fs_extents.max_x_advance == 0 || scaled_font->fs_extents.height == 0 || scaled_font->max_scale == 0) {
		return FALSE;
	}
	assert(num_glyphs);
	x0 = x1 = glyphs[0].x;
    ...
    cairo_region_t          * clip_region)
{
	...
	if(scaled_font->backend->show_glyphs != NULL) {
		...
		status = scaled_font->backend->show_glyphs(scaled_font, op, pattern, surface, source_x, source_y,
		    dest_x, dest_y, width, height, glyphs, num_glyphs, clip_region, &remaining_glyphs);
	...
	status = _cairo_surface_composite(op, pattern, &mask_pattern.base,
	    surface, source_x, source_y, 0, 0, dest_x, dest_y, width, height, clip_region);
	_cairo_pattern_fini(&mask_pattern.base);
CLEANUP_MASK:
	_cairo_scaled_font_thaw_cache(scaled_font);
	if(mask != NULL)
...
static cairo_status_t _trace_mask_to_path(cairo_image_surface_t * mask, cairo_path_fixed_t * path, double tx, double ty)
{
	...
	for(y = 0, rows = mask->height; rows--; row += mask->stride, y++) {
		...
		for(cols = bytes_per_row; cols--; ) {
			...
			byte = (int)CAIRO_BITSWAP8_IF_LITTLE_ENDIAN(byte);
    ...
    const cairo_glyph_t * glyphs, int num_glyphs, cairo_path_fixed_t * path)
{
	...
	for(i = 0; i < num_glyphs; i++) {
		...
		if(status == CAIRO_INT_STATUS_SUCCESS) {
			status = _cairo_path_fixed_append(path, scaled_glyph->path,
			    _cairo_fixed_from_double(glyphs[i].x), _cairo_fixed_from_double(glyphs[i].y));
		}
			...
			if(unlikely(status))
				goto BAIL;
			status = _trace_mask_to_path(scaled_glyph->surface, path, glyphs[i].x, glyphs[i].y);
		}
		if(unlikely(status))
			goto BAIL;
	}
 ...
 * %CAIRO_SCALED_GLYPH_INFO_METRICS.
 **/
void _cairo_scaled_glyph_set_metrics(cairo_scaled_glyph_t * scaled_glyph, const cairo_scaled_font_t * scaled_font, const cairo_text_extents_t * fs_metrics)
{
	...
	for(hm = 0.0; hm <= 1.0; hm += 1.0) {
		for(wm = 0.0; wm <= 1.0; wm += 1.0) {
			...
			const double __x = fs_metrics->x_bearing + fs_metrics->width * wm;
			const double __y = fs_metrics->y_bearing + fs_metrics->height * hm;
			double user_x = __x;
			double user_y = __y;
			double dvc_x = __x;
			double dvc_y = __y;
			cairo_matrix_transform_point(&scaled_font->font_matrix, &user_x, &user_y);
			// Transform this corner to device space from glyph origin 
			cairo_matrix_transform_distance(&scaled_font->scale, &dvc_x, &dvc_y);
			if(first) {
				min_user_x = max_user_x = user_x;
				min_user_y = max_user_y = user_y;
				min_device_x = max_device_x = dvc_x;
				min_device_y = max_device_y = dvc_y;
				first = FALSE;
			}
			else {
				SETMIN(min_user_x, user_x);
				SETMAX(max_user_x, user_x);
				SETMIN(min_user_y, user_y);
				SETMAX(max_user_y, user_y);
				SETMIN(min_device_x, dvc_x);
				SETMAX(max_device_x, dvc_x);
				SETMIN(min_device_y, dvc_y);
				SETMAX(max_device_y, dvc_y);
			}
		}
	}
...
static cairo_bool_t _cairo_scaled_glyph_page_can_remove(const void * closure)
{
	...
	const cairo_scaled_font_t * scaled_font = (cairo_scaled_font_t*)page->cache_entry.hash;
...
static cairo_status_t _cairo_scaled_font_allocate_glyph(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t ** scaled_glyph)
{
	...
	page = (cairo_scaled_glyph_page_t *)SAlloc::M(sizeof(cairo_scaled_glyph_page_t));
	...
	if(scaled_font->global_cache_frozen == FALSE) {
		if(unlikely(cairo_scaled_glyph_page_cache.hash_table == NULL)) {
			status = _cairo_cache_init(&cairo_scaled_glyph_page_cache,
			    NULL, _cairo_scaled_glyph_page_can_remove, _cairo_scaled_glyph_page_pluck,
...
static void _cairo_scaled_font_free_last_glyph(cairo_scaled_font_t * scaled_font, cairo_scaled_glyph_t * scaled_glyph)
{
	...
	cairo_scaled_glyph_page_t * page = cairo_list_last_entry(&scaled_font->glyph_pages, cairo_scaled_glyph_page_t, link);
