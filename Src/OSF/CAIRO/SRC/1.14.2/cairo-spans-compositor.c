...
static cairo_int_status_t composite_boxes(const cairo_spans_compositor_t * compositor,
    cairo_composite_rectangles_t * extents,
    cairo_boxes_t          * boxes);

static cairo_int_status_t clip_and_composite_polygon(const cairo_spans_compositor_t      * compositor,
    cairo_composite_rectangles_t * extents, cairo_polygon_t * polygon,
    CairoFillRule fill_rule, cairo_antialias_t antialias);

static cairo_surface_t * get_clip_surface(const cairo_spans_compositor_t * compositor,
    cairo_surface_t * dst,
...
static cairo_int_status_t fixup_unbounded_mask(const cairo_spans_compositor_t * compositor,
    const cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
	cairo_composite_rectangles_t composite;
	cairo_surface_t * clip;
	cairo_int_status_t status;
	TRACE((stderr, "%s\n", __FUNCTION__));
	clip = get_clip_surface(compositor, extents->surface, extents->clip, &extents->unbounded);
...
static cairo_surface_t * FASTCALL unwrap_source(const cairo_pattern_t * pattern)
{
	CairoIRect limit;
	return _cairo_pattern_get_source((cairo_surface_pattern_t*)pattern, &limit);
}

static cairo_bool_t FASTCALL is_recording_pattern(const cairo_pattern_t * pattern)
{
...
static cairo_bool_t FASTCALL recording_pattern_contains_sample(const cairo_pattern_t * pattern, const CairoIRect * sample)
{
	if(!is_recording_pattern(pattern))
		...
		cairo_recording_surface_t * surface = (cairo_recording_surface_t*)unwrap_source(pattern);
...
static cairo_bool_t FASTCALL op_reduces_to_source(const cairo_composite_rectangles_t * extents, cairo_bool_t no_mask)
{
	if(extents->op == CAIRO_OPERATOR_SOURCE)
		return TRUE;
	else if(extents->surface->is_clear)
		return oneof2(extents->op, CAIRO_OPERATOR_OVER, CAIRO_OPERATOR_ADD);
	else if(no_mask && extents->op == CAIRO_OPERATOR_OVER)
		return _cairo_pattern_is_opaque(&extents->source_pattern.base, &extents->source_sample_area);
	else
		return FALSE;
}

static cairo_status_t upload_boxes(const cairo_spans_compositor_t * compositor,
    const cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
...
static cairo_bool_t FASTCALL _clip_is_region(const cairo_clip_t * clip)
{
	if(clip->is_region)
		return TRUE;
	else if(clip->path)
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		if(!no_mask) {
			src = compositor->pattern_to_surface(dst, &extents->mask_pattern.base, TRUE,
			    &extents->bounded, &extents->mask_sample_area, &src_x, &src_y);
...
static cairo_bool_t FASTCALL composite_needs_clip(const cairo_composite_rectangles_t * composite, const cairo_box_t * extents)
{
    ...
    cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
	...
	status = compositor->renderer_init(&renderer, extents, CAIRO_ANTIALIAS_DEFAULT, FALSE);
	if(likely(status == CAIRO_INT_STATUS_SUCCESS))
    ...
    cairo_antialias_t antialias)
{
	...
	if(needs_clip) {
		TRACE((stderr, "%s: unsupported clip\n", __FUNCTION__));
		return CAIRO_INT_STATUS_UNSUPPORTED;
		converter = _cairo_clip_tor_scan_converter_create(extents->clip, polygon, fill_rule, antialias);
	}
	else {
	...
	if(unlikely(status))
		goto cleanup_converter;
	status = compositor->renderer_init(&renderer, extents, antialias, needs_clip);
	if(likely(status == CAIRO_INT_STATUS_SUCCESS))
...
static cairo_int_status_t FASTCALL trim_extents_to_boxes(cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
	cairo_box_t box;
...
static cairo_int_status_t FASTCALL trim_extents_to_polygon(cairo_composite_rectangles_t * extents, cairo_polygon_t * polygon)
{
    ...
    cairo_composite_rectangles_t * extents, cairo_boxes_t * boxes)
{
	...
	if(extents->clip->path != NULL && extents->is_bounded) {
		...
		CairoFillRule fill_rule;
		cairo_antialias_t antialias;
		cairo_clip_t * clip = _cairo_clip_copy(extents->clip);
	...
	if(unlikely(status))
		return status;
	status = composite_polygon(compositor, extents, &polygon, CAIRO_FILL_RULE_WINDING, CAIRO_ANTIALIAS_DEFAULT);
	_cairo_polygon_fini(&polygon);
	return status;
}
    ...
    cairo_antialias_t antialias)
{
	...
	if(extents->is_bounded && extents->clip->path) {
		...
		CairoFillRule clip_fill_rule;
		TRACE((stderr, "%s - combining shape with clip polygon\n", __FUNCTION__));
		status = _cairo_clip_get_polygon(extents->clip, &clipper, &clip_fill_rule, &clip_antialias);
		if(likely(status == CAIRO_INT_STATUS_SUCCESS)) {
			cairo_clip_t * old_clip;
			if(clip_antialias == antialias) {
				status = _cairo_polygon_intersect(polygon, fill_rule, &clipper, clip_fill_rule);
...
static cairo_int_status_t _cairo_spans_compositor_paint(const cairo_compositor_t * _compositor, cairo_composite_rectangles_t * extents)
{
...
static cairo_int_status_t _cairo_spans_compositor_mask(const cairo_compositor_t          * _compositor,
    cairo_composite_rectangles_t      * extents)
{
	const cairo_spans_compositor_t * compositor = (cairo_spans_compositor_t*)_compositor;
	cairo_int_status_t status;
...
static cairo_int_status_t _cairo_spans_compositor_stroke(const cairo_compositor_t        * _compositor,
    cairo_composite_rectangles_t     * extents,
    ...
    cairo_antialias_t antialias)
{
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		cairo_polygon_t polygon;
		CairoFillRule fill_rule = CAIRO_FILL_RULE_WINDING;
		...
		status = _cairo_path_fixed_stroke_to_polygon(path,
		    style, ctm, ctm_inverse, tolerance, &polygon);
