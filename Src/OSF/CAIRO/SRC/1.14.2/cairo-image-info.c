...
static const uchar *
_jpeg_skip_segment (const uchar *p)
{
    int len;
...
static void
_jpeg_extract_info (cairo_image_info_t *info, const uchar *p)
{
...
static const uchar *
_jpx_next_box (const uchar *p)
{
    return p + get_unaligned_be32 (p);
}

static const uchar *
_jpx_get_box_contents (const uchar *p)
{
    return p + 8;
}

static cairo_bool_t
_jpx_match_box (const uchar *p, const uchar *end, uint32_t type)
{
    uint32_t length;
...
static const uchar *
_jpx_find_box (const uchar *p, const uchar *end, uint32_t type)
{
    while (p < end) {
...
static void
_jpx_extract_info (const uchar *p, cairo_image_info_t *info)
{
...
static const uchar *
_jbig2_find_data_end (const uchar *p,
		      const uchar *end,
		      int                  type)
{
    uchar end_seq[2];
...
static const uchar *
_jbig2_get_next_segment (const uchar  *p,
			 const uchar  *end,
			 int                  *type,
			 const uchar **data,
			 ulong        *data_len)
...
static void
_jbig2_extract_info (cairo_image_info_t *info, const uchar *p)
{
    info->width = get_unaligned_be32 (p);
