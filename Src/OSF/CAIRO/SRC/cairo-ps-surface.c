...
extern const cairo_surface_backend_t cairo_ps_surface_backend;
extern const cairo_paginated_surface_backend_t cairo_ps_surface_paginated_backend;

static cairo_bool_t _cairo_ps_surface_get_extents(void * abstract_surface, CairoIRect * rectangle);
...
static const char * _cairo_ps_level_strings[CAIRO_PS_LEVEL_LAST] = { "PS Level 2", "PS Level 3" };
static const char * _cairo_ps_supported_mime_types[] = { CAIRO_MIME_TYPE_JPEG, NULL };
...
static void _cairo_ps_surface_emit_header(cairo_ps_surface_t * surface)
{
	//char ctime_buf[26];
	char ** comments;
	int i, num_comments;
	...
	time_t now = surface->has_creation_date ? surface->creation_date : time(NULL);
	...
	if(!cairo_list_is_empty(&surface->document_media)) {
		...
		cairo_list_foreach_entry(page, cairo_page_media_t, &surface->document_media, link) {
			if(first) {
				_cairo_output_stream_puts(surface->final_stream, "%%%%DocumentMedia: ");
				first = FALSE;
			}
			else {
				_cairo_output_stream_puts(surface->final_stream, "%%%%+ ");
			}
	...
	_cairo_output_stream_puts(surface->final_stream, "%%%%EndComments\n");
	_cairo_output_stream_puts(surface->final_stream, "%%%%BeginProlog\n");
	if(surface->eps) {
		_cairo_output_stream_puts(surface->final_stream, "save\n50 dict begin\n");
	}
	else {
		_cairo_output_stream_printf(surface->final_stream,
		    "/languagelevel where\n"
	...
	_cairo_output_stream_puts(surface->final_stream, "%%%%EndProlog\n");
	_cairo_output_stream_puts(surface->final_stream, "%%%%BeginSetup\n");
...
static cairo_status_t _cairo_ps_surface_emit_type1_font_subset(cairo_ps_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
...
#if DEBUG_PS
	_cairo_output_stream_puts(surface->final_stream, "%% _cairo_ps_surface_emit_type1_font_subset\n");
#endif
	...
	_cairo_output_stream_puts(surface->final_stream, "%%%%EndResource\n");
...
static cairo_status_t _cairo_ps_surface_emit_type1_font_fallback(cairo_ps_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
	...
	if(unlikely(status))
		return status;
#if DEBUG_PS
	_cairo_output_stream_puts(surface->final_stream, "%% _cairo_ps_surface_emit_type1_font_fallback\n");
#endif
	_cairo_output_stream_printf(surface->final_stream, "%%%%BeginResource: font %s\n", subset.base_font);
	length = subset.header_length + subset.data_length + subset.trailer_length;
	_cairo_output_stream_write(surface->final_stream, subset.data, length);
...
static cairo_status_t _cairo_ps_surface_emit_truetype_font_subset(cairo_ps_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
	...
	/* FIXME: Figure out document structure convention for fonts */
#if DEBUG_PS
	_cairo_output_stream_puts(surface->final_stream, "%% _cairo_ps_surface_emit_truetype_font_subset\n");
#endif
	_cairo_output_stream_printf(surface->final_stream, "%%%%BeginResource: font %s\n", subset.ps_name);
	_cairo_output_stream_printf(surface->final_stream,
	    "11 dict begin\n"
	...
	_cairo_output_stream_puts(surface->final_stream, "end readonly def\n");
	_cairo_output_stream_puts(surface->final_stream, "/sfnts [\n");
	begin = 0;
	end = 0;
	for(i = 0; i < subset.num_string_offsets; i++) {
		...
		_cairo_output_stream_puts(surface->final_stream, "00>\n");
		begin = end;
	}
...
static cairo_int_status_t _cairo_ps_emit_imagemask(cairo_image_surface_t * image, cairo_output_stream_t * stream)
{
	uint8 * row, * byte;
	int rows, cols;
	...
	_cairo_output_stream_printf(stream,
	    ...
	    -image->height,
	    image->height);
	_cairo_output_stream_puts(stream, "   /DataSource {<\n   ");
	...
	_cairo_output_stream_puts(stream, ">}\n>>\n");
	_cairo_output_stream_puts(stream, "imagemask\n");
...
static cairo_status_t _cairo_ps_surface_emit_type3_font_subset(cairo_ps_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
	...
	if(font_subset->num_glyphs == 0)
		return CAIRO_STATUS_SUCCESS;
#if DEBUG_PS
	_cairo_output_stream_puts(surface->final_stream, "%% _cairo_ps_surface_emit_type3_font_subset\n");
#endif
	_cairo_output_stream_puts(surface->final_stream, "%%%%BeginResource: font\n");
	_cairo_output_stream_puts(surface->final_stream, "8 dict begin\n/FontType 3 def\n/FontMatrix [1 0 0 1 0 0] def\n/Encoding 256 array def\n0 1 255 { Encoding exch /.notdef put } for\n");
	...
	_cairo_output_stream_puts(surface->final_stream, "/Glyphs [\n");
	for(i = 0; i < font_subset->num_glyphs; i++) {
...
static cairo_int_status_t _cairo_ps_surface_emit_unscaled_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)closure;
	cairo_int_status_t status = _cairo_scaled_font_subset_create_glyph_names(font_subset);
...
static cairo_int_status_t _cairo_ps_surface_emit_scaled_font_subset(cairo_scaled_font_subset_t * font_subset,
    void * closure)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)closure;
	cairo_int_status_t status = _cairo_scaled_font_subset_create_glyph_names(font_subset);
...
static cairo_status_t _cairo_ps_surface_emit_font_subsets(cairo_ps_surface_t * surface)
{
	cairo_status_t status;
#if DEBUG_PS
	_cairo_output_stream_puts(surface->final_stream, "%% _cairo_ps_surface_emit_font_subsets\n");
#endif
	status = _cairo_scaled_font_subsets_foreach_user(surface->font_subsets, _cairo_ps_surface_analyze_user_font_subset, surface);
	if(unlikely(status))
		return status;
	status = _cairo_scaled_font_subsets_foreach_unscaled(surface->font_subsets, _cairo_ps_surface_emit_unscaled_font_subset, surface);
	if(unlikely(status))
		return status;
	status = _cairo_scaled_font_subsets_foreach_scaled(surface->font_subsets, _cairo_ps_surface_emit_scaled_font_subset, surface);
	if(unlikely(status))
		return status;
	return _cairo_scaled_font_subsets_foreach_user(surface->font_subsets, _cairo_ps_surface_emit_scaled_font_subset, surface);
}

static cairo_status_t _cairo_ps_surface_emit_body(cairo_ps_surface_t * surface)
{
	char buf[4096];
	int n;
...
{
	_cairo_output_stream_puts(surface->final_stream, "%%%%Trailer\n");
	if(surface->eps) {
		_cairo_output_stream_puts(surface->final_stream, "end restore\n");
	}
	_cairo_output_stream_puts(surface->final_stream, "%%%%EOF\n");
}

static cairo_bool_t _path_covers_bbox(cairo_ps_surface_t * surface, cairo_path_fixed_t * path)
{
	cairo_box_t box;
	if(_cairo_path_fixed_is_box(path, &box)) {
		...
		if(_cairo_rectangle_intersect(&rect, &surface->page_bbox)) {
			if(rect.x == surface->page_bbox.x && rect.width == surface->page_bbox.width &&
			    rect.y == surface->page_bbox.y && rect.height == surface->page_bbox.height) {
				return TRUE;
			}
		}
	}
...
#if DEBUG_PS
	_cairo_output_stream_puts(stream, "%% _cairo_ps_surface_intersect_clip_path\n");
#endif
	if(path == NULL) {
		status = _cairo_pdf_operators_flush(&surface->pdf_operators);
		if(unlikely(status))
			return status;
		_cairo_output_stream_puts(stream, "Q q\n");
...
static cairo_bool_t _ps_page_dimension_equal(int a, int b)
{
	return (abs(a - b) < 5);
}

static const char * _cairo_ps_surface_get_page_media(cairo_ps_surface_t * surface)
{
	int i;
	char buf[50];
	cairo_page_media_t * page;
	const char * page_name;
	int width = _cairo_lround(surface->width);
	int height = _cairo_lround(surface->height);
	/* search previously used page sizes */
	cairo_list_foreach_entry(page, cairo_page_media_t, &surface->document_media, link) {
		if(_ps_page_dimension_equal(width, page->width) && _ps_page_dimension_equal(height, page->height))
			return page->name;
	}
...
static cairo_surface_t * _cairo_ps_surface_create_for_stream_internal(cairo_output_stream_t * stream, double width, double height)
{
	...
	if(surface->tmpfile == NULL) {
		switch(errno) {
			...
			default: status = _cairo_error(CAIRO_STATUS_TEMP_FILE_ERROR); break;
		}
...
void cairo_ps_surface_set_size(cairo_surface_t * surface, double width_in_points, double height_in_points)
{
	...
	_cairo_pdf_operators_set_cairo_to_pdf_matrix(&ps_surface->pdf_operators, &ps_surface->cairo_to_ps);
	status = _cairo_paginated_surface_set_size(ps_surface->paginated_surface, (int)width_in_points, (int)height_in_points);
	if(status)
...
static cairo_status_t _cairo_ps_surface_finish(void * abstract_surface)
{
	...
	if(unlikely(status))
		goto CLEANUP;
	_cairo_output_stream_puts(surface->final_stream, "%%%%EndSetup\n");
...
static cairo_int_status_t _cairo_ps_surface_start_page(void * abstract_surface)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)abstract_surface;
...
static cairo_int_status_t _cairo_ps_surface_show_page(void * abstract_surface)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)abstract_surface;
 ...
 **/
static cairo_status_t _cairo_ps_surface_acquire_source_surface_from_pattern(cairo_ps_surface_t * surface,
 ...
 **/
static cairo_status_t _cairo_ps_surface_create_padded_image_from_image(cairo_ps_surface_t * surface,
...
static cairo_int_status_t _cairo_ps_surface_analyze_surface_pattern_transparency(cairo_ps_surface_t * surface,
    const cairo_pattern_t * pattern,
    const CairoIRect * extents)
{
	int width, height;
...
static cairo_bool_t _gradient_pattern_supported(cairo_ps_surface_t * surface,
    const cairo_pattern_t * pattern)
{
...
static cairo_bool_t pattern_supported(cairo_ps_surface_t * surface, const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		    return TRUE;
...
static cairo_bool_t mask_supported(cairo_ps_surface_t * surface,
...
static cairo_int_status_t _cairo_ps_surface_analyze_operation(cairo_ps_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * pattern,
    const cairo_pattern_t * mask,
    const CairoIRect * extents)
...
static cairo_bool_t _cairo_ps_surface_operation_supported(cairo_ps_surface_t * surface,
    cairo_operator_t op,
    const cairo_pattern_t * pattern,
    const cairo_pattern_t * mask,
    const CairoIRect * extents)
...
static cairo_status_t _string_array_stream_write(cairo_output_stream_t * base, const uchar * data, uint length)
{
	...
	while(length--) {
		...
		if(stream->use_strings && stream->string_size >= STRING_ARRAY_MAX_STRING_SIZE) {
			_cairo_output_stream_puts(stream->output, ")\n");
...
static cairo_status_t _string_array_stream_close(cairo_output_stream_t * base)
{
	cairo_status_t status;
	string_array_stream_t * stream = (string_array_stream_t*)base;
	if(stream->use_strings)
...
static cairo_output_stream_t * _string_array_stream_create(cairo_output_stream_t * output)
{
	...
	_cairo_output_stream_init(&stream->base, _string_array_stream_write, NULL, _string_array_stream_close);
...
static cairo_status_t _cairo_ps_surface_flatten_image_transparency(cairo_ps_surface_t * surface,
    cairo_image_surface_t * image,
    ...
    cairo_bool_t use_strings)
{
	...
	_cairo_output_stream_puts(string_array_stream, "~>");
	status2 = _cairo_output_stream_destroy(string_array_stream);
	if(status == CAIRO_STATUS_SUCCESS)
		status = status2;
	return status;
    ...
    cairo_bool_t stencil_mask)
{
	...
	if(surface->use_string_datasource) {
		...
		_cairo_output_stream_puts(surface->stream, "/CairoImageData [\n");
		status = _cairo_ps_surface_emit_base85_string(surface, data, data_size, compress, TRUE);
		if(unlikely(status))
			goto bail2;
		_cairo_output_stream_puts(surface->stream, "] def\n");
		_cairo_output_stream_puts(surface->stream, "/CairoImageDataIndex 0 def\n");
	}
	...
	if(use_mask) {
		...
		if(surface->use_string_datasource) {
			...
			_cairo_output_stream_printf(surface->stream, "    /DataSource currentfile /ASCII85Decode filter /%s filter def\n", compress_filter);
		}
		_cairo_output_stream_printf(surface->stream,
	...
	if(!surface->use_string_datasource) {
		...
		status = _cairo_ps_surface_emit_base85_string(surface, data, data_size, compress, FALSE);
		_cairo_output_stream_puts(surface->stream, "\n");
	}
	else {
		status = CAIRO_STATUS_SUCCESS;
	}
...
static cairo_status_t _cairo_ps_surface_emit_jpeg_image(cairo_ps_surface_t * surface,
    cairo_surface_t * source, int width, int height)
{
	cairo_status_t status;
	...
	switch(info.num_components) {
		case 1: colorspace = "/DeviceGray"; decode = "0 1"; break;
		case 3: colorspace = "/DeviceRGB"; decode =  "0 1 0 1 0 1"; break;
		case 4: colorspace = "/DeviceCMYK"; decode =  "0 1 0 1 0 1 0 1"; break;
	...
	_cairo_output_stream_printf(surface->stream,
	    ...
	    "  /BitsPerComponent %d def\n"
	    "  /Decode [ %s ] def\n",
	    colorspace, info.width, info.height, info.bits_per_component, decode);
	if(surface->use_string_datasource) {
		_cairo_output_stream_puts(surface->stream,
		    "  /DataSource {\n"
		    "    CairoImageData CairoImageDataIndex get\n"
		...
		_cairo_output_stream_puts(surface->stream, "  /DataSource currentfile /ASCII85Decode filter /DCTDecode filter def\n");
	}
	_cairo_output_stream_printf(surface->stream, "  /ImageMatrix [ 1 0 0 -1 0 %d ] def\nend\nimage\n", info.height);
	if(!surface->use_string_datasource) {
		/* Emit the image data as a base85-encoded string which will
		 * be used as the data source for the image operator. */
		status = _cairo_ps_surface_emit_base85_string(surface, mime_data, mime_data_length, CAIRO_PS_COMPRESS_NONE, FALSE);
	}
	return status;
}

static cairo_status_t _cairo_ps_surface_emit_recording_surface(cairo_ps_surface_t * surface, cairo_surface_t * recording_surface)
{
	cairo_surface_t * free_me = NULL;
	cairo_box_t bbox;
	cairo_int_status_t status;
	cairo_content_t old_content = surface->content;
	double old_width = surface->width;
	double old_height = surface->height;
	CairoIRect old_page_bbox = surface->page_bbox;
	cairo_matrix_t old_cairo_to_ps = surface->cairo_to_ps;
	cairo_surface_clipper_t old_clipper = surface->clipper;
	_cairo_surface_clipper_init(&surface->clipper, _cairo_ps_surface_clipper_intersect_clip_path);
	...
	_cairo_pdf_operators_set_cairo_to_pdf_matrix(&surface->pdf_operators, &surface->cairo_to_ps);
	_cairo_output_stream_puts(surface->stream, "  q\n");
	if(recording_surface->content == CAIRO_CONTENT_COLOR) {
		surface->content = CAIRO_CONTENT_COLOR;
		_cairo_output_stream_printf(surface->stream, "  0 g %d %d %d %d rectfill\n",
		    surface->page_bbox.x, surface->page_bbox.y, surface->page_bbox.width, surface->page_bbox.height);
	}
	status = _cairo_recording_surface_replay_region(recording_surface, NULL, &surface->base, CAIRO_RECORDING_REGION_NATIVE);
...
static cairo_int_status_t _cairo_ps_surface_emit_recording_subsurface(cairo_ps_surface_t * surface,
    cairo_surface_t * recording_surface, const CairoIRect * extents)
{
	cairo_surface_t * free_me = NULL;
	cairo_int_status_t status;
	cairo_content_t old_content = surface->content;
	...
	status = _cairo_recording_surface_replay_region(recording_surface, extents, &surface->base, CAIRO_RECORDING_REGION_NATIVE);
	assert(status != CAIRO_INT_STATUS_UNSUPPORTED);
	if(unlikely(status))
		goto err;
...
static void _cairo_ps_surface_flatten_transparency(cairo_ps_surface_t * surface,
...
static void _cairo_ps_surface_emit_solid_pattern(cairo_ps_surface_t * surface, cairo_solid_pattern_t * pattern)
{
	double red, green, blue;
	_cairo_ps_surface_flatten_transparency(surface, &pattern->color, &red, &green, &blue);
	if(color_is_gray(red, green, blue))
		_cairo_output_stream_printf(surface->stream, "%f g\n", red);
	else
		_cairo_output_stream_printf(surface->stream, "%f %f %f rg\n", red, green, blue);
}

static cairo_status_t _cairo_ps_surface_emit_surface(cairo_ps_surface_t * surface,
...
static void _path_fixed_init_rectangle(cairo_path_fixed_t * path,
    CairoIRect * rect)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_ps_surface_paint_surface(cairo_ps_surface_t * surface,
    ...
    cairo_bool_t stencil_mask)
{
	...
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_FALLBACK) {
		...
		_cairo_output_stream_printf(surface->stream, "%% Fallback Image: x=%f y=%f w=%d h=%d ",
		    -cairo_p2d.x0/x_scale, -cairo_p2d.y0/y_scale, (int)(width/x_scale), (int)(height/y_scale));
		if(x_scale == y_scale) {
			_cairo_output_stream_printf(surface->stream, "res=%fppi ", x_scale*72);
		}
		else {
		...
		if(op == CAIRO_OPERATOR_SOURCE) {
			_cairo_output_stream_printf(surface->stream, "%d g 0 0 %f %f rectfill\n",
			    surface->content == CAIRO_CONTENT_COLOR ? 0 : 1, surface->width, surface->height);
		}
	}
	...
	if(!_cairo_matrix_is_identity(&ps_p2d)) {
		...
		_cairo_output_stream_puts(surface->stream, " ] concat\n");
	}
	status = _cairo_ps_surface_emit_surface(surface, pattern, image ? &image->base : source, op, width, height, stencil_mask);
release_source:
	if(image)
		cairo_surface_destroy(&image->base);
	_cairo_ps_surface_release_source_surface_from_pattern(surface, pattern, source, image_extra);
...
static cairo_status_t _cairo_ps_surface_emit_surface_pattern(cairo_ps_surface_t * surface, cairo_pattern_t * pattern, CairoIRect * extents, cairo_operator_t op)
{
	...
	/* cairo_pattern_set_matrix ensures the matrix is invertible */
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_ps_surface_acquire_source_surface_from_pattern(surface, pattern, extents,
	    &pattern_width, &pattern_height, &x_offset, &y_offset, &source, &image_extra);
	if(unlikely(status))
		return status;
	if(pattern->extend == CAIRO_EXTEND_PAD) {
		cairo_image_surface_t * img;
		...
		img = (cairo_image_surface_t*)source;
		status = _cairo_ps_surface_create_padded_image_from_image(surface, img, &pattern->matrix,
		    extents, &pattern_width, &pattern_height, &x_offset, &y_offset, &image);
		if(unlikely(status))
			goto release_source;
	}
	if(unlikely(status))
	...
	switch(pattern->extend) {
		 ...
		 * care if it's not as tight as possible.*/
		    xstep = ystep = ceil((x2 - x1) + (y2 - y1) + pattern_width + pattern_height);
		    break;
	    }
	...
	_cairo_output_stream_puts(surface->stream, "/CairoPattern {\n");
	old_use_string_datasource = surface->use_string_datasource;
	surface->use_string_datasource = TRUE;
	if(op == CAIRO_OPERATOR_SOURCE) {
		_cairo_output_stream_printf(surface->stream, "%d g 0 0 %f %f rectfill\n", surface->content == CAIRO_CONTENT_COLOR ? 0 : 1, xstep, ystep);
	}
	status = _cairo_ps_surface_emit_surface(surface, pattern, image ? &image->base : source, op, pattern_width, pattern_height, FALSE);
	if(unlikely(status))
		goto release_source;
	surface->use_string_datasource = old_use_string_datasource;
	_cairo_output_stream_puts(surface->stream, "} bind def\n");
	_cairo_output_stream_puts(surface->stream, "<< /PatternType 1\n   /PaintType 1\n   /TilingType 1\n");
	...
	if(pattern->extend == CAIRO_EXTEND_REFLECT) {
		...
		_cairo_output_stream_puts(surface->stream, "   /PaintProc { pop CairoPattern }\n");
	}
	_cairo_output_stream_puts(surface->stream, ">>\n");
	cairo_p2d = pattern->matrix;
	status = cairo_matrix_invert(&cairo_p2d);
...
static void _cairo_ps_surface_emit_linear_colorgradient(cairo_ps_surface_t * surface, cairo_ps_color_stop_t * stop1, cairo_ps_color_stop_t * stop2)
{
	_cairo_output_stream_printf(surface->stream,
	    "   << /FunctionType 2\n"
	    "      /Domain [ 0 1 ]\n"
...
static void _cairo_ps_surface_emit_stitched_colorgradient(cairo_ps_surface_t * surface, uint n_stops, cairo_ps_color_stop_t stops[])
{
	uint i;
	_cairo_output_stream_puts(surface->stream, "<< /FunctionType 3\n   /Domain [ 0 1 ]\n   /Functions [\n");
	for(i = 0; i < n_stops - 1; i++)
		_cairo_ps_surface_emit_linear_colorgradient(surface, &stops[i], &stops[i+1]);
	_cairo_output_stream_puts(surface->stream, "   ]\n");
	_cairo_output_stream_puts(surface->stream, "   /Bounds [ ");
...
static void calc_gradient_color(cairo_ps_color_stop_t * new_stop, cairo_ps_color_stop_t * stop1, cairo_ps_color_stop_t * stop2)
{
...
static cairo_status_t _cairo_ps_surface_emit_pattern_stops(cairo_ps_surface_t * surface, cairo_gradient_pattern_t * pattern)
{
	...
	for(i = 0; i < n_stops; i++) {
		double red, green, blue;
		cairo_color_t color;
		_cairo_color_init_rgba(&color, stops[i].color[0], stops[i].color[1], stops[i].color[2], stops[i].color[3]);
		_cairo_ps_surface_flatten_transparency(surface, &color, &red, &green, &blue);
		stops[i].color[0] = red;
		stops[i].color[1] = green;
		stops[i].color[2] = blue;
	}
	_cairo_output_stream_puts(surface->stream, "/CairoFunction\n");
...
static cairo_status_t _cairo_ps_surface_emit_repeating_function(cairo_ps_surface_t * surface,
    cairo_gradient_pattern_t * pattern, int begin, int end)
{
	_cairo_output_stream_printf(surface->stream,
	    "/CairoFunction\n"
	    "<< /FunctionType 3\n"
	    ...
	    "   /Bounds [ %d 1 %d {} for ]\n",
	    begin, end, end - begin, begin + 1, end - 1);
	if(pattern->base.extend == CAIRO_EXTEND_REFLECT) {
...
static cairo_status_t _cairo_ps_surface_emit_gradient(cairo_ps_surface_t * surface, cairo_gradient_pattern_t * pattern, cairo_bool_t is_ps_pattern)
{
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		int repeat_begin = (int)floor(domain[0]);
		int repeat_end = (int)ceil(domain[1]);
		status = _cairo_ps_surface_emit_repeating_function(surface, pattern, repeat_begin, repeat_end);
		if(unlikely(status))
			return status;
	}
	...
	if(is_ps_pattern) {
		_cairo_output_stream_puts(surface->stream, "<< /PatternType 2\n   /Shading\n");
	}
	...
	if(pattern->base.extend != CAIRO_EXTEND_NONE) {
		_cairo_output_stream_puts(surface->stream, "      /Extend [ true true ]\n");
	}
	else {
		_cairo_output_stream_puts(surface->stream, "      /Extend [ false false ]\n");
	}

	if(domain[0] == 0.0 && domain[1] == 1.0) {
		_cairo_output_stream_puts(surface->stream, "      /Function CairoFunction\n");
	}
	else {
		_cairo_output_stream_printf(surface->stream,
		    "      /Function <<\n"
	...
	if(is_ps_pattern) {
		...
		_cairo_output_stream_puts(surface->stream, "shfill\n");
	}
	return status;
}
    ...
    cairo_bool_t is_ps_pattern)
{
	...
	_cairo_output_stream_printf(surface->stream,
	    ...
	    "      /BitsPerFlag %d\n"
	    "      /Decode [",
	    shading.shading_type, shading.bits_per_coordinate, shading.bits_per_component, shading.bits_per_flag);
	...
	_cairo_output_stream_puts(surface->stream, "currentdict /CairoData undef\n");
	_cairo_pdf_shading_fini(&shading);
	return status;
}

static cairo_status_t _cairo_ps_surface_emit_pattern(cairo_ps_surface_t * surface, const cairo_pattern_t * pattern, CairoIRect * extents, cairo_operator_t op)
{
	cairo_status_t status;
	if(pattern->type == CAIRO_PATTERN_TYPE_SOLID) {
		cairo_solid_pattern_t * solid = (cairo_solid_pattern_t*)pattern;
		if(!surface->current_pattern_is_solid_color || !_cairo_color_equal(&surface->current_color, &solid->color)) {
			status = _cairo_pdf_operators_flush(&surface->pdf_operators);
			if(unlikely(status))
				return status;
	...
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_SURFACE:
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
		    status = _cairo_ps_surface_emit_surface_pattern(surface, (cairo_pattern_t*)pattern, extents, op);
		    if(unlikely(status))
			    return status;
		    break;
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL:
		    status = _cairo_ps_surface_emit_gradient(surface, (cairo_gradient_pattern_t*)pattern, TRUE);
		    if(unlikely(status))
			    return status;
		    break;
		case CAIRO_PATTERN_TYPE_MESH:
...
static cairo_status_t _cairo_ps_surface_paint_gradient(cairo_ps_surface_t * surface, const cairo_pattern_t * source, const CairoIRect * extents)
{
	...
	if(source->type == CAIRO_PATTERN_TYPE_MESH) {
		status = _cairo_ps_surface_emit_mesh_pattern(surface, (cairo_mesh_pattern_t*)source, FALSE);
		if(unlikely(status))
			return status;
	}
	else {
...
static cairo_status_t _cairo_ps_surface_paint_pattern(cairo_ps_surface_t * surface,
    const cairo_pattern_t * source, CairoIRect * extents, cairo_operator_t op, cairo_bool_t stencil_mask)
{
	switch(source->type) {
		case CAIRO_PATTERN_TYPE_SURFACE:
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: return _cairo_ps_surface_paint_surface(surface, (cairo_pattern_t*)source, extents, op, stencil_mask);
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL:
		case CAIRO_PATTERN_TYPE_MESH: return _cairo_ps_surface_paint_gradient(surface, source, extents);
		case CAIRO_PATTERN_TYPE_SOLID:
		default:
		    ASSERT_NOT_REACHED;
		    return CAIRO_STATUS_SUCCESS;
...
static cairo_bool_t _can_paint_pattern(const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_SURFACE:
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: return (pattern->extend == CAIRO_EXTEND_NONE || pattern->extend == CAIRO_EXTEND_PAD);
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL:
		case CAIRO_PATTERN_TYPE_MESH: return TRUE;
		default: 
			ASSERT_NOT_REACHED;
		    return FALSE;
	}
...
static void _cairo_ps_surface_get_font_options(void * abstract_surface, cairo_font_options_t * options)
{
...
static cairo_int_status_t _cairo_ps_surface_set_clip(cairo_ps_surface_t * surface, cairo_composite_rectangles_t * composite)
{
...
static cairo_int_status_t _cairo_ps_surface_paint(void * abstract_surface,
    cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
	...
	cairo_status_t status;
	status = _cairo_composite_rectangles_init_for_paint(&extents, &surface->base, op, source, clip);
	if(unlikely(status))
		return status;
...
#if DEBUG_PS
	_cairo_output_stream_puts(stream, "%% _cairo_ps_surface_paint\n");
#endif
	...
	if(_can_paint_pattern(source)) {
		...
		if(unlikely(status))
			goto cleanup_composite;
		_cairo_output_stream_puts(stream, "q\n");
		status = _cairo_ps_surface_paint_pattern(surface, source, &extents.bounded, op, FALSE);
		if(unlikely(status))
			goto cleanup_composite;
		_cairo_output_stream_puts(stream, "Q\n");
	}
...
static cairo_int_status_t _cairo_ps_surface_mask(void * abstract_surface,
    cairo_operator_t op,
    ...
    const cairo_clip_t * clip)
{
...
#if DEBUG_PS
	_cairo_output_stream_puts(stream, "%% _cairo_ps_surface_mask\n");
#endif
	status = _cairo_ps_surface_set_clip(surface, &extents);
	if(unlikely(status))
		goto cleanup_composite;
	...
	_cairo_output_stream_puts(stream, "q\n");
	status = _cairo_ps_surface_paint_pattern(surface, mask, &extents.bounded, op, TRUE);
	if(unlikely(status))
		goto cleanup_composite;
	_cairo_output_stream_puts(stream, "Q\n");
cleanup_composite:
    ...
    const cairo_clip_t * clip)
{
	...
	/* use the more accurate extents */
	{
		CairoIRect r;
		cairo_box_t b;
		status = _cairo_path_fixed_stroke_extents(path, style, ctm, ctm_inverse, tolerance, &r);
		if(unlikely(status))
			goto cleanup_composite;
	...
	assert(_cairo_ps_surface_operation_supported(surface, op, source, NULL, &extents.bounded));
#if DEBUG_PS
	_cairo_output_stream_puts(surface->stream, "%% _cairo_ps_surface_stroke\n");
#endif
	status = _cairo_ps_surface_set_clip(surface, &extents);
	if(unlikely(status))
		goto cleanup_composite;
...
static cairo_int_status_t _cairo_ps_surface_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const cairo_path_fixed_t* path,
    ...
    const cairo_clip_t * clip)
{
	...
	assert(_cairo_ps_surface_operation_supported(surface, op, source, NULL, &extents.bounded));
#if DEBUG_PS
	_cairo_output_stream_puts(surface->stream, "%% _cairo_ps_surface_fill\n");
#endif
	status = _cairo_pdf_operators_flush(&surface->pdf_operators);
	if(unlikely(status))
		goto cleanup_composite;
...
static cairo_bool_t _cairo_ps_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}

static cairo_int_status_t _cairo_ps_surface_show_text_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const char * utf8,
    int utf8_len,
    ...
    const cairo_clip_t * clip)
{
	...
	cairo_status_t status = _cairo_composite_rectangles_init_for_glyphs(&extents, &surface->base,
	    op, source, scaled_font, glyphs, num_glyphs, clip, &overlap);
	if(unlikely(status))
		return status;
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
		status = _cairo_ps_surface_analyze_operation(surface, op, source, NULL, &extents.bounded);
	...
	assert(_cairo_ps_surface_operation_supported(surface, op, source, NULL, &extents.bounded));
#if DEBUG_PS
	_cairo_output_stream_puts(surface->stream, "%% _cairo_ps_surface_show_glyphs\n");
#endif
	status = _cairo_ps_surface_set_clip(surface, &extents);
	if(unlikely(status))
		goto cleanup_composite;
...
static const char ** _cairo_ps_surface_get_supported_mime_types(void * abstract_surface)
{
	return _cairo_ps_supported_mime_types;
}

static void _cairo_ps_surface_set_paginated_mode(void * abstract_surface, cairo_paginated_mode_t paginated_mode)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)abstract_surface;
	cairo_status_t status;
...
static cairo_int_status_t _cairo_ps_surface_set_bounding_box(void * abstract_surface, cairo_box_t * bbox)
{
	cairo_ps_surface_t * surface = (cairo_ps_surface_t *)abstract_surface;
	int i, num_comments;
	char ** comments;
	...
	int x1 = (int)floor(_cairo_fixed_to_double(bbox->p1.x));
	int y1 = (int)floor(surface->height - _cairo_fixed_to_double(bbox->p2.y));
	int x2 = (int)ceil(_cairo_fixed_to_double(bbox->p2.x));
	int y2 = (int)ceil(surface->height - _cairo_fixed_to_double(bbox->p1.y));
	surface->page_bbox.x = x1;
	surface->page_bbox.y = y1;
	surface->page_bbox.width  = x2 - x1;
	surface->page_bbox.height = y2 - y1;
	_cairo_output_stream_printf(surface->stream, "%%%%Page: %d %d\n", surface->num_pages, surface->num_pages);
	_cairo_output_stream_puts(surface->stream, "%%%%BeginPageSetup\n");
	...
	if(!has_page_media && !surface->eps) {
		...
		if(unlikely(page_media == NULL))
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
		_cairo_output_stream_printf(surface->stream, "%%%%PageMedia: %s\n", page_media);
	}
	if(!has_page_bbox) {
		_cairo_output_stream_printf(surface->stream, "%%%%PageBoundingBox: %d %d %d %d\n", x1, y1, x2, y2);
	}
	if(!surface->eps) {
		_cairo_output_stream_printf(surface->stream, "%f %f cairo_set_page_size\n", ceil(surface->width), ceil(surface->height));
	}
	_cairo_output_stream_printf(surface->stream, "%%%%EndPageSetup\nq %d %d %d %d rectclip q\n",
	    surface->page_bbox.x, surface->page_bbox.y, surface->page_bbox.width, surface->page_bbox.height);
...
static cairo_bool_t _cairo_ps_surface_supports_fine_grained_fallbacks(void * abstract_surface)
{
	return TRUE;
}
