...
#if CAIRO_HAS_XCB_DRM_FUNCTIONS
static void _cairo_xcb_connection_query_dri2(cairo_xcb_connection_t * connection)
{
	xcb_connection_t * c = connection->xcb_connection;
...
static cairo_bool_t _xrender_formats_equal(const void * A, const void * B)
{
...
static void _cairo_xcb_return_socket(void * closure)
{
...
cairo_status_t _cairo_xcb_connection_take_socket(cairo_xcb_connection_t * connection)
{
	assert(CAIRO_MUTEX_IS_LOCKED(connection->device.mutex));
...
#if 0
void cairo_xcb_device_debug_cap_xcairo_version(cairo_device_t * device,
	int major_version,
	int minor_version)
{
	cairo_xcb_connection_t * connection = (cairo_xcb_connection_t *)device;
