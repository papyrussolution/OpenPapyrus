...
static cairo_surface_t * _cairo_beos_surface_create_internal(BView * view, BBitmap * bmp, bool owns_bitmap_view = false);

static inline BRect _cairo_rectangle_to_brect(const cairo_rectangle_int_t * rect)
{
...
static inline cairo_rectangle_int_t _brect_to_cairo_rectangle(const BRect & rect)
{
...
static inline rgb_color _cairo_color_to_be_color(const cairo_color_t * color)
{
 ...
 **/
static ViewCopyStatus _cairo_beos_view_to_bitmap(BView * view,
	BBitmap ** bitmap,
	BRect * rect = NULL,
...
static void unpremultiply_bgra(unsigned char * data,
	int width,
	int height,
	int stride,
...
static unsigned char * premultiply_bgra(unsigned char * data,
	int width,
	int height,
	int stride)
{
...
static cairo_int_status_t _cairo_beos_surface_set_clip_region(cairo_beos_surface_t * surface,
 ...
 * the surface.
 **/
static cairo_image_surface_t * _cairo_beos_bitmap_to_surface(BBitmap * bitmap)
{
 ...
 * delete.
 **/
static BBitmap * _cairo_image_surface_to_bitmap(cairo_image_surface_t * surface)
{
 ...
 **/
static bool _cairo_op_to_be_op(cairo_operator_t cairo_op,
...
static cairo_surface_t * _cairo_beos_surface_create_similar(void * abstract_surface,
...
static cairo_status_t _cairo_beos_surface_finish(void * abstract_surface)
{
	cairo_beos_surface_t * surface = reinterpret_cast <cairo_beos_surface_t *>(
	        abstract_surface);
...
static cairo_status_t _cairo_beos_surface_acquire_source_image(void * abstract_surface,
...
static void _cairo_beos_surface_release_source_image(void * abstract_surface,
...
static cairo_status_t _cairo_beos_surface_acquire_dest_image(void * abstract_surface,
...
static void _cairo_beos_surface_release_dest_image(void * abstract_surface,
...
static cairo_int_status_t _cairo_beos_surface_composite(cairo_operator_t op,
	cairo_pattern_t * src,
...
static cairo_int_status_t _cairo_beos_surface_fill_rectangles(void * abstract_surface,
	cairo_operator_t op,
...
static cairo_bool_t _cairo_beos_surface_get_extents(void * abstract_surface,
	cairo_rectangle_int_t * rectangle)
{
	cairo_beos_surface_t * surface = reinterpret_cast <cairo_beos_surface_t *>(abstract_surface);
	AutoLockView locker(surface->view);
	if(!locker)
		return FALSE;
...
static cairo_surface_t * _cairo_beos_surface_create_internal(BView * view, BBitmap * bmp, bool owns_bitmap_view)
{
	// Must use malloc, because cairo code will use free() on the surface
	cairo_beos_surface_t * surface = static_cast <cairo_beos_surface_t *>(malloc(sizeof(cairo_beos_surface_t)));
	if(surface == NULL) {
	...
	_cairo_surface_init(&surface->base, &cairo_beos_surface_backend, NULL, /* device */ content);
	surface->view = view;
	surface->bitmap = bmp;
 ...
 **/
cairo_surface_t * cairo_beos_surface_create(BView * view)
{
 ...
 **/
cairo_surface_t * cairo_beos_surface_create_for_bitmap(BView * view, BBitmap * bmp)
{
