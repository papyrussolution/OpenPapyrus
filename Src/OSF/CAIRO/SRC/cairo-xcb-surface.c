 ...
 *
 * Contributor(s): Behdad Esfahbod <behdad@behdad.org> Carl D. Worth <cworth@cworth.org>
...
static cairo_status_t _cairo_xcb_surface_finish(void * abstract_surface)
{
...
static cairo_surface_t * _get_shm_image(cairo_xcb_surface_t * surface,
    int x, int y,
    int width, int height)
{
#if CAIRO_HAS_XCB_SHM_FUNCTIONS
...
static cairo_surface_t * _cairo_xcb_surface_source(void * abstract_surface,
    CairoIRect * extents)
{
	cairo_xcb_surface_t * surface = abstract_surface;
...
static cairo_status_t _cairo_xcb_surface_acquire_source_image(void * abstract_surface,
...
static void _cairo_xcb_surface_release_source_image(void * abstract_surface,
    cairo_image_surface_t * image,
    void * image_extra)
{
...
static void _cairo_xcb_surface_get_font_options(void * abstract_surface,
...
static cairo_status_t _put_shm_image(cairo_xcb_surface_t    * surface,
    xcb_gcontext_t gc,
    cairo_image_surface_t  * image)
{
#if CAIRO_HAS_XCB_SHM_FUNCTIONS
...
static cairo_status_t _put_image(cairo_xcb_surface_t    * surface,
    cairo_image_surface_t  * image)
{
...
static cairo_int_status_t _put_shm_image_boxes(cairo_xcb_surface_t * surface, cairo_image_surface_t * image, xcb_gcontext_t gc, cairo_boxes_t * boxes)
{
#if CAIRO_HAS_XCB_SHM_FUNCTIONS
	cairo_xcb_shm_info_t * shm_info = _cairo_user_data_array_get_data(&image->base.user_data, (const cairo_user_data_key_t*)surface->connection);
	if(shm_info != NULL) {
	...
	} // @v1.14.6 @fix (Скобка была над return)
#endif
	return CAIRO_INT_STATUS_UNSUPPORTED;
}

static cairo_status_t _put_image_boxes(cairo_xcb_surface_t * surface, cairo_image_surface_t * image, cairo_boxes_t * boxes)
{
	cairo_int_status_t status = CAIRO_INT_STATUS_SUCCESS;
	xcb_gcontext_t gc;
	...
	assert(image->depth == surface->depth);
	assert(image->stride == (int)CAIRO_STRIDE_FOR_WIDTH_BPP(image->width, PIXMAN_FORMAT_BPP(image->pixman_format)));
	gc = _cairo_xcb_screen_get_gc(surface->screen, surface->drawable, surface->depth);
	...
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		...
		for(chunk = &boxes->chunks; chunk; chunk = chunk->next) {
			...
			for(i = 0; i < chunk->count; i++) {
				...
				/* @v1.14.6
				_cairo_xcb_connection_put_image(surface->connection,
				    surface->drawable, gc, width, height,
				    x, y, image->depth, image->stride, 
					image->data + x * PIXMAN_FORMAT_BPP(image->pixman_format) / 8 + y * image->stride);
				*/
				// @v1.14.6 {
				_cairo_xcb_connection_put_subimage(surface->connection,
					surface->drawable, gc, x, y, width, height,
					PIXMAN_FORMAT_BPP (image->pixman_format) / 8,
					image->stride, x, y, image->depth, image->data);
				// } @v1.14.6 
			}
		}
		status = CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _cairo_xcb_surface_flush(void * abstract_surface, unsigned flags)
{
	cairo_xcb_surface_t * surface = abstract_surface;
	cairo_status_t status;
	if(flags)
	...
	if(status == CAIRO_STATUS_SUCCESS && (!surface->base._finishing || !surface->owns_pixmap)) {
		status = cairo_surface_status(&surface->fallback->base);
		if(status == CAIRO_STATUS_SUCCESS)
			status = _cairo_bentley_ottmann_tessellate_boxes(&surface->fallback_damage, CAIRO_FILL_RULE_WINDING, &surface->fallback_damage);
		if(status == CAIRO_STATUS_SUCCESS)
			status = _put_image_boxes(surface, surface->fallback, &surface->fallback_damage);
		if(status == CAIRO_STATUS_SUCCESS && !surface->base._finishing) {
			_cairo_surface_attach_snapshot(&surface->base, &surface->fallback->base, cairo_surface_finish);
		}
	}
...
static cairo_image_surface_t * _cairo_xcb_surface_map_to_image(void * abstract_surface,
...
static cairo_int_status_t _cairo_xcb_surface_unmap(void * abstract_surface,
    cairo_image_surface_t * image)
{
	cairo_xcb_surface_t * surface = abstract_surface;
	cairo_int_status_t status;
...
static cairo_surface_t * _cairo_xcb_surface_fallback(cairo_xcb_surface_t * surface,
...
static cairo_int_status_t _cairo_xcb_fallback_compositor_paint(const cairo_compositor_t     * compositor,
...
static cairo_int_status_t _cairo_xcb_fallback_compositor_mask(const cairo_compositor_t     * compositor,
    cairo_composite_rectangles_t * extents)
{
	cairo_xcb_surface_t * surface = (cairo_xcb_surface_t*)extents->surface;
	cairo_surface_t * fallback = _cairo_xcb_surface_fallback(surface, extents);
...
static cairo_int_status_t _cairo_xcb_fallback_compositor_fill(const cairo_compositor_t     * compositor,
    cairo_composite_rectangles_t * extents,
    const cairo_path_fixed_t     * path,
...
static cairo_int_status_t _cairo_xcb_fallback_compositor_glyphs(const cairo_compositor_t     * compositor,
    cairo_composite_rectangles_t * extents,
...
static inline const cairo_compositor_t * get_compositor(cairo_surface_t ** s)
{
...
static cairo_int_status_t _cairo_xcb_surface_paint(void * abstract_surface,
    cairo_operator_t op,
...
static cairo_int_status_t _cairo_xcb_surface_mask(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t  * source,
...
static cairo_int_status_t _cairo_xcb_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t        * source,
    const cairo_path_fixed_t     * path,
    const cairo_stroke_style_t   * style,
...
static cairo_int_status_t _cairo_xcb_surface_fill(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t  * source,
    const cairo_path_fixed_t* path,
    CairoFillRule fill_rule,
...
static cairo_int_status_t _cairo_xcb_surface_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t        * source,
    cairo_glyph_t                * glyphs,
    int num_glyphs,
    ...
    int height)
{
	...
	image_masks.alpha_mask =
	    (ulong)format->direct.alpha_mask << format->direct.alpha_shift;
	image_masks.red_mask =
	    (ulong)format->direct.red_mask << format->direct.red_shift;
	image_masks.green_mask =
	    (ulong)format->direct.green_mask << format->direct.green_shift;
	image_masks.blue_mask =
	    (ulong)format->direct.blue_mask << format->direct.blue_shift;
#if 0
 ...
 * Since: 1.12
 **/
void cairo_xcb_surface_set_size(cairo_surface_t * abstract_surface,
    int width,
    int height)
{
	cairo_xcb_surface_t * surface;
 ...
 * Since: 1.12
 **/
void cairo_xcb_surface_set_drawable(cairo_surface_t * abstract_surface,
    xcb_drawable_t drawable,
    int width,
    int height)
{
