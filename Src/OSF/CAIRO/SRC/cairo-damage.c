...
cairo_damage_t * _cairo_damage_create_in_error(cairo_status_t status)
{
...
{
	_cairo_damage::_cairo_damage_chunk * chunk;
	int n, size;
	TRACE((stderr, "%s x%d\n", __FUNCTION__, count));
	if(damage == NULL)
	...
	if(n > damage->remain)
		n = damage->remain;
	memcpy(damage->tail->base + damage->tail->count, boxes, n * sizeof(cairo_box_t));
	count -= n;
	damage->tail->count += n;
	damage->remain -= n;
	if(count == 0)
...
cairo_damage_t * _cairo_damage_add_box(cairo_damage_t * damage,
...
cairo_damage_t * _cairo_damage_add_rectangle(cairo_damage_t * damage,
    const CairoIRect * r)
{
	cairo_box_t box;
...
cairo_damage_t * _cairo_damage_add_region(cairo_damage_t * damage,
...
cairo_damage_t * _cairo_damage_reduce(cairo_damage_t * damage)
{
	cairo_box_t * free_boxes = NULL;
	cairo_box_t * boxes, * b;
	_cairo_damage::_cairo_damage_chunk * chunk, * last;
