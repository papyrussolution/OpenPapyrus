 ...
 **/
cairo_status_t _cairo_surface_set_error(cairo_surface_t * surface, cairo_status_t status)
{
...
static cairo_bool_t _cairo_surface_is_writable(cairo_surface_t * surface)
{
	return !surface->finished && surface->snapshot_of == NULL && !_cairo_surface_has_snapshots(surface) && !_cairo_surface_has_mime_data(surface);
}

static void _cairo_surface_begin_modification(cairo_surface_t * surface)
{
...
void _cairo_surface_init(cairo_surface_t * surface, const cairo_surface_backend_t * backend, cairo_device_t * device, cairo_content_t content)
{
...
cairo_surface_t * _cairo_surface_create_similar_scratch(cairo_surface_t * other, cairo_content_t content, int width, int height)
{
...
cairo_surface_t * cairo_surface_create_similar(cairo_surface_t * other, cairo_content_t content, int width, int height)
{
	...
	return _cairo_surface_create_similar_solid(other, content, width, height, CAIRO_COLOR_TRANSPARENT, TRUE);
}

cairo_surface_t * _cairo_surface_create_similar_solid(cairo_surface_t * other,
	...
	cairo_bool_t allow_fallback)
{
	...
	cairo_surface_t * surface = _cairo_surface_create_similar_scratch(other, content, width, height);
	if(surface == NULL && allow_fallback)
		surface = _cairo_image_surface_create_with_content(content, width, height);
	if(surface == NULL || surface->status)
		return surface;
...
cairo_surface_t * _cairo_surface_create_solid_pattern_surface(cairo_surface_t * other,
	const cairo_solid_pattern_t * solid_pattern)
{
	if(other->backend->create_solid_pattern_surface != NULL) {
		cairo_surface_t * surface = other->backend->create_solid_pattern_surface(other, solid_pattern);
		if(surface)
			return surface;
	}
	return _cairo_surface_create_similar_solid(other, _cairo_color_get_content(&solid_pattern->color),
		1, 1, &solid_pattern->color, FALSE);
}

cairo_int_status_t _cairo_surface_repaint_solid_pattern_surface(cairo_surface_t * other,
	cairo_surface_t * solid_surface, const cairo_solid_pattern_t * solid_pattern)
{
	...
	int height)
{
	...
	if(surface->backend->mark_dirty_rectangle != NULL) {
		...
		status = surface->backend->mark_dirty_rectangle(surface, (int)(x+surface->device_transform.x0), (int)(y+surface->device_transform.y0), width, height);
		if(unlikely(status))
			status = _cairo_surface_set_error(surface, status);
	}
}
 ...
 **/
void _cairo_surface_set_device_scale(cairo_surface_t * surface,
	double sx,
	double sy)
{
	cairo_status_t status;
 ...
 *  case.
 **/
cairo_status_t _cairo_surface_acquire_dest_image(cairo_surface_t * surface,
 ...
 **/
void _cairo_surface_release_dest_image(cairo_surface_t * surface,
	cairo_rectangle_int_t * interest_rect,
	cairo_image_surface_t * image,
	cairo_rectangle_int_t * image_rect,
	void * image_extra)
...
static cairo_status_t _cairo_recording_surface_clone_similar(cairo_surface_t * surface,
	cairo_surface_t * src,
	int src_x,
	int src_y,
	int width,
 ...
 **/
cairo_status_t _cairo_surface_clone_similar(cairo_surface_t * surface,
	cairo_surface_t * src,
	int src_x,
	int src_y,
	int width,
 ...
 **/
cairo_bool_t _cairo_surface_is_similar(cairo_surface_t * surface_a,
...
cairo_status_t _cairo_surface_composite(cairo_operator_t op,
	...
	cairo_region_t * clip_region)
{
	...
	return _cairo_surface_set_error(dst, _cairo_surface_fallback_composite(op,
			src, mask, dst, src_x, src_y, mask_x, mask_y, dst_x, dst_y, width, height, clip_region));
}
 ...
 **/
cairo_status_t _cairo_surface_fill_rectangle(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_color_t * color,
 ...
 * Return value: %CAIRO_STATUS_SUCCESS or the error that occurred
 **/
cairo_status_t _cairo_surface_fill_region(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_color_t * color, cairo_region_t * region)
{
	int num_rects;
	...
	if(num_rects > ARRAY_LENGTH(stack_rects)) {
		rects = (cairo_rectangle_int_t *)_cairo_malloc_ab(num_rects, sizeof(cairo_rectangle_int_t));
		if(rects == NULL) {
			return _cairo_surface_set_error(surface, _cairo_error(CAIRO_STATUS_NO_MEMORY));
		}
	}
	...
	status =  _cairo_surface_fill_rectangles(surface, op, color, rects, num_rects);
 ...
 * Return value: %CAIRO_STATUS_SUCCESS or the error that occurred
 **/
cairo_status_t _cairo_surface_fill_rectangles(cairo_surface_t * surface,
	cairo_operator_t op, const cairo_color_t * color, cairo_rectangle_int_t * rects, int num_rects)
{
	...
	if(surface->backend->fill_rectangles) {
		status = surface->backend->fill_rectangles(surface, op, color, rects, num_rects);
		if(status != CAIRO_INT_STATUS_UNSUPPORTED)
			return _cairo_surface_set_error(surface, status);
	}
	return _cairo_surface_set_error(surface, _cairo_surface_fallback_fill_rectangles(surface, op, color, rects, num_rects));
}
...
cairo_status_t _cairo_surface_composite_trapezoids(cairo_operator_t op,
...
cairo_span_renderer_t * _cairo_surface_create_span_renderer(cairo_operator_t op,
	const cairo_pattern_t * pattern,
	cairo_surface_t * dst,
	cairo_antialias_t antialias,
...
cairo_bool_t _cairo_surface_check_span_renderer(cairo_operator_t op,
	const cairo_pattern_t * pattern,
	cairo_surface_t * dst,
	cairo_antialias_t antialias)
{
...
void cairo_surface_copy_page(cairo_surface_t * surface)
{
	...
	if(surface->backend->copy_page == NULL)
		return;
	status_ignored = _cairo_surface_set_error(surface, surface->backend->copy_page(surface));
}
...
void cairo_surface_show_page(cairo_surface_t * surface)
{
	...
	if(surface->backend->show_page == NULL)
		return;
	status_ignored = _cairo_surface_set_error(surface, surface->backend->show_page(surface));
}
	...
	cairo_clip_t * clip)
{
	...
	if(clusters) {
		...
		if(surface->backend->show_glyphs != NULL) {
			 ...
			 */
			status = surface->backend->show_text_glyphs(surface, op,
				source, utf8, utf8_len, glyphs, num_glyphs, clusters, num_clusters, cluster_flags,
				dev_scaled_font, clip);
		}
	}
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
		status = _cairo_surface_fallback_show_glyphs(surface, op,
			source, glyphs, num_glyphs, dev_scaled_font, clip);
	}
...
cairo_status_t _cairo_surface_old_show_glyphs(cairo_scaled_font_t * scaled_font,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	cairo_surface_t * dst,
	int source_x,
...
static cairo_status_t _cairo_surface_composite_fixup_unbounded_internal(cairo_surface_t * dst,
 ...
 **/
cairo_status_t _cairo_surface_composite_fixup_unbounded(cairo_surface_t * dst,
 ...
 **/
cairo_status_t _cairo_surface_composite_shape_fixup_unbounded(cairo_surface_t * dst,
	cairo_surface_attributes_t * src_attr,
	int src_width,
	int src_height,
	int mask_width,
...
static void _rectangle_intersect_clip(cairo_rectangle_int_t * extents, cairo_clip_t * clip)
{
...
static void _cairo_surface_operation_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_clip_t * clip,
	cairo_rectangle_int_t * extents)
...
cairo_status_t _cairo_surface_paint_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_clip_t * clip,
	cairo_rectangle_int_t * extents)
...
cairo_status_t _cairo_surface_mask_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	const cairo_pattern_t * mask,
	cairo_clip_t * clip,
...
cairo_status_t _cairo_surface_stroke_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
...
cairo_status_t _cairo_surface_fill_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule,
...
cairo_status_t _cairo_surface_glyphs_extents(cairo_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_glyph_t * glyphs,
	int num_glyphs,
