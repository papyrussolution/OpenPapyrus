 ...
 **/
cairo_int_status_t FASTCALL _cairo_surface_set_error(cairo_surface_t * surface, cairo_int_status_t status)
{
	...
	if(status == CAIRO_INT_STATUS_SUCCESS || status >= (int)CAIRO_INT_STATUS_LAST_STATUS)
		return status;
...
static uint _cairo_surface_allocate_unique_id(void)
{
...
static cairo_bool_t FASTCALL _cairo_surface_has_snapshots(cairo_surface_t * surface)
{
	return !cairo_list_is_empty(&surface->snapshots);
}

static cairo_bool_t FASTCALL _cairo_surface_has_mime_data(cairo_surface_t * surface)
{
	return surface->mime_data.num_elements != 0;
}

static void FASTCALL _cairo_surface_detach_mime_data(cairo_surface_t * surface)
{
...
static void FASTCALL _cairo_surface_detach_snapshots(cairo_surface_t * surface)
{
	while(_cairo_surface_has_snapshots(surface)) {
		_cairo_surface_detach_snapshot(cairo_list_first_entry(&surface->snapshots, cairo_surface_t, snapshot));
	}
}
...
void _cairo_surface_init(cairo_surface_t * surface, const cairo_surface_backend_t * backend,
    cairo_device_t * device, cairo_content_t content)
{
...
cairo_surface_t * cairo_surface_create_similar_image(cairo_surface_t  * other, cairo_format_t format, int width, int height)
{
	...
	if(other->backend->create_similar_image)
		image = other->backend->create_similar_image(other, format, width, height);
	if(image == NULL)
...
cairo_int_status_t _cairo_surface_unmap_image(cairo_surface_t * surface, cairo_image_surface_t * image)
{
	...
	status = _cairo_surface_paint(surface, CAIRO_OPERATOR_SOURCE, &pattern.base, clip);
 ...
 * Since: 1.12
 **/
cairo_surface_t * cairo_surface_map_to_image(cairo_surface_t  * surface, const CairoIRect * extents)
{
	CairoIRect rect;
...
cairo_surface_t * _cairo_surface_create_scratch(cairo_surface_t * other, cairo_content_t content, int width, int height, const cairo_color_t * color)
{
...
{
	return (surface == NULL || CAIRO_REFERENCE_COUNT_IS_INVALID(&surface->ref_count)) ? 0 : CAIRO_REFERENCE_COUNT_GET_VALUE(&surface->ref_count);
}
...
void * cairo_surface_get_user_data(cairo_surface_t * surface, const cairo_user_data_key_t * key)
{
	...
	return CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&surface->ref_count) ? _cairo_user_data_array_get_data(&surface->user_data, key) : 0;
}
...
{
	cairo_mime_data_t * mime_data = (cairo_mime_data_t *)ptr;
...
{
	cairo_mime_data_t * mime_data = (cairo_mime_data_t *)elt;
 ...
 * Since: 1.0
 **/
void FASTCALL cairo_surface_get_font_options(cairo_surface_t * surface, cairo_font_options_t  * options)
{
...
{
	if(surface->status == 0 && !surface->finished) {
...
void cairo_surface_mark_dirty(cairo_surface_t * surface)
{
	...
	cairo_surface_mark_dirty_rectangle(surface, extents.x, extents.y, extents.width, extents.height);
}
...
void cairo_surface_mark_dirty_rectangle(cairo_surface_t * surface, int x, int y, int width, int height)
{
	...
	if(surface->backend->mark_dirty_rectangle) {
		...
		status = surface->backend->mark_dirty_rectangle(surface, 
			(int)(x + surface->device_transform.x0), (int)(y + surface->device_transform.y0), width, height);
		if(unlikely(status))
			_cairo_surface_set_error(surface, status);
	}
}
...
{
	ASSIGN_PTR(x_scale, surface->device_transform.xx);
	ASSIGN_PTR(y_scale, surface->device_transform.yy);
}
...
{
	ASSIGN_PTR(x_offset, surface->device_transform.x0);
	ASSIGN_PTR(y_offset, surface->device_transform.y0);
}
...
{
	ASSIGN_PTR(x_pixels_per_inch, surface->x_fallback_resolution);
	ASSIGN_PTR(y_pixels_per_inch, surface->y_fallback_resolution);
}

cairo_bool_t FASTCALL _cairo_surface_has_device_transform(cairo_surface_t * surface)
{
...
cairo_surface_t * _cairo_surface_default_source(void * surface, CairoIRect * extents)
{
	if(extents)
...
static cairo_bool_t FASTCALL nothing_to_do(cairo_surface_t * surface, cairo_operator_t op, const cairo_pattern_t * source)
{
	if(_cairo_pattern_is_clear(source)) {
		if(oneof2(op, CAIRO_OPERATOR_OVER, CAIRO_OPERATOR_ADD))
			return TRUE;
	...
	if(op == CAIRO_OPERATOR_CLEAR && surface->is_clear)
		return TRUE;
	else if(op == CAIRO_OPERATOR_ATOP && (surface->content & CAIRO_CONTENT_COLOR) ==0)
		return TRUE;
	else
		return FALSE;
}
...
cairo_status_t _cairo_surface_stroke(cairo_surface_t * surface, cairo_operator_t op,
...
cairo_status_t _cairo_surface_fill(cairo_surface_t * surface, cairo_operator_t op, const cairo_pattern_t * source, 
...
cairo_bool_t cairo_surface_has_show_text_glyphs(cairo_surface_t * surface)
{
	if(unlikely(surface->status))
		return FALSE;
	else if(unlikely(surface->finished)) {
		_cairo_surface_set_error(surface, CAIRO_STATUS_SURFACE_FINISHED);
		return FALSE;
	}
	else
		return (surface->backend->has_show_text_glyphs) ? surface->backend->has_show_text_glyphs(surface) : (surface->backend->show_text_glyphs != NULL);
}
...
cairo_status_t _cairo_surface_show_text_glyphs(cairo_surface_t * surface,
    ...
    const char * utf8, int utf8_len, cairo_glyph_t * glyphs,
    ...
    cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	if(clusters) {
		...
		if(surface->backend->show_glyphs) {
			 ...
			 * zero-length).
			 */
			status = surface->backend->show_text_glyphs(surface, op, source, 
				utf8, utf8_len, glyphs, num_glyphs, clusters, num_clusters, cluster_flags, scaled_font, clip);
		}
	}
	if(status != CAIRO_INT_STATUS_NOTHING_TO_DO) {
		surface->is_clear = FALSE;
...
cairo_surface_t * FASTCALL _cairo_surface_create_in_error(cairo_status_t status)
{
	...
	switch(status) {
		case CAIRO_STATUS_NO_MEMORY: return (cairo_surface_t*)&_cairo_surface_nil;
		case CAIRO_STATUS_SURFACE_TYPE_MISMATCH: return (cairo_surface_t*)&_cairo_surface_nil_surface_type_mismatch;
		case CAIRO_STATUS_INVALID_STATUS: return (cairo_surface_t*)&_cairo_surface_nil_invalid_status;
		case CAIRO_STATUS_INVALID_CONTENT: return (cairo_surface_t*)&_cairo_surface_nil_invalid_content;
		case CAIRO_STATUS_INVALID_FORMAT: return (cairo_surface_t*)&_cairo_surface_nil_invalid_format;
		case CAIRO_STATUS_INVALID_VISUAL: return (cairo_surface_t*)&_cairo_surface_nil_invalid_visual;
		case CAIRO_STATUS_READ_ERROR: return (cairo_surface_t*)&_cairo_surface_nil_read_error;
		case CAIRO_STATUS_WRITE_ERROR: return (cairo_surface_t*)&_cairo_surface_nil_write_error;
		case CAIRO_STATUS_FILE_NOT_FOUND: return (cairo_surface_t*)&_cairo_surface_nil_file_not_found;
		case CAIRO_STATUS_TEMP_FILE_ERROR: return (cairo_surface_t*)&_cairo_surface_nil_temp_file_error;
		case CAIRO_STATUS_INVALID_STRIDE: return (cairo_surface_t*)&_cairo_surface_nil_invalid_stride;
		case CAIRO_STATUS_INVALID_SIZE: return (cairo_surface_t*)&_cairo_surface_nil_invalid_size;
		case CAIRO_STATUS_DEVICE_TYPE_MISMATCH: return (cairo_surface_t*)&_cairo_surface_nil_device_type_mismatch;
		case CAIRO_STATUS_DEVICE_ERROR: return (cairo_surface_t*)&_cairo_surface_nil_device_error;
...
cairo_surface_t * _cairo_int_surface_create_in_error(cairo_int_status_t status)
{
	if(status < CAIRO_INT_STATUS_LAST_STATUS)
		...
		switch((int)status) {
			case CAIRO_INT_STATUS_UNSUPPORTED: return (cairo_surface_t*)&_cairo_surface_nil_unsupported;
			case CAIRO_INT_STATUS_NOTHING_TO_DO: return (cairo_surface_t*)&_cairo_surface_nil_nothing_to_do;
			default:
				_cairo_error_throw(CAIRO_STATUS_NO_MEMORY);
				return (cairo_surface_t*)&_cairo_surface_nil;
		}
