...
typedef struct _sweep_line {
	...
	struct _sweep_line_coverage coverage;
	struct event_queue {
		pqueue_t pq;
...
static cairo_bool_t bo_edge_contains_intersect_point(const edge_t * edge, cairo_bo_intersect_point_t * point)
{
	 ...
	 */
	int cmp_top = bo_intersect_ordinate_32_compare(point->y.ordinate, edge->edge.top, point->y.exactness);
	if(cmp_top < 0)
		return FALSE;
...
inline static void coverage_rewind(struct _sweep_line_coverage * cells)
{
	cells->cursor = &cells->head;
}

static void coverage_init(struct _sweep_line_coverage * cells)
{
...
static void coverage_fini(struct _sweep_line_coverage * cells)
{
	_cairo_freepool_fini(&cells->pool);
}

inline static void coverage_reset(struct _sweep_line_coverage * cells)
{
	cells->head.next = &cells->tail;
	cells->tail.prev = &cells->head;
	cells->count = 0;
...
inline static struct cell * coverage_alloc(sweep_line_t * sweep_line, struct cell * tail, int x)
{
		...
		if(cell->x != ix1) {
			if(unlikely(cell->x > ix1)) {
				do {
					...
					UNROLL3({ if(cell->x >= ix1) break; cell = cell->next; });
				} while(TRUE);
			if(cell->x != ix1)
...
static void render_rows(cairo_botor_scan_converter_t * self,
	sweep_line_t * sweep_line, int y, int height, cairo_span_renderer_t * renderer)
{
...
{
	struct _run * run = (struct _run *)_cairo_freepool_alloc(&sweep_line->runs);
...
{
	struct _run tail;
	struct _run * run = &tail;
	tail.next = NULL;
	tail.y = y2;
	/* Order the runs top->bottom */
	while(edge->runs) {
		struct _run * r = edge->runs;
		edge->runs = r->next;
		r->next = run;
		run = r;
	}
	...
	int num_events)
{
	...
	_cairo_freepool_init(&sweep_line->runs, sizeof(struct _run));
...
static cairo_status_t botor_generate(cairo_botor_scan_converter_t * self,
	event_t ** start_events, cairo_span_renderer_t * renderer)
{
	cairo_status_t status;
	sweep_line_t sweep_line;
	cairo_fixed_t ybot;
...
{
	struct _cairo_botor_scan_converter_chunk * chunk = self->tail;
	if(chunk->count == chunk->size) {
		int size = chunk->size*2;
		chunk->next = (struct _cairo_botor_scan_converter_chunk *)_cairo_malloc_ab_plus_c(size, sizeof(edge_t), sizeof(struct _cairo_botor_scan_converter_chunk));
...
static cairo_status_t _cairo_botor_scan_converter_add_edge(void * converter,
...
static cairo_status_t _cairo_botor_scan_converter_add_polygon(void * converter, const cairo_polygon_t * polygon)
{
	cairo_botor_scan_converter_t * self = (cairo_botor_scan_converter_t *)converter;
	cairo_status_t status;
	int i;
