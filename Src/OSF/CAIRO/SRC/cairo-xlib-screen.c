...
void _cairo_xlib_screen_destroy(cairo_xlib_display_t * display,
    cairo_xlib_screen_t * info)
{
	Display * dpy;
	int i;
    ...
    cairo_xlib_screen_t ** out)
{
	...
	memzero(info->gc_depths, sizeof(info->gc_depths));
	memzero(info->gc, sizeof(info->gc));
...
GC _cairo_xlib_screen_get_gc(cairo_xlib_display_t * display, cairo_xlib_screen_t * info, int depth, Drawable drawable)
{
	GC gc = NULL;
	int i;
	for(i = 0; i < SIZEOFARRAY(info->gc); i++) {
...
void _cairo_xlib_screen_put_gc(cairo_xlib_display_t * display,
    cairo_xlib_screen_t * info,
    int depth,
    GC gc)
{
...
cairo_status_t _cairo_xlib_screen_get_visual_info(cairo_xlib_display_t * display,
    cairo_xlib_screen_t * info,
    Visual * v,
...
cairo_font_options_t * _cairo_xlib_screen_get_font_options(cairo_xlib_screen_t * info)
{
