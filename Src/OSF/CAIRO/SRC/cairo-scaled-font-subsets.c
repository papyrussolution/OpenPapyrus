...
static cairo_status_t _cairo_sub_font_map_glyph(cairo_sub_font_t * sub_font, unsigned long scaled_font_glyph_index,
	const char * utf8, int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph);
...
static cairo_bool_t _cairo_sub_font_glyphs_equal(const void * key_a, const void * key_b)
{
	const cairo_sub_font_glyph_t * sub_font_glyph_a = (const cairo_sub_font_glyph_t *)key_a;
	const cairo_sub_font_glyph_t * sub_font_glyph_b = (const cairo_sub_font_glyph_t *)key_b;
	return sub_font_glyph_a->base.hash == sub_font_glyph_b->base.hash;
}

static cairo_sub_font_glyph_t * _cairo_sub_font_glyph_create(unsigned long scaled_font_glyph_index,
	unsigned int subset_id, unsigned int subset_glyph_index, double x_advance, double y_advance)
{
	cairo_sub_font_glyph_t * sub_font_glyph = (cairo_sub_font_glyph_t *)malloc(sizeof(cairo_sub_font_glyph_t));
...
static void _cairo_sub_font_glyph_destroy(cairo_sub_font_glyph_t * sub_font_glyph)
{
...
static void _cairo_sub_font_glyph_pluck(void * entry, void * closure)
{
	cairo_sub_font_glyph_t * sub_font_glyph = (cairo_sub_font_glyph_t *)entry;
	cairo_hash_table_t * sub_font_glyphs = (cairo_hash_table_t *)closure;
	_cairo_hash_table_remove(sub_font_glyphs, &sub_font_glyph->base);
	_cairo_sub_font_glyph_destroy(sub_font_glyph);
}

static void _cairo_sub_font_glyph_collect(void * entry, void * closure)
{
	cairo_sub_font_glyph_t * sub_font_glyph = (cairo_sub_font_glyph_t *)entry;
	cairo_sub_font_collection_t * collection = (cairo_sub_font_collection_t *)closure;
	unsigned long scaled_font_glyph_index;
	unsigned int subset_glyph_index;
...
static cairo_bool_t _cairo_sub_fonts_equal(const void * key_a, const void * key_b)
{
	const cairo_sub_font_t * sub_font_a = (const cairo_sub_font_t *)key_a;
	const cairo_sub_font_t * sub_font_b = (const cairo_sub_font_t *)key_b;
...
static void _cairo_sub_font_init_key(cairo_sub_font_t * sub_font, cairo_scaled_font_t * scaled_font)
{
...
static cairo_status_t _cairo_sub_font_create(cairo_scaled_font_subsets_t * parent,
	cairo_scaled_font_t * scaled_font,
	unsigned int font_id,
	int max_glyphs_per_subset,
	cairo_bool_t is_scaled,
	...
	cairo_sub_font_t ** sub_font_out)
{
	...
	cairo_sub_font_t * sub_font = (cairo_sub_font_t *)malloc(sizeof(cairo_sub_font_t));
...
static void _cairo_sub_font_destroy(cairo_sub_font_t * sub_font)
{
	_cairo_hash_table_foreach(sub_font->sub_font_glyphs, _cairo_sub_font_glyph_pluck, sub_font->sub_font_glyphs);
	_cairo_hash_table_destroy(sub_font->sub_font_glyphs);
	cairo_scaled_font_destroy(sub_font->scaled_font);
	free(sub_font);
}

static void _cairo_sub_font_pluck(void * entry, void * closure)
{
	cairo_sub_font_t * sub_font = (cairo_sub_font_t *)entry;
	cairo_hash_table_t * sub_fonts = (cairo_hash_table_t *)closure;
	_cairo_hash_table_remove(sub_fonts, &sub_font->base);
	_cairo_sub_font_destroy(sub_font);
}

static cairo_status_t _cairo_sub_font_glyph_lookup_unicode(cairo_sub_font_glyph_t * sub_font_glyph,
	cairo_scaled_font_t * scaled_font, unsigned long scaled_font_glyph_index)
{
	uint32_t unicode;
	char buf[8];
	int len;
	...
	if(unicode != (uint32_t)-1) {
		...
		if(len > 0) {
			sub_font_glyph->utf8 = (char *)malloc(len+1);
...
static cairo_status_t _cairo_sub_font_glyph_map_to_unicode(cairo_sub_font_glyph_t * sub_font_glyph,
	const char * utf8,
	int utf8_len,
	cairo_bool_t * is_mapped)
{
	*is_mapped = FALSE;
	...
	if(utf8 != NULL && utf8_len != 0) {
		if(sub_font_glyph->utf8 != NULL) {
			...
			sub_font_glyph->utf8 = (char *)malloc(utf8_len+1);
			if(unlikely(sub_font_glyph->utf8 == NULL))
				return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_int_status_t _cairo_sub_font_lookup_glyph(cairo_sub_font_t * sub_font,
	unsigned long scaled_font_glyph_index, const char * utf8,
	int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	...
	if(sub_font_glyph != NULL) {
		...
		status = _cairo_sub_font_glyph_map_to_unicode(sub_font_glyph, utf8, utf8_len, &subset_glyph->utf8_is_mapped);
...
static cairo_status_t _cairo_sub_font_map_glyph(cairo_sub_font_t * sub_font,
	unsigned long scaled_font_glyph_index, const char * utf8, int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	cairo_sub_font_glyph_t key, * sub_font_glyph;
	cairo_status_t status;
	_cairo_sub_font_glyph_init_key(&key, scaled_font_glyph_index);
	...
	if(sub_font_glyph == NULL) {
		...
		if(sub_font->is_scaled) {
			...
			if(sub_font->num_glyphs_in_current_subset >
...
static void _cairo_sub_font_collect(void * entry, void * closure)
{
	cairo_sub_font_t * sub_font = (cairo_sub_font_t *)entry;
	cairo_sub_font_collection_t * collection = (cairo_sub_font_collection_t *)closure;
	...
	for(i = 0; i <= sub_font->current_subset; i++) {
		...
		subset.to_unicode = (unsigned long *)_cairo_malloc_ab(collection->num_glyphs, sizeof(unsigned long));
...
static cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_internal(cairo_subsets_type_t type)
{
	cairo_scaled_font_subsets_t * subsets = (cairo_scaled_font_subsets_t *)malloc(sizeof(cairo_scaled_font_subsets_t));
...
cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_simple(void)
{
	return _cairo_scaled_font_subsets_create_internal(CAIRO_SUBSETS_SIMPLE);
}

cairo_scaled_font_subsets_t * _cairo_scaled_font_subsets_create_composite(void)
{
	return _cairo_scaled_font_subsets_create_internal(CAIRO_SUBSETS_COMPOSITE);
}

void _cairo_scaled_font_subsets_destroy(cairo_scaled_font_subsets_t * subsets)
{
	...
	const char * utf8, int utf8_len, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	...
	if(subsets->type != CAIRO_SUBSETS_SCALED) {
		...
		sub_font = (cairo_sub_font_t *)_cairo_hash_table_lookup(subsets->unscaled_sub_fonts, &key.base);
		if(sub_font != NULL) {
			status = _cairo_sub_font_lookup_glyph(sub_font, scaled_font_glyph_index, utf8, utf8_len, subset_glyph);
	...
	key.is_scaled = TRUE;
	_cairo_sub_font_init_key(&key, scaled_font);
	sub_font = (cairo_sub_font_t *)_cairo_hash_table_lookup(subsets->scaled_sub_fonts, &key.base);
	if(sub_font != NULL) {
		status = _cairo_sub_font_lookup_glyph(sub_font, scaled_font_glyph_index, utf8, utf8_len, subset_glyph);
		if(status != CAIRO_INT_STATUS_UNSUPPORTED)
			return status;
	...
	if(_cairo_status_is_error(status))
		return status;
	if(status == CAIRO_STATUS_SUCCESS && subsets->type != CAIRO_SUBSETS_SCALED && !_cairo_font_face_is_user(scaled_font->font_face)) {
		...
		if(sub_font == NULL) {
			...
			if(subsets->type == CAIRO_SUBSETS_SCALED)
				...
				max_glyphs = MAX_GLYPHS_PER_SIMPLE_FONT;
			status = _cairo_sub_font_create(subsets, cairo_scaled_font_reference(scaled_font),
				subsets->num_sub_fonts, max_glyphs, subset_glyph->is_scaled, subset_glyph->is_composite, &sub_font);
			if(unlikely(status)) {
	...
	return _cairo_sub_font_map_glyph(sub_font, scaled_font_glyph_index, utf8, utf8_len, subset_glyph);
}
	...
	cairo_subsets_foreach_type_t type)
{
	...
	if(!collection.glyphs_size)
		return CAIRO_STATUS_SUCCESS;
	collection.glyphs = (unsigned long *)_cairo_malloc_ab(collection.glyphs_size, sizeof(unsigned long));
...
cairo_status_t _cairo_scaled_font_subsets_foreach_scaled(cairo_scaled_font_subsets_t * font_subsets,
	cairo_scaled_font_subset_callback_func_t font_subset_callback,
	void * closure)
{
...
cairo_status_t _cairo_scaled_font_subsets_foreach_unscaled(cairo_scaled_font_subsets_t * font_subsets,
	cairo_scaled_font_subset_callback_func_t font_subset_callback,
	void * closure)
{
	return _cairo_scaled_font_subsets_foreach_internal(font_subsets,
...
cairo_status_t _cairo_scaled_font_subsets_foreach_user(cairo_scaled_font_subsets_t * font_subsets,
	cairo_scaled_font_subset_callback_func_t font_subset_callback, void * closure)
{
	return _cairo_scaled_font_subsets_foreach_internal(font_subsets,
		font_subset_callback, closure, CAIRO_SUBSETS_FOREACH_USER);
}

static cairo_bool_t _cairo_string_equal(const void * key_a, const void * key_b)
{
	...
	return (strcmp(a->string, b->string) == 0) ? TRUE : FALSE;
}

static void _cairo_string_init_key(cairo_string_entry_t * key, char * s)
{
	unsigned long sum = 0;
	unsigned int i;
...
{
	*entry = (cairo_string_entry_t *)malloc(sizeof(cairo_string_entry_t));
...
static void _pluck_entry(void * entry, void * closure)
{
	_cairo_hash_table_remove((cairo_hash_table_t *)closure, (cairo_hash_entry_t *)entry);
	free(entry);
}

cairo_int_status_t _cairo_scaled_font_subset_create_glyph_names(cairo_scaled_font_subset_t * subset)
{
	unsigned int i;
