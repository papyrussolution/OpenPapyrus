...
typedef struct _cairo_sub_font_collection {
	ulong * glyphs; /* scaled_font_glyph_index */
	char       ** utf8;
	uint glyphs_size;
	int           * to_latin_char;
...
static cairo_sub_font_glyph_t * _cairo_sub_font_glyph_create(ulong scaled_font_glyph_index,
    uint subset_id,
    uint subset_glyph_index,
    double x_advance,
    double y_advance,
    ...
    int utf8_len)
{
	cairo_sub_font_glyph_t * sub_font_glyph = (cairo_sub_font_glyph_t *)SAlloc::M(sizeof(cairo_sub_font_glyph_t));
...
{
	SAlloc::F(sub_font_glyph->utf8);
	SAlloc::F(sub_font_glyph);
}
...
static void _cairo_sub_font_init_key(cairo_sub_font_t      * sub_font,
    cairo_scaled_font_t   * scaled_font)
{
	if(sub_font->is_scaled) {
		sub_font->base.hash = (ulong)scaled_font;
		sub_font->scaled_font = scaled_font;
	}
	else {
		sub_font->base.hash = (ulong)scaled_font->font_face;
		sub_font->scaled_font = scaled_font;
	}
}
...
{
	int i;
	cairo_sub_font_t * sub_font = (cairo_sub_font_t *)SAlloc::M(sizeof(cairo_sub_font_t));
...
int _cairo_unicode_to_winansi(ulong uni)
{
	int i;
...
static cairo_status_t _cairo_sub_font_glyph_lookup_unicode(cairo_scaled_font_t    * scaled_font,
    ulong scaled_font_glyph_index,
	...
	*is_mapped = FALSE;
	...
	if(utf8 != NULL && utf8_len != 0) {
		if(sub_font_glyph->utf8 != NULL) {
			...
			sub_font_glyph->utf8 = (char *)SAlloc::M(utf8_len + 1);
...
static cairo_status_t _cairo_sub_font_add_glyph(cairo_sub_font_t        * sub_font,
    ulong scaled_font_glyph_index,
    cairo_bool_t is_latin,
    int latin_character,
    uint32_t unicode,
		...
		if(text_utf8 != NULL && text_utf8_len > 0) {
			...
			if(status == CAIRO_STATUS_SUCCESS) {
				if(ucs4_len == 1) {
					font_unicode = ucs4[0];
					SAlloc::F(font_utf8);
					font_utf8 = (char *)SAlloc::M(text_utf8_len + 1);
					if(font_utf8 == NULL) {
						SAlloc::F(ucs4);
						return _cairo_error(CAIRO_STATUS_NO_MEMORY);
					}
	...
	for(i = 0; i <= sub_font->current_subset; i++) {
		...
		if(subset.glyph_names != NULL) {
			for(j = 0; j < collection->num_glyphs; j++)
				SAlloc::F(subset.glyph_names[j]);
			SAlloc::F(subset.glyph_names);
		}
...
{
	cairo_scaled_font_subsets_t * subsets = (cairo_scaled_font_subsets_t *)SAlloc::M(sizeof(cairo_scaled_font_subsets_t));
...
void _cairo_scaled_font_subsets_enable_latin_subset(cairo_scaled_font_subsets_t * font_subsets,
    ...
    cairo_subsets_foreach_type_t type)
{
	...
	if(unlikely(collection.glyphs == NULL ||
		...
		SAlloc::F(collection.latin_to_subset_glyph_index);
...
{
	*entry = (cairo_string_entry_t *)SAlloc::M(sizeof(cairo_string_entry_t));
