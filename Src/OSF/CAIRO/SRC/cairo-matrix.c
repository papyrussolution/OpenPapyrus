...
{
	return (matrix->xx * matrix->yy) - (matrix->yx * matrix->xy);
}
...
cairo_bool_t _cairo_matrix_is_identity(const cairo_matrix_t * matrix)
{
	return matrix->xx == 1.0 && matrix->yx == 0.0 && matrix->xy == 0.0 && matrix->yy == 1.0 && matrix->x0 == 0.0 && matrix->y0 == 0.0;
}

cairo_bool_t _cairo_matrix_is_translation(const cairo_matrix_t * matrix)
{
...
double _cairo_matrix_transformed_circle_major_axis(const cairo_matrix_t * matrix, double radius)
{
	...
	_cairo_matrix_get_affine(matrix, &a, &b, &c, &d, NULL, NULL);
	i = a*a+b*b;
	j = c*c+d*d;
	f = 0.5*(i+j);
	g = 0.5*(i-j);
...
void _cairo_matrix_to_pixman_matrix(const cairo_matrix_t * matrix, pixman_transform_t * pixman_transform, double xc, double yc)
{
