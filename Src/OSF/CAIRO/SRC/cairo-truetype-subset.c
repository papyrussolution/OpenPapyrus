...
static cairo_status_t cairo_truetype_font_use_glyph(cairo_truetype_font_t * font, unsigned short glyph,
	unsigned short * out);
...
static cairo_status_t _cairo_truetype_font_create(cairo_scaled_font_subset_t * scaled_font_subset, cairo_truetype_font_t ** font_return)
{
	cairo_status_t status;
	...
	status = backend->load_truetype_table(scaled_font_subset->scaled_font, TT_TAG_head, 0, (unsigned char *)&head, &size);
	if(unlikely(status))
		return status;
	...
	if(unlikely(status))
		return status;
	font = (cairo_truetype_font_t *)malloc(sizeof(cairo_truetype_font_t));
	...
	status = _cairo_truetype_read_font_name(scaled_font_subset->scaled_font, &font->base.ps_name, &font->base.font_name);
	if(_cairo_status_is_error(status))
		goto fail3;
	/* If the PS name is not found, create a CairoFont-x-y name. */
	if(font->base.ps_name == NULL) {
		font->base.ps_name = (char *)malloc(30);
		if(unlikely(font->base.ps_name == NULL)) {
			status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
			goto fail3;
		}
		snprintf(font->base.ps_name, 30, "CairoFont-%u-%u", scaled_font_subset->font_id, scaled_font_subset->subset_id);
	}
...
static cairo_status_t cairo_truetype_font_write_glyf_table(cairo_truetype_font_t * font, unsigned long tag)
{
	...
	u.bytes = (unsigned char *)malloc(size);
...
static cairo_status_t cairo_truetype_font_write_maxp_table(cairo_truetype_font_t * font,
	unsigned long tag)
{
	tt_maxp_t * maxp;
	unsigned long size;
...
static void cairo_truetype_font_update_entry(cairo_truetype_font_t * font,
	int index, unsigned long tag, unsigned long start, unsigned long end)
{
...
static cairo_status_t cairo_truetype_font_generate(cairo_truetype_font_t * font,
	const char ** data, unsigned long * length, const unsigned long ** string_offsets,
	unsigned long * num_strings)
{
	...
	*num_strings = _cairo_array_num_elements(&font->string_offsets);
	if(*num_strings != 0)
		*string_offsets = (unsigned long *)_cairo_array_index(&font->string_offsets, 0);
	else
		*string_offsets = NULL;
FAIL:
	return _cairo_truetype_font_set_error(font, status);
...
static void cairo_truetype_font_create_truetype_table_list(cairo_truetype_font_t * font)
{
	...
	if(has_fpgm)
		cairo_truetype_font_add_truetype_table(font, TT_TAG_fpgm, cairo_truetype_font_write_generic_table,
			pos++);
	pos++;
...
cairo_status_t _cairo_truetype_subset_init(cairo_truetype_subset_t * truetype_subset,
	cairo_scaled_font_subset_t * font_subset)
{
	...
	cairo_status_t status = _cairo_truetype_font_create(font_subset, &font);
	if(unlikely(status))
		return status;
	...
	if(length) {
		truetype_subset->data = (unsigned char *)malloc(length);
	...
	if(num_strings) {
		...
		truetype_subset->string_offsets = (unsigned long *)malloc(offsets_length);
...
static cairo_int_status_t _cairo_truetype_reverse_cmap(cairo_scaled_font_t * scaled_font,
	unsigned long table_offset, unsigned long index, uint32_t * ucs4)
{
	cairo_status_t status;
	...
	status = backend->load_truetype_table(scaled_font, TT_TAG_cmap, table_offset, (unsigned char *)&buf, &size);
	if(unlikely(status))
		return status;
	...
	map = (tt_segment_map_t *)malloc(size);
	if(unlikely(map == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = backend->load_truetype_table(scaled_font, TT_TAG_cmap, table_offset, (unsigned char *)map, &size);
	if(unlikely(status))
		goto fail;
	...
	for(i = 0; i < num_segments; i++) {
		...
		if(range_offset[i] == 0 && c >= be16_to_cpu(start_code[i]) && c <= be16_to_cpu(end_code[i])) {
			*ucs4 = c;
			goto found;
		}
	}
...
cairo_int_status_t _cairo_truetype_index_to_ucs4(cairo_scaled_font_t * scaled_font, unsigned long index, uint32_t * ucs4)
{
	...
	if(!backend->load_truetype_table)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	size = 4;
	status = backend->load_truetype_table(scaled_font, TT_TAG_cmap, 0, (unsigned char *)&buf, &size);
	if(unlikely(status))
		return status;
	cmap = (tt_cmap_t *)buf;
	...
	if(unlikely(cmap == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	status = backend->load_truetype_table(scaled_font, TT_TAG_cmap, 0, (unsigned char *)cmap, &size);
	if(unlikely(status))
		goto cleanup;
		...
		if((be16_to_cpu(record->platform) == 1) &&
		   (be16_to_cpu(record->encoding) == 0)) {
			if(be16_to_cpu(record->name) == 4) {
				font_name = (char *)malloc(be16_to_cpu(record->length)+1);
				if(font_name == NULL) {
					status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
					goto fail;
				}
			...
			if(be16_to_cpu(record->name) == 6) {
				ps_name = (char *)malloc(be16_to_cpu(record->length)+1);
				if(ps_name == NULL) {
					status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
					goto fail;
				}
				strncpy(ps_name, ((char *)name)+be16_to_cpu(name->strings_offset)+be16_to_cpu(record->offset),
					be16_to_cpu(record->length));
