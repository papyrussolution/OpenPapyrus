    ...
    cairo_truetype_font_t      ** font_return)
{
	...
	if(unlikely(status))
		return status;
	font = (cairo_truetype_font_t *)SAlloc::M(sizeof(cairo_truetype_font_t));
	...
	if(font->base.ps_name == NULL) {
		font->base.ps_name = (char *)SAlloc::M(30);
    ...
    ulong tag)
{
	...
	if(font->status)
		return font->status;
	size = sizeof(tt_head_t);
	status = font->backend->load_truetype_table(font->scaled_font_subset->scaled_font, TT_TAG_head, 0, (uchar*)&header, &size);
	if(unlikely(status))
		return _cairo_truetype_font_set_error(font, status);
	if(be16_to_cpu(header.index_to_loc_format) == 0)
		size = sizeof(int16) * (font->num_glyphs_in_face + 1);
	else
		size = sizeof(int32_t) * (font->num_glyphs_in_face + 1);
	u.bytes = (uchar *)SAlloc::M(size);
...
static cairo_status_t cairo_truetype_font_write_hhea_table(cairo_truetype_font_t * font, ulong tag)
{
	...
	hhea->num_hmetrics = cpu_to_be16((uint16)(font->base.num_glyphs));
    ...
    ulong tag)
{
	...
	if(be16_to_cpu(header.index_to_loc_format) == 0) {
		for(i = 0; i < font->base.num_glyphs + 1; i++)
			cairo_truetype_font_write_be16(font, (uint16)(font->glyphs[i].location / 2));
	}
	else {
		for(i = 0; i < font->base.num_glyphs + 1; i++)
...
static cairo_status_t cairo_truetype_font_write_offset_table(cairo_truetype_font_t * font)
{
	...
	ushort search_range, entry_selector, range_shift;
    ...
    cairo_bool_t is_pdf)
{
	...
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		ushort parent_glyph = (ushort)font->scaled_font_subset->glyphs[i];
	...
	if(length) {
		truetype_subset->data = (uchar *)SAlloc::M(length);
	...
	if(num_strings) {
		...
		truetype_subset->string_offsets = (ulong *)SAlloc::M(offsets_length);
    ...
    uint32_t            * ucs4)
{
	...
	map = (tt_segment_map_t *)SAlloc::M(size);
	...
	/* A Format 4 cmap contains 8 uint16 numbers and 4 arrays of
	 * uint16 each num_segments long. */
	...
	for(i = 0; i < num_segments; i++) {
		c = (uint16)(index - be16_to_cpu(delta[i]));
	...
	for(i = 0; i < num_segments; i++) {
		if(range_offset[i] != 0) {
			...
			uint16 g_id_be = cpu_to_be16((uint16)index);
			int j;
...
static cairo_status_t find_name(tt_name_t * name, int name_id, int platform, int encoding, int language, char ** str_out)
{
	...
	if(platform == 3) { /* Win platform, unicode encoding */
		...
		int u_len = len/2;
		for(i = 0; i < u_len; i++)
			size += _cairo_ucs4_to_utf8(be16_to_cpu(u[i]), 0);
		utf8 = (char *)SAlloc::M(size + 1);
		if(utf8 == NULL) {
			status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
			goto fail;
		}
