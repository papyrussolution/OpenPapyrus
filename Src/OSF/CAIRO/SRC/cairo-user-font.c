...
static cairo_surface_t * _cairo_user_scaled_font_create_recording_surface(const cairo_user_scaled_font_t * scaled_font)
{
	cairo_content_t content = scaled_font->base.options.antialias == CAIRO_ANTIALIAS_SUBPIXEL ?
	          CAIRO_CONTENT_COLOR_ALPHA : CAIRO_CONTENT_ALPHA;
	return cairo_recording_surface_create(content, NULL);
}

static cairo_t * _cairo_user_scaled_font_create_recording_context(const cairo_user_scaled_font_t * scaled_font,
...
static cairo_int_status_t _cairo_user_scaled_glyph_init(void * abstract_font,
	cairo_scaled_glyph_t * scaled_glyph, cairo_scaled_glyph_info_t info)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_user_scaled_font_t * scaled_font = (cairo_user_scaled_font_t *)abstract_font;
	...
	if(!scaled_glyph->recording_surface) {
		...
		if(scaled_font->base.options.hint_metrics != CAIRO_HINT_METRICS_OFF) {
			extents.x_advance =
			        _cairo_lround(extents.x_advance/scaled_font->snap_x_scale)*scaled_font->snap_x_scale;
			extents.y_advance =
			        _cairo_lround(extents.y_advance/scaled_font->snap_y_scale)*scaled_font->snap_y_scale;
		}
...
static unsigned long _cairo_user_ucs4_to_index(void * abstract_font,
	uint32_t ucs4)
{
	cairo_user_scaled_font_t * scaled_font = (cairo_user_scaled_font_t *)abstract_font;
	cairo_user_font_face_t * face = (cairo_user_font_face_t *)scaled_font->base.font_face;
	...
	if(face->scaled_font_methods.unicode_to_glyph) {
		cairo_status_t status = face->scaled_font_methods.unicode_to_glyph(&scaled_font->base, ucs4, &glyph);
...
static cairo_int_status_t _cairo_user_text_to_glyphs(void * abstract_font,
	double x,
	double y,
	const char * utf8,
	int utf8_len,
	...
	cairo_text_cluster_flags_t * cluster_flags)
{
	...
	if(face->scaled_font_methods.text_to_glyphs) {
		...
		status = face->scaled_font_methods.text_to_glyphs(&scaled_font->base,
			utf8, utf8_len, glyphs, num_glyphs, clusters, num_clusters, cluster_flags);
		if(status != CAIRO_STATUS_SUCCESS && status != CAIRO_STATUS_USER_FONT_NOT_IMPLEMENTED)
			return status;
...
static cairo_status_t _cairo_user_font_face_scaled_font_create(void * abstract_face, const cairo_matrix_t * font_matrix,
	const cairo_matrix_t * ctm, const cairo_font_options_t * options,
	cairo_scaled_font_t ** scaled_font);

static cairo_status_t _cairo_user_font_face_create_for_toy(cairo_toy_font_face_t * toy_face,
...
static cairo_status_t _cairo_user_font_face_scaled_font_create(void * abstract_face,
	...
	cairo_scaled_font_t ** scaled_font)
{
	...
	cairo_user_font_face_t * font_face = (cairo_user_font_face_t *)abstract_face;
	...
	status = _cairo_scaled_font_init(&user_scaled_font->base,
		&font_face->base, font_matrix, ctm, options, &_cairo_user_scaled_font_backend);
	if(unlikely(status)) {
