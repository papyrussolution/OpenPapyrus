...
static cairo_t * _cairo_user_scaled_font_create_recording_context(const cairo_user_scaled_font_t * scaled_font, cairo_surface_t * recording_surface)
{
	...
	if(!_cairo_matrix_is_scale_0(&scaled_font->base.scale)) {
		cairo_matrix_t scale = scaled_font->base.scale;
		scale.x0 = scale.y0 = 0.;
    ...
    cairo_scaled_glyph_info_t info)
{
	...
	if(!scaled_glyph->recording_surface) {
		...
		if(!_cairo_matrix_is_scale_0(&scaled_font->base.scale)) {
			...
			status = face->scaled_font_methods.render_glyph((cairo_scaled_font_t*)scaled_font,
			    _cairo_scaled_glyph_index(scaled_glyph), cr, &extents);
	...
	if(info & CAIRO_SCALED_GLYPH_INFO_SURFACE) {
		 ...
		 * rendering first.
		 */
		width = _cairo_fixed_integer_ceil(scaled_glyph->bbox.p2.x) - _cairo_fixed_integer_floor(scaled_glyph->bbox.p1.x);
		height = _cairo_fixed_integer_ceil(scaled_glyph->bbox.p2.y) - _cairo_fixed_integer_floor(scaled_glyph->bbox.p1.y);
		...
		_cairo_scaled_glyph_set_surface(scaled_glyph, &scaled_font->base, (cairo_image_surface_t*)surface);
	}
