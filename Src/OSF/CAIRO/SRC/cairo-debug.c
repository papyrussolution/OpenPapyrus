...
void _cairo_debug_check_image_surface_is_defined(const cairo_surface_t * surface)
{
	...
	bits = image->data;
	switch(image->format) {
	    case CAIRO_FORMAT_A1: width = (image->width+7)/8; break;
	    case CAIRO_FORMAT_A8: width = image->width; break;
	    case CAIRO_FORMAT_RGB16_565: width = image->width*2; break;
	    case CAIRO_FORMAT_RGB24:
	    case CAIRO_FORMAT_ARGB32: width = image->width*4; break;
...
void _cairo_debug_print_path(FILE * stream, cairo_path_fixed_t * path)
{
	...
	status = _cairo_path_fixed_interpret(path, CAIRO_DIRECTION_FORWARD, _print_move_to, _print_line_to, _print_curve_to, _print_close, stream);
