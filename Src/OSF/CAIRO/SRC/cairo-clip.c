...
static cairo_clip_path_t * _cairo_clip_path_reference(cairo_clip_path_t * clip_path)
{
...
static void _cairo_clip_path_destroy(cairo_clip_path_t * clip_path)
{
	assert(CAIRO_REFERENCE_COUNT_HAS_REFERENCE(&clip_path->ref_count));
...
void _cairo_clip_init(cairo_clip_t * clip)
{
	clip->all_clipped = FALSE;
	clip->path = NULL;
}

static void _cairo_clip_set_all_clipped(cairo_clip_t * clip)
{
...
static cairo_status_t _cairo_clip_intersect_rectangle(cairo_clip_t * clip, const cairo_rectangle_int_t * rect)
{
	cairo_clip_path_t * clip_path;
	cairo_status_t status;
	if(clip->path != NULL) {
		if(rect->x <= clip->path->extents.x && rect->y <= clip->path->extents.y &&
	...
	status = _cairo_path_fixed_move_to(&clip_path->path, _cairo_fixed_from_int(rect->x), _cairo_fixed_from_int(rect->y));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(&clip_path->path, _cairo_fixed_from_int(rect->width), _cairo_fixed_from_int(0));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(&clip_path->path, _cairo_fixed_from_int(0), _cairo_fixed_from_int(rect->height));
	assert(status == CAIRO_STATUS_SUCCESS);
	status = _cairo_path_fixed_rel_line_to(&clip_path->path, _cairo_fixed_from_int(-rect->width), _cairo_fixed_from_int(0));
	assert(status == CAIRO_STATUS_SUCCESS);
	...
	if(clip_path->prev != NULL) {
		if(!_cairo_rectangle_intersect(&clip_path->extents, &clip_path->prev->extents)) {
...
cairo_clip_t * _cairo_clip_init_copy(cairo_clip_t * clip, cairo_clip_t * other)
{
	if(other != NULL) {
...
void _cairo_clip_reset(cairo_clip_t * clip)
{
	clip->all_clipped = FALSE;
	if(clip->path != NULL) {
		_cairo_clip_path_destroy(clip->path);
...
static cairo_status_t _cairo_clip_intersect_path(cairo_clip_t * clip,
	const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule,
	double tolerance, cairo_antialias_t antialias)
{
	...
	if(clip->path != NULL) {
		if(clip->path->fill_rule == fill_rule && (path->is_rectilinear || tolerance == clip->path->tolerance) &&
		   antialias == clip->path->antialias && _cairo_path_fixed_equal(&clip->path->path, path)) {
			return CAIRO_STATUS_SUCCESS;
		}
	}
...
{
	const cairo_clip_path_t * clip_path_a = clip_a->path;
	const cairo_clip_path_t * clip_path_b = clip_b->path;
...
cairo_status_t _cairo_clip_clip(cairo_clip_t * clip,
	const cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule,
	double tolerance, cairo_antialias_t antialias)
{
	if(clip->all_clipped)
...
cairo_status_t _cairo_clip_rectangle(cairo_clip_t * clip, const cairo_rectangle_int_t * rectangle)
{
	if(clip->all_clipped)
		return CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _cairo_clip_path_reapply_clip_path_transform(cairo_clip_t * clip,
	cairo_clip_path_t * other_path, const cairo_matrix_t * matrix)
{
	cairo_status_t status;
	cairo_clip_path_t * clip_path;
	cairo_bool_t is_empty;
	if(other_path->prev != NULL) {
		status = _cairo_clip_path_reapply_clip_path_transform(clip, other_path->prev, matrix);
		if(unlikely(status))
			return status;
	}
	clip_path = _cairo_clip_path_create(clip);
	...
	_cairo_path_fixed_approximate_clip_extents(&clip_path->path, &clip_path->extents);
	if(clip_path->prev != NULL) {
		is_empty = _cairo_rectangle_intersect(&clip_path->extents, &clip_path->prev->extents);
	}
...
static cairo_status_t _cairo_clip_path_reapply_clip_path_translate(cairo_clip_t * clip,
	cairo_clip_path_t * other_path, int tx, int ty)
{
	cairo_status_t status;
	cairo_clip_path_t * clip_path;
	if(other_path->prev != NULL) {
		status = _cairo_clip_path_reapply_clip_path_translate(clip, other_path->prev, tx, ty);
		if(unlikely(status))
			return status;
	}
	clip_path = _cairo_clip_path_create(clip);
	...
	_cairo_path_fixed_translate(&clip_path->path, _cairo_fixed_from_int(tx), _cairo_fixed_from_int(ty));
...
cairo_status_t _cairo_clip_init_copy_transformed(cairo_clip_t * clip, cairo_clip_t * other, const cairo_matrix_t * matrix)
{
...
static cairo_status_t _cairo_clip_apply_clip_path(cairo_clip_t * clip,
...
cairo_status_t _cairo_clip_apply_clip(cairo_clip_t * clip,
...
static inline cairo_bool_t _clip_paths_are_rectilinear(cairo_clip_path_t * clip_path)
{
...
static cairo_int_status_t _cairo_clip_path_to_region_geometric(cairo_clip_path_t * clip_path)
{
	cairo_traps_t traps;
	...
	if(traps.num_traps > ARRAY_LENGTH(stack_boxes)) {
		boxes = (cairo_box_t *)_cairo_malloc_ab(traps.num_traps, sizeof(cairo_box_t));
	...
	status = _cairo_path_fixed_fill_to_traps(&clip_path->prev->path,
		clip_path->prev->fill_rule, clip_path->prev->tolerance, &traps);
...
static cairo_int_status_t _cairo_clip_path_to_region(cairo_clip_path_t * clip_path)
{
...
static cairo_status_t _region_clip_to_boxes(const cairo_region_t * region,
	cairo_box_t ** boxes,
	int * num_boxes,
	int * size_boxes)
{
	...
	if(traps.num_traps > n) {
		cairo_box_t * new_boxes = (cairo_box_t *)_cairo_malloc_ab(traps.num_traps, sizeof(cairo_box_t));
...
static cairo_status_t _rectilinear_clip_to_boxes(const cairo_path_fixed_t * path,
...
BOXES:
			...
			if(traps.num_traps > i) {
				...
				new_boxes = (cairo_box_t *)_cairo_malloc_ab(new_size, sizeof(cairo_box_t));
				if(unlikely(new_boxes == NULL)) {
					status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
					goto CLEANUP;
				}
...
static cairo_int_status_t _cairo_clip_path_to_boxes(cairo_clip_path_t * clip_path,
	cairo_box_t ** boxes,
	int * count)
{
	int size = -*count;
	...
	if(clip_path->region != NULL) {
		...
		if(num_rects > -size) {
			cairo_box_t * new_boxes = (cairo_box_t *)_cairo_malloc_ab(num_rects, sizeof(cairo_box_t));
			if(unlikely(new_boxes == NULL))
				return _cairo_error(CAIRO_STATUS_NO_MEMORY);
			*boxes = new_boxes;
		}
...
static cairo_surface_t * _cairo_clip_path_get_surface(cairo_clip_path_t * clip_path,
...
cairo_bool_t _cairo_clip_contains_rectangle(cairo_clip_t * clip, const cairo_rectangle_int_t * rect)
{
	cairo_clip_path_t * clip_path;
	if(clip == NULL)
		return FALSE;
...
cairo_bool_t _cairo_clip_contains_extents(cairo_clip_t * clip, const cairo_composite_rectangles_t * extents)
{
...
void _cairo_debug_print_clip(FILE * stream, cairo_clip_t * clip)
{
	cairo_clip_path_t * clip_path;
	if(clip == NULL) {
...
cairo_surface_t * _cairo_clip_get_surface(cairo_clip_t * clip, cairo_surface_t * target, int * tx, int * ty)
{
...
cairo_status_t _cairo_clip_combine_with_surface(cairo_clip_t * clip, cairo_surface_t * dst, int dst_x, int dst_y)
{
	...
	do {
		if(clip_path->surface != NULL && clip_path->surface->backend == dst->backend) {
			cairo_surface_pattern_t pattern;
			_cairo_pattern_init_for_surface(&pattern, clip_path->surface);
			cairo_matrix_init_translate(&pattern.base.matrix, dst_x-clip_path->extents.x, dst_y-clip_path->extents.y);
			pattern.base.filter = CAIRO_FILTER_NEAREST;
			status = _cairo_surface_paint(dst, CAIRO_OPERATOR_IN, &pattern.base, NULL);
			_cairo_pattern_fini(&pattern.base);
			return status;
		}
		if(clip_path->flags&CAIRO_CLIP_PATH_IS_BOX && clip_path->path.maybe_fill_region) {
			continue;
		}
		if(need_translate) {
			_cairo_path_fixed_translate(&clip_path->path, _cairo_fixed_from_int(-dst_x), _cairo_fixed_from_int(-dst_y));
		}
...
const cairo_rectangle_int_t * _cairo_clip_get_extents(const cairo_clip_t * clip)
{
	return (clip->all_clipped) ? (&_cairo_empty_rectangle_int) : (clip->path ? (&clip->path->extents) : NULL);
}

void _cairo_clip_drop_cache(cairo_clip_t * clip)
{
	cairo_clip_path_t * clip_path;
	if(clip->path == NULL)
		return;
...
static cairo_bool_t _cairo_clip_int_rect_to_user(cairo_gstate_t * gstate,
	cairo_rectangle_int_t * clip_rect, cairo_rectangle_t * user_rect)
{
...
cairo_int_status_t _cairo_clip_get_region(cairo_clip_t * clip, cairo_region_t ** region)
{
	cairo_int_status_t status;
	if(clip->all_clipped)
		goto CLIPPED;
...
cairo_int_status_t _cairo_clip_get_boxes(cairo_clip_t * clip, cairo_box_t ** boxes, int * count)
{
	cairo_int_status_t status;
	if(clip->all_clipped)
		return CAIRO_INT_STATUS_NOTHING_TO_DO;
...
static cairo_bool_t box_is_aligned(const cairo_box_t * box)
{
	return _cairo_fixed_is_integer(box->p1.x) && _cairo_fixed_is_integer(box->p1.y) &&
		_cairo_fixed_is_integer(box->p2.x) && _cairo_fixed_is_integer(box->p2.y);
}

static void intersect_with_boxes(cairo_composite_rectangles_t * extents, cairo_box_t * boxes, int num_boxes)
{
	cairo_rectangle_int_t rect;
	cairo_box_t box;
	cairo_bool_t is_empty;
...
cairo_status_t _cairo_clip_to_boxes(cairo_clip_t ** clip, cairo_composite_rectangles_t * extents, cairo_box_t ** boxes, int * num_boxes)
{
	cairo_status_t status;
	...
	switch((int)status) {
		...
		if(rect->width == 0 || rect->height == 0 || extents->is_bounded || (*num_boxes == 1 && box_is_aligned(*boxes))) {
			*clip = NULL;
		}
		goto DONE;
...
static cairo_rectangle_list_t * _cairo_rectangle_list_create_in_error(cairo_status_t status)
{
