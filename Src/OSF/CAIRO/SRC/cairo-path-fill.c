...
static void _cairo_filler_init(cairo_filler_t * filler, double tolerance, cairo_polygon_t * polygon)
{
	filler->tolerance = tolerance;
	filler->polygon = polygon;
}

static void _cairo_filler_fini(cairo_filler_t * filler)
{
}

static cairo_status_t _cairo_filler_move_to(void * closure, const cairo_point_t * point)
{
	cairo_filler_t * filler = (cairo_filler_t *)closure;
	cairo_polygon_t * polygon = filler->polygon;
	return (cairo_status_t)(_cairo_polygon_close(polygon) || _cairo_polygon_move_to(polygon, point));
}

static cairo_status_t _cairo_filler_line_to(void * closure, const cairo_point_t * point)
{
	cairo_filler_t * filler = (cairo_filler_t *)closure;
	...
	const cairo_point_t * b, const cairo_point_t * c, const cairo_point_t * d)
{
	...
	if(!_cairo_spline_init(&spline, _cairo_filler_line_to, filler, &filler->polygon->current_point, b, c, d)) {
		return _cairo_filler_line_to(closure, d);
	}
	return _cairo_spline_decompose(&spline, filler->tolerance);
}

static cairo_status_t _cairo_filler_close_path(void * closure)
{
	cairo_filler_t * filler = (cairo_filler_t *)closure;
...
cairo_status_t _cairo_path_fixed_fill_to_traps(const cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule, double tolerance, cairo_traps_t * traps)
{
	cairo_polygon_t polygon;
	cairo_status_t status;
	...
	if(path->is_rectilinear) {
		status = _cairo_bentley_ottmann_tessellate_rectilinear_polygon(traps, &polygon, fill_rule);
	}
	else {
...
static cairo_region_t * _cairo_path_fixed_fill_rectilinear_tessellate_to_region(const cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule, const cairo_rectangle_int_t * extents)
{
	cairo_box_t box;
	cairo_polygon_t polygon;
	cairo_traps_t traps;
	...
	status = _cairo_path_fixed_fill_rectilinear_to_traps(path, fill_rule, &traps);
	...
	cairo_fill_rule_t fill_rule, const cairo_rectangle_int_t * extents)
{
	...
	if(_cairo_path_fixed_is_box(path, &box)) {
		...
		rectangle_stack[0].width = _cairo_fixed_integer_part(box.p2.x)-rectangle_stack[0].x;
		rectangle_stack[0].height = _cairo_fixed_integer_part(box.p2.y)-rectangle_stack[0].y;
		...
		while(_cairo_path_fixed_iter_is_fill_box(&iter, &box)) {
			...
			if(count == size) {
				...
				if(rects == rectangle_stack) {
					new_rects = (cairo_rectangle_int_t *)_cairo_malloc_ab(size, sizeof(cairo_rectangle_int_t));
					...
					new_rects = (cairo_rectangle_int_t *)_cairo_realloc_ab(rects, size, sizeof(cairo_rectangle_int_t));
					if(unlikely(new_rects == NULL)) {
						/* XXX _cairo_region_nil */
						break;
					}
...
TESSELLATE:
	...
	if(region == NULL) {
		/* Hmm, complex polygon */
		region = _cairo_path_fixed_fill_rectilinear_tessellate_to_region(path, fill_rule, extents);
	}
	return region;
}

cairo_int_status_t _cairo_path_fixed_fill_rectilinear_to_traps(const cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule, cairo_traps_t * traps)
{
	cairo_box_t box;
	cairo_status_t status;
	...
	if(_cairo_path_fixed_is_box(path, &box)) {
		...
		while(_cairo_path_fixed_iter_is_fill_box(&iter, &box)) {
			...
			status = _cairo_traps_tessellate_rectangle(traps, &box.p1, &box.p2);
			if(unlikely(status)) {
...
static cairo_status_t _cairo_path_fixed_fill_rectilinear_tessellate_to_boxes(const cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule, cairo_boxes_t * boxes)
{
	cairo_polygon_t polygon;
	cairo_status_t status;
	_cairo_polygon_init(&polygon);
	...
	cairo_fill_rule_t fill_rule, cairo_boxes_t * boxes)
{
	...
	return _cairo_path_fixed_fill_rectilinear_tessellate_to_boxes(path, fill_rule, boxes);
}
