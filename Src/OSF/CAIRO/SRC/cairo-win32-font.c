...
static inline HDC _get_global_font_dc()
{
	return SLS.GetTLA().GetFontDC();
	/*
	static HDC hdc;
	if(!hdc) {
...
static BYTE _get_system_quality(void)
{
	...
	if(!SystemParametersInfo(SPI_GETFONTSMOOTHING, 0, &font_smoothing, 0)) {
		...
		if(_have_cleartype_quality()) {
			...
			else if(smoothing_type == FE_FONTSMOOTHINGCLEARTYPE)
...
// all be 0, and face_hfont is the result of calling CreateFontIndirectW on logfont.
//
static cairo_status_t _win32_scaled_font_create(LOGFONTW * logfont, HFONT face_hfont,
	cairo_font_face_t * font_face, const cairo_matrix_t * font_matrix, const cairo_matrix_t * ctm,
	const cairo_font_options_t * options, cairo_scaled_font_t ** font_out)
{
	...
	f = (cairo_win32_scaled_font_t *)malloc(sizeof(cairo_win32_scaled_font_t));
	if(f == NULL)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	f->logfont = *logfont;
	...
	if(options->antialias == CAIRO_ANTIALIAS_DEFAULT)
		...
		switch(options->antialias) {
		    ...
		    case CAIRO_ANTIALIAS_GRAY: f->quality = ANTIALIASED_QUALITY; break;
		    case CAIRO_ANTIALIAS_SUBPIXEL:
		    	f->quality = _have_cleartype_quality() ? CLEARTYPE_QUALITY : ANTIALIASED_QUALITY;
				break;
...
static cairo_status_t _win32_scaled_font_get_unscaled_hfont(cairo_win32_scaled_font_t * scaled_font, HDC hdc, HFONT * hfont_out)
{
	if(scaled_font->unscaled_hfont == NULL) {
		...
		HFONT scaled_hfont;
		LOGFONTW logfont;
		cairo_status_t status = _win32_scaled_font_get_scaled_hfont(scaled_font, &scaled_hfont);
		if(status)
			return status;
		...
		otm = (OUTLINETEXTMETRIC *)malloc(otm_size);
		if(otm == NULL)
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_status_t _cairo_win32_font_face_create_for_toy(cairo_toy_font_face_t * toy_face,
	...
	double x, double y, const char * utf8, cairo_glyph_t ** glyphs, int * num_glyphs)
{
	...
	HDC hdc = NULL;
	cairo_matrix_t mat;
	cairo_status_t status = _cairo_utf8_to_utf16(utf8, -1, &utf16, &n16);
	if(status)
		return status;
...
static cairo_status_t _cairo_win32_scaled_font_init_glyph_metrics(cairo_win32_scaled_font_t * scaled_font, cairo_scaled_glyph_t * scaled_glyph)
{
	...
	if(scaled_font->is_bitmap) {
		...
		extents.height = scaled_font->base.ctm.yy*
		                 (font_extents.ascent+font_extents.descent)/scaled_font->y_scale;
 ...
 */
#if 0
static cairo_status_t _cairo_win32_scaled_font_glyph_bbox(void * abstract_font,
...
static cairo_status_t _flush_glyphs(cairo_glyph_state_t * state)
{
	...
	dx_elements = (int *)_cairo_array_index(&state->dx, 0);
	if(!ExtTextOutW(state->hdc, state->start_x, state->last_y, ETO_GLYPH_INDEX,
		NULL, elements, state->glyphs.num_elements, dx_elements)) {
...
static cairo_status_t _add_glyph(cairo_glyph_state_t * state,
	unsigned long index, double device_x, double device_y)
{
	cairo_status_t status;
...
static cairo_status_t _draw_glyphs_on_surface(cairo_win32_surface_t * surface,
	cairo_win32_scaled_font_t * scaled_font, COLORREF color, int x_offset, int y_offset,
 ...
 */
static void _compute_argb32_mask_alpha(cairo_win32_surface_t * mask_surface)
{
...
/* Invert a mask
 */
static void _invert_argb32_mask(cairo_win32_surface_t * mask_surface)
{
	cairo_image_surface_t * image = (cairo_image_surface_t *)mask_surface->image;
	int i, j;
 ...
 */
static cairo_surface_t * _compute_a8_mask(cairo_win32_surface_t * mask_surface)
{
...
static cairo_int_status_t _cairo_win32_scaled_font_glyph_init(void * abstract_font,
...
static cairo_int_status_t _cairo_win32_scaled_font_show_glyphs(void * abstract_font,
	cairo_operator_t op,
	...
	int * remaining_glyphs)
{
	...
	if(_cairo_surface_is_win32(generic_surface) &&
	   ...
	   _cairo_pattern_is_opaque_solid(pattern)) {
		...
		tmp_surface = (cairo_win32_surface_t *)cairo_win32_surface_create_with_dib(CAIRO_FORMAT_ARGB32, width,
			height);
		...
		r.right = width;
		r.bottom = height;
		FillRect(tmp_surface->dc, &r, (HBRUSH)GetStockObject(WHITE_BRUSH));
		status = _draw_glyphs_on_surface(tmp_surface, scaled_font, RGB(0, 0, 0), dest_x, dest_y, glyphs, num_glyphs);
		if(status) {
...
static cairo_status_t _cairo_win32_scaled_font_init_glyph_surface(cairo_win32_scaled_font_t * scaled_font,
	cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_status_t status;
	cairo_glyph_t glyph;
	...
	int width = x2-x1;
	int height = y2-y1;
	cairo_win32_surface_t * surface = (cairo_win32_surface_t *)cairo_win32_surface_create_with_dib(CAIRO_FORMAT_RGB24, width, height);
	cairo_t * cr = cairo_create(&surface->base);
	...
	glyph.x = -x1;
	glyph.y = -y1;
	status = _draw_glyphs_on_surface(surface, scaled_font, RGB(0, 0, 0), 0, 0, &glyph, 1);
	if(status)
		goto FAIL;
	GdiFlush();
...
static void _cairo_win32_transform_FIXED_to_fixed(cairo_matrix_t * matrix,
	FIXED Fx, FIXED Fy, cairo_fixed_t * fx, cairo_fixed_t * fy)
{
...
static cairo_status_t _cairo_win32_scaled_font_init_glyph_path(cairo_win32_scaled_font_t * scaled_font,
	cairo_scaled_glyph_t * scaled_glyph)
{
	static const MAT2 matrix = { { 0, 1 }, { 0, 0 }, { 0, 0 }, { 0, -1 } };
	cairo_status_t status;
	...
	ptr = buffer = (unsigned char *)malloc(bytesGlyph);
	if(!buffer) {
		status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
		goto CLEANUP_FONT;
	}
	if(GetGlyphOutlineW(hdc, _cairo_scaled_glyph_index(scaled_glyph),
		   GGO_NATIVE|GGO_GLYPH_INDEX, &metrics, bytesGlyph, buffer, &matrix) == GDI_ERROR) {
		status = _cairo_win32_print_gdi_error("_cairo_win32_scaled_font_glyph_path");
		goto CLEANUP_BUFFER;
	}
		...
		while(ptr < endPoly) {
			...
			switch(curve->wType) {
				...
				for(i = 0; i < curve->cpfx-2; i += 2) {
					...
					_cairo_win32_transform_FIXED_to_fixed(&transform, points[i].x, points[i].y, &x, &y);
					_cairo_win32_transform_FIXED_to_fixed(&transform, points[i+1].x, points[i+1].y, &x1, &y1);
					_cairo_win32_transform_FIXED_to_fixed(&transform, points[i+2].x, points[i+2].y, &x2, &y2);
...
static void _cairo_win32_font_face_destroy(void * abstract_face)
{
}

static cairo_bool_t _is_scale(const cairo_matrix_t * matrix, double scale)
{
	return matrix->xx == scale && matrix->yy == scale && matrix->xy == 0. && matrix->yx == 0. &&
...
{
	cairo_win32_font_face_t * font_face = (cairo_win32_font_face_t *)malloc(sizeof(cairo_win32_font_face_t));
	if(!font_face) {
