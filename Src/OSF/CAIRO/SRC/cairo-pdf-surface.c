...
static cairo_status_t _cairo_pdf_surface_add_font(unsigned int font_id, unsigned int subset_id, void * closure);
static void _cairo_pdf_group_resources_init(cairo_pdf_group_resources_t * res);
static cairo_status_t _cairo_pdf_surface_open_stream(cairo_pdf_surface_t * surface, cairo_pdf_resource_t * resource,
	cairo_bool_t compressed, const char * fmt, ...) CAIRO_PRINTF_FORMAT(4, 5);
static cairo_status_t _cairo_pdf_surface_close_stream(cairo_pdf_surface_t * surface);
static cairo_status_t _cairo_pdf_surface_write_page(cairo_pdf_surface_t * surface);
...
static cairo_status_t _cairo_pdf_surface_write_page(cairo_pdf_surface_t * surface);
static cairo_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface);
...
static cairo_surface_t * _cairo_pdf_surface_create_for_stream_internal(cairo_output_stream_t * output,
...
static cairo_status_t _cairo_pdf_surface_add_alpha(cairo_pdf_surface_t * surface, double alpha, int * index)
{
	int i;
	double other;
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_add_smask(cairo_pdf_surface_t * surface, cairo_pdf_resource_t smask)
{
	return _cairo_array_append(&(surface->resources.smasks), &smask);
}

static cairo_status_t _cairo_pdf_surface_add_pattern(cairo_pdf_surface_t * surface, cairo_pdf_resource_t pattern)
{
	return _cairo_array_append(&(surface->resources.patterns), &pattern);
}

static cairo_status_t _cairo_pdf_surface_add_xobject(cairo_pdf_surface_t * surface, cairo_pdf_resource_t xobject)
{
...
static cairo_pdf_resource_t _cairo_pdf_surface_get_font_resource(cairo_pdf_surface_t * surface,
	unsigned int font_id, unsigned int subset_id)
{
	cairo_pdf_font_t font;
	int i;
	int num_fonts = _cairo_array_num_elements(&surface->fonts);
	for(i = 0; i < num_fonts; i++) {
		_cairo_array_copy_element(&surface->fonts, i, &font);
		if(font.font_id == font_id && font.subset_id == subset_id)
...
static cairo_pdf_smask_group_t * _cairo_pdf_surface_create_smask_group(cairo_pdf_surface_t * surface)
{
...
static cairo_status_t _cairo_pdf_surface_add_smask_group(cairo_pdf_surface_t * surface, cairo_pdf_smask_group_t * group)
{
...
static cairo_int_status_t _get_jpx_image_info(cairo_surface_t * source,
	cairo_image_info_t * info, const unsigned char ** mime_data, unsigned long * mime_data_length)
{
...
static cairo_int_status_t _get_jpeg_image_info(cairo_surface_t * source,
...
static cairo_status_t _get_source_surface_size(cairo_surface_t * source,
	int * width,
	int * height)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_add_source_surface(cairo_pdf_surface_t * surface,
...
static cairo_bool_t _gradient_stops_are_opaque(const cairo_gradient_pattern_t * gradient)
{
	unsigned int i;
...
static cairo_status_t _cairo_pdf_surface_add_pdf_pattern(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_open_stream(cairo_pdf_surface_t * surface,
	cairo_pdf_resource_t * resource,
...
static cairo_status_t _cairo_pdf_surface_open_group(cairo_pdf_surface_t * surface,
	cairo_pdf_resource_t * resource)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_open_knockout_group(cairo_pdf_surface_t * surface)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_close_group(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_open_content_stream(cairo_pdf_surface_t * surface,
	cairo_pdf_resource_t * resource, cairo_bool_t is_form)
{
	cairo_status_t status;
	assert(surface->pdf_stream.active == FALSE);
	assert(surface->group_stream.active == FALSE);
...
static cairo_status_t _cairo_pdf_surface_close_content_stream(cairo_pdf_surface_t * surface)
{
	cairo_status_t status;
 ...
 */
static cairo_status_t _cairo_pdf_surface_emit_smask(cairo_pdf_surface_t * surface, cairo_image_surface_t * image, cairo_pdf_resource_t * stream_ret)
{
	...
	assert(image->format == CAIRO_FORMAT_ARGB32 || image->format == CAIRO_FORMAT_A8 || image->format == CAIRO_FORMAT_A1 );
	stream_ret->id = 0;
	...
	for(y = 0; y < image->height; y++) {
		if(image->format == CAIRO_FORMAT_ARGB32) {
			...
			for(x = 0; x < (image->width+7)/8; x++, pixel8++) {
				a = *pixel8;
				a = (uint8_t)(CAIRO_BITSWAP8_IF_LITTLE_ENDIAN(a));
				alpha[i++] = a;
				if(a != 0xff)
					opaque = FALSE;
			}
...
static cairo_status_t _cairo_pdf_surface_emit_image(cairo_pdf_surface_t * surface,
	...
	cairo_filter_t filter)
{
	...
	rgb = (char *)_cairo_malloc_abc(image->width, image->height, 3);
...
static cairo_status_t _cairo_pdf_surface_emit_image_surface(cairo_pdf_surface_t * surface,
	cairo_surface_t * source, cairo_pdf_resource_t resource, cairo_bool_t interpolate)
{
	cairo_image_surface_t * image;
	void * image_extra;
	cairo_status_t status = _cairo_pdf_surface_emit_jpx_image(surface, source, resource);
	if(status != CAIRO_INT_STATUS_UNSUPPORTED)
		return status;
	...
	if(unlikely(status))
		return status;
	status = _cairo_pdf_surface_emit_image(surface, image, &resource, interpolate ? CAIRO_FILTER_GOOD : CAIRO_FILTER_FAST);
	if(unlikely(status))
		goto BAIL;
BAIL:
...
static cairo_status_t _cairo_pdf_surface_emit_padded_image_surface(cairo_pdf_surface_t * surface,
	cairo_pdf_pattern_t * pdf_pattern,
	cairo_pdf_resource_t * resource,
	int * width,
	int * height,
	...
	int * origin_y)
{
	...
	int y = 0;
	cairo_bool_t interpolate;
	cairo_status_t status = _cairo_surface_acquire_source_image(pattern->surface, &image, &image_extra);
	if(unlikely(status))
		return status;
	...
	*resource = _cairo_pdf_surface_new_object(surface);
	...
	status = _cairo_pdf_surface_emit_image(surface, (cairo_image_surface_t *)pad_image, resource, interpolate ? CAIRO_FILTER_GOOD : CAIRO_FILTER_FAST);
	if(unlikely(status))
		goto BAIL;
...
static cairo_status_t _cairo_pdf_surface_emit_recording_surface(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_emit_recording_subsurface(cairo_pdf_surface_t * surface,
	cairo_surface_t * recording_surface,
	const cairo_rectangle_int_t * extents,
	cairo_pdf_resource_t resource)
{
...
static cairo_status_t _cairo_pdf_surface_emit_surface(cairo_pdf_surface_t * surface,
	cairo_pdf_source_surface_t * src_surface)
{
...
static cairo_status_t _cairo_pdf_surface_emit_surface_pattern(cairo_pdf_surface_t * surface,
	cairo_pdf_pattern_t * pdf_pattern)
{
	cairo_surface_pattern_t * pattern = (cairo_surface_pattern_t *)pdf_pattern->pattern;
	cairo_status_t status;
...
static cairo_status_t cairo_pdf_surface_emit_rgb_linear_function(cairo_pdf_surface_t * surface,
...
static cairo_status_t cairo_pdf_surface_emit_alpha_linear_function(cairo_pdf_surface_t * surface,
	cairo_pdf_color_stop_t * stop1,
	cairo_pdf_color_stop_t * stop2,
	cairo_pdf_resource_t * function)
{
...
static cairo_status_t _cairo_pdf_surface_emit_stitched_colorgradient(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_emit_pattern_stops(cairo_pdf_surface_t * surface,
	cairo_gradient_pattern_t * pattern, cairo_pdf_resource_t * color_function, cairo_pdf_resource_t * alpha_function)
{
	...
	if(n_stops <= 2) {
		...
		status = cairo_pdf_surface_emit_rgb_linear_function(surface,
			&stops[0], &stops[n_stops-1], color_function);
		if(unlikely(status))
			goto BAIL;
		if(emit_alpha) {
			status = cairo_pdf_surface_emit_alpha_linear_function(surface,
				&stops[0], &stops[n_stops-1], alpha_function);
			if(unlikely(status))
				goto BAIL;
		}
	}
...
static cairo_status_t _cairo_pdf_surface_emit_repeating_function(cairo_pdf_surface_t * surface,
...
static cairo_status_t cairo_pdf_surface_emit_transparency_group(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_emit_linear_pattern(cairo_pdf_surface_t * surface,
	cairo_pdf_pattern_t * pdf_pattern)
{
...
static cairo_status_t _cairo_pdf_surface_emit_radial_pattern(cairo_pdf_surface_t * surface,
	cairo_pdf_pattern_t * pdf_pattern)
{
	cairo_pdf_resource_t color_function, alpha_function;
...
static cairo_status_t _cairo_pdf_surface_emit_pattern(cairo_pdf_surface_t * surface, cairo_pdf_pattern_t * pdf_pattern)
{
	double old_width, old_height;
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_paint_surface_pattern(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_select_operator(cairo_pdf_surface_t * surface,
	cairo_operator_t op)
{
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_select_pattern(cairo_pdf_surface_t * surface,
	const cairo_pattern_t * pattern,
	cairo_pdf_resource_t pattern_res,
...
static cairo_bool_t _cairo_pdf_surface_get_extents(void * abstract_surface, cairo_rectangle_int_t * rectangle)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	rectangle->x = 0;
	rectangle->y = 0;
...
static cairo_status_t _cairo_pdf_surface_emit_unicode_for_glyph(cairo_pdf_surface_t * surface, const char * utf8)
{
	uint16_t * utf16 = NULL;
	int utf16_len = 0;
	cairo_status_t status;
...
static uint32_t _hash_data(const unsigned char * data, int length, uint32_t initval)
{
	uint32_t a, b, c, len;
...
static void _create_font_subset_tag(cairo_scaled_font_subset_t * font_subset, const char * font_name, char * tag)
{
	...
	hash = _hash_data((unsigned char *)(font_subset->glyphs), font_subset->num_glyphs*sizeof(unsigned long), hash);
	numerator = labs((long)hash);
	for(i = 0; i < 6; i++) {
		d = ldiv(numerator, 26);
		numerator = d.quot;
		tag[i] = (char)('A'+d.rem);
...
static cairo_int_status_t _cairo_pdf_surface_emit_to_unicode_stream(cairo_pdf_surface_t * surface,
...
static cairo_status_t _cairo_pdf_surface_emit_cff_font(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset,
...
static cairo_status_t _cairo_pdf_surface_emit_cff_font_subset(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_status_t status;
	cairo_cff_subset_t subset;
...
static cairo_status_t _cairo_pdf_surface_emit_cff_fallback_font(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_status_t status;
	cairo_cff_subset_t subset;
...
static cairo_status_t _cairo_pdf_surface_emit_type1_font(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset,
...
static cairo_status_t _cairo_pdf_surface_emit_type1_font_subset(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_status_t status;
	cairo_type1_subset_t subset;
...
static cairo_status_t _cairo_pdf_surface_emit_type1_fallback_font(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_status_t status;
	cairo_type1_subset_t subset;
...
static cairo_status_t _cairo_pdf_surface_emit_truetype_font_subset(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_pdf_resource_t stream, descriptor, cidfont_dict;
	cairo_pdf_resource_t subset_resource, to_unicode_stream;
	...
	status = _cairo_pdf_surface_emit_to_unicode_stream(surface, font_subset, TRUE, &to_unicode_stream);
	if(_cairo_status_is_error(status)) {
		_cairo_truetype_subset_fini(&subset);
		return status;
	}
...
static cairo_status_t _cairo_pdf_emit_imagemask(cairo_image_surface_t * image, cairo_output_stream_t * stream)
{
...
static cairo_status_t _cairo_pdf_surface_analyze_user_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_status_t status2;
	...
	cairo_surface_t * type3_surface = _cairo_type3_glyph_surface_create(font_subset->scaled_font,
		null_stream, _cairo_pdf_emit_imagemask, surface->font_subsets);
...
static cairo_status_t _cairo_pdf_surface_emit_type3_font_subset(cairo_pdf_surface_t * surface,
	cairo_scaled_font_subset_t * font_subset)
{
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _cairo_pdf_surface_emit_unscaled_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_status_t status;
...
static cairo_status_t _cairo_pdf_surface_emit_scaled_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_status_t status = _cairo_pdf_surface_emit_type3_font_subset(surface, font_subset);
	if(status != CAIRO_INT_STATUS_UNSUPPORTED)
		return status;
	ASSERT_NOT_REACHED;
	return CAIRO_STATUS_SUCCESS;
...
static cairo_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface)
{
	cairo_status_t status = _cairo_scaled_font_subsets_foreach_user(surface->font_subsets,
		_cairo_pdf_surface_analyze_user_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
	status = _cairo_scaled_font_subsets_foreach_unscaled(surface->font_subsets,
		_cairo_pdf_surface_emit_unscaled_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
	status = _cairo_scaled_font_subsets_foreach_scaled(surface->font_subsets,
		_cairo_pdf_surface_emit_scaled_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
...
static cairo_status_t _cairo_pdf_surface_write_mask_group(cairo_pdf_surface_t * surface,
	cairo_pdf_smask_group_t * group)
{
	...
	/* Create mask group */
	cairo_status_t status = _cairo_pdf_surface_open_group(surface, NULL);
	if(unlikely(status))
		return status;
	pattern_res.id = 0;
	gstate_res.id = 0;
...
static cairo_status_t _cairo_pdf_surface_write_smask_group(cairo_pdf_surface_t * surface,
	cairo_pdf_smask_group_t * group)
{
	cairo_status_t status;
	...
	switch(group->operation) {
		...
		status = _cairo_pdf_operators_stroke(&surface->pdf_operators,
			&group->path, &group->style, &group->ctm, &group->ctm_inverse);
		break;
		...
		status = _cairo_pdf_operators_show_text_glyphs(&surface->pdf_operators,
			...
			(cairo_text_cluster_flags_t)group->cluster_flags, // @sobolev (cairo_text_cluster_flags_t)
			group->scaled_font);
		break;
	}
	if(unlikely(status))
...
static cairo_status_t _cairo_pdf_surface_write_patterns_and_smask_groups(cairo_pdf_surface_t * surface)
{
	cairo_pdf_pattern_t pattern;
	cairo_pdf_smask_group_t * group;
	cairo_pdf_source_surface_t src_surface;
...
static cairo_bool_t _gradient_pattern_supported(const cairo_pattern_t * pattern)
{
	cairo_extend_t extend;
...
static cairo_int_status_t _cairo_pdf_surface_paint(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	cairo_pdf_smask_group_t * group;
	cairo_pdf_resource_t pattern_res, gstate_res;
	cairo_composite_rectangles_t extents;
	cairo_status_t status = _cairo_composite_rectangles_init_for_paint(&extents, (int)surface->width, (int)surface->height, op, source, clip);
	if(unlikely(status)) {
	...
	if(source->type == CAIRO_PATTERN_TYPE_SURFACE &&
	   source->extend == CAIRO_EXTEND_NONE) {
		_cairo_output_stream_printf(surface->output, "q\n");
		status = _cairo_pdf_surface_paint_surface_pattern(surface, (cairo_surface_pattern_t *)source);
		if(unlikely(status))
			return status;
		_cairo_output_stream_printf(surface->output, "Q\n");
		return _cairo_output_stream_get_status(surface->output);
	...
	pattern_res.id = 0;
	gstate_res.id = 0;
	status = _cairo_pdf_surface_add_pdf_pattern(surface, source, &extents.bounded, &pattern_res, &gstate_res);
...
static cairo_int_status_t _cairo_pdf_surface_mask(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, const cairo_pattern_t * mask, cairo_clip_t * clip)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	cairo_pdf_smask_group_t * group;
	cairo_composite_rectangles_t extents;
	cairo_status_t status = _cairo_composite_rectangles_init_for_mask(&extents, (int)surface->width, (int)surface->height, op, source, mask, clip);
	if(unlikely(status)) {
		if(status == CAIRO_INT_STATUS_NOTHING_TO_DO)
			return CAIRO_STATUS_SUCCESS;
		return status;
	...
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
		cairo_status_t mask_status;
		cairo_status_t source_status = _cairo_pdf_surface_analyze_operation(surface, op, source, &extents.bounded);
	...
	cairo_clip_t * clip)
{
	...
	cairo_pdf_resource_t pattern_res, gstate_res;
	cairo_composite_rectangles_t extents;
	cairo_status_t status = _cairo_composite_rectangles_init_for_stroke(&extents, (int)surface->width, (int)surface->height, op, source, path, style, ctm, clip);
	if(unlikely(status)) {
		if(status == CAIRO_INT_STATUS_NOTHING_TO_DO)
			return CAIRO_STATUS_SUCCESS;
		return status;
	...
	if(extents.is_bounded) {
		status = _cairo_path_fixed_stroke_extents(path, style, ctm, ctm_inverse, tolerance, &extents.mask);
		if(unlikely(status))
			return status;
	...
	cairo_clip_t * clip)
{
	...
	cairo_pdf_resource_t pattern_res, gstate_res;
	cairo_composite_rectangles_t extents;
	cairo_status_t status = _cairo_composite_rectangles_init_for_fill(&extents, (int)surface->width, (int)surface->height, op, source, path, clip);
	if(unlikely(status)) {
		if(status == CAIRO_INT_STATUS_NOTHING_TO_DO)
			return CAIRO_STATUS_SUCCESS;
		return status;
	...
	/* use the more accurate extents */
	if(extents.is_bounded) {
		_cairo_path_fixed_fill_extents(path, fill_rule, tolerance, &extents.mask);
		if(!_cairo_rectangle_intersect(&extents.bounded, &extents.mask))
			return CAIRO_STATUS_SUCCESS;
	}
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
	...
	cairo_clip_t * clip)
{
	...
	cairo_status_t status = _cairo_composite_rectangles_init_for_glyphs(&extents, (int)surface->width, (int)surface->height, op, source, scaled_font, glyphs, num_glyphs, clip, &overlap);
	if(unlikely(status)) {
		if(status == CAIRO_INT_STATUS_NOTHING_TO_DO)
			return CAIRO_STATUS_SUCCESS;
		return status;
