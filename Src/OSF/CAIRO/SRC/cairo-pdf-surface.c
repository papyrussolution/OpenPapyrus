...
static const cairo_pdf_version_t _cairo_pdf_versions[] = { CAIRO_PDF_VERSION_1_4, CAIRO_PDF_VERSION_1_5 };

#define CAIRO_PDF_VERSION_LAST SIZEOFARRAY(_cairo_pdf_versions)

static const char * _cairo_pdf_version_strings[CAIRO_PDF_VERSION_LAST] = { "PDF 1.4", "PDF 1.5" };
...
static cairo_int_status_t _cairo_pdf_surface_open_stream(cairo_pdf_surface_t * surface, cairo_pdf_resource_t * resource, cairo_bool_t compressed, const char * fmt, ...) CAIRO_PRINTF_FORMAT(4, 5);
...
static cairo_pdf_resource_t _cairo_pdf_surface_write_info(cairo_pdf_surface_t * surface);
static cairo_pdf_resource_t _cairo_pdf_surface_write_catalog(cairo_pdf_surface_t * surface);
static long _cairo_pdf_surface_write_xref(cairo_pdf_surface_t * surface);
static cairo_int_status_t _cairo_pdf_surface_write_page(cairo_pdf_surface_t * surface);
static cairo_int_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface);
static cairo_bool_t _cairo_pdf_source_surface_equal(const void * key_a, const void * key_b);

extern const cairo_surface_backend_t cairo_pdf_surface_backend;
extern const cairo_paginated_surface_backend_t cairo_pdf_surface_paginated_backend;

static cairo_pdf_resource_t FASTCALL _cairo_pdf_surface_new_object(cairo_pdf_surface_t * surface)
{
	cairo_pdf_resource_t resource;
...
static void FASTCALL _cairo_pdf_surface_update_object(cairo_pdf_surface_t * surface, cairo_pdf_resource_t resource)
{
	cairo_pdf_object_t * object = (cairo_pdf_object_t *)_cairo_array_index(&surface->objects, resource.id - 1);
	object->offset = _cairo_output_stream_get_position(surface->output);
}

static void FASTCALL _cairo_pdf_surface_set_size_internal(cairo_pdf_surface_t * surface, double width, double height)
{
	surface->width = width;
	surface->height = height;
	cairo_matrix_init(&surface->cairo_to_pdf, 1, 0, 0, -1, 0, height);
	_cairo_pdf_operators_set_cairo_to_pdf_matrix(&surface->pdf_operators, &surface->cairo_to_pdf);
}

static cairo_bool_t _path_covers_bbox(cairo_pdf_surface_t * surface, cairo_path_fixed_t * path)
{
	cairo_box_t box;
	return _cairo_path_fixed_is_box(path, &box) && box.p1.x <= 0 && box.p1.y <= 0 &&
		box.p2.x >= _cairo_fixed_from_double(surface->width) && box.p2.y >= _cairo_fixed_from_double(surface->height);
}

static cairo_status_t _cairo_pdf_surface_clipper_intersect_clip_path(cairo_surface_clipper_t * clipper,
    cairo_path_fixed_t * path, CairoFillRule fill_rule, double tolerance, cairo_antialias_t antialias)
{
	cairo_pdf_surface_t * surface = cairo_container_of(clipper, cairo_pdf_surface_t, clipper);
	cairo_int_status_t status = _cairo_pdf_operators_flush(&surface->pdf_operators);
	if(unlikely(status))
		return status;
	if(path == NULL) {
		_cairo_output_stream_puts(surface->output, "Q q\n");
		surface->current_pattern_is_solid_color = FALSE;
		_cairo_pdf_operators_reset(&surface->pdf_operators);
		return CAIRO_STATUS_SUCCESS;
	}
	return _path_covers_bbox(surface, path) ? CAIRO_STATUS_SUCCESS : _cairo_pdf_operators_clip(&surface->pdf_operators, path, fill_rule);
}
...
static cairo_surface_t * _cairo_pdf_surface_create_for_stream_internal(cairo_output_stream_t * output, double width, double height)
{
	...
	_cairo_surface_init(&surface->base, &cairo_pdf_surface_backend, NULL/* device */, CAIRO_CONTENT_COLOR_ALPHA);
	...
	_cairo_surface_clipper_init(&surface->clipper, _cairo_pdf_surface_clipper_intersect_clip_path);
	_cairo_pdf_operators_init(&surface->pdf_operators, surface->output, &surface->cairo_to_pdf, surface->font_subsets, FALSE);
	_cairo_pdf_operators_set_font_subsets_callback(&surface->pdf_operators, _cairo_pdf_surface_add_font, surface);
...
{
	cairo_output_stream_t * output = _cairo_output_stream_create(write_func, NULL, closure);
 ...
 * Since: 1.10
 **/
void cairo_pdf_surface_restrict_to_version(cairo_surface_t * abstract_surface, cairo_pdf_version_t version)
{
	...
	if(_extract_pdf_surface(abstract_surface, &surface)) {
		...
		_cairo_pdf_operators_enable_actual_text(&surface->pdf_operators, version >= CAIRO_PDF_VERSION_1_5);
	}
}
...
{
	ASSIGN_PTR(versions, _cairo_pdf_versions);
	ASSIGN_PTR(num_versions, CAIRO_PDF_VERSION_LAST);
}
...
{
	return (version >= CAIRO_PDF_VERSION_LAST) ? NULL : _cairo_pdf_version_strings[version];
}
/**
...
void cairo_pdf_surface_set_size(cairo_surface_t * surface, double width_in_points, double height_in_points)
{
	...
	if(_extract_pdf_surface(surface, &pdf_surface)) {
		...
		status = _cairo_paginated_surface_set_size(pdf_surface->paginated_surface, (int)width_in_points, (int)height_in_points);
		if(status)
...
static void _cairo_pdf_surface_clear(cairo_pdf_surface_t * surface)
{
	...
	int size = _cairo_array_num_elements(&surface->page_patterns);
	for(i = 0; i < size; i++) {
...
static void _cairo_pdf_group_resources_init(cairo_pdf_group_resources_t * res)
{
	for(int i = 0; i < CAIRO_NUM_OPERATORS; i++)
...
static void _cairo_pdf_group_resources_fini(cairo_pdf_group_resources_t * res)
{
...
static void _cairo_pdf_group_resources_clear(cairo_pdf_group_resources_t * res)
{
	for(int i = 0; i < CAIRO_NUM_OPERATORS; i++)
		res->operators[i] = FALSE;
...
static void _cairo_pdf_surface_add_operator(cairo_pdf_surface_t * surface, cairo_operator_t op)
{
	cairo_pdf_group_resources_t * res = &surface->resources;
	res->operators[op] = TRUE;
}

static cairo_int_status_t _cairo_pdf_surface_add_alpha(cairo_pdf_surface_t * surface, double alpha, int * index)
{
	int i;
	double other;
	cairo_int_status_t status;
	cairo_pdf_group_resources_t * res = &surface->resources;
	int num_alphas = _cairo_array_num_elements(&res->alphas);
...
static cairo_int_status_t _cairo_pdf_surface_add_smask(cairo_pdf_surface_t  * surface, cairo_pdf_resource_t smask)
{
	return _cairo_array_append(&(surface->resources.smasks), &smask);
}

static cairo_int_status_t _cairo_pdf_surface_add_pattern(cairo_pdf_surface_t  * surface, cairo_pdf_resource_t pattern)
{
	return _cairo_array_append(&(surface->resources.patterns), &pattern);
}

static cairo_int_status_t _cairo_pdf_surface_add_shading(cairo_pdf_surface_t  * surface, cairo_pdf_resource_t shading)
{
	return _cairo_array_append(&(surface->resources.shadings), &shading);
}

static cairo_int_status_t _cairo_pdf_surface_add_xobject(cairo_pdf_surface_t  * surface, cairo_pdf_resource_t xobject)
{
...
static cairo_int_status_t _cairo_pdf_surface_add_font(uint font_id, uint subset_id, void * closure)
{
	...
	cairo_int_status_t status;
	cairo_pdf_group_resources_t * res = &surface->resources;
	int num_fonts = _cairo_array_num_elements(&res->fonts);
...
static cairo_pdf_resource_t _cairo_pdf_surface_get_font_resource(cairo_pdf_surface_t * surface, uint font_id, uint subset_id)
{
	cairo_pdf_font_t font;
	const int num_fonts = _cairo_array_num_elements(&surface->fonts);
...
static const char * _cairo_operator_to_pdf_blend_mode(cairo_operator_t op)
{
	switch(op) {
...
static void _cairo_pdf_surface_emit_group_resources(cairo_pdf_surface_t * surface, cairo_pdf_group_resources_t * res)
{
	int num_alphas, num_smasks, num_resources, i;
	double alpha;
	cairo_pdf_resource_t * smask, * pattern, * shading, * xobject;
	cairo_pdf_font_t * font;
	_cairo_output_stream_puts(surface->output, "<<\n");
	...
	if(num_alphas > 0 || num_smasks > 0) {
		_cairo_output_stream_puts(surface->output, "   /ExtGState <<\n");
		for(i = 0; i < CAIRO_NUM_OPERATORS; i++) {
			if(res->operators[i]) {
				_cairo_output_stream_printf(surface->output, "      /b%d << /BM /%s >>\n", i, _cairo_operator_to_pdf_blend_mode((cairo_operator_t)i));
			}
		}
		for(i = 0; i < num_alphas; i++) {
		...
		for(i = 0; i < num_smasks; i++) {
			smask = (cairo_pdf_resource_t *)_cairo_array_index(&res->smasks, i);
	...
	if(num_resources > 0) {
		_cairo_output_stream_puts(surface->output, "   /Pattern <<");
		for(i = 0; i < num_resources; i++) {
			pattern = (cairo_pdf_resource_t *)_cairo_array_index(&res->patterns, i);
			_cairo_output_stream_printf(surface->output, " /p%d %d 0 R", pattern->id, pattern->id);
		}
		_cairo_output_stream_puts(surface->output, " >>\n");
	}
	num_resources = _cairo_array_num_elements(&res->shadings);
	if(num_resources > 0) {
		_cairo_output_stream_puts(surface->output, "   /Shading <<");
		for(i = 0; i < num_resources; i++) {
			shading = (cairo_pdf_resource_t *)_cairo_array_index(&res->shadings, i);
			_cairo_output_stream_printf(surface->output, " /sh%d %d 0 R", shading->id, shading->id);
		}
		_cairo_output_stream_puts(surface->output, " >>\n");
	}
	num_resources = _cairo_array_num_elements(&res->xobjects);
	if(num_resources > 0) {
		_cairo_output_stream_puts(surface->output, "   /XObject <<");
		for(i = 0; i < num_resources; i++) {
			xobject = (cairo_pdf_resource_t *)_cairo_array_index(&res->xobjects, i);
			_cairo_output_stream_printf(surface->output, " /x%d %d 0 R", xobject->id, xobject->id);
		}
		_cairo_output_stream_puts(surface->output, " >>\n");
	}
	num_resources = _cairo_array_num_elements(&res->fonts);
	if(num_resources > 0) {
		_cairo_output_stream_puts(surface->output, "   /Font <<\n");
		for(i = 0; i < num_resources; i++) {
			font = (cairo_pdf_font_t *)_cairo_array_index(&res->fonts, i);
			_cairo_output_stream_printf(surface->output, "      /f-%d-%d %d 0 R\n", font->font_id, font->subset_id, font->subset_resource.id);
		}
		_cairo_output_stream_puts(surface->output, "   >>\n");
	}
	_cairo_output_stream_puts(surface->output, ">>\n");
...
static cairo_int_status_t _cairo_pdf_surface_add_smask_group(cairo_pdf_surface_t * surface, cairo_pdf_smask_group_t * group)
{
...
static cairo_bool_t _cairo_pdf_source_surface_equal(const void * key_a, const void * key_b)
{
	const cairo_pdf_source_surface_entry_t * a = (const cairo_pdf_source_surface_entry_t *)key_a;
	const cairo_pdf_source_surface_entry_t * b = (const cairo_pdf_source_surface_entry_t *)key_b;
...
static cairo_int_status_t FASTCALL _cairo_pdf_surface_acquire_source_image_from_pattern(cairo_pdf_surface_t * surface,
    const cairo_pattern_t * pattern, cairo_image_surface_t ** image, void ** image_extra)
{
	switch(pattern->type) {
		...
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
			{
				cairo_surface_t * surf = _cairo_raster_source_pattern_acquire(pattern, &surface->base, 0);
				if(!surf)
...
static void _cairo_pdf_surface_release_source_image_from_pattern(cairo_pdf_surface_t * surface,
    const cairo_pattern_t * pattern, cairo_image_surface_t * image, void * image_extra)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SURFACE:
...
static cairo_int_status_t _get_jbig2_image_info(cairo_surface_t * source, cairo_image_info_t * info,
    const uchar ** mime_data, ulong * mime_data_length)
{
	cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_JBIG2, mime_data, mime_data_length);
	return (*mime_data == NULL) ? CAIRO_INT_STATUS_UNSUPPORTED : _cairo_image_info_get_jbig2_info(info, *mime_data, *mime_data_length);
}

static cairo_int_status_t _get_jpx_image_info(cairo_surface_t * source, cairo_image_info_t * info,
    const uchar ** mime_data, ulong * mime_data_length)
{
	cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_JP2, mime_data, mime_data_length);
	return (*mime_data == NULL) ? CAIRO_INT_STATUS_UNSUPPORTED : _cairo_image_info_get_jpx_info(info, *mime_data, *mime_data_length);
}

static cairo_int_status_t _get_jpeg_image_info(cairo_surface_t * source, cairo_image_info_t * info,
    const uchar ** mime_data, ulong * mime_data_length)
{
	cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_JPEG, mime_data, mime_data_length);
	return (*mime_data == NULL) ? CAIRO_INT_STATUS_UNSUPPORTED : _cairo_image_info_get_jpeg_info(info, *mime_data, *mime_data_length);
}

static cairo_int_status_t _get_source_surface_size(cairo_surface_t * source, int * width, int * height, CairoIRect * extents)
{
	cairo_int_status_t status;
	...
	if(source->type == CAIRO_SURFACE_TYPE_RECORDING) {
		if(source->backend->type == CAIRO_SURFACE_TYPE_SUBSURFACE) {
			...
			*height = extents->height;
			...
			CairoIRect surf_extents;
			cairo_box_t box;
			cairo_bool_t bounded;
			if(_cairo_surface_is_snapshot(source))
				free_me = source = _cairo_surface_snapshot_get_target(source);
			status = _cairo_recording_surface_get_ink_bbox((cairo_recording_surface_t*)source, &box, 0);
			if(unlikely(status)) {
 ...
 **/
static cairo_int_status_t _cairo_pdf_surface_add_source_surface(cairo_pdf_surface_t   * surface,
    ...
    CairoIRect * source_extents)
{
	...
	*y_offset = 0;
	...
	cairo_surface_get_mime_data(source_surface, CAIRO_MIME_TYPE_UNIQUE_ID,
	    (const uchar**)&surface_key.unique_id, &surface_key.unique_id_length);
	...
	if(surface_entry) {
		...
		*source_extents = surface_entry->extents;
		...
		if(surface_key.unique_id && surface_key.unique_id_length > 0) {
			unique_id = (uchar *)_cairo_malloc(surface_key.unique_id_length);
			if(unique_id == NULL) {
				status = _cairo_error(CAIRO_STATUS_NO_MEMORY);
				goto release_source;
			}
...
static cairo_int_status_t _cairo_pdf_surface_add_pdf_pattern_or_shading(cairo_pdf_surface_t  * surface,
...
//
static void FASTCALL _get_bbox_from_extents(double surface_height, const CairoIRect * extents, cairo_box_double_t * bbox)
{
	bbox->p1.x = extents->x;
...
static cairo_int_status_t _cairo_pdf_surface_add_pdf_shading(cairo_pdf_surface_t  * surface, const cairo_pattern_t * pattern,
    cairo_operator_t op, const CairoIRect * extents, cairo_pdf_resource_t * shading_res, cairo_pdf_resource_t * gstate_res)
{
	return _cairo_pdf_surface_add_pdf_pattern_or_shading(surface, pattern, op, extents, TRUE, shading_res, gstate_res);
}

static cairo_int_status_t _cairo_pdf_surface_add_pdf_pattern(cairo_pdf_surface_t * surface, const cairo_pattern_t * pattern,
    cairo_operator_t op, const CairoIRect * extents, cairo_pdf_resource_t * pattern_res, cairo_pdf_resource_t * gstate_res)
{
	return _cairo_pdf_surface_add_pdf_pattern_or_shading(surface, pattern, op, extents, FALSE, pattern_res, gstate_res);
}

static cairo_int_status_t _cairo_pdf_surface_open_stream(cairo_pdf_surface_t * surface, cairo_pdf_resource_t * resource,
    cairo_bool_t compressed, const char * fmt, ...)
{
	va_list ap;
	...
	surface->current_operator = CAIRO_OPERATOR_OVER;
	_cairo_pdf_operators_reset(&surface->pdf_operators);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Length %d 0 R\n", surface->pdf_stream.self.id, surface->pdf_stream.length.id);
	if(compressed)
		_cairo_output_stream_puts(surface->output, "   /Filter /FlateDecode\n");
	if(fmt != NULL) {
		va_start(ap, fmt);
...
static cairo_int_status_t _cairo_pdf_surface_close_stream(cairo_pdf_surface_t * surface)
{
	...
	if(surface->pdf_stream.compressed) {
		cairo_int_status_t status2 = _cairo_output_stream_destroy(surface->output);
	...
	length = _cairo_output_stream_get_position(surface->output) - surface->pdf_stream.start_offset;
	_cairo_output_stream_puts(surface->output, "\nendstream\nendobj\n");
	_cairo_pdf_surface_update_object(surface, surface->pdf_stream.length);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n   %ld\nendobj\n", surface->pdf_stream.length.id, length);
	surface->pdf_stream.active = FALSE;
	if(likely(status == CAIRO_INT_STATUS_SUCCESS))
...
static void _cairo_pdf_surface_write_memory_stream(cairo_pdf_surface_t  * surface,
    ...
    const cairo_box_double_t * bbox)
{
	...
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /XObject\n   /Length %d\n", resource.id, _cairo_memory_stream_length(mem_stream));
	...
	if(is_knockout_group)
		_cairo_output_stream_puts(surface->output, "      /K true\n");
	_cairo_output_stream_puts(surface->output, "   >>\n   /Resources\n");
	_cairo_pdf_surface_emit_group_resources(surface, resources);
	_cairo_output_stream_puts(surface->output, ">>\nstream\n");
	_cairo_memory_stream_copy(mem_stream, surface->output);
	_cairo_output_stream_puts(surface->output, "endstream\nendobj\n");
}

static cairo_int_status_t _cairo_pdf_surface_open_group(cairo_pdf_surface_t  * surface,
    const cairo_box_double_t * bbox, cairo_pdf_resource_t * resource)
{
	cairo_int_status_t status;
...
static cairo_int_status_t _cairo_pdf_surface_open_knockout_group(cairo_pdf_surface_t  * surface, const cairo_box_double_t * bbox)
{
	cairo_int_status_t status = _cairo_pdf_surface_open_group(surface, bbox, 0);
	if(unlikely(status))
		return status;
...
static cairo_int_status_t _cairo_pdf_surface_close_group(cairo_pdf_surface_t * surface, cairo_pdf_resource_t * group)
{
	...
	if(surface->compress_content) {
		...
		surface->group_stream.stream = NULL;
		_cairo_output_stream_puts(surface->group_stream.mem_stream, "\n");
	}
	surface->output = surface->group_stream.old_output;
	_cairo_pdf_operators_set_stream(&surface->pdf_operators, surface->output);
	surface->group_stream.active = FALSE;
	_cairo_pdf_surface_write_memory_stream(surface, surface->group_stream.mem_stream,
	    surface->group_stream.resource, &surface->resources, surface->group_stream.is_knockout, &surface->group_stream.bbox);
	...
	if(is_form) {
		...
		if(is_group) {
			status = _cairo_pdf_surface_open_stream(surface, resource, surface->compress_content,
			    ...
			    "   >>\n"
			    "   /Resources %d 0 R\n",
			    bbox->p1.x, bbox->p1.y, bbox->p2.x, bbox->p2.y, surface->content_resources.id);
		}
		else {
			status = _cairo_pdf_surface_open_stream(surface, resource, surface->compress_content,
			    "   /Type /XObject\n"
		...
		status = _cairo_pdf_surface_open_stream(surface, resource, surface->compress_content, NULL);
	}
	if(unlikely(status))
		return status;
	surface->content = surface->pdf_stream.self;
	_cairo_output_stream_puts(surface->output, "q\n");
	return _cairo_output_stream_get_status(surface->output);
}

static cairo_int_status_t _cairo_pdf_surface_close_content_stream(cairo_pdf_surface_t * surface)
{
	cairo_int_status_t status;
	...
	_cairo_output_stream_puts(surface->output, "endobj\n");
	return _cairo_output_stream_get_status(surface->output);
}

static void _cairo_pdf_source_surface_entry_pluck(void * entry, void * closure)
{
...
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	long offset;
	...
	_cairo_output_stream_printf(surface->output, "trailer\n<< /Size %d\n   /Root %d 0 R\n   /Info %d 0 R\n>>\n",
	    surface->next_available_resource.id, catalog.id, info.id);
	_cairo_output_stream_printf(surface->output, "startxref\n%ld\n%%%%EOF\n", offset);
...
static cairo_int_status_t _cairo_pdf_surface_start_page(void * abstract_surface)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	// Document header
...
static cairo_int_status_t _cairo_pdf_surface_has_fallback_images(void * abstract_surface, cairo_bool_t has_fallbacks)
{
	cairo_int_status_t status;
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
...
static cairo_bool_t _cairo_pdf_surface_supports_fine_grained_fallbacks(void * abstract_surface)
{
	return TRUE;
}

static cairo_int_status_t _cairo_pdf_surface_add_padded_image_surface(cairo_pdf_surface_t   * surface,
    ...
    double   * y_offset)
{
	...
	_cairo_matrix_transform_bounding_box_fixed(&source->matrix, &box, 0);
 ...
 */
static cairo_int_status_t _cairo_pdf_surface_emit_smask(cairo_pdf_surface_t  * surface,
    cairo_image_surface_t * image,
    cairo_bool_t stencil_mask,
    cairo_bool_t interpolate,
    cairo_pdf_resource_t * stream_res)
{
	...
	if(transparency == CAIRO_IMAGE_HAS_BILEVEL_ALPHA || transparency == CAIRO_IMAGE_IS_OPAQUE) {
		...
		alpha = (char *)_cairo_malloc_ab((image->width+7) / 8, image->height);
	}
	else {
		alpha_size = image->height * image->width;
		alpha = (char *)_cairo_malloc_ab(image->height, image->width);
	}
	...
	if(stencil_mask) {
		status = _cairo_pdf_surface_open_stream(surface, stream_res, TRUE,
		    "   /Type /XObject\n"
		    "   /Subtype /Image\n"
		    "   /ImageMask true\n"
		    "   /Width %d\n"
 ...
 **/
static cairo_int_status_t _cairo_pdf_surface_emit_image(cairo_pdf_surface_t * surface,
...
static cairo_int_status_t _cairo_pdf_surface_lookup_jbig2_global(cairo_pdf_surface_t   * surface,
    const uchar   * global_id,
    ulong global_id_length,
...
static cairo_int_status_t _cairo_pdf_surface_emit_jbig2_image(cairo_pdf_surface_t * surface,
    ...
    cairo_pdf_source_surface_entry_t * surface_entry)
{
	...
	const uchar * global_id;
	ulong global_id_length;
	const uchar * global_data;
	ulong global_data_length;
	...
	if(status)
		return status;
	cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_JBIG2_GLOBAL_ID, &global_id, &global_id_length);
	if(global_id && global_id_length > 0) {
		...
		if(!global_entry->emitted) {
			cairo_surface_get_mime_data(source, CAIRO_MIME_TYPE_JBIG2_GLOBAL, &global_data, &global_data_length);
			if(global_data) {
				status = _cairo_pdf_surface_open_stream(surface, &global_entry->res, FALSE, 0);
				if(unlikely(status))
					return status;
		...
		snprintf(decode_parms_buf, sizeof(decode_parms_buf), "   /DecodeParms << /JBIG2Globals %d 0 R >>\n", global_entry->res.id);
	}
	else {
    ...
    cairo_pdf_source_surface_entry_t * surface_entry)
{
	...
	if(surface_entry->stencil_mask) {
		status = _cairo_pdf_surface_open_stream(surface, &surface_entry->surface_res, FALSE,
		    ...
		    "   /Decode [1 0]\n"
		    "   /Filter /JPXDecode\n",
		    info.width, info.height, surface_entry->interpolate ? "true" : "false");
	}
	else {
		status = _cairo_pdf_surface_open_stream(surface, &surface_entry->surface_res, FALSE,
		    "   /Type /XObject\n"
		    ...
		    "%s"
		    "   /Filter /JPXDecode\n",
		    info.width, info.height, surface_entry->interpolate ? "true" : "false", smask_buf);
	}
	if(status)
		return status;
	_cairo_output_stream_write(surface->output, mime_data, mime_data_length);
...
static cairo_int_status_t _cairo_pdf_surface_emit_jpeg_image(cairo_pdf_surface_t * surface, cairo_surface_t * source, cairo_pdf_source_surface_entry_t * surface_entry)
{
	cairo_int_status_t status;
	const uchar * mime_data;
	ulong mime_data_length;
	...
	if(surface_entry->stencil_mask) {
		...
		status = _cairo_pdf_surface_open_stream(surface, &surface_entry->surface_res, FALSE,
		    ...
		    "%s"
		    "   /Filter /DCTDecode\n",
		    info.width, info.height, colorspace, surface_entry->interpolate ? "true" : "false", info.bits_per_component, smask_buf);
	}
	if(unlikely(status))
		return status;
	_cairo_output_stream_write(surface->output, mime_data, mime_data_length);
...
static cairo_int_status_t _cairo_pdf_surface_emit_image_surface(cairo_pdf_surface_t * surface, cairo_pdf_source_surface_t * source)
{
	cairo_image_surface_t * image;
	void * image_extra;
	cairo_int_status_t status;
	...
	if(source->type == CAIRO_PATTERN_TYPE_SURFACE) {
		...
		status = _cairo_pdf_surface_acquire_source_image_from_pattern(surface, source->raster_pattern, &image, &image_extra);
	}
	if(unlikely(status))
		return status;
	...
	if(source->type == CAIRO_PATTERN_TYPE_SURFACE)
		...
		_cairo_pdf_surface_release_source_image_from_pattern(surface, source->raster_pattern, image, image_extra);
	return status;
}

static cairo_int_status_t _cairo_pdf_surface_emit_recording_surface(cairo_pdf_surface_t * surface, cairo_pdf_source_surface_t * pdf_source)
{
	 ...
	 */
	transparency_group = !(pdf_source->hash_entry->Op == CAIRO_OPERATOR_OVER &&
	    _cairo_recording_surface_has_only_bilevel_alpha(recording) && _cairo_recording_surface_has_only_op_over(recording));
	...
	if(source->content == CAIRO_CONTENT_COLOR) {
		...
		if(unlikely(status))
			goto err;
		_cairo_output_stream_printf(surface->output, "q /a%d gs 0 0 0 rg 0 0 %f %f re f Q\n", alpha, surface->width, surface->height);
	}
	status = _cairo_recording_surface_replay_region(source, is_subsurface ? extents : NULL, &surface->base, CAIRO_RECORDING_REGION_NATIVE);
...
static cairo_int_status_t _cairo_pdf_surface_emit_surface(cairo_pdf_surface_t * surface, cairo_pdf_source_surface_t * src_surface)
{
	if(src_surface->type == CAIRO_PATTERN_TYPE_SURFACE && src_surface->surface->type == CAIRO_SURFACE_TYPE_RECORDING)
...
static cairo_int_status_t _cairo_pdf_surface_emit_surface_pattern(cairo_pdf_surface_t * surface, cairo_pdf_pattern_t * pdf_pattern)
{
	...
	if(pattern->type == CAIRO_PATTERN_TYPE_SURFACE &&
		...
		snprintf(draw_surface, sizeof(draw_surface), "/x%d Do\n", pattern_resource.id);
	}
	else {
		snprintf(draw_surface, sizeof(draw_surface), "q %d 0 0 %d 0 0 cm /x%d Do Q",
		    pattern_width, pattern_height, pattern_resource.id);
	}
	if(extend == CAIRO_EXTEND_REFLECT) {
		_cairo_output_stream_printf(surface->output,
		    ...
		    pattern_width, pattern_height, draw_surface, pattern_width*2, pattern_width, pattern_height,
		    draw_surface, pattern_height*2, pattern_width, pattern_height, draw_surface,
		    pattern_width*2, pattern_height*2, pattern_width, pattern_height, draw_surface);
	}
	else {
		_cairo_output_stream_printf(surface->output, " %s \n", draw_surface);
	}
	status = _cairo_pdf_surface_close_stream(surface);
	return (unlikely(status)) ? status : _cairo_output_stream_get_status(surface->output);
}
...
static cairo_int_status_t cairo_pdf_surface_emit_rgb_linear_function(cairo_pdf_surface_t * surface,
    cairo_pdf_color_stop_t * stop1, cairo_pdf_color_stop_t * stop2, cairo_pdf_resource_t   * function)
{
	int i;
	cairo_pdf_rgb_linear_function_t elem;
	cairo_pdf_resource_t res;
	cairo_int_status_t status;
	int num_elems = _cairo_array_num_elements(&surface->rgb_linear_functions);
	...
	if(res.id == 0)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /FunctionType 2\n   /Domain [ 0 1 ]\n   /C0 [ %f %f %f ]\n   /C1 [ %f %f %f ]\n   /N 1\n>>\nendobj\n",
		res.id, stop1->color[0], stop1->color[1], stop1->color[2], stop2->color[0], stop2->color[1], stop2->color[2]);
	elem.resource = res;
...
static cairo_int_status_t cairo_pdf_surface_emit_alpha_linear_function(cairo_pdf_surface_t * surface,
    ...
    cairo_pdf_resource_t   * function)
{
	...
	cairo_pdf_resource_t res;
	cairo_int_status_t status;
	int num_elems = _cairo_array_num_elements(&surface->alpha_linear_functions);
	for(i = 0; i < num_elems; i++) {
...
static cairo_int_status_t _cairo_pdf_surface_emit_stitched_colorgradient(cairo_pdf_surface_t * surface,
    uint n_stops, cairo_pdf_color_stop_t * stops, cairo_bool_t is_alpha, cairo_pdf_resource_t   * function)
{
	cairo_pdf_resource_t res;
	uint i;
	cairo_int_status_t status;
	...
	for(i = 0; i < n_stops-1; i++) {
		if(is_alpha) {
			status = cairo_pdf_surface_emit_alpha_linear_function(surface, &stops[i], &stops[i+1], &stops[i].resource);
			if(unlikely(status))
				return status;
		}
		else {
			status = cairo_pdf_surface_emit_rgb_linear_function(surface, &stops[i], &stops[i+1], &stops[i].resource);
			if(unlikely(status))
				return status;
		}
	}
	...
	if(res.id == 0)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /FunctionType 3\n   /Domain [ %f %f ]\n", res.id, stops[0].offset, stops[n_stops - 1].offset);
	_cairo_output_stream_puts(surface->output, "   /Functions [ ");
	for(i = 0; i < n_stops-1; i++)
		_cairo_output_stream_printf(surface->output, "%d 0 R ", stops[i].resource.id);
	_cairo_output_stream_puts(surface->output, "]\n");
	_cairo_output_stream_puts(surface->output, "   /Bounds [ ");
	for(i = 1; i < n_stops-1; i++)
		_cairo_output_stream_printf(surface->output, "%f ", stops[i].offset);
	_cairo_output_stream_puts(surface->output, "]\n");
	_cairo_output_stream_puts(surface->output, "   /Encode [ ");
	for(i = 1; i < n_stops; i++)
		_cairo_output_stream_puts(surface->output, "0 1 ");
	_cairo_output_stream_puts(surface->output, "]\n");
	_cairo_output_stream_puts(surface->output, ">>\nendobj\n");
	*function = res;
	return _cairo_output_stream_get_status(surface->output);
...
static void calc_gradient_color(cairo_pdf_color_stop_t * new_stop, cairo_pdf_color_stop_t * stop1, cairo_pdf_color_stop_t * stop2)
{
	const double offset = stop1->offset / (stop1->offset + 1.0 - stop2->offset);
	for(int i = 0; i < 4; i++)
...
static cairo_int_status_t _cairo_pdf_surface_emit_pattern_stops(cairo_pdf_surface_t  * surface,
    ...
    cairo_pdf_resource_t * alpha_function)
{
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		...
		if(stops[n_stops-1].offset < 1.0 - COLOR_STOP_EPSILON) {
			if(pattern->base.extend == CAIRO_EXTEND_REFLECT) {
				memcpy(&stops[n_stops], &stops[n_stops - 1], sizeof(cairo_pdf_color_stop_t));
			}
			else {
	...
	if(stops[0].offset == stops[n_stops - 1].offset) {
		...
		status = _cairo_pdf_surface_emit_stitched_colorgradient(surface, 4, pad_stops, FALSE, color_function);
		if(unlikely(status))
			goto BAIL;
		if(emit_alpha) {
			status = _cairo_pdf_surface_emit_stitched_colorgradient(surface, 4, pad_stops, TRUE, alpha_function);
			if(unlikely(status))
				goto BAIL;
		}
	}
		...
		status = cairo_pdf_surface_emit_rgb_linear_function(surface, &stops[0], &stops[n_stops - 1], color_function);
		if(unlikely(status))
			goto BAIL;
		if(emit_alpha) {
			status = cairo_pdf_surface_emit_alpha_linear_function(surface, &stops[0], &stops[n_stops - 1], alpha_function);
			if(unlikely(status))
				goto BAIL;
		}
	}
		...
		status = _cairo_pdf_surface_emit_stitched_colorgradient(surface, n_stops, stops, FALSE, color_function);
		if(unlikely(status))
			goto BAIL;
		if(emit_alpha) {
			status = _cairo_pdf_surface_emit_stitched_colorgradient(surface, n_stops, stops, TRUE, alpha_function);
			if(unlikely(status))
				goto BAIL;
		}
	}
...
static cairo_int_status_t _cairo_pdf_surface_emit_repeating_function(cairo_pdf_surface_t * surface,
    cairo_gradient_pattern_t * pattern, cairo_pdf_resource_t * function, int begin, int end)
{
	int i;
	cairo_pdf_resource_t res = _cairo_pdf_surface_new_object(surface);
	if(res.id == 0)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /FunctionType 3\n   /Domain [ %d %d ]\n", res.id, begin, end);
	_cairo_output_stream_puts(surface->output, "   /Functions [ ");
	for(i = begin; i < end; i++)
		_cairo_output_stream_printf(surface->output, "%d 0 R ", function->id);
	_cairo_output_stream_puts(surface->output, "]\n");
	_cairo_output_stream_puts(surface->output, "   /Bounds [ ");
	...
	for(i = begin; i < end; i++) {
		if((i % 2) && pattern->base.extend == CAIRO_EXTEND_REFLECT) {
			_cairo_output_stream_puts(surface->output, "1 0 ");
		}
		else {
			_cairo_output_stream_puts(surface->output, "0 1 ");
		}
...
static cairo_int_status_t cairo_pdf_surface_emit_transparency_group(cairo_pdf_surface_t  * surface,
    cairo_pdf_pattern_t  * pdf_pattern, cairo_pdf_resource_t gstate_resource, cairo_pdf_resource_t gradient_mask)
{
	...
	if(pdf_pattern->is_shading) {
		...
		_cairo_matrix_transform_bounding_box(&pdf_pattern->pattern->matrix, &x1, &y1, &x2, &y2, 0);
	}
	else {
	...
	if(pdf_pattern->is_shading) {
		_cairo_output_stream_printf(surface->output, "/a0 gs /sh%d sh\n", gradient_mask.id);
	}
	else {
		_cairo_output_stream_printf(surface->output, "q\n/a0 gs\n/Pattern cs /p%d scn\n0 0 %f %f re\nf\nQ\n", gradient_mask.id, surface->width, surface->height);
	}
	status = _cairo_pdf_surface_close_stream(surface);
	if(unlikely(status))
		return status;
	...
	if(smask_resource.id == 0)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /Mask\n   /S /Luminosity\n   /G %d 0 R\n>>\nendobj\n",
	    smask_resource.id, surface->pdf_stream.self.id);
...
static void _cairo_pdf_surface_output_gradient(cairo_pdf_surface_t * surface,
    ...
    cairo_pdf_resource_t color_function)
{
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n", pattern_resource.id);
	if(!pdf_pattern->is_shading) {
		_cairo_output_stream_puts(surface->output, "<< /Type /Pattern\n   /PatternType 2\n   /Matrix [ ");
		_cairo_output_stream_print_matrix(surface->output, pat_to_pdf);
		_cairo_output_stream_puts(surface->output, " ]\n   /Shading\n");
	}
	if(pdf_pattern->pattern->type == CAIRO_PATTERN_TYPE_LINEAR) {
		_cairo_output_stream_printf(surface->output, "      << /ShadingType 2\n         /ColorSpace %s\n         /Coords [ %f %f %f %f ]\n",
		    colorspace, start->center.x, start->center.y, end->center.x, end->center.y);
	}
	else {
		_cairo_output_stream_printf(surface->output, "      << /ShadingType 3\n         /ColorSpace %s\n         /Coords [ %f %f %f %f %f %f ]\n",
		    colorspace, start->center.x, start->center.y, MAX(start->radius, 0), end->center.x, end->center.y, MAX(end->radius, 0));
	}
	_cairo_output_stream_printf(surface->output, "         /Domain [ %f %f ]\n", domain[0], domain[1]);
	if(pdf_pattern->pattern->extend != CAIRO_EXTEND_NONE) {
		_cairo_output_stream_puts(surface->output, "         /Extend [ true true ]\n");
	}
	else {
		_cairo_output_stream_puts(surface->output, "         /Extend [ false false ]\n");
	}
...
static cairo_int_status_t _cairo_pdf_surface_emit_gradient(cairo_pdf_surface_t * surface, cairo_pdf_pattern_t * pdf_pattern)
{
	...
	if(oneof2(pattern->base.extend, CAIRO_EXTEND_REPEAT, CAIRO_EXTEND_REFLECT)) {
		...
		double x_scale = surface->base.x_resolution / surface->base.x_fallback_resolution;
		double y_scale = surface->base.y_resolution / surface->base.y_fallback_resolution;
		double tolerance = fabs(_cairo_matrix_compute_determinant(&pattern->base.matrix));
	...
	* coordinates in if there are not stops at offset 0 and 1. */
	...
	if(pattern->base.extend == CAIRO_EXTEND_REPEAT || pattern->base.extend == CAIRO_EXTEND_REFLECT) {
		...
		status = _cairo_pdf_surface_emit_repeating_function(surface, pattern, &color_function, repeat_begin, repeat_end);
		if(unlikely(status))
			return status;
		if(alpha_function.id != 0) {
			status = _cairo_pdf_surface_emit_repeating_function(surface, pattern, &alpha_function, repeat_begin, repeat_end);
			if(unlikely(status))
				return status;
		}
	}
	...
	_cairo_pdf_surface_update_object(surface, pdf_pattern->pattern_res);
	_cairo_pdf_surface_output_gradient(surface, pdf_pattern, pdf_pattern->pattern_res,
	    &pat_to_pdf, &start, &end, domain, "/DeviceRGB", color_function);
	if(alpha_function.id != 0) {
...
static cairo_int_status_t _cairo_pdf_surface_emit_mesh_pattern(cairo_pdf_surface_t * surface,
    cairo_pdf_pattern_t * pdf_pattern)
{
	cairo_matrix_t pat_to_pdf;
	cairo_int_status_t status;
	...
	_cairo_output_stream_printf(surface->output,
	    ...
	    "   /BitsPerFlag %d\n"
	    "   /Decode [",
	    res.id, shading.shading_type, shading.bits_per_coordinate, shading.bits_per_component, shading.bits_per_flag);
	...
	_cairo_pdf_surface_update_object(surface, pdf_pattern->pattern_res);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /Pattern\n   /PatternType 2\n   /Matrix [ ", pdf_pattern->pattern_res.id);
	_cairo_output_stream_print_matrix(surface->output, &pat_to_pdf);
	_cairo_output_stream_printf(surface->output, " ]\n   /Shading %d 0 R\n>>\nendobj\n", res.id);
	if(pdf_pattern->gstate_res.id != 0) {
		cairo_pdf_resource_t mask_resource;
		...
		if(unlikely(mask_resource.id == 0))
			return _cairo_error(CAIRO_STATUS_NO_MEMORY);
		_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /Pattern\n   /PatternType 2\n   /Matrix [ ", mask_resource.id);
		_cairo_output_stream_print_matrix(surface->output, &pat_to_pdf);
		_cairo_output_stream_printf(surface->output, " ]\n   /Shading %d 0 R\n>>\nendobj\n", res.id);
		status = cairo_pdf_surface_emit_transparency_group(surface, pdf_pattern, pdf_pattern->gstate_res, mask_resource);
		if(unlikely(status))
			return status;
	}
	return _cairo_output_stream_get_status(surface->output);
...
static cairo_int_status_t _cairo_pdf_surface_emit_pattern(cairo_pdf_surface_t * surface, cairo_pdf_pattern_t * pdf_pattern)
{
	cairo_int_status_t status;
	double old_width = surface->width;
	double old_height = surface->height;
	_cairo_pdf_surface_set_size_internal(surface, pdf_pattern->width, pdf_pattern->height);
...
static cairo_int_status_t _cairo_pdf_surface_paint_surface_pattern(cairo_pdf_surface_t   * surface,
    cairo_operator_t op,
    const cairo_pattern_t * source,
    const CairoIRect  * extents,
    cairo_pdf_resource_t  * smask_res,
    cairo_bool_t stencil_mask)
{
	...
	if(source->extend == CAIRO_EXTEND_PAD && !(source->type == CAIRO_PATTERN_TYPE_SURFACE &&
		    ((cairo_surface_pattern_t*)source)->surface->type == CAIRO_SURFACE_TYPE_RECORDING)) {
		status = _cairo_pdf_surface_add_padded_image_surface(surface,
		    source, extents, &surface_res, &width, &height, &x_offset, &y_offset);
	}
	else {
		status = _cairo_pdf_surface_add_source_surface(surface, NULL, source, op, source->filter,
		    stencil_mask, FALSE, extents, smask_res, &surface_res, &width, &height, &x_offset, &y_offset, &extents2);
	}
	if(unlikely(status))
		return status;
	...
	cairo_matrix_scale(&pdf_p2d, 1.0, -1.0);
	if(!(source->type == CAIRO_PATTERN_TYPE_SURFACE && ((cairo_surface_pattern_t*)source)->surface->type == CAIRO_SURFACE_TYPE_RECORDING)) {
	...
	if(!_cairo_matrix_is_identity(&pdf_p2d)) {
		...
		_cairo_output_stream_puts(surface->output, " cm\n");
	}
	status = _cairo_pdf_surface_add_alpha(surface, 1.0, &alpha);
	if(unlikely(status))
		return status;
	if(stencil_mask) {
		_cairo_output_stream_printf(surface->output, "/x%d Do\n", surface_res.id);
	}
	else {
		_cairo_output_stream_printf(surface->output, "/a%d gs /x%d Do\n", alpha, surface_res.id);
	}
	return _cairo_pdf_surface_add_xobject(surface, surface_res);
}

static cairo_int_status_t _cairo_pdf_surface_paint_gradient(cairo_pdf_surface_t  * surface,
    cairo_operator_t op, const cairo_pattern_t * source, const CairoIRect * extents)
{
	cairo_pdf_resource_t shading_res, gstate_res;
	cairo_matrix_t pat_to_pdf;
	int alpha;
	cairo_int_status_t status = _cairo_pdf_surface_add_pdf_shading(surface, source, op, extents, &shading_res, &gstate_res);
	...
	if(gstate_res.id != 0) {
		...
		if(unlikely(status))
			return status;
		_cairo_output_stream_printf(surface->output, "/s%d gs /sh%d sh\n", gstate_res.id, shading_res.id);
	}
	else {
		status = _cairo_pdf_surface_add_alpha(surface, 1.0, &alpha);
		if(unlikely(status))
			return status;
		_cairo_output_stream_printf(surface->output, "/a%d gs /sh%d sh\n", alpha, shading_res.id);
	}
	return status;
}

static cairo_int_status_t _cairo_pdf_surface_paint_pattern(cairo_pdf_surface_t * surface, cairo_operator_t op,
	const cairo_pattern_t * source, const CairoIRect * extents, cairo_bool_t mask)
{
	switch(source->type) {
		case CAIRO_PATTERN_TYPE_SURFACE:
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE:
		    return _cairo_pdf_surface_paint_surface_pattern(surface, op, source, extents, NULL, mask);
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL:
		case CAIRO_PATTERN_TYPE_MESH:
		    return _cairo_pdf_surface_paint_gradient(surface, op, source, extents);
		case CAIRO_PATTERN_TYPE_SOLID:
		default:
		    ASSERT_NOT_REACHED;
		    return CAIRO_STATUS_SUCCESS;
...
static cairo_bool_t _can_paint_pattern(const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID: return FALSE;
		case CAIRO_PATTERN_TYPE_SURFACE:
		case CAIRO_PATTERN_TYPE_RASTER_SOURCE: return oneof2(pattern->extend, CAIRO_EXTEND_NONE, CAIRO_EXTEND_PAD);
		case CAIRO_PATTERN_TYPE_LINEAR:
		case CAIRO_PATTERN_TYPE_RADIAL: return TRUE;
		case CAIRO_PATTERN_TYPE_MESH: return FALSE;
		default: ASSERT_NOT_REACHED; return FALSE;
	}
}

static cairo_int_status_t _cairo_pdf_surface_select_operator(cairo_pdf_surface_t * surface, cairo_operator_t op)
{
	cairo_int_status_t status;
	...
	if(unlikely(status))
		return status;
	_cairo_output_stream_printf(surface->output, "/b%d gs\n", op);
	surface->current_operator = op;
	_cairo_pdf_surface_add_operator(surface, op);
	return CAIRO_STATUS_SUCCESS;
}

static cairo_int_status_t _cairo_pdf_surface_select_pattern(cairo_pdf_surface_t * surface, const cairo_pattern_t * pattern,
    cairo_pdf_resource_t pattern_res, cairo_bool_t is_stroke)
{
	cairo_int_status_t status;
	int alpha;
	...
	if(solid_color != NULL) {
		if(surface->current_pattern_is_solid_color == FALSE ||
			...
			if(unlikely(status))
				return status;
			_cairo_output_stream_printf(surface->output, "%f %f %f ", solid_color->red, solid_color->green, solid_color->blue);
			if(is_stroke)
				_cairo_output_stream_puts(surface->output, "RG ");
			else
				_cairo_output_stream_puts(surface->output, "rg ");
			surface->current_color_red = solid_color->red;
			surface->current_color_green = solid_color->green;
		...
		if(surface->current_pattern_is_solid_color == FALSE ||
			...
			if(unlikely(status))
				return status;
			_cairo_output_stream_printf(surface->output, "/a%d gs\n", alpha);
			surface->current_color_alpha = solid_color->alpha;
		}
		...
		if(!surface->select_pattern_gstate_saved)
			_cairo_output_stream_puts(surface->output, "q ");
		if(is_stroke) {
			_cairo_output_stream_printf(surface->output, "/Pattern CS /p%d SCN ", pattern_res.id);
		}
		else {
			_cairo_output_stream_printf(surface->output, "/Pattern cs /p%d scn ", pattern_res.id);
		}
...
static cairo_int_status_t _cairo_pdf_surface_unselect_pattern(cairo_pdf_surface_t * surface)
{
	cairo_int_status_t status;
	if(surface->select_pattern_gstate_saved) {
		status = _cairo_pdf_operators_flush(&surface->pdf_operators);
...
static cairo_int_status_t _cairo_pdf_surface_show_page(void * abstract_surface)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	cairo_int_status_t status = _cairo_pdf_surface_close_content_stream(surface);
	if(unlikely(status))
		return status;
	_cairo_surface_clipper_reset(&surface->clipper);
...
static cairo_pdf_resource_t _cairo_pdf_surface_write_info(cairo_pdf_surface_t * surface)
{
	cairo_pdf_resource_t info = _cairo_pdf_surface_new_object(surface);
	if(info.id) {
		_cairo_output_stream_printf(surface->output,
			"%d 0 obj\n"
...
static void _cairo_pdf_surface_write_pages(cairo_pdf_surface_t * surface)
{
	...
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /Pages\n   /Kids [ ", surface->pages_resource.id);
...
static cairo_int_status_t _utf8_to_pdf_string(const char * utf8, char ** str_out)
{
	int i;
	char * str;
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	cairo_bool_t ascii = TRUE;
	int len = strlen(utf8);
	for(i = 0; i < len; i++) {
		unsigned c = utf8[i];
		if(c < 32 || c > 126 || c == '(' || c == ')' || c == '\\') {
...
static cairo_int_status_t _cairo_pdf_surface_emit_unicode_for_glyph(cairo_pdf_surface_t  * surface, const char * utf8)
{
	...
	if(utf16 == NULL || utf16_len == 0) {
		 ...
		 */
		_cairo_output_stream_puts(surface->output, "fffd");
	}
	else {
		for(i = 0; i < utf16_len; i++)
...
static void _create_font_subset_tag(cairo_scaled_font_subset_t * font_subset, const char * font_name, char * tag)
{
	...
	long numerator;
	ldiv_t d;
	uint32_t hash = _hash_data((uchar*)font_name, strlen(font_name), 0);
...
static cairo_int_status_t _cairo_pdf_surface_emit_to_unicode_stream(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset, cairo_pdf_resource_t * stream)
{
	uint i, num_bfchar;
	cairo_int_status_t status;
	stream->id = 0;
	status = _cairo_pdf_surface_open_stream(surface, NULL, surface->compress_content, 0);
	if(unlikely(status))
		return status;
	_cairo_output_stream_puts(surface->output,
	...
	if(font_subset->is_composite && !font_subset->is_latin) {
		_cairo_output_stream_puts(surface->output, "<0000> <ffff>\n");
	}
	else {
		_cairo_output_stream_puts(surface->output, "<00> <ff>\n");
	}
	_cairo_output_stream_puts(surface->output, "endcodespacerange\n");
	if(font_subset->is_scaled) {
		...
		for(i = 0; i < num_bfchar; i++) {
			...
			status = _cairo_pdf_surface_emit_unicode_for_glyph(surface, font_subset->utf8[i]);
			if(unlikely(status))
				return status;
			_cairo_output_stream_puts(surface->output, "\n");
		}
		...
		for(i = 0; i < num_bfchar; i++) {
			...
			status = _cairo_pdf_surface_emit_unicode_for_glyph(surface, font_subset->utf8[i + 1]);
			if(unlikely(status))
				return status;
			_cairo_output_stream_puts(surface->output, "\n");
		}
	}
	_cairo_output_stream_puts(surface->output, "endbfchar\n");
	_cairo_output_stream_puts(surface->output, "endcmap\nCMapName currentdict /CMap defineresource pop\nend\nend\n");
...
static cairo_int_status_t _cairo_pdf_surface_emit_cff_font(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset, cairo_cff_subset_t * subset)
{
	...
	subset_resource = _cairo_pdf_surface_get_font_resource(surface, font_subset->font_id, font_subset->subset_id);
	if(subset_resource.id == 0)
		return CAIRO_STATUS_SUCCESS;
	status = _cairo_pdf_surface_open_stream(surface, NULL, TRUE, font_subset->is_latin ? "   /Subtype /Type1C\n" : "   /Subtype /CIDFontType0C\n");
	if(unlikely(status))
		return status;
	stream = surface->pdf_stream.self;
	_cairo_output_stream_write(surface->output, subset->data, subset->data_length);
	status = _cairo_pdf_surface_close_stream(surface);
	if(unlikely(status))
		return status;
	status = _cairo_pdf_surface_emit_to_unicode_stream(surface, font_subset, &to_unicode_stream);
	...
	if(descriptor.id == 0)
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /FontDescriptor\n   /FontName /%s+%s\n",
	    descriptor.id, tag, subset->ps_name);
	if(subset->family_name_utf8) {
		char * pdf_str;
		status = _utf8_to_pdf_string(subset->family_name_utf8, &pdf_str);
		if(unlikely(status))
			return status;
		_cairo_output_stream_printf(surface->output, "   /FontFamily %s\n", pdf_str);
		SAlloc::F(pdf_str);
	}
	_cairo_output_stream_printf(surface->output,
	    "   /Flags 4\n"
	...
	if(font_subset->is_latin) {
		...
		for(i = 32; i < last_glyph + 1; i++) {
			...
			if(glyph > 0) {
				_cairo_output_stream_printf(surface->output, " %ld", (long)(subset->widths[glyph]*PDF_UNITS_PER_EM));
			}
			else {
				_cairo_output_stream_puts(surface->output, " 0");
			}
		}
		_cairo_output_stream_puts(surface->output, " ]\n");
		if(to_unicode_stream.id != 0)
			_cairo_output_stream_printf(surface->output, "    /ToUnicode %d 0 R\n", to_unicode_stream.id);
		_cairo_output_stream_puts(surface->output, ">>\nendobj\n");
	}
	else {
		...
		for(i = 0; i < font_subset->num_glyphs; i++)
			_cairo_output_stream_printf(surface->output, " %ld", (long)(subset->widths[i]*PDF_UNITS_PER_EM));
		_cairo_output_stream_puts(surface->output, " ]]\n>>\nendobj\n");
		_cairo_pdf_surface_update_object(surface, subset_resource);
		_cairo_output_stream_printf(surface->output,
		    "%d 0 obj\n<< /Type /Font\n   /Subtype /Type0\n   /BaseFont /%s+%s\n   /Encoding /Identity-H\n   /DescendantFonts [ %d 0 R]\n",
		    subset_resource.id, tag, subset->ps_name, cidfont_dict.id);
		if(to_unicode_stream.id != 0)
			_cairo_output_stream_printf(surface->output, "   /ToUnicode %d 0 R\n", to_unicode_stream.id);
		_cairo_output_stream_puts(surface->output, ">>\nendobj\n");
	}
...
static cairo_int_status_t _cairo_pdf_surface_emit_cff_font_subset(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
	cairo_int_status_t status;
	cairo_cff_subset_t subset;
	char name[64];
	snprintf(name, sizeof name, "CairoFont-%d-%d", font_subset->font_id, font_subset->subset_id);
...
static cairo_int_status_t _cairo_pdf_surface_emit_type1_font(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset, cairo_type1_subset_t * subset)
{
	...
	status = _cairo_pdf_surface_open_stream(surface, NULL, TRUE, "   /Length1 %lu\n   /Length2 %lu\n   /Length3 %lu\n",
	...
	_cairo_output_stream_printf(surface->output,
	    ...
	    "   /LastChar %d\n"
	    "   /FontDescriptor %d 0 R\n",
	    subset_resource.id, tag, subset->base_font, font_subset->is_latin ? 32 : 0, last_glyph, descriptor.id);
	if(font_subset->is_latin)
		_cairo_output_stream_puts(surface->output, "   /Encoding /WinAnsiEncoding\n");
	_cairo_output_stream_puts(surface->output, "   /Widths [");
	if(font_subset->is_latin) {
		for(i = 32; i < last_glyph + 1; i++) {
			int glyph = font_subset->latin_to_subset_glyph_index[i];
			if(glyph > 0) {
...
static cairo_int_status_t _cairo_pdf_emit_imagemask(cairo_image_surface_t * image, cairo_output_stream_t * stream)
{
	...
	_cairo_output_stream_printf(stream, "BI\n/IM true\n/W %d\n/H %d\n/BPC 1\n/D [1 0]\n", image->width, image->height);
	_cairo_output_stream_puts(stream, "ID ");
...
static cairo_int_status_t _cairo_pdf_surface_analyze_user_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_int_status_t status = CAIRO_INT_STATUS_SUCCESS;
	cairo_int_status_t status2;
	uint i;
	cairo_output_stream_t * null_stream = _cairo_null_stream_create();
	cairo_surface_t * type3_surface = _cairo_type3_glyph_surface_create(font_subset->scaled_font,
	    null_stream, _cairo_pdf_emit_imagemask, surface->font_subsets, FALSE);
	...
	status2 = _cairo_output_stream_destroy(null_stream);
	SETIFZ(status, status2);
	return status;
}

static cairo_int_status_t _cairo_pdf_surface_emit_type3_font_subset(cairo_pdf_surface_t * surface, cairo_scaled_font_subset_t * font_subset)
{
	cairo_int_status_t status = CAIRO_STATUS_SUCCESS;
	...
	if(subset_resource.id == 0)
		return CAIRO_STATUS_SUCCESS;
	glyphs = (cairo_pdf_resource_t *)_cairo_malloc_ab(font_subset->num_glyphs, sizeof(cairo_pdf_resource_t));
	...
	_cairo_pdf_group_resources_clear(&surface->resources);
	type3_surface = _cairo_type3_glyph_surface_create(font_subset->scaled_font, NULL, _cairo_pdf_emit_imagemask, surface->font_subsets, FALSE);
	if(unlikely(type3_surface->status)) {
		SAlloc::F(glyphs);
		SAlloc::F(widths);
		return type3_surface->status;
	...
	_cairo_output_stream_puts(surface->output, "]\n");
	SAlloc::F(widths);
	_cairo_output_stream_puts(surface->output, "   /Resources\n");
	_cairo_pdf_surface_emit_group_resources(surface, &surface->resources);
	if(to_unicode_stream.id != 0)
		_cairo_output_stream_printf(surface->output, "    /ToUnicode %d 0 R\n", to_unicode_stream.id);
	_cairo_output_stream_puts(surface->output, ">>\nendobj\n");
...
static cairo_int_status_t _cairo_pdf_surface_emit_unscaled_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_int_status_t status = _cairo_pdf_surface_emit_cff_font_subset(surface, font_subset);
	if(status != CAIRO_INT_STATUS_UNSUPPORTED)
		return status;
...
static cairo_int_status_t _cairo_pdf_surface_emit_scaled_font_subset(cairo_scaled_font_subset_t * font_subset, void * closure)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)closure;
	cairo_int_status_t status = _cairo_pdf_surface_emit_type3_font_subset(surface, font_subset);
	if(status != CAIRO_INT_STATUS_UNSUPPORTED)
		return status;
	ASSERT_NOT_REACHED;
	return CAIRO_INT_STATUS_SUCCESS;
...
static cairo_int_status_t _cairo_pdf_surface_emit_font_subsets(cairo_pdf_surface_t * surface)
{
	cairo_int_status_t status = _cairo_scaled_font_subsets_foreach_user(surface->font_subsets, _cairo_pdf_surface_analyze_user_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
	status = _cairo_scaled_font_subsets_foreach_unscaled(surface->font_subsets, _cairo_pdf_surface_emit_unscaled_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
	status = _cairo_scaled_font_subsets_foreach_scaled(surface->font_subsets, _cairo_pdf_surface_emit_scaled_font_subset, surface);
	if(unlikely(status))
		goto BAIL;
	status = _cairo_scaled_font_subsets_foreach_user(surface->font_subsets, _cairo_pdf_surface_emit_scaled_font_subset, surface);
BAIL:
	_cairo_scaled_font_subsets_destroy(surface->font_subsets);
	surface->font_subsets = NULL;
	return status;
...
static cairo_pdf_resource_t _cairo_pdf_surface_write_catalog(cairo_pdf_surface_t * surface)
{
	cairo_pdf_resource_t catalog = _cairo_pdf_surface_new_object(surface);
	if(catalog.id)
		_cairo_output_stream_printf(surface->output, "%d 0 obj\n<< /Type /Catalog\n   /Pages %d 0 R\n>>\nendobj\n", catalog.id, surface->pages_resource.id);
	return catalog;
}
...
static long _cairo_pdf_surface_write_xref(cairo_pdf_surface_t * surface)
{
	...
	int i;
	char buffer[11];
	int  num_objects = _cairo_array_num_elements(&surface->objects);
	long offset = _cairo_output_stream_get_position(surface->output);
	_cairo_output_stream_printf(surface->output, "xref\n%d %d\n", 0, num_objects + 1);
	_cairo_output_stream_puts(surface->output, "0000000000 65535 f \n");
	for(i = 0; i < num_objects; i++) {
		object = (cairo_pdf_object_t *)_cairo_array_index(&surface->objects, i);
		snprintf(buffer, sizeof buffer, "%010ld", object->offset);
		_cairo_output_stream_printf(surface->output, "%s 00000 n \n", buffer);
	}
	return offset;
}

static cairo_int_status_t _cairo_pdf_surface_write_mask_group(cairo_pdf_surface_t * surface,
    cairo_pdf_smask_group_t * group)
{
	...
	status = _cairo_pdf_surface_open_group(surface, &bbox, 0);
	if(unlikely(status))
		return status;

	if(_can_paint_pattern(group->mask)) {
		_cairo_output_stream_puts(surface->output, "q\n");
		status = _cairo_pdf_surface_paint_pattern(surface, CAIRO_OPERATOR_OVER, group->mask, &group->extents, FALSE);
		if(unlikely(status))
			return status;
		_cairo_output_stream_puts(surface->output, "Q\n");
	}
		...
		pattern_res.id = 0;
		gstate_res.id = 0;
		status = _cairo_pdf_surface_add_pdf_pattern(surface, group->mask, CAIRO_OPERATOR_OVER, NULL, &pattern_res, &gstate_res);
		if(unlikely(status))
			return status;
		if(gstate_res.id != 0) {
			...
			if(unlikely(status))
				return status;
			_cairo_output_stream_printf(surface->output, "q /s%d gs /x%d Do Q\n", gstate_res.id, smask_group->group_res.id);
		}
		else {
			status = _cairo_pdf_surface_select_pattern(surface, group->mask, pattern_res, FALSE);
			if(unlikely(status))
				return status;
			_cairo_output_stream_printf(surface->output, "%f %f %f %f re f\n",
			    bbox.p1.x, bbox.p1.y, bbox.p2.x - bbox.p1.x, bbox.p2.y - bbox.p1.y);
	...
	if(_can_paint_pattern(group->source)) {
		_cairo_output_stream_puts(surface->output, "q\n");
		status = _cairo_pdf_surface_paint_pattern(surface, CAIRO_OPERATOR_OVER, group->source, &group->extents, FALSE);
		if(unlikely(status))
			return status;
		_cairo_output_stream_puts(surface->output, "Q\n");
	}
		...
		pattern_res.id = 0;
		gstate_res.id = 0;
		status = _cairo_pdf_surface_add_pdf_pattern(surface, group->source, CAIRO_OPERATOR_OVER, NULL, &pattern_res, &gstate_res);
		if(unlikely(status))
			return status;
		if(gstate_res.id != 0) {
			smask_group = _cairo_pdf_surface_create_smask_group(surface, &group->extents);
	...
	status = _cairo_pdf_surface_close_group(surface, 0);
	if(unlikely(status))
		return status;
...
static cairo_int_status_t _cairo_pdf_surface_write_smask_group(cairo_pdf_surface_t * surface,
    cairo_pdf_smask_group_t * group)
{
	cairo_int_status_t status;
	cairo_box_double_t bbox;
	...
	if(unlikely(status))
		return status;
	status = _cairo_pdf_surface_select_pattern(surface, group->source, group->source_res, group->operation == PDF_STROKE);
	if(unlikely(status))
		return status;
	switch(group->operation) {
		...
		case PDF_MASK:
		    ...
		    break;
		case PDF_FILL:
		    status = _cairo_pdf_operators_fill(&surface->pdf_operators, &group->path, group->fill_rule);
		    break;
		case PDF_STROKE:
		    status = _cairo_pdf_operators_stroke(&surface->pdf_operators, &group->path, &group->style, &group->ctm, &group->ctm_inverse);
		    break;
		case PDF_SHOW_GLYPHS:
		    status = _cairo_pdf_operators_show_text_glyphs(&surface->pdf_operators, group->utf8, group->utf8_len, group->glyphs,
				group->num_glyphs, group->clusters, group->num_clusters, (cairo_text_cluster_flags_t)group->cluster_flags, group->scaled_font);
		    break;
	}
	if(unlikely(status))
		return status;
...
static cairo_int_status_t _cairo_pdf_surface_write_patterns_and_smask_groups(cairo_pdf_surface_t * surface)
{
	cairo_pdf_pattern_t pattern;
	cairo_pdf_smask_group_t * group;
	cairo_pdf_source_surface_t src_surface;
	uint pattern_index, group_index, surface_index;
	cairo_int_status_t status;
...
static cairo_int_status_t _cairo_pdf_surface_analyze_surface_pattern_transparency(cairo_pdf_surface_t  * surface, cairo_surface_pattern_t * pattern)
{
	cairo_image_surface_t  * image;
	void * image_extra;
	cairo_image_transparency_t transparency;
	cairo_int_status_t status = _cairo_surface_acquire_source_image(pattern->surface, &image, &image_extra);
	if(unlikely(status))
		return status;
	if(image->base.status)
		return image->base.status;
	transparency = _cairo_image_analyze_transparency(image);
	status = (transparency == CAIRO_IMAGE_IS_OPAQUE) ? CAIRO_STATUS_SUCCESS : CAIRO_INT_STATUS_FLATTEN_TRANSPARENCY;
	_cairo_surface_release_source_image(pattern->surface, image, image_extra);
	return status;
}

static cairo_bool_t _surface_pattern_supported(cairo_surface_pattern_t * pattern)
{
...
static cairo_bool_t _pattern_supported(const cairo_pattern_t * pattern)
{
	switch(pattern->type) {
		case CAIRO_PATTERN_TYPE_SOLID:
		case CAIRO_PATTERN_TYPE_LINEAR:
...
static cairo_bool_t _pdf_operator_supported(cairo_operator_t op)
{
	switch(op) {
		case CAIRO_OPERATOR_OVER:
...
static cairo_int_status_t _cairo_pdf_surface_analyze_operation(cairo_pdf_surface_t  * surface,
    cairo_operator_t op,
    const cairo_pattern_t  * pattern,
    const CairoIRect * extents)
{
...
static cairo_bool_t _cairo_pdf_surface_operation_supported(cairo_pdf_surface_t  * surface,
    cairo_operator_t op,
    const cairo_pattern_t  * pattern,
    const CairoIRect * extents)
{
...
static cairo_int_status_t _cairo_pdf_surface_start_fallback(cairo_pdf_surface_t * surface)
{
	cairo_box_double_t bbox;
	cairo_int_status_t status;
 ...
 */
static cairo_int_status_t _cairo_pdf_surface_emit_combined_smask(cairo_pdf_surface_t  * surface,
    cairo_operator_t op,
    const cairo_pattern_t   * source,
    ...
    const CairoIRect * extents)
{
	...
	if(mask->type == CAIRO_PATTERN_TYPE_SURFACE && ((cairo_surface_pattern_t*)mask)->surface->type == CAIRO_SURFACE_TYPE_RECORDING) {
		return CAIRO_INT_STATUS_UNSUPPORTED;
	}
	...
	if(transparency != CAIRO_IMAGE_IS_OPAQUE)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	status = _cairo_pdf_surface_acquire_source_image_from_pattern(surface, mask, &image, &image_extra);
	if(unlikely(status))
		return status;
	if(image->base.status)
		return image->base.status;
	...
	e = (src_radius < mask_radius) ? src_radius : mask_radius;
	if(fabs(src_x1 - mask_x1) > e || fabs(src_x2 - mask_x2) > e || fabs(src_y1 - mask_y1) > e || fabs(src_y2 - mask_y2) > e)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	/* Check both images have same device offset */
	if(fabs(src_x_offset - mask_x_offset) > e || fabs(src_y_offset - mask_y_offset) > e)
		return CAIRO_INT_STATUS_UNSUPPORTED;
	if(need_smask) {
		status = _cairo_pdf_surface_add_source_surface(surface, NULL, mask, op, source->filter, FALSE, TRUE,
		    extents, NULL, &smask_res, &mask_width, &mask_height, &mask_x_offset, &mask_y_offset, &extents2);
		if(unlikely(status))
			return status;
	}
	status = _cairo_pdf_operators_flush(&surface->pdf_operators);
	if(unlikely(status))
		return status;
	_cairo_output_stream_puts(surface->output, "q\n");
	status = _cairo_pdf_surface_paint_surface_pattern(surface, op, source, extents, need_smask ? &smask_res : NULL, FALSE);
	if(unlikely(status))
		return status;
	_cairo_output_stream_puts(surface->output, "Q\n");
	status = _cairo_output_stream_get_status(surface->output);
...
static cairo_int_status_t _cairo_pdf_surface_emit_stencil_mask(cairo_pdf_surface_t * surface, cairo_operator_t op,
    const cairo_pattern_t * source, const cairo_pattern_t * mask, const CairoIRect * extents)
{
	cairo_int_status_t status;
	cairo_image_surface_t  * image;
	void * image_extra;
...
static cairo_int_status_t _cairo_pdf_surface_set_clip(cairo_pdf_surface_t * surface, cairo_composite_rectangles_t * composite)
{
	cairo_clip_t * clip = composite->clip;
	if(_cairo_composite_rectangles_can_reduce_clip(composite, clip))
		clip = NULL;
	if(clip == NULL) {
		if(_cairo_composite_rectangles_can_reduce_clip(composite, surface->clipper.clip))
			return CAIRO_STATUS_SUCCESS;
	}
	return _cairo_surface_clipper_set_clip(&surface->clipper, clip);
}

static cairo_int_status_t _cairo_pdf_surface_paint(void * abstract_surface, cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	cairo_pdf_smask_group_t * group;
	cairo_pdf_resource_t pattern_res, gstate_res;
	cairo_composite_rectangles_t extents;
	cairo_int_status_t status = _cairo_composite_rectangles_init_for_paint(&extents, &surface->base, op, source, clip);
	if(unlikely(status))
		return status;
	...
	if(_can_paint_pattern(source)) {
		_cairo_output_stream_puts(surface->output, "q\n");
		status = _cairo_pdf_surface_paint_pattern(surface, op, source, &extents.bounded, FALSE);
		if(unlikely(status))
			goto cleanup;
		_cairo_output_stream_puts(surface->output, "Q\n");
	...
	if(gstate_res.id != 0) {
		...
		if(unlikely(status))
			goto cleanup;
		_cairo_output_stream_printf(surface->output, "q /s%d gs /x%d Do Q\n", gstate_res.id, group->group_res.id);
	}
	else {
		status = _cairo_pdf_surface_select_pattern(surface, source, pattern_res, FALSE);
		if(unlikely(status))
...
static cairo_int_status_t _cairo_pdf_surface_mask(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t  * source,
    const cairo_pattern_t  * mask,
    const cairo_clip_t * clip)
{
	...
	CairoIRect r;
	cairo_box_t box;
	cairo_int_status_t status = _cairo_composite_rectangles_init_for_mask(&extents, &surface->base, op, source, mask, clip);
	if(unlikely(status))
		return status;
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
...
static cairo_int_status_t _cairo_pdf_surface_stroke(void * abstract_surface, cairo_operator_t op,
    const cairo_pattern_t * source, const cairo_path_fixed_t * path, const cairo_stroke_style_t   * style,
    const cairo_matrix_t * ctm, const cairo_matrix_t * ctm_inverse, double tolerance, cairo_antialias_t antialias,
    const cairo_clip_t   * clip)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
	cairo_pdf_smask_group_t * group;
	...
	if(gstate_res.id != 0) {
		...
		if(unlikely(status))
			goto cleanup;
		status = _cairo_pdf_operators_stroke(&surface->pdf_operators, path, style, ctm, ctm_inverse);
		if(unlikely(status))
			goto cleanup;
		status = _cairo_pdf_surface_unselect_pattern(surface);
		if(unlikely(status))
...
static cairo_int_status_t _cairo_pdf_surface_fill(void * abstract_surface,
    cairo_operator_t op, const cairo_pattern_t  * source, const cairo_path_fixed_t* path, CairoFillRule fill_rule,
    double tolerance, cairo_antialias_t antialias, const cairo_clip_t * clip)
{
	...
	if(extents.is_bounded) {
		CairoIRect mask;
		cairo_box_t box;
		_cairo_path_fixed_fill_extents(path, fill_rule, tolerance, &mask);
		_cairo_box_from_rectangle(&box, &mask);
		status = _cairo_composite_rectangles_intersect_mask_extents(&extents, &box);
		if(unlikely(status))
			goto cleanup;
	...
	if(_can_paint_pattern(source)) {
		...
		if(unlikely(status))
			goto cleanup;
		_cairo_output_stream_puts(surface->output, "q\n");
		status =  _cairo_pdf_operators_clip(&surface->pdf_operators, path, fill_rule);
		if(unlikely(status))
			goto cleanup;
		status = _cairo_pdf_surface_paint_pattern(surface, op, source, &extents.bounded, FALSE);
		if(unlikely(status))
...
static cairo_int_status_t _cairo_pdf_surface_fill_stroke(void * abstract_surface,
    ...
    const cairo_clip_t  * clip)
{
	...
	if(extents.is_bounded) {
		CairoIRect mask;
		cairo_box_t box;
		_cairo_path_fixed_fill_extents(path, fill_rule, fill_tolerance, &mask);
		_cairo_box_from_rectangle(&box, &mask);
		status = _cairo_composite_rectangles_intersect_mask_extents(&extents, &box);
		if(unlikely(status))
			goto cleanup;
	...
	gstate_res.id = 0;
	status = _cairo_pdf_surface_add_pdf_pattern(surface, fill_source, fill_op,
	    &extents.bounded, &fill_pattern_res, &gstate_res);
	if(unlikely(status))
		goto cleanup;
	assert(gstate_res.id == 0);
	stroke_pattern_res.id = 0;
	gstate_res.id = 0;
	status = _cairo_pdf_surface_add_pdf_pattern(surface, stroke_source, stroke_op,
	    &extents.bounded, &stroke_pattern_res, &gstate_res);
	if(unlikely(status))
		goto cleanup;
	assert(gstate_res.id == 0);
	...
	if(unlikely(status))
		goto cleanup;
	status = _cairo_pdf_surface_select_pattern(surface, stroke_source, stroke_pattern_res, TRUE);
	if(unlikely(status))
		goto cleanup;
	status = _cairo_pdf_operators_fill_stroke(&surface->pdf_operators, path, fill_rule,
...
static cairo_bool_t _cairo_pdf_surface_has_show_text_glyphs(void * abstract_surface)
{
	return TRUE;
}

static cairo_int_status_t _cairo_pdf_surface_show_text_glyphs(void * abstract_surface,
    cairo_operator_t op,
    const cairo_pattern_t  * source,
    const char * utf8,
    int utf8_len,
    ...
    const cairo_clip_t  * clip)
{
	...
	cairo_int_status_t status = _cairo_composite_rectangles_init_for_glyphs(&extents,
	    &surface->base, op, source, scaled_font, glyphs, num_glyphs, clip, &overlap);
	if(unlikely(status))
		return status;
	if(surface->paginated_mode == CAIRO_PAGINATED_MODE_ANALYZE) {
		status = _cairo_pdf_surface_analyze_operation(surface, op, source, &extents.bounded);
	...
	if(gstate_res.id != 0) {
		...
		status = _cairo_pdf_operators_show_text_glyphs(&surface->pdf_operators, utf8, utf8_len,
		    glyphs, num_glyphs, clusters, num_clusters, cluster_flags, scaled_font);
		if(unlikely(status))
			goto cleanup;
		status = _cairo_pdf_surface_unselect_pattern(surface);
		if(unlikely(status))
...
static const char ** _cairo_pdf_surface_get_supported_mime_types(void * abstract_surface)
{
	return _cairo_pdf_supported_mime_types;
}

static void _cairo_pdf_surface_set_paginated_mode(void * abstract_surface, cairo_paginated_mode_t paginated_mode)
{
	cairo_pdf_surface_t * surface = (cairo_pdf_surface_t *)abstract_surface;
