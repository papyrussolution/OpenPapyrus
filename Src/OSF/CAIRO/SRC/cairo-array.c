 ...
 **/
void _cairo_array_init_snapshot(cairo_array_t * array, const cairo_array_t * other)
{
...
{
	if(!array->is_snapshot && array->elements) {
		free(*array->elements);
		free(array->elements);
	}
}
...
cairo_status_t _cairo_array_grow_by(cairo_array_t * array, unsigned int additional)
{
	...
	if(array->elements == NULL) {
		array->elements = (char **)malloc(sizeof(char *));
 ...
 * operation.
 **/
cairo_status_t _cairo_array_append_multiple(cairo_array_t * array, const void * elements, int num_elements)
{
 ...
 * This space was left intentionally blank, but gtk-doc filled it.
 **/
int _cairo_array_size(cairo_array_t * array)
{
	return array->size;
}
...
{
	unsigned int num_slots = array->num_elements;
	if(num_slots) {
...
cairo_status_t _cairo_user_data_array_copy(cairo_user_data_array_t * dst, cairo_user_data_array_t * src)
{
	...
	if(src->num_elements == 0)
		return CAIRO_STATUS_SUCCESS;
	return _cairo_array_append_multiple(dst, _cairo_array_index(src, 0), src->num_elements);
}
