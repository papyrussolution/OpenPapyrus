	...
	cairo_scaled_font_subsets_t * font_subsets)
{
	...
	_cairo_surface_init(&surface->base, &cairo_type3_glyph_surface_backend,
		NULL, /* device */ CAIRO_CONTENT_COLOR_ALPHA);
...
static cairo_status_t _cairo_type3_glyph_surface_finish(void * abstract_surface)
{
...
static cairo_int_status_t _cairo_type3_glyph_surface_paint(void * abstract_surface,
	cairo_operator_t op, const cairo_pattern_t * source, cairo_clip_t * clip)
{
	...
	if(unlikely(status))
		goto fail;
	status = _cairo_type3_glyph_surface_emit_image_pattern(surface, image, &pattern->base.matrix);
fail:
	_cairo_surface_release_source_image(pattern->surface, image, image_extra);
	return status;
}

static cairo_int_status_t _cairo_type3_glyph_surface_mask(void * abstract_surface,
	cairo_operator_t op,
...
static cairo_int_status_t _cairo_type3_glyph_surface_stroke(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	...
	cairo_clip_t * clip)
{
	...
	if(unlikely(status))
		return status;
	return _cairo_pdf_operators_stroke(&surface->pdf_operators, path, style, ctm, ctm_inverse);
}

static cairo_int_status_t _cairo_type3_glyph_surface_fill(void * abstract_surface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule,
	...
	int * remaining_glyphs)
{
	...
	font = cairo_scaled_font_create(scaled_font->font_face,
		&scaled_font->font_matrix, &new_ctm, &scaled_font->options);
	if(unlikely(font->status))
		return font->status;
	status = _cairo_pdf_operators_show_text_glyphs(&surface->pdf_operators, NULL, 0, glyphs, num_glyphs, NULL, 0, CAIRO_TEXT_CLUSTER_FLAG_UNDEF, font);
...
static cairo_status_t _cairo_type3_glyph_surface_emit_fallback_image(cairo_type3_glyph_surface_t * surface, unsigned long glyph_index)
{
	cairo_scaled_glyph_t * scaled_glyph;
	cairo_image_surface_t * image;
	cairo_matrix_t mat;
	double x, y;
	cairo_status_t status = _cairo_scaled_glyph_lookup(surface->scaled_font, glyph_index, CAIRO_SCALED_GLYPH_INFO_METRICS|CAIRO_SCALED_GLYPH_INFO_SURFACE, &scaled_glyph);
	if(unlikely(status))
		return status;
...
cairo_status_t _cairo_type3_glyph_surface_analyze_glyph(void * abstract_surface, unsigned long glyph_index)
{
	cairo_type3_glyph_surface_t * surface = (cairo_type3_glyph_surface_t *)abstract_surface;
	cairo_scaled_glyph_t * scaled_glyph;
	...
	status = _cairo_scaled_glyph_lookup(surface->scaled_font,
		glyph_index, CAIRO_SCALED_GLYPH_INFO_RECORDING_SURFACE, &scaled_glyph);
...
cairo_status_t _cairo_type3_glyph_surface_emit_glyph(void * abstract_surface,
	cairo_output_stream_t * stream, unsigned long glyph_index, cairo_box_t * bbox, double * width)
{
	cairo_type3_glyph_surface_t * surface = (cairo_type3_glyph_surface_t *)abstract_surface;
	cairo_scaled_glyph_t * scaled_glyph;
	cairo_status_t status, status2;
	...
	_cairo_scaled_font_freeze_cache(surface->scaled_font);
	status = _cairo_scaled_glyph_lookup(surface->scaled_font,
		glyph_index, CAIRO_SCALED_GLYPH_INFO_METRICS|CAIRO_SCALED_GLYPH_INFO_RECORDING_SURFACE, &scaled_glyph);
	if(status == CAIRO_INT_STATUS_UNSUPPORTED) {
	...
	*bbox = scaled_glyph->bbox;
	_cairo_matrix_transform_bounding_box_fixed(&surface->scaled_font->scale_inverse, bbox, NULL);
	_cairo_output_stream_printf(surface->stream, "%f 0 %f %f %f %f d1\n",
		x_advance, _cairo_fixed_to_double(bbox->p1.x), -_cairo_fixed_to_double(bbox->p2.y),
		_cairo_fixed_to_double(bbox->p2.x), -_cairo_fixed_to_double(bbox->p1.y));
	if(status == CAIRO_STATUS_SUCCESS) {
