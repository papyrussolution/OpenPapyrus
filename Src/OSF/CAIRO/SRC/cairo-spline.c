...
static cairo_status_t _cairo_spline_add_point(cairo_spline_t * spline, cairo_point_t * point)
{
...
static double _cairo_spline_error_squared(const cairo_spline_knots_t * knots)
{
	...
	*/
	double bdx = _cairo_fixed_to_double(knots->b.x-knots->a.x);
	double bdy = _cairo_fixed_to_double(knots->b.y-knots->a.y);
	double cdx = _cairo_fixed_to_double(knots->c.x-knots->a.x);
	double cdy = _cairo_fixed_to_double(knots->c.y-knots->a.y);
	if(knots->a.x != knots->d.x || knots->a.y != knots->d.y) {
		 ...
		 *     u = ((p - p1) ∙ (p2 - p1)) / ∥p2 - p1∥²;
		 */
		double dx = _cairo_fixed_to_double(knots->d.x-knots->a.x);
		double dy = _cairo_fixed_to_double(knots->d.y-knots->a.y);
		double v = dx*dx+dy*dy;
		double u = bdx*dx+bdy*dy;
		if(u <= 0) {
			/* bdx -= 0;
			 * bdy -= 0;
			 */
...
static cairo_status_t _cairo_spline_decompose_into(cairo_spline_knots_t * s1, double tolerance_squared,
...
cairo_status_t _cairo_spline_bound(cairo_spline_add_point_func_t add_point_func,
	void * closure, const cairo_point_t * p0, const cairo_point_t * p1,
