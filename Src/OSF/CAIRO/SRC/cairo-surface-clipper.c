...
cairo_status_t _cairo_surface_clipper_set_clip(cairo_surface_clipper_t * clipper,
	cairo_clip_t * clip)
{
	cairo_status_t status;
	cairo_bool_t clear;
	...
	if(clear) {
		...
		if(clip != NULL && clip->path != NULL) {
			status = _cairo_surface_clipper_intersect_clip_path_recursive(clipper, clip->path);
