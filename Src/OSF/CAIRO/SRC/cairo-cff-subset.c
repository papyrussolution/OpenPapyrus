...
static unsigned char * decode_operator(unsigned char * p, unsigned short * pOp)
{
	unsigned short op = 0;
	op = *p++;
	if(op == 12) {
...
static cairo_status_t cff_index_append_copy(cairo_array_t * index,
	const unsigned char * object, unsigned int length)
{
	cff_index_element_t element;
	cairo_status_t status;
	element.length = length;
	element.is_copy = TRUE;
	element.data = (unsigned char *)malloc(element.length);
...
static void _cairo_dict_init_key(cff_dict_operator_t * key, int op)
{
	key->base.hash = (unsigned long)op;
	key->op = op;
}

static cairo_status_t cff_dict_create_operator(int op, unsigned char * operand, int size, cff_dict_operator_t ** out)
{
	cff_dict_operator_t * p_op = (cff_dict_operator_t *)malloc(sizeof(cff_dict_operator_t));
	if(unlikely(p_op == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	_cairo_dict_init_key(p_op, op);
	p_op->operand = (unsigned char *)malloc(size);
	if(unlikely(p_op->operand == NULL)) {
		free(p_op);
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	}
	memcpy(p_op->operand, operand, size);
	p_op->operand_length = size;
	p_op->operand_offset = -1;
...
static cairo_status_t cff_dict_read(cairo_hash_table_t * dict, unsigned char * p, int dict_size)
{
	...
	cff_dict_operator_t * p_op;
	unsigned short op;
	cairo_status_t status = CAIRO_STATUS_SUCCESS;
	int size;
	unsigned char * end = p+dict_size;
	...
	while(p < end) {
		...
		if(size != 0) {
			...
			p = decode_operator(p, &op);
			status = cff_dict_create_operator(op, (unsigned char *)_cairo_array_index(&operands, 0), _cairo_array_num_elements(&operands), &p_op);
			if(unlikely(status))
				goto fail;
			status = _cairo_hash_table_insert(dict, &p_op->base);
			if(unlikely(status))
				goto fail;
...
static void cff_dict_remove(cairo_hash_table_t * dict, unsigned short op)
{
	cff_dict_operator_t key, * p_op;
	_cairo_dict_init_key(&key, op);
	p_op = (cff_dict_operator_t *)_cairo_hash_table_lookup(dict, &key.base);
	if(p_op != NULL) {
		free(p_op->operand);
		_cairo_hash_table_remove(dict, (cairo_hash_entry_t *)p_op);
		free(p_op);
	}
}

static unsigned char * cff_dict_get_operands(cairo_hash_table_t * dict, unsigned short op, int * size)
{
	cff_dict_operator_t key, * p_op;
	_cairo_dict_init_key(&key, op);
	p_op = (cff_dict_operator_t *)_cairo_hash_table_lookup(dict, &key.base);
	if(p_op != NULL) {
		*size = p_op->operand_length;
		return p_op->operand;
	}
	return NULL;
}

static cairo_status_t cff_dict_set_operands(cairo_hash_table_t * dict, unsigned short op, unsigned char * operand, int size)
{
	cff_dict_operator_t key, * p_op;
	cairo_status_t status;
	_cairo_dict_init_key(&key, op);
	...
	if(p_op != NULL) {
		...
		status = cff_dict_create_operator(op, operand, size, &p_op);
		if(unlikely(status))
			return status;
		status = _cairo_hash_table_insert(dict, &p_op->base);
		if(unlikely(status))
...
static int cff_dict_get_location(cairo_hash_table_t * dict, unsigned short op, int * size)
{
	cff_dict_operator_t key, * p_op;
	_cairo_dict_init_key(&key, op);
	p_op = (cff_dict_operator_t *)_cairo_hash_table_lookup(dict, &key.base);
...
static cairo_status_t cairo_cff_font_subset_dict_string(cairo_cff_font_t * font, cairo_hash_table_t * dict, int op)
{
	int size;
	int sid;
	unsigned char buf[100];
	cff_index_element_t * element;
	cairo_status_t status;
	unsigned char * p = cff_dict_get_operands(dict, op, &size);
	if(!p)
		return CAIRO_STATUS_SUCCESS;
	...
	status = cff_dict_set_operands(dict, op, buf, p-buf);
	if(unlikely(status))
		return status;
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t cairo_cff_font_subset_charstrings(cairo_cff_font_t * font)
{
	cff_index_element_t * element;
	unsigned int i;
	cairo_status_t status;
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		element = (cff_index_element_t *)_cairo_array_index(&font->charstrings_index, font->scaled_font_subset->glyphs[i]);
		status = cff_index_append(&font->charstrings_subset_index, element->data, element->length);
		if(unlikely(status))
			return status;
	}
	return CAIRO_STATUS_SUCCESS;
...
{
	unsigned int i;
	cairo_status_t status = cairo_cff_font_subset_dict_strings(font, font->top_dict);
	if(unlikely(status))
		return status;
	if(font->is_cid) {
...
{
	cairo_status_t status = cairo_cff_font_set_ros_strings(font);
	if(unlikely(status))
		return status;
...
static void cairo_cff_font_set_topdict_operator_to_cur_pos(cairo_cff_font_t * font, int op)
{
	int size;
	unsigned char buf[10];
	unsigned char * op_ptr;
	int cur_pos = _cairo_array_num_elements(&font->output);
	unsigned char * buf_end = encode_integer_max(buf, cur_pos);
	int offset = cff_dict_get_location(font->top_dict, op, &size);
	assert(offset > 0);
	op_ptr = (unsigned char *)_cairo_array_index(&font->output, offset);
...
static cairo_status_t cairo_cff_font_write_top_dict(cairo_cff_font_t * font)
{
	...
	p = (unsigned char *)_cairo_array_index(&font->output, offset_index);
...
static cairo_status_t cairo_cff_font_write_charset(cairo_cff_font_t * font)
{
	unsigned char byte;
	uint16_t word;
	cairo_status_t status;
	...
	cairo_hash_table_t * private_dict)
{
	...
	assert(offset > 0);
	p = (unsigned char *)_cairo_array_index(&font->output, offset);
	memcpy(p, buf, buf_end-buf);
	return CAIRO_STATUS_SUCCESS;
}

static cairo_status_t cairo_cff_font_write_local_sub(cairo_cff_font_t * font,
	int dict_num, cairo_hash_table_t * private_dict, cairo_array_t * local_sub_index)
{
	int offset;
	int size;
	unsigned char buf[10];
	...
	const char * subset_name)
{
	...
	if(font->ps_name == NULL) {
		...
		snprintf(font->ps_name, 30, "CairoFont-%u-%u", scaled_font_subset->font_id, scaled_font_subset->subset_id);
	}
	...
	font->data = (unsigned char *)malloc(data_length);
...
{
	cairo_status_t status;
	cairo_cff_font_t * font = (cairo_cff_font_t *)malloc(sizeof(cairo_cff_font_t));
	if(unlikely(font == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	font->backend = NULL;
	font->scaled_font_subset = scaled_font_subset;
...
static cairo_int_status_t cairo_cff_font_fallback_generate(cairo_cff_font_t * font,
	cairo_type2_charstrings_t * type2_subset, const char ** data, unsigned long * length)
{
	cairo_int_status_t status;
	cff_header_t header;
	...
	for(i = 0; i < font->scaled_font_subset->num_glyphs; i++) {
		...
		status = cff_index_append(&font->charstrings_subset_index, (unsigned char *)_cairo_array_index(charstring, 0), _cairo_array_num_elements(charstring));
		if(unlikely(status))
			return status;
	}
	status = cairo_cff_font_write_subset(font);
...
cairo_status_t _cairo_cff_fallback_init(cairo_cff_subset_t * cff_subset,
	const char * subset_name, cairo_scaled_font_subset_t * font_subset)
{
	cairo_cff_font_t * font = NULL; /* squelch bogus compiler warning */
	const char * data = NULL; /* squelch bogus compiler warning */
	unsigned long length = 0; /* squelch bogus compiler warning */
