...
static unsigned long _cairo_cogl_linear_gradient_hash(unsigned int n_stops,
...
static cairo_cogl_linear_gradient_t * _cairo_cogl_linear_gradient_lookup(cairo_cogl_device_t         * ctx,
    unsigned long hash,
...
cairo_cogl_linear_gradient_t * _cairo_cogl_linear_gradient_reference(cairo_cogl_linear_gradient_t * gradient)
{
...
static float get_max_color_component_range(const cairo_color_stop_t * color0, const cairo_color_stop_t * color1)
{
	float range;
	float max = 0;
...
static int _cairo_cogl_linear_gradient_width_for_stops(cairo_extend_t extend,
    unsigned int n_stops,
    const cairo_gradient_stop_t * stops)
{
	unsigned int n;
...
static CoglPixelFormat _cairo_cogl_linear_gradient_format_for_stops(cairo_extend_t extend,
    unsigned int n_stops,
    const cairo_gradient_stop_t * stops)
{
	unsigned int n;
...
static cairo_cogl_gradient_compatibility_t _cairo_cogl_compatibility_from_extend_mode(cairo_extend_t extend_mode)
{
	switch(extend_mode)
	{
...
cairo_cogl_linear_texture_entry_t * _cairo_cogl_linear_gradient_texture_for_extend(cairo_cogl_linear_gradient_t * gradient,
...
static void color_stop_lerp(const cairo_color_stop_t * c0,
...
static size_t _cairo_cogl_linear_gradient_size(cairo_cogl_linear_gradient_t * gradient)
{
	GList * l;
	size_t size = 0;
	for(l = gradient->textures; l; l = l->next) {
...
static void emit_stop(CoglVertexP2C4 ** position,
    float left,
    float right,
...
static void dump_gradient_to_png(CoglTexture * texture)
{
...
cairo_int_status_t _cairo_cogl_get_linear_gradient(cairo_cogl_device_t * device,
    cairo_extend_t extend_mode,
    int n_stops,
    const cairo_gradient_stop_t * stops,
