...
static void _cairo_pattern_init(cairo_pattern_t * pattern, cairo_pattern_type_t type)
{
#if HAVE_VALGRIND
	switch(type) {
...
cairo_status_t _cairo_pattern_create_copy(cairo_pattern_t ** pattern_out,
...
void _cairo_pattern_init_linear(cairo_linear_pattern_t * pattern,
...
void _cairo_pattern_init_radial(cairo_radial_pattern_t * pattern,
...
void cairo_pattern_set_matrix(cairo_pattern_t * pattern, const cairo_matrix_t * matrix)
{
	...
	cairo_status_t status;
	if(pattern->status == 0 && memcmp(&pattern->matrix, matrix, sizeof(cairo_matrix_t)) != 0) {
...
static void _cairo_linear_pattern_classify(cairo_linear_pattern_t * pattern,
	double offset_x,
	double offset_y,
	int width,
	int height,
	...
	cairo_bool_t * is_vertical)
{
	...
	_cairo_matrix_get_affine(&pattern->base.base.matrix, &a, &b, &c, &d, &tx, &ty);
...
static cairo_int_status_t _cairo_pattern_acquire_surface_for_gradient(const cairo_gradient_pattern_t * pattern,
	cairo_surface_t * dst, int x, int y, unsigned int width, unsigned int height, cairo_surface_t ** out, cairo_surface_attributes_t * attr)
{
	...
	if(pattern->base.type == CAIRO_PATTERN_TYPE_LINEAR) {
		...
		cairo_fixed_t xdim = linear->p2.x-linear->p1.x;
		cairo_fixed_t ydim = linear->p2.y-linear->p1.y;
...
#define PIXMAN_MAX_INT ((pixman_fixed_1>>1)-pixman_fixed_e)     /* need to ensure deltas also fit */
		...
		pixman_image = pixman_image_create_radial_gradient(&c1, &c2, r1, r2, pixman_stops, pattern->n_stops);
	}
	...
	if(_cairo_surface_is_image(dst)) {
		image = (cairo_image_surface_t *)_cairo_image_surface_create_for_pixman_image(pixman_image, PIXMAN_a8r8g8b8);
	...
	if(pattern->base.type == CAIRO_PATTERN_TYPE_LINEAR) {
		cairo_bool_t is_horizontal;
		cairo_bool_t is_vertical;
		_cairo_linear_pattern_classify((cairo_linear_pattern_t *)pattern, x, y, width, height, &is_horizontal, &is_vertical);
		if(is_horizontal) {
			height = 1;
			repeat = TRUE;
		}
	...
	if(!pixman_image_set_filter(pixman_image, PIXMAN_FILTER_BILINEAR, NULL, 0)) {
		pixman_image_unref(pixman_image);
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	}
	image = (cairo_image_surface_t *)cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
	if(image->base.status) {
		pixman_image_unref(pixman_image);
		return image->base.status;
	}
...
static struct {
	struct _cairo_pattern_solid_surface_cache cache[MAX_SURFACE_CACHE_SIZE];
	int size;
} solid_surface_cache;

static cairo_bool_t _cairo_pattern_solid_surface_matches(
	const struct _cairo_pattern_solid_surface_cache * cache,
	const cairo_solid_pattern_t * pattern, cairo_surface_t * dst)
{
...
static cairo_bool_t _cairo_pattern_solid_surface_matches_color(
	const struct _cairo_pattern_solid_surface_cache * cache,
...
static cairo_int_status_t _cairo_pattern_acquire_surface_for_solid(const cairo_solid_pattern_t * pattern,
	cairo_surface_t * dst,
	int x,
	int y,
	unsigned int width,
	...
	cairo_surface_attributes_t * attribs)
{
	...
	/* Check cache first */
	if(i < solid_surface_cache.size && _cairo_pattern_solid_surface_matches_color(&solid_surface_cache.cache[i], pattern, dst)) {
		goto DONE;
	}
	for(i = 0; i < solid_surface_cache.size; i++) {
		if(_cairo_pattern_solid_surface_matches_color(&solid_surface_cache.cache[i], pattern, dst)) {
			goto DONE;
		}
	}
	...
	if(solid_surface_cache.size == MAX_SURFACE_CACHE_SIZE) {
		...
		if(_cairo_pattern_solid_surface_matches(&solid_surface_cache.cache[i], pattern, dst)) {
...
static void _cairo_pattern_reset_solid_surface_cache(void)
{
	CAIRO_MUTEX_LOCK(_cairo_pattern_solid_surface_cache_lock);
...
static void _extents_to_linear_parameter(const cairo_linear_pattern_t * linear, const cairo_rectangle_int_t * extents, double t[2])
{
	double t0, tdx, tdy;
	double p1x = _cairo_fixed_to_double(linear->p1.x);
	double p1y = _cairo_fixed_to_double(linear->p1.y);
	double pdx = _cairo_fixed_to_double(linear->p2.x)-p1x;
	double pdy = _cairo_fixed_to_double(linear->p2.y)-p1y;
...
{
	return radial->r1 == radial->r2 && (radial->r1 == 0 /* && radial->r2 == 0 */ || (radial->c1.x == radial->c2.x && radial->c1.y == radial->c2.y));
}
	...
	const cairo_rectangle_int_t * extents, cairo_color_t * color)
{
	...
	for(i = 1; i < gradient->n_stops; i++)
		if(!_cairo_color_stop_equal(&gradient->stops[0].color, &gradient->stops[i].color))
			return FALSE;
	_cairo_color_init_rgba(color, gradient->stops[0].color.red, gradient->stops[0].color.green,
		gradient->stops[0].color.blue, gradient->stops[0].color.alpha);
	return TRUE;
}
	...
	const cairo_rectangle_int_t * r)
{
	...
	if(r != NULL) {
		...
		if(!_cairo_surface_get_extents(pattern->surface, &extents))
			return TRUE;
		if(r->x >= extents.x && r->y >= extents.y &&
...
static cairo_bool_t _gradient_is_opaque(const cairo_gradient_pattern_t * gradient, const cairo_rectangle_int_t * extents)
{
	unsigned int i;
	assert(gradient->base.type == CAIRO_PATTERN_TYPE_LINEAR || gradient->base.type == CAIRO_PATTERN_TYPE_RADIAL);
 ...
 **/
cairo_bool_t _cairo_pattern_is_opaque(const cairo_pattern_t * abstract_pattern, const cairo_rectangle_int_t * extents)
{
 ...
 */
cairo_filter_t _cairo_pattern_analyze_filter(const cairo_pattern_t * pattern, double * pad_out)
{
	double pad;
...
static cairo_int_status_t _cairo_pattern_acquire_surface_for_surface(const cairo_surface_pattern_t * pattern,
	cairo_surface_t * dst,
	int x,
	int y,
	unsigned int width,
	...
	cairo_surface_attributes_t * attr)
{
	...
	cairo_surface_t * surface = cairo_surface_reference(pattern->surface);
	cairo_bool_t is_identity = FALSE;
	...
	y2 = y+(int)height;
	if(!is_identity) {
		_cairo_matrix_transform_bounding_box(&attr->matrix, &x1, &y1, &x2, &y2, NULL);
	}
	sampled_area.x = (int)floor(x1-pad);
	sampled_area.y = (int)floor(y1-pad);
	sampled_area.width  = (int)ceil(x2+pad)-sampled_area.x;
	sampled_area.height = (int)ceil(y2+pad)-sampled_area.y;
	sampled_area.x += tx;
	sampled_area.y += ty;
	...
	status = _cairo_surface_clone_similar(dst, surface, extents.x, extents.y, extents.width, extents.height, &x, &y, out);
	if(unlikely(status))
		goto BAIL;
	if(x != 0 || y != 0) {
		if(is_identity) {
	...
	if(!is_identity) {
		cairo_matrix_t m = attr->matrix;
		cairo_status_t invert_status = cairo_matrix_invert(&m);
 ...
 **/
cairo_int_status_t _cairo_pattern_acquire_surface(const cairo_pattern_t * pattern,
	cairo_surface_t * dst,
	int x,
	int y,
	unsigned int width,
	...
	cairo_surface_attributes_t * attributes)
{
	...
	switch(pattern->type) {
		...
		return _cairo_pattern_acquire_surface_for_solid((cairo_solid_pattern_t *)pattern,
			dst, x, y, width, height, surface_out, attributes);
	    case CAIRO_PATTERN_TYPE_LINEAR:
	    case CAIRO_PATTERN_TYPE_RADIAL:
		...
		return _cairo_pattern_acquire_surface_for_surface((cairo_surface_pattern_t *)pattern,
			dst, x, y, width, height, flags, surface_out, attributes);
	    default:
		ASSERT_NOT_REACHED;
		return _cairo_error(CAIRO_STATUS_PATTERN_TYPE_MISMATCH);
	}
 ...
 **/
void _cairo_pattern_release_surface(const cairo_pattern_t * pattern,
	cairo_surface_t * surface, cairo_surface_attributes_t * attributes)
{
	cairo_surface_destroy(surface);
}

cairo_int_status_t _cairo_pattern_acquire_surfaces(const cairo_pattern_t * src,
	...
	cairo_surface_attributes_t * mask_attributes)
{
	...
	if(src->type == CAIRO_PATTERN_TYPE_SOLID && mask && !mask->has_component_alpha && mask->type == CAIRO_PATTERN_TYPE_SOLID) {
	...
	status = _cairo_pattern_acquire_surface(src, dst,
		src_x, src_y, width, height, flags, src_out, src_attributes);
	if(unlikely(status))
		goto BAIL;
	if(mask == NULL) {
		*mask_out = NULL;
 ...
 **/
void _cairo_pattern_get_extents(const cairo_pattern_t * pattern, cairo_rectangle_int_t * extents)
{
	double x1, y1, x2, y2;
	cairo_status_t status;
	switch(pattern->type) {
	...
	extents->x = (int)x1;
	extents->width = (int)(x2-x1);
	extents->y = (int)y1;
	extents->height = (int)(y2-y1);
	return;
...
static unsigned long _cairo_solid_pattern_hash(unsigned long hash, const cairo_pattern_t * pattern)
{
...
static unsigned long _cairo_gradient_color_stops_hash(unsigned long hash, const cairo_gradient_pattern_t * gradient)
{
	unsigned int n;
	...
	for(n = 0; n < gradient->n_stops; n++) {
		...
		hash = _cairo_hash_bytes(hash, &gradient->stops[n].color, sizeof(cairo_color_t));
	}
	return hash;
}
...
static unsigned long _cairo_surface_pattern_hash(unsigned long hash, const cairo_pattern_t * pattern)
{
...
static unsigned long _cairo_gradient_pattern_color_stops_size(const cairo_pattern_t * pattern)
{
	cairo_gradient_pattern_t * gradient = (cairo_gradient_pattern_t *)pattern;
...
unsigned long _cairo_pattern_size(const cairo_pattern_t * pattern)
{
	...
	switch(pattern->type) {
		...
		break;
	    case CAIRO_PATTERN_TYPE_LINEAR:
		return sizeof(cairo_linear_pattern_t)+_cairo_gradient_pattern_color_stops_size(pattern);
		break;
	    case CAIRO_PATTERN_TYPE_RADIAL:
		return sizeof(cairo_radial_pattern_t)+_cairo_gradient_pattern_color_stops_size(pattern);
	    default:
		ASSERT_NOT_REACHED;
		return 0;
	}
...
static cairo_bool_t _cairo_solid_pattern_equal(const cairo_pattern_t * A, const cairo_pattern_t * B)
{
...
static cairo_bool_t _cairo_surface_pattern_equal(const cairo_pattern_t * A, const cairo_pattern_t * B)
{
...
cairo_bool_t _cairo_pattern_equal(const cairo_pattern_t * a, const cairo_pattern_t * b)
{
	...
	switch(a->type) {
	    case CAIRO_PATTERN_TYPE_SOLID: return _cairo_solid_pattern_equal(a, b);
	    case CAIRO_PATTERN_TYPE_LINEAR: return _cairo_linear_pattern_equal((cairo_linear_pattern_t *)a, (cairo_linear_pattern_t *)b);
	    case CAIRO_PATTERN_TYPE_RADIAL: return _cairo_radial_pattern_equal((cairo_radial_pattern_t *)a, (cairo_radial_pattern_t *)b);
	    case CAIRO_PATTERN_TYPE_SURFACE: return _cairo_surface_pattern_equal(a, b);
 ...
 * Since: 1.4
 **/
cairo_status_t cairo_pattern_get_radial_circles(cairo_pattern_t * pattern, double * x0, double * y0, double * r0,
