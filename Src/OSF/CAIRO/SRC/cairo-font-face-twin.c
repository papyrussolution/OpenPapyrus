...
static void parse_field(twin_face_properties_t * props, const char * str, int len)
{
	if(field_matches("Normal", str, len))
		return;
#define FIELD(NAME) if(find_field(STRINGIFY(NAME), NAME ## _map, ARRAY_LENGTH(NAME ## _map), str, len, (int *)(void *)&props->NAME)) return;
...
static cairo_status_t twin_font_face_create_properties(cairo_font_face_t * twin_face,
	twin_face_properties_t ** props_out)
{
	cairo_status_t status;
	twin_face_properties_t * props = (twin_face_properties_t *)malloc(sizeof(twin_face_properties_t));
	...
	status = cairo_font_face_set_user_data(twin_face, &twin_properties_key, props, free);
	if(unlikely(status)) {
		free(props);
		return status;
	}
...
static cairo_status_t twin_font_face_set_properties_from_toy(cairo_font_face_t * twin_face, cairo_toy_font_face_t * toy_face)
{
	...
	cairo_status_t status = twin_font_face_create_properties(twin_face, &props);
	if(unlikely(status))
		return status;
...
static void compute_hinting_scales(cairo_t * cr,
...
{
	cairo_status_t status;
	twin_scaled_properties_t * props = (twin_scaled_properties_t *)malloc(sizeof(twin_scaled_properties_t));
	if(unlikely(props == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
...
static cairo_status_t twin_scaled_font_render_glyph(cairo_scaled_font_t * scaled_font,
	unsigned long glyph, cairo_t * cr, cairo_text_extents_t * metrics)
{
...
static cairo_status_t twin_scaled_font_unicode_to_glyph(cairo_scaled_font_t * scaled_font, unsigned long unicode, unsigned long * glyph)
{
...
cairo_font_face_t * _cairo_font_face_twin_create_fallback(void)
{
	...
	cairo_status_t status = twin_font_face_create_properties(twin_font_face, NULL);
	if(status) {
