...
static void face_props_parse(twin_face_properties_t * props, const char * s)
{
...
{
	twin_face_properties_t * props = (twin_face_properties_t *)SAlloc::M(sizeof(twin_face_properties_t));
	...
	if(unlikely(cairo_font_face_set_user_data(twin_face, &twin_properties_key, props, free))) {
		SAlloc::F(props);
		return NULL;
	}
	return props;
...
static cairo_status_t twin_font_face_set_properties_from_toy(cairo_font_face_t * twin_face, cairo_toy_font_face_t * toy_face)
{
	twin_face_properties_t * props = twin_font_face_create_properties(twin_face);
	if(unlikely(props == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	props->slant = toy_face->slant;
	props->weight = toy_face->weight == CAIRO_FONT_WEIGHT_NORMAL ? TWIN_WEIGHT_NORMAL : TWIN_WEIGHT_BOLD;
...
static void compute_hinting_scale(cairo_t * cr, double x, double y, double * scale, double * inv)
{
...
static void compute_hinting_scales(cairo_t * cr, double * x_scale, double * x_scale_inv, double * y_scale, double * y_scale_inv)
{
	compute_hinting_scale(cr, 1.0, 0.0, x_scale, x_scale_inv);
	compute_hinting_scale(cr, 0.0, 1.0, y_scale, y_scale_inv);
}
...
static cairo_status_t twin_scaled_font_compute_properties(cairo_scaled_font_t * scaled_font, cairo_t * cr)
{
	...
	twin_scaled_properties_t * props = (twin_scaled_properties_t *)SAlloc::M(sizeof(twin_scaled_properties_t));
	if(unlikely(props == NULL))
		return _cairo_error(CAIRO_STATUS_NO_MEMORY);
	...
	/* weight */
	props->weight = props->face_props->weight * (F__(4) / TWIN_WEIGHT_NORMAL);
	/* pen & margins */
	props->penx = props->peny = props->weight;
	props->marginl = props->marginr = F__(4);
	if(scaled_font->options.hint_style > CAIRO_HINT_STYLE_SLIGHT)
		twin_hint_pen_and_margins(cr, &props->penx, &props->peny, &props->marginl, &props->marginr);
	/* stretch */
...
static cairo_status_t twin_scaled_font_init(cairo_scaled_font_t  * scaled_font, cairo_t * cr, cairo_font_extents_t * metrics)
{
	metrics->ascent  = F__(54);
...
static void twin_compute_snap(cairo_t * cr, twin_snap_info_t * info, const signed char   * b)
{
	int s, n;
	...
	for(s = 0; s < n; s++) {
		...
		info->snapped_x[s] = SNAPXI(F__(snap[s]));
	}
	...
	for(s = 0; s < n; s++) {
		...
		info->snapped_y[s] = SNAPYI(F__(snap[s]));
	}
}
...
static cairo_status_t twin_scaled_font_render_glyph(cairo_scaled_font_t  * scaled_font,
    ulong glyph, cairo_t * cr, cairo_text_extents_t * metrics)
{
	...
	if(props->snap)
		twin_compute_snap(cr, &info, (const signed char *)b);
	else
...
static cairo_status_t twin_scaled_font_unicode_to_glyph(cairo_scaled_font_t * scaled_font, ulong unicode, ulong * glyph)
{
...
{
	cairo_font_face_t * twin_font_face = _cairo_font_face_twin_create_internal();
...
{
	cairo_font_face_t * twin_font_face = _cairo_font_face_twin_create_internal();
	cairo_status_t status = twin_font_face_set_properties_from_toy(twin_font_face, toy_face);
	if(status) {
		cairo_font_face_destroy(twin_font_face);
		return status;
	}
