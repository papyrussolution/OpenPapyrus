...
cairo_status_t _cairo_pdf_operators_fini(cairo_pdf_operators_t        * pdf_operators)
{
	return _cairo_pdf_operators_flush(pdf_operators);
}

void _cairo_pdf_operators_set_font_subsets_callback(cairo_pdf_operators_t    * pdf_operators,
 ...
 * this function.
 */
void _cairo_pdf_operators_set_stream(cairo_pdf_operators_t   * pdf_operators,
...
void _cairo_pdf_operators_set_cairo_to_pdf_matrix(cairo_pdf_operators_t * pdf_operators,
...
cairo_int_status_t _cairo_pdf_operators_clip(cairo_pdf_operators_t * pdf_operators, const cairo_path_fixed_t * path, CairoFillRule fill_rule)
{
...
static cairo_int_status_t _cairo_pdf_operators_emit_stroke(cairo_pdf_operators_t         * pdf_operators,
    ...
    const char        * pdf_operator)
{
	...
	status = _cairo_pdf_operators_emit_path(pdf_operators, path, &path_transform, style->line_cap);
	if(unlikely(status))
		return status;
...
cairo_int_status_t _cairo_pdf_operators_stroke(cairo_pdf_operators_t  * pdf_operators,
    const cairo_path_fixed_t           * path,
    const cairo_stroke_style_t         * style,
    const cairo_matrix_t   * ctm,
    const cairo_matrix_t   * ctm_inverse)
{
	return _cairo_pdf_operators_emit_stroke(pdf_operators, path, style, ctm, ctm_inverse, "S");
}

cairo_int_status_t _cairo_pdf_operators_fill(cairo_pdf_operators_t * pdf_operators, const cairo_path_fixed_t * path, CairoFillRule fill_rule)
{
	const char * pdf_operator;
	cairo_status_t status;
	if(pdf_operators->in_text_object) {
...
static void _cairo_pdf_operators_emit_glyph_index(cairo_pdf_operators_t * pdf_operators, cairo_output_stream_t * stream, uint glyph)
{
	if(pdf_operators->is_latin) {
		...
		_cairo_output_stream_printf(stream, "%0*x", pdf_operators->hex_width, glyph);
	}
}
...
static cairo_status_t _cairo_pdf_operators_emit_glyph_string(cairo_pdf_operators_t * pdf_operators, cairo_output_stream_t * stream)
{
	int i;
 ...
 */
static cairo_status_t _cairo_pdf_operators_emit_glyph_string_with_positioning(cairo_pdf_operators_t * pdf_operators, cairo_output_stream_t * stream)
{
	int i;
	_cairo_output_stream_printf(stream, "[%s", pdf_operators->is_latin ? "(" : "<");
	for(i = 0; i < pdf_operators->num_glyphs; i++) {
...
static cairo_status_t _cairo_pdf_operators_flush_glyphs(cairo_pdf_operators_t    * pdf_operators)
{
...
static cairo_status_t _cairo_pdf_operators_set_text_matrix(cairo_pdf_operators_t  * pdf_operators, cairo_matrix_t * matrix)
{
	...
	cairo_matrix_multiply(&pdf_operators->cairo_to_pdftext, &pdf_operators->cairo_to_pdf, &pdf_operators->cairo_to_pdftext);
	return _cairo_output_stream_get_status(pdf_operators->stream);
}
 ...
 */
static cairo_status_t _cairo_pdf_operators_set_text_position(cairo_pdf_operators_t  * pdf_operators,
    double x,
    double y)
{
 ...
 */
static cairo_status_t _cairo_pdf_operators_set_font_subset(cairo_pdf_operators_t * pdf_operators, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	cairo_status_t status;
	_cairo_output_stream_printf(pdf_operators->stream, "/f-%d-%d 1 Tf\n", subset_glyph->font_id, subset_glyph->subset_id);
	if(pdf_operators->use_font_subset) {
		status = pdf_operators->use_font_subset(subset_glyph->font_id, subset_glyph->subset_id, pdf_operators->use_font_subset_closure);
		if(unlikely(status))
			return status;
	}
...
static cairo_status_t _cairo_pdf_operators_begin_actualtext(cairo_pdf_operators_t * pdf_operators, const char * utf8, int utf8_len)
{
	...
	if(utf8_len) {
		...
		for(i = 0; i < utf16_len; i++) {
			_cairo_output_stream_printf(pdf_operators->stream, "%04x", (int)(utf16[i]));
		}
		SAlloc::F(utf16);
	}
...
static cairo_status_t _cairo_pdf_operators_emit_glyph(cairo_pdf_operators_t * pdf_operators,
    cairo_glyph_t * glyph, cairo_scaled_font_subsets_glyph_t * subset_glyph)
{
	...
	//y = glyph->y;
	_p = glyph->P;
	cairo_matrix_transform_rpoint(&pdf_operators->cairo_to_pdftext, _p);
	 ...
	 * adjustments in TJ.
	 */
	if(fabs(_p.x - pdf_operators->glyph_buf_x_pos) > 10 || fabs(_p.y - pdf_operators->cur_y) > GLYPH_POSITION_TOLERANCE) {
		status = _cairo_pdf_operators_flush_glyphs(pdf_operators);
		if(unlikely(status))
			return status;
		_p = glyph->P;
		cairo_matrix_transform_rpoint(&pdf_operators->cairo_to_pdf, _p);
		status = _cairo_pdf_operators_set_text_position(pdf_operators, _p.x, _p.y);
		if(unlikely(status))
			return status;
		_p.Set(0.0);
	}
	status = _cairo_pdf_operators_add_glyph(pdf_operators, subset_glyph, _p.x);
	return status;
}
 ...
 * empty string.
 */
static cairo_int_status_t _cairo_pdf_operators_emit_cluster(cairo_pdf_operators_t      * pdf_operators,
    const char     * utf8,
    int utf8_len,
...
cairo_int_status_t _cairo_pdf_operators_show_text_glyphs(cairo_pdf_operators_t      * pdf_operators,
    const char     * utf8,
    int utf8_len,
    cairo_glyph_t  * glyphs,
    int num_glyphs,
    ...
    cairo_scaled_font_t        * scaled_font)
{
	...
	if(pdf_operators->in_text_object == FALSE) {
		...
		if(unlikely(status))
			return status;
		// Force Tm and Tf to be emitted when starting a new text object.
	...
	if(pdf_operators->is_new_text_object || !_cairo_matrix_scale_equal(&pdf_operators->text_matrix, &text_matrix)) {
		status = _cairo_pdf_operators_flush_glyphs(pdf_operators);
		if(unlikely(status))
			return status;
		_p = glyphs[0].P;
	...
	if(num_clusters > 0) {
		...
		for(i = 0; i < num_glyphs; i++) {
			status = _cairo_pdf_operators_emit_cluster(pdf_operators, NULL,
			    -1, /* no unicode string available */
			    &glyphs[i], 1, CAIRO_TEXT_CLUSTER_FLAG_NONE, scaled_font);
			if(unlikely(status))
				return status;
		}
	}
