...
void _cairo_pdf_operators_init(cairo_pdf_operators_t * pdf_operators,
	cairo_output_stream_t * stream, cairo_matrix_t * cairo_to_pdf,
...
static int _count_word_up_to(const unsigned char * s, int length)
{
	int word = 0;
 ...
 */
static int _count_hexstring_up_to(const unsigned char * s, int length, int columns)
{
	int word = 0;
...
static cairo_output_stream_t * _word_wrap_stream_create(cairo_output_stream_t * output, int max_column)
{
...
cairo_int_status_t _cairo_pdf_operators_fill(cairo_pdf_operators_t * pdf_operators,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule)
{
	const char * pdf_operator;
...
cairo_int_status_t _cairo_pdf_operators_fill_stroke(cairo_pdf_operators_t * pdf_operators,
	cairo_path_fixed_t * path, cairo_fill_rule_t fill_rule, const cairo_stroke_style_t * style,
	...
	cairo_output_stream_t * stream)
{
	...
	for(i = 0; i < pdf_operators->num_glyphs; i++) {
		_cairo_output_stream_printf(stream, "%0*x", pdf_operators->hex_width, pdf_operators->glyphs[i].glyph_index);
 ...
 */
static cairo_status_t _cairo_pdf_operators_emit_glyph_string_with_positioning(
 ...
 */
static cairo_status_t _cairo_pdf_operators_set_font_subset(cairo_pdf_operators_t * pdf_operators,
...
static cairo_status_t _cairo_pdf_operators_begin_actualtext(cairo_pdf_operators_t * pdf_operators,
	const char * utf8,
	int utf8_len)
{
	uint16_t * utf16;
	...
	cairo_scaled_font_t * scaled_font)
{
	...
	if(num_clusters > 0) {
		...
		for(i = 0; i < num_clusters; i++) {
			...
			status = _cairo_pdf_operators_emit_cluster(pdf_operators, cur_text, 
				clusters[i].num_bytes, cur_glyph, clusters[i].num_glyphs, cluster_flags, scaled_font);
			if(unlikely(status))
				return status;
		...
		for(i = 0; i < num_glyphs; i++) {
			status = _cairo_pdf_operators_emit_cluster(pdf_operators, NULL,
				-1, /* no unicode string available */ &glyphs[i], 1, CAIRO_TEXT_CLUSTER_FLAG_UNDEF, scaled_font);
			if(unlikely(status))
				return status;
		}
	}
