...
static cairo_script_surface_t * _cairo_script_surface_create_internal(cairo_script_context_t * ctx,
	cairo_content_t content, double width, double height, cairo_surface_t * passthrough);
static void _cairo_script_surface_scaled_font_fini(cairo_scaled_font_t * scaled_font);
...
static cairo_status_t _emit_image_surface(cairo_script_surface_t * surface,
	cairo_image_surface_t * image)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_output_stream_t * base85_stream;
...
static cairo_status_t _emit_image_surface_pattern(cairo_script_surface_t * surface,
...
static cairo_status_t _emit_subsurface_pattern(cairo_script_surface_t * surface,
...
static cairo_status_t _emit_surface_pattern(cairo_script_surface_t * surface,
	const cairo_pattern_t * pattern)
{
...
static cairo_status_t _emit_pattern(cairo_script_surface_t * surface,
	const cairo_pattern_t * pattern)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_status_t status;
...
static cairo_status_t _emit_identity(cairo_script_surface_t * surface,
...
static cairo_status_t _emit_source(cairo_script_surface_t * surface,
	cairo_operator_t op,
...
static void _device_flush(void * abstract_device)
{
	...
	cairo_clip_t * clip)
{
	...
	for(n = 0; n < num_glyphs; n++) {
		if(font_private->has_sfnt) {
			...
			if((ulong)scaled_glyph->surface_private > 256)
				break;
		}
	}
	if(n == num_glyphs) {
	...
	for(n = 0; n < num_glyphs; n++) {
		...
		if(base85_stream != NULL) {
			...
			if(font_private->has_sfnt)
				...
				c = (uint8_t)(ulong)scaled_glyph->surface_private;
			...
			if(font_private->has_sfnt)
				...
				_cairo_output_stream_printf(ctx->stream, " %lu", (ulong)scaled_glyph->surface_private);
		}
...
static cairo_device_t * _cairo_script_context_create_internal(cairo_output_stream_t * stream)
{
	cairo_script_context_t * ctx;
...
void cairo_script_write_comment(cairo_device_t * device,
...
void cairo_script_set_mode(cairo_device_t * device,
	cairo_script_mode_t mode)
{
	cairo_script_context_t * context = (cairo_script_context_t *)device;
	context->mode = mode;
...
cairo_script_mode_t cairo_script_get_mode(cairo_device_t * device)
{
	cairo_script_context_t * context = (cairo_script_context_t *)device;
	return context->mode;
}

cairo_surface_t * cairo_script_surface_create(cairo_device_t * device,
	cairo_content_t content,
	double width,
	double height)
{
...
cairo_surface_t * cairo_script_surface_create_for_target(cairo_device_t * device,
...
cairo_status_t cairo_script_from_recording_surface(cairo_device_t * device,
