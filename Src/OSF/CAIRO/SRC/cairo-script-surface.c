...
// @sobolev (replaced with function) #define _cairo_output_stream_puts(S, STR) _cairo_output_stream_write((S), (STR), strlen(STR))
...
struct _cairo_script_implicit_context {
	...
	CairoFillRule current_fill_rule;
...
extern const cairo_surface_backend_t _cairo_script_surface_backend;
...
static void _bitmap_release_id(_cairo_script_context::_bitmap * b, ulong token)
{
	_cairo_script_context::_bitmap ** prev = NULL;
	do {
...
static cairo_status_t _bitmap_next_id(_cairo_script_context::_bitmap * b, ulong * id)
{
	_cairo_script_context::_bitmap * bb, ** prev = NULL;
	ulong min = 0;
	do {
		if(b->min != min)
			break;
...
static void _bitmap_fini(_cairo_script_context::_bitmap * b)
{
	while(b != NULL) {
...
static const char * _fill_rule_to_string(CairoFillRule rule)
{
	static const char * names[] = {
...
static inline cairo_script_context_t * to_context(cairo_script_surface_t * surface)
{
...
static void _get_target(cairo_script_surface_t * surface)
{
	...
	if(surface->defined) {
		_cairo_output_stream_printf(ctx->stream, "s%u ", surface->base.unique_id);
	}
	else {
		...
		if(ctx->active) {
			...
			if(depth == 1) {
				...
				_cairo_output_stream_printf(ctx->stream, "%d -1 roll ", depth);
			}
			target_push(surface);
			_cairo_output_stream_puts(ctx->stream, "dup ");
		}
...
static cairo_status_t _emit_surface(cairo_script_surface_t * surface)
{
	...
	if(surface->base.x_fallback_resolution != CAIRO_SURFACE_FALLBACK_RESOLUTION_DEFAULT || surface->base.y_fallback_resolution != CAIRO_SURFACE_FALLBACK_RESOLUTION_DEFAULT) {
...
{
	assert(target_is_active(surface));
	if((!force || fabs(line_width - CAIRO_GSTATE_LINE_WIDTH_DEFAULT) < 1e-5) && surface->cr.current_style.line_width == line_width) {
		return CAIRO_STATUS_SUCCESS;
	}
...
{
	assert(target_is_active(surface));
	if((!force || fabs(miter_limit - CAIRO_GSTATE_MITER_LIMIT_DEFAULT) < 1e-5) && surface->cr.current_style.miter_limit == miter_limit) {
		return CAIRO_STATUS_SUCCESS;
	}
...
static cairo_status_t _emit_mesh_pattern(cairo_script_surface_t * surface, const cairo_pattern_t * pattern)
{
	...
	if(unlikely(status))
		return status;
	_cairo_output_stream_puts(ctx->stream, "mesh");
	for(i = 0; i < n; i++) {
		cairo_path_t * path;
...
static cairo_status_t _emit_recording_surface_pattern(cairo_script_surface_t * surface, cairo_recording_surface_t * source)
{
	...
	if(_cairo_recording_surface_get_bounds(&source->base, &r))
		extents = &r;
	similar = _cairo_script_surface_create_internal(ctx, source->base.content, extents, 0);
...
static cairo_int_status_t _emit_png_surface(cairo_script_surface_t * surface, cairo_image_surface_t * image)
{
	...
	_cairo_output_stream_printf(ctx->stream, "<< /width %d /height %d /format //%s /mime-type (image/png) /source <~",
	    image->width, image->height, _format_to_string(image->format));
...
static cairo_int_status_t _emit_image_surface(cairo_script_surface_t * surface, cairo_image_surface_t * image)
{
	...
	const uint8 * mime_data;
	ulong mime_data_length;
	snapshot = _cairo_surface_has_snapshot(&image->base, &script_snapshot_backend);
	if(snapshot) {
	...
	if(_cairo_int_status_is_error(status)) {
		...
		switch(clone->format) {
			case CAIRO_FORMAT_A1: len = (clone->width + 7)/8; break;
			case CAIRO_FORMAT_A8: len = clone->width; break;
			case CAIRO_FORMAT_RGB16_565: len = clone->width * 2; break;
			case CAIRO_FORMAT_RGB24: len = clone->width * 3; break;
			case CAIRO_FORMAT_RGB30:
			case CAIRO_FORMAT_ARGB32: len = clone->width * 4; break;
			case CAIRO_FORMAT_INVALID:
			default:
			    ASSERT_NOT_REACHED;
			    len = 0;
...
static cairo_int_status_t _emit_surface_pattern(cairo_script_surface_t * surface, const cairo_pattern_t * pattern)
{
	...
	if(_cairo_surface_is_snapshot(source)) {
		...
		if(snapshot) {
			_cairo_output_stream_printf(ctx->stream, "s%d pattern ", snapshot->unique_id);
			return CAIRO_INT_STATUS_SUCCESS;
		}
...
static cairo_int_status_t _emit_raster_pattern(cairo_script_surface_t * surface,
    const cairo_pattern_t * pattern)
{
	cairo_surface_t * source;
	cairo_int_status_t status;

	source = _cairo_raster_source_pattern_acquire(pattern, &surface->base, 0);
...
static cairo_int_status_t _emit_pattern(cairo_script_surface_t * surface,
    const cairo_pattern_t * pattern)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_int_status_t status;
...
{
	_cairo_output_stream_printf((cairo_output_stream_t *)closure, " %f %f %f %f %f %f c",
	    _cairo_fixed_to_double(p1->x), _cairo_fixed_to_double(p1->y), _cairo_fixed_to_double(p2->x), _cairo_fixed_to_double(p2->y),
	    _cairo_fixed_to_double(p3->x), _cairo_fixed_to_double(p3->y));
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _emit_path_boxes(cairo_script_surface_t * surface, const cairo_path_fixed_t * path)
{
	...
	if(!_cairo_path_fixed_iter_at_end(&iter)) {
		_cairo_boxes_fini(&boxes);
		// @v1.14.6 return (cairo_status_t)FALSE;
		return CAIRO_STATUS_INVALID_PATH_DATA; // @v1.14.6
	}
	for(chunk = &boxes.chunks; chunk; chunk = chunk->next) {
		for(i = 0; i < chunk->count; i++) {
			...
			_cairo_output_stream_printf(ctx->stream, "\n  %f %f %f %f rectangle", x1, y1, x2 - x1, y2 - y1);
		}
	}
	_cairo_boxes_fini(&boxes);
	return status;
...
{
	cairo_script_context_t * ctx = (cairo_script_context_t *)abstract_device;
		...
		if(!cairo_list_is_empty(&surface->operand.link)) {
			if(!ctx->active) {
				...
				if(link == NULL) {
					...
					link->operand.type = _operand::DEFERRED;
...
static void inactive(cairo_script_surface_t * surface)
{
	...
	while(!cairo_list_is_empty(&ctx->deferred)) {
		...
		int depth = 0;
		struct deferred_finish * df = cairo_list_first_entry(&ctx->deferred, struct deferred_finish, link);
		cairo_list_foreach(operand, &ctx->operands) {
			if(operand == &df->operand.link)
				break;
			depth++;
		}
		df->operand.type = (_operand::Typ)depth;
		if(cairo_list_is_empty(&sorted)) {
			cairo_list_move(&df->link, &sorted);
		}
		else {
			struct deferred_finish * pos;
			cairo_list_foreach_entry(pos, struct deferred_finish, &sorted, link)
			{
	...
	while(!cairo_list_is_empty(&sorted)) {
		...
		cairo_list_t * operand;
		int depth;
		df = cairo_list_first_entry(&sorted, struct deferred_finish, link);
		depth = 0;
		cairo_list_foreach(operand, &ctx->operands) {
			if(operand == &df->operand.link)
				break;
...
static cairo_int_status_t _cairo_script_surface_stroke(void * abstract_surface,
    cairo_operator_t op,
...
{
	cairo_script_context_t * ctx = to_context(surface);
	if(cairo_font_options_equal(&surface->cr.current_font_options, font_options)) {
		return CAIRO_STATUS_SUCCESS;
	}
	_cairo_output_stream_puts(ctx->stream, "<<");
	if(font_options->antialias != surface->cr.current_font_options.antialias) {
		_cairo_output_stream_printf(ctx->stream, " /antialias //%s", _antialias_to_string(font_options->antialias));
	}
	if(font_options->subpixel_order != surface->cr.current_font_options.subpixel_order) {
		_cairo_output_stream_printf(ctx->stream, " /subpixel-order //%s", _subpixel_order_to_string(font_options->subpixel_order));
	}
	if(font_options->hint_style != surface->cr.current_font_options.hint_style) {
		_cairo_output_stream_printf(ctx->stream, " /hint-style //%s", _hint_style_to_string(font_options->hint_style));
	}
	if(font_options->hint_metrics != surface->cr.current_font_options.hint_metrics) {
		_cairo_output_stream_printf(ctx->stream, " /hint-metrics //%s", _hint_metrics_to_string(font_options->hint_metrics));
	}
	_cairo_output_stream_puts(ctx->stream, " >> set-font-options\n");
...
static void _cairo_script_scaled_font_fini(cairo_scaled_font_private_t * abstract_private, cairo_scaled_font_t * scaled_font)
{
	...
	cairo_status_t status = cairo_device_acquire(&ctx->base);
	if(likely(status == CAIRO_STATUS_SUCCESS)) {
		_cairo_output_stream_printf(ctx->stream, "/f%lu undef /sf%lu undef\n", priv->id, priv->id);
...
static cairo_status_t _emit_type42_font(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font)
{
	...
	load_flags = 0;
#endif
	_cairo_output_stream_printf(ctx->stream, "<< /type 42 /index 0 /flags %d /source <|", load_flags);
	base85_stream = _cairo_base85_stream_create(ctx->stream);
	len = to_be32(size);
	_cairo_output_stream_write(base85_stream, &len, sizeof(len));
	zlib_stream = _cairo_deflate_stream_create(base85_stream);
...
static cairo_status_t _emit_scaled_font_init(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font, cairo_script_font_t ** font_out)
{
	...
	_cairo_output_stream_printf(ctx->stream, "dict\n  /type 3 set\n  /metrics [%f %f %f %f %f] set\n  /glyphs array set\n  font dup /f%lu exch def set-font-face",
	    scaled_font->fs_extents.ascent, scaled_font->fs_extents.descent, scaled_font->fs_extents.height, scaled_font->fs_extents.max_x_advance,
		scaled_font->fs_extents.max_y_advance, font_private->id);
	*font_out = font_private;
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _emit_scaled_font(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font)
{
	...
	if(font_private == NULL) {
		...
		if(unlikely(status))
			return status;
		assert(target_is_active(surface));
		_cairo_output_stream_printf(ctx->stream, " /scaled-font get /sf%lu exch def\n", font_private->id);
	}
	else {
		_cairo_output_stream_printf(ctx->stream, "sf%lu set-scaled-font\n", font_private->id);
	}
	return CAIRO_STATUS_SUCCESS;
}

static cairo_status_t _emit_scaled_glyph_vector(cairo_script_surface_t * surface,
    cairo_scaled_font_t * scaled_font, cairo_script_font_t * font_private, cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_script_implicit_context_t old_cr;
	cairo_status_t status;
	...
	_cairo_output_stream_printf(ctx->stream, "%lu <<\n  /metrics [%f %f %f %f %f %f]\n  /render {\n",
	    index, scaled_glyph->fs_metrics.x_bearing, scaled_glyph->fs_metrics.y_bearing, scaled_glyph->fs_metrics.width,
	    scaled_glyph->fs_metrics.height, scaled_glyph->fs_metrics.x_advance, scaled_glyph->fs_metrics.y_advance);

	if(!_cairo_matrix_is_identity(&scaled_font->scale_inverse)) {
		_cairo_output_stream_printf(ctx->stream, "[%f %f %f %f %f %f] transform\n",
		    scaled_font->scale_inverse.xx, scaled_font->scale_inverse.yx, scaled_font->scale_inverse.xy,
		    scaled_font->scale_inverse.yy, scaled_font->scale_inverse.x0, scaled_font->scale_inverse.y0);
	}
	old_cr = surface->cr;
	_cairo_script_implicit_context_init(&surface->cr);
...
static cairo_status_t _emit_scaled_glyph_bitmap(cairo_script_surface_t * surface,
    cairo_scaled_font_t * scaled_font, cairo_script_font_t * font_private, cairo_scaled_glyph_t * scaled_glyph)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_status_t status;
	...
	_cairo_output_stream_printf(ctx->stream, "%lu <<\n  /metrics [%f %f %f %f %f %f]\n  /render {\n%f %f translate\n",
	    index, scaled_glyph->fs_metrics.x_bearing, scaled_glyph->fs_metrics.y_bearing, scaled_glyph->fs_metrics.width,
	    scaled_glyph->fs_metrics.height, scaled_glyph->fs_metrics.x_advance, scaled_glyph->fs_metrics.y_advance,
	    scaled_glyph->fs_metrics.x_bearing, scaled_glyph->fs_metrics.y_bearing);
	...
	if(!_cairo_matrix_is_identity(&scaled_font->font_matrix)) {
		_cairo_output_stream_printf(ctx->stream, "\n  [%f %f %f %f %f %f] set-matrix\n",
		    scaled_font->font_matrix.xx, scaled_font->font_matrix.yx, scaled_font->font_matrix.xy,
		    scaled_font->font_matrix.yy, scaled_font->font_matrix.x0, scaled_font->font_matrix.y0);
	}
	_cairo_output_stream_puts(ctx->stream, "mask\n} >> set\n");
	return CAIRO_STATUS_SUCCESS;
}
...
static cairo_status_t _emit_scaled_glyphs(cairo_script_surface_t * surface, cairo_scaled_font_t * scaled_font, cairo_glyph_t * glyphs, uint num_glyphs)
{
	cairo_script_context_t * ctx = to_context(surface);
	cairo_script_font_t * font_private;
	cairo_status_t status;
	...
	for(n = 0; n < num_glyphs; n++) {
		...
		if(scaled_glyph->dev_private_key == ctx)
			continue;
		status = _cairo_scaled_glyph_lookup(scaled_font, glyphs[n].index, CAIRO_SCALED_GLYPH_INFO_RECORDING_SURFACE, &scaled_glyph);
		...
		if(status == CAIRO_STATUS_SUCCESS) {
			...
			status = _emit_scaled_glyph_vector(surface, scaled_font, font_private, scaled_glyph);
			if(unlikely(status))
				break;
			continue;
		}
		status = _cairo_scaled_glyph_lookup(scaled_font, glyphs[n].index, CAIRO_SCALED_GLYPH_INFO_SURFACE, &scaled_glyph);
		if(_cairo_status_is_error(status))
			break;
		if(status == CAIRO_STATUS_SUCCESS) {
			if(!have_glyph_prologue) {
			...
			status = _emit_scaled_glyph_bitmap(surface, scaled_font, font_private, scaled_glyph);
			if(unlikely(status))
				break;
			continue;
		}
    ...
    const cairo_clip_t          * clip)
{
	...
	if(utf8 != NULL && clusters != NULL) {
		for(n = 0; n < num_clusters; n++) {
			if(clusters[n].num_bytes > UCHAR_MAX || clusters[n].num_glyphs > UCHAR_MAX) {
				break;
			}
		}
		if(n < num_clusters) {
		...
		_cairo_output_stream_printf(ctx->stream, " //%s show-text-glyphs\n", _direction_to_string(backward));
	}
	else {
		_cairo_output_stream_puts(ctx->stream, "] show-glyphs\n");
	}
	inactive(surface);
	if(_cairo_surface_wrapper_is_active(&surface->wrapper)) {
		return _cairo_surface_wrapper_show_text_glyphs(&surface->wrapper,
		    op, source, utf8, utf8_len, glyphs, num_glyphs, clusters, num_clusters, backward, scaled_font, clip);
	}
...
static cairo_bool_t _cairo_script_surface_get_extents(void * abstract_surface, CairoIRect * rectangle)
{
	cairo_script_surface_t * surface = (cairo_script_surface_t *)abstract_surface;
	if(_cairo_surface_wrapper_is_active(&surface->wrapper)) {
...
static cairo_script_surface_t * _cairo_script_surface_create_internal(cairo_script_context_t * ctx,
    cairo_content_t content, cairo_rectangle_t * extents, cairo_surface_t * passthrough)
{
	...
	_cairo_surface_init(&surface->base, &_cairo_script_surface_backend, &ctx->base, content);
	_cairo_surface_wrapper_init(&surface->wrapper, passthrough);
	_cairo_surface_clipper_init(&surface->clipper, _cairo_script_surface_clipper_intersect_clip_path);
	surface->width = surface->height = -1;
	if(extents) {
		surface->width = extents->width;
		surface->height = extents->height;
		cairo_surface_set_device_offset(&surface->base, -extents->x, -extents->y);
	}
	surface->emitted = FALSE;
	surface->defined = FALSE;
	surface->active = FALSE;
	surface->operand.type = _operand::SURFACE;
...
{
	cairo_script_context_t * ctx = (cairo_script_context_t*)device;
...
cairo_surface_t * cairo_script_surface_create(cairo_device_t * script, cairo_content_t content, double width, double height)
{
	...
	return &_cairo_script_surface_create_internal((cairo_script_context_t*)script, content, extents, NULL)->base;
}
