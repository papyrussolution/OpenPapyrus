...
static inline int _cairo_bo_point32_compare(cairo_bo_intersect_point_t const * a, cairo_bo_intersect_point_t const * b)
{
	int cmp = a->y.ordinate - b->y.ordinate;
	if(cmp)
		return cmp;
	...
	switch(have_dx_adx_bdx) {
		...
		case HAVE_DX_ADX:
		    ...
		    if((-adx ^ dx) < 0) {
			    ...
			    cairo_int64_t dy_adx = _cairo_int32x32_64_mul(a->edge.line.p1.y - y, adx);
		...
		case HAVE_DX_BDX:
		    ...
		    if((bdx ^ dx) < 0) {
			    ...
			    cairo_int64_t dy_bdx = _cairo_int32x32_64_mul(y - b->edge.line.p1.y, bdx);
...
static inline int _line_equal(const cairo_line_t * a, const cairo_line_t * b)
{
...
static inline cairo_bo_intersect_ordinate_t round_to_nearest(cairo_quorem64_t d, cairo_int64_t den)
{
	cairo_bo_intersect_ordinate_t ordinate;
	int32_t quo = (int32_t)d.quo;
	...
	ordinate.approx = _cairo_int64_is_zero(drem_2) ? 
		_cairo_bo_intersect_ordinate::EXACT : _cairo_int64_negative(drem_2) ? 
		_cairo_bo_intersect_ordinate::EXCESS : _cairo_bo_intersect_ordinate::DEFAULT;
	return ordinate;
}
...
static inline cairo_status_t _pqueue_push(pqueue_t * pq, cairo_bo_event_t * event)
{
	cairo_bo_event_t ** elements;
	int i, parent;
	...
	for(i = ++pq->size; i != PQ_FIRST_ENTRY && cairo_bo_event_compare(event, elements[parent = PQ_PARENT_INDEX(i)]) < 0; i = parent) {
...
static inline cairo_status_t _cairo_bo_event_queue_insert(cairo_bo_event_queue_t    * queue,
    cairo_bo_event_type_t type, cairo_bo_edge_t * e1, cairo_bo_edge_t * e2,
...
static void _cairo_bo_event_queue_init(cairo_bo_event_queue_t * event_queue,
    cairo_bo_event_t ** start_events, int num_events)
{
...
static cairo_status_t event_queue_insert_stop(cairo_bo_event_queue_t * event_queue, cairo_bo_edge_t * edge)
{
	...
	point.y.approx   = _cairo_bo_intersect_ordinate::EXACT;
	point.x.ordinate = _line_compute_intersection_x_for_y(&edge->edge.line, point.y.ordinate);
	point.x.approx   = _cairo_bo_intersect_ordinate::EXACT;
...
static inline cairo_bool_t edges_colinear(const cairo_bo_edge_t * a, const cairo_bo_edge_t * b)
{
	...
	if(a->edge.line.p1.y == b->edge.line.p1.y) {
		...
		return edge_compare_for_y_against_x(b, a->edge.line.p1.y, a->edge.line.p1.x) == 0;
	}
	else {
		return edge_compare_for_y_against_x(a, b->edge.line.p1.y, b->edge.line.p1.x) == 0;
	}
}
...
cairo_status_t _cairo_polygon_intersect(cairo_polygon_t * a, int winding_a, cairo_polygon_t * b, int winding_b)
{
	...
	if(winding_a != CAIRO_FILL_RULE_WINDING) {
		status = _cairo_polygon_reduce(a, (CairoFillRule)winding_a);
		if(unlikely(status))
			return status;
	}
	if(winding_b != CAIRO_FILL_RULE_WINDING) {
		status = _cairo_polygon_reduce(b, (CairoFillRule)winding_b);
		if(unlikely(status))
			return status;
	}
	...
	for(i = 0; i < a->num_edges; i++) {
		...
		events[j].point.y.approx = _cairo_bo_intersect_ordinate::EXACT;
		events[j].point.x.ordinate = _line_compute_intersection_x_for_y(&a->edges[i].line, events[j].point.y.ordinate);
		events[j].point.x.approx = _cairo_bo_intersect_ordinate::EXACT;
	...
	for(i = 0; i < b->num_edges; i++) {
		...
		events[j].point.y.approx = _cairo_bo_intersect_ordinate::EXACT;
		events[j].point.x.ordinate = _line_compute_intersection_x_for_y(&b->edges[i].line, events[j].point.y.ordinate);
		events[j].point.x.approx = _cairo_bo_intersect_ordinate::EXACT;
...
cairo_status_t _cairo_polygon_intersect_with_boxes(cairo_polygon_t * polygon, CairoFillRule * winding, cairo_box_t * boxes, int num_boxes)
{
	cairo_polygon_t b;
	cairo_status_t status;
	int n;
	...
	for(n = 0; n < num_boxes; n++) {
		if(polygon->extents.p1.x >= boxes[n].p1.x && polygon->extents.p2.x <= boxes[n].p2.x &&
		    polygon->extents.p1.y >= boxes[n].p1.y && polygon->extents.p2.y <= boxes[n].p2.y) {
			return CAIRO_STATUS_SUCCESS;
		}
	}
	_cairo_polygon_init(&b, NULL, 0);
	for(n = 0; n < num_boxes; n++) {
		if(boxes[n].p2.x > polygon->extents.p1.x && boxes[n].p1.x < polygon->extents.p2.x && 
			boxes[n].p2.y > polygon->extents.p1.y && boxes[n].p1.y < polygon->extents.p2.y) {
			cairo_point_t p1, p2;
			p1.y = boxes[n].p1.y;
			p2.y = boxes[n].p2.y;
	...
	status = _cairo_polygon_intersect(polygon, *winding, &b, CAIRO_FILL_RULE_WINDING);
