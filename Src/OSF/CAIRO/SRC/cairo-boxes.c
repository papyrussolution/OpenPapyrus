...
static void _cairo_boxes_add_internal(cairo_boxes_t * boxes, const cairo_box_t * box)
{
	struct _cairo_boxes_chunk * chunk;
	if(unlikely(boxes->status))
		return;
	chunk = (struct _cairo_boxes_chunk *)boxes->tail;
	if(unlikely(chunk->count == chunk->size)) {
		int size = chunk->size*2;
		chunk->next = (struct _cairo_boxes_chunk *)_cairo_malloc_ab_plus_c(size, sizeof(cairo_box_t), sizeof(struct _cairo_boxes_chunk));
	...
	if(boxes->is_pixel_aligned) {
		boxes->is_pixel_aligned = _cairo_fixed_is_integer(box->p1.x) && _cairo_fixed_is_integer(box->p1.y) &&
		        _cairo_fixed_is_integer(box->p2.x) && _cairo_fixed_is_integer(box->p2.y);
	}
}

cairo_status_t _cairo_boxes_add(cairo_boxes_t * boxes, const cairo_box_t * box)
{
...
void _cairo_boxes_extents(const cairo_boxes_t * boxes, cairo_rectangle_int_t * extents)
{
