...
static cairo_status_t closure_write(cairo_output_stream_t * stream,
...
cairo_output_stream_t * _cairo_output_stream_create(cairo_write_func_t write_func,
	cairo_close_func_t close_func, void * closure)
{
	...
	_cairo_output_stream_init(&stream->base, closure_write, NULL, closure_close);
	stream->write_func = write_func;
	stream->close_func = close_func;
...
void _cairo_output_stream_write(cairo_output_stream_t * stream, const void * data, size_t length)
{
	...
	if(stream->status)
		return;
	stream->status = stream->write_func(stream, (const unsigned char *)data, length);
...
{
	stdio_stream_t * stream = (stdio_stream_t *)base;
	cairo_status_t status = stdio_flush(base);
	fclose(stream->file);
	return status;
}
...
cairo_output_stream_t * _cairo_output_stream_create_for_file(FILE * file)
{
	...
	_cairo_output_stream_init(&stream->base, stdio_write, stdio_flush, stdio_flush);
	stream->file = file;
	return &stream->base;
}
...
cairo_output_stream_t * _cairo_output_stream_create_for_filename(const char * filename)
{
	...
	_cairo_output_stream_init(&stream->base, stdio_write, stdio_flush, stdio_close);
	stream->file = file;
	return &stream->base;
}
...
static cairo_status_t memory_write(cairo_output_stream_t * base,
	const unsigned char * data, unsigned int length)
{
...
static cairo_status_t memory_close(cairo_output_stream_t * base)
{
	memory_stream_t * stream = (memory_stream_t *)base;
...
cairo_status_t _cairo_memory_stream_destroy(cairo_output_stream_t * abstract_stream,
	unsigned char ** data_out, unsigned long * length_out)
{
	...
	*data_out = (unsigned char *)malloc(*length_out);
...
static cairo_status_t null_write(cairo_output_stream_t * base,
	const unsigned char * data, unsigned int length)
{
	return CAIRO_STATUS_SUCCESS;
}
