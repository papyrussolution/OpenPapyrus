...
DONE_ALLOCATE:
	...
	for(i = 0, j = 0; i < 256; i++) {
		if(j < CUBE_SIZE-1 && (((i<<8)+i)-(int)cube_index_to_short[j]) >
		   ((int)cube_index_to_short[j+1]-((i<<8)+i)))
			j++;
	...
	for(i = 0, j = 0; i < 256; i++) {
		if(j < RAMP_SIZE-1 && (((i<<8)+i)-(int)ramp_index_to_short[j]) >
		   ((int)ramp_index_to_short[j+1]-((i<<8)+i)))
			j++;
