    ...
    cairo_surface_t * surface, cairo_operator_t op, const cairo_pattern_t * source, const cairo_clip_t * clip)
{
	...
	if(status == CAIRO_INT_STATUS_SUCCESS && surface->damage) {
		TRACE((stderr, "%s: applying damage (%d,%d)x(%d, %d)\n", __FUNCTION__, extents.unbounded.x, extents.unbounded.y, extents.unbounded.width, extents.unbounded.height));
...
cairo_int_status_t _cairo_compositor_mask(const cairo_compositor_t * compositor, cairo_surface_t * surface, cairo_operator_t op,
...
cairo_int_status_t _cairo_compositor_fill(const cairo_compositor_t * compositor,
    ...
    const cairo_path_fixed_t * path, CairoFillRule fill_rule, double tolerance,
    ...
    int num_glyphs, cairo_scaled_font_t * scaled_font, const cairo_clip_t * clip)
{
	...
	cairo_int_status_t status;
	TRACE((stderr, "%s\n", __FUNCTION__));
	status = _cairo_composite_rectangles_init_for_glyphs(&extents, surface, op, source, scaled_font, glyphs, num_glyphs, clip, &overlap);
	if(unlikely(status))
		return status;
	do {
