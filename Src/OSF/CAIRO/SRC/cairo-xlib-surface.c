...
#if DEBUG
 #define UNSUPPORTED(reason) fprintf(stderr, "cairo-xlib: hit unsupported operation %s(), line %d: %s\n", \
...
static void CAIRO_PRINTF_FORMAT(2, 3) _x_bread_crumb(Display*dpy, const char * fmt, ...)
{
	xReq * req;
	char buf[2048];
...
static cairo_surface_t * _cairo_xlib_surface_create_internal(cairo_xlib_screen_t * screen, Drawable drawable,
	Visual * visual, XRenderPictFormat * xrender_format, int width, int height,
	int depth);

static cairo_bool_t _cairo_surface_is_xlib(cairo_surface_t * surface);

static cairo_bool_t _native_byte_order_lsb(void);

static cairo_int_status_t _cairo_xlib_surface_show_glyphs(void * abstract_dst, cairo_operator_t op,
	const cairo_pattern_t * src_pattern, cairo_glyph_t * glyphs, int num_glyphs, cairo_scaled_font_t * scaled_font,
	cairo_clip_t * clip,
...
static Visual * _visual_for_xrender_format(Screen * screen,
...
static cairo_status_t _cairo_xlib_surface_set_clip_region(cairo_xlib_surface_t * surface,
...
static cairo_content_t _xrender_format_to_content(XRenderPictFormat * xrender_format)
{
...
static cairo_surface_t * _cairo_xlib_surface_create_similar(void * abstract_src,
...
static cairo_status_t _cairo_xlib_surface_finish(void * abstract_surface)
{
...
static cairo_status_t _cairo_xlib_surface_get_gc(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
	GC * gc)
{
...
static void _cairo_xlib_surface_put_gc(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
	GC gc)
{
...
static void _swap_ximage_3bytes(XImage * ximage)
{
	int i, j;
	char * line = ximage->data;
...
static void _swap_ximage_4bytes(XImage * ximage)
{
	int i, j;
	char * line = ximage->data;
...
static void _swap_ximage_nibbles(XImage * ximage)
{
	int i, j;
	char * line = ximage->data;
...
static void _swap_ximage_bits(XImage * ximage)
{
	int i, j;
	char * line = ximage->data;
...
static void _swap_ximage_to_native(XImage * ximage)
{
	int unit_bytes = 0;
	...
	int8_t dither_adjustment)
{
	if(r == g && g == b) {
		...
		return visual_info->cube_to_pseudocolor[visual_info->field8_to_cube[_adjust_field(r,
											    dither_adjustment)]]
...
static cairo_status_t _get_image_surface(cairo_xlib_surface_t * surface,
			...
			for(x = 0, x_off = x0%ARRAY_LENGTH(dither_pattern[0]);
				...
				if(visual_info == NULL) {
					out_pixel = (
					        ...
					        _field_to_8_undither(in_pixel&r_mask, r_width, r_shift,
							dither_adjustment)<<16|
					        _field_to_8_undither(in_pixel&g_mask, g_width, g_shift,
							dither_adjustment)<<8|
					        _field_to_8_undither(in_pixel&b_mask, b_width, b_shift,
							dither_adjustment));
				}
				else {
...
static void _cairo_xlib_surface_ensure_src_picture(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface)
{
...
static void _cairo_xlib_surface_set_picture_clip_rects(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface)
{
...
static void _cairo_xlib_surface_set_precision(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
...
static void _cairo_xlib_surface_ensure_dst_picture(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface)
{
...
static cairo_status_t _draw_image_surface(cairo_xlib_surface_t * surface,
		...
		for(y = 0, y_off = y0%ARRAY_LENGTH(dither_pattern);
			...
			for(x = 0, x_off = x0%ARRAY_LENGTH(dither_pattern[0]);
				...
				if(true_color) {
					...
					out_pixel = _pseudocolor_from_rgb888_dither(visual_info, r, g, b,
						dither_adjustment);
				}
...
static cairo_surface_t * _cairo_xlib_surface_snapshot(void * abstract_surface)
{
	cairo_xlib_surface_t * surface = abstract_surface;
	cairo_image_surface_t * image;
	cairo_status_t status;
...
static void _cairo_xlib_surface_release_source_image(void * abstract_surface,
	cairo_image_surface_t * image,
	void * image_extra)
{
	cairo_surface_destroy(&image->base);
...
static cairo_status_t _cairo_xlib_surface_acquire_dest_image(void * abstract_surface,
	cairo_rectangle_int_t * interest_rect,
	cairo_image_surface_t ** image_out,
...
static void _cairo_xlib_surface_release_dest_image(void * abstract_surface,
	cairo_rectangle_int_t * interest_rect,
	cairo_image_surface_t * image,
	cairo_rectangle_int_t * image_rect,
	void * image_extra)
 ...
 */
static inline cairo_bool_t _cairo_xlib_surface_same_screen(cairo_xlib_surface_t * dst,
	cairo_xlib_surface_t * src)
{
	return dst->screen == src->screen;
}

static cairo_status_t _cairo_xlib_surface_clone_similar(void * abstract_surface,
	cairo_surface_t * src,
	int src_x,
	int src_y,
	int width,
...
static cairo_surface_t * _cairo_xlib_surface_create_solid_pattern_surface(void * abstract_surface,
...
static cairo_bool_t _cairo_xlib_surface_can_repaint_solid_pattern_surface(void * abstract_surface,
	const cairo_solid_pattern_t * solid_pattern)
{
	cairo_xlib_surface_t * other = abstract_surface;
...
static cairo_status_t _cairo_xlib_surface_set_matrix(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
...
static cairo_status_t _cairo_xlib_surface_set_filter(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
...
static cairo_status_t _cairo_xlib_surface_set_repeat(cairo_xlib_surface_t * surface,
...
static cairo_status_t _cairo_xlib_surface_set_component_alpha(cairo_xlib_surface_t * surface,
	cairo_bool_t ca,
	unsigned long * mask,
	XRenderPictureAttributes * pa)
{
...
static cairo_int_status_t _cairo_xlib_surface_set_attributes(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * surface,
 ...
 * a tile in a GC.
 */
static cairo_bool_t _surfaces_compatible(cairo_xlib_surface_t * dst,
	cairo_xlib_surface_t * src)
{
	/* same screen */
 ...
 */
static cairo_bool_t _operator_needs_alpha_composite(cairo_operator_t op,
 ...
 */
static composite_operation_t _categorize_composite_operation(cairo_xlib_surface_t * dst,
	cairo_operator_t op,
 ...
 * Render.
 */
static composite_operation_t _recategorize_composite_operation(cairo_xlib_surface_t * dst,
	cairo_operator_t op,
	cairo_xlib_surface_t * src,
...
static cairo_int_status_t _cairo_xlib_surface_acquire_pattern_surface(cairo_xlib_display_t * display,
...
static cairo_int_status_t _cairo_xlib_surface_acquire_pattern_surfaces(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * dst,
...
static cairo_int_status_t _cairo_xlib_surface_upload(cairo_xlib_surface_t * surface,
	cairo_operator_t op,
	const cairo_pattern_t * pattern,
	int src_x, int src_y,
	int dst_x, int dst_y,
...
static cairo_int_status_t _cairo_xlib_surface_composite(cairo_operator_t op,
	const cairo_pattern_t * src_pattern,
...
static cairo_int_status_t _cairo_xlib_surface_solid_fill_rectangles(cairo_xlib_surface_t * surface,
...
static cairo_int_status_t _cairo_xlib_surface_fill_rectangles(void * abstract_surface,
	cairo_operator_t op,
	const cairo_color_t * color,
	cairo_rectangle_int_t * rects,
	int num_rects)
...
static cairo_int_status_t _cairo_xlib_surface_composite_trapezoids(cairo_operator_t op,
	const cairo_pattern_t * pattern,
	void * abstract_dst,
	cairo_antialias_t antialias,
	int src_x,
...
static void _cairo_xlib_surface_get_font_options(void * abstract_surface,
...
static void _cairo_xlib_surface_scaled_font_fini(cairo_scaled_font_t * scaled_font);

static void _cairo_xlib_surface_scaled_glyph_fini(cairo_scaled_glyph_t * scaled_glyph,
	cairo_scaled_font_t * scaled_font);

static cairo_bool_t _cairo_xlib_surface_is_similar(void * surface_a,
	void * surface_b)
{
...
static void _cairo_xlib_surface_detach_display(cairo_xlib_display_t * display, void * data)
{
...
static cairo_surface_t * _cairo_xlib_surface_create_internal(cairo_xlib_screen_t * screen,
	Drawable drawable,
	Visual * visual,
	XRenderPictFormat * xrender_format,
	int width,
...
/* callback from CloseDisplay */
static void _cairo_xlib_surface_remove_scaled_font(cairo_xlib_display_t * display,
	void * data)
{
...
static cairo_status_t _cairo_xlib_surface_font_init(cairo_xlib_display_t * display,
	cairo_scaled_font_t * scaled_font)
{
	cairo_xlib_surface_font_private_t * font_private;
	int i;
...
static void _cairo_xlib_render_free_glyphs(Display * dpy,
...
static cairo_xlib_font_glyphset_info_t * _cairo_xlib_scaled_glyph_get_glyphset_info(cairo_scaled_glyph_t * scaled_glyph)
{
	return scaled_glyph->surface_private;
}

static void _cairo_xlib_scaled_glyph_set_glyphset_info(cairo_scaled_glyph_t * scaled_glyph,
	cairo_xlib_font_glyphset_info_t * glyphset_info)
{
...
static cairo_xlib_font_glyphset_info_t * _cairo_xlib_scaled_font_get_glyphset_info_for_format(
	cairo_scaled_font_t * scaled_font,
	cairo_format_t format)
{
	cairo_xlib_surface_font_private_t * font_private;
...
static cairo_bool_t _cairo_xlib_glyphset_info_has_pending_free_glyph(
	cairo_xlib_font_glyphset_info_t * glyphset_info,
...
static cairo_xlib_font_glyphset_info_t * _cairo_xlib_scaled_font_get_glyphset_info_for_pending_free_glyph(
	cairo_scaled_font_t * scaled_font,
	unsigned long glyph_index,
	...
	cairo_scaled_glyph_t ** pscaled_glyph)
{
	...
	glyphset_info = _cairo_xlib_scaled_font_get_glyphset_info_for_pending_free_glyph(scaled_font, glyph_index,
		glyph_surface);
	...
	{
		int len =
		        cairo_format_stride_for_width(glyphset_info->format,
				glyph_surface->width)*glyph_surface->height;
		int max_request_size =
		        (XExtendedMaxRequestSize(display->display) ? XExtendedMaxRequestSize(display->display)
...
static cairo_status_t _cairo_xlib_surface_emit_glyphs(cairo_xlib_display_t * display,
	cairo_xlib_surface_t * dst,
	cairo_xlib_glyph_t * glyphs,
	int num_glyphs,
	cairo_scaled_font_t * scaled_font,
...
static cairo_bool_t _cairo_xlib_surface_owns_font(cairo_xlib_surface_t * dst,
	cairo_scaled_font_t * scaled_font)
{
	cairo_xlib_surface_font_private_t * font_private;
...
static cairo_int_status_t _cairo_xlib_surface_show_glyphs(void * abstract_dst,
	cairo_operator_t op,
	const cairo_pattern_t * src_pattern,
