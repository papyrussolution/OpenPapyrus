...
static cairo_skia_surface_t * _cairo_skia_surface_create_internal(SkBitmap::Config config, bool opaque,
 ...
 * format conversion
 */
static inline bool format_to_sk_config(cairo_format_t format,
...
static inline cairo_format_t sk_config_to_format(SkBitmap::Config config,
	bool opaque)
{
	switch(config) {
 ...
 */
static inline bool surface_to_sk_bitmap(cairo_surface_t * surface, SkBitmap & bitmap)
{
...
static inline SkXfermode::Mode operator_to_sk(cairo_operator_t op)
{
 ...
 */
static SkShader::TileMode extend_to_sk(cairo_extend_t extend)
{
 ...
 * color conversion
 */
static inline SkColor color_to_sk(const cairo_color_t & c)
{
 ...
 * matrix conversion
 */
static inline SkMatrix matrix_to_sk(const cairo_matrix_t & mat)
{
	SkMatrix skm;
...
static inline SkMatrix matrix_inverse_to_sk(const cairo_matrix_t & mat)
{
 ...
 * pattern conversion
 */
static inline cairo_surface_t * surface_from_pattern(const cairo_pattern_t * pattern)
{
	return (reinterpret_cast <const cairo_surface_pattern_t *> (pattern))->surface;
}

static SkShader * pattern_to_sk_shader(cairo_skia_surface_t * dst, const cairo_pattern_t * pattern,
...
static inline bool pattern_filter_to_sk(const cairo_pattern_t * pattern)
{
...
static inline bool pattern_to_sk_color(const cairo_pattern_t * pattern, SkColor & color)
{
	if(pattern->type != CAIRO_PATTERN_TYPE_SOLID)
		return false;
...
static cairo_status_t cpc_move_to(void * closure, const cairo_point_t * point)
{
...
static cairo_status_t cpc_line_to(void * closure, const cairo_point_t * point)
{
	struct cpc * cpc = static_cast <struct cpc *> (closure);
	if(cpc->matrix) {
		double x = _cairo_fixed_to_double(point->x);
...
static cairo_status_t cpc_curve_to(void * closure,
...
static cairo_status_t cpc_close_path(void * closure)
{
	struct cpc * cpc = static_cast <struct cpc *> (closure);
	cpc->skPath.close();
	return CAIRO_STATUS_SUCCESS;
...
static inline SkPath path_to_sk(cairo_path_fixed_t * path,
...
static cairo_surface_t * _cairo_skia_surface_create_similar(void * asurface,
...
static cairo_status_t _cairo_skia_surface_finish(void * asurface)
{
...
static cairo_image_surface_t * _get_image_surface(cairo_skia_surface_t * surface)
{
...
static cairo_status_t _cairo_skia_surface_acquire_source_image(void * asurface,
...
static void _cairo_skia_surface_release_source_image(void * asurface,
...
static cairo_status_t _cairo_skia_surface_acquire_dest_image(void * asurface,
...
static void _cairo_skia_surface_release_dest_image(void * asurface,
	cairo_rectangle_int_t * interest_rect,
	cairo_image_surface_t * image,
	cairo_rectangle_int_t * image_rect,
	void * image_extra)
...
#if 0
static cairo_status_t _cairo_skia_surface_clone_similar(void * asurface,
	cairo_surface_t * src,
	cairo_content_t content,
	int src_x,
	int src_y,
...
static cairo_status_t _cairo_skia_surface_clipper_intersect_clip_path(cairo_surface_clipper_t * clipper,
...
static cairo_bool_t _cairo_skia_surface_get_extents(void * asurface,
...
static SkBitmap * pattern_to_sk_bitmap(cairo_skia_surface_t * dst,
...
static cairo_int_status_t _cairo_skia_surface_paint(void * asurface,
	cairo_operator_t op,
...
static cairo_int_status_t _cairo_skia_surface_stroke(void * asurface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
...
static cairo_int_status_t _cairo_skia_surface_fill(void * asurface,
	cairo_operator_t op,
	const cairo_pattern_t * source,
	cairo_path_fixed_t * path,
	cairo_fill_rule_t fill_rule,
...
static cairo_skia_surface_t * _cairo_skia_surface_create_internal(SkBitmap::Config config,
	bool opaque,
	unsigned char * data,
	int width,
	int height,
...
cairo_surface_t * cairo_skia_surface_create(cairo_format_t format,
	int width,
	int height)
{
	SkBitmap::Config config;
...
cairo_surface_t * cairo_skia_surface_create_for_data(unsigned char * data,
	cairo_format_t format,
	int width,
	int height,
	int stride)
...
unsigned char * cairo_skia_surface_get_data(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return NULL;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
...
cairo_format_t cairo_skia_surface_get_format(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return (cairo_format_t)-1;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
...
int cairo_skia_surface_get_width(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return 0;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
...
int cairo_skia_surface_get_height(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return 0;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
...
int cairo_skia_surface_get_stride(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return 0;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
...
cairo_surface_t * cairo_skia_surface_get_image(cairo_surface_t * surface)
{
	if(surface->type != CAIRO_SURFACE_TYPE_SKIA)
		return NULL;
	cairo_skia_surface_t * esurf = (cairo_skia_surface_t *)surface;
