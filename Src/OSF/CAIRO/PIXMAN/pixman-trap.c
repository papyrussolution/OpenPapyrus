...
PIXMAN_EXPORT pixman_fixed_t pixman_sample_ceil_y(pixman_fixed_t y, int n)
{
 ...
 * starting y value
 */
PIXMAN_EXPORT void pixman_edge_init(pixman_edge_t * e, int n, pixman_fixed_t y_start, pixman_fixed_t x_top, pixman_fixed_t y_top, pixman_fixed_t x_bot, pixman_fixed_t y_bot)
{
...
#if 0
static void dump_image(pixman_image_t * image, const char *    title)
{
	int i, j;
	if(!image->type == BITS)
...
PIXMAN_EXPORT void pixman_rasterize_trapezoid(pixman_image_t * image, const pixman_trapezoid_t * trap, int x_off, int y_off)
{
	pixman_edge_t l, r;
	pixman_fixed_t t, b;
	...
	if(pixman_trapezoid_valid(trap)) {
		...
		int bpp = PIXMAN_FORMAT_BPP(image->bits.format);
		pixman_fixed_t y_off_fixed = pixman_int_to_fixed(y_off);
...
static pixman_bool_t get_trap_extents(pixman_op_t op, pixman_image_t * dest,
    const pixman_trapezoid_t * traps, int n_traps, pixman_box32_t * box)
{
	int i;
...
#define EXTEND(x)							\
	...
	return (box->x1 >= box->x2 || box->y1 >= box->y2) ? FALSE : TRUE;
}
...
PIXMAN_EXPORT void pixman_composite_trapezoids(pixman_op_t op,
    pixman_image_t * src, pixman_image_t * dst, pixman_format_code_t mask_format,
    int x_src, int y_src, int x_dst, int y_dst, int n_traps, const pixman_trapezoid_t * traps)
{
	...
	if(op == PIXMAN_OP_ADD && (src->common.flags & FAST_PATH_IS_OPAQUE) && (mask_format == dst->common.extended_format_code) &&
		...
		pixman_image_composite(op, src, tmp, dst, x_src + box.x1, y_src + box.y1,
		    0, 0, x_dst + box.x1, y_dst + box.y1, box.x2 - box.x1, box.y2 - box.y1);
		pixman_image_unref(tmp);
	}
}

static int FASTCALL greater_y(const pixman_point_fixed_t * a, const pixman_point_fixed_t * b)
{
...
static void FASTCALL triangle_to_trapezoids(const pixman_triangle_t * tri, pixman_trapezoid_t * traps)
{
	const pixman_point_fixed_t * tmp;
	const pixman_point_fixed_t * top = &tri->p1;
	const pixman_point_fixed_t * left = &tri->p2;
	const pixman_point_fixed_t * right = &tri->p3;
...
static pixman_trapezoid_t * FASTCALL convert_triangles(int n_tris, const pixman_triangle_t * tris)
{
...
PIXMAN_EXPORT void pixman_add_triangles(pixman_image_t * image, int32_t x_off, int32_t y_off, int n_tris, const pixman_triangle_t * tris)
{
	pixman_trapezoid_t * traps;
	if((traps = convert_triangles(n_tris, tris))) {
