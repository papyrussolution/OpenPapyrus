...
static force_inline uint16 pack_565_32_16(uint32_t pixel)
{
...
{
	__m128i r, g1, g2, b;
	__m128i data = pack_2x128_128(lo, hi);
...
{
	__m128i lo = _mm_shufflelo_epi16(data_lo, _MM_SHUFFLE(3, 3, 3, 3));
	__m128i hi = _mm_shufflelo_epi16(data_hi, _MM_SHUFFLE(3, 3, 3, 3));
...
{
	__m128i lo = _mm_shufflelo_epi16(data_lo, _MM_SHUFFLE(0, 0, 0, 0));
	__m128i hi = _mm_shufflelo_epi16(data_hi, _MM_SHUFFLE(0, 0, 0, 0));
...
static force_inline void pix_add_multiply_2x128(__m128i* src_lo, __m128i* src_hi, __m128i* alpha_dst_lo, __m128i* alpha_dst_hi,
    __m128i* dst_lo, __m128i* dst_hi, __m128i* alpha_src_lo, __m128i* alpha_src_hi, __m128i* ret_lo, __m128i* ret_hi)
{
	__m128i t1_lo, t1_hi;
	__m128i t2_lo, t2_hi;
...
static force_inline void invert_colors_2x128(__m128i data_lo, __m128i data_hi, __m128i* inv_lo, __m128i* inv_hi)
{
	__m128i lo = _mm_shufflelo_epi16(data_lo, _MM_SHUFFLE(3, 0, 1, 2));
	__m128i hi = _mm_shufflelo_epi16(data_hi, _MM_SHUFFLE(3, 0, 1, 2));
	*inv_lo = _mm_shufflehi_epi16(lo, _MM_SHUFFLE(3, 0, 1, 2));
	*inv_hi = _mm_shufflehi_epi16(hi, _MM_SHUFFLE(3, 0, 1, 2));
}

static force_inline void over_2x128(__m128i* src_lo, __m128i* src_hi, __m128i* alpha_lo, __m128i* alpha_hi, __m128i* dst_lo, __m128i* dst_hi)
{
	__m128i t1, t2;
...
static force_inline void over_rev_non_pre_2x128(__m128i src_lo, __m128i src_hi, __m128i* dst_lo, __m128i* dst_hi)
{
	__m128i lo, hi;
...
static force_inline void in_over_2x128(__m128i* src_lo, __m128i * src_hi, __m128i * alpha_lo,
    __m128i* alpha_hi, __m128i* mask_lo, __m128i* mask_hi, __m128i* dst_lo, __m128i* dst_hi)
{
	__m128i s_lo, s_hi;
	__m128i a_lo, a_hi;
 ...
 */
static force_inline void save_128_write_combining(__m128i* dst, __m128i data)
{
...
static force_inline __m128i pix_add_multiply_1x128(__m128i* src, __m128i* alpha_dst, __m128i* dst, __m128i* alpha_src)
{
...
static force_inline __m128i over_rev_non_pre_1x128(__m128i src, __m128i dst)
{
	...
	return over_1x128(pix_multiply_1x128(invert_colors_1x128(src), _mm_or_si128(alpha, mask_alpha)), alpha, dst);
}
...
static force_inline uint32_t core_combine_over_u_pixel_sse2(uint32_t src, uint32_t dst)
{
	...
	if(a == 0xff) {
		...
		return pack_1x128_32(over_1x128(xmms, expand_alpha_1x128(xmms), unpack_32_1x128(dst)));
	}
	return dst;
}
...
static force_inline void core_combine_over_u_sse2_mask(uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
	...
	while(w >= 4) {
		...
		if(!is_zero(mask)) {
			...
			__m128i src = load_128_unaligned((__m128i*)ps);
			if(is_opaque(_mm_and_si128(src, mask))) {
				...
				pix_multiply_2x128(&src_lo, &src_hi, &mask_lo, &mask_hi, &src_lo, &src_hi);
				unpack_128_2x128(dst, &dst_lo, &dst_hi);
				expand_alpha_2x128(src_lo, src_hi, &alpha_lo, &alpha_hi);
				over_2x128(&src_lo, &src_hi, &alpha_lo, &alpha_hi, &dst_lo, &dst_hi);
...
static force_inline void core_combine_over_u_sse2_no_mask(uint32_t * pd, const uint32_t * ps, int w)
{
	uint32_t s, d;
	/* Align dst on a 16-byte boundary */
	while(w && ((uintptr_t)pd & 15)) {
...
static force_inline void sse2_combine_over_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	if(pm)
...
static void sse2_combine_over_reverse_u(pixman_implementation_t * imp, pixman_op_t op, uint32_t * pd,
    const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
...
static force_inline uint32_t core_combine_in_u_pixel_sse2(uint32_t src, uint32_t dst)
{
	...
	if(maska == 0) {
		...
		return pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(dst), expand_alpha_1x128(unpack_32_1x128(src))));
	}
	return dst;
}

static void sse2_combine_in_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
	__m128i xmm_src_lo, xmm_src_hi;
...
static void sse2_combine_in_reverse_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
	__m128i xmm_src_lo, xmm_src_hi;
...
static void sse2_combine_out_reverse_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	while(w && ((uintptr_t)pd & 15)) {
		uint32_t s = combine1(ps, pm);
		uint32_t d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(d), negate_1x128(expand_alpha_1x128(unpack_32_1x128(s)))));
		if(pm)
			pm++;
		ps++;
		w--;
...
static void sse2_combine_out_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	while(w && ((uintptr_t)pd & 15)) {
		uint32_t s = combine1(ps, pm);
		uint32_t d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(s), negate_1x128(expand_alpha_1x128(unpack_32_1x128(d)))));
		w--;
		ps++;
		if(pm)
			pm++;
...
static force_inline uint32_t core_combine_atop_u_pixel_sse2(uint32_t src, uint32_t dst)
{
...
static void sse2_combine_atop_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
	__m128i xmm_src_lo, xmm_src_hi;
	...
	while(w >= 4) {
		...
		pix_add_multiply_2x128(&xmm_src_lo, &xmm_src_hi, &xmm_alpha_dst_lo, &xmm_alpha_dst_hi,
		    &xmm_dst_lo, &xmm_dst_hi, &xmm_alpha_src_lo, &xmm_alpha_src_hi, &xmm_dst_lo, &xmm_dst_hi);
		save_128_aligned((__m128i*)pd, pack_2x128_128(xmm_dst_lo, xmm_dst_hi));
		ps += 4;
		pd += 4;
		w -= 4;
...
static force_inline uint32_t core_combine_reverse_atop_u_pixel_sse2(uint32_t src, uint32_t dst)
{
	__m128i s = unpack_32_1x128(src);
	__m128i d = unpack_32_1x128(dst);
...
static void sse2_combine_atop_reverse_u(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, d;
	__m128i xmm_src_lo, xmm_src_hi;
...
static force_inline uint32_t core_combine_xor_u_pixel_sse2(uint32_t src, uint32_t dst)
{
	__m128i s = unpack_32_1x128(src);
	__m128i d = unpack_32_1x128(dst);
...
static void sse2_combine_xor_u(pixman_implementation_t * imp, pixman_op_t op, uint32_t * dst,
    ...
    int w)
{
	...
	while(w && (uintptr_t)pd & 15) {
		...
		m = *pm++;
		d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(pix_multiply_1x128(unpack_32_1x128(s), unpack_32_1x128(m)), expand_alpha_1x128(unpack_32_1x128(d))));
		w--;
	}
	while(w >= 4) {
		xmm_dst_hi = load_128_aligned((__m128i*)pd);
...
static void sse2_combine_in_reverse_ca(pixman_implementation_t * imp, pixman_op_t op, uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	...
	while(w && (uintptr_t)pd & 15) {
		...
		m = *pm++;
		d = *pd;
		*pd++ = pack_1x128_32(pix_multiply_1x128(unpack_32_1x128(d), pix_multiply_1x128(unpack_32_1x128(m), expand_alpha_1x128(unpack_32_1x128(s)))));
		w--;
	}
	while(w >= 4) {
		xmm_dst_hi = load_128_aligned((__m128i*)pd);
...
static force_inline uint32_t core_combine_atop_ca_pixel_sse2(uint32_t src, uint32_t mask, uint32_t dst)
{
...
static void sse2_combine_atop_ca(pixman_implementation_t * imp, pixman_op_t op,
    uint32_t * pd, const uint32_t * ps, const uint32_t * pm, int w)
{
	uint32_t s, m, d;
	__m128i xmm_src_lo, xmm_src_hi;
	...
	while(w >= 4) {
		...
		pix_add_multiply_2x128(&xmm_dst_lo, &xmm_dst_hi, &xmm_mask_lo, &xmm_mask_hi,
		    &xmm_src_lo, &xmm_src_hi, &xmm_alpha_dst_lo, &xmm_alpha_dst_hi, &xmm_dst_lo, &xmm_dst_hi);
		save_128_aligned((__m128i*)pd, pack_2x128_128(xmm_dst_lo, xmm_dst_hi));
		ps += 4;
		pd += 4;
		pm += 4;
...
static force_inline uint32_t core_combine_reverse_atop_ca_pixel_sse2(uint32_t src, uint32_t mask, uint32_t dst)
{
	__m128i m = unpack_32_1x128(mask);
	__m128i s = unpack_32_1x128(src);
	__m128i d = unpack_32_1x128(dst);
...
static force_inline uint32_t core_combine_xor_ca_pixel_sse2(uint32_t src, uint32_t mask, uint32_t dst)
{
	__m128i a = unpack_32_1x128(mask);
	__m128i s = unpack_32_1x128(src);
	__m128i d = unpack_32_1x128(dst);
	__m128i alpha_dst = negate_1x128(pix_multiply_1x128(a, expand_alpha_1x128(s)));
	__m128i dest      = pix_multiply_1x128(s, a);
	__m128i alpha_src = negate_1x128(expand_alpha_1x128(d));
	return pack_1x128_32(pix_add_multiply_1x128(&d, &alpha_dst, &dest, &alpha_src));
}
...
static void sse2_composite_over_n_0565(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		while(w && (uintptr_t)dst & 15) {
			d = *dst;
			*dst++ = pack_565_32_16(
			    pack_1x128_32(over_1x128(xmm_src, xmm_alpha, expand565_16_1x128(d))));
			w--;
		}
		while(w >= 8) {
			xmm_dst = load_128_aligned((__m128i*)dst);
...
static force_inline uint16 composite_over_8888_0565pixel(uint32_t src, uint16 dst)
{
	__m128i ms;
...
static void sse2_composite_add_8_8(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	while(height--) {
		...
		sse2_combine_add_u(imp, op, (uint32_t*)dst, (uint32_t*)src, NULL, w >> 2);
		/* Small tail */
		dst += w & 0xfffc;
		src += w & 0xfffc;
		w &= 3;
	...
	while(height--) {
		...
		while(w >= 4) {
			...
			if(m) {
				...
				__m128i xmm_mask = _mm_unpacklo_epi8(unpack_32_1x128(m), _mm_setzero_si128());
				unpack_128_2x128(xmm_mask, &xmm_mask_lo, &xmm_mask_hi);
				unpack_128_2x128(xmm_dst, &xmm_dst_lo, &xmm_dst_hi);
...
static void sse2_composite_over_reverse_n_8888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	int dst_stride;
	int32_t w;
	uint32_t src = _pixman_image_get_solid(imp, src_image, dest_image->bits.format);
	if(src == 0)
		return;
	PIXMAN_IMAGE_GET_LINE(dest_image, dest_x, dest_y, uint32_t, dst_stride, dst_line, 1);
	xmm_src = expand_pixel_32_1x128(src);
...
FAST_NEAREST_MAINLOOP(sse2_8888_8888_cover_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, COVER)
FAST_NEAREST_MAINLOOP(sse2_8888_8888_none_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, NONE)
FAST_NEAREST_MAINLOOP(sse2_8888_8888_pad_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, PAD)
FAST_NEAREST_MAINLOOP(sse2_8888_8888_normal_OVER, scaled_nearest_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, NORMAL)
...
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_cover_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, COVER, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_pad_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, PAD, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_none_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, NONE, TRUE, TRUE)
FAST_NEAREST_MAINLOOP_COMMON(sse2_8888_n_8888_normal_OVER, scaled_nearest_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, NORMAL, TRUE, TRUE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_cover_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32_t, uint32_t, uint32_t, COVER, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_pad_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32_t, uint32_t, uint32_t, PAD, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_none_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32_t, uint32_t, uint32_t, NONE, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_normal_SRC, scaled_bilinear_scanline_sse2_8888_8888_SRC, uint32_t, uint32_t, uint32_t, NORMAL, FLAG_NONE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, uint32_t, COVER, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, uint32_t, PAD, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_none_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, uint32_t, NONE, FLAG_NONE)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8888_normal_OVER, scaled_bilinear_scanline_sse2_8888_8888_OVER, uint32_t, uint32_t, uint32_t, NORMAL, FLAG_NONE)
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32_t, uint8, uint32_t, COVER, FLAG_HAVE_NON_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32_t, uint8, uint32_t, PAD, FLAG_HAVE_NON_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_none_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32_t, uint8, uint32_t, NONE, FLAG_HAVE_NON_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_8_8888_normal_OVER, scaled_bilinear_scanline_sse2_8888_8_8888_OVER, uint32_t, uint8, uint32_t, NORMAL, FLAG_HAVE_NON_SOLID_MASK)

static force_inline void scaled_bilinear_scanline_sse2_8888_n_8888_OVER(uint32_t * dst,
    const uint32_t * mask, const uint32_t * src_top, const uint32_t * src_bottom,
    int32_t w, int wt, int wb, pixman_fixed_t vx_, pixman_fixed_t unit_x_, pixman_fixed_t max_vx, pixman_bool_t zero_src)
{
	intptr_t vx = vx_;
	intptr_t unit_x = unit_x_;
	BILINEAR_DECLARE_VARIABLES;
...
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_cover_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, COVER, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_pad_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, PAD, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_none_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, NONE, FLAG_HAVE_SOLID_MASK)
FAST_BILINEAR_MAINLOOP_COMMON(sse2_8888_n_8888_normal_OVER, scaled_bilinear_scanline_sse2_8888_n_8888_OVER, uint32_t, uint32_t, uint32_t, NORMAL, FLAG_HAVE_SOLID_MASK)
...
static uint32_t * sse2_fetch_x8r8g8b8(pixman_iter_t * iter, const uint32_t * mask)
{
	...
	while(w >= 4) {
		save_128_aligned((__m128i*)dst, _mm_or_si128(load_128_unaligned((__m128i*)src), ff000000));
		dst += 4;
		src += 4;
		w -= 4;
	}
