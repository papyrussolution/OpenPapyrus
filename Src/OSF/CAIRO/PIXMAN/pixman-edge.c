 ...
 */
/* Этот файл используется как включаемый
#include "cairoint.h"
#pragma hdrstop
*/
				...
				if(rxi - lxi > 4) {
					if(fill_start < 0) {
						...
						if(lxi >= fill_end || rxi < fill_start) {
							...
							/* Update fill_start */
							if(lxi > fill_start) {
								ADD_SATURATE_8(ap + fill_start, fill_size * N_X_FRAC(8), lxi - fill_start);
								fill_start = lxi;
							}
							else if(lxi < fill_start) {
								ADD_SATURATE_8(ap + lxi, N_X_FRAC(8), fill_start - lxi);
							}

							/* Update fill_end */
							if(rxi < fill_end) {
								ADD_SATURATE_8(ap + rxi, fill_size * N_X_FRAC(8), fill_end - rxi);
								fill_end = rxi;
							}
							else if(fill_end < rxi) {
								ADD_SATURATE_8(ap + fill_end, N_X_FRAC(8), rxi - fill_end);
							}
							fill_size++;
						}
					}
...
PIXMAN_EXPORT void pixman_rasterize_edges(pixman_image_t * image,
    pixman_edge_t * l, pixman_edge_t * r, pixman_fixed_t t, pixman_fixed_t b)
{
