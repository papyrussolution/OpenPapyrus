...
static uint32_t * radial_get_scanline_narrow(pixman_iter_t * iter, const uint32_t * mask)
{
	...
	if(unit.vector[2] == 0 && v.vector[2] == pixman_fixed_1) {
		...
		dc = dot(2 * (pixman_fixed_48_16_t)v.vector[0] + unit.vector[0],
		    2 * (pixman_fixed_48_16_t)v.vector[1] + unit.vector[1], 0,
...
{
	uint32_t * buffer = radial_get_scanline_narrow(iter, 0);
...
PIXMAN_EXPORT pixman_image_t * pixman_image_create_radial_gradient(const pixman_point_fixed_t *  inner,
