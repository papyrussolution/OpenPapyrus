...
static force_inline int count_leading_zeros(uint32_t x)
{
...
static force_inline uint64_t rounded_udiv_128_by_48(uint64_t hi, uint64_t lo, uint64_t div, uint64_t * result_hi)
{
...
static inline int64_t rounded_sdiv_128_by_49(int64_t hi, uint64_t lo, int64_t div, int64_t  * signed_result_hi)
{
...
PIXMAN_EXPORT pixman_bool_t pixman_transform_point_31_16(const pixman_transform_t * t, const pixman_vector_48_16_t * v, pixman_vector_48_16_t * result)
{
	...
	if(divint == pixman_fixed_1 && divfrac == 0) {
		 ...
		 */
		int32_t hi32divbits = (int32_t)(divint >> 32);
		if(hi32divbits < 0)
...
PIXMAN_EXPORT void pixman_transform_point_31_16_affine(const pixman_transform_t * t,
    const pixman_vector_48_16_t * v, pixman_vector_48_16_t * result)
{
...
PIXMAN_EXPORT void pixman_transform_init_identity(struct pixman_transform * matrix)
{
	int i;
	memzero(matrix, sizeof(struct pixman_transform));
	for(i = 0; i < 3; i++)
...
PIXMAN_EXPORT pixman_bool_t pixman_transform_multiply(struct pixman_transform * dst,
    const struct pixman_transform * l, const struct pixman_transform * r)
{
...
PIXMAN_EXPORT void pixman_transform_init_scale(struct pixman_transform * t, pixman_fixed_t sx, pixman_fixed_t sy)
{
...
static pixman_fixed_t FASTCALL fixed_inverse(pixman_fixed_t x)
{
...
PIXMAN_EXPORT void pixman_transform_init_translate(struct pixman_transform * t, pixman_fixed_t tx, pixman_fixed_t ty)
{
	memzero(t, sizeof(struct pixman_transform));
	t->matrix[0][0] = F(1);
	t->matrix[0][2] = tx;
...
#define IS_UNIT(a)    (within_epsilon(a, F(1), EPSILON) || within_epsilon(a, F(-1), EPSILON) || IS_ZERO(a))
...
{
	return (IS_SAME(t->matrix[0][0], t->matrix[1][1]) && IS_SAME(t->matrix[0][0], t->matrix[2][2]) &&
	    !IS_ZERO(t->matrix[0][0]) && IS_ZERO(t->matrix[0][1]) && IS_ZERO(t->matrix[0][2]) &&
	    IS_ZERO(t->matrix[1][0]) && IS_ZERO(t->matrix[1][2]) && IS_ZERO(t->matrix[2][0]) && IS_ZERO(t->matrix[2][1]));
}

PIXMAN_EXPORT pixman_bool_t pixman_transform_is_scale(const struct pixman_transform * t)
{
	return (!IS_ZERO(t->matrix[0][0]) && IS_ZERO(t->matrix[0][1]) && IS_ZERO(t->matrix[0][2]) &&
	    IS_ZERO(t->matrix[1][0]) && !IS_ZERO(t->matrix[1][1]) && IS_ZERO(t->matrix[1][2]) &&
	    IS_ZERO(t->matrix[2][0]) && IS_ZERO(t->matrix[2][1]) && !IS_ZERO(t->matrix[2][2]));
}

PIXMAN_EXPORT pixman_bool_t pixman_transform_is_int_translate(const struct pixman_transform * t)
{
	return (IS_ONE(t->matrix[0][0]) && IS_ZERO(t->matrix[0][1]) && IS_INT(t->matrix[0][2]) &&
	    IS_ZERO(t->matrix[1][0]) && IS_ONE(t->matrix[1][1]) && IS_INT(t->matrix[1][2]) &&
	    IS_ZERO(t->matrix[2][0]) && IS_ZERO(t->matrix[2][1]) && IS_ONE(t->matrix[2][2]));
}

PIXMAN_EXPORT pixman_bool_t pixman_transform_is_inverse(const struct pixman_transform * a, const struct pixman_transform * b)
{
	struct pixman_transform t;
	return pixman_transform_multiply(&t, a, b) ? pixman_transform_is_identity(&t) : FALSE;
}
...
PIXMAN_EXPORT pixman_bool_t pixman_f_transform_bounds(const struct pixman_f_transform * t, struct pixman_box16 * b)
{
	...
	for(i = 0; i < 4; i++) {
		if(!pixman_f_transform_point(t, &v[i]))
			return FALSE;
		x1 = (int)floor(v[i].v[0]);
		y1 = (int)floor(v[i].v[1]);
		x2 = (int)ceil(v[i].v[0]);
		y2 = (int)ceil(v[i].v[1]);
		if(i == 0) {
			b->x1 = x1;
