...
static force_inline uint32_t fetch_24(uint8 * a)
{
	if(((uintptr_t)a) & 1) {
...
static force_inline void store_24(uint8 * a, uint32_t v)
{
	if(((uintptr_t)a) & 1) {
#ifdef WORDS_BIGENDIAN
		*a = (uint8)(v >> 16);
...
static force_inline uint32_t over(uint32_t src, uint32_t dest)
{
	uint32_t a = ~src >> 24;
...
static force_inline uint32_t in(uint32_t x, uint8 y)
{
	uint16 a = y;
	UN8x4_MUL_UN8(x, a);
	return x;
...
static void fast_composite_over_n_8888_0565_ca(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	...
	uint32_t srca = src >> 24;
	if(src) {
		uint16 src16 = convert_8888_to_0565(src);
		PIXMAN_IMAGE_GET_LINE(dest_image, dest_x, dest_y, uint16, dst_stride, dst_line, 1);
		PIXMAN_IMAGE_GET_LINE(mask_image, mask_x, mask_y, uint32_t, mask_stride, mask_line, 1);
		while(height--) {
			dst = dst_line;
...
#if 0
static void fast_composite_over_8888_0888(pixman_implementation_t * imp, pixman_composite_info_t * info)
{
	PIXMAN_COMPOSITE_ARGS(info);
	uint8     * dst_line, * dst;
	uint32_t d;
    ...
    pixman_composite_info_t * info)
{
	...
	src = (uint8*)src_image->bits.bits + src_y * src_stride + src_x * bpp;
	dst = (uint8*)dest_image->bits.bits + dest_y * dst_stride + dest_x * bpp;
...
FAST_NEAREST(8888_565_cover, 8888, 0565, uint32_t, uint16, SRC, COVER)
FAST_NEAREST(8888_565_none, 8888, 0565, uint32_t, uint16, SRC, NONE)
FAST_NEAREST(8888_565_pad, 8888, 0565, uint32_t, uint16, SRC, PAD)
FAST_NEAREST(8888_565_normal, 8888, 0565, uint32_t, uint16, SRC, NORMAL)
FAST_NEAREST(565_565_normal, 0565, 0565, uint16, uint16, SRC, NORMAL)
FAST_NEAREST(8888_565_cover, 8888, 0565, uint32_t, uint16, OVER, COVER)
FAST_NEAREST(8888_565_none, 8888, 0565, uint32_t, uint16, OVER, NONE)
FAST_NEAREST(8888_565_pad, 8888, 0565, uint32_t, uint16, OVER, PAD)
FAST_NEAREST(8888_565_normal, 8888, 0565, uint32_t, uint16, OVER, NORMAL)

#define REPEAT_MIN_WIDTH    32

static void fast_composite_tiled_repeat(pixman_implementation_t * imp,
    pixman_composite_info_t * info)
{
	PIXMAN_COMPOSITE_ARGS(info);
	...
	_pixman_implementation_lookup_composite(imp->toplevel, info->op,
	    src_image->common.extended_format_code, src_flags, mask_format, mask_flags,
	    dest_image->common.extended_format_code, info->dest_flags, &imp, &func);
	src_bpp = PIXMAN_FORMAT_BPP(src_image->bits.format);
	if(src_image->bits.width < REPEAT_MIN_WIDTH && oneof3(src_bpp, 32, 16, 8) && !src_image->bits.indexed) {
		sx = src_x;
		...
		_pixman_bits_image_init(&extended_src_image, src_image->bits.format, src_width, 1, &extended_src[0], src_stride, FALSE);
	...
	while(--height >= 0) {
		...
		if(need_src_extension) {
			if(src_bpp == 32) {
				...
				uint16 * src_line_16;
				PIXMAN_IMAGE_GET_LINE(src_image, 0, sy, uint16, src_stride, src_line_16, 1);
				...
				uint8 * src_line_8;
				PIXMAN_IMAGE_GET_LINE(src_image, 0, sy, uint8, src_stride, src_line_8, 1);
		...
		while(width_remain > 0) {
			...
			SETMIN(num_pixels, width_remain);
			info2.src_x = sx;
...
static force_inline void scaled_nearest_scanline_565_565_SRC(uint16 * dst, const uint16 * src,
    int32_t w, pixman_fixed_t vx, pixman_fixed_t unit_x, pixman_fixed_t max_vx, pixman_bool_t fully_transparent_src)
{
...
static force_inline uint32_t fetch_nearest(pixman_repeat_t src_repeat, pixman_format_code_t format, uint32_t * src, int x, int src_width)
{
...
#define FAST_SIMPLE_ROTATE(suffix, pix_type)				      \
		...
		if(trailing_pixels) { \
			...
			blt_rotated_270_trivial_ ## suffix(				       \
			    ...
			    H);								      \
									      ...
									      \
	static void fast_composite_rotate_90_ ## suffix(pixman_implementation_t *imp, pixman_composite_info_t *info) \
	{									      \
		PIXMAN_COMPOSITE_ARGS(info);						 \
		pix_type       * dst_line;						   \
		pix_type       * src_line;						   \
	...
	{									      \
		...
		PIXMAN_IMAGE_GET_LINE(src_image, src_x_t, src_y_t, pix_type, src_stride, src_line, 1); \
		blt_rotated_270_ ## suffix(dst_line, dst_stride, src_line, src_stride, width, height); \
	}
...
#define SIMPLE_ROTATE_FAST_PATH(op, s, d, suffix)			     \
	...
	{   PIXMAN_OP_any,
	    PIXMAN_any,
	    (FAST_PATH_STANDARD_FLAGS | FAST_PATH_ID_TRANSFORM | FAST_PATH_BITS_IMAGE | FAST_PATH_NORMAL_REPEAT),
	    PIXMAN_any, 0,
...
{
	int short_stride = (stride * (int)sizeof(uint32_t)) / (int)sizeof(uint16);
...
static pixman_bool_t fast_path_fill(pixman_implementation_t * imp, uint32_t * bits,
    int stride, int bpp, int x, int y, int width, int height, uint32_t filler)
{
	switch(bpp) {
		case 1: pixman_fill1(bits, stride, x, y, width, height, filler); break;
		case 8: pixman_fill8(bits, stride, x, y, width, height, filler); break;
		case 16: pixman_fill16(bits, stride, x, y, width, height, filler); break;
		case 32: pixman_fill32(bits, stride, x, y, width, height, filler); break;
...
static uint32_t * bits_image_fetch_bilinear_no_repeat_8888(pixman_iter_t * iter, const uint32_t * mask)
{
	...
	while(buffer < end && x < 0) {
		uint32_t tr = top_row[pixman_fixed_to_int(x_top) + 1] | top_mask;
		uint32_t br = bottom_row[pixman_fixed_to_int(x_bottom) + 1] | bottom_mask;
	...
	while(buffer < end  &&  x < w) {
		if(*mask) {
			uint32_t tl = top_row [pixman_fixed_to_int(x_top)] | top_mask;
			uint32_t tr = top_row [pixman_fixed_to_int(x_top) + 1] | top_mask;
			uint32_t bl = bottom_row [pixman_fixed_to_int(x_bottom)] | bottom_mask;
			uint32_t br = bottom_row [pixman_fixed_to_int(x_bottom) + 1] | bottom_mask;
			int32_t distx = pixman_fixed_to_bilinear_weight(x);
...
typedef uint32_t (*convert_pixel_t)(const uint8 * row, int x);

static force_inline void bits_image_fetch_separable_convolution_affine(pixman_image_t * image,
    int offset, int line, int width, uint32_t * buffer, const uint32_t * mask,
...
static force_inline void bits_image_fetch_bilinear_affine(pixman_image_t * image, int offset, int line,
    int width, uint32_t * buffer, const uint32_t * mask, convert_pixel_t convert_pixel,
    pixman_format_code_t format, pixman_repeat_t repeat_mode)
{
	pixman_fixed_t x, y;
	pixman_fixed_t ux, uy;
	pixman_vector_t v;
	...
	for(i = 0; i < width; ++i) {
		...
		buffer[i] = bilinear_interpolation(tl, tr, bl, br, distx, disty);
next:
		x += ux;
		y += uy;
	}
...
static force_inline void bits_image_fetch_nearest_affine(pixman_image_t * image, int offset, int line, int width,
    uint32_t * buffer, const uint32_t * mask, convert_pixel_t convert_pixel, pixman_format_code_t format, pixman_repeat_t repeat_mode)
{
	pixman_fixed_t x, y;
	pixman_fixed_t ux, uy;
	pixman_vector_t v;
...
static force_inline uint32_t convert_a8r8g8b8(const uint8 * row, int x)
{
	return *(((uint32_t*)row) + x);
}

static force_inline uint32_t convert_x8r8g8b8(const uint8 * row, int x)
{
	return *(((uint32_t*)row) + x);
}

static force_inline uint32_t convert_a8(const uint8 * row, int x)
{
	return *(row + x) << 24;
}

static force_inline uint32_t convert_r5g6b5(const uint8 * row, int x)
{
	return convert_0565_to_0888(*((uint16*)row + x));
}

#define MAKE_SEPARABLE_CONVOLUTION_FETCHER(name, format, repeat_mode)  \
	static uint32_t * bits_image_fetch_separable_convolution_affine_ ## name(pixman_iter_t *iter, const uint32_t * mask) \
	{								    \
...
#define MAKE_BILINEAR_FETCHER(name, format, repeat_mode) \
	static uint32_t * bits_image_fetch_bilinear_affine_ ## name(pixman_iter_t   *iter, const uint32_t * mask)   \
	{								    \
		bits_image_fetch_bilinear_affine(iter->image, iter->x, iter->y++, iter->width, iter->buffer, mask, \
		    convert_ ## format, PIXMAN_ ## format, repeat_mode); \
		return iter->buffer; \
	}

#define MAKE_NEAREST_FETCHER(name, format, repeat_mode)			\
	static uint32_t * bits_image_fetch_nearest_affine_ ## name(pixman_iter_t   *iter, const uint32_t * mask) \
	{								    \
		bits_image_fetch_nearest_affine(iter->image, iter->x, iter->y++, iter->width, iter->buffer, mask, \
		    convert_ ## format, PIXMAN_ ## format, repeat_mode); \
		return iter->buffer; \
	}
...
static const pixman_iter_info_t fast_iters[] =
{
	...
	{ PIXMAN_r5g6b5, FAST_PATH_STD_DEST_FLAGS, (iter_flags_t)(ITER_NARROW | ITER_DEST), _pixman_iter_init_bits_stride,
	  fast_fetch_r5g6b5, fast_write_back_r5g6b5 },
	{ PIXMAN_r5g6b5, FAST_PATH_STD_DEST_FLAGS, (iter_flags_t)(ITER_NARROW | ITER_DEST | ITER_IGNORE_RGB | ITER_IGNORE_ALPHA),
	  _pixman_iter_init_bits_stride, fast_dest_fetch_noop, fast_write_back_r5g6b5 },
	{ PIXMAN_a8r8g8b8, (FAST_PATH_STANDARD_FLAGS | FAST_PATH_SCALE_TRANSFORM | FAST_PATH_BILINEAR_FILTER | FAST_PATH_SAMPLES_COVER_CLIP_BILINEAR),
	  (iter_flags_t)(ITER_NARROW | ITER_SRC), fast_bilinear_cover_iter_init, NULL, NULL},
...
#define FAST_BILINEAR_FLAGS						\
	...
	{ PIXMAN_a8r8g8b8, FAST_BILINEAR_FLAGS, (iter_flags_t)(ITER_NARROW | ITER_SRC),
	  NULL, bits_image_fetch_bilinear_no_repeat_8888, NULL},
	{ PIXMAN_x8r8g8b8, FAST_BILINEAR_FLAGS, (iter_flags_t)(ITER_NARROW | ITER_SRC),
	  NULL, bits_image_fetch_bilinear_no_repeat_8888, NULL},
...
#define GENERAL_NEAREST_FLAGS (FAST_PATH_NO_ALPHA_MAP | FAST_PATH_NO_ACCESSORS | FAST_PATH_HAS_TRANSFORM | FAST_PATH_AFFINE_TRANSFORM | FAST_PATH_NEAREST_FILTER)
#define GENERAL_SEPARABLE_CONVOLUTION_FLAGS (FAST_PATH_NO_ALPHA_MAP | FAST_PATH_NO_ACCESSORS | FAST_PATH_HAS_TRANSFORM | \
	    FAST_PATH_AFFINE_TRANSFORM | FAST_PATH_SEPARABLE_CONVOLUTION_FILTER)
