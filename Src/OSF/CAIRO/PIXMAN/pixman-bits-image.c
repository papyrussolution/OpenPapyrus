...
static force_inline uint32_t bits_image_fetch_pixel_nearest(bits_image_t * image, pixman_fixed_t x, pixman_fixed_t y, get_pixel_t get_pixel)
{
...
static void bits_image_fetch_untransformed_repeat_none(bits_image_t * image,
    pixman_bool_t wide, int x, int y, int width, uint32_t * buffer)
{
	uint32_t w;
	...
	if(x < 0) {
		w = MIN(width, -x);
		memzero(buffer, w * (wide ? sizeof(argb_t) : 4));
		width -= w;
		buffer += w * (wide ? 4 : 1);
		x += w;
	}
...
static uint32_t * bits_image_fetch_untransformed_float(pixman_iter_t * iter, const uint32_t * mask)
{
	...
	if(image->common.repeat == PIXMAN_REPEAT_NONE) {
		...
		bits_image_fetch_untransformed_repeat_normal(&image->bits, TRUE, x, y, width, buffer);
	}
	iter->y++;
	return buffer;
}
...
static uint32_t * dest_get_scanline_narrow(pixman_iter_t * iter, const uint32_t * mask)
{
	...
	if(image->common.alpha_map) {
		...
		if((alpha = (uint32_t *)SAlloc::M(width * sizeof(uint32_t)))) {
			...
			x -= image->common.alpha_origin_x;
			y -= image->common.alpha_origin_y;
			image->common.alpha_map->fetch_scanline_32(image->common.alpha_map, x, y, width, alpha, mask);
			for(i = 0; i < width; ++i) {
				buffer[i] &= ~0xff000000;
...
static void dest_write_back_narrow(pixman_iter_t * iter)
{
	...
	if(image->common.alpha_map) {
		x -= image->common.alpha_origin_x;
		y -= image->common.alpha_origin_y;
		image->common.alpha_map->store_scanline_32(image->common.alpha_map, x, y, width, buffer);
	}
	iter->y++;
}
