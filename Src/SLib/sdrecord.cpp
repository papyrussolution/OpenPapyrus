// SDRECORD.CPP
// Copyright (c) A.Sobolev 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022
// @codepage UTF-8
//
		...
		if(t) {
			{
				...
				if(GetNextToken(temp_scan, rRl, &lv, temp_symb) == TOK_TYPE) { // @recursion
					...
					//
					// Если символ не распознается, то "чему быть - того не миновать": следуем штатным маршрутом.
					//
				}
			}
			rScan.IncrLen();
