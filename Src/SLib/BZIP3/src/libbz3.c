...
//
static const uint32 crc32Table[256] = { // castagnoli-crc
...
static uint32 crc32sum(uint32 crc, uint8 * _RESTRICT buf, size_t size) 
{
	while(size--)
...
static uint32 lzp_upcast(const uint8 * ptr) 
{
	// val = *(uint32 *)ptr; - written this way to avoid UB
	uint32 val;
	memcpy(&val, ptr, sizeof(val));
	return val;
}

static int32 lzp_encode_block(const uint8 * _RESTRICT in, const uint8 * in_end, uint8 * _RESTRICT out, uint8 * out_end, int32 * _RESTRICT lut) 
{
	const uint8 * ins = in;
	const uint8 * outs = out;
	const uint8 * out_eob = out_end - 8;
	const uint8 * heur = in;
	uint32 ctx;
	for(int32 i = 0; i < 4; ++i)
		*out++ = *in++;
	ctx = ((uint32)in[-1]) | (((uint32)in[-2]) << 8) | (((uint32)in[-3]) << 16) | (((uint32)in[-4]) << 24);
	while(in < in_end - LZP_MIN_MATCH - 32 && out < out_eob) {
		uint32 idx = (ctx >> 15 ^ ctx ^ ctx >> 3) & ((int32)(1 << LZP_DICTIONARY) - 1);
		int32 val = lut[idx];
		lut[idx] = in - ins;
		if(val > 0) {
			const uint8 * _RESTRICT ref = ins + val;
			if(memcmp(in + LZP_MIN_MATCH - 4, ref + LZP_MIN_MATCH - 4, sizeof(uint32)) == 0 &&
			    memcmp(in, ref, sizeof(uint32)) == 0) {
				if(heur > in && lzp_upcast(heur) != lzp_upcast(ref + (heur - in)))
					goto not_found;
				int32 len = 4;
				for(; in + len < in_end - LZP_MIN_MATCH - 32; len += sizeof(uint32)) {
					if(lzp_upcast(in + len) != lzp_upcast(ref + len))  
						break;
				}
...
not_found:                      ;
	...
	return out >= out_eob ? -1 : (int32)(out - outs);
}

static int32 lzp_decode_block(const uint8 * _RESTRICT in, const uint8 * in_end, int32 * _RESTRICT lut, 
	uint8 * _RESTRICT out, const uint8 * out_end) 
{
	const uint8 * outs = out;
	for(int32 i = 0; i < 4; ++i)  
		*out++ = *in++;
	uint32 ctx = ((uint32)out[-1]) | (((uint32)out[-2]) << 8) | (((uint32)out[-3]) << 16) | (((uint32)out[-4]) << 24);
	while(in < in_end && out < out_end) {
		uint32 idx = (ctx >> 15 ^ ctx ^ ctx >> 3) & ((int32)(1 << LZP_DICTIONARY) - 1);
		int32 val = lut[idx];
		lut[idx] = (int32)(out - outs);
		if(*in == MATCH && val > 0) {
			in++;
			if(*in != 255) {
				int32 len = LZP_MIN_MATCH;
				while(1) {
					if(in == in_end)  
						return -1;
					len += *in;
				...
				const uint8 * ref = outs + val;
				const uint8 * oe = out + len;
				if(oe > out_end)  
					oe = out_end;
				while(out < oe)  
					*out++ = *ref++;
				ctx = ((uint32)out[-1]) | (((uint32)out[-2]) << 8) | (((uint32)out[-3]) << 16) | (((uint32)out[-4]) << 24);
			}
			else {
				in++;
				ctx = (ctx << 8) | (*out++ = MATCH);
...
static int32 lzp_compress(const uint8 * _RESTRICT in, uint8 * _RESTRICT out, int32 n, int32 * _RESTRICT lut) 
{
	if(n < LZP_MIN_MATCH + 32)  
		return -1;
	memzero(lut, sizeof(int32) * (1 << LZP_DICTIONARY));
	return lzp_encode_block(in, in + n, out, out + n, lut);
}

static int32 lzp_decompress(const uint8 * _RESTRICT in, uint8 * _RESTRICT out, int32 n, int32 max, int32 * _RESTRICT lut) 
{
	if(n < 4)  
		return -1;
	memzero(lut, sizeof(int32) * (1 << LZP_DICTIONARY));
...
static int32 mrlec(uint8 * in, int32 inlen, uint8 * out) 
{
	uint8 * ip = in;
	uint8 * in_end = in + inlen;
	int32 op = 0;
	int32 c, pc = -1;
	int32 t[256] = { 0 };
	int32 run = 0;
	...
	do {
		...
		if(c == pc)
			...
			out[op++] = static_cast<uint8>(pc);
			for(; run > 255; run -= 255)  
				out[op++] = 255;
			out[op++] = static_cast<uint8>(run - 1);
			run = 1;
		}
		else
			for(++run; run > 1; --run)  
...
static int mrled(uint8 * _RESTRICT in, uint8 * _RESTRICT out, int32 outlen, int32 maxin) 
{
	int32 op = 0, ip = 0;
	int32 c, pc = -1;
	int32 t[256] = { 0 };
	...
	for(int32 i = 0; i < size; i++) {
		...
		while(ctx < 256) {
			...
			if(c & 128) {
				...
				low += (((uint64)(high - low) * (ssep * 3 + p)) >> 18) + 1;
...
const char * bz3_strerror(struct bz3_state * state) 
{
	switch(state->last_error) {
		case BZ3_OK: return "No error";
		case BZ3_ERR_OUT_OF_BOUNDS: return "Data index out of bounds";
		case BZ3_ERR_BWT: return "Burrows-Wheeler transform failed";
		case BZ3_ERR_CRC: return "CRC32 check failed";
		case BZ3_ERR_MALFORMED_HEADER: return "Malformed header";
		case BZ3_ERR_TRUNCATED_DATA: return "Truncated data";
		case BZ3_ERR_DATA_TOO_BIG: return "Too much data";
		default: return "Unknown error";
	}
}

struct bz3_state * bz3_new(int32 block_size) 
{
	if(block_size < SKILOBYTE(65) || block_size > SMEGABYTE(511)) {
		return NULL;
	}
	struct bz3_state * bz3_state = (struct bz3_state *)SAlloc::M(sizeof(struct bz3_state));
	if(!bz3_state) {
		return NULL;
	}
	bz3_state->cm_state = (state *)SAlloc::M(sizeof(state));
	bz3_state->swap_buffer = (uint8 *)SAlloc::M(bz3_bound(block_size));
	bz3_state->sais_array = (int32 *)SAlloc::M(BWT_BOUND(block_size) * sizeof(int32));
	memzero(bz3_state->sais_array, sizeof(int32) * BWT_BOUND(block_size));
	bz3_state->lzp_lut = (int32 *)SAlloc::C(1 << LZP_DICTIONARY, sizeof(int32));
	if(!bz3_state->cm_state || !bz3_state->swap_buffer || !bz3_state->sais_array || !bz3_state->lzp_lut) {
		if(bz3_state->cm_state)  SAlloc::F(bz3_state->cm_state);
		if(bz3_state->swap_buffer)  SAlloc::F(bz3_state->swap_buffer);
		if(bz3_state->sais_array)  SAlloc::F(bz3_state->sais_array);
		if(bz3_state->lzp_lut)  SAlloc::F(bz3_state->lzp_lut);
		SAlloc::F(bz3_state);
		return NULL;
	}
...
{
	SAlloc::F(state->swap_buffer);
	SAlloc::F(state->sais_array);
	SAlloc::F(state->cm_state);
	SAlloc::F(state->lzp_lut);
	SAlloc::F(state);
}
...
int32 bz3_encode_block(struct bz3_state * state, uint8 * buffer, int32 data_size) 
{
	uint8 * b1 = buffer, * b2 = state->swap_buffer;
	if(data_size > state->block_size) {
		state->last_error = BZ3_ERR_DATA_TOO_BIG;
		return -1;
	}
	uint32 crc32 = crc32sum(1, b1, data_size);
	...
	int32 bwt_idx = libsais_bwt(b1, b2, (int32_t *)state->sais_array, data_size, 0, NULL);
	if(bwt_idx < 0) {
	...
	int32 overhead = 2;       // CRC32 + BWT index
	...
	int32 p = 0;
	if(model & 2)  
		write_neutral_s32(b1 + 9 + 4 * p++, lzp_size);
	if(model & 4)  
		write_neutral_s32(b1 + 9 + 4 * p++, rle_size);
...
int32 bz3_decode_block(struct bz3_state * state, uint8 * buffer, int32 data_size, int32 orig_size) 
{
	// Read the header.
	uint32 crc32 = read_neutral_s32(buffer);
	int32  bwt_idx = read_neutral_s32(buffer + 4);
	if(data_size > static_cast<int32>(bz3_bound(state->block_size)) || data_size < 0) {
	...
	int32  p = 0;
	if(model & 2)  
		lzp_size = read_neutral_s32(buffer + 9 + 4 * p++);
	if(model & 4)  
		rle_size = read_neutral_s32(buffer + 9 + 4 * p++);
	p += 2;
	data_size -= p * 4 + 1;
	if(((model & 2) && (lzp_size > static_cast<int32>(bz3_bound(state->block_size)) || lzp_size < 0)) || 
		((model & 4) && (rle_size > static_cast<int32>(bz3_bound(state->block_size)) || rle_size < 0))) {
		state->last_error = BZ3_ERR_MALFORMED_HEADER;
		return -1;
	}
	if(orig_size > static_cast<int32>(bz3_bound(state->block_size)) || orig_size < 0) {
		state->last_error = BZ3_ERR_MALFORMED_HEADER;
		return -1;
	}
	// Decode the data.
	uint8 * b1 = buffer;
	uint8 * b2 = state->swap_buffer;
	begin(state->cm_state);
	...
	// Undo BWT
	memzero(state->sais_array, sizeof(int32) * BWT_BOUND(state->block_size));
	memzero(b2, size_src);
	if(libsais_unbwt(b1, b2, (int32_t *)state->sais_array, size_src, NULL, bwt_idx) < 0) {
		state->last_error = BZ3_ERR_BWT;
		return -1;
	}
	swap(b1, b2);
...
void bz3_encode_blocks(struct bz3_state * states[], uint8 * buffers[], int32 sizes[], int32 n) 
{
...
void bz3_decode_blocks(struct bz3_state * states[], uint8 * buffers[], int32 sizes[], int32 orig_sizes[], int32 n) 
{
...
//
int bz3_compress(uint32 block_size, const uint8 * const in, uint8 * out, size_t in_size, size_t * out_size) 
{
	if(block_size > in_size)  
		block_size = (in_size + 16);
	block_size = (block_size <= SKILOBYTE(65)) ? SKILOBYTE(65) : block_size;
	struct bz3_state * state = bz3_new(block_size);
	if(!state)  
		return BZ3_ERR_INIT;
	uint8 * compression_buf = (uint8 *)SAlloc::M(block_size);
	if(!compression_buf) {
		bz3_free(state);
		return BZ3_ERR_INIT;
	}
	...
	*out_size = 0;
	uint32 n_blocks = in_size / block_size;
...
int bz3_decompress(const uint8 * in, uint8 * out, size_t in_size, size_t * out_size) 
{
	if(in_size < 13)  
	...
	const uint32 block_size = read_neutral_s32(in + 5);
	const uint32 n_blocks = read_neutral_s32(in + 9);
	in_size -= 13;
	in += 13;
	struct bz3_state * state = bz3_new(block_size);
	if(!state)  
		return BZ3_ERR_INIT;
	uint8 * compression_buf = (uint8 *)SAlloc::M(bz3_bound(block_size));
	if(!compression_buf) {
		bz3_free(state);
		return BZ3_ERR_INIT;
	}
...
malformed_header:
		...
		int32 size = read_neutral_s32(in);
		if(size < 0 || size > static_cast<int32>(block_size))
			goto malformed_header;
		if(in_size < static_cast<size_t>(size + 8)) {
			bz3_free(state);
			SAlloc::F(compression_buf);
			return BZ3_ERR_TRUNCATED_DATA;
		}
		{
			const int32 orig_size = read_neutral_s32(in + 4);
			if(orig_size < 0)
				goto malformed_header;
			if((*out_size + orig_size) > buf_max) {
				bz3_free(state);
				SAlloc::F(compression_buf);
				return BZ3_ERR_DATA_TOO_BIG;
			}
