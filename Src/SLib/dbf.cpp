...
int DbfRecord::get(int fldN, DBFDate * data) const
{
	...
	if(!getFldBuf(fldN, tmp, sizeof(tmp), &f)) {
		...
		//
		// @v11.4.5 Так как некоторые долбоклюи вносят дату в dbf-таблицу в строчном формате, то приходится делать
		// защиту от таких придурков. Важно: сейчас предполагается, что дата представлена в текстовом формате GERMAN, но в общем случае
...
int FASTCALL DbfTable::updateRec(const DbfRecord * pRec)
{
	int    ok = 1;
	long   p = ftell(Stream);
...
int FASTCALL DbfTable::appendRec(const DbfRecord * pRec)
{
	int    ok = 1;
