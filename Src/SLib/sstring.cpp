...
const wchar_t * SStringU::SearchCharPos(size_t startPos, wchar_t c, size_t * pPos) const
{
	size_t pos = 0;
	const  wchar_t * p = 0;
	if(L > (startPos+1)) {
		p = static_cast<const wchar_t *>(wmemchr(P_Buf+startPos, c, Len()-startPos));
		if(p)
			pos = static_cast<size_t>(p - P_Buf);
	}
	ASSIGN_PTR(pPos, pos);
...
const wchar_t * SStringU::SearchChar(wchar_t c, size_t * pPos) const { return SearchCharPos(0, c, pPos); }
		...
		if(flags & npfCompensateDotDot) {
			...
			if(rNormalizedPath.Search(dotdot_pattern, 0, 1, 0)) {
				...
				while(rNormalizedPath.Search(dotdot_pattern, start_pos, 1, &p)) {
					const  char next_chr = rNormalizedPath.C(p+3);
					if(p > 0 && oneof2(next_chr, 0, divider) && rNormalizedPath.C(p-1) != divider) { // @v12.7.4 @fix (p>3)-->(p > 0)
						size_t local_start_pos = 0; 
						size_t i = p; // @v12.7.4 @fix (p-3)-->(p)
						if(i) {
							do {
