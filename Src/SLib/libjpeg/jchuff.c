 ...
 */
	...
	#define ASSIGN_STATE(dest, src)  ((dest) = (src))
		...
		#define ASSIGN_STATE(dest, src) ((dest).put_buffer = (src).put_buffer, (dest).put_bits = (src).put_bits, \
			(dest).last_dc_val[0] = (src).last_dc_val[0], (dest).last_dc_val[1] = (src).last_dc_val[1], \
			(dest).last_dc_val[2] = (src).last_dc_val[2], (dest).last_dc_val[3] = (src).last_dc_val[3])
	#endif
#endif
...
// 
	...
	#define IRIGHT_SHIFT(x, shft)  ((ishift_temp = (x)) < 0 ? (ishift_temp >> (shft)) | ((~0) << (16-(shft))) : (ishift_temp >> (shft)))
#else
	#define ISHIFT_TEMPS
 ...
 */
static void jpeg_make_c_derived_tbl(j_compress_ptr cinfo, boolean isDC, int tblno, c_derived_tbl **pdtbl)
{
	JHUFF_TBL * htbl;
	c_derived_tbl * dtbl;
...
#define emit_byte_s(state, val, action) { *(state)->next_output_byte++ = (JOCTET)(val); if(--(state)->free_in_buffer == 0) if(!dump_buffer_s(state)) { action; }}
// Emit a byte 
#define emit_byte_e(entropy, val) { *(entropy)->next_output_byte++ = (JOCTET)(val); if(--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); }

// Empty the output buffer; return TRUE if successful, FALSE if must suspend 
LOCAL(boolean) FASTCALL dump_buffer_s(working_state * state)
{
...
//
static void FASTCALL dump_buffer_e(huff_entropy_ptr entropy)
{
...
LOCAL(boolean) FASTCALL flush_bits_s(working_state * state)
{
...
static void FASTCALL flush_bits_e(huff_entropy_ptr entropy)
{
...
// 
INLINE static void emit_dc_symbol(huff_entropy_ptr entropy, int tbl_no, int symbol)
{
	if(entropy->gather_statistics)
...
INLINE static void emit_ac_symbol(huff_entropy_ptr entropy, int tbl_no, int symbol)
{
	if(entropy->gather_statistics)
...
// 
static void FASTCALL emit_buffered_bits(huff_entropy_ptr entropy, char * bufstart, uint nbits)
{
	if(!entropy->gather_statistics) {
		while(nbits > 0) {
 ...
 */
static void FASTCALL emit_eobrun(huff_entropy_ptr entropy)
{
	int temp, nbits;
 ...
 */
LOCAL(boolean) emit_restart_s(working_state * state, int restart_num)
{
	int ci;
...
static void FASTCALL emit_restart_e(huff_entropy_ptr entropy, int restart_num)
{
	int ci;
	emit_eobrun(entropy);
	if(!entropy->gather_statistics) {
...
METHODDEF(boolean) encode_mcu_AC_refine(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
	...
	for(k = cinfo->Ss; k <= Se; k++) {
		...
		emit_eobrun(entropy); // Emit any pending EOBRUN and the BE correction bits 
		emit_ac_symbol(entropy, entropy->ac_tbl_no, (r << 4) + 1); // Count/emit Huffman symbol for run length / number of bits 
...
LOCAL(boolean) encode_one_block(working_state * state, const JCOEFPTR block, int last_dc_val, const c_derived_tbl *dctbl, const c_derived_tbl *actbl)
{
	...
	const uint8 * natural_order = state->cinfo->natural_order; // @sobolev int-->uint8
...
static void htest_one_block(j_compress_ptr cinfo, const JCOEFPTR block, int last_dc_val, long dc_counts[], long ac_counts[])
{
	...
	int r, k;
	int Se = cinfo->lim_Se;
	const uint8 * natural_order = cinfo->natural_order; // @sobolev int-->uint8
	/* Encode the DC coefficient difference per section F.1.2.1 */
...
METHODDEF(boolean) encode_mcu_gather(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
	...
	for(blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
		ci = cinfo->MCU_membership[blkn];
		compptr = cinfo->cur_comp_info[ci];
		htest_one_block(cinfo, MCU_data[blkn][0], entropy->saved.last_dc_val[ci], entropy->dc_count_ptrs[compptr->dc_tbl_no], entropy->ac_count_ptrs[compptr->ac_tbl_no]);
...
METHODDEF(void) start_pass_huff(j_compress_ptr cinfo, boolean gather_statistics)
{
	...
	if(cinfo->progressive_mode) {
		...
		if(cinfo->Ah == 0) {
			entropy->pub.encode_mcu = (cinfo->Ss == 0) ? encode_mcu_DC_first : encode_mcu_AC_first;
		}
		else {
			if(cinfo->Ss == 0)
		...
		entropy->pub.encode_mcu = gather_statistics ? encode_mcu_gather : encode_mcu_huff;
	for(ci = 0; ci < cinfo->comps_in_scan; ci++) {
		compptr = cinfo->cur_comp_info[ci];
		// DC needs no table for refinement scan 
		if(cinfo->Ss == 0 && cinfo->Ah == 0) {
			...
			if(gather_statistics) {
				...
				jpeg_make_c_derived_tbl(cinfo, TRUE, tbl, &entropy->dc_derived_tbls[tbl]);
			}
			entropy->saved.last_dc_val[ci] = 0; // Initialize DC predictions to 0 
		}
		/* AC needs no table when not present */
		if(cinfo->Se) {
			tbl = compptr->ac_tbl_no;
