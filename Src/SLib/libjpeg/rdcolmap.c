...
static void read_gif_map(j_decompress_ptr cinfo, FILE * infile)
{
	...
	for(i = 0; i < colormaplen; i++) {
		...
		if(R == EOF || G == EOF || B == EOF)
			ERREXIT(cinfo, JERR_BAD_CMAP_FILE);
		add_map_entry(cinfo, R << (BITS_IN_JSAMPLE-8), G << (BITS_IN_JSAMPLE-8), B << (BITS_IN_JSAMPLE-8));
	}
}
...
static void read_ppm_map(j_decompress_ptr cinfo, FILE * infile)
{
	...
	uint maxval = read_pbm_integer(cinfo, infile);
...
void  read_color_map(j_decompress_ptr cinfo, FILE * infile)
{
	...
	cinfo->colormap = (*cinfo->mem->alloc_sarray)(reinterpret_cast<j_common_ptr>(cinfo), JPOOL_IMAGE, (JDIMENSION)(MAXJSAMPLE+1), (JDIMENSION)3);
