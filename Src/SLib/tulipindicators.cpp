...
SLTEST_R(TulipIndicators)
{
	...
	public:
		static int ReadTestSet(SFile & rF, SString & rFuncIdent, RealArray & rArgList, TSCollection <RealArray> & rDataSet)
		{
			...
			int    ok = -1;
			int    rlr = 0;
			uint   _line_count = 0; // Количество считанных строк без учета комментариев и пустых строк
			uint   _real_line_count = 0; // Физическое количество считанных строк (хоть комментарий, хоть что)
			SString line_buf;
			bool   prev_blanc = true;
			for(; (rlr = rF.ReadLine(line_buf, SFile::rlfChomp|SFile::rlfStrip)) != 0; _real_line_count++) {
				if(!line_buf.HasPrefix("#")) { // Комментарии просто игнорируются (они не значимы как пустые строки, которые выступают в роли разделителей)
