...
static void run(uint32_t outlen, char * pwd, size_t pwdlen, char * salt, uint32_t t_cost,
    uint32_t m_cost, uint32_t lanes, uint32_t threads, argon2_type type, int encoded_only, int raw_only, uint32_t version) 
{
	clock_t stop_time;
	...
	out = static_cast<unsigned char *>(SAlloc::M(outlen + 1));
	if(!out) {
		clear_internal_memory(pwd, pwdlen);
		fatal("could not allocate memory for output");
	}
	encodedlen = argon2_encodedlen(t_cost, m_cost, lanes, (uint32_t)saltlen, outlen, type);
	encoded = static_cast<char *>(SAlloc::M(encodedlen + 1));
	if(!encoded) {
		clear_internal_memory(pwd, pwdlen);
		fatal("could not allocate memory for hash");
	}
	result = argon2_hash(t_cost, m_cost, threads, pwd, pwdlen, salt, saltlen, out, outlen, encoded, encodedlen, type, version);
	if(result != ARGON2_OK)
...
int main_Argon2_Run(int argc, char * argv[]) 
{
	...
	for(i = 2; i < argc; i++) {
		...
		if(!strcmp(a, "-h")) {
			...
			if(i < argc - 1) {
				...
				m_cost = MIN(UINT64_C(1) << input, UINT32_C(0xFFFFFFFF));
			...
			if(i < argc - 1) {
				...
				m_cost = MIN(input, UINT32_C(0xFFFFFFFF));
				if(m_cost > ARGON2_MAX_MEMORY) {
					fatal("m_cost overflow");
				}
				continue;
