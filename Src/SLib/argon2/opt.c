 ...
 *
 * You should have received a copy of both of these licenses along with this . If not, they may be obtained at the above URLs.
 */
...
#endif
	memzero(zero_block, sizeof(zero_block));
	memzero(zero2_block, sizeof(zero2_block));
	input_block->v[6]++; /*Increasing index counter*/
	fill_block(zero_block, input_block, address_block, 0); /*First iteration of G*/
	fill_block(zero2_block, address_block, address_block, 0); /*Second iteration of G*/
}
