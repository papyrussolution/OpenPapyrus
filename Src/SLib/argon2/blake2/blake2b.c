...
{
	const uint8 * pin = (const uint8 *)in;
	if(!inlen) {
		return 0;
	}
	/* Sanity check */
...
{
	uint8 * out = (uint8 *)pout;
	blake2b_state blake_state;
	uint8 outlen_bytes[sizeof(uint32)] = {0};
	int ret = -1;
...
#define TRY(statement)    \
	...
	if(outlen <= BLAKE2B_OUTBYTES) {
		...
		uint32 toproduce;
		uint8  out_buffer[BLAKE2B_OUTBYTES];
		uint8  in_buffer[BLAKE2B_OUTBYTES];
