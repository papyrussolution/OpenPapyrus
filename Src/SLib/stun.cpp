 ...
 *
 * Based on project Minimalistic STUN client "ministun" https://code.google.com/p/ministun/
 *
...
static const struct StunSrv StunSrvList[241] = {
	{"23.21.150.121",       3478},
...
static void stun_req_id(struct stun_header * req)
{
	...
	register char x = 20;
	do {
		uint32 s_elm = (uint32)S_block[(uint8)randfiller];
		randfiller = (randfiller << 5) | (randfiller >> (64 - 5));
		randfiller += s_elm ^ x;
		req->id[x&3] += (uint)(randfiller ^ (randfiller >> 19));
	} while(--x);
	req->id[0] = STUN_XORMAGIC; // Set magic for RFC5389
}
//
...
// 
static int stun_handle_packet(int s, struct sockaddr_in * src, uchar * data, size_t len, void * arg)
{
...
static int StunRequest2(int sock, struct sockaddr_in * server, struct sockaddr_in * mapped) 
{
	...
	uchar reply_buf[1024];
	fd_set rfds;
	struct timeval to = { /*STUN_TIMEOUT*/3, 0 };
	...
	if(res <= 0)
		return -12;
	memzero(mapped, sizeof(struct sockaddr_in));
...
//
// Input: two random values (pos, step) for generate uniuqe way over server list
// Output: populate struct struct mapped
// Retval:
// bits 0-7 = STUN tokens set, 8-32 = attempt number
//
int GetExternalIPbySTUN(uint64 rnd, struct sockaddr_in * mapped, const char ** srv) 
{
	const int StunSrvListQty = 241; // Must be PRIME!!!!!
	randfiller = rnd;
	uint16 pos  = (uint16)rnd;
	uint16 step;
	do {
		rnd = (rnd >> 8) | 0xff00000000000000LL;
		step = rnd % StunSrvListQty;
	} while(step == 0);
	// runs in 8 birs offset, for keep flags in low byte
	for(int attempt = 256; attempt < StunSrvListQty * 2 * 256; attempt += 256) {
