...
static int decompress_file_internal(FILE* f_in, FILE* f_out, LZ4F_dctx* dctx,
    void* src, size_t srcCapacity, size_t filled, size_t alreadyConsumed, void* dst, size_t dstCapacity)
{
	int firstChunk = 1;
	size_t ret = 1;
...
static int decompress_file_allocDst(FILE* f_in, FILE* f_out, LZ4F_dctx* dctx, void* src, size_t srcCapacity)
{
	...
	int const decompressionResult = decompress_file_internal(f_in, f_out, dctx,
		src, srcCapacity, readSize-consumedSize, consumedSize, dst, dstCapacity);
	free(dst);
...
int main(int argc, const char ** argv) 
{
	...
	/* compress */
	{   
		FILE* const f_in = fopen(inpFilename, "rb");
	    FILE* const f_out = fopen(lz4Filename, "wb");
	    printf("compress : %s -> %s\n", inpFilename, lz4Filename);
		compressResult_t const ret = 0;
	    //ret = compress_file(f_in, f_out);
		//static compressResult_t compress_file(FILE* f_in, FILE* f_out)
		{
			...
			if(!LZ4F_isError(ctxCreation) && src && outbuff) {
				//ret = compress_file_internal(f_in, f_out, ctx, src, IN_CHUNK_SIZE, outbuff, outbufCapacity);
