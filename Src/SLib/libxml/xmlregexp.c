...
{
	if(oneof6(type1, XML_REGEXP_EPSILON, XML_REGEXP_CHARVAL, XML_REGEXP_RANGES, XML_REGEXP_SUBREG, XML_REGEXP_STRING, XML_REGEXP_ANYCHAR))
		return 1;
	if(oneof6(type2, XML_REGEXP_EPSILON, XML_REGEXP_CHARVAL, XML_REGEXP_RANGES, XML_REGEXP_SUBREG, XML_REGEXP_STRING, XML_REGEXP_ANYCHAR))
		return 1;
	if(type1 == type2) 
		return 1;
	// simplify subsequent compares by making sure type1 < type2 
	SExchangeForOrder(&type1, &type2);
	/*if(type1 > type2) {
	...
	switch(type1) {
		case XML_REGEXP_ANYSPACE: /* \s */
		    ...
		    if((type2 == XML_REGEXP_NOTSPACE) || ((type2 >= XML_REGEXP_LETTER) &&
			    (type2 <= XML_REGEXP_LETTER_OTHERS)) || ((type2 >= XML_REGEXP_NUMBER) &&
			    (type2 <= XML_REGEXP_NUMBER_OTHERS)) || ((type2 >= XML_REGEXP_MARK) &&
			    (type2 <= XML_REGEXP_MARK_ENCLOSING)) || ((type2 >= XML_REGEXP_PUNCT) &&
			    (type2 <= XML_REGEXP_PUNCT_OTHERS)) || ((type2 >= XML_REGEXP_SYMBOL) &&
		...
		case XML_REGEXP_INITNAME: /* \l */
		    ...
		    if((type2 == XML_REGEXP_NOTINITNAME) || ((type2 >= XML_REGEXP_NUMBER) &&
			    (type2 <= XML_REGEXP_NUMBER_OTHERS)) || ((type2 >= XML_REGEXP_MARK) &&
			    (type2 <= XML_REGEXP_MARK_ENCLOSING)) || ((type2 >= XML_REGEXP_SEPAR) &&
			    (type2 <= XML_REGEXP_SEPAR_PARA)) || ((type2 >= XML_REGEXP_PUNCT) &&
			    (type2 <= XML_REGEXP_PUNCT_OTHERS)) || ((type2 >= XML_REGEXP_SYMBOL) &&
			    (type2 <= XML_REGEXP_SYMBOL_OTHERS)) || ((type2 >= XML_REGEXP_OTHER) &&
		...
		case XML_REGEXP_NAMECHAR: /* \c */
		    ...
		    if((type2 == XML_REGEXP_NOTNAMECHAR) || ((type2 >= XML_REGEXP_MARK) && (type2 <= XML_REGEXP_MARK_ENCLOSING)) ||
...
static int FASTCALL xmlRegCheckCharacterRange(xmlRegAtomType type, int codepoint, int neg, int start, int end, const xmlChar * blockName) 
{
	...
	switch(type) {
		...
		case XML_REGEXP_NAMECHAR:
		    ret = (IS_LETTER(codepoint) || isdec(codepoint) || (codepoint == '.') || (codepoint == '-') ||
...
static void xmlFAParseCharClassEsc(xmlRegParserCtxt * ctxt)
{
	...
	if(CUR == '.') {
		if(ctxt->atom == NULL) {
			...
			xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_ANYCHAR, 0, 0, 0);
		}
		NEXT;
		return;
	}
