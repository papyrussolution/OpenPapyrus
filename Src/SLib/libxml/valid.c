// valid.c
// part of the code use to do the DTD handling and the validity checking
 ...
 */
static void xmlErrValidNode(xmlValidCtxtPtr ctxt, xmlNode * pNode, xmlParserErrors error, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
	...
	__xmlRaiseError(schannel, channel, data, pctxt, pNode, XML_FROM_VALID, error, XML_ERR_ERROR, NULL, 0,
...
void FASTCALL xmlFreeDocElementContent(xmlDoc * pDoc, xmlElementContent * pCur) 
{
	if(pCur) {
		...
		while(pCur) {
			...
			XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pCur->name));
			XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pCur->prefix));
			SAlloc::F(pCur);
			pCur = next;
		}
	}
...
xmlElement * xmlAddElementDecl(xmlValidCtxtPtr ctxt, xmlDtd * dtd, const xmlChar * name, xmlElementTypeVal type, xmlElementContent * content)
{
	...
	if(ctxt && oneof2(ctxt->finishDtd, XML_CTXT_FINISH_DTD_0, XML_CTXT_FINISH_DTD_1)) {
		ret->content = content;
		if(content)
			content->parent = reinterpret_cast<xmlElementContent *>(1);
	}
	else
 ...
 */
static xmlElement * xmlCopyElement(const xmlElement * elem) 
{
...
{
	return static_cast<xmlElementTable *>(xmlHashCopy(table, (xmlHashCopier)xmlCopyElement));
}
...
{
	if(buf && table)
		xmlHashScan(table, reinterpret_cast<xmlHashScanner>(xmlDumpElementDeclScan), buf);
}
...
{
	xmlEnumeration * ret = cur ? xmlCreateEnumeration(cur->name) : 0;
	if(ret) 
...
static int xmlScanIDAttributeDecl(xmlValidCtxtPtr ctxt, xmlElement * elem, int err) 
{
	int ret = 0;
	if(elem) {
		for(const xmlAttribute * cur = elem->attributes; cur; cur = cur->nexth) {
...
static void xmlFreeAttribute(xmlAttribute * pAttr)
{
	if(pAttr) {
		...
		XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pAttr->elem)); // @badcast
		XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pAttr->name)); // @badcast
		XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pAttr->prefix)); // @badcast
		XmlDestroyStringWithDict(dict, const_cast<xmlChar *>(pAttr->defaultValue)); // @badcast
		SAlloc::F(pAttr);
	}
}
    ...
    const xmlChar * name, const xmlChar * ns, xmlAttributeType type, xmlAttributeDefault def, const xmlChar * defaultValue, xmlEnumeration * tree) 
{
	 ...
	 */
	switch(type) {
		case XML_ATTRIBUTE_CDATA: break;
		case XML_ATTRIBUTE_ID: break;
		case XML_ATTRIBUTE_IDREF: break;
		case XML_ATTRIBUTE_IDREFS: break;
		case XML_ATTRIBUTE_ENTITY: break;
		case XML_ATTRIBUTE_ENTITIES: break;
		case XML_ATTRIBUTE_NMTOKEN: break;
		case XML_ATTRIBUTE_NMTOKENS: break;
		case XML_ATTRIBUTE_ENUMERATION: break;
		case XML_ATTRIBUTE_NOTATION: break;
		default:
	...
	if(dtd->doc && (dtd->doc->extSubset == dtd) && dtd->doc->intSubset && dtd->doc->intSubset->attributes) {
		ret = static_cast<xmlAttribute *>(xmlHashLookup3(static_cast<xmlHashTable *>(dtd->doc->intSubset->attributes), name, ns, elem));
		if(ret) {
			xmlFreeEnumeration(tree);
			return 0;
		}
	...
	/*
	 * Validity Check: Multiple ID per element
	 */
	...
	if(elemDef) {
		 ...
		 */
		if((sstreq(ret->name, reinterpret_cast<const xmlChar *>("xmlns"))) || ((ret->prefix && (sstreq(ret->prefix, "xmlns"))))) {
			ret->nexth = elemDef->attributes;
			elemDef->attributes = ret;
		}
		else {
			xmlAttribute * tmp = elemDef->attributes;
			while(tmp && ((sstreq(tmp->name, reinterpret_cast<const xmlChar *>("xmlns"))) || ((ret->prefix && (sstreq(ret->prefix, "xmlns")))))) {
				if(tmp->nexth == NULL)
					break;
				tmp = tmp->nexth;
			}
 ...
 */
static xmlAttribute * xmlCopyAttribute(const xmlAttribute * attr)
{
...
{
	return static_cast<xmlAttributeTable *>(xmlHashCopy(table, (xmlHashCopier)xmlCopyAttribute));
}
...
void xmlDumpAttributeDecl(xmlBuffer * buf, xmlAttribute * attr) 
{
	...
	switch(attr->atype) {
		case XML_ATTRIBUTE_CDATA: xmlBufferWriteChar(buf, " CDATA"); break;
		case XML_ATTRIBUTE_ID: xmlBufferWriteChar(buf, " ID"); break;
		case XML_ATTRIBUTE_IDREF: xmlBufferWriteChar(buf, " IDREF"); break;
		case XML_ATTRIBUTE_IDREFS: xmlBufferWriteChar(buf, " IDREFS"); break;
		case XML_ATTRIBUTE_ENTITY: xmlBufferWriteChar(buf, " ENTITY"); break;
		case XML_ATTRIBUTE_ENTITIES: xmlBufferWriteChar(buf, " ENTITIES"); break;
		case XML_ATTRIBUTE_NMTOKEN: xmlBufferWriteChar(buf, " NMTOKEN"); break;
		case XML_ATTRIBUTE_NMTOKENS: xmlBufferWriteChar(buf, " NMTOKENS"); break;
	...
	switch(attr->def) {
		...
		case XML_ATTRIBUTE_REQUIRED: xmlBufferWriteChar(buf, " #REQUIRED"); break;
		case XML_ATTRIBUTE_IMPLIED: xmlBufferWriteChar(buf, " #IMPLIED"); break;
		case XML_ATTRIBUTE_FIXED: xmlBufferWriteChar(buf, " #FIXED"); break;
		default: xmlErrValid(NULL, XML_ERR_INTERNAL_ERROR, "Internal: ATTRIBUTE struct corrupted invalid def\n", 0);
	}
...
xmlNotation * xmlAddNotationDecl(xmlValidCtxtPtr ctxt, xmlDtd * dtd, const xmlChar * name, const xmlChar * PublicID, const xmlChar * SystemID) 
{
	 ...
	 */
	table = static_cast<xmlNotationTable *>(dtd->notations);
	if(table == NULL) {
		xmlDict * dict = dtd->doc ? dtd->doc->dict : 0;
 ...
 */
static xmlNotation * xmlCopyNotation(const xmlNotation * nota) 
{
...
{
	return ((xmlNotationTable *)xmlHashCopy(table, (xmlHashCopier)xmlCopyNotation));
}
...
int xmlIsID(xmlDoc * doc, xmlNode * elem, xmlAttr * attr)
{
	...
	if((doc->intSubset == NULL) && (doc->extSubset == NULL) && (doc->type != XML_HTML_DOCUMENT_NODE)) {
		...
		if((sstreq(reinterpret_cast<const xmlChar *>("id"), attr->name)) || ((sstreq(reinterpret_cast<const xmlChar *>("name"), attr->name)) && (!elem || (sstreq(elem->name, reinterpret_cast<const xmlChar *>("a"))))))
			return 1;
		return 0;
	}
	else if(!elem) {
...
int xmlRemoveRef(xmlDoc * doc, xmlAttr * attr)
{
	...
	if(!attr) 
		return -1;
	table = (xmlRefTable *)doc->refs;
	if(table == NULL)
		return -1;
	ID = xmlNodeListGetString(doc, attr->children, 1);
	if(ID == NULL)
		return -1;
	ref_list = static_cast<xmlList *>(xmlHashLookup(table, ID));
	if(ref_list == NULL) {
		SAlloc::F(ID);
		return -1;
	}
...
{
	xmlRefTable * table = doc ? static_cast<xmlRefTable *>(doc->refs) : 0;
	return (table && pID) ? static_cast<xmlList *>(xmlHashLookup(table, pID)) : 0;
}
//
...
xmlElement * FASTCALL xmlGetDtdElementDesc(xmlDtd * dtd, const xmlChar * name)
{
	...
	if(dtd && name && dtd->elements) {
		xmlElementTable * table = static_cast<xmlElementTable *>(dtd->elements);
		xmlChar * prefix = NULL;
		xmlChar * uqname = xmlSplitQName2(name, &prefix);
		if(uqname)
			name = uqname;
		cur = static_cast<xmlElement *>(xmlHashLookup2(table, name, prefix));
		SAlloc::F(prefix);
		SAlloc::F(uqname);
	}
	return cur;
...
static xmlElement * xmlGetDtdElementDesc2(xmlDtd * dtd, const xmlChar * name, int create)
{
	...
	if(dtd) {
		if(!dtd->elements) {
			xmlDict * dict = dtd->doc ? dtd->doc->dict : NULL;
			if(!create)
				return 0;
			/*
			 * Create the Element table if needed.
		...
		cur = static_cast<xmlElement *>(xmlHashLookup2(table, name, prefix));
		if(!cur && create) {
			cur = static_cast<xmlElement *>(SAlloc::M(sizeof(xmlElement)));
			if(!cur) {
				xmlVErrMemory_MallocFailed(0);
				return 0;
			}
...
{
	return (dtd && dtd->elements) ? static_cast<xmlElement *>(xmlHashLookup2(static_cast<xmlElementTable *>(dtd->elements), name, prefix)) : 0;
}
/**
 * xmlGetDtdAttrDesc:
 * @dtd:  a pointer to the DtD to search
...
xmlAttribute * xmlGetDtdAttrDesc(xmlDtd * dtd, const xmlChar * elem, const xmlChar * name)
{
	...
	if(dtd && dtd->attributes) {
		xmlAttributeTable * table = static_cast<xmlAttributeTable *>(dtd->attributes);
		if(table) {
			xmlChar * prefix = NULL;
			xmlChar * uqname = xmlSplitQName2(name, &prefix);
			if(uqname) {
				cur = static_cast<xmlAttribute *>(xmlHashLookup3(table, uqname, prefix, elem));
				SAlloc::F(prefix);
				SAlloc::F(uqname);
			}
			else
				cur = static_cast<xmlAttribute *>(xmlHashLookup3(table, name, NULL, elem));
		}
	}
	return cur;
}
...
xmlNotation * xmlGetDtdNotationDesc(xmlDtd * dtd, const xmlChar * name)
{
	...
	table = (xmlNotationTable *)dtd->notations;
...
static int xmlIsDocNameChar(xmlDoc * doc, int c) 
{
	if(!doc || (doc->properties & XML_DOC_OLD10) == 0) {
		...
		if((IS_LETTER(c)) || (isdec(c)) || (c == '.') || (c == '-') || (c == '_') || (c == ':') || (IS_COMBINING(c)) || (IS_EXTENDER(c)))
			return 1;
	}
	return 0;
}
...
int xmlValidateAttributeDecl(xmlValidCtxtPtr ctxt, xmlDoc * doc, xmlAttribute * attr) 
{
	...
	if(attr->atype == XML_ATTRIBUTE_ID) {
		...
		if(elem) {
			...
			nbId = 0;
			if(doc->intSubset) {
				table = (xmlAttributeTable *)doc->intSubset->attributes;
...
int xmlValidateOneAttribute(xmlValidCtxtPtr ctxt, xmlDoc * doc, xmlNode * elem, xmlAttr * attr, const xmlChar * value)
{
	...
	int ret = 1;
	CHECK_DTD;
	if((!elem || !elem->name) || (!attr || !attr->name))
		return 0;
	if(elem->ns && elem->ns->prefix) {
		xmlChar fn[50];
		xmlChar * fullname = xmlBuildQName(elem->name, elem->ns->prefix, fn, 50);
...
int xmlValidateOneNamespace(xmlValidCtxtPtr ctxt, xmlDoc * doc, xmlNode * elem, const xmlChar * prefix, xmlNs * ns, const xmlChar * value) 
{
	...
	if(prefix) {
		...
		if(ns->prefix) {
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, fullname, ns->prefix, reinterpret_cast<const xmlChar *>("xmlns"));
			if((attrDecl == NULL) && doc->extSubset)
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, fullname, ns->prefix, reinterpret_cast<const xmlChar *>("xmlns"));
		}
		else {
			attrDecl = xmlGetDtdAttrDesc(doc->intSubset, fullname, reinterpret_cast<const xmlChar *>("xmlns"));
			if((attrDecl == NULL) && doc->extSubset)
				attrDecl = xmlGetDtdAttrDesc(doc->extSubset, fullname, reinterpret_cast<const xmlChar *>("xmlns"));
		}
		if((fullname != fn) && (fullname != elem->name))
			SAlloc::F(fullname);
	}
	if(attrDecl == NULL) {
		if(ns->prefix) {
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elem->name, ns->prefix, reinterpret_cast<const xmlChar *>("xmlns"));
			if((attrDecl == NULL) && doc->extSubset)
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elem->name, ns->prefix, reinterpret_cast<const xmlChar *>("xmlns"));
		}
		else {
			attrDecl = xmlGetDtdAttrDesc(doc->intSubset, elem->name, reinterpret_cast<const xmlChar *>("xmlns"));
			if((attrDecl == NULL) && doc->extSubset)
				attrDecl = xmlGetDtdAttrDesc(doc->extSubset, elem->name, reinterpret_cast<const xmlChar *>("xmlns"));
		}
	}
	/* Validity Constraint: Attribute Value Type */
	if(attrDecl == NULL) {
	...
	if(ns->prefix) {
		...
		ret &= xmlValidateAttributeValue2(ctxt, doc, reinterpret_cast<const xmlChar *>("xmlns"), attrDecl->atype, value);
	}
	return ret;
}
...
cont:
	 ...
	 */
	if(CONT && ((CONT->parent == NULL) || (CONT->parent->type != XML_ELEMENT_CONTENT_OR)) && ((CONT->ocur == XML_ELEMENT_CONTENT_MULT) ||
		    (CONT->ocur == XML_ELEMENT_CONTENT_OPT) || ((CONT->ocur == XML_ELEMENT_CONTENT_PLUS) && (OCCURRENCE)))) {
		...
		case XML_ELEMENT_CONTENT_PCDATA:
			 ...
			 * skipping ignorable elems
			    ...
			    do {
				    ...
				    if(NODE && NODE->type == XML_ENTITY_REF_NODE)
					    return -2;
			    } while(NODE && ((NODE->type != XML_ELEMENT_NODE) && (NODE->type != XML_TEXT_NODE) && (NODE->type != XML_CDATA_SECTION_NODE)));
			    ret = 1;
			    break;
		    }
		    else {
			...
			while(cur) {
				switch(cur->type) {
					 ...
					 * and process within the entity
					     */
					    if(cur->children && cur->children->children) {
						    nodeVPush(ctxt, cur);
...
static int xmlValidateCheckMixed(xmlValidCtxtPtr ctxt, xmlElementContent * cont, const xmlChar * qname) 
{
	...
	if(!name) {
		...
		while(cont) {
			if(cont->type == XML_ELEMENT_CONTENT_ELEMENT) {
				if(cont->prefix && (xmlStrncmp(cont->prefix, qname, plen) == 0) && (sstreq(cont->name, name)))
					return 1;
			}
			else if((cont->type == XML_ELEMENT_CONTENT_OR) && cont->c1 && (cont->c1->type == XML_ELEMENT_CONTENT_ELEMENT)) {
				if(cont->c1->prefix && (xmlStrncmp(cont->c1->prefix, qname, plen) == 0) && (sstreq(cont->c1->name, name)))
					return 1;
			}
			else if((cont->type != XML_ELEMENT_CONTENT_OR) || (cont->c1 == NULL) || (cont->c1->type != XML_ELEMENT_CONTENT_PCDATA)) {
...
child_ok:
	...
	while(attr) {
		if(attr->def == XML_ATTRIBUTE_REQUIRED) {
			int qualified = -1;
			if((attr->prefix == NULL) && (sstreq(attr->name, reinterpret_cast<const xmlChar *>("xmlns")))) {
				xmlNs * ns = elem->nsDef;
				while(ns) {
					if(ns->prefix == NULL)
						goto found;
			...
			else if(sstreq(attr->prefix, reinterpret_cast<const xmlChar *>("xmlns"))) {
				xmlNs * ns = elem->nsDef;
				while(ns) {
...
static void xmlValidateRef(const xmlRef * ref, xmlValidCtxtPtr ctxt, const xmlChar * name) 
{
	...
	if(!attr) {
		...
		if(!dup)
			...
			for(xmlChar * cur = dup; *cur != 0;) {
				xmlChar * str = cur;
				while((*cur != 0) && (!IS_BLANK_CH(*cur))) 
					cur++;
				{
					const xmlChar save = *cur;
					*cur = 0;
...
int xmlValidateDtd(xmlValidCtxtPtr ctxt, xmlDoc * doc, xmlDtd * dtd)
{
	int ret = 0;
	if(dtd && doc) {
		xmlDtd * oldExt = doc->extSubset;
		xmlDtd * oldInt = doc->intSubset;
		doc->extSubset = dtd;
		doc->intSubset = NULL;
...
static void xmlValidateNotationCallback(xmlEntity * cur, xmlValidCtxtPtr ctxt, const xmlChar * name ATTRIBUTE_UNUSED) 
{
	if(cur) {
		if(cur->etype == XML_EXTERNAL_GENERAL_UNPARSED_ENTITY) {
			const xmlChar * notation = cur->content;
			if(notation) {
...
int xmlValidateDtdFinal(xmlValidCtxtPtr ctxt, xmlDoc * doc) 
{
	...
	if(!doc || !ctxt || (!doc->intSubset && !doc->extSubset)) 
		return 0;
	ctxt->doc = doc;
	ctxt->valid = 1;
	dtd = doc->intSubset;
...
int FASTCALL xmlValidGetPotentialChildren(xmlElementContent * ctree, const xmlChar ** names, int * len, int max) 
{
	...
	switch(ctree->type) {
		case XML_ELEMENT_CONTENT_PCDATA:
		    for(i = 0; i < *len; i++)
			    if(sstreq(reinterpret_cast<const xmlChar *>("#PCDATA"), names[i])) 
					return (*len);
		    names[(*len)++] = reinterpret_cast<const xmlChar *>("#PCDATA");
		    break;
		case XML_ELEMENT_CONTENT_ELEMENT:
		    for(i = 0; i < *len; i++)
...
int xmlValidGetValidElements(xmlNode * prev, xmlNode * next, const xmlChar ** names, int max)
{
	...
	if(!element_desc && parent->doc->extSubset)
