...
static void fill_strings(void) 
{
	...
	for(i = 0; seeds1[i] != NULL; i++) {
		strings1[i] = sstrdup((const xmlChar*)seeds1[i]);
		if(strings1[i] == NULL) {
	...
	for(i = 0; seeds2[i] != NULL; i++) {
		strings2[i] = sstrdup((const xmlChar*)seeds2[i]);
		if(strings2[i] == NULL) {
 ...
 */
static int run_test2(xmlDict * parent) 
{
	int i, j;
	xmlDict * dict;
	int ret = 0;
	...
	for(i = 0; i < NB_STRINGS_MIN; i++, j++) {
		test2[j] = xmlDictLookup(dict, strings2[j], sstrlen(strings2[j]));
		if(test2[j] == NULL) {
			fprintf(stderr, "Failed lookup for '%s'\n", strings2[j]);
			ret = 1;
			nbErrors++;
...
static int run_test1(void) 
{
	...
	/* ":foo" like strings1 */
	for(i = 0; i < NB_STRINGS_MIN; i++, j++) {
		test1[j] = xmlDictLookup(dict, strings1[j], sstrlen(strings1[j]));
		if(test1[j] == NULL) {
			fprintf(stderr, "Failed lookup for '%s'\n", strings1[j]);
			ret = 1;
			nbErrors++;
...
{
	int ret;
	LIBXML_TEST_VERSION fill_strings();
#ifdef WITH_PRINT
	print_strings();
#endif
	ret = run_test1();
