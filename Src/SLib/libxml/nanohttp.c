...
static int xmlNanoHTTPSend(xmlNanoHTTPCtxtPtr ctxt, const char * xmt_ptr, int outlen)
{
	...
	if((ctxt->state & XML_NANO_HTTP_WRITE) && (xmt_ptr != NULL)) {
		while(total_sent < outlen) {
			int nsent = send(ctxt->fd, SEND_ARG2_CAST(xmt_ptr + total_sent), outlen - total_sent, 0);
			if(nsent > 0)
				total_sent += nsent;
			else if((nsent == -1) &&
...
static SOCKET xmlNanoHTTPConnectAttempt(struct sockaddr * addr)
{
#ifndef HAVE_POLL_H
	fd_set wfd;
#ifdef _WINSOCKAPI_
...
#endif /* !HAVE_POLL_H */
	{
		XML_SOCKLEN_T len = sizeof(status);
#ifdef SO_ERROR
		if(getsockopt(s, SOL_SOCKET, SO_ERROR, (char*)&status, (int *)&len) < 0) {
			/* Solaris error code */
...
static SOCKET xmlNanoHTTPConnectHost(const char * host, int port)
{
	struct hostent * h;
