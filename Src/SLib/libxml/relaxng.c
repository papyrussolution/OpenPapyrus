...
#define IS_RELAXNG(_p_node_, typ) (_p_node_ && _p_node_->ns && (_p_node_->type == XML_ELEMENT_NODE) && (sstreq(_p_node_->name, typ)) && (sstreq(_p_node_->ns->href, xmlRelaxNGNs)))
...
struct xmlRelaxNGInterleaveGroup {
	xmlRelaxNGDefine * rule; /* the rule to satisfy */
	xmlRelaxNGDefine ** defs; /* the array of element definitions */
	xmlRelaxNGDefine ** attrs; /* the array of attributes definitions */
};
 ...
 */
static void FASTCALL xmlRngPErr(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node, int error, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	xmlStructuredErrorFunc schannel = NULL;
	xmlGenericErrorFunc channel = NULL;
	void * data = NULL;
...
static void xmlRelaxNGFreeDefine(xmlRelaxNGDefine * define);
 ...
 */
static void xmlRelaxNGFreeDocumentList(xmlRelaxNGDocument * docu)
{
	while(docu) {
 ...
 */
static void xmlRelaxNGFreeInclude(xmlRelaxNGInclude * incl)
{
	if(incl) {
		SAlloc::F(incl->href);
		if(incl->doc)
 ...
 * Deallocate a RelaxNG include structure.
 */
static void xmlRelaxNGFreeIncludeList(xmlRelaxNGInclude * incl)
{
	while(incl) {
		xmlRelaxNGInclude * next = incl->next;
 ...
 */
static xmlRelaxNGPtr xmlRelaxNGNewRelaxNG(xmlRelaxNGParserCtxt * ctxt)
{
...
static xmlRelaxNGDefine * FASTCALL xmlRelaxNGNewDefine(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	...
	if(ctxt->defMax == 0) {
		ctxt->defMax = 16;
		ctxt->defNr = 0;
		ctxt->defTab = (xmlRelaxNGDefine **)SAlloc::M(ctxt->defMax * sizeof(xmlRelaxNGDefine *));
		if(ctxt->defTab == NULL) {
...
static void xmlRelaxNGFreeDefine(xmlRelaxNGDefine * define)
{
	if(define) {
		if(define->type == XML_RELAXNG_VALUE && define->attrs) {
			xmlRelaxNGTypeLibrary * lib = static_cast<xmlRelaxNGTypeLibrary *>(define->data);
			if(lib && lib->freef)
 ...
 * Returns the newly allocated structure or NULL in case or error
 */
static xmlRelaxNGStates * xmlRelaxNGNewStates(xmlRelaxNGValidCtxtPtr ctxt, int size)
{
...
static void FASTCALL xmlRelaxNGFreeStates(xmlRelaxNGValidCtxt * ctxt, xmlRelaxNGStates * states)
{
	if(states) {
		if(ctxt && (ctxt->freeStates == NULL)) {
			...
			ctxt->freeStatesNr = 0;
			ctxt->freeStates = static_cast<xmlRelaxNGStates **>(SAlloc::M(ctxt->freeStatesMax * sizeof(xmlRelaxNGStates *)));
			if(ctxt->freeStates == NULL) {
				xmlRngVErrMemory(ctxt, "storing states\n");
			}
		}
		else if((ctxt) && (ctxt->freeStatesNr >= ctxt->freeStatesMax)) {
			xmlRelaxNGStates ** tmp = static_cast<xmlRelaxNGStates **>(SAlloc::R(ctxt->freeStates, 2 * ctxt->freeStatesMax * sizeof(xmlRelaxNGStates *)));
			if(!tmp) {
				xmlRngVErrMemory(ctxt, "storing states\n");
...
static xmlRelaxNGValidState * xmlRelaxNGNewValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlNode * P_Node)
{
	...
	if(nbAttrs > 0) {
		if(ret->attrs == NULL) {
			...
			ret->attrs = static_cast<xmlAttr **>(SAlloc::M(ret->maxAttrs * sizeof(xmlAttr*)));
			if(ret->attrs == NULL) {
				xmlRngVErrMemory(ctxt, "allocating states\n");
				return ret;
			}
			...
			xmlAttr ** tmp = static_cast<xmlAttr **>(SAlloc::R(ret->attrs, nbAttrs * sizeof(xmlAttr *)));
			if(!tmp) {
				xmlRngVErrMemory(ctxt, "allocating states\n");
				return ret;
			}
...
static xmlRelaxNGValidState * xmlRelaxNGCopyValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGValidState * state)
{
	...
	if(state->nbAttrs > 0) {
		if(ret->attrs == NULL) {
			...
			xmlAttr ** tmp = static_cast<xmlAttr **>(SAlloc::R(ret->attrs, state->maxAttrs * sizeof(xmlAttr *)));
			if(!tmp) {
				xmlRngVErrMemory(ctxt, "allocating states\n");
				ret->nbAttrs = 0;
				return ret;
...
// Document functions
//
static xmlDoc * xmlRelaxNGCleanupDoc(xmlRelaxNGParserCtxt * ctxt, xmlDoc * doc);
/**
 ...
 */
static int xmlRelaxNGIncludePush(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGInclude * value)
{
	if(!ctxt->incTab) {
		ctxt->incMax = 4;
		ctxt->incNr = 0;
		ctxt->incTab = static_cast<xmlRelaxNGInclude **>(SAlloc::M(ctxt->incMax * sizeof(ctxt->incTab[0])));
		if(ctxt->incTab == NULL) {
	...
	if(ctxt->incNr >= ctxt->incMax) {
		ctxt->incMax *= 2;
		ctxt->incTab = static_cast<xmlRelaxNGInclude **>(SAlloc::R(ctxt->incTab, ctxt->incMax * sizeof(ctxt->incTab[0])));
		if(ctxt->incTab == NULL) {
			xmlRngPErrMemory(ctxt, "allocating include\n");
			return 0;
		}
 ...
 */
static int xmlRelaxNGRemoveRedefine(xmlRelaxNGParserCtxt * ctxt, const xmlChar * URL ATTRIBUTE_UNUSED, xmlNode * target, const xmlChar * name)
{
	int found = 0;
	xmlNode * tmp;
	xmlNode * tmp2;
	...
	while(tmp) {
		tmp2 = tmp->next;
		if(!name && (IS_RELAXNG(tmp, "start"))) {
			found = 1;
			xmlUnlinkNode(tmp);
			xmlFreeNode(tmp);
		}
		else if(name && (IS_RELAXNG(tmp, "define"))) {
			name2 = xmlGetProp(tmp, reinterpret_cast<const xmlChar *>("name"));
			...
			xmlChar * href = NULL;
			xmlRelaxNGDocument * inc = (xmlRelaxNGDocument *)tmp->psvi;
			if(inc && inc->doc && inc->doc->children) {
				if(sstreq(inc->doc->children->name, "grammar")) {
#ifdef DEBUG_INCLUDE
					href = xmlGetProp(tmp, reinterpret_cast<const xmlChar *>("href"));
#endif
 ...
 */
static xmlRelaxNGInclude * xmlRelaxNGLoadInclude(xmlRelaxNGParserCtxt * ctxt, const xmlChar * URL, xmlNode * pNode, const xmlChar * ns)
{
	xmlRelaxNGInclude * ret = NULL;
	xmlDoc * doc;
	int i;
	xmlNode * root;
	xmlNode * cur;
	...
	if(!doc) {
		xmlRngPErr(ctxt, pNode, XML_RNGP_PARSE_ERROR, "xmlRelaxNG: could not load %s\n", URL, 0);
		return 0;
	}
#ifdef DEBUG_INCLUDE
	 ...
	 */
	ret = (xmlRelaxNGInclude *)SAlloc::M(sizeof(xmlRelaxNGInclude));
	if(!ret) {
		xmlRngPErrMemory(ctxt, "allocating include\n");
		xmlFreeDoc(doc);
		return 0;
	...
	if(ns) {
		...
		if(root) {
			if(xmlHasProp(root, reinterpret_cast<const xmlChar *>("ns")) == NULL) {
				xmlSetProp(root, reinterpret_cast<const xmlChar *>("ns"), ns);
			}
		}
	}
	/*
	...
	root = xmlDocGetRootElement(doc);
	if(root == NULL) {
		xmlRngPErr(ctxt, pNode, XML_RNGP_EMPTY, "xmlRelaxNG: included document is empty %s\n", URL, 0);
		return 0;
	}
	if(!IS_RELAXNG(root, "grammar")) {
		xmlRngPErr(ctxt, pNode, XML_RNGP_GRAMMAR_MISSING, "xmlRelaxNG: included document %s root is not a grammar\n", URL, 0);
		return 0;
	}
	/*
	 * Elimination of redefined rules in the include.
	 */
	for(cur = pNode->children; cur; cur = cur->next) {
		if(IS_RELAXNG(cur, "start")) {
			int found = xmlRelaxNGRemoveRedefine(ctxt, URL, root->children, 0);
			if(!found)
				xmlRngPErr(ctxt, pNode, XML_RNGP_START_MISSING, "xmlRelaxNG: include %s has a start but not the included grammar\n", URL, 0);
		}
		else if(IS_RELAXNG(cur, "define")) {
			xmlChar * name = xmlGetProp(cur, reinterpret_cast<const xmlChar *>("name"));
			if(!name)
				xmlRngPErr(ctxt, pNode, XML_RNGP_NAME_MISSING, "xmlRelaxNG: include %s has define without name\n", URL, 0);
			else {
				xmlRelaxNGNormExtSpace(name);
				int found = xmlRelaxNGRemoveRedefine(ctxt, URL, root->children, name);
				if(!found)
					xmlRngPErr(ctxt, pNode, XML_RNGP_DEFINE_MISSING, "xmlRelaxNG: include %s has a define %s but not the included grammar\n", URL, name);
				SAlloc::F(name);
			}
		}
	}
...
static int xmlRelaxNGValidErrorPush(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGValidErr err, const xmlChar * arg1, const xmlChar * arg2, int dup)
{
	...
	if(ctxt->errTab == NULL) {
		ctxt->errMax = 8;
		ctxt->errNr = 0;
		ctxt->errTab = static_cast<xmlRelaxNGValidError *>(SAlloc::M(ctxt->errMax * sizeof(xmlRelaxNGValidError)));
		if(ctxt->errTab == NULL) {
	...
	if(ctxt->errNr >= ctxt->errMax) {
		ctxt->errMax *= 2;
		ctxt->errTab = static_cast<xmlRelaxNGValidError *>(SAlloc::R(ctxt->errTab, ctxt->errMax * sizeof(xmlRelaxNGValidError)));
		if(ctxt->errTab == NULL) {
			xmlRngVErrMemory(ctxt, "pushing error\n");
			return 0;
		}
 ...
 * Returns 0 in case of error, the index in the stack otherwise
 */
static int xmlRelaxNGDocumentPush(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDocument * value)
{
	if(ctxt->docTab == NULL) {
		ctxt->docMax = 4;
		ctxt->docNr = 0;
		ctxt->docTab = (xmlRelaxNGDocument **)SAlloc::M(ctxt->docMax * sizeof(ctxt->docTab[0]));
		if(ctxt->docTab == NULL) {
	...
	if(ctxt->docNr >= ctxt->docMax) {
		ctxt->docMax *= 2;
		ctxt->docTab = (xmlRelaxNGDocument **)SAlloc::R(ctxt->docTab, ctxt->docMax * sizeof(ctxt->docTab[0]));
		if(ctxt->docTab == NULL) {
			xmlRngPErrMemory(ctxt, "adding document\n");
			return 0;
		}
 ...
 */
static xmlRelaxNGDocument * xmlRelaxNGLoadExternalRef(xmlRelaxNGParserCtxt * ctxt, const xmlChar * URL, const xmlChar * ns)
{
	xmlRelaxNGDocument * ret = NULL;
	xmlDoc * doc;
	xmlNode * root;
	int i;
	/*
	 ...
	 * Allocate the document structures and register it first.
	 */
	ret = (xmlRelaxNGDocument *)SAlloc::M(sizeof(xmlRelaxNGDocument));
	if(!ret) {
...
#endif
	xmlChar * msg = xmlRelaxNGGetErrorString(err, arg1, arg2);
	if(msg) {
		if(ctxt->errNo == XML_RELAXNG_OK)
			ctxt->errNo = err;
		xmlRngVErr(ctxt, (child == NULL ? P_Node : child), err, reinterpret_cast<const char *>(msg), arg1, arg2);
		SAlloc::F(msg);
	}
}
/**
...
static int xmlRelaxNGSchemaTypeHave(void * data ATTRIBUTE_UNUSED, const xmlChar * type)
{
	if(type == NULL)
		...
		xmlSchemaType * typ = xmlSchemaGetPredefinedType(type, InetUrl::MkHttp("www.w3.org", "2001/XMLSchema").ucptr());
		return BIN(typ);
	}
}
/**
...
static int xmlRelaxNGSchemaTypeCheck(void * data ATTRIBUTE_UNUSED, const xmlChar * pType, const xmlChar * value, void ** result, xmlNode * P_Node)
{
	...
	if(!pType || !value)
		return -1;
	typ = xmlSchemaGetPredefinedType(pType, InetUrl::MkHttp("www.w3.org", "2001/XMLSchema").ucptr());
	if(typ == NULL)
		return -1;
	ret = xmlSchemaValPredefTypeNode(typ, value, (xmlSchemaVal **)result, P_Node);
    ...
    const xmlChar * type, const xmlChar * facetname, const xmlChar * val, const xmlChar * strval, void * value)
{
	...
	if(!type || !strval)
		return -1;
	typ = xmlSchemaGetPredefinedType(type, InetUrl::MkHttp("www.w3.org", "2001/XMLSchema").ucptr());
	if(typ == NULL)
		return -1;
 ...
 */
static void xmlRelaxNGFreeTypeLibrary(xmlRelaxNGTypeLibrary * lib, const xmlChar * pNamespace ATTRIBUTE_UNUSED)
{
	if(lib) {
    ...
    xmlRelaxNGTypeCheck check, xmlRelaxNGTypeCompare comp, xmlRelaxNGFacetCheck facet, xmlRelaxNGTypeFree freef)
{
	...
	lib = static_cast<xmlRelaxNGTypeLibrary *>(SAlloc::M(sizeof(xmlRelaxNGTypeLibrary)));
	if(lib == NULL) {
...
int xmlRelaxNGInitTypes()
{
	if(xmlRelaxNGTypeInitialized) {
		...
		xmlRelaxNGRegisterTypeLibrary(InetUrl::MkHttp("www.w3.org", "2001/XMLSchema-datatypes").ucptr(), NULL, xmlRelaxNGSchemaTypeHave, 
...
void xmlRelaxNGCleanupTypes()
{
	...
	if(xmlRelaxNGTypeInitialized) {
		xmlHashFree(xmlRelaxNGRegisteredTypes, reinterpret_cast<xmlHashDeallocator>(xmlRelaxNGFreeTypeLibrary));
...
static int xmlRelaxNGTryCompile(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * def);
/**
		...
		case XML_RELAXNG_TEXT: 
			{
				...
				xmlAutomataNewTransition(ctxt->am, ctxt->state, ctxt->state, reinterpret_cast<const xmlChar *>("#text"), 0);
				ctxt->state = xmlAutomataNewEpsilon(ctxt->am, oldstate, 0);
			}
			break;
		case XML_RELAXNG_EMPTY:
...
// Parsing functions
//
static xmlRelaxNGDefine * xmlRelaxNGParseAttribute(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node);
static xmlRelaxNGDefine * xmlRelaxNGParseElement(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node);
static xmlRelaxNGDefine * xmlRelaxNGParsePatterns(xmlRelaxNGParserCtxt * ctxt, xmlNode * nodes, int group);
static xmlRelaxNGDefine * xmlRelaxNGParsePattern(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node);
...
static xmlRelaxNGDefine * xmlRelaxNGParseNameClass(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node, xmlRelaxNGDefine * def);
static xmlRelaxNGGrammar * xmlRelaxNGParseGrammar(xmlRelaxNGParserCtxt * ctxt, xmlNode * nodes);
static int xmlRelaxNGElementMatch(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define, xmlNode * elem);
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error
 */
static int xmlRelaxNGIsNullable(xmlRelaxNGDefine * define)
{
	int ret;
	if(define == NULL)
		return -1;
 ...
 */
static int FASTCALL xmlRelaxNGIsBlank(const xmlChar * str)
{
	if(!str)
		return 1;
	while(*str != 0) {
 ...
 */
static xmlChar * xmlRelaxNGGetDataTypeLibrary(xmlRelaxNGParserCtxt * ctxt ATTRIBUTE_UNUSED, xmlNode * P_Node)
{
	xmlChar * ret, * escape;
	if(!P_Node)
		return 0;
	if((IS_RELAXNG(P_Node, "data")) || (IS_RELAXNG(P_Node, "value"))) {
		ret = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("datatypeLibrary"));
		if(ret) {
			if(ret[0] == 0) {
				SAlloc::F(ret);
				return 0;
			}
			escape = xmlURIEscapeStr(ret, reinterpret_cast<const xmlChar *>(":/#?"));
			if(escape == NULL) {
				return ret;
			}
			SAlloc::F(ret);
 ...
 */
static xmlRelaxNGDefine * xmlRelaxNGParseValue(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	xmlRelaxNGTypeLibrary * lib = NULL;
	xmlChar * type;
	xmlChar * library;
	int success = 0;
	...
	type = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("type"));
	if(type) {
		...
		SETIFZ(library, sstrdup(reinterpret_cast<const xmlChar *>("http://relaxng.org/ns/structure/1.0")));
		def->name = type;
		def->ns = library;
		lib = static_cast<xmlRelaxNGTypeLibrary *>(xmlHashLookup(xmlRelaxNGRegisteredTypes, library));
		if(lib == NULL) {
	...
	if(P_Node->children == NULL) {
		...
		if(def->value == NULL) {
			...
			if(success != 1) {
				...
				def->attrs = static_cast<xmlRelaxNGDefine *>(val);
		}
	}
	return (def);
}
 ...
 * Returns the definition pointer or NULL in case of error
 */
static xmlRelaxNGDefine * xmlRelaxNGParseData(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	xmlRelaxNGDefine * def = NULL;
	xmlRelaxNGDefine * except;
	xmlRelaxNGDefine * param;
	xmlRelaxNGDefine * lastparam = NULL;
	xmlRelaxNGTypeLibrary * lib;
	xmlChar * library;
	xmlNode * content;
	int tmp;
	xmlChar * type = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("type"));
	if(type == NULL) {
	...
	if(lib == NULL) {
		...
		if(lib->have == NULL) {
			...
			else if(sstreq(library, InetUrl::MkHttp("www.w3.org", "2001/XMLSchema-datatypes").ucptr()) && (sstreq(def->name, "IDREF") || sstreq(def->name, "IDREFS"))) {
				ctxt->idref = 1;
			}
		}
	}
	...
	while(content) {
		...
		if(sstreq(library, "http://relaxng.org/ns/structure/1.0")) {
			...
			if(param) {
				...
				param->name = xmlGetProp(content, reinterpret_cast<const xmlChar *>("name"));
				if(param->name == NULL) {
...
static const xmlChar * invalidName = reinterpret_cast<const xmlChar *>("\1");
 ...
 */
static int xmlRelaxNGCompareNameClasses(xmlRelaxNGDefine * def1, xmlRelaxNGDefine * def2)
{
	int ret = 1;
	xmlNode node;
	xmlNs ns;
 ...
 * Returns 1 disttinct, 0 if equal
 */
static int xmlRelaxNGCompareElemDefLists(xmlRelaxNGParserCtxt * ctxt ATTRIBUTE_UNUSED, xmlRelaxNGDefine ** def1, xmlRelaxNGDefine ** def2)
{
	xmlRelaxNGDefine ** basedef2 = def2;
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGGenerateAttributes(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * def)
{
	//
 ...
 */
static xmlRelaxNGDefine ** xmlRelaxNGGetElements(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * def, int eora)
{
	xmlRelaxNGDefine ** ret = NULL;
	xmlRelaxNGDefine * parent;
	...
	while(cur) {
		if(((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) || (cur->type == XML_RELAXNG_TEXT))) || ((eora == 1) && (cur->type == XML_RELAXNG_ATTRIBUTE))) {
			if(!ret) {
				max = 10;
				ret = static_cast<xmlRelaxNGDefine **>(SAlloc::M((max + 1) * sizeof(xmlRelaxNGDefine *)));
				if(!ret) {
				...
				max *= 2;
				xmlRelaxNGDefine ** temp = static_cast<xmlRelaxNGDefine **>(SAlloc::R(ret, (max + 1) * sizeof(xmlRelaxNGDefine *)));
				if(temp == NULL) {
					xmlRngPErrMemory(ctxt, "getting element list\n");
					SAlloc::F(ret);
					return 0;
 ...
 */
static void xmlRelaxNGCheckChoiceDeterminism(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * def)
{
	xmlRelaxNGDefine *** list;
	...
	if(!ctxt->nbErrors) { // Don't run that check in case of error. Infinite recursion becomes possible.
		...
		while(cur) {
			...
			if((list[i] == NULL) || (list[i][0] == NULL)) {
				...
				int res;
				xmlRelaxNGDefine ** tmp = list[i];
				while(*tmp && (is_triable == 1)) {
					if((*tmp)->type == XML_RELAXNG_TEXT) {
						res = xmlHashAddEntry2(triage, reinterpret_cast<const xmlChar *>("#text"), NULL, (void *)cur);
						if(res != 0)
							is_triable = -1;
					}
						...
						res = xmlHashAddEntry2(triage, reinterpret_cast<const xmlChar *>("#any"), isempty((*tmp)->ns) ? NULL : (*tmp)->ns, (void *)cur);
						if(res != 0)
							is_triable = -1;
					}
					else {
 ...
 */
static void xmlRelaxNGCheckGroupAttrs(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * def)
{
	xmlRelaxNGDefine *** list;
	xmlRelaxNGDefine * cur;
	int nbchild = 0, i, j, ret;
 ...
 *   algorithm
 */
static void xmlRelaxNGComputeInterleaves(xmlRelaxNGDefine * def, xmlRelaxNGParserCtxt * ctxt, xmlChar * name ATTRIBUTE_UNUSED)
{
	xmlRelaxNGDefine * cur;
	xmlRelaxNGDefine ** tmp;
 ...
 * Returns the definition pointer or NULL in case of error
 */
static xmlRelaxNGDefine * xmlRelaxNGParseInterleave(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	xmlRelaxNGDefine * last = NULL;
	xmlRelaxNGDefine * cur;
	xmlNode * child;
 ...
 */
static int xmlRelaxNGParseInclude(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	xmlNode * root;
	int ret = 0, tmp;
	xmlRelaxNGInclude * incl = (xmlRelaxNGInclude *)P_Node->psvi;
	if(incl == NULL) {
 ...
 * Returns 0 in case of success or -1 in case of error
 */
static int xmlRelaxNGParseDefine(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	int ret = 0, tmp;
	xmlRelaxNGDefine * def;
	const xmlChar * olddefine;
	xmlChar * name = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("name"));
	if(!name) {
		...
		if(ctxt->grammar->defs == NULL) {
			...
			if(tmp < 0) {
				xmlRelaxNGDefine * prev;
				prev = (xmlRelaxNGDefine *)xmlHashLookup(ctxt->grammar->defs, name);
				if(prev == NULL) {
...
{
	xmlRelaxNGParserCtxt * ctxt = static_cast<xmlRelaxNGParserCtxt *>(data);
	xmlRelaxNGDefine * def = static_cast<xmlRelaxNGDefine *>(payload);
	int tmp;
	def->dflags |= IS_EXTERNAL_REF;
	tmp = xmlHashAddEntry(ctxt->grammar->refs, name, def);
	if(tmp < 0) {
		xmlRelaxNGDefine * prev = static_cast<xmlRelaxNGDefine *>(xmlHashLookup(ctxt->grammar->refs, def->name));
		if(!prev) {
			if(def->name)
 ...
 */
static xmlRelaxNGDefine * xmlRelaxNGProcessExternalRef(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	xmlRelaxNGDefine * def = 0;
	xmlNode * root;
	xmlNode * tmp;
	xmlChar * ns;
	int newNs = 0, oldflags;
	xmlRelaxNGDocument * docu = (xmlRelaxNGDocument *)P_Node->psvi;
	if(docu) {
		def = xmlRelaxNGNewDefine(ctxt, P_Node);
		if(def) {
			...
			if(docu->content == NULL) {
				...
				*/
				if(newNs == 1)
					xmlUnsetProp(root, reinterpret_cast<const xmlChar *>("ns"));
			}
...
static xmlRelaxNGDefine * xmlRelaxNGParsePattern(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
	...
	if(P_Node) {
		if(IS_RELAXNG(P_Node, "element")) {
			...
			def->name = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("name"));
			if(def->name == NULL)
			...
			if(ctxt->grammar->refs == NULL) {
				...
				if(tmp < 0) {
					xmlRelaxNGDefine * prev = (xmlRelaxNGDefine *)xmlHashLookup(ctxt->grammar->refs, def->name);
					if(prev == NULL) {
						if(def->name)
			...
			if(ctxt->parentgrammar->refs == NULL) {
				...
				if(tmp < 0) {
					xmlRelaxNGDefine * prev = (xmlRelaxNGDefine *)xmlHashLookup(ctxt->parentgrammar->refs, def->name);
					if(prev == NULL) {
 ...
 * Returns the definition pointer or NULL in case of error.
 */
static xmlRelaxNGDefine * xmlRelaxNGParseExceptNameClass(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node, int attr)
{
	xmlRelaxNGDefine * ret;
	xmlRelaxNGDefine * cur;
	xmlRelaxNGDefine * last = NULL;
...
static xmlRelaxNGDefine * xmlRelaxNGParseNameClass(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node, xmlRelaxNGDefine * def)
{
	...
	if(IS_RELAXNG(P_Node, "name")) {
		...
		ret->name = NULL;
		ret->ns = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("ns"));
		if(ret->ns == NULL) {
 ...
 */
static int xmlRelaxNGParseStart(xmlRelaxNGParserCtxt * ctxt, xmlNode * nodes)
{
	int ret = 0;
	xmlRelaxNGDefine * def = NULL;
	xmlRelaxNGDefine * last;
 ...
 */
static void xmlRelaxNGCheckReference(xmlRelaxNGDefine * ref, xmlRelaxNGParserCtxt * ctxt, const xmlChar * name)
{
	/*
 ...
 * element of a given grammar using the same name.
 */
static void xmlRelaxNGCheckCombine(xmlRelaxNGDefine * define, xmlRelaxNGParserCtxt * ctxt, const xmlChar * name)
{
	...
	if(define->nextHash) {
		...
		while(cur) {
			xmlChar * combine = xmlGetProp(cur->P_Node, reinterpret_cast<const xmlChar *>("combine"));
			if(combine) {
...
static void xmlRelaxNGCombineStart(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGGrammar * grammar)
{
	...
	int missing = 0;
	xmlRelaxNGDefine * cur;
	xmlRelaxNGDefine * starts = grammar->start;
	if((starts == NULL) || (starts->next == NULL))
		return;
	cur = starts;
	while(cur) {
		if(!cur->P_Node || !cur->P_Node->P_ParentNode || !sstreq(cur->P_Node->P_ParentNode->name, "start")) {
			combine = NULL;
			xmlRngPErr(ctxt, cur->P_Node, XML_RNGP_START_MISSING, "Internal error: start element not found\n", 0, 0);
		}
		else {
			combine = xmlGetProp(cur->P_Node->P_ParentNode, reinterpret_cast<const xmlChar *>("combine"));
		}
		if(combine) {
			if(sstreq(combine, "choice")) {
				if(choiceOrInterleave == -1)
 ...
 */
static int xmlRelaxNGCheckCycles(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * cur, int depth)
{
	int ret = 0;
	while((ret == 0) && cur) {
 ...
 */
static xmlRelaxNGDefine * xmlRelaxNGTryUnlink(xmlRelaxNGParserCtxt * ctxt ATTRIBUTE_UNUSED, xmlRelaxNGDefine * cur, xmlRelaxNGDefine * parent, xmlRelaxNGDefine * prev)
{
	if(prev) {
		prev->next = cur->next;
	}
 ...
 */
static void FASTCALL xmlRelaxNGSimplify(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * cur, xmlRelaxNGDefine * parent)
{
 ...
 */
static xmlRelaxNGContentType FASTCALL xmlRelaxNGGroupContentType(xmlRelaxNGContentType ct1, xmlRelaxNGContentType ct2)
{
 ...
 * Returns the content type
 */
static xmlRelaxNGContentType xmlRelaxNGMaxContentType(xmlRelaxNGContentType ct1, xmlRelaxNGContentType ct2)
{
	if((ct1 == XML_RELAXNG_CONTENT_ERROR) || (ct2 == XML_RELAXNG_CONTENT_ERROR))
		return (XML_RELAXNG_CONTENT_ERROR);
 ...
 */
static xmlRelaxNGContentType FASTCALL xmlRelaxNGCheckRules(xmlRelaxNGParserCtxt * ctxt, xmlRelaxNGDefine * cur, int flags, xmlRelaxNGType ptype)
{
	int nflags;
	...
	while(cur) {
		...
		if(ptype == XML_RELAXNG_GROUP) {
			...
			/*
			 * @todo scan complain that tmp is never used, seems on purpose need double-checking
			 */
...
xmlRelaxNGParserCtxt * xmlRelaxNGNewParserCtxt(const char * URL)
{
	...
	if(URL) {
		ret = static_cast<xmlRelaxNGParserCtxt *>(SAlloc::M(sizeof(xmlRelaxNGParserCtxt)));
		if(!ret) {
 ...
 */
static void xmlRelaxNGCleanupAttributes(xmlRelaxNGParserCtxt * ctxt, xmlNode * P_Node)
{
 ...
 */
static void xmlRelaxNGCleanupTree(xmlRelaxNGParserCtxt * ctxt, xmlNode * root)
{
	...
	while(cur) {
		...
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(!cur->ns || !sstreq(cur->ns->href, xmlRelaxNGNs)) {
				if(cur->P_ParentNode && (cur->P_ParentNode->type == XML_ELEMENT_NODE) &&
				    (sstreq(cur->P_ParentNode->name, "name") || sstreq(cur->P_ParentNode->name, "value") || sstreq(cur->P_ParentNode->name, "param"))) {
					xmlRngPErr(ctxt, cur, XML_RNGP_FOREIGN_ELEMENT, "element %s doesn't allow foreign elements\n", cur->P_ParentNode->name, 0);
				}
				p_delete = cur;
				goto skip_children;
			}
				...
				if(sstreq(cur->name, "externalRef")) {
					...
					href = xmlGetProp(cur, reinterpret_cast<const xmlChar *>("href"));
					if(!href) {
					...
					SAlloc::F(href);
					SAlloc::F(base);
					ns = xmlGetProp(cur, reinterpret_cast<const xmlChar *>("ns"));
					if(!ns) {
						tmp = cur->P_ParentNode;
						while(tmp && tmp->type == XML_ELEMENT_NODE) {
							ns = xmlGetProp(tmp, reinterpret_cast<const xmlChar *>("ns"));
					...
					if(name) {
						if(cur->children == NULL) {
							text = xmlNewChild(cur, cur->ns, reinterpret_cast<const xmlChar *>("name"), name);
						}
						else {
							xmlNode * p_node = xmlNewDocNode(cur->doc, cur->ns, reinterpret_cast<const xmlChar *>("name"), 0);
							if(p_node) {
						...
						xmlUnsetProp(cur, reinterpret_cast<const xmlChar *>("name"));
						SAlloc::F(name);
						ns = xmlGetProp(cur, reinterpret_cast<const xmlChar *>("ns"));
						if(ns) {
							if(text) {
								xmlSetProp(text, reinterpret_cast<const xmlChar *>("ns"), ns);
								/* xmlUnsetProp(cur, reinterpret_cast<const xmlChar *>("ns")); */
							}
							SAlloc::F(ns);
						}
					 ...
					 * and attribute elements
					 */
					if(xmlHasProp(cur, reinterpret_cast<const xmlChar *>("ns")) == NULL) {
						xmlChar * ns = NULL;
						xmlNode * P_Node = cur->P_ParentNode;
						while(P_Node && (P_Node->type == XML_ELEMENT_NODE)) {
							ns = xmlGetProp(P_Node, reinterpret_cast<const xmlChar *>("ns"));
							if(ns) {
								break;
							}
							P_Node = P_Node->P_ParentNode;
						...
						if(name) {
							...
							if(local) {
								...
								if(!ns) {
									...
									xmlSetProp(cur, reinterpret_cast<const xmlChar *>("ns"), ns->href);
					 ...
					 * 4.16
					 */
					if(cur->P_ParentNode && sstreq(cur->P_ParentNode->name, "anyName")) {
						ctxt->flags |= XML_RELAXNG_IN_ANYEXCEPT;
						xmlRelaxNGCleanupTree(ctxt, cur);
						ctxt->flags = oldflags;
						goto skip_children;
					}
					else if(cur->P_ParentNode && sstreq(cur->P_ParentNode->name, "nsName")) {
				...
				if(sstreq(cur->name, "div")) {
					...
					while(child) {
						if(ns) {
							if(!xmlHasProp(child, reinterpret_cast<const xmlChar *>("ns"))) {
								xmlSetProp(child, reinterpret_cast<const xmlChar *>("ns"), ns);
							}
						}
						tmp = child->next;
						xmlUnlinkNode(child);
					 ...
					 * (bug 143738).
					 */
					if(cur->nsDef && cur->P_ParentNode) {
						xmlNs * parDef = (xmlNs *)&cur->P_ParentNode->nsDef;
						while(parDef->next)
							parDef = parDef->next;
		 ...
		 */
			...
			if(IS_BLANK_NODE(cur)) {
				if(cur->P_ParentNode && cur->P_ParentNode->type == XML_ELEMENT_NODE) {
					if((!sstreq(cur->P_ParentNode->name, "value")) && (!sstreq(cur->P_ParentNode->name, "param")))
						p_delete = cur;
				}
				else {
					p_delete = cur;
...
xmlRelaxNGPtr xmlRelaxNGParse(xmlRelaxNGParserCtxt * ctxt)
{
	...
	if(ctxt->URL) {
		...
		ctxt->URL = sstrdup(reinterpret_cast<const xmlChar *>("in_memory_buffer"));
	}
	...
	root = xmlDocGetRootElement(doc);
	if(root == NULL) {
		xmlRngPErr(ctxt, (xmlNode *)doc, XML_RNGP_EMPTY, "xmlRelaxNGParse: %s is empty\n", (ctxt->URL ? ctxt->URL : reinterpret_cast<const xmlChar *>("schemas")), 0);
		xmlFreeDoc(ctxt->document);
		ctxt->document = NULL;
		return 0;
	}
...
//
static void xmlRelaxNGDumpDefine(FILE * output, xmlRelaxNGDefine * define);
/**
 ...
 */
static void FASTCALL xmlRelaxNGDumpDefines(FILE * output, xmlRelaxNGDefine * defines)
{
	while(defines) {
...
static void xmlRelaxNGDumpGrammar(FILE * output, xmlRelaxNGGrammar * grammar, int top)
{
	if(grammar) {
		...
		switch(grammar->combine) {
			case XML_RELAXNG_COMBINE_UNDEFINED: break;
			case XML_RELAXNG_COMBINE_CHOICE: fprintf(output, " combine=\"choice\""); break;
			case XML_RELAXNG_COMBINE_INTERLEAVE: fprintf(output, " combine=\"interleave\""); break;
			default: fprintf(output, " <!-- invalid combine value -->");
		}
		fprintf(output, ">\n");
...
//
static int xmlRelaxNGValidateDefinition(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define);
/**
...
static void xmlRelaxNGValidateCompiledCallback(xmlRegExecCtxtPtr exec ATTRIBUTE_UNUSED, const xmlChar * token, void * transdata, void * inputdata)
{
	...
	xmlRelaxNGDefine * define = (xmlRelaxNGDefine *)transdata;
	int ret;
#ifdef DEBUG_COMPILE
		...
		switch(cur->type) {
			...
			case XML_CDATA_SECTION_NODE:
			    if(xmlIsBlankNode(cur))
				    break;
			    ret = xmlRegExecPushString(exec, reinterpret_cast<const xmlChar *>("#text"), ctxt);
			    if(ret < 0) {
				    VALID_ERR2(XML_RELAXNG_ERR_TEXTWRONG, cur->P_ParentNode->name);
			    }
			    break;
...
//
static int xmlRelaxNGValidateAttributeList(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * defines);
...
{
	xmlRelaxNGValidCtxtPtr ctxt = (xmlRelaxNGValidCtxtPtr)inputdata;
	xmlRelaxNGDefine * p_define = (xmlRelaxNGDefine *)transdata;
	...
	exec = xmlRegNewExecCtxt(p_define->contModel, xmlRelaxNGValidateProgressiveCallback, ctxt);
	if(!exec) {
		ctxt->pstate = -1;
		return;
	}
	...
	ctxt->state = state;
	if(p_define->attrs) {
		ret = xmlRelaxNGValidateAttributeList(ctxt, p_define->attrs);
		if(ret) {
			ctxt->pstate = -1;
...
int xmlRelaxNGValidatePushCData(xmlRelaxNGValidCtxtPtr ctxt, const xmlChar * data, int len ATTRIBUTE_UNUSED)
{
	...
	if(*data == 0)
		return 1;
	ret = xmlRegExecPushString(ctxt->elem, reinterpret_cast<const xmlChar *>("#text"), ctxt);
	if(ret < 0) {
		VALID_ERR2(XML_RELAXNG_ERR_TEXTWRONG, reinterpret_cast<const xmlChar *>(" TODO "));
#ifdef DEBUG_PROGRESSIVE
...
//
static int xmlRelaxNGValidateValue(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define);
/**
 ...
 */
static xmlNode * FASTCALL xmlRelaxNGSkipIgnored(xmlRelaxNGValidCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlNode * pNode)
{
	/*
	 * @todo complete and handle entities
	 */
	while(pNode && (oneof4(pNode->type, XML_COMMENT_NODE, XML_PI_NODE, XML_XINCLUDE_START, XML_XINCLUDE_END) ||
		(oneof2(pNode->type, XML_TEXT_NODE, XML_CDATA_SECTION_NODE) && ((ctxt->flags & FLAGS_MIXED_CONTENT) || (IS_BLANK_NODE(pNode)))))) {
		pNode = pNode->next;
	}
	return pNode;
}
 ...
 */
static int xmlRelaxNGValidateDatatype(xmlRelaxNGValidCtxtPtr ctxt, const xmlChar * value, xmlRelaxNGDefine * define, xmlNode * P_Node)
{
	int ret, tmp;
	xmlRelaxNGTypeLibrary * lib;
	void * result = NULL;
	...
	lib = static_cast<xmlRelaxNGTypeLibrary *>(define->data);
	if(lib->check) {
	...
	cur = define->attrs;
	while(!ret && cur && (cur->type == XML_RELAXNG_PARAM)) {
		if(lib->facet) {
	...
	if((ret == 0) && define->content) {
		...
		ctxt->state->value = const_cast<xmlChar *>(value); // @badcast
		ctxt->state->endvalue = NULL;
		ret = xmlRelaxNGValidateValue(ctxt, define->content);
		ctxt->state->value = const_cast<xmlChar *>(oldvalue); // @badcast
		ctxt->state->endvalue = const_cast<xmlChar *>(oldendvalue); // @badcast
	}
	if(result && lib && lib->freef)
		lib->freef(lib->data, result);
	return ret;
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateValueList(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * defines)
{
	int ret = 0;
		...
		case XML_RELAXNG_VALUE: {
		    if(!sstreq(value, define->value)) {
				 ...
				 * @todo trivial optimizations are possible by computing at compile-time
				    ...
				    if(!nval || !nvalue || !sstreq(nval, nvalue))
					    ret = -1;
				    SAlloc::F(nval);
				    SAlloc::F(nvalue);
			    }
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateValueContent(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * defines)
{
	int ret = 0;
	for(; !ret && defines; defines = defines->next)
		ret = xmlRelaxNGValidateValue(ctxt, defines);
 ...
 */
static int xmlRelaxNGAttributeMatch(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define, xmlAttr * prop)
{
	int ret;
	if(define->name) {
	...
	if(define->type == XML_RELAXNG_EXCEPT) {
		...
		xmlRelaxNGDefine * list = define->nameClass;
		while(list) {
			ret = xmlRelaxNGAttributeMatch(ctxt, list, prop);
			if(ret == 1)
				return 1;
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateAttribute(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define)
{
	int ret = 0, i;
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateInterleave(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define)
{
		...
		if(partitions->triage && (partitions->flags & IS_DETERMINIST)) {
			...
			if(oneof2(cur->type, XML_TEXT_NODE, XML_CDATA_SECTION_NODE)) {
				...
				if(cur->ns) {
					...
					SETIFZ(tmp, xmlHashLookup2(partitions->triage, reinterpret_cast<const xmlChar *>("#any"), cur->ns->href));
				}
				else
					tmp = xmlHashLookup2(partitions->triage, cur->name, NULL);
				SETIFZ(tmp, xmlHashLookup2(partitions->triage, reinterpret_cast<const xmlChar *>("#any"), NULL));
			}
			if(!tmp) {
				i = nbgroups;
			}
 ...
 * Returns 0 if the validation succeeded or an error code.
 */
static int xmlRelaxNGValidateDefinitionList(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * defines)
{
	int ret = 0, res;
	if(defines == NULL) {
		VALID_ERR2(XML_RELAXNG_ERR_INTERNAL, reinterpret_cast<const xmlChar *>("NULL definition list"));
		return -1;
	}
	while(defines) {
		if(ctxt->state || ctxt->states) {
...
static int xmlRelaxNGValidateDefinition(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefine * define)
{
	xmlRelaxNGStates * states;
	xmlRelaxNGStates * res;
...
#endif
	if(!ret && ctxt->errNo != XML_RELAXNG_OK)
		ret = -1;
	return ret;
}
/**
 ...
 *  of error.
 */
static void xmlRelaxNGCleanPSVI(xmlNode * pNode) 
{
	if(pNode && oneof3(pNode->type, XML_ELEMENT_NODE, XML_DOCUMENT_NODE, XML_HTML_DOCUMENT_NODE)) {
		if(pNode->type == XML_ELEMENT_NODE)
			pNode->psvi = NULL;
		for(xmlNode * cur = pNode->children; cur;) {
			if(cur->type == XML_ELEMENT_NODE) {
				cur->psvi = NULL;
				if(cur->children) {
					cur = cur->children;
...
{
	xmlRelaxNGValidCtxtPtr ret = static_cast<xmlRelaxNGValidCtxtPtr>(SAlloc::M(sizeof(xmlRelaxNGValidCtxt)));
	if(!ret) {
