...
static void testRegexp(xmlRegexp * comp, const char * value) 
{
	int ret = xmlRegexpExec(comp, (const xmlChar*)value);
	if(ret == 1)
		printf("%s: Ok\n", value);
	else if(ret == 0)
		printf("%s: Fail\n", value);
...
static void testReduce(xmlExpCtxtPtr ctxt, xmlExpNodePtr expr, const char * tst) 
{
	xmlBuffer * xmlExpBuf;
...
static void exprDebug(xmlExpCtxtPtr ctxt, xmlExpNodePtr expr) 
{
	xmlBuffer * xmlExpBuf;
	xmlExpNodePtr deriv;
	const char * list[40];
