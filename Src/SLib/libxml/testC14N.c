...
static xmlXPathObject * load_xpath_expr(xmlDoc * parent_doc, const char* filename);

static xmlChar ** parse_list(xmlChar * str);

/* static void print_xpath_nodes(xmlNodeSetPtr nodes); */

static int test_c14n(const char* xml_filename, int with_comments, int mode, const char* xpath_filename, xmlChar ** inclusive_namespaces) 
{
	xmlDocPtr doc;
...
static xmlXPathObject * load_xpath_expr(xmlDocPtr parent_doc, const char* filename) 
{
	...
	node = doc->children;
	while(node && !sstreq(node->name, "XPath")) {
		node = node->next;
	}
	if(node == NULL) {
