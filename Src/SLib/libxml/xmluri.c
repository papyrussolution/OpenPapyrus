 ...
 */
// @v11.7.9 (replaced with isdec()) #define IS_DIGIT_Removed(x) (((x) >= '0') && ((x) <= '9'))
/*
...
static int xmlParse3986Authority(xmlURI * uri, const char ** str)
{
	 ...
	 */
	int ret = xmlParse3986Userinfo(uri, &cur);
	if((ret != 0) || (*cur != '@'))
		cur = *str;
	else
		cur++;
...
xmlURI * xmlParseURIRaw(const char * str, int raw)
{
	...
	if(str) {
		...
		if(uri) {
			...
			int ret = xmlParseURIReference(uri, str);
			if(ret) {
				xmlFreeURI(uri);
				return 0;
			}
	...
	if(uri->opaque) {
		...
		while(*p != 0) {
			...
			if(IS_RESERVED(*(p)) || IS_UNRESERVED(*(p)))
				...
				int val = *reinterpret_cast<const uchar *>(p++);
 ...
 * output is a direct uchar translation of %XX values (no encoding)
