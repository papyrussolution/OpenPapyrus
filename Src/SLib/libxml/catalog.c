...
{
	if(catal) {
		for(xmlCatalogEntryPtr children = catal->children; children;) {
...
{
	while(cur) {
		if(cur->ns && cur->ns->href && (sstreq(cur->ns->href, XML_CATALOGS_NAMESPACE))) {
			...
			while(catal) {
				if(catal->type == XML_CATA_CATALOG) {
					...
					if(catal->children) {
						...
						else if(catal->children && (catal->children->depth > MAX_CATAL_DEPTH)) {
							ret = NULL;
							break;
						}
					}
...
static const xmlChar * xmlParseSGMLCatalogName(const xmlChar * cur, xmlChar ** name) 
{
	...
	*name = NULL;
	...
	if((!IS_LETTER(c) && !oneof2(c, '_', ':'))) {
		...
		while(((IS_LETTER(c)) || (isdec(c)) || oneof4(c, '.', '-', '_', ':'))) {
			buf[len++] = c;
			cur++;
			c = *cur;
