 ...
 * Come casting macros.
 */
#define ACTXT_CAST (xmlSchemaAbstractCtxt *)
#define PCTXT_CAST (xmlSchemaParserCtxt *)
#define VCTXT_CAST (xmlSchemaValidCtxt *)
#define WXS_BASIC_CAST (xmlSchemaBasicItem *) 
#define WXS_TREE_CAST (xmlSchemaTreeItem *)
#define WXS_PTC_CAST (xmlSchemaParticle *)
#define WXS_TYPE_CAST (xmlSchemaType *)
#define WXS_ELEM_CAST (xmlSchemaElementPtr)
#define WXS_ATTR_GROUP_CAST (xmlSchemaAttributeGroup *)
#define WXS_ATTR_CAST (xmlSchemaAttribute *)
#define WXS_ATTR_USE_CAST (xmlSchemaAttributeUse *)
...
#define WXS_IDC_CAST (xmlSchemaIDC *)
#define WXS_QNAME_CAST (xmlSchemaQNameRef *)
#define WXS_LIST_CAST (xmlSchemaItemList *)
...
#define IS_SCHEMA(pNode, type) _IsSchema(pNode, type)
...
#define HSTOP(ctx) if((ctx)->Stop_) goto exit;
/*
...
struct xmlSchemaAbstractCtxt {
	int type; /* E.g. XML_SCHEMA_CTXT_VALIDATOR */
};

//typedef struct _xmlSchemaBucket xmlSchemaBucket;
struct xmlSchemaBucket;
...
struct xmlSchemaSchemaRelation {
	xmlSchemaSchemaRelation * next;
...
struct xmlSchemaBucket {
	...
	xmlDoc * doc;
	xmlSchemaSchemaRelation * relations;
	int located;
	int parsed;
	int imported;
	int preserveDoc;
	xmlSchemaItemList * globals; /* Global components. */
	xmlSchemaItemList * locals; /* Local components. */
};
/**
 * xmlSchemaImport:
...
struct xmlSchemaImport {
	...
	xmlSchemaSchemaRelation * relations; // @relations will hold any included/imported/redefined schemas
	int located;
	int parsed;
	int imported;
	int preserveDoc;
	xmlSchemaItemList * globals;
	xmlSchemaItemList * locals;
...
struct _xmlSchemaConstructionCtxt {
	...
	xmlSchemaBucket * mainBucket; /* The main schema bucket */
	xmlDict * dict;
	xmlSchemaItemList * buckets; /* List of schema buckets. */
	/* xmlSchemaItemList * relations; */ /* List of schema relations. */
	xmlSchemaBucket * bucket; /* The current schema bucket */
	xmlSchemaItemList * pending; // All Components of all schemas that need to be fixed. 
...
struct xmlSchemaParserCtxt {
	...
	bool   Preserve_; // Whether the doc should be freed // @sobolev preserve-->Preserve_
	bool   ownsConstructor; // @todo Move this to parser *flags*
	bool   isS4S;
	bool   isRedefine;
	bool   xsiAssemble;
	uint8  Reserve[3];
	int    options;
	int    Stop_; // If the parser should stop; i.e. a critical error // @sobolev stop-->Stop_
	int    redefCounter; // Used for redefinitions. 
	const  xmlChar * targetNamespace;
	xmlSchemaBucket * redefined; // The schema to be redefined. 
	xmlSchemaRedef * redef; // Used for redefinitions. 
...
struct _xmlSchemaIDCAug {
	...
	xmlSchemaIDC * def; // the IDC definition 
...
static int FASTCALL xmlSchemaTypeFixup(xmlSchemaType * type, xmlSchemaAbstractCtxt * ctxt);
...
static int xmlSchemaCheckFacetValues(xmlSchemaType * typeDecl, xmlSchemaParserCtxtPtr ctxt);
static void xmlSchemaClearValidCtxt(xmlSchemaValidCtxt * vctxt);
static xmlSchemaWhitespaceValueType xmlSchemaGetWhiteSpaceFacetValue(xmlSchemaType * type);
...
static xmlSchemaTypeLink * xmlSchemaGetUnionSimpleTypeMemberTypes(xmlSchemaType * type);
static void xmlSchemaInternalErr(xmlSchemaAbstractCtxt * actxt, const char * funcName, const char * message);
static int xmlSchemaCheckCOSSTDerivedOK(xmlSchemaAbstractCtxt * ctxt, xmlSchemaType * type, xmlSchemaType * baseType, int subset);
static void xmlSchemaCheckElementDeclComponent(xmlSchemaElementPtr elemDecl, xmlSchemaParserCtxtPtr ctxt);
static void FASTCALL xmlSchemaComponentListFree(xmlSchemaItemList * list);
static xmlSchemaQNameRef * xmlSchemaParseAttributeGroupRef(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNode * P_Node);
//
// Helper functions
//
/**
...
{
	switch(type) {
		case XML_SCHEMA_TYPE_BASIC: return (reinterpret_cast<const xmlChar *>("simple type definition"));
		case XML_SCHEMA_TYPE_SIMPLE: return (reinterpret_cast<const xmlChar *>("simple type definition"));
		case XML_SCHEMA_TYPE_COMPLEX: return (reinterpret_cast<const xmlChar *>("complex type definition"));
		case XML_SCHEMA_TYPE_ELEMENT: return (reinterpret_cast<const xmlChar *>("element declaration"));
		case XML_SCHEMA_TYPE_ATTRIBUTE_USE: return (reinterpret_cast<const xmlChar *>("attribute use"));
		case XML_SCHEMA_TYPE_ATTRIBUTE: return (reinterpret_cast<const xmlChar *>("attribute declaration"));
		case XML_SCHEMA_TYPE_GROUP: return (reinterpret_cast<const xmlChar *>("model group definition"));
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: return (reinterpret_cast<const xmlChar *>("attribute group definition"));
		case XML_SCHEMA_TYPE_NOTATION: return (reinterpret_cast<const xmlChar *>("notation declaration"));
		case XML_SCHEMA_TYPE_SEQUENCE: return (reinterpret_cast<const xmlChar *>("model group (sequence)"));
		case XML_SCHEMA_TYPE_CHOICE: return (reinterpret_cast<const xmlChar *>("model group (choice)"));
		case XML_SCHEMA_TYPE_ALL: return (reinterpret_cast<const xmlChar *>("model group (all)"));
		case XML_SCHEMA_TYPE_PARTICLE: return (reinterpret_cast<const xmlChar *>("particle"));
		case XML_SCHEMA_TYPE_IDC_UNIQUE: return (reinterpret_cast<const xmlChar *>("unique identity-constraint"));
		/* return (reinterpret_cast<const xmlChar *>("IDC (unique)")); */
		case XML_SCHEMA_TYPE_IDC_KEY: return (reinterpret_cast<const xmlChar *>("key identity-constraint"));
		/* return (reinterpret_cast<const xmlChar *>("IDC (key)")); */
		case XML_SCHEMA_TYPE_IDC_KEYREF: return (reinterpret_cast<const xmlChar *>("keyref identity-constraint"));
		/* return (reinterpret_cast<const xmlChar *>("IDC (keyref)")); */
		case XML_SCHEMA_TYPE_ANY: return (reinterpret_cast<const xmlChar *>("wildcard (any)"));
		case XML_SCHEMA_EXTRA_QNAMEREF: return (reinterpret_cast<const xmlChar *>("[helper component] QName reference"));
		case XML_SCHEMA_EXTRA_ATTR_USE_PROHIB: return (reinterpret_cast<const xmlChar *>("[helper component] attribute use prohibition"));
		default: return (reinterpret_cast<const xmlChar *>("Not a schema component"));
	}
}
	...
	switch(item->type) {
		case XML_SCHEMA_TYPE_BASIC:
		    if(WXS_IS_COMPLEX(WXS_TYPE_CAST item))
			    return (reinterpret_cast<const xmlChar *>("complex type definition"));
		    else
			    return (reinterpret_cast<const xmlChar *>("simple type definition"));
		default:
...
static xmlNode * xmlSchemaGetComponentNode(xmlSchemaBasicItem * item)
{
	switch(item->type) {
		...
		case XML_SCHEMA_TYPE_ATTRIBUTE: return (((xmlSchemaAttribute *)item)->P_Node);
		...
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: return reinterpret_cast<xmlSchemaAttributeGroup *>(item)->P_Node;
		case XML_SCHEMA_TYPE_IDC_UNIQUE:
		case XML_SCHEMA_TYPE_IDC_KEY:
		case XML_SCHEMA_TYPE_IDC_KEYREF: return reinterpret_cast<xmlSchemaIDC *>(item)->P_Node;
		case XML_SCHEMA_EXTRA_QNAMEREF: return (((xmlSchemaQNameRef *)item)->P_Node);
		/* @todo What to do with NOTATIONs?
		   case XML_SCHEMA_TYPE_NOTATION:
		    return (((xmlSchemaNotation *) item)->node);
		 */
		case XML_SCHEMA_TYPE_ATTRIBUTE_USE: return (((xmlSchemaAttributeUse *)item)->P_Node);
		default: return 0;
	}
}
...
static xmlSchemaBasicItemPtr xmlSchemaGetNextComponent(xmlSchemaBasicItemPtr item)
{
	switch(item->type) {
		...
		case XML_SCHEMA_TYPE_ATTRIBUTE: return ((xmlSchemaBasicItem *)((xmlSchemaAttribute *)item)->next);
		case XML_SCHEMA_TYPE_COMPLEX:
		case XML_SCHEMA_TYPE_SIMPLE: return ((xmlSchemaBasicItem *)((xmlSchemaType *)item)->next);
		case XML_SCHEMA_TYPE_ANY:
		...
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: return ((xmlSchemaBasicItem *)((xmlSchemaAttributeGroup *)item)->next);
		case XML_SCHEMA_TYPE_IDC_UNIQUE:
		case XML_SCHEMA_TYPE_IDC_KEY:
		case XML_SCHEMA_TYPE_IDC_KEYREF: return ((xmlSchemaBasicItem *)((xmlSchemaIDC *)item)->next);
		default: return 0;
	}
}
 ...
 *
 * Returns the given QName in the format "{namespaceName}localName" or just "localName" if @namespaceName is NULL.
 *
 * Returns the localName if @namespaceName is NULL, a formatted string otherwise.
 */
static const xmlChar * FASTCALL xmlSchemaFormatQName(xmlChar ** buf, const xmlChar * namespaceName, const xmlChar * localName)
{
	ZFREE(*buf);
	if(namespaceName) {
		*buf = sstrdup(reinterpret_cast<const xmlChar *>("{"));
	...
	if(localName) {
		...
		*buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("(NULL)"));
	}
	return static_cast<const xmlChar *>(*buf);
}

static const xmlChar* xmlSchemaFormatQNameNs(xmlChar ** buf, const xmlNs * ns, const xmlChar * localName)
{
	if(ns)
...
static const xmlChar * xmlSchemaGetComponentName(const xmlSchemaBasicItem * item)
{
	switch(item->type) {
		case XML_SCHEMA_TYPE_ELEMENT: return (((xmlSchemaElementPtr)item)->name);
		case XML_SCHEMA_TYPE_ATTRIBUTE: return (((xmlSchemaAttribute *)item)->name);
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: return reinterpret_cast<const xmlSchemaAttributeGroup *>(item)->name;
		case XML_SCHEMA_TYPE_BASIC:
		case XML_SCHEMA_TYPE_SIMPLE:
		case XML_SCHEMA_TYPE_COMPLEX: return (((xmlSchemaType *)item)->name);
		case XML_SCHEMA_TYPE_GROUP: return (((xmlSchemaModelGroupDefPtr)item)->name);
		case XML_SCHEMA_TYPE_IDC_KEY:
		case XML_SCHEMA_TYPE_IDC_UNIQUE:
		case XML_SCHEMA_TYPE_IDC_KEYREF: return reinterpret_cast<const xmlSchemaIDC *>(item)->name;
...
/*
   static const xmlChar * xmlSchemaGetQNameRefName(void *ref) { return (((xmlSchemaQNameRef *)ref)->name); }
   static const xmlChar * xmlSchemaGetQNameRefTargetNs(void *ref) { return (((xmlSchemaQNameRef *)ref)->targetNamespace); }
 */

static const xmlChar * xmlSchemaGetComponentTargetNs(const xmlSchemaBasicItem * item)
{
	switch(item->type) {
		case XML_SCHEMA_TYPE_ELEMENT: return reinterpret_cast<const xmlSchemaElement *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_ATTRIBUTE: return reinterpret_cast<const xmlSchemaAttribute *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: return reinterpret_cast<const xmlSchemaAttributeGroup *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_BASIC: return reinterpret_cast<const xmlChar *>("http://www.w3.org/2001/XMLSchema");
		case XML_SCHEMA_TYPE_SIMPLE:
		case XML_SCHEMA_TYPE_COMPLEX: return reinterpret_cast<const xmlSchemaType *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_GROUP: return reinterpret_cast<const xmlSchemaModelGroupDef *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_IDC_KEY:
		case XML_SCHEMA_TYPE_IDC_UNIQUE:
		case XML_SCHEMA_TYPE_IDC_KEYREF: return reinterpret_cast<const xmlSchemaIDC *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_ATTRIBUTE_USE:
		    if(WXS_ATTRUSE_DECL(item)) {
		...
		case XML_SCHEMA_EXTRA_QNAMEREF:
		    return reinterpret_cast<const xmlSchemaQNameRef *>(item)->targetNamespace;
		case XML_SCHEMA_TYPE_NOTATION:
...
static const xmlChar* xmlSchemaGetIDCDesignation(xmlChar ** buf, xmlSchemaIDC * idc)
{
...
{
	switch(pc) {
		case XML_SCHEMAS_ANY_SKIP: return (reinterpret_cast<const xmlChar *>("skip"));
		case XML_SCHEMAS_ANY_LAX: return (reinterpret_cast<const xmlChar *>("lax"));
		case XML_SCHEMAS_ANY_STRICT: return (reinterpret_cast<const xmlChar *>("strict"));
		default: return (reinterpret_cast<const xmlChar *>("invalid process contents"));
	}
}
/**
 ...
 */
static int xmlSchemaGetCanonValueWhtspExt(xmlSchemaVal * val, xmlSchemaWhitespaceValueType ws, xmlChar ** retValue)
{
	int list;
	...
	*retValue = NULL;
	do {
		...
		if(*retValue == NULL)
			...
			/* List. */
			*retValue = xmlStrcat((xmlChar *)*retValue, reinterpret_cast<const xmlChar *>(" "));
	...
	*buf = NULL;
	if(itemDes) {
		*buf = sstrdup(itemDes);
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_BASIC: {
			    xmlSchemaType * type = WXS_TYPE_CAST item;
			    if(WXS_IS_ATOMIC(type))
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("atomic type 'xs:"));
			    else if(WXS_IS_LIST(type))
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("list type 'xs:"));
			    else if(WXS_IS_UNION(type))
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("union type 'xs:"));
			    else
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("simple type 'xs:"));
			...
			case XML_SCHEMA_TYPE_SIMPLE: {
			    ...
			    if(type->flags & XML_SCHEMAS_TYPE_GLOBAL) {
				    ...
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("local "));
			    }
			    if(WXS_IS_ATOMIC(type))
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("atomic type"));
			    else if(WXS_IS_LIST(type))
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("list type"));
			    else if(WXS_IS_UNION(type))
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("union type"));
			    else
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("simple type"));
			    if(type->flags & XML_SCHEMAS_TYPE_GLOBAL) {
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>(" '"));
				    *buf = xmlStrcat(*buf, type->name);
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("'"));
			...
			case XML_SCHEMA_TYPE_COMPLEX: {
			    ...
			    if(type->flags & XML_SCHEMAS_TYPE_GLOBAL)
				    ...
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("local "));
			    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("complex type"));
			    if(type->flags & XML_SCHEMAS_TYPE_GLOBAL) {
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>(" '"));
				    *buf = xmlStrcat(*buf, type->name);
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("'"));
			...
			case XML_SCHEMA_TYPE_ATTRIBUTE_USE: {
			    ...
			    *buf = sstrdup(reinterpret_cast<const xmlChar *>("attribute use "));
			    if(WXS_ATTRUSE_DECL(ause)) {
				    ...
				    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("(unknown)"));
			    }
		    }
		    break;
			...
			case XML_SCHEMA_TYPE_ELEMENT: 
				{
					...
					*buf = sstrdup(reinterpret_cast<const xmlChar *>("element decl."));
					*buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>(" '"));
			...
			case XML_SCHEMA_TYPE_IDC_KEYREF:
			    if(item->type == XML_SCHEMA_TYPE_IDC_UNIQUE)
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("unique '"));
			    else if(item->type == XML_SCHEMA_TYPE_IDC_KEY)
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("key '"));
			    else
				    *buf = sstrdup(reinterpret_cast<const xmlChar *>("keyRef '"));
			    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlSchemaIDC *>(item)->name);
			    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("'"));
			    break;
			case XML_SCHEMA_TYPE_ANY:
			...
			case XML_SCHEMA_FACET_MINLENGTH:
			    *buf = sstrdup(reinterpret_cast<const xmlChar *>("facet '"));
			    *buf = xmlStrcat(*buf, xmlSchemaFacetTypeToString(item->type));
			    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>("'"));
			    break;
			case XML_SCHEMA_TYPE_GROUP: {
			    *buf = sstrdup(reinterpret_cast<const xmlChar *>("model group def."));
			    *buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>(" '"));
	...
	if(itemNode && (itemNode->type == XML_ATTRIBUTE_NODE)) {
		*buf = xmlStrcat(*buf, reinterpret_cast<const xmlChar *>(", attribute '"));
		if(itemNode->ns) {
 ...
 */
static const xmlChar * xmlSchemaFormatFacetEnumSet(xmlSchemaAbstractCtxt * actxt, xmlChar ** buf, xmlSchemaType * type)
{
 ...
 */
static void FASTCALL xmlSchemaPErrMemory(xmlSchemaParserCtxtPtr ctxt, const char * extra, xmlNode * pNode)
{
	if(ctxt)
		ctxt->nberrors++;
	__xmlSimpleError(XML_FROM_SCHEMASP, XML_ERR_NO_MEMORY, pNode, NULL, extra);
}
/**
 * xmlSchemaPErr:
 ...
 */
static void FASTCALL xmlSchemaPErr(xmlSchemaParserCtxtPtr ctxt, xmlNode * pNode, int error, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	...
	__xmlRaiseError(schannel, channel, data, ctxt, pNode, XML_FROM_SCHEMASP,
 ...
 * Handle an out of memory condition
 */
static void FASTCALL xmlSchemaVErrMemory(xmlSchemaValidCtxt * ctxt, const char * extra, xmlNode * P_Node)
{
	if(ctxt) {
		ctxt->nberrors++;
 ...
 */
static void xmlSchemaErr4Line(xmlSchemaAbstractCtxt * ctxt, xmlErrorLevel errorLevel,
    int error, xmlNode * P_Node, int line, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3, const xmlChar * str4)
{
	...
	if(ctxt) {
		if(ctxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
			xmlSchemaValidCtxt * vctxt = (xmlSchemaValidCtxt *)ctxt;
 ...
 * Handle a validation error
 */
static void xmlSchemaErr3(xmlSchemaAbstractCtxt * actxt, int error, xmlNode * P_Node, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
	xmlSchemaErr4Line(actxt, XML_ERR_ERROR, error, P_Node, 0, msg, str1, str2, str3, 0);
}

static void xmlSchemaErr4(xmlSchemaAbstractCtxt * actxt, int error, xmlNode * P_Node, const char * msg, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3, const xmlChar * str4)
{
	xmlSchemaErr4Line(actxt, XML_ERR_ERROR, error, P_Node, 0, msg, str1, str2, str3, str4);
}

static void xmlSchemaErr(xmlSchemaAbstractCtxt * actxt, int error, xmlNode * P_Node, const char * msg, const xmlChar * str1, const xmlChar * str2)
{
	xmlSchemaErr4(actxt, error, P_Node, msg, str1, str2, 0, 0);
}

static xmlChar * xmlSchemaFormatNodeForError(xmlChar ** msg, xmlSchemaAbstractCtxt * actxt, xmlNode * P_Node)
{
	xmlChar * str = NULL;
	*msg = NULL;
	if(P_Node && (P_Node->type != XML_ELEMENT_NODE) && (P_Node->type != XML_ATTRIBUTE_NODE)) {
		 ...
		 */
		*msg = sstrdup(reinterpret_cast<const xmlChar *>(""));
		return (*msg);
	}
	if(P_Node) {
		/*
		...
		if(P_Node->type == XML_ATTRIBUTE_NODE) {
			...
			*msg = sstrdup(reinterpret_cast<const xmlChar *>("Element '"));
			...
			ZFREE(str);
			*msg = xmlStrcat(*msg, reinterpret_cast<const xmlChar *>("', "));
			*msg = xmlStrcat(*msg, reinterpret_cast<const xmlChar *>("attribute '"));
		}
		else {
			*msg = sstrdup(reinterpret_cast<const xmlChar *>("Element '"));
		}
		...
		*msg = xmlStrcat(*msg, reinterpret_cast<const xmlChar *>("': "));
		...
		xmlSchemaValidCtxt * vctxt = (xmlSchemaValidCtxt *)actxt;
		/*
		 * Work on node infos.
		 */
...
static void xmlSchemaInternalErr2(xmlSchemaAbstractCtxt * actxt, const char * funcName, const char * message, const xmlChar * str1, const xmlChar * str2)
{
	if(actxt) {
		xmlChar * msg = sstrdup(reinterpret_cast<const xmlChar *>("Internal error: "));
		msg = xmlStrcat(msg, BAD_CAST funcName);
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(", "));
		msg = xmlStrcat(msg, BAD_CAST message);
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
		if(actxt->type == XML_SCHEMA_CTXT_VALIDATOR)
			xmlSchemaErr(actxt, XML_SCHEMAV_INTERNAL, NULL, reinterpret_cast<const char *>(msg), str1, str2);
		else if(actxt->type == XML_SCHEMA_CTXT_PARSER)
			xmlSchemaErr(actxt, XML_SCHEMAP_INTERNAL, NULL, reinterpret_cast<const char *>(msg), str1, str2);
		ZFREE(msg);
	}
}
...
static void xmlSchemaCustomErr4(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error,
    xmlNode * pNode, xmlSchemaBasicItem * item, const char * message, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3, const xmlChar * str4)
{
	xmlChar * msg = NULL;
	if(!pNode && item && (actxt->type == XML_SCHEMA_CTXT_PARSER)) {
		pNode = WXS_ITEM_NODE(item);
		xmlSchemaFormatItemForReport(&msg, NULL, item, 0);
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(": "));
	}
	else
		xmlSchemaFormatNodeForError(&msg, actxt, pNode);
	msg = xmlStrcat(msg, (const xmlChar *)message);
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
	xmlSchemaErr4(actxt, error, pNode, reinterpret_cast<const char *>(msg), str1, str2, str3, str4);
	ZFREE(msg);
}

static void xmlSchemaCustomErr(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error, xmlNode * pNode,
    xmlSchemaBasicItem * item, const char * message, const xmlChar * str1, const xmlChar * str2)
{
	xmlSchemaCustomErr4(actxt, error, pNode, item, message, str1, str2, 0, 0);
}

static void xmlSchemaCustomWarning(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error,
    xmlNode * pNode, xmlSchemaType * type ATTRIBUTE_UNUSED, const char * message,
    const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
	...
	xmlSchemaErr4Line(actxt, XML_ERR_WARNING, error, pNode, 0, reinterpret_cast<const char *>(msg), str1, str2, str3, 0);
	ZFREE(msg);
}

static void xmlSchemaKeyrefErr(xmlSchemaValidCtxt * vctxt, xmlParserErrors error,
    xmlSchemaPSVIIDCNodePtr idcNode, xmlSchemaType * type ATTRIBUTE_UNUSED,
    const char * message, const xmlChar * str1, const xmlChar * str2)
{
	xmlChar * qname = NULL;
	xmlChar * msg = sstrdup(reinterpret_cast<const xmlChar *>("Element '%s': "));
	msg = xmlStrcat(msg, (const xmlChar *)message);
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
	xmlSchemaErr4Line(ACTXT_CAST vctxt, XML_ERR_ERROR, error, NULL, idcNode->nodeLine, reinterpret_cast<const char *>(msg), xmlSchemaFormatQName(&qname,
		(xmlChar *)vctxt->nodeQNames->items[idcNode->nodeQNameID +1], (xmlChar *)vctxt->nodeQNames->items[idcNode->nodeQNameID]), str1, str2, 0);
	ZFREE(qname);
	ZFREE(msg);
}

static int xmlSchemaEvalErrorNodeType(xmlSchemaAbstractCtxt * actxt, const xmlNode * pNode)
{
	if(pNode)
		return (pNode->type);
	if((actxt->type == XML_SCHEMA_CTXT_VALIDATOR) && ((xmlSchemaValidCtxt *)actxt)->inode)
		return ( ((xmlSchemaValidCtxt *)actxt)->inode->nodeType);
	return -1;
}

static int xmlSchemaIsGlobalItem(xmlSchemaType * item)
{
	switch(item->type) {
		case XML_SCHEMA_TYPE_COMPLEX:
		case XML_SCHEMA_TYPE_SIMPLE:
		...
		case XML_SCHEMA_TYPE_ELEMENT:
		    ...
		    break;
		case XML_SCHEMA_TYPE_ATTRIBUTE:
		    if(((xmlSchemaAttribute *)item)->flags & XML_SCHEMAS_ATTR_GLOBAL)
			    return 1;
		    break;
...
static void xmlSchemaSimpleTypeErr(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error, xmlNode * P_Node, const xmlChar * value, xmlSchemaType * type, int displayValue)
{
	xmlChar * msg = NULL;
	xmlSchemaFormatNodeForError(&msg, actxt, P_Node);
	if(displayValue || (xmlSchemaEvalErrorNodeType(actxt, P_Node) == XML_ATTRIBUTE_NODE))
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("'%s' is not a valid value of "));
	else
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The character content is not a valid value of "));
	if(!xmlSchemaIsGlobalItem(type))
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("the local "));
	else
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("the "));
	if(WXS_IS_ATOMIC(type))
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("atomic type"));
	else if(WXS_IS_LIST(type))
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("list type"));
	else if(WXS_IS_UNION(type))
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("union type"));
	if(xmlSchemaIsGlobalItem(type)) {
		xmlChar * str = NULL;
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" '"));
		if(type->builtInType != 0) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("xs:"));
	...
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
	if(displayValue || (xmlSchemaEvalErrorNodeType(actxt, P_Node) == XML_ATTRIBUTE_NODE))
		xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, 0);
	else
		xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), 0, 0);
	ZFREE(msg);
}

static const xmlChar * xmlSchemaFormatErrorNodeQName(xmlChar ** str, const xmlSchemaNodeInfo * pNi, const xmlNode * pNode)
{
	if(pNode) {
		if(pNode->ns)
			return (xmlSchemaFormatQName(str, pNode->ns->href, pNode->name));
		else
			return (xmlSchemaFormatQName(str, NULL, pNode->name));
	}
	else if(pNi)
		return (xmlSchemaFormatQName(str, pNi->nsName, pNi->localName));
	return 0;
}

static void xmlSchemaIllegalAttrErr(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error, xmlSchemaAttrInfo * ni, xmlNode * P_Node)
{
	xmlChar * msg = NULL, * str = NULL;
	xmlSchemaFormatNodeForError(&msg, actxt, P_Node);
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The attribute '%s' is not allowed.\n"));
	xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), xmlSchemaFormatErrorNodeQName(&str, (xmlSchemaNodeInfoPtr)ni, P_Node), 0);
	ZFREE(str);
	ZFREE(msg);
}

static void xmlSchemaComplexTypeErr(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error, xmlNode * P_Node,
    xmlSchemaType * type ATTRIBUTE_UNUSED, const char * message, int nbval, int nbneg, xmlChar ** values)
{
	...
	if(nbval + nbneg > 0) {
		if(nbval + nbneg > 1)
			str = sstrdup(reinterpret_cast<const xmlChar *>(" Expected is one of ( "));
		else
			str = sstrdup(reinterpret_cast<const xmlChar *>(" Expected is ( "));
		nsName = NULL;
		for(i = 0; i < nbval + nbneg; i++) {
			...
			if((cur[0] == 'n') && (cur[1] == 'o') && (cur[2] == 't') && (cur[3] == ' ')) {
				cur += 4;
				str = xmlStrcat(str, reinterpret_cast<const xmlChar *>("##other"));
			}
			/*
			 * Get the local name.
			 */
			...
			if(*end == '*') {
				localName = sstrdup(reinterpret_cast<const xmlChar *>("*"));
				end++;
			}
			else {
				while((*end != 0) && (*end != '|'))
			...
			if(*end != 0) {
				...
				if((nbneg == 0) || (*end != '*') || (*localName != '*')) {
					...
					cur = end;
					if(*end == '*') {
						nsName = sstrdup(reinterpret_cast<const xmlChar *>("{*}"));
					}
					else {
						while(*end != 0)
							end++;
						if(i >= nbval)
							nsName = sstrdup(reinterpret_cast<const xmlChar *>("{##other:"));
						else
							nsName = sstrdup(reinterpret_cast<const xmlChar *>("{"));
						nsName = xmlStrncat(nsName, BAD_CAST cur, end - cur);
						nsName = xmlStrcat(nsName, reinterpret_cast<const xmlChar *>("}"));
					}
			...
			ZFREE(localName);
			if(i < nbval + nbneg -1)
				str = xmlStrcat(str, reinterpret_cast<const xmlChar *>(", "));
		}
		str = xmlStrcat(str, reinterpret_cast<const xmlChar *>(" ).\n"));
...
static void xmlSchemaFacetErr(xmlSchemaAbstractCtxt * actxt, xmlParserErrors error, xmlNode * P_Node, const xmlChar * value,
    unsigned long length, xmlSchemaType * type, xmlSchemaFacet * facet, const char * message, const xmlChar * str1, const xmlChar * str2)
{
	xmlChar * str = NULL, * msg = NULL;
	...
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("["));
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("facet '"));
	...
	if(message == NULL) {
		...
		if(oneof3(facetType, XML_SCHEMA_FACET_LENGTH, XML_SCHEMA_FACET_MINLENGTH, XML_SCHEMA_FACET_MAXLENGTH)) {
			...
			if(nodeType == XML_ATTRIBUTE_NODE)
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' has a length of '%s'; "));
			else
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value has a length of '%s'; "));
			snprintf(len, 24, "%lu", xmlSchemaGetFacetValueAsULong(facet));
			snprintf(actLen, 24, "%lu", length);
			if(facetType == XML_SCHEMA_FACET_LENGTH)
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("this differs from the allowed length of '%s'.\n"));
			else if(facetType == XML_SCHEMA_FACET_MAXLENGTH)
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("this exceeds the allowed maximum length of '%s'.\n"));
			else if(facetType == XML_SCHEMA_FACET_MINLENGTH)
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("this underruns the allowed minimum length of '%s'.\n"));
			if(nodeType == XML_ATTRIBUTE_NODE)
				xmlSchemaErr3(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, (const xmlChar *)actLen, (const xmlChar *)len);
			else
				xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), (const xmlChar *)actLen, (const xmlChar *)len);
		}
		else if(facetType == XML_SCHEMA_FACET_ENUMERATION) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is not an element of the set {%s}.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, xmlSchemaFormatFacetEnumSet(actxt, &str, type));
		}
		else if(facetType == XML_SCHEMA_FACET_PATTERN) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is not accepted by the pattern '%s'.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_MININCLUSIVE) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is less than the minimum value allowed ('%s').\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_MAXINCLUSIVE) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is greater than the maximum value allowed ('%s').\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_MINEXCLUSIVE) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' must be greater than '%s'.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_MAXEXCLUSIVE) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' must be less than '%s'.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_TOTALDIGITS) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' has more digits than are allowed ('%s').\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(facetType == XML_SCHEMA_FACET_FRACTIONDIGITS) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' has more fractional digits than are allowed ('%s').\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, facet->value);
		}
		else if(nodeType == XML_ATTRIBUTE_NODE) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is not facet-valid.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), value, 0);
		}
		else {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value is not facet-valid.\n"));
			xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), 0, 0);
		}
	}
	else {
		msg = xmlStrcat(msg, (const xmlChar *)message);
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
		xmlSchemaErr(actxt, error, P_Node, reinterpret_cast<const char *>(msg), str1, str2);
	}
	ZFREE(str);
	SAlloc::F(msg);
}
 ...
 */
static void FASTCALL xmlSchemaPMissingAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaBasicItem * ownerItem,
...
static void xmlSchemaPCustomAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlChar ** ownerDes,
    xmlSchemaBasicItem * ownerItem, xmlAttr * attr, const char * msg)
{
	xmlChar * des = NULL;
	if(ownerDes == NULL)
 ...
 * Reports an illegal attribute during the parse.
 */
static void xmlSchemaPIllegalAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaBasicItem * ownerComp ATTRIBUTE_UNUSED, xmlAttr * attr)
{
    ...
    xmlSchemaBasicItem * item, xmlNode * itemElem, const char * message, const xmlChar * str1, const xmlChar * str2, const xmlChar * str3)
{
	...
	msg = sstrdup(reinterpret_cast<const xmlChar *>("%s: "));
	msg = xmlStrcat(msg, (const xmlChar *)message);
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
	if((itemElem == NULL) && item)
		itemElem = WXS_ITEM_NODE(item);
	xmlSchemaPErrExt(ctxt, itemElem, error, NULL, NULL, NULL, reinterpret_cast<const char *>(msg), BAD_CAST des, str1, str2, str3, 0);
	ZFREE(des);
	ZFREE(msg);
}
    ...
    const xmlChar * str1, const xmlChar * str2, const xmlChar * str3, const xmlChar * str4)
{
	...
	msg = xmlStrcat(msg, (const xmlChar *)message);
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
	xmlSchemaErr4(ACTXT_CAST ctxt, error, P_Node, reinterpret_cast<const char *>(msg), str1, str2, str3, str4);
	SAlloc::F(msg);
}
 ...
 */
static void xmlSchemaPIllegalFacetListUnionErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error, xmlSchemaType * type, xmlSchemaFacet * facet)
{
	xmlChar * des = NULL;
	xmlSchemaFormatItemForReport(&des, NULL, WXS_BASIC_CAST type, type->P_Node);
...
static void xmlSchemaPMutualExclAttrErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
    xmlSchemaBasicItem * ownerItem, xmlAttr * attr, const char * name1, const char * name2)
{
	xmlChar * des = NULL;
 ...
 */
static void FASTCALL xmlSchemaPSimpleTypeErr(xmlSchemaParserCtxtPtr ctxt, xmlParserErrors error,
    xmlSchemaBasicItem * ownerItem ATTRIBUTE_UNUSED, xmlNode * pNode, xmlSchemaType * type,
    const char * expected, const xmlChar * value, const char * message, const xmlChar * str1, const xmlChar * str2)
{
	xmlChar * msg = NULL;
	xmlSchemaFormatNodeForError(&msg, ACTXT_CAST ctxt, pNode);
	if(message == NULL) {
		/*
		...
		if(type) {
			...
			if(pNode->type == XML_ATTRIBUTE_NODE)
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The value '%s' is not valid."));
			else
				msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("The character content is not valid."));
		}
		if(expected) {
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" Expected is '"));
			...
			msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("\n"));
		if(pNode->type == XML_ATTRIBUTE_NODE)
			xmlSchemaPErr(ctxt, pNode, error, reinterpret_cast<const char *>(msg), value, 0);
		else
			xmlSchemaPErr(ctxt, pNode, error, reinterpret_cast<const char *>(msg), 0, 0);
	}
	else {
		msg = xmlStrcat(msg, BAD_CAST message);
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(".\n"));
		xmlSchemaPErrExt(ctxt, pNode, error, NULL, NULL, NULL, reinterpret_cast<const char *>(msg), str1, str2, NULL, 0, 0);
	}
	/* Cleanup. */
	ZFREE(msg);
}
...
{
	xmlSchemaFacet * ret = static_cast<xmlSchemaFacet *>(SAlloc::M(sizeof(xmlSchemaFacet)));
	if(!ret) {
		return 0;
	}
...
{
	xmlSchemaAnnotPtr ret = (xmlSchemaAnnotPtr)SAlloc::M(sizeof(xmlSchemaAnnot));
	if(!ret) {
...
static xmlSchemaItemList * xmlSchemaItemListCreate()
{
	xmlSchemaItemList * ret = (xmlSchemaItemList *)SAlloc::M(sizeof(xmlSchemaItemList));
	if(!ret) {
...
static void xmlSchemaItemListClear(xmlSchemaItemList * list)
{
	if(list) {
		SAlloc::F(list->items);
		list->items = NULL;
...
static int xmlSchemaItemListAdd(xmlSchemaItemList * list, void * item)
{
	if(list->items == NULL) {
...
static int xmlSchemaItemListAddSize(xmlSchemaItemList * list, int initialSize, void * item)
{
	if(list->items == NULL) {
		if(initialSize <= 0)
			initialSize = 1;
...
static int xmlSchemaItemListInsert(xmlSchemaItemList * list, void * item, int idx)
{
	if(list->items == NULL) {
		list->items = (void **)SAlloc::M(20 * sizeof(void *));
		if(list->items == NULL) {
...
static int xmlSchemaItemListInsertSize(xmlSchemaItemList * list, int initialSize, void * item, int idx)
{
	if(list->items == NULL) {
		if(initialSize <= 0)
			initialSize = 1;
...
static int xmlSchemaItemListRemove(xmlSchemaItemList * list, int idx)
{
	int i;
...
static void FASTCALL xmlSchemaBucketFree(xmlSchemaBucket * pBucket)
{
	if(pBucket) {
		if(pBucket->globals) {
...
static xmlSchemaBucket * xmlSchemaBucketCreate(xmlSchemaParserCtxtPtr pctxt, int type, const xmlChar * targetNamespace)
{
	xmlSchemaBucket * ret;
	int size;
	xmlSchemaPtr mainSchema;
	if(WXS_CONSTRUCTOR(pctxt)->mainSchema == NULL) {
		PERROR_INT(__FUNCTION__, "no main schema on constructor");
		return 0;
	}
	mainSchema = WXS_CONSTRUCTOR(pctxt)->mainSchema;
	/* Create the schema bucket. */
	size = WXS_IS_BUCKET_INCREDEF(type) ? sizeof(xmlSchemaInclude) : sizeof(xmlSchemaImport);
	ret = (xmlSchemaBucket *)SAlloc::M(size);
	if(!ret) {
	...
	if(!WXS_HAS_BUCKETS(pctxt)) {
		if(WXS_IS_BUCKET_INCREDEF(type)) {
			PERROR_INT(__FUNCTION__, "first bucket but it's an include or redefine");
		...
		if(type == XML_SCHEMA_SCHEMA_MAIN) {
			PERROR_INT(__FUNCTION__, "main bucket but it's not the first one");
			xmlSchemaBucketFree(ret);
			return 0;
		}
	...
	if(WXS_IS_BUCKET_IMPMAIN(type)) {
		...
		if(res != 0) {
			PERROR_INT(__FUNCTION__, "failed to add the schema bucket to the hash");
			xmlSchemaBucketFree(ret);
			return 0;
		}
	}
...
static int xmlSchemaAddItemSize(xmlSchemaItemList ** list, int initialSize, void * item)
{
	if(*list == NULL) {
 ...
 */
static void xmlSchemaFreeAttributeGroup(xmlSchemaAttributeGroup * attrGr)
{
	if(attrGr) {
 ...
 */
static void xmlSchemaFreeQNameRef(xmlSchemaQNameRef * item)
{
	SAlloc::F(item);
}
/**
...
static void xmlSchemaFreeIDCStateObjList(xmlSchemaIDCStateObj * sto)
{
	while(sto) {
		...
		xmlFreeStreamCtxt((xmlStreamCtxt *)sto->xpathCtxt);
		SAlloc::F(sto);
		sto = next;
	}
}
 ...
 */
static void xmlSchemaFreeIDC(xmlSchemaIDC * idcDef)
{
	if(idcDef) {
		xmlSchemaIDCSelectPtr cur;
		xmlSchemaIDCSelectPtr prev;
...
void xmlSchemaFreeType(xmlSchemaType * pType)
{
	if(pType) {
		...
		xmlSchemaItemListFree((xmlSchemaItemList *)pType->attrUses);
...
static void FASTCALL xmlSchemaComponentListFree(xmlSchemaItemList * list)
{
	if(list || list->nbItems) {
		...
		xmlSchemaTreeItem ** items = reinterpret_cast<xmlSchemaTreeItem **>(list->items);
		int i;
		for(i = 0; i < list->nbItems; i++) {
			...
			switch(item->type) {
				case XML_SCHEMA_TYPE_SIMPLE:
				case XML_SCHEMA_TYPE_COMPLEX: xmlSchemaFreeType((xmlSchemaType *)item); break;
				case XML_SCHEMA_TYPE_ATTRIBUTE: xmlSchemaFreeAttribute((xmlSchemaAttribute *)item); break;
				case XML_SCHEMA_TYPE_ATTRIBUTE_USE: xmlSchemaFreeAttributeUse((xmlSchemaAttributeUse *)item); break;
				case XML_SCHEMA_EXTRA_ATTR_USE_PROHIB: xmlSchemaFreeAttributeUseProhib((xmlSchemaAttributeUseProhibPtr)item); break;
				case XML_SCHEMA_TYPE_ELEMENT: xmlSchemaFreeElement((xmlSchemaElementPtr)item); break;
				case XML_SCHEMA_TYPE_PARTICLE: 
					xmlSchemaFreeAnnot(item->annot);
				    SAlloc::F(item);
				    break;
				case XML_SCHEMA_TYPE_SEQUENCE:
				case XML_SCHEMA_TYPE_CHOICE:
				case XML_SCHEMA_TYPE_ALL: xmlSchemaFreeModelGroup((xmlSchemaModelGroupPtr)item); break;
				case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: xmlSchemaFreeAttributeGroup(reinterpret_cast<xmlSchemaAttributeGroup *>(item)); break;
				...
				case XML_SCHEMA_TYPE_IDC_KEY:
				case XML_SCHEMA_TYPE_IDC_UNIQUE:
				case XML_SCHEMA_TYPE_IDC_KEYREF: xmlSchemaFreeIDC(reinterpret_cast<xmlSchemaIDC *>(item)); break;
				case XML_SCHEMA_TYPE_NOTATION: xmlSchemaFreeNotation((xmlSchemaNotation *)item); break;
				case XML_SCHEMA_EXTRA_QNAMEREF: xmlSchemaFreeQNameRef((xmlSchemaQNameRef *)item); break;
				default: {
...
void FASTCALL xmlSchemaFree(xmlSchema * schema)
{
	if(schema) {
		...
		if(schema->includes) {
			xmlSchemaItemList * list = (xmlSchemaItemList *)schema->includes;
			for(int i = 0; i < list->nbItems; i++) {
				xmlSchemaBucketFree((xmlSchemaBucket *)list->items[i]);
			}
...
static void xmlSchemaTypeDump(xmlSchemaType * type, FILE * output); /* forward */
/**
 ...
 */
static void xmlSchemaAttrUsesDump(xmlSchemaItemList * uses, FILE * output)
{
 ...
 * Dump a SchemaType structure
 */
static void xmlSchemaTypeDump(xmlSchemaType * type, FILE * output)
{
	if(type == NULL) {
	...
	if(type->attrUses)
		xmlSchemaAttrUsesDump((xmlSchemaItemList *)type->attrUses, output);
	if(type->annot)
 ...
 */
static xmlAttr * FASTCALL xmlSchemaGetPropNode(xmlNode * pNode, const char * name)
{
	if(pNode && name) {
		for(xmlAttr * prop = pNode->properties; prop; prop = prop->next)
 ...
 * Returns the attribute or NULL if not present.
 */
static xmlAttr * xmlSchemaGetPropNodeNs(xmlNode * pNode, const char * uri, const char * name)
{
	if(pNode && name) {
		for(xmlAttr * prop = pNode->properties; prop; prop = prop->next) {
...
static const xmlChar * FASTCALL xmlSchemaGetNodeContent(xmlSchemaParserCtxtPtr ctxt, xmlNode * pNode)
{
	const xmlChar * ret;
	xmlChar * val = xmlNodeGetContent(pNode);
...
static const xmlChar * xmlSchemaGetNodeContentNoDict(const xmlNode * pNode)
{
	return static_cast<const xmlChar *>(xmlNodeGetContent(pNode));
}
/**
 * xmlSchemaGetProp:
 ...
 */
static const xmlChar * xmlSchemaGetProp(xmlSchemaParserCtxtPtr ctxt, const xmlNode * pNode, const char * name)
{
	const xmlChar * ret = 0;
	xmlChar * val = xmlGetNoNsProp(pNode, BAD_CAST name);
	if(val) {
		ret = xmlDictLookupSL(ctxt->dict, val);
		SAlloc::F(val);
	}
...
#define WXS_FIND_GLOBAL_ITEM(slot, RetType) \
	if(sstreq(nsName, schema->targetNamespace)) { \
		ret = static_cast<RetType>(xmlHashLookup(schema->slot, name)); \
		if(ret) goto exit; \
	} \
	if(xmlHashSize(schema->schemasImports) > 1) { \
		xmlSchemaImportPtr import = static_cast<xmlSchemaImportPtr>(xmlHashLookup(schema->schemasImports, nsName ? nsName : XML_SCHEMAS_NO_NAMESPACE)); \
		if(import == NULL) goto exit; \
		ret = static_cast<RetType>(xmlHashLookup(import->schema->slot, name)); \
	}
/**
 * xmlSchemaGetElem:
 ...
 */
static xmlSchemaType * xmlSchemaGetType(xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName)
{
	xmlSchemaType * ret = NULL;
	if(!name)
		return 0;
 ...
 */
static xmlSchemaAttribute * xmlSchemaGetAttributeDecl(xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName)
{
	xmlSchemaAttribute * ret = NULL;
	if(!name || !schema)
		return 0;
	if(schema) {
 ...
 */
static xmlSchemaAttributeGroup * xmlSchemaGetAttributeGroup(xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName)
{
	xmlSchemaAttributeGroup * ret = NULL;
	if(!name || !schema)
		return 0;
...
static xmlSchemaIDC * xmlSchemaGetIDC(xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName)
{
	xmlSchemaIDC * ret = NULL;
	if(!name || !schema)
		return 0;
	WXS_FIND_GLOBAL_ITEM(idcDef, xmlSchemaIDC *)
exit:
	return ret;
}
/**
...
{
	switch(itemType) {
		case XML_SCHEMA_TYPE_GROUP: return reinterpret_cast<xmlSchemaBasicItem *>(xmlSchemaGetGroup(schema, name, targetNs));
		case XML_SCHEMA_TYPE_ELEMENT: return reinterpret_cast<xmlSchemaBasicItem *>(xmlSchemaGetElem(schema, name, targetNs));
		default:
		    TODO
		    return 0;
	}
...
// Parsing functions
// 
#define IS_BLANK_NODE(n) (((n)->type == XML_TEXT_NODE) && (xmlSchemaIsBlank((n)->content, -1)))
/**
 * xmlSchemaIsBlank:
 * @str:  a string
 ...
 */
static int FASTCALL xmlSchemaIsBlank(const xmlChar * str, int len)
{
	if(!str)
		return 1;
	if(len < 0) {
...
#define WXS_COMP_NAME(c, t) (reinterpret_cast<const t>(c))->name
#define WXS_COMP_TNS(c, t) (reinterpret_cast<const t>(c))->targetNamespace
/*
 * xmlSchemaFindRedefCompInGraph:
 * ATTENTION TODO: This uses pointer comp. for strings.
 */
static xmlSchemaBasicItem * xmlSchemaFindRedefCompInGraph(xmlSchemaBucket * bucket, xmlSchemaTypeType type, const xmlChar * name, const xmlChar * nsName)
{
	...
	for(i = 0; i < bucket->globals->nbItems; i++) {
		...
		if(ret->type == type) {
			switch(type) {
				case XML_SCHEMA_TYPE_COMPLEX:
				case XML_SCHEMA_TYPE_SIMPLE:
				    if((WXS_COMP_NAME(ret, xmlSchemaType *) == name) && (WXS_COMP_TNS(ret, xmlSchemaType *) == nsName)) {
					    return ret;
				    }
				    break;
				case XML_SCHEMA_TYPE_GROUP:
				    if((WXS_COMP_NAME(ret, xmlSchemaModelGroupDefPtr) == name) && (WXS_COMP_TNS(ret, xmlSchemaModelGroupDefPtr) == nsName)) {
					    return ret;
				    }
				    break;
				case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
				    if((WXS_COMP_NAME(ret, xmlSchemaAttributeGroup *) == name) && (WXS_COMP_TNS(ret, xmlSchemaAttributeGroup *) == nsName)) {
					    return ret;
				    }
				    break;
				default:
...
subschemas:
	 ...
	 */
	if(bucket->relations) {
		xmlSchemaSchemaRelation * rel = bucket->relations;
		/*
    ...
    const xmlChar * name, const xmlChar * nsName, xmlNode * P_Node ATTRIBUTE_UNUSED)
{
	...
	// @todo do we need the node to be set? ret->node = node;
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaAttribute * xmlSchemaAddAttribute(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName, xmlNode * P_Node, int topLevel)
{
	xmlSchemaAttribute * ret = NULL;
	if(!ctxt || (schema == NULL))
		return 0;
	ret = (xmlSchemaAttribute *)SAlloc::M(sizeof(xmlSchemaAttribute));
	if(!ret) {
...
static xmlSchemaAttributeUsePtr xmlSchemaAddAttributeUse(xmlSchemaParserCtxtPtr pctxt, xmlNode * P_Node)
{
	...
	if(pctxt == NULL)
		return 0;
	ret = (xmlSchemaAttributeUse *)SAlloc::M(sizeof(xmlSchemaAttributeUse));
	if(!ret) {
 ...
 */
static xmlSchemaRedefPtr xmlSchemaAddRedef(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBucket * targetBucket, void * item, const xmlChar * refName, const xmlChar * refTargetNs)
{
	xmlSchemaRedefPtr ret = (xmlSchemaRedefPtr)SAlloc::M(sizeof(xmlSchemaRedef));
	if(!ret) {
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaAttributeGroup * xmlSchemaAddAttributeGroupDefinition(xmlSchemaParserCtxtPtr pctxt,
    xmlSchemaPtr schema ATTRIBUTE_UNUSED, const xmlChar * name, const xmlChar * nsName, xmlNode * P_Node)
{
	xmlSchemaAttributeGroup * ret = NULL;
	if(!pctxt || !name)
		return 0;
	ret = static_cast<xmlSchemaAttributeGroup *>(SAlloc::M(sizeof(xmlSchemaAttributeGroup)));
	if(!ret) {
 ...
 * Returns the new struture or NULL in case of error
 */
static xmlSchemaType * xmlSchemaAddType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema,
    xmlSchemaTypeType type, const xmlChar * name, const xmlChar * nsName, xmlNode * P_Node, int topLevel)
{
	xmlSchemaType * ret = NULL;
	if(!ctxt || (schema == NULL))
		return 0;
	ret = (xmlSchemaType *)SAlloc::M(sizeof(xmlSchemaType));
	if(!ret) {
...
static xmlSchemaQNameRef * xmlSchemaNewQNameRef(xmlSchemaParserCtxtPtr pctxt, xmlSchemaTypeType refType, const xmlChar * refName, const xmlChar * refNs)
{
	xmlSchemaQNameRef * ret = (xmlSchemaQNameRef *)SAlloc::M(sizeof(xmlSchemaQNameRef));
	if(!ret) {
...
static xmlSchemaIDC * xmlSchemaAddIDC(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, const xmlChar * name, const xmlChar * nsName, int category, xmlNode * P_Node)
{
	xmlSchemaIDC * ret = NULL;
	if(!ctxt || (schema == NULL) || !name)
		return 0;
	ret = static_cast<xmlSchemaIDC *>(SAlloc::M(sizeof(xmlSchemaIDC)));
	if(!ret) {
...
static xmlSchemaSubstGroup * xmlSchemaSubstGroupAdd(xmlSchemaParserCtxtPtr pctxt, xmlSchemaElementPtr head)
{
	...
	if(xmlHashAddEntry2(WXS_SUBST_GROUPS(pctxt), head->name, head->targetNamespace, ret) != 0) {
		PERROR_INT(__FUNCTION__, "failed to add a new substitution container");
		xmlSchemaSubstGroupFree(ret);
		return 0;
	}
	return ret;
...
static int xmlSchemaPValAttrNodeQNameValue(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlSchemaBasicItem * ownerItem,
    xmlAttr * attr, const xmlChar * value, const xmlChar ** uri, const xmlChar ** local) 
{
	const xmlChar * pref;
	xmlNs * ns;
	int len, ret;
...
static int xmlSchemaPValAttrNodeQName(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlSchemaBasicItem * ownerItem,
    xmlAttr * attr, const xmlChar ** uri, const xmlChar ** local)
{
 ...
 * if not valid and -1 if an internal error occurs.
 */
static int xmlSchemaPValAttrNodeID(xmlSchemaParserCtxtPtr ctxt, xmlAttr * attr)
{
	int ret;
	const xmlChar * value;
	if(!attr)
		return 0;
	value = xmlSchemaGetNodeContentNoDict(reinterpret_cast<const xmlNode *>(attr));
	...
	if(!ret) {
		...
		ret = XML_SCHEMAP_S4S_ATTR_INVALID_VALUE;
		xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNode *)attr, xmlSchemaGetBuiltInType(XML_SCHEMAS_ID),
		    NULL, NULL, "The value '%s' of simple type 'xs:ID' is not a valid 'xs:NCName'", value, 0);
	}
	SAlloc::F((xmlChar *)value);
	return ret;
}
...
{
	xmlAttr * attr = xmlSchemaGetPropNode(ownerElem, (const char *)name);
	return attr ? xmlSchemaPValAttrNodeID(ctxt, attr) : 0;
}
/**
 * xmlGetMaxOccurs:
 * @ctxt:  a schema validation context
...
static int xmlGetMaxOccurs(xmlSchemaParserCtxtPtr ctxt, xmlNode * P_Node, int min, int max, int def, const char * expected)
{
	...
	int ret = 0;
	xmlAttr * attr = xmlSchemaGetPropNode(P_Node, "maxOccurs");
	if(!attr)
		return (def);
...
static int xmlGetMinOccurs(xmlSchemaParserCtxtPtr ctxt, xmlNode * P_Node, int min, int max, int def, const char * expected)
{
	const xmlChar * val, * cur;
	int ret = 0;
	xmlAttr * attr = xmlSchemaGetPropNode(P_Node, "minOccurs");
	if(!attr)
		return (def);
	val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
	cur = val;
...
// 
static xmlSchemaType * xmlSchemaParseSimpleType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, int topLevel);
static xmlSchemaType * xmlSchemaParseComplexType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, int topLevel);
static xmlSchemaType * xmlSchemaParseRestriction(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, xmlSchemaTypeType parentType);
static xmlSchemaBasicItem * xmlSchemaParseLocalAttribute(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNode * P_Node, xmlSchemaItemList * uses, int parentType);
 ...
 */
static int xmlSchemaPValAttrNodeValue(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBasicItem * ownerItem, xmlAttr * attr, const xmlChar * value, xmlSchemaType * type)
{
	int ret = 0;
	/*
	...
	if(type->type != XML_SCHEMA_TYPE_BASIC) {
		PERROR_INT(__FUNCTION__, "the given type is not a built-in type");
		return -1;
	}
	switch(type->builtInType) {
		...
		case XML_SCHEMAS_LANGUAGE:
		    ...
		    break;
		default: {
		    PERROR_INT(__FUNCTION__, "validation using the given type is not supported while parsing a schema");
		    return -1;
	    }
	}
	/*
	 ...
	 */
	if(ret < 0) {
		PERROR_INT(__FUNCTION__, "failed to validate a schema attribute value");
		return -1;
	}
	else if(ret > 0) {
		if(WXS_IS_LIST(type))
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int FASTCALL xmlSchemaPValAttrNode(xmlSchemaParserCtxtPtr ctxt, xmlSchemaBasicItem * ownerItem, xmlAttr * attr, xmlSchemaType * type, const xmlChar ** value)
{
	if(!ctxt || !type || !attr)
		return -1;
	else {
		const xmlChar * val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
		ASSIGN_PTR(value, val);
...
static int xmlSchemaPValAttr(xmlSchemaParserCtxtPtr ctxt, xmlSchemaBasicItem * ownerItem, xmlNode * ownerElem,
    const char * name, xmlSchemaType * type, const xmlChar ** value)
{
	if(!ctxt || (type == NULL)) {
		ASSIGN_PTR(value, 0);
		return -1;
...
static int xmlSchemaCheckReference(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema ATTRIBUTE_UNUSED,
    xmlNode * pNode, xmlAttr * attr, const xmlChar * namespaceName)
{
	...
	if(WXS_BUCKET(pctxt)->relations) {
		xmlSchemaSchemaRelation * rel = WXS_BUCKET(pctxt)->relations;
		do {
	 ...
	 */
	{
		xmlNode * n = attr ? reinterpret_cast<xmlNode *>(attr) : pNode;
		if(namespaceName == NULL)
 ...
 */
static int xmlSchemaParseLocalAttributes(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * * child, xmlSchemaItemList ** list, int parentType, int * hasRefs)
{
	void * item;
...
static xmlSchemaAnnotPtr xmlSchemaParseAnnotation(xmlSchemaParserCtxtPtr ctxt, xmlNode * P_Node, int needed)
{
	...
	if(!ctxt || !P_Node)
		return 0;
	ret = needed ? xmlSchemaNewAnnot(ctxt, P_Node) : NULL;
	attr = P_Node->properties;
	while(attr) {
		if((!attr->ns && (!sstreq(attr->name, "id"))) || (attr->ns && sstreq(attr->ns->href, xmlSchemaNs))) {
 ...
 */
static xmlSchemaFacet * xmlSchemaParseFacet(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node)
{
	...
	if(ctxt && schema && P_Node) {
		...
		if(!facet) {
			...
			if(IS_SCHEMA(P_Node, "minInclusive")) { facet->type = XML_SCHEMA_FACET_MININCLUSIVE; }
			else if(IS_SCHEMA(P_Node, "minExclusive")) { facet->type = XML_SCHEMA_FACET_MINEXCLUSIVE; }
			else if(IS_SCHEMA(P_Node, "maxInclusive")) { facet->type = XML_SCHEMA_FACET_MAXINCLUSIVE; }
			else if(IS_SCHEMA(P_Node, "maxExclusive")) { facet->type = XML_SCHEMA_FACET_MAXEXCLUSIVE; }
			else if(IS_SCHEMA(P_Node, "totalDigits")) { facet->type = XML_SCHEMA_FACET_TOTALDIGITS; }
			else if(IS_SCHEMA(P_Node, "fractionDigits")) { facet->type = XML_SCHEMA_FACET_FRACTIONDIGITS; }
			else if(IS_SCHEMA(P_Node, "pattern")) { facet->type = XML_SCHEMA_FACET_PATTERN; }
			else if(IS_SCHEMA(P_Node, "enumeration")) { facet->type = XML_SCHEMA_FACET_ENUMERATION; }
			else if(IS_SCHEMA(P_Node, "whiteSpace")) { facet->type = XML_SCHEMA_FACET_WHITESPACE; }
			else if(IS_SCHEMA(P_Node, "length")) { facet->type = XML_SCHEMA_FACET_LENGTH; }
			else if(IS_SCHEMA(P_Node, "maxLength")) { facet->type = XML_SCHEMA_FACET_MAXLENGTH; }
			else if(IS_SCHEMA(P_Node, "minLength")) { facet->type = XML_SCHEMA_FACET_MINLENGTH; }
			else {
...
static int xmlSchemaParseWildcardNs(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema ATTRIBUTE_UNUSED, xmlSchemaWildcardPtr wildc, xmlNode * P_Node)
{
	...
	if(!pc || sstreq(pc, "strict")) {
		...
		xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, P_Node, NULL, "(strict | skip | lax)", pc, NULL, 0, 0);
		wildc->processContents = XML_SCHEMAS_ANY_STRICT;
		ret = XML_SCHEMAP_S4S_ATTR_INVALID_VALUE;
	}
	/*
...
static xmlSchemaBasicItem * xmlSchemaParseLocalAttribute(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema,
    xmlNode * P_Node, xmlSchemaItemList * uses, int parentType)
{
		...
		if(attr->ns == NULL) {
			if(isRef) {
				...
				if(sstreq(attr->name, "name")) {
					...
					if(sstreq(attrValue, "qualified")) {
						...
						xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNode *)attr, NULL, "(qualified | unqualified)", attrValue, NULL, 0, 0);
					}
					goto attr_next;
				}
			}
			if(sstreq(attr->name, "use")) {
				...
				if(sstreq(attrValue, "optional"))
					...
					xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_INVALID_ATTR_USE, NULL, (xmlNode *)attr,
					    NULL, "(optional | prohibited | required)", attrValue, NULL, 0, 0);
				}
				goto attr_next;
			}
...
attr_next:
	 ...
	 */
	if((defValueType == WXS_ATTR_DEF_VAL_DEFAULT) && (occurs != XML_SCHEMAS_ATTR_USE_OPTIONAL)) {
		xmlSchemaPSimpleTypeErr(pctxt, XML_SCHEMAP_SRC_ATTRIBUTE_2, NULL, P_Node, NULL, "(optional | prohibited | required)", NULL,
		    "The value of the attribute 'use' must be 'optional' if the attribute 'default' is present", NULL, 0);
	}
	/*
	...
	if(!isRef) {
		...
		if(sstreq(ns, xmlSchemaInstanceNs)) {
			xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_NO_XSI, P_Node, NULL, "The target namespace must not match '%s'", xmlSchemaInstanceNs, 0);
		}
		...
		if(xmlSchemaPValAttrNode(pctxt, NULL, attr, xmlSchemaGetBuiltInType(XML_SCHEMAS_NCNAME), &name) != 0) {
			return 0;
		}
		/*
		 ...
		 */
		ref = xmlSchemaNewQNameRef(pctxt, XML_SCHEMA_TYPE_ATTRIBUTE, tmpName, tmpNs);
		if(ref == NULL)
			return 0;
		/*
...
static xmlSchemaAttribute * xmlSchemaParseGlobalAttribute(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNode * P_Node)
{
	const xmlChar * attrValue;
	xmlSchemaAttribute * ret;
	xmlNode * child = NULL;
	xmlAttr * attr;
	/*
	...
	xmlSchemaPValAttrID(pctxt, P_Node, reinterpret_cast<const xmlChar *>("id"));
	/*
	 * Attribute "fixed".
	 */
 ...
 */
static xmlSchemaAttributeGroup * xmlSchemaParseAttributeGroupDefinition(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNode * P_Node)
{
	const xmlChar * name;
	...
	ret = xmlSchemaAddAttributeGroupDefinition(pctxt, schema, name, pctxt->targetNamespace, P_Node);
	if(!ret)
		return 0;
	/*
	 * Check for illegal attributes.
	 ...
	 */
	if(xmlSchemaParseLocalAttributes(pctxt, schema, &child, (xmlSchemaItemList **)&(ret->attrUses), XML_SCHEMA_TYPE_ATTRIBUTEGROUP, &hasRefs) == -1)
		return 0;
	if(hasRefs)
...
static int xmlSchemaCheckCSelectorXPath(xmlSchemaParserCtxtPtr ctxt, xmlSchemaIDC * idc, xmlSchemaIDCSelectPtr selector, xmlAttr * attr, int isField)
{
	xmlNode * P_Node;
	/*
	 * c-selector-xpath:
	...
	if(selector->xpath == NULL) {
		xmlSchemaPCustomErr(ctxt, /* @todo Adjust error code. */ XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, P_Node, "The XPath expression of the selector is not valid", 0);
...
static xmlSchemaAnnotPtr xmlSchemaAddAnnotation(xmlSchemaAnnotItem * annItem, xmlSchemaAnnotPtr annot)
{
	...
	switch(annItem->type) {
	    ...
	    break;
		case XML_SCHEMA_TYPE_ATTRIBUTE: {
		    xmlSchemaAttribute * item = (xmlSchemaAttribute *)annItem;
		    ADD_ANNOTATION(annot)
	    }
	    break;
		case XML_SCHEMA_TYPE_ANY_ATTRIBUTE:
		...
		case XML_SCHEMA_TYPE_ATTRIBUTEGROUP: 
			{
				xmlSchemaAttributeGroup * item = reinterpret_cast<xmlSchemaAttributeGroup *>(annItem);
				ADD_ANNOTATION(annot)
			}
			break;
		case XML_SCHEMA_TYPE_NOTATION: 
			{
				xmlSchemaNotationPtr item = (xmlSchemaNotation *)annItem;
				ADD_ANNOTATION(annot)
			}
			break;
		case XML_SCHEMA_FACET_MININCLUSIVE:
		...
		case XML_SCHEMA_FACET_MAXLENGTH:
		case XML_SCHEMA_FACET_MINLENGTH: {
		    xmlSchemaFacet * item = reinterpret_cast<xmlSchemaFacet *>(annItem);
		    ADD_ANNOTATION(annot)
	    }
	    break;
		case XML_SCHEMA_TYPE_SIMPLE:
		case XML_SCHEMA_TYPE_COMPLEX: {
		    xmlSchemaType * item = reinterpret_cast<xmlSchemaType *>(annItem);
		    ADD_ANNOTATION(annot)
	    }
	    break;
		case XML_SCHEMA_TYPE_GROUP: {
	    ...
	    break;
		default:
		    xmlSchemaPCustomErr(NULL, XML_SCHEMAP_INTERNAL, NULL, NULL, "Internal error: xmlSchemaAddAnnotation, The item is not a annotated schema component", 0);
		    break;
	}
	return (annot);
}
 ...
 */
static xmlSchemaIDCSelectPtr xmlSchemaParseIDCSelectorAndField(xmlSchemaParserCtxtPtr ctxt, xmlSchemaIDC * idc, xmlNode * P_Node, int isField)
{
	 ...
	 * Check for illegal attributes.
	 */
	xmlAttr * attr = P_Node->properties;
	while(attr) {
		if(attr->ns == NULL) {
 ...
 * Returns the parsed identity-constraint definition.
 */
static xmlSchemaIDC * xmlSchemaParseIDC(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, xmlSchemaTypeType idcCategory, const xmlChar * targetNamespace)
{
	xmlSchemaIDC * item = NULL;
	xmlNode * child = NULL;
	...
	if(idcCategory == XML_SCHEMA_TYPE_IDC_KEYREF) {
		...
		if(!attr) {
			xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, P_Node, "refer", 0);
		}
		else {
			/*
			 * Create a reference item.
			 */
			item->ref = xmlSchemaNewQNameRef(ctxt, XML_SCHEMA_TYPE_IDC_KEY, NULL, 0);
			if(item->ref == NULL)
				return 0;
...
declaration_part:
	{
		...
		xmlSchemaIDC * curIDC = NULL;
		xmlSchemaIDC * lastIDC = NULL;
		...
		if(topLevel) {
			...
			if(attr) {
				...
				if(sstreq(attrValue, "qualified")) {
					...
					xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNode *)attr,
					    NULL, "(qualified | unqualified)", attrValue, NULL, 0, 0);
				}
			}
		...
		if(topLevel) {
			...
			if(!attr) {
				...
				attrValue = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
				if(xmlSchemaPValAttrBlockFinal(attrValue, &(decl->flags), -1, XML_SCHEMAS_ELEM_FINAL_EXTENSION, XML_SCHEMAS_ELEM_FINAL_RESTRICTION, -1, -1, -1) != 0) {
					xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, NULL, (xmlNode *)attr, NULL, "(#all | List of (extension | restriction))", attrValue, NULL, 0, 0);
				}
			}
		}
		/*
...
static int xmlSchemaParseUnion(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node)
{
	...
	if(attr) {
		...
		xmlSchemaTypeLink * link;
		xmlSchemaTypeLink * lastLink = NULL;
		xmlSchemaQNameRef * ref;
		...
		do {
			...
			if(xmlSchemaPValAttrNodeQNameValue(ctxt, schema, NULL, attr, BAD_CAST tmp, &nsName, &localName) == 0) {
				 ...
				 */
				link = static_cast<xmlSchemaTypeLink *>(SAlloc::M(sizeof(xmlSchemaTypeLink)));
				if(!link) {
	...
	if(IS_SCHEMA(child, "simpleType")) {
		xmlSchemaType * subtype;
		xmlSchemaType * last = NULL;
		/*
	...
	if(!attr && !type->subtypes) {
		 ...
		 */
		xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_UNION_MEMBERTYPES_OR_SIMPLETYPES, NULL, P_Node,
		    "Either the attribute 'memberTypes' or at least one <simpleType> child must be present", 0);
	}
	return 0;
}
...
static xmlSchemaType * xmlSchemaParseSimpleType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, int topLevel)
{
		...
		if(!attr) {
			...
			if(ctxt->isS4S) {
				...
				if(ctxt->isRedefine) {
					 ...
					 * about this case.
					 */
					xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_SRC_REDEFINE, NULL, P_Node, "Redefinition of built-in simple types is not supported", 0);
					return 0;
				}
	...
	if(topLevel == 0) {
		...
		if(!attr) {
			...
			if(xmlSchemaPValAttrBlockFinal(attrValue, &(type->flags), -1, -1, XML_SCHEMAS_TYPE_FINAL_RESTRICTION, -1, XML_SCHEMAS_TYPE_FINAL_LIST, XML_SCHEMAS_TYPE_FINAL_UNION) != 0) {
				xmlSchemaPSimpleTypeErr(ctxt, XML_SCHEMAP_S4S_ATTR_INVALID_VALUE, WXS_BASIC_CAST type, (xmlNode *)attr, NULL, 
					"(#all | List of (list | union | restriction)", attrValue, NULL, 0, 0);
			}
		}
	}
...
static xmlSchemaTreeItem * xmlSchemaParseModelGroupDefRef(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node)
{
	...
	attr = xmlSchemaGetPropNode(P_Node, "ref");
	if(!attr) {
		xmlSchemaPMissingAttrErr(ctxt, XML_SCHEMAP_S4S_ATTR_MISSING, NULL, P_Node, "ref", 0);
		return 0;
	}
...
{
	// @v10.8.0 {
	static const int16 _flags[] = {XML_SCHEMAS_QUALIF_ELEM, XML_SCHEMAS_QUALIF_ATTR, XML_SCHEMAS_FINAL_DEFAULT_EXTENSION, 
		XML_SCHEMAS_FINAL_DEFAULT_RESTRICTION, XML_SCHEMAS_FINAL_DEFAULT_LIST, XML_SCHEMAS_FINAL_DEFAULT_UNION, 
		XML_SCHEMAS_BLOCK_DEFAULT_EXTENSION, XML_SCHEMAS_BLOCK_DEFAULT_RESTRICTION, XML_SCHEMAS_BLOCK_DEFAULT_SUBSTITUTION };
	for(size_t i = 0; i < SIZEOFARRAY(_flags); i++) {
		if(schema->flags & _flags[i])
			schema->flags ^= _flags[i];
	}
	// } @v10.8.0 
	/* @v10.8.0 if(schema->flags & XML_SCHEMAS_QUALIF_ELEM)
...
static int xmlSchemaParseSchemaElement(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * pNode)
{
	xmlAttr * attr;
	const xmlChar * val;
	int oldErrs = ctxt->nberrors;
	/*
	 ...
	 */
	int res = xmlSchemaPValAttrID(ctxt, pNode, reinterpret_cast<const xmlChar *>("id"));
	HFAILURE;
	/*
	 ...
	 */
	attr = xmlSchemaGetPropNode(pNode, "targetNamespace");
	if(attr) {
		res = xmlSchemaPValAttrNode(ctxt, NULL, attr, xmlSchemaGetBuiltInType(XML_SCHEMAS_ANYURI), 0);
		HFAILURE;
		if(res != 0) {
			ctxt->Stop_ = XML_SCHEMAP_S4S_ATTR_INVALID_VALUE;
			goto exit;
		}
	}
	attr = xmlSchemaGetPropNode(pNode, "elementFormDefault");
	if(attr) {
		val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
	...
	attr = xmlSchemaGetPropNode(pNode, "attributeFormDefault");
	if(attr) {
		val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
	...
	attr = xmlSchemaGetPropNode(pNode, "finalDefault");
	if(attr) {
		val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
	...
	attr = xmlSchemaGetPropNode(pNode, "blockDefault");
	if(attr) {
		val = xmlSchemaGetNodeContent(ctxt, (xmlNode *)attr);
...
static xmlSchemaSchemaRelation * xmlSchemaSchemaRelationCreate()
{
	xmlSchemaSchemaRelation * ret = static_cast<xmlSchemaSchemaRelation *>(SAlloc::M(sizeof(xmlSchemaSchemaRelation)));
	if(!ret) {
...
#if 0
static void xmlSchemaSchemaRelationFree(xmlSchemaSchemaRelation * rel) { SAlloc::F(rel); }
#endif
...
static void xmlSchemaConstructionCtxtFree(xmlSchemaConstructionCtxt * con)
{
	if(con) {
		// 
		// After the construction context has been freed, there will be
		// no schema graph available any more. Only the schema buckets
		// will stay alive, which are put into the "schemasImports" and "includes" slots of the xmlSchema.
		// 
...
{
	xmlSchemaParserCtxtPtr ret = (xmlSchemaParserCtxt *)SAlloc::M(sizeof(xmlSchemaParserCtxt));
	if(!ret) {
...
static int xmlSchemaCreatePCtxtOnVCtxt(xmlSchemaValidCtxt * vctxt)
{
	if(vctxt->pctxt == NULL) {
		if(vctxt->schema)
 ...
 */
static xmlSchemaBucket * xmlSchemaGetSchemaBucket(xmlSchemaParserCtxtPtr pctxt, const xmlChar * schemaLocation)
{
	xmlSchemaBucket * cur;
	xmlSchemaItemList * list = pctxt->constructor->buckets;
	if(list->nbItems == 0)
		return 0;
	else {
		for(int i = 0; i < list->nbItems; i++) {
			cur = (xmlSchemaBucket *)list->items[i];
			/* Pointer comparison! */
...
static xmlSchemaBucket * xmlSchemaGetChameleonSchemaBucket(xmlSchemaParserCtxtPtr pctxt, const xmlChar * schemaLocation, const xmlChar * targetNamespace)
{
	xmlSchemaBucket * cur;
	xmlSchemaItemList * list = pctxt->constructor->buckets;
	if(list->nbItems == 0)
...
static xmlSchemaBucket * xmlSchemaGetSchemaBucketByTNS(xmlSchemaParserCtxtPtr pctxt, const xmlChar * targetNamespace, int imported)
{
	xmlSchemaBucket * cur;
	xmlSchemaItemList * list = pctxt->constructor->buckets;
	if(list->nbItems == 0)
...
static int xmlSchemaParseNewDocWithContext(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlSchemaBucket * bucket)
{
	int oldFlags;
	xmlDoc * oldDoc;
	xmlNode * p_node;
	int ret, oldErrs;
	xmlSchemaBucket * oldbucket = pctxt->constructor->bucket;
	/*
	...
	if(bucket->targetNamespace && sstreq(bucket->targetNamespace, xmlSchemaNs))
		pctxt->isS4S = true; // We are parsing the schema for schemas!
...
static int xmlSchemaParseNewDoc(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlSchemaBucket * bucket)
{
	...
	if(bucket == NULL)
		return 0;
	if(bucket->parsed) {
		PERROR_INT(__FUNCTION__, "reparsing a schema doc");
		return -1;
	}
	if(bucket->doc == NULL) {
		PERROR_INT(__FUNCTION__, "parsing a schema doc, but there's no doc");
		return -1;
	}
	if(pctxt->constructor == NULL) {
		PERROR_INT(__FUNCTION__, "no constructor");
		return -1;
	}
...
static void xmlSchemaSchemaRelationAddChild(xmlSchemaBucket * bucket, xmlSchemaSchemaRelation * rel)
{
...
static int xmlSchemaAddSchemaDoc(xmlSchemaParserCtxtPtr pctxt, int type/* import or include or redefine */,
    ...
    const xmlChar * sourceTargetNamespace, const xmlChar * importNamespace, xmlSchemaBucket ** bucket)
{
	const xmlChar * targetNamespace = NULL;
	xmlSchemaSchemaRelation * relation = NULL;
	xmlDoc * doc = NULL;
	...
	if(bkt) {
		if((WXS_IS_BUCKET_IMPMAIN(type)) && (!bkt->imported)) {
			...
			// 
			SETIFZ(schemaLocation, reinterpret_cast<const xmlChar *>("in_memory_buffer"));
	...
	if(bkt && bkt->doc) {
		PERROR_INT(__FUNCTION__, "trying to load a schema doc, but a doc is already assigned to the schema bucket");
		goto exit_failure;
	}
doc_load:
	// 
	...
	if(schemaDoc) {
		...
		schemaLocation = schemaDoc->URL ? xmlDictLookupSL(pctxt->dict, schemaDoc->URL) : reinterpret_cast<const xmlChar *>("in_memory_buffer");
	}
		...
		if(schemaLocation) {
			...
			schemaLocation = reinterpret_cast<const xmlChar *>("in_memory_buffer");
			if(doc)
		...
		if(!doc) {
			...
			if(!lerr || lerr->domain != XML_FROM_IO) {
				...
				xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)res, invokingNode, NULL, "Failed to parse the XML resource '%s'", schemaLocation, 0);
			}
		}
		...
		xmlSchemaPErr(pctxt, NULL, XML_SCHEMAP_NOTHING_TO_PARSE, "No information for parsing was provided with the given schema parser context.\n", NULL, 0);
		goto exit_failure;
	}
	/*
	...
	if(doc) {
		...
		if(docElem == NULL) {
			xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_NOROOT, invokingNode, NULL, "The document '%s' has no document element", schemaLocation, 0);
			goto exit_error;
		}
		/*
		 * Remove all the blank text nodes.
		...
		if(!IS_SCHEMA(docElem, "schema")) {
			xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_NOT_SCHEMA, invokingNode, NULL, "The XML document '%s' is not a schema document", schemaLocation, 0);
			goto exit_error;
		}
    ...
    xmlChar ** schemaLocation, int type)
{
	...
	*schemaLocation = NULL;
	...
	if(attr) {
		...
		if(!uri) {
			PERROR_INT(__FUNCTION__, "could not build an URI from the schemaLocation")
			goto exit_failure;
		}
...
static int xmlSchemaParseIncludeOrRedefine(xmlSchemaParserCtxtPtr pctxt, xmlSchemaPtr schema, xmlNode * P_Node, int type)
{
	...
	if(bucket && (!bucket->parsed) && bucket->doc) {
		if(isChameleon) {
			...
			if(!(schema->flags & XML_SCHEMAS_INCLUDING_CONVERT_NS))
	...
	if(type == XML_SCHEMA_SCHEMA_REDEFINE) {
		 ...
		 */
		pctxt->isRedefine = true;
		while(IS_SCHEMA(child, "annotation") || IS_SCHEMA(child, "simpleType") || IS_SCHEMA(child, "complexType") || IS_SCHEMA(child, "group") || IS_SCHEMA(child, "attributeGroup")) {
			if(IS_SCHEMA(child, "annotation")) {
				/*
				 * @todo discard or not?
				 */
	...
	if(child) {
		...
		if(type == XML_SCHEMA_SCHEMA_REDEFINE)
			xmlSchemaPContentErr(pctxt, (xmlParserErrors)res, NULL, P_Node, child, NULL, "(annotation | (simpleType | complexType | group | attributeGroup))*");
		else
...
static xmlSchemaType * xmlSchemaParseRestriction(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, xmlSchemaTypeType parentType)
{
	...
	if(parentType == XML_SCHEMA_TYPE_SIMPLE) {
		...
		if(IS_SCHEMA(child, "simpleType")) {
			if(type->base) {
				...
				type->baseType = (xmlSchemaType *)xmlSchemaParseSimpleType(ctxt, schema, child, 0);
			}
			child = child->next;
		}
		else if(!type->base) {
		 ...
		 */
		if(IS_SCHEMA(child, "all")) {
			type->subtypes = (xmlSchemaType *)xmlSchemaParseModelGroup(ctxt, schema, child, XML_SCHEMA_TYPE_ALL, 1);
			child = child->next;
		}
		else if(IS_SCHEMA(child, "choice")) {
			type->subtypes = (xmlSchemaType *)xmlSchemaParseModelGroup(ctxt, schema, child, XML_SCHEMA_TYPE_CHOICE, 1);
			child = child->next;
		}
		else if(IS_SCHEMA(child, "sequence")) {
			type->subtypes = (xmlSchemaType *)xmlSchemaParseModelGroup(ctxt, schema, child, XML_SCHEMA_TYPE_SEQUENCE, 1);
			child = child->next;
			/*
			...
			type->subtypes = (xmlSchemaType *)xmlSchemaParseModelGroupDefRef(ctxt, schema, child);
			/*
		...
		if(IS_SCHEMA(child, "simpleType")) {
			 ...
			 */
			type->contentTypeDef = (xmlSchemaType *)xmlSchemaParseSimpleType(ctxt, schema, child, 0);
	...
	if(oneof2(parentType, XML_SCHEMA_TYPE_SIMPLE, XML_SCHEMA_TYPE_SIMPLE_CONTENT)) {
		...
		if(type->facets) {
			...
			xmlSchemaFacetLink * lastFacetLink = NULL;
			facet = type->facets;
			do {
				facetLink = static_cast<xmlSchemaFacetLink *>(SAlloc::M(sizeof(xmlSchemaFacetLink)));
				if(!facetLink) {
	...
	if(type->type == XML_SCHEMA_TYPE_COMPLEX) {
		 ...
		 */
		if(xmlSchemaParseLocalAttributes(ctxt, schema, &child, (xmlSchemaItemList **)&(type->attrUses), XML_SCHEMA_TYPE_RESTRICTION, 0) == -1)
			return 0;
		/*
		 * Attribute wildcard.
		 */
 ...
 */
static xmlSchemaType * xmlSchemaParseExtension(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, xmlSchemaTypeType parentType)
{
	xmlSchemaType * type;
	xmlNode * child = NULL;
	xmlAttr * attr;
	...
	if(child) {
		 ...
		 * Attribute uses/declarations.
		 */
		if(xmlSchemaParseLocalAttributes(ctxt, schema, &child, (xmlSchemaItemList **)&(type->attrUses), XML_SCHEMA_TYPE_EXTENSION, NULL) == -1)
			return 0;
		/*
		 * Attribute wildcard.
		 */
...
static xmlSchemaType * xmlSchemaParseComplexType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaPtr schema, xmlNode * P_Node, int topLevel)
{
	...
	if(IS_SCHEMA(child, "simpleContent")) {
		 ...
		 */
		if(xmlSchemaParseLocalAttributes(ctxt, schema, &child, (xmlSchemaItemList **)&(type->attrUses), XML_SCHEMA_TYPE_RESTRICTION, NULL) == -1)
			return 0;
		/*
 ...
 */
static int xmlSchemaBuildContentModelForSubstGroup(xmlSchemaParserCtxtPtr pctxt, xmlSchemaParticlePtr particle, int counter, xmlAutomataState * end)
{
	xmlAutomataState * start;
	xmlAutomataState * tmp;
...
static int xmlSchemaBuildAContentModel(xmlSchemaParserCtxtPtr pctxt, xmlSchemaParticlePtr particle)
{
	int ret = 0, tmp2;
	if(particle == NULL) {
		PERROR_INT(__FUNCTION__, "particle is NULL");
		return 1;
	}
	if(particle->children == NULL) {
		/*
	...
	switch(particle->children->type) {
		case XML_SCHEMA_TYPE_ANY: {
		    ...
		    if(particle->maxOccurs == 1) {
				 ...
				 * 2. the {"*"} for elements in no namespace.
				    ...
				    do {
					    pctxt->state = start;
					    pctxt->state = xmlAutomataNewTransition2(pctxt->am, pctxt->state, NULL, reinterpret_cast<const xmlChar *>("*"), ns->value, wild);
					    xmlAutomataNewEpsilon(pctxt->am, pctxt->state, end);
					    ns = ns->next;
				    } while(ns);
			    }
			    else if(wild->negNsSet) {
				    pctxt->state = xmlAutomataNewNegTrans(pctxt->am, start, end, reinterpret_cast<const xmlChar *>("*"), wild->negNsSet->value, wild);
			    }
		    }
		    else {
			    int maxOccurs = particle->maxOccurs == UNBOUNDED ? UNBOUNDED : particle->maxOccurs - 1;
			    ...
			    if(wild->any == 1) {
				    ...
				    ns = wild->nsSet;
				    do {
					    pctxt->state = xmlAutomataNewTransition2(pctxt->am, start, NULL, reinterpret_cast<const xmlChar *>("*"), ns->value, wild);
					    xmlAutomataNewEpsilon(pctxt->am, pctxt->state, hop);
					    ns = ns->next;
				    } while(ns);
			    }
			    else if(wild->negNsSet) {
				    pctxt->state = xmlAutomataNewNegTrans(pctxt->am, start, hop, reinterpret_cast<const xmlChar *>("*"), wild->negNsSet->value, wild);
			    }
			    xmlAutomataNewCountedTrans(pctxt->am, hop, start, counter);
			    xmlAutomataNewCounterTrans(pctxt->am, hop, end, counter);
		    }
		...
		case XML_SCHEMA_TYPE_SEQUENCE: {
		    ...
		    if((particle->minOccurs == 1) && (particle->maxOccurs == 1)) {
			    ...
			    xmlAutomataState * oldstate = pctxt->state;
			    if(particle->maxOccurs >= UNBOUNDED) {
				    if(particle->minOccurs > 1) {
					    xmlAutomataState * tmp;
					    int counter;
		...
		case XML_SCHEMA_TYPE_ALL: {
		    ...
		    while(sub) {
			    ...
			    if(elemDecl == NULL) {
				    PERROR_INT(__FUNCTION__, "<element> particle has no term");
				    return ret;
			    }
			    ;
			    /*
		 ...
		 * all in the content model at this point. Fix this.
		    ...
		    break;
		default:
		    xmlSchemaInternalErr2(ACTXT_CAST pctxt, __FUNCTION__, "found unexpected term of type '%s' in content model", WXS_ITEM_TYPE_NAME(particle->children), 0);
		    return ret;
	}
	return ret;
}
 ...
 */
static void xmlSchemaBuildContentModel(xmlSchemaType * type, xmlSchemaParserCtxtPtr ctxt)
{
	...
	if(type->contModel == NULL) {
		xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_INTERNAL, WXS_BASIC_CAST type, type->P_Node, "Failed to compile the content model", 0);
	}
	else if(xmlRegexpIsDeterminist(type->contModel) != 1) {
		xmlSchemaPCustomErr(ctxt, XML_SCHEMAP_NOT_DETERMINISTIC, /* XML_SCHEMAS_ERR_NOTDETERMINIST, */WXS_BASIC_CAST type, type->P_Node, "The content model is not determinist", 0);
	}
	else {
 ...
 */
static int xmlSchemaResolveUnionMemberTypes(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	xmlSchemaTypeLink * newLink;
	 ...
	 * Resolve references.
	 */
	xmlSchemaTypeLink * link = type->memberTypes;
	xmlSchemaTypeLink * lastLink = NULL;
	while(link) {
		const xmlChar * name = ((xmlSchemaQNameRef *)link->type)->name;
		const xmlChar * nsName = ((xmlSchemaQNameRef *)link->type)->targetNamespace;
 ...
 */
static int xmlSchemaIsDerivedFromBuiltInType(xmlSchemaType * type, int valType)
{
	if(type == NULL)
		return 0;
	...
	if(type->type == XML_SCHEMA_TYPE_BASIC) {
		if(type->builtInType == valType)
			return 1;
		if(oneof2(type->builtInType, XML_SCHEMAS_ANYSIMPLETYPE, XML_SCHEMAS_ANYTYPE))
			return 0;
 ...
 * is derived from such a type.
 */
static int xmlSchemaIsUserDerivedFromBuiltInType(xmlSchemaType * type, int valType)
{
	if(type == NULL)
		return 0;
	if(WXS_IS_COMPLEX(type))
...
static xmlSchemaType * xmlSchemaQueryBuiltInType(xmlSchemaType * type)
{
	if(type == NULL)
		return 0;
	if(WXS_IS_COMPLEX(type))
 ...
 * NULL in case of error.
 */
static xmlSchemaType * xmlSchemaGetBuiltInTypeAncestor(xmlSchemaType * type)
{
...
static int xmlSchemaGetEffectiveValueConstraint(xmlSchemaAttributeUsePtr attruse, int * fixed, const xmlChar ** value, xmlSchemaVal ** val)
{
	*fixed = 0;
	*value = NULL;
	ASSIGN_PTR(val, NULL);
...
static int xmlSchemaCheckDerivationOKRestriction2to4(xmlSchemaParserCtxtPtr pctxt, int action,
    xmlSchemaBasicItem * item, xmlSchemaBasicItem * baseItem, xmlSchemaItemList * uses, xmlSchemaItemList * baseUses,
    xmlSchemaWildcardPtr wild, xmlSchemaWildcardPtr baseWild)
{
	...
	if(uses) {
		for(i = 0; i < uses->nbItems; i++) {
			cur = (xmlSchemaAttributeUse *)uses->items[i];
			found = 0;
			if(baseUses == NULL)
				goto not_found;
			for(j = 0; j < baseUses->nbItems; j++) {
				bcur = (xmlSchemaAttributeUse *)baseUses->items[j];
...
static int xmlSchemaExpandAttributeGroupRefs(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBasicItem * item, xmlSchemaWildcardPtr * completeWild,
    xmlSchemaItemList * list, xmlSchemaItemList * prohibs);
/**
 ...
 */
static int xmlSchemaFixupTypeAttributeUses(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	xmlSchemaType * baseType = NULL;
	xmlSchemaAttributeUsePtr use;
	xmlSchemaItemList * uses;
	xmlSchemaItemList * baseUses;
	xmlSchemaItemList * prohibs = NULL;
	if(type->baseType == NULL) {
		PERROR_INT(__FUNCTION__, "no base type");
		return -1;
	}
	...
	if(WXS_IS_TYPE_NOT_FIXED(baseType))
		if(xmlSchemaTypeFixup(baseType, ACTXT_CAST pctxt) == -1)
			return -1;
	uses = (xmlSchemaItemList *)type->attrUses;
	baseUses = (xmlSchemaItemList *)baseType->attrUses;
	/*
	...
	if(uses) {
		if(WXS_IS_RESTRICTION(type)) {
			...
			if(xmlSchemaExpandAttributeGroupRefs(pctxt, WXS_BASIC_CAST type, &(type->attributeWildcard), uses, pctxt->attrProhibs) == -1) {
				PERROR_INT(__FUNCTION__, "failed to expand attributes");
			}
	...
	if(baseUses) {
		...
		if(WXS_IS_RESTRICTION(type)) {
			...
			for(i = 0; i < baseUses->nbItems; i++) {
				use = (xmlSchemaAttributeUse *)baseUses->items[i];
				if(prohibs) {
					/*
				...
				if(usesCount) {
					...
					for(j = 0; j < usesCount; j++) {
						tmp = (xmlSchemaAttributeUse *)uses->items[j];
 ...
 * 0 otherwise.
 */
static int xmlSchemaTypeFinalContains(xmlSchemaType * type, int final)
{
	if(type == NULL)
		return 0;
	if(type->flags & final)
 ...
 */
static int xmlSchemaCheckCOSSTDerivedOK(xmlSchemaAbstractCtxt * actxt, xmlSchemaType * type, xmlSchemaType * baseType, int subset)
{
	/*
	...
	if(WXS_IS_UNION(baseType)) {
		xmlSchemaTypeLink * cur = baseType->memberTypes;
		while(cur) {
 ...
 */
static int xmlSchemaCheckTypeDefCircularInternal(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * ctxtType, xmlSchemaType * ancestor)
{
	int ret;
	if((ancestor == NULL) || (ancestor->type == XML_SCHEMA_TYPE_BASIC))
		return 0;
	if(ctxtType == ancestor) {
		xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_ST_PROPS_CORRECT_2, WXS_BASIC_CAST ctxtType, WXS_ITEM_NODE(ctxtType), "The definition is circular", 0);
 ...
 */
static void xmlSchemaCheckTypeDefCircular(xmlSchemaType * item, xmlSchemaParserCtxtPtr ctxt)
{
 ...
 */
static int xmlSchemaCheckUnionTypeDefCircularRecur(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * ctxType, xmlSchemaTypeLink * members)
{
	xmlSchemaType * memberType;
	xmlSchemaTypeLink * member = members;
	while(member) {
...
static int xmlSchemaCheckUnionTypeDefCircular(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
 ...
 */
static void xmlSchemaResolveTypeReferences(xmlSchemaType * typeDef, xmlSchemaParserCtxtPtr ctxt)
{
	if(typeDef == NULL)
		return;
	/*
	 ...
	 */
		...
		xmlSchemaQNameRef * ref = WXS_QNAME_CAST WXS_TYPE_PARTICLE_TERM(typeDef);
 ...
 * errors.
 */
static int xmlSchemaCheckSTPropsCorrect(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	xmlSchemaType * baseType = type->baseType;
	xmlChar * str = NULL;
 ...
 */
static int xmlSchemaCheckCOSSTRestricts(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	xmlChar * str = NULL;
	if(type->type != XML_SCHEMA_TYPE_SIMPLE) {
		PERROR_INT(__FUNCTION__, "given type is not a user-derived simpleType");
		return -1;
	}
	if(WXS_IS_ATOMIC(type)) {
		...
		if(!WXS_IS_ATOMIC(type->baseType)) {
			xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_COS_ST_RESTRICTS_1_1, WXS_BASIC_CAST type, NULL, "The base type '%s' is not an atomic simple type",
		...
		if(type->facets) {
			...
			if(primitive == NULL) {
				PERROR_INT(__FUNCTION__, "failed to get primitive type");
				return -1;
			}
			facet = type->facets;
			do {
		...
		xmlSchemaType * itemType = NULL;
		itemType = type->subtypes;
		if((itemType == NULL) || (!WXS_IS_SIMPLE(itemType))) {
			PERROR_INT(__FUNCTION__, "failed to evaluate the item type");
			return -1;
		}
		...
		if((!WXS_IS_ATOMIC(itemType)) && (!WXS_IS_UNION(itemType))) {
			...
			xmlSchemaTypeLink * member = itemType->memberTypes;
			while(member) {
		...
		if(WXS_IS_ANY_SIMPLE_TYPE(type->baseType)) {
			 ...
			 */
			{
				xmlSchemaType * baseItemType = type->baseType->subtypes;
				if((baseItemType == NULL) || (!WXS_IS_SIMPLE(baseItemType))) {
					PERROR_INT(__FUNCTION__, "failed to eval the item type of a base type");
					return -1;
				}
			 ...
			 */
			{
				...
				if(type->memberTypes) {
					...
					if((member == NULL) && baseMember) {
						PERROR_INT(__FUNCTION__, "different number of member types in base");
					}
					while(member) {
						if(baseMember == NULL) {
							PERROR_INT(__FUNCTION__, "different number of member types in base");
 ...
 */
#if 0
static int xmlSchemaCheckSRCSimpleType(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	/*
...
static int xmlSchemaVCheckCVCSimpleType(xmlSchemaAbstractCtxt * actxt,
	xmlNode * P_Node, xmlSchemaType * type, const xmlChar * value, xmlSchemaVal ** retVal, int fireErrors, int normalize, int isNormalized);
/**
 ...
 * errors.
 */
static int xmlSchemaParseCheckCOSValidDefault(xmlSchemaParserCtxtPtr pctxt, xmlNode * P_Node, xmlSchemaType * type, const xmlChar * value, xmlSchemaVal ** val)
{
	int ret = 0;
	/*
	 * cos-valid-default:
 ...
 */
static int xmlSchemaCheckCTPropsCorrect(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	/*
	 ...
	 */
	if(type->attrUses && (((xmlSchemaItemList *)type->attrUses)->nbItems > 1)) {
		xmlSchemaItemList * uses = (xmlSchemaItemList *)type->attrUses;
...
static int xmlSchemaAreEqualTypes(xmlSchemaType * typeA, xmlSchemaType * typeB)
{
	/*
 ...
 * if not.
 */
static int xmlSchemaCheckCOSCTDerivedOK(xmlSchemaAbstractCtxt * actxt, xmlSchemaType * type, xmlSchemaType * baseType, int set)
{
	...
	if(!equal) {
		 ...
		 */
		if(((set & SUBSET_EXTENSION) && (WXS_IS_EXTENSION(type))) || ((set & SUBSET_RESTRICTION) && (WXS_IS_RESTRICTION(type))))
			return 1;
	}
	else {
		/*
 ...
 * Returns 0 on success, an positive error code otherwise.
 */
static int xmlSchemaCheckCOSDerivedOK(xmlSchemaAbstractCtxt * actxt, xmlSchemaType * type, xmlSchemaType * baseType, int set)
{
	if(WXS_IS_SIMPLE(type))
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckCOSCTExtends(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	xmlSchemaType * base = type->baseType;
	/*
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckDerivationOKRestriction(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	/*
	...
	if(xmlSchemaCheckDerivationOKRestriction2to4(ctxt, XML_SCHEMA_ACTION_DERIVE,
	    WXS_BASIC_CAST type, WXS_BASIC_CAST base, (xmlSchemaItemList *)type->attrUses, (xmlSchemaItemList *)base->attrUses,
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckCTComponent(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	/*
 ...
 * error code if not and -1 if an internal error occured.
 */
static int xmlSchemaCheckSRCCT(xmlSchemaParserCtxtPtr ctxt, xmlSchemaType * type)
{
	int ret = 0;
	/*
...
static void xmlSchemaDeriveFacetErr(xmlSchemaParserCtxtPtr pctxt, xmlSchemaFacet * facet1, xmlSchemaFacet * facet2, int lessGreater, int orEqual, int ofBase)
{
	xmlChar * msg = sstrdup(reinterpret_cast<const xmlChar *>("'"));
	msg = xmlStrcat(msg, xmlSchemaFacetTypeToString(facet1->type));
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("' has to be"));
	if(lessGreater == 0)
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" equal to"));
	if(lessGreater == 1)
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" greater than"));
	else
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" less than"));
	if(orEqual)
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" or equal to"));
	msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>(" '"));
	msg = xmlStrcat(msg, xmlSchemaFacetTypeToString(facet2->type));
	if(ofBase)
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("' of the base type"));
	else
		msg = xmlStrcat(msg, reinterpret_cast<const xmlChar *>("'"));
	xmlSchemaPCustomErr(pctxt, XML_SCHEMAP_INVALID_FACET_VALUE, WXS_BASIC_CAST facet1, NULL, reinterpret_cast<const char *>(msg), 0);
	SAlloc::F(msg);
}
/*
 * xmlSchemaDeriveAndValidateFacets:
 ...
 * (st-restrict-facets)
 */
static int xmlSchemaDeriveAndValidateFacets(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	xmlSchemaType * base = type->baseType;
	xmlSchemaFacetLink * link;
	xmlSchemaFacetLink * cur;
	xmlSchemaFacetLink * last = NULL;
	xmlSchemaFacet * facet;
	xmlSchemaFacet * bfacet;
	xmlSchemaFacet * flength = NULL;
	xmlSchemaFacet * ftotdig = NULL;
	xmlSchemaFacet * ffracdig = NULL;
	xmlSchemaFacet * fmaxlen = NULL;
	xmlSchemaFacet * fminlen = NULL; /* facets of the current type */
	xmlSchemaFacet * fmininc = NULL;
	xmlSchemaFacet * fmaxinc = NULL;
	xmlSchemaFacet * fminexc = NULL;
	xmlSchemaFacet * fmaxexc = NULL;
	xmlSchemaFacet * bflength = NULL;
	xmlSchemaFacet * bftotdig = NULL;
	xmlSchemaFacet * bffracdig = NULL;
	xmlSchemaFacet * bfmaxlen = NULL;
	xmlSchemaFacet * bfminlen = NULL; /* facets of the base type */
	xmlSchemaFacet * bfmininc = NULL;
	xmlSchemaFacet * bfmaxinc = NULL;
	xmlSchemaFacet * bfminexc = NULL;
	xmlSchemaFacet * bfmaxexc = NULL;
	...
	for(cur = base->facetSet; cur; cur = cur->next) {
		 ...
		 */
		if(link == NULL) {
			link = static_cast<xmlSchemaFacetLink *>(SAlloc::M(sizeof(xmlSchemaFacetLink)));
			if(link == NULL) {
...
static int xmlSchemaFinishMemberTypeDefinitionsProperty(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	xmlSchemaTypeLink * lastLink;
	xmlSchemaTypeLink * prevLink;
	xmlSchemaTypeLink * subLink;
	xmlSchemaTypeLink * newLink;
	/*
		...
		if(WXS_IS_UNION(link->type)) {
			...
			if(subLink) {
				...
				if(subLink->next) {
					...
					prevLink = link;
					while(subLink) {
						newLink = static_cast<xmlSchemaTypeLink *>(SAlloc::M(sizeof(xmlSchemaTypeLink)));
						if(!newLink) {
...
static void xmlSchemaTypeFixupOptimFacets(xmlSchemaType * type)
{
...
static int xmlSchemaTypeFixupWhitespace(xmlSchemaType * type)
{
	/*
	...
	if(WXS_IS_LIST(type)) {
		...
		return 0;
	if(type->facetSet) {
		for(const xmlSchemaFacetLink * lin = type->facetSet; lin; lin = lin->next) {
	 ...
	 */
	{
		for(xmlSchemaType * anc = type->baseType; anc && anc->builtInType != XML_SCHEMAS_ANYTYPE; anc = anc->baseType) {
			if(anc->type == XML_SCHEMA_TYPE_BASIC) {
				if(anc->builtInType == XML_SCHEMAS_NORMSTRING) {
					type->flags |= XML_SCHEMAS_TYPE_WHITESPACE_REPLACE;
				}
				else if((anc->builtInType == XML_SCHEMAS_STRING) || (anc->builtInType == XML_SCHEMAS_ANYSIMPLETYPE)) {
...
static int xmlSchemaFixupSimpleTypeStageOne(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	if(type->type != XML_SCHEMA_TYPE_SIMPLE)
		return 0;
	...
	if(WXS_IS_LIST(type)) {
		...
		if(type->subtypes == NULL) {
			 ...
			 */
			PERROR_INT(__FUNCTION__, "list type has no item-type assigned");
			return -1;
		}
	}
	else if(WXS_IS_UNION(type)) {
		...
		if(type->memberTypes == NULL) {
			 ...
			 * This one is really needed, so get out.
			 */
			PERROR_INT(__FUNCTION__, "union type has no member-types assigned");
			return -1;
		}
	}
	else {
		 ...
		 */
		if(type->baseType == NULL) {
			PERROR_INT(__FUNCTION__, "type has no base-type assigned");
			return -1;
		}
...
#ifdef DEBUG_TYPE
static void xmlSchemaDebugFixedType(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	if(type->node) {
 ...
 */
static int xmlSchemaFixupSimpleTypeStageTwo(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
	 ...
	 *
	 * xmlSchemaPErr(ctxt, type->node, XML_SCHEMAP_SRC_SIMPLE_TYPE_1, "Simple type '%s' does not satisfy the constraints on simple type definitions.\n", type->name, 0);
	 */
	/*
	 * Schema Component Constraint: Simple Type Restriction (Facets)
	 * (st-restrict-facets)
...
static int xmlSchemaFixupComplexType(xmlSchemaParserCtxtPtr pctxt, xmlSchemaType * type)
{
		...
		if((particle == NULL) ||
			...
			if(type->flags & XML_SCHEMAS_TYPE_MIXED) {
				...
				if(!particle || (particle->children->type != XML_SCHEMA_TYPE_SEQUENCE)) {
					...
					if(particle->children == NULL)
						goto exit_failure;
					type->subtypes = (xmlSchemaType *)particle;
				}
				dummySequence = 1;
		...
		if(WXS_IS_RESTRICTION(type)) {
			...
			if(type->contentType == XML_SCHEMA_CONTENT_EMPTY) {
				...
				if(WXS_TYPE_PARTICLE(type) && WXS_TYPE_PARTICLE_TERM(type) && ((WXS_TYPE_PARTICLE_TERM(type))->type == XML_SCHEMA_TYPE_ALL)) {
					...
					if(particle->children == NULL)
						goto exit_failure;
					WXS_TYPE_CONTENTTYPE(type) = (xmlSchemaType *)particle;
					/*
 ...
 */
static int FASTCALL xmlSchemaTypeFixup(xmlSchemaType * type, xmlSchemaAbstractCtxt * actxt)
{
	if(type) {
 ...
 *    -1 in case of an internal or API error.
 */
int xmlSchemaCheckFacet(xmlSchemaFacet * facet, xmlSchemaType * typeDecl, xmlSchemaParserCtxtPtr pctxt, const xmlChar * name ATTRIBUTE_UNUSED)
{
	int ret = 0;
	...
	switch(facet->type) {
		 ...
		 * already* the base type.
		    ...
		    if(typeDecl->type != XML_SCHEMA_TYPE_BASIC) {
			    ...
			    if(base == NULL) {
				    PERROR_INT(__FUNCTION__, "a type user derived type has no base type");
				    return -1;
			    }
		    }
		    else
		 ...
		 * of the facet.
		    ...
		    if(ret) {
			    if(ret < 0) {
				    ...
				    if(ctxtGiven) {
					    xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_INTERNAL, facet->P_Node, NULL,
						    "Internal error: " __FUNCTION__ ", failed to validate the value '%s' of the facet '%s' against the base type",
			    ...
			    if(ctxtGiven) {
				    PERROR_INT(__FUNCTION__, "value was not computed");
			    }
			    TODO
		    }
		    break;
		    ...
		    if(facet->regexp == NULL) {
			    ...
			    if(ctxtGiven) {
				    xmlSchemaCustomErr(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->P_Node, WXS_BASIC_CAST typeDecl,
						"The value '%s' of the facet 'pattern' is not a valid regular expression", facet->value, 0);
			    }
		    }
		    break;
		case XML_SCHEMA_FACET_TOTALDIGITS:
		...
		case XML_SCHEMA_FACET_MINLENGTH:
		    ...
		    if(ret) {
			    if(ret < 0) {
				    /* No error message for RelaxNG. */
				    if(ctxtGiven) {
					    PERROR_INT(__FUNCTION__, "validating facet value");
				    }
				    goto internal_error;
			    }
			    ret = XML_SCHEMAP_INVALID_FACET_VALUE;
			    ...
			    if(ctxtGiven) {
				    ...
				    xmlSchemaCustomErr4(ACTXT_CAST pctxt, (xmlParserErrors)ret, facet->P_Node, WXS_BASIC_CAST typeDecl,
						...
						facet->value, xmlSchemaFacetTypeToString(facet->type),
						(facet->type != XML_SCHEMA_FACET_TOTALDIGITS) ? reinterpret_cast<const xmlChar *>("nonNegativeInteger") : reinterpret_cast<const xmlChar *>("positiveInteger"),
						NULL);
			    }
		    }
		    break;
		...
		case XML_SCHEMA_FACET_WHITESPACE: {
		    if(sstreq(facet->value, reinterpret_cast<const xmlChar *>("preserve")))
			    facet->whitespace = XML_SCHEMAS_FACET_PRESERVE;
		    else if(sstreq(facet->value, reinterpret_cast<const xmlChar *>("replace")))
			    facet->whitespace = XML_SCHEMAS_FACET_REPLACE;
		    else if(sstreq(facet->value, reinterpret_cast<const xmlChar *>("collapse")))
 ...
 */
static int xmlSchemaCheckFacetValues(xmlSchemaType * typeDecl, xmlSchemaParserCtxtPtr pctxt)
{
	int res, olderrs = pctxt->nberrors;
	const xmlChar * name = typeDecl->name;
	// NOTE: It is intended to use the facets list, instead of facetSet.
	if(typeDecl->facets) {
		xmlSchemaFacet * facet = typeDecl->facets;
		/*
 ...
 */
static xmlSchemaQNameRef * FASTCALL xmlSchemaCheckAttrGroupCircularRecur(xmlSchemaAttributeGroup * ctxtGr, xmlSchemaItemList * list)
{
	//
	// We will search for an attribute group reference which references the context attribute group.
	//
	for(int i = 0; i < list->nbItems; i++) {
		xmlSchemaQNameRef * ref = (xmlSchemaQNameRef *)list->items[i];
		if((ref->type == XML_SCHEMA_EXTRA_QNAMEREF) && (ref->itemType == XML_SCHEMA_TYPE_ATTRIBUTEGROUP) && ref->item) {
			xmlSchemaAttributeGroup * gr = WXS_ATTR_GROUP_CAST ref->item;
			if(gr == ctxtGr)
				return (ref);
			...
			if((gr->attrUses) && (gr->flags & XML_SCHEMAS_ATTRGROUP_HAS_REFS)) {
				...
				xmlSchemaQNameRef * circ = xmlSchemaCheckAttrGroupCircularRecur(ctxtGr, (xmlSchemaItemList *)gr->attrUses);
				gr->flags ^= XML_SCHEMAS_ATTRGROUP_MARKED;
				if(circ)
					return (circ);
			}
 ...
 */
static int xmlSchemaCheckAttrGroupCircular(xmlSchemaAttributeGroup * attrGr, xmlSchemaParserCtxtPtr ctxt)
{
	/*
	 * Schema Representation Constraint:
	...
	if(attrGr->attrUses == NULL)
		...
		xmlSchemaQNameRef * circ = xmlSchemaCheckAttrGroupCircularRecur(attrGr, (xmlSchemaItemList *)attrGr->attrUses);
		if(circ) {
			xmlChar * str = NULL;
			/*
...
static int xmlSchemaAttributeGroupExpandRefs(xmlSchemaParserCtxtPtr pctxt, xmlSchemaAttributeGroup * attrGr);
/**
...
static int xmlSchemaExpandAttributeGroupRefs(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBasicItem * item,
    xmlSchemaWildcardPtr * completeWild, xmlSchemaItemList * list, xmlSchemaItemList * prohibs)
{
	xmlSchemaAttributeGroup * gr;
	xmlSchemaAttributeUsePtr use;
	xmlSchemaItemList * sublist;
	int i, j;
	int created = (*completeWild == NULL) ? 0 : 1;
	if(prohibs)
		prohibs->nbItems = 0;
	for(i = 0; i < list->nbItems; i++) {
		use = (xmlSchemaAttributeUse *)list->items[i];
		if(use->type == XML_SCHEMA_EXTRA_ATTR_USE_PROHIB) {
			if(prohibs == NULL) {
				PERROR_INT(__FUNCTION__, "unexpected attr prohibition found");
				return -1;
			}
			/*
		...
		if((use->type == XML_SCHEMA_EXTRA_QNAMEREF) && ((WXS_QNAME_CAST use)->itemType == XML_SCHEMA_TYPE_ATTRIBUTEGROUP)) {
			 ...
			 */
			sublist = ((xmlSchemaItemList *)gr->attrUses);
	...
	if(prohibs && prohibs->nbItems && list->nbItems) {
		for(i = prohibs->nbItems -1; i >= 0; i--) {
			...
			for(j = 0; j < list->nbItems; j++) {
				use = (xmlSchemaAttributeUse *)list->items[j];
...
static int xmlSchemaAttributeGroupExpandRefs(xmlSchemaParserCtxtPtr pctxt, xmlSchemaAttributeGroup * attrGr)
{
	...
	if(xmlSchemaExpandAttributeGroupRefs(pctxt, WXS_BASIC_CAST attrGr, &(attrGr->attributeWildcard), (xmlSchemaItemList *)attrGr->attrUses, NULL) == -1)
		return -1;
	return 0;
}
 ...
 */
static int xmlSchemaCheckAGPropsCorrect(xmlSchemaParserCtxtPtr pctxt, xmlSchemaAttributeGroup * attrGr)
{
	/*
	...
	if(attrGr->attrUses && (WXS_LIST_CAST attrGr->attrUses)->nbItems > 1) {
		xmlSchemaItemList * uses = WXS_LIST_CAST attrGr->attrUses;
		xmlSchemaAttributeUsePtr use, tmp;
		int i, j, hasId = 0;
		for(i = uses->nbItems -1; i >= 0; i--) {
			use = (xmlSchemaAttributeUse *)uses->items[i];
 ...
 */
static int xmlSchemaResolveAttrGroupReferences(xmlSchemaQNameRef * ref, xmlSchemaParserCtxtPtr ctxt)
{
	if(!ref->item) {
 ...
 * builded and checked.
 */
static int xmlSchemaCheckAttrPropsCorrect(xmlSchemaParserCtxtPtr pctxt, xmlSchemaAttribute * attr)
{
	/*
...
{
	int ret = 0;
	xmlSchemaType * typeDef = WXS_ELEM_TYPEDEF(elemDecl);
	/*
	...
	if(elemDecl->value && ((WXS_IS_SIMPLE(typeDef) && xmlSchemaIsDerivedFromBuiltInType(typeDef, XML_SCHEMAS_ID)) || (WXS_IS_COMPLEX(typeDef) &&
		...
		if(elemDecl->P_Node) {
			if(elemDecl->flags & XML_SCHEMAS_ELEM_FIXED)
				p_node = (xmlNode *)xmlHasProp(elemDecl->P_Node, reinterpret_cast<const xmlChar *>("fixed"));
			else
				p_node = (xmlNode *)xmlHasProp(elemDecl->P_Node, reinterpret_cast<const xmlChar *>("default"));
		}
...
{
	if((WXS_SUBST_HEAD(elemDecl) == NULL) || /* SPEC (1) "Its {abstract} is false." */ (elemDecl->flags & XML_SCHEMAS_ELEM_ABSTRACT))
		return;
	{
		xmlSchemaElementPtr head;
		xmlSchemaType * headType;
		xmlSchemaType * type;
		int set, methSet;
		/*
 ...
 */
static int xmlSchemaResolveAttrUseReferences(xmlSchemaAttributeUsePtr ause, xmlSchemaParserCtxtPtr ctxt)
{
	if(!ctxt || (ause == NULL))
		return -1;
	if((ause->attrDecl == NULL) || (ause->attrDecl->type != XML_SCHEMA_EXTRA_QNAMEREF))
		return 0;
	{
		xmlSchemaQNameRef * ref = WXS_QNAME_CAST ause->attrDecl;
 ...
 */
static int xmlSchemaResolveAttrTypeReferences(xmlSchemaAttribute * item, xmlSchemaParserCtxtPtr ctxt)
{
	/*
	...
	if(item->subtypes)
		return 0;
	if(item->typeName) {
		xmlSchemaType * type = xmlSchemaGetType(ctxt->schema, item->typeName, item->typeNs);
		if(!type || !WXS_IS_SIMPLE(type)) {
 ...
 */
static int xmlSchemaResolveIDCKeyReferences(xmlSchemaIDC * idc, xmlSchemaParserCtxtPtr pctxt)
{
	...
	if(idc->ref->name) {
		...
		if(idc->ref->item == NULL) {
			 ...
			 */
			xmlSchemaCustomErr(ACTXT_CAST pctxt, XML_SCHEMAP_C_PROPS_CORRECT, NULL, WXS_BASIC_CAST idc, "The keyref references a keyref", NULL, 0);
			idc->ref->item = NULL;
			return pctxt->err;
		}
		else {
			if(idc->nbFields != reinterpret_cast<const xmlSchemaIDC *>(idc->ref->item)->nbFields) {
				xmlChar * str = NULL;
				xmlSchemaIDC * refer = reinterpret_cast<xmlSchemaIDC *>(idc->ref->item);
				/*
...
#define WXS_REDEFINED_TYPE(c) (((xmlSchemaType *)item)->flags & XML_SCHEMAS_TYPE_REDEFINED)
#define WXS_REDEFINED_MODEL_GROUP_DEF(c) (((xmlSchemaModelGroupDefPtr)item)->flags & XML_SCHEMA_MODEL_GROUP_DEF_REDEFINED)
#define WXS_REDEFINED_ATTR_GROUP(c) (reinterpret_cast<const xmlSchemaAttributeGroup *>(item)->flags & XML_SCHEMAS_ATTRGROUP_REDEFINED)
	...
	do {
		...
		switch(item->type) {
			 ...
			 * @todo How
			    ...
			    break;
			case XML_SCHEMA_TYPE_GROUP:
			    if((WXS_MODEL_GROUPDEF_CAST prev)->flags & XML_SCHEMA_MODEL_GROUP_DEF_REDEFINED) {
				    wasRedefined = 1;
				    break;
			    }
			    /* Mark it as redefined. */
		...
		switch(item->type) {
			 ...
			 * in I)."
			    ...
			    err = xmlSchemaCheckDerivationOKRestriction2to4(pctxt, XML_SCHEMA_ACTION_REDEFINE, item, redef->target,
					(xmlSchemaItemList *)(WXS_ATTR_GROUP_CAST item)->attrUses, (xmlSchemaItemList *)(WXS_ATTR_GROUP_CAST redef->target)->attrUses,
...
static int xmlSchemaAddComponents(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBucket * bucket)
{
	xmlSchemaBasicItem * item;
	int err;
	xmlHashTable ** table;
	...
	for(i = 0; i < bucket->globals->nbItems; i++) {
		...
		switch(item->type) {
			...
			case XML_SCHEMA_TYPE_IDC_KEYREF:
			    ...
			    break;
			case XML_SCHEMA_TYPE_NOTATION:
			    name = ((xmlSchemaNotation *)item)->name;
			    WXS_GET_GLOBAL_HASH(bucket, notaDecl)
			    break;
			default:
			    PERROR_INT(__FUNCTION__, "Unexpected global component type");
			    continue;
		}
		if(*table == NULL) {
			*table = xmlHashCreateDict(10, pctxt->dict);
			if(*table == NULL) {
				PERROR_INT(__FUNCTION__, "failed to create a component hash table");
				return -1;
			}
		}
...
static int xmlSchemaFixupComponents(xmlSchemaParserCtxtPtr pctxt, xmlSchemaBucket * rootBucket)
{
	xmlSchemaConstructionCtxtPtr con = pctxt->constructor;
	xmlSchemaTreeItem * item;
	xmlSchemaTreeItem ** items;
	int nbItems, i, ret = 0;
	xmlSchemaBucket * oldbucket = con->bucket;
	xmlSchemaElementPtr elemDecl;
#define FIXHFAILURE if(pctxt->err == XML_SCHEMAP_INTERNAL) goto exit_failure;
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			...
			case XML_SCHEMA_TYPE_COMPLEX:
			case XML_SCHEMA_TYPE_SIMPLE:
			    xmlSchemaResolveTypeReferences((xmlSchemaType *)item, pctxt);
			    FIXHFAILURE;
			    break;
			case XML_SCHEMA_TYPE_ATTRIBUTE:
			    xmlSchemaResolveAttrTypeReferences((xmlSchemaAttribute *)item, pctxt);
			    FIXHFAILURE;
			    break;
			case XML_SCHEMA_TYPE_ATTRIBUTE_USE:
			    xmlSchemaResolveAttrUseReferences((xmlSchemaAttributeUse *)item, pctxt);
			    FIXHFAILURE;
			    break;
			case XML_SCHEMA_EXTRA_QNAMEREF:
			...
			case XML_SCHEMA_TYPE_IDC_UNIQUE:
			case XML_SCHEMA_TYPE_IDC_KEYREF:
			    xmlSchemaResolveIDCKeyReferences(reinterpret_cast<xmlSchemaIDC *>(item), pctxt);
			    FIXHFAILURE;
			    break;
			case XML_SCHEMA_EXTRA_ATTR_USE_PROHIB:
			    // Handle attribue prohibition which had a "ref" attribute.
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_COMPLEX:
			case XML_SCHEMA_TYPE_SIMPLE:
			    xmlSchemaCheckTypeDefCircular((xmlSchemaType *)item, pctxt);
			    FIXHFAILURE;
			    if(pctxt->nberrors != 0)
				    goto exit_error;
			    break;
			case XML_SCHEMA_TYPE_GROUP:
			    ...
			    break;
			case XML_SCHEMA_TYPE_ATTRIBUTEGROUP:
			    xmlSchemaCheckAttrGroupCircular(reinterpret_cast<xmlSchemaAttributeGroup *>(item), pctxt);
			    FIXHFAILURE;
			    if(pctxt->nberrors != 0)
				    goto exit_error;
			    break;
	...
	// a seperate step, since otherwise we won't be able to detect circular union types in all cases.
	// 
	for(i = 0; i < nbItems; i++) {
		item = items[i];
		switch(item->type) {
			case XML_SCHEMA_TYPE_SIMPLE:
			    if(WXS_IS_TYPE_NOT_FIXED_1((xmlSchemaType *)item)) {
				    xmlSchemaFixupSimpleTypeStageOne(pctxt, (xmlSchemaType *)item);
					FIXHFAILURE;
			    }
			    break;
			default:
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_SIMPLE:
			    if(((xmlSchemaType *)item)->memberTypes) {
				    xmlSchemaCheckUnionTypeDefCircular(pctxt, (xmlSchemaType *)item);
				    FIXHFAILURE;
			    }
			    break;
			default:
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_ATTRIBUTE_USE:
			    if(((xmlSchemaAttributeUse *)item)->defValue) {
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_ELEMENT:
			    elemDecl = (xmlSchemaElement *)item;
			    if(!(elemDecl->flags & XML_SCHEMAS_ELEM_INTERNAL_CHECKED)) {
				    xmlSchemaCheckElementDeclComponent((xmlSchemaElement *)elemDecl, pctxt); 
					FIXHFAILURE;
			    }
	...
	for(i = 0; i < nbItems; i++) {
		...
		switch(item->type) {
			case XML_SCHEMA_TYPE_COMPLEX:
			    xmlSchemaBuildContentModel((xmlSchemaType *)item, pctxt);
			    // FIXHFAILURE; 
			    break;
			default:
			    break;
...
xmlSchemaPtr xmlSchemaParse(xmlSchemaParserCtxtPtr ctxt)
{
	...
	mainSchema->Preserve_ = ctxt->Preserve_;
...
{
	switch(type) {
		case XML_SCHEMA_FACET_PATTERN: return (reinterpret_cast<const xmlChar *>("pattern"));
		case XML_SCHEMA_FACET_MAXEXCLUSIVE: return (reinterpret_cast<const xmlChar *>("maxExclusive"));
		case XML_SCHEMA_FACET_MAXINCLUSIVE: return (reinterpret_cast<const xmlChar *>("maxInclusive"));
		case XML_SCHEMA_FACET_MINEXCLUSIVE: return (reinterpret_cast<const xmlChar *>("minExclusive"));
		case XML_SCHEMA_FACET_MININCLUSIVE: return (reinterpret_cast<const xmlChar *>("minInclusive"));
		case XML_SCHEMA_FACET_WHITESPACE: return (reinterpret_cast<const xmlChar *>("whiteSpace"));
		case XML_SCHEMA_FACET_ENUMERATION: return (reinterpret_cast<const xmlChar *>("enumeration"));
		case XML_SCHEMA_FACET_LENGTH: return (reinterpret_cast<const xmlChar *>("length"));
		case XML_SCHEMA_FACET_MAXLENGTH: return (reinterpret_cast<const xmlChar *>("maxLength"));
		case XML_SCHEMA_FACET_MINLENGTH: return (reinterpret_cast<const xmlChar *>("minLength"));
		case XML_SCHEMA_FACET_TOTALDIGITS: return (reinterpret_cast<const xmlChar *>("totalDigits"));
		case XML_SCHEMA_FACET_FRACTIONDIGITS: return (reinterpret_cast<const xmlChar *>("fractionDigits"));
		default: break;
	}
	return (reinterpret_cast<const xmlChar *>("Internal Error"));
}
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaAssembleByLocation(xmlSchemaValidCtxt * vctxt, xmlSchemaPtr schema, xmlNode * P_Node, const xmlChar * nsName, const xmlChar * location)
{
	int ret = 0;
	...
	if((vctxt == NULL) || (schema == NULL))
		return -1;
	if(vctxt->pctxt == NULL) {
		VERROR_INT(__FUNCTION__, "no parser context available");
		return -1;
	}
	pctxt = vctxt->pctxt;
	if(pctxt->constructor == NULL) {
 ...
 * number otherwise and -1 in case of an internal or API error.
 */
static int xmlSchemaAssembleByXSI(xmlSchemaValidCtxt * vctxt)
{
	const xmlChar * cur, * end;
...
static const xmlChar * xmlSchemaLookupNamespace(xmlSchemaValidCtxt * vctxt, const xmlChar * prefix)
{
	if(vctxt->sax) {
		int i, j;
 ...
 */
static int xmlSchemaValidateNotation(xmlSchemaValidCtxt * vctxt, xmlSchemaPtr schema, xmlNode * P_Node, const xmlChar * value, xmlSchemaVal ** val, int valNeeded)
{
	int ret;
		...
		return ret;
	{
		...
		if(prefix) {
			...
			if(xmlSchemaGetNotation(schema, localName, nsName)) {
				if((valNeeded) && val) {
			...
			if(xmlSchemaGetNotation(schema, value, NULL)) {
				if(valNeeded && val) {
					(*val) = xmlSchemaNewNOTATIONValue(sstrdup(value), 0);
					if(*val == NULL)
						ret = -1;
				}
			}
...
static int xmlSchemaVAddNodeQName(xmlSchemaValidCtxt * vctxt, const xmlChar* lname, const xmlChar* nsname)
{
	int i;
 ...
 */
static void xmlSchemaAugmentIDC(xmlSchemaIDC * idcDef, xmlSchemaValidCtxt * vctxt)
{
 ...
 */
static void xmlSchemaAugmentImportedIDC(xmlSchemaImportPtr imported, xmlSchemaValidCtxt * vctxt) 
{
 ...
 */
static xmlSchemaPSVIIDCBindingPtr xmlSchemaIDCNewBinding(xmlSchemaIDC * idcDef)
{
 ...
 */
static int xmlSchemaIDCStoreNodeTableItem(xmlSchemaValidCtxt * vctxt, xmlSchemaPSVIIDCNodePtr item)
{
	/*
	 * Add to gobal list.
	 */
 ...
 * Returns 0 if succeeded, -1 on internal errors.
 */
static int xmlSchemaIDCStoreKey(xmlSchemaValidCtxt * vctxt, xmlSchemaPSVIIDCKeyPtr key)
{
	/*
	 * Add to gobal list.
	 */
 ...
 */
static xmlSchemaPSVIIDCBindingPtr xmlSchemaIDCAcquireBinding(xmlSchemaValidCtxt * vctxt, xmlSchemaIDCMatcherPtr matcher)
{
...
static xmlSchemaItemList * xmlSchemaIDCAcquireTargetList(xmlSchemaValidCtxt * vctxt ATTRIBUTE_UNUSED, xmlSchemaIDCMatcherPtr matcher)
{
 ...
 */
static void xmlSchemaIDCReleaseMatcherList(xmlSchemaValidCtxt * vctxt, xmlSchemaIDCMatcherPtr matcher)
{
	while(matcher) {
		xmlSchemaIDCMatcher * next = matcher->next;
		if(matcher->keySeqs) {
 ...
 * Returns 0 on success and -1 on internal errors.
 */
static int xmlSchemaIDCAddStateObject(xmlSchemaValidCtxt * vctxt, xmlSchemaIDCMatcherPtr matcher, xmlSchemaIDCSelectPtr sel, int type)
{
 ...
 */
static int xmlSchemaXPathEvaluate(xmlSchemaValidCtxt * vctxt, xmlElementType nodeType)
{
	...
	while(sto != head) {
		...
		if(nodeType == XML_ELEMENT_NODE)
			res = xmlStreamPush((xmlStreamCtxt *)sto->xpathCtxt, vctxt->inode->localName, vctxt->inode->nsName);
		else
			res = xmlStreamPushAttr((xmlStreamCtxt *)sto->xpathCtxt, vctxt->inode->localName, vctxt->inode->nsName);
		if(res == -1) {
...
static const xmlChar * xmlSchemaFormatIDCKeySequence(xmlSchemaValidCtxt * vctxt, xmlChar ** buf, xmlSchemaPSVIIDCKeyPtr * seq, int count)
{
	int i, res;
	xmlChar * value = NULL;
	*buf = sstrdup(reinterpret_cast<const xmlChar *>("["));
 ...
 * Returns 0 on success and -1 on internal errors.
 */
static int xmlSchemaXPathPop(xmlSchemaValidCtxt * vctxt)
{
	if(vctxt->xpathStates) {
		xmlSchemaIDCStateObjPtr sto = vctxt->xpathStates;
		do {
			int res = xmlStreamPop((xmlStreamCtxt *)sto->xpathCtxt);
			if(res == -1)
				return -1;
			sto = sto->next;
		} while(sto);
 ...
 * Returns 0 on success and -1 on internal errors.
 */
static int xmlSchemaXPathProcessHistory(xmlSchemaValidCtxt * vctxt, int depth)
{
	xmlSchemaIDCStateObjPtr sto;
	xmlSchemaIDCStateObjPtr nextsto;
	int res, matchDepth;
	xmlSchemaPSVIIDCKeyPtr key = NULL;
	xmlSchemaType * type = vctxt->inode->typeDef;
	xmlSchemaType * simpleType = NULL;
	if(vctxt->xpathStates == NULL)
		return 0;
	sto = vctxt->xpathStates;
#ifdef DEBUG_IDC
	 ...
	 */
	while(sto) {
		res = xmlStreamPop((xmlStreamCtxt *)sto->xpathCtxt);
		if(res == -1) {
 ...
 */
static int xmlSchemaIDCRegisterMatchers(xmlSchemaValidCtxt * vctxt, xmlSchemaElementPtr elemDecl)
{
	xmlSchemaIDCMatcherPtr matcher, last = NULL;
	xmlSchemaIDC * refIdc;
	xmlSchemaIDCAugPtr aidc;
	xmlSchemaIDC * idc = static_cast<xmlSchemaIDC *>(elemDecl->idcs);
	if(!idc)
		return 0;
#ifdef DEBUG_IDC
	{
	...
	do {
		if(idc->type == XML_SCHEMA_TYPE_IDC_KEYREF) {
			 ...
			 */
			refIdc = reinterpret_cast<xmlSchemaIDC *>(idc->ref->item);
			if(refIdc) {
				/*
...
static int xmlSchemaIDCFillNodeTables(xmlSchemaValidCtxt * vctxt, xmlSchemaNodeInfoPtr ielem)
{
	xmlSchemaPSVIIDCBindingPtr bind;
	...
	while(matcher) {
		...
		nbNodeTable = bind->nodeTable ? bind->nbNodes : 0;
 ...
 * Returns 0 if OK and -1 on internal errors.
 */
static int xmlSchemaBubbleIDCNodeTables(xmlSchemaValidCtxt * vctxt)
{
 ...
 */
static int xmlSchemaCheckCVCIDCKeyRef(xmlSchemaValidCtxt * vctxt)
{
	xmlSchemaPSVIIDCBindingPtr bind;
		...
		if((matcher->idcType == XML_SCHEMA_TYPE_IDC_KEYREF) && matcher->targets && matcher->targets->nbItems) {
			...
			for(i = 0; i < matcher->targets->nbItems; i++) {
						...
						for(j = 0; j < bind->dupls->nbItems; j++) {
							...
							if(res == 1) {
								...
								xmlSchemaKeyrefErr(vctxt, XML_SCHEMAV_CVC_IDC, refNode,
								    (xmlSchemaType *)matcher->aidc->def, "More than one match found for key-sequence %s of keyref '%s'",
				...
				if(res == 0) {
					xmlChar * str = NULL, * strB = NULL;
					xmlSchemaKeyrefErr(vctxt, XML_SCHEMAV_CVC_IDC, refNode,
					    (xmlSchemaType *)matcher->aidc->def,
...
static xmlSchemaAttrInfo * xmlSchemaGetFreshAttrInfo(xmlSchemaValidCtxt * vctxt)
{
	xmlSchemaAttrInfo * iattr;
	/*
...
static int xmlSchemaValidatorPushAttribute(xmlSchemaValidCtxt * vctxt, xmlNode * attrNode, int nodeLine,
 ...
 */
static xmlSchemaNodeInfo * xmlSchemaGetFreshElemInfo(xmlSchemaValidCtxt * vctxt)
{
...
pattern_and_enum:
	if(error >= 0) {
		...
		tmpType = type;
		do {
			for(facet = tmpType->facets; facet; facet = facet->next) {
				if(facet->type != XML_SCHEMA_FACET_ENUMERATION)
					continue;
				found = 1;
	...
	if(error >= 0) {
		...
		do {
			found = 0;
			for(facetLink = tmpType->facetSet; facetLink; facetLink = facetLink->next) {
...
static xmlChar * FASTCALL xmlSchemaNormalizeValue(xmlSchemaType * type, const xmlChar * value)
{
...
static int xmlSchemaValidateQName(xmlSchemaValidCtxt * vctxt, const xmlChar * value, xmlSchemaVal ** val, int valNeeded)
{
	...
	if(valNeeded && val)
		*val = xmlSchemaNewQNameValue(sstrdup(nsName), BAD_CAST local);
	else
		SAlloc::F(local);
	return 0;
}
		...
		if(actxt->type == XML_SCHEMA_CTXT_VALIDATOR) {
			switch(biType->builtInType) {
				case XML_SCHEMAS_NOTATION:
				    ret = xmlSchemaValidateNotation((xmlSchemaValidCtxt *)actxt, ((xmlSchemaValidCtxt *)actxt)->schema, NULL, value, &val, valNeeded);
				    break;
				case XML_SCHEMAS_QNAME:
				    ret = xmlSchemaValidateQName((xmlSchemaValidCtxt *)actxt, value, &val, valNeeded);
				    break;
				default:
				    /* ws = xmlSchemaGetWhiteSpaceFacetValue(type); */
				    if(valNeeded)
...
static int xmlSchemaVExpandQName(xmlSchemaValidCtxt * vctxt, const xmlChar * value, const xmlChar ** nsName, const xmlChar ** localName)
{
	int ret = 0;
	...
	}
	{
		...
		*nsName = xmlSchemaLookupNamespace(vctxt, prefix);
		if(prefix) {
			 ...
			 */
			if(*nsName == NULL) {
				xmlSchemaCustomErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_DATATYPE_VALID_1_2_1, NULL, WXS_BASIC_CAST xmlSchemaGetBuiltInType(XML_SCHEMAS_QNAME),
				    "The QName value '%s' has no corresponding namespace declaration in scope", value, 0);
				return (2);
			}
		}
...
static int xmlSchemaProcessXSIType(xmlSchemaValidCtxt * vctxt, xmlSchemaAttrInfo * iattr, xmlSchemaType ** localType, xmlSchemaElementPtr elemDecl)
{
	int ret = 0;
	/*
	 * cvc-elt (3.3.4) : (4)
...
static int xmlSchemaValidateElemDecl(xmlSchemaValidCtxt * vctxt)
{
	xmlSchemaElementPtr elemDecl = vctxt->inode->decl;
	xmlSchemaType * actualType;
	/*
	 * cvc-elt (3.3.4) : 1
	 */
	if(elemDecl == NULL) {
			...
			if(!ret) {
				if((elemDecl->flags & XML_SCHEMAS_ELEM_NILLABLE) == 0) {
					...
					if(xmlSchemaValueGetAsBoolean(iattr->val)) {
						 ...
						 * cvc-elt (3.3.4) : 3.2.2
						 */
						if((elemDecl->flags & XML_SCHEMAS_ELEM_FIXED) && elemDecl->value) {
	 ...
	 */
	if(elemDecl->idcs && (xmlSchemaIDCRegisterMatchers(vctxt, elemDecl) == -1))
		return -1;
	/*
...
static int xmlSchemaVAttributesSimple(xmlSchemaValidCtxt * vctxt)
{
	int ret = 0;
	/*
	 * SPEC cvc-type (3.1.1)
 ...
 */
static void xmlSchemaClearAttrInfos(xmlSchemaValidCtxt * vctxt)
{
	if(vctxt->nbAttrInfos) {
		for(int i = 0; i < vctxt->nbAttrInfos; i++) {
 ...
 */
static int xmlSchemaVAttributesComplex(xmlSchemaValidCtxt * vctxt)
{
	xmlSchemaType * type = vctxt->inode->typeDef;
	xmlSchemaItemList * attrUseList;
	 ...
	 *
	 */
	attrUseList = (xmlSchemaItemList *)type->attrUses;
	/*
	 * @nbAttrs is the number of attributes present in the instance.
	 */
	nbAttrs = vctxt->nbAttrInfos;
	nbUses = attrUseList ? attrUseList->nbItems : 0;
	for(i = 0; i < nbUses; i++) {
		found = 0;
		attrUse = (xmlSchemaAttributeUse *)attrUseList->items[i];
		...
		else if((attrUse->occurs == XML_SCHEMAS_ATTR_USE_OPTIONAL) && (attrUse->defValue || attrDecl->defValue)) {
			/*
	...
	for(i = 0; i < vctxt->nbAttrInfos; i++) {
		 ...
		 */
		if((iattr->decl->flags & XML_SCHEMAS_ATTR_FIXED) || (iattr->use && (iattr->use->flags & XML_SCHEMAS_ATTR_FIXED)))
			fixed = 1;
		else
			fixed = 0;
		/*
		...
		switch(iattr->state) {
			 ...
			 * for the following errors.
			    ...
			    if(type->attributeWildcard == NULL) {
				    ...
				    xmlSchemaIllegalAttrErr(ACTXT_CAST vctxt, XML_SCHEMAV_CVC_COMPLEX_TYPE_3_2_2, iattr, 0);
			    }
			    break;
			default:
			    break;
...
static int xmlSchemaValidateElemWildcard(xmlSchemaValidCtxt * vctxt, int * skip)
{
	...
	// 
	if(!skip || !wild || wild->type != XML_SCHEMA_TYPE_ANY) {
 ...
 * value is provided.
 */
static int xmlSchemaCheckCOSValidDefault(xmlSchemaValidCtxt * vctxt, const xmlChar * value, xmlSchemaVal ** val)
{
	int ret = 0;
...
static void xmlSchemaVContentModelCallback(xmlSchemaValidCtxt * vctxt ATTRIBUTE_UNUSED,
...
static int xmlSchemaValidatorPushElem(xmlSchemaValidCtxt * vctxt)
{
...
static int xmlSchemaVCheckINodeDataType(xmlSchemaValidCtxt * vctxt, xmlSchemaNodeInfoPtr inode, xmlSchemaType * type, const xmlChar * value)
{
 ...
 */
static int xmlSchemaValidatorPopElem(xmlSchemaValidCtxt * vctxt)
{
	int ret = 0;
	xmlSchemaNodeInfoPtr inode = vctxt->inode;
	if(vctxt->nbAttrInfos != 0)
	...
	if(!inode->typeDef || (inode->flags & XML_SCHEMA_NODE_INFO_ERR_BAD_TYPE)) {
		/*
		 * 1. the type definition might be missing if the element was error prone
				...
				if(WXS_HAS_MIXED_CONTENT(inode->typeDef)) {
					...
					if(!sstreq(inode->value, inode->decl->value)) {
						...
						xmlSchemaCustomErr(ACTXT_CAST vctxt, (xmlParserErrors)ret, NULL, NULL,
						    "The initial value '%s' does not match the fixed value constraint '%s'", inode->value, inode->decl->value);
						goto end_elem;
					}
				}
				else if(WXS_HAS_SIMPLE_CONTENT(inode->typeDef)) {
					...
					if(!sstreq(inode->value, inode->decl->value)) {
						...
						xmlSchemaCustomErr(ACTXT_CAST vctxt, (xmlParserErrors)ret, NULL, NULL,
						    "The actual value '%s' does not match the fixed value constraint '%s'", inode->value, inode->decl->value);
						goto end_elem;
					}
				}
			}
 ...
 */
static int xmlSchemaValidateChildElem(xmlSchemaValidCtxt * vctxt)
{
...
static int xmlSchemaVPushText(xmlSchemaValidCtxt * vctxt, int nodeType, const xmlChar * value, int len, int mode, int * consumed)
{
	// 
	...
	if(INODE_NILLED(vctxt->inode)) {
		// 
		// SPEC cvc-elt (3.3.4 - 3.2.1)
		// "The element information item must have no character or element information item [children]."
		// 
	...
	if(vctxt->inode->typeDef->contentType == XML_SCHEMA_CONTENT_ELEMENTS) {
		if(nodeType != XML_TEXT_NODE || !xmlSchemaIsBlank((xmlChar *)value, len)) {
			...
			// "If the {content type} is element-only, then the element information item has no character information
			// item [children] other than those whose [character code] is defined as a white space in [XML 1.0 (Second Edition)]."
			// 
	...
	// 
	// Save the value.
	// NOTE that even if the content type is *mixed*, we need the *initial value* for default/fixed value constraints.
	// 
	if((vctxt->inode->typeDef->contentType == XML_SCHEMA_CONTENT_MIXED) && ((vctxt->inode->decl == NULL) || (vctxt->inode->decl->value == NULL)))
		return 0;
	if(!vctxt->inode->value) {
		// 
		// Set the value.
		// 
		switch(mode) {
			...
			case XML_SCHEMA_PUSH_TEXT_CREATED:
			    // 
			    // When working with the reader. The value will be freed by the element info.
			    // 
			    vctxt->inode->value = value;
			...
			case XML_SCHEMA_PUSH_TEXT_VOLATILE:
			    // 
			    // When working with SAX. The value will be freed by the element info.
			    // 
			    vctxt->inode->value = (len != -1) ? (BAD_CAST xmlStrndup(value, len)) : sstrdup(value);
			    vctxt->inode->flags |= XML_SCHEMA_NODE_INFO_FLAG_OWNED_VALUES;
			    break;
			default:
			    break;
...
static int xmlSchemaValidateElem(xmlSchemaValidCtxt * vctxt)
{
	int ret = 0;
	...
	if(vctxt->xsiAssemble) {
		// 
		// We will stop validation if there was an error during dynamic schema construction.
		// Note that we simply set @skipDepth to 0, this could mean that a streaming document via SAX would be
		// still read to the end but it won't be validated any more.
		// @todo If we are sure how to stop the validation at once for all input scenarios, then this should be changed to
	...
	if(vctxt->depth > 0) {
		...
		if(vctxt->depth == vctxt->skipDepth)
			goto exit;
		if(!vctxt->inode->decl && !vctxt->inode->typeDef) {
...
type_validation:
	...
	// during validation against the declaration. This must be done _before_ attribute validation.
	// 
	if(vctxt->xpathStates) {
	...
	if(WXS_IS_COMPLEX(vctxt->inode->typeDef)) {
		if(vctxt->nbAttrInfos || vctxt->inode->typeDef->attrUses) {
...
static int xmlSchemaVReaderWalk(xmlSchemaValidCtxt * vctxt)
{
...
static void xmlSchemaClearValidCtxt(xmlSchemaValidCtxt * vctxt)
{
	if(vctxt) {
		...
		//   Might be problematic if one reuses the context and assumes that the options remain the same.
		// 
		vctxt->flags = 0;
...
static int xmlSchemaVDocWalk(xmlSchemaValidCtxt * vctxt)
{
	xmlAttr * attr;
	int ret = 0;
	...
	xmlNode * valRoot = NZOR(vctxt->validationRoot, xmlDocGetRootElement(vctxt->doc));
	if(valRoot == NULL) {
		// VAL TODO: Error code? 
		...
		for(xmlNode * p_node = valRoot; p_node;) {
			if(vctxt->skipDepth != -1 && vctxt->depth >= vctxt->skipDepth)
				goto next_sibling;
			if(p_node->type == XML_ELEMENT_NODE) {
				...
				if(p_node->properties) {
					...
					do {
						...
						ret = xmlSchemaValidatorPushAttribute(vctxt, (xmlNode *)attr, ielem->nodeLine, attr->name, nsName, 0, xmlNodeListGetString(attr->doc, attr->children, 1), 1);
						if(ret == -1) {
				...
				// 
				VERROR_INT("xmlSchemaVDocWalk", "there is at least one entity reference in the node-tree currently being validated. Processing of entities with "
...
static int xmlSchemaPreRun(xmlSchemaValidCtxt * vctxt) 
{
	/*
	 * Some initialization.
	 */
	...
	if(vctxt->schema == NULL) {
		...
		if(!vctxt->pctxt && (xmlSchemaCreatePCtxtOnVCtxt(vctxt) == -1))
			return -1;
		pctxt = vctxt->pctxt;
		pctxt->xsiAssemble = true;
		vctxt->schema = xmlSchemaNewSchema(pctxt); // Create the schema.
		if(vctxt->schema == NULL)
			return -1;
		pctxt->constructor = xmlSchemaConstructionCtxtCreate(pctxt->dict); // Create the schema construction context.
		if(pctxt->constructor == NULL)
			return -1;
		pctxt->constructor->mainSchema = vctxt->schema;
		pctxt->ownsConstructor = true; // Take ownership of the constructor to be able to free it.
	}
	// 
	// Augment the IDC definitions for the main schema and all imported ones
...
static void xmlSchemaPostRun(xmlSchemaValidCtxt * vctxt) 
{
	if(vctxt->xsiAssemble) {
		if(vctxt->schema) {
...
static int xmlSchemaVStart(xmlSchemaValidCtxt * vctxt)
{
	int ret = 0;
...
{
	xmlSchemaSAXPlugPtr ctxt = static_cast<xmlSchemaSAXPlugStruct *>(ctx);
...
xmlSchemaSAXPlugPtr xmlSchemaSAXPlug(xmlSchemaValidCtxt * ctxt, xmlSAXHandler ** sax, void ** user_data)
{
	...
	if(old_sax == NULL) {
		...
		*user_data = ctxt;
		...
		ret->schemas_sax.ignorableWhitespace = (old_sax->ignorableWhitespace && old_sax->ignorableWhitespace != old_sax->characters) ? ignorableWhitespaceSplit : charactersSplit;
...
int xmlSchemaValidateStream(xmlSchemaValidCtxt * ctxt, xmlParserInputBuffer * input, xmlCharEncoding enc, xmlSAXHandler * sax, void * user_data)
{
	...
	if(!ret && !ctxt->parserCtxt->wellFormed) {
 ...
 *   number otherwise and -1 in case of an internal or API error.
 */
int xmlSchemaValidateFile(xmlSchemaValidCtxt * ctxt, const char * filename, int options ATTRIBUTE_UNUSED)
{
	int ret = -1;
	if(ctxt && !isempty(filename)) {
		xmlParserInputBuffer * input = xmlParserInputBufferCreateFilename(filename, XML_CHAR_ENCODING_NONE);
		if(input)
