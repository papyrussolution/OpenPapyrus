 ...
 */
static xmlParserInputPtr testExternalEntityLoader(const char * URL, const char * ID,
...
static void testStructuredErrorHandler(void * ctx ATTRIBUTE_UNUSED, xmlErrorPtr err) 
{
	...
	level = err->level;
	node = err->node;
	if((domain == XML_FROM_PARSER) || (domain == XML_FROM_HTML) || (domain == XML_FROM_DTD) || (domain == XML_FROM_NAMESPACE) || (domain == XML_FROM_IO) || (domain == XML_FROM_VALID)) {
		ctxt = err->ctxt;
	}
	str = err->message;
	if(code == XML_ERR_OK)
 ...
 */
static xmlParserInput * resolveEntityDebug(void * ctx ATTRIBUTE_UNUSED, const xmlChar * publicId, const xmlChar * systemId)
{
	callbacks++;
	if(quiet)
		return NULL;
 ...
 */
static void attributeDeclDebug(void * ctx ATTRIBUTE_UNUSED, const xmlChar * elem, const xmlChar * name, int type, int def, const xmlChar * defaultValue, xmlEnumeration * tree)
{
	callbacks++;
	if(quiet)
		return;
 ...
 */
static void elementDeclDebug(void * ctx ATTRIBUTE_UNUSED, const xmlChar * name, int type, xmlElementContent * content ATTRIBUTE_UNUSED)
{
	callbacks++;
	if(quiet)
		return;
...
{
	const xmlChar * nullstr = BAD_CAST "(null)";
	SETIFZQ(publicId, nullstr);
	SETIFZQ(systemId, nullstr);
	SETIFZQ(notationName, nullstr);
	callbacks++;
	if(quiet)
		return;
...
static xmlSAXHandler * debugHTMLSAXHandler = &debugHTMLSAXHandlerStruct;
 ...
 * Returns 0 in case of success, an error code otherwise
 */
static int errParseTest(const char * filename, const char * result, const char * err,
    int options) {
	xmlDocPtr doc;
...
static int streamProcessTest(const char * filename, const char * result, const char * err, xmlTextReader * reader, const char * rng, int options) 
{
	int ret;
	char * temp = NULL;
	FILE * t = NULL;
...
static void testXPath(const char * str, int xptr, int expr) 
{
	...
	ctxt->P_Node = xmlDocGetRootElement(xpathDocument);
	if(expr)
 ...
 * Returns 0 in case of success, an error code otherwise
 */
static int uriParseTest(const char * filename,
    const char * result,
    const char * err,
    int options ATTRIBUTE_UNUSED) {
 ...
 * Returns 0 in case of success, an error code otherwise
 */
static int uriBaseTest(const char * filename,
    const char * result,
    const char * err,
    int options ATTRIBUTE_UNUSED) {
...
*									*
************************************************************************/
static int rngOneTest(const char * sch,
    const char * filename,
    const char * result,
    const char * err,
    int options,
 ...
 * Returns 0 in case of success, an error code otherwise
 */
static int rngTest(const char * filename,
    const char * resul ATTRIBUTE_UNUSED,
    const char * errr ATTRIBUTE_UNUSED,
    int options) {
	const char * base = baseFilename(filename);
...
*									*
************************************************************************/
static void patternNode(FILE * out, xmlTextReader * reader, const char * pattern, xmlPatternPtr patternc, xmlStreamCtxtPtr patstream) 
{
	xmlChar * path = NULL;
	int match = -1;
	int type, empty;
