...
static void FASTCALL htmlErrMemory(xmlParserCtxt * ctxt, const char * extra)
{
	...
	if(extra)
		__xmlRaiseError(0, 0, 0, ctxt, 0, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, 0, 0, 0, 0, "Memory allocation failed : %s\n", extra);
	else
		__xmlRaiseError(0, 0, 0, ctxt, 0, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, 0, 0, 0, 0, 0, 0, 0, "Memory allocation failed\n");
}
/**
 * htmlParseErr:
 * @ctxt:  an HTML parser context
...
static void FASTCALL htmlParseErrInt(xmlParserCtxt * ctxt, xmlParserErrors error, const char * msg, int val)
{
	...
	if(ctxt)
		ctxt->errNo = error;
	__xmlRaiseError(0, 0, 0, ctxt, 0, XML_FROM_HTML, error, XML_ERR_ERROR, 0, 0, 0, 0, 0, val, 0, msg, val);
	if(ctxt)
		ctxt->wellFormed = 0;
}
// 
 ...
 */
static int FASTCALL htmlnamePush(htmlParserCtxt * ctxt, const xmlChar * value)
{
	if((ctxt->html < 3) && (sstreq(value, "head")))
		ctxt->html = 3;
	if((ctxt->html < 10) && (sstreq(value, "body")))
		ctxt->html = 10;
 ...
 */
static const xmlChar * FASTCALL htmlnamePop(htmlParserCtxt * ctxt)
{
	const xmlChar * ret;
	if(ctxt->nameNr <= 0)
		return NULL;
 ...
 * Returns 0 in case of error, the index in the stack otherwise
 */
static int htmlNodeInfoPush(htmlParserCtxt * ctxt, htmlParserNodeInfo * value)
{
	if(ctxt->nodeInfoNr >= ctxt->nodeInfoMax) {
		SETIFZ(ctxt->nodeInfoMax, 5);
		ctxt->nodeInfoMax *= 2;
		ctxt->nodeInfoTab = (htmlParserNodeInfo*)SAlloc::R((htmlParserNodeInfo*)ctxt->nodeInfoTab, ctxt->nodeInfoMax * sizeof(ctxt->nodeInfoTab[0]));
 ...
 */
static htmlParserNodeInfo * htmlNodeInfoPop(htmlParserCtxt * ctxt)
{
	...
	if(ctxt->nodeInfoNr < 0)
		return NULL;
	ctxt->nodeInfo = (ctxt->nodeInfoNr > 0) ? &ctxt->nodeInfoTab[ctxt->nodeInfoNr - 1] : 0;
...
static xmlChar * htmlFindEncoding(xmlParserCtxt * ctxt)
{
	...
	if(!ctxt || (ctxt->input == NULL) || ctxt->input->encoding || (ctxt->input->buf == NULL) || ctxt->input->buf->encoder)
		return NULL;
	 ...
	 */
	{
		...
		if(guess == NULL) {
			...
			if(handler) {
				...
				htmlParseErr(ctxt, XML_ERR_INVALID_ENCODING, "Unsupported encoding %s", guess, 0);
			}
		}
...
static const char * const a_attrs[] = { ATTRS, "charset", "type", "name", "href", "hreflang", "rel", "rev", "accesskey", "shape", "coords", "tabindex", "onfocus", "onblur", NULL };
...
static const char * const basefont_attrs[] = { "id", "size", "color", "face", NULL };
...
static const htmlElemDesc
    html40ElementTable[] = {
	{ "a",          0, 0, 0, 0, 0, 0, 1, "anchor ", DECL html_inline, NULL, DECL a_attrs, DECL target_attr, NULL},
	{ "abbr",       0, 0, 0, 0, 0, 0, 1, "abbreviated form", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "acronym",    0, 0, 0, 0, 0, 0, 1, "", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "address",    0, 0, 0, 0, 0, 0, 0, "information on author ", DECL inline_p, NULL, DECL html_attrs, NULL, NULL},
	{ "applet",     0, 0, 0, 0, 1, 1, 2, "java applet ", DECL flow_param, NULL, NULL, DECL applet_attrs, NULL},
	{ "area",       0, 2, 2, 1, 0, 0, 0, "client-side image map area ", EMPTY,  NULL, DECL area_attrs, DECL target_attr, DECL alt_attr},
	{ "b",          0, 3, 0, 0, 0, 0, 1, "bold text style", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "base",       0, 2, 2, 1, 0, 0, 0, "document base uri ", EMPTY, NULL, NULL, DECL target_attr, DECL href_attrs},
	{ "basefont",   0, 2, 2, 1, 1, 1, 1, "base font size ", EMPTY, NULL, NULL, DECL basefont_attrs, NULL},
	{ "bdo",        0, 0, 0, 0, 0, 0, 1, "i18n bidi over-ride ", DECL html_inline, NULL, DECL core_i18n_attrs, NULL, DECL dir_attr},
	{ "big",        0, 3, 0, 0, 0, 0, 1, "large text style", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "blockquote", 0, 0, 0, 0, 0, 0, 0, "long quotation ", DECL html_flow, NULL, DECL quote_attrs, NULL, NULL},
	{ "body",       1, 1, 0, 0, 0, 0, 0, "document body ", DECL body_contents, "div", DECL body_attrs, DECL body_depr, NULL},
	{ "br",         0, 2, 2, 1, 0, 0, 1, "forced line break ", EMPTY, NULL, DECL core_attrs, DECL clear_attrs, NULL},
	{ "button",     0, 0, 0, 0, 0, 0, 2, "push button ", DECL html_flow MODIFIER, NULL, DECL button_attrs, NULL, NULL},
	{ "caption",    0, 0, 0, 0, 0, 0, 0, "table caption ", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "center",     0, 3, 0, 0, 1, 1, 0, "shorthand for div align=center ", DECL html_flow, NULL, NULL, DECL html_attrs, NULL},
	{ "cite",       0, 0, 0, 0, 0, 0, 1, "citation", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "code",       0, 0, 0, 0, 0, 0, 1, "computer code fragment", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "col",        0, 2, 2, 1, 0, 0, 0, "table column ", EMPTY, NULL, DECL col_attrs, NULL, NULL},
	{ "colgroup",   0, 1, 0, 0, 0, 0, 0, "table column group ", DECL col_elt, "col", DECL col_attrs, NULL, NULL},
	{ "dd",         0, 1, 0, 0, 0, 0, 0, "definition description ", DECL html_flow, NULL, DECL html_attrs, NULL, NULL},
	{ "del",        0, 0, 0, 0, 0, 0, 2, "deleted text ", DECL html_flow, NULL, DECL edit_attrs, NULL, NULL},
	{ "dfn",        0, 0, 0, 0, 0, 0, 1, "instance definition", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "dir",        0, 0, 0, 0, 1, 1, 0, "directory list", DECL blockli_elt, "li", NULL, DECL compact_attrs, NULL},
	{ "div",        0, 0, 0, 0, 0, 0, 0, "generic language/style container", DECL html_flow, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "dl",         0, 0, 0, 0, 0, 0, 0, "definition list ", DECL dl_contents, "dd", DECL html_attrs, DECL compact_attr, NULL},
	{ "dt",         0, 1, 0, 0, 0, 0, 0, "definition term ", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "em",         0, 3, 0, 0, 0, 0, 1, "emphasis", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "embed",      0, 1, 0, 0, 1, 1, 1, "generic embedded object ", EMPTY, NULL, DECL embed_attrs, NULL, NULL},
	{ "fieldset",   0, 0, 0, 0, 0, 0, 0, "form control group ", DECL fieldset_contents, NULL, DECL html_attrs, NULL, NULL},
	{ "font",       0, 3, 0, 0, 1, 1, 1, "local change to font ", DECL html_inline, NULL, NULL, DECL font_attrs, NULL},
	{ "form",       0, 0, 0, 0, 0, 0, 0, "interactive form ", DECL form_contents, "fieldset", DECL form_attrs, DECL target_attr, DECL action_attr},
	{ "frame",      0, 2, 2, 1, 0, 2, 0, "subwindow ", EMPTY, NULL, NULL, DECL frame_attrs, NULL},
	{ "frameset",   0, 0, 0, 0, 0, 2, 0, "window subdivision", DECL frameset_contents, "noframes", NULL, DECL frameset_attrs, NULL},
	{ "h1",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "h2",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "h3",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "h4",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "h5",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "h6",         0, 0, 0, 0, 0, 0, 0, "heading ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "head",       1, 1, 0, 0, 0, 0, 0, "document head ", DECL head_contents, NULL, DECL head_attrs, NULL, NULL},
	{ "hr",         0, 2, 2, 1, 0, 0, 0, "horizontal rule ", EMPTY, NULL, DECL html_attrs, DECL hr_depr, NULL},
	{ "html",       1, 1, 0, 0, 0, 0, 0, "document root element ", DECL html_content, NULL, DECL i18n_attrs, DECL version_attr, NULL},
	{ "i",          0, 3, 0, 0, 0, 0, 1, "italic text style", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "iframe",     0, 0, 0, 0, 0, 1, 2, "inline subwindow ", DECL html_flow, NULL, NULL, DECL iframe_attrs, NULL},
	{ "img",        0, 2, 2, 1, 0, 0, 1, "embedded image ", EMPTY, NULL, DECL img_attrs, DECL align_attr, DECL src_alt_attrs},
	{ "input",      0, 2, 2, 1, 0, 0, 1, "form control ", EMPTY, NULL, DECL input_attrs, DECL align_attr, NULL},
	{ "ins",        0, 0, 0, 0, 0, 0, 2, "inserted text", DECL html_flow, NULL, DECL edit_attrs, NULL, NULL},
	{ "isindex",    0, 2, 2, 1, 1, 1, 0, "single line prompt ", EMPTY, NULL, NULL, DECL prompt_attrs, NULL},
	{ "kbd",        0, 0, 0, 0, 0, 0, 1, "text to be entered by the user", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "label",      0, 0, 0, 0, 0, 0, 1, "form field label text ", DECL html_inline MODIFIER, NULL, DECL label_attrs, NULL, NULL},
	{ "legend",     0, 0, 0, 0, 0, 0, 0, "fieldset legend ", DECL html_inline, NULL, DECL legend_attrs, DECL align_attr, NULL},
	{ "li",         0, 1, 1, 0, 0, 0, 0, "list item ", DECL html_flow, NULL, DECL html_attrs, NULL, NULL},
	{ "link",       0, 2, 2, 1, 0, 0, 0, "a media-independent link ", EMPTY, NULL, DECL link_attrs, DECL target_attr, NULL},
	{ "map",        0, 0, 0, 0, 0, 0, 2, "client-side image map ", DECL map_contents, NULL, DECL html_attrs, NULL, DECL name_attr},
	{ "menu",       0, 0, 0, 0, 1, 1, 0, "menu list ", DECL blockli_elt, NULL, NULL, DECL compact_attrs, NULL},
	{ "meta",       0, 2, 2, 1, 0, 0, 0, "generic metainformation ", EMPTY, NULL, DECL meta_attrs, NULL, DECL content_attr},
	{ "noframes",   0, 0, 0, 0, 0, 2, 0, "alternate content container for non frame-based rendering ", DECL noframes_content, "body", DECL html_attrs, NULL, NULL},
	{ "noscript",   0, 0, 0, 0, 0, 0, 0, "alternate content container for non script-based rendering ", DECL html_flow, "div", DECL html_attrs, NULL, NULL},
	{ "object",     0, 0, 0, 0, 0, 0, 2, "generic embedded object ", DECL object_contents, "div", DECL object_attrs, DECL object_depr, NULL},
	{ "ol",         0, 0, 0, 0, 0, 0, 0, "ordered list ", DECL li_elt, "li", DECL html_attrs, DECL ol_attrs, NULL},
	{ "optgroup",   0, 0, 0, 0, 0, 0, 0, "option group ", DECL option_elt, "option", DECL optgroup_attrs, NULL, DECL label_attr},
	{ "option",     0, 1, 0, 0, 0, 0, 0, "selectable choice ", DECL html_pcdata, NULL, DECL option_attrs, NULL, NULL},
	{ "p",          0, 1, 0, 0, 0, 0, 0, "paragraph ", DECL html_inline, NULL, DECL html_attrs, DECL align_attr, NULL},
	{ "param",      0, 2, 2, 1, 0, 0, 0, "named property value ", EMPTY, NULL, DECL param_attrs, NULL, DECL name_attr},
	{ "pre",        0, 0, 0, 0, 0, 0, 0, "preformatted text ", DECL pre_content, NULL, DECL html_attrs, DECL width_attr, NULL},
	{ "q",          0, 0, 0, 0, 0, 0, 1, "short inline quotation ", DECL html_inline, NULL, DECL quote_attrs, NULL, NULL},
	{ "s",          0, 3, 0, 0, 1, 1, 1, "strike-through text style", DECL html_inline, NULL, NULL, DECL html_attrs, NULL},
	{ "samp",       0, 0, 0, 0, 0, 0, 1, "sample program output, scripts, etc.", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "script",     0, 0, 0, 0, 0, 0, 2, "script statements ", DECL html_cdata, NULL, DECL script_attrs, DECL language_attr, DECL type_attr},
	{ "select",     0, 0, 0, 0, 0, 0, 1, "option selector ", DECL select_content, NULL, DECL select_attrs, NULL, NULL},
	{ "small",      0, 3, 0, 0, 0, 0, 1, "small text style", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "span",       0, 0, 0, 0, 0, 0, 1, "generic language/style container ", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "strike",     0, 3, 0, 0, 1, 1, 1, "strike-through text", DECL html_inline, NULL, NULL, DECL html_attrs, NULL},
	{ "strong",     0, 3, 0, 0, 0, 0, 1, "strong emphasis", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "style",      0, 0, 0, 0, 0, 0, 0, "style info ", DECL html_cdata, NULL, DECL style_attrs, NULL, DECL type_attr},
	{ "sub",        0, 3, 0, 0, 0, 0, 1, "subscript", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "sup",        0, 3, 0, 0, 0, 0, 1, "superscript ", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "table",      0, 0, 0, 0, 0, 0, 0, "", DECL table_contents, "tr", DECL table_attrs, DECL table_depr, NULL},
	{ "tbody",      1, 0, 0, 0, 0, 0, 0, "table body ", DECL tr_elt, "tr", DECL talign_attrs, NULL, NULL},
	{ "td",         0, 0, 0, 0, 0, 0, 0, "table data cell", DECL html_flow, NULL, DECL th_td_attr, DECL th_td_depr, NULL},
	{ "textarea",   0, 0, 0, 0, 0, 0, 1, "multi-line text field ", DECL html_pcdata, NULL, DECL textarea_attrs, NULL, DECL rows_cols_attr},
	{ "tfoot",      0, 1, 0, 0, 0, 0, 0, "table footer ", DECL tr_elt, "tr", DECL talign_attrs, NULL, NULL},
	{ "th",         0, 1, 0, 0, 0, 0, 0, "table header cell", DECL html_flow, NULL, DECL th_td_attr, DECL th_td_depr, NULL},
	{ "thead",      0, 1, 0, 0, 0, 0, 0, "table header ", DECL tr_elt, "tr", DECL talign_attrs, NULL, NULL},
	{ "title",      0, 0, 0, 0, 0, 0, 0, "document title ", DECL html_pcdata, NULL, DECL i18n_attrs, NULL, NULL},
	{ "tr",         0, 0, 0, 0, 0, 0, 0, "table row ", DECL tr_contents, "td", DECL talign_attrs, DECL bgcolor_attr, NULL},
	{ "tt",         0, 3, 0, 0, 0, 0, 1, "teletype or monospaced text style", DECL html_inline, NULL, DECL html_attrs, NULL, NULL},
	{ "u",          0, 3, 0, 0, 1, 1, 1, "underlined text style", DECL html_inline, NULL, NULL, DECL html_attrs, NULL},
	{ "ul",         0, 0, 0, 0, 0, 0, 0, "unordered list ", DECL li_elt, "li", DECL html_attrs, DECL ul_depr, NULL},
	{ "var",        0, 0, 0, 0, 0, 0, 1, "instance of a variable or program argument", DECL html_inline, NULL, DECL html_attrs, NULL, NULL}
};
...
static int htmlCheckAutoClose(const xmlChar * newtag, const xmlChar * oldtag)
{
	...
	for(indx = 0; indx < 100; indx++) {
		...
		if(closed == NULL)
			return 0;
		if(sstreq(BAD_CAST *closed, newtag))
			break;
	}
 ...
 */
static void htmlAutoCloseOnClose(htmlParserCtxt * ctxt, const xmlChar * newtag)
{
	int i;
	int priority = htmlGetEndPriority(newtag);
	for(i = (ctxt->nameNr - 1); i >= 0; i--) {
		if(sstreq(newtag, ctxt->nameTab[i]))
			break;
		/*
 ...
 */
static void htmlAutoCloseOnEnd(htmlParserCtxt * ctxt)
{
	if(ctxt->nameNr) {
		for(int i = (ctxt->nameNr - 1); i >= 0; i--) {
			if(ctxt->sax && ctxt->sax->endElement)
				ctxt->sax->endElement(ctxt->userData, ctxt->name);
			htmlnamePop(ctxt);
		}
 ...
 * and we should check
 */
static void htmlAutoClose(htmlParserCtxt * ctxt, const xmlChar * newtag)
{
 ...
 */
static void htmlCheckImplied(htmlParserCtxt * ctxt, const xmlChar * newtag) 
{
	...
	if(!ctxt)
		...
		if(tag == NULL) {
			...
			if(htmlOmittedDefaultValue) {
				for(int i = 0; htmlNoContentElements[i]; i++) {
...
#define growBuffer(buffer) {						\
		buffer ## _size *= 2;						      \
		xmlChar * tmp = static_cast<xmlChar *>(SAlloc::R(buffer, buffer ## _size * sizeof(xmlChar))); \
		if(!tmp) {					   \
...
const htmlEntityDesc * htmlEntityLookup(const xmlChar * name)
{
	for(uint i = 0; i < SIZEOFARRAY(html40EntitiesTable); i++) {
		if(sstreq(name, html40EntitiesTable[i].name))
...
static htmlParserInputPtr htmlNewInputStream(htmlParserCtxt * ctxt)
{
	...
	if(!input)
		...
		memzero(input, sizeof(htmlParserInput));
		input->filename = NULL;
...
static xmlChar * htmlParseAttValue(htmlParserCtxt * ctxt) 
{
	xmlChar * ret = NULL;
	if(CUR == '"') {
		xmlNextChar(ctxt);
...
static xmlChar * htmlParseSystemLiteral(htmlParserCtxt * ctxt) 
{
	const xmlChar * q;
	xmlChar * ret = NULL;
	if(CUR == '"') {
 ...
 */
static xmlChar * htmlParsePubidLiteral(htmlParserCtxt * ctxt) 
{
	const xmlChar * q;
	xmlChar * ret = NULL;
	/*
...
static void htmlParseScript(htmlParserCtxt * ctxt) 
{
	...
	while(IS_CHAR_CH(cur)) {
		if((cur == '<') && (NXT(1) == '/')) {
			 ...
			 */
			if(ctxt->recovery) {
				if(xmlStrncasecmp(ctxt->name, ctxt->input->cur+2, sstrlen(ctxt->name)) == 0) {
					break; /* while */
				}
				else {
...
static void htmlParseCharData(htmlParserCtxt * ctxt)
{
	xmlChar buf[HTML_PARSER_BIG_BUFFER_SIZE + 5];
	int nbchar = 0;
	int cur, l;
	...
	SHRINK;
	cur = CUR_CHAR(l);
	while(((cur != '<') || (ctxt->token == '<')) && ((cur != '&') || (ctxt->token == '&')) && cur) {
		if(!(IS_CHAR(cur))) {
			htmlParseErrInt(ctxt, XML_ERR_INVALID_CHAR, "Invalid char in CDATA 0x%X\n", cur);
		}
		else {
...
{
	xmlChar * URI = NULL;
	if((UPPER == 'S') && (UPP(1) == 'Y') && (UPP(2) == 'S') && (UPP(3) == 'T') && (UPP(4) == 'E') && (UPP(5) == 'M')) {
		SKIP(6);
		if(!IS_BLANK_CH(CUR)) {
	...
	else if((UPPER == 'P') && (UPP(1) == 'U') && (UPP(2) == 'B') && (UPP(3) == 'L') && (UPP(4) == 'I') && (UPP(5) == 'C')) {
		SKIP(6);
		if(!IS_BLANK_CH(CUR)) {
 ...
 */
static void htmlParsePI(htmlParserCtxt * ctxt) 
{
	xmlChar * buf = NULL;
	int len = 0;
	...
	if((RAW == '<') && (NXT(1) == '?')) {
		...
		if(target) {
			...
			htmlParseErr(ctxt, XML_ERR_PI_NOT_STARTED, "PI is not started correctly", 0, 0);
		}
		ctxt->instate = state;
	}
}
...
static void htmlParseDocTypeDecl(htmlParserCtxt * ctxt) 
{
	 ...
	 */
	if(ctxt->sax && ctxt->sax->internalSubset && (!ctxt->disableSAX))
...
static const xmlChar * htmlParseAttribute(htmlParserCtxt * ctxt, xmlChar ** value) 
{
	const xmlChar * name;
	xmlChar * val = NULL;
	*value = NULL;
 ...
 * it and pass UTF8
 */
static void htmlCheckEncodingDirect(htmlParserCtxt * ctxt, const xmlChar * encoding)
{
	if(!ctxt || (encoding == NULL) || (ctxt->options & HTML_PARSE_IGNORE_ENC))
		return;
	...
	if(encoding) {
		...
		if(enc != XML_CHAR_ENCODING_ERROR) {
			if(((enc == XML_CHAR_ENCODING_UTF16LE) || (enc == XML_CHAR_ENCODING_UTF16BE) || (enc == XML_CHAR_ENCODING_UCS4LE) ||
				    (enc == XML_CHAR_ENCODING_UCS4BE)) && ctxt->input->buf && (ctxt->input->buf->encoder == NULL)) {
				htmlParseErr(ctxt, XML_ERR_INVALID_ENCODING, "htmlCheckEncoding: wrong encoding meta\n", 0, 0);
			}
			else {
			...
			if(handler) {
				...
				htmlParseErr(ctxt, XML_ERR_UNSUPPORTED_ENCODING, "htmlCheckEncoding: unknown encoding %s\n", encoding, 0);
			}
		}
		if(ctxt->input->buf && ctxt->input->buf->encoder && ctxt->input->buf->raw && ctxt->input->buf->buffer) {
			int nbchars;
			int processed;
			/*
			...
			if(nbchars < 0) {
				htmlParseErr(ctxt, XML_ERR_INVALID_ENCODING, "htmlCheckEncoding: encoder error\n", 0, 0);
			}
 ...
 * it and pass UTF8
 */
static void htmlCheckEncoding(htmlParserCtxt * ctxt, const xmlChar * attvalue)
{
 ...
 */
static void htmlCheckMeta(htmlParserCtxt * ctxt, const xmlChar ** atts)
{
	int i;
 ...
 */
static int htmlParseStartTag(htmlParserCtxt * ctxt)
{
	const xmlChar * name;
	...
	if(sstreq(name, "body")) {
		...
		for(indx = 0; indx < ctxt->nameNr; indx++) {
			if(sstreq(ctxt->nameTab[indx], "body")) {
...
failed:
		...
		if(cons == ctxt->nbChars) {
			htmlParseErr(ctxt, XML_ERR_INTERNAL_ERROR, "htmlParseStartTag: problem parsing attributes\n", 0, 0);
			break;
		}
	}
	/*
	...
	if(!discardtag) {
		...
		if(ctxt->sax && ctxt->sax->startElement) {
			if(nbatts != 0)
				...
				ctxt->sax->startElement(ctxt->userData, name, 0);
		}
	}
	if(atts) {
...
static int htmlParseEndTag(htmlParserCtxt * ctxt)
{
	const xmlChar * name;
	const xmlChar * oldname;
	int i, ret;
	if((CUR != '<') || (NXT(1) != '/')) {
		htmlParseErr(ctxt, XML_ERR_LTSLASH_REQUIRED, "htmlParseEndTag: '</' not found\n", 0, 0);
		return 0;
	}
	SKIP(2);
	name = htmlParseHTMLName(ctxt);
	 ...
	 * out now.
	 */
	if((ctxt->depth > 0) && (sstreq(name, "html") || sstreq(name, "body") || sstreq(name, "head"))) {
		ctxt->depth--;
		return 0;
	}
	/*
 ...
 */
static void htmlParseReference(htmlParserCtxt * ctxt) 
{
	xmlChar out[6];
	if(CUR == '&') {
		if(NXT(1) == '#') {
			...
			const htmlEntityDesc * ent = htmlParseEntityRef(ctxt, &name);
			if(!name) {
				htmlCheckParagraph(ctxt);
				if(ctxt->sax && ctxt->sax->characters)
 ...
 */
static void htmlParseContent(htmlParserCtxt * ctxt)
{
	const xmlChar * name;
	...
	while(1) {
		 ...
		 */
		if((CUR == '<') && (NXT(1) == '/')) {
			if(htmlParseEndTag(ctxt) && (currentNode || (ctxt->nameNr == 0))) {
		...
		if((CUR != 0) && ((sstreq(currentNode, "script")) || (sstreq(currentNode, "style")))) {
			/*
...
void htmlParseElement(htmlParserCtxt * ctxt)
{
	...
	if(!ctxt || (ctxt->input == NULL)) {
		htmlParseErr(ctxt, XML_ERR_INTERNAL_ERROR, "htmlParseElement: context error\n", 0, 0);
		return;
	}
	if(ctxt->instate == XML_PARSER_EOF)
		return;
	 ...
	 * Capture end position and add node
	 */
	if(currentNode && ctxt->record_info) {
		node_info.end_pos = ctxt->input->consumed + (CUR_PTR - ctxt->input->base);
		node_info.end_line = ctxt->input->line;
		node_info.P_Node = ctxt->P_Node;
		xmlParserAddNodeInfo(ctxt, &node_info);
...
static void htmlParserFinishElementParsing(htmlParserCtxt * ctxt)
{
	/*
	 * Capture end position and add node
	 */
	if(ctxt->P_Node && ctxt->record_info) {
 ...
 * [41] Attribute ::= Name Eq AttValue
 */
static void htmlParseElementInternal(htmlParserCtxt * ctxt)
{
	const xmlChar * name;
	const htmlElemDesc * info;
 ...
 */
static void htmlParseContentInternal(htmlParserCtxt * ctxt)
{
	const xmlChar * name;
	xmlChar * currentNode = sstrdup(ctxt->name);
	int depth = ctxt->nameNr;
...
{
	if(ctxt)
		htmlParseContentInternal((htmlParserCtxt *)ctxt);
}
/**
 * htmlParseDocument:
 * @ctxt:  an HTML parser context
...
int htmlParseDocument(htmlParserCtxt * ctxt)
{
	 ...
	 */
	while(((CUR == '<') && (NXT(1) == '!') && (NXT(2) == '-') && (NXT(3) == '-')) || ((CUR == '<') && (NXT(1) == '?'))) {
		htmlParseComment(ctxt);
		htmlParsePI(ctxt);
		SKIP_BLANKS;
	}
 ...
 */
static int htmlInitParserCtxt(htmlParserCtxt * ctxt)
{
	htmlSAXHandler * sax;
	if(!ctxt)
		return -1;
 ...
 * Returns the new parser context or NULL
 */
static htmlParserCtxt * htmlCreateDocParserCtxt(const xmlChar * cur, const char * encoding)
{
	int len;
	htmlParserCtxt * ctxt;
	if(!cur)
	...
	while(1) {
		...
		switch(ctxt->instate) {
			 ...
			 * Very first chars read from the document flow.
			    ...
			    cur = in->cur[0];
			    next = in->cur[1];
			    if((cur == '<') && (next == '!') && (UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y') && (UPP(7) == 'P') && (UPP(8) == 'E')) {
				    if((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 0, 1) < 0))
					    goto done;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: Parsing internal subset\n");
#endif
				    htmlParseDocTypeDecl(ctxt);
				    ctxt->instate = XML_PARSER_PROLOG;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: entering PROLOG\n");
#endif
			    }
			    else {
				    ctxt->instate = XML_PARSER_MISC;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: entering MISC\n");
#endif
			    }
			    break;
			case XML_PARSER_MISC:
			 ...
			 * not enouth chars in buffer
			    ...
			    (in->cur[2] == '-') && (in->cur[3] == '-')) {
				    if((!terminate) && (htmlParseLookupSequence(ctxt, '-', '-', '>', 1, 1) < 0))
					    goto done;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: Parsing Comment\n");
#endif
				    htmlParseComment(ctxt);
				    ctxt->instate = XML_PARSER_MISC;
			    }
				    ...
				    if((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 0, 1) < 0))
					    goto done;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: Parsing PI\n");
#endif
				    htmlParsePI(ctxt);
				    ctxt->instate = XML_PARSER_MISC;
			    }
			    else if((cur == '<') && (next == '!') && (UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y') && (UPP(7) == 'P') && (UPP(8) == 'E')) {
				    if((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 0, 1) < 0))
					    goto done;
#ifdef DEBUG_PUSH
				    xmlGenericError(0, "HPP: Parsing internal subset\n");
			...
			case XML_PARSER_END_TAG:
			    ...
			    ctxt->instate = (ctxt->nameNr == 0) ? XML_PARSER_EPILOG : XML_PARSER_CONTENT;
			    ctxt->CheckIndex = 0;
#ifdef DEBUG_PUSH
			    xmlGenericError(0, "HPP: entering CONTENT\n");
#endif
...
int htmlParseChunk(htmlParserCtxt * ctxt, const char * chunk, int size, int terminate) 
{
	...
	if(size > 0 && chunk && ctxt->input && ctxt->input->buf && (ctxt->instate != XML_PARSER_EOF)) {
		...
		int res = xmlParserInputBufferPush(ctxt->input->buf, size, chunk);
		if(res < 0) {
		...
		if(ctxt->input && ctxt->input->buf) {
			xmlParserInputBuffer * in = ctxt->input->buf;
			if(in->encoder && in->buffer && in->raw) {
	...
	htmlParseTryOrFinish(ctxt, terminate);
	if(terminate) {
		if(!oneof3(ctxt->instate, XML_PARSER_EOF, XML_PARSER_EPILOG, XML_PARSER_MISC)) {
			ctxt->errNo = XML_ERR_DOCUMENT_END;
			ctxt->wellFormed = 0;
		}
		if(ctxt->instate != XML_PARSER_EOF) {
	...
	if(buf) {
		...
		if(!ctxt) {
			...
			if(enc==XML_CHAR_ENCODING_UTF8 || buf->encoder)
				ctxt->charset = XML_CHAR_ENCODING_UTF8;
			if(sax) {
				if(ctxt->sax != (xmlSAXHandler *)&htmlDefaultSAXHandler)
					SAlloc::F(ctxt->sax);
...
htmlParserCtxt * htmlCreateFileParserCtxt(const char * filename, const char * encoding)
{
	...
	if(encoding) {
		...
		if(l < 1000) {
			xmlChar * content = static_cast<xmlChar *>(SAlloc::M(sstrlen(content_line) + l + 1));
			if(content) {
 ...
 */
static htmlDocPtr FASTCALL htmlDoRead(htmlParserCtxt * ctxt, const char * URL, const char * encoding, int options, int reuse)
{
	htmlDocPtr ret;
	...
	ctxt->myDoc = NULL;
	if(!reuse) {
		if(ctxt->dictNames && ret && (ret->dict == ctxt->dict))
			ctxt->dict = NULL;
		xmlFreeParserCtxt(ctxt);
	}
	return ret;
...
htmlDocPtr htmlReadDoc(const xmlChar * cur, const char * URL, const char * encoding, int options)
{
	if(!cur)
		return NULL;
	xmlInitParser();
	htmlParserCtxt * ctxt = htmlCreateDocParserCtxt(cur, 0);
	return ctxt ? htmlDoRead(ctxt, URL, encoding, options, 0) : 0;
}
...
htmlDocPtr htmlReadFile(const char * filename, const char * encoding, int options)
{
	xmlInitParser();
	htmlParserCtxt * ctxt = htmlCreateFileParserCtxt(filename, encoding);
	return ctxt ? htmlDoRead(ctxt, NULL, NULL, options, 0) : 0;
}
...
{
	xmlInitParser();
	htmlParserCtxt * ctxt = xmlCreateMemoryParserCtxt(buffer, size);
	if(!ctxt)
		return NULL;
	htmlDefaultSAXHandlerInit();
	if(ctxt->sax)
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadDoc(htmlParserCtxt * ctxt, const xmlChar * cur, const char * URL, const char * encoding, int options)
{
	xmlParserInput * stream;
	if(!cur || !ctxt)
		return NULL;
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadFile(htmlParserCtxt * ctxt, const char * filename, const char * encoding, int options)
{
	xmlParserInput * stream;
	if(!filename || !ctxt)
		return NULL;
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadMemory(htmlParserCtxt * ctxt, const char * buffer, int size, const char * URL, const char * encoding, int options)
{
	xmlParserInputBuffer * input;
	xmlParserInput * stream;
	if(!ctxt || !buffer)
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadFd(htmlParserCtxt * ctxt, int fd, const char * URL, const char * encoding, int options)
{
	xmlParserInputBuffer * input;
	xmlParserInput * stream;
	if(fd < 0 || !ctxt)
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadIO(htmlParserCtxt * ctxt, xmlInputReadCallback ioread, xmlInputCloseCallback ioclose, void * ioctx,
