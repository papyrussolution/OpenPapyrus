// dict.c: dictionary of reusable strings, just used to avoid allocation and freeing operations.
 ...
 * -1 in case of error
 */
int FASTCALL xmlDictOwns(const xmlDict * dict, const xmlChar * str) 
{
	if(!dict || !str)
		return -1;
	else {
		for(const xmlDictStrings * pool = dict->strings; pool; pool = pool->next) {
