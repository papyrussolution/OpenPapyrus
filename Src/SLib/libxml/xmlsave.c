...
#define XHTML_FRAME_PUBLIC_ID reinterpret_cast<const xmlChar *>("-//W3C//DTD XHTML 1.0 Frameset//EN")
#define XHTML_FRAME_SYSTEM_ID reinterpret_cast<const xmlChar *>("http://www.w3.org/TR/xhtml1/DTD/xhtml1-frameset.dtd")
#define XHTML_TRANS_PUBLIC_ID reinterpret_cast<const xmlChar *>("-//W3C//DTD XHTML 1.0 Transitional//EN")
#define XHTML_TRANS_SYSTEM_ID reinterpret_cast<const xmlChar *>("http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd")
#define XHTML_NS_NAME reinterpret_cast<const xmlChar *>("http://www.w3.org/1999/xhtml")
/**
 * xmlIsXHTML:
 ...
 * Handle an out of memory condition
 */
static void FASTCALL xmlSaveErr(int code, xmlNode * pNode, const char * extra)
{
	const char * msg = NULL;
	switch(code) {
 ...
 */
static void FASTCALL xmlSaveCtxtInit(xmlSaveCtxt * ctxt)
{
	if(ctxt) {
 ...
 */
static void FASTCALL xmlFreeSaveCtxt(xmlSaveCtxt * ctxt)
{
	if(ctxt) {
...
{
	xmlSaveCtxt * ret = static_cast<xmlSaveCtxt *>(SAlloc::M(sizeof(xmlSaveCtxt)));
	if(!ret)
...
{
	for(const xmlNode * children = attr->children; children; children = children->next) {
		switch(children->type) {
			case XML_TEXT_NODE:
			    xmlBufAttrSerializeTxtContent(buf->buffer, attr->doc, attr, children->content);
			    break;
			case XML_ENTITY_REF_NODE:
			    xmlBufAdd(buf->buffer, reinterpret_cast<const xmlChar *>("&"), 1);
...
// 
static int FASTCALL xmlSaveSwitchEncoding(xmlSaveCtxt * ctxt, const char * encoding)
{
	xmlOutputBuffer * buf = ctxt->buf;
	if(encoding && !buf->encoder && !buf->conv) {
		buf->encoder = xmlFindCharEncodingHandler(reinterpret_cast<const char *>(encoding));
		if(buf->encoder == NULL) {
			xmlSaveErr(XML_SAVE_UNKNOWN_ENCODING, NULL, reinterpret_cast<const char *>(encoding));
			return -1;
		}
...
static int FASTCALL xmlSaveClearEncoding(xmlSaveCtxt * ctxt)
{
	xmlOutputBuffer * buf = ctxt->buf;
	...
	static void xhtmlNodeDumpOutput(xmlSaveCtxt * ctxt, xmlNode * cur);
#endif
static void xmlNodeListDumpOutput(xmlSaveCtxt * ctxt, xmlNode * cur);
static void xmlNodeDumpOutputInternal(xmlSaveCtxt * ctxt, xmlNode * cur);
void xmlNsListDumpOutput(xmlOutputBuffer * buf, xmlNs * cur);
static int xmlDocContentDumpOutput(xmlSaveCtxt * ctxt, xmlDoc * cur);
/**
 ...
 */
static void FASTCALL xmlOutputBufferWriteWSNonSig(xmlSaveCtxt * ctxt, int extra)
{
	if(ctxt && ctxt->buf) {
 ...
 */
static void xmlNsDumpOutput(xmlOutputBuffer * buf, xmlNs * cur, xmlSaveCtxt * ctxt)
{
	if(cur && buf && (cur->type == XML_LOCAL_NAMESPACE) && cur->href) {
		if(sstreq(cur->prefix, reinterpret_cast<const xmlChar *>("xml")) == 0) {
 ...
 */
static void xmlNsDumpOutputCtxt(xmlSaveCtxt * ctxt, xmlNs * cur)
{
 ...
 * Should be called in the context of attribute dumps.
 */
static void FASTCALL xmlNsListDumpOutputCtxt(xmlSaveCtxt * ctxt, xmlNs * cur)
{
 ...
 */
static void xmlDtdDumpOutput(xmlSaveCtxt * ctxt, xmlDtd * dtd)
{
	int format, level;
	xmlDoc * doc;
	if(dtd && ctxt && ctxt->buf) {
		 ...
		 */
		if(dtd->notations && (!dtd->doc || (dtd->doc->intSubset == dtd))) {
			xmlBufDumpNotationTable(buf->buffer, (xmlNotationTable *)dtd->notations);
		}
		format = ctxt->format;
		level = ctxt->level;
		doc = ctxt->doc;
 ...
 */
static void xmlAttrDumpOutput(xmlSaveCtxt * ctxt, xmlAttr * cur) 
{
	if(cur) {
		xmlOutputBuffer * buf = ctxt->buf;
		if(buf) {
 ...
 */
static void xmlAttrListDumpOutput(xmlSaveCtxt * ctxt, xmlAttr * cur) 
{
	for(; cur; cur = cur->next)
...
static void xmlNodeListDumpOutput(xmlSaveCtxt * ctxt, xmlNode * cur) 
{
	if(cur) {
		xmlOutputBuffer * buf = ctxt->buf;
		while(cur) {
			if((ctxt->format == 1) && (xmlIndentTreeOutput) && ((cur->type == XML_ELEMENT_NODE) || (cur->type == XML_COMMENT_NODE) || (cur->type == XML_PI_NODE)))
 ...
 */
static int htmlNodeDumpOutputInternal(xmlSaveCtxt * ctxt, xmlNode * cur)
{
	...
	if(ctxt->options & XML_SAVE_FORMAT)
		htmlNodeDumpFormatOutput(buf, doc, cur, reinterpret_cast<const char *>(encoding), 1);
	else
		htmlNodeDumpFormatOutput(buf, doc, cur, reinterpret_cast<const char *>(encoding), 0);
	/*
...
static void xmlNodeDumpOutputInternal(xmlSaveCtxt * ctxt, xmlNode * cur)
{
	...
	if(cur && !oneof2(cur->type, XML_XINCLUDE_START, XML_XINCLUDE_END)) {
		...
		if(oneof2(cur->type, XML_DOCUMENT_NODE, XML_HTML_DOCUMENT_NODE)) {
			xmlDocContentDumpOutput(ctxt, (xmlDoc *)cur);
		}
#ifdef LIBXML_HTML_ENABLED
			...
			if(cur->content) {
				if(cur->name != xmlStringTextNoenc) {
					...
					xmlOutputBufferWriteString(p_buf, reinterpret_cast<const char *>(cur->content));
				}
			}
		}
			...
			if(cur->content) {
				...
				xmlOutputBufferWriteString(p_buf, reinterpret_cast<const char *>(cur->name));
				if(cur->content) {
					if(ctxt->format == 2)
			...
			if(isempty(cur->content))
				...
				while(*end != '\0') {
					if((*end == ']') && (*(end + 1) == ']') && (*(end + 2) == '>')) {
						...
						xmlOutputBufferWrite(p_buf, end - start, reinterpret_cast<const char *>(start));
						xmlOutputBufferWrite(p_buf, 3, "]]>");
						start = end;
					}
					end++;
				...
				if(start != end) {
					xmlOutputBufferWrite(p_buf, 9, "<![CDATA[");
					xmlOutputBufferWriteString(p_buf, reinterpret_cast<const char *>(start));
					xmlOutputBufferWrite(p_buf, 3, "]]>");
				}
			}
		}
		else if(cur->type == XML_ATTRIBUTE_NODE)
			xmlAttrDumpOutput(ctxt, reinterpret_cast<xmlAttr *>(cur));
		else if(cur->type == XML_NAMESPACE_DECL)
			xmlNsDumpOutputCtxt(ctxt, reinterpret_cast<xmlNs *>(cur));
		else {
			int format = ctxt->format;
			if(format == 1) {
				for(xmlNode * p_tmp = cur->children; p_tmp; p_tmp = p_tmp->next) {
					if(oneof3(p_tmp->type, XML_TEXT_NODE, XML_CDATA_SECTION_NODE, XML_ENTITY_REF_NODE)) {
						ctxt->format = 0;
						break;
					}
				}
			...
			if(((cur->type == XML_ELEMENT_NODE) || !cur->content) && !cur->children && ((ctxt->options & XML_SAVE_NO_EMPTY) == 0)) {
				if(ctxt->format == 2)
					xmlOutputBufferWriteWSNonSig(ctxt, 0);
				xmlOutputBufferWrite(p_buf, 2, "/>");
				ctxt->format = format;
...
static int xmlDocContentDumpOutput(xmlSaveCtxt * ctxt, xmlDoc * cur)
{
	...
	if(ctxt->encoding) {
		...
		encoding = reinterpret_cast<const xmlChar *>(xmlGetCharEncodingName((xmlCharEncoding)cur->charset));
	}
	if(((cur->type == XML_HTML_DOCUMENT_NODE) && !(ctxt->options & XML_SAVE_AS_XML) && !(ctxt->options & XML_SAVE_XHTML)) || (ctxt->options & XML_SAVE_AS_HTML)) {
		...
		if(ctxt->options & XML_SAVE_FORMAT)
			htmlDocContentDumpFormatOutput(buf, cur, reinterpret_cast<const char *>(encoding), 1);
		else
			htmlDocContentDumpFormatOutput(buf, cur, reinterpret_cast<const char *>(encoding), 0);
		if(ctxt->encoding)
			cur->encoding = oldenc;
		return 0;
#else
		...
		if(!(ctxt->options & XML_SAVE_NO_DECL)) {
			...
			switch(cur->standalone) {
				case 0: xmlOutputBufferWrite(buf, 16, " standalone=\"no\""); break;
				case 1: xmlOutputBufferWrite(buf, 17, " standalone=\"yes\""); break;
			}
			xmlOutputBufferWrite(buf, 3, "?>\n");
		}
 ...
 */
static int xhtmlIsEmpty(const xmlNode * pNode)
{
	if(!pNode)
		return -1;
	...
	switch(pNode->name[0]) {
		case 'a':
		    if(sstreq(pNode->name, "area"))
			    return 1;
		    return 0;
		case 'b':
		    if(sstreq(pNode->name, "br"))
			    return 1;
		    if(sstreq(pNode->name, "base"))
			    return 1;
		    if(sstreq(pNode->name, "basefont"))
			    return 1;
		    return 0;
		case 'c':
		    if(sstreq(pNode->name, "col"))
			    return 1;
		    return 0;
		case 'f':
		    if(sstreq(pNode->name, "frame"))
			    return 1;
		    return 0;
		case 'h':
		    if(sstreq(pNode->name, "hr"))
			    return 1;
		    return 0;
		case 'i':
		    if(sstreq(pNode->name, "img"))
			    return 1;
		    if(sstreq(pNode->name, "input"))
			    return 1;
		    if(sstreq(pNode->name, "isindex"))
			    return 1;
		    return 0;
		case 'l':
		    if(sstreq(pNode->name, "link"))
			    return 1;
		    return 0;
		case 'm':
		    if(sstreq(pNode->name, "meta"))
			    return 1;
		    return 0;
		case 'p':
		    if(sstreq(pNode->name, "param"))
			    return 1;
		    return 0;
	}
	return 0;
 ...
 * Dump a list of XML attributes
 */
static void xhtmlAttrListDumpOutput(xmlSaveCtxt * ctxt, xmlAttr * cur) 
{
	...
	while(cur) {
		if((cur->ns == NULL) && (sstreq(cur->name, "id")))
			...
			xml_lang = cur;
		else if(!cur->ns && (!cur->children || !cur->children->content || (cur->children->content[0] == 0)) && htmlIsBooleanAttr(cur->name)) {
			if(cur->children)
	 ...
	 */
	if(name && !id) {
		if(parent && parent->name && (sstreq(parent->name, "a") || sstreq(parent->name, "p") || sstreq(parent->name, "div") || sstreq(parent->name, "img") ||
			sstreq(parent->name, "map") || sstreq(parent->name, "applet") || sstreq(parent->name, "form") || sstreq(parent->name, "frame") || sstreq(parent->name, "iframe"))) {
 ...
 */
static void xhtmlNodeListDumpOutput(xmlSaveCtxt * ctxt, xmlNode * cur) 
{
	if(cur) {
		xmlOutputBuffer * buf = ctxt->buf;
		while(cur) {
			if((ctxt->format == 1) && (xmlIndentTreeOutput) && (cur->type == XML_ELEMENT_NODE))
				xmlOutputBufferWrite(buf, ctxt->indent_size * (ctxt->level > ctxt->indent_nr ? ctxt->indent_nr : ctxt->level), ctxt->indent);
			xhtmlNodeDumpOutput(ctxt, cur);
			if(ctxt->format == 1) {
				xmlOutputBufferWrite(buf, 1, "\n");
		...
		if(isempty(cur->content)) {
			...
			while(*end != '\0') {
				if(*end == ']' && *(end + 1) == ']' && *(end + 2) == '>') {
					...
					xmlOutputBufferWrite(buf, end - start, reinterpret_cast<const char *>(start));
					xmlOutputBufferWrite(buf, 3, "]]>");
					start = end;
				}
				end++;
			...
			if(start != end) {
				xmlOutputBufferWrite(buf, 9, "<![CDATA[");
				xmlOutputBufferWriteString(buf, reinterpret_cast<const char *>(start));
				xmlOutputBufferWrite(buf, 3, "]]>");
			}
		}
		return;
	...
	if((cur->type == XML_ELEMENT_NODE) && cur->P_ParentNode && (cur->P_ParentNode->P_ParentNode == (xmlNode *)cur->doc) &&
		...
		while(tmp) {
			if(sstreq(tmp->name, "meta")) {
				xmlChar * httpequiv = xmlGetProp(tmp, reinterpret_cast<const xmlChar *>("http-equiv"));
				if(httpequiv) {
					if(xmlStrcasecmp(httpequiv, reinterpret_cast<const xmlChar *>("Content-Type")) == 0) {
						SAlloc::F(httpequiv);
						break;
					}
					SAlloc::F(httpequiv);
...
xmlSaveCtxt * xmlSaveToBuffer(xmlBuffer * buffer, const char * encoding, int options)
{
	xmlSaveCtxt * ret = xmlNewSaveCtxt(encoding, options);
	if(ret) {
		xmlCharEncodingHandler * handler = 0;
		if(encoding) {
...
long xmlSaveTree(xmlSaveCtxt * ctxt, xmlNode * pNode)
{
	if(!ctxt || !pNode) 
		...
		xmlNodeDumpOutputInternal(ctxt, pNode);
		return 0;
	}
}
/**
	...
	if(string) {
		...
		while(*cur) {
			if(*cur == '\n') {
				...
				xmlBufAdd(buf, reinterpret_cast<const xmlChar *>("&#10;"), 5);
				cur++;
				base = cur;
			}
			else if(*cur == '\r') {
				...
				if(base != cur)
					xmlBufAdd(buf, base, cur - base);
				xmlBufAdd(buf, reinterpret_cast<const xmlChar *>("&#9;"), 4);
				cur++;
				base = cur;
			}
			else if(*cur == '"') {
				...
				if(base != cur)
					xmlBufAdd(buf, base, cur - base);
				xmlBufAdd(buf, reinterpret_cast<const xmlChar *>("&amp;"), 5);
				cur++;
				base = cur;
			}
			else if((*cur >= 0x80) && (!doc || !doc->encoding)) {
				...
				if(base != cur)
					xmlBufAdd(buf, base, cur - base);
				if(*cur < 0xC0) {
					xmlSaveErr(XML_SAVE_NOT_UTF8, reinterpret_cast<xmlNode *>(attr), 0);
					if(doc)
				...
				if((l == 1) || (!IS_CHAR(val))) {
					xmlSaveErr(XML_SAVE_CHAR_INVALID, reinterpret_cast<xmlNode *>(attr), 0);
					if(doc)
						doc->encoding = sstrdup(reinterpret_cast<const xmlChar *>("ISO-8859-1"));
					xmlSerializeHexCharRef(tmp, *cur);
					xmlBufAdd(buf, tmp, -1);
...
size_t xmlBufNodeDump(xmlBuf * buf, xmlDoc * doc, xmlNode * cur, int level, int format)
{
	...
	if(!cur) {
		...
		outbuf = static_cast<xmlOutputBuffer *>(SAlloc::M(sizeof(xmlOutputBuffer)));
		if(outbuf == NULL) {
...
#endif
		xmlOutputBuffer * outbuf = xmlOutputBufferCreateFile(f, 0);
		if(outbuf) {
...
int xmlSaveFormatFileEnc(const char * filename, xmlDoc * cur, const char * encoding, int format)
{
	...
	xmlCharEncodingHandler * handler = NULL;
	if(cur) {
		SETIFZ(encoding, PTRCHRC_(cur->encoding));
		if(encoding) {
			handler = xmlFindCharEncodingHandler(encoding);
			if(handler == NULL)
				return -1;
 ...
 * returns: the number of bytes written or -1 in case of failure.
 */
int xmlSaveFileEnc(const char * filename, xmlDoc * cur, const char * encoding) { return xmlSaveFormatFileEnc(filename, cur, encoding, 0); }
/**
 * xmlSaveFormatFile:
 * @filename:  the filename (or URL)
 * @cur:  the document
 ...
 * returns: the number of bytes written or -1 in case of failure.
 */
int xmlSaveFormatFile(const char * filename, xmlDoc * cur, int format) { return xmlSaveFormatFileEnc(filename, cur, NULL, format); }
/**
 * xmlSaveFile:
 * @filename:  the filename (or URL)
 * @cur:  the document
