		...
		switch(uMsg) {
			...
			case WM_ERASEBKGND:
				if(p_view->HasState(SmartListBox::stOwnerDraw)) {
					...
					di.P_View = p_view;
					TView::message(p_view->owner, evCommand, cmDrawItem, &di);
					if(di.ItemAction == 0)
						return 1;
				}
				break;
	...
	switch(LOWORD(wParam)) {
		...
		case VK_INSERT:
			if(IsInState(sfSelected) || State & stTreeList) {
				TView::message(owner, evCommand, cmaInsert, this);
				sf = 1;
			}
			break;
		case VK_DELETE:
			if(IsInState(sfSelected) || State & stTreeList) {
				TView::message(owner, evCommand, cmaDelete, this);
				sf = 1;
			}
			break;
		case VK_ADD:
			TView::message(owner, evCommand, cmaLevelDown, this);
			sf = 1;
			break;
		case VK_SUBTRACT:
			TView::message(owner, evCommand, cmaLevelUp, this);
			sf = 1;
			break;
		default:
			return 0;
...
int SmartListBox::handleWindowsMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg) {
		case WM_INITDIALOG:
			onInitDialog(1);
			break;
		case WM_DESTROY: // данная команда практически никогда не вызывается. См. WM_DESTROY в DialogProc. Из-за этого, если список картинок валиден, то на каждом вызове диалога с таким списком теряется 4 объекта GDI.
			/*{ // @v6.0.14 AHTOXA
		...
		case WM_COMMAND: {
			switch(HIWORD(wParam)) {
				case LBN_DBLCLK:
					TView::message(owner, evCommand, cmLBDblClk, this);
					break;
				case LBN_SELCHANGE:
					{
						...
						if(index >= 0) {
							...
							if(def->Options & lbtFocNotify)
								TView::message(owner, evCommand, cmLBItemFocused, this);
							Scroll(-1, 0);
		...
		case WM_RBUTTONDOWN:
			{
				...
				TView::message(owner, evCommand, cmRightClick, this);
			}
			break;
		case WM_VKEYTOITEM:
			{
					...
					case NM_DBLCLK:
						{
							...
							SelectTreeItem();
							TView::message(owner, evCommand, (p_nm->code == NM_RCLICK) ? cmRightClick : cmLBDblClk, this);
						}
						break;
					case NM_CLICK:
						State |= stLButtonDown;
					...
					case NM_CUSTOMDRAW:
						{
							...
							switch(p_nmlvcd->nmcd.dwDrawStage) {
								...
								case CDDS_ITEMPREPAINT:
									if(TView::message(owner, evCommand, cmLBItemDraw, this))
		...
		case WM_COMMAND:
			if(HIWORD(wParam) == BN_CLICKED) {
				...
				if(p_slb) {
					...
					p_slb->IsBnClicked = 1;
					if(p_slb->P_WordSel)
						TView::message(p_slb->P_WordSel, evCommand, cmCancel);
				}
...
//
//
ListWindowSmartListBox::ListWindowSmartListBox(const TRect & r, ListBoxDef * d, int) : SmartListBox(r, d)
	{ combo = 0; }
//
//
//
		...
		case WM_VKEYTOITEM:
			{
				...
				if(def->_curItem() == 0 && oneof3(code, SB_LINEUP, SB_PAGEUP, SB_TOP)) {
					...
					TView::message(p_selector, evCommand, cmUp);
				}
			}
			break;
	}
