...
{
	// note: we don't require `size > offset`, we just guarantee that the address at offset is aligned regardless of
	// the allocated size.
	...
	if(mi_unlikely(alignment > MI_ALIGNMENT_MAX)) { // we cannot align at a boundary larger than this (or otherwise
		                                        // we cannot find segment headers)
    #if MI_DEBUG > 0
		_mi_error_message(EOVERFLOW,
		    "aligned allocation has a maximum alignment of %zu (size %zu, alignment %zu)\n",
		    MI_ALIGNMENT_MAX,
		    size,
		    alignment);
    #endif
	...
	if(mi_likely(padsize <= MI_SMALL_SIZE_MAX)) {
		...
		if(mi_likely(page->free && is_aligned)) {
      #if MI_STAT>1
