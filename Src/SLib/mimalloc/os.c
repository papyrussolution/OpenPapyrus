...
size_t _mi_os_good_alloc_size(size_t size) 
{
	...
	if(UNLIKELY(size >= (SIZE_MAX - align_size))) return size; // possible overflow?
...
#define KK_HINT_AREA ((uintptr_t)4 << 40)  // upto 6TiB   (since before win8 there is "only" 8TiB available to
	                                   // processes)
