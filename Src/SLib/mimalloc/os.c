...
#define MI_HINT_AREA ((uintptr_t)4 << 40)  // upto 6TiB   (since before win8 there is "only" 8TiB available to
	                                   // processes)
...
static void* mi_os_get_aligned_hint(size_t try_alignment, size_t size)
{
	...
	if(size > 1*MI_GiB) return NULL; // guarantee the chance of fixed valid address is at most 1/(MI_HINT_AREA /
	                                 // 1<<30) = 1/4096.
  #if (MI_SECURE>0)
	size += MI_SEGMENT_SIZE;  // put in `MI_SEGMENT_SIZE` virtual gaps between hinted blocks; this splits VLA's but
	                          // increases guarded areas.
  #endif
	...
	if(hint == 0 || hint > MI_HINT_MAX) { // wrap or initialize
		...
		init = init + ((MI_SEGMENT_SIZE * ((r>>17) & 0xFFFFF)) % MI_HINT_AREA); // (randomly 20 bits)*4MiB == 0
		                                                                        // to 4TiB
    #endif
		uintptr_t expected = hint + size;
		mi_atomic_cas_strong_acq_rel(&aligned_base, &expected, init);
		hint = mi_atomic_add_acq_rel(&aligned_base, size); // this may still give 0 or > MI_HINT_MAX but that is
...
static void* mi_win_virtual_alloc(void* addr,
    size_t size,
	...
	if((large_only || use_large_os_page(size, try_alignment))
		...
		if(!large_only && try_ok > 0) {
			...
			// therefore, once a large page allocation failed, we don't try again for `large_page_try_ok`
			// times.
			...
			*is_large = true;
			...
			if(p == NULL) {
				mi_atomic_store_release(&large_page_try_ok, 10UL); // on error, don't try again for the
				                                                   // next N allocations
			}
		}
	}
	...
	if(p == NULL) {
		_mi_warning_message(
			"unable to allocate OS memory (%zu bytes, error code: 0x%x, address: %p, alignment: %zu, flags: 0x%x, large only: %d, allow large: %d)\n",
			size,
			GetLastError(),
			addr,
			try_alignment,
...
static void* mi_heap_grow(size_t size, size_t try_alignment) {
	void* p = NULL;
	if(try_alignment <= 1) {
		// `sbrk` is not thread safe in general so try to protect it (we could skip this on WASM but leave it in
		// for now)
    #if defined(MI_USE_PTHREADS)
...
static void* mi_unix_mmapx(void* addr, size_t size, size_t try_alignment, int protect_flags, int flags, int fd) {
	...
	if(addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
		...
		if(((size_t)1 << n) == try_alignment && n >= 12 && n <= 30) { // alignment is a power of 2 and 4096 <=
			                                                      // alignment <= 1GiB
			flags |= MAP_ALIGNED(n);
	...
	if(addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
		void* p = mmap((void*)try_alignment, size, protect_flags, flags | MAP_ALIGN, fd, 0); // addr parameter
		                                                                                     // is the required
		                                                                                     // alignment
		if(p!=MAP_FAILED) return p;
		// fall back to regular mmap
...
static int mi_unix_mmap_fd(void) {
	...
	// macOS: tracking anonymous page with a specific ID. (All up to 98 are taken officially but LLVM sanitizers had
	// taken 99)
...
static void* mi_unix_mmap(void* addr,
    size_t size,
    size_t try_alignment,
    int protect_flags,
    bool large_only,
  ...
  #define MAP_NORESERVE  0
			...
			if(large_only || lflags != flags) {
				...
				*is_large = true;
				...
				if(p == NULL && (lflags & MAP_HUGE_1GB) != 0) {
					...
					_mi_warning_message(
						"unable to allocate huge (1GiB) page, trying large (2MiB) pages instead (error %i)\n",
						errno);
				...
				if(large_only) return p;
				if(p == NULL) {
					mi_atomic_store_release(&large_page_try_ok, (size_t)8); // on error, don't try
	...
	if(p == NULL) {
		_mi_warning_message(
			"unable to allocate OS memory (%zu bytes, error code: %i, address: %p, large only: %d, allow large: %d)\n",
			size,
			errno,
			addr,
			large_only,
...
static bool mi_os_commitx(void* addr, size_t size, bool commit, bool conservative, bool* is_zero, mi_stats_t* stats) {
  ...
  #if defined(_WIN32)
	if(commit) {
		// *is_zero = true;  // note: if the memory was already committed, the call succeeds but the memory is
		// not zero'd
...
static void* mi_os_alloc_huge_os_pagesx(void* addr, size_t size, int numa_node)
{
	...
	if(pNtAllocateVirtualMemoryEx != NULL && mi_huge_pages_available) {
		...
		if(err == 0 && base != NULL) {
			...
			_mi_warning_message(
				"unable to allocate using huge (1GiB) pages, trying large (2MiB) pages instead (status 0x%lx)\n",
				err);
		}
	}
...
static long mi_os_mbind(void* start,
    unsigned long len,
    unsigned long mode,
...
static size_t mi_os_numa_node_countx(void) {
	...
	for(node = 0; node < 256; node++) {
		// enumerate node entries -- todo: it there a more efficient way to do this? (but ensure there is no
		// allocation)
