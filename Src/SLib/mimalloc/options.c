...
#else
	{ 1, UNINIT, MI_OPTION(eager_commit_delay) }, // the first N segments per thread are not eagerly committed (but
	                                              // per page in the segment on demand)
#endif
	{ 100, UNINIT, MI_OPTION(reset_delay) }, // reset delay in milli-seconds
	{ 0,   UNINIT, MI_OPTION(use_numa_nodes) },// 0 = use available numa nodes, otherwise use at most N nodes.
	{ 0,   UNINIT, MI_OPTION(limit_os_alloc) },// 1 = do not use OS memory for allocation (but only reserved arenas)
	{ 100, UNINIT, MI_OPTION(os_tag) },      // only apple specific for now but might serve more or less related
	                                         // purpose
...
static void mi_out_buf_flush(mi_output_fun* out, bool no_more_buf, void * arg) 
{
	if(out) {
		...
		SETMIN(count, MI_MAX_DELAY_OUTPUT);
		out_buf[count] = 0;
		out(out_buf, arg);
		if(!no_more_buf) {
