...
int mi_version() mi_attr_noexcept { return MI_MALLOC_VERSION; }
...
mi_decl_nodiscard bool mi_option_is_enabled(mi_option_t option) { return (mi_option_get(option) != 0); }
void mi_option_set_enabled(mi_option_t option, bool enable) { mi_option_set(option, (enable ? 1 : 0)); }
void mi_option_set_enabled_default(mi_option_t option, bool enable) { mi_option_set_default(option, (enable ? 1 : 0)); }
...
static mi_decl_noinline bool mi_recurse_enter_prim() 
{
...
static mi_decl_noinline void mi_recurse_exit_prim() 
{
	recurse = false;
}

static bool mi_recurse_enter() 
{
...
{
	if(prefix && strlen(prefix) <= 32 && !_mi_is_main_thread()) {
		char tprefix[64];
...
static char** mi_get_environ() { return (*_NSGetEnviron()); }
#else
extern char** environ;
static char** mi_get_environ() { return environ; }
#endif
static bool mi_getenv(const char* name, char* result, size_t result_size) 
{
