...
// "page.c"
void *  _mi_malloc_generic(mi_heap_t* heap, size_t size)  NOEXCEPT mi_attr_malloc;
...
// "alloc.c"
void *   _mi_page_malloc(mi_heap_t* heap, mi_page_t* page, size_t size) NOEXCEPT;  // called from `_mi_malloc_generic`
...
// Branches
//
/* (see slport.h; replaced with LIKELY/UNLIKELY)
#if defined(__GNUC__) || defined(__clang__)
	#define mi_unlikely_Removed(x)     __builtin_expect((x), 0)
	#define mi_likely_Removed(x)       __builtin_expect((x), 1)
#else
	#define mi_unlikely_Removed(x)     (x)
...
static inline bool mi_count_size_overflow(size_t count, size_t size, size_t* total) {
	...
	else if(UNLIKELY(mi_mul_overflow(count, size, total))) {
...
static inline void * mi_tls_slot(size_t slot) NOEXCEPT;   // forward declaration
...
// This thread local variable is only used when neither MI_TLS_SLOT, MI_TLS_PTHREAD, or MI_TLS_PTHREAD_SLOT_OFS are
// defined.
...
static inline mi_heap_t* mi_get_default_heap(void) 
{
	...
	return (UNLIKELY(heap == NULL) ? (mi_heap_t*)&_mi_heap_empty : heap);
#elif defined(MI_TLS_PTHREAD_SLOT_OFS)
	mi_heap_t* heap = *mi_tls_pthread_heap_slot();
	return (UNLIKELY(heap == NULL) ? (mi_heap_t*)&_mi_heap_empty : heap);
#elif defined(MI_TLS_PTHREAD)
	mi_heap_t* heap = (UNLIKELY(_mi_heap_default_key == (pthread_key_t)(-1)) ? _mi_heap_main_get() : (mi_heap_t*)pthread_getspecific(_mi_heap_default_key));
	return (UNLIKELY(heap == NULL) ? (mi_heap_t*)&_mi_heap_empty : heap);
#else
  #if defined(MI_TLS_RECURSE_GUARD)
	if(UNLIKELY(!_mi_process_is_initialized)) return _mi_heap_main_get();
  #endif
	return _mi_heap_default;
#endif
}

static inline bool mi_heap_is_default(const mi_heap_t* heap) { return (heap == mi_get_default_heap()); }
static inline bool mi_heap_is_backing(const mi_heap_t* heap) { return (heap->tld->heap_backing == heap); }
...
static inline size_t mi_page_block_size(const mi_page_t* page) 
{
	...
	if(LIKELY(bsize < MI_HUGE_BLOCK_SIZE)) {
		return bsize;
	}
	else {
		size_t psize;
...
static inline mi_block_t* mi_block_next(const mi_page_t* page, const mi_block_t* block) 
{
	...
	if(UNLIKELY(next!=NULL && !mi_is_in_same_page(block, next))) {
		_mi_error_message(EFAULT, "corrupted free list entry of size %zub at %p: value 0x%zx\n", mi_page_block_size(page), block,
		    (uintptr_t)next);
		next = NULL;
	}
	return next;
...
static inline size_t mi_ctz32(uint32_t x) 
{
	...
	static const uchar debruijn[32] = {
...
{
	if(_mi_cpu_has_fsrm) {
		__movsb((uchar *)dst, (const uchar *)src, n);
	}
	else {
