...
// arena.c
void*      _mi_arena_alloc_aligned(size_t size,
    size_t alignment,
    bool* commit,
    bool* large,
    bool* is_pinned,
...
void       _mi_abandoned_await_readers();
...
static inline mi_heap_t** mi_tls_pthread_heap_slot() 
{
...
#elif defined(MI_TLS_PTHREAD)
	mi_heap_t* heap =
	    (mi_unlikely(_mi_heap_default_key ==
	    (pthread_key_t)(-1)) ? _mi_heap_main_get() : (mi_heap_t*)pthread_getspecific(_mi_heap_default_key));
	return (mi_unlikely(heap == NULL) ? (mi_heap_t*)&_mi_heap_empty : heap);
#else
...
static inline mi_segment_t* _mi_page_segment(const mi_page_t* page) {
	...
	mi_assert_internal(segment == NULL ||
	    ((mi_slice_t*)page >= segment->slices && (mi_slice_t*)page < segment->slices + segment->slice_entries));
	return segment;
}
...
#include <windows.h>
static inline mi_threadid_t _mi_thread_id() mi_attr_noexcept 
{
