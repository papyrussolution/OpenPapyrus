...
// #define MI_DEBUG 1  // basic assertion checks and statistics, check double free, corrupted free list, and invalid
// pointer free.
...
typedef enum mi_delayed_e {
	...
	MI_NO_DELAYED_FREE    = 2,// optimize: push on page local thread free queue if another block is already in the
	                        // heap thread delayed free list
...
typedef struct mi_segment_s {
	...
	size_t page_shift; // `1 << page_shift` == the page sizes == `page->block_size * page->reserved`
...
// in the fast path.
//
typedef struct mi_tld_s mi_tld_t; // Thread local data
//
...
struct mi_heap_s {
	mi_tld_t  * tld;
	mi_page_t * pages_free_direct[MI_PAGES_DIRECT];// optimize: array where every entry points a page with
		// possibly free blocks in the corresponding queue for that size.
