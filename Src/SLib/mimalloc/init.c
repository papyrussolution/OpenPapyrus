...
static mi_thread_data_t* mi_thread_data_alloc() 
{
...
static void mi_thread_data_collect() 
{
...
static bool _mi_heap_init() 
{
	...
	if(_mi_is_main_thread()) {
		// mi_assert_internal(_mi_heap_main.thread_id != 0);  // can happen on freeBSD where alloc is called
...
//
static void mi_process_done();
static bool os_preloading = true;    // true until this module is initialized
static bool mi_redirected = false;   // true if malloc redirects to mi_malloc

// Returns true if this module has not been initialized; Don't use C runtime routines until it returns false.
bool _mi_preloading() { return os_preloading; }
mi_decl_nodiscard bool mi_is_redirected() mi_attr_noexcept { return mi_redirected; }
...
static void mi_detect_cpu_features() 
{
	// FSRM for fast rep movsb support (AMD Zen3+ (~2020) or Intel Ice Lake+ (~2017))
	int32_t cpu_info[4];
	__cpuid(cpu_info, 7);
	_mi_cpu_has_fsrm = ((cpu_info[3] & (1 << 4)) != 0); // bit 4 of EDX : see <https
	                                                    // ://en.wikipedia.org/wiki/CPUID#EAX=7,_ECX=0:_Extended_Features>
}
...
static void mi_process_done() 
{
  ...
  #if defined(_WIN32) && !defined(MI_SHARED_LIB)
	FlsFree(mi_fls_key); // call thread-done on all threads (except the main thread) to prevent dangling callback
	                     // pointer if statically linked with a DLL; Issue #208
  #endif
...
static int _mi_process_init() 
{
	mi_process_load();
	return 0;
}

typedef int (* _mi_crt_callback_t)();
