...
static void mi_segment_protect(mi_segment_t* segment, bool protect, mi_os_tld_t* tld) {
	...
	if(MI_SECURE != 0) {
		...
		mi_assert_internal((segment->segment_info_size - os_psize) >=
		    (sizeof(mi_segment_t) + ((segment->capacity - 1) * sizeof(mi_page_t))));
		...
		if(MI_SECURE <= 1 || segment->capacity == 1) {
			...
			if(protect && !segment->mem_is_committed) {
				if(protect) {
					...
					if(_mi_mem_commit(start, os_psize, NULL, tld)) { // if this fails that is ok (as
						                                         // it is an unaccessible page)
...
uint8_t* _mi_segment_page_start(const mi_segment_t* segment, const mi_page_t* page, size_t block_size, size_t* page_size, size_t* pre_size)
{
	...
	if(page->segment_idx == 0 && block_size > 0 && segment->page_kind <= MI_PAGE_MEDIUM) {
		// for small and medium objects, ensure the page start is aligned with the block size (PR#66 by
		// kickunderscore)
    ...
    mi_os_tld_t* os_tld)
{
	...
	if(segment != NULL) {
			...
			for(size_t i = 0; i < segment->capacity; i++) {
				...
				if(page->is_reset) {
					if(!commit && mi_option_is_enabled(mi_option_reset_decommits)) {
						...
						mi_page_unreset(segment, page, 0, tld); // todo: only unreset the part
						                                        // that was reset? (instead of
						                                        // the full page)
					}
				}
			}
		...
		size_t memid;
		bool mem_large = (!eager_delayed && (MI_SECURE==0)); // only allow large OS pages once we are no longer
		                                                     // lazy
		bool is_pinned = false;
		segment = (mi_segment_t*)_mi_mem_alloc_aligned(segment_size,
			MI_SEGMENT_SIZE,
			&commit,
			&mem_large,
			&is_pinned,
		...
		if(!commit) {
			...
			if(commit_zero) is_zero = true;
			if(!ok) {
				// commit failed; we cannot touch the memory: free the segment directly and return
				// `NULL`
	...
	if(!pages_still_good) {
		...
		memzero((uint8_t*)segment + ofs, info_size - ofs);
		...
		ptrdiff_t ofs = offsetof(mi_segment_t, next);
		memzero((uint8_t*)segment + ofs, offsetof(mi_segment_t, pages) - ofs);
	}
	// initialize
...
static mi_segment_t* mi_segment_alloc(size_t required,
    mi_page_kind_t page_kind,
    size_t page_shift,
    mi_segments_tld_t* tld,
    mi_os_tld_t* os_tld) {
...
void _mi_segment_page_free(mi_page_t* page, bool force, mi_segments_tld_t* tld)
{
	...
	if(segment->used == 0) {
		...
		if(segment->used == segment->abandoned) {
			...
			mi_assert_internal(segment->page_kind <= MI_PAGE_MEDIUM); // for now we only support small and
			                                                          // medium pages
...
static mi_segment_t* mi_abandoned_pop(void) {
	...
	if(LIKELY(segment == NULL)) {
		if(LIKELY(!mi_abandoned_visited_revisit())) { // try to swap in the visited list on NULL
			return NULL;
		}
	}
    ...
    mi_segments_tld_t* tld)
{
	*reclaimed = false;
	...
	while((max_tries-- > 0) && ((segment = mi_abandoned_pop()) != NULL)) {
		...
		bool all_pages_free;
		bool has_page = mi_segment_check_free(segment, block_size, &all_pages_free); // try to free up pages
		                                                                             // (due to concurrent
		                                                                             // frees)
		if(all_pages_free) {
...
static mi_page_t* mi_segment_page_alloc(mi_heap_t* heap,
    size_t block_size,
    mi_page_kind_t kind,
    size_t page_shift,
    mi_segments_tld_t* tld,
...
static mi_page_t* mi_segment_huge_page_alloc(size_t size, mi_segments_tld_t* tld, mi_os_tld_t* os_tld)
{
	...
	if(segment == NULL) return NULL;
	mi_assert_internal(mi_segment_page_size(
		    segment) - segment->segment_info_size - (2*(MI_SECURE == 0 ? 0 : _mi_os_page_size())) >= size);
...
mi_page_t* _mi_segment_page_alloc(mi_heap_t* heap, size_t block_size, mi_segments_tld_t* tld, mi_os_tld_t* os_tld) {
	...
	mi_assert_internal(page == NULL || (mi_segment_page_size(_mi_page_segment(
		    page)) - (MI_SECURE == 0 ? 0 : _mi_os_page_size())) >= block_size);
	mi_reset_delayed(tld);
