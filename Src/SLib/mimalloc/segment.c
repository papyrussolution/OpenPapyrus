...
static bool mi_segment_is_valid(mi_segment_t* segment, mi_segments_tld_t* tld) {
	...
	mi_assert_internal(mi_commit_mask_all_set(&segment->commit_mask, &segment->decommit_mask)); // can only decommit
	                                                                                            // committed blocks
	...
	while(slice < end) {
		...
		size_t maxindex =
		    (index + slice->slice_count >= segment->slice_entries ? segment->slice_entries : index + slice->slice_count) - 1;
		if(mi_slice_is_used(slice)) { // a page in use, we need at least MAX_SLICE_OFFSET valid back offsets
			...
			if(segment->kind != MI_SEGMENT_HUGE && segment->thread_id != 0) { // segment is not huge or
				                                                          // abandoned
...
static void mi_segment_os_free(mi_segment_t* segment, mi_segments_tld_t* tld) {
	...
	if(size != MI_SEGMENT_SIZE ||
	    !_mi_segment_cache_push(segment, size, segment->memid, &segment->commit_mask, &segment->decommit_mask, segment->mem_is_large,
	    segment->mem_is_pinned, tld->os)) {
		const size_t csize = _mi_commit_mask_committed_size(&segment->commit_mask, size);
		if(csize > 0 && !segment->mem_is_pinned) _mi_stat_decrease(&_mi_stats_main.committed, csize);
		_mi_abandoned_await_readers(); // wait until safe to free
		_mi_arena_free(segment,
		    mi_segment_size(segment),
		    segment->memid,
		    segment->mem_is_pinned /* pretend not committed to not double count decommits */,
		    tld->os);
	}
}
...
static void mi_segment_commit_mask(mi_segment_t* segment,
    bool conservative,
    uint8_t* p,
    size_t size,
    uint8_t** start_p,
	...
	if(pstart >= segstart && start < segstart) { // note: the mask is also calculated for an initial commit of the
		                                     // info area
		start = segstart;
	}
	if(end > segsize) {
...
static bool mi_segment_commitx(mi_segment_t* segment, bool commit, uint8_t* p, size_t size, mi_stats_t* stats) {
	...
	if(commit && !mi_commit_mask_all_set(&segment->commit_mask, &mask)) {
		...
		_mi_stat_decrease(&_mi_stats_main.committed, _mi_commit_mask_committed_size(&cmask, MI_SEGMENT_SIZE)); // adjust
		                                                                                                       // for
		                                                                                                       // overlap
		...
		mi_commit_mask_t cmask;
		mi_commit_mask_create_intersect(&segment->commit_mask, &mask, &cmask);
		_mi_stat_increase(&_mi_stats_main.committed, full_size - _mi_commit_mask_committed_size(&cmask, MI_SEGMENT_SIZE)); // adjust
		                                                                                                                   // for
		                                                                                                                   // overlap
...
static bool mi_segment_ensure_committed(mi_segment_t* segment, uint8_t* p, size_t size, mi_stats_t* stats) {
	...
	if(mi_commit_mask_is_full(&segment->commit_mask) && mi_commit_mask_is_empty(&segment->decommit_mask)) return true; // fully
	                                                                                                                   // committed
...
static void mi_segment_perhaps_decommit(mi_segment_t* segment, uint8_t* p, size_t size, mi_stats_t* stats) {
	...
	if(mi_option_get(mi_option_decommit_delay) == 0) {
		...
		mi_commit_mask_t cmask;
		mi_commit_mask_create_intersect(&segment->commit_mask, &mask, &cmask); // only decommit what is
		                                                                       // committed; span_free may try
		                                                                       // to decommit more
		...
		if(segment->decommit_expire == 0) {
			...
			segment->decommit_expire = now + mi_option_get(mi_option_decommit_extend_delay); // (mi_option_get(mi_option_decommit_delay)
			                                                                                 // / 8); //
			                                                                                 // wait a tiny
			                                                                                 // bit longer
			                                                                                 // in case
...
static mi_page_t* mi_segment_span_allocate(mi_segment_t* segment, size_t slice_index, size_t slice_count, mi_segments_tld_t* tld) {
	...
	if(!mi_segment_ensure_committed(segment, _mi_segment_page_start_from_slice(segment, slice, 0, NULL),
	...
	if(slice_index + extra >= segment->slice_entries) extra = segment->slice_entries - slice_index - 1; // huge
	                                                                                                    // objects
	                                                                                                    // may have
	                                                                                                    // more
	                                                                                                    // slices
	...
	// note: the cast is needed for ubsan since the index can be larger than MI_SLICES_PER_SEGMENT for huge
	// allocations (see #543)
...
static mi_segment_t* mi_segment_init(mi_segment_t* segment,
    size_t required,
    ...
    mi_page_t** huge_page)
{
	...
	// Commit eagerly only if not the first N lazy segments (to reduce impact of many threads that allocate just a
	// little)
	...
	if(segment==NULL) {
		...
		bool mem_large = (!eager_delay && (MI_SECURE==0)); // only allow large OS pages once we are no longer
		                                                   // lazy
		bool is_pinned = false;
		size_t memid = 0;
		segment = (mi_segment_t*)_mi_segment_cache_pop(segment_size,
			&commit_mask,
			&decommit_mask,
			&mem_large,
			&is_pinned,
			...
			os_tld);
		if(segment==NULL) {
			segment = (mi_segment_t*)_mi_arena_alloc_aligned(segment_size,
				MI_SEGMENT_SIZE,
				&commit,
				&mem_large,
				&is_pinned,
	...
	if(!is_zero) {
		...
		memzero((uint8_t*)segment+ofs, prefix + sizeof(mi_slice_t)*segment_slices);
	}
	...
	// memzero(segment->slices, sizeof(mi_slice_t)*(info_slices+1));
...
static mi_slice_t* mi_segment_page_clear(mi_page_t* page, mi_segments_tld_t* tld) {
	...
	memzero((uint8_t*)page + ofs, sizeof(*page) - ofs);
	page->xblock_size = 1;
	// and free it
...
static mi_segment_t* mi_abandoned_pop(void) {
	...
	do {
		...
		if(segment != NULL) {
			...
			next = mi_tagged_segment(anext, ts); // note: reads the segment's `abandoned_next` field so
			                                     // should not be decommitted
		}
...
static mi_segment_t* mi_segment_reclaim(mi_segment_t* segment,
    mi_heap_t* heap,
	...
	while(slice < end) {
		...
		if(mi_slice_is_used(slice)) {
			...
			_mi_page_use_delayed_free(page, MI_USE_DELAYED_FREE, true); // override never (after heap is
			                                                            // set)
...
static mi_segment_t* mi_segment_try_reclaim(mi_heap_t* heap,
    ...
    mi_segments_tld_t* tld)
{
	*reclaimed = false;
	mi_segment_t* segment;
	long max_tries = mi_option_get_clamp(mi_option_max_segment_reclaim, 8, 1024); // limit the work to bound
	                                                                              // allocation times
	while((max_tries-- > 0) && ((segment = mi_abandoned_pop()) != NULL)) {
		segment->abandoned_visits++;
		bool has_page = mi_segment_check_free(segment, needed_slices, block_size, tld); // try to free up pages
		                                                                                // (due to concurrent
		                                                                                // frees)
		if(segment->used == 0) {
			...
			mi_segment_delayed_decommit(segment, true /* force? */, tld->stats); // forced decommit if
			                                                                     // needed as we may not
			                                                                     // visit soon again
...
static mi_segment_t* mi_segment_reclaim_or_alloc(mi_heap_t* heap,
    size_t needed_slices,
    size_t block_size,
    mi_segments_tld_t* tld,
    mi_os_tld_t* os_tld)
{
	...
	if(reclaimed) {
		...
		mi_assert_internal(segment != NULL);
		return NULL; // pretend out-of-memory as the page will be in the page queue of the heap with available
		             // blocks
	}
...
static mi_page_t* mi_segments_page_alloc(mi_heap_t* heap,
    ...
    mi_os_tld_t* os_tld)
{
	...
	mi_page_t* page = mi_segments_page_find_and_allocate(slices_needed, tld); //(required <= MI_SMALL_SIZE_MAX ? 0 :
	                                                                          // slices_needed), tld);
	if(page==NULL) {
...
void _mi_segment_huge_page_free(mi_segment_t* segment, mi_page_t* page, mi_block_t* block) {
	...
	// claim it and free
	mi_heap_t* heap = mi_heap_get_default(); // issue #221; don't use the internal get_default_heap as we need to
