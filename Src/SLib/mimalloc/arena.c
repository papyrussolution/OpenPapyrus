...
// can lead to fragmentation but fortunately most objects will be regions of 256MiB in practice.
// 
...
// 
#define MI_MEMID_OS   0 // Use `0` as a special id for direct OS allocated memory.
    ...
    size_t* memid, mi_os_tld_t* tld)
{
	...
	*is_pinned = false;
	...
	if(alignment <= MI_SEGMENT_ALIGN && size >= MI_ARENA_MIN_OBJ_SIZE && mi_atomic_load_relaxed(&mi_arena_count) > 0) {
