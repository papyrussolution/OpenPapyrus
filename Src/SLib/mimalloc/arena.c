...
typedef struct mi_arena_s {
	...
	mi_bitmap_field_t* blocks_committed; // are the blocks committed? (can be NULL for memory that cannot be
	                                     // decommitted)
...
{
	size_t idx = 0; // mi_atomic_load_relaxed(&arena->search_idx);  // start from last search; ok to be relaxed as
	                // the exact start does not matter
	if(_mi_bitmap_try_find_from_claim_across(arena->blocks_inuse, arena->field_count, idx, blocks, bitmap_idx)) {
		mi_atomic_store_relaxed(&arena->search_idx, mi_bitmap_index_field(*bitmap_idx)); // start search from
		                                                                                 // found location next
		                                                                                 // time around
		return true;
	}
...
static mi_decl_noinline void* mi_arena_allocate(int numa_node,
    size_t size,
    size_t alignment,
    bool* commit,
    bool* large,
...
void* _mi_arena_alloc_aligned(size_t size, size_t alignment, bool* commit, bool* large, bool* is_pinned, bool* is_zero, size_t* memid, mi_os_tld_t* tld)
{
	...
	*is_pinned = false;
	...
	// try to allocate in an arena if the alignment is small enough and the object is not too small (as for heap
	// meta data)
...
void _mi_arena_free(void* p, size_t size, size_t memid, bool all_committed, mi_os_tld_t* tld) 
{
	...
	if(memid == MI_MEMID_OS) {
		...
		if(!arena->allow_decommit || arena->blocks_committed == NULL) {
			mi_assert_internal(all_committed); // note: may be not true as we may "pretend" to be not
			                                   // committed (in segment.c)
		}
		else {
...
bool mi_manage_os_memory(void* start, size_t size, bool is_committed, bool is_large, bool is_zero, int numa_node) mi_attr_noexcept
{
	...
	mi_arena_t* arena   = (mi_arena_t*)_mi_os_alloc(asize, &_mi_stats_main);// TODO: can we avoid allocating from
	                                                                       // the OS?
	...
	arena->numa_node    = numa_node;// TODO: or get the current numa node if -1? (now it allows anyone to allocate
	                              // on -1)
	...
	arena->blocks_committed = (!arena->allow_decommit ? NULL : &arena->blocks_inuse[2*fields]); // just after dirty
	                                                                                            // bitmap
	// the bitmaps are already zero initialized due to os_alloc
	// initialize committed bitmap?
	if(arena->blocks_committed && is_committed) {
...
void mi_debug_show_arenas() mi_attr_noexcept 
{
