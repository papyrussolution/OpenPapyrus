...
/*
   ...
   bool _mi_bitmap_try_find_claim(mi_bitmap_t bitmap, const size_t bitmap_fields, const size_t count, mi_bitmap_index_t*
      bitmap_idx) {
    ...
    mi_bitmap_index_t* bitmap_idx)
{
	...
	if(initial >= count) return _mi_bitmap_try_find_claim_field(bitmap, idx, count, bitmap_idx); // no need to cross
	                                                                                             // fields
	if(_mi_divide_up(count - initial, MI_BITMAP_FIELD_BITS) >= (bitmap_fields - idx)) return false; // not enough
	                                                                                                // entries
...
static size_t mi_bitmap_mask_across(mi_bitmap_index_t bitmap_idx,
    size_t bitmap_fields,
    size_t count,
    uintptr_t* pre_mask,
    uintptr_t* mid_mask,
	...
	const size_t bitidx = mi_bitmap_index_bit_in_field(bitmap_idx);
	if(LIKELY(bitidx + count <= MI_BITMAP_FIELD_BITS)) {
...
static bool mi_bitmap_is_claimedx_across(mi_bitmap_t bitmap,
    size_t bitmap_fields,
    size_t count,
    mi_bitmap_index_t bitmap_idx,
    bool* pany_ones) {
