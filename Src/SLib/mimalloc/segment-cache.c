...
mi_decl_noinline void* _mi_segment_cache_pop(size_t size, mi_commit_mask_t* commit_mask, mi_commit_mask_t* decommit_mask, bool* large,
    bool* is_pinned, bool* is_zero, size_t* memid, mi_os_tld_t* tld)
{
...
void _mi_segment_cache_collect(bool force, mi_os_tld_t* tld) { mi_segment_cache_purge(force, tld); }
...
static bool  mi_is_valid_pointer(const void* p) { return (_mi_segment_of(p) != NULL); }
mi_decl_nodiscard mi_decl_export bool mi_is_in_heap_region(const void* p) mi_attr_noexcept { return mi_is_valid_pointer(p); }
/*
   ...
   mi_assert_internal(page == NULL || (mi_segment_page_size(_mi_page_segment(page)) - (MI_SECURE == 0 ? 0 :
      _mi_os_page_size())) >= block_size);
