...
static bool FASTCALL mi_is_in_main(const void * stat) 
{
	return ((uint8_t*)stat >= (uint8_t*)&_mi_stats_main && (uint8_t*)stat < ((uint8_t*)&_mi_stats_main + sizeof(mi_stats_t)));
}

static void FASTCALL mi_stat_update(mi_stat_count_t* stat, int64_t amount) 
{
	if(amount) {
...
static void STDCALL mi_stat_add(mi_stat_count_t* stat, const mi_stat_count_t* src, int64_t unit) 
{
	if(stat == src) 
		return;
...
static void STDCALL mi_stat_counter_add(mi_stat_counter_t* stat, const mi_stat_counter_t* src, int64_t unit) 
{
	if(stat==src) 
		return;
...
// must be thread safe as it is called from stats_merge
static void FASTCALL mi_stats_add(mi_stats_t* stats, const mi_stats_t* src) 
{
	if(stats==src) 
		return;
...
static void STDCALL mi_print_amount(int64_t n, int64_t unit, mi_output_fun* out, void * arg) 
{
	mi_printf_amount(n, unit, out, arg, NULL);
}

static void STDCALL mi_print_count(int64_t n, int64_t unit, mi_output_fun* out, void * arg) 
{
	if(unit==1) 
...
static void mi_stat_process_info(mi_msecs_t* elapsed,
    mi_msecs_t* utime,
    mi_msecs_t* stime,
    size_t* current_rss,
    size_t* peak_rss,
...
static void mi_stats_merge_from(mi_stats_t* stats) 
{
	if(stats != &_mi_stats_main) {
		...
		memzero(stats, sizeof(mi_stats_t));
	}
}
...
void mi_stats_reset(void) NOEXCEPT 
{
	...
	memzero(&_mi_stats_main, sizeof(mi_stats_t));
...
mi_decl_export void mi_process_info(size_t* elapsed_msecs, size_t* user_msecs, size_t* system_msecs, size_t* current_rss,
    size_t* peak_rss, size_t* current_commit, size_t* peak_commit, size_t* page_faults) NOEXCEPT
{
	...
	ASSIGN_PTR(elapsed_msecs, (elapsed < 0 ? 0 : (elapsed < (mi_msecs_t)PTRDIFF_MAX ? (size_t)elapsed : PTRDIFF_MAX)));
	ASSIGN_PTR(user_msecs, (utime < 0 ? 0 : (utime < (mi_msecs_t)PTRDIFF_MAX ? (size_t)utime : PTRDIFF_MAX)));
	ASSIGN_PTR(system_msecs, (stime < 0 ? 0 : (stime < (mi_msecs_t)PTRDIFF_MAX ? (size_t)stime : PTRDIFF_MAX)));
	ASSIGN_PTR(current_rss, current_rss0);
	ASSIGN_PTR(peak_rss, peak_rss0);
	ASSIGN_PTR(current_commit, current_commit0);
	ASSIGN_PTR(peak_commit, peak_commit0);
	ASSIGN_PTR(page_faults, page_faults0);
}
