...
static bool mi_region_try_claim(int numa_node,
    size_t blocks,
    bool allow_large,
    mem_region_t** region,
    mi_bitmap_index_t* bit_idx,
    mi_os_tld_t* tld)
{
	...
	size_t idx = tld->region_idx; // Or start at 0 to reuse low addresses? Starting at 0 seems to increase latency
	                              // though
...
static void * mi_region_try_alloc(size_t blocks, bool* commit, bool* large, bool* is_pinned, bool* is_zero, size_t* memid, mi_os_tld_t* tld)
{
	...
	*memid     = mi_memid_create(region, bit_idx);
	...
	if(_mi_bitmap_is_any_claimed(&region->reset, 1, blocks, bit_idx)) {
		...
		mi_assert_internal(!info.x.is_large && !info.x.is_pinned);
		mi_assert_internal(!mi_option_is_enabled(mi_option_eager_commit) || *commit || mi_option_get(
			    mi_option_eager_commit_delay) > 0);
