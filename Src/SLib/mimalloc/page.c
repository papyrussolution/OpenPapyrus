...
bool _mi_page_is_valid(mi_page_t* page) {
	...
	if(mi_page_heap(page)!=NULL) {
		...
		mi_assert_internal(!_mi_process_is_initialized || segment->thread_id==0 || segment->thread_id == mi_page_heap(
			    page)->thread_id);
		if(segment->kind != MI_SEGMENT_HUGE) {
			mi_page_queue_t* pq = mi_page_queue_of(page);
			mi_assert_internal(mi_page_queue_contains(pq, page));
			mi_assert_internal(pq->block_size==mi_page_block_size(page) || mi_page_block_size(
				    page) > MI_MEDIUM_OBJ_SIZE_MAX || mi_page_is_in_full(page));
...
void _mi_page_use_delayed_free(mi_page_t* page, mi_delayed_t delay, bool override_never) {
	...
	do {
		tfree = mi_atomic_load_acquire(&page->xthread_free); // note: must acquire as we can break/repeat this
		                                                     // loop and not do a CAS;
...
static void mi_page_init(mi_heap_t* heap, mi_page_t* page, size_t block_size, mi_tld_t* tld) {
	...
	page->xblock_size = (block_size < MI_HUGE_BLOCK_SIZE ? (uint32_t)block_size : MI_HUGE_BLOCK_SIZE); // initialize
	                                                                                                   // before
...
static mi_page_t* mi_large_huge_page_alloc(mi_heap_t* heap, size_t size) {
	...
	if(page != NULL) {
		...
		const size_t bsize = mi_page_usable_block_size(page); // note: not `mi_page_block_size` to account for
		                                                      // padding
...
void* _mi_malloc_generic(mi_heap_t* heap, size_t size) mi_attr_noexcept
{
	...
	if(mi_unlikely(page == NULL)) { // out of memory
		const size_t req_size = size - MI_PADDING_SIZE; // correct for padding_size in case of an overflow on
		                                                // `size`
