...
bool _mi_page_is_valid(mi_page_t* page) {
	...
	if(mi_page_heap(page) != NULL) {
		...
		if(segment->page_kind != MI_PAGE_HUGE) {
			...
			mi_assert_internal(pq->block_size==mi_page_block_size(page) || mi_page_block_size(
				    page) > MI_LARGE_OBJ_SIZE_MAX || mi_page_is_in_full(page));
...
void _mi_page_use_delayed_free(mi_page_t* page, mi_delayed_t delay, bool override_never) 
{
	...
	do {
		...
		if(UNLIKELY(old_delay == MI_DELAYED_FREEING)) {
...
static void _mi_page_thread_free_collect(mi_page_t* page)
{
	...
	// if `count > max_count` there was a memory corruption (possibly infinite list due to double multi-threaded
	// free)
	if(count > max_count) {
...
static void mi_page_to_full(mi_page_t* page, mi_page_queue_t* pq) 
{
	...
	_mi_page_free_collect(page, false); // try to collect right away in case another thread freed just before
...
void _mi_page_retire(mi_page_t* page) 
{
	...
	mi_page_queue_t* pq = mi_page_queue_of(page);
	if(LIKELY(page->xblock_size <= MI_MAX_RETIRE_SIZE && !mi_page_is_in_full(page))) {
...
static mi_page_t* mi_find_page(mi_heap_t* heap, size_t size) NOEXCEPT 
{
	// huge allocation?
	const size_t req_size = size - MI_PADDING_SIZE; // correct for padding_size in case of an overflow on `size`
	if(UNLIKELY(req_size > (MI_LARGE_OBJ_SIZE_MAX - MI_PADDING_SIZE) )) {
		if(UNLIKELY(req_size > PTRDIFF_MAX)) { // we don't allocate more than PTRDIFF_MAX (see <https://sourceware.org/ml/libc-announce/2019/msg00001.html>)
...
// Note: in debug mode the size includes MI_PADDING_SIZE and might have overflowed.
//
void * _mi_malloc_generic(mi_heap_t* heap, size_t size) NOEXCEPT
{
	mi_assert_internal(heap != NULL);
	// initialize if necessary
	if(UNLIKELY(!mi_heap_is_initialized(heap))) {
	...
	_mi_deferred_free(heap, false); // call potential deferred free routines
	_mi_heap_delayed_free(heap); // free delayed frees from other threads
	mi_page_t* page = mi_find_page(heap, size); // find (or allocate) a page of the right size
	if(UNLIKELY(page == NULL)) { // first time out of memory, try to collect and retry the allocation once more
		mi_heap_collect(heap, true /* force */);
		page = mi_find_page(heap, size);
	}
	if(UNLIKELY(page == NULL)) { // out of memory
		const size_t req_size = size - MI_PADDING_SIZE; // correct for padding_size in case of an overflow on `size`
