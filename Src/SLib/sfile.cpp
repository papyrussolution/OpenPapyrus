				...
				if(_fsize > 0) {
					if(entry_sign.NotEmpty()) {
                        if(entry_sign.C(0) == 'T') {
							...
							//
							// За пределами read_block_size-байтовой зоны ничего читать не будем!
							//
							used_offs_list.clear();
							binary_chunk_list.Z();
							ss_subsigns.Z();
...
CsvSniffer::Result & CsvSniffer::Result::Z()
{
	FieldDivisor = 0;
	Flags = 0;
	EmptyLineCount = 0;
...
CsvSniffer::Result::ColumnStat * CsvSniffer::Result::GetColumnStat(uint fldNo/*[1..]*/)
{
...
public:
	struct ColumnStat {
		...
		StatBase LenStat; // Статистика длины текста (без первой строки и только для непустых значений)
		...
		LongArray DataRefList; // Позиции текста в TemporaryCsvDataCollector::HT
	};	
					...
					if(f_out.IsValid()) {
						{
							...
							if(first_column_combined_nta.getCount()) {
								...
								for(uint i = 0; i < first_column_combined_nta.getCount(); i++) {
									const SNaturalToken & r_tok = first_column_combined_nta.at(i);
