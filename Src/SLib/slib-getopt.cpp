...
#pragma hdrstop
	...
	#define _(msgid) (msgid) // @sobolev
#endif

extern int _getopt_internal(int ___argc, char ** ___argv, const char * __shortopts, const struct option * __longopts, int * __longind, int __long_only, int __posixly_correct);
...
struct _getopt_data {
	...
	// variables, except that they are used for the reentrant versions of getopt.
	int optind;
	int opterr;
	int optopt;
	char * optarg;
	...
	char * __nextchar;
	enum __ord __ordering; /* See __ord above.  */
	int __posixly_correct; /* If the POSIXLY_CORRECT environment variable is set or getopt was called.  */
...
#define _GETOPT_DATA_INITIALIZER { 1, 1 } // The initializer is necessary to set OPTIND and OPTERR to their default values and to clear the initialization flag

extern int _getopt_internal_r(int ___argc, char ** ___argv, const char * __shortopts, const struct option * __longopts, int * __longind, int __long_only, struct _getopt_data * __data, int __posixly_correct);
extern int _getopt_long_r(int ___argc, char ** ___argv, const char * __shortopts, const struct option * __longopts, int * __longind, struct _getopt_data * __data);
extern int _getopt_long_only_r(int ___argc, char ** ___argv, const char * __shortopts, const struct option * __longopts, int * __longind, struct _getopt_data * __data);
//
...
// 
// Using `getopt' or setting the environment variable POSIXLY_CORRECT disables permutation.
...
// 
int optind = 1; // 1003.2 says this must be 1 before any call. 
int opterr = 1; // Callers store zero here to inhibit the error message for unrecognized options.
// 
...
// 
int optopt = '?';
static struct _getopt_data getopt_data; // Keep a global copy of all internal members of getopt_data
	...
	// that we can compare them later.  But we must not use SAlloc::M(3).
	extern int __libc_argc;
	extern char ** __libc_argv;
	// Bash 2.0 gives us an environment variable containing flags
	// indicating ARGV elements that should not be considered arguments. 
	#ifdef USE_NONOPTION_FLAGS
		extern char * __getopt_nonoption_flags; /* Defined in getopt_init.c  */
	#endif
	#ifdef USE_NONOPTION_FLAGS
...
void getopt_init()
{
	...
	opterr = 1;
	optopt = '?';
	memzero(&getopt_data, sizeof(getopt_data));
}
// 
...
static void exchange(char ** argv, struct _getopt_data * d)
{
	...
	// string can work normally.  Our top argument must be in the range of the string. 
...
static const char * _getopt_initialize(int argc, char ** argv, const char * optstring, struct _getopt_data * d, int posixly_correct)
{
	...
	// is the program name); the sequence of previously skipped non-option ARGV-elements is empty.  */
		...
		if(d->__nonoption_flags_max_len == 0) {
			if(__getopt_nonoption_flags == NULL || __getopt_nonoption_flags[0] == '\0')
				...
				if(__getopt_nonoption_flags == NULL)
					...
					memzero(__mempcpy(__getopt_nonoption_flags, orig_str, len), d->__nonoption_flags_max_len - len);
			}
		}
...
int _getopt_internal_r(int argc, char ** argv, const char * optstring,
    const struct option * longopts, int * longind, int long_only, struct _getopt_data * d, int posixly_correct)
{
	...
	if(d->optind == 0 || !d->__initialized) {
		SETIFZ(d->optind, 1); // Don't scan ARGV[0], the program name
		optstring = _getopt_initialize(argc, argv, optstring, d, posixly_correct);
		d->__initialized = 1;
	}
	else if(oneof2(optstring[0], '-', '+'))
		optstring++;
	if(optstring[0] == ':')
		print_errors = 0;
...
#endif
	if(!d->__nextchar || *d->__nextchar == '\0') {
	...
	if(longopts && (argv[d->optind][1] == '-' || (long_only && (argv[d->optind][2] || !strchr(optstring, argv[d->optind][1]))))) {
		...
		for(p = longopts, option_index = 0; p->name; p++, option_index++) {
			if(!strncmp(p->name, d->__nextchar, nameend - d->__nextchar)) {
				if((uint)(nameend - d->__nextchar) == (uint)strlen(p->name)) {
					// Exact match found. 
					pfound = p;
		...
		if(pfound) {
...
#if defined _LIBC && defined USE_IN_LIBIO
						char * buf;
						if(__asprintf(&buf, _("%s: option '--%s' requires an argument\n"), argv[0], pfound->name) >= 0) {
							_IO_flockfile(stderr);
							int old_flags2 = ((_IO_FILE*)stderr)->_flags2;
							((_IO_FILE*)stderr)->_flags2 |= _IO_FLAGS2_NOTCANCEL;
							__fxprintf(NULL, "%s", buf);
			...
			d->__nextchar += strlen(d->__nextchar);
			ASSIGN_PTR(longind, option_index);
			if(pfound->flag) {
	...
	// Look at and handle the next short option-character
	{
		...
		if(temp[0] == 'W' && temp[1] == ';') {
			...
			if(*d->__nextchar != '\0') {
				...
				d->optarg = argv[d->optind++]; // We already incremented `d->optind' once; increment it again when taking next ARGV-elt as argument. 
...
#if defined _LIBC && defined USE_IN_LIBIO
							char * buf;
							if(__asprintf(&buf, _("%s: option '-W %s' doesn't allow an argument\n"), argv[0], pfound->name) >= 0) {
								_IO_flockfile(stderr);
								int old_flags2 = ((_IO_FILE*)stderr)->_flags2;
								((_IO_FILE*)stderr)->_flags2 |= _IO_FLAGS2_NOTCANCEL;
								__fxprintf(NULL, "%s", buf);
...
#if defined _LIBC && defined USE_IN_LIBIO
							char * buf;
							if(__asprintf(&buf, _("%s: option '-W %s' requires an argument\n"), argv[0], pfound->name) >= 0) {
								_IO_flockfile(stderr);
								int old_flags2 = ((_IO_FILE*)stderr)->_flags2;
								((_IO_FILE*)stderr)->_flags2 |= _IO_FLAGS2_NOTCANCEL;
								__fxprintf(NULL, "%s", buf);
		...
		if(temp[1] == ':') {
			if(temp[2] == ':') {
				...
				if(*d->__nextchar != '\0') {
					...
					d->optarg = argv[d->optind++]; // We already incremented `optind' once; increment it again when taking next ARGV-elt as argument
				d->__nextchar = NULL;
			}
		}
		return c;
...
// glibc gets a LSB-compliant getopt. Standalone applications get a POSIX-compliant getopt. 
#if _LIBC
...
{
	return _getopt_internal(argc, argv, optstring, (const struct option *)0, (int*)0, 0, 1);
}
#endif
//
// getopt1.c
...
#ifdef TEST
	// 
	// Compile with -DTEST to make an executable for use in testing the above definition of `getopt'.
	// 
