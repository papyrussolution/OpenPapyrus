...
int compress(char *src, char *dest, int isdecomp, PercentFunc pf, ulong *sz)
{
	...
	if(lzo_init() != LZO_E_OK)
		return r;
	wrkmem = (long huge*)farcalloc(((LZO1X_1_MEM_COMPRESS)+sizeof(long)-1)/sizeof(long), sizeof(long));
	THROW_V(wrkmem, SLERR_NOMEM);
	do {
		ZFREE(inbuf);
		ZFREE(outbuf);
		inbuf = (unsigned char huge*)malloc(BUFLEN);
		outbuf = (unsigned char huge*)malloc(BUFLEN);
...
// } AHTOXA
	...
	if(fout) {
		...
		fout = 0;
#ifdef __WIN32__
		srchdl = CreateFile(dest, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		THROW_V(srchdl >= 0, SLERR_OPENFAULT);
