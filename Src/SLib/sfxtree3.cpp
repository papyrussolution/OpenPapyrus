// SFXTREE2.CPP
// Copyright (c) A.Sobolev 2016, 2023
// @codepage UTF-8
//
#include <slib.h>
...
private:
	...
	struct Edge : public SSuffixTree::IndexBase {
		...
		//uint   AlphaP; // Позиция в SSuffixTree::Alphabet
		uint   SrcNodeP;
		uint   DestNodeP;
	};

	class EdgeSlot : public SArray {
	public:
		struct Item {
			uint   AlphaP;
			uint   EdgeP;
		};
		EdgeSlot() : SArray(sizeof(Item))
        {
        }
        int    Insert(uint alphaP, uint edgeP)
        {
			Item item;
			item.AlphaP = alphaP;
			item.EdgeP = edgeP;
            uint   spos = 0;
            assert(!lsearch(&alphaP, &spos, CMPF_LONG));
            return ordInsert(&item, &spos, CMPF_LONG);
        }
        uint   SearchEdge(uint alphaP)
        {
        	uint   result = 0;
        	uint   spos = 0;
        	if(bsearch(&alphaP, &spos, CMPF_LONG)) {
				const Item * p_item = (const Item *)at(spos);
				result = p_item->EdgeP;
        	}
            return result;
        }
	...
	struct Node {
		...
		//TSArray <uint> EdgePosList; // Список индексов в массиве SSuffixArray::EdgeL исходящих ребер

		//TAILQ_ENTRY(LstNode) iteration;  // For DFS/BFS iteration, we maintain a list as well.
		//SSuffixTree::Edge * P_UpEdge;
		//SSuffixTree::Node * P_SuffixLinkNode;
		uint   UpEdgeP; // Индекс в массиве SSuffixArray::EdgeL входящего ребра
		uint   SlotP;        // Индекс слота исходящих ребер. Для листового узла - 0
	...
	EdgeSlot * FASTCALL GetSlot(uint p) const;
	...
	void   OutputChr(const SSuffixTree::String * pStr, uint chrIdx, long flags, SString & rBuf);
	void   OutputChrChunk(const SSuffixTree::IndexBase & rRange, long flags, SString & rBuf);
	void   OutputEdge(uint edgeP, long flags, SString & rBuf);
	void   OutputNode(uint nodeP, uint tabs, long flags, SString & rBuf);
	//
	//
	//
	enum {
	...
	String Alphabet; // Список символов, встречающихся во всех строках StrList. Каждый
		...
		// Нулевая позиция резервируется для псевдосимвола конца строки.
	//TSCollection <Node> NodeL; // Нулевая позиция в массиве - эксклюзивная
	TSArray <Node> NodeL; // Нулевая позиция в массиве - эксклюзивная
	TSCollection <EdgeSlot> SlotL; // Нулевая позиция в массиве - эксклюзивная
...
SSuffixTree::String::String(uint itemSize, uint32 id) : SArray(itemSize, 128)
{
	ID = id;
	PhasePosition = 0;
}
...
//
//
SSuffixTree::Node::Node(/*uint idx*/)
{
	UpEdgeP = 0;
	SlotP = 0;
	SfxLinkNodeP = 0;
...
SSuffixTree::SSuffixTree(uint32 itemSize) : Alphabet(itemSize, 0)
{
	...
	{
		// Нулевая позиция списка слотов - эксклюзивная
		EdgeSlot * p_new_slot = new EdgeSlot();
		SlotL.insert(p_new_slot);
	}
	{
	...
	{
		// Нулевая позиция алфавита резервируется для псевдосимвола конца строки
		uint8 zero_alpha[64];
		memzero(zero_alpha, sizeof(zero_alpha));
		Alphabet.insert(zero_alpha);
	}
}
...
{
	uint   pos = 1; // Нулевую позицию пропускаем (символ конца строки)
...
uint SLAPI SSuffixTree::CreateSlot()
{
	uint   new_slot_p = SlotL.getCount();
	assert(new_slot_p > 0);
	EdgeSlot * p_new_slot = new EdgeSlot();
	THROW(p_new_slot);
	THROW(SlotL.insert(p_new_slot));
	CATCH
		new_slot_p = 0;
	ENDCATCH
	return new_slot_p;	
...
uint SSuffixTree::Helper_CreateEdge(uint srcNodeP, uint destNodeP, uint strP, uint startIdx, uint endIdx, uint flags)
{
	...
	THROW(EdgeL.insert(&new_edge));
	{
		...
		assert(p_src_node);
		if(p_src_node) {
			uint   slot_p = p_src_node->SlotP;
			THROW(SETIFZ(slot_p, CreateSlot()));
			EdgeSlot * p_slot = GetSlot(slot_p);
			assert(p_slot);
			//p_src_node->EdgePosList.insert(&new_edge_p);
			{
				const SSuffixTree::String * p_str = GetStr(strP);
				assert(p_str);
				uint   alpha_p = 0;
				if(startIdx < p_str->getCount()) {
					uint _ap = SearchChrInAlphabet(p_str->at(startIdx));
					assert(_ap);
					alpha_p = _ap-1;
				}
				THROW(p_slot->Insert(alpha_p, new_edge_p));
			}
		}
	}
	{
...
// Descr: Создает листовой узел с индексом leafNodeIdx и ребро от srcNodeP к новому листовому узлу
//
uint SSuffixTree::CreateLeaf(uint srcNodeP, uint strP, uint startIdx)
{
	uint   new_edge_p = 0;
	uint   leaf_node_p = CreateNode();
	THROW(leaf_node_p);
...
SSuffixTree::EdgeSlot * FASTCALL SSuffixTree::GetSlot(uint p) const
{
	return (p && p < SlotL.getCount()) ? SlotL.at(p) : 0;
}

int FASTCALL SSuffixTree::IsLeafNode(uint nodeP) const
{
	const Node * p_node = GetNode(nodeP);
	return p_node ? BIN(p_node->SlotP == 0) : 0;
}
...
int SSuffixTree::StrEq(uint str1P, uint item1, uint str2P, uint item2) const
{
	...
	if(item1 <= len1 && item2 <= len2) {
		...
		if(item1 == len1 || item2 == len2) {
			if(item1 == len1 && item2 == len2)
				yes = (str1P == str2P) ?  1/* Comparing end of identical strings */ : 0/* Comparing end of different strings */;
			else
				yes = 0 /*Comparing end and non-end*/;
		}
		else
			yes = IsEqChr(p_str1->at(item1), p_str2->at(item2));
	}
	return yes;
}
...
uint SSuffixTree::FindEdgeWithStartitem(uint nodeP, uint strP, uint index) const
{
	...
	if(p_node && index <= GetStrLen(strP)) { //D0("Invalid input\n");
		const uint _c = p_node->EdgePosList.getCount();
		for(uint i = 0; !result_edge_p && i < _c; i++) {
			const uint edge_p = p_node->EdgePosList.at(i);
			...
			if(end.NodeP) {
				...
				assert(end.EdgeP);
				{
					...
					if(StrEq(stringP, skipstring.ExtraIdx, p_end_edge->StrP, p_end_edge->StartIdx + end.Offset)) {
						...
						THROW(new_inner_node_p = CreateNode());
...
void SSuffixTree::OutputChr(const SSuffixTree::String * pStr, uint chrIdx, long flags, SString & rBuf)
{
	if(pStr) {
...
void SSuffixTree::OutputChrChunk(const SSuffixTree::IndexBase & rRange, long flags, SString & rBuf)
{
...
void SSuffixTree::OutputEdge(uint edgeP, long flags, SString & rBuf)
{
...
void SSuffixTree::OutputNode(uint nodeP, uint tabs, long flags, SString & rBuf)
{
	const Node * p_node = GetNode(nodeP);
	if(tabs)
		rBuf.Tab(tabs);
	if(p_node) {
		...
		const uint ec = p_node->EdgePosList.getCount();
		if(ec) {
			rBuf.CR();
			for(uint i = 0; i < p_node->EdgePosList.getCount(); i++) {
				const uint edge_p = p_node->EdgePosList.at(i);
				rBuf.Tab(tabs+1);
...
void SSuffixTree::CalcStat(SSuffixTree::Stat * pStat) const
{
	if(pStat) {
		...
		MemLeakTracer mlt;
		{
			SSuffixTree st(1);
			{
				...
				/*
				{
					...
					{
						SFile f_out("TestSuffixTree", SFile::mWrite);
						SString outp_buf;
