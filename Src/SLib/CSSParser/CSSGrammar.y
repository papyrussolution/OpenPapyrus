...
%union {
    ...
    TSCollection <CSSSelector> * selectorList;
    ...
    TSCollection <MediaQueryExp> * mediaQueryExpList;
...
static int cssyylex(YYSTYPE * yylval, void * pParser) { return static_cast<CSSParser*>(pParser)->lex(yylval); }
%}
...
stylesheet: maybe_charset maybe_sgml import_list variables_list namespace_list rule_list | 
...
valid_rule_or_import: valid_rule | import ;

webkit_rule: WEBKIT_RULE_SYM '{' maybe_space valid_rule_or_import maybe_space '}' 
{
	static_cast<CSSParser*>(parser)->m_rule = $4;
};

webkit_keyframe_rule: WEBKIT_KEYFRAME_RULE_SYM '{' maybe_space keyframe_rule maybe_space '}' 
{
	static_cast<CSSParser*>(parser)->m_keyframe = $4;
};

webkit_decls: WEBKIT_DECLS_SYM '{' maybe_space declaration_list '}' 
{
	/* can be empty */
};

webkit_variables_decls: WEBKIT_VARIABLES_DECLS_SYM '{' maybe_space variables_declaration_list '}' 
{
	/* can be empty */
};

webkit_value: WEBKIT_VALUE_SYM '{' maybe_space expr '}' 
{
...
webkit_mediaquery: WEBKIT_MEDIAQUERY_SYM WHITESPACE maybe_space media_query '}' 
{
	CSSParser * p = static_cast<CSSParser*>(parser);
	p->m_mediaQuery = p->sinkFloatingMediaQuery($4);
};

webkit_selector: WEBKIT_SELECTOR_SYM '{' maybe_space selector_list '}' 
{
	if($4) {
		CSSParser * p = static_cast<CSSParser*>(parser);
...
charset: CHARSET_SYM maybe_space STRING maybe_space ';' 
{
	CSSParser * p = static_cast<CSSParser*>(parser);
...
import_list: /* empty */ | import_list import maybe_sgml 
{
	CSSParser * p = static_cast<CSSParser*>(parser);
	if($2 && p->m_styleSheet)
...
variables_list: /* empty */ | variables_list variables_rule maybe_sgml 
{
	CSSParser * p = static_cast<CSSParser*>(parser);
	if($2 && p->m_styleSheet)
		p->m_styleSheet->append($2);
...
namespace_list: /* empty */ | namespace_list namespace maybe_sgml;

rule_list: /* empty */ | rule_list rule maybe_sgml 
{
	CSSParser * p = static_cast<CSSParser*>(parser);
	if($2 && p->m_styleSheet)
		p->m_styleSheet->append($2);
...
valid_rule: ruleset | media | page | font_face | keyframes;

rule: valid_rule 
{
	static_cast<CSSParser*>(parser)->m_hadSyntacticallyValidCSSRule = true;
} | invalid_rule | invalid_at | invalid_import;

block_rule_list: /* empty */ 
{ 
	$$ = 0; 
...
block_valid_rule: ruleset | page | font_face | keyframes;
block_rule: block_valid_rule | invalid_rule | invalid_at | invalid_import | invalid_media;

import: IMPORT_SYM maybe_space string_or_uri maybe_space maybe_media_list ';' 
{
...
variables_rule: WEBKIT_VARIABLES_SYM maybe_space maybe_media_list '{' maybe_space variables_declaration_list '}' 
{
...
variables_declaration_list: variables_declaration 
{
	$$ = $1;
...
variables_declaration: variable_name ':' maybe_space expr 
{
...
variable_name: IDENT maybe_space 
{
	$$ = $1;
};

namespace: NAMESPACE_SYM maybe_space maybe_ns_prefix string_or_uri maybe_space ';' 
{
    CSSParser* p = static_cast<CSSParser*>(parser);
    if (p->m_styleSheet)
...
maybe_ns_prefix: /* empty */ { $$.characters = 0; } | IDENT WHITESPACE { $$ = $1; };
string_or_uri: STRING | URI;
media_feature: IDENT maybe_space { $$ = $1; };
maybe_media_value: /*empty*/ { $$ = 0; } | ':' maybe_space expr maybe_space { $$ = $3; };

media_query_exp: '(' maybe_space media_feature maybe_space maybe_media_value ')' maybe_space 
{
	$3.lower();
...
maybe_and_media_query_exp_list: /*empty*/ 
{
...
keyframes: WEBKIT_KEYFRAMES_SYM maybe_space keyframe_name maybe_space '{' maybe_space keyframes_rule '}' 
{
	$$ = $7;
	$7->setNameInternal($3);
};
...
key: PERCENTAGE 
{ 
	$$.id = 0; $$.isInt = false; $$.fValue = $1; $$.unit = CSSPrimitiveValue::CSS_NUMBER; 
} | IDENT 
{
...
page: PAGE_SYM error invalid_block 
{
	$$ = 0;
} | PAGE_SYM error ';' 
{
...
font_face: FONT_FACE_SYM maybe_space '{' maybe_space declaration_list '}'  maybe_space 
{
...
ruleset: selector_list '{' maybe_space declaration_list closing_brace 
{
	$$ = static_cast<CSSParser*>(parser)->createStyleRule($1);
};

selector_list: selector %prec UNIMPORTANT_TOK 
{
    if ($1) {
        CSSParser* p = static_cast<CSSParser*>(parser);
...
selector_with_trailing_whitespace: selector WHITESPACE 
{
	$$ = $1;
};
...
namespace_selector: /* empty */ '|' 
{ 
	$$.characters = 0; $$.length = 0; 
...
pseudo: ':' IDENT 
{
		...
		else if(type == CSSSelector::PseudoBefore || type == CSSSelector::PseudoAfter) {
            CSSParser* p = static_cast<CSSParser*>(parser);
            if(Document* doc = p->document())
...
prio: IMPORTANT_SYM maybe_space { $$ = true; } | /* empty */ { $$ = false; };
...
function: FUNCTION maybe_space expr ')' maybe_space 
{
	CSSParser* p = static_cast<CSSParser*>(parser);
 ...
 */
hexcolor: HEX maybe_space { $$ = $1; } | IDSEL maybe_space { $$ = $1; };


/* error handling rules */
save_block: closing_brace 
{
	$$ = 0;
} | error closing_brace 
{
...
invalid_at: ATKEYWORD error invalid_block 
{
	$$ = 0;
} | ATKEYWORD error ';' 
{
...
invalid_at_list: invalid_at maybe_sgml | invalid_at_list invalid_at maybe_sgml;
...
invalid_block: '{' error invalid_block_list error closing_brace 
{
...
invalid_block_list: invalid_block | invalid_block_list error invalid_block;
