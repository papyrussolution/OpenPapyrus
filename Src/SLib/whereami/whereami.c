...
static int wai_getModulePath_(HMODULE module, char * out, int capacity, int* dirname_length)
{
	...
	for(;;) {
		...
		DWORD size = GetModuleFileNameW(module, buffer1, SIZEOFARRAY(buffer1));
		if(size == 0)
			break;
		else if(size == SIZEOFARRAY(buffer1)) {
			DWORD size_ = size;
			do {
				wchar_t * path_ = (wchar_t *)SAlloc::R(path, sizeof(wchar_t) * size_ * 2);
				if(!path_)
					break;
				size_ *= 2;
				path = path_;
		...
		SETIFZ(length__, WideCharToMultiByte(CP_UTF8, 0, buffer2, length_, NULL, 0, NULL, NULL));
		if(length__ == 0)
			break;
		if(length__ <= capacity && dirname_length) {
			for(int i = length__ - 1; i >= 0; --i) {
				if(out[i] == '\\') {
					*dirname_length = i;
					break;
				}
...
CXX_NOINLINE int wai_getExecutablePath(char* out, int capacity, int* dirname_length)
{
	return wai_getModulePath_(NULL, out, capacity, dirname_length);
}

CXX_NOINLINE int wai_getModulePath(char* out, int capacity, int* dirname_length)
{
	HMODULE module;
	int length = -1;
#if defined(_MSC_VER)
