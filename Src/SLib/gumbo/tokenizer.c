...
//
// Descr: This is the main tokenizer state struct, containing all state used by in
...
struct GumboTokenizerState {
	...
	// returns from the lexer.  kGumboNoChar is used to represent 'no character stored.'
	//
	...
	GumboTagState _tag_state; // Current tag state.
	...
	Utf8Iterator _input; // The UTF8Iterator over the tokenizer input.
};
...
static void FASTCALL clear_temporary_buffer(GumboParser * parser) 
{
	GumboTokenizerState * tokenizer = parser->_tokenizer_state;
...
static void FASTCALL append_char_to_temporary_buffer(GumboParser * parser, int codepoint) 
{
	gumbo_string_buffer_append_codepoint(codepoint, &parser->_tokenizer_state->_temporary_buffer);
}
...
// Make sure this remains side-effect free; it's used in assertions.
	...
	static bool FASTCALL temporary_buffer_equals(GumboParser * parser, const char* text) 
	{
		...
		return (text_len == buffer->length && memcmp(buffer->data, text, text_len) == 0);
	}
#endif
...
static void FASTCALL emit_char(GumboParser * parser, int c, GumboToken* output) 
{
...
static StateResult FASTCALL emit_current_char(GumboParser * parser, GumboToken* output) 
{
...
static void FASTCALL emit_doctype(GumboParser * parser, GumboToken* output) 
{
...
static StateResult FASTCALL emit_current_tag(GumboParser * parser, GumboToken * output) 
{
	...
	if(tag_state->_is_start_tag) {
		...
		for(uint i = 0; i < tag_state->_attributes.length; ++i) {
			gumbo_destroy_attribute(static_cast<GumboAttribute *>(tag_state->_attributes.data[i]));
		}
		SAlloc::F(tag_state->_attributes.data);
		mark_tag_state_as_empty(tag_state);
...
//
static StateResult emit_char_ref(GumboParser * parser, int additional_allowed_char, bool is_in_attribute, GumboToken* output) 
{
	GumboTokenizerState* tokenizer = parser->_tokenizer_state;
...
static bool maybe_emit_from_temporary_buffer(GumboParser * parser, GumboToken* output) 
{
	GumboTokenizerState * tokenizer = parser->_tokenizer_state;
...
static void append_char_to_tag_buffer(GumboParser * parser, int codepoint, bool reinitilize_position_on_first) 
{
...
static void copy_over_original_tag_text(GumboParser * parser,
    GumboStringPiece* original_text, GumboSourcePosition* start_pos, GumboSourcePosition * end_pos) 
{
	GumboTokenizerState * tokenizer = parser->_tokenizer_state;
	GumboTagState* tag_state = &tokenizer->_tag_state;
...
{
	SAlloc::F(parser->_tokenizer_state->_tag_state._buffer.data);
	initialize_tag_buffer(parser);
}

// Moves some data from the temporary buffer over the the tag-based fields in
// TagState.
static void FASTCALL finish_tag_name(GumboParser * parser) 
{
	GumboTokenizerState * tokenizer = parser->_tokenizer_state;
	GumboTagState* tag_state = &tokenizer->_tag_state;
	tag_state->_tag = gumbo_tagn_enum(tag_state->_buffer.data, tag_state->_buffer.length);
...
static bool FASTCALL finish_attribute_name(GumboParser * parser) 
{
	GumboTokenizerState* tokenizer = parser->_tokenizer_state;
	GumboTagState* tag_state = &tokenizer->_tag_state;
	...
	for(uint i = 0; i < attributes->length; ++i) {
		...
		if(strlen(attr->name) == tag_state->_buffer.length && memcmp(attr->name, tag_state->_buffer.data, tag_state->_buffer.length) == 0) {
...
static void finish_attribute_value(GumboParser * parser) 
{
	...
	if(tag_state->_drop_next_attr_value) {
		// Duplicate attribute name detected in an earlier state, so we have to ignore the value.
		tag_state->_drop_next_attr_value = false;
		reinitialize_tag_buffer(parser);
	}
	else {
...
void gumbo_tokenizer_state_destroy(GumboParser * parser) 
{
	...
	if(tokenizer) {
		...
		SAlloc::F(tokenizer);
		parser->_tokenizer_state = 0; // @sobolev
	}
}
...
static StateResult handle_rawtext_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		case '<':
		    ...
		    append_char_to_temporary_buffer(parser, '<');
		    return NEXT_CHAR;
		case '\0': return emit_replacement_char(parser, output);
		case -1: return emit_eof(parser, output);
		default: return emit_current_char(parser, output);
	}
}

// http://www.whatwg.org/specs/web-apps/current-work/complete5/tokenization.html#script-data-state
static StateResult handle_script_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		case '<':
...
static StateResult handle_end_tag_open_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	...
	switch(c) {
		...
		case -1:
		    ...
		    gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
		    return static_cast<StateResult>(emit_temporary_buffer(parser, output));
		default:
		    if(isasciialpha(c)) {
			    gumbo_tokenizer_set_state(parser, GUMBO_LEX_TAG_NAME);
...
static StateResult handle_rcdata_end_tag_open_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	assert(temporary_buffer_equals(parser, "</"));
	if(isasciialpha(c)) {
	...
	return static_cast<StateResult>(true);
}
...
static StateResult handle_script_escaped_lt_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	...
	if(c == '/') {
		...
		append_char_to_temporary_buffer(parser, c);
		gumbo_string_buffer_append_codepoint(ensure_lowercase(c), &tokenizer->_script_data_buffer);
		return static_cast<StateResult>(emit_temporary_buffer(parser, output));
	}
	else {
		gumbo_tokenizer_set_state(parser, GUMBO_LEX_SCRIPT_ESCAPED);
    ...
    GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		...
		case '/':
		case '>':
		    gumbo_tokenizer_set_state(parser, gumbo_string_equals(&kScriptTag,
			    (GumboStringPiece*)&tokenizer->_script_data_buffer) ? GUMBO_LEX_SCRIPT_DOUBLE_ESCAPED : GUMBO_LEX_SCRIPT_ESCAPED);
		    return emit_current_char(parser, output);
		default:
		    if(isasciialpha(c)) {
			    gumbo_string_buffer_append_codepoint(ensure_lowercase(c), &tokenizer->_script_data_buffer);
...
static StateResult handle_script_double_escaped_dash_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		case '-':
...
static StateResult handle_script_double_escaped_dash_dash_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c,
    GumboToken* output) 
{
	switch(c) {
		case '-':
...
static StateResult handle_script_double_escaped_lt_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	if(c == '/') {
    ...
    GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		...
		case '>':
		    gumbo_tokenizer_set_state(parser, gumbo_string_equals(&kScriptTag,
			    (GumboStringPiece*)&tokenizer->_script_data_buffer) ? GUMBO_LEX_SCRIPT_ESCAPED : GUMBO_LEX_SCRIPT_DOUBLE_ESCAPED);
		    return emit_current_char(parser, output);
		default:
		    if(isasciialpha(c)) {
			    gumbo_string_buffer_append_codepoint(ensure_lowercase(c), &tokenizer->_script_data_buffer);
...
static StateResult handle_markup_declaration_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	if(utf8iterator_maybe_consume_match(&tokenizer->_input, "--", sizeof("--") - 1, true)) {
		...
		// doctype token.
		tokenizer->_doc_type_state.name = sstrdup("");
		tokenizer->_doc_type_state.public_identifier = sstrdup("");
		tokenizer->_doc_type_state.system_identifier = sstrdup("");
	}
...
static StateResult handle_doctype_public_id_double_quoted_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		case '"':
...
static StateResult handle_after_doctype_public_id_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	switch(c) {
		...
		case '\f':
		case ' ':
		    gumbo_tokenizer_set_state(parser, GUMBO_LEX_BETWEEN_DOCTYPE_PUBLIC_SYSTEM_ID);
		    return NEXT_CHAR;
		case '>':
		    gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
		    emit_doctype(parser, output);
...
static StateResult handle_bogus_doctype_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
	if(c == '>' || c == -1) {
		gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
		emit_doctype(parser, output);
...
static StateResult handle_cdata_state(GumboParser * parser, GumboTokenizerState* tokenizer, int c, GumboToken* output) 
{
		...
		switch(token->type) {
			...
			case GUMBO_TOKEN_START_TAG:
				for(uint i = 0; i < token->v.start_tag.attributes.length; ++i) {
					GumboAttribute * attr = static_cast<GumboAttribute *>(token->v.start_tag.attributes.data[i]);
