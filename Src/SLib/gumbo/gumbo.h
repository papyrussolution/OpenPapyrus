...
struct GumboStringPiece {
	const char * data; /** A pointer to the beginning of the string.  NULL iff length == 0. */
...
struct GumboAttribute {
	...
	GumboSourcePosition name_start; /** The starting position of the attribute name. */
	/**
	...
	GumboSourcePosition value_start; /** The starting position of the attribute value. */
	GumboSourcePosition value_end; /** The ending position of the attribute value. */
};
/**
 ...
 */
GumboAttribute * FASTCALL gumbo_get_attribute(const GumboVector * attrs, const char * name);
// 
// Descr: Enum denoting the type of node. This determines the type of the node.v union.
// 
enum GumboNodeType {
...
enum GumboParseFlags {
	 ...
	 * create a <p> tag and immediately close it, while </br> means the same thing as <br>.
	 */
	GUMBO_INSERTION_CONVERTED_FROM_END_TAG = 1 << 4,
	GUMBO_INSERTION_FROM_ISINDEX = 1 << 5, // A flag for nodes that are converted from the parse of an <isindex> tag
	GUMBO_INSERTION_FROM_IMAGE   = 1 << 6, // A flag for <image> tags that are rewritten as <img>
	/**
	...
	GUMBO_INSERTION_ADOPTION_AGENCY_CLONED = 1 << 8, // A flag for nodes that are cloned by the adoption agency algorithm
	GUMBO_INSERTION_ADOPTION_AGENCY_MOVED  = 1 << 9, // A flag for nodes that are moved by the adoption agency algorithm
	/**
...
struct GumboElement {
	 ...
	 */
	GumboVector /* GumboNode* */ children;
	GumboTag tag; /** The GumboTag enum for this element. */
	GumboNamespaceEnum tag_namespace; /** The GumboNamespaceEnum for this element. */
	/**
	...
	GumboSourcePosition start_pos; /** The source position for the start of the start tag. */
	GumboSourcePosition end_pos; /** The source position for the start of the end tag. */
	/**
...
struct GumboNode {
	...
	// inserted into the parse tree, including a variety of special parse situations.
	// 
 ...
 */
struct GumboOptions {
	//GumboAllocatorFunction allocator; /** A memory allocator function.  Default: malloc. */
	//GumboDeallocatorFunction deallocator; /** A memory deallocator function. Default: free. */
	/**
	 ...
	 * differentiate between, say, parsing a <title> tag in SVG vs. parsing it in HTML.
...
//
// Descr: The output struct containing the results of the parse
//
struct GumboOutput {
	/**
