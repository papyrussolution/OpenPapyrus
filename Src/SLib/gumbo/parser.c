...
// SourcePositions and yet the constant needs some linkage, so this is as good as any.
static const GumboSourcePosition kGumboEmptySourcePosition = {0, 0, 0};
//
// Debug function to trace operation of the parser.  Pass --copts=-DGUMBO_DEBUG to use.
//
...
GumboAttribute * FASTCALL gumbo_get_attribute(const GumboVector * attributes, const char * name) 
{
	if(attributes) {
...
static GumboParseFlags operator | (GumboParseFlags lv, GumboParseFlags rv)
{
	return static_cast<GumboParseFlags>(static_cast<int>(lv) | static_cast<int>(rv));
}

static GumboParseFlags & SetFlag(GumboParseFlags & rV, const GumboParseFlags f)
{
	rV = static_cast<GumboParseFlags>(static_cast<int>(rV) | static_cast<int>(f));
	return rV;
}

static GumboParseFlags & ResetFlag(GumboParseFlags & rV, const GumboParseFlags f)
{
	rV = static_cast<GumboParseFlags>(static_cast<int>(rV) & ~static_cast<int>(f));
	return rV;
}
...
#define _TAG(tag)        GUMBO_NAMESPACE_HTML, GUMBO_TAG_ ## tag
#define _TAG_SVG(tag)    GUMBO_NAMESPACE_SVG,  GUMBO_TAG_ ## tag
#define _TAG_MATHML(tag) GUMBO_NAMESPACE_MATHML, GUMBO_TAG_ ## tag

#define TAGSET_INCLUDES(tagset, __namespace, tag) (tag < GUMBO_TAG_LAST && tagset[(int)tag] == (1 << (int)__namespace))
...
public:
	...
	operator const gumbo_tagset & ()
	{
		return Set;
	}
	GumboTagSet & Append(int ns, int tag)
	{
		assert(oneof3(ns, GUMBO_NAMESPACE_HTML, GUMBO_NAMESPACE_SVG, GUMBO_NAMESPACE_MATHML));
		assert(tag >= 0 && tag < GUMBO_TAG_LAST);
		Set[tag] = (1 << ns);
		return *this;
	}
	gumbo_tagset Set;
...
static bool FASTCALL node_html_tag_is(const GumboNode *, GumboTag);
...
//
struct TextNodeBufferState {
	GumboStringBuffer _buffer; // The accumulated text to be inserted into the current text node.
	...
	GumboSourcePosition _start_position; // The source position of the start of this text node.
	GumboNodeType _type; // The type of node that will be inserted (TEXT, CDATA, or WHITESPACE).
};
...
struct GumboParserState {
	...
	GumboNode * _fragment_ctx; // The element used as fragment context when parsing in fragment mode
	bool _reprocess_current_token; // The flag for when the spec says "Reprocess the current token in..."
	bool _self_closing_flag_acknowledged; // The flag for "acknowledge the token's self-closing flag".
	bool _frameset_ok; // The "frameset-ok" flag from the spec.
	bool _ignore_next_linefeed; // The flag for "If the next token is a LINE FEED, ignore that token...".
	...
	TextNodeBufferState _text_node; // The accumulated text node buffer state.
...
{
	uint num_unmatched_attr2_elements = attr2->length;
	for(uint i = 0; i < attr1->length; ++i) {
		const GumboAttribute * attr = static_cast<const GumboAttribute *>(attr1->data[i]);
...
static GumboNode * FASTCALL create_node(GumboNodeType type) 
{
	GumboNode * node = static_cast<GumboNode *>(SAlloc::M(sizeof(GumboNode)));
	node->parent = NULL;
...
static GumboNode* new_document_node(GumboParser * parser) 
{
	...
	// Must be initialized explicitly, as there's no guarantee that we'll see a doc type token.
...
{
	GumboOutput * output = static_cast<GumboOutput *>(SAlloc::M(sizeof(GumboOutput)));
	output->root = NULL;
...
{
	GumboParserState * parser_state = static_cast<GumboParserState *>(SAlloc::M(sizeof(GumboParserState)));
...
static bool FASTCALL is_fragment_parser(const GumboParser * parser) { return !!parser->_parser_state->_fragment_ctx; }
//
// Returns the node at the bottom of the stack of open elements, or NULL if no
// elements have been added yet.
//
static GumboNode * FASTCALL get_cur_node(GumboParser * parser) 
{
...
{
	GumboParserState * state = parser->_parser_state;
	return (state->_open_elements.length == 1 && state->_fragment_ctx) ? state->_fragment_ctx : get_cur_node(parser);
}
//
...
static void FASTCALL set_insertion_mode(GumboParser * parser, GumboInsertionMode mode) 
...
static GumboInsertionMode get_appropriate_insertion_mode(const GumboParser * parser, int index) 
{
	...
	const GumboNode * node = static_cast<const GumboNode *>(open_elements->data[index]);
	...
	switch(node->v.element.tag) {
		case GUMBO_TAG_SELECT: 
			{
				if(is_last)
					...
					for(int i = index; i > 0; --i) {
						const GumboNode * ancestor = static_cast<const GumboNode *>(open_elements->data[i]);
						if(node_html_tag_is(ancestor, GUMBO_TAG_TEMPLATE))
							return GUMBO_INSERTION_MODE_IN_SELECT;
						else if(node_html_tag_is(ancestor, GUMBO_TAG_TABLE))
		...
		case GUMBO_TAG_TH:
		    ...
		    break;
		case GUMBO_TAG_TR: return GUMBO_INSERTION_MODE_IN_ROW;
		case GUMBO_TAG_TBODY:
		case GUMBO_TAG_THEAD:
		case GUMBO_TAG_TFOOT: return GUMBO_INSERTION_MODE_IN_TABLE_BODY;
		case GUMBO_TAG_CAPTION: return GUMBO_INSERTION_MODE_IN_CAPTION;
		case GUMBO_TAG_COLGROUP: return GUMBO_INSERTION_MODE_IN_COLUMN_GROUP;
		case GUMBO_TAG_TABLE: return GUMBO_INSERTION_MODE_IN_TABLE;
		case GUMBO_TAG_TEMPLATE: return get_current_template_insertion_mode(parser);
		case GUMBO_TAG_HEAD:
		    if(!is_last) 
				return GUMBO_INSERTION_MODE_IN_HEAD;
		    break;
		case GUMBO_TAG_BODY: return GUMBO_INSERTION_MODE_IN_BODY;
		case GUMBO_TAG_FRAMESET: return GUMBO_INSERTION_MODE_IN_FRAMESET;
		case GUMBO_TAG_HTML: return parser->_parser_state->_head_element ? GUMBO_INSERTION_MODE_AFTER_HEAD : GUMBO_INSERTION_MODE_BEFORE_HEAD;
		default:
		    break;
	}
	return is_last ? GUMBO_INSERTION_MODE_IN_BODY : GUMBO_INSERTION_MODE_INITIAL;
...
static GumboError * FASTCALL parser_add_parse_error(GumboParser * parser, const GumboToken* token) 
{
...
//
static bool FASTCALL tag_in(const GumboToken * token, bool is_start, const gumbo_tagset & tags) 
{
	GumboTag token_tag;
...
static bool FASTCALL tag_is(const GumboToken * token, bool is_start, GumboTag tag) 
{
	if(is_start && token->type == GUMBO_TOKEN_START_TAG)
...
static bool FASTCALL node_qualified_tag_is(const GumboNode* node, GumboNamespaceEnum ns, GumboTag tag) 
{
	assert(node);
	return oneof2(node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE) && node->v.element.tag == tag && node->v.element.tag_namespace == ns;
}
...
static GumboInsertionMode get_current_template_insertion_mode(const GumboParser * parser) 
{
	...
	return (GumboInsertionMode)(reinterpret_cast<long>(template_insertion_modes->data[(template_insertion_modes->length - 1)]));
}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG_MATHML(MI)).Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).Append(_TAG_MATHML(MTEXT));
	//gumbo_tagset ts = {TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT)};
	return node_tag_in_set(node, _ts);
}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG_SVG(FOREIGNOBJECT)).Append(_TAG_SVG(DESC)).Append(_TAG_SVG(TITLE));
	return node_tag_in_set(node, _ts/*(gumbo_tagset){TAG_SVG(FOREIGNOBJECT), TAG_SVG(DESC), TAG_SVG(TITLE)}*/) ||
		(node_qualified_tag_is(node, GUMBO_NAMESPACE_MATHML, GUMBO_TAG_ANNOTATION_XML) &&
		(attribute_matches(&node->v.element.attributes, "encoding", "text/html") ||
...
InsertionLocation get_appropriate_insertion_location(GumboParser * parser, GumboNode* override_target) 
{
	...
	{
		GumboTagSet _ts;
		_ts.Append(_TAG(TABLE)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TR));
		if(!parser->_parser_state->_foster_parent_insertions ||
			!node_tag_in_set(retval.target, _ts/*(gumbo_tagset){TAG(TABLE), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)}*/)) {
			return retval;
		}
	}
	...
	GumboVector* open_elements = &parser->_parser_state->_open_elements;
	for(uint i = 0; i < open_elements->length; ++i) {
		if(node_html_tag_is(static_cast<GumboNode *>(open_elements->data[i]), GUMBO_TAG_TEMPLATE)) {
			last_template_index = i;
		}
		if(node_html_tag_is(static_cast<GumboNode *>(open_elements->data[i]), GUMBO_TAG_TABLE)) {
			last_table_index = i;
		}
	}
	if(last_template_index != -1 && (last_table_index == -1 || last_template_index > last_table_index)) {
		retval.target = static_cast<GumboNode *>(open_elements->data[last_template_index]);
		return retval;
	}
	if(last_table_index == -1) {
		retval.target = static_cast<GumboNode *>(open_elements->data[0]);
		return retval;
	}
	...
	retval.target = static_cast<GumboNode *>(open_elements->data[last_table_index - 1]);
	return retval;
}

// Appends a node to the end of its parent, setting the "parent" and
// "index_within_parent" fields appropriately.
static void FASTCALL append_node(GumboNode * parent, GumboNode * node) 
{
	assert(node->parent == NULL);
	assert(node->index_within_parent == -1);
	GumboVector * children;
	if(oneof2(parent->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE)) {
...
//
static void FASTCALL insert_node(GumboNode * node, InsertionLocation location) 
{
	assert(node->parent == NULL);
	assert(node->index_within_parent == -1);
...
static void maybe_flush_text_node_buffer(GumboParser * parser) 
{
	...
	if(buffer_state->_buffer.length) {
		assert(oneof3(buffer_state->_type, GUMBO_NODE_WHITESPACE, GUMBO_NODE_TEXT, GUMBO_NODE_CDATA));
		...
		if(location.target->type == GUMBO_NODE_DOCUMENT) {
			// The DOM does not allow Document nodes to have Text children, so per the spec, they are dropped on the floor.
...
static GumboNode * FASTCALL pop_current_node(GumboParser * parser) 
{
	GumboParserState * state = parser->_parser_state;
	maybe_flush_text_node_buffer(parser);
	if(state->_open_elements.length > 0) {
		assert(node_html_tag_is(static_cast<const GumboNode *>(state->_open_elements.data[0]), GUMBO_TAG_HTML));
		gumbo_debug("Popping %s node.\n", gumbo_normalized_tagname(get_cur_node(parser)->v.element.tag));
	}
	GumboNode * current_node = static_cast<GumboNode *>(gumbo_vector_pop(parser, &state->_open_elements));
	if(!current_node) {
		assert(state->_open_elements.length == 0);
		return NULL;
	}
	assert(oneof2(current_node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE));
	bool is_closed_body_or_html_tag = (node_html_tag_is(current_node, GUMBO_TAG_BODY) && state->_closed_body_tag) ||
	    (node_html_tag_is(current_node, GUMBO_TAG_HTML) && state->_closed_html_tag);
	if((state->_current_token->type != GUMBO_TOKEN_END_TAG ||
	    !node_html_tag_is(current_node, state->_current_token->v.end_tag)) && !is_closed_body_or_html_tag) {
		SetFlag(current_node->parse_flags, GUMBO_INSERTION_IMPLICIT_END_TAG);
	}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(HTML)).Append(_TAG(TR)).Append(_TAG(TEMPLATE));
	while(!node_tag_in_set(get_cur_node(parser),
	    _ts/*(gumbo_tagset){TAG(HTML), TAG(TR), TAG(TEMPLATE)}*/)) {
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(HTML)).Append(_TAG(TABLE)).Append(_TAG(TEMPLATE));
	while(!node_tag_in_set(get_cur_node(parser),
	    _ts/*(gumbo_tagset){TAG(HTML), TAG(TABLE), TAG(TEMPLATE)}*/)) {
		pop_current_node(parser);
	}
}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(HTML)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TEMPLATE));
	while(!node_tag_in_set(get_cur_node(parser),
	    _ts/*(gumbo_tagset){TAG(HTML), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TEMPLATE)}*/)) {
		pop_current_node(parser);
	}
}
...
static GumboNode * FASTCALL insert_element_from_token(GumboParser * parser, GumboToken * token) 
{
...
static GumboNode* insert_element_of_tag_type(GumboParser * parser, GumboTag tag, GumboParseFlags reason) 
{
	...
	SetFlag(element->parse_flags, GUMBO_INSERTION_BY_PARSER | reason);
	insert_element(parser, element, false);
...
static GumboNode* insert_foreign_element(GumboParser * parser, GumboToken* token, GumboNamespaceEnum tag_namespace) 
{
	...
	insert_element(parser, element, false);
	if(token_has_attribute(token, "xmlns") && !attribute_matches_case_sensitive(&token->v.start_tag.attributes, "xmlns",
	...
	if(token_has_attribute(token, "xmlns:xlink") && !attribute_matches_case_sensitive(&token->v.start_tag.attributes,
...
static void FASTCALL insert_text_token(GumboParser * parser, GumboToken * token) 
{
	...
	gumbo_string_buffer_append_codepoint(token->v.character, &buffer_state->_buffer);
...
static bool find_last_anchor_index(GumboParser * parser, int* anchor_index) 
{
	...
	for(int i = elements->length; --i >= 0;) {
		GumboNode* node = static_cast<GumboNode *>(elements->data[i]);
...
{
	assert(node == &kActiveFormattingScopeMarker || node->type == GUMBO_NODE_ELEMENT);
	GumboVector * elements = &parser->_parser_state->_active_formatting_elements;
	if(node == &kActiveFormattingScopeMarker)
...
{
	assert(oneof2(node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE));
	GumboNode * new_node = static_cast<GumboNode *>(SAlloc::M(sizeof(GumboNode)));
	*new_node = *node;
	...
	ResetFlag(new_node->parse_flags, GUMBO_INSERTION_IMPLICIT_END_TAG);
	SetFlag(new_node->parse_flags, reason | GUMBO_INSERTION_BY_PARSER);
	...
	for(uint i = 0; i < old_attributes->length; ++i) {
		const GumboAttribute * old_attr = static_cast<const GumboAttribute *>(old_attributes->data[i]);
		GumboAttribute * attr = static_cast<GumboAttribute *>(SAlloc::M(sizeof(GumboAttribute)));
		*attr = *old_attr;
		attr->name = sstrdup(old_attr->name);
		attr->value = sstrdup(old_attr->value);
...
static void reconstruct_active_formatting_elements(GumboParser * parser) 
{
	...
	// Step 2 & 3
	uint i = elements->length - 1;
	GumboNode * element = static_cast<GumboNode *>(elements->data[i]);
	...
	for(; i < elements->length; ++i) {
		...
		GumboNode* clone = clone_node(parser, element, GUMBO_INSERTION_RECONSTRUCTED_FORMATTING_ELEMENT);
		// Step 9.
		InsertionLocation location = get_appropriate_insertion_location(parser, NULL);
...
static void clear_active_formatting_elements(GumboParser * parser) 
{
	...
	do {
		node = static_cast<GumboNode *>(gumbo_vector_pop(parser, elements));
...
static GumboQuirksModeEnum compute_quirks_mode(const GumboTokenDocType* doctype) 
{
	if(doctype->force_quirks || strcmp(doctype->name, kDoctypeHtml.data) ||
	    is_in_static_list(doctype->public_identifier, kQuirksModePublicIdPrefixes, false) ||
	    is_in_static_list(doctype->public_identifier, kQuirksModePublicIdExactMatches, true) ||
	    is_in_static_list(doctype->system_identifier, kQuirksModeSystemIdExactMatches, true) ||
	    (is_in_static_list(doctype->public_identifier, kLimitedQuirksRequiresSystemIdPublicIdPrefixes, false) &&
	    !doctype->has_system_identifier)) {
		return GUMBO_DOCTYPE_QUIRKS;
	}
	else if(is_in_static_list(doctype->public_identifier, kLimitedQuirksPublicIdPrefixes, false) ||
	    (is_in_static_list(doctype->public_identifier, kLimitedQuirksRequiresSystemIdPublicIdPrefixes, false) &&
	    doctype->has_system_identifier)) {
...
//
static bool FASTCALL has_an_element_in_specific_scope(GumboParser * parser, int expected_size, const GumboTag * expected, bool negate, const gumbo_tagset tags) 
{
	GumboVector * open_elements = &parser->_parser_state->_open_elements;
	for(int i = open_elements->length; --i >= 0;) {
		const GumboNode * node = static_cast<GumboNode *>(open_elements->data[i]);
		if(oneof2(node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE)) {
...
static bool FASTCALL has_open_element(GumboParser * parser, GumboTag tag) 
{
	GumboTagSet _ts;
	_ts.Append(_TAG(HTML));
	return has_an_element_in_specific_scope(parser, 1, &tag, false, _ts/*(gumbo_tagset){TAG(HTML)}*/);
}

// http://www.whatwg.org/specs/web-apps/current-work/multipage/parsing.html#has-an-element-in-scope
static bool FASTCALL has_an_element_in_scope(GumboParser * parser, GumboTag tag) 
{
	GumboTagSet _ts;
	_ts.Append(_TAG(APPLET)).Append(_TAG(CAPTION)).Append(_TAG(HTML)).Append(_TAG(TABLE)).
		Append(_TAG(TD)).Append(_TAG(TH)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT)).Append(_TAG(TEMPLATE)).
		Append(_TAG_MATHML(MI)).Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).
		Append(_TAG_MATHML(MTEXT)).Append(_TAG_MATHML(ANNOTATION_XML)).Append(_TAG_SVG(FOREIGNOBJECT)).
		Append(_TAG_SVG(DESC)).Append(_TAG_SVG(TITLE));
	return has_an_element_in_specific_scope(parser, 1, &tag, false,
		   _ts/*(gumbo_tagset){TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD),
...
static bool has_node_in_scope(GumboParser * parser, const GumboNode* node) 
{
	...
	GumboTagSet _ts;
	_ts.Append(_TAG(APPLET)).Append(_TAG(CAPTION)).Append(_TAG(HTML)).Append(_TAG(TABLE)).
		Append(_TAG(TD)).Append(_TAG(TH)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT)).
		Append(_TAG(TEMPLATE)).Append(_TAG_MATHML(MI)).Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).
		Append(_TAG_MATHML(MS)).Append(_TAG_MATHML(MTEXT)).Append(_TAG_MATHML(ANNOTATION_XML)).
		Append(_TAG_SVG(FOREIGNOBJECT)).Append(_TAG_SVG(DESC)).Append(_TAG_SVG(TITLE));
	for(int i = open_elements->length; --i >= 0;) {
		const GumboNode* current = static_cast<GumboNode *>(open_elements->data[i]);
		if(current == node) {
			return true;
		}
		if(!oneof2(current->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE)) {
			continue;
		}
		if(node_tag_in_set(current,
		    _ts/*(gumbo_tagset){TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE),
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(APPLET)).Append(_TAG(CAPTION)).Append(_TAG(HTML)).Append(_TAG(TABLE)).Append(_TAG(TD)).
		Append(_TAG(TH)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT)).Append(_TAG(TEMPLATE)).Append(_TAG_MATHML(MI)).
		Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).Append(_TAG_MATHML(MTEXT)).
		Append(_TAG_MATHML(ANNOTATION_XML)).Append(_TAG_SVG(FOREIGNOBJECT)).Append(_TAG_SVG(DESC)).Append(_TAG_SVG(TITLE));
	return has_an_element_in_specific_scope(parser, expected_len, expected, false,
		_ts/*(gumbo_tagset){TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD),
		TAG(TH), TAG(MARQUEE), TAG(OBJECT), TAG(TEMPLATE), TAG_MATHML(MI),
		TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT),
		TAG_MATHML(ANNOTATION_XML), TAG_SVG(FOREIGNOBJECT), TAG_SVG(DESC), TAG_SVG(TITLE)}*/);
}
...
static bool has_an_element_in_list_scope(GumboParser * parser, GumboTag tag) 
{
	...
	_ts.Append(_TAG(APPLET)).Append(_TAG(CAPTION)).Append(_TAG(HTML)).Append(_TAG(TABLE)).Append(_TAG(TD)).
		Append(_TAG(TH)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT)).Append(_TAG(TEMPLATE)).Append(_TAG_MATHML(MI)).
		Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).Append(_TAG_MATHML(MTEXT)).
		Append(_TAG_MATHML(ANNOTATION_XML)).Append(_TAG_SVG(FOREIGNOBJECT)).Append(_TAG_SVG(DESC)).
		Append(_TAG_SVG(TITLE)).Append(_TAG(OL)).Append(_TAG(UL));
	return has_an_element_in_specific_scope(parser, 1, &tag, false,
		_ts/*(gumbo_tagset){TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD),
		TAG(TH), TAG(MARQUEE), TAG(OBJECT), TAG(TEMPLATE), TAG_MATHML(MI),
		TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT),
...
static bool has_an_element_in_button_scope(GumboParser * parser, GumboTag tag) 
{
	GumboTagSet _ts;
	_ts.Append(_TAG(APPLET)).Append(_TAG(CAPTION)).Append(_TAG(HTML)).Append(_TAG(TABLE)).Append(_TAG(TD)).
		Append(_TAG(TH)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT)).Append(_TAG(TEMPLATE)).
		Append(_TAG_MATHML(MI)).Append(_TAG_MATHML(MO)).Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).
		Append(_TAG_MATHML(MTEXT)).Append(_TAG_MATHML(ANNOTATION_XML)).Append(_TAG_SVG(FOREIGNOBJECT)).
		Append(_TAG_SVG(DESC)).Append(_TAG_SVG(TITLE)).Append(_TAG(BUTTON));
	return has_an_element_in_specific_scope(parser, 1, &tag, false,
		_ts/*(gumbo_tagset){TAG(APPLET), TAG(CAPTION), TAG(HTML), TAG(TABLE), TAG(TD),
				  TAG(TH), TAG(MARQUEE), TAG(OBJECT), TAG(TEMPLATE), TAG_MATHML(MI),
				  TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT),
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(OPTGROUP)).Append(_TAG(OPTION));
	return has_an_element_in_specific_scope(parser, 1, &tag, true, _ts/*(gumbo_tagset){TAG(OPTGROUP), TAG(OPTION)}*/);
}
...
static void FASTCALL generate_implied_end_tags(GumboParser * parser, GumboTag exception) 
{
	...
	_ts.Append(_TAG(DD)).Append(_TAG(DT)).Append(_TAG(LI)).Append(_TAG(OPTION)).Append(_TAG(OPTGROUP)).
		Append(_TAG(P)).Append(_TAG(RP)).Append(_TAG(RB)).Append(_TAG(RT)).Append(_TAG(RTC));
	for(; node_tag_in_set(get_cur_node(parser),
	    _ts/*(gumbo_tagset){TAG(DD), TAG(DT), TAG(LI), TAG(OPTION), TAG(OPTGROUP), TAG(P), TAG(RP), TAG(RB), TAG(RT), TAG(RTC)}*/) &&
	    !node_html_tag_is(get_cur_node(parser), exception);
	    pop_current_node(parser))
		;
}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(CAPTION)).Append(_TAG(COLGROUP)).Append(_TAG(DD)).Append(_TAG(DT)).Append(_TAG(LI)).
		Append(_TAG(OPTION)).Append(_TAG(OPTGROUP)).Append(_TAG(P)).Append(_TAG(RP)).Append(_TAG(RT)).
		Append(_TAG(RTC)).Append(_TAG(TBODY)).Append(_TAG(TD)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(HEAD)).Append(_TAG(TR));
	for(; node_tag_in_set(get_cur_node(parser),
		_ts/*(gumbo_tagset){TAG(CAPTION), TAG(COLGROUP), TAG(DD), TAG(DT), TAG(LI),
...
static bool close_table_cell(GumboParser * parser, const GumboToken* token, GumboTag cell_tag) 
{
	...
	const GumboNode* node = get_cur_node(parser);
...
static bool is_special_node(const GumboNode* node) 
{
	assert(oneof2(node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE));
	GumboTagSet _ts;
	_ts.Append(_TAG(ADDRESS)).Append(_TAG(APPLET)).Append(_TAG(AREA)).Append(_TAG(ARTICLE)).Append(_TAG(ASIDE)).
		Append(_TAG(BASE)).Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(BLOCKQUOTE)).
		Append(_TAG(BODY)).Append(_TAG(BR)).Append(_TAG(BUTTON)).Append(_TAG(CAPTION)).Append(_TAG(CENTER)).
		Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(MENUITEM)).Append(_TAG(DD)).Append(_TAG(DETAILS)).
		Append(_TAG(DIR)).Append(_TAG(DIV)).Append(_TAG(DL)).Append(_TAG(DT)).Append(_TAG(EMBED)).Append(_TAG(FIELDSET)).
		Append(_TAG(FIGCAPTION)).Append(_TAG(FIGURE)).Append(_TAG(FOOTER)).Append(_TAG(FORM)).Append(_TAG(FRAME)).
		Append(_TAG(FRAMESET)).Append(_TAG(H1)).Append(_TAG(H2)).Append(_TAG(H3)).Append(_TAG(H4)).Append(_TAG(H5)).
		Append(_TAG(H6)).Append(_TAG(HEAD)).Append(_TAG(HEADER)).Append(_TAG(HGROUP)).Append(_TAG(HR)).Append(_TAG(HTML)).
		Append(_TAG(IFRAME)).Append(_TAG(IMG)).Append(_TAG(INPUT)).Append(_TAG(ISINDEX)).Append(_TAG(LI)).Append(_TAG(LINK)).
		Append(_TAG(LISTING)).Append(_TAG(MARQUEE)).Append(_TAG(MENU)).Append(_TAG(META)).Append(_TAG(NAV)).
		Append(_TAG(NOEMBED)).Append(_TAG(NOFRAMES)).Append(_TAG(NOSCRIPT)).Append(_TAG(OBJECT)).Append(_TAG(OL)).
		Append(_TAG(P)).Append(_TAG(PARAM)).Append(_TAG(PLAINTEXT)).Append(_TAG(PRE)).Append(_TAG(SCRIPT)).Append(_TAG(SECTION)).
		Append(_TAG(SELECT)).Append(_TAG(STYLE)).Append(_TAG(SUMMARY)).Append(_TAG(TABLE)).Append(_TAG(TBODY)).Append(_TAG(TD)).
		Append(_TAG(TEMPLATE)).Append(_TAG(TEXTAREA)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TITLE)).
		Append(_TAG(TR)).Append(_TAG(UL)).Append(_TAG(WBR)).Append(_TAG(XMP)).Append(_TAG_MATHML(MI)).Append(_TAG_MATHML(MO)).
		Append(_TAG_MATHML(MN)).Append(_TAG_MATHML(MS)).Append(_TAG_MATHML(MTEXT)).Append(_TAG_MATHML(ANNOTATION_XML)).
		Append(_TAG_SVG(FOREIGNOBJECT)).Append(_TAG_SVG(DESC));

	return node_tag_in_set(node,
		   _ts/*(gumbo_tagset){TAG(ADDRESS), TAG(APPLET), TAG(AREA), TAG(ARTICLE),
...
static bool implicitly_close_tags(GumboParser * parser, GumboToken* token, GumboNamespaceEnum target_ns, GumboTag target) 
{
	...
	if(!node_qualified_tag_is(get_cur_node(parser), target_ns, target)) {
		parser_add_parse_error(parser, token);
		while(!node_qualified_tag_is(get_cur_node(parser), target_ns, target)) {
			pop_current_node(parser);
		}
		result = false;
	}
	assert(node_qualified_tag_is(get_cur_node(parser), target_ns, target));
	pop_current_node(parser);
	return result;
}
...
static void maybe_implicitly_close_list_tag(GumboParser * parser, GumboToken* token, bool is_li) 
{
	GumboTagSet _ts;
	GumboTagSet _ts2;
	_ts.Append(_TAG(DD)).Append(_TAG(DT));
	_ts2.Append(_TAG(ADDRESS)).Append(_TAG(DIV)).Append(_TAG(P));
	GumboParserState * state = parser->_parser_state;
	state->_frameset_ok = false;
	for(int i = state->_open_elements.length; --i >= 0;) {
		const GumboNode * node = static_cast<const GumboNode *>(state->_open_elements.data[i]);
		bool is_list_tag = is_li ? node_html_tag_is(node, GUMBO_TAG_LI) : node_tag_in_set(node, _ts/*(gumbo_tagset){TAG(DD), TAG(DT)}*/);
		if(is_list_tag) {
			implicitly_close_tags(parser, token, node->v.element.tag_namespace, node->v.element.tag);
			return;
		}
		if(is_special_node(node) &&!node_tag_in_set(node, _ts2/*(gumbo_tagset){TAG(ADDRESS), TAG(DIV), TAG(P)}*/)) {
			return;
		}
	}
}
...
static void merge_attributes(GumboParser * parser, GumboToken* token, GumboNode* node) 
{
	...
	for(uint i = 0; i < token_attr->length; ++i) {
		GumboAttribute * attr = static_cast<GumboAttribute *>(token_attr->data[i]);
...
static void adjust_foreign_attributes(GumboParser * parser, GumboToken* token) 
{
	...
	for(size_t i = 0; i < sizeof(kForeignAttributeReplacements) / sizeof(NamespacedAttributeReplacement); ++i) {
...
static void FASTCALL remove_from_parent(GumboNode * node) 
{
	if(!node->parent) {
	...
	node->parent = NULL;
	node->index_within_parent = -1;
	for(uint i = index; i < children->length; ++i) {
...
static bool adoption_agency_algorithm(GumboParser * parser, GumboToken* token, GumboTag subject) 
{
	...
	// Step 1.
	GumboNode* current_node = get_cur_node(parser);
	if(current_node->v.element.tag_namespace == GUMBO_NAMESPACE_HTML && current_node->v.element.tag == subject &&
	...
	for(uint i = 0; i < 8; ++i) {
		...
		for(int j = state->_active_formatting_elements.length; --j >= 0;) {
			GumboNode * current_node = static_cast<GumboNode *>(state->_active_formatting_elements.data[j]);
			...
			if(node_html_tag_is(current_node, subject)) {
				...
				formatting_node_in_open_elements = gumbo_vector_index_of(&state->_open_elements, formatting_node);
				gumbo_debug("Formatting element of tag %s at %d.\n", gumbo_normalized_tagname(subject), formatting_node_in_open_elements);
				break;
			}
		}
		if(!formatting_node) {
		...
		// Step 8
		if(formatting_node != get_cur_node(parser)) {
		...
		for(uint j = formatting_node_in_open_elements; j < state->_open_elements.length; ++j) {
			assert(j > 0);
			GumboNode * current = static_cast<GumboNode *>(state->_open_elements.data[j]);
		...
		if(!furthest_block) {
			// Step 10.
			while(get_cur_node(parser) != formatting_node) {
				pop_current_node(parser);
			}
		...
		for(int j = 0;;) {
			...
			assert(node_index > 0);
			assert((uint)node_index < state->_open_elements.capacity);
			node = static_cast<GumboNode *>(state->_open_elements.data[node_index]);
			assert(node->parent);
			...
			if(j > 3 && formatting_index != -1) {
				...
				gumbo_vector_remove_at(formatting_index, &state->_active_formatting_elements);
			...
			if(last_node == furthest_block) {
				...
				assert((uint)bookmark <= state->_active_formatting_elements.length);
			}
			// Step 13.9.
			SetFlag(last_node->parse_flags, GUMBO_INSERTION_ADOPTION_AGENCY_MOVED);
		...
		// Step 14.
		gumbo_debug("Removing %s node from parent ", gumbo_normalized_tagname(last_node->v.element.tag));
		remove_from_parent(last_node);
		SetFlag(last_node->parse_flags, GUMBO_INSERTION_ADOPTION_AGENCY_MOVED);
		InsertionLocation location = get_appropriate_insertion_location(parser, common_ancestor);
		gumbo_debug("and inserting it into %s.\n", gumbo_normalized_tagname(location.target->v.element.tag));
		insert_node(last_node, location);
		// Step 15.
		GumboNode* new_formatting_node = clone_node(parser, formatting_node, GUMBO_INSERTION_ADOPTION_AGENCY_CLONED);
		SetFlag(formatting_node->parse_flags, GUMBO_INSERTION_IMPLICIT_END_TAG);
		...
		for(uint i = 0; i < temp.length; ++i) {
			GumboNode * child = static_cast<GumboNode *>(temp.data[i]);
		...
		// adjust the bookmark.
		int formatting_node_index = gumbo_vector_index_of(&state->_active_formatting_elements, formatting_node);
		...
		assert(bookmark >= 0);
		assert((uint)bookmark <= state->_active_formatting_elements.length);
		gumbo_vector_insert_at(new_formatting_node, bookmark, &state->_active_formatting_elements);
		// Step 19.
		gumbo_vector_remove(formatting_node, &state->_open_elements);
		int insert_at = gumbo_vector_index_of(&state->_open_elements, furthest_block) + 1;
		assert(insert_at >= 0);
		assert((uint)insert_at <= state->_open_elements.length);
...
static void FASTCALL ignore_token(GumboParser * parser) 
{
...
static void finish_parsing(GumboParser * parser) 
{
	...
	for(GumboNode* node = pop_current_node(parser); node;
	    node = pop_current_node(parser)) {
		if((node_html_tag_is(node, GUMBO_TAG_BODY) && state->_closed_body_tag) || (node_html_tag_is(node, GUMBO_TAG_HTML) && state->_closed_html_tag)) {
			continue;
		}
		SetFlag(node->parse_flags, GUMBO_INSERTION_IMPLICIT_END_TAG);
	}
...
static bool handle_before_html(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_DOCTYPE) {
		...
		GumboTagSet _ts;
		_ts.Append(_TAG(HEAD)).Append(_TAG(BODY)).Append(_TAG(HTML)).Append(_TAG(BR));
		if(token->type == GUMBO_TOKEN_END_TAG && !tag_in(token, false,
			_ts/*(gumbo_tagset){TAG(HEAD), TAG(BODY), TAG(HTML), TAG(BR)}*/)) {
			parser_add_parse_error(parser, token);
			ignore_token(parser);
			return false;
		}
...
static bool handle_before_head(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_DOCTYPE) {
		...
		append_comment_node(parser, get_cur_node(parser), token);
		return true;
	}
	else if(token->type == GUMBO_TOKEN_WHITESPACE) {
		ignore_token(parser);
		...
		if(token->type == GUMBO_TOKEN_END_TAG && !tag_in(token, false,
			...
			GumboNode* node = insert_element_of_tag_type(parser, GUMBO_TAG_HEAD, GUMBO_INSERTION_IMPLIED);
			set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_HEAD);
			parser->_parser_state->_head_element = node;
			parser->_parser_state->_reprocess_current_token = true;
			return true;
...
static bool handle_in_head(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_WHITESPACE) {
		...
		GumboTagSet _ts2;
		_ts2.Append(_TAG(BASE)).Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(MENUITEM)).Append(_TAG(LINK));
		if(tag_in(token, kStartTag,
			_ts2/*(gumbo_tagset){TAG(BASE), TAG(BASEFONT), TAG(BGSOUND), TAG(MENUITEM), TAG(LINK)}*/)) {
			insert_element_from_token(parser, token);
			pop_current_node(parser);
			...
			GumboTagSet _ts3;
			_ts3.Append(_TAG(NOFRAMES)).Append(_TAG(STYLE));
			if(tag_in(token, kStartTag, _ts3/*(gumbo_tagset){TAG(NOFRAMES), TAG(STYLE)}*/)) {
				...
				GumboTagSet _ts;
				_ts.Append(_TAG(BODY)).Append(_TAG(HTML)).Append(_TAG(BR));
				if(tag_in(token, kEndTag, _ts/*(gumbo_tagset){TAG(BODY), TAG(HTML), TAG(BR)}*/)) {
					pop_current_node(parser);
					set_insertion_mode(parser, GUMBO_INSERTION_MODE_AFTER_HEAD);
					parser->_parser_state->_reprocess_current_token = true;
					return true;
					...
					bool success = true;
					if(!node_html_tag_is(get_cur_node(parser), GUMBO_TAG_TEMPLATE)) {
						parser_add_parse_error(parser, token);
						success = false;
					}
...
static bool handle_in_head_noscript(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_DOCTYPE) {
		...
		GumboTagSet _ts;
		_ts.Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(LINK)).Append(_TAG(META)).Append(_TAG(NOFRAMES)).Append(_TAG(STYLE));
		if(oneof2(token->type, GUMBO_TOKEN_WHITESPACE, GUMBO_TOKEN_COMMENT) ||
			tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(BASEFONT), TAG(BGSOUND), TAG(LINK), TAG(META), TAG(NOFRAMES), TAG(STYLE)}*/)) {
			return handle_in_head(parser, token);
		}
		else {
			GumboTagSet _ts2;
			_ts2.Append(_TAG(HEAD)).Append(_TAG(NOSCRIPT));
			if(tag_in(token, kStartTag, _ts2/*(gumbo_tagset){TAG(HEAD), TAG(NOSCRIPT)}*/) ||
				(token->type == GUMBO_TOKEN_END_TAG && !tag_is(token, kEndTag, GUMBO_TAG_BR))) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
			}
...
static bool handle_after_head(GumboParser * parser, GumboToken* token) 
{
	...
	if(token->type == GUMBO_TOKEN_WHITESPACE) {
		...
		GumboTagSet _ts2;
		_ts2.Append(_TAG(BASE)).Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(LINK)).Append(_TAG(META)).
			Append(_TAG(NOFRAMES)).Append(_TAG(SCRIPT)).Append(_TAG(STYLE)).Append(_TAG(TEMPLATE)).Append(_TAG(TITLE));
		if(tag_in(token, kStartTag,
			...
			GumboTagSet _ts;
			_ts.Append(_TAG(BODY)).Append(_TAG(HTML)).Append(_TAG(BR));
			if(tag_is(token, kStartTag, GUMBO_TAG_HEAD) || (token->type == GUMBO_TOKEN_END_TAG &&
				!tag_in(token, kEndTag, _ts/*(gumbo_tagset){TAG(BODY), TAG(HTML), TAG(BR)}*/))) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
			}
...
{
	GumboParserState * state = parser->_parser_state;
	GumboTagSet _ts_dddt;
	_ts_dddt.Append(_TAG(DD)).Append(_TAG(DT));
	GumboTagSet _ts_h;
	_ts_h.Append(_TAG(H1)).Append(_TAG(H2)).Append(_TAG(H3)).Append(_TAG(H4)).Append(_TAG(H5)).Append(_TAG(H6));
	assert(state->_open_elements.length > 0);
	if(token->type == GUMBO_TOKEN_NULL) {
	...
	else if(oneof2(token->type, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_CDATA)) {
		reconstruct_active_formatting_elements(parser);
		insert_text_token(parser, token);
		...
		GumboTagSet _ts14;
		_ts14.Append(_TAG(BASE)).Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(MENUITEM)).
			Append(_TAG(LINK)).Append(_TAG(META)).Append(_TAG(NOFRAMES)).Append(_TAG(SCRIPT)).
			Append(_TAG(STYLE)).Append(_TAG(TEMPLATE)).Append(_TAG(TITLE));
		if(tag_in(token, kStartTag, _ts14/*(gumbo_tagset){TAG(BASE), TAG(BASEFONT), TAG(BGSOUND),
			TAG(MENUITEM), TAG(LINK), TAG(META), TAG(NOFRAMES), TAG(SCRIPT), TAG(STYLE), TAG(TEMPLATE), TAG(TITLE)}*/) ||
			tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
			return handle_in_head(parser, token);
		}
		else if(tag_is(token, kStartTag, GUMBO_TAG_BODY)) {
			parser_add_parse_error(parser, token);
			if(state->_open_elements.length < 2 || !node_html_tag_is(static_cast<const GumboNode *>(state->_open_elements.data[1]), GUMBO_TAG_BODY) ||
				has_open_element(parser, GUMBO_TAG_TEMPLATE)) {
				ignore_token(parser);
				return false;
			}
			state->_frameset_ok = false;
			merge_attributes(parser, token, static_cast<GumboNode *>(state->_open_elements.data[1]));
			return false;
		}
		else if(tag_is(token, kStartTag, GUMBO_TAG_FRAMESET)) {
			parser_add_parse_error(parser, token);
			if(state->_open_elements.length < 2 || !node_html_tag_is(static_cast<const GumboNode *>(state->_open_elements.data[1]), GUMBO_TAG_BODY) || !state->_frameset_ok) {
				ignore_token(parser);
				return false;
			}
			// Save the body node for later removal.
			GumboNode * body_node = static_cast<GumboNode *>(state->_open_elements.data[1]);
			...
			GumboTagSet _ts;
			_ts.Append(_TAG(DD)).Append(_TAG(DT)).Append(_TAG(LI)).Append(_TAG(P)).Append(_TAG(TBODY)).Append(_TAG(TD)).
				Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR)).Append(_TAG(BODY)).Append(_TAG(HTML));
			for(uint i = 0; i < state->_open_elements.length; ++i) {
				if(!node_tag_in_set(static_cast<const GumboNode *>(state->_open_elements.data[i]),
					_ts/*(gumbo_tagset){TAG(DD), TAG(DT), TAG(LI), TAG(P), TAG(TBODY),
						   TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR), TAG(BODY), TAG(HTML)}*/)) {
					parser_add_parse_error(parser, token);
				}
			}
			...
			GumboTagSet _ts13;
			_ts13.Append(_TAG(BODY)).Append(_TAG(HTML));
			if(tag_in(token, kEndTag, _ts13/*(gumbo_tagset){TAG(BODY), TAG(HTML)}*/)) {
				...
				bool success = true;
				GumboTagSet _ts3;
				_ts3.Append(_TAG(DD)).Append(_TAG(DT)).Append(_TAG(LI)).Append(_TAG(OPTGROUP)).Append(_TAG(OPTION)).
					Append(_TAG(P)).Append(_TAG(RB)).Append(_TAG(RP)).Append(_TAG(RT)).Append(_TAG(RTC)).Append(_TAG(TBODY)).
					Append(_TAG(TD)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR)).Append(_TAG(BODY)).Append(_TAG(HTML));
				for(uint i = 0; i < state->_open_elements.length; ++i) {
					if(!node_tag_in_set(static_cast<const GumboNode *>(state->_open_elements.data[i]), _ts3
				...
				if(tag_is(token, kEndTag, GUMBO_TAG_HTML)) {
					...
					GumboNode * body = static_cast<GumboNode *>(state->_open_elements.data[1]);
				...
				GumboTagSet _ts12;
				_ts12.Append(_TAG(ADDRESS)).Append(_TAG(ARTICLE)).Append(_TAG(ASIDE)).Append(_TAG(BLOCKQUOTE)).Append(_TAG(CENTER)).
					Append(_TAG(DETAILS)).Append(_TAG(DIR)).Append(_TAG(DIV)).Append(_TAG(DL)).Append(_TAG(FIELDSET)).
					Append(_TAG(FIGCAPTION)).Append(_TAG(FIGURE)).Append(_TAG(FOOTER)).Append(_TAG(HEADER)).Append(_TAG(HGROUP)).
					Append(_TAG(MENU)).Append(_TAG(MAIN)).Append(_TAG(NAV)).Append(_TAG(OL)).Append(_TAG(P)).Append(_TAG(SECTION)).Append(_TAG(SUMMARY)).Append(_TAG(UL));
				if(tag_in(token, kStartTag, _ts12/*(gumbo_tagset){TAG(ADDRESS), TAG(ARTICLE), TAG(ASIDE),
				...
				else if(tag_in(token, kStartTag, _ts_h/*(gumbo_tagset){TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)}*/)) {
					bool result = maybe_implicitly_close_p_tag(parser, token);
					if(node_tag_in_set(get_cur_node(parser), _ts_h/*(gumbo_tagset){TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)}*/)) {
						parser_add_parse_error(parser, token);
						pop_current_node(parser);
						result = false;
					}
					...
					GumboTagSet _ts11;
					_ts11.Append(_TAG(PRE)).Append(_TAG(LISTING));
					if(tag_in(token, kStartTag, _ts11/*(gumbo_tagset){TAG(PRE), TAG(LISTING)}*/)) {
						bool result = maybe_implicitly_close_p_tag(parser, token);
						insert_element_from_token(parser, token);
					...
					else if(tag_in(token, kStartTag, _ts_dddt/*(gumbo_tagset){TAG(DD), TAG(DT)}*/)) {
						...
						GumboTagSet _ts9;
						_ts9.Append(_TAG(ADDRESS)).Append(_TAG(ARTICLE)).Append(_TAG(ASIDE)).Append(_TAG(BLOCKQUOTE)).Append(_TAG(BUTTON)).
							Append(_TAG(CENTER)).Append(_TAG(DETAILS)).Append(_TAG(DIR)).Append(_TAG(DIV)).Append(_TAG(DL)).
							Append(_TAG(FIELDSET)).Append(_TAG(FIGCAPTION)).Append(_TAG(FIGURE)).Append(_TAG(FOOTER)).Append(_TAG(HEADER)).
							Append(_TAG(HGROUP)).Append(_TAG(LISTING)).Append(_TAG(MAIN)).Append(_TAG(MENU)).Append(_TAG(NAV)).
							Append(_TAG(OL)).Append(_TAG(PRE)).Append(_TAG(SECTION)).Append(_TAG(SUMMARY)).Append(_TAG(UL));
						if(tag_in(token, kEndTag, _ts9/*(gumbo_tagset){TAG(ADDRESS), TAG(ARTICLE), TAG(ASIDE),
							   TAG(BLOCKQUOTE), TAG(BUTTON), TAG(CENTER), TAG(DETAILS), TAG(DIR), TAG(DIV), TAG(DL), TAG(FIELDSET),
							   TAG(FIGCAPTION), TAG(FIGURE), TAG(FOOTER), TAG(HEADER), TAG(HGROUP), TAG(LISTING), TAG(MAIN), TAG(MENU), TAG(NAV),
							...
							if(has_open_element(parser, GUMBO_TAG_TEMPLATE)) {
								...
								bool success = true;
								generate_implied_end_tags(parser, GUMBO_TAG_LAST);
								if(!node_html_tag_is(get_cur_node(parser), GUMBO_TAG_FORM)) {
									parser_add_parse_error(parser, token);
									return false;
								}
							...
							if(!has_an_element_in_button_scope(parser, GUMBO_TAG_P)) {
								parser_add_parse_error(parser, token);
								// reconstruct_active_formatting_elements(parser);
								insert_element_of_tag_type(parser, GUMBO_TAG_P, GUMBO_INSERTION_CONVERTED_FROM_END_TAG);
								state->_reprocess_current_token = true;
								return false;
							}
						...
						else if(tag_in(token, kEndTag, _ts_dddt/*(gumbo_tagset){TAG(DD), TAG(DT)}*/)) {
						...
						else if(tag_in(token, kEndTag, _ts_h/*(gumbo_tagset){TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)}*/)) {
							const GumboTag _tl_h[] = {
								GUMBO_TAG_H1, GUMBO_TAG_H2, GUMBO_TAG_H3, GUMBO_TAG_H4, GUMBO_TAG_H5, GUMBO_TAG_H6
							};
							if(!has_an_element_in_scope_with_tagname(parser, SIZEOFARRAY(_tl_h), _tl_h/*(GumboTag[]){GUMBO_TAG_H1, GUMBO_TAG_H2, GUMBO_TAG_H3,
								...
								generate_implied_end_tags(parser, GUMBO_TAG_LAST);
								const GumboNode * current_node = get_cur_node(parser);
								bool success = node_html_tag_is(current_node, token->v.end_tag);
								if(!success) {
									// There're children of the heading currently open; close them below and record a parse error.
									// TODO(jdtang): Add a way to distinguish this error case from the one above.
									parser_add_parse_error(parser, token);
								}
								do {
									current_node = pop_current_node(parser);
								} while(!node_tag_in_set(current_node, _ts_h/*(gumbo_tagset){TAG(H1), TAG(H2), TAG(H3), TAG(H4), TAG(H5), TAG(H6)}*/));
								return success;
							}
						}
							...
							if(has_matching_a) {
								...
								if(find_last_anchor_index(parser, &last_a)) {
									void * last_element = gumbo_vector_remove_at(last_a, &state->_active_formatting_elements);
							...
							GumboTagSet _ts7;
							_ts7.Append(_TAG(B)).Append(_TAG(BIG)).Append(_TAG(CODE)).Append(_TAG(EM)).Append(_TAG(FONT)).Append(_TAG(I)).
								Append(_TAG(S)).Append(_TAG(SMALL)).Append(_TAG(STRIKE)).Append(_TAG(STRONG)).Append(_TAG(TT)).Append(_TAG(U));
							if(tag_in(token, kStartTag, _ts7/*(gumbo_tagset){TAG(B), TAG(BIG), TAG(CODE), TAG(EM), TAG(FONT),
								...
								insert_element_from_token(parser, token);
								add_formatting_element(parser, get_cur_node(parser));
								return result;
							}
							else {
								GumboTagSet _ts6;
								_ts6.Append(_TAG(A)).Append(_TAG(B)).Append(_TAG(BIG)).Append(_TAG(CODE)).Append(_TAG(EM)).Append(_TAG(FONT)).
									Append(_TAG(I)).Append(_TAG(NOBR)).Append(_TAG(S)).Append(_TAG(SMALL)).Append(_TAG(STRIKE)).Append(_TAG(STRONG)).
									Append(_TAG(TT)).Append(_TAG(U));
								if(tag_in(token, kEndTag, _ts6/*(gumbo_tagset){TAG(A), TAG(B), TAG(BIG), TAG(CODE), TAG(EM),
									TAG(FONT), TAG(I), TAG(NOBR), TAG(S), TAG(SMALL), TAG(STRIKE), TAG(STRONG), TAG(TT), TAG(U)}*/)) {
									return adoption_agency_algorithm(parser, token, token->v.end_tag);
								}
								else {
									GumboTagSet _ts;
									_ts.Append(_TAG(APPLET)).Append(_TAG(MARQUEE)).Append(_TAG(OBJECT));
									if(tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(APPLET), TAG(MARQUEE), TAG(OBJECT)}*/)) {
										reconstruct_active_formatting_elements(parser);
										insert_element_from_token(parser, token);
										add_formatting_element(parser, &kActiveFormattingScopeMarker);
										set_frameset_not_ok(parser);
									...
									else if(tag_in(token, kEndTag, _ts/*(gumbo_tagset){TAG(APPLET), TAG(MARQUEE), TAG(OBJECT)}*/)) {
										GumboTag token_tag = token->v.end_tag;
										...
										GumboTagSet _ts5;
										_ts5.Append(_TAG(AREA)).Append(_TAG(BR)).Append(_TAG(EMBED)).Append(_TAG(IMG)).Append(_TAG(IMAGE)).Append(_TAG(KEYGEN)).Append(_TAG(WBR));
										if(tag_in(token, kStartTag, _ts5/*(gumbo_tagset){TAG(AREA), TAG(BR), TAG(EMBED), TAG(IMG), TAG(IMAGE), TAG(KEYGEN), TAG(WBR)}*/)) {
											bool success = true;
											...
											if(tag_is(token, kStartTag, GUMBO_TAG_IMAGE)) {
												...
												SetFlag(node->parse_flags, GUMBO_INSERTION_FROM_IMAGE);
											}
											pop_current_node(parser);
											acknowledge_self_closing_tag(parser);
											set_frameset_not_ok(parser);
											...
											GumboTagSet _ts8;
											_ts8.Append(_TAG(PARAM)).Append(_TAG(SOURCE)).Append(_TAG(TRACK));
											if(tag_in(token, kStartTag, _ts8/*(gumbo_tagset){TAG(PARAM), TAG(SOURCE), TAG(TRACK)}*/)) {
												insert_element_from_token(parser, token);
												pop_current_node(parser);
												acknowledge_self_closing_tag(parser);
												return true;
												...
												parser_add_parse_error(parser, token);
												if(parser->_parser_state->_form_element && !has_open_element(parser, GUMBO_TAG_TEMPLATE)) {
													ignore_token(parser);
													return false;
												}
												acknowledge_self_closing_tag(parser);
												...
												GumboNode * form = insert_element_of_tag_type(parser, GUMBO_TAG_FORM, GUMBO_INSERTION_FROM_ISINDEX);
												if(!has_open_element(parser, GUMBO_TAG_TEMPLATE)) {
												...
												insert_element_of_tag_type(parser, GUMBO_TAG_HR, GUMBO_INSERTION_FROM_ISINDEX);
												pop_current_node(parser); // <hr>
												insert_element_of_tag_type(parser, GUMBO_TAG_LABEL, GUMBO_INSERTION_FROM_ISINDEX);
												...
												if(prompt_attr) {
													...
													text_state->_buffer.data = sstrdup(prompt_attr->value);
													...
													GumboStringPiece prompt_text = GUMBO_STRING("This is a searchable index. Enter search keywords: ");
													gumbo_string_buffer_append_string(&prompt_text, &text_state->_buffer);
												}
												GumboNode * input = insert_element_of_tag_type(parser, GUMBO_TAG_INPUT, GUMBO_INSERTION_FROM_ISINDEX);
												for(uint i = 0; i < token_attrs->length; ++i) {
													GumboAttribute * attr = static_cast<GumboAttribute*>(token_attrs->data[i]);
												...
												// when copying over.  The action attribute should be kept since it's moved to the form.
												gumbo_destroy_attribute(name_attr);
												GumboAttribute * name = static_cast<GumboAttribute *>(SAlloc::M(sizeof(GumboAttribute)));
												...
												name->name = sstrdup("name");
												name->value = sstrdup("isindex");
												...
												insert_element_from_token(parser, token);
												set_frameset_not_ok(parser);
												const GumboInsertionMode state = parser->_parser_state->_insertion_mode;
												if(oneof5(state, GUMBO_INSERTION_MODE_IN_TABLE, GUMBO_INSERTION_MODE_IN_CAPTION, 
													GUMBO_INSERTION_MODE_IN_TABLE_BODY, GUMBO_INSERTION_MODE_IN_ROW, GUMBO_INSERTION_MODE_IN_CELL)) {
												...
												GumboTagSet _ts10;
												_ts10.Append(_TAG(OPTION)).Append(_TAG(OPTGROUP));
												if(tag_in(token, kStartTag, _ts10/*(gumbo_tagset){TAG(OPTION), TAG(OPTGROUP)}*/)) {
													if(node_html_tag_is(get_cur_node(parser), GUMBO_TAG_OPTION)) {
														pop_current_node(parser);
													}
													reconstruct_active_formatting_elements(parser);
													insert_element_from_token(parser, token);
													...
													GumboTagSet _ts4;
													_ts4.Append(_TAG(RB)).Append(_TAG(RP)).Append(_TAG(RT)).Append(_TAG(RTC));
													if(tag_in(token, kStartTag, _ts4/*(gumbo_tagset){TAG(RB), TAG(RP), TAG(RT), TAG(RTC)}*/)) {
														bool success = true;
														GumboTagSet _ts_rtrp;
														_ts_rtrp.Append(_TAG(RT)).Append(_TAG(RP));
														GumboTag exception = tag_in(token, kStartTag, _ts_rtrp/*(gumbo_tagset){TAG(RT), TAG(RP)}*/) ? GUMBO_TAG_RTC : GUMBO_TAG_LAST;
														if(has_an_element_in_scope(parser, GUMBO_TAG_RUBY)) {
															generate_implied_end_tags(parser, exception);
														}
														if(!node_html_tag_is(get_cur_node(parser), GUMBO_TAG_RUBY) && !(exception == GUMBO_TAG_LAST ||
															node_html_tag_is(get_cur_node(parser), GUMBO_TAG_RTC))) {
															parser_add_parse_error(parser, token);
															success = false;
														}
														insert_element_from_token(parser, token);
														...
														parser_add_parse_error(parser, token);
														reconstruct_active_formatting_elements(parser);
														insert_element_of_tag_type(parser, GUMBO_TAG_BR, GUMBO_INSERTION_CONVERTED_FROM_END_TAG);
														pop_current_node(parser);
														return false;
													}
														...
														GumboTagSet _ts2;
														_ts2.Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(FRAME)).Append(_TAG(HEAD)).
															Append(_TAG(TBODY)).Append(_TAG(TD)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR));
														if(tag_in(token, kStartTag,
															_ts2/*(gumbo_tagset){TAG(CAPTION), TAG(COL), TAG(COLGROUP),
																	TAG(FRAME), TAG(HEAD), TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)}*/)) {
															parser_add_parse_error(parser, token);
															ignore_token(parser);
															return false;
														}
...
{
	if(oneof2(token->type, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_WHITESPACE)) {
		insert_text_token(parser, token);
	}
	else {
...
static bool handle_in_table(GumboParser * parser, GumboToken* token) 
{
	...
	if(oneof2(token->type, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_WHITESPACE)) {
		...
		clear_stack_to_table_context(parser);
		insert_element_of_tag_type(parser, GUMBO_TAG_COLGROUP, GUMBO_INSERTION_IMPLIED);
		parser->_parser_state->_reprocess_current_token = true;
		set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_COLUMN_GROUP);
		return true;
	}
		...
		GumboTagSet _ts3;
		_ts3.Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TD)).Append(_TAG(TH)).Append(_TAG(TR));
		if(tag_in(token, kStartTag, _ts3/*(gumbo_tagset){TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TD), TAG(TH), TAG(TR)}*/)) {
			clear_stack_to_table_context(parser);
			set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE_BODY);
			GumboTagSet _ts4;
			_ts4.Append(_TAG(TD)).Append(_TAG(TH)).Append(_TAG(TR));
			if(tag_in(token, kStartTag, _ts4/*(gumbo_tagset){TAG(TD), TAG(TH), TAG(TR)}*/)) {
				insert_element_of_tag_type(parser, GUMBO_TAG_TBODY, GUMBO_INSERTION_IMPLIED);
				state->_reprocess_current_token = true;
			}
			else {
				insert_element_from_token(parser, token);
			...
			GumboTagSet _ts2;
			_ts2.Append(_TAG(BODY)).Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(HTML)).
				Append(_TAG(TBODY)).Append(_TAG(TD)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR));
			if(tag_in(token, kEndTag, _ts2/*(gumbo_tagset){TAG(BODY), TAG(CAPTION), TAG(COL),
				TAG(COLGROUP), TAG(HTML), TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)}*/)) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
			}
...
static bool handle_in_table_text(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_NULL) {
	...
	else if(oneof2(token->type, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_WHITESPACE)) {
		insert_text_token(parser, token);
		return true;
	}
	else {
		...
		for(uint i = 0; i < buffer->length; ++i) {
			if(!isspace((uchar)buffer->data[i]) || buffer->data[i] == '\v') {
				state->_foster_parent_insertions = true;
				reconstruct_active_formatting_elements(parser);
				break;
			}
...
static bool handle_in_caption(GumboParser * parser, GumboToken* token)
{
	if(tag_is(token, kEndTag, GUMBO_TAG_CAPTION)) {
		if(!has_an_element_in_table_scope(parser, GUMBO_TAG_CAPTION)) {
			...
			generate_implied_end_tags(parser, GUMBO_TAG_LAST);
			bool result = true;
			if(!node_html_tag_is(get_cur_node(parser), GUMBO_TAG_CAPTION)) {
				parser_add_parse_error(parser, token);
			}
		...
		GumboTagSet _ts2;
		_ts2.Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(TBODY)).Append(_TAG(TD)).
			Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR));
		if(tag_in(token, kStartTag, _ts2/*(gumbo_tagset){TAG(CAPTION), TAG(COL), TAG(COLGROUP),
			TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)}*/) || (tag_is(token, kEndTag, GUMBO_TAG_TABLE))) {
			if(!has_an_element_in_table_scope(parser, GUMBO_TAG_CAPTION)) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
			...
			GumboTagSet _ts;
			_ts.Append(_TAG(BODY)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(HTML)).Append(_TAG(TBODY)).
				Append(_TAG(TD)).Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR));
			if(tag_in(token, kEndTag, _ts/*(gumbo_tagset){TAG(BODY), TAG(COL), TAG(COLGROUP), TAG(HTML),
				TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)}*/)) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
...
static bool handle_in_column_group(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_WHITESPACE) {
		...
		if(!node_html_tag_is(get_cur_node(parser), GUMBO_TAG_COLGROUP)) {
			parser_add_parse_error(parser, token);
			ignore_token(parser);
			return false;
		}
	...
	else if(tag_is(token, kStartTag, GUMBO_TAG_TEMPLATE) || tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
		return handle_in_head(parser, token);
	}
	else if(token->type == GUMBO_TOKEN_EOF) {
		return handle_in_body(parser, token);
...
static bool handle_in_table_body(GumboParser * parser, GumboToken* token) 
{
	if(tag_is(token, kStartTag, GUMBO_TAG_TR)) {
		...
		GumboTagSet _ts;
		_ts.Append(_TAG(TD)).Append(_TAG(TH));
		if(tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(TD), TAG(TH)}*/)) {
			parser_add_parse_error(parser, token);
			clear_stack_to_table_body_context(parser);
			...
			GumboTagSet _ts2;
			_ts2.Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD));
			if(tag_in(token, kEndTag, _ts2/*(gumbo_tagset){TAG(TBODY), TAG(TFOOT), TAG(THEAD)}*/)) {
				...
				GumboTagSet _ts3;
				_ts3.Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD));
				if(tag_in(token, kStartTag,
					_ts3/*(gumbo_tagset){TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD)}*/) ||
					...
					GumboTagSet _ts4;
					_ts4.Append(_TAG(BODY)).Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(TR)).Append(_TAG(COLGROUP)).
						Append(_TAG(HTML)).Append(_TAG(TD)).Append(_TAG(TH));
					if(tag_in(token, kEndTag,
						_ts4/*(gumbo_tagset){TAG(BODY), TAG(CAPTION), TAG(COL), TAG(TR), TAG(COLGROUP), TAG(HTML), TAG(TD), TAG(TH)}*/)) {
						parser_add_parse_error(parser, token);
						ignore_token(parser);
						return false;
					}
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(TH)).Append(_TAG(TD));
	if(tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(TH), TAG(TD)}*/)) {
		...
		GumboTagSet _ts2;
		_ts2.Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TR));
		if(tag_in(token, kStartTag, _ts2/*(gumbo_tagset){TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)}*/) ||
			tag_is(token, kEndTag, GUMBO_TAG_TABLE)) {
			if(!has_an_element_in_table_scope(parser, GUMBO_TAG_TR)) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
			...
			GumboTagSet _ts3;
			_ts3.Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD));
			if(tag_in(token, kEndTag, _ts3/*(gumbo_tagset){TAG(TBODY), TAG(TFOOT), TAG(THEAD)}*/)) {
				if(!has_an_element_in_table_scope(parser, token->v.end_tag) || (!has_an_element_in_table_scope(parser, GUMBO_TAG_TR))) {
					parser_add_parse_error(parser, token);
					ignore_token(parser);
					return false;
				}
				...
				GumboTagSet _ts4;
				_ts4.Append(_TAG(BODY)).Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(HTML)).Append(_TAG(TD)).Append(_TAG(TH));
				if(tag_in(token, kEndTag, _ts4/*(gumbo_tagset){TAG(BODY), TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(HTML), TAG(TD), TAG(TH)}*/)) {
					parser_add_parse_error(parser, token);
					ignore_token(parser);
					return false;
				}
...
static bool handle_in_cell(GumboParser * parser, GumboToken* token) 
{
	GumboTagSet _ts;
	_ts.Append(_TAG(TD)).Append(_TAG(TH));
	if(tag_in(token, kEndTag, _ts/*(gumbo_tagset){TAG(TD), TAG(TH)}*/)) {
		GumboTag token_tag = token->v.end_tag;
		if(!has_an_element_in_table_scope(parser, token_tag)) {
			parser_add_parse_error(parser, token);
			ignore_token(parser);
		...
		_ts2.Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(TBODY)).Append(_TAG(TD)).
			Append(_TAG(TFOOT)).Append(_TAG(TH)).Append(_TAG(THEAD)).Append(_TAG(TR));
		if(tag_in(token, kStartTag,
			_ts2/*(gumbo_tagset){TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(TBODY), TAG(TD), TAG(TFOOT), TAG(TH), TAG(THEAD), TAG(TR)}*/)) {
			gumbo_debug("Handling <td> in cell.\n");
			if(!has_an_element_in_table_scope(parser, GUMBO_TAG_TH) && !has_an_element_in_table_scope(parser, GUMBO_TAG_TD)) {
			...
			GumboTagSet _ts3;
			_ts3.Append(_TAG(BODY)).Append(_TAG(CAPTION)).Append(_TAG(COL)).Append(_TAG(COLGROUP)).Append(_TAG(HTML));
			if(tag_in(token, kEndTag, _ts3/*(gumbo_tagset){TAG(BODY), TAG(CAPTION), TAG(COL), TAG(COLGROUP), TAG(HTML)}*/)) {
				parser_add_parse_error(parser, token);
				ignore_token(parser);
				return false;
			}
				...
				GumboTagSet _ts4;
				_ts4.Append(_TAG(TABLE)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TR));
				if(tag_in(token, kEndTag, _ts4/*(gumbo_tagset){TAG(TABLE), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR)}*/)) {
					if(!has_an_element_in_table_scope(parser, token->v.end_tag)) {
						parser_add_parse_error(parser, token);
						ignore_token(parser);
						return false;
...
static bool handle_in_select(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_NULL) {
		...
		if(node_html_tag_is(get_cur_node(parser), GUMBO_TAG_OPTGROUP)) {
			pop_current_node(parser);
		}
		insert_element_from_token(parser, token);
		return true;
		...
		GumboVector* open_elements = &parser->_parser_state->_open_elements;
		if(node_html_tag_is(get_cur_node(parser), GUMBO_TAG_OPTION) && 
			node_html_tag_is(static_cast<GumboNode *>(open_elements->data[open_elements->length - 2]), GUMBO_TAG_OPTGROUP)) {
			pop_current_node(parser);
		}
		if(node_html_tag_is(get_cur_node(parser), GUMBO_TAG_OPTGROUP)) {
			pop_current_node(parser);
		...
		GumboTagSet _ts;
		_ts.Append(_TAG(INPUT)).Append(_TAG(KEYGEN)).Append(_TAG(TEXTAREA));
		if(tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(INPUT), TAG(KEYGEN), TAG(TEXTAREA)}*/)) {
			parser_add_parse_error(parser, token);
			if(!has_an_element_in_select_scope(parser, GUMBO_TAG_SELECT)) {
				ignore_token(parser);
			}
			...
			GumboTagSet _ts2;
			_ts2.Append(_TAG(SCRIPT)).Append(_TAG(TEMPLATE));
			if(tag_in(token, kStartTag, _ts2/*(gumbo_tagset){TAG(SCRIPT), TAG(TEMPLATE)}*/) || tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
				return handle_in_head(parser, token);
			}
			else if(token->type == GUMBO_TOKEN_EOF) {
				return handle_in_body(parser, token);
...
{
	GumboTagSet _ts;
	_ts.Append(_TAG(CAPTION)).Append(_TAG(TABLE)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD)).Append(_TAG(TR)).Append(_TAG(TD)).Append(_TAG(TH));
	if(tag_in(token, kStartTag,
	    _ts/*(gumbo_tagset){TAG(CAPTION), TAG(TABLE), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TR), TAG(TD), TAG(TH)}*/)) {
		parser_add_parse_error(parser, token);
		close_current_select(parser);
		parser->_parser_state->_reprocess_current_token = true;
		return false;
...
{
	GumboParserState * state = parser->_parser_state;
	if(oneof5(token->type, GUMBO_TOKEN_WHITESPACE, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_COMMENT, GUMBO_TOKEN_NULL, GUMBO_TOKEN_DOCTYPE)) {
		return handle_in_body(parser, token);
	}
	else {
		GumboTagSet _ts3;
		_ts3.Append(_TAG(BASE)).Append(_TAG(BASEFONT)).Append(_TAG(BGSOUND)).Append(_TAG(LINK)).Append(_TAG(META)).
			Append(_TAG(NOFRAMES)).Append(_TAG(SCRIPT)).Append(_TAG(STYLE)).Append(_TAG(TEMPLATE)).Append(_TAG(TITLE));
		if(tag_in(token, kStartTag, _ts3/*(gumbo_tagset){TAG(BASE), TAG(BASEFONT), TAG(BGSOUND), TAG(LINK), TAG(META), TAG(NOFRAMES), TAG(SCRIPT),
				   TAG(STYLE), TAG(TEMPLATE), TAG(TITLE)}*/) || tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
			return handle_in_head(parser, token);
		}
		else {
			GumboTagSet _ts2;
			_ts2.Append(_TAG(CAPTION)).Append(_TAG(COLGROUP)).Append(_TAG(TBODY)).Append(_TAG(TFOOT)).Append(_TAG(THEAD));
			if(tag_in(token, kStartTag, _ts2/*(gumbo_tagset){TAG(CAPTION), TAG(COLGROUP), TAG(TBODY), TAG(TFOOT), TAG(THEAD)}*/)) {
				pop_template_insertion_mode(parser);
...
{
	if(token->type == GUMBO_TOKEN_WHITESPACE || tag_is(token, kStartTag, GUMBO_TAG_HTML)) {
		return handle_in_body(parser, token);
	}
	else if(token->type == GUMBO_TOKEN_COMMENT) {
		...
		GumboNode * html = static_cast<GumboNode *>(parser->_parser_state->_open_elements.data[0]);
...
static bool handle_in_frameset(GumboParser * parser, GumboToken* token) 
{
	if(token->type == GUMBO_TOKEN_WHITESPACE) {
		...
		if(node_html_tag_is(get_cur_node(parser), GUMBO_TAG_HTML)) {
			parser_add_parse_error(parser, token);
			ignore_token(parser);
			return false;
		}
		pop_current_node(parser);
		if(!is_fragment_parser(parser) && !node_html_tag_is(get_cur_node(parser), GUMBO_TAG_FRAMESET)) {
...
static bool handle_after_after_body(GumboParser * parser, GumboToken* token) 
{
	...
	else if(token->type == GUMBO_TOKEN_DOCTYPE || token->type == GUMBO_TOKEN_WHITESPACE ||
...
static bool handle_after_after_frameset(GumboParser * parser, GumboToken* token) 
{
	...
	else if(oneof2(token->type, GUMBO_TOKEN_DOCTYPE, GUMBO_TOKEN_WHITESPACE) || tag_is(token, kStartTag, GUMBO_TAG_HTML)) {
		return handle_in_body(parser, token);
	}
	else if(token->type == GUMBO_TOKEN_EOF) {
		return true;
...
static bool handle_html_content(GumboParser * parser, GumboToken* token) 
{
	...
	return kTokenHandlers[(uint)parser->_parser_state->_insertion_mode](parser, token);
}
...
static bool handle_in_foreign_content(GumboParser * parser, GumboToken* token) 
{
	...
	{
		GumboTagSet _ts;
		_ts.Append(_TAG(B)).Append(_TAG(BIG)).Append(_TAG(BLOCKQUOTE)).Append(_TAG(BODY)).Append(_TAG(BR)).
			Append(_TAG(CENTER)).Append(_TAG(CODE)).Append(_TAG(DD)).Append(_TAG(DIV)).Append(_TAG(DL)).
			Append(_TAG(DT)).Append(_TAG(EM)).Append(_TAG(EMBED)).Append(_TAG(H1)).Append(_TAG(H2)).Append(_TAG(H3)).
			Append(_TAG(H4)).Append(_TAG(H5)).Append(_TAG(H6)).Append(_TAG(HEAD)).Append(_TAG(HR)).Append(_TAG(I)).
			Append(_TAG(IMG)).Append(_TAG(LI)).Append(_TAG(LISTING)).Append(_TAG(MENU)).Append(_TAG(META)).Append(_TAG(NOBR)).
			Append(_TAG(OL)).Append(_TAG(P)).Append(_TAG(PRE)).Append(_TAG(RUBY)).Append(_TAG(S)).Append(_TAG(SMALL)).
			Append(_TAG(SPAN)).Append(_TAG(STRONG)).Append(_TAG(STRIKE)).Append(_TAG(SUB)).Append(_TAG(SUP)).
			Append(_TAG(TABLE)).Append(_TAG(TT)).Append(_TAG(U)).Append(_TAG(UL)).Append(_TAG(VAR));
		// Order matters for these clauses.
		if(tag_in(token, kStartTag,
			_ts/*(gumbo_tagset){TAG(B), TAG(BIG), TAG(BLOCKQUOTE), TAG(BODY), TAG(BR),
				   ...
				   TAG(UL), TAG(VAR)}*/) ||
			(tag_is(token, kStartTag, GUMBO_TAG_FONT) && (token_has_attribute(token, "color") ||
			token_has_attribute(token, "face") || token_has_attribute(token, "size")))) {
			/* Parse error */
			parser_add_parse_error(parser, token);
			/*
			...
			if(!is_fragment_parser(parser)) {
				do {
					pop_current_node(parser);
				} while(!(is_mathml_integration_point(get_cur_node(parser)) ||
					is_html_integration_point(get_cur_node(parser)) ||
					get_cur_node(parser)->v.element.tag_namespace == GUMBO_NAMESPACE_HTML));
				parser->_parser_state->_reprocess_current_token = true;
				return false;
			}
			assert(token->type == GUMBO_TOKEN_START_TAG);
		...
		if(token->type == GUMBO_TOKEN_START_TAG) {
			const GumboNamespaceEnum current_namespace = get_adjusted_current_node(parser)->v.element.tag_namespace;
			...
			for(--i; i > 0;) {
				...
				--i;
				node = static_cast<GumboNode *>(parser->_parser_state->_open_elements.data[i]);
				if(node->v.element.tag_namespace == GUMBO_NAMESPACE_HTML) {
					// Must break before gumbo_tag_from_original_text to avoid passing parser-inserted nodes through.
					break;
				}
...
static bool handle_token(GumboParser * parser, GumboToken* token) 
{
	if(parser->_parser_state->_ignore_next_linefeed && token->type == GUMBO_TOKEN_WHITESPACE && token->v.character == '\n') {
		...
		assert(!current_node || oneof2(current_node->type, GUMBO_NODE_ELEMENT, GUMBO_NODE_TEMPLATE));
		if(current_node) {
			gumbo_debug("Current node: <%s>.\n", gumbo_normalized_tagname(current_node->v.element.tag));
		}
		{
			GumboTagSet _ts;
			_ts.Append(_TAG(MGLYPH)).Append(_TAG(MALIGNMARK));
			if(!current_node || current_node->v.element.tag_namespace == GUMBO_NAMESPACE_HTML ||
				(is_mathml_integration_point(current_node) &&
				(oneof3(token->type, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_WHITESPACE, GUMBO_TOKEN_NULL) || (token->type == GUMBO_TOKEN_START_TAG &&
				!tag_in(token, kStartTag, _ts/*(gumbo_tagset){TAG(MGLYPH), TAG(MALIGNMARK)}*/)))) ||
				...
				tag_is(token, kStartTag, GUMBO_TAG_SVG)) || (is_html_integration_point(current_node) &&
				oneof4(token->type, GUMBO_TOKEN_START_TAG, GUMBO_TOKEN_CHARACTER, GUMBO_TOKEN_NULL, GUMBO_TOKEN_WHITESPACE)) || token->type == GUMBO_TOKEN_EOF) {
...
static void fragment_parser_init(GumboParser * parser, GumboTag fragment_ctx, GumboNamespaceEnum fragment_namespace) 
{
	...
	parser->_parser_state->_fragment_ctx->v.element.tag_namespace = fragment_namespace;
	// 4
	if(fragment_namespace == GUMBO_NAMESPACE_HTML) {
		// Non-HTML namespaces always start in the DATA state.
		switch(fragment_ctx) {
			case GUMBO_TAG_TITLE:
			case GUMBO_TAG_TEXTAREA: gumbo_tokenizer_set_state(parser, GUMBO_LEX_RCDATA); break;
			case GUMBO_TAG_STYLE:
			case GUMBO_TAG_XMP:
			case GUMBO_TAG_IFRAME:
			case GUMBO_TAG_NOEMBED:
			case GUMBO_TAG_NOFRAMES: gumbo_tokenizer_set_state(parser, GUMBO_LEX_RAWTEXT); break;
			case GUMBO_TAG_SCRIPT: gumbo_tokenizer_set_state(parser, GUMBO_LEX_SCRIPT); break;
			case GUMBO_TAG_NOSCRIPT:
			    // scripting is disabled in Gumbo, so leave the tokenizer in the default data state
			    break;
			case GUMBO_TAG_PLAINTEXT: gumbo_tokenizer_set_state(parser, GUMBO_LEX_PLAINTEXT); break;
			default: break; /* default data state */
		}
	}
	{
		// 5. 6. 7.
		GumboNode * root = insert_element_of_tag_type(parser, GUMBO_TAG_HTML, GUMBO_INSERTION_IMPLIED);
...
GumboOutput * gumbo_parse_with_options(const GumboOptions* options, const char* buffer, size_t length) 
{
	...
	do {
		...
		switch(token.type) {
			...
			case GUMBO_TOKEN_END_TAG: token_type = gumbo_normalized_tagname(token.v.end_tag); break;
		...
		// Check for memory leaks when ownership is transferred from start tag tokens to nodes.
		assert(state->_reprocess_current_token || token.type != GUMBO_TOKEN_START_TAG || token.v.start_tag.attributes.data == NULL);
	...
	} while((token.type != GUMBO_TOKEN_EOF || state->_reprocess_current_token) && !(options->stop_on_first_error && has_error));
	...
	SETIFZ(doc_type->name, sstrdup(""));
	SETIFZ(doc_type->public_identifier, sstrdup(""));
	SETIFZ(doc_type->system_identifier, sstrdup(""));
