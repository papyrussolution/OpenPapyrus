...
public:
	...
	uint   ChunkNo;
	//
	// Из-за того, что в файле отрезка может быть более 4G элементов следующие 2 поля 64-битные
	// (элементы хранятся только в файле, потому переполнения памяти не будет)
	//
	uint64 LineCount_;
			...
			THROW(f_src.IsValid());
			{
				...
				while(f_src.ReadLine(line_buf)) {
					...
					if(chunk_.GetPoolDataLen() >= max_chunk_size) {
						...
						if(max_thread > 1) {
							//
							// Если количество потоков превысило предел, то придется ждать завершения всех (у нас сейчас нет
							// механизма межпоточного ожидания по значению счетчика).
							// Не забываем, что мы искусственно увеличиваем на 1 количество потоков.
							//
