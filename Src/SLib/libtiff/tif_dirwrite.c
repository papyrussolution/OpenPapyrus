...
static int FASTCALL TIFFWriteDirectorySec(TIFF * tif, int isimage, int imagedone, uint64* pdiroff);
...
#endif
static int FASTCALL TIFFWriteDirectoryTagShort(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint16 value);
static int FASTCALL TIFFWriteDirectoryTagShortArray(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint32 count, uint16* value);
static int FASTCALL TIFFWriteDirectoryTagShortPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint16 value);
#ifdef notdef
		...
		if(tif->tif_header.big.tiff_diroff == tif->tif_diroff) {
			...
			while(1) {
				...
				if(dircount64>0xFFFF) {
					TIFFErrorExt(tif->tif_clientdata, module, "Sanity check on tag count failed, likely corrupt TIFF");
					return 0;
				}
...
static int FASTCALL TIFFWriteDirectorySec(TIFF * tif, int isimage, int imagedone, uint64* pdiroff)
{
	...
	while(1) {
		...
		if(isimage) {
			...
			if(TIFFFieldSet(tif, FIELD_STRIPOFFSETS)) {
				if(!isTiled(tif)) {
					 ...
					 */
					if(tif->tif_dir.td_stripoffset && !TIFFWriteDirectoryTagLongLong8Array(tif, &ndir, dir, TIFFTAG_STRIPOFFSETS, tif->tif_dir.td_nstrips, tif->tif_dir.td_stripoffset))
						goto bad;
				}
				else {
			...
			if(TIFFFieldSet(tif, FIELD_SMINSAMPLEVALUE)) {
				if(!TIFFWriteDirectoryTagSampleformatArray(tif, &ndir, dir, TIFFTAG_SMINSAMPLEVALUE,
					    tif->tif_dir.td_samplesperpixel, tif->tif_dir.td_sminsamplevalue))
					goto bad;
			}
			if(TIFFFieldSet(tif, FIELD_SMAXSAMPLEVALUE)) {
				if(!TIFFWriteDirectoryTagSampleformatArray(tif, &ndir, dir, TIFFTAG_SMAXSAMPLEVALUE,
					    tif->tif_dir.td_samplesperpixel, tif->tif_dir.td_smaxsamplevalue))
					goto bad;
			}
			...
			}
			{
				...
				for(n = 0; n<tif->tif_nfields; n++) {
					const TIFFField * o = tif->tif_fields[n];
					if((o->field_bit>=FIELD_CODEC)&&(TIFFFieldSet(tif, o->field_bit))) {
						switch(o->get_field_type) {
							...
							case TIFF_SETGET_C32_UINT8:
						    {
							    ...
							    if(!TIFFWriteDirectoryTagUndefinedArray(tif, &ndir, dir, (uint16)o->field_tag, pa, (uint8 *)pb))
								    goto bad;
						    }
						    break;
							default:
		...
		if(dir)
			break;
		dir = (TIFFDirEntry *)SAlloc::M(ndir*sizeof(TIFFDirEntry));
		if(!dir) {
		...
		ASSIGN_PTR(pdiroff, tif->tif_diroff);
		dirsize = (!(tif->tif_flags&TIFF_BIGTIFF)) ? (2+ndir*12+4) : (8+ndir*20+8);
	...
	if(isimage) {
		if(TIFFFieldSet(tif, FIELD_SUBIFD)&&(tif->tif_subifdoff==0)) {
			...
			tif->tif_subifdoff = (!(tif->tif_flags&TIFF_BIGTIFF)) ? (tif->tif_diroff+2+na*12+8) : (tif->tif_diroff+8+na*20+12);
		}
	}
	dirmem = SAlloc::M(dirsize);
	if(!dirmem) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		goto bad;
	}
	if(!(tif->tif_flags&TIFF_BIGTIFF)) {
		uint32 nTmp;
		TIFFDirEntry* o;
		uint8 * n = static_cast<uint8 *>(dirmem);
		*reinterpret_cast<uint16 *>(n) = (uint16)ndir;
		if(tif->tif_flags&TIFF_SWAB)
			TIFFSwabShort(reinterpret_cast<uint16 *>(n));
		n += 2;
		o = dir;
		for(m = 0; m<ndir; m++) {
			*reinterpret_cast<uint16 *>(n) = o->tdir_tag;
			if(tif->tif_flags&TIFF_SWAB)
				TIFFSwabShort(reinterpret_cast<uint16 *>(n));
			n += 2;
			*reinterpret_cast<uint16 *>(n) = o->tdir_type;
			if(tif->tif_flags&TIFF_SWAB)
				TIFFSwabShort(reinterpret_cast<uint16 *>(n));
			n += 2;
	...
	if(!WriteOK(tif, dirmem, static_cast<tmsize_t>(dirsize))) {
		TIFFErrorExt(tif->tif_clientdata, module, "IO error writing directory");
		goto bad;
	}
	SAlloc::F(dirmem);
	if(imagedone) {
		...
		tif->tif_flags &= ~(TIFF_DIRTYDIRECT|TIFF_DIRTYSTRIP);
		(*tif->tif_cleanup)(tif);
		// 
...
static int TIFFWriteDirectoryTagBytePerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint8 value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(uint8));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagSbytePerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, int8 value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(int8));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int FASTCALL TIFFWriteDirectoryTagShortPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint16 value)
{
	...
	m = static_cast<uint16 *>(SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(uint16)));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagSshortPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, int16 value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(int16));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagLongPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint32 value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(uint32));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagSlongPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, int32 value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(int32));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagFloatPerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, float value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(float));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagDoublePerSample(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, double value)
{
	...
	m = SAlloc::M(tif->tif_dir.td_samplesperpixel*sizeof(double));
	if(!m) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
...
static int TIFFWriteDirectoryTagLongLong8Array(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint32 count, uint64* value)
{
	...
	*/
	p = (uint32 *)SAlloc::M(count*sizeof(uint32));
	if(!p) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
	for(q = p, ma = value, mb = 0; mb<count; ma++, mb++, q++) {
		if(*ma>0xFFFFFFFF) {
			TIFFErrorExt(tif->tif_clientdata, module, "Attempt to write value larger than 0xFFFFFFFF in Classic TIFF file.");
			SAlloc::F(p);
			return 0;
		}
		*q = (uint32)(*ma);
...
static int TIFFWriteDirectoryTagShortLongLong8Array(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint32 count, uint64* value)
{
	...
	if(n==0) {
		uint16 * q;
		uint16 * p = SAlloc::M(count*sizeof(uint16));
		if(!p) {
			TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
			return 0;
		}
		...
		uint32 * q;
		uint32 * p = SAlloc::M(count*sizeof(uint32));
		if(!p) {
			TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
			return 0;
		}
...
static int TIFFWriteDirectoryTagColormap(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir)
{
	...
	if(!dir) {
		...
		n = static_cast<uint16 *>(SAlloc::M(3*m*sizeof(uint16)));
		if(!n) {
			TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
			return 0;
		}
		else {
			memcpy(&n[0], tif->tif_dir.td_colormap[0], m*sizeof(uint16));
			memcpy(&n[m], tif->tif_dir.td_colormap[1], m*sizeof(uint16));
			memcpy(&n[2*m], tif->tif_dir.td_colormap[2], m*sizeof(uint16));
...
static int TIFFWriteDirectoryTagTransferfunction(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir)
{
	...
	SETMIN(n, 3);
	if(n==3) {
		if(memcmp(tif->tif_dir.td_transferfunction[0], tif->tif_dir.td_transferfunction[2], m*sizeof(uint16)) == 0)
			n = 2;
	}
	if(n==2) {
		if(memcmp(tif->tif_dir.td_transferfunction[0], tif->tif_dir.td_transferfunction[1], m*sizeof(uint16)) == 0)
			n = 1;
	}
	SETIFZ(n, 1);
	o = static_cast<uint16 *>(SAlloc::M(n*m*sizeof(uint16)));
	if(!o) {
		TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
		return 0;
	}
	memcpy(&o[0], tif->tif_dir.td_transferfunction[0], m*sizeof(uint16));
	if(n>1)
		memcpy(&o[m], tif->tif_dir.td_transferfunction[1], m*sizeof(uint16));
	if(n>2)
		memcpy(&o[2*m], tif->tif_dir.td_transferfunction[2], m*sizeof(uint16));
...
static int TIFFWriteDirectoryTagSubifd(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir)
{
	...
	if(!(tif->tif_flags&TIFF_BIGTIFF)) {
		...
		uint32 * pb;
		uint16 p;
		uint32 * o = (uint32 *)SAlloc::M(tif->tif_dir.td_nsubifd*sizeof(uint32));
		if(!o) {
			TIFFErrorExtOutOfMemory(tif->tif_clientdata, module);
			return 0;
		}
	...
	tif->tif_subifdoff = (tif->tif_dir.td_nsubifd==1) ? 0 : m;
	return 1;
}
...
static int FASTCALL TIFFWriteDirectoryTagData(TIFF * tif, uint32 * ndir, TIFFDirEntry* dir, uint16 tag, uint16 datatype, uint32 count, uint32 datalength, void * data)
{
	...
	if(datalength<=((tif->tif_flags&TIFF_BIGTIFF) ? 0x8U : 0x4U))
		memcpy(&dir[m].tdir_offset, data, datalength);
	else {
		uint64 na = tif->tif_dataoff;
		uint64 nb = na+datalength;
		if(!(tif->tif_flags&TIFF_BIGTIFF))
			nb = (uint32)nb;
...
// stripbytecount values after the directory is already on disk.
// 
...
int _TIFFRewriteField(TIFF * tif, uint16 tag, TIFFDataType in_datatype, tmsize_t count, void * data)
{
	...
	if(tif->tif_diroff == 0) {
		TIFFErrorExt(tif->tif_clientdata, module, "Attempt to reset field on directory not already on disk.");
		return 0;
	}
	// 
	...
	if(entry_tag != tag) {
		TIFFErrorExt(tif->tif_clientdata, module, "%s: Could not find tag %d.", tif->tif_name, tag);
		return 0;
	}
	// 
	...
	if(TIFFDataWidth(in_datatype) == 8 && !(tif->tif_flags&TIFF_BIGTIFF)) {
		...
		datatype = in_datatype;
	// 
	// Prepare buffer of actual data to write.  This includes swabbing as needed.
	// 
	...
	// over the old values without altering the directory entry at all.
	// 
	...
	if(!WriteOK(tif, direntry_raw, dirsize)) {
		TIFFErrorExt(tif->tif_clientdata, module, "%s: Can not write TIFF directory entry.", tif->tif_name);
		return 0;
	}
	return 1;
}
