...
static int TIFFNoEncode(TIFF * tif, const char * method)
{
	...
	if(c) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name, "%s %s encoding is not implemented", c->name, method);
	}
	else {
...
TIFFCodec* TIFFRegisterCODEC(uint16 scheme, const char * name, TIFFInitMethod init)
{
	...
	if(cd) {
		...
		TIFFErrorExt(0, "TIFFRegisterCODEC", "No space to register compression scheme %s", name);
		return NULL;
	}
	return (cd->info);
}
...
TIFFCodec* TIFFGetConfiguredCODECs()
{
	...
	codecs = new_codecs;
	memzero(codecs + i - 1, sizeof(TIFFCodec));
	return codecs;
}
