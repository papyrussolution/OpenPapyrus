...
tmsize_t TIFFWriteEncodedStrip(TIFF * tif, uint32 strip, void * data, tmsize_t cc)
{
	...
	if(td->td_stripbytecount[strip] > 0) {
		...
		if(tif->tif_rawdatasize <= (tmsize_t)td->td_stripbytecount[strip]) {
			if(!(TIFFWriteBufferSetup(tif, NULL, (tmsize_t)TIFFroundup_64((uint64)(td->td_stripbytecount[strip] + 1), 1024))) )
	...
	if(td->td_compression == COMPRESSION_NONE) {
		/* swab if needed - note that source buffer will be altered */
		tif->tif_postdecode(tif, static_cast<uint8 *>(data), cc);
		if(!isFillOrder(tif, td->td_fillorder) && (tif->tif_flags & TIFF_NOBITREV) == 0)
			TIFFReverseBits(static_cast<uint8 *>(data), cc);
		if(cc > 0 && !TIFFAppendToStrip(tif, strip, static_cast<uint8 *>(data), cc))
			return ((tmsize_t)-1);
		return (cc);
	}
	...
	/* swab if needed - note that source buffer will be altered */
	tif->tif_postdecode(tif, static_cast<uint8 *>(data), cc);
	if(!(*tif->tif_encodestrip)(tif, static_cast<uint8 *>(data), cc, sample))
		return ((tmsize_t)-1);
...
tmsize_t TIFFWriteRawStrip(TIFF * tif, uint32 strip, void * data, tmsize_t cc)
{
	...
	tif->tif_row = (strip % td->td_stripsperimage) * td->td_rowsperstrip;
	return (TIFFAppendToStrip(tif, strip, static_cast<uint8 *>(data), cc) ? cc : (tmsize_t)-1);
}
...
tmsize_t TIFFWriteEncodedTile(TIFF * tif, uint32 tile, void * data, tmsize_t cc)
{
	...
	if(tile >= td->td_nstrips) {
		TIFFErrorExt(tif->tif_clientdata, module, "Tile %lu out of range, max %lu",
		    (ulong)tile, (ulong)td->td_nstrips);
		return static_cast<tmsize_t>(-1);
	}
	/*
	 * Handle delayed allocation of data buffer.  This
	...
	if(td->td_stripbytecount[tile] > 0) {
		...
		if(tif->tif_rawdatasize <= (tmsize_t)td->td_stripbytecount[tile]) {
			if(!(TIFFWriteBufferSetup(tif, NULL, (tmsize_t)TIFFroundup_64((uint64)(td->td_stripbytecount[tile] + 1), 1024))) )
				return static_cast<tmsize_t>(-1);
		}
	...
	if(td->td_compression == COMPRESSION_NONE) {
		...
		if(!isFillOrder(tif, td->td_fillorder) && (tif->tif_flags & TIFF_NOBITREV) == 0)
			TIFFReverseBits(static_cast<uint8 *>(data), cc);
		if(cc > 0 && !TIFFAppendToStrip(tif, tile, static_cast<uint8 *>(data), cc))
			return ((tmsize_t)-1);
		return (cc);
	}
	...
	/* swab if needed - note that source buffer will be altered */
	tif->tif_postdecode(tif, static_cast<uint8 *>(data), cc);
	if(!(*tif->tif_encodetile)(tif, static_cast<uint8 *>(data), cc, sample))
		return ((tmsize_t)-1);
	if(!(*tif->tif_postencode)(tif))
		return static_cast<tmsize_t>(-1);
	if(!isFillOrder(tif, td->td_fillorder) && (tif->tif_flags & TIFF_NOBITREV) == 0)
...
tmsize_t TIFFWriteRawTile(TIFF * tif, uint32 tile, void * data, tmsize_t cc)
{
	...
	if(tile >= tif->tif_dir.td_nstrips) {
		TIFFErrorExt(tif->tif_clientdata, module, "Tile %lu out of range, max %lu", (ulong)tile, (ulong)tif->tif_dir.td_nstrips);
		return static_cast<tmsize_t>(-1);
	}
	return (TIFFAppendToStrip(tif, tile, static_cast<uint8 *>(data), cc) ? cc : (tmsize_t)(-1));
}
...
int TIFFSetupStrips(TIFF * tif)
{
	TIFFDirectory* td = &tif->tif_dir;
	if(isTiled(tif))
		td->td_stripsperimage = isUnspecified(tif, FIELD_TILEDIMENSIONS) ? td->td_samplesperpixel : TIFFNumberOfTiles(tif);
	else
		td->td_stripsperimage = isUnspecified(tif, FIELD_ROWSPERSTRIP) ? td->td_samplesperpixel : TIFFNumberOfStrips(tif);
	 ...
	 */
	memzero(td->td_stripoffset, td->td_nstrips*sizeof(uint64));
...
int FASTCALL TIFFWriteCheck(TIFF * tif, int tiles, const char * module)
{
	...
	if(tiles ^ isTiled(tif)) {
		TIFFErrorExt(tif->tif_clientdata, module, tiles ? "Can not write tiles to a stripped image" : "Can not write scanlines to a tiled image");
		return 0;
	}
	...
	if(!TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
		TIFFErrorExt(tif->tif_clientdata, module, "Must set \"ImageWidth\" before writing data");
		return 0;
	}
	if(tif->tif_dir.td_samplesperpixel == 1) {
		...
		if(!TIFFFieldSet(tif, FIELD_PLANARCONFIG)) {
			TIFFErrorExt(tif->tif_clientdata, module, "Must set \"PlanarConfiguration\" before writing data");
			return 0;
		}
	}
	if(tif->tif_dir.td_stripoffset == NULL && !TIFFSetupStrips(tif)) {
		tif->tif_dir.td_nstrips = 0;
		TIFFErrorExt(tif->tif_clientdata, module, "No space for %s arrays", isTiled(tif) ? "tile" : "strip");
		return 0;
	}
	if(isTiled(tif)) {
...
int TIFFWriteBufferSetup(TIFF * tif, void * bp, tmsize_t size)
{
	...
	if(bp == NULL) {
		...
		tif->tif_flags &= ~TIFF_MYBUFFER;
	tif->tif_rawdata = reinterpret_cast<uint8 *>(bp);
...
static int TIFFGrowStrips(TIFF * tif, uint32 delta, const char * module)
{
	...
	memzero(td->td_stripoffset + td->td_nstrips, delta*sizeof(uint64));
...
static int TIFFAppendToStrip(TIFF * tif, uint32 strip, uint8 * data, tmsize_t cc)
{
	...
	if(td->td_stripoffset[strip] == 0 || tif->tif_curoff == 0) {
		...
		if(td->td_stripbytecount[strip] != 0 && td->td_stripoffset[strip] != 0 && td->td_stripbytecount[strip] >= (uint64)cc) {
			/*
			...
			if(!SeekOK(tif, td->td_stripoffset[strip])) {
				TIFFErrorExt(tif->tif_clientdata, module, "Seek error at scanline %lu", (ulong)tif->tif_row);
				return 0;
			}
		}
		else {
	...
	if(!WriteOK(tif, data, cc)) {
		TIFFErrorExt(tif->tif_clientdata, module, "Write error at scanline %lu", (ulong)tif->tif_row);
		return 0;
	}
	tif->tif_curoff = m;
