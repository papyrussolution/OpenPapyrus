...
static void OJPEGPrintDir(TIFF * tif, FILE* fd, long flags)
{
	...
	if(TIFFFieldSet(tif, FIELD_OJPEG_JPEGQTABLES)) {
		...
		for(m = 0; m<sp->qtable_offset_count; m++)
			fprintf(fd, " " TIFF_UINT64_FORMAT, (uint64)sp->qtable_offset[m]);
		fprintf(fd, "\n");
	}
	if(TIFFFieldSet(tif, FIELD_OJPEG_JPEGDCTABLES)) {
		fprintf(fd, "  JpegDcTables:");
		for(m = 0; m<sp->dctable_offset_count; m++)
			fprintf(fd, " " TIFF_UINT64_FORMAT, (uint64)sp->dctable_offset[m]);
		fprintf(fd, "\n");
	}
	if(TIFFFieldSet(tif, FIELD_OJPEG_JPEGACTABLES)) {
		fprintf(fd, "  JpegAcTables:");
		for(m = 0; m<sp->actable_offset_count; m++)
			fprintf(fd, " " TIFF_UINT64_FORMAT, (uint64)sp->actable_offset[m]);
		fprintf(fd, "\n");
	}
...
static int OJPEGPreDecodeSkipRaw(TIFF * tif)
{
	...
	while(m>=sp->subsampling_convert_clines) {
		if(jpeg_read_raw_data_encap(sp, &(sp->libjpeg_jpeg_decompress_struct), sp->subsampling_convert_ycbcrimage,
...
static void OJPEGCleanup(TIFF * tif)
{
	...
	if(sp!=0) {
		...
		if(sp->qtable[0]!=0)
			SAlloc::F(sp->qtable[0]);
		if(sp->qtable[1]!=0)
			SAlloc::F(sp->qtable[1]);
		if(sp->qtable[2]!=0)
			SAlloc::F(sp->qtable[2]);
		if(sp->qtable[3]!=0)
			SAlloc::F(sp->qtable[3]);
		if(sp->dctable[0]!=0)
			SAlloc::F(sp->dctable[0]);
		if(sp->dctable[1]!=0)
			SAlloc::F(sp->dctable[1]);
		if(sp->dctable[2]!=0)
			SAlloc::F(sp->dctable[2]);
		if(sp->dctable[3]!=0)
			SAlloc::F(sp->dctable[3]);
		if(sp->actable[0]!=0)
			SAlloc::F(sp->actable[0]);
		if(sp->actable[1]!=0)
			SAlloc::F(sp->actable[1]);
		if(sp->actable[2]!=0)
			SAlloc::F(sp->actable[2]);
		if(sp->actable[3]!=0)
			SAlloc::F(sp->actable[3]);
		if(sp->libjpeg_session_active!=0)
			OJPEGLibjpegSessionAbort(tif);
		if(sp->subsampling_convert_ycbcrbuf!=0)
			SAlloc::F(sp->subsampling_convert_ycbcrbuf);
		if(sp->subsampling_convert_ycbcrimage!=0)
			SAlloc::F(sp->subsampling_convert_ycbcrimage);
		if(sp->skip_buffer!=0)
			SAlloc::F(sp->skip_buffer);
		SAlloc::F(sp);
		tif->tif_data = NULL;
...
static void OJPEGSubsamplingCorrect(TIFF * tif)
{
	...
	if((tif->tif_dir.td_samplesperpixel!=3) || ((tif->tif_dir.td_photometric!=PHOTOMETRIC_YCBCR) &&
		...
		if(((sp->subsampling_hor!=mh) ||
			    (sp->subsampling_ver!=mv)) && (sp->subsampling_force_desubsampling_inside_decompression==0)) {
			if(sp->subsampling_tag==0)
				TIFFWarningExt(tif->tif_clientdata, module,
	...
	if((sp->subsampling_force_desubsampling_inside_decompression==0) && (sp->samples_per_pixel_per_plane>1)) {
		...
		if(sp->subsampling_convert_log==0) {
			...
			sp->subsampling_convert_ylinelen =
			    ((sp->strile_width+sp->subsampling_hor*8-1)/(sp->subsampling_hor*8)*sp->subsampling_hor*8);
	...
	do {
		...
		switch(m) {
			...
			case JPEG_MARKER_APP0+15:
			    /* this type of marker has data, but it has no use to us (and no place here) and should be
			      skipped */
...
static int OJPEGReadHeaderInfoSecStreamSof(TIFF * tif, uint8 marker_id)
{
	...
	/* per component stuff */
	/* TODO: double-check that flow implies that n cannot be as big as to make us overflow sof_c, sof_hv and sof_tq
	  arrays */
	for(q = 0; q<n; q++) {
	...
	do {
		...
		switch(sp->in_buffer_source) {
			...
			case osibsJpegInterchangeFormat:
			    ...
			    break;
			case osibsStrile:
			    if(!_TIFFFillStriles(sp->tif) || sp->tif->tif_dir.td_stripoffset == NULL || sp->tif->tif_dir.td_stripbytecount == NULL)
				    return 0;
...
static int OJPEGReadBlock(OJPEGState* sp, uint16 len, void * mem)
{
	...
	do {
		...
		if(n>sp->in_buffer_togo)
			n = sp->in_buffer_togo;
		memcpy(mmem, sp->in_buffer_cur, n);
		sp->in_buffer_cur += n;
		sp->in_buffer_togo -= n;
		mlen -= n;
		mmem += n;
