...
void PNGAPI png_set_hIST(png_const_structrp png_ptr, png_inforp info_ptr, png_const_uint_16p hist)
{
	 ...
	 * version 1.2.1
	 */
	info_ptr->hist = png_voidcast(png_uint_16p, png_malloc_warn(png_ptr, PNG_MAX_PALETTE_LENGTH * (sizeof(uint16))));
...
void PNGAPI png_set_oFFs(png_const_structrp png_ptr, png_inforp info_ptr, int32_t offset_x, int32_t offset_y, int unit_type)
{
...
void PNGAPI png_set_pCAL(png_const_structrp png_ptr, png_inforp info_ptr, const char * purpose, int32_t X0, int32_t X1, int type, int nparams, const char * units, png_charpp params)
{
	size_t length;
	int i;
	...
	info_ptr->pcal_params = png_voidcast(png_charpp, png_malloc_warn(png_ptr, (size_t)((nparams + 1) * (sizeof(char *)))));
...
static uint8 check_location(png_const_structrp png_ptr, int location)
{
