...
PNG_ALLOCATED png_structp PNGAPI png_create_read_struct(const char * user_png_ver, void * error_ptr, png_error_ptr error_fn, png_error_ptr warn_fn)
{
#ifndef PNG_USER_MEM_SUPPORTED
	png_structp png_ptr = png_create_png_struct(user_png_ver, error_ptr, error_fn, warn_fn, 0, 0, 0);
#else
	return png_create_read_struct_2(user_png_ver, error_ptr, error_fn, warn_fn, 0, 0, 0);
}

/* Alternate create PNG structure for reading, and allocate any memory
 * needed.
 */
PNG_ALLOCATED png_structp PNGAPI png_create_read_struct_2(const char * user_png_ver, void * error_ptr,
	png_error_ptr error_fn, png_error_ptr warn_fn, void * mem_ptr, png_malloc_ptr malloc_fn, png_free_ptr free_fn)
{
	png_structp png_ptr = png_create_png_struct(user_png_ver, error_ptr, error_fn, warn_fn, mem_ptr, malloc_fn, free_fn);
#endif /* USER_MEM */
	if(png_ptr) {
...
void PNGAPI png_read_info(png_structrp png_ptr, png_inforp info_ptr)
{
	...
	for(;;) {
		...
		if(chunk_name == png_IDAT) {
			...
			else if(png_ptr->color_type == PNG_COLOR_TYPE_PALETTE && (png_ptr->mode & PNG_HAVE_PLTE) == 0)
...
void PNGAPI png_read_row(png_structrp png_ptr, png_bytep row, png_bytep dsp_row)
{
	...
	if(!png_ptr)
		return;
	png_debug2(1, "in " __FUNCTION__ "(row %lu, pass %d)", (ulong)png_ptr->row_number, png_ptr->pass);
...
void PNGAPI png_read_rows(png_structrp png_ptr, png_bytepp row, png_bytepp display_row, uint32 num_rows)
{
	uint32 i;
	png_debug(1, "in " __FUNCTION__);
	if(png_ptr) {
...
void PNGAPI png_read_end(png_structrp png_ptr, png_inforp info_ptr)
{
	...
	if(png_ptr->color_type == PNG_COLOR_TYPE_PALETTE && png_ptr->num_palette_max > png_ptr->num_palette)
	...
	do {
		...
		if(chunk_name == png_IEND)
			...
			if(chunk_name == png_IDAT) {
				if((length > 0 && !(png_ptr->flags & PNG_FLAG_ZSTREAM_ENDED)) || (png_ptr->mode & PNG_HAVE_CHUNK_AFTER_IDAT) != 0)
...
static int png_image_read_init(png_imagep image)
{
	...
	return png_image_error(image, __FUNCTION__ ": opaque pointer not NULL");
}

/* Utility to find the base format of a PNG file from a png_struct. */
static uint32 png_image_format(png_structrp png_ptr)
{
	uint32 format = 0;
...
static int png_gamma_not_sRGB(png_fixed_point g)
{
	if(g < PNG_FP_1) {
		...
		return (g == 0) ? 0 : png_gamma_significant((g * 11 + 2)/5 /* i.e. *2.2, rounded */);
	}
	return 1;
}
 ...
 */
static int png_image_read_header(void * argument)
{
	...
	image->height = png_ptr->height;
	{
		...
		if((format & PNG_FORMAT_FLAG_COLOR) != 0 && ((png_ptr->colorspace.flags & 
			(PNG_COLORSPACE_HAVE_ENDPOINTS|PNG_COLORSPACE_ENDPOINTS_MATCH_sRGB|PNG_COLORSPACE_INVALID)) == PNG_COLORSPACE_HAVE_ENDPOINTS))
...
int PNGAPI png_image_begin_read_from_memory(png_imagep image, const void * memory, size_t size)
{
	if(image && image->version == PNG_IMAGE_VERSION) {
		if(memory && size > 0) {
			if(png_image_read_init(image) != 0) {
...
#        ifdef PNG_READ_iCCP_SUPPORTED
		...
		png_set_keep_unknown_chunks(png_ptr, PNG_HANDLE_CHUNK_AS_DEFAULT, chunks_to_process, (int)/*SAFE*/ (sizeof chunks_to_process)/5);
	}
}
...
static uint decode_gamma(png_image_read_control * display, uint32 value, int encoding)
{
...
static uint32 png_colormap_compose(png_image_read_control * display, uint32 foreground, int foreground_encoding, uint32 alpha, uint32 background, int encoding)
{
	 ...
	 * P_LINEAR or P_sRGB.
	 */
	uint32 f = decode_gamma(display, foreground, foreground_encoding);
	uint32 b = decode_gamma(display, background, encoding);
 ...
 * be 8-bit.
 */
static void png_create_colormap_entry(png_image_read_control * display, uint32 ip, uint32 red, uint32 green, uint32 blue, uint32 alpha, int encoding)
{
	png_imagep image = display->image;
	const int output_encoding = (image->format & PNG_FORMAT_FLAG_LINEAR) != 0 ? P_LINEAR : P_sRGB;
	const int convert_to_Y = (image->format & PNG_FORMAT_FLAG_COLOR) == 0 && (red != green || green != blue);
	if(ip > 255)
	...
	if(encoding == P_LINEAR) {
		if(convert_to_Y != 0) {
			...
			uint32 y = (uint32)6968 * red  + (uint32)23434 * green + (uint32)2366 * blue;
	...
	// Store the value
	{
		...
		const int afirst = (image->format & PNG_FORMAT_FLAG_AFIRST) != 0 && (image->format & PNG_FORMAT_FLAG_ALPHA) != 0;
#else
#define afirst 0
#endif
		...
		if(output_encoding == P_LINEAR) {
			...
			switch(PNG_IMAGE_SAMPLE_CHANNELS(image->format)) {
				...
				case 3:
				    ...
				    entry[afirst + (2 ^ bgr)] = (uint16)blue;
				    entry[afirst + 1] = (uint16)green;
				    entry[afirst + bgr] = (uint16)red;
				    break;
				case 2:
				    entry[1 ^ afirst] = (uint16)alpha;
				// @fallthrough
				case 1:
				    if(alpha < 65535) {
					    if(alpha > 0)
			...
			switch(PNG_IMAGE_SAMPLE_CHANNELS(image->format)) {
				case 4:
				    entry[afirst ? 0 : 3] = (uint8)alpha;
				case 3:
				    entry[afirst + (2 ^ bgr)] = (uint8)blue;
				    entry[afirst + 1] = (uint8)green;
				    entry[afirst + bgr] = (uint8)red;
				    break;
				case 2:
				    entry[1 ^ afirst] = (uint8)alpha;
				case 1:
				    entry[afirst] = (uint8)green;
				    break;
...
#define PNG_RGB_INDEX(r, g, b) ((uint8)(6 * (6 * PNG_DIV51(r) + PNG_DIV51(g)) + PNG_DIV51(b)))

static int png_image_read_colormap(void * argument)
{
	 ...
	 */
	uint   background_index = 256;
	uint32 back_r, back_g, back_b;
	 ...
	 * color-map.
	 */
	if(((png_ptr->color_type & PNG_COLOR_MASK_ALPHA) != 0 || png_ptr->num_trans > 0) /* alpha in input */ &&
	...
	switch(png_ptr->color_type) {
		case PNG_COLOR_TYPE_GRAY:
		    if(png_ptr->bit_depth <= 8) {
			     ...
			     */
			    uint   step, i, val, trans = 256 /*ignore*/, back_alpha = 0;
			 ...
			 * value or, if the output has no alpha, the background color.
			    ...
			    for(i = val = 0; i<cmap_entries; ++i, val += step) {
				    ...
				    if(i != trans)
					    png_create_colormap_entry(display, i, val, val, val, 255, P_FILE /*8-bit with file gamma*/);
					// Else this entry is transparent.  The colors don't matter if
					// there is an alpha channel (back_alpha == 0), but it does no
					// harm to pass them in; the values are not set above so this passes in white.
					// 
					// NOTE: this preserves the full precision of the application supplied background color when it is used.
					// 
				    else
					    png_create_colormap_entry(display, i, back_r, back_g, back_b, back_alpha, output_encoding);
			    }
			    ...
			    if(png_ptr->num_trans > 0) {
				    ...
				    if((output_format & PNG_FORMAT_FLAG_ALPHA) != 0)
					    ...
					    if(back_r == back_g && back_g == back_b) {
						    ...
						    c.index = 0; /*unused*/
						    c.gray = c.red = c.green = c.blue = (uint16)gray;
							//
			  ...
			  */
				 ...
				 * 'A' is linear.  We need the linear background colors.
					 ...
					 * by an 8-bit alpha value (0..255).
					    ...
					    uint32 back_gx = (255-alpha) * back_g;
					    uint32 back_bx = (255-alpha) * back_b;

					    for(g = 0; g<6; ++g) {
						    uint32 gray = png_sRGB_table[g*51] * alpha;
				 ...
				 * processed into a special RGB cube with alpha support.
					 ...
					 * index.
						 ...
						 * the background at alpha 0.5.
						    ...
						    c.index = 0; /*unused*/
						    c.red = (uint16)back_r;
						    c.gray = c.green = (uint16)back_g;
						    c.blue = (uint16)back_b;
	...
	if(expand_tRNS != 0 && png_ptr->num_trans > 0 && (png_ptr->color_type & PNG_COLOR_MASK_ALPHA) == 0)
...
static int png_image_read_and_map(void * argument)
{
					...
					case PNG_CMAP_TRANS:
					    for(; outrow < end_row; outrow += stepx) {
						    ...
						    if(alpha == 0)
							    ...
							    *outrow = (uint8)(PNG_CMAP_TRANS_BACKGROUND+1);
					    }
					    break;
...
static int png_image_read_composite(void * argument)
{
	...
	}
	{
		...
		uint32 width = image->width;
		ptrdiff_t step_row = display->row_bytes;
		uint   channels = (image->format & PNG_FORMAT_FLAG_COLOR) != 0 ? 3 : 1;
		int pass;
		for(pass = 0; pass < passes; ++pass) {
			uint   startx, stepx, stepy;
			uint32 y;
			...
			for(; y<height; y += stepy) {
				...
				outrow += startx;
				for(; outrow < end_row; outrow += stepx) {
					uint8 alpha = inrow[channels];
					if(alpha > 0) { /* else no change to the output */
						uint   c;
						for(c = 0; c<channels; ++c) {
							uint32 component = inrow[c];
 ...
 */
static int png_image_read_background(void * argument)
{
	png_image_read_control * display = png_voidcast(png_image_read_control*,
	    argument);
	png_imagep image = display->image;
	...
	if((image->format & PNG_FORMAT_FLAG_LINEAR) == 0 && (image->format & PNG_FORMAT_FLAG_ALPHA) != 0)
	...
	switch(info_ptr->bit_depth) {
		     ...
		     */
	    {
		    ...
		    for(pass = 0; pass < passes; ++pass) {
			    ...
			    if(display->background == NULL) {
				    ...
				    uint16 background = png_sRGB_table[background8];
				    for(; y<height; y += stepy) {
					    png_bytep inrow = png_voidcast(png_bytep, display->local_row);
					    png_bytep outrow = first_row + y * step_row;
					    png_const_bytep end_row = outrow + width;
		     ...
		     */
	    {
		    ...
		    uint   outchannels = 1+preserve_alpha;
		    int    swap_alpha = 0;
			    ...
			    for(; y < height; y += stepy) {
				    ...
				    for(; outrow < end_row; outrow += stepx) {
					    ...
					    if(alpha > 0) { /* else 0 */
						    ...
						    component = 0;
					    outrow[swap_alpha] = (uint16)component;
					    if(preserve_alpha != 0)
...
static int png_image_read_direct(void * argument)
{
	...
	{
		uint32 base_format = png_image_format(png_ptr) &
		    ~PNG_FORMAT_FLAG_COLORMAP /* removed by png_set_expand */;
		uint32 change = format ^ base_format;
...
int PNGAPI png_image_finish_read(png_imagep image, png_const_colorp background, void * buffer, int32_t row_stride, void * colormap)
{
	if(image && image->version == PNG_IMAGE_VERSION) {
		...
		if(image->width <= 0x7FFFFFFFU/channels) { /* no overflow */
			...
			if(row_stride == 0)
				row_stride = (int32_t)/*SAFE*/ png_row_stride;
			if(row_stride < 0)
			...
			if(image->opaque && buffer && check >= png_row_stride) {
				...
				if(image->height <= 0xFFFFFFFF/png_row_stride) {
					if((image->format & PNG_FORMAT_FLAG_COLORMAP) == 0 || (image->colormap_entries > 0 && colormap)) {
						...
						return png_image_error(image, __FUNCTION__ "[color-map]: no color-map");
				}
				else
					return png_image_error(image, __FUNCTION__ ": image too large");
			}
			else
				return png_image_error(image, __FUNCTION__ ": invalid argument");
		}
		else
			return png_image_error(image, __FUNCTION__ ": row_stride too large");
	}
	else if(image)
		return png_image_error(image, __FUNCTION__ ": damaged PNG_IMAGE_VERSION");
	return 0;
}
