...
// to have been read in all cases; the need_IHDR parameter allows for this check too.
// 
static int FASTCALL png_rtran_ok(png_structrp png_ptr, int need_IHDR)
{
	if(png_ptr) {
...
void PNGFAPI png_set_alpha_mode_fixed(png_structrp png_ptr, int mode, png_fixed_point output_gamma)
{
	 ...
	 * the side effect that the gamma in a second call to png_set_alpha_mode will be ignored.)
	 */
	...
	if(compose != 0) {
		...
		memzero(&png_ptr->background, (sizeof png_ptr->background));
...
void PNGAPI png_set_quantize(png_structrp png_ptr, png_colorp palette, int num_palette, int maximum_colors, png_const_uint_16p histogram, int full_quantize)
{
	...
	if(full_quantize == 0) {
		int i;
		png_ptr->quantize_index = (png_bytep)png_malloc(png_ptr, (uint32)(num_palette * (sizeof(uint8))));
	...
	if(num_palette > maximum_colors) {
		if(histogram) {
			...
			png_ptr->quantize_sort = (png_bytep)png_malloc(png_ptr, (uint32)(num_palette * (sizeof(uint8))));
			/* Initialize the quantize_sort array */
			for(i = 0; i < num_palette; i++)
				png_ptr->quantize_sort[i] = (uint8)i;
			/* Find the least used palette entries by starting a
			 * bubble sort, and running it until we have sorted
			 * out enough colors.  Note that we don't care about
			 * sorting all the colors, just finding which are least used.
			 */
			for(i = num_palette - 1; i >= maximum_colors; i--) {
				int j;
				int done = 1; // To stop early if the list is pre-sorted 
				for(j = 0; j < i; j++) {
			...
			if(full_quantize != 0) {
				...
				for(i = 0; i < maximum_colors; i++) {
					...
					if((int)png_ptr->quantize_sort[i] >= maximum_colors) {
						...
						png_ptr->quantize_index[j] = (uint8)i;
						png_ptr->quantize_index[i] = (uint8)j;
					}
				}
				...
				for(i = 0; i < num_palette; i++) {
					if((int)png_ptr->quantize_index[i] >= maximum_colors) {
						...
						png_ptr->quantize_index[i] = (uint8)min_k;
					}
				}
			}
			...
			png_ptr->index_to_palette = (png_bytep)png_malloc(png_ptr,
			    (uint32)(num_palette * (sizeof(uint8))));
			...
			hash = (png_dsortpp)png_calloc(png_ptr, (uint32)(769 *
				    (sizeof(png_dsortp))));
	...
	if(full_quantize != 0) {
		...
		png_ptr->palette_lookup = (png_bytep)png_calloc(png_ptr, (uint32)(num_entries * (sizeof(uint8))));
		distance = (png_bytep)png_malloc(png_ptr, (uint32)(num_entries * (sizeof(uint8))));
		memset(distance, 0xff, num_entries * (sizeof(uint8)));
		for(i = 0; i < num_palette; i++) {
			int ir, ig, ib;
			int r = (palette[i].R >> (8 - PNG_QUANTIZE_RED_BITS));
			int g = (palette[i].G >> (8 - PNG_QUANTIZE_GREEN_BITS));
			int b = (palette[i].B >> (8 - PNG_QUANTIZE_BLUE_BITS));
...
static void /*PRIVATE*/ png_init_palette_transformations(png_structrp png_ptr)
{
	...
	if((png_ptr->transformations & PNG_BACKGROUND_EXPAND) != 0 && (png_ptr->transformations & PNG_EXPAND) != 0) {
		{
			png_ptr->background.red   = png_ptr->palette[png_ptr->background.index].R;
			png_ptr->background.green = png_ptr->palette[png_ptr->background.index].G;
			png_ptr->background.blue  = png_ptr->palette[png_ptr->background.index].B;
			...
			if((png_ptr->transformations & PNG_INVERT_ALPHA) != 0) {
				if((png_ptr->transformations & PNG_EXPAND_tRNS) == 0) {
					...
					for(i = 0; i<istop; i++)
						png_ptr->trans_alpha[i] = (uint8)(255 - png_ptr->trans_alpha[i]);
				}
			}
...
static void /*PRIVATE*/ png_init_rgb_transformations(png_structrp png_ptr)
{
	 ...
	 * because PNG_BACKGROUND_EXPAND is cancelled below.
	 */
	if((png_ptr->transformations & PNG_BACKGROUND_EXPAND) != 0 && (png_ptr->transformations & PNG_EXPAND) != 0 && (png_ptr->color_type & PNG_COLOR_MASK_COLOR) == 0) {
		/* i.e., GRAY or GRAY_ALPHA */
		{
			...
			png_ptr->background.red = png_ptr->background.green =
			    png_ptr->background.blue = (uint16)gray;

			if((png_ptr->transformations & PNG_EXPAND_tRNS) == 0) {
				png_ptr->trans_color.red = png_ptr->trans_color.green =
				    png_ptr->trans_color.blue = (uint16)trans_gray;
			}
		}
	} /* background expand and (therefore) no alpha association. */
#endif /* READ_EXPAND && READ_BACKGROUND */
...
void /*PRIVATE*/ png_init_read_transformations(png_structrp png_ptr)
{
	...
	if((png_ptr->transformations & PNG_EXPAND_16) != 0 &&
		 ...
		 */
#define CHOP(x) (x) = ((uint16)PNG_DIV257(x))
...
#     undef CHOP
	...
	if((png_ptr->transformations & (PNG_16_TO_8|PNG_SCALE_16_TO_8)) != 0 &&
		 ...
		 * @todo fix this too.
		 */
		png_ptr->background.red = (uint16)(png_ptr->background.red * 257);
		png_ptr->background.green =
		    (uint16)(png_ptr->background.green * 257);
		png_ptr->background.blue = (uint16)(png_ptr->background.blue * 257);
		png_ptr->background.gray = (uint16)(png_ptr->background.gray * 257);
	}
#endif
	...
	if((png_ptr->transformations & PNG_GAMMA) != 0 ||
		...
		if((png_ptr->transformations & PNG_COMPOSE) != 0) {
			...
			if((png_ptr->color_type == PNG_COLOR_TYPE_PALETTE) != 0) {
				...
				SColorRGB back, back_1;
				SColorRGB * palette = png_ptr->palette;
				int num_palette = png_ptr->num_palette;
				int i;
				if(png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_FILE) {
					back.R = png_ptr->gamma_table[png_ptr->background.red];
					back.G = png_ptr->gamma_table[png_ptr->background.green];
					back.B = png_ptr->gamma_table[png_ptr->background.blue];
					back_1.R = png_ptr->gamma_to_1[png_ptr->background.red];
					back_1.G = png_ptr->gamma_to_1[png_ptr->background.green];
					back_1.B = png_ptr->gamma_to_1[png_ptr->background.blue];
				}
				else {
					...
					if(png_gamma_significant(gs) != 0) {
						back.R = png_gamma_8bit_correct(png_ptr->background.red, gs);
						back.G = png_gamma_8bit_correct(png_ptr->background.green, gs);
						back.B = png_gamma_8bit_correct(png_ptr->background.blue, gs);
					}
					else {
						back.R   = (uint8)png_ptr->background.red;
						back.G = (uint8)png_ptr->background.green;
						back.B  = (uint8)png_ptr->background.blue;
					}
					if(png_gamma_significant(g) != 0) {
						back_1.R = png_gamma_8bit_correct(png_ptr->background.red, g);
						back_1.G = png_gamma_8bit_correct(png_ptr->background.green, g);
						back_1.B = png_gamma_8bit_correct(png_ptr->background.blue, g);
					}
					else {
						back_1.R = (uint8)png_ptr->background.red;
						back_1.G = (uint8)png_ptr->background.green;
						back_1.B = (uint8)png_ptr->background.blue;
					}
				}
				for(i = 0; i < num_palette; i++) {
					if(i < (int)png_ptr->num_trans && png_ptr->trans_alpha[i] != 0xff) {
						if(png_ptr->trans_alpha[i] == 0) {
							...
							palette[i].R = png_ptr->gamma_from_1[w];
							v = png_ptr->gamma_to_1[palette[i].G];
							png_composite(w, v, png_ptr->trans_alpha[i], back_1.G);
							palette[i].G = png_ptr->gamma_from_1[w];
							v = png_ptr->gamma_to_1[palette[i].B];
							png_composite(w, v, png_ptr->trans_alpha[i], back_1.B);
							palette[i].B = png_ptr->gamma_from_1[w];
						}
					}
					else {
						palette[i].R = png_ptr->gamma_table[palette[i].R];
						palette[i].G = png_ptr->gamma_table[palette[i].G];
						palette[i].B = png_ptr->gamma_table[palette[i].B];
					}
				}
	...
	if((png_ptr->transformations & PNG_COMPOSE) != 0 &&
		...
		for(i = 0; i < istop; i++) {
			if(png_ptr->trans_alpha[i] == 0) {
				...
				png_composite(palette[i].R, palette[i].R, png_ptr->trans_alpha[i], back.R);
				png_composite(palette[i].G, palette[i].G, png_ptr->trans_alpha[i], back.G);
				png_composite(palette[i].B, palette[i].B, png_ptr->trans_alpha[i], back.B);
			}
		}
		png_ptr->transformations &= ~PNG_COMPOSE;
	}
	...
	if((png_ptr->transformations & PNG_SHIFT) != 0 && (png_ptr->transformations & PNG_EXPAND) == 0 && (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE)) {
		...
		if(shift > 0 && shift < 8)
			for(i = 0; i<istop; ++i) {
				int component = png_ptr->palette[i].R;
				component >>= shift;
				png_ptr->palette[i].R = (uint8)component;
			}

		shift = 8 - png_ptr->sig_bit.green;
		if(shift > 0 && shift < 8)
			for(i = 0; i<istop; ++i) {
				int component = png_ptr->palette[i].G;
				component >>= shift;
				png_ptr->palette[i].G = (uint8)component;
			}

		shift = 8 - png_ptr->sig_bit.blue;
		if(shift > 0 && shift < 8)
			for(i = 0; i<istop; ++i) {
				int component = png_ptr->palette[i].B;
				component >>= shift;
				png_ptr->palette[i].B = (uint8)component;
			}
	}
#endif /* READ_SHIFT */
}
...
#ifdef PNG_READ_GRAY_TO_RGB_SUPPORTED
	if((png_ptr->transformations & PNG_GRAY_TO_RGB) != 0)
		info_ptr->color_type = (uint8)(info_ptr->color_type | PNG_COLOR_MASK_COLOR);
#endif
#ifdef PNG_READ_RGB_TO_GRAY_SUPPORTED
	if((png_ptr->transformations & PNG_RGB_TO_GRAY) != 0)
		info_ptr->color_type = (uint8)(info_ptr->color_type & ~PNG_COLOR_MASK_COLOR);
...
#ifdef PNG_READ_STRIP_ALPHA_SUPPORTED
	if((png_ptr->transformations & PNG_STRIP_ALPHA) != 0) {
		info_ptr->color_type = (uint8)(info_ptr->color_type &
...
static void png_do_read_filler(png_row_infop row_info, png_bytep row, uint32 filler, uint32 flags)
{
	uint32 i;
	uint32 row_width = row_info->width;
#ifdef PNG_READ_16BIT_SUPPORTED
	uint8 hi_filler = (uint8)(filler>>8);
#endif
	uint8 lo_filler = (uint8)filler;
	png_debug(1, "in " __FUNCTION__);
	if(
...
static void png_do_gray_to_rgb(png_row_infop row_info, png_bytep row)
{
	...
	if(row_info->bit_depth >= 8 && (row_info->color_type & PNG_COLOR_MASK_COLOR) == 0) {
		...
		row_info->channels = (uint8)(row_info->channels + 2);
...

{
	...
	if((row_info->color_type & PNG_COLOR_MASK_PALETTE) == 0 &&
		...
		const uint32 rc = png_ptr->rgb_to_gray_red_coeff;
		const uint32 gc = png_ptr->rgb_to_gray_green_coeff;
		const uint32 bc = 32768 - rc - gc;
		const uint32 row_width = row_info->width;
		const int have_alpha = (row_info->color_type & PNG_COLOR_MASK_ALPHA) != 0;
		if(row_info->bit_depth == 8) {
#ifdef PNG_READ_GAMMA_SUPPORTED
			...
			if(png_ptr->gamma_16_to_1 && png_ptr->gamma_16_from_1) {
				...
				for(i = 0; i < row_width; i++) {
					...
					if(red == green && red == blue) {
						...
						uint16 red_1   = png_ptr->gamma_16_to_1[(red & 0xff) >> png_ptr->gamma_shift][red>>8];
						uint16 green_1 = png_ptr->gamma_16_to_1[(green & 0xff) >> png_ptr->gamma_shift][green>>8];
						uint16 blue_1  = png_ptr->gamma_16_to_1[(blue & 0xff) >> png_ptr->gamma_shift][blue>>8];
						uint16 gray16  = (uint16)((rc*red_1 + gc*green_1 + bc*blue_1 + 16384)>>15);
...
#endif
			{
				...
				for(i = 0; i < row_width; i++) {
					 ...
					 */
					gray16  = (uint16)((rc*red + gc*green + bc*blue + 16384) >> 15);
		...
		row_info->channels = (uint8)(row_info->channels - 2);
		row_info->color_type = (uint8)(row_info->color_type & ~PNG_COLOR_MASK_COLOR);
		row_info->pixel_depth = (uint8)(row_info->channels * row_info->bit_depth);
		row_info->rowbytes = PNG_ROWBYTES(row_info->pixel_depth, row_width);
	}
	return rgb_error;
}
		...
		switch(row_info->color_type) {
			case PNG_COLOR_TYPE_GRAY:
		    {
				    ...
				    case 1:
				{
					...
					shift = 7;
					for(i = 0; i < row_width; i++) {
						if((uint16)((*sp >> shift) & 0x01) == png_ptr->trans_color.gray) {
							uint tmp = *sp & (0x7f7f >> (7 - shift));
				    ...
				    case 2:
				{
					...
					if(gamma_table) {
						...
						shift = 6;
						for(i = 0; i < row_width; i++) {
							if((uint16)((*sp >> shift) & 0x03) == png_ptr->trans_color.gray) {
								uint tmp = *sp & (0x3f3f >> (6 - shift));
								tmp |= png_ptr->background.gray << shift;
								*sp = (uint8)(tmp & 0xff);
							}
							else {
								uint p = (*sp >> shift) & 0x03;
								uint g = (gamma_table [p | (p << 2) | (p << 4) | (p << 6)] >> 6) & 0x03;
								uint tmp = *sp & (0x3f3f >> (6 - shift));
								tmp |= g << shift;
								*sp = (uint8)(tmp & 0xff);
							}
				    ...
				    case 4:
				{
					...
					if(gamma_table) {
						...
						shift = 4;
						for(i = 0; i < row_width; i++) {
							if((uint16)((*sp >> shift) & 0x0f) == png_ptr->trans_color.gray) {
								uint tmp = *sp & (0x0f0f >> (4 - shift));
								tmp |= png_ptr->background.gray << shift;
								*sp = (uint8)(tmp & 0xff);
							}
							else {
								uint p = (*sp >> shift) & 0x0f;
								uint g = (gamma_table[p | (p << 4)] >> 4) & 0x0f;
								uint tmp = *sp & (0x0f0f >> (4 - shift));
								tmp |= g << shift;
								*sp = (uint8)(tmp & 0xff);
							}
				    ...
				    if(gamma_16) {
					    ...
					    for(i = 0; i < row_width; i++, sp += 6) {
						    ...
						    if(r == png_ptr->trans_color.red && g == png_ptr->trans_color.green && b == png_ptr->trans_color.blue) {
							    ...
							    *(sp + 3) = (uint8)(png_ptr->background.green & 0xff);
							    *(sp + 4) = (uint8)((png_ptr->background.blue >> 8) & 0xff);
							    *(sp + 5) = (uint8)(png_ptr->background.blue & 0xff);
						    }
			    ...
			    if(row_info->bit_depth == 8) {
				    ...
				    if(gamma_16 && gamma_16_from_1 && gamma_16_to_1) {
					    ...
					    for(i = 0; i < row_width; i++, sp += 8) {
						    uint16 a = (uint16)(((uint16)(*(sp + 6)) << 8) + (uint16)(*(sp + 7)));
						    if(a == (uint16)0xffff) {
							    uint16 v = gamma_16[*(sp + 1) >> gamma_shift][*sp];
							    *sp = (uint8)((v >> 8) & 0xff);
							    *(sp + 1) = (uint8)(v & 0xff);
		...
		if(row_info->color_type == PNG_COLOR_TYPE_GRAY) {
			...
			if(trans_color) {
				if(row_info->bit_depth == 8) {
					...
					uint gray_high = (gray >> 8) & 0xff;
					uint gray_low = gray & 0xff;
			...
			if(row_info->bit_depth == 8) {
				...
				uint8 blue = (uint8)(trans_color->blue & 0xff);
				...
				uint8 red_high = (uint8)((trans_color->red >> 8) & 0xff);
				uint8 green_high = (uint8)((trans_color->green >> 8) & 0xff);
				uint8 blue_high = (uint8)((trans_color->blue >> 8) & 0xff);
				uint8 red_low = (uint8)(trans_color->red & 0xff);
				uint8 green_low = (uint8)(trans_color->green & 0xff);
				uint8 blue_low = (uint8)(trans_color->blue & 0xff);
				sp = row + row_info->rowbytes - 1;
				dp = row + (size_t)(row_width << 3) - 1;
				for(i = 0; i < row_width; i++) {
...
static void png_do_expand_16(png_row_infop row_info, png_bytep row)
{
	if(row_info->bit_depth == 8 && row_info->color_type != PNG_COLOR_TYPE_PALETTE) {
		 ...
		 */
		uint8 * sp = row + row_info->rowbytes; /* source, last byte + 1 */
		uint8 * dp = sp + row_info->rowbytes; /* destination, end + 1 */
		while(dp > sp)
			dp[-2] = dp[-1] = *--sp, dp -= 2;
		row_info->rowbytes *= 2;
		row_info->bit_depth = 16;
		row_info->pixel_depth = (uint8)(row_info->channels * 16);
	}
}
...
void /*PRIVATE*/ png_do_read_transformations(png_structrp png_ptr, png_row_infop row_info)
{
	...
	if((png_ptr->transformations & PNG_EXPAND) != 0) {
		if(row_info->color_type == PNG_COLOR_TYPE_PALETTE) {
			png_do_expand_palette(row_info, png_ptr->row_buf + 1, png_ptr->palette, png_ptr->trans_alpha, png_ptr->num_trans);
		}
		else {
	...
	if((png_ptr->transformations & PNG_STRIP_ALPHA) != 0 && (png_ptr->transformations & PNG_COMPOSE) == 0 &&
	    oneof2(row_info->color_type, PNG_COLOR_TYPE_RGB_ALPHA, PNG_COLOR_TYPE_GRAY_ALPHA))
