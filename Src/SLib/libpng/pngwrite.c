...
/* Write uint16 input to a 16-bit PNG; the png_ptr has already been set to
...
static int png_write_image_16bit(void * argument)
{
	...
	while(y-- > 0) {
		...
		while(out_ptr < row_end) {
			const uint16 alpha = in_ptr[aindex];
			uint32 reciprocal = 0;
			int c;
			...
			do { /* always at least one channel */
				 ...
				 */
					...
					uint32 calc = component * reciprocal;
					calc += 16384; /* round to nearest */
					component = (uint16)(calc >> 15);
				}
				*out_ptr++ = component;
			} while(--c > 0);
		...
		input_row += display->row_bytes/(sizeof(uint16));
	}
	return 1;
}
...
static uint8 png_unpremultiply(uint32 component, uint32 alpha, uint32 reciprocal /*from the above macro*/)
{
	 ...
	 */
		...
		return (uint8)PNG_sRGB_FROM_LINEAR(component);
	}
...
static int png_write_image_8bit(void * argument)
{
	png_image_write_control * display = png_voidcast(png_image_write_control*, argument);
	png_imagep image = display->image;
	png_structrp png_ptr = image->opaque->png_ptr;
	...
	if((image->format & PNG_FORMAT_FLAG_ALPHA) != 0) {
		...
		while(y-- > 0) {
			...
			png_bytep out_ptr = output_row;
			while(out_ptr < row_end) {
				uint32 component = *in_ptr++;
				component *= 255;
				*out_ptr++ = (uint8)PNG_sRGB_FROM_LINEAR(component);
			}
	...
	for(i = num_trans = 0; i<entries; ++i) {
		...
		if((format & PNG_FORMAT_FLAG_LINEAR) != 0) {
			...
			if((channels & 1) != 0) { /* no alpha */
				...
				uint16 alpha = entry[afirst ? 0 : channels-1];
				uint8 alphabyte = (uint8)PNG_DIV257(alpha);
				uint32 reciprocal = 0;
	 ...
	 */
	{
		...
		if(image->width <= 0x7FFFFFFFU/channels) { /* no overflow */
			...
			if(display->row_stride == 0)
				display->row_stride = (int32_t)/*SAFE*/ png_row_stride;
			if(display->row_stride < 0)
...
int PNGAPI png_image_write_to_stdio(png_imagep image, FILE * file, int convert_to_8bit, const void * buffer, int32_t row_stride, const void * colormap)
{
...
int PNGAPI png_image_write_to_file(png_imagep image, const char * file_name,
    int convert_to_8bit, const void * buffer, int32_t row_stride, const void * colormap)
{
