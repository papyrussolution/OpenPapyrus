...
void /*PRIVATE*/ png_reset_crc(png_structrp png_ptr)
{
	...
	png_ptr->crc = (uint32)crc32(0, Z_NULL, 0);
}
 ...
 */
void /*PRIVATE*/ png_calculate_crc(png_structrp png_ptr, png_const_bytep ptr, size_t length)
{
	int need_crc = 1;
...
void PNGAPI png_data_freer(png_const_structrp png_ptr, png_inforp info_ptr, int freer, uint32 mask)
{
	png_debug(1, "in " __FUNCTION__);
...
void PNGAPI png_free_data(png_const_structrp png_ptr, png_inforp info_ptr, uint32 mask, int num)
{
	png_debug(1, "in " __FUNCTION__);
	if(png_ptr == NULL || info_ptr == NULL)
		return;
	...
	if(info_ptr->text != 0 && ((mask & PNG_FREE_TEXT) & info_ptr->free_me) != 0) {
		if(num != -1) {
			...
			info_ptr->text = NULL;
			info_ptr->num_text = 0;
			info_ptr->max_text = 0; // libpng-v-1629
		}
	}
#endif
 ...
 */
void PNGAPI png_save_int_32(png_bytep buf, int32_t i)
{
...
int /*PRIVATE*/ png_chunk_unknown_handling(png_const_structrp png_ptr, uint32 chunk_name)
{
	uint8 chunk_string[5];
...
void /*PRIVATE*/ png_colorspace_set_gamma(png_const_structrp png_ptr, png_colorspacerp colorspace, png_fixed_point gAMA)
{
...
void /*PRIVATE*/ png_colorspace_sync_info(png_const_structrp png_ptr, png_inforp info_ptr)
{
	if((info_ptr->colorspace.flags & PNG_COLORSPACE_INVALID) != 0) {
		...
		SETFLAG(info_ptr->valid, PNG_INFO_sRGB, (info_ptr->colorspace.flags & PNG_COLORSPACE_MATCHES_sRGB));
		SETFLAG(info_ptr->valid, PNG_INFO_cHRM, (info_ptr->colorspace.flags & PNG_COLORSPACE_HAVE_ENDPOINTS));
#endif
		SETFLAG(info_ptr->valid, PNG_INFO_gAMA, (info_ptr->colorspace.flags & PNG_COLORSPACE_HAVE_GAMMA));
	}
}
	...
	{
		if(info_ptr) { // reduce code size; check here not in the caller
...
{
	int32_t dwhite, whiteX, whiteY;
	int32_t d = XYZ->red_X + XYZ->red_Y + XYZ->red_Z;
...
int /*PRIVATE*/ png_colorspace_set_chromaticities(png_const_structrp png_ptr, png_colorspacerp colorspace, const png_xy * xy, int preferred)
{
...
static char png_icc_tag_char(uint32 byte)
{
	byte &= 0xff;
	if(byte >= 32 && byte <= 126)
		return (char)byte;
...
static void png_icc_tag_name(char * name, uint32 tag)
{
	name[0] = '\'';
...
static int png_icc_profile_error(png_const_structrp png_ptr, png_colorspacerp colorspace,
    const char * name, png_alloc_size_t value, const char * reason)
{
	size_t pos;
	...
	if(is_ICC_signature(value) != 0) {
		...
		png_icc_tag_name(message+pos, (uint32)value);
...
int /*PRIVATE*/ png_colorspace_set_sRGB(png_const_structrp png_ptr, png_colorspacerp colorspace, int intent)
{
...
#ifdef PNG_iCCP_SUPPORTED
#if 0 // @libpng-1629 {
int /*PRIVATE*/ png_icc_check_length(png_const_structrp png_ptr, png_colorspacerp colorspace, const char * name, uint32 profile_length)
{
...
int /*PRIVATE*/ png_icc_check_header(png_const_structrp png_ptr, png_colorspacerp colorspace,
    const char * name, uint32 profile_length,
...
{
	uint32 tag_count = png_get_uint_32(profile+128);
	uint32 itag;
	...
	for(itag = 0; itag < tag_count; ++itag, tag += 12) {
		uint32 tag_id = png_get_uint_32(tag+0);
		uint32 tag_start = png_get_uint_32(tag+4); /* must be aligned */
		uint32 tag_length = png_get_uint_32(tag+8); /* not padded */
...
} png_sRGB_checks[] =
{
	...
	PNG_ICC_CHECKSUM(0x0a3fd9f6, 0x3b8772b9, PNG_MD5(0x29f83dde, 0xaff255ae, 0x7842fae4, 0xca83390d), 0, 0, "2009/03/27 21:36:31", 3048, "sRGB_IEC61966-2-1_black_scaled.icc")
	/* ICC sRGB v2 perceptual no black-compensation: */
	PNG_ICC_CHECKSUM(0x4909e5e1, 0x427ebb21, PNG_MD5(0xc95bd637, 0xe95d8a3b, 0x0df38f99, 0xc1320389), 1, 0, "2009/03/27 21:37:45", 3052, "sRGB_IEC61966-2-1_no_black_scaling.icc")
	PNG_ICC_CHECKSUM(0xfd2144a1, 0x306fd8ae, PNG_MD5(0xfc663378, 0x37e2886b, 0xfd72e983, 0x8228f1b8), 0, 0, "2009/08/10 17:28:01", 60988, "sRGB_v4_ICC_preference_displayclass.icc")
	/* ICC sRGB v4 perceptual */
	PNG_ICC_CHECKSUM(0x209c35d2, 0xbbef7812, PNG_MD5(0x34562abf, 0x994ccd06, 0x6d2c5721, 0xd0d68c5d), 0, 0, "2007/07/25 00:05:37", 60960, "sRGB_v4_ICC_preference.icc")
	/* The following profiles have no known MD5 checksum. If there is a match
	 * on the (empty) MD5 the other fields are used to attempt a match and
	 * a warning is produced.  The first two of these profiles have a 'cprt' tag
	 * which suggests that they were also made by Hewlett Packard.
	 */
	PNG_ICC_CHECKSUM(0xa054d762, 0x5d5129ce, PNG_MD5(0x00000000, 0x00000000, 0x00000000, 0x00000000), 1, 0, "2004/07/21 18:57:42", 3024, "sRGB_IEC61966-2-1_noBPC.icc")
	 ...
	 */
	PNG_ICC_CHECKSUM(0xf784f3fb, 0x182ea552, PNG_MD5(0x00000000, 0x00000000, 0x00000000, 0x00000000), 0, 1 /*broken*/, "1998/02/09 06:49:00", 3144, "HP-Microsoft sRGB v2 perceptual")
	PNG_ICC_CHECKSUM(0x0398f3fc, 0xf29e526d, PNG_MD5(0x00000000, 0x00000000, 0x00000000, 0x00000000), 1, 1 /*broken*/, "1998/02/09 06:49:00", 3144, "HP-Microsoft sRGB v2 media-relative")
};
...
void /*PRIVATE*/ png_icc_set_sRGB(png_const_structrp png_ptr, png_colorspacerp colorspace, png_const_bytep profile, uLong adler)
{
...
int /*PRIVATE*/ png_colorspace_set_ICC(png_const_structrp png_ptr, png_colorspacerp colorspace,
    const char * name, uint32 profile_length, png_const_bytep profile, int color_type)
{
	if((colorspace->flags & PNG_COLORSPACE_INVALID) != 0)
		return 0;
...
// @libpng-1629 #ifdef PNG_sRGB_SUPPORTED
#if defined(PNG_sRGB_SUPPORTED) && (PNG_sRGB_PROFILE_CHECKS >= 0) // @libpng-1629 
	...
	if(png_ptr->rgb_to_gray_coefficients_set == 0 &&
		...
		if(total > 0 &&
			...
			if(r+g+b != 32768)
				...
				png_ptr->rgb_to_gray_red_coeff   = (uint16)r;
				png_ptr->rgb_to_gray_green_coeff = (uint16)g;
			}
		}
...
void /*PRIVATE*/ png_check_IHDR(png_const_structrp png_ptr, uint32 width, uint32 height, int bit_depth,
    int color_type, int interlace_type, int compression_type, int filter_type)
{
	...
	if(png_gt(((width + 7) & (~7U)), ((PNG_SIZE_MAX
		-48 /* big_row_buf hack */
		-1) /* filter byte */
...
int /*PRIVATE*/ png_check_fp_number(const char * string, size_t size, int * statep, png_size_tp whereami)
{
	int state = *statep;
	size_t i = *whereami;
	while(i < size) {
...
int png_check_fp_string(const char * string, size_t size)
{
	int state = 0;
	size_t char_index = 0;
		...
		if(fp >= DBL_MIN && fp <= DBL_MAX) {
			 ...
			 */
			{
				...
				if(exp_b10 < 0 && exp_b10 > -3) { /* PLUS 3 TOTAL 4 */
					czero = (uint)(-exp_b10); // PLUS 2 digits: TOTAL 3 
...
//
void /*PRIVATE*/ png_ascii_from_fixed(png_const_structrp png_ptr, char * ascii, size_t size, png_fixed_point fp)
{
 ...
 */
int STDCALL png_muldiv(png_fixed_point_p res, png_fixed_point a, int32_t times, int32_t divisor)
{
	...
	png_fixed_point png_muldiv_warn(png_const_structrp png_ptr, png_fixed_point a, int32_t times, int32_t divisor)
	{
...
static int32_t png_log8bit(uint x)
{
	uint lg2 = 0;
	...
	return (int32_t)((lg2 << 16) + ((png_8bit_l2[x-128]+32768)>>16));
}
 ...
 */
#ifdef PNG_16BIT_SUPPORTED
static int32_t png_log16bit(uint32 x)
{
	uint lg2 = 0;
...
static uint32 png_exp(png_fixed_point x)
{
	if(x > 0 && x <= 0xfffff) { /* Else overflow or zero (underflow) */
		/* Obtain a 4-bit approximation */
		uint32 e = png_32bit_exp[(x >> 12) & 0x0f];
...
static uint8 png_exp8bit(png_fixed_point lg2)
{
	/* Get a 32-bit value: */
	uint32 x = png_exp(lg2);
	 ...
	 */
	x -= x >> 8;
	return (uint8)(((x + 0x7fffffU) >> 24) & 0xff);
}

#ifdef PNG_16BIT_SUPPORTED
static uint16 png_exp16bit(png_fixed_point lg2)
{
	/* Get a 32-bit value: */
	uint32 x = png_exp(lg2);
...
#ifdef PNG_16BIT_SUPPORTED
uint16 png_gamma_16bit_correct(uint value, png_fixed_point gamma_val)
{
	if(value > 0 && value < 65535) {
		 ...
		 */
		double r = floor(65535*pow((int32_t)value/65535.,
			    gamma_val*.00001)+.5);
		return (uint16)r;
#else
		int32_t lg2 = png_log16bit(value);
		png_fixed_point res;
 ...
 */
uint16 /*PRIVATE*/ png_gamma_correct(png_structrp png_ptr, uint value, png_fixed_point gamma_val)
{
...
static void png_build_16bit_table(png_structrp png_ptr, png_uint_16pp * ptable, const uint shift, const png_fixed_point gamma_val)
{
	 ...
	 */
	const double fmax = 1./(((int32_t)1 << (16U - shift))-1);
#endif
	...
	for(i = 0; i < num; i++) {
		png_uint_16p sub_table = table[i] = (png_uint_16p)png_malloc(png_ptr, 256 * (sizeof(uint16)));
...
static void png_build_16to8_table(png_structrp png_ptr, png_uint_16pp * ptable, const uint shift, const png_fixed_point gamma_val)
{
	 ...
	 */
	for(i = 0; i < num; i++)
		table[i] = (png_uint_16p)png_malloc(png_ptr, 256 * (sizeof(uint16)));
	...
	for(i = 0; i < 255; ++i) { /* 8-bit output value */
		...
		uint16 out = (uint16)(i * 257U); /* 16-bit output value */

		/* Find the boundary value in 16 bits: */
		uint32 bound = png_gamma_16bit_correct(out+128U, gamma_val);
...
void /*PRIVATE*/ png_build_gamma_table(png_structrp png_ptr, int bit_depth)
{
	...
	if(bit_depth <= 8) {
		...
		if(sig_bit > 0 && sig_bit < 16U)
			shift = (uint8)((16U - sig_bit) & 0xff); /* shift == insignificant bits */
		else
		...
		if((png_ptr->transformations & (PNG_16_TO_8 | PNG_SCALE_16_TO_8)) != 0) {
			 ...
			 */
			SETMAX(shift, (16U - PNG_MAX_GAMMA_8));
		}
		SETMIN(shift, 8U); // Guarantees at least one table! 
		 ...
		 */
		if((png_ptr->transformations & (PNG_16_TO_8 | PNG_SCALE_16_TO_8)) != 0)
			png_build_16to8_table(png_ptr, &png_ptr->gamma_16_table, shift, png_ptr->screen_gamma > 0 ? png_product2(png_ptr->colorspace.gamma, png_ptr->screen_gamma) : PNG_FP_1);
		else
			png_build_16bit_table(png_ptr, &png_ptr->gamma_16_table, shift, png_ptr->screen_gamma > 0 ? png_reciprocal2(png_ptr->colorspace.gamma, png_ptr->screen_gamma) : PNG_FP_1);
#if defined(PNG_READ_BACKGROUND_SUPPORTED) || defined(PNG_READ_ALPHA_MODE_SUPPORTED) || defined(PNG_READ_RGB_TO_GRAY_SUPPORTED)
		if((png_ptr->transformations & (PNG_COMPOSE | PNG_RGB_TO_GRAY)) != 0) {
...
const uint16 png_sRGB_table[256] =
{
 ...
 */
const uint16 png_sRGB_base[512] =
{
...
const uint8 png_sRGB_delta[512] =
{
...
static int png_image_free_function(void * argument)
{
...
void PNGAPI png_image_free(png_imagep image)
{
	...
	if(image && image->opaque && image->opaque->error_buf == NULL) {
		// Ignore errors here:
		// @libpng(1.6.37) (void)png_safe_execute(image, png_image_free_function, image);
		png_image_free_function(image); // @libpng(1.6.37) 
		image->opaque = NULL;
	}
}
