...
#     endif
		...
		for(y = 0; y < pass_height; y++) {
			...
			row_buf = (uint8 *)png_malloc(read_ptr, png_get_rowbytes(read_ptr, read_info_ptr));
			pngtest_debug2("\t0x%08lx (%lu bytes)", (ulong)row_buf,
			    (ulong)png_get_rowbytes(read_ptr, read_info_ptr));
#endif /* !SINGLE_ROWBUF_ALLOC */
...
int main(int argc, char * argv[])
{
	...
	fprintf(STDERR, " library (%lu):%s", (ulong)png_access_version_number(), png_get_header_version(NULL));
	/* Show the version of libpng used in building the application */
	fprintf(STDERR, " pngtest (%lu):%s", (ulong)PNG_LIBPNG_VER, PNG_HEADER_VERSION_STRING);
...
int main(void)
{
	fprintf(STDERR, " test ignored because libpng was not built with read support\n");
	/* And skip this test */
	return SKIP;
}
