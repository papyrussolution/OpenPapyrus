...
int SymbHashTable::Serialize(int dir, SBuffer & rBuf, SSerializeContext * pCtx)
{
	...
	THROW(InitTab()); // Инициализировать таблицу необходимо и при записи в поток. Поэтому, InitTab вызывается независимо от значения dir
	for(uint i = 0; i < Size; ++i) {
...
#define HASH_MUR(key, keylen, num_bkts, hashv, bkt)			   \
	do {								       \
		...
		const uint32_t _mur_c2 = SlConst::MagicMurmurC2; \
		uint32_t _mur_k1 = 0;						     \
