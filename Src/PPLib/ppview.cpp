// PPVIEW.CPP
// Copyright (c) A.Sobolev 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2025, 2026
// @codepage UTF-8
//
#include <pp.h>
#pragma hdrstop
	...
	if(!pNf)
		...
		if(!dl600_name.NotEmptyS()) {
			if(p_view->DefReportId) {
				THROW(SReport::GetReportAttributes(p_view->DefReportId, 0, &dl600_name));
			}
		}
		{
			DlContext ctx;
...
#define CTL_TOOLBAR_INPUTLI (WINDOWS_ID_BIAS + 1)
	...
	if(parent) {
		...
		hwnd_li = ::CreateWindowExW(0, L"EDIT", 0, style|ES_AUTOHSCROLL|ES_READONLY,
			rect.left, rect.top, rect.right, rect.bottom, parent, (HMENU)CTL_TOOLBAR_INPUTLI, TProgram::GetInst(), 0);
		::CreateWindowExW(0, L"BUTTON", 0, style|BS_PUSHBUTTON|BS_BITMAP|BS_FLAT, rect.left + rect.right - 1,
