...
int PPViewSysJournal::SerializeState(int dir, SBuffer & rBuf, SSerializeContext * pCtx)
{
	...
	THROW_SL(pCtx->Serialize(dir, &EvVerList, rBuf));
	CATCHZOK
	return ok;
}
				...
				while(P_IterQuery->nextIteration() > 0) {
					...
					if(Filt.Sgsj != sgsjNone || Filt.Sgd != sgdNone) {
						if(CheckRecForFilt(&r_rec) > 0) {
							...
							P_Tbl->Subst(Filt.Sgsj, r_rec.Action, r_rec.UserID, r_rec.ObjType, &temp_rec.ID);
					 		if(Filt.Sgd != sgdNone) {
								...
								StrPool.AddS(temp_buf, &temp_rec.DtSubstP);
								dt.v = (Filt.Sgd == sgdHour) ? tm.v : dt.v;
								temp_rec.Dt  = dt;
								temp_rec.ID2 = dt.v;
					 		}
							else {
								temp_buf.Z().Cat(temp_rec.Dt, DATF_DMY);
								StrPool.AddS(temp_buf, &temp_rec.DtSubstP);
								temp_rec.Dt = r_rec.Dt;
							}
							P_Tbl->GetSubstName(Filt.Sgsj, temp_rec.ID, temp_buf);
							StrPool.AddS(temp_buf, &temp_rec.NameP);
							k0.ID  = temp_rec.ID;
							k0.ID2 = temp_rec.ID2;
							temp_rec.Count = 1;
						...
						if(Filt.Flags & Filt.fShowObjects) {
							if(!oneof2(r_rec.Action, PPACN_OBJRMV, PPACN_RMVBILL)) {
								PPObjNamePEntry objn_entry(r_rec.ObjType, r_rec.ObjID);
								if(objn_entry.Obj) {
									objn_entry.NameP = 0;
									uint   objn_pos = 0;
						...
						THROW_DB(!p_bei || p_bei->insert(&r_rec));
					}
					Counter.Increment();
...
int FASTCALL PPViewSysJournal::NextIteration(SysJournalViewItem * pItem)
{
	...
	while(pItem && P_IterQuery && P_IterQuery->nextIteration() > 0) {
		if(P_SubstTbl) {
			...
			StrPool.GetS(P_SubstTbl->data.DtSubstP, pItem->GrpText1);
			StrPool.GetS(P_SubstTbl->data.NameP, pItem->ObjName);
			{
...
int PPViewSysJournal::Detail(const void * pHdr, PPViewBrowser * pBrw)
{
	...
	if(Filt.Sgsj != sgsjNone || Filt.Sgd != sgdNone) {
		...
		ObjTagItem tag_item;
		if(PPRef->Ot.GetTag(hdr.Obj, hdr.Id, static_cast<PPID>(hdr.Extra), &tag_item) > 0) {
		...
		if(InitIteration()) {
			...
			}
			{
				...
				while(P_IterQuery->nextIteration() > 0) {
					...
					THROW(PPCheckUserBreak());
					if(Filt.Flags & Filt.fShowObjects) {
						PPObjNamePEntry objn_entry(T.data.ObjType, T.data.ObjID);
						if(objn_entry.Obj) {
							objn_entry.NameP = 0;
							uint   objn_pos = 0;
							if(!ObjNameList.bsearch(&objn_entry, &objn_pos, PTR_CMPFUNC(PPObjID))) {
					...
					THROW_DB(!p_bei || p_bei->insert(&T.data));
					Counter.Increment();
					PPWaitPercent(Counter);
				}
				THROW_DB(!p_bei || p_bei->flash());
				THROW(tra.Commit());
			}
		}
	}
