...
SLAPI PPViewLinkedBill::PPViewLinkedBill() : PPView(0, &Filt, PPVIEW_LINKEDBILL)
{
...
int SLAPI PPViewLinkedBill::PreprocessBrowser(PPViewBrowser * pBrw)
{
	int    ok = -1;
	if(pBrw) {
		pBrw->SetDefUserProc(PPViewLinkedBill::GetDataForBrowser, this);
		ok = 1;
	}
	return ok;
}
...
int CfmReckoningDialog::updateList()
{
	if(P_List && Data.P_BillList) {
		...
		for(uint i = 0; i < Data.P_BillList->getCount(); i++) {
			...
			ss.clear(1);
			if(P_BObj->Fetch(bill_id, &bill_rec) > 0) { // @v7.0.6 Search-->Fetch
...
int PPALDD_PaymBillList::Destroy()
{
