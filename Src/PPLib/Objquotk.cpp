...
int PPObjQuotKind::GetPacket(PPID id, PPQuotKindPacket * pPack)
{
	...
	{
		SString text_buf;
		THROW(P_Ref->UtrC.SearchUtf8(SObjTextRefIdent(Obj, id, PPTRPROP_QUOTKIND), text_buf));
		...
		if(*pID) {
			if(pPack) {
				...
				if(!IsPacketEq(*pPack, org_pack, 0)) {
					...
					THROW(P_Ref->UtrC.SetTextUtf8(SObjTextRefIdent(Obj, *pID, PPTRPROP_QUOTKIND), ext_buffer.Transf(CTRANSF_INNER_TO_UTF8), 0));
					// } @v12.0.10 
				}
				// @v12.3.6 {
				else
				...
				THROW(CheckRights(PPR_DEL));
				{
					...
					THROW(P_Ref->UtrC.Remove(SObjTextRefIdent(Obj, *pID, PPTRPROP_QUOTKIND), 0)); // @v12.0.10
...
/*virtual*/int  PPObjQuotKind::RemoveObjV(PPID id, ObjCollection * pObjColl, uint options/* = rmv_default*/, void * pExtraParam)
{
	...
	if(!(options & PPObject::user_request) || PPMessage(mfConf|mfYesNo, PPCFM_DELQUOTKIND) == cmYes) {
		...
		PPWaitStart();
		{
			...
			THROW(P_Ref->UtrC.Remove(SObjTextRefIdent(Obj, id, PPTRPROP_QUOTKIND), 0)); // @v12.0.10
			THROW(RemoveSync(id));
			THROW(tra.Commit());
		}
		r = 1;
