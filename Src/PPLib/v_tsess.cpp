			...
			if(TSesObj.GetPacket(_id, &pack, 0) > 0) {
				...
				if(item.PrcID && TSesObj.GetPrc(item.PrcID, &prc_rec, 0, 0) > 0) {
					...
					if(prc_rec.Code[0] == 0) {
						...
						// Ќе удалось идентифицировать процессор сессии по коду
					}
				}
			}
			else {
...
int SLAPI PPViewTSessLine::CreateIterQuery()
{
	...
	if(P_TempTbl) {
		if(P_IterQuery == 0) {
			...
			//
			// ≈сли функци€ NextIteration попытаетс€ снова вызвать
			// создание запроса, то мы ответим ей отказом и таким образом
			// избежим зацикливани€.
			// ѕри этом мы полагаемс€ на то, что InitIteration обнулила P_IterQuery.
			//
			ok = -1;
		}
	}
		...
		while(P_IterQuery && P_IterQuery->nextIteration() > 0) {
			...
			if(P_TempTbl)
				...
				if(GoodsListIter >= 0 && !GoodsIdList.bsearch(rec.GoodsID))
					continue;
				//
				// ≈сли список сессий не определен, но отчет поднимаетс€ по множеству сессий,
				// то провер€ем каждую сессию на удовлетворение ограничени€ по периоду начала
				//
