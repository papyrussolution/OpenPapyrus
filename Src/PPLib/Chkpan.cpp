...
int CPosProcessor::Packet::SetQueue(uint itemIdx, int8 queue)
{
	...
	if(i < c) {
		at(i++).Queue = queue;
		//
		// Для всех элементов, сгруппированных с данным, необходимо выставить тот же самый
		// номер очереди.
		//
				...
				if(P.Eccd.Addr_.Tail[0]) {
					if(!P.Eccd.Addr_.ID) {
						...
						if(temp_buf.NotEmptyS()) {
							...
							// повторно использовать этот адрес.
							//
			...
			if(mode == accmRegular && pPack && !(pPack->Rec.Flags & (CCHKF_JUNK|CCHKF_SKIP))) {
				...
				if(pPack->AL_Const().getCount()) {
					...
					if(F(fRetCheck)) {
						...
						if(_charge != 0.0) {
		#if 0 // @v8.1.0 Списание с бонусных (кредитных) карт осуществляется теперь функцией проведения чека {
		#endif // } 0 @v8.1.0
						}
								...
								for(int _again = 1; _again && paym_blk2.EditDialog2() > 0;) {
									...
									if(CsObj.GetEqCfg().Flags & PPEquipConfig::fUnifiedPaymentCfmBank &&
										...
										if(paym_blk2.NoteAmt > 0.0 && paym_blk2.DeliveryAmt > 0.0) {
											CDispCommand(cdispcmdChange, 0, paym_blk2.NoteAmt, paym_blk2.DeliveryAmt);
											InformCashNoteAndDelivery(H(), paym_blk2); // @v9.8.4
										}
										else
					...
					if(!isempty(pPhone)) {
						...
						if(ScObj.Search(Data.SCardID_, &sc_rec) > 0 && sc_rec.PersonID && PsnObj.GetPacket(sc_rec.PersonID, &pack, 0) > 0) {
							...
							if(loc_rec.ID) {
								...
								//
								// Обнуляем идентификатор адреса поскольку эта запись станет
								// собственностью чека.
								//
								loc_rec.ID = 0;
								Data.Addr_ = loc_rec;
...
int CPosProcessor::PreprocessRowBeforeRemoving(/*IN*/long rowNo, /*OUT*/double * pResultQtty)
{
	...
	if(rowNo >= 0 && rowNo < (long)P.getCount()) {
		const  CCheckItem item = P.at((uint)rowNo); // @note Переменная не должна быть ссылкой (оригинал может измениться)
...
private:
	...
	enum {
		...
		modeSelectByMultCode // @v8.1.6 Режим выбора одной из карт, имеющих один и тот же код
	};
	enum {
		stCreditCard     = 0x0002,
		stAsSelector     = 0x0004,
				...
				if(!ext_cancel) { // Если расширенный выбор был отменен, то это блок следует пропустить
					if(is_found) {
						...
						if(!cr) {
							...
							//
							// @v9.1.8 Автоактивируемая карта начиная с 9.1.8 на первом чеке ведет себя так же, как и обычная
							//
							{
...
int CPosProcessor::AcceptRow(PPID giftID)
{
	...
	if(ok) {
		...
		if(!F(fRetCheck) && !giftID) {
			if(ProcessGift() > 0) {
				CalcRestByCrdCard_(0); // @v7.5.12 Предыдущий вызов (@01) не учел подарочную скидку
			}
		}
	}
	Flags &= ~fSuspSleepTimeout;
		...
		if(format_name.NotEmpty() || SelectCheck(&chk_id, &format_name, scfSelSlipDocForm|scfThisNodeOnly|scfAllowReturns/*1, 1*/ /* thisNodeOnly */) > 0) {
			...
			if(IsState(sLIST_EMPTYBUF)) {
				...
				if(SuspCheckID && GetCc().Search(SuspCheckID, &last_chk_rec) > 0)
					...
					GetNewCheckCode(CashNodeID, &pack.Rec.Code);
				THROW(Helper_InitCcPacket(&pack, 0, 0, iccpSetCurTime));
				r = 1; // @v7.2.10 (из-за того, что значение оставалось меньше нуля документ не печатался)
			}
			else {
				if(chk_id)
					THROW(r = GetCc().LoadPacket(chk_id, 0, &pack));
