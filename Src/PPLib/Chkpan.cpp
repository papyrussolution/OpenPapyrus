...
//
//
CPosProcessor::LinkBlock::LinkBlock() : _Op(opGeneral), _CcID(0), _CcAmount(0.0), _CcCredit(0.0)
{
}
...
{
	const bool preserve_value = LOGIC(CnFlags & CASHF_DISABLEZEROAGENT);
	SETFLAG(CnFlags, CASHF_DISABLEZEROAGENT, set);
	SETFLAG(CnExtFlags, CASHF_DISABLEZEROAGENT, set);
				...
				if(!(sc_cfg.Flags & sc_cfg.fDontUseBonusCards)) {
					if(!F(fRetCheck)) { // По бонусной карте в возврате бонус не учитываем (пока)
						...
						if(pPack) {
							...
							for(i = 0; P.enumItems(&i, (void **)&p_item);) {
								p_item->GetRec(ccl_rec, IsCurrentOp(opReturn));
						...
						if(pCurItem) {
							pCurItem->GetRec(ccl_rec, IsCurrentOp(opReturn));
							charge += CalcSCardOpBonusAmount(ccl_rec, bonus_goods_grp_id, &nca);
							non_crd_amt += nca;
							bonus_charge_amt += CalcSCardOpBonusAmount(ccl_rec, bonus_charge_grp_id, 0);
						}
...
int CPosProcessor::Helper_InitCcPacket(CCheckPacket * pPack, CCheckPacket * pExtPack, const CcAmountList * pCcPl, long options)
{
	int    ok = 1;
	{
		...
		if(IsCurrentOp(opReturn))
			sf = CCHKF_RETURN;
		else if(IsCurrentOp(opCorrection))
			sf = CCHKF_CORRECTION;
		if(sf) {
			if(pPack)
				pPack->Rec.Flags |= sf;
				...
				if(oneof2(EgaisMode, 1, 2)) {
							...
							if(r > 0) {
								...
								if(is_ccs_inited) {
									...
									if(r_cc.TurnCheck(&cc_shadow_egais, 1)) {
										...
										THROW(P_EgPrc->PutCCheck(cc_shadow_egais, CnLocID, true/*horecaAutoWo*/, eg_ack));
					...
					if(P_EgPrc) {
						...
						}
						{
							...
							THROW(P_EgPrc->PutCCheck(epb.Pack, CnLocID, false/*horecaAutoWo*/, eg_ack));
						...
						if(epb.Flags & epb.fIsExtPack) {
							PPEgaisProcessor::Ack eg_ack;
							THROW(P_EgPrc->PutCCheck(epb.ExtPack, ExtCnLocID, false/*horecaAutoWo*/, eg_ack));
							if(eg_ack.Sign[0] && eg_ack.SignSize) {
								msg_buf.Z().CatN(reinterpret_cast<const char *>(eg_ack.Sign), eg_ack.SignSize);
		...
		if(r == cmYes) {
			...
			for(uint i = 0; pPack->EnumItems(&i, &item) > 0;) {
				...
				if(dlg->GetRgi(item.GoodsID, item.Qtty, 0, rgi)) {
					...
					chk_item.Quantity = R6((dlg->IsCurrentOp(opReturn)) ? -fabs(item.Qtty) : fabs(item.Qtty));
	...
	GoodsListFontHeight(0), GoodsListEntryGap(0), P_PalmWaiter(0), P_UhttImporter(0), P_CDY(0), P_BNKTERM(0), P_PhnSvcClient(0)
{
	...
	else if(CnSpeciality == PPCashNode::spShop) {
		showButton(cmSelTable, 0);
		if(!(Flags & fNoEdit))
			showButton(cmChkPanPrint, 0);
		showButton(cmToLocPrinters, 0);
...
int CPosProcessor::CalculatePaymentList(PosPaymentBlock & rBlk, int interactive)
{
	...
	if(ok > 0) {
		...
		if(IsCurrentOp(opReturn) && P.Lb_.AmL.GetTotal() != 0.0) {
			//
...
//
struct _SelCheck {
	_SelCheck() : CheckID(0), BillID(0), Flags(0)
	{
	}
	enum {
		fUnfinished = 0x0001, // Структура возвращает неотпечатанный чек
		fCorrection = 0x0002, // @v12.2.8 Была выбрана операция коррекции чека (из пары возврат/коррекция)
	};
	PPID    CheckID;
	PPID    BillID; // @v11.8.8
	long    Flags;
...
void SelCheckListDialog::Init(PPCashMachine * pCm)
{
	...
	LastChkNo = -1;
	LastTopID = 0;
	LastOp = CPosProcessor::opGeneral; // @v12.2.8
...
int SelCheckListDialog::getDTS(_SelCheck * pSelCheck)
{
	...
	if(State & stSelectBill) {
		...
		if(sel_chk.CheckID) {
			// @v12.2.8 {
			if(Op == CPosProcessor::opCorrection) {
					...
					if(chk_no != LastChkNo || diffdate(dt, LastDate) || Op != LastOp) {
						...
						THROW(r_cc.SearchByDateAndCode(chk_no, dt, unprinted_only, &temp_list));
						{
							...
							if(i) do {
								...
								if(!do_remove && Op == CPosProcessor::opCorrection) { 
	...
	if(TVBROADCAST) {
		...
		switch(TVCMD) {
			...
			case cmRetCheck:      
				...
				BARRIER(SetupRetCheck(!(IsCurrentOp(opReturn) || IsCurrentOp(opCorrection)))); // @v12.2.9
				break;
...
int CheckPaneDialog::SelectCheck(const char * pTitle, long flags, SelectCheckResult & rResult)
{
	...
	if(param.Flags & param.fRetOrCorrSelection) {
		rResult.Op = (sch.Flags & _SelCheck::fCorrection) ? 2 : 1;
	}
	delete dlg;
	Flags &= ~fSuspSleepTimeout;
	return ok;
...
int CPosProcessor::RestoreSuspendedCheck(PPID ccID, CCheckPacket * pPack, int unfinishedForReprinting)
{
	...
	if(cc_pack.Rec.Flags & CCHKF_CORRECTION)
		SetCurrentOp(opCorrection);
	else if(cc_pack.Rec.Flags & CCHKF_RETURN)
		SetCurrentOp(opReturn);
	// } @v12.2.9 
		...
		if(!ret) {
			...
			if(is_pane_empty || oneof2(GetState(), sLISTSEL_EMPTYBUF, sLISTSEL_BUF)) {
				if(!(OperRightsFlags & orfReturns)) {
					...
					else if(IsCurrentOp(opReturn) || IsCurrentOp(opCorrection)) {
						ret = true;
					}
					if(ret)*/
					{
						selected_op = opReturn;
						CCheckPacket chk_pack;
						...
						if(chk_pack.GetCount()) {
							...
							// @v12.2.9 {
							else if(selected_op == opCorrection) {
...
{
	assert(oneof3(op, opCorrection, opReturn, opGeneral));
	if(op == opCorrection) {
			...
			if(r > 0 || (r == -2 && (CnFlags & CASHF_USEQUOT || gift_money))) {
				...
				if(ok) {
					...
					if(price > 0.0 || (price == 0.0 && rgi.QuotKindUsedForPrice) || giftID || r == 2000) {
						...
						r_item.Quantity = R6((IsCurrentOp(opReturn)) ? -fabs(rBlk.Qtty) : fabs(rBlk.Qtty));
						r_item.Price    = price;
						r_item.Discount = 0.0;
...
int CPosProcessor::AcceptRow(PPID giftID)
{
	...
	if(ok) {
		...
		ClearRow();
		if(!IsCurrentOp(opReturn) && !IsCurrentOp(opCorrection) && !giftID) {
			if(ProcessGift() > 0) {
