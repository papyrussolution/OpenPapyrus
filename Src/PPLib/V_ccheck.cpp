// V_CCHECK.CPP
// Copyright (c) A.Sobolev 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026
// @codepage UTF-8
//
#include <pp.h>
#pragma hdrstop
	...
	if(P_Ct) {
		...
		if(P_TmpGdsCorrTbl) {
			...
			p_q = & Select_(
				...
				g->Qtty,        // #8
				0L);
			p_q->addField(*p_dbe_avrg);    // #9
			p_q->addField(g->LinesCount);  // #10
			p_q->addField(g->SkuCount);    // #11
			...
			if(P_TmpTbl) {
				...
				p_q = & Select_(
					...
					cq->Amount,       // #6
					0L);
				p_q->addField(dbe_sc_code);      // #7
				p_q->addField(cq->Discount);     // #8
				p_q->addField(dbe_posnode);      // #9
				p_q->addField(cq->Qtty);         // #10
				p_q->addField(cq->LinesCount);   // #11
