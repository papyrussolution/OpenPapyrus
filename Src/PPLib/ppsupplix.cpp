...
/*
	@20250918 Соображения по поводу учета размещения оборудования поставщиков (холодильники etc) у клиентов.
	На основании того, что я сейчас знаю, поставщики могут предъявлять следующие требования: //
	1. Прием заявки от поставщика на размещение/перемещение/возврат оборудования у клиента
	2. Отчет о действии в ответ на заявку п.1
	3. Отчет об оборудовании, находящемся у клиентов

	Вопрос в том, как это все учитывать. 
	Вариант 1. Доработать складские операции. В этом случае вместо ячеек товар будет двигаться по адресам доставки клиентов.
		Однако, у нас нет операции заказа. С отчетом (п.3) все более менее норм.

		Механизм в этом случае следующий:
		-- приходуем оборудование от поставщика на ответственное хранение - формируется лот на каком-либо из наших складов
...
int PPSupplExchange_Baltika::Export(PPLogger & rLogger)
{
	...
	if(P.Actions & P.opExportDebts) {
		...
		THROW(ExportSaldo2(excl_ar_list, temp_client_code, &rLogger));
	}
	THROW(Send());
	{
...
int PPSupplExchange_Baltika::ExportRestParties()
{
	...
	TSVector <Sdr_Baltika_RestPartLine> wotarebeerrest_list;
	GoodsRestFilt filt;
	SoapExporter soap_e;
	...
	filt.DiffParam = GoodsRestParam::_diffLotTag;
	filt.DiffLotTagID = PPTAG_LOT_MANUFTIME;
	//
...
int PPSupplExchange_Baltika::ExportBills(const BillExpParam & rExpParam, const char * pClientCode, PPLogger & rLogger)
{
	...
	for(v.InitIteration(PPViewBill::OrdByDefault); v.NextIteration(&item) > 0;) {
		...
		PPBillPacket bpack;
		TSVector <Sdr_SupplBillLine> items_list;
			...
			for(bpack.InitExtTIter(0/*ETIEF_UNITEBYGOODS*/); bpack.EnumTItemsExt(0, &trfr_item) > 0;) {
				...
				if(check_modif_op && (GObj.BelongToGroup(trfr_item.GoodsID, Ep.GoodsGrpID) > 0 ||
					...
					if(GetBarcode(trfr_item.GoodsID, line_rec.WareId, sizeof(line_rec.WareId), 0, &is_horeca, &bc_pack) > 0) {
						...
						line_rec.ActionDate      = trfr_item.Date;
...
public:
	...
	DECL_DIALOG_GETDTS()
	{
		...
		GetClusterData(CTL_SUPPLIX_FLAGS, &Data.Flags);
		...
		getCtrlString(CTL_SUPPLIX_ADDPARAM, temp_buf);
...
int iSalesPepsi::ReceiveGoods(int forceSettings, int useStorage)
{
	...
	if(ok < 0 || forceSettings) {
		...
		if(p_result->getCount() == 1) {
			const iSalesGoodsPacket * p_np = p_result->at(0);
			if(p_np && p_np->ErrMsg.NotEmpty()) {
				CALLEXCEPT_PP_S(PPERR_UHTTSVCFAULT, p_np->ErrMsg);
			}
		}
		{
			...
			for(uint i = 0; i < p_result->getCount(); i++) {
				...
				if(p_item) {
					line_buf.Z().Cat(p_item->iSalesId).Tab().Cat(p_item->DocType).Tab().Cat(p_item->Code).Tab().
						Cat(p_item->Dtm, DATF_DMY, TIMF_HMS).Tab().Cat(p_item->PayerCode).Tab().Cat(p_item->Amount, MKSFMTD_020).Tab().
						Cat(p_item->Debt, MKSFMTD_020).Tab().Cat(p_item->ErrMsg);
					line_buf.Transf(CTRANSF_INNER_TO_UTF8);
					f_out.WriteLine(line_buf.CR());
				}
			}
			...
			if(p_item) {
				for(uint j = 0; j < p_item->VisitList.getCount(); j++) {
					...
					if(p_visit && p_visit->OuterClientCode.NotEmpty()) {
						(temp_buf = p_visit->InnerClientCode).Transf(CTRANSF_INNER_TO_UTF8);
...
void SapEfes::Init()
{
	...
	InitGoodsList(iglfWithArCodesOnly);
	State |= stInited;
}
	...
	if(pPack && !!pPack->Client.Uuid) {
		...
		}
		{
			...
			for(uint itemidx = 0; itemidx < pPack->ItemList.getCount(); itemidx++) {
				...
				p_js_item->InsertString("cost", temp_buf.Z().Cat(r_item.Cost, MKSFMTD_020));
				p_js_item->InsertString("costWithoutVAT", temp_buf.Z().Cat(r_item.CostWoVat, MKSFMTD_020));
					...
					p_new_pack->ContractorName = psn_pack.Rec.Name;
					{
						...
						bool   dlvr_loc_pack_is_valid = false;
						{
							if(LocObj.GetPacket(dlvr_loc_id, &loc_pack) > 0 && loc_pack.Type == LOCTYP_ADDRESS) {
								...
								LocationCore::GetAddress(loc_pack, 0, p_new_pack->DlvrAddrText_);
							}
							else if(contractor_rloc_addr.NotEmpty()) {
								dlvr_loc_pack_is_valid = true;
								p_new_pack->DlvrLocID = psn_pack.Rec.RLoc;
								p_new_pack->DlvrAddrText_ = contractor_rloc_addr;
							}
							else if(contractor_mainloc_addr.NotEmpty()) {
								dlvr_loc_pack_is_valid = true;
								p_new_pack->DlvrLocID = psn_pack.Rec.MainLoc;
								p_new_pack->DlvrAddrText_ = contractor_mainloc_addr;
							}
						...
						if(acs_id == sell_acs_id) {
							...
							ClientEntry::DlvrLoc * p_founded_dlvr_loc = 0;
							{
								...
								if(!p_founded_dlvr_loc && p_new_pack->DlvrLocID) {
									...
									p_new_loc_entry->Addr = p_new_pack->DlvrAddrText_; // @v12.2.2 @fix p_new_pack->ContractorAddrText-->p_new_pack->DlvrAddrText_
			...
			for(uint billidx = 0; billidx < bill_list.getCount(); billidx++) {
				...
				if(p_pack) {
					SXml::WNode n_s(p_x, "Document-Invoice");
					{
						...
						n_h.PutInner("Address", XmlCp1251EncText(p_pack->DlvrAddrText_));
...
private:
	...
	struct BillPacket {
		BillPacket() : ID(0), OrderBillID(0), Dtm(ZERODATETIME), ContractorID(0), DlvrLocID(0), AgentID(0), LocID(0)
		{
			Code[0] = 0;
			ContractorCityName[0] = 0;
			ContractorINN[0] = 0;
		...
		struct Position { // @flat
			Position() : GoodsID(0), Qtty(0.0), Volume(0.0), Cost(0.0), CostWoVat(0.0), VatInCost(0.0), VatRate(0.0)
			{
			}
			PPID   GoodsID;
			double Qtty;
...
Ostankino::Ostankino(PrcssrSupplInterchange::ExecuteBlock & rEb, PPLogger & rLogger) : PrcssrSupplInterchange::ExecuteBlock(rEb), R_Logger(rLogger), TsHt(4096)
{
}
...
SString & Ostankino::MakeFileName(const char * pPrefix, const char * pInn, const char * pGln, const char * pDocNumber, SString & rBuf)
{
	/*
...
int Ostankino::SendRest(StringSet & rSsFileName)
{
	...
	xmlTextWriter * p_x = 0;
	PPID main_org_id;
	PPPersonPacket main_psn_pack;
	SString own_inn_buf;
	SString own_gln_buf;
	GetMainOrgID(&main_org_id);
	if(PsnObj.GetPacket(main_org_id, &main_psn_pack, 0) > 0) {
		main_psn_pack.Regs.GetRegNumber(PPREGT_TPID, now_dtm.d, own_inn_buf);
		main_psn_pack.Regs.GetRegNumber(PPREGT_GLN, now_dtm.d, own_gln_buf);
	}
	//
	/*
...
int Ostankino::SendSales(StringSet & rSsFileName)
{
	const LDATETIME now_dtm = getcurdatetime_();
	int    ok = 1;
	Reference * p_ref(PPRef);
	...
	//
	{
		SString local_gln_buf;
		for(uint docidx = 0; docidx < bill_list.getCount(); docidx++) {
			const BillPacket * p_pack = bill_list.at(docidx);
			if(p_pack) {
				PPLocationPacket loc_pack;
				if(LocObj.GetPacket(p_pack->LocID, &loc_pack) > 0) {
					loc_pack.Regs.GetRegNumber(PPREGT_GLN, loc_gln_buf);
				}
				{
					if(loc_gln_buf.NotEmpty())
						local_gln_buf = loc_gln_buf;
					else if(own_gln_buf.NotEmpty())
						local_gln_buf = own_gln_buf;
					else
...
const Ostankino::GoodsEntry * Ostankino::SearchGoodsEntry(const TSVector <Ostankino::GoodsEntry> & rGoodsList, PPID goodsID) const
{
	uint   goods_idx = 0;
	return rGoodsList.lsearch(&goodsID, &goods_idx, CMPF_LONG) ? &rGoodsList.at(goods_idx) : 0;
}

int Ostankino::MakeGoodsList(TSVector <Ostankino::GoodsEntry> & rGoodsList)
{
	rGoodsList.clear();
	int    ok = -1;
	SString temp_buf;
			...
			if(GObj.Search(new_entry.ID, &goods_rec) > 0) {
				...
				if(temp_buf.NotEmpty()) {
					...
					if(UObj.Fetch(goods_rec.UnitID, &u_rec) > 0) {
						if(UObj.TranslateToBase(goods_rec.UnitID, SUOM_KILOGRAM, &oe.KgPerUnit) > 0) {
							STRNSCPY(oe.UnitName, "KGM");
						}
						else {
							STRNSCPY(oe.UnitName, "PCE");
							oe.KgPerUnit = 1.0;
						}
					}
					rGoodsList.insert(&oe);
...
int Ostankino::Helper_MakeBillEntry(PPID billID, PPBillPacket * pBp, const TSVector <Ostankino::GoodsEntry> & rGoodsList, BillPacketCollection & rList)
{
	int    ok = -1;
	const LDATETIME now_dtm = getcurdatetime_();
	Reference * p_ref(PPRef);
...
int Ostankino::Helper_MakeBillList(PPID opID, PPID locID, const TSVector <Ostankino::GoodsEntry> & rGoodsList, BillPacketCollection & rList)
{
	int    ok = -1;
	BillFilt b_filt;
	PPViewBill b_view;
...
SString & FASTCALL Ostankino::Helper_GetToken(long tokId)
{
	TokBuf.Z();
...
// 
// 
class Tsaritsyno : public PrcssrSupplInterchange::ExecuteBlock { // @v12.2.2
	struct ClientEntry {
		struct DlvrLoc {
			PPID   ID;
			PPID   AgentID;
	...
	struct BillPacket {
		BillPacket() : ID(0), DocType(doctypUndef), Dt(ZERODATE), DueDt(ZERODATE), ContractorID(0), DlvrLocID(0), AgentID(0), LocID(0)
		{
			Code[0] = 0;
			ContractorINN[0] = 0;
			ContractorKPP[0] = 0;
...
public:
	/*
		...
		ФТДКРФормаОтгрузки
		ФТДКРСпособОтгрузки
		ФТДТТСетеваяПринадлежность
		ФТДТТФорматТорговойТочки
		КаналСбыта
	*/
	struct GoodsEntry {
		GoodsEntry()
	...
	Tsaritsyno(PrcssrSupplInterchange::ExecuteBlock & rEb, PPLogger & rLogger) : PrcssrSupplInterchange::ExecuteBlock(rEb), R_Logger(rLogger), TsHt(4096)
	{
	}
	~Tsaritsyno()
	{
	...
	int    SendClients(StringSet & rSsFileName)
	{
		...
		THROW_SL(p_x = xmlNewTextWriterFilename(out_file_name, 0));
		{
			...
			SXml::WDoc _doc(p_x, cpUTF8);
			SXml::WNode n_o(p_x, Helper_GetToken(PPHSC_RU_CONTRACTOR_PL));
			...
			for(uint cliidx = 0; cliidx < ClientList.getCount(); cliidx++) {
				...
				if(p_cli_entry) {
					for(uint dlvrlocidx = 0; dlvrlocidx < p_cli_entry->DlvrLocList.getCount(); dlvrlocidx++) {
						const ClientEntry::DlvrLoc * p_dlvr_loc = p_cli_entry->DlvrLocList.at(dlvrlocidx);
						if(p_dlvr_loc) {
							SXml::WNode n_i(p_x, Helper_GetToken(PPHSC_RU_LINE));
							//<Строка КлиентКод="1676" КодПлательщика="1676" МТКодТочки="ДРДКВ1676" КлиентИНН="616831008267" КлиентКПП="" 
							// КлиентНаименование="Полунин А.Н. ИП (Ленточная 1)" Площадка="606945" КлиентАдрес="344041, Ростов-на-Дону г, Ленточная ул, 1,, " 
							// ФТДРегион="61" МТКодТП="ДРДКВ83097" 
							// ФТДФИОТП="Агент ТП Царицыно-01 Ксенз В." ФТДКРФормаОтгрузки="" ФТДКРСпособОтгрузки="" ФТДТТСетеваяПринадлежность="" ФТДТТФорматТорговойТочки="" КаналСбыта=""/>
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CLIENTCODE), temp_buf.Z().Cat(p_dlvr_loc->ID));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_PAYERCODE), temp_buf.Z().Cat(p_dlvr_loc->ID));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_DLVRLOCCODE_MT), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CLIENTINN), XmlUtf8EncText(p_cli_entry->INN));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CLIENTKPP), XmlUtf8EncText(p_cli_entry->KPP));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CLIENTNAME), XmlUtf8EncText(p_dlvr_loc->Name));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_PLACE_TSARITSYNO), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CLIENTADDR), XmlUtf8EncText(p_dlvr_loc->Addr));
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_REGIONCODE_FTD), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_AGENTCODE_MT), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_AGENT_FTDFIOTP), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_SHIPPINGTYPE), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_SHIPPINGMETH), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_CHAINOFSTORESKIND), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_STOREKIND), "");
							n_i.PutAttrib(Helper_GetToken(PPHSC_RU_DISTRIBUTIONCHANNEL), "");
						}
					}
				}
			}
...
private:
	SString & FASTCALL Helper_GetToken(long tokId)
	{
		TokBuf.Z();
		if(!TsHt.Get(tokId, &TokBuf)) {
			PPLoadStringUtf8(PPSTR_HASHTOKEN_C, tokId, TokBuf);
			TsHt.Put(tokId, TokBuf);
		}
		return TokBuf;
	}
	...
	//
	// Для Царицыно код клиента в опциях обмена соглашения должен быть задан парой code:abbr
	// где code - цифровой код клиента, а abbr - условное краткое текстовое обозначение. Например "продавец из арзамаса" ну или что-то в этом
	// роде в зависимости от договоренности с поставщиком.
	// Соответственно, функция GetCliCode() возвращает цифровой код, а GetCliAbbr() возвращает условную аббервиатуру.
	//
	SString & GetCliCode(SString & rBuf) const
	{
		rBuf.Z();
		SString temp_buf;
		SString lbuf, rbuf;
		Ep.GetExtStrData(Ep.extssClientCode, temp_buf);
		if(temp_buf.Divide(':', lbuf, rbuf) > 0) {
			(rBuf = rbuf).Transf(CTRANSF_INNER_TO_UTF8);
		}
		return rBuf;
	}
	SString & GetCliAbbr(SString & rBuf) const
	{
		rBuf.Z();
		SString temp_buf;
		SString lbuf, rbuf;
...
public:
	VladimirskiyStandard(PrcssrSupplInterchange::ExecuteBlock & rEb, PPLogger & rLogger);
	...
	class BillPacketCollection : public TSCollection <BillPacket> {
	public:
		BillPacketCollection(PrcssrSupplInterchange::ExecuteBlock & rEb);
...
VladimirskiyStandard::BillPacketCollection::BillPacketCollection(PrcssrSupplInterchange::ExecuteBlock & rEb) : TSCollection <BillPacket>(), R_Eb(rEb)
{
}
		
int VladimirskiyStandard::BillPacketCollection::SetupEntry(uint idx, int use_ta)
{
	int   ok = 1;
	if(idx < getCount()) {
...
VladimirskiyStandard::VladimirskiyStandard(PrcssrSupplInterchange::ExecuteBlock & rEb, PPLogger & rLogger) : PrcssrSupplInterchange::ExecuteBlock(rEb), R_Logger(rLogger), TsHt(4096), LocAgentTagID(0)
{
}
	
VladimirskiyStandard::~VladimirskiyStandard()
{
}
	
int VladimirskiyStandard::Init()
{
	int    ok = 1;
	PPObjTag tag_obj;
...
void VladimirskiyStandard::WriteWarehouses(xmlTextWriter * pX, const TSVector <ObjEntry> & rLocList)
{
	SString temp_buf;
...
void VladimirskiyStandard::WriteGoods(xmlTextWriter * pX, const TSVector <ObjEntry> & rGoodsList)
{
	SString temp_buf;
...
int VladimirskiyStandard::SendRest(StringSet & rSsFileName)
{
	const LDATETIME now_dtm = getcurdatetime_();
	int    ok = 1;
	SString temp_buf;
...
int VladimirskiyStandard::SendSales(StringSet & rSsFileName)
{
	const LDATETIME now_dtm = getcurdatetime_();
	int    ok = 1;
	Reference * p_ref(PPRef);
...
int VladimirskiyStandard::TransmitFiles(const StringSet & rFileNameSet)
{
	//const char * pp_pfx_list[] = { "customers", "debts", "movements", "products", "salesrefunds", "salesreps", "stocks", "warehouses", "trade_points" };
	int    ok = -1;
	SString temp_buf;
...
SString & FASTCALL VladimirskiyStandard::Helper_GetToken(long tokId)
{
	TokBuf.Z();
	if(!TsHt.Get(tokId, &TokBuf)) {
		PPLoadStringUtf8(PPSTR_HASHTOKEN_AGENTPLUS, tokId, TokBuf);
...
int VladimirskiyStandard::MakeLocList(TSVector <ObjEntry> & rLocList)
{
	int    ok = 1;
	rLocList.clear();
	for(uint i = 0; i < P.LocList.GetCount(); i++) {
...
const VladimirskiyStandard::ObjEntry * VladimirskiyStandard::SearchGoodsEntry(const TSVector <VladimirskiyStandard::ObjEntry> & rGoodsList, PPID goodsID) const
{
	uint   goods_idx = 0;
	return rGoodsList.lsearch(&goodsID, &goods_idx, CMPF_LONG) ? &rGoodsList.at(goods_idx) : 0;
}
	
int VladimirskiyStandard::MakeGoodsList(TSVector <VladimirskiyStandard::ObjEntry> & rGoodsList)
{
	rGoodsList.clear();
	int    ok = -1;
	GoodsFilt filt;
...
int VladimirskiyStandard::Helper_MakeBillList(PPID opID, PPID locID, const TSVector <VladimirskiyStandard::ObjEntry> & rGoodsList, BillPacketCollection & rList)
{
	int    ok = -1;
	BillFilt b_filt;
	PPViewBill b_view;
		...
		StringSet ss_fn_src;
		{
			...
			if(remote_addr.NotEmptyS()) {
				...
				if(url_src.Parse(remote_addr) > 0) {
					...
					else if(prot_src == InetUrl::protFtp) {
						SUniformFileTransmission uft; // cocacola
						(uft.SrcPath = remote_addr).SetLastDSlash().Cat("inbox").SetLastDSlash().Cat("*.xml");
						PPGetPath(PPPATH_TEMP, temp_buf);
						(uft.DestPath = temp_buf).Transf(CTRANSF_INNER_TO_OUTER);
						uft.Flags = 0;
						//uft.Format = iff;
						uft.AccsName = accs_name;
						uft.AccsPassword.EncodeUrl(accs_passw, 0);
						const int uftprr = uft.Run(0, 0);
						if(uftprr) {
							for(uint i = 0; i < uft.ResultList.getCount(); i++) {
								const SUniformFileTransmission::ResultItem & r_ri = uft.ResultList.at(i);
								uft.GetS(r_ri.DestPathP, temp_buf);
...
int COCACOLA::ExportBailmentReplies()
{
	...
	if(order_op_id) {
		...
		if(bill_id_list.getCount()) {
			...
			if(ss_result_file_list.IsCountGreaterThan(0)) {
				...
				Ep.GetExtStrData(PPSupplAgreement::ExchangeParam::extssAccsName, accs_name);
				Ep.GetExtStrData(PPSupplAgreement::ExchangeParam::extssAccsPassw, accs_passw);
				if(remote_addr.NotEmpty()) {
					InetUrl url_dest;
					if(url_dest.Parse(remote_addr) > 0) {
						const int prot_dest = url_dest.GetProtocol();
						for(uint ssp = 0; ss_result_file_list.get(&ssp, result_file_name);) {
							if(fileExists(result_file_name)) {
								SUniformFileTransmission uft; // cocacola
								uft.SrcPath = result_file_name;
								{
									//SFsPath ps(result_file_name);
									//ps.Merge(SFsPath::fNam|SFsPath::fExt, temp_buf);
									(uft.DestPath = remote_addr).SetLastDSlash().Cat("outbox").SetLastDSlash();
								}
								uft.Flags = 0;
								//uft.Format = iff;
								uft.AccsName = accs_name;
...
int PrcssrSupplInterchange::Run()
{
	...
	THROW_PP(State & stInited && P_Eb, PPERR_SUPPLIXNOTINITED);
	{
		...
		else if(temp_buf.IsEqiAscii("TSARITSYNO")) { // @v12.2.2
			Tsaritsyno cli(r_eb, logger);
			const long actions = r_eb.P.Actions;
			StringSet ss_file_name;
			PPWaitStart();
			THROW(cli.Init());
