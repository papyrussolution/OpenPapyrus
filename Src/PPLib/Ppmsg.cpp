		...
		_PPStrStore = new StringStore2();
		{
			...
			for(uint i = 0; i < file_lang_list.getCount(); i++) {
				...
				if(_PPStrStore->Load(name, self_test)) {
					...
					char err_msg[1024];
					sprintf(err_msg, "Unable load string resource '%s'", (const char *)name);
					::MessageBox(0, err_msg, "Error", MB_OK|MB_ICONERROR); // @unicodeproblem
					ZDELETE(_PPStrStore);
				}
			}
...
__loadstring:
		if(!PPLoadString(group, msgcode, p_tmp_buf+1, PP_MSGLEN)) {
			if(SLibError == SLERR_NOFOUND && addcode) {
				...
				// @v9.0.4 sprintf(p_tmp_buf+1, "ЌҐў®§¬®¦­® § Јаг§Ёвм б®®ЎйҐ­ЁҐ: (%d)%d", group, msgcode);
				// @v9.0.4 {
