...
/*virtual*/int BillFilt::Describe(long flags, SString & rBuf) const
{
	...
	else if(SortOrder == ordByContragentDateCode) // @v12.5.11
		PutMembToBuf(STRINGIZE(ordByContragentDateCode), STRINGIZE(SortOrder), rBuf);
	else
...
void FASTCALL BillFilt::SetupBrowseBillsType(BrowseBillsType bbt)
{
	switch(bbt) {
		...
		case bbtWmsBills:
			...
			Flags |= fWmsOnly;
			break;
		case bbtAgreemen: // @v12.5.10
			Flags &= ~(fOrderOnly|fAccturnOnly|fPoolOnly|fInvOnly|fDraftOnly|fWmsOnly);
			Flags |= fAccturnOnly;
			break;
		default:
			bbt = bbtUndef;
...
int BillFiltDialog::setDTS(const BillFilt * pFilt)
{
	if(!Data.PPBaseFilt::IsEq(pFilt, 0)) {
		...
		SetPeriodInput(this, CTL_BILLFLT_DUEPERIOD, Data.DuePeriod);
		if(Data.Bbt == bbtAgreemen) { // @v12.5.10
			types.add(PPOPT_AGREEMENT);
		}
		else if(Data.Flags & BillFilt::fOrderOnly)
		...
		AddClusterAssoc(CTL_BILLFLT_ORDER, 3, BillFilt::ordByObject);
		// @v12.5.11 {
		if(this->resourceID == DLG_BILLFLT) { // Остальные диалоги пока не перевели на новый формат!
			AddClusterAssoc(CTL_BILLFLT_ORDER, 4, BillFilt::ordByContragentDateCode); 
		}
		// } @v12.5.11 
...
int PPViewBill::Init_(const PPBaseFilt * pFilt)
{
	...
	if(IsTempTblNeeded()) {
		...
		switch(Filt.SortOrder) {
			...
			case BillFilt::ordByDateCode: ord = OrdByDateCode; break;
			case BillFilt::ordByContragentDateCode: ord = OrdByContragentDateCode; break; // @v12.5.11
		}
...
int PPViewBill::GetOpList(const BillFilt * pFilt, PPIDArray * pList, PPID * pSingleOpID) const
{
	...
	if(!pFilt->OpID || IsGenericOp(pFilt->OpID) > 0) {
		...
		if(ot)
			ot_list.add(ot);
		else if(pFilt->Bbt == bbtAgreemen) { // @v12.5.10
...
int PPViewBill::InitOrderRec(IterOrder ord, const BillTbl::Rec * pBillRec, TempOrderTbl::Rec * pOrdRec)
{
	...
	else if(ord == OrdByContragentDateCode) { // @v12.5.11
		GetArticleName(pBillRec->Object, temp_buf);
		temp_buf.Align(127, ADJ_LEFT);
		temp_buf.Cat(pBillRec->Dt, DATF_YMD|DATF_NODIV|DATF_CENTURY).Cat(pBillRec->Code);
	}
...
int PPViewBill::CreateTempTable(IterOrder ord, int * pIsOrdTbl)
{
	...
	ZDELETE(P_TempOrd);
	ZDELETE(P_TempTbl);
	if(oneof5(ord, OrdByCode, OrdByObjectName, OrdByOpName, OrdByDateCode, OrdByContragentDateCode) || (ord && Filt.Flags & BillFilt::fDescOrder)) { // @v12.5.11 OrdByContragentDateCode
		param.IsOrdTbl = 1;
...
int PPViewBill::InitIteration(IterOrder ord)
{
	...
	switch(ord) {
		...
		case OrdByDateCode:   idx = 1; use_ord_tbl = 1; break;
		case OrdByContragentDateCode: idx = 1; use_ord_tbl = 1; break; // @v12.5.11
		default:              idx = 1;
	}
	if(Filt.Flags & BillFilt::fDescOrder)
		use_ord_tbl = 1;
...
DBQuery * PPViewBill::CreateBrowserQuery(uint * pBrwId, SString * pSubTitle)
{
	...
	if(P_TempOrd && oneof4(Filt.SortOrder, BillFilt::ordByCode, BillFilt::ordByObject, BillFilt::ordByDateCode, BillFilt::ordByContragentDateCode)) { // @v12.5.11 BillFilt::ordByContragentDateCode
	...
	if(Filt.Flags & BillFilt::fCashOnly) {
		...
		if(Filt.ObjectID && Filt.Flags & BillFilt::fDebtsWithPayments) {
			...
			// } Restrictions
			//
			if(ordt && oneof4(Filt.SortOrder, BillFilt::ordByCode, BillFilt::ordByObject, BillFilt::ordByDateCode, BillFilt::ordByContragentDateCode)) // @v12.5.11 BillFilt::ordByContragentDateCode
		...
		if(!(Filt.DenyFlags & BillFilt::fDenyAdd)) {
			...
			if(op_id == 0) {
				if(r_cfg.Cash) {
					...
					PPIDArray base_op_type_list; // @v12.5.10
					uint   opklf = 0;
					PPIDArray op_type_list;
					{
						if(Filt.Bbt == bbtAgreemen) { // @v12.5.10
							base_op_type_list.add(PPOPT_AGREEMENT);
						}
						else if(Filt.Flags & BillFilt::fAccturnOnly)
							base_op_type_list.addzlist(PPOPT_ACCTURN, PPOPT_AGREEMENT, 0L);
						else if(Filt.Flags & BillFilt::fOrderOnly)
							base_op_type_list.add(PPOPT_GOODSORDER);
						else if(Filt.Flags & BillFilt::fInvOnly)
							base_op_type_list.add(PPOPT_INVENTORY);
						else if(Filt.Flags & BillFilt::fPoolOnly)
							base_op_type_list.add(PPOPT_POOL);
						else if(Filt.Flags & BillFilt::fDraftOnly)
							base_op_type_list.addzlist(PPOPT_DRAFTRECEIPT, PPOPT_DRAFTEXPEND, PPOPT_DRAFTTRANSIT, PPOPT_DRAFTQUOTREQ, 0L);
						else if(Filt.Flags & BillFilt::fWmsOnly)
							base_op_type_list.addzlist(PPOPT_WAREHOUSE, 0L);
						else
							base_op_type_list.addzlist(PPOPT_GOODSRECEIPT, PPOPT_GOODSEXPEND, PPOPT_GOODSREVAL, PPOPT_GOODSMODIF, PPOPT_PAYMENT, 0L);
					}
					if(Filt.AccSheetID) {
						while(EnumOperations(0, &op_id, &op_rec) > 0) {
							if(Filt.AccSheetID == op_rec.AccSheetID) {
								if(!base_op_type_list.getCount() || base_op_type_list.lsearch(op_rec.OpTypeID)) {
						...
						op_type_list = base_op_type_list;
					}
					// @v12.3.9 {
...
int PPViewBill::DeleteItem(PPID billID)
{
	...
	if(!(Filt.DenyFlags & BillFilt::fDenyRemove)) {
		bool   is_bill_found = false;
		bool   is_wrd_draft = false;
		BillTbl::Rec bill_rec;
...
int PPViewBill::AttachBillToOrder(PPID billID)
{
	...
	if(P_BObj->Search(billID, &bill_rec) > 0 && CheckOpFlags(bill_rec.OpID, OPKF_ONORDER)) {
		...
		if(ViewGoodsBills(&flt, true/*modeless*/) > 0) {
			const  PPID order_bill_id = flt.Sel;
			LastSelID = flt.Sel;
...
int PPViewBill::AttachBillToDraft(PPID billID, const BrowserWindow * pBrw)
{
	...
	if(P_BObj->Search(billID, &bill_rec) > 0) {
		...
		if(bill_rec.LinkBillID)
			...
			const  PPID acc_sheet_id = op_rec.AccSheetID;
			DateRange period;
			period.Set(plusdate(bill_rec.Dt, -180), bill_rec.Dt);
			PPIDArray op_list;
...
int PPViewBill::UniteInventory()
{
	...
	if(ExecView(dlg) == cmOK) {
		...
		rmv_src = BIN(v & 0x01);
		{
			...
			{
				BillTbl::Rec bill_rec;
				uint   iter_idx = ary.getCount();
				if(iter_idx) {
					do {
						const  PPID bill_id = ary.get(--iter_idx);
						if(P_BObj->Search(bill_id, &bill_rec) > 0) {
							if(P_BObj->CheckStatusFlag(bill_rec.StatusID, BILSTF_DENY_MERGE)) {
								//PPINF_BILLCANTBEMERGED_STATUS "Документ '%s' не можеть быть объединен поскольку это запрещено его статусом" // @v12.5.10
								PPObjBill::MakeCodeString(&bill_rec, PPObjBill::mcsAddOpName|PPObjBill::mcsAddObjName, temp_buf);
								logger.LogMsgCode(mfInfo, PPINF_BILLCANTBEMERGED_STATUS, temp_buf);
								ary.atFree(iter_idx);
							}
						}
						else {
...
int PPViewBill::UniteSellBills()
{
	...
	if(PPMessage(mfConf|mfYesNo, PPCFM_UNITESELL) == cmYes) {
		...
		{
			BillTbl::Rec bill_rec;
			uint   iter_idx = src_ids.getCount();
			if(iter_idx) {
				do {
					const  PPID bill_id = src_ids.get(--iter_idx);
					if(P_BObj->Search(bill_id, &bill_rec) > 0) {
						if(P_BObj->CheckStatusFlag(bill_rec.StatusID, BILSTF_DENY_MERGE)) {
							//PPINF_BILLCANTBEMERGED_STATUS "Документ '%s' не можеть быть объединен поскольку это запрещено его статусом" // @v12.5.10
							PPObjBill::MakeCodeString(&bill_rec, PPObjBill::mcsAddOpName|PPObjBill::mcsAddObjName, temp_buf);
							logger.LogMsgCode(mfInfo, PPINF_BILLCANTBEMERGED_STATUS, temp_buf);
							src_ids.atFree(iter_idx);
						}
					}
					else {
						src_ids.atFree(iter_idx);
		...
		if(src_ids.getCount() > 1) {
			...
			PPBill last_bill;
			PPTransaction tra(1);
			const  PPID src_bill_id = src_ids.get(src_ids.getCount()-1);
			THROW(tra);
			...
			for(i = 0; i < src_ids.getCount(); i++) {
				...
				THROW(P_BObj->UniteGoodsBill(pack, src_ids.get(i), 0));
...
int PPViewBill::UniteReceiptBills()
{
	...
	if(PPMessage(mfConf|mfYesNo, PPCFM_UNITERCPT) == cmYes) {
		...
		// } @v12.5.10 
		if(src_ids.getCount() > 1) {
			const PPID dest_id = src_ids.get(0);
			src_ids.reverse(0, src_ids.getCount());
			src_ids.atFree(src_ids.getCount()-1);
			THROW(P_BObj->UniteReceiptBill(dest_id, src_ids, 1));
			ok = 1;
		}
		PPWaitStop();
	}
...
int PPViewBill::PrintAllBills()
{
	...
	if(skip) {
		...
		else if(Filt.SortOrder == BillFilt::ordByContragentDateCode) // @v12.5.11
			order = OrdByContragentDateCode;
		int    r = 1;
...
int PPViewBill::Print()
{
	...
	if(Filt.SortOrder == BillFilt::ordByDate)
		...
		order = OrdByCode;
	else if(Filt.SortOrder == BillFilt::ordByDateCode)
		order = OrdByDateCode;
	else if(Filt.SortOrder == BillFilt::ordByObject)
		order = OrdByObjectName;
	else if(Filt.SortOrder == BillFilt::ordByContragentDateCode) // @v12.5.11
	...
	if(ok == -2) {
		...
		switch(ppvCmd) {
			...
			case PPVCMD_EDITBILLFREIGHT:
				ok = P_BObj->EditBillFreight(hdr.ID);
				if(ok > 0)
					update = 1;
				break;
			case PPVCMD_BROWSE:
				ok = ShowDetails(hdr.ID);
				if(ok > 0)
					update = 1;
				break;
