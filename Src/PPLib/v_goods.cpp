...
const ObjIdListFilt & GoodsFilt::GetResultBrandList() const
{
	return ResultBrandList;
}

int SLAPI GoodsFilt::IsRestrictedByAr() const
{
	return BIN(Flags & fShowArCode && !(Flags & fShowWoArCode) && (CodeArID || Flags & GoodsFilt::fShowOwnArCode));
}
//
...
int SLAPI GoodsFilt::WriteToProp(PPID obj, PPID id, PPID prop, PPID propBefore8604)
{
	...
	Setup();
	if(IsEmpty()) {
		THROW(PPRef->RemoveProp(obj, id, prop, -1));
	}
	else {
		SBuffer buffer;
		SSerializeContext sctx;
...
PPObjGoods * SLAPI PPViewGoods::GetObj()
{
	return &GObj;
}

int SLAPI PPViewGoods::IsAltFltGroup()
{
	return (Filt.GrpID > 0 && PPObjGoodsGroup::IsAlt(Filt.GrpID) > 0);
}

int SLAPI PPViewGoods::IsGenGoodsFlt()
{
	return (Filt.GrpID > 0 && Filt.Flags & GoodsFilt::fGenGoods);
}
...
DBQuery * SLAPI PPViewGoods::CreateBrowserQuery(uint * pBrwId, SString * pSubTitle)
{
	...
	*/
	if(tmp_t) {
		...
		dbq = ppcheckflag(dbq, g->Flags, GF_PASSIV,
			(Filt.Flags & GoodsFilt::fHidePassive) ? -1 : BIN(Filt.Flags & GoodsFilt::fPassiveOnly));
...
int SLAPI PPViewGoods::DeleteItem(PPID id)
{
	...
	if(id) {
		if(IsGenGoodsFlt()) {
			...
			ok = GObj.RemoveObjV(id, 0, PPObject::rmv_default, (void *)grp_id);
		}
		if(P_TempTbl && ok > 0) {
...
#define GRP_LOC 1
	...
	public:
		...
		int    setDTS(const GoodsMoveParam * pData)
		{
			Data = *pData;
		...
		int    getDTS(GoodsMoveParam * pData)
		{
			int    ok = 1;
			uint   sel = 0;
			...
			if(P_View) {
				...
				if(uniq_id_list.getCount()) {
					...
					list.Sort();
					{
						...
						PPFormatT(PPTXT_UHTTEXPGOODS_REFS, &msg_buf, (long)uniq_id_list.getCount(), (long)ref_count);
				...
				if(PPRef->Ot.GetTag(PPOBJ_GLOBALUSERACC, glob_acc_id, PPTAG_GUA_PGGTAG, &pgg_tag) > 0) {
					if(pgg_tag.GetStr(temp_buf.Z()) > 0) {
						...
						if(r_blk.TagObj.FetchBySymb(temp_buf, &pgg_tag_id) > 0) {
							...
							if(pgg_tag_value.SetStr(pgg_tag_id, r_blk.PrivateGoodsGroup)) {
