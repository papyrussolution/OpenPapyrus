...
int FASTCALL PPTransferItem::SetupByRec(const TransferTbl::Rec * tr)
{
	...
	//
	// ‘лаги PPTFR_PLUS и PPTFR_MINUS введены с v1.10.1
	// ƒл€ поддержки совместимости с документами, созданными
	// предыдущими верси€ми, при загрузки строк устанавливаем
	// эти флаги.
	//
...
int SLAPI Transfer::EnumItems(PPID billID, int *pRByBill, PPTransferItem * pTI)
{
	...
	CATCH
		//
		// ¬ журнале pperrmsg.log уточн€ем природу ошибки
		//
		{
...
int SLAPI Transfer::GetGoodsIdList(const GoodsByTransferFilt & rFilt, PPIDArray & rList)
{
	...
	if(!rFilt.IsEmpty()) {
		...
		//
		// ≈сли требуютс€ только новые товары (лотов по которым не было до периода поступлени€ //
		// то по каждому товару из списка просматриваем лоты за период 0..pFilt->LotPeriod.low-1
		// и удал€ет из списка товары, дл€ которых найден хот€ бы один лот по такому условию.
		//
