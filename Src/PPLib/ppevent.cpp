		...
		void SetupType()
		{
			...
			switch(Data.Rec.EventType) {
				case PPEVENTTYPE_OBJCREATED:
					...
					enableCommand(cmFilter, true);
					break;
				case PPEVENTTYPE_OUTER:
					disableCtrl(CTLSEL_EVNTSUBSCR_OBJ, true);
					enableCommand(cmFilter, false);
					break;
				case PPEVENTTYPE_SPCBILLCHANGE:
		...
		if(!pID || !*pID) {
			if(pPack) {
				...
				destroyLobData(VT);
				{
					...
					THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EVENT, id, PPTRPROP_DESCR), temp_buf, 0));
				}
				ASSIGN_PTR(pID, id);
			}
		}
			...
			if(*pID) {
				if(pPack) {
					...
					{
						(temp_buf = pPack->Text).Strip().Transf(CTRANSF_INNER_TO_UTF8);
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EVENT, *pID, PPTRPROP_DESCR), temp_buf, 0));
					}
				}
				else {
					if(SearchByID_ForUpdate(this, PPOBJ_EVENT, *pID, 0) > 0) {
						THROW_DB(deleteRec());
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EVENT, *pID, PPTRPROP_DESCR), temp_buf.Z(), 0));
					}
				}
			}
		}
...
int PPEventCore::Get(PPID id, Packet * pPack)
{
	...
	if(SearchByID(this, PPOBJ_EVENT, id, 0) > 0) {
		if(pPack) {
			...
			p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EVENT, id, PPTRPROP_DESCR), pPack->Text);
