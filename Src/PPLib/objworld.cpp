...
int EditWorldDialog::setDTS(const PPWorldPacket * pData)
{
	...
	if(oneof2(Id, DLG_STREET, DLG_CITYAREA)) {
		...
		if(Id == DLG_COUNTRY)
			...
			SetupPPObjCombo(this, CTLSEL_WORLD_STATUS,  PPOBJ_CITYSTATUS, Data.Rec.Status, OLW_CANINSERT);
	...
	if(extra_ptr) {
		SetupPPObjCombo(this, CTLSEL_WORLD_PARENT, PPOBJ_WORLD, Data.Rec.ParentID, parent_flags|OLW_WORDSELECTOR, extra_ptr);
	}
