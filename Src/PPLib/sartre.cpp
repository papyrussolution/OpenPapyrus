...
public:
	...
	struct ExprItem {
		...
		uint16 K;
		uint16 ArgCount; // ?????????? ?????????? (??? K == kOp)
		union {
			uint   SymbP; // ??????? ??????? ? R_Set.Pool (??? oneof(K, kLiteral, kConcept, kMorph, kRule))
			uint32 Op;    // ?? ???????? (??? K == kOp)
		};
		uint64 RSymb; // ????????????? ???????????? ??????? SymbP ? ???? ?????? (??? K == kConcept)
	};
	...
	struct ResolveRuleBlock {
		ResolveRuleBlock(SrDatabase & rDb, const STokenizer & rT, const SrSyntaxRuleSet::Rule & rRule) :
			R_Db(rDb), R_T(rT), R_Rule(rRule)
		{
		}
		const  SrSyntaxRuleSet::Rule & R_Rule;
		const  STokenizer & R_T;
		SrDatabase & R_Db;
		uint   StackP;
		uint   TextIdx;

		STokenizer::Item TItemBuf;
		SString TempBuf;
	};

	int    SLAPI __ResolveExprRule(ResolveRuleBlock & rB) const;
	...
	TSCollection <SrWordForm> WfList; // ?????? ?????????, ?????????? ? ????????. 
		// ????? ?????????? ????????? member ExprItem::RSymb ??? K == kMorph ????????? ?? ???????? 
		// ????? ?????? ?? ???????? [1..]
};
...
int SLAPI SrSyntaxRuleSet::ParseRule(SStrScan & rScan)
{
    ...
    THROW_PP(rScan.GetIdent(temp_buf), PPERR_SR_S_RULEIDEXPECTED); // @err ????????? ????? ???????
	ScanSkip(rScan);
	THROW_PP(rScan.Is("="), PPERR_SR_S_EQEXPECTED); // @err ????????? '='
	rScan.Incr();
					...
					if(!r_ei.RSymb) {
						if(rSet.GetS(r_ei.SymbP, temp_buf)) {
							if(temp_buf.NotEmptyS()) {
								uint   wf_pos = 0;
								SrWordForm * p_new_wf = rSet.WfList.CreateNewItem(&wf_pos);
								THROW_SL(p_new_wf);
...
int SrSyntaxRuleSet::__ResolveExprRule(ResolveRuleBlock & rB) const
{
	int    ok = 0;
	const SrSyntaxRuleSet::ExprStack & r_st = rB.R_Rule.ES;
	const SrSyntaxRuleSet::ExprItem * p_sti = (const SrSyntaxRuleSet::ExprItem *)r_st.at(--rB.StackP);
	if(p_sti->K == SrSyntaxRuleSet::kOp) {
		if(p_sti->Op == SrSyntaxRuleSet::opAnd) {
			assert(p_sti->ArgCount == 2);
			if(__ResolveExprRule(rB) && __ResolveExprRule(rB)) {
				ok = 1;
			}
		}
	...
	else if(p_sti->K == SrSyntaxRuleSet::kLiteral) {
		GetS(p_sti->SymbP, rB.TempBuf);
		if(rB.R_T.Get(rB.TextIdx, rB.TItemBuf) && rB.TItemBuf.Text == rB.TempBuf) {
			rB.TextIdx++;
			ok = 1;
		}
	}
	else if(p_sti->K == SrSyntaxRuleSet::kMorph) {
		if(p_sti->RSymb > 0 && p_sti->RSymb <= WfList.getCount()) {
			if(rB.R_T.Get(rB.TextIdx, rB.TItemBuf)) {
				SrWordForm * p_wf = WfList.at(p_sti->RSymb-1);
	...
	else if(p_sti->K == SrSyntaxRuleSet::kConcept) {
		if(p_sti->RSymb > 0) {
			SrNGram sng;
			SrNGram sng_local;
				...
				if(rB.R_Db.FetchWord(rB.TItemBuf.Text, &word_id) > 0) {
					...
					if(rB.R_Db.P_NgT->SearchByPrefix(sng, ng_list) > 0) {
						for(uint i = 0; i < ng_list.getCount(); i++) {
							...
							if(rB.R_Db.P_NgT->Search(ng_id, &sng_local) > 0) {
								if(sng_local.WordIdList.getCount() == 1) {
									assert(sng_local.WordIdList.at(0) == word_id);
									rB.R_Db.GetNgConceptList(sng_local.ID, 0, concept_list);
	...
	else if(p_sti->K == SrSyntaxRuleSet::kRule) {
	}
	return ok;
}
...
int __ProcessText(SrDatabase & rDb, const SrSyntaxRuleSet & rSet, const char * pResource, const SString & rTextUtf8)
{
	...
	if(idx_count) {
		...
		for(uint ridx = 0; ridx < rSet.GetRuleCount(); ridx++) {
			p_rule = rSet.GetRule(ridx);
			if(p_rule) {
				uint   stack_p = p_rule->ES.getPointer();
				rSet.__ResolveExprRule(blk);
			}
		}
	}
