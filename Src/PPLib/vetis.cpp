...
/* @2020213
//Запрос (ApplicationID: 427a50ec-36e9-44c7-b9aa-b6b4680287d0) вернулся с ошибкой, так как сведения о точке маршрута routePoint 1 указаны некорректно. 
// Не существует страны с guid:74a3cbb1-56fa-94f3-ab3f-e8db4940d96b и региона с guid:1fb7f16b-2ed0-4fc1-9719-b516fdb5c78b.
*/
		...
		if(oneof5(rec.Kind, kProductItem, kProduct, kSubProduct, kEnterprise, kBusinessEntity)) {
			if(rec.GuidRef || rec.UuidRef) {
				if(!rList.lsearch(&rec.ID, 0, CMPF_LONG)) {
					if(p_ref->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, rec.ID, PPTRPROP_NAME), temp_buf_u) > 0 && temp_buf_u.Len())
						;
					else {
...
int VetisEntityCore::EntityRecToEntity(const VetisEntityTbl::Rec & rRec, Entity & rE)
{
	...
	{
		SStringU temp_buf_u;
		PPRef->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, rRec.ID, PPTRPROP_NAME), temp_buf_u);
		if(temp_buf_u.Len()) {
...
int VetisEntityCore::DeleteEntity(PPID id, int use_ta)
{
	...
	};
	{
		...
		if(SearchByID_ForUpdate(&ET, PPOBJ_VETISENTITY, id, &rec) > 0) {
			...
			THROW(p_ref->TrT.SetText(SObjTextRefIdent(PPOBJ_VETISENTITY, id, PPTRPROP_NAME), 0, 0));
			if(rec.Kind == kVetDocument) {
				THROW(p_ref->TrT.SetText(SObjTextRefIdent(PPOBJ_VETISENTITY, id, VetisEntityCore::txtprpProductItemName), 0, 0));
				for(uint tti = 0; tti < SIZEOFARRAY(transp_num_tab); tti++) {
					const TranspNumTabEntry & r_tt_entry = transp_num_tab[tti];
					THROW(p_ref->TrT.SetText(SObjTextRefIdent(PPOBJ_VETISENTITY, id, r_tt_entry.TxtIdent), 0, 0));
				}
				THROW(p_ref->TrT.SetText(SObjTextRefIdent(PPOBJ_VETISENTITY, id, VetisEntityCore::txtprpGoodsCodeList), 0, 0));
				if(SearchByID_ForUpdate(&DT, PPOBJ_VETISENTITY, id, &doc_rec) > 0) {
					THROW_DB(DT.deleteRec());
				}
			}
...
int VetisEntityCore::SetEntity(Entity & rE, TSVector <UnresolvedEntity> * pUreList, PPID * pID, int use_ta)
{
	...
	if(!rE.Guid && !rE.Uuid) {
		...
		if(name_buf.NotEmptyS()) {
			...
			int sr = p_ref->TrT.SetText(SObjTextRefIdent(PPOBJ_VETISENTITY, entity_id, PPTRPROP_NAME), temp_buf_u, 0);
			THROW(sr);
			if(sr > 0 && ok < 0)
				ok = 1;
		}
		...
		if(pUreList && rE.Kind != kVetDocument) {
			if(!is_unresolved) {
				if(p_ref->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, entity_id, PPTRPROP_NAME), temp_buf_u) > 0 && temp_buf_u.Len())
					;
				else
					is_unresolved = 1;
			}
	...
	if(ok > 0) {
		...
		if(SearchDocument(entity.ID, &rec) > 0) {
			...
			if(rec.ProductItemID) {
				...
				p_ref->UtrC.SearchU(SObjTextRefIdent(PPOBJ_VETISENTITY, id, VetisEntityCore::txtprpProductItemName), temp_buf_u);
				if(temp_buf_u.Len()) {
			...
			}
			{
				...
				for(uint tti = 0; tti < SIZEOFARRAY(transp_num_tab); tti++) {
					const TranspNumTabEntry & r_tt_entry = transp_num_tab[tti];
					p_ref->UtrC.SearchU(SObjTextRefIdent(PPOBJ_VETISENTITY, id, r_tt_entry.TxtIdent), temp_buf_u);
					if(temp_buf_u.Len()) {
						temp_buf_u.CopyToUtf8(temp_buf, 0);
						temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
			...
			{
				p_ref->UtrC.SearchU(SObjTextRefIdent(PPOBJ_VETISENTITY, id, VetisEntityCore::txtprpGoodsCodeList), temp_buf_u);
				if(temp_buf_u.Len()) {
					temp_buf_u.CopyToUtf8(temp_buf, 0);
					temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
		...
		THROW(SetEntity(entity, pUreList, &result_id, 0));
		{
			...
			if(!rec.ProductItemID) {
				...
				if(temp_buf.NotEmptyS()) {
					int sr = p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_VETISENTITY, entity.ID, txtprpProductItemName), temp_buf.ToLower().Transf(CTRANSF_INNER_TO_UTF8), 0);
					THROW(sr);
					if(sr > 0 && ok < 0)
						ok = 1;
				}
			...
			if(r_bat.ProductMarkingList.getCount()) {
				...
				if(code_list_buf.NotEmptyS()) {
					int sr = p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_VETISENTITY, entity.ID, txtprpGoodsCodeList), code_list_buf.ToLower().Transf(CTRANSF_INNER_TO_UTF8), 0);
					THROW(sr);
					if(sr > 0 && ok < 0)
						ok = 1;
				}
		...
		THROW(SetEntity(entity, pUreList, &result_id, 0));
		{
			...
			}
			{
				...
				for(uint tti = 0; tti < SIZEOFARRAY(transp_num_tab); tti++) {
					...
					if(temp_buf.NotEmptyS()) {
						int sr = p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_VETISENTITY, entity.ID, r_tt_entry.TxtIdent), temp_buf.ToLower().Transf(CTRANSF_INNER_TO_UTF8), 0);
						THROW(sr);
						if(sr > 0 && ok < 0)
							ok = 1;
					}
	...
	Entity entity(kBusinessEntity, rItem);
	{
		...
		THROW(SetEntity(entity, pUreList, &rec.EntityID, 0));
		{
			...
			}
			{
				...
				if(temp_buf.NotEmptyS()) {
					int sr = p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_VETISENTITY, entity.ID, PPTRPROP_RAWADDR), temp_buf.ToLower().Transf(CTRANSF_INNER_TO_UTF8), 0);
					THROW(sr);
					if(sr > 0 && ok < 0)
						ok = 1;
				}
...
int VetisEntityCore::Get(PPID id, VetisEnterprise & rItem)
{
	...
	if(ok > 0) {
		...
		if(BT.search(0, &k0, spEq)) {
			...
			p_ref->UtrC.SearchU(SObjTextRefIdent(PPOBJ_VETISENTITY, entity.ID, PPTRPROP_RAWADDR), temp_buf_u);
			if(temp_buf_u.Len()) {
...
static IMPL_DBE_PROC(dbqf_vetis_entitytextfld_ip)
{
	...
	if(option == CALC_SIZE) {
		...
		SStringU temp_buf_u;
		if(entity_id) {
			p_ref->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, entity_id, PPTRPROP_NAME), temp_buf_u);
		}
		if(temp_buf_u.Len()) {
			temp_buf_u.CopyToUtf8(temp_buf, 0);
			temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
...
static IMPL_DBE_PROC(dbqf_vetis_businessmembtextfld_iip)
{
	...
	if(option == CALC_SIZE) {
		...
		if(enterprise_id) {
			p_ref->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, enterprise_id, PPTRPROP_NAME), temp_buf_u);
		}
		if(!temp_buf_u.Len() && bent_id) {
			p_ref->TrT.Search(SObjTextRefIdent(PPOBJ_VETISENTITY, bent_id, PPTRPROP_NAME), temp_buf_u);
		}
		if(temp_buf_u.Len()) {
			temp_buf_u.CopyToUtf8(temp_buf, 0);
			temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
	...
	for(uint i = 0; i < SIZEOFARRAY(tptab); i++) {
		if(tptab[i].ObjID) {
			int   sr = (tptab[i].TxtProp == PPTRPROP_NAME) ?
				p_ref->TrT.Search(SObjTextRefIdent(tptab[i].ObjType, tptab[i].ObjID, tptab[i].TxtProp), temp_buf_u) :
				p_ref->UtrC.SearchU(SObjTextRefIdent(tptab[i].ObjType, tptab[i].ObjID, tptab[i].TxtProp), temp_buf_u);
			if(sr > 0) {
				if(temp_buf_u.Len()) {
	...
	if(oneof2(objToMatch, otmFrom, otmTo)) {
		...
		PPAccSheet acs_rec;
		{
			...
			if(!temp_buf_u.Len() && enterprise_id) {
				VetisPersonTbl::Rec vp_rec;
				if(EC.SearchPerson(enterprise_id, &vp_rec) > 0) {
					p_ref->UtrC.SearchU(SObjTextRefIdent(PPOBJ_VETISENTITY, enterprise_id, PPTRPROP_RAWADDR), temp_buf_u);
					if(temp_buf_u.Len()) {
