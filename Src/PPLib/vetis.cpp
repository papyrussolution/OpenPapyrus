...
PPVetisInterface::PutBillRowBlock::PutBillRowBlock() : BillID(0), PersonID(0), DlvrLocID(0)
{
}
...
int SLAPI PPVetisInterface::ParseVetDocument(xmlNode * pParentNode, VetisVetDocument & rResult)
{
	...
	// @v10.5.4 ParseCertifiedConsignment(pParentNode, r_crtc);
	{
		PPObjBill * p_bobj = BillObj;
		PPIDArray lot_list;
						...
						SXml::WNode n_data(srb, SXml::nst("apl", "data"));
						{
							...
							}
							{
								...
								}
								{
									...
									n_c.PutInner(SXml::nst("vd", "perishable"), temp_buf.Z().Cat((r_bat.Flags & VetisBatch::fPerishable) ? "true" : "false"));
									{
										...
										for(uint pridx = 0; pridx < r_bat.Origin.Producer.getCount(); pridx++) { // @v10.5.6 single-->list
											...
											SXml::WNode n_producer(srb, SXml::nst("vd", "producer"));
											{
												...
												PutNonEmptyText(n_ent, "dt", "name", temp_buf);
											}
											const char * p_role = 0;
											if(p_pl_item->Role == VetisProducer::rolePRODUCER)
												p_role = "PRODUCER";
											else
												p_role = "UNKNOWN";
											if(p_role)
									...
									n_c.PutInner(SXml::nst("vd", "lowGradeCargo"), temp_buf.Z().Cat((r_bat.Flags & VetisBatch::fLowGradeCargo) ? "true" : "false"));
...
int EditVetisVetDocument(VetisVetDocument & rData)
{
	...
	if(CheckDialogPtrErr(&dlg)) {
		...
		{
			text_buf.Z();
			if(PPVetisInterface::GoodsDateToString(r_crtc.Batch.ExpiryDate.FirstDate, temp_buf) > 0)
				text_buf.Cat(temp_buf);
			if(PPVetisInterface::GoodsDateToString(r_crtc.Batch.ExpiryDate.SecondDate, temp_buf) > 0)
				text_buf.CatDivIfNotEmpty('-', 1).Cat(temp_buf);
			if(r_crtc.Batch.ExpiryDate.InformalDate.NotEmptyS())
				text_buf.CatDivIfNotEmpty(' ', 0).CatParStr(r_crtc.Batch.ExpiryDate.InformalDate);
		...
		if(rData.Flags & rData.fFromMainOrg) {
			...
			dlg->setCtrlReal(CTL_VETVDOC_ACKQTTY, r_crtc.Batch.AckVolume);
		...
		if(ExecView(dlg) == cmOK) {
			// @v10.5.8 {
			if(!(rData.Flags & rData.fFromMainOrg)) {
				dlg->getCtrlData(CTL_VETVDOC_ACKQTTY, &r_crtc.Batch.AckVolume);
				...
				if(id) {
					...
					if(EC.Get(id, item) > 0) {
						if(Filt.Flags & Filt.fAsSelector && pBrw->IsInState(sfModal)) {
							...
							if(EditVetisVetDocument(item) > 0) {
								// @v10.5.8 {
