// ATRNTMPL.CPP
// Copyright (c) A.Sobolev 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2013, 2014, 2015, 2016
...
int SLAPI PPAccTurnTempl::ResolveAlias(int side, AcctID * pAcct, const ATSubstObjects * pAtso) const
{
	...
	if(pAcct->ac && r_acc_obj.Fetch(pAcct->ac, &acc_rec) > 0 && acc_rec.Type == ACY_ALIAS) { // @v8.9.12 Search-->Fetch
		...
		const  TSArray <ATSubstObjects::Item> * p_atso_list = 0;
		if(side == PPDEBIT) {
...
{
	if(TVCOMMAND && TVCMD == cmOK && IsInState(sfModal)) {
