...
{ 
	int   ok = 1;
	{
		...
		PPRef->UtrC.Remove(SObjTextRefIdent(PPOBJ_PERSONEVENT, id, PPTRPROP_MEMO), 0);
		THROW(tra.Commit());
	}
	CATCHZOK
	return ok;
...
SString & PersonEventCore::GetItemMemo(PPID id, SString & rBuf) // @v11.1.12
{
	rBuf.Z();
	if(id) {
		PPRef->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_PERSONEVENT, id, PPTRPROP_MEMO), rBuf);
	...
	if(opID) {
		...
		if(prmrPersonID) {
			if(pop_pack.PCPrmr.PersonKindID)
				THROW(PsnObj.P_Tbl->BelongsToKind(prmrPersonID, pop_pack.PCPrmr.PersonKindID));
...
int PPObjPersonEvent::InitPacket(PPPsnEventPacket * pPack, const AddPersonEventFilt & rFilt, int interactive)
{
	...
	THROW(pop_obj.GetPacket(op_id, &pop_pack) > 0);
	{
		...
		if(pPack->Rec.PersonID && pop_pack.PCPrmr.PersonKindID) {
			THROW(PsnObj.P_Tbl->BelongsToKind(pPack->Rec.PersonID, pop_pack.PCPrmr.PersonKindID));
		}
	}
	{
		scard_id = 0;
		...
		if(pPack->Rec.SecondID && pop_pack.PCScnd.PersonKindID) {
			THROW(PsnObj.P_Tbl->BelongsToKind(pPack->Rec.SecondID, pop_pack.PCScnd.PersonKindID));
		}
	}
	CATCHZOK
	return ok;
...
int PPObjPersonEvent::CheckRestrictions(const PPPsnEventPacket * pPack, PPID personID, PPID scardID, const PPPsnOpKindPacket::PsnConstr * pConstr)
{
	...
	if(pConstr && personID) {
		...
		if(pConstr->PersonKindID) {
			THROW(PsnObj.P_Tbl->BelongsToKind(personID, pConstr->PersonKindID));
		}
	...
	}
	{
		...
		if(id) {
			...
			if(pPack == 0) {
				...
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(Obj, id, PPTRPROP_MEMO), pPack->SMemo.Transf(CTRANSF_INNER_TO_UTF8), 0)); // @v11.1.12
...
PsnEventDialog::PsnEventDialog(Param * pParam, PPObjPersonEvent * pPeObj) : PPListDialog(DLG_PSNEVNT, CTL_PSNEVNT_TAGLIST), P_PeObj(pPeObj), P(*pParam)
{
	...
	SetupTimePicker(this, CTL_PSNEVNT_TIME, CTLTM_PSNEVNT_TIME);
...
void PsnEventDialog::editRegister()
{
	...
	if(P_PeObj->RegObj.EditDialog(&regrec, 0, static_cast<const PPPersonPacket *>(0)) > 0)
		Pack.Reg = regrec;
}
...
void PsnEventDialog::postVK()
{
	...
	public:
		PostWallVkDlg(VkInterface & rIfc, SString & rMsg, SString & rLinkFilePath) : TDialog(DLG_VK_POST), R_Ifc(rIfc),
			Message(rMsg), LinkFilePath(rLinkFilePath)
		{
			addGroup(ctlgroupVKImg, new ImageBrowseCtrlGroup(CTL_VKPOST_IMAGE, cmAddImage, cmDelImage));
			setCtrlString(CTL_VK_POST_TEXT, Message);
			{
				ImageBrowseCtrlGroup::Rec rec;
				rec.Path = LinkFilePath;
			...
			if(TVCOMMAND) {
				if(TVCMD == cmOK) {
					...
					if(getGroupData(ctlgroupVKImg, &rec)) {
						LinkFilePath = rec.Path;
						//LinkFileType = 1;
					}
					//vk_client.WallPost(Data, temp_buf);
					R_Ifc.WallPost(Message, LinkFilePath, temp_buf);
				}
			}
	...
	if(ifc.Setup(0, 0)) {
		SString message_buf;
		SString img_link_buf;
		getCtrlString(CTL_PSNEVNT_MEMO, message_buf);
		ImageBrowseCtrlGroup::Rec rec;
		if(getGroupData(ctlgroupIBG, &rec)) {
			img_link_buf = rec.Path;
			//data.LinkFileType = 1;
		}
		PostWallVkDlg * dlg = new PostWallVkDlg(ifc, message_buf, img_link_buf);
		//dlg->setDTS(&data);
