// V_GDSOPR.CPP
// Copyright (c) A.Sobolev 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2013, 2014, 2015, 2016
...
#define GOA_CACHE_DELTA           0 // Количество элементов, сбрасываемое из кэша в базу данных при переполнении кэша // @v6.4.4 1024-->0
...
struct GoaAddingBlock {
	...
	double Rest;           // @v6.2.0
	double PhRest;         // @v7.5.5
	double RestCostSum;    // @v6.2.0
	double RestPriceSum;   // @v6.2.0
};
...
{
	int    si;
	CMPCASCADE4(si, (GoaCacheItem *)i1, (GoaCacheItem *)i2, Sign, GoodsID, ArID, LocID);
	return si;
}

IMPL_CMPFUNC(GoaCacheItem_P, i1, i2)
{
	GoaCacheItem * p1 = (GoaCacheItem *)i1;
	GoaCacheItem * p2 = (GoaCacheItem *)i2;
	int    si;
...
{
	GoaUniqItem * p1 = (GoaUniqItem *)i1;
	GoaUniqItem * p2 = (GoaUniqItem *)i2;
	int    si;
	CMPCASCADE4(si, p1, p2, Sign, GoodsID, ArID, LocID);
	if(si == 0) {
		double p = R6(p1->Price-p2->Price);
...
int SLAPI GoodsOpAnalyzeTotal::Init()
{
	Count = 0;
	InCount = 0;
...
int GoodsOpAnalyzeFilt::AddTradePlanBillID(PPID billID)
{
...
int GoodsOpAnalyzeFilt::ZeroCompareItems()
{
	CmpPeriod.SetZero();
...
void GoodsOpAnlzFiltDialog::setupCtrls(long prevFlags)
{
	...
	uint   enbl = (Data.OpGrpID != GoodsOpAnalyzeFilt::ogInOutAnalyze &&
		!(Data.Flags & GoodsOpAnalyzeFilt::fIntrReval) && op_type != PPOPT_GOODSREVAL);
	uint   disable_abc = 0;
...
int GoodsOpAnlzFiltDialog::setupFlags()
{
	if(!!Data.Sgb) {
...
public:
	...
	{
		PrevID = 0;
		PPLoadString(PPSTR_TEXT, PPTXT_GOODSOPRADDVALNAMES, Items);
...
int SLAPI GoodsOpAnlzCmpFiltDialog::editItem(long pos, long id)
{
	long   set_flags = (Data.Flags & GoodsOpAnalyzeFilt::fCrosstab) ? GoodsOpAnalyzeFilt::ffldMainPeriod :
		(GoodsOpAnalyzeFilt::ffldMainPeriod|GoodsOpAnalyzeFilt::ffldCmpPeriod|GoodsOpAnalyzeFilt::ffldDiff); // @v6.4.11 AHTOXA
	long   flags = 0;
...
int SLAPI GoodsOpAnlzCmpFiltDialog::setupFlags()
{
	uint   p = 0;
...
public:
	...
	int setDTS(const GoodsOpAnalyzeFilt * pData)
	{
		...
		int enbl = (Data.OpGrpID != GoodsOpAnalyzeFilt::ogInOutAnalyze &&
			(!(Data.Flags & GoodsOpAnalyzeFilt::fIntrReval)) &&
			(!(Data.Flags & GoodsOpAnalyzeFilt::fPriceDeviation)) &&
			GetOpType(Data.OpID) != PPOPT_GOODSREVAL);
	...
	int getDTS(GoodsOpAnalyzeFilt * pData)
	{
		...
		// } @v7.9.9
		// @v7.9.9 getCtrlData(CTLSEL_BILLFLT_SUBSTGDS, &Data.Sgg);
...
int SLAPI PPViewGoodsOpAnalyze::Init_(const PPBaseFilt * pFilt)
{
	...
	double ufp_factor[4];
	MEMSZERO(ufp_factor);
	PPUserFuncProfiler ufp(PPUPRF_VIEW_GOODSOPANLZ); // @v8.1.12
	...
	do {
		while(P_IterQuery && P_IterQuery->nextIteration() > 0) {
			if(pItem) {
				...
				pItem->Rest           = rec.Rest;
				pItem->PhRest         = rec.PhRest; // @v7.5.5
				...
				if(P_Ct == 0 && Filt.CmpPeriod.IsZero() == 0) {
					...
					pItem->PhRest.SetCm(rec2.PhRest); // @v7.5.5
	...
	for(q.initIteration(0, &k, spFirst); q.nextIteration() > 0;) {
		if(Filt.IsValidABCGroup(p_t->data.InOutTag)) {
			...
			if(p_t->data.InOutTag < 0 && (!(Filt.Flags & GoodsOpAnalyzeFilt::fUseABCAnlz))) {
				...
				pTotal->Rest   += p_t->data.Rest; // @v7.5.5
				pTotal->PhRest += p_t->data.PhRest; // @v7.5.5
...
int ABCGrpStorageList::IncTotalItem(uint groupBy, GoodsOpAnalyzeViewItem * pItem)
{
	int ok = -1;
	if(pItem) {
		long pos = GetGoodsGrpPos(pItem->GoodsID);
		if(pos >= 0) {
...
ABCGroupingRecsStorage * ABCGrpStorageList::GetStorage(PPID goodsID)
{
	...
	if(P_StorageList) {
		long pos = GetGoodsGrpPos(goodsID);
		if(pos >= 0)
...
int SLAPI PPViewGoodsOpAnalyze::CreateTempTable(double * pUfpFactors)
{
	...
	if(P_TradePlanPacket) {
		...
		*/
		...
		pUfpFactors[0] += (double)P_TradePlanPacket->getCount(); // @v8.1.12
	}
	if(Filt.AgentID) {
	...
	if(Filt.BillList.IsExists() && !(Filt.Flags & GoodsOpAnalyzeFilt::fBillListAsTradePlan)) {
		...
		if(oneof2(Filt.OpGrpID, GoodsOpAnalyzeFilt::ogSelected, GoodsOpAnalyzeFilt::ogInOutAnalyze)) {
			int    are_all_reval = 1;
			...
			if(P_TempTbl->searchForUpdate(3, &k3, spFirst)) do {
				...
				if(rec.GoodsID != prev_goods_id || rec.LocID != prev_loc_id) {
					...
					if(r > 0) {
						...
						P_TempTbl->data.PhRest = gr_item.PhRest; // @v7.5.5
			...
			if(!(Filt.Flags & GoodsOpAnalyzeFilt::fBadSellingGoods) && (Filt.Flags & GoodsOpAnalyzeFilt::fAddNzRestItems)) {
				...
				for(gr_view.InitIteration(); gr_view.NextIteration(&gr_item) > 0;) {
					if(!goods_list.SearchPair(gr_item.GoodsID, gr_item.LocID, 0)) {
						...
						blk.PhRest       = gr_item.PhRest; // @v7.5.5
...
int SLAPI PPViewGoodsOpAnalyze::FlashCacheItem(BExtInsert * pBei, const GoaCacheItem * pItem)
{
	...
	if(!P_TempTbl)
		...
		r_rec.PhRest   = pItem->PhRest; // @v7.5.5
		...
		rec.Rest     = pItem->Rest;
		rec.PhRest   = pItem->PhRest; // @v7.5.5
...
int SLAPI PPViewGoodsOpAnalyze::AddItem(GoaAddingBlock * pBlk)
{
	...
	if(r > 0) {
		...
		if(pBlk->Flags & GoaAddingBlock::fTradePlan) {
			...
			p_item->PhRest += pBlk->PhRest; // @v7.5.5
		...
		if(pBlk->Flags & GoaAddingBlock::fTradePlan) {
			...
			item.Qtty     = _q;
			item.Rest     = pBlk->Rest;
			item.PhRest   = pBlk->PhRest; // @v7.5.5
...
int SLAPI PPViewGoodsOpAnalyze::InitAddingBlock(const PPBillPacket * pPack, double part, int sign, GoaAddingBlock * pBlk)
{
	const  long f = Filt.Flags;
	pBlk->ParentGrpID = 0;
...
int SLAPI PPViewGoodsOpAnalyze::PreprocessTi(const PPTransferItem * pTi, const PPIDArray * pSupplBillList, long substBillVal, GoaAddingBlock * pBlk)
{
	...
	if(Filt.GoodsIdList.IsExists()) {
		if(!Filt.GoodsIdList.CheckID(labs(pTi->GoodsID)))
			return -1;
	}
...
int SLAPI PPViewGoodsOpAnalyze::ViewDetail(PPID locID, PPID goodsID, short abcGroup, int viewAllLots)
{
	...
	if(abcGroup != 0 && Filt.ABCAnlzGroup > 0) {
		...
		if(Filt.Flags & GoodsOpAnalyzeFilt::fEachLocation) {
			if(locID) {
				temp_filt.LocList.Set(0); // @v6.4.11 FreeAll()->Set(0)
...
int SLAPI PPViewGoodsOpAnalyze::ViewGraph()
{
	...
	if(Filt.ABCAnlzGroup > 0) {
		...
		if(abc_ary.getCount()) {
			...
			if(goa_total.Cost) {
				plot.PutData(PPGetWord(PPWORD_SUM, 1, temp_buf).Cat("_ЦП"), 1);
				for(c = 0; c < abc_ary.getCount(); c++)
					plot.PutData(abc_ary.at(c).SumCost / goa_total.Cost);
				plot.PutEOR();
			}
			if(goa_total.Price) {
				plot.PutData(PPGetWord(PPWORD_SUM, 1, temp_buf).Cat("_ЦР"), 1);
				for(c = 0; c < abc_ary.getCount(); c++)
...
int SLAPI PPViewGoodsOpAnalyze::GetTempTableName(SString & rBuf) const
{
...
int SLAPI PPViewGoodsOpAnalyze::Print(const void *)
{
	...
	int    no_cost = ((State & sAccsCost) ? 0 : 1), disable_grouping = 0;
	uint   rpt_id = 0;
	TDialog * dlg = 0;
	GoodsOpAnalyzeFilt old_filt = Filt;
	if(Filt.ABCAnlzGroup <= 0) {
...
int PPALDD_GoodsOpAnlz::NextIteration(PPIterID iterId, long rsrv)
{
	...
	I.Rest     = item.Rest;
	I.PhRest   = item.PhRest;        // @v7.5.5
	I.RestCost = item.RestCostSum;   // @v7.5.5
	I.RestPrice = item.RestPriceSum; // @v7.5.5
