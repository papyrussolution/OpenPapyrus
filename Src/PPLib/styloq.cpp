...
SJson * PPStyloQInterchange::Document::ToJsonObject() const
{
	...
	*/
	...
	if(PosNodeID) { // @v11.4.6
		p_result->InsertInt("posnodeid", PosNodeID);
	}
	if(AgentID) { // @v11.4.6
		p_result->InsertInt("agentid", AgentID);
	}
	if(ClientID) {
...
int PPStyloQInterchange::MakeIndexingRequestCommand(const StyloQCore::StoragePacket * pOwnPack, const StyloQCommandList::Item * pCmd, long expirationSec, 
	PPObjIDArray & rOidList, S_GUID & rDocUuid, SString & rResult)
{
	/*
	...
	SString & rResult, SString & rDocDeclaration, bool debugOutput)
{
	...
	THROW(GetOwnIdent(bc_own_ident, 0));
	{
		...
		THROW(GetOpData(op_id, &op_rec) > 0);
		{
			...
			PPIDArray goods_id_list;
			PPIDArray cli_id_list;
			BillFilt bill_filt;
			PPViewBill bill_view;
			BillViewItem bill_item;
			bill_filt.OpID = op_id;
			bill_filt.Period.low = plusdate(getcurdate_(), -lookback_days);
			...
			if(temp_entry_list.getCount()) {
				...
				if(_c) do {
					...
					PPBillPacket bpack;
					if(p_bobj->ExtractPacket(r_entry.ID, &bpack) > 0) {
						assert(bpack.Rec.OpID == op_id);
						...
						for(uint i = 0; i < bpack.GetTCount(); i++) {
							...
							p_doc_ti->GoodsID = labs(r_ti.GoodsID);
			...
			case StyloQCommandList::sqbcRsrvPushIndexContent:
				{
					...
					THROW(MakeIndexingRequestCommand(&own_pack, pCmd, 3600*24, oid_list, doc_uuid, result));
				}
				break;
			case StyloQCommandList::sqbcReport:
				PPWait(1);
	...
	const SJson * pDeclaration, const SJson * pDocument, PPID * pResultID)
{
	...
	THROW_PP(p_dict && p_dict->GetDbSymb(db_symb), PPERR_SESSNAUTH);
	{
		...
		THROW(doc.Uuid); // @err
		THROW(oneof3(doc.OpID, PPEDIOP_ORDER, StyloQCommandList::sqbdtSvcReq, StyloQCommandList::sqbdtCCheck));
		THROW(checkdate(doc.CreationTime.d) || checkdate(doc.Time.d));
		if(pDeclaration) {
			
		}
		if(doc.OpID == StyloQCommandList::sqbdtCCheck) {
			PPID   pos_node_id = 0;
			PPObjCashNode cn_obj;
		...
		else if(doc.OpID == StyloQCommandList::sqbdtSvcReq) {
				...
				}
				{
					...
					}
					{
						...
						if(!p_bpack) {
								...
								if(StyloQCommandList::GetFullList(db_symb, full_cmd_list)) {
									...
									if(p_cmd && p_cmd->BaseCmdId == StyloQCommandList::sqbcRsrvOrderPrereq) {
										...
										if(stylopalm_id) {
											PPObjStyloPalm stp_obj;
											PPStyloPalmPacket stp_pack;
											if(stp_obj.GetPacket(stylopalm_id, &stp_pack) > 0) {
												op_id = stp_pack.Rec.OrderOpID;
												loc_id = stp_pack.LocList.GetSingle();
											}
										}
									}
								}
							...
							THROW(GetOpData(op_id, &op_rec) > 0);
							// } @v11.4.6 
							THROW(bpack_.CreateBlank_WithoutCode(op_id, 0, loc_id, 0));
							...
							op_id = p_bpack->Rec.OpID;
							THROW(GetOpData(op_id, &op_rec) > 0);
						}
						assert(op_rec.ID > 0);
						assert(p_bpack);
						assert(p_bpack->Rec.OpID == op_id);
			...
			if(event.isCmd(cmOK)) {
				...
				// "requestblobinfolist"
					...
					if(P_Ic->GetOwnPeerEntry(&own_pack) > 0) {
						...
						dip.ClearParam();
						if(P_Ic->MakeIndexingRequestCommand(&own_pack, p_cmd_item, 3600*24, oid_list, doc_uuid, dip.CommandJson)) {
							// @v11.4.5 {
							assert(doc_uuid);
							{
								PPID   doc_id = 0;
	...
	const TSCollection <StyloQCore::IgnitionServerEntry> & rMediatorList, const StyloQCommandList::Item * pCmdItem)
{
	...
	THROW(MakeIndexingRequestCommand(&rOwnPack, pCmdItem, 3600*24, oid_list, doc_uuid, dip_indexing.CommandJson));
	THROW_PP(rOwnPack.Pool.Get(SSecretTagPool::tagSvcIdent, &own_ident), PPERR_SQ_UNDEFOWNSVCID);
	// @v11.4.5 {
	assert(doc_uuid);
	{
...
bool PPStyloQInterchange::FetchBlobSignature(const SBinaryChunk & rOwnIdent, PPObjID oid, uint blobN, SString & rSignature)
{
	...
	int    gbr = DS.Stq_GetBlob(rOwnIdent, oid, blobN, bi);
	if(gbr > 0) {
		rSignature = bi.Signature;
	}
	else if(gbr < 0) {
		int gbir = GetBlobInfo(rOwnIdent, oid, blobN, gbifSignatureOnly, bi, 0);
