...
#define POP3_PW_SIZE 20 // @attention изменение значени€ требует конвертации хранимого парол€
	...
	if(Msg.Flags & SMailMessage::fMultipart) {
		...
		while(line_readed || ReadLine()) {
			...
			if(oneof3(bound_kind, 0, 1, 2)) {
				...
				if(p_out) {
					//
					// ƒо этого сохран€лс€ предыдущий присоединенный файл.
					// «акрываем его handler.
					//
					if(boundary)
						ok = 1;
					SFile::ZClose(&p_out);
				}
				if(bound_kind == 1) // ƒальше (оп€ть) следует присоединенный файл
					boundary = 1;
				else                // ¬се присоединенные файлы закончились
					boundary = 0;
			}
			else if(boundary) {
				SString disp;
				...
				if(GetField(P_LineBuf, PPMAILFLD_CONTENTCOD, disp) > 0) {
					if(disp.CmpNC("base64") != 0)
						boundary = 0; // ≈сли тип кодировки отличаетс€ от base64, то не принимаем эту полосу.
				}
				else {
#if 0 // @v8.2.2 {
	...
	*/
	...
	_PUTS(buf, out);
	{
		if(pMsg->GetField(SMailMessage::fldSubj, temp_buf) == SUBJECTDBDIV) {
			//
			// —пециальный случай: так как кодировка в UTF8 введена с версии 7.6.3, котора€ не предполагает
			// обновлений во всех разделах, то дабы более старые версии могли прин€ть почту из 7.6.3 и выше,
