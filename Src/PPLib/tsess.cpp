	...
	while(ok < 0 && search(4, &k4, sp) && k4.PrcID == prcID) {
		if(PPObjTSession::GetSessionKind(data, 0) == kind && data.Status != TSESST_CANCELED) {
			...
			if(finish.d == 0 || cmp(finish, rDtm) > 0) {
				...
				//
				// ≈сли обнаружена сесси€, котора€ закончилась ранее заданного момента, то
				// поиск можно не продолжать - все остальные сессии имеют меньшее врем€ окончани€,
	...
	TSessionTbl::Rec rec;
	{
		...
		if(*pID) {
			...
			if(pRec == 0) {
				...
				// реализуетс€ функцией PPObject::Remove, котора€ самосто€тельно заносит
				// это событие
			}
			else {
				//
				// »зменение пакета
				//
				// ≈сли сесси€ €вл€етс€ незначительным простоем, то просто удал€ем ее
				if(PPObjTSession::IsIdleInsignificant(pRec, rec.Status)) {
					...
					if(rec.PrcID != pRec->PrcID || pRec->Num == 0) { // @v7.6.5 (|| pRec->Num == 0)
						//
						// ≈сли измен€етс€ процессор в сессии, то обновл€ем значение счетчика Number
						// дл€ того, чтобы избежать дублировани€ индекса
						//
						long   sess_number = 0;
