				...
				if(in_tbl.isDeletedRec() <= 0) {
					...
					if(sc_rec.Code[0]) {
						...
						if(P_Tbl->SearchCode(0, sc_rec.Code, 0) > 0) {
							...
							sc_rec.PDis = fmul100i(pctdis);
							P_Tbl->CopyBufFrom(&sc_rec, sizeof(sc_rec));
		...
		while(ie.ReadRecord(&rec, sizeof(rec), &dyn_rec) > 0) {
			...
			if(name.NotEmptyS()) {
				if(IeParam.Flags & PPPersonImpExpParam::f2GIS) {
					...
					//
					(temp_buf2 = "офис").Transf(CTRANSF_OUTER_TO_INNER); // @[ru] // @v12.5.7 ToOem()-->Transf(CTRANSF_OUTER_TO_INNER)
