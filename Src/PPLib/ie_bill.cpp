...
int DocNalogRu_Reader::ReadSingleXmlFile(const char * pFileName, FileInfo & rHeader, TSCollection <DocumentInfo> & rDocList)
{
	...
	if(fileExists(pFileName)) {
		...
		SString extra_key;
		SString extra_val;
		SString norm_barcode; // Нормализованное представление штрихкода (только как параметр-заглушка для DiagBarcode)
						...
						if(SXml::IsName(p_n2, GetToken_Utf8(PPHSC_RU_INVOICEHEADER))) {
									...
									for(const xmlNode * p_n4 = p_n3->children; p_n4; p_n4 = p_n4->next) {
										if(SXml::IsName(p_n4, GetToken_Utf8(PPHSC_RU_TEXTINF))) {
											if(ReadExtraValue(p_n4, extra_key, extra_val) > 0) {
												if(extra_key.IsEqiUtf8(GetToken_Utf8(PPHSC_RU_EXTRA_GLN_SUPPL))) {
													Participant * p_part = p_new_doc->GetParticipant(EDIPARTYQ_SELLER, true);
													if(p_part)
														p_part->GLN = extra_val.Transf(CTRANSF_UTF8_TO_INNER);
												}
												else if(extra_key.IsEqiUtf8(GetToken_Utf8(PPHSC_RU_EXTRA_GLN_CONSIGNOR))) {
													Participant * p_part = p_new_doc->GetParticipant(EDIPARTYQ_CONSIGNOR, true);
													if(p_part)
														p_part->GLN = extra_val.Transf(CTRANSF_UTF8_TO_INNER);
												}
												else if(extra_key.IsEqiUtf8(GetToken_Utf8(PPHSC_RU_EXTRA_WAYBILLCODE))) {
													p_new_doc->Code = extra_val.Transf(CTRANSF_UTF8_TO_INNER);
												}
												else if(extra_key.IsEqiUtf8(GetToken_Utf8(PPHSC_RU_EXTRA_WAYBILLDATE))) {
							...
							for(const xmlNode * p_n3 = p_n2->children; p_n3; p_n3 = p_n3->next) {
								if(SXml::IsName(p_n3, GetToken_Utf8(PPHSC_RU_WAREINFO))) {
									...
									for(const xmlNode * p_n4 = p_n3->children; p_n4; p_n4 = p_n4->next) {
										if(SXml::IsName(p_n4, GetToken_Utf8(PPHSC_RU_GTD))) { // @v12.7.9 Сведения о декларации на товары
											if(SXml::GetAttrib(p_n4, GetToken_Utf8(PPHSC_RU_GTD_COUNTRYCODE), temp_buf)) { // @v12.7.9 Цифровой код страны происхождения товара (графа 10 счета-фактуры)
												if(GetCountrySymb2(temp_buf.ToULong(), temp_buf)) {
													const SrUedContainer_Rt * p_uedc = DS.GetUedContainer();
													if(p_uedc) {
														p_item->UedGtdCountry = p_uedc->SearchSymb(temp_buf, UED_META_STATU);
													}
												}
											}
											if(SXml::GetAttrib(p_n4, GetToken_Utf8(PPHSC_RU_GTD_NUMBER), temp_buf)) { // @v12.7.9 Регистрационный номер декларации на товары (графа 11 счета-фактуры)
												p_item->GtdNumber = temp_buf;
											}
										}
										else if(SXml::IsName(p_n4, GetToken_Utf8(PPHSC_RU_EXCISE))) {
										}
											...
											if(SXml::GetAttrib(p_n4, GetToken_Utf8(PPHSC_RU_WARECODE), temp_buf)) {
												temp_buf.Strip();
												int   bc_diag = 0;
												int   bc_std = 0;
											...
											for(const xmlNode * p_n5 = p_n4->children; p_n5; p_n5 = p_n5->next) {
												if(SXml::IsName(p_n5, GetToken_Utf8(PPHSC_RU_WAREIDENTBLOCK))) {
													if(SXml::GetAttrib(p_n5, GetToken_Utf8(PPHSC_RU_WAREIDENT_TPACKCODE), temp_buf)) { // Бывает и такое: единственная марка в атрибуте!
													...
													for(const xmlNode * p_n6 = p_n5->children; p_n6; p_n6 = p_n6->next) {
														...
														else if(SXml::GetContentByName(p_n6, GetToken_Utf8(PPHSC_RU_WAREIDENT_TPACKCODE), temp_buf) > 0) {
															p_item->MarkList.add(temp_buf.Transf(CTRANSF_UTF8_TO_INNER)); // номер марки
														}
													}
												}
...
PPBillImpExpParam::PPBillImpExpParam(uint recId, long flags) : PPImpExpParam(recId, flags), Flags(0), ImpOpID(0), PredefFormat(piefUndef), DtoPersonID(0),
	FixTagID(0), TiOrdCodeTagID(0)
{
	Object1SrchCode.Z();
	Object2SrchCode.Z();
	TiOrdCodeTagID = PPObjTag::Helper_GetTagIdBySymb("TI-ORDER-CODE", OTTYP_STRING); // @v12.5.12
}
...
/*virtual*/int PPBillImpExpParam::SerializeConfig(int dir, PPConfigDatabase::CObjHeader & rHdr, SBuffer & rTail, SSerializeContext * pSCtx)
{
	...
	if(dir > 0) {
		...
		if(FixTagID && TagObj.Search(FixTagID, &tag_rec) > 0 && tag_rec.ObjTypeID == PPOBJ_BILL) {
			if(tag_rec.Symb[0])
		...
		for(uint i = 0; i < param_list.getCount(); i++) {
			...
			switch(item.Id) {
				case IMPEXPPARAM_BILH_IMPOPSYMB:
					...
					break;
				case IMPEXPPARAM_BILH_FIXTAG:
					{
						PPID   reg_tag_id = 0;
						if(TagObj.SearchBySymb(temp_buf, &reg_tag_id) > 0)
							FixTagID = reg_tag_id;
						else if(TagObj.Search(temp_buf.ToLong(), &tag_rec) > 0)
							FixTagID = tag_rec.ID;
					}
...
int PPBillImpExpParam::WriteIni(PPIniFile * pFile, const char * pSect) const
{
	...
	{
		// Фиксирующий тег используется терерь и при импорте и при экспорте {
		PPObjectTag2 tag_rec;
		PPGetSubStr(params, IMPEXPPARAM_BILH_FIXTAG, fld_name);
		if(FixTagID && TagObj.Search(FixTagID, &tag_rec) > 0) {
			if(tag_rec.Symb[0])
...
int PPBillImpExpParam::ReadIni(PPIniFile * pFile, const char * pSect, const StringSet * pExclParamList)
{
	...
	if(PPGetSubStr(params, IMPEXPPARAM_BILH_FIXTAG, fld_name)) {
		...
		if(pFile->GetParam(pSect, fld_name, param_val) > 0) {
			PPObjectTag2 tag_rec;
			PPID   tag_id = 0;
			if(TagObj.SearchBySymb(param_val, &tag_id) > 0)
				FixTagID = tag_id;
			else if(TagObj.Search(param_val.ToLong(), &tag_rec) > 0) 
				FixTagID = tag_rec.ID;
		}
	}
...
int BillHdrImpExpDialog::setDTS(const PPBillImpExpParam * pData)
{
	...
	AddClusterAssoc(CTL_IMPEXPBILH_FLAGS, 6, PPBillImpExpParam::fChZnMarkAsCDATA);
	AddClusterAssoc(CTL_IMPEXPBILH_FLAGS, 7, PPBillImpExpParam::fChZnMarkGTINSER);
	...
	setCtrlString(CTL_IMPEXPBILH_OUTRFMTV, Data.OuterFormatVer);
...
int BillHdrImpExpDialog::getDTS(PPBillImpExpParam * pData)
{
	...
	getCtrlString(CTL_IMPEXPBILH_OUTRFMTV, Data.OuterFormatVer);
	getCtrlData(CTLSEL_IMPEXPBILH_REGTAG, &Data.FixTagID);
	...
	public:
		...
		SelectBillImpCfgDialog(uint dlgId, const char * pIniFileName, int import) : TDialog(dlgId), P_Data(0), Import(import)
		{
			...
			if(!Import) {
				...
				PPSetupLocalStateWordSelectorOnInputLine(this, DLG_RUNIE_BILL_EXP, CTL_IEBILLSEL_MAILSUBJ); // @v12.7.9
			}
		}
		...
		int    setDTS(PPBillImpExpBaseProcessBlock * pData)
		{
			...
			if(Import) {
				...
				AddClusterAssoc(CTL_IEBILLSEL_FLAGS, 3, PPBillImpExpBaseProcessBlock::fEgaisVer4);
				...
				// @v12.7.9 SetupWordSelector(CTL_IEBILLSEL_MAILSUBJ, new TextHistorySelExtra("emailsubj-exportbills-common"), 0, 2, WordSel_ExtraBlock::fFreeText);
			}
			CATCHZOKPPERR
			return ok;
		}
							...
							if(dfk == dfkFormula) {
								...
								if(arg_scan.GetIdent(temp_buf) && rIep.OtrRec.SearchName(temp_buf.Strip(), &outer_fld_pos)) {
									...
									if(TagObj.SearchBySymb(temp_buf, &tag_id, &tag_rec) > 0 && rIep.OtrRec.GetFieldByPos(outer_fld_pos, &outer_fld)) {
										...
										/* @v12.5.10 temp_str_[0] = 0; sttostr(outer_fld.T.Typ, p_outer_fld_data, 0, temp_str_); temp_buf = temp_str_; */
										sttosstr(outer_fld.T.Typ, p_outer_fld_data, 0, temp_buf); // @v12.5.10
			...
			if(isempty(rRgi.GoodsName)) {
				...
				if(GObj.InitPacket(&gpack, gpkndGoods, parent_id, 0, /*rRgi.Barcode*/0)) {
					...
					while(GObj.SearchByName(goods_name_buf, &by_name_id, 0) > 0) {
						...
						(goods_name_buf = rRgi.GoodsName).Strip();
						const size_t sum_len = goods_name_buf.Len() + suffix.Len();
...
int PPBillImporter::BillToBillRec(const Sdr_Bill * pBill, PPBillPacket * pPack)
{
	...
	if(pBill && pPack) {
		...
		if(ar_id || !acs_id) {
			...
			pPack->SMemo = (temp_buf = pBill->Memo).Transf(CTRANSF_OUTER_TO_INNER);
...
int PPBillImporter::ReadSingleEancomFile(xmlParserCtxt * pXmlCtx, const SString & rFilePath, void * pDocPackList/*TSCollection <PPEdiProcessor::Packet>* */)
{
	int    ok = -1;
	PPXmlFileDetector xfd;
	int    format = 0;
	TSCollection <PPEdiProcessor::Packet> * p_doc_pack_list = static_cast<TSCollection <PPEdiProcessor::Packet> *>(pDocPackList);
	if(xfd.Run(rFilePath, &format)) {
		if(format == xfd.Eancom) {
			PPEanComDocument s_doc(this);
			s_doc.SetImportOpID(OpID);
			s_doc.SetImportLocID(LocID);
			SFsPath ps(rFilePath);
			SString edi_ident_buf(ps.Nam);
			edi_ident_buf.Transf(CTRANSF_UTF8_TO_INNER);
			if(p_doc_pack_list) {
				const int rdr = s_doc.Read_Document(pXmlCtx, rFilePath, edi_ident_buf, *p_doc_pack_list);
				if(rdr) {
					ok = 1;
				}
				else {
...
int PPBillImporter::Run()
{
				...
				if(SArchive::List(SArchive::providerLA, &arc_format, file_name, 0, fep) > 0) {
					...
					for(uint i = 0; i < fep.GetCount(); i++) {
						if(fep.Get(i, &fe, &arc_sub) > 0) {
							...
							if(SFile::WildcardMatch("*.xml", arc_sub_filename)) {
								...
								for(uint ssap = 0; ss_arc_files.get(&ssap, temp_buf);) {
									if(fileExists(temp_buf)) {
										ReadSingleEancomFile(p_xml_ctx, temp_buf, &doc_pack_list);
									}
								}
							}
						}
	...
	else if(BillParam.PredefFormat == piefCokeOrder) {
		PPID   suppl_ar_id = 0;
	...
	if(ok > 0) {
		if(Flags & fPaymOrdersExp) {
			...
			if(BillParam.PredefFormat) {
				const   int predef_fmt_xml[] = {
					piefNalogR_Invoice, piefNalogR_REZRUISP, piefNalogR_SCHFDOPPR, piefNalogR_ON_NSCHFDOPPRMARK, piefNalogR_ON_NSCHFDOPPR, piefNalogR_ON_NKORSCHFDOPPR,
					piefNalogR_Etrn_T1, piefNalogR_Etrn_T2, piefNalogR_Etrn_T3, piefNalogR_Etrn_T4
				};
				for(uint i = 0; i < SIZEOFARRAY(predef_fmt_xml); i++) {
					if(BillParam.PredefFormat == predef_fmt_xml[i]) {
...
int PPBillExporter::BillRecToBill(const PPBillPacket * pPack, Sdr_Bill * pBill)
{
	...
	if(pBill && pPack) {
		...
		temp_buf = pPack->GetInvoiceCode(1);
		STRNSCPY(pBill->InvoiceCode, temp_buf.Transf(CTRANSF_INNER_TO_OUTER));
		pBill->InvoiceDate = pPack->GetInvoiceDate(1);
		...
		(temp_buf = pPack->SMemo).Transf(CTRANSF_INNER_TO_OUTER);
...
int DocNalogRu_Generator::MakeOutFileIdent(const PPBillPacket * pBPack, FileInfo & rHi)
{
	...
	*/
	...
	if(/*rHi.EdiProviderSymb.IsEqiAscii("SBIS")*/false) {
		...
		if(rHi.PredefFormat == piefNalogR_Etrn_T1) {
			/* ЭТРН
				ON_TRNACLGROT_2BM-7114814101-2020080300000000000000000000000_2BM-2979065531-2020080300000000000000000000000_2BM-4061639049-2020080300000000000000000000000_0_20260707_4315345f-79fb-4352-9474-6583ca55611b
				Имя файла обмена должно иметь следующий вид: 
				R_Т_A_О_W_GGGGMMDD_N, где:
				R_Т – префикс, принимающий значение ON_DOGVTRGO;
				А – уникальный идентификатор получателя файла обмена электронного договора перевозки груза, информация грузоотправителя. 
					Значение элемента представляется в виде «УИОЭДОУИПол», где:
						«УИОЭДО» – уникальный идентификатор оператора электронного документооборота (оператор ЭДО) – символьный трехзначный код (префикс), 
							присваивается Федеральной налоговой службой. В значении уникального идентификатора допускаются символы латинского алфавита «A – Z», «a – z», цифры «0 – 9», знаки «@», «.», «-». Значение уникального идентификатора регистронезависимо;
						«УИПол» – уникальный идентификатор получателя файла обмена (перевозчика). При направлении файла обмена через 
							оператора ЭДО «УИПол» принимает значение уникального идентификатора участника, присваиваемого оператором ЭДО, 
							длина уникального идентификатора получателя не более 43 символов;
				О – уникальный идентификатор отправителя файла обмена электронного договора перевозки груза, информация грузоотправителя. 
					Значение элемента представляется в виде «УИОЭДОУИОтпр», где:
						«УИОЭДО» – уникальный идентификатор оператора электронного документооборота (оператор ЭДО) – символьный трехзначный код (префикс), 
							присваивается Федеральной налоговой службой. В значении уникального идентификатора допускаются символы латинского алфавита «A – Z», «a – z», цифры «0 – 9», знаки «@», «.», «-». Значение уникального идентификатора регистронезависимо;
						«УИОтпр» – уникальный идентификатор отправителя файла обмена (грузоотправителя). При направлении файла обмена через оператора ЭДО «УИОтпр» 
							принимает значение уникального идентификатора участника, присваиваемого оператором ЭДО, длина уникального идентификатора отправителя не более 43 символов;
				W – признак наличия идентификаторов дополнительных получателей файла обмена информации грузоотправителя. Может принимать следующие значения: 
					«0» – дополнительные получатели файла обмена информации грузоотправителя отсутствуют, 
					«1» – дополнительные получатели файла обмена информации грузоотправителя присутствуют и указаны в файле обмена информации грузоотправителя;
				...
				// my: ON_TRNACLGROT_2BM-1001257748-100101001-201408110617043649981_2BM-100100910817-20121218104345889223900000000_20260903_C6F80FE8-3253-4146-9B5A-211A1A7403BD_0_1_0_0_0_00.xml 
			*/ 
			(rHi.FileId = rHi.FormatPrefix).CatChar('_').Cat(rHi.ReceiverIdent).CatChar('_').Cat(rHi.SenderIdent).CatChar('_').CatChar('0')/*W*/.CatChar('_').
				Cat(temp_buf.Z().Cat(rHi.CurDtm.d, DATF_ANSI|DATF_CENTURY|DATF_NODIV)).CatChar('_').Cat(S_GUID(SCtrGenerate()), S_GUID::fmtIDL);
		}
		else {
			(rHi.FileId = rHi.FormatPrefix).CatChar('_').Cat(rHi.ReceiverIdent).CatChar('_').Cat(rHi.SenderIdent).CatChar('_').
				Cat(temp_buf.Z().Cat(rHi.CurDtm.d, DATF_ANSI|DATF_CENTURY|DATF_NODIV));
			{
				...
				// @v12.4.12 {
				{
					...
					if(pBPack) {
						...
						//
						PPGoodsType2 gt_rec;
						PPObjGoods::ExportDataCodeSet goods_code_set;
						for(uint i = 0; i < pBPack->GetTCount(); i++) {
							...
							if(GObj.Fetch(r_ti.GoodsID, &goods_rec) > 0 && GObj.FetchGoodsType(goods_rec.GoodsTypeID, &gt_rec) > 0) {
								...
								*/
								// @v12.5.5 {
								if(!n_list[1]) {
									if(!no_marks_because_notch) { // @v12.5.10
										if(gt_rec.Flags & GTF_GMARKED && gt_rec.ChZnProdType) {
											GObj.GetExportDataCodeSet(r_ti.GoodsID, goods_code_set);
...
DocNalogRu_Generator::Document::Document(DocNalogRu_Generator & rG, const DocumentInfo & rInfo) : N(rG.P_X, rG.GetToken_Ansi(PPHSC_RU_DOCUMENT))
{
	...
	if(!(rInfo.Flags & DocumentInfo::fIndepFormatProvider)) { // @v11.9.9
		...
		*/
		...
		if(oneof3(rInfo.KND, "1115131", "1115101", "1115133")) {
		...
		if(rInfo.Subj.NotEmpty()) {
			...
			if(rInfo.SubjReason.NotEmpty()) {
				...
				if(rInfo.KND == "1110339") {
					tok_id = PPHSC_RU_REASONECSUBJCOMP2; // ОснДовОргСост
				}
				else {
					tok_id = PPHSC_RU_REASONECSUBJCOMP; // ОснДоверОргСост
				}
...
bool DocNalogRu_Generator::WriteIdentifValPair(const char * pNodeName, const char * pIdent, const char * pVal)
{
	bool   ok = true;
	SXml::WNode n(P_X, pNodeName);
	n.PutAttrib(GetToken_Ansi(PPHSC_RU_IDENTIF), pIdent);
	n.PutAttrib(GetToken_Ansi(PPHSC_RU_VAL), pVal);
	return ok;
}
...
int DocNalogRu_Generator::WriteMarkListOnInvoiceItem3(xmlTextWriter * pX, int contextId, const PPBillImpExpParam & rParam, 
	int chznProdType, int chznIntQtty, const PPLotExtCodeContainer::MarkSet & rSet)
{
	...
	int   tag_token = 0;
	if(contextId == wmlictxUnified) {
		tag_token = 0; // explicit!
		StringSet ss;
		rSet.GetByBoxID(0, ss);
		StringSet ss_etc;
		StringSet ss_pak;
		for(uint ecsp = 0; ss.get(&ecsp, mark_buf);) {
			GtinStruc gts;
			const int pczcr = PPChZnPrcssr::ParseChZnCode(mark_buf, gts, 0);
			if(oneof2(pczcr, SNTOK_SSCC, SNTOK_CHZN_PALLET_MOTOROIL)) {
				ss_pak.add(mark_buf);
			}
			else {
				ss_etc.add(mark_buf);
		...
		if(ss_etc.IsCountGreaterThan(0)) {
			SXml::WNode xn(pX, GetToken_Ansi(PPHSC_RU_WAREIDENTBLOCK));
			for(uint ecsp = 0; ss_etc.get(&ecsp, mark_buf);) {
				...
				if(oneof2(ntok, SNTOK_CHZN_CIGITEM, SNTOK_CHZN_ALTCIGITEM)) {
					...
					if(chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty() && chzn_price_buf.NotEmpty()) {
						...
						xn.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), EncText(temp_buf));
						result = 1;
				...
				if(!is_mark_accepted && chznIntQtty > 0) {
					...
					if(chzn_gtin14_buf.NotEmpty()) {
						...
						xn.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
						result = 1;
						is_mark_accepted = true;
					}
				}
				...
				if(!is_mark_accepted && oneof2(chznProdType, GTCHZNPT_WATER, GTCHZNPT_CHEMISTRY)) { // @v12.7.0 GTCHZNPT_CHEMISTRY
					if(PPChZnPrcssr::InterpretChZnCodeResult(pczcr) > 0) {
						...
						if(temp_buf.NotEmptyS()) {
							if(SsNotch.searchNcAscii("#sbis", 0, 0)) { // @v12.5.2
								xn.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
							}
							else if(rParam.Flags & PPBillImpExpParam::fChZnMarkAsCDATA) {
								...
								xn.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), temp_buf);
							}
							else {
				...
				if(!is_mark_accepted) {
					if(rParam.Flags & PPBillImpExpParam::fChZnMarkAsCDATA) {
						...
						xn.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), mark_buf);
					}
					else {
	...
	else if(contextId == wmlictxInvoice) {
		tag_token = PPHSC_RU_WAREIDENTBLOCK;
	}
	else if(contextId == wmlictxCorrectionBefore) {
		tag_token = PPHSC_RU_WAREIDENTBLOCK_BEFORE;
	}
	else if(contextId == wmlictxCorrectionAfter) {
		tag_token = PPHSC_RU_WAREIDENTBLOCK_AFTER;
	}
	if(tag_token) {
		StringSet ss;
		rSet.GetByBoxID(0, ss);
		p_n_marks_common = (contextId == wmlictxInvoice) ? new SXml::WNode(pX, GetToken_Ansi(PPHSC_RU_WAREIDENTBLOCK)) : 0;
		for(uint ecsp = 0; ss.get(&ecsp, mark_buf);) {
			bool  is_mark_accepted = false;
			GtinStruc gts;
			const int pczcr = PPChZnPrcssr::ParseChZnCode(mark_buf, gts, 0);
			...
			if(oneof2(ntok, SNTOK_CHZN_CIGITEM, SNTOK_CHZN_ALTCIGITEM)) {
				...
				if(chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty() && chzn_price_buf.NotEmpty()) {
					...
					temp_buf.Z().Cat(chzn_gtin14_buf).Cat(chzn_serial_buf);
					if(p_n_marks_common) {
						p_n_marks_common->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), EncText(temp_buf));
					}
					else {
			...
			if(!is_mark_accepted && chznIntQtty > 0) {
				...
				if(chzn_gtin14_buf.NotEmpty()) {
					temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("37").Cat(chznIntQtty);
					if(p_n_marks_common) {
						p_n_marks_common->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
					}
					else {
						SXml::WNode xn(pX, GetToken_Ansi(tag_token));
						xn.PutAttrib(GetToken_Ansi(PPHSC_RU_WAREIDENT_TPACKCODE), EncText(temp_buf));
					}
					result = 1;
					is_mark_accepted = true;
				}
					...
					if(temp_buf.NotEmptyS()) {
						if(p_n_marks_common) {
							if(SsNotch.searchNcAscii("#sbis", 0, 0)) { // @v12.5.2
								...
								// Для CDATA не следует экранировать символы (так же в марке нет русских символов). По-этому мы здесь не применяем EncText
								SXml::WNode::CDATA(temp_buf);
								p_n_marks_common->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), temp_buf);
							}
							else {
								if(temp_buf.HasPrefix("01") && chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty()) {
									temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("21").Cat(chzn_serial_buf);
			...
			if(!is_mark_accepted) {
				if(p_n_marks_common) {
					if(rParam.Flags & PPBillImpExpParam::fChZnMarkAsCDATA) {
						// Для CDATA не следует экранировать символы (так же в марке нет русских символов). По-этому мы здесь не применяем EncText
						SXml::WNode::CDATA(mark_buf);
						p_n_marks_common->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), mark_buf);
					}
					else {
						if(chznProdType == GTCHZNPT_ALTTOBACCO) {
							gts.GetToken(GtinStruc::fldGTIN14, &chzn_gtin14_buf);
...
#if 0 // @v12.6.1 {
void DocNalogRu_Generator::WriteMarkListOnInvoiceItem2(xmlTextWriter * pX, int tagToken, bool tagTokenOnEachItem, const PPBillImpExpParam & rParam, int chznProdType, int chznIntQtty, const PPLotExtCodeContainer::MarkSet & rSet)
{
	SString temp_buf;
	SString mark_buf;
	SString chzn_gtin14_buf;
	...
	StringSet ss;
	rSet.GetByBoxID(0, ss);
	SXml::WNode * p_n_marks_common = tagTokenOnEachItem ? 0 : new SXml::WNode(pX, GetToken_Ansi(tagToken));
	for(uint ecsp = 0; ss.get(&ecsp, mark_buf);) {
		bool  is_mark_accepted = false;
		GtinStruc gts;
		const int pczcr = PPChZnPrcssr::ParseChZnCode(mark_buf, gts, 0);
		...
		if(oneof2(ntok, SNTOK_CHZN_CIGITEM, SNTOK_CHZN_ALTCIGITEM)) {
			...
			if(chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty() && chzn_price_buf.NotEmpty()) {
				...
				// @v12.2.5 {
				// Начиная с 1 февраля 2025 года не надо передавать МРЦ в марке честный знак
				SXml::WNode * p_n_marks_local = tagTokenOnEachItem ? new SXml::WNode(pX, GetToken_Ansi(tagToken)) : 0;
				SXml::WNode * p_n_scope = NZOR(p_n_marks_local, p_n_marks_common);
				assert(p_n_scope);
				if(p_n_scope) {
					temp_buf.Z().Cat(chzn_gtin14_buf).Cat(chzn_serial_buf);
					// МРЦ более не нужен temp_buf.Cat(chzn_price_buf);
					// } @v12.2.5 
					p_n_scope->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), EncText(temp_buf)); // @v11.9.6 @fix temp_buf-->EncText(temp_buf)
					is_mark_accepted = true;
				}
		...
		if(!is_mark_accepted && chznIntQtty > 0) {
			...
			if(chzn_gtin14_buf.NotEmpty()) {
				...
				assert(p_n_scope);
				if(p_n_scope) {
					p_n_scope->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("37").Cat(chznIntQtty)));
					is_mark_accepted = true;
				}
				ZDELETE(p_n_marks_local);
			}
				...
				if(temp_buf.NotEmptyS()) {
					...
					if(p_n_scope) {
						if(SsNotch.searchNcAscii("#sbis", 0, 0)) { // @v12.5.2
							...
							// Для CDATA не следует экранировать символы (так же в марке нет русских символов). По-этому мы здесь не применяем EncText
							SXml::WNode::CDATA(temp_buf);
							p_n_scope->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), temp_buf);
						}
						else {
							if(temp_buf.HasPrefix("01") && chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty()) {
								p_n_scope->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("21").Cat(chzn_serial_buf)));
							}
							else {
								p_n_scope->PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
							}
...
void DocNalogRu_Generator::WriteMarkListOnInvoiceItem(SXml::WNode & rN, const PPBillImpExpParam & rParam, int chznProdType, int chznIntQtty, const PPLotExtCodeContainer::MarkSet & rSet)
{
	...
	for(uint ecsp = 0; ss.get(&ecsp, mark_buf);) {
		...
		if(!is_mark_accepted && chznIntQtty > 0) {
			...
			gts.GetToken(GtinStruc::fldGTIN14, &chzn_gtin14_buf);
			if(chzn_gtin14_buf.NotEmpty()) {
				rN.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("37").Cat(chznIntQtty)));
				is_mark_accepted = true;
			}
		}
		// @v12.3.4 {
			...
			if(PPChZnPrcssr::InterpretChZnCodeResult(pczcr) > 0) {
				...
				if(temp_buf.NotEmptyS()) {
					if(SsNotch.searchNcAscii("#sbis", 0, 0)) { // @v12.5.2
						...
						if(temp_buf.HasPrefix("01") && chzn_gtin14_buf.NotEmpty() && chzn_serial_buf.NotEmpty()) {
							rN.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf.Z().Cat("01").Cat(chzn_gtin14_buf).Cat("21").Cat(chzn_serial_buf)));
						}
						else {
							rN.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
						}
		...
		if(!is_mark_accepted) {
			if(rParam.Flags & PPBillImpExpParam::fChZnMarkAsCDATA) {
				// Для CDATA не следует экранировать символы (так же в марке нет русских символов). По-этому мы здесь не применяем EncText
				SXml::WNode::CDATA(mark_buf);
				rN.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_KIZ), mark_buf);
			}
			else {
				if(chznProdType == GTCHZNPT_ALTTOBACCO) {
					gts.GetToken(GtinStruc::fldGTIN14, &chzn_gtin14_buf);
					...
					rN.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(mark_buf));
			}
		}
	}
}
	...
	const PPObjGoods::ExportDataCodeSet & rGcS, bool correction, const PPBillPacket * pOrgBp) // @v12.2.12
{
	...
	if(itemIdx < rBp.GetTCount()) {
		...
		bool   is_whs_marking_ware = false; // @v12.7.9 проекция флага GTF_GMARKED_WHS товарного типа. Означает, что оптовая отгрузка осуществляется по-марочно.
		...
		}
		{
			...
			n_e.PutAttrib(GetToken_Ansi(PPHSC_RU_WARECODE), rGcS.Code);
			if(!IsVer503()) {
				if(correction) {
			...
			if(!correction) { // @v11.7.1 (Для корректировки не нужно)
				// @v12.5.12 {
				const   long _ware_type = GObj.CheckFlag(goods_id, GF_UNLIM) ? 3L : 1L; // 1 - Товар, 3 - Услуги
				temp_buf.Z().Cat(_ware_type); 
				// } @v12.5.12 
				n_e.PutAttrib(GetToken_Ansi(PPHSC_RU_WARETYPE), temp_buf); // ПрТовРаб
			}
			n_e.PutAttrib(GetToken_Ansi(PPHSC_RU_WAREARTICLE), temp_buf.Z().Cat(goods_id)); // Собственный идентификатор (по специальной просьбе)
			if(!correction) {
				if(!no_marks_because_notch) { // @v12.5.10 @condition
					...
					if(is_there_extcodes) {
						WriteMarkListOnInvoiceItem3(P_X, wmlictxUnified, rParam, chzn_prod_type, chzn_int_qty, ext_codes_set);
					}
					else if(rGcS.CodeForMarking.NotEmpty() && PPChZnPrcssr::IsTypeSuitableForSurrogateMarking(chzn_prod_type)) {
						assert(rGcS.CodeForMarking.Len() < 14);
						...
						if(chzn_int_qty > 0 && chzn_int_qty < 1000) { // @v12.4.8
							...
							(temp_buf = rGcS.CodeForMarking).PadLeft(14-rGcS.CodeForMarking.Len(), '0').Insert(0, "02").Cat("37").Cat(chzn_int_qty);
						...
						else if(!is_whs_marking_ware) { // @v12.7.9 @condition
							(temp_buf = rGcS.CodeForMarking).PadLeft(14-rGcS.CodeForMarking.Len(), '0').Insert(0, "02").Cat("37").Cat(R0i(qtty_local));
							SXml::WNode n_marks(P_X, GetToken_Ansi(PPHSC_RU_WAREIDENTBLOCK));
							n_marks.PutInner(GetToken_Ansi(PPHSC_RU_WAREIDENT_PACKCODE), EncText(temp_buf));
						}
					}
		...
		//
		// @v12.6.2 Новое дело, блядь! Кажется, для корректировочных документов марки должны идти после тега PPHSC_RU_WAREEXTRAINFO
		//
		if(correction) { // @v12.4.8
			...
			if(!no_marks_because_notch) { // @v12.5.10 @condition
				...
				if(is_there_extcodes_before || is_there_extcodes_after) {
					WriteMarkListOnInvoiceItem3(P_X, wmlictxCorrectionBefore, rParam, chzn_prod_type, chzn_int_qty_before, ext_codes_set_before);
					WriteMarkListOnInvoiceItem3(P_X, wmlictxCorrectionAfter, rParam, chzn_prod_type, chzn_int_qty, ext_codes_set_after);
				}
				else if(oneof3(chzn_prod_type, GTCHZNPT_MILK, GTCHZNPT_WATER, GTCHZNPT_SOFTDRINKS) && rGcS.CodeForMarking.NotEmpty()) {
					assert(rGcS.CodeForMarking.Len() < 14);
...
void DocNalogRu_WriteBillBlock::WriteInvoiceItem_Extra2Block(uint trfrItemIdx, bool correction, const SString & rGoodsArCode, const PPObjGoods::ExportDataCodeSet & rGoodsCodeSet)
{
...
int DocNalogRu_WriteBillBlock::WriteInvoiceItems_(bool correction)
{
	...
	const  long exclude_tax_flags = (_Hi.Flags & DocNalogRu_Generator::FileInfo::fVatFree) ? GTAXVF_VAT : 0L;
	SString temp_buf;
	SString goods_ext_strings;
	SString unit_name;
	SString unit_code;
	SString goods_ar_code; // Код товара по статье контрагента
	PPObjGoods::ExportDataCodeSet goods_code_set; //GetGoodsCodeSet 
	...
	if(correction) {
		p_bobj->GetCorrectionBackChain(R_Bp.Rec, correction_bill_chain);
		// @v12.4.8 {
	...
	const int tiamt = (R_Bp.OutAmtType == TIAMT_COST) ? TIAMT_COST : ((R_Bp.OutAmtType == TIAMT_PRICE) ? TIAMT_PRICE : /*TIAMT_AMOUNT*/GTaxVect::GetTaxNominalAmountType(R_Bp.Rec));
	SXml::WNode n_t(G.P_X, GetToken(correction ? PPHSC_RU_CORRINVOICETAB : PPHSC_RU_INVOICETAB));
	for(uint item_idx = 0; item_idx < R_Bp.GetTCount(); item_idx++) {
		double vat_sum = 0.0;
		double excise_sum = 0.0;
		const  PPTransferItem & r_ti = R_Bp.ConstTI(item_idx);
		const  double qtty_local = fabs(r_ti.Qtty());
		const  PPID  goods_id = labs(r_ti.GoodsID);
		int    chzn_prod_type = 0;
		...
		// </СведТов>
		// !!! <ДопСведТов ПрТовРаб="3" КодТов="00000000027" НаимЕдИзм="шт"/>
		G.GObj.GetExportDataCodeSet(goods_id, goods_code_set);
		...
		if(correction) { // Корректирующий документа
			...
			if(G.IsVer503()) {
				...
				G.WriteWareInfoAddendum(R_P, R_Bp, item_idx, goods_code_set, correction, p_org_bpack);
			}
			{
	...
	if(correction) {
		...
		*/
		if(total_amt_after > total_amt_before) {
			SXml::WNode n_t_incr(G.P_X, GetToken(PPHSC_RU_VALUETOTAL_INCR)/*"ВсегоУвел"*/);
			n_t_incr.PutAttrib(GetToken(PPHSC_RU_WAREAMTWOVATTOTAL), temp_buf.Z().Cat(total_amt_wovat_after - total_amt_wovat_before, MKSFMTD_020));
			n_t_incr.PutAttrib(GetToken(PPHSC_RU_WAREAMTTOTAL), temp_buf.Z().Cat(total_amt_after - total_amt_before, MKSFMTD_020));
			{
				SXml::WNode n_tax(G.P_X, GetToken(PPHSC_RU_AMTTAX));
				if(total_vat_after != 0.0 || total_vat_before != 0.0)
					n_tax.PutInnerReal(GetToken(PPHSC_RU_AMTVAT), total_vat_after - total_vat_before, MKSFMTD_020);
				else
					n_tax.PutInner(GetToken(PPHSC_RU_NOVAT_TAG), 0);
			}
		}
		if(total_amt_after < total_amt_before) {
			SXml::WNode n_t_decr(G.P_X, GetToken(PPHSC_RU_VALUETOTAL_DECR)/*"ВсегоУм"*/);
			n_t_decr.PutAttrib(GetToken(PPHSC_RU_WAREAMTWOVATTOTAL), temp_buf.Z().Cat(total_amt_wovat_before - total_amt_wovat_after, MKSFMTD_020));
			n_t_decr.PutAttrib(GetToken(PPHSC_RU_WAREAMTTOTAL), temp_buf.Z().Cat(total_amt_before - total_amt_after, MKSFMTD_020));
			{
				SXml::WNode n_tax(G.P_X, GetToken(PPHSC_RU_AMTTAX));
				if(total_vat_after != 0.0 || total_vat_before != 0.0)
					n_tax.PutInnerReal(GetToken(PPHSC_RU_AMTVAT), total_vat_before - total_vat_after, MKSFMTD_020);
				else
					n_tax.PutInner(GetToken(PPHSC_RU_NOVAT_TAG), 0);
			}
		}
		...
		SXml::WNode n_t(G.P_X, GetToken(PPHSC_RU_TOTALTOPAYM)/*"ВсегоОпл"*/);
		n_t.PutAttrib(GetToken(PPHSC_RU_WAREAMTWOVATTOTAL), temp_buf.Z().Cat(total_amt_wovat, MKSFMTD_020));
		n_t.PutAttrib(GetToken(PPHSC_RU_WAREAMTTOTAL), temp_buf.Z().Cat(total_amt, MKSFMTD_020));
		{
			SXml::WNode n_e(G.P_X, GetToken(PPHSC_RU_AMTTAXTOTAL));
			if(total_vat != 0.0)
...
int DocNalogRu_Generator::WriteAddress(const PPLocationPacket & rP, int regionCode, int hdrTag/*PPHSC_RU_ADDRESS||PPHSC_RU_ORGADDR*/)
{
	...
	PPLocAddrStruc las;
	las.Recognize((temp_buf = addr_text).Transf(CTRANSF_INNER_TO_OUTER));
	const  bool is_addr2 = oneof2(hdrTag, PPHSC_RU_DLVRADDR, PPHSC_RU_LOADINGINFO_SHPADDR); // (АдресПользТип)
	...
	if(is_addr2) {
		if(rP.Regs.GetRegNumber(PPREGT_GLN, ZERODATE, temp_buf) > 0 && temp_buf.IsDec() && temp_buf.Len() == 13) {
			n__.PutInner(GetToken_Ansi(PPHSC_RU_GLN2), temp_buf); //PPHSC_RU_GLN2 "ГЛН"
		}
		if(rP.Latitude != 0.0 && rP.Longitude != 0.0) {
			SXml::WNode n2(P_X, GetToken_Ansi(PPHSC_RU_GPSCOORD)); //PPHSC_RU_GPSCOORD "Коорд"
			n2.PutInnerReal(GetToken_Ansi(PPHSC_RU_GPSCOORD_LAT), rP.Latitude, MKSFMTD(0, 8, 0)); //PPHSC_RU_GPSCOORD_LAT  "Широта"
			n2.PutInnerReal(GetToken_Ansi(PPHSC_RU_GPSCOORD_LON), rP.Longitude, MKSFMTD(0, 8, 0)); //PPHSC_RU_GPSCOORD_LON "Долгота"
		}
	}
	if(hdr_tag == PPHSC_RU_ORGADDR) {
		...
		/*
			PPHSC_RU_ADDR_RF               "АдрРФ"
			PPHSC_RU_ADDR2_RF              "АдресРФ"  // @v12.7.2 (АдресПользТип)
			PPHSC_RU_ADDR_OFFSHR           "АдрИнф"
			PPHSC_RU_ADDR2_OFFSHR          "АдресИнф" // @v12.7.2 (АдресПользТип)
		*/
		PPCountryBlock cb;
...
int DocNalogRu_Generator::GetAgreementParams(/*PPID arID*/const PPBillPacket & rBillPack, SString & rAgtCode, LDATE & rAgtDate, LDATE & rAgtExpiry)
{
	...
	if(rBillPack.Rec.AgtBillID && p_bobj->Search(rBillPack.Rec.AgtBillID, &agt_bill_rec) > 0) {
...
/*static*/void DocNalogRu_WriteBillBlock::GetNotchList(const PPBillPacket & rBp, StringSet & rSs)
{
	rSs.Z();
	const  PPID contragent_id = ObjectToPerson(rBp.Rec.Object, 0);
	if(contragent_id) {
		Reference * p_ref = PPRef;
		SString temp_buf;
		SString notch_buf;
		StringSet ss;
		if(p_ref->Ot.GetTagStr(PPOBJ_PERSON, contragent_id, PPTAG_PERSON_NOTCH, temp_buf) > 0) {
			for(const char * p = temp_buf.cptr(); *p; p++) {
				if(!oneof4(*p, ' ', '\t', ',', ';')) {
					notch_buf.CatChar(*p);
				}
				else if(notch_buf.NotEmpty()) {
					ss.add(notch_buf);
					notch_buf.Z();
				}
			}
			if(notch_buf.NotEmpty()) {
				ss.add(notch_buf);
				notch_buf.Z();
			}
			//
			for(uint ssp = 0; ss.get(&ssp, temp_buf);) {
				if(!rSs.searchNcAscii(temp_buf, 0, 0)) {
					rSs.add(temp_buf);
				}
			}
		}
...
{
	// @v12.7.7 (EpiBlk) MainOrgID = GetMainOrgID();
	// @v12.7.7 (EpiBlk) ContragentID = ObjectToPerson(R_Bp.Rec.Object, 0);
	PPID   receiver_psn_id = 0; // @v12.7.7 Персоналия-получатель документа
	BillObj->MakeExportParticipantIdentBlock(R_Bp, EpiBlk); // @v12.7.7
	...
	_Hi.PredefFormat = rParam.PredefFormat; // @v12.7.6
	{
		...
		// @v12.3.9 {
		if(rParam.PredefFormat == piefNalogR_Etrn_T1) {
			output_format_ver.Set(5, 1, 0);
			receiver_psn_id = EpiBlk.TransporterPsnID;
		}
		else {
			receiver_psn_id = EpiBlk.ContactorPsnID;
	...
	DocNalogRu_WriteBillBlock::GetNotchList(R_Bp, G.SsNotch); // @v12.4.11 // @v12.5.10 dedicated-func
	THROW(G.CreateHeaderInfo(HeaderSymb, EpiBlk.MainOrgID, receiver_psn_id, DtoPersonID, &rBp, R_NominalFileName, _Hi));
		...
		SString temp_buf;
        {
			...
			d.N.PutAttrib(GetToken(PPHSC_RU_EXECUTORINFOTIME), temp_buf.Z().Cat(_Hi.CurDtm.t, TIMF_HMS|TIMF_DOTDIV));
			{
				...
				SXml::WNode n_(G.P_X, "СвДокПРУ");
				{
					...
					n_2.PutAttrib(GetToken(PPHSC_RU_NAMEOFDOC2), GetToken(PPHSC_RU_NAMEOFDOC_S_SRVC));
					...
					// SXml::WNode n_3(g.P_X, "ДенИзм");
					{
						SXml::WNode n_4(G.P_X, GetToken(PPHSC_RU_TRANSACTIONCONTENT1));
						{
                            ...
                            G.WriteParticipant(GetToken(PPHSC_RU_EXECUTOR), EpiBlk.MainOrgID, 0);
                            G.WriteParticipant(GetToken(PPHSC_RU_CUSTOMER),  EpiBlk.ContactorPsnID, 0);
							...
							n_48.PutAttribSkipEmpty(GetToken(PPHSC_RU_FILEOFINFFLDUUID)/*"ИдФайлИнфПол"*/, ""); // @optional
							{
								...
								if(State & stAgreementParamIsValid) {
									{
										G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_CONTRACT), EncText(AgtCode));
									}
									if(checkdate(AgtDate)) {
										G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_CONTRACTDATE), temp_buf.Z().Cat(AgtDate, DATF_GERMANCENT));
									}
									if(checkdate(AgtExpiry)) {
										G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_PERIOD), temp_buf.Z().Cat(AgtExpiry, DATF_GERMANCENT));
									}
								}
								else {
...
int DocNalogRu_WriteBillBlock::Do_Invoice2(SString & rResultFileName)
{
	...
	rResultFileName.Z();
	{
        ...
        {
			BillTbl::Rec order_bill_rec;
			GetOrderRec(order_bill_rec);
			//
			DocNalogRu_Generator::File f(G, _Hi);
			PPPersonPacket psn_pack;
			temp_buf.Z();
			if(G.PsnObj.GetPacket(EpiBlk.MainOrgID, &psn_pack, 0) > 0) {
				if(!psn_pack.GetExtName(temp_buf))
					temp_buf = psn_pack.Rec.Name;
			}
			DocNalogRu_Base::DocumentInfo docinfo;
			...
			DocNalogRu_Generator::Document d(G, docinfo);
			{
				...
				// @v12.7.7 PPObjBill::ExportParticipantIdentBlock epi_blk;
				// @v12.7.7 BillObj->MakeExportParticipantIdentBlock(R_Bp, epi_blk); // @v12.6.9
				// @v11.7.12 {
				...
				// @v11.9.0 {
				{
					if(EpiBlk.ConsigneeLocID && G.PsnObj.LocObj.GetRegister(EpiBlk.ConsigneeLocID, PPREGT_GLN, ZERODATE, true, &reg_rec) > 0)
						consignee_gln = reg_rec.Num;
					if(consignee_gln.IsEmpty() && EpiBlk.ConsigneePsnID)
						G.PsnObj.GetRegNumber(EpiBlk.ConsigneePsnID, PPREGT_GLN, consignee_gln);
				}
				// } @v11.9.0 
				G.WriteOrgInfo(PPHSC_RU_SELLERINFO, EpiBlk.ConsignorPsnID, /*EpiBlk.ConsignorLocID*/0, R_Bp.Rec.Dt, 0);
				if(!are_all_goods_unlim) { // @v11.7.12
					{
						SXml::WNode n_1(G.P_X, GetToken(PPHSC_RU_CONSIGNORINFO));
						G.WriteOrgInfo(PPHSC_RU_CONSIGNORINFO2, EpiBlk.ConsignorPsnID, EpiBlk.ConsignorLocID, R_Bp.Rec.Dt, 0);
					}
					G.WriteOrgInfo(PPHSC_RU_CONSIGNEEINFO, EpiBlk.BuyerPsnID, EpiBlk.ConsigneeLocID, R_Bp.Rec.Dt, /*DocNalogRu_Generator::woifAddrLoc_KppOnly*/0);
				}
				if(G.IsVer503()) {
				...
				}
				{
					...
					PPIDArray rel_list;
					if(EpiBlk.BuyerPsnID && G.PsnObj.GetRelPersonList(EpiBlk.BuyerPsnID, PPPSNRELTYP_AFFIL, 0, &rel_list) > 0)
						_buyer_person_id = rel_list.at(0); // @todo Здесь, наверное, какое-то сообщение в лог нужно или еще что? (неоднозначность)
					else
						_buyer_person_id = EpiBlk.BuyerPsnID;
					G.WriteOrgInfo(PPHSC_RU_BUYERINFO, _buyer_person_id, /*EpiBlk.ConsigneeLocID*/0, R_Bp.Rec.Dt, /*DocNalogRu_Generator::woifAddrLoc_KppOnly*/0);
				}
				if(!G.IsVer503()) {
					G.WriteDocRequisites(R_Bp);
				}
				...
				{
					{
						...
						G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_WAYBILLCODE), EncText(temp_buf = R_Bp.Rec.Code));
						if(consignee_gln.NotEmpty()) {
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_GLN_CONSIGNEE), EncText(consignee_gln));
						}
						if(AgtCode.NotEmpty()) {
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_CONTRACT), EncText(AgtCode));
							if(checkdate(AgtDate)) {
							...
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), "none", "none");
						}
						{
							...
							if(p_tag && p_tag->GetInt(&disposal_reason) > 0 && (disposal_reason >= 1 && disposal_reason <= 4)) {
								G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_CHZNDISPOSALREASON), temp_buf.Z().Cat(disposal_reason));
							}
						}
						// @v12.4.4 {
						{
							S_GUID uuid;
							{
								if(EpiBlk.ConsignorLocID && p_ref->Ot.GetTagGuid(PPOBJ_LOCATION, EpiBlk.ConsignorLocID, PPTAG_LOC_FIASGUID_ADR, uuid) > 0) {
									assert(uuid);
									G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_SUPPLIER_FIASID), uuid.ToStr(S_GUID::fmtIDL|S_GUID::fmtLower, temp_buf));
								}
							}
							{
								const PPID buyer_loc_id = EpiBlk.ConsigneeLocID;
								if(buyer_loc_id && p_ref->Ot.GetTagGuid(PPOBJ_LOCATION, buyer_loc_id, PPTAG_LOC_FIASGUID_ADR, uuid) > 0) {
									assert(uuid);
									G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_BUYER_FIASID), uuid.ToStr(S_GUID::fmtIDL|S_GUID::fmtLower, temp_buf));
								}
							}
							{
								PPFreight freight;
								...
								if(R_Bp.GetFreight(&freight) > 0) {
									if(freight.Name[0]) {
										G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_FREIGHTCODE), EncText(temp_buf = freight.Name));
									}
									{
										...
										if(checkdate(freight_date)) { 
											G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_FREIGHTDATE), temp_buf.Z().Cat(freight_date, DATF_GERMANCENT));
										}
									}
									if(freight.ShipID && tr_obj.Get(freight.ShipID, &tr_pack) > 0 && tr_pack.Rec.TrType == PPTRTYP_CAR && tr_pack.Rec.Code[0]) {
										G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_LICENSEPLATE), EncText(temp_buf = tr_pack.Rec.Code));
									}
								}
							}
							// @v12.4.11 {
							if(G.SsNotch.searchNcAscii("#esphere", 0, 0)) {
								if(order_bill_rec.ID) {
									G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERCODE3), EncText(temp_buf = order_bill_rec.Code));
									G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERDATE3), temp_buf.Z().Cat(order_bill_rec.Dt, DATF_GERMANCENT));
								}
							}
							// } @v12.4.11 
						}
...
int DocNalogRu_WriteBillBlock::Do_CorrInvoice(SString & rResultFileName)
{
	...
	if(!is_exp_correction)
		...
		THROW(IsValid());
		{
			...
			*/
			docinfo.Function = "КСЧФДИС"; // @v11.7.1 // @v12.4.11 "КСЧФ"-->"КСЧФДИС"
			(docinfo.NameOfDoc = G.GetToken_Utf8(PPHSC_RU_NAMEOFDOC_INVCCORR)).Transf(CTRANSF_UTF8_TO_INNER); // @v12.4.11
			(docinfo.NameOfDoc2 = G.GetToken_Utf8(PPHSC_RU_NAMEOFDOC2_INVCCORR)).Transf(CTRANSF_UTF8_TO_INNER); // @v12.4.11
			DocNalogRu_Generator::Document d(G, docinfo);
			{
				DocNalogRu_Generator::Invoice inv(G, R_Bp, true/*correction*/);
				// @v12.7.7 PPObjBill::ExportParticipantIdentBlock epi_blk;
				// @v12.7.7 BillObj->MakeExportParticipantIdentBlock(R_Bp, epi_blk); // @v12.6.9
				G.WriteOrgInfo(PPHSC_RU_SELLERINFO, EpiBlk.SupplPsnID, /*EpiBlk.ConsignorLocID*/0, R_Bp.Rec.Dt, 0);
				{
					PPID   _buyer_person_id = 0;
					PPIDArray rel_list;
					if(EpiBlk.BuyerPsnID && G.PsnObj.GetRelPersonList(EpiBlk.BuyerPsnID, PPPSNRELTYP_AFFIL, 0, &rel_list) > 0) {
			...
			WriteInvoiceItems_(true/*correction*/);
			G.Underwriter(0);
		}
		G.EndDocument();
		rResultFileName = _Hi.FileName;
...
int DocNalogRu_WriteBillBlock::Do_Etrn_T1(SString & rResultFileName, StringSet & rSsDaignose) // @v12.6.9
{
	...
	// @v12.7.6 {
	{
		bool   fault_on_data = false; // Признак того, что есть неостатки в исходных данных (сообшения заносятся в rSsDaignose)
		...
		// @v12.7.7 p_bobj->MakeExportParticipantIdentBlock(R_Bp, epi_blk);
		if(!EpiBlk.ConsignorPsnID) {
			fault_on_data = true;
			PPGetMessage(mfError, PPERR_ETRNEXP_CONSIGNORNEEDED, bill_text, 1, temp_buf);
			if(temp_buf.NotEmpty())
				rSsDaignose.add(temp_buf);
		}
		if(!EpiBlk.ConsigneePsnID) {
			fault_on_data = true;
			PPGetMessage(mfError, PPERR_ETRNEXP_CONSIGNEENEEDED, bill_text, 1, temp_buf);
			if(temp_buf.NotEmpty())
				rSsDaignose.add(temp_buf);
		}
		if(is_there_freight) { // Нет смысла выводить сообщения об отсутсвии транспортера и водителя если нет фрахта
			if(!EpiBlk.TransporterPsnID) {
				fault_on_data = true;
		...
		}
        {
			...
			DocNalogRu_Generator::Document d(G, docinfo);
			{
				...
				SXml::WNode n(G.P_X, G.GetToken_Ansi(PPHSC_RU_TRANSACTIONCONTENT_TRNT1));
				{
					...
					//PPHSC_RU_TRN_N                 "НомерТрН" // @v12.7.0
					//PPHSC_RU_CONTOFOP СодОпер
					
					// PPHSC_RU_TRN_ORD_N             "НомЗак"   // Порядковый номер заказа (заявки)
					// PPHSC_RU_TRN_ORD_D             "ДатаЗак"  // Дата заказа (заявки)
				...
				//PPHSC_RU_CONSIGNORINFO3        "СвГО"     // @v12.6.9
				//PPHSC_RU_CONSIGNEEINFO3        "СвГП"     // @v12.6.9
				//PPHSC_RU_TRANSPORTERINFO       "СвПер"    // @v12.6.9
				//PPHSC_RU_DRIVERINFO            "СвВодит"  // @v12.6.9
				//PPHSC_RU_TRANSPORTINFO         "СвТС"     // @v12.6.9
				//PPHSC_RU_LOADINGINFO           "СвПогруз" // @v12.6.9
				//PPHSC_RU_PAYLOADINFO           "СвГруз"   // @v12.6.9 
				//PPHSC_RU_CONSIGNORDIRECTION    "УказГО"   // @v12.6.9 
				{
					SXml::WNode n2(G.P_X, G.GetToken_Ansi(PPHSC_RU_CONSIGNORINFO3));
					//PPHSC_RU_ISFORWARDER           "ГОЭксп"     // @v12.7.6 Статус грузоотправителя (грузоотправитель является / не является экспедитором)
					n2.PutAttrib(G.GetToken_Ansi(PPHSC_RU_ISFORWARDER), "0"); // "0" || "1" // @v12.7.6
					G.WriteOrgInfo(PPHSC_RU_CONSIGNORREQ, EpiBlk.ConsignorPsnID, EpiBlk.ConsignorLocID, R_Bp.Rec.Dt, DocNalogRu_Generator::woifForcePhone);
				}
				{
					SXml::WNode n2(G.P_X, G.GetToken_Ansi(PPHSC_RU_CONSIGNEEINFO3));
					{
						PPLocationPacket dlvr_loc_pack;
						G.WriteOrgInfo(PPHSC_RU_CONSIGNEEREQ, EpiBlk.ConsigneePsnID, /*EpiBlk.ConsigneeLocID*/0, R_Bp.Rec.Dt, DocNalogRu_Generator::woifForcePhone);
				...
				if(R_Bp.GetTCount()) {
					...
					for(uint ti_idx = 0; ti_idx < R_Bp.GetTCount(); ti_idx++) {
						...
						// ВидТар UED_META_ECETRADE_PACKAGETYPE

						// PPHSC_RU_PAYLOAD_DESCR         "ОпГруз"     // @v12.6.9 
						// PPHSC_RU_PAYLOAD_APPEL         "НаимГруз"   // @v12.6.9 
						// PPHSC_RU_PAYLOAD_COND          "СостГруз"   // @v12.6.9 
						// PPHSC_RU_PAYLOAD_COND_EXCEL    "Отличное"   // @v12.6.9 
						// PPHSC_RU_PAYLOAD_PCKG          "СпУпак"     // @v12.6.9  
						// PPHSC_RU_PAYLOAD_TARE          "ВидТар"     // @v12.6.9 
						// PPHSC_RU_PAYLOAD_PCKGQTY       "КолМестГр"  // @v12.6.9 
						// PPHSC_RU_PAYLOAD_STREGKIND     "УчГосСист"  // @v12.6.9 
						if(G.GObj.Fetch(r_ti.GoodsID, &goods_rec) > 0 && !(goods_rec.Flags & GF_UNLIM)) {
							...
							G.GObj.GetExportDataCodeSet(r_ti.GoodsID, goods_code_set);
							if(G.GObj.FetchUnit(goods_rec.UnitID, &u_rec) > 0) {
								is_weighted_ware = (u_rec.ID == SUOM_KILOGRAM || u_rec.BaseUnitID == SUOM_KILOGRAM);
								//(unit_name = u_rec.Name).Transf(CTRANSF_INNER_TO_OUTER);
							}
									...
									if(is_there_extcodes) {
										...
										if(oneof5(chzn_prod_type, GTCHZNPT_MILK, GTCHZNPT_WATER, GTCHZNPT_SOFTDRINKS, GTCHZNPT_VEGETABLEOIL, GTCHZNPT_PETFOOD) && goods_code_set.CodeForMarking.NotEmpty()) {
											...
											if(chzn_int_qty > 0 && chzn_int_qty < 1000) {
												assert(chzn_prod_type == GTCHZNPT_MILK && is_weighted_ware); // подразумевается условием (chzn_int_qty > 0 && chzn_int_qty < 1000)
												// Для весовой молочной продукции указывается количество упаковок - пока пишем фиксированную единицу
							...
							{
								//PPHSC_RU_PAYLOAD_ESTMASS       "ПлМасГруз"  // @v12.6.9
									// PPHSC_RU_PAYLOAD_BRUTTO        "МасБрутЗнач" // @v12.6.9 Масса брутто груза в килограммах
				...
				}
				{
					...
					n2.PutAttrib(G.GetToken_Ansi(PPHSC_RU_CONSIGNORDIRECTION_NORM), G.GetToken_Ansi(PPHSC_RU_ABSENCE)); // @v12.7.7
					{
						...
						{
							//EpiBlk.ConsignorPsnID, EpiBlk.ConsignorLocID	
							if(EpiBlk.ConsignorPsnID) {
								PPPersonPacket local_psn_pack;
								//PPLocationPacket local_loc_pack;
								if(G.PsnObj.GetPacket(EpiBlk.ConsignorPsnID, &local_psn_pack, 0) > 0) {
									// Контакт
									StringSet ss_phones;
				...
				}
				{
					...
					G.WriteOrgInfo(PPHSC_RU_TRANSPORTERINFO, EpiBlk.TransporterPsnID, 0, R_Bp.Rec.Dt, DocNalogRu_Generator::woifForcePhone);
				}
				{
					SXml::WNode n2(G.P_X, G.GetToken_Ansi(PPHSC_RU_DRIVERINFO));
					//<СвВодит НомВУ="1234567890" СерВУ="1234" ДатаВыдВУ="01.01.2015">
					if(G.PsnObj.GetPacket(EpiBlk.CaptainID, &psn_pack, 0) > 0) {
						uint   reg_pos = 0;
						RegisterTbl::Rec reg_rec;
				...
				}
				{
					...
					/*
						...
						PPHSC_RU_EXTRA0                "ИнфПол"     // @v12.6.9
				...
				}
				{
					...
					{
						LDATE   loading_date = ZERODATE;
						if(checkdate(freight.IssueDate)) { // @v12.7.7 @fix freight.ArrivalDate-->freight.IssueDate
							loading_date = freight.IssueDate;
						}
						else {
							loading_date = R_Bp.Rec.Dt;
						}
						assert(checkdate(loading_date));
						ut_surrogate_loading.SetDate(loading_date);
						if(loading_date == now_dtm.d) {
							ut_surrogate_loading.SetTime(encodetime(now_dtm.t.hour(), now_dtm.t.minut(), 0, 0));
						}
						else {
					...
					}
					{
						...
						if(EpiBlk.ConsignorPsnID) {
							G.PsnObj.GetRegNumber(EpiBlk.ConsignorPsnID, PPREGT_TPID, R_Bp.Rec.Dt, temp_buf);
							if(temp_buf.IsDec()) {
								consignor_inn = temp_buf;
							}
						}
						{
							SXml::WNode n3(G.P_X, G.GetToken_Ansi(PPHSC_RU_LOADINGINFO_LOADER)); // "СвЛицПогрГр"
							n3.PutAttrib(G.GetToken_Ansi(PPHSC_RU_LOADINGINFO_CSNERISLDR), "1"); // Признак совпадения с грузоотправителем: лицо, которое осуществляет погрузку груза в транспортное средство, является грузоотправителем
							if(consignor_inn.NotEmpty()) {
								SXml::WNode n4(G.P_X, G.GetToken_Ansi(PPHSC_RU_LOADINGINFO_LOADERID)); // "ИдентРекГО"   // Идентифицирующий реквизит грузоотправителя
					...
					/*
						PPHSC_RU_LOADINGINFO           "СвПогруз"   // @v12.6.9
						PPHSC_RU_LOADINGINFO_SHPTIME   "ЗаявПогр"   // Заявленные дата и время подачи транспортного средства под погрузку
						PPHSC_RU_LOADINGINFO_ARRVTIME  "ФДатВрПриб"        // Фактические дата и время прибытия под погрузку
						PPHSC_RU_LOADINGINFO_DPRTTIME  "ФДатВрУбыт"        // Фактические дата и время убытия
						PPHSC_RU_LOADINGINFO_SHPUTCTAG "НалКоорТочВрЗаяв" // Применение координации точного времени (UTC) в типовом элементе ДатаВремяВЗТип
						PPHSC_RU_LOADINGINFO_ARRVUTCTAG "НалКоорТочВрФПогр" // Применение координации точного времени (UTC) в типовом элементе ДатаВремяВЗТип
						PPHSC_RU_LOADINGINFO_DPRTUTCTAG "НалКоорТочВрФУбыт" // Применение координации точного времени (UTC) в типовом элементе ДатаВремяВЗТип
						PPHSC_RU_LOADINGINFO_TARESTATE  "ФСостТар" // Фактическое состояние тары при приеме груза перевозчиком от лица, осуществляющего погрузку груза в транспортное средство
						PPHSC_RU_LOADINGINFO_PACKSTATE  "ФСостУп"  // Фактическое состояние упаковки при приеме груза перевозчиком от лица, осуществляющего погрузку груза в транспортное средство
						PPHSC_RU_LOADINGINFO_TRKBDYSTATE "ФСостКуз" // Фактическое состояние кузова при приеме груза перевозчиком от лица, осуществляющего погрузку груза в транспортное средство
						PPHSC_RU_LOADINGINFO_BRUTTO     "МасБрутОтгр" // Масса брутто груза
						PPHSC_RU_LOADINGINFO_BRUTTOMETH "МетОпрМасс"  // Метод определения массы груза
						PPHSC_RU_LOADINGINFO_PACKCOUNT  "КолМестПрием" // Количество грузовых мест при приеме груза перевозчиком
						PPHSC_RU_LOADINGINFO_SHPADDR    "ФАдресПогр"   // Адрес места погрузки
						PPHSC_RU_LOADINGINFO_LOADER     "СвЛицПогрГр"  // Сведения о лице, производящем погрузку груза
							PPHSC_RU_LOADINGINFO_CSNERISLDR "СовпГОП"      // Признак совпадения с грузоотправителем: лицо, которое осуществляет погрузку груза в транспортное средство, является грузоотправителем 
						PPHSC_RU_LOADINGINFO_PORTOWNER  "ВладИнфр"     // Сведения о владельце объекта инфраструктуры пункта погрузки
						PPHSC_RU_LOADINGINFO_ACCSDOC    "ОснДостОбИнфрДок" // Doc - Основание беспрепятственного доступа к объекту инфраструктуры пункта погрузки лица, осуществляющего погрузку груза в транспортное средство 
						PPHSC_RU_LOADINGINFO_ACCSTEXT   "ОснДостОбИнфрТекст" // Text - Основание беспрепятственного доступа к объекту инфраструктуры пункта погрузки лица, осуществляющего погрузку груза в транспортное средство
					*/ 
				}
				{ // @v12.7.7
					if(G.SsNotch.searchNcAscii("#ptz_borodinskiy", 0, 0)) {
						/* бородинский:
						ORDERS Желательно / номер заказа покупателя (формируется на стороне партнера);
						ORDERS_DATE (ДД.ММ.ГГГГ) Желательно / дата заказа покупателя //
						DESADV Желательно / номер EDI-сообщения об отгрузке
						DESADV_DATE (ДД.ММ.ГГГГ) Желательно / дата EDI-сообщения об отгрузке
						INVOIC Обязательно / номер счета-фактуры или УПД
						INVOIC_DATE (ДД.ММ.ГГГГ) Обязательно / дата счета-фактуры или УПД (формат ДД.ММ.ГГГГ)
						*/ 
						SXml::WNode n2(G.P_X, G.GetToken_Ansi(PPHSC_RU_EXTRA0)); // ИнфПол
						BillTbl::Rec ord_rec;
...
int DocNalogRu_WriteBillBlock::GetOrderRec(BillTbl::Rec & rOrderBillRec)
{
	rOrderBillRec.ID = 0;
	int    ok = -1;
	...
	if(order_id_list.getCount()) {
		...
		if(BillObj->Search(order_bill_id, &rOrderBillRec) > 0) {
			ok = 1;
		}
		else
			rOrderBillRec.ID = 0;
	...
	rResultFileName.Z();
	{
		...
		GetOrderRec(order_bill_rec);
        {
			...
			//d.N.PutAttrib(g.GetToken_Ansi(PPHSC_RU_SELLERINFOTIME), temp_buf.Z().Cat(_hi.CurDtm.t, TIMF_HMS|TIMF_DOTDIV));
			{
				DocNalogRu_Generator::Invoice inv(G, R_Bp);
				RegisterTbl::Rec reg_rec;
				SString consignor_gln;
				SString consignee_gln;
				// @v12.7.7 PPObjBill::ExportParticipantIdentBlock epi_blk;
				// @v12.7.7 BillObj->MakeExportParticipantIdentBlock(R_Bp, epi_blk); // @v12.6.9
				// @v11.7.9 {
				if(EpiBlk.ConsigneeLocID && G.PsnObj.LocObj.GetRegister(EpiBlk.ConsigneeLocID, PPREGT_GLN, ZERODATE, true, &reg_rec) > 0)
				...
				// @v11.7.9 {
				if(EpiBlk.ConsignorLocID && G.PsnObj.LocObj.GetRegister(EpiBlk.ConsignorLocID, PPREGT_GLN, ZERODATE, true, &reg_rec) > 0)
					consignor_gln = reg_rec.Num;
				// } @v11.7.9 
				if(consignor_gln.IsEmpty() && EpiBlk.ConsignorPsnID) {
					G.PsnObj.GetRegNumber(EpiBlk.ConsignorPsnID, PPREGT_GLN, consignor_gln);
				}
				G.WriteOrgInfo(PPHSC_RU_SELLERINFO, EpiBlk.ConsignorPsnID, /*EpiBlk.ConsignorLocID*/0, R_Bp.Rec.Dt, 0);
				G.WriteDocRequisites(R_Bp);
				{
				...
				}
				{
					SXml::WNode n(G.P_X, GetToken(PPHSC_RU_EXTRA1)); // Do_UPD
					{
						if(R_P.EdiProviderSymb.IsEqiAscii("SBIS")) {
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_GENDATE), temp_buf.Z().Cat(getcurdatetime_(), DATF_GERMANCENT, TIMF_HMS));
							if(order_bill_rec.ID) {
								G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERCODE), EncText(temp_buf = order_bill_rec.Code));
								G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERDATE), temp_buf.Z().Cat(order_bill_rec.Dt, DATF_GERMANCENT));
							}
						}
						// @v12.4.11 {
						else if(G.SsNotch.searchNcAscii("#esphere", 0, 0)) {
							if(order_bill_rec.ID) {
								G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERCODE3), EncText(temp_buf = order_bill_rec.Code));
								G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(PPHSC_RU_EXTRA_ORDERDATE3), temp_buf.Z().Cat(order_bill_rec.Dt, DATF_GERMANCENT));
							}
					...
					}
					{
						...
						G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(ident_tok_id), EncText(temp_buf = R_Bp.Rec.Code));
					}
					// @v11.9.12 {
					{
						const long ident_tok_id = (R_P.EdiProviderSymb.IsEqiAscii("SBIS") ? PPHSC_RU_EXTRA_WAYBILLDATE2 : PPHSC_RU_EXTRA_WAYBILLDATE);
						G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(ident_tok_id), EncText(temp_buf.Z().Cat(R_Bp.Rec.Dt, DATF_GERMANCENT)));
					}
					// } @v11.9.12 
					if(consignee_gln.NotEmpty()) {
						{ // @fixme Здесь должен быть GLN покупателя, а не грузополучателя, но пусть пока так будет
							...
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(ident_tok_id), EncText(consignee_gln));
						}
						{
					...
					if(consignor_gln.NotEmpty()) {
						{ // @fixme Вообще-то здесь должен быть GLN поставщика, а не склада-отправителя, но пока так пусть будет
							...
							G.WriteIdentifValPair(GetToken(PPHSC_RU_TEXTINF), GetToken(ident_tok_id), EncText(consignor_gln));
						}
						{
					...
					// @v12.4.4 {
					{
						S_GUID uuid;
						{
							const PPID seller_loc_id = EpiBlk.ConsignorLocID;
