...
PrcTechCtrlGroup::Rec::Rec() : PrcID(0), PrcParentID(0), TechID(0), ArID(0), Ar2ID(0), GoodsID(0), GStrucID(0), IdleStatus(false)
{
}

PrcTechCtrlGroup::PrcTechCtrlGroup(uint ctlSelPrc, uint ctlSelTech, uint ctlStGoods,
	uint ctlSelAr, uint ctlSelAr2, uint cmdSelTechByGoods, uint cmdCreateGoods) : CtrlGroup(), CtlselPrc(ctlSelPrc), CtlselTech(ctlSelTech), CtlselGStruc(0),
...
void PrcTechCtrlGroup::SetGoodsStrucCtlsel(uint ctlSelGoodsStruc)
{
	CtlselGStruc = ctlSelGoodsStruc;
}
...
void PrcTechCtrlGroup::setupArticle(TDialog * pDlg, const ProcessorTbl::Rec * pPrcRec)
{
	...
	if(!was_idle_sheet_set) {
		...
		if(!acs_id) {
			...
			if(PrcObj.GetRecWithInheritance(Data.PrcID, &prc_rec, 1) > 0 && prc_rec.Flags & PRCF_ADDEDOBJASAGENT)
...
void PrcTechCtrlGroup::onPrcSelection(TDialog * pDlg, int onIdleStatus)
{
	...
	if(PrcObj.GetRecWithInheritance(Data.PrcID, &prev_prc_rec, 1) <= 0) {
		prev_prc_rec.Clear();
	}
	if(onIdleStatus) {
...
int PrcTechCtrlGroup::GetGoodsID(TDialog * pDlg, PPID * pGoodsID)
{
	...
	if(TecObj.Fetch(Data.TechID, &tec_rec) > 0) { // @v12.6.7 Search-->Fetch
...
void PrcTechCtrlGroup::SetupGoods_(TDialog * pDlg, PPID * pGoodsID)
{
	SString goods_name;
	// @v12.6.1 {
	PPID   goods_id = 0;
...
void PrcTechCtrlGroup::SetupTech(TDialog * pDlg, bool onInit)
{
	PPID   goods_id = 0;
	//pDlg->getCtrlData(CtlselPrc, &Data.TechID);
	SetupGoods_(pDlg, &goods_id);
	bool   enable_select_gstruc = false;
	if(CtlselGStruc && goods_id) {
		PPID   final_gstruc_id = 0;
		PPID   tec_gstruc_id = 0;
		const  PPID ex_gstruc_id = onInit ? Data.GStrucID : pDlg->getCtrlLong(CtlselGStruc);
		TechTbl::Rec tec_rec;
		if(TecObj.Fetch(Data.TechID, &tec_rec) > 0) {
			tec_gstruc_id = tec_rec.GStrucID;
			if(!tec_gstruc_id)
				enable_select_gstruc = true;
		}
		else {
			enable_select_gstruc = true;
		}
		if(ex_gstruc_id) {
			PPGoodsStruc::Ident gsi;
			gsi.GoodsID = goods_id;
			gsi.GStrucID = ex_gstruc_id;
			if(GObj.LoadGoodsStruc(gsi, 0) > 0) {
				final_gstruc_id = ex_gstruc_id;
			}
		}
		SETIFZ(final_gstruc_id, tec_gstruc_id);
		SetupPPObjCombo(pDlg, CtlselGStruc, PPOBJ_GOODSSTRUC, final_gstruc_id, 0, reinterpret_cast<void *>(NZOR(goods_id, -1)));
		pDlg->disableCtrl(CtlselGStruc, !enable_select_gstruc);
	}
}
...
int PrcTechCtrlGroup::setData(TDialog * pDlg, void * pData)
{
	...
	int    r = PrcObj.GetRecWithInheritance(Data.PrcID, &prc_rec);
	if(r > 0) {
		SetupPPObjCombo(pDlg, CtlselTech, PPOBJ_TECH, Data.TechID, GetTechComboOlwFlags()|OLW_SETUPSINGLE, reinterpret_cast<void *>(Data.PrcID));
		// @v12.6.7 SetupGoods_(pDlg, 0);
		SetupTech(pDlg, true/*onInit*/); // @v12.6.7
		setupArticle(pDlg, &prc_rec);
	}
	else {
		if(!r)
...
int PrcTechCtrlGroup::getData(TDialog * pDlg, void * pData)
{
	...
	// @v12.6.7 {
	if(TView::IsSubSign(pDlg->getCtrlView(CtlselGStruc), TV_SUBSIGN_COMBOBOX)) {
		PPID   gstruc_id = pDlg->getCtrlLong(CtlselGStruc);
		if(gstruc_id) {
			TechTbl::Rec tec_rec;
			if(Data.TechID && TecObj.Fetch(Data.TechID, &tec_rec) > 0 && tec_rec.GStrucID) {
				gstruc_id = 0;
			}
			else {
				PPID   goods_id = 0;
...
// ReceiptCore::TecRouteAssignment
class TecRouteAssignmentDialog : public TDialog { // @v12.6.6
	DECL_DIALOG_DATA(ReceiptCore::TecRouteAssignment);
	enum {
		ctlgroupGoods     = 1,
	};
public:
...
int EditTecRouteAssignment(ReceiptCore::TecRouteAssignment * pData)
{
	return PPDialogProcBody <TecRouteAssignmentDialog, ReceiptCore::TecRouteAssignment> (pData);
}
...
IMPL_HANDLE_EVENT(TSessionDialog)
{
	...
	if(TVCOMMAND) {
		if(event.isCmd(cmTSessDetail))
			...
			dlg_data.FinalProductID = Data.Rec.FinalProductID;
			dlg_data.TechRouteID = Data.Rec.TechRouteID;
			if(EditTecRouteAssignment(&dlg_data) > 0) {
				Data.Rec.FinalProductID = dlg_data.FinalProductID;
				Data.Rec.TechRouteID = dlg_data.TechRouteID;
			}
		}
			...
			long   new_status = 0;
			GetClusterData(CTL_TSESS_STATUS, &new_status);
			const  PPID prc_id = getCtrlLong(CTLSEL_TSESS_PRC);
...
int TSessionDialog::setDTS(const TSessionPacket * pData)
{
	...
	if(Data.Rec.Flags & TSESF_PLAN) {
		...
		ptcg_rec.GStrucID = Data.Rec.ArbitraryGsID; // @v12.6.7
	}
	{
		PrcTechCtrlGroup * p_grp = static_cast<PrcTechCtrlGroup *>(getGroup(ctlgroupPrcTech));
		if(p_grp) {
	...
	if(setGroupData(ctlgroupPrcTech, &ptcg_rec)) {
		...
		// что вызов SetupTech (косвенно через события) приводит к тому, что из этих полей извлекается время.
		//
		setCtrlDatetime(CTL_TSESS_STDT, CTL_TSESS_STTM, Data.Rec.StDt, Data.Rec.StTm);
		//
...
int TSessionDialog::getDTS(TSessionPacket * pData)
{
	...
	if(Data.Rec.Flags & TSESF_PLAN) {
		...
		if(getGroupData(ctlgroupPrcTech, &ptcg_rec)) {
			...
			Data.Rec.ArbitraryGsID = ptcg_rec.GStrucID; // @v12.6.7
		}
...
int PPObjTSession::Edit_ExecSessionOnTechRoute(const ExecSessionOnTechRouteFilt * pFilt, ExecSessionOnTechRouteBlock * pData)
{
	class ExecSessionOnTechRouteDialog : public TDialog {
		...
		PPID   LotID;
		double LotQtty;
		PPID   CurrentTSessID; // Если процессор занять, то это - ид сессии, которой он занят
		PPObjTSession TSesObj;
		PPObjBill * P_BObj;
	public:
		enum {
		...
		ExecSessionOnTechRouteDialog() : TDialog(DLG_TSESS_EXEC_TR), P_BObj(BillObj), LotID(0), LotQtty(0.0), CurrentTSessID(0), State(0)
		{
			addGroup(ctlgroupPrcTech, new PrcTechCtrlGroup(CTLSEL_TSESS_PRC, CTLSEL_TSESS_TECH, CTL_TSESS_ST_GOODS, CTLSEL_TSESS_OBJ, CTLSEL_TSESS_OBJ2, 0/*cmSelTechByGoods*/, 0/*cmCreateGoods*/));
		}
		DECL_DIALOG_SETDTS()
		{
			RVALUEPTR(Data, pData);
		...
		DECL_DIALOG_GETDTS()
		{
			...
			if((TrIdent.ID || TrIdent.Oid.IsFullyDefined()) && Data.TechID) {
				...
				if(trmgr.Get(TrIdent, troute) > 0) {
					LongArray idx_list;
					const  int geibtr = troute.GetEntryIdxByTech(Data.TechID, &idx_list);
					if(geibtr == 1) {
						TrIdent.ItemIdx = idx_list.get(0);
					}
					else {
						// @todo Тут надо как-то отреагировать - ситуация плохая //
					}
				}
			}
			Data.TrIdent = TrIdent;
			SETFLAG(Data.Flags, ExecSessionOnTechRouteBlock::fStopCurrentSess, LOGIC(State & stBusy && Data.TSessID));
			ASSIGN_PTR(pData, Data);
			return ok;
		}
	private:
		int    OnPrcSelection(PPID prcID) // @v12.6.2 
		{
			int    ok = 1;
			const  LDATETIME now_dtm = getcurdatetime_();
			SString info_buf;
			State &= ~stBusy;
			const   bool is_prc_changed = (prcID != Data.PrcID);
			if(is_prc_changed) {
				Data.TSessID = 0;
				Data.TechID = 0;
				Data.GoodsID = 0;
				...
				Data.Serial.Z();
				setCtrlString(CTL_TSESS_SERIAL, Data.Serial);
				setCtrlLong(CTLSEL_TSESS_TECH, Data.TechID);
				setCtrlLong(CTLSEL_TSESS_OBJ, Data.ArID);
				setCtrlLong(CTLSEL_TSESS_OBJ2, Data.Ar2ID);
				setCtrlDatetime(CTL_TSESS_STDT, CTL_TSESS_STTM, Data.TmR.Start);
				setCtrlDatetime(CTL_TSESS_FNDT, CTL_TSESS_FNTM, Data.TmR.Finish);
			}
			if(prcID) {
				...
				if(TSesObj.GetPrc(prcID, &prc_rec, 1, 1) > 0) {
					Data.PrcID = prc_rec.ID;
					PPID   cur_tses_id = 0;
					TSessionTbl::Rec tses_rec_inwork;
					TSessionPacket tses_pack;
					int    r2 = TSesObj.P_Tbl->GetProcessed(prcID, TSESK_SESSION, &tses_rec_inwork);
					if(r2 > 0) {
						cur_tses_id = tses_rec_inwork.ID;
						if(TSesObj.GetPacket(cur_tses_id, &tses_pack, PPObjTSession::gpoLoadLines) > 0) {
							State |= stBusy;
							CurrentTSessID = tses_rec_inwork.ID;
							//
							Data.TSessID = CurrentTSessID;
							Data.TechID = tses_pack.Rec.TechID;
							Data.TmR.Start.Set(tses_pack.Rec.StDt, tses_pack.Rec.StTm);
							Data.TmR.Finish.Set(tses_pack.Rec.FinDt, tses_pack.Rec.FinTm);
							if(tses_pack.Lines.getCount()) {
								const TSessLineTbl::Rec & r_tsl_rec = tses_pack.Lines.at(0);
								LotID = r_tsl_rec.LotID;
								Data.GoodsID = r_tsl_rec.GoodsID;
								Data.Serial = r_tsl_rec.Serial;
							}
							setCtrlString(CTL_TSESS_SERIAL, Data.Serial);
							{
								PrcTechCtrlGroup * p_grp = static_cast<PrcTechCtrlGroup *>(getGroup(ctlgroupPrcTech));
								if(p_grp) {
									...
									ptcg_rec.Ar2ID = Data.Ar2ID;
									if(setGroupData(ctlgroupPrcTech, &ptcg_rec)) {
										p_grp->SetGoodsID(this, Data.GoodsID);
									}
								}
							}
							setCtrlDatetime(CTL_TSESS_STDT, CTL_TSESS_STTM, Data.TmR.Start);
						...
						int    r = TSesObj.IsProcessorBusy(prcID, 0, TSESK_SESSION, now_dtm, 1, &cur_tses_id);
						if(r > 0) {
							if(TSesObj.GetPacket(cur_tses_id, &tses_pack, PPObjTSession::gpoLoadLines) > 0) {
								CurrentTSessID = cur_tses_id;
								//
								Data.TechID = tses_pack.Rec.TechID;
							}
						}
		...
		{
			TDialog::handleEvent(event);
			if(event.isCmd(cmCloseCurrentTSess)) {
				if(State & stBusy && CurrentTSessID) {
					endModal(cmOK);
					return;
				}
			}
			else if(event.isCmd(cmInputUpdated) && event.isCtlEvent(CTL_TSESS_SERIAL)) {
				SString serial;
...
/*static*/int PPObjTSession::ExecSessionOnTechRoute(const ExecSessionOnTechRouteFilt * pParam) // @v12.6.0
{
	...
	if(tses_obj.Edit_ExecSessionOnTechRoute(pParam, &blk) > 0) {
		...
		if(blk.Flags & ExecSessionOnTechRouteBlock::fStopCurrentSess) {
			if(blk.TSessID) {
				TSessionPacket pack;
				if(tses_obj.GetPacket(blk.TSessID, &pack, PPObjTSession::gpoLoadLines) > 0) {
					if(pack.Rec.Status == TSESST_INPROCESS) {
						if(tses_obj.SetSessionState(&pack.Rec, TSESST_CLOSED, 0)) {
							PPID   _id = pack.Rec.ID;
							//pack.Rec.FinDt = 
							PPTransaction tra(1);
							THROW(tra);
			...
			}
			{
				...
				if(tses_obj.InitPacket(&new_sess_pack, TSESK_SESSION, blk.PrcID, 0)) {
					...
					{
						const  long def_duration_sec = 60;
						long   duration_sec = 0;
						const  double _local_qtty = (blk.LotQtty > 0.0) ? blk.LotQtty : 1.0;
						if(p_tre && p_tre->NominalTimeSec) {
							duration_sec = R0i(p_tre->NominalTimeSec * _local_qtty);
						}
						else if(tec_rec.Capacity > 0.0) {
							duration_sec = R0i(_local_qtty / tec_rec.Capacity);
						}
						else {
							duration_sec = def_duration_sec;
						}
						if(duration_sec > 0) {
							LDATETIME end_time = start_dtm.plussec(duration_sec);
							new_sess_pack.Rec.FinDt = end_time.d;
							new_sess_pack.Rec.FinTm = end_time.t;
						}
					}
							...
							if(tses_obj.PutLine(new_id, &oprno, &line_rec, 0)) {
								if(tec_route_stage_tag) {
									if((blk.TrIdent.ID || blk.TrIdent.Oid.IsFullyDefined()) && blk.TrIdent.ItemIdx) {
										...
										ObjTagItem tag_item;
										if(tag_item.SetStr(tec_route_stage_tag, temp_buf)) {
											THROW(p_ref->Ot.PutTag(PPOBJ_LOT, blk.LotID, &tag_item, 0));
										}
									}
								}
							}
