...
const uint16 PPTssModel::Default_OptRangeStepCount = 1;
...
#define CMPF(f) if(f!=rS.f) eq = 0;
	...
	else CMPF(OptRangeStep_Measure)
	else CMPF(StrategyPoolSortOrder)
#undef CMPF
	return eq;
}
...
int FASTCALL PPTssModelPacket::Extension::IsEq(const Extension & rS) const
{
	...
	if(AdoptSlShiftDn != rS.AdoptSlShiftDn)
		...
		yes = 0;
	else if(MaxStakeCountVariation != rS.MaxStakeCountVariation)
		yes = 0;
	return yes;
}
...
int PPTssModelPacket::Output(SString & rBuf) const
{
	...
	if(Rec.Flags & PPTssModel::fTrendErrLimitAsMedianPart)
		rBuf.CR().CatEq("partitial_trend_err_limit", Rec.InitTrendErrLimit_, MKSFMTD_030);
	else
		rBuf.CR().CatEq("init_trend_err_limit", Rec.InitTrendErrLimit_, MKSFMTD_030);
	rBuf.CR().CatEq("init_main_trend_err_limit", Rec.InitMainTrendErrLimit, MKSFMTD_030);
	rBuf.CR();
	{
	...
	}
	{
		...
		if(Rec.OptTargetCriterion == PPTssModel::tcWinRatio)
			...
			rBuf.Cat("velocity");
		else if(Rec.OptTargetCriterion == PPTssModel::tcAngularRatio)
			rBuf.Cat("angleratio");
		else
			rBuf.Cat("amount");
	}
	{
		...
		rBuf.CR().CatEq("opt_range_step_count", Rec.OptRangeStepCount);
		rBuf.CR().CatEq("opt_range_max_ext_probe", Rec.OptRangeMaxExtProbe);
	}
	rBuf.CR().CatEq("min_win_rate", Rec.MinWinRate, MKSFMTD_020);
	rBuf.CR().CatEq("overall_win_rate_limit", Rec.OverallWinRateLimit, MKSFMTD_020);
	rBuf.CR().CatEq("max_stake_count_variation", E.MaxStakeCountVariation, MKSFMTD_030);
	rBuf.CR().CatEq("strategy_pool_sort_order", static_cast<uint>(Rec.StrategyPoolSortOrder));
...
public:
	...
	DECL_DIALOG_SETDTS()
	{
		...
		AddClusterAssoc(CTL_TSSMODEL_OPTCRIT, 3, PPTssModel::tcAngularRatio);
		SetClusterData(CTL_TSSMODEL_OPTCRIT, Data.Rec.OptTargetCriterion);
		AddClusterAssoc(CTL_TSSMODEL_BSUBSCHNK,    0, 0);
		...
		setCtrlData(CTL_TSSMODEL_MAXSTCTVAR, &Data.E.MaxStakeCountVariation);
		return 1;
	}
	DECL_DIALOG_GETDTS()
	{
		...
		getCtrlData(CTL_TSSMODEL_OPTRSTEPCT, &Data.Rec.OptRangeStepCount);
		...
		getCtrlData(CTL_TSSMODEL_MAXSTCTVAR, &Data.E.MaxStakeCountVariation);
...
int FASTCALL PPTimeSeries::IsEq(const PPTimeSeries & rS) const
{
	...
	if(!sstreq(Name, rS.Name))
		...
		eq = 0;
	else if(TargetQuant != rS.TargetQuant)
		eq = 0;
	else if(TssModelID != rS.TssModelID)
		eq = 0;
...
bool FASTCALL PPTimeSeriesPacket::Extension::IsEq(const PPTimeSeriesPacket::Extension & rS) const
{
	...
	if(MarginManual != rS.MarginManual)
		eq = false;
	else if(FixedStakeVolume != rS.FixedStakeVolume)
		eq = false;
	else if(AvgLocalDeviation != rS.AvgLocalDeviation)
		eq = false;
	else if(UseDataForStrategiesSince != rS.UseDataForStrategiesSince)
		eq = false;
	return eq;
}
...
PPObjTimeSeries::Config::ExtBlock::ExtBlock() : MaxAvgTimeSec(0), TsFlashTimer(0), MinLossQuantForReverse(0), MinAgeSecondsForReverse(0),
	TerminalTimeAdjustment(0), LocalDevPtCount(0), LDMT_Factor(0), MainTrendMaxErrRel(0.0f), TestCount(0), Reserve2(0), Reserve3(0)
{
	memzero(Reserve, sizeof(Reserve));
}
...
int FASTCALL PPObjTimeSeries::Config::IsEq(const PPObjTimeSeries::Config & rS) const
{
	...
	if(Ver != rS.Ver)
		...
		eq = 0;
	else if(E.MinLossQuantForReverse != rS.E.MinLossQuantForReverse)
		eq = 0;
	else if(E.MinAgeSecondsForReverse != rS.E.MinAgeSecondsForReverse)
		eq = 0;
	else if(E.TerminalTimeAdjustment != rS.E.TerminalTimeAdjustment)
		eq = 0;
	else if(E.LocalDevPtCount != rS.E.LocalDevPtCount)
		eq = 0;
	else if(E.LDMT_Factor != rS.E.LDMT_Factor)
		eq = 0;
	else if(E.MainTrendMaxErrRel != rS.E.MainTrendMaxErrRel)
		eq = 0;
	else if(E.TestCount != rS.E.TestCount)
		eq = 0;
...
PPObjTimeSeries::Config & PPObjTimeSeries::Config::Z()
{
	...
	E.TsFlashTimer = 0;
	E.MinLossQuantForReverse  = 0;
	E.MinAgeSecondsForReverse = 0;
	E.LocalDevPtCount = 0;
	E.LDMT_Factor = 0;
	E.TestCount = 0;
...
public:
	...
	DECL_DIALOG_SETDTS()
	{
		...
		setCtrlData(CTL_TIMSERCFG_RVRMLQ,    &Data.E.MinLossQuantForReverse);
		setCtrlData(CTL_TIMSERCFG_RVRMAS,    &Data.E.MinAgeSecondsForReverse);
		setCtrlData(CTL_TIMSERCFG_TERMTMADJ, &Data.E.TerminalTimeAdjustment);
		setCtrlData(CTL_TIMSERCFG_TSFLSHTMR, &Data.E.TsFlashTimer);
		setCtrlData(CTL_TIMSERCFG_LOCDEVPT,  &Data.E.LocalDevPtCount);
		setCtrlData(CTL_TIMSERCFG_LDMTFACTOR,  &Data.E.LDMT_Factor);
		setCtrlData(CTL_TIMSERCFG_MTMAXERR,   &Data.E.MainTrendMaxErrRel);
		setCtrlData(CTL_TIMSERCFG_TESTCOUNT,  &Data.E.TestCount);
		AddClusterAssoc(CTL_TIMSERCFG_FLAGS, 0, PPObjTimeSeries::Config::fTestMode);
		DisableClusterItem(CTL_TIMSERCFG_FLAGS, 1, 1);
		DisableClusterItem(CTL_TIMSERCFG_FLAGS, 2, 1);
		AddClusterAssoc(CTL_TIMSERCFG_FLAGS, 3, PPObjTimeSeries::Config::fAllowReverse);
		AddClusterAssoc(CTL_TIMSERCFG_FLAGS, 4, PPObjTimeSeries::Config::fVerifMode);
		AddClusterAssoc(CTL_TIMSERCFG_FLAGS, 5, PPObjTimeSeries::Config::fIgnoreStrangeStakes);
		AddClusterAssoc(CTL_TIMSERCFG_FLAGS, 6, PPObjTimeSeries::Config::fLogStakeEvaluation);
	...
	DECL_DIALOG_GETDTS()
	{
		...
		getCtrlData(CTL_TIMSERCFG_RVRMLQ,    &Data.E.MinLossQuantForReverse);
		getCtrlData(CTL_TIMSERCFG_RVRMAS,    &Data.E.MinAgeSecondsForReverse);
		getCtrlData(CTL_TIMSERCFG_TERMTMADJ, &Data.E.TerminalTimeAdjustment);
		getCtrlData(CTL_TIMSERCFG_TSFLSHTMR, &Data.E.TsFlashTimer);
		getCtrlData(CTL_TIMSERCFG_LOCDEVPT,  &Data.E.LocalDevPtCount);
		getCtrlData(CTL_TIMSERCFG_LDMTFACTOR, &Data.E.LDMT_Factor);
		getCtrlData(CTL_TIMSERCFG_MTMAXERR,   &Data.E.MainTrendMaxErrRel);
		getCtrlData(CTL_TIMSERCFG_TESTCOUNT,  &Data.E.TestCount);
...
int PPObjTimeSeries::EditDialog(PPTimeSeriesPacket * pEntry)
{
	...
	public:
		...
		DECL_DIALOG_SETDTS()
		{
			...
			AddClusterAssoc(CTL_TIMSER_TYPE, 3, PPTimeSeries::tCrypto);
			...
			setCtrlReal(CTL_TIMSER_MANMARG, Data.E.MarginManual);
			setCtrlReal(CTL_TIMSER_FXSTAKE, Data.E.FixedStakeVolume);
			...
			setCtrlData(CTL_TIMSER_TGTQUANT, &Data.Rec.TargetQuant);
			setCtrlData(CTL_TIMSER_SBIDT, &Data.E.UseDataForStrategiesSince);
			SetupPPObjCombo(this, CTLSEL_TIMSER_TSSMODEL, PPOBJ_TSSMODEL, Data.Rec.TssModelID, 0);
			return ok;
		}
		DECL_DIALOG_GETDTS()
		{
			...
			getCtrlData(CTL_TIMSER_TGTQUANT, &Data.Rec.TargetQuant);
			getCtrlData(sel = CTL_TIMSER_MANMARG, &Data.E.MarginManual);
			THROW_PP(Data.E.MarginManual >= 0.0 && Data.E.MarginManual <= 2.0, PPERR_USERINPUT);
			getCtrlData(CTL_TIMSER_FXSTAKE, &Data.E.FixedStakeVolume);
			getCtrlData(CTL_TIMSER_SBIDT, &Data.E.UseDataForStrategiesSince);
			getCtrlData(CTLSEL_TIMSER_TSSMODEL, &Data.Rec.TssModelID);
			ASSIGN_PTR(pData, Data);
			CATCHZOKPPERRBYDLG
			return ok;
		}
				...
				if(Data.Rec.ID) {
					...
					if(Obj.GetTimeSeries(Data.Rec.ID, ts) > 0) {
						...
						if(tsc) {
							...
							stat.LocalDevPtCount = cfg.E.LocalDevPtCount;
							//ts.Sort(); // @debug
							...
							out_buf.CatEq("local-dev-avg", stat.LocalDevAvg, MKSFMTD(0, 8, 0)).Space().CRB();
...
struct TimeSeriesExtention {
	...
	uint8  Reserve[32];       // 
	LDATE  UseDataForStrategiesSince; //
	double AvgLocalDeviation; //
	double FixedStakeVolume;  //
	double MarginManual;      //
	long   Reserve1;          //
	long   Reserve2;          //
};
	...
	PPTimeSeriesPacket org_pack;
	{
		...
		if(pPack == 0) {
			...
			if(!pPack->E.IsEmpty()) {
				...
				ext_blk.FixedStakeVolume = pPack->E.FixedStakeVolume;
				ext_blk.AvgLocalDeviation = pPack->E.AvgLocalDeviation;
				ext_blk.UseDataForStrategiesSince = pPack->E.UseDataForStrategiesSince;
				p_ext = &ext_blk;
			}
			THROW(CheckRightsModByID(pID));
			if(_id) {
				if(pPack->IsEq(org_pack))
					...
					THROW(P_Ref->UpdateItem(Obj, _id, &pPack->Rec, 1, 0));
					THROW(P_Ref->PutProp(Obj, _id, TIMSERPRP_EXTENSION, p_ext, sizeof(*p_ext), 0));
					DS.LogAction(PPACN_OBJUPD, Obj, _id, 0, 0);
				}
			}
			else {
...
int PPObjTimeSeries::GetPacket(PPID id, PPTimeSeriesPacket * pPack)
{
	...
	if(ok > 0) {
		...
		if(P_Ref->GetProperty(Obj, id, TIMSERPRP_EXTENSION, &ext_blk, sizeof(ext_blk)) > 0) {
			...
			pPack->E.FixedStakeVolume = ext_blk.FixedStakeVolume;
			pPack->E.AvgLocalDeviation = ext_blk.AvgLocalDeviation;
			pPack->E.UseDataForStrategiesSince = ext_blk.UseDataForStrategiesSince;
		}
	}
	return ok;
}
...
int PPObjTimeSeries::SetTimeSeries(PPID id, STimeSeries * pTs, int use_ta)
{
	...
	PPTimeSeries ts_rec;
	{
		...
		PPUserFuncProfiler ufp(PPUPRF_TIMSERWRITE);
		{
			...
			DS.LogAction(PPACN_TSSERIESUPD, Obj, id, stored_ts_count, 0);
			THROW(tra.Commit());
		}
		ufp.SetFactor(0, static_cast<double>(stored_ts_count));
		ufp.Commit();
	}
	CATCHZOK
	return ok;
	...
	if(file_list.getCount()) {
		...
		if(CheckDialogPtr(&dlg)) {
			...
			if(param_.SegmentCount > 0 && param_.SegmentN > 0)
				temp_buf.Cat(param_.SegmentN).Slash().Cat(param_.SegmentCount);
...
PPViewTimeSeries::BrwItem::BrwItem(const PPTimeSeriesPacket * pTs)
{
	if(pTs) {
		...
		ManualMarg = pTs->E.MarginManual;
		Type = static_cast<int16>(pTs->Rec.Type);
		Prec = pTs->Rec.Prec;
					...
					if(GetStrategies(id, sst_list[sst_idx], scontainer) > 0) {
						...
						for(uint i = 0; i < scontainer.getCount(); i++) {
							...
							line_buf.Z().
								...
								CatEq("TrendErrAvg", r_s.TrendErrAvg, MKSFMTD(0, 6, 0)).Space().
								CatEq("TrendErrLim", r_s.TrendErrLim, MKSFMTD(0, 1, 0)).Space();
...
struct TestStrategyRawResult {
	...
	RealRange FactorR;       // Диапазон изменения тангенса наклона регрессии для всего множества значений
	uint   NegFactorCount;   // Количество отрицательных значений тангенса в списке SoList
	uint   PosFactorCount;   // Количество положительных значений тангенса в списке SoList
	double TotalResult;
	double TotalProb;
	...
	int16  StakeMode;        // Режим покупки: 0 - сплошной (случайный); 1 - по значению тренда // unused: 2 - по изменению тренда, 3 - по значению и изменению тренда
	...
	const PPObjTimeSeries::CommonTsParamBlock & rCtspb, uint valueIdx, PPObjTimeSeries::StrategyResultValueEx & rV) const
{
	...
	if(tsc && tsc == rCtspb.ValList.getCount() && valueIdx < tsc) {
		...
		const  int prec = Prec;
		const  int adjust_sl = BIN(StakeCloseMode == clsmodAdjustLoss);
		{
			...
			for(uint i = init_offset; i < tsc; i++) {
				if(SelectS2(scsb.Init(static_cast<int>(i), start_tm)) > 0) {
					...
					if(csr == 2) {
						...
						i = rv_ex.LastPoint; // Дальше продолжим со следующей точки
						ok = 1;
					}
					else {
						ok = 100;
...
struct Ts_Strategy_Before10810 { // @flat @persistent
...
int PPObjTimeSeries::StrategyContainer::FinalizeStrategySelection(SelectBlock & rBlk, uint strategyIdx, int lastTrendIdx) const
{
	...
	if(local_result > 0.0 || (is_single && local_result == 0.0)) {
		...
		if(rBlk.MainTrendMaxErrRel <= 0.0f || main_trend_err_rel <= static_cast<double>(rBlk.MainTrendMaxErrRel)) {
...
int PPObjTimeSeries::StrategyContainer::SelectS2(SelectBlock & rBlk) const
{
	...
	const  bool is_single = (getCount() == 1); //(rBlk.R_Index.getCount() == 1);
	{
		...
		for(uint tidx = 0; tidx < rBlk.R_TrendList.getCount(); tidx++) {
			...
			if(rBlk.R_Index.getCount()) {
				uint  ip = 0;
				if((is_single && rBlk.R_Index.at(ip/*==0*/)->Stride == p_te->Stride) || rBlk.R_Index.bsearch(&p_te->Stride, &ip, CMPF_LONG))
			...
			if(p_ie) {
				...
				assert(!is_single || range_list_count == 1);
							...
							if(!do_skip) {
								...
								if(r_s.MainFrameSize) {
									...
									if(p_main_te) {
										...
										main_trend_val = p_main_te->TL.at(trend_idx);
										if(!(r_s.BaseFlags & Strategy::bfDynMainTrend)) { // @v11.4.5
				...
				// больше нечего делать в этом цикле раз для единственной стратегии тренд рассмотрен {
				if(is_single) 
					break;
				//
			}
...
int PPObjTimeSeries::StrategyContainer::AddStrategyToOrderIndex(uint pos, long flags, TSArray <PPObjTimeSeries::StrategyContainer::CritEntry> & rIndex) const
{
	...
	if(pos >= 0 && pos < getCount()) {
		...
		crit_entry.StakeCount = r_item.StakeCount;
		...
		else if(flags & gbsfCritStakeCount) {
...
int PPObjTimeSeries::StrategyContainer::MakeConfidenceEliminationIndex(const PPTssModelPacket & rTssPacket, uint maxCount, const LongArray * pSrcIdxList, LongArray & rToRemoveIdxList) const
{
	...
	// @20200807 {
	if(maxCount) {
		// { Попытка решить проблему устойчивости стратегий путем удаления тех из них, чья плотность GetStakeDensity
	...
	// } @20200807
		const double max_vc = (rTssPacket.E.MaxStakeCountVariation > 0.0 && rTssPacket.E.MaxStakeCountVariation < 1.0) ? rTssPacket.E.MaxStakeCountVariation : 0.12;
	...
	StrategyContainer & rScDest, StrategyContainer * pScSkipDueDup, LongArray * pToRemovePosList) const
{
	rScDest.Z();
	int    ok = 1;
	long _ssflags = gbsfLong|gbsfShort|gbsfStakeMode1;
	const double _min_win_rate = rTssPacket.Rec.MinWinRate;
	{
		...
		switch(rTssPacket.Rec.StrategyPoolSortOrder) {
			...
			case PPTssModel::sppsStakeCount: _ssflags |= gbsfCritStakeCount; break;
		}
	}
	...
	LongArray pos_to_remove;
	{
		...
		for(uint ridx = 0; ridx < range_list_.getCount(); ridx++) {
				...
				for(uint j = 0; !do_skip && j < rScDest.getCount(); j++) {
					...
					if(r_j_item.StakeMode == r_item.StakeMode && r_j_item.InputFrameSize == r_item.InputFrameSize) {
						if(feqeps(r_j_item.OptDeltaRange.low, r_item.OptDeltaRange.low, _eps) && feqeps(r_j_item.OptDeltaRange.upp, r_item.OptDeltaRange.upp, _eps)) {
							if(r_j_item.MainFrameSize == r_item.MainFrameSize) {
								if(feqeps(r_j_item.OptDelta2Range.low, r_item.OptDelta2Range.low, _eps) && feqeps(r_j_item.OptDelta2Range.upp, r_item.OptDelta2Range.upp, _eps)) {
									if(pScSkipDueDup) {
	...
	//rScDest.Fb.AngularDensity = Fb.AngularDensity;
...
double PPObjTimeSeries::StrategyContainer::EvaluateScore(int scoreId, const LongArray * pOnlyIndices, const LongArray * pExceptIndices) const
{
	...
	if(scoreId == scoreEvStakeCountMean)
		...
		score = fdivnz(sb.GetStdDev(), sb.GetExp());
	else if(scoreId == scoreEvAngularStakeDensity) { // Угловая плотность ставок на этапе вычисления стратегий
		score = static_cast<double>(total_stake_ev) / total_angle;
	}
	else if(scoreId == scoreSeAngularStakeDensity) { // Угловая плотность ставок на этапе отбора стратегий
		score = static_cast<double>(total_stake_se) / total_angle;
	}
	else if(scoreId == scoreEvAngularWinRate) { // Угловой коэффициент выигрышей
		score = static_cast<double>(total_win_ev) / total_angle;
	}
	else if(scoreId == scoreTotalAngle) {
		score = total_angle;
	}
	return score;
}
...
int PPObjTimeSeries::StrategyContainer::GetInputFramSizeList(LongArray & rList, uint * pMaxOptDelta2Stride) const
{
	...
	for(uint i = 0; i < getCount(); i++) {
		...
		rList.addnz(static_cast<long>(r_item.MainFrameSize));
...
int PPObjTimeSeries::PutStrategies(PPID id, StrategySetType sst, StrategyContainer * pL, int use_ta)
{
	...
	}
	{
		...
		DS.LogAction(PPACN_TSSTRATEGYUPD, Obj, id, static_cast<long>(_c), 0);
		THROW(tra.Commit());
	}
	CATCHZOK
	return ok;
...
PPObjTimeSeries::TrainNnParam::TrainNnParam(const char * pSymb, long flags) : Strategy(), Symb(pSymb), ActionFlags(flags)
{
}

PPObjTimeSeries::TrainNnParam::TrainNnParam(const PPTimeSeriesPacket & rTsPack, long flags) : 
	Strategy(), Symb(rTsPack.Rec.Symb), ActionFlags(flags)
{
	Prec = rTsPack.Rec.Prec;
	Margin = (rTsPack.E.MarginManual > 0.0) ? rTsPack.E.MarginManual : rTsPack.Rec.BuyMarg;
	SpreadAvg = rTsPack.Rec.AvgSpread;
}

void PPObjTimeSeries::TrainNnParam::Reset() { Strategy::Reset(); }
SString & PPObjTimeSeries::TrainNnParam::MakeFileName(SString & rBuf) const { return rBuf.Z().Cat(Symb).CatChar('-').Cat(InputFrameSize).CatChar('-').Cat(MaxDuckQuant).DotCat("fann").ToLower(); }
bool FASTCALL PPObjTimeSeries::Strategy::IsStakeSideEq(const Strategy & rS) const { return ((BaseFlags & bfShort) == (rS.BaseFlags & bfShort)); }
...
double PPObjTimeSeries::Strategy::GetAngularDensity() const { return static_cast<double>(GenPtCount) / GetAngularRange(); }
double PPObjTimeSeries::Strategy::GetStakeDensity() const { return fdivui(StakeCount, GenPtCount); }
double PPObjTimeSeries::Strategy::CalcConfidenceFactor() const { return (GenPtCount > 0) ? (100.0 * static_cast<double>(StakeCount) / log(static_cast<double>(GenPtCount))) : 0.0; }
//
// Descr: Возвращает средний катет (OptDeltaRange*InputFrameSize) / SpikeQuant
//
double PPObjTimeSeries::Strategy::GetOptDeltaRangeCQA() const { return (InputFrameSize > 0 && SpikeQuant_s > 0.0) ? ((InputFrameSize * OptDeltaRange.GetMiddle()) / SpikeQuant_s) : 0.0; }
...
/*static*/double PPObjTimeSeries::Strategy::CalcSL_withExternalFactors(double peak, bool isShort, int prec, uint maxDuckQuant, double spikeQuant, double averageSpreadForAdjustment)
{
	const  double mdv = (maxDuckQuant * spikeQuant);
	const  double __spread = CalcSlTpAdjustment(prec, averageSpreadForAdjustment);
	return isShort ? round(peak * (1.0 + mdv) + __spread, prec) : round(peak * (1.0 - mdv) - __spread, prec);
}

/*static*/double PPObjTimeSeries::Strategy::CalcTP_withExternalFactors(double stakeBase, bool isShort, int prec, uint targetQuant, double spikeQuant, double averageSpreadForAdjustment)
{
	if(targetQuant) {
		const  double tv = (targetQuant * spikeQuant);
		const  double __spread = CalcSlTpAdjustment(prec, averageSpreadForAdjustment);
		return isShort ? round(stakeBase * (1.0 - tv) + __spread, prec) : round(stakeBase * (1.0 + tv) - __spread, prec);
	}
	else
		return 0.0;
}
...
int PrcssrTsStrategyAnalyze::EditParam(PPBaseFilt * pBaseFilt)
{
	...
	public:
		...
		{
			SetupCalDate(CTLCAL_TSSA_SBTDT, CTL_TSSA_SBTDT);
			SetupCalDate(CTLCAL_TSSA_SBTDT2, CTL_TSSA_SBTDT2);
		}
		DECL_DIALOG_SETDTS()
		{
			int    ok = 1;
			...
			AddClusterAssoc(CTL_TSSA_FLAGS, 6, Data.fSimulateAfter);
			AddClusterAssoc(CTL_TSSA_FLAGS, 7, Data.fAnalyzeModels);
			...
			setCtrlDate(CTL_TSSA_SBTDT, Data.UseDataForStrategiesTill);
			setCtrlDate(CTL_TSSA_SBTDT2, Data.UseDataForStrategiesTill2);
			Setup();
			return ok;
		}
		DECL_DIALOG_GETDTS()
		{
			...
			getCtrlData(CTL_TSSA_SBTDT, &Data.UseDataForStrategiesTill);
			getCtrlData(CTL_TSSA_SBTDT2, &Data.UseDataForStrategiesTill2);
			ASSIGN_PTR(pData, Data);
			return ok;
		}
	private:
...
/*static*/SString & PPObjTimeSeries::StrategyToString(const PPObjTimeSeries::Strategy & rS, const PPObjTimeSeries::StrategyContainer * pSc, const double * pOptFactor, const double * pOptFactor2, SString & rBuf)
{
	...
	rBuf.Space().Cat("Potential").CatDiv(':', 2).
		Cat(rS.V.GetResultPerDay(), MKSFMTD_020).Slash().Cat(rS.V.Result, MKSFMTD(5, 1, 0)).Slash();
	{
	...
	if(rS.MainFrameSize) {
		...
		if(pOptFactor2) {
			...
			if(op2r_dist > 0.0 && rS.OptDelta2Range.Check(*pOptFactor2)) {
				...
				rBuf.Cat(tv2_part, MKSFMTD_030).CatChar('|');
			}
			else
		...
		if(pOptFactor) {
			if(rS.StakeMode == 1) {
				...
				if(opr_dist > 0.0 && rS.OptDeltaRange.Check(*pOptFactor)) {
					...
					rBuf.Cat(tv_part, MKSFMTD_030).CatChar('|');
				}
				else
					rBuf.CatChar('?').CatChar('|');
				...
				rBuf.Cat(*pOptFactor, trange_fmt).CatChar('|').Cat(rS.OptDelta2Stride).Slash().Cat(rS.OptDelta2Range, trange_fmt);
			...
			if(rS.StakeMode == 1) {
				...
				rBuf.Cat(rS.OptDelta2Stride).Slash().Cat(rS.OptDelta2Range, trange_fmt);
			else if(rS.StakeMode == 3) {
	...
	if(pSre == 0) {
			...
			CatLongZ(pSre->PeakAvgQuant, 2).Colon().CatLongZ(pSre->PeakMaxQuant, 2).Colon().CatLongZ(pSre->BottomAvgQuant, 2).Space();
		{
			rBuf.CatChar('[').Cat(pSre->OptDeltaRange, MKSFMTD(0, 12, NMBF_FORCEPOS)).
				...
				CatChar('|').Cat(pSre->GetOptDeltaRangeCQA(), MKSFMTD_020).CatChar(']');
			rBuf.Space();
			if(pSre->StakeMode == 0)
			...
			if(pSre->StakeMode != 1) {
				rBuf.Cat(pSre->OptDelta2Range, MKSFMTD(0, 12, 0)).Space().
					Cat(pSre->OptDelta2Stride).Slash().Cat(pSre->OptDelta2Range.Count);
			}
			rBuf.Space();
			if(pSre->StakeMode == 0)
...
PrcssrTsStrategyAnalyze::ModelParam::ModelParam() : Flags(0), OptTargetCriterion(tcWinRatio), InitTrendErrLimit(0.0), PartitialTrendErrLimit(0.0), 
	...
	OverallWinRateLimit(0.0), Reserve(0)
{
}
...
PrcssrTsStrategyAnalyze::ModelParam & PrcssrTsStrategyAnalyze::ModelParam::Z()
{
	...
	PartitialTrendErrLimit = 0.0;
	InitMainTrendErrLimit = 0.0;
	BestSubsetDimention = 0;
	BestSubsetMaxPhonyIters = 0;
	BestSubsetOptChunk = 0;
	MaxDuckQuantList_Obsolete.clear();
	TargetQuantList_Obsolete.clear();
	StakeBoundList.clear();
	InputFrameSizeList.clear();
	UseDataSince = ZERODATE;
	UseDataTill = ZERODATE;
	OverallWinRateLimit = 0.0;
	MainFrameRangeCount = 0;
	MinWinRate = 0.0;
	OptRangeStep = 0;
	OptRangeStepMkPart = 0;
	OptRangeMultiLimit = 0;
	OptRangeMaxExtProbe = 0;
	Reserve = 0;
	return *this;
}
...
int PrcssrTsStrategyAnalyze::ReadModelParam(ModelParam & rMp)
{
	...
	}
	{
		if(ini_file.Get(PPINISECT_TSSTAKE, PPINIPARAM_TSSTAKE_OPTRANGETARGET, temp_buf) > 0) {
			...
			else if(temp_buf.IsEqiAscii("winratio"))
				rMp.OptTargetCriterion = rMp.tcWinRatio;
			else if(temp_buf.IsEqiAscii("angleratio"))
				rMp.OptTargetCriterion = rMp.tcAngularRatio;
			else if(temp_buf.IsEqiAscii("amount"))
...
{
	rBuf.Z().
		CatEq("Result", rEntry.V.Result, MKSFMTD_030).
		Space().CatEq("StakeCount", rEntry.StakeCount).Space().
		Space().CatEq("WinCount", rEntry.WinCount).
		Space().CatEq("LossCount", rEntry.StakeCount-rEntry.WinCount).
		Space().CatEq("WinCountRate", rEntry.GetWinCountRate(), MKSFMTD_030).
	...
	//rBuf.Space().CatEq("StakeDensity", fdivui(rEntry.StakeCount, rEntry.GenPtCount), MKSFMTD_030);
	return rBuf;
}
...
struct TsFindStrategiesLoopBlock {
	...
	TSCollection <TestStrategyRawResult> * P_TsrrList;
	SFile * P_FOut;
};
...
struct TsFindStrategiesBlock {
	...
	enum TsTestStrategyOfrOption {
		...
		tstsofroTotalOnly        = 0x4000,
		tstsofroEvaluateResonanceMeasure = 0x8000
	};
	//
	// useInitialSplitting:
		...
		TSVector<PPObjTimeSeries::OptimalFactorRange_WithPositions> * pRc, TestStrategyRawResult * pRawResult) const
	{
		...
		else if(r_tssm.OptTargetCriterion == PPTssModel::tcAngularRatio)
		...
		if(rList.InitialSplitting(useInitialSplitting, &_first_idx, &_last_idx)) {
			...
			if(pRc) {
				...
				const  int single_scan_algorithm = 1;
					...
					for(int do_next_iter = 1; do_next_iter;) {
						...
						if(_sfd_extr_entry.Result > 0.0 && (_sfd_extr_entry.SfIdx > 0 || _sfd_extr_entry.SfIdxUp < (_c-1))) {
							...
							if(total_max_result == 0.0) {
								...
								else if(oneof2(r_tssm.OptTargetCriterion, PPTssModel::tcWinRatio, PPTssModel::tcAngularRatio)) {
									total_max_result = _sfd_extr_entry.Result;
									do_reckon = 1;
								}
				...
				if(rSre.StakeMode == 0) {
						...
						initial_splitting = 4;
					{
						...
						for(uint ofridx = 0; ofridx < ofr_list.getCount(); ofridx++) {
							...
							sre_temp.GenSeq = r_ofr_wp.GenSeq;
							sre_temp.GenPtCount = r_ofr_wp.GenPtCount;
							sre_temp.FactorRangeLo = r_ofr_wp.LoPos;
							sre_temp.FactorRangeUp = r_ofr_wp.UpPos;
							if(main_ifs) {
	...
	uint firstIdx, uint lastIdx, uint maxDuckQuant, uint targetQuant, PPObjTimeSeries::StrategyResultEntry & rResult, TSVector <PPObjTimeSeries::StrategyResultValueEx> * pDetailList) const
{
	...
	assert(!pDetailList || pDetailList->getCount() == sre_sim.StakeCount);
...
static void EvaluateAfterSimulateFactors(TsFindStrategiesBlock * pBlk, 
	...
	const TSVector <PPObjTimeSeries::StrategyResultValueEx> & rSrDetailList, PPObjTimeSeries::StrategyResultEntry & rSre)
{
	uint max_seq = 0;
	...
	rSre.GenPtCount = (max_gen_pt - min_gen_pt + 1);
	{
		...
		rSre.CADF = static_cast<float>(GetStrategyResultCADF(rSrDetailList));
	}
	if(pBlk->P_Tsrr && pBlk->P_Tsrr->SoList.getCount()) { // @v11.0.0 pBlk->P_Tsrr->SoList.getCount()
		rSre.ADF = static_cast<float>(pBlk->P_Tsrr->CalcADF(min_gen_pt, max_gen_pt));
		rSre.UEF = static_cast<float>(pBlk->P_Tsrr->CalcUEF(min_gen_pt, max_gen_pt));
	}
}
...
int PrcssrTsStrategyAnalyze::FindStrategies(void * pBlk) const
{
	...
	uint    lt_min_winrate_count = 0; // Количество стратегий, надежность которых меньше лимита. При превышении этим
		// значением max_lt_min_winrate_count прекращаем тестирование - скорее всего все остальные хуже.
	const uint max_lt_min_winrate_count = 100000; // (very large in order to avoid limit)
	// @v11.4.5 const bool adopt_sltp = (p_blk->R_TssModel.E.AdoptSlShiftDn || p_blk->R_TssModel.E.AdoptSlShiftUp);
	...
	const PPObjTimeSeries::TrendEntry * p_main_trend_entry = 0; // @v11.4.5 p_blk->R_Tnnp2.MainFrameSize ? PPObjTimeSeries::SearchTrendEntry(p_blk->R_Ctspb.TrendList, p_blk->R_Tnnp2.MainFrameSize) : 0;
	const PPObjTimeSeries::TrendEntry * p_dyn_main_trend_entry = 0; // @v11.4.5 
	// @v11.4.5 {
	if(p_blk->R_Tnnp2.MainFrameSize) {
		if(p_blk->R_Tnnp2.BaseFlags & PPObjTimeSeries::Strategy::bfDynMainTrend) {
			p_dyn_main_trend_entry = PPObjTimeSeries::SearchTrendEntry(p_blk->R_Ctspb.TrendList, p_blk->R_Tnnp2.MainFrameSize);
			assert(p_dyn_main_trend_entry);
		}
		else
			p_main_trend_entry = PPObjTimeSeries::SearchTrendEntry(p_blk->R_Ctspb.TrendList, p_blk->R_Tnnp2.MainFrameSize);
	}
	// } @v11.4.5 
	//
	//
...
#endif // } 0 @v11.4.5
					{
						assert(srridx_up == srridx_lo);
						// @v11.4.5 SimulateStrategyResultEntries должна посчитать результаты с одновременной засечкой значений магистрального тренда!
						SimulateStrategyResultEntries(p_blk, sr_raw_list, srridx_lo, srridx_up, 0, 0, sre_test, &sr_detail_list);
						// @v11.4.5 {
						PPObjTimeSeries::StrategyResultEntry & r_local_strategy = sr_raw_list.at(srridx_lo);
						if(p_dyn_main_trend_entry) {
							RAssocArray main_trend_val_list;
							{
								for(uint ai = 0; ai < sr_detail_list.getCount(); ai++) {
									const PPObjTimeSeries::StrategyResultValueEx & r_entry = sr_detail_list.at(ai);
									//const double angle = atan(r_entry.OptFactor2);
									main_trend_val_list.Add(static_cast<long>(ai+1), /*angle*/r_entry.OptFactor2);
								}
							}
...
int PrcssrTsStrategyAnalyze::FindStrategiesLoop(void * pBlk)
{
	...
	SString temp_buf;
	SString msg_buf;
	uint   max_chunk_count = 0; // Максимальный количество допустимых стратегий в одной итерации
	...
	const bool main_frame_as_correlation_tool = LOGIC(r_tss_model.MainFrameSizeList.getCount() && !r_tss_model.Rec.MainFrameRangeCount); // @v11.2.8
	const int  force_fixed_maxduck_values = 1;
	uint  main_frame_as_correlation_tool_size = 0; 
	...
	if(P.Flags & (P.fFindStrategies|P.fAnalyzeModels)) {
		...
		else if(main_frame_as_correlation_tool) { // @v11.2.8
			assert(r_tss_model.MainFrameSizeList.getCount());
			// Используем только 1-ю длину MainFrameSizeList
			main_frame_as_correlation_tool_size = static_cast<uint>(r_tss_model.MainFrameSizeList.get(0));
			frame_size_list.addUnique(main_frame_as_correlation_tool_size); 
		}
		{
	...
	{
		class FindStrategiesTask : public SlThread_WithStartupSignal {
		public:
		...
		//
		const size_t thread_limit = 32;
		const  uint max_threads = (P.Flags & P.fAnalyzeModels) ? 2 : 16;
		TSCollection <PPObjTimeSeries::StrategyContainer> sc_list;
		TSCollection <TestStrategyRawResult> local_tsrr_list;
		// @v11.4.5 TSCollection <PPObjTimeSeries::TrainNnParam> tnnp_list;
				...
				for(uint mfrlidx = 0; mfrlidx < main_frame_range_list.getCount(); mfrlidx++) {
					...
					for(uint ifsidx = 0; ifsidx < effective_input_frame_size_list.getCount(); ifsidx++) {
						...
						for(uint bpidx = 0; bpidx < effective_stake_bound_list.getCount(); bpidx++) {
							...
							else if(main_frame_as_correlation_tool) { // @v11.4.5 
								p_tnnp2->MainFrameSize = main_frame_as_correlation_tool_size;
								p_tnnp2->OptDelta2Range.Z();
								p_tnnp2->BaseFlags |= PPObjTimeSeries::TrainNnParam::bfDynMainTrend;
							}
							else {
							...
							// @v11.4.5 tnnp_list.insert(p_tnnp2);
...
int PrcssrTsStrategyAnalyze::EvaluateStrategies(void * pBlk, LDATE dateTill, long flags, SFile & rFOutTotal, PPObjTimeSeries::StrategyResultEntry * pResult)
{
	...
	if(p_blk->R_SContainer.GetBestSubset2(p_blk->R_TssModel, sc_process, &sc_skip_due_dup, &to_remove_pos_list) > 0) {
		{
	...
	if(sc_selection.getCount()) {
		...
		sc_selection.AccumulateFactors(sre);
		...
		if(rFOutTotal.IsValid()) {
			...
			}
			{
				...
				for(uint clidx = 0; clidx < ifs_count_list.getCount(); clidx++) {
					...
					msg_buf.CatDivConditionally(' ', 0, LOGIC(clidx)).Cat(r_assoc.Key).Slash().Cat(r_assoc.Val);
				}
				rFOutTotal.WriteLine(msg_buf.CR());
			}
			{
			...
			if(main_frame_count_list.getCount()) {
				...
				for(uint clidx = 0; clidx < main_frame_count_list.getCount(); clidx++) {
					...
					msg_buf.CatDivConditionally(' ', 0, LOGIC(clidx)).CatChar('[').Cat(p_entry->R, trange_fmt).CatChar(']').Slash().Cat(p_entry->Count);
				}
				rFOutTotal.WriteLine(msg_buf.CR());
			}
			rFOutTotal.WriteBlancLine();
			rFOutTotal.Flush();
		}
	}
	CATCHZOK
			...
			if(TsObj.GetPacket(id, &ts_pack) > 0 && GetTssModel(&ts_pack.Rec, &tss_model) > 0) {
				...
				if(do_iterate_throw_periods && checkdate(P.UseDataForStrategiesTill)) {
					...
					if(gtsr > 0) {
						...
						// @v11.4.5 const bool use_main_frame = (tss_model.MainFrameSizeList.getCount() && tss_model.Rec.MainFrameRangeCount);
						//uint   vec_idx = 0;
						...
						}
						{
							...
							if(P.Flags & P.fFindStrategies) {
								if(tss_model.Rec.Flags & PPTssModel::fSeparateInpFrameSizes && tss_model.InputFrameSizeList.getCount() > 1) {
									...
									PPObjTimeSeries::StrategyContainer aggregated_container;
																...
																for(uint rcidx = 0; rcidx < result_collection.GetIndexCount(); rcidx++) {
																	...
																	if(p_ri) {
																		...
																		if(checkdate(till_date) && r_dli.TmR.Start.d > till_date) {
																			...
																			dmap_entry.CADF = r_s.CADF;
																			dmap_entry.TrendErrRel = r_dli.TrendErrRel;
																			dmap_entry.AngleR.Set(atan(r_s.OptDeltaRange.low), atan(r_s.OptDeltaRange.upp));
										...
										int use_adf_range = 0;
										{
											...
											for(uint cpidx = 0; cpidx < SIZEOFARRAY(cplist); cpidx++) {
												...
												if(dmap.SearchBestRange(ts_pack.Rec.ID, cplist[cpidx].Side, cplist[cpidx].Edge, cplist[cpidx].Factor, 0.8, re) > 0) {
													...
													switch(cplist[cpidx].Factor) {
														case PPObjTimeSeries::TsDensityMapEntry::factorADF: temp_buf.Cat("ADF").CatCharN(' ', 8); break;
														case PPObjTimeSeries::TsDensityMapEntry::factorUEF: temp_buf.Cat("UEF").CatCharN(' ', 8); break;
														case PPObjTimeSeries::TsDensityMapEntry::factorGenCount: temp_buf.Cat("GenCount").CatCharN(' ', 3); break;
														case PPObjTimeSeries::TsDensityMapEntry::factorGenTEA: temp_buf.Cat("GenTEA").CatCharN(' ', 5); break;
														case PPObjTimeSeries::TsDensityMapEntry::factorGenTED: temp_buf.Cat("GenTED").CatCharN(' ', 5); break;
													...
													temp_buf.Space().Cat(re.Sc).Space().Cat(re.Wc).Space().Cat(re.Sc-re.Wc).Space().Cat(re.Prob, MKSFMTD_030);
...
int PPObjTimeSeries::TrendEntry::Analyze(const PPTssModelPacket & rTssModel, SString & rOutBuf) const
{
	...
	rOutBuf.Tab().CatEq("stride", input_frame_size).CR();
	{
		...
		rOutBuf.Tab_(2).Cat("error-list").CatDiv(':', 2).CR().Tab_(3).
			...
			CatEq("median/avg", median/errl_stat.GetExp(), MKSFMTD_030).Space().
			CatEq("median (1/4)/avg", median_2/errl_stat.GetExp(), MKSFMTD_030).Space().
			CatEq("median (3/4)/avg", median_3/errl_stat.GetExp(), MKSFMTD_030);
		if(!(rTssModel.Rec.Flags & PPTssModel::fTrendErrLimitAsMedianPart) && model_err_limit_ > 0.0) {
			rOutBuf.CR().Tab_(3).CatEq("init_trend_err_limit", model_err_limit_, MKSFMTD_030).Space();
			...
			rOutBuf.CatEq("err_lim_part", fdivui(err_lim_pos, temp_real_list.getCount()), MKSFMTD_030);
		}
		if((rTssModel.Rec.Flags & PPTssModel::fTrendErrLimitAsMedianPart) && model_err_limit_ > 0.0 && model_err_limit_ <= 1.0) {
			rOutBuf.CR().Tab_(3).CatEq("partitial_trend_err_limit", model_err_limit_, MKSFMTD_030).Space();
			...
			if(lim_pos >= 0 && lim_pos < temp_real_list.getCount()) {
				...
				rOutBuf.CatEq("err_limit_by_pel", err_limit_by_pel, MKSFMTD_030);
			}
		}
	}
	return ok;
						...
						}
						{
							...
							if(flags & tscfSkipOutput) {
								...
								if(rResultCollection.Atdr.StakeCount && single_MainFrameSize >= 0 && single_InputFrameSize > 0 && single_TargetQuant > 0 && single_MaxDuckQuant > 0) {
									...
									msg_buf.Cat(single_InputFrameSize).Slash().Cat(single_MaxDuckQuant).Colon().Cat(single_TargetQuant).Space().
...
int PPObjTimeSeries::TsDensityMap::SearchBestRange(PPID tsID, int side, int edge, int factor, double minProb, ResultEntry & rResult) const
{
	...
	if(oneof7(factor, TsDensityMapEntry::factorADF, TsDensityMapEntry::factorTrendErrRel, TsDensityMapEntry::factorUEF, 
		...
		{
			switch(factor) {
				case TsDensityMapEntry::factorADF: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_ADF)); break;
				case TsDensityMapEntry::factorUEF: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_UEF)); break;
				case TsDensityMapEntry::factorTrendErrRel: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_TrendErrRel)); break;
				case TsDensityMapEntry::factorGenCount: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_GenCount)); break;
				case TsDensityMapEntry::factorGenTEA: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_GenTEA)); break;
				case TsDensityMapEntry::factorGenTED: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_GenTED)); break;
				case TsDensityMapEntry::factorCADF: temp_map.sort(PTR_CMPFUNC(TsDensityMapEntry_CADF)); break;
			}
			if(edge == 1)
			...
			for(uint i = 0; i < _c; i++) {
				...
				if(temp_map.GetNextChunk(factor, minProb, i, &last_chunk_idx, &local_re)) {
					if(local_re.Sc > best_re.Sc || (local_re.Sc == best_re.Sc && local_re.Prob > best_re.Prob)) {
						best_re = local_re;
						switch(factor) {
							case TsDensityMapEntry::factorADF: best_re.FactorR.Set(temp_map.at(i).ADF, temp_map.at(last_chunk_idx).ADF); break;
							case TsDensityMapEntry::factorUEF: best_re.FactorR.Set(temp_map.at(i).UEF, temp_map.at(last_chunk_idx).UEF); break;
							case TsDensityMapEntry::factorGenCount: best_re.FactorR.Set(temp_map.at(i).GenCount, temp_map.at(last_chunk_idx).GenCount); break;
							case TsDensityMapEntry::factorGenTEA: best_re.FactorR.Set(temp_map.at(i).GenTEA, temp_map.at(last_chunk_idx).GenTEA); break;
							case TsDensityMapEntry::factorGenTED: best_re.FactorR.Set(temp_map.at(i).GenTED, temp_map.at(last_chunk_idx).GenTED); break;
							case TsDensityMapEntry::factorCADF: best_re.FactorR.Set(temp_map.at(i).CADF, temp_map.at(last_chunk_idx).CADF); break;
							case TsDensityMapEntry::factorTrendErrRel: best_re.FactorR.Set(temp_map.at(i).TrendErrRel, temp_map.at(last_chunk_idx).TrendErrRel); break;
						}
						ok = 1;
					}
				}
				if(edge < 0) {
					switch(factor) {
						case TsDensityMapEntry::factorADF: while((i+1) < _c && feqeps(temp_map.at(i+1).ADF, temp_map.at(i).ADF, 1E-5)) { i++; } break;
						case TsDensityMapEntry::factorUEF: while((i+1) < _c && (temp_map.at(i+1).UEF == temp_map.at(i).UEF)) { i++; } break;
						case TsDensityMapEntry::factorGenCount: while((i+1) < _c && (temp_map.at(i+1).GenCount == temp_map.at(i).GenCount)) { i++; } break;
						case TsDensityMapEntry::factorTrendErrRel: while((i+1) < _c && feqeps(temp_map.at(i+1).TrendErrRel, temp_map.at(i).TrendErrRel, 1E-5)) { i++; } break;
						case TsDensityMapEntry::factorGenTEA: while((i+1) < _c && feqeps(temp_map.at(i+1).GenTEA, temp_map.at(i).GenTEA, 1E-5)) { i++; } break;
						case TsDensityMapEntry::factorGenTED: while((i+1) < _c && feqeps(temp_map.at(i+1).GenTED, temp_map.at(i).GenTED, 1E-5)) { i++; } break;
					}
				}
				else
					break;
...
int TestTsDensityMap() // @debug
{
	...
	if(dmap.Import(inp_file_name)) {
		...
		line_buf.Z().CatEq("knn-011-win-rate", fdivui(knn_win_count_011, knn_count_011), MKSFMTD_030).
			Space().CatEq("knn-111-win-rate", fdivui(knn_win_count_111, knn_count_111), MKSFMTD_030);
		f_out_knn.WriteLine(line_buf.CR());
		{
			CP cplist[] = {
				{ -1, PPObjTimeSeries::TsDensityMapEntry::factorADF },
