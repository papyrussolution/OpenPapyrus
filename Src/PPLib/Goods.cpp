...
int SLAPI GoodsCore::RemoveBarcodeLeadingZeros(int use_ta)
{
	...
	if(code_list_to_update.getCount()) {
		...
		for(uint i = 0; i < code_list_to_update.getCount(); i++) {
			STRNSCPY(k0.Code, code_list_to_update.at(i).Txt);
...
private:
	...
	struct StockExt {
		...
		TSArray <GoodsStockExt::Pallet> PltList;
	};
	...
	if(!ok) {
		...
		if(!ok) {
			...
			if(r > 0) {
				...
				ext.MinShippmQtty = temp.MinShippmQtty; // @v7.2.7
		...
		if(!ok) {
			...
			if(!ok) {
				...
				if(r > 0) {
					...
					SbcList.ExcTable.Remove((ulong)goodsID); // @v7.4.3
					ok = 1;
				}
				else {
					SbcList.Remove(goodsID); // @v7.4.3
