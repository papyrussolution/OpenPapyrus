		...
		*/
		{
			...
			for(SEnum en = wb_obj.P_Tbl->EnumByType(PPWBTYP_KEYWORD, 0); en.Next(&wb_rec) > 0;) {
				...
				{
					context_buf.Z().CatChar('@').CatEq("kws", wb_rec.Symb).Semicol().CatEq("locn", (temp_buf = "чебоксары").Transf(CTRANSF_UTF8_TO_OUTER));
					if(PPGenerateKeywordSequence(context_buf, word_buf, 0) > 0) {
						temp_buf.Z().Cat(context_buf).Tab().Cat(word_buf);
					}
					else {
...
uint AutotranslCache::AddSrcText(const char * pText)
{
	...
	if(!isempty(pText)) {
		...
		if(r_temp_buf.IsLegalUtf8()) {
			uint   ex_id = 0;
			if(Ht_SrcText.Search(r_temp_buf, &ex_id, 0)) {
				result = ex_id;
			}
			else if(Ht_SrcText.Add(r_temp_buf, (MaxHtSrcId+1))) {
				result = (++MaxHtSrcId);
			}
		}
	}
...
uint AutotranslCache::GetSrcText(const char * pText) const
{
	uint   result = 0;
	if(!isempty(pText)) {
		SString & r_temp_buf = SLS.AcquireRvlStr();
					...
					if(line_buf.SearchChar('^', &sp) && sp > 0) {
						if(last_src_text.NotEmpty()) {
							...
							if(transl_text.NotEmpty()) {
								...
								assert(lang_id);
								if(lang_id) {
									uint   src_text_id = GetSrcText(last_src_text);
									if(!src_text_id) {
...
int TestAutotranslateCache()
{
	...
	if(p_ss) {
		...
		if(p_list0 && p_list0->List.getCount()) {
			LongArray dup_src_list_idx_list;
			...
			if(lang_id_list.getCount()) {
				...
				{ // Формируем кэш
					for(uint i = 0; i < p_list0->List.getCount(); i++) {
						...
						org_text = item.Txt;
						uint   src_str_cache_id = cache.GetSrcText(org_text);
						if(src_str_cache_id) {
							dup_src_list_idx_list.add(i+1);
						}
						for(uint li = 0; li < lang_id_list.getCount(); li++) {
							...
							if(p_ss_item && p_ss_item->Get(item.Id, transl_text) && transl_text.NotEmpty()) {
								if(transl_text.IsEqiAscii("error-autotranslation")) {
									debug_mark = true;
								}
								if(!src_str_cache_id) {
								...
								if(src_str_cache_id) {
									if(!cache.GetTranslation(org_text, lang_id, transl_text2)) {
						...
						for(uint li = 0; li < lang_id_list.getCount(); li++) {
							...
							if(p_ss_item && p_ss_item->Get(item.Id, transl_text) && transl_text.NotEmpty()) {
								...
								if(transl_text2 != transl_text) {
									//
									// Так как я тестирую все это на вполне реальном строковом ресурсе, то там есть дубликаты исходных строк 
									// и, причем, иногда с разными переводами. Потому не фиксируем ошибку если ранее исходная строка уже встречалась.
									//
									if(!dup_src_list_idx_list.lsearch(i+1))
										err_count++; // @error
								}
							}
						}
...
//
	...
	*/ 
	...
	public:
		...
		public:
			struct Entry {
				...
				{
					int    ok = 1;
					THROW_SL(pSCtx->Serialize(dir, TokenId, rBuf));
					THROW_SL(pSCtx->Serialize(dir, &DocRefList, rBuf));
					CATCHZOK
					return ok;
				}
		...
		public:
			...
			void   SortList();
			float  EvaluateTfIdfNorm() const;
			int    Serialize(int dir, SBuffer & rBuf, SSerializeContext * pSCtx);

			uint64 ID;   // @firstmember
			float  Norm; // Норма вектора (для косинусного сходства). Рассчитывается методом EvaluateTfIdfNorm()
	...
	PrcssrTextSimilarityFinder::SourceTCollection::SourceTCollection(int dummy, const char * pRdFileName, const char * pWrFileName) : 
	...
	PrcssrTextSimilarityFinder::SourceTCollection::HBlock & PrcssrTextSimilarityFinder::SourceTCollection::HBlock::Z()
	{
	...
	PrcssrTextSimilarityFinder::SourceTCollection::SBlock::SBlock() : Signature(BSignature), Kind(0), Size(0)
	{
	}
				
	PrcssrTextSimilarityFinder::SourceTCollection::SBlock & PrcssrTextSimilarityFinder::SourceTCollection::SBlock::Z()
	{
	...
	int PrcssrTextSimilarityFinder::SourceTCollection::ReadFilePrefix(HBlock & rH)
	{
		rH.Z();
		int    ok = 1;
		THROW(!(State & stError));
	...
	int PrcssrTextSimilarityFinder::SourceTCollection::ReadBlockPrefix(SBlock & rBlk)
	{
		rBlk.Z();
		int    ok = 1;
		THROW(!(State & stError));
	...
	// Поиск k ближайших соседей
	int PrcssrTextSimilarityFinder::SourceTCollection::FindKNearest(const TSVector <SourceTEntry::TokenItem> & rPattern, uint k, float threshold, RAssocArray & rResult) const
	{
		int    ok = 0;
		for(uint i = 0; i < getCount(); i++) {
	...
	const PrcssrTextSimilarityFinder::SourceTEntry * PrcssrTextSimilarityFinder::SourceTCollection::GetEntryByID(uint64 id) const
	{
		uint   pos = 0;
		return lsearch(&id, &pos, CMPF_UINT64) ? at(pos) : 0;
	}
			
	int PrcssrTextSimilarityFinder::SourceTCollection::ReadTokenList(ReadIterator & rIter)
	{
		int    ok = 1;
	...
	//
	// Descr: Вычисление нормы вектора TF-IDF
	//
	float PrcssrTextSimilarityFinder::SourceTEntry::EvaluateTfIdfNorm() const
	{
		float norm_squared = 0.0f;
		for(uint i = 0; i < TokList_.getCount(); i++) {
			const TokenItem & r_item = TokList_.at(i);
			norm_squared += (r_item.TF_IDF * r_item.TF_IDF);
		}
		...
		float  norm2 = 0.0f;
		{
			for(uint i = 0; i < rVec1.getCount(); i++) {
				...
				if(r_t1.TF_IDF != 0.0f) {
					...
					uint   pos2 = 0;
					if(rVec2.bsearch(&r_t1.Id, &pos2, PTR_CMPFUNC(uint16))) { // списки rVec1 и rVec2 должны быть отсортированы по возрастанию //
	...
	PrcssrTextSimilarityFinder::PrcssrTextSimilarityFinder() : State(0)
	{
	}
		
	int	PrcssrTextSimilarityFinder::InitParam(Param * pP)
	{
		int    ok = 1;
		if(pP) {
		}
	...
	int	PrcssrTextSimilarityFinder::EditParam(Param * pP)
	{
		int    ok = 1;
		if(pP) {
		}
	...
	int	PrcssrTextSimilarityFinder::Init(const Param * pP)
	{
		int    ok = 1;
		bool   debug_mark = false; // @debug
		State &= ~stInitialized;
	...
	int	PrcssrTextSimilarityFinder::Run()
	{
		int    ok = 1;
		return ok;
	}
	...
	int PrcssrTextSimilarityFinder::Search(const char * pPattern)
	{
		...
		if(!isempty(pPattern)) {
			...
			pattern.Utf8ToLower();
			{
				PPGetFilePath(PPPATH_LOG, "PrcssrTextSimilarityFinder-construction.log", log_file_path);
				{
						...
						*/ 
							...
							do {
								...
								search_result.SortByValRev();
								{
									for(uint i = 0; i < search_result.getCount(); i++) {
										const  RAssoc & r_sr_item = search_result.at(i);
										const  SourceTEntry * p_se = stc.GetEntryByID(r_sr_item.Key);
										if(p_se && p_se->OrgTextUtf8.NotEmpty()) {
											if(!text_list.Search(r_sr_item.Key, 0))
												text_list.AddFast(r_sr_item.Key, p_se->OrgTextUtf8);
										}
									}
								}
						...
						THROW(token_block_count); // @debug
						{
							for(uint i = 0; i < search_result.getCount(); i++) {
								...
								temp_buf.Z();
								uint   pos = 0;
								if(text_list.Search(r_sr_item.Key, &pos)) {
									temp_buf = text_list.at_WithoutParent(pos).Txt;
								}
								temp_buf.Tab().Cat(r_sr_item.Key).Tab().Cat(r_sr_item.Val);
								PPLogMessage(log_file_path, temp_buf, LOGMSGF_TIME|LOGMSGF_UTF8);
							}
						}
					}
				}
						...
						if(!isempty(sl_item.Txt)) {
							...
							if(enc_status.ok()) {
								...
								if(p_tentry) {
									...
									p_tentry->SortList();
									p_tentry->OrgTextUtf8 = sl_item.Txt;
						...
						do {
							...
							for(uint i = 0; i < stc.getCount(); i++) {
								...
								if(p_item) {
									...
									p_item->Norm = p_item->EvaluateTfIdfNorm();
									// @construction (построение обратного индекса) {
									{
										/*
											...
											private:
												struct DocumentInfo {
													std::string text;
													float norm; // Норма вектора (для косинусного сходства)
													std::unordered_map<int, float> tf_idf; // TF-IDF вектор
												};
												std::vector<DocumentInfo> documents;
												// Инвертированный индекс: token_id -> список документов с этим токеном
												std::unordered_map<int, std::vector<std::pair<size_t, float>>> inverted_index;
    										public:
												void BuildIndex(const std::vector<std::string>& strings) 
												{
													documents.clear();
													inverted_index.clear();
													// 1. Построение документов
													for(const auto& str : strings) {
														DocumentInfo doc;
														doc.text = str;
														doc.tf_idf = CalculateTFIDF(str);
														doc.norm = CalculateNorm(doc.tf_idf);
														documents.push_back(std::move(doc));
													}
													// 2. Построение инвертированного индекса
													for (size_t doc_id = 0; doc_id < documents.size(); ++doc_id) {
														for (const auto& [token, weight] : documents[doc_id].tf_idf) {
															inverted_index[token].emplace_back(doc_id, weight);
														}
													}
												}
												// Быстрый поиск с использованием инвертированного индекса
										...
										*/ 
										for(uint tidx = 0; tidx < p_item->TokList_.getCount(); tidx++) {
											...
											if(p_ii_entry) {
												...
												p_ii_entry->DocRefList.insert(&new_ref_item);
											}
										}
									}
									// } @construction 
	...
	PrcssrTextSimilarityFinder::TokenStatEntry::TokenStatEntry() : TokenId(0), DbEntryCount(0), IDF(0.0f)
	{
	}

	PrcssrTextSimilarityFinder::SourceTEntry::TokenItem::TokenItem() : Id(0), Count(0), TF_IDF(0.0f)
	{
	}
	
	PrcssrTextSimilarityFinder::SourceTEntry::SourceTEntry() : ID(0), Norm(0.0f)
	{
	}

	PrcssrTextSimilarityFinder::SourceTEntry::SourceTEntry(const SourceTEntry & rS) : ID(rS.ID), Norm(rS.Norm), TokList_(rS.TokList_)
	{
	}
	...
	void PrcssrTextSimilarityFinder::SourceTEntry::SortList()
	{
		TokList_.sort2(PTR_CMPFUNC(uint16));
	}
	...
	int PrcssrTextSimilarityFinder::SourceTEntry::Serialize(int dir, SBuffer & rBuf, SSerializeContext * pSCtx)
	{
		...
		THROW_SL(pSCtx->Serialize(dir, Norm, rBuf));
		THROW_SL(pSCtx->Serialize(dir, &TokList_, rBuf));
		THROW_SL(pSCtx->Serialize(dir, OrgTextUtf8, rBuf));
		CATCHZOK
		return ok;
	}
	//
	...
	int SentencePieceExperiments()
	{
		...
		if(p_dict) {
			...
			if(action == actionMakeGoodsNameList) {
				...
				param.SourceFilePath = raw_input_file_path;
				if(tsf.Init(&param)) {
					//const char * p_pattern = "Крем-гель для душа Dove 250ml миндальное молочко и гибискус /12";
					const char * p_pattern = "Крем-гель для душа Dove 250 мл миндальное молочко и гибискус /12";
					if(tsf.Search(p_pattern)) {
						;
					}
				}
			}
...
//
//
PhoneNumberMetaData::ValidationResult::ValidationResult() : State(0), TerrIdx(0), NumberDescrIdx(0), UedPhoneNumberKind(0ULL), Confidence(0.0f)
{
	CarrierName[0] = 0;
}

PhoneNumberMetaData::PhoneNumberMetaData() : State(0)
{
}
...
int PhoneNumberMetaData::MakeValidationResultText(const ValidationResult & rR, SString & rBuf) const
{
	...
	if(!isempty(rR.CarrierName)) {
		rBuf.CatEq("Carrier", rR.CarrierName).Space();
	}
	...
	THROW(p_nd);
	if(PhoneNumberMetaData::GetNumberDescriptorSymb(p_nd->UedPhoneNumberKind, temp_buf)) {
...
int PhoneNumberMetaData::PreprocessPatterns()
{
	...
	for(uint i = 0; i < L.getCount(); i++) {
		...
		if(p_item) {
			...
			{
				struct KindAttr {
					uint64 UedPhoneNumberKind;
					long   Priority;
					float  Confidence;
				};
				static const KindAttr kind_attr_list[] = {
					{ UED_PHONENUMBERKIND_MOBILE,          1, 0.90f },
					{ UED_PHONENUMBERKIND_FIXEDLINE,       2, 0.85f },
					{ UED_PHONENUMBERKIND_VOIP,            3, 0.75f },
					{ UED_PHONENUMBERKIND_PAGER,           4, 0.65f },
					{ UED_PHONENUMBERKIND_UAN,             5, 0.65f },
					{ UED_PHONENUMBERKIND_PREMIUMRATE,     6, 0.70f },
					{ UED_PHONENUMBERKIND_SHAREDCOST,      7, 0.65f },
					{ UED_PHONENUMBERKIND_PERSONAL,        8, 0.65f },
					{ UED_PHONENUMBERKIND_TOLLFREE,        9, 0.80f },
					{ UED_PHONENUMBERKIND_VOICEMAIL,      10, 0.65f },
					{ UED_PHONENUMBERKIND_GENERAL,        11, 0.65f },
					{ 0ULL,     MAXLONG, 0.00f },
				};
					...
					if(p_nd && !p_nd->P_Re) {
						{
							...
							for(uint kai = 0; !kai_found && kai < SIZEOFARRAY(kind_attr_list); kai++) {
								...
								if(r_kai.UedPhoneNumberKind == p_nd->UedPhoneNumberKind) {
...
int PhoneNumberMetaData::ValidateNumber(const char * pRawInput, size_t rawInputLen, TSVector <ValidationResult> * pResultList) const
{
	int    ok = 0;
	//bool   debug_mark = false;
	const  size_t input_len = sstrnlen(pRawInput, rawInputLen); // Вызов sstrnlen страхует от ситуации, когда внтури цепочки символов декларируемой длины присутствует '\0'
	if(input_len) {
		...
		if(checkirangef(_processed_phn.Len32(), 1U, 15U)) { // Быстрая отсечка по общей длине (1-15 цифр для телефонных номеров)
			...
			for(uint ti = 0; (pResultList || !ok) && ti < L.getCount(); ++ti) { // Перебор территорий.
				_phn = _processed_phn;
				_phn_intl.Z(); // Тот же номер телефона в строго международной нотации (если возможно, если нет, то empty)
						...
						if(_phn.C(0) == '+') {
							if(!isempty(p_terr->E164)) {
								...
								if(_phn.HasPrefix(temp_buf)) {
									_phn_intl.Z().Cat(_phn.cptr()+1); // без ведущего '+'
							...
							if(StrPool.GetS(p_terr->NationalPfxP, temp_buf) && temp_buf.NotEmpty() && _phn.HasPrefix(temp_buf)) {
								...
								country_code_tag = 2;
								if(!isempty(p_terr->E164)) {
									_phn_intl.Z().Cat(p_terr->E164).Cat(_phn.cptr() + country_code_len);
								}
							}
										...
										if(p_desc->P_Re && p_desc->P_Re->IsValid()) {
											...
											if(p_desc->P_Re->Find(_phn, _phn.Len(), 0, &fr)) {
												...
												if(fr.getCount() && fr.at(0).low == 0 && fr.at(0).upp == _phn.Len()) {
													/*
														TerrIdx=41 TerrSymb=CG Carrier=Equateur Telecom NdKindSymb=generalDesc Confidence=0.98
														TerrIdx=41 TerrSymb=CG Carrier=Equateur Telecom NdKindSymb=mobile Confidence=0.98
													*/ 
													if(pResultList) {
														ValidationResult new_vr;
														new_vr.State |= ValidationResult::stValid;
														new_vr.TerrIdx = ti+1;
														new_vr.UedPhoneNumberKind = p_desc->UedPhoneNumberKind;
														new_vr.NumberDescrIdx = di+1;
														
														if(_phn_intl.NotEmpty() && CrrPfxS.FindPrefixForCode(_phn_intl, 0, &temp_buf)) {
															assert(temp_buf.NotEmpty());
															STRNSCPY(new_vr.CarrierName, temp_buf);
															new_vr.Confidence = 0.98f; // Если удалось идентифицировать оператора, 
																// то считаем что это - очень вероятный результат. Даже игнорируем штраф за отсутствие + перед международным кодом.
														}
														else {
	...
	if(pResultList && pResultList->getCount()) {
		//
		// Если в списке результатов есть одновременно UED_PHONENUMBERKIND_GENERAL-номер и более конкретный для той де территории и с тем же carrier, 
		// то убираем general-элемент ибо это - дубликат.
		//
		uint i = pResultList->getCount();
		if(i) do {
			const ValidationResult & r_item = pResultList->at(--i);
			if(r_item.UedPhoneNumberKind == UED_PHONENUMBERKIND_GENERAL) {
				bool   do_remove_i = false;
				for(uint j = 0; !do_remove_i && j < pResultList->getCount(); j++) {
					if(j != i) {
						const ValidationResult & r_item2 = pResultList->at(j);
						if(r_item2.UedPhoneNumberKind != UED_PHONENUMBERKIND_GENERAL && r_item2.TerrIdx == r_item.TerrIdx && sstreq(r_item2.CarrierName, r_item.CarrierName)) {
							do_remove_i = true;
						}
					}
				}
				if(do_remove_i) {
					pResultList->atFree(i);
				}
			}
		} while(i);
...
{
	return ValidateNumber(pRawInput, sstrlen(pRawInput), pResultList);
}

static const SIntToSymbTabEntry PhoneNumberMetaData_NumberDescriptorKindSymbList[] = {
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_GENERAL), "generalDesc" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_FIXEDLINE), "fixedLine" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_MOBILE), "mobile" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_PAGER), "pager" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_TOLLFREE), "tollFree" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_PREMIUMRATE), "premiumRate" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_SHAREDCOST), "sharedCost" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_PERSONAL), "personalNumber" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_VOIP), "voip" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_UAN), "uan" },
	{ UED::GetRawValue32(UED_PHONENUMBERKIND_VOICEMAIL), "voicemail" },
};
...
PhoneNumberMetaData::Terr::Terr() : Flags(0), UedStatu(0ULL), IntlPfxP(0), PrefIntlPfxP(0), NationalPfxP(0), NationalPfxForParsingP(0), NationalPfxTransformRuleP(0), PrefExtnPfxP(0)
{
	Id[0] = 0;
	E164[0] = 0;
		...
		for(const SJson * p_cur = pJs->P_Child; p_cur; p_cur = p_cur->P_Next) {
			if(p_cur->Text.IsEqiAscii("id")) {
				...
				if(temp_buf.Len() == 2) {
					...
					UedStatu = p_uedc ? p_uedc->GetBySymb(UED_META_STATU, temp_buf.ToLower()) : 0;
				}
				else
					UedStatu = 0;
			}
				...
				for(uint ndk = 0; ndk < SIZEOFARRAY(PhoneNumberMetaData_NumberDescriptorKindSymbList); ndk++) {
					if(p_cur->Text.IsEqiAscii(PhoneNumberMetaData_NumberDescriptorKindSymbList[ndk].P_Symb)) {
						...
						if(p_new_nd) {
							uint64 ued = UED::ApplyMetaToRawValue32(UED_META_PHONENUMBERKIND, PhoneNumberMetaData_NumberDescriptorKindSymbList[ndk].Id);
							p_new_nd->UedPhoneNumberKind = ued;
...
PhoneNumberMetaData::NumberDescriptor::NumberDescriptor() : P_Re(0), Priority(0), UedPhoneNumberKind(0), PossibleLen_National(0), PossibleLen_LocalOnly(0), 
...
bool PhoneNumberMetaData::Load()
{
	bool   ok = false;
	SString file_path;
	PPGetFilePath(PPPATH_DD, "PhoneNumberMetadata.json", file_path);
	if(ParseJsonFile(file_path)) {
		ok = true;
		{
			PPGetFilePath(PPPATH_DD, "phone-carrier.txt", file_path);
			CrrPfxS.Load(file_path);
		}
	}
	return ok;
}
...
PhoneNumberMetaData::CarrierPrefixSet::CarrierPrefixSet() : TsHt(SKILOBYTE(128))
{
}

int  PhoneNumberMetaData::CarrierPrefixSet::Load(const char * pFileName)
{
	int    ok = -1;
	SFile f_in(pFileName, SFile::mRead);
	if(f_in.IsValid()) {
		...
		SString carrier;
		while(f_in.ReadLine(line_buf, SFile::rlfChomp|SFile::rlfStrip)) {
			if(line_buf.Divide('|', prefix, carrier) > 0) {
				const  long pfx_value = prefix.ToLong();
				if(pfx_value > 0 && carrier.NotEmptyS()) {
					if(TsHt.Put(pfx_value, carrier)) {
						ok = 1;
					}
				}
			}
...
bool PhoneNumberMetaData::CarrierPrefixSet::FindPrefixForCode(const char * pCode, SString * pPrefix, SString * pCarrierSymb) const
{
	CALLPTRMEMB(pPrefix, Z());
	CALLPTRMEMB(pCarrierSymb, Z());
	bool   ok = false;
	const  size_t code_len = sstrlen(pCode);
	char   prefix_buf[32];
	for(size_t prefix_len = 9; !ok && prefix_len >= 3; prefix_len--) {
		if(prefix_len < code_len) {
			memcpy(prefix_buf, pCode, prefix_len);
			prefix_buf[prefix_len] = 0;
			const  long pfx_value = satoi(prefix_buf);
			if(pfx_value > 0) {
				if(TsHt.Get(pfx_value, pCarrierSymb)) {
					ASSIGN_PTR(pPrefix, prefix_buf);
					ok = true;
				}
			}
		}
...
/*static*/int PhoneNumberMetaData::MakeCarrierSourceData(const char * pSrcDir, const char * pDestFilePath) // @v12.6.3 @construction
{
	int    ok = -1;
	SFile * p_f_dest = 0;
	if(!isempty(pDestFilePath) && SFile::IsDir(pSrcDir)) {
		SString temp_buf;
		SString prefix;
		SString carrier;
		SDirEntry de;
		SFsPath ps;
		(temp_buf = pSrcDir).SetLastSlash().Cat("*.*");
		for(SDirec sd(temp_buf, 0); sd.Next(&de) > 0;) {
			if(de.IsFile()) {
				de.GetNameUtf8(pSrcDir, temp_buf);
				ps.Z().Split(temp_buf);
				if(ps.Nam.IsDec() && ps.Ext.IsEqiUtf8("txt")) {
					SFile f_in(temp_buf, SFile::mRead);
					if(f_in.IsValid()) {
						while(f_in.ReadLine(temp_buf, SFile::rlfChomp|SFile::rlfStrip)) {
							if(temp_buf.NotEmpty() && !temp_buf.HasPrefix("#")) {
								if(temp_buf.Divide('|', prefix, carrier) > 0) {
									if(prefix.Strip().IsDec() && carrier.Strip().IsLegalUtf8()) {
										if(!p_f_dest) {
											p_f_dest = new SFile(pDestFilePath, SFile::mWrite);
											THROW_SL(p_f_dest && p_f_dest->IsValid());
										}
										assert(p_f_dest);
										temp_buf.Z().Cat(prefix).CatChar('|').Cat(carrier).CR();
										THROW_SL(p_f_dest->WriteLine(temp_buf));
										ok = 1;
									}
								}
							}
...
int TestPhoneNumberMetadata() // test
{
	...
	/*{
		const char * p_src_dir = "D:/Papyrus/Src/Rsrc/Data/phone-carrier/en";
		PPGetFilePath(PPPATH_OUT, "phone-carrier.txt", temp_buf);
		r = PhoneNumberMetaData::MakeCarrierSourceData(p_src_dir, temp_buf); // @v12.6.3 @construction
	}
