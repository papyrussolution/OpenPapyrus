...
static IMPL_DBE_PROC(dbqf_unxtext_iii)
{
	...
	if(!DbeInitSize(option, result, sizeof(text_buf))) {
        ...
        SString & r_temp_buf = SLS.AcquireRvlStr();
		PPRef->UtrC.SearchUtf8(SObjTextRefIdent(obj_type, obj_id, static_cast<int16>(text_prop)), r_temp_buf);
...
static IMPL_DBE_PROC(dbqf_objname_loc_i)
{
	...
	if(!DbeInitSize(option, result, sizeof(rec.Name))) {
		...
		result->InitForeignStr(rec.Name);
	}
}
...
static IMPL_DBE_PROC(dbqf_objname_accsheet_i) { dbqf_objname_i <PPObjAccSheet, PPAccSheet2> (option, result, params); }
static IMPL_DBE_PROC(dbqf_objname_cashnode_i) { dbqf_objname_i <PPObjCashNode, PPCashNode2> (option, result, params); }
...
static IMPL_DBE_PROC(dbqf_ufpfuncname_i)
{
	...
	if(!DbeInitSize(option, result, sizeof(name))) {
		const long id = PPDbqFuncPool::helper_dbq_name(params, name);
		if(id) {
			uint16 func_ver = 0;
...
static IMPL_DBE_PROC(dbqf_objmemo_bill_i)
{
	...
	if(!DbeInitSize(option, result, sizeof(name_buf))) {
		...
		if(id) {
			...
			SString & r_temp_buf = SLS.AcquireRvlStr();
			if(id) {
				PPRef->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_BILL, id, PPTRPROP_MEMO), r_temp_buf);
				r_temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
			}
			STRNSCPY(name_buf, r_temp_buf);
			// } @v11.1.12 
...
static IMPL_DBE_PROC(dbqf_billmemosubstr_is) // @v11.7.4
{
	int    local_result = 0;
	const  PPID id = params[0].lval;
	const  char * p_pattern = params[1].sptr;
	if(isempty(p_pattern))
		local_result = 1; // Считаем, что пустая подстрока содержится в любой строке
	else {
		SString & r_temp_buf = SLS.AcquireRvlStr();
		if(id) {
			PPRef->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_BILL, id, PPTRPROP_MEMO), r_temp_buf);
...
static IMPL_DBE_PROC(dbqf_objmemo_person_i) // @v11.1.12
{
	...
	if(!DbeInitSize(option, result, sizeof(name_buf))) {
		...
		if(id) {
			SString & r_temp_buf = SLS.AcquireRvlStr();
			if(id) {
				PPRef->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_PERSON, id, PPTRPROP_MEMO), r_temp_buf);
				r_temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
			}
			STRNSCPY(name_buf, r_temp_buf);
		}
...
static IMPL_DBE_PROC(dbqf_objmemo_personevent_i) // @v11.1.12
{
	...
	if(!DbeInitSize(option, result, sizeof(name_buf))) {
		...
		if(id) {
			SString & r_temp_buf = SLS.AcquireRvlStr();
			if(id) {
				PPRef->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_PERSONEVENT, id, PPTRPROP_MEMO), r_temp_buf);
				r_temp_buf.Transf(CTRANSF_UTF8_TO_INNER);
			}
			STRNSCPY(name_buf, r_temp_buf);
		}
...
static IMPL_DBE_PROC(dbqf_tacost_rr) { result->init(R2(fdivnz(params[1].rval, params[0].rval))); /*params[0] - qtty; params[1] - cost*/ }
...
static IMPL_DBE_PROC(dbqf_idobjtitle_i)
{
	...
	if(!DbeInitSize(option, result, sizeof(buf))) {
		SString & r_temp_buf = SLS.AcquireRvlStr();
		DS.GetObjectTitle(params[0].lval, r_temp_buf);
...
static IMPL_DBE_PROC(dbqf_sysjaction_i)
{
	char   buf[64];
	if(!DbeInitSize(option, result, sizeof(buf))) {
		PPLoadStringS(PPSTR_ACTION, params[0].lval, SLS.AcquireRvlStr()).CopyTo(buf, sizeof(buf));
		result->init(buf);
	}
}
...
static IMPL_DBE_PROC(dbqf_gtajaction_i)
{
	char   buf[64];
	if(!DbeInitSize(option, result, sizeof(buf))) {
		PPLoadStringS(PPSTR_GTA, params[0].lval, SLS.AcquireRvlStr()).CopyTo(buf, sizeof(buf));
		result->init(buf);
	}
}
...
static IMPL_DBE_PROC(dbqf_yeswordbyflag_i)
{
	...
	if(!DbeInitSize(option, result, sizeof(buf))) {
		const long flags = params[0].lval;
		const long flag  = params[1].lval;
		const  char * p_out_symb = params[2].sptr;
		if(flags & flag) {
...
int PPDbqFuncPool::IdClientActivityStatisticsIndicator = 0; // @v12.2.2 (personID, actualDate, indicator/*PPObjPerson::casiXXX*/) Возвращает значение индикатора статистики клиентской активности
