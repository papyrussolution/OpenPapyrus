...
/*static*/SString & FASTCALL PPObjBill::MakeCodeString(const BillTbl::Rec * pRec, int options, SString & rBuf)
{
	SString name;
	rBuf.Z();
	rBuf.Cat(pRec->Dt, DATF_DMY|DATF_CENTURY).CatDiv('-', 1).Cat(pRec->Code);
	...
	CCheckPacket & rCp, CcTotal & rCcT, TSCollection <SCompoundError> * pErrList) const // @v12.7.3
{
	...
	const  bool do_insert_chzn_marks_into_cc = false; //is_return; // @v12.7.4 true-->false // @temporary(false-->is_return)
	{
		...
		for(uint tiidx = 0; tiidx < GetTCount(); tiidx++) {
			...
			Goods2Tbl::Rec goods_rec;
			PPGoodsType2 gt_rec;
			LTagL.GetString(PPTAG_LOT_SN, tiidx, serial);
			...
			if(goods_obj.Fetch(r_ti.GoodsID, &goods_rec) > 0) {
				...
				if(qtty_ > 0.0) {
					...
					rCcT.Discount += R2(r_ti.Discount * qtty_);
					ok = 1;
				}
			}
		}
	...
	THROW(rParam.PosNodeID && cn_obj.GetSync(rParam.PosNodeID, &cn_rec) > 0);
	{
		...
		if(oneof3(OpTypeID, PPOPT_GOODSEXPEND, PPOPT_GOODSRECEIPT, PPOPT_GOODSRETURN) || IsDraft()) {
			if(CheckOpPrnFlags(Rec.OpID, OPKF_PRT_CHECKTI)) {
				THROW(Helper_ConvertToCheck2_InsertTItems(rParam, cn_rec, cp, cct, pErrList));
			}
				...
				cct.Amount += R2(n_pr * qtty);
			}
			if(cp.GetCount()) {
				LDBLTOMONEY(cct.Amount, cp.Rec.Amount);
				LDBLTOMONEY(cct.Discount, cp.Rec.Discount);
				cp._Cash = cct.Amount;
				if(is_return)
			...
			if(amt != 0.0) {
				...
				{
					if(CheckOpPrnFlags(Rec.OpID, OPKF_PRT_CHECKTI)) {
						if(mult == 1.0) { // Если оплата в точности совпадает с суммой документа, то чек строим так, словно по самому документу отгрузки (с марками и прочим смаком)
							THROW(link_pack.Helper_ConvertToCheck2_InsertTItems(rParam, cn_rec, cp, cct, pErrList));
						}
						else {
							for(uint tiidx = 0; tiidx < link_pack.GetTCount(); tiidx++) {
								const  PPTransferItem & r_ti = link_pack.ConstTI(tiidx);
								const  double qtty = R6(fabs(r_ti.Quantity_) * mult);
								const  double n_pr = r_ti.NetPrice();
								THROW(cp.InsertCclSimple(r_ti.GoodsID, qtty, n_pr, 0.0, rParam.DivisionN));
								cct.Amount += R2(n_pr * qtty);
								cct.Discount += R2(r_ti.Discount * qtty);
							}
						}
					}
					else if(prepay_goods_id) {
					...
					if(cp.GetCount()) {
						CcTotal result_cct;
						cp.CalcAmount(&result_cct);
						LDBLTOMONEY(result_cct.Amount, cp.Rec.Amount);
						LDBLTOMONEY(result_cct.Discount, cp.Rec.Discount);
						...
						if(rParam.PaymType == cpmBank)
							cp.Rec.Flags |= CCHKF_BANKING;
						if(!feqeps(fabs(result_cct.Amount), fabs(cc_req_amount), 1E-8)) {
							double fixup_discount = result_cct.Discount + (fabs(result_cct.Amount) - fabs(cc_req_amount));
							cp.SetTotalDiscount__(fabs(fixup_discount), (fixup_discount < 0.0) ? CCheckPacket::stdfPlus : 0);
							cp.CalcAmount(&result_cct);
							LDBLTOMONEY(result_cct.Amount, cp.Rec.Amount);
							LDBLTOMONEY(result_cct.Discount, cp.Rec.Discount);
						}
						cp._Cash = result_cct.Amount;
						ok = 1;
					}
				}
			}
		...
		if(_mode && (!(bill_rec.Flags & BILLF_CHECK) || (PPMaster && PPMessage(mfConf|mfYesNo, PPCFM_BILLCHECKED) == cmYes))) {
			...
			if(_EditCcByBillParam(param) > 0) {
				...
				if(_mode == 2) { // correction
					...
					fc.Dt = pack.Rec.Dt;
					fc.Code = pack.Rec.Code;
					fc.Reason = pack.SMemo;
			...
			if(is_there_limited_goods) {
				...
				//{
					...
					//{
						...
						//(rcpt_bpack.SMemo = "@autoreceipt").Space().Cat(temp_buf);
					//}
				//}
...
int PPObjBill::GetLabelLotInfo(PPID lotID, RetailGoodsInfo * pData)
{
	...
	if(trfr->Rcpt.Search(lotID, &lot_rec) > 0) {
		...
		if(Search(lot_rec.BillID, &bill_rec) > 0) {
			...
			STRNSCPY(rgi.BillCode, bill_rec.Code);
			if(bill_rec.Object) {
		...
		if(is_quot && CONFIRM(PPCFM_SETPRICEBYQUOT)) {
			...
			if(p_qbo_ary->getCount() > 1) {
				...
				THROW_MEM(def);
				{
					...
					if(p_combo) {
						THROW_MEM(p_combo->setListWindow(new ListWindow(def, 0), qk_id));
					}
					while(!valid_data && ExecView(dlg) == cmOK) {
...
int PPObjBill::Helper_StoreClbList(PPBillPacket * pPack)
{
	...
	PPObjTag * p_tag_obj = 0;
	const  bool is_intrexpnd = IsIntrExpndOp(pPack->Rec.OpID);
	const  int do_force_unmirr = pPack->SMemo.Search("#MIRROR-REFAB", 0, 0, 0);
	SString img_path;
	SString img_tag_addendum;
	SString fname;
	SString temp_buf;
					...
					switch(sym) {
						...
						case PPSYM_TSESS:
							if(pPack->CSessID && pPack->Rec.Flags & BILLF_TSESSWROFF) {
								...
								const  int r = p_tses_obj->GetPacket(pPack->CSessID, &tses_pack, 0);
								if(r > 0) {
									if(p[next] == '.') {
										p_tsess_pack = &tses_pack;
										p += (next+1);
										next = 0;
										sym = st.Translate(p, &next);
									}
									else {
										p_tses_obj->MakeName(&tses_pack.Rec, subst_buf.Z());
...
int PPObjBill::ConvertGenAccturnToExtAccBill(PPID srcID, PPID * pDestID, const CvtAt2Ab_Param * pParam, int use_ta)
{
	...
	PPAccTurn * p_at = 0;
	{
		...
		dest_pack.SMemo = src_pack.SMemo;
