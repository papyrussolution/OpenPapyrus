	...
	State2(0) /*, DemoRestrict(-1)*/
{
	...
	TLP_OPEN(P_Tbl);
	TLP_OPEN(trfr);
	// @v9.8.11 if(CcFlags & CCFLG_USEDRAFTBILL)
		TLP_OPEN(P_CpTrfr);
...
int SLAPI PPObjBill::InsertShipmentItemByOrder(PPBillPacket * pPack, const PPBillPacket * pOrderPack, int orderItemIdx, int interactive)
{
	int    ok = -1;
	...
	if(loc_id) {
		...
		if(pParam->Flags & pParam->fNonInteractive) {
			for(uint i = 0; i < sample_pack.GetTCount(); i++) {
				THROW(InsertShipmentItemByOrder(&pack, &sample_pack, i, 0 /*noninteractive*/));
			}
		}
		else {
			res = Helper_EditGoodsBill(pBillID, &pack);
...
struct __PPBillConfig {    // @persistent @store(PropertyTbl)
	...
	uint8  Reserve2[20];       // @reserve @v8.4.11 [28]-->[24]
};
...
SString & SLAPI PPObjBill::MakeLotText(const ReceiptTbl::Rec * pLotRec, long fmt, SString & rBuf)
{
	BillTbl::Rec bill_rec;
	SString temp_buf;
	rBuf = "LOT";
...
//
//
int SLAPI PPObjBill::SelectQuotKind(PPBillPacket * pPack, const PPTransferItem * pTi, int interactive, double * pQuot)
{
...
//
//
SLAPI PPObjBill::SubstParam::SubstParam() : P_DebtDimAgentList(0)
{
}
...
int SLAPI PPObjBill::ProcessShadowPacket(PPBillPacket * pPack, int doUpdate)
{
	int    ok = 1, r;
	uint   pos;
	ReceiptTbl::Rec lot_rec;
	...
	if(fl & BPLD_SKIPTRFR) {
		...
		if(pPack->IsDraft()) {
			if(P_CpTrfr) {
				...
				THROW(LoadClbList(pPack, 2)); // @v8.5.5 // @v10.0.0 force 0-->2
			}
		}
		else {
