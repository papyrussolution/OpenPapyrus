...
int SLAPI PPEgaisProcessor::ReadInput(PPID locID, const DateRange * pPeriod, long flags)
{
	...
	if(reply_count) {
		...
		ediop_list.addzlist(PPEDIOP_EGAIS_WAYBILL, PPEDIOP_EGAIS_WAYBILL_V2, PPEDIOP_EGAIS_TTNINFORMBREG, PPEDIOP_EGAIS_TTNINFORMF2REG,
			...
			PPEDIOP_EGAIS_REPLYFORMA, PPEDIOP_EGAIS_REPLYRESTSSHOP, PPEDIOP_EGAIS_REPLYBARCODE, PPEDIOP_EGAIS_WAYBILLACT_V2, 0);
					...
					if(p_pack->DocType == PPEDIOP_EGAIS_REPLYRESTS) {
						...
						if(!do_skip) {
							...
							for(uint j = 0; !_bill_found && j < bill_id_list.getCount(); j++) {
								...
								if(do_process == 3) {
									/* @v9.0.4 Ничего не делаем по справке Б для собственного документа
