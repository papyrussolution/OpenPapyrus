...
SLAPI PPEgaisProcessor::PPEgaisProcessor(long cflags, PPLogger * pOuterLogger, int __reserve) : PrcssrAlcReport(), State(0), P_LecT(0), P_Logger(0), P_UtmEntry(0), P_Taw(0)
{
	{
		//
			...
			}
			{
				SXml::WNode n_d(_doc, "ns:Document");
				{
					...
					if(oneof3(doc_type, PPEDIOP_EGAIS_WAYBILL, PPEDIOP_EGAIS_WAYBILL_V2, PPEDIOP_EGAIS_WAYBILL_V3)) { // @v9.9.5 PPEDIOP_EGAIS_WAYBILL_V3
						...
						//SXml::WNode n_iv(_doc, "ns:InfoVersionTTN");
						n_dt.PutInner("qp:ClientId", EncText(fsrar_ident));
						n_dt.PutInner("qp:WBTypeUsed", EncText(temp_buf = "WayBill_v3"));
					}
