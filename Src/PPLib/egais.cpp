...
static SString & Egais_GetBillCode(const BillTbl::Rec & rRec, SString & rBuf)
{
	...
	/* @v12.7.4 
	//
	// Когда-то давно эта функция понадобилась для обработки номеров документов егаис, которые не помещались в штатный размер BillTbl::Rec::Code.
	// Эта проблема была потом решена увеличением длины BillTbl::Rec::Code, потому извлечение специального длинного наименования комментируется за давностью лет.
	//
...
PPEgaisProcessor::Packet::Packet(int docType) : DocType(docType), Flags(0), IntrBillID(0), P_Data(0), SrcReplyPos(0)
{
	switch(DocType) {
		...
		case PPEDIOP_EGAIS_QUERYFORMF1:
		case PPEDIOP_EGAIS_QUERYFORMF2:
...
int PPEgaisProcessor::QueryInfA(PPID locID, const char * pInfA)
{
	int    ok = -1;
	Ack    ack;
    Packet qp(/*PPEDIOP_EGAIS_QUERYFORMA*/PPEDIOP_EGAIS_QUERYFORMF1);
	if(qp.P_Data) {
        *static_cast<SString *>(qp.P_Data) = pInfA;
	}
	THROW(PutQuery(qp, locID, /*"QueryFormA"*/"QueryFormF1", ack));
	CATCHZOK
    return ok;
}
...
int PPEgaisProcessor::QueryInfB(PPID locID, const char * pInfB)
{
	int    ok = -1;
	Ack    ack;
    Packet qp(/*PPEDIOP_EGAIS_QUERYFORMB*/PPEDIOP_EGAIS_QUERYFORMF2);
	if(qp.P_Data)
		*static_cast<SString *>(qp.P_Data) = pInfB;
	THROW(PutQuery(qp, locID, /*"QueryFormB"*/"QueryFormF2", ack));
	CATCHZOK
    return ok;
}
	...
	State(0), P_LecT(0), P_UtmEntry(0), P_Taw(0)
{
	...
	if(cflags & cfUseVerByConfig) {
		...
		SETFLAG(State, stUseEgaisVer4, BIN(Cfg.E.Flags & Cfg.fEgaisVer4Fmt));
	}
	else {
		SETFLAG(State, stUseEgaisVer3, BIN(cflags & cfVer3));
		SETFLAG(State, stUseEgaisVer4, BIN(cflags & cfVer4));
	}
...
static const SIntToSymbTabEntry _EgaisDocTypes[] = {
	...
	{ PPEDIOP_EGAIS_QUERYFORMF1,      "QueryFormF1" },
	{ PPEDIOP_EGAIS_QUERYFORMF2,      "QueryFormF2" },
	...
	{ PPEDIOP_EGAIS_REPLYFORM1,       "ReplyForm1" },
	{ PPEDIOP_EGAIS_REPLYFORM2,       "ReplyForm2" },
	...
	{ PPEDIOP_EGAIS_NOTIFY_WBVER4,    "InfoVersionTTN" },
	{ PPEDIOP_EGAIS_WAYBILL_V4,       "WayBill_v4" },
	{ PPEDIOP_EGAIS_WAYBILLACT_V4,    "WayBillAct_v4" },
};
...
int PPEgaisProcessor::WriteInformCode(SXml::WDoc & rXmlDoc, const char * pNs, char informKind, SString & rCode, int docType)
{
    ...
    if(oneof2(informKind, 'A', 'B')) {
		int    done = 0;
		SString temp_buf;
		if(oneof2(docType, PPEDIOP_EGAIS_WAYBILL_V3, PPEDIOP_EGAIS_WAYBILL_V4)) {
			if(informKind == 'A') {
		...
		SXml::WDoc _doc(pX, cpUTF8);
		{
			...
			n_docs.PutAttrib("Version", "1.0");
			{
				static const struct EgaisNsEntry {
					...
					{ 34, "wa",   "ActTTNSingle_v4"      }, // ambiguity
					{ 35, "wb",   "TTNSingle_v4"         }, // ambiguity
				};
			...
			}
			{
				SXml::WNode n_d(_doc, SXml::nst("ns", "Document"));
				{
					...
					if(oneof4(doc_type, PPEDIOP_EGAIS_WAYBILL, PPEDIOP_EGAIS_WAYBILL_V2, PPEDIOP_EGAIS_WAYBILL_V3, PPEDIOP_EGAIS_WAYBILL_V4)) {
					...
					else if(doc_type == PPEDIOP_EGAIS_NOTIFY_WBVER4) {
						n_dt.PutInner(SXml::nst("qp", "ClientId"), EncText(fsrar_ident));
						...
						n_dt.PutInner(SXml::nst("awr", "Identity"), temp_buf.Z().Cat(p_bp->Rec.ID));
						{
							...
							if(p_bp->BTagL.GetItemStr(PPTAG_BILL_FORMALREASON, temp_buf) <= 0) {
								PPLoadText(/*PPTXT_EGAIS_LACK*/PPTXT_EGAIS_SALE, temp_buf);
							}
...
int PPEgaisProcessor::Read_IformA(xmlNode * pFirstNode, Packet * pPack, PrcssrAlcReport::RefCollection * pRefC)
{
    ...
    for(const xmlNode * p_n = pFirstNode; p_n; p_n = p_n->next) {
		...
		else if(SXml::IsName(p_n, "OriginalClient")) {
		...
		else if(SXml::GetContentByName(p_n, "OriginalDocNumber", temp_buf)) {
		}
		else if(SXml::GetContentByName(p_n, "OriginalDocDate", temp_buf)) {
		}
		else if(SXml::GetContentByName(p_n, "GTDNUMBER", temp_buf)) {
		}
		else if(SXml::GetContentByName(p_n, "GTDDate", temp_buf)) {
		}
					...
					if(SXml::IsName(p_c, "Position")) {
						...
						if(is_pack_inited && ti.GoodsID && _src_qtty != 0.0) {
							...
							}
							{
								...
								STRNSCPY(rt.GoodsCategory, alc_ext.CategoryCode);
								row_tags.insert(&rt);
							}
						}
					}
			...
			if(pPack->IntrBillID > 0) {
				...
				THROW(P_BObj->ExtractPacket(pPack->IntrBillID, &wroff_pack) > 0);
				{
					...
					if(do_update_wroff_pack) {
						wroff_pack.ProcessFlags |= PPBillPacket::pfIgnoreStatusRestr;
						PPTransaction tra(1);
						THROW(tra);
		...
		for(const xmlNode * p_n = p_root->children; p_n; p_n = p_n->next) {
			if(SXml::IsName(p_n, "Owner")) {
				...
				for(const xmlNode * p_nd = p_n->children; p_nd; p_nd = p_nd->next) {
					...
					else if(oneof2(doc_type, PPEDIOP_EGAIS_REPLYFORMA, PPEDIOP_EGAIS_REPLYFORM1)) {
						THROW(Helper_InitNewPack(doc_type, pPackList, &p_new_pack));
...
int PPEgaisProcessor::SearchActChargeByActInform(const PPEgaisProcessor::ActInform & rInf, PPID * pBillID)
{
    ...
    if(rInf.ActNumber.NotEmpty()) {
		...
		for(DateIter di(encodedate(1, 12, 2015), ZERODATE); ok < 0 && P_BObj->P_Tbl->EnumByOpr(op_id, &di, &bill_rec) > 0;) {
			Egais_GetBillCode(bill_rec, temp_buf);
			...
			if(ediop_list.bsearch(doc_type)) {
				...
				if(adr && Helper_Read(p_ctx, p_reply->AcceptedFileName, 0, locID, pPeriod, (i+1), &pack_list, P_RefC)) {
					if(oneof6(doc_type, PPEDIOP_EGAIS_REPLYCLIENT, PPEDIOP_EGAIS_REPLYAP, PPEDIOP_EGAIS_REPLYRESTS, PPEDIOP_EGAIS_REPLYRESTS_V2, 
						PPEDIOP_EGAIS_REPLYFORMA, PPEDIOP_EGAIS_REPLYFORM1)) {
	...
	}
	{
		...
		for(packidx = 0; packidx < pack_list.getCount(); packidx++) {
			...
			if(p_pack && p_pack->P_Data && !skip_packidx_list.lsearch(static_cast<long>(packidx))) {
				...
				else if(oneof2(p_pack->DocType, PPEDIOP_EGAIS_REPLYFORMA, PPEDIOP_EGAIS_REPLYFORM1)) {
...
int PPEgaisProcessor::SendBillActs(const PPBillIterchangeFilt & rP)
{
	...
	for(uint i = 0; i < accepted_bill_list.getCount(); i++) {
		...
		if(P_BObj->ExtractPacket(bill_id, p_bp) > 0) {
			...
			if(p_bp->Rec.EdiOp == PPEDIOP_EGAIS_WAYBILL_V4) {
	...
	for(uint i = 0; i < accepted_bill_list.getCount(); i++) {
		...
		if(P_BObj->ExtractPacket(bill_id, &bp) > 0) {
			...
			if(url_suffix.NotEmpty()) {
				...
				Egais_GetBillCode(bp, p_rwb->ReqNumber);
...
int PPEgaisProcessor::SendBills(const PPBillIterchangeFilt & rP)
{
	...
	const  int __v4 = BIN(State & stUseEgaisVer4);
	SString file_name;
	SString temp_buf;
			...
			if(P_BObj->Search(bill_id, &bill_rec) > 0) {
				if(PPRef->Ot.GetTagStr(PPOBJ_BILL, bill_rec.ID, PPTAG_BILL_EDIIDENT, reg_ident) > 0) {
					...
					if(oneof2(p_ticket->Conclusion, 0, 1)) {
						Ack ack;
						Egais_GetBillCode(bill_rec, p_ticket->Code);
...
int PPEgaisProcessor::EditQueryParam(PPEgaisProcessor::QueryParam * pData)
{
	...
	public:
		...
		DECL_DIALOG_SETDTS()
		{
			...
			AddClusterAssoc(CTL_EGAISQ_WHAT, 14, PPEDIOP_EGAIS_NOTIFY_WBVER4);
			...
			if(event.isClusterClk(CTL_EGAISQ_WHAT)) {
				...
				if(doc_type == (PPEDIOP_EGAIS_QUERYCLIENTS+2000))
					...
					switch(doc_type) {
						...
						case PPEDIOP_EGAIS_NOTIFY_WBVER4: info_text_id = PPTXT_HINT_EGAIS_NOTIFY_WBVER4; break;
...
int EgaisPersonCore::RecToItem(const EgaisPersonTbl::Rec & rRec, EgaisPersonCore::Item & rItem)
{
	...
	{
		Reference * p_ref(PPRef);
		THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rRec.ID, PPTRPROP_NAME), rItem.Name));
		rItem.Name.Transf(CTRANSF_UTF8_TO_INNER);
		THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rRec.ID, PPTRPROP_LONGNAME), rItem.FullName));
		rItem.FullName.Transf(CTRANSF_UTF8_TO_INNER);
		THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rRec.ID, txtprpAddressDescr), rItem.AddressDescr));
	...
	SString temp_buf;
	{
		...
		if(*pID) {
			...
			if(!pItem) {
				...
				THROW_DB(deleteRec());
				THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_NAME), 0));
				THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_LONGNAME), 0));
				THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, txtprpAddressDescr), 0));
			}
				...
				if(pItem->Name.NotEmptyS()) {
					...
					THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_NAME), temp_buf, 0));
				}
				if(pItem->FullName.NotEmptyS()) {
					(temp_buf = pItem->FullName).Transf(CTRANSF_INNER_TO_UTF8);
					THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_LONGNAME), temp_buf, 0));
				}
				if(pItem->AddressDescr.NotEmptyS()) {
					(temp_buf = pItem->AddressDescr).Transf(CTRANSF_INNER_TO_UTF8);
					THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, txtprpAddressDescr), temp_buf, 0));
				}
			}
		}
		else {
			...
			if(search(1, &k1, spEq)) {
				...
				}
				{
					...
					THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, PPTRPROP_NAME), name));
					name.Transf(CTRANSF_UTF8_TO_INNER);
					THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, PPTRPROP_LONGNAME), full_name));
					full_name.Transf(CTRANSF_UTF8_TO_INNER);
					THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, txtprpAddressDescr), address_descr));
					address_descr.Transf(CTRANSF_UTF8_TO_INNER);
					if(pItem->Name.Len() > name.Len()) {
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, PPTRPROP_NAME), (temp_buf = pItem->Name).Transf(CTRANSF_INNER_TO_UTF8), 0));
						ok = 1;
					}
					if(pItem->FullName.Len() > full_name.Len()) {
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, PPTRPROP_LONGNAME), (temp_buf = pItem->FullName).Transf(CTRANSF_INNER_TO_UTF8), 0));
						ok = 1;
					}
					if(pItem->AddressDescr.Len() > address_descr.Len()) {
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, rec.ID, txtprpAddressDescr), (temp_buf = pItem->AddressDescr).Transf(CTRANSF_INNER_TO_UTF8), 0));
						ok = 1;
					}
				}
				if(do_update) {
				...
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_NAME), (temp_buf = pItem->Name).Transf(CTRANSF_INNER_TO_UTF8), 0));
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, PPTRPROP_LONGNAME), (temp_buf = pItem->FullName).Transf(CTRANSF_INNER_TO_UTF8), 0));
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPERSON, *pID, txtprpAddressDescr), (temp_buf = pItem->AddressDescr).Transf(CTRANSF_INNER_TO_UTF8), 0));
			}
		}
		THROW(tra.Commit());
	}
...
int EgaisPersonCore::Clear(int use_ta)
{
	...
	msg_buf.CatDiv(':', 2).Cat(this->GetTableName());
	{
		...
		if(search(0, &k0, spFirst)) do {
			...
			THROW_DB(rereadForUpdate(0, 0));
			THROW_DB(deleteRec());
			THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, id, PPTRPROP_NAME), 0));
			THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, id, PPTRPROP_LONGNAME), 0));
			THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPERSON, id, txtprpAddressDescr), 0));
...
int EgaisProductCore::RecToItem(const EgaisProductTbl::Rec & rRec, EgaisProductCore::Item & rItem)
{
	...
	{
		Reference * p_ref(PPRef);
		THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rRec.ID, PPTRPROP_NAME), rItem.Name));
		rItem.Name.Transf(CTRANSF_UTF8_TO_INNER);
		THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rRec.ID, PPTRPROP_LONGNAME), rItem.FullName));
		rItem.FullName.Transf(CTRANSF_UTF8_TO_INNER);
	}
	CATCHZOK
	return ok;
	...
	long   conflict_flags = 0;
	{
		...
		if(*pID) {
			...
			if(!pItem) {
				THROW_DB(rereadForUpdate(0, 0));
				THROW_DB(deleteRec());
				THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, *pID, PPTRPROP_NAME), 0));
				THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, *pID, PPTRPROP_LONGNAME), 0));
			}
			else if(pItem->IsEq(org_item, 0)) {
				ok = -1;
			}
				...
				THROW_DB(rereadForUpdate(0, 0));
				THROW_DB(updateRecBuf(&rec));
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, *pID, PPTRPROP_NAME), (temp_buf = pItem->Name).Transf(CTRANSF_INNER_TO_UTF8), 0));
				THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, *pID, PPTRPROP_LONGNAME), (temp_buf = pItem->FullName).Transf(CTRANSF_INNER_TO_UTF8), 0));
			}
		}
		else {
			...
			if(search(1, &k1, spEq)) {
				...
				}
				{
					SString name;
					SString full_name;
					THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rec.ID, PPTRPROP_NAME), name));
					name.Transf(CTRANSF_UTF8_TO_INNER);
					THROW(p_ref->UtrC.SearchUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rec.ID, PPTRPROP_LONGNAME), full_name));
					full_name.Transf(CTRANSF_UTF8_TO_INNER);
					if(pItem->Name.Len() > name.Len()) {
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rec.ID, PPTRPROP_NAME), (temp_buf = pItem->Name).Transf(CTRANSF_INNER_TO_UTF8), 0));
						ok = 1;
					}
					if(pItem->FullName.Len() > full_name.Len()) {
						THROW(p_ref->UtrC.SetTextUtf8(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, rec.ID, PPTRPROP_LONGNAME), (temp_buf = pItem->FullName).Transf(CTRANSF_INNER_TO_UTF8), 0));
						ok = 1;
					}
				}
...
int EgaisProductCore::Clear(int use_ta)
{
	...
	msg_buf.CatDiv(':', 2).Cat(this->GetTableName());
	{
		...
		if(search(0, &k0, spFirst)) do {
			...
			THROW_DB(rereadForUpdate(0, 0));
			THROW_DB(deleteRec());
			THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, id, PPTRPROP_NAME), 0));
			THROW(p_ref->UtrC.Remove(SObjTextRefIdent(PPOBJ_EGAISPRODUCT, id, PPTRPROP_LONGNAME), 0));
			PPWaitPercent(cntr.Increment(), msg_buf);
			ok = 1;
		} while(search(0, &k0, spNext));
		THROW(tra.Commit());
