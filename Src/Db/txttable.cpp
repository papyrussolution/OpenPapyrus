...
int TextDbFile::Scan()
{
	...
	if(F.IsValid()) {
		...
		long   line_no = 0;
		F.Seek(0, SEEK_SET);
		int    is_terminator = 0; // ≈сли !0, то предыдуща€ строка была терминатором вертикальной записи.
		int    is_first_rec = 1;
		uint   vert_fld_no = 0;
		{
			uint8 sign[8];
		...
		while(F.ReadLine(line)) {
			...
			if(line_no > P.HdrLinesCount) {
				...
				if(P.Flags & fVerticalRec) {
					if(is_terminator) {
						//
						// ѕредыдуща€ строка была терминатором.
						// —ледовательно:
					...
					//
					// is_terminator == 2 означает не€вный конец записи по количеству полей
					//
					if(is_terminator != 1) {
						if(is_first_rec) {
							if(P.Flags & fFldNameRec) {
								...
								//
								// ѕерва€ строка (терминатора, естественно, до этого не было):
			...
			if(P.Flags & fFixedFields) {
				...
				for(uint fld_pos = 0; fld_pos < rRec.GetCount(); fld_pos++) {
					if(rRec.GetFieldByPos(fld_pos, &fld) > 0) {
						...
						field_buf.Strip(); // @v5.3.4 Ќе уверен, что это правильный оператор.
							// ¬ общем случае следует использовать поле со всеми пробелами, однако
							// случаи, когда лидирующие и хвостовые пробелы могут быть полезны
							// припомнить не смог, в то врем€ как вред от них очевиден.
...
int TextDbFile::AppendRecord(const SdRecord & rRec, const void * pDataBuf)
{
	...
	if(pos == 0) {
		//
		// ≈сли файл пустой, то добавл€ем специфицированное количество пустых строк
		// и (если необходимо) запись, содержащую наименовани€ полей.
		//
