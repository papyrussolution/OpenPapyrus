...
#define SERVERADDR				"ftp-edi.kontur.ru"
#define INBOX					"Inbox"
...
SEnumImp * FtpClient::Enum(const char * pWildcard)
{
	...
	public:
		...
		virtual int Next(void * pRec)
		{
			...
			WIN32_FIND_DATA find_data;
			if(!H) {
				H = FtpFindFirstFile(HSess, Wildcard, &find_data, 0, 0);
				if(H)
					ok = 1;
			}
			else {
...
public:
	...
	int    ParseForDocData2(Sdr_Bill * pBill);
...
// Разбираем полученный документ и заполняем Sdr_Bill
//
int ImportCls::ParseForDocData2(Sdr_Bill * pBill)
{
	int    ok = 0, exit_while = 0;
	SString str;
	xmlDoc * p_doc = 0;
	...
	// } @v8.5.6
	{
		SEancomXmlSegment main_seg(p_node);
		int    msg_type = main_seg.GetMsgTypeName();
		if(msg_type == MessageType) {
			SEancomXmlSegment seg;
			while(main_seg.GetNext(seg)) {
				SEancomXmlSegment::DTM dtm;
				SEancomXmlSegment::MOA moa;
				if(seg.Is("UNH")) {
					SEancomXmlSegment seg2;
					while(seg.GetNext(seg2)) {
						if(seg2.Is("E0062")) { // Message reference number
						}
						else if(seg2.Is("S009")) { // MESSAGE IDENTIFIER
							SEancomXmlSegment seg3;
							while(seg2.GetNext(seg3)) {
								if(seg3.Is("E0065")) { // Message Type
									int msg_type_inner = seg3.GetMsgTypeContent();
									if(msg_type_inner == msg_type || (msg_type_inner == PPEDIOP_DESADV && msg_type == PPEDIOP_ALCODESADV)) {
										; // ok
									}
									else {
									}
								}
								else if(seg3.Is("E0052")) { // Message version number
									// fixed "D"
								}
								else if(seg3.Is("E0054")) { // Message release number
									// fixed "01B"
								}
								else if(seg3.Is("E0051")) { // Controlling agency, coded
									// fixed "UN"
								}
								else if(seg3.Is("E0057")) { // Association assigned code
									// fixed "EAN007"
								}
							}
						}
				...
				else if(seg.Is("BGM")) { // Beginning of message
					SEancomXmlSegment seg2;
					while(seg.GetNext(seg2)) {
						if(seg2.Is("C002")) { // DOCUMENT/MESSAGE NAME
							SEancomXmlSegment seg3;
							while(seg2.GetNext(seg3)) {
								if(seg3.Is("E1001")) { // Document name code
								}
							}
						}
						else if(seg2.Is("C106")) { // DOCUMENT/MESSAGE IDENTIFICATION
							SEancomXmlSegment seg3;
							while(seg2.GetNext(seg3)) {
								if(seg3.Is("E1004")) { // Despatch Advice number assigned by the document sender
								}
							}
						}
						else if(seg2.Is("E1225")) { // Message function code
						}
					}
				}
				else if(seg.GetDate(dtm)) {
					switch(dtm.Type) {
						case SEancomXmlSegment::dtmMsg:
							break;
						case SEancomXmlSegment::dtmRef:
							break;
						case SEancomXmlSegment::dtmSched:
							break;
						case SEancomXmlSegment::dtmExpiry:
							break;
						case SEancomXmlSegment::dtmExpiryDays:
							break;
						default:
							break;
					}
				}
				else if(seg.GetMOA(moa)) { // Monetary amount
					/*
					switch(moa.Type) {
					}
					*/
...
void ProcessError(const char * pProcess)
{
	...
	FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, code, MAKELANGID(LANG_NEUTRAL,SUBLANG_DEFAULT), temp_err_buf, 256, NULL);
