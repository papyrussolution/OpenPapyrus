// REVERSK2.CPP
// Работа с контроллером Реверс-К2. Также реализован функционал для работы с турникетом Ростов-Дон через данный контроллер
//
#include <ppdrvapi.h>
#include <conio.h>
...
int K2Controller::K2Read(K2FrameSt & rResp)
{
	...
	while(!do_exit && ReadFile(Handle, &ch, 1, &ret_size, NULL) && ret_size) {
		if(ch == 0x55) {
			if(++count_55 == 10) {
				ReadFile(Handle, &ch, 1, &ret_size, NULL); // Считаем неинформативный байт и выйдем из цикла
				do_exit = 1;
			}
		}
		else
