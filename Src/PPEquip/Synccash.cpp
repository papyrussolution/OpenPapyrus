...
public:
	...
	virtual int PrintFiscalCorrection(const CcFiscalCorrection * pFc);
...
int SCS_SYNCCASH::PrintFiscalCorrection(const CcFiscalCorrection * pFc)
{
	int    ok = 1;
	SString temp_buf;
	...
	*/
	...
	if(pFc->FiscalSign.NotEmpty()) {
		THROW(ArrAdd(Arr_In, DVCPARAM_FISCALSIGN, pFc->FiscalSign)); 
	}
	// } @v12.3.3 
...
int SCS_SYNCCASH::PrintCheck(CCheckPacket * pPack, uint flags)
{
	...
	if(pPack->GetCount() == 0)
		...
		if(flags & PRNCHK_RETURN && amt_cash != 0.0) {
			const int is_cash = CheckForCash(amt_cash); // @v12.0.5 @fix amt-->amt_cash // @v12.3.4 (видимо в 12.0.5 что-то пошло не так и я не изменил amt на amt_cash)
			THROW(is_cash);
		...
		if(P_SlipFmt) {
			...
			if(r > 0) {
				...
				CheckForRibbonUsing(sdc_param.RegTo, Arr_In);
				{
					...
					if(pPack->IsCorrection()) {
						CcFiscalCorrection fc;
						fc.AmtCash = amt_cash;
						fc.AmtBank = amt_bnk;
						fc.AmtPostpay = amt_ccrd;
						if(oneof2(ccop, CCOP_CORRECTION_RET, CCOP_CORRECTION_RETSTORNO)) {
							fc.Flags |= CcFiscalCorrection::fIncome;
						}
						if(is_vat_free) {
							fc.Flags |= CcFiscalCorrection::fVatFree;
						}
						else {
							BVATAccmArray bva_list;
							pPack->EvaluateVatAmounts(bva_list);
							if(!bva_list.getCount()) {
								;
							}
							else if(bva_list.getCount() == 1) {
								fc.VatRate = bva_list.at(0).PRate;
							}
							else {
								for(uint i = 0; i < bva_list.getCount(); i++) {
									...
									if(vat_rate == 20.0)
										...
										fc.AmtVat18 = _amount;
									else if(vat_rate == 10.0)
										fc.AmtVat10 = _amount;
									else if(feqeps(vat_rate, 5.0, 1E-6))
										fc.AmtVat05 = _amount;
									else if(feqeps(vat_rate, 7.0, 1E-6))
										fc.AmtVat07 = _amount;
									else if(vat_rate == 0.0)
										fc.AmtVat00 = _amount;
								}
							}
						}
						pPack->GetExtStrData(CCheckPacket::extssFiscalSign, fc.FiscalSign);
						{
							THROW(ArrAdd(Arr_In, DVCPARAM_PAYMCASH, fc.AmtCash));
							THROW(ArrAdd(Arr_In, DVCPARAM_PAYMCARD, fc.AmtBank));
							THROW(ArrAdd(Arr_In, DVCPARAM_POSTPAY, fc.AmtPostpay));
							THROW(ArrAdd(Arr_In, DVCPARAM_PREPAY, fc.AmtPrepay));
							THROW(ArrAdd(Arr_In, DVCPARAM_CODE, fc.Code));
							THROW(ArrAdd(Arr_In, DVCPARAM_DATE, temp_buf.Z().Cat(fc.Dt, DATF_ISO8601CENT)));
							THROW(ArrAdd(Arr_In, DVCPARAM_TEXT, (temp_buf = fc.Reason).Transf(CTRANSF_INNER_TO_OUTER)));
							THROW(ArrAdd(Arr_In, DVCPARAM_FISCALSIGN, fc.FiscalSign));
							if(fc.Flags & fc.fVatFree) {
								THROW(ArrAdd(Arr_In, DVCPARAM_VATFREE, 1));
								THROW(ArrAdd(Arr_In, DVCPARAM_VATFREEAMOUNT, (fc.AmtCash + fc.AmtBank + fc.AmtPrepay + fc.AmtPostpay)));
							}
							else {
								if(fc.VatRate > 0.0) {
									THROW(ArrAdd(Arr_In, DVCPARAM_VATRATE, fc.VatRate));
								}
								else {
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT20, fabs(fc.AmtVat20)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT18, fabs(fc.AmtVat18)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT10, fabs(fc.AmtVat10)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT00, fabs(fc.AmtVat00)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT05, fabs(fc.AmtVat05)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATAMOUNT07, fabs(fc.AmtVat07)));
									THROW(ArrAdd(Arr_In, DVCPARAM_VATFREEAMOUNT, fabs(fc.AmtNoVat)));
								}
							}
						}
						//PrintFiscalCorrection(&fc);
						THROW(ExecPrintOper(DVCCMD_OPENCHECK_CORRECTION, Arr_In, Arr_Out));
					}
					else {
						...
						THROW(ArrAdd(Arr_In, DVCPARAM_OFDVER, temp_buf));
						if(paperless) {
...
int SCS_SYNCCASH::PrintIncasso(double sum, bool isIncome)
{
	ResCode = RESCODE_NO_ERROR;
	int    ok = 1;
	THROW(Connect());
