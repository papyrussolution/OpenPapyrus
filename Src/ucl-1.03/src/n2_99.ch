...
static void code_match(UCL_COMPRESS_T *c, ucl_uint m_len, const ucl_uint m_off)
{
    unsigned m_low = 0;
...
static void code_run(UCL_COMPRESS_T *c, const ucl_bytep ii, ucl_uint lit)
{
	if(lit) {
		c->lit_bytes += lit;
		if(lit > c->result[5])
...
#undef F
    ...
    /* a match */
        ...
        while(ahead < max_ahead && c->look > m_len) {
            s->max_chain = (m_len >= sc->good_length) ? (sc->max_chain >> 2) : sc->max_chain;
            r = find_match(c,s,1,0);
            ahead++;
            assert(r == 0);
            assert(c->look > 0);
...
lazy_match_done: ;
    ...
    *out_len = (ucl_uint) (c->bb_op - out);
    ...
    printf("%7ld %7ld -> %7ld   %7ld %7ld   %ld  (max: %d %d %d)\n",
		...
		c->match_bytes, c->lit_bytes, c->lazy, c->result[1], c->result[3], c->result[5]);
#endif
