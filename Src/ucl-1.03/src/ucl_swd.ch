...
*/
#if defined(__UCL_CHECKER)
#	define s_get_head3(s,key) ((s->llen3[key] == 0) ? SWD_UINT_MAX : s->head3[key])
#else
...
static void swd_initdict(ucl_swd_t *s, const ucl_bytep dict, ucl_uint dict_len)
{
...
static void swd_insertdict(ucl_swd_t *s, ucl_uint node, ucl_uint len)
{
    ucl_uint key;
...
static int swd_init(ucl_swd_t *s, const ucl_bytep dict, ucl_uint dict_len)
{
#if defined(SWD_USE_MALLOC)
    s->b = NULL;
    s->head3 = NULL;
    ...
    // @sobolev {
    // SWD_HSIZE    defined as  (SWD_HMASK + 1)
    // SWD_HMASK    defined as  (UCL_UINT32_C(65535)) = 65535
    // } @sobolev
...
static void swd_remove_node(ucl_swd_t *s, ucl_uint node)
{
    if(s->node_count == 0) {
        ucl_uint key;
#ifdef UCL_DEBUG
...
static void swd_accept(ucl_swd_t *s, ucl_uint n)
{
    assert(n <= s->look);
    if(n > 0) 
		do {
...
static void swd_search(ucl_swd_t *s, ucl_uint node, ucl_uint cnt)
{
    const ucl_bytep p1;
    const ucl_bytep p2;
    const ucl_bytep px;
...
static ucl_bool swd_search2(ucl_swd_t *s)
{
    ucl_uint key;
    assert(s->look >= 2);
    assert(s->m_len > 0);
