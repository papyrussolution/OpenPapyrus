...
static int init_match(UCL_COMPRESS_T *c, ucl_swd_t *s, const ucl_bytep dict, ucl_uint dict_len, ucl_uint32 flags)
{
    int r;
    assert(!c->init);
    c->init = 1;
...
static int find_match(UCL_COMPRESS_T *c, ucl_swd_t *s, ucl_uint this_len, ucl_uint skip)
{
    assert(c->init);
    if(skip > 0) {
